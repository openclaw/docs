...
<AccordionGroup>
  ...
  <Accordion title="api.runtime.state">
    ...
    An older binary restores its aggregate 50,000-row quota. If retained state exceeds that quota, new writes can fail or evict cache rows again. Keep the pre-update backup for rollback; do not delete durable ownership records to make an older binary accept the database.
