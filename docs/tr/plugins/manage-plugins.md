...
`optionalDependencies` bildiriyorsa kayıt tanılamalarını ve her Plugin'in
...
manifestler ve Plugin kayıt defterinden neleri keşfedebildiğini gösterir; zaten
çalışan bir Gateway işleminin Plugin çalışma zamanını içe aktardığını kanıtlamaz.
...
Plugin'in araçlar, hook'lar, hizmetler, Gateway yöntemleri veya Plugin'e ait CLI
komutları gibi çalışma zamanı yüzeylerini kaydettiğine dair kanıta ihtiyacınız
olduğunda `inspect --runtime` kullanın.
...
Bir Plugin `@beta` gibi bir npm dist-tag'inden yüklendiyse sonraki
`update <plugin-id>` çağrıları kaydedilen bu etiketi yeniden kullanır. Açık bir npm belirtimi geçirmek,
izlenen yüklemeyi gelecekteki güncellemeler için bu belirtime geçirir.
...
Plugin kayıtları önce eşleşen Plugin `@beta` yayını denemeye çalışır. Bu beta
yayını yoksa OpenClaw kaydedilen default/latest belirtimine geri döner.
Tam sürümler ve `@rc` veya `@beta` gibi açık etiketler korunur.
...
Kaldırma, geçerli olduğunda Plugin'in yapılandırma girdisini, Plugin dizin kaydını,
izin/verme listesi girdilerini ve bağlantılı yükleme yollarını kaldırır. Yönetilen
yükleme dizinleri, `--keep-files` geçmediğiniz sürece kaldırılır.

## Plugin'leri yayımlama

Harici Plugin'leri [ClawHub](https://clawhub.ai), npmjs.com veya
her ikisine yayımlayabilirsiniz.

### ClawHub'a yayımlama

ClawHub, OpenClaw Plugin'leri için birincil genel keşif yüzeyidir. Kullanıcılara
yüklemeden önce aranabilir metadata, sürüm geçmişi ve kayıt defteri tarama
sonuçları sağlar.
...
Kullanıcılar ClawHub'dan şununla yükler:
...
Yalın biçim yine önce ClawHub'ı denetler.

### npmjs.com'a yayımlama

Yerel npm Plugin'leri bir Plugin manifesti ve `package.json` OpenClaw
giriş noktası metadata'sı içermelidir.
...
Kullanıcılar yalnızca npm üzerinden şununla yükler:
...
npm çözümlemesini zorunlu kılar.
