...
summary: OpenClaw tanılama verilerini diagnostics-otel Plugin'i (OTLP/HTTP) aracılığıyla herhangi bir OpenTelemetry toplayıcısına dışa aktarın
title: OpenTelemetry dışa aktarımı
x-i18n:
    generated_at: "2026-05-06T09:14:35Z"
    model: gpt-5.5
    provider: openai
...
OpenClaw, resmi `diagnostics-otel` Plugin'i aracılığıyla tanılamaları dışa aktarır
ve **OTLP/HTTP (protobuf)** kullanır. OTLP/HTTP kabul eden herhangi bir collector
veya backend kod değişikliği olmadan çalışır. Yerel dosya günlükleri ve bunların
nasıl okunacağı için bkz. [Günlükleme](/tr/logging).

## Nasıl birlikte çalışır

- **Tanılama olayları**, Gateway ve birlikte gelen Plugin'ler tarafından model
  çalıştırmaları, ileti akışı, oturumlar, kuyruklar ve exec için yayımlanan
...
- **Sağlayıcı çağrıları**, sağlayıcı taşıması özel header'ları kabul ettiğinde
  ...
  header'ı alır. Plugin tarafından yayımlanan iz bağlamı yayılmaz.
- Exporter'lar yalnızca hem tanılama yüzeyi hem de Plugin etkin olduğunda
  bağlanır; bu nedenle işlem içi maliyet varsayılan olarak sıfıra yakın kalır.

## Hızlı başlangıç

Paketlenmiş kurulumlarda önce Plugin'i yükleyin:
...
| **İzler**    | Model kullanımı, model çağrıları, harness yaşam döngüsü, araç yürütme, exec, webhook/ileti işleme, bağlam birleştirme ve araç döngüleri için span'lar.              |
| **Günlükler** | `diagnostics.otel.logs` etkin olduğunda OTLP üzerinden dışa aktarılan yapılandırılmış `logging.file` kayıtları.                                                     |

`traces`, `metrics` ve `logs` seçeneklerini bağımsız olarak açıp
kapatabilirsiniz. `diagnostics.otel.enabled` true olduğunda üçünün de varsayılanı
açıktır.
...
| `OTEL_SEMCONV_STABILITY_OPT_IN`                                                                                   | Eski `gen_ai.system` yerine en son deneysel GenAI span özniteliğini (`gen_ai.provider.name`) yayımlamak için `gen_ai_latest_experimental` olarak ayarlayın. GenAI metrikleri ne olursa olsun her zaman sınırlı, düşük kardinaliteli semantik öznitelikler kullanır. |
| `OPENCLAW_OTEL_PRELOADED`                                                                                         | Başka bir preload veya ana makine işlemi global OpenTelemetry SDK'sını zaten kaydettiyse `1` olarak ayarlayın. Plugin bu durumda kendi NodeSDK yaşam döngüsünü atlar, ancak tanılama dinleyicilerini yine bağlar ve `traces`/`metrics`/`logs` ayarlarına uyar.       |

## Gizlilik ve içerik yakalama

Ham model/araç içeriği varsayılan olarak dışa **aktarılmaz**. Span'lar sınırlı
tanımlayıcılar (kanal, sağlayıcı, model, hata kategorisi, yalnızca hash içeren
istek kimlikleri) taşır ve hiçbir zaman prompt metni, yanıt metni, araç
...
oluşturulur. Çağıran tarafından sağlanan mevcut `traceparent` header'ları
değiştirilir; bu nedenle Plugin'ler veya özel sağlayıcı seçenekleri servisler
arası iz soyunu taklit edemez.

`diagnostics.otel.captureContent.*` değerini yalnızca collector'ınız ve saklama
politikanız prompt, yanıt, araç veya sistem prompt'u metni için onaylandıysa
...
Herhangi bir alt anahtar etkinleştirildiğinde, model ve araç span'ları yalnızca
...
- **Dosya günlüğü korelasyonu:** JSONL dosya günlükleri, günlük çağrısı geçerli
  ...
  `parentSpanId` ve `traceFlags` içerir; bu da günlük işleyicilerin yerel günlük
  satırlarını dışa aktarılan span'larla birleştirmesini sağlar.
- **İstek korelasyonu:** Gateway HTTP istekleri ve WebSocket frame'leri dahili
  ...
  varsayılan olarak istek izini devralır; agent çalıştırması ve model çağrısı
  span'ları ise çocuk olarak oluşturulur, böylece sağlayıcı `traceparent`
  header'ları aynı iz üzerinde kalır.
...
- `gen_ai.client.token.usage` (histogram, GenAI semantic-conventions metriği, attrs: `gen_ai.token.type` = `input`/`output`, `gen_ai.provider.name`, `gen_ai.operation.name`, `gen_ai.request.model`)
- `gen_ai.client.operation.duration` (histogram, saniye, GenAI semantic-conventions metriği, attrs: `gen_ai.provider.name`, `gen_ai.operation.name`, `gen_ai.request.model`, isteğe bağlı `error.type`)
- `openclaw.model_call.duration_ms` (histogram, attrs: `openclaw.provider`, `openclaw.model`, `openclaw.api`, `openclaw.transport`, sınıflandırılmış hatalarda ayrıca `openclaw.errorCategory` ve `openclaw.failureKind`)
- `openclaw.model_call.request_bytes` (histogram, son model isteği payload'unun UTF-8 bayt boyutu; ham payload içeriği yok)
- `openclaw.model_call.response_bytes` (histogram, akışa alınan model yanıtı olaylarının UTF-8 bayt boyutu; ham yanıt içeriği yok)
- `openclaw.model_call.time_to_first_byte_ms` (histogram, akışa alınan ilk yanıt olayından önce geçen süre)
...
- `openclaw.session.stuck` (counter, attrs: `openclaw.state`; yalnızca etkin işi olmayan eski oturum kayıt tutma işlemi için yayımlanır)
- `openclaw.session.stuck_age_ms` (histogram, attrs: `openclaw.state`; yalnızca etkin işi olmayan eski oturum kayıt tutma işlemi için yayımlanır)
...
blok veya ACP runtime ilerlemesi gözlemlerken bu eşik yönünde yaşlanmaz.
...
- `session.stalled`: etkin çalışma var, ancak etkin çalıştırma yakın zamanda
  ilerleme bildirmedi. Duraklamış gömülü çalıştırmalar başlangıçta yalnızca gözlem
  modunda kalır, ardından ilerleme olmazsa `diagnostics.stuckSessionAbortMs` sonrasında
  iptal edip boşaltır; böylece hattın arkasındaki kuyruktaki dönüşler sürdürülebilir.
  Ayarlanmadığında, iptal eşiği varsayılan olarak en az 10 dakika ve
  `diagnostics.stuckSessionWarnMs` değerinin 5 katı olan daha güvenli genişletilmiş
  pencereye döner.
- `session.stuck`: etkin çalışma olmadan bayat oturum kayıt yönetimi. Bu, etkilenen
  oturum hattını hemen serbest bırakır.

Kurtarma, yapılandırılmış `session.recovery.requested` ve
`session.recovery.completed` olayları yayar. Tanılama oturum durumu yalnızca değişiklik
yapan bir kurtarma sonucu (`aborted` veya `released`) sonrasında ve yalnızca aynı
işleme nesli hâlâ geçerliyse boşta olarak işaretlenir.
...
iz aralığını yayar. Tekrarlanan `session.stuck` tanılamaları, oturum değişmeden kaldığı
sürece geri çekilir; bu nedenle panolar her Heartbeat tikinde değil, sürdürülen
artışlarda uyarı vermelidir. Yapılandırma düğmesi ve varsayılanlar için
...
### Tanılama iç işleyişleri (bellek ve araç döngüsü)
...
- `openclaw.model.call`
  ...
  - `openclaw.provider.request_id_hash` (üst sağlayıcı istek kimliğinin SHA tabanlı sınırlı karması; ham kimlikler dışa aktarılmaz)
...
İçerik yakalama açıkça etkinleştirildiğinde, model ve araç iz aralıkları seçtiğiniz
...
Aşağıdaki olaylar yukarıdaki metrikleri ve iz aralıklarını destekler. Plugin'ler
OTLP dışa aktarımı olmadan da bunlara doğrudan abone olabilir.
...
- `model.usage` - token'lar, maliyet, süre, bağlam, sağlayıcı/model/kanal,
  ...
  `context.used` geçerli istem/bağlam anlık görüntüsüdür ve önbelleğe alınmış girdi
  veya araç döngüsü çağrıları söz konusu olduğunda sağlayıcı `usage.total`
...
- `diagnostic.heartbeat` (toplam sayaçlar: webhook'lar/kuyruk/oturum)

**Koşum yaşam döngüsü**

- `harness.run.started` / `harness.run.completed` / `harness.run.error` -
  ajan koşumu için çalıştırma başına yaşam döngüsü. `harnessId`, isteğe bağlı
...
bayraklarını kullanın. Bayraklar büyük/küçük harfe duyarlı değildir ve joker
...
- [Gateway günlükleme iç işleyişleri](/tr/gateway/logging) - WS günlük stilleri, alt sistem önekleri ve konsol yakalama
