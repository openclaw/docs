---
read_when:
    - Alan düzeyindeki kesin yapılandırma semantiğine veya varsayılan değerlere ihtiyacınız var
...
OpenClaw ana config başvurusu: `~/.openclaw/openclaw.json`. Görev odaklı bir genel bakış için bkz. [Yapılandırma](/tr/gateway/configuration).

Ana OpenClaw config yüzeylerini kapsar ve bir alt sistemin kendi daha derin başvurusu olduğunda bağlantı verir. Kanal ve Plugin sahipli komut katalogları ile derin bellek/QMD ayarları bu sayfa yerine kendi sayfalarında yer alır.
...
- `config.schema.lookup`, ayrıntılı inceleme araçları için yol kapsamlı tek bir schema düğümü döndürür
- `pnpm config:docs:check` / `pnpm config:docs:gen`, config-doc temel hash'ini geçerli schema yüzeyine göre doğrular
...
Config biçimi **JSON5**'tir (yorumlara + sondaki virgüllere izin verilir). Tüm alanlar isteğe bağlıdır — OpenClaw atlandıklarında güvenli varsayılanlar kullanır.

---

## Kanallar

Kanal başına config anahtarları özel bir sayfaya taşındı — Slack, Discord, Telegram, WhatsApp, Matrix, iMessage ve diğer paketle gelen kanallar (kimlik doğrulama, erişim kontrolü, çoklu hesap, bahsetme geçidi) dahil `channels.*` için bkz. [Yapılandırma — kanallar](/tr/gateway/config-channels).
...
Araç ilkesi, deneysel anahtarlar, sağlayıcı destekli araç config'i ve özel sağlayıcı / base-URL kurulumu özel bir sayfaya taşındı — bkz. [Yapılandırma — araçlar ve özel sağlayıcılar](/tr/gateway/config-tools).

## Modeller

Sağlayıcı tanımları, model izin listeleri ve özel sağlayıcı kurulumu [Yapılandırma — araçlar ve özel sağlayıcılar](/tr/gateway/config-tools#custom-providers-and-base-urls) içinde yer alır. `models` kökü ayrıca küresel model kataloğu davranışına sahiptir.
...
OpenClaw tarafından yönetilen MCP sunucu tanımları `mcp.servers` altında yer alır ve gömülü Pi ile diğer çalışma zamanı adaptörleri tarafından tüketilir. `openclaw mcp list`, `show`, `set` ve `unset` komutları, config düzenlemeleri sırasında hedef sunucuya bağlanmadan bu bloğu yönetir.
...
- `mcp.sessionIdleTtlMs`: oturum kapsamlı paketle gelen MCP çalışma zamanları için boşta kalma TTL'si. Tek seferlik gömülü çalıştırmalar çalıştırma sonu temizliği ister; bu TTL uzun ömürlü oturumlar ve gelecekteki çağıranlar için son güvencedir.
- `mcp.*` altındaki değişiklikler, önbelleğe alınmış oturum MCP çalışma zamanlarını dispose ederek sıcak uygulanır. Bir sonraki araç keşfi/kullanımı bunları yeni config'ten yeniden oluşturur; bu yüzden kaldırılan `mcp.servers` girdileri boşta kalma TTL'sini beklemek yerine hemen temizlenir.
...
- Keşif, yerel OpenClaw Plugin'leri ile uyumlu Codex paketlerini ve Claude paketlerini kabul eder; manifest'siz Claude varsayılan düzen paketleri buna dahildir.
...
- `plugins.entries.<id>.hooks.allowPromptInjection`: `false` olduğunda core, `before_prompt_build` öğesini engeller ve legacy `before_agent_start` içinden prompt'u değiştiren alanları yok sayarken legacy `modelOverride` ve `providerOverride` öğelerini korur. Yerel Plugin hook'larına ve desteklenen paket tarafından sağlanan hook dizinlerine uygulanır.
- `plugins.entries.<id>.hooks.allowConversationAccess`: `true` olduğunda, güvenilen paketle gelmeyen Plugin'ler `llm_input`, `llm_output`, `before_agent_finalize` ve `agent_end` gibi tipli hook'lardan ham konuşma içeriğini okuyabilir.
- `plugins.entries.<id>.subagent.allowModelOverride`: bu Plugin'in arka plan subagent çalıştırmaları için çalıştırma başına `provider` ve `model` override'ları istemesine açıkça güven.
- `plugins.entries.<id>.subagent.allowedModels`: güvenilen subagent override'ları için standart `provider/model` hedeflerinin isteğe bağlı izin listesi. `"*"` değerini yalnızca herhangi bir modele izin vermeyi bilerek istediğinizde kullanın.
...
- `plugins.entries.firecrawl.config.webFetch`: Firecrawl web-fetch sağlayıcı ayarları.
  - `apiKey`: Firecrawl API anahtarı (SecretRef kabul eder). `plugins.entries.firecrawl.config.webSearch.apiKey`, legacy `tools.web.fetch.firecrawl.apiKey` veya `FIRECRAWL_API_KEY` env var değerine geri düşer.
...
- `plugins.entries.memory-core.config.dreaming`: bellek dreaming ayarları. Aşamalar ve eşikler için bkz. [Dreaming](/tr/concepts/dreaming).
  ...
  - `model`: isteğe bağlı Dream Diary subagent model override'ı. `plugins.entries.memory-core.subagent.allowModelOverride: true` gerektirir; hedefleri kısıtlamak için `allowedModels` ile eşleştirin. Model-kullanılamaz hataları oturum varsayılan modeliyle bir kez yeniden denenir; güven veya izin listesi başarısızlıkları sessizce geri düşmez.
  - Aşama ilkesi ve eşikler uygulama ayrıntılarıdır (kullanıcıya yönelik config anahtarları değildir).
- Tam bellek config'i [Bellek yapılandırması başvurusu](/tr/reference/memory-config) içinde yer alır:
  - `agents.defaults.memorySearch.*`
  - `memory.backend`
  - `memory.citations`
  - `memory.qmd.*`
  - `plugins.entries.memory-core.config.dreaming`
- Etkin Claude paket Plugin'leri `settings.json` içinden gömülü Pi varsayılanlarına da katkıda bulunabilir; OpenClaw bunları ham OpenClaw config yamaları olarak değil, temizlenmiş agent ayarları olarak uygular.
...
- `plugins.slots.contextEngine`: etkin context engine Plugin kimliğini seçin; başka bir engine yükleyip seçmediğiniz sürece varsayılan `"legacy"` olur.
...
`commitments`, çıkarımsal takip belleğini kontrol eder: OpenClaw, konuşma turlarından check-in'leri algılayabilir ve bunları Heartbeat çalıştırmaları üzerinden teslim edebilir.
...
- `commitments.maxPerDay`: kayan bir gün içinde agent oturumu başına teslim edilen maksimum çıkarımsal takip taahhüdü. Varsayılan: `3`.
...
- `profiles.*.cdpUrl`, `http://`, `https://`, `ws://` ve `wss://` kabul eder.
  OpenClaw’ın `/json/version` keşfi yapmasını istediğinizde HTTP(S) kullanın;
...
- `remoteCdpTimeoutMs` ve `remoteCdpHandshakeTimeoutMs`, uzak ve `attachOnly`
  CDP erişilebilirliğine ve sekme açma isteklerine uygulanır. Yönetilen local loopback
...
- Yerel yönetilen profiller, süreç başlatıldıktan sonra Chrome CDP HTTP
  keşfi için `browser.localLaunchTimeoutMs` ve başlatma sonrası CDP websocket
  hazır olma durumu için `browser.localCdpReadyTimeoutMs` kullanır. Chrome’un
  başarıyla başladığı ancak hazır olma kontrollerinin başlangıçla yarıştığı daha yavaş ana makinelerde bunları artırın. Her iki değer de
...
- `browser.executablePath` ve `browser.profiles.<name>.executablePath` Chromium başlatılmadan önce
  işletim sisteminizin ana dizini için `~` ve `~/...` değerlerini kabul eder.
  `existing-session` profillerindeki profil başına `userDataDir` de tilde genişletmesine tabi tutulur.
- Kontrol servisi: yalnızca loopback (bağlantı noktası `gateway.port` değerinden türetilir, varsayılan `18791`).
...
- **Eski bind takma adları**: `gateway.bind` içinde host takma adları (`0.0.0.0`, `127.0.0.1`, `localhost`, `::`, `::1`) yerine bind modu değerlerini (`auto`, `loopback`, `lan`, `tailnet`, `custom`) kullanın.
- **Docker notu**: varsayılan `loopback` bind'i, kapsayıcı içinde `127.0.0.1` üzerinde dinler. Docker köprü ağıyla (`-p 18789:18789`) trafik `eth0` üzerinden gelir, bu nedenle gateway'e erişilemez. Tüm arayüzlerde dinlemek için `--network host` kullanın veya `bind: "lan"` (ya da `customBindHost: "0.0.0.0"` ile `bind: "custom"`) ayarlayın.
- **Kimlik doğrulama**: varsayılan olarak gereklidir. Loopback dışı bind'ler gateway kimlik doğrulaması gerektirir. Pratikte bu, paylaşılan bir token/parola veya `gateway.auth.mode: "trusted-proxy"` ile kimlik farkındalığı olan bir ters proxy anlamına gelir. Başlatma sihirbazı varsayılan olarak bir token oluşturur.
- Hem `gateway.auth.token` hem de `gateway.auth.password` yapılandırılmışsa (SecretRef'ler dahil), `gateway.auth.mode` değerini açıkça `token` veya `password` olarak ayarlayın. İkisi de yapılandırılmışken mod ayarlanmamışsa başlatma ve hizmet kurulum/onarım akışları başarısız olur.
- `gateway.auth.mode: "none"`: açık kimlik doğrulamasız mod. Yalnızca güvenilen local loopback kurulumları için kullanın; bu, bilinçli olarak başlatma istemlerinde sunulmaz.
...
- `gateway.auth.rateLimit`: isteğe bağlı başarısız kimlik doğrulama sınırlayıcısı. İstemci IP'si ve kimlik doğrulama kapsamı başına uygulanır (shared-secret ve device-token bağımsız olarak izlenir). Engellenen denemeler `429` + `Retry-After` döndürür.
  - Asenkron Tailscale Serve Control UI yolunda, aynı `{scope, clientIp}` için başarısız denemeler, hata yazımından önce sıraya alınır. Bu nedenle aynı istemciden gelen eşzamanlı hatalı denemeler, ikisinin de düz eşleşmeme olarak yarışıp geçmesi yerine ikinci istekte sınırlayıcıyı tetikleyebilir.
...
- Tarayıcı kaynaklı WS kimlik doğrulama denemeleri her zaman loopback muafiyeti devre dışı bırakılarak kısıtlanır (tarayıcı tabanlı localhost kaba kuvvet saldırılarına karşı derinlemesine savunma).
- Loopback üzerinde, bu tarayıcı kaynaklı kilitlemeler normalleştirilmiş `Origin`
  değerine göre yalıtılır; böylece bir localhost origin'inden gelen tekrarlı başarısızlıklar farklı bir origin'i otomatik olarak
  kilitlemez.
...
- `controlUi.dangerouslyAllowHostHeaderOriginFallback`: Host-header origin ilkesine bilinçli olarak dayanan dağıtımlar için Host-header origin yedeğini etkinleştiren tehlikeli mod.
...
- `gateway.push.apns.relay.baseUrl`: resmi/TestFlight iOS derlemelerinin relay destekli kayıtları gateway'e yayımlamasından sonra kullandığı harici APNs relay'i için temel HTTPS URL'si. Bu URL, iOS derlemesine derlenmiş relay URL'siyle eşleşmelidir.
- `gateway.push.apns.relay.timeoutMs`: gateway'den relay'e gönderim zaman aşımı, milisaniye cinsinden. Varsayılan değer `10000`.
- Relay destekli kayıtlar belirli bir gateway kimliğine devredilir. Eşleştirilmiş iOS uygulaması `gateway.identity.get` çağrısıyla kimliği alır, bu kimliği relay kaydına dahil eder ve kayıt kapsamlı gönderme yetkisini gateway'e iletir. Başka bir gateway bu saklanan kaydı yeniden kullanamaz.
...
- `gateway.auth.token` / `gateway.auth.password` SecretRef üzerinden açıkça yapılandırılmış ve çözümlenmemişse, çözümleme kapalı başarısız olur (uzak yedeklemenin maskelemesi olmaz).
- `trustedProxies`: TLS'i sonlandıran veya iletilmiş istemci başlıkları ekleyen ters proxy IP'leri. Yalnızca kontrol ettiğiniz proxy'leri listeleyin. Loopback girdileri aynı host proxy/yerel algılama kurulumları için (örneğin Tailscale Serve veya yerel bir ters proxy) hâlâ geçerlidir, ancak loopback isteklerini `gateway.auth.mode: "trusted-proxy"` için uygun hâle **getirmezler**.
...
- `gateway.nodes.pairing.autoApproveCidrs`: istenen kapsam olmadan ilk kez node cihaz eşleştirmesini otomatik onaylamak için isteğe bağlı CIDR/IP izin listesi. Ayarlanmadığında devre dışıdır. Bu, operatör/tarayıcı/Control UI/WebChat eşleştirmesini otomatik onaylamaz ve rol, kapsam, meta veri veya genel anahtar yükseltmelerini otomatik onaylamaz.
- `gateway.nodes.allowCommands` / `gateway.nodes.denyCommands`: eşleştirme ve platform izin listesi değerlendirmesinden sonra bildirilen node komutları için genel izin/ret şekillendirmesi. `camera.snap`, `camera.clip` ve `screen.record` gibi tehlikeli node komutlarını etkinleştirmek için `allowCommands` kullanın; `denyCommands`, bir platform varsayılanı veya açık izin aksi hâlde onu dahil edecek olsa bile bir komutu kaldırır. Bir node bildirilen komut listesini değiştirdikten sonra, gateway'in güncellenmiş komut anlık görüntüsünü saklaması için bu cihaz eşleştirmesini reddedin ve yeniden onaylayın.
...
- `debounceMs`: yapılandırma değişiklikleri uygulanmadan önce ms cinsinden bekletme penceresi (negatif olmayan tam sayı).
- `deferralTimeoutMs`: yeniden başlatmayı zorlamadan önce devam eden işlemleri beklemek için ms cinsinden isteğe bağlı en uzun süre. Varsayılan sınırlı beklemeyi (`300000`) kullanmak için bunu atlayın; süresiz beklemek ve periyodik hâlâ beklemede uyarıları günlüğe yazmak için `0` olarak ayarlayın.
...
Sorgu dizesi hook belirteçleri reddedilir.
...
- `transform`, bir hook eylemi döndüren bir JS/TS modülünü gösterebilir.
  ...
  - `hooks.transformsDir` değerini `~/.openclaw/hooks/transforms` altında tutun; çalışma alanı skill dizinleri reddedilir. `openclaw doctor` bu yolu geçersiz olarak bildirirse transform modülünü hooks transforms dizinine taşıyın veya `hooks.transformsDir` değerini kaldırın.
- `agentId` belirli bir ajana yönlendirir; bilinmeyen ID'ler varsayılana geri döner.
- `allowedAgentIds`: açık yönlendirmeyi sınırlar (`*` veya belirtilmemiş = tümüne izin ver, `[]` = tümünü reddet).
- `defaultSessionKey`: açık `sessionKey` olmadan yapılan hook ajan çalıştırmaları için isteğe bağlı sabit oturum anahtarı.
- `allowRequestSessionKey`: `/hooks/agent` çağırıcılarının ve şablon güdümlü eşleme oturum anahtarlarının `sessionKey` ayarlamasına izin verin (varsayılan: `false`).
...
- Gateway yanında ayrı bir `gog gmail watch serve` çalıştırmayın.
...
- Loopback dışı bind'ler: canvas rotaları, diğer Gateway HTTP yüzeyleriyle aynı şekilde Gateway kimlik doğrulaması (belirteç/parola/trusted-proxy) gerektirir.
- Node WebView'leri genellikle kimlik doğrulama başlıkları göndermez; bir node eşleştirilip bağlandıktan sonra Gateway, canvas/A2UI erişimi için node kapsamlı yetenek URL'leri duyurur.
...
- `full`: `cliPath` + `sshPort` değerlerini içerir; LAN çok noktaya yayın reklamı yine de yerleşik `bonjour` Plugin'in etkin olmasını gerektirir.
- `off`: Plugin etkinliğini değiştirmeden LAN çok noktaya yayın reklamını bastırır.
- Yerleşik `bonjour` Plugin, macOS ana makinelerinde otomatik başlar ve Linux, Windows ile kapsayıcılaştırılmış Gateway dağıtımlarında isteğe bağlıdır.
...
`~/.openclaw/dns/` altında bir unicast DNS-SD bölgesi yazar. Ağlar arası keşif için bir DNS sunucusu (CoreDNS önerilir) + Tailscale split DNS ile eşleştirin.
...
- `.env` dosyaları: geçerli çalışma dizini `.env` + `~/.openclaw/.env` (hiçbiri mevcut değişkenleri geçersiz kılmaz).
...
- `source: "exec"` id’leri `.` veya `..` eğik çizgiyle sınırlandırılmış yol segmentleri içermemelidir (örneğin `a/../b` reddedilir)
...
- `exec` alt ortamı varsayılan olarak en az düzeydedir; gerekli değişkenleri `passEnv` ile açıkça iletin.
...
- Etkinleştirme sırasında etkin yüzey filtrelemesi uygulanır: etkin yüzeylerdeki çözümlenmemiş başvurular başlatma/yeniden yükleme işlemini başarısız kılar, etkin olmayan yüzeyler ise tanılamalarla atlanır.
...
- Eski OAuth içe aktarmaları `~/.openclaw/credentials/oauth.json` konumundan gelir.
...
- `billingBackoffHours`: bir profil gerçek
  ...
  kuruluş/çalışma alanı harcama sınırı mesajları bunun yerine `rate_limit` yolunda
  kalır.
...
- `consoleLevel`, `--verbose` kullanıldığında `debug` değerine yükselir.
- `maxFileBytes`: rotasyondan önce etkin günlük dosyasının bayt cinsinden maksimum boyutu (pozitif tam sayı; varsayılan: `104857600` = 100 MB). OpenClaw, etkin dosyanın yanında en fazla beş numaralı arşiv tutar.
- `redactSensitive` / `redactPatterns`: konsol çıktısı, dosya günlükleri, OTLP günlük kayıtları ve kalıcı oturum transkripti metni için en iyi çaba ile maskeleme. `redactSensitive: "off"` yalnızca bu genel günlük/transkript ilkesini devre dışı bırakır; UI/araç/tanı güvenliği yüzeyleri sırları yaymadan önce yine de redakte eder.
...
- `stuckSessionWarnMs`: uzun süren işleme oturumlarını `session.long_running`, `session.stalled` veya `session.stuck` olarak sınıflandırmak için ms cinsinden ilerleme olmayan yaş eşiği. Yanıt, araç, durum, blok ve ACP ilerlemesi zamanlayıcıyı sıfırlar; tekrarlanan `session.stuck` tanıları değişmediği sürece geri çekilir.
- `otel.enabled`: OpenTelemetry dışa aktarma hattını etkinleştirir (varsayılan: `false`). Tam yapılandırma, sinyal kataloğu ve gizlilik modeli için [OpenTelemetry dışa aktarma](/tr/gateway/opentelemetry) bölümüne bakın.
...
- `OPENCLAW_OTEL_PRELOADED=1`: genel bir OpenTelemetry SDK'sını zaten kaydetmiş ana makineler için ortam anahtarı. OpenClaw bu durumda tanı dinleyicilerini etkin tutarken Plugin tarafından sahiplenilen SDK başlatma/kapatma işlemlerini atlar.
- `OTEL_EXPORTER_OTLP_TRACES_ENDPOINT`, `OTEL_EXPORTER_OTLP_METRICS_ENDPOINT` ve `OTEL_EXPORTER_OTLP_LOGS_ENDPOINT`: eşleşen yapılandırma anahtarı ayarlanmamışken kullanılan sinyale özgü uç nokta ortam değişkenleri.
...
- `checkOnStart`: Gateway başladığında npm güncellemelerini kontrol eder (varsayılan: `true`).
...
- `auto.stableJitterHours`: kararlı kanal dağıtımı için saat cinsinden ek yayılım penceresi (varsayılan: `12`; maks: `168`).
- `auto.betaCheckIntervalHours`: beta kanal kontrollerinin saat cinsinden ne sıklıkta çalışacağı (varsayılan: `1`; maks: `24`).
...
- `backend`: varsayılan ACP runtime backend kimliği (kayıtlı bir ACP runtime Plugin ile eşleşmelidir).
  Önce backend Plugin'i kurun; `plugins.allow` ayarlanmışsa backend Plugin kimliğini (örneğin `acpx`) dahil edin, aksi halde ACP backend yüklenmez.
- `defaultAgent`: spawn'lar açık bir hedef belirtmediğinde yedek ACP hedef agent kimliği.
- `allowedAgents`: ACP runtime oturumları için izin verilen agent kimliklerinin izin listesi; boş olması ek kısıtlama olmadığı anlamına gelir.
- `maxConcurrentSessions`: aynı anda etkin olabilecek maksimum ACP oturumu sayısı.
- `stream.coalesceIdleMs`: akışla gönderilen metin için ms cinsinden boşta bekleme sonrası boşaltma penceresi.
- `stream.maxChunkChars`: akışla gönderilen blok projeksiyonu bölünmeden önceki maksimum parça boyutu.
- `stream.repeatSuppression`: her tur için tekrarlanan durum/araç satırlarını bastırır (varsayılan: `true`).
...
- `stream.maxOutputChars`: ACP turu başına yansıtılan maksimum asistan çıktı karakteri.
- `stream.maxSessionUpdateChars`: yansıtılan ACP durum/güncelleme satırları için maksimum karakter sayısı.
- `stream.tagVisibility`: akış olayları için tag adlarından boolean görünürlük geçersiz kılmalarına kayıt.
- `runtime.ttlMinutes`: ACP oturum worker'larının temizlemeye uygun hale gelmeden önce dakika cinsinden boşta kalma TTL'si.
...
- `sessionRetention`: tamamlanmış izole cron çalıştırma oturumlarının `sessions.json` içinden budanmadan önce ne kadar süre tutulacağı. Arşivlenmiş silinmiş cron transkriptlerinin temizlenmesini de denetler. Varsayılan: `24h`; devre dışı bırakmak için `false` ayarlayın.
...
- `webhook`: yalnızca hâlâ `notify: true` değerine sahip depolanmış işler için kullanılan, kullanımdan kaldırılmış eski yedek Webhook URL'si (http/https).
...
- `mode`: teslim modu — `"announce"` bir kanal iletisi aracılığıyla gönderir; `"webhook"` yapılandırılmış Webhook'a gönderi yapar.
...
- Ne genel ne de iş başına hata hedefi ayarlandığında, zaten `announce` aracılığıyla teslim eden işler hata durumunda birincil duyuru hedefine geri döner.
...
| `{{GroupSubject}}` | Grup konusu (eldeki en iyi çaba)                  |
| `{{GroupMembers}}` | Grup üyeleri önizlemesi (eldeki en iyi çaba)      |
| `{{SenderName}}`   | Gönderen görünen adı (eldeki en iyi çaba)         |
| `{{SenderE164}}`   | Gönderen telefon numarası (eldeki en iyi çaba)    |
...
- İç içe dahil etmeler: en fazla 10 seviye derinlik.
- Yollar: dahil eden dosyaya göre çözümlenir, ancak üst düzey yapılandırma dizininin (`openclaw.json` öğesinin `dirname` değeri) içinde kalmalıdır. Mutlak/`../` biçimlerine yalnızca yine de bu sınır içinde çözümlendiklerinde izin verilir.
- Yalnızca tek dosyalı dahil etme ile desteklenen bir üst düzey bölümü değiştiren OpenClaw'a ait yazmalar, o dahil edilen dosyaya yazılır. Örneğin, `plugins install`, `plugins: { $include: "./plugins.json5" }` değerini `plugins.json5` içinde günceller ve `openclaw.json` dosyasını olduğu gibi bırakır.
- Kök dahil etmeler, dahil etme dizileri ve kardeş geçersiz kılmaları olan dahil etmeler, OpenClaw'a ait yazmalar için salt okunurdur; bu yazmalar yapılandırmayı düzleştirmek yerine kapalı başarısız olur.
