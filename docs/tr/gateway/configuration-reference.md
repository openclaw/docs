---
read_when:
    - Alan düzeyindeki yapılandırma semantiği veya varsayılan değerler hakkında kesin bilgiye ihtiyacınız var
    - Kanal, model, Gateway veya araç yapılandırma bloklarını doğruluyorsunuz
summary: Çekirdek OpenClaw anahtarları, varsayılan değerler ve özel alt sistem referanslarına bağlantılar için Gateway yapılandırma referansı
...
Temel yapılandırma başvurusu: `~/.openclaw/openclaw.json`. Görev odaklı genel bakış için bkz. [Yapılandırma](/tr/gateway/configuration).
...
- `openclaw config schema`, doğrulama ve Denetim kullanıcı arayüzü için kullanılan canlı JSON Şemasını yazdırır; mevcut olduğunda paketlenmiş/Plugin/kanal meta verileri birleştirilir
- `config.schema.lookup`, ayrıntılı inceleme araçları için yol kapsamlı tek bir şema düğümü döndürür
- `pnpm config:docs:check` / `pnpm config:docs:gen`, yapılandırma belgesi temel hash değerini mevcut şema yüzeyine göre doğrular
...
Kanal başına yapılandırma anahtarları özel bir sayfaya taşındı - Slack, Discord, Telegram, WhatsApp, Matrix, iMessage ve diğer paketlenmiş kanallar (kimlik doğrulama, erişim denetimi, çoklu hesap, bahsetme geçidi) dahil `channels.*` için bkz. [Yapılandırma - kanallar](/tr/gateway/config-channels).
...
- `talk.*` (Talk modu)
  - `talk.speechLocale`: iOS/macOS'ta Talk konuşma tanıma için isteğe bağlı BCP 47 yerel ayar kimliği
...
Sağlayıcı tanımları, model izin listeleri ve özel sağlayıcı kurulumu [Yapılandırma - araçlar ve özel sağlayıcılar](/tr/gateway/config-tools#custom-providers-and-base-urls) içinde bulunur. `models` kökü ayrıca küresel model kataloğu davranışına sahiptir.
...
- `mcp.servers`: yapılandırılmış MCP araçlarını açığa çıkaran çalışma zamanları için adlandırılmış stdio veya uzak MCP sunucu tanımları. Uzak girdiler `transport: "streamable-http"` veya `transport: "sse"` kullanır; `type: "http"`, `openclaw mcp set` ve `openclaw doctor --fix` tarafından standart `transport` alanına normalleştirilen CLI yerel bir takma addır.
- `mcp.sessionIdleTtlMs`: oturum kapsamlı paketlenmiş MCP çalışma zamanları için boşta kalma TTL değeri. Tek seferlik gömülü çalıştırmalar, çalıştırma sonunda temizlik ister; bu TTL, uzun ömürlü oturumlar ve gelecekteki çağıranlar için geri güvencedir.
- `mcp.*` altındaki değişiklikler, önbelleğe alınmış oturum MCP çalışma zamanlarını elden çıkararak sıcak uygulanır. Bir sonraki araç keşfi/kullanımı bunları yeni yapılandırmadan yeniden oluşturur; bu nedenle kaldırılan `mcp.servers` girdileri boşta kalma TTL'sini beklemek yerine hemen temizlenir.
...
- `install.preferBrew`: true olduğunda, `brew` mevcutsa diğer yükleyici türlerine geri dönmeden önce Homebrew yükleyicilerini tercih eder.
...
- Keşif, yerel OpenClaw Plugins ile uyumlu Codex paketlerini ve Claude paketlerini kabul eder; buna manifestsiz Claude varsayılan düzen paketleri dahildir.
...
- `bundledDiscovery`: yeni yapılandırmalar için varsayılan olarak `"allowlist"` değerini alır; böylece boş olmayan bir `plugins.allow`, web arama çalışma zamanı sağlayıcıları dahil paketlenmiş sağlayıcı Plugins için de geçit olur. Doctor, mevcut paketlenmiş sağlayıcı davranışını siz katılana kadar korumak için taşınmış eski izin listesi yapılandırmalarına `"compat"` yazar.
...
- `plugins.entries.<id>.hooks.allowConversationAccess`: `true` olduğunda güvenilen paketlenmemiş Plugins, `llm_input`, `llm_output`, `before_model_resolve`, `before_agent_reply`, `before_agent_run`, `before_agent_finalize` ve `agent_end` gibi türlenmiş hook'lardan ham konuşma içeriğini okuyabilir.
- `plugins.entries.<id>.subagent.allowModelOverride`: bu Plugin'e arka plan subagent çalıştırmaları için çalıştırma başına `provider` ve `model` geçersiz kılmaları istemesi için açıkça güvenin.
- `plugins.entries.<id>.subagent.allowedModels`: güvenilen subagent geçersiz kılmaları için standart `provider/model` hedeflerinin isteğe bağlı izin listesi. `"*"` değerini yalnızca herhangi bir modele izin vermeyi bilinçli olarak istediğinizde kullanın.
...
- Kanal Plugin hesabı/çalışma zamanı ayarları `channels.<id>` altında bulunur ve merkezi bir OpenClaw seçenek kayıt defteri tarafından değil, sahip Plugin'in manifest `channelConfigs` meta verileri tarafından açıklanmalıdır.
...
- `plugins.entries.memory-core.config.dreaming`: bellek Dreaming ayarları. Evreler ve eşikler için bkz. [Dreaming](/tr/concepts/dreaming).
  ...
  - `model`: isteğe bağlı Dream Diary subagent modeli geçersiz kılması. `plugins.entries.memory-core.subagent.allowModelOverride: true` gerektirir; hedefleri sınırlamak için `allowedModels` ile eşleştirin. Modelin kullanılamaması hataları, oturum varsayılan modeliyle bir kez yeniden denenir; güven veya izin listesi hataları sessizce geri düşmez.
  - evre ilkesi ve eşikler uygulama ayrıntılarıdır (kullanıcıya dönük yapılandırma anahtarları değildir).
- Tam bellek yapılandırması [Bellek yapılandırması başvurusu](/tr/reference/memory-config) içinde bulunur:
  - `agents.defaults.memorySearch.*`
  - `memory.backend`
  - `memory.citations`
  - `memory.qmd.*`
  - `plugins.entries.memory-core.config.dreaming`
- Etkin Claude paket Plugins, `settings.json` içinden gömülü Pi varsayılanları da katkı olarak sağlayabilir; OpenClaw bunları ham OpenClaw yapılandırma yamaları olarak değil, arındırılmış agent ayarları olarak uygular.
...
`commitments`, çıkarımsanan takip belleğini denetler: OpenClaw konuşma dönüşlerinden check-in'leri algılayabilir ve bunları Heartbeat çalıştırmaları aracılığıyla teslim edebilir.

- `commitments.enabled`: çıkarımsanan takip taahhütleri için gizli LLM çıkarımı, depolama ve Heartbeat teslimini etkinleştirir. Varsayılan: `false`.
- `commitments.maxPerDay`: kayan bir gün içinde agent oturumu başına teslim edilen maksimum çıkarımsanan takip taahhüdü sayısı. Varsayılan: `3`.
...
- `tabCleanup`, boşta kalma süresinden sonra veya bir oturum sınırını aştığında izlenen birincil ajan sekmelerini geri kazanır. Bu tekil temizleme modlarını devre dışı bırakmak için `idleMinutes: 0` veya `maxTabsPerSession: 0` ayarlayın.
...
- Uzak profiller yalnızca ekleme modundadır (başlatma/durdurma/sıfırlama devre dışı).
...
- Harici olarak yönetilen bir CDP hizmetine loopback üzerinden erişilebiliyorsa, bu profilin `attachOnly: true` değerini ayarlayın; aksi takdirde OpenClaw loopback bağlantı noktasını yerel olarak yönetilen bir tarayıcı profili olarak değerlendirir ve yerel bağlantı noktası sahipliği hataları bildirebilir.
...
- `existing-session` profilleri mevcut Chrome MCP rota sınırlarını korur:
  CSS seçici hedefleme yerine snapshot/ref odaklı eylemler, tek dosyalı yükleme kancaları, iletişim kutusu zaman aşımı geçersiz kılmaları yok, `wait --load networkidle` yok ve `responsebody`, PDF dışa aktarma, indirme yakalama veya toplu eylemler yok.
- Yerel olarak yönetilen `openclaw` profilleri `cdpPort` ve `cdpUrl` değerlerini otomatik atar; `cdpUrl` değerini yalnızca uzak CDP için açıkça ayarlayın.
- Yerel olarak yönetilen profiller, bu profil için genel `browser.executablePath` değerini geçersiz kılmak üzere `executablePath` ayarlayabilir. Bunu bir profili Chrome’da, diğerini Brave’de çalıştırmak için kullanın.
- Yerel olarak yönetilen profiller, işlem başladıktan sonra Chrome CDP HTTP keşfi için `browser.localLaunchTimeoutMs`, başlatma sonrası CDP websocket hazır olma durumu için `browser.localCdpReadyTimeoutMs` kullanır. Chrome’un başarıyla başladığı ancak hazır olma kontrollerinin başlangıçla yarıştığı daha yavaş ana makinelerde bunları artırın. Her iki değer de `120000` ms’ye kadar pozitif tam sayılar olmalıdır; geçersiz yapılandırma değerleri reddedilir.
...
- **Docker notu**: varsayılan `loopback` bind, konteyner içinde `127.0.0.1` üzerinde dinler. Docker bridge ağıyla (`-p 18789:18789`) trafik `eth0` üzerinden gelir, bu yüzden gateway erişilemez olur. Tüm arayüzlerde dinlemek için `--network host` kullanın veya `bind: "lan"` (ya da `customBindHost: "0.0.0.0"` ile `bind: "custom"`) ayarlayın.
- **Kimlik doğrulama**: varsayılan olarak gereklidir. Loopback dışı bind'lar gateway kimlik doğrulaması gerektirir. Pratikte bu, paylaşılan bir belirteç/parola veya `gateway.auth.mode: "trusted-proxy"` kullanan kimlik farkındalığına sahip bir ters proxy anlamına gelir. İlk kurulum sihirbazı varsayılan olarak bir belirteç üretir.
...
- `gateway.auth.mode: "none"`: açık no-auth modu. Yalnızca güvenilen local loopback kurulumları için kullanın; bu, özellikle ilk kurulum istemlerinde sunulmaz.
- `gateway.auth.mode: "trusted-proxy"`: tarayıcı/kullanıcı kimlik doğrulamasını kimlik farkındalığına sahip bir ters proxy'ye devredin ve `gateway.trustedProxies` içinden gelen kimlik başlıklarına güvenin (bkz. [Trusted Proxy Auth](/tr/gateway/trusted-proxy-auth)). Bu mod varsayılan olarak **loopback dışı** bir proxy kaynağı bekler; aynı makinedeki loopback ters proxy'leri açıkça `gateway.auth.trustedProxy.allowLoopback = true` gerektirir. Dahili aynı makine çağırıcıları yerel doğrudan yedek olarak `gateway.auth.password` kullanabilir; `gateway.auth.token`, trusted-proxy modu ile karşılıklı olarak dışlayıcı kalır.
- `gateway.auth.allowTailscale`: `true` olduğunda, Tailscale Serve kimlik başlıkları Control UI/WebSocket kimlik doğrulamasını karşılayabilir (`tailscale whois` ile doğrulanır). HTTP API uç noktaları bu Tailscale başlık kimlik doğrulamasını **kullanmaz**; bunun yerine gateway'in normal HTTP kimlik doğrulama modunu izler. Bu belirteçsiz akış, gateway host'unun güvenilir olduğunu varsayar. `tailscale.mode = "serve"` olduğunda varsayılan değer `true` olur.
- `gateway.auth.rateLimit`: isteğe bağlı başarısız kimlik doğrulama sınırlayıcısı. İstemci IP'si ve kimlik doğrulama kapsamı başına uygulanır (shared-secret ve device-token bağımsız izlenir). Engellenen denemeler `429` + `Retry-After` döndürür.
  - Asenkron Tailscale Serve Control UI yolunda, aynı `{scope, clientIp}` için başarısız denemeler başarısızlık yazımından önce seri hale getirilir. Bu nedenle aynı istemciden eşzamanlı kötü denemeler, ikisinin de düz uyuşmazlık olarak yarışıp geçmesi yerine ikinci istekte sınırlayıcıyı tetikleyebilir.
  - `gateway.auth.rateLimit.exemptLoopback` varsayılan olarak `true` olur; localhost trafiğinin de özellikle hız sınırlamasına tabi olmasını istediğinizde (test kurulumları veya katı proxy dağıtımları için) `false` olarak ayarlayın.
- Tarayıcı-origin WS kimlik doğrulama denemeleri, loopback muafiyeti devre dışı bırakılmış şekilde her zaman yavaşlatılır (tarayıcı tabanlı localhost kaba kuvvet denemelerine karşı savunma katmanı).
- Loopback üzerinde, bu tarayıcı-origin kilitlenmeleri normalize edilmiş `Origin`
  değeri başına izole edilir, böylece bir localhost origin'inden tekrarlanan başarısızlıklar
...
- `controlUi.dangerouslyAllowHostHeaderOriginFallback`: kasıtlı olarak Host başlığı origin politikasına dayanan dağıtımlar için Host-header origin fallback'i etkinleştiren tehlikeli mod.
...
- `OPENCLAW_ALLOW_INSECURE_PRIVATE_WS=1`: güvenilen özel ağ
  IP'lerine düz metin `ws://` kullanımına izin veren istemci tarafı süreç ortamı
  acil durum geçersiz kılması; varsayılan, düz metin için loopback-only kalır. Bunun `openclaw.json`
  eşdeğeri yoktur ve
...
- `gateway.push.apns.relay.baseUrl`: resmi/TestFlight iOS derlemeleri relay destekli kayıtları gateway'e yayımladıktan sonra kullanılan harici APNs relay'i için temel HTTPS URL'si. Bu URL, iOS derlemesine gömülen relay URL'siyle eşleşmelidir.
- `gateway.push.apns.relay.timeoutMs`: gateway'den relay'e gönderim zaman aşımı, milisaniye cinsinden. Varsayılan değer `10000`.
- Relay destekli kayıtlar belirli bir gateway kimliğine devredilir. Eşleştirilmiş iOS uygulaması `gateway.identity.get` çağrısını alır, bu kimliği relay kaydına dahil eder ve kayıt kapsamlı gönderim iznini gateway'e iletir. Başka bir gateway bu saklanan kaydı yeniden kullanamaz.
...
- `gateway.channelStaleEventThresholdMinutes`: eski socket eşiği, dakika cinsinden. Bunu `gateway.channelHealthCheckMinutes` değerinden büyük veya ona eşit tutun. Varsayılan: `30`.
...
- `channels.<provider>.healthMonitor.enabled`: genel izleyici etkin kalırken sağlık izleyicisi yeniden başlatmaları için kanal başına kapsam dışı bırakma.
...
- `gateway.auth.token` / `gateway.auth.password` SecretRef üzerinden açıkça yapılandırılmış ve çözümlenmemişse, çözüm kapalı başarısız olur (remote fallback ile maskeleme olmaz).
- `trustedProxies`: TLS'yi sonlandıran veya forwarded-client başlıkları enjekte eden ters proxy IP'leri. Yalnızca kontrol ettiğiniz proxy'leri listeleyin. Loopback girdileri aynı makine proxy/yerel algılama kurulumları için (örneğin Tailscale Serve veya yerel bir ters proxy) hâlâ geçerlidir, ancak loopback isteklerini `gateway.auth.mode: "trusted-proxy"` için uygun hale **getirmez**.
- `allowRealIpFallback`: `true` olduğunda, `X-Forwarded-For` eksikse gateway `X-Real-IP` kabul eder. Fail-closed davranışı için varsayılan `false`.
...
Tek bir host üzerinde benzersiz portlar ve state dizinleriyle birden çok gateway çalıştırın:
...
- `autoGenerate`: açık dosyalar yapılandırılmamışsa yerel self-signed sertifika/anahtar çifti otomatik üretir; yalnızca yerel/geliştirme kullanımı içindir.
...
- `deferralTimeoutMs`: yeniden başlatmayı zorlamadan önce sürmekte olan işlemleri beklemek için ms cinsinden isteğe bağlı azami süre. Varsayılan sınırlı beklemeyi (`300000`) kullanmak için bunu atlayın; süresiz beklemek ve periyodik hâlâ beklemede uyarıları kaydetmek için `0` ayarlayın.
...
- Bir eşleme veya ön ayar şablonlu bir `sessionKey` kullanıyorsa `hooks.allowedSessionKeyPrefixes` ayarını yapın ve `hooks.allowRequestSessionKey=true` olarak belirleyin. Statik eşleme anahtarları bu açık izni gerektirmez.
...
- `transform`, hook eylemi döndüren bir JS/TS modülünü gösterebilir.
  ...
  - `hooks.transformsDir` değerini `~/.openclaw/hooks/transforms` altında tutun; çalışma alanı skill dizinleri reddedilir. `openclaw doctor` bu yolu geçersiz olarak bildirirse dönüşüm modülünü hooks dönüşümleri dizinine taşıyın veya `hooks.transformsDir` ayarını kaldırın.
...
- `allowRequestSessionKey`: `/hooks/agent` çağıranlarının ve şablonla yönlendirilen eşleme oturum anahtarlarının `sessionKey` ayarlamasına izin verir (varsayılan: `false`).
- `allowedSessionKeyPrefixes`: açık `sessionKey` değerleri için isteğe bağlı önek izin listesi (istek + eşleme), ör. `["hook:"]`. Herhangi bir eşleme veya ön ayar şablonlu bir `sessionKey` kullandığında zorunlu hale gelir.
...
- Bu ileti başına yönlendirmeyi korursanız `hooks.allowRequestSessionKey: true` ayarını yapın ve `hooks.allowedSessionKeyPrefixes` değerini Gmail ad alanıyla eşleşecek şekilde sınırlandırın, örneğin `["hook:", "hook:gmail:"]`.
...
- Gateway, yapılandırıldığında açılışta `gog gmail watch serve` komutunu otomatik başlatır. Devre dışı bırakmak için `OPENCLAW_SKIP_GMAIL_WATCHER=1` ayarını yapın.
...
- `full`: `cliPath` + `sshPort` değerlerini ekler; LAN çok noktaya yayın duyurusu yine de yerleşik `bonjour` plugin'inin etkin olmasını gerektirir.
- `off`: plugin etkinleştirmesini değiştirmeden LAN çok noktaya yayın duyurusunu bastırır.
...
- Ana makine adı, geçerli bir DNS etiketi olduğunda varsayılan olarak sistem ana makine adı olur; aksi durumda `openclaw` değerine geri döner. `OPENCLAW_MDNS_HOSTNAME` ile geçersiz kılın.
...
`~/.openclaw/dns/` altında bir tek noktaya yayın DNS-SD bölgesi yazar. Ağlar arası keşif için bir DNS sunucusu (CoreDNS önerilir) + Tailscale bölünmüş DNS ile eşleştirin.
...
- Windows ACL doğrulaması kullanılamadığında dosya ve exec sağlayıcısı yolları kapalı olarak başarısız olur. `allowInsecurePath: true` değerini yalnızca doğrulanamayan güvenilir yollar için ayarlayın.
...
- Etkin yüzey filtrelemesi etkinleştirme sırasında uygulanır: etkin yüzeylerde çözümlenemeyen referanslar başlatma/yeniden yüklemeyi başarısız kılar, etkin olmayan yüzeyler ise tanılamalarla atlanır.
...
- Gizli bilgiler çalışma zamanı davranışı ve `audit/configure/apply` araçları: [Gizli Bilgiler Yönetimi](/tr/gateway/secrets).
...
- `billingBackoffHours`: bir profil gerçek faturalandırma/yetersiz kredi hataları nedeniyle başarısız olduğunda saat cinsinden temel geri çekilme süresi (varsayılan: `5`). Açık faturalandırma metni, `401`/`403` yanıtlarında bile yine de buraya düşebilir; ancak sağlayıcıya özgü metin eşleştiriciler, bunların sahibi olan sağlayıcıyla sınırlı kalır (örneğin OpenRouter `Key limit exceeded`). Yeniden denenebilir HTTP `402` kullanım penceresi veya kuruluş/çalışma alanı harcama sınırı mesajları bunun yerine `rate_limit` yolunda kalır.
...
- `overloadedProfileRotations`: model yedeğine geçmeden önce aşırı yük hataları için aynı sağlayıcıdaki en fazla kimlik doğrulama profili rotasyonu sayısı (varsayılan: `1`). `ModelNotReadyException` gibi sağlayıcı meşgul biçimleri buraya düşer.
- `overloadedBackoffMs`: aşırı yüklenmiş bir sağlayıcı/profil rotasyonunu yeniden denemeden önceki sabit gecikme (varsayılan: `0`).
- `rateLimitedProfileRotations`: model yedeğine geçmeden önce hız sınırı hataları için aynı sağlayıcıdaki en fazla kimlik doğrulama profili rotasyonu sayısı (varsayılan: `1`). Bu hız sınırı kovası `Too many concurrent requests`, `ThrottlingException`, `concurrency limit reached`, `workers_ai ... quota limit exceeded` ve `resource exhausted` gibi sağlayıcı biçimli metinleri içerir.
...
- `maxFileBytes`: döndürme öncesinde etkin günlük dosyasının bayt cinsinden en büyük boyutu (pozitif tamsayı; varsayılan: `104857600` = 100 MB). OpenClaw, etkin dosyanın yanında en fazla beş numaralı arşiv tutar.
- `redactSensitive` / `redactPatterns`: konsol çıktısı, dosya günlükleri, OTLP günlük kayıtları ve kalıcı oturum dökümü metni için en iyi çabayla maskeleme. `redactSensitive: "off"` yalnızca bu genel günlük/döküm politikasını devre dışı bırakır; UI/araç/tanılama güvenlik yüzeyleri sırları yaymadan önce yine de sansürler.
...
- `stuckSessionWarnMs`: uzun süren işleme oturumlarını `session.long_running`, `session.stalled` veya `session.stuck` olarak sınıflandırmak için ms cinsinden ilerleme yok yaş eşiği. Yanıt, araç, durum, blok ve ACP ilerlemesi zamanlayıcıyı sıfırlar; tekrarlanan `session.stuck` tanılamaları değişiklik olmadığında geri çekilir.
- `stuckSessionAbortMs`: kurtarma için uygun duraklamış etkin işin abort-drain yapılabilmesinden önce ms cinsinden ilerleme yok yaş eşiği. Ayarlanmadığında OpenClaw, en az 10 dakika ve 5x `stuckSessionWarnMs` olan daha güvenli genişletilmiş gömülü çalıştırma penceresini kullanır.
...
- `otel.captureContent`: OTEL span öznitelikleri için isteğe bağlı ham içerik yakalama. Varsayılan olarak kapalıdır. Boole `true`, sistem dışı mesaj/araç içeriğini yakalar; nesne biçimi `inputMessages`, `outputMessages`, `toolInputs`, `toolOutputs` ve `systemPrompt` alanlarını açıkça etkinleştirmenizi sağlar.
...
- `OPENCLAW_OTEL_PRELOADED=1`: zaten global bir OpenTelemetry SDK kaydetmiş ana makineler için ortam anahtarı. OpenClaw bu durumda tanılama dinleyicilerini etkin tutarken Plugin sahibi SDK başlatma/kapatma işlemlerini atlar.
...
- `auto.stableDelayHours`: kararlı kanal otomatik uygulaması öncesinde saat cinsinden en az gecikme (varsayılan: `6`; üst sınır: `168`).
- `auto.stableJitterHours`: saat cinsinden ek kararlı kanal yayılım penceresi (varsayılan: `12`; üst sınır: `168`).
- `auto.betaCheckIntervalHours`: beta kanal denetimlerinin saat cinsinden ne sıklıkla çalışacağı (varsayılan: `1`; üst sınır: `24`).
...
- `backend`: varsayılan ACP çalışma zamanı backend kimliği (kayıtlı bir ACP çalışma zamanı Plugin ile eşleşmelidir).
  Önce backend Plugin'ini kurun ve `plugins.allow` ayarlanmışsa backend Plugin kimliğini ekleyin (örneğin `acpx`), aksi halde ACP backend yüklenmez.
...
- `stream.maxChunkChars`: akışla gönderilen blok projeksiyonunu bölmeden önceki en büyük parça boyutu.
- `stream.repeatSuppression`: tur başına tekrarlanan durum/araç satırlarını bastırır (varsayılan: `true`).
- `stream.deliveryMode`: `"live"` artımlı olarak akış yapar; `"final_only"` tur sonlandırma olaylarına kadar tamponlar.
...
- `stream.tagVisibility`: akış olayları için etiket adlarından boole görünürlük geçersiz kılmalarına kayıt.
- `runtime.ttlMinutes`: ACP oturum işçileri için uygun temizleme öncesi dakika cinsinden boşta TTL.
- `runtime.installCommand`: bir ACP çalışma zamanı ortamı başlatılırken çalıştırılacak isteğe bağlı kurulum komutu.
...
- `sessionRetention`: tamamlanmış yalıtılmış cron çalıştırma oturumlarının `sessions.json` dosyasından budanmadan önce ne kadar süre tutulacağı. Arşivlenmiş silinmiş cron transkriptlerinin temizlenmesini de denetler. Varsayılan: `24h`; devre dışı bırakmak için `false` olarak ayarlayın.
- `runLog.maxBytes`: budama öncesinde çalıştırma günlüğü dosyası (`cron/runs/<jobId>.jsonl`) başına en büyük boyut. Varsayılan: `2_000_000` bayt.
...
- `webhook`: hâlâ `notify: true` değerine sahip saklanan işler için kullanılan, kullanımdan kaldırılmış eski yedek Webhook URL'si (http/https).
...
- Ne genel ne de iş başına hata hedefi ayarlandığında, zaten `announce` üzerinden teslim eden işler hata durumunda birincil duyuru hedefine geri döner.
...
- İç içe dahil etmeler: en fazla 10 düzey derinliğe kadar.
- Yollar: dahil eden dosyaya göre çözümlenir, ancak üst düzey yapılandırma dizini (`openclaw.json` dosyasının `dirname` değeri) içinde kalmalıdır. Mutlak/`../` biçimlerine yalnızca hâlâ bu sınır içinde çözümlendiklerinde izin verilir.
- Yalnızca tek dosyalı dahil etmeyle desteklenen bir üst düzey bölümü değiştiren OpenClaw sahipli yazmalar, bu dahil edilen dosyaya yazılır. Örneğin, `plugins install`, `plugins: { $include: "./plugins.json5" }` bölümünü `plugins.json5` içinde günceller ve `openclaw.json` dosyasını olduğu gibi bırakır.
- Kök dahil etmeler, dahil etme dizileri ve kardeş geçersiz kılmaları olan dahil etmeler OpenClaw sahipli yazmalar için salt okunurdur; bu yazmalar yapılandırmayı düzleştirmek yerine kapalı şekilde başarısız olur.
- Hatalar: eksik dosyalar, ayrıştırma hataları ve döngüsel dahil etmeler için anlaşılır iletiler.
