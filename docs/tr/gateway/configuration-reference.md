---
read_when:
    - Alan düzeyindeki yapılandırma semantiğine veya varsayılan değerlere ihtiyacınız var
    - Kanal, model, Gateway veya araç yapılandırma bloklarını doğruluyorsunuz
summary: Temel OpenClaw anahtarları, varsayılan değerleri ve özel alt sistem başvurularına bağlantılar için Gateway yapılandırma başvurusu
...
Temel `~/.openclaw/openclaw.json` yapılandırma başvurusu. Görev odaklı bir genel bakış için bkz. [Yapılandırma](/tr/gateway/configuration).

Ana OpenClaw yapılandırma yüzeylerini kapsar ve bir alt sistemin kendi daha derin başvurusu olduğunda bağlantı verir. Kanal ve Plugin sahipliğindeki komut katalogları ile derin bellek/QMD düğmeleri bu sayfa yerine kendi sayfalarında yer alır.

Kod gerçeği:

- `openclaw config schema`, doğrulama ve Denetim UI için kullanılan canlı JSON Schema'yı yazdırır; mevcut olduğunda paketli/Plugin/kanal meta verileri birleştirilir
- `config.schema.lookup`, ayrıntılı inceleme araçları için yol kapsamlı tek bir şema düğümü döndürür
- `pnpm config:docs:check` / `pnpm config:docs:gen`, yapılandırma belgeleri temel hash'ini mevcut şema yüzeyine göre doğrular

Ajan arama yolu: düzenlemelerden önce tam alan düzeyi belgeler ve kısıtlamalar için `gateway` araç eylemi `config.schema.lookup` kullanın. Görev odaklı rehberlik için [Yapılandırma](/tr/gateway/configuration), daha geniş alan haritası, varsayılanlar ve alt sistem başvurularına bağlantılar için bu sayfayı kullanın.
...
- Geçerli yerleşik + paketli komut kataloğu için [Eğik çizgi komutları](/tr/tools/slash-commands)
...
Kanal başına yapılandırma anahtarları özel bir sayfaya taşındı — Slack, Discord, Telegram, WhatsApp, Matrix, iMessage ve diğer paketli kanallar (kimlik doğrulama, erişim denetimi, çoklu hesap, bahsetme geçidi) dahil `channels.*` için bkz. [Yapılandırma — kanallar](/tr/gateway/config-channels).
...
- `agents.defaults.*` (çalışma alanı, model, düşünme, Heartbeat, bellek, medya, skills, kum havuzu)
...
- `talk.*` (Konuşma modu)
  - `talk.speechLocale`: iOS/macOS üzerinde Konuşma konuşma tanıma için isteğe bağlı BCP 47 yerel kimliği
  - `talk.silenceTimeoutMs`: ayarlanmadığında Konuşma, transkripti göndermeden önce platformun varsayılan duraklama penceresini korur (`macOS ve Android'de 700 ms, iOS'ta 900 ms`)
...
Sağlayıcı tanımları, model izin listeleri ve özel sağlayıcı kurulumu [Yapılandırma — araçlar ve özel sağlayıcılar](/tr/gateway/config-tools#custom-providers-and-base-urls) içinde yer alır. `models` kökü ayrıca genel model kataloğu davranışına da sahiptir.
...
- `mcp.sessionIdleTtlMs`: oturum kapsamlı paketli MCP çalışma zamanları için boşta kalma TTL'si. Tek seferlik gömülü çalıştırmalar, çalıştırma sonu temizliği ister; bu TTL, uzun ömürlü oturumlar ve gelecekteki çağırıcılar için güvenlik ağıdır.
- `mcp.*` altındaki değişiklikler, önbelleğe alınmış oturum MCP çalışma zamanları elden çıkarılarak sıcak uygulanır. Sonraki araç keşfi/kullanımı bunları yeni yapılandırmadan yeniden oluşturur; böylece kaldırılan `mcp.servers` girdileri, boşta kalma TTL'sini beklemek yerine hemen temizlenir.
...
- `install.preferBrew`: true olduğunda, `brew` kullanılabiliyorsa diğer yükleyici türlerine geri dönmeden önce Homebrew yükleyicilerini tercih eder.
...
- Keşif, yerel OpenClaw plugins ile uyumlu Codex paketlerini ve Claude paketlerini kabul eder; buna manifestsiz Claude varsayılan yerleşimli paketleri de dahildir.
- **Yapılandırma değişiklikleri Gateway'in yeniden başlatılmasını gerektirir.**
- `allow`: isteğe bağlı izin listesi (yalnızca listelenen plugins yüklenir). `deny` önceliklidir.
- `bundledDiscovery`: yeni yapılandırmalar için varsayılan olarak `"allowlist"` olur; böylece boş olmayan bir `plugins.allow`, web araması çalışma zamanı sağlayıcıları dahil paketli sağlayıcı plugins için de geçit görevi görür. Doctor, mevcut paketli sağlayıcı davranışını siz tercih edene kadar korumak için taşınmış eski izin listesi yapılandırmalarına `"compat"` yazar.
...
- `plugins.entries.<id>.hooks.allowPromptInjection`: `false` olduğunda core, `before_prompt_build` öğesini engeller ve eski `before_agent_start` içindeki prompt değiştiren alanları yok sayarken eski `modelOverride` ve `providerOverride` alanlarını korur. Yerel Plugin hook'larına ve desteklenen paket tarafından sağlanan hook dizinlerine uygulanır.
- `plugins.entries.<id>.hooks.allowConversationAccess`: `true` olduğunda güvenilir paketli olmayan plugins, `llm_input`, `llm_output`, `before_agent_finalize` ve `agent_end` gibi türlenmiş hook'lardan ham konuşma içeriğini okuyabilir.
- `plugins.entries.<id>.subagent.allowModelOverride`: bu Plugin'e, arka plan alt ajan çalıştırmaları için çalıştırma başına `provider` ve `model` geçersiz kılmaları istemesi konusunda açıkça güven.
- `plugins.entries.<id>.subagent.allowedModels`: güvenilir alt ajan geçersiz kılmaları için kanonik `provider/model` hedeflerinin isteğe bağlı izin listesi. `"*"` öğesini yalnızca herhangi bir modele izin vermeyi özellikle istediğinizde kullanın.
...
- `plugins.entries.xai.config.xSearch`: xAI X Arama (Grok web araması) ayarları.
  - `enabled`: X Arama sağlayıcısını etkinleştirir.
...
- `plugins.entries.memory-core.config.dreaming`: bellek Dreaming ayarları. Aşamalar ve eşikler için bkz. [Dreaming](/tr/concepts/dreaming).
  ...
  - `model`: isteğe bağlı Dream Diary alt ajan modeli geçersiz kılması. `plugins.entries.memory-core.subagent.allowModelOverride: true` gerektirir; hedefleri kısıtlamak için `allowedModels` ile eşleştirin. Modelin kullanılamaması hataları, oturum varsayılan modeliyle bir kez yeniden denenir; güven veya izin listesi hataları sessizce geri dönmez.
...
- Etkin Claude paketi plugins, `settings.json` içinden gömülü Pi varsayılanlarına da katkıda bulunabilir; OpenClaw bunları ham OpenClaw yapılandırma yamaları olarak değil, temizlenmiş ajan ayarları olarak uygular.
- `plugins.slots.memory`: etkin bellek Plugin kimliğini seçin veya bellek plugins devre dışı bırakmak için `"none"` kullanın.
- `plugins.slots.contextEngine`: etkin bağlam motoru Plugin kimliğini seçin; başka bir motor yükleyip seçmediğiniz sürece varsayılan olarak `"legacy"` olur.
...
`commitments`, çıkarımsal takip belleğini denetler: OpenClaw, konuşma dönüşlerinden yoklamaları algılayabilir ve bunları Heartbeat çalıştırmaları üzerinden teslim edebilir.
...
- `commitments.maxPerDay`: kayan bir günde ajan oturumu başına teslim edilen maksimum çıkarımsal takip taahhüdü sayısı. Varsayılan: `3`.
...
- `tabCleanup`, izlenen birincil ajan sekmelerini boşta kalma süresinden sonra veya bir oturum sınırını aştığında geri alır. Bu tekil temizleme modlarını devre dışı bırakmak için `idleMinutes: 0` veya `maxTabsPerSession: 0` ayarlayın.
...
- Uzak profiller yalnızca ekleme modundadır (başlatma/durdurma/sıfırlama devre dışıdır).
...
- Haricen yönetilen bir CDP hizmetine loopback üzerinden erişilebiliyorsa, ilgili profilin `attachOnly: true` değerini ayarlayın; aksi takdirde OpenClaw, loopback bağlantı noktasını yerel yönetilen tarayıcı profili olarak ele alır ve yerel bağlantı noktası sahipliği hataları bildirebilir.
- `existing-session` profilleri CDP yerine Chrome MCP kullanır ve seçilen ana makineye ya da bağlı bir tarayıcı Node üzerinden eklenebilir.
...
- `existing-session` profilleri mevcut Chrome MCP rota sınırlarını korur: CSS seçici hedefleme yerine snapshot/ref odaklı eylemler, tek dosya yükleme hook’ları, iletişim kutusu zaman aşımı geçersiz kılmaları yok, `wait --load networkidle` yok ve `responsebody`, PDF dışa aktarma, indirme yakalama veya toplu eylemler yok.
...
- Yerel yönetilen profiller, ilgili profil için genel `browser.executablePath` değerini geçersiz kılmak üzere `executablePath` ayarlayabilir. Bunu bir profili Chrome’da, diğerini Brave’de çalıştırmak için kullanın.
- Yerel yönetilen profiller, süreç başlatıldıktan sonra Chrome CDP HTTP keşfi için `browser.localLaunchTimeoutMs` ve başlatma sonrası CDP websocket hazır oluşu için `browser.localCdpReadyTimeoutMs` kullanır. Chrome’un başarıyla başladığı ancak hazır oluş denetimlerinin başlangıçla yarıştığı daha yavaş ana makinelerde bunları artırın. Her iki değer de `120000` ms’ye kadar pozitif tam sayılar olmalıdır; geçersiz yapılandırma değerleri reddedilir.
- Otomatik algılama sırası: Chromium tabanlıysa varsayılan tarayıcı → Chrome → Brave → Edge → Chromium → Chrome Canary.
- `browser.executablePath` ve `browser.profiles.<name>.executablePath`, Chromium başlatılmadan önce işletim sisteminizin giriş dizini için `~` ve `~/...` değerlerini kabul eder. `existing-session` profillerindeki profil başına `userDataDir` de tilde ile genişletilir.
- Denetim hizmeti: yalnızca loopback (bağlantı noktası `gateway.port` üzerinden türetilir, varsayılan `18791`).
...
- `port`: WS + HTTP için tek multiplekslenmiş port. Öncelik: `--port` > `OPENCLAW_GATEWAY_PORT` > `gateway.port` > `18789`.
- `bind`: `auto`, `loopback` (varsayılan), `lan` (`0.0.0.0`), `tailnet` (yalnızca Tailscale IP'si) veya `custom`.
- **Eski bind alias'ları**: `gateway.bind` içinde host alias'ları (`0.0.0.0`, `127.0.0.1`, `localhost`, `::`, `::1`) yerine bind modu değerlerini (`auto`, `loopback`, `lan`, `tailnet`, `custom`) kullanın.
- **Docker notu**: varsayılan `loopback` bind, konteyner içinde `127.0.0.1` üzerinde dinler. Docker bridge ağıyla (`-p 18789:18789`) trafik `eth0` üzerinden gelir, bu yüzden gateway erişilemez olur. Tüm arabirimlerde dinlemek için `--network host` kullanın veya `bind: "lan"` ayarlayın (ya da `customBindHost: "0.0.0.0"` ile `bind: "custom"` kullanın).
...
- `gateway.auth.rateLimit`: isteğe bağlı başarısız kimlik doğrulama sınırlayıcısı. İstemci IP'si ve kimlik doğrulama kapsamı başına uygulanır (shared-secret ve device-token bağımsız izlenir). Engellenen denemeler `429` + `Retry-After` döndürür.
  - Async Tailscale Serve Control UI yolunda, aynı `{scope, clientIp}` için başarısız denemeler hata yazımından önce seri hale getirilir. Bu nedenle aynı istemciden gelen eşzamanlı hatalı denemeler, her ikisi de düz uyuşmazlık olarak yarışıp geçmek yerine ikinci istekte sınırlayıcıyı tetikleyebilir.
  - `gateway.auth.rateLimit.exemptLoopback` varsayılan olarak `true` olur; localhost trafiğinin de bilerek hız sınırına tabi olmasını istediğinizde (test kurulumları veya katı proxy dağıtımları için) `false` ayarlayın.
- Tarayıcı origin'li WS kimlik doğrulama denemeleri, local loopback muafiyeti devre dışı bırakılarak her zaman yavaşlatılır (tarayıcı tabanlı localhost brute force'a karşı defense-in-depth).
- local loopback üzerinde, bu tarayıcı origin'li kilitlemeler normalize edilmiş `Origin`
...
- `controlUi.allowedOrigins`: Gateway WebSocket bağlantıları için açık tarayıcı origin allowlist'i. Tarayıcı istemcileri local loopback olmayan origin'lerden bekleniyorsa gereklidir.
- `controlUi.chatMessageMaxWidth`: gruplanmış Control UI sohbet mesajları için isteğe bağlı max-width. `960px`, `82%`, `min(1280px, 82%)` ve `calc(100% - 2rem)` gibi kısıtlanmış CSS genişlik değerlerini kabul eder.
- `controlUi.dangerouslyAllowHostHeaderOriginFallback`: Host-header origin politikasına bilerek dayanan dağıtımlar için Host-header origin fallback'i etkinleştiren tehlikeli mod.
...
- `OPENCLAW_ALLOW_INSECURE_PRIVATE_WS=1`: düz metin `ws://` kullanımına güvenilir özel ağ
  IP'leri için izin veren istemci tarafı process-environment
  break-glass override'ı; düz metin için varsayılan local loopback-only kalır. Bunun `openclaw.json`
  eşdeğeri yoktur ve
...
- `gateway.remote.token` / `.password` uzak istemci credential alanlarıdır. Bunlar tek başına gateway kimlik doğrulamasını yapılandırmaz.
- `gateway.push.apns.relay.baseUrl`: official/TestFlight iOS build'leri relay destekli kayıtları gateway'e yayımladıktan sonra kullanılan harici APNs relay için temel HTTPS URL'si. Bu URL, iOS build'ine derlenen relay URL'siyle eşleşmelidir.
- `gateway.push.apns.relay.timeoutMs`: milisaniye cinsinden gateway'den relay'e gönderim timeout'u. Varsayılan `10000`.
- Relay destekli kayıtlar belirli bir gateway kimliğine devredilir. Eşleştirilmiş iOS uygulaması `gateway.identity.get` değerini alır, bu kimliği relay kaydına dahil eder ve kayıt kapsamlı bir gönderim iznini gateway'e iletir. Başka bir gateway bu saklanan kaydı yeniden kullanamaz.
...
- `gateway.handshakeTimeoutMs`: kimlik doğrulama öncesi Gateway WebSocket handshake timeout'u, milisaniye cinsinden. Varsayılan: `15000`. Ayarlandığında `OPENCLAW_HANDSHAKE_TIMEOUT_MS` önceliklidir. Yerel istemcilerin, başlangıç ısınması hâlâ otururken bağlanabildiği yüklü veya düşük güçlü host'larda bunu artırın.
...
- `channels.<provider>.healthMonitor.enabled`: global monitör etkin kalırken health-monitor yeniden başlatmaları için kanal başına opt-out.
...
- Yerel gateway çağrı yolları yalnızca `gateway.auth.*` ayarlanmamış olduğunda fallback olarak `gateway.remote.*` kullanabilir.
- `gateway.auth.token` / `gateway.auth.password` SecretRef üzerinden açıkça yapılandırılmış ve çözümlenememişse, çözümleme fail-closed olur (uzak fallback maskelemesi yoktur).
- `trustedProxies`: TLS'i sonlandıran veya forwarded-client başlıkları ekleyen ters proxy IP'leri. Yalnızca kontrol ettiğiniz proxy'leri listeleyin. Loopback girdileri aynı host proxy/local-detection kurulumları için hâlâ geçerlidir (örneğin Tailscale Serve veya yerel bir ters proxy), ancak local loopback isteklerini `gateway.auth.mode: "trusted-proxy"` için uygun hale **getirmez**.
- `allowRealIpFallback`: `true` olduğunda, `X-Forwarded-For` eksikse gateway `X-Real-IP` değerini kabul eder. Fail-closed davranışı için varsayılan `false`.
- `gateway.nodes.pairing.autoApproveCidrs`: istenen kapsam olmadan ilk kez node cihaz eşleştirmesini otomatik onaylamak için isteğe bağlı CIDR/IP allowlist'i. Ayarlanmamışsa devre dışıdır. Bu, operator/browser/Control UI/WebChat eşleştirmesini otomatik onaylamaz ve rol, kapsam, metadata veya public-key yükseltmelerini otomatik onaylamaz.
- `gateway.nodes.allowCommands` / `gateway.nodes.denyCommands`: eşleştirme ve platform allowlist değerlendirmesinden sonra bildirilen node komutları için global allow/deny şekillendirmesi. `camera.snap`, `camera.clip` ve `screen.record` gibi tehlikeli node komutlarına opt-in yapmak için `allowCommands` kullanın; `denyCommands`, platform varsayılanı veya açık bir izin normalde dahil edecek olsa bile bir komutu kaldırır. Bir node bildirdiği komut listesini değiştirdikten sonra, bu cihaz eşleştirmesini reddedip yeniden onaylayın ki gateway güncellenmiş komut snapshot'ını saklasın.
- `gateway.tools.deny`: HTTP `POST /tools/invoke` için engellenen ek tool adları (varsayılan deny list'i genişletir).
- `gateway.tools.allow`: tool adlarını varsayılan HTTP deny list'inden kaldırır.
...
- Responses URL-input sertleştirmesi:
...
Tek bir host üzerinde benzersiz portlar ve state dizinleriyle birden fazla gateway çalıştırın:
...
- `enabled`: gateway listener'ında TLS sonlandırmasını etkinleştirir (HTTPS/WSS) (varsayılan: `false`).
- `autoGenerate`: açık dosyalar yapılandırılmadığında yerel self-signed cert/key çifti otomatik oluşturur; yalnızca local/dev kullanımı içindir.
- `certPath`: TLS sertifika dosyasının filesystem yolu.
- `keyPath`: TLS private key dosyasının filesystem yolu; izinleri kısıtlı tutun.
- `caPath`: istemci doğrulaması veya özel trust chain'ler için isteğe bağlı CA bundle yolu.
...
- `mode`: yapılandırma düzenlemelerinin runtime'da nasıl uygulanacağını denetler.
  - `"off"`: canlı düzenlemeleri yok say; değişiklikler açık bir yeniden başlatma gerektirir.
  - `"restart"`: yapılandırma değişikliğinde gateway process'ini her zaman yeniden başlat.
  - `"hot"`: değişiklikleri yeniden başlatmadan process içinde uygula.
  - `"hybrid"` (varsayılan): önce hot reload dene; gerekiyorsa yeniden başlatmaya fallback yap.
- `debounceMs`: yapılandırma değişiklikleri uygulanmadan önce ms cinsinden debounce penceresi (negatif olmayan tam sayı).
- `deferralTimeoutMs`: yeniden başlatmayı zorlamadan önce uçuşta olan işlemleri beklemek için ms cinsinden isteğe bağlı maksimum süre. Varsayılan sınırlı beklemeyi (`300000`) kullanmak için bunu atlayın; süresiz beklemek ve periyodik still-pending uyarıları loglamak için `0` ayarlayın.
...
- Bir eşleme veya hazır ayar şablonlu bir `sessionKey` kullanıyorsa, `hooks.allowedSessionKeyPrefixes` değerini ayarlayın ve `hooks.allowRequestSessionKey=true` yapın. Statik eşleme anahtarları bu açık onayı gerektirmez.
...
- `transform`, bir kanca eylemi döndüren bir JS/TS modülünü gösterebilir.
  - `transform.module` göreli bir yol olmalı ve `hooks.transformsDir` içinde kalmalıdır (mutlak yollar ve dizin aşma reddedilir).
  - `hooks.transformsDir` değerini `~/.openclaw/hooks/transforms` altında tutun; çalışma alanı Skills dizinleri reddedilir. `openclaw doctor` bu yolu geçersiz olarak bildirirse, dönüştürme modülünü kanca dönüştürmeleri dizinine taşıyın veya `hooks.transformsDir` değerini kaldırın.
- `agentId`, belirli bir araca yönlendirir; bilinmeyen kimlikler varsayılana geri döner.
- `allowedAgentIds`: açık yönlendirmeyi kısıtlar (`*` veya atlanmış = tümüne izin ver, `[]` = tümünü reddet).
- `defaultSessionKey`: açık `sessionKey` olmadan yapılan kanca araç çalıştırmaları için isteğe bağlı sabit oturum anahtarı.
...
- `allowedSessionKeyPrefixes`: açık `sessionKey` değerleri (istek + eşleme) için isteğe bağlı önek izin listesi, örn. `["hook:"]`. Herhangi bir eşleme veya hazır ayar şablonlu bir `sessionKey` kullandığında zorunlu hale gelir.
- `deliver: true`, son yanıtı bir kanala gönderir; `channel` varsayılan olarak `last` olur.
- `model`, bu kanca çalıştırması için LLM değerini geçersiz kılar (model kataloğu ayarlanmışsa izin verilmiş olmalıdır).
...
- Aracın düzenleyebileceği HTML/CSS/JS ve A2UI'yi Gateway bağlantı noktası altında HTTP üzerinden sunar:
  - `http://<gateway-host>:<gateway.port>/__openclaw__/canvas/`
  - `http://<gateway-host>:<gateway.port>/__openclaw__/a2ui/`
- Yalnızca yerel: `gateway.bind: "loopback"` değerini koruyun (varsayılan).
- local loopback dışı bağlamalar: canvas rotaları, diğer Gateway HTTP yüzeyleriyle aynı şekilde Gateway kimlik doğrulaması (belirteç/parola/güvenilir proxy) gerektirir.
- Node WebView'leri genellikle kimlik doğrulama başlıkları göndermez; bir düğüm eşleştirilip bağlandıktan sonra Gateway, canvas/A2UI erişimi için düğüm kapsamlı yetenek URL'lerini duyurur.
...
- Yerleşik `bonjour` Plugin, macOS ana makinelerinde otomatik başlar; Linux, Windows ve konteynerleştirilmiş Gateway dağıtımlarında isteğe bağlıdır.
...
- Satır içi ortam değişkenleri yalnızca süreç ortamında ilgili anahtar eksikse uygulanır.
...
- Sabit bir `${VAR}` için `$${VAR}` ile kaçış yapın.
- `$include` ile çalışır.
...
- `source: "exec"` id değerleri `.` veya `..` içeren, eğik çizgiyle ayrılmış yol segmentleri içermemelidir (örneğin `a/../b` reddedilir)
...
- Etkin yüzey filtreleme etkinleştirme sırasında uygulanır: etkin yüzeylerdeki çözümlenmemiş referanslar başlatma/yeniden yükleme işlemini başarısız kılar, etkin olmayan yüzeyler ise tanılama ile atlanır.
...
- `{ "provider": { "apiKey": "..." } }` gibi eski düz `auth-profiles.json` haritaları bir çalışma zamanı biçimi değildir; `openclaw doctor --fix` bunları `.legacy-flat.*.bak` yedeğiyle kurallı `provider:default` API anahtarı profillerine yeniden yazar.
...
- `overloadedProfileRotations`: model yedeğine geçmeden önce aşırı yüklenme hataları için aynı sağlayıcı içinde en fazla kimlik doğrulama profili rotasyonu (varsayılan: `1`). `ModelNotReadyException` gibi sağlayıcı meşgul biçimleri buraya düşer.
- `overloadedBackoffMs`: aşırı yüklenmiş bir sağlayıcı/profil rotasyonunu yeniden denemeden önce sabit gecikme (varsayılan: `0`).
- `rateLimitedProfileRotations`: model yedeğine geçmeden önce hız sınırı hataları için aynı sağlayıcı içinde en fazla kimlik doğrulama profili rotasyonu (varsayılan: `1`). Bu hız sınırı kovası `Too many concurrent requests`, `ThrottlingException`, `concurrency limit reached`, `workers_ai ... quota limit exceeded` ve `resource exhausted` gibi sağlayıcı biçimli metinleri içerir.
...
- `redactSensitive` / `redactPatterns`: konsol çıktısı, dosya günlükleri, OTLP günlük kayıtları ve kalıcı oturum transkript metni için en iyi çaba esaslı maskeleme. `redactSensitive: "off"` yalnızca bu genel günlük/transkript politikasını devre dışı bırakır; UI/araç/tanılama güvenlik yüzeyleri yayımdan önce sırları yine de redakte eder.
...
- `stuckSessionWarnMs`: uzun süren işleme oturumlarını `session.long_running`, `session.stalled` veya `session.stuck` olarak sınıflandırmak için ms cinsinden ilerlemesiz yaş eşiği. Yanıt, araç, durum, blok ve ACP ilerlemesi zamanlayıcıyı sıfırlar; tekrarlanan `session.stuck` tanılamaları değişmeden kaldıkça geri çekilir.
- `stuckSessionAbortMs`: uygun duraklamış etkin işin kurtarma için iptal edilip boşaltılabilmesinden önce ms cinsinden ilerlemesiz yaş eşiği. Ayarlanmadığında OpenClaw, en az 10 dakika ve `stuckSessionWarnMs` değerinin 5 katı olan daha güvenli genişletilmiş gömülü çalıştırma penceresini kullanır.
...
- `otel.captureContent`: OTEL span öznitelikleri için ham içerik yakalamaya katılım. Varsayılan olarak kapalıdır. Boole `true`, sistem dışı ileti/araç içeriğini yakalar; nesne biçimi `inputMessages`, `outputMessages`, `toolInputs`, `toolOutputs` ve `systemPrompt` değerlerini açıkça etkinleştirmenize olanak tanır.
...
- `OPENCLAW_OTEL_PRELOADED=1`: zaten genel bir OpenTelemetry SDK kaydetmiş ana makineler için ortam anahtarı. OpenClaw bu durumda tanılama dinleyicilerini etkin tutarken Plugin'e ait SDK başlatma/kapatma işlemlerini atlar.
...
- `auto.stableDelayHours`: stable kanalında otomatik uygulamadan önce saat cinsinden en az gecikme (varsayılan: `6`; en fazla: `168`).
- `auto.stableJitterHours`: stable kanalı yayılımı için saat cinsinden ek dağıtım penceresi (varsayılan: `12`; en fazla: `168`).
...
- `enabled`: genel ACP özellik kapısı (varsayılan: `true`; ACP dağıtımını ve oluşturma olanaklarını gizlemek için `false` ayarlayın).
- `dispatch.enabled`: ACP oturum turu dağıtımı için bağımsız kapı (varsayılan: `true`). ACP komutlarını kullanılabilir tutarken yürütmeyi engellemek için `false` ayarlayın.
...
- `defaultAgent`: oluşturmalarda açık bir hedef belirtilmediğinde yedek ACP hedef ajan kimliği.
...
- `stream.maxChunkChars`: akışlı blok projeksiyonunu bölmeden önce en büyük parça boyutu.
- `stream.repeatSuppression`: tur başına tekrarlanan durum/araç satırlarını bastırır (varsayılan: `true`).
- `stream.deliveryMode`: `"live"` artımlı olarak akıtır; `"final_only"` tur sonlandırma olaylarına kadar arabelleğe alır.
...
- `runtime.ttlMinutes`: ACP oturum işçileri için temizlenmeye uygun olmadan önce dakika cinsinden boşta TTL.
...
Bkz. [Ajan varsayılanları](/tr/gateway/config-agents#agent-defaults) altındaki `agents.list` kimlik alanları.
...
- `sessionRetention`: tamamlanmış yalıtılmış cron çalıştırma oturumlarının `sessions.json` içinden budanmadan önce ne kadar süre tutulacağı. Ayrıca arşivlenmiş silinmiş cron transkriptlerinin temizlenmesini de denetler. Varsayılan: `24h`; devre dışı bırakmak için `false` ayarlayın.
- `runLog.maxBytes`: budamadan önce çalıştırma günlüğü dosyası (`cron/runs/<jobId>.jsonl`) başına maksimum boyut. Varsayılan: `2_000_000` bayt.
...
- Ne genel ne de iş başına hata hedefi ayarlanmışsa, zaten `announce` üzerinden teslim eden işler hata durumunda birincil announce hedefine geri döner.
...
| `{{SenderName}}`   | Gönderenin görünen adı (en iyi çabayla)           |
| `{{SenderE164}}`   | Gönderenin telefon numarası (en iyi çabayla)      |
...
- Yollar: dahil eden dosyaya göre çözümlenir, ancak en üst düzey yapılandırma dizininin (`openclaw.json` dosyasının `dirname` değeri) içinde kalmalıdır. Mutlak/`../` biçimlerine yalnızca yine bu sınırın içinde çözümlendiklerinde izin verilir.
- Yalnızca tek dosyalı bir dahil etmeyle desteklenen bir üst düzey bölümü değiştiren OpenClaw'a ait yazmalar, bu dahil edilen dosyaya yazılır. Örneğin, `plugins install`, `plugins: { $include: "./plugins.json5" }` öğesini `plugins.json5` içinde günceller ve `openclaw.json` dosyasını olduğu gibi bırakır.
