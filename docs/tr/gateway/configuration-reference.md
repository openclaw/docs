...
Ana OpenClaw yapılandırma yüzeylerini kapsar ve bir alt sistemin kendi daha derin referansı olduğunda bağlantı verir. Kanala ve plugin'e ait komut katalogları ile derin bellek/QMD ayarları bu sayfa yerine kendi sayfalarında yer alır.

Kod gerçeği:

- `openclaw config schema`, doğrulama ve Control UI için kullanılan canlı JSON Schema'yı yazdırır; varsa bundled/plugin/kanal meta verileri birleştirilir
- `config.schema.lookup`, ayrıntılı inceleme araçları için yol kapsamlı tek bir şema düğümü döndürür
- `pnpm config:docs:check` / `pnpm config:docs:gen`, yapılandırma dokümanı temel hash'ini güncel şema yüzeyine göre doğrular

Ajan arama yolu: düzenlemelerden önce tam alan düzeyi dokümanlar ve kısıtlamalar için `gateway` araç eylemi `config.schema.lookup` kullanın. Görev odaklı rehberlik için [Yapılandırma](/tr/gateway/configuration) sayfasını, daha geniş alan haritası, varsayılanlar ve alt sistem referanslarına bağlantılar için bu sayfayı kullanın.
...
- Güncel yerleşik + bundled komut kataloğu için [Eğik çizgi komutları](/tr/tools/slash-commands)
...
Kanal başına yapılandırma anahtarları özel bir sayfaya taşındı - Slack, Discord, Telegram, WhatsApp, Matrix, iMessage ve diğer bundled kanallar dahil `channels.*` için [Yapılandırma - kanallar](/tr/gateway/config-channels) sayfasına bakın (kimlik doğrulama, erişim denetimi, çoklu hesap, bahsetme kapısı).

## Ajan varsayılanları, çoklu ajan, oturumlar ve mesajlar

Özel bir sayfaya taşındı - şunlar için [Yapılandırma - ajanlar](/tr/gateway/config-agents) sayfasına bakın:
...
Sağlayıcı tanımları, model izin listeleri ve özel sağlayıcı kurulumu [Yapılandırma - araçlar ve özel sağlayıcılar](/tr/gateway/config-tools#custom-providers-and-base-urls) içinde bulunur. `models` kökü ayrıca genel model kataloğu davranışına sahiptir.
...
- `models.pricing.enabled`: sidecar'lar ve kanallar Gateway hazır yoluna ulaştıktan sonra başlayan arka plan fiyatlandırma bootstrap'ini denetler. `false` olduğunda Gateway, OpenRouter ve LiteLLM fiyatlandırma kataloğu getirmelerini atlar; yapılandırılmış `models.providers.*.models[].cost` değerleri yerel maliyet tahminleri için yine de çalışır.

## MCP

OpenClaw tarafından yönetilen MCP sunucu tanımları `mcp.servers` altında bulunur ve gömülü Pi ile diğer runtime bağdaştırıcıları tarafından tüketilir. `openclaw mcp list`, `show`, `set` ve `unset` komutları, yapılandırma düzenlemeleri sırasında hedef sunucuya bağlanmadan bu bloğu yönetir.
...
- `mcp.servers`: yapılandırılmış MCP araçlarını açığa çıkaran runtime'lar için adlandırılmış stdio veya uzak MCP sunucu tanımları. Uzak girdiler `transport: "streamable-http"` veya `transport: "sse"` kullanır; `type: "http"`, `openclaw mcp set` ve `openclaw doctor --fix` tarafından standart `transport` alanına normalleştirilen CLI'a özgü bir takma addır.
- `mcp.sessionIdleTtlMs`: oturum kapsamlı bundled MCP runtime'ları için boşta kalma TTL'i. Tek seferlik gömülü çalıştırmalar çalışma sonu temizliği ister; bu TTL uzun ömürlü oturumlar ve gelecekteki çağıranlar için emniyet desteğidir.
- `mcp.*` altındaki değişiklikler, önbelleğe alınmış oturum MCP runtime'larını elden çıkararak sıcak uygulanır. Sonraki araç keşfi/kullanımı bunları yeni yapılandırmadan yeniden oluşturur; böylece kaldırılan `mcp.servers` girdileri boşta kalma TTL'ini beklemek yerine hemen temizlenir.

Runtime davranışı için bkz. [MCP](/tr/cli/mcp#openclaw-as-an-mcp-client-registry) ve [CLI arka uçları](/tr/gateway/cli-backends#bundle-mcp-overlays).
...
- `install.preferBrew`: true olduğunda, `brew` mevcutsa diğer kurucu türlerine dönmeden önce Homebrew kurucularını tercih eder.
...
- `~/.openclaw/extensions`, `<workspace>/.openclaw/extensions` ve ayrıca `plugins.load.paths` üzerinden yüklenir.
- Keşif, yerel OpenClaw pluginlerinin yanında uyumlu Codex bundle'larını ve Claude bundle'larını kabul eder; buna manifestsiz Claude varsayılan yerleşim bundle'ları dahildir.
...
- `bundledDiscovery`: yeni yapılandırmalar için varsayılan olarak `"allowlist"` olur; bu nedenle boş olmayan bir `plugins.allow`, web arama runtime sağlayıcıları dahil bundled sağlayıcı pluginlerini de kapılar. Doctor, mevcut bundled sağlayıcı davranışını siz katılana kadar korumak için geçirilmiş eski izin listesi yapılandırmalarına `"compat"` yazar.
...
- `plugins.entries.<id>.hooks.allowPromptInjection`: `false` olduğunda çekirdek `before_prompt_build` öğesini engeller ve eski `before_agent_start` içinden istemi değiştiren alanları yok sayarken eski `modelOverride` ve `providerOverride` öğelerini korur. Yerel plugin hook'larına ve desteklenen bundle tarafından sağlanan hook dizinlerine uygulanır.
- `plugins.entries.<id>.hooks.allowConversationAccess`: `true` olduğunda, güvenilir bundled olmayan pluginler `llm_input`, `llm_output`, `before_agent_finalize` ve `agent_end` gibi türlendirilmiş hook'lardan ham konuşma içeriğini okuyabilir.
...
- `plugins.entries.<id>.subagent.allowedModels`: güvenilir alt ajan geçersiz kılmaları için standart `provider/model` hedeflerinden oluşan isteğe bağlı izin listesi. `"*"` öğesini yalnızca herhangi bir modele bilinçli olarak izin vermek istediğinizde kullanın.
...
- Kanal plugin hesap/runtime ayarları `channels.<id>` altında bulunur ve merkezi bir OpenClaw seçenek kaydı yerine, sahip plugin'in manifest `channelConfigs` meta verileri tarafından açıklanmalıdır.
- `plugins.entries.firecrawl.config.webFetch`: Firecrawl web getirme sağlayıcısı ayarları.
  ...
  - `baseUrl`: Firecrawl API temel URL'si (varsayılan: `https://api.firecrawl.dev`; kendi barındırılan geçersiz kılmalar özel/dahili uç noktalara hedeflenmelidir).
...
- `plugins.entries.memory-core.config.dreaming`: bellek dreaming ayarları. Aşamalar ve eşikler için bkz. [Dreaming](/tr/concepts/dreaming).
  ...
  - `model`: isteğe bağlı Dream Diary alt ajan model geçersiz kılması. `plugins.entries.memory-core.subagent.allowModelOverride: true` gerektirir; hedefleri sınırlamak için `allowedModels` ile eşleştirin. Model kullanılamıyor hataları oturum varsayılan modeliyle bir kez yeniden denenir; güven veya izin listesi hataları sessizce geri düşmez.
  - aşama ilkesi ve eşikler uygulama ayrıntılarıdır (kullanıcıya yönelik yapılandırma anahtarları değildir).
- Tam bellek yapılandırması [Bellek yapılandırma referansı](/tr/reference/memory-config) içinde bulunur:
  - `agents.defaults.memorySearch.*`
  - `memory.backend`
  - `memory.citations`
  - `memory.qmd.*`
  - `plugins.entries.memory-core.config.dreaming`
- Etkin Claude bundle pluginleri `settings.json` üzerinden gömülü Pi varsayılanları da sağlayabilir; OpenClaw bunları ham OpenClaw yapılandırma yamaları olarak değil, temizlenmiş ajan ayarları olarak uygular.
...
`commitments`, çıkarımsanan takip belleğini denetler: OpenClaw konuşma dönüşlerinden check-in'leri algılayabilir ve bunları heartbeat çalıştırmaları üzerinden teslim edebilir.

- `commitments.enabled`: çıkarımsanan takip taahhütleri için gizli LLM çıkarımını, depolamayı ve heartbeat teslimini etkinleştir. Varsayılan: `false`.
- `commitments.maxPerDay`: kayan bir günde ajan oturumu başına teslim edilen maksimum çıkarımsanan takip taahhüdü sayısı. Varsayılan: `3`.
...
- Harici olarak yönetilen bir CDP hizmetine loopback üzerinden erişilebiliyorsa, o profilin `attachOnly: true` değerini ayarlayın; aksi takdirde OpenClaw, loopback bağlantı noktasını yerel yönetilen tarayıcı profili olarak değerlendirir ve yerel bağlantı noktası sahipliği hataları bildirebilir.
...
- `existing-session` profilleri mevcut Chrome MCP rota sınırlarını korur:
  CSS seçici hedefleme yerine anlık görüntü/ref odaklı eylemler, tek dosyalı yükleme kancaları, iletişim kutusu zaman aşımı geçersiz kılmaları yok, `wait --load networkidle` yok ve `responsebody`, PDF dışa aktarma, indirme yakalama veya toplu eylemler yok.
...
- `mode`: `local` (Gateway çalıştır) veya `remote` (uzak Gateway'e bağlan). Gateway, `local` olmadığı sürece başlatmayı reddeder.
...
- **Docker notu**: varsayılan `loopback` bind, kapsayıcının içinde `127.0.0.1` üzerinde dinler. Docker köprü ağıyla (`-p 18789:18789`) trafik `eth0` üzerinden gelir, bu yüzden Gateway erişilemez olur. Tüm arayüzlerde dinlemek için `--network host` kullanın ya da `bind: "lan"` ayarlayın (veya `customBindHost: "0.0.0.0"` ile `bind: "custom"`).
- **Kimlik doğrulama**: varsayılan olarak gereklidir. local loopback olmayan bind'lar Gateway kimlik doğrulaması gerektirir. Pratikte bu, paylaşılan bir token/parola veya `gateway.auth.mode: "trusted-proxy"` içeren kimlik farkındalıklı bir ters proxy anlamına gelir. İlk kurulum sihirbazı varsayılan olarak bir token üretir.
- Hem `gateway.auth.token` hem de `gateway.auth.password` yapılandırılmışsa (SecretRef'ler dahil), `gateway.auth.mode` değerini açıkça `token` veya `password` olarak ayarlayın. İkisi de yapılandırılmış ve mod ayarlanmamış olduğunda başlatma ve hizmet kurulum/onarım akışları başarısız olur.
...
- `gateway.auth.rateLimit`: isteğe bağlı başarısız kimlik doğrulama sınırlayıcı. İstemci IP'si ve kimlik doğrulama kapsamı başına uygulanır (shared-secret ve device-token bağımsız izlenir). Engellenen denemeler `429` + `Retry-After` döndürür.
  - Eşzamansız Tailscale Serve Control UI yolunda, aynı `{scope, clientIp}` için başarısız denemeler hata yazımından önce serileştirilir. Bu nedenle aynı istemciden gelen eşzamanlı hatalı denemeler, ikisinin de düz uyumsuzluk olarak yarışması yerine ikinci istekte sınırlayıcıyı tetikleyebilir.
  - `gateway.auth.rateLimit.exemptLoopback` varsayılan olarak `true` değerindedir; localhost trafiğini de kasıtlı olarak hız sınırlamasına tabi tutmak istediğinizde (test kurulumları veya sıkı proxy dağıtımları için) `false` olarak ayarlayın.
- Tarayıcı kökenli WS kimlik doğrulama denemeleri her zaman local loopback muafiyeti devre dışı bırakılarak kısıtlanır (tarayıcı tabanlı localhost kaba kuvvet denemelerine karşı katmanlı savunma).
- local loopback üzerinde, bu tarayıcı kökenli kilitlenmeler normalleştirilmiş `Origin`
  değeri başına yalıtılır; böylece bir localhost kaynağından tekrarlanan hatalar otomatik olarak
  farklı bir kaynağı kilitlemez.
- `tailscale.mode`: `serve` (yalnızca tailnet, local loopback bind) veya `funnel` (genel, kimlik doğrulaması gerektirir).
- `controlUi.allowedOrigins`: Gateway WebSocket bağlantıları için açık tarayıcı kaynağı izin listesi. Tarayıcı istemcilerinin local loopback olmayan kaynaklardan gelmesi bekleniyorsa gereklidir.
- `controlUi.chatMessageMaxWidth`: gruplanmış Control UI sohbet iletileri için isteğe bağlı maksimum genişlik. `960px`, `82%`, `min(1280px, 82%)` ve `calc(100% - 2rem)` gibi kısıtlı CSS genişlik değerlerini kabul eder.
- `controlUi.dangerouslyAllowHostHeaderOriginFallback`: bilinçli olarak Host başlığı kaynak politikasına dayanan dağıtımlar için Host başlığı kaynak geri dönüşünü etkinleştiren tehlikeli mod.
...
- `OPENCLAW_ALLOW_INSECURE_PRIVATE_WS=1`: güvenilir özel ağ
  ...
  son çare geçersiz kılmasıdır; düz metin için varsayılan local loopback-only kalır. Bunun bir `openclaw.json`
  karşılığı yoktur ve
...
- `gateway.push.apns.relay.baseUrl`: resmi/TestFlight iOS derlemelerinin röle destekli kayıtları Gateway'e yayımlamasından sonra kullandığı harici APNs rölesi için temel HTTPS URL'si. Bu URL, iOS derlemesine gömülen röle URL'siyle eşleşmelidir.
- `gateway.push.apns.relay.timeoutMs`: Gateway'den röleye gönderim zaman aşımı, milisaniye cinsinden. Varsayılan değer `10000`.
- Röle destekli kayıtlar belirli bir Gateway kimliğine devredilir. Eşleştirilmiş iOS uygulaması `gateway.identity.get` bilgisini alır, bu kimliği röle kaydına ekler ve kayıt kapsamlı bir gönderim iznini Gateway'e iletir. Başka bir Gateway bu saklanan kaydı yeniden kullanamaz.
- `OPENCLAW_APNS_RELAY_BASE_URL` / `OPENCLAW_APNS_RELAY_TIMEOUT_MS`: yukarıdaki röle yapılandırması için geçici ortam geçersiz kılmaları.
- `OPENCLAW_APNS_RELAY_ALLOW_HTTP=true`: local loopback HTTP röle URL'leri için yalnızca geliştirme amaçlı kaçış kapısı. Üretim röle URL'leri HTTPS üzerinde kalmalıdır.
- `gateway.handshakeTimeoutMs`: kimlik doğrulama öncesi Gateway WebSocket el sıkışma zaman aşımı, milisaniye cinsinden. Varsayılan: `15000`. Ayarlandığında `OPENCLAW_HANDSHAKE_TIMEOUT_MS` önceliklidir. Yerel istemcilerin bağlanabildiği ancak başlatma ısınmasının hâlâ oturduğu yüklü veya düşük güçlü ana makinelerde bunu artırın.
...
- `channels.<provider>.healthMonitor.enabled`: genel izleyiciyi etkin tutarken sağlık izleme yeniden başlatmaları için kanal başına vazgeçme seçeneği.
...
- Yerel Gateway çağrı yolları, yalnızca `gateway.auth.*` ayarlanmamışsa `gateway.remote.*` değerini geri dönüş olarak kullanabilir.
- `gateway.auth.token` / `gateway.auth.password` SecretRef ile açıkça yapılandırılmış ve çözümlenememişse çözümleme kapalı biçimde başarısız olur (uzak geri dönüş maskelemesi yoktur).
- `trustedProxies`: TLS'yi sonlandıran veya yönlendirilmiş istemci başlıkları enjekte eden ters proxy IP'leri. Yalnızca kontrol ettiğiniz proxy'leri listeleyin. local loopback girdileri aynı ana makine proxy/yerel algılama kurulumları için hâlâ geçerlidir (örneğin Tailscale Serve veya yerel ters proxy), ancak local loopback isteklerini `gateway.auth.mode: "trusted-proxy"` için uygun hale **getirmez**.
- `allowRealIpFallback`: `true` olduğunda Gateway, `X-Forwarded-For` eksikse `X-Real-IP` değerini kabul eder. Kapalı başarısızlık davranışı için varsayılan `false`.
- `gateway.nodes.pairing.autoApproveCidrs`: istenen kapsam olmadan ilk kez Node aygıt eşleştirmesini otomatik onaylamak için isteğe bağlı CIDR/IP izin listesi. Ayarlanmadığında devre dışıdır. Bu, operatör/tarayıcı/Control UI/WebChat eşleştirmesini otomatik onaylamaz ve rol, kapsam, meta veri veya genel anahtar yükseltmelerini otomatik onaylamaz.
- `gateway.nodes.allowCommands` / `gateway.nodes.denyCommands`: eşleştirme ve platform izin listesi değerlendirmesinden sonra bildirilen Node komutları için genel izin/ret şekillendirmesi. `camera.snap`, `camera.clip` ve `screen.record` gibi tehlikeli Node komutlarını etkinleştirmek için `allowCommands` kullanın; `denyCommands`, bir platform varsayılanı veya açık izin normalde onu dahil edecek olsa bile komutu kaldırır. Bir Node bildirdiği komut listesini değiştirdikten sonra, Gateway'in güncellenmiş komut anlık görüntüsünü saklaması için o aygıt eşleştirmesini reddedin ve yeniden onaylayın.
...
- Responses URL girdisi sağlamlaştırması:
  ...
  - `gateway.http.endpoints.responses.images.urlAllowlist`
    Boş izin listeleri ayarlanmamış olarak değerlendirilir; URL getirmeyi devre dışı bırakmak için `gateway.http.endpoints.responses.files.allowUrl=false`
...
Tek bir ana makinede benzersiz portlar ve durum dizinleriyle birden çok Gateway çalıştırın:
...
- `autoGenerate`: açık dosyalar yapılandırılmadığında yerel kendinden imzalı sertifika/anahtar çifti otomatik üretir; yalnızca yerel/geliştirme kullanımı içindir.
...
- `deferralTimeoutMs`: yeniden başlatmayı zorlamadan önce devam eden işlemleri beklemek için ms cinsinden isteğe bağlı en fazla süre. Varsayılan sınırlı beklemeyi (`300000`) kullanmak için bunu atlayın; süresiz beklemek ve periyodik hâlâ beklemede uyarıları günlüğe yazmak için `0` ayarlayın.
...
Sorgu dizesindeki hook token'ları reddedilir.
...
- `POST /hooks/<name>` → `hooks.mappings` aracılığıyla çözümlenir
  - Şablonla oluşturulmuş eşleme `sessionKey` değerleri dışarıdan sağlanmış kabul edilir ve ayrıca `hooks.allowRequestSessionKey=true` gerektirir.
...
- `allowedSessionKeyPrefixes`: açık `sessionKey` değerleri için isteğe bağlı önek izin listesi (istek + eşleme), örn. `["hook:"]`. Herhangi bir eşleme veya hazır ayar şablonlu bir `sessionKey` kullandığında gerekli hale gelir.
...
- `full`: `cliPath` + `sshPort` değerlerini dahil eder; LAN multicast duyurusu yine de yerleşik `bonjour` Plugin'in etkin olmasını gerektirir.
- `off`: Plugin etkinliğini değiştirmeden LAN multicast duyurusunu bastırır.
- Yerleşik `bonjour` Plugin macOS host'larında otomatik başlar; Linux, Windows ve kapsayıcılaştırılmış Gateway dağıtımlarında açık katılımlıdır.
...
`~/.openclaw/dns/` altında tekil yayın DNS-SD bölgesi yazar. Ağlar arası keşif için bir DNS sunucusu (CoreDNS önerilir) + Tailscale split DNS ile eşleştirin.
...
Gizli değer referansları eklemelidir: düz metin değerler hâlâ çalışır.
...
- `exec` alt ortamı varsayılan olarak minimumdur; gerekli değişkenleri `passEnv` ile açıkça geçirin.
- Gizli değer referansları etkinleştirme sırasında bellek içi bir anlık görüntüye çözümlenir, ardından istek yolları yalnızca anlık görüntüyü okur.
- Etkin yüzey filtrelemesi etkinleştirme sırasında uygulanır: etkin yüzeylerdeki çözümlenmemiş referanslar başlatmayı/yeniden yüklemeyi başarısız kılarken, etkin olmayan yüzeyler tanılamalarla atlanır.
...
- Gizli değer çalışma zamanı davranışı ve `audit/configure/apply` araçları: [Gizli Değer Yönetimi](/tr/gateway/secrets).
...
- `overloadedProfileRotations`: model yedeğine geçmeden önce aşırı yüklenmiş hatalar için en fazla aynı sağlayıcı auth-profile rotasyonu (varsayılan: `1`). `ModelNotReadyException` gibi sağlayıcı-meşgul şekilleri buraya düşer.
...
- `redactSensitive` / `redactPatterns`: konsol çıktısı, dosya günlükleri, OTLP günlük kayıtları ve kalıcı oturum transkript metni için en iyi çaba maskelemesi. `redactSensitive: "off"` yalnızca bu genel günlük/transkript ilkesini devre dışı bırakır; UI/araç/tanılama güvenlik yüzeyleri, yayımdan önce gizli bilgileri yine de redakte eder.
...
- `stuckSessionAbortMs`: uygun takılmış etkin işlerin kurtarma için iptal edilip boşaltılabilmesinden önce ms cinsinden ilerleme yok yaşı eşiği. Ayarlanmadığında OpenClaw, en az 10 dakika ve 5x `stuckSessionWarnMs` olan daha güvenli genişletilmiş gömülü çalıştırma penceresini kullanır.
...
- `otel.captureContent`: OTEL span öznitelikleri için katılımlı ham içerik yakalama. Varsayılan olarak kapalıdır. Boolean `true`, sistem dışı ileti/araç içeriğini yakalar; nesne biçimi `inputMessages`, `outputMessages`, `toolInputs`, `toolOutputs` ve `systemPrompt` değerlerini açıkça etkinleştirmenizi sağlar.
...
- `OPENCLAW_OTEL_PRELOADED=1`: zaten global bir OpenTelemetry SDK'sı kaydetmiş ana makineler için ortam anahtarı. OpenClaw bu durumda tanılama dinleyicilerini etkin tutarken Plugin sahibi SDK başlatma/kapatma işlemini atlar.
...
- `dispatch.enabled`: ACP oturum turn dispatch için bağımsız kapı (varsayılan: `true`). ACP komutlarını kullanılabilir tutarken yürütmeyi engellemek için `false` ayarlayın.
- `backend`: varsayılan ACP runtime arka uç kimliği (kayıtlı bir ACP runtime Plugin ile eşleşmelidir).
  Önce arka uç Plugin'ini kurun ve `plugins.allow` ayarlıysa arka uç Plugin kimliğini dahil edin (örneğin `acpx`); aksi halde ACP arka ucu yüklenmez.
- `defaultAgent`: spawn'lar açık bir hedef belirtmediğinde yedek ACP hedef ajan kimliği.
- `allowedAgents`: ACP runtime oturumları için izin verilen ajan kimliklerinin izin listesi; boş olması ek kısıtlama olmadığı anlamına gelir.
...
- `stream.repeatSuppression`: turn başına yinelenen durum/araç satırlarını bastırır (varsayılan: `true`).
- `stream.deliveryMode`: `"live"` artımlı olarak akış yapar; `"final_only"` turn terminal olaylarına kadar arabelleğe alır.
- `stream.hiddenBoundarySeparator`: gizli araç olaylarından sonra görünür metinden önceki ayırıcı (varsayılan: `"paragraph"`).
- `stream.maxOutputChars`: ACP turn başına projeksiyonu yapılan en fazla asistan çıktı karakteri.
- `stream.maxSessionUpdateChars`: projeksiyonu yapılan ACP durum/güncelleme satırları için en fazla karakter.
- `stream.tagVisibility`: akışlı olaylar için etiket adlarından boolean görünürlük geçersiz kılmalarına kayıt.
- `runtime.ttlMinutes`: ACP oturum worker'ları için temizlemeye uygun olmadan önce dakika cinsinden boşta TTL.
- `runtime.installCommand`: bir ACP runtime ortamı önyüklenirken çalıştırılacak isteğe bağlı kurulum komutu.
...
[Agent defaults](/tr/gateway/config-agents#agent-defaults) altındaki `agents.list` kimlik alanlarına bakın.
...
- `sessionRetention`: tamamlanmış yalıtılmış Cron çalıştırma oturumlarının `sessions.json` dosyasından budanmadan önce ne kadar süre tutulacağını belirler. Arşivlenmiş silinmiş Cron transcript'lerinin temizlenmesini de denetler. Varsayılan: `24h`; devre dışı bırakmak için `false` olarak ayarlayın.
- `runLog.maxBytes`: budamadan önce çalıştırma günlük dosyası (`cron/runs/<jobId>.jsonl`) başına azami boyut. Varsayılan: `2_000_000` bayt.
- `runLog.keepLines`: çalıştırma günlüğü budaması tetiklendiğinde korunan en yeni satırlar. Varsayılan: `2000`.
- `webhookToken`: Cron Webhook POST teslimi (`delivery.mode = "webhook"`) için kullanılan bearer belirteci; atlanırsa kimlik doğrulama başlığı gönderilmez.
- `webhook`: hâlâ `notify: true` değerine sahip kayıtlı işler için kullanılan, kullanımdan kaldırılmış eski yedek Webhook URL'si (http/https).
...
- `accountId`: uyarı teslimini kapsamla sınırlamak için isteğe bağlı hesap veya kanal kimliği.
...
- Genel veya iş başına hata hedefi ayarlanmadığında, zaten `announce` üzerinden teslim yapan işler hata durumunda bu birincil announce hedefine geri döner.
...
| `{{MaxChars}}`     | CLI girişleri için çözümlenmiş azami çıktı karakteri |
...
Yapılandırmayı birden fazla dosyaya ayırın:
...
- Yollar: include eden dosyaya göre çözümlenir, ancak üst düzey yapılandırma dizininin (`openclaw.json` dosyasının `dirname` değeri) içinde kalmalıdır. Mutlak/`../` biçimlerine yalnızca hâlâ bu sınırın içine çözümlendiklerinde izin verilir.
- Yalnızca tek dosyalı bir include tarafından desteklenen bir üst düzey bölümü değiştiren OpenClaw'a ait yazmalar, doğrudan o dahil edilen dosyaya yazılır. Örneğin, `plugins install`, `plugins.json5` içindeki `plugins: { $include: "./plugins.json5" }` değerini günceller ve `openclaw.json` dosyasını olduğu gibi bırakır.
- Kök include'lar, include dizileri ve kardeş geçersiz kılmaları olan include'lar OpenClaw'a ait yazmalar için salt okunurdur; bu yazmalar yapılandırmayı düzleştirmek yerine kapalı şekilde başarısız olur.
