...
x-i18n:
    generated_at: "2026-07-02T22:44:39Z"
    model: gpt-5.5
...
- `RateLimit-Remaining`: üst bilgi mevcut olduğunda tam kalan kotanız. `429` durumunda bu değer `0` olur.
- `RateLimit-Reset` veya `X-RateLimit-Reset`: sıfırlama zamanlaması.

Birçok kullanıcı tek bir çıkış IP adresini paylaşıyorsa, her kişi yalnızca birkaç
...
Plugins için `package.json` uyumluluk metadata bilgilerini kontrol edin. Code-plugin yayımlamaları
...
ClawHub, paketleri yayımcılarına bağlamak için GitHub kimliğini ve kaynak atfını
kullanır.
...
## Bir namespace talep edilmiş veya ayrılmış olduğu için yayımlama başarısız oluyor

Sahip kullanıcı adı, kuruluş namespace'i, paket kapsamı, skill slug değeri
veya paket adı zaten talep edilmiş ya da ayrılmış olduğu için yayımlama başarısız olursa, önce
namespace ile eşleşen sahip olarak yayımladığınızı doğrulayın. Plugin paketleri için
`@example-org/example-plugin` gibi kapsamlı adlar eşleşen `example-org`
sahibi olarak yayımlanmalıdır.

Kuruluşunuzun, projenizin veya markanızın ilgili namespace'in hak sahibi olduğuna inanıyor
ancak mevcut ClawHub sahibini yönetemiyorsanız, herkese açık ve hassas olmayan kanıtlarla bir
[Kuruluş / Namespace Talebi issue'su](https://github.com/openclaw/clawhub/issues/new?template=org-namespace-claim.yml)
açın. Kanıt rehberi ve herkese açık issue'ların dışında tutulması gerekenler için
...
- Tarama durumu ve uyumluluk metadata bilgileri için paket ayrıntı sayfasını kontrol edin.
- OpenClaw sürümünüzün paketin bildirdiği uyumluluk aralığını karşıladığını
  doğrulayın.
...
- Kullanıcıları standart ClawHub listesine geri bağlayın.
- Gizli, özel, bekletilen veya moderasyon tarafından engellenmiş içeriği genel
