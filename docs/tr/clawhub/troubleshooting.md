...
x-i18n:
    generated_at: "2026-07-02T14:10:12Z"
    model: gpt-5.5
...
- `RateLimit-Remaining`: başlık mevcut olduğunda tam kalan bütçeniz. `429` durumunda `0` olur.
...
Plugin'ler için `package.json` uyumluluk meta verilerini kontrol edin. Kod Plugin yayımlamaları,
...
## Yayınlama, bir ad alanı sahiplenildiği veya ayrıldığı için başarısız oluyor

Bir yayımlama; sahip kullanıcı adı, kuruluş ad alanı, paket kapsamı, skill
slug'ı veya paket adı zaten sahiplenilmiş ya da ayrılmış olduğu için başarısız olursa önce ad alanıyla eşleşen sahip
ile yayımlama yaptığınızı doğrulayın. Plugin paketleri için
...
- Paket gizli, bekletilmiş veya engellenmişse sahip sorunu çözene kadar yüklenebilir olmayabilir.
...
- Gizli, özel, bekletilmiş veya moderasyon tarafından engellenmiş içeriği herkese açık
