...
summary: ClawHub oturum açma, yükleme, yayınlama, güncelleme ve API sorunlarını giderme.
x-i18n:
...
- Özel bir yapılandırma yolu kullanıyorsanız `CLAWHUB_CONFIG_PATH` değişkeninin,
  geçerli belirtecinizi içeren dosyayı gösterdiğini doğrulayın.
- API belirteci kullanıyorsanız bunun web arayüzünde iptal edilmediğini doğrulayın.
...
- `RateLimit-Remaining`: üst bilgi mevcut olduğunda kalan kesin hakkınız. `429` durumunda bu değer `0` olur.
...
Pluginler için `package.json` uyumluluk meta verilerini kontrol edin. Kod-plugin yayınları,
...
Yayınlama; sahip tanıtıcısı, kuruluş ad alanı, paket kapsamı, skill
slug değeri veya paket adı zaten talep edildiği ya da rezerve edildiği için başarısız olursa,
önce ad alanıyla eşleşen sahiple yayınladığınızı doğrulayın. Plugin paketleri için
`@example-org/example-plugin` gibi kapsamlı adlar, eşleşen `example-org` sahibi
olarak yayınlanmalıdır.

Kuruluşunuzun, projenizin veya markanızın hak sahibi ad alanı sahibi olduğuna inanıyor ancak
...
açın. Kanıt yönlendirmesi ve herkese açık sorunların dışında tutulması gerekenler için
...
Taramasını istediğiniz kökleri gösterin:
