...
x-i18n:
    generated_at: "2026-07-02T01:11:14Z"
    model: gpt-5.5
...
- `RateLimit-Remaining`: üstbilgi mevcut olduğunda kalan tam bütçeniz. `429` durumunda bu değer `0` olur.
...
## Zorunlu metadata eksik olduğu için yayınlama başarısız oluyor

Beceriler için `SKILL.md` frontmatter bölümünü kontrol edin. Zorunlu ortam değişkenleri ve
...
## Bir namespace sahiplenildiği veya ayrıldığı için yayınlama başarısız oluyor

Sahip tanıtıcısı, kuruluş namespace’i, paket kapsamı, beceri
slug değeri veya paket adı zaten sahiplenildiği ya da ayrıldığı için yayınlama başarısız olursa, önce
namespace ile eşleşen sahiple yayınladığınızı doğrulayın. Plugin paketleri için
...
- `429` yeniden deneme üstbilgilerine uyun ve genel liste/arama yanıtlarını önbelleğe alın.
...
- Gizli, özel, bekletilen veya moderasyon tarafından engellenen içeriği
  genel API yüzeyinin dışında yansıtmayın.
