...
x-i18n:
    generated_at: "2026-07-01T08:23:02Z"
    model: gpt-5.5
...
Sahibe görünür tanı bilgileri tarama, yükleme kapısı veya moderasyon durumunu açıklayabilir.
...
## Bir namespace sahiplenildiği veya rezerve edildiği için yayınlama başarısız oluyor

Yayınlama; sahip kullanıcı adı, org namespace'i, paket kapsamı, skill
slug'ı veya paket adı zaten sahiplenildiği ya da rezerve edildiği için başarısız olursa,
önce namespace ile eşleşen sahiple yayınladığınızı doğrulayın. Plugin paketleri için
`@example-org/example-plugin` gibi scoped adlar, eşleşen `example-org` sahibi olarak
yayınlanmalıdır.

Org, proje veya markanızın haklı namespace sahibi olduğuna inanıyorsanız ancak
...
açın. Kanıt rehberliği ve herkese açık issue'ların dışında tutulması gerekenler için
[Org and Namespace Claims](/clawhub/namespace-claims) bölümüne bakın.
...
## Public API istekleri başarısız oluyor
