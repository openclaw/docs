...
x-i18n:
    generated_at: "2026-07-04T10:58:23Z"
    model: gpt-5.5
...
- Bir API belirteci kullanıyorsanız bunun web kullanıcı arayüzünde iptal edilmediğini doğrulayın.
...
- `RateLimit-Remaining`: başlık mevcut olduğunda tam kalan bütçeniz. `429` durumunda bu değer `0` olur.
- `RateLimit-Reset` veya `X-RateLimit-Reset`: sıfırlama zamanı.

Birçok kullanıcı aynı çıkış IP'sini paylaşıyorsa, her kişi yalnızca birkaç istek gönderse bile
anonim IP sınırlarına ulaşılabilir. Mümkün olduğunda oturum açın ve bildirilen gecikmeden sonra yeniden deneyin.
...
`openclaw.compat.pluginApi` ve `openclaw.build.openclawVersion` gibi OpenClaw uyumluluk alanlarına ihtiyaç duyar.
...
## Namespace talep edilmiş veya ayrılmış olduğu için yayınlama başarısız oluyor

Yayınlama; sahip tanıtıcısı, kuruluş namespace'i, paket kapsamı, Skills
slug'ı veya paket adı zaten talep edilmiş ya da ayrılmış olduğu için başarısız olursa, önce namespace ile eşleşen sahiple
...
- Paket gizli, beklemede veya engellenmişse, sahibi sorunu çözene kadar kurulamayabilir.
