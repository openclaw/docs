...
x-i18n:
    generated_at: "2026-07-04T18:13:37Z"
    model: gpt-5.5
...
CLI, tarayıcı oturumu açma sırasında kısa ömürlü bir yerel geri çağırma sunucusu başlatır.
...
- Başsız ortamlarda, ClawHub web UI içinde bir API token oluşturun ve şunu çalıştırın:
...
- Bir API token kullanıyorsanız, web UI içinde iptal edilmediğini doğrulayın.
...
Sahip tarafından görülebilen tanılar tarama, yükleme geçidi veya moderasyon durumunu açıklayabilir.

## Zorunlu meta veriler eksik olduğu için yayınlama başarısız oluyor

Beceriler için `SKILL.md` frontmatter'ını kontrol edin. Zorunlu ortam değişkenleri ve
...
zaten sahiplenilmiş ya da ayrılmış olduğu için yayınlama başarısız olursa, önce
...
Kuruluşunuzun, projenizin veya markanızın ad alanının gerçek sahibi olduğuna inanıyor
...
- Kullanıcıları kanonik ClawHub liste kaydına geri bağlayın.
