...
x-i18n:
    generated_at: "2026-06-28T08:18:59Z"
    model: gpt-5.5
...
- Özel bir yapılandırma yolu kullanıyorsanız `CLAWHUB_CONFIG_PATH` değerinin mevcut belirtecinizi içeren dosyayı gösterdiğini doğrulayın.
- API belirteci kullanıyorsanız web arayüzünde iptal edilmediğini doğrulayın.
...
- `RateLimit-Remaining`: üst bilgi mevcut olduğunda kalan kesin kotanız. `429` durumunda `0` olur.
- `RateLimit-Reset` veya `X-RateLimit-Reset`: sıfırlama zamanlaması.

Birçok kullanıcı tek bir çıkış IP’sini paylaşıyorsa, her kişi yalnızca birkaç istek gönderse bile anonim IP sınırlarına ulaşılabilir. Mümkün olduğunda oturum açın ve bildirilen gecikmeden sonra yeniden deneyin.
...
## Bir Skills öğesi aramada görünmüyor
...
- Skills öğesinin sahibiyseniz oturum açın ve inceleyin:
...
Plugin’ler için `package.json` uyumluluk meta verilerini kontrol edin. Kod Plugin’i yayınlamaları, `openclaw.compat.pluginApi` ve `openclaw.build.openclawVersion` gibi OpenClaw uyumluluk alanlarına ihtiyaç duyar.
...
## Yayınlama, bir namespace talep edildiği veya ayrıldığı için başarısız oluyor

Yayınlama; sahip kullanıcı adı, kuruluş namespace’i, paket kapsamı, Skills slug değeri veya paket adı zaten talep edilmiş ya da ayrılmış olduğu için başarısız olursa, önce namespace ile eşleşen sahip üzerinden yayınladığınızı doğrulayın. Plugin paketleri için `@example-org/example-plugin` gibi kapsamlı adlar, eşleşen `example-org` sahibi olarak yayınlanmalıdır.

Kuruluşunuzun, projenizin veya markanızın hak sahibi namespace sahibi olduğunu düşünüyor ancak mevcut ClawHub sahibini yönetemiyorsanız, herkese açık ve hassas olmayan kanıtlarla bir [Kuruluş / Namespace Talep sorunu](https://github.com/openclaw/clawhub/issues/new?template=org-namespace-claim.yml) açın. Kanıt rehberliği ve herkese açık sorunların dışında tutulması gerekenler için [Kuruluş ve Namespace Talepleri](/tr/clawhub/namespace-claims) bölümüne bakın.
...
- Gizli, özel, bekletilen veya moderasyon nedeniyle engellenmiş içerikleri genel API yüzeyinin dışında yansıtmayın.
