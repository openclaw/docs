...
x-i18n:
    generated_at: "2026-07-03T09:55:33Z"
    model: gpt-5.5
...
- Geri çağırma hiç gelmiyorsa yerel güvenlik duvarı, VPN ve proxy kurallarını denetleyin.
...
- `RateLimit-Remaining`: üst bilgi mevcut olduğunda kalan tam kotanız. `429` durumunda bu değer `0` olur.
...
- Biliyorsanız tam slug değerini veya sahip sayfasını denetleyin.
- Yayının herkese açık olduğunu ve tarama ya da moderasyon nedeniyle bekletilmediğini doğrulayın.
- Skill size aitse oturum açıp inceleyin:
...
Skills için `SKILL.md` frontmatter bölümünü denetleyin. Gerekli ortam değişkenleri ve
araçlar, kullanıcıların ve tarayıcıların paketi anlayabilmesi için bildirilmelidir.

Plugins için `package.json` uyumluluk meta verilerini denetleyin. Code-plugin yayımlamaları,
...
- Kaynak URL'sinin herkese açık veya ClawHub tarafından erişilebilir olduğunu denetleyin.
- GitHub kaynakları için `owner/repo`, `owner/repo@ref` veya tam bir GitHub URL'si kullanın.

## Bir ad alanı sahiplenildiği veya rezerve edildiği için yayımlama başarısız oluyor

Sahip tanıtıcısı, kuruluş ad alanı, paket kapsamı, skill slug değeri veya paket adı zaten
sahiplenilmiş ya da rezerve edilmiş olduğu için yayımlama başarısız olursa, önce ad alanıyla
eşleşen sahip üzerinden yayımladığınızı doğrulayın. Plugin paketleri için
...
geçerli ClawHub sahibini yönetemiyorsanız herkese açık, hassas olmayan kanıtlarla bir
...
- Tarama durumu ve uyumluluk meta verileri için paket ayrıntı sayfasını denetleyin.
- OpenClaw sürümünüzün paketin duyurduğu uyumluluk aralığını karşıladığını doğrulayın.
- Paket gizli, bekletiliyor veya engellenmişse, sahibi sorunu çözene kadar yüklenemeyebilir.
