...
x-i18n:
    generated_at: "2026-07-04T04:02:40Z"
    model: gpt-5.5
...
- `RateLimit-Remaining`: üstbilgi mevcut olduğunda tam kalan bütçeniz. `429` durumunda `0` olur.
- `RateLimit-Reset` veya `X-RateLimit-Reset`: sıfırlama zamanlaması.

Birçok kullanıcı tek bir çıkış IP'sini paylaşıyorsa, her kişi yalnızca birkaç istek
gönderse bile anonim IP limitlerine ulaşılabilir. Mümkün olduğunda oturum açın ve
...
`openclaw.compat.pluginApi` ve `openclaw.build.openclawVersion` gibi OpenClaw uyumluluk alanlarını
gerektirir.
...
Sahip tanıtıcısı, kuruluş ad alanı, paket kapsamı, beceri slug'ı veya paket adı zaten
sahiplenildiği ya da ayrıldığı için yayınlama başarısız olursa, önce ad alanıyla eşleşen
sahip üzerinden yayınladığınızı doğrulayın. Plugin paketleri için
...
- Paket gizli, bekletiliyor veya engellenmişse, sahip sorunu çözene kadar kurulabilir olmayabilir.
