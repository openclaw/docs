...
istek gönderse bile anonim IP sınırlarına ulaşılabilir. Mümkün olan yerlerde
...
Sahibin görebileceği tanılama bilgileri tarama, yükleme kapısı veya moderasyon durumunu açıklayabilir.
...
Plugin'ler için `package.json` uyumluluk metadata'sını kontrol edin. Code-plugin yayımlamaları,
...
ClawHub, paketleri yayımlayanlarıyla ilişkilendirmek için GitHub kimliği ve kaynak
...
## Namespace alınmış veya ayrılmış olduğu için yayımlama başarısız oluyor

Sahip kullanıcı adı, kuruluş namespace'i, paket kapsamı, beceri slug'ı veya paket
adı zaten alınmış ya da ayrılmış olduğu için yayımlama başarısız olursa, önce
namespace ile eşleşen sahip olarak yayımlama yaptığınızı doğrulayın. Plugin paketleri için,
`@example-org/example-plugin` gibi kapsamlı adlar eşleşen `example-org` sahibi
olarak yayımlanmalıdır.

Kuruluşunuzun, projenizin veya markanızın namespace'in meşru sahibi olduğuna
