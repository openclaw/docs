...
read_when:
    ...
    - Skill veya plugin yüklenip yüklenmeyeceğine karar verme
...
x-i18n:
    generated_at: "2026-07-04T04:02:05Z"
    model: gpt-5.5
...
güvenli olup olmadığına karar vermenize yardımcı olur. Bunlar, bir sürümün ne
yaptığını, hangi yetkileri istediğini ve dosyalara, hesaplara, kimlik
bilgilerine, koda veya harici hizmetlere erişmeden önce ek dikkat gerektiren
bir şey olup olmadığını gösterir.
...
`Pass` güven vericidir, ancak kendi değerlendirmenizin yerine geçmez. Bu,
içerik yayımlayabilen, verileri düzenleyebilen, komut çalıştırabilen, dosya
okuyabilen veya üretim sistemlerine erişebilen araçlar için en önemlidir.
...
Örneğin, yayımlama yapan bir beceri `Medium` riskle birlikte `Review`
gösterebilir. Bu, onun kötü amaçlı olduğu anlamına gelmez. Becerinin amaçla
uyumlu göründüğü, ancak anlamlı hesap yetkisiyle hareket edebildiği anlamına
gelir.
...
Düşük güven düzeyine sahip bulgular, sayfanın yararlı kanıtlara odaklı kalması
...
amaçlı yapıtlar, motor isabetleri ve itibar sinyalleri için özellikle yararlıdır.
...
ClawHub'ın özetlenecek tedarikçi sayısı telemetrisi olmadığında, denetim şöyle der:
...
yönergeler, meta veriler, bildirilen izinler, dosyalar, yetenek sinyalleri,
...
Yeteneğin açıklanmış, amaçla uyumlu ve sürümün belirtilen kullanım amacıyla
