...
summary: ClawHub güvenlik denetimi sonuçlarını bir Skills veya Plugin yüklemeden önce nasıl anlayabilirsiniz.
...
harici hizmetlere erişmeden önce herhangi bir şeyin ekstra dikkat gerektirip
...
`Pass` güven vericidir, ancak kendi değerlendirmenizin yerini almaz. Bu, içerik
yayımlayabilen, verileri düzenleyebilen, komut çalıştırabilen, dosyaları okuyabilen
veya üretim sistemlerine erişebilen araçlar için en önemlidir.

## Risk seviyesi

Risk seviyesi etki alanını açıklar: amaçlandığı şekilde kullandığınızda sürümün
...
önemdeki bulgular, risk seviyesine ve denetim durumuna daha güçlü katkıda bulunur.
...
yerel komutlara, sağlayıcı API'lerine veya paket yüklemelerine ihtiyaç duyar. Denetim,
...
VirusTotal, bilinen kötü amaçlı yapıtlar, motor isabetleri ve ClawHub'ın ajan
farkındalıklı incelemesini tamamlayan itibar sinyalleri için özellikle yararlıdır.
Tedarikçi motor sayıları mevcut olduğunda denetim bunları sade dille özetler,
örneğin:
...
bağlamdır; bağımsız bir herkese açık denetim bölümü veya yüklemeyi engelleyen
hüküm değildir.

Risk analizi, prompt enjeksiyonu, araç kötüye kullanımı, kimlik bilgisi açığa
...
Yeteneğin açıklanmış, amaca uygun ve sürümün belirtilen kullanım senaryosuyla
