...
x-i18n:
    generated_at: "2026-07-01T18:17:51Z"
    model: gpt-5.5
...
## Yüklemeden önce ne kontrol edilmeli
...
`Pass` güven vericidir, ancak kendi değerlendirmenizin yerini almaz. Bu, içerik yayımlayabilen, verileri düzenleyebilen, komut çalıştırabilen, dosya okuyabilen veya üretim sistemlerine erişebilen araçlar için en önemlidir.

## Risk seviyesi

Risk seviyesi etki alanını açıklar: sürüm, amaçlandığı şekilde kullanıldığında ne kadar güce sahip görünüyor.
...
Örneğin, yayımlama yapan bir beceri `Medium` riskle `Review` gösterebilir. Bu, onun kötü amaçlı olduğu anlamına gelmez. Becerinin amacına uygun göründüğü, ancak anlamlı hesap yetkisiyle hareket edebildiği anlamına gelir.
...
Bulgular `Info`, `Low`, `Medium`, `High` veya `Critical` olarak etiketlenebilir. Daha yüksek önem derecesine sahip bulgular, risk seviyesine ve denetim durumuna daha güçlü katkıda bulunur.

Düşük güvenli bulgular herkese açık denetim özetinden gizlenir; böylece sayfa faydalı kanıtlara odaklı kalır.
...
Ana soru tutarlılıktır: ad, özet, meta veriler, istenen yetki ve gerçek içerik, kullanıcıların makul olarak bekleyeceği şeylerle uyumlu mu?
...
ClawHub, denetim yığınında VirusTotal'ı kötü amaçlı yazılım telemetrisi olarak kullanır. VirusTotal, dosya itibarı ve kötü amaçlı yazılım taraması için güvenilir bir sektör standardıdır ve iş birliğimiz ClawHub'ın beceri ve Plugin incelemesine daha geniş güvenlik istihbaratı eklemesini sağlar.

VirusTotal özellikle bilinen kötü amaçlı yapıtlar, motor eşleşmeleri ve ClawHub'ın ajan farkındalıklı incelemesini tamamlayan itibar sinyalleri için yararlıdır. Tedarikçi motor sayıları mevcut olduğunda denetim bunları sade bir dille özetler, örneğin:
...
Risk analizi, ClawHub'ın kendi güvenlik denetim sistemi olan ClawScan tarafından dahili olarak desteklenir. Her sürümü ajanlara yönelik bir yapıt olarak inceler: talimatlar, meta veriler, beyan edilen izinler, dosyalar, yetenek sinyalleri, statik tarama sinyalleri, SkillSpector bulguları, VirusTotal telemetrisi ve yayımcı tarafından sağlanan bağlam. Statik tarama sinyalleri bu inceleme için dahili bağlamdır; bağımsız bir herkese açık denetim bölümü veya yüklemeyi engelleyen bir karar değildir.

Risk analizi, prompt enjeksiyonu, araç kötüye kullanımı, kimlik bilgisi açığa çıkması, güvenli olmayan yürütme, bellek veya bağlam zehirlenmesi ve aşırı yetki gibi riskleri değerlendirmek için [OWASP Agentic Skills Top 10](https://owasp.org/www-project-agentic-skills-top-10/) listesini bir mercek olarak kullanır.
