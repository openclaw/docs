...
read_when:
    ...
    - Bir skill veya plugin kurup kurmamaya karar verme
...
x-i18n:
    generated_at: "2026-07-02T22:44:13Z"
    model: gpt-5.5
...
garantisi değildir. Hassas erişim vermeden önce her zaman kendi muhakemenizi
kullanın.
...
`Pass` güven vericidir, ancak kendi muhakemenizin yerini tutmaz. Bu, içerik
yayınlayabilen, verileri düzenleyebilen, komut çalıştırabilen, dosya okuyabilen
veya üretim sistemlerine erişebilen araçlar için en önemlidir.

## Risk düzeyi

Risk düzeyi etki alanını açıklar: sürümü amaçlandığı şekilde kullandığınızda
...
Örneğin, yayınlama amaçlı bir beceri `Medium` riskle `Review` gösterebilir. Bu,
onun kötü amaçlı olduğu anlamına gelmez. Becerinin amacına uygun göründüğü,
ancak anlamlı hesap yetkisiyle hareket edebileceği anlamına gelir.
...
- beceri yönergeleri veya eklenti meta verileri
...
kullanıcıların makul şekilde bekleyeceği şeyle uyuşuyor mu?
...
Tedarikçi motor sayıları mevcut olduğunda denetim bunları şu gibi yalın bir dille
özetler:
...
yönergeler, meta veriler, beyan edilen izinler, dosyalar, yetenek sinyalleri,
...
için
[OWASP Ajan Temelli Skills İlk 10](https://owasp.org/www-project-agentic-skills-top-10/)
çerçevesini kullanır.

ClawScan, korkutucu görünen bir yeteneği otomatik olarak kötü amaçlı saymaz.
Yeteneğin açıklanıp açıklanmadığını, amaçla uyumlu olup olmadığını ve sürümün
