...
read_when:
    ...
    - Bir Skills veya Plugin yükleyip yüklememeye karar verme
...
x-i18n:
    generated_at: "2026-07-04T20:39:40Z"
    model: gpt-5.5
...
Risk düzeyi etki alanını tanımlar: Sürümü amaçlandığı şekilde kullandığınızda ne kadar güce sahip göründüğünü belirtir.
...
Örneğin, bir yayımlama becerisi `Medium` riskle `Review` gösterebilir. Bu onun kötü amaçlı olduğu anlamına gelmez. Becerinin amaca uygun göründüğü, ancak anlamlı hesap yetkisiyle hareket edebildiği anlamına gelir.
...
Bulgular `Info`, `Low`, `Medium`, `High` veya `Critical` olarak etiketlenebilir. Daha yüksek önem düzeyindeki bulgular, risk düzeyine ve denetim durumuna daha güçlü katkıda bulunur.
...
Ana soru tutarlılıktır: ad, özet, meta veriler, istenen yetki ve gerçek içerik, kullanıcıların makul biçimde bekleyeceği şeyle uyumlu mu?
...
ClawHub, denetim yığınında kötü amaçlı yazılım telemetrisi olarak VirusTotal kullanır. VirusTotal, dosya itibarı ve kötü amaçlı yazılım taraması için güvenilir bir sektör standardıdır ve ortaklığımız ClawHub'ın beceri ve Plugin incelemesine daha geniş güvenlik istihbaratı eklemesini sağlar.
...
Risk analizi, prompt injection, araçların kötüye kullanımı, kimlik bilgisi ifşası, güvenli olmayan yürütme, bellek veya bağlam zehirlenmesi ve aşırı agency gibi riskler için [OWASP Agentic Skills Top 10](https://owasp.org/www-project-agentic-skills-top-10/) listesini bir mercek olarak kullanır.

ClawScan, korkutucu görünen bir yeteneği otomatik olarak kötü amaçlı saymaz. Yeteneğin açıklanmış, amaca uygun ve sürümün belirtilen kullanım durumuyla desteklenmiş olup olmadığını sorar.
