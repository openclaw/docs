...
x-i18n:
    generated_at: "2026-07-04T10:58:01Z"
    model: gpt-5.5
...
`Pass` güven vericidir, ancak kendi değerlendirmenizin yerine geçmez. Bu en çok içerik yayımlayabilen, verileri düzenleyebilen, komut çalıştırabilen, dosyaları okuyabilen veya üretim sistemlerine erişebilen araçlar için önemlidir.
...
Örneğin, yayımlama yapan bir beceri `Medium` riskle `Review` gösterebilir. Bu, onun kötü amaçlı olduğu anlamına gelmez. Beceri amaca uygun görünüyor, ancak anlamlı hesap yetkisiyle hareket edebiliyor demektir.
...
- yükleme yönergeleri ve paket meta verileri
- dahil edilen dosyalar ve dosya manifestleri
- uyumluluk ve yetenek meta verileri

Ana soru tutarlılıktır: ad, özet, meta veriler, istenen yetki ve gerçek içerik, kullanıcıların makul şekilde bekleyeceği şeyle örtüşüyor mu?

Güçlü davranış otomatik olarak kötü değildir. Birçok yararlı araç kimlik bilgilerine, yerel komutlara, sağlayıcı API'lerine veya paket yüklemelerine ihtiyaç duyar. Denetim, bu gücün beklenen, açıklanmış ve orantılı olup olmadığını kontrol eder.
...
VirusTotal özellikle bilinen kötü amaçlı artefaktlar, motor isabetleri ve ClawHub'ın ajan farkındalıklı incelemesini tamamlayan itibar sinyalleri için yararlıdır. Tedarikçi motor sayıları mevcut olduğunda, denetim bunları sade bir dille özetler, örneğin:
...
Risk analizi, ClawHub'ın kendi güvenlik denetim sistemi olan ClawScan tarafından dahili olarak desteklenir. Her sürümü ajanlara yönelik bir artefakt olarak inceler: yönergeler, meta veriler, beyan edilen izinler, dosyalar, yetenek sinyalleri, statik tarama sinyalleri, SkillSpector bulguları, VirusTotal telemetrisi ve yayımcı tarafından sağlanan bağlam. Statik tarama sinyalleri bu inceleme için dahili bağlamdır; bağımsız bir herkese açık denetim bölümü veya yüklemeyi engelleyen karar değildir.

Risk analizi, prompt enjeksiyonu, araçların kötüye kullanımı, kimlik bilgisi ifşası, güvenli olmayan yürütme, bellek veya bağlam zehirlenmesi ve aşırı ajan yetkisi gibi riskler için [OWASP Agentic Skills Top 10](https://owasp.org/www-project-agentic-skills-top-10/) listesini bir bakış açısı olarak kullanır.

ClawScan, korkutucu görünen bir yeteneği otomatik olarak kötü amaçlı kabul etmez. Yeteneğin açıklanıp açıklanmadığını, amaca uygun olup olmadığını ve sürümün belirtilen kullanım amacı tarafından desteklenip desteklenmediğini sorar.
