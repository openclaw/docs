...
- Kullanıcıların kaynak kayıt kaydını inceleyebilmesi için listelemeleri görüntülerken kanonik ClawHub skill URL'sine geri bağlantı verin.
...
Başlıklar: `X-RateLimit-Limit`, `X-RateLimit-Remaining`, `X-RateLimit-Reset`, `RateLimit-Limit`, `RateLimit-Remaining`, `RateLimit-Reset`, `Retry-After` (`429` durumunda).
...
- `POST /api/v1/users/reserve`, bir owner handle için kök slug'ları ve özel yayımsız paket yer tutucularını ayırır.
