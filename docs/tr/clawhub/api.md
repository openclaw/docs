...
x-i18n:
    generated_at: "2026-07-05T07:59:57Z"
    model: gpt-5.5
...
ClawHub'ın herkese açık okuma API'leri üzerine üçüncü taraf bir katalog, dizin veya arama yüzeyi oluşturabilirsiniz. Herkese açık skill meta verileri ve skill dosyaları ClawHub'ın skill lisansı kuralları kapsamında yayımlanırken, API'nin kendisi hız sınırına tabidir ve sorumlu şekilde tüketilmelidir.
...
- Yanıtları önbelleğe alın ve agresif şekilde yoklama yapmak yerine `429`, `Retry-After` ve hız sınırı üst bilgilerine uyun.
...
- Herkese açık API filtrelerini veya kimlik doğrulama sınırlarını atlayarak gizli, özel ya da moderasyon tarafından engellenmiş içerikleri yansıtmayın.
...
- `GET /api/v1/search?q=...`
  ...
  - Eski alias: `nonSuspicious=true`
- `GET /api/v1/skills?limit=&cursor=&sort=`
  - `sort`: `updated` (varsayılan), `recommended` (`default`), `createdAt` (`newest`), `downloads`, `stars` (`rating`), eski kurulum alias'ları `installsCurrent`/`installs`/`installsAllTime` `downloads` değerine eşlenir, `trending`
...
- `GET /api/v1/download?slug=&version=&tag=`
  ...
  - `clean` veya `suspicious` taraması olan mevcut GitHub destekli Skills, ClawHub baytları yerine
    JSON `public-github` devir tanımlayıcısı döndürür.
...
- `GET /api/v1/packages?limit=&cursor=&sort=`
  - `sort`: `updated` (varsayılan), `recommended`, `downloads`, eski alias `installs`
  - Geçersiz `sort` değerleri `400` döndürür
- `GET /api/v1/plugins?limit=&cursor=&sort=`
  - `sort`: `recommended` (varsayılan), `downloads`, `updated`, eski alias `installs`
...
- `POST /api/v1/skills` (yayınlama, multipart tercih edilir)
