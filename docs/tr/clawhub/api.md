...
x-i18n:
    generated_at: "2026-06-28T08:15:09Z"
    model: gpt-5.5
...
- Herkese açık API filtrelerini veya kimlik doğrulama sınırlarını atlayarak gizli, özel veya moderasyonla engellenmiş içeriği yansıtmayın.
...
- `GET /api/v1/skills?limit=&cursor=&sort=`
  - `sort`: `updated` (varsayılan), `recommended` (`default`), `createdAt` (`newest`), `downloads`, `stars` (`rating`), eski kurulum diğer adları `installsCurrent`/`installs`/`installsAllTime` `downloads` ile eşlenir, `trending`
