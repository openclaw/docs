...
x-i18n:
    generated_at: "2026-06-30T14:19:33Z"
    model: gpt-5.5
...
ClawHub'ın genel okuma API'lerinin üzerine üçüncü taraf bir katalog, dizin veya arama yüzeyi oluşturabilirsiniz. Genel Skills meta verileri ve Skills dosyaları, ClawHub'ın Skills lisans kuralları kapsamında yayımlanır; API'nin kendisi ise hız sınırına tabidir ve sorumlu şekilde tüketilmelidir.
...
- Agresif yoklama yapmak yerine yanıtları önbelleğe alın ve `429`, `Retry-After` ve hız sınırı başlıklarına uyun.
- Listelemeleri gösterirken kullanıcıların kaynak kayıt defteri kaydını inceleyebilmesi için kanonik ClawHub Skills URL'sine bağlantı verin.
...
`X-RateLimit-Remaining`, `RateLimit-Remaining` ve `Retry-After`, `429` durumunda eklenir.
...
- `GET /api/v1/skills?limit=&cursor=&sort=`
  - `sort`: `updated` (varsayılan), `recommended` (`default`), `createdAt` (`newest`), `downloads`, `stars` (`rating`), eski kurulum takma adları `installsCurrent`/`installs`/`installsAllTime` `downloads` ile eşlenir, `trending`
  ...
  - Eski takma ad: `nonSuspicious=true`
  - `nonSuspiciousOnly=true` ile imleç tabanlı sayfalar `limit` öğeden daha az öğe içerebilir; devam etmek için `nextCursor` kullanın.
...
- `GET /api/v1/download?slug=&version=&tag=`
  - Barındırılan Skills öğeleri deterministik ZIP baytları döndürür.
  - `clean` veya `suspicious` taraması olan mevcut GitHub destekli Skills öğeleri, ClawHub baytları yerine
    JSON `public-github` devretme tanımlayıcısı döndürür.
- `GET /api/v1/skills/export?startDate=&endDate=&limit=&cursor=`
  - Barındırılan Skills öğeleri saklanan dosyalar olarak dışa aktarılır.
  - `clean` veya `suspicious` taraması olan mevcut GitHub destekli Skills öğeleri
    `public-github` devretme tanımlayıcıları olarak dışa aktarılır.
...
- `POST /api/v1/users/reserve`, bir sahip kullanıcı adı için kök slug'ları ve özel yayımsız paket yer tutucularını ayırır.
