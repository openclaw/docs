...
x-i18n:
    generated_at: "2026-07-02T17:43:46Z"
    model: gpt-5.5
...
ClawHub'ın herkese açık okuma API'leri üzerine üçüncü taraf bir katalog, dizin veya arama yüzeyi oluşturabilirsiniz. Herkese açık beceri meta verileri ve beceri dosyaları ClawHub'ın beceri lisansı kuralları kapsamında yayımlanırken API'nin kendisi hız sınırına tabidir ve sorumlu şekilde kullanılmalıdır.
...
- Listelemeleri gösterirken kullanıcıların kaynak kayıt defteri kaydını inceleyebilmesi için kanonik ClawHub beceri URL'sine geri bağlantı verin.
...
- Herkese açık API filtrelerini veya kimlik doğrulama sınırlarını aşarak gizli, özel veya moderasyon tarafından engellenmiş içeriği yansıtmayın.
...
- `GET /api/v1/download?slug=&version=&tag=`
  ...
  - `clean` veya `suspicious` taramasına sahip güncel GitHub destekli beceriler,
    ClawHub baytları yerine bir JSON `public-github` devir tanımlayıcısı döndürür.
- `GET /api/v1/skills/export?startDate=&endDate=&limit=&cursor=`
  - Barındırılan beceriler depolanan dosyalar olarak dışa aktarılır.
  - `clean` veya `suspicious` taramasına sahip güncel GitHub destekli beceriler
