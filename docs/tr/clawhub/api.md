...
x-i18n:
    generated_at: "2026-07-01T15:29:32Z"
    model: gpt-5.5
...
ClawHub'ın herkese açık okuma API'leri üzerine üçüncü taraf bir katalog, dizin veya arama yüzeyi oluşturabilirsiniz. Herkese açık Skills meta verileri ve Skills dosyaları ClawHub'ın Skills lisans kuralları kapsamında yayımlanırken API'nin kendisi hız sınırına tabidir ve sorumlu şekilde tüketilmelidir.
...
- Yanıtları önbelleğe alın ve agresif şekilde yoklama yapmak yerine `429`, `Retry-After` ve hız sınırı başlıklarına uyun.
- Listelemeleri gösterirken kullanıcıların kaynak kayıt defteri kaydını inceleyebilmesi için kurallı ClawHub Skills URL'sine geri bağlantı verin.
- Kurallı sayfa URL'lerini `https://clawhub.ai/<owner>/skills/<slug>` biçiminde kullanın.
...
Kimlik doğrulamadan haberdar zorlama:
...
- `GET /api/v1/download?slug=&version=&tag=`
  ...
  - `clean` veya `suspicious` taramasına sahip mevcut GitHub destekli Skills, ClawHub baytları yerine bir JSON `public-github` devir tanımlayıcısı döndürür.
