...
- Listelemeleri gösterirken kullanıcıların kaynak kayıt defteri kaydını inceleyebilmesi için kanonik ClawHub skill URL'sine bağlantı verin.
...
- `X-RateLimit-Remaining` / `RateLimit-Remaining`: mevcut olduğunda tam kalan bütçe;
  parçalı başarılı istekler yaklaşık bir küresel değer döndürmek yerine bunu atlar
...
- `GET /api/v1/download?slug=&version=&tag=`
  ...
  - `clean` veya `suspicious` taramasına sahip mevcut GitHub destekli skill'ler, ClawHub baytları yerine bir
    JSON `public-github` devir tanımlayıcısı döndürür.
- `GET /api/v1/skills/export?startDate=&endDate=&limit=&cursor=`
  - Barındırılan skill'ler depolanmış dosyalar olarak dışa aktarılır.
