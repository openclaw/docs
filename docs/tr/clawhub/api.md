...
ClawHub'ın genel okuma API'leri üzerine üçüncü taraf bir katalog, dizin veya arama yüzeyi oluşturabilirsiniz. Genel beceri meta verileri ve beceri dosyaları ClawHub'ın beceri lisansı kuralları kapsamında yayımlanırken, API'nin kendisi hız sınırlıdır ve sorumlu şekilde tüketilmelidir.
...
- Listelemeleri gösterirken kullanıcıların kaynak kayıt defteri kaydını inceleyebilmesi için kanonik ClawHub beceri URL'sine bağlantı verin.
...
- `GET /api/v1/download?slug=&version=&tag=`
  - Barındırılan beceriler belirlenimci ZIP baytları döndürür.
  - `clean` veya `suspicious` taramasına sahip mevcut GitHub destekli beceriler,
    ClawHub baytları yerine bir JSON `public-github` devir tanımlayıcısı döndürür.
- `GET /api/v1/skills/export?startDate=&endDate=&limit=&cursor=`
  - Barındırılan beceriler depolanmış dosyalar olarak dışa aktarılır.
  - `clean` veya `suspicious` taramasına sahip mevcut GitHub destekli beceriler
