...
ClawHub'ın genel okuma API'leri üzerine üçüncü taraf bir katalog, dizin veya arama yüzeyi oluşturabilirsiniz. Genel Skills meta verileri ve Skills dosyaları ClawHub'ın Skills lisans kuralları kapsamında yayımlanır; API'nin kendisi ise hız sınırına tabidir ve sorumlu şekilde tüketilmelidir.
...
- Listelemeleri gösterirken kullanıcıların kaynak kayıt kaydını inceleyebilmesi için kanonik ClawHub Skills URL'sine bağlantı verin.
...
Üstbilgiler: `X-RateLimit-Limit`, `X-RateLimit-Remaining`, `X-RateLimit-Reset`, `RateLimit-Limit`, `RateLimit-Remaining`, `RateLimit-Reset`, `Retry-After` (`429` üzerinde).
...
- Aksi halde `RateLimit-Reset` kullanın veya gecikmeyi `X-RateLimit-Reset` öğesinden türetin.
