...
x-i18n:
    generated_at: "2026-07-04T06:44:48Z"
    model: gpt-5.5
...
ClawHub'ın herkese açık okuma API'leri üzerinde üçüncü taraf bir katalog, dizin veya arama yüzeyi oluşturabilirsiniz. Herkese açık skill meta verileri ve skill dosyaları ClawHub'ın skill lisans kuralları kapsamında yayımlanırken, API'nin kendisi hız sınırına tabidir ve sorumlu biçimde tüketilmelidir.
...
- Agresif yoklama yapmak yerine yanıtları önbelleğe alın ve `429`, `Retry-After` ve hız sınırı üst bilgilerine uyun.
...
- Standart sayfa URL'lerini `https://clawhub.ai/<owner>/skills/<slug>` biçiminde kullanın.
...
- `X-RateLimit-Remaining` / `RateLimit-Remaining`: mevcut olduğunda kalan kesin bütçe;
  parçalanmış başarılı istekler yaklaşık bir genel değer döndürmek yerine bunu atlar
...
- Yeniden denemelere rastgele gecikme ekleyin.
...
- `GET /api/v1/download?slug=&version=&tag=`
  ...
  - `clean` veya `suspicious` taramasına sahip güncel GitHub destekli skill'ler,
...
- `POST /api/v1/skills` (yayımlama, çok parçalı tercih edilir)
