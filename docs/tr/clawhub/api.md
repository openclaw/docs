...
x-i18n:
    generated_at: "2026-07-03T17:38:43Z"
    model: gpt-5.5
...
ClawHub'ın genel okuma API'leri üzerine üçüncü taraf bir katalog, dizin veya arama yüzeyi oluşturabilirsiniz. Genel yetenek meta verileri ve yetenek dosyaları ClawHub'ın yetenek lisansı kuralları kapsamında yayımlanır; API'nin kendisi ise hız sınırına tabidir ve sorumlu biçimde tüketilmelidir.
...
- Listelemeleri görüntülerken kullanıcıların kaynak kayıt defteri kaydını inceleyebilmesi için kanonik ClawHub yetenek URL'sine geri bağlantı verin.
...
- Genel API filtrelerini veya kimlik doğrulama sınırlarını atlayarak gizli, özel veya moderasyon tarafından engellenmiş içeriği yansıtmayın.
...
Kimlik doğrulamasına duyarlı uygulama:
...
- `GET /api/v1/download?slug=&version=&tag=`
  - Barındırılan yetenekler deterministik ZIP baytları döndürür.
  - `clean` veya `suspicious` taramasına sahip mevcut GitHub destekli yetenekler,
    ClawHub baytları yerine JSON `public-github` devretme tanımlayıcısı döndürür.
- `GET /api/v1/skills/export?startDate=&endDate=&limit=&cursor=`
  - Barındırılan yetenekler saklanan dosyalar olarak dışa aktarılır.
  - `clean` veya `suspicious` taramasına sahip mevcut GitHub destekli yetenekler
...
- `POST /api/v1/users/reserve`, bir sahip tanıtıcısı için kök slug'ları ve özel yayımsız paket yer tutucularını rezerve eder.
