...
x-i18n:
    generated_at: "2026-07-04T15:31:02Z"
    model: gpt-5.5
...
ClawHub'ın herkese açık okuma API'leri üzerine üçüncü taraf bir katalog, dizin veya arama yüzeyi oluşturabilirsiniz. Herkese açık skill meta verileri ve skill dosyaları ClawHub'ın skill lisans kuralları kapsamında yayımlanırken API'nin kendisi hız sınırına tabidir ve sorumlu şekilde tüketilmelidir.
...
- Agresif şekilde yoklama yapmak yerine yanıtları önbelleğe alın ve `429`, `Retry-After` ve hız sınırı üstbilgilerine uyun.
...
- `X-RateLimit-Remaining` / `RateLimit-Remaining`: varsa tam kalan bütçe; parçalanmış başarılı istekler yaklaşık bir küresel değer döndürmek yerine bunu atlar
...
- Aksi halde `RateLimit-Reset` kullanın veya `X-RateLimit-Reset` üzerinden gecikmeyi türetin.
...
- `GET /api/v1/skills?limit=&cursor=&sort=`
  ...
  - Eski takma ad: `nonSuspicious=true`
  - `nonSuspiciousOnly=true` ile imleç tabanlı sayfalar `limit` öğesinden daha az öğe içerebilir; devam etmek için `nextCursor` kullanın.
