...
x-i18n:
    generated_at: "2026-07-05T05:29:23Z"
    model: gpt-5.5
...
ClawHub'ın herkese açık okuma API'leri üzerine üçüncü taraf bir katalog, dizin veya arama yüzeyi oluşturabilirsiniz. Herkese açık skill meta verileri ve skill dosyaları ClawHub'ın skill lisansı kuralları kapsamında yayımlanır; API'nin kendisi ise hız sınırlıdır ve sorumlu biçimde kullanılmalıdır.
...
- Herkese açık API filtrelerini veya auth sınırlarını baypas ederek gizli, özel ya da moderasyon tarafından engellenmiş içeriği yansıtmayın.
...
- `X-RateLimit-Reset`: Unix epoch saniyeleri (mutlak sıfırlanma zamanı)
- `RateLimit-Reset`: sıfırlanmaya kadar gecikme saniyeleri
- `X-RateLimit-Remaining` / `RateLimit-Remaining`: mevcut olduğunda tam kalan bütçe;
  parçalanmış başarılı istekler yaklaşık bir küresel değer döndürmek yerine bunu atlar
...
- Mevcut olduğunda `Retry-After` öğesini tercih edin.
...
- `GET /api/v1/skills?limit=&cursor=&sort=`
  - `sort`: `updated` (varsayılan), `recommended` (`default`), `createdAt` (`newest`), `downloads`, `stars` (`rating`), eski kurulum takma adları `installsCurrent`/`installs`/`installsAllTime` `downloads` öğesine eşlenir, `trending`
  - Geçersiz `sort` değerleri `400` döndürür
  - `cursor`, `trending` olmayan sıralamalar için geçerlidir
...
- `GET /api/v1/skills/export?startDate=&endDate=&limit=&cursor=`
  - Barındırılan Skills, saklanan dosyalar olarak dışa aktarılır.
  - `clean` veya `suspicious` taramasına sahip mevcut GitHub destekli Skills,
