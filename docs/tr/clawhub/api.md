...
ClawHub'ın herkese açık okuma API'leri üzerine üçüncü taraf bir katalog, dizin veya arama yüzeyi oluşturabilirsiniz. Herkese açık skill meta verileri ve skill dosyaları ClawHub'ın skill lisansı kuralları kapsamında yayımlanırken, API'nin kendisi oran sınırlıdır ve sorumlu şekilde kullanılmalıdır.
...
- Yanıtları önbelleğe alın ve agresif yoklama yapmak yerine `429`, `Retry-After` ve oran sınırı üst bilgilerine uyun.
...
- Herkese açık API filtrelerini veya kimlik doğrulama sınırlarını aşarak gizli, özel veya moderasyon nedeniyle engellenmiş içerikleri yansıtmayın.
...
- `Retry-After`: `429` durumunda bekleme için gecikme saniyesi
