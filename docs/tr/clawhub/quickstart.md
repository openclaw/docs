...
x-i18n:
    generated_at: "2026-07-03T23:41:11Z"
    model: gpt-5.5
...
Komut, değişmemiş içeriği atlar. Yeni skills öğeleri `1.0.0` sürümünden başlar; sonraki değişiklikler bir sonraki yama sürümünü otomatik olarak yayınlar. Önizleme için `--dry-run`, açık bir sürüm seçmek için `--version` kullanın.

Yayınlamadan önce `SKILL.md` içindeki meta verileri kontrol edin. Gerekli ortam değişkenlerini, araçları ve izinleri bildirin; böylece kullanıcılar yüklemeden önce skillin neye ihtiyaç duyduğunu anlayabilir. Bkz. [Skill biçimi](/tr/clawhub/skill-format).
...
Kod Pluginleri, `package.json` içinde `openclaw.compat.pluginApi` ve `openclaw.build.openclawVersion` dahil olmak üzere OpenClaw uyumluluk meta verilerini içermelidir.
