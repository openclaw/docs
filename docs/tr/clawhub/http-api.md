...
x-i18n:
    generated_at: "2026-07-01T13:16:48Z"
    model: gpt-5.5
...
Üçüncü taraf dizinleri, ClawHub Skills öğelerini listelemek veya aramak için herkese açık okuma uç noktalarını kullanabilir. Lütfen sonuçları önbelleğe alın, `429`/`Retry-After` değerlerine uyun, kullanıcıları kanonik ClawHub listesine (`https://clawhub.ai/<owner>/skills/<slug>`) geri bağlayın ve ClawHub'ın üçüncü taraf siteyi desteklediği izlenimini vermekten kaçının. Gizli, özel veya moderasyon tarafından engellenmiş içerikleri herkese açık API yüzeyinin dışında yansıtmaya çalışmayın.
...
- `X-RateLimit-Remaining` / `RateLimit-Remaining`: mevcut olduğunda tam kalan bütçe.
  Parçalı başarılı istekler, yaklaşık bir küresel değer döndürmek yerine bu başlığı atlar.
...
- Yalnızca dağıtım güvenilir iletilmiş başlıkları açıkça etkinleştirdiğinde, `cf-connecting-ip` dahil güvenilir istemci IP başlıklarını kullanır.
- ClawHub, uçta istemci IP'lerini tanımlamak için güvenilir iletme başlıklarını kullanır.
- Güvenilir istemci IP'si yoksa, anonim istekler yalnızca hız sınırı türüyle kapsamlandırılmış yedek kotaları kullanır. Bu yedek kotalar çağıran tarafından sağlanan yolları, slug'ları, paket adlarını, sürümleri, sorgu dizelerini veya diğer yapıt parametrelerini içermez.

## Hata yanıtları

Herkese açık v1 hata yanıtları `content-type: text/plain; charset=utf-8` ile düz metindir.
Buna doğrulama hataları (`400`), eksik herkese açık kaynaklar (`404`), kimlik doğrulama ve izin hataları (`401`/`403`), hız sınırları (`429`) ve engellenmiş indirmeler dahildir. İstemciler yanıt gövdesini insan tarafından okunabilir bir dize olarak okumalıdır. Bilinmeyen sorgu parametreleri uyumluluk için yok sayılır, ancak tanınan sorgu parametrelerinde geçersiz değerler `400` döndürür.
...
- Sonuçlar alaka sırasına göre döndürülür (gömme benzerliği + tam slug/ad belirteci artırımları + küçük bir popülerlik önceliği).
- Alaka popülerlikten daha güçlüdür. Kesin bir slug veya görünen ad belirteci eşleşmesi, çok daha güçlü etkileşime sahip daha gevşek bir eşleşmeyi geride bırakabilir.
- ASCII metin, kelime ve noktalama sınırlarında belirteçlere ayrılır. Örneğin `personal-map` bağımsız bir `map` belirteci içerirken, `amap-jsapi-skill` `amap`, `jsapi` ve `skill` içerir; bu nedenle `map` araması `personal-map` için `amap-jsapi-skill` öğesine göre daha güçlü bir sözcüksel eşleşme verir.
- Popülerlik logaritmik olarak ölçeklenir ve üst sınır uygulanır. Yüksek etkileşimli Skills öğeleri, sorgu metni daha zayıf bir eşleşmeyse daha alt sırada yer alabilir.
...
- Kullanıcıların kelimenin tam anlamıyla arayacağı terimleri görünen ada, özete ve etiketlere koyun. Bağımsız bir slug belirtecini yalnızca aynı zamanda korumak istediğiniz kararlı bir kimlik olduğunda kullanın.
- Yeni slug daha iyi bir uzun vadeli kanonik ad olmadığı sürece yalnızca tek bir sorguyu yakalamak için slug'ı yeniden adlandırmayın. Eski slug'lar yönlendirme takma adları olur, ancak kanonik URL, gösterilen slug ve gelecekteki arama özetleri yeni slug'ı kullanır.
- Yeniden adlandırma takma adları, kayıt defteri üzerinden çözümlenen eski URL'ler ve yüklemeler için çözümlemeyi korur, ancak arama sıralaması yeniden adlandırma dizinlendikten sonra kanonik Skill meta verilerine dayanır. Mevcut istatistikler Skill ile kalır.
- Bir Skill beklenmedik şekilde görünmezse, sıralamayla ilgili meta verileri değiştirmeden önce oturum açmışken önce `clawhub inspect @owner/slug` ile moderasyon durumunu kontrol edin.
...
- `cursor` (isteğe bağlı): herhangi bir `trending` olmayan sıralama için sayfalama imleci
...
- `createdAt`, yeni Skill taramaları için kararlıdır; `updated`, mevcut Skills öğeleri yeniden yayımlandığında değişir.
- `nonSuspiciousOnly=true` olduğunda, şüpheli Skills öğeleri sayfa alımından sonra filtrelendiği için imleç tabanlı sıralamalar bir sayfada `limit` öğeden daha azını döndürebilir.
...
- Herkese açık çağıranlar yalnızca zaten işaretlenmiş görünür Skills öğeleri için `200` alır.
- Kanıt herkese açık çağıranlar için redakte edilir ve yalnızca sahipler/moderatörler için ham parçacıklar içerir.
...
`note`, `confirmed` ve `dismissed` için gereklidir; `status` yeniden `open` olarak ayarlanırken atlanabilir. Aynı denetlenebilir iş akışında Skill'i gizlemek için triyaj edilmiş bir bildirimle `finalAction: "hide"` geçirin.
...
- `version.security`, kullanılabilir olduğunda normalleştirilmiş tarama doğrulama durumunu ve tarayıcı ayrıntılarını (VirusTotal + LLM) içerir.
...
- `moderation`, en son sürümden türetilmiş mevcut Skills düzeyinde bir moderasyon anlık görüntüsüdür.
...
- Yanıt, `{ "ok": true, "scanId": "...", "jobId": "...", "status": "queued", "sourceKind": "published", "update": false, "queue": { "queuedAhead": 0, "queuedAheadIsEstimate": false, "position": 1, "running": 0, "runningIsEstimate": false, "note": "Taramalar eşzamansızdır ve tamamlanması zaman alabilir." } }` ile `202` olur.
...
Gönderilen bir tarama için kimlik doğrulamalı yoklama uç noktası.
...
- Skills veya Plugin için sahip/yayımcı yönetim erişimi ya da platform moderatörü/yönetici yetkisi gerektirir.
- Engellenmiş veya gizli sürümler dahil olmak üzere tam gönderilen sürüm için depolanmış tarama sonuçlarını döndürür.
...
- `version` (isteğe bağlı): belirli sürüm dizesi.
- `tag` (isteğe bağlı): etiketli bir sürümü çözümler (örneğin `latest`).
...
- `security`, üst düzey ClawScan/güvenlik hükmüdür. Otomasyon `ok`, `decision`, `reasons` ve `security.status` değerlerini temel almalıdır.
...
- `provenance` yalnızca ClawHub yayımlama veya içe aktarma sırasında bir GitHub depo/ref/commit/path değerini çözüp depoladığında `server-resolved-github-import` olur; aksi takdirde `unavailable` olur.

### `POST /api/v1/skills/-/security-verdicts`

Tam Skills sürümleri için mevcut kompakt güvenlik hükümlerini döndürür. Bu koleksiyon uç noktası, OpenClaw Control UI gibi, hangi kurulu ClawHub Skills sürümlerini göstermesi gerektiğini zaten bilen istemciler için tasarlanmıştır.
...
- Sonuçlar öğe başınadır; eksik bir Skills veya sürüm tüm yanıtı başarısız kılmaz.
- Yanıt yalnızca güvenlikle ilgilidir. Skill Card verilerini, oluşturulan kart durumunu, yapıt dosya listelerini veya ayrıntılı tarayıcı yüklerini içermez.
...
- Tek Skills Skill Card doğrulama zarfına ihtiyacınız olduğunda `/verify`, oluşturulmuş kart Markdown'ına ihtiyacınız olduğunda `/card` ve ayrıntılı tarayıcı verilerine ihtiyacınız olduğunda `/scan` kullanın.
...
- `category` (isteğe bağlı): Plugin kategori filtresi. Yalnızca istek
  ...
  sınırlandırıldığında desteklenir. Denetimli kategoriler ve
...
- Skills girdileri Skills kayıt defteriyle desteklenmeye devam eder ve hâlâ yalnızca `POST /api/v1/skills` üzerinden yayımlanabilir.
...
- Kimliği doğrulanmış çağıranlar, listeleme/arama sonuçlarında üyesi oldukları yayıncıların özel paketlerini görebilir.
...
- `isOfficial` (isteğe bağlı): `true` veya `false`
- `category` (isteğe bağlı): Plugin kategori filtresi. Yalnızca istek
  Plugin paketleriyle sınırlandırıldığında desteklenir. Denetimli kategoriler ve
  eski v1 filtre takma adları `GET /api/v1/plugins` altında belgelenmiştir.
...
- Anonim çağıranlar yalnızca herkese açık paket kanallarını görür.
- Kimliği doğrulanmış çağıranlar, üyesi oldukları yayıncıların özel paketlerinde arama yapabilir.
- `channel=private` yalnızca kimliği doğrulanmış çağıranın okuyabildiği paketleri döndürür.
...
Birleşik `/api/v1/packages` uç noktasında yalnızca Plugin'lere özeldir; Skills kataloğu için
...
- `startDate` (gerekli): Skills `updatedAt` için Unix milisaniye alt sınırı.
- `endDate` (gerekli): Skills `updatedAt` için Unix milisaniye üst sınırı.
...
- Gövde: ZIP arşivi.
- Dışa aktarılan her Skills öğesi `{publisher}/{slug}/` kökünde yer alır.
- Barındırılan Skills öğeleri en son depolanan sürüm dosyalarını içerir ve
  `_manifest.json` içinde `sourceRef: "public-clawhub"` ile listelenir.
- `clean` veya `suspicious` taramasına sahip geçerli GitHub destekli Skills öğeleri
  `_source_handoff.json` içinde `sourceRef: "public-github"`, depo, commit, yol,
  içerik karması ve arşiv URL'si içerir. ClawHub'da barındırılan kaynak dosyalarını içermezler.
- Her Skills öğesi `_export_skill_meta.json` içerir.
- `_manifest.json` her zaman ZIP kökünde bulunur.
- Tekil Skills öğeleri veya dosyalar dışa aktarılamadığında
...
verileri dahil bir paket sürümünü döndürür.
...
- `release.artifactKind`, `release.artifactSha256`, `release.npmIntegrity`,
  `release.npmShasum` ve `release.npmTarballName`, sürüm yapıtı için
...
- `trust.blockedFromDownload`, kurulum engelleme sinyalidir. OpenClaw ve diğer
  kurulum istemcileri, engelleme kurallarını tarayıcı veya moderasyon
  alanlarından yeniden türetmek yerine bu değer `true` olduğunda kurulumu
  engellemelidir.
...
- `trust.stale`, güven özetinin güncelliğini yitirmiş girdilerden hesaplandığı
  ve yüksek güvenli izin kararı öncesinde yenileme gerektiriyor kabul edilmesi
  gerektiği anlamına gelir.

Notlar:

- Bu uç nokta sürüm düzeyinde kesindir. İstemciler, yalnızca en son paket meta
...
- Bu uç nokta, sahip/moderatör moderasyon uç noktalarından bilerek daha dardır.
  Bildirimci kimliklerini, rapor gövdelerini, özel kanıtları veya dahili inceleme
...
- ClawPack sürümleri, tam olarak yüklenen npm-pack `.tgz` baytlarını akıtır.
...
- `packageName` npm-name normalize edilir; planlanan geçişlerde paket eksik
  olabilir.
...
kendi başlarına indirmeleri otomatik gizlemez veya engellemezler; moderatörler
yapıtları onaylamak, karantinaya almak veya iptal etmek için sürüm moderasyonunu
kullanmalıdır.
...
- Bir moderatör veya yönetici kullanıcısı için API token'ı gerektirir.
...
- Paket sahibi, yayıncı üyesi, moderatör veya yönetici kullanıcısı için API
  token'ı gerektirir.
...
Karantinaya alınmış ve iptal edilmiş sürümler, artefakt indirme rotalarından
...
- Özel paketler, çağıran taraf sahip yayıncıyı okuyamıyorsa `404` döndürür.
...
- Bu rota yalnızca ZIP olarak kalır. ClawPack `.tgz` dosyalarını akış olarak göndermez.
...
- Özel paketler, çağıran taraf sahip değilse `404` döndürür.
...
npm ayna istemcileri için tam olarak yüklenmiş ClawPack tarball baytlarını akış olarak gönderir.
...
taramaya sahip, barındırılan sürümü olmayan geçerli GitHub destekli bir skill
...
- GitHub destekli skill devirleri baytları proxy'lemez veya aynalamaz. JSON
  ...
  ve `archiveUrl` içerir; tarama/geçerli durum bir geçittir ve başarı yükü meta
...
- İsteğe bağlı payload alanı: `migrateOwner`. `ownerHandle` ile `true` olduğunda,
  ...
  bu sahibe taşınabilir. Bu açık katılım olmadan sahip değişiklikleri reddedilir.
...
- İzin verilen form alanları `payload`, tekrarlanan `files` blob'ları veya bir
  ...
  akışı tarafından döndürülen bir storage id olabilir. Aşamalı storage-id
  yayımları ayrıca bu yükleme URL'siyle döndürülen `clawpackUploadTicket`
  değerini içermelidir.
- Aynı istekte `files` veya `clawpack` kullanın, ikisini birden kullanmayın.
...
- Code plugin'ler `package.json`, kaynak repo meta verileri, kaynak commit meta
...
- Başkası adına yayımlamalar, official-channel uygunluğunu hedef sahip hesabına göre yine de doğrular.
...
başka bir yayıncı tarafından talep edilebilir. Silme yanıtı, bu süre sonu geçerliyse
...
Yalnızca yönetici. Bir handle için kuruluş yayıncısının var olmasını sağlar. Handle hâlâ
eski paylaşımlı kullanıcı/kişisel yayıncıyı gösteriyorsa uç nokta önce bunu bir kuruluş yayıncısına taşır.
...
çağıran tarafı sahip olarak ekler. Bu uç nokta mevcut kullanıcı/kişisel handle'ları taşımaz ve
...
daha sonra gerçek code-plugin veya bundle-plugin sürümünü bu ad altında yayımlayabilir.
...
için kişisel bir yayıncıyı kurtarır. İstek, değiştirilemez her iki GitHub sağlayıcı hesap id'sini
...
Kurtarma ayrıca, kurtarılan yayıncının skills öğeleri, skill slug takma adları, paketleri,
paket denetleyici uyarıları ve türetilmiş arama digest satırları için eski `ownerUserId` alanlarını taşır; böylece
...
profil eşitlemesi eski kullanıcının rekabet eden yetkisini geri yükleyemez. Her birincil tablo,
uygulama işlemi başına 100 satırla sınırlıdır; daha büyük kurtarmalar önce sürdürülebilir bir sahip geçişi kullanmalıdır.
...
Bir kullanıcıyı yasakla ve sahip olduğu skills öğelerini kalıcı olarak sil (yalnızca moderatör/yönetici).
...
Yasağı kaldırmadan veya içeriği geri yüklemeden mevcut bir yasak için saklanan nedeni değiştir
(yalnızca yönetici). `dryRun`, `false` olmadığı sürece varsayılan olarak dry-run kullanılır.
...
Bir yıldız ekle/kaldır (öne çıkanlar). Her iki uç nokta da idempotenttir.
...
`POST /api/cli/upload-url`, `uploadUrl` ve `uploadTicket` döndürür. ClawPack tarball aşamalandıran paket
yayınları, ortaya çıkan depolama kimliğini `clawpack` olarak
