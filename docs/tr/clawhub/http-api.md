...
- ClawHub, istemci IP'lerini edge üzerinde tanımlamak için güvenilir iletme başlıklarını kullanır.
- Güvenilir bir istemci IP'si yoksa anonim indirme istekleri tek bir global `ip:unknown` kovası yerine uç nokta kapsamlı bir yedek kovayı kullanır. Anonim okuma/yazma istekleri hâlâ paylaşılan bilinmeyen kovayı kullanır; böylece eksik-IP yönlendirmesi görünür ve korumacı kalır.
...
- Sonuçlar alaka sırasına göre döndürülür (embedding benzerliği + tam slug/ad belirteci artırmaları + indirmelerden gelen popülerlik önceliği).
- Alaka, popülerlikten daha güçlüdür. Kesin bir slug veya görünen-ad belirteci eşleşmesi, çok daha fazla indirmeye sahip daha gevşek bir eşleşmenin önüne geçebilir.
- ASCII metin, sözcük ve noktalama sınırlarında belirteçlere ayrılır. Örneğin `personal-map` bağımsız bir `map` belirteci içerirken `amap-jsapi-skill` `amap`, `jsapi` ve `skill` içerir; bu nedenle `map` araması `personal-map` için `amap-jsapi-skill` öğesine göre daha güçlü bir sözlüksel eşleşme sağlar.
- İndirmeler birincil sıralama sinyali olarak değil, küçük log ölçekli bir öncelik ve eşitlik bozucu olarak kullanılır. Çok indirilen Skills, sorgu metni daha zayıf eşleştiğinde daha düşük sıralanabilir.
...
- Yeni slug daha iyi uzun vadeli kanonik ad değilse, yalnızca tek bir sorgunun peşinden gitmek için slug'ı yeniden adlandırmayın. Eski slug'lar yönlendirme alias'larına dönüşür, ancak kanonik URL, görüntülenen slug ve gelecekteki arama özetleri yeni slug'ı kullanır.
- Yeniden adlandırma alias'ları eski URL'ler ve registry üzerinden çözümlenen kurulumlar için çözümlemeyi korur, ancak arama sıralaması, yeniden adlandırma indekslendikten sonra kanonik Skill meta verilerine dayanır. Mevcut istatistikler Skill ile kalır.
- Bir Skill beklenmedik şekilde görünmüyorsa sıralamayla ilgili meta verileri değiştirmeden önce oturum açmış durumdayken `clawhub inspect <slug>` ile önce moderasyon durumunu kontrol edin.
...
- `nonSuspiciousOnly=true` olduğunda, imleç tabanlı sıralamalar bir sayfada `limit` öğesinden daha az döndürebilir çünkü şüpheli Skills sayfa alındıktan sonra filtrelenir.
...
ayarlanırken atlanabilir. Aynı denetlenebilir iş akışında Skill öğesini gizlemek için triyajlanmış
bildirimle birlikte `finalAction: "hide"` iletin.
...
- `security.capabilityTags`, tespit edildiğinde `crypto`, `requires-wallet`, `can-make-purchases`, `can-sign-transactions`,
...
- Geçmiş bir sürüm sorgulanırken `moderation` ve `security` öğelerini aynı sürüm bağlamı olarak değerlendirmeden önce `moderation.matchesRequestedVersion` ve `moderation.sourceVersion` değerlerini kontrol edin.
...
- `target` / `hostTarget`, `os`, `arch`, `libc`, `requiresBrowser`,
  ...
  `osPermission`, yaygın yetenek etiketleri için kısa yazım olarak kabul edilir
- `artifactKind` (isteğe bağlı): `legacy-zip` veya `npm-pack`
- `npmMirror` (isteğe bağlı): npm yansısı üzerinden kullanılabilen
...
Bir paketi ve tüm sürümlerini soft-delete ile siler.
...
- Eski paket sürümleri bir `legacy-zip` artifact ve eski ZIP `downloadUrl` döndürür.
- ClawPack sürümleri bir `npm-pack` artifact, npm bütünlük alanları, bir `tarballUrl` ve eski ZIP uyumluluk URL’si döndürür.
...
- ClawPack sürümleri, tam yüklenen npm-pack `.tgz` baytlarını stream eder.
...
Bir paketi moderatör incelemesi için raporlar. Raporlar paket düzeyindedir ve isteğe bağlı olarak bir sürüme bağlanabilir. Moderasyon kuyruğunu beslerler ancak kendi başlarına indirmeleri otomatik olarak gizlemez veya engellemezler; moderatörler artifact’leri onaylamak, karantinaya almak veya iptal etmek için sürüm moderasyonunu kullanmalıdır.
...
ayarlanırken atlanabilir. Aynı denetlenebilir iş akışında yayın moderasyonu uygulamak
için doğrulanmış bir raporla `finalAction: "quarantine"` veya
...
Karantinaya alınmış ve iptal edilmiş yayınlar, yapı indirme rotalarından `403` döndürür.
Her değişiklik bir denetim günlüğü girdisi yazar.

### `POST /api/v1/packages/backfill/artifacts`

Eski paket yayınlarını açık yapı türü meta verileriyle etiketlemek için yalnızca
yönetici bakım uç noktası.
...
- Bu işlem ClawPack oluşturmaz veya yapı baytlarını değiştirmez.
...
- İndirme kotası bölmesini değil, okuma kotası bölmesini kullanır.
- İkili dosyalar `415` döndürür.
- Dosya boyutu sınırı: 200KB.
- Bekleyen VirusTotal taramaları okumaları engellemez; kötü amaçlı yayınlar başka yerlerde yine de alıkonabilir.
- Özel paketler, çağıran sahip yayıncıyı okuyamıyorsa `404` döndürür.

### `GET /api/v1/packages/{name}/download`

Bir paket yayını için eski deterministik ZIP arşivini indirir.
...
npm ayna istemcileri için yüklenen ClawPack tarball baytlarının aynısını akış olarak sunar.

Notlar:

- İndirme kotası bölmesini kullanır.
...
- İsteğe bağlı yük alanı: `ownerHandle`. Varsa API bu yayıncıyı sunucu tarafında
  çözümler ve aktörün yayıncı erişimine sahip olmasını gerektirir.
...
- Tercih edilen: `payload` JSON + `files[]` blob’ları ile `multipart/form-data`.
- `files` içeren JSON gövdesi (storageId tabanlı) de kabul edilir.
- İsteğe bağlı yük alanı: `ownerHandle`. Varsa yalnızca yöneticiler bu sahip adına yayın yapabilir.
...
- Başkası adına yapılan yayınlar, resmi kanal uygunluğunu yine hedef sahip hesabına göre doğrular.
...
yayıncı tarafından talep edilebilir. Silme yanıtı, bu sürenin geçerli olduğu durumlarda
...
Yalnızca yönetici. Bir tanıtıcı için bir kuruluş yayıncısının var olmasını sağlar. Tanıtıcı hâlâ
...
Yalnızca yönetici. Kök slug’ları ve paket adlarını, yayın yayımlamadan hak sahibi için ayırır.
Paket adları, yayın satırı olmayan özel yer tutucu paketlere dönüşür; böylece aynı sahip
daha sonra gerçek code-plugin veya bundle-plugin yayınını bu ada yayımlayabilir.
...
Yıldız ekler/kaldırır (öne çıkarır). Her iki uç nokta da idempotenttir.
...
CLI, site üzerinden kayıt defteri/kimlik doğrulama ayarlarını keşfedebilir:
...
Kendi barındırmanızda, bu dosyayı sunun (veya `CLAWHUB_REGISTRY` değişkenini açıkça ayarlayın; eski `CLAWDHUB_REGISTRY`).
