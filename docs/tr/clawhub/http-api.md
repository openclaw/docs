...
x-i18n:
    generated_at: "2026-07-04T06:46:27Z"
    model: gpt-5.5
...
Üçüncü taraf dizinler, ClawHub Skills öğelerini listelemek veya aramak için herkese açık okuma uç noktalarını kullanabilir. Lütfen sonuçları önbelleğe alın, `429`/`Retry-After` değerlerine uyun, kullanıcıları kanonik ClawHub listesine (`https://clawhub.ai/<owner>/skills/<slug>`) geri bağlayın ve ClawHub'ın üçüncü taraf siteyi desteklediğini ima etmekten kaçının. Gizli, özel veya moderasyon tarafından engellenmiş içeriği herkese açık API yüzeyi dışında yansıtmaya çalışmayın.
...
- Kimliği doğrulanmış yazma uç noktaları, sunucu nedeni bildiğinde yalın bir `Unauthorized`
  ...
  istemcilerinin kullanıcıya neyin engel olduğunu söyleyebilmesi için uygulanabilir metin almalıdır.
...
- Güvenilir istemci IP'si yoksa, anonim istekler yalnızca hız sınırı türüne göre
  kapsamlandırılmış yedek kovalardan yararlanır. Bu yedek kovalar
  çağıranın sağladığı yolları, slug değerlerini, paket adlarını, sürümleri, sorgu dizelerini veya diğer
...
- `highlightedOnly` (isteğe bağlı): vurgulanmış Skills öğelerine filtrelemek için `true`
...
- Sonuçlar alaka sırasına göre döndürülür (gömme benzerliği + kesin slug/ad belirteci artırımları + küçük bir popülerlik önceliği).
- Alaka, popülerlikten daha güçlüdür. Kesin bir slug veya görünen ad belirteci eşleşmesi, çok daha güçlü etkileşime sahip daha gevşek bir eşleşmenin üstüne çıkabilir.
- ASCII metin, sözcük ve noktalama sınırlarında belirteçlere ayrılır. Örneğin `personal-map` bağımsız bir `map` belirteci içerirken, `amap-jsapi-skill` `amap`, `jsapi` ve `skill` içerir; bu nedenle `map` aramak `personal-map` için `amap-jsapi-skill` değerinden daha güçlü bir sözcüksel eşleşme sağlar.
...
- Kullanıcıların gerçekten arayacağı terimleri görünen ada, özete ve etiketlere koyun. Bağımsız bir slug belirtecini yalnızca aynı zamanda korumak istediğiniz kararlı bir kimlikse kullanın.
- Yeni slug daha iyi uzun vadeli kanonik ad değilse, yalnızca tek bir sorgunun peşinden gitmek için slug adını değiştirmeyin. Eski slug değerleri yönlendirme takma adlarına dönüşür, ancak kanonik URL, görüntülenen slug ve gelecekteki arama özetleri yeni slug değerini kullanır.
...
- `nonSuspiciousOnly=true` olduğunda, şüpheli Skills sayfa alımından sonra filtrelendiği için imleç tabanlı sıralamalar bir sayfada `limit` öğeden daha az döndürebilir.
...
- Sahip yeniden adlandırma/birleştirme akışları tarafından oluşturulan eski slug değerleri kanonik Skill öğesine çözümlenir.
- `metadata.os`: Skill frontmatter içinde bildirilen OS kısıtlamaları (ör. `["macos"]`, `["linux"]`). Bildirilmemişse `null`.
- `metadata.systems`: Nix sistem hedefleri (ör. `["aarch64-darwin", "x86_64-linux"]`). Bildirilmemişse `null`.
- Skill platform meta verisine sahip değilse `metadata` `null` olur.
...
Bir Skill öğesini moderatör incelemesi için bildirin. Bildirimler Skill düzeyindedir, isteğe bağlı olarak
...
ayarlanırken atlanabilir. Aynı denetlenebilir iş akışında Skill öğesini gizlemek için önceliklendirilmiş
bildirimle `finalAction: "hide"` gönderin.
...
- Geçmiş bir sürüm sorgulanırken, `moderation` ve `security` değerlerini aynı sürüm bağlamı olarak ele almadan önce `moderation.matchesRequestedVersion` ve `moderation.sourceVersion` değerlerini denetleyin.
...
- Kuyruktayken `queue.queuedAhead` ve `queue.position` döndürür; böylece istemciler isteğin önünde kaç önceliklendirilmiş manuel tarama olduğunu gösterebilir. Çok büyük kuyruklar sınırlandırılır ve `queuedAheadIsEstimate: true` ile bildirilir.
...
Tam Skill sürümleri için geçerli kompakt güvenlik kararlarını döndürür. Bu
koleksiyon uç noktası, OpenClaw Control UI gibi hangi yüklü
ClawHub Skill sürümlerini görüntülemeleri gerektiğini zaten bilen istemciler için tasarlanmıştır.
...
- `category` (isteğe bağlı): Plugin kategori filtresi. Yalnızca istek
  ...
  `family=code-plugin`/`family=bundle-plugin` içeren paket uç noktaları) desteklenir.
...
- Kimliği doğrulanmış çağıranlar, listeleme/arama sonuçlarında üyesi oldukları yayımcılara ait özel paketleri görebilir.
...
- Anonim çağıranlar yalnızca herkese açık paket kanallarını görür.
- Kimliği doğrulanmış çağıranlar, üyesi oldukları yayımcılara ait özel paketlerde arama yapabilir.
- `channel=private` yalnızca kimliği doğrulanmış çağıranın okuyabildiği paketleri döndürür.
...
- `clean` veya `suspicious` taramasına sahip mevcut GitHub destekli Skills,
  ...
  içerik karması ve arşiv URL'si ile `_source_handoff.json` içerir.
...
- `GET /api/v1/plugins` altında belgelenen eski v1 filtre takma adları da
  kabul edilir.
- Kategori filtreleme, arama sorgusunu yeniden yazma değil, Plugin kategori özeti
...
- Plugin araması için tarayıcı kullanıcı arayüzü sıralama denetimleri, yüklenen
  alaka sonuçlarını yeniden sıralar ve geçerli `/skills` göz atma davranışıyla eşleşir.
...
Bir paketi ve tüm sürümlerini yazılımsal olarak siler.
...
- Bu, OpenClaw çözümleyici yüzeyidir; paylaşılan bir URL'den arşiv biçimi tahmin
  etmeyi önler.
...
- ClawPack sürümleri, tam olarak yüklenen npm-pack `.tgz` baytlarını yayınlar.
...
Moderatör incelemesi için bir paketi raporlar. Raporlar paket düzeyindedir ve
isteğe bağlı olarak bir sürüme bağlanır. Moderasyon kuyruğunu beslerler ancak
kendi başlarına indirmeleri otomatik olarak gizlemez veya engellemez; moderatörler
...
Moderatör/yönetici paket raporu alım uç noktası.
...
- Dosya boyutu sınırı: 200KB.
- Bekleyen VirusTotal taramaları okumaları engellemez; kötü amaçlı sürümler başka yerlerde yine de saklanabilir.
...
- Eski yalnızca ZIP sürümler kasıtlı olarak atlanır.
- `dist.tarball`, `dist.integrity` ve `dist.shasum`, kullanıcılar isterse npm'i
  yansıya yönlendirebilsin diye npm uyumlu alanlar kullanır.
...
npm yansısı istemcileri için tam olarak yüklenen ClawPack tarball baytlarını akış olarak sunar.
...
taramaya sahip ve barındırılan sürümü olmayan güncel GitHub destekli bir beceri
...
- GitHub destekli beceri devirleri baytları proxy'lemez veya yansıtmaz. JSON yanıtı
  ...
  `archiveUrl` içerir; tarama/güncel durum bir kapıdır ve başarı yükü meta verisi
  olarak dahil edilmez.
- İndirme istatistikleri UTC günü başına benzersiz kimlikler olarak sayılır (`userId`, API belirteci geçerliyse; aksi takdirde IP).
...
- İsteğe bağlı yük alanı: `ownerHandle`. Varsa API bu yayımcıyı sunucu tarafında
  çözer ve aktörün yayımcı erişimine sahip olmasını gerektirir.
- İsteğe bağlı yük alanı: `migrateOwner`. `ownerHandle` ile `true` olduğunda,
  aktör hem mevcut hem de hedef yayımcılarda yönetici/sahip ise mevcut bir beceri
...
- İzin verilen form alanları `payload`, tekrarlanan `files` blob'ları veya bir
  `clawpack` tarball başvurusudur. `clawpack`, bir `.tgz` blob'u veya upload-url
...
- Doğrudan multipart yayım istekleri 18MB ile sınırlandırılmıştır. ClawPack tarball'ları,
...
- Code Plugin'ler `package.json`, kaynak depo meta verileri, kaynak commit meta
...
- Yalnızca `openclaw` kuruluş yayımcısı ve mevcut `openclaw` kuruluş üyelerinin
  kişisel yayımcıları `official` kanalına yayım yapabilir.
...
Sahip tarafından başlatılan geçici silmeler slug'ı 30 gün rezerve eder; ardından slug
başka bir yayımcı tarafından alınabilir. Silme yanıtı, bu sürenin geçerli olduğu durumlarda
`slugReservedUntil` içerir. Moderatör/yönetici gizlemeleri ve güvenlik kaldırmaları bu
şekilde sona ermez.
...
Yalnızca yönetici. Bir tanıtıcı için kuruluş yayımcısının var olmasını sağlar. Tanıtıcı
hâlâ eski paylaşılan kullanıcı/kişisel yayımcıyı gösteriyorsa uç nokta önce bunu bir
kuruluş yayımcısına geçirir. Yeni oluşturulan bir kuruluş için `memberHandle` sağlayın;
işlemi yapan yönetici üye olarak eklenmez. `memberRole` varsayılan olarak `owner` olur.
...
Kimliği doğrulanmış self servis kuruluş yayımcısı oluşturma. Yeni bir kuruluş yayımcısı
...
- Tanıtıcı zaten bir yayımcı, kullanıcı veya kişisel yayımcı tarafından kullanıldığında `409` döndürür.

### `POST /api/v1/users/reserve`

Yalnızca yönetici. Hak sahibi için kök slug'ları ve paket adlarını bir sürüm yayımlamadan
rezerve eder. Paket adları, sürüm satırları olmayan özel yer tutucu paketlere dönüşür; böylece
aynı sahip daha sonra gerçek code-plugin veya bundle-plugin sürümünü bu ada yayımlayabilir.
...
Yalnızca yönetici. Convex Auth hesap satırlarını düzenlemeden, doğrulanmış bir yedek GitHub
OAuth ana kimliği için kişisel yayımcıyı kurtarır. İstek her iki değişmez GitHub sağlayıcı
hesap kimliğini de adlandırmalıdır; değiştirilebilir tanıtıcılar yalnızca operatöre yönelik
bir koruma olarak kullanılır.

Uç nokta varsayılan olarak kuru çalıştırmaya ayarlanır. Kurtarmayı uygulamak için personel iki GitHub ana kişisi arasındaki sürekliliği bağımsız olarak doğruladıktan sonra `dryRun: false` ve
...
skill slug diğer adları, paketleri, paket denetleyicisi uyarıları ve türetilmiş arama özeti satırları için eski `ownerUserId` alanlarını da taşır. Kurtarılan handle için etkin bir korumalı handle
rezervasyonu da yedek kullanıcıya yeniden atanır; böylece daha sonraki
profil senkronizasyonu eski kullanıcının rakip yetkisini geri yükleyemez. Her birincil tablo, uygulama işlemi başına
...
- `rename`, önceki slug'ı bir yönlendirme diğer adı olarak korur.
...
Bir kullanıcıyı yasaklayın ve sahip olduğu Skills'leri kalıcı olarak silin (yalnızca moderatör/yönetici).
...
Bir kullanıcının yasağını kaldırın ve uygun Skills'leri geri yükleyin (yalnızca yönetici).
...
- `limit` (isteğe bağlı): maksimum sonuç (varsayılan 20, maksimum 200)
