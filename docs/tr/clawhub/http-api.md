...
x-i18n:
    generated_at: "2026-07-01T18:17:15Z"
    model: gpt-5.5
...
- Kimliği doğrulanmış yazma uç noktaları, sunucu nedeni biliyorsa yalın bir `Unauthorized`
  ...
  istemcilerinin kullanıcılara kendilerini neyin engellediğini söyleyebilmesi için eyleme geçirilebilir metin almalıdır.
...
- `X-RateLimit-Remaining` / `RateLimit-Remaining`: varsa tam kalan bütçe.
  Parçalanmış başarılı istekler, yaklaşık bir genel değer döndürmek yerine bu üst bilgiyi atlar.
...
- Güvenilir istemci IP üst bilgilerini, `cf-connecting-ip` dahil, yalnızca dağıtım
  güvenilir iletilmiş üst bilgileri açıkça etkinleştirdiğinde kullanır.
- ClawHub, kenarda istemci IP'lerini tanımlamak için güvenilir yönlendirme üst bilgilerini kullanır.
- Güvenilir istemci IP'si yoksa, anonim istekler yalnızca hız sınırı türüne göre
  kapsamlanan yedek kovalardan yararlanır. Bu yedek kovalar, çağıranın sağladığı
...
Bu; doğrulama hatalarını (`400`), eksik genel kaynakları (`404`), kimlik doğrulama ve
izin hatalarını (`401`/`403`), hız sınırlarını (`429`) ve engellenmiş indirmeleri içerir. İstemciler
...
- Sonuçlar alaka sırasına göre döndürülür (embedding benzerliği + tam slug/ad token artırımları + küçük bir popülerlik öncülü).
...
- ASCII metin, sözcük ve noktalama sınırlarında tokenlara ayrılır. Örneğin, `personal-map` bağımsız bir `map` tokenı içerirken `amap-jsapi-skill` `amap`, `jsapi` ve `skill` içerir; bu nedenle `map` araması `personal-map` için `amap-jsapi-skill`'e göre daha güçlü bir sözcüksel eşleşme sağlar.
- Popülerlik log ölçeklidir ve sınırlandırılır. Yüksek etkileşimli skills, sorgu metni daha zayıf bir eşleşmeyse daha düşük sıralanabilir.
...
- Kullanıcıların gerçekten arayacağı terimleri görünen ada, özete ve etiketlere koyun. Bağımsız bir slug tokenını yalnızca korumak istediğiniz kararlı bir kimlikse kullanın.
- Yeni slug daha iyi bir uzun vadeli kanonik ad olmadıkça, yalnızca tek bir sorguyu yakalamak için slug'ı yeniden adlandırmayın. Eski slug'lar yönlendirme takma adları olur, ancak kanonik URL, görüntülenen slug ve gelecekteki arama özetleri yeni slug'ı kullanır.
- Yeniden adlandırma takma adları eski URL'ler ve kayıt defteri üzerinden çözümlenen yüklemeler için çözümlemeyi korur, ancak arama sıralaması yeniden adlandırma dizine alındıktan sonra kanonik skill meta verilerine dayanır. Mevcut istatistikler skill ile kalır.
...
Moderasyon incelemesi için bir skill bildirin. Bildirimler skill düzeyindedir, isteğe bağlı olarak
...
ayarlanırken atlanabilir. Aynı denetlenebilir iş akışında skill'i gizlemek için triyajlanmış
...
- Geçmiş bir sürümü sorgularken, `moderation` ve `security` alanlarını aynı sürüm bağlamı olarak ele almadan önce `moderation.matchesRequestedVersion` ve `moderation.sourceVersion` alanlarını kontrol edin.
...
- Beceri veya plugin için sahip/yayımcı yönetim erişimi ya da platform moderatörü/yönetici yetkisi gerektirir.
...
- `ok` yalnızca seçilen sürümün oluşturulmuş bir Beceri Kartı olduğunda, moderasyon tarafından kötü amaçlı yazılım nedeniyle engellenmediğinde ve ClawScan doğrulaması temiz olduğunda `true` olur.
- Beceri kimliği, yayımcı kimliği ve seçilen sürüm meta verileri üst düzey zarf alanlarıdır (`slug`, `displayName`, `publisherHandle`, `version`, `resolvedFrom`, `tag`, `createdAt`); böylece shell otomasyonu bunları iç içe sarmalayıcıları açmadan okuyabilir.
...
- `security.signals.dependencyRegistry`, v1 yanıt uyumluluğu için tutulur, ancak bağımlılık kayıt defteri varlık tarayıcısı kullanımdan kaldırılmıştır ve bu anahtar her zaman `null` olur.
...
Tam beceri sürümleri için güncel kompakt güvenlik kararlarını döndürür. Bu koleksiyon uç noktası, OpenClaw Control UI gibi görüntülemesi gereken yüklü ClawHub beceri sürümlerini zaten bilen istemciler için tasarlanmıştır.
...
- Sonuçlar öğe başınadır; eksik bir beceri veya sürüm tüm yanıtı başarısız kılmaz.
- Yanıt yalnızca güvenlikle ilgilidir. Beceri Kartı verilerini, oluşturulan kart durumunu, artifact dosya listelerini veya ayrıntılı tarayıcı yüklerini içermez.
...
- Tek becerilik Beceri Kartı doğrulama zarfına ihtiyaç duyduğunuzda `/verify`, oluşturulmuş kart markdown içeriğine ihtiyaç duyduğunuzda `/card` ve ayrıntılı tarayıcı verilerine ihtiyaç duyduğunuzda `/scan` kullanın.
...
- `category` (isteğe bağlı): Plugin kategorisi filtresi. Yalnızca istek
  ...
  `family=code-plugin`/`family=bundle-plugin` olan paket uç noktaları) sınırlandırıldığında desteklenir.
  Denetimli kategoriler ve eski v1 filtre diğer adları `GET /api/v1/plugins`
...
- `GET /api/v1/code-plugins` ve `GET /api/v1/bundle-plugins` sabit aileli diğer adlar olarak kalır.
...
- Kimliği doğrulanmış çağıranlar, liste/arama sonuçlarında ait oldukları yayımcıların özel paketlerini görebilir.
...
Eski diğer adlar, saklanan veya yazar tarafından beyan edilen kategori değerleri olarak kabul edilmez.
...
- Plugin araması için tarayıcı kullanıcı arayüzü sıralama denetimleri, yüklenen
  ilgililik sonuçlarını yeniden sıralar ve mevcut `/skills` gezinme davranışıyla
  eşleşir.
...
- `version.sha256hash`, eski istemciler için kullanımdan kaldırılmış uyumluluk
  ...
  ZIP baytlarının hash’ini alır. Modern istemciler, kanonik sürüm artifact’ını
...
çözümlenmiş bir sürümün kurulup kurulamayacağına karar vermek için herkese açık
...
- `trust.scanStatus`, tarayıcı girdilerinden ve elle sürüm moderasyonundan
  türetilen etkin güven durumudur.
- `trust.moderationState` null olabilir. Elle sürüm moderasyonu yoksa `null`
  olur.
...
- `trust.stale`, güven özetinin güncel olmayan girdilerden hesaplandığı ve
  yüksek güvenli bir izin kararı öncesinde yenileme gerektiriyor kabul edilmesi
  gerektiği anlamına gelir.

Notlar:

- Bu uç nokta sürüme tam bağlıdır. İstemciler bunu yalnızca en son paket meta
  verilerini okuduktan sonra değil, kurmayı düşündükleri paket sürümünü
...
Açık çözümleyici yolu üzerinden sürüm artifact’ını indirir.
...
- `manual`: elle moderasyon geçersiz kılması olan herhangi bir sürüm.
- `all`: elle geçersiz kılması, temiz olmayan tarama durumu veya paket raporu olan herhangi bir sürüm.
...
Paket raporlarını çözmek veya yeniden açmak için moderatör/yönetici endpoint’i.
...
moderasyonu uygulamak için onaylanmış bir raporla birlikte
`finalAction: "quarantine"` veya `finalAction: "revoke"` gönderin.
...
- İndirme kotası kovasını değil, okuma kotası kovasını kullanır.
...
- Plugin/paket arşivleri, eski OpenClaw istemcilerinin çalışmayı sürdürmesi için
...
- Yalnızca kayıt defteri meta verileri indirilen arşive eklenmez.
...
- Eski, yalnızca ZIP olan sürümler özellikle atlanır.
...
- Kapsamlı paket packument’leri hem `/api/npm/@scope/name` hem de npm’in kodlanmış
  `/api/npm/@scope%2Fname` istek yolunu destekler.

### `GET /api/npm/{package}/-/{tarball}.tgz`

npm ayna istemcileri için tam olarak yüklenmiş ClawPack tarball baytlarını akış olarak sunar.
...
- GitHub destekli skill devirleri baytları proxy’lemez veya aynalamaz. JSON
  ...
  ve `archiveUrl` içerir; tarama/güncel durum bir kapıdır ve başarı yükü meta
...
- İzin verilen form alanları `payload`, tekrarlanan `files` blob’ları veya bir
  ...
  akışı tarafından döndürülen bir depolama kimliği olabilir. Hazırlanmış
  storage-id yayınları, o yükleme URL’siyle döndürülen `clawpackUploadTicket`
  değerini de içermelidir.
- Aynı istekte ya `files` ya da `clawpack` kullanın, ikisini birden kullanmayın.
...
- Kod Plugin’leri `package.json`, kaynak repo meta verileri, kaynak commit meta
...
- Başkası adına yayınlar, resmi kanal uygunluğunu yine de hedef sahip hesabına göre doğrular.

### `DELETE /api/v1/skills/{slug}` / `POST /api/v1/skills/{slug}/undelete`

Bir skill’i geçici siler / geri yükler (sahip, moderatör veya yönetici).
...
Sahip tarafından başlatılan geçici silmeler slug’ı 30 gün boyunca rezerve eder; ardından slug başka bir
yayıncı tarafından alınabilir. Silme yanıtı, bu sona erme geçerliyse `slugReservedUntil` içerir.
...
eski paylaşılan kullanıcı/kişisel yayıncıyı gösteriyorsa endpoint önce onu bir kuruluş yayıncısına taşır.
...
çağıranı sahip olarak ekler. Bu endpoint mevcut kullanıcı/kişisel tanıtıcıları taşımaz ve
...
Yalnızca yönetici. Hak sahibi için sürüm yayınlamadan kök slug’ları ve paket adlarını rezerve eder.
Paket adları sürüm satırı olmayan özel yer tutucu paketler haline gelir; böylece aynı sahip daha sonra
...
Yalnızca yönetici. Convex Auth hesap satırlarını düzenlemeden, doğrulanmış yedek GitHub OAuth asıl hesabı
için kişisel yayıncıyı kurtarır. İstek iki değişmez GitHub sağlayıcı hesap kimliğini de belirtmelidir;
...
GitHub sorumlusu arasındaki sürekliliği bağımsız olarak doğrulamasından sonra `dryRun: false` ve
`confirmIdentityVerified: true` gerektirir. Hedef kullanıcının mevcut kişisel
yayıncısında skills, paketler veya GitHub skill kaynakları varsa kurtarma güvenli şekilde başarısız olur.
Kurtarma ayrıca, doğrudan sahip yollarının yeni yayıncı yetkisiyle uyumlu olması için kurtarılan yayıncının skills,
skill slug takma adları, paketleri, paket denetleyicisi uyarıları ve türetilmiş arama özeti satırları için eski `ownerUserId` alanlarını da taşır. Kurtarılan handle için etkin bir protected-handle
rezervasyonu da yedek kullanıcıya yeniden atanır; böylece daha sonraki
profil senkronizasyonu önceki kullanıcının rakip yetkisini geri yükleyemez. Her birincil tablo,
...
Bir kullanıcıyı yasakla ve sahip olduğu skills öğelerini kalıcı olarak sil (yalnızca moderator/admin).
...
Bir kullanıcının yasağını kaldır ve uygun skills öğelerini geri yükle (yalnızca admin).
...
Yasağı kaldırmadan veya içeriği geri yüklemeden mevcut bir yasağın saklanan nedenini değiştir
(yalnızca admin). `dryRun` `false` olmadığı sürece varsayılan olarak dry-run kullanılır.
...
- `limit` (isteğe bağlı): maksimum sonuç sayısı (varsayılan 20, en fazla 200)
...
Bir yıldız ekle/kaldır (öne çıkarır). Her iki uç nokta da idempotenttir.
...
CLI, site üzerinden registry/auth ayarlarını keşfedebilir:
