...
x-i18n:
    generated_at: "2026-07-04T20:39:03Z"
    model: gpt-5.5
...
- ClawHub, istemci IP'lerini edge'de tanımlamak için güvenilir iletme başlıkları kullanır.
- Güvenilir istemci IP'si yoksa anonim istekler yalnızca hız sınırı türüne
  göre kapsamlanan yedek kovalara gider. Bu yedek kovalar çağıranın sağladığı
  yolları, slug'ları, paket adlarını, sürümleri, sorgu dizelerini veya diğer
  artifact parametrelerini içermez.
...
- Popülerlik log ölçeklidir ve üst sınırı vardır. Yüksek etkileşimli Skills'ler, sorgu metni daha zayıf eşleştiğinde daha düşük sıralanabilir.
...
- Kullanıcıların kelimenin tam anlamıyla arayacağı terimleri görünen ada, özete ve etiketlere koyun. Bağımsız bir slug token'ını yalnızca korumak istediğiniz kararlı bir kimlik de olduğunda kullanın.
- Yeni slug daha iyi uzun vadeli kanonik ad değilse yalnızca tek bir sorguyu hedeflemek için slug'ı yeniden adlandırmayın. Eski slug'lar yönlendirme takma adları olur, ancak kanonik URL, gösterilen slug ve gelecekteki arama özetleri yeni slug'ı kullanır.
- Yeniden adlandırma takma adları, eski URL'ler ve kayıt üzerinden çözümlenen kurulumlar için çözümlemeyi korur, ancak arama sıralaması yeniden adlandırma indekslendikten sonra kanonik Skill metadata'sına dayanır. Mevcut istatistikler Skill ile kalır.
- Bir Skill beklenmedik şekilde görünmezse, sıralamayla ilgili metadata'yı değiştirmeden önce oturum açmışken `clawhub inspect @owner/slug` ile önce moderasyon durumunu kontrol edin.
...
- `createdAt` yeni Skill taramaları için kararlıdır; `updated` mevcut Skills'ler yeniden yayımlandığında değişir.
- `nonSuspiciousOnly=true` olduğunda, şüpheli Skills'ler sayfa alındıktan sonra filtrelendiği için imleç tabanlı sıralamalar bir sayfada `limit` değerinden daha az öğe döndürebilir.
...
Moderatör incelemesi için bir Skill'i bildirin. Bildirimler Skill düzeyindedir, isteğe bağlı olarak
...
ayarlanırken atlanabilir. Skill'i aynı denetlenebilir iş akışında gizlemek için triajı yapılmış
...
- `moderation`, en son sürümden türetilmiş güncel skill düzeyinde bir moderasyon anlık görüntüsüdür.
...
- Tarama isteği yükleri ve indirilebilir raporlar, saklama süresinden sonra tarama isteği deposundan silinir.
...
- Tarama işleri eşzamansızdır. Manuel tarama isteklerine normal yayınlama/geri doldurma işlerinden önce öncelik verilir, ancak tamamlanma yine de worker kullanılabilirliğine bağlıdır.
...
- Engellenmiş veya gizlenmiş sürümler dahil olmak üzere, tam olarak gönderilen sürümün saklanan tarama sonuçlarını döndürür.
...
- `provenance`, yalnızca ClawHub yayınlama veya içe aktarma sırasında bir GitHub repo/ref/commit/path çözüp sakladığında `server-resolved-github-import` olur; aksi takdirde `unavailable` olur.
...
- Sonuçlar öğe bazındadır; eksik bir skill veya sürüm tüm yanıtı başarısız kılmaz.
- Yanıt yalnızca güvenlik içerir. Skill Card verilerini, oluşturulan kart durumunu, artifact dosya listelerini veya ayrıntılı tarayıcı yüklerini içermez.
...
- `security.signals.dependencyRegistry`, v1 yanıt uyumluluğu için korunur, ancak bağımlılık kayıt varlığı tarayıcısı emekliye ayrılmıştır ve bu anahtar her zaman `null` olur.
- Skill Card bulunmaması, bu uç noktanın `ok`, `decision` veya `reasons` değerlerini etkilemez; istemciler kart içeriğine ihtiyaç duyduğunda yüklü `skill-card.md` dosyasını yerel olarak okumalıdır.
...
- `category` (isteğe bağlı): plugin kategorisi filtresi. Yalnızca istek plugin paketleriyle sınırlandırıldığında desteklenir (`/api/v1/plugins`,
  `/api/v1/code-plugins`, `/api/v1/bundle-plugins` veya `family=code-plugin`/`family=bundle-plugin` içeren paket uç noktaları). Denetimli kategoriler ve
...
- `channel` (isteğe bağlı): `official`, `community` veya `private`
- `isOfficial` (isteğe bağlı): `true` veya `false`
- `category` (isteğe bağlı): plugin kategorisi filtresi. Yalnızca istek plugin paketleriyle sınırlandırıldığında desteklenir. Denetimli kategoriler ve eski v1
...
Code-plugin ve bundle-plugin paketleri genelinde yalnızca Plugin kataloğunda gezinme.
...
- `clean` veya `suspicious` taramasına sahip mevcut GitHub destekli skills,
  repo, commit, path, içerik karması ve arşiv URL'siyle birlikte `sourceRef: "public-github"` içeren
  `_source_handoff.json` dosyasını içerir. ClawHub tarafından barındırılan kaynak dosyalarını içermezler.
...
Çevrimdışı analiz için herkese açık en son Plugin sürümlerinin toplu dışa aktarımı.
...
- Gövde: ZIP arşivi.
- Dışa aktarılan her Plugin kökü `{family}/{packageName}/` konumundadır.
...
- Plugin araması için tarayıcı kullanıcı arayüzü sıralama kontrolleri, yüklenen
  alaka sonuçlarını mevcut `/skills` göz atma davranışıyla eşleşecek şekilde yeniden sıralar.
...
- `packageName` npm adı olarak normalleştirilir; planlanan taşımalar için paket
  eksik olabilir.
- Bu yalnızca taşıma hazırlığını izler. OpenClaw üzerinde değişiklik yapmaz veya
  ClawPack üretmez.
...
- `manual`: elle yapılan moderasyon geçersiz kılması olan herhangi bir sürüm.
...
kendiliğinden indirmeleri otomatik olarak gizlemez veya engellemezler; moderatörler
...
onaylanmış bir raporla `finalAction: "quarantine"` veya `finalAction: "revoke"`
gönderin.
...
- Özel paketler, çağıran sahip olan yayıncıyı okuyamıyorsa `404` döndürür.
...
taraması olan ve barındırılan sürümü bulunmayan güncel GitHub destekli bir beceri için
GitHub kaynak devrini döndürür.
...
- `version` ve `tag` değerlerinden hiçbiri sağlanmazsa en son sürüm kullanılır.
...
- İsteğe bağlı yük alanı: `migrateOwner`. `ownerHandle` ile `true` olduğunda,
  aktör hem mevcut hem de hedef yayıncılarda yönetici/sahipse mevcut bir beceri
...
- İzin verilen form alanları `payload`, yinelenen `files` blob'ları veya tek bir
  `clawpack` tarball referansıdır. `clawpack`, upload-url akışı tarafından döndürülen
  bir `.tgz` blob'u veya storage id olabilir. Aşamalandırılmış storage-id yayımları
  o yükleme URL'siyle döndürülen `clawpackUploadTicket` değerini de içermelidir.
...
- Doğrudan multipart yayımlama istekleri 18MB ile sınırlandırılır. ClawPack tarball'ları
...
- Tanıtıcı bir yayıncı, kullanıcı veya kişisel yayıncı tarafından zaten kullanılıyorsa `409` döndürür.
...
için kişisel yayıncıyı kurtarır. İstek her iki değişmez GitHub sağlayıcı hesap kimliğini
adlandırmalıdır; değişebilir tanıtıcılar yalnızca operatöre yönelik koruma olarak kullanılır.

Uç nokta varsayılan olarak deneme çalıştırması kullanır. Kurtarmayı uygulamak için personel her iki
GitHub yetkilisi arasındaki sürekliliği bağımsız olarak doğruladıktan sonra `dryRun: false` ve
`confirmIdentityVerified: true` gerekir. Hedef kullanıcının geçerli kişisel
yayıncısında beceriler, paketler veya GitHub beceri kaynakları varsa kurtarma kapalı şekilde başarısız olur.
Kurtarma ayrıca, doğrudan sahip yollarının yeni yayıncı yetkisiyle uyumlu olması için kurtarılan yayıncının becerileri,
beceri slug takma adları, paketleri, paket denetleyicisi uyarıları ve türetilmiş arama özeti satırları için eski
`ownerUserId` alanlarını da taşır. Kurtarılan kullanıcı adı için etkin bir korumalı kullanıcı adı
rezervasyonu da yedek kullanıcıya yeniden atanır; böylece sonraki profil senkronizasyonu eski kullanıcının rakip yetkisini geri yükleyemez. Her birincil tablo,
...
içeriği geri yüklemeden değiştirin (yalnızca yönetici). `dryRun` `false` olmadığı sürece varsayılan olarak deneme çalıştırmasıdır.
...
Bir yıldız ekleyin/kaldırın (vurgular). Her iki uç nokta da idempotenttir.
...
yayınları, sonuçta oluşan depolama kimliğini `clawpack` olarak
