...
Üçüncü taraf dizinler, ClawHub Skills öğelerini listelemek veya aramak için genel okuma uç noktalarını kullanabilir. Lütfen sonuçları önbelleğe alın, `429`/`Retry-After` değerlerine uyun, kullanıcıları kanonik ClawHub listesine (`https://clawhub.ai/<owner>/skills/<slug>`) geri bağlayın ve ClawHub'ın üçüncü taraf siteyi desteklediği izlenimini vermekten kaçının. Gizli, özel veya moderasyon tarafından engellenmiş içeriği genel API yüzeyinin dışında aynalamaya çalışmayın.
...
- Kimliği doğrulanmış yazma uç noktaları, sunucu nedeni bildiğinde yalın bir `Unauthorized`
  ...
  istemcilerinin kullanıcılara onları neyin engellediğini söyleyebilmesi için eyleme dönüştürülebilir metin almalıdır.
...
- ClawHub, kenarda istemci IP'lerini belirlemek için güvenilir iletme başlıklarını kullanır.
- Güvenilir istemci IP'si yoksa anonim istekler yalnızca hız sınırı türüne
  göre kapsamlandırılmış yedek kova kullanır. Bu yedek kovalar
  çağıranın sağladığı yolları, slug'ları, paket adlarını, sürümleri, sorgu dizelerini veya diğer
  yapı parametrelerini içermez.
...
- ASCII metin, sözcük ve noktalama sınırlarında token'lara ayrılır. Örneğin, `personal-map` bağımsız bir `map` token'ı içerirken `amap-jsapi-skill` `amap`, `jsapi` ve `skill` içerir; bu nedenle `map` aramak, `personal-map` için `amap-jsapi-skill` öğesine göre daha güçlü bir sözcüksel eşleşme verir.
- Popülerlik logaritmik ölçeklenir ve üst sınır uygulanır. Yüksek etkileşimli Skills öğeleri, sorgu metni daha zayıf bir eşleşmeyse daha düşük sıralanabilir.
...
- Kullanıcıların gerçekten arayacağı terimleri görünen ad, özet ve etiketlere koyun. Bağımsız bir slug token'ını yalnızca korumak istediğiniz kararlı bir kimlik de olduğunda kullanın.
- Yeni slug daha iyi bir uzun vadeli kanonik ad olmadıkça, yalnızca tek bir sorguyu hedeflemek için slug adını değiştirmeyin. Eski slug'lar yönlendirme takma adları olur, ancak kanonik URL, görüntülenen slug ve gelecekteki arama özetleri yeni slug'ı kullanır.
...
- Bir Skill beklenmedik biçimde görünmezse, sıralamayla ilgili meta verileri değiştirmeden önce oturum açmışken önce `clawhub inspect @owner/slug` ile moderasyon durumunu kontrol edin.
...
- `cursor` (isteğe bağlı): `trending` dışındaki herhangi bir sıralama için sayfalandırma imleci
...
- Varsa sayfalandırmaya devam etmek için `nextCursor` kullanın. Kısa bir sayfa tek başına sonuçların bittiği anlamına gelmez.
...
- Kanıt genel çağıranlar için düzeltilir ve ham parçaları yalnızca sahipler/moderatörler için içerir.
...
ayarlanırken atlanabilir. Aynı denetlenebilir iş akışında Skill'i gizlemek için incelenmiş
...
- `security.signals.dependencyRegistry`, v1 yanıt uyumluluğu için korunur, ancak bağımlılık kayıt defteri varlık tarayıcısı kullanımdan kaldırılmıştır ve bu anahtar her zaman `null` olur.
- Skill Card olmaması, bu uç noktanın `ok`, `decision` veya `reasons` değerlerini etkilemez; istemciler kart içeriğine ihtiyaç duyduğunda yüklü `skill-card.md` dosyasını yerel olarak okumalıdır.
...
- `category` (isteğe bağlı): Plugin kategori filtresi. Yalnızca istek Plugin paketleriyle
...
- `clean` veya `suspicious` taramasına sahip mevcut GitHub destekli Skills,
  ...
  içerik hash’i ve arşiv URL’si içerir. ClawHub’da barındırılan kaynak dosyalarını içermezler.
...
- `GET /api/v1/plugins` altında belgelenen eski v1 filtre takma adları da
  kabul edilir.
- Kategori filtreleme, arama sorgusunun yeniden yazımı değil, Plugin kategori özeti
...
- `version.sha256hash`, eski istemciler için kullanımdan kaldırılmış uyumluluk meta verisidir. Bu,
  `/api/v1/packages/{name}/download` tarafından döndürülen tam ZIP baytlarını karma değerler.
...
- Bu uç nokta sürüme tam olarak bağlıdır. İstemciler, yalnızca en son paket
...
- Bu uç nokta, sahip/moderatör moderasyon uç noktalarından kasıtlı olarak daha dardır.
  ...
  rapor gövdelerini, özel kanıtları veya dahili inceleme zaman çizelgelerini sunmaz.
...
- `manual`: manuel moderasyon geçersiz kılması bulunan herhangi bir sürüm.
- `all`: manuel geçersiz kılması, temiz olmayan tarama durumu veya paket raporu bulunan herhangi bir sürüm.
...
otomatik olarak gizlemez veya engellemez; moderatörler yapıtları onaylamak, karantinaya almak
...
- `revoked`: daha önce güvenilen bir sürümden sonra engellenmiş.
...
- Dosya boyutu sınırı: 200KB.
- Bekleyen VirusTotal taramaları okumaları engellemez; kötü amaçlı sürümler başka yerlerde yine alıkonabilir.
- Özel paketler, çağıran taraf sahibi olan yayıncıyı okuyamıyorsa `404` döndürür.
...
- `dist.tarball`, `dist.integrity` ve `dist.shasum`, kullanıcılar isterse
  npm'i aynaya yöneltebilsin diye npm uyumlu alanlar kullanır.
...
npm ayna istemcileri için tam olarak yüklenmiş ClawPack tarball baytlarını akış olarak iletir.
...
taramasına sahip ve barındırılan sürümü olmayan güncel GitHub destekli bir
Skills için GitHub kaynak devri döndürür.
...
- İndirme istatistikleri UTC günü başına benzersiz kimlikler olarak sayılır
  (API belirteci geçerliyse `userId`, aksi halde IP).
...
- İsteğe bağlı yük alanı: `migrateOwner`. `ownerHandle` ile `true` olduğunda,
  aktör hem mevcut hem de hedef yayıncılarda yönetici/sahip ise mevcut bir
  Skills bu sahibe taşınabilir. Bu açık onay olmadan sahip değişiklikleri
  reddedilir.
...
- İzin verilen form alanları `payload`, yinelenen `files` blob'ları veya bir
  ...
  depolama kimliği yayınları, bu yükleme URL'siyle döndürülen
  `clawpackUploadTicket` öğesini de içermelidir.
- Aynı istekte `files` veya `clawpack` kullanın, ikisini birden asla kullanmayın.
- JSON gövdeleri ve çağıran tarafından sağlanan `payload.files` /
  `payload.artifact` meta verileri reddedilir.
...
- İsteğe bağlı yük alanı: `ownerHandle`. Mevcut olduğunda yalnızca yöneticiler
  bu sahip adına yayın yapabilir.
...
- Başkası adına yayınlar, resmi kanal uygunluğunu hedef sahip hesabına göre yine doğrular.
...
bir yayıncı tarafından talep edilebilir. Silme yanıtı, bu sürenin geçerli olduğu durumlarda
`slugReservedUntil` içerir. Moderatör/yönetici gizlemeleri ve güvenlik kaldırmaları bu şekilde
sona ermez.
...
eski paylaşımlı kullanıcı/kişisel yayıncıyı gösteriyorsa uç nokta önce onu bir kuruluş
...
Kimliği doğrulanmış kendi kendine hizmet kuruluş yayıncısı oluşturma. Yeni bir kuruluş
yayıncısı oluşturur ve çağıran tarafı sahip olarak ekler. Bu uç nokta mevcut kullanıcı/kişisel
tanıtıcıları taşımaz ve yayıncıyı güvenilir/resmi olarak işaretlemez.
...
Paket adları, sürüm satırları olmayan özel yer tutucu paketler hâline gelir; böylece aynı sahip
daha sonra gerçek code-plugin veya bundle-plugin sürümünü bu ada yayınlayabilir.
...
OAuth sorumlusu için kişisel yayıncıyı kurtarır. İstek, iki değişmez GitHub sağlayıcı hesap
kimliğini de belirtmelidir; değişebilir tanıtıcılar yalnızca operatöre yönelik koruma olarak kullanılır.

Uç nokta varsayılan olarak kuru çalıştırma modundadır. Kurtarmayı uygulamak için, personel her iki
...
skill slug takma adları, paketleri, paket denetleyici uyarıları ve türetilmiş arama özeti satırları için eski `ownerUserId` alanlarını taşır. Kurtarılan kullanıcı adı için etkin bir korumalı kullanıcı adı
...
### Sahip slug yönetim uç noktaları
...
içeriği geri yüklemeden değiştirin (yalnızca admin). `dryRun` `false` olmadığı sürece varsayılan olarak kuru çalıştırma modundadır.
...
`POST /api/cli/upload-url`, `uploadUrl` ve `uploadTicket` döndürür. Bir ClawPack tarball hazırlayan paket
