...
- `cf-connecting-ip` dahil güvenilir istemci IP başlıklarını yalnızca dağıtım güvenilir iletilen başlıkları açıkça etkinleştirdiğinde kullanır.
- ClawHub, istemci IP'lerini uçta tanımlamak için güvenilir iletme başlıklarını kullanır.
- Güvenilir istemci IP'si yoksa anonim istekler yalnızca hız sınırı türüne göre kapsamlanan geri dönüş kovalarını kullanır. Bu geri dönüş kovaları, çağıranın sağladığı yolları, slug'ları, paket adlarını, sürümleri, sorgu dizelerini veya diğer yapıt parametrelerini içermez.
...
- Sonuçlar alaka sırasına göre döndürülür (gömme benzerliği + tam slug/ad token destekleri + küçük bir popülerlik önceliği).
- Alaka, popülerlikten daha güçlüdür. Kesin bir slug veya görüntü adı token eşleşmesi, çok daha güçlü etkileşime sahip daha gevşek bir eşleşmenin önüne geçebilir.
- ASCII metin, sözcük ve noktalama sınırlarında token'lara ayrılır. Örneğin `personal-map` bağımsız bir `map` token'ı içerirken `amap-jsapi-skill` `amap`, `jsapi` ve `skill` içerir; bu nedenle `map` aramak `personal-map` için `amap-jsapi-skill` değerinden daha güçlü bir sözlüksel eşleşme verir.
- Popülerlik logaritmik olarak ölçeklenir ve sınırlandırılır. Yüksek etkileşimli beceriler, sorgu metni daha zayıf eşleştiğinde daha alt sıralarda yer alabilir.
...
Yayıncı keşfedilebilirliği rehberi:

- Kullanıcıların birebir arayacağı terimleri görüntü adı, özet ve etiketlere koyun. Bağımsız bir slug token'ını yalnızca aynı zamanda korumak istediğiniz kararlı bir kimlik olduğunda kullanın.
- Yeni slug daha iyi uzun vadeli kanonik ad değilse yalnızca tek bir sorguyu yakalamak için slug'ı yeniden adlandırmayın. Eski slug'lar yönlendirme takma adları olur, ancak kanonik URL, gösterilen slug ve gelecekteki arama özetleri yeni slug'ı kullanır.
- Yeniden adlandırma takma adları, eski URL'ler ve kayıt defteri üzerinden çözümlenen kurulumlar için çözümlemeyi korur, ancak arama sıralaması yeniden adlandırma dizine eklendikten sonra kanonik beceri meta verilerine dayanır. Mevcut istatistikler beceriyle kalır.
...
- `nonSuspiciousOnly=true` olduğunda, imleç tabanlı sıralamalar bir sayfada `limit` değerinden daha az öğe döndürebilir çünkü şüpheli beceriler sayfa alımından sonra filtrelenir.
...
- `metadata.os`: beceri frontmatter'ında bildirilen işletim sistemi kısıtlamaları (ör. `["macos"]`, `["linux"]`). Bildirilmemişse `null`.
...
- Genel çağıranlar yalnızca önceden işaretlenmiş görünür beceriler için `200` alır.
- Kanıt genel çağıranlar için redakte edilir ve ham parçaları yalnızca sahipler/moderatörler için içerir.

### `POST /api/v1/skills/{slug}/report`

Moderatör incelemesi için bir beceriyi bildirin. Bildirimler beceri düzeyindedir, isteğe bağlı olarak bir sürüme bağlanır ve beceri bildirim kuyruğunu besler.
...
`confirmed` ve `dismissed` için `note` gereklidir; `status` tekrar `open` olarak ayarlanırken atlanabilir. Aynı denetlenebilir iş akışında beceriyi gizlemek için triyaj edilmiş bir bildirimle birlikte `finalAction: "hide"` geçirin.
...
- Geçmiş bir sürüm sorgulanırken, `moderation` ve `security` öğelerini aynı sürüm bağlamı olarak ele almadan önce `moderation.matchesRequestedVersion` ve `moderation.sourceVersion` değerlerini kontrol edin.
...
- Yanıt `202` durumuyla `{ "ok": true, "scanId": "...", "jobId": "...", "status": "queued", "sourceKind": "published", "update": false, "queue": { "queuedAhead": 0, "queuedAheadIsEstimate": false, "position": 1, "running": 0, "runningIsEstimate": false, "note": "Scans are asynchronous and may take time to complete." } }` olur.
- Tarama işleri eşzamansızdır. Manuel tarama istekleri normal yayımlama/geri doldurma çalışmalarının önüne önceliklendirilir, ancak tamamlanma yine de worker kullanılabilirliğine bağlıdır.
...
- Skills veya Plugin için sahip/yayıncı yönetim erişimi ya da platform moderatörü/yöneticisi yetkisi gerektirir.
- Engellenmiş veya gizli sürümler dahil olmak üzere tam gönderilmiş sürüm için saklanan tarama sonuçlarını döndürür.
- `kind` varsayılan olarak `skill` değerine ayarlanır; plugin/paket taramaları için `kind=plugin` kullanın.
...
- `ok` yalnızca seçilen sürüm oluşturulmuş bir Skill Card içerdiğinde, moderasyon tarafından kötü amaçlı yazılım nedeniyle engellenmemiş olduğunda ve ClawScan doğrulaması temiz olduğunda `true` olur.
- Skill kimliği, yayıncı kimliği ve seçilen sürüm meta verileri üst düzey zarf alanlarıdır (`slug`, `displayName`, `publisherHandle`, `version`, `resolvedFrom`, `tag`, `createdAt`); böylece shell otomasyonu bunları iç içe sarmalayıcıları açmadan okuyabilir.
...
Tam Skills sürümleri için güncel kompakt güvenlik kararlarını döndürür. Bu koleksiyon uç noktası, OpenClaw Control UI gibi hangi kurulu ClawHub Skills sürümlerini göstermeleri gerektiğini zaten bilen istemciler için tasarlanmıştır.
...
- Sonuçlar öğe bazındadır; eksik bir Skills veya sürüm tüm yanıtı başarısız kılmaz.
- Yanıt yalnızca güvenlikle ilgilidir. Skill Card verilerini, oluşturulmuş kart durumunu, yapıt dosya listelerini veya ayrıntılı tarayıcı yüklerini içermez.
...
- `security.signals.dependencyRegistry`, v1 yanıt uyumluluğu için korunur, ancak bağımlılık kayıt defteri varlık tarayıcısı kullanımdan kaldırılmıştır ve bu anahtar her zaman `null` olur.
- Skill Card yokluğu bu uç noktanın `ok`, `decision` veya `reasons` değerlerini etkilemez; istemciler kart içeriğine ihtiyaç duyduklarında kurulu `skill-card.md` dosyasını yerel olarak okumalıdır.
- Tek Skills için Skill Card doğrulama zarfına ihtiyaç duyduğunuzda `/verify`, oluşturulmuş kart Markdown'ına ihtiyaç duyduğunuzda `/card` ve ayrıntılı tarayıcı verilerine ihtiyaç duyduğunuzda `/scan` kullanın.
...
- `category` (isteğe bağlı): Plugin kategorisi filtresi. Yalnızca istek
  Plugin paketleriyle sınırlandırıldığında (`/api/v1/plugins`,
  `/api/v1/code-plugins`, `/api/v1/bundle-plugins` veya
  `family=code-plugin`/`family=bundle-plugin` içeren paket uç noktaları)
  desteklenir. Denetimli kategoriler ve eski v1 filtre takma adları
  `GET /api/v1/plugins` altında belgelenmiştir.

Notlar:

- `family`, `channel`, `isOfficial`, `featured`,
  `highlightedOnly` veya `sort` için geçersiz değerler `400` döndürür. Bilinmeyen sorgu parametreleri yoksayılır.
...
- `family`, `channel`, `isOfficial`, `featured` veya
  `highlightedOnly` için geçersiz değerler `400` döndürür. Bilinmeyen sorgu parametreleri yoksayılır.
- Anonim çağıranlar yalnızca herkese açık paket kanallarını görür.
...
- Gövde: ZIP arşivi.
- Dışa aktarılan her Skill `{publisher}/{slug}/` kökünde bulunur.
...
- `clean` veya `suspicious` taramasına sahip geçerli GitHub destekli Skills,
  `sourceRef: "public-github"`, repo, commit, path, içerik karması ve arşiv URL'si
  ile `_source_handoff.json` içerir. ClawHub tarafından barındırılan kaynak dosyaları içermezler.
...
- `_manifest.json` her zaman ZIP kökünde bulunur.
- Tek tek Pluginler veya dosyalar dışa aktarılamadığında `_errors.json` dahil
  edilir.
...
- `GET /api/v1/plugins` altında belgelenen eski v1 filtre takma adları da
  kabul edilir.
- Kategori filtreleme, arama sorgusunu yeniden yazma değil, Plugin kategori özet
  satırlarıyla desteklenen gerçek bir API filtresidir.
- Sonuçlar alaka sırasıyla döndürülür ve şu anda sayfalanmaz.
- Plugin araması için tarayıcı arayüzündeki sıralama kontrolleri, yüklenen alaka
...
- `version.sha256hash`, eski istemciler için kullanımdan kaldırılmış uyumluluk
  ...
  ZIP baytlarının karmasını alır. Modern istemciler, kanonik sürüm yapıtını
...
- `trust.reasons`, kullanıcıya gösterilen ve denetim açıklama listesidir. Gerekçe
...
- `trust.stale`, güven özetinin güncelliğini yitirmiş girdilerden hesaplandığı ve
  yüksek güvenli bir izin kararı verilmeden önce yenileme gerektiriyor olarak
...
- Özel paketler, çağıran taraf sahip yayıncıyı okuyamadığı sürece `404` döndürür.
- Bu uç nokta, sahip/moderatör moderasyon uç noktalarından özellikle daha dardır.
  Muhabir kimliklerini, rapor gövdelerini, özel kanıtları veya iç inceleme zaman
  çizelgelerini değil, kurulum kararını ve herkese açık açıklamayı gösterir.
...
- `open`: şüpheli, kötü amaçlı, bekleyen, karantinaya alınmış, iptal edilmiş veya rapor edilmiş sürümler.
...
Bir paketi moderatör incelemesi için raporlayın. Raporlar paket düzeyindedir ve
...
Karantinaya alınmış ve geri çekilmiş sürümler, yapıt indirme rotalarından `403`
...
- Özel paketler, çağıran taraf sahip yayımcıyı okuyamadığı sürece `404` döndürür.

### `GET /api/v1/packages/{name}/download`

Bir paket sürümü için eski belirlenimci ZIP arşivini indirir.
...
- Yalnızca kayıt defteri metadatası indirilen arşive enjekte edilmez.
- Bekleyen VirusTotal taramaları indirmeleri engellemez; kötü amaçlı sürümler `403` döndürür.
- Özel paketler, çağıran taraf sahip olmadığı sürece `404` döndürür.
...
- Kapsamlı paket packument'ları hem `/api/npm/@scope/name` hem de npm'in kodlanmış
  `/api/npm/@scope%2Fname` istek yolunu destekler.

### `GET /api/npm/{package}/-/{tarball}.tgz`

npm ayna istemcileri için yüklenen ClawPack tarball baytlarını aynen akış olarak iletir.

Notlar:

- İndirme hız kovasını kullanır.
- İndirme üst bilgileri, ClawHub SHA-256 ile npm integrity/shasum metadatasını içerir.
...
- GitHub destekli skill devirleri baytları proxy'lemez veya aynalamaz. JSON yanıtı
  ...
  `archiveUrl` içerir; tarama/güncel durum bir kapıdır ve başarı yükü metadatası
  olarak dahil edilmez.
- İndirme istatistikleri UTC gün başına benzersiz kimlikler olarak sayılır (API belirteci geçerliyse `userId`, aksi halde IP).
...
Belirteci doğrular ve kullanıcı handle'ını döndürür.
...
- İsteğe bağlı yük alanı: `ownerHandle`. Mevcut olduğunda API bu yayımcıyı
...
- İsteğe bağlı yük alanı: `migrateOwner`. `ownerHandle` ile `true` olduğunda,
  ...
  o sahibe taşınabilir. Bu açık seçim olmadan sahip değişiklikleri reddedilir.
...
- İzin verilen form alanları `payload`, tekrarlanan `files` blob'ları veya bir
  ...
  yayınları, o yükleme URL'siyle döndürülen `clawpackUploadTicket` değerini de
  içermelidir.
...
- JSON gövdeleri ve çağıran tarafından sağlanan `payload.files` / `payload.artifact`
  metadatası reddedilir.
- Doğrudan multipart yayımlama istekleri 18 MB ile sınırlıdır. ClawPack tarball'ları
  120 MB tarball sınırına kadar upload-url akışını kullanabilir.
...
- Code Plugin'ler `package.json`, kaynak depo metadatası, kaynak commit metadatası,
  config şeması metadatası, `openclaw.compat.pluginApi` ve
  `openclaw.build.openclawVersion` gerektirir.
- `openclaw.hostTargets` ve `openclaw.environment` isteğe bağlı metadatadır.
- Yalnızca `openclaw` org yayımcısı ve mevcut `openclaw` org üyelerinin kişisel
  yayımcıları `official` kanalına yayımlayabilir.
...
yayımcı tarafından alınabilir. Silme yanıtı, bu sona erme geçerliyse `slugReservedUntil` içerir.
...
Yalnızca yönetici. Bir handle için org yayımcısının var olmasını sağlar. Handle hâlâ
eski paylaşılan kullanıcı/kişisel yayımcıyı işaret ediyorsa, uç nokta önce onu bir
org yayımcısına geçirir. Yeni oluşturulan bir org için `memberHandle` sağlayın; işlem yapan
...
Kimliği doğrulanmış self servis org yayımcısı oluşturma. Yeni bir org yayımcısı oluşturur ve
...
Yalnızca yönetici. Bir sürüm yayımlamadan, hak sahibi için kök slug'ları ve paket adlarını
ayırır. Paket adları, sürüm satırları olmayan özel yer tutucu paketlere dönüşür; böylece aynı
sahip daha sonra gerçek code-plugin veya bundle-plugin sürümünü o ada yayımlayabilir.
...
için kişisel yayımcıyı kurtarır. İstek, değişmez GitHub sağlayıcı hesap kimliklerinin ikisini de
adlandırmalıdır; değişebilir handle'lar yalnızca operatöre dönük bir koruma olarak kullanılır.
...
yayıncısında beceriler, paketler veya GitHub beceri kaynakları varsa kurtarma güvenli şekilde başarısız olur.
Kurtarma ayrıca kurtarılan yayıncının becerileri, beceri slug takma adları, paketleri,
paket denetleyici uyarıları ve türetilmiş arama özeti satırları için eski `ownerUserId`
alanlarını da taşır; böylece doğrudan sahip yolları yeni yayıncı yetkisiyle uyumlu olur.
Kurtarılan handle için etkin bir korumalı-handle rezervasyonu da yedek kullanıcıya yeniden atanır;
böylece sonraki profil eşitlemesi eski kullanıcının rakip yetkisini geri yükleyemez. Her birincil tablo,
...
değiştirin (yalnızca yönetici). `dryRun`, `false` olmadığı sürece varsayılan olarak dry-run modundadır.
...
Bir yıldız ekleyin/kaldırın (öne çıkanlar). Her iki uç nokta da idempotenttir.
