...
Üçüncü taraf dizinler, ClawHub Skills'lerini listelemek veya aramak için herkese açık okuma uç noktalarını kullanabilir. Lütfen sonuçları önbelleğe alın, `429`/`Retry-After` değerlerine uyun, kullanıcıları kanonik ClawHub listesine (`https://clawhub.ai/<owner>/skills/<slug>`) geri bağlayın ve ClawHub'ın üçüncü taraf siteyi onayladığı izlenimini vermekten kaçının. Gizli, özel veya moderasyon tarafından engellenmiş içeriği herkese açık API yüzeyi dışında yansıtmaya çalışmayın.
...
- Kimliği doğrulanmış yazma uç noktaları, sunucu nedeni bildiğinde yalın bir `Unauthorized`
  ...
  istemcilerinin kullanıcılara neyin onları engellediğini söyleyebilmesi için işlem yapılabilir metin almalıdır.
...
- ClawHub, istemci IP'lerini edge'de tanımlamak için güvenilen iletme başlıklarını kullanır.
- Güvenilen bir istemci IP'si yoksa anonim istekler yalnızca hız sınırı türüne
  göre kapsamlanan yedek kovalardan yararlanır. Bu yedek kovalar
  çağıran tarafından sağlanan yolları, slug'ları, paket adlarını, sürümleri, sorgu dizelerini veya diğer
...
- `highlightedOnly` (isteğe bağlı): öne çıkarılmış Skills'lerle filtrelemek için `true`
...
- Sonuçlar alaka sırasına göre döndürülür (embedding benzerliği + tam slug/ad token destekleri + küçük bir popülerlik önceliği).
...
- ASCII metin sözcük ve noktalama sınırlarında token'lara ayrılır. Örneğin `personal-map` bağımsız bir `map` token'ı içerirken, `amap-jsapi-skill` `amap`, `jsapi` ve `skill` içerir; bu nedenle `map` aramak `personal-map` için `amap-jsapi-skill` değerinden daha güçlü bir sözcüksel eşleşme sağlar.
- Popülerlik logaritmik ölçeklenir ve üst sınır uygulanır. Yüksek etkileşimli Skills'ler, sorgu metni daha zayıf eşleştiğinde daha düşük sıralanabilir.
- Şüpheli veya gizli moderasyon durumu, çağıran filtrelerine ve geçerli moderasyon durumuna bağlı olarak bir Skills'i herkese açık aramadan kaldırabilir.

Yayıncı keşfedilebilirlik rehberi:

- Kullanıcıların kelimesi kelimesine arayacağı terimleri görünen ad, özet ve etiketlere koyun. Bağımsız bir slug token'ını yalnızca aynı zamanda korumak istediğiniz kararlı bir kimlikse kullanın.
- Yeni slug daha iyi uzun vadeli kanonik ad değilse yalnızca tek bir sorguyu yakalamak için slug'ı yeniden adlandırmayın. Eski slug'lar yönlendirme takma adlarına dönüşür, ancak kanonik URL, gösterilen slug ve gelecekteki arama özetleri yeni slug'ı kullanır.
- Yeniden adlandırma takma adları, eski URL'ler ve kayıt üzerinden çözümlenen kurulumlar için çözümlemeyi korur, ancak arama sıralaması yeniden adlandırma dizine eklendikten sonra kanonik Skills meta verilerine dayanır. Mevcut istatistikler Skills ile kalır.
- Bir Skills beklenmedik biçimde görünmüyorsa, sıralamayla ilgili meta verileri değiştirmeden önce oturum açmışken `clawhub inspect @owner/slug` ile önce moderasyon durumunu kontrol edin.
...
- `createdAt` yeni Skills taramaları için kararlıdır; `updated` mevcut Skills'ler yeniden yayımlandığında değişir.
- `nonSuspiciousOnly=true` olduğunda, imleç tabanlı sıralamalar şüpheli Skills'ler sayfa alımından sonra filtrelendiği için bir sayfada `limit` öğeden daha az döndürebilir.
...
- Sahip yeniden adlandırma/birleştirme akışları tarafından oluşturulan eski slug'lar kanonik Skills'e çözümlenir.
- `metadata.os`: Skills frontmatter içinde bildirilen işletim sistemi kısıtlamaları (örn. `["macos"]`, `["linux"]`). Bildirilmediyse `null`.
- `metadata.systems`: Nix sistem hedefleri (örn. `["aarch64-darwin", "x86_64-linux"]`). Bildirilmediyse `null`.
...
- Herkese açık çağıranlar yalnızca halihazırda işaretlenmiş görünür Skills'ler için `200` alır.
- Kanıt herkese açık çağıranlar için redakte edilir ve ham parçaları yalnızca sahipler/moderatörler için içerir.

### `POST /api/v1/skills/{slug}/report`

Moderatör incelemesi için bir Skills'i bildirin. Bildirimler Skills düzeyindedir, isteğe bağlı olarak
...
ayarlanırken atlanabilir. Aynı denetlenebilir iş akışında Skills'i gizlemek için triyajlanmış bir
...
- `moderation`, en son sürümden türetilmiş güncel bir skill düzeyinde moderasyon anlık görüntüsüdür.
- Geçmiş bir sürümü sorgularken, `moderation` ve `security` alanlarını aynı sürüm bağlamı olarak değerlendirmeden önce `moderation.matchesRequestedVersion` ve `moderation.sourceVersion` alanlarını kontrol edin.
...
- Skill veya Plugin üzerinde sahip/yayımcı yönetim erişimi ya da platform moderatörü/yöneticisi yetkisi gerektirir.
- Engellenmiş veya gizlenmiş sürümler dahil, tam olarak gönderilmiş sürüm için saklanan tarama sonuçlarını döndürür.
...
- `ok`, yalnızca seçilen sürümün oluşturulmuş bir Skill Card'ı olduğunda, moderasyon tarafından zararlı yazılım nedeniyle engellenmediğinde ve ClawScan doğrulaması temiz olduğunda `true` olur.
...
koleksiyon uç noktası, OpenClaw Control UI gibi görüntülemeleri gereken yüklü
ClawHub skill sürümlerini zaten bilen istemciler için tasarlanmıştır.
...
- Yanıt yalnızca güvenlik içindir. Skill Card verilerini, oluşturulan kart durumunu, yapı dosyası listelerini veya ayrıntılı tarayıcı yüklerini içermez.
...
- `security.signals.dependencyRegistry`, v1 yanıt uyumluluğu için korunur, ancak bağımlılık kayıt defteri varlık tarayıcısı kullanımdan kaldırılmıştır ve bu anahtar her zaman `null` olur.
- Skill Card yokluğu bu uç noktanın `ok`, `decision` veya `reasons` alanlarını etkilemez; istemciler kart içeriğine ihtiyaç duyduklarında yüklü `skill-card.md` dosyasını yerel olarak okumalıdır.
- Tek skill için Skill Card doğrulama zarfına ihtiyaç duyduğunuzda `/verify`, oluşturulan kart markdown'ına ihtiyaç duyduğunuzda `/card` ve ayrıntılı tarayıcı verilerine ihtiyaç duyduğunuzda `/scan` kullanın.
...
- `startDate` (gerekli): skill `updatedAt` için Unix milisaniyesi alt sınırı.
- `endDate` (gerekli): skill `updatedAt` için Unix milisaniyesi üst sınırı.
...
- `clean` veya `suspicious` taramasına sahip geçerli GitHub destekli skills,
  ...
  içerik karması ve arşiv URL'si içerir. ClawHub tarafından barındırılan kaynak dosyaları içermezler.
...
- Plugin araması için tarayıcı kullanıcı arayüzü sıralama denetimleri, geçerli
  `/skills` göz atma davranışıyla eşleşerek yüklenen alaka sonuçlarını yeniden sıralar.
...
- `version.sha256hash`, eski istemciler için kullanım dışı uyumluluk meta
  verisidir. `/api/v1/packages/{name}/download` tarafından döndürülen tam ZIP
  baytlarını hashler. Modern istemciler, kanonik sürüm artefaktını tanımlayan
...
- `release.artifactKind`, `release.artifactSha256`, `release.npmIntegrity`,
  `release.npmShasum` ve `release.npmTarballName`, sürüm artefaktı için
...
- `trust.blockedFromDownload`, kurulum engelleme sinyalidir. OpenClaw ve diğer
  ...
  engellemelidir.
- `trust.reasons`, kullanıcıya dönük ve denetim amaçlı açıklama listesidir.
  Neden kodları `manual:quarantined`, `scan:malicious` ve
  `package:malicious` gibi kararlı, kompakt dizelerdir.
...
- `trust.stale`, güven özetinin güncel olmayan girdilerden hesaplandığı ve
  yüksek güvenli izin kararından önce yenileme gerektiriyor olarak ele alınması
...
- Bu uç nokta, sahip/moderatör moderasyon uç noktalarından kasıtlı olarak daha
  dardır. Muhabir kimlikleri, rapor gövdeleri, özel kanıtlar veya dahili
...
- ClawPack sürümleri, tam yüklenen npm-pack `.tgz` baytlarını akış olarak
  gönderir.
...
Gelecekteki OpenClaw tüketimi için hesaplanmış hazırlığı döndürür.
...
moderatörler artefaktları onaylamak, karantinaya almak veya iptal etmek için
...
Moderatör/yönetici paketi rapor alımı endpoint'i.
...
- GitHub destekli skill devirleri baytları proxy'lemez veya aynalamaz. JSON
  ...
  ve `archiveUrl` içerir; tarama/güncel durum bir kapıdır ve başarılı yük meta
...
- Aynı istekte ya `files` ya da `clawpack` kullanın, asla ikisini birlikte kullanmayın.
...
Bir skill'i geçici sil / geri yükle (sahip, moderatör veya admin).
...
yayıncı tarafından talep edilebilir. Silme yanıtı, bu süre sonu geçerliyse `slugReservedUntil` içerir.
...
eski paylaşılan kullanıcı/kişisel yayıncıyı işaret ediyorsa endpoint önce onu bir kuruluş
yayıncısına geçirir. Yeni oluşturulan bir kuruluş için `memberHandle` sağlayın; işlem yapan
admin üye olarak eklenmez. `memberRole` varsayılan olarak `owner` olur.
...
çağıranı sahip olarak ekler. Bu endpoint mevcut kullanıcı/kişisel tanıtıcıları geçirmez ve
...
için kişisel yayıncıyı kurtarır. İstek her iki değişmez GitHub sağlayıcı hesap kimliğini de
adlandırmalıdır; değişebilir tanıtıcılar yalnızca operatöre yönelik bir koruma olarak kullanılır.

Uç nokta varsayılan olarak deneme çalıştırması yapar. Kurtarmayı uygulamak için personel her iki GitHub yetkilisi arasındaki sürekliliği bağımsız olarak doğruladıktan sonra `dryRun: false` ve
`confirmIdentityVerified: true` gerekir. Hedef kullanıcının geçerli kişisel
yayıncısında beceriler, paketler veya GitHub beceri kaynakları varsa kurtarma kapalı kalarak başarısız olur.
Kurtarma ayrıca, doğrudan sahip yollarının yeni yayıncı yetkisiyle uyumlu olması için kurtarılan yayıncının becerileri,
beceri slug takma adları, paketleri, paket denetçisi uyarıları ve türetilmiş arama özeti satırları için eski `ownerUserId` alanlarını taşır. Kurtarılan handle için etkin bir korumalı handle
...
GitHub beceri kaynakları yayıncı kapsamındadır ve yeniden yazılmak yerine kontrol edilmiş olarak raporlanır.
...
Mevcut bir yasak için saklanan gerekçeyi, yasağı kaldırmadan veya içeriği geri yüklemeden
değiştirin (yalnızca yönetici). `dryRun`, `false` olmadığı sürece varsayılan olarak deneme çalıştırması yapar.
