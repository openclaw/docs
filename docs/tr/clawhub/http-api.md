...
x-i18n:
    generated_at: "2026-07-04T18:16:10Z"
    model: gpt-5.5
...
Üçüncü taraf dizinler, ClawHub Skills öğelerini listelemek veya aramak için genel okuma uç noktalarını kullanabilir. Lütfen sonuçları önbelleğe alın, `429`/`Retry-After` değerlerine uyun, kullanıcıları kanonik ClawHub listesine (`https://clawhub.ai/<owner>/skills/<slug>`) geri bağlayın ve üçüncü taraf sitenin ClawHub tarafından desteklendiğini ima etmekten kaçının. Gizli, özel veya moderasyon tarafından engellenmiş içeriği genel API yüzeyi dışında yansıtmaya çalışmayın.

Web slug kısayolları kayıt defteri aileleri genelinde çözümlenir, ancak API istemcileri rota önceliğini yeniden kurmak yerine okuma uç noktaları tarafından döndürülen kanonik URL'leri kullanmalıdır.
...
- Güvenilir bir istemci IP'si yoksa, anonim istekler yalnızca hız sınırı türüyle kapsamlandırılmış yedek kovaları kullanır. Bu yedek kovalar, çağıran tarafından sağlanan yolları, slug'ları, paket adlarını, sürümleri, sorgu dizelerini veya diğer artifact parametrelerini içermez.
...
- Sonuçlar alaka sırasına göre döndürülür (gömme benzerliği + kesin slug/ad belirteci artırmaları + küçük bir popülerlik önceliği).
...
- Kullanıcıların kelimenin tam anlamıyla arayacağı terimleri görünen ad, özet ve etiketlere koyun. Bağımsız bir slug belirtecini yalnızca korumak istediğiniz kararlı bir kimlik olduğunda kullanın.
...
- Bir Skill beklenmedik şekilde görünmüyorsa, sıralamayla ilgili meta verileri değiştirmeden önce oturum açmışken önce `clawhub inspect @owner/slug` ile moderasyon durumunu kontrol edin.
...
- `nonSuspiciousOnly=true` olduğunda, imleç tabanlı sıralamalar sayfa alımından sonra şüpheli Skills öğeleri filtrelendiği için bir sayfada `limit` değerinden daha az öğe döndürebilir.
...
Bir Skill öğesini moderatör incelemesi için bildirin. Raporlar Skill düzeyindedir, isteğe bağlı olarak bir sürüme bağlanır ve Skill rapor kuyruğunu besler.
...
Skill rapor alımı için moderatör/yönetici uç noktası.
...
`note`, `confirmed` ve `dismissed` için gereklidir; `status` tekrar `open` olarak ayarlanırken atlanabilir. Skill öğesini aynı denetlenebilir iş akışında gizlemek için triyaj edilmiş bir raporla `finalAction: "hide"` gönderin.
...
- `security.hasScanResult` yalnızca bir tarama aracı kesin bir hüküm (`clean`, `suspicious` veya `malicious`) ürettiğinde `true` olur.
...
- Tarama isteği yükleri ve indirilebilir raporlar, saklama süresi dolduktan sonra tarama isteği deposundan sona erer.
...
- Engellenmiş veya gizlenmiş sürümler dahil olmak üzere tam gönderilmiş sürüm için depolanmış tarama sonuçlarını döndürür.
...
- `ok` yalnızca seçili sürüm oluşturulmuş bir Skill Card içerdiğinde, moderasyon tarafından kötü amaçlı yazılım nedeniyle engellenmediğinde ve ClawScan doğrulaması temiz olduğunda `true` olur.
- Beceri kimliği, yayımcı kimliği ve seçili sürüm meta verileri üst düzey zarf alanlarıdır (`slug`, `displayName`, `publisherHandle`, `version`, `resolvedFrom`, `tag`, `createdAt`); böylece shell otomasyonu bunları iç içe sarmalayıcıları açmadan okuyabilir.
...
Tam beceri sürümleri için güncel kompakt güvenlik hükümlerini döndürür. Bu
koleksiyon uç noktası, OpenClaw Control UI gibi, hangi yüklü
ClawHub beceri sürümlerini göstermeleri gerektiğini zaten bilen istemciler için tasarlanmıştır.
...
- Yanıt yalnızca güvenlik içerir. Skill Card verileri, oluşturulmuş kart durumu, yapıt dosya listeleri veya ayrıntılı tarama aracı yükleri içermez.
...
- Tek beceri Skill Card doğrulama zarfına ihtiyacınız olduğunda `/verify`, oluşturulmuş kart Markdown'ına ihtiyacınız olduğunda `/card` ve ayrıntılı tarama aracı verilerine ihtiyacınız olduğunda `/scan` kullanın.
...
- `clean` veya `suspicious` taramasına sahip mevcut GitHub destekli skills,
  ...
  `sourceRef: "public-github"` içeren `_source_handoff.json` içerir.
  ClawHub üzerinde barındırılan kaynak dosyalarını içermezler.
...
- `trust.stale`, güven özetinin güncelliğini yitirmiş girdilerden hesaplandığı ve
  yüksek güvenilirlikli izin kararından önce yenileme gerektiriyor olarak ele
  alınması gerektiği anlamına gelir.

Notlar:

- Bu uç nokta sürüme özgüdür. İstemciler, yalnızca en son paket meta verilerini
  okuduktan sonra değil, kurmak istedikleri paket sürümünü çözdükten sonra bunu
  çağırmalıdır.
- Özel paketler, çağıran sahip yayıncıyı okuyamadıkça `404` döndürür.
...
tek başlarına indirmeleri otomatik olarak gizlemez veya engellemezler; moderatörler
...
- Dosya boyutu sınırı: 200KB.
- Bekleyen VirusTotal taramaları okumaları engellemez; kötü amaçlı sürümler başka yerlerde yine de saklı tutulabilir.
...
npm ayna istemcileri için yüklenen tam ClawPack tarball baytlarını akış olarak sunar.
...
`suspicious` taramasına sahip, barındırılan sürümü olmayan mevcut GitHub destekli
bir skill için GitHub kaynak devrini döndürür.
...
- GitHub destekli skill devirleri baytları proxy'lemez veya aynalamaz. JSON
  ...
  ve `archiveUrl` içerir; tarama/geçerli durum bir kapıdır ve başarı yükü meta
  verisi olarak dahil edilmez.
- İndirme istatistikleri UTC günü başına benzersiz kimlikler olarak sayılır
  (API token'ı geçerliyse `userId`, aksi takdirde IP).
...
- İsteğe bağlı yük alanı: `migrateOwner`. `ownerHandle` ile `true` olduğunda,
  ...
  skill o sahibe taşınabilir. Bu açık onay olmadan sahip değişiklikleri
  reddedilir.
...
- Aynı istekte ikisi birden değil, ya `files` ya da `clawpack` kullanın.
...
Sahip tarafından başlatılan yumuşak silmeler slug'ı 30 gün rezerve eder; ardından slug başka
bir yayıncı tarafından talep edilebilir. Silme yanıtı, bu sona erme geçerli olduğunda
...
Yalnızca yönetici. Bir handle için org yayıncısının var olmasını sağlar. Handle hâlâ eski
paylaşılan kullanıcı/kişisel yayıncıyı gösteriyorsa uç nokta önce onu org yayıncısına taşır.
...
sonra gerçek code-plugin veya bundle-plugin sürümünü bu ad altında yayımlayabilir.
...
Yalnızca yönetici. Convex Auth hesap satırlarını düzenlemeden, doğrulanmış yedek GitHub OAuth asıl
kimliği için kişisel yayıncıyı kurtarır. İstek her iki değişmez GitHub sağlayıcı hesap kimliğini
adlandırmalıdır; değişebilir handle'lar yalnızca operatöre dönük bir koruma olarak kullanılır.

Uç nokta varsayılan olarak dry-run kullanır. Kurtarmayı uygulamak, personel her iki
...
yayımcısında Skills, paketler veya GitHub skill kaynakları varsa kurtarma güvenli kapalı şekilde başarısız olur.
...
Mevcut bir yasaklamanın depolanan nedenini, yasağı kaldırmadan veya içeriği geri yüklemeden
değiştirir (yalnızca yönetici). `dryRun`, `false` olmadığı sürece varsayılan olarak dry-run kullanır.
...
yayımlamaları, oluşan depolama kimliğini `clawpack` olarak ve döndürülen bileti
