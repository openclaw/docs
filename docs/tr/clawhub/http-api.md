...
x-i18n:
    generated_at: "2026-06-28T00:18:06Z"
    model: gpt-5.5
...
Üçüncü taraf dizinler, ClawHub becerilerini listelemek veya aramak için genel okuma uç noktalarını kullanabilir. Lütfen sonuçları önbelleğe alın, `429`/`Retry-After` değerlerine uyun, kullanıcıları kanonik ClawHub kaydına (`https://clawhub.ai/<owner>/skills/<slug>`) geri yönlendirin ve ClawHub'ın üçüncü taraf siteyi onayladığı izlenimini vermekten kaçının. Gizli, özel veya moderasyon tarafından engellenmiş içerikleri genel API yüzeyi dışında yansıtmaya çalışmayın.
...
- Kimliği doğrulanmış yazma uç noktaları, sunucu nedeni bildiğinde yalın bir `Unauthorized`
  ...
  silinmiş/yasaklanmış/devre dışı hesapların her biri, CLI istemcilerinin kullanıcılara
  onları neyin engellediğini söyleyebilmesi için eyleme geçirilebilir metin almalıdır.
...
- Eşzamanlı yeniden denemelerden kaçınmak için jitter eklenmiş geri çekilme kullanın.
...
- Güvenilir istemci IP'si yoksa anonim istekler yalnızca hız sınırı türüne göre
  kapsamlanan yedek kovaları kullanır. Bu yedek kovalar çağıranın sağladığı
...
- Sonuçlar alaka sırasına göre döndürülür (embedding benzerliği + tam slug/ad token artırmaları + küçük bir popülerlik önceliği).
...
- Popülerlik log ölçeklidir ve sınırlandırılır. Yüksek etkileşimli beceriler, sorgu metni daha zayıf bir eşleşmeyse daha düşük sıralanabilir.
- Şüpheli veya gizli moderasyon durumu, çağıran filtrelerine ve geçerli moderasyon durumuna bağlı olarak bir beceriyi genel aramadan kaldırabilir.
...
- Yeni slug daha iyi bir uzun vadeli kanonik ad olmadığı sürece, yalnızca tek bir sorguyu yakalamak için slug'ı yeniden adlandırmayın. Eski slug'lar yönlendirme takma adlarına dönüşür, ancak kanonik URL, görüntülenen slug ve gelecekteki arama özetleri yeni slug'ı kullanır.
- Yeniden adlandırma takma adları, kayıt üzerinden çözümlenen eski URL'ler ve kurulumlar için çözümlemeyi korur, ancak arama sıralaması yeniden adlandırma dizine alındıktan sonra kanonik beceri meta verilerine dayanır. Mevcut istatistikler beceriyle kalır.
...
- `nonSuspiciousOnly=true` olduğunda imleç tabanlı sıralamalar, şüpheli beceriler sayfa alındıktan sonra filtrelendiği için bir sayfada `limit` değerinden daha az öğe döndürebilir.
...
- Beceri platform meta verilerine sahip değilse `metadata` `null` olur.
...
bir bildirimle birlikte `finalAction: "hide"` iletin.
...
- `moderation`, en son sürümden türetilen geçerli skill düzeyi moderasyon anlık görüntüsüdür.
...
- Yanıt `202` ve `{ "ok": true, "scanId": "...", "jobId": "...", "status": "queued", "sourceKind": "published", "update": false, "queue": { "queuedAhead": 0, "queuedAheadIsEstimate": false, "position": 1, "running": 0, "runningIsEstimate": false, "note": "Scans are asynchronous and may take time to complete." } }` olur.
- Tarama işleri asenkrondur. Manuel tarama istekleri normal yayımlama/geri doldurma işlerinden önce önceliklendirilir, ancak tamamlanma yine de işçi kullanılabilirliğine bağlıdır.
...
- Engellenmiş veya gizlenmiş sürümler dahil, tam gönderilmiş sürüm için saklanan tarama sonuçlarını döndürür.
...
- `ok` yalnızca seçilen sürümde oluşturulmuş bir Skill Card varsa, moderasyon tarafından kötü amaçlı yazılım olarak engellenmemişse ve ClawScan doğrulaması temizse `true` olur.
- Skill kimliği, yayıncı kimliği ve seçilen sürüm meta verileri üst düzey zarf alanlarıdır (`slug`, `displayName`, `publisherHandle`, `version`, `resolvedFrom`, `tag`, `createdAt`); böylece kabuk otomasyonları bunları iç içe sarmalayıcıları açmadan okuyabilir.
...
- `security.signals`, `staticScan`, `virusTotal` ve `skillSpector` gibi destekleyici tarayıcı kanıtları içerir.
...
Tam skill sürümleri için geçerli kompakt güvenlik kararlarını döndürür. Bu koleksiyon uç noktası, OpenClaw Control UI gibi, hangi yüklü ClawHub skill sürümlerini göstermesi gerektiğini zaten bilen istemciler için tasarlanmıştır.
...
- Yanıt yalnızca güvenlik içindir. Skill Card verisi, oluşturulmuş kart durumu, yapıt dosya listeleri veya ayrıntılı tarayıcı yükleri içermez.
...
- Tek skill Skill Card doğrulama zarfına ihtiyaç duyduğunuzda `/verify`, oluşturulmuş kart Markdown'ına ihtiyaç duyduğunuzda `/card`, ayrıntılı tarayıcı verilerine ihtiyaç duyduğunuzda `/scan` kullanın.
...
- `category` (isteğe bağlı): plugin kategorisi filtresi. Yalnızca istek
  ...
  içeren paket uç noktaları). Kontrollü kategoriler ve eski v1 filtre takma
...
Çevrimdışı analiz için en son genel Skills toplu dışa aktarımı.
...
- `_manifest.json` her zaman ZIP kökünde bulunur.
- Tekil Plugin’ler veya dosyalar dışa aktarılamadığında `_errors.json` eklenir.
...
- Plugin araması için tarayıcı UI sıralama denetimleri, yüklenen alaka sonuçlarını
  yeniden sıralar ve geçerli `/skills` göz atma davranışıyla eşleşir.
...
- Tarama verileri varsa `version.vtAnalysis`, `version.llmAnalysis` ve
...
- `trust.stale`, güven özetinin eski girdilerden hesaplandığı ve yüksek güvenli
  izin kararından önce yenileme gerektiriyor kabul edilmesi gerektiği anlamına
  gelir.
...
- Özel paketler, çağıran sahip yayıncıyı okuyamıyorsa `404` döndürür.
- Bu uç nokta, sahip/moderatör moderasyon uç noktalarından özellikle daha
...
ancak kendi başlarına indirmeleri otomatik olarak gizlemez veya engellemezler;
...
- Paket sahibi, yayıncı üyesi, moderatör veya yönetici kullanıcı için API token
  gerektirir.
...
`finalAction: "quarantine"` veya `finalAction: "revoke"` geçirin.
...
- Varsayılan olarak en son sürümü kullanır.
- İndirme kotasını değil, okuma kota kovasını kullanır.
...
npm yansı istemcileri için tam olarak yüklenen ClawPack tarball baytlarını akış olarak verir.

Notlar:

- İndirme kota kovasını kullanır.
...
taraması olan ve barındırılan sürümü bulunmayan geçerli GitHub destekli bir skill
...
- GitHub destekli skill devirleri baytları proxy'lemez veya yansıtmaz. JSON
...
- İndirme istatistikleri UTC günü başına benzersiz kimlikler olarak sayılır
  (API token geçerliyse `userId`, aksi halde IP).
...
Token'ı doğrular ve kullanıcı handle değerini döndürür.
...
- İzin verilen form alanları `payload`, tekrarlanan `files` blobları veya tek
  bir `clawpack` tarball referansıdır. `clawpack`, bir `.tgz` blobu veya
...
- Doğrudan multipart yayın istekleri 18MB ile sınırlandırılır. ClawPack tarball
  dosyaları upload-url akışını 120MB tarball sınırına kadar kullanabilir.
- İsteğe bağlı yük alanı: `ownerHandle`. Mevcut olduğunda, yalnızca yöneticiler o sahip adına yayınlayabilir.
...
- Code plugin'leri `package.json`, kaynak repo meta verileri, kaynak commit meta
  verileri, config şeması meta verileri, `openclaw.compat.pluginApi` ve
...
- Başkası adına yapılan yayınlar da hedef sahip hesabına göre official-channel uygunluğunu doğrular.
...
Sahip tarafından başlatılan geçici silmeler slug'ı 30 gün boyunca rezerve eder; ardından slug başka
...
eski paylaşılan kullanıcı/kişisel yayıncıyı gösteriyorsa, uç nokta önce bunu bir org
yayıncısına taşır. Yeni oluşturulan bir org için `memberHandle` sağlayın; işlemi yapan
...
Yalnızca yönetici. Kök slug'ları ve paket adlarını sürüm yayınlamadan hak sahibi için rezerve eder.
Paket adları, sürüm satırları olmayan özel yer tutucu paketlere dönüşür; böylece aynı sahip daha
sonra gerçek code-plugin veya bundle-plugin sürümünü bu adla yayınlayabilir.
...
için kişisel bir yayıncıyı kurtarır. İstek, iki değişmez GitHub sağlayıcı hesap kimliğini de
adlandırmalıdır; değişebilir handle'lar yalnızca operatöre dönük bir koruma olarak kullanılır.

Uç nokta varsayılan olarak deneme çalıştırmasına ayarlıdır. Kurtarmayı uygulamak için personelin her iki
GitHub kimliği arasındaki sürekliliği bağımsız olarak doğrulamasından sonra `dryRun: false` ve
`confirmIdentityVerified: true` gerekir. Hedef kullanıcının mevcut kişisel
yayıncısında Skills, paketler veya GitHub Skills kaynakları varsa kurtarma kapalı durumda başarısız olur.
Kurtarma ayrıca kurtarılan yayıncının Skills, Skills slug takma adları, paketleri,
paket denetçisi uyarıları ve türetilmiş arama özeti satırları için eski `ownerUserId` alanlarını da taşır; böylece
doğrudan sahip yolları yeni yayıncı yetkisiyle uyumlu olur. Kurtarılan handle için etkin bir korumalı handle
rezervasyonu da değiştirme kullanıcısına yeniden atanır; böylece sonraki
...
GitHub Skills kaynakları yayıncı kapsamındadır ve yeniden yazılmak yerine denetlenmiş olarak raporlanır.
...
Bir kullanıcıyı yasaklar ve sahip olunan Skills öğelerini kalıcı olarak siler (yalnızca moderator/admin).
...
`POST /api/cli/upload-url`, `uploadUrl` ve `uploadTicket` döndürür. ClawPack tarball hazırlayan
paket yayınları, ortaya çıkan depolama kimliğini `clawpack` olarak ve döndürülen bileti
