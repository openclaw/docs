...
x-i18n:
    generated_at: "2026-07-03T02:54:13Z"
    model: gpt-5.5
...
Üçüncü taraf dizinler, ClawHub Skills listelemek veya aramak için genel okuma uç noktalarını kullanabilir. Lütfen sonuçları önbelleğe alın, `429`/`Retry-After` değerlerine uyun, kullanıcıları kanonik ClawHub listelemesine (`https://clawhub.ai/<owner>/skills/<slug>`) geri bağlayın ve ClawHub'ın üçüncü taraf siteyi desteklediği izlenimini vermekten kaçının. Genel API yüzeyi dışında gizli, özel veya moderasyon tarafından engellenmiş içeriği yansıtmaya çalışmayın.
...
- Kimliği doğrulanmış yazma uç noktaları, sunucu nedeni bildiğinde yalın bir
  `Unauthorized` döndürmemelidir. Eksik belirteçler, geçersiz/iptal edilmiş
  belirteçler ve silinmiş/yasaklanmış/devre dışı bırakılmış hesapların her biri,
  CLI istemcilerinin kullanıcılarına neyin engel olduğunu söyleyebilmesi için
...
- Güvenilir istemci IP'si yoksa, anonim istekler yalnızca hız sınırı türüne
  göre kapsamlandırılmış yedek kovalara gider. Bu yedek kovalar çağıran tarafından
  sağlanan yolları, slug'ları, paket adlarını, sürümleri, sorgu dizelerini veya diğer
...
izin hataları (`401`/`403`), hız sınırları (`429`) ve engellenmiş indirmeler dahildir.
İstemciler yanıt gövdesini insan tarafından okunabilir bir dize olarak okumalıdır. Bilinmeyen
sorgu parametreleri uyumluluk için yok sayılır, ancak geçersiz değerlere sahip tanınan sorgu
...
- Alaka popülerlikten daha güçlüdür. Kesin bir slug veya görüntü adı belirteci eşleşmesi, çok daha güçlü etkileşime sahip daha gevşek bir eşleşmenin önüne geçebilir.
- ASCII metin, sözcük ve noktalama sınırlarında belirteçlere ayrılır. Örneğin `personal-map` bağımsız bir `map` belirteci içerirken, `amap-jsapi-skill` `amap`, `jsapi` ve `skill` içerir; bu nedenle `map` araması `personal-map` için `amap-jsapi-skill` öğesine göre daha güçlü bir sözcüksel eşleşme verir.
- Popülerlik log ölçeklidir ve sınırlandırılır. Yüksek etkileşimli Skills, sorgu metni daha zayıf bir eşleşme olduğunda daha düşük sıralanabilir.
...
- Kullanıcıların kelimenin tam anlamıyla arayacağı terimleri görüntü adına, özete ve etiketlere koyun. Bağımsız bir slug belirtecini yalnızca aynı zamanda korumak istediğiniz kararlı bir kimlikse kullanın.
...
ayarlanırken atlanabilir. Aynı denetlenebilir iş akışında Skill'i gizlemek için triyaj yapılmış
bildirimle birlikte `finalAction: "hide"` gönderin.
...
- Beceri veya plugin için sahip/yayıncı yönetim erişimi ya da platform moderatörü/yönetici yetkisi gerektirir.
- Engellenmiş veya gizli sürümler dahil, tam olarak gönderilmiş sürüm için saklanan tarama sonuçlarını döndürür.
...
Yalnızca yöneticilere açık standart toplu yeniden tarama rotası. Eski `POST /api/v1/skills/-/rescan-batch` ile aynı yük biçimini kabul eder.
...
- Yanıt yalnızca güvenlikle ilgilidir. Skill Card verileri, oluşturulan kart durumu, yapıt dosya listeleri veya ayrıntılı tarayıcı yükleri içermez.
...
- Tek beceriye ait Skill Card doğrulama zarfına ihtiyaç duyduğunuzda `/verify`, oluşturulan kart Markdown'ına ihtiyaç duyduğunuzda `/card`, ayrıntılı tarayıcı verilerine ihtiyaç duyduğunuzda `/scan` kullanın.
...
- Tek tek Plugin’ler veya dosyalar dışa aktarılamadığında `_errors.json` dahil
  edilir.
...
- `GET /api/v1/plugins` altında belgelenen eski v1 filtre takma adları da kabul
  edilir.
...
- Plugin araması için tarayıcı kullanıcı arayüzü sıralama denetimleri, yüklenen
  alaka sonuçlarını yeniden sıralar ve mevcut `/skills` göz atma davranışıyla
  eşleşir.
...
- `trust.blockedFromDownload`, kurulum engelleme sinyalidir. OpenClaw ve diğer
  ...
  engellemelidir.
- `trust.reasons`, kullanıcıya yönelik ve denetim amaçlı açıklama listesidir.
...
- Bu uç nokta sürüme özeldir. İstemciler, yalnızca en son paket meta verilerini
  okuduktan sonra değil, kurmayı düşündükleri paket sürümünü çözdükten sonra onu
  çağırmalıdır.
- Özel paketler, çağıran sahip yayıncıyı okuyamadığı sürece `404` döndürür.
...
- ClawPack sürümleri, yüklenen tam npm-pack `.tgz` baytlarını aktarır.
...
Moderatör incelemesi için bir paketi bildirir. Raporlar paket düzeyindedir ve
isteğe bağlı olarak bir sürüme bağlanır. Moderasyon kuyruğunu beslerler ancak
indirmeleri kendi başlarına otomatik olarak gizlemez veya engellemezler;
...
moderasyonunu uygulamak için onaylanmış bir raporla `finalAction: "quarantine"`
...
Paket sürüm incelemesi için moderatör/yönetici uç noktası.
...
Karantinaya alınmış ve iptal edilmiş sürümler, artifact indirme rotalarından `403`
...
- Varsayılan olarak en son sürümü kullanır.
- İndirme kotası yerine okuma kotası demetini kullanır.
...
- Çağıran, sahip olan yayıncıyı okuyamıyorsa özel paketler `404` döndürür.
...
- Plugin/paket arşivleri, eski OpenClaw istemcilerinin çalışmaya devam etmesi
  için `package/` kökü olan zip dosyalarıdır.
...
- Yalnızca registry metadatası indirilen arşive enjekte edilmez.
...
- Scoped paket packument'ları hem `/api/npm/@scope/name` hem de npm'in kodlanmış
...
- İndirme kotası demetini kullanır.
...
taramasına sahip, barındırılan sürümü olmayan geçerli GitHub destekli bir beceri
...
- GitHub destekli beceri devirleri baytları proxy'lemez veya aynalamaz. JSON
  ...
  metadatası olarak dahil edilmez.
...
- İsteğe bağlı payload alanı: `migrateOwner`. `ownerHandle` ile `true` olduğunda,
  aktör hem geçerli hem de hedef yayıncılarda admin/owner ise mevcut bir beceri
...
- İzin verilen form alanları `payload`, tekrarlanan `files` blob'ları veya tek bir
  `clawpack` tarball referansıdır. `clawpack`, `.tgz` blob'u veya upload-url akışı
  tarafından döndürülen bir storage id olabilir. Aşamalandırılmış storage-id
  yayınları, o yükleme URL'siyle döndürülen `clawpackUploadTicket` değerini de
  içermelidir.
- Aynı istekte `files` veya `clawpack` kullanın; ikisini birlikte kullanmayın.
- JSON gövdeleri ve çağıran tarafından sağlanan `payload.files` / `payload.artifact`
  metadatası reddedilir.
- Doğrudan multipart yayımlama istekleri 18 MB ile sınırlandırılır. ClawPack
  tarball'ları upload-url akışını 120 MB tarball sınırına kadar kullanabilir.
...
- Kod Plugin'leri `package.json`, kaynak repo metadatası, kaynak commit
  metadatası, config schema metadatası, `openclaw.compat.pluginApi` ve
...
Bir beceriyi soft-delete yapar / geri yükler (sahip, moderatör veya admin).
...
yayıncı tarafından talep edilebilir. Bu süre sonu geçerli olduğunda silme yanıtı `slugReservedUntil` içerir.
...
Yalnızca admin. Bir handle için org yayıncısının var olduğundan emin olur. Handle hâlâ
...
için kişisel yayıncıyı kurtarır. İstek her iki değişmez GitHub provider hesap kimliğini de belirtmelidir;
değişebilir handle'lar yalnızca operatöre dönük bir koruma olarak kullanılır.

Uç nokta varsayılan olarak dry-run modundadır. Kurtarmayı uygulamak için personelin her iki
...
Kurtarma ayrıca, kurtarılan yayıncının becerileri, beceri slug alias'ları, paketleri,
paket inceleyici uyarıları ve türetilmiş arama özeti satırları için eski `ownerUserId` alanlarını da taşır; böylece
...
- `rename`, önceki slug'ı yönlendirme alias'ı olarak korur.
...
Bir kullanıcıyı banlayın ve sahip olduğu becerileri kalıcı olarak silin (yalnızca moderatör/yönetici).
...
Bir kullanıcının banını kaldırın ve uygun becerileri geri yükleyin (yalnızca yönetici).
...
Mevcut bir ban için kayıtlı nedeni, banı kaldırmadan veya içeriği geri yüklemeden değiştirin
(yalnızca yönetici). `dryRun`, `false` olmadığı sürece varsayılan olarak dry-run modundadır.
...
- `query` (isteğe bağlı): `q` için alias
...
Bir star (vurgulama) ekleyin/kaldırın. Her iki uç nokta da idempotenttir.
...
aşamalandıran paket yayınları, elde edilen depolama kimliğini `clawpack` olarak ve
döndürülen bileti `clawpackUploadTicket` olarak göndermelidir.
