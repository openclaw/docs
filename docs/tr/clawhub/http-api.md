...
x-i18n:
    generated_at: "2026-06-30T14:20:49Z"
    model: gpt-5.5
...
Üçüncü taraf dizinler, ClawHub becerilerini listelemek veya aramak için herkese açık okuma uç noktalarını kullanabilir. Lütfen sonuçları önbelleğe alın, `429`/`Retry-After` kurallarına uyun, kullanıcıları kanonik ClawHub listesine (`https://clawhub.ai/<owner>/skills/<slug>`) geri yönlendirin ve ClawHub'ın üçüncü taraf siteyi onayladığı izlenimini vermekten kaçının. Gizli, özel veya moderasyon tarafından engellenmiş içeriği herkese açık API yüzeyi dışında yansıtmaya çalışmayın.
...
- Güvenilir istemci IP'si yoksa anonim istekler yalnızca hız sınırı türüyle kapsamlanan yedek kovalardan yararlanır. Bu yedek kovalar çağıranın sağladığı yolları, slug'ları, paket adlarını, sürümleri, sorgu dizelerini veya diğer yapıt parametrelerini içermez.
...
- ASCII metni sözcük ve noktalama sınırlarında belirteçlere ayrılır. Örneğin, `personal-map` bağımsız bir `map` belirteci içerirken `amap-jsapi-skill` `amap`, `jsapi` ve `skill` içerir; bu nedenle `map` araması `personal-map` için `amap-jsapi-skill` öğesine göre daha güçlü bir sözcüksel eşleşme sağlar.
- Popülerlik log ölçeklidir ve sınırlandırılmıştır. Sorgu metni daha zayıf bir eşleşmeyse yüksek etkileşimli beceriler daha düşük sıralanabilir.
...
- Kullanıcıların gerçekten arayacağı terimleri görüntülenen ada, özete ve etiketlere koyun. Bağımsız bir slug belirtecini yalnızca aynı zamanda korumak istediğiniz kararlı bir kimlikse kullanın.
- Yeni slug daha iyi uzun vadeli kanonik ad olmadığı sürece yalnızca tek bir sorguyu yakalamak için slug adını değiştirmeyin. Eski slug'lar yönlendirme takma adları olur, ancak kanonik URL, görüntülenen slug ve gelecekteki arama özetleri yeni slug'ı kullanır.
- Yeniden adlandırma takma adları, eski URL'ler ve kayıt defteri üzerinden çözümlenen kurulumlar için çözümlemeyi korur, ancak arama sıralaması yeniden adlandırma dizine alındıktan sonra kanonik beceri meta verilerine dayanır. Mevcut istatistikler beceriyle kalır.
- Bir beceri beklenmedik şekilde görünmüyorsa, sıralamayla ilgili meta verileri değiştirmeden önce oturum açmışken önce `clawhub inspect @owner/slug` ile moderasyon durumunu kontrol edin.
...
- Varsa sayfalamaya devam etmek için `nextCursor` kullanın. Kısa bir sayfa tek başına sonuçların sonu anlamına gelmez.
...
- `metadata.os`: beceri frontmatter bölümünde bildirilen işletim sistemi kısıtlamaları (örn. `["macos"]`, `["linux"]`). Bildirilmemişse `null`.
...
Bir beceriyi moderatör incelemesi için bildirin. Bildirimler beceri düzeyindedir, isteğe bağlı olarak bir sürüme bağlanır ve beceri bildirim kuyruğunu besler.
...
`note`, `confirmed` ve `dismissed` için gereklidir; `status` tekrar `open` olarak ayarlanırken atlanabilir. Aynı denetlenebilir iş akışında beceriyi gizlemek için triage edilmiş bir bildirimle `finalAction: "hide"` iletin.
...
- Geçmiş bir sürümü sorgularken, `moderation` ve `security` değerlerini aynı sürüm bağlamı olarak değerlendirmeden önce `moderation.matchesRequestedVersion` ve `moderation.sourceVersion` değerlerini denetleyin.
...
- Engellenmiş veya gizlenmiş sürümler dahil olmak üzere, tam gönderilen sürüme ait saklanan tarama sonuçlarını döndürür.
...
- `ok` yalnızca seçilen sürüm oluşturulmuş bir Skill Card’a sahipse, moderasyon tarafından kötü amaçlı yazılım nedeniyle engellenmemişse ve ClawScan doğrulaması temizse `true` olur.
...
- `provenance` yalnızca ClawHub yayımlama veya içe aktarma sırasında bir GitHub depo/ref/commit/yol çözüp sakladığında `server-resolved-github-import` olur; aksi takdirde `unavailable` olur.
...
- Yanıt yalnızca güvenlikle ilgilidir. Skill Card verilerini, oluşturulan kart durumunu, artifact dosya listelerini veya ayrıntılı tarayıcı yüklerini içermez.
...
- Kimliği doğrulanmış çağıranlar, liste/arama sonuçlarında üyesi oldukları yayımcıların özel paketlerini görebilir.
...
- `GET /api/v1/plugins` altında belgelenen eski v1 filtre takma adları da
  kabul edilir.
- Kategori filtreleme, arama sorgusu yeniden yazımı değil, Plugin kategorisi özet
...
- `version.sha256hash`, eski istemciler için kullanımdan kaldırılmış uyumluluk meta verisidir.
  ...
  karmasını alır. Modern istemciler, kanonik sürüm yapıtını tanımlayan
...
- `package.name`, `package.displayName` ve `package.family`, çözümlenen
  kayıt paketini tanımlar.
...
- `trust.scanStatus`, tarayıcı girdilerinden ve elle yapılan sürüm moderasyonundan
  türetilen etkin güven durumudur.
- `trust.moderationState` null olabilir. Elle yapılan sürüm moderasyonu yoksa
  `null` değerindedir.
- `trust.blockedFromDownload`, kurulum engelleme sinyalidir. OpenClaw ve diğer
  ...
  alanlarından engelleme kurallarını yeniden türetmek yerine kurulumu engellemelidir.
- `trust.reasons`, kullanıcıya dönük ve denetim açıklaması listesidir. Neden kodları
...
- `trust.stale`, güven özetinin güncel olmayan girdilerden hesaplandığı ve
  yüksek güvenli izin kararından önce yenileme gerektiriyor kabul edilmesi gerektiği
  anlamına gelir.

Notlar:

- Bu uç nokta sürüme özeldir. İstemciler bunu yalnızca en son paket meta verilerini
...
- Bu uç nokta, sahip/moderatör moderasyon uç noktalarından kasıtlı olarak daha dardır.
  Kurulum kararını ve genel açıklamayı sunar; bildiren kimliklerini, rapor gövdelerini,
  özel kanıtları veya dahili inceleme zaman çizelgelerini sunmaz.
...
- `packageName`, npm adına göre normalleştirilir; planlanan geçişler için paket
  eksik olabilir.
- Bu yalnızca geçiş hazır olma durumunu izler. OpenClaw üzerinde değişiklik yapmaz
  veya ClawPack üretmez.
...
Bir paketi moderatör incelemesi için bildirir. Raporlar paket düzeyindedir ve isteğe bağlı olarak
bir sürüme bağlanır. Moderasyon kuyruğunu beslerler ancak tek başlarına indirmeleri
otomatik olarak gizlemez veya engellemez; moderatörler yapıtları onaylamak,
...
- Moderatör veya admin kullanıcı için API token gerekir.
...
- Paket sahibi, yayıncı üyesi, moderatör veya admin kullanıcı için API token
  gerekir.
...
Paket raporlarını çözümlemek veya yeniden açmak için moderatör/admin uç noktası.
...
`confirmed` ve `dismissed` için `note` zorunludur; `status` yeniden `open`
...
- `revoked`: bir sürüm daha önce güvenilir bulunduktan sonra engellendi.
...
- Eski yalnızca ZIP sürümler bilerek atlanır.
...
npm yansıtıcı istemcileri için tam olarak yüklenen ClawPack tarball baytlarını akış olarak sunar.
...
`suspicious` taraması olan ve barındırılmış sürümü bulunmayan güncel GitHub
...
- GitHub destekli beceri devirleri baytları proxy'lemez veya yansıtmaz. JSON yanıtı
  ...
  `archiveUrl` içerir; tarama/güncel durum bir kapıdır ve başarı yükü meta
...
- İsteğe bağlı yük alanı: `migrateOwner`. `ownerHandle` ile birlikte `true`
  olduğunda, aktör hem mevcut hem de hedef yayıncılarda admin/sahip ise mevcut
  bir beceri o sahibe taşınabilir. Bu açık seçim olmadan sahip değişiklikleri
  reddedilir.
...
- İzin verilen form alanları `payload`, tekrarlanan `files` blobları veya bir
  `clawpack` tarball referansıdır. `clawpack`, bir `.tgz` blobu veya upload-url
...
- Doğrudan multipart yayın istekleri 18MB ile sınırlıdır. ClawPack tarball dosyaları
  120MB tarball sınırına kadar upload-url akışını kullanabilir.
- İsteğe bağlı yük alanı: `ownerHandle`. Mevcut olduğunda yalnızca adminler o sahip adına yayınlayabilir.
...
- Adına yapılan yayınlar, official-channel uygunluğunu yine hedef sahip hesabına göre doğrular.

### `DELETE /api/v1/skills/{slug}` / `POST /api/v1/skills/{slug}/undelete`

Bir beceriyi geçici olarak siler / geri yükler (sahip, moderatör veya admin).
...
bir yayıncı tarafından talep edilebilir. Bu süre sonu geçerliyse silme yanıtı `slugReservedUntil` içerir.
...
Yalnızca admin. Bir handle için org yayıncısının var olmasını sağlar. Handle hâlâ
eski paylaşılan kullanıcı/kişisel yayıncıyı gösteriyorsa uç nokta önce onu bir org yayıncısına taşır.
Yeni oluşturulmuş bir org için `memberHandle` sağlayın; işlem yapan admin üye olarak eklenmez.
...
Paket adları sürüm satırı olmayan özel yer tutucu paketlere dönüşür; böylece aynı sahip daha
sonra gerçek code-plugin veya bundle-plugin sürümünü o ada yayınlayabilir.
...
Yalnızca admin. Convex Auth hesap satırlarını düzenlemeden doğrulanmış yedek GitHub OAuth principal'ı
...
Uç nokta varsayılan olarak deneme çalıştırması yapar. Kurtarmayı uygulamak için personelin her iki GitHub kimliği arasındaki sürekliliği bağımsız olarak doğrulamasından sonra `dryRun: false` ve
`confirmIdentityVerified: true` gerekir. Hedef kullanıcının mevcut kişisel yayımcısında beceriler, paketler veya GitHub beceri kaynakları varsa kurtarma güvenli kapalı şekilde başarısız olur.
Kurtarma ayrıca kurtarılan yayımcının becerileri, beceri slug takma adları, paketleri, paket inceleyici uyarıları ve türetilmiş arama özeti satırları için eski `ownerUserId` alanlarını da geçirir; böylece doğrudan sahip yolları yeni yayımcı yetkisiyle uyumlu olur. Kurtarılan handle için etkin korumalı handle rezervasyonu da yedek kullanıcıya yeniden atanır; böylece sonraki profil eşitlemesi eski kullanıcının rakip yetkisini geri yükleyemez. Her birincil tablo, uygulama işlemi başına
...
Yasağı kaldırmadan veya içeriği geri yüklemeden mevcut bir yasak için depolanan nedeni değiştir
...
yayınları, ortaya çıkan depolama kimliğini `clawpack` olarak ve dönen bileti `clawpackUploadTicket` olarak göndermelidir.
