...
x-i18n:
    generated_at: "2026-07-02T08:42:47Z"
    model: gpt-5.5
...
Üçüncü taraf dizinler, ClawHub Skills öğelerini listelemek veya aramak için genel okuma uç noktalarını kullanabilir. Lütfen sonuçları önbelleğe alın, `429`/`Retry-After` değerlerine uyun, kullanıcıları kanonik ClawHub kaydına (`https://clawhub.ai/<owner>/skills/<slug>`) geri bağlayın ve ClawHub'ın üçüncü taraf siteyi onayladığı izlenimini vermekten kaçının. Gizli, özel veya moderasyon tarafından engellenmiş içeriği genel API yüzeyi dışında yansıtmaya çalışmayın.
...
- Kimliği doğrulanmış yazma uç noktaları, sunucu nedeni bildiğinde yalın bir `Unauthorized`
  ...
  istemcilerinin kullanıcılara onları neyin engellediğini söyleyebilmesi için uygulanabilir metin almalıdır.
...
- Güvenilir istemci IP başlıklarını, `cf-connecting-ip` dahil, yalnızca dağıtım
  güvenilir iletilen başlıkları açıkça etkinleştirdiğinde kullanır.
- ClawHub, istemci IP'lerini uçta tanımlamak için güvenilir iletme başlıkları kullanır.
- Güvenilir istemci IP'si yoksa anonim istekler yalnızca hız sınırı türüne göre
  kapsamlandırılmış yedek kovalardan yararlanır. Bu yedek kovalar, çağıranın sağladığı
...
- Popülerlik logaritmik olarak ölçeklenir ve sınırlandırılır. Yüksek etkileşimli Skills öğeleri, sorgu metni daha zayıf eşleştiğinde daha düşük sıralanabilir.
...
- Kullanıcıların kelimenin tam anlamıyla arayacağı terimleri görüntülenen ada, özete ve etiketlere koyun. Bağımsız bir slug token'ını yalnızca korumak istediğiniz kararlı bir kimlik olduğunda kullanın.
- Yeni slug daha iyi bir uzun vadeli kanonik ad değilse, yalnızca tek bir sorguyu yakalamak için slug'ı yeniden adlandırmayın. Eski slug'lar yönlendirme takma adları olur, ancak kanonik URL, görüntülenen slug ve gelecekteki arama özetleri yeni slug'ı kullanır.
...
- `nonSuspiciousOnly=true` olduğunda, imleç tabanlı sıralamalar bir sayfada `limit` öğeden daha azını döndürebilir çünkü şüpheli Skills öğeleri sayfa alımından sonra filtrelenir.
...
- Genel çağıranlar yalnızca halihazırda işaretlenmiş görünür Skills için `200` alır.
...
ayarlanırken atlanabilir. Skill'i aynı denetlenebilir iş akışında gizlemek için, triyaj edilmiş
bir bildirimle birlikte `finalAction: "hide"` geçirin.
...
- `version` ya da `tag` sağlanmazsa en son sürümü kullanır.
...
- `moderation`, en son sürümden türetilmiş geçerli Skill düzeyinde moderasyon anlık görüntüsüdür.
...
- Tarama isteği yükleri ve indirilebilir raporlar, saklama penceresinden sonra scan-request deposundan sona erer.
...
- Tarama işleri eşzamansızdır. Manuel tarama isteklerine normal yayımlama/geri doldurma işlerinden önce öncelik verilir, ancak tamamlanma yine de çalışan uygunluğuna bağlıdır.
...
- Kuyruktayken `queue.queuedAhead` ve `queue.position` değerlerini döndürür; böylece istemciler isteğin önünde kaç öncelikli manuel tarama olduğunu gösterebilir. Çok büyük kuyruklar sınırlandırılır ve `queuedAheadIsEstimate: true` ile bildirilir.
...
- Engellenmiş veya gizli sürümler dahil, tam olarak gönderilen sürüm için saklanan tarama sonuçlarını döndürür.
...
- Skill kimliği, yayımcı kimliği ve seçilen sürüm meta verileri, kabuk otomasyonunun iç içe sarmalayıcıları açmadan okuyabilmesi için üst düzey zarf alanlarıdır (`slug`, `displayName`, `publisherHandle`, `version`, `resolvedFrom`, `tag`, `createdAt`).
...
- Sonuçlar öğe başınadır; eksik bir Skill veya sürüm tüm yanıtın başarısız olmasına neden olmaz.
- Yanıt yalnızca güvenlik içindir. Skill Card verilerini, oluşturulmuş kart durumunu, yapıt dosya listelerini veya ayrıntılı tarayıcı yüklerini içermez.
...
- Tek Skill için Skill Card doğrulama zarfına ihtiyacınız olduğunda `/verify`, oluşturulmuş kart markdown'una ihtiyacınız olduğunda `/card` ve ayrıntılı tarayıcı verilerine ihtiyacınız olduğunda `/scan` kullanın.
...
- `category` (isteğe bağlı): Plugin kategori filtresi. Yalnızca istek
  Plugin paketleriyle kapsamlandırıldığında desteklenir (`/api/v1/plugins`,
...
- Skill girişleri Skill kayıt defteri tarafından desteklenmeye devam eder ve hâlâ yalnızca `POST /api/v1/skills` üzerinden yayımlanabilir.
...
- `isOfficial` (isteğe bağlı): `true` veya `false`
- `category` (isteğe bağlı): Plugin kategori filtresi. Yalnızca istek
  Plugin paketleriyle kapsamlandırıldığında desteklenir. Denetimli kategoriler
...
- `clean` veya `suspicious` taramasına sahip geçerli GitHub destekli Skills,
  ...
  ClawHub'da barındırılan kaynak dosyalarını içermezler.
- Her Skill `_export_skill_meta.json` içerir.
- `_manifest.json` her zaman ZIP kökünde bulunur.
- Tekil Skills veya dosyalar dışa aktarılamadığında `_errors.json` dahil edilir.
...
- `startDate` (gerekli): Plugin `updatedAt` için Unix milisaniyesi alt sınırı.
- `endDate` (gerekli): Plugin `updatedAt` için Unix milisaniyesi üst sınırı.
- `limit` (isteğe bağlı): tam sayı (1-250), varsayılan `250`.
- `cursor` (isteğe bağlı): önceki yanıttan sayfalama imleci.
...
- Plugin araması için tarayıcı kullanıcı arayüzü sıralama denetimleri,
  yüklenen alaka sonuçlarını yeniden sıralar ve geçerli `/skills` göz atma
...
- `version.artifact.kind`, eski dünya paket arşivleri için `legacy-zip` veya
  ClawPack destekli sürümler için `npm-pack` değerindedir.
...
- `trust.blockedFromDownload`, kurulum engelleme sinyalidir. OpenClaw ve diğer
  ...
  engellemelidir.
- `trust.reasons`, kullanıcıya yönelik ve denetim açıklama listesidir. Neden
...
- `trust.stale`, güven özetinin güncel olmayan girdilerden hesaplandığı ve yüksek
  güvenli izin kararından önce yenileme gerektirir şekilde ele alınması gerektiği
  anlamına gelir.

Notlar:

- Bu uç nokta sürüme özeldir. İstemciler, yalnızca en son paket meta verilerini
  okuduktan sonra değil, yüklemeyi amaçladıkları paket sürümünü çözdükten sonra
  bunu çağırmalıdır.
- Özel paketler, çağıran sahip yayıncıyı okuyamıyorsa `404` döndürür.
- Bu uç nokta, sahip/moderatör moderasyon uç noktalarından kasıtlı olarak daha
  dardır. Bildirici kimliklerini, rapor gövdelerini, özel kanıtları veya dahili
...
- ClawPack sürümleri, yüklenen tam npm-pack `.tgz` baytlarını akış olarak verir.
- Eski ZIP sürümleri `/api/v1/packages/{name}/download?version=` konumuna
  yönlendirir.
...
- `packageName` npm adı olarak normalleştirilir; planlanan geçişlerde paket
  eksik olabilir.
...
- `open`: şüpheli, kötü amaçlı, beklemede, karantinaya alınmış, geri alınmış veya bildirilen sürümler.
- `blocked`: karantinaya alınmış, geri alınmış veya kötü amaçlı sürümler.
...
moderatörler yapıtları onaylamak, karantinaya almak veya geri almak için sürüm
...
- Yalnızca registry meta verileri indirilen arşive enjekte edilmez.
...
- `slug` (gerekli)
- `hash` (gerekli): bundle parmak izinin 64 karakterli onaltılık sha256 değeri
...
- `tag` (isteğe bağlı): tag adı (ör. `latest`)
...
- İzin verilen form alanları `payload`, yinelenen `files` blob'ları veya tek bir
  ...
  yayımları, bu yükleme URL'siyle döndürülen `clawpackUploadTicket` değerini de
  içermelidir.
...
- İsteğe bağlı yük alanı: `ownerHandle`. Mevcut olduğunda, yalnızca admin'ler bu sahip adına yayımlayabilir.
...
- Başkası adına yayımlar, official kanal uygunluğunu hedef sahip hesabına göre doğrulamaya devam eder.
...
başka bir yayıncı tarafından alınabilir. Silme yanıtı, bu sürenin geçerli olduğu durumlarda
...
Yalnızca admin. Bir handle için kuruluş yayıncısının var olmasını sağlar. Handle hâlâ
eski paylaşımlı kullanıcı/kişisel yayıncıyı işaret ediyorsa uç nokta önce bunu bir kuruluş
yayıncısına taşır. Yeni oluşturulan bir kuruluş için `memberHandle` sağlayın; işlemi yapan
...
Kimliği doğrulanmış self-servis kuruluş yayıncısı oluşturma. Yeni bir kuruluş yayıncısı
oluşturur ve çağıran tarafı sahip olarak ekler. Bu uç nokta mevcut kullanıcı/kişisel
handle'ları taşımaz ve yayıncıyı trusted/official olarak işaretlemez.
...
Yalnızca admin. Bir sürüm yayımlamadan, hak sahibi için kök slug'ları ve paket adlarını
...
Yalnızca admin. Convex Auth hesap satırlarını düzenlemeden, doğrulanmış yedek GitHub OAuth
principal'ı için kişisel yayıncıyı kurtarır. İstek, değiştirilemez iki GitHub sağlayıcı
hesap kimliğini de adlandırmalıdır; değiştirilebilir handle'lar yalnızca operatöre dönük
bir koruma olarak kullanılır.

Uç nokta varsayılan olarak deneme çalıştırması kullanır. Kurtarmayı uygulamak için personel her iki GitHub kimliği arasındaki sürekliliği bağımsız olarak doğruladıktan sonra `dryRun: false` ve
...
Kurtarma ayrıca, doğrudan sahip yollarının yeni yayımcı yetkisiyle uyumlu olması için kurtarılan yayımcının Skills,
skill slug diğer adları, paketleri, paket denetleyici uyarıları ve türetilmiş arama özeti satırları için eski `ownerUserId` alanlarını da taşır. Kurtarılan handle için etkin bir korumalı handle
...
GitHub skill kaynakları yayımcı kapsamındadır ve yeniden yazılmak yerine denetlendi olarak raporlanır.
...
Bir kullanıcıyı yasaklar ve sahip olunan Skills’i kalıcı olarak siler (yalnızca moderatör/yönetici).
...
içeriği geri yüklemeden değiştirir (yalnızca yönetici). `dryRun`, `false` olmadığı sürece varsayılan olarak deneme çalıştırması kullanır.
...
Yıldız ekler/kaldırır (öne çıkarmalar). Her iki uç nokta da idempotenttir.
...
yayımları, ortaya çıkan depolama kimliğini `clawpack` olarak ve döndürülen bileti `clawpackUploadTicket` olarak göndermelidir.
