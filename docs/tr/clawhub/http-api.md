...
x-i18n:
    generated_at: "2026-07-05T07:57:03Z"
    model: gpt-5.5
...
Üçüncü taraf dizinler, ClawHub Skills öğelerini listelemek veya aramak için herkese açık okuma uç noktalarını kullanabilir. Lütfen sonuçları önbelleğe alın, `429`/`Retry-After` değerlerine uyun, kullanıcıları kanonik ClawHub listesine (`https://clawhub.ai/<owner>/skills/<slug>`) geri yönlendirin ve ClawHub’ın üçüncü taraf siteyi onayladığı izlenimini vermekten kaçının. Gizli, özel veya moderasyon tarafından engellenmiş içeriği herkese açık API yüzeyinin dışında yansıtmaya çalışmayın.

Web slug kısayolları kayıt defteri aileleri genelinde çözümlenir, ancak API istemcileri rota önceliğini yeniden oluşturmak yerine okuma uç noktalarının döndürdüğü kanonik URL’leri kullanmalıdır.
...
- Kimliği doğrulanmış yazma uç noktaları, sunucu nedeni bildiğinde çıplak bir `Unauthorized` döndürmemelidir. Eksik token’lar, geçersiz/iptal edilmiş token’lar ve silinmiş/yasaklanmış/devre dışı bırakılmış hesaplar, CLI istemcilerinin kullanıcılara onları neyin engellediğini söyleyebilmesi için ayrı ayrı eyleme dönük metin almalıdır.
...
- `Retry-After` yoksa `RateLimit-Reset` değerine geri dönün (veya `X-RateLimit-Reset` değerinden hesaplayın).
...
- Güvenilir istemci IP’si yoksa, anonim istekler yalnızca hız sınırı türüne göre kapsamlandırılmış geri dönüş kovaları kullanır. Bu geri dönüş kovaları çağıranın sağladığı yolları, slug’ları, paket adlarını, sürümleri, sorgu dizelerini veya diğer yapıt parametrelerini içermez.
...
- Sonuçlar alaka sırasına göre döndürülür (gömme benzerliği + tam slug/ad token yükseltmeleri + küçük bir popülerlik önceliği).
- Alaka popülerlikten daha güçlüdür. Kesin bir slug veya görüntülenen ad token eşleşmesi, çok daha güçlü etkileşime sahip daha gevşek bir eşleşmenin önüne geçebilir.
- ASCII metin, sözcük ve noktalama sınırlarında token’lara ayrılır. Örneğin `personal-map` bağımsız bir `map` token’ı içerirken, `amap-jsapi-skill` `amap`, `jsapi` ve `skill` içerir; bu nedenle `map` araması `personal-map` için `amap-jsapi-skill` öğesine göre daha güçlü bir sözcüksel eşleşme sağlar.
- Popülerlik logaritmik ölçeklenir ve sınırlandırılır. Yüksek etkileşimli Skills öğeleri, sorgu metni daha zayıf bir eşleşmeyse daha düşük sıralanabilir.
...
- Kullanıcıların kelimesi kelimesine arayacağı terimleri görüntülenen ada, özete ve etiketlere koyun. Bağımsız bir slug token’ını yalnızca korumak istediğiniz kararlı bir kimlik olduğunda kullanın.
- Yeni slug daha iyi bir uzun vadeli kanonik ad değilse, yalnızca tek bir sorguyu kovalamak için slug’ı yeniden adlandırmayın. Eski slug’lar yönlendirme alias’larına dönüşür, ancak kanonik URL, görüntülenen slug ve gelecekteki arama özetleri yeni slug’ı kullanır.
- Yeniden adlandırma alias’ları eski URL’ler ve kayıt defteri üzerinden çözümlenen kurulumlar için çözümlemeyi korur, ancak arama sıralaması, yeniden adlandırma dizine eklendikten sonra kanonik Skill meta verilerine dayanır. Mevcut istatistikler Skill ile kalır.
...
- `sort` (isteğe bağlı): `updated` (varsayılan), `recommended` (alias: `default`), `createdAt` (alias: `newest`), `downloads`, `stars` (alias: `rating`), eski kurulum alias’ları `installsCurrent`/`installs`/`installsAllTime` `downloads` ile eşlenir, `trending`
- `nonSuspiciousOnly` (isteğe bağlı): şüpheli (`flagged.suspicious`) Skills öğelerini gizlemek için `true`
- `nonSuspicious` (isteğe bağlı): `nonSuspiciousOnly` için eski alias
...
- Kanıt herkese açık çağıranlar için redakte edilir ve yalnızca sahipler/moderatörler için ham parçalar içerir.

### `POST /api/v1/skills/{slug}/report`

Moderatör incelemesi için bir Skill bildirin. Raporlar Skill düzeyindedir, isteğe bağlı olarak bir sürüme bağlanır ve Skill rapor kuyruğunu besler.
...
`note`, `confirmed` ve `dismissed` için gereklidir; `status` yeniden `open` olarak ayarlanırken atlanabilir. Skill’i aynı denetlenebilir iş akışında gizlemek için triage edilmiş bir raporla `finalAction: "hide"` geçirin.
...
- `moderation`, en son sürümden türetilen güncel Skills düzeyinde moderasyon anlık görüntüsüdür.
...
- Engellenmiş veya gizli sürümler dahil olmak üzere, tam gönderilmiş sürüm için depolanmış tarama sonuçlarını döndürür.
...
Yalnızca yöneticiye açık standart toplu yeniden tarama rotası. Eski `POST /api/v1/skills/-/rescan-batch` ile aynı yük biçimini kabul eder.
...
- `ok` yalnızca seçilen sürümün oluşturulmuş bir Skill Card'ı olduğunda, moderasyon tarafından kötü amaçlı yazılım nedeniyle engellenmemiş olduğunda ve ClawScan doğrulaması temiz olduğunda `true` olur.
...
- `provenance` yalnızca ClawHub yayımlama veya içe aktarma sırasında bir GitHub deposu/ref'i/commit'i/yolu çözüp depoladığında `server-resolved-github-import` olur; aksi halde `unavailable` olur.
...
ClawHub Skill sürümlerini göstermeleri gerektiğini zaten bilen istemciler için tasarlanmıştır.
...
- `security.signals.dependencyRegistry`, v1 yanıt uyumluluğu için korunur, ancak bağımlılık kayıt defteri varlık tarayıcısı kullanımdan kaldırılmıştır ve bu anahtar her zaman `null` olur.
- Skill Card'ın olmaması bu uç noktanın `ok`, `decision` veya `reasons` değerlerini etkilemez; istemciler kart içeriğine ihtiyaç duyduklarında yüklü `skill-card.md` dosyasını yerel olarak okumalıdır.
...
Birleşik `/api/v1/packages` uç noktasında yalnızca Plugin'lere yöneliktir; Skill kataloğu için
...
- `clean` veya `suspicious` taraması olan mevcut GitHub destekli Skills,
  ...
  ile `_source_handoff.json` içerir. ClawHub tarafından barındırılan kaynak dosyalarını içermezler.
...
`code-plugin` ve `bundle-plugin` paketleri arasında yalnızca Plugin araması.
...
- `GET /api/v1/plugins` altında belgelenen eski v1 filtre takma adları da
  kabul edilir.
- Kategori filtreleme, arama sorgusunun yeniden yazılması değil, Plugin kategori
  özet satırlarıyla desteklenen gerçek bir API filtresidir.
...
- Plugin araması için tarayıcı kullanıcı arayüzü sıralama denetimleri, yüklenen
  alaka sonuçlarını yeniden sıralar ve geçerli `/skills` göz atma davranışıyla
  eşleşir.
...
- Paket sahibi, kuruluş yayıncısı sahibi/yöneticisi, platform moderatörü veya
  platform yöneticisi için bir API belirteci gerektirir.
...
- `version.artifact.kind`, eski tip paket arşivleri için `legacy-zip` veya
...
- `version.sha256hash`, eski istemciler için kullanımdan kaldırılmış uyumluluk
  ...
  ZIP baytlarını karmalar. Modern istemciler, kanonik sürüm yapıtını tanımlayan
...
- `trust.reasons`, kullanıcıya dönük ve denetim amaçlı açıklama listesidir. Neden
...
- ClawPack sürümleri, yüklenen tam npm-pack `.tgz` baytlarını akış olarak aktarır.
...
kendi başlarına indirmeleri otomatik gizlemez veya engellemez; moderatörler
yapıtları onaylamak, karantinaya almak veya iptal etmek için sürüm
...
- Paket sahibi, yayımcı üyesi, moderatör veya yönetici kullanıcı için API
...
npm ayna istemcileri için tam olarak yüklenmiş ClawPack tarball baytlarını akışla aktarır.

Notlar:

- İndirme hız kovasını kullanır.
- İndirme üst bilgileri, ClawHub SHA-256 ile birlikte npm integrity/shasum meta verilerini içerir.
...
Barındırılan bir beceri sürümü ZIP dosyasını indirir veya `clean` ya da
`suspicious` taramasına sahip ve barındırılan sürümü olmayan güncel GitHub
destekli bir beceri için GitHub kaynak devri döndürür.
...
- GitHub destekli beceri devirleri baytları proxy'lemez veya aynalamaz. JSON
  yanıtı `sourceRef: "public-github"`, `repo`, `commit`, `path`, `contentHash`
  ve `archiveUrl` içerir; tarama/güncel durum bir geçittir ve başarı yükü meta
  verisi olarak dahil edilmez.
- İndirme istatistikleri UTC günü başına benzersiz kimlikler olarak sayılır (`userId`, API belirteci geçerliyse; aksi halde IP).
...
- İsteğe bağlı yük alanı: `migrateOwner`. `ownerHandle` ile `true` olduğunda,
  aktör hem mevcut hem de hedef yayımcılarda yönetici/sahip ise mevcut bir
  beceri o sahibe taşınabilir. Bu açık onay olmadan sahip değişiklikleri reddedilir.
...
- İzin verilen form alanları `payload`, yinelenen `files` blob'ları veya bir
  ...
  akışı tarafından döndürülen bir depolama kimliği olabilir. Aşamaya alınmış
  storage-id yayımları, o yükleme URL'siyle döndürülen `clawpackUploadTicket`
  değerini de içermelidir.
- Aynı istekte `files` veya `clawpack` kullanın; ikisini birlikte asla kullanmayın.
...
- Adına yapılan yayımlar, resmi kanal uygunluğunu yine de hedef sahip hesabına göre doğrular.

### `DELETE /api/v1/skills/{slug}` / `POST /api/v1/skills/{slug}/undelete`

Bir beceriyi geçici siler / geri yükler (sahip, moderatör veya yönetici).
...
eski paylaşılan kullanıcı/kişisel yayımcıyı gösteriyorsa, uç nokta önce onu kuruluş yayımcısına
geçirir. Yeni oluşturulan bir kuruluş için `memberHandle` sağlayın; işlemi yapan yönetici üye
olarak eklenmez. `memberRole` varsayılan olarak `owner` olur.
...
Kimliği doğrulanmış kendi kendine hizmet kuruluş yayımcısı oluşturma. Yeni bir kuruluş yayımcısı
oluşturur ve çağıranı sahip olarak ekler. Bu uç nokta mevcut kullanıcı/kişisel tanıtıcıları
geçirmez ve yayımcıyı güvenilir/resmi olarak işaretlemez.
...
sonra gerçek code-plugin veya bundle-plugin sürümünü o ada yayımlayabilir.
...
Yalnızca yönetici. Convex Auth hesap satırlarını düzenlemeden, doğrulanmış yedek GitHub OAuth
sorumlusu için kişisel yayımcıyı kurtarır. İstek, değişmez GitHub sağlayıcı hesap kimliklerinin
ikisini de adlandırmalıdır; değişebilir tanıtıcılar yalnızca operatöre yönelik bir koruma olarak kullanılır.

Uç nokta varsayılan olarak kuru çalıştırma modundadır. Kurtarmayı uygulamak, personelin her iki
...
yayıncısında beceriler, paketler veya GitHub beceri kaynakları varsa kurtarma kapalı durumda başarısız olur.
Kurtarma ayrıca, doğrudan sahip yollarının yeni yayıncı yetkisiyle uyumlu olması için kurtarılan yayıncının becerileri,
beceri slug takma adları, paketleri, paket inceleyici uyarıları ve türetilmiş arama özeti satırları için eski
...
değiştirin (yalnızca yönetici). `dryRun`, `false` olmadığı sürece varsayılan olarak kuru çalıştırmadır.
...
Daha eski CLI sürümleri için hâlâ desteklenir:
...
`POST /api/cli/upload-url`, `uploadUrl` ve `uploadTicket` döndürür. Bir ClawPack tarball dosyasını
hazırlayan paket yayınları, ortaya çıkan depolama kimliğini `clawpack` olarak
