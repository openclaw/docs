...
x-i18n:
    generated_at: "2026-07-02T01:09:03Z"
    model: gpt-5.5
...
Üçüncü taraf dizinler, ClawHub becerilerini listelemek veya aramak için herkese açık okuma uç noktalarını kullanabilir. Lütfen sonuçları önbelleğe alın, `429`/`Retry-After` değerlerine uyun, kullanıcıları standart ClawHub listesine (`https://clawhub.ai/<owner>/skills/<slug>`) geri bağlayın ve ClawHub’ın üçüncü taraf siteyi onayladığı izlenimini vermekten kaçının. Gizli, özel veya moderasyon tarafından engellenmiş içerikleri herkese açık API yüzeyinin dışında yansıtmaya çalışmayın.

Web slug kısayolları kayıt aileleri genelinde çözümlenir, ancak API istemcileri rota
önceliğini yeniden oluşturmak yerine okuma uç noktalarının döndürdüğü
standart URL’leri kullanmalıdır.
...
- Kimliği doğrulanmış yazma uç noktaları, sunucu nedeni biliyorsa çıplak bir `Unauthorized`
  döndürmemelidir. Eksik tokenlar, geçersiz/iptal edilmiş tokenlar ve
  silinmiş/yasaklanmış/devre dışı bırakılmış hesaplar, CLI
  istemcilerinin kullanıcılara kendilerini neyin engellediğini söyleyebilmesi için işlem yapılabilir metin almalıdır.
...
- Senkronize yeniden denemeleri önlemek için jitter uygulanmış geri çekilme kullanın.
...
- Güvenilir istemci IP’si yoksa, anonim istekler yalnızca hız sınırı türüne
  göre kapsamlandırılmış yedek kovalari kullanır. Bu yedek kovalar,
  çağıranın sağladığı yolları, slug’ları, paket adlarını, sürümleri, sorgu dizelerini veya diğer
...
- Sonuçlar alaka sırasına göre döndürülür (embedding benzerliği + tam slug/ad token artırımları + küçük bir popülerlik önceliği).
...
- ASCII metin, sözcük ve noktalama sınırlarında token’lara ayrılır. Örneğin `personal-map` bağımsız bir `map` token’ı içerirken, `amap-jsapi-skill` `amap`, `jsapi` ve `skill` içerir; bu nedenle `map` aramak, `personal-map` için `amap-jsapi-skill` değerinden daha güçlü bir sözcüksel eşleşme verir.
- Popülerlik log ölçeklidir ve sınırlandırılmıştır. Yüksek etkileşimli beceriler, sorgu metni daha zayıf eşleştiğinde daha düşük sıralanabilir.
...
- Kullanıcıların gerçekten arayacağı terimleri görünen ada, özete ve etiketlere koyun. Bağımsız bir slug token’ını yalnızca korumak istediğiniz kararlı bir kimlikse kullanın.
- Yeni slug daha iyi bir uzun vadeli standart ad değilse, yalnızca tek bir sorgunun peşinden gitmek için slug’ı yeniden adlandırmayın. Eski slug’lar yönlendirme takma adları olur, ancak standart URL, görüntülenen slug ve gelecekteki arama özetleri yeni slug’ı kullanır.
- Yeniden adlandırma takma adları, eski URL’ler ve kayıt üzerinden çözümlenen kurulumlar için çözümlemeyi korur, ancak arama sıralaması, yeniden adlandırma dizine eklendikten sonra standart beceri meta verilerine dayanır. Mevcut istatistikler beceriyle kalır.
- Bir beceri beklenmedik şekilde görünmezse, sıralamayla ilgili meta verileri değiştirmeden önce oturum açmışken `clawhub inspect @owner/slug` ile önce moderasyon durumunu kontrol edin.
...
- `nonSuspiciousOnly=true` olduğunda, imleç tabanlı sıralamalar sayfa alındıktan sonra şüpheli beceriler filtrelendiği için bir sayfada `limit` değerinden daha az öğe döndürebilir.
...
- `moderation` yalnızca beceri işaretlendiğinde veya sahibi görüntülediğinde dahil edilir.
...
ayarlanırken atlanabilir. Aynı denetlenebilir iş akışında beceriyi gizlemek için triyaj edilmiş
...
- `moderation`, en son sürümden türetilmiş güncel beceri düzeyinde bir moderasyon anlık görüntüsüdür.
- Geçmiş bir sürüm sorgulanırken, `moderation` ve `security` alanlarını aynı sürüm bağlamı olarak değerlendirmeden önce `moderation.matchesRequestedVersion` ve `moderation.sourceVersion` alanlarını kontrol edin.
...
- Beceriye veya Plugin’e sahip/yayımcı yönetim erişimi ya da platform moderatörü/yönetici yetkisi gerektirir.
- Engellenmiş veya gizlenmiş sürümler dahil olmak üzere, tam gönderilmiş sürüm için saklanan tarama sonuçlarını döndürür.
...
- `ok`, yalnızca seçilen sürümde oluşturulmuş bir Beceri Kartı olduğunda, moderasyon tarafından zararlı yazılım nedeniyle engellenmediğinde ve ClawScan doğrulaması temiz olduğunda `true` olur.
...
Tam beceri sürümleri için güncel kompakt güvenlik kararlarını döndürür. Bu koleksiyon uç noktası, OpenClaw Control UI gibi, hangi yüklü ClawHub beceri sürümlerini görüntülemeleri gerektiğini zaten bilen istemciler için tasarlanmıştır.
...
- Sonuçlar öğe bazındadır; eksik bir beceri veya sürüm tüm yanıtı başarısız kılmaz.
- Yanıt yalnızca güvenlikle ilgilidir. Beceri Kartı verilerini, oluşturulan kart durumunu, yapıt dosya listelerini veya ayrıntılı tarayıcı yüklerini içermez.
...
- `security.signals.dependencyRegistry`, v1 yanıt uyumluluğu için korunur, ancak bağımlılık kayıt defteri varlığı tarayıcısı kullanımdan kaldırılmıştır ve bu anahtar her zaman `null` olur.
- Beceri Kartı’nın olmaması bu uç noktanın `ok`, `decision` veya `reasons` değerlerini etkilemez; istemciler kart içeriğine ihtiyaç duyduklarında yüklü `skill-card.md` dosyasını yerel olarak okumalıdır.
- Tek becerili Beceri Kartı doğrulama zarfına ihtiyacınız olduğunda `/verify`, oluşturulmuş kart Markdown’ına ihtiyacınız olduğunda `/card`, ayrıntılı tarayıcı verilerine ihtiyacınız olduğunda `/scan` kullanın.
...
- `category` (isteğe bağlı): plugin kategori filtresi. Yalnızca istek plugin paketleriyle sınırlandırıldığında desteklenir (`/api/v1/plugins`,
  `/api/v1/code-plugins`, `/api/v1/bundle-plugins` veya `family=code-plugin`/`family=bundle-plugin` içeren paket uç noktaları). Kontrollü kategoriler ve
...
- Skill girişleri skill registry tarafından desteklenmeye devam eder ve hâlâ yalnızca `POST /api/v1/skills` üzerinden yayımlanabilir.
...
- Barındırılan skill'ler en son saklanan sürüm dosyalarını içerir ve
...
- `clean` veya `suspicious` taramasına sahip mevcut GitHub destekli skill'ler
  `_source_handoff.json` içinde `sourceRef: "public-github"`, depo, commit, path,
  içerik hash'i ve arşiv URL'si ile yer alır. ClawHub'da barındırılan kaynak dosyalarını içermezler.
- Her skill `_export_skill_meta.json` içerir.
- `_manifest.json` her zaman ZIP kökünde bulunur.
- Tekil skill'ler veya dosyalar dışa aktarılamadığında `_errors.json` eklenir.
...
- `version.sha256hash`, eski istemciler için kullanımdan kaldırılmış uyumluluk
  ...
  ZIP baytlarını hash'ler. Modern istemciler, kanonik sürüm artifact'ini
  tanımlayan `version.artifact.sha256` kullanmalıdır.
...
- `release.artifactKind`, `release.artifactSha256`, `release.npmIntegrity`,
  `release.npmShasum` ve `release.npmTarballName`, sürüm artifact'i için
...
- `trust.blockedFromDownload`, kurulum engelleme sinyalidir. OpenClaw ve diğer
  ...
  engellemelidir.
- `trust.reasons`, kullanıcıya dönük ve denetim açıklama listesidir. Neden
...
- `trust.stale`, güven özetinin güncel olmayan girdilerden hesaplandığı ve
  yüksek güvenli izin kararı öncesinde yenileme gerektiriyor kabul edilmesi
...
- Eski paket sürümleri bir `legacy-zip` artifact'i ve eski ZIP `downloadUrl`
  döndürür.
- ClawPack sürümleri bir `npm-pack` artifact'i, npm bütünlük alanları, bir
...
Açık çözücü yolu üzerinden sürüm artifact'ini indirir.
...
- `packageName` npm adına göre normalize edilir; planlanan geçişlerde paket
  eksik olabilir.
...
- `open`: şüpheli, kötü amaçlı, beklemede, karantinaya alınmış, iptal edilmiş veya bildirilen sürümler.
...
moderatörler artifact'leri onaylamak, karantinaya almak veya iptal etmek için
...
- Bir moderatör veya yönetici kullanıcı için API belirteci gerektirir.
...
- Paket sahibi, yayıncı üyesi, moderatör veya yönetici kullanıcı için API belirteci
  gerektirir.
...
ayarlanırken atlanabilir. Sürüm moderasyonunu aynı denetlenebilir iş akışı içinde
...
- Yalnızca kayıt defteri metaverileri indirilen arşive enjekte edilmez.
...
- İndirme hız kovasını kullanır.
- İndirme başlıkları, ClawHub SHA-256 ile npm integrity/shasum metaverilerini içerir.
...
taraması olan ve barındırılan sürümü olmayan güncel GitHub destekli bir beceri
...
- GitHub destekli beceri devirleri baytları proxy'lemez veya aynalamaz. JSON yanıtı
  ...
  `archiveUrl` içerir; tarama/geçerli durum bir kapıdır ve başarı yükü metaverisi
...
- İsteğe bağlı yük alanı: `ownerHandle`. Varsa API bu yayıncıyı sunucu tarafında
  çözer ve aktörün yayıncı erişimine sahip olmasını gerektirir.
- İsteğe bağlı yük alanı: `migrateOwner`. `ownerHandle` ile `true` olduğunda,
  ...
  beceri o sahibe taşınabilir. Bu açık onay olmadan sahip değişiklikleri
  reddedilir.
...
- İzin verilen form alanları `payload`, tekrarlanan `files` blob'ları veya tek bir
  ...
  akışı tarafından döndürülen bir depolama kimliği olabilir. Aşamalı depolama kimliği
  yayımları, o yükleme URL'siyle döndürülen `clawpackUploadTicket` değerini de
  içermelidir.
- Aynı istekte ya `files` ya da `clawpack` kullanın, ikisini birlikte kullanmayın.
- JSON gövdeleri ve çağıran tarafından sağlanan `payload.files` / `payload.artifact`
  metaverileri reddedilir.
...
- Code plugin'ler `package.json`, kaynak depo metaverileri, kaynak commit
  metaverileri, yapılandırma şeması metaverileri, `openclaw.compat.pluginApi` ve
  `openclaw.build.openclawVersion` gerektirir.
- `openclaw.hostTargets` ve `openclaw.environment` isteğe bağlı metaverilerdir.
...
- Başkası adına yayımlar, official-channel uygunluğunu yine de hedef sahip hesabına göre doğrular.

### `DELETE /api/v1/skills/{slug}` / `POST /api/v1/skills/{slug}/undelete`

Bir beceriyi geçici sil / geri yükle (sahip, moderatör veya yönetici).
...
Varsa `reason`, beceri moderasyon notu olarak saklanır ve denetim günlüğüne kopyalanır.
...
eski paylaşılan kullanıcı/kişisel yayıncıyı gösteriyorsa uç nokta önce onu bir kuruluş
...
Yalnızca yönetici. Kök slug'ları ve paket adlarını bir sürüm yayımlamadan hak sahibi için ayırır.
...
Yalnızca yönetici. Convex Auth hesap satırlarını düzenlemeden, doğrulanmış yedek GitHub OAuth kimliği
için kişisel yayıncıyı kurtarır. İstek, iki değişmez GitHub sağlayıcı hesap kimliğini de
belirtmelidir; değişebilir tanıtıcılar yalnızca operatöre yönelik bir koruma olarak kullanılır.

Uç nokta varsayılan olarak kuru çalıştırma modundadır. Kurtarmayı uygulamak, personel her iki
GitHub kimliği arasında sürekliliği bağımsız olarak doğruladıktan sonra `dryRun: false` ve
...
paket denetleyici uyarıları ve türetilmiş arama özeti satırları için eski `ownerUserId` alanlarını
taşır; böylece doğrudan sahip yolları yeni yayıncı yetkisiyle uyumlu olur. Kurtarılan handle için
etkin bir korumalı handle rezervasyonu da yedek kullanıcıya yeniden atanır; böylece sonraki
...
Bir kullanıcıyı yasaklar ve sahip olunan skills öğelerini kalıcı olarak siler (yalnızca moderatör/yönetici).
...
Mevcut bir yasağın saklanan gerekçesini, yasağı kaldırmadan veya içeriği geri yüklemeden
değiştirir (yalnızca yönetici). `dryRun`, `false` olmadığı sürece varsayılan olarak kuru çalıştırma modundadır.
...
Bir yıldız (vurgular) ekler/kaldırır. Her iki uç nokta da idempotenttir.
...
`POST /api/cli/upload-url`, `uploadUrl` ve `uploadTicket` döndürür. ClawPack tarball aşamalayan paket
