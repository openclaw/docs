...
x-i18n:
    generated_at: "2026-07-04T15:28:44Z"
    model: gpt-5.5
...
Üçüncü taraf dizinler, ClawHub Skills listelemek veya aramak için genel okuma uç noktalarını kullanabilir. Lütfen sonuçları önbelleğe alın, `429`/`Retry-After` değerlerine uyun, kullanıcıları kurallı ClawHub listesine (`https://clawhub.ai/<owner>/skills/<slug>`) geri bağlayın ve ClawHub'ın üçüncü taraf siteyi onayladığı izlenimini vermekten kaçının. Gizli, özel veya moderasyon tarafından engellenmiş içeriği genel API yüzeyinin dışında yansıtmaya çalışmayın.
...
kurallı URL'leri kullanmalıdır.
...
- Kimliği doğrulanmış yazma uç noktaları, sunucu nedeni bildiğinde çıplak bir `Unauthorized`
  ...
  silinmiş/yasaklanmış/devre dışı bırakılmış hesapların her biri, CLI istemcilerinin
  kullanıcılara onları neyin engellediğini söyleyebilmesi için eyleme geçirilebilir metin almalıdır.
...
- Eşzamanlı yeniden denemelerden kaçınmak için titreşimli geri çekilme kullanın.
...
- Güvenilir istemci IP'si yoksa, anonim istekler yalnızca oran sınırı türüyle
  kapsamlandırılmış yedek kovalari kullanır. Bu yedek kovalar, çağıranın sağladığı
...
izin hataları (`401`/`403`), oran sınırları (`429`) ve engellenmiş indirmeler dahildir. İstemciler
...
- ASCII metin, kelime ve noktalama sınırlarında belirteçlere ayrılır. Örneğin `personal-map` bağımsız bir `map` belirteci içerirken, `amap-jsapi-skill` `amap`, `jsapi` ve `skill` içerir; bu nedenle `map` aramak `personal-map` için `amap-jsapi-skill` değerinden daha güçlü bir sözcüksel eşleşme verir.
...
- Yeni slug daha iyi bir uzun vadeli kurallı ad değilse, yalnızca bir sorguyu yakalamak için slug adını değiştirmeyin. Eski slug'lar yönlendirme takma adlarına dönüşür, ancak kurallı URL, görüntülenen slug ve gelecekteki arama özetleri yeni slug'ı kullanır.
- Yeniden adlandırma takma adları, kayıt üzerinden çözümlenen eski URL'ler ve kurulumlar için çözümlemeyi korur, ancak arama sıralaması, yeniden adlandırma dizine alındıktan sonra kurallı Skill meta verilerine dayanır. Mevcut istatistikler Skill ile kalır.
...
- `nonSuspiciousOnly=true` olduğunda, imleç tabanlı sıralamalar bir sayfada `limit` öğesinden daha az öğe döndürebilir, çünkü şüpheli Skills sayfa alımından sonra filtrelenir.
...
- `moderation` yalnızca Skill işaretliyse veya sahibi onu görüntülüyorsa dahil edilir.
...
- Kanıt genel çağıranlar için redakte edilir ve yalnızca sahipler/moderatörler için ham parçaları içerir.
...
ayarlanırken atlanabilir. Aynı denetlenebilir iş akışında Skill'i gizlemek için sınıflandırılmış
...
- `moderation`, en son sürümden türetilmiş güncel Skills düzeyi denetim anlık görüntüsüdür.
...
- Tarama isteği yükleri ve indirilebilir raporlar, saklama penceresinden sonra tarama isteği deposundan süre aşımına uğrar.
...
- Tarama işleri eşzamansızdır. Manuel tarama istekleri normal yayımlama/geri doldurma işlerinden önce önceliklendirilir, ancak tamamlanma yine de çalışan kullanılabilirliğine bağlıdır.
...
Gönderilen sürümler için kimlik doğrulamalı saklanan rapor arşivi uç noktası.

- Skills veya Plugin için sahip/yayıncı yönetim erişimi ya da platform moderatörü/yönetici yetkisi gerektirir.
- Engellenmiş veya gizli sürümler dahil olmak üzere tam gönderilen sürüm için saklanan tarama sonuçlarını döndürür.
...
- `ok`, yalnızca seçilen sürüm oluşturulmuş bir Skill Card’a sahip olduğunda, denetim tarafından kötü amaçlı yazılım nedeniyle engellenmediğinde ve ClawScan doğrulaması temiz olduğunda `true` olur.
- Skills kimliği, yayıncı kimliği ve seçilen sürüm meta verileri üst düzey zarf alanlarıdır (`slug`, `displayName`, `publisherHandle`, `version`, `resolvedFrom`, `tag`, `createdAt`); böylece kabuk otomasyonu bunları iç içe sarmalayıcıları açmadan okuyabilir.
...
Tam Skills sürümleri için güncel kompakt güvenlik kararlarını döndürür. Bu koleksiyon uç noktası, OpenClaw Control UI gibi hangi yüklü ClawHub Skills sürümlerini göstermesi gerektiğini zaten bilen istemciler için tasarlanmıştır.
...
- Sonuçlar öğe bazındadır; eksik bir Skills veya sürüm tüm yanıtı başarısız yapmaz.
- Yanıt yalnızca güvenlik içindir. Skill Card verilerini, oluşturulmuş kart durumunu, yapıt dosyası listelerini veya ayrıntılı tarayıcı yüklerini içermez.
...
- Tek Skills için Skill Card doğrulama zarfına ihtiyacınız olduğunda `/verify`, oluşturulmuş kart Markdown’ına ihtiyacınız olduğunda `/card` ve ayrıntılı tarayıcı verilerine ihtiyacınız olduğunda `/scan` kullanın.
...
- `category` (isteğe bağlı): plugin kategori filtresi. Yalnızca istek
  ...
  sınırlandırıldığında desteklenir. Kontrollü kategoriler ve eski v1
...
- Kimliği doğrulanmış çağıranlar, liste/arama sonuçlarında bağlı oldukları yayımcıların özel paketlerini görebilir.
...
- `isOfficial` (isteğe bağlı): `true` veya `false`
- `category` (isteğe bağlı): plugin kategori filtresi. Yalnızca istek
  plugin paketleriyle sınırlandırıldığında desteklenir. Kontrollü kategoriler
...
- Anonim çağıranlar yalnızca herkese açık paket kanallarını görür.
- Kimliği doğrulanmış çağıranlar, bağlı oldukları yayımcıların özel paketlerinde arama yapabilir.
- `channel=private` yalnızca kimliği doğrulanmış çağıranın okuyabildiği paketleri döndürür.
...
Birleşik `/api/v1/packages` uç noktasında yalnızca pluginler içindir; skill kataloğu için
...
- Barındırılan skills, depolanan en son sürüm dosyalarını içerir ve
...
- Plugin başına dışa aktarma metaverisi
  `__clawhub_export/{family}/{packageName}/plugin_meta.json` konumunda depolanır.
- `_manifest.json` her zaman ZIP kökünde bulunur.
- Tekil Plugin'ler veya dosyalar dışa aktarılamadığında `_errors.json` dahil edilir.
...
Paket ayrıntı metaverisini döndürür.
...
- Paket sahibi, kuruluş yayıncısı sahibi/yöneticisi, platform moderatörü veya platform yöneticisi için API token'ı gerektirir.
...
Dosya metaverisi, uyumluluk, doğrulama, yapıt metaverisi ve tarama verileri dahil olmak üzere tek bir paket sürümünü döndürür.
...
- `version.sha256hash`, eski istemciler için kullanımdan kaldırılmış uyumluluk metaverisidir.
...
Kurulum istemcileri için tam paket sürümü güvenlik ve güven özetini döndürür. Bu, çözümlenen bir sürümün kurulup kurulamayacağına karar vermek için herkese açık OpenClaw tüketim yüzeyidir.
...
- `package.name`, `package.displayName` ve `package.family`, çözümlenen kayıt paketini tanımlar.
- `release.releaseId`, `release.version` ve `release.createdAt`, değerlendirilen tam sürümü tanımlar.
- `release.artifactKind`, `release.artifactSha256`, `release.npmIntegrity`,
  `release.npmShasum` ve `release.npmTarballName`, sürüm yapıtı için bilindiğinde bulunur.
- `trust.scanStatus`, tarayıcı girdilerinden ve manuel sürüm moderasyonundan türetilen etkili güven durumudur.
...
- `trust.blockedFromDownload` kurulum engelleme sinyalidir. OpenClaw ve diğer
  kurulum istemcileri, tarayıcı veya moderasyon alanlarından engelleme kurallarını yeniden türetmek yerine
  bu değer `true` olduğunda kurulumu engellemelidir.
- `trust.reasons`, kullanıcıya dönük ve denetim açıklama listesidir. Neden kodları
  `manual:quarantined`, `scan:malicious` ve `package:malicious` gibi kararlı, kompakt dizelerdir.
...
- `trust.stale`, güven özetinin güncel olmayan girdilerden hesaplandığı ve
  yüksek güvenli izin kararı öncesinde yenileme gerektiriyor olarak ele alınması gerektiği anlamına gelir.

Notlar:

- Bu uç nokta sürüme özeldir. İstemciler, yalnızca en son paket metaverisini okuduktan sonra değil,
  kurmayı amaçladıkları paket sürümünü çözdükten sonra bunu çağırmalıdır.
- Özel paketler, çağıran sahip yayıncıyı okuyamadığı sürece `404` döndürür.
- Bu uç nokta, sahip/moderatör moderasyon uç noktalarından kasıtlı olarak daha dardır.
  Muhabir kimliklerini, rapor gövdelerini, özel kanıtları veya dahili inceleme
  zaman çizelgelerini değil, kurulum kararını ve herkese açık açıklamayı gösterir.

### `GET /api/v1/packages/{name}/versions/{version}/artifact`

Bir paket sürümü için açık yapıt çözümleyici metaverisini döndürür.

Notlar:

- Eski paket sürümleri bir `legacy-zip` yapıtı ve eski ZIP
  `downloadUrl` döndürür.
- ClawPack sürümleri bir `npm-pack` yapıtı, npm bütünlük alanları,
  bir `tarballUrl` ve eski ZIP uyumluluk URL'si döndürür.
...
- `packageName` npm-name normalize edilir; planlanan geçişler için paket eksik olabilir.
...
- Varsayılan olarak en son sürümü kullanır.
- İndirme kotası yerine okuma kota grubunu kullanır.
- İkili dosyalar `415` döndürür.
- Dosya boyutu sınırı: 200KB.
- Bekleyen VirusTotal taramaları okumaları engellemez; kötü amaçlı sürümler başka yerde yine de saklı tutulabilir.
- Özel paketler, çağıran sahip yayıncıyı okuyamadığı sürece `404` döndürür.
...
npm ayna istemcileri için yüklenen ClawPack tarball baytlarını aynen akış olarak sunar.

Notlar:

- İndirme kota grubunu kullanır.
- İndirme üst bilgileri, ClawHub SHA-256 ile npm integrity/shasum metadata'sını içerir.
- Moderasyon ve özel paket erişim kontrolleri hâlâ uygulanır.
...
- `slug` (gerekli)
- `hash` (gerekli): bundle parmak izinin 64 karakterlik hex sha256 değeri
...
taramaya sahip ve barındırılan sürümü olmayan mevcut GitHub destekli bir skill
için GitHub kaynak devrini döndürür.
...
- GitHub destekli skill devirleri baytları proxy'lemez veya aynalamaz. JSON
  ...
  ve `archiveUrl` içerir; tarama/geçerli durum bir geçittir ve başarı yükü
...
- İsteğe bağlı payload alanı: `migrateOwner`. `ownerHandle` ile birlikte `true`
  olduğunda, aktör hem geçerli hem de hedef yayıncılarda yönetici/sahip ise
  mevcut bir skill bu sahibe taşınabilir. Bu açık katılım olmadan sahip
...
- Aynı istekte asla ikisini birden değil, ya `files` ya da `clawpack` kullanın.
- JSON gövdeleri ve çağıran tarafından sağlanan `payload.files` /
  `payload.artifact` metadata'sı reddedilir.
- Doğrudan multipart yayımlama istekleri 18MB ile sınırlandırılmıştır. ClawPack
...
- Adına yapılan yayımlar yine de resmi kanal uygunluğunu hedef sahip hesabına göre doğrular.

### `DELETE /api/v1/skills/{slug}` / `POST /api/v1/skills/{slug}/undelete`

Bir skill'i yumuşak siler / geri yükler (sahip, moderatör veya yönetici).
...
Yalnızca yönetici. Kök slug'ları ve paket adlarını, sürüm yayımlamadan hak sahibi için rezerve eder.
Paket adları, sürüm satırı olmayan özel yer tutucu paketlere dönüşür; böylece aynı
...
için kişisel bir yayıncıyı kurtarır. İstek, değiştirilemez GitHub sağlayıcı hesap kimliklerinin
ikisini de adlandırmalıdır; değiştirilebilir handle'lar yalnızca operatöre dönük bir koruma olarak kullanılır.

Uç nokta varsayılan olarak dry-run kullanır. Kurtarmayı uygulamak, personelin her iki
GitHub öznesi arasındaki sürekliliği bağımsız olarak doğrulamasından sonra `dryRun: false` ve
`confirmIdentityVerified: true` gerektirir. Hedef kullanıcının mevcut kişisel
yayıncısında skills, paketler veya GitHub skill kaynakları varsa kurtarma kapalı başarısız olur.
Kurtarma ayrıca, doğrudan sahip yollarının yeni yayıncı yetkisiyle uyumlu olması için kurtarılan yayıncının skills,
skill slug takma adları, paketleri, paket denetleyicisi uyarıları ve türetilmiş arama özeti satırları için eski `ownerUserId` alanlarını da taşır. Kurtarılan handle için etkin bir korumalı handle
...
Bir kullanıcıyı yasaklayın ve sahip olunan skills'i kalıcı olarak silin (yalnızca moderatör/yönetici).
...
Bir kullanıcının yasağını kaldırın ve uygun skills'i geri yükleyin (yalnızca yönetici).
...
Yasağı kaldırmadan veya içeriği geri yüklemeden mevcut bir yasaklama için depolanan nedeni değiştirin
