...
x-i18n:
    generated_at: "2026-07-01T15:28:58Z"
    model: gpt-5.5
...
Üçüncü taraf dizinler, ClawHub Skills öğelerini listelemek veya aramak için herkese açık okuma uç noktalarını kullanabilir. Lütfen sonuçları önbelleğe alın, `429`/`Retry-After` değerlerine uyun, kullanıcıları kanonik ClawHub listesine (`https://clawhub.ai/<owner>/skills/<slug>`) geri bağlayın ve ClawHub'ın üçüncü taraf siteyi desteklediği izlenimini vermekten kaçının. Herkese açık API yüzeyi dışında gizli, özel veya moderasyon tarafından engellenmiş içeriği yansıtmaya çalışmayın.
...
- Kimliği doğrulanmış istekler (geçerli Bearer token): kullanıcı kovası başına zorlanır.
...
- ClawHub, uçta istemci IP'lerini belirlemek için güvenilir iletme başlıkları kullanır.
- Güvenilir bir istemci IP'si yoksa, anonim istekler yalnızca hız sınırı türüne
  göre kapsamlanmış yedek kovalar kullanır. Bu yedek kovalar, çağıran tarafından sağlanan
...
- Alaka popülerlikten daha güçlüdür. Kesin bir slug veya görünen ad token eşleşmesi, çok daha güçlü etkileşime sahip daha gevşek bir eşleşmeyi geride bırakabilir.
- ASCII metin, sözcük ve noktalama sınırlarında tokenlere ayrılır. Örneğin, `personal-map` bağımsız bir `map` tokeni içerirken `amap-jsapi-skill`, `amap`, `jsapi` ve `skill` içerir; bu nedenle `map` araması `personal-map` için `amap-jsapi-skill` değerinden daha güçlü bir sözlüksel eşleşme sağlar.
- Popülerlik log ölçeklidir ve sınırlandırılır. Sorgu metni daha zayıf bir eşleşmeyse yüksek etkileşimli Skills öğeleri daha düşük sıralanabilir.
...
- Kullanıcıların kelimesi kelimesine arayacağı terimleri görünen ada, özete ve etiketlere koyun. Bağımsız bir slug tokenini yalnızca korumak istediğiniz kararlı bir kimlikse kullanın.
- Yeni slug daha iyi bir uzun vadeli kanonik ad değilse yalnızca tek bir sorguyu kovalamak için slug'ı yeniden adlandırmayın. Eski slug'lar yönlendirme takma adları olur, ancak kanonik URL, görüntülenen slug ve gelecekteki arama özetleri yeni slug'ı kullanır.
- Yeniden adlandırma takma adları, kayıt üzerinden çözümlenen eski URL'ler ve kurulumlar için çözümlemeyi korur, ancak arama sıralaması yeniden adlandırma dizine eklendikten sonra kanonik Skill metaverilerine dayanır. Mevcut istatistikler Skill ile kalır.
- Bir Skill beklenmedik şekilde görünmüyorsa, sıralamayla ilgili metaverileri değiştirmeden önce oturum açmışken `clawhub inspect @owner/slug` ile önce moderasyon durumunu kontrol edin.
...
- `createdAt`, yeni Skill taramaları için kararlıdır; `updated`, mevcut Skills öğeleri yeniden yayınlandığında değişir.
...
- Mevcut olduğunda sayfalandırmaya devam etmek için `nextCursor` kullanın. Kısa bir sayfa tek başına sonuçların bittiği anlamına gelmez.
...
- Skill'in platform metaverisi yoksa `metadata` `null` olur.
...
`note`, `confirmed` ve `dismissed` için zorunludur; `status` değerini tekrar
`open` olarak ayarlarken atlanabilir. Aynı denetlenebilir iş akışında Skill'i
gizlemek için önceliklendirilmiş bir bildirimle `finalAction: "hide"` geçirin.
...
Sürüm metaverilerini + dosya listesini döndürür.
...
- `moderation`, en son sürümden türetilmiş güncel beceri düzeyi moderasyon anlık görüntüsüdür.
...
- Tarama işleri eşzamansızdır. Manuel tarama istekleri normal yayımlama/geri doldurma işlerinden önce önceliklendirilir, ancak tamamlanma yine de işçi kullanılabilirliğine bağlıdır.
...
- Engellenmiş veya gizlenmiş sürümler dahil olmak üzere, tam olarak gönderilen sürüm için depolanmış tarama sonuçlarını döndürür.
...
- `provenance`, yalnızca ClawHub yayımlama veya içe aktarma sırasında bir GitHub repo/ref/commit/path çözüp depoladığında `server-resolved-github-import` olur; aksi takdirde `unavailable` olur.
...
- Varsayılan olarak en son sürüme ayarlanır.
...
- `category` (isteğe bağlı): Plugin kategori filtresi. Yalnızca istek
  ...
  sınırlandırıldığında desteklenir. Denetimli kategoriler ve eski v1
...
`trending`, yedi günlük kurulum/indirme liderlik sıralamasıdır ve tüm zamanların toplamlarını kullanmaz.
...
- `startDate` (gerekli): Skill `updatedAt` için Unix milisaniyesi cinsinden alt sınır.
- `endDate` (gerekli): Skill `updatedAt` için Unix milisaniyesi cinsinden üst sınır.
...
- Gövde: ZIP arşivi.
- Dışa aktarılan her Skill'in kökü `{publisher}/{slug}/` olur.
...
- `clean` veya `suspicious` taraması olan mevcut GitHub destekli Skills,
  `sourceRef: "public-github"` ile `_source_handoff.json` dosyasını, depoyu, commit'i, yolu,
  içerik karmasını ve arşiv URL'sini içerir. ClawHub'da barındırılan kaynak dosyalarını içermez.
...
- Plugin araması için tarayıcı kullanıcı arayüzü sıralama kontrolleri, yüklenen
  alaka sonuçlarını geçerli `/skills` göz atma davranışıyla eşleşecek şekilde yeniden sıralar.
...
- `trust.stale`, güven özetinin güncelliğini yitirmiş girdilerden hesaplandığı ve
  yüksek güvenli bir izin kararından önce yenileme gerektiriyor gibi ele alınması
  gerektiği anlamına gelir.

Notlar:

- Bu uç nokta sürüme özeldir. İstemciler, yalnızca en son paket meta verilerini
  okuduktan sonra değil, kurmayı düşündükleri paket sürümünü çözdükten sonra bunu
  çağırmalıdır.
- Özel paketler, çağıran sahip yayıncıyı okuyamadığı sürece `404` döndürür.
- Bu uç nokta, sahip/moderatör moderasyon uç noktalarından bilerek daha dardır.
  Bildirici kimliklerini, rapor gövdelerini, özel kanıtları veya dahili inceleme
...
- `approved`: el ile incelendi ve izin verildi.
...
npm yansı istemcileri için tam olarak yüklenmiş ClawPack tarball baytlarını akışla gönderir.

Notlar:

- İndirme oranı kovasını kullanır.
- İndirme başlıkları, ClawHub SHA-256'nın yanı sıra npm integrity/shasum meta verilerini içerir.
...
taramasına sahip, barındırılan sürümü olmayan geçerli GitHub destekli bir Skills
...
- GitHub destekli Skills devirleri baytları proxy'lemez veya yansıtmaz. JSON yanıtı
  `sourceRef: "public-github"`, `repo`, `commit`, `path`, `contentHash` ve
  `archiveUrl` içerir; tarama/geçerli durum bir kapıdır ve başarı yükü meta verisi
...
- İsteğe bağlı yük alanı: `migrateOwner`. `ownerHandle` ile `true` olduğunda,
  aktör hem mevcut hem de hedef yayıncılarda admin/sahip ise mevcut bir Skills
  bu sahibe taşınabilir. Bu opt-in olmadan sahip değişiklikleri reddedilir.
...
- İzin verilen form alanları `payload`, tekrarlanan `files` blob'ları veya bir
  ...
  akışı tarafından döndürülen bir depolama kimliği olabilir. Aşamalanmış
...
- İsteğe bağlı yük alanı: `ownerHandle`. Mevcut olduğunda, yalnızca yöneticiler bu sahip adına yayın yapabilir.
...
- Başkası adına yayımlar, hedef sahip hesabına göre resmi kanal uygunluğunu yine doğrular.
...
Sahip tarafından başlatılan yumuşak silmeler slug'ı 30 gün ayırır, ardından slug başka
bir yayıncı tarafından talep edilebilir. Silme yanıtı, bu süre sonu geçerliyse `slugReservedUntil` içerir.
...
Yalnızca yönetici. Convex Auth hesap satırlarını düzenlemeden, doğrulanmış yedek GitHub OAuth sorumlusu için
kişisel bir yayıncıyı kurtarır. İstek, değişmez GitHub sağlayıcı hesap kimliklerinin ikisini de adlandırmalıdır;
...
rezervasyonu da yedek kullanıcıya yeniden atanır; böylece sonraki profil eşitlemesi
eski kullanıcının rakip yetkisini geri yükleyemez. Her birincil tablo, uygulama işlemi başına
...
içeriği geri yüklemeden değiştirir (yalnızca yönetici). `dryRun`, `false` olmadığı sürece varsayılan olarak dry-run modundadır.
...
yayınları, ortaya çıkan depolama kimliğini `clawpack` olarak ve dönen bileti
