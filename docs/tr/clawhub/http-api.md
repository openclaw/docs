...
x-i18n:
    generated_at: "2026-06-28T20:41:19Z"
    model: gpt-5.5
...
- Kimliği doğrulanmış yazma uç noktaları, sunucu nedeni bildiğinde yalın bir `Unauthorized`
  ...
  silinmiş/yasaklanmış/devre dışı bırakılmış hesapların her biri eyleme dönüştürülebilir metin almalıdır; böylece CLI
...
- Eşzamanlı yeniden denemeleri önlemek için jitter içeren backoff kullanın.
...
- Güvenilir istemci IP'si yoksa, anonim istekler yalnızca hız sınırı türüne
  göre kapsamlanmış yedek kovalara gider. Bu yedek kovalar, çağıranın sağladığı
...
- Sonuçlar ilgi sırasına göre döndürülür (embedding benzerliği + tam slug/ad token güçlendirmeleri + küçük bir popülerlik öncülü).
- İlgi, popülerlikten daha güçlüdür. Kesin bir slug veya görüntülenen ad token eşleşmesi, çok daha güçlü etkileşime sahip daha gevşek bir eşleşmenin önüne geçebilir.
- ASCII metin, sözcük ve noktalama sınırlarında token'lara ayrılır. Örneğin `personal-map` bağımsız bir `map` token'ı içerirken, `amap-jsapi-skill` `amap`, `jsapi` ve `skill` içerir; bu nedenle `map` araması `personal-map` için `amap-jsapi-skill` değerinden daha güçlü bir sözlüksel eşleşme verir.
...
- Yeni slug daha iyi uzun vadeli kanonik ad değilse, yalnızca tek bir sorguyu hedeflemek için slug'ı yeniden adlandırmayın. Eski slug'lar yönlendirme takma adları olur, ancak kanonik URL, görüntülenen slug ve gelecekteki arama özetleri yeni slug'ı kullanır.
...
- Herkese açık çağıranlar yalnızca önceden işaretlenmiş görünür Skills için `200` alır.
...
`confirmed` ve `dismissed` için `note` gereklidir; `status` tekrar `open` olarak
ayarlanırken atlanabilir. Değerlendirilmiş bir bildirimle birlikte `finalAction: "hide"` geçirerek
Skill öğesini aynı denetlenebilir iş akışında gizleyin.
...
Tam beceri sürümleri için güncel kompakt güvenlik kararlarını döndürür. Bu koleksiyon uç noktası, OpenClaw Control UI gibi, hangi yüklü ClawHub beceri sürümlerini göstermesi gerektiğini zaten bilen istemciler için tasarlanmıştır.
...
- Yanıt yalnızca güvenlikle ilgilidir. Beceri Kartı verisi, oluşturulmuş kart durumu, yapıt dosya listeleri veya ayrıntılı tarayıcı yükleri içermez.
...
Code-plugin ve bundle-plugin paketleri genelinde yalnızca Plugin kataloğu taraması.
...
- `clean` veya `suspicious` taramasına sahip geçerli GitHub destekli skills,
  `sourceRef: "public-github"` ile `_source_handoff.json`, repo, commit, path,
...
- Tek tek Plugin'ler veya dosyalar dışa aktarılamadığında `_errors.json` eklenir.
...
- Plugin araması için tarayıcı kullanıcı arayüzü sıralama kontrolleri, yüklenen ilgililik sonuçlarını
  mevcut `/skills` gezinme davranışıyla eşleşecek şekilde yeniden sıralar.
...
- Özel paketler, çağıran taraf sahip yayımlayıcıyı okuyamadığı sürece `404` döndürür.
...
- Paket sahibi, kuruluş yayımlayıcı sahibi/yöneticisi, platform moderatörü veya
...
- Herkese açık okuma uç noktasıdır. Sahip, yayımlayıcı, moderatör veya yönetici token'ı
  gerekmez.
...
- `trust.reasons`, kullanıcıya yönelik ve denetim amaçlı açıklama listesidir. Neden kodları
...
- `trust.stale`, güven özetinin güncel olmayan girdilerden hesaplandığı ve
  yüksek güvenli izin kararından önce yenileme gerektiriyor gibi ele alınması gerektiği anlamına gelir.

Notlar:

- Bu uç nokta sürüme tam olarak özeldir. İstemciler, yalnızca en son paket meta verilerini
  okuduktan sonra değil, kurmayı amaçladıkları paket sürümünü çözdükten sonra bunu çağırmalıdır.
- Özel paketler, çağıran taraf sahip yayımlayıcıyı okuyamadığı sürece `404` döndürür.
- Bu uç nokta, sahip/moderatör moderasyon uç noktalarından bilerek daha dardır.
  Kurulum kararını ve herkese açık açıklamayı açığa çıkarır; bildiren kimliklerini,
  rapor gövdelerini, özel kanıtları veya dahili inceleme zaman çizelgelerini açığa çıkarmaz.
...
- Bu OpenClaw çözücü yüzeyidir; paylaşılan bir URL'den arşiv formatı tahmin etmeyi
  önler.
...
- ClawPack sürümleri tam olarak yüklenmiş npm-pack `.tgz` baytlarını akış olarak sunar.
...
- `packageName` npm adı olarak normalleştirilir; paket, planlanan geçişler için eksik olabilir.
- Bu yalnızca geçiş hazırlığını izler. OpenClaw'ı değiştirmez veya
  ClawPack üretmez.
...
bir sürüme bağlanır. Moderasyon kuyruğunu beslerler ancak kendi başlarına indirmeleri
otomatik olarak gizlemez veya engellemezler; moderatörler yapıtları onaylamak,
...
Moderatör/yönetici uç noktası, paket raporu alımı için.
...
Sahip/moderatör uç noktası, paket moderasyon görünürlüğü için.
...
`confirmed` ve `dismissed` için `note` gereklidir; `status` tekrar `open`
...
- Özel paketler, çağıran kişi sahip yayıncıyı okuyamıyorsa `404` döndürür.
...
- Özel paketler, çağıran kişi sahip değilse `404` döndürür.
...
- İndirme hız kovasını kullanır.
- İndirme üstbilgileri ClawHub SHA-256 ile npm integrity/shasum metaverilerini içerir.
...
Barındırılan bir skill sürümü ZIP'ini indirir veya geçerli GitHub destekli,
`clean` ya da `suspicious` taramasına sahip ve barındırılan sürümü olmayan bir
skill için GitHub kaynak devrini döndürür.
...
- İsteğe bağlı yük alanı: `ownerHandle`. Mevcut olduğunda API bu yayıncıyı sunucu
...
- İsteğe bağlı yük alanı: `migrateOwner`. `ownerHandle` ile `true` olduğunda,
  aktör hem mevcut hem hedef yayıncılarda yönetici/sahipse mevcut bir skill o
  sahibe taşınabilir. Bu açık katılım olmadan sahip değişiklikleri reddedilir.
...
- İzin verilen form alanları `payload`, yinelenen `files` blob'ları veya tek bir
  `clawpack` tarball başvurusudur. `clawpack`, `.tgz` blob'u veya upload-url
...
- Code plugin'ler `package.json`, kaynak depo metaverileri, kaynak commit
  metaverileri, config şeması metaverileri, `openclaw.compat.pluginApi` ve
...
başka bir yayıncı tarafından talep edilebilir. Silme yanıtı, bu sona erme geçerliyse `slugReservedUntil` içerir.
...
Yalnızca yönetici. Hak sahibi için kök slug'ları ve paket adlarını bir sürüm yayımlamadan rezerve eder.
...
için kişisel yayıncıyı kurtarır. İstek her iki değişmez GitHub sağlayıcı hesap kimliğini de adlandırmalıdır;
...
GitHub asıl kimliği arasındaki sürekliliği bağımsız olarak doğruladıktan sonra `dryRun: false` ve
`confirmIdentityVerified: true` gerekir. Hedef kullanıcının mevcut kişisel
yayıncısında skills, paketler veya GitHub skill kaynakları varsa kurtarma güvenli kapalı şekilde başarısız olur.
Kurtarma ayrıca, doğrudan sahip yollarının yeni yayıncı yetkisiyle uyumlu olması için kurtarılan yayıncının skills,
skill slug takma adları, paketleri, paket denetleyici uyarıları ve türetilmiş arama özeti satırları için eski `ownerUserId` alanlarını da taşır. Kurtarılan handle için etkin bir korumalı handle
rezervasyonu da yedek kullanıcıya yeniden atanır; böylece sonraki profil senkronizasyonu eski kullanıcının rakip yetkisini geri yükleyemez. Her birincil tablo, uygulama işlemi başına
100 satırla sınırlıdır; daha büyük kurtarmalar önce sürdürülebilir bir sahip migrasyonu kullanmalıdır.
GitHub skill kaynakları yayıncı kapsamlıdır ve yeniden yazılmak yerine denetlenmiş olarak raporlanır.
...
(yalnızca yönetici). `dryRun`, `false` olmadığı sürece varsayılan olarak dry-run kullanır.
...
`POST /api/cli/upload-url`, `uploadUrl` ve `uploadTicket` döndürür. Bir ClawPack tarball öğesini hazırlayan paket
yayınları, ortaya çıkan storage id değerini `clawpack` olarak ve döndürülen ticket değerini `clawpackUploadTicket` olarak göndermelidir.

## Kayıt keşfi (`/.well-known/clawhub.json`)

CLI, siteden kayıt/kimlik doğrulama ayarlarını keşfedebilir:
...
Kendi sunucunuzda barındırıyorsanız bu dosyayı sunun (veya açıkça `CLAWHUB_REGISTRY` ayarlayın; eski `CLAWDHUB_REGISTRY`).
