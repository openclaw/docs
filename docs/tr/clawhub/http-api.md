...
x-i18n:
    generated_at: "2026-07-02T14:09:14Z"
    model: gpt-5.5
...
önceliğini yeniden oluşturmak yerine okuma uç noktaları tarafından döndürülen kanonik
...
- `X-RateLimit-Remaining` / `RateLimit-Remaining`: mevcut olduğunda tam kalan bütçe.
  Parçalanmış başarılı istekler, yaklaşık bir küresel değer döndürmek yerine bu üst bilgiyi atlar.
...
- Senkronize yeniden denemelerden kaçınmak için jitter uygulanmış geri çekilme kullanın.
- `Retry-After` eksikse, `RateLimit-Reset` değerine geri dönün (veya `X-RateLimit-Reset` üzerinden hesaplayın).

IP kaynağı:

- `cf-connecting-ip` dahil güvenilir istemci IP üst bilgilerini yalnızca dağıtım
  güvenilir yönlendirilmiş üst bilgileri açıkça etkinleştirdiğinde kullanır.
- ClawHub, istemci IP'lerini uçta tanımlamak için güvenilir yönlendirme üst bilgileri kullanır.
- Güvenilir istemci IP'si yoksa, anonim istekler yalnızca hız sınırı türüne göre
  kapsamlanan yedek kovalara kullanır. Bu yedek kovalar, çağıranın sağladığı
...
- Popülerlik logaritmik olarak ölçeklenir ve sınırlandırılır. Yüksek etkileşimli Skills'ler, sorgu metni daha zayıf bir eşleşmeyse daha düşük sıralanabilir.
...
- Yeniden adlandırma takma adları, kayıt üzerinden çözümlenen eski URL'ler ve kurulumlar için çözümlemeyi korur, ancak arama sıralaması yeniden adlandırma dizine eklendikten sonra kanonik Skill meta verilerine dayanır. Mevcut istatistikler Skill ile kalır.
...
- `nonSuspiciousOnly=true` olduğunda, imleç tabanlı sıralamalar bir sayfada `limit` değerinden daha az öğe döndürebilir, çünkü şüpheli Skills'ler sayfa alımından sonra filtrelenir.
...
ayarlanırken atlanabilir. Skill'i aynı denetlenebilir iş akışında gizlemek için triajlanmış
...
- `moderation`, en son sürümden türetilen güncel Skills düzeyinde bir moderasyon anlık görüntüsüdür.
...
- Tarama işleri eşzamansızdır. Manuel tarama istekleri normal yayınlama/geriye dönük doldurma işlerinden önce önceliklendirilir, ancak tamamlanma yine de çalışan kullanılabilirliğine bağlıdır.
...
- Engellenmiş veya gizli sürümler dahil olmak üzere, tam olarak gönderilen sürüm için saklanan tarama sonuçlarını döndürür.
...
- `security.signals.dependencyRegistry`, v1 yanıt uyumluluğu için korunur, ancak bağımlılık kayıt varlığı tarayıcısı kullanımdan kaldırılmıştır ve bu anahtar her zaman `null` olur.
- Skill Card olmaması, bu uç noktanın `ok`, `decision` veya `reasons` değerlerini etkilemez; istemciler kart içeriğine ihtiyaç duyduklarında yüklü `skill-card.md` dosyasını yerel olarak okumalıdır.
...
- `family`, `channel`, `isOfficial`, `featured` veya
  `highlightedOnly` için geçersiz değerler `400` döndürür. Bilinmeyen sorgu
...
`trending`, yedi günlük kurulum/indirme sıralamasıdır ve tüm zamanların toplamlarını kullanmaz.
...
- `clean` veya `suspicious` taraması olan geçerli GitHub destekli Skills,
  ...
  ile `_source_handoff.json` içerir. ClawHub'da barındırılan kaynak dosyaları içermez.
...
- `trust.blockedFromDownload`, kurulum engelleme sinyalidir. OpenClaw ve diğer
  ...
  engellemelidir.
- `trust.reasons`, kullanıcıya görünen ve denetim açıklaması listesidir. Neden
...
- `trust.stale`, güven özetinin güncelliğini yitirmiş girdilerden
  hesaplandığı ve yüksek güvenli izin kararı öncesinde yenileme gerektiriyor
  olarak ele alınması gerektiği anlamına gelir.
...
Sürüm yapıtını açık çözücü yolu üzerinden indirir.

Notlar:

- ClawPack sürümleri, yüklenen tam npm-pack `.tgz` baytlarını akıtır.
...
- `revoked`: bir sürüm daha önce güvenilir sayıldıktan sonra engellenmiş.
...
- Yanıtlar, çözümleyici bütünlüğü denetimleri için `ETag`, `Digest`,
...
npm ayna istemcileri için tam olarak yüklenmiş ClawPack tarball baytlarını akışa verir.
...
- İsteğe bağlı yük alanı: `migrateOwner`. `ownerHandle` ile `true` olduğunda,
  ...
  Skills o sahibe taşınabilir. Bu açık tercih olmadan sahip değişiklikleri
  reddedilir.
...
bir yayıncı tarafından talep edilebilir. Silme yanıtı, bu sona erme geçerliyse
...
sahip daha sonra gerçek code-plugin veya bundle-plugin sürümünü o ada yayınlayabilir.
...
kimliği için kişisel yayıncıyı kurtarır. İstek, her iki değişmez GitHub
sağlayıcı hesap kimliğini de adlandırmalıdır; değişebilir handle’lar yalnızca operatöre dönük bir koruma olarak kullanılır.
...
yayıncısında Skills, paketler veya GitHub Skills kaynakları varsa kurtarma güvenli şekilde kapalı başarısız olur.
Kurtarma ayrıca, doğrudan sahip yollarının yeni yayıncı yetkisiyle uyuşması için kurtarılan yayıncının Skills,
skill slug takma adları, paketleri, paket denetleyici uyarıları ve türetilmiş arama özet satırları için eski
...
Mevcut bir yasağın depolanan nedenini, yasağı kaldırmadan veya içeriği geri yüklemeden
değiştirin (yalnızca admin). `dryRun`, `false` olmadığı sürece varsayılan olarak dry-run modundadır.
...
Bir yıldız (öne çıkarma) ekleyin/kaldırın. Her iki uç nokta da idempotenttir.
...
hazırlayan paket yayınları, ortaya çıkan depolama kimliğini
`clawpack` olarak ve döndürülen bileti `clawpackUploadTicket` olarak göndermelidir.
