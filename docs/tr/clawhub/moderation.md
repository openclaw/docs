---
read_when:
    - Bir beceri, Plugin veya paketi bildirme
...
x-i18n:
    generated_at: "2026-07-04T20:39:51Z"
    model: gpt-5.5
...
ClawHub yayınlamaya açıktır, ancak herkese açık keşif ve kurulum yüzeylerinin yine de
koruma önlemlerine ihtiyacı vardır. Raporlar, moderasyon bekletmeleri, gizli listelemeler ve hesap işlemleri,
...
gibi denetim etiketleri ve risk düzeyi için bkz.
...
Bir beceri sayfasındaki **Beceriyi raporla** düğmesini veya paketler için paket raporlama
komutunu/API'sini kullanın.

ClawHub raporlarını üçüncü taraf bir becerinin veya
...
deposuna bildirin. ClawHub, üçüncü taraf beceri veya Plugin kodunun bakımını yapmaz ya da yama uygulamaz.
...
danışmanlarını üçüncü taraf beceriler veya Plugin'lerdeki güvenlik açıkları için kullanmayın.
...
bu süreci kullanın. Herkese açık bir konuya sırlar, özel belgeler, özel yasal
...
vaka çözümleri üzerinde çalışırken kullanıcıları korumayı amaçlar. Yanlış pozitif doğrulandığında da kaldırılabilirler.
...
Bu durumlardan birini görürseniz, sahip sorunu çözmediği veya moderasyon listelemeyi geri yüklemediği sürece
sürümü kurmayın.

Sahipler, kendi bekletilmiş veya gizli listelemeleri için tanı bilgilerini yine de görebilir. Bu
tanı bilgileri, ne olduğunu ve listelemenin
herkese açık yüzeylere dönebilmesi için neyin değişmesi gerektiğini açıklamaya yardımcı olur.
...
Yayımcı kötüye kullanım baskısı sinyalleri günlük olarak kontrol edilir. ClawHub'ın
olası yasak eşiğine ulaşan sinyaller otomatik bir uyarıyı tetikleyebilir. Uyarı son tarihinden sonraki ilk
uygun tarama yayımcıyı hâlâ
...
durumunu incelemek için web kullanıcı arayüzünde oturum açın. Oturum açma veya normal CLI erişimi bir yasak ya da devre dışı bırakılmış hesap nedeniyle engellenmişse,
kurtarma incelemesi için [ClawHub itiraz formunu](https://appeals.openclaw.ai/) kullanın.

Tarayıcı tarafından tetiklenen bir e-posta bir beceri veya Plugin sürümünü kötü amaçlı olarak adlandırıyorsa,
engellenen gönderilmiş sürüm için depolanan tarama sonuçlarını indirin:
