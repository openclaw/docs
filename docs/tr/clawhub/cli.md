...
read_when:
    ...
    - Yükleme, güncelleme, yayımlama veya eşitleme işlemlerinde hata ayıklama
...
Bu değişkenlerden herhangi biri ayarlandığında CLI, dışa giden istekleri
belirtilen proxy üzerinden yönlendirir. `HTTPS_PROXY`, HTTPS istekleri için;
`HTTP_PROXY` düz HTTP için kullanılır. Belirli host'lar veya alan adları için
proxy'yi atlamak üzere `NO_PROXY` / `no_proxy` dikkate alınır.

Bu, doğrudan dışa giden bağlantıların engellendiği sistemlerde gereklidir
(örn. Docker container'ları, yalnızca proxy üzerinden internete çıkan Hetzner
...
- Öne çıkanlarınıza bir skill ekler/kaldırır.
...
- İndirmeler küçük bir popülerlik öncülüdür, en üst sırada yer almanın garantisi değildir.
- Bir skill görünmesi gerektiği halde görünmüyorsa, metadata'yı yeniden adlandırmadan önce sahip tarafından görülebilen moderasyon tanılarını kontrol etmek için oturum açıkken `clawhub inspect <slug>` çalıştırın.

### `explore`

- En yeni Skills listesini `/api/v1/skills?limit=...&sort=createdAt` üzerinden listeler (`createdAt` azalan sıralı).
- Bayraklar:
...
- Sabitlenmiş Skills üzerine yazmayı reddeder; önce `clawhub unpin <slug>` çalıştırın.
- Şunları yazar:
...
- `clawhub pin` ile dondurulan Skills yanında, isteğe bağlı gerekçe dahil olmak üzere `pinned` gösterir.

### `pin <slug>`

- Yüklü bir beceriyi kilit dosyasında sabitlenmiş olarak işaretler.
- `--reason <text>` becerinin neden dondurulduğunu kaydeder.
- Sabitlenmiş beceriler `update --all` tarafından atlanır ve doğrudan `update <slug>` ile reddedilir.
- Sabitlenmiş beceriler ayrıca `install --force` komutunu reddeder, böylece yerel baytlar yanlışlıkla değiştirilemez.
...
- Yayımlanmış beceriler atıf olmadan kullanılabilir, değiştirilebilir ve yeniden dağıtılabilir.
...
- En son yayımlanmış beceri sürümü için güvenlik yeniden taraması ister.
- Sahipler ve yayımcı yöneticileri, sürüm başına kurtarma sınırına kadar kendi becerilerini yeniden tarayabilir.
- Platform moderatörleri ve yöneticileri herhangi bir beceriyi yeniden tarayabilir ve sahip kurtarma sınırı tarafından engellenmez; ancak sürüm başına aynı anda yalnızca bir yeniden tarama çalışabilir.
...
- En son yayımlanmış paket sürümü için güvenlik yeniden taraması isteyin.
- Sahipler ve yayıncı yöneticileri, kendi paketlerini sürüm başına kurtarma
  sınırına kadar yeniden taratabilir.
- Platform moderatörleri ve yöneticileri herhangi bir paketi yeniden taratabilir ve
  sahip kurtarma sınırı tarafından engellenmez; ancak sürüm başına aynı anda
  yalnızca bir yeniden tarama çalışabilir.
...
- Bildirimler paket düzeyindedir, isteğe bağlı olarak bir sürüme bağlanabilir ve
...
- Bayraklar:
  - `--version <version>`: gerekli paket sürümü.
  - `--message <text>`: gerekli itiraz mesajı.
  - `--json`: makine tarafından okunabilir çıktı.
...
- Resmi durum, ClawPack kullanılabilirliği, yapıt özeti,
  kaynak menşei, OpenClaw uyumluluğu, ana makine hedefleri, ortam meta verileri
...
- Kod Plugin'i klasörleri yüklemeden önce bir ClawPack npm tarball dosyasına
  paketlenir; böylece OpenClaw kurulumları tam yapıtı doğrulayabilir.
  Paket Plugin'i klasörleri yine çıkarılmış dosya yayımlama yolunu kullanır.
- GitHub kaynakları için kaynak atfı depodan, çözümlenen commit'ten, ref'ten ve alt yoldan otomatik olarak doldurulur.
- Yerel klasörler için kaynak atfı, origin uzak deposu GitHub'ı işaret ettiğinde yerel git'ten otomatik olarak algılanır.
...
ihtiyaç duyar. Bu asgari bildirim, başarılı bir yayımlama için yeterlidir:
...
- `package.json.version`, paket sürüm numaranızdır; ancak OpenClaw uyumluluk/derleme doğrulaması
  için yedek olarak kullanılmaz.
- `openclaw.hostTargets` ve `openclaw.environment` isteğe bağlı meta verilerdir.
  ClawHub bunları mevcut olduğunda gösterebilir, ancak yayımlama için gerekli değildir.
...
- Oturum açıldığında, `CLAWHUB_DISABLE_TELEMETRY=1` olmadığı sürece (`CLAWDHUB_DISABLE_TELEMETRY=1` eski kullanım) `sync` sırasında gönderilir.
