...
read_when:
    ...
    - Kurulum, güncelleme, yayımlama veya senkronizasyon işlemlerinde hata ayıklama
...
API belirtecinizi ve önbelleğe alınmış kayıt URL'sini saklar.
...
- Bir becerinin görünmesi gerekiyor ancak görünmüyorsa, meta verileri yeniden adlandırmadan önce sahip tarafından görülebilen moderasyon tanılarını denetlemek için oturum açıkken `clawhub inspect <slug>` çalıştırın.
...
- `clawhub pin` ile dondurulmuş becerilerin yanında, isteğe bağlı gerekçeyle birlikte `pinned` gösterir.

### `pin <slug>`

- Yüklü bir beceriyi lockfile içinde sabitlenmiş olarak işaretler.
...
- Sabitlenmiş beceriler ayrıca `install --force` işlemini de reddeder, böylece yerel baytlar yanlışlıkla değiştirilemez.

### `unpin <slug>`

- Yüklü bir beceriden lockfile sabitlemesini kaldırır, böylece gelecekteki güncellemeler onu değiştirebilir.

### `update [slug]` / `update --all`

- Parmak izini yerel dosyalardan hesaplar.
...
- Bir beceri yayımlamak, onun ClawHub üzerinde `MIT-0` altında yayımlanması anlamına gelir.
...
- Bir beceriyi soft-delete yapar (sahip, moderatör veya yönetici).
...
- Kullanıcı tanıtıcılarına yapılan aktarımlar, alıcının kabul ettiği bekleyen bir istek oluşturur.
...
- `--tag <tag>`: etiketlenmiş bir sürümü inceler (örn. `latest`).
- `--versions`: sürüm geçmişini listeler (ilk sayfa).
- `--limit <n>`: listelenecek maksimum sürüm sayısı (1-100).
- `--files`: seçilen sürüm için dosyaları listeler.
...
- Yerel bir artifact için ClawHub SHA-256, npm `sha512` bütünlüğü ve npm shasum değerini hesaplar.
- `--package` ile beklenen meta verileri ClawHub’dan çözümler ve yerel dosyayı yayımlanan artifact meta verileriyle karşılaştırır.
...
- Paket sahibi, kuruluş yayıncı sahibi/yöneticisi, platform moderatörü
...
- Mevcut paket tarama durumunu, açık bildirim sayısını, en son yayın manuel
...
- Meta veriler `package.json`, `openclaw.plugin.json` ve
  ...
  gibi gerçek OpenClaw paket işaretçilerinden otomatik algılanır.
...
- Kod plugin’i klasörleri yüklemeden önce bir ClawPack npm tarball içine paketlenir; böylece
...
- GitHub kaynakları için kaynak atfı depodan, çözümlenmiş commit’ten, ref’ten ve alt yoldan otomatik doldurulur.
- Yerel klasörler için kaynak atfı, origin uzak deposu GitHub’ı işaret ettiğinde yerel git’ten otomatik algılanır.
...
- `--clawscan-note <text>` bir ClawScan notu ekler. Bu not, ağ erişimi,
  yerel ana makine erişimi veya sağlayıcıya özgü kimlik bilgileri gibi aksi halde olağan dışı görünebilecek davranışlar için ClawScan’e
  bağlam sağlar. Not, yayımlanan yayında saklanır.
...
Canlı bir yayın oluşturmadan önce çözümlenen paket meta verilerini ve
...
Kod plugin’leri için klasör yayımlama, paket klasöründen bir ClawPack yapıtı oluşturup yükler:
...
Harici kod plugin’leri `package.json` içinde az miktarda OpenClaw meta verisine ihtiyaç duyar. Bu en küçük manifest başarılı bir yayımlama için yeterlidir:
...
- `package.json.version` paket yayın sürümünüzdür, ancak OpenClaw uyumluluğu/derleme doğrulaması için
  geri dönüş olarak kullanılmaz.
...
- Daha ayrıntılı uyumluluk meta verileri yayımlamak istiyorsanız
  ...
  isteğe bağlı ek alanlardır.
- Daha eski bir `clawhub` CLI yayını kullanıyorsanız, yerel ön uçuş denetimlerinin yüklemeden önce çalışması için
...
- Monorepo’lar için, iş akışının plugin
  paket klasörünü yayımlaması amacıyla `source_path` iletin; örneğin `source_path: extensions/codex`.
- Yeniden kullanılabilir iş akışını kararlı bir etikete veya tam commit SHA’sına sabitleyin. Yayın yayımlamayı `@main` üzerinden çalıştırmayın.
- CI’nin kirlenmemesi için `pull_request`, `dry_run: true` kullanmalıdır.
...
- İlk yayımlama, güvenilmeyen paketler veya acil durum yayımlamaları için `clawhub_token` değerini erişilebilir tutun.
...
- Oturum açılmışken, `CLAWHUB_DISABLE_TELEMETRY=1` olmadığı sürece `sync` sırasında gönderilir (eski `CLAWDHUB_DISABLE_TELEMETRY=1`).
