...
(ör. Docker kapsayıcıları, yalnızca proxy üzerinden internete sahip Hetzner VPS,
...
- Saklanan API token'ını standart çıktıya yazdırır.
- Yerel oturum açma token'ını CI gizli dizi kurulum komutlarına aktarmak için kullanışlıdır.
...
- Popülerlik küçük bir sıralama önceliğidir, en üst sırada yer alma garantisi değildir.
- Bir beceri görünmesi gerekirken görünmüyorsa, meta veriyi yeniden adlandırmadan önce sahip tarafından görülebilen moderasyon tanılamalarını kontrol etmek için oturum açıkken `clawhub inspect @owner/slug` çalıştırın.
...
- Bayraklar:
  ...
  - `--sort newest|updated|rating|downloads|trending` (varsayılan: newest). Eski kurulum sıralama adları uyumluluk için hâlâ çalışır.
  - `--json` (makine tarafından okunabilir çıktı)
- Çıktı: `<slug>  v<version>  <age>  <summary>` (özet 50 karaktere kısaltılır).

### `inspect @owner/slug`

- Kurmadan beceri meta verisini ve sürüm dosyalarını getirir.
...
- `<workdir>/<dir>/<slug>` konumunu kaldırır ve kilit dosyası girdisini siler.
- Mevcut kurulum sayılarının devre dışı bırakılabilmesi için oturum açıkken en iyi çaba telemetrisi gönderir.
...
- Sabitlenmiş beceriler, yerel baytların yanlışlıkla değiştirilememesi için `install --force` komutunu da reddeder.

### `unpin <skill>`

- Gelecekteki güncellemelerin kurulu bir beceriyi değiştirebilmesi için kilit dosyası sabitlemesini kaldırır.
...
- Yeni beceriler varsayılan olarak `1.0.0` olur; değiştirilmiş beceriler varsayılan olarak bir sonraki yama sürümüne geçer.
- `--version <version>` açıkça bir sürüm seçer ve içerik mevcut bir sürümle eşleşse bile yayımlar.
- `--dry-run` yüklemeden yayımlamayı çözer; `--json` makine tarafından okunabilir bir sonuç yazdırır.
- `--owner <handle>` aktörün yayımcı erişimi olduğunda bir kuruluş/kullanıcı yayımcı tanıtıcısı altında yayımlar.
- `--migrate-owner` mevcut bir beceriyi yeni bir sürüm yayımlarken `--owner` konumuna taşır. Her iki yayımcıda da yönetici/sahip erişimi gerektirir.
...
iş akışı, bir `skill_path` için veya `root` altındaki her doğrudan beceri
klasörü için `skill publish` çağırır (varsayılan: `skills`). Değişmemiş
becerileri atlar ve aynı otomatik yama sürümü davranışını kullanır.
...
- Yeni beceriler `1.0.0` olarak yayımlanır; değiştirilmiş beceriler varsayılan
  olarak bir sonraki yama sürümünü yayımlar. Daha büyük bir semver adımıyla
  ilerlemesi gereken güncelleme grupları için `--bump minor|major` kullanın.
- `--dry-run` yüklemeden yayımlama planını gösterir; `--json` makine tarafından
  okunabilir bir plan yazdırır.
- `--all` her yeni veya değiştirilmiş beceriyi istem göstermeden yayımlar.
  `--all` olmadan, etkileşimli terminaller yayımlanacak becerileri seçmenize izin verir.
...
- `sync` yalnızca tek yönlü yayımlamadır. Kurmaz, güncellemez, indirmez veya
  kurulum/indirme telemetrisi bildirmez.
...
- ClawHub ClawScan'i `POST /api/v1/skills/-/scan` üzerinden çalıştırır, ardından tarama son duruma ulaşana kadar yoklama yapar.
...
- Yayımlanmış taramalar sahiplik veya yayımcı yönetim erişimi gerektirir. Moderatörler/yöneticiler aynı arka ucu `clawhub-admin` üzerinden kullanabilir.
...
- `clawhub login` gerektirir.
- ClawHub güvenlik kontrolleri tarafından engellenmiş veya gizlenmiş sürümler dahil olmak üzere, gönderilmiş bir beceri veya Plugin sürümü için saklanan tarama raporu ZIP dosyasını indirir.
...
ClawHub, beceri depoları ve katalog depoları için şu adreste resmi bir yeniden kullanılabilir iş akışı sunar:
...
- Katalog depoları için `root` varsayılan olarak `skills` olur.
- Bir beceri klasörünü işlemek için `skill_path: skills/review-helper` geçin.
- `owner`, CLI `--owner` bayrağıyla eşleşir; kimliği doğrulanmış kullanıcı olarak yayımlamak için atlayın.
...
- `--reason <text>`, tüm skill'i kapsayan geçici silme ve denetim günlüğüne bir moderasyon notu kaydeder.
- `--note <text>`, `--reason` için bir takma addır.
- `--yes`, onayı atlar.

### `undelete <skill>`

- Gizlenmiş bir skill'i geri yükler (sahip, moderatör veya yönetici).
...
- Bir skill'i görünür yapar (sahip, moderatör veya yönetici).
...
- Kuruluş/yayımcı tanıtıcılarına yapılan aktarımlar yalnızca aktörün hem geçerli sahipte hem de hedef yayımcıda
...
- Artifact'i çözücünün `downloadUrl` adresinden indirir.
...
- ClawHub CLI'ın paketli Plugin Inspector aracını yerel bir Plugin paketi
  klasörüne karşı çalıştırır.
- Varsayılan olarak yerel bir OpenClaw checkout'ını bulmadan veya içe aktarmadan çevrimdışı/statik doğrulama yapar.
- Kesin uyumluluk hataları sıfır olmayan kodla çıkar. Yalnızca uyarı bulguları yazdırılır ancak
  sıfır kodla çıkar.
- Bayraklar:
  - `--out <dir>`: Plugin Inspector raporlarını bu dizine yaz.
  - `--openclaw <path>`: açık bir yerel OpenClaw checkout'ına karşı incele.
...
Doğrulama bir paket, manifest, SDK içe aktarımı veya artifact bulgusu bildirirse bkz.
[Plugin doğrulama düzeltmeleri](/tr/clawhub/plugin-validation-fixes), ardından komutu yeniden çalıştırın.
...
- `--version <version>`, sahip olunan ve en son olmayan tek bir sürümü hata durumunda kapalı,
  sürüme özgü bir rota üzerinden kalıcı olarak siler.
  Silinen sürümler geri yüklenemez veya yeniden yayımlanamaz. Geçerli en son sürümü silmeden önce
  bir yedek yayımlayın. Bu yalnızca sürüm akışı paket sahibini veya bir kuruluş yayımcısı
  yöneticisini gerektirir; platform personeli paket sahipliğini atlayamaz.
- Tüm paket geçici silme işlemi paket sahibini, bir kuruluş yayımcısı sahibi/yöneticisini, platform
...
- Sürüm geri alma yoktur; kalıcı olarak silinen sürümler geri yüklenemez.
- Paket sahibini, bir kuruluş yayımcısı sahibi/yöneticisini, platform moderatörünü
...
- Platform yöneticisi tarafından yapılmadığı sürece hem geçerli paket sahibine hem de hedef
  yayımcıya yönetici erişimi gerektirir.
...
- Paketli bir OpenClaw Plugin'inin yerini alabilecek bir paket için operatör odaklı geçiş durumunu gösterir.
...
- `POST /api/v1/packages` aracılığıyla bir kod plugin'i veya bundle plugin'i yayınlar.  
...
- Metadata `package.json`, `openclaw.plugin.json` ve `.codex-plugin/plugin.json`,
  `.claude-plugin/plugin.json`, `.cursor-plugin/plugin.json` gibi gerçek OpenClaw
  bundle işaretçilerinden otomatik algılanır.
...
- Kod plugin'i klasörleri, yüklemeden önce bir ClawPack npm tarball dosyasına
  paketlenir; böylece OpenClaw kurulumları tam artifact'i doğrulayabilir.
  Bundle plugin'i klasörleri ise ayıklanmış dosya yayınlama yolunu kullanmaya
  devam eder.
...
- `--dry-run`, çözümlenen yayın payload'unu yükleme yapmadan önizler.
- `--json`, CI için makine tarafından okunabilir çıktı üretir.
- `--owner <handle>`, aktörün yayıncı erişimi olduğunda bir kullanıcı veya kuruluş yayıncı handle'ı altında yayınlar.
...
Canlı bir sürüm oluşturmadan önce çözümlenen paket metadata'sını ve kaynak
atfını doğrulayabilmeniz için önce `--dry-run` kullanın:
...
Kod plugin'leri için klasör yayını, paket klasöründen bir ClawPack artifact'i
oluşturup yükler:
...
Harici kod plugin'lerinin `package.json` içinde az miktarda OpenClaw
metadata'sına ihtiyacı vardır. Bu minimal manifest başarılı bir yayın için yeterlidir:
...
- `package.json.version`, paket sürüm numaranızdır; ancak OpenClaw
  uyumluluk/build doğrulaması için fallback olarak kullanılmaz.
- `openclaw.hostTargets` ve `openclaw.environment` isteğe bağlı metadata'dır.
  ClawHub bunlar mevcut olduğunda gösterebilir, ancak yayın için zorunlu değildir.
- `openclaw.compat.minGatewayVersion` ve
  `openclaw.build.pluginSdkVersion`, daha ayrıntılı uyumluluk metadata'sı
  yayınlamak istiyorsanız isteğe bağlı eklerdir.
- Daha eski bir `clawhub` CLI sürümü kullanıyorsanız yerel preflight
  kontrollerinin yüklemeden önce çalışması için yayınlamadan önce yükseltin.
...
- Monorepo'lar için iş akışının plugin paket klasörünü yayınlaması amacıyla
  `source_path` iletin; örneğin `source_path: extensions/codex`.
- Yeniden kullanılabilir iş akışını kararlı bir etikete veya tam commit SHA'sına sabitleyin. Sürüm yayınlamayı `@main` üzerinden çalıştırmayın.
- CI'ın kirlenmemesi için `pull_request`, `dry_run: true` kullanmalıdır.
- Gerçek yayınlar `workflow_dispatch` veya tag push'ları gibi güvenilir olaylarla sınırlandırılmalıdır.
...
- İlk yayın, güvenilmeyen paketler veya break-glass yayınları için `clawhub_token` değerini kullanılabilir tutun.
- İş akışı JSON sonucunu artifact olarak yükler ve iş akışı çıktıları olarak sunar.

### `package trusted-publisher get <name>`

- Bir paket için GitHub Actions güvenilir yayıncı yapılandırmasını gösterir.
- Yapılandırmayı ayarladıktan sonra depoyu, iş akışı dosya adını ve isteğe bağlı
...
- Mevcut bir paket için GitHub Actions güvenilir yayıncı yapılandırması ekler
  veya değiştirir.
- Paket önce normal manuel ya da token ile kimliği doğrulanmış
  `clawhub package publish` aracılığıyla oluşturulmalıdır.
- Yapılandırma ayarlandıktan sonra gelecekte desteklenen GitHub Actions
  yayınları uzun ömürlü bir ClawHub token'ı olmadan OIDC/güvenilir yayınlama kullanabilir.
...
- Bu komut çalıştığında ClawHub yapılandırılan GitHub deposunu doğrular.
  Genel depolar, herkese açık GitHub metadata'sı üzerinden doğrulanabilir. Özel
  depolar, örneğin gelecekteki bir ClawHub GitHub App kurulumu veya başka bir
  yetkili GitHub entegrasyonu aracılığıyla ClawHub'ın o depoya GitHub erişimi olmasını gerektirir.
- Flag'ler:
...
- İş akışı, depo veya ortam sabitlemesinin devre dışı bırakılması ya da yeniden
  oluşturulması gerekiyorsa bunu rollback olarak kullanın.
- Gelecekteki gerçek yayınlar, yapılandırma yeniden ayarlanana kadar normal kimliği doğrulanmış yayınlamayı kullanmalıdır.
- Flag'ler:
  - `--json`: makine tarafından okunabilir çıktı.
...
- Raporlama best-effort olarak yapılır. Telemetri kullanılamıyorsa kurulum
