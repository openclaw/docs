...
read_when:
    ...
    - Yükleme, güncelleme veya yayımlama hata ayıklaması
summary: 'CLI referansı: komutlar, bayraklar, yapılandırma ve kilit dosyası davranışı.'
x-i18n:
    generated_at: "2026-06-30T22:30:00Z"
    model: gpt-5.5
...
proxy üzerinden yönlendirir. `HTTPS_PROXY` HTTPS istekleri için, `HTTP_PROXY`
düz HTTP için kullanılır. Belirli ana makineler veya etki alanları için proxy'yi
atlamak üzere `NO_PROXY` / `no_proxy` dikkate alınır.

Bu, doğrudan giden bağlantıların engellendiği sistemlerde gereklidir
(ör. Docker kapsayıcıları, yalnızca proxy üzerinden internete çıkan Hetzner VPS,
...
- Vurgulananlarınıza bir beceri ekler veya kaldırır.
...
- Çıktı beceri slug'ını, sahip tanıtıcısını, görünen adı ve alaka skorunu içerir.
...
- Bir becerinin görünmesi gerekiyorsa ancak görünmüyorsa, metadata'yı yeniden adlandırmadan önce sahip tarafından görülebilen moderasyon tanılarını kontrol etmek için oturum açıkken `clawhub inspect @owner/slug` çalıştırın.
...
- Bayraklar:
  ...
  - `--sort newest|updated|rating|downloads|trending` (varsayılan: newest). Eski kurulum sıralama alias'ları uyumluluk için çalışmaya devam eder.
...
- Pinlenmiş becerilerin üzerine yazmayı reddeder; önce `clawhub unpin <skill>` çalıştırın.
- Şunları yazar:
...
- Mevcut kurulum sayılarının devre dışı bırakılabilmesi için oturum açıkken
  en iyi çaba telemetrisi gönderir.
...
- `clawhub pin` ile dondurulan becerilerin yanında, isteğe bağlı neden dahil `pinned` gösterir.

### `pin <skill>`

- Kurulu bir beceriyi lockfile içinde pinlenmiş olarak işaretler.
- `--reason <text>` becerinin neden dondurulduğunu kaydeder.
- Pinlenmiş beceriler `update --all` tarafından atlanır ve doğrudan `update <skill>` tarafından reddedilir.
- Pinlenmiş beceriler ayrıca yerel baytların yanlışlıkla değiştirilememesi için `install --force` işlemini de reddeder.

### `unpin <skill>`

- Gelecekteki güncellemelerin değiştirebilmesi için kurulu bir beceriden lockfile pin'ini kaldırır.
...
- Pinlenmiş beceriler `--force` tarafından hiçbir zaman güncellenmez.
- `update <skill>` pinlenmiş beceriler için hızlıca başarısız olur ve önce `clawhub unpin <skill>` çalıştırmanızı söyler.
- `update --all` pinlenmiş slug'ları atlar ve nelerin dondurulmuş kaldığına dair bir özet yazdırır.

### `skill publish <path>`

- Yerel bundle parmak izini ClawHub ile karşılaştırır ve içerik zaten
  yayımlanmışsa başarıyla çıkar.
- Yeni beceriler varsayılan olarak `1.0.0` alır; değişen beceriler varsayılan
  olarak sonraki patch sürümünü alır.
- `--version <version>` açıkça bir sürüm seçer ve içerik mevcut bir sürümle
  eşleşse bile yayımlar.
- `--dry-run` yükleme yapmadan yayını çözer; `--json` makine tarafından
  okunabilir bir sonuç yazdırır.
- `--owner <handle>` aktörün yayıncı erişimi varsa bir kuruluş/kullanıcı yayıncı
  tanıtıcısı altında yayımlar.
- `--migrate-owner`, yeni bir sürüm yayımlarken mevcut bir beceriyi `--owner`
  konumuna taşır. Her iki yayıncıda da admin/sahip erişimi gerektirir.
...
- Yayımlanan beceriler atıf olmadan kullanılabilir, değiştirilebilir ve yeniden dağıtılabilir.
...
becerileri atlar ve aynı otomatik patch sürümü davranışını kullanır.
...
- Mevcut workdir'i, yapılandırılmış beceriler dizinini ve `SKILL.md` veya
...
- Yeni beceriler `1.0.0` olarak yayımlanır; değişmiş beceriler varsayılan olarak
  sonraki patch sürümünü yayımlar. Daha büyük bir semver adımıyla ilerlemesi
  gereken güncelleme toplu işlemleri için `--bump minor|major` kullanın.
- `--dry-run` yükleme yapmadan yayın planını gösterir; `--json` makine tarafından
  okunabilir bir plan yazdırır.
- `--all` istem göstermeden her yeni veya değişmiş beceriyi yayımlar. `--all`
  olmadan etkileşimli terminaller yayımlanacak becerileri seçmenizi sağlar.
- `--owner <handle>` aktörün yayıncı erişimi varsa bir kuruluş/kullanıcı yayıncı
  tanıtıcısı altında yayımlar.
- `sync` yalnızca tek yönlü yayındır. Kurulum, güncelleme, indirme yapmaz veya
...
- `POST /api/v1/skills/-/scan` üzerinden ClawHub ClawScan çalıştırır, ardından tarama terminal olana kadar yoklar.
...
- Yayımlanan taramalar sahiplik veya yayıncı yönetim erişimi gerektirir. Moderatörler/adminler aynı backend'i `clawhub-admin` üzerinden kullanabilir.
...
- Yerel yol taramaları artık desteklenmiyor. Yeni bir sürüm yükleyin, ardından gönderilen o sürüm için saklanan tarama sonuçlarını almak üzere `scan download` kullanın.
...
- `clawhub login` gerektirir.
- ClawHub güvenlik kontrolleri tarafından engellenen veya gizlenen sürümler dahil, gönderilmiş bir beceri veya Plugin sürümü için saklanan tarama raporu ZIP'ini indirir.
...
ClawHub, beceri repoları ve katalog repoları için
...
konumunda resmi bir yeniden kullanılabilir workflow gönderir.
...
- `root`, katalog repoları için varsayılan olarak `skills` değerini alır.
...
- `--version` olmadan bir beceriyi geçici siler (sahip, moderatör veya yönetici).
...
- `--version <version>`, sahip olunan en son olmayan tek bir sürümü güvenli biçimde kapalı kalan,
  ...
  yerine geçecek bir sürüm yayımlayın. Platform personeli, yalnızca sürüm akışında sahipliği atlamaz.
- `--reason <text>`, tüm beceriyi geçici silme ve denetim günlüğü için bir moderasyon notu kaydeder.
...
- Bir becerinin gizliliğini kaldırır (sahip, moderatör veya yönetici).
...
- Kuruluş/yayıncı tanıtıcılarına aktarımlar yalnızca aktörün hem geçerli sahipte hem de hedef yayıncıda
...
- Kurmadan paket meta verilerini getirir.
...
- `--files`: seçili sürümün dosyalarını listeleyin.
- `--file <path>`: ham dosya içeriğini getirir (yalnızca metin dosyaları; 200 KB sınırı).
- `--json`: makine tarafından okunabilir çıktı.
...
- Yerel bir artefakt için ClawHub SHA-256, npm `sha512` bütünlüğü ve npm shasum
  hesaplar.
- `--package` ile beklenen meta verileri ClawHub'dan çözümler ve yerel dosyayı
  yayımlanan artefakt meta verileriyle karşılaştırır.
...
- Kesin uyumluluk hataları sıfır olmayan çıkışla sonlanır. Yalnızca uyarı bulguları yazdırılır ancak
  sıfır çıkışla sonlanır.
- Bayraklar:
  - `--out <dir>`: Plugin Inspector raporlarını bu dizine yazar.
  - `--openclaw <path>`: açık bir yerel OpenClaw checkout konumuna karşı inceler.
...
Doğrulama bir paket, manifest, SDK içe aktarması veya artefakt bulgusu bildirirse
[Plugin doğrulama düzeltmeleri](/clawhub/plugin-validation-fixes) sayfasına bakın, ardından komutu yeniden çalıştırın.

### `package delete <name>`

- `--version` olmadan bir paketi ve tüm sürümleri geçici siler.
- `--version <version>`, sahip olunan en son olmayan tek bir sürümü güvenli biçimde kapalı kalan,
  sürüme özgü bir rota üzerinden kalıcı olarak siler.
  Silinen sürümler geri yüklenemez veya yeniden yayımlanamaz. Geçerli en son sürümü silmeden önce
  yerine geçecek bir sürüm yayımlayın. Bu yalnızca sürüm akışı paket sahibini veya bir kuruluş yayıncısı
  yöneticisini gerektirir; platform personeli paket sahipliğini atlamaz.
- Tüm paketi geçici silme için paket sahibi, kuruluş yayıncısı sahibi/yöneticisi, platform
...
- Geçici silinmiş bir paketi ve sürümleri geri yükler.
- Sürüm geri yükleme yoktur; kalıcı olarak silinen sürümler geri yüklenemez.
...
- Bildirimler tek başına paketleri otomatik gizlemez veya indirmeleri engellemez.
- Bayraklar:
...
- Geçerli paket tarama durumunu, açık bildirim sayısını, en son sürüm manuel
...
- Bir paketin gelecekte OpenClaw tüketimi için hazır olup olmadığını kontrol eder.
...
- Birlikte gelen bir OpenClaw Plugininin yerini alabilecek bir paket için operatör odaklı
...
- Kimliği doğrulanmış kullanıcıya ait bir kuruluş yayıncısı oluşturur.
- Tanıtıcı küçük harfe normalize edilir ve `@` ile veya olmadan geçirilebilir.
- Yeni oluşturulan kuruluş yayıncıları varsayılan olarak güvenilir/resmi değildir.
- Tanıtıcı mevcut bir yayıncı, kullanıcı veya rezerve rota tarafından zaten kullanılıyorsa başarısız olur.
...
- Bir kod plugin'ini veya paket plugin'ini `POST /api/v1/packages` üzerinden yayımlar.
...
- Meta veriler `package.json`, `openclaw.plugin.json` ve
  ...
  `.cursor-plugin/plugin.json` gibi gerçek OpenClaw paket işaretleyicilerinden otomatik algılanır.
...
- Kod plugin'i klasörleri, yüklemeden önce ClawPack npm tarball'ı olarak paketlenir; böylece
  OpenClaw kurulumları tam yapıtı doğrulayabilir. Paket plugin'i klasörleri ise yine
...
konumunda resmi yeniden kullanılabilir bir iş akışı da sunar.
...
- Gerçek yayımlamalar `workflow_dispatch` veya etiket push'ları gibi güvenilir olaylarla sınırlı olmalıdır.
...
- İlk yayımlama, güvenilmeyen paketler veya acil durum yayımlamaları için `clawhub_token` kullanılabilir tutun.
...
- Depoyu, iş akışı dosya adını ve isteğe bağlı ortam sabitlemesini doğrulamak için yapılandırmayı ayarladıktan sonra bunu kullanın.
- Bayraklar:
  - `--json`: makine tarafından okunabilir çıktı.
...
- ClawHub, bu komut çalıştığında yapılandırılan GitHub deposunu doğrular.
  ...
  GitHub entegrasyonu üzerinden ClawHub'ın bu depoya GitHub erişimi olmasını gerektirir.
- Bayraklar:
  ...
  - `--environment <name>`: isteğe bağlı tam eşleşen GitHub Actions ortamı.
  - `--json`: makine tarafından okunabilir çıktı.
...
- Güvenilir yayımcı yapılandırmasını bir paketten kaldırır.
- İş akışı, depo veya ortam sabitlemesinin devre dışı bırakılması ya da yeniden oluşturulması gerekiyorsa bunu geri alma olarak kullanın.
...
- Oturum açılmışken `CLAWHUB_DISABLE_TELEMETRY=1` ayarlanmadığı sürece
  `clawhub install <slug>` sonrasında gönderilir.
- Raporlama en iyi çaba esasına göre yapılır. Telemetri kullanılamıyorsa kurulum komutları
  başarısız olmaz.
