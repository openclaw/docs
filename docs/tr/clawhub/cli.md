...
API token'ınızı + önbelleğe alınmış kayıt URL'sini saklar.
...
- Uzak/başsız etkileşimli: `clawhub login --device` bir kod yazdırır ve siz bunu `<site>/cli/device` adresinde yetkilendirirken bekler.
...
- Öne çıkanlarınıza bir beceri ekler veya kaldırır.
- `POST /api/v1/stars/<slug>` ve `DELETE /api/v1/stars/<slug>` çağrıları yapar.
- `--yes` onayı atlar.
...
- İndirmeler küçük bir popülerlik öncülüdür, en üstte yer alma garantisi değildir.
- Bir becerinin görünmesi gerekiyor ancak görünmüyorsa meta verileri yeniden adlandırmadan önce sahip tarafından görülebilen moderasyon tanılamalarını kontrol etmek için oturum açmışken `clawhub inspect <slug>` çalıştırın.
...
- Beceri meta verilerini ve sürüm dosyalarını kurulum yapmadan getirir.
...
- `clawhub pin` ile dondurulmuş becerilerin yanında, isteğe bağlı gerekçe dahil `pinned` gösterir.
...
- Sabitlenmiş beceriler ayrıca `install --force` işlemini reddeder; böylece yerel baytlar yanlışlıkla değiştirilemez.

### `unpin <slug>`

- Gelecekteki güncellemelerin kurulu bir beceriyi değiştirebilmesi için lockfile sabitlemesini kaldırır.
...
- Sabitlenmiş beceriler `--force` ile hiçbir zaman güncellenmez.
...
- `--migrate-owner`, yeni bir sürüm yayımlarken mevcut bir beceriyi `--owner` altına taşır. Her iki yayımcıda da admin/owner erişimi gerektirir.
- Sahip ve inceleme davranışı `docs/publishing.md` içinde açıklanır.
- Bir beceriyi yayımlamak, onun ClawHub üzerinde `MIT-0` altında yayınlandığı anlamına gelir.
...
- `--clawscan-note <text>` bir ClawScan notu ekler. Bu not, ağ erişimi, yerel host erişimi veya sağlayıcıya özgü kimlik bilgileri gibi aksi halde olağandışı görünebilecek davranışlar için ClawScan'e bağlam sağlar. Not, yayımlanan sürümde saklanır.
...
- Bir beceriyi geçici olarak siler (owner, moderator veya admin).
- `DELETE /api/v1/skills/{slug}` çağrısı yapar.
- Owner tarafından başlatılan geçici silmeler slug'ı 30 gün boyunca ayırır; komut sona erme zamanını yazdırır.
...
- Gizli bir beceriyi geri yükler (owner, moderator veya admin).
...
- Bir beceriyi gizler (owner, moderator veya admin).
- `delete` için takma addır.

### `unhide <slug>`

- Bir beceriyi görünür yapar (owner, moderator veya admin).
...
- `GET /api/v1/packages/{name}/versions/{version}/artifact` üzerinden bir paket sürümünü çözümler.
- Artifact'i çözümleyicinin `downloadUrl` adresinden indirir.
- Tüm artifact'ler için ClawHub SHA-256'yı doğrular.
...
- Bildirimler paket düzeyindedir, isteğe bağlı olarak bir sürüme bağlanabilir
  ve incelenmek üzere moderatörlere görünür hale gelir.
- Bildirimler paketleri kendiliğinden otomatik gizlemez veya indirmeleri engellemez.
- Bayraklar:
...
- Bir paketin gelecekteki OpenClaw tüketimine hazır olup olmadığını denetler.
- `GET /api/v1/packages/{name}/readiness` çağrısı yapar.
- Resmi durum, ClawPack kullanılabilirliği, artefakt özeti, kaynak kökeni,
  OpenClaw uyumluluğu, host hedefleri, ortam meta verileri ve tarama durumu
  için engelleyicileri raporlar.
- Bayraklar:
  - `--json`: makine tarafından okunabilir çıktı.
...
- Meta veriler `package.json`, `openclaw.plugin.json` ve
  ...
  `.cursor-plugin/plugin.json` gibi gerçek OpenClaw paket işaretleyicilerinden
  otomatik algılanır.
...
- Kod Plugin'i klasörleri yükleme öncesinde bir ClawPack npm tarball'ına
  paketlenir, böylece OpenClaw kurulumları tam artefaktı doğrulayabilir. Paket
  Plugin'i klasörleri ise çıkarılmış dosya yayım yolunu kullanmaya devam eder.
- GitHub kaynakları için kaynak atfı depodan, çözümlenen commit'ten, ref'ten ve alt yoldan otomatik doldurulur.
- Yerel klasörler için kaynak atfı, origin remote'u GitHub'ı gösterdiğinde yerel git'ten otomatik algılanır.
- Harici kod Plugin'leri `openclaw.compat.pluginApi` ve
  `openclaw.build.openclawVersion` değerlerini açıkça bildirmelidir.
  Üst düzey `package.json.version`, yayım doğrulaması için geri dönüş olarak kullanılmaz.
- `--dry-run`, yükleme yapmadan çözümlenen yayım yükünü önizler.
...
- `--clawscan-note <text>` bir ClawScan notu ekler. Bu not ClawScan'e ağ erişimi,
  yerel host erişimi veya sağlayıcıya özgü kimlik bilgileri gibi aksi halde
  olağandışı görünebilecek davranışlar için bağlam sağlar. Not, yayımlanan sürümde saklanır.
...
Canlı bir sürüm oluşturmadan önce çözümlenen paket meta verilerini ve kaynak
atfını onaylayabilmek için önce `--dry-run` kullanın:
...
Kod Plugin'leri için klasörden yayım, paket klasöründen bir ClawPack artefaktı
oluşturur ve yükler:
...
ihtiyaç duyar. Bu en küçük manifest başarılı bir yayım için yeterlidir:
...
- `openclaw.hostTargets` ve `openclaw.environment` isteğe bağlı meta verilerdir.
  ClawHub mevcut olduklarında bunları gösterebilir, ancak yayım için zorunlu değildirler.
- `openclaw.compat.minGatewayVersion` ve
  `openclaw.build.pluginSdkVersion`, daha ayrıntılı uyumluluk meta verileri
  yayımlamak istiyorsanız isteğe bağlı eklerdir.
- Daha eski bir `clawhub` CLI sürümü kullanıyorsanız, yerel ön kontrol
  denetimlerinin yükleme öncesinde çalışması için yayımlamadan önce yükseltin.
...
- Yeniden kullanılabilir workflow, `source` varsayılanını çağıran depo olarak ayarlar.
...
- Yeniden kullanılabilir workflow'u kararlı bir etikete veya tam commit SHA'sına sabitleyin. Sürüm yayımını `@main` üzerinden çalıştırmayın.
...
- Gizli olmadan güvenilir yayım yalnızca `workflow_dispatch` üzerinde çalışır; etiket push'ları yine de `clawhub_token` gerektirir.
- İlk yayım, güvenilmeyen paketler veya acil durum yayımları için `clawhub_token` kullanılabilir durumda tutun.
- Workflow, JSON sonucunu bir artefakt olarak yükler ve workflow çıktıları olarak sunar.
...
- Oturum açılmışken, `CLAWHUB_DISABLE_TELEMETRY=1` olmadığı sürece (`CLAWDHUB_DISABLE_TELEMETRY=1` eski), `sync` sırasında gönderilir.
