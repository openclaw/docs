...
summary: ClawHub oturum açma, API token’ları, CLI ile giriş, token saklama ve iptal etme.
x-i18n:
    generated_at: "2026-07-04T15:28:53Z"
    model: gpt-5.5
...
açma işlemini tamamlayamaz. Oturum açma sizi oturum kapalı duruma geri
döndürürse hesabınızın durumu iyi olmayabilir. Hesabınız yasaklandıysa veya
...
4. Tarayıcı yerel geri çağırmaya geri yönlendirilir.
5. CLI, tokenı ClawHub yapılandırma dosyanıza kaydeder.

Tarayıcınız güvenlik duvarı, VPN veya proxy kuralları nedeniyle yerel geri
çağırmaya ulaşamıyorsa başsız token akışını kullanın.
...
CI kurulumu için depolanan tokenı şununla yazdırın:
...
İptal edilmiş, geçersiz veya eksik tokenlar `401 Unauthorized` döndürür.
`clawhub login` ile yeniden oturum açın veya `clawhub login --token` ile yeni
bir token sağlayın.

Silinmiş, yasaklanmış veya devre dışı bırakılmış hesaplar mevcut API tokenlarını
