...
read_when:
    ...
    - 401'lerde Hata Ayıklama
summary: ClawHub oturum açma, API token'ları, CLI oturum açma, token depolama ve iptal etme.
x-i18n:
    generated_at: "2026-07-01T08:26:15Z"
    model: gpt-5.5
...
ClawHub, web oturumu açma için GitHub kullanır. CLI, oturum açılmış hesap
...
çağırmaya erişemiyorsa, başsız token akışını kullanın.

## Başsız oturum açma

ClawHub web kullanıcı arayüzünde bir token oluşturun, ardından bunu CLI'a
geçirin:
...
CI kurulumu için saklanan tokenı şu şekilde yazdırın:
...
İptal edilmiş, geçersiz veya eksik tokenlar `401 Unauthorized` döndürür. Yeniden
