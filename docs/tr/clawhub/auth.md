...
summary: ClawHub oturum açma, API tokenları, CLI oturum açma, token depolama ve iptal.
x-i18n:
...
ClawHub, web oturumu açma için GitHub kullanır. CLI, oturum açılmış hesap üzerinden oluşturulan ClawHub API belirteçlerini kullanır.

## Web oturumu açma

[clawhub.ai](https://clawhub.ai) adresinde GitHub ile oturum açın.

Silinmiş, yasaklanmış veya devre dışı bırakılmış hesaplar normal ClawHub oturum açma işlemini tamamlayamaz. Oturum açma sizi oturum kapalı durumuna döndürürse hesabınız iyi durumda olmayabilir.
...
3. GitHub oturumu açıldıktan sonra ClawHub bir API belirteci oluşturur.
...
İptal edilmiş, geçersiz veya eksik belirteçler `401 Unauthorized` döndürür. `clawhub login` ile tekrar oturum açın veya `clawhub login --token` ile yeni bir belirteç sağlayın.

Silinmiş, yasaklanmış veya devre dışı bırakılmış hesaplar mevcut API belirteçlerini kullanmaya devam edemez.
