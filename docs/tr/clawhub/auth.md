...
Oturum açma sizi oturum kapalı durumuna geri döndürürse, hesabınızın durumu iyi olmayabilir.
Hesabınız yasaklandıysa veya devre dışı bırakıldıysa, bunun bir hata olduğunu düşünüyorsanız
...
Tarayıcınız güvenlik duvarı, VPN veya proxy kuralları nedeniyle yerel geri çağırmaya ulaşamıyorsa,
başsız belirteç akışını kullanın.
...
İptal edilmiş, geçersiz veya eksik belirteçler `401 Unauthorized` döndürür. `clawhub login`
ile yeniden oturum açın veya `clawhub login --token` ile yeni bir belirteç sağlayın.
