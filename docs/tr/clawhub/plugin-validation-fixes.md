...
read_when:
    ...
    - ClawHub bir Plugin paketi yayımlamayı reddetti veya uyarı verdi
...
x-i18n:
    generated_at: "2026-06-30T14:20:11Z"
    model: gpt-5.5
...
ClawHub, Plugin paketlerini yayımlamadan önce doğrular ve otomatik paket taramalarından bulguları da gösterebilir. Bu sayfa, yazarın paket meta verilerinde, manifestinde, SDK içe aktarmalarında veya yayımlanmış yapıtında düzeltebileceği bulgular anlamına gelen, yazara yönelik bulguları kapsar.

İç Plugin Inspector kapsam bulgularını kapsamaz. Tam bir rapor, yazarın düzeltmesine yönelik rehberlik olmadan tarayıcı bakım kodları içeriyorsa, bunlar Plugin yazarlarından ziyade OpenClaw bakımcıları içindir.
...
| `channel-env-vars`                      | [Kanal env vars değerlerini geçerli meta verilerde yansıtın](/tr/clawhub/plugin-validation-fixes#channel-env-vars)                                |
...
Paket kökü `package.json` içermiyor, bu nedenle ClawHub npm paketini, sürümü, giriş noktalarını veya OpenClaw meta verilerini tanımlayamıyor.

- `name`, `version` ve `type` içeren bir `package.json` ekleyin.
- Paket bir OpenClaw Plugin gönderiyorsa bir `openclaw` bloğu ekleyin.
- Minimal bir paket örneği için [Plugin oluşturma](/tr/plugins/building-plugins), paket ile manifest ayrımı için [Plugin manifesti](/tr/plugins/manifest#manifest-versus-packagejson) bölümünü kullanın.
...
- Yayımlanmış paket derlenmiş JavaScript yüklemeliyse `openclaw.runtimeExtensions` ekleyin.
...
- `openclaw.install` değerini `clawhubSpec`, `npmSpec` veya `localPath` gibi desteklenen kurulum kaynağıyla doldurun.
...
- Buna karşı derleyip test ettiğiniz OpenClaw Plugin API sürümünü veya semver alt sınırını kullanın.
...
Paket minimum ana makine sürümü, paketin buna karşı derlendiği OpenClaw sürüm meta verileriyle eşleşmiyor.
...
- `openclaw.plugin.json` içinde de `version` varsa, eşleşecek şekilde güncelleyin veya paket meta verileri otorite ise eski manifest sürüm meta verilerini kaldırın.
...
- Paket giriş noktalarını, uyumluluk, kurulum, ayar ve katalog meta verilerini desteklenen `package.json#openclaw` alanlarında tutun.
- [Keşfi etkileyen package.json alanları](/tr/plugins/manifest#packagejson-fields-that-affect-discovery) bölümüne bakın.
- `clawhub package validate <path-to-plugin>` komutunu yeniden çalıştırın.
...
- Bu paket herkese açık yayınlama için tasarlandıysa `private: true` değerini kaldırın.
- `clawhub package validate <path-to-plugin>` komutunu yeniden çalıştırın.
...
- `npm pack --dry-run` komutunu çalıştırın ve dahil edilecek dosyaları inceleyin.
...
Paketlenmiş yapıt, kaynak paketinizde bulunan OpenClaw meta verilerini eksik bırakıyor.
...
- Paket meta verilerinin hariç tutulmaması için `files` veya `.npmignore` değerini güncelleyin.
...
Plugin bildiriminin üst düzeyinde OpenClaw tarafından desteklenmeyen alanlar var.
...
Plugin, paketlenmiş Plugin'lere veya dahili uyumluluğa ayrılmış bir SDK yolunu içe aktarıyor.
...
- `loadSessionStore(...)` öğesini yalnızca tanımladığınız uyumluluk aralığı hâlâ bunu gerektiren eski OpenClaw sürümlerini desteklerken koruyun.
...
- Tüm oturum deposu nesnesini yüklemekten, değiştirmekten ve kaydetmekten kaçının.
- Tüm depo yazma yardımcılarını yalnızca tanımladığınız uyumluluk aralığı hâlâ bunları gerektiren eski OpenClaw sürümlerini desteklerken koruyun.
- Bkz. [Çalışma zamanı API'si](/tr/plugins/sdk-runtime#agent-session-state) ve
  [Plugin SDK alt yolları](/tr/plugins/sdk-subpaths).
- `clawhub package validate <path-to-plugin>` komutunu yeniden çalıştırın.
...
- Eski yardımcıyı yalnızca tanımladığınız uyumluluk aralığı hâlâ bunu gerektiren eski OpenClaw sürümlerini desteklerken koruyun.
- Bkz. [Çalışma zamanı API'si](/tr/plugins/sdk-runtime#agent-session-state) ve
  [Plugin SDK alt yolları](/tr/plugins/sdk-subpaths).
- `clawhub package validate <path-to-plugin>` komutunu yeniden çalıştırın.
...
Plugin hâlâ `appendSessionTranscriptMessage` veya `emitSessionTranscriptUpdate` gibi kullanımdan kaldırılmış düşük seviyeli transcript yardımcıları kullanıyor.
...
- OpenClaw'ın doğru işlem sınırlarını ve kimlik işlemeyi uygulayabilmesi için yapılandırılmış transcript çalışma zamanı yüzeyini tercih edin.
- Düşük seviyeli transcript yardımcılarını yalnızca tanımladığınız uyumluluk aralığı hâlâ bunları gerektiren eski OpenClaw sürümlerini desteklerken koruyun.
- Bkz. [Çalışma zamanı API'si](/tr/plugins/sdk-runtime#agent-session-state) ve
  [Plugin SDK alt yolları](/tr/plugins/sdk-subpaths).
- `clawhub package validate <path-to-plugin>` komutunu yeniden çalıştırın.
...
- `before_agent_start` öğesini yalnızca tanımladığınız uyumluluk aralığı hâlâ bunu gerektiren eski OpenClaw sürümlerini desteklerken koruyun.
...
- `providerAuthEnvVars` öğesini yalnızca desteklediğiniz OpenClaw aralığı hâlâ buna ihtiyaç duyarken uyumluluk meta verisi olarak koruyun.
...
Bildirim, ClawHub'ın beklediği güncel setup veya config meta verileri olmadan eski veya daha eski kanal env-var meta verileri kullanıyor.

- Kanal env-var meta verilerini bildirime dayalı tutun; böylece OpenClaw kanal çalışma zamanını yüklemeden setup durumunu inceleyebilir.
- Env odaklı kanal kurulumunu Plugin şeklinizin kullandığı güncel setup, kanal config veya paket kanal meta verilerine yansıtın.
- `channelEnvVars` öğesini yalnızca desteklenen eski OpenClaw sürümleri hâlâ bunu gerektirirken uyumluluk meta verisi olarak koruyun.
...
- Belgelenmiş sürümlü bir şemayı yalnızca OpenClaw bunu yayımladıktan sonra kullanın.
- `clawhub package validate <path-to-plugin>` komutunu yeniden çalıştırın.
...
- OpenClaw sürümlü bir güvenlik bildirimi şemasını ve ClawHub davranışını belgeleyene kadar `openclaw.security.json` dosyasını kaldırın.
- Bildirim sözleşmesi var olana kadar güvenlik açısından hassas davranışı herkese açık paket belgelerinizde veya README dosyanızda belgeleyin.
- `clawhub package validate <path-to-plugin>` komutunu yeniden çalıştırın.
