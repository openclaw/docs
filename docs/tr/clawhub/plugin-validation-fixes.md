...
ClawHub, yayımdan önce Plugin paketlerini doğrular ve otomatik paket taramalarından
gelen bulguları da gösterebilir. Bu sayfa, yazara yönelik bulguları kapsar; yani
Plugin yazarının paket meta verilerinde, manifestinde, SDK içe aktarmalarında
veya yayımlanmış artifaktında düzeltebileceği bulguları.

İç Plugin Inspector kapsam bulgularını kapsamaz. Tam bir raporda yazarın
gidermesine yönelik rehberlik içermeyen tarayıcı bakım kodları varsa, bunlar
...
| `package-npm-pack-unavailable`          | [npm artifaktını paketlenebilir yapın](/tr/clawhub/plugin-validation-fixes#package-npm-pack-unavailable)                                 |
...
ClawHub paketin nasıl kurulacağını veya güncelleneceğini anlayamaz.

- `openclaw.install` öğesini `clawhubSpec`, `npmSpec` veya `localPath` gibi
...
- Karşısında geliştirdiğiniz ve test ettiğiniz OpenClaw Plugin API sürümünü
  veya semver taban sürümünü kullanın.
...
- `openclaw.install.minHostVersion` öğesini kontrol edin.
...
## Yayımlanmış artifakt

### package-npm-pack-unavailable

Paket, ClawHub’ın inceleyeceği veya yayımlayacağı artifakta paketlenemiyor.
...
- Bu paket herkese açık yayımlama için tasarlandıysa `private: true` öğesini
  kaldırın.
- `clawhub package validate <path-to-plugin>` komutunu yeniden çalıştırın.

### package-npm-pack-entrypoint-missing

Paket paketlenebiliyor, ancak paketlenmiş artifakt `package.json#openclaw`
...
Paketlenmiş artifaktta kaynak paketinizde bulunan OpenClaw meta verileri eksik.

- `npm pack --dry-run` çalıştırın ve dahil edilen meta veri dosyalarını
  inceleyin.
- Paketlenmiş artifaktta `package.json` dosyasının `openclaw` bloğunu
  içerdiğinden emin olun.
- Paket yerel bir OpenClaw Plugin olduğunda `openclaw.plugin.json` dosyasının
...
- Oturum meta verilerini aracıya ve oturum kimliğine göre okumak için
...
- Kod bir transcript işlemi hazırlıyorsa transcript kimliği veya hedef yardımcılarını
  kullanın.
...
- `before_agent_start` değerini yalnızca bildirdiğiniz uyumluluk aralığı hâlâ bunu
  gerektiren eski OpenClaw sürümlerini desteklerken tutun.
...
Manifest, ClawHub'ın beklediği mevcut setup veya config meta verileri olmadan
eski veya daha eski kanal ortam değişkeni meta verilerini kullanıyor.
...
- Ortam değişkeniyle yönlendirilen kanal setup'ını Plugin şeklinizin kullandığı
  mevcut setup, kanal config veya paket kanal meta verilerine yansıtın.
...
- OpenClaw sürümlü bir güvenlik manifesti şemasını ve ClawHub davranışını
  belgeleyene kadar `openclaw.security.json` dosyasını kaldırın.
- Manifest sözleşmesi var olana kadar güvenlik açısından hassas davranışı genel paket
