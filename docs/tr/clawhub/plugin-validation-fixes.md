...
x-i18n:
    generated_at: "2026-07-03T23:40:39Z"
    model: gpt-5.5
...
bulguları kapsar; yani Plugin yazarının kendi paket meta verilerinde,
manifestinde, SDK importlarında veya yayımlanmış artefaktında düzeltebileceği
bulguları.

İç Plugin Inspector kapsam bulgularını kapsamaz. Tam bir raporda yazar için
düzeltme rehberi olmayan tarayıcı bakım kodları varsa, bunlar Plugin yazarları
...
| `legacy-root-sdk-import`                | [Kök SDK importlarını değiştirin](/tr/clawhub/plugin-validation-fixes#legacy-root-sdk-import)                                   |
| `reserved-sdk-import`                   | [Ayrılmış SDK importlarını kaldırın](/tr/clawhub/plugin-validation-fixes#reserved-sdk-import)                                   |
| `sdk-load-session-store`                | [Tam oturum deposu erişimini değiştirin](/tr/clawhub/plugin-validation-fixes#sdk-load-session-store)                            |
| `sdk-session-store-write`               | [Tam oturum deposu yazımlarını değiştirin](/tr/clawhub/plugin-validation-fixes#sdk-session-store-write)                         |
...
Paket kökü `package.json` içermiyor, bu nedenle ClawHub npm paketini,
sürümü, giriş noktalarını veya OpenClaw meta verilerini tanımlayamaz.

- `name`, `version` ve `type` içeren `package.json` ekleyin.
...
- Birden fazla kurulum kaynağı mevcut olduğunda
  `openclaw.install.defaultChoice` ayarlayın.
...
Paketin minimum ana makine sürümü, paketin üzerine inşa edildiği OpenClaw
sürüm meta verileriyle eşleşmiyor.

- `openclaw.install.minHostVersion` değerini kontrol edin.
- Paketteki OpenClaw derleme meta verilerini, örneğin sürüm sırasında kullanılan
  OpenClaw sürümünü, kontrol edin.
...
`package.json#openclaw` bloğu, desteklenmeyen OpenClaw paket meta verileri olan
alanlar içeriyor.
...
Paketlenmiş artefakt, kaynak paketinizde bulunan OpenClaw meta verilerini
içermiyor.
...
- Kök barrel içe aktarımlarını odaklı herkese açık alt yol içe aktarımlarıyla değiştirin.
...
- Eski transkript dosya yollarını kalıcı hale getirmeyin veya bunlara bağlı kalmayın.
- Bkz. [Çalışma zamanı API'si](/tr/plugins/sdk-runtime#agent-session-state) ve
  [Plugin SDK alt yolları](/tr/plugins/sdk-subpaths).
- `clawhub package validate <path-to-plugin>` komutunu yeniden çalıştırın.
...
- Eski yardımcıyı yalnızca bildirdiğiniz uyumluluk aralığı hâlâ
  bunu gerektiren eski OpenClaw sürümlerini destekliyorsa tutun.
- Bkz. [Çalışma zamanı API'si](/tr/plugins/sdk-runtime#agent-session-state) ve
  [Plugin SDK alt yolları](/tr/plugins/sdk-subpaths).
- `clawhub package validate <path-to-plugin>` komutunu yeniden çalıştırın.
...
kullanımdan kaldırılmış düşük seviyeli transkript yardımcıları kullanıyor.
...
- `before_agent_start` işlevini yalnızca bildirdiğiniz uyumluluk aralığı hâlâ
  bunu gerektiren eski OpenClaw sürümlerini destekliyorsa tutun.
...
Manifest, ClawHub'ın beklediği güncel setup veya yapılandırma meta verileri olmadan
eski ya da daha eski kanal env-var meta verileri kullanıyor.

- Kanal env-var meta verilerini bildirime dayalı tutun; böylece OpenClaw kanal çalışma zamanını yüklemeden
  setup durumunu inceleyebilir.
- Env ile yönlendirilen kanal setup'ını Plugin şeklinizin kullandığı güncel setup, kanal yapılandırması veya
  paket kanal meta verilerine yansıtın.
- `channelEnvVars` değerini yalnızca desteklenen eski OpenClaw sürümleri hâlâ
  bunu gerektiriyorsa uyumluluk meta verisi olarak tutun.
...
Paket, ClawHub'ın kullanılabilir olarak tanımadığı bir şema başvurusu içeren
...
Paket desteklenmeyen bir güvenlik manifestosu dosyası gönderiyor.

- OpenClaw sürümlü bir güvenlik manifestosu şeması ve ClawHub davranışı belgeleyene kadar
