...
summary: Yayınlamadan önce ClawHub Plugin paketi doğrulama bulgularını düzelt
title: Plugin doğrulama düzeltmeleri
x-i18n:
    generated_at: "2026-07-04T04:02:26Z"
    model: gpt-5.5
...
bulguları da gösterebilir. Bu sayfa, yazarın görebileceği bulguları kapsar; yani
Plugin yazarının paket meta verilerinde, manifest dosyasında, SDK içe aktarmalarında
veya yayımlanan yapıtında düzeltebileceği bulguları.

Dahili Plugin Inspector kapsam bulgularını kapsamaz. Tam bir raporda yazarın
uygulayabileceği düzeltme yönlendirmesi olmayan tarayıcı bakım kodları varsa, bunlar
...
| `sdk-session-transcript-file-target`    | [Eski transkript dosya hedeflerini değiştirin](/tr/clawhub/plugin-validation-fixes#sdk-session-transcript-file-target)                   |
...
Paketin `package.json` dosyası var, ancak OpenClaw paket meta verilerini
bildirmiyor.
...
Paket meta verileri mevcut, ancak bir OpenClaw çalışma zamanı giriş noktasını
bildirmiyor.
...
- Üzerine derleyip test ettiğiniz OpenClaw Plugin API sürümünü veya semver
  tabanını kullanın.
...
`package.json#openclaw` bloğu, OpenClaw paket meta verisi olarak desteklenmeyen
alanlar içeriyor.
...
- Paket meta verilerinin dışlanmaması için `files` veya `.npmignore` dosyasını güncelleyin.
...
- `name` alanını insanlar tarafından okunabilir tutun ve `id` alanını kararlı makine kimliği olarak bırakın.
...
- Davranışın genel bir SDK karşılığı yoksa yardımcıyı kendi paketinizin içinde tutun veya
...
- Kod yapılandırılmış bir transkript işlem hedefine ihtiyaç duyduğunda
...
- Eski yardımcıyı yalnızca bildirdiğiniz uyumluluk aralığı bunu gerektiren eski
  OpenClaw sürümlerini hâlâ destekliyorsa tutun.
- Bkz. [Çalışma zamanı API’si](/tr/plugins/sdk-runtime#agent-session-state) ve
  [Plugin SDK alt yolları](/tr/plugins/sdk-subpaths).
- `clawhub package validate <path-to-plugin>` komutunu yeniden çalıştırın.
...
kullanımdan kaldırılmış düşük düzey transkript yardımcılarını kullanıyor.
...
- Model veya sağlayıcı geçersiz kılma çalışmasını `before_model_resolve` öğesine taşıyın.
- Prompt veya bağlam değiştirme çalışmasını `before_prompt_build` öğesine taşıyın.
- `before_agent_start` öğesini yalnızca bildirdiğiniz uyumluluk aralığı bunu gerektiren eski
  OpenClaw sürümlerini hâlâ destekliyorsa tutun.
...
- `providerAuthEnvVars` öğesini yalnızca desteklenen OpenClaw aralığınız
  hâlâ buna ihtiyaç duyuyorsa uyumluluk meta verisi olarak tutun.
...
- Kanal env-var meta verilerini bildirimsel tutun; böylece OpenClaw kanal çalışma zamanını
  yüklemeden setup durumunu inceleyebilir.
- Env odaklı kanal setup’ını Plugin biçiminizin kullandığı mevcut setup, kanal yapılandırması
  veya paket kanal meta verilerine yansıtın.
...
- Manifest contract’ı var olana kadar güvenlik açısından hassas davranışı genel paket
  belgelerinizde veya README dosyanızda belgelendirin.
- `clawhub package validate <path-to-plugin>` komutunu yeniden çalıştırın.
