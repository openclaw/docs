...
read_when:
    ...
    - ClawHub bir Plugin paketi yayımını reddetti veya uyarı verdi
...
x-i18n:
    generated_at: "2026-06-28T08:16:53Z"
    model: gpt-5.5
...
ClawHub, Plugin paketlerini yayımlamadan önce doğrular ve otomatik paket taramalarından gelen bulguları da gösterebilir. Bu sayfa, yazarın paket meta verilerinde, manifestinde, SDK içe aktarmalarında veya yayımlanan yapıtında düzeltebileceği bulgular anlamına gelen yazar odaklı bulguları kapsar.

İç Plugin Inspector kapsam bulgularını kapsamaz. Tam bir rapor, yazar için düzeltme rehberliği olmayan tarayıcı bakım kodları içeriyorsa, bunlar Plugin yazarlarından çok OpenClaw bakımcıları içindir.
...
Paket kökü `package.json` içermez, bu nedenle ClawHub npm paketini, sürümü, giriş noktalarını veya OpenClaw meta verilerini belirleyemez.
...
- Geliştirip test ettiğiniz OpenClaw Plugin API sürümünü veya semver alt sınırını kullanın.
...
Paketin en düşük ana makine sürümü, paketin temel alınarak derlendiği OpenClaw sürüm meta verileriyle eşleşmiyor.

- `openclaw.install.minHostVersion` değerini kontrol edin.
- Paketteki, örneğin sürüm sırasında kullanılan OpenClaw sürümü gibi OpenClaw derleme meta verilerini kontrol edin.
- En düşük ana makine sürümünü paketin gerçekten desteklediği ana makine sürüm aralığıyla hizalayın.
- [Keşfi etkileyen package.json alanları](/tr/plugins/manifest#packagejson-fields-that-affect-discovery) bölümüne bakın.
- `clawhub package validate <path-to-plugin>` komutunu yeniden çalıştırın.
...
- `openclaw.plugin.json` içinde de `version` varsa, eşleşecek şekilde güncelleyin veya paket meta verileri yetkiliyse eski manifest sürüm meta verilerini kaldırın.
...
- Paket giriş noktalarını, uyumluluğu, kurulumu, ayarlamayı ve katalog meta verilerini desteklenen `package.json#openclaw` alanlarında tutun.
- [Keşfi etkileyen package.json alanları](/tr/plugins/manifest#packagejson-fields-that-affect-discovery) bölümüne bakın.
- `clawhub package validate <path-to-plugin>` komutunu yeniden çalıştırın.
...
- Paketlemeyi başarısız kılan geçersiz paket meta verilerini, bozuk yaşam döngüsü betiklerini veya dosya girdilerini düzeltin.
...
Yerel Plugin manifesti bir görüntü adı içermiyor.

- `openclaw.plugin.json` dosyasına boş olmayan bir `name` alanı ekleyin.
- `name` alanını insanlar tarafından okunabilir tutun ve `id` alanını kararlı makine kimliği olarak tutun.
- [Plugin manifesti](/tr/plugins/manifest) bölümüne bakın.
- `clawhub package validate <path-to-plugin>` komutunu yeniden çalıştırın.
...
- Kök barrel içe aktarmalarını odaklanmış genel subpath içe aktarmalarıyla değiştirin.
...
- Dar içe aktarmayı bulmak için [İçe aktarma kuralları](/tr/plugins/building-plugins#import-conventions) ve
  [Plugin SDK subpath'leri](/tr/plugins/sdk-subpaths) bölümlerini kullanın.
- `clawhub package validate <path-to-plugin>` komutunu yeniden çalıştırın.

### reserved-sdk-import

Plugin, bundled Plugin'lere veya dahili uyumluluğa ayrılmış bir SDK yolundan
içe aktarıyor.

- Ayrılmış OpenClaw dahili SDK içe aktarmalarını belgelenmiş genel
  `openclaw/plugin-sdk/*` subpath'leriyle değiştirin.
- Davranış için genel bir SDK yoksa yardımcıyı kendi paketinizin içinde tutun veya
  genel bir OpenClaw API isteyin.
- Desteklenen bir içe aktarma seçmek için [Plugin SDK subpath'leri](/tr/plugins/sdk-subpaths) ve
...
- [Runtime API](/tr/plugins/sdk-runtime#agent-session-state) ve
  [Plugin SDK subpath'leri](/tr/plugins/sdk-subpaths) bölümlerine bakın.
- `clawhub package validate <path-to-plugin>` komutunu yeniden çalıştırın.
...
- `providerAuthEnvVars` değerini yalnızca desteklediğiniz OpenClaw aralığı hâlâ
  buna ihtiyaç duyuyorsa uyumluluk meta verisi olarak tutun.
...
- Kanal env-var meta verilerini bildirime dayalı tutun; böylece OpenClaw kanal runtime'ını
...
- `channelEnvVars` değerini yalnızca desteklenen eski OpenClaw sürümleri hâlâ
  gerektiriyorsa uyumluluk meta verisi olarak tutun.
...
- Manifest contract'ı var olana kadar güvenlik açısından hassas davranışı genel paket belgelerinizde veya
  README dosyanızda belgelemeye devam edin.
- `clawhub package validate <path-to-plugin>` komutunu yeniden çalıştırın.
