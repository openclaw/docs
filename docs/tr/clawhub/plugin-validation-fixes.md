...
x-i18n:
    generated_at: "2026-07-03T01:02:38Z"
    model: gpt-5.5
...
manifestinde, SDK içe aktarımlarında veya yayımlanan artefaktında
düzeltebileceği bulgular.

Bu sayfa, dahili Plugin Inspector kapsam bulgularını kapsamaz. Tam raporda
yazarın uygulayabileceği giderim yönergeleri olmadan tarayıcı bakım kodları
bulunuyorsa, bunlar Plugin yazarları için değil OpenClaw bakımcıları içindir.
...
- Minimal paket örneği için [Plugin oluşturma](/tr/plugins/building-plugins)
  ...
  [Plugin manifesti](/tr/plugins/manifest#manifest-versus-packagejson) sayfasını
  kullanın.
...
Paketin minimum ana makine sürümü, paketin kendisine göre derlendiği OpenClaw
...
- `openclaw.plugin.json` içinde de `version` varsa, paket meta verileri yetkili
  kayıtsa onu eşleşecek şekilde güncelleyin veya eski manifest sürüm meta
  verilerini kaldırın.
- Yayımlanan meta verileri değiştirdikten sonra yeni bir paket sürümü
  yayımlayın.
...
- Paket giriş noktalarını, uyumluluğu, kurulumu, kurulumu başlatma ve katalog
...
- Paketlemeyi başarısız yapan geçersiz paket meta verilerini, bozuk yaşam
...
- `npm pack --dry-run` çalıştırın ve dahil edilen meta veri dosyalarını
  inceleyin.
- Paketlenen artefakta `package.json` dosyasının `openclaw` bloğunu içerdiğini
  doğrulayın.
...
Plugin, paketlenmiş Plugin'ler veya dahili
...
- Davranış için herkese açık SDK yoksa yardımcıyı paketinizin içinde tutun veya
...
- `providerAuthEnvVars` öğesini yalnızca desteklenen
  OpenClaw aralığınız buna hâlâ ihtiyaç duyarken uyumluluk meta verisi olarak tutun.
...
- Kanal env-var meta verilerini bildirime dayalı tutun; böylece OpenClaw kanal çalışma zamanını
  yüklemeden setup durumunu inceleyebilir.
- Env ile yönlendirilen kanal setup'ını Plugin şeklinizin kullandığı güncel setup, kanal config veya
  paket kanal meta verilerine yansıtın.
- `channelEnvVars` öğesini yalnızca desteklenen eski
  OpenClaw sürümleri bunu hâlâ gerektiriyorsa uyumluluk meta verisi olarak tutun.
...
- Yalnızca OpenClaw bir tane yayımladıktan sonra belgelenmiş sürümlü şema kullanın.
- `clawhub package validate <path-to-plugin>` komutunu yeniden çalıştırın.
...
- Manifest sözleşmesi mevcut olana kadar güvenlik açısından hassas davranışı herkese açık paket dokümanlarınızda veya
  README dosyanızda belgelendirin.
- `clawhub package validate <path-to-plugin>` komutunu yeniden çalıştırın.
