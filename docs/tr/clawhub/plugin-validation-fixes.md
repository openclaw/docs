...
x-i18n:
    generated_at: "2026-07-03T09:55:32Z"
    model: gpt-5.5
...
SDK içe aktarımlarında veya yayımlanmış yapıtında düzeltebileceği bulguları.

Dahili Plugin Inspector kapsam bulgularını kapsamaz. Tam bir rapor, yazarın
gidermesine yönelik rehberlik olmadan tarayıcı bakım kodları içeriyorsa bunlar
...
| `provider-auth-env-vars`                | [Sağlayıcı env var öğelerini kurulum meta verilerine taşıyın](/tr/clawhub/plugin-validation-fixes#provider-auth-env-vars)      |
| `channel-env-vars`                      | [Kanal env var öğelerini geçerli meta verilerde yansıtın](/tr/clawhub/plugin-validation-fixes#channel-env-vars)                |
...
Pakette `package.json` var, ancak OpenClaw paket meta verileri bildirilmiyor.
...
- Birden fazla kurulum kaynağı varsa `openclaw.install.defaultChoice` ayarını yapın.
...
Paketin minimum host sürümü, paketin hedef alınarak derlendiği OpenClaw sürüm
...
- `openclaw.plugin.json` içinde de `version` varsa, eşleşecek şekilde güncelleyin
  veya paket meta verileri yetkili kaynaksa eski manifest sürüm meta verilerini
  kaldırın.
...
Paket, ClawHub tarafından incelenecek veya yayımlanacak yapıta
paketlenemiyor.
...
Paketlenmiş yapıt, kaynak paketinizde bulunan OpenClaw meta verilerini
içermiyor.

- `npm pack --dry-run` komutunu çalıştırın ve dahil edilen meta veri dosyalarını inceleyin.
- Paketlenmiş yapıt içinde `package.json` dosyasının `openclaw` bloğunu
...
- Düşük düzey transcript yardımcılarını yalnızca bildirdiğiniz uyumluluk aralığı hâlâ
  bunları gerektiren eski OpenClaw sürümlerini desteklerken tutun.
- Bkz. [Çalışma zamanı API'si](/tr/plugins/sdk-runtime#agent-session-state) ve
  [Plugin SDK alt yolları](/tr/plugins/sdk-subpaths).
- `clawhub package validate <path-to-plugin>` komutunu yeniden çalıştırın.
...
- OpenClaw'ın kanal çalışma zamanını yüklemeden setup durumunu inceleyebilmesi için
  kanal ortam değişkeni meta verilerini bildirime dayalı tutun.
- Ortam değişkeni odaklı kanal setup'ını Plugin şeklinizin kullandığı güncel setup,
  kanal config'i veya paket kanal meta verilerine yansıtın.
...
- Güvenlik açısından hassas davranışı, manifest contract'ı var olana kadar herkese açık paket dokümanlarınızda veya
  README dosyanızda belgeli tutun.
- `clawhub package validate <path-to-plugin>` komutunu yeniden çalıştırın.
