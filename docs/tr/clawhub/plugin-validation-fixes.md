...
x-i18n:
    generated_at: "2026-07-05T08:01:27Z"
    model: gpt-5.5
...
ClawHub, Plugin paketlerini yayımlamadan önce doğrular ve otomatik paket taramalarından gelen bulguları da gösterebilir. Bu sayfa, yazara dönük bulguları kapsar; yani Plugin yazarının kendi paket meta verilerinde, manifestinde, SDK içe aktarımlarında veya yayımlanmış yapıtında düzeltebileceği bulguları.

İç Plugin Inspector kapsam bulgularını kapsamaz. Tam bir rapor, yazar için düzeltme rehberi olmadan tarayıcı bakım kodları içeriyorsa, bunlar Plugin yazarlarından ziyade OpenClaw bakımcıları içindir.
...
- Minimal paket örneği için [Plugin oluşturma](/tr/plugins/building-plugins) sayfasını ve paket ile manifest ayrımı için [Plugin manifesti](/tr/plugins/manifest#manifest-versus-packagejson) sayfasını kullanın.
...
- Paket ClawHub üzerinden yayımlanacak veya kurulacaksa uyumluluk ve kurulum meta verileri ekleyin.
...
- Yayımlanan paketin oluşturulmuş JavaScript'i yüklemesi gerekiyorsa `openclaw.runtimeExtensions` ekleyin.
...
- Birden fazla kurulum kaynağı mevcutsa `openclaw.install.defaultChoice` ayarlayın.
...
- Üzerine geliştirme ve test yaptığınız OpenClaw Plugin API sürümünü veya semver taban sürümünü kullanın.
- Bunu paket sürümünden ayrı tutun. Paket sürümü Plugin yayınını açıklar; `openclaw.compat.pluginApi` ise ana makine API sözleşmesini açıklar.
- [Keşfi etkileyen package.json alanları](/tr/plugins/manifest#packagejson-fields-that-affect-discovery) bölümüne bakın.
- `clawhub package validate <path-to-plugin>` komutunu yeniden çalıştırın.

### package-min-host-version-drift

Paketin minimum ana makine sürümü, paketin derlendiği OpenClaw sürüm meta verileriyle eşleşmiyor.

- `openclaw.install.minHostVersion` değerini denetleyin.
- Paketteki, örneğin yayın sırasında kullanılan OpenClaw sürümü gibi OpenClaw derleme meta verilerini denetleyin.
...
- Paketlemenin başarısız olmasına neden olan geçersiz paket meta verilerini, bozuk yaşam döngüsü betiklerini veya files girdilerini düzeltin.
- Bu paket genel yayımlama için amaçlanıyorsa `private: true` değerini kaldırın.
- `clawhub package validate <path-to-plugin>` komutunu yeniden çalıştırın.
...
Paketlenmiş yapıt, kaynak paketinizde bulunan OpenClaw meta verilerinden yoksun.
...
- Çalışma zamanı davranışını Plugin kayıt koduna taşıyın ve `contracts` öğesini statik yetenek sahipliği meta verileriyle sınırlı tutun.
- `clawhub package validate <path-to-plugin>` komutunu yeniden çalıştırın.
...
Plugin, paketlenmiş Plugin'ler veya iç uyumluluk için ayrılmış bir SDK yolundan içe aktarım yapıyor.

- Ayrılmış OpenClaw dahili SDK içe aktarımlarını belgelenmiş genel
  `openclaw/plugin-sdk/*` alt yollarıyla değiştirin.
- Davranış için genel bir SDK yoksa yardımcıyı paketinizin içinde tutun veya genel bir OpenClaw API isteyin.
...
- `loadSessionStore(...)` öğesini yalnızca bildirdiğiniz uyumluluk aralığı hâlâ bunu gerektiren eski OpenClaw sürümlerini desteklediği sürece tutun.
...
- Tüm oturum deposu nesnesini yüklemekten, değiştirmekten ve kaydetmekten kaçının.
- Tüm depo yazma yardımcılarını yalnızca bildirdiğiniz uyumluluk aralığı hâlâ bunları gerektiren eski OpenClaw sürümlerini desteklediği sürece tutun.
- Bkz. [Çalışma zamanı API'si](/tr/plugins/sdk-runtime#agent-session-state) ve
  [Plugin SDK alt yolları](/tr/plugins/sdk-subpaths).
- `clawhub package validate <path-to-plugin>` komutunu yeniden çalıştırın.
...
- Eski yardımcıyı yalnızca bildirdiğiniz uyumluluk aralığı hâlâ bunu gerektiren eski OpenClaw sürümlerini desteklediği sürece tutun.
- Bkz. [Çalışma zamanı API'si](/tr/plugins/sdk-runtime#agent-session-state) ve
  [Plugin SDK alt yolları](/tr/plugins/sdk-subpaths).
- `clawhub package validate <path-to-plugin>` komutunu yeniden çalıştırın.
...
Plugin hâlâ `appendSessionTranscriptMessage` veya `emitSessionTranscriptUpdate` gibi kullanımdan kaldırılmış düşük seviyeli transkript yardımcıları kullanıyor.
...
- Düşük seviyeli transkript yardımcılarını yalnızca bildirdiğiniz uyumluluk aralığı hâlâ bunları gerektiren eski OpenClaw sürümlerini desteklediği sürece tutun.
- Bkz. [Çalışma zamanı API'si](/tr/plugins/sdk-runtime#agent-session-state) ve
  [Plugin SDK alt yolları](/tr/plugins/sdk-subpaths).
- `clawhub package validate <path-to-plugin>` komutunu yeniden çalıştırın.
...
- `before_agent_start` öğesini yalnızca bildirdiğiniz uyumluluk aralığı hâlâ bunu gerektiren eski OpenClaw sürümlerini desteklediği sürece tutun.
...
- `providerAuthEnvVars` öğesini yalnızca desteklenen OpenClaw aralığınız hâlâ buna ihtiyaç duyduğu sürece uyumluluk meta verisi olarak tutun.
...
Manifest, ClawHub'ın beklediği güncel setup veya yapılandırma meta verileri olmadan eski veya daha eski kanal ortam değişkeni meta verileri kullanıyor.

- Kanal ortam değişkeni meta verilerini bildirime dayalı tutun; böylece OpenClaw, kanal çalışma zamanını yüklemeden setup durumunu inceleyebilir.
- Ortam değişkeni odaklı kanal setup'ını Plugin şeklinizin kullandığı güncel setup, kanal yapılandırması veya paket kanal meta verilerine yansıtın.
- `channelEnvVars` öğesini yalnızca desteklenen eski OpenClaw sürümleri hâlâ bunu gerektirdiği sürece uyumluluk meta verisi olarak tutun.
...
Paket, ClawHub'ın kullanılabilir olarak tanımadığı bir şema başvurusuna sahip `openclaw.security.json` gönderiyor.
...
- Manifest contract'ı var olana kadar güvenlik açısından hassas davranışı genel paket belgelerinizde veya README dosyanızda belgeleyin.
- `clawhub package validate <path-to-plugin>` komutunu yeniden çalıştırın.
