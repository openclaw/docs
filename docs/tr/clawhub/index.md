...
title: ClawHub
x-i18n:
    generated_at: "2026-05-12T00:57:33Z"
    model: gpt-5.5
    provider: openai
...
- Kayıt defteri kimlik doğrulaması, yayımlama, silme/silmeyi geri alma ve eşitleme iş akışları için ayrı `clawhub` CLI'sını kullanın.
...
Yayımlama, eşitleme veya silme/silmeyi geri alma gibi kayıt defteri kimliği doğrulanmış iş akışları istediğinizde ClawHub CLI'sını yükleyin:
...
ClawHub semver sürümlerini, `latest` gibi etiketleri, değişiklik günlüklerini, dosyaları, indirmeleri, yıldızları ve güvenlik taraması özetlerini izler. Herkese açık sayfalar, kullanıcıların yüklemeden önce bir Skill'i veya Plugin'i inceleyebilmesi için mevcut kayıt defteri durumunu gösterir.
...
Bir Plugin yüklemesinin ClawHub üzerinden çözümlenmesi gerektiğinde `clawhub:<package>` kullanın. Düz npm uyumlu Plugin belirtimleri, yayın geçişleri sırasında npm üzerinden çözümlenebilir ve bir kaynağın açık olması gerektiğinde `npm:<package>` yalnızca npm olarak kalır.
...
CLI, doğrudan kayıt defteri iş akışları için Skill yükleme/güncelleme komutlarına da sahiptir:
...
Kod Plugin'leri, `package.json` içinde `openclaw.compat.pluginApi` ve `openclaw.build.openclawVersion` dahil olmak üzere gerekli OpenClaw uyumluluk meta verilerini içermelidir. Tam komut başvurusu için [CLI](/tr/clawhub/cli), Skill meta verileri için [Skill biçimi](/tr/clawhub/skill-format) bölümüne bakın.

## Güvenlik ve moderasyon

ClawHub varsayılan olarak açıktır: herkes yükleme yapabilir, ancak yayımlama için yükleme geçidinden geçecek kadar eski bir GitHub hesabı gerekir. Herkese açık ayrıntı sayfaları, yükleme veya indirme öncesinde en son tarama durumunu özetler.

ClawHub, yayımlanan Skills ve Plugin sürümleri üzerinde otomatik denetimler çalıştırır. Taramada bekletilen veya engellenen sürümler, `/dashboard` içinde sahiplerine görünür kalırken herkese açık katalogdan ve yükleme yüzeylerinden kaybolabilir.
...
| `CLAWHUB_CONFIG_PATH`         | CLI'nın token/yapılandırma durumunu depoladığı konumu geçersiz kılar. |
