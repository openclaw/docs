...
title: ClawHub
x-i18n:
    generated_at: "2026-06-28T08:16:06Z"
    model: gpt-5.5
...
Yayımlama veya silme/silmeyi geri alma gibi kayıt deposu kimlik doğrulamalı
iş akışları istediğinizde ClawHub CLI'ını kurun:
...
durumunu gösterir; böylece kullanıcılar bir Skill veya Plugin'i kurmadan önce inceleyebilir.

## Yerel OpenClaw akışları

Yerel OpenClaw komutları etkin OpenClaw çalışma alanına kurar ve daha sonraki
güncelleme komutlarının ClawHub'da kalabilmesi için kaynak meta verilerini kalıcı hale getirir.

Bir Plugin kurulumunun ClawHub üzerinden çözümlenmesi gerektiğinde `clawhub:<package>` kullanın.
Yalın npm güvenli Plugin tanımları, başlatma geçişleri sırasında npm üzerinden çözümlenebilir ve
...
CLI ayrıca doğrudan kayıt deposu iş akışları için Skill kurma/güncelleme komutlarına sahiptir:
...
Bu komutlar, Skills'i geçerli çalışma dizini altında `./skills` içine kurar
...
[Skill formatı](/tr/clawhub/skill-format) sayfasına bakın.

## Güvenlik ve moderasyon

ClawHub varsayılan olarak açıktır: herkes yükleme yapabilir, ancak yayımlama için yükleme geçidini
geçecek kadar eski bir GitHub hesabı gerekir. Herkese açık ayrıntı sayfaları, kurulum veya indirme öncesinde
...
veya engellenen sürümler, sahipleri için `/dashboard` içinde görünür kalırken herkese açık katalog ve kurulum
...
Oturum açmışken `clawhub install` çalıştırdığınızda CLI, ClawHub'ın toplu kurulum sayılarını hesaplayabilmesi için
en iyi çabayla bir kurulum olayı gönderebilir. Bunu şu şekilde devre dışı bırakın:
