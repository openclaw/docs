...
summary: Herkese açık ClawHub keşif, kurulum, yayımlama, güvenlik ve clawhub CLI genel bakışı.
title: ClawHub
x-i18n:
...
Yayımlama, eşitleme veya silme/silmeyi geri alma gibi kayıt kimliği doğrulanmış
iş akışları istediğinizde ClawHub CLI'yi yükleyin:
...
indirmeleri, yıldızları ve güvenlik taraması özetlerini izler. Herkese açık sayfalar güncel kayıt
...
uyumluluğunu doğrular. Bir paket sürümü bir ClawPack yapıtı yayımladığında, OpenClaw tam olarak yüklenmiş npm-pack `.tgz` dosyasını tercih eder,
ClawHub özet başlığını ve indirilen baytları doğrular ve sonraki güncellemeler için yapıt meta verilerini kaydeder.
...
yükleme geçidini geçecek kadar eski bir GitHub hesabı gerekir. Herkese açık ayrıntı sayfaları,
yükleme veya indirme öncesinde en son tarama durumunu özetler.

ClawHub, yayımlanan Skills ve Plugin sürümleri üzerinde otomatik kontroller çalıştırır. Tarama nedeniyle tutulan
...
Oturum açmışken `clawhub sync` çalıştırdığınızda, CLI ClawHub'ın yükleme sayılarını hesaplayabilmesi için
en küçük kapsamlı bir anlık görüntü gönderir. Bunu şu şekilde devre dışı bırakın:
...
| `CLAWHUB_CONFIG_PATH`         | CLI'nin token/yapılandırma durumunu depoladığı konumu geçersiz kılar. |
