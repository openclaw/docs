...
title: ClawHub
x-i18n:
    generated_at: "2026-07-02T22:43:52Z"
    model: gpt-5.5
...
- Kayıt kimlik doğrulaması, yayımlama ve silme/geri alma iş akışları için ayrı `clawhub` CLI'sini kullanın.
...
istediğinizde ClawHub CLI'sini yükleyin:
...
açık sayfalar güncel kayıt durumunu gösterir; böylece kullanıcılar bir Skill veya
...
`clawhub:<package>` kullanın. Düz npm uyumlu Plugin belirtimleri, lansman geçişleri
sırasında npm üzerinden çözümlenebilir ve bir kaynağın açık olması gerektiğinde
...
CLI'de doğrudan kayıt iş akışları için Skill yükleme/güncelleme komutları da bulunur:
...
içermelidir. Tam komut başvurusu için [CLI](/tr/clawhub/cli) sayfasına, Skill meta
...
`/dashboard` içinde görünür kalırken herkese açık katalog ve yükleme yüzeylerinden
kaybolabilir.
...
kullanan hesapları yasaklayabilir. Politika ve yaptırım ayrıntıları için
...
sayılarını hesaplayabilmesi için elinden geldiğince bir yükleme olayı gönderebilir.
...
| `CLAWHUB_CONFIG_PATH`         | CLI'nin token/yapılandırma durumunu nerede saklayacağını geçersiz kılar. |
