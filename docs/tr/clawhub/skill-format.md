...
Web GitHub içe aktarıcısı, yerel yayınlama/senkronizasyondan daha katıdır. Yalnızca oturum açmış GitHub hesabına ait herkese açık, fork olmayan depolardaki
...
Beceri meta verileri, `SKILL.md` dosyanızın başındaki YAML frontmatter’da bildirilir. Bu, kayıt sistemine (ve güvenlik analizine) becerinizin çalışmak için neye ihtiyaç duyduğunu söyler.
...
| `homepage`         | `string`   | Becerinin ana sayfasına veya dokümanlarına URL.                                                                                              |
...
- Beceriniz ücretli bir üçüncü taraf hizmetiyle entegre oluyorsa, dış maliyeti ve gerekli hesabı beceri talimatlarında ve ortam bildirimlerinde açıkça belgeleyin (gerekli değişkenler için `requires.env` veya isteğe bağlı değişkenler için `required: false` ile `envVars`).
