...
x-i18n:
    generated_at: "2026-07-04T04:03:00Z"
    model: gpt-5.5
...
Web GitHub içe aktarıcısı, yerel yayımlama/eşitlemeye göre daha katıdır. Yalnızca oturum açmış GitHub hesabına ait herkese açık, fork olmayan depolarda bulunan
...
- `description`, arayüzde/aramada beceri özeti olarak kullanılır.

## Frontmatter meta verileri

Beceri meta verileri, `SKILL.md` dosyanızın üstündeki YAML frontmatter içinde bildirilir. Bu, registry'ye (ve güvenlik analizine) becerinizin çalışmak için neye ihtiyaç duyduğunu söyler.
...
Beceri çalışmadan önce mevcut olması gereken ortam değişkenleri için `requires.env` kullanın. İsteğe bağlı değişkenler de dahil olmak üzere değişken başına meta veriye ihtiyaç duyduğunuzda `envVars` kullanın ve bunlar için `required: false` belirtin.
...
| `homepage`         | `string`   | Becerinin ana sayfasına veya belgelerine URL.                                                                                                               |
...
- `text/` ile başlayan içerik türleri metin olarak değerlendirilir; ayrıca küçük bir izin listesi (JSON/YAML/TOML/JS/TS/Markdown/SVG) vardır.
...
- Paket kapsamları ClawHub yayıncı tanıtıcısıyla tam olarak eşleşmelidir. Yayıncı tanıtıcıları küçük harfler, sayılar, kısa çizgiler, noktalar ve alt çizgiler kullanabilir; küçük harf veya sayı ile başlamalı ve bitmelidir.
...
- ClawHub'da yayımlanan tüm beceriler `MIT-0` altında lisanslanır.
- Herkes yayımlanmış becerileri ticari kullanım dahil olmak üzere kullanabilir, değiştirebilir ve yeniden dağıtabilir.
