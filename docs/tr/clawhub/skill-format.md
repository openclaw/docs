...
x-i18n:
    generated_at: "2026-07-02T01:11:03Z"
    model: gpt-5.5
...
Web GitHub içe aktarıcısı, yerel yayınlama/eşitlemeden daha katıdır. Yalnızca oturum açmış GitHub hesabına ait herkese açık, fork olmayan depolardaki
...
Beceri meta verileri, `SKILL.md` dosyanızın üst kısmındaki YAML frontmatter içinde bildirilir. Bu, kayıt defterine (ve güvenlik analizine) becerinizin çalışmak için nelere ihtiyaç duyduğunu söyler.
...
Beceri çalışmadan önce mevcut olması gereken ortam değişkenleri için `requires.env` kullanın. İsteğe bağlı değişkenler dahil olmak üzere değişken başına meta veriye ihtiyaç duyduğunuzda `envVars` kullanın; isteğe bağlı değişkenler için `required: false` ayarlayın.
...
- Beceriniz ücretli bir üçüncü taraf hizmetle entegre oluyorsa, harici maliyeti ve gerekli hesabı beceri yönergelerinde ve env bildirimlerinde açıkça belgeleyin (gerekli değişkenler için `requires.env` veya isteğe bağlı değişkenler için `required: false` ile `envVars`).
