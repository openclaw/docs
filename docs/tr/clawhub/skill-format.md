...
read_when:
    - Skills yayımlama
    - Yayınlama/eşitleme başarısızlıklarında hata ayıklama
...
- herhangi bir destekleyici _metin tabanlı_ dosya (“İzin verilen dosyalar” bölümüne bakın)
- `.clawhubignore` (yayımlama/eşitleme için yok sayma desenleri, eski `.clawdhubignore`)
...
- Sunucu, yayımlama sırasında metadatayı frontmatter'dan çıkarır.
- `description`, UI/aramada skill özeti olarak kullanılır.

## Frontmatter metadatası

Skill metadatası, `SKILL.md` dosyanızın en üstündeki YAML frontmatter içinde bildirilir. Bu, registry'ye (ve güvenlik analizine) skill'inizin çalışmak için neye ihtiyaç duyduğunu söyler.
...
Skill'in çalışabilmesi için önceden mevcut olması gereken ortam değişkenleri için `requires.env` kullanın. İsteğe bağlı değişkenler dahil olmak üzere değişken başına metadata gerektiğinde `envVars` kullanın ve `required: false` belirtin.
...
ClawHub'ın güvenlik analizi, skill'inizin bildirdikleriyle gerçekte yaptıklarının eşleştiğini denetler. Kodunuz `TODOIST_API_KEY` öğesine başvuruyor ancak frontmatter bunu `requires.env`, `primaryEnv` veya `envVars` altında bildirmiyorsa analiz bir metadata uyumsuzluğu işaretler. Bildirimleri doğru tutmak, skill'inizin incelemeden geçmesine ve kullanıcıların ne kurduklarını anlamasına yardımcı olur.
...
- Etiketler, bir sürüme işaret eden string işaretçilerdir; yaygın olarak `latest` kullanılır.

## Lisans

- ClawHub'da yayımlanan tüm skill'ler `MIT-0` kapsamında lisanslanır.
- Herkes yayımlanmış skill'leri ticari amaçlar dahil kullanabilir, değiştirebilir ve yeniden dağıtabilir.
...
- `SKILL.md` içine fiyatlandırma metadatası eklemeyin; bu skill biçiminin bir parçası değildir ve yayımlanmış bir skill'i ücretli yapmaz.
- Skill'iniz ücretli bir üçüncü taraf hizmetiyle entegre oluyorsa, harici maliyeti ve gerekli hesabı skill talimatlarında ve ortam bildirimlerinde açıkça belgeleyin (gerekli değişkenler için `requires.env`, isteğe bağlı değişkenler için `required: false` içeren `envVars`).
