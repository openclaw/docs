...
x-i18n:
    generated_at: "2026-06-30T22:29:08Z"
    model: gpt-5.5
...
Beceri meta verileri, `SKILL.md` dosyanızın başındaki YAML frontmatter’da bildirilir. Bu, kayıt defterine (ve güvenlik analizine) becerinizin çalışmak için neye ihtiyaç duyduğunu söyler.
...
| `homepage`         | `string`   | Becerinin ana sayfasına veya belgelerine ait URL.                                                                                                                  |
...
ClawHub’ın güvenlik analizi, becerinizin bildirdikleriyle gerçekte yaptıklarının eşleşip eşleşmediğini denetler. Kodunuz `TODOIST_API_KEY` değerine başvuruyor ancak frontmatter bunu `requires.env`, `primaryEnv` veya `envVars` altında bildirmiyorsa analiz bir meta veri uyumsuzluğunu işaretler. Bildirimleri doğru tutmak, becerinizin incelemeden geçmesine yardımcı olur ve kullanıcıların ne kurduklarını anlamasını sağlar.
...
- Betik dosyaları yüklemeden sonra da taranır; PowerShell `.ps1`, `.psm1` ve `.psd1` dosyaları metin olarak kabul edilir.
...
- Paket kapsamları ClawHub yayıncı tanıtıcısıyla tam olarak eşleşmelidir. Yayıncı tanıtıcıları küçük harf, rakam, kısa çizgi, nokta ve alt çizgi kullanabilir; küçük harf veya rakamla başlayıp bitmelidir.
...
- Beceriniz ücretli bir üçüncü taraf hizmetle bütünleşiyorsa harici maliyeti ve gerekli hesabı beceri talimatlarında ve ortam bildirimlerinde açıkça belgeleyin (gerekli değişkenler için `requires.env` veya isteğe bağlı değişkenler için `required: false` ile `envVars`).
