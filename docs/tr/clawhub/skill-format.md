...
x-i18n:
    generated_at: "2026-06-28T00:20:02Z"
    model: gpt-5.5
...
Beceri meta verileri, `SKILL.md` dosyanızın başındaki YAML frontmatter içinde bildirilir. Bu, kayıt defterine (ve güvenlik analizine) becerinizin çalışmak için neye ihtiyaç duyduğunu söyler.
...
Beceri çalışmadan önce mevcut olması gereken ortam değişkenleri için `requires.env` kullanın. İsteğe bağlı değişkenler dahil olmak üzere değişken başına meta veriye ihtiyaç duyduğunuzda `envVars` kullanın ve isteğe bağlı değişkenler için `required: false` ayarlayın.
...
ClawHub’ın güvenlik analizi, becerinizin bildirdikleri ile gerçekten yaptıklarının eşleşip eşleşmediğini denetler. Kodunuz `TODOIST_API_KEY` referans alıyor ancak frontmatter bunu `requires.env`, `primaryEnv` veya `envVars` altında bildirmiyorsa analiz bir meta veri uyuşmazlığını işaretler. Bildirimleri doğru tutmak, becerinizin incelemeden geçmesine yardımcı olur ve kullanıcıların ne kurduklarını anlamasını sağlar.
...
- Herkes yayımlanan becerileri ticari amaçlarla da dahil olmak üzere kullanabilir, değiştirebilir ve yeniden dağıtabilir.
...
- `SKILL.md` içine fiyatlandırma meta verisi eklemeyin; bu, beceri biçiminin bir parçası değildir ve yayımlanan bir beceriyi ücretli yapmaz.
- Beceriniz ücretli bir üçüncü taraf hizmetiyle entegre oluyorsa harici maliyeti ve gerekli hesabı, beceri talimatlarında ve ortam bildirimlerinde açıkça belgeleyin (gerekli değişkenler için `requires.env` veya isteğe bağlı değişkenler için `required: false` ile `envVars`).
