...
Web GitHub içe aktarıcısı, yerel yayınlama/eşitlemeden daha katıdır. Yalnızca oturum açmış GitHub hesabının sahip olduğu herkese açık, fork olmayan depolarda bulunan `SKILL.md` veya eski `skills.md` dosyalarını keşfeder. Özel depoları, forkları, arşivlenmiş/devre dışı bırakılmış depoları veya üçüncü taraf herkese açık depoları içe aktarmaz.

Yerel kurulum üst verisi (CLI tarafından yazılır):
...
- Sunucu, yayınlama sırasında üst veriyi frontmatter'dan çıkarır.
- `description`, UI/aramada beceri özeti olarak kullanılır.

## Frontmatter üst verisi

Beceri üst verisi, `SKILL.md` dosyanızın en üstündeki YAML frontmatter içinde bildirilir. Bu, kayıt defterine (ve güvenlik analizine) becerinizin çalışmak için neye ihtiyaç duyduğunu söyler.
...
### Çalışma zamanı üst verisi (`metadata.openclaw`)
...
Beceri çalışmadan önce mevcut olması gereken ortam değişkenleri için `requires.env` kullanın. İsteğe bağlı değişkenleri `required: false` ile belirtmek dahil, değişken başına üst veriye ihtiyaç duyduğunuzda `envVars` kullanın.
...
ClawHub'ın güvenlik analizi, becerinizin bildirdikleri ile gerçekte yaptıklarının eşleştiğini denetler. Kodunuz `TODOIST_API_KEY` öğesine başvuruyorsa ancak frontmatter bunu `requires.env`, `primaryEnv` veya `envVars` altında bildirmiyorsa analiz bir üst veri uyuşmazlığı işaretler. Bildirimleri doğru tutmak, becerinizin incelemeden geçmesine yardımcı olur ve kullanıcıların ne kurduklarını anlamasını sağlar.

### Örnek: tam frontmatter
...
- Gömme metni `SKILL.md` + yaklaşık 40'a kadar `.md` olmayan dosya içerir (en iyi çabayla sınır).

## Slug'lar

- Varsayılan olarak klasör adından türetilir.
- Paket kapsamları ClawHub yayıncı tanıtıcısıyla tam olarak eşleşmelidir. Yayıncı tanıtıcıları küçük harfler, sayılar, kısa çizgiler, noktalar ve alt çizgiler kullanabilir; küçük harf veya sayıyla başlayıp bitmelidir.
- Paket slug'ları küçük harf olmalı ve npm açısından güvenli olmalıdır; örneğin `@example.tools/demo-plugin` veya `demo-plugin`.
...
- ClawHub ücretli Skills'i, beceri başına fiyatlandırmayı, ödeme duvarlarını veya gelir paylaşımını desteklemez.
- `SKILL.md` içine fiyatlandırma üst verisi eklemeyin; bu beceri biçiminin parçası değildir ve yayınlanan bir beceriyi ücretli yapmaz.
- Beceriniz ücretli bir üçüncü taraf hizmetiyle entegre oluyorsa dış maliyeti ve gerekli hesabı beceri talimatlarında ve env bildirimlerinde açıkça belgeleyin (gerekli değişkenler için `requires.env` veya isteğe bağlı değişkenler için `required: false` ile `envVars`).
