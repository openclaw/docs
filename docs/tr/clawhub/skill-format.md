...
x-i18n:
    generated_at: "2026-07-02T22:45:24Z"
    model: gpt-5.5
...
- herhangi bir destekleyici _metin tabanlı_ dosya (bkz. “İzin verilen dosyalar”)
...
Beceri çalışmadan önce mevcut olması gereken ortam değişkenleri için `requires.env` kullanın. İsteğe bağlı değişkenler dahil olmak üzere değişken başına meta veriye ihtiyaç duyduğunuzda `envVars` kullanın ve isteğe bağlı değişkenler için `required: false` belirtin.
...
| `requires.bins`    | `string[]` | Tamamı kurulu olması gereken CLI ikili dosyaları.                                                                                                             |
...
- Herkes yayımlanan becerileri ticari kullanım dahil olmak üzere kullanabilir, değiştirebilir ve yeniden dağıtabilir.
- Atıf gerekmez.
...
- `SKILL.md` içine fiyatlandırma meta verileri eklemeyin; bu, beceri biçiminin parçası değildir ve yayımlanan bir beceriyi ücretli yapmaz.
