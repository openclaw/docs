...
summary: Sağlayıcı destekli model, görüntü, ses, TTS, video, web ve embedding iş akışları için önce çıkarım yapan CLI
title: Çıkarım CLI
x-i18n:
    generated_at: "2026-06-28T00:22:22Z"
    model: gpt-5.5
...
`openclaw infer`, sağlayıcı destekli çıkarım iş akışları için kurallı başsız arayüzdür.

Bilerek ham Gateway RPC adlarını veya ham ajan araç kimliklerini değil, yetenek ailelerini açığa çıkarır.

## infer'i bir skill'e dönüştürme
...
- kapsadığı iş akışları için birkaç kurallı infer örneği eklemek
- örneklerde ve önerilerde `openclaw infer ...` kullanımını tercih etmek
- skill gövdesinin içinde tüm infer yüzeyini yeniden belgelendirmekten kaçınmak
...
`openclaw infer`, OpenClaw içinde sağlayıcı destekli çıkarım görevleri için tek ve tutarlı bir CLI sağlar.
...
- Betikler, otomasyon ve ajan güdümlü iş akışları için kararlı bir `--json` çıktı biçimi kullanın.
...
Uçtan uca sağlayıcı kontrolleri için, düşük seviyeli sağlayıcı testleri yeşil olduğunda `openclaw infer ...` tercih edin. Bu, sağlayıcı isteği yapılmadan önce gönderilen CLI'yi, yapılandırma yüklemeyi, varsayılan ajan çözümlemeyi, paketlenmiş Plugin etkinleştirmeyi ve paylaşılan yetenek çalışma zamanını çalıştırır.
...
| Görüntüler üzerinde model istemi çalıştır | `openclaw infer model run --prompt "Describe this" --file ./image.png --model provider/model` | Birden çok görüntü girdisi için `--file` tekrarlanır  |
| Görüntü oluştur               | `openclaw infer image generate --prompt "..." --json`                                         | Mevcut bir dosyadan başlarken `image edit` kullanın   |
| Görüntü dosyasını veya URL'yi açıkla | `openclaw infer image describe --file ./image.png --prompt "..." --json`                      | `--model`, görüntü yetenekli bir `<provider/model>` olmalıdır |
...
- `image describe` için `--file`, yerel yolları ve HTTP(S) görüntü URL'lerini kabul eder. Uzak URL'ler normal medya getirme SSRF politikasını kullanır.
- `image describe` için açık bir `--model`, o sağlayıcı/modeli doğrudan çalıştırır. Model, model kataloğunda veya sağlayıcı yapılandırmasında görüntü yetenekli olmalıdır. `codex/<model>`, sınırlı bir Codex uygulama sunucusu görüntü anlama turu çalıştırır; `openai/<model>`, API anahtarı veya ChatGPT/Codex OAuth kimlik doğrulamasıyla OpenAI sağlayıcı yolunu kullanır.
...
- Yerel `model run`, yalın bir tek seferlik sağlayıcı tamamlama işlemidir. Yapılandırılmış ajan modelini ve kimlik doğrulamasını çözümler, ancak bir sohbet ajanı turu başlatmaz, araçları yüklemez veya paketlenmiş MCP sunucularını açmaz.
- `model run --file`, görüntü dosyalarını kabul eder, MIME türlerini algılar ve bunları sağlanan istemle birlikte seçili modele gönderir. Birden çok görüntü için `--file` tekrarlanır.
...
- `model run --gateway`, Gateway yönlendirmesini, kaydedilmiş kimlik doğrulamayı, sağlayıcı seçimini ve gömülü çalışma zamanını çalıştırır, ancak yine de ham model yoklaması olarak çalışır: sağlanan istemi ve görüntü eklerini önceki oturum transkripti, bootstrap/AGENTS bağlamı, bağlam motoru derlemesi, araçlar veya paketlenmiş MCP sunucuları olmadan gönderir.
...
Gateway'i başlatmadan veya tam ajan araç yüzeyini yüklemeden belirli bir sağlayıcıyı duman testinden geçirmek için tam `<provider/model>` referanslarını kullanın:
...
- Yerel `model run`, sağlayıcı/model/kimlik doğrulama sağlığı için en dar CLI duman testidir çünkü Codex dışı sağlayıcılar için yalnızca sağlanan istemi seçili modele gönderir.
- Yerel `model run --model <provider/model>`, sağlayıcı yapılandırmaya yazılmadan önce `models list --all` içindeki kesin paketlenmiş statik katalog satırlarını kullanabilir. Sağlayıcı kimlik doğrulaması yine de gereklidir; eksik kimlik bilgileri `Unknown model` olarak değil, kimlik doğrulama hataları olarak başarısız olur.
- Mistral Medium 3.5 akıl yürütme yoklamaları için sıcaklığı ayarlamayın/varsayılanda bırakın. Mistral, `reasoning_effort="high"` ile `temperature: 0` birleşimini reddeder; varsayılan sıcaklıkla veya `0.7` gibi sıfır olmayan bir akıl yürütme modu değeriyle `mistral/mistral-medium-3-5` kullanın.
- Codex Responses yerel yoklamaları dar istisnadır: OpenClaw, tam ajan bağlamı, araçlar, bellek veya oturum transkripti eklemeden taşımanın gerekli `instructions` alanını doldurabilmesi için minimal bir sistem yönergesi ekler.
...
- Seçili çok modlu metin modelini doğrudan test etmek istediğinizde `model run --file` en uygundur. OpenClaw'ın görüntü anlama sağlayıcı seçimini ve varsayılan görüntü modeli yönlendirmesini istediğinizde `infer image describe` kullanın.
...
- Şeffaf arka planlı OpenAI PNG çıktısı için `--model openai/gpt-image-1.5` ile `--output-format png --background transparent` kullanın; `--openai-background` OpenAI'ye özgü bir takma ad olarak kullanılabilir kalır. Arka plan desteği beyan etmeyen sağlayıcılar, ipucunu yok sayılan bir geçersiz kılma olarak bildirir.
- OpenAI dahil görsel kalitesi ipuçlarını destekleyen sağlayıcılar için `--quality low|medium|high|auto` kullanın. OpenAI, sağlayıcıya özgü moderasyon ipucu için `--openai-moderation low|auto` değerlerini de kabul eder.
- Hangi paketlenmiş görsel sağlayıcılarının keşfedilebilir, yapılandırılmış ve seçilmiş olduğunu, ayrıca her sağlayıcının hangi üretim/düzenleme yeteneklerini sunduğunu doğrulamak için `image providers --json` kullanın.
- Görsel üretim değişiklikleri için en dar canlı CLI duman testi olarak `image generate --model <provider/model> --json` kullanın. Örnek:
...
- `image describe` ve `image describe-many` için, görme modeline OCR, karşılaştırma, kullanıcı arayüzü incelemesi veya kısa başlıklandırma gibi göreve özgü bir talimat vermek üzere `--prompt` kullanın.
- Yavaş yerel görme modelleri veya soğuk Ollama başlatmalarıyla `--timeout-ms` kullanın.
- `image describe` için `--model`, görsel destekli bir `<provider/model>` olmalıdır.
- Yerel Ollama görme modelleri için önce modeli çekin ve `OLLAMA_API_KEY` değerini herhangi bir yer tutucu değere ayarlayın, örneğin `ollama-local`. Bkz. [Ollama](/tr/providers/ollama#vision-and-image-description).
...
- `tts status` varsayılan olarak gateway kullanır çünkü gateway tarafından yönetilen TTS durumunu yansıtır.
...
Üretilen medya komutları için `outputs`, OpenClaw tarafından yazılan dosyaları içerir. Otomasyon için insan tarafından okunabilir stdout'u ayrıştırmak yerine bu dizideki `path`, `mimeType`, `size` ve medyaya özgü tüm boyutları kullanın.
