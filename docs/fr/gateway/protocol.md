...
x-i18n:
    generated_at: "2026-07-03T13:27:41Z"
    model: gpt-5.5
...
Le protocole WS du Gateway est le **plan de contrôle + transport de nœuds unique** pour
OpenClaw. Tous les clients (CLI, UI web, application macOS, nœuds iOS/Android,
nœuds headless) se connectent via WebSocket et déclarent leur **rôle** + leur
**portée** au moment du handshake.
...
le rôle et les portées négociés. `pluginSurfaceUrls` est facultatif et associe les noms de
surfaces de plugin, comme `canvas`, à des URL hébergées à portée limitée.

Les URL de surface de plugin à portée limitée peuvent expirer. Les nœuds peuvent appeler
...
`client.mode: "backend"`) peuvent omettre `device` sur les connexions directes en loopback lorsqu’ils
s’authentifient avec le jeton/mot de passe partagé du gateway. Ce chemin est réservé
aux RPC internes du plan de contrôle et empêche les bases de référence obsolètes d’association CLI/appareil de
bloquer le travail backend local, comme les mises à jour de session de sous-agent. Les clients distants,
les clients d’origine navigateur, les clients nœuds et les clients explicites à jeton d’appareil/identité
d’appareil utilisent toujours les vérifications normales d’association et de montée en portée.
...
Le bootstrap intégré par QR/code de configuration est un nouveau chemin de transfert mobile. Une connexion
réussie avec un code de configuration de base renvoie un jeton de nœud principal plus un jeton
opérateur borné :
...
Le transfert opérateur est intentionnellement borné afin que l’onboarding QR puisse démarrer la
boucle opérateur mobile et terminer la configuration native sans accorder de portées de mutation
d’association ni `operator.admin`. Il inclut `operator.talk.secrets` afin que le
client natif puisse lire la configuration Talk dont il a besoin après le bootstrap. Un accès plus large
à l’association et à l’administration nécessite un flux séparé d’association opérateur approuvée ou de jeton.
Les clients doivent persister
`hello-ok.auth.deviceTokens` uniquement
lorsque la connexion a utilisé l’authentification de bootstrap sur un transport de confiance comme `wss://` ou
...
Pour le modèle complet des portées opérateur, les vérifications au moment de l’approbation et la
...
Les méthodes RPC de Gateway enregistrées par un plugin peuvent demander leur propre portée opérateur, mais
...
`node.pair.approve` dispose également d’une vérification de portée supplémentaire au moment de l’approbation par-dessus la
...
`background` par le gateway avant la persistance. L’événement n’est durable que pour les sessions d’appareil
de nœud authentifiées ; les sessions sans appareil ou non associées renvoient `handled: false`.
...
## Portée des événements broadcast

Les événements broadcast WebSocket poussés par le serveur sont soumis à des portées afin que les sessions limitées à l’association ou réservées aux nœuds ne reçoivent pas passivement le contenu de session.

- Les **trames de chat, d’agent et de résultat d’outil** (y compris les événements `agent` diffusés en streaming et les résultats d’appels d’outils) nécessitent au minimum `operator.read`. Les sessions sans `operator.read` ignorent entièrement ces trames.
- Les **broadcasts `plugin.*` définis par les plugins** sont limités à `operator.write` ou `operator.admin`, selon la façon dont le plugin les a enregistrés.
- Les **événements de statut et de transport** (`heartbeat`, `presence`, `tick`, cycle de vie connexion/déconnexion, etc.) restent non restreints afin que l’état de santé du transport reste observable par chaque session authentifiée.
- Les **familles d’événements broadcast inconnues** sont soumises à une portée par défaut (échec fermé), sauf si un gestionnaire enregistré les assouplit explicitement.

Chaque connexion cliente conserve son propre numéro de séquence par client afin que les broadcasts préservent un ordre monotone sur ce socket, même lorsque différents clients voient différents sous-ensembles filtrés par portée du flux d’événements.

## Familles courantes de méthodes RPC

La surface WS publique est plus large que les exemples de handshake/authentification ci-dessus. Il ne
s’agit pas d’un dump généré — `hello-ok.features.methods` est une liste de découverte
prudente construite à partir de `src/gateway/server-methods-list.ts` plus les exports de méthodes
de plugins/canaux chargés. Traitez-la comme une découverte de fonctionnalités, pas comme une
énumération complète de `src/gateway/server-methods/*.ts`.

  <AccordionGroup>
  <Accordion title="Système et identité">
    - `health` renvoie l’instantané d’état du gateway, mis en cache ou récemment sondé.
    - `diagnostics.stability` renvoie l’enregistreur borné récent de stabilité des diagnostics. Il conserve les métadonnées opérationnelles comme les noms d’événements, les décomptes, les tailles en octets, les relevés de mémoire, l’état des files d’attente/sessions, les noms de canaux/plugins et les identifiants de session. Il ne conserve pas le texte des conversations, les corps de webhooks, les sorties d’outils, les corps bruts de requêtes ou de réponses, les jetons, les cookies ni les valeurs secrètes. Le périmètre de lecture opérateur est requis.
    - `status` renvoie le résumé du gateway de style `/status` ; les champs sensibles ne sont inclus que pour les clients opérateur disposant du périmètre administrateur.
  ...
  <Accordion title="Modèles et utilisation">
    - `models.list` renvoie le catalogue de modèles autorisés par le runtime. Passez `{ "view": "configured" }` pour les modèles configurés dimensionnés pour un sélecteur (`agents.defaults.models` d’abord, puis `models.providers.*.models`), ou `{ "view": "all" }` pour le catalogue complet.
  ...
  <Accordion title="Canaux et aides à la connexion">
  ...
  <Accordion title="Messagerie et journaux">
    - `send` est le RPC de livraison sortante directe pour les envois ciblés par canal/compte/fil de discussion en dehors du lanceur de conversation.
    - `logs.tail` renvoie la fin du journal de fichiers configuré du gateway avec contrôles de curseur/limite et d’octets maximum.

  </Accordion>

  <Accordion title="Talk et TTS">
    - `talk.catalog` renvoie le catalogue Talk en lecture seule des fournisseurs pour la synthèse vocale, la transcription en streaming et la voix en temps réel. Il inclut les identifiants canoniques des fournisseurs, les alias de registre, les libellés, l’état configuré, un résultat facultatif `ready` au niveau du groupe, les identifiants de modèles/voix exposés, les modes canoniques, les transports, les stratégies de cerveau et les indicateurs audio/capacités en temps réel, sans renvoyer de secrets de fournisseur ni modifier la configuration globale. Les Gateway actuels définissent `ready` après application de la sélection de fournisseur au runtime ; les clients doivent traiter son absence comme non vérifiée pour rester compatibles avec les anciens Gateway.
    ...
    - `talk.session.cancelOutput` arrête la sortie audio de l’assistant, principalement pour l’interruption déclenchée par VAD dans les sessions de relais Gateway.
    - `talk.session.submitToolResult` termine un appel d’outil fournisseur émis par une session de relais en temps réel détenue par le Gateway. Passez `options: { willContinue: true }` pour une sortie d’outil intermédiaire lorsqu’un résultat final suivra, ou `options: { suppressResponse: true }` lorsque le résultat de l’outil doit satisfaire l’appel du fournisseur sans démarrer une autre réponse d’assistant en temps réel.
    - `talk.session.steer` envoie le contrôle vocal d’exécution active dans une session Talk adossée à un agent et détenue par le Gateway. Il accepte `{ sessionId, text, mode? }`, où `mode` est `status`, `steer`, `cancel` ou `followup` ; le mode omis est classé à partir du texte prononcé.
    ...
    - `talk.client.steer` envoie le contrôle vocal d’exécution active pour les transports en temps réel détenus par le client. Le Gateway résout l’exécution embarquée active depuis `sessionKey` et renvoie un résultat structuré accepté/refusé au lieu d’ignorer silencieusement le pilotage.
    ...
    - `tts.status` renvoie l’état d’activation de TTS, le fournisseur actif, les fournisseurs de repli et l’état de configuration du fournisseur.
  ...
  <Accordion title="Secrets, configuration, mise à jour et assistant">
    - `secrets.reload` résout à nouveau les SecretRefs actifs et remplace l’état des secrets au runtime uniquement en cas de réussite complète.
    ...
    - `config.patch` fusionne une mise à jour partielle de configuration. Le remplacement destructif de tableau
      ...
      sous les entrées de tableau utilisent des chemins `[]` tels que `agents.list[].skills`.
    - `config.apply` valide + remplace la charge utile complète de configuration.
    - `config.schema` renvoie la charge utile du schéma de configuration en direct utilisée par Control UI et les outils CLI : schéma, `uiHints`, version et métadonnées de génération, y compris les métadonnées de schéma des plugins + canaux lorsque le runtime peut les charger. Le schéma inclut les métadonnées de champs `title` / `description` dérivées des mêmes libellés et textes d’aide que ceux utilisés par l’UI, y compris les objets imbriqués, les caractères génériques, les éléments de tableau et les branches de composition `anyOf` / `oneOf` / `allOf` lorsqu’une documentation de champ correspondante existe.
    - `config.schema.lookup` renvoie une charge utile de recherche limitée à un chemin pour un chemin de configuration : chemin normalisé, nœud de schéma superficiel, indice correspondant + `hintPath`, `reloadKind` facultatif et résumés des enfants immédiats pour l’exploration UI/CLI. `reloadKind` vaut `restart`, `hot` ou `none` et reflète le planificateur de rechargement de configuration du Gateway pour le chemin demandé. Les nœuds de schéma de recherche conservent la documentation destinée à l’utilisateur et les champs de validation courants (`title`, `description`, `type`, `enum`, `const`, `format`, `pattern`, bornes numériques/de chaîne/de tableau/d’objet et indicateurs comme `additionalProperties`, `deprecated`, `readOnly`, `writeOnly`). Les résumés d’enfants exposent `key`, `path` normalisé, `type`, `required`, `hasChildren`, `reloadKind` facultatif, ainsi que le `hint` / `hintPath` correspondant.
    - `update.run` exécute le flux de mise à jour du gateway et planifie un redémarrage uniquement lorsque la mise à jour elle-même a réussi ; les appelants avec une session peuvent inclure `continuationMessage` afin que le démarrage reprenne un tour d’agent de suivi via la file de continuité de redémarrage. Les mises à jour par gestionnaire de paquets et les mises à jour supervisées de checkout git depuis le plan de contrôle utilisent un transfert détaché vers un service géré au lieu de remplacer l’arborescence du paquet ou de modifier la sortie de checkout/build dans le Gateway en direct. Un transfert démarré renvoie `ok: true` avec `result.reason: "managed-service-handoff-started"` et `handoff.status: "started"` ; les transferts indisponibles ou échoués renvoient `ok: false` avec `managed-service-handoff-unavailable` ou `managed-service-handoff-failed`, plus `handoff.command` lorsqu’une mise à jour manuelle par shell est requise. Un transfert indisponible signifie qu’OpenClaw ne dispose pas d’une frontière de supervision sûre ni d’une identité de service durable, comme `OPENCLAW_SYSTEMD_UNIT` pour systemd. Pendant un transfert démarré, le sentinelle de redémarrage peut signaler brièvement `stats.reason: "restart-health-pending"` ; la continuation est retardée jusqu’à ce que la CLI vérifie le Gateway redémarré et écrive le sentinelle final `ok`.
    - `update.status` actualise et renvoie le dernier sentinelle de redémarrage de mise à jour, y compris la version en cours d’exécution après redémarrage lorsqu’elle est disponible.
  ...
  <Accordion title="Assistants pour agents et espaces de travail">
    ...
    - `agent.wait` attend qu’un run se termine et renvoie l’instantané terminal lorsqu’il est disponible.
  ...
  <Accordion title="Contrôle de session">
    ...
    - `sessions.abort` interrompt le travail actif pour une session. Un appelant peut passer `key` plus un `runId` facultatif, ou passer seulement `runId` pour les runs actifs que le Gateway peut résoudre vers une session.
    ...
    - L’exécution du chat utilise toujours `chat.history`, `chat.send`, `chat.abort` et `chat.inject`. `chat.history` est normalisé pour l’affichage par les clients UI : les balises de directives en ligne sont retirées du texte visible, les charges utiles XML d’appels d’outils en texte brut (y compris `<tool_call>...</tool_call>`, `<function_call>...</function_call>`, `<tool_calls>...</tool_calls>`, `<function_calls>...</function_calls>` et les blocs d’appels d’outils tronqués) ainsi que les jetons de contrôle de modèle ASCII/pleine chasse divulgués sont retirés, les lignes d’assistant constituées uniquement de jetons silencieux comme les valeurs exactes `NO_REPLY` / `no_reply` sont omises, et les lignes surdimensionnées peuvent être remplacées par des placeholders.
    - `chat.message.get` est le lecteur additif borné de message complet pour une seule entrée de transcription visible. Les clients passent `sessionKey`, un `agentId` facultatif lorsque la sélection de session est limitée à un agent, ainsi qu’un `messageId` de transcription précédemment exposé par `chat.history`, et le Gateway renvoie la même projection normalisée pour l’affichage sans la limite de troncature de l’historique léger lorsque l’entrée stockée est toujours disponible et n’est pas surdimensionnée.
    - `chat.send` accepte un `fastMode: "auto"` valable pour un tour afin d’utiliser le mode rapide pour les appels de modèle démarrés avant la coupure automatique, puis de démarrer plus tard les appels de nouvelle tentative, de fallback, de résultat d’outil ou de continuation sans mode rapide. La coupure vaut 60 secondes par défaut et peut être configurée par modèle avec `agents.defaults.models["<provider>/<model>"].params.fastAutoOnSeconds`. Un appelant `chat.send` peut passer un `fastAutoOnSeconds` valable pour un tour afin de remplacer la coupure pour cette requête.
  ...
  <Accordion title="Association de nœuds, invocation et travail en attente">
    - `node.pair.request`, `node.pair.list`, `node.pair.approve`, `node.pair.reject`, `node.pair.remove` et `node.pair.verify` couvrent l’association de nœuds et la vérification d’amorçage.
    ...
    - `node.event` transporte les événements issus du nœud vers le gateway.
    - `node.pending.pull` et `node.pending.ack` sont les API de file d’attente de nœud connecté.
  ...
  <Accordion title="Familles d’approbation">
    - `exec.approval.request`, `exec.approval.get`, `exec.approval.list` et `exec.approval.resolve` couvrent les requêtes d’approbation d’exec ponctuelles ainsi que la recherche/relecture des approbations en attente.
    - `exec.approval.waitDecision` attend une approbation d’exec en attente et renvoie la décision finale (ou `null` en cas de délai d’expiration).
    - `exec.approvals.get` et `exec.approvals.set` gèrent les instantanés de politique d’approbation d’exec du gateway.
    - `exec.approvals.node.get` et `exec.approvals.node.set` gèrent la politique d’approbation d’exec locale au nœud via des commandes de relais de nœud.
  ...
  <Accordion title="Automatisation, Skills et outils">
    - Automatisation : `wake` planifie une injection immédiate de texte de réveil ou au prochain Heartbeat ; `cron.get`, `cron.list`, `cron.status`, `cron.add`, `cron.update`, `cron.remove`, `cron.run`, `cron.runs` gèrent le travail planifié.
    ...
    - `cron.runs` accepte un filtre facultatif `runId` non vide afin que les clients puissent suivre un run manuel mis en file d’attente sans entrer en concurrence avec d’autres entrées d’historique pour la même tâche.
...
- `chat` : mises à jour de chat UI comme `chat.inject` et autres événements de chat
  uniquement liés à la transcription. Dans le protocole v4, les charges utiles delta portent `deltaText` ; `message` reste
  l’instantané cumulatif de l’assistant. Les remplacements qui ne sont pas des préfixes définissent `replace=true`
  et utilisent `deltaText` comme texte de remplacement.
- `session.message`, `session.operation` et `session.tool` : mises à jour de transcription,
  d’opération de session en cours et de flux d’événements pour une session
  abonnée.
- `sessions.changed` : l’index de session ou les métadonnées ont changé.
- `presence` : mises à jour d’instantané de présence système.
- `tick` : événement périodique de keepalive / disponibilité.
...
- `node.pair.requested` / `node.pair.resolved` : cycle de vie de l’association de nœuds.
...
- `voicewake.changed` : la configuration de déclenchement par mot de réveil a changé.
...
Les clients opérateur peuvent inspecter et annuler les enregistrements de tâches d’arrière-plan du Gateway via
les RPC du registre des tâches. Ces méthodes renvoient des résumés de tâches nettoyés, et non l’état
d’exécution brut.
...
- `tasks.cancel` nécessite `operator.write`.
  ...
  - `found` indique si le registre contenait une tâche correspondante. `cancelled`
    indique si l’exécution a accepté ou enregistré l’annulation.

`TaskSummary` inclut `id`, `status` et des métadonnées facultatives telles que `kind`,
`runtime`, `title`, `agentId`, `sessionKey`, `childSessionKey`, `ownerKey`,
`runId`, `taskId`, `flowId`, `parentTaskId`, `sourceId`, des horodatages, la progression,
un résumé terminal et le texte d’erreur nettoyé. `agentId` identifie l’agent
qui exécute la tâche ; `sessionKey` et `ownerKey` préservent le contexte de demandeur et de contrôle.
...
- Les opérateurs peuvent appeler `commands.list` (`operator.read`) pour récupérer l’inventaire des commandes d’exécution pour un agent.
  ...
  - `scope` contrôle la surface ciblée par le `name` principal :
    ...
    - `native` et le chemin `both` par défaut renvoient les noms natifs adaptés au fournisseur quand ils sont disponibles
  - `textAliases` transporte les alias avec barre oblique exacts tels que `/model` et `/m`.
...
- Les opérateurs peuvent appeler `tools.catalog` (`operator.read`) pour récupérer le catalogue d’outils d’exécution pour un agent. La réponse inclut des outils groupés et des métadonnées de provenance :
...
- Les opérateurs peuvent appeler `tools.effective` (`operator.read`) pour récupérer l’inventaire des outils effectivement disponibles à l’exécution pour une session.
  ...
  - La Gateway déduit le contexte d’exécution approuvé côté serveur à partir de la session au lieu d’accepter un contexte d’authentification ou de livraison fourni par l’appelant.
  - La réponse est une projection dérivée par le serveur et limitée à la session de l’inventaire actif, incluant les outils du cœur, des plugins, du canal, et les outils de serveurs MCP déjà découverts.
  - `tools.effective` est en lecture seule pour MCP : il peut projeter un catalogue MCP de session déjà chaud à travers la politique finale d’outils, mais il ne crée pas de runtimes MCP, ne connecte pas de transports et n’émet pas `tools/list`. S’il n’existe aucun catalogue chaud correspondant, la réponse peut inclure une notification telle que `mcp-not-yet-connected`, `mcp-not-yet-listed` ou `mcp-stale-catalog`.
  - Les entrées d’outil effectives utilisent `source="core"`, `source="plugin"`, `source="channel"` ou `source="mcp"`.
- Les opérateurs peuvent appeler `tools.invoke` (`operator.write`) pour invoquer un outil disponible via le même chemin de politique de Gateway que `/tools/invoke`.
  ...
  - Les wrappers du cœur réservés aux propriétaires, tels que `cron`, `gateway` et `nodes`, exigent une identité propriétaire/admin (`operator.admin`), même si la méthode `tools.invoke` elle-même est `operator.write`.
  - La réponse est une enveloppe destinée au SDK avec `ok`, `toolName`, `output` facultatif et des champs `error` typés. Les refus d’approbation ou de politique renvoient `ok:false` dans la charge utile au lieu de contourner le pipeline de politique d’outils de la Gateway.
...
- Les opérateurs peuvent appeler `skills.upload.begin`, `skills.upload.chunk` et `skills.upload.commit` (`operator.admin`) pour préparer une archive de skill privée avant de l’installer. Il s’agit d’un chemin de téléversement admin séparé pour les clients approuvés, distinct du flux normal d’installation de skill ClawHub, et il est désactivé par défaut sauf si `skills.install.allowUploadedArchives` est activé.
  ...
  - `skills.upload.commit({ uploadId, sha256? })` vérifie la taille finale et le SHA-256. La validation ne fait que finaliser le téléversement ; elle n’installe pas la skill.
...
- Les opérateurs peuvent appeler `skills.install` (`operator.admin`) selon trois modes :
  ...
  - Mode installateur Gateway : `{ name, installId, timeoutMs? }` exécute une action `metadata.openclaw.install` déclarée sur l’hôte de la Gateway. Les anciens clients peuvent encore envoyer `dangerouslyForceUnsafeInstall` ; ce champ est obsolète, accepté uniquement pour la compatibilité du protocole et ignoré. Utilisez `security.installPolicy` pour les décisions d’installation appartenant à l’opérateur.
...
- Omis ou `"default"` : comportement d’exécution actuel. Si `agents.defaults.models` est configuré, la réponse est le catalogue autorisé, y compris les modèles découverts dynamiquement pour les entrées `provider/*`. Sinon, la réponse est le catalogue Gateway complet.
- `"configured"` : comportement dimensionné pour un sélecteur. Si `agents.defaults.models` est configuré, il reste prioritaire, y compris la découverte limitée au fournisseur pour les entrées `provider/*`. Sans liste d’autorisation, la réponse utilise les entrées explicites `models.providers.*.models`, avec repli sur le catalogue complet uniquement lorsqu’aucune ligne de modèle configurée n’existe.
...
- Après approbation, les appels transférés `node.invoke system.run` réutilisent ce `systemRunPlan` canonique comme contexte faisant autorité pour la commande, le cwd et la session.
- Si un appelant modifie `command`, `rawCommand`, `cwd`, `agentId` ou `sessionKey` entre la préparation et le transfert final approuvé de `system.run`, la Gateway rejette l’exécution au lieu de faire confiance à la charge utile modifiée.
...
- Les résultats finaux `agent` peuvent inclure `result.deliveryStatus` lorsqu’une livraison a été demandée, en utilisant les mêmes statuts `sent`, `suppressed`, `partial_failed` et `failed` que ceux documentés pour [`openclaw agent --json --deliver`](/fr/cli/agent#json-delivery-status).
...
Le client de référence dans `src/gateway/client.ts` utilise ces valeurs par défaut. Les valeurs sont stables à travers le protocole v4 et constituent la base attendue pour les clients tiers.
...
| Délai preauth / connect-challenge         | `15_000` ms                                           | `src/gateway/handshake-timeouts.ts` (config/env peut augmenter le budget serveur/client associé) |
...
| Limite de nouvelle tentative rapide après fermeture de device-token | `250` ms                         | `src/gateway/client.ts`                                                                    |
...
- Une reconnexion avec ce jeton d’appareil **stocké** doit aussi réutiliser l’ensemble
  de portées approuvées stocké pour ce jeton. Cela préserve l’accès lecture/sonde/statut
...
- Assemblage de l’authentification de connexion côté client (`selectConnectAuth` dans
  ...
  - `auth.token` est renseigné par ordre de priorité : jeton partagé explicite d’abord,
    ...
    `deviceId` + `role`).
  - `auth.bootstrapToken` est envoyé uniquement lorsque rien de ce qui précède n’a résolu un
...
- L’amorçage par code de configuration intégré renvoie le
  `hello-ok.auth.deviceToken` du nœud principal plus un jeton opérateur borné dans
  `hello-ok.auth.deviceTokens` pour un transfert mobile fiable. Le jeton opérateur
  inclut `operator.talk.secrets` pour les lectures de configuration Talk natives, mais
  exclut les portées de mutation d’appairage et `operator.admin`.
- Pendant qu’un amorçage par code de configuration non basique attend une approbation, les détails `PAIRING_REQUIRED`
...
- Conservez `hello-ok.auth.deviceTokens` uniquement lorsque la connexion a utilisé l’authentification d’amorçage
  sur un transport fiable comme `wss://` ou un appairage loopback/local.
- Si un client fournit un `deviceToken` **explicite** ou des `scopes` explicites, cet
  ensemble de portées demandé par l’appelant reste l’autorité ; les portées en cache sont uniquement
  réutilisées lorsque le client réutilise le jeton par appareil stocké.
- Les jetons d’appareil peuvent être renouvelés/révoqués via `device.token.rotate` et
  `device.token.revoke` (requiert la portée `operator.pairing`). Renouveler ou
  révoquer un nœud ou un autre rôle non-opérateur requiert aussi `operator.admin`.
- `device.token.rotate` renvoie des métadonnées de rotation. Il renvoie en écho le jeton
  porteur de remplacement uniquement pour les appels du même appareil déjà authentifiés avec
  ce jeton d’appareil, afin que les clients uniquement fondés sur un jeton puissent conserver leur remplacement avant
  de se reconnecter. Les rotations partagées/admin ne renvoient pas le jeton porteur en écho.
- L’émission, la rotation et la révocation des jetons restent bornées à l’ensemble de rôles approuvé
...
- Pour les sessions de jeton d’appareil appairé, la gestion des appareils est auto-limitée sauf si
  l’appelant a aussi `operator.admin` : les appelants non-admin peuvent gérer uniquement le
  jeton opérateur pour l’entrée de leur **propre** appareil. La gestion des jetons de nœud et autres
  jetons non-opérateur est réservée aux administrateurs, même pour le propre appareil de l’appelant.
- `device.token.rotate` et `device.token.revoke` vérifient aussi l’ensemble de portées du jeton
  opérateur cible par rapport aux portées de session actuelles de l’appelant. Les appelants non-admin
  ne peuvent pas renouveler ou révoquer un jeton opérateur plus large que celui qu’ils détiennent déjà.
...
- Comportement client pour `AUTH_TOKEN_MISMATCH` :
  ...
  - Si cette nouvelle tentative échoue, les clients doivent arrêter les boucles de reconnexion automatiques et afficher des indications d’action opérateur.
- `AUTH_SCOPE_MISMATCH` signifie que le jeton d’appareil a été reconnu mais ne couvre pas
  ...
  invitez l’opérateur à ré-appairer ou à approuver le contrat de portée plus étroit/plus large.
...
- Des approbations d’appairage sont requises pour les nouveaux ID d’appareil sauf si l’approbation automatique locale
  est activée.
- L’approbation automatique d’appairage est centrée sur les connexions directes via local loopback.
- OpenClaw dispose aussi d’un chemin étroit d’auto-connexion locale au backend/conteneur pour
  les flux d’assistants fiables à secret partagé.
- Les connexions tailnet ou LAN depuis le même hôte sont toujours traitées comme distantes pour l’appairage et
...
- Les clients WS incluent normalement l’identité `device` pendant `connect` (opérateur +
  ...
  - `gateway.controlUi.dangerouslyDisableDeviceAuth=true` (break-glass, forte dégradation de sécurité).
  - RPCs backend `gateway-client` via direct-loopback sur le chemin d’assistant interne
    réservé.
- L’omission de l’identité d’appareil a des conséquences sur les portées. Lorsqu’une connexion opérateur
  ...
  les portées auto-déclarées pour obtenir un ensemble vide, sauf si ce chemin dispose d’une exception nommée
  de préservation des portées. Les méthodes soumises à des portées échouent alors avec
  `missing scope`.
- `gateway.controlUi.dangerouslyDisableDeviceAuth=true` est un chemin Control UI
  break-glass de préservation des portées. Il n’accorde pas de portées à des clients WebSocket
  backend personnalisés arbitraires ou de forme CLI.
- Le chemin d’assistant backend `gateway-client` réservé via direct-loopback préserve
  les portées uniquement pour les RPCs de plan de contrôle local internes ; les ID backend personnalisés ne
...
- Les signatures héritées `v2` restent acceptées pour compatibilité, mais l’épinglage des
  métadonnées d’appareil appairé contrôle toujours la politique de commandes lors de la reconnexion.
