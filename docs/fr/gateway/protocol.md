...
x-i18n:
    generated_at: "2026-05-02T07:08:08Z"
    model: gpt-5.5
    provider: openai
...
sans interface) se connectent via WebSocket et déclarent leur **rôle** + **périmètre** au
...
- Les trames pré-connexion sont limitées à 64 Kio. Après une poignée de main réussie, les clients
...
dans leur budget global de connexion au lieu de la présenter comme un échec terminal de
poignée de main.
...
le rôle/les périmètres négociés. `canvasHostUrl` est facultatif.

Lorsqu’aucun jeton d’appareil n’est émis, `hello-ok.auth` signale les
...
aux RPC internes du plan de contrôle et évite que des références de base obsolètes d’association CLI/appareil
bloquent le travail backend local, comme les mises à jour de sessions de sous-agents. Les clients distants,
les clients d’origine navigateur, les clients nœuds et les clients explicites avec jeton d’appareil/identité
d’appareil utilisent toujours les vérifications normales d’association et de montée en périmètre.
...
Pour le flux d’amorçage intégré nœud/opérateur, le jeton de nœud principal conserve
...
`operator.talk.secrets`, `operator.write`). Les vérifications de périmètre d’amorçage restent
préfixées par le rôle : les entrées opérateur ne satisfont que les requêtes opérateur, et les rôles
non opérateurs nécessitent toujours des périmètres sous leur propre préfixe de rôle.
...
Les méthodes RPC du gateway enregistrées par des Plugins peuvent demander leur propre périmètre opérateur, mais
les préfixes d’administration cœur réservés (`config.*`, `exec.approvals.*`, `wizard.*`,
`update.*`) se résolvent toujours en `operator.admin`.

Le périmètre de méthode n’est que la première barrière. Certaines commandes slash atteintes via
...
`node.pair.approve` dispose également d’une vérification de périmètre supplémentaire au moment de l’approbation, en plus du
...
vivant lors d’un réveil en arrière-plan sans le marquer comme connecté.
...
## Périmétrage des événements de diffusion

Les événements de diffusion WebSocket envoyés par le serveur sont restreints par périmètre afin que les sessions limitées à l’association ou réservées aux nœuds ne reçoivent pas passivement le contenu de session.

- **Trames de chat, d’agent et de résultat d’outil** (y compris les événements `agent` diffusés en continu et les résultats d’appels d’outil) exigent au minimum `operator.read`. Les sessions sans `operator.read` ignorent entièrement ces trames.
- **Diffusions `plugin.*` définies par des Plugins** sont limitées à `operator.write` ou `operator.admin`, selon la façon dont le Plugin les a enregistrées.
- **Événements d’état et de transport** (`heartbeat`, `presence`, `tick`, cycle de vie connexion/déconnexion, etc.) restent non restreints afin que la santé du transport reste observable par chaque session authentifiée.
- **Familles d’événements de diffusion inconnues** sont restreintes par périmètre par défaut (échec fermé), sauf si un gestionnaire enregistré les assouplit explicitement.

Chaque connexion cliente conserve son propre numéro de séquence par client afin que les diffusions préservent un ordre monotone sur ce socket, même lorsque différents clients voient différents sous-ensembles filtrés par périmètre du flux d’événements.

## Familles courantes de méthodes RPC

La surface WS publique est plus large que les exemples de poignée de main/authentification ci-dessus. Il
ne s’agit pas d’un dump généré — `hello-ok.features.methods` est une liste de découverte
prudente construite à partir de `src/gateway/server-methods-list.ts` plus les exports de méthodes de Plugins/canaux
...
<AccordionGroup>
  <Accordion title="Système et identité">
    ...
    - `status` renvoie le résumé du gateway de style `/status` ; les champs sensibles ne sont inclus que pour les clients opérateurs avec périmètre admin.
  ...
  <Accordion title="Modèles et utilisation">
    - `models.list` renvoie le catalogue des modèles autorisés à l’exécution. Passez `{ "view": "configured" }` pour les modèles configurés au format sélecteur (`agents.defaults.models` d’abord, puis `models.providers.*.models`), ou `{ "view": "all" }` pour le catalogue complet.
  ...
  <Accordion title="Canaux et assistants de connexion">
    ...
    - `channels.logout` déconnecte un canal/compte précis lorsque le canal prend en charge la déconnexion.
  ...
  <Accordion title="Messagerie et journaux">
    - `send` est le RPC de livraison sortante directe pour les envois ciblés par canal/compte/fil en dehors du lanceur de chat.
  ...
  <Accordion title="Talk et TTS">
    ...
    - `tts.status` renvoie l’état d’activation de TTS, le fournisseur actif, les fournisseurs de repli et l’état de configuration des fournisseurs.
  ...
  <Accordion title="Secrets, configuration, mise à jour et assistant">
    - `secrets.reload` résout à nouveau les SecretRefs actives et remplace l’état des secrets d’exécution uniquement en cas de réussite complète.
    - `secrets.resolve` résout les affectations de secrets ciblées par commande pour un ensemble commande/cible précis.
    ...
    - `config.schema.lookup` renvoie une charge utile de recherche limitée à un chemin pour un chemin de configuration : chemin normalisé, nœud de schéma superficiel, indice correspondant + `hintPath`, et résumés des enfants immédiats pour l’exploration UI/CLI. Les nœuds de schéma de recherche conservent la documentation destinée à l’utilisateur et les champs de validation courants (`title`, `description`, `type`, `enum`, `const`, `format`, `pattern`, limites numériques/de chaîne/de tableau/d’objet, et indicateurs comme `additionalProperties`, `deprecated`, `readOnly`, `writeOnly`). Les résumés enfants exposent `key`, `path` normalisé, `type`, `required`, `hasChildren`, ainsi que le `hint` / `hintPath` correspondant.
    - `update.run` exécute le flux de mise à jour du Gateway et planifie un redémarrage uniquement lorsque la mise à jour elle-même a réussi. Les mises à jour par gestionnaire de paquets imposent un redémarrage de mise à jour non différé et sans délai de refroidissement après l’échange du paquet afin que l’ancien processus Gateway ne continue pas à charger paresseusement depuis une arborescence `dist` remplacée.
  ...
  <Accordion title="Assistants d’agent et d’espace de travail">
    ...
    - `agents.files.list`, `agents.files.get` et `agents.files.set` gèrent les fichiers d’espace de travail de bootstrap exposés pour un agent.
  ...
  <Accordion title="Contrôle des sessions">
    ...
    - `sessions.preview` renvoie des aperçus limités de transcription pour des clés de session précises.
    ...
    - `sessions.patch` met à jour les métadonnées/remplacements de session et rapporte le modèle canonique résolu ainsi que l’`agentRuntime` effectif.
    ...
    - L’exécution du chat utilise toujours `chat.history`, `chat.send`, `chat.abort` et `chat.inject`. `chat.history` est normalisé pour l’affichage des clients UI : les balises de directive en ligne sont retirées du texte visible, les charges utiles XML d’appel d’outil en texte brut (y compris `<tool_call>...</tool_call>`, `<function_call>...</function_call>`, `<tool_calls>...</tool_calls>`, `<function_calls>...</function_calls>` et les blocs d’appel d’outil tronqués) et les jetons de contrôle de modèle ASCII/pleine chasse divulgués sont retirés, les lignes d’assistant composées uniquement de jetons silencieux comme les valeurs exactes `NO_REPLY` / `no_reply` sont omises, et les lignes surdimensionnées peuvent être remplacées par des espaces réservés.
  ...
  <Accordion title="Appairage de nœud, invocation et travail en attente">
    - `node.pair.request`, `node.pair.list`, `node.pair.approve`, `node.pair.reject`, `node.pair.remove` et `node.pair.verify` couvrent l’appairage de nœud et la vérification de bootstrap.
...
- `chat` : mises à jour de chat UI comme `chat.inject` et autres événements de chat limités à la transcription.
...
- `node.invoke.request` : diffusion d’une demande d’invocation de nœud.
...
- Les opérateurs peuvent appeler `commands.list` (`operator.read`) pour récupérer l’inventaire des commandes d’exécution pour un agent.
  ...
  - `scope` contrôle quelle surface cible le `name` principal :
  ...
  - `nativeName` transporte le nom de commande natif tenant compte du fournisseur lorsqu’il existe.
...
- Les opérateurs peuvent appeler `tools.effective` (`operator.read`) pour récupérer l’inventaire des outils effectivement disponibles à l’exécution pour une session.
  ...
  - Le gateway déduit le contexte d’exécution approuvé côté serveur à partir de la session au lieu d’accepter un contexte d’authentification ou de livraison fourni par l’appelant.
...
- Les opérateurs peuvent appeler `tools.invoke` (`operator.write`) pour invoquer un outil disponible via le même chemin de politique du gateway que `/tools/invoke`.
  ...
  - Si `sessionKey` et `agentId` sont tous deux présents, l’agent résolu pour la session doit correspondre à `agentId`.
...
- `"configured"` : comportement adapté à un sélecteur. Si `agents.defaults.models` est configuré, il reste prioritaire. Sinon, la réponse utilise les entrées explicites `models.providers.*.models`, avec repli vers le catalogue complet uniquement lorsqu’aucune ligne de modèle configurée n’existe.
...
- Si un appelant modifie `command`, `rawCommand`, `cwd`, `agentId` ou `sessionKey` entre la préparation et le transfert final approuvé `system.run`, le gateway rejette l’exécution au lieu de faire confiance à la charge utile modifiée.
...
- `bestEffortDeliver=true` autorise un repli vers une exécution limitée à la session lorsqu’aucune route livrable externe ne peut être résolue (par exemple, sessions internes/webchat ou configurations multicanaux ambiguës).
...
- Les schémas et modèles sont générés à partir de définitions TypeBox :
  - `pnpm protocol:gen`
...
| Délai d’expiration de préauthentification / défi de connexion | `15_000` ms                                           | `src/gateway/handshake-timeouts.ts` (la config/l’env peut augmenter le budget serveur/client associé) |
...
- Les modes porteurs d’identité tels que Tailscale Serve
  (`gateway.auth.allowTailscale: true`) ou le mode hors boucle locale
...
- L’entrée privée `gateway.auth.mode: "none"` ignore entièrement l’authentification de connexion par secret partagé ;
...
- Après l’appairage, le Gateway émet un **jeton d’appareil** limité au rôle de connexion
  + aux portées. Il est renvoyé dans `hello-ok.auth.deviceToken` et devrait être
  conservé par le client pour les connexions futures.
- Les clients devraient conserver le `hello-ok.auth.deviceToken` principal après toute
  connexion réussie.
- La reconnexion avec ce jeton d’appareil **stocké** devrait aussi réutiliser l’ensemble de portées
  approuvé et stocké pour ce jeton. Cela préserve l’accès en lecture/sonde/statut
...
- Assemblage côté client de l’authentification de connexion (`selectConnectAuth` dans
  ...
  - La promotion automatique d’un jeton d’appareil stocké lors de la nouvelle tentative unique
    ...
    boucle locale, ou `wss://` avec une `tlsFingerprint` épinglée. Un `wss://` public
...
- Les entrées supplémentaires `hello-ok.auth.deviceTokens` sont des jetons de transfert d’amorçage.
  ...
  tel que `wss://` ou un appairage via boucle locale/local.
- Si un client fournit un `deviceToken` **explicite** ou des `scopes` explicites, cet
  ensemble de portées demandé par l’appelant reste l’autorité ; les portées en cache ne sont
...
- `device.token.rotate` renvoie des métadonnées de renouvellement. Il renvoie aussi le jeton
  porteur de remplacement uniquement pour les appels du même appareil déjà authentifiés avec
  ce jeton d’appareil, afin que les clients utilisant seulement des jetons puissent conserver leur remplacement avant
  de se reconnecter. Les renouvellements partagés/admin ne renvoient pas le jeton porteur.
- L’émission, le renouvellement et la révocation des jetons restent limités à l’ensemble de rôles approuvé
...
- Pour les sessions de jeton d’appareil appairé, la gestion des appareils est limitée à soi-même, sauf si
  l’appelant dispose aussi de `operator.admin` : les appelants non admin peuvent supprimer/révoquer/renouveler
...
- `device.token.rotate` et `device.token.revoke` vérifient aussi l’ensemble de portées du jeton opérateur
  cible par rapport aux portées de session actuelles de l’appelant. Les appelants non admin
...
- Comportement client pour `AUTH_TOKEN_MISMATCH` :
  - Les clients fiables peuvent tenter une seule nouvelle tentative bornée avec un jeton en cache par appareil.
  - Si cette nouvelle tentative échoue, les clients devraient arrêter les boucles de reconnexion automatique et afficher des indications d’action pour l’opérateur.

## Identité d’appareil + appairage

- Les Nodes devraient inclure une identité d’appareil stable (`device.id`) dérivée d’une
...
- Les connexions tailnet même hôte ou LAN sont toujours traitées comme distantes pour l’appairage et
...
- Les clients WS incluent normalement l’identité `device` pendant `connect` (opérateur +
  ...
  - authentification réussie du Control UI opérateur via `gateway.auth.mode: "trusted-proxy"`.
  - `gateway.controlUi.dangerouslyDisableDeviceAuth=true` (solution de dernier recours, dégradation de sécurité sévère).
  - RPC backend `gateway-client` via boucle locale directe authentifiés avec le jeton/mot de passe
    Gateway partagé.
