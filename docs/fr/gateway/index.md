...
x-i18n:
    generated_at: "2026-05-06T07:22:38Z"
    model: gpt-5.5
    provider: openai
...
Avec un Gateway joignable, cette commande exécute des sondes de canaux en direct par compte et des audits facultatifs.
Si le Gateway est injoignable, la CLI revient à des résumés de canaux fondés uniquement sur la configuration au lieu
...
<Note>
Le rechargement de la configuration du Gateway surveille le chemin du fichier de configuration actif (résolu à partir des valeurs par défaut de profil/état, ou de `OPENCLAW_CONFIG_PATH` lorsqu’il est défini).
...
- L’authentification est requise par défaut. Les configurations à secret partagé utilisent
  ...
  non-loopback peuvent utiliser `gateway.auth.mode: "trusted-proxy"`.
...
validation d’exécution. Ajoutez explicitement toute origine de navigateur distante, comme les URL de proxy HTTPS, à
...
`gateway status --deep` sert à une découverte supplémentaire de services (LaunchDaemons/unités système systemd/schtasks),
pas à une sonde de santé RPC plus approfondie.

## Gateways multiples (même hôte)

La plupart des installations doivent exécuter un seul Gateway par machine. Un seul Gateway peut héberger plusieurs
agents et canaux.
...
- `gateway status --deep` peut signaler `Other gateway-like services detected (best effort)`
  et afficher des conseils de nettoyage lorsque de vieilles installations launchd/systemd/schtasks sont encore présentes.
- `gateway probe` peut avertir au sujet de `multiple reachable gateways` lorsque plusieurs cibles
  répondent.
...
- `hello-ok.features.methods` / `events` sont une liste de découverte conservatrice, pas
  un vidage généré de toutes les routes d’assistance appelables.
...
Pour les enchaînements de diagnostic complets, utilisez [Dépannage Gateway](/fr/gateway/troubleshooting).
