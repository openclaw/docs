...
title: Heartbeat
x-i18n:
    generated_at: "2026-05-02T07:06:33Z"
    model: gpt-5.5
    provider: openai
...
Heartbeat exécute des **tours d’agent périodiques** dans la session principale afin que le modèle puisse signaler ce qui nécessite une attention sans vous envoyer de messages indésirables.
...
<Steps>
  <Step title="Choisir une cadence">
    Laissez les Heartbeat activés (la valeur par défaut est `30m`, ou `1h` pour l’authentification Anthropic OAuth/par jeton, y compris la réutilisation de Claude CLI) ou définissez votre propre cadence.
  </Step>
...
- L’invite Heartbeat est envoyée **textuellement** comme message utilisateur. L’invite système inclut une section « Heartbeat » uniquement lorsque les Heartbeat sont activés pour l’agent par défaut, et l’exécution est signalée en interne.
- Lorsque les Heartbeat sont désactivés avec `0m`, les exécutions normales omettent également `HEARTBEAT.md` du contexte d’amorçage afin que le modèle ne voie pas les instructions réservées à Heartbeat.
...
- Les Heartbeat se reportent automatiquement pendant qu’un travail Cron est actif ou en file d’attente. Définissez `heartbeat.skipWhenBusy: true` pour reporter aussi sur les voies particulièrement occupées (travail de sous-agent ou de commande imbriquée) ; c’est utile pour Ollama local et les autres hôtes à environnement d’exécution unique contraint.

## À quoi sert l’invite Heartbeat

L’invite par défaut est volontairement générale :

- **Tâches en arrière-plan** : « Tenir compte des tâches en attente » incite l’agent à passer en revue les suivis (boîte de réception, calendrier, rappels, travail en file d’attente) et à signaler tout élément urgent.
- **Prise de nouvelles de l’humain** : « Prendre parfois des nouvelles de votre humain pendant la journée » incite à envoyer occasionnellement un message léger du type « besoin de quelque chose ? », tout en évitant les messages nocturnes indésirables grâce au fuseau horaire local configuré (voir [Fuseau horaire](/fr/concepts/timezone)).
...
Si vous voulez qu’un Heartbeat fasse quelque chose de très précis (par exemple « vérifier les statistiques Gmail PubSub » ou « vérifier l’état de santé du Gateway »), définissez `agents.defaults.heartbeat.prompt` (ou `agents.list[].heartbeat.prompt`) sur un corps personnalisé (envoyé textuellement).

## Contrat de réponse

- Si rien ne nécessite d’attention, répondez avec **`HEARTBEAT_OK`**.
- Les exécutions Heartbeat capables d’utiliser des outils peuvent plutôt appeler `heartbeat_respond` avec `notify: false` pour ne produire aucune mise à jour visible, ou `notify: true` avec `notificationText` pour une alerte. Lorsqu’elle est présente, la réponse structurée de l’outil a priorité sur le texte de secours.
...
En dehors des Heartbeat, un `HEARTBEAT_OK` isolé au début ou à la fin d’un message est supprimé et journalisé ; un message qui contient uniquement `HEARTBEAT_OK` est abandonné.
...
- `channels.<channel>.accounts.<id>.heartbeat` (canaux multicompte) remplace les paramètres propres au canal.

### Heartbeats par agent

Si une entrée `agents.list[]` inclut un bloc `heartbeat`, **seuls ces agents** exécutent les Heartbeats. Le bloc par agent fusionne par-dessus `agents.defaults.heartbeat` (vous pouvez donc définir des valeurs par défaut partagées une seule fois et les remplacer pour chaque agent).
...
En dehors de cette plage (avant 9 h ou après 22 h, heure de l’Est), les Heartbeats sont ignorés. Le prochain déclenchement planifié dans cette plage s’exécutera normalement.
...
<Warning>
Ne définissez pas la même heure de `start` et de `end` (par exemple de `08:00` à `08:00`). Cela est traité comme une fenêtre de largeur nulle, donc les Heartbeat sont toujours ignorés.
</Warning>

### Exemple multicomptes

Utilisez `accountId` pour cibler un compte spécifique sur les canaux multicomptes comme Telegram :
...
<ParamField path="includeReasoning" type="boolean" default="false">
  Lorsque cette option est activée, transmettre également le message `Reasoning:` séparé lorsqu’il est disponible (même forme que `/reasoning on`).
</ParamField>
<ParamField path="lightContext" type="boolean" default="false">
  Lorsque cette valeur est vraie, les exécutions de Heartbeat utilisent un contexte d’amorçage léger et ne conservent que `HEARTBEAT.md` depuis les fichiers d’amorçage de l’espace de travail.
</ParamField>
<ParamField path="isolatedSession" type="boolean" default="false">
  Lorsque cette valeur est vraie, chaque Heartbeat s’exécute dans une nouvelle session sans historique de conversation préalable. Utilise le même modèle d’isolation que Cron `sessionTarget: "isolated"`. Réduit considérablement le coût en tokens par Heartbeat. À combiner avec `lightContext: true` pour des économies maximales. Le routage de la livraison utilise toujours le contexte de la session principale.
</ParamField>
<ParamField path="skipWhenBusy" type="boolean" default="false">
  Lorsque cette valeur est vraie, les exécutions de Heartbeat sont différées sur les voies très occupées : travail de sous-agent ou de commande imbriquée. Les voies Cron diffèrent toujours les Heartbeat, même sans cet indicateur, afin que les hôtes de modèles locaux n’exécutent pas les invites Cron et Heartbeat en même temps.
</ParamField>
<ParamField path="session" type="string">
  Clé de session facultative pour les exécutions de Heartbeat.

- `main` (par défaut) : session principale de l’agent.
- Clé de session explicite (copiez-la depuis `openclaw sessions --json` ou le [CLI des sessions](/fr/cli/sessions)).
...
<ParamField path="directPolicy" type='"allow" | "block"' default="allow">
  Contrôle le comportement de livraison directe/en DM. `allow` : autoriser la livraison directe/en DM du heartbeat. `block` : supprimer la livraison directe/en DM (`reason=dm-blocked`).

</ParamField>
<ParamField path="to" type="string">
  Remplacement facultatif du destinataire (id propre au canal, par exemple E.164 pour WhatsApp ou un id de chat Telegram). Pour les sujets/fils Telegram, utilisez `<chatId>:topic:<messageThreadId>`.

</ParamField>
<ParamField path="accountId" type="string">
  Id de compte facultatif pour les canaux multicomptes. Lorsque `target: "last"`, l’id de compte s’applique au dernier canal résolu s’il prend en charge les comptes ; sinon, il est ignoré. Si l’id de compte ne correspond pas à un compte configuré pour le canal résolu, la livraison est ignorée.
...
<AccordionGroup>
  <Accordion title="Routage de session et de cible">
    ...
    - Si `skipWhenBusy: true`, les voies de sous-agents et les voies imbriquées diffèrent également les exécutions Heartbeat.
  ...
  <Accordion title="Visibilité et comportement d’ignorance">
    ...
    - Si la cible Heartbeat résolue prend en charge l’indication de saisie, OpenClaw affiche la saisie pendant que l’exécution Heartbeat est active. Cela utilise la même cible que celle à laquelle le Heartbeat enverrait la sortie de chat, et c’est désactivé par `typingMode: "never"`.

  </Accordion>
  <Accordion title="Cycle de vie de session et audit">
    - Les réponses uniquement Heartbeat ne gardent **pas** la session active. Les métadonnées Heartbeat peuvent mettre à jour la ligne de session, mais l’expiration pour inactivité utilise `lastInteractionAt` du dernier vrai message utilisateur/canal, et l’expiration quotidienne utilise `sessionStartedAt`.
    ...
    - Les [tâches en arrière-plan](/fr/automation/tasks) détachées peuvent mettre en file un événement système et réveiller Heartbeat lorsque la session principale doit remarquer quelque chose rapidement. Ce réveil ne transforme pas l’exécution Heartbeat en tâche en arrière-plan.
...
Si un fichier `HEARTBEAT.md` existe dans l’espace de travail, le prompt par défaut indique à l’agent de le lire. Considérez-le comme votre « checklist Heartbeat » : courte, stable et sûre à inclure toutes les 30 minutes.
...
`HEARTBEAT.md` prend aussi en charge un petit bloc structuré `tasks:` pour des vérifications basées sur un intervalle au sein même de Heartbeat.
...
<AccordionGroup>
  <Accordion title="Comportement">
    ...
    - Seules les tâches **échues** sont incluses dans le prompt Heartbeat pour ce tick.
    - Si aucune tâche n’est échue, le Heartbeat est entièrement ignoré (`reason=no-tasks-due`) afin d’éviter un appel au modèle inutile.
    - Le contenu hors tâches dans `HEARTBEAT.md` est conservé et ajouté comme contexte supplémentaire après la liste des tâches échues.
...
Le mode tâches est utile lorsque vous voulez qu’un seul fichier Heartbeat contienne plusieurs vérifications périodiques sans payer pour toutes à chaque tick.
...
- « Réécrivez `HEARTBEAT.md` pour le raccourcir et le concentrer sur les suivis de boîte de réception. »

Si vous voulez que cela se produise de manière proactive, vous pouvez aussi inclure une ligne explicite dans votre prompt Heartbeat, par exemple : « Si la checklist devient obsolète, mettez à jour HEARTBEAT.md avec une meilleure version. »

<Warning>
Ne mettez pas de secrets (clés API, numéros de téléphone, jetons privés) dans `HEARTBEAT.md` — il devient partie intégrante du contexte de prompt.
</Warning>
...
Lorsque cette option est activée, les Heartbeats livreront aussi un message séparé préfixé par `Reasoning:` (même forme que `/reasoning on`). Cela peut être utile lorsque l’agent gère plusieurs sessions/codex et que vous voulez voir pourquoi il a décidé de vous alerter — mais cela peut aussi divulguer plus de détails internes que vous ne le souhaitez. Préférez le garder désactivé dans les discussions de groupe.

## Sensibilité aux coûts

Les Heartbeats exécutent des tours complets d’agent. Des intervalles plus courts consomment plus de tokens. Pour réduire les coûts :
...
Si un Heartbeat utilise un modèle local plus petit, par exemple un modèle Ollama avec une fenêtre de 32k, et que le tour suivant de la session principale signale un débordement de contexte, vérifiez si le Heartbeat précédent a laissé la session sur le modèle Heartbeat. Le message de réinitialisation d’OpenClaw le signale lorsque le dernier modèle d’exécution correspond au `heartbeat.model` configuré.

Utilisez `isolatedSession: true` pour exécuter les Heartbeats dans une nouvelle session, combinez-le avec `lightContext: true` pour le prompt le plus petit, ou choisissez un modèle Heartbeat avec une fenêtre de contexte suffisamment grande pour la session partagée.
