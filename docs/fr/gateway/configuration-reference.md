...
Cette page couvre les principales surfaces de configuration d’OpenClaw et renvoie vers d’autres pages lorsqu’un sous-système dispose de sa propre référence plus détaillée. Elle n’essaie **pas** d’inclure en ligne chaque catalogue de commandes appartenant à un canal/plugin ni chaque réglage profond de mémoire/QMD sur une seule page.

Source de vérité du code :

- `openclaw config schema` affiche le schéma JSON en direct utilisé pour la validation et Control UI, avec les métadonnées bundlées/plugin/canal fusionnées lorsqu’elles sont disponibles
- `config.schema.lookup` renvoie un nœud de schéma limité à un chemin pour les outils d’exploration détaillée
- `pnpm config:docs:check` / `pnpm config:docs:gen` valident le hash de référence des documents de configuration par rapport à la surface actuelle du schéma
...
- [Commandes slash](/fr/tools/slash-commands) pour le catalogue actuel des commandes intégrées + bundlées
...
Chaque canal démarre automatiquement lorsque sa section de configuration existe (sauf `enabled: false`).

### Accès DM et groupe

Tous les canaux prennent en charge les politiques de messages privés et les politiques de groupe :

| DM policy           | Comportement                                                    |
| ------------------- | --------------------------------------------------------------- |
| `pairing` (default) | Les expéditeurs inconnus reçoivent un code d’association à usage unique ; le propriétaire doit l’approuver |
...
| Group policy          | Comportement                                             |
| --------------------- | -------------------------------------------------------- |
| `allowlist` (default) | Seuls les groupes correspondant à la liste d’autorisation configurée |
| `open`                | Contourner les listes d’autorisation de groupes (le contrôle par mention s’applique toujours) |
...
Si un bloc fournisseur est entièrement absent (`channels.<provider>` absent), la politique de groupe à l’exécution se replie sur `allowlist` (échec fermé) avec un avertissement au démarrage.
</Note>

### Remplacements de modèle par canal

Utilisez `channels.modelByChannel` pour fixer des ID de canal spécifiques à un modèle. Les valeurs acceptent `provider/model` ou des alias de modèle configurés. Le mappage de canal s’applique lorsqu’une session n’a pas déjà un remplacement de modèle (par exemple, défini via `/model`).
...
WhatsApp s’exécute via le canal web de la Gateway (Baileys Web). Il démarre automatiquement lorsqu’une session liée existe.
...
- Dans les configurations multi-comptes (2+ ID de compte), définissez une valeur par défaut explicite (`channels.telegram.defaultAccount` ou `channels.telegram.accounts.default`) pour éviter le routage de repli ; `openclaw doctor` avertit lorsque cette valeur est absente ou invalide.
...
- `maxLinesPerMessage` (valeur par défaut 17) segmente les messages hauts même lorsqu’ils sont en dessous de 2000 caractères.
- `channels.discord.threadBindings` contrôle le routage lié aux fils Discord :
  ...
  - `idleHours` : remplacement Discord pour la désactivation automatique après inactivité en heures (`0` désactive)
  - `maxAgeHours` : remplacement Discord pour l’âge maximal strict en heures (`0` désactive)
  - `spawnSubagentSessions` : commutateur d’activation pour la création/liaison automatique de fil par `sessions_spawn({ thread: true })`
- Les entrées `bindings[]` de premier niveau avec `type: "acp"` configurent des liaisons ACP persistantes pour les canaux et fils (utilisez l’id du canal/fil dans `match.peer.id`). La sémantique des champs est partagée dans [Agents ACP](/fr/tools/acp-agents#channel-specific-settings).
- `channels.discord.ui.components.accentColor` définit la couleur d’accent pour les conteneurs Discord components v2.
- `channels.discord.voice` active les conversations dans les canaux vocaux Discord et les remplacements facultatifs d’auto-join + TTS.
...
- `channels.discord.autoPresence` mappe la disponibilité à l’exécution à la présence du bot (sain => online, dégradé => idle, épuisé => dnd) et autorise des remplacements facultatifs du texte de statut.
...
- `channels.discord.execApprovals` : livraison native Discord des approbations exec et autorisation des approbateurs.
  ...
  - `approvers` : ID d’utilisateur Discord autorisés à approuver les demandes exec. Se replie sur `commands.ownerAllowFrom` lorsqu’il est omis.
  ...
  - `target` : où envoyer les invites d’approbation. `"dm"` (par défaut) envoie aux DM des approbateurs, `"channel"` envoie au canal d’origine, `"both"` envoie aux deux. Lorsque la cible inclut `"channel"`, les boutons ne peuvent être utilisés que par les approbateurs résolus.
...
- Les instantanés de compte Slack exposent des champs de source/statut par identifiant tels que
  ...
  configuré via SecretRef mais que le chemin actuel de commande/d’exécution n’a pas
...
**Isolement de session des fils :** `thread.historyScope` est par fil (par défaut) ou partagé sur tout le canal. `thread.inheritParent` copie la transcription du canal parent vers les nouveaux fils.

- Le streaming natif Slack plus le statut de fil « is typing... » de type assistant Slack nécessitent une cible de fil de réponse. Les DM de premier niveau restent hors fil par défaut ; ils utilisent donc `typingReaction` ou la livraison normale au lieu de l’aperçu de type fil.
...
| memberInfo      | activé     | Informations du membre   |
...
- `channels.signal.account` : fixe le démarrage du canal à une identité de compte Signal spécifique.
...
- `channels.matrix.autoJoin` vaut `off` par défaut, donc les salles invitées et les nouvelles invitations de type DM sont ignorées jusqu’à ce que vous définissiez `autoJoin: "allowlist"` avec `autoJoinAllowlist` ou `autoJoin: "always"`.
...
- Les sondes de statut Matrix et les recherches de répertoire en direct utilisent la même politique proxy que le trafic d’exécution.
...
- Si vous ajoutez un compte non par défaut via `openclaw channels add` (ou l’onboarding du canal) alors que vous êtes encore sur une configuration de canal de premier niveau à compte unique, OpenClaw promeut d’abord les valeurs de premier niveau à compte unique limitées au compte dans la map de comptes du canal afin que le compte d’origine continue de fonctionner. La plupart des canaux les déplacent vers `channels.<channel>.accounts.default` ; Matrix peut à la place préserver une cible nommée/par défaut existante correspondante.
- Les liaisons existantes limitées au canal uniquement (sans `accountId`) continuent de correspondre au compte par défaut ; les liaisons limitées au compte restent facultatives.
- `openclaw doctor --fix` répare également les formes mixtes en déplaçant les valeurs de premier niveau à compte unique limitées au compte vers le compte promu choisi pour ce canal. La plupart des canaux utilisent `accounts.default` ; Matrix peut à la place préserver une cible nommée/par défaut existante correspondante.

### Autres canaux plugin

De nombreux canaux plugin sont configurés sous la forme `channels.<id>` et documentés sur leurs pages de canal dédiées (par exemple Feishu, Matrix, LINE, Nostr, Zalo, Nextcloud Talk, Synology Chat et Twitch).
Voir l’index complet des canaux : [Canaux](/fr/channels).

### Contrôle par mention en discussion de groupe

Les messages de groupe exigent par défaut une **mention requise** (mention de métadonnées ou modèles regex sûrs). S’applique aux discussions de groupe WhatsApp, Telegram, Discord, Google Chat et iMessage.

**Types de mention :**

- **Mentions de métadonnées** : @mentions natives de la plateforme. Ignorées en mode discussion avec soi-même sur WhatsApp.
- **Modèles texte** : modèles regex sûrs dans `agents.list[].groupChat.mentionPatterns`. Les modèles invalides et les répétitions imbriquées non sûres sont ignorés.
...
Incluez votre propre numéro dans `allowFrom` pour activer le mode discussion avec soi-même (ignore les @mentions natives, ne répond qu’aux modèles texte) :
...
- Ce bloc configure les surfaces de commandes. Pour le catalogue actuel des commandes intégrées + bundlées, voir [Commandes slash](/fr/tools/slash-commands).
...
- `config: true` active `/config` (lecture/écriture de `openclaw.json`). Pour les clients Gateway `chat.send`, les écritures persistantes `/config set|unset` nécessitent aussi `operator.admin` ; la lecture seule `/config show` reste disponible pour les clients operator normaux avec portée d’écriture.
...
- `restart: false` désactive `/restart` et les actions de l’outil de redémarrage de la Gateway. Par défaut : `true`.
...
- `allowFrom` est par fournisseur. Lorsqu’il est défini, c’est l’**unique** source d’autorisation (les listes d’autorisation/associations de canal et `useAccessGroups` sont ignorés).
...
- Carte de la documentation des commandes :
  - catalogue intégré + bundlé : [Commandes slash](/fr/tools/slash-commands)
...
Racine de dépôt facultative affichée sur la ligne Runtime du prompt système. Si elle n’est pas définie, OpenClaw la détecte automatiquement en remontant depuis l’espace de travail.
...
intentionnellement répartis par sous-système au lieu de passer tous par un
seul réglage générique.
...
- `agents.defaults.startupContext.*` :
  ...
  fichiers récents `memory/*.md` du jour.
- `skills.limits.*` :
  la liste compacte de Skills injectée dans le prompt système.
- `agents.defaults.contextLimits.*` :
  extraits limités à l’exécution et blocs injectés possédés par l’exécution.
- `memory.qmd.limits.*` :
...
- `memoryGetMaxChars` : plafond d’extrait par défaut de `memory_get` avant l’ajout
...
Remplacement par agent des réglages partagés `contextLimits`. Les champs omis héritent
...
Taille maximale en pixels du plus grand côté d’image dans les blocs image de transcription/outil avant les appels fournisseur.
Par défaut : `1200`.

Des valeurs plus basses réduisent généralement l’utilisation de jetons de vision et la taille de la charge utile des requêtes pour les exécutions riches en captures d’écran.
...
- `imageGenerationModel` : accepte soit une chaîne (`"provider/model"`), soit un objet (`{ primary, fallbacks }`).
  ...
  - S’il est omis, `image_generate` peut toujours déduire une valeur par défaut de fournisseur adossée à une authentification. Il essaie d’abord le fournisseur par défaut actuel, puis les autres fournisseurs de génération d’images enregistrés dans l’ordre des ID de fournisseur.
- `musicGenerationModel` : accepte soit une chaîne (`"provider/model"`), soit un objet (`{ primary, fallbacks }`).
  ...
  - S’il est omis, `music_generate` peut toujours déduire une valeur par défaut de fournisseur adossée à une authentification. Il essaie d’abord le fournisseur par défaut actuel, puis les autres fournisseurs de génération musicale enregistrés dans l’ordre des ID de fournisseur.
...
- `videoGenerationModel` : accepte soit une chaîne (`"provider/model"`), soit un objet (`{ primary, fallbacks }`).
  ...
  - S’il est omis, `video_generate` peut toujours déduire une valeur par défaut de fournisseur adossée à une authentification. Il essaie d’abord le fournisseur par défaut actuel, puis les autres fournisseurs de génération vidéo enregistrés dans l’ordre des ID de fournisseur.
  - Si vous sélectionnez directement un provider/model, configurez aussi l’authentification/la clé API du fournisseur correspondant.
  - Le fournisseur bundlé de génération vidéo Qwen prend en charge jusqu’à 1 vidéo de sortie, 1 image d’entrée, 4 vidéos d’entrée, 10 secondes de durée, et les options de niveau fournisseur `size`, `aspectRatio`, `resolution`, `audio` et `watermark`.
- `pdfModel` : accepte soit une chaîne (`"provider/model"`), soit un objet (`{ primary, fallbacks }`).
  ...
  - S’il est omis, l’outil PDF se replie sur `imageModel`, puis sur le modèle de session/par défaut résolu.
...
- `elevatedDefault` : niveau elevated-output par défaut pour les agents. Valeurs : `"off"`, `"on"`, `"ask"`, `"full"`. Par défaut : `"on"`.
- `model.primary` : format `provider/model` (par ex. `openai/gpt-5.4`). Si vous omettez le fournisseur, OpenClaw essaie d’abord un alias, puis une correspondance unique de fournisseur configuré pour cet ID de modèle exact, et seulement ensuite se replie sur le fournisseur par défaut configuré (comportement de compatibilité déprécié, donc préférez `provider/model` explicite). Si ce fournisseur n’expose plus le modèle par défaut configuré, OpenClaw se replie sur le premier provider/model configuré au lieu d’exposer une valeur par défaut obsolète d’un fournisseur supprimé.
- `models` : le catalogue de modèles configuré et la liste d’autorisation pour `/model`. Chaque entrée peut inclure `alias` (raccourci) et `params` (spécifiques au fournisseur, par exemple `temperature`, `maxTokens`, `cacheRetention`, `context1m`).
  ...
  - Les flux configure/onboarding limités au fournisseur fusionnent les modèles du fournisseur sélectionné dans cette map et préservent les fournisseurs non liés déjà configurés.
...
- `embeddedHarness` : politique par défaut du runtime d’agent intégré de bas niveau. Utilisez `runtime: "auto"` pour laisser les harness de plugin enregistrés prendre en charge les modèles pris en charge, `runtime: "pi"` pour forcer le harness PI intégré, ou un ID de harness enregistré tel que `runtime: "codex"`. Définissez `fallback: "none"` pour désactiver le repli automatique vers PI.
- Les écrivains de configuration qui modifient ces champs (par exemple `/models set`, `/models set-image` et les commandes add/remove de repli) enregistrent la forme objet canonique et préservent si possible les listes de repli existantes.
- `maxConcurrent` : nombre maximal d’exécutions parallèles d’agent entre les sessions (chaque session reste sérialisée). Par défaut : 4.
...
- `runtime` : `"auto"`, `"pi"` ou un ID de harness de plugin enregistré. Le plugin Codex bundlé enregistre `codex`.
- `fallback` : `"pi"` ou `"none"`. `"pi"` conserve le harness PI intégré comme repli de compatibilité lorsqu’aucun harness de plugin n’est sélectionné. `"none"` fait échouer une sélection de harness de plugin absente ou non prise en charge au lieu d’utiliser silencieusement PI. Les échecs du harness de plugin sélectionné sont toujours exposés directement.
...
- Cela ne contrôle que le harness de discussion intégré. La génération de médias, la vision, le PDF, la musique, la vidéo et le TTS utilisent toujours leurs paramètres provider/model.
...
Les modèles Z.AI GLM-4.x activent automatiquement le mode de raisonnement sauf si vous définissez `--thinking off` ou définissez vous-même `agents.defaults.models["zai/<model>"].params.thinking`.
...
Les modèles Anthropic Claude 4.6 utilisent par défaut le raisonnement `adaptive` lorsqu’aucun niveau de raisonnement explicite n’est défini.

### `agents.defaults.cliBackends`

Backends CLI facultatifs pour les exécutions de repli texte uniquement (sans appels d’outils). Utiles comme sauvegarde lorsque les fournisseurs API échouent.
...
- Passage d’image pris en charge lorsque `imageArg` accepte des chemins de fichiers.
...
Overlays de prompt indépendants du fournisseur appliqués par famille de modèles. Les ID de modèle de la famille GPT-5 reçoivent le contrat de comportement partagé entre fournisseurs ; `personality` contrôle uniquement la couche de style d’interaction amical.
...
- L’ancienne valeur `plugins.entries.openai.config.personality` est toujours lue lorsque ce paramètre partagé n’est pas défini.
...
- `directPolicy` : politique de livraison directe/DM. `allow` (par défaut) autorise la livraison directe vers la cible. `block` supprime la livraison directe vers la cible et émet `reason=dm-blocked`.
- `lightContext` : lorsqu’il vaut true, les exécutions Heartbeat utilisent un contexte bootstrap léger et ne conservent que `HEARTBEAT.md` parmi les fichiers bootstrap de l’espace de travail.
- `isolatedSession` : lorsqu’il vaut true, chaque exécution Heartbeat s’exécute dans une nouvelle session sans historique de conversation antérieur. Même modèle d’isolation que Cron `sessionTarget: "isolated"`. Réduit le coût en jetons par Heartbeat d’environ ~100K à ~2-5K jetons.
...
- `provider` : id d’un Plugin provider de Compaction enregistré. Lorsqu’il est défini, la méthode `summarize()` du provider est appelée au lieu du résumé LLM intégré. Se replie sur l’implémentation intégrée en cas d’échec. Définir un provider force `mode: "safeguard"`. Voir [Compaction](/fr/concepts/compaction).
- `timeoutSeconds` : nombre maximal de secondes autorisé pour une opération de Compaction unique avant qu’OpenClaw ne l’interrompe. Par défaut : `900`.
...
- `notifyUser` : lorsqu’il vaut `true`, envoie de brefs avis à l’utilisateur au début et à la fin de la Compaction (par exemple, « Compacting context... » et « Compaction complete »). Désactivé par défaut pour garder la Compaction silencieuse.
...
- `ttl` contrôle à quelle fréquence l’élagage peut être relancé (après le dernier accès au cache).
- L’élagage applique d’abord une troncature légère aux résultats d’outil surdimensionnés, puis efface complètement les anciens résultats d’outil si nécessaire.
...
Voir [Streaming](/fr/concepts/streaming) pour les détails sur le comportement + la segmentation.
...
- `identityData` / `certificateData` / `knownHostsData` : contenus inline ou SecretRef que OpenClaw matérialise en fichiers temporaires à l’exécution
...
- `mirror` : initialise l’espace distant depuis le local avant `exec`, resynchronise après `exec` ; l’espace de travail local reste canonique
- `remote` : initialise l’espace distant une seule fois lors de la création du sandbox, puis conserve l’espace de travail distant comme canonique
...
- `allowHostControl: false` (par défaut) bloque les sessions sandboxées ciblant le navigateur de l’hôte.
...
- `cdpSourceRange` restreint facultativement l’entrée CDP à la périphérie du conteneur à une plage CIDR (par exemple `172.21.0.1/32`).
- `sandbox.browser.binds` monte des répertoires hôtes supplémentaires uniquement dans le conteneur de navigateur sandboxé. Lorsqu’il est défini (y compris `[]`), il remplace `docker.binds` pour le conteneur de navigateur.
- Les valeurs par défaut de lancement sont définies dans `scripts/sandbox-browser-entrypoint.sh` et ajustées pour les hôtes conteneurisés :
  ...
  - Les valeurs par défaut constituent la base de l’image conteneur ; utilisez une image de navigateur personnalisée avec un entrypoint personnalisé pour modifier les valeurs par défaut du conteneur.
...
- `id` : id stable de l’agent (requis).
- `default` : lorsque plusieurs sont définis, le premier l’emporte (un avertissement est consigné). Si aucun n’est défini, la première entrée de la liste est l’agent par défaut.
...
- `thinkingDefault` : niveau de raisonnement par défaut facultatif par agent (`off | minimal | low | medium | high | xhigh | adaptive | max`). Remplace `agents.defaults.thinkingDefault` pour cet agent lorsqu’aucun remplacement par message ou session n’est défini.
...
- `embeddedHarness` : remplacement facultatif par agent de la politique de harness de bas niveau. Utilisez `{ runtime: "codex", fallback: "none" }` pour faire d’un agent un agent Codex uniquement tandis que d’autres agents conservent le repli PI par défaut.
...
Pour les entrées `type: "acp"`, OpenClaw résout par identité exacte de conversation (`match.channel` + compte + `match.peer.id`) et n’utilise pas l’ordre des niveaux de liaison route ci-dessus.
...
- **`dmScope`** : comment les DM sont regroupés.
  ...
  - `per-account-channel-peer` : isoler par compte + canal + expéditeur (recommandé pour le multi-comptes).
- **`identityLinks`** : map des id canoniques vers des pairs préfixés par fournisseur pour le partage de session inter-canaux.
- **`reset`** : politique principale de réinitialisation. `daily` réinitialise à `atHour` heure locale ; `idle` réinitialise après `idleMinutes`. Lorsque les deux sont configurés, le premier à expirer l’emporte.
...
- **`agentToAgent.maxPingPongTurns`** : nombre maximal de tours de réponse entre agents lors des échanges agent à agent (entier, plage : `0`–`5`). `0` désactive l’enchaînement ping-pong.
...
Regroupe les messages texte rapides du même expéditeur en un seul tour d’agent. Les médias/pièces jointes déclenchent un envoi immédiat. Les commandes de contrôle contournent le debounce.
...
- Les anciennes clés plates Talk (`talk.voiceId`, `talk.voiceAliases`, `talk.modelId`, `talk.outputFormat`, `talk.apiKey`) sont uniquement conservées pour compatibilité et migrées automatiquement vers `talk.providers.<provider>`.
...
Politique globale d’autorisation/refus d’outils (le refus l’emporte). Insensible à la casse, prend en charge les jokers `*`. S’applique même lorsque le sandbox Docker est désactivé.
...
- `exec` elevated contourne le sandboxing et utilise le chemin d’échappement configuré (`gateway` par défaut, ou `node` lorsque la cible exec est `node`).
...
Les contrôles de sécurité contre les boucles d’outils sont **désactivés par défaut**. Définissez `enabled: true` pour activer la détection.
...
- `warningThreshold` : seuil de schéma répétitif sans progression pour les avertissements.
...
- `detectors.pingPong` : avertit/bloque sur les schémas alternés par paires sans progression.
...
Par défaut : `tree` (session actuelle + sessions générées par elle, comme les sous-agents).
...
- `agent` : toute session appartenant à l’id d’agent actuel (peut inclure d’autres utilisateurs si vous exécutez des sessions per-sender sous le même id d’agent).
...
- Les entrées Base64 sont validées avec des contrôles stricts d’alphabet/padding et une protection de taille avant décodage.
...
Indicateurs d’outils intégrés expérimentaux. Désactivés par défaut sauf lorsqu’une règle d’activation automatique stricte-agentic GPT-5 s’applique.
...
- Par défaut : `false` sauf si `agents.defaults.embeddedPi.executionContract` (ou un remplacement par agent) est défini sur `"strict-agentic"` pour une exécution OpenAI ou OpenAI Codex de la famille GPT-5. Définissez `true` pour forcer l’outil en dehors de cette portée, ou `false` pour le garder désactivé même pour les exécutions strict-agentic GPT-5.
...
- Priorité de fusion pour les ID de provider correspondants :
  ...
  - Les valeurs `apiKey` non vides de l’agent ne l’emportent que lorsque ce provider n’est pas géré par SecretRef dans le contexte actuel de configuration/profil d’authentification.
  - Les valeurs `apiKey` de provider gérées par SecretRef sont rafraîchies à partir des marqueurs de source (`ENV_VAR_NAME` pour les références env, `secretref-managed` pour les références file/exec) au lieu de persister les secrets résolus.
  - Les valeurs d’en-tête de provider gérées par SecretRef sont rafraîchies à partir des marqueurs de source (`secretref-env:ENV_VAR_NAME` pour les références env, `secretref-managed` pour les références file/exec).
  ...
  - Les `contextTokens` du modèle correspondant conservent une limite d’exécution explicite lorsqu’elle est présente ; utilisez-la pour limiter le contexte effectif sans modifier les métadonnées natives du modèle.
  - Utilisez `models.mode: "replace"` lorsque vous voulez que la configuration réécrive complètement `models.json`.
  - La persistance des marqueurs est autoritaire selon la source : les marqueurs sont écrits à partir de l’instantané de configuration source actif (avant résolution), et non à partir des valeurs de secret résolues à l’exécution.
...
- `models.providers.*.apiKey` : identifiant du provider (préférez SecretRef/substitution env).
...
- `models.providers.*.baseUrl` : URL de base API upstream.
- `models.providers.*.headers` : en-têtes statiques supplémentaires pour le routage proxy/tenant.
- `models.providers.*.request` : remplacements de transport pour les requêtes HTTP du model-provider.
  ...
  - `request.auth` : remplacement de stratégie d’authentification. Modes : `"provider-default"` (utiliser l’authentification intégrée du provider), `"authorization-bearer"` (avec `token`), `"header"` (avec `headerName`, `value`, `prefix` facultatif).
  ...
  - `request.allowPrivateNetwork` : lorsqu’il vaut `true`, autorise HTTPS vers `baseUrl` lorsque le DNS se résout vers des plages privées, CGNAT ou similaires, via la protection SSRF du fetch HTTP provider (activation explicite opérateur pour des points de terminaison auto-hébergés fiables compatibles OpenAI). WebSocket utilise le même `request` pour les en-têtes/TLS mais pas cette protection SSRF de fetch. Par défaut `false`.
...
- `models.providers.*.models.*.compat.requiresStringContent` : indication facultative de compatibilité pour les points de terminaison de chat compatibles OpenAI limités aux chaînes. Lorsqu’il vaut `true`, OpenClaw aplatit les tableaux `messages[].content` purement textuels en chaînes simples avant d’envoyer la requête.
...
- `plugins.entries.amazon-bedrock.config.discovery.refreshInterval` : intervalle de polling pour le rafraîchissement de la découverte.
...
Les points de terminaison natifs Moonshot annoncent une compatibilité d’utilisation du streaming sur le transport partagé
`openai-completions`, et OpenClaw s’appuie sur les capacités du point de terminaison
plutôt que sur le seul id intégré du provider.
...
Sur le chemin de streaming compatible Anthropic, OpenClaw désactive le raisonnement MiniMax
...
Voir [Modèles locaux](/fr/gateway/local-models). En bref : exécutez un grand modèle local via l’API LM Studio Responses sur un matériel sérieux ; conservez les modèles hébergés fusionnés pour le repli.
...
- `allowBundled` : liste d’autorisation facultative pour les Skills bundlées uniquement (les Skills gérées/de l’espace de travail ne sont pas affectées).
...
- `entries.<skillKey>.enabled: false` désactive un Skills même s’il est bundlé/installé.
- `entries.<skillKey>.apiKey` : champ pratique pour les Skills déclarant une variable env principale (chaîne en clair ou objet SecretRef).
...
- La découverte accepte les plugins OpenClaw natifs ainsi que les bundles compatibles Codex et les bundles Claude, y compris les bundles Claude sans manifeste utilisant la disposition par défaut.
...
- `plugins.entries.<id>.subagent.allowedModels` : liste d’autorisation facultative des cibles canoniques `provider/model` pour les remplacements fiables de sous-agent. Utilisez `"*"` uniquement lorsque vous souhaitez intentionnellement autoriser n’importe quel modèle.
...
- `plugins.entries.firecrawl.config.webFetch` : paramètres du provider web-fetch Firecrawl.
  ...
  - `baseUrl` : URL de base API Firecrawl (par défaut : `https://api.firecrawl.dev`).
...
- Les plugins Claude bundle activés peuvent aussi fournir des valeurs par défaut embedded Pi depuis `settings.json` ; OpenClaw les applique comme paramètres d’agent assainis, et non comme correctifs bruts de configuration OpenClaw.
...
- En mode strict, les points de terminaison de profils CDP distants (`profiles.*.cdpUrl`) sont soumis au même blocage du réseau privé pendant les contrôles de joignabilité/découverte.
...
- Les profils `existing-session` utilisent Chrome MCP au lieu de CDP et peuvent s’attacher
  sur l’hôte sélectionné ou via un browser Node connecté.
- Les profils `existing-session` peuvent définir `userDataDir` pour cibler un profil
  spécifique de navigateur basé sur Chromium tel que Brave ou Edge.
- Les profils `existing-session` conservent les limites actuelles de routage Chrome MCP :
  actions basées sur instantané/référence au lieu du ciblage par sélecteur CSS, hooks de téléversement
  d’un seul fichier, pas de remplacements du délai d’expiration des boîtes de dialogue, pas de `wait --load networkidle`, et pas de `responsebody`, export PDF, interception des téléchargements ni actions par lots.
- Les profils gérés localement `openclaw` attribuent automatiquement `cdpPort` et `cdpUrl` ; ne définissez
...
- `assistant` : remplacement d’identité de Control UI. Se replie sur l’identité de l’agent actif.
...
- **Alias de bind hérités** : utilisez les valeurs de mode de bind dans `gateway.bind` (`auto`, `loopback`, `lan`, `tailnet`, `custom`), et non les alias d’hôte (`0.0.0.0`, `127.0.0.1`, `localhost`, `::`, `::1`).
...
- **Auth** : requise par défaut. Les binds non loopback exigent l’authentification Gateway. En pratique, cela signifie un jeton/mot de passe partagé ou un proxy inverse conscient de l’identité avec `gateway.auth.mode: "trusted-proxy"`. L’assistant d’onboarding génère un jeton par défaut.
...
- `gateway.auth.mode: "none"` : mode sans authentification explicite. À utiliser uniquement pour des configurations locales loopback de confiance ; ce mode n’est intentionnellement pas proposé par les invites d’onboarding.
- `gateway.auth.mode: "trusted-proxy"` : déléguer l’authentification à un proxy inverse conscient de l’identité et faire confiance aux en-têtes d’identité provenant de `gateway.trustedProxies` (voir [Auth Trusted Proxy](/fr/gateway/trusted-proxy-auth)). Ce mode attend une source proxy **non loopback** ; les proxies inverses loopback sur le même hôte ne satisfont pas l’authentification trusted-proxy.
- `gateway.auth.allowTailscale` : lorsqu’il vaut `true`, les en-têtes d’identité Tailscale Serve peuvent satisfaire l’authentification de Control UI/WebSocket (vérifiée via `tailscale whois`). Les points de terminaison d’API HTTP n’utilisent **pas** cette authentification par en-tête Tailscale ; ils suivent à la place le mode HTTP normal de la Gateway. Ce flux sans jeton suppose que l’hôte Gateway est de confiance. Vaut par défaut `true` lorsque `tailscale.mode = "serve"`.
- `gateway.auth.rateLimit` : limiteur facultatif des échecs d’authentification. S’applique par IP client et par portée d’authentification (secret partagé et jeton d’appareil sont suivis indépendamment). Les tentatives bloquées renvoient `429` + `Retry-After`.
  - Sur le chemin asynchrone Tailscale Serve de Control UI, les tentatives échouées pour le même `{scope, clientIp}` sont sérialisées avant l’écriture de l’échec. Des tentatives incorrectes concurrentes provenant du même client peuvent donc déclencher le limiteur sur la deuxième requête au lieu de laisser les deux passer comme simples incompatibilités.
  - `gateway.auth.rateLimit.exemptLoopback` vaut par défaut `true` ; définissez `false` lorsque vous souhaitez intentionnellement limiter aussi le trafic localhost (pour des configurations de test ou des déploiements proxy stricts).
- Les tentatives d’authentification WS d’origine navigateur sont toujours limitées avec l’exemption loopback désactivée (défense en profondeur contre la force brute localhost basée navigateur).
- En loopback, ces verrouillages d’origine navigateur sont isolés par valeur `Origin`
  normalisée, de sorte que des échecs répétés provenant d’une origine localhost ne
...
- `OPENCLAW_ALLOW_INSECURE_PRIVATE_WS=1` : remplacement côté client break-glass qui autorise `ws://` en clair vers des IP de réseau privé de confiance ; par défaut, le mode en clair reste limité au loopback.
...
- `gateway.push.apns.relay.baseUrl` : URL HTTPS de base du relais APNs externe utilisé par les builds iOS officiels/TestFlight après qu’ils ont publié des enregistrements adossés au relais vers la Gateway. Cette URL doit correspondre à l’URL du relais compilée dans le build iOS.
- `gateway.push.apns.relay.timeoutMs` : délai d’expiration en millisecondes pour les envois Gateway-vers-relais. Par défaut : `10000`.
- Les enregistrements adossés au relais sont délégués à une identité Gateway spécifique. L’application iOS associée récupère `gateway.identity.get`, inclut cette identité dans l’enregistrement du relais et transmet une autorisation d’envoi limitée à l’enregistrement à la Gateway. Une autre Gateway ne peut pas réutiliser cet enregistrement stocké.
...
- `gateway.channelStaleEventThresholdMinutes` : seuil de socket périmée en minutes. Conservez une valeur supérieure ou égale à `gateway.channelHealthCheckMinutes`. Par défaut : `30`.
...
- Si `gateway.auth.token` / `gateway.auth.password` est explicitement configuré via SecretRef et non résolu, la résolution échoue de façon fermée (aucun repli distant masquant).
- `trustedProxies` : IP de proxies inverses qui terminent TLS ou injectent des en-têtes de client transféré. N’indiquez que des proxies que vous contrôlez. Les entrées loopback restent valides pour les configurations de détection proxy locale/sur le même hôte (par exemple Tailscale Serve ou un proxy inverse local), mais elles ne rendent **pas** les requêtes loopback éligibles à `gateway.auth.mode: "trusted-proxy"`.
- `allowRealIpFallback` : lorsqu’il vaut `true`, la Gateway accepte `X-Real-IP` si `X-Forwarded-For` est absent. Par défaut `false` pour un comportement en échec fermé.
...
- En-tête facultatif de renforcement des réponses :
  - `gateway.http.securityHeaders.strictTransportSecurity` (à définir uniquement pour les origines HTTPS que vous contrôlez ; voir [Auth Trusted Proxy](/fr/gateway/trusted-proxy-auth#tls-termination-and-hsts))
...
- `autoGenerate` : génère automatiquement une paire locale certificat/clé auto-signée lorsqu’aucun fichier explicite n’est configuré ; pour usage local/dev uniquement.
- `certPath` : chemin système de fichiers vers le fichier de certificat TLS.
- `keyPath` : chemin système de fichiers vers le fichier de clé privée TLS ; conservez des permissions restrictives.
...
- `mode` : contrôle la façon dont les modifications de configuration sont appliquées à l’exécution.
  ...
  - `"hot"` : appliquer les changements en cours de processus sans redémarrer.
  - `"hybrid"` (par défaut) : essayer d’abord le rechargement à chaud ; se replier sur un redémarrage si nécessaire.
- `debounceMs` : fenêtre de debounce en ms avant l’application des changements de configuration (entier non négatif).
- `deferralTimeoutMs` : temps maximal en ms d’attente des opérations en cours avant de forcer un redémarrage (par défaut : `300000` = 5 minutes).
...
- Si un mappage ou un preset utilise un `sessionKey` avec modèle, définissez `hooks.allowedSessionKeyPrefixes` et `hooks.allowRequestSessionKey=true`. Les clés de mappage statiques ne nécessitent pas cette activation explicite.
...
- `POST /hooks/<name>` → résolu via `hooks.mappings`
  - Les valeurs `sessionKey` de mappage rendues par modèle sont traitées comme fournies de l’extérieur et exigent aussi `hooks.allowRequestSessionKey=true`.
...
- `allowRequestSessionKey` : autorise les appelants `/hooks/agent` et les clés de session de mappage pilotées par modèles à définir `sessionKey` (par défaut : `false`).
- `allowedSessionKeyPrefixes` : liste d’autorisation facultative de préfixes pour les valeurs explicites de `sessionKey` (requête + mappage), par ex. `["hook:"]`. Elle devient requise lorsqu’un mappage ou preset utilise un `sessionKey` avec modèle.
...
- Si vous avez besoin de `hooks.allowRequestSessionKey: false`, remplacez le preset avec un `sessionKey` statique au lieu de la valeur par défaut à modèle.
...
- Sert le HTML/CSS/JS modifiable par agent et A2UI en HTTP sous le port Gateway :
...
- Les Node WebViews n’envoient généralement pas d’en-têtes d’authentification ; une fois qu’un Node est associé et connecté, la Gateway annonce des URL de capacité limitées au Node pour l’accès canvas/A2UI.
- Les URL de capacité sont liées à la session WS active du Node et expirent rapidement. Le repli basé sur IP n’est pas utilisé.
- Injecte le client live-reload dans le HTML servi.
...
- Désactivez le live reload pour les grands répertoires ou en cas d’erreurs `EMFILE`.
...
- Les variables d’environnement inline ne sont appliquées que si la variable manque dans l’environnement du processus.
...
Les références de secret sont additives : les valeurs en clair fonctionnent toujours.
...
- les id `source: "exec"` ne doivent pas contenir de segments de chemin `/./` ou `/../` délimités par des barres obliques (par exemple `a/../b` est rejeté)
...
- Si `trustedDirs` est configuré, le contrôle de répertoire de confiance s’applique au chemin cible résolu.
...
- Les identifiants statiques à l’exécution proviennent d’instantanés résolus en mémoire ; les anciennes entrées statiques `auth.json` sont nettoyées lorsqu’elles sont détectées.
...
- `billingBackoffHours` : délai de repli de base en heures lorsqu’un profil échoue en raison de véritables
  ...
  correspondances textuelles spécifiques au provider restent limitées au provider qui les possède (par exemple OpenRouter
  `Key limit exceeded`). Les messages `402` d’utilisation dans une fenêtre de temps ou de
  limite de dépenses d’organisation/espace de travail qui peuvent être réessayés restent dans la voie `rate_limit`
  à la place.
- `billingBackoffHoursByProvider` : remplacements facultatifs par provider pour le délai de repli de facturation en heures.
...
- `overloadedProfileRotations` : nombre maximal de rotations de profil d’authentification pour un même provider en cas d’erreurs de surcharge avant de passer au modèle de repli (par défaut : `1`). Les formes provider-busy telles que `ModelNotReadyException` aboutissent ici.
- `overloadedBackoffMs` : délai fixe avant nouvelle tentative d’une rotation de provider/profil surchargé (par défaut : `0`).
- `rateLimitedProfileRotations` : nombre maximal de rotations de profil d’authentification pour un même provider en cas d’erreurs de limitation de débit avant de passer au modèle de repli (par défaut : `1`). Ce compartiment rate-limit inclut des textes propres au provider tels que `Too many concurrent requests`, `ThrottlingException`, `concurrency limit reached`, `workers_ai ... quota limit exceeded` et `resource exhausted`.
...
- `stuckSessionWarnMs` : seuil d’ancienneté en ms pour émettre des avertissements de session bloquée lorsqu’une session reste à l’état de traitement.
...
- `otel.flushIntervalMs` : intervalle périodique de flush de télémétrie en ms.
...
- `cacheTrace.includeMessages` / `includePrompt` / `includeSystem` : contrôle ce qui est inclus dans la sortie de trace de cache (tous à `true` par défaut).
...
- `dispatch.enabled` : contrôle indépendant pour l’envoi des tours de session ACP (par défaut : `true`). Définissez `false` pour conserver les commandes ACP disponibles tout en bloquant l’exécution.
- `backend` : id par défaut du backend de runtime ACP (doit correspondre à un plugin de runtime ACP enregistré).
- `defaultAgent` : id d’agent ACP cible de repli lorsque les lancements ne spécifient pas de cible explicite.
- `allowedAgents` : liste d’autorisation des id d’agent autorisés pour les sessions runtime ACP ; vide signifie aucune restriction supplémentaire.
...
- `stream.maxChunkChars` : taille maximale d’un segment avant division de la projection du bloc diffusé.
- `stream.repeatSuppression` : supprime les lignes répétées de statut/outil par tour (par défaut : `true`).
...
- `stream.tagVisibility` : enregistrement des noms de tags vers des remplacements booléens de visibilité pour les événements diffusés.
...
Voir les champs `identity` de `agents.list` sous [Valeurs par défaut des agents](#agent-defaults).

---

## Bridge (hérité, supprimé)

Les builds actuels n’incluent plus le bridge TCP. Les Nodes se connectent via la WebSocket Gateway. Les clés `bridge.*` ne font plus partie du schéma de configuration (la validation échoue tant qu’elles ne sont pas supprimées ; `openclaw doctor --fix` peut supprimer les clés inconnues).
...
- `sessionRetention` : durée de conservation des sessions d’exécution Cron isolées terminées avant élagage de `sessions.json`. Contrôle aussi le nettoyage des transcriptions Cron archivées supprimées. Par défaut : `24h` ; définissez `false` pour désactiver.
...
- `runLog.keepLines` : dernières lignes conservées lorsque l’élagage du journal d’exécution est déclenché. Par défaut : `2000`.
...
- `webhook` : URL webhook héritée dépréciée (http/https) utilisée uniquement pour les tâches stockées qui ont encore `notify: true`.
...
- `maxAttempts` : nombre maximal de nouvelles tentatives pour les tâches one-shot en cas d’erreurs transitoires (par défaut : `3` ; plage : `0`–`10`).
- `backoffMs` : tableau des délais de repli en ms pour chaque nouvelle tentative (par défaut : `[30000, 60000, 300000]` ; 1–10 entrées).
...
S’applique uniquement aux tâches Cron one-shot. Les tâches récurrentes utilisent une gestion d’échec distincte.
...
- `accountId` : id de compte ou de canal facultatif pour limiter la livraison de l’alerte.
...
- Lorsqu’aucune destination d’échec globale ou par tâche n’est définie, les tâches déjà livrées via `announce` se replient sur cette cible announce principale en cas d’échec.
...
Espaces réservés de modèle étendus dans `tools.media.models[].args` :

| Variable           | Description                                       |
| ------------------ | ------------------------------------------------- |
| `{{Body}}`         | Corps complet du message entrant                  |
| `{{RawBody}}`      | Corps brut (sans habillages d’historique/d’expéditeur) |
...
## Inclus de configuration (`$include`)
...
- Inclus imbriqués : jusqu’à 10 niveaux de profondeur.
- Chemins : résolus relativement au fichier incluant, mais doivent rester dans le répertoire de configuration de niveau supérieur (`dirname` de `openclaw.json`). Les formes absolues/`../` ne sont autorisées que si elles se résolvent toujours à l’intérieur de cette limite.
- Les écritures gérées par OpenClaw qui modifient uniquement une section de niveau supérieur adossée à une inclusion de fichier unique écrivent directement dans ce fichier inclus. Par exemple, `plugins install` met à jour `plugins: { $include: "./plugins.json5" }` dans `plugins.json5` et laisse `openclaw.json` intact.
- Les inclusions racine, les tableaux d’inclusions et les inclusions avec remplacements par clés sœurs sont en lecture seule pour les écritures gérées par OpenClaw ; ces écritures échouent de façon fermée au lieu d’aplatir la configuration.
