---
read_when:
    - Vous avez besoin de la sémantique ou des valeurs par défaut exactes au niveau des champs de configuration
...
x-i18n:
    generated_at: "2026-05-06T17:55:47Z"
    model: gpt-5.5
    provider: openai
...
Couvre les principales surfaces de configuration d’OpenClaw et renvoie vers d’autres pages lorsqu’un sous-système dispose de sa propre référence plus approfondie. Les catalogues de commandes propres aux canaux et aux plugins, ainsi que les réglages mémoire/QMD avancés, se trouvent sur leurs propres pages plutôt qu’ici.

Source de vérité du code :

- `openclaw config schema` affiche le JSON Schema actif utilisé pour la validation et la Control UI, avec les métadonnées intégrées/de plugin/de canal fusionnées lorsqu’elles sont disponibles
...
Chemin de recherche pour les agents : utilisez l’action d’outil `gateway` `config.schema.lookup` pour
...
canaux groupés (authentification, contrôle d’accès, multi-compte, déclenchement par mention).
...
- `agents.defaults.*` (espace de travail, modèle, raisonnement, heartbeat, mémoire, médias, Skills, sandbox)
...
La politique des outils, les bascules expérimentales, la configuration d’outils adossés à un fournisseur et la
configuration de fournisseurs personnalisés / d’URL de base ont été déplacées vers une page dédiée - consultez
...
- `mcp.servers` : définitions nommées de serveurs MCP stdio ou distants pour les environnements d’exécution qui
  ...
  `type: "http"` est un alias propre à la CLI que `openclaw mcp set` et
  `openclaw doctor --fix` normalisent vers le champ canonique `transport`.
- `mcp.sessionIdleTtlMs` : TTL d’inactivité pour les environnements d’exécution MCP groupés et limités à la session.
...
- Les modifications sous `mcp.*` s’appliquent à chaud en supprimant les environnements d’exécution MCP de session mis en cache.
  La prochaine découverte/utilisation d’outil les recrée à partir de la nouvelle configuration ; ainsi, les entrées
...
- `allowBundled` : liste d’autorisation facultative pour les Skills groupés uniquement (Skills gérés/de l’espace de travail non affectés).
...
- La découverte accepte les plugins OpenClaw natifs ainsi que les bundles Codex compatibles et les bundles Claude, y compris les bundles Claude à disposition par défaut sans manifeste.
...
- `bundledDiscovery` : vaut par défaut `"allowlist"` pour les nouvelles configurations, de sorte qu’un
  `plugins.allow` non vide filtre également les plugins de fournisseurs groupés, y compris les fournisseurs d’exécution
...
- `plugins.entries.<id>.hooks.allowConversationAccess` : lorsque `true`, les plugins non groupés de confiance peuvent lire le contenu brut des conversations depuis des hooks typés tels que `llm_input`, `llm_output`, `before_model_resolve`, `before_agent_reply`, `before_agent_run`, `before_agent_finalize` et `agent_end`.
...
- `plugins.entries.memory-core.config.dreaming` : paramètres de memory dreaming. Consultez [Dreaming](/fr/concepts/dreaming) pour les phases et les seuils.
  ...
  - `model` : remplacement facultatif du modèle de sous-agent Dream Diary. Nécessite `plugins.entries.memory-core.subagent.allowModelOverride: true` ; associez-le à `allowedModels` pour restreindre les cibles. Les erreurs d’indisponibilité du modèle réessaient une fois avec le modèle par défaut de la session ; les échecs de confiance ou de liste d’autorisation ne reviennent pas silencieusement en arrière.
  - la politique de phase et les seuils sont des détails d’implémentation (pas des clés de configuration exposées aux utilisateurs).
...
`commitments` contrôle la mémoire de suivi inférée : OpenClaw peut détecter des points de suivi à partir des tours de conversation et les livrer via des exécutions de heartbeat.

- `commitments.enabled` : activer l’extraction LLM masquée, le stockage et la livraison par heartbeat des engagements de suivi inférés. Par défaut : `false`.
...
- Les profils `existing-session` peuvent définir `userDataDir` pour cibler un profil de navigateur spécifique
  basé sur Chromium, comme Brave ou Edge.
- Les profils `existing-session` conservent les limites actuelles des routes Chrome MCP :
  actions pilotées par instantané/référence au lieu du ciblage par sélecteur CSS, hooks de téléversement
  d’un fichier, aucune substitution de délai d’attente de dialogue, pas de `wait --load networkidle`, et aucun
  `responsebody`, export PDF, interception de téléchargement ni action par lot.
...
- `seamColor` : couleur d’accentuation pour le chrome de l’interface utilisateur native de l’application (teinte de bulle Talk Mode, etc.).
- `assistant` : remplacement de l’identité de la Control UI. Repli sur l’identité de l’agent actif.
...
- **Note Docker** : la liaison `loopback` par défaut écoute sur `127.0.0.1` à l’intérieur du conteneur. Avec la mise en réseau par pont Docker (`-p 18789:18789`), le trafic arrive sur `eth0`, donc le Gateway est inaccessible. Utilisez `--network host`, ou définissez `bind: "lan"` (ou `bind: "custom"` avec `customBindHost: "0.0.0.0"`) pour écouter sur toutes les interfaces.
- **Authentification** : requise par défaut. Les liaisons non-loopback nécessitent l’authentification du Gateway. En pratique, cela signifie un jeton/mot de passe partagé ou un proxy inverse sensible à l’identité avec `gateway.auth.mode: "trusted-proxy"`. L’assistant d’intégration génère un jeton par défaut.
...
- `gateway.auth.mode: "none"` : mode explicite sans authentification. À utiliser uniquement pour des configurations local loopback de confiance ; il n’est volontairement pas proposé par les invites d’intégration.
...
- Les tentatives d’authentification WS provenant d’une origine navigateur sont toujours limitées avec l’exemption loopback désactivée (défense en profondeur contre la force brute localhost depuis le navigateur).
...
- `controlUi.chatMessageMaxWidth` : largeur maximale facultative pour les messages de discussion groupés de l’interface de contrôle. Accepte des valeurs de largeur CSS contraintes telles que `960px`, `82%`, `min(1280px, 82%)` et `calc(100% - 2rem)`.
...
- `OPENCLAW_ALLOW_INSECURE_PRIVATE_WS=1` : dérogation d’urgence côté client via l’environnement de processus
  ...
  `openclaw.json`, et une configuration de réseau privé navigateur telle que
  `browser.ssrfPolicy.dangerouslyAllowPrivateNetwork` n’affecte pas les clients WebSocket
  du Gateway.
- `gateway.remote.token` / `.password` sont des champs d’identifiants de client distant. Ils ne configurent pas l’authentification du Gateway à eux seuls.
- `gateway.push.apns.relay.baseUrl` : URL HTTPS de base du relais APNs externe utilisé par les builds iOS officiels/TestFlight après publication des enregistrements adossés au relais dans le Gateway. Cette URL doit correspondre à l’URL du relais compilée dans le build iOS.
...
- Les enregistrements adossés au relais sont délégués à une identité de Gateway spécifique. L’app iOS appairée récupère `gateway.identity.get`, inclut cette identité dans l’enregistrement du relais, puis transmet au Gateway une autorisation d’envoi limitée à l’enregistrement. Un autre Gateway ne peut pas réutiliser cet enregistrement stocké.
- `OPENCLAW_APNS_RELAY_BASE_URL` / `OPENCLAW_APNS_RELAY_TIMEOUT_MS` : substitutions temporaires par variables d’environnement pour la configuration du relais ci-dessus.
...
- `gateway.handshakeTimeoutMs` : délai d’expiration de la négociation WebSocket pré-authentification du Gateway en millisecondes. Valeur par défaut : `15000`. `OPENCLAW_HANDSHAKE_TIMEOUT_MS` est prioritaire lorsqu’il est défini. Augmentez-le sur les hôtes chargés ou peu puissants lorsque des clients locaux peuvent se connecter pendant que le préchauffage du démarrage est encore en cours.
...
- `channels.<provider>.accounts.<accountId>.healthMonitor.enabled` : remplacement par compte pour les canaux à comptes multiples. Lorsqu’il est défini, il est prioritaire sur le remplacement au niveau du canal.
- Les chemins d’appel du Gateway local ne peuvent utiliser `gateway.remote.*` comme solution de repli que lorsque `gateway.auth.*` n’est pas défini.
- Si `gateway.auth.token` / `gateway.auth.password` est explicitement configuré via SecretRef et non résolu, la résolution échoue de manière fermée (aucun masquage par repli distant).
...
- `gateway.nodes.allowCommands` / `gateway.nodes.denyCommands` : modelage global d’autorisation/refus pour les commandes de nœud déclarées après l’appairage et l’évaluation de la liste d’autorisation de la plateforme. Utilisez `allowCommands` pour autoriser explicitement des commandes de nœud dangereuses telles que `camera.snap`, `camera.clip` et `screen.record` ; `denyCommands` retire une commande même si une valeur par défaut de plateforme ou une autorisation explicite l’inclurait autrement. Après qu’un nœud a modifié sa liste de commandes déclarées, refusez puis réapprouvez cet appairage d’appareil afin que le Gateway stocke l’instantané de commandes mis à jour.
...
- En-tête facultatif de renforcement de réponse :
...
- `autoGenerate` : génère automatiquement une paire cert/clé auto-signée locale lorsque des fichiers explicites ne sont pas configurés ; réservé à une utilisation locale/dev.
...
- `caPath` : chemin facultatif d’un bundle CA pour la vérification client ou les chaînes de confiance personnalisées.
...
- `POST /hooks/agent` → `{ message, name?, agentId?, sessionKey?, wakeMode?, deliver?, channel?, to?, model?, thinking?, timeoutSeconds? }`
  - Le `sessionKey` provenant de la charge utile de la requête est accepté uniquement lorsque `hooks.allowRequestSessionKey=true` (par défaut : `false`).
- `POST /hooks/<name>` → résolu via `hooks.mappings`
  - Les valeurs `sessionKey` de mappage rendues à partir d’un modèle sont traitées comme fournies en externe et nécessitent également `hooks.allowRequestSessionKey=true`.
...
- `transform` peut pointer vers un module JS/TS renvoyant une action de hook.
  ...
  - Gardez `hooks.transformsDir` sous `~/.openclaw/hooks/transforms` ; les répertoires de Skills de l’espace de travail sont rejetés. Si `openclaw doctor` signale ce chemin comme non valide, déplacez le module de transformation dans le répertoire de transformations des hooks ou supprimez `hooks.transformsDir`.
...
- `allowedSessionKeyPrefixes` : liste d’autorisation facultative de préfixes pour les valeurs `sessionKey` explicites (requête + mappage), par exemple `["hook:"]`. Elle devient obligatoire lorsqu’un mappage ou préréglage utilise un `sessionKey` basé sur un modèle.
...
- Sert le HTML/CSS/JS modifiable par les agents et A2UI via HTTP sous le port du Gateway :
...
- Les WebViews Node n’envoient généralement pas d’en-têtes d’authentification ; une fois qu’un node est appairé et connecté, le Gateway annonce des URL de capacité limitées au node pour l’accès canvas/A2UI.
...
- `full` : inclut `cliPath` + `sshPort` ; la publicité multicast LAN nécessite toujours que le Plugin `bonjour` intégré soit activé.
- `off` : supprime la publicité multicast LAN sans modifier l’activation du Plugin.
- Le Plugin `bonjour` intégré démarre automatiquement sur les hôtes macOS et est activable explicitement sur Linux, Windows et les déploiements Gateway conteneurisés.
- Le nom d’hôte vaut par défaut le nom d’hôte système lorsqu’il s’agit d’une étiquette DNS valide, avec repli sur `openclaw`. Remplacez-le avec `OPENCLAW_MDNS_HOSTNAME`.
...
- Les variables d’environnement intégrées ne sont appliquées que si la variable d’environnement du processus ne contient pas la clé.
...
- Modèle de `provider` : `^[a-z][a-z0-9_-]{0,63}$`
- Modèle d’id pour `source: "env"` : `^[A-Z][A-Z0-9_]{0,127}$`
- Id pour `source: "file"` : pointeur JSON absolu (par exemple `"/providers/openai/apiKey"`)
- Modèle d’id pour `source: "exec"` : `^[A-Za-z0-9][A-Za-z0-9._:/-]{0,255}$`
- Les ids de `source: "exec"` ne doivent pas contenir de segments de chemin délimités par des barres obliques `.` ou `..` (par exemple `a/../b` est rejeté)
...
- Les chemins des fournisseurs file et exec échouent en mode fermé lorsque la vérification des ACL Windows n’est pas disponible. Définissez `allowInsecurePath: true` uniquement pour les chemins approuvés qui ne peuvent pas être vérifiés.
- Le fournisseur `exec` nécessite un chemin `command` absolu et utilise des charges utiles de protocole sur stdin/stdout.
- Par défaut, les chemins de commande de liens symboliques sont rejetés. Définissez `allowSymlinkCommand: true` pour autoriser les chemins de liens symboliques tout en validant le chemin cible résolu.
...
- Les anciennes cartes plates `auth-profiles.json`, comme `{ "provider": { "apiKey": "..." } }`, ne sont pas un format d’exécution ; `openclaw doctor --fix` les réécrit en profils de clé d’API canoniques `provider:default` avec une sauvegarde `.legacy-flat.*.bak`.
...
- `authPermanentBackoffMinutes` : backoff de base en minutes pour les échecs `auth_permanent` à haut niveau de confiance (par défaut : `10`).
...
- `overloadedProfileRotations` : nombre maximal de rotations de profils d’authentification du même fournisseur pour les erreurs de surcharge avant de basculer vers le fallback de modèle (par défaut : `1`). Les formes de fournisseur occupé comme `ModelNotReadyException` arrivent ici.
- `overloadedBackoffMs` : délai fixe avant de réessayer une rotation de fournisseur/profil surchargé (par défaut : `0`).
- `rateLimitedProfileRotations` : nombre maximal de rotations de profils d’authentification du même fournisseur pour les erreurs de limite de débit avant de basculer vers le fallback de modèle (par défaut : `1`). Ce compartiment de limite de débit inclut du texte au format fournisseur comme `Too many concurrent requests`, `ThrottlingException`, `concurrency limit reached`, `workers_ai ... quota limit exceeded` et `resource exhausted`.
...
- Fichier de journal par défaut : `/tmp/openclaw/openclaw-YYYY-MM-DD.log`.
- Définissez `logging.file` pour un chemin stable.
- `consoleLevel` passe à `debug` avec `--verbose`.
- `maxFileBytes` : taille maximale du fichier de journal actif en octets avant rotation (entier positif ; par défaut : `104857600` = 100 Mo). OpenClaw conserve jusqu’à cinq archives numérotées à côté du fichier actif.
...
- `stuckSessionWarnMs` : seuil d’âge sans progression en ms pour classer les sessions de traitement longues comme `session.long_running`, `session.stalled` ou `session.stuck`. Les réponses, outils, statuts, blocs et la progression ACP réinitialisent le minuteur ; les diagnostics `session.stuck` répétés appliquent un backoff tant qu’ils restent inchangés.
- `stuckSessionAbortMs` : seuil d’âge sans progression en ms avant que le travail actif bloqué éligible puisse être abort-drained pour récupération. Lorsqu’il n’est pas défini, OpenClaw utilise la fenêtre d’exécution intégrée étendue plus sûre d’au moins 10 minutes et 5x `stuckSessionWarnMs`.
...
- `otel.captureContent` : capture de contenu brut opt-in pour les attributs d’étendue OTEL. Désactivé par défaut. Le booléen `true` capture le contenu non système des messages/outils ; la forme objet permet d’activer explicitement `inputMessages`, `outputMessages`, `toolInputs`, `toolOutputs` et `systemPrompt`.
...
- `auto.stableJitterHours` : fenêtre supplémentaire en heures pour étaler le déploiement sur le canal stable (par défaut : `12` ; max : `168`).
...
- `enabled` : garde globale de fonctionnalité ACP (par défaut : `true` ; définissez `false` pour masquer les affordances de dispatch et de spawn ACP).
- `dispatch.enabled` : garde indépendante pour le dispatch de tours de session ACP (par défaut : `true`). Définissez `false` pour garder les commandes ACP disponibles tout en bloquant l’exécution.
...
- `defaultAgent` : identifiant d’agent cible ACP de fallback lorsque les spawns ne spécifient pas de cible explicite.
...
- `stream.maxChunkChars` : taille maximale des morceaux avant de scinder la projection de bloc diffusée.
- `stream.repeatSuppression` : supprime les lignes de statut/outils répétées par tour (par défaut : `true`).
...
Les builds actuelles n’incluent plus le pont TCP. Les Nodes se connectent via le WebSocket du Gateway. Les clés `bridge.*` ne font plus partie du schéma de configuration (la validation échoue jusqu’à leur suppression ; `openclaw doctor --fix` peut retirer les clés inconnues).

<Accordion title="Configuration du pont historique (référence historique)">
...
- `sessionRetention`: durée de conservation des sessions d’exécution Cron isolées terminées avant leur suppression de `sessions.json`. Contrôle aussi le nettoyage des transcriptions Cron supprimées archivées. Valeur par défaut : `24h` ; définir sur `false` pour désactiver.
- `runLog.maxBytes`: taille maximale par fichier de journal d’exécution (`cron/runs/<jobId>.jsonl`) avant suppression. Valeur par défaut : `2_000_000` octets.
- `runLog.keepLines`: lignes les plus récentes conservées lorsque la suppression du journal d’exécution est déclenchée. Valeur par défaut : `2000`.
...
- `webhook`: URL Webhook de repli héritée obsolète (http/https) utilisée uniquement pour les tâches enregistrées qui ont encore `notify: true`.
...
- `accountId`: identifiant facultatif de compte ou de canal pour limiter la portée de la livraison des alertes.
...
- Lorsque ni la destination d’échec globale ni celle par tâche n’est définie, les tâches qui livrent déjà via `announce` se replient sur cette cible d’annonce principale en cas d’échec.
...
| `{{GroupSubject}}` | Objet du groupe (au mieux)                        |
...
- Chemins : résolus relativement au fichier qui inclut, mais doivent rester dans le répertoire de configuration de niveau supérieur (`dirname` de `openclaw.json`). Les formes absolues/`../` sont autorisées uniquement lorsqu’elles se résolvent toujours à l’intérieur de cette limite.
- Les écritures détenues par OpenClaw qui ne modifient qu’une seule section de niveau supérieur sauvegardée par une inclusion de fichier unique écrivent directement dans ce fichier inclus. Par exemple, `plugins install` met à jour `plugins: { $include: "./plugins.json5" }` dans `plugins.json5` et laisse `openclaw.json` intact.
- Les inclusions racine, les tableaux d’inclusions et les inclusions avec remplacements par clés sœurs sont en lecture seule pour les écritures détenues par OpenClaw ; ces écritures échouent de façon fermée au lieu d’aplatir la configuration.
