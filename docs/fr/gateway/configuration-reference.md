---
read_when:
    - Vous avez besoin de la sémantique ou des valeurs par défaut exactes de la configuration au niveau des champs
...
x-i18n:
    generated_at: "2026-07-02T08:15:21Z"
    model: gpt-5.5
...
Couvre les principales surfaces de configuration d’OpenClaw et renvoie vers d’autres pages lorsqu’un sous-système dispose de sa propre référence plus détaillée. Les catalogues de commandes propres aux canaux et aux plugins, ainsi que les réglages avancés de mémoire/QMD, vivent sur leurs propres pages plutôt que sur celle-ci.

Vérité du code :

- `openclaw config schema` affiche le JSON Schema actif utilisé pour la validation et Control UI, avec les métadonnées intégrées/plugin/canal fusionnées lorsqu’elles sont disponibles
...
- pages des canaux/plugins propriétaires pour les surfaces de commandes propres à chaque canal
...
- `agents.defaults.*` (espace de travail, modèle, réflexion, heartbeat, mémoire, média, skills, sandbox)
...
- `talk.*` (mode Talk)
  - `talk.consultThinkingLevel` : remplacement du niveau de réflexion pour l’exécution complète de l’agent OpenClaw derrière les consultations realtime Talk de Control UI
  - `talk.consultFastMode` : remplacement ponctuel du mode rapide pour les consultations realtime Talk de Control UI
  ...
  - `talk.realtime.consultRouting` : repli de relais Gateway pour les transcriptions realtime Talk finalisées qui ignorent `openclaw_agent_consult`

## Outils et fournisseurs personnalisés

La politique d’outils, les bascules expérimentales, la configuration d’outils adossée aux fournisseurs et la configuration de
...
Les définitions de serveurs MCP gérées par OpenClaw vivent sous `mcp.servers` et sont
...
- `mcp.servers.<name>.enabled` : définissez `false` pour conserver une définition de serveur enregistrée
  tout en l’excluant de la découverte MCP intégrée d’OpenClaw et de la projection d’outils.
...
- `mcp.servers.<name>.connectTimeout` / `connectionTimeoutMs` : délai d’expiration de connexion par serveur
  en secondes ou en millisecondes.
- `mcp.servers.<name>.supportsParallelToolCalls` : indice de concurrence facultatif pour
...
- `mcp.servers.<name>.oauth` : remplacements facultatifs de portée OAuth, d’URL de redirection et
  d’URL de métadonnées client.
...
- `mcp.servers.<name>.codex` : contrôles facultatifs de projection du serveur d’application Codex.
  ...
  garder le serveur projeté pour chaque agent de serveur d’application Codex avec le
  comportement d’approbation MCP par défaut de Codex.
- `mcp.sessionIdleTtlMs` : TTL d’inactivité pour les runtimes MCP groupés à portée de session.
...
- Les changements sous `mcp.*` s’appliquent à chaud en éliminant les runtimes MCP de session mis en cache.
...
- La découverte runtime honore également les notifications de changement de liste d’outils MCP en supprimant
  ...
  les prompts. Les échecs répétés d’appels d’outils mettent brièvement en pause le serveur affecté avant
...
- `allowBundled` : liste d’autorisation facultative pour les skills groupés uniquement (skills gérés/d’espace de travail non affectés).
...
- `load.allowSymlinkTargets` : racines de cibles réelles approuvées vers lesquelles les liens symboliques de skills peuvent
  se résoudre lorsque le lien vit hors de sa racine source configurée.
...
- `install.allowUploadedArchives` : autorise les clients Gateway `operator.admin` approuvés
  ...
  (valeur par défaut : false). Cela active seulement le chemin des archives téléversées ; les installations ClawHub
...
- Placez les fichiers de plugin autonomes dans `plugins.load.paths` ; les racines d’extension détectées automatiquement ignorent les fichiers `.js`, `.mjs` et `.ts` de premier niveau afin que les scripts d’assistance dans ces racines ne bloquent pas le démarrage.
- La découverte accepte les plugins OpenClaw natifs ainsi que les bundles Codex compatibles et les bundles Claude, y compris les bundles Claude sans manifeste avec la disposition par défaut.
...
- `plugins.entries.<id>.hooks.allowPromptInjection` : lorsque `false`, le cœur bloque `before_prompt_build` et ignore les champs modifiant le prompt provenant de l’ancien `before_agent_start`, tout en préservant les anciens `modelOverride` et `providerOverride`. S’applique aux hooks de plugins natifs et aux répertoires de hooks fournis par des bundles pris en charge.
- `plugins.entries.<id>.hooks.allowConversationAccess` : lorsque `true`, les plugins non groupés de confiance peuvent lire le contenu brut de la conversation depuis des hooks typés tels que `llm_input`, `llm_output`, `before_model_resolve`, `before_agent_reply`, `before_agent_run`, `before_agent_finalize` et `agent_end`.
...
- `plugins.entries.<id>.llm.allowedModels` : liste d’autorisation facultative des cibles canoniques `provider/model` pour les remplacements de complétion LLM de plugin de confiance. Utilisez `"*"` uniquement lorsque vous voulez intentionnellement autoriser n’importe quel modèle.
- `plugins.entries.<id>.llm.allowAgentIdOverride` : fait explicitement confiance à ce plugin pour exécuter `api.runtime.llm.complete` avec un identifiant d’agent autre que celui par défaut.
...
- Les paramètres de compte/runtime des plugins de canal se trouvent sous `channels.<id>` et doivent être décrits par les métadonnées `channelConfigs` du manifeste du plugin propriétaire, et non par un registre central d’options OpenClaw.
...
et le [harnais Codex](/fr/plugins/codex-harness) pour le modèle runtime.
...
- `plugins.entries.codex.config.codexPlugins.allow_destructive_actions` :
  politique d’action destructive par défaut pour les sollicitations d’applications plugin migrées.
  Utilisez `true` pour accepter les schémas d’approbation Codex sûrs sans demander confirmation, `false`
  pour les refuser, `"auto"` pour router les approbations requises par Codex via les approbations
  de plugin OpenClaw, ou `"ask"` pour demander confirmation pour chaque écriture/action destructive
  de plugin sans approbation durable. Le mode `"ask"` efface les remplacements d’approbation Codex
  durables par outil pour l’application concernée et sélectionne le relecteur d’approbations humain
...
- `plugins.entries.codex.config.codexPlugins.plugins.<key>.marketplaceName` :
  identité stable de marketplace. La V1 ne prend en charge que `"openai-curated"`.
...
- `plugins.entries.codex.config.codexPlugins.plugins.<key>.allow_destructive_actions` :
  remplacement par plugin de la politique d’action destructive. Lorsqu’elle est omise, la valeur globale
  `allow_destructive_actions` est utilisée. La valeur par plugin accepte les mêmes politiques
  `true`, `false`, `"auto"` ou `"ask"`.

Chaque application plugin admise qui utilise `"ask"` route les demandes d’approbation de cette application
vers le relecteur humain. Les autres applications et les approbations de fil hors application conservent leur
relecteur configuré ; les politiques de plugins mixtes n’héritent donc pas du comportement `"ask"`.
...
de manière asynchrone lorsqu’ils sont périmés. La configuration d’application du fil Codex est calculée
à l’établissement de la session du harnais Codex, et non à chaque tour ; utilisez `/new`, `/reset` ou un
redémarrage du Gateway après avoir modifié la configuration native de plugin.
...
- `plugins.entries.firecrawl.config.webFetch` : paramètres du fournisseur de récupération web Firecrawl.
  ...
  - `timeoutSeconds` : délai d’expiration de la requête de scrape en secondes (par défaut : `60`).
...
- `plugins.entries.memory-core.config.dreaming` : paramètres de Dreaming de la mémoire. Consultez [Dreaming](/fr/concepts/dreaming) pour les phases et les seuils.
  ...
  - `model` : remplacement facultatif du modèle du sous-agent Dream Diary. Nécessite `plugins.entries.memory-core.subagent.allowModelOverride: true` ; associez-le à `allowedModels` pour restreindre les cibles. Les erreurs de modèle indisponible réessaient une fois avec le modèle par défaut de la session ; les échecs de confiance ou de liste d’autorisation ne se replient pas silencieusement.
...
- `plugins.slots.contextEngine` : choisissez l’identifiant du plugin de moteur de contexte actif ; vaut `"legacy"` par défaut sauf si vous installez et sélectionnez un autre moteur.
...
- `commitments.enabled` : active l’extraction LLM masquée, le stockage et la livraison Heartbeat des engagements de suivi inférés. Par défaut : `false`.
...
- Les profils `existing-session` conservent les limites de routage Chrome MCP actuelles :
  actions pilotées par instantané/référence plutôt que ciblage par sélecteur CSS, crochets de téléversement
  d’un seul fichier, aucune substitution de délai d’expiration de dialogue, pas de `wait --load networkidle`, et pas de
...
- Les profils `openclaw` gérés localement attribuent automatiquement `cdpPort` et `cdpUrl` ; définissez
  explicitement `cdpUrl` uniquement pour les profils CDP distants ou l’attachement à un point de terminaison
  existing-session.
...
- `extraArgs` ajoute des options de lancement supplémentaires au démarrage local de Chromium (par exemple
  `--disable-gpu`, le dimensionnement de fenêtre ou des options de débogage).
...
- `seamColor` : couleur d’accentuation pour l’habillage de l’UI de l’application native (teinte de bulle du mode Talk, etc.).
...
- **Note Docker** : la liaison `loopback` par défaut écoute sur `127.0.0.1` dans le conteneur. Avec le réseau bridge Docker (`-p 18789:18789`), le trafic arrive sur `eth0`, donc le gateway est inaccessible. Utilisez `--network host`, ou définissez `bind: "lan"` (ou `bind: "custom"` avec `customBindHost: "0.0.0.0"`) pour écouter sur toutes les interfaces.
- **Authentification** : requise par défaut. Les liaisons non loopback exigent une authentification du gateway. En pratique, cela signifie un jeton/mot de passe partagé ou un proxy inverse sensible à l’identité avec `gateway.auth.mode: "trusted-proxy"`. L’assistant d’onboarding génère un jeton par défaut.
...
- `gateway.auth.mode: "none"` : mode explicite sans authentification. À utiliser uniquement pour les configurations local loopback approuvées ; cette option n’est intentionnellement pas proposée par les invites d’onboarding.
- `gateway.auth.mode: "trusted-proxy"` : délègue l’authentification navigateur/utilisateur à un proxy inverse sensible à l’identité et approuve les en-têtes d’identité provenant de `gateway.trustedProxies` (voir [Authentification par proxy approuvé](/fr/gateway/trusted-proxy-auth)). Ce mode attend par défaut une source de proxy **non loopback** ; les proxys inverses loopback sur le même hôte nécessitent explicitement `gateway.auth.trustedProxy.allowLoopback = true`. Les appelants internes sur le même hôte peuvent utiliser `gateway.auth.password` comme solution de repli directe locale ; `gateway.auth.token` reste mutuellement exclusif avec le mode trusted-proxy.
- `gateway.auth.allowTailscale` : lorsque `true`, les en-têtes d’identité Tailscale Serve peuvent satisfaire l’authentification Control UI/WebSocket (vérifiée via `tailscale whois`). Les points de terminaison de l’API HTTP n’utilisent **pas** cette authentification par en-tête Tailscale ; ils suivent plutôt le mode d’authentification HTTP normal du gateway. Ce flux sans jeton suppose que l’hôte du gateway est approuvé. La valeur par défaut est `true` lorsque `tailscale.mode = "serve"`.
- `gateway.auth.rateLimit` : limiteur facultatif des échecs d’authentification. S’applique par IP cliente et par périmètre d’authentification (`shared-secret` et `device-token` sont suivis indépendamment). Les tentatives bloquées renvoient `429` + `Retry-After`.
...
- Les tentatives d’authentification WS provenant d’une origine navigateur sont toujours limitées avec l’exemption loopback désactivée (défense en profondeur contre la force brute localhost depuis un navigateur).
- Sur loopback, ces verrouillages provenant d’une origine navigateur sont isolés par valeur `Origin`
...
- `tailscale.serviceName` : nom facultatif du service Tailscale pour le mode Serve, par exemple
...
--service` afin que la Control UI puisse être exposée via un Service nommé plutôt
  que le nom d’hôte de l’appareil. La valeur doit utiliser le format de nom de Service `svc:<dns-label>`
  de Tailscale ; le démarrage signale l’URL de Service dérivée.
...
- `remote.transport` : `ssh` (par défaut) ou `direct` (ws/wss). Pour `direct`, `remote.url` doit être en `wss://` pour les hôtes publics ; le texte en clair `ws://` n’est accepté que pour loopback, LAN, link-local, `.local`, `.ts.net` et les hôtes CGNAT Tailscale.
- `remote.remotePort` : port du gateway sur l’hôte SSH distant. Valeur par défaut : `18789` ; utilisez ceci lorsque le port du tunnel local diffère du port du gateway distant.
- `gateway.remote.token` / `.password` sont des champs d’identifiants pour client distant. Ils ne configurent pas à eux seuls l’authentification du gateway.
- `gateway.push.apns.relay.baseUrl` : URL HTTPS de base du relais APNs externe utilisé après que les builds iOS adossés au relais publient les inscriptions auprès du gateway. Les builds publics App Store utilisent le relais OpenClaw hébergé. Les URL de relais personnalisées doivent correspondre à un chemin de build/déploiement iOS délibérément séparé dont l’URL de relais pointe vers ce relais.
...
- `gateway.handshakeTimeoutMs` : délai d’expiration de la négociation WebSocket Gateway avant authentification, en millisecondes. Par défaut : `15000`. `OPENCLAW_HANDSHAKE_TIMEOUT_MS` est prioritaire lorsqu’il est défini. Augmentez cette valeur sur les hôtes chargés ou peu puissants où les clients locaux peuvent se connecter pendant que l’échauffement du démarrage est encore en cours.
...
- `gateway.channelMaxRestartsPerHour` : nombre maximal de redémarrages par le moniteur d’état, par canal/compte, sur une heure glissante. Par défaut : `10`.
...
- `gateway.tools.allow` : retire des noms d’outils de la liste de refus HTTP par défaut pour
  les appelants propriétaire/admin. Cela ne convertit pas les appelants `operator.write`
  porteurs d’identité en accès propriétaire/admin ; `cron`, `gateway` et `nodes` restent
  indisponibles pour les appelants non propriétaires même lorsqu’ils sont dans la liste d’autorisation.
...
- `autoGenerate` : génère automatiquement une paire cert/clé locale auto-signée lorsque les fichiers explicites ne sont pas configurés ; uniquement pour une utilisation locale/dev.
...
- `deferralTimeoutMs` : durée maximale facultative en ms à attendre pour les opérations en cours avant de forcer un redémarrage ou un rechargement à chaud du canal. Omettez-la pour utiliser l’attente bornée par défaut (`300000`) ; définissez `0` pour attendre indéfiniment et journaliser des avertissements périodiques indiquant que des opérations sont toujours en attente.
...
- `openclaw security audit` signale la réutilisation de l’authentification hook/Gateway comme un constat critique, y compris l’authentification par mot de passe Gateway fournie uniquement au moment de l’audit (`--auth password --password <password>`). Exécutez `openclaw doctor --fix` pour faire tourner un `hooks.token` persistant réutilisé, puis mettez à jour les émetteurs de hooks externes pour utiliser le nouveau jeton de hook.
...
- `transform` peut pointer vers un module JS/TS renvoyant une action de hook.
  ...
  - Gardez `hooks.transformsDir` sous `~/.openclaw/hooks/transforms` ; les répertoires de Skills de l’espace de travail sont rejetés. Si `openclaw doctor` signale ce chemin comme non valide, déplacez le module de transformation dans le répertoire des transformations de hooks ou supprimez `hooks.transformsDir`.
- `agentId` achemine vers un agent spécifique ; les ID inconnus reviennent à l’agent par défaut.
- `allowedAgentIds` : restreint l’acheminement effectif des agents, y compris le chemin de l’agent par défaut lorsque `agentId` est omis (`*` ou omis = tout autoriser, `[]` = tout refuser).
...
- Si vous conservez cet acheminement par message, définissez `hooks.allowRequestSessionKey: true` et contraignez `hooks.allowedSessionKeyPrefixes` pour correspondre à l’espace de noms Gmail, par exemple `["hook:", "hook:gmail:"]`.
...
- N’exécutez pas de `gog gmail watch serve` séparé en parallèle du Gateway.
...
- Les WebViews Node n’envoient généralement pas d’en-têtes d’authentification ; après l’appairage et la connexion d’un nœud, le Gateway annonce des URL de capacité propres au nœud pour l’accès Canvas/A2UI.
...
- `full` : inclure `cliPath` + `sshPort` ; la publicité multicast LAN nécessite toujours que le Plugin `bonjour` groupé soit activé.
- `off` : supprimer la publicité multicast LAN sans modifier l’activation du Plugin.
- Le Plugin `bonjour` groupé démarre automatiquement sur les hôtes macOS et est en opt-in sur Linux, Windows et les déploiements Gateway conteneurisés.
- Le nom d’hôte prend par défaut le nom d’hôte système lorsqu’il s’agit d’une étiquette DNS valide, avec repli sur `openclaw`. Remplacez-le avec `OPENCLAW_MDNS_HOSTNAME`.
...
- Les chemins de fournisseurs file et exec échouent en mode fermé lorsque la vérification des ACL Windows est indisponible. Définissez `allowInsecurePath: true` uniquement pour les chemins approuvés qui ne peuvent pas être vérifiés.
- Le fournisseur `exec` exige un chemin `command` absolu et utilise des charges utiles de protocole sur stdin/stdout.
- Par défaut, les chemins de commande par lien symbolique sont rejetés. Définissez `allowSymlinkCommand: true` pour autoriser les chemins de liens symboliques tout en validant le chemin cible résolu.
- Si `trustedDirs` est configuré, le contrôle de répertoire approuvé s’applique au chemin cible résolu.
...
- Les anciennes maps plates `auth-profiles.json`, comme `{ "provider": { "apiKey": "..." } }`, ne sont pas un format d’exécution ; `openclaw doctor --fix` les réécrit en profils de clé API canoniques `provider:default` avec une sauvegarde `.legacy-flat.*.bak`.
...
- `billingBackoffHours` : délai d’attente de base en heures lorsqu’un profil échoue en raison de véritables erreurs de
  ...
  propres à un fournisseur restent limitées au fournisseur qui les possède (par exemple OpenRouter
  `Key limit exceeded`). Les messages HTTP `402` réessayables de fenêtre d’utilisation ou
  de limite de dépenses d’organisation/espace de travail restent plutôt dans le chemin `rate_limit`.
...
- `overloadedProfileRotations` : nombre maximal de rotations de profils d’authentification du même fournisseur pour les erreurs de surcharge avant de basculer vers le repli de modèle (par défaut : `1`). Les formes de fournisseur occupé, comme `ModelNotReadyException`, arrivent ici.
...
- `stuckSessionAbortMs` : seuil d’âge sans progression en ms avant que le travail actif bloqué éligible puisse être vidé par abandon pour récupération. Lorsqu’il n’est pas défini, OpenClaw utilise la fenêtre intégrée étendue plus sûre d’au moins 5 minutes et 3x `stuckSessionWarnMs`.
- `memoryPressureSnapshot` : capture un instantané de stabilité expurgé avant OOM lorsque la pression mémoire atteint `critical` (par défaut : `false`). Définissez sur `true` pour ajouter l’analyse/écriture du fichier de bundle de stabilité tout en conservant les événements normaux de pression mémoire.
...
- `enabled` : garde globale de fonctionnalité ACP (par défaut : `true` ; définissez `false` pour masquer la dispatch ACP et les affordances de lancement).
- `dispatch.enabled` : garde indépendante pour la dispatch des tours de session ACP (par défaut : `true`). Définissez `false` pour garder les commandes ACP disponibles tout en bloquant l’exécution.
...
Les builds actuels n’incluent plus le pont TCP. Les nœuds se connectent via le WebSocket Gateway. Les clés `bridge.*` ne font plus partie du schéma de configuration (la validation échoue jusqu’à leur suppression ; `openclaw doctor --fix` peut retirer les clés inconnues).
...
- `sessionRetention` : durée de conservation des sessions d’exécution Cron isolées terminées avant l’élagage depuis `sessions.json`. Contrôle aussi le nettoyage des transcriptions Cron supprimées archivées. Par défaut : `24h` ; définissez `false` pour désactiver.
...
- `webhook` : URL Webhook de secours héritée obsolète (http/https) utilisée par `openclaw doctor --fix` pour migrer les tâches stockées qui ont encore `notify: true` ; la livraison à l’exécution utilise `delivery.mode="webhook"` par tâche avec `delivery.to`, ou `delivery.completionDestination` lors de la conservation d’une livraison d’annonce.
...
- `retryOn` : types d’erreurs déclenchant de nouvelles tentatives - `"rate_limit"`, `"overloaded"`, `"network"`, `"timeout"`, `"server_error"`. Omettez pour réessayer tous les types transitoires.

Les tâches ponctuelles restent activées jusqu’à l’épuisement des tentatives, puis se désactivent tout en conservant l’état d’erreur final. Les tâches récurrentes utilisent la même stratégie de nouvelle tentative transitoire pour s’exécuter à nouveau après le délai d’attente avant leur prochain créneau planifié ; les erreurs permanentes ou les nouvelles tentatives transitoires épuisées reviennent au planning récurrent normal avec délai d’attente sur erreur.
...
- `cooldownMs` : délai minimal en millisecondes entre des alertes répétées pour la même tâche (entier non négatif).
...
- `accountId` : compte ou identifiant de canal facultatif pour limiter la portée de la livraison d’alertes.
...
- `mode` : `"announce"` ou `"webhook"` ; vaut par défaut `"announce"` lorsque les données de cible suffisantes existent.
...
- Lorsqu’aucune destination d’échec globale ni par tâche n’est définie, les tâches qui livrent déjà via `announce` se replient sur cette cible d’annonce principale en cas d’échec.
...
- Chemins : résolus relativement au fichier qui inclut, mais doivent rester dans le répertoire de configuration de premier niveau (`dirname` de `openclaw.json`). Les formes absolues/`../` ne sont autorisées que lorsqu’elles se résolvent toujours à l’intérieur de cette limite. Les chemins ne doivent pas contenir d’octets nuls et doivent être strictement plus courts que 4096 caractères avant et après résolution.
- Les écritures appartenant à OpenClaw qui ne modifient qu’une seule section de premier niveau adossée à une inclusion de fichier unique écrivent dans ce fichier inclus. Par exemple, `plugins install` met à jour `plugins: { $include: "./plugins.json5" }` dans `plugins.json5` et laisse `openclaw.json` intact.
- Les inclusions racine, les tableaux d’inclusion et les inclusions avec remplacements par clés voisines sont en lecture seule pour les écritures appartenant à OpenClaw ; ces écritures échouent de manière fermée au lieu d’aplatir la configuration.
- Erreurs : messages clairs pour les fichiers manquants, les erreurs d’analyse, les inclusions circulaires, les formats de chemin invalides et les longueurs excessives.
