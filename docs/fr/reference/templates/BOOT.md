...
Ajoutez ici des instructions de démarrage courtes et explicites. Le hook `boot-md` fourni exécute ce fichier une fois par espace de travail d’agent à chaque démarrage du Gateway, si le fichier existe et contient des caractères autres que des espaces. Plusieurs agents partageant un même espace de travail ne déclenchent qu’une seule exécution.

Le hook est désactivé par défaut. Activez-le d’abord :

```bash
openclaw hooks enable boot-md
```

Si un élément de la liste de contrôle envoie un message, utilisez l’outil de messagerie, puis répondez avec le jeton silencieux exact `NO_REPLY` (insensible à la casse).
