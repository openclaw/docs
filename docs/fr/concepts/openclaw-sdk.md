...
summary: SDK public de l’application OpenClaw pour les applications externes, les scripts, les tableaux de bord, les tâches CI et les extensions d’IDE
title: SDK d’application OpenClaw
x-i18n:
    generated_at: "2026-05-02T07:04:46Z"
    model: gpt-5.5
    provider: openai
...
Le **OpenClaw App SDK** est l’API cliente publique pour les applications extérieures au
...
<Note>
  L’App SDK est différent du [Plugin SDK](/fr/plugins/sdk-overview).
...
| `oc.tools`                | Prêt   | Liste, délimite et invoque les outils Gateway via le pipeline de politique. |
| `oc.artifacts`            | Prêt   | Liste, obtient et télécharge les artefacts de transcript Gateway.          |
| `oc.approvals`            | Prêt   | Liste et résout les approbations exec via les RPC d’approbation Gateway.   |
...
Créez un client avec une URL Gateway explicite, ou injectez un transport personnalisé pour les
tests et les environnements d’exécution d’applications embarquées.
...
Les références de modèle qualifiées par fournisseur comme `openai/gpt-5.5` sont séparées en remplacements Gateway
...
pendant que l’exécution est encore active renvoie `status: "accepted"` au lieu de prétendre
que l’exécution elle-même a expiré. Les délais d’exécution, les exécutions interrompues et les exécutions annulées sont
...
`Session.send()` appelle `sessions.send` et renvoie un `Run`. Les handles de session prennent également
en charge :
...
| `tool.call.failed`    | Échec ou statut bloqué d’outil/élément/commande |
...
`Run.events()` filtre les événements vers un seul identifiant d’exécution et rejoue les événements déjà vus pour les
exécutions rapides. Cela signifie que le flux documenté est sûr :
...
Les assistants d’outils exposent le catalogue Gateway, la vue effective des outils et l’invocation directe
...
Les assistants d’artefacts exposent la projection d’artefacts Gateway pour le contexte de session, d’exécution ou
...
Les assistants d’approbation utilisent les RPC d’approbation exec :
...
Le SDK inclut des noms pour le modèle produit que nous voulons, mais il ne fait pas silencieusement
semblant que des RPC Gateway existent. Ces appels lèvent actuellement des erreurs explicites
...
## App SDK Contre Plugin SDK

Utilisez l’App SDK lorsque le code vit en dehors d’OpenClaw :
...
Le code App SDK doit importer depuis `@openclaw/sdk`. Le code Plugin doit importer depuis les
sous-chemins documentés `openclaw/plugin-sdk/*`. Ne mélangez pas les deux contrats.

## Docs Connexes

- [Conception de l’API OpenClaw App SDK](/fr/reference/openclaw-sdk-api-design)
