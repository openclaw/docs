...
summary: Side-car de recherche local-first avec BM25, vecteurs, reranking et expansion de requêtes
...
[QMD](https://github.com/tobi/qmd) est un processus auxiliaire de recherche conçu d’abord pour le local, qui s’exécute
...
- **Reclassement et expansion de requête** pour un meilleur rappel.
- **Indexation de répertoires supplémentaires** -- docs de projet, notes d’équipe, tout élément sur disque.
- **Indexation des transcriptions de session** -- retrouvez les conversations antérieures.
...
`~/.openclaw/agents/<agentId>/qmd/` et gère automatiquement le cycle de vie du processus auxiliaire
-- collections, mises à jour et exécutions d’embeddings sont gérées pour vous.
Il privilégie les formes actuelles de collection QMD et de requête MCP, mais revient encore à
des flags de modèles de collection alternatifs et à d’anciens noms d’outils MCP si nécessaire.
La réconciliation au démarrage recrée aussi les collections gérées obsolètes selon leurs
modèles canoniques lorsqu’une collection QMD plus ancienne portant le même nom est encore
présente.

## Fonctionnement du processus auxiliaire

- OpenClaw crée des collections à partir de vos fichiers de mémoire d’espace de travail et de tous les
  `memory.qmd.paths` configurés, puis exécute `qmd update` lorsque le gestionnaire QMD est
  ouvert et périodiquement ensuite (par défaut toutes les 5 minutes). Ces actualisations
  passent par des sous-processus QMD, pas par une exploration du système de fichiers dans le processus.
  Les modes sémantiques exécutent aussi `qmd embed`.
- La collection d’espace de travail par défaut suit `MEMORY.md` ainsi que l’arborescence
  `memory/`. `memory.md` en minuscules n’est pas indexé comme fichier de mémoire racine.
- Le scanner propre à QMD ignore les chemins cachés et les répertoires courants de dépendances/build
  ...
  `build`. Le démarrage du Gateway n’initialise pas QMD par défaut ; un démarrage à froid
  évite donc d’importer le runtime de mémoire ou de créer le watcher à longue durée de vie avant
  la première utilisation de la mémoire.
- Si vous voulez quand même initialiser QMD au démarrage du Gateway, définissez
  ...
  gestionnaire à longue durée de vie lorsque des intervalles de mise à jour ou d’embedding sont configurés, afin que QMD puisse
  gérer son watcher et ses minuteurs réguliers.
- Les recherches utilisent le `searchMode` configuré (par défaut : `search`; prend aussi en charge
  `vsearch` et `query`). `search` utilise uniquement BM25 ; OpenClaw ignore donc les sondes de
...
- Lorsque `searchMode` est `query`, définissez `memory.qmd.rerank` sur `false` pour utiliser le
  chemin de requête hybride de QMD sans le reclasseur. OpenClaw passe `--no-rerank` au
...
- Avec les versions de QMD qui annoncent des filtres multi-collections, OpenClaw regroupe
  les collections de même source dans une seule invocation de recherche QMD. Les anciennes versions de QMD
  conservent le repli compatible par collection.
- Si QMD échoue entièrement, OpenClaw revient au moteur SQLite intégré.
  Les tentatives répétées pendant les tours de chat appliquent un bref délai après un échec d’ouverture afin qu’un
  binaire manquant ou une dépendance cassée du processus auxiliaire ne crée pas une tempête de nouvelles tentatives ;
  `openclaw memory status` et les sondes CLI ponctuelles revérifient quand même QMD directement.

<Info>
La première recherche peut être lente -- QMD télécharge automatiquement des modèles GGUF (~2 Go) pour
le reclassement et l’expansion de requête lors de la première exécution de `qmd query`.
</Info>
...
Les collections de transcriptions de session restent dans leur propre groupe de sources, de sorte que les recherches mixtes
`memory` + `sessions` fournissent quand même au diversificateur de résultats des entrées issues des deux
sources.

Les anciens builds QMD n’acceptent qu’un seul filtre de collection. Lorsqu’OpenClaw détecte l’un
de ces builds, il conserve le chemin de compatibilité et recherche chaque collection
...
Les variables d’environnement de modèle QMD sont transmises inchangées depuis le processus
...
Les extraits issus des chemins supplémentaires apparaissent sous la forme `qmd/<collection>/<relative-path>` dans les
résultats de recherche. `memory_get` comprend ce préfixe et lit depuis la racine de
collection correcte.

## Indexation des transcriptions de session

Activez l’indexation des sessions pour retrouver les conversations antérieures :
...
Les transcriptions sont exportées sous forme de tours User/Assistant nettoyés dans une collection QMD
...
Lorsqu’une portée refuse une recherche, OpenClaw journalise un avertissement avec le canal dérivé et
...
- Rechercher dans des docs de projet ou des notes en dehors de l’espace de travail.
...
**QMD introuvable ?** Assurez-vous que le binaire est dans le `PATH` du Gateway. Si OpenClaw
...
**Première recherche très lente ?** QMD télécharge les modèles GGUF à la première utilisation. Pré-chauffez
...
repli par collection pour garantir la correction.

**QMD en BM25 uniquement essaie quand même de construire llama.cpp ?** Définissez
`memory.qmd.searchMode = "search"`. OpenClaw traite ce mode comme lexical uniquement,
...
**Résultats vides dans les chats de groupe ?** Vérifiez `memory.qmd.scope` -- la valeur par défaut autorise uniquement
les sessions directes et de canal.

**La recherche de mémoire racine est soudain devenue trop large ?** Redémarrez le Gateway ou attendez
la prochaine réconciliation au démarrage. OpenClaw recrée les collections gérées obsolètes
selon les modèles canoniques `MEMORY.md` et `memory/` lorsqu’il détecte un conflit de même nom.
...
les règles de liens symboliques intégrées d’OpenClaw. Gardez les checkouts temporaires de monorepo sous
des répertoires cachés comme `.tmp/` ou en dehors des racines QMD indexées jusqu’à ce que QMD expose
une traversée protégée contre les cycles ou des contrôles d’exclusion explicites.
