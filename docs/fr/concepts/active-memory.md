...
title: Active Memory
x-i18n:
    generated_at: "2026-05-02T07:03:31Z"
    model: gpt-5.5
    provider: openai
...
Il existe parce que la plupart des systèmes de mémoire sont capables mais réactifs. Ils comptent sur
...
Active Memory donne au système une possibilité bornée de faire émerger la mémoire pertinente
...
- `config.agents: ["main"]` n’active Active Memory que pour l’agent `main`
- `config.allowedChatTypes: ["direct"]` le limite aux sessions de messages directs (activez explicitement les groupes/canaux)
- `config.model` (optionnel) épingle un modèle de rappel dédié ; non défini, il hérite du modèle de session actuel
...
- Active Memory ne s’exécute toujours que pour les sessions de chat persistantes interactives éligibles

## Recommandations de rapidité

La configuration la plus simple consiste à laisser `config.model` non défini et à permettre à Active Memory d’utiliser
le même modèle que vous utilisez déjà pour les réponses normales. C’est la valeur par défaut la plus sûre,
...
modèle choisi — la visibilité via `/v1/models` seule ne le garantit pas.

## Comment le voir

Active Memory injecte un préfixe d’invite non fiable masqué pour le modèle. Il
...
Ces lignes dérivent du même passage Active Memory qui alimente le préfixe d’invite
masqué, mais elles sont formatées pour les humains au lieu d’exposer le balisage brut de l’invite.
Elles sont envoyées comme message de diagnostic de suivi après la réponse normale de
l’assistant, afin que les clients de canaux comme Telegram n’affichent pas une bulle de diagnostic
distincte avant la réponse.
...
Active Memory utilise deux portes :
...
2. **Éligibilité stricte à l’exécution**
   Même lorsqu’il est activé et ciblé, Active Memory ne s’exécute que pour les sessions de chat
   persistantes interactives éligibles.
...
cette liste. Cela restreint tous les types de chats autorisés à la fois, y compris les messages directs.
Si vous voulez tous les messages directs plus uniquement certains groupes, incluez
...
Les identifiants proviennent de la clé de session persistante du canal : par exemple le
`chat_id` / `open_id` de Feishu, l’identifiant de chat Telegram ou l’identifiant de canal Slack. La correspondance
est insensible à la casse. Si `allowedChatIds` n’est pas vide et qu’OpenClaw ne peut pas résoudre un
...
| Autres sessions de canaux interactives sur le même chemin de chat persistant | Oui, si le Plugin est activé et que l’agent est ciblé |
...
- les emplacements où une personnalisation masquée serait surprenante
...
voit. Choisissez le mode le plus petit qui répond encore bien aux questions de suivi ;
...
<Tabs>
  ...
  <Tab title="recent">
    Le dernier message utilisateur plus une petite fin récente de conversation est envoyé.
  ...
  <Tab title="full">
    ...
    - la conversation contient une configuration importante loin en amont dans le fil
...
lorsqu’il décide s’il doit retourner de la mémoire.

Styles disponibles :

- `balanced` : valeur par défaut polyvalente pour le mode `recent`
- `strict` : le moins réactif ; idéal lorsque vous voulez très peu d’influence du contexte voisin
...
Si vous définissez `config.promptStyle` explicitement, cette valeur prend le dessus.
...
`config.modelFallback` contrôle l’étape de fallback configurée.

Fallback personnalisé facultatif :

```json5
modelFallback: "google/gemini-3-flash"
```

Si aucun modèle explicite, hérité ou configuré en fallback n’est résolu, Active Memory
...
contrat de rappel différent. Le prompt par défaut est ajusté pour retourner soit `NONE`,
...
- ces transcriptions contiennent du contexte de prompt masqué et des mémoires rappelées
...
| `config.model`               | `string`                                                                                             | Référence facultative du modèle du sous-agent de mémoire bloquant ; si non définie, Active Memory utilise le modèle de la session actuelle |
| `config.allowedChatTypes`    | `("direct" \| "group" \| "channel")[]`                                                               | Types de session pouvant exécuter Active Memory ; par défaut, sessions de type message direct                  |
...
| `config.deniedChatIds`       | `string[]`                                                                                           | Liste de refus facultative par conversation qui remplace les types de session autorisés et les ids autorisés   |
| `config.queryMode`           | `"message" \| "recent" \| "full"`                                                                    | Contrôle la quantité de conversation vue par le sous-agent de mémoire bloquant                                 |
| `config.promptStyle`         | `"balanced" \| "strict" \| "contextual" \| "recall-heavy" \| "precision-heavy" \| "preference-only"` | Contrôle à quel point le sous-agent de mémoire bloquant est réactif ou strict lorsqu’il décide de retourner de la mémoire |
...
| `config.setupGraceTimeoutMs` | `number`                                                                                             | Budget avancé de configuration supplémentaire avant l’expiration du délai de rappel ; valeur par défaut 0 et plafonné à 30000 ms |
...
| `config.circuitBreakerMaxTimeouts` | `number` | Ignorer le rappel après ce nombre d’expirations consécutives pour le même agent/modèle. Réinitialisé après un rappel réussi ou après l’expiration du délai de récupération (plage : 1-20 ; valeur par défaut : 3). |
| `config.circuitBreakerCooldownMs`  | `number` | Durée pendant laquelle ignorer le rappel après le déclenchement du circuit breaker, en ms (plage : 5000-600000 ; valeur par défaut : 60000).                             |
...
ligne d’état normale et `/trace on` pour le résumé de débogage active-memory, plutôt que
...
- `full` si vous estimez que le contexte supplémentaire vaut le ralentissement du sous-agent de mémoire bloquant
...
3. Vérifiez que vous testez via une session de discussion interactive persistante.
4. Activez `config.logging: true` et observez les journaux du gateway.
5. Vérifiez que la recherche mémoire elle-même fonctionne avec `openclaw memory status --deep`.

Si les résultats mémoire sont bruyants, resserrez :
...
- abaisser `queryMode`
- abaisser `timeoutMs`
- réduire le nombre de tours récents
- réduire les limites de caractères par tour

## Problèmes courants

Active Memory s’appuie sur le pipeline de rappel du Plugin de mémoire configuré, donc la plupart des surprises de rappel sont des problèmes de fournisseur d’embeddings, et non des bugs d’Active Memory. Le chemin `memory-core` par défaut utilise `memory_search`; `memory-lancedb` utilise `memory_recall`.
...
<AccordionGroup>
  <Accordion title="Embedding provider switched or stopped working">
    ...
    fournisseur hébergé soumis à une limite de débit peut modifier le fournisseur résolu d’une
    exécution à l’autre. Si aucun fournisseur n’est résolu, `memory_search` peut se dégrader vers une
  ...
  <Accordion title="Recall feels slow, empty, or inconsistent">
    ...
    - Surveillez les journaux du Gateway pour `active-memory: ... start|done`,
      `memory sync failed (search-bootstrap)` ou des erreurs d’embedding du fournisseur.
