...
Google Meet prend en charge les participants pour OpenClaw — le Plugin est explicite par conception :
...
- La voix realtime peut rappeler l’agent OpenClaw complet lorsque des
  raisonnements plus poussés ou des outils sont nécessaires.
- Les agents choisissent le comportement de connexion avec `mode` : utilisez `realtime` pour l’écoute et la
  réponse vocale en direct, ou `transcribe` pour rejoindre/contrôler le navigateur sans le
  pont vocal realtime.
...
realtime backend. OpenAI est utilisé par défaut ; Google Gemini Live fonctionne aussi avec
...
l’épinglage de nœud et, pour les connexions Chrome realtime, le pont audio
BlackHole/SoX et les vérifications d’introduction realtime différée. Pour les connexions en observation seule, vérifiez le même
transport avec `--mode transcribe` ; ce mode ignore les prérequis audio realtime
...
Plugin `voice-call`, les identifiants Twilio et l’exposition Webhook publique sont prêts.
Traitez toute vérification `ok: false` comme un blocage pour le transport et le mode
vérifiés avant de demander à un agent de rejoindre. Utilisez `openclaw googlemeet setup --json` pour les
scripts ou les sorties lisibles par machine. Utilisez `--transport chrome`,
`--transport chrome-node` ou `--transport twilio` pour prévérifier un
transport spécifique avant qu’un agent ne l’essaie.

Pour Twilio, prévérifiez toujours le transport explicitement lorsque le transport par défaut
est Chrome :
...
Cela détecte le câblage `voice-call` manquant, les identifiants Twilio absents ou une
exposition Webhook inaccessible avant que l’agent n’essaie d’appeler la réunion.
...
- Repli navigateur : utilisé lorsque les identifiants OAuth sont absents. OpenClaw utilise le
  ...
  redirige vers une véritable URL avec code de réunion, puis renvoie cette URL. Ce chemin nécessite
  que le profil Chrome OpenClaw sur le nœud soit déjà connecté à Google.
  L’automatisation du navigateur gère la propre invite de microphone de première utilisation de Meet ; cette invite
  ...
  nouvelle tentative d’agent devrait cibler la réunion déjà ouverte au lieu de créer un second
  onglet Chrome.

La sortie de la commande/de l’outil inclut un champ `source` (`api` ou `browser`) afin que les agents
puissent expliquer le chemin utilisé. `create` rejoint la nouvelle réunion par défaut et
renvoie `joined: true` avec la session de connexion. Pour créer seulement l’URL, utilisez
`create --no-join` dans la CLI ou passez `"join": false` à l’outil.

Ou dites à un agent : « Crée un Google Meet, rejoins-le avec la voix realtime, et envoie-moi le lien. » L’agent doit appeler `google_meet` avec `action: "create"` puis
...
démarre pas le pont de modèle realtime duplex, ne nécessite pas BlackHole ni SoX,
et ne répondra pas vocalement dans la réunion. Les connexions Chrome dans ce mode évitent aussi
l’octroi d’autorisation microphone/caméra d’OpenClaw et évitent le chemin **Use
microphone** de Meet. Si Meet affiche un interstitiel de choix audio, l’automatisation essaie
...
ainsi qu’une courte fin `recentTranscript` afin que les opérateurs puissent savoir si le navigateur
a rejoint l’appel et si les sous-titres Meet produisent du texte.

Pendant les sessions realtime, le statut `google_meet` inclut l’état du navigateur et du pont audio,
...
apparaît, l’automatisation du navigateur la gère quand elle le peut. La connexion, l’admission par l’hôte et
les invites d’autorisation du navigateur/système d’exploitation sont signalées comme action manuelle avec une raison et
un message que l’agent peut relayer. Les sessions Chrome gérées n’émettent la phrase d’introduction ou
de test qu’après que l’état du navigateur indique `inCall: true` ; sinon le statut indique
`speechReady: false` et la tentative de parole est bloquée au lieu de faire semblant que
l’agent a parlé dans la réunion.

Les connexions Chrome locales passent par le profil de navigateur OpenClaw connecté. Le mode realtime
...
- Hôte Gateway : Gateway OpenClaw, espace de travail de l’agent, clés de modèle/API, fournisseur
  realtime et configuration du Plugin Google Meet.
...
paramètre `openclaw.json`. `openclaw node install` le stocke dans l’environnement LaunchAgent
...
et la capacité de navigateur/`browser.proxy` :

```bash
openclaw nodes status
```

Routez Meet via ce nœud sur l’hôte Gateway :
...
Rejoignez ensuite normalement depuis l’hôte Gateway :
...
et affiche l’état de la session :

```bash
openclaw googlemeet test-speech https://meet.google.com/abc-defg-hij
```

Pendant la connexion realtime, l’automatisation du navigateur OpenClaw renseigne le nom d’invité, clique sur
Join/Ask to join et accepte le choix « Use microphone » de première utilisation de Meet lorsque cette
invite apparaît. Pendant une connexion en observation seule ou une création de réunion avec navigateur uniquement, elle
...
si Chrome a besoin d’une autorisation microphone/caméra pour une connexion realtime, ou si Meet est bloqué
...
ainsi que les `browserUrl`/`browserTitle` actuels, puis réessayer uniquement après que
...
- Chrome s’ouvre mais ne peut pas rejoindre : connectez-vous au profil de navigateur dans la VM, ou
  conservez `chrome.guestName` défini pour rejoindre en tant qu’invité. La connexion automatique en invité utilise l’automatisation de navigateur OpenClaw
...
- Pas d’audio : dans Meet, acheminez le microphone/haut-parleur par le chemin de périphérique audio virtuel
  utilisé par OpenClaw ; utilisez des périphériques virtuels séparés ou un routage de style Loopback
  pour un audio duplex propre.

## Notes d’installation

Le mode temps réel Chrome par défaut utilise deux outils externes :
...
OpenClaw n’intègre ni ne redistribue aucun de ces deux paquets. La documentation demande aux utilisateurs de
...
programme d’installation ou une appliance qui regroupe BlackHole avec OpenClaw, vérifiez les
...
sur un nœud appairé, comme une VM macOS Parallels. Pour Chrome local, choisissez le
...
par appel téléphonique. Google Meet doit exposer un numéro de connexion téléphonique et un code PIN pour la
...
Fournissez les identifiants Twilio via l’environnement ou la configuration. L’environnement évite
de placer des secrets dans `openclaw.json` :
...
Lorsque la délégation Twilio est raccordée, `googlemeet setup` inclut des vérifications réussies
...
OAuth est optionnel pour créer un lien Meet, car `googlemeet create` peut se rabattre
...
Meet : créer des espaces de réunion, résoudre des espaces, et exécuter les vérifications préalables
de l’API Meet Media.
...
`meetings.conference.media.readonly` sert à la vérification préalable de l’API Meet Media et au travail média ;
Google peut exiger l’inscription au programme Developer Preview pour l’utilisation réelle de l’API Media.
Si vous avez seulement besoin de connexions Chrome basées sur le navigateur, ignorez entièrement OAuth.

### Créer le jeton d’actualisation
...
Privilégiez les variables d’environnement lorsque vous ne voulez pas placer le jeton d’actualisation dans la configuration.
Si des valeurs de configuration et d’environnement sont toutes deux présentes, le Plugin résout d’abord la configuration
puis utilise l’environnement comme solution de secours.

Le consentement OAuth inclut la création d’espaces Meet, l’accès en lecture aux espaces Meet, et l’accès en lecture
aux médias de conférence Meet. Si vous vous êtes authentifié avant l’existence de la prise en charge
de la création de réunions, réexécutez `openclaw googlemeet auth login --json` afin que le jeton d’actualisation
possède la portée `meetings.space.created`.

### Vérifier OAuth avec doctor

Exécutez le doctor OAuth lorsque vous voulez une vérification de santé rapide sans secret :
...
vérifie que la configuration OAuth existe et que le jeton d’actualisation peut créer un jeton d’accès.
...
| `oauth-token`        | Le jeton d’accès en cache est encore valide, ou le jeton d’actualisation a créé un nouveau jeton d’accès. |
...
que l’API Meet est activée sur le projet Google Cloud et que le compte autorisé
possède la portée `meetings.space.created`.
...
n’a pas la portée requise, ou que le compte Google ne peut pas accéder à cet espace Meet.
Une erreur de jeton d’actualisation signifie qu’il faut réexécuter `openclaw googlemeet auth login
...
provient du profil Chrome connecté sur le nœud sélectionné, et non de la configuration
OpenClaw.
...
Exécutez le contrôle préalable avant tout travail média :
...
La recherche dans Calendar peut résoudre l’URL de réunion depuis Google Calendar avant la lecture
des artefacts Meet :
...
`--calendar <id>` pour un calendrier non principal. La recherche dans Calendar nécessite une nouvelle
connexion OAuth incluant le périmètre en lecture seule des événements Calendar.
`calendar-events` affiche un aperçu des événements Meet correspondants et marque l’événement que
...
lignes de session de participant avec les heures de première et dernière apparition, la durée totale de session,
les indicateurs de retard/départ anticipé, et les ressources de participant en doublon fusionnées par utilisateur connecté
...
les fichiers de sortie, les décomptes, la source du jeton, l’événement Calendar lorsqu’il en a été utilisé un, et tout
...
nouvelle connexion OAuth incluant le périmètre en lecture seule Drive Meet. Sans
...
une grande exportation ou lorsqu’un agent n’a besoin que des décomptes, des enregistrements sélectionnés et
des avertissements.
...
Environnement de smoke test live :
...
Calendar nécessite `https://www.googleapis.com/auth/calendar.events.readonly`. L’exportation
du corps de document Drive nécessite
...
utilise en repli le profil de navigateur connecté du nœud Chrome épinglé. Les agents peuvent
utiliser l’outil `google_meet` avec `action: "create"` pour créer et rejoindre en une seule
étape. Pour une création limitée à l’URL, passez `"join": false`.
...
Si le repli navigateur rencontre la connexion Google ou un blocage d’autorisation Meet avant de
...
La création d’un Meet rejoint la réunion par défaut. Le transport Chrome ou nœud Chrome a encore
...
Le chemin temps réel Chrome commun n’a besoin que du Plugin activé, de BlackHole, de SoX,
et d’une clé de fournisseur vocal temps réel backend. OpenAI est la valeur par défaut ; définissez
...
- `chrome.autoJoin: true` : remplissage du nom d’invité et clic Rejoindre maintenant au mieux
  via l’automatisation du navigateur OpenClaw sur `chrome-node`
- `chrome.reuseExistingTab: true` : active un onglet Meet existant au lieu
  d’ouvrir des doublons
- `chrome.waitForInCallMs: 20000` : attend que l’onglet Meet signale l’état en appel
...
- `chrome.bargeInRmsThreshold: 650` : niveau RMS comptant comme une interruption humaine
  sur `chrome.bargeInInputCommand`
- `chrome.bargeInPeakThreshold: 2500` : niveau de crête comptant comme une interruption humaine
  sur `chrome.bargeInInputCommand`
- `chrome.bargeInCooldownMs: 900` : délai minimum entre les effacements répétés d’interruption
  humaine
- `realtime.provider: "openai"`
- `realtime.toolPolicy: "safe-read-only"`
- `realtime.instructions` : réponses orales brèves, avec
  `openclaw_agent_consult` pour les réponses plus approfondies
- `realtime.introMessage` : bref contrôle oral de disponibilité lorsque le pont temps réel
...
joue la séquence DTMF avant d'ouvrir le flux multimédia en temps réel, puis utilise le
texte d'introduction enregistré comme message d'accueil initial en temps réel. Si `voice-call` n'est pas
...
`transport: "chrome-node"` lorsque Chrome s'exécute sur un nœud jumelé, comme une VM
Parallels. Dans les deux cas, le modèle en temps réel et `openclaw_agent_consult` s'exécutent sur l'hôte
...
en temps réel. Utilisez `action: "test_speech"` pour créer ou réutiliser la session,
...
s'exécuter en `mode: "transcribe"`, car les sessions en observation seule ne peuvent intentionnellement pas
émettre de parole. Son résultat `speechOutputVerified` est basé sur l'augmentation des octets
de sortie audio en temps réel pendant cet appel de test ; une session réutilisée avec de l'audio plus ancien
ne compte donc pas comme une nouvelle vérification de parole réussie. Utilisez `action: "leave"` pour marquer
...
- `lastInputAt` / `lastOutputAt` : dernier son vu depuis le pont ou envoyé vers celui-ci
- `lastSuppressedInputAt` / `suppressedInputBytes` : entrée local loopback ignorée lorsque
...
de transcription de la réunion et retourne une réponse orale concise à la session vocale
en temps réel. Le modèle vocal peut ensuite prononcer cette réponse dans la réunion.
...
une politique d'outils, la mémoire et l'historique de session.

`realtime.toolPolicy` contrôle l'exécution de la consultation :

- `safe-read-only` : exposer l'outil de consultation et limiter l'agent standard à
  `read`, `web_search`, `web_fetch`, `x_search`, `memory_search` et
  `memory_get`.
- `owner` : exposer l'outil de consultation et laisser l'agent standard utiliser la politique
  d'outils normale de l'agent.
- `none` : ne pas exposer l'outil de consultation au modèle vocal en temps réel.

La clé de session de consultation est limitée à chaque session Meet, de sorte que les appels de consultation
suivants peuvent réutiliser le contexte de consultation précédent pendant la même réunion.
...
Pour le test rapide complet de connexion et de parole :
...
## Liste de vérification de test en direct
...
- L'onglet Meet rejoint l'appel et `test-speech` retourne l'état de Chrome avec
  `inCall: true`.
...
Pour un test rapide Twilio, utilisez une réunion qui expose les détails d'appel téléphonique entrant :
...
Si `googlemeet setup` échoue à `chrome-node-connected` ou si le journal du Gateway signale
...
Rechargez ensuite le service de nœud et réexécutez :
...
- Faites admettre l'invité depuis le compte hôte Meet.
- Accordez les autorisations de microphone/caméra Chrome lorsque l'invite d'autorisation native
  de Chrome apparaît.
- Fermez ou réparez une boîte de dialogue d'autorisation Meet bloquée.

Ne signalez pas « non connecté » simplement parce que Meet affiche « Voulez-vous que les personnes
vous entendent dans la réunion ? » Il s'agit de l'interstitiel de choix audio de Meet ; OpenClaw
clique sur **Utiliser le microphone** via l'automatisation du navigateur lorsqu'elle est disponible et continue
...
- Pour la création via API : le jeton de rafraîchissement a été émis après l'ajout de la prise en charge de la création.
  Les jetons plus anciens peuvent ne pas avoir le périmètre `meetings.space.created` ; réexécutez
...
- Pour le repli navigateur : les nouvelles tentatives réutilisent un onglet existant `https://meet.google.com/new`
  ...
  réessayez l'appel de l'outil plutôt que d'ouvrir manuellement un autre onglet Meet.
...
- Pour le repli navigateur : si Meet affiche « Voulez-vous que les personnes vous entendent dans la
  réunion ? », laissez l'onglet ouvert. OpenClaw devrait cliquer sur **Utiliser le microphone** ou, pour
  le repli en création seule, **Continuer sans microphone** via l'automatisation du navigateur
  et continuer d'attendre l'URL Meet générée. S'il ne peut pas, l'erreur
...
Utilisez `mode: "realtime"` pour l’écoute/la réponse vocale. `mode: "transcribe"` ne démarre intentionnellement
pas le pont vocal duplex en temps réel. Pour le débogage en observation seule,
...
`speechOutputTimedOut` est true, le fournisseur en temps réel peut avoir accepté
l’énoncé, mais OpenClaw n’a pas vu de nouveaux octets de sortie atteindre le pont
audio Chrome.
...
- Le microphone et le haut-parleur Meet sont routés par le chemin audio virtuel utilisé par
  OpenClaw.
...
`googlemeet doctor --oauth` lorsque vous devez vérifier l’actualisation OAuth de Google Meet
...
du jeton d’authentification ou du numéro d’appelant. Définissez-les sur l’hôte Gateway :
...
publique, ou lorsque `publicUrl` pointe vers un loopback ou un espace réseau privé.
...
Les URL de loopback et privées ne sont pas valides pour les rappels opérateur. N’utilisez pas
...
`voicecall smoke` vérifie seulement la préparation par défaut. Pour effectuer un essai à blanc avec un numéro précis :
...
Confirmez que l’événement Meet expose les détails d’appel téléphonique. Passez le numéro d’appel
...
- Exécutez `openclaw googlemeet doctor <session-id>` pour confirmer l’ID d’appel Twilio
  délégué, si le DTMF a été mis en file d’attente, et si le message d’accueil initial a été demandé.
...
- Exécutez `openclaw voicecall tail` et vérifiez que les webhooks Twilio arrivent au
  Gateway.
- Exécutez `openclaw logs --follow` et recherchez la séquence Meet Twilio : Google
  Meet délègue l’entrée, Voice Call stocke le TwiML DTMF de préconnexion, sert
  ce TwiML initial, puis sert le TwiML en temps réel et démarre le pont en temps réel
  avec `initialGreeting=queued`.
- Réexécutez `openclaw googlemeet setup --transport twilio` ; une vérification de configuration réussie est
  requise, mais ne prouve pas que la séquence de code PIN de la réunion est correcte.
...
nécessite donc toujours un chemin de participant. Ce plugin rend cette limite visible :
Chrome gère la participation au navigateur et le routage audio local ; Twilio gère
la participation par appel téléphonique.

Le mode en temps réel de Chrome nécessite `BlackHole 2ch` plus l’un des éléments suivants :

- `chrome.audioInputCommand` plus `chrome.audioOutputCommand` : OpenClaw possède le
  pont de modèle en temps réel et transmet l’audio en `chrome.audioFormat` entre ces
  commandes et le fournisseur vocal en temps réel sélectionné. Le chemin Chrome par défaut est
  PCM16 24 kHz ; le G.711 mu-law 8 kHz reste disponible pour les anciennes paires de commandes.
...
microphone local distinct et effacer la lecture de l’assistant lorsque l’humain commence
à parler. Cela maintient la parole humaine avant la sortie de l’assistant même lorsque l’entrée
local loopback BlackHole partagée est temporairement supprimée pendant la lecture de l’assistant.
Comme `chrome.audioInputCommand` et `chrome.audioOutputCommand`, c’est une
...
`googlemeet speak` déclenche le pont audio en temps réel actif pour une session Chrome.
