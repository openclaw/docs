...
- Les points de terminaison d’écriture authentifiés ne devraient pas renvoyer un simple `Unauthorized` lorsque
  le serveur connaît la raison. Les jetons manquants, les jetons invalides/révoqués et
  les comptes supprimés/bannis/désactivés devraient chacun recevoir un texte actionnable afin que les clients
...
devraient lire le corps de la réponse comme une chaîne lisible par un humain. Les paramètres de requête inconnus sont
...
- Les résultats sont renvoyés par ordre de pertinence (similarité d’embedding + boosts sur correspondances exactes de jetons slug/nom + un faible a priori de popularité).
- La pertinence est plus forte que la popularité. Une correspondance précise sur un slug ou un jeton de nom affiché peut dépasser une correspondance plus vague avec un engagement beaucoup plus élevé.
...
- La popularité est mise à l’échelle logarithmique et plafonnée. Les Skills à fort engagement peuvent être classés plus bas lorsque le texte de requête correspond moins bien.
...
- Mettez les termes que les utilisateurs rechercheront littéralement dans le nom affiché, le résumé et les tags. Utilisez un jeton de slug autonome uniquement lorsqu’il s’agit aussi d’une identité stable que vous voulez conserver.
...
- `metadata.os` : restrictions de système d’exploitation déclarées dans le frontmatter du Skill (par exemple `["macos"]`, `["linux"]`). `null` si non déclaré.
...
Point de terminaison modérateur/admin pour l’arrivée des signalements de Skills.
...
Les analyses d’envois locaux ne sont plus prises en charge. Les requêtes utilisant
...
- Les analyses publiées nécessitent un accès de gestion propriétaire/éditeur, ou une autorité de modérateur/administrateur de plateforme.
...
- Les tâches d’analyse sont asynchrones. Les requêtes d’analyse manuelles sont prioritaires par rapport au travail normal de publication/remplissage, mais leur achèvement dépend tout de même de la disponibilité des workers.
...
- Renvoie `queue.queuedAhead` et `queue.position` pendant la mise en file afin que les clients puissent afficher combien d’analyses manuelles prioritaires précèdent la requête. Les très grandes files sont bornées et signalées avec `queuedAheadIsEstimate: true`.
...
- Nécessite un accès de gestion propriétaire/éditeur à la skill ou au Plugin, ou une autorité de modérateur/administrateur de plateforme.
...
Route canonique de nouvelle analyse par lot réservée aux administrateurs. Elle accepte la même forme de charge utile que l’ancien `POST /api/v1/skills/-/rescan-batch`.
...
- `security.signals` contient des preuves de scanner à l’appui telles que `staticScan`, `virusTotal` et `skillSpector`.
...
- Utilisez `/verify` lorsque vous avez besoin de l’enveloppe de vérification Skill Card pour une seule skill, `/card` lorsque vous avez besoin du Markdown de carte généré, et `/scan` lorsque vous avez besoin de données détaillées de scanner.
...
- `category` (facultatif) : filtre de catégorie de plugin. Pris en charge uniquement lorsque la
  ...
  les alias de filtres v1 hérités sont documentés sous `GET /api/v1/plugins`.
...
- `isOfficial` (facultatif) : `true` ou `false`
- `category` (facultatif) : filtre de catégorie de plugin. Pris en charge uniquement lorsque la
  requête est limitée aux packages de plugins. Les catégories contrôlées et les alias de filtres
...
Les alias de filtres v1 hérités restent acceptés sur les points de terminaison de lecture :
...
`trending` est un classement des installations/téléchargements sur sept jours et n’utilise pas les totaux toutes périodes confondues.
...
- `family` (facultatif) : `code-plugin` ou `bundle-plugin`. Une omission signifie les deux
  familles de Plugins.
...
Renvoie une version de package, avec les métadonnées de fichiers, la compatibilité,
...
- `trust.stale` signifie que le résumé de confiance a été calculé à partir d’entrées obsolètes et
  doit être considéré comme nécessitant un rafraîchissement avant une décision d’autorisation à forte confiance.
...
- Ce point de terminaison est volontairement plus restreint que les points de terminaison de modération
  ...
  les identités des rapporteurs, les corps de rapports, les preuves privées ni les calendriers de revue
  internes.
...
- `packageName` est normalisé comme nom npm ; le package peut être absent pour les migrations
  planifiées.
- Cela ne suit que l’état de préparation de la migration. Cela ne modifie pas OpenClaw et ne génère pas
  de ClawPacks.
...
- `all` : toute version avec une dérogation manuelle, un état d’analyse non sain ou un rapport de package.
...
Signale un package pour revue par un modérateur. Les rapports sont au niveau du package, éventuellement
...
Point de terminaison modérateur/admin pour résoudre ou rouvrir des rapports de packages.
...
- Les transferts de skills adossées à GitHub ne proxient ni ne mettent en miroir les octets. La réponse JSON
  ...
  et `archiveUrl` ; l’analyse/l’état actuel constitue une barrière et n’est pas inclus comme métadonnée de charge utile
  de réussite.
...
- Le corps JSON avec `files` (basés sur storageId) est également accepté.
...
- Les packages de Plugin exigent `openclaw.plugin.json`. Les téléversements `.tgz` ClawPack doivent
  le contenir à `package/openclaw.plugin.json`.
- Les plugins de code exigent `package.json`, des métadonnées de dépôt source, des métadonnées de commit
...
- Les publications pour le compte d’un tiers valident toujours l’éligibilité au canal officiel par rapport au compte du propriétaire cible.
...
release. Les noms de packages deviennent des packages d’espace réservé privés sans lignes de release, afin que le même
...
Le point de terminaison fonctionne par défaut en mode simulation. Appliquer la récupération nécessite `dryRun: false` et
`confirmIdentityVerified: true` après que le personnel a vérifié indépendamment la continuité entre les deux
principaux GitHub. La récupération échoue en refusant l'opération lorsque l'éditeur personnel actuel de l'utilisateur de destination
...
les alias de slug de skill, les paquets, les avertissements de l'inspecteur de paquets et les lignes de condensé de recherche dérivé afin que
les chemins de propriétaire direct correspondent à la nouvelle autorité d'éditeur. Une réservation active de handle protégé
...
Les sources de skill GitHub ont une portée par éditeur et sont signalées comme vérifiées plutôt que réécrites.
...
- `POST /api/v1/skills/{slug}/transfer/cancel`
  - Réponse (accepter/refuser/annuler) : `{ "ok": true, "skillSlug": "demo-skill?" }`
...
Bannir un utilisateur et supprimer définitivement les skills détenus (modérateur/administrateur uniquement).
