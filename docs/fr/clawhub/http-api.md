---
read_when:
    - Ajout/modification des points de terminaison
...
Les répertoires tiers peuvent utiliser les points de terminaison publics en lecture pour lister ou rechercher les Skills ClawHub. Mettez les résultats en cache, respectez `429`/`Retry-After`, renvoyez les utilisateurs vers la fiche ClawHub canonique (`https://clawhub.ai/<owner>/<slug>`) et évitez de suggérer une approbation de ClawHub pour le site tiers. N’essayez pas de dupliquer du contenu masqué, privé ou bloqué par la modération en dehors de la surface publique de l’API.
...
- Les résultats sont renvoyés par ordre de pertinence (similarité d’embedding + boosts exacts de jetons slug/nom + a priori de popularité basé sur les téléchargements).
- La pertinence est plus forte que la popularité. Une correspondance précise de jeton de slug ou de nom d’affichage peut dépasser une correspondance plus vague avec beaucoup plus de téléchargements.
...
- Les téléchargements sont utilisés comme petit a priori à échelle logarithmique et comme critère de départage, pas comme signal de classement principal. Les Skills très téléchargés peuvent être moins bien classés lorsque le texte de requête correspond moins bien.
...
- Placez les termes que les utilisateurs rechercheront littéralement dans le nom d’affichage, le résumé et les tags. Utilisez un jeton de slug autonome uniquement lorsqu’il constitue aussi une identité stable que vous voulez conserver.
- Ne renommez pas un slug uniquement pour cibler une requête, sauf si le nouveau slug est un meilleur nom canonique à long terme. Les anciens slugs deviennent des alias de redirection, mais l’URL canonique, le slug affiché et les futurs digests de recherche utilisent le nouveau slug.
...
- Si un Skill est invisible de manière inattendue, vérifiez d’abord son état de modération avec `clawhub inspect <slug>` en étant connecté avant de modifier les métadonnées liées au classement.
...
- `moderation` n’est inclus que lorsque le Skill est signalé ou lorsque le propriétaire le consulte.
...
Signaler un Skill pour examen par les modérateurs. Les signalements sont au niveau du Skill, facultativement liés
...
- Inclut l’état normalisé de vérification ainsi que les détails propres au scanner.
...
- `requiresBrowser`, `requiresDesktop`, `requiresNativeDeps`,
  ...
  (facultatif) : raccourci `true`/`1` pour les balises d’exigence d’environnement
- `externalService`, `binary`, `osPermission` (facultatif) : raccourci pour les balises
  d’exigence d’environnement nommées
...
Recherche unifiée dans le catalogue entre Skills + packages de Plugin.
...
- `channel=private` ne renvoie que les packages que l’appelant authentifié peut lire.
- Les filtres d’artefacts sont adossés à des balises de capacité indexées :
...
Point de terminaison modérateur pour lister les lignes de migration de Plugin OpenClaw officiel.
...
- `packageName` est normalisé comme nom npm ; le package peut être absent pour les migrations
  planifiées.
- Cela suit uniquement la préparation de la migration. Cela ne modifie pas OpenClaw et ne génère pas
  de ClawPacks.

### `GET /api/v1/packages/moderation/queue`

Point de terminaison modérateur/admin pour les files de revue des versions de packages.
...
Point de terminaison modérateur/admin pour l’arrivée des signalements de packages.
...
Point de terminaison modérateur/admin pour la revue des publications de paquet.
...
- `approved` : revu manuellement et autorisé.
- `quarantined` : bloqué en attente de suivi.
- `revoked` : bloqué après qu’une publication a été précédemment considérée comme fiable.
...
Point de terminaison de maintenance réservé aux admins pour étiqueter les anciennes publications de paquet avec
...
- Par défaut en simulation.
- Les publications sans stockage ClawPack sont étiquetées `legacy-zip`.
- Les lignes existantes adossées à ClawPack auxquelles il manque `artifactKind` sont réparées en tant que
  `npm-pack`.
...
Télécharge l’archive ZIP déterministe historique d’une publication de paquet.
...
- Cette route reste réservée au ZIP. Elle ne diffuse pas de fichiers ClawPack `.tgz`.
...
- Les versions supprimées temporairement renvoient `410`.
...
Publie une publication code-plugin ou bundle-plugin.
...
- Les Plugins de code nécessitent `package.json`, les métadonnées du dépôt source, les métadonnées de commit
...
- Les publications pour le compte d’un tiers valident tout de même l’éligibilité au canal officiel par rapport au compte propriétaire cible.

### `DELETE /api/v1/skills/{slug}` / `POST /api/v1/skills/{slug}/undelete`

Supprime temporairement / restaure une skill (propriétaire, modérateur ou admin).
...
Les suppressions temporaires initiées par le propriétaire réservent le slug pendant 30 jours, après quoi le slug peut être revendiqué par
...
publication. Les noms de paquet deviennent des paquets privés fictifs sans ligne de publication, afin que le même
propriétaire puisse publier ultérieurement la vraie publication code-plugin ou bundle-plugin sous ce nom.
...
- Les deux points de terminaison nécessitent une authentification par jeton API et ne fonctionnent que pour le propriétaire de la skill.
...
- `limit` (facultatif) : résultats maximums (par défaut 20, max 200)
...
Si vous vous auto-hébergez, servez ce fichier (ou définissez `CLAWHUB_REGISTRY` explicitement ; ancien `CLAWDHUB_REGISTRY`).
