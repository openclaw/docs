...
Les annuaires tiers peuvent utiliser les points de terminaison publics en lecture pour lister ou rechercher des Skills ClawHub. Veuillez mettre les résultats en cache, respecter `429`/`Retry-After`, renvoyer les utilisateurs vers la fiche ClawHub canonique (`https://clawhub.ai/<owner>/skills/<slug>`) et éviter de laisser entendre que ClawHub approuve le site tiers. N’essayez pas de dupliquer du contenu masqué, privé ou bloqué par la modération en dehors de la surface de l’API publique.
...
- ClawHub utilise les en-têtes de transfert de confiance pour identifier les IP client en périphérie.
...
- Les résultats sont renvoyés par ordre de pertinence (similarité d’embedding + renforcements exacts de jetons de slug/nom + un léger a priori de popularité).
- La pertinence est plus forte que la popularité. Une correspondance précise de jeton de slug ou de nom d’affichage peut dépasser une correspondance plus approximative avec un engagement beaucoup plus élevé.
- Le texte ASCII est découpé en jetons sur les limites de mots et de ponctuation. Par exemple, `personal-map` contient un jeton `map` autonome, tandis que `amap-jsapi-skill` contient `amap`, `jsapi` et `skill` ; rechercher `map` donne donc à `personal-map` une correspondance lexicale plus forte qu’à `amap-jsapi-skill`.
...
- Les alias de renommage préservent la résolution des anciennes URL et des installations qui passent par le registre, mais le classement de recherche est basé sur les métadonnées canoniques de la Skill une fois le renommage indexé. Les statistiques existantes restent associées à la Skill.
- Si une Skill est invisiblement inattendue, vérifiez d’abord l’état de modération avec `clawhub inspect @owner/slug` en étant connecté avant de modifier les métadonnées liées au classement.
...
- Les anciens slugs créés par les flux de renommage/fusion de propriétaires se résolvent vers la Skill canonique.
...
- `version.security` inclut l’état de vérification normalisé du scan et les détails du scanner
...
- Les tâches d’analyse sont asynchrones. Les requêtes d’analyse manuelles sont prioritaires par rapport aux travaux normaux de publication/remplissage rétroactif, mais leur achèvement dépend toujours de la disponibilité des workers.
...
- L’identité du Skill, l’identité de l’éditeur et les métadonnées de la version sélectionnée sont des champs d’enveloppe de premier niveau (`slug`, `displayName`, `publisherHandle`, `version`, `resolvedFrom`, `tag`, `createdAt`) afin que les automatisations shell puissent les lire sans dépaqueter des wrappers imbriqués.
...
- Les résultats sont renvoyés par élément ; un Skill ou une version manquante ne fait pas échouer toute la réponse.
...
- Utilisez `/verify` lorsque vous avez besoin de l’enveloppe de vérification Skill Card d’un seul Skill, `/card` lorsque vous avez besoin du Markdown de carte généré, et `/scan` lorsque vous avez besoin de données détaillées de scanner.
...
- `category` (facultatif) : filtre de catégorie de Plugin. Pris en charge uniquement lorsque la
  ...
  `/api/v1/code-plugins`, `/api/v1/bundle-plugins`, ou aux points de terminaison de package avec
...
Recherche de catalogue unifié dans les Skills et les packages de Plugin.
...
- `isOfficial` (facultatif) : `true` ou `false`
- `category` (facultatif) : filtre de catégorie de Plugin. Pris en charge uniquement lorsque la
  requête est limitée aux packages de Plugin. Les catégories contrôlées et les alias
  de filtre hérités de v1 sont documentés sous `GET /api/v1/plugins`.
...
Exportation en masse des derniers Skills publics pour analyse hors ligne.
...
- Les Skills actuels adossés à GitHub avec une analyse `clean` ou `suspicious` incluent
  ...
  hachage de contenu et URL d’archive. Ils n’incluent pas les fichiers source hébergés par ClawHub.
...
- Les alias de filtre hérités v1 documentés sous `GET /api/v1/plugins` sont également
  acceptés.
- Le filtrage par catégorie est un véritable filtre d’API adossé aux lignes de condensé de catégorie de plugin,
...
- Les versions de package héritées renvoient un artefact `legacy-zip` et une
...
- Les packuments de paquets avec scope prennent en charge à la fois `/api/npm/@scope/name` et le chemin de requête encodé
  npm `/api/npm/@scope%2Fname`.
...
- Utilise le compartiment de débit de téléchargement.
- Les en-têtes de téléchargement incluent le SHA-256 ClawHub plus les métadonnées d’intégrité/shasum npm.
...
Télécharge un ZIP de version de skill hébergée, ou renvoie un relais vers la source GitHub pour une
...
- Les relais de skills adossées à GitHub ne proxifient ni ne mettent en miroir les octets. La réponse JSON
  inclut `sourceRef: "public-github"`, `repo`, `commit`, `path`, `contentHash`,
  et `archiveUrl` ; l’état d’analyse/courant est une condition d’accès et n’est pas inclus comme métadonnée de charge utile
  de réussite.
...
- Champ facultatif de la charge utile : `migrateOwner`. Lorsqu’il vaut `true` avec `ownerHandle`, une
...
- Champ facultatif de la charge utile : `ownerHandle`. Lorsqu’il est présent, seuls les admins peuvent publier pour le compte de ce propriétaire.
...
Supprime de manière réversible / restaure une skill (propriétaire, modérateur ou admin).
...
l’appelant comme propriétaire. Ce point de terminaison ne migre pas les handles d’utilisateurs/personnels existants et ne
...
Admin uniquement. Réserve les slugs racine et les noms de paquets pour un propriétaire légitime sans publier de
...
immuables ; les handles mutables sont uniquement utilisés comme garde-fou destiné à l’opérateur.
...
identités GitHub principales. La récupération échoue de façon fermée lorsque l'éditeur personnel actuel de l'utilisateur de destination
...
pour le handle récupéré est également réassignée à l'utilisateur de remplacement afin qu'une synchronisation ultérieure du profil
...
Les sources de Skill GitHub sont propres à l'éditeur et signalées comme vérifiées plutôt que réécrites.
