...
Les répertoires tiers peuvent utiliser les points de terminaison publics en lecture pour lister ou rechercher des Skills ClawHub. Veuillez mettre les résultats en cache, respecter `429`/`Retry-After`, renvoyer les utilisateurs vers la fiche ClawHub canonique (`https://clawhub.ai/<owner>/skills/<slug>`), et éviter de laisser entendre que ClawHub approuve le site tiers. Ne tentez pas de refléter du contenu masqué, privé ou bloqué par la modération hors de la surface de l’API publique.
...
- Si aucune IP client approuvée n’est disponible, les requêtes anonymes utilisent des compartiments de secours
  délimités uniquement par le type de limite de débit. Ces compartiments de secours n’incluent pas
...
- Les résultats sont renvoyés par ordre de pertinence (similarité d’embedding + boosts exacts de jetons de slug/nom + un faible a priori de popularité).
- La pertinence est plus forte que la popularité. Une correspondance précise de slug ou de jeton de nom d’affichage peut devancer une correspondance plus approximative avec un engagement bien plus élevé.
...
- Les alias de renommage préservent la résolution des anciennes URL et des installations qui se résolvent via le registre, mais le classement de recherche repose sur les métadonnées canoniques du Skill après indexation du renommage. Les statistiques existantes restent associées au Skill.
...
trié pour masquer le Skill dans le même workflow auditable.
...
- Les charges utiles des requêtes d’analyse et les rapports téléchargeables expirent du magasin des requêtes d’analyse après la période de rétention.
...
- Renvoie `queue.queuedAhead` et `queue.position` pendant la file d’attente afin que les clients puissent afficher combien d’analyses manuelles prioritaires précèdent la requête. Les très grandes files sont bornées et signalées avec `queuedAheadIsEstimate: true`.
...
- L’identité de la skill, l’identité de l’éditeur et les métadonnées de la version sélectionnée sont des champs de premier niveau de l’enveloppe (`slug`, `displayName`, `publisherHandle`, `version`, `resolvedFrom`, `tag`, `createdAt`) afin que l’automatisation shell puisse les lire sans décompresser des wrappers imbriqués.
...
- `isOfficial` (facultatif) : `true` ou `false`
- `category` (facultatif) : filtre de catégorie de plugin. Pris en charge uniquement lorsque la
  requête est limitée aux packages de plugin. Les catégories contrôlées et les alias de
  filtres v1 hérités sont documentés sous `GET /api/v1/plugins`.
...
- Les alias de filtres v1 hérités documentés sous `GET /api/v1/plugins` sont également
  acceptés.
- Le filtrage par catégorie est un véritable filtre d’API adossé aux lignes de synthèse de catégorie
...
Renvoie la disponibilité calculée pour la future consommation OpenClaw.

Les contrôles de disponibilité couvrent :
...
Signale un paquet pour revue par les modérateurs. Les signalements se situent au niveau du paquet, avec un lien facultatif
...
skill actuel basé sur GitHub avec une analyse `clean` ou `suspicious` et sans version
hébergée.
...
- Les transferts de skills basés sur GitHub ne proxient ni ne mettent en miroir les octets. La réponse JSON
  inclut `sourceRef: "public-github"`, `repo`, `commit`, `path`, `contentHash`
  et `archiveUrl` ; l’état d’analyse/actuel est une barrière et n’est pas inclus comme métadonnées de charge utile de réussite.
...
- Les champs de formulaire autorisés sont `payload`, des blobs `files` répétés ou une référence de tarball `clawpack`
  ...
  le flux upload-url. Les publications avec id de stockage en préparation doivent également inclure le
...
Admin uniquement. Réserve des slugs racine et des noms de paquet pour un propriétaire légitime sans publier de
...
fournisseur GitHub immuables ; les handles mutables ne sont utilisés que comme garde-fou destiné à l’opérateur.
...
les alias de slug de Skills, les packages, les avertissements de l’inspecteur de packages et les lignes de digest de recherche dérivées afin que
...
Bannir un utilisateur et supprimer définitivement les Skills possédées (modérateur/administrateur uniquement).
...
le contenu (administrateur uniquement). Utilise par défaut la simulation sauf si `dryRun` vaut `false`.
...
Si vous auto-hébergez, servez ce fichier (ou définissez explicitement `CLAWHUB_REGISTRY` ; hérité `CLAWDHUB_REGISTRY`).
