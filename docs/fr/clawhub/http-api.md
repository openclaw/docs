...
summary: Référence de l’API HTTP (publique + points de terminaison CLI + auth).
x-i18n:
    generated_at: "2026-07-05T05:51:08Z"
    model: gpt-5.5
...
Les annuaires tiers peuvent utiliser les points de terminaison publics en lecture pour lister ou rechercher des skills ClawHub. Veuillez mettre les résultats en cache, respecter `429`/`Retry-After`, renvoyer les utilisateurs vers la fiche ClawHub canonique (`https://clawhub.ai/<owner>/skills/<slug>`) et éviter de suggérer une approbation de ClawHub envers le site tiers. N’essayez pas de dupliquer du contenu masqué, privé ou bloqué par la modération en dehors de la surface de l’API publique.

Les raccourcis de slug web se résolvent entre familles de registres, mais les clients API doivent utiliser
les URL canoniques renvoyées par les points de terminaison de lecture au lieu de reconstruire la précédence
des routes.
...
- Les points de terminaison d’écriture authentifiés ne doivent pas renvoyer un simple `Unauthorized` lorsque
  le serveur connaît la raison. Les jetons manquants, invalides/révoqués et
...
- `X-RateLimit-Remaining` / `RateLimit-Remaining` : budget restant exact lorsqu’il est présent.
  Les requêtes réussies partitionnées omettent cet en-tête au lieu de renvoyer une valeur globale approximative.
...
- Utilisez un repli exponentiel avec jitter pour éviter les nouvelles tentatives synchronisées.
- Si `Retry-After` est absent, utilisez `RateLimit-Reset` comme solution de repli (ou calculez à partir de `X-RateLimit-Reset`).

Source IP :

- Utilise les en-têtes d’IP client de confiance, notamment `cf-connecting-ip`, uniquement lorsque le
  déploiement active explicitement les en-têtes transférés de confiance.
- ClawHub utilise des en-têtes de transfert de confiance pour identifier les IP clientes en périphérie.
- Si aucune IP client de confiance n’est disponible, les requêtes anonymes utilisent des compartiments de repli
  ...
  les chemins, slugs, noms de package, versions, chaînes de requête ou autres
...
- `nonSuspicious` (facultatif) : alias historique de `nonSuspiciousOnly`
...
- Les résultats sont renvoyés par ordre de pertinence (similarité d’embedding + renforcements exacts des tokens de slug/nom + un faible a priori de popularité).
- La pertinence est plus forte que la popularité. Une correspondance précise de slug ou de token de nom d’affichage peut devancer une correspondance plus vague avec un engagement bien plus fort.
- Le texte ASCII est découpé en tokens aux limites de mots et de ponctuation. Par exemple, `personal-map` contient un token autonome `map`, tandis que `amap-jsapi-skill` contient `amap`, `jsapi` et `skill` ; rechercher `map` donne donc à `personal-map` une correspondance lexicale plus forte que `amap-jsapi-skill`.
- La popularité est mise à l’échelle logarithmique et plafonnée. Les skills à fort engagement peuvent être moins bien classées lorsque le texte de la requête correspond moins fortement.
...
- Ne renommez pas un slug uniquement pour viser une requête, sauf si le nouveau slug est un meilleur nom canonique à long terme. Les anciens slugs deviennent des alias de redirection, mais l’URL canonique, le slug affiché et les futurs résumés de recherche utilisent le nouveau slug.
- Les alias de renommage préservent la résolution des anciennes URL et des installations qui se résolvent via le registre, mais le classement de recherche est basé sur les métadonnées canoniques de la skill après l’indexation du renommage. Les statistiques existantes restent associées à la skill.
- Si une skill est invisiblement de manière inattendue, vérifiez d’abord l’état de modération avec `clawhub inspect @owner/slug` en étant connecté avant de modifier les métadonnées liées au classement.
...
- `sort` (facultatif) : `updated` (par défaut), `recommended` (alias : `default`), `createdAt` (alias : `newest`), `downloads`, `stars` (alias : `rating`), les alias historiques d’installation `installsCurrent`/`installs`/`installsAllTime` correspondent à `downloads`, `trending`
- `nonSuspiciousOnly` (facultatif) : `true` pour masquer les skills suspectes (`flagged.suspicious`)
- `nonSuspicious` (facultatif) : alias historique de `nonSuspiciousOnly`
...
- `createdAt` est stable pour les parcours de nouvelles skills ; `updated` change lorsque des skills existantes sont republiées.
- Lorsque `nonSuspiciousOnly=true`, les tris basés sur un curseur peuvent renvoyer moins de `limit` éléments sur une page parce que les skills suspectes sont filtrées après la récupération de la page.
...
- `metadata.os` : restrictions de SE déclarées dans le frontmatter de la skill (par ex. `["macos"]`, `["linux"]`). `null` si non déclaré.
...
Signale une skill pour examen par un modérateur. Les signalements se font au niveau de la skill, peuvent être liés facultativement
...
`status` à `open`. Passez `finalAction: "hide"` avec un signalement trié
...
`multipart/form-data` ou `{ "source": { "kind": "upload" } }` retournent `410`.
...
- Les tâches d’analyse sont asynchrones. Les requêtes d’analyse manuelles sont prioritaires par rapport au travail normal de publication/remplissage rétroactif, mais l’achèvement dépend toujours de la disponibilité des workers.

### `GET /api/v1/skills/-/scan/{scanId}`

Point de terminaison d’interrogation authentifié pour une analyse soumise.

- Retourne l’état en file d’attente/en cours/réussi/échoué.
- Retourne `queue.queuedAhead` et `queue.position` pendant la mise en file d’attente afin que les clients puissent afficher le nombre d’analyses manuelles prioritaires qui précèdent la requête. Les très grandes files sont bornées et signalées avec `queuedAheadIsEstimate: true`.
- Lorsqu’il est disponible, `report` contient les sections `clawscan`, `skillspector`, `staticAnalysis` et `virustotal`.
- Les tâches d’analyse échouées retournent `status: "failed"` avec `lastError`.

### `GET /api/v1/skills/-/scan/{scanId}/download`

Point de terminaison authentifié d’archive de rapport.

- Nécessite une analyse réussie ; les analyses non terminales retournent `409`.
- Retourne un ZIP avec `manifest.json`, `clawscan.json`, `skillspector.json`, `static-analysis.json`, `virustotal.json` et `README.md`.
...
- Retourne les résultats d’analyse stockés pour la version soumise exacte, y compris les versions bloquées ou masquées.
- `kind` vaut par défaut `skill` ; utilisez `kind=plugin` pour les analyses de plugin/package.
- Retourne la même forme de ZIP que les téléchargements de requêtes d’analyse.
...
Route canonique d’état de lot réservée aux administrateurs. Elle accepte `{ "jobIds": ["..."] }` et retourne les mêmes compteurs agrégés que l’ancien `POST /api/v1/skills/-/rescan-batch/status`.

### `GET /api/v1/skills/{slug}/verify`

Retourne l’enveloppe de vérification Skill Card utilisée par `clawhub skill verify`.
...
- `ok` vaut `true` uniquement lorsque la version sélectionnée dispose d’une Skill Card générée, n’est pas bloquée comme logiciel malveillant par la modération, et que la vérification ClawScan est propre.
- L’identité de la skill, l’identité de l’éditeur et les métadonnées de la version sélectionnée sont des champs d’enveloppe de premier niveau (`slug`, `displayName`, `publisherHandle`, `version`, `resolvedFrom`, `tag`, `createdAt`) afin que les automatisations shell puissent les lire sans décompresser de wrappers imbriqués.
...
- `security.signals` contient les preuves de scanner à l’appui, telles que `staticScan`, `virusTotal` et `skillSpector`.
- `security.signals.dependencyRegistry` est conservé pour la compatibilité des réponses v1, mais le scanner d’existence du registre de dépendances est retiré et cette clé vaut toujours `null`.
- `provenance` vaut `server-resolved-github-import` uniquement lorsque ClawHub a résolu et stocké un dépôt/ref/commit/chemin GitHub lors de la publication ou de l’importation ; sinon, elle vaut `unavailable`.

### `POST /api/v1/skills/-/security-verdicts`

Retourne les verdicts de sécurité compacts actuels pour des versions exactes de skills. Ce
...
- `items` doit contenir de 1 à 100 paires uniques `{ slug, version }`.
- Les résultats sont par élément ; une skill ou une version manquante ne fait pas échouer toute la réponse.
- La réponse concerne uniquement la sécurité. Elle n’inclut pas les données Skill Card, l’état de carte générée, les listes de fichiers d’artéfacts ni les charges utiles détaillées des scanners.
- `security.signals` contient uniquement des preuves à l’appui au niveau de l’état ; utilisez `/scan` ou la page d’audit de sécurité ClawHub pour obtenir tous les détails des scanners.
- `security.signals.dependencyRegistry` est conservé pour la compatibilité des réponses v1, mais le scanner d’existence du registre de dépendances est retiré et cette clé vaut toujours `null`.
...
- `category` (facultatif) : filtre de catégorie de plugin. Pris en charge uniquement lorsque la
  ...
  les alias de filtres hérités de la v1 sont documentés sous `GET /api/v1/plugins`.
...
- `isOfficial` (facultatif) : `true` ou `false`
- `category` (facultatif) : filtre de catégorie de plugin. Pris en charge uniquement lorsque la
  requête est limitée aux packages de plugins. Les catégories contrôlées et les alias de
  filtres hérités de la v1 sont documentés sous `GET /api/v1/plugins`.
...
Navigation dans le catalogue réservé aux plugins parmi les packages code-plugin et bundle-plugin.
...
Les alias de filtres hérités de la v1 restent acceptés sur les points de terminaison de lecture :
...
Export groupé des dernières publications publiques de Plugin pour analyse hors ligne.
...
- Chaque Plugin exporté inclut les fichiers stockés de la dernière publication.
- Les métadonnées d’exportation par Plugin sont stockées dans
...
Recherche limitée aux Plugin dans les packages code-plugin et bundle-plugin.
...
- Les contrôles de tri de l’interface navigateur pour la recherche de Plugin réordonnent les résultats de pertinence chargés,
...
- Skills peut également être résolu via cette route dans le catalogue unifié.
- Les packages privés renvoient `404`, sauf si l’appelant peut lire l’éditeur propriétaire.

### `DELETE /api/v1/packages/{name}`

Supprime de manière réversible un package et toutes ses publications.
...
- `version.sha256hash` est une métadonnée de compatibilité obsolète pour les anciens clients. Elle
  ...
  l’artefact canonique de publication.
...
Renvoie le résumé exact de sécurité et de confiance de la publication du package pour les clients
d’installation. Il s’agit de la surface publique de consommation OpenClaw pour décider si une
publication résolue peut être installée.
...
- `release.releaseId`, `release.version` et `release.createdAt` identifient la
  publication exacte qui a été évaluée.
- `release.artifactKind`, `release.artifactSha256`, `release.npmIntegrity`,
  `release.npmShasum` et `release.npmTarballName` sont présents lorsqu’ils sont connus pour
  l’artefact de publication.
- `trust.scanStatus` est l’état de confiance effectif dérivé des entrées du scanner
  et de la modération manuelle de publication.
- `trust.moderationState` est nullable. Il vaut `null` lorsqu’aucune modération manuelle de publication
  n’existe.
...
- `trust.reasons` est la liste d’explication destinée à l’utilisateur et à l’audit. Les codes de raison
  sont des chaînes stables et compactes telles que `manual:quarantined`, `scan:malicious`,
  et `package:malicious`.
- `trust.pending` signifie qu’une ou plusieurs entrées de confiance attendent encore leur finalisation.
...
Renvoie l’état de préparation calculé pour une future consommation OpenClaw.
...
Point de terminaison modérateur pour lister les lignes de migration de Plugin OpenClaw officiels.
...
Point de terminaison modérateur/administrateur pour les files de revue des publications de package.
...
- `manual` : toute publication avec un remplacement de modération manuel.
- `all` : toute publication avec un remplacement manuel, un état d’analyse non propre ou un rapport de package.
...
Signaler un package pour revue par les modérateurs. Les signalements sont au niveau du package, facultativement
liés à une version. Ils alimentent la file de modération mais ne masquent pas automatiquement ni ne
bloquent les téléchargements par eux-mêmes ; les modérateurs doivent utiliser la modération de publication pour
...
Point de terminaison modérateur/admin pour la réception des rapports de paquet.
...
Point de terminaison propriétaire/modérateur pour la visibilité de la modération du paquet.

Authentification :

- Nécessite un jeton API pour le propriétaire du paquet, un membre de l'éditeur, un modérateur ou
...
Point de terminaison modérateur/admin pour résoudre ou rouvrir des rapports de paquet.
...
`finalAction: "revoke"` avec un rapport confirmé pour appliquer la modération de release dans le
...
Point de terminaison modérateur/admin pour l'examen d'une release de paquet.
...
- `revoked` : bloqué après qu'une release a précédemment été approuvée.

Les releases en quarantaine et révoquées renvoient `403` depuis les routes de téléchargement d'artéfacts.
...
- Utilise par défaut la dernière release.
- Utilise le compartiment de débit de lecture, pas le compartiment de téléchargement.
- Les fichiers binaires renvoient `415`.
- Limite de taille de fichier : 200 Ko.
- Les analyses VirusTotal en attente ne bloquent pas les lectures ; les releases malveillantes peuvent toujours être retenues ailleurs.
- Les paquets privés renvoient `404` sauf si l'appelant peut lire l'éditeur propriétaire.

### `GET /api/v1/packages/{name}/download`

Télécharge l'archive ZIP déterministe héritée pour une release de paquet.
...
- Les analyses VirusTotal en attente ne bloquent pas les téléchargements ; les releases malveillantes renvoient `403`.
...
- Les packuments de paquets avec scope prennent en charge à la fois `/api/npm/@scope/name` et le chemin de requête
...
- Les transferts de skills adossées à GitHub ne proxifient ni ne mettent en miroir les octets. La réponse JSON
  inclut `sourceRef: "public-github"`, `repo`, `commit`, `path`, `contentHash`
  et `archiveUrl` ; l'état d'analyse/actuel est un garde-fou et n'est pas inclus comme métadonnée de charge utile de succès.
...
- Champ de charge utile facultatif : `migrateOwner`. Lorsque `true` avec `ownerHandle`, une
  skill existante peut être déplacée vers ce propriétaire si l'acteur est administrateur/propriétaire à la fois sur les
  éditeurs actuel et cible. Sans cette option explicite, les changements de propriétaire sont
  rejetés.

### `POST /api/v1/packages`

Publie une release de code-plugin ou bundle-plugin.
...
- Les champs de formulaire autorisés sont `payload`, des blobs `files` répétés ou une référence de tarball `clawpack`
  unique. `clawpack` peut être un blob `.tgz` ou un id de stockage renvoyé par
...
- Les Plugins de code nécessitent `package.json`, des métadonnées de dépôt source, des métadonnées de commit source,
  des métadonnées de schéma de configuration, `openclaw.compat.pluginApi` et
...
Les masquages par modérateur/admin et les suppressions de sécurité n'expirent pas ainsi.
...
Administrateur uniquement. Garantit qu'un éditeur d'organisation existe pour un handle. Si le handle pointe encore vers un
ancien utilisateur partagé/éditeur personnel, le point de terminaison le migre d'abord en éditeur d'organisation.
...
Administrateur uniquement. Réserve des slugs racine et des noms de paquet pour un propriétaire légitime sans publier de
release. Les noms de paquet deviennent des paquets indicateurs privés sans lignes de release, afin que le même
propriétaire puisse publier plus tard la vraie release code-plugin ou bundle-plugin sous ce nom.
...
Administrateur uniquement. Récupère un éditeur personnel pour un principal GitHub OAuth de remplacement vérifié
sans modifier les lignes de compte Convex Auth. La requête doit nommer les deux ids de compte fournisseur GitHub
immuables ; les handles modifiables servent uniquement de garde-fou destiné à l'opérateur.
...
identités GitHub. La récupération échoue en mode fermé lorsque le publisher personnel actuel de l’utilisateur de destination
possède des skills, des packages ou des sources de skill GitHub.
La récupération migre aussi les anciens champs `ownerUserId` pour les skills du publisher récupéré,
les alias de slug de skill, les packages, les avertissements de l’inspecteur de package et les lignes dérivées de condensé de recherche afin que
...
pour le handle récupéré est également réaffectée à l’utilisateur de remplacement afin que la synchronisation ultérieure du profil
...
Les sources de skill GitHub sont rattachées au publisher et signalées comme vérifiées plutôt que réécrites.
...
Modifier la raison enregistrée pour un bannissement existant sans lever le bannissement ni restaurer le
contenu (administrateur uniquement). Utilise par défaut le mode simulation sauf si `dryRun` vaut `false`.
...
Toujours pris en charge pour les anciennes versions du CLI :
...
qui préparent une archive tar ClawPack doivent envoyer l’identifiant de stockage obtenu en tant que
`clawpack` et le ticket renvoyé en tant que `clawpackUploadTicket`.

## Découverte du registre (`/.well-known/clawhub.json`)

Le CLI peut découvrir les paramètres de registre/authentification depuis le site :
