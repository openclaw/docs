...
x-i18n:
    generated_at: "2026-07-03T17:18:55Z"
    model: gpt-5.5
...
Les annuaires tiers peuvent utiliser les points de terminaison publics en lecture pour lister ou rechercher des Skills ClawHub. Veuillez mettre les résultats en cache, respecter `429`/`Retry-After`, renvoyer les utilisateurs vers la fiche ClawHub canonique (`https://clawhub.ai/<owner>/skills/<slug>`) et éviter de laisser entendre que ClawHub approuve le site tiers. N’essayez pas de répliquer du contenu masqué, privé ou bloqué par la modération en dehors de la surface d’API publique.

Les raccourcis de slugs Web se résolvent entre les familles de registres, mais les clients API doivent utiliser
les URL canoniques renvoyées par les points de terminaison de lecture plutôt que de reconstruire la priorité
des routes.
...
- Les résultats sont renvoyés par ordre de pertinence (similarité d’embedding + boosts sur correspondance exacte de jetons de slug/nom + léger a priori de popularité).
- La pertinence est plus forte que la popularité. Une correspondance précise de slug ou de jeton de nom affiché peut dépasser une correspondance plus large ayant un engagement beaucoup plus fort.
...
- Les alias de renommage préservent la résolution des anciennes URL et des installations qui passent par le registre, mais le classement de recherche est basé sur les métadonnées canoniques du Skill une fois le renommage indexé. Les statistiques existantes restent associées au Skill.
...
- `metadata.os` : restrictions de systèmes d’exploitation déclarées dans le frontmatter du Skill (par ex. `["macos"]`, `["linux"]`). `null` si elles ne sont pas déclarées.
- `metadata.systems` : cibles système Nix (par ex. `["aarch64-darwin", "x86_64-linux"]`). `null` si elles ne sont pas déclarées.
...
retour de `status` à `open`. Transmettez `finalAction: "hide"` avec un signalement trié
...
- Les charges utiles des demandes d’analyse et les rapports téléchargeables expirent du stockage des demandes d’analyse après la fenêtre de rétention.
...
- Les tâches d’analyse sont asynchrones. Les demandes d’analyse manuelles sont prioritaires sur le travail normal de publication/remplissage rétroactif, mais leur achèvement dépend toujours de la disponibilité des workers.
...
- Renvoie `queue.queuedAhead` et `queue.position` pendant la mise en file d’attente, afin que les clients puissent afficher combien d’analyses manuelles prioritaires précèdent la demande. Les très grandes files sont bornées et signalées avec `queuedAheadIsEstimate: true`.
...
- L’identité du skill, l’identité de l’éditeur et les métadonnées de la version sélectionnée sont des champs de premier niveau de l’enveloppe (`slug`, `displayName`, `publisherHandle`, `version`, `resolvedFrom`, `tag`, `createdAt`) afin que les automatisations shell puissent les lire sans décompresser des wrappers imbriqués.
...
- Utilisez `/verify` lorsque vous avez besoin de l’enveloppe de vérification Skill Card d’un seul skill, `/card` lorsque vous avez besoin du Markdown de carte généré, et `/scan` lorsque vous avez besoin de données de scanner détaillées.
...
Export en masse des dernières versions publiques de Plugin pour analyse hors ligne.
...
- Les alias de filtres legacy v1 documentés sous `GET /api/v1/plugins` sont aussi
  acceptés.
- Le filtrage par catégorie est un vrai filtre d’API adossé aux lignes de résumé
...
- Les contrôles de tri de l’interface navigateur pour la recherche de Plugin réordonnent les résultats de pertinence chargés,
  en cohérence avec le comportement actuel de navigation `/skills`.
...
- `version.artifact.kind` vaut `legacy-zip` pour les archives de paquets de l’ancien modèle ou
...
d’installation. C’est la surface de consommation publique OpenClaw pour décider si une
...
- `trust.scanStatus` est le statut de confiance effectif dérivé des entrées d’analyse
...
- `trust.blockedFromDownload` est le signal de blocage d’installation. OpenClaw et les autres
  ...
  redériver des règles de blocage à partir des champs d’analyse ou de modération.
...
- `trust.stale` signifie que le résumé de confiance a été calculé à partir d’entrées obsolètes et
  doit être considéré comme nécessitant un rafraîchissement avant une décision d’autorisation à confiance élevée.

Notes :

- Ce point de terminaison est exact à la version. Les clients doivent l’appeler après avoir résolu la
  version de paquet qu’ils ont l’intention d’installer, pas seulement après avoir lu les dernières
...
Signaler un paquet pour revue par un modérateur. Les signalements sont au niveau du paquet, éventuellement
liés à une version. Ils alimentent la file de modération mais ne masquent pas automatiquement ni ne
bloquent les téléchargements à eux seuls ; les modérateurs doivent utiliser la modération de version pour
...
Point de terminaison propriétaire/modérateur pour la visibilité de modération du paquet.
...
- Les archives de Plugins/paquets sont des fichiers zip avec une racine `package/` afin que les anciens clients OpenClaw
...
- `dist.tarball`, `dist.integrity` et `dist.shasum` utilisent des champs
  compatibles npm afin que les utilisateurs puissent pointer npm vers le miroir s’ils le souhaitent.
...
Télécharge un ZIP de version de Skill hébergé, ou renvoie une remise de source GitHub pour une
...
- `tag` (facultatif) : nom d’étiquette (par ex. `latest`)
...
- Les remises de Skills adossées à GitHub ne proxyfient ni ne mettent en miroir les octets. La réponse JSON
...
- Champ de charge utile facultatif : `migrateOwner`. Lorsque `true` avec `ownerHandle`, une
  Skill existante peut être déplacée vers ce propriétaire si l’acteur est admin/propriétaire chez les deux
...
- Les paquets de Plugins nécessitent `openclaw.plugin.json`. Les téléversements `.tgz` ClawPack doivent
...
- Les publications pour le compte d’autrui valident toujours l’éligibilité au canal official par rapport au compte propriétaire cible.

### `DELETE /api/v1/skills/{slug}` / `POST /api/v1/skills/{slug}/undelete`

Supprimer de façon réversible / restaurer une Skill (propriétaire, modérateur ou admin).
...
utilisateur partagé hérité/éditeur personnel, le point de terminaison le migre d’abord vers un éditeur d’organisation.
...
l’appelant comme propriétaire. Ce point de terminaison ne migre pas les handles utilisateur/personnel existants et ne
marque pas l’éditeur comme trusted/official.
...
identités GitHub. La récupération échoue en mode fermé lorsque l’éditeur personnel actuel de l’utilisateur de destination
...
les alias de slugs de Skills, les paquets, les avertissements de l’inspecteur de paquets et les lignes de résumé de recherche dérivées afin que
...
## Points de terminaison de CLI hérités (obsolètes)

Toujours pris en charge pour les anciennes versions de CLI :
...
`POST /api/cli/upload-url` renvoie `uploadUrl` et `uploadTicket`. Les publications de paquet
