...
Les annuaires tiers peuvent utiliser les points de terminaison publics en lecture pour lister ou rechercher des Skills ClawHub. Veuillez mettre les résultats en cache, respecter `429`/`Retry-After`, renvoyer les utilisateurs vers la fiche ClawHub canonique (`https://clawhub.ai/<owner>/skills/<slug>`) et éviter de suggérer une approbation de ClawHub pour le site tiers. N’essayez pas de dupliquer du contenu masqué, privé ou bloqué par modération en dehors de la surface de l’API publique.

Les raccourcis de slug Web se résolvent entre les familles de registres, mais les clients d’API doivent utiliser
...
- Si `Retry-After` est absent, utilisez `RateLimit-Reset` en solution de repli (ou calculez depuis `X-RateLimit-Reset`).
...
- Les résultats sont renvoyés par ordre de pertinence (similarité d’embedding + boosts pour correspondance exacte de token de slug/nom + un léger a priori de popularité).
- La pertinence est plus forte que la popularité. Une correspondance précise de slug ou de token de nom d’affichage peut devancer une correspondance plus vague avec un engagement beaucoup plus élevé.
- Le texte ASCII est segmenté aux limites des mots et de la ponctuation. Par exemple, `personal-map` contient un token autonome `map`, tandis que `amap-jsapi-skill` contient `amap`, `jsapi` et `skill` ; rechercher `map` donne donc à `personal-map` une correspondance lexicale plus forte qu’à `amap-jsapi-skill`.
...
- Les alias de renommage préservent la résolution pour les anciennes URL et les installations qui se résolvent via le registre, mais le classement de recherche est basé sur les métadonnées canoniques de la Skill après l’indexation du renommage. Les statistiques existantes restent associées à la Skill.
...
- Lorsque `nonSuspiciousOnly=true`, les tris basés sur curseur peuvent renvoyer moins de `limit` éléments sur une page, car les Skills suspectes sont filtrées après la récupération de la page.
...
- `metadata.os` : restrictions d’OS déclarées dans le frontmatter de la Skill (p. ex. `["macos"]`, `["linux"]`). `null` si non déclaré.
- `metadata.systems` : cibles système Nix (p. ex. `["aarch64-darwin", "x86_64-linux"]`). `null` si non déclaré.
...
- Les appelants publics obtiennent seulement `200` pour les Skills visibles déjà signalées.
...
Signaler une Skill pour examen par les modérateurs. Les signalements sont au niveau de la Skill, facultativement liés
...
- Inclut l’état de vérification normalisé ainsi que des détails propres à chaque analyseur.
...
- `moderation` est un instantané actuel de modération au niveau de la compétence, dérivé de la dernière version.
...
Point de terminaison authentifié pour soumettre de nouvelles tâches ClawScan.
...
- Les charges utiles de requête d’analyse et les rapports téléchargeables expirent du magasin des requêtes d’analyse après la fenêtre de conservation.
- Les analyses publiées nécessitent un accès de gestion propriétaire/éditeur, ou l’autorité d’un modérateur/administrateur de plateforme.
...
Point de terminaison authentifié de sondage pour une analyse soumise.

- Renvoie l’état en file d’attente/en cours/réussie/échouée.
- Renvoie `queue.queuedAhead` et `queue.position` pendant la mise en file d’attente afin que les clients puissent afficher le nombre d’analyses manuelles prioritaires précédant la requête. Les très grandes files sont plafonnées et signalées avec `queuedAheadIsEstimate: true`.
...
- Nécessite un accès de gestion propriétaire/éditeur à la compétence ou au plugin, ou l’autorité d’un modérateur/administrateur de plateforme.
- Renvoie les résultats d’analyse stockés pour la version soumise exacte, y compris les versions bloquées ou masquées.
- `kind` vaut par défaut `skill` ; utilisez `kind=plugin` pour les analyses de plugins/paquets.
...
- L’identité de la compétence, l’identité de l’éditeur et les métadonnées de la version sélectionnée sont des champs d’enveloppe de premier niveau (`slug`, `displayName`, `publisherHandle`, `version`, `resolvedFrom`, `tag`, `createdAt`) afin que l’automatisation shell puisse les lire sans décompresser d’enveloppes imbriquées.
...
- `security.signals.dependencyRegistry` est conservé pour la compatibilité de réponse v1, mais l’analyseur d’existence du registre de dépendances est retiré et cette clé vaut toujours `null`.
...
point de terminaison de collection est destiné aux clients qui savent déjà quelles versions de compétences ClawHub installées ils doivent afficher, comme OpenClaw Control UI.
...
- Les résultats sont par élément ; une compétence ou une version manquante ne fait pas échouer toute la réponse.
- La réponse est uniquement liée à la sécurité. Elle n’inclut pas les données de Skill Card, l’état de carte générée, les listes de fichiers d’artéfact ni les charges utiles détaillées des analyseurs.
- `security.signals` contient uniquement des preuves complémentaires au niveau de l’état ; utilisez `/scan` ou la page d’audit de sécurité ClawHub pour obtenir tous les détails des analyseurs.
- `security.signals.dependencyRegistry` est conservé pour la compatibilité de réponse v1, mais l’analyseur d’existence du registre de dépendances est retiré et cette clé vaut toujours `null`.
...
- Utilisez `/verify` lorsque vous avez besoin de l’enveloppe de vérification Skill Card pour une seule compétence, `/card` lorsque vous avez besoin du Markdown de carte généré, et `/scan` lorsque vous avez besoin de données détaillées d’analyseur.
...
- `category` (facultatif) : filtre de catégorie de Plugin. Pris en charge uniquement lorsque la
  ...
  alias de filtre hérités v1 sont documentés sous `GET /api/v1/plugins`.
...
Recherche dans le catalogue unifié à travers les Skills + paquets de Plugins.
...
- `category` (facultatif) : filtre de catégorie de Plugin. Pris en charge uniquement lorsque la
  ...
  de filtre hérités v1 sont documentés sous `GET /api/v1/plugins`.
...
Navigation dans le catalogue réservé aux Plugins, à travers les paquets code-plugin et bundle-plugin.
...
Les alias de filtre hérités v1 restent acceptés sur les points de terminaison de lecture :
...
- `trust.stale` signifie que le résumé de confiance a été calculé à partir d’entrées obsolètes et
  doit être considéré comme nécessitant une actualisation avant une décision d’autorisation à haut niveau de confiance.

Notes :

- Ce point de terminaison est exact à la version. Les clients doivent l’appeler après avoir résolu la
  version du paquet qu’ils prévoient d’installer, et pas seulement après avoir lu les dernières
...
- Ce point de terminaison est volontairement plus restreint que les points de terminaison de modération
  ...
  l’identité des rapporteurs, le contenu des signalements, les preuves privées ni les chronologies
  d’examen internes.

### `GET /api/v1/packages/{name}/versions/{version}/artifact`

Renvoie les métadonnées explicites de résolution d’artéfact pour une version de paquet.

Notes :

- Les versions de paquets héritées renvoient un artéfact `legacy-zip` et une URL ZIP héritée
  `downloadUrl`.
...
Télécharge l’artéfact de version par le chemin de résolution explicite.
...
Renvoie la préparation calculée pour une consommation future par OpenClaw.

Les contrôles de préparation couvrent :

- statut de canal officiel
- disponibilité de la dernière version
- disponibilité de l’artéfact ClawPack npm-pack
- condensé de l’artéfact
...
- `packageName` est normalisé comme nom npm ; le paquet peut être absent pour les migrations
  planifiées.
- Cela suit uniquement la préparation à la migration. Cela ne modifie pas OpenClaw et ne génère pas
  de ClawPacks.
...
Signaler un paquet pour examen par un modérateur. Les signalements se font au niveau du paquet, éventuellement
...
approuver, mettre en quarantaine ou révoquer des artéfacts.
...
Point de terminaison modérateur/admin pour l’ingestion des signalements de packages.
...
Point de terminaison modérateur/admin pour résoudre ou rouvrir des signalements de packages.
...
Point de terminaison modérateur/admin pour l’examen des versions de packages.
...
Télécharge un ZIP de version de Skill hébergé, ou renvoie un transfert vers une source GitHub pour une
...
- Les transferts de Skills adossées à GitHub ne proxient ni ne mettent en miroir les octets. La réponse JSON
  ...
  et `archiveUrl` ; l’analyse/l’état actuel sert de garde et n’est pas inclus comme métadonnée de charge utile
  de succès.
...
- Les champs de formulaire autorisés sont `payload`, des blobs `files` répétés, ou une référence de tarball `clawpack`.
  ...
  le flux upload-url. Les publications d’identifiants de stockage préparés doivent aussi inclure le
...
- Les requêtes directes de publication multipart sont plafonnées à 18 Mo. Les tarballs ClawPack peuvent
...
Les masquages par modérateur/admin et les retraits pour sécurité n’expirent pas de cette façon.
...
version. Les noms de packages deviennent des packages de remplacement privés sans ligne de version, afin que le même
...
sans modifier les lignes de compte Convex Auth. La requête doit nommer les deux identifiants de compte fournisseur GitHub
immutables ; les handles modifiables sont seulement utilisés comme garde visible par l’opérateur.

Le point de terminaison utilise par défaut le mode dry-run. L’application de la récupération nécessite `dryRun: false` et
`confirmIdentityVerified: true` après que le personnel a vérifié indépendamment la continuité entre les deux
principaux GitHub. La récupération échoue en mode fermé lorsque le publisher personnel actuel de l’utilisateur de destination
...
les alias de slugs de skill, les packages, les avertissements de l’inspecteur de packages et les lignes dérivées de digest de recherche afin que
...
pour le handle récupéré est également réassignée à l’utilisateur de remplacement afin qu’une synchronisation de profil ultérieure
...
Les sources de skill GitHub sont limitées au publisher et signalées comme vérifiées plutôt que réécrites.
...
Bannir un utilisateur et supprimer définitivement les skills possédées (modérateur/admin uniquement).
...
Modifier la raison enregistrée d’un bannissement existant sans lever le bannissement ni restaurer
le contenu (admin uniquement). Utilise par défaut le mode dry-run sauf si `dryRun` vaut `false`.
...
- `limit` (facultatif) : nombre maximal de résultats (par défaut 20, maximum 200)
...
Ajouter/supprimer une étoile (mise en avant). Les deux points de terminaison sont idempotents.
...
## Points de terminaison de l’ancien CLI (obsolètes)
...
## Découverte du registry (`/.well-known/clawhub.json`)

Le CLI peut découvrir les paramètres de registry/authentification depuis le site :
