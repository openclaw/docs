...
x-i18n:
    generated_at: "2026-07-01T08:00:06Z"
    model: gpt-5.5
...
Les raccourcis de slug web sont résolus entre les familles de registres, mais les clients API doivent utiliser
...
- Si `Retry-After` est absent, utilisez en repli `RateLimit-Reset` (ou calculez à partir de `X-RateLimit-Reset`).
...
- Si aucune IP client de confiance n’est disponible, les requêtes anonymes utilisent des compartiments de repli
  ...
  les chemins, slugs, noms de paquet, versions, chaînes de requête ni autres
...
- Les résultats sont renvoyés par ordre de pertinence (similarité d’embedding + bonus pour correspondance exacte de tokens slug/nom + faible a priori de popularité).
- La pertinence est plus forte que la popularité. Une correspondance précise de slug ou de token de nom d’affichage peut dépasser une correspondance plus vague avec un engagement beaucoup plus fort.
...
- Placez les termes que les utilisateurs chercheront littéralement dans le nom d’affichage, le résumé et les tags. N’utilisez un token de slug autonome que lorsqu’il s’agit aussi d’une identité stable que vous voulez conserver.
...
- Les alias de renommage préservent la résolution pour les anciennes URL et les installations qui passent par le registre, mais le classement de recherche est basé sur les métadonnées canoniques du Skill une fois le renommage indexé. Les statistiques existantes restent associées au Skill.
...
- Lorsque `nonSuspiciousOnly=true`, les tris basés sur curseur peuvent renvoyer moins de `limit` éléments sur une page parce que les Skills suspects sont filtrés après récupération de la page.
...
- Les anciens slugs créés par les flux de renommage/fusion de propriétaire résolvent vers le Skill canonique.
...
- `moderation` n’est inclus que lorsque le Skill est signalé ou que son propriétaire le consulte.
...
- Les tâches d’analyse sont asynchrones. Les requêtes d’analyse manuelles sont prioritaires par rapport aux tâches normales de publication/remplissage rétroactif, mais leur achèvement dépend toujours de la disponibilité des workers.

### `GET /api/v1/skills/-/scan/{scanId}`

Point de terminaison de consultation authentifié pour une analyse soumise.
...
Route canonique réservée aux administrateurs pour la nouvelle analyse par lot. Elle accepte la même forme de charge utile que l’ancien `POST /api/v1/skills/-/rescan-batch`.
...
- L’identité de la compétence, l’identité de l’éditeur et les métadonnées de la version sélectionnée sont des champs de premier niveau de l’enveloppe (`slug`, `displayName`, `publisherHandle`, `version`, `resolvedFrom`, `tag`, `createdAt`) afin que l’automatisation shell puisse les lire sans dépaqueter de wrappers imbriqués.
...
- `security.signals` contient les preuves de scanner à l’appui, comme `staticScan`, `virusTotal` et `skillSpector`.
...
- `security.signals` contient uniquement des preuves d’appui au niveau de l’état ; utilisez `/scan` ou la page d’audit de sécurité ClawHub pour les détails complets des scanners.
- `security.signals.dependencyRegistry` est conservé pour la compatibilité des réponses v1, mais le scanner d’existence du registre de dépendances est retiré et cette clé vaut toujours `null`.
...
- Utilisez `/verify` lorsque vous avez besoin de l’enveloppe de vérification Skill Card d’une seule compétence, `/card` lorsque vous avez besoin du Markdown de carte généré, et `/scan` lorsque vous avez besoin des données détaillées des scanners.
...
- `category` (facultatif) : filtre de catégorie de plugin. Pris en charge uniquement lorsque la
  ...
  filtre v1 hérités sont documentés sous `GET /api/v1/plugins`.
...
- `trust.scanStatus` est le statut de confiance effectif dérivé des entrées d’analyseur
...
- `packageName` est normalisé comme nom npm ; le paquet peut être manquant pour les migrations
  planifiées.
...
Point de terminaison modérateur/administrateur pour les files d’examen de versions de paquets.
...
- `all` : toute version avec une substitution manuelle, un état d’analyse non propre ou un rapport de paquet.
...
Signaler un paquet pour examen par un modérateur. Les rapports se font au niveau du paquet, avec un lien facultatif
à une version. Ils alimentent la file de modération mais ne masquent pas automatiquement ni ne
...
- Les métadonnées uniquement issues du registre ne sont pas injectées dans l’archive téléchargée.
...
- Les transferts de skill adossée à GitHub ne proxifient ni ne mettent en miroir les octets. La réponse JSON
  ...
  et `archiveUrl` ; l’analyse/l’état actuel sert de barrière et n’est pas inclus comme métadonnée de charge utile de succès.
- Les statistiques de téléchargement sont comptées comme des identités uniques par jour UTC (`userId` quand le jeton d’API est valide, sinon IP).
...
- Seuls l’éditeur de l’org `openclaw` et les éditeurs personnels des membres actuels de l’org `openclaw`
  peuvent publier dans le canal `official`.
- Les publications pour le compte d’autrui valident toujours l’éligibilité au canal officiel par rapport au compte du propriétaire cible.
...
Admin uniquement. Garantit qu’un éditeur d’org existe pour un identifiant. Si l’identifiant pointe encore vers un
ancien utilisateur partagé/éditeur personnel, le point de terminaison le migre d’abord vers un éditeur d’org.
Pour une org nouvellement créée, fournissez `memberHandle` ; l’admin agissant n’est pas ajouté comme membre.
...
Création authentifiée en libre-service d’un éditeur d’org. Crée un nouvel éditeur d’org et ajoute
...
Admin uniquement. Réserve des slugs racine et des noms de package pour un propriétaire légitime sans publier de
version. Les noms de package deviennent des packages d’espace réservé privés sans lignes de version, afin que le même
propriétaire puisse plus tard publier la vraie version code-plugin ou bundle-plugin sous ce nom.
...
sans modifier les lignes de compte Convex Auth. La requête doit nommer les deux ids de compte
fournisseur GitHub immuables ; les identifiants modifiables sont uniquement utilisés comme garde-fou destiné à l’opérateur.

Le point de terminaison utilise la simulation par défaut. Appliquer la récupération nécessite `dryRun: false` et
...
pour le handle récupéré est aussi réattribuée à l’utilisateur de remplacement afin qu’une synchronisation ultérieure du profil
...
- Les deux points de terminaison exigent une authentification par jeton d’API et fonctionnent uniquement pour le propriétaire de la Skill.
