...
Les anciens `/api/...` et `/api/cli/...` sont conservés pour la compatibilité (voir `DEPRECATIONS.md`).
OpenAPI : `/api/v1/openapi.json`.

## Réutilisation du catalogue public

Les annuaires tiers peuvent utiliser les points de terminaison publics en lecture pour lister ou rechercher des Skills ClawHub. Veuillez mettre les résultats en cache, respecter `429`/`Retry-After`, renvoyer les utilisateurs vers la fiche ClawHub canonique (`https://clawhub.ai/<owner>/<slug>`) et éviter de suggérer une approbation du site tiers par ClawHub. N’essayez pas de dupliquer du contenu masqué, privé ou bloqué par la modération en dehors de la surface de l’API publique.
...
- Les résultats sont renvoyés par ordre de pertinence (similarité d’embedding + bonus de jetons exacts de slug/nom + a priori de popularité issu des téléchargements).
- La pertinence est plus forte que la popularité. Une correspondance précise sur un slug ou un jeton du nom d’affichage peut devancer une correspondance plus approximative avec beaucoup plus de téléchargements.
- Le texte ASCII est segmenté sur les frontières de mots et de ponctuation. Par exemple, `personal-map` contient un jeton autonome `map`, tandis que `amap-jsapi-skill` contient `amap`, `jsapi` et `skill` ; rechercher `map` donne donc à `personal-map` une correspondance lexicale plus forte qu’à `amap-jsapi-skill`.
- Les téléchargements sont utilisés comme un petit a priori à échelle logarithmique et comme départage, pas comme signal de classement principal. Les Skills très téléchargées peuvent être classées plus bas lorsque le texte de la requête correspond moins bien.
...
- Ne renommez pas un slug simplement pour cibler une requête, sauf si le nouveau slug est un meilleur nom canonique à long terme. Les anciens slugs deviennent des alias de redirection, mais l’URL canonique, le slug affiché et les futurs condensés de recherche utilisent le nouveau slug.
- Les alias de renommage préservent la résolution pour les anciennes URL et les installations qui se résolvent via le registre, mais le classement de recherche est basé sur les métadonnées canoniques de la Skill après indexation du renommage. Les statistiques existantes restent associées à la Skill.
- Si une Skill est anormalement invisible, vérifiez d’abord l’état de modération avec `clawhub inspect <slug>` en étant connecté avant de modifier les métadonnées liées au classement.
...
- `version.security` inclut le statut normalisé de vérification d’analyse et les détails du scanner
...
- `os`, `arch`, `libc` (facultatif) : raccourci pour les filtres de capacités de l’hôte
...
- `npmMirror` (facultatif) : `true`/`1` pour afficher les versions de paquets basées sur ClawPack
...
- `target` / `hostTarget`, `os`, `arch`, `libc`, `requiresBrowser`,
  ...
  `osPermission` sont acceptés comme raccourcis pour les balises de capacités courantes
- `artifactKind` (facultatif) : `legacy-zip` ou `npm-pack`
- `npmMirror` (facultatif) : `true`/`1` pour rechercher les versions de paquets basées sur ClawPack
  disponibles via le miroir npm
...
- `channel=private` ne renvoie que les paquets que l’appelant authentifié peut lire.
- Les filtres d’artefacts reposent sur des balises de capacités indexées :
...
- Nécessite un jeton d’API pour le propriétaire du paquet, un propriétaire/admin de l’éditeur d’organisation,
...
Renvoie une version de paquet, avec les métadonnées de fichier, la compatibilité,
...
- Ce point de terminaison est volontairement plus restreint que les points de terminaison de modération
  propriétaires/modérateurs. Il expose la décision d’installation et l’explication publique, pas
...
- Utilise le bucket de limite de débit de téléchargement.
...
Point de terminaison modérateur/admin pour les files de revue des versions de paquets.
...
Signaler un paquet pour examen par les modérateurs. Les signalements se font au niveau du paquet, avec un lien facultatif vers une version. Ils alimentent la file de modération, mais ne masquent pas automatiquement les artefacts et ne bloquent pas les téléchargements par eux-mêmes ; les modérateurs doivent utiliser la modération des versions pour approuver, mettre en quarantaine ou révoquer les artefacts.
...
Point de terminaison modérateur/admin pour l'arrivée des signalements de paquets.
...
- `revoked` : bloqué après qu'une version a été précédemment jugée fiable.
...
Point de terminaison de maintenance réservé aux admins pour étiqueter les anciennes versions de paquets avec
...
- Utilise le mode simulation par défaut.
...
- Les métadonnées présentes uniquement dans le registre ne sont pas injectées dans l'archive téléchargée.
...
- Les statistiques de téléchargement sont comptées comme identités uniques par heure (`userId` lorsque le jeton API est valide, sinon l'IP).
...
Réservé aux admins. Réserve des slugs racines et des noms de paquets pour un propriétaire légitime sans publier de
version. Les noms de paquets deviennent des paquets d'espace réservé privés sans lignes de version, de sorte que le même
propriétaire peut publier ultérieurement la vraie version code-plugin ou bundle-plugin sous ce nom.
...
- Les deux points de terminaison nécessitent une authentification par jeton API et fonctionnent uniquement pour le propriétaire de la skill.
