...
x-i18n:
    generated_at: "2026-07-01T20:18:52Z"
    model: gpt-5.5
...
Les annuaires tiers peuvent utiliser les points de terminaison publics de lecture pour lister ou rechercher des Skills ClawHub. Veuillez mettre les résultats en cache, respecter `429`/`Retry-After`, renvoyer les utilisateurs vers la fiche ClawHub canonique (`https://clawhub.ai/<owner>/skills/<slug>`) et éviter de suggérer une approbation de ClawHub pour le site tiers. N’essayez pas de dupliquer du contenu masqué, privé ou bloqué par modération en dehors de la surface d’API publique.
...
- Les points de terminaison d’écriture authentifiés ne doivent pas renvoyer un simple `Unauthorized` lorsque
  le serveur connaît la raison. Les jetons manquants, les jetons invalides/révoqués et les
  comptes supprimés/bannis/désactivés doivent chacun recevoir un texte actionnable afin que les clients
...
- Si aucune IP client fiable n’est disponible, les requêtes anonymes utilisent des compartiments de repli
  ...
  les chemins fournis par l’appelant, les slugs, les noms de package, les versions, les chaînes de requête ni d’autres
...
- La pertinence est plus importante que la popularité. Une correspondance précise de jeton de slug ou de nom d’affichage peut dépasser une correspondance plus large avec un engagement beaucoup plus fort.
...
- Les analyses publiées exigent un accès de gestion propriétaire/éditeur, ou une autorité de modérateur/administrateur de la plateforme.
...
- Les tâches d’analyse sont asynchrones. Les requêtes d’analyse manuelles sont prioritaires par rapport au travail normal de publication/remplissage rétroactif, mais leur achèvement dépend tout de même de la disponibilité des workers.
...
- Renvoie `queue.queuedAhead` et `queue.position` pendant la file d’attente afin que les clients puissent afficher combien d’analyses manuelles prioritaires précèdent la requête. Les très grandes files sont plafonnées et signalées avec `queuedAheadIsEstimate: true`.
...
Point d’entrée authentifié d’archive de rapport.
...
Point d’entrée authentifié d’archive de rapport stockée pour les versions soumises.

- Exige un accès de gestion propriétaire/éditeur à la skill ou au plugin, ou une autorité de modérateur/administrateur de la plateforme.
...
- L’identité de la skill, l’identité de l’éditeur et les métadonnées de la version sélectionnée sont des champs d’enveloppe de premier niveau (`slug`, `displayName`, `publisherHandle`, `version`, `resolvedFrom`, `tag`, `createdAt`) afin que l’automatisation shell puisse les lire sans décompacter des wrappers imbriqués.
...
- `security.signals` contient uniquement les preuves complémentaires au niveau de l’état ; utilisez `/scan` ou la page d’audit de sécurité ClawHub pour les détails complets des scanners.
- `security.signals.dependencyRegistry` est conservé pour la compatibilité de réponse v1, mais le scanner d’existence du registre de dépendances est retiré et cette clé vaut toujours `null`.
...
- Les entrées Skills restent adossées au registre des Skills et peuvent toujours être publiées uniquement via `POST /api/v1/skills`.
...
Parcours du catalogue réservé aux Plugins pour les packages de Plugins de code et de Plugins de bundle.
...
Les alias de filtre hérités de la v1 restent acceptés sur les points de terminaison en lecture :
...
- Les Skills hébergées incluent les derniers fichiers de version stockés et sont répertoriées dans
...
- Les Skills actuelles adossées à GitHub avec une analyse `clean` ou `suspicious` incluent
  ...
  le hachage de contenu et l’URL d’archive. Elles n’incluent pas les fichiers sources hébergés par ClawHub.
...
- Les Skills peuvent également se résoudre via cette route dans le catalogue unifié.
...
- Nécessite un jeton d'API pour le propriétaire du paquet, un propriétaire/administrateur
...
- `version.artifact.kind` est `legacy-zip` pour les archives de paquets de l'ancien monde ou
...
clients d'installation. C'est la surface de consommation publique d'OpenClaw pour décider si une
...
- `trust.blockedFromDownload` est le signal de blocage d'installation. OpenClaw et les autres
  ...
  redériver les règles de blocage à partir des champs du scanner ou de modération.
...
Point de terminaison modérateur pour lister les lignes de migration de Plugin officiel OpenClaw.
...
Signaler un paquet pour examen par un modérateur. Les rapports sont au niveau du paquet, éventuellement
...
Point de terminaison modérateur/administrateur pour l’examen des versions de paquet.
...
Télécharge un ZIP de version de Skill hébergée, ou renvoie une transmission vers la source GitHub pour une
...
- `tag` (facultatif) : nom de balise (p. ex. `latest`)
...
- Les transmissions de Skills adossées à GitHub ne relaient ni ne mettent en miroir les octets. La réponse JSON
...
- Les champs de formulaire autorisés sont `payload`, les blobs `files` répétés, ou une référence de tarball `clawpack`.
  ...
  le flux upload-url. Les publications avec identifiant de stockage préparé doivent aussi inclure le
...
- Les code plugins nécessitent `package.json`, les métadonnées de dépôt source, les métadonnées de commit
...
Les masquages modérateur/administrateur et les suppressions de sécurité n’expirent pas de cette manière.
...
utilisateur partagé hérité/éditeur personnel, le point de terminaison le migre d’abord en éditeur d’organisation.
...
version. Les noms de paquet deviennent des paquets espaces réservés privés sans lignes de version, afin que le même
...
le contenu (administrateur uniquement). Fonctionne par défaut en simulation sauf si `dryRun` vaut `false`.
