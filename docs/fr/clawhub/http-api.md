...
x-i18n:
    generated_at: "2026-07-04T06:29:15Z"
    model: gpt-5.5
...
Les annuaires tiers peuvent utiliser les points de terminaison publics en lecture pour lister ou rechercher les compétences ClawHub. Veuillez mettre les résultats en cache, respecter `429`/`Retry-After`, renvoyer les utilisateurs vers la fiche ClawHub canonique (`https://clawhub.ai/<owner>/skills/<slug>`) et éviter de suggérer une approbation de ClawHub pour le site tiers. N’essayez pas de dupliquer du contenu masqué, privé ou bloqué par la modération en dehors de la surface de l’API publique.

Les raccourcis de slug web se résolvent entre les familles de registre, mais les clients API doivent utiliser
...
- `X-RateLimit-Remaining` / `RateLimit-Remaining` : budget restant exact lorsqu’il est présent.
  Les requêtes shardées réussies omettent cet en-tête au lieu de renvoyer une valeur globale approximative.
...
- Si `Retry-After` est manquant, repliez-vous sur `RateLimit-Reset` (ou calculez depuis `X-RateLimit-Reset`).
...
- ClawHub utilise les en-têtes de transfert fiables pour identifier les IP des clients à la périphérie.
- Si aucune IP client fiable n’est disponible, les requêtes anonymes utilisent des compartiments de repli
  limités uniquement par le type de limite de débit. Ces compartiments de repli n’incluent pas
  les chemins, slugs, noms de packages, versions, chaînes de requête ou autres
  paramètres d’artefact fournis par l’appelant.

## Réponses d’erreur

Les réponses d’erreur v1 publiques sont en texte brut avec `content-type: text/plain; charset=utf-8`.
...
doivent lire le corps de la réponse comme une chaîne lisible par l’humain. Les paramètres de requête inconnus sont
...
- `highlightedOnly` (facultatif) : `true` pour filtrer sur les compétences mises en avant
- `nonSuspiciousOnly` (facultatif) : `true` pour masquer les compétences suspectes (`flagged.suspicious`)
- `nonSuspicious` (facultatif) : alias ancien pour `nonSuspiciousOnly`
...
- Les résultats sont renvoyés par ordre de pertinence (similarité d’embedding + bonus sur les correspondances exactes de jetons de slug/nom + un léger a priori de popularité).
- La pertinence est plus forte que la popularité. Une correspondance précise de slug ou de jeton de nom d’affichage peut dépasser une correspondance plus lâche avec un engagement bien plus élevé.
- Le texte ASCII est segmenté selon les limites de mots et de ponctuation. Par exemple, `personal-map` contient un jeton autonome `map`, tandis que `amap-jsapi-skill` contient `amap`, `jsapi` et `skill` ; rechercher `map` donne donc à `personal-map` une correspondance lexicale plus forte que `amap-jsapi-skill`.
- La popularité est mise à l’échelle logarithmique et plafonnée. Les compétences à fort engagement peuvent être moins bien classées lorsque le texte de la requête correspond moins bien.
- Un état de modération suspect ou masqué peut retirer une compétence de la recherche publique selon les filtres de l’appelant et l’état de modération actuel.
...
- Les alias de renommage préservent la résolution pour les anciennes URL et les installations qui se résolvent via le registre, mais le classement de recherche se base sur les métadonnées canoniques de la compétence une fois le renommage indexé. Les statistiques existantes restent associées à la compétence.
- Si une compétence est invisibile de manière inattendue, vérifiez d’abord l’état de modération avec `clawhub inspect @owner/slug` en étant connecté avant de modifier les métadonnées liées au classement.
...
- `sort` (facultatif) : `updated` (par défaut), `recommended` (alias : `default`), `createdAt` (alias : `newest`), `downloads`, `stars` (alias : `rating`), les alias anciens d’installation `installsCurrent`/`installs`/`installsAllTime` correspondent à `downloads`, `trending`
- `nonSuspiciousOnly` (facultatif) : `true` pour masquer les compétences suspectes (`flagged.suspicious`)
- `nonSuspicious` (facultatif) : alias ancien pour `nonSuspiciousOnly`
...
- `createdAt` est stable pour les crawls de nouvelles compétences ; `updated` change lorsque des compétences existantes sont republiées.
- Lorsque `nonSuspiciousOnly=true`, les tris basés sur curseur peuvent renvoyer moins de `limit` éléments sur une page, car les compétences suspectes sont filtrées après la récupération de la page.
...
- Les anciens slugs créés par les flux de renommage/fusion du propriétaire se résolvent vers la compétence canonique.
- `metadata.os` : restrictions de système d’exploitation déclarées dans le frontmatter de la compétence (par ex. `["macos"]`, `["linux"]`). `null` si non déclaré.
- `metadata.systems` : cibles système Nix (par ex. `["aarch64-darwin", "x86_64-linux"]`). `null` si non déclaré.
- `metadata` vaut `null` si la compétence n’a aucune métadonnée de plateforme.
- `moderation` est inclus uniquement lorsque la compétence est signalée ou que le propriétaire la consulte.
...
- Les propriétaires et modérateurs peuvent accéder aux détails de modération des compétences masquées.
- Les appelants publics obtiennent uniquement `200` pour les compétences visibles déjà signalées.
...
Signaler une compétence pour examen par les modérateurs. Les signalements sont au niveau de la compétence, éventuellement liés
à une version, et alimentent la file des signalements de compétences.
...
Point de terminaison modérateur/admin pour l’arrivée des signalements de compétences.
...
Point de terminaison modérateur/admin pour résoudre ou rouvrir les signalements de compétences.
...
passage de `status` à `open`. Passez `finalAction: "hide"` avec un signalement trié
pour masquer la compétence dans le même flux de travail auditable.
...
Renvoie les détails de vérification d’analyse de sécurité pour une version de compétence.
...
- Les charges utiles de requête de scan et les rapports téléchargeables expirent du magasin des requêtes de scan après la fenêtre de conservation.
- Les scans publiés nécessitent un accès de gestion propriétaire/éditeur, ou une autorité de modérateur/administrateur de la plateforme.
- Les scans publiés écrivent en retour uniquement lorsque `update: true` et que le scan se termine avec succès.
...
- Les tâches de scan sont asynchrones. Les requêtes de scan manuelles sont prioritaires par rapport aux travaux normaux de publication/remplissage rétroactif, mais leur achèvement dépend toujours de la disponibilité des workers.
...
- Renvoie `queue.queuedAhead` et `queue.position` pendant que la tâche est en file d’attente afin que les clients puissent afficher combien de scans manuels prioritaires se trouvent avant la requête. Les très grandes files d’attente sont bornées et signalées avec `queuedAheadIsEstimate: true`.
...
- Nécessite un scan réussi ; les scans non terminaux renvoient `409`.
...
- Renvoie les résultats de scan stockés pour la version exacte soumise, y compris les versions bloquées ou masquées.
- `kind` utilise `skill` par défaut ; utilisez `kind=plugin` pour les scans de plugins/packages.
...
- L’identité de la skill, l’identité de l’éditeur et les métadonnées de la version sélectionnée sont des champs de premier niveau de l’enveloppe (`slug`, `displayName`, `publisherHandle`, `version`, `resolvedFrom`, `tag`, `createdAt`) afin que l’automatisation shell puisse les lire sans dépaqueter d’enveloppes imbriquées.
...
point de terminaison de collection est destiné aux clients qui savent déjà quelles versions de skills ClawHub installées ils doivent afficher, comme OpenClaw Control UI.
...
- Les résultats sont par élément ; une skill ou version manquante ne fait pas échouer toute la réponse.
- La réponse concerne uniquement la sécurité. Elle n’inclut pas les données Skill Card, l’état de carte générée, les listes de fichiers d’artefacts ni les charges utiles détaillées de scanners.
- `security.signals` contient uniquement les preuves à l’appui au niveau de l’état ; utilisez `/scan` ou la page d’audit de sécurité ClawHub pour obtenir tous les détails des scanners.
- `security.signals.dependencyRegistry` est conservé pour la compatibilité de réponse v1, mais le scanner d’existence du registre de dépendances est retiré et cette clé vaut toujours `null`.
...
- Utilisez `/verify` lorsque vous avez besoin de l’enveloppe de vérification Skill Card d’une seule skill, `/card` lorsque vous avez besoin du Markdown de carte généré, et `/scan` lorsque vous avez besoin des données détaillées de scanner.
...
- `category` (facultatif) : filtre de catégorie de plugin. Pris en charge uniquement lorsque la
  ...
  les alias de filtre hérités de la v1 sont documentés sous `GET /api/v1/plugins`.
...
- `POST /api/v1/packages` reste réservé aux versions de plugins de code et de plugins de bundle.
...
Recherche de catalogue unifié dans les Skills et les packages de plugins.
...
- `category` (facultatif) : filtre de catégorie de plugin. Pris en charge uniquement lorsque la
  ...
  filtre hérités de la v1 sont documentés sous `GET /api/v1/plugins`.
...
Parcours du catalogue réservé aux plugins parmi les packages `code-plugin` et `bundle-plugin`.
...
Les alias de filtre hérités de la v1 restent acceptés sur les points de terminaison de lecture :
...
- Les Skills actuelles adossées à GitHub avec une analyse `clean` ou `suspicious` incluent
  ...
  empreinte de contenu et URL d’archive. Elles n’incluent pas les fichiers sources hébergés par ClawHub.
...
- Les alias de filtres v1 hérités documentés sous `GET /api/v1/plugins` sont également
  acceptés.
- Le filtrage par catégorie est un vrai filtre d’API adossé aux lignes de condensé
...
- Les contrôles de tri de l’interface navigateur pour la recherche de plugins réordonnent les résultats de pertinence chargés,
  comme le comportement de navigation actuel de `/skills`.
...
- Nécessite un jeton d’API pour le propriétaire du package, un propriétaire/administrateur
  de l’éditeur d’organisation, un modérateur de plateforme ou un administrateur de plateforme.
...
d’installation. Il s’agit de la surface de consommation publique d’OpenClaw permettant de décider si une
...
- Ce point de terminaison est exact à la version. Les clients doivent l’appeler après avoir résolu la
  version du package qu’ils ont l’intention d’installer, et pas seulement après avoir lu les dernières
...
- Les versions de package héritées renvoient un artefact `legacy-zip` et une URL de téléchargement ZIP héritée
  `downloadUrl`.
...
Point de terminaison d’administration pour créer ou mettre à jour une ligne de migration de plugin officiel.
...
- `all` : toute version avec une substitution manuelle, un état d’analyse non propre ou un rapport de package.
...
Signale un package pour revue par un modérateur. Les signalements sont au niveau du package, avec un lien facultatif
vers une version. Ils alimentent la file de modération, mais ne masquent pas automatiquement les téléchargements et ne les
bloquent pas à eux seuls ; les modérateurs doivent utiliser la modération de version pour
...
Point de terminaison modérateur/admin pour la réception des rapports de paquets.
...
Point de terminaison modérateur/admin pour résoudre ou rouvrir les rapports de paquets.
...
Point de terminaison modérateur/admin pour l’examen d’une version de paquet.
...
- Utilise par défaut la dernière version.
- Utilise le quota de débit de lecture, pas le quota de téléchargement.
...
- Cette route reste limitée au ZIP. Elle ne diffuse pas de fichiers ClawPack `.tgz`.
...
- Les packuments de paquets à scope prennent en charge à la fois `/api/npm/@scope/name` et le chemin de requête
  encodé npm `/api/npm/@scope%2Fname`.

### `GET /api/npm/{package}/-/{tarball}.tgz`

Diffuse les octets exacts du tarball ClawPack téléversé pour les clients miroir npm.
...
skill actuelle basée sur GitHub avec une analyse `clean` ou `suspicious` et aucune
version hébergée.
...
- Les transferts de skills basées sur GitHub ne proxyfient ni ne reflètent les octets. La réponse JSON
  inclut `sourceRef: "public-github"`, `repo`, `commit`, `path`, `contentHash`
  et `archiveUrl` ; l’état d’analyse/actuel est une barrière et n’est pas inclus comme métadonnée de charge utile
  de succès.
...
- Champ facultatif de charge utile : `ownerHandle`. Lorsqu’il est présent, l’API résout cet
  éditeur côté serveur et exige que l’acteur dispose de l’accès éditeur.
- Champ facultatif de charge utile : `migrateOwner`. Lorsque `true` avec `ownerHandle`, une
...
- Les champs de formulaire autorisés sont `payload`, les blobs `files` répétés, ou une référence de tarball
  `clawpack`. `clawpack` peut être un blob `.tgz` ou un identifiant de stockage renvoyé par
  le flux upload-url. Les publications basées sur un storage-id préparé doivent aussi inclure le
...
Supprime logiquement / restaure une skill (propriétaire, modérateur ou admin).
...
Les suppressions logiques lancées par le propriétaire réservent le slug pendant 30 jours, après quoi le slug peut être revendiqué par
un autre éditeur. La réponse de suppression inclut `slugReservedUntil` lorsque cette expiration s’applique.
Les masquages par modérateur/admin et les retraits de sécurité n’expirent pas de cette manière.
...
Admin uniquement. Garantit qu’un éditeur d’organisation existe pour un handle. Si le handle pointe encore vers un
ancien éditeur partagé utilisateur/personnel, le point de terminaison le migre d’abord vers un éditeur d’organisation.
Pour une organisation nouvellement créée, fournissez `memberHandle` ; l’admin acteur n’est pas ajouté comme membre.
...
Admin uniquement. Réserve des slugs racine et des noms de paquets pour un propriétaire légitime sans publier de
version. Les noms de paquets deviennent des paquets indicateurs privés sans lignes de version, afin que le même
...
Admin uniquement. Récupère un éditeur personnel pour un principal GitHub OAuth de remplacement vérifié
...
Le point de terminaison utilise l’exécution à blanc par défaut. Appliquer la récupération nécessite `dryRun: false` et
`confirmIdentityVerified: true` après que le personnel a vérifié indépendamment la continuité entre les deux
identités GitHub principales. La récupération échoue en mode fermé lorsque le publisher personnel actuel de l’utilisateur de destination
possède des compétences, des packages ou des sources de compétences GitHub.
La récupération migre également les anciens champs `ownerUserId` pour les compétences du publisher récupéré,
les alias de slug de compétence, les packages, les avertissements de l’inspecteur de packages et les lignes dérivées du condensé de recherche, afin que
...
Les sources de compétences GitHub sont liées au publisher et signalées comme vérifiées plutôt que réécrites.
...
- Les deux points de terminaison exigent une authentification par jeton d’API et fonctionnent uniquement pour le propriétaire de la compétence.
...
Bannir un utilisateur et supprimer définitivement les compétences qu’il possède (modérateur/admin uniquement).
...
Débannir un utilisateur et restaurer les compétences éligibles (admin uniquement).
...
Modifier la raison enregistrée pour un bannissement existant sans débannir ni restaurer
le contenu (admin uniquement). Utilise l’exécution à blanc par défaut, sauf si `dryRun` vaut `false`.
...
## Anciens points de terminaison CLI (obsolètes)
