...
x-i18n:
    generated_at: "2026-07-04T03:44:06Z"
    model: gpt-5.5
...
Les répertoires tiers peuvent utiliser les points de terminaison publics en lecture pour lister ou rechercher des skills ClawHub. Veuillez mettre les résultats en cache, respecter `429`/`Retry-After`, renvoyer les utilisateurs vers la fiche ClawHub canonique (`https://clawhub.ai/<owner>/skills/<slug>`) et éviter de suggérer une approbation de ClawHub pour le site tiers. Ne tentez pas de dupliquer du contenu masqué, privé ou bloqué par modération en dehors de la surface de l’API publique.
...
- ClawHub utilise les en-têtes de transfert approuvés pour identifier les IP clientes en périphérie.
- Si aucune IP cliente approuvée n’est disponible, les requêtes anonymes utilisent des compartiments de secours
  limités uniquement au type de limite de débit. Ces compartiments de secours n’incluent pas
...
- Les résultats sont renvoyés par ordre de pertinence (similarité d’embeddings + boosts de jetons exacts de slug/nom + léger a priori de popularité).
- La pertinence est plus forte que la popularité. Une correspondance précise de slug ou de jeton de nom d’affichage peut devancer une correspondance plus approximative avec un engagement beaucoup plus fort.
- Le texte ASCII est découpé en jetons aux limites des mots et de la ponctuation. Par exemple, `personal-map` contient un jeton autonome `map`, tandis que `amap-jsapi-skill` contient `amap`, `jsapi` et `skill` ; rechercher `map` donne donc à `personal-map` une correspondance lexicale plus forte qu’à `amap-jsapi-skill`.
...
- Les alias de renommage préservent la résolution des anciennes URL et installations qui passent par le registre, mais le classement de recherche repose sur les métadonnées canoniques de la skill après l’indexation du renommage. Les statistiques existantes restent associées à la skill.
...
Signaler une skill pour examen par un modérateur. Les signalements sont au niveau de la skill, liés facultativement
...
Les analyses d’import local ne sont plus prises en charge. Les requêtes utilisant
...
- Renvoie `queue.queuedAhead` et `queue.position` pendant la mise en file d’attente afin que les clients puissent afficher combien d’analyses manuelles prioritaires précèdent la requête. Les files très volumineuses sont bornées et signalées avec `queuedAheadIsEstimate: true`.
...
- `provenance` vaut `server-resolved-github-import` uniquement lorsque ClawHub a résolu et stocké un dépôt/réf/commit/chemin GitHub pendant la publication ou l’import ; sinon, il vaut `unavailable`.
...
- `security.signals` contient uniquement des preuves à l’appui au niveau de l’état ; utilisez `/scan` ou la page d’audit de sécurité ClawHub pour obtenir les détails complets des scanners.
- `security.signals.dependencyRegistry` est conservé pour la compatibilité de réponse v1, mais le scanner d’existence du registre de dépendances est retiré et cette clé vaut toujours `null`.
...
- `category` (facultatif) : filtre de catégorie de plugin. Pris en charge uniquement lorsque la
  ...
  `/api/v1/code-plugins`, `/api/v1/bundle-plugins` ou points de terminaison de paquets avec
...
Recherche de catalogue unifiée sur les Skills et les paquets de plugins.
...
Parcours du catalogue réservé aux plugins, sur les paquets de plugins de code et de plugins groupés.
...
Exportation groupée des dernières Skills publiques pour analyse hors ligne.
...
Export groupé des dernières versions publiques de Plugins pour l’analyse hors ligne.
...
- Les alias de filtre historiques v1 documentés sous `GET /api/v1/plugins` sont aussi
  acceptés.
...
d’installation. C’est la surface publique de consommation OpenClaw pour décider si une
...
- Ce point de terminaison est volontairement plus étroit que les points de terminaison de modération
  de propriétaire/modérateur. Il expose la décision d’installation et l’explication publique, pas
  les identités des auteurs de signalement, les corps de signalement, les preuves privées ni les chronologies
  de revue internes.
...
- Les versions de package historiques renvoient un artefact `legacy-zip` et une
  `downloadUrl` ZIP historique.
- Les versions ClawPack renvoient un artefact `npm-pack`, des champs d’intégrité npm, une
  `tarballUrl` et l’URL de compatibilité ZIP historique.
...
- Les versions ZIP historiques redirigent vers `/api/v1/packages/{name}/download?version=`.
- Utilise le compartiment de débit de téléchargement.

### `GET /api/v1/packages/{name}/readiness`

Renvoie l’état de préparation calculé pour la consommation future par OpenClaw.
...
Point de terminaison modérateur/administrateur pour les files de revue de versions de packages.
...
Signaler un package pour revue par un modérateur. Les signalements se situent au niveau du package, et sont éventuellement
...
Point de terminaison modérateur/admin pour l’ingestion des signalements de paquets.
...
Point de terminaison modérateur/admin pour l’examen des versions de paquet.
...
- Cette route reste limitée au format ZIP. Elle ne diffuse pas de fichiers ClawPack `.tgz`.
...
- Seules les versions avec des archives tarball npm-pack ClawPack téléversées sont listées.
...
Télécharge un ZIP de version de compétence hébergée, ou renvoie un transfert de source GitHub pour une
compétence actuelle adossée à GitHub avec une analyse `clean` ou `suspicious` et sans version
hébergée.
...
- Les transferts de compétences adossées à GitHub ne proxifient ni ne répliquent les octets. La réponse JSON
...
- Champ de charge utile facultatif : `migrateOwner`. Lorsque défini à `true` avec `ownerHandle`, une
  compétence existante peut être déplacée vers ce propriétaire si l’acteur est admin/propriétaire sur les deux
  éditeurs, l’actuel et la cible. Sans cette option explicite, les changements de propriétaire sont
  rejetés.

### `POST /api/v1/packages`

Publie une version de Plugin de code ou de Plugin de bundle.

- Nécessite une authentification par jeton Bearer.
- Nécessite `multipart/form-data`.
- Les champs de formulaire autorisés sont `payload`, des blobs `files` répétés, ou une référence d’archive tar
  `clawpack`. `clawpack` peut être un blob `.tgz` ou un id de stockage renvoyé par
  le flux upload-url. Les publications par id de stockage préparé doivent également inclure le
  `clawpackUploadTicket` renvoyé avec cette URL d’envoi.
...
- Les requêtes de publication multipart directes sont plafonnées à 18 Mo. Les archives tar ClawPack peuvent
  utiliser le flux upload-url jusqu’à la limite d’archive tar de 120 Mo.
...
- Les paquets de Plugin nécessitent `openclaw.plugin.json`. Les envois `.tgz` ClawPack doivent
...
Supprime de façon réversible / restaure une compétence (propriétaire, modérateur ou admin).
...
Réservé aux admins. Réserve les slugs racine et les noms de paquet pour un propriétaire légitime sans publier de
version. Les noms de paquet deviennent des paquets d’espace réservé privés sans lignes de version, afin que le même
propriétaire puisse publier ultérieurement la véritable version de Plugin de code ou de Plugin de bundle sous ce nom.
...
immuables ; les handles mutables ne sont utilisés que comme garde destiné à l’opérateur.

Le point de terminaison utilise le mode dry-run par défaut. L’application de la récupération nécessite `dryRun: false` et
...
possède des compétences, des packages ou des sources de Skills GitHub.
La récupération migre aussi les anciens champs `ownerUserId` pour les compétences du publisher récupéré,
...
les chemins de propriétaire direct correspondent à la nouvelle autorité du publisher. Une réservation active de handle protégé
pour le handle récupéré est également réaffectée à l’utilisateur de remplacement afin qu’une synchronisation ultérieure
...
Les sources de Skills GitHub sont liées au publisher et signalées comme vérifiées plutôt que réécrites.
