---
read_when:
    - Ajout/modification de endpoints
    - Déboguer les requêtes CLI ↔ registre
summary: Référence de l’API HTTP (points de terminaison publics + CLI + authentification).
x-i18n:
    generated_at: "2026-07-01T15:22:36Z"
    model: gpt-5.5
...
Les répertoires tiers peuvent utiliser les points de terminaison publics en lecture pour lister ou rechercher des Skills ClawHub. Veuillez mettre les résultats en cache, respecter `429`/`Retry-After`, renvoyer les utilisateurs vers la fiche ClawHub canonique (`https://clawhub.ai/<owner>/skills/<slug>`) et éviter de suggérer que ClawHub approuve le site tiers. N’essayez pas de répliquer du contenu masqué, privé ou bloqué par la modération en dehors de la surface de l’API publique.

Les raccourcis de slugs web se résolvent entre familles de registre, mais les clients API doivent utiliser
...
- Les points de terminaison d’écriture authentifiés ne doivent pas renvoyer simplement `Unauthorized` lorsque
...
- Les résultats sont renvoyés par ordre de pertinence (similarité d’embedding + renforcements exacts sur les jetons de slug/nom + un léger a priori de popularité).
- La pertinence prime sur la popularité. Une correspondance précise de slug ou de jeton de nom d’affichage peut dépasser une correspondance plus approximative avec un engagement beaucoup plus fort.
- Le texte ASCII est découpé en jetons aux limites de mots et de ponctuation. Par exemple, `personal-map` contient un jeton autonome `map`, tandis que `amap-jsapi-skill` contient `amap`, `jsapi` et `skill` ; rechercher `map` donne donc à `personal-map` une correspondance lexicale plus forte que `amap-jsapi-skill`.
...
Recommandations pour la découvrabilité des éditeurs :

- Placez les termes que les utilisateurs chercheront littéralement dans le nom d’affichage, le résumé et les tags. N’utilisez un jeton de slug autonome que lorsqu’il s’agit aussi d’une identité stable que vous souhaitez conserver.
...
- Les alias de renommage préservent la résolution pour les anciennes URL et installations qui passent par le registre, mais le classement de recherche est basé sur les métadonnées canoniques du Skill une fois le renommage indexé. Les statistiques existantes restent associées au Skill.
...
- Les anciens slugs créés par des flux de renommage/fusion de propriétaire se résolvent vers le Skill canonique.
...
- Les propriétaires et les modérateurs peuvent accéder aux détails de modération pour les Skills masqués.
...
- `version.security` inclut l’état normalisé de vérification de l’analyse et les détails du scanner
...
- Les charges utiles des demandes d’analyse et les rapports téléchargeables expirent du magasin des demandes d’analyse après la fenêtre de conservation.
...
- Les tâches d’analyse sont asynchrones. Les demandes d’analyse manuelles sont prioritaires par rapport au travail normal de publication/remplissage rétroactif, mais l’achèvement dépend toujours de la disponibilité des workers.

### `GET /api/v1/skills/-/scan/{scanId}`

Point de terminaison de polling authentifié pour une analyse soumise.

- Renvoie le statut queued/running/succeeded/failed.
- Renvoie `queue.queuedAhead` et `queue.position` pendant la mise en file afin que les clients puissent afficher combien d’analyses manuelles prioritaires précèdent la demande. Les très grandes files sont bornées et signalées avec `queuedAheadIsEstimate: true`.
...
Route canonique de statut de lot réservée aux administrateurs. Elle accepte `{ "jobIds": ["..."] }` et renvoie les mêmes compteurs agrégés que l’ancien `POST /api/v1/skills/-/rescan-batch/status`.
...
- L’identité de la skill, l’identité de l’éditeur et les métadonnées de la version sélectionnée sont des champs d’enveloppe de premier niveau (`slug`, `displayName`, `publisherHandle`, `version`, `resolvedFrom`, `tag`, `createdAt`) afin que l’automatisation shell puisse les lire sans décompresser des wrappers imbriqués.
...
- `security.signals` contient uniquement des preuves complémentaires au niveau du statut ; utilisez `/scan` ou la page d’audit de sécurité ClawHub pour les détails complets des scanners.
- `security.signals.dependencyRegistry` est conservé pour la compatibilité des réponses v1, mais le scanner d’existence du registre de dépendances est retiré et cette clé vaut toujours `null`.
...
- `category` (facultatif) : filtre de catégorie de Plugin. Pris en charge uniquement lorsque la
  ...
  alias de filtre hérités de la v1 sont documentés sous `GET /api/v1/plugins`.
...
- `category` (facultatif) : filtre de catégorie de Plugin. Pris en charge uniquement lorsque la
  ...
  de filtre hérités de la v1 sont documentés sous `GET /api/v1/plugins`.
...
- Les alias de filtres v1 hérités documentés sous `GET /api/v1/plugins` sont également
  acceptés.
- Le filtrage par catégorie est un vrai filtre d’API adossé aux lignes de condensé de catégorie
...
- Les contrôles de tri de l’interface de navigateur pour la recherche de Plugins réordonnent les résultats de pertinence chargés,
  conformément au comportement de navigation actuel de `/skills`.
...
- Requiert un jeton d’API pour le propriétaire du paquet, un propriétaire/administrateur d’éditeur d’organisation,
...
- `trust.scanStatus` est le statut de confiance effectif dérivé des entrées des analyseurs
  et de la modération manuelle de version.
...
- Ce point de terminaison est exact à la version. Les clients doivent l’appeler après avoir résolu la
  version de paquet qu’ils comptent installer, pas seulement après avoir lu les dernières
...
- Ce point de terminaison est volontairement plus restreint que les points de terminaison de modération
  ...
  l’identité des rapporteurs, les corps de signalement, les preuves privées ni les chronologies de revue
  internes.
...
- Les versions de paquet héritées renvoient un artéfact `legacy-zip` et une URL ZIP héritée
  `downloadUrl`.
...
Renvoie la préparation calculée pour la consommation future par OpenClaw.
...
- condensé d’artéfact
- dépôt source et provenance de commit
...
- `packageName` est normalisé comme nom npm ; le paquet peut être absent pour les migrations
  planifiées.
- Cela suit uniquement la préparation de migration. Cela ne modifie pas OpenClaw et ne génère pas
  de ClawPacks.

### `GET /api/v1/packages/moderation/queue`

Point de terminaison modérateur/administrateur pour les files de revue des versions de paquet.
...
- `all` : toute version avec une dérogation manuelle, un état d’analyse non propre ou un signalement de paquet.
...
Signaler un paquet pour revue par un modérateur. Les signalements se font au niveau du paquet, éventuellement
...
`finalAction: "revoke"` avec un signalement confirmé pour appliquer la modération de release dans le
...
Point de terminaison modérateur/admin pour l’examen des releases de packages.
...
Télécharge le ZIP d’une version de skill hébergée, ou renvoie un transfert vers la source GitHub pour un
...
- Les transferts de skills adossés à GitHub ne proxifient ni ne mettent en miroir les octets. La réponse JSON
  ...
  et `archiveUrl` ; l’état d’analyse/actuel sert de garde et n’est pas inclus comme métadonnée de payload
  de réussite.
...
- Champ de payload facultatif : `migrateOwner`. Lorsque `true` avec `ownerHandle`, un
  skill existant peut être déplacé vers ce propriétaire si l’acteur est admin/propriétaire à la fois sur les
...
- Les champs de formulaire autorisés sont `payload`, des blobs `files` répétés, ou une référence de tarball
...
- Champ de payload facultatif : `ownerHandle`. Lorsqu’il est présent, seuls les admins peuvent publier pour le compte de ce propriétaire.
...
- Les code plugins nécessitent `package.json`, les métadonnées du dépôt source, les métadonnées du commit
...
Supprime de façon réversible / restaure un skill (propriétaire, modérateur ou admin).
...
Réservé aux admins. Réserve des slugs racine et des noms de package pour un propriétaire légitime sans publier de
release. Les noms de package deviennent des packages d’espace réservé privés sans lignes de release, afin que le même
propriétaire puisse ensuite publier la vraie release code-plugin ou bundle-plugin dans ce nom.
...
Le point de terminaison utilise par défaut l’exécution à blanc. L’application de la récupération exige `dryRun: false` et
...
possède des Skills, des packages ou des sources GitHub de Skills.
...
pour le handle récupéré est également réattribuée à l’utilisateur de remplacement afin qu’une synchronisation ultérieure du
profil ne puisse pas restaurer l’autorité concurrente de l’ancien utilisateur. Chaque table principale est limitée à
...
Les sources GitHub de Skills sont limitées à l’éditeur et signalées comme vérifiées plutôt que réécrites.
...
le contenu (administrateur uniquement). Utilise par défaut l’exécution à blanc, sauf si `dryRun` vaut `false`.
...
qui préparent une archive tar ClawPack doivent envoyer l’identifiant de stockage résultant sous
