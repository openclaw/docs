...
x-i18n:
    generated_at: "2026-06-30T22:13:34Z"
    model: gpt-5.5
...
Les anciens chemins `/api/...` et `/api/cli/...` restent pour compatibilité (voir `DEPRECATIONS.md`).
OpenAPI : `/api/v1/openapi.json`.

## Réutilisation du catalogue public

Les annuaires tiers peuvent utiliser les points de terminaison publics en lecture pour lister ou rechercher des skills ClawHub. Veuillez mettre les résultats en cache, respecter `429`/`Retry-After`, rediriger les utilisateurs vers la fiche ClawHub canonique (`https://clawhub.ai/<owner>/skills/<slug>`) et éviter de laisser entendre que ClawHub approuve le site tiers. N’essayez pas de répliquer du contenu masqué, privé ou bloqué par la modération en dehors de la surface de l’API publique.
...
- Requêtes authentifiées (jeton Bearer valide) : appliquées par bucket utilisateur.
...
- `X-RateLimit-Remaining` / `RateLimit-Remaining` : budget restant exact lorsqu’il est présent.
  Les requêtes réussies shardées omettent cet en-tête au lieu de renvoyer une valeur globale approximative.
...
- Si aucune IP client de confiance n’est disponible, les requêtes anonymes utilisent des buckets de repli
  limités uniquement par type de limite de débit. Ces buckets de repli n’incluent pas
...
- Les résultats sont renvoyés par ordre de pertinence (similarité d’embedding + boosts de correspondance exacte sur les tokens de slug/nom + un faible a priori de popularité).
- La pertinence est plus forte que la popularité. Une correspondance précise de slug ou de token de nom affiché peut dépasser une correspondance plus large avec un engagement beaucoup plus élevé.
- Le texte ASCII est tokenisé sur les limites de mots et de ponctuation. Par exemple, `personal-map` contient un token autonome `map`, tandis que `amap-jsapi-skill` contient `amap`, `jsapi` et `skill` ; rechercher `map` donne donc à `personal-map` une correspondance lexicale plus forte qu’à `amap-jsapi-skill`.
...
- Placez les termes que les utilisateurs rechercheront littéralement dans le nom affiché, le résumé et les tags. N’utilisez un token de slug autonome que lorsqu’il s’agit aussi d’une identité stable que vous voulez conserver.
- Ne renommez pas un slug seulement pour viser une requête, sauf si le nouveau slug est un meilleur nom canonique à long terme. Les anciens slugs deviennent des alias de redirection, mais l’URL canonique, le slug affiché et les futurs condensés de recherche utilisent le nouveau slug.
- Les alias de renommage préservent la résolution des anciennes URL et des installations qui se résolvent via le registre, mais le classement de recherche est basé sur les métadonnées canoniques de la skill une fois le renommage indexé. Les statistiques existantes restent associées à la skill.
...
- `metadata.os` : restrictions d’OS déclarées dans le frontmatter de la skill (par ex. `["macos"]`, `["linux"]`). `null` si non déclaré.
...
Point de terminaison d’envoi authentifié pour les nouvelles tâches ClawScan.

Les analyses d’envoi local ne sont plus prises en charge. Les requêtes utilisant
...
- Les tâches d’analyse sont asynchrones. Les requêtes d’analyse manuelle sont prioritaires par rapport aux travaux normaux de publication/remplissage, mais leur achèvement dépend toujours de la disponibilité des workers.
...
- `kind` vaut `skill` par défaut ; utilisez `kind=plugin` pour les analyses de plugin/package.
- Renvoie la même forme ZIP que les téléchargements de requêtes d’analyse.

### `POST /api/v1/skills/-/scan/batch`

Route canonique de réanalyse par lot réservée aux administrateurs. Elle accepte la même forme de charge utile que l’ancienne `POST /api/v1/skills/-/rescan-batch`.

### `POST /api/v1/skills/-/scan/batch/status`

Route canonique d’état de lot réservée aux administrateurs. Elle accepte `{ "jobIds": ["..."] }` et renvoie les mêmes compteurs agrégés que l’ancienne `POST /api/v1/skills/-/rescan-batch/status`.
...
- `version` (facultatif) : chaîne de version spécifique.
- `tag` (facultatif) : résoudre une version balisée (par exemple `latest`).
...
- La réponse concerne uniquement la sécurité. Elle n’inclut pas les données de Skill Card, l’état de carte générée, les listes de fichiers d’artéfacts ni les charges utiles détaillées des scanners.
- `security.signals` contient uniquement des preuves d’appui au niveau de l’état ; utilisez `/scan` ou la page d’audit de sécurité ClawHub pour obtenir les détails complets des scanners.
- `security.signals.dependencyRegistry` est conservé pour la compatibilité de réponse v1, mais le scanner d’existence du registre de dépendances est retiré et cette clé vaut toujours `null`.
- L’absence de Skill Card n’affecte pas les champs `ok`, `decision` ou `reasons` de ce point de terminaison ; les clients doivent lire le fichier `skill-card.md` installé localement lorsqu’ils ont besoin du contenu de la carte.
- Utilisez `/verify` lorsque vous avez besoin de l’enveloppe de vérification Skill Card pour une seule skill, `/card` lorsque vous avez besoin du Markdown de carte généré, et `/scan` lorsque vous avez besoin des données détaillées des scanners.
...
- `category` (facultatif) : filtre de catégorie de plugin. Pris en charge uniquement lorsque la
  ...
  `/api/v1/code-plugins`, `/api/v1/bundle-plugins` ou les points de terminaison de packages avec
  `family=code-plugin`/`family=bundle-plugin`). Les catégories contrôlées et
  les alias de filtres hérités de v1 sont documentés sous `GET /api/v1/plugins`.
...
Recherche de catalogue unifiée parmi les skills et les packages de plugins.
...
- `category` (facultatif) : filtre de catégorie de plugin. Pris en charge uniquement lorsque la
  ...
  de filtres hérités de v1 sont documentés sous `GET /api/v1/plugins`.
...
Les alias de filtres hérités de v1 restent acceptés sur les points de terminaison de lecture :
...
Export en masse des derniers skills publics pour analyse hors ligne.
...
- `startDate` (obligatoire) : limite inférieure en millisecondes Unix pour le champ `updatedAt` du skill.
- `endDate` (obligatoire) : limite supérieure en millisecondes Unix pour le champ `updatedAt` du skill.
...
- Les alias de filtres v1 hérités documentés sous `GET /api/v1/plugins` sont également
  acceptés.
- Le filtrage par catégorie est un vrai filtre d’API adossé aux lignes de condensé de catégorie de Plugin,
...
- Requiert un jeton d’API pour le propriétaire du package, un propriétaire/administrateur d’éditeur d’organisation,
...
- Ce point de terminaison est exact par version. Les clients doivent l’appeler après avoir résolu la
  version du package qu’ils prévoient d’installer, pas seulement après avoir lu les dernières
...
liés à une version. Ils alimentent la file de modération, mais ne masquent ni ne
bloquent automatiquement les téléchargements par eux-mêmes ; les modérateurs doivent utiliser la modération de version pour
...
Point de terminaison modérateur/administrateur pour la réception des signalements de paquets.
...
- Cette route reste limitée aux fichiers ZIP. Elle ne diffuse pas les fichiers ClawPack `.tgz`.
...
Télécharge un ZIP de version de skill hébergée, ou renvoie un transfert vers la source GitHub pour un
...
- `tag` (facultatif) : nom d’étiquette (par exemple `latest`)
...
- Les transferts de skills adossés à GitHub ne proxifient ni ne reflètent les octets. La réponse JSON
  inclut `sourceRef: "public-github"`, `repo`, `commit`, `path`, `contentHash`
  et `archiveUrl` ; l’état d’analyse/actuel sert de garde et n’est pas inclus comme métadonnée de charge utile de réussite.
- Les statistiques de téléchargement sont comptées comme des identités uniques par jour UTC (`userId` lorsque le jeton API est valide, sinon IP).
...
- Champ de charge utile facultatif : `migrateOwner`. Lorsqu’il vaut `true` avec `ownerHandle`, un
  skill existant peut être déplacé vers ce propriétaire si l’acteur est administrateur/propriétaire à la fois sur
  l’éditeur actuel et sur l’éditeur cible. Sans cet accord explicite, les changements de propriétaire sont
  rejetés.
...
- Les requêtes de publication multipart directes sont plafonnées à 18 Mo. Les archives tar ClawPack peuvent
  utiliser le flux upload-url jusqu’au plafond de 120 Mo pour les archives tar.
...
- Les paquets de Plugin nécessitent `openclaw.plugin.json`. Les téléversements `.tgz` ClawPack doivent
...
Supprimer de manière réversible / restaurer un skill (propriétaire, modérateur ou administrateur).
...
utilisateur partagé/éditeur personnel hérité, le point de terminaison le migre d’abord vers un éditeur d’organisation.
...
sans modifier les lignes de compte Convex Auth. La requête doit nommer les deux identifiants de compte de fournisseur GitHub
immuables ; les handles modifiables ne sont utilisés que comme garde destiné à l’opérateur.

Par défaut, l’endpoint fonctionne en simulation. L’application de la récupération nécessite `dryRun: false` et
`confirmIdentityVerified: true` après que le personnel a vérifié indépendamment la continuité entre les deux
identités GitHub principales. La récupération échoue en mode fermé lorsque l’éditeur personnel actuel de l’utilisateur de destination
...
les alias de slugs de skills, les packages, les avertissements de l’inspecteur de packages et les lignes de condensé de recherche dérivées afin que
...
- Corps : `{ "handle": "gingiris", "nextUserHandle": "gingiris-1031", "previousGitHubProviderAccountId": "123", "nextGitHubProviderAccountId": "456", "reason": "Continuité du compte vérifiée pour l’issue #2555", "confirmIdentityVerified": true, "dryRun": false }`
- Réponse : `{ "ok": true, "dryRun": false, "recovered": true, "publisherId": "...", "handle": "gingiris", "previousUser": { "userId": "...", "handle": "gingiris", "nextHandle": "gingiris-recovered", "githubProviderAccountId": "123", "authAccountCount": 1 }, "nextUser": { "userId": "...", "handle": "gingiris-1031", "nextHandle": "gingiris", "githubProviderAccountId": "456", "authAccountCount": 1 }, "retiredPersonalPublisher": null, "resourceOwnerMigration": { "limitPerTable": 100, "skills": 1, "skillSlugAliases": 1, "packages": 0, "packageInspectorWarnings": 0, "githubSourcesChecked": 1, "handleReservations": 1 }, "identityVerified": true, "reason": "Continuité du compte vérifiée pour l’issue #2555" }`
...
- Les deux endpoints nécessitent une authentification par jeton d’API et ne fonctionnent que pour le propriétaire de la skill.
...
- `POST /api/v1/skills/{slug}/transfer`
  - Corps : `{ "toUserHandle": "target_handle", "message": "facultatif" }`
...
Ajouter/retirer une étoile (mise en avant). Les deux endpoints sont idempotents.
