...
x-i18n:
    generated_at: "2026-06-27T17:14:54Z"
    model: gpt-5.5
...
Les répertoires tiers peuvent utiliser les endpoints de lecture publics pour lister ou rechercher des Skills ClawHub. Veuillez mettre les résultats en cache, respecter `429`/`Retry-After`, renvoyer les utilisateurs vers la fiche ClawHub canonique (`https://clawhub.ai/<owner>/skills/<slug>`) et éviter de suggérer une approbation de ClawHub pour le site tiers. N’essayez pas de dupliquer du contenu masqué, privé ou bloqué par la modération en dehors de la surface de l’API publique.

Les raccourcis de slug web sont résolus entre familles de registre, mais les clients API doivent utiliser
...
- Requêtes authentifiées (jeton Bearer valide) : appliqué par bucket utilisateur.
- Si le jeton est absent/invalide, le comportement retombe sur l’application par IP.
- Les endpoints d’écriture authentifiés ne doivent pas renvoyer un simple `Unauthorized` lorsque
  ...
  les comptes supprimés/bannis/désactivés doivent chacun recevoir un texte actionnable afin que les clients
...
- Si `Retry-After` est absent, retombez sur `RateLimit-Reset` (ou calculez depuis `X-RateLimit-Reset`).
...
- ClawHub utilise des en-têtes de transfert approuvés pour identifier les IP des clients à la périphérie.
- Si aucune IP client approuvée n’est disponible, les requêtes anonymes utilisent des buckets de secours
  limités uniquement par type de limite de débit. Ces buckets de secours n’incluent pas
...
- Les résultats sont renvoyés par ordre de pertinence (similarité d’embedding + boosts exacts sur les jetons de slug/nom + un faible prior de popularité).
- La pertinence prime sur la popularité. Une correspondance précise de slug ou de jeton de nom d’affichage peut dépasser une correspondance plus lâche avec un engagement bien plus fort.
...
- Les alias de renommage préservent la résolution pour les anciennes URL et les installations qui passent par le registre, mais le classement de recherche est basé sur les métadonnées canoniques du Skill après l’indexation du renommage. Les statistiques existantes restent associées au Skill.
...
- `trending` classe par installations sur les 7 derniers jours (basé sur la télémétrie).
...
- `metadata.os` : restrictions de système d’exploitation déclarées dans le frontmatter du Skill (par ex. `["macos"]`, `["linux"]`). `null` si non déclaré.
- `metadata.systems` : cibles système Nix (par ex. `["aarch64-darwin", "x86_64-linux"]`). `null` si non déclaré.
- `metadata` vaut `null` si le Skill n’a aucune métadonnée de plateforme.
...
Signaler un Skill pour examen par les modérateurs. Les signalements se font au niveau du Skill, peuvent être liés
...
Endpoint modérateur/admin pour l’admission des signalements de Skills.
...
- Lors de la requête d’une version historique, vérifiez `moderation.matchesRequestedVersion` et `moderation.sourceVersion` avant de traiter `moderation` et `security` comme relevant du même contexte de version.
...
- Les charges utiles de requête d’analyse et les rapports téléchargeables expirent du magasin de requêtes d’analyse après la fenêtre de rétention.
...
- Les analyses publiées réécrivent les données uniquement lorsque `update: true` et que l’analyse se termine avec succès.
...
Point de terminaison d’interrogation authentifié pour une analyse envoyée.

- Renvoie l’état en file d’attente/en cours/réussi/échoué.
- Renvoie `queue.queuedAhead` et `queue.position` pendant la mise en file afin que les clients puissent afficher le nombre d’analyses manuelles prioritaires qui précèdent la requête. Les files très volumineuses sont plafonnées et signalées avec `queuedAheadIsEstimate: true`.
...
Point de terminaison authentifié pour l’archive de rapport.
...
Point de terminaison authentifié pour l’archive de rapport stockée des versions envoyées.
...
- La réponse concerne uniquement la sécurité. Elle n’inclut pas les données Skill Card, l’état de carte générée, les listes de fichiers d’artefact ni les charges utiles détaillées des scanners.
...
- Utilisez `/verify` lorsque vous avez besoin de l’enveloppe de vérification Skill Card d’une seule skill, `/card` lorsque vous avez besoin du Markdown de carte généré, et `/scan` lorsque vous avez besoin des données détaillées des scanners.
...
Recherche unifiée dans le catalogue, couvrant les Skills et les packages de plugins.
...
- `category` (facultatif) : filtre de catégorie de plugin. Pris en charge uniquement lorsque la
  ...
  filtres hérités v1 sont documentés sous `GET /api/v1/plugins`.
...
- `startDate` (obligatoire) : borne inférieure en millisecondes Unix pour le `updatedAt` du Skill.
- `endDate` (obligatoire) : borne supérieure en millisecondes Unix pour le `updatedAt` du Skill.
...
- `limit` (facultatif) : entier (1-250), valeur par défaut `250`.
- `cursor` (facultatif) : curseur de pagination issu de la réponse précédente.
- `family` (facultatif) : `code-plugin` ou `bundle-plugin`. L’absence de valeur signifie les deux
  familles de plugins.
...
- Les Skills peuvent également être résolus par cette route dans le catalogue unifié.
...
- Nécessite un jeton API du propriétaire du package, d’un propriétaire/administrateur de l’organisation éditrice,
...
- `version.artifact.kind` vaut `legacy-zip` pour les archives de packages de l’ancien monde ou
  `npm-pack` pour les versions fondées sur ClawPack.
...
- `trust.blockedFromDownload` est le signal de blocage de l’installation. OpenClaw et les autres
  ...
  redériver les règles de blocage à partir des champs d’analyse ou de modération.
...
Signaler un package pour révision par un modérateur. Les signalements sont au niveau du package et peuvent être
liés facultativement à une version. Ils alimentent la file de modération mais ne masquent pas automatiquement ni ne
...
Point de terminaison modérateur/admin pour l’ingestion des rapports de package.
...
Point de terminaison propriétaire/modérateur pour la visibilité de modération des packages.
...
`note` est requis pour `confirmed` et `dismissed` ; il peut être omis lors du
retour de `status` à `open`. Transmettez `finalAction: "quarantine"` ou
...
Les releases mises en quarantaine et révoquées renvoient `403` depuis les routes de téléchargement d’artefacts.
...
skill actuelle adossée à GitHub avec une analyse `clean` ou `suspicious` et sans
version hébergée.
...
- Les transferts de skill adossée à GitHub ne proxient ni ne mettent en miroir les octets. La réponse JSON
  inclut `sourceRef: "public-github"`, `repo`, `commit`, `path`, `contentHash`
  et `archiveUrl` ; l’analyse/l’état courant sert de garde et n’est pas inclus comme métadonnée
...
- Champ facultatif de payload : `migrateOwner`. Lorsque `true` avec `ownerHandle`, une
  skill existante peut passer à ce propriétaire si l’acteur est admin/propriétaire à la fois sur
  l’éditeur actuel et sur l’éditeur cible. Sans cette option explicite, les changements de propriétaire sont
  rejetés.
...
- Champ facultatif de payload : `ownerHandle`. Lorsqu’il est présent, seuls les admins peuvent publier au nom de ce propriétaire.
...
release. Les noms de package deviennent des packages réservés privés sans lignes de release, afin que le même
propriétaire puisse ensuite publier la vraie release code-plugin ou bundle-plugin sous ce nom.
...
fournisseur GitHub ; les handles modifiables ne sont utilisés que comme garde destiné à l’opérateur.

Le point de terminaison utilise la simulation par défaut. L’application de la récupération nécessite `dryRun: false` et
...
La récupération migre aussi les anciens champs `ownerUserId` pour les compétences de l’éditeur récupéré,
les alias de slug de compétence, les packages, les avertissements de l’inspecteur de package et les lignes de condensé de recherche dérivées, afin que
...
pour le handle récupéré est aussi réattribuée à l’utilisateur de remplacement afin que la synchronisation ultérieure
...
Les sources de compétences GitHub sont limitées à l’éditeur et signalées comme vérifiées plutôt que réécrites.
...
- Les deux points de terminaison nécessitent une authentification par jeton d’API et ne fonctionnent que pour le propriétaire de la compétence.
...
- `GET /api/v1/transfers/outgoing`
  - Forme de réponse : `{ "transfers": [{ "_id": "...", "skill": { "slug": "demo", "displayName": "Demo" }, "fromUser"|"toUser": { "handle": "..." }, "message": "...", "requestedAt": 0, "expiresAt": 0 }] }`

### `POST /api/v1/users/ban`

Bannir un utilisateur et supprimer définitivement les compétences possédées (modérateur/administrateur uniquement).
...
Débannir un utilisateur et restaurer les compétences éligibles (administrateur uniquement).
...
Modifier le motif enregistré pour un bannissement existant sans débannir ni restaurer
le contenu (administrateur uniquement). Utilise la simulation par défaut sauf si `dryRun` vaut `false`.
