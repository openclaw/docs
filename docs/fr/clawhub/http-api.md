...
- Si aucune IP cliente de confiance n’est disponible, les requêtes anonymes utilisent des compartiments de repli
  ...
  les chemins, slugs, noms de paquets, versions, chaînes de requête fournis par l’appelant ni d’autres
...
- Les résultats sont renvoyés par ordre de pertinence (similarité d’embedding + boosts de jetons exacts de slug/nom + petit a priori de popularité).
- La pertinence est plus forte que la popularité. Une correspondance précise sur un slug ou un jeton de nom d’affichage peut dépasser une correspondance plus large avec un engagement beaucoup plus fort.
- Le texte ASCII est découpé en jetons sur les limites de mots et de ponctuation. Par exemple, `personal-map` contient un jeton `map` autonome, tandis que `amap-jsapi-skill` contient `amap`, `jsapi` et `skill` ; une recherche de `map` donne donc à `personal-map` une correspondance lexicale plus forte qu’à `amap-jsapi-skill`.
...
- Les alias de renommage préservent la résolution des anciennes URL et des installations qui passent par le registre, mais le classement de recherche se fonde sur les métadonnées canoniques de la Skill une fois le renommage indexé. Les statistiques existantes restent associées à la Skill.
...
- `metadata.systems` : cibles de système Nix (par exemple `["aarch64-darwin", "x86_64-linux"]`). `null` si non déclaré.
...
- Les tâches de scan sont asynchrones. Les requêtes de scan manuel sont prioritaires par rapport au travail normal de publication/remplissage rétroactif, mais leur achèvement dépend toujours de la disponibilité des workers.

### `GET /api/v1/skills/-/scan/{scanId}`

Point de terminaison de sondage authentifié pour un scan soumis.

- Renvoie l’état en file d’attente/en cours/réussi/échoué.
- Renvoie `queue.queuedAhead` et `queue.position` pendant la mise en file d’attente afin que les clients puissent afficher combien de scans manuels prioritaires précèdent la requête. Les très grandes files sont bornées et signalées avec `queuedAheadIsEstimate: true`.
...
- `kind` vaut `skill` par défaut ; utilisez `kind=plugin` pour les scans de Plugin/package.
- Renvoie la même forme de ZIP que les téléchargements de requêtes de scan.

### `POST /api/v1/skills/-/scan/batch`

Route canonique de relance de scan par lot réservée aux administrateurs. Elle accepte la même forme de charge utile que l’ancienne route `POST /api/v1/skills/-/rescan-batch`.
...
- L’identité du Skill, l’identité de l’éditeur et les métadonnées de la version sélectionnée sont des champs d’enveloppe de premier niveau (`slug`, `displayName`, `publisherHandle`, `version`, `resolvedFrom`, `tag`, `createdAt`) afin que l’automatisation shell puisse les lire sans dépaqueter des wrappers imbriqués.
...
- Utilisez `/verify` lorsque vous avez besoin de l’enveloppe de vérification Skill Card d’un seul Skill, `/card` lorsque vous avez besoin du Markdown de carte généré, et `/scan` lorsque vous avez besoin des données détaillées des scanners.
...
Recherche de catalogue unifié sur les Skills et les packages de plugins.
...
Les alias de filtres v1 hérités restent acceptés sur les points de terminaison en lecture :
...
`trending` est un classement des installations/téléchargements sur sept jours et n’utilise pas les totaux de tous les temps.
...
d’installation. Il s’agit de la surface publique de consommation OpenClaw permettant de décider si une
version résolue peut être installée.

Authentification :

- Point de terminaison de lecture public. Aucun jeton de propriétaire, d’éditeur, de modérateur ou d’administrateur
  n’est requis.
...
- Les versions ClawPack diffusent les octets `.tgz` exacts de `npm-pack` téléversés.
...
Signale un paquet pour revue par les modérateurs. Les rapports sont au niveau du paquet, éventuellement
...
- Les transferts de skills adossées à GitHub ne proxyfient ni ne mettent en miroir les octets. La réponse JSON
...
pour le handle récupéré est également réaffectée à l’utilisateur de remplacement afin que la synchronisation de profil ultérieure
ne puisse pas restaurer l’autorité concurrente de l’ancien utilisateur. Chaque table primaire est limitée à
