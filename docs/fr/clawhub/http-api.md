...
Les répertoires tiers peuvent utiliser les points de terminaison publics en lecture pour lister ou rechercher des skills ClawHub. Veuillez mettre les résultats en cache, respecter `429`/`Retry-After`, renvoyer les utilisateurs vers la fiche ClawHub canonique (`https://clawhub.ai/<owner>/skills/<slug>`) et éviter de laisser entendre que ClawHub approuve le site tiers. N’essayez pas de répliquer du contenu masqué, privé ou bloqué par modération en dehors de la surface publique de l’API.
...
- Les points de terminaison d’écriture authentifiés ne doivent pas renvoyer un simple `Unauthorized` lorsque
  le serveur connaît la raison. Les jetons manquants, invalides/révoqués et les comptes
...
- Les résultats sont renvoyés par ordre de pertinence (similarité d’embedding + bonus sur les jetons exacts de slug/nom + léger a priori de popularité).
- La pertinence est plus forte que la popularité. Une correspondance précise sur un slug ou un jeton de nom d’affichage peut dépasser une correspondance plus lâche avec un engagement bien plus élevé.
...
- Mettez les termes que les utilisateurs rechercheront littéralement dans le nom d’affichage, le résumé et les tags. N’utilisez un jeton de slug autonome que lorsqu’il constitue aussi une identité stable que vous souhaitez conserver.
...
- Les alias de renommage préservent la résolution des anciennes URL et des installations qui se résolvent via le registre, mais le classement de recherche est basé sur les métadonnées canoniques de la skill après indexation du renommage. Les statistiques existantes restent associées à la skill.
...
Signaler une skill pour examen par les modérateurs. Les signalements sont au niveau de la skill, éventuellement liés
...
- `version.security` inclut l’état de vérification d’analyse normalisé et les détails des analyseurs
...
- Les charges utiles des requêtes d’analyse et les rapports téléchargeables expirent du magasin des requêtes d’analyse après la fenêtre de conservation.
...
- Renvoie `queue.queuedAhead` et `queue.position` pendant l’attente afin que les clients puissent afficher combien d’analyses manuelles prioritaires précèdent la requête. Les très grandes files d’attente sont bornées et signalées avec `queuedAheadIsEstimate: true`.
...
- L’identité de la skill, l’identité de l’éditeur et les métadonnées de la version sélectionnée sont des champs de premier niveau de l’enveloppe (`slug`, `displayName`, `publisherHandle`, `version`, `resolvedFrom`, `tag`, `createdAt`) afin que l’automatisation shell puisse les lire sans décompacter des enveloppes imbriquées.
...
- `provenance` vaut `server-resolved-github-import` uniquement lorsque ClawHub a résolu et stocké un dépôt/réf/commit/chemin GitHub lors de la publication ou de l’importation ; sinon, elle vaut `unavailable`.
...
- Utilisez `/verify` lorsque vous avez besoin de l’enveloppe de vérification de Skill Card pour une seule skill, `/card` lorsque vous avez besoin du Markdown de carte généré, et `/scan` lorsque vous avez besoin des données détaillées des scanners.
...
Navigation dans le catalogue réservé aux Plugins, sur les packages code-plugin et bundle-plugin.
...
- Le filtrage par catégorie est un véritable filtre d’API adossé aux lignes de condensé
  de catégories de Plugins, pas une réécriture de requête de recherche.
...
d’installation. C’est la surface publique de consommation OpenClaw permettant de décider si une
...
- `trust.scanStatus` est le statut de confiance effectif dérivé des entrées de l’analyseur
  et de la modération manuelle de la version.
- `trust.moderationState` est nullable. Il vaut `null` lorsqu’il n’existe aucune modération
  manuelle de la version.
...
- Ce point de terminaison est volontairement plus étroit que les points de terminaison de modération
  ...
  les identités des rapporteurs, les corps de rapport, les preuves privées ni les chronologies de revue
  internes.
...
- C’est la surface du résolveur OpenClaw ; elle évite de deviner le format d’archive à partir
  d’une URL partagée.
...
- `packageName` est normalisé comme nom npm ; le package peut être absent pour les migrations
  planifiées.
- Cela suit uniquement la préparation de la migration. Cela ne modifie pas OpenClaw et ne génère
  pas de ClawPacks.
...
bloquent automatiquement les téléchargements par eux-mêmes ; les modérateurs doivent utiliser la modération des versions pour
...
Point de terminaison modérateur/administrateur pour la réception des rapports de package.
...
Point de terminaison modérateur/administrateur pour résoudre ou rouvrir les rapports de package.
...
Point de terminaison modérateur/administrateur pour la vérification d’une version de package.
...
- `approved` : vérifié manuellement et autorisé.
...
- Les métadonnées limitées au registre ne sont pas injectées dans l’archive téléchargée.
...
Télécharge un ZIP de version de skill hébergée, ou renvoie un transfert de source GitHub pour une
...
- Les transferts de skill adossée à GitHub ne proxient ni ne répliquent les octets. La réponse JSON
  inclut `sourceRef: "public-github"`, `repo`, `commit`, `path`, `contentHash`,
  et `archiveUrl` ; l’état d’analyse/actuel sert de barrière et n’est pas inclus comme métadonnée
...
- Champ facultatif de payload : `migrateOwner`. Lorsque `true` avec `ownerHandle`, une
  ...
  éditeurs actuel et cible. Sans cette acceptation explicite, les changements de propriétaire sont
  rejetés.
...
- Les champs de formulaire autorisés sont `payload`, les blobs `files` répétés, ou une référence de tarball `clawpack`
  unique. `clawpack` peut être un blob `.tgz` ou un identifiant de stockage renvoyé par
...
Supprime de manière réversible / restaure une skill (propriétaire, modérateur ou administrateur).
...
version. Les noms de package deviennent des packages espaces réservés privés sans lignes de version, afin que le même
...
Les sources de skill GitHub sont liées à l’éditeur et signalées comme vérifiées plutôt que réécrites.
