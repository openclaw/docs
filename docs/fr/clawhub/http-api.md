...
Les répertoires tiers peuvent utiliser les points de terminaison publics en lecture pour lister ou rechercher des skills ClawHub. Veuillez mettre les résultats en cache, respecter `429`/`Retry-After`, renvoyer les utilisateurs vers la fiche ClawHub canonique (`https://clawhub.ai/<owner>/skills/<slug>`) et éviter de suggérer une approbation de ClawHub pour le site tiers. Ne tentez pas de mettre en miroir du contenu masqué, privé ou bloqué par la modération en dehors de la surface publique de l’API.

Les raccourcis de slug web se résolvent sur plusieurs familles de registre, mais les clients d’API doivent utiliser
les URL canoniques renvoyées par les points de terminaison de lecture plutôt que de reconstruire la précédence
des routes.
...
- `highlightedOnly` (facultatif) : `true` pour filtrer uniquement les skills mises en avant
...
- Les résultats sont renvoyés par ordre de pertinence (similarité d’embedding + boosts de jetons exacts de slug/nom + un faible a priori de popularité).
- La pertinence est plus forte que la popularité. Une correspondance précise sur un slug ou un jeton de nom d’affichage peut devancer une correspondance plus approximative avec un engagement beaucoup plus élevé.
...
- Placez les termes que les utilisateurs rechercheront littéralement dans le nom d’affichage, le résumé et les étiquettes. N’utilisez un jeton de slug autonome que lorsqu’il s’agit également d’une identité stable que vous souhaitez conserver.
...
- Les alias de renommage préservent la résolution des anciennes URL et des installations qui passent par le registre, mais le classement de recherche repose sur les métadonnées canoniques de la skill après l’indexation du renommage. Les statistiques existantes restent associées à la skill.
...
Signaler une skill pour examen par un modérateur. Les signalements s’appliquent au niveau de la skill, sont éventuellement liés
...
- `version.security` inclut l’état de vérification d’analyse normalisé et les détails de l’analyseur
...
- La réponse est `202` avec `{ "ok": true, "scanId": "...", "jobId": "...", "status": "queued", "sourceKind": "published", "update": false, "queue": { "queuedAhead": 0, "queuedAheadIsEstimate": false, "position": 1, "running": 0, "runningIsEstimate": false, "note": "Les analyses sont asynchrones et peuvent prendre du temps." } }`.
- Les tâches d’analyse sont asynchrones. Les requêtes d’analyse manuelles sont prioritaires par rapport aux travaux normaux de publication/remplissage, mais leur achèvement dépend toujours de la disponibilité des workers.
...
- Renvoie `queue.queuedAhead` et `queue.position` tant que la requête est en file d’attente, afin que les clients puissent afficher le nombre d’analyses manuelles prioritaires qui la précèdent. Les très grandes files sont bornées et signalées avec `queuedAheadIsEstimate: true`.
...
- L’identité de la skill, l’identité de l’éditeur et les métadonnées de la version sélectionnée sont des champs de premier niveau de l’enveloppe (`slug`, `displayName`, `publisherHandle`, `version`, `resolvedFrom`, `tag`, `createdAt`) afin que l’automatisation shell puisse les lire sans décompacter d’enveloppes imbriquées.
...
- Utilisez `/verify` lorsque vous avez besoin de l’enveloppe de vérification Skill Card d’une seule skill, `/card` lorsque vous avez besoin du Markdown de carte générée, et `/scan` lorsque vous avez besoin des données détaillées des scanners.
...
- `category` (facultatif) : filtre de catégorie de plugin. Pris en charge uniquement lorsque la
  ...
  hérités v1 sont documentés sous `GET /api/v1/plugins`.
...
- Corps : archive ZIP.
- Chaque Skill exportée est enracinée dans `{publisher}/{slug}/`.
...
- Les Skills actuelles adossées à GitHub avec une analyse `clean` ou `suspicious` incluent
  ...
  hachage de contenu et URL d’archive. Elles n’incluent pas les fichiers sources hébergés sur ClawHub.
...
- Les alias de filtres v1 hérités documentés sous `GET /api/v1/plugins` sont également
  acceptés.
- Le filtrage par catégorie est un vrai filtre d’API adossé aux lignes de synthèse des catégories de plugins,
...
- Skills peut également se résoudre par cette route dans le catalogue unifié.
...
Signaler un paquet pour revue par les modérateurs. Les signalements sont au niveau du paquet et facultativement
...
`finalAction: "revoke"` avec un signalement confirmé pour appliquer la modération de la release dans le
...
- `revoked` : bloqué après qu’une release a été précédemment jugée fiable.
...
skill actuelle basée sur GitHub avec une analyse `clean` ou `suspicious` et sans
version hébergée.
...
- Les transferts de skills basées sur GitHub ne proxient ni ne dupliquent les octets. La réponse JSON
  inclut `sourceRef: "public-github"`, `repo`, `commit`, `path`, `contentHash`
  et `archiveUrl` ; l’état d’analyse/actuel sert de garde et n’est pas inclus comme métadonnées de charge utile
  de réussite.
...
- Les champs de formulaire autorisés sont `payload`, des blobs `files` répétés ou une référence de tarball
  ...
  le flux upload-url. Les publications avec identifiant de stockage intermédiaire doivent également inclure le
...
Admin uniquement. Réserve des slugs racine et des noms de packages pour un propriétaire légitime sans publier de
...
fournisseur GitHub ; les handles modifiables sont uniquement utilisés comme garde destiné à l’opérateur.
...
identités GitHub principales. La récupération échoue en mode fermé lorsque l’éditeur personnel actuel de l’utilisateur
de destination possède des Skills, des packages ou des sources de Skills GitHub.
La récupération migre également les anciens champs `ownerUserId` pour les Skills de l’éditeur récupéré,
les alias de slugs de Skills, les packages, les avertissements de l’inspecteur de packages et les lignes dérivées du digest de recherche afin que
les chemins de propriétaire direct concordent avec la nouvelle autorité de l’éditeur. Une réservation de handle protégé active
...
Les sources de Skills GitHub sont délimitées par éditeur et signalées comme vérifiées plutôt que réécrites.
...
- Les deux points de terminaison nécessitent une authentification par jeton API et fonctionnent uniquement pour le propriétaire du Skill.
