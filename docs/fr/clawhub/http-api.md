...
x-i18n:
    generated_at: "2026-07-01T12:58:09Z"
    model: gpt-5.5
...
Les répertoires tiers peuvent utiliser les points de terminaison publics en lecture pour lister ou rechercher des Skills ClawHub. Veuillez mettre les résultats en cache, respecter `429`/`Retry-After`, renvoyer les utilisateurs vers la fiche ClawHub canonique (`https://clawhub.ai/<owner>/skills/<slug>`) et éviter de suggérer une approbation de ClawHub pour le site tiers. N’essayez pas de dupliquer du contenu masqué, privé ou bloqué par la modération en dehors de la surface de l’API publique.

Les raccourcis de slugs web se résolvent entre familles de registres, mais les clients API doivent utiliser
...
- Les points de terminaison d’écriture authentifiés ne doivent pas renvoyer un simple `Unauthorized` quand
...
- Utilise les en-têtes d’IP client approuvés, y compris `cf-connecting-ip`, uniquement lorsque le
  déploiement active explicitement les en-têtes transférés approuvés.
- ClawHub utilise des en-têtes de transfert approuvés pour identifier les IP des clients en périphérie.
- Si aucune IP client approuvée n’est disponible, les requêtes anonymes utilisent des compartiments de repli
  ...
  paramètres d’artefacts fournis par l’appelant.
...
doivent lire le corps de réponse comme une chaîne lisible par l’humain. Les paramètres de requête inconnus sont
...
- Les résultats sont renvoyés par ordre de pertinence (similarité d’intégration + bonus de correspondance exacte sur les jetons de slug/nom + un léger a priori de popularité).
- La pertinence est plus forte que la popularité. Une correspondance précise avec un slug ou un jeton de nom d’affichage peut dépasser une correspondance plus lâche avec un engagement nettement plus élevé.
- Le texte ASCII est découpé en jetons sur les limites de mots et de ponctuation. Par exemple, `personal-map` contient un jeton autonome `map`, tandis que `amap-jsapi-skill` contient `amap`, `jsapi` et `skill` ; une recherche de `map` donne donc à `personal-map` une correspondance lexicale plus forte que `amap-jsapi-skill`.
- La popularité est mise à l’échelle logarithmique et plafonnée. Les Skills à fort engagement peuvent être moins bien classées quand le texte de recherche correspond moins bien.
- Un état de modération suspect ou masqué peut retirer une Skill de la recherche publique selon les filtres de l’appelant et le statut de modération actuel.

Conseils de découvrabilité pour les éditeurs :

- Placez les termes que les utilisateurs rechercheront littéralement dans le nom d’affichage, le résumé et les balises. N’utilisez un jeton de slug autonome que lorsqu’il s’agit aussi d’une identité stable que vous voulez conserver.
...
- Si une Skill est invisiblement de façon inattendue, vérifiez d’abord l’état de modération avec `clawhub inspect @owner/slug` après connexion avant de modifier les métadonnées liées au classement.
...
- Quand `nonSuspiciousOnly=true`, les tris basés sur des curseurs peuvent renvoyer moins de `limit` éléments sur une page parce que les Skills suspectes sont filtrées après la récupération de la page.
...
- `metadata.os` : restrictions de système d’exploitation déclarées dans le frontmatter de la Skill (par exemple `["macos"]`, `["linux"]`). `null` si non déclaré.
- `metadata.systems` : cibles de systèmes Nix (par exemple `["aarch64-darwin", "x86_64-linux"]`). `null` si non déclaré.
...
Signaler une Skill pour examen par la modération. Les signalements se font au niveau de la Skill, peuvent être liés
...
- `moderation` est un instantané actuel de modération au niveau Skill, dérivé de la dernière version.
...
- Les analyses publiées exigent un accès de gestion propriétaire/éditeur, ou une autorité de modération/administration de la plateforme.
- Les analyses publiées ne réécrivent les résultats que lorsque `update: true` et que l’analyse se termine avec succès.
...
- Les tâches d’analyse sont asynchrones. Les requêtes d’analyse manuelles sont prioritaires par rapport au travail normal de publication/remplissage, mais leur achèvement dépend toujours de la disponibilité des workers.
...
- Renvoie `queue.queuedAhead` et `queue.position` pendant la mise en file d’attente afin que les clients puissent afficher combien d’analyses manuelles prioritaires précèdent la requête. Les très grandes files d’attente sont bornées et signalées avec `queuedAheadIsEstimate: true`.
...
- Exige une analyse réussie ; les analyses non terminales renvoient `409`.
- Renvoie un ZIP contenant `manifest.json`, `clawscan.json`, `skillspector.json`, `static-analysis.json`, `virustotal.json` et `README.md`.

### `GET /api/v1/skills/-/scan/download/{name}?version=<version>&kind=skill|plugin`

Point de terminaison authentifié d’archive de rapport stocké pour les versions soumises.

- Exige un accès de gestion propriétaire/éditeur au Skill ou au Plugin, ou une autorité de modération/administration de la plateforme.
- Renvoie les résultats d’analyse stockés pour la version exacte soumise, y compris les versions bloquées ou masquées.
- `kind` vaut `skill` par défaut ; utilisez `kind=plugin` pour les analyses de Plugins/paquets.
- Renvoie la même forme ZIP que les téléchargements de requêtes d’analyse.

### `POST /api/v1/skills/-/scan/batch`

Route canonique réservée aux administrateurs pour la nouvelle analyse par lots. Elle accepte la même forme de charge utile que l’ancien `POST /api/v1/skills/-/rescan-batch`.

### `POST /api/v1/skills/-/scan/batch/status`

Route canonique d’état par lots réservée aux administrateurs. Elle accepte `{ "jobIds": ["..."] }` et renvoie les mêmes compteurs agrégés que l’ancien `POST /api/v1/skills/-/rescan-batch/status`.
...
- L’identité du Skill, l’identité de l’éditeur et les métadonnées de la version sélectionnée sont des champs d’enveloppe de premier niveau (`slug`, `displayName`, `publisherHandle`, `version`, `resolvedFrom`, `tag`, `createdAt`) afin que l’automatisation shell puisse les lire sans décompresser des enveloppes imbriquées.
...
- La réponse concerne uniquement la sécurité. Elle n’inclut pas les données Skill Card, l’état de carte générée, les listes de fichiers d’artefacts ni les charges utiles détaillées des scanners.
- `security.signals` contient uniquement des preuves complémentaires au niveau de l’état ; utilisez `/scan` ou la page d’audit de sécurité ClawHub pour les détails complets des scanners.
- `security.signals.dependencyRegistry` est conservé pour la compatibilité de réponse v1, mais le scanner d’existence du registre de dépendances est retiré et cette clé vaut toujours `null`.
...
- Utilisez `/verify` lorsque vous avez besoin de l’enveloppe de vérification Skill Card pour un Skill unique, `/card` lorsque vous avez besoin du Markdown de carte généré, et `/scan` lorsque vous avez besoin de données détaillées des scanners.
...
- `category` (facultatif) : filtre de catégorie de plugin. Pris en charge uniquement lorsque la
  ...
  alias de filtre v1 hérités sont documentés sous `GET /api/v1/plugins`.
...
- Corps : archive ZIP.
- Chaque skill exporté est enraciné à `{publisher}/{slug}/`.
- Les skills hébergés incluent les fichiers de la dernière version stockée et sont listés dans
...
- Les skills actuels adossés à GitHub avec une analyse `clean` ou `suspicious` incluent
  ...
  hachage du contenu et URL d’archive. Ils n’incluent pas les fichiers sources hébergés par ClawHub.
...
- `startDate` (requis) : limite inférieure en millisecondes Unix pour le `updatedAt` du Plugin.
- `endDate` (requis) : limite supérieure en millisecondes Unix pour le `updatedAt` du Plugin.
- `limit` (facultatif) : entier (1-250), valeur par défaut `250`.
- `cursor` (facultatif) : curseur de pagination de la réponse précédente.
- `family` (facultatif) : `code-plugin` ou `bundle-plugin`. Omettre ce paramètre signifie les deux
  familles de Plugins.
...
- Les alias de filtres legacy v1 documentés sous `GET /api/v1/plugins` sont également
  acceptés.
- Le filtrage par catégorie est un véritable filtre d'API adossé aux lignes de condensé de catégories de Plugins,
  et non une réécriture de requête de recherche.
...
- Nécessite un jeton d'API pour le propriétaire du paquet, un propriétaire/admin d'éditeur d'organisation,
...
d'installation. Il s'agit de la surface publique de consommation OpenClaw pour déterminer si une
...
- `trust.scanStatus` est le statut de confiance effectif dérivé des entrées de scanners
  et de la modération manuelle des versions.
...
- `trust.blockedFromDownload` est le signal de blocage d'installation. OpenClaw et les autres
  ...
  redériver les règles de blocage depuis les champs de scanner ou de modération.
...
- `trust.pending` signifie qu'une ou plusieurs entrées de confiance attendent encore leur achèvement.
...
- Ce point de terminaison est exact à la version. Les clients doivent l'appeler après avoir résolu la
  version de paquet qu'ils prévoient d'installer, et non simplement après avoir lu les dernières
...
- Ce point de terminaison est volontairement plus étroit que les points de terminaison de modération
  ...
  les identités des rapporteurs, les corps de rapports, les preuves privées ni les chronologies de révision
  internes.
...
- Les versions de paquets legacy renvoient un artefact `legacy-zip` et une URL ZIP legacy
  `downloadUrl`.
- Les versions ClawPack renvoient un artefact `npm-pack`, des champs d'intégrité npm, une
  `tarballUrl` et l'URL de compatibilité ZIP legacy.
...
- Les versions ZIP legacy redirigent vers `/api/v1/packages/{name}/download?version=`.
- Utilise le compartiment de limitation de débit des téléchargements.
...
Point de terminaison modérateur/admin pour les files de révision des versions de paquets.
...
Signale un paquet pour examen par les modérateurs. Les rapports sont au niveau du paquet, éventuellement
...
Point de terminaison modérateur/admin pour la réception des rapports de package.
...
Point de terminaison propriétaire/modérateur pour la visibilité de la modération d’un package.

Authentification :

- Requiert un jeton d’API pour le propriétaire du package, un membre de l’éditeur, un modérateur ou
...
Point de terminaison modérateur/admin pour résoudre ou rouvrir les rapports de package.
...
`finalAction: "revoke"` avec un rapport confirmé pour appliquer la modération de la release dans le
...
Point de terminaison modérateur/admin pour l’examen d’une release de package.
...
Les releases en quarantaine et révoquées renvoient `403` depuis les routes de téléchargement d’artefacts.
...
- Utilise le quota de débit de lecture, pas celui de téléchargement.
- Les fichiers binaires renvoient `415`.
- Limite de taille de fichier : 200 Ko.
- Les analyses VirusTotal en attente ne bloquent pas les lectures ; les releases malveillantes peuvent tout de même être retenues ailleurs.
- Les packages privés renvoient `404` sauf si l’appelant peut lire l’éditeur propriétaire.

### `GET /api/v1/packages/{name}/download`

Télécharge l’archive ZIP déterministe héritée pour une release de package.
...
- Cette route reste uniquement en ZIP. Elle ne diffuse pas les fichiers ClawPack `.tgz`.
...
skill actuelle adossée à GitHub avec une analyse `clean` ou `suspicious` et aucune version
hébergée.
...
- Les transferts de skills adossées à GitHub ne proxifient ni ne mettent en miroir les octets. La réponse JSON
  ...
  et `archiveUrl` ; l’état d’analyse/actuel sert de gate et n’est pas inclus comme métadonnée
  de charge utile de succès.
- Les statistiques de téléchargement sont comptées comme des identités uniques par jour UTC (`userId` lorsque le jeton d’API est valide, sinon IP).

## Points de terminaison d’authentification (jeton Bearer)

Tous les points de terminaison requièrent :
...
Publie une release code-plugin ou bundle-plugin.

- Requiert l’authentification par jeton Bearer.
- Requiert `multipart/form-data`.
...
- Les packages de Plugin requièrent `openclaw.plugin.json`. Les téléversements ClawPack `.tgz` doivent
  le contenir à `package/openclaw.plugin.json`.
- Les Plugins de code requièrent `package.json`, des métadonnées de dépôt source, des métadonnées de commit
...
Supprimer de manière réversible / restaurer une skill (propriétaire, modérateur ou administrateur).
...
release. Les noms de package deviennent des packages d’espace réservé privés sans lignes de release, afin que le même
propriétaire puisse publier ultérieurement la vraie release code-plugin ou bundle-plugin sous ce nom.
...
Le point de terminaison utilise le mode dry-run par défaut. Appliquer la récupération nécessite `dryRun: false` et
...
les alias de slug de Skill, les packages, les avertissements de l'inspecteur de package et les lignes dérivées de résumé de recherche afin que
...
Débannir un utilisateur et restaurer les Skills admissibles (admin uniquement).
...
Modifier le motif enregistré d'un bannissement existant sans débannir ni restaurer
le contenu (admin uniquement). Utilise le mode dry-run par défaut sauf si `dryRun` vaut `false`.
...
Ajouter/supprimer une étoile (éléments mis en avant). Les deux points de terminaison sont idempotents.
