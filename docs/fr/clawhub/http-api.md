...
x-i18n:
    generated_at: "2026-07-03T23:30:48Z"
    model: gpt-5.5
...
Les anciens `/api/...` et `/api/cli/...` restent disponibles pour la compatibilité (voir `DEPRECATIONS.md`).
OpenAPI : `/api/v1/openapi.json`.

## Réutilisation du catalogue public

Les annuaires tiers peuvent utiliser les points de terminaison publics en lecture pour lister ou rechercher des Skills ClawHub. Veuillez mettre les résultats en cache, respecter `429`/`Retry-After`, renvoyer les utilisateurs vers la fiche ClawHub canonique (`https://clawhub.ai/<owner>/skills/<slug>`), et éviter de laisser entendre que ClawHub approuve le site tiers. N’essayez pas de dupliquer du contenu masqué, privé ou bloqué par modération en dehors de la surface publique de l’API.
...
- Si `Retry-After` est absent, utilisez `RateLimit-Reset` comme repli (ou calculez depuis `X-RateLimit-Reset`).
...
- Les résultats sont renvoyés par ordre de pertinence (similarité d’embedding + bonus de jetons exacts de slug/nom + un léger a priori de popularité).
...
- Les alias de renommage préservent la résolution pour les anciennes URL et les installations qui se résolvent via le registre, mais le classement de recherche repose sur les métadonnées canoniques du Skill après indexation du renommage. Les statistiques existantes restent associées au Skill.
...
- Les appelants publics obtiennent `200` uniquement pour les Skills visibles déjà signalés.
...
- Renvoie `queue.queuedAhead` et `queue.position` pendant la mise en file d’attente afin que les clients puissent afficher le nombre d’analyses manuelles prioritaires précédant la requête. Les files très volumineuses sont bornées et signalées avec `queuedAheadIsEstimate: true`.
...
- L’identité de la Skill, l’identité de l’éditeur et les métadonnées de la version sélectionnée sont des champs de premier niveau de l’enveloppe (`slug`, `displayName`, `publisherHandle`, `version`, `resolvedFrom`, `tag`, `createdAt`) afin que l’automatisation shell puisse les lire sans déballer d’enveloppes imbriquées.
...
- Utilisez `/verify` lorsque vous avez besoin de l’enveloppe de vérification Skill Card d’une seule Skill, `/card` lorsque vous avez besoin du Markdown de carte généré, et `/scan` lorsque vous avez besoin de données détaillées de scanner.
...
- Les entrées de Skill restent adossées au registre de Skills et peuvent toujours être publiées uniquement via `POST /api/v1/skills`.
...
Parcours du catalogue limité aux Plugins pour les packages code-plugin et bundle-plugin.
...
- Les alias de filtre v1 hérités documentés sous `GET /api/v1/plugins` sont également
  acceptés.
- Le filtrage par catégorie est un véritable filtre d’API adossé à des lignes de synthèse
...
- Les versions de paquets héritées renvoient un artefact `legacy-zip` et une URL de téléchargement ZIP héritée
  `downloadUrl`.
...
Signale un paquet pour revue par les modérateurs. Les rapports s’appliquent au niveau du paquet, éventuellement
...
- Utilise par défaut la dernière version.
- Utilise le compartiment de débit de lecture, pas celui des téléchargements.
...
- Les packuments de paquets scoped prennent en charge à la fois `/api/npm/@scope/name` et le chemin de requête
...
Télécharge un ZIP de version de Skill hébergé, ou renvoie un transfert vers la source GitHub pour un
...
- Les transferts de Skills adossés à GitHub ne proxifient ni ne mettent en miroir les octets. La réponse JSON
  ...
  et `archiveUrl` ; l’analyse/l’état actuel est une porte de validation et n’est pas inclus comme métadonnées de charge utile
  de succès.
- Les statistiques de téléchargement sont comptées comme des identités uniques par jour UTC (`userId` quand le jeton API est valide, sinon l’IP).
...
- Champ facultatif de la charge utile : `migrateOwner`. Lorsque `true` avec `ownerHandle`, un
  Skill existant peut être déplacé vers ce propriétaire si l’acteur est admin/propriétaire sur les deux
...
- Champ facultatif de la charge utile : `ownerHandle`. Lorsqu’il est présent, seuls les admins peuvent publier au nom de ce propriétaire.
...
Réservé aux admins. Réserve les slugs racine et les noms de paquets pour un propriétaire légitime sans publier de
...
immuables ; les handles mutables ne sont utilisés que comme garde-fou destiné à l’opérateur.
...
- `limit` (facultatif) : résultats max. (20 par défaut, 200 max.)
