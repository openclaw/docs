...
read_when:
    ...
    - Débogage des requêtes entre la CLI et le registre
summary: Référence de l’API HTTP (points de terminaison publics et CLI, et authentification).
x-i18n:
    generated_at: "2026-07-12T02:23:37Z"
    model: gpt-5.6
...
Les annuaires tiers peuvent utiliser les points de terminaison publics en lecture pour répertorier ou rechercher des Skills ClawHub. Veuillez mettre les résultats en cache, respecter `429`/`Retry-After`, rediriger les utilisateurs vers la fiche ClawHub canonique (`https://clawhub.ai/<owner>/skills/<slug>`) et éviter de laisser entendre que ClawHub approuve le site tiers. Ne tentez pas de reproduire du contenu masqué, privé ou bloqué par la modération en dehors de la surface de l’API publique.

Les raccourcis de slugs Web sont résolus entre les différentes familles du registre, mais les clients API doivent utiliser les URL canoniques renvoyées par les points de terminaison en lecture plutôt que de reconstruire la priorité des routes.
...
- Requêtes authentifiées (jeton Bearer valide) : limite appliquée par quota utilisateur.
- Si le jeton est absent ou non valide, l’application de la limite se rabat sur l’adresse IP.
- Les points de terminaison d’écriture authentifiés ne doivent pas renvoyer uniquement `Unauthorized` lorsque le serveur connaît la raison. Les jetons absents, non valides ou révoqués, ainsi que les comptes supprimés, bannis ou désactivés, doivent chacun produire un texte exploitable afin que les clients CLI puissent indiquer aux utilisateurs ce qui les a bloqués.

- Lecture : 3 000/min par IP, 12 000/min par clé
- Écriture : 300/min par IP, 3 000/min par clé
- Téléchargement : 1 200/min par IP, 6 000/min par clé (points de terminaison de téléchargement)

En-têtes :

- Compatibilité historique : `X-RateLimit-Limit`, `X-RateLimit-Reset`
- Normalisés : `RateLimit-Limit`, `RateLimit-Reset`
...
- `X-RateLimit-Remaining` / `RateLimit-Remaining` : quota restant exact, lorsqu’il est présent.
  Les requêtes réussies réparties entre plusieurs partitions omettent cet en-tête au lieu de renvoyer une valeur globale approximative.
...
- Si `Retry-After` existe, attendez le nombre de secondes indiqué avant de réessayer.
- Utilisez un délai exponentiel avec gigue pour éviter les nouvelles tentatives synchronisées.
- Si `Retry-After` est absent, rabattez-vous sur `RateLimit-Reset` (ou calculez le délai à partir de `X-RateLimit-Reset`).

Source de l’adresse IP :

- Utilise les en-têtes d’adresse IP cliente de confiance, notamment `cf-connecting-ip`, uniquement lorsque le déploiement active explicitement les en-têtes transférés de confiance.
- ClawHub utilise les en-têtes de transfert de confiance pour identifier les adresses IP clientes à la périphérie.
- Si aucune adresse IP cliente de confiance n’est disponible, les requêtes anonymes utilisent des quotas de secours définis uniquement selon le type de limite de débit. Ces quotas de secours n’incluent pas les chemins, slugs, noms de paquets, versions, chaînes de requête ou autres paramètres d’artefact fournis par l’appelant.

## Réponses d’erreur

Les réponses d’erreur publiques v1 sont en texte brut avec `content-type: text/plain; charset=utf-8`.
Cela inclut les échecs de validation (`400`), les ressources publiques absentes (`404`), les échecs d’authentification et d’autorisation (`401`/`403`), les limites de débit (`429`) et les téléchargements bloqués. Les clients doivent lire le corps de la réponse comme une chaîne destinée à être comprise par un humain. Les paramètres de requête inconnus sont ignorés à des fins de compatibilité, mais les paramètres de requête reconnus dont les valeurs ne sont pas valides renvoient `400`.
...
- `highlightedOnly` (facultatif) : `true` pour ne conserver que les Skills mises en avant
...
- Les résultats sont renvoyés par ordre de pertinence (similarité des plongements + bonus pour les correspondances exactes de jetons du slug ou du nom + faible facteur préalable de popularité).
- La pertinence prime sur la popularité. Une correspondance précise avec un jeton du slug ou du nom d’affichage peut devancer une correspondance moins précise bénéficiant d’un engagement bien supérieur.
- Le texte ASCII est découpé en jetons aux limites des mots et des signes de ponctuation. Par exemple, `personal-map` contient un jeton autonome `map`, tandis que `amap-jsapi-skill` contient `amap`, `jsapi` et `skill` ; une recherche portant sur `map` donne donc à `personal-map` une correspondance lexicale plus forte qu’à `amap-jsapi-skill`.
- La popularité utilise une échelle logarithmique et est plafonnée. Les Skills à fort engagement peuvent être moins bien classées lorsque le texte de la requête correspond moins précisément.
- Un état de modération suspect ou masqué peut retirer une Skill de la recherche publique selon les filtres de l’appelant et l’état actuel de la modération.

Recommandations de visibilité pour les éditeurs :
...
- Ne renommez pas un slug uniquement pour cibler une requête, sauf si le nouveau slug constitue un meilleur nom canonique à long terme. Les anciens slugs deviennent des alias de redirection, mais l’URL canonique, le slug affiché et les futurs condensés de recherche utilisent le nouveau slug.
- Les alias de renommage préservent la résolution des anciennes URL et des installations résolues par l’intermédiaire du registre, mais le classement dans les résultats de recherche repose sur les métadonnées canoniques de la Skill après l’indexation du renommage. Les statistiques existantes restent associées à la Skill.
- Si une Skill est inexplicablement invisible, vérifiez d’abord son état de modération avec `clawhub inspect @owner/slug` en étant connecté, avant de modifier les métadonnées liées au classement.
...
- `metadata` vaut `null` si la Skill ne comporte aucune métadonnée de plateforme.
- `moderation` est inclus uniquement lorsque la Skill est signalée ou consultée par son propriétaire.

### `GET /api/v1/skills/{slug}/moderation`

Renvoie l’état structuré de la modération.
...
- Les appelants publics ne reçoivent un code `200` que pour les Skills visibles déjà signalées.
...
Signale une Skill afin qu’elle soit examinée par un modérateur. Les signalements concernent une Skill entière, peuvent être liés à une version et alimentent la file d’attente des signalements de Skills.
...
Point de terminaison destiné aux modérateurs et aux administrateurs pour recevoir les signalements de Skills.
...
Point de terminaison destiné aux modérateurs et aux administrateurs pour résoudre ou rouvrir les signalements de Skills.
...
`note` est obligatoire pour `confirmed` et `dismissed` ; il peut être omis lorsque `status` est redéfini sur `open`. Transmettez `finalAction: "hide"` avec un signalement traité pour masquer la Skill dans le même processus vérifiable.
...
- `version.security` comprend l’état normalisé de vérification de l’analyse et les détails de l’analyseur (VirusTotal + LLM), lorsqu’ils sont disponibles.
...
- Inclut l’état de vérification normalisé ainsi que les détails propres à chaque analyseur.
...
- Lors de l’interrogation d’une version historique, vérifiez `moderation.matchesRequestedVersion` et `moderation.sourceVersion` avant de considérer que `moderation` et `security` se rapportent au même contexte de version.

### `POST /api/v1/skills/-/scan`

Point de terminaison authentifié permettant de soumettre de nouvelles tâches ClawScan.
...
Les analyses publiées utilisent du JSON :
...
- Les charges utiles des demandes d’analyse et les rapports téléchargeables expirent du stockage des demandes d’analyse après la période de conservation.
- Les analyses publiées nécessitent un accès de gestion en tant que propriétaire ou éditeur, ou des droits de modérateur ou d’administrateur de la plateforme.
...
- Les tâches d’analyse sont asynchrones. Les demandes d’analyse manuelles sont prioritaires sur les opérations normales de publication et de traitement rétroactif, mais leur achèvement dépend toujours de la disponibilité des workers.

### `GET /api/v1/skills/-/scan/{scanId}`

Point de terminaison authentifié d’interrogation de l’état d’une analyse soumise.

- Renvoie l’état en file d’attente, en cours d’exécution, réussi ou échoué.
- Renvoie `queue.queuedAhead` et `queue.position` pendant la mise en file d’attente afin que les clients puissent afficher le nombre d’analyses manuelles prioritaires précédant la demande. Les files d’attente très volumineuses sont plafonnées et signalées avec `queuedAheadIsEstimate: true`.
...
Point de terminaison authentifié de téléchargement de l’archive du rapport.
...
Point de terminaison authentifié de téléchargement de l’archive du rapport stocké pour les versions soumises.

- Nécessite un accès de gestion en tant que propriétaire ou éditeur de la Skill ou du Plugin, ou des droits de modérateur ou d’administrateur de la plateforme.
- Renvoie les résultats d’analyse stockés pour la version exacte soumise, y compris les versions bloquées ou masquées.
- `kind` vaut `skill` par défaut ; utilisez `kind=plugin` pour les analyses de Plugin ou de paquet.
- Renvoie une archive ZIP de même structure que les téléchargements de demandes d’analyse.
...
- `ok` vaut `true` uniquement lorsque la version sélectionnée possède une fiche de Skill générée, n’est pas bloquée comme logiciel malveillant par la modération et que la vérification ClawScan est sans anomalie.
- L’identité de la Skill, celle de l’éditeur et les métadonnées de la version sélectionnée sont des champs de premier niveau de l’enveloppe (`slug`, `displayName`, `publisherHandle`, `version`, `resolvedFrom`, `tag`, `createdAt`), afin que les automatisations shell puissent les lire sans décompresser d’enveloppes imbriquées.
...
- `provenance` vaut `server-resolved-github-import` uniquement lorsque ClawHub a résolu et stocké un dépôt, une référence, un commit et un chemin GitHub pendant la publication ou l’importation ; sinon, il vaut `unavailable`.
...
de Skills ClawHub installées qu’ils doivent afficher, comme l’interface de contrôle OpenClaw.
...
- `items` doit contenir de 1 à 100 paires `{ slug, version }` uniques.
- Les résultats sont fournis par élément ; l’absence d’une Skill ou d’une version ne fait pas échouer toute la réponse.
- La réponse concerne uniquement la sécurité. Elle n’inclut ni les données de la fiche de Skill, ni l’état de génération de la fiche, ni les listes de fichiers des artefacts, ni les charges utiles détaillées des analyseurs.
...
- `security.signals.dependencyRegistry` est conservé pour assurer la compatibilité des réponses v1, mais l’analyseur d’existence dans le registre des dépendances a été retiré et cette clé vaut toujours `null`.
- L’absence d’une fiche de Skill n’affecte pas les valeurs `ok`, `decision` ou `reasons` de ce point de terminaison ; les clients doivent lire localement le fichier `skill-card.md` installé lorsqu’ils ont besoin du contenu de la fiche.
...
Point de terminaison unifié du catalogue pour :
...
- `category` (facultatif) : filtre de catégorie de Plugin. Pris en charge uniquement lorsque la
  ...
  `/api/v1/code-plugins`, `/api/v1/bundle-plugins` ou aux points de terminaison des paquets avec
  `family=code-plugin`/`family=bundle-plugin`). Les catégories contrôlées et
  les anciens alias de filtre de la v1 sont documentés sous `GET /api/v1/plugins`.
...
Recherche unifiée dans le catalogue parmi les Skills et les paquets de Plugins.
...
- `category` (facultatif) : filtre de catégorie de Plugin. Pris en charge uniquement lorsque la
  ...
  alias de filtre de la v1 sont documentés sous `GET /api/v1/plugins`.
...
Les anciens alias de filtre de la v1 restent acceptés sur les points de terminaison de lecture :
...
`trending` est un classement des installations et téléchargements sur sept jours et n’utilise pas les totaux cumulés.
...
Les anciens alias ne sont pas acceptés comme valeurs de catégorie enregistrées ou déclarées par l’auteur.
...
- `startDate` (obligatoire) : limite inférieure, en millisecondes Unix, pour le champ `updatedAt` de la Skill.
- `endDate` (obligatoire) : limite supérieure, en millisecondes Unix, pour le champ `updatedAt` de la Skill.
...
- Corps : archive ZIP.
- Chaque Skill exportée se trouve sous `{publisher}/{slug}/`.
- Les Skills hébergées incluent les fichiers de la dernière version enregistrée et sont répertoriées dans
  `_manifest.json` avec `sourceRef: "public-clawhub"`.
- Les Skills actuelles adossées à GitHub dont l’analyse est `clean` ou `suspicious` incluent
...
Exportation en masse des dernières versions publiques des plugins pour analyse hors ligne.
...
- `family` (facultatif) : `code-plugin` ou `bundle-plugin`. Si omis, les deux
  familles de plugins sont incluses.

Réponse :

- Corps : archive ZIP.
- La racine de chaque plugin exporté est `{family}/{packageName}/`.
...
- Les anciens alias de filtres v1 documentés sous `GET /api/v1/plugins` sont également
  acceptés.
- Le filtrage par catégorie est un véritable filtre d’API fondé sur les lignes de condensé
...
- Les contrôles de tri de l’interface du navigateur pour la recherche de plugins réorganisent les résultats de pertinence chargés,
  conformément au comportement actuel de navigation dans `/skills`.
...
Renvoie une version du paquet, y compris les métadonnées des fichiers, la compatibilité,
...
clients d’installation. Il s’agit de l’interface publique utilisée par OpenClaw pour déterminer si une
...
- Ce point de terminaison correspond à une version précise. Les clients doivent l’appeler après avoir résolu la
...
- Ce point de terminaison est volontairement plus restreint que les points de terminaison de modération
  destinés aux propriétaires ou aux modérateurs. Il expose la décision d’installation et son explication publique, mais pas
  l’identité des auteurs de signalements, le contenu des signalements, les preuves privées ni les calendriers
  internes d’examen.

### `GET /api/v1/packages/{name}/versions/{version}/artifact`

Renvoie les métadonnées explicites de résolution de l’artefact pour une version du paquet.

Remarques :

- Les anciennes versions de paquets renvoient un artefact `legacy-zip` et une
  ancienne `downloadUrl` ZIP.
- Les versions ClawPack renvoient un artefact `npm-pack`, les champs d’intégrité npm, une
  `tarballUrl` et l’URL de compatibilité avec l’ancien format ZIP.
- Il s’agit de l’interface de résolution d’OpenClaw ; elle évite de déduire le format de l’archive à partir
  d’une URL partagée.
...
- Les versions ClawPack transmettent exactement les octets `.tgz` npm-pack téléversés.
...
Point de terminaison pour les modérateurs permettant de répertorier les entrées de migration des plugins OpenClaw officiels.
...
Point de terminaison pour les administrateurs permettant de créer ou de mettre à jour une entrée de migration d’un plugin officiel.
...
- `bundledPluginId` est normalisé en minuscules et constitue la clé stable de l’opération d’insertion ou de mise à jour.
- `packageName` est normalisé selon les règles de nommage npm ; le paquet peut être absent pour les migrations
  planifiées.
...
- `manual` : toute version comportant une dérogation manuelle de modération.
- `all` : toute version comportant une dérogation manuelle, un état d’analyse autre que sain ou un signalement de paquet.
...
Point de terminaison destiné au propriétaire ou au modérateur pour consulter les informations de modération d’un package.

Authentification :

- Nécessite un jeton d’API appartenant au propriétaire du package, à un membre de l’éditeur, à un modérateur ou à un utilisateur administrateur.
...
Point de terminaison modérateur/administrateur permettant de résoudre ou de rouvrir des signalements de packages.
...
`note` est requis pour `confirmed` et `dismissed` ; il peut être omis lorsque `status` est redéfini sur `open`. Transmettez `finalAction: "quarantine"` ou `finalAction: "revoke"` avec un signalement confirmé afin d’appliquer la modération de la version dans le même processus auditable.
...
Les versions mises en quarantaine et révoquées renvoient `403` depuis les routes de téléchargement des artefacts. Chaque modification écrit une entrée dans le journal d’audit.
...
- Taille maximale des fichiers : 200 Ko.
...
- Les archives de Plugin/package sont des fichiers ZIP dont la racine est `package/`, afin que les anciens clients OpenClaw continuent de fonctionner.
- Cette route reste exclusivement réservée aux fichiers ZIP. Elle ne diffuse pas les fichiers ClawPack `.tgz`.
- Les réponses incluent les en-têtes `ETag`, `Digest`, `X-ClawHub-Artifact-Type` et `X-ClawHub-Artifact-Sha256` pour les contrôles d’intégrité du résolveur.
...
Renvoie un document de métadonnées de package compatible avec npm pour les versions de packages fondées sur ClawPack.
...
- `dist.tarball`, `dist.integrity` et `dist.shasum` utilisent des champs compatibles avec npm afin que les utilisateurs puissent configurer npm pour utiliser le miroir s’ils le souhaitent.
- Les documents de métadonnées des packages avec portée prennent en charge le chemin `/api/npm/@scope/name` ainsi que le chemin de requête encodé `/api/npm/@scope%2Fname` de npm.

### `GET /api/npm/{package}/-/{tarball}.tgz`

Diffuse les octets exacts de l’archive tar ClawPack téléversée aux clients du miroir npm.

Remarques :

- Utilise le quota de débit de téléchargement.
- Les en-têtes de téléchargement incluent le SHA-256 ClawHub ainsi que les métadonnées d’intégrité et de somme de contrôle npm.
- Les contrôles de modération et d’accès aux packages privés continuent de s’appliquer.
...
- `slug` (requis)
- `hash` (requis) : SHA-256 hexadécimal de 64 caractères de l’empreinte du paquet
...
Télécharge le fichier ZIP d’une version de Skill hébergée, ou renvoie une redirection vers la source GitHub pour une Skill actuelle adossée à GitHub dont l’analyse est `clean` ou `suspicious` et qui ne dispose d’aucune version hébergée.

Paramètres de requête :

- `slug` (requis)
- `version` (facultatif) : chaîne semver
- `tag` (facultatif) : nom de balise (par exemple `latest`)
...
- Les redirections de Skills adossées à GitHub ne transmettent ni ne mettent en miroir les octets. La réponse JSON inclut `sourceRef: "public-github"`, `repo`, `commit`, `path`, `contentHash` et `archiveUrl` ; l’état de l’analyse et de la version actuelle sert de contrôle et n’est pas inclus dans les métadonnées de la réponse en cas de réussite.
- Les statistiques de téléchargement sont comptabilisées par identité unique et par jour UTC (`userId` lorsque le jeton d’API est valide, sinon l’adresse IP).
...
- Méthode recommandée : `multipart/form-data` avec du JSON dans `payload` et des objets blob dans `files[]`.
- Un corps JSON contenant `files` (fondé sur des identifiants de stockage) est également accepté.
- Champ facultatif de la charge utile : `ownerHandle`. Lorsqu’il est présent, l’API résout cet éditeur côté serveur et exige que l’acteur dispose d’un accès à l’éditeur.
- Champ facultatif de la charge utile : `migrateOwner`. Lorsque sa valeur est `true` avec `ownerHandle`, une Skill existante peut être transférée à ce propriétaire si l’acteur est administrateur ou propriétaire des éditeurs actuel et cible. Sans cette activation explicite, les changements de propriétaire sont rejetés.
...
- Les champs de formulaire autorisés sont `payload`, des objets blob `files` répétés, ou une référence unique à une archive tar `clawpack`. `clawpack` peut être un objet blob `.tgz` ou un identifiant de stockage renvoyé par le processus d’obtention d’une URL de téléversement. Les publications préparées utilisant un identifiant de stockage doivent également inclure le `clawpackUploadTicket` renvoyé avec cette URL de téléversement.
- Utilisez soit `files`, soit `clawpack`, mais jamais les deux dans une même requête.
- Les corps JSON et les métadonnées `payload.files` / `payload.artifact` fournies par l’appelant sont rejetés.
- Les requêtes de publication multipartes directes sont limitées à 18 Mo. Les archives tar ClawPack peuvent utiliser le processus d’obtention d’une URL de téléversement jusqu’à la limite de 120 Mo par archive tar.
...
- Les packages de Plugin nécessitent `openclaw.plugin.json`. Les téléversements ClawPack `.tgz` doivent le contenir à l’emplacement `package/openclaw.plugin.json`.
- Les Plugins de code nécessitent `package.json`, les métadonnées du dépôt source, les métadonnées du commit source, les métadonnées du schéma de configuration, `openclaw.compat.pluginApi` et `openclaw.build.openclawVersion`.
- `openclaw.hostTargets` et `openclaw.environment` sont des métadonnées facultatives.
- Seuls l’éditeur de l’organisation `openclaw` et les éditeurs personnels des membres actuels de l’organisation `openclaw` peuvent publier sur le canal `official`.
- Les publications pour le compte d’un tiers vérifient également l’éligibilité au canal officiel par rapport au compte du propriétaire cible.

### `DELETE /api/v1/skills/{slug}` / `POST /api/v1/skills/{slug}/undelete`

Supprime logiquement ou restaure une Skill (propriétaire, modérateur ou administrateur).
...
Lorsqu’il est présent, `reason` est enregistré comme note de modération de la Skill et copié dans le journal d’audit. Les suppressions logiques déclenchées par le propriétaire réservent le slug pendant 30 jours, après quoi un autre éditeur peut le revendiquer. La réponse de suppression inclut `slugReservedUntil` lorsque cette expiration s’applique. Les masquages effectués par un modérateur ou un administrateur et les suppressions pour raisons de sécurité n’expirent pas de cette manière.
...
Réservé aux administrateurs. Garantit l’existence d’un éditeur d’organisation pour un identifiant. Si l’identifiant désigne encore un ancien utilisateur partagé ou éditeur personnel, le point de terminaison le migre d’abord vers un éditeur d’organisation. Pour une organisation nouvellement créée, fournissez `memberHandle` ; l’administrateur effectuant l’action n’est pas ajouté comme membre. `memberRole` vaut `owner` par défaut.
...
Création en libre-service et avec authentification d’un éditeur d’organisation. Crée un nouvel éditeur d’organisation et ajoute l’appelant comme propriétaire. Ce point de terminaison ne migre pas les identifiants d’utilisateur ou d’éditeur personnel existants et ne marque pas l’éditeur comme fiable ou officiel.
...
Réservé aux administrateurs. Réserve les slugs racine et les noms de packages à leur propriétaire légitime sans publier de version. Les noms de packages deviennent des packages substituts privés sans lignes de version, ce qui permet au même propriétaire de publier ultérieurement la véritable version de `code-plugin` ou de `bundle-plugin` sous ce nom.
...
Réservé aux administrateurs. Récupère un éditeur personnel pour un principal OAuth GitHub de remplacement vérifié sans modifier les lignes de comptes Convex Auth. La requête doit nommer les deux identifiants immuables de comptes du fournisseur GitHub ; les identifiants modifiables servent uniquement de garde-fou destiné à l’opérateur.

Le point de terminaison fonctionne par défaut en mode simulation. L’application de la récupération nécessite `dryRun: false` et
`confirmIdentityVerified: true` après que le personnel a vérifié de manière indépendante la continuité entre les deux
...
pour le pseudonyme récupéré est également réaffectée à l’utilisateur de remplacement afin qu’une synchronisation ultérieure
...
le contenu (administrateur uniquement). Fonctionne par défaut en mode simulation, sauf si `dryRun` vaut `false`.
...
qui préparent une archive tar ClawPack doivent envoyer l’identifiant de stockage obtenu dans
