...
x-i18n:
    generated_at: "2026-06-30T13:59:03Z"
    model: gpt-5.5
...
Les annuaires tiers peuvent utiliser les points de terminaison publics en lecture pour lister ou rechercher les Skills ClawHub. Veuillez mettre les résultats en cache, respecter `429`/`Retry-After`, renvoyer les utilisateurs vers la fiche ClawHub canonique (`https://clawhub.ai/<owner>/skills/<slug>`) et éviter de laisser entendre que ClawHub approuve le site tiers. Ne tentez pas de dupliquer du contenu masqué, privé ou bloqué par la modération en dehors de la surface de l’API publique.

Les raccourcis de slugs Web sont résolus entre les familles de registres, mais les clients API doivent utiliser
...
- Requêtes anonymes : appliquées par adresse IP.
...
- Les points de terminaison d’écriture authentifiés ne doivent pas renvoyer un simple `Unauthorized` lorsque
  le serveur connaît la raison. Les jetons absents, les jetons invalides/révoqués et les
  comptes supprimés/bannis/désactivés doivent chacun recevoir un texte exploitable afin que les clients
...
- Utilisez une attente exponentielle avec gigue pour éviter les nouvelles tentatives synchronisées.
- Si `Retry-After` est absent, revenez à `RateLimit-Reset` (ou calculez depuis `X-RateLimit-Reset`).

Source IP :

- Utilise des en-têtes d’IP client de confiance, dont `cf-connecting-ip`, uniquement lorsque le
...
- Si aucune IP client de confiance n’est disponible, les requêtes anonymes utilisent des compartiments de secours
  limités uniquement par le type de limite de débit. Ces compartiments de secours n’incluent pas
  les chemins, slugs, noms de packages, versions, chaînes de requête fournis par l’appelant, ni d’autres
...
- Les résultats sont renvoyés par ordre de pertinence (similarité d’embedding + boosts pour correspondances exactes de jetons de slug/nom + un faible a priori de popularité).
- La pertinence est plus forte que la popularité. Une correspondance précise de slug ou de jeton de nom d’affichage peut dépasser une correspondance plus approximative avec un engagement bien plus fort.
...
- La popularité est mise à l’échelle logarithmique et plafonnée. Les Skills à fort engagement peuvent être moins bien classés lorsque le texte de la requête correspond moins bien.
...
- Ne renommez pas un slug uniquement pour viser une requête, sauf si le nouveau slug est un meilleur nom canonique à long terme. Les anciens slugs deviennent des alias de redirection, mais l’URL canonique, le slug affiché et les futurs condensés de recherche utilisent le nouveau slug.
- Les alias de renommage conservent la résolution pour les anciennes URL et les installations qui se résolvent via le registre, mais le classement de recherche repose sur les métadonnées canoniques du Skill une fois le renommage indexé. Les statistiques existantes restent associées au Skill.
...
- Les anciens slugs créés par les flux de renommage/fusion de propriétaires sont résolus vers le Skill canonique.
...
Point de terminaison modérateur/administrateur pour l’arrivée des signalements de Skills.
...
Point de terminaison modérateur/administrateur pour résoudre ou rouvrir les signalements de Skills.
...
- `version.security` inclut l’état normalisé de vérification par analyse et les détails du scanner
...
- `moderation` est un instantané de modération actuel au niveau de la compétence, dérivé de la dernière version.
...
- Les tâches d’analyse sont asynchrones. Les requêtes d’analyse manuelle sont prioritaires par rapport au travail normal de publication/remplissage rétroactif, mais leur achèvement dépend toujours de la disponibilité des workers.
...
- Renvoie `queue.queuedAhead` et `queue.position` pendant la mise en file d’attente afin que les clients puissent afficher le nombre d’analyses manuelles prioritaires avant la requête. Les files très volumineuses sont bornées et signalées avec `queuedAheadIsEstimate: true`.
- Lorsque disponible, `report` contient les sections `clawscan`, `skillspector`, `staticAnalysis` et `virustotal`.
...
- Nécessite un accès de gestion propriétaire/éditeur à la compétence ou au plugin, ou une autorité de modérateur/administrateur de la plateforme.
...
Route canonique de relance d’analyse par lot, réservée aux administrateurs. Elle accepte la même forme de charge utile que l’ancien `POST /api/v1/skills/-/rescan-batch`.
...
- L’identité de la compétence, l’identité de l’éditeur et les métadonnées de la version sélectionnée sont des champs de premier niveau de l’enveloppe (`slug`, `displayName`, `publisherHandle`, `version`, `resolvedFrom`, `tag`, `createdAt`) afin que l’automatisation shell puisse les lire sans décompresser de wrappers imbriqués.
...
- `security.signals` contient les preuves de scanner de support comme `staticScan`, `virusTotal` et `skillSpector`.
...
- `provenance` vaut `server-resolved-github-import` uniquement lorsque ClawHub a résolu et stocké un dépôt/ref/commit/chemin GitHub lors de la publication ou de l’import ; sinon, il vaut `unavailable`.

### `POST /api/v1/skills/-/security-verdicts`

Renvoie les verdicts de sécurité compacts actuels pour des versions exactes de compétences. Ce
point de terminaison de collection est destiné aux clients qui savent déjà quelles versions de compétences
...
- Les résultats sont fournis par élément ; une compétence ou une version manquante ne fait pas échouer toute la réponse.
- La réponse est limitée à la sécurité. Elle n’inclut pas les données de Skill Card, l’état de carte générée, les listes de fichiers d’artefact ni les charges utiles détaillées des scanners.
- `security.signals` contient uniquement des preuves de support au niveau de l’état ; utilisez `/scan` ou la page d’audit de sécurité ClawHub pour les détails complets des scanners.
- `security.signals.dependencyRegistry` est conservé pour la compatibilité des réponses v1, mais le scanner d’existence du registre de dépendances est retiré et cette clé vaut toujours `null`.
...
- Utilisez `/verify` lorsque vous avez besoin de l’enveloppe de vérification Skill Card pour une seule compétence, `/card` lorsque vous avez besoin du Markdown de carte généré, et `/scan` lorsque vous avez besoin de données détaillées de scanner.
...
- Les entrées de Skills restent adossées au registre de Skills et peuvent toujours être publiées uniquement via `POST /api/v1/skills`.
...
Parcours du catalogue limité aux Plugins, entre packages code-plugin et bundle-plugin.
...
- `startDate` (obligatoire) : borne inférieure en millisecondes Unix pour `updatedAt` de Skill.
- `endDate` (obligatoire) : borne supérieure en millisecondes Unix pour `updatedAt` de Skill.
...
- Corps : archive ZIP.
- Chaque Skill exporté est enraciné dans `{publisher}/{slug}/`.
...
- `_manifest.json` est toujours inclus à la racine du ZIP.
- `_errors.json` est inclus lorsque des Plugins ou fichiers individuels n’ont pas
  pu être exportés.
...
Recherche limitée aux Plugins parmi les packages code-plugin et bundle-plugin.
...
- Les alias de filtres v1 hérités documentés sous `GET /api/v1/plugins` sont aussi
  acceptés.
- Le filtrage par catégorie est un véritable filtre d’API adossé aux lignes de
  condensé de catégorie de Plugin, et non une réécriture de requête de recherche.
- Les résultats sont renvoyés par ordre de pertinence et ne sont pas paginés pour
  le moment.
- Les contrôles de tri de l’interface navigateur pour la recherche de Plugins
  réordonnent les résultats de pertinence chargés, conformément au comportement
  actuel de navigation de `/skills`.
...
- Nécessite un jeton d’API pour le propriétaire du package, un propriétaire/admin
  de l’éditeur d’organisation, un modérateur de plateforme ou un admin de plateforme.
...
Renvoie une version de package, y compris les métadonnées de fichiers, la
compatibilité, la vérification, les métadonnées d’artefact et les données d’analyse.
...
- Les versions ClawPack incluent les champs compatibles npm `npmIntegrity`, `npmShasum` et
  `npmTarballName`.
- `version.sha256hash` est une métadonnée de compatibilité dépréciée pour les anciens clients. Elle
...
Renvoie le résumé exact de sécurité et de confiance de la version du package pour les
clients d’installation. Il s’agit de la surface de consommation publique d’OpenClaw pour décider si une
version résolue peut être installée.

Authentification :

- Point de terminaison de lecture public. Aucun jeton de propriétaire, d’éditeur, de modérateur ou d’admin
  n’est requis.
...
- `trust.stale` signifie que le résumé de confiance a été calculé à partir d’entrées obsolètes et
  doit être considéré comme nécessitant une actualisation avant une décision d’autorisation à forte confiance.

Notes :

- Ce point de terminaison est exact à la version. Les clients doivent l’appeler après avoir résolu la
  version du package qu’ils ont l’intention d’installer, et non seulement après avoir lu les dernières
...
Renvoie l’état de préparation calculé pour la future consommation OpenClaw.
...
Point de terminaison admin pour créer ou mettre à jour une ligne de migration de Plugin officiel.
...
Point de terminaison modérateur/admin pour les files de revue des versions de package.
...
Signaler un package pour revue par un modérateur. Les rapports sont au niveau du package, éventuellement
...
- Utilise par défaut la dernière version.
- Utilise le compartiment de limite de lecture, pas le compartiment de téléchargement.
...
Télécharge l’archive ZIP déterministe héritée d’une version de paquet.
...
- Les métadonnées uniquement registre ne sont pas injectées dans l’archive téléchargée.
...
- `dist.tarball`, `dist.integrity` et `dist.shasum` utilisent des champs
  compatibles npm afin que les utilisateurs puissent faire pointer npm vers le miroir s’ils le souhaitent.
...
- Utilise le compartiment de limite de téléchargement.
...
Télécharge un ZIP de version de Skill hébergée, ou renvoie un transfert vers une source GitHub pour un
Skill actuel adossé à GitHub avec une analyse `clean` ou `suspicious` et sans version
hébergée.
...
- Les transferts de Skill adossée à GitHub ne proxifient ni ne mettent en miroir les octets. La réponse JSON
  ...
  de réussite.
- Les statistiques de téléchargement sont comptabilisées comme identités uniques par jour UTC (`userId` lorsque le jeton d’API est valide, sinon l’IP).
...
- Recommandé : `multipart/form-data` avec JSON `payload` + blobs `files[]`.
...
- Champ facultatif de charge utile : `migrateOwner`. Lorsque `true` avec `ownerHandle`, un
  Skill existant peut être déplacé vers ce propriétaire si l’acteur est admin/propriétaire chez les deux
...
- Les Plugins de code nécessitent `package.json`, des métadonnées de dépôt source, des métadonnées de commit
  source, des métadonnées de schéma de configuration, `openclaw.compat.pluginApi` et
  `openclaw.build.openclawVersion`.
- `openclaw.hostTargets` et `openclaw.environment` sont des métadonnées facultatives.
- Seul l’éditeur de l’organisation `openclaw` et les éditeurs personnels des membres actuels de l’organisation `openclaw`
  peuvent publier vers le canal `official`.
...
Suppression réversible / restauration d’un Skill (propriétaire, modérateur ou admin).
...
Réservé aux admins. Garantit qu’un éditeur d’organisation existe pour un identifiant. Si l’identifiant pointe encore vers un
...
Réservé aux admins. Réserve des slugs racine et des noms de paquet pour un propriétaire légitime sans publier de
...
sans modifier les lignes de compte Convex Auth. La requête doit nommer les deux identifiants de compte
fournisseur GitHub immuables ; les identifiants modifiables ne sont utilisés que comme garde-fou destiné à l’opérateur.
...
les alias de slug de Skill, les packages, les avertissements de l’inspecteur de packages et les lignes de résumé de recherche dérivées, afin que
...
Les sources de Skills GitHub sont limitées au périmètre de l’éditeur et signalées comme vérifiées plutôt que réécrites.
...
- Les deux points de terminaison nécessitent une authentification par jeton d’API et fonctionnent uniquement pour le propriétaire de la Skill.
...
Bannir un utilisateur et supprimer définitivement les Skills détenues (modérateur/administrateur uniquement).
