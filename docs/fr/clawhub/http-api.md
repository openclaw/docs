...
x-i18n:
    generated_at: "2026-07-03T02:46:30Z"
    model: gpt-5.5
...
Les répertoires tiers peuvent utiliser les points de terminaison publics en lecture pour lister ou rechercher des Skills ClawHub. Veuillez mettre les résultats en cache, respecter `429`/`Retry-After`, rediriger les utilisateurs vers la fiche ClawHub canonique (`https://clawhub.ai/<owner>/skills/<slug>`) et éviter de suggérer une approbation de ClawHub pour le site tiers. N’essayez pas de dupliquer du contenu masqué, privé ou bloqué par la modération en dehors de la surface de l’API publique.
...
- Les points de terminaison d’écriture authentifiés ne doivent pas renvoyer un simple `Unauthorized` lorsque
  le serveur connaît la raison. Les jetons manquants, les jetons invalides ou révoqués, et
...
- Les résultats sont renvoyés par ordre de pertinence (similarité d’embedding + boosts de jetons de slug/nom exacts + faible prior de popularité).
- La pertinence est plus forte que la popularité. Une correspondance précise de slug ou de jeton de nom d’affichage peut devancer une correspondance plus vague avec un engagement nettement plus élevé.
- Le texte ASCII est segmenté sur les limites de mots et de ponctuation. Par exemple, `personal-map` contient un jeton autonome `map`, tandis que `amap-jsapi-skill` contient `amap`, `jsapi` et `skill` ; une recherche de `map` donne donc à `personal-map` une correspondance lexicale plus forte qu’à `amap-jsapi-skill`.
- La popularité est mise à l’échelle logarithmique et plafonnée. Les Skills à fort engagement peuvent être moins bien classés lorsque le texte de la requête correspond moins précisément.
...
- Ne renommez pas un slug seulement pour cibler une requête, sauf si le nouveau slug constitue un meilleur nom canonique à long terme. Les anciens slugs deviennent des alias de redirection, mais l’URL canonique, le slug affiché et les futurs résumés de recherche utilisent le nouveau slug.
- Les alias de renommage préservent la résolution des anciennes URL et des installations résolues via le registre, mais le classement de recherche est basé sur les métadonnées canoniques du Skill après l’indexation du renommage. Les statistiques existantes restent associées au Skill.
- Si un Skill est invisiblement inattendu, vérifiez d’abord l’état de modération avec `clawhub inspect @owner/slug` en étant connecté avant de modifier les métadonnées liées au classement.
...
- Les anciens slugs créés par les flux de renommage ou de fusion de propriétaire sont résolus vers le Skill canonique.
...
- Les preuves sont expurgées pour les appelants publics et n’incluent des extraits bruts que pour les propriétaires et modérateurs.
...
Point de terminaison modérateur/administrateur pour la réception des signalements de Skills.
...
redéfinition de `status` sur `open`. Passez `finalAction: "hide"` avec un signalement trié
...
- Les charges utiles des requêtes d’analyse et les rapports téléchargeables expirent du stockage des requêtes d’analyse après la fenêtre de rétention.
...
- Renvoie `queue.queuedAhead` et `queue.position` pendant que la requête est en file d’attente afin que les clients puissent afficher combien d’analyses manuelles prioritaires précèdent la requête. Les très grandes files sont bornées et signalées avec `queuedAheadIsEstimate: true`.
...
- L’identité de la Skill, l’identité de l’éditeur et les métadonnées de la version sélectionnée sont des champs de premier niveau de l’enveloppe (`slug`, `displayName`, `publisherHandle`, `version`, `resolvedFrom`, `tag`, `createdAt`) afin que les automatisations shell puissent les lire sans décompresser des wrappers imbriqués.
...
- `security.signals` contient des preuves de scanner complémentaires comme `staticScan`, `virusTotal` et `skillSpector`.
...
- Les résultats sont fournis par élément ; une Skill ou version manquante ne fait pas échouer toute la réponse.
...
- `security.signals.dependencyRegistry` est conservé pour la compatibilité de réponse v1, mais le scanner d’existence du registre de dépendances est retiré et cette clé vaut toujours `null`.
- L’absence de Skill Card n’affecte pas les valeurs `ok`, `decision` ou `reasons` de ce point d’entrée ; les clients doivent lire localement le `skill-card.md` installé lorsqu’ils ont besoin du contenu de la carte.
...
- `POST /api/v1/packages` reste réservé aux publications de plugins de code et de plugins de bundle.
...
Parcours du catalogue limité aux plugins, couvrant les paquets de plugins de code et de plugins de bundle.
...
- `startDate` (obligatoire) : limite inférieure en millisecondes Unix pour le `updatedAt` du Plugin.
- `endDate` (obligatoire) : limite supérieure en millisecondes Unix pour le `updatedAt` du Plugin.
- `limit` (facultatif) : entier (1-250), valeur par défaut `250`.
...
- Les alias de filtres v1 hérités documentés sous `GET /api/v1/plugins` sont également
  acceptés.
- Le filtrage par catégorie est un véritable filtre d’API adossé aux lignes de digest
...
- Nécessite un jeton d’API pour le propriétaire du package, un propriétaire/admin d’éditeur d’organisation,
...
- Ce point de terminaison est exact à la version. Les clients doivent l’appeler après avoir résolu la
  version de package qu’ils ont l’intention d’installer, et non seulement après avoir lu les dernières
...
- Ce point de terminaison est intentionnellement plus étroit que les points de terminaison de modération
  ...
  les identités des rapporteurs, les corps de rapport, les preuves privées ni les chronologies
  internes d’examen.
...
- Les versions de packages héritées renvoient un artefact `legacy-zip` et une
...
- `packageName` est normalisé comme nom npm ; le package peut être absent pour les migrations
  planifiées.
- Ceci suit uniquement l’état de préparation de la migration. Il ne modifie pas OpenClaw et ne génère pas
  de ClawPacks.
...
- Nécessite un token d’API pour un utilisateur modérateur ou administrateur.
...
- Nécessite un token d’API pour le propriétaire du paquet, un membre de l’éditeur, un modérateur ou
...
- Les transferts de skills basées sur GitHub ne proxifient ni ne mettent en miroir les octets. La réponse JSON
  ...
  et `archiveUrl` ; l’état de l’analyse/état courant sert de garde et n’est pas inclus comme métadonnée de charge utile de succès.
- Les statistiques de téléchargement sont comptées comme identités uniques par jour UTC (`userId` lorsque le token d’API est valide, sinon IP).

## Points de terminaison d’authentification (token Bearer)
...
Valide le token et renvoie le handle de l’utilisateur.
...
- Les champs de formulaire autorisés sont `payload`, des blobs `files` répétés, ou une référence d’archive tar `clawpack`.
  ...
  le flux upload-url. Les publications avec identifiant de stockage en attente doivent aussi inclure le
...
- Les requêtes de publication multipart directes sont limitées à 18 Mo. Les archives tar ClawPack peuvent
  utiliser le flux upload-url jusqu’à la limite de 120 Mo pour les archives tar.
...
Réservé aux administrateurs. Récupère un éditeur personnel pour un principal de remplacement GitHub OAuth vérifié
sans modifier les lignes de compte Convex Auth. La requête doit nommer les deux identifiants immuables de compte
fournisseur GitHub ; les handles modifiables ne sont utilisés que comme garde côté opérateur.

Par défaut, le point de terminaison fonctionne en simulation. L’application de la récupération nécessite `dryRun: false` et
`confirmIdentityVerified: true` après que l’équipe a vérifié indépendamment la continuité entre les deux
identités GitHub principales. La récupération échoue de manière fermée lorsque l’éditeur personnel actuel
de l’utilisateur de destination possède des skills, des packages ou des sources de skill GitHub.
La récupération migre aussi les anciens champs `ownerUserId` pour les skills, les alias de slug de skill,
les packages, les avertissements de l’inspecteur de packages et les lignes de digest de recherche dérivées de l’éditeur récupéré, afin que
...
pour le handle récupéré est aussi réattribuée à l’utilisateur de remplacement, afin qu’une synchronisation ultérieure
...
Les sources de skill GitHub sont limitées au périmètre de l’éditeur et signalées comme vérifiées plutôt que réécrites.
...
### Points de terminaison de gestion du slug de propriétaire
...
le contenu (administrateur uniquement). Fonctionne en simulation par défaut, sauf si `dryRun` vaut `false`.
