...
x-i18n:
    generated_at: "2026-07-02T08:14:53Z"
    model: gpt-5.5
...
- Les points de terminaison d’écriture authentifiés ne doivent pas renvoyer un simple `Unauthorized` lorsque
  le serveur connaît la raison. Les jetons manquants, jetons invalides/révoqués et
...
- La pertinence est plus forte que la popularité. Une correspondance précise de slug ou de jeton de nom affiché peut dépasser une correspondance plus approximative avec un engagement beaucoup plus fort.
- Le texte ASCII est découpé en jetons sur les frontières de mots et de ponctuation. Par exemple, `personal-map` contient un jeton autonome `map`, tandis que `amap-jsapi-skill` contient `amap`, `jsapi` et `skill` ; rechercher `map` donne donc à `personal-map` une correspondance lexicale plus forte qu’à `amap-jsapi-skill`.
...
- Les alias de renommage préservent la résolution des anciennes URL et des installations qui passent par le registre, mais le classement de recherche est basé sur les métadonnées canoniques de la skill après indexation du renommage. Les statistiques existantes restent associées à la skill.
- Si une skill est étonnamment invisible, vérifiez d’abord l’état de modération avec `clawhub inspect @owner/slug` en étant connecté avant de modifier les métadonnées liées au classement.
...
Signaler une skill pour examen par les modérateurs. Les signalements se font au niveau de la skill, peuvent facultativement être liés
...
définition de `status` à nouveau sur `open`. Passez `finalAction: "hide"` avec un signalement trié
...
- Les charges utiles de demande d’analyse et les rapports téléchargeables expirent du magasin des demandes d’analyse après la fenêtre de rétention.
...
- Renvoie `queue.queuedAhead` et `queue.position` pendant l’attente afin que les clients puissent afficher combien d’analyses manuelles prioritaires précèdent la demande. Les très grandes files sont bornées et signalées avec `queuedAheadIsEstimate: true`.
...
Route canonique réservée aux administrateurs pour la nouvelle analyse par lot. Elle accepte la même forme de charge utile que l’ancienne route `POST /api/v1/skills/-/rescan-batch`.

### `POST /api/v1/skills/-/scan/batch/status`

Route canonique réservée aux administrateurs pour l’état de lot. Elle accepte `{ "jobIds": ["..."] }` et renvoie les mêmes compteurs agrégés que l’ancienne route `POST /api/v1/skills/-/rescan-batch/status`.
...
- `security.signals` contient les éléments de preuve de scanner complémentaires tels que `staticScan`, `virusTotal` et `skillSpector`.
- `security.signals.dependencyRegistry` est conservé pour la compatibilité de réponse v1, mais le scanner d’existence de registre de dépendances est retiré et cette clé vaut toujours `null`.
...
- `security.signals` contient uniquement des éléments de preuve de prise en charge au niveau de l’état ; utilisez `/scan` ou la page d’audit de sécurité ClawHub pour les détails complets des scanners.
- `security.signals.dependencyRegistry` est conservé pour la compatibilité de réponse v1, mais le scanner d’existence de registre de dépendances est retiré et cette clé vaut toujours `null`.
...
- Les entrées de Skill restent adossées au registre des Skills et peuvent toujours être publiées uniquement via `POST /api/v1/skills`.
...
- `isOfficial` (facultatif) : `true` ou `false`
- `category` (facultatif) : filtre de catégorie de Plugin. Pris en charge uniquement lorsque la
  requête est limitée aux packages de Plugin. Les catégories contrôlées et les alias de filtres
...
- `startDate` (obligatoire) : limite inférieure en millisecondes Unix pour le champ `updatedAt` du Plugin.
- `endDate` (obligatoire) : limite supérieure en millisecondes Unix pour le champ `updatedAt` du Plugin.
- `limit` (facultatif) : entier (1-250), valeur par défaut `250`.
- `cursor` (facultatif) : curseur de pagination issu de la réponse précédente.
- `family` (facultatif) : `code-plugin` ou `bundle-plugin`. Si omis, inclut les deux
  familles de Plugin.
...
- Les métadonnées d’exportation par Plugin sont stockées à
...
Recherche réservée aux Plugins dans les packages `code-plugin` et `bundle-plugin`.
...
d’installation. Il s’agit de la surface de consommation publique OpenClaw permettant de décider si une
...
- Ce point de terminaison est volontairement plus restreint que les points de terminaison de modération
  destinés aux propriétaires/modérateurs. Il expose la décision d’installation et l’explication publique, et non
...
- Il s’agit de la surface de résolution OpenClaw ; elle évite de deviner le format d’archive depuis
  une URL partagée.
...
- `all` : toute version avec une substitution manuelle, un état d’analyse non propre ou un signalement de package.
...
Signale un package pour revue par un modérateur. Les signalements se situent au niveau du package, avec un lien facultatif
vers une version. Ils alimentent la file de modération, mais ne masquent pas automatiquement les téléchargements et ne les
bloquent pas par eux-mêmes ; les modérateurs doivent utiliser la modération des versions pour
...
- Nécessite un jeton d’API pour le propriétaire du package, un membre de l’éditeur,
  un modérateur ou un utilisateur admin.
...
- Les versions héritées limitées au ZIP sont intentionnellement omises.
...
Télécharge un ZIP de version de skill hébergée, ou renvoie un relais vers une source GitHub pour une
...
- Les relais de skills adossées à GitHub ne font ni proxy ni miroir des octets. La réponse JSON
  inclut `sourceRef: "public-github"`, `repo`, `commit`, `path`, `contentHash`
  et `archiveUrl` ; l’état d’analyse/actuel est une condition d’accès et n’est pas inclus comme métadonnée
...
- Les requêtes de publication multipart directe sont plafonnées à 18 Mo. Les tarballs ClawPack peuvent
...
propriétaire puisse ensuite publier la vraie version code-plugin ou bundle-plugin sous ce nom.
...
de fournisseur GitHub immuables ; les handles mutables ne sont utilisés que comme garde-fou visible par l’opérateur.
