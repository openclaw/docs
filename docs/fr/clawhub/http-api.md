...
summary: Référence de l’API HTTP (publique + points de terminaison CLI + authentification).
x-i18n:
    generated_at: "2026-07-03T09:32:49Z"
    model: gpt-5.5
...
Les répertoires tiers peuvent utiliser les points de terminaison publics en lecture pour lister ou rechercher des Skills ClawHub. Veuillez mettre les résultats en cache, respecter `429`/`Retry-After`, renvoyer les utilisateurs vers la fiche ClawHub canonique (`https://clawhub.ai/<owner>/skills/<slug>`) et éviter de suggérer une approbation du site tiers par ClawHub. N’essayez pas de dupliquer du contenu masqué, privé ou bloqué par la modération en dehors de la surface de l’API publique.

Les raccourcis de slug Web se résolvent entre les familles de registre, mais les clients API doivent utiliser
les URL canoniques renvoyées par les points de terminaison en lecture au lieu de reconstruire la
priorité des routes.
...
- Si `Retry-After` existe, attendez autant de secondes avant de réessayer.
...
- Si aucune IP cliente de confiance n’est disponible, les requêtes anonymes utilisent des compartiments de repli
  ...
  paramètres d’artifact fournis par l’appelant.
...
- Les résultats sont renvoyés par ordre de pertinence (similarité d’embedding + boosts des jetons exacts de slug/nom + un faible a priori de popularité).
- La pertinence est plus forte que la popularité. Une correspondance précise de slug ou de jeton de nom affiché peut dépasser une correspondance plus approximative avec un engagement beaucoup plus élevé.
...
- La popularité est mise à l’échelle logarithmiquement et plafonnée. Les Skills à fort engagement peuvent être moins bien classées lorsque le texte de la requête correspond moins bien.
...
- Les alias de renommage préservent la résolution pour les anciennes URL et les installations qui se résolvent via le registre, mais le classement de recherche est basé sur les métadonnées canoniques de la Skill une fois le renommage indexé. Les statistiques existantes restent associées à la Skill.
- Si une Skill est invisibile de manière inattendue, vérifiez d’abord l’état de modération avec `clawhub inspect @owner/slug` en étant connecté avant de modifier les métadonnées liées au classement.
...
- Les anciens slugs créés par les flux de renommage/fusion du propriétaire se résolvent vers la Skill canonique.
...
Point de terminaison modérateur/admin pour l’admission des signalements de Skills.
...
- Inclut un statut de vérification normalisé ainsi que des détails propres au scanner.
...
- `moderation` est un instantané actuel de modération au niveau de la Skills, dérivé de la dernière version.
...
- Les analyses publiées réécrivent les résultats uniquement lorsque `update: true` et que l’analyse se termine correctement.
...
- Renvoie le statut queued/running/succeeded/failed.
...
- Nécessite un accès de gestion propriétaire/éditeur à la Skills ou au plugin, ou une autorité de modérateur/administrateur de la plateforme.
...
Route canonique de statut de lot réservée aux administrateurs. Elle accepte `{ "jobIds": ["..."] }` et renvoie les mêmes compteurs agrégés que l’ancienne route `POST /api/v1/skills/-/rescan-batch/status`.
...
- `ok` vaut `true` uniquement lorsque la version sélectionnée possède une Skill Card générée, n’est pas bloquée comme malware par la modération, et que la vérification ClawScan est clean.
- L’identité de la Skills, l’identité de l’éditeur et les métadonnées de la version sélectionnée sont des champs de premier niveau de l’enveloppe (`slug`, `displayName`, `publisherHandle`, `version`, `resolvedFrom`, `tag`, `createdAt`) afin que l’automatisation shell puisse les lire sans déballer des wrappers imbriqués.
...
- `security.signals` contient les preuves de scanner complémentaires, comme `staticScan`, `virusTotal` et `skillSpector`.
...
- `provenance` vaut `server-resolved-github-import` uniquement lorsque ClawHub a résolu et stocké un dépôt/ref/commit/chemin GitHub lors de la publication ou de l’import ; sinon, elle vaut `unavailable`.
...
- Les résultats sont par élément ; une Skills ou une version manquante ne fait pas échouer toute la réponse.
- La réponse concerne uniquement la sécurité. Elle n’inclut pas les données Skill Card, le statut de carte générée, les listes de fichiers d’artefacts ni les charges utiles détaillées des scanners.
- `security.signals` contient uniquement des preuves complémentaires au niveau du statut ; utilisez `/scan` ou la page d’audit de sécurité ClawHub pour obtenir tous les détails des scanners.
- `security.signals.dependencyRegistry` est conservé pour la compatibilité de réponse v1, mais le scanner d’existence du registre de dépendances est retiré et cette clé est toujours `null`.
...
- Utilisez `/verify` lorsque vous avez besoin de l’enveloppe de vérification Skill Card pour une seule Skills, `/card` lorsque vous avez besoin du markdown de carte généré, et `/scan` lorsque vous avez besoin de données de scanner détaillées.
...
Recherche de catalogue unifié parmi les Skills et les packages de plugins.
...
Exportation groupée des dernières versions publiques de plugins pour analyse hors ligne.
...
- `family` (facultatif) : `code-plugin` ou `bundle-plugin`. Son omission signifie les deux
  familles de plugins.
...
- Les anciens alias de filtre v1 documentés sous `GET /api/v1/plugins` sont également
  acceptés.
- Le filtrage par catégorie est un véritable filtre d’API fondé sur des lignes de condensé
...
- Les contrôles de tri de l’interface navigateur pour la recherche de plugins réordonnent les résultats de pertinence chargés,
  ce qui correspond au comportement de navigation `/skills` actuel.
...
- Les Skills peuvent aussi être résolues par cette route dans le catalogue unifié.
...
d’installation. C’est la surface de consommation publique OpenClaw permettant de décider si une
...
- Ce point de terminaison est volontairement plus restreint que les points de terminaison de modération
  pour propriétaires/modérateurs. Il expose la décision d’installation et l’explication publique, et non
  les identités des rapporteurs, les corps de signalement, les preuves privées ou les calendriers de revue
  internes.

### `GET /api/v1/packages/{name}/versions/{version}/artifact`

Renvoie les métadonnées explicites du résolveur d’artéfact pour une version de package.

Notes :

- Les anciennes versions de package renvoient un artéfact `legacy-zip` et une URL ZIP héritée
  `downloadUrl`.
...
Point de terminaison de modérateur/administrateur pour les files de revue des versions de packages.
...
- `all` : toute version avec une substitution manuelle, un état d’analyse non sain ou un signalement de package.
...
Signaler un package pour revue par un modérateur. Les signalements sont au niveau du package, facultativement
...
Les versions mises en quarantaine et révoquées renvoient `403` depuis les routes de téléchargement d’artifacts.
...
- Utilise par défaut la dernière version.
- Utilise le bucket de limite de lecture, pas le bucket de téléchargement.
...
- Les packuments de packages scopés prennent en charge à la fois `/api/npm/@scope/name` et le chemin de requête
  encodé npm `/api/npm/@scope%2Fname`.

### `GET /api/npm/{package}/-/{tarball}.tgz`

Diffuse les octets exacts de l’archive tar ClawPack téléversée pour les clients du miroir npm.

Notes :

- Utilise le bucket de limite de téléchargement.
...
- Les transferts de Skill adossée à GitHub ne proxifient ni ne mettent en miroir les octets. La réponse JSON
  ...
  et `archiveUrl` ; l’état d’analyse/actuel est une barrière et n’est pas inclus comme métadonnée de
  payload de réussite.
...
- Champ facultatif du payload : `ownerHandle`. Lorsqu’il est présent, l’API résout cet
  éditeur côté serveur et exige que l’acteur ait accès à l’éditeur.
- Champ facultatif du payload : `migrateOwner`. Lorsque `true` avec `ownerHandle`, une
  Skill existante peut être déplacée vers ce propriétaire si l’acteur est administrateur/propriétaire à la fois sur
  l’éditeur actuel et l’éditeur cible. Sans cette option explicite, les changements de propriétaire sont
  rejetés.
...
- Les champs de formulaire autorisés sont `payload`, les blobs `files` répétés, ou une référence d’archive tar `clawpack`.
  `clawpack` peut être un blob `.tgz` ou un id de stockage renvoyé par
  le flux upload-url. Les publications via id de stockage préparé doivent également inclure le
...
- Les requêtes de publication multipart directe sont plafonnées à 18 Mo. Les archives tar ClawPack peuvent
  utiliser le flux upload-url jusqu’au plafond de 120 Mo par archive tar.
- Champ facultatif du payload : `ownerHandle`. Lorsqu’il est présent, seuls les administrateurs peuvent publier au nom de ce propriétaire.
...
- Les publications pour le compte d’un tiers valident tout de même l’éligibilité au canal officiel par rapport au compte du propriétaire cible.
...
Les suppressions réversibles initiées par le propriétaire réservent le slug pendant 30 jours, après quoi le slug peut être réclamé par
...
version. Les noms de packages deviennent des packages indicateurs privés sans lignes de version, afin que le même
...
sans modifier les lignes de compte Convex Auth. La requête doit nommer les deux ids immuables de compte fournisseur GitHub ;
les handles mutables ne sont utilisés que comme garde-fou destiné à l’opérateur.

Le point de terminaison utilise la simulation par défaut. L'application de la récupération exige `dryRun: false` et
...
les alias de slug de Skills, les packages, les avertissements de l'inspecteur de packages et les lignes de condensé de recherche dérivées afin que
les chemins de propriétaire direct correspondent à la nouvelle autorité de l'éditeur. Une réservation active de handle protégé
pour le handle récupéré est également réattribuée à l'utilisateur de remplacement afin que la synchronisation ultérieure du profil
...
qui préparent une archive tarball ClawPack doivent envoyer l'identifiant de stockage obtenu comme
