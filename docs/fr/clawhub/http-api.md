...
x-i18n:
    generated_at: "2026-07-03T13:26:54Z"
    model: gpt-5.5
...
Les répertoires tiers peuvent utiliser les points de terminaison publics en lecture pour lister ou rechercher des Skills ClawHub. Veuillez mettre les résultats en cache, respecter `429`/`Retry-After`, rediriger les utilisateurs vers la fiche ClawHub canonique (`https://clawhub.ai/<owner>/skills/<slug>`) et éviter de laisser entendre que ClawHub approuve le site tiers. N’essayez pas de dupliquer du contenu masqué, privé ou bloqué par la modération en dehors de la surface de l’API publique.
...
- Utilisez un repli exponentiel avec gigue pour éviter les nouvelles tentatives synchronisées.
...
- ClawHub utilise les en-têtes de transfert approuvés pour identifier les IP client en périphérie.
...
- Les résultats sont renvoyés par ordre de pertinence (similarité d’intégration + bonus pour correspondances exactes de jetons slug/nom + un faible a priori de popularité).
- La pertinence est plus forte que la popularité. Une correspondance précise de slug ou de jeton de nom d’affichage peut dépasser une correspondance plus vague avec un engagement beaucoup plus élevé.
...
- La popularité est mise à l’échelle logarithmique et plafonnée. Les Skills à fort engagement peuvent être moins bien classées lorsque le texte de la requête correspond plus faiblement.
...
- Placez les termes que les utilisateurs rechercheront littéralement dans le nom d’affichage, le résumé et les étiquettes. Utilisez un jeton de slug autonome uniquement lorsqu’il s’agit aussi d’une identité stable que vous voulez conserver.
...
- `sort` (facultatif) : `updated` (par défaut), `recommended` (alias : `default`), `createdAt` (alias : `newest`), `downloads`, `stars` (alias : `rating`), les alias d’installation historiques `installsCurrent`/`installs`/`installsAllTime` correspondent à `downloads`, `trending`
- `nonSuspiciousOnly` (facultatif) : `true` pour masquer les Skills suspectes (`flagged.suspicious`)
- `nonSuspicious` (facultatif) : alias historique de `nonSuspiciousOnly`
...
- Lorsque `nonSuspiciousOnly=true`, les tris par curseur peuvent renvoyer moins de `limit` éléments sur une page, car les Skills suspectes sont filtrées après la récupération de la page.
...
- Les anciens slugs créés par des flux de renommage/fusion de propriétaire se résolvent vers la Skill canonique.
- `metadata.os` : restrictions d’OS déclarées dans le frontmatter de la Skill (par exemple `["macos"]`, `["linux"]`). `null` si elles ne sont pas déclarées.
...
Signaler une Skill pour examen par les modérateurs. Les signalements se font au niveau de la Skill, sont éventuellement liés
...
- Les analyses publiées écrivent en retour uniquement lorsque `update: true` et que l’analyse se termine avec succès.
...
- Les tâches d’analyse sont asynchrones. Les requêtes d’analyse manuelles sont prioritaires sur le travail normal de publication/remplissage, mais leur achèvement dépend toujours de la disponibilité des workers.
...
- Renvoie `queue.queuedAhead` et `queue.position` pendant la mise en file afin que les clients puissent afficher combien d’analyses manuelles prioritaires précèdent la requête. Les files très volumineuses sont bornées et signalées avec `queuedAheadIsEstimate: true`.
...
- La réponse porte uniquement sur la sécurité. Elle n’inclut pas les données de Skill Card, l’état de carte générée, les listes de fichiers d’artefact ni les charges utiles détaillées des scanners.
...
- Utilisez `/verify` lorsque vous avez besoin de l’enveloppe de vérification de Skill Card pour une seule Skill, `/card` lorsque vous avez besoin du Markdown de carte généré, et `/scan` lorsque vous avez besoin de données de scanner détaillées.
...
- Limite de taille de fichier : 200KB.
...
- Les appelants authentifiés peuvent voir les packages privés pour les éditeurs auxquels ils appartiennent dans les résultats de liste/recherche.
- `channel=private` renvoie uniquement les packages que l’appelant authentifié peut lire.

### `GET /api/v1/packages/search`

Recherche de catalogue unifiée sur les Skills et les packages de plugins.
...
- `isOfficial` (facultatif) : `true` ou `false`
- `category` (facultatif) : filtre de catégorie de plugin. Pris en charge uniquement lorsque la
  requête est limitée aux packages de plugins. Les catégories contrôlées et les alias de filtres hérités de la v1
...
- Les appelants anonymes ne voient que les canaux de packages publics.
- Les appelants authentifiés peuvent rechercher des packages privés pour les éditeurs auxquels ils appartiennent.
- `channel=private` renvoie uniquement les packages que l’appelant authentifié peut lire.
...
- Les Skills actuelles adossées à GitHub avec un scan `clean` ou `suspicious` incluent
  `_source_handoff.json` avec `sourceRef: "public-github"`, le dépôt, le commit, le chemin,
  le hachage de contenu et l’URL de l’archive. Elles n’incluent pas les fichiers sources hébergés par ClawHub.
...
- Les alias de filtre hérités de v1 documentés sous `GET /api/v1/plugins` sont aussi
  acceptés.
- Le filtrage par catégorie est un vrai filtre d’API appuyé par les lignes de condensé
...
- Nécessite un jeton API pour le propriétaire du package, un propriétaire/admin d’éditeur d’organisation,
  un modérateur de plateforme ou un admin de plateforme.
...
- `version.artifact.kind` vaut `legacy-zip` pour les anciennes archives de package ou
  `npm-pack` pour les versions appuyées par ClawPack.
...
- Point de terminaison de lecture public. Aucun jeton de propriétaire, d’éditeur, de modérateur ou d’admin n’est
  requis.
...
- Ce point de terminaison est exact par version. Les clients doivent l’appeler après avoir résolu la
  version du package qu’ils prévoient d’installer, et pas seulement après avoir lu les dernières
...
- Ce point de terminaison est volontairement plus restreint que les points de terminaison de modération
  ...
  les identités des signalants, les corps de rapports, les preuves privées ni les chronologies de revue
  internes.

### `GET /api/v1/packages/{name}/versions/{version}/artifact`

Renvoie les métadonnées explicites de résolution d’artefact pour une version de package.
...
- Les versions ClawPack diffusent les octets `.tgz` exacts de l’npm-pack téléversé.
...
- Nécessite un jeton API pour un utilisateur modérateur ou admin.
...
- Nécessite un jeton API pour un utilisateur admin.
...
- `open` : versions suspectes, malveillantes, en attente, en quarantaine, révoquées ou signalées.
- `blocked` : versions en quarantaine, révoquées ou malveillantes.
...
Signale un package pour revue par les modérateurs. Les signalements sont au niveau du package, facultativement
...
Point de terminaison propriétaire/modérateur pour la visibilité de la modération de packages.

Authentification :

- Nécessite un jeton API pour le propriétaire du package, un membre éditeur, un modérateur ou
...
Télécharge l’archive ZIP déterministe historique pour une version de package.
...
- Les versions historiques uniquement ZIP sont intentionnellement omises.
...
- Les transferts de skills basées sur GitHub ne proxifient ni ne reflètent les octets. La réponse JSON
  inclut `sourceRef: "public-github"`, `repo`, `commit`, `path`, `contentHash`
  et `archiveUrl` ; l’état d’analyse/actuel est une porte de validation et n’est pas inclus comme métadonnée de charge utile
  de succès.
...
- Les code plugins nécessitent `package.json`, les métadonnées de dépôt source, les métadonnées de commit source,
...
- Les publications au nom d’un tiers valident toujours l’éligibilité au canal officiel par rapport au compte propriétaire cible.
...
l’appelant comme propriétaire. Ce point de terminaison ne migre pas les handles d’utilisateur/personnels existants et ne
...
sans modifier les lignes de compte Convex Auth. La requête doit nommer les deux ids de compte de fournisseur GitHub
immuables ; les handles modifiables sont uniquement utilisés comme garde-fou destiné à l’opérateur.

Par défaut, le point de terminaison est en mode simulation. L’application de la récupération nécessite `dryRun: false` et
...
possède des compétences, des packages ou des sources de compétence GitHub.
La récupération migre aussi les champs hérités `ownerUserId` pour les compétences de l’éditeur récupéré,
les alias de slug de compétence, les packages, les avertissements de l’inspecteur de package et les lignes dérivées de résumé de recherche afin que
...
Les sources de compétence GitHub sont délimitées par éditeur et signalées comme vérifiées plutôt que réécrites.
...
Bannir un utilisateur et supprimer définitivement les compétences possédées (modérateur/admin uniquement).
...
Lever le bannissement d’un utilisateur et restaurer les compétences éligibles (admin uniquement).
...
Modifier la raison stockée pour un bannissement existant sans lever le bannissement ni restaurer
le contenu (admin uniquement). Par défaut, fonctionne en mode simulation sauf si `dryRun` vaut `false`.
