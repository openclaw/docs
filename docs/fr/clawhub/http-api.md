...
Les répertoires tiers peuvent utiliser les points de terminaison publics en lecture pour lister ou rechercher des skills ClawHub. Veuillez mettre les résultats en cache, respecter `429`/`Retry-After`, renvoyer les utilisateurs vers la fiche canonique ClawHub (`https://clawhub.ai/<owner>/<slug>`) et éviter de suggérer une approbation de ClawHub pour le site tiers. N’essayez pas de répliquer du contenu masqué, privé ou bloqué par la modération en dehors de la surface de l’API publique.
...
- Si `Retry-After` est absent, retombez sur `RateLimit-Reset` (ou calculez à partir de `X-RateLimit-Reset`).
...
- La pertinence est plus forte que la popularité. Une correspondance précise avec un slug ou un jeton de nom d’affichage peut devancer une correspondance plus approximative avec beaucoup plus de téléchargements.
...
- Les téléchargements sont utilisés comme un petit a priori à échelle logarithmique et comme critère de départage, pas comme signal de classement principal. Les skills très téléchargées peuvent être moins bien classées lorsque le texte de la requête correspond moins fortement.
...
- Les alias de renommage préservent la résolution pour les anciennes URL et les installations qui passent par le registre, mais le classement de recherche repose sur les métadonnées canoniques de la skill après indexation du renommage. Les statistiques existantes restent associées à la skill.
- Si une skill est invisiblement masquée de manière inattendue, vérifiez d’abord l’état de modération avec `clawhub inspect <slug>` en étant connecté avant de modifier les métadonnées liées au classement.
...
- Les éléments probants sont expurgés pour les appelants publics et n’incluent des extraits bruts que pour les propriétaires/modérateurs.
...
Point de terminaison modérateur/admin pour les files d’examen des publications de packages.
...
Signale un package pour examen par un modérateur. Les signalements portent sur le package, avec un lien
facultatif vers une version. Ils alimentent la file de modération, mais ne masquent ni ne
bloquent automatiquement les téléchargements à eux seuls ; les modérateurs doivent utiliser la modération des publications pour
...
Point de terminaison de maintenance réservé aux admins pour étiqueter les anciennes versions de paquet avec
...
- Les packuments de paquets scopés prennent en charge à la fois `/api/npm/@scope/name` et le chemin de requête
...
- Recommandé : `multipart/form-data` avec le JSON `payload` + des blobs `files[]`.
...
- Champ facultatif de payload : `migrateOwner`. Lorsqu’il vaut `true` avec `ownerHandle`, une
  skill existante peut être déplacée vers ce propriétaire si l’acteur est admin/propriétaire à la fois sur
  les éditeurs actuel et cible. Sans cet opt-in, les changements de propriétaire sont
  rejetés.
...
- Les paquets Plugin exigent `openclaw.plugin.json`. Les téléversements ClawPack `.tgz` doivent
...
Les suppressions logiques initiées par le propriétaire réservent le slug pendant 30 jours, après quoi le slug peut être revendiqué par
...
- Les deux points de terminaison exigent une authentification par jeton API et ne fonctionnent que pour le propriétaire de la skill.
...
Bannit un utilisateur et supprime définitivement les skills détenues (modérateur/admin uniquement).
...
- `limit` (facultatif) : résultats maximum (20 par défaut, 200 max)
...
Ajoute/supprime une étoile (favoris). Les deux points de terminaison sont idempotents.
