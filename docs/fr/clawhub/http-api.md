...
Les répertoires tiers peuvent utiliser les points de terminaison publics de lecture pour lister ou rechercher des Skills ClawHub. Veuillez mettre les résultats en cache, respecter `429`/`Retry-After`, renvoyer les utilisateurs vers la fiche ClawHub canonique (`https://clawhub.ai/<owner>/skills/<slug>`) et éviter de suggérer une approbation de ClawHub pour le site tiers. N’essayez pas de répliquer du contenu masqué, privé ou bloqué par modération en dehors de la surface de l’API publique.
...
- Utilisez un délai d’attente exponentiel avec jitter pour éviter les nouvelles tentatives synchronisées.
- Si `Retry-After` est absent, revenez à `RateLimit-Reset` (ou calculez à partir de `X-RateLimit-Reset`).

Source IP :

- Utilise les en-têtes IP client fiables, y compris `cf-connecting-ip`, uniquement lorsque le
  déploiement active explicitement les en-têtes transférés fiables.
- ClawHub utilise les en-têtes de transfert fiables pour identifier les IP client à la périphérie.
- Si aucune IP client fiable n’est disponible, les requêtes anonymes utilisent des buckets de repli
  limités uniquement au type de limite de débit. Ces buckets de repli n’incluent pas
...
- Les résultats sont renvoyés par ordre de pertinence (similarité d’embedding + boosts de jetons exacts de slug/nom + un léger a priori de popularité).
...
- Mettez les termes que les utilisateurs rechercheront littéralement dans le nom d’affichage, le résumé et les tags. N’utilisez un jeton de slug autonome que lorsqu’il s’agit aussi d’une identité stable que vous souhaitez conserver.
- Ne renommez pas un slug seulement pour viser une requête, sauf si le nouveau slug est un meilleur nom canonique à long terme. Les anciens slugs deviennent des alias de redirection, mais l’URL canonique, le slug affiché et les futurs résumés de recherche utilisent le nouveau slug.
- Les alias de renommage préservent la résolution pour les anciennes URL et les installations qui passent par le registre, mais le classement de recherche est basé sur les métadonnées canoniques de la Skill après indexation du renommage. Les statistiques existantes restent associées à la Skill.
- Si une Skill est invisiblement masquée de manière inattendue, vérifiez d’abord l’état de modération avec `clawhub inspect @owner/slug` en étant connecté avant de modifier les métadonnées liées au classement.
...
- Les propriétaires et les modérateurs peuvent accéder aux détails de modération pour les Skills masquées.
...
redéfinition de `status` sur `open`. Passez `finalAction: "hide"` avec un signalement traité
...
- Les charges utiles de demande d’analyse et les rapports téléchargeables expirent du magasin des demandes d’analyse après la fenêtre de conservation.
...
- Les tâches d’analyse sont asynchrones. Les demandes d’analyse manuelle sont prioritaires par rapport aux tâches normales de publication/remplissage rétroactif, mais leur achèvement dépend toujours de la disponibilité des workers.
...
- Renvoie `queue.queuedAhead` et `queue.position` pendant la file d’attente afin que les clients puissent afficher le nombre d’analyses manuelles prioritaires devant la demande. Les très grandes files d’attente sont bornées et signalées avec `queuedAheadIsEstimate: true`.
...
- `kind` vaut `skill` par défaut ; utilisez `kind=plugin` pour les analyses de plugin/paquet.
- Renvoie la même forme de ZIP que les téléchargements de demandes d’analyse.

### `POST /api/v1/skills/-/scan/batch`

Route canonique de relance d’analyse par lot réservée aux administrateurs. Elle accepte la même forme de charge utile que l’ancienne route `POST /api/v1/skills/-/rescan-batch`.
...
Renvoie l’enveloppe de vérification de la Skill Card utilisée par `clawhub skill verify`.
...
- `provenance` vaut `server-resolved-github-import` uniquement lorsque ClawHub a résolu et stocké un dépôt/réf/commit/chemin GitHub pendant la publication ou l’importation ; sinon, elle vaut `unavailable`.
...
- La réponse concerne uniquement la sécurité. Elle n’inclut pas les données de Skill Card, l’état de carte générée, les listes de fichiers d’artefact ni les charges utiles détaillées des scanners.
...
- Utilisez `/verify` lorsque vous avez besoin de l’enveloppe de vérification de Skill Card pour une seule Skill, `/card` lorsque vous avez besoin du Markdown de carte généré, et `/scan` lorsque vous avez besoin de données détaillées de scanner.
...
- Les entrées de skill restent adossées au registre de skills et ne peuvent toujours être publiées que via `POST /api/v1/skills`.
...
- `category` (facultatif) : filtre de catégorie de Plugin. Pris en charge uniquement lorsque la
  ...
  v1 hérités sont documentés sous `GET /api/v1/plugins`.
...
Navigation dans le catalogue réservé aux Plugins pour les packages code-plugin et bundle-plugin.
...
- Les alias de filtres v1 hérités documentés sous `GET /api/v1/plugins` sont également
  acceptés.
- Le filtrage par catégorie est un vrai filtre d’API appuyé par des lignes de condensé de catégories
  de plugins, et non une réécriture de requête de recherche.
...
- Skills peuvent aussi être résolus par cette route dans le catalogue unifié.
...
- Nécessite un jeton API pour le propriétaire du package, un propriétaire/administrateur d’éditeur d’organisation,
...
- `version.vtAnalysis`, `version.llmAnalysis` et `version.staticScan` sont
  inclus lorsque les données d’analyse existent.
- Les packages privés renvoient `404`, sauf si l’appelant peut lire l’éditeur propriétaire.
...
- `trust.stale` signifie que le résumé de confiance a été calculé à partir d’entrées obsolètes et
  devrait être traité comme nécessitant une actualisation avant une décision d’autorisation à haut niveau de confiance.
...
- Ce point de terminaison est volontairement plus étroit que les points de terminaison de modération
  ...
  les identités des signalants, les corps de rapports, les preuves privées ou les chronologies de revue
  internes.
...
- `all` : toute version avec une dérogation manuelle, un état d’analyse non sain ou un signalement de package.
...
Point de terminaison modérateur/admin pour résoudre ou rouvrir des rapports de package.
...
Télécharge une version ZIP de skill hébergée, ou renvoie un transfert de source GitHub pour un
...
- Les transferts de skill adossés à GitHub ne proxient ni ne mettent en miroir les octets. La réponse JSON
  inclut `sourceRef: "public-github"`, `repo`, `commit`, `path`, `contentHash`
  et `archiveUrl` ; l’état d’analyse/actuel est une porte de validation et n’est pas inclus comme métadonnée de payload
  de réussite.
...
- Les champs de formulaire autorisés sont `payload`, les blobs `files` répétés, ou une référence de tarball `clawpack`.
  `clawpack` peut être un blob `.tgz` ou un id de stockage renvoyé par le
  flux upload-url. Les publications d’id de stockage préparé doivent également inclure le
...
Supprime de manière réversible / restaure un skill (propriétaire, modérateur ou admin).
...
ancien éditeur utilisateur/personnel partagé, le point de terminaison le migre d’abord en éditeur d’organisation.
...
Les sources de compétences GitHub sont liées à l’éditeur et signalées comme vérifiées plutôt que réécrites.
...
- Les deux points de terminaison exigent une authentification par jeton d’API et ne fonctionnent que pour le propriétaire de la compétence.
...
Lever le bannissement d’un utilisateur et restaurer les compétences admissibles (admin uniquement).
