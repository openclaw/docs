...
x-i18n:
    generated_at: "2026-07-02T17:35:21Z"
    model: gpt-5.5
...
Les annuaires tiers peuvent utiliser les points de terminaison publics en lecture pour lister ou rechercher les Skills ClawHub. Veuillez mettre les résultats en cache, respecter `429`/`Retry-After`, renvoyer les utilisateurs vers la fiche ClawHub canonique (`https://clawhub.ai/<owner>/skills/<slug>`) et éviter de suggérer une approbation de ClawHub pour le site tiers. N’essayez pas de dupliquer du contenu masqué, privé ou bloqué par la modération en dehors de la surface d’API publique.
...
- Les points de terminaison d’écriture authentifiés ne doivent pas renvoyer un simple `Unauthorized` lorsque
  le serveur en connaît la raison. Les jetons manquants, les jetons invalides/révoqués et
...
- Utilisez une temporisation de repli avec jitter pour éviter les nouvelles tentatives synchronisées.
...
doivent lire le corps de réponse comme une chaîne lisible par un humain. Les paramètres de requête inconnus sont
...
- Les résultats sont renvoyés par ordre de pertinence (similarité d’embedding + renforcements exacts sur les jetons slug/nom + un léger prior de popularité).
- La pertinence est plus forte que la popularité. Une correspondance précise sur un slug ou un jeton de nom d’affichage peut dépasser une correspondance plus approximative avec un engagement beaucoup plus fort.
...
- La popularité est mise à l’échelle logarithmique et plafonnée. Les Skills à fort engagement peuvent être moins bien classés lorsque le texte de requête correspond moins fortement.
...
- Les alias de renommage préservent la résolution des anciennes URL et des installations qui se résolvent via le registre, mais le classement de recherche repose sur les métadonnées canoniques du Skill après l’indexation du renommage. Les statistiques existantes restent associées au Skill.
...
Signaler un Skill pour examen par un modérateur. Les signalements s’appliquent au niveau du Skill, sont éventuellement liés
...
pour masquer le Skill dans le même flux de travail auditável.
...
- Les charges utiles des requêtes d’analyse et les rapports téléchargeables expirent du magasin de requêtes d’analyse après la fenêtre de rétention.
...
- Les tâches d’analyse sont asynchrones. Les requêtes d’analyse manuelles sont priorisées avant le travail normal de publication/remplissage rétroactif, mais leur achèvement dépend toujours de la disponibilité des workers.
...
- `kind` utilise `skill` par défaut ; utilisez `kind=plugin` pour les analyses de Plugin/package.
- Renvoie la même forme ZIP que les téléchargements de requêtes d’analyse.

### `POST /api/v1/skills/-/scan/batch`

Route canonique de réanalyse par lot réservée aux administrateurs. Elle accepte la même forme de charge utile que l’ancien `POST /api/v1/skills/-/rescan-batch`.
...
- L’identité de la skill, l’identité de l’éditeur et les métadonnées de la version sélectionnée sont des champs de premier niveau de l’enveloppe (`slug`, `displayName`, `publisherHandle`, `version`, `resolvedFrom`, `tag`, `createdAt`) afin que l’automatisation shell puisse les lire sans décompresser des enveloppes imbriquées.
...
- `security.signals` contient des preuves de scanner justificatives telles que `staticScan`, `virusTotal` et `skillSpector`.
...
Renvoie les verdicts de sécurité compacts actuels pour des versions de skills exactes. Ce
...
- `security.signals` contient uniquement des preuves justificatives au niveau de l’état ; utilisez `/scan` ou la page d’audit de sécurité ClawHub pour obtenir les détails complets des scanners.
- `security.signals.dependencyRegistry` est conservé pour la compatibilité de réponse v1, mais le scanner d’existence du registre de dépendances est retiré et cette clé vaut toujours `null`.
...
- `isOfficial` (facultatif) : `true` ou `false`
- `category` (facultatif) : filtre de catégorie de plugin. Pris en charge uniquement lorsque la
  requête est limitée aux packages de plugins. Les catégories contrôlées et les alias de filtres v1
...
- Les alias de filtres v1 hérités documentés sous `GET /api/v1/plugins` sont également
  acceptés.
- Le filtrage par catégorie est un vrai filtre d’API appuyé par les lignes de synthèse
  des catégories de plugins, pas une réécriture de requête de recherche.
...
Point de terminaison modérateur pour lister les lignes de migration des plugins officiels OpenClaw.
...
Signale un package pour revue par les modérateurs. Les signalements sont au niveau du package, éventuellement
...
Point de terminaison modérateur/administrateur pour la réception des rapports de packages.
...
- Nécessite un jeton API pour le propriétaire du package, un membre éditeur, un modérateur ou un
...
Point de terminaison modérateur/administrateur pour résoudre ou rouvrir les rapports de packages.
...
Point de terminaison modérateur/administrateur pour l’examen d’une release de package.
...
- Les transferts de skills adossées à GitHub ne proxifient ni ne mettent en miroir les octets. La réponse JSON
  ...
  et `archiveUrl` ; l’état d’analyse/actuel est une porte de validation et n’est pas inclus comme métadonnée
  de succès de la charge utile.
- Les statistiques de téléchargement sont comptées comme identités uniques par jour UTC (`userId` lorsque le jeton API est valide, sinon l’IP).
...
- Champ facultatif de charge utile : `ownerHandle`. Lorsqu’il est présent, l’API résout cet
  éditeur côté serveur et exige que l’acteur ait un accès éditeur.
...
- Les champs de formulaire autorisés sont `payload`, des blobs `files` répétés, ou une référence d’archive tar
  ...
  le flux upload-url. Les publications avec storage-id préparé doivent également inclure le
...
- Les requêtes directes de publication multipart sont limitées à 18 Mo. Les archives tar ClawPack peuvent
...
Réservé aux administrateurs. Récupère un éditeur personnel pour un principal OAuth GitHub de remplacement vérifié
...
les alias de slug de Skills, les paquets, les avertissements de l'inspecteur de paquets et les lignes de résumé de recherche dérivées, afin que
...
du profil ne puisse pas restaurer l'autorité concurrente de l'ancien utilisateur. Chaque table primaire est limitée à
...
qui préparent une archive tarball ClawPack doivent envoyer l'identifiant de stockage résultant comme
