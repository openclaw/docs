...
x-i18n:
    generated_at: "2026-07-02T14:01:55Z"
    model: gpt-5.5
...
- Les résultats sont renvoyés par ordre de pertinence (similarité d’embedding + bonus pour correspondances exactes de jetons de slug/nom + faible a priori de popularité).
...
à une version, et alimentent la file de signalements des Skills.
...
- Lors de la requête d’une version historique, vérifiez `moderation.matchesRequestedVersion` et `moderation.sourceVersion` avant de traiter `moderation` et `security` comme appartenant au même contexte de version.
...
- Renvoie `queue.queuedAhead` et `queue.position` pendant la mise en file d’attente afin que les clients puissent afficher le nombre d’analyses manuelles prioritaires devant la requête. Les très grandes files sont bornées et signalées avec `queuedAheadIsEstimate: true`.
...
- L’identité de la skill, l’identité de l’éditeur et les métadonnées de la version sélectionnée sont des champs d’enveloppe de premier niveau (`slug`, `displayName`, `publisherHandle`, `version`, `resolvedFrom`, `tag`, `createdAt`) afin que l’automatisation shell puisse les lire sans dépaqueter d’enveloppes imbriquées.
...
- `provenance` vaut `server-resolved-github-import` uniquement lorsque ClawHub a résolu et stocké un dépôt/ref/commit/chemin GitHub pendant la publication ou l’importation ; sinon, sa valeur est `unavailable`.

### `POST /api/v1/skills/-/security-verdicts`

Renvoie les verdicts de sécurité compacts actuels pour les versions exactes de skills. Ce
...
- `category` (facultatif) : filtre de catégorie de Plugin. Pris en charge uniquement lorsque la
  ...
  alias de filtres historiques v1 sont documentés sous `GET /api/v1/plugins`.
...
- `category` (facultatif) : filtre de catégorie de Plugin. Pris en charge uniquement lorsque la
  ...
  de filtres historiques v1 sont documentés sous `GET /api/v1/plugins`.
...
Parcours du catalogue réservé aux Plugins, couvrant les packages code-plugin et bundle-plugin.
...
Les alias de filtres historiques v1 restent acceptés sur les points de terminaison de lecture :
...
Export en masse des Skills publiques les plus récentes pour analyse hors ligne.
...
- Les Skills actuelles adossées à GitHub avec une analyse `clean` ou `suspicious` incluent
  ...
  le hachage de contenu et l’URL de l’archive. Elles n’incluent pas les fichiers source hébergés par ClawHub.
...
- Les contrôles de tri de l’interface du navigateur pour la recherche de Plugins réordonnent les résultats pertinents chargés,
...
- `trust.scanStatus` est le statut de confiance effectif dérivé des entrées des scanners
...
- `trust.pending` signifie qu’une ou plusieurs entrées de confiance sont toujours en attente de finalisation.
...
- Ce point de terminaison est exact à la version. Les clients doivent l’appeler après avoir résolu la
  version de package qu’ils ont l’intention d’installer, et pas seulement après avoir lu les dernières
...
- Utilise le compartiment de limitation du taux de téléchargement.
...
Point de terminaison modérateur/administrateur pour la réception des signalements de package.
...
- Les transferts de Skills basées sur GitHub ne proxyfient ni ne mettent en miroir les octets. La réponse JSON
...
- Les statistiques de téléchargement sont comptées comme des identités uniques par jour UTC (`userId` quand le jeton d’API est valide, sinon l’IP).
...
- Champ de charge utile facultatif : `migrateOwner`. Quand `true` avec `ownerHandle`, une
...
- Les champs de formulaire autorisés sont `payload`, les blobs `files` répétés ou une référence de tarball `clawpack`.
  ...
  le flux upload-url. Les publications par storage-id préparé doivent aussi inclure le
...
Les masquages modérateur/administrateur et les suppressions de sécurité n’expirent pas ainsi.
...
Le point de terminaison utilise par défaut un essai à blanc. L’application de la récupération nécessite `dryRun: false` et
...
La récupération migre également les champs hérités `ownerUserId` pour les skills du publisher récupéré,
les alias de slug de skill, les packages, les avertissements de l’inspecteur de package et les lignes de résumé de recherche dérivées, afin que
...
Les sources de skill GitHub sont liées au publisher et signalées comme vérifiées plutôt que réécrites.
...
le contenu (admin uniquement). Utilise par défaut un essai à blanc sauf si `dryRun` vaut `false`.
