...
summary: Référence de l’API HTTP (points de terminaison publics + CLI + auth).
x-i18n:
    generated_at: "2026-07-02T22:28:24Z"
    model: gpt-5.5
...
Les annuaires tiers peuvent utiliser les points de terminaison publics en lecture pour lister ou rechercher les Skills ClawHub. Veuillez mettre les résultats en cache, respecter `429`/`Retry-After`, renvoyer les utilisateurs vers la fiche ClawHub canonique (`https://clawhub.ai/<owner>/skills/<slug>`) et éviter de laisser entendre que ClawHub approuve le site tiers. Ne tentez pas de répliquer du contenu masqué, privé ou bloqué par modération hors de la surface de l’API publique.
...
- `X-RateLimit-Remaining` / `RateLimit-Remaining` : budget restant exact lorsqu’il est présent.
  Les requêtes réussies distribuées par shards omettent cet en-tête au lieu de renvoyer une valeur globale approximative.
...
- Utilise les en-têtes d’IP client approuvés, notamment `cf-connecting-ip`, uniquement lorsque le
...
- Les résultats sont renvoyés par ordre de pertinence (similarité d’embedding + bonus pour correspondance exacte de token de slug/nom + un léger a priori de popularité).
- La pertinence est plus forte que la popularité. Une correspondance précise sur un slug ou un token de nom d’affichage peut dépasser une correspondance plus vague avec beaucoup plus d’engagement.
- Le texte ASCII est découpé en tokens sur les limites de mots et de ponctuation. Par exemple, `personal-map` contient un token autonome `map`, tandis que `amap-jsapi-skill` contient `amap`, `jsapi` et `skill` ; rechercher `map` donne donc à `personal-map` une correspondance lexicale plus forte que `amap-jsapi-skill`.
...
- Placez les termes que les utilisateurs rechercheront littéralement dans le nom d’affichage, le résumé et les tags. N’utilisez un token de slug autonome que lorsqu’il s’agit aussi d’une identité stable que vous voulez conserver.
...
- Si un Skill est invisiblement de façon inattendue, vérifiez d’abord l’état de modération avec `clawhub inspect @owner/slug` en étant connecté avant de modifier les métadonnées liées au classement.
...
- `trending` classe selon les installations au cours des 7 derniers jours (basé sur la télémétrie).
...
- Les anciens slugs créés par des flux de renommage/fusion de propriétaires se résolvent vers le Skill canonique.
...
Signaler un Skill pour examen par les modérateurs. Les signalements sont au niveau du Skill, éventuellement liés
...
Point de terminaison modérateur/administrateur pour l’ingestion des signalements de Skills.
...
- `version.security` inclut l’état de vérification d’analyse normalisé et les détails des scanners
...
- Inclut un état de vérification normalisé ainsi que des détails propres au scanner.
...
- Les charges utiles des requêtes de scan et les rapports téléchargeables expirent du magasin des requêtes de scan après la fenêtre de rétention.
...
- Les scans publiés réécrivent les données uniquement lorsque `update: true` et que le scan se termine avec succès.
...
- Les tâches de scan sont asynchrones. Les requêtes de scan manuel sont prioritaires par rapport au travail normal de publication/remplissage, mais leur achèvement dépend toujours de la disponibilité des workers.
...
- Renvoie `queue.queuedAhead` et `queue.position` pendant la mise en file afin que les clients puissent afficher combien de scans manuels prioritaires précèdent la requête. Les très grandes files sont bornées et signalées avec `queuedAheadIsEstimate: true`.
...
- Renvoie la même forme de ZIP que les téléchargements de requêtes de scan.

### `POST /api/v1/skills/-/scan/batch`

Route canonique de rescan par lot réservée aux administrateurs. Elle accepte la même forme de charge utile que l’ancien `POST /api/v1/skills/-/rescan-batch`.
...
- L’identité du Skill, l’identité de l’éditeur et les métadonnées de la version sélectionnée sont des champs d’enveloppe de premier niveau (`slug`, `displayName`, `publisherHandle`, `version`, `resolvedFrom`, `tag`, `createdAt`) afin que l’automatisation shell puisse les lire sans décompresser de wrappers imbriqués.
- `security` est le verdict ClawScan/sécurité de premier niveau. L’automatisation doit s’appuyer sur `ok`, `decision`, `reasons` et `security.status`.
- `security.signals` contient des preuves de scanner à l’appui, comme `staticScan`, `virusTotal` et `skillSpector`.
...
Renvoie les verdicts de sécurité compacts actuels pour des versions exactes de Skill. Ce
...
- Les résultats sont par élément ; un Skill ou une version manquants ne font pas échouer toute la réponse.
- La réponse concerne uniquement la sécurité. Elle n’inclut pas les données de Skill Card, l’état de carte générée, les listes de fichiers d’artéfact ni les charges utiles détaillées des scanners.
- `security.signals` contient uniquement des preuves à l’appui au niveau de l’état ; utilisez `/scan` ou la page d’audit de sécurité ClawHub pour les détails complets des scanners.
- `security.signals.dependencyRegistry` est conservé pour la compatibilité de réponse v1, mais le scanner d’existence du registre de dépendances est retiré et cette clé vaut toujours `null`.
- L’absence de Skill Card n’affecte pas `ok`, `decision` ou `reasons` pour ce point de terminaison ; les clients doivent lire localement le `skill-card.md` installé lorsqu’ils ont besoin du contenu de la carte.
- Utilisez `/verify` lorsque vous avez besoin de l’enveloppe de vérification Skill Card d’un seul Skill, `/card` lorsque vous avez besoin du Markdown de carte générée, et `/scan` lorsque vous avez besoin de données de scanner détaillées.
...
- `category` (facultatif) : filtre de catégorie de plugin. Pris en charge uniquement lorsque la
  ...
  les alias de filtres historiques de la v1 sont documentés sous `GET /api/v1/plugins`.
...
- `isOfficial` (facultatif) : `true` ou `false`
- `category` (facultatif) : filtre de catégorie de plugin. Pris en charge uniquement lorsque la
  requête est limitée aux packages de plugins. Les catégories contrôlées et les alias de filtres historiques de la v1
  sont documentés sous `GET /api/v1/plugins`.
...
Les alias de filtres historiques de la v1 restent acceptés sur les points de terminaison de lecture :
...
- Les Skills actuelles adossées à GitHub avec une analyse `clean` ou `suspicious` incluent
  ...
  hachage du contenu et URL de l’archive. Elles n’incluent pas les fichiers source hébergés par ClawHub.
...
- Les alias de filtres v1 hérités documentés sous `GET /api/v1/plugins` sont également
  acceptés.
- Le filtrage par catégorie est un vrai filtre d’API appuyé par des lignes de condensé de catégorie
  de Plugin, et non une réécriture de requête de recherche.
- Les résultats sont renvoyés par ordre de pertinence et ne sont pas paginés actuellement.
- Les contrôles de tri de l’interface du navigateur pour la recherche de Plugin réordonnent les résultats de pertinence chargés,
  ce qui correspond au comportement de navigation actuel de `/skills`.
...
- Skills peut aussi se résoudre via cette route dans le catalogue unifié.
...
- Nécessite un jeton API pour le propriétaire du package, un propriétaire/administrateur de l’éditeur d’organisation,
...
- Il s’agit de la surface de résolveur d’OpenClaw ; elle évite de deviner le format d’archive à partir
  d’une URL partagée.
...
- `manual` : toute version avec un remplacement de modération manuel.
- `all` : toute version avec un remplacement manuel, un état d’analyse non propre ou un rapport de package.
...
Signaler un package pour revue par les modérateurs. Les rapports sont au niveau du package, avec un lien facultatif
...
Point de terminaison modérateur/administrateur pour l’ingestion des rapports de paquets.
...
Point de terminaison modérateur/administrateur pour résoudre ou rouvrir les rapports de paquets.
...
même flux de travail auditable.
...
Point de terminaison modérateur/administrateur pour l’examen des versions de paquets.
...
- Les archives Plugin/paquet sont des fichiers zip avec une racine `package/` afin que les anciens clients OpenClaw
...
Diffuse les octets exacts de l’archive tar ClawPack téléversée pour les clients miroir npm.

Remarques :

- Utilise le compartiment de limitation de téléchargement.
...
skill actuelle basée sur GitHub avec une analyse `clean` ou `suspicious` et aucune version hébergée.
...
- Les transferts de skills basées sur GitHub ne proxifient ni ne répliquent les octets. La réponse JSON
  inclut `sourceRef: "public-github"`, `repo`, `commit`, `path`, `contentHash`,
  et `archiveUrl` ; l’état d’analyse/actuel sert de garde et n’est pas inclus comme métadonnée
  de charge utile de réussite.
- Les statistiques de téléchargement sont comptées comme identités uniques par jour UTC (`userId` quand le jeton d’API est valide, sinon IP).
...
- Nécessite l’authentification par jeton Bearer.
...
- Les champs de formulaire autorisés sont `payload`, des blobs `files` répétés, ou une référence d’archive tar `clawpack`.
  ...
  le flux upload-url. Les publications par identifiant de stockage préparé doivent également inclure le
...
- Les requêtes de publication multipart directes sont plafonnées à 18 Mo. Les archives tar ClawPack peuvent
  utiliser le flux upload-url jusqu’au plafond d’archive tar de 120 Mo.
...
Points importants de validation :

- `family` doit être `code-plugin` ou `bundle-plugin`.
- Les paquets Plugin requièrent `openclaw.plugin.json`. Les téléversements ClawPack `.tgz` doivent
  le contenir à `package/openclaw.plugin.json`.
- Les Plugins de code requièrent `package.json`, les métadonnées de dépôt source, les métadonnées de commit source,
...
Les masquages par modérateur/administrateur et les retraits de sécurité n’expirent pas de cette façon.
...
Création d’éditeur d’organisation en libre-service authentifié. Crée un nouvel éditeur d’organisation et ajoute
...
Réservé aux administrateurs. Réserve des slugs racine et noms de paquets pour un propriétaire légitime sans publier de
...
fournisseur GitHub ; les identifiants modifiables ne sont utilisés que comme garde destinée à l’opérateur.
...
identités principales GitHub. La récupération échoue en mode fermé lorsque l’éditeur personnel actuel de l’utilisateur de destination
...
les alias de slug de Skill, les packages, les avertissements d’inspecteur de package et les lignes de résumé de recherche dérivées afin que
les chemins de propriétaire direct concordent avec la nouvelle autorité d’éditeur. Une réservation active de
handle protégé pour le handle récupéré est aussi réattribuée à l’utilisateur de remplacement afin que la
synchronisation ultérieure du profil ne puisse pas restaurer l’autorité concurrente de l’ancien utilisateur. Chaque table principale est limitée à
...
Les sources de Skills GitHub sont scoped à l’éditeur et signalées comme vérifiées plutôt que réécrites.
...
Modifier la raison stockée d’un bannissement existant sans lever le bannissement ni restaurer
le contenu (admin uniquement). Utilise par défaut le mode simulation, sauf si `dryRun` vaut `false`.
...
Ajouter/supprimer une étoile (mises en évidence). Les deux points de terminaison sont idempotents.
