...
x-i18n:
    generated_at: "2026-07-03T17:19:48Z"
    model: gpt-5.5
...
personne n’envoie que quelques requêtes. Connectez-vous si possible et réessayez après le
délai indiqué.
...
Si une publication échoue parce que l’identifiant du propriétaire, l’espace de noms d’organisation, la portée du paquet, le
slug de Skill ou le nom du paquet est déjà revendiqué ou réservé, vérifiez d’abord que vous
publiez avec le propriétaire qui correspond à l’espace de noms. Pour les paquets de plugin,
les noms avec portée tels que `@example-org/example-plugin` doivent être publiés comme le
...
## `update` refuse l’opération en raison de modifications locales
...
- Publier votre copie modifiée comme nouveau slug ou fork.
