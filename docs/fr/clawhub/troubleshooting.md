...
La CLI lance un serveur de rappel local à courte durée de vie pendant la connexion via le navigateur.
...
Les diagnostics visibles par le propriétaire peuvent expliquer l’état de l’analyse, du blocage à l’envoi ou de la modération.
...
Si une publication échoue parce que l’identifiant du propriétaire, l’espace de noms de l’organisation, le périmètre du package, le slug du Skill
ou le nom du package est déjà revendiqué ou réservé, confirmez d’abord que vous publiez
avec le propriétaire correspondant à l’espace de noms. Pour les packages de Plugin,
les noms avec périmètre comme `@example-org/example-plugin` doivent être publiés avec le
...
[ticket de revendication d’organisation / d’espace de noms](https://github.com/openclaw/clawhub/issues/new?template=org-namespace-claim.yml)
avec des preuves publiques non sensibles. Consultez
[Revendications d’organisation et d’espace de noms](/fr/clawhub/namespace-claims) pour des conseils sur les preuves et sur ce
qu’il faut exclure des tickets publics.
...
- Vérifiez la page de détail du package pour l’état de l’analyse et les métadonnées de compatibilité.
- Confirmez que votre version d’OpenClaw satisfait la plage de
  compatibilité annoncée par le package.
