...
- `RateLimit-Remaining` : votre quota restant exact lorsque l'en-tête est présent. En cas de `429`, il vaut `0`.
...
- Vérifiez que la version est publique et qu'elle n'est pas retenue par une analyse ou par la modération.
- Si vous êtes propriétaire de la Skill, connectez-vous et examinez-la :
...
## La publication échoue en raison de métadonnées obligatoires manquantes
...
Pour les plugins, vérifiez les métadonnées de compatibilité de `package.json`. La publication de plugins de code
nécessite des champs de compatibilité OpenClaw tels que `openclaw.compat.pluginApi` et
...
ClawHub utilise l'identité GitHub et l'attribution de la source pour relier les paquets à leurs
éditeurs.

- Assurez-vous d'être connecté avec le compte GitHub qui possède le paquet ou qui peut le publier.
...
slug de la Skill ou le nom du paquet est déjà revendiqué ou réservé, vérifiez d'abord que vous
publiez avec le propriétaire correspondant à l'espace de noms. Pour les paquets de plugins,
les noms délimités tels que `@example-org/example-plugin` doivent être publiés sous le
...
[Revendications d'organisations et d'espaces de noms](/clawhub/namespace-claims) pour obtenir des conseils sur les preuves à fournir et les éléments
à ne pas inclure dans les problèmes publics.

## `sync` indique qu'aucune Skill n'a été trouvée

`sync` recherche les dossiers contenant `SKILL.md` ou `skill.md`.

Indiquez-lui les répertoires racines que vous souhaitez analyser :
...
## `update` refuse de s'exécuter en raison de modifications locales
...
- Écrasez-les avec la version publiée :

```bash
clawhub update @openclaw/demo --force
```

- Publiez votre copie modifiée sous un nouveau slug ou en tant que fork.
...
- Consultez la page de détails du paquet pour connaître l'état de l'analyse et les métadonnées de compatibilité.
- Vérifiez que votre version d'OpenClaw respecte la plage de compatibilité annoncée par le
  paquet.
- Si le paquet est masqué, retenu ou bloqué, il peut rester impossible à installer jusqu'à ce que
  le propriétaire résolve le problème.

## Les requêtes vers l'API publique échouent
...
- Ne reproduisez pas les contenus masqués, privés, retenus ou bloqués par la modération en dehors de la
