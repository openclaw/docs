---
read_when:
    - Les commandes CLI de ClawHub ou de registre OpenClaw échouent
...
x-i18n:
    generated_at: "2026-07-01T15:23:35Z"
    model: gpt-5.5
...
Les diagnostics visibles par le propriétaire peuvent expliquer l’état de l’analyse, de la porte d’envoi ou de la modération.

## La publication échoue parce que des métadonnées obligatoires sont manquantes

Pour les Skills, vérifiez le frontmatter de `SKILL.md`. Les variables d’environnement requises et les
outils doivent être déclarés afin que les utilisateurs et les scanners puissent comprendre le package.
...
ClawHub utilise l’identité GitHub et l’attribution de source pour associer les packages à leurs
éditeurs.
...
Si une publication échoue parce que le handle propriétaire, l’espace de noms d’organisation, la portée du package, le slug de skill
ou le nom du package est déjà revendiqué ou réservé, confirmez d’abord que vous
publiez avec le propriétaire qui correspond à l’espace de noms. Pour les packages de plugins,
...
- Consultez la page de détail du package pour l’état de l’analyse et les métadonnées de compatibilité.
...
- Ne répliquez pas de contenu masqué, privé, retenu ou bloqué par la modération hors de la
