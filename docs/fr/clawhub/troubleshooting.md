...
x-i18n:
    generated_at: "2026-06-30T13:57:29Z"
    model: gpt-5.5
...
- `RateLimit-Remaining` : votre budget restant exact lorsque l’en-tête est présent. Avec `429`, il vaut `0`.
- `RateLimit-Reset` ou `X-RateLimit-Reset` : moment de réinitialisation.

Si de nombreux utilisateurs partagent une seule IP de sortie, les limites d’IP anonymes peuvent être atteintes même lorsque chaque
...
- Confirmez que la publication est publique et qu’elle n’est pas retenue par une analyse ou une modération.
...
Les diagnostics visibles par le propriétaire peuvent expliquer l’état d’analyse, de blocage de téléversement ou de modération.

## La publication échoue car des métadonnées requises sont manquantes
...
ont besoin de champs de compatibilité OpenClaw tels que `openclaw.compat.pluginApi` et
...
Si une publication échoue parce que l’identifiant du propriétaire, l’espace de noms de l’organisation, la portée du paquet, le slug de Skill
ou le nom du paquet est déjà revendiqué ou réservé, confirmez d’abord que vous
publiez avec le propriétaire correspondant à l’espace de noms. Pour les paquets de plugins,
les noms avec portée comme `@example-org/example-plugin` doivent être publiés en tant que
...
[demande de revendication d’organisation / espace de noms](https://github.com/openclaw/clawhub/issues/new?template=org-namespace-claim.yml)
avec des preuves publiques et non sensibles. Consultez
[Revendications d’organisation et d’espace de noms](/clawhub/namespace-claims) pour des conseils sur les preuves et ce
qu’il faut éviter de publier dans les issues publiques.

## `sync` indique qu’aucune Skill n’a été trouvée

`sync` recherche des dossiers contenant `SKILL.md` ou `skill.md`.

Indiquez-lui les racines que vous voulez analyser :
...
- Publier votre copie modifiée sous forme d’un nouveau slug ou d’un fork.
...
- Consultez la page de détail du paquet pour connaître l’état d’analyse et les métadonnées de compatibilité.
...
Consultez [API HTTP](/clawhub/http-api) pour le détail des points de terminaison.
