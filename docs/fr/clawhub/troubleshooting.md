---
read_when:
    - Échec des commandes CLI de ClawHub ou du registre OpenClaw
...
x-i18n:
    generated_at: "2026-07-03T02:47:32Z"
    model: gpt-5.5
...
Si une publication échoue parce que l’identifiant du propriétaire, l’espace de noms de l’organisation, le périmètre du paquet, le slug de Skill
ou le nom du paquet est déjà revendiqué ou réservé, confirmez d’abord que vous
publiez avec le propriétaire qui correspond à l’espace de noms. Pour les paquets Plugin,
les noms avec périmètre tels que `@example-org/example-plugin` doivent être publiés par le
...
- Vérifiez la page de détails du paquet pour l’état de l’analyse et les métadonnées de compatibilité.
