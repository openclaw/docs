---
read_when:
    - Les commandes de la CLI ClawHub ou du registre OpenClaw échouent
...
x-i18n:
    generated_at: "2026-07-03T13:27:45Z"
    model: gpt-5.5
...
La CLI démarre un serveur de rappel local à courte durée de vie pendant la connexion dans le navigateur.
...
personne n’envoie que quelques requêtes. Connectez-vous si possible, puis réessayez après le
délai indiqué.
...
- Confirmez que la version est publique et non retenue par l’analyse ou la modération.
...
ClawHub utilise l’identité GitHub et l’attribution de la source pour relier les packages à leurs
éditeurs.
...
Si une publication échoue parce que l’identifiant du propriétaire, l’espace de noms de l’organisation, la portée du package, le slug de la skill
ou le nom du package est déjà revendiqué ou réservé, confirmez d’abord que vous publiez
avec le propriétaire qui correspond à l’espace de noms. Pour les packages de plugin,
les noms à portée comme `@example-org/example-plugin` doivent être publiés avec le propriétaire
`example-org` correspondant.
...
[issue de revendication d’organisation / espace de noms](https://github.com/openclaw/clawhub/issues/new?template=org-namespace-claim.yml)
avec des preuves publiques et non sensibles. Consultez
[Revendications d’organisation et d’espace de noms](/clawhub/namespace-claims) pour obtenir des conseils sur les preuves et savoir ce qu’il faut
ne pas inclure dans les issues publiques.
...
- Vérifiez la page de détail du package pour l’état d’analyse et les métadonnées de compatibilité.
