...
x-i18n:
    generated_at: "2026-06-28T05:07:55Z"
    model: gpt-5.5
...
- `RateLimit-Remaining` : votre budget restant exact lorsque l’en-tête est présent. En cas de `429`, il vaut `0`.
- `RateLimit-Reset` ou `X-RateLimit-Reset` : moment de réinitialisation.

Si de nombreux utilisateurs partagent une seule adresse IP de sortie, les limites d’IP anonymes peuvent être atteintes même lorsque chaque
...
- Confirmez que la version est publique et n’est pas bloquée par une analyse ou une modération.
...
outils requis doivent être déclarés afin que les utilisateurs et les scanners puissent comprendre le paquet.
...
publiez avec le propriétaire qui correspond à l’espace de noms. Pour les paquets de plugins,
...
[Revendications d’organisation et d’espace de noms](/fr/clawhub/namespace-claims) pour obtenir des conseils sur les preuves et savoir ce
qu’il ne faut pas inclure dans les issues publiques.
...
- Confirmez que votre version d’OpenClaw satisfait la plage de
  compatibilité annoncée par le paquet.
- Si le paquet est masqué, bloqué ou refusé, il peut ne pas être installable tant que
...
- Ne dupliquez pas le contenu masqué, privé, bloqué ou bloqué par modération en dehors de la
