...
x-i18n:
    generated_at: "2026-06-28T05:43:53Z"
    model: gpt-5.5
...
La CLI démarre un serveur de rappel local de courte durée pendant la connexion par navigateur.
...
Les diagnostics visibles par le propriétaire peuvent expliquer l’état d’analyse, de blocage du téléversement ou de modération.
...
nécessitent des champs de compatibilité OpenClaw comme `openclaw.compat.pluginApi` et
...
Si une publication échoue parce que l’identifiant du propriétaire, l’espace de noms d’organisation, la portée du paquet, le slug du Skill
...
les noms avec portée comme `@example-org/example-plugin` doivent être publiés avec le
propriétaire `example-org` correspondant.

Si vous estimez que votre organisation, projet ou marque est le propriétaire légitime de l’espace de noms mais
...
[Revendications d’organisation et d’espace de noms](/fr/clawhub/namespace-claims) pour des indications sur les preuves et sur ce
