...
summary: Comment demander une revue ClawHub pour les litiges de propriété concernant une organisation, une marque, un identifiant de propriétaire, un périmètre de paquet, un slug de Skills ou un espace de noms.
title: Revendications d’organisation et d’espace de noms
x-i18n:
    generated_at: "2026-07-03T02:47:02Z"
    model: gpt-5.5
...
les scopes de packages comme espaces de noms publics. Si un espace de noms semble appartenir à un
...
Utilisez ce parcours pour un examen de propriété public et non sensible. N’utilisez pas les
...
Ouvrez une revendication d’espace de noms lorsque vous estimez que l’équipe ClawHub doit examiner si un
...
- un scope de package tel que `@example-org/*` qui ne devrait être publié que sous le
...
suivez également les consignes de modération ou de sécurité applicables. Le formulaire de revendication d’espace de noms
...
- contrôle de scope npm, PyPI, crates.io ou d’un autre registre de packages
...
Ne placez pas de secrets ni de preuves privées dans une issue GitHub publique. N’incluez pas :
