...
portées de packages comme espaces de noms publics. Si un espace de noms semble appartenir à un
projet, une marque, un écosystème de packages ou une organisation réels, mais qu’il est déjà
revendiqué, réservé, trompeur ou contesté sur ClawHub, demandez à l’équipe de l’examiner
avec le
[formulaire de problème Org / Namespace Claim](https://github.com/openclaw/clawhub/issues/new?template=org-namespace-claim.yml).
...
suivez aussi les consignes de modération ou de sécurité pertinentes. Le formulaire de revendication
...
Pour les packages de Plugins, les noms à portée comme `@example-org/example-plugin` doivent être
publiés sous le propriétaire `example-org` correspondant.

Si vous pouvez gérer le propriétaire actuel, corrigez l’espace de noms directement en publiant,
...
- historique d’organisation GitHub, de dépôt, de publication ou de mainteneur
...
- jetons API, clés de signature ou identifiants
- jetons de défi DNS
