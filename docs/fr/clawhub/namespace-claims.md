...
summary: Comment demander un examen par ClawHub en cas de litige concernant la propriété d’une organisation, d’une marque, d’un identifiant de propriétaire, d’une portée de paquet, d’un slug de skill ou d’un espace de noms.
...
Ouvrez une revendication d’espace de noms si vous estimez que l’équipe de ClawHub doit déterminer si un
...
- une portée de paquet telle que `@example-org/*` qui ne doit être publiée que sous le
...
renommant, transférant, masquant ou supprimant la ressource concernée. Déposez une revendication
lorsque vous ne pouvez pas gérer le propriétaire actuel ou lorsque l’équipe doit résoudre un
litige.

## Preuves à fournir

Utilisez des preuves publiques et non sensibles. Les éléments utiles comprennent :

- l’historique d’une organisation GitHub, d’un dépôt, d’une version ou des responsables de maintenance
...
- les preuves de propriété d’une marque déposée, d’une marque ou d’un projet pouvant être discutées
...
- les liens vers le propriétaire, la Skill, le Plugin, le paquet ou le ticket ClawHub contesté
...
- de fichiers juridiques ou de contrats privés
...
Selon les preuves et les risques, l’équipe de ClawHub peut réserver un espace de noms,
...
L’équipe évalue les preuves publiques, l’utilisation existante, les risques de sécurité et l’impact sur les utilisateurs.
