...
x-i18n:
    generated_at: "2026-05-11T22:19:12Z"
    model: gpt-5.5
    provider: openai
...
Chaque listing public est un enregistrement du registre avec :
...
- des signaux de téléchargement, d’installation, d’étoile et de commentaires
- un état d’analyse de sécurité et de modération

La page du listing est l’endroit canonique où les utilisateurs peuvent inspecter ce qu’un skill ou
...
Utilisez les exécutions à blanc pour prévisualiser la charge utile résolue avant l’envoi. Les pages publiques affichent ensuite
les métadonnées publiées, les fichiers, l’attribution de la source et l’état d’analyse.
...
de mise à jour directe de skills pour les utilisateurs qui veulent des dossiers de skills gérés par le registre en dehors d’un
espace de travail OpenClaw complet.

## État de sécurité

ClawHub est ouvert à la publication, mais les releases restent soumises à des contrôles d’envoi,
des vérifications automatisées, des signalements d’utilisateurs et des actions de modération.

Les pages publiques affichent des résumés d’analyse lorsqu’ils sont disponibles. Le contenu qui est retenu, masqué
ou bloqué peut disparaître de la recherche publique et des flux d’installation tout en restant
visible par le propriétaire pour les diagnostics.
...
téléchargements. Les catalogues tiers peuvent utiliser ces API lorsqu’ils créent un lien vers le
listing canonique ClawHub, respectent les limites de débit et évitent de suggérer une approbation.
