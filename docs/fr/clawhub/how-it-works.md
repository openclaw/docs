...
summary: Fonctionnement des annonces, versions, installations, publications, analyses et mises à jour ClawHub.
x-i18n:
    generated_at: "2026-07-03T13:26:07Z"
    model: gpt-5.5
...
endroit pour découvrir des paquets, aux éditeurs un endroit pour publier des versions, et fournit à
OpenClaw suffisamment de métadonnées pour installer et mettre à jour ces paquets en toute sécurité.
...
- des informations de changelog et d’étiquette telles que `latest`
...
Une Skill est un ensemble de texte versionné centré sur `SKILL.md`. Elle peut inclure des
fichiers de support, des exemples, des modèles et des scripts.
...
la version minimale du Gateway, les cibles hôtes, les prérequis d’environnement et les condensats
d’artefacts.
...
Utilisez les simulations pour prévisualiser la charge utile résolue avant l’envoi. Les pages publiques
...
paquet de registre plus tard. Le CLI ClawHub prend également en charge les workflows directs d’installation et de
...
ClawHub est ouvert à la publication, mais les versions restent soumises aux contrôles d’envoi,
aux vérifications automatisées, aux signalements d’utilisateurs et aux actions de modération.
