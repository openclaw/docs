...
read_when:
    ...
    - Signaler un skill ou un package
...
ClawHub est ouvert à la publication, mais les listings publics passent toujours par des contrôles de confiance,
...
Avant d’installer une skill ou un plugin, vérifiez son listing ClawHub pour :
...
diagnostics visibles par les propriétaires.
...
- `held`, `quarantined`, `revoked` ou `hidden` : la version n’est pas pleinement
...
ClawHub prête une attention particulière aux écarts entre ce qu’une skill déclare et
...
Les résultats d’analyse sont fondés sur les artefacts. Les comportements attendus d’un fournisseur, tels que les
identifiants d’API déclarés, les rappels OAuth localhost, le nettoyage de désinstallation limité au périmètre, l’encodage Basic Auth,
ou les téléversements de fichiers sélectionnés par l’utilisateur vers le fournisseur indiqué, sont traités
différemment du transfert masqué d’identifiants, de l’accès large aux fichiers privés,
des destinations réseau sans rapport ou de l’abus furtif du navigateur.
...
OpenClaw vérifie la compatibilité avant d’installer les plugins hébergés par ClawHub. Les enregistrements de
packages peuvent également exposer des métadonnées de digest afin qu’OpenClaw puisse vérifier les
artefacts téléchargés. ClawScan inclut les métadonnées env/config `openclaw.environment` déclarées du package
lors de la revue des versions de plugins, afin que les exigences d’exécution déclarées soient
...
Les signalements doivent être spécifiques et exploitables. L’abus de signalement peut lui-même entraîner une
...
## Notes ClawScan pour les éditeurs
...
inhabituel, comme l’accès réseau, l’accès à l’hôte natif ou des
...
l’utilisateur). Cela masque toutes les skills de l’éditeur, fait en sorte que les publications futures
...
statique ne bloque immédiatement que les signatures malveillantes. Les détections par les moteurs
VirusTotal restent des preuves de sécurité visibles, mais les verdicts VirusTotal Code Insight/Palm
sont consultatifs et ne masquent pas les skills à eux seuls. Les revues LLM de ClawScan
conservent les notes alignées sur l’objectif comme conseils. Les résultats de revue moyens restent visibles sur
...
les skills masquées par la retenue au niveau utilisateur, et écrit une entrée de journal d’audit
...
peuvent entraîner des bannissements de compte, la révocation de jetons, du contenu masqué ou la suppression de
listings.
...
- ajoutez une note ClawScan d’éditeur lorsqu’une version présente un comportement inhabituel mais intentionnel
