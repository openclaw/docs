...
title: ClawHub
x-i18n:
    generated_at: "2026-07-05T05:57:01Z"
    model: gpt-5.5
...
| -------------- | ----------------------------------------------------------------- | -------------------------------------------- |
| Skills         | Bundles de texte versionnés avec `SKILL.md` et fichiers associés  | `openclaw skills install @openclaw/demo`     |
| Plugins de code | Packages de Plugin OpenClaw avec métadonnées de compatibilité    | `openclaw plugins install clawhub:<package>` |
| Plugins de bundle | Bundles de Plugin empaquetés pour la distribution OpenClaw     | `clawhub package publish <source>`           |
...
Les spécifications de Plugin brutes compatibles npm peuvent être résolues via npm lors des transitions de lancement, et
`npm:<package>` reste limité à npm lorsqu’une source doit être explicite.

Les installations de Plugin valident la compatibilité `pluginApi` et `minGatewayVersion`
annoncée avant l’installation de l’archive. Lorsqu’une version de package publie un artefact
ClawPack, OpenClaw privilégie le `.tgz` npm-pack exact téléversé, vérifie
...
ClawHub exécute des vérifications automatisées sur les Skills publiés et les versions de Plugin. Les versions
retenues par analyse ou bloquées peuvent disparaître du catalogue public et des surfaces d’installation tout en
...
Lorsque vous exécutez `clawhub install` en étant connecté, la CLI peut envoyer un événement d’installation au mieux
afin que ClawHub puisse calculer les totaux agrégés d’installations. Désactivez cela avec :
...
| `CLAWHUB_CONFIG_PATH`         | Remplacer l’emplacement où la CLI stocke l’état des tokens/config. |
...
Consultez [Télémétrie](/fr/clawhub/telemetry), [API HTTP](/clawhub/http-api), et
[Dépannage](/clawhub/troubleshooting) pour une documentation de référence plus approfondie.
