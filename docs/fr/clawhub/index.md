...
- Utilisez la CLI `clawhub` séparée pour les flux de travail d’authentification au registre, de publication et de suppression/restauration.
...
Installez la CLI ClawHub lorsque vous voulez des flux de travail authentifiés auprès du registre, comme
...
| -------------- | ---------------------------------------------------------------- | -------------------------------------------- |
| Skills         | Bundles de texte versionnés avec `SKILL.md` et fichiers de support | `openclaw skills install @openclaw/demo`     |
| Plugins de code | Packages de plugins OpenClaw avec métadonnées de compatibilité   | `openclaw plugins install clawhub:<package>` |
| Plugins bundle | Bundles de plugins empaquetés pour la distribution OpenClaw      | `clawhub package publish <source>`           |

ClawHub suit les versions semver, les étiquettes comme `latest`, les journaux des modifications, les fichiers,
...
Les spécifications de plugin simples et sûres pour npm peuvent être résolues via npm pendant les transitions de lancement, et
...
annoncée avant l’exécution de l’installation de l’archive. Lorsqu’une version de package publie un artefact
ClawPack, OpenClaw préfère le `.tgz` exact npm-pack téléversé, vérifie
...
La CLI dispose aussi de commandes d’installation/mise à jour de Skills pour les flux de travail directs avec le registre :
...
assez ancien pour passer la barrière de téléversement. Les pages de détail publiques résument le
dernier état d’analyse avant l’installation ou le téléchargement.
...
Lorsque vous exécutez `clawhub install` en étant connecté, la CLI peut envoyer un événement d’installation en mode meilleur effort
afin que ClawHub puisse calculer les nombres agrégés d’installations. Désactivez cela avec :
...
| `CLAWHUB_CONFIG_PATH`         | Remplace l’emplacement où la CLI stocke l’état jeton/config. |
