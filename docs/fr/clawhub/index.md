...
title: ClawHub
x-i18n:
    generated_at: "2026-05-13T02:52:12Z"
    model: gpt-5.5
    provider: openai
...
- Utilisez le CLI `clawhub` séparé pour les workflows d’authentification au registre, de publication, de suppression/restauration et de synchronisation.
...
Installez le CLI ClawHub lorsque vous voulez des workflows authentifiés auprès du registre, comme la publication, la synchronisation ou la suppression/restauration :
...
| -------------- | ------------------------------------------------------------ | -------------------------------------------- |
| Skills         | Bundles de texte versionnés avec `SKILL.md` et fichiers associés | `openclaw skills install <slug>`             |
...
| Souls          | Bundles `SOUL.md` affichés sur onlycrabs.ai                  | Flux de publication Web et API              |
...
Utilisez `clawhub:<package>` lorsqu’une installation de plugin doit être résolue via ClawHub. Les spécifications de plugin compatibles npm sans préfixe peuvent être résolues via npm pendant les bascules de lancement, et `npm:<package>` reste réservé à npm lorsqu’une source doit être explicite.
...
Le CLI ClawHub sert aux tâches authentifiées auprès du registre :
...
Le CLI dispose également de commandes d’installation/mise à jour de Skills pour les workflows directs avec le registre :
...
ClawHub est ouvert par défaut : tout le monde peut téléverser, mais la publication nécessite un compte GitHub suffisamment ancien pour passer la porte de téléversement. Les pages de détail publiques résument le dernier état d’analyse avant l’installation ou le téléchargement.
...
Lorsque vous exécutez `clawhub sync` en étant connecté, le CLI envoie un instantané minimal afin que ClawHub puisse calculer les nombres d’installations. Désactivez cela avec :
