...
à partir de ce compte connecté.
...
Si la connexion vous renvoie à un état déconnecté, il se peut que votre compte ne soit pas en règle.
Si votre compte a été banni ou désactivé, utilisez le
[formulaire de recours de ClawHub](https://appeals.openclaw.ai/) si vous pensez qu’il s’agit d’une
erreur.
...
4. Le navigateur vous redirige vers le rappel local.
5. La CLI stocke le jeton dans votre fichier de configuration ClawHub.

Si votre navigateur ne peut pas accéder au rappel local en raison des règles du pare-feu, du VPN ou
du proxy, utilisez la procédure de connexion sans interface graphique par jeton.
