...
Si la connexion vous ramène à un état déconnecté, votre compte peut ne pas être en règle.
...
1. La CLI démarre un serveur de callback temporaire sur `127.0.0.1`.
2. Votre navigateur ouvre la page de connexion ClawHub.
3. Après la connexion GitHub, ClawHub crée un jeton d’API.
4. Le navigateur redirige vers le callback local.
5. La CLI stocke le jeton dans votre fichier de configuration ClawHub.

Si votre navigateur ne peut pas atteindre le callback local à cause de règles de pare-feu, de VPN ou
