...
x-i18n:
    generated_at: "2026-07-02T08:15:00Z"
    model: gpt-5.5
...
ClawHub utilise GitHub pour la connexion Web. La CLI utilise des jetons API ClawHub créés
avec ce compte connecté.

## Connexion Web

Utilisez GitHub pour vous connecter sur [clawhub.ai](https://clawhub.ai).

Les comptes supprimés, bannis ou désactivés ne peuvent pas finaliser la connexion ClawHub normale.
...
3. Après la connexion GitHub, ClawHub crée un jeton API.
...
Vous pouvez révoquer les jetons API dans l’interface Web ClawHub.
...
Les comptes supprimés, bannis ou désactivés ne peuvent pas continuer à utiliser les jetons API existants.
Si votre compte a été banni ou désactivé, utilisez le
[formulaire d’appel ClawHub](https://appeals.openclaw.ai/) si vous pensez qu’il s’agit d’une
erreur.
