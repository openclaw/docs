...
Le flux de connexion par défaut de la CLI ouvre votre navigateur :
...
Si votre navigateur ne peut pas atteindre le rappel local à cause de règles de pare-feu, de VPN ou de proxy, utilisez le flux de jeton sans interface graphique.
...
Utilisez ce flux pour les serveurs, les tâches CI ou les environnements uniquement en terminal.
...
Les jetons révoqués, non valides ou manquants renvoient `401 Unauthorized`. Connectez-vous à nouveau avec `clawhub login` ou fournissez un nouveau jeton avec `clawhub login --token`.
