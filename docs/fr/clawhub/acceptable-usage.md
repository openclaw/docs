...
x-i18n:
    generated_at: "2026-07-04T03:44:20Z"
    model: gpt-5.5
...
| Sécurité défensive, modération et revue des abus | L’outil est présenté comme destiné à une revue autorisée, préserve les preuves et maintient des limites claires d’approbation humaine.                          |
...
| Exigences d’exécution masquées, non sécurisées ou trompeuses        | Commandes d’installation obfusquées, installateurs de type pipe-to-shell tels que du contenu téléchargé exécuté avec `sh` ou `bash` sans possibilité de revue claire, exigences non déclarées de secrets ou de clés privées, exécution distante de `npx @latest` sans possibilité de revue claire, ou métadonnées qui masquent ce dont la fiche a réellement besoin pour s’exécuter. |
| Matériel portant atteinte au droit d’auteur ou violant des droits           | Republier le skill, le plugin, la documentation, les éléments de marque ou le code propriétaire de quelqu’un d’autre sans autorisation ; violer des conditions de licence ; ou usurper l’identité de l’auteur ou de l’éditeur d’origine.                                                                                                                            |
...
- publier en masse un grand nombre de fiches peu soignées, dupliquées, provisoires ou
...
pour les réclamations relatives au droit d’auteur ou aux droits, sauf si la fiche est également non sécurisée,
