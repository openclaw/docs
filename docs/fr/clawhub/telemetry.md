...
ClawHub utilise une **télémétrie minimale** pour calculer les **nombres d’installations** (ce qui est réellement utilisé) et alimenter un meilleur tri/filtrage.
...
- Vous êtes **connecté** dans la CLI (nous exigeons déjà une authentification pour les flux de synchronisation/publication).
...
- Aucun chemin de dossier absolu brut (seulement `rootId` haché + un court libellé d’affichage).
- Aucun contenu de fichier.
- Aucun journal par exécution, prompt ou autre sortie de CLI.
- Aucun suivi des Skills qui ne sont pas téléversés dans le registre (les slugs inconnus sont ignorés).
...
### Détection des désinstallations
...
Les racines qui ne signalent aucune télémétrie pendant **120 jours** sont marquées comme obsolètes et leurs installations ne sont plus comptabilisées dans `installsCurrent`.
