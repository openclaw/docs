...
- Aucun journal par exécution, prompt ni autre sortie de CLI.

## Nombres d’installations

ClawHub conserve des compteurs agrégés par skill :

- `installsAllTime` : utilisateurs uniques qui ont signalé au moins une installation CLI de la skill.
- `installsCurrent` : utilisateurs uniques qui ont signalé une installation et n’ont pas supprimé leur
  télémétrie.

## Transparence + contrôles utilisateur

Tout le monde ne voit que les **compteurs d’installations agrégés**.
...
Avec ce paramètre, la CLI n’enverra pas de télémétrie d’installation.
