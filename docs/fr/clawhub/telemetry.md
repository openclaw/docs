...
summary: Télémétrie d’installation collectée via `clawhub sync` + option de refus.
x-i18n:
...
ClawHub utilise une **télémétrie minimale** pour calculer les **compteurs d’installation** (ce qui est réellement utilisé) et améliorer le tri/filtrage.
...
- `label` : un libellé lisible par un humain, dérivé des deux derniers segments du chemin (les chemins du dossier personnel sont affichés avec `~`).
...
Tout le monde ne voit que des **compteurs d’installation agrégés** ; personne d’autre ne peut voir vos racines/dossiers.
