...
summary: Installez la télémétrie collectée par la CLI ClawHub et découvrez comment vous désinscrire.
x-i18n:
    generated_at: "2026-07-04T17:58:29Z"
    model: gpt-5.5
...
Si vous n’êtes pas connecté, rien n’est transmis.

## Ce que nous collectons

À chaque `clawhub install` transmis, la CLI envoie un événement d’installation au mieux.
...
ClawHub conserve des compteurs agrégés par skill :

- `installsAllTime` : utilisateurs uniques ayant transmis au moins une installation CLI pour la skill.
- `installsCurrent` : utilisateurs uniques ayant transmis une installation et n’ayant pas supprimé leur
  télémétrie.
...
Lorsque cette variable est définie, la CLI n’envoie pas la télémétrie d’installation.
