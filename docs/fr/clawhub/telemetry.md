...
- Vous êtes **connecté** dans le CLI (nous exigeons déjà l’authentification pour les flux de synchronisation/publication).
...
À chaque `clawhub sync`, le CLI signale un **instantané complet** de ce qu’il a trouvé, regroupé par racine d’analyse (« dossier/racine »).
...
- `label` : un libellé lisible par un humain, dérivé des deux derniers segments du chemin (les chemins du répertoire personnel sont affichés avec `~`).
...
- Aucun chemin absolu brut de dossier (seulement un `rootId` haché + un court libellé d’affichage).
- Aucun contenu de fichier.
- Aucun journal par exécution, prompt ou autre sortie du CLI.
- Aucun suivi pour les compétences qui ne sont pas téléversées dans le registre (les slugs inconnus sont ignorés).
...
- Si la compétence est supprimée de toutes vos racines, elle ne compte plus dans `installsCurrent`.
...
Cette évaluation est effectuée paresseusement (lors du prochain rapport de télémétrie) afin d’éviter les tâches en arrière-plan.
...
Avec ce paramètre, le CLI n’enverra pas de télémétrie pendant `clawhub sync`.
