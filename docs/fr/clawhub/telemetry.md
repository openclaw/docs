...
- Aucun chemin de dossier absolu brut (uniquement `rootId` haché + un court libellé d’affichage).
...
Les racines qui ne signalent pas de télémétrie pendant **120 jours** sont marquées comme obsolètes, et leurs installations cessent d’être comptabilisées dans `installsCurrent`.
...
Tous les autres utilisateurs ne voient que des **compteurs d’installations agrégés** ; personne d’autre ne peut voir vos racines/dossiers.
