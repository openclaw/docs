...
pour être installé. Ils montrent ce que fait une version publiée, l’autorité qu’elle demande et
si un élément mérite une attention particulière avant qu’elle puisse accéder à des fichiers, des comptes,
des identifiants, du code ou des services externes.

Les audits sont de forts signaux de sécurité, mais ils ne garantissent pas qu’une version publiée est
...
| `Review`    | Lisez les conclusions avant l’installation. La version publiée peut rester légitime. |
...
Le niveau de risque décrit le rayon d’impact : le pouvoir que la version publiée semble avoir si
...
| `Medium`         | La version publiée dispose d’une autorité significative, comme l’accès à des comptes ou la modification de données. |
| `High`           | La version publiée dispose d’une autorité à fort impact, présente des conclusions graves ou des signaux malveillants. |
...
pas qu’il est malveillant. Cela signifie que le Skill semble aligné avec son objectif, mais peut
...
ClawHub audite les artefacts de version publiée soumis, notamment :
...
partenariat permet à ClawHub d’ajouter des renseignements de sécurité plus larges à la revue des Skills et des Plugins.

VirusTotal est particulièrement utile pour les artefacts malveillants connus, les détections par moteurs et
les signaux de réputation qui complètent la revue de ClawHub tenant compte des agents. Lorsque les nombres de moteurs
de fournisseurs sont disponibles, l’audit les résume en langage clair, par exemple :
...
Il examine chaque version publiée comme un artefact destiné aux agents : instructions,
...
Les signaux d’analyse statique constituent un contexte interne pour cette revue ; ils ne sont pas une
...
Il demande si la capacité est divulguée, alignée avec l’objectif et étayée par
le cas d’usage déclaré de la version publiée.
