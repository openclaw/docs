...
read_when:
    ...
    - Décider d’installer une Skills ou un Plugin
    - Expliquer le statut d’audit, le niveau de risque ou les constatations de ClawHub
...
x-i18n:
    generated_at: "2026-07-03T17:19:30Z"
    model: gpt-5.5
...
| `Warn`      | Redoublez de prudence. ClawHub a trouvé une préoccupation à fort impact ou un signal d’alerte. |
| `Malicious` | Ne pas installer.                                                           |
| `Pending`   | Les audits ne sont pas encore terminés.                                             |
| `Error`     | L’audit n’a pas pu être terminé.                                         |

Un `Pass` est rassurant, mais il ne remplace pas votre propre jugement. C’est surtout important
