...
| État        | Signification                                                            |
| ----------- | ------------------------------------------------------------------------ |
| `Pass`      | Aucun problème visible au-delà d’un faible risque n’a été trouvé.        |
...
Les constatations peuvent être étiquetées `Info`, `Low`, `Medium`, `High` ou `Critical`. Les constatations de sévérité
...
Les pages d’artefacts contiennent un lien vers l’audit complet à l’adresse :
...
décomptes de moteurs de fournisseurs sont disponibles, l’audit les résume en langage clair, par exemple :
...
Il demande si la capacité est divulguée, alignée sur l’objectif et prise en charge par
