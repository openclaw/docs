...
read_when:
    ...
    - Expliquer l’état de l’audit ClawHub, le niveau de risque ou les constats
...
| `Warn`      | Redoublez de prudence. ClawHub a détecté une préoccupation à fort impact ou un signal d’avertissement. |
...
| ---------- | ----------------------------------------------------------------------------- |
| `Low`      | Peu d’autorisations sensibles ou d’impact utilisateur ont été détectés.                          |
...
- Le niveau de risque demande : « Quel est le pouvoir accordé ici ? »
...
Les conclusions peuvent être libellées `Info`, `Low`, `Medium`, `High` ou `Critical`. Les conclusions
de gravité plus élevée contribuent plus fortement au niveau de risque et au statut de l’audit.
...
décomptes des moteurs de fournisseurs sont disponibles, l’audit les résume en langage clair, par
exemple :
...
Il vérifie si la capacité est divulguée, alignée sur l’objectif et appuyée par
le cas d’usage déclaré de la version.
