...
x-i18n:
    generated_at: "2026-07-04T06:30:19Z"
    model: gpt-5.5
...
pour être installé. Ils montrent ce que fait une version, quelles autorités elle demande, et
si un élément mérite une attention particulière avant de pouvoir accéder aux fichiers, comptes,
...
| État        | Signification                                                            |
| ----------- | ------------------------------------------------------------------------ |
| `Pass`      | Aucun problème visible au-dessus d’un faible risque n’a été trouvé.      |
...
- Le niveau de risque demande : « Quel est le pouvoir disponible ici ? »
...
reste concentrée sur des preuves utiles.
...
Les pages d’artefact renvoient vers l’audit complet à :
...
les signaux de réputation qui complètent l’examen de ClawHub sensible aux agents. Lorsque les nombres de détections
...
VirusTotal reste de la télémétrie. Il ne remplace pas la propre analyse des risques de ClawHub
...
comme prisme pour les risques tels que l’injection de prompt, la mauvaise utilisation des outils, l’exposition des identifiants,
l’exécution non sûre, l’empoisonnement de la mémoire ou du contexte, et l’autonomie excessive.

ClawScan ne traite pas une capacité d’apparence inquiétante comme automatiquement malveillante.
Il vérifie si la capacité est divulguée, alignée sur son objectif et soutenue par
