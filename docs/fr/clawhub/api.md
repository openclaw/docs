...
x-i18n:
    generated_at: "2026-07-03T13:26:28Z"
    model: gpt-5.5
...
Vous pouvez créer un catalogue, un annuaire ou une interface de recherche tierce sur les API publiques de lecture de ClawHub. Les métadonnées publiques des Skills et les fichiers de Skills sont publiés selon les règles de licence des Skills de ClawHub, tandis que l’API elle-même est limitée en débit et doit être utilisée de manière responsable.
...
- Créez un lien vers l’URL canonique de la Skill ClawHub lors de l’affichage des listes afin que les utilisateurs puissent inspecter l’enregistrement source du registre.
...
- `X-RateLimit-Remaining` / `RateLimit-Remaining` : budget restant exact lorsqu’il est
  présent ; les requêtes partitionnées réussies l’omettent plutôt que de renvoyer une valeur
...
- `POST /api/v1/users/reserve` réserve les slugs racine et les espaces réservés privés de paquets sans publication pour un identifiant propriétaire.
