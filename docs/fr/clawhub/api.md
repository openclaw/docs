...
x-i18n:
    generated_at: "2026-07-12T02:27:37Z"
    model: gpt-5.6
...
- Utilisez les points de terminaison publics en lecture, tels que `GET /api/v1/skills`, `GET /api/v1/search` et `GET /api/v1/skills/{slug}`, pour les listes du catalogue.
- Mettez les réponses en cache et respectez `429`, `Retry-After` et les en-têtes de limite de débit au lieu d’interroger l’API de manière intensive.
- Lorsque vous affichez des listes, ajoutez un lien vers l’URL canonique du Skill sur ClawHub afin que les utilisateurs puissent consulter l’enregistrement du registre source.
...
- Un jeton manquant ou non valide entraîne l’application de la limite par adresse IP.

- Lecture : 3 000/min par adresse IP, 12 000/min par clé
- Écriture : 300/min par adresse IP, 3 000/min par clé
- Téléchargement : 1 200/min par adresse IP, 6 000/min par clé
...
- `X-RateLimit-Remaining` / `RateLimit-Remaining` : quota restant exact lorsqu’il est
  présent ; les requêtes réussies réparties entre plusieurs partitions l’omettent au lieu de renvoyer une valeur
...
- Les paramètres de requête connus contenant des valeurs non valides renvoient `400`.
...
- `GET /api/v1/skills?limit=&cursor=&sort=`
  - `sort` : `updated` (par défaut), `recommended` (`default`), `createdAt` (`newest`), `downloads`, `stars` (`rating`), les alias historiques d’installation `installsCurrent`/`installs`/`installsAllTime` correspondent à `downloads`, `trending`
...
- `POST /api/v1/users/reserve` réserve les slugs racine et les espaces réservés privés de paquets sans version pour un identifiant de propriétaire.
