...
x-i18n:
    generated_at: "2026-07-03T02:46:33Z"
    model: gpt-5.5
...
Vous pouvez créer un catalogue, un annuaire ou une surface de recherche tiers par-dessus les API publiques de lecture de ClawHub. Les métadonnées publiques de Skills et les fichiers Skills sont publiés selon les règles de licence des Skills de ClawHub, tandis que l’API elle-même est soumise à des limites de débit et doit être consommée de manière responsable.
...
- `POST /api/v1/users/reserve` réserve les slugs racines et les espaces réservés privés de paquets sans publication pour un identifiant de propriétaire.
