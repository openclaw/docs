...
x-i18n:
    generated_at: "2026-07-02T22:29:50Z"
    model: gpt-5.5
...
Vous pouvez créer un catalogue tiers, un annuaire ou une interface de recherche à partir des API publiques de lecture de ClawHub. Les métadonnées publiques des Skills et les fichiers de Skills sont publiés selon les règles de licence des Skills de ClawHub, tandis que l’API elle-même est soumise à des limites de débit et doit être consommée de manière responsable.
...
- `X-RateLimit-Remaining` / `RateLimit-Remaining` : budget restant exact lorsqu’il est
  présent ; les requêtes réussies réparties sur des partitions l’omettent au lieu de renvoyer une valeur
...
- `GET /api/v1/skills/export?startDate=&endDate=&limit=&cursor=`
  - Les Skills hébergées sont exportées comme fichiers stockés.
...
- `POST /api/v1/users/reserve` réserve des slugs racine et des espaces réservés privés de packages sans publication pour un identifiant de propriétaire.
