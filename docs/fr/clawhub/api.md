...
summary: 'API REST publique (v1) : vue d’ensemble et conventions.'
x-i18n:
    generated_at: "2026-07-01T15:22:56Z"
    model: gpt-5.5
...
Vous pouvez créer un catalogue tiers, un répertoire ou une interface de recherche au-dessus des API de lecture publiques de ClawHub. Les métadonnées publiques des Skills et les fichiers de Skills sont publiés selon les règles de licence des Skills de ClawHub, tandis que l’API elle-même est soumise à des limites de débit et doit être consommée de manière responsable.

Directives :

- Utilisez les endpoints de lecture publics comme `GET /api/v1/skills`, `GET /api/v1/search` et `GET /api/v1/skills/{slug}` pour les listes de catalogue.
...
- Ajoutez un lien vers l’URL canonique de Skill ClawHub lors de l’affichage des listes afin que les utilisateurs puissent inspecter l’enregistrement source du registre.
- Utilisez les URL de page canoniques sous la forme `https://clawhub.ai/<owner>/skills/<slug>`.
- Ne suggérez pas que ClawHub approuve, vérifie ou exploite le site tiers.
- Ne miroitez pas du contenu masqué, privé ou bloqué par la modération en contournant les filtres de l’API publique ou les limites d’authentification.
