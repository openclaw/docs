...
Vous pouvez créer un catalogue, un répertoire ou une interface de recherche tiers à partir des API publiques de lecture de ClawHub. Les métadonnées publiques des Skills et les fichiers des Skills sont publiés selon les règles de licence des Skills de ClawHub, tandis que l’API elle-même est soumise à des limites de débit et doit être consommée de manière responsable.
...
- `X-RateLimit-Remaining` / `RateLimit-Remaining` : budget restant exact lorsque
  présent ; les requêtes réparties réussies l’omettent plutôt que de renvoyer une valeur
...
- `POST /api/v1/users/reserve` réserve des slugs racine et des espaces réservés de packages privés sans version pour un identifiant de propriétaire.
