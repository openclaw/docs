...
read_when:
    ...
    - Ajout de endpoints ou de schémas
summary: Présentation et conventions de l’API REST publique (v1).
x-i18n:
    generated_at: "2026-07-02T00:51:37Z"
    model: gpt-5.5
...
Vous pouvez créer un catalogue tiers, un répertoire ou une surface de recherche au-dessus des API de lecture publiques de ClawHub. Les métadonnées publiques des Skills et les fichiers de Skills sont publiés selon les règles de licence des Skills de ClawHub, tandis que l’API elle-même est limitée en débit et doit être consommée de manière responsable.
...
- Ne dupliquez pas le contenu masqué, privé ou bloqué par modération en contournant les filtres de l’API publique ou les limites d’authentification.
...
- `X-RateLimit-Remaining` / `RateLimit-Remaining` : budget restant exact quand il est
...
- `GET /api/v1/search?q=...`
  ...
  - Alias ancien : `nonSuspicious=true`
- `GET /api/v1/skills?limit=&cursor=&sort=`
  - `sort` : `updated` (par défaut), `recommended` (`default`), `createdAt` (`newest`), `downloads`, `stars` (`rating`), les alias d’installation anciens `installsCurrent`/`installs`/`installsAllTime` correspondent à `downloads`, `trending`
...
- `GET /api/v1/skills/export?startDate=&endDate=&limit=&cursor=`
  ...
  - Les Skills actuels adossés à GitHub avec une analyse `clean` ou `suspicious` sont exportés
    en tant que descripteurs de relais `public-github`.
- `GET /api/v1/packages?limit=&cursor=&sort=`
  - `sort` : `updated` (par défaut), `recommended`, `downloads`, alias ancien `installs`
  - Les valeurs `sort` invalides renvoient `400`
- `GET /api/v1/plugins?limit=&cursor=&sort=`
  - `sort` : `recommended` (par défaut), `downloads`, `updated`, alias ancien `installs`
