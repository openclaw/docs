...
x-i18n:
    generated_at: "2026-07-04T15:14:23Z"
    model: gpt-5.5
...
Vous pouvez créer un catalogue, un annuaire ou une surface de recherche tiers à partir des API de lecture publiques de ClawHub. Les métadonnées publiques des skills et les fichiers de skills sont publiés selon les règles de licence des skills de ClawHub, tandis que l’API elle-même est soumise à des limites de débit et doit être consommée de manière responsable.
...
- Ne répliquez pas du contenu masqué, privé ou bloqué par la modération en contournant les filtres de l’API publique ou les limites d’authentification.
...
- `X-RateLimit-Reset` : secondes Unix epoch (heure de réinitialisation absolue)
