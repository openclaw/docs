...
x-i18n:
    generated_at: "2026-07-03T09:33:11Z"
    model: gpt-5.5
...
Vous pouvez créer un catalogue, un annuaire ou une interface de recherche tiers au-dessus des API publiques de lecture de ClawHub. Les métadonnées publiques des Skills et les fichiers de Skills sont publiés selon les règles de licence des Skills de ClawHub, tandis que l’API elle-même est limitée en débit et doit être consommée de manière responsable.

Consignes :

- Utilisez les points de terminaison publics en lecture, comme `GET /api/v1/skills`, `GET /api/v1/search` et `GET /api/v1/skills/{slug}`, pour les listes de catalogue.
...
- `X-RateLimit-Remaining` / `RateLimit-Remaining` : quota restant exact quand il est
...
- Les erreurs v1 sont du texte brut (`text/plain; charset=utf-8`), y compris les réponses
  `400`, `401`, `403`, `404`, `429` et les réponses de téléchargement bloqué.
...
- `POST /api/v1/users/reserve` réserve les slugs racine et les espaces réservés de packages privés sans publication pour un identifiant propriétaire.
