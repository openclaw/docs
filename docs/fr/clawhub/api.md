...
x-i18n:
    generated_at: "2026-07-04T20:29:14Z"
    model: gpt-5.5
...
Vous pouvez construire un catalogue tiers, un annuaire ou une surface de recherche sur les API de lecture publiques de ClawHub. Les métadonnées publiques des Skills et les fichiers de Skills sont publiés selon les règles de licence des Skills de ClawHub, tandis que l’API elle-même est soumise à des limites de débit et doit être consommée de manière responsable.

Consignes :

- Utilisez les points de terminaison de lecture publics comme `GET /api/v1/skills`, `GET /api/v1/search` et `GET /api/v1/skills/{slug}` pour les listes de catalogue.
...
- Ajoutez un lien vers l’URL canonique de la Skills ClawHub lors de l’affichage des listes afin que les utilisateurs puissent inspecter l’enregistrement source du registre.
- Utilisez les URL de pages canoniques sous la forme `https://clawhub.ai/<owner>/skills/<slug>`.
- N’impliquez pas que ClawHub approuve, vérifie ou exploite le site tiers.
...
- `X-RateLimit-Remaining` / `RateLimit-Remaining` : budget restant exact quand
  il est présent ; les requêtes fragmentées réussies l’omettent au lieu de renvoyer une valeur
...
- Préférez `Retry-After` quand il est présent.
...
- `GET /api/v1/skills/export?startDate=&endDate=&limit=&cursor=`
  - Les Skills hébergés sont exportés comme fichiers stockés.
  - Les Skills actuels adossés à GitHub avec une analyse `clean` ou `suspicious` sont exportés
    comme descripteurs de transfert `public-github`.
...
- `POST /api/v1/users/reserve` réserve les slugs racine et les espaces réservés de packages privés sans publication pour un identifiant de propriétaire.
