...
x-i18n:
    generated_at: "2026-07-03T00:55:30Z"
    model: gpt-5.5
...
Vous pouvez créer un catalogue, un annuaire ou une interface de recherche tiers au-dessus des API de lecture publiques de ClawHub. Les métadonnées publiques des Skills et les fichiers de Skills sont publiés selon les règles de licence des Skills de ClawHub, tandis que l’API elle-même est soumise à des limites de débit et doit être consommée de manière responsable.
...
- Un jeton manquant/invalide retombe sur l’application par IP.
...
- `POST /api/v1/users/reserve` réserve des slugs racine et des espaces réservés privés de paquets sans publication pour un identifiant de propriétaire.
