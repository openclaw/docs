...
read_when:
    ...
    - Ajouter des points de terminaison ou des schémas
...
Vous pouvez créer un catalogue, un répertoire ou une interface de recherche tiers au-dessus des API de lecture publiques de ClawHub. Les métadonnées publiques des Skills et les fichiers de Skills sont publiés selon les règles de licence des Skills de ClawHub, tandis que l'API elle-même est limitée en débit et doit être consommée de manière responsable.
...
- Mettez les réponses en cache et respectez `429`, `Retry-After` et les en-têtes de limite de débit au lieu d'effectuer des interrogations agressives.
- Créez un lien vers l'URL canonique du Skill ClawHub lors de l'affichage des listes, afin que les utilisateurs puissent inspecter l'enregistrement source du registre.
- Utilisez les URL de pages canoniques au format `https://clawhub.ai/<owner>/skills/<slug>`.
...
- `X-RateLimit-Remaining` / `RateLimit-Remaining` : budget restant exact lorsqu'il est
  présent ; les requêtes fragmentées réussies l'omettent plutôt que de renvoyer une valeur
