...
summary: Vue d’ensemble et conventions de l’API REST publique (v1).
x-i18n:
...
Vous pouvez créer un catalogue, un annuaire ou une surface de recherche tiers sur les API de lecture publiques de ClawHub. Les métadonnées publiques des Skills et les fichiers de Skills sont publiés selon les règles de licence des Skills de ClawHub, tandis que l’API elle-même est soumise à des limites de débit et doit être consommée de manière responsable.

Consignes :

- Utilisez des points de terminaison de lecture publics comme `GET /api/v1/skills`, `GET /api/v1/search` et `GET /api/v1/skills/{slug}` pour les listes de catalogue.
- Mettez les réponses en cache et respectez `429`, `Retry-After` et les en-têtes de limite de débit au lieu d’interroger de façon agressive.
- Ajoutez un lien vers l’URL canonique de Skill ClawHub lors de l’affichage des listes afin que les utilisateurs puissent consulter l’enregistrement source du registre.
- Utilisez les URL de pages canoniques sous la forme `https://clawhub.ai/<owner>/skills/<slug>`.
- Ne laissez pas entendre que ClawHub approuve, vérifie ou exploite le site tiers.
- Ne mettez pas en miroir du contenu masqué, privé ou bloqué par la modération en contournant les filtres de l’API publique ou les frontières d’authentification.
...
- Un jeton manquant ou invalide retombe sur l’application par IP.
...
`X-RateLimit-Remaining`, `RateLimit-Remaining` et `Retry-After` sont inclus avec `429`.
...
- `X-RateLimit-Remaining` / `RateLimit-Remaining` : budget restant exact lorsqu’il est
  présent ; les requêtes réussies partitionnées l’omettent plutôt que de renvoyer une valeur
...
- Sinon, utilisez `RateLimit-Reset` ou déduisez le délai de `X-RateLimit-Reset`.
- Ajoutez de la gigue aux nouvelles tentatives.

## Erreurs

- Les erreurs v1 sont en texte brut (`text/plain; charset=utf-8`), notamment `400`,
...
- `GET /api/v1/download?slug=&version=&tag=`
  ...
  - Les Skills actuels adossés à GitHub avec un scan `clean` ou `suspicious` renvoient un
    descripteur de relais JSON `public-github` au lieu d’octets ClawHub.
- `GET /api/v1/skills/export?startDate=&endDate=&limit=&cursor=`
  ...
  - Les Skills actuels adossés à GitHub avec un scan `clean` ou `suspicious` sont exportés
    sous forme de descripteurs de relais `public-github`.
...
- `POST /api/v1/skills` (publication, multipart préférable)
