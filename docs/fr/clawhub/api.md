...
x-i18n:
    generated_at: "2026-07-05T05:02:52Z"
    model: gpt-5.5
...
Vous pouvez créer un catalogue tiers, un répertoire ou une interface de recherche par-dessus les API publiques de lecture de ClawHub. Les métadonnées publiques des compétences et les fichiers de compétences sont publiés selon les règles de licence des compétences de ClawHub, tandis que l'API elle-même est limitée en débit et doit être consommée de manière responsable.
...
- Ajoutez un lien vers l'URL canonique de la compétence ClawHub lorsque vous affichez des listes, afin que les utilisateurs puissent inspecter l'enregistrement du registre source.
...
- `RateLimit-Reset` : secondes d'attente avant réinitialisation
- `X-RateLimit-Remaining` / `RateLimit-Remaining` : budget restant exact lorsqu'il
  est présent ; les requêtes réussies réparties en fragments l'omettent au lieu de renvoyer une valeur
  globale approximative
- `Retry-After` : secondes d'attente sur `429`
