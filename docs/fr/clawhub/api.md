...
- Utilisez les points de terminaison publics de lecture tels que `GET /api/v1/skills`, `GET /api/v1/search` et `GET /api/v1/skills/{slug}` pour les listes de catalogues.
...
- `X-RateLimit-Remaining` / `RateLimit-Remaining` : budget restant exact lorsque
  présent ; les requêtes partitionnées réussies l’omettent plutôt que de retourner une valeur
