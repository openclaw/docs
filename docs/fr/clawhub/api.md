...
x-i18n:
    generated_at: "2026-07-01T05:38:55Z"
    model: gpt-5.5
...
Vous pouvez créer un catalogue, un répertoire ou une surface de recherche tiers à partir des API publiques de lecture de ClawHub. Les métadonnées publiques des Skills et les fichiers de Skills sont publiés selon les règles de licence des Skills de ClawHub, tandis que l’API elle-même est limitée en débit et doit être consommée de manière responsable.
...
- Mettez les réponses en cache et respectez les en-têtes `429`, `Retry-After` et de limite de débit au lieu d’interroger de façon agressive.
...
- Un jeton manquant ou non valide revient à une application par IP.
...
- `X-RateLimit-Remaining` / `RateLimit-Remaining` : budget restant exact lorsque
  présent ; les requêtes distribuées réussies l’omettent plutôt que de renvoyer une valeur
...
- `POST /api/v1/users/reserve` réserve les slugs racines et les espaces réservés de paquets privés sans publication pour un identifiant de propriétaire.
