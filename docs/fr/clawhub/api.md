...
x-i18n:
    generated_at: "2026-07-01T12:57:55Z"
    model: gpt-5.5
...
Vous pouvez créer un catalogue, un répertoire ou une interface de recherche tierce au-dessus des API publiques de lecture de ClawHub. Les métadonnées publiques des Skills et les fichiers de Skills sont publiés selon les règles de licence des Skills de ClawHub, tandis que l’API elle-même est soumise à des limites de débit et doit être consommée de manière responsable.

Consignes :

- Utilisez les points de terminaison publics en lecture tels que `GET /api/v1/skills`, `GET /api/v1/search` et `GET /api/v1/skills/{slug}` pour les listes de catalogue.
...
- `GET /api/v1/download?slug=&version=&tag=`
  ...
  - Les Skills actuels adossés à GitHub avec une analyse `clean` ou `suspicious` renvoient un
    descripteur de transfert JSON `public-github` au lieu des octets ClawHub.
...
- `POST /api/v1/users/reserve` réserve les slugs racine et les espaces réservés privés de packages sans version pour un identifiant de propriétaire.
