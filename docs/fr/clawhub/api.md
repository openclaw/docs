...
Vous pouvez créer un catalogue, un annuaire ou une surface de recherche tiers à partir des API de lecture publiques de ClawHub. Les métadonnées publiques des Skills et les fichiers de Skills sont publiés selon les règles de licence des Skills de ClawHub, tandis que l’API elle-même est limitée en débit et doit être consommée de manière responsable.
...
- `X-RateLimit-Reset` : secondes de l’époque Unix (heure de réinitialisation absolue)
- `RateLimit-Reset` : secondes de délai jusqu’à la réinitialisation
