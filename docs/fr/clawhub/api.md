...
x-i18n:
    generated_at: "2026-07-12T21:37:29Z"
    model: gpt-5.6
...
Vous pouvez créer un catalogue, un annuaire ou une interface de recherche tierce reposant sur les API publiques de lecture de ClawHub. Les métadonnées et les fichiers publics des Skills sont publiés conformément aux règles de licence des Skills de ClawHub, tandis que l'API elle-même est soumise à des limites de débit et doit être utilisée de manière responsable.
...
- Mettez les réponses en cache et respectez les en-têtes `429`, `Retry-After` et de limite de débit au lieu d'effectuer des interrogations trop fréquentes.
...
- `Retry-After` : délai d'attente en secondes lors d'une réponse `429`
...
- Les paramètres de requête connus ayant des valeurs non valides renvoient `400`.
...
- `GET /api/v1/search?q=...`
  ...
  - Ancien alias : `nonSuspicious=true`
