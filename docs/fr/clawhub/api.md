...
x-i18n:
    generated_at: "2026-06-30T13:56:17Z"
    model: gpt-5.5
...
Vous pouvez créer un catalogue, un répertoire ou une interface de recherche tiers par-dessus les API publiques de lecture de ClawHub. Les métadonnées publiques des Skills et les fichiers des Skills sont publiés selon les règles de licence des Skills de ClawHub, tandis que l’API elle-même est limitée en débit et doit être consommée de façon responsable.

Consignes :

- Utilisez des points de terminaison publics en lecture comme `GET /api/v1/skills`, `GET /api/v1/search` et `GET /api/v1/skills/{slug}` pour les listes de catalogue.
...
- Ajoutez un lien vers l’URL canonique du Skill ClawHub lorsque vous affichez des listes, afin que les utilisateurs puissent inspecter l’enregistrement du registre source.
...
- Ne répliquez pas du contenu masqué, privé ou bloqué par la modération en contournant les filtres de l’API publique ou les frontières d’authentification.
...
- Un jeton manquant/invalide revient à une application par IP.
...
`X-RateLimit-Remaining`, `RateLimit-Remaining` et `Retry-After` sont inclus lors d’un `429`.
...
- `X-RateLimit-Remaining` / `RateLimit-Remaining` : budget restant exact lorsqu’il est
  présent ; les requêtes réussies fragmentées l’omettent au lieu de renvoyer une valeur
  globale approximative
- `Retry-After` : délai en secondes à attendre lors d’un `429`
...
- `GET /api/v1/skills?limit=&cursor=&sort=`
  ...
  - Alias hérité : `nonSuspicious=true`
  - Avec `nonSuspiciousOnly=true`, les pages fondées sur le curseur peuvent contenir moins de `limit` éléments ; utilisez `nextCursor` pour continuer.
...
- `POST /api/v1/users/reserve` réserve les slugs racine et les espaces réservés privés de packages sans version pour un identifiant propriétaire.
