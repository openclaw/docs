...
- En cas de jeton manquant ou non valide, l'application par adresse IP est utilisée.
...
- `Retry-After` : délai d'attente en secondes pour `429`
...
- Les paramètres de requête connus dont les valeurs ne sont pas valides renvoient `400`.
...
- `POST /api/v1/skills` (publication, multipart privilégié)
...
- `POST /api/v1/users/reserve` réserve les slugs racine et les espaces réservés de paquets privés sans version pour un identifiant de propriétaire.
