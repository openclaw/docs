...
Vous pouvez créer un catalogue, un répertoire ou une surface de recherche tiers par-dessus les API publiques de lecture de ClawHub. Les métadonnées publiques des Skills et les fichiers des Skills sont publiés selon les règles de licence des Skills de ClawHub, tandis que l’API elle-même est limitée en débit et doit être consommée de manière responsable.
...
- Ne laissez pas entendre que ClawHub cautionne, vérifie ou exploite le site tiers.
...
- Un jeton manquant ou invalide bascule vers l’application par IP.
...
- `POST /api/v1/users/reserve` réserve les slugs racine et les emplacements réservés de paquets privés sans publication pour un identifiant de propriétaire.
