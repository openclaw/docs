...
Vous pouvez créer un catalogue, un annuaire ou une interface de recherche tiers par-dessus les API de lecture publiques de ClawHub. Les métadonnées publiques des Skills et les fichiers de Skills sont publiés selon les règles de licence des Skills de ClawHub, tandis que l’API elle-même est soumise à des limites de débit et doit être consommée de manière responsable.
...
- Ajoutez un lien vers l’URL canonique de la Skill ClawHub lors de l’affichage des listes afin que les utilisateurs puissent inspecter l’enregistrement source du registre.
...
- Ne dupliquez pas de contenu masqué, privé ou bloqué par la modération en contournant les filtres de l’API publique ou les limites d’authentification.
...
- `X-RateLimit-Reset` : secondes Unix epoch (heure absolue de réinitialisation)
