...
x-i18n:
    generated_at: "2026-06-28T05:43:28Z"
    model: gpt-5.5
...
L’importateur web GitHub est plus strict que la publication/synchronisation locale. Il découvre uniquement les fichiers
`SKILL.md` ou les anciens fichiers `skills.md` dans les dépôts publics non forkés appartenant au
...
| `primaryEnv`       | `string`   | Variable d’environnement principale d’identifiants pour votre compétence.                                                                    |
...
| `emoji`            | `string`   | Emoji d’affichage de la compétence.                                                                                                          |
...
- Le texte intégré inclut `SKILL.md` + jusqu’à environ 40 fichiers non `.md` (plafond au mieux).
...
- Les balises sont des pointeurs de chaîne vers une version ; `latest` est couramment utilisé.
