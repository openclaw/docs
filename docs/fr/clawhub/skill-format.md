...
x-i18n:
    generated_at: "2026-06-30T22:13:52Z"
    model: gpt-5.5
...
- `.clawhubignore` (motifs à ignorer pour la publication, ancien `.clawdhubignore`)
...
`SKILL.md` ou les anciens fichiers `skills.md` dans des dépôts publics, non issus d’un fork, appartenant au compte GitHub connecté. Il n’importe pas les dépôts privés, les forks,
...
| `primaryEnv`       | `string`   | La variable d’environnement principale contenant l’identifiant de votre skill.                                                                   |
...
Si votre skill a besoin de dépendances installées, déclarez-les dans le tableau `install` :
...
- Les types de contenu commençant par `text/` sont traités comme du texte, avec en plus une petite liste autorisée (JSON/YAML/TOML/JS/TS/Markdown/SVG).
...
- Les scopes de paquet doivent correspondre exactement à l’identifiant de publication ClawHub. Les identifiants de publication peuvent utiliser des lettres minuscules, des chiffres, des traits d’union, des points et des underscores ; ils doivent commencer et se terminer par une lettre minuscule ou un chiffre.
