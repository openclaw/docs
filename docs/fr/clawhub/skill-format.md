...
x-i18n:
    generated_at: "2026-07-03T09:32:30Z"
    model: gpt-5.5
...
| `primaryEnv`       | `string`   | Variable d’environnement principale d’identifiants pour votre skill.                                                                                    |
...
Si votre skill a besoin que des dépendances soient installées, déclarez-les dans le tableau `install` :
...
- Les types de contenu commençant par `text/` sont traités comme du texte, plus une petite liste d’autorisation (JSON/YAML/TOML/JS/TS/Markdown/SVG).
...
- Les étiquettes sont des pointeurs textuels vers une version ; `latest` est couramment utilisé.
...
- N’ajoutez pas de conditions de licence contradictoires dans `SKILL.md` ; ClawHub ne prend pas en charge les dérogations de licence par skill.

## Skills payants

- ClawHub ne prend pas en charge les skills payants, la tarification par skill, les paywalls ni le partage des revenus.
- N’ajoutez pas de métadonnées de prix à `SKILL.md` ; cela ne fait pas partie du format de skill et ne rendra pas un skill publié payant.
