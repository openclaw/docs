...
x-i18n:
    generated_at: "2026-07-03T02:47:06Z"
    model: gpt-5.5
...
`SKILL.md` ou l'ancien `skills.md` dans des dépôts publics, non forkés, appartenant au compte GitHub connecté. Il n'importe pas les dépôts privés, les forks,
...
| `primaryEnv`       | `string`   | Variable d'environnement principale d'identifiant pour votre skill.                                                                          |
...
L'analyse de sécurité de ClawHub vérifie que ce que déclare votre skill correspond à ce qu'il fait réellement. Si votre code référence `TODOIST_API_KEY` mais que votre frontmatter ne la déclare pas sous `requires.env`, `primaryEnv` ou `envVars`, l'analyse signalera une incohérence de métadonnées. Garder des déclarations exactes aide votre skill à réussir l'examen et aide les utilisateurs à comprendre ce qu'ils installent.
...
- Les portées de package doivent correspondre exactement au pseudonyme d'éditeur ClawHub. Les pseudonymes d'éditeur peuvent utiliser des lettres minuscules, des chiffres, des traits d'union, des points et des underscores ; ils doivent commencer et se terminer par une lettre minuscule ou un chiffre.
