...
les fichiers `SKILL.md` ou les anciens fichiers `skills.md` dans des dépôts publics, non forkés, appartenant au
...
Utilisez `requires.env` pour les variables d’environnement qui doivent être présentes avant l’exécution de la compétence. Utilisez `envVars` lorsque vous avez besoin de métadonnées par variable, y compris pour des variables facultatives avec `required: false`.
...
| `primaryEnv`       | `string`   | La variable d’environnement d’identifiant principale pour votre compétence.                                                                                                       |
...
- Les portées de paquet doivent correspondre exactement à l’identifiant d’éditeur ClawHub. Les identifiants d’éditeur peuvent utiliser des lettres minuscules, des chiffres, des traits d’union, des points et des underscores ; ils doivent commencer et se terminer par une lettre minuscule ou un chiffre.
