...
| `requires.config`  | `string[]` | Chemins des fichiers de configuration que votre compétence lit.                                                                               |
...
- Les balises sont des pointeurs de type chaîne vers une version ; `latest` est couramment utilisé.

## Licence

- Toutes les compétences publiées sur ClawHub sont sous licence `MIT-0`.
- Toute personne peut utiliser, modifier et redistribuer les compétences publiées, y compris commercialement.
- L’attribution n’est pas requise.
- N’ajoutez pas de conditions de licence conflictuelles dans `SKILL.md` ; ClawHub ne prend pas en charge les remplacements de licence par compétence.
