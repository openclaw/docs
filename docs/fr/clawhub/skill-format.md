...
read_when:
    ...
    - Échec de publication de débogage
...
`SKILL.md` ou l’ancien `skills.md` dans les dépôts publics, non dérivés, appartenant
...
- `description` est utilisé comme résumé du skill dans l’UI/la recherche.
...
- Les portées de package doivent correspondre exactement au handle de publication ClawHub. Les handles de publication peuvent utiliser des lettres minuscules, des chiffres, des traits d’union, des points et des underscores ; ils doivent commencer et se terminer par une lettre minuscule ou un chiffre.
