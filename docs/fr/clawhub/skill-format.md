...
`SKILL.md` ou les anciens fichiers `skills.md` dans des dépôts publics, non forkés, appartenant au
...
| `primaryEnv`       | `string`   | La variable d’environnement d’identifiant principale pour votre Skill.                                                                               |
...
- Les fichiers de script sont quand même analysés après téléversement ; les fichiers PowerShell `.ps1`, `.psm1` et `.psd1` sont acceptés comme texte.
- Les types de contenu commençant par `text/` sont traités comme du texte, avec une petite liste autorisée supplémentaire (JSON/YAML/TOML/JS/TS/Markdown/SVG).
