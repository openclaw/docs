...
x-i18n:
    generated_at: "2026-07-01T15:23:29Z"
    model: gpt-5.5
...
- `.clawhubignore` (motifs à ignorer lors de la publication, ancien `.clawdhubignore`)
- `.gitignore` (également respecté)

## Import GitHub

L’importateur GitHub web est plus strict que la publication/synchronisation locale. Il découvre uniquement les fichiers `SKILL.md` ou l’ancien `skills.md` dans les dépôts publics, non forkés, appartenant au compte GitHub connecté. Il n’importe pas les dépôts privés, les forks, les dépôts archivés/désactivés ni les dépôts publics tiers.
...
| `primaryEnv`       | `string`   | Variable d’environnement principale de l’identifiant pour votre compétence.                                                                                  |
...
| `emoji`            | `string`   | Émoji d’affichage pour la compétence.                                                                                                                       |
| `homepage`         | `string`   | URL vers la page d’accueil ou la documentation de la compétence.                                                                                            |
...
- Les fichiers de script sont tout de même analysés après téléversement ; les fichiers PowerShell `.ps1`, `.psm1` et `.psd1` sont acceptés comme texte.
...
- Les scopes de package doivent correspondre exactement au handle d’éditeur ClawHub. Les handles d’éditeur peuvent utiliser des lettres minuscules, des chiffres, des traits d’union, des points et des underscores ; ils doivent commencer et se terminer par une lettre minuscule ou un chiffre.
