...
x-i18n:
    generated_at: "2026-07-04T10:37:40Z"
    model: gpt-5.5
...
- tous les fichiers de prise en charge _textuels_ (voir « Fichiers autorisés »)
- `.clawhubignore` (motifs à ignorer pour la publication, ancien `.clawdhubignore`)
- `.gitignore` (également pris en compte)

## Import GitHub

L’importateur GitHub web est plus strict que la publication/synchronisation locale. Il ne détecte que les fichiers
`SKILL.md` ou l’ancien `skills.md` dans des dépôts publics, non forkés, appartenant
...
| `primaryEnv`       | `string`   | La variable d’environnement principale pour les identifiants de votre compétence.                                                             |
...
- Les fichiers de script sont tout de même analysés après le téléversement ; les fichiers PowerShell `.ps1`, `.psm1` et `.psd1` sont acceptés comme texte.
...
- Les portées de package doivent correspondre exactement à l’identifiant de publication ClawHub. Les identifiants de publication peuvent utiliser des lettres minuscules, des chiffres, des traits d’union, des points et des tirets bas ; ils doivent commencer et se terminer par une lettre minuscule ou un chiffre.
...
- ClawHub ne prend pas en charge les compétences payantes, la tarification par compétence, les péages d’accès ni le partage de revenus.
