...
x-i18n:
    generated_at: "2026-07-02T22:30:23Z"
    model: gpt-5.5
...
- La liste d’autorisations des extensions se trouve dans `packages/schema/src/textFiles.ts` (`TEXT_FILE_EXTENSIONS`).
...
- Les types de contenu commençant par `text/` sont traités comme du texte, ainsi qu’une petite liste d’autorisations (JSON/YAML/TOML/JS/TS/Markdown/SVG).

Limites (côté serveur) :

- Taille totale du bundle : 50 Mo.
- Le texte d’intégration inclut `SKILL.md` + jusqu’à environ 40 fichiers non-`.md` (limite au mieux).
