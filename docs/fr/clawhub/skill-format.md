...
summary: Format du dossier Skill, fichiers obligatoires, types de fichiers autorisés, limites.
x-i18n:
    generated_at: "2026-07-01T12:58:50Z"
    model: gpt-5.5
...
- tout fichier _texte_ de prise en charge (voir « Fichiers autorisés »)
...
les fichiers `SKILL.md` ou l’ancien `skills.md` dans des dépôts publics, non dérivés, appartenant
...
- `description` est utilisé comme résumé de la Skill dans l’interface utilisateur/la recherche.
...
| `primaryEnv`       | `string`   | Variable d’environnement principale contenant les identifiants pour votre Skill.                                                                                         |
...
---
name: todoist-cli
description: Gérer les tâches, projets et étiquettes Todoist depuis la ligne de commande.
version: 1.2.0
metadata:
  openclaw:
    requires:
...
- Les types de contenu commençant par `text/` sont traités comme du texte, ainsi qu’une petite liste d’autorisation (JSON/YAML/TOML/JS/TS/Markdown/SVG).
...
- Tout le monde peut utiliser, modifier et redistribuer les Skills publiées, y compris commercialement.
...
- N’ajoutez pas de métadonnées de prix à `SKILL.md` ; elles ne font pas partie du format de Skill et ne rendront pas payante une Skill publiée.
