...
read_when:
    ...
    - Vous mettez à jour les métadonnées du paquet de plugin avant la publication
summary: Corriger les constats de validation du paquet du Plugin ClawHub avant la publication
title: Corrections de validation des Plugins
x-i18n:
    generated_at: "2026-07-01T08:01:33Z"
    model: gpt-5.5
...
ClawHub valide les paquets de Plugin avant publication et peut également afficher les constats issus des
analyses automatisées de paquets. Cette page couvre les constats destinés aux auteurs, c’est-à-dire
les constats que l’auteur du Plugin peut corriger dans les métadonnées de son paquet, son manifeste, ses importations SDK
...
- Ajoutez `openclaw.runtimeExtensions` lorsque le paquet publié doit charger du JavaScript
  compilé.
- Conservez tous les chemins de point d’entrée dans le répertoire du paquet.
...
- Vérifiez toutes les métadonnées de build OpenClaw dans le paquet, comme la version OpenClaw
...
Il manque à l’artefact empaqueté des métadonnées OpenClaw présentes dans votre paquet
source.
...
- Remplacez les imports du SDK interne réservé d’OpenClaw par des sous-chemins publics documentés
  `openclaw/plugin-sdk/*`.
...
Le plugin utilise encore un helper obsolète d’écriture de magasin de session complet, tel que
...
Le plugin utilise encore des helpers obsolètes de bas niveau pour les transcriptions, tels que
`appendSessionTranscriptMessage` ou `emitSessionTranscriptUpdate`.

- Utilisez `appendSessionTranscriptMessageByIdentity(...)` pour les ajouts aux transcriptions.
...
- Déplacez le travail de mutation d’invite ou de contexte vers `before_prompt_build`.
...
- Gardez `providerAuthEnvVars` uniquement comme métadonnée de compatibilité tant que votre plage
...
- Répliquez le setup de canal piloté par les variables d’environnement dans les métadonnées actuelles de setup, de configuration de canal ou
  de canal de package utilisées par la forme de votre plugin.
- Gardez `channelEnvVars` uniquement comme métadonnée de compatibilité tant que les anciennes versions prises en charge
...
Le package distribue `openclaw.security.json` avec une référence de schéma que ClawHub
...
Le package distribue un fichier de manifeste de sécurité non pris en charge.
