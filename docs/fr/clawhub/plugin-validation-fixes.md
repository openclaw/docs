...
summary: Corriger les problèmes de validation du package Plugin ClawHub avant publication
title: Correctifs de validation des Plugins
x-i18n:
    generated_at: "2026-07-02T17:36:38Z"
    model: gpt-5.5
...
d’analyses automatisées des paquets. Cette page couvre les constats destinés aux auteurs, c’est-à-dire
...
| `sdk-session-store-write`               | [Remplacer les écritures dans le stockage de session complet](/fr/clawhub/plugin-validation-fixes#sdk-session-store-write)                                  |
...
- Ajoutez des métadonnées de compatibilité et d’installation lorsque le paquet sera publié ou
...
- Remplissez `openclaw.install` avec la source d’installation prise en charge, par exemple
...
- Utilisez la version de l’API de Plugin OpenClaw, ou le plancher semver, avec laquelle vous avez construit et testé
  le paquet.
- Gardez-la séparée de la version du paquet. La version du paquet décrit la
...
- Vérifiez toute métadonnée de build OpenClaw dans le paquet, comme la version d’OpenClaw
  utilisée pendant la publication.
- Alignez la version minimale de l’hôte avec la plage de versions de l’hôte que le paquet
...
- Si `openclaw.plugin.json` comporte aussi `version`, mettez-la à jour pour la faire correspondre ou supprimez
...
- Conservez les points d’entrée du paquet, la compatibilité, l’installation, la configuration et les métadonnées de catalogue
...
- Utilisez `getSessionEntry(...)` pour lire les métadonnées de session par agent et par identité de session.
- Utilisez `patchSessionEntry(...)` ou `upsertSessionEntry(...)` pour persister les métadonnées de session.
- Utilisez les helpers d’identité de transcription ou de cible lorsque le code prépare une
...
- Utilisez `resolveSessionTranscriptTarget(...)` lorsque le code a besoin d’une cible structurée d’opération de transcription.
...
Le plugin utilise encore des helpers de transcription de bas niveau obsolètes, tels que
...
- Préférez la surface d’exécution de transcription structurée afin qu’OpenClaw puisse appliquer les
  limites de transaction et la gestion de l’identité correctes.
...
- Conservez `providerAuthEnvVars` uniquement comme métadonnée de compatibilité tant que votre plage
...
Le manifeste utilise des métadonnées anciennes ou antérieures de variables d’environnement de canal sans les métadonnées
de configuration ou de setup actuelles attendues par ClawHub.
...
- Reflétez la configuration de canal pilotée par l’environnement dans les métadonnées actuelles de setup, de configuration de canal ou
  de canal de paquet utilisées par la forme de votre plugin.
- Conservez `channelEnvVars` uniquement comme métadonnée de compatibilité tant que les anciennes versions d’OpenClaw prises en charge
  l’exigent encore.
...
- Supprimez l’URL du schéma si elle n’est fournie qu’à titre indicatif.
