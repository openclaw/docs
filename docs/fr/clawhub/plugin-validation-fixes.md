...
read_when:
    ...
    - ClawHub a rejeté la publication d’un package Plugin ou émis un avertissement à son sujet
...
x-i18n:
    generated_at: "2026-07-04T17:56:39Z"
    model: gpt-5.5
...
les résultats que l’auteur du Plugin peut corriger dans les métadonnées du package, le manifeste, les
imports SDK ou l’artéfact publié.
...
s’adressent aux mainteneurs d’OpenClaw plutôt qu’aux auteurs de Plugin.
...
| `package-npm-pack-unavailable`          | [Rendre l’artéfact npm empaquetable](/fr/clawhub/plugin-validation-fixes#package-npm-pack-unavailable)                                 |
...
- Utilisez [Créer des plugins](/fr/plugins/building-plugins) pour un exemple de package
  minimal et [Manifeste Plugin](/fr/plugins/manifest#manifest-versus-packagejson)
...
- Consultez [Points d’entrée Plugin](/fr/plugins/sdk-entrypoints) et
...
- Consultez [Points d’entrée Plugin](/fr/plugins/sdk-entrypoints).
- Relancez `clawhub package validate <path-to-plugin>`.
...
Le package ne déclare pas la plage d’API Plugin OpenClaw qu’il prend en charge.

- Ajoutez `openclaw.compat.pluginApi` à `package.json`.
- Utilisez la version de l’API Plugin OpenClaw ou le plancher semver avec lequel vous avez compilé et testé.
...
- Vérifiez toutes les métadonnées de compilation OpenClaw dans le package, comme la version OpenClaw
  utilisée pendant la publication.
- Alignez la version minimale de l’hôte avec la plage de versions d’hôte que le package
...
- Si `openclaw.plugin.json` contient aussi `version`, mettez-la à jour pour la faire correspondre ou supprimez
...
- Consultez [Manifeste Plugin](/fr/plugins/manifest).
- Relancez `clawhub package validate <path-to-plugin>`.
...
- Gardez les points d’entrée du package, la compatibilité, l’installation, la configuration et les métadonnées
  de catalogue dans les champs `package.json#openclaw` pris en charge.
- Consultez [Champs package.json qui affectent la découverte](/fr/plugins/manifest#packagejson-fields-that-affect-discovery).
- Relancez `clawhub package validate <path-to-plugin>`.
...
Le package ne peut pas être empaqueté dans l’artéfact que ClawHub inspecterait ou
publierait.
...
Le package peut être empaqueté, mais l’artéfact empaqueté ne contient pas les
...
L’artéfact empaqueté ne contient pas les métadonnées OpenClaw qui existent dans votre package
source.

- Exécutez `npm pack --dry-run` et inspectez les fichiers de métadonnées inclus.
- Assurez-vous que `package.json` inclut le bloc `openclaw` dans l’artéfact empaqueté.
...
- Utilisez [Conventions d’import](/fr/plugins/building-plugins#import-conventions) et
  [Sous-chemins du SDK Plugin](/fr/plugins/sdk-subpaths) pour trouver l’import restreint.
- Relancez `clawhub package validate <path-to-plugin>`.
...
Le Plugin utilise encore l’assistant obsolète de magasin complet de sessions
`loadSessionStore`.
...
- Utilisez `patchSessionEntry(...)` ou `upsertSessionEntry(...)` lors de l’écriture de l’état
  de session.
- Évitez de charger, modifier et enregistrer l’objet complet de magasin de sessions.
...
Le Plugin utilise encore un assistant d’écriture obsolète pour le magasin complet de sessions, comme
...
- Gardez les assistants de transcription bas niveau uniquement tant que votre plage de compatibilité déclarée
  prend encore en charge les anciennes versions d’OpenClaw qui les exigent.
- Consultez [API d’exécution](/fr/plugins/sdk-runtime#agent-session-state) et
  [Sous-chemins du SDK Plugin](/fr/plugins/sdk-subpaths).
- Relancez `clawhub package validate <path-to-plugin>`.
...
- Consultez la [référence de configuration](/fr/plugins/manifest#setup-reference) et
...
- Répliquez la configuration de canal pilotée par les variables d’environnement dans les métadonnées actuelles de setup, de configuration de canal ou
...
- Supprimez l’URL du schéma si elle est uniquement consultative.
...
- Gardez les comportements sensibles à la sécurité documentés dans la documentation publique de votre package ou
