...
summary: Corriger les constats de validation des packages de Plugin ClawHub avant la publication
title: Correctifs de validation de Plugin
x-i18n:
    generated_at: "2026-07-03T00:55:48Z"
    model: gpt-5.5
...
analyses automatisées de package. Cette page couvre les constats destinés aux auteurs, c’est-à-dire
...
contient des codes de maintenance du scanner sans recommandations de remédiation pour l’auteur, ils
...
| Code                                    | Commencez ici                                                                                                             |
| --------------------------------------- | ------------------------------------------------------------------------------------------------------------------------- |
| `package-json-missing`                  | [Ajouter les métadonnées de package](/fr/clawhub/plugin-validation-fixes#package-json-missing)                               |
...
- Utilisez la version de l’API de Plugin OpenClaw ou le plancher semver avec lesquels vous avez compilé et testé.
...
- Conservez les métadonnées de points d’entrée de package, de compatibilité, d’installation, de configuration et de catalogue
...
Il manque dans l’artefact empaqueté des métadonnées OpenClaw qui existent dans votre package
source.
...
Le Plugin utilise encore un helper d'écriture obsolète du store de session complet, tel que
...
- Évitez de charger, modifier et enregistrer l'objet complet du store de session.
- Gardez les helpers d'écriture du store complet uniquement tant que votre plage de compatibilité déclarée
...
- Gardez les helpers de transcription bas niveau uniquement tant que votre plage de compatibilité déclarée
  prend encore en charge les anciennes versions d'OpenClaw qui les exigent.
- Consultez [API d'exécution](/fr/plugins/sdk-runtime#agent-session-state) et
  [Sous-chemins du SDK de Plugin](/fr/plugins/sdk-subpaths).
- Réexécutez `clawhub package validate <path-to-plugin>`.
...
- Dupliquez les métadonnées de variables d'environnement du fournisseur dans `setup.providers[].envVars`.
...
Le manifeste utilise des métadonnées de variables d'environnement de canal héritées ou anciennes sans les
...
- Dupliquez le setup de canal piloté par les variables d'environnement dans le setup actuel, la configuration de canal ou les
