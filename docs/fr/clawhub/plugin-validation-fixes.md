---
read_when:
    - Vous avez exécuté clawhub package validate et devez corriger les constats liés au Plugin
    - ClawHub a rejeté ou signalé par avertissement la publication d’un package de Plugin
...
x-i18n:
    generated_at: "2026-07-02T08:11:50Z"
    model: gpt-5.5
...
ClawHub valide les packages de Plugin avant leur publication et peut aussi afficher les résultats des analyses automatisées de packages. Cette page couvre les résultats destinés aux auteurs, c’est-à-dire les résultats que l’auteur du Plugin peut corriger dans les métadonnées du package, le manifeste, les imports du SDK ou l’artefact publié.

Elle ne couvre pas les résultats de couverture internes de Plugin Inspector. Si un rapport complet contient des codes de maintenance du scanner sans consignes de correction pour l’auteur, ils sont destinés aux mainteneurs d’OpenClaw plutôt qu’aux auteurs de Plugins.
...
| `sdk-load-session-store`                | [Remplacer l’accès au stockage complet de session](/fr/clawhub/plugin-validation-fixes#sdk-load-session-store)                |
| `sdk-session-store-write`               | [Remplacer les écritures dans le stockage complet de session](/fr/clawhub/plugin-validation-fixes#sdk-session-store-write)    |
...
La racine du package ne contient pas `package.json`, ClawHub ne peut donc pas identifier le package npm, la version, les points d’entrée ni les métadonnées OpenClaw.
...
- Ajoutez `openclaw.runtimeExtensions` lorsque le package publié doit charger du JavaScript généré.
- Gardez tous les chemins de points d’entrée dans le répertoire du package.
...
Le Plugin utilise encore un assistant d’écriture obsolète de magasin de session complet, tel que
...
- Utilisez les assistants d’identité de transcription ou de cible lorsque le code prépare une
  opération de transcription.
- Ne persistez pas et ne dépendez pas des chemins de fichiers de transcription hérités.
- Consultez [API d’exécution](/fr/plugins/sdk-runtime#agent-session-state) et
  [Sous-chemins du SDK de Plugin](/fr/plugins/sdk-subpaths).
- Relancez `clawhub package validate <path-to-plugin>`.
...
- Évitez de lire ou de construire directement des cibles de fichiers de transcription héritées.
- Conservez l’assistant hérité uniquement tant que votre plage de compatibilité déclarée prend encore en charge
  les anciennes versions d’OpenClaw qui l’exigent.
- Consultez [API d’exécution](/fr/plugins/sdk-runtime#agent-session-state) et
  [Sous-chemins du SDK de Plugin](/fr/plugins/sdk-subpaths).
- Relancez `clawhub package validate <path-to-plugin>`.
...
Le Plugin utilise encore des assistants de transcription bas niveau obsolètes, tels que
...
- Préférez la surface d’exécution de transcription structurée afin qu’OpenClaw puisse appliquer les
  bonnes limites transactionnelles et la bonne gestion d’identité.
- Conservez les assistants de transcription bas niveau uniquement tant que votre plage de compatibilité déclarée
  prend encore en charge les anciennes versions d’OpenClaw qui les exigent.
- Consultez [API d’exécution](/fr/plugins/sdk-runtime#agent-session-state) et
  [Sous-chemins du SDK de Plugin](/fr/plugins/sdk-subpaths).
- Relancez `clawhub package validate <path-to-plugin>`.
...
- Conservez `before_agent_start` uniquement tant que votre plage de compatibilité déclarée prend encore en charge
  les anciennes versions d’OpenClaw qui l’exigent.
...
- Reproduisez les métadonnées de variables d’environnement de fournisseur dans `setup.providers[].envVars`.
...
- Reproduisez le setup de canal piloté par les variables d’environnement dans les métadonnées actuelles de setup, de configuration de canal ou
