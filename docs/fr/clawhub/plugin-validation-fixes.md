...
x-i18n:
    generated_at: "2026-07-02T14:02:44Z"
    model: gpt-5.5
...
- Gardez les métadonnées de points d’entrée, de compatibilité, d’installation, de configuration et de catalogue du package
...
- Utilisez `openclaw/plugin-sdk/channel-core` pour les helpers de points d’entrée de canal.
...
Le Plugin utilise encore le helper obsolète pour le store de session complet
`loadSessionStore`.
...
Le Plugin utilise encore un helper d’écriture obsolète pour le store de session complet, tel que
...
Le Plugin utilise encore des helpers de transcript de bas niveau obsolètes tels que
...
- Voir la [référence setup](/fr/plugins/manifest#setup-reference) et la
...
- Reproduisez le setup de canal piloté par les variables d’environnement dans les métadonnées actuelles de setup, de config de canal ou
