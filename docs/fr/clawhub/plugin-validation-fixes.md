---
read_when:
    - Vous avez exécuté clawhub package validate et devez corriger les signalements du Plugin
...
summary: Corriger les problèmes relevés par la validation du package de plugin ClawHub avant publication
title: Correctifs de validation des Plugins
x-i18n:
    generated_at: "2026-07-02T22:29:17Z"
    model: gpt-5.5
...
ClawHub valide les packages de plugins avant publication et peut aussi afficher les résultats des analyses automatisées de packages. Cette page couvre les résultats destinés aux auteurs, c’est-à-dire ceux que l’auteur du plugin peut corriger dans les métadonnées du package, le manifeste, les imports SDK ou l’artefact publié.

Elle ne couvre pas les résultats de couverture internes du Plugin Inspector. Si un rapport complet contient des codes de maintenance du scanner sans consignes de remédiation pour l’auteur, ils sont destinés aux mainteneurs OpenClaw plutôt qu’aux auteurs de plugins.
...
| `sdk-session-file-helper`               | [Remplacer les helpers de chemin de fichier de session](/fr/clawhub/plugin-validation-fixes#sdk-session-file-helper)                                   |
...
- Remplissez `openclaw.install` avec la source d’installation prise en charge, telle que `clawhubSpec`, `npmSpec` ou `localPath`.
...
- Vérifiez toute métadonnée de build OpenClaw dans le package, comme la version OpenClaw utilisée pendant la publication.
- Alignez la version minimale de l’hôte sur la plage de versions de l’hôte réellement prise en charge par le package.
- Consultez [champs package.json qui affectent la découverte](/fr/plugins/manifest#packagejson-fields-that-affect-discovery).
- Relancez `clawhub package validate <path-to-plugin>`.
...
- Gardez les points d’entrée, la compatibilité, l’installation, la configuration et les métadonnées de catalogue du package dans les champs `package.json#openclaw` pris en charge.
- Consultez [champs package.json qui affectent la découverte](/fr/plugins/manifest#packagejson-fields-that-affect-discovery).
- Relancez `clawhub package validate <path-to-plugin>`.
...
- Déplacez les métadonnées de package ou d’installation vers les champs pris en charge
  `package.json#openclaw` au lieu du manifeste.
- Relancez `clawhub package validate <path-to-plugin>`.
...
- Utilisez les [conventions d’import](/fr/plugins/building-plugins#import-conventions) et
  les [sous-chemins du SDK de Plugin](/fr/plugins/sdk-subpaths) pour trouver l’import précis.
- Relancez `clawhub package validate <path-to-plugin>`.
...
- Remplacez les imports SDK internes réservés d’OpenClaw par des sous-chemins publics documentés
  `openclaw/plugin-sdk/*`.
- Si le comportement n’a pas de SDK public, conservez l’assistant dans votre package ou
...
Le plugin utilise encore un assistant d’écriture obsolète de magasin de session complet, comme
...
Le plugin utilise encore des assistants de chemins de fichiers de session obsolètes, comme
...
- Utilisez `resolveSessionTranscriptIdentity(...)` lorsque le code n’a besoin que de l’identité de session
  publique.
...
- Préférez la surface d’exécution structurée du transcript afin qu’OpenClaw puisse appliquer les
  limites de transaction et la gestion d’identité appropriées.
- Conservez les assistants de transcript bas niveau uniquement tant que votre plage de compatibilité déclarée
  prend encore en charge les anciennes versions d’OpenClaw qui les exigent.
- Consultez l’[API d’exécution](/fr/plugins/sdk-runtime#agent-session-state) et les
  [sous-chemins du SDK de Plugin](/fr/plugins/sdk-subpaths).
- Relancez `clawhub package validate <path-to-plugin>`.
