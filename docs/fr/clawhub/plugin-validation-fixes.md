---
read_when:
    - Vous avez exécuté clawhub package validate et devez corriger les problèmes signalés concernant le Plugin
...
x-i18n:
    generated_at: "2026-07-03T17:19:40Z"
    model: gpt-5.5
...
ClawHub valide les paquets de Plugins avant publication et peut aussi afficher des constats issus
d’analyses automatisées de paquets. Cette page couvre les constats destinés aux auteurs, c’est-à-dire
...
contient des codes de maintenance du scanner sans consignes de correction pour l’auteur, ils sont
...
- Ajoutez `openclaw.runtimeExtensions` lorsque le paquet publié doit charger du JavaScript
  compilé.
- Gardez tous les chemins de points d’entrée à l’intérieur du répertoire du paquet.
...
- Utilisez la version de l’API de Plugin OpenClaw ou le plancher semver avec lesquels vous avez construit et testé
  le paquet.
- Gardez cette information séparée de la version du paquet. La version du paquet décrit la
...
La version minimale de l’hôte indiquée par le paquet ne correspond pas aux métadonnées de version OpenClaw
...
- Remplacez les imports depuis le barrel racine par des imports publics ciblés de sous-chemins.
...
- Utilisez `getSessionEntry(...)` pour lire les métadonnées de session par identité d’agent et de
  session.
...
- Utilisez `publishSessionTranscriptUpdateByIdentity(...)` pour les notifications de mise à jour de
  transcription.
...
- Gardez `channelEnvVars` uniquement comme métadonnées de compatibilité tant que les anciennes versions d’OpenClaw
  prises en charge l’exigent encore.
