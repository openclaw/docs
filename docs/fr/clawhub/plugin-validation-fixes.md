...
x-i18n:
    generated_at: "2026-06-30T22:14:28Z"
    model: gpt-5.5
...
des analyses automatisées de packages. Cette page couvre les constats destinés aux auteurs, c’est-à-dire
...
contient des codes de maintenance du scanner sans recommandations de correction pour les auteurs, ceux-ci
...
| `legacy-root-sdk-import`                | [Remplacer les imports du SDK racine](/fr/clawhub/plugin-validation-fixes#legacy-root-sdk-import)                                             |
...
- Utilisez [Créer des Plugins](/fr/plugins/building-plugins) pour un exemple minimal de package
  et [Manifeste du Plugin](/fr/plugins/manifest#manifest-versus-packagejson)
...
- Vérifiez toutes les métadonnées de construction OpenClaw dans le package, comme la version OpenClaw
...
- Conservez les points d’entrée de package, la compatibilité, l’installation, la configuration et les métadonnées de catalogue
...
- Évitez de charger, modifier et enregistrer l’objet complet du magasin de session.
- Gardez les assistants d’écriture sur le magasin complet uniquement tant que votre plage de compatibilité déclarée
...
Le plugin utilise encore des assistants de transcription de bas niveau obsolètes comme
...
- Gardez les assistants de transcription de bas niveau uniquement tant que votre plage de compatibilité déclarée
  prend encore en charge les anciennes versions d’OpenClaw qui les exigent.
- Consultez l’[API d’exécution](/fr/plugins/sdk-runtime#agent-session-state) et les
  [sous-chemins du SDK de Plugin](/fr/plugins/sdk-subpaths).
- Réexécutez `clawhub package validate <path-to-plugin>`.
...
- Déplacez le travail de modification d’invite ou de contexte vers `before_prompt_build`.
...
- Consultez les [hooks](/fr/plugins/hooks) et la
  [compatibilité de Plugin](/fr/plugins/compatibility).
- Réexécutez `clawhub package validate <path-to-plugin>`.
...
- Gardez les métadonnées de variables d’environnement de canal déclaratives afin qu’OpenClaw puisse inspecter l’état
  du setup sans charger l’exécution du canal.
