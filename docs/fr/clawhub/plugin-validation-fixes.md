...
read_when:
    ...
    - ClawHub a rejeté la publication d’un paquet Plugin ou a émis un avertissement
    - Vous mettez à jour les métadonnées du package de Plugin avant la publication
summary: Corriger les constats de validation du paquet de plugin ClawHub avant publication
title: Correctifs de validation des Plugins
x-i18n:
    generated_at: "2026-07-04T15:14:34Z"
    model: gpt-5.5
...
ceux que l’auteur du Plugin peut corriger dans les métadonnées du package, le manifeste, les imports SDK
ou l’artefact publié.
...
- Utilisez la version de l’API Plugin OpenClaw ou le plancher semver avec lesquels vous avez compilé et testé
  le package.
...
- Vérifiez toute métadonnée de compilation OpenClaw dans le package, par exemple la version OpenClaw
...
- Conservez les métadonnées de Plugin natives dans `openclaw.plugin.json`.
...
L’artefact empaqueté ne contient pas les métadonnées OpenClaw présentes dans votre package
source.
...
- Utilisez les [conventions d’import](/fr/plugins/building-plugins#import-conventions) et
  les [sous-chemins du Plugin SDK](/fr/plugins/sdk-subpaths) pour trouver l’import étroit.
- Relancez `clawhub package validate <path-to-plugin>`.
...
Le plugin utilise encore un helper d’écriture obsolète pour le magasin de session complet, comme
...
- Utilisez `resolveSessionTranscriptIdentity(...)` lorsque le code n’a besoin que de l’identité publique
  de session.
- Utilisez `resolveSessionTranscriptTarget(...)` lorsque le code a besoin d’une cible d’opération de transcription
  structurée.
...
Le plugin utilise encore des helpers de transcription bas niveau obsolètes comme
...
- Gardez `before_agent_start` uniquement tant que votre plage de compatibilité déclarée prend encore
  en charge d’anciennes versions d’OpenClaw qui l’exigent.
- Consultez [Hooks](/fr/plugins/hooks) et la
