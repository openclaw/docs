---
read_when:
    - Vous avez exécuté `clawhub package validate` et devez corriger les constatations du Plugin
...
summary: Corriger les constats de validation du paquet Plugin ClawHub avant la publication
title: Correctifs de validation des Plugins
x-i18n:
    generated_at: "2026-07-01T05:39:25Z"
    model: gpt-5.5
...
ClawHub valide les paquets de Plugins avant publication et peut aussi afficher les constats issus des analyses automatisées de paquets. Cette page couvre les constats destinés aux auteurs, c’est-à-dire ceux que l’auteur du Plugin peut corriger dans les métadonnées de son paquet, son manifeste, ses imports SDK ou son artefact publié.

Elle ne couvre pas les constats de couverture internes de Plugin Inspector. Si un rapport complet contient des codes de maintenance du scanner sans consignes de remédiation pour l’auteur, ils sont destinés aux mainteneurs OpenClaw plutôt qu’aux auteurs de Plugins.
...
| `package-plugin-api-compat-missing`     | [Déclarer la compatibilité avec l’API du Plugin](/fr/clawhub/plugin-validation-fixes#package-plugin-api-compat-missing)                          |
...
| `sdk-session-store-write`               | [Remplacer les écritures du magasin de session complet](/fr/clawhub/plugin-validation-fixes#sdk-session-store-write)                                  |
...
| `security-manifest-schema-unavailable`  | [Supprimer les références de schéma de manifeste de sécurité indisponibles](/fr/clawhub/plugin-validation-fixes#security-manifest-schema-unavailable) |
...
La racine du paquet n’inclut pas `package.json`, ClawHub ne peut donc pas identifier le paquet npm, sa version, ses points d’entrée ni les métadonnées OpenClaw.

- Ajoutez `package.json` avec `name`, `version` et `type`.
- Ajoutez un bloc `openclaw` lorsque le paquet fournit un Plugin OpenClaw.
- Utilisez [Créer des Plugins](/fr/plugins/building-plugins) pour un exemple de paquet minimal et [Manifeste de Plugin](/fr/plugins/manifest#manifest-versus-packagejson) pour la séparation entre paquet et manifeste.
...
- Incluez les métadonnées de point d’entrée telles que `openclaw.extensions` ou `openclaw.runtimeExtensions`.
...
- Conservez tous les chemins de point d’entrée à l’intérieur du répertoire du paquet.
- Consultez [Points d’entrée de Plugin](/fr/plugins/sdk-entrypoints) et [champs package.json qui influencent la découverte](/fr/plugins/manifest#packagejson-fields-that-affect-discovery).
- Relancez `clawhub package validate <path-to-plugin>`.
...
Le paquet ne déclare pas la plage d’API du Plugin OpenClaw qu’il prend en charge.

- Ajoutez `openclaw.compat.pluginApi` à `package.json`.
- Utilisez la version de l’API de Plugin OpenClaw ou la borne semver minimale contre laquelle vous avez développé et testé.
...
La version minimale d’hôte du paquet ne correspond pas aux métadonnées de version OpenClaw contre lesquelles le paquet a été construit.

- Vérifiez `openclaw.install.minHostVersion`.
- Vérifiez toute métadonnée de build OpenClaw dans le paquet, comme la version OpenClaw utilisée lors de la publication.
...
La version du paquet et la version du manifeste du Plugin ne concordent pas.

- Préférez `package.json#version` comme version de publication du paquet.
- Si `openclaw.plugin.json` contient aussi `version`, mettez-la à jour pour qu’elle corresponde, ou supprimez les métadonnées de version de manifeste obsolètes lorsque les métadonnées du paquet font autorité.
...
- Conservez les points d’entrée, la compatibilité, l’installation, la configuration et les métadonnées de catalogue du paquet dans les champs pris en charge de `package.json#openclaw`.
- Consultez [champs package.json qui influencent la découverte](/fr/plugins/manifest#packagejson-fields-that-affect-discovery).
- Relancez `clawhub package validate <path-to-plugin>`.
...
- Corrigez les métadonnées de paquet invalides, les scripts de cycle de vie cassés ou les entrées `files` qui empêchent l’empaquetage.
...
Il manque dans l’artefact empaqueté des métadonnées OpenClaw qui existent dans votre paquet source.
...
## Migration du SDK et de compatibilité

### legacy-root-sdk-import

Le Plugin importe depuis le barrel racine du SDK déprécié :
`openclaw/plugin-sdk`.
...
- Utilisez les [conventions d’import](/fr/plugins/building-plugins#import-conventions) et
  les [sous-chemins du SDK de Plugin](/fr/plugins/sdk-subpaths) pour trouver l’import étroit.
- Relancez `clawhub package validate <path-to-plugin>`.
...
- Remplacez les imports du SDK interne réservé d’OpenClaw par des sous-chemins
...
Le Plugin utilise encore l’assistant déprécié de magasin de session complet
`loadSessionStore`.
...
- Utilisez `patchSessionEntry(...)` ou `upsertSessionEntry(...)` lors de l’écriture de l’état
  de session.
- Évitez de charger, modifier et enregistrer tout l’objet de magasin de session.
...
Le Plugin utilise encore un assistant déprécié d’écriture de magasin de session complet tel que
...
- Évitez de charger, modifier et enregistrer tout l’objet de magasin de session.
- Gardez les assistants d’écriture de magasin complet uniquement tant que votre plage de compatibilité déclarée
...
Le Plugin utilise encore des assistants dépréciés de chemins de fichiers de session tels que
...
- Évitez de lire ou de construire directement les anciennes cibles de fichiers de transcript.
- Gardez l’ancien assistant uniquement tant que votre plage de compatibilité déclarée prend encore
...
Le Plugin utilise encore des assistants de transcript bas niveau dépréciés tels que
...
- Reflétez les métadonnées des variables d’environnement du fournisseur dans `setup.providers[].envVars`.
...
- Reflétez le setup de canal piloté par les variables d’environnement dans les métadonnées actuelles de setup, de config de canal ou
...
- Supprimez l’URL du schéma si elle est seulement informative.
