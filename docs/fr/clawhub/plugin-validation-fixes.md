...
summary: Corrigez les problèmes détectés lors de la validation des packages de Plugin ClawHub avant la publication
title: Correctifs de validation des Plugins
x-i18n:
    generated_at: "2026-07-12T21:37:57Z"
    model: gpt-5.6
...
des analyses automatisées des paquets. Cette page traite des résultats destinés aux auteurs, c’est-à-dire
ceux que l’auteur du plugin peut corriger dans les métadonnées de son paquet, son manifeste, ses imports
du SDK ou son artefact publié.

Elle ne traite pas des résultats de couverture internes de Plugin Inspector. Si un rapport complet
contient des codes de maintenance de l’analyseur sans instructions de correction pour l’auteur, ceux-ci
sont destinés aux mainteneurs d’OpenClaw plutôt qu’aux auteurs de plugins.
...
| `package-plugin-api-compat-missing`     | [Déclarer la compatibilité de l’API des plugins](/fr/clawhub/plugin-validation-fixes#package-plugin-api-compat-missing)                          |
...
| `sdk-session-file-helper`               | [Remplacer les utilitaires de chemin de fichier de session](/fr/clawhub/plugin-validation-fixes#sdk-session-file-helper)                                   |
...
- Consultez [Création de plugins](/fr/plugins/building-plugins) pour obtenir un exemple de paquet
  minimal et [Manifeste de plugin](/fr/plugins/manifest#manifest-versus-packagejson)
  pour comprendre la répartition entre le paquet et le manifeste.
- Réexécutez `clawhub package validate <path-to-plugin>`.

### package-openclaw-metadata-missing

Le paquet contient `package.json`, mais ne déclare pas les métadonnées de paquet
OpenClaw.
...
- Consultez [Champs de package.json influant sur la découverte](/fr/plugins/manifest#packagejson-fields-that-affect-discovery).
- Réexécutez `clawhub package validate <path-to-plugin>`.
...
- Consultez [Points d’entrée des plugins](/fr/plugins/sdk-entrypoints) et
  [Champs de package.json influant sur la découverte](/fr/plugins/manifest#packagejson-fields-that-affect-discovery).
- Réexécutez `clawhub package validate <path-to-plugin>`.
...
- Utilisez la version de l’API des plugins OpenClaw ou la version semver minimale avec laquelle vous avez
  développé et testé le paquet.
- Distinguez-la de la version du paquet. La version du paquet décrit la
...
OpenClaw avec lesquelles le paquet a été compilé.
...
Le bloc `package.json#openclaw` contient des champs qui ne sont pas pris en charge dans les
métadonnées de paquet OpenClaw.
...
- Conservez les métadonnées des points d’entrée, de compatibilité, d’installation, de configuration et de catalogue
...
- Déplacez les métadonnées du paquet ou de l’installation vers les champs pris en charge de `package.json#openclaw`
...
- Utilisez `openclaw/plugin-sdk/channel-core` pour les assistants de point d’entrée des canaux.
- Consultez les [Conventions d’importation](/fr/plugins/building-plugins#import-conventions) et les
  [Sous-chemins du SDK de plugin](/fr/plugins/sdk-subpaths) pour trouver l’importation la plus précise.
- Réexécutez `clawhub package validate <path-to-plugin>`.
...
- Remplacez les importations internes réservées du SDK OpenClaw par des sous-chemins publics documentés
  `openclaw/plugin-sdk/*`.
- Si le comportement ne dispose d’aucun SDK public, conservez l’assistant dans votre paquet ou
...
Le plugin utilise encore l’assistant obsolète de chargement de l’ensemble du stockage des sessions
`loadSessionStore`.
...
Le plugin utilise encore un assistant obsolète d’écriture de l’ensemble du stockage des sessions, tel que
...
- Évitez de charger, modifier et enregistrer l’intégralité de l’objet de stockage des sessions.
- Conservez les assistants d’écriture de l’ensemble du stockage uniquement tant que votre plage de compatibilité déclarée
...
Le plugin utilise encore des assistants obsolètes de chemin de fichier de session, tels que
...
- Utilisez `patchSessionEntry(...)` ou `upsertSessionEntry(...)` pour conserver les métadonnées de
  session.
- Utilisez les assistants d’identité ou de cible de transcription lorsque le code prépare une
  opération de transcription.
- Ne conservez pas et ne dépendez pas des anciens chemins de fichiers de transcription.
- Consultez l’[API d’exécution](/fr/plugins/sdk-runtime#agent-session-state) et les
  [Sous-chemins du SDK de plugin](/fr/plugins/sdk-subpaths).
- Réexécutez `clawhub package validate <path-to-plugin>`.
...
- Conservez l’ancien assistant uniquement tant que votre plage de compatibilité déclarée
  prend encore en charge les anciennes versions d’OpenClaw qui l’exigent.
- Consultez l’[API d’exécution](/fr/plugins/sdk-runtime#agent-session-state) et les
  [Sous-chemins du SDK de plugin](/fr/plugins/sdk-subpaths).
- Réexécutez `clawhub package validate <path-to-plugin>`.
...
Le plugin utilise encore des assistants de transcription de bas niveau obsolètes, tels que
...
- Utilisez `publishSessionTranscriptUpdateByIdentity(...)` pour les notifications de mise à jour de la
  transcription.
- Privilégiez la surface d’exécution structurée des transcriptions afin qu’OpenClaw puisse appliquer les
...
Le manifeste utilise des métadonnées anciennes ou obsolètes de variables d’environnement de canal sans les métadonnées actuelles
de configuration ou de paramétrage attendues par ClawHub.

- Veillez à ce que les métadonnées des variables d’environnement du canal restent déclaratives afin qu’OpenClaw puisse inspecter l’état de la configuration
  sans charger l’environnement d’exécution du canal.
- Reproduisez la configuration du canal pilotée par l’environnement dans les métadonnées actuelles de configuration, de paramétrage du canal ou
