...
x-i18n:
    generated_at: "2026-07-03T23:31:22Z"
    model: gpt-5.5
...
ClawHub valide les packages de Plugins avant publication et peut aussi afficher les constats issus des analyses automatisées de packages. Cette page couvre les constats destinés aux auteurs, c’est-à-dire les constats que l’auteur du Plugin peut corriger dans les métadonnées de son package, son manifeste, ses imports SDK ou son artefact publié.

Elle ne couvre pas les constats de couverture internes de Plugin Inspector. Si un rapport complet contient des codes de maintenance du scanner sans indications de correction pour l’auteur, ils sont destinés aux mainteneurs d’OpenClaw plutôt qu’aux auteurs de Plugins.
...
| `package-plugin-api-compat-missing`     | [Déclarer la compatibilité avec l’API Plugin](/fr/clawhub/plugin-validation-fixes#package-plugin-api-compat-missing)          |
...
- Utilisez la version de l’API Plugin OpenClaw ou le plancher semver avec lesquels vous avez développé et testé.
...
- Vérifiez toutes les métadonnées de build OpenClaw dans le package, comme la version d’OpenClaw utilisée lors de la publication.
...
Le package peut être empaqueté, mais l’artefact empaqueté ne contient pas les fichiers de point d’entrée déclarés dans `package.json#openclaw`.
...
Le plugin utilise encore un assistant d’écriture obsolète du magasin de session complet, tel que
...
Le plugin utilise encore des assistants de transcript de bas niveau obsolètes, tels que
...
- Voir [référence setup](/fr/plugins/manifest#setup-reference) et
