...
read_when:
    ...
    - ClawHub a rejeté ou averti lors de la publication d’un package de Plugin
...
x-i18n:
    generated_at: "2026-07-01T20:19:41Z"
    model: gpt-5.5
...
ClawHub valide les packages de Plugins avant la publication et peut également afficher les constats issus d’analyses automatisées de packages. Cette page couvre les constats destinés aux auteurs, c’est-à-dire ceux que l’auteur du Plugin peut corriger dans les métadonnées de son package, son manifeste, ses imports SDK ou son artefact publié.
...
| `sdk-session-transcript-file-target`    | [Remplacer les anciennes cibles de fichiers de transcript](/fr/clawhub/plugin-validation-fixes#sdk-session-transcript-file-target)                   |
| `sdk-session-transcript-low-level`      | [Remplacer les helpers de transcript de bas niveau](/fr/clawhub/plugin-validation-fixes#sdk-session-transcript-low-level)                       |
...
La racine du package n’inclut pas `package.json`, ClawHub ne peut donc pas identifier le package npm, sa version, ses points d’entrée ni les métadonnées OpenClaw.

- Ajoutez `package.json` avec `name`, `version` et `type`.
- Ajoutez un bloc `openclaw` lorsque le package livre un Plugin OpenClaw.
...
Le package possède `package.json`, mais il ne déclare pas de métadonnées de package OpenClaw.
...
- Conservez tous les chemins de point d’entrée à l’intérieur du répertoire du package.
...
- Gardez cela séparé de la version du package. La version du package décrit la publication du Plugin ; `openclaw.compat.pluginApi` décrit le contrat d’API de l’hôte.
- Consultez [Champs package.json qui influencent la découverte](/fr/plugins/manifest#packagejson-fields-that-affect-discovery).
- Relancez `clawhub package validate <path-to-plugin>`.
...
- Vérifiez toutes les métadonnées de compilation OpenClaw dans le package, comme la version d’OpenClaw utilisée lors de la publication.
...
- Si `openclaw.plugin.json` comporte aussi `version`, mettez-la à jour pour qu’elle corresponde, ou supprimez les métadonnées de version de manifeste obsolètes lorsque les métadonnées du package font autorité.
...
- Conservez les points d’entrée, la compatibilité, l’installation, la configuration et les métadonnées de catalogue du package dans les champs `package.json#openclaw` pris en charge.
- Consultez [Champs package.json qui influencent la découverte](/fr/plugins/manifest#packagejson-fields-that-affect-discovery).
- Relancez `clawhub package validate <path-to-plugin>`.
...
- Déplacez plutôt les métadonnées de package ou d’installation dans les champs `package.json#openclaw`
...
- Remplacez les imports SDK internes OpenClaw réservés par des sous-chemins publics
...
- Consultez l’[API d’exécution](/fr/plugins/sdk-runtime#agent-session-state) et
  les [sous-chemins du SDK de Plugin](/fr/plugins/sdk-subpaths).
- Relancez `clawhub package validate <path-to-plugin>`.
...
Le Plugin utilise encore des helpers obsolètes de chemins de fichiers de session tels que
...
- Conservez le helper hérité uniquement tant que votre plage de compatibilité déclarée prend encore
...
- Utilisez `publishSessionTranscriptUpdateByIdentity(...)` pour les notifications de mise à jour de transcript.
- Préférez la surface d’exécution de transcript structurée afin qu’OpenClaw puisse appliquer les
  bonnes limites de transaction et la gestion correcte de l’identité.
- Conservez les helpers de transcript bas niveau uniquement tant que votre plage de compatibilité déclarée
  prend encore en charge les anciennes versions d’OpenClaw qui les exigent.
- Consultez l’[API d’exécution](/fr/plugins/sdk-runtime#agent-session-state) et
  les [sous-chemins du SDK de Plugin](/fr/plugins/sdk-subpaths).
- Relancez `clawhub package validate <path-to-plugin>`.
...
- Conservez `providerAuthEnvVars` uniquement comme métadonnées de compatibilité tant que votre plage OpenClaw prise en charge
  en a encore besoin.
...
Le manifeste utilise des métadonnées de variables d’environnement de canal héritées ou anciennes sans les métadonnées de
...
- Répliquez le setup de canal piloté par variables d’environnement dans les métadonnées de setup, de configuration de canal ou
  de canal de package actuelles utilisées par la forme de votre Plugin.
- Conservez `channelEnvVars` uniquement comme métadonnées de compatibilité tant que les anciennes versions d’OpenClaw prises en charge
  l’exigent encore.
- Consultez le [manifeste de Plugin](/fr/plugins/manifest) et
  les [Plugins de canal](/fr/plugins/sdk-channel-plugins).
- Relancez `clawhub package validate <path-to-plugin>`.
...
- Conservez le comportement sensible à la sécurité documenté dans la documentation publique de votre package ou
