...
title: Correctifs de validation de Plugin
x-i18n:
    generated_at: "2026-07-04T06:30:26Z"
    model: gpt-5.5
...
Elle ne couvre pas les constats de couverture internes du Plugin Inspector. Si un rapport complet contient des codes de maintenance du scanner sans consignes de remédiation pour l’auteur, ils sont destinés aux mainteneurs d’OpenClaw plutôt qu’aux auteurs de Plugins.
...
| `package-openclaw-entry-missing`        | [Déclarer les points d’entrée de package OpenClaw](/fr/clawhub/plugin-validation-fixes#package-openclaw-entry-missing)              |
...
| `sdk-load-session-store`                | [Remplacer l’accès à l’intégralité du stockage de session](/fr/clawhub/plugin-validation-fixes#sdk-load-session-store)              |
| `sdk-session-store-write`               | [Remplacer les écritures de l’intégralité du stockage de session](/fr/clawhub/plugin-validation-fixes#sdk-session-store-write)      |
...
La racine du package n’inclut pas `package.json`, ClawHub ne peut donc pas identifier le package npm, la version, les points d’entrée ni les métadonnées OpenClaw.
...
Le package contient `package.json`, mais il ne déclare pas de métadonnées de package OpenClaw.
...
- Consultez [Points d’entrée de Plugin](/fr/plugins/sdk-entrypoints) et [Champs de package.json qui influencent la découverte](/fr/plugins/manifest#packagejson-fields-that-affect-discovery).
- Relancez `clawhub package validate <path-to-plugin>`.

### package-entrypoint-missing

Le package déclare un point d’entrée OpenClaw, mais le fichier référencé manque dans le package en cours de validation.
...
- Utilisez la version de l’API de Plugin OpenClaw, ou le plancher semver, avec laquelle vous avez développé et testé.
- Gardez cela séparé de la version du package. La version du package décrit la publication du Plugin ; `openclaw.compat.pluginApi` décrit le contrat de l’API hôte.
- Consultez [Champs de package.json qui influencent la découverte](/fr/plugins/manifest#packagejson-fields-that-affect-discovery).
- Relancez `clawhub package validate <path-to-plugin>`.

### package-min-host-version-drift

La version minimale de l’hôte du package ne correspond pas aux métadonnées de version OpenClaw avec lesquelles le package a été compilé.

- Vérifiez `openclaw.install.minHostVersion`.
- Vérifiez toutes les métadonnées de compilation OpenClaw dans le package, comme la version d’OpenClaw utilisée pendant la publication.
- Alignez la version minimale de l’hôte sur la plage de versions d’hôte réellement prise en charge par le package.
- Consultez [Champs de package.json qui influencent la découverte](/fr/plugins/manifest#packagejson-fields-that-affect-discovery).
- Relancez `clawhub package validate <path-to-plugin>`.

### package-manifest-version-drift

La version du package et la version du manifeste de Plugin ne correspondent pas.

- Préférez `package.json#version` comme version de publication du package.
- Si `openclaw.plugin.json` contient aussi `version`, mettez-la à jour pour qu’elle corresponde, ou supprimez les métadonnées obsolètes de version du manifeste lorsque les métadonnées du package font autorité.
- Publiez une nouvelle version du package après modification des métadonnées publiées.
- Consultez [Manifeste de Plugin](/fr/plugins/manifest).
- Relancez `clawhub package validate <path-to-plugin>`.

### package-openclaw-unsupported-metadata

Le bloc `package.json#openclaw` contient des champs qui ne font pas partie des métadonnées de package OpenClaw prises en charge.

- Supprimez les champs non pris en charge comme `openclaw.bundle`.
- Gardez les métadonnées de Plugin natives dans `openclaw.plugin.json`.
- Gardez les points d’entrée du package, la compatibilité, l’installation, la configuration et les métadonnées de catalogue dans les champs `package.json#openclaw` pris en charge.
- Consultez [Champs de package.json qui influencent la découverte](/fr/plugins/manifest#packagejson-fields-that-affect-discovery).
- Relancez `clawhub package validate <path-to-plugin>`.
...
- Déplacez plutôt les métadonnées de paquet ou d’installation dans les champs `package.json#openclaw` pris en charge
...
- Utilisez les [conventions d’import](/fr/plugins/building-plugins#import-conventions) et les
  [sous-chemins du SDK de Plugin](/fr/plugins/sdk-subpaths) pour trouver l’import précis.
- Relancez `clawhub package validate <path-to-plugin>`.
...
- Évitez de charger, modifier et enregistrer l’objet entier du magasin de session.
...
- Voir l’[API d’exécution](/fr/plugins/sdk-runtime#agent-session-state) et les
...
- Utilisez `getSessionEntry(...)` pour lire les métadonnées de session par agent et par
  identité de session.
...
- Ne persistez pas et ne dépendez pas des chemins de fichiers de transcript hérités.
- Voir l’[API d’exécution](/fr/plugins/sdk-runtime#agent-session-state) et les
  [sous-chemins du SDK de Plugin](/fr/plugins/sdk-subpaths).
- Relancez `clawhub package validate <path-to-plugin>`.
...
- Évitez de lire ou de construire directement des cibles de fichier de transcript héritées.
...
Le Plugin utilise encore des helpers de transcript bas niveau obsolètes, comme
...
- Gardez `providerAuthEnvVars` uniquement comme métadonnées de compatibilité tant que votre plage OpenClaw prise en charge
  en a encore besoin.
...
Le manifeste utilise des métadonnées héritées ou plus anciennes de variables d’environnement de canal sans les métadonnées actuelles
...
Le paquet livre un fichier manifeste de sécurité non pris en charge.
