...
Utilisez d’abord `--dry-run` pour prévisualiser les métadonnées résolues du paquet, les champs de compatibilité,
...
les métadonnées, liens sources, versions, journaux de modifications et l’état d’analyse :
...
la modération peuvent être masquées des interfaces de recherche et d’installation jusqu’à leur résolution.
