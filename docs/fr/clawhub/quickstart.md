...
summary: 'Commencer à utiliser ClawHub : rechercher, installer, mettre à jour et publier des Skills ou des plugins.'
x-i18n:
    generated_at: "2026-07-03T13:26:39Z"
    model: gpt-5.5
...
ClawHub est un registre pour les compétences et plugins OpenClaw.
...
## Trouver et installer une compétence
...
OpenClaw enregistre la provenance de la compétence afin que les mises à jour ultérieures puissent continuer à
...
Les environnements sans interface graphique peuvent utiliser un jeton d’API depuis l’interface web ClawHub :
...
Une compétence est un dossier avec un fichier `SKILL.md` obligatoire et des fichiers de support
facultatifs.
...
La commande ignore le contenu inchangé. Les nouvelles compétences commencent à `1.0.0` ; les modifications ultérieures
...
Utilisez d’abord `--dry-run` pour prévisualiser les métadonnées de paquet résolues, les champs de
...
les métadonnées, les liens de source, les versions, les journaux de modifications et l’état d’analyse :
...
la modération peuvent être masquées de la recherche et des surfaces d’installation jusqu’à leur résolution.
