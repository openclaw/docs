...
read_when:
    ...
    - Installer un Skill ou un Plugin depuis le registre
...
x-i18n:
    generated_at: "2026-07-03T00:56:05Z"
    model: gpt-5.5
...
ClawHub est un registre de Skills et de plugins OpenClaw.
...
Utilisez le préfixe `clawhub:` lorsque vous souhaitez qu’OpenClaw résolve le package via
...
Une Skill est un dossier contenant un fichier `SKILL.md` obligatoire et des
fichiers de support facultatifs.
...
Utilisez d’abord `--dry-run` pour prévisualiser les métadonnées de paquet résolues, les champs
...
Avant l’installation, utilisez la page web de ClawHub ou les commandes de détail de la CLI pour inspecter
...
Les listes publiques affichent le dernier état d’analyse. Les versions retenues ou bloquées par
