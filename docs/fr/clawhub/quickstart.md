...
des flux de travail propres au registre.
...
d’environnement, les outils et les autorisations requis afin que les utilisateurs puissent comprendre ce dont la
Skill a besoin avant de l’installer. Consultez [Format des Skills](/fr/clawhub/skill-format).

Pour les dépôts contenant plusieurs Skills, le flux de travail GitHub réutilisable appelle
`skill publish` pour chaque dossier de Skill directement placé sous `skills/` :
...
compatibilité, l’attribution de la source et le plan de téléversement sans effectuer de publication.
...
Les fiches publiques affichent l’état de la dernière analyse. Les versions retenues ou bloquées par
