...
x-i18n:
    generated_at: "2026-07-02T00:50:10Z"
    model: gpt-5.5
...
- `--workdir <dir>` : répertoire de travail (par défaut : cwd ; utilise l’espace de travail Clawdbot de repli s’il est configuré)
...
- Interactif distant/sans interface : `clawhub login --device` affiche un code et attend que vous l’autorisiez sur `<site>/cli/device`.
...
- La recherche privilégie les correspondances exactes de jetons slug/nom avant la popularité des téléchargements. Un jeton slug autonome tel que `map` correspond plus fortement à `personal-map` qu’à la sous-chaîne dans `amap`.
...
- Récupère les métadonnées du skill et les fichiers de version sans installation.
- `--version <version>` : inspecter une version spécifique (par défaut : la dernière).
- `--tag <tag>` : inspecter une version étiquetée (p. ex. `latest`).
...
- Envoie une télémétrie au mieux pendant que vous êtes connecté afin que les nombres d’installations actuels puissent être
  désactivés.
...
- Les skills épinglés rejettent également `install --force` afin que les octets locaux ne puissent pas être remplacés accidentellement.
...
- Les nouveaux skills utilisent `1.0.0` par défaut ; les skills modifiés utilisent par défaut la version de correctif suivante.
...
- `--migrate-owner` déplace un skill existant vers `--owner` pendant la publication d’une nouvelle
...
- Analyse le répertoire de travail actuel, le répertoire de skills configuré et tous les dossiers
  `--root <dir>` pour trouver les dossiers de skills locaux contenant `SKILL.md` ou
  `skill.md`.
...
- Les analyses sont asynchrones et peuvent prendre du temps. Pendant la file d’attente, l’indicateur du terminal affiche la position priorisée actuelle de l’analyse et le nombre d’analyses devant elle.
...
- `--update` est valide uniquement avec `--slug` ; il réécrit les résultats d’analyse publiée réussis dans la version sélectionnée.
...
- Les téléchargements de Plugins utilisent le nom du package et nécessitent `--kind plugin`.
...
- `root` vaut par défaut `skills` pour les dépôts de catalogue.
...
- La publication de skills V1 utilise `clawhub_token` ; la publication de confiance GitHub OIDC est réservée aux packages pour le moment.

### `delete <skill>`

- Sans `--version`, supprimez logiquement une skill (propriétaire, modérateur ou administrateur).
- Appelle `DELETE /api/v1/skills/{slug}`.
- Les suppressions logiques déclenchées par le propriétaire réservent le slug pendant 30 jours ; la commande affiche l’heure d’expiration.
...
- `--reason <text>` enregistre une note de modération sur la suppression logique de toute la skill et dans le journal d’audit.
- `--note <text>` est un alias de `--reason`.
- `--yes` ignore la confirmation.

### `undelete <skill>`

- Restaurer une skill masquée (propriétaire, modérateur ou administrateur).
...
- Masquer une skill (propriétaire, modérateur ou administrateur).
- Alias de `delete`.

### `unhide <skill>`

- Afficher de nouveau une skill (propriétaire, modérateur ou administrateur).
- Alias de `undelete`.

### `skill rename <skill> <new-name>`

- Renommer une skill détenue et conserver le slug précédent comme alias de redirection.
- Appelle `POST /api/v1/skills/{slug}/rename`.
- `--yes` ignore la confirmation.

### `skill merge <source> <target>`

- Fusionner une skill détenue dans une autre skill détenue.
...
- Parcourt ou recherche dans le catalogue de paquets unifié via `GET /api/v1/packages` et `GET /api/v1/packages/search`.
- Utilisez ceci pour les plugins et les autres entrées de famille de paquets ; `search` au niveau supérieur reste la surface de recherche de skills.
- Options :
...
- Options :
  ...
  - `--force` : remplacer un fichier de sortie existant.
  - `--json` : sortie lisible par machine.
...
- Avec des options directes de condensat, vérifie sans recherche réseau.
- Options :
...
- Exécute l’Inspecteur Plugin intégré à la CLI ClawHub sur un dossier local de paquet de Plugin.
- Par défaut, effectue une validation hors ligne/statique, sans localiser ni importer un checkout
  OpenClaw local.
- Les erreurs de compatibilité bloquantes quittent avec un code non nul. Les constats limités aux avertissements sont affichés mais
  quittent avec un code zéro.
- Options :
  - `--out <dir>` : écrire les rapports de l’Inspecteur Plugin dans ce répertoire.
  - `--openclaw <path>` : inspecter avec un checkout OpenClaw local explicite.
  - `--runtime` : activer la capture à l’exécution ; importe le code du Plugin.
  - `--allow-execute` : autoriser la capture à l’exécution dans un espace de travail isolé.
  - `--no-mock-sdk` : désactiver le SDK OpenClaw simulé pendant la capture à l’exécution.
  - `--json` : sortie lisible par machine.
...
Si la validation signale un constat concernant le paquet, le manifeste, l’import du SDK ou l’artefact, consultez
[Correctifs de validation de Plugin](/clawhub/plugin-validation-fixes), puis relancez la commande.

### `package delete <name>`

- Sans `--version`, supprime logiquement un paquet et toutes ses versions.
- `--version <version>` supprime définitivement une version détenue non la plus récente via une route fail-closed,
  spécifique à la version.
  Les versions supprimées ne peuvent pas être restaurées ni republiées. Publiez un remplacement avant de supprimer la
  version actuellement la plus récente. Ce flux limité à une version nécessite le propriétaire du paquet ou un administrateur
  d’éditeur d’organisation ; le personnel de la plateforme ne contourne pas la propriété du paquet.
- La suppression logique de tout le paquet nécessite le propriétaire du paquet, un propriétaire/administrateur d’éditeur d’organisation, un
...
- Restaure un paquet supprimé logiquement et ses versions.
- Il n’existe pas de restauration de version ; les versions supprimées définitivement ne peuvent pas être restaurées.
...
- Les signalements sont au niveau du paquet, éventuellement liés à une version, et deviennent visibles
  par les modérateurs pour examen.
- Les signalements ne masquent pas automatiquement les paquets et ne bloquent pas les téléchargements par eux-mêmes.
- Options :
...
- Affiche l’état actuel d’analyse du paquet, le nombre de signalements ouverts, l’état de modération manuelle de la dernière version,
...
- Vérifie si un paquet est prêt pour une future utilisation par OpenClaw.
...
- Les métadonnées sont détectées automatiquement depuis `package.json`, `openclaw.plugin.json` et
  les marqueurs réels de bundle OpenClaw comme `.codex-plugin/plugin.json`,
...
- Les dossiers de Plugin de code sont empaquetés dans une archive tarball npm ClawPack avant le téléversement afin que
  les installations OpenClaw puissent vérifier l’artefact exact. Les dossiers de Plugin de bundle continuent
...
- Pour les dossiers locaux, l’attribution de la source est détectée automatiquement depuis le dépôt git local lorsque le remote origin pointe vers GitHub.
...
- `--owner <handle>` publie sous un handle d’éditeur utilisateur ou organisation lorsque l’acteur dispose d’un accès éditeur.
- Les noms de packages délimités par portée doivent correspondre au propriétaire sélectionné. Voir `docs/publishing.md`.
- Les indicateurs existants (`--family`, `--name`, `--version`, `--source-repo`, `--source-commit`, `--source-ref`, `--source-path`) fonctionnent toujours comme surcharges.
...
- Pour les monorepos, transmettez `source_path` afin que le workflow publie le dossier
  de package du Plugin, par exemple `source_path: extensions/codex`.
- Épinglez le workflow réutilisable à un tag stable ou à un SHA de commit complet. N’exécutez pas la publication de version depuis `@main`.
- `pull_request` doit utiliser `dry_run: true` afin que la CI reste sans pollution.
- Les publications réelles doivent être limitées aux événements de confiance tels que `workflow_dispatch` ou les pushes de tags.
- La publication de confiance sans secret fonctionne uniquement sur `workflow_dispatch` ; les pushes de tags nécessitent toujours `clawhub_token`.
...
- Utilisez ceci après avoir défini la configuration pour confirmer le dépôt, le nom de fichier du workflow
  et l’éventuel environnement épinglé.
- Indicateurs :
  - `--json` : sortie lisible par machine.
...
- Une fois la configuration définie, les futures publications GitHub Actions prises en charge peuvent utiliser
  OIDC/la publication de confiance sans jeton ClawHub longue durée.
...
- ClawHub vérifie le dépôt GitHub configuré lorsque cette commande s’exécute.
  ...
  privés exigent que ClawHub ait accès à ce dépôt GitHub, par
  exemple via une future installation de GitHub App ClawHub ou une autre intégration GitHub
  autorisée.
- Indicateurs :
...
- Utilisez ceci comme retour arrière si le workflow, le dépôt ou l’environnement épinglé doit
