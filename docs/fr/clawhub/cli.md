...
x-i18n:
    generated_at: "2026-07-02T17:35:23Z"
    model: gpt-5.5
...
- Si une skill devrait apparaître mais n’apparaît pas, exécutez `clawhub inspect @owner/slug` en étant connecté pour vérifier les diagnostics de modération visibles par le propriétaire avant de renommer les métadonnées.
...
- Envoie de la télémétrie au mieux lorsqu’une session est ouverte afin que les décomptes d’installation actuels puissent être
  désactivés.
...
- Supprime l’épinglage du fichier de verrouillage pour une skill installée afin que les futures mises à jour puissent la modifier.
...
- Les nouvelles skills utilisent `1.0.0` par défaut ; les skills modifiées utilisent par défaut la version
  corrective suivante.
...
- Analyse le répertoire de travail actuel, le répertoire de skills configuré et tout
  dossier `--root <dir>` à la recherche de dossiers de skills locaux contenant `SKILL.md` ou
  `skill.md`.
- Compare chaque empreinte de skill locale avec ClawHub et ne publie que les skills nouvelles ou
  modifiées.
- Les nouvelles skills sont publiées en `1.0.0` ; les skills modifiées publient par défaut la version corrective
  suivante. Utilisez `--bump minor|major` pour les lots de mise à jour qui doivent avancer d’un
...
- Les analyses sont asynchrones et peuvent prendre du temps. Pendant la mise en file, l’indicateur du terminal affiche la position actuelle de l’analyse priorisée et le nombre d’analyses en attente devant elle.
...
- `--output <file.zip>` télécharge l’archive de rapport complète avec `manifest.json`, `clawscan.json`, `skillspector.json`, `static-analysis.json`, `virustotal.json` et `README.md`.
- `--json` affiche la réponse d’interrogation complète pour l’automatisation.
- Les analyses de chemin local ne sont plus prises en charge. Téléversez une nouvelle version, puis utilisez `scan download` pour récupérer les résultats d’analyse stockés pour cette version soumise.
...
- Nécessite `clawhub login`.
- Télécharge le ZIP du rapport d’analyse stocké pour une version de skill ou de Plugin soumise, y compris les versions qui ont été bloquées ou masquées par les contrôles de sécurité ClawHub.
- Les téléchargements de skills utilisent le slug de la skill et utilisent `--kind skill` par défaut.
...
- La publication de skills V1 utilise `clawhub_token` ; la publication de confiance GitHub OIDC est réservée aux paquets pour le moment.

### `delete <skill>`

- Sans `--version`, supprime logiquement une compétence (propriétaire, modérateur ou administrateur).
- Appelle `DELETE /api/v1/skills/{slug}`.
- Les suppressions logiques initiées par le propriétaire réservent le slug pendant 30 jours ; la commande affiche l’heure d’expiration.
- `--version <version>` supprime définitivement une version possédée non la plus récente via une route à échec fermé,
...
- `--reason <text>` enregistre une note de modération sur une suppression logique de compétence entière et dans le journal d’audit.
...
- Les transferts vers des identifiants d’utilisateurs créent une demande en attente que le destinataire accepte.
...
- Parcourt ou recherche le catalogue unifié de paquets via `GET /api/v1/packages` et `GET /api/v1/packages/search`.
- Utilisez ceci pour les plugins et les autres entrées de familles de paquets ; `search` de niveau supérieur reste la surface de recherche de compétences.
- Indicateurs :
...
- Les anciennes versions ZIP se téléchargent via l’ancienne route ZIP.
- Indicateurs :
...
- Calcule le SHA-256 ClawHub, l’intégrité npm `sha512` et le shasum npm pour un artéfact
  local.
...
- Exécute le Plugin Inspector intégré à la CLI ClawHub sur un dossier de paquet Plugin
  local.
- Utilise par défaut une validation hors ligne/statique, sans localiser ni importer un checkout
  OpenClaw local.
- Les erreurs de compatibilité bloquantes renvoient un code de sortie non nul. Les constats limités aux avertissements sont affichés mais
  renvoient un code de sortie zéro.
- Indicateurs :
...
Si la validation signale un problème de paquet, manifeste, import SDK ou artéfact, consultez
[Correctifs de validation de Plugin](/clawhub/plugin-validation-fixes), puis relancez la commande.

### `package delete <name>`

- Sans `--version`, supprime logiquement un paquet et toutes ses versions publiées.
- `--version <version>` supprime définitivement une version possédée non la plus récente via une route à échec fermé,
  spécifique à la version.
  Les versions supprimées ne peuvent pas être restaurées ni republiées. Publiez un remplacement avant de supprimer la
...
- La suppression logique d’un paquet entier nécessite le propriétaire du paquet, un propriétaire/administrateur d’éditeur d’organisation, un modérateur
...
- Restaure un paquet supprimé logiquement et ses versions publiées.
- Il n’existe pas de restauration de version ; les versions supprimées définitivement ne peuvent pas être restaurées.
...
- Nécessite un accès administrateur à la fois au propriétaire actuel du paquet et à l’éditeur de destination,
  sauf si l’action est effectuée par un administrateur de plateforme.
- Les noms de paquets scopés doivent être transférés au propriétaire de scope correspondant.
...
- Indicateurs :
  ...
  - `--reason <text>` : raison obligatoire du signalement.
  - `--json` : sortie lisible par machine.
...
- Signale les blocages concernant le statut officiel, la disponibilité ClawPack, le condensat d’artéfact,
...
- Appelle le même point de terminaison de préparation calculée que `package readiness`, mais affiche
  un état axé sur la migration, la dernière version, l’état de paquet officiel, les vérifications et
  les blocages.
- Indicateurs :
  - `--json` : sortie lisible par machine.
...
- Crée un éditeur d’organisation possédé par l’utilisateur authentifié.
...
- Pour les sources GitHub, l’attribution de source est renseignée automatiquement depuis le dépôt, le commit résolu, la ref et le sous-chemin.
...
- `--owner <handle>` publie sous l’identifiant d’un éditeur utilisateur ou organisation lorsque l’acteur dispose d’un accès éditeur.
- Les noms de packages avec scope doivent correspondre au propriétaire sélectionné. Consultez `docs/publishing.md`.
...
- Si vous utilisez une ancienne version de la CLI `clawhub`, mettez-la à niveau avant de publier afin que
  les vérifications de prévol locales s’exécutent avant le téléversement.
...
- Gardez `clawhub_token` disponible pour la première publication, les packages non fiables ou les publications de secours.
...
- Attache ou remplace la configuration d’éditeur de confiance GitHub Actions pour un package
  existant.
- Le package doit d’abord être créé via la commande normale manuelle ou authentifiée par jeton
...
- ClawHub vérifie le dépôt GitHub configuré lorsque cette commande s’exécute.
  ...
  privés exigent que ClawHub dispose d’un accès GitHub à ce dépôt, par
...
- Utilisez cette commande comme retour arrière si le workflow, le dépôt ou le verrouillage d’environnement doit
