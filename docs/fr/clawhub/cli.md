...
(par ex. conteneurs Docker, VPS Hetzner avec accès Internet uniquement via proxy, pare-feu
d’entreprise).
...
- Par défaut : ouvre le navigateur sur `<site>/cli/auth` et termine via le rappel local loopback.
...
- La recherche privilégie les correspondances exactes de jetons de slug/nom avant la popularité des téléchargements. Un jeton de slug autonome tel que `map` correspond plus fortement à `personal-map` qu’à la sous-chaîne dans `amap`.
...
- Liste les Skills les plus récents via `/api/v1/skills?limit=...&sort=createdAt` (triés par `createdAt` desc).
- Options :
...
- Affiche `pinned` à côté des Skills gelées avec `clawhub pin`, y compris le motif facultatif.

### `pin <slug>`

- Marque une Skill installée comme épinglée dans le fichier de verrouillage.
- `--reason <text>` enregistre la raison pour laquelle la Skill est gelée.
...
- Supprime de façon réversible une Skill (propriétaire, modérateur ou administrateur).
...
- Renomme une Skill détenue et conserve le slug précédent comme alias de redirection.
- Appelle `POST /api/v1/skills/{slug}/rename`.
- `--yes` ignore la confirmation.

### `skill merge <source-slug> <target-slug>`

- Fusionne une Skill détenue dans une autre Skill détenue.
- Le slug source cesse d’apparaître publiquement et devient un alias de redirection vers la cible.
- Appelle `POST /api/v1/skills/{sourceSlug}/merge`.
- `--yes` ignore la confirmation.

### `skill rescan <slug>`

- Demande une nouvelle analyse de sécurité pour la dernière version publiée de la Skill.
- Les propriétaires et administrateurs d’éditeurs peuvent réanalyser leurs propres Skills jusqu’à la limite de
  récupération par version.
- Les modérateurs et administrateurs de la plateforme peuvent réanalyser n’importe quelle Skill et ne sont pas bloqués par la
  limite de récupération du propriétaire, bien qu’une seule réanalyse puisse s’exécuter à la fois par version.
- Appelle `POST /api/v1/skills/{slug}/rescan`.
- Options :
...
- Les transferts vers des identifiants d’organisation/éditeur s’appliquent immédiatement uniquement lorsque l’acteur dispose d’un
  accès administrateur au propriétaire actuel et à l’éditeur de destination.
- Sous-commandes :
...
- Utilisez ceci pour les plugins et autres entrées de famille de packages ; `search` de premier niveau reste la surface de recherche de Skills.
- Options :
...
- `--version <version>` : inspecte une version spécifique (par défaut : dernière).
...
- Avec les options de condensé directes, vérifie sans requête réseau.
- Options :
...
- Supprime logiquement un package et toutes ses releases.
- Nécessite le propriétaire du package, un propriétaire/admin de l’éditeur d’organisation, un modérateur de plateforme,
...
- Nécessite un accès admin au propriétaire actuel du package et à l’éditeur de destination,
...
- Demande une nouvelle analyse de sécurité pour la dernière release publiée du package.
- Les propriétaires et les admins d’éditeurs peuvent relancer l’analyse de leurs propres packages jusqu’à la limite de récupération
  par release.
- Les modérateurs et admins de plateforme peuvent relancer l’analyse de n’importe quel package et ne sont pas bloqués par
  la limite de récupération du propriétaire, même si une seule nouvelle analyse peut s’exécuter à la fois par release.
- Appelle `POST /api/v1/packages/{name}/rescan`.
- Options :
  - `--yes` : ignorer la confirmation.
  - `--json` : sortie lisible par machine.
...
- Commande de propriétaire/éditeur pour contester la modération d’une release.
- Appelle `POST /api/v1/packages/{name}/appeal`.
- Les appels sont acceptés pour les releases mises en quarantaine, révoquées, suspectes ou malveillantes.
- Options :
  - `--version <version>` : version du package requise.
  - `--message <text>` : message d’appel requis.
  - `--json` : sortie lisible par machine.
...
- Signale les bloqueurs pour le statut officiel, la disponibilité ClawPack, le condensat d’artefact,
...
- Appelle le même point de terminaison de readiness calculée que `package readiness`, mais imprime
...
- Les sources `.tgz` sont traitées comme ClawPack. La CLI téléverse les octets npm-pack exacts
  ...
  des métadonnées.
- Les dossiers de plugins de code sont empaquetés dans une archive tar npm ClawPack avant téléversement afin que
...
- Les plugins de code externes doivent déclarer explicitement `openclaw.compat.pluginApi` et
  ...
  Le champ de premier niveau `package.json.version` n’est pas utilisé comme fallback pour la validation de publication.
...
Pour les plugins de code, la publication depuis un dossier crée et téléverse un artefact ClawPack à partir
...
- `package.json.version` est la version de release de votre package, mais elle n’est pas utilisée comme
  fallback pour la validation de compatibilité/build OpenClaw.
...
- Si vous utilisez une ancienne release de la CLI `clawhub`, effectuez une mise à niveau avant de publier afin que
  les vérifications préalables locales s’exécutent avant le téléversement.

#### GitHub Actions

ClawHub fournit aussi un workflow réutilisable officiel à l’adresse
...
- Pour les monorepos, passez `source_path` afin que le workflow publie le dossier du package
  du plugin, par exemple `source_path: extensions/codex`.
- Épinglez le workflow réutilisable à une balise stable ou à un SHA de commit complet. N’exécutez pas la publication de release depuis `@main`.
- `pull_request` doit utiliser `dry_run: true` afin que la CI ne pollue pas l’état.
- Les vraies publications doivent être limitées aux événements de confiance tels que `workflow_dispatch` ou les poussées de tags.
- La publication de confiance sans secret fonctionne uniquement sur `workflow_dispatch` ; les poussées de tags nécessitent toujours `clawhub_token`.
...
- Les racines peuvent être n’importe quel dossier : un répertoire de Skills ou un dossier de Skill unique avec `SKILL.md`.
...
- Envoyée pendant `sync` lorsque vous êtes connecté, sauf si `CLAWHUB_DISABLE_TELEMETRY=1` (héritage `CLAWDHUB_DISABLE_TELEMETRY=1`).
