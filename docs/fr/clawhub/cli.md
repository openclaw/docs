...
- `--workdir <dir>` : répertoire de travail (par défaut : cwd ; revient à l’espace de travail Clawdbot s’il est configuré)
...
(p. ex. conteneurs Docker, VPS Hetzner avec Internet uniquement via proxy, pare-feux
d’entreprise).
...
- La recherche privilégie les correspondances exactes de jeton slug/nom avant la popularité des téléchargements. Un jeton de slug autonome comme `map` correspond à `personal-map` plus fortement que la sous-chaîne dans `amap`.
...
- Récupère les métadonnées de la Skill et les fichiers de version sans installation.
...
- Les skills épinglés sont ignorés par `update --all` et rejetés par un `update <slug>` direct.
...
- `update --all` ignore les slugs épinglés et affiche un récapitulatif de ce qui est resté figé.
...
- Supprime temporairement un skill (propriétaire, modérateur ou admin).
...
- Utilisez ceci pour les plugins et les autres entrées de familles de packages ; `search` au niveau supérieur reste la surface de recherche de skills.
- Indicateurs :
...
- Requiert le propriétaire du paquet, un propriétaire/admin d’éditeur d’organisation, un modérateur de plateforme,
...
- Restaure un paquet et ses versions supprimés de manière réversible.
- Requiert le propriétaire du paquet, un propriétaire/admin d’éditeur d’organisation, un modérateur de plateforme,
  ou un admin de plateforme.
...
- Requiert un accès admin au propriétaire actuel du paquet et à l’éditeur
  de destination, sauf si l’opération est effectuée par un admin de plateforme.
...
- Les signalements s’appliquent au niveau du paquet, peuvent être liés à une version, et deviennent visibles
...
- Commande de propriétaire pour vérifier la visibilité de modération du paquet.
- Appelle `GET /api/v1/packages/{name}/moderation`.
- Affiche l’état actuel d’analyse du paquet, le nombre de signalements ouverts, l’état de modération manuelle
  de la dernière version, l’état de blocage des téléchargements et les raisons de modération.
- Indicateurs :
  - `--json` : sortie lisible par machine.
...
- Signale les blocages concernant le statut officiel, la disponibilité ClawPack, le condensat de l’artifact,
...
- Affiche l’état de migration orienté opérateur pour un paquet pouvant remplacer un
  plugin OpenClaw intégré.
- Appelle le même point de terminaison de préparation calculée que `package readiness`, mais affiche
  un état axé migration, la dernière version, l’état du paquet officiel, les vérifications et
  les blocages.
- Indicateurs :
  - `--json` : sortie lisible par machine.
...
- Les métadonnées sont détectées automatiquement à partir de `package.json`, `openclaw.plugin.json` et
  de véritables marqueurs de bundle OpenClaw comme `.codex-plugin/plugin.json`,
...
- Les dossiers de plugins de code sont empaquetés dans une archive tar npm ClawPack avant le téléversement afin que
  les installations OpenClaw puissent vérifier l’artifact exact. Les dossiers de plugins groupés continuent
  d’utiliser le chemin de publication par fichiers extraits.
- Pour les sources GitHub, l’attribution source est automatiquement renseignée à partir du dépôt, du commit résolu, de la ref et du sous-chemin.
- Pour les dossiers locaux, l’attribution source est détectée automatiquement depuis le git local lorsque le remote d’origine pointe vers GitHub.
...
l’attribution source avant de créer une version active :
...
Pour les plugins de code, la publication depuis un dossier construit et téléverse un artifact ClawPack depuis
...
- Si vous utilisez une ancienne version de la CLI `clawhub`, mettez-la à niveau avant la publication afin que
...
- Épinglez le workflow réutilisable sur une balise stable ou un SHA de commit complet. N’exécutez pas la publication de version depuis `@main`.
...
- Le workflow téléverse le résultat JSON comme artifact et l’expose comme sorties de workflow.
...
- Envoyée pendant `sync` lorsque l’utilisateur est connecté, sauf si `CLAWHUB_DISABLE_TELEMETRY=1` (ancien `CLAWDHUB_DISABLE_TELEMETRY=1`).
