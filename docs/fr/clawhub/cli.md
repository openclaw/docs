...
- Ajoute/retire un Skill de vos mises en avant.
...
- La recherche favorise les correspondances exactes de jeton slug/nom avant la popularité des téléchargements. Un jeton slug autonome comme `map` correspond à `personal-map` plus fortement qu’à la sous-chaîne dans `amap`.
- Les téléchargements sont un faible a priori de popularité, pas une garantie de première position.
...
- Supprime du fichier de verrouillage l’épinglage d’un skill installé afin que les futures mises à jour puissent le modifier.
...
- Si l’empreinte ne correspond pas :
  - refuse par défaut
  - écrase avec `--force` (ou sur invite, si interactif)
- Les skills épinglés ne sont jamais mis à jour par `--force`.
- `update <slug>` échoue rapidement pour les slugs épinglés et vous demande d’exécuter d’abord `clawhub unpin <slug>`.
...
- Indicateurs :
  - `--package <name>` : nom du package dont résoudre les métadonnées d’artifact attendues.
...
- Restaure un paquet supprimé de manière réversible et ses versions.
- Nécessite le propriétaire du paquet, un propriétaire/administrateur d’éditeur d’organisation, un modérateur de plateforme
  ou un administrateur de plateforme.
...
- Les noms de paquets avec périmètre doivent être transférés vers le propriétaire de périmètre correspondant.
...
- Pour les sources GitHub, l’attribution de source est remplie automatiquement depuis le dépôt, le commit résolu, la référence et le sous-chemin.
...
- `--clawscan-note <text>` ajoute une note ClawScan. Cette note donne à ClawScan
  ...
  la version publiée.
- Les noms de paquets avec périmètre doivent correspondre au propriétaire sélectionné. Consultez `docs/publishing.md`.
- Les options existantes (`--family`, `--name`, `--version`, `--source-repo`, `--source-commit`, `--source-ref`, `--source-path`) continuent de fonctionner comme remplacements.
...
Configuration appelante typique :
...
- Les publications réelles doivent être limitées aux événements de confiance comme `workflow_dispatch` ou les pushes de balises.
- La publication de confiance sans secret ne fonctionne que sur `workflow_dispatch` ; les pushes de balises nécessitent toujours `clawhub_token`.
