...
- Par défaut : ouvre le navigateur sur `<site>/cli/auth` et se termine via un rappel local loopback.
...
- La recherche privilégie les correspondances exactes de jetons slug/nom avant la popularité des téléchargements. Un jeton slug autonome comme `map` correspond plus fortement à `personal-map` qu’à la sous-chaîne dans `amap`.
- Les téléchargements constituent un léger a priori de popularité, pas une garantie de meilleur classement.
- Si une compétence devrait apparaître mais n’apparaît pas, exécutez `clawhub inspect <slug>` en étant connecté afin de vérifier les diagnostics de modération visibles par le propriétaire avant de renommer les métadonnées.
...
- Récupère les métadonnées de compétence et les fichiers de version sans installer.
...
- Refuse d’écraser les compétences épinglées ; exécutez d’abord `clawhub unpin <slug>`.
- Écrit :
...
- Les skills épinglés sont ignorés par `update --all` et refusés par `update <slug>` direct.
...
- `update --all` ignore les slugs épinglés et imprime un résumé de ce qui est resté gelé.
...
- Le comportement relatif au propriétaire et à la revue est expliqué dans `docs/publishing.md`.
...
- Supprime temporairement un skill (propriétaire, modérateur ou administrateur).
...
- Utilisez ceci pour les plugins et les autres entrées de famille de packages ; `search` de premier niveau reste la surface de recherche de skills.
- Indicateurs :
...
- Récupère les métadonnées du package sans installer.
...
- `--tag <tag>` : inspecte une version balisée (par exemple `latest`).
...
- `--file <path>` : récupère le contenu brut du fichier (fichiers texte uniquement ; limite de 200 KB).
- `--json` : sortie lisible par machine.
...
- Indicateurs :
  ...
  - `--tag <tag>` : télécharge une version balisée (valeur par défaut : `latest`).
...
- Supprime de façon réversible un paquet et toutes ses versions.
- Nécessite le propriétaire du paquet, un propriétaire/admin de l’éditeur d’organisation, un modérateur de plateforme
...
- Restaure un paquet supprimé de façon réversible et ses versions.
- Nécessite le propriétaire du paquet, un propriétaire/admin de l’éditeur d’organisation, un modérateur de plateforme
  ou un admin de plateforme.
...
- Nécessite un accès admin à la fois au propriétaire actuel du paquet et à l’éditeur de destination,
  sauf si l’opération est effectuée par un admin de plateforme.
...
- Les signalements s’appliquent au niveau du paquet, peuvent éventuellement être liés à une version, et deviennent visibles
  aux modérateurs pour examen.
...
- Signale les blocages concernant le statut officiel, la disponibilité ClawPack, le condensat d’artefact,
  la provenance source, la compatibilité OpenClaw, les cibles d’hôte, les métadonnées d’environnement
  et l’état d’analyse.
- Options :
  - `--json` : sortie lisible par machine.
...
- Pour les sources GitHub, l’attribution de source est automatiquement renseignée à partir du dépôt, du commit résolu, de la référence et du sous-chemin.
...
- `--clawscan-note <text>` ajoute une note ClawScan. Cette note fournit à ClawScan
  du contexte pour les comportements qui pourraient autrement sembler inhabituels, comme l’accès réseau,
  l’accès à l’hôte natif ou des identifiants propres à un fournisseur. La note est stockée sur
  la version publiée.
- Les noms de paquets avec portée doivent correspondre au propriétaire sélectionné. Voir `docs/publishing.md`.
- Les options existantes (`--family`, `--name`, `--version`, `--source-repo`, `--source-commit`, `--source-ref`, `--source-path`) continuent de fonctionner comme substitutions.
...
Utilisez d’abord `--dry-run` afin de pouvoir confirmer les métadonnées de paquet résolues et
...
- `openclaw.compat.minGatewayVersion` et
  `openclaw.build.pluginSdkVersion` sont des extras facultatifs si vous souhaitez publier
...
- Épinglez le workflow réutilisable à un tag stable ou à un SHA de commit complet. N’exécutez pas de publication de version depuis `@main`.
...
- Analyse les dossiers Skills locaux et publie ceux qui sont nouveaux ou modifiés.
