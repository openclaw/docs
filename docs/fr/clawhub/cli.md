...
Le CLI respecte les variables d’environnement de proxy HTTP standard pour les systèmes derrière
...
Quand l’une de ces variables est définie, le CLI achemine les requêtes sortantes via
...
- Repli hérité : si `clawhub/config.json` n’existe pas encore mais que `clawdhub/config.json` existe, le CLI réutilise le chemin hérité
...
- Interactif distant/sans interface graphique : `clawhub login --device` affiche un code et attend que vous l’autorisiez sur `<site>/cli/device`.
...
- Les téléchargements constituent un faible a priori de popularité, pas une garantie de premier classement.
- Si une compétence devrait apparaître mais n’apparaît pas, exécutez `clawhub inspect <slug>` une fois connecté pour vérifier les diagnostics de modération visibles par le propriétaire avant de renommer les métadonnées.
...
- Si l’empreinte ne correspond pas :
  - refuse par défaut
  - remplace avec `--force` (ou invite, si interactif)
...
- Supprime un Skill de façon réversible (propriétaire, modérateur ou administrateur).
...
- Fusionne un Skill détenu dans un autre Skill détenu.
...
- Supprime temporairement un package et toutes ses releases.
- Requiert le propriétaire du package, un propriétaire/administrateur d’éditeur d’organisation, un modérateur de plateforme
...
- Restaure un package et ses releases supprimés temporairement.
- Requiert le propriétaire du package, un propriétaire/administrateur d’éditeur d’organisation, un modérateur de plateforme
  ou un administrateur de plateforme.
...
- Requiert un accès administrateur au propriétaire actuel du package et à l’éditeur
...
- Les signalements s’appliquent au niveau du package, peuvent être liés facultativement à une version et deviennent visibles
...
- Affiche l’état actuel d’analyse du package, le nombre de signalements ouverts, l’état de modération manuelle de la dernière release,
  l’état de blocage du téléchargement et les motifs de modération.
- Options :
  - `--json` : sortie lisible par machine.
...
- Signale les blocages concernant le statut officiel, la disponibilité de ClawPack, le condensat de l’artefact,
...
- Les dossiers de Plugins de code sont empaquetés dans une tarball npm ClawPack avant le téléversement afin que
...
- Les Plugins de code externes doivent déclarer explicitement `openclaw.compat.pluginApi` et
  ...
  Le `package.json.version` de niveau supérieur n’est pas utilisé comme valeur de repli pour la validation de publication.
...
- `--clawscan-note <text>` ajoute une note ClawScan. Cette note fournit à ClawScan
  le contexte d’un comportement qui pourrait autrement paraître inhabituel, comme l’accès réseau,
  l’accès à l’hôte natif ou des identifiants propres à un fournisseur. La note est stockée sur
  la release publiée.
...
- Les dépôts GitHub privés requièrent `GITHUB_TOKEN`.
...
l’attribution de source avant de créer une release active :
...
- `package.json.version` est la version de release de votre package, mais elle n’est pas utilisée comme
  valeur de repli pour la validation de compatibilité/build OpenClaw.
- `openclaw.hostTargets` et `openclaw.environment` sont des métadonnées facultatives.
  ClawHub peut les exposer lorsqu’elles sont présentes, mais elles ne sont pas requises pour la publication.
...
ClawHub fournit également un workflow réutilisable officiel à l’emplacement
...
- Envoyée pendant `sync` lorsque l’utilisateur est connecté, sauf si `CLAWHUB_DISABLE_TELEMETRY=1` (héritage `CLAWDHUB_DISABLE_TELEMETRY=1`).
