...
<Steps>
  <Step title="Créer une application et un bot Discord">
    Dans le [Discord Developer Portal](https://discord.com/developers/applications), cliquez sur **New Application** et donnez-lui un nom (par exemple « OpenClaw »).
  ...
  <Step title="Activer les intentions privilégiées">
    ...
    - **Server Members Intent** (recommandé ; obligatoire pour les listes d’autorisation de rôles, la correspondance entre noms et ID et les groupes d’accès à l’audience des canaux)
  ...
  <Step title="Générer une URL d’invitation et ajouter le bot à votre serveur">
    ...
    **Text Permissions**
    ...
    Il s’agit de la configuration de base pour les canaux textuels normaux. Si le bot doit publier dans des fils de discussion — y compris dans des flux de canaux de forum ou multimédias qui créent ou poursuivent un fil — activez également **Send Messages in Threads**.
  ...
  <Step title="Activer le mode développeur et récupérer vos ID">
    Dans l’application Discord, activez le mode développeur afin de pouvoir copier les ID :
    ...
    Conservez l’ID du serveur et l’ID utilisateur avec le jeton de votre bot ; vous aurez besoin des trois à l’étape suivante.
  ...
  <Step title="Définir le jeton de votre bot de manière sécurisée (ne l’envoyez pas dans une conversation)">
...
```
    ...
    Si OpenClaw s’exécute déjà en tant que service d’arrière-plan, redémarrez-le à l’aide de l’application OpenClaw pour Mac ou en arrêtant puis en relançant le processus `openclaw gateway run`.
    Pour les installations sous forme de service géré, exécutez `openclaw gateway install` depuis un shell dans lequel `DISCORD_BOT_TOKEN` est défini, ou stockez la variable dans `~/.openclaw/.env` afin que le service puisse résoudre la SecretRef d’environnement après le redémarrage.
    Si votre hôte est bloqué ou limité par le débit lors de la recherche de l’application au démarrage par Discord, définissez l’ID d’application/client à partir du Developer Portal afin que le démarrage puisse ignorer cet appel REST : `channels.discord.applicationId` pour le compte par défaut, ou `channels.discord.accounts.<accountId>.applicationId` pour chaque bot.
  ...
  <Step title="Configurer OpenClaw et effectuer l’appairage">
    ...
    <Tabs>
      <Tab title="Demander à votre agent">
        Discutez avec votre agent OpenClaw sur un canal existant (par exemple Telegram) et donnez-lui les instructions suivantes. Si Discord est votre premier canal, utilisez plutôt l’onglet CLI / configuration.
...
```
        ...
        Pour une configuration scriptée ou à distance, écrivez le même bloc JSON5 avec `openclaw config patch --file ./discord.patch.json5 --dry-run`, puis réexécutez la commande sans `--dry-run`. Les chaînes `token` en texte brut fonctionnent également, et les valeurs SecretRef sont prises en charge pour `channels.discord.token` avec les fournisseurs env/file/exec. Consultez [Gestion des secrets](/fr/gateway/secrets).

        Pour plusieurs bots Discord, conservez le jeton et l’ID d’application de chaque bot dans son compte. Une valeur `channels.discord.applicationId` de niveau supérieur est héritée par les comptes ; ne la définissez donc à cet endroit que si tous les comptes utilisent le même ID d’application.
...
<Note>
La résolution des jetons tient compte des comptes. Les valeurs de jeton de la configuration prévalent sur la variable d’environnement de secours, et `DISCORD_BOT_TOKEN` n’est utilisé que pour le compte par défaut.
Si deux comptes Discord activés correspondent au même jeton de bot, OpenClaw ne démarre qu’un seul moniteur Gateway pour ce jeton : un jeton provenant de la configuration prévaut sur la variable d’environnement de secours ; sinon, le premier compte activé prévaut et le compte en double est signalé comme désactivé avec le motif `duplicate bot token`.
Pour les appels sortants avancés (outil de messagerie/actions de canal), un `token` explicite propre à chaque appel est utilisé pour cet appel. Cela s’applique aux actions d’envoi ainsi qu’aux actions de type lecture/sonde (lecture/recherche/récupération/fil de discussion/épingles/autorisations). Les paramètres de stratégie et de nouvelle tentative du compte proviennent toujours du compte sélectionné dans l’instantané d’exécution actif.
</Note>

## Recommandation : configurer un espace de travail de serveur

Une fois les messages privés opérationnels, vous pouvez transformer votre serveur en espace de travail complet dans lequel chaque canal dispose de sa propre session d’agent et de son propre contexte. Cette configuration est recommandée pour les serveurs privés où il n’y a que vous et votre bot.
...
<Steps>
  <Step title="Ajouter votre serveur à la liste d’autorisation des serveurs">
    ...
    <Tabs>
      <Tab title="Demander à votre agent">
        > « Ajoutez l’ID de mon serveur Discord `<server_id>` à la liste d’autorisation des serveurs »
      </Tab>
...
```
  ...
  <Step title="Autoriser les réponses sans @mention">
    ...
    Dans les canaux du serveur, les réponses normales sont publiées automatiquement par défaut. Pour les salons partagés actifs en permanence, activez `messages.groupChat.visibleReplies: "message_tool"` afin que l’agent puisse rester en observation et ne publier que lorsqu’il estime qu’une réponse dans le canal est utile. Cela fonctionne mieux avec les modèles de dernière génération fiables dans l’utilisation des outils, tels que GPT-5.6 Sol. Les événements ambiants du salon restent silencieux tant que l’outil n’effectue aucun envoi. Consultez [Événements ambiants des salons](/fr/channels/ambient-room-events) pour obtenir la configuration complète du mode d’observation.

    Si Discord affiche l’indicateur de saisie et que les journaux indiquent une utilisation de jetons, mais qu’aucun message n’est publié, vérifiez si le tour a été configuré comme événement ambiant de salon ou s’il utilise les réponses visibles via l’outil de messagerie.

    <Tabs>
      <Tab title="Demander à votre agent">
        > « Autorisez mon agent à répondre sur ce serveur sans devoir être mentionné avec @ »
      </Tab>
      <Tab title="Configuration">
...
```
        ...
        Pour imposer les envois via l’outil de messagerie pour les réponses visibles dans les groupes/canaux, définissez `messages.groupChat.visibleReplies: "message_tool"`.
  ...
  <Step title="Planifier l’utilisation de la mémoire dans les canaux du serveur">
    La mémoire à long terme (MEMORY.md) n’est chargée automatiquement que dans les sessions de messages privés ; les canaux du serveur ne la chargent pas.

    <Tabs>
      <Tab title="Demander à votre agent">
        > « Lorsque je pose des questions dans les canaux Discord, utilisez memory_search ou memory_get si vous avez besoin du contexte à long terme de MEMORY.md. »
      </Tab>
      <Tab title="Manuel">
        Pour partager un contexte dans chaque canal, placez les instructions stables dans `AGENTS.md` ou `USER.md` (injectés dans chaque session). Conservez les notes à long terme dans `MEMORY.md` et consultez-les à la demande à l’aide des outils de mémoire.
      </Tab>
    </Tabs>
...
- Les commandes slash natives s’exécutent dans des sessions de commande isolées (`agent:<agentId>:discord:slash:<userId>`), tout en transmettant `CommandTargetSessionKey` à la session de conversation acheminée.
- La remise à Discord des annonces Cron/Heartbeat uniquement textuelles est réduite à la réponse finale visible de l’assistant, envoyée une seule fois. Les contenus multimédias et les charges utiles de composants structurés restent répartis sur plusieurs messages lorsque l’agent émet plusieurs charges utiles à remettre.

## Canaux de forum

Les canaux de forum et multimédias de Discord n’acceptent que les publications dans des fils de discussion. OpenClaw permet de les créer de deux façons :

- Envoyez un message au forum parent (`channel:<forumId>`) pour créer automatiquement un fil. Le titre du fil correspond à la première ligne non vide du message (tronquée à la limite de 100 caractères imposée par Discord aux noms de fils).
...
Envoyez un message au forum parent pour créer un fil :
...
Pour limiter les utilisateurs autorisés à cliquer sur un bouton, définissez `allowedUsers` sur ce bouton (identifiants utilisateur Discord, étiquettes ou `*`). Les utilisateurs non correspondants reçoivent un refus éphémère.

Les rappels de composants expirent par défaut après 30 minutes. Définissez `channels.discord.agentComponents.ttlMs` pour modifier la durée de vie du registre de rappels du compte par défaut, ou `channels.discord.accounts.<accountId>.agentComponents.ttlMs` pour chaque compte. La valeur est exprimée en millisecondes, doit être un entier positif et est plafonnée à `86400000` (24 heures). Des TTL plus longs conviennent aux processus de révision ou d’approbation dont les boutons doivent rester utilisables, mais ils prolongent la période pendant laquelle un ancien message Discord peut encore déclencher une action. Préférez le TTL le plus court adapté à votre besoin et conservez la valeur par défaut si des rappels obsolètes seraient inattendus.

Les commandes slash `/model` et `/models` ouvrent un sélecteur de modèle interactif comportant des listes déroulantes pour le fournisseur, le modèle et les environnements d’exécution compatibles, ainsi qu’une étape Submit. `/models add` est obsolète et renvoie un message d’obsolescence au lieu d’enregistrer des modèles depuis la conversation. La réponse du sélecteur est éphémère et utilisable uniquement par l’utilisateur qui l’a invoqué. Les menus de sélection Discord sont limités à 25 options ; ajoutez donc des entrées `provider/*` à `agents.defaults.models` lorsque vous souhaitez que le sélecteur affiche des modèles découverts dynamiquement uniquement pour des fournisseurs sélectionnés tels que `openai` ou `vllm`.
...
<Tabs>
  <Tab title="Politique des messages privés">
    ...
    Si la politique des messages privés n’est pas ouverte, les utilisateurs inconnus sont bloqués (ou invités à procéder à un appairage en mode `pairing`).
    ...
    - Pour un compte, `allowFrom` prévaut sur l’ancien paramètre `dm.allowFrom`.
    - Les comptes nommés héritent de `channels.discord.allowFrom` lorsque leurs propres paramètres `allowFrom` et l’ancien `dm.allowFrom` ne sont pas définis.
    ...
    Format de la cible de message privé pour la distribution :

    - `user:<id>`
    - mention `<@id>`

    Les identifiants numériques seuls sont normalement interprétés comme des identifiants de canal lorsqu’un canal par défaut est actif, mais les identifiants figurant dans la liste `allowFrom` effective des messages privés du compte sont traités comme des cibles utilisateur de message privé à des fins de compatibilité.
  ...
  <Tab title="Groupes d’accès">
    ...
    Les noms de groupes d’accès sont partagés entre les canaux de messagerie. Utilisez `type: "message.senders"` pour un groupe statique dont les membres sont exprimés avec la syntaxe `allowFrom` habituelle de chaque canal, ou `type: "discord.channelAudience"` lorsque l’audience `ViewChannel` actuelle d’un canal Discord doit définir dynamiquement l’appartenance. Comportement partagé des groupes d’accès : [Groupes d’accès](/fr/channels/access-groups).
...
```
    ...
    Exemple : autorisez toute personne pouvant voir `#maintainers` à envoyer un message privé au bot, tout en maintenant les messages privés fermés à tous les autres utilisateurs.
...
```
    ...
    Les recherches échouent en mode fermé. Si Discord renvoie `Missing Access`, si la recherche du membre échoue ou si le canal appartient à un autre serveur, l’expéditeur du message privé est considéré comme non autorisé.

    Activez **Server Members Intent** dans le portail des développeurs Discord lorsque vous utilisez des groupes d’accès fondés sur l’audience d’un canal. Les messages privés n’incluent pas l’état des membres du serveur ; OpenClaw résout donc le membre via l’API REST de Discord au moment de l’autorisation.
  ...
  <Tab title="Politique des serveurs">
    ...
    La configuration de sécurité de référence lorsque `channels.discord` existe est `allowlist`.

    Comportement de `allowlist` :

    - le serveur doit correspondre à `channels.discord.guilds` (`id` recommandé, slug accepté)
    - listes d’autorisation facultatives des expéditeurs : `users` (identifiants stables recommandés) et `roles` (identifiants de rôle uniquement) ; si l’une ou l’autre est configurée, les expéditeurs sont autorisés lorsqu’ils correspondent à `users` OU à `roles`
    - la correspondance directe par nom ou étiquette est désactivée par défaut ; activez `channels.discord.dangerouslyAllowNameMatching: true` uniquement comme mode de compatibilité de dernier recours
    - les noms et étiquettes sont pris en charge pour `users`, mais les identifiants sont plus sûrs ; `openclaw security audit` émet un avertissement lorsque des entrées utilisant un nom ou une étiquette sont employées
    - si des `channels` sont configurés pour un serveur, les canaux non répertoriés sont refusés
    - si un serveur ne possède aucun bloc `channels`, tous les canaux de ce serveur autorisé sont permis
...
```
  ...
  <Tab title="Mentions et messages privés de groupe">
    ...
    `requireMention` est configuré pour chaque serveur ou canal (`channels.discord.guilds...`).
    `ignoreOtherMentions` permet d’ignorer les messages qui mentionnent un autre utilisateur ou rôle, mais pas le bot (à l’exclusion de @everyone/@here).
...
## Commandes natives et autorisation des commandes
  ...
  <Accordion title="Balises de réponse et réponses natives">
    ...
    - `batched` : l’associe uniquement lorsque l’événement entrant était un lot temporisé de plusieurs messages — utile lorsque vous souhaitez principalement des réponses natives pour les conversations ambiguës comportant des rafales de messages, plutôt que pour chaque tour à message unique

    Les identifiants de message sont exposés dans le contexte et l’historique afin que les agents puissent cibler des messages précis.
...
```
    ...
    - `progress` conserve un seul brouillon d’état modifiable et l’actualise avec la progression des outils jusqu’à la livraison finale ; le libellé initial partagé est une ligne défilante, qui disparaît donc comme les autres dès que suffisamment de tâches s’affichent.
    ...
    - Les lignes des outils et de progression s’affichent sous une forme compacte avec un émoji, un titre et des détails lorsqu’ils sont disponibles, par exemple `🛠️ Bash: run tests` ou `🔎 Web Search: for "query"`.
    - `streaming.progress.commentary` (`false` par défaut) permet d’inclure le texte de commentaire ou de préambule de l’assistant dans le brouillon temporaire de progression. Le commentaire est nettoyé avant l’affichage, reste transitoire et ne modifie pas la livraison de la réponse finale.
    - `streaming.progress.maxLineChars` détermine la longueur maximale de chaque ligne de l’aperçu de progression. Le texte est raccourci aux limites des mots ; les détails des commandes et des chemins conservent les suffixes utiles.
    - `streaming.preview.commandText` / `streaming.progress.commandText` détermine l’affichage des détails des commandes et de leur exécution dans les lignes de progression compactes : `raw` (par défaut) ou `status` (libellé de l’outil uniquement).
    ...
    L’aperçu en streaming prend uniquement en charge le texte ; les réponses contenant des médias utilisent la diffusion normale.

  </Accordion>

  <Accordion title="Historique, contexte et comportement des fils">
    Contexte de l’historique des serveurs :
    ...
    - Les fils Discord sont acheminés comme des sessions de canal et héritent de la configuration du canal parent, sauf remplacement explicite.
    - Les sessions de fil héritent de la sélection `/model` du canal parent au niveau de la session uniquement comme modèle de repli ; les sélections `/model` propres au fil sont prioritaires, et l’historique de transcription du parent n’est pas copié, sauf si l’héritage de transcription est activé.
    ...
    - `guilds.<guild>.channels.<channel>.requireMention: false` est préservé lors du mécanisme de repli d’activation à l’étape de réponse.

    Les sujets des canaux sont injectés en tant que contexte **non fiable**. Les listes d’autorisation déterminent qui peut déclencher l’agent ; elles ne constituent pas une limite complète de masquage du contexte supplémentaire.

  </Accordion>

  <Accordion title="Sessions liées aux fils pour les sous-agents">
    Discord peut lier un fil à une cible de session afin que les messages suivants de ce fil continuent d’être acheminés vers la même session, y compris les sessions de sous-agents.
    ...
    - `/session idle <duration|off>` consulte/modifie la suppression automatique de la liaison après une période d’inactivité pour les liaisons ciblées
    - `/session max-age <duration|off>` consulte/modifie l’âge maximal absolu des liaisons ciblées
...
```
    ...
    - `spawnSessions` contrôle la création et la liaison automatiques de fils pour `sessions_spawn({ thread: true })` et les créations de fils ACP. Valeur par défaut : `true`.
...
```
    ...
    - `spawnSessions` conditionne la création et la liaison des fils enfants via `--thread auto|here`.
  ...
  <Accordion title="Notifications de réactions">
    ...
    Les événements de réaction sont convertis en événements système et joints à la session Discord acheminée.
  ...
  <Accordion title="Réactions d’accusé de réception">
    ...
    **Portée (`messages.ackReactionScope`) :**
    ...
    Valeurs : `"all"` (messages privés + groupes, y compris les événements ambiants des salons), `"direct"` (messages privés uniquement), `"group-all"` (tous les messages de groupe sauf les événements ambiants des salons, aucun message privé), `"group-mentions"` (groupes lorsque le bot est mentionné ; **aucun message privé**, valeur par défaut), `"off"` / `"none"` (désactivé).

    <Note>
    La portée par défaut (`"group-mentions"`) ne déclenche pas de réaction d’accusé de réception dans les messages privés ni pour les événements ambiants des salons. Pour obtenir une réaction d’accusé de réception sur les messages privés Discord entrants et les événements des salons silencieux, définissez `messages.ackReactionScope` sur `"all"`.
    </Note>
...
```
  ...
  <Accordion title="Proxy du Gateway">
    Acheminez le trafic WebSocket du Gateway Discord et les requêtes REST de démarrage (ID d’application + résolution de la liste d’autorisation) par un proxy HTTP(S) avec `channels.discord.proxy`.
    Le proxy des WebSockets du Gateway Discord est explicite ; les connexions WebSocket n’héritent pas des variables d’environnement de proxy ambiantes du processus Gateway. Les requêtes REST de démarrage utilisent ce proxy lorsque `channels.discord.proxy` est configuré.
...
```
  ...
  <Accordion title="Prise en charge de PluralKit">
    Activez la résolution PluralKit pour associer les messages relayés à l’identité d’un membre du système :
...
```
  ...
  <Accordion title="Alias de mention sortante">
    Utilisez `mentionAliases` lorsque les agents ont besoin de mentions sortantes déterministes pour des utilisateurs Discord connus. Les clés sont des identifiants sans le préfixe `@` ; les valeurs sont des ID d’utilisateur Discord. Les identifiants inconnus, `@everyone`, `@here` et les mentions dans les fragments de code Markdown restent inchangés.
...
```
    ...
    - 2 : Écoute
    - 3 : Regarde
    - 4 : Personnalisée (utilise le texte de l’activité comme état du statut ; l’émoji est facultatif)
    - 5 : Participe à une compétition

    Présence automatique (signal d’état de santé de l’environnement d’exécution) :
...
```
  ...
  <Accordion title="Approbations dans Discord">
    ...
    Discord active automatiquement les approbations natives d’exécution lorsque `enabled` n’est pas défini ou vaut `"auto"` et qu’au moins un approbateur peut être déterminé, soit à partir de `execApprovals.approvers`, soit à partir de `commands.ownerAllowFrom`. Discord ne déduit pas les approbateurs d’exécution à partir de la valeur `allowFrom` du canal, de l’ancienne valeur `dm.allowFrom` ou de la valeur `defaultTo` des messages privés. Définissez explicitement `enabled: false` pour désactiver Discord comme client d’approbation natif.
    ...
    Lorsque `target` vaut `channel` ou `both`, la demande d’approbation est visible dans le canal. Seuls les approbateurs déterminés peuvent utiliser les boutons ; les autres utilisateurs reçoivent un refus éphémère. Les demandes d’approbation incluent le texte de la commande ; n’activez donc la diffusion dans le canal que pour des canaux de confiance. Si l’ID du canal ne peut pas être déduit de la clé de session, OpenClaw utilise les messages privés comme solution de repli.

    Discord affiche les boutons d’approbation partagés utilisés par les autres canaux de discussion ; l’adaptateur Discord natif ajoute principalement le routage des messages privés vers les approbateurs et la diffusion dans le canal. Lorsque ces boutons sont présents, ils constituent l’interface principale d’approbation ; OpenClaw ne doit inclure une commande manuelle `/approve` que lorsque le résultat de l’outil indique que les approbations dans la discussion sont indisponibles ou que l’approbation manuelle est la seule possibilité. Si l’environnement d’exécution d’approbation natif de Discord n’est pas actif, OpenClaw conserve l’invite déterministe locale `/approve <id> <decision>` visible. Si l’environnement d’exécution est actif, mais qu’aucune carte native ne peut être remise à une destination, OpenClaw envoie dans la même discussion un avis de repli contenant la commande `/approve` exacte de l’approbation en attente.
...
- `channels.discord.agentComponents.ttlMs` détermine combien de temps les rappels des composants Discord envoyés restent enregistrés (valeur par défaut `1800000`, maximum `86400000`). Par compte : `channels.discord.accounts.<id>.agentComponents.ttlMs`.
- Les `embeds` sont ignorés lorsque les composants v2 sont présents.
- Les aperçus d’URL simples sont masqués par défaut. Définissez `suppressEmbeds: false` dans une action de message lorsqu’un lien sortant unique doit être développé.
...
Discord dispose de deux surfaces vocales distinctes : les **canaux vocaux** en temps réel (conversations continues) et les **pièces jointes de message vocal** (format d’aperçu de la forme d’onde). Le Gateway prend en charge les deux.
...
- La voix Discord est facultative pour les configurations uniquement textuelles ; définissez `channels.discord.voice.enabled=true` (ou conservez un bloc `channels.discord.voice` existant) pour activer les commandes `/vc`, l’environnement d’exécution vocal et l’intention Gateway `GuildVoiceStates`. `channels.discord.intents.voiceStates` peut explicitement remplacer l’abonnement à l’intention ; laissez cette option non définie pour suivre l’activation effective de la voix.
- `voice.mode` contrôle le chemin de conversation. La valeur par défaut est `agent-proxy` : une interface vocale en temps réel gère le séquençage des tours, les interruptions et la lecture, délègue le travail de fond à l’agent OpenClaw acheminé via `openclaw_agent_consult` et traite le résultat comme une invite Discord saisie par cet interlocuteur. `stt-tts` conserve l’ancien flux par lots combinant STT et TTS. `bidi` permet au modèle en temps réel de converser directement tout en exposant `openclaw_agent_consult` pour le cerveau OpenClaw.
...
- `voice.followUsers` permet au bot de rejoindre, suivre et quitter des utilisateurs sélectionnés dans les canaux vocaux Discord. Consultez [Suivre des utilisateurs dans les canaux vocaux](#follow-users-in-voice).
- `agent-proxy` achemine la parole via `discord-voice`, qui conserve les autorisations normales de propriétaire et d’outils pour l’interlocuteur et la session cible, mais masque l’outil `tts` de l’agent, car la voix Discord prend en charge la lecture. Par défaut, `agent-proxy` accorde à la consultation un accès aux outils complet, équivalent à celui du propriétaire, pour les interlocuteurs propriétaires (`voice.realtime.toolPolicy: "owner"`) et privilégie fortement la consultation de l’agent OpenClaw avant toute réponse substantielle (`voice.realtime.consultPolicy: "always"`). Dans ce mode `always` par défaut, la couche en temps réel ne prononce pas automatiquement de paroles de remplissage avant la réponse de la consultation ; elle capture et transcrit la parole, puis énonce la réponse OpenClaw acheminée. Si plusieurs réponses de consultation forcée se terminent alors que Discord lit encore la première réponse, les réponses vocales exactes suivantes sont mises en file d’attente jusqu’à ce que la lecture soit inactive, au lieu de remplacer la parole au milieu d’une phrase.
...
- Dans le mode en temps réel OpenAI `agent-proxy`, définissez `voice.realtime.requireWakeName: true` pour que la voix Discord en temps réel reste silencieuse jusqu’à ce qu’une transcription commence ou se termine par un nom d’activation. Les noms d’activation configurés doivent comporter un ou deux mots. Si `voice.realtime.wakeNames` n’est pas défini, OpenClaw utilise le `name` de l’agent acheminé ainsi que `OpenClaw`, avec comme solution de repli l’identifiant de l’agent ainsi que `OpenClaw`. Le filtrage par nom d’activation désactive la réponse automatique du fournisseur en temps réel, achemine les tours acceptés via le chemin de consultation de l’agent OpenClaw et émet un bref accusé de réception vocal lorsqu’un nom d’activation placé au début est reconnu dans la transcription partielle avant l’arrivée de la transcription finale.
- Le fournisseur OpenAI en temps réel accepte les noms d’événements Realtime 2 actuels et les alias hérités compatibles avec Codex pour les événements de sortie audio et de transcription, afin que les instantanés compatibles du fournisseur puissent évoluer sans perdre l’audio de l’assistant.
- `voice.realtime.bargeIn` contrôle si les événements de début de parole Discord interrompent la lecture active en temps réel. Si cette option n’est pas définie, elle suit le paramètre d’interruption par audio d’entrée du fournisseur en temps réel.
- `voice.realtime.minBargeInAudioEndMs` contrôle la durée minimale de lecture de l’assistant avant qu’une interruption en temps réel OpenAI ne tronque l’audio. Valeur par défaut : `250`. Définissez `0` pour une interruption immédiate dans les pièces à faible écho, ou augmentez cette valeur pour les installations de haut-parleurs générant beaucoup d’écho.
- `voice.tts` remplace `messages.tts` uniquement pour la lecture vocale `stt-tts` ; les modes en temps réel utilisent plutôt `voice.realtime.speakerVoice`. Pour utiliser une voix OpenAI lors de la lecture sur Discord, définissez `voice.tts.provider: "openai"` et choisissez une voix de synthèse vocale sous `voice.tts.providers.openai.speakerVoice`. `cedar` est un bon choix à consonance masculine avec le modèle TTS OpenAI actuel.
...
- Les tours de transcription vocale déduisent le statut de propriétaire à partir de `allowFrom` (ou `dm.allowFrom`) de Discord pour les commandes et actions de canal réservées aux propriétaires. La visibilité des outils de l’agent suit la politique d’outils configurée pour la session acheminée.
- Si `voice.autoJoin` contient plusieurs entrées pour le même serveur, OpenClaw rejoint le dernier canal configuré pour ce serveur.
- `voice.allowedChannels` est une liste d’autorisation de résidence facultative. Laissez cette option non définie pour permettre à `/vc join` de rejoindre n’importe quel canal vocal Discord autorisé. Lorsqu’elle est définie, `/vc join`, la connexion automatique au démarrage et les déplacements de l’état vocal du bot sont limités aux entrées `{ guildId, channelId }` répertoriées. Définissez-la sur un tableau vide pour refuser toutes les connexions vocales Discord. Si Discord déplace le bot hors de la liste d’autorisation, OpenClaw quitte ce canal et rejoint la cible de connexion automatique configurée lorsqu’elle est disponible.
...
- OpenClaw utilise le codec `libopus-wasm` intégré pour la réception vocale Discord et la lecture en temps réel de données PCM brutes. Il fournit une version WebAssembly épinglée de libopus et ne nécessite aucun module complémentaire opus natif.
- `voice.connectTimeoutMs` contrôle l’attente initiale de l’état Ready de `@discordjs/voice` pour les tentatives `/vc join` et de connexion automatique. Valeur par défaut : `30000`.
...
- Dans les modes en temps réel, l’écho des haut-parleurs capté par un microphone ouvert peut être interprété comme une interruption et interrompre la lecture. Pour les salles Discord générant beaucoup d’écho, définissez `voice.realtime.providers.openai.interruptResponseOnInputAudio: false` pour empêcher OpenAI d’interrompre automatiquement la réponse lors de la réception d’audio. Ajoutez `voice.realtime.bargeIn: true` si vous souhaitez toujours que les événements de début de parole Discord interrompent la lecture active. Le pont OpenAI en temps réel ignore les troncatures de lecture plus courtes que `voice.realtime.minBargeInAudioEndMs`, considérées comme de l’écho ou du bruit probable, et les consigne comme ignorées au lieu d’effacer la lecture Discord.
...
- Lorsque ElevenLabs est le fournisseur TTS sélectionné, la lecture vocale Discord utilise la TTS en streaming et démarre à partir du flux de réponse du fournisseur. Les fournisseurs sans prise en charge du streaming utilisent en repli le chemin du fichier temporaire synthétisé.
- OpenClaw surveille les échecs de déchiffrement à la réception et récupère automatiquement en quittant puis en rejoignant le canal vocal après plusieurs échecs sur une courte période.
- Si, après une mise à jour, les journaux de réception affichent à plusieurs reprises `DecryptionFailed(UnencryptedWhenPassthroughDisabled)`, recueillez un rapport sur les dépendances et les journaux. La version intégrée de `@discordjs/voice` comprend le correctif de remplissage en amont issu de la PR discord.js #11449, qui a résolu le problème discord.js #11419.
- Les événements de réception `The operation was aborted` sont attendus lorsque OpenClaw finalise un segment d’interlocuteur capturé ; il s’agit de diagnostics détaillés, et non d’avertissements.
- Les journaux vocaux Discord détaillés incluent un aperçu borné sur une ligne de la transcription STT pour chaque segment d’interlocuteur accepté, afin que le débogage affiche à la fois le côté utilisateur et le côté réponse de l’agent sans déverser un volume illimité de texte de transcription.
- En mode `agent-proxy`, la solution de repli de consultation forcée ignore les fragments de transcription probablement incomplets, tels que le texte se terminant par `...` ou par un connecteur final comme « et », ainsi que les conclusions manifestement non exploitables comme « je reviens tout de suite » ou « au revoir ». Les journaux affichent `forced agent consult skipped reason=...` lorsque ce mécanisme empêche une réponse obsolète mise en file d’attente.
...
- `followUsers` accepte les identifiants utilisateur Discord bruts et les valeurs `discord:<id>`. OpenClaw normalise les deux formes avant de les comparer aux événements d’état vocal.
- `followUsersEnabled` vaut par défaut `true` lorsque `followUsers` est configuré. Définissez cette option sur `false` pour conserver la liste enregistrée tout en désactivant le suivi vocal automatique.
...
- Si plusieurs utilisateurs suivis se trouvent sur le même serveur et que l’utilisateur suivi actif part, OpenClaw rejoint le canal d’un autre utilisateur suivi avant de quitter le serveur. Si plusieurs utilisateurs suivis se déplacent simultanément, le dernier événement d’état vocal observé l’emporte.
- `allowedChannels` reste applicable. Un utilisateur suivi se trouvant dans un canal non autorisé est ignoré, et une session appartenant au suivi rejoint un autre utilisateur suivi ou se déconnecte.
- OpenClaw réconcilie les événements d’état vocal manqués au démarrage et à intervalles bornés. La réconciliation échantillonne les serveurs configurés et limite le nombre de recherches REST par exécution ; les très grandes listes `followUsers` peuvent donc nécessiter plusieurs intervalles pour converger.
...
- La récupération de la réception DAVE peut quitter puis rejoindre le même canal après plusieurs échecs de déchiffrement. Les sessions appartenant au suivi conservent cette propriété pendant ce processus de récupération ; une déconnexion ultérieure de l’utilisateur suivi entraîne donc toujours le départ du canal.

Choisissez parmi les modes de connexion :

- Utilisez `followUsers` pour les configurations personnelles ou d’exploitation dans lesquelles le bot doit être automatiquement présent dans le canal vocal lorsque vous l’êtes.
- Utilisez `autoJoin` pour les bots associés à un canal fixe qui doivent être présents même lorsqu’aucun utilisateur suivi ne se trouve dans un canal vocal.
- Utilisez `/vc join` pour les connexions ponctuelles ou les canaux dans lesquels une présence vocale automatique serait inattendue.

Codec vocal Discord :

- Les journaux de réception vocale affichent `discord voice: opus decoder: libopus-wasm`.
- La lecture en temps réel encode les données PCM stéréo brutes à 48 kHz en Opus avec le même paquet `libopus-wasm` intégré avant de transmettre les paquets à `@discordjs/voice`.
- La lecture depuis un fichier ou un flux de fournisseur transcode les données en PCM stéréo brutes à 48 kHz avec ffmpeg, puis utilise `libopus-wasm` pour le flux de paquets Opus envoyé à Discord.

Pipeline STT et TTS :

- La capture PCM de Discord est convertie en fichier WAV temporaire.
- `tools.media.audio` gère la reconnaissance vocale, par exemple `openai/gpt-4o-mini-transcribe`.
- La transcription est transmise via l’entrée et le routage Discord tandis que le LLM de réponse s’exécute avec une politique de sortie vocale qui masque l’outil `tts` de l’agent et demande le renvoi de texte, car le canal vocal Discord assure la lecture TTS finale.
- Lorsqu’il est défini, `voice.model` remplace uniquement le LLM de réponse pour cette interaction sur le canal vocal.
...
Exemple de session de canal vocal par défaut avec agent mandataire :
...
Sans bloc `voice.agentSession`, chaque canal vocal dispose de sa propre session OpenClaw routée. Par exemple, `/vc join channel:234567890123456789` communique avec la session de ce canal vocal Discord. Le modèle en temps réel sert uniquement d’interface vocale ; les demandes substantielles sont transmises à l’agent OpenClaw configuré. Si le modèle en temps réel produit une transcription finale sans appeler l’outil de consultation, OpenClaw force la consultation comme solution de secours afin que le comportement par défaut reste équivalent à une conversation avec l’agent.

Exemple de reconnaissance vocale héritée suivie de synthèse vocale :
...
En mode `agent-proxy`, le bot rejoint le canal vocal configuré, mais les interactions de l’agent OpenClaw utilisent la session routée et l’agent habituels du canal cible. La session vocale en temps réel restitue oralement le résultat renvoyé dans le canal vocal. L’agent superviseur peut toujours utiliser les outils de messagerie habituels conformément à sa politique d’outils, notamment envoyer un message Discord distinct si cette action est appropriée.

Lorsqu’une exécution OpenClaw déléguée est active, les nouvelles transcriptions vocales Discord sont traitées comme des commandes en direct de l’exécution avant le démarrage d’une autre interaction de l’agent. Des phrases comme « état », « annule cela », « utilise la correction plus petite » ou « lorsque vous aurez terminé, vérifiez aussi les tests » sont classées comme une demande d’état, une annulation, une réorientation ou une demande de suivi pour la session active. Les résultats des demandes d’état, d’annulation, de réorientation acceptée et de suivi sont restitués oralement dans le canal vocal afin que l’appelant sache si OpenClaw a traité la demande.
...
Exemple OpenAI Realtime en environnement à fort écho :
...
Utilisez cette configuration lorsque le modèle entend sa propre lecture Discord via un microphone ouvert, mais que vous souhaitez tout de même pouvoir l’interrompre en parlant. OpenClaw empêche OpenAI d’interrompre automatiquement la réponse à la réception de données audio brutes, tandis que `bargeIn: true` permet aux événements de début de prise de parole Discord et aux données audio d’un locuteur déjà actif d’annuler les réponses actives en temps réel avant que le prochain tour capturé n’atteigne OpenAI. Les signaux d’interruption très précoces dont la valeur `audioEndMs` est inférieure à `minBargeInAudioEndMs` sont considérés comme probablement dus à un écho ou à du bruit et sont ignorés afin que le modèle ne s’interrompe pas dès la première trame de lecture.
...
- Pour les données audio du locuteur : `discord voice: realtime speaker turn opened ...`, `discord voice: realtime input audio started ... outputAudioMs=... outputActive=...` et `discord voice: realtime speaker turn closed ... chunks=... discordBytes=... realtimeBytes=... interruptedPlayback=...`
...
- Lors de la mise en file d’attente d’une parole exacte : `discord voice: realtime exact speech queued ... queued=... outputAudioMs=... outputActive=...`, suivi de `discord voice: realtime exact speech dequeued reason=player-idle ...`
...
- Lorsqu’un écho ou du bruit est ignoré : `discord voice: realtime model interrupt ignored client:conversation.item.truncate.skipped reason=barge-in audioEndMs=0 minAudioEndMs=250`
...
Pour diagnostiquer les coupures audio, lisez les journaux vocaux en temps réel comme une chronologie :

1. `realtime audio playback started` signifie que Discord a commencé à lire les données audio de l’assistant. À partir de ce moment, le pont commence à compter les segments de sortie audio de l’assistant, les octets PCM Discord, les octets en temps réel du fournisseur et la durée audio synthétisée.
2. `realtime speaker turn opened` indique qu’un locuteur Discord devient actif. Si la lecture est déjà active et que `bargeIn` est activé, ce message peut être suivi de `barge-in detected source=speaker-start`.
3. `realtime input audio started` indique la première trame audio réellement reçue pour ce tour de parole. La présence de `outputActive=true` ou d’une valeur `outputAudioMs` non nulle signifie que le microphone envoie des données d’entrée alors que la lecture de l’assistant est toujours active.
4. `barge-in detected source=active-speaker-audio` signifie qu’OpenClaw a détecté les données audio en direct d’un locuteur pendant que la lecture de l’assistant était active. Cela permet de distinguer une véritable interruption d’un événement de début de prise de parole Discord sans données audio utiles.
5. `barge-in requested reason=...` signifie qu’OpenClaw a demandé au fournisseur en temps réel d’annuler ou de tronquer la réponse active. Ce message inclut `outputAudioMs`, `outputActive` et `playbackChunks` afin que vous puissiez voir la quantité de données audio de l’assistant réellement lue avant l’interruption.
...
7. `realtime speaker turn closed` résume le tour d’entrée capturé. `chunks=0` ou `hasAudio=false` signifie que le tour de parole s’est ouvert, mais qu’aucune donnée audio exploitable n’a atteint le pont en temps réel. `interruptedPlayback=true` signifie que ce tour d’entrée a chevauché la sortie de l’assistant et déclenché la logique d’interruption.

Champs utiles :

- `outputAudioMs` : durée des données audio de l’assistant générées par le fournisseur en temps réel avant cette ligne de journal.
- `audioMs` : durée des données audio de l’assistant comptabilisée par OpenClaw avant l’arrêt de la lecture.
- `elapsedMs` : temps réel écoulé entre l’ouverture et la fermeture du flux de lecture ou du tour de parole.
...
- `sinceLastAudioMs` : intervalle entre la dernière trame audio capturée du locuteur et la fermeture du tour de parole.

Schémas courants :

- Une coupure immédiate avec `source=active-speaker-audio`, une faible valeur `outputAudioMs` et le même utilisateur à proximité indique généralement que l’écho du haut-parleur entre dans le microphone. Augmentez `voice.realtime.minBargeInAudioEndMs`, réduisez le volume du haut-parleur, utilisez un casque ou définissez `voice.realtime.providers.openai.interruptResponseOnInputAudio: false`.
- `source=speaker-start` suivi de `speaker turn closed ... hasAudio=false` signifie que Discord a signalé le début d’une prise de parole, mais qu’aucune donnée audio n’a atteint OpenClaw. Il peut s’agir d’un événement vocal Discord transitoire, du comportement de la porte de bruit ou de l’activation très brève du microphone par un client.
- `audio playback stopped reason=stream-close` sans interruption proche ni `provider-clear-audio` signifie que le flux de lecture Discord local s’est terminé de manière inattendue. Consultez les journaux précédents du fournisseur et du lecteur Discord.
- `capture ignored during playback (barge-in disabled)` signifie qu’OpenClaw a intentionnellement ignoré l’entrée pendant que les données audio de l’assistant étaient actives. Activez `voice.realtime.bargeIn` si vous souhaitez que la parole interrompe la lecture.
- `barge-in ignored ... outputActive=false` signifie que Discord ou la détection d’activité vocale du fournisseur a signalé de la parole, mais qu’OpenClaw n’avait aucune lecture active à interrompre. Cela ne devrait pas couper les données audio.

Les identifiants sont résolus séparément pour chaque composant : authentification de la route du LLM pour `voice.model`, authentification de la reconnaissance vocale pour `tools.media.audio`, authentification de la synthèse vocale pour `messages.tts`/`voice.tts`, et authentification du fournisseur en temps réel pour `voice.realtime.providers` ou la configuration d’authentification habituelle du fournisseur.

### Messages vocaux

Les messages vocaux Discord affichent un aperçu de la forme d’onde et nécessitent des données audio OGG/Opus. OpenClaw génère automatiquement la forme d’onde, mais nécessite `ffmpeg` et `ffprobe` sur l’hôte du Gateway pour inspecter et convertir les données.

- Fournissez un **chemin de fichier local** (les URL sont refusées).
- Omettez le contenu textuel (Discord refuse le texte et un message vocal dans la même charge utile).
...
<AccordionGroup>
  ...
  <Accordion title="Messages de serveur bloqués de manière inattendue">
    ...
    - si une map `channels` existe pour un serveur, seuls les canaux répertoriés sont autorisés
...
```
  ...
  <Accordion title="Tours Discord de longue durée ou réponses en double">
    ...
    Discord n’applique aucun délai d’expiration propre au canal aux tours d’agent mis en file d’attente. Les écouteurs de messages transmettent immédiatement le traitement, et les exécutions Discord mises en file d’attente conservent l’ordre propre à chaque session jusqu’à ce que le cycle de vie de la session, de l’outil ou de l’environnement d’exécution se termine ou annule le travail.
...
```
  ...
  <Accordion title="Avertissements de délai d’expiration lors de la récupération des métadonnées du Gateway">
    OpenClaw récupère les métadonnées Discord `/gateway/bot` avant la connexion. En cas d’échecs temporaires, il utilise l’URL du Gateway Discord par défaut et limite la fréquence des messages dans les journaux.
  ...
  <Accordion title="Redémarrages dus au délai d’expiration de READY du Gateway">
    OpenClaw attend l’événement `READY` du Gateway Discord au démarrage et après les reconnexions de l’environnement d’exécution. Les configurations à plusieurs comptes avec un démarrage échelonné peuvent nécessiter une fenêtre READY de démarrage plus longue que celle par défaut.
    ...
    - variable d’environnement de secours de l’environnement d’exécution lorsque la configuration n’est pas définie : `OPENCLAW_DISCORD_RUNTIME_READY_TIMEOUT_MS`
    - valeur par défaut de l’environnement d’exécution : `30000` (30 secondes), maximum : `120000`

  </Accordion>

  <Accordion title="Incohérences dans l’audit des autorisations">
    Les vérifications des autorisations de `channels status --probe` fonctionnent uniquement avec les identifiants numériques de canal.

    Si vous utilisez des clés de slug, la mise en correspondance à l’exécution peut toujours fonctionner, mais la sonde ne peut pas vérifier entièrement les autorisations.
  ...
  <Accordion title="Boucles entre bots">
    ...
    OpenClaw fournit également une [protection partagée contre les boucles de bots](/fr/channels/bot-loop-protection). Lorsque `allowBots` permet aux messages créés par des bots d’atteindre le répartiteur, Discord convertit l’événement entrant en faits `(account, channel, bot pair)`, et le garde-fou générique de paire bloque celle-ci dès qu’elle dépasse le budget d’événements configuré. Ce garde-fou empêche les boucles incontrôlées entre deux bots, qui devaient auparavant être interrompues par les limites de débit de Discord ; il n’affecte ni les déploiements à bot unique ni les réponses ponctuelles de bots qui restent sous le budget.
    ...
    - `cooldownSeconds: 60` -- une fois le budget dépassé, chaque message supplémentaire entre bots, dans un sens comme dans l’autre, est rejeté pendant une minute
...
```json5
{
  channels: {
    ...
    discord: {
      // Remplacement facultatif à l’échelle de Discord. Les blocs de comptes remplacent les champs
      // individuels et héritent d’ici les champs omis.
      botLoopProtection: {
        maxEventsPerWindow: 4,
      },
      accounts: {
        alpha: {
          // Alpha n’écoute les autres bots que lorsqu’ils le mentionnent.
          allowBots: "mentions",
        },
        bravo: {
          ...
          botLoopProtection: {
            // Autorise jusqu’à cinq messages par minute avant de mettre la paire en sourdine.
...
```
  ...
  <Accordion title="Échecs de STT vocal avec DecryptionFailed(...)">
    ...
    - surveillez les journaux à la recherche de :
...
<Accordion title="Champs Discord à forte valeur diagnostique">
...
- diffusion : `textChunkLimit` (valeur par défaut `2000`), `maxLinesPerMessage` (valeur par défaut `17`)
- diffusion en continu : `streaming.mode`, `streaming.chunkMode`, `streaming.preview.*`, `streaming.progress.*`, `streaming.block.*` (les anciennes clés à plat `streamMode`, `draftChunk`, `blockStreaming`, `blockStreamingCoalesce`, `chunkMode` sont migrées vers `streaming.*` par `openclaw doctor --fix`)
