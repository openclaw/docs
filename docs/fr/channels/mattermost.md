...
title: Mattermost
x-i18n:
    generated_at: "2026-04-23T06:58:44Z"
    model: gpt-5.4
    provider: openai
...
Mattermost est fourni comme plugin intégré dans les versions actuelles d’OpenClaw, donc les
builds empaquetés normaux n’ont pas besoin d’une installation séparée.

Si vous utilisez un build plus ancien ou une installation personnalisée qui exclut Mattermost,
...
Copie locale du dépôt (lors d’une exécution depuis un dépôt git) :
...
4. Configurez OpenClaw et démarrez la Gateway.
...
l’API Mattermost et reçoit les POST de rappel sur le serveur HTTP de la Gateway.
  ...
  channels: {
    mattermost: {
      commands: {
        ...
        // À utiliser lorsque Mattermost ne peut pas atteindre directement la Gateway (proxy inverse/URL publique).
...
- Pour les configurations multi-comptes, `commands` peut être défini au niveau supérieur ou sous
  `channels.mattermost.accounts.<id>.commands` (les valeurs du compte remplacent les champs du niveau supérieur).
...
- Les rappels slash échouent en mode fermé lorsque l’enregistrement a échoué, que le démarrage a été partiel, ou
  que le jeton de rappel ne correspond à aucune des commandes enregistrées.
- Exigence d’accessibilité : le point de terminaison de rappel doit être accessible depuis le serveur Mattermost.
  ...
  - Ne définissez pas `callbackUrl` sur votre URL de base Mattermost sauf si cette URL proxyfie en inverse `/api/channels/mattermost/command` vers OpenClaw.
  - Une vérification rapide consiste à exécuter `curl https://<gateway-host>/api/channels/mattermost/command` ; un GET doit renvoyer `405 Method Not Allowed` depuis OpenClaw, et non `404`.
- Exigence de liste d’autorisation de sortie Mattermost :
  - Si votre rappel cible des adresses privées/tailnet/internes, définissez
    `ServiceSettings.AllowedUntrustedInternalConnections` de Mattermost pour inclure l’hôte/domaine du rappel.
...
- `channels.mattermost.requireMention` est pris en charge pour les configurations héritées, mais `chatmode` est préférable.

## Fils de discussion et sessions

Utilisez `channels.mattermost.replyToMode` pour contrôler si les réponses dans les canaux et groupes restent dans le
canal principal ou démarrent un fil sous le message déclencheur.

- `off` (par défaut) : répondre dans un fil uniquement si le message entrant s’y trouve déjà.
- `first` : pour les messages de canal/groupe de premier niveau, démarrer un fil sous ce message et acheminer la
  conversation vers une session limitée à ce fil.
- `all` : même comportement que `first` pour Mattermost aujourd’hui.
- Les messages privés ignorent ce réglage et restent sans fil.
...
- `first` et `all` sont actuellement équivalents, car une fois que Mattermost possède une racine de fil,
  les segments de suivi et les médias continuent dans ce même fil.
...
- Remarque d’exécution : si `channels.mattermost` est entièrement absent, l’exécution revient à `groupPolicy="allowlist"` pour les vérifications de groupe (même si `channels.defaults.groupPolicy` est défini).
...
Utilisez ces formats de cible avec `openclaw message send` ou Cron/Webhook :
...
## Nouvelle tentative pour le canal de message privé

Lorsqu’OpenClaw envoie vers une cible de message privé Mattermost et doit d’abord résoudre le canal direct, il
retente par défaut les échecs transitoires de création de canal direct.
...
- Cela s’applique uniquement à la création de canal de message privé (`/api/v4/channels/direct`), pas à tous les appels API Mattermost.
...
- Les erreurs client 4xx autres que `429` sont traitées comme permanentes et ne sont pas retentées.

## Aperçu en streaming

Mattermost diffuse la réflexion, l’activité des outils et le texte partiel de la réponse dans un unique **brouillon d’aperçu** qui se finalise sur place lorsque la réponse finale peut être envoyée en toute sécurité. L’aperçu est mis à jour sur le même identifiant de message au lieu de spammer le canal avec des messages par segment. Les messages finaux de média/erreur annulent les modifications d’aperçu en attente et utilisent la livraison normale au lieu de vider un message d’aperçu jetable.
...
- `block` utilise des segments de brouillon de type ajout dans le message d’aperçu.
- `progress` affiche un aperçu d’état pendant la génération et ne publie la réponse finale qu’à la fin.
- `off` désactive l’aperçu en streaming.
- Si le flux ne peut pas être finalisé sur place (par exemple si le message a été supprimé en cours de flux), OpenClaw revient à l’envoi d’un nouveau message final afin que la réponse ne soit jamais perdue.
- Les charges utiles contenant uniquement du raisonnement sont supprimées des messages du canal, y compris le texte qui arrive comme bloc de citation `> Reasoning:`. Définissez `/reasoning on` pour voir la réflexion dans d’autres surfaces ; le message final Mattermost conserve uniquement la réponse.
...
- `messageId` est l’identifiant du message Mattermost.
...
- `channels.mattermost.actions.reactions` : activer/désactiver les actions de réaction (valeur par défaut : true).
...
Utilisez `message action=send` avec un paramètre `buttons`. Les boutons forment un tableau 2D (lignes de boutons) :
...
- `callback_data` (obligatoire) : valeur renvoyée lors du clic (utilisée comme identifiant d’action).
- `style` (facultatif) : `"default"`, `"primary"` ou `"danger"`.

Lorsqu’un utilisateur clique sur un bouton :

1. Tous les boutons sont remplacés par une ligne de confirmation (par exemple, "✓ **Yes** selected by @user").
...
- Mattermost supprime les données de rappel de ses réponses API (fonction de sécurité), donc tous les boutons
  sont supprimés au clic — une suppression partielle n’est pas possible.
- Les identifiants d’action contenant des tirets ou des traits de soulignement sont nettoyés automatiquement
...
- `channels.mattermost.interactions.callbackBaseUrl` : URL de base externe facultative pour les
  ...
  atteindre directement la Gateway à son hôte de liaison.
...
- Si `interactions.callbackBaseUrl` est omis, OpenClaw dérive l’URL de rappel à partir de
  `gateway.customBindHost` + `gateway.port`, puis se replie sur `http://localhost:<port>`.
- Règle d’accessibilité : l’URL de rappel de bouton doit être accessible depuis le serveur Mattermost.
  `localhost` fonctionne uniquement lorsque Mattermost et OpenClaw s’exécutent sur le même hôte/espace de noms réseau.
...
Les scripts externes et les Webhook peuvent publier des boutons directement via l’API REST Mattermost
au lieu de passer par l’outil `message` de l’agent. Utilisez `buildButtonAttachments()` depuis
le plugin lorsque c’est possible ; si vous publiez du JSON brut, suivez ces règles :
...
3. Chaque action doit avoir un champ `id` — Mattermost ignore les actions sans identifiants.
4. L’`id` d’action doit être **strictement alphanumérique** (`[a-zA-Z0-9]`). Les tirets et les traits de soulignement cassent
   le routage d’action côté serveur de Mattermost (renvoie 404). Supprimez-les avant utilisation.
...
La Gateway vérifie les clics sur les boutons avec HMAC-SHA256. Les scripts externes doivent générer des jetons
qui correspondent à la logique de vérification de la Gateway :
...
3. Sérialisez avec des **clés triées** et **sans espaces** (la Gateway utilise `JSON.stringify`
   avec des clés triées, ce qui produit une sortie compacte).
4. Signez : `HMAC-SHA256(key=secret, data=serializedContext)`
5. Ajoutez l’empreinte hexadécimale obtenue comme `_token` dans le contexte.
...
- Signez toujours **tous** les champs du contexte (moins `_token`). La Gateway supprime `_token` puis
  signe tout ce qui reste. Signer seulement un sous-ensemble provoque un échec silencieux de la vérification.
- Utilisez `sort_keys=True` — la Gateway trie les clés avant de signer, et Mattermost peut
  réordonner les champs du contexte lors du stockage de la charge utile.
- Dérivez le secret à partir du jeton du bot (déterministe), et non à partir d’octets aléatoires. Le secret
  doit être le même dans le processus qui crée les boutons et dans la Gateway qui les vérifie.
...
Aucune configuration n’est nécessaire — l’adaptateur utilise le jeton du bot de la configuration du compte.
...
- Les commandes slash natives renvoient `Unauthorized: invalid command token.` : OpenClaw
  ...
  - la Gateway a redémarré sans réactiver les commandes slash natives
...
- Si `callbackUrl` est omis et que les journaux avertissent que le rappel s’est résolu vers
  `http://127.0.0.1:18789/...`, cette URL n’est probablement accessible que lorsque
  Mattermost s’exécute sur le même hôte/espace de noms réseau qu’OpenClaw. Définissez plutôt une
  `commands.callbackUrl` explicite et accessible de l’extérieur.
- Les boutons apparaissent comme des cases blanches : l’agent envoie peut-être des données de bouton mal formées. Vérifiez que chaque bouton possède à la fois les champs `text` et `callback_data`.
...
- Les boutons renvoient 404 au clic : l’`id` du bouton contient probablement des tirets ou des traits de soulignement. Le routeur d’action de Mattermost ne fonctionne pas avec des identifiants non alphanumériques. Utilisez uniquement `[a-zA-Z0-9]`.
- La Gateway journalise `invalid _token` : incompatibilité HMAC. Vérifiez que vous signez tous les champs du contexte (pas seulement un sous-ensemble), que vous utilisez des clés triées et du JSON compact (sans espaces). Voir la section HMAC ci-dessus.
- La Gateway journalise `missing _token in context` : le champ `_token` n’est pas dans le contexte du bouton. Assurez-vous qu’il est inclus lors de la construction de la charge utile d’intégration.
- La confirmation affiche l’identifiant brut au lieu du nom du bouton : `context.action_id` ne correspond pas à l’`id` du bouton. Définissez les deux avec la même valeur nettoyée.
