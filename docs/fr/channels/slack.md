...
title: Slack
x-i18n:
    generated_at: "2026-05-11T20:21:49Z"
    model: gpt-5.5
    provider: openai
...
**Choisissez les HTTP Request URLs** lorsque vous exécutez plusieurs réplicas de Gateway derrière un répartiteur de charge, lorsque le WSS sortant est bloqué mais que le HTTPS entrant est autorisé, ou lorsque vous terminez déjà les Webhook Slack sur un proxy inverse.
</Note>
...
```
        ...
        <Note>
          **Recommandé** correspond à l’ensemble complet de fonctionnalités du Plugin Slack intégré : App Home, commandes slash, fichiers, réactions, épingles, DM de groupe et lectures d’emojis/groupes d’utilisateurs. Choisissez **Minimal** lorsque la politique de l’espace de travail limite les portées : il couvre les DM, l’historique des canaux/groupes, les mentions et les commandes slash, mais exclut les fichiers, les réactions, les épingles, les DM de groupe (`mpim:*`), `emoji:read` et `usergroups:read`. Consultez la [liste de contrôle du manifest et des portées](#manifest-and-scope-checklist) pour la justification de chaque portée et les options additives comme des commandes slash supplémentaires.
        </Note>
...
- `configured_unavailable` signifie que le compte est configuré via SecretRef
  ou une autre source de secret non intégrée, mais que la commande ou le chemin
...
<Tabs>
  ...
  <Tab title="Mentions et utilisateurs de canal">
    ...
    - `systemPrompt`
    - `tools`, `toolsBySender`
    - format des clés `toolsBySender` : `channel:`, `id:`, `e164:`, `username:`, `name:` ou caractère générique `"*"`
      (les anciennes clés sans préfixe sont encore mappées uniquement vers `id:`)
...
<Note>
`replyToMode="off"` désactive **tout** le threading de réponses dans Slack, y compris les balises explicites `[[reply_to_*]]`. Cela diffère de Telegram, où les balises explicites sont toujours honorées en mode `"off"`. Les threads Slack masquent les messages du canal, tandis que les réponses Telegram restent visibles en ligne.
</Note>
...
opter pour la livraison au chat d’origine :
...
séparé ; les boutons natifs Slack peuvent toujours résoudre les approbations de Plugin lorsque ces requêtes arrivent déjà
dans Slack.
...
```
  ...
  <Accordion title="Messages DM ignorés">
    ...
    - événements DM de l’assistant Slack : les journaux verbeux mentionnant `drop message_changed`
...
```
  ...
  <Accordion title="Socket mode ne se connecte pas">
    ...
    configuré, mais l’exécution actuelle n’a pas pu résoudre la valeur reposant sur SecretRef.

  </Accordion>

  <Accordion title="Mode HTTP ne recevant pas les événements">
    Valider :
    ...
    instantanés de compte, le compte HTTP est configuré, mais l’exécution
    actuelle n’a pas pu résoudre le secret de signature adossé à SecretRef.

  </Accordion>

  <Accordion title="Commandes natives/slash non déclenchées">
