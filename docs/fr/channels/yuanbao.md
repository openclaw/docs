...
title: Yuanbao
x-i18n:
    generated_at: "2026-07-12T15:09:33Z"
    model: gpt-5.6
...
<Steps>
  ...
  ```
  `--token` utilise le format `appKey:appSecret` séparé par un deux-points. Obtenez ces valeurs dans l’application Yuanbao en créant un bot dans les paramètres de votre application.
  </Step>
...
| `pairing`        | Les utilisateurs inconnus reçoivent un code d’association ; approuver via CLI |
| `allowlist`      | Seuls les utilisateurs figurant dans `allowFrom` peuvent discuter             |
| `disabled`       | Désactiver tous les messages directs                                          |

Approuvez une demande d’association :
...
`channels.yuanbao.requireMention` (`true` par défaut) : exige une @mention avant que le bot ne réponde dans un groupe. Une réponse au propre message du bot est considérée comme une mention implicite.
...
Restreindre les messages directs à des utilisateurs précis :
...
```json5
{
  channels: {
    yuanbao: {
      ...
      maxChars: 3000, // forcer le fractionnement au-delà de cette limite
...
Yuanbao prend en charge les menus natifs de commandes à barre oblique ; les commandes sont automatiquement synchronisées avec la plateforme au démarrage du Gateway.
...
- `maxChars` : nombre maximal de caractères par message (valeur par défaut : `3000`)
- `mediaMaxMb` : limite de téléversement/téléchargement des médias (valeur par défaut : `20` MB)
...
Par défaut, le bot injecte une instruction dans l’invite système afin d’empêcher le modèle d’encapsuler l’intégralité de la réponse dans un bloc de code Markdown.
...
Active la sortie de journaux non expurgée pour les identifiants de bots répertoriés.
...
| `channels.yuanbao.accounts.<id>.appKey`    | App Key (signature + génération de tickets)                          | -                                      |
...
| `channels.yuanbao.accounts.<id>.enabled`   | Activer/désactiver un compte précis                                  | `true`                                 |
| `channels.yuanbao.dm.policy`               | Politique des messages directs                                       | `open`                                 |
| `channels.yuanbao.dm.allowFrom`            | Liste d’autorisation des messages directs (liste d’identifiants utilisateur) | -                              |
...
| `channels.yuanbao.markdownHintEnabled`     | Injecter des instructions empêchant l’encapsulation Markdown          | `true`                                 |
