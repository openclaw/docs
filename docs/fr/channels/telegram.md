...
title: Telegram
x-i18n:
    generated_at: "2026-06-30T13:56:18Z"
    model: gpt-5.5
...
Prêt pour la production pour les DM et groupes de bots via grammY. Le long polling est le mode par défaut ; le mode Webhook est facultatif.
...
```
  ...
  <Step title="Ajouter le bot à un groupe">
    ...
    Les ID négatifs de supergroupes Telegram qui commencent par `-100` sont des ID de discussion de groupe. Placez-les sous `channels.telegram.groups`, pas sous `groupAllowFrom`.

  </Step>
</Steps>

<Note>
L’ordre de résolution des jetons tient compte du compte. En pratique, les valeurs de configuration l’emportent sur la solution de repli d’environnement, et `TELEGRAM_BOT_TOKEN` s’applique uniquement au compte par défaut.
Après un démarrage réussi, OpenClaw met en cache l’identité du bot dans le répertoire d’état jusqu’à 24 heures afin que les redémarrages puissent éviter un appel Telegram `getMe` supplémentaire ; modifier ou supprimer le jeton efface ce cache.
</Note>
...
<AccordionGroup>
  <Accordion title="Mode de confidentialité et visibilité des groupes">
    ...
    Lorsque vous modifiez le mode de confidentialité, retirez puis rajoutez le bot dans chaque groupe afin que Telegram applique le changement.
...
Dans les groupes et sujets de forum Telegram, une mention explicite de l’identifiant du bot configuré (par exemple `@my_bot`) est traitée comme une adresse à l’agent OpenClaw sélectionné, même lorsque le nom de persona de l’agent diffère du nom d’utilisateur Telegram. La politique de silence du groupe s’applique toujours au trafic de groupe non lié, mais l’identifiant du bot lui-même n’est pas considéré comme « quelqu’un d’autre ».
...
<Tabs>
  <Tab title="Politique de DM">
    ...
    L’appairage accorde l’accès aux DM. Si aucun propriétaire de commande n’existe encore, le premier appairage approuvé définit aussi `commands.ownerAllowFrom` afin que les commandes réservées au propriétaire et les approbations d’exécution disposent d’un compte opérateur explicite.
    L’autorisation des expéditeurs de groupe provient toujours de listes d’autorisation de configuration explicites.
...
```
  ...
  <Tab title="Politique de groupe et listes d’autorisation">
    ...
    Ne placez pas les ID de discussion de groupes ou supergroupes Telegram dans `groupAllowFrom`. Les ID de discussion négatifs appartiennent à `channels.telegram.groups`.
    ...
    Note d’exécution : si `channels.telegram` est complètement absent, l’exécution utilise par défaut `groupPolicy="allowlist"` en mode fail-closed, sauf si `channels.defaults.groupPolicy` est explicitement défini.
...
```
    ...
    <Warning>
      ...
      - Placez les ID de discussion négatifs de groupes ou supergroupes Telegram comme `-1001234567890` sous `channels.telegram.groups`.
...
```
    ...
    Le contexte d’historique de groupe utilise `mention-only` par défaut : les messages de groupe antérieurs sont
...
- Les messages entrants sont normalisés dans l’enveloppe de canal partagée avec les métadonnées de réponse, les espaces réservés aux médias et le contexte persistant de chaîne de réponses pour les réponses Telegram observées par le Gateway.
...
- Les messages DM peuvent porter `message_thread_id` ; OpenClaw le conserve pour les réponses. Les sessions de sujet DM ne se divisent que lorsque Telegram `getMe` indique `has_topics_enabled: true` pour le bot ; sinon les DM restent sur la session plate.
- Le long polling utilise grammY runner avec un séquençage par chat/par fil. La concurrence globale du sink runner utilise `agents.defaults.maxConcurrent`.
- Le démarrage multi-comptes limite les sondes Telegram `getMe` simultanées afin que les grandes flottes de bots ne lancent pas toutes les sondes de compte en même temps.
- Le long polling est protégé dans chaque processus Gateway afin qu’un seul poller actif puisse utiliser un jeton de bot à la fois. Si vous voyez toujours des conflits `getUpdates` 409, un autre Gateway OpenClaw, script ou poller externe utilise probablement le même jeton.
- Les redémarrages déclenchés par le watchdog de long polling se produisent par défaut après 120 secondes sans liveness `getUpdates` terminée. Augmentez `channels.telegram.pollingStallThresholdMs` uniquement si votre déploiement voit encore de faux redémarrages pour blocage du polling pendant des travaux longs. La valeur est en millisecondes et autorisée de `30000` à `600000` ; les remplacements par compte sont pris en charge.
- L’API Telegram Bot ne prend pas en charge les accusés de lecture (`sendReadReceipts` ne s’applique pas).

<Note>
  `channels.telegram.dm.threadReplies` et `channels.telegram.direct.<chatId>.threadReplies` ont été supprimés. Exécutez `openclaw doctor --fix` après la mise à niveau si votre configuration contient encore ces clés. Le routage des sujets DM suit désormais la capacité du bot depuis Telegram `getMe.has_topics_enabled`, qui est contrôlée par le mode avec fils de BotFather : les bots avec sujets activés utilisent des sessions DM limitées au fil lorsque Telegram envoie `message_thread_id` ; les autres DM restent sur la session plate.
</Note>
...
<AccordionGroup>
  <Accordion title="Live stream preview (message edits)">
    ...
    - les courts aperçus de réponse initiale sont soumis à un debounce, puis matérialisés après un délai borné si l’exécution est toujours active
    ...
    Utilisez le mode `progress` lorsque vous voulez une progression d’outil visible sans modifier la réponse finale dans ce même message. Placez la règle de texte de commande sous `streaming.progress` :
    ...
    - les longues réponses finales textuelles découpées en plusieurs messages Telegram réutilisent l’aperçu existant comme premier fragment final lorsque c’est possible, puis envoient seulement les fragments restants
    ...
    Le streaming d’aperçu est distinct du streaming par bloc. Lorsque le streaming par bloc est explicitement activé pour Telegram, OpenClaw ignore le flux d’aperçu pour éviter une double diffusion.
  ...
  <Accordion title="Rich message formatting">
    Le texte sortant utilise par défaut les messages HTML Telegram standards afin que les réponses restent lisibles sur les clients Telegram actuels. Ce mode de compatibilité prend en charge le gras, l’italique, les liens, le code, les spoilers et les citations ordinaires, mais pas les blocs enrichis uniquement de Bot API 10.1, comme les tableaux natifs, les détails, les médias enrichis et les formules.
...
```
    ...
    - L’agent reçoit l’information que les messages enrichis Telegram sont disponibles pour ce bot/compte.
    - Le texte Markdown est rendu via l’IR Markdown d’OpenClaw et envoyé en HTML enrichi Telegram.
    - Les charges utiles HTML enrichies explicites conservent les balises prises en charge par Bot API 10.1, comme les titres, tableaux, détails, médias enrichis et formules.
...
```
    ...
    - L’échec de `deleteWebhook`, `deleteMyCommands` ou `setMyCommands` avec `404: Not Found` alors que les commandes directes curl de Bot API fonctionnent peut signifier que `channels.telegram.apiRoot` a été défini sur le point de terminaison complet `/bot<TOKEN>`. `apiRoot` doit être uniquement la racine de Bot API, et `openclaw doctor --fix` supprime un `/bot<TOKEN>` final accidentel.
...
```
  ...
  <Accordion title="Actions de messages Telegram pour les agents et l’automatisation">
    ...
    Les envois à l’exécution utilisent l’instantané actif de configuration/secrets (démarrage/rechargement), les chemins d’action ne réanalysent donc pas les SecretRef de façon ponctuelle à chaque envoi.
  ...
  <Accordion title="Sujets de forum et comportement des fils">
    ...
    **Spawn ACP lié au fil depuis la discussion** : `/acp spawn <agent> --thread here|auto` lie le sujet actuel à une nouvelle session ACP ; les suivis y sont routés directement. OpenClaw épingle la confirmation de spawn dans le sujet. Nécessite que `channels.telegram.threadBindings.spawnSessions` reste activé (par défaut : `true`).
    ...
    Les anciens remplacements `dm.threadReplies` et `direct.*.threadReplies` sont volontairement retirés ; utilisez le mode avec fils de BotFather comme source unique de vérité et exécutez `openclaw doctor --fix` pour supprimer les clés de configuration obsolètes.
...
```
  ...
  <Accordion title="Ack reactions">
    ...
    **Portée (`messages.ackReactionScope`) :**
    ...
    Le fournisseur Telegram lit la portée depuis `messages.ackReactionScope` (par défaut `"group-mentions"`). Il n’existe actuellement pas de remplacement au niveau du compte Telegram ou du canal Telegram.
...
```
  ...
  <Accordion title="Long polling vs webhook">
    ...
    En mode long-polling, OpenClaw conserve son repère de redémarrage uniquement après qu’une mise à jour a été distribuée avec succès. Si un gestionnaire échoue, cette mise à jour reste réessayable dans le même processus et n’est pas écrite comme terminée pour la déduplication au redémarrage.
  ...
  <Accordion title="Limites, nouvelle tentative et cibles CLI">
    ...
    - le contexte supplémentaire des réponses/citations/transferts est normalisé dans une fenêtre de contexte de conversation sélectionnée lorsque le gateway a observé les messages parents ; le cache des messages observés réside dans l’état de Plugin SQLite OpenClaw, et `openclaw doctor --fix` importe les anciens fichiers latéraux. Telegram n’inclut qu’un seul `reply_to_message` superficiel dans les mises à jour, les chaînes plus anciennes que le cache sont donc limitées à la charge utile de mise à jour actuelle de Telegram.
    ...
    - La configuration `channels.telegram.retry` s’applique aux helpers d’envoi Telegram (CLI/outils/actions) pour les erreurs d’API sortantes récupérables. La livraison de la réponse finale entrante utilise aussi une nouvelle tentative d’envoi sûre et bornée pour les échecs de préconnexion Telegram, mais elle ne retente pas les enveloppes réseau ambiguës après envoi qui pourraient dupliquer des messages visibles.

    Les cibles d’envoi CLI et d’outils de message peuvent être un identifiant de discussion numérique, un nom d’utilisateur ou une cible de sujet de forum :
...
```
  ...
  <Accordion title="Approbations d’exécution dans Telegram">
    Telegram prend en charge les approbations d’exécution dans les DM d’approbateurs et peut facultativement publier les invites dans la discussion ou le sujet d’origine. Les approbateurs doivent être des identifiants numériques d’utilisateurs Telegram.

    Chemin de configuration :

    - `channels.telegram.execApprovals.enabled` (s’active automatiquement lorsqu’au moins un approbateur peut être résolu)
    - `channels.telegram.execApprovals.approvers` (se rabat sur les identifiants numériques des propriétaires issus de `commands.ownerAllowFrom`)
    - `channels.telegram.execApprovals.target` : `dm` (par défaut) | `channel` | `both`
    - `agentFilter`, `sessionFilter`

    `channels.telegram.allowFrom`, `groupAllowFrom` et `defaultTo` contrôlent qui peut parler au bot et où il envoie les réponses normales. Ils ne font pas d’une personne un approbateur d’exécution. Le premier appairage DM approuvé amorce `commands.ownerAllowFrom` lorsqu’aucun propriétaire de commande n’existe encore, de sorte que la configuration à propriétaire unique fonctionne toujours sans dupliquer les identifiants dans `execApprovals.approvers`.
    ...
    Les boutons d’approbation intégrés exigent aussi que `channels.telegram.capabilities.inlineButtons` autorise la surface cible (`dm`, `group` ou `all`). Les identifiants d’approbation préfixés par `plugin:` se résolvent via les approbations de plugin ; les autres se résolvent d’abord via les approbations d’exécution.
...
Lorsque l’agent rencontre une erreur de livraison ou de fournisseur, la politique d’erreur contrôle si des messages d’erreur sont envoyés à la discussion Telegram :

| Clé                                 | Valeurs                    | Par défaut      | Description                                                                                                                                                                                                                                  |
| ----------------------------------- | -------------------------- | --------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `channels.telegram.errorPolicy`     | `always`, `once`, `silent` | `always`        | `always` — envoyer chaque message d’erreur à la discussion. `once` — envoyer chaque message d’erreur unique une fois par fenêtre de refroidissement (supprime les erreurs identiques répétées). `silent` — ne jamais envoyer de messages d’erreur à la discussion. |
...
<AccordionGroup>
  <Accordion title="Le bot ne répond pas aux messages de groupe sans mention">
    ...
    - `openclaw channels status --probe` peut vérifier les identifiants numériques de groupe explicites ; le caractère générique `"*"` ne peut pas être sondé pour l’appartenance.
  ...
  <Accordion title="Le démarrage signale un jeton non autorisé">
    ...
    - `deleteWebhook 401 Unauthorized` pendant le démarrage est aussi un échec d’authentification ; le traiter comme « aucun Webhook n’existe » ne ferait que différer le même échec de mauvais jeton à des appels API ultérieurs.
  ...
  <Accordion title="Instabilité de polling ou réseau">
    ...
    - Si les journaux incluent `TypeError: fetch failed` ou `Network request for 'getUpdates' failed!`, OpenClaw retente maintenant ces erreurs réseau comme récupérables.
    ...
    - Si `deleteWebhook` échoue avec une erreur réseau transitoire pendant le démarrage du polling, OpenClaw continue en long polling au lieu d’effectuer un autre appel de plan de contrôle avant le polling. Un webhook encore actif apparaît comme un conflit `getUpdates` ; OpenClaw reconstruit alors le transport Telegram et retente le nettoyage du webhook.
    - Si les sockets Telegram sont recyclés à une cadence fixe courte, vérifiez si `channels.telegram.timeoutSeconds` est bas ; les clients bot plafonnent les valeurs configurées sous les gardes de requêtes sortantes et `getUpdates`, mais les versions plus anciennes pouvaient abandonner chaque polling ou réponse lorsque cette valeur était définie sous ces gardes.
...
```
    ...
    - Les réponses de la plage de benchmark RFC 2544 (`198.18.0.0/15`) sont déjà autorisées
      par défaut pour les téléchargements de médias Telegram. Si un fake-IP approuvé ou
      ...
      explicitement le contournement réservé à Telegram :
...
```
    ...
    - La même activation explicite est disponible par compte à
    ...
    <Warning>
      ...
      des médias Telegram. Utilisez-le uniquement pour des environnements de proxy approuvés
      ...
      RFC 2544. Laissez-le désactivé pour l’accès Telegram normal via l’internet public.
    </Warning>
...
- valeurs par défaut des sujets : `groups.<chatId>.topics."*"` s’applique aux sujets de forum sans correspondance ; les ID de sujet exacts le remplacent
...
<CardGroup cols={2}>
  ...
  <Card title="Routage multi-agent" icon="sitemap" href="/fr/concepts/multi-agent">
    Mappez les groupes et les sujets vers les agents.
  </Card>
