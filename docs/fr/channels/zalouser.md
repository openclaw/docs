...
x-i18n:
    generated_at: "2026-07-12T15:10:04Z"
    model: gpt-5.6
...
Zalo Personal est un plugin externe officiel, qui n'est pas inclus dans le cœur. Installez-le avant de l'utiliser :
...
- Depuis une copie de travail des sources : `openclaw plugins install ./path/to/local/zalouser-plugin`
...
5. Par défaut, l'accès aux messages privés utilise l'association ; approuvez le code d'association lors du premier contact.

## Présentation

- S'exécute entièrement au sein du processus via la bibliothèque `zca-js` (sans binaire externe `zca`/`openzca`).
- Utilise des écouteurs d'événements natifs (`message`, `error`) pour recevoir les messages entrants.
...
L'identifiant du canal est `zalouser` afin d'indiquer explicitement qu'il automatise un **compte utilisateur Zalo personnel** (non officiel). `zalo` est réservé à une éventuelle future intégration officielle de l'API Zalo.

## Recherche d'identifiants (annuaire)
...
- Le texte sortant est fractionné en segments de 2000 caractères (limite du client Zalo).
...
`channels.zalouser.allowFrom` doit utiliser des identifiants utilisateur Zalo stables. Il peut également référencer des groupes statiques d'accès des expéditeurs (`accessGroup:<name>`). Pendant la configuration interactive, les noms saisis peuvent être résolus en identifiants à l'aide de la recherche de contacts intégrée au processus du plugin.

Si un nom brut demeure dans la configuration, il n'est résolu au démarrage que lorsque `channels.zalouser.dangerouslyAllowNameMatching: true` est activé. Sans cette activation explicite, les vérifications des expéditeurs à l'exécution se fondent uniquement sur les identifiants et les noms bruts sont ignorés pour l'autorisation.
...
- Par défaut, la correspondance avec la liste d'autorisation des groupes se fonde uniquement sur les identifiants. Les noms non résolus sont ignorés pour l'autorisation, sauf si `channels.zalouser.dangerouslyAllowNameMatching: true` est activé.
- `channels.zalouser.dangerouslyAllowNameMatching: true` est un mode de compatibilité de dernier recours qui réactive la résolution au démarrage des noms modifiables ainsi que la correspondance avec les noms de groupe à l'exécution.
- Pour les messages de groupe normaux, `groupAllowFrom` ne se rabat **pas** sur `allowFrom` : si cette valeur est laissée vide pour un groupe figurant dans la liste d'autorisation, tout expéditeur peut accéder à ce groupe. Les commandes de contrôle autorisées (par exemple `/new`) constituent l'exception ; les vérifications de l'expéditeur d'une commande se rabattent sur `allowFrom` lorsque `groupAllowFrom` est vide.
...
### Filtrage des groupes par mention

- `channels.zalouser.groups.<group>.requireMention` détermine si les réponses dans un groupe nécessitent une mention.
- Ordre de résolution : identifiant du groupe -> alias `group:<id>` -> nom/slug du groupe (les candidats fondés sur le nom ne s'appliquent que lorsque `dangerouslyAllowNameMatching: true`) -> `*` -> valeur par défaut (`true`).
...
- Limite de l'historique de groupe : `channels.zalouser.historyLimit`, puis `messages.groupChat.historyLimit`, puis une valeur de repli de `50`.
...
| `ZCA_PROFILE`      | Valeur de repli héritée, utilisée uniquement lorsque `ZALOUSER_PROFILE` n'est pas définie.                    |
...
Pour les configurations à plusieurs comptes, il est préférable de définir `profile` sur chaque compte dans la configuration afin qu'une seule variable d'environnement ne conduise pas plusieurs comptes à partager la même session de connexion.

## Saisie, réactions et accusés de réception

- OpenClaw envoie un événement de saisie avant d'expédier une réponse (dans la mesure du possible).
- L'action de réaction aux messages `react` est prise en charge pour `zalouser` dans les actions de canal.
  - Utilisez `remove: true` pour retirer un emoji de réaction précis d'un message.
  - Sémantique des réactions : [Réactions](/fr/tools/reactions)
- Pour les messages entrants qui comprennent des métadonnées d'événement, OpenClaw envoie des accusés de réception de remise et de lecture (dans la mesure du possible).
...
- Supprimez toute dépendance supposée envers un processus externe `zca` ; le canal s'exécute désormais entièrement au sein du processus via `zca-js`, sans binaire CLI externe.
