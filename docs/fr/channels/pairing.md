...
summary: 'Vue d’ensemble de l’appairage : approuvez qui peut vous envoyer un message direct + quels nœuds peuvent rejoindre'
title: Appairage
x-i18n:
...
"L'appariement" est l'étape explicite d'approbation d'accès d'OpenClaw.
Il est utilisé à deux endroits :

1. **Appariement DM** (qui est autorisé à parler au bot)
2. **Appariement de Node** (quels appareils/nodes sont autorisés à rejoindre le réseau Gateway)

Contexte de sécurité : [Sécurité](/fr/gateway/security)

## 1) Appariement DM (accès au chat entrant)
...
que ces expéditeurs, et les approbations du magasin d'appariement n'élargissent pas l'accès `open`.

Codes d'appariement :

- 8 caractères, majuscules, aucun caractère ambigu (`0O1I`).
- **Expirent après 1 heure**. Le bot n'envoie le message d'appariement que lorsqu'une nouvelle demande est créée (environ une fois par heure et par expéditeur).
- Les demandes d'appariement DM en attente sont limitées à **3 par canal** par défaut ; les demandes supplémentaires sont ignorées jusqu'à ce que l'une expire ou soit approuvée.
...
Si aucun propriétaire de commande n'est encore configuré, l'approbation d'un code d'appariement DM initialise aussi
...
d'approbation d'exécution. Une fois qu'un propriétaire existe, les approbations d'appariement ultérieures ne donnent que
...
<Note>
Le magasin de liste d'autorisation d'appariement sert à l'accès DM. L'autorisation de groupe est distincte.
L'approbation d'un code d'appariement DM n'autorise pas automatiquement cet expéditeur à exécuter des commandes
de groupe ni à contrôler le bot dans les groupes. L'initialisation du premier propriétaire est un état de configuration
distinct dans `commands.ownerAllowFrom`, et la livraison dans les chats de groupe suit toujours les listes d'autorisation
de groupe du canal (par exemple `groupAllowFrom`, `groups`, ou les remplacements par groupe
ou par sujet selon le canal).
</Note>

## 2) Appariement d'appareil Node (nodes iOS/Android/macOS/sans interface)

Les nodes se connectent au Gateway comme **appareils** avec `role: node`. Le Gateway
crée une demande d'appariement d'appareil qui doit être approuvée.

### Apparier via Telegram (recommandé pour iOS)
...
2. Le bot répond avec deux messages : un message d'instructions et un message de **code de configuration** séparé (facile à copier/coller dans Telegram).
...
- `bootstrapToken` : un jeton d'initialisation de courte durée pour un seul appareil, utilisé pour la poignée de main d'appariement initiale

Ce jeton d'initialisation porte le profil d'initialisation d'appariement intégré :

- le jeton `node` transféré principal reste `scopes: []`
- tout jeton `operator` transféré reste limité à la liste d'autorisation d'initialisation :
  `operator.approvals`, `operator.read`, `operator.talk.secrets`, `operator.write`
- les vérifications de portée d'initialisation sont préfixées par rôle, et non issues d'un seul pool de portées plat :
...
Pour Tailscale, un appariement mobile public ou tout autre appariement mobile distant, utilisez Tailscale Serve/Funnel
ou une autre URL Gateway `wss://`. Les codes de configuration en clair `ws://` ne sont acceptés que
pour local loopback, les adresses LAN privées, les hôtes Bonjour `.local` et l'hôte de l'émulateur
Android. Les adresses CGNAT de tailnet, les noms `.ts.net` et les hôtes publics
échouent toujours de manière fermée avant l'émission du QR/code de configuration.
...
a été ouverte avec une portée limitée à l'appariement, la CLI réessaie la même demande avec
`operator.admin`. Cela permet à un appareil appairé existant capable d'administration de récupérer un nouvel
appariement Control UI/navigateur sans modifier `devices/paired.json` à la main. Le
...
L'appariement d'appareil reste manuel par défaut. Pour les réseaux de nodes strictement contrôlés,
vous pouvez activer l'approbation automatique du premier appariement de node avec des CIDR explicites ou des IP exactes :
...
Cela ne s'applique qu'aux nouvelles demandes d'appariement `role: node` sans
portées demandées. Les clients opérateur, navigateur, Control UI et WebChat exigent toujours une approbation
manuelle. Les changements de rôle, de portée, de métadonnées et de clé publique exigent toujours une approbation
manuelle.

### Stockage de l'état d'appariement Node
...
- L'API héritée `node.pair.*` (CLI : `openclaw nodes pending|approve|reject|remove|rename`) est un
  magasin d'appariement distinct appartenant au Gateway. Les nodes WS exigent toujours l'appariement d'appareil.
- L'enregistrement d'appariement est la source de vérité durable pour les rôles approuvés. Les jetons d'appareil
  actifs restent limités à cet ensemble de rôles approuvés ; une entrée de jeton isolée
