...
read_when:
    ...
    - Configuration du chiffrement E2EE et de la vérification Matrix
...
Il utilise le `matrix-js-sdk` officiel et prend en charge les MP, les salons, les fils, les médias, les réactions, les sondages, la localisation et l’E2EE.
...
L’assistant demande : l’URL du serveur d’accueil, la méthode d’authentification (jeton d’accès ou mot de passe), l’ID utilisateur (authentification par mot de passe uniquement), le nom d’appareil facultatif, s’il faut activer l’E2EE, et s’il faut configurer l’accès aux salons et l’auto-join.
...
`channels.matrix.autoJoin` vaut `off` par défaut. Avec la valeur par défaut, le bot n’apparaîtra pas dans les nouveaux salons ou MP issus de nouvelles invitations tant que vous ne l’aurez pas rejoint manuellement.

OpenClaw ne peut pas savoir au moment de l’invitation si un salon invité est un MP ou un groupe ; toutes les invitations, y compris celles de type MP, passent donc d’abord par `autoJoin`. `dm.policy` ne s’applique que plus tard, après que le bot a rejoint le salon et que celui-ci a été classé.
...
`autoJoinAllowlist` n’accepte que des cibles stables : `!roomId:server`, `#alias:server` ou `*`. Les noms de salon simples sont rejetés ; les entrées d’alias sont résolues auprès du serveur d’accueil, et non d’après l’état revendiqué par le salon invité.
</Warning>
...
Les listes d’autorisation de MP et de salons doivent de préférence être remplies avec des ID stables :

- MP (`dm.allowFrom`, `groupAllowFrom`, `groups.<room>.users`) : utilisez `@user:server`. Les noms d’affichage sont ignorés par défaut car ils sont modifiables ; définissez `dangerouslyAllowNameMatching: true` uniquement lorsque vous avez explicitement besoin d’une compatibilité avec les entrées de nom d’affichage.
- Clés de liste d’autorisation de salon (`groups`, ancien `rooms`) : utilisez `!room:server` ou `#alias:server`. Les noms de salon simples sont ignorés par défaut ; définissez `dangerouslyAllowNameMatching: true` uniquement lorsque vous avez explicitement besoin d’une compatibilité avec la recherche par nom de salon rejoint.
...
Une base pratique avec appairage des MP, liste d’autorisation de salon et E2EE :
...
Le streaming des réponses Matrix est à activation explicite. `streaming` contrôle la façon dont OpenClaw transmet la réponse de l’assistant en cours ; `blockStreaming` contrôle si chaque bloc terminé est conservé comme son propre message Matrix.
...
| `"partial"`          | Modifie un message texte normal sur place pendant que le modèle écrit le bloc actuel. Les clients Matrix standards peuvent notifier au premier aperçu, pas à l’édition finale. |
| `"quiet"`            | Identique à `"partial"`, mais le message est une notification non notifiée. Les destinataires ne reçoivent une notification que lorsqu’une règle push par utilisateur correspond à l’édition finalisée (voir ci-dessous). |
...
- Les réponses multimédias envoient toujours les pièces jointes normalement. Si un aperçu obsolète ne peut plus être réutilisé en sécurité, OpenClaw le caviarde avant d’envoyer la réponse multimédia finale.
...
Les notes vocales Matrix entrantes sont transcrites avant la barrière de mention du salon. Cela permet à une note vocale qui prononce le nom du bot de déclencher l’agent dans un salon `requireMention: true`, et donne à l’agent la transcription au lieu d’un simple espace réservé de pièce jointe audio.
...
- Dans les salons chiffrés, OpenClaw déchiffre la pièce jointe via le chemin multimédia Matrix existant avant la transcription.
...
Les prompts d’approbation natifs Matrix sont des événements `m.room.message` normaux avec un contenu d’événement personnalisé propre à OpenClaw sous `com.openclaw.approval`. Matrix autorise les clés de contenu d’événement personnalisées ; les clients standards affichent donc toujours le corps du texte, tandis que les clients compatibles avec OpenClaw peuvent lire l’ID d’approbation structuré, le type, l’état, les décisions disponibles et les détails d’exécution/de Plugin.

Lorsqu’un prompt d’approbation est trop long pour un seul événement Matrix, OpenClaw découpe le texte visible et attache `com.openclaw.approval` au premier morceau uniquement. Les réactions pour les décisions d’autorisation/refus sont liées à ce premier événement, de sorte que les longs prompts conservent la même cible d’approbation que les prompts à événement unique.

### Règles push auto-hébergées pour les aperçus finalisés silencieux

`streaming: "quiet"` ne notifie les destinataires qu’une fois qu’un bloc ou un tour est finalisé ; une règle push par utilisateur doit correspondre au marqueur d’aperçu finalisé. Consultez [Règles push Matrix pour les aperçus silencieux](/fr/channels/matrix-push-rules) pour la recette complète (jeton destinataire, vérification du pusher, installation de règle, notes par serveur d’accueil).
...
- `allowBots: true` accepte les messages d'autres comptes de bot Matrix configurés dans les salons autorisés et les DM.
...
- Les messages acceptés provenant de bots configurés utilisent la [protection contre les boucles de bots](/fr/channels/bot-loop-protection) partagée. Configurez `channels.defaults.botLoopProtection`, puis remplacez-la par `channels.matrix.botLoopProtection` ou `channels.matrix.groups.<room>.botLoopProtection` lorsqu'un salon nécessite un budget différent.
...
Utilisez des listes d'autorisation strictes de salons et des exigences de mention lorsque vous activez le trafic bot-à-bot dans des salons partagés.
...
- `--recovery-key <key>` appliquer une clé de récupération avant l'amorçage (préférez la forme stdin documentée ci-dessous)
...
- `Signed by owner` : signé avec votre propre clé d'auto-signature (diagnostic uniquement)
...
Elle se termine avec un code non nul lorsque la confiance complète dans l'identité est incomplète, même si la clé de récupération a déverrouillé le matériel de sauvegarde. Dans ce cas, terminez l'auto-vérification depuis un autre client Matrix :
...
- crée une sauvegarde de clés de salon côté serveur s'il n'en existe pas déjà une
...
- `--force-reset-cross-signing` pour supprimer l'identité de signature croisée actuelle (uniquement intentionnel ; nécessite que la clé de récupération active soit stockée ou fournie avec `--recovery-key-stdin`)
...
Pour remplacer une sauvegarde défectueuse par une base fraîche (accepte la perte de l'ancien historique irrécupérable ; peut également recréer le stockage secret si le secret de sauvegarde actuel ne peut pas être chargé) :
...
Pour une gestion de cycle de vie de plus bas niveau - généralement lors du suivi de demandes entrantes depuis un autre client - ces commandes agissent sur une demande spécifique `<id>` (affichée par `verify list` et `verify request`) :
...
`accept`, `start`, `sas`, `confirm-sas`, `mismatch-sas` et `cancel` acceptent tous `--user-id` et `--room-id` comme indications de suivi DM lorsque la vérification est ancrée à un salon de message direct spécifique.

### Notes multi-comptes

Sans `--account <id>`, les commandes CLI Matrix utilisent le compte par défaut implicite. Si vous avez plusieurs comptes nommés et que vous n'avez pas défini `channels.matrix.defaultAccount`, elles refusent de deviner et vous demandent de choisir. Lorsque E2EE est désactivé ou indisponible pour un compte nommé, les erreurs pointent vers la clé de configuration de ce compte, par exemple `channels.matrix.accounts.assistant.encryption`.
...
<AccordionGroup>
  <Accordion title="Startup behavior">
    ...
    Le démarrage exécute également une passe prudente d'amorçage crypto qui réutilise le stockage secret et l'identité de signature croisée actuels. Si l'état d'amorçage est défectueux, OpenClaw tente une réparation encadrée même sans `channels.matrix.password` ; si le serveur d'accueil exige une UIA par mot de passe, le démarrage journalise un avertissement et reste non fatal. Les appareils déjà signés par le propriétaire sont conservés.
  ...
  <Accordion title="Verification notices">
    Matrix publie les avis de cycle de vie de vérification dans le salon strict de vérification DM sous forme de messages `m.notice` : demande, prêt (avec des indications « Vérifier par emoji »), démarrage/achèvement et détails SAS (emoji/décimal) lorsqu'ils sont disponibles.

    Les demandes entrantes depuis un autre client Matrix sont suivies et acceptées automatiquement. Pour l'auto-vérification, OpenClaw démarre automatiquement le flux SAS et confirme son propre côté une fois la vérification par emoji disponible - vous devez toujours comparer et confirmer « Ils correspondent » dans votre client Matrix.
...
```
  ...
  <Accordion title="Device hygiene">
    Les anciens appareils gérés par OpenClaw peuvent s'accumuler. Listez et supprimez :
...
```
  ...
  <Accordion title="Crypto store">
    ...
    Une seule racine plus ancienne de hachage de jeton peut être un chemin normal de continuité de rotation de jeton. Si OpenClaw journalise `matrix: multiple populated token-hash storage roots detected`, inspectez le répertoire du compte et archivez les racines sœurs obsolètes uniquement après avoir confirmé que la racine active sélectionnée est saine. Préférez déplacer les racines obsolètes dans un répertoire `_archive/` plutôt que les supprimer immédiatement.
...
Matrix prend en charge les fils Matrix natifs pour les réponses automatiques comme pour les envois par outil de message. Deux réglages indépendants contrôlent le comportement :
...
- `"off"` : les réponses sont au niveau supérieur. Les messages entrants dans un fil restent sur la session parente.
...
`dm.threadReplies` remplace ce comportement pour les DM uniquement - par exemple, pour garder les fils de salon isolés tout en gardant les DM à plat.

### Héritage de fil et commandes slash

- Les messages entrants dans des fils incluent le message racine du fil comme contexte agent supplémentaire.
- Les envois par outil de message héritent automatiquement du fil Matrix courant lorsqu’ils ciblent le même salon (ou la même cible utilisateur DM), sauf si un `threadId` explicite est fourni.
- La réutilisation de cible utilisateur DM ne s’active que lorsque les métadonnées de session courantes prouvent le même pair DM sur le même compte Matrix ; sinon OpenClaw revient au routage normal limité à l’utilisateur.
...
- Dans un DM ou salon Matrix au niveau supérieur, le DM/salon courant reste la surface de discussion et les messages futurs sont routés vers la session ACP générée.
...
Matrix hérite les valeurs par défaut globales de `session.threadBindings` et prend aussi en charge les surcharges par canal :
...
Matrix prend en charge les réactions sortantes, les notifications de réactions entrantes et les réactions d’accusé.
...
| Paramètre               | Ordre                                                                            |
| ----------------------- | -------------------------------------------------------------------------------- |
| `ackReaction`           | par compte → canal → `messages.ackReaction` → repli sur l’emoji d’identité agent |
...
`reactionNotifications: "own"` transfère les événements `m.reaction` ajoutés lorsqu’ils ciblent des messages Matrix rédigés par le bot ; `"off"` désactive les événements système de réaction. Les suppressions de réactions ne sont pas synthétisées en événements système, car Matrix les expose comme des occultations, et non comme des suppressions `m.reaction` autonomes.

## Contexte d’historique

- `channels.matrix.historyLimit` contrôle combien de messages récents du salon sont inclus comme `InboundHistory` lorsqu’un message de salon Matrix déclenche l’agent. Repli sur `messages.groupChat.historyLimit` ; si les deux sont non définis, la valeur par défaut effective est `0`. Définissez `0` pour désactiver.
- L’historique de salon Matrix est limité au salon. Les DM continuent d’utiliser l’historique de session normal.
- L’historique de salon Matrix est uniquement en attente : OpenClaw met en mémoire tampon les messages de salon qui n’ont pas encore déclenché de réponse, puis prend un instantané de cette fenêtre lorsqu’une mention ou un autre déclencheur arrive.
...
- `contextVisibility: "allowlist"` filtre le contexte supplémentaire pour ne conserver que les expéditeurs autorisés par les vérifications de liste d’autorisation actives du salon/de l’utilisateur.
...
Pour faire taire entièrement les DM tout en gardant les salons fonctionnels, définissez `dm.enabled: false` :
...
Voir [Groupes](/fr/channels/groups) pour le comportement de blocage par mention et de liste d’autorisation.
...
Si un utilisateur Matrix non approuvé continue de vous envoyer des messages avant l’approbation, OpenClaw réutilise le même code d’appairage en attente et peut envoyer une réponse de rappel après un court délai de récupération au lieu d’émettre un nouveau code.

Voir [Appairage](/fr/channels/pairing) pour le flux d’appairage DM partagé et l’organisation du stockage.

## Réparation de salon direct

Si l’état des messages directs se désynchronise, OpenClaw peut se retrouver avec des mappages `m.direct` obsolètes qui pointent vers d’anciens salons solo au lieu du DM actif. Inspectez le mappage courant pour un pair :
...
- privilégie un DM strict 1:1 qui est déjà mappé dans `m.direct`
- se rabat sur n’importe quel DM strict 1:1 actuellement rejoint avec cet utilisateur
...
Matrix peut agir comme client d’approbation natif. Configurez sous `channels.matrix.execApprovals` (ou `channels.matrix.accounts.<account>.execApprovals` pour une surcharge par compte) :

- `enabled` : livrer les approbations via des invites natives Matrix. Lorsque non défini ou défini sur `"auto"`, Matrix s’active automatiquement dès qu’au moins un approbateur peut être résolu. Définissez `false` pour désactiver explicitement.
- `approvers` : identifiants utilisateur Matrix (`@owner:example.org`) autorisés à approuver les requêtes d’exécution. Facultatif - repli sur `channels.matrix.dm.allowFrom`.
...
Les commandes slash (`/new`, `/reset`, `/model`, `/focus`, `/unfocus`, `/agents`, `/session`, `/acp`, `/approve`, etc.) fonctionnent directement dans les DM. Dans les salons, OpenClaw reconnaît aussi les commandes préfixées par la propre mention Matrix du bot, de sorte que `@bot:server /new` déclenche le chemin de commande sans regex de mention personnalisée. Cela garde le bot réactif aux publications de style salon `@mention /command` qu’Element et les clients similaires émettent lorsqu’un utilisateur complète le bot avec la touche Tab avant de saisir la commande.

Les règles d’autorisation s’appliquent toujours : les expéditeurs de commandes doivent satisfaire les mêmes politiques de liste d’autorisation/propriétaire DM ou salon que les messages ordinaires.
...
l’autorisez explicitement par compte.
...
Cette option explicite n’autorise que les cibles privées/internes de confiance. Les homeservers publics en texte clair tels que
...
OpenClaw utilise le même paramètre de proxy pour le trafic Matrix d’exécution et les sondes d’état de compte.
...
lors de la configuration de cibles de distribution explicites, de tâches cron, de liaisons ou de listes d’autorisation.
OpenClaw conserve les clés de session internes sous une forme canonique pour le stockage, de sorte que ces clés en minuscules
ne sont pas une source fiable pour les ID de distribution Matrix.
...
- Les recherches de salons acceptent directement les ID de salon et les alias explicites. La recherche par nom de salon rejoint est effectuée au mieux et ne s’applique aux listes d’autorisation de salons à l’exécution que lorsque `dangerouslyAllowNameMatching: true` est défini.
- Si un nom de salon ne peut pas être résolu en ID ou alias, il est ignoré par la résolution de liste d’autorisation à l’exécution.

## Référence de configuration

Les champs utilisateur de type liste d’autorisation (`groupAllowFrom`, `dm.allowFrom`, `groups.<room>.users`) acceptent les ID utilisateur Matrix complets (le plus sûr). Les entrées utilisateur qui ne sont pas des ID sont ignorées par défaut. Si vous définissez `dangerouslyAllowNameMatching: true`, les correspondances exactes de noms d’affichage dans l’annuaire Matrix sont résolues au démarrage et chaque fois que la liste d’autorisation change pendant que le moniteur est en cours d’exécution ; les entrées qui ne peuvent pas être résolues sont ignorées à l’exécution.

Les clés de liste d’autorisation de salons (`groups`, `rooms` hérité) doivent être des ID de salon ou des alias. Les clés en nom de salon simple sont ignorées par défaut ; `dangerouslyAllowNameMatching: true` rétablit une recherche au mieux sur les noms de salons rejoints.
...
- `deviceName` : nom d’affichage de l’appareil utilisé au moment de la connexion par mot de passe.
- `avatarUrl` : URL d’avatar personnel stockée pour la synchronisation de profil et les mises à jour `profile set`.
...
- `startupVerificationCooldownHours` : délai de récupération avant la prochaine demande de démarrage automatique. Par défaut : `24`.
...
- `dm.threadReplies` : remplacement propre aux DM pour l’enfilage des réponses (`"off"`, `"inbound"`, `"always"`).
...
- `dangerouslyAllowNameMatching` : lorsque `true`, autorise la recherche dans l’annuaire Matrix par nom d’affichage pour les entrées de liste d’autorisation utilisateur et la recherche par nom de salon rejoint pour les clés de liste d’autorisation de salon. Préférez les ID complets `@user:server` et les ID de salon ou alias.
...
- `autoJoinAllowlist` : salons/alias autorisés lorsque `autoJoin` vaut `"allowlist"`. Les entrées d’alias sont résolues auprès du homeserver, et non à partir de l’état revendiqué par le salon invitant.
...
- `textChunkLimit` : taille des morceaux sortants en caractères lorsque `chunkMode: "length"`. Par défaut : `4000`.
...
- `groups` : carte de politiques par salon. L’identité de session utilise l’ID de salon stable après résolution. (`rooms` est un alias hérité.)
