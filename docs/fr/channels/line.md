...
- La vérification de la signature LINE dépend du corps (HMAC calculé sur le corps brut). OpenClaw applique donc une limite stricte de 64 Ko au corps avant authentification ainsi qu’un délai maximal de lecture avant la vérification.
- OpenClaw traite les événements de Webhook à partir des octets bruts vérifiés de la requête. Les valeurs de `req.body` transformées par un middleware en amont sont ignorées afin de préserver l’intégrité de la signature.
...
- `channels.line.allowFrom` : identifiants utilisateur LINE autorisés pour les messages directs ; `dmPolicy: "open"` exige `["*"]`
...
- Remplacements propres à chaque groupe : `channels.line.groups.<groupId>.allowFrom` (ainsi que `enabled`, `requireMention`, `systemPrompt`, `skills`)
- Les groupes d’accès statiques d’expéditeurs peuvent être référencés depuis `allowFrom`, `groupAllowFrom` et la valeur `allowFrom` de chaque groupe avec `accessGroup:<name>` ; consultez [Groupes d’accès](/fr/channels/access-groups).
...
- Les réponses diffusées en continu sont mises en mémoire tampon ; LINE reçoit des segments complets accompagnés d’une animation
...
- **Audio** : envoyé sous forme de messages audio LINE ; la durée par défaut est de 60 secondes, sauf si `channelData.line.durationMs` est défini.

Le type de média provient de `channelData.line.mediaKind` lorsqu’il est défini ; sinon, il est déduit
des autres options LINE ou du suffixe de fichier de l’URL, avec l’image comme valeur de repli.

Les URL des médias sortants doivent être des URL HTTPS publiques de 2000 caractères au maximum. OpenClaw
valide le nom d’hôte cible avant de transmettre l’URL à LINE et refuse les cibles de bouclage,
...
- **Erreurs de téléchargement des médias :** augmentez `channels.line.mediaMaxMb` si la taille du média dépasse la
  limite par défaut.
