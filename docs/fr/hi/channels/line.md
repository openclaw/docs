...
read_when:
    ...
    - Vous devez configurer LINE Webhook et les identifiants.
...
État : Plugin téléchargeable. Les messages directs, les discussions de groupe, les médias, les emplacements, les messages Flex,
...
Le Gateway répond à la vérification Webhook de LINE (GET) et accepte les événements entrants signés
(POST) immédiatement après la validation de la signature et de la charge utile ; le traitement par l’agent
se poursuit de façon asynchrone.
Si vous avez besoin d’un chemin personnalisé, définissez `channels.line.webhookPath` ou
`channels.line.accounts.<id>.webhookPath`, puis mettez l’URL à jour en conséquence.

Note de sécurité :

- La vérification de signature LINE dépend du corps (HMAC sur le corps brut), OpenClaw applique donc des limites de corps strictes avant authentification et un délai d’expiration avant la vérification.
- OpenClaw traite les événements Webhook depuis les octets bruts vérifiés de la requête. Les valeurs `req.body` transformées par des middlewares en amont sont ignorées pour préserver la sécurité de l’intégrité de la signature.
...
Les messages directs utilisent le jumelage par défaut. Les expéditeurs inconnus reçoivent un code de jumelage, et leurs
...
- Les groupes d’accès statiques d’expéditeur peuvent être référencés depuis `allowFrom`, `groupAllowFrom` et le `allowFrom` par groupe avec `accessGroup:<name>`.
...
- Les réponses en streaming sont mises en mémoire tampon ; pendant que l’agent travaille, LINE reçoit des fragments complets
  avec une animation de chargement.
...
- Les médias entrants sont enregistrés sous `~/.openclaw/media/inbound/` avant d’être transmis à l’agent,
  ce qui correspond au magasin média partagé utilisé par les autres Plugins de canal
  groupés.
...
Le Plugin LINE fournit également la commande `/card` pour les préréglages de messages Flex :
...
- Les liaisons ACP configurées et les sessions ACP actives liées à une conversation fonctionnent sur LINE comme les autres canaux de conversation.

Pour plus de détails, consultez [Agents ACP](/fr/tools/acp-agents).
...
Les URL des médias sortants doivent être des URL HTTPS publiques. OpenClaw valide le nom d’hôte cible avant de remettre l’URL à LINE et refuse les cibles loopback, link-local et de réseau privé.
...
- **La vérification Webhook échoue :** assurez-vous que l’URL Webhook est en HTTPS et que
...
- **Erreurs de téléchargement de médias :** si le média dépasse la limite par défaut, augmentez `channels.line.mediaMaxMb`.
