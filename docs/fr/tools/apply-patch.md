...
x-i18n:
    generated_at: "2026-07-12T03:21:47Z"
    model: gpt-5.6
...
L’outil accepte une seule chaîne `input` qui englobe une ou plusieurs opérations sur des fichiers :
...
- Les chemins du correctif peuvent être relatifs (par rapport au répertoire de l’espace de travail) ou absolus.
- `tools.exec.applyPatch.workspaceOnly` vaut `true` par défaut (limité à l’espace de travail). Définissez-le sur `false` uniquement si vous souhaitez intentionnellement que `apply_patch` écrive ou supprime des fichiers en dehors du répertoire de l’espace de travail.
- Utilisez `*** Move to:` dans un bloc `*** Update File:` pour renommer des fichiers.
- `*** End of File` indique une insertion uniquement en fin de fichier lorsque cela est nécessaire.
- Activé par défaut pour chaque modèle. Définissez `tools.exec.applyPatch.enabled: false`
  ...
  `tools.exec.applyPatch.allowModels` (accepte les identifiants bruts tels que `gpt-5.4` ou les
  identifiants complets tels que `openai/gpt-5.4`).
