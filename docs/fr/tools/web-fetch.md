...
x-i18n:
    generated_at: "2026-07-12T16:06:54Z"
    model: gpt-5.6
...
`web_fetch` effectue une simple requête HTTP GET et extrait le contenu lisible (conversion du HTML en
Markdown ou en texte). Il **n’exécute pas** JavaScript. Pour les sites qui dépendent fortement de JS ou
...
Tronque la sortie à ce nombre de caractères. Limité à `tools.web.fetch.maxCharsCap`.
</ParamField>
...
<Steps>
  <Step title="Récupération">
    Envoie une requête HTTP GET avec un User-Agent semblable à celui de Chrome et un en-tête
  ...
  <Step title="Solution de repli (facultative)">
    Si Readability échoue et qu’un fournisseur de récupération est disponible, réessaie par
    l’intermédiaire de ce fournisseur (par exemple, le mode de contournement des bots de Firecrawl).
  </Step>
  <Step title="Cache">
...
`web_fetch` émet une ligne de progression publique uniquement si la récupération est toujours en attente
après cinq secondes :
...
Les accès rapides au cache et les réponses réseau rapides se terminent avant le déclenchement du minuteur et
n’affichent donc jamais de ligne de progression. L’annulation de l’appel efface le minuteur. La
ligne de progression représente uniquement l’état de l’interface utilisateur du canal et ne contient jamais le contenu récupéré de la page.
  ...
  tools: {
    web: {
      fetch: {
        ...
        ssrfPolicy: {
          allowRfc2544BenchmarkRange: true, // activation explicite pour les proxys de confiance utilisant de fausses IP dans 198.18.0.0/15
          allowIpv6UniqueLocalRange: true, // activation explicite pour les proxys de confiance utilisant de fausses IP dans fc00::/7
        },
      },
    },
  },
...
[Firecrawl](/fr/tools/firecrawl) comme solution de repli pour contourner les bots et améliorer l’extraction :
...
```json5
{
  tools: {
    web: {
      fetch: {
        provider: "firecrawl", // facultatif ; omettez-le pour la détection automatique à partir des identifiants disponibles
      },
    },
  },
  plugins: {
    entries: {
      firecrawl: {
        ...
        config: {
          webFetch: {
            // apiKey: "fc-...", // facultatif ; omettez-la pour l’accès de démarrage sans clé
...
<Note>
  Si vous configurez une SecretRef de clé d’API Firecrawl qui ne peut pas être résolue et qu’aucune variable
  d’environnement `FIRECRAWL_API_KEY` de repli n’est disponible, le démarrage du Gateway échoue immédiatement.
</Note>
...
- Si `provider` est omis, OpenClaw détecte automatiquement le premier fournisseur de récupération Web
  prêt à l’emploi à partir des identifiants configurés. Les appels `web_fetch` non exécutés dans un bac à sable peuvent utiliser
  ...
  solution de repli.
- Les appels `web_fetch` exécutés dans un bac à sable autorisent les fournisseurs intégrés ainsi que les fournisseurs installés
  dont la provenance officielle npm ou ClawHub est vérifiée. Actuellement, cela autorise le
  plugin Firecrawl officiel ; les plugins externes tiers de récupération restent exclus.
...
Dans ce mode, OpenClaw applique toujours des contrôles SSRF fondés sur le nom d’hôte avant d’envoyer
la requête, mais laisse le proxy résoudre le DNS au lieu d’effectuer un épinglage DNS
...
- Le corps de la réponse est limité à `maxResponseBytes` (valeur par défaut : `750000`, bornée entre
...
- `tools.web.fetch.ssrfPolicy.allowRfc2544BenchmarkRange` et
  ...
  destinées aux piles de proxys de confiance utilisant de fausses IP ; laissez-les non définies sauf si votre proxy contrôle
...
- `useTrustedEnvProxy` nécessite une activation explicite et ne doit être activé que pour les
  proxys contrôlés par l’opérateur qui appliquent toujours la politique de trafic sortant après la résolution
  DNS
...
- [Navigateur Web](/fr/tools/browser) — automatisation complète du navigateur pour les sites qui dépendent fortement de JS
- [Firecrawl](/fr/tools/firecrawl) — outils de recherche et d’extraction de Firecrawl
