...
read_when:
    ...
    - Vous voulez comprendre la détection automatique et le repli du fournisseur
...
title: Recherche web
x-i18n:
    generated_at: "2026-05-02T07:22:50Z"
    model: gpt-5.5
    provider: openai
...
<Info>
  ...
  récupérer une URL spécifique, utilisez la [récupération Web](/fr/tools/web-fetch).
</Info>
...
<CardGroup cols={2}>
  ...
  <Card title="Kimi" icon="moon" href="/fr/tools/kimi-search">
    Réponses synthétisées par IA avec citations via la recherche Web Moonshot ; les replis vers une discussion non ancrée échouent explicitement.
  </Card>
  ...
  <Card title="SearXNG" icon="server" href="/fr/tools/searxng-search">
    Métarecherche auto-hébergée. Aucune clé d’API nécessaire. Agrège Google, Bing, DuckDuckGo et plus encore.
  </Card>
...
| [Kimi](/fr/tools/kimi-search)                | Synthétisé par IA + citations ; échoue sur les replis vers une discussion non ancrée | --                                               | `KIMI_API_KEY` / `MOONSHOT_API_KEY`                                                     |
...
Si la recherche native Codex est activée mais que le modèle actuel n’est pas compatible Codex, OpenClaw conserve le comportement `web_search` géré normal.
...
Si aucun fournisseur n’est détecté, il se replie sur Brave (vous obtiendrez une
erreur de clé manquante vous invitant à en configurer une).

<Note>
  Tous les champs de clé de fournisseur prennent en charge les objets SecretRef. Les SecretRef à portée Plugin
  sous `plugins.entries.<plugin>.config.webSearch.apiKey` sont résolus pour les
  fournisseurs de recherche Web inclus reposant sur une API, notamment Brave, Exa, Firecrawl,
  ...
  conserver plusieurs fournisseurs configurés sans payer le coût de résolution pour ceux
...
`models.providers.google.apiKey` et `models.providers.google.baseUrl` comme replis de priorité inférieure
après sa configuration dédiée à la recherche Web et `GEMINI_API_KEY`. Consultez les
pages des fournisseurs pour des exemples.

La sélection du fournisseur de repli de `web_fetch` est séparée :
...
- `web_fetch` non sandboxé peut utiliser les fournisseurs de plugins installés qui déclarent `contracts.webFetchProviders` ; les récupérations sandboxées restent limitées aux éléments intégrés
...
Il s’agit d’une étape de suivi distincte dans le parcours Grok, et non d’un choix séparé
de fournisseur de recherche web au niveau supérieur. Si vous choisissez un autre fournisseur, OpenClaw n’affiche pas
l’invite `x_search`.
...
| `language`            | Code langue ISO 639-1 (p. ex. "en", "de")             |
...
<Warning>
  ...
  de fraîcheur personnalisées de Brave exigent une date de début et une date de fin.
  ...
  SearXNG accepte `http://` uniquement pour les hôtes de réseau privé ou local loopback approuvés ;
...
<Note>
  La documentation xAI indique que `x_search` prend en charge la recherche par mots-clés, la recherche sémantique, la recherche d’utilisateurs
  ...
  ou de l’ID de statut. Les recherches larges par mots-clés peuvent trouver la bonne publication, mais renvoyer des métadonnées
  par publication moins complètes. Un bon schéma consiste à localiser d’abord la publication, puis
  à lancer une seconde requête `x_search` centrée sur cette publication exacte.
</Note>
...
| `enable_image_understanding` | Laisser xAI inspecter les images jointes aux publications correspondantes |
| `enable_video_understanding` | Laisser xAI inspecter les vidéos jointes aux publications correspondantes |
