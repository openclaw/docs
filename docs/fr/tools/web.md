...
<Info>
  ...
  les sites fortement basés sur JS ou les connexions, utilisez le [navigateur web](/fr/tools/browser). Pour
...
<Steps>
  ...
  <Step title="Configurer">
    ...
    d’environnement (par exemple `BRAVE_API_KEY`) et ignorer cette étape pour les fournisseurs adossés à une API.
  </Step>
...
<CardGroup cols={2}>
  <Card title="Brave Search" icon="shield" href="/fr/tools/brave-search">
    Résultats structurés avec extraits. Prend en charge le mode `llm-context` et les filtres de pays/langue. Palier gratuit disponible.
  </Card>
  ...
  <Card title="Kimi" icon="moon" href="/fr/tools/kimi-search">
    Réponses synthétisées par IA avec citations via la recherche web Moonshot ; les solutions de repli de discussion non ancrées échouent explicitement.
  </Card>
  ...
  <Card title="SearXNG" icon="server" href="/fr/tools/searxng-search">
    Métarecherche auto-hébergée. Aucune clé API nécessaire. Agrège Google, Bing, DuckDuckGo et d’autres sources.
  </Card>
...
| [Kimi](/fr/tools/kimi-search)                | Synthétisé par IA + citations ; échoue sur les solutions de repli de discussion non ancrées | --                                               | `KIMI_API_KEY` / `MOONSHOT_API_KEY`                                                     |
...
Si aucun fournisseur n’est détecté, il revient à Brave (vous recevrez une erreur de clé manquante
...
`tools.web.search.provider` est validé par rapport aux ids de fournisseurs de recherche web
déclarés par les manifestes de Plugins intégrés et installés. Une faute de frappe comme `"brvae"`
...
La sélection du fournisseur de secours `web_fetch` est séparée :

- choisissez-le avec `tools.web.fetch.provider`
- ou omettez ce champ et laissez OpenClaw auto-détecter le premier fournisseur
  web-fetch prêt à partir des identifiants disponibles
...
| `count`               | Résultats à retourner (1-10, valeur par défaut : 5)                  |
...
<Warning>
  ...
  fraîcheur personnalisées de Brave exigent une date de début et une date de fin.
  Gemini, Grok et Kimi retournent une seule réponse synthétisée avec citations. Ils
  acceptent `count` pour la compatibilité avec l’outil partagé, mais cela ne change pas la forme
  de la réponse ancrée. Gemini prend en charge `freshness`, `date_after` et
  `date_before` en les convertissant en plages temporelles de grounding Google Search.
...
des filtres structurés facultatifs. OpenClaw n’active l’outil intégré xAI `x_search`
...
<Note>
  ...
  d’utilisateurs et la récupération de fils. Pour les statistiques d’engagement par publication, comme les republications,
  les réponses, les favoris ou les vues, privilégiez une recherche ciblée sur l’URL exacte de la publication
  ou l’ID de statut. Les recherches larges par mots-clés peuvent trouver la bonne publication, mais retourner des métadonnées
  par publication moins complètes. Un bon modèle consiste à : localiser d’abord la publication, puis
