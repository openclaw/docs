...
read_when:
    ...
    - Vous souhaitez comprendre la détection automatique et le repli du fournisseur
...
<CardGroup cols={2}>
  <Card title="Brave Search" icon="shield" href="/fr/tools/brave-search">
    Résultats structurés avec extraits. Prend en charge le mode `llm-context`, les filtres de pays et de langue. Offre gratuite disponible.
  </Card>
  <Card title="DuckDuckGo" icon="bird" href="/fr/tools/duckduckgo-search">
    Solution de secours sans clé. Aucune clé d’API requise. Intégration non officielle basée sur HTML.
  </Card>
  <Card title="Exa" icon="brain" href="/fr/tools/exa-search">
    Recherche neuronale + par mots-clés avec extraction de contenu (passages importants, texte, résumés).
  </Card>
  ...
  <Card title="Kimi" icon="moon" href="/fr/tools/kimi-search">
    Réponses synthétisées par IA avec citations via la recherche web Moonshot ; les solutions de secours de chat non ancrées échouent explicitement.
  </Card>
...
| [Kimi](/fr/tools/kimi-search)                | Synthétisé par IA + citations ; échoue sur les solutions de secours de chat non ancrées | --                                               | `KIMI_API_KEY` / `MOONSHOT_API_KEY`                                                     |
...
- Il s’active uniquement pour les modèles compatibles Codex (`openai-codex/*` ou les fournisseurs utilisant `api: "openai-codex-responses"`)
...
les hôtes d’API fournisseurs de confiance, OpenClaw autorise les réponses DNS fake-IP
Surge, Clash et sing-box dans `198.18.0.0/15` et `fc00::/7` uniquement pour ce nom d’hôte fournisseur.
...
Les listes de fournisseurs dans les docs et les flux de configuration sont alphabétiques. La détection automatique conserve un
...
Fournisseurs adossés à une API en premier :
...
Solutions de secours sans clé ensuite :

10. **DuckDuckGo** -- solution de secours HTML sans clé, sans compte ni clé d’API (ordre 100)
11. **Ollama Web Search** -- solution de secours sans clé via votre hôte Ollama local configuré lorsqu’il est joignable et connecté avec `ollama signin` ; peut réutiliser l’authentification par porteur du fournisseur Ollama lorsque l’hôte en a besoin, et peut appeler la recherche directe `https://ollama.com` lorsqu’elle est configurée avec `OLLAMA_API_KEY` (ordre 110)
12. **SearXNG** -- `SEARXNG_BASE_URL` ou `plugins.entries.searxng.config.webSearch.baseUrl` (ordre 200)

Si aucun fournisseur n’est détecté, le système revient à Brave (vous obtiendrez une erreur de
clé manquante vous invitant à en configurer une).
...
<Note>
  ...
  fournisseurs de recherche web adossés à une API intégrés, notamment Brave, Exa, Firecrawl,
  ...
  sélectionné par détection automatique. En mode détection automatique, OpenClaw résout uniquement la
...
`models.providers.google.apiKey` et `models.providers.google.baseUrl` comme solutions de repli de priorité inférieure
après sa configuration de recherche web dédiée et `GEMINI_API_KEY`. Consultez les
...
déclarés par les manifestes des plugins groupés et installés. Une faute de frappe comme `"brvae"`
fait échouer la validation de la configuration au lieu de revenir silencieusement à l’auto-détection. Si un
fournisseur configuré ne dispose que d’indices de plugin obsolètes, par exemple un bloc
`plugins.entries.<plugin>` restant après la désinstallation d’un plugin tiers,
OpenClaw garde un démarrage résilient et signale un avertissement afin que vous puissiez réinstaller le
...
Il s’agit d’une étape de suivi séparée dans le parcours Grok, et non d’un choix de fournisseur
de recherche web de premier niveau distinct. Si vous choisissez un autre fournisseur, OpenClaw n’affiche pas
l’invite `x_search`.
...
<Warning>
  ...
  rejette `ui_lang` ; `date_before` nécessite aussi `date_after` car les plages de
  fraîcheur personnalisées de Brave requièrent à la fois des dates de début et de fin.
  Gemini, Grok et Kimi renvoient une réponse synthétisée avec citations. Ils
  acceptent `count` pour la compatibilité avec l’outil partagé, mais cela ne modifie pas la
  forme de la réponse fondée. Gemini prend en charge `freshness`, `date_after` et
...
des filtres structurés facultatifs. OpenClaw n’active l’outil intégré `x_search`
de xAI que sur la requête qui sert cet appel d’outil.

<Note>
  La documentation xAI indique que `x_search` prend en charge la recherche par mots-clés, la recherche sémantique, la recherche
  d’utilisateur et la récupération de fil. Pour les statistiques d’engagement par publication comme les republications,
  réponses, signets ou vues, privilégiez une recherche ciblée sur l’URL exacte de la publication
  ou l’identifiant de statut. Les recherches larges par mots-clés peuvent trouver la bonne publication, mais renvoyer des
  métadonnées par publication moins complètes. Un bon schéma consiste à localiser d’abord la publication, puis
  à exécuter une seconde requête `x_search` centrée sur cette publication exacte.
</Note>
...
il se replie sur `plugins.entries.xai.config.webSearch.baseUrl`, puis sur l’ancien
`tools.web.search.grok.baseUrl`, et enfin sur le point de terminaison public xAI.
