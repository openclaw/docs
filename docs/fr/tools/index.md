...
x-i18n:
    generated_at: "2026-05-03T21:39:48Z"
    model: gpt-5.5
    provider: openai
...
<Steps>
  <Step title="Les outils sont ce que l’agent appelle">
    ...
    `web_search`, `message`). OpenClaw fournit un ensemble d’**outils intégrés** et les
    plugins peuvent en enregistrer d’autres.
  ...
  <Step title="Les plugins regroupent tout">
    Un plugin est un paquet qui peut enregistrer n’importe quelle combinaison de fonctionnalités :
...
| `apply_patch`                              | Correctifs de fichiers à plusieurs blocs                             | [Apply Patch](/fr/tools/apply-patch)                            |
| `message`                                  | Envoyer des messages sur tous les canaux                             | [Envoi par l’agent](/fr/tools/agent-send)                       |
| `canvas`                                   | Piloter Canvas de Node (présenter, évaluer, instantané)              |                                                              |
...
Pour le travail sur les images, utilisez `image` pour l’analyse et `image_generate` pour la génération ou l’édition. Si vous ciblez `openai/*`, `google/*`, `fal/*` ou un autre fournisseur d’images non par défaut, configurez d’abord l’authentification/la clé d’API de ce fournisseur.

Pour le travail sur la musique, utilisez `music_generate`. Si vous ciblez `google/*`, `minimax/*` ou un autre fournisseur de musique non par défaut, configurez d’abord l’authentification/la clé d’API de ce fournisseur.

Pour le travail sur la vidéo, utilisez `video_generate`. Si vous ciblez `qwen/*` ou un autre fournisseur vidéo non par défaut, configurez d’abord l’authentification/la clé d’API de ce fournisseur.
...
`session_status` est l’outil léger d’état/de retour de lecture dans le groupe des sessions.
...
remplacement. Comme `/status`, il peut rétroremplir les compteurs clairsemés de tokens/cache et
le libellé du modèle d’exécution actif depuis la dernière entrée d’utilisation de la transcription.

`gateway` est l’outil d’exécution réservé aux propriétaires pour les opérations de Gateway :
...
Pour les modifications partielles, privilégiez `config.schema.lookup` puis `config.patch`. Utilisez
`config.apply` uniquement lorsque vous remplacez volontairement toute la configuration.
...
Les outils de plugins sont toujours écrits avec `api.registerTool(...)` et déclarés dans
...
| ----------- | ------------------------------------------------------------------------------------------------------------------------------------------------- |
| `full`      | Tous les outils core et de plugins facultatifs ; base sans restriction pour un accès plus large aux commandes/contrôles                           |
| `coding`    | `group:fs`, `group:runtime`, `group:web`, `group:sessions`, `group:memory`, `cron`, `image`, `image_generate`, `music_generate`, `video_generate` |
| `messaging` | `group:messaging`, `sessions_list`, `sessions_history`, `sessions_send`, `session_status`                                                         |
| `minimal`   | `session_status` uniquement                                                                                                                       |

<Note>
`tools.profile: "messaging"` est volontairement étroit pour les agents axés sur les canaux.
...
l’accès avec `tools.allow` / `tools.deny` lorsque nécessaire.
</Note>
...
d’appels d’outil en texte brut (y compris `<tool_call>...</tool_call>`,
`<function_call>...</function_call>`, `<tool_calls>...</tool_calls>`,
`<function_calls>...</function_calls>`, et les blocs d’appels d’outil tronqués),
la structure d’appels d’outil rétrogradée, les jetons de contrôle de modèle
ASCII/pleine largeur divulgués, et le XML d’appels d’outil MiniMax mal formé du texte
de l’assistant, puis applique la censure/troncature et de possibles espaces réservés
pour les lignes surdimensionnées, au lieu d’agir comme un vidage brut de transcript.
