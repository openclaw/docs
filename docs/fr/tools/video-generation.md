...
x-i18n:
    generated_at: "2026-04-25T14:00:11Z"
    model: gpt-5.4
    provider: openai
...
Les agents OpenClaw peuvent générer des vidéos à partir de prompts textuels, d’images de référence ou de vidéos existantes. Quatorze backends fournisseurs sont pris en charge, chacun avec des options de modèle, des modes d’entrée et des ensembles de fonctionnalités différents. L’agent choisit automatiquement le bon fournisseur en fonction de votre configuration et des clés API disponibles.
...
- `generate` pour les demandes texte-vers-vidéo sans média de référence
...
Pendant qu’une tâche est en cours, les appels `video_generate` en double dans la même session renvoient l’état actuel de la tâche au lieu de lancer une nouvelle génération. Utilisez `openclaw tasks list` ou `openclaw tasks show <taskId>` pour vérifier la progression depuis la CLI.
...
fournisseur renvoie des octets. Le plafond par défaut d’enregistrement des vidéos générées suit la limite des médias vidéo,
et `agents.defaults.mediaMaxMb` l’augmente pour les rendus plus volumineux.
...
| OpenAI      | Oui        | Oui            | Oui            | `generate`, `imageToVideo` ; `videoToVideo` partagé ignoré car ce chemin d’organisation/d’entrée exige actuellement un accès inpaint/remix côté fournisseur |
...
| --------- | ------ | ----------------------------------------------------------------------------- |
| `prompt`  | string | Description textuelle de la vidéo à générer (requise pour `action: "generate"`) |
...
| `imageRoles` | string[] | Indications de rôle facultatives par position, parallèles à la liste combinée des images. Valeurs canoniques : `first_frame`, `last_frame`, `reference_image` |
| `video`      | string   | Vidéo de référence unique (chemin ou URL)                                                                                                |
| `videos`     | string[] | Plusieurs vidéos de référence (jusqu’à 4)                                                                                                |
| `videoRoles` | string[] | Indications de rôle facultatives par position, parallèles à la liste combinée des vidéos. Valeur canonique : `reference_video`          |
...
| `audioRoles` | string[] | Indications de rôle facultatives par position, parallèles à la liste combinée des audios. Valeur canonique : `reference_audio`          |

Les indications de rôle sont transmises au fournisseur telles quelles. Les valeurs canoniques proviennent
...
liste de références correspondante ; les erreurs de décalage d’un index échouent avec une erreur claire.
...
| `audio`           | boolean | Active l’audio généré dans la sortie lorsque pris en charge. Distinct de `audioRef*` (entrées) |
| `watermark`       | boolean | Active ou désactive le filigrane du fournisseur lorsque pris en charge                  |
...
`details.ignoredOverrides` dans le résultat de l’outil afin que cet abandon soit visible.
...
| `model`           | string | Remplacement du fournisseur/modèle (par ex. `runway/gen4.5`)                                                                                                                                                                                                                                                                                         |
...
| `providerOptions` | object | Options spécifiques au fournisseur sous forme d’objet JSON (par ex. `{"seed": 42, "draft": true}`). Les fournisseurs qui déclarent un schéma typé valident les clés et les types ; les clés inconnues ou incompatibles font ignorer le candidat pendant le repli. Les fournisseurs sans schéma déclaré reçoivent les options telles quelles. Exécutez `video_generate action=list` pour voir ce que chaque fournisseur accepte |

Tous les fournisseurs ne prennent pas en charge tous les paramètres. OpenClaw normalise déjà la durée vers la valeur prise en charge la plus proche par le fournisseur, et il remappe aussi les indications géométriques traduites telles que size-to-aspect-ratio lorsqu’un fournisseur de repli expose une surface de contrôle différente. Les remplacements réellement non pris en charge sont ignorés au mieux et signalés comme avertissements dans le résultat de l’outil. Les limites strictes de capacité (comme un trop grand nombre d’entrées de référence) échouent avant l’envoi.

Les résultats de l’outil indiquent les paramètres appliqués. Lorsque OpenClaw remappe la durée ou la géométrie pendant un repli de fournisseur, les valeurs renvoyées `durationSeconds`, `size`, `aspectRatio` et `resolution` reflètent ce qui a été soumis, et `details.normalization` capture la traduction entre la valeur demandée et la valeur appliquée.
...
Certaines vérifications de capacité sont appliquées au niveau du repli plutôt qu’à la frontière
de l’outil afin qu’une demande qui dépasse les limites du fournisseur principal
...
- Si la demande contient `providerOptions` et que le candidat actif
  ...
  options telles quelles (transmission compatible avec l’existant). Un fournisseur peut
  explicitement refuser toutes les options fournisseur en déclarant un schéma vide
  (`capabilities.providerOptions: {}`), ce qui entraîne le même comportement d’ignorance qu’une
  incompatibilité de type.

La première raison d’ignorance dans une demande est journalisée au niveau `warn` afin que les opérateurs voient
quand leur fournisseur principal a été ignoré ; les ignorances suivantes sont journalisées au niveau
`debug` pour garder les longues chaînes de repli discrètes. Si tous les candidats sont ignorés,
l’erreur agrégée inclut la raison d’ignorance pour chacun.

## Actions

- **generate** (par défaut) -- crée une vidéo à partir du prompt donné et d’entrées de référence facultatives.
...
4. **Détection automatique** -- utilise les fournisseurs disposant d’une authentification valide, en commençant par le fournisseur par défaut actuel, puis les fournisseurs restants par ordre alphabétique.
...
<AccordionGroup>
  ...
  <Accordion title="BytePlus Seedance 1.5">
    Nécessite le Plugin [`@openclaw/byteplus-modelark`](https://www.npmjs.com/package/@openclaw/byteplus-modelark). Identifiant du fournisseur : `byteplus-seedance15`. Modèle : `seedance-1-5-pro-251215`.

    Utilise l’API unifiée `content[]`. Prend en charge au maximum 2 images d’entrée (`first_frame` + `last_frame`). Toutes les entrées doivent être des URL distantes `https://`. Définissez `role: "first_frame"` / `"last_frame"` sur chaque image, ou passez les images positionnellement.
  ...
  <Accordion title="BytePlus Seedance 2.0">
    Nécessite le Plugin [`@openclaw/byteplus-modelark`](https://www.npmjs.com/package/@openclaw/byteplus-modelark). Identifiant du fournisseur : `byteplus-seedance2`. Modèles : `dreamina-seedance-2-0-260128`, `dreamina-seedance-2-0-fast-260128`.
  ...
  <Accordion title="fal">
    Utilise un flux adossé à une file d’attente pour les tâches longues. Une seule image de référence uniquement.
  </Accordion>
  ...
  <Accordion title="Runway">
    Prend en charge les fichiers locaux via des URI de données. La vidéo-vers-vidéo exige `runway/gen4_aleph`. Les exécutions texte seul exposent les ratios `16:9` et `9:16`.
  </Accordion>
  ...
  <Accordion title="Vydra">
    Utilise directement `https://www.vydra.ai/api/v1` pour éviter les redirections qui font perdre l’authentification. `veo3` est intégré comme texte-vers-vidéo uniquement ; `kling` exige une URL d’image distante.
  </Accordion>

  <Accordion title="xAI">
    Prend en charge les flux texte-vers-vidéo, image-vers-vidéo et édition/extension vidéo distante.
  </Accordion>
</AccordionGroup>
...
capacités spécifiques à chaque mode au lieu de seulement des limites globales plates. Les nouvelles implémentations
de fournisseurs doivent privilégier des blocs de mode explicites :
...
pour annoncer la prise en charge des modes de transformation. Les fournisseurs doivent déclarer
...
par défaut les clés API live/env par rapport aux profils d’authentification stockés, et exécute par défaut un smoke compatible avec une publication :
