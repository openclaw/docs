...
read_when:
    ...
    - Configuration des fournisseurs et modèles de génération de vidéos
...
x-i18n:
    generated_at: "2026-05-06T07:43:16Z"
    model: gpt-5.5
    provider: openai
...
OpenClaw agents peut générer des vidéos à partir de prompts textuels, d’images de référence ou de
vidéos existantes. Seize backends de fournisseurs sont pris en charge, chacun avec
différentes options de modèle, différents modes d’entrée et différents ensembles de fonctionnalités. L’agent choisit le
bon fournisseur automatiquement en fonction de votre configuration et des clés API
disponibles.

<Note>
L’outil `video_generate` apparaît uniquement lorsqu’au moins un fournisseur de génération de vidéos
...
4. L’agent informe l’utilisateur et joint la vidéo terminée. Dans les discussions de groupe/canal
   qui utilisent une livraison visible uniquement par outil de message, l’agent relaie le
...
En dehors des exécutions d’agent adossées à une session (par exemple, les invocations directes d’outils),
...
liste de références correspondante ; les erreurs de décalage d’un élément échouent avec une erreur claire.
...
rôle ou utilisez `first_frame` pour la conversion image-vers-vidéo avec une seule image.
</Note>
...
traduction demandée-vers-appliquée.
</Note>
...
Le mélange de références image et vidéo n’est pas une surface de capacité partagée stable.
Préférez un seul type de référence par requête.

#### Fallback et options typées

Certaines vérifications de capacité sont appliquées au niveau de fallback plutôt qu’à la
limite de l’outil, de sorte qu’une requête qui dépasse les limites du fournisseur principal peut
encore s’exécuter sur un fallback capable :
...
4. **Détection automatique** - les fournisseurs qui disposent d’une authentification valide, en commençant par le
   fournisseur par défaut actuel, puis les fournisseurs restants par ordre
   alphabétique.
...
<AccordionGroup>
  ...
  <Accordion title="ComfyUI">
    Exécution locale ou dans le cloud pilotée par workflow. Prend en charge le texte-vers-vidéo et
  ...
  <Accordion title="fal">
    ...
    minutes par défaut avant de considérer une tâche de file fal en cours comme expirée. La plupart des modèles vidéo fal
...
FAL est facultatif, car la latence de file côté fournisseur peut dominer le
temps de release :
...
- `videoToVideo` lorsque `capabilities.videoToVideo.enabled` et que le
  fournisseur/modèle accepte une entrée vidéo locale adossée à un buffer dans le sweep
  partagé.
