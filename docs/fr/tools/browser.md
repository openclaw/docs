...
- Le profil `user` intégré s’attache à votre vraie session Chrome connectée via Chrome MCP.
...
- Des actions d’agent (clic/saisie/glisser/sélection), des instantanés, des captures d’écran, des PDF.
- Une Skills `browser-automation` fournie qui enseigne aux agents la boucle de récupération des instantanés,
  des onglets stables, des références obsolètes et des blocages manuels lorsque le Plugin de navigateur
  est activé.
...
sous-agent généré doit utiliser l’automatisation de navigateur, ajoutez browser au niveau du profil :
...
Le Plugin de navigateur fournit deux niveaux de consignes pour l’agent :
...
- La description de l’outil `browser` porte le contrat compact toujours actif : choisir
  ...
  des onglets et charger la Skills du navigateur pour les tâches en plusieurs étapes.
- La Skills `browser-automation` fournie porte la boucle d’exploitation plus longue :
...
Plugin est activé. Les instructions complètes de la Skills sont chargées à la demande, afin que les tours
...
Un bloc racine `browser` explicite, par exemple `browser.enabled=true` ou `browser.profiles.<name>`, active le Plugin de navigateur fourni même avec un `plugins.allow` restrictif, ce qui correspond au comportement de configuration des canaux. `plugins.entries.browser.enabled=true` et `tools.alsoAllow: ["browser"]` ne remplacent pas à eux seuls l’appartenance à la liste d’autorisation. Supprimer entièrement `plugins.allow` restaure également le comportement par défaut.

## Profils : `openclaw` et `user`
...
- Préférez `profile="user"` lorsque des sessions déjà connectées comptent et que l’utilisateur
...
- Le service de contrôle se lie au loopback sur un port dérivé de `gateway.port` (par défaut `18791` = Gateway + 2). Surcharger `gateway.port` ou `OPENCLAW_GATEWAY_PORT` décale les ports dérivés dans la même famille.
...
- `localLaunchTimeoutMs` est le budget accordé à un processus Chrome géré lancé localement
  ...
  démarre lentement. Les valeurs doivent être des entiers positifs jusqu’à `120000` ms ; les valeurs
  de configuration invalides sont rejetées.
- Les échecs répétés de lancement/disponibilité de Chrome géré sont soumis à un coupe-circuit par
  profil. Après plusieurs échecs consécutifs, OpenClaw suspend brièvement les nouvelles tentatives de lancement
  au lieu de générer Chromium à chaque appel d’outil de navigateur. Corrigez
...
- Les variables d’environnement Gateway/provider `HTTP_PROXY`, `HTTPS_PROXY`, `ALL_PROXY` et `NO_PROXY` ne proxyfient pas automatiquement le navigateur géré par OpenClaw. Chrome géré est lancé en direct par défaut afin que les paramètres de proxy du provider n’affaiblissent pas les vérifications SSRF du navigateur.
- Pour proxyfier le navigateur géré lui-même, passez des indicateurs de proxy Chrome explicites via `browser.extraArgs`, comme `--proxy-server=...` ou `--proxy-pac-url=...`. Le mode SSRF strict bloque le routage explicite par proxy du navigateur sauf si l’accès navigateur au réseau privé est activé intentionnellement.
...
- `POST /start?headless=true` et `openclaw browser start --headless` demandent un
  ...
  `browser.headless` ni la configuration du profil. Les profils à session existante, attach-only et
  CDP distants rejettent la surcharge parce qu’OpenClaw ne lance pas ces
...
- Sur les hôtes Linux sans `DISPLAY` ni `WAYLAND_DISPLAY`, les profils locaux gérés
  ...
  du profil/globale ne choisissent explicitement le mode avec interface graphique. `openclaw browser status --json`
...
- `OPENCLAW_BROWSER_HEADLESS=1` force les lancements locaux gérés en mode headless pour le
  ...
  une requête explicite `start --headless` l’emporte tout de même pour ce lancement ponctuel.
- `executablePath` peut être défini globalement ou par profil local géré. Les valeurs par profil remplacent `browser.executablePath`, de sorte que différents profils gérés peuvent lancer différents navigateurs basés sur Chromium. Les deux formes acceptent `~` pour le répertoire personnel de votre OS.
- `color` (au niveau supérieur et par profil) teinte l’interface utilisateur du navigateur pour que vous puissiez voir quel profil est actif.
...
- `driver: "existing-session"` utilise Chrome DevTools MCP au lieu de CDP brut. Ne définissez pas `cdpUrl` pour ce driver.
- Définissez `browser.profiles.<name>.userDataDir` lorsqu’un profil existing-session doit s’attacher à un profil utilisateur Chromium non par défaut (Brave, Edge, etc.). Ce chemin accepte aussi `~` pour le répertoire personnel de votre OS.
...
- Pour les services CDP gérés extérieurement sur loopback (par exemple Browserless dans
  Docker publié sur `127.0.0.1`), définissez aussi `attachOnly: true`. Le CDP loopback
...
- profils attach-only et CDP distants : `openclaw browser stop` ferme la session de
  ...
  palette de couleurs, locale, fuseau horaire, mode hors ligne et état similaire), même
...
acheminer automatiquement les appels d’outils du navigateur vers ce Node sans configuration de navigateur supplémentaire.
...
- Si vous définissez `nodeHost.browserProxy.allowProfiles`, OpenClaw le traite comme une frontière de moindre privilège : seuls les profils figurant dans la liste d’autorisation peuvent être ciblés, et les routes persistantes de création/suppression de profils sont bloquées sur la surface du proxy.
...
Browserless comme un service CDP géré extérieurement :
...
définissez `EXTERNAL` de Browserless sur cette même base WebSocket publique vers OpenClaw, comme
`ws://127.0.0.1:3000`, `ws://browserless:3000` ou une adresse réseau Docker
privée stable. Si `/json/version` renvoie `webSocketDebuggerUrl` pointant vers
une adresse qu’OpenClaw ne peut pas atteindre, le CDP HTTP peut sembler sain tandis que l’attachement
...
et peut indiquer que le port est utilisé mais n’appartient pas à OpenClaw.
...
- **Racines WebSocket nues** - `ws://host[:port]` ou `wss://host[:port]` sans
  ...
  se rabat sur une négociation WebSocket directe à la racine nue. Si le point de terminaison
  WebSocket annoncé rejette la négociation CDP mais que la racine nue configurée
  l’accepte, OpenClaw se rabat également sur cette racine. Cela permet à un simple `ws://`
  pointant vers un Chrome local de toujours se connecter, puisque Chrome n’accepte les mises à niveau
  WebSocket que sur le chemin spécifique par cible issu de `/json/version`, tandis que les fournisseurs
...
des navigateurs headless avec résolution de CAPTCHA intégrée, mode furtif et proxys
résidentiels.
...
- L’offre gratuite autorise une session simultanée et une heure de navigateur par mois.
  Consultez la [tarification](https://www.browserbase.com/pricing) pour les limites des forfaits payants.
...
- Le contrôle du navigateur est limité au loopback ; les flux d’accès passent par l’authentification du Gateway ou l’appairage de nœud.
- L’API HTTP autonome du navigateur en loopback utilise **uniquement une authentification par secret partagé** :
  l’authentification bearer par jeton du gateway, `x-openclaw-password`, ou l’authentification HTTP Basic avec le
...
- Si le contrôle du navigateur est activé et qu’aucune authentification par secret partagé n’est configurée, OpenClaw
  génère un jeton de gateway uniquement valable à l’exécution pour ce démarrage. Configurez
...
- Gardez le Gateway et tous les hôtes de nœud sur un réseau privé (Tailscale) ; évitez l’exposition publique.
...
- Le profil `user` est intégré pour l’attachement de session existante avec Chrome MCP.
...
Optionnel : créez votre propre profil personnalisé de session existante si vous voulez un
...
`~` s’étend vers le répertoire personnel de votre OS :
...
- Si vous utilisez un profil personnalisé de session existante, passez ce nom de profil explicite.
...
- La session existante peut s’attacher sur l’hôte sélectionné ou via un nœud de navigateur connecté.
  Si Chrome se trouve ailleurs et qu’aucun nœud de navigateur n’est connecté, utilisez
...
versions épinglées, binaires vendoriés) :

| Champ        | Ce qu’il fait                                                                                                             |
| ------------ | -------------------------------------------------------------------------------------------------------------------------- |
| `mcpCommand` | Exécutable à lancer à la place de `npx`. Résolu tel quel ; les chemins absolus sont respectés.                            |
| `mcpArgs`    | Tableau d’arguments transmis textuellement à `mcpCommand`. Remplace les arguments par défaut `chrome-devtools-mcp@latest --autoConnect`. |
...
- **Attente / téléversement / dialogue** - `wait --url` prend en charge les motifs exacts, de sous-chaîne et glob ; `wait --load networkidle` n’est pas pris en charge. Les hooks de téléversement nécessitent `ref` ou `inputRef`, un fichier à la fois, pas de `element` CSS. Les hooks de dialogue ne prennent pas en charge les remplacements de délai d’expiration.
- **Fonctionnalités uniquement gérées** - les actions par lot, l’export PDF, l’interception des téléchargements et `responsebody` nécessitent toujours le chemin de navigateur géré.
...
- **Contrôle déterministe des onglets** : `tabs` renvoie d’abord `suggestedTargetId`, puis
  des handles `tabId` stables comme `t1`, des libellés optionnels et le `targetId` brut.
...
uniquement en loopback** ainsi qu’une CLI `openclaw browser` correspondante (snapshots, refs, attentes
améliorées, sortie JSON, flux de débogage). Consultez
...
- Si `start` réussit mais que `tabs` échoue, le plan de contrôle est encore malsain. Traitez cela comme un problème d’accessibilité CDP, pas comme un problème de navigation de page.
...
- La configuration du navigateur utilise par défaut un objet de politique SSRF en mode échec fermé, même lorsque vous ne configurez pas `browser.ssrfPolicy`.
- Pour le profil géré `openclaw` en local loopback, les vérifications de santé CDP ignorent intentionnellement l’application de l’accessibilité SSRF du navigateur pour le plan de contrôle local propre à OpenClaw.
...
- Préférez des exceptions d’hôtes étroites comme `hostnameAllowlist` ou `allowedHostnames` à un accès large au réseau privé.
- Utilisez `dangerouslyAllowPrivateNetwork: true` uniquement dans des environnements intentionnellement fiables où l’accès du navigateur au réseau privé est requis et validé.
...
- `browser screenshot` capture les pixels (page complète, élément ou références libellées).
...
- `browser` accepte :
  ...
  - Dans les sessions isolées, `target: "host"` nécessite `agents.defaults.sandbox.browser.allowHostControl=true`.
  - Si `target` est omis : les sessions isolées utilisent `sandbox` par défaut, les sessions non isolées utilisent `host` par défaut.
  - Si un nœud compatible avec le navigateur est connecté, l’outil peut s’y acheminer automatiquement, sauf si vous fixez `target="host"` ou `target="node"`.
...
- [Isolation](/fr/gateway/sandboxing) - contrôle du navigateur dans les environnements isolés
