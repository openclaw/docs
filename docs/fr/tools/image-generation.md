...
summary: Générez et modifiez des images avec image_generate sur OpenAI, Google, fal, Microsoft Foundry, MiniMax, ComfyUI, DeepInfra, OpenRouter, LiteLLM, xAI et Vydra
title: Génération d’images
x-i18n:
    generated_at: "2026-07-12T15:53:26Z"
    model: gpt-5.6
...
fournisseurs que vous avez configurés. Dans les sessions de discussion, il
s’exécute de manière asynchrone : OpenClaw enregistre une tâche en arrière-plan,
renvoie immédiatement l’identifiant de la tâche et réveille l’agent lorsque
le fournisseur a terminé. L’agent d’achèvement suit le mode normal de réponse
...
messagerie. Si la session à l’origine de la demande est inactive ou si son
réveil actif échoue, OpenClaw envoie directement un résultat de secours
idempotent contenant les images générées afin que le résultat ne soit pas
perdu.

<Note>
L’outil n’apparaît que lorsqu’au moins un fournisseur de génération d’images
est disponible. Si `image_generate` ne figure pas parmi les outils de votre
agent, configurez `agents.defaults.imageGenerationModel`, définissez une clé
d’API de fournisseur ou connectez-vous avec OAuth OpenAI ChatGPT/Codex.
</Note>
...
<Steps>
  ...
  <Step title="Choisir un modèle par défaut (facultatif)">
    ...
    OpenClaw achemine les demandes d’images par ce profil OAuth au lieu
    ...
    réactive l’acheminement direct vers l’API OpenAI Images.
  ...
  <Step title="Interroger l’agent">
    ...
    générée par l’intermédiaire de l’outil `message` lorsqu’elle est prête.

  </Step>
</Steps>

<Warning>
Pour les points de terminaison LAN compatibles avec OpenAI, tels que LocalAI,
conservez la valeur personnalisée de `models.providers.openai.baseUrl` et
activez explicitement l’accès avec
...
| Génération d’images OpenAI avec authentification par abonnement Codex | `openai/gpt-image-2`                  | OAuth OpenAI ChatGPT/Codex               |
| PNG/WebP OpenAI avec arrière-plan transparent        | `openai/gpt-image-1.5`                             | `OPENAI_API_KEY` ou OAuth OpenAI Codex   |
...
Le même outil prend en charge la génération de texte en image et la
modification à partir d’images de référence. Utilisez `image` pour une seule
référence ou `images` pour plusieurs. Pour les modèles Krea 2 sur fal, ces
références sont envoyées comme références de style plutôt que comme entrées
de modification. Les indications de sortie prises en charge par le
fournisseur, telles que `quality`, `outputFormat` et `background`, sont
transmises lorsqu’elles sont disponibles et signalées comme ignorées
lorsqu’un fournisseur ne déclare pas les prendre en charge. La prise en
charge intégrée des arrière-plans transparents est propre à OpenAI ; les
autres fournisseurs peuvent néanmoins préserver le canal alpha PNG si leur
backend le produit.
...
Utilisez `action: "status"` pour examiner la tâche de génération d’images
...
| --------------------- | ---------------------- | --------- | ---------------------------------------------- | ------------------- | ----------------- | ----------------------------- | ------------------- | ----- | -------------------- |
| Génération (maximum)  | 1                      | 4         | 4                                              | 4                   | 1                 | 9                             | 4                   | 1     | 4                    |
| Modification/référence | 1 image (workflow)    | 1 image   | Flux : 1 ; GPT : 10 ; réf. de style Krea : 10 ; NB2 : 14 | Jusqu’à 5 images | 1 image           | 1 image (réf. de sujet)       | Jusqu’à 5 images    | -     | Jusqu’à 3 images     |
...
<ParamField path="model" type="string">
  ...
  Utilisez `openai/gpt-image-1.5` pour des arrière-plans OpenAI transparents.
</ParamField>
...
<ParamField path="images" type="string[]">
  ...
  à références de style (jusqu’à 14 par l’intermédiaire de l’outil partagé ;
  les limites propres aux fournisseurs continuent de s’appliquer).
</ParamField>
<ParamField path="size" type="string">
  Indication de taille : `1024x1024`, `1536x1024`, `1024x1536`, `2048x2048`, `3840x2160`.
</ParamField>
<ParamField path="aspectRatio" type="string">
  Rapport d’aspect : `1:1`, `2:1`, `20:9`, `19.5:9`, `2:3`, `3:2`, `2.35:1`, `3:4`,
  `4:3`, `4:5`, `5:4`, `9:16`, `9:19.5`, `9:20`, `16:9`, `21:9`, `1:2`, `4:1`,
  `1:4`, `8:1`, `1:8`. Les fournisseurs valident le sous-ensemble propre à
  leur modèle.
</ParamField>
...
<ParamField path="background" type='"transparent" | "opaque" | "auto"'>
  Indication d’arrière-plan lorsque le fournisseur la prend en charge.
  Utilisez `transparent` avec `outputFormat: "png"` ou `"webp"` pour les
  fournisseurs prenant en charge la transparence.
</ParamField>
<ParamField path="count" type="number">Nombre d’images à générer (1-4).</ParamField>
<ParamField path="timeoutMs" type="number">
  Délai d’expiration facultatif de la demande au fournisseur, en millisecondes.
...
Lorsqu’un fournisseur de secours prend en charge une option géométrique
proche plutôt que celle demandée exactement, OpenClaw la remplace par la
taille, le rapport d’aspect ou la résolution pris en charge les plus proches
avant l’envoi. Les indications de sortie non prises en charge sont supprimées
pour les fournisseurs qui ne déclarent pas les prendre en charge et sont
signalées dans le résultat de l’outil. Les résultats de l’outil indiquent les
paramètres appliqués ; `details.normalization` consigne toute conversion entre
la valeur demandée et la valeur appliquée.
</Note>
...
4. **Détection automatique** — uniquement les valeurs par défaut des fournisseurs disposant d’une authentification :
   - le fournisseur actuellement défini par défaut en premier ;
...
candidat configuré suivant est essayé automatiquement. S’ils échouent tous,
l’erreur inclut les détails de chaque tentative.

<AccordionGroup>
  <Accordion title="Les remplacements de modèle par appel sont exacts">
    Un remplacement de `model` par appel essaie uniquement ce fournisseur/modèle
    et ne poursuit pas avec le fournisseur principal, les fournisseurs de secours
    configurés ou les fournisseurs détectés automatiquement.
  </Accordion>
  <Accordion title="La détection automatique tient compte de l’authentification">
    La valeur par défaut d’un fournisseur n’entre dans la liste des candidats que
    lorsqu’OpenClaw peut effectivement authentifier ce fournisseur. Définissez
  ...
  <Accordion title="Délais d’expiration">
    Définissez `agents.defaults.imageGenerationModel.timeoutMs` pour les moteurs
    d’image lents. Un paramètre d’outil `timeoutMs` fourni par appel remplace la
    valeur par défaut configurée, et les valeurs par défaut configurées remplacent
    celles des fournisseurs définies par les plugins. Les fournisseurs d’images
    hébergés par Google et OpenRouter utilisent des valeurs par défaut de 180 secondes ;
    la génération d’images Microsoft Foundry MAI, xAI et Azure OpenAI utilise
    600 secondes. Les appels d’outils dynamiques Codex utilisent une valeur par
    défaut de 120 secondes pour la passerelle `image_generate` et respectent le
    même délai configuré, dans la limite maximale de 600000 ms imposée par OpenClaw
    à la passerelle d’outils dynamiques.
  </Accordion>
...
ComfyUI et xAI prennent en charge la retouche d’images de référence. Les modèles
Krea 2 sur fal utilisent les mêmes champs `image` / `images` comme références
de style plutôt que comme entrées de retouche. Transmettez le chemin ou l’URL
d’une image de référence :

```text
"Générer une version à l’aquarelle de cette photo" + image: "/path/to/photo.jpg"
```

OpenAI, OpenRouter et Google prennent en charge jusqu’à 5 images de référence
via le paramètre `images` ; xAI en prend en charge jusqu’à 3. fal prend en charge
1 image de référence pour Flux image-to-image, jusqu’à 10 pour les retouches
GPT Image 2, jusqu’à 10 références de style pour Krea 2 et jusqu’à 14 pour les
retouches Nano Banana 2. Microsoft Foundry, MiniMax et ComfyUI en prennent en
charge 1.
...
<AccordionGroup>
  <Accordion title="OpenAI gpt-image-2 (et gpt-image-1.5)">
    ...
    profil OAuth `openai` est configuré, OpenClaw réutilise le même profil
    OAuth que celui utilisé par les modèles de conversation de l’abonnement Codex
    et envoie la requête d’image au moteur Codex Responses. Les anciennes URL de
    base Codex telles que `https://chatgpt.com/backend-api` sont normalisées en
    ...
    pour imposer un routage direct vers l’API OpenAI Images, configurez
    explicitement `models.providers.openai` avec une clé API, une URL de base
    ...
    Utilisez `gpt-image-1.5` pour produire des fichiers PNG/WebP avec un arrière-plan
    ...
    OpenClaw transmet à OpenAI `prompt`, `count`, `size`, `quality`, `outputFormat`
    et les images de référence. OpenAI ne reçoit **pas** directement
    `aspectRatio` ou `resolution` ; lorsque cela est possible, OpenClaw les
    convertit en une valeur `size` prise en charge, sinon l’outil les signale
    comme des remplacements ignorés.
    ...
    les sorties transparentes nécessitent un `outputFormat` `png` ou `webp`
    ainsi qu’un modèle d’image OpenAI prenant en charge la transparence. OpenClaw
    achemine les requêtes avec arrière-plan transparent destinées par défaut à
    `gpt-image-2` vers `gpt-image-1.5`.
    ...
    plutôt que par `api.openai.com`, consultez
  ...
  <Accordion title="Modèles d’image Microsoft Foundry MAI">
    ...
    d’images MAI déployés sous le préfixe de fournisseur `microsoft-foundry/`.
    ...
    - Authentification : `AZURE_OPENAI_API_KEY` / clé API du fournisseur, ou Entra ID via `az login`
    ...
    - Retouches : une image de référence PNG ou JPEG, prise en charge uniquement par les
      déploiements `MAI-Image-2.5-Flash` et `MAI-Image-2.5`

    La génération à partir d’une invite seule peut utiliser un nom de déploiement
    personnalisé avec uniquement le point de terminaison Foundry configuré. Les
    retouches utilisant des noms de déploiement personnalisés nécessitent des
    métadonnées d’intégration initiale/de modèle afin qu’OpenClaw puisse vérifier
    que le déploiement repose sur `MAI-Image-2.5-Flash` ou `MAI-Image-2.5`.

    Les modèles d’image MAI actuels sont `MAI-Image-2.5-Flash`, `MAI-Image-2.5`,
    `MAI-Image-2e` et `MAI-Image-2`. Consultez
    [Plugin Microsoft Foundry](/fr/plugins/reference/microsoft-foundry) pour la configuration
    et le comportement des modèles de conversation.

  </Accordion>
  <Accordion title="Modèles d’image OpenRouter">
    La génération d’images OpenRouter utilise la même `OPENROUTER_API_KEY` et
    passe par l’API d’images des complétions de conversation d’OpenRouter.
    Sélectionnez les modèles d’image OpenRouter avec le préfixe `openrouter/` :
    ...
    OpenClaw transmet à OpenRouter `prompt`, `count`, les images de référence
    ainsi que les indications `aspectRatio` / `resolution` compatibles avec Gemini.
    Les raccourcis intégrés actuels vers les modèles d’image OpenRouter incluent
    `google/gemini-3.1-flash-image-preview`,
  ...
  <Accordion title="fal Krea 2">
    Les modèles Krea 2 sur fal utilisent le schéma Krea natif de fal plutôt que
    le schéma générique `image_size` utilisé par Flux. OpenClaw envoie :

    - `aspect_ratio` pour les indications de rapport d’aspect
    - `creativity`, dont la valeur par défaut est `medium`
    ...
    et Krea 2 Large pour des rendus photoréalistes et texturés plus lents et plus
    détaillés :
    ...
    pour Krea ; OpenClaw associe `size` au rapport d’aspect Krea pris en charge
    le plus proche et rejette `resolution` pour Krea au lieu de l’ignorer. Utilisez
    `fal.creativity` lorsque vous souhaitez un niveau de créativité Krea natif :

    ```json
    {
      "model": "fal/krea/v2/medium/text-to-image",
      "prompt": "Un portrait de fanzine cyber avec une texture de risographie",
      "aspectRatio": "9:16",
      "fal": {
        "creativity": "high"
      }
  ...
  <Accordion title="Double authentification MiniMax">
    La génération d’images MiniMax est disponible via les deux mécanismes
  ...
  <Accordion title="xAI grok-imagine-image">
    ...
    contenant uniquement une invite et `/v1/images/edits` lorsque `image` ou
    `images` est présent.

    - Modèles : `xai/grok-imagine-image`, `xai/grok-imagine-image-quality`
    - Nombre : jusqu’à 4
    - Références : un `image` ou jusqu’à trois `images`
    ...
    OpenClaw n’expose volontairement pas les paramètres natifs xAI `quality`,
    `mask`, `user` ni le rapport d’aspect `auto` tant que ces contrôles n’existent
    pas dans le contrat `image_generate` partagé entre les fournisseurs.
...
```text
/tool image_generate action=generate model=openai/gpt-image-1.5 prompt="Un simple autocollant en forme de cercle rouge sur un arrière-plan transparent" outputFormat=png background=transparent
```
...
openclaw infer image generate \
  ...
  --prompt "Un simple autocollant en forme de cercle rouge sur un arrière-plan transparent" \
  --json
```
...
```text
/tool image_generate action=generate model=openai/gpt-image-2 prompt="Brouillon d’affiche à faible coût pour une application de productivité discrète" quality=low openai='{"moderation":"low"}'
```
...
openclaw infer image generate \
  ...
  --prompt "Brouillon d’affiche à faible coût pour une application de productivité discrète" \
  --json
```
...
autres qu’OpenAI ne déclarent actuellement aucun contrôle explicite de l’arrière-plan ; pour eux,
`background: "transparent"` est donc signalé comme ignoré.
...
- [Modèles](/fr/concepts/models) - configuration des modèles et basculement automatique
