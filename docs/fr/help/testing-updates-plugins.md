...
read_when:
    ...
    - Débogage de mise à jour de package, de nettoyage des dépendances de Plugin ou de régressions d’installation de Plugin
...
x-i18n:
    generated_at: "2026-05-03T21:34:39Z"
    model: gpt-5.5
    provider: openai
...
Pour la cartographie plus large du lanceur de tests, consultez [Tests](/fr/help/testing). Pour les
clés de fournisseurs live et les suites qui touchent au réseau, consultez [Tests live](/fr/help/testing-live).
...
- `openclaw doctor --fix --non-interactive` possède les chemins de nettoyage et de réparation
  hérités. Le démarrage ne doit pas accumuler de migrations de compatibilité cachées pour les
  états de plugin obsolètes.
...
Pour les changements d’installation, de désinstallation, de dépendance ou d’inventaire de package de plugin, exécutez aussi
les tests ciblés qui couvrent le point de jonction modifié :

```bash
pnpm test src/plugins/uninstall.test.ts src/infra/package-dist-inventory.test.ts test/scripts/package-acceptance-workflow.test.ts
```

Avant qu’un volet Docker de package consomme une archive tarball, prouvez l’artefact de package :

```bash
pnpm release:check
```

`release:check` exécute les contrôles de dérive de configuration/docs/API, écrit l’inventaire de distribution
du package, exécute `npm pack --dry-run`, rejette les fichiers empaquetés interdits, installe
l’archive tarball dans un préfixe temporaire, exécute postinstall, et effectue un smoke test des points d’entrée
des canaux groupés.

## Volets Docker

Les volets Docker constituent la preuve au niveau produit. Ils installent ou mettent à jour un vrai
...
Utilisez des volets ciblés pendant l’itération :
...
- `test:docker:plugins` valide le smoke test d’installation de plugin, les installations de dossiers locaux,
  le comportement d’ignorance des mises à jour de dossiers locaux, les dossiers locaux avec dépendances
  préinstallées, les installations de packages `file:`, les installations git avec exécution CLI, les mises à jour
  git de références mobiles, les installations depuis registre npm avec dépendances transitives
  hissées, les non-opérations de mise à jour npm, les installations de fixtures ClawHub locales et les
  non-opérations de mise à jour, le comportement de mise à jour du marketplace, ainsi que l’activation/inspection du bundle Claude. Définissez
  `OPENCLAW_PLUGINS_E2E_CLAWHUB=0` pour garder le bloc ClawHub hermétique/hors ligne.
- `test:docker:plugin-lifecycle-matrix` installe le package candidat dans un conteneur
  nu, fait passer un plugin npm par l’installation, l’inspection, la désactivation, l’activation,
  la montée de version explicite, le retour de version explicite et la désinstallation après suppression du code
...
- `test:docker:upgrade-survivor` installe l’archive tarball candidate par-dessus une fixture
  d’ancien utilisateur sale, exécute la mise à jour de package plus le doctor non interactif, puis démarre
...
- `test:docker:published-upgrade-survivor` installe d’abord une base publiée,
  la configure via une recette `openclaw config set` intégrée, la met à jour vers l’archive tarball
  candidate, exécute doctor, vérifie le nettoyage hérité, démarre le Gateway, et
  sonde `/healthz`, `/readyz` et l’état RPC.
- `test:docker:update-migration` est le volet de mise à jour publiée fortement axé sur le nettoyage. Il
  part d’un état utilisateur configuré de style Discord/Telegram, exécute le doctor de base
  afin que les dépendances de plugins configurés aient une chance de se matérialiser, injecte
  des débris hérités de dépendances de plugin pour un plugin empaqueté configuré, met à jour vers
  l’archive tarball candidate, et exige que le doctor post-mise à jour supprime les racines
  de dépendances héritées.

Variantes utiles du survivor de mise à jour publiée :
...
`plugin-deps-cleanup`, `configured-plugin-installs`, `tilde-log-path` et
`versioned-runtime-deps`. Dans les exécutions agrégées,
`OPENCLAW_UPGRADE_SURVIVOR_SCENARIOS=reported-issues` s’étend à tous les scénarios
ayant la forme de problèmes signalés, y compris la migration d’installation de plugins configurés.
...
Package Acceptance est la barrière de package native GitHub. Elle résout un package
candidat en une archive tarball `package-under-test`, enregistre la version et le SHA-256, puis
exécute des volets Docker E2E réutilisables contre cette archive tarball exacte. La référence du harnais
de workflow est séparée de la référence source du package, afin que la logique de test actuelle puisse valider
...
- `source=npm` : valider `openclaw@beta`, `openclaw@latest` ou une version
  publiée exacte.
- `source=ref` : empaqueter une branche, une étiquette ou un commit de confiance avec le harnais actuel
  sélectionné.
...
`package_acceptance_package_spec=openclaw@YYYY.M.D` afin que la même matrice de mise à jour
...
Cela garde la migration de package, le basculement de canal de mise à jour, le nettoyage des dépendances
de plugin obsolètes, la couverture de plugins hors ligne, le comportement de mise à jour des plugins et la QA du package
Telegram sur le même artefact résolu.

`all-since-2026.4.23` est l’échantillon de mise à jour de la CI Full Release : chaque release stable publiée sur npm depuis `2026.4.23` jusqu’à `latest`. Pour une couverture exhaustive de migration de mise à jour
publiée, utilisez `all-since-2026.4.23` dans le workflow séparé Update
Migration au lieu de la CI Full Release. `release-history` reste
disponible pour un échantillonnage manuel plus large lorsque vous voulez aussi l’ancre héritée antérieure à la date.
...
3. Le profil Package Acceptance `package` ou les volets de package personnalisés des contrôles de release
   pour les contrats d’installation/mise à jour/plugin.
4. Les contrôles de release multi-OS pour le programme d’installation, l’onboarding et le comportement
   de plateforme propres à chaque OS.
5. Les suites live uniquement lorsque la surface modifiée touche au comportement d’un fournisseur ou d’un service
   hébergé.

Sur les machines de maintainers, les barrières larges et la preuve produit Docker/package doivent s’exécuter
...
- Le package publié `2026.4.26` peut émettre un avertissement pour des fichiers d’horodatage de métadonnées de build local
  déjà livrés.
...
Lorsque vous changez le comportement de mise à jour ou de plugin, ajoutez une couverture au niveau le plus bas qui
peut échouer pour la bonne raison :

- Logique pure de chemin ou de métadonnées : test unitaire à côté de la source.
- Inventaire de package ou comportement de fichiers empaquetés : test `package-dist-inventory` ou vérificateur
  d’archive tarball.
- Comportement CLI d’installation/mise à jour : assertion ou fixture de volet Docker.
...
- Comportement d’agencement ou de nettoyage des dépendances : affirmez à la fois l’exécution runtime et la
  frontière du système de fichiers. Les dépendances npm peuvent être hissées sous la racine npm
  gérée, donc les tests doivent prouver que la racine est analysée/nettoyée au lieu de supposer une arborescence
  `node_modules` locale au package.

Gardez les nouvelles fixtures Docker hermétiques par défaut. Utilisez des registres de fixtures locaux et
de faux packages sauf si le but du test est le comportement de registre live.
...
- Artefacts Docker : `.artifacts/docker-tests/**/summary.json`,
  `failures.json`, journaux de volet et commandes de réexécution.
- Résumé upgrade survivor : `.artifacts/upgrade-survivor/summary.json`,
  incluant la version de base, la version candidate, le scénario, les durées de phase et
  les étapes de recette.

Préférez réexécuter le volet exact en échec avec le même artefact de package plutôt que
