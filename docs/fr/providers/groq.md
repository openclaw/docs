...
summary: Configuration de Groq (authentification + sélection du modèle + transcription avec Whisper)
title: Groq
x-i18n:
    generated_at: "2026-07-12T03:00:08Z"
    model: gpt-5.6
...
Les modèles de raisonnement Groq (`reasoning: true` dans le tableau ci-dessus) associent les niveaux `/think` communs d’OpenClaw aux valeurs `reasoning_effort` `low`, `medium` ou `high`. `/think off` ou `/think none` omet `reasoning_effort` de la requête au lieu d’envoyer une valeur désactivée.

Consultez [Modes de réflexion](/fr/tools/thinking) pour découvrir les niveaux `/think` communs et la façon dont OpenClaw les traduit pour chaque fournisseur.

## Transcription audio

Le plugin Groq enregistre également un **fournisseur de compréhension de médias audio**, afin que les messages vocaux puissent être transcrits par l’intermédiaire de l’interface partagée `tools.media.audio`.
...
Pour utiliser Groq comme moteur audio par défaut :
...
<AccordionGroup>
  <Accordion title="Disponibilité de l’environnement pour le démon">
    Si le Gateway s’exécute en tant que service géré (launchd, systemd, Docker), `GROQ_API_KEY` doit être visible par ce processus, et pas uniquement par votre shell interactif.

    <Warning>
      Une clé exportée uniquement dans un shell interactif ne sera pas accessible à un démon launchd ou systemd, sauf si cet environnement y est également importé. Définissez la clé dans `~/.openclaw/.env` ou via `env.shellEnv` afin qu’elle soit lisible par le processus du Gateway.
    </Warning>

  </Accordion>

  <Accordion title="Identifiants de modèles Groq personnalisés">
    OpenClaw accepte n’importe quel identifiant de modèle Groq lors de l’exécution. Utilisez l’identifiant exact indiqué par Groq et ajoutez-lui le préfixe `groq/`. Le catalogue statique couvre les cas courants ; les identifiants absents du catalogue utilisent le modèle compatible avec OpenAI par défaut.
...
<CardGroup cols={2}>
  ...
  <Card title="Référence de configuration" href="/fr/gateway/configuration-reference" icon="gear">
    Schéma de configuration complet, notamment les paramètres des fournisseurs et de l’audio.
  </Card>
