...
title: Google (Gemini)
x-i18n:
    generated_at: "2026-07-12T15:44:22Z"
    model: gpt-5.6
...
- Option d’exécution : `agentRuntime.id: "google-gemini-cli"` réutilise l’authentification OAuth de la CLI Gemini tout en conservant des références de modèle canoniques sous la forme `google/*`.
...
<Tabs>
  ...
  <Tab title="CLI Gemini (OAuth)">
    **Idéal pour :** réutiliser une connexion existante à la CLI Gemini via OAuth avec PKCE plutôt qu’une clé API distincte.
    ...
    <Steps>
      <Step title="Installer la CLI Gemini">
    ...
    **Variables d’environnement :**
    ...
    <Note>
    Si les requêtes OAuth de la CLI Gemini échouent après la connexion, définissez `GOOGLE_CLOUD_PROJECT` ou
    `GOOGLE_CLOUD_PROJECT_ID` sur l’hôte du Gateway, puis réessayez.
    </Note>

    <Note>
    Si la connexion échoue avant le démarrage du parcours dans le navigateur, assurez-vous que la commande locale `gemini`
    ...
    configurations doivent utiliser des références de modèle `google/*` ainsi que l’environnement d’exécution `google-gemini-cli`
    lorsqu’elles souhaitent une exécution locale par la CLI Gemini.

  </Tab>
</Tabs>

<Note>
`google/gemini-3-pro-preview` a été retiré le 2026-03-09 ; utilisez plutôt `google/gemini-3.1-pro-preview`. La réexécution de la configuration de la clé API Gemini (`openclaw onboard --auth-choice gemini-api-key` ou `openclaw models auth login --provider google`) remplace un modèle par défaut configuré obsolète par le modèle actuel.
</Note>
      ...
      google: {
        config: {
          webSearch: {
            ...
            baseUrl: "https://generativelanguage.googleapis.com/v1beta", // revient à models.providers.google.baseUrl
...
sert aux proxys d’opérateur ou aux points de terminaison compatibles avec l’API Gemini ; lorsqu’il est omis,
...
les contrôles de raisonnement des alias Gemini 3, Gemini 3.1 et `gemini-*-latest` à
...
convertit `thinkingBudget` en un `thinkingLevel` Google pris en charge pour Gemma 4.
Définir la réflexion sur `off` la maintient désactivée au lieu de l’associer à
`MINIMAL`.

Gemini 2.5 Pro fonctionne uniquement en mode réflexion et rejette une valeur explicite
...
<Note>
Consultez [Génération d’images](/fr/tools/image-generation) pour les paramètres d’outil communs, la sélection du fournisseur et le comportement de basculement.
</Note>
...
- Prend en charge `aspectRatio` (`16:9`, `9:16`) et `resolution` (`720P`, `1080P`) ; la sortie audio n’est actuellement pas prise en charge par Veo
...
<Note>
Consultez [Génération de vidéos](/fr/tools/video-generation) pour les paramètres d’outil communs, la sélection du fournisseur et le comportement de basculement.
</Note>
...
- Les exécutions associées à une session se détachent via le flux partagé de tâche/état, notamment `action: "status"`
...
<Note>
Consultez [Génération de musique](/fr/tools/music-generation) pour les paramètres d’outil communs, la sélection du fournisseur et le comportement de basculement.
</Note>
...
- Sortie : WAV pour les pièces jointes TTS ordinaires, Opus pour les destinations de type note vocale, PCM pour Talk/téléphonie
- Sortie de note vocale : le PCM Google est encapsulé en WAV et transcodé en Opus à 48 kHz avec `ffmpeg`

Le parcours TTS Gemini par lots de Google renvoie l’audio généré dans la réponse
`generateContent` terminée. Pour des conversations vocales avec la latence la plus faible, utilisez le
...
`audioProfile` afin d’ajouter une instruction de style réutilisable avant le texte prononcé. Définissez
...
l’API Gemini Live pour les passerelles audio côté serveur telles que Voice Call et Google Meet.
...
OpenClaw adapte l’audio du pont de téléphonie/Meet au flux de l’API PCM Live de Gemini et
maintient les appels d’outils sur le contrat vocal partagé en temps réel. Laissez `temperature`
...
<Note>
Gemini 3.1 Live accepte du texte conversationnel via les entrées en temps réel et utilise
l’appel séquentiel de fonctions. OpenClaw omet les anciens champs `NON_BLOCKING`, de planification
des réponses de fonction et de dialogue affectif pour ce modèle. Préférez
`thinkingLevel` ; les valeurs positives configurées pour `thinkingBudget` sont associées au
...
<Note>
Talk dans Control UI prend en charge les sessions Google Live dans le navigateur avec des jetons
à usage unique et à portée limitée. Les fournisseurs vocaux en temps réel réservés au backend peuvent
également fonctionner via le transport de relais générique du Gateway, qui conserve les identifiants
...
Le test de contrôle couvre également les chemins backend/WebRTC d’OpenAI ; la partie Google émet le même
format de jeton à portée limitée pour l’API Live que celui utilisé par Talk dans Control UI, ouvre le
...
<AccordionGroup>
  <Accordion title="Réutilisation directe du cache Gemini">
    ...
    transmet aux requêtes Gemini un identifiant `cachedContent` configuré.

    - Configurez les paramètres par modèle ou globaux avec
      `cachedContent` ou l’ancien `cached_content`
    - Les paramètres d’une portée plus spécifique (niveau du modèle plutôt que global) sont toujours prioritaires.
      Dans une même portée, si les deux clés sont définies, `cached_content` est prioritaire.
      N’utilisez qu’une seule clé par portée pour éviter les surprises.
    - Exemple de valeur : `cachedContents/prebuilt-context`
    - L’utilisation lors d’un accès au cache Gemini est normalisée dans le champ `cacheRead` d’OpenClaw à partir de
      la valeur `cachedContentTokenCount` en amont
  ...
  <Accordion title="Remarques sur l’utilisation de la CLI Gemini">
    Lorsque vous utilisez le fournisseur OAuth `google-gemini-cli`, OpenClaw utilise par défaut
    la sortie `stream-json` de la CLI Gemini et normalise l’utilisation à partir de la charge utile
    `stats` finale. Les remplacements par l’ancien format `--output-format json` utilisent toujours
    l’analyseur JSON.
    ...
    - `stats.cached` est normalisé dans le champ `cacheRead` d’OpenClaw.
    - Si `stats.input` est absent, OpenClaw calcule les jetons d’entrée à partir de
