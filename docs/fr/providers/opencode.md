...
title: OpenCode
x-i18n:
    generated_at: "2026-07-12T15:53:50Z"
    model: gpt-5.6
...
Les deux catalogues partagent une même clé API OpenCode (`OPENCODE_API_KEY`, alias
`OPENCODE_ZEN_API_KEY`). OpenClaw conserve des identifiants de fournisseur d’exécution distincts afin
que le routage en amont propre à chaque modèle reste correct, mais l’intégration initiale et la documentation les traitent comme
...
<Tabs>
  <Tab title="Catalogue Zen">
    **Idéal pour :** le proxy multimodèle sélectionné par OpenCode (Claude, GPT, Gemini, GLM,
...
Exécutez `openclaw models list --provider opencode` pour obtenir la liste complète actuelle, qui
...
Consultez [OpenCode Go](/fr/providers/opencode-go) pour le tableau complet des modèles Go.
...
<AccordionGroup>
  ...
  <Accordion title="Identifiants partagés">
    La saisie d’une clé OpenCode lors de la configuration enregistre les identifiants pour les deux fournisseurs
    d’exécution. Vous n’avez pas besoin d’effectuer séparément l’intégration initiale de chaque catalogue.
  </Accordion>

  <Accordion title="Obtenir une clé API">
    Créez un compte OpenCode et générez une clé API sur
  ...
  <Accordion title="Comportement de relecture de Gemini">
    Les références OpenCode reposant sur Gemini restent sur le chemin proxy-Gemini, afin qu’OpenClaw y conserve
    l’assainissement des signatures de réflexion Gemini sans activer la validation de relecture native de Gemini
    ni les réécritures d’amorçage.
  </Accordion>

  <Accordion title="Comportement de relecture hors Gemini">
    Les références OpenCode hors Gemini conservent la politique de relecture minimale compatible avec OpenAI.
  </Accordion>
</AccordionGroup>
