...
Le [Vercel AI Gateway](https://vercel.com/ai-gateway) fournit une API unifiée permettant
d’accéder à des centaines de modèles via un point de terminaison unique.
...
<Tip>
OpenClaw découvre automatiquement le catalogue `/v1/models` du Gateway. Ainsi, la
commande de chat `/models vercel-ai-gateway` et la commande
...
<Tip>
Utilisez l’une ou l’autre forme dans votre configuration ; OpenClaw résout automatiquement
la référence canonique `anthropic/...`.
</Tip>
...
<AccordionGroup>
  <Accordion title="Variable d’environnement pour les processus démons">
    Si le Gateway OpenClaw s’exécute en tant que démon (launchd/systemd), assurez-vous que
    `AI_GATEWAY_API_KEY` est accessible à ce processus.
    ...
    la clé dans `~/.openclaw/.env` ou via `env.shellEnv` afin que le processus
  ...
  <Accordion title="Routage des fournisseurs">
    Vercel AI Gateway achemine chaque requête vers le fournisseur en amont indiqué dans le
  ...
  <Accordion title="Niveaux de réflexion">
    Les options `/think` suivent le préfixe du modèle en amont lorsqu’OpenClaw le reconnaît.
    `vercel-ai-gateway/anthropic/...` utilise le profil de réflexion de Claude,
    y compris la valeur adaptative par défaut pour les modèles Claude 4.6. Les références
    `vercel-ai-gateway/openai/...` approuvées (`gpt-5.2` et versions ultérieures, ainsi que les variantes Codex
    jusqu’à `gpt-5.1-codex`) proposent `/think xhigh`. Les autres références avec espace de noms
