...
title: Gateway Kilo
x-i18n:
    generated_at: "2026-07-12T03:15:24Z"
    model: gpt-5.6
...
Au démarrage, OpenClaw interroge `GET https://api.kilo.ai/api/gateway/models` et fusionne les modèles détectés
avant un catalogue de secours statique. Ce catalogue ne contient que `kilocode/kilo/auto` (`Kilo Auto`,
`input: ["text", "image"]`, `reasoning: true`, `contextWindow: 1000000`, `maxTokens: 128000`).

Tout modèle du Gateway est accessible sous la forme `kilocode/<upstream-id>` (par exemple
`kilocode/anthropic/claude-sonnet-4`, `kilocode/openai/gpt-5.5`). Exécutez `/models kilocode` ou
`openclaw models list --provider kilocode` pour afficher la liste complète des modèles détectés.
...
<AccordionGroup>
  <Accordion title="Transport et compatibilité">
    Kilo Gateway est compatible avec OpenRouter ; il utilise donc le chemin de requête compatible avec OpenAI de type proxy
    plutôt que le formatage natif des requêtes OpenAI (pas de `store`, ni de charge utile OpenAI pour l’effort de raisonnement).

    - Les références Kilo reposant sur Gemini restent sur le chemin proxy Gemini : OpenClaw nettoie les signatures
      de réflexion Gemini à cet endroit, mais n’active ni la validation native de la relecture Gemini ni les réécritures d’amorçage.
    - Les requêtes utilisent un jeton Bearer généré à partir de votre clé API.
  ...
  <Accordion title="Enveloppe de flux et raisonnement">
    ...
    modifiable avec la variable d’environnement `KILOCODE_FEATURE`) et normalise les charges utiles d’effort de raisonnement pour
    les modèles qui le prennent en charge.

    <Warning>
    Les références `kilocode/kilo/auto` et `x-ai/*` ignorent l’injection de l’effort de raisonnement. Utilisez une référence de modèle
    concrète telle que `kilocode/anthropic/claude-sonnet-4` si vous avez besoin de la prise en charge du raisonnement.
    </Warning>

  </Accordion>

  <Accordion title="Dépannage">
    - Si la détection des modèles échoue au démarrage, OpenClaw utilise le catalogue statique de secours contenant `kilocode/kilo/auto`.
