---
read_when:
    - Vous transférez OpenClaw vers un nouvel ordinateur portable ou serveur
    - Vous venez d’un autre système d’agents et souhaitez conserver l’état
    - Vous mettez à niveau un plugin sur place
...
OpenClaw prend en charge trois chemins de migration : l’importation depuis un autre système d’agents, le déplacement d’une installation existante vers une nouvelle machine et la mise à niveau sur place d’un plugin.

## Importer depuis un autre système d’agents

Les fournisseurs de migration intégrés importent dans OpenClaw les instructions, les serveurs MCP, les Skills, la configuration des modèles et, avec votre accord, les clés API. Les plans sont prévisualisés avant toute modification, les secrets sont masqués dans les rapports et l’application des changements s’appuie sur une sauvegarde vérifiée.

<CardGroup cols={2}>
  <Card title="Migrer depuis Claude" href="/fr/install/migrating-claude" icon="brain">
  ...
  <Card title="Migrer depuis Hermes" href="/fr/install/migrating-hermes" icon="feather">
...
<Tip>
Exécutez `openclaw status` sur l’ancienne machine pour confirmer le chemin de votre répertoire d’état. Les profils personnalisés utilisent `~/.openclaw-<profile>/` ou un chemin défini avec `OPENCLAW_STATE_DIR`.
</Tip>
...
<Steps>
  ...
  <Step title="Installer OpenClaw sur la nouvelle machine">
    [Installez](/fr/install) la CLI (et Node si nécessaire) sur la nouvelle machine. Il n’est pas gênant que l’intégration crée un nouveau répertoire `~/.openclaw/` : vous le remplacerez à l’étape suivante.
  </Step>
  ...
  <Step title="Copier le répertoire d’état et l’espace de travail">
    ...
    Vérifiez que les répertoires masqués ont été inclus et que le propriétaire des fichiers correspond à l’utilisateur qui exécutera le Gateway.
...
Si Telegram ou Discord utilise le mécanisme de secours par défaut fondé sur les variables d’environnement (`TELEGRAM_BOT_TOKEN` ou `DISCORD_BOT_TOKEN`), vérifiez que le fichier `.env` du répertoire d’état migré contient ces clés sans afficher les valeurs secrètes :

```bash
awk -F= '/^(TELEGRAM_BOT_TOKEN|DISCORD_BOT_TOKEN)=/ { print $1 "=present" }' ~/.openclaw/.env
```

`openclaw doctor` émet également un avertissement lorsqu’un compte Telegram ou Discord par défaut activé ne dispose d’aucun jeton configuré et que la variable d’environnement correspondante n’est pas accessible au processus Doctor.

### Problèmes courants

<AccordionGroup>
  <Accordion title="Incompatibilité de profil ou de répertoire d’état">
  ...
  <Accordion title="Autorisations et propriété">
    Si vous avez effectué la copie en tant que root ou changé d’utilisateur, le Gateway peut ne pas parvenir à lire les identifiants. Assurez-vous que le répertoire d’état et l’espace de travail appartiennent à l’utilisateur qui exécute le Gateway.
  </Accordion>
  ...
  <Accordion title="Secrets dans les sauvegardes">
    Le répertoire d’état contient les profils d’authentification, les identifiants des canaux et d’autres états de fournisseurs. Stockez les sauvegardes sous forme chiffrée, évitez les canaux de transfert non sécurisés et renouvelez les clés si vous suspectez une exposition.
  </Accordion>
</AccordionGroup>
...
Les mises à niveau de plugins sur place conservent le même identifiant de plugin et les mêmes clés de configuration, mais peuvent déplacer l’état stocké sur disque vers la structure actuelle. Les guides de mise à niveau propres aux plugins se trouvent avec leurs canaux respectifs :

- [Migration de Matrix](/fr/channels/matrix-migration) : limites de récupération de l’état chiffré, comportement de création automatique d’instantanés et commandes de récupération manuelle.
