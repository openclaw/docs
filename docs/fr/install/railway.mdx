...
<Step title="Ajouter un volume">
  ...
  <Step title="Définir les variables">
    ...
    - `OPENCLAW_GATEWAY_PORT=8080` (requis -- doit correspondre au port indiqué dans Public Networking)
...
<Step title="Activer le réseau public">
  ...
  <Step title="Se connecter">
    Recherchez votre URL publique dans **Railway -> votre service -> Settings -> Domains** -- soit un domaine généré (souvent `https://<something>.up.railway.app`), soit le domaine personnalisé que vous avez associé.
...
Cette commande crée une archive de sauvegarde portable contenant l’état d’OpenClaw ainsi que tout espace de travail configuré. Consultez [Sauvegarde](/fr/cli/backup) pour plus de détails.
