...
read_when:
    ...
    - Vous devez comprendre ce qu’OpenClaw importe automatiquement et ce qui reste uniquement dans les archives.
summary: Déplacez l’état local de Claude Code et Claude Desktop vers OpenClaw grâce à un import prévisualisé
...
<AccordionGroup>
  ...
  <Accordion title="Serveurs MCP">
    Les définitions des serveurs MCP sont importées depuis le fichier de projet `.mcp.json`, le fichier Claude Code `~/.claude.json` et le fichier Claude Desktop `claude_desktop_config.json`, lorsqu’ils sont présents.
  </Accordion>
  <Accordion title="Skills et commandes">
    ...
    - Les fichiers Markdown de commandes Claude situés sous `.claude/commands/` ou `~/.claude/commands/` sont convertis en Skills OpenClaw avec `disable-model-invocation: true`.
...
Le fournisseur copie les éléments suivants dans le rapport de migration pour examen manuel, mais ne les charge **pas** dans la configuration OpenClaw active :
...
- `CLAUDE.local.md`
- `.claude/rules/`
- Les sous-agents Claude sous `.claude/agents/` ou `~/.claude/agents/`
- Les répertoires de caches, de plans et d’historique de projets Claude Code
- Les extensions Claude Desktop et les identifiants stockés par le système d’exploitation

OpenClaw refuse d’exécuter automatiquement les hooks, de faire confiance aux listes d’autorisations ou de décoder l’état opaque des identifiants OAuth et Desktop. Déplacez manuellement les éléments dont vous avez besoin après avoir examiné l’archive.

## Sélection de la source

Sans `--from`, OpenClaw examine le répertoire personnel Claude Code par défaut situé à `~/.claude`, le fichier d’état échantillonné Claude Code `~/.claude.json` et la configuration MCP de Claude Desktop sous macOS.

Lorsque `--from` désigne une racine de projet, OpenClaw importe uniquement les fichiers Claude de ce projet, tels que `CLAUDE.md`, `.claude/settings.json`, `.claude/commands/`, `.claude/skills/` et `.mcp.json`. Il ne lit pas votre répertoire personnel Claude global lors de l’importation d’une racine de projet.
...
<Steps>
  ...
  <Step title="Appliquer avec une sauvegarde">
    ...
    OpenClaw crée et vérifie une sauvegarde avant d’appliquer les modifications.
  ...
  <Step title="Exécuter Doctor">
    ...
    [Doctor](/fr/gateway/doctor) recherche les problèmes de configuration ou d’état après l’importation.
...
<Warning>
Relancez la commande avec `--overwrite` uniquement si vous souhaitez remplacer la destination existante. Les fournisseurs peuvent tout de même créer des sauvegardes individuelles des fichiers remplacés dans le répertoire du rapport de migration.
</Warning>

Pour une nouvelle installation d’OpenClaw, les conflits sont inhabituels. Ils apparaissent généralement lorsque vous relancez l’importation sur une configuration qui comporte déjà des modifications utilisateur.
...
`--yes` est requis pour `migrate apply` en dehors d’un terminal interactif ; sans cette option, OpenClaw renvoie une erreur au lieu d’appliquer les modifications. Les scripts et la CI doivent donc transmettre explicitement `--yes`. Affichez d’abord un aperçu avec `--dry-run --json`, puis appliquez les modifications avec `--json --yes` une fois le plan validé.

## Résolution des problèmes

<AccordionGroup>
  <Accordion title="L’état Claude se trouve en dehors de ~/.claude">
    Transmettez `--from /actual/path` (CLI) ou `--import-source /actual/path` (intégration).
  </Accordion>
  <Accordion title="L’intégration refuse l’importation dans une configuration existante">
    Les imports lors de l’intégration nécessitent une nouvelle configuration. Réinitialisez l’état et recommencez l’intégration, ou utilisez directement `openclaw migrate apply claude`, qui prend en charge `--overwrite` et le contrôle explicite des sauvegardes.
  </Accordion>
  ...
  <Accordion title="Les commandes Claude sont devenues des Skills avec l’invocation par le modèle désactivée">
...
- [Guide de migration](/fr/install/migrating) : tous les parcours de migration.
- [Migration depuis Hermes](/fr/install/migrating-hermes) : l’autre parcours d’importation entre systèmes.
