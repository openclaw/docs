...
x-i18n:
    generated_at: "2026-05-02T07:11:39Z"
    model: gpt-5.5
    provider: openai
...
| [`install-cli.sh`](#install-clish) | macOS / Linux / WSL  | Installe Node + OpenClaw dans un préfixe local (`~/.openclaw`) avec npm ou les modes git checkout. Aucun root requis. |
...
<Steps>
  ...
  <Step title="Installer OpenClaw">
    ...
    - méthode `git` : cloner/mettre à jour le dépôt, installer les dépendances avec pnpm, compiler, puis installer le wrapper dans `~/.local/bin/openclaw`
...
par défaut, ainsi que les installations par git-checkout sous le même flux de préfixe.
</Info>
...
<Steps>
  ...
  <Step title="Actualiser le service Gateway chargé">
    ...
    sonde au mieux l’état du Gateway.
  </Step>
</Steps>
...
| `OPENCLAW_INSTALL_METHOD=git\|npm`          | Méthode d’installation                        |
| `OPENCLAW_VERSION=<ver>`                    | Version d’OpenClaw ou dist-tag                |
...
<Steps>
  ...
  <Step title="Gérer les échecs">
    `iwr ... | iex` et les installations par bloc de script signalent une erreur terminale sans fermer la session PowerShell actuelle. Les installations directes avec `powershell -File` / `pwsh -File` se terminent toujours avec un code non nul pour l’automatisation.
  </Step>
</Steps>
...
| `OPENCLAW_DRY_RUN=1`               | Mode d’exécution à blanc |

  </Accordion>
</AccordionGroup>

<Note>
Si `-InstallMethod git` est utilisé et que Git est absent, le script se termine et affiche le lien Git for Windows.
</Note>
...
<AccordionGroup>
  ...
  <Accordion title="Pourquoi npm rencontre-t-il EACCES sous Linux ?">
    Certaines configurations Linux font pointer le préfixe global npm vers des chemins appartenant à root. `install.sh` peut basculer le préfixe vers `~/.npm-global` et ajouter les exports PATH aux fichiers rc du shell (lorsque ces fichiers existent).
  </Accordion>

  <Accordion title="Problèmes sharp/libvips">
    Les scripts définissent par défaut `SHARP_IGNORE_GLOBAL_LIBVIPS=1` pour éviter que sharp soit compilé avec la libvips système. Pour remplacer ce comportement :
