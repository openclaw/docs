---
read_when:
    - Vous souhaitez bénéficier de l’autocomplétion dans le shell pour zsh/bash/fish/PowerShell
...
title: Achèvement
x-i18n:
    generated_at: "2026-07-12T02:41:35Z"
    model: gpt-5.6
...
Générez des scripts de complétion pour le shell, mettez-les en cache dans l’état d’OpenClaw et, si vous le souhaitez, installez-les dans votre profil de shell.
...
- `--write-state` : écrire le ou les scripts de complétion dans `$OPENCLAW_STATE_DIR/completions` (`~/.openclaw/completions` par défaut) sans les afficher sur la sortie standard ; avec `--shell`, écrit uniquement celui de ce shell, sinon ceux des quatre shells
- `-y, --yes` : ignorer les demandes de confirmation d’installation (mode non interactif)

## Processus d’installation

`--install` configure votre profil pour utiliser le script mis en cache ; le cache doit donc exister au préalable. S’il est absent, la commande échoue et vous invite à exécuter `openclaw completion --write-state`. Combinez `--write-state --install` pour effectuer les deux opérations en une seule étape. Sans `--shell`, `--install` détecte le shell à partir de `$SHELL` (avec zsh comme solution de repli).
...
- La génération de la complétion charge immédiatement l’arborescence complète des commandes, y compris les commandes CLI des plugins, afin d’inclure les sous-commandes imbriquées.
- `openclaw update` actualise automatiquement le cache de complétion après une mise à jour réussie ; `openclaw doctor` peut réparer les configurations de complétion absentes ou obsolètes.
