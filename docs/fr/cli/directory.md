---
read_when:
    - Vous souhaitez rechercher les identifiants des contacts, des groupes ou de votre propre compte pour un canal
    - Vous développez un adaptateur d’annuaire de canaux
summary: Référence de la CLI pour `openclaw directory` (soi-même, pairs, groupes)
title: Répertoire
x-i18n:
    generated_at: "2026-07-12T02:29:35Z"
    model: gpt-5.6
...
Recherche dans l’annuaire pour les canaux qui la prennent en charge : contacts/pairs, groupes et « moi » (soi-même).

Les résultats sont destinés à être collés dans d’autres commandes, notamment `openclaw message send --target ...`.

## Options communes

- `--channel <name>` : identifiant/alias du canal (obligatoire lorsque plusieurs canaux sont configurés ; sélectionné automatiquement lorsqu’un seul est configuré)
...
- Pour de nombreux canaux, les résultats proviennent de la configuration (listes d’autorisation/groupes configurés) plutôt que d’un annuaire dynamique du fournisseur.
...
| ----------------------------------- | ------------------------------------------------------------------------------------------------------------------------------- |
| WhatsApp                            | `+15551234567` (message privé), `1234567890-1234567890@g.us` (groupe), `120363123456789@newsletter` (canal/newsletter, envoi uniquement) |
| Signal                              | Les alias configurés correspondent à des cibles de message privé E.164/UUID ou à des cibles de groupe `group:<id>`            |
...
| Zalo Personal / `zalouser` (Plugin) | Identifiant du fil de discussion (message privé/groupe), obtenu depuis `zca` (`me`, `friend list`, `group list`)              |
