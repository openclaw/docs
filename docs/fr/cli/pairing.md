...
title: Jumelage
x-i18n:
    generated_at: "2026-07-12T15:09:14Z"
    model: gpt-5.6
...
Approuvez ou inspectez les demandes d’association par message privé pour les canaux qui prennent en charge l’association (messages privés uniquement — l’association de nœuds/appareils utilise `openclaw devices`).
...
Si plusieurs canaux prenant en charge l’association sont configurés, indiquez un canal en argument positionnel ou avec `--channel`. Les canaux d’extension fonctionnent dès lors que l’identifiant du canal est valide.
