...
Écrire une archive zip de diagnostics locale conçue pour être jointe aux rapports de bogues. Pour le modèle de confidentialité et le contenu du bundle, consultez [Export de diagnostics](/fr/gateway/diagnostics).
...
<ParamField path="--output <path>" type="string">
  Chemin de l’archive zip de sortie. Par défaut, un export de support sous le répertoire d’état.
</ParamField>
...
`OPENCLAW_WRAPPER` dans l’environnement du service pour les réinstallations forcées, les mises à jour et les réparations doctor ultérieures.
...
- DNS-SD unicast (Bonjour étendu) : choisissez un domaine (exemple : `openclaw.internal.`) et configurez un DNS fractionné ainsi qu’un serveur DNS ; consultez [Bonjour](/fr/gateway/bonjour).
...
- Sur mDNS `local.`, `sshPort` et `cliPath` ne sont diffusés que lorsque `discovery.mdns.mode` vaut `full`. Le DNS-SD étendu écrit toujours `cliPath` ; `sshPort` y reste également facultatif.
