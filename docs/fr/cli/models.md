---
read_when:
    - Vous souhaitez modifier les modèles par défaut ou afficher l’état de l’authentification du fournisseur
...
`openclaw models status` affiche les valeurs par défaut/solutions de secours résolues ainsi qu’un aperçu de l’authentification.
Lorsque des instantanés d’utilisation fournisseur sont disponibles, la section d’état OAuth/clé d’API inclut
les fenêtres d’utilisation fournisseur et les instantanés de quota.
...
Dans la sortie `--json`, `auth.providers` est l’aperçu des fournisseurs tenant compte de l’environnement/la configuration/le magasin,
tandis que `auth.oauth` correspond uniquement à l’état de santé des profils du magasin d’authentification.
Ajoutez `--probe` pour exécuter des sondes d’authentification en direct auprès de chaque profil fournisseur configuré.
...
la commande utilise `OPENCLAW_AGENT_DIR`/`PI_CODING_AGENT_DIR` si défini, sinon l’agent par défaut
configuré.
...
- `models list` est en lecture seule : il lit la configuration, les profils d’authentification, l’état de catalogue
  ...
  `models.json`.
- La colonne `Auth` est au niveau fournisseur et en lecture seule. Elle est calculée à partir des métadonnées locales
  des profils d’authentification, des marqueurs d’environnement, des clés fournisseur configurées, des marqueurs
  ...
  fournisseur et ne prouve pas que l’exécution exacte par modèle est prête.
- `models list --all --provider <id>` peut inclure des lignes de catalogue statiques détenues par le fournisseur
  depuis des manifestes de Plugin ou des métadonnées de catalogue fournisseur groupées, même lorsque vous
  ne vous êtes pas encore authentifié auprès de ce fournisseur. Ces lignes restent affichées comme
  indisponibles tant que l’authentification correspondante n’est pas configurée.
- `models list` garde le plan de contrôle réactif lorsque la découverte du catalogue fournisseur
  ...
  que vous acceptez d’attendre la découverte fournisseur.
- Un large `models list --all` fusionne les lignes de catalogue de manifeste avec les lignes de registre
  sans charger les hooks de supplément de runtime fournisseur. Les chemins rapides de manifeste filtrés par fournisseur
  utilisent uniquement les fournisseurs marqués `static` ; les fournisseurs marqués `refreshable`
  restent adossés au registre/cache et ajoutent les lignes de manifeste en suppléments, tandis que
  les fournisseurs marqués `runtime` restent sur la découverte registre/runtime.
- `models list` garde les métadonnées natives de modèle et les limites de runtime distinctes. Dans la sortie en tableau,
  `Ctx` affiche `contextTokens/contextWindow` lorsqu’une limite de runtime effective
  diffère de la fenêtre de contexte native ; les lignes JSON incluent `contextTokens`
  lorsqu’un fournisseur expose cette limite.
- `models list --provider <id>` filtre par identifiant fournisseur, comme `moonshot` ou
  `openai-codex`. Il n’accepte pas les libellés d’affichage des sélecteurs de fournisseurs
  interactifs, comme `Moonshot AI`.
- Les références de modèle sont analysées en les scindant sur le **premier** `/`. Si l’ID de modèle inclut `/` (style OpenRouter), incluez le préfixe fournisseur (exemple : `openrouter/moonshotai/kimi-k2`).
- Si vous omettez le fournisseur, OpenClaw résout d’abord l’entrée comme un alias, puis
  ...
  se rabat sur le premier fournisseur/modèle configuré au lieu d’afficher une
...
et explique que les modèles `:free` nécessitent tout de même `OPENROUTER_API_KEY` pour
...
Cas de détail/code de motif de sonde à prévoir :

- `excluded_by_auth_order` : un profil stocké existe, mais `auth.order.<provider>` explicite
  l’a omis, donc la sonde signale l’exclusion au lieu de
  l’essayer.
...
(OAuth/clé d’API) ou vous guider vers un collage manuel de jeton, selon le
...
afficher de jeton, de clé d’API ni de secret OAuth. Utilisez `--provider <id>` pour
...
- `paste-token` nécessite `--provider`, demande la valeur du jeton et l’écrit
  ...
  `--profile-id`.
- `paste-token --expires-in <duration>` stocke une expiration absolue de jeton depuis une
  durée relative comme `365d` ou `12h`.
- Note Anthropic : le personnel d’Anthropic nous a indiqué que l’utilisation de Claude CLI de style OpenClaw est de nouveau autorisée, OpenClaw traite donc la réutilisation de Claude CLI et l’utilisation de `claude -p` comme approuvées pour cette intégration, sauf si Anthropic publie une nouvelle politique.
