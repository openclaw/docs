---
read_when:
    - Vous voulez modifier les modèles par défaut ou consulter l’état d’authentification du fournisseur
    - Vous voulez analyser les modèles/fournisseurs disponibles et déboguer les profils d’authentification
summary: Référence CLI pour `openclaw models` (status/list/set/scan, alias, mécanismes de repli, auth)
title: Modèles
x-i18n:
...
`openclaw models status` affiche le modèle par défaut et les replis résolus, ainsi qu’une vue d’ensemble de l’authentification.
Quand des instantanés d’utilisation des fournisseurs sont disponibles, la section d’état OAuth/clé API inclut
...
Codex, MiniMax, Xiaomi et z.ai. L’authentification d’utilisation provient de hooks propres aux fournisseurs
lorsqu’ils sont disponibles ; sinon OpenClaw se rabat sur les identifiants
OAuth/clé API correspondants depuis les profils d’authentification, l’environnement ou la configuration.
Dans la sortie `--json`, `auth.providers` est la vue d’ensemble des fournisseurs tenant compte de l’environnement, de la configuration et du magasin,
tandis que `auth.oauth` concerne uniquement l’état des profils du magasin d’authentification.
...
Utilisez `--agent <id>` pour inspecter l’état modèle/authentification d’un agent configuré. En l’absence de cet argument,
...
Pour dépanner l’OAuth Codex, `openclaw models status`,
...
confirmer si un agent utilise `openai-codex/*` via PI ou `openai/*`
via le runtime Codex natif. Consultez [configuration du fournisseur OpenAI](/fr/providers/openai#check-and-recover-codex-oauth-routing).
...
- `models list` est en lecture seule : il lit la configuration, les profils d’authentification, l’état existant du catalogue
  et les lignes de catalogue détenues par les fournisseurs, mais ne réécrit pas
  `models.json`.
- La colonne `Auth` est au niveau fournisseur et en lecture seule. Elle est calculée à partir des
  métadonnées des profils d’authentification locaux, des marqueurs d’environnement, des clés fournisseur configurées, des marqueurs de fournisseur local,
  des marqueurs d’environnement/profil AWS Bedrock et des métadonnées d’authentification synthétique des plugins ;
  elle ne charge pas le runtime du fournisseur, ne lit pas les secrets du trousseau, n’appelle pas les
  API fournisseur et ne prouve pas la disponibilité exacte d’exécution par modèle.
- `models list --all --provider <id>` peut inclure des lignes de catalogue statiques détenues par le fournisseur
  depuis les manifestes de plugins ou les métadonnées de catalogue de fournisseurs intégrés, même si vous
  ne vous êtes pas encore authentifié auprès de ce fournisseur. Ces lignes s’affichent toujours comme
  indisponibles tant qu’une authentification correspondante n’est pas configurée.
- `models list` garde le plan de contrôle réactif lorsque la découverte de catalogue du fournisseur
  ...
  que vous acceptez d’attendre la découverte du fournisseur.
- Un large `models list --all` fusionne les lignes de catalogue des manifestes par-dessus les lignes du registre
...
- `models list` garde distincts les métadonnées natives du modèle et les plafonds du runtime. Dans la sortie en tableau,
...
- Si vous omettez le fournisseur, OpenClaw résout d’abord l’entrée comme un alias, puis
  comme une correspondance unique de fournisseur configuré pour cet identifiant exact de modèle, et seulement ensuite
  ...
  se rabat sur le premier fournisseur/modèle configuré au lieu de signaler une
  valeur par défaut périmée d’un fournisseur supprimé.
...
une utilisation en repli. Le catalogue lui-même est public, donc les analyses limitées aux métadonnées ne nécessitent pas
de clé OpenRouter.
...
et de démarrage sont acheminés vers stderr afin que les scripts puissent rediriger stdout directement
...
- `excluded_by_auth_order` : un profil stocké existe, mais l’option explicite
...
fournisseur (OAuth/clé API) ou vous guider vers le collage manuel d’un jeton, selon le
...
afficher de jeton, de clé API ni de matériau secret OAuth. Utilisez `--provider <id>` pour
