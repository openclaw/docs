...
title: Configuration
x-i18n:
    generated_at: "2026-06-27T17:17:37Z"
    model: gpt-5.5
...
<AccordionGroup>
  <Accordion title="Ce qu’il inclut">
    ...
    - Les métadonnées de documentation de champs `title` et `description` utilisées par l’interface Control UI.
    ...
    - Métadonnées de schéma de Plugin + canal en direct au mieux lorsque les manifestes runtime peuvent être chargés.
    - Un schéma de repli propre même lorsque la configuration actuelle est invalide.

  </Accordion>
  <Accordion title="RPC runtime associé">
    `config.schema.lookup` renvoie un chemin de configuration normalisé avec un nœud de schéma superficiel (`title`, `description`, `type`, `enum`, `const`, limites communes), les métadonnées d’indice d’interface correspondantes et les résumés des enfants immédiats. Utilisez-le pour l’exploration par chemin dans Control UI ou dans des clients personnalisés.
  </Accordion>
</AccordionGroup>
...
Les chemins utilisent la notation par points ou par crochets. Mettez les chemins en notation par crochets entre guillemets dans les exemples shell afin que les shells comme zsh ne développent pas `[0]` comme un glob avant qu’OpenClaw reçoive le chemin :
...
Utilisez `config patch` lorsque vous voulez coller ou rediriger un correctif en forme de configuration au lieu d’exécuter de nombreuses commandes `config set` fondées sur des chemins. L’entrée est un objet JSON5. Les objets fusionnent récursivement, les tableaux et les valeurs scalaires remplacent la valeur cible, et `null` supprime le chemin cible.
...
Vous pouvez aussi rediriger un correctif via stdin, ce qui est utile pour les scripts de configuration distants :
...
`--dry-run` exécute les vérifications de schéma et de résolubilité SecretRef sans écrire. Les SecretRefs adossées à exec sont ignorées par défaut pendant le dry-run ; ajoutez `--allow-exec` lorsque vous voulez intentionnellement que le dry-run exécute les commandes de fournisseur.
...
<AccordionGroup>
  <Accordion title="Comportement du dry-run">
    - Mode générateur : exécute les vérifications de résolubilité SecretRef pour les refs/fournisseurs modifiés.
    ...
    - Utilisez `--allow-exec` avec `--dry-run` pour accepter les vérifications SecretRef exec (cela peut exécuter des commandes de fournisseur).
...
<AccordionGroup>
  <Accordion title="If dry-run fails">
    ...
    - `SecretRef assignment(s) could not be resolved` : le provider/ref référencé ne peut actuellement pas être résolu (variable d’environnement manquante, pointeur de fichier invalide, échec du fournisseur exec ou incohérence provider/source).
    - `Dry run note: skipped <n> exec SecretRef resolvability check(s)` : la simulation a ignoré les refs exec ; relancez avec `--allow-exec` si vous avez besoin de valider la résolvabilité exec.
    - En mode lot, corrigez les entrées en échec et relancez `--dry-run` avant l’écriture.
...
`openclaw config set` et les autres outils d’écriture de configuration détenus par OpenClaw valident toute la configuration après modification avant de l’enregistrer sur le disque. Si la nouvelle charge utile échoue à la validation de schéma ou ressemble à un écrasement destructeur, la configuration active reste inchangée et la charge utile rejetée est enregistrée à côté sous le nom `openclaw.json.rejected.*`.

<Warning>
Le chemin de configuration actif doit être un fichier standard. Les agencements `openclaw.json` sous forme de lien symbolique ne sont pas pris en charge pour les écritures ; utilisez plutôt `OPENCLAW_CONFIG_PATH` pour pointer directement vers le fichier réel.
</Warning>
...
Si une écriture est rejetée, inspectez la charge utile enregistrée et corrigez toute la forme de configuration :
...
Les écritures directes avec un éditeur restent autorisées, mais le Gateway en cours d’exécution les traite comme non fiables tant qu’elles ne sont pas validées. Les modifications directes invalides font échouer le démarrage ou sont ignorées par le rechargement à chaud ; Gateway ne réécrit pas `openclaw.json`. Exécutez `openclaw doctor --fix` pour réparer une configuration préfixée/écrasée ou restaurer la dernière copie connue comme valide. Consultez [Dépannage de Gateway](/fr/gateway/troubleshooting#gateway-rejected-invalid-config).

La récupération de fichier entier est réservée à la réparation par doctor. Les changements de schéma de Plugin ou les décalages de `minHostVersion` restent visibles au lieu de restaurer des paramètres utilisateur sans rapport, comme la configuration des modèles, providers, profils d’authentification, canaux, exposition du Gateway, outils, mémoire, navigateur ou Cron.

## Sous-commandes

- `config file` : affiche le chemin du fichier de configuration actif (résolu depuis `OPENCLAW_CONFIG_PATH` ou l’emplacement par défaut). Le chemin doit désigner un fichier standard, pas un lien symbolique.
...
Une fois `openclaw config validate` réussi, vous pouvez utiliser le TUI local pour qu’un agent intégré compare la configuration active à la documentation pendant que vous validez chaque changement depuis le même terminal :
