...
x-i18n:
    generated_at: "2026-07-12T02:26:44Z"
    model: gpt-5.6
...
Il n’existe pas de commande de premier niveau `openclaw flows`. L’inspection des TaskFlows durables s’effectue sous `openclaw tasks flow`.
...
L’option `--status` de `list` accepte l’une des valeurs suivantes : `queued`, `running`, `waiting`, `blocked`, `succeeded`, `failed`, `cancelled`, `lost`.
