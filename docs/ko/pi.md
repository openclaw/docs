...
read_when:
    ...
    - Pi의 에이전트 세션 수명 주기, 도구 체계 또는 제공자 연결 구성 수정
summary: OpenClaw의 내장 Pi 에이전트 통합 및 세션 수명 주기 아키텍처
title: Pi 통합 아키텍처
x-i18n:
    generated_at: "2026-05-06T06:32:23Z"
    model: gpt-5.5
    provider: openai
...
OpenClaw은 [pi-coding-agent](https://github.com/badlogic/pi-mono/tree/main/packages/coding-agent) 및 그 형제 패키지(`pi-ai`, `pi-agent-core`, `pi-tui`)와 통합되어 AI 에이전트 기능을 구동합니다.

## 개요

OpenClaw은 pi SDK를 사용하여 AI 코딩 에이전트를 메시징 Gateway 아키텍처에 임베드합니다. pi를 하위 프로세스로 생성하거나 RPC 모드를 사용하는 대신, OpenClaw은 `createAgentSession()`을 통해 pi의 `AgentSession`을 직접 가져와 인스턴스화합니다. 이 임베드 방식은 다음을 제공합니다.
...
| `pi-coding-agent` | 고수준 SDK: `createAgentSession`, `SessionManager`, `AuthStorage`, `ModelRegistry`, 기본 제공 도구 |
...
이제 채널별 메시지 작업 런타임은 `src/agents/tools` 아래가 아니라 Plugin이 소유한 확장
...
SDK는 LLM으로 전송, 도구 호출 실행, 응답 스트리밍까지 전체 에이전트 루프를 처리합니다.

이미지 주입은 프롬프트 로컬입니다. OpenClaw은 현재 프롬프트에서 이미지 참조를 로드하고
...
2. **사용자 지정 대체**: OpenClaw은 bash를 `exec`/`process`로 대체하고, 샌드박스용으로 read/edit/write를 사용자 지정합니다
3. **OpenClaw 도구**: 메시징, 브라우저, 캔버스, 세션, Cron, Gateway 등
4. **채널 도구**: Discord/Telegram/Slack/WhatsApp별 작업 도구
5. **정책 필터링**: 프로필, 제공자, 에이전트, 그룹, 샌드박스 정책에 따라 도구 필터링
...
이렇게 하면 OpenClaw의 정책 필터링, 샌드박스 통합, 확장 도구 모음이 제공자 전반에서 일관되게 유지됩니다.

## 시스템 프롬프트 구성

시스템 프롬프트는 `buildAgentSystemPrompt()`(`system-prompt.ts`)에서 빌드됩니다. 이 함수는 도구, 도구 호출 스타일, 안전 가드레일, OpenClaw CLI 참조, Skills, 문서, 작업 영역, 샌드박스, 메시징, 응답 태그, 음성, 무음 응답, Heartbeat, 런타임 메타데이터를 포함하고, 활성화된 경우 메모리와 반응, 그리고 선택적 컨텍스트 파일과 추가 시스템 프롬프트 콘텐츠를 포함하는 섹션들로 전체 프롬프트를 조립합니다. 하위 에이전트에서 사용하는 최소 프롬프트 모드에서는 섹션이 다듬어집니다.
...
`limitHistoryTurns()`는 채널 유형(DM과 그룹)에 따라 대화 기록을 잘라냅니다.
...
OpenClaw는 제공자별 여러 API 키를 가진 인증 프로필 저장소를 유지합니다.
...
1. **도구 시그니처 정렬**: 현재 pi-agent-core와 pi-coding-agent 시그니처 간에 조정 중
2. **세션 관리자 래핑**: `guardSessionManager`는 안전성을 추가하지만 복잡도를 높임
3. **확장 로딩**: pi의 `ResourceLoader`를 더 직접적으로 사용할 수 있음
4. **스트리밍 핸들러 복잡도**: `subscribeEmbeddedPiSession`이 커졌음
5. **제공자 특이 동작**: pi가 잠재적으로 처리할 수 있는 제공자별 코드 경로가 많음

## 테스트

Pi 통합 범위는 다음 테스트 모음에 걸쳐 있습니다.
