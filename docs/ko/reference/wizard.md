...
summary: 'CLI 온보딩 전체 참조 문서: 모든 단계, 플래그, 구성 필드'
title: 온보딩 참조
x-i18n:
...
이는 `openclaw onboard`의 전체 참조입니다.
...
<Steps>
  <Step title="기존 구성 감지">
    ...
    - 온보딩을 다시 실행해도 명시적으로 **재설정**을 선택하지 않는 한 아무것도 지워지지 않습니다
      (`--reset`을 전달한 경우도 해당).
    - CLI `--reset`의 기본값은 `config+creds+sessions`입니다. workspace도 제거하려면 `--reset-scope full`을 사용하세요.
    - 구성이 유효하지 않거나 레거시 키가 포함되어 있으면 마법사가 중지되고 계속하기 전에
  ...
  <Step title="모델/인증">
    ...
    - **MiniMax**: 구성이 자동으로 작성되며, 호스팅 기본값은 `MiniMax-M2.7`입니다.
...
<Note>
GUI가 감지되지 않으면 온보딩은 브라우저를 여는 대신 Control UI용 SSH 포트 전달 지침을 출력합니다.
...
제공자별 명령 예제는 [CLI 자동화](/ko/start/wizard-cli-automation#provider-specific-examples)에 있습니다.
