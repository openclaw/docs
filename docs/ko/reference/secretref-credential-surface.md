...
read_when:
    ...
    - 자격 증명이 지원 범위를 벗어난 이유 확인
summary: SecretRef 자격 증명 표면의 표준 지원 및 미지원 범위
title: SecretRef 자격 증명 서피스
x-i18n:
    generated_at: "2026-05-03T21:37:56Z"
    model: gpt-5.5
    provider: openai
...
- 범위 밖: 런타임에서 발급되거나 순환되는 자격 증명, OAuth 갱신 자료, 세션과 유사한 아티팩트.
...
- `profiles.*.keyRef` (`type: "api_key"`; `auth.profiles.<id>.mode = "oauth"`일 때 지원되지 않음)
- `profiles.*.tokenRef` (`type: "token"`; `auth.profiles.<id>.mode = "oauth"`일 때 지원되지 않음)
...
- 인증 프로필 참조는 런타임 해석 및 감사 범위에 포함됩니다.
...
- OAuth 정책 가드: `auth.profiles.<id>.mode = "oauth"`는 해당 프로필의 SecretRef 입력과 함께 사용할 수 없습니다. 이 정책을 위반하면 시작/다시 로드 및 인증 프로필 해석이 빠르게 실패합니다.
- SecretRef가 관리하는 모델 제공자의 경우, 생성된 `agents/*/agent/models.json` 항목은 `apiKey`/헤더 표면에 대해 비밀이 아닌 마커(해석된 비밀 값이 아님)를 유지합니다.
- 마커 지속성은 소스 권위적입니다. OpenClaw는 해석된 런타임 비밀 값이 아니라 활성 소스 구성 스냅샷(해석 전)의 마커를 씁니다.
- 웹 검색의 경우:
  - 명시적 제공자 모드(`tools.web.search.provider` 설정)에서는 선택한 제공자 키만 활성화됩니다.
  - 자동 모드(`tools.web.search.provider` 미설정)에서는 우선순위에 따라 해석되는 첫 번째 제공자 키만 활성화됩니다.
  - 자동 모드에서 선택되지 않은 제공자 참조는 선택될 때까지 비활성으로 처리됩니다.
...
- 이러한 자격 증명은 발급되었거나, 순환되거나, 세션을 포함하거나, OAuth로 지속되는 유형이므로 읽기 전용 외부 SecretRef 해석에 맞지 않습니다.
