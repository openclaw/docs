...
read_when:
    ...
    - 앱 SDK에 대한 초안 네임스페이스, 이벤트, 결과, 아티팩트, 승인 또는 보안 계약이 필요합니다
    - Gateway 프로토콜 리소스와 고수준 OpenClaw App SDK 래퍼를 비교하고 있습니다
sidebarTitle: App SDK API design
summary: 공개 OpenClaw App SDK API, 이벤트 분류 체계, 아티팩트, 승인 및 패키지 구조를 위한 참조 설계
...
이 페이지는 공개
[OpenClaw App SDK](/ko/concepts/openclaw-sdk)의 상세 API 참조 설계입니다. 의도적으로
[Plugin SDK](/ko/plugins/sdk-overview)와 분리되어 있습니다.

<Note>
  `@openclaw/sdk`는 Gateway와 통신하기 위한 외부 앱/클라이언트 패키지입니다.
  `openclaw/plugin-sdk/*`는 인프로세스 Plugin 작성 계약입니다.
  에이전트를 실행하기만 하면 되는 앱에서는 Plugin SDK 하위 경로를 가져오지 마세요.
</Note>

공개 앱 SDK는 두 계층으로 구축해야 합니다.

1. 저수준으로 생성된 Gateway 클라이언트.
2. `OpenClaw`, `Agent`, `Session`, `Run`,
   `Task`, `Artifact`, `Approval`, `Environment` 객체를 제공하는 고수준의 사용하기 쉬운 래퍼.
...
`id`는 재생 커서입니다. 소비자는
`events({ after: id })`로 다시 연결하고 보존 기간이 허용하는 경우 놓친 이벤트를 받을 수 있어야 합니다.
...
| `assistant.message`   | 전체 어시스턴트 메시지 또는 대체 메시지.                  |
| `thinking.delta`      | 정책이 노출을 허용하는 경우의 추론 또는 계획 델타.       |
...
| `approval.resolved`   | 승인이 허가, 거부, 만료 또는 취소되었습니다.        |
...
| `git.branch`          | 런타임이 브랜치 상태를 관찰하거나 변경했습니다.                   |
| `git.diff`            | 런타임이 diff를 생성하거나 변경했습니다.                         |
| `git.pr`              | 런타임이 풀 리퀘스트를 열거나, 업데이트하거나, 연결했습니다.          |

런타임 네이티브 페이로드는 `raw`를 통해 사용할 수 있어야 하지만, 앱이 일반 UI를 위해
`raw`를 파싱할 필요는 없어야 합니다.
...
결과는 단순하고 안정적이어야 합니다. 타임스탬프 값은 Gateway
형태를 보존하므로, 현재 수명 주기 기반 실행은 일반적으로 에포크 밀리초 숫자를 보고하지만
어댑터는 여전히 ISO 문자열을 표시할 수 있습니다. 풍부한 UI, 도구 추적, 런타임 네이티브 세부 정보는 이벤트와 아티팩트에 속합니다.

`accepted`는 비종결 대기 결과입니다. 이는 실행이 수명 주기 종료/오류를 생성하기 전에 Gateway 대기 기한이 만료되었음을 의미합니다. 이를
`timed_out`으로 취급해서는 안 됩니다. `timed_out`은 실행이 자체 런타임 제한 시간을 초과한 경우에만 사용됩니다.

## 승인 및 질문

코딩 에이전트는 안전 경계를 지속적으로 넘나들기 때문에 승인은 일급 개념이어야 합니다.
...
앱은 Plugin 내부를 가져오지 않고도 도구 표면을 이해할 수 있어야 합니다.
...
- 승인 정책
- 런타임 호환성
- 도구가 숨겨졌는지, 읽기 전용인지, 쓰기 가능한지, 또는 호스트 가능인지 여부
...
아티팩트 접근은 모든 아티팩트가 일반적인 로컬 파일이라고 가정하지 않고 삭제 처리, 보존, 다운로드 URL을 지원해야 합니다.

## 보안 모델

앱 SDK는 권한을 명시적으로 다뤄야 합니다.
...
| `environment.write` | 관리형 환경을 생성하거나 삭제합니다.             |
| `admin`             | 관리 작업.                          |
...
- 명시적 샌드박스 및 네트워크 정책
- 명시적 원격 환경 보존
- 정책이 달리 입증하지 않는 한 호스트 실행에 대한 승인
- 호출자가 더 강한 진단 범위를 갖지 않는 한, 원시 런타임 이벤트는 Gateway를 벗어나기 전에 삭제 처리됨
...
첫 구현은 호스팅 SaaS일 필요가 없습니다. 기존 Node 호스트, 임시 작업 영역, CI 스타일 러너 또는 Testbox 스타일 환경을 대상으로 할 수 있습니다. 중요한 계약은 다음과 같습니다.
...
이 저장소에는 이미 Plugin용 `openclaw/plugin-sdk/*`가 있습니다. Plugin 작성자와 앱 개발자를 혼동하지 않도록 해당 네임스페이스를 분리해 두세요.

## 생성된 클라이언트 전략

저수준 클라이언트는 버전이 지정된 Gateway 프로토콜 스키마에서 생성한 다음, 손으로 작성한 사용하기 쉬운 클래스로 래핑해야 합니다.
...
- 앱 SDK는 Plugin SDK 내부 구현과 독립적으로 유지됩니다
- 저수준 사용자는 여전히 전체 프로토콜에 접근할 수 있습니다
- 고수준 사용자는 작은 제품 API를 얻습니다
