...
summary: 게시 전 ClawHub Plugin 패키지 유효성 검사 발견 사항 수정
title: Plugin 검증 수정 사항
x-i18n:
    generated_at: "2026-07-03T09:27:43Z"
    model: gpt-5.5
...
ClawHub는 게시 전에 Plugin 패키지를 검증하며, 자동 패키지 스캔에서 나온 발견 사항도 표시할 수 있습니다. 이 페이지는 작성자 대상 발견 사항을 다룹니다. 즉 Plugin 작성자가 패키지 메타데이터, 매니페스트, SDK 가져오기 또는 게시된 아티팩트에서 수정할 수 있는 발견 사항입니다.

내부 Plugin Inspector 커버리지 발견 사항은 다루지 않습니다. 전체 보고서에 작성자용 수정 지침이 없는 스캐너 유지보수 코드가 포함되어 있다면, 해당 코드는 Plugin 작성자가 아니라 OpenClaw 유지관리자를 위한 것입니다.
...
- [Plugin 진입점](/ko/plugins/sdk-entrypoints)과 [검색에 영향을 주는 package.json 필드](/ko/plugins/manifest#packagejson-fields-that-affect-discovery)를 참조하세요.
- `clawhub package validate <path-to-plugin>`을 다시 실행합니다.
...
ClawHub가 패키지를 어떻게 설치하거나 업데이트해야 하는지 확인할 수 없습니다.
...
- 이것을 패키지 버전과 분리해서 유지합니다. 패키지 버전은 Plugin 릴리스를 설명하고, `openclaw.compat.pluginApi`는 호스트 API 계약을 설명합니다.
- [검색에 영향을 주는 package.json 필드](/ko/plugins/manifest#packagejson-fields-that-affect-discovery)를 참조하세요.
- `clawhub package validate <path-to-plugin>`을 다시 실행합니다.
...
- `openclaw.plugin.json`에도 `version`이 있는 경우, 패키지 메타데이터가 권위 있는 정보라면 이를 일치하도록 업데이트하거나 오래된 매니페스트 버전 메타데이터를 제거합니다.
- 게시된 메타데이터를 변경한 뒤 새 패키지 버전을 게시합니다.
...
- `name`은 사람이 읽기 쉬운 형태로 유지하고, `id`는 안정적인 머신 ID로 유지합니다.
- [Plugin 매니페스트](/ko/plugins/manifest)를 참조하세요.
...
- 루트 배럴 가져오기를 초점이 맞춰진 공개 하위 경로 가져오기로 교체합니다.
...
- 선언된 호환성 범위가 이를 요구하는 이전 OpenClaw 버전을 아직 지원하는 동안에만
...
Plugin이 여전히 `saveSessionStore` 또는 `updateSessionStore`와 같은 사용 중단된 전체 세션 저장소 쓰기 헬퍼를 사용합니다.
...
Plugin이 여전히 `resolveSessionFilePath` 또는 `resolveAndPersistSessionFile`과 같은 사용 중단된 세션 파일 경로 헬퍼를 사용합니다.
...
Plugin이 여전히 `appendSessionTranscriptMessage` 또는 `emitSessionTranscriptUpdate`와 같은 사용 중단된 저수준 transcript 헬퍼를 사용합니다.
...
- 지원되는 OpenClaw 범위에 아직 필요한 동안에만 `providerAuthEnvVars`를 호환성 메타데이터로 유지합니다.
...
매니페스트가 ClawHub가 기대하는 현재 setup 또는 구성 메타데이터 없이 레거시 또는 이전 채널 env-var 메타데이터를 사용합니다.

- OpenClaw가 채널 런타임을 로드하지 않고도 setup 상태를 검사할 수 있도록 채널 env-var 메타데이터를 선언적으로 유지합니다.
- env 기반 채널 setup을 Plugin 형태에서 사용하는 현재 setup, 채널 구성 또는 패키지 채널 메타데이터로 미러링합니다.
...
- 매니페스트 계약이 존재할 때까지 보안에 민감한 동작은 공개 패키지 문서 또는 README에 문서화합니다.
- `clawhub package validate <path-to-plugin>`를 다시 실행합니다.
