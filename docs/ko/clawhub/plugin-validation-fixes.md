...
- `clawhubSpec`, `npmSpec` 또는 `localPath`와 같은 지원되는 설치 소스로 `openclaw.install`을 채우세요.
...
- 패키지가 실제로 지원하는 호스트 버전 범위에 최소 호스트 버전을 맞추세요.
- [탐색에 영향을 주는 package.json 필드](/ko/plugins/manifest#packagejson-fields-that-affect-discovery)를 참조하세요.
- `clawhub package validate <path-to-plugin>`을 다시 실행하세요.
...
- 패킹 실패를 유발하는 잘못된 패키지 메타데이터, 손상된 생명주기 스크립트 또는 files 항목을 수정하세요.
...
- 에이전트 및 세션 ID로 세션 메타데이터를 읽으려면 `getSessionEntry(...)`를 사용하세요.
...
- 지원하는 OpenClaw 범위에서 아직 필요로 하는 동안에만
...
- 지원되는 이전 OpenClaw 버전에서 아직 필요로 하는 동안에만
  `channelEnvVars`를 호환성 메타데이터로 유지하세요.
...
- OpenClaw가 스키마를 게시한 뒤에만 문서화된 버전 스키마를 사용하세요.
- `clawhub package validate <path-to-plugin>`을 다시 실행하세요.
