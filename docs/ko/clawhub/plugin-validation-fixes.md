...
ClawHub는 게시 전에 Plugin 패키지를 검증하며, 자동 패키지 스캔 결과도 표시할 수 있습니다. 이 페이지는 작성자 대상 결과를 다룹니다. 즉, Plugin 작성자가 패키지 메타데이터, 매니페스트, SDK 가져오기 또는 게시된 아티팩트에서 수정할 수 있는 결과를 의미합니다.

내부 Plugin Inspector 커버리지 결과는 다루지 않습니다. 전체 보고서에 작성자 수정 지침이 없는 스캐너 유지관리 코드가 포함되어 있다면, 이는 Plugin 작성자가 아니라 OpenClaw 유지관리자를 위한 것입니다.
...
- 최소 패키지 예시는 [Plugin 빌드](/ko/plugins/building-plugins)를, 패키지와 매니페스트의 분리는 [Plugin 매니페스트](/ko/plugins/manifest#manifest-versus-packagejson)를 참조하세요.
...
패키지의 최소 호스트 버전이 패키지가 빌드된 기준의 OpenClaw 버전 메타데이터와 일치하지 않습니다.
...
- 런타임 동작은 Plugin 등록 코드로 이동하고, `contracts`는 정적 capability 소유권 메타데이터로 제한하세요.
- `clawhub package validate <path-to-plugin>`을 다시 실행하세요.
...
- 루트 배럴 import를 초점이 맞춰진 공개 하위 경로 import로 바꾸세요.
...
Plugin이 `saveSessionStore` 또는 `updateSessionStore` 같은 더 이상 권장되지 않는 전체 세션 저장소 쓰기 헬퍼를 아직 사용합니다.

- 기존 세션 엔트리의 필드를 업데이트할 때는 `patchSessionEntry(...)`를 사용하세요.
- 세션 엔트리를 교체하거나 생성할 때는 `upsertSessionEntry(...)`를 사용하세요.
- 전체 세션 저장소 객체를 로드하고, 변경하고, 저장하는 방식을 피하세요.
- 선언한 호환성 범위가 이를 필요로 하는 이전 OpenClaw 버전을 여전히 지원하는 동안에만
  전체 저장소 쓰기 헬퍼를 유지하세요.
- [Runtime API](/ko/plugins/sdk-runtime#agent-session-state) 및
...
Plugin이 `resolveSessionFilePath` 또는 `resolveAndPersistSessionFile` 같은 더 이상 권장되지 않는 세션 파일 경로 헬퍼를 아직 사용합니다.

- 에이전트와 세션 ID별 세션 메타데이터를 읽으려면 `getSessionEntry(...)`를 사용하세요.
...
`resolveSessionTranscriptLegacyFileTarget`를 아직 사용합니다.

- 코드가 공개 세션 ID만 필요로 하는 경우 `resolveSessionTranscriptIdentity(...)`를 사용하세요.
- 코드가 구조화된 transcript 작업 대상을 필요로 하는 경우 `resolveSessionTranscriptTarget(...)`을 사용하세요.
...
Plugin이 `appendSessionTranscriptMessage` 또는 `emitSessionTranscriptUpdate` 같은 더 이상 권장되지 않는 저수준 transcript 헬퍼를 아직 사용합니다.
...
Plugin이 레거시 `before_agent_start` 훅을 아직 사용합니다.

- 모델 또는 provider override 작업은 `before_model_resolve`로 이동하세요.
...
매니페스트가 레거시 `providerAuthEnvVars` provider auth 메타데이터를 아직 사용합니다.

- provider env-var 메타데이터를 `setup.providers[].envVars`에 미러링하세요.
- 지원하는 OpenClaw 범위가 여전히 필요로 하는 동안에만 `providerAuthEnvVars`를 호환성 메타데이터로 유지하세요.
...
매니페스트가 ClawHub에서 기대하는 현재 setup 또는 config 메타데이터 없이 레거시 또는 이전 channel env-var 메타데이터를 사용합니다.

- OpenClaw가 채널 런타임을 로드하지 않고도 setup 상태를 검사할 수 있도록 channel env-var 메타데이터를 선언적으로 유지하세요.
- env 기반 채널 setup을 Plugin 형태에서 사용하는 현재 setup, 채널 config 또는 패키지 채널 메타데이터로 미러링하세요.
- 이전 지원 OpenClaw 버전이 여전히 필요로 하는 동안에만 `channelEnvVars`를 호환성 메타데이터로 유지하세요.
...
패키지가 ClawHub가 사용 가능하다고 인식하지 않는 스키마 참조와 함께 `openclaw.security.json`을 배포합니다.

- advisory 용도일 뿐이라면 스키마 URL을 제거하세요.
- OpenClaw가 문서화된 버전 스키마를 게시한 뒤에만 사용하세요.
- `clawhub package validate <path-to-plugin>`을 다시 실행하세요.

### unrecognized-security-manifest

패키지가 지원되지 않는 보안 매니페스트 파일을 배포합니다.

- OpenClaw가 버전이 지정된 보안 매니페스트 스키마와 ClawHub 동작을 문서화할 때까지 `openclaw.security.json`을 제거하세요.
