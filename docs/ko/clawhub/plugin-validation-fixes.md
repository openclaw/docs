...
ClawHub는 게시 전에 Plugin 패키지를 검증하며 자동 패키지 스캔의 발견 사항도 표시할 수 있습니다. 이 페이지에서는 작성자 대상 발견 사항, 즉 Plugin 작성자가 패키지 메타데이터, 매니페스트, SDK 가져오기 또는 게시된 아티팩트에서 수정할 수 있는 발견 사항을 다룹니다.
...
- `name`, `version`, `type`이 있는 `package.json`을 추가합니다.
- 패키지가 OpenClaw Plugin을 제공하는 경우 `openclaw` 블록을 추가합니다.
- 최소 패키지 예제는 [Plugin 빌드](/ko/plugins/building-plugins)를, 패키지와 매니페스트 분리는 [Plugin 매니페스트](/ko/plugins/manifest#manifest-versus-packagejson)를 사용하세요.
...
패키지 메타데이터는 있지만 OpenClaw 런타임 진입점을 선언하지 않았습니다.
...
- 설치 소스가 둘 이상 제공되는 경우 `openclaw.install.defaultChoice`를 설정합니다.
...
패키지 최소 호스트 버전이 패키지가 빌드된 기준의 OpenClaw 버전 메타데이터와 일치하지 않습니다.
...
- `openclaw.plugin.json`에도 `version`이 있으면 일치하도록 업데이트하거나, 패키지 메타데이터가 권위 있는 경우 오래된 매니페스트 버전 메타데이터를 제거합니다.
- 게시된 메타데이터를 변경한 후에는 새 패키지 버전을 게시합니다.
...
- 이 패키지가 공개 게시 대상이면 `private: true`를 제거합니다.
- `clawhub package validate <path-to-plugin>`을 다시 실행합니다.
...
- 루트 배럴 가져오기를 집중된 공개 하위 경로 가져오기로 교체하세요.
...
Plugin이 여전히 지원 중단된 전체 세션 저장소 헬퍼
...
Plugin이 여전히 `saveSessionStore` 또는 `updateSessionStore` 같은 지원 중단된 전체 세션 저장소 쓰기 헬퍼를 사용합니다.
...
Plugin이 여전히 `resolveSessionFilePath` 또는 `resolveAndPersistSessionFile` 같은 지원 중단된 세션 파일 경로 헬퍼를 사용합니다.

- 에이전트 및 세션 식별자별로 세션 메타데이터를 읽으려면 `getSessionEntry(...)`를 사용하세요.
- 세션 메타데이터를 유지하려면 `patchSessionEntry(...)` 또는 `upsertSessionEntry(...)`를 사용하세요.
- 코드가 transcript 작업을 준비하는 경우 transcript 식별자 또는 대상 헬퍼를 사용하세요.
...
Plugin이 여전히 지원 중단된 transcript 파일 대상 헬퍼
`resolveSessionTranscriptLegacyFileTarget`을 사용합니다.

- 코드에 공개 세션 식별자만 필요한 경우 `resolveSessionTranscriptIdentity(...)`를 사용하세요.
- 코드에 구조화된 transcript 작업 대상이 필요한 경우 `resolveSessionTranscriptTarget(...)`을 사용하세요.
...
Plugin이 여전히 `appendSessionTranscriptMessage` 또는 `emitSessionTranscriptUpdate` 같은 지원 중단된 저수준 transcript 헬퍼를 사용합니다.
...
- OpenClaw가 올바른 트랜잭션 경계와 식별자 처리를 적용할 수 있도록 구조화된 transcript 런타임 표면을 선호하세요.
- 선언한 호환성 범위가 이를 요구하는 이전 OpenClaw 버전을 여전히 지원하는 동안에만
...
- 제공자 env-var 메타데이터를 `setup.providers[].envVars`에 미러링하세요.
- 지원하는 OpenClaw 범위가 여전히 이를 필요로 하는 동안에만 `providerAuthEnvVars`를 호환성 메타데이터로 유지하세요.
...
- OpenClaw가 채널 런타임을 로드하지 않고도 setup 상태를 검사할 수 있도록 채널 env-var 메타데이터를 선언적으로 유지하세요.
- env 기반 채널 setup을 Plugin 형태에서 사용하는 현재 setup, 채널 config 또는
  패키지 채널 메타데이터로 미러링하세요.
- 이전에 지원되는 OpenClaw 버전이 여전히 이를 요구하는 동안에만 `channelEnvVars`를 호환성 메타데이터로 유지하세요.
