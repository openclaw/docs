...
- 패키지가 ClawHub를 통해 게시되거나 설치될 예정이면 호환성과 설치 메타데이터를 추가하세요.
...
ClawHub가 패키지를 어떻게 설치하거나 업데이트해야 하는지 판단할 수 없습니다.
...
- 최소 호스트 버전을 패키지가 실제로 지원하는 호스트 버전 범위와 정렬하세요.
- [검색에 영향을 주는 package.json 필드](/ko/plugins/manifest#packagejson-fields-that-affect-discovery)를 참조하세요.
- `clawhub package validate <path-to-plugin>`를 다시 실행하세요.
...
- 패킹을 실패하게 만드는 잘못된 패키지 메타데이터, 깨진 수명 주기 스크립트 또는 files 항목을 수정하세요.
- 이 패키지를 공개 게시할 예정이라면 `private: true`를 제거하세요.
- `clawhub package validate <path-to-plugin>`를 다시 실행하세요.

### package-npm-pack-entrypoint-missing

패키지를 패킹할 수는 있지만, 패킹된 아티팩트에 `package.json#openclaw`에서 선언된 진입점 파일이 포함되어 있지 않습니다.

- `npm pack --dry-run`을 실행하고 포함될 파일을 확인하세요.
- 패킹하기 전에 생성되는 진입점을 빌드하세요.
...
- 해당 동작에 공개 SDK가 없으면 헬퍼를 패키지 내부에 유지하거나 공개 OpenClaw API를
  요청하세요.
...
Plugin이 여전히 `saveSessionStore` 또는 `updateSessionStore` 같은 사용 중단된 전체 세션 저장소
쓰기 헬퍼를 사용합니다.
...
Plugin이 여전히 `resolveSessionFilePath` 또는 `resolveAndPersistSessionFile` 같은 사용 중단된
세션 파일 경로 헬퍼를 사용합니다.

- 에이전트 및 세션 ID별로 세션 메타데이터를 읽으려면 `getSessionEntry(...)`를 사용하세요.
- 세션 메타데이터를 영속화하려면 `patchSessionEntry(...)` 또는 `upsertSessionEntry(...)`를
  사용하세요.
- 코드가 transcript 작업을 준비하는 경우 transcript ID 또는 대상 헬퍼를 사용하세요.
- 레거시 transcript 파일 경로를 영속화하거나 의존하지 마세요.
- [런타임 API](/ko/plugins/sdk-runtime#agent-session-state) 및
  [Plugin SDK 하위 경로](/ko/plugins/sdk-subpaths)를 참조하세요.
- `clawhub package validate <path-to-plugin>`을 다시 실행하세요.
...
- 코드에 구조화된 transcript 작업 대상이 필요한 경우 `resolveSessionTranscriptTarget(...)`를
  사용하세요.
...
Plugin이 여전히 `appendSessionTranscriptMessage` 또는 `emitSessionTranscriptUpdate` 같은 사용 중단된
저수준 transcript 헬퍼를 사용합니다.
...
- OpenClaw가 올바른 트랜잭션 경계와 ID 처리를 적용할 수 있도록 구조화된 transcript 런타임
  표면을 선호하세요.
- 선언한 호환성 범위가 이를 요구하는 이전 OpenClaw 버전을 여전히 지원하는 동안에만
...
매니페스트가 여전히 레거시 `providerAuthEnvVars` provider 인증 메타데이터를 사용합니다.

- provider 환경 변수 메타데이터를 `setup.providers[].envVars`로 미러링하세요.
- 지원하는 OpenClaw 범위가 여전히 필요로 하는 동안에만 `providerAuthEnvVars`를 호환성
  메타데이터로 유지하세요.
...
매니페스트가 ClawHub가 기대하는 현재 setup 또는 config 메타데이터 없이 레거시 또는 이전 채널
환경 변수 메타데이터를 사용합니다.

- OpenClaw가 채널 런타임을 로드하지 않고도 setup 상태를 검사할 수 있도록 채널 환경 변수
  메타데이터를 선언적으로 유지하세요.
- 환경 변수 기반 채널 setup을 현재 setup, 채널 config 또는 Plugin 형태에서 사용하는 패키지 채널
  메타데이터로 미러링하세요.
- 이전 지원 대상 OpenClaw 버전이 여전히 요구하는 동안에만 `channelEnvVars`를 호환성 메타데이터로
  유지하세요.
...
패키지가 ClawHub가 사용 가능하다고 인식하지 못하는 스키마 참조와 함께
...
- 매니페스트 contract가 존재할 때까지 보안에 민감한 동작은 공개 패키지 문서 또는 README에
  문서화해 두세요.
- `clawhub package validate <path-to-plugin>`을 다시 실행하세요.
