---
read_when:
    - clawhub package validate를 실행했고 Plugin 지적 사항을 수정해야 합니다
...
x-i18n:
    generated_at: "2026-07-01T15:21:48Z"
    model: gpt-5.5
...
| `sdk-session-transcript-file-target`    | [레거시 전사 파일 대상 교체](/ko/clawhub/plugin-validation-fixes#sdk-session-transcript-file-target)                   |
| `sdk-session-transcript-low-level`      | [저수준 전사 헬퍼 교체](/ko/clawhub/plugin-validation-fixes#sdk-session-transcript-low-level)                       |
...
- [발견에 영향을 주는 package.json 필드](/ko/plugins/manifest#packagejson-fields-that-affect-discovery)를 참조하세요.
- `clawhub package validate <path-to-plugin>`을 다시 실행하세요.
...
- [Plugin 진입점](/ko/plugins/sdk-entrypoints) 및 [발견에 영향을 주는 package.json 필드](/ko/plugins/manifest#packagejson-fields-that-affect-discovery)를 참조하세요.
- `clawhub package validate <path-to-plugin>`을 다시 실행하세요.
...
- 둘 이상의 설치 소스를 사용할 수 있으면 `openclaw.install.defaultChoice`를 설정하세요.
...
- `openclaw.plugin.json`에도 `version`이 있다면, 패키지 메타데이터가 기준일 때 이를 일치하도록 업데이트하거나 오래된 매니페스트 버전 메타데이터를 제거하세요.
...
- 레거시 트랜스크립트 파일 경로를 유지하거나 이에 의존하지 마세요.
- [런타임 API](/ko/plugins/sdk-runtime#agent-session-state)와
  [Plugin SDK 하위 경로](/ko/plugins/sdk-subpaths)를 참조하세요.
- `clawhub package validate <path-to-plugin>`을 다시 실행하세요.
...
- 모델 또는 제공자 오버라이드 작업은 `before_model_resolve`로 이동하세요.
...
매니페스트가 ClawHub에서 기대하는 현재 설정 또는 구성 메타데이터 없이 레거시 또는 이전 채널 환경 변수 메타데이터를 사용합니다.

- OpenClaw가 채널 런타임을 로드하지 않고도 설정 상태를 검사할 수 있도록 채널 환경 변수 메타데이터를 선언적으로 유지하세요.
- 환경 변수 기반 채널 설정을 Plugin 형태에서 사용하는 현재 설정, 채널 구성, 또는 패키지 채널 메타데이터로 미러링하세요.
- 지원되는 이전 OpenClaw 버전에서 아직 필요한 동안에만 `channelEnvVars`를 호환성 메타데이터로 유지하세요.
...
패키지가 ClawHub에서 사용 가능하다고 인식하지 못하는 스키마 참조와 함께 `openclaw.security.json`을 제공합니다.

- 권고용에 불과하다면 스키마 URL을 제거하세요.
- OpenClaw가 문서화된 버전 스키마를 게시한 후에만 이를 사용하세요.
- `clawhub package validate <path-to-plugin>`을 다시 실행하세요.
