...
x-i18n:
    generated_at: "2026-07-04T17:52:48Z"
    model: gpt-5.5
...
ClawHub는 게시 전에 Plugin 패키지를 검증하며, 자동 패키지 스캔 결과의 발견 사항도 표시할 수 있습니다. 이 페이지는 작성자 대상 발견 사항, 즉 Plugin 작성자가 패키지 메타데이터, 매니페스트, SDK 가져오기 또는 게시된 아티팩트에서 수정할 수 있는 발견 사항을 다룹니다.

내부 Plugin Inspector 커버리지 발견 사항은 다루지 않습니다. 전체 보고서에 작성자용 수정 안내 없이 스캐너 유지관리 코드가 포함되어 있다면, 이는 Plugin 작성자가 아니라 OpenClaw 유지관리자를 위한 것입니다.

수정 사항을 적용한 뒤에는 다시 실행하세요.
...
| `channel-env-vars`                      | [channel 환경 변수를 현재 메타데이터에 반영](/ko/clawhub/plugin-validation-fixes#channel-env-vars)                                |
...
- 최소 패키지 예시는 [Plugin 빌드](/ko/plugins/building-plugins)를, 패키지와 매니페스트의 역할 분리는 [Plugin 매니페스트](/ko/plugins/manifest#manifest-versus-packagejson)를 참고하세요.
...
- 패키지를 ClawHub를 통해 게시하거나 설치할 경우 호환성 및 설치 메타데이터를 추가합니다.
...
- 모든 진입점 경로를 패키지 디렉터리 내부에 유지합니다.
- [Plugin 진입점](/ko/plugins/sdk-entrypoints)과 [탐색에 영향을 주는 package.json 필드](/ko/plugins/manifest#packagejson-fields-that-affect-discovery)를 참고하세요.
- `clawhub package validate <path-to-plugin>`을 다시 실행합니다.
...
- 이것을 패키지 버전과 분리해 유지합니다. 패키지 버전은 Plugin 릴리스를 설명하고, `openclaw.compat.pluginApi`는 호스트 API 계약을 설명합니다.
- [탐색에 영향을 주는 package.json 필드](/ko/plugins/manifest#packagejson-fields-that-affect-discovery)를 참고하세요.
- `clawhub package validate <path-to-plugin>`을 다시 실행합니다.
...
- 패키지 진입점, 호환성, 설치, 설정, 카탈로그 메타데이터는 지원되는 `package.json#openclaw` 필드에 유지합니다.
- [탐색에 영향을 주는 package.json 필드](/ko/plugins/manifest#packagejson-fields-that-affect-discovery)를 참고하세요.
- `clawhub package validate <path-to-plugin>`을 다시 실행합니다.
...
- 잘못된 패키지 메타데이터, 깨진 수명 주기 스크립트 또는 패킹 실패를 일으키는 files 항목을 수정합니다.
- 이 패키지를 공개 게시하려는 경우 `private: true`를 제거합니다.
- `clawhub package validate <path-to-plugin>`을 다시 실행합니다.

### package-npm-pack-entrypoint-missing

패키지를 패킹할 수는 있지만, 패킹된 아티팩트에 `package.json#openclaw`에서 선언한 진입점 파일이 포함되어 있지 않습니다.
...
패킹된 아티팩트에 소스 패키지에는 있는 OpenClaw 메타데이터가 없습니다.
...
- `name`은 사람이 읽기 쉬운 상태로 유지하고, `id`는 안정적인 머신 ID로 유지합니다.
...
- 런타임 동작은 Plugin 등록 코드로 옮기고, `contracts`는 정적 capability 소유권 메타데이터로 제한합니다.
- `clawhub package validate <path-to-plugin>`을 다시 실행합니다.
...
- 코드가 transcript 작업을 준비하는 경우 transcript ID 또는 target 헬퍼를 사용합니다.
...
Plugin이 아직 사용 중단된 transcript 파일 target 헬퍼
`resolveSessionTranscriptLegacyFileTarget`를 사용합니다.

- 코드에 공개 세션 ID만 필요한 경우 `resolveSessionTranscriptIdentity(...)`를 사용합니다.
- 코드에 구조화된 transcript 작업 target이 필요한 경우 `resolveSessionTranscriptTarget(...)`을 사용합니다.
- 레거시 transcript 파일 target을 직접 읽거나 구성하지 않습니다.
- 선언한 호환성 범위가 이를 요구하는 이전 OpenClaw 버전을 아직 지원하는 동안에만
  레거시 헬퍼를 유지합니다.
- [런타임 API](/ko/plugins/sdk-runtime#agent-session-state) 및
  [Plugin SDK 하위 경로](/ko/plugins/sdk-subpaths)를 참조하세요.
...
- transcript append에는 `appendSessionTranscriptMessageByIdentity(...)`를 사용합니다.
- transcript 업데이트 알림에는 `publishSessionTranscriptUpdateByIdentity(...)`를 사용합니다.
- OpenClaw가 올바른 transaction 경계와 ID 처리를 적용할 수 있도록 구조화된 transcript 런타임 surface를 선호합니다.
- 선언한 호환성 범위가 이를 요구하는 이전 OpenClaw 버전을 아직 지원하는 동안에만
...
- 모델 또는 provider override 작업은 `before_model_resolve`로 옮깁니다.
- prompt 또는 context 변경 작업은 `before_prompt_build`로 옮깁니다.
- 선언한 호환성 범위가 이를 요구하는 이전 OpenClaw 버전을 아직 지원하는 동안에만
...
매니페스트가 아직 레거시 `providerAuthEnvVars` provider auth 메타데이터를 사용합니다.

- provider env-var 메타데이터를 `setup.providers[].envVars`로 mirror합니다.
...
- env 기반 채널 setup을 Plugin 형태에서 사용하는 현재 setup, 채널 config 또는 패키지 채널 메타데이터로 mirror합니다.
...
패키지가 ClawHub가 사용 가능하다고 인식하지 않는 schema 참조와 함께
`openclaw.security.json`을 제공합니다.

- schema URL이 안내용일 뿐이라면 제거합니다.
- OpenClaw가 문서화된 버전 지정 schema를 게시한 후에만 사용합니다.
- `clawhub package validate <path-to-plugin>`을 다시 실행합니다.
...
- OpenClaw가 버전 지정 보안 매니페스트 schema와 ClawHub 동작을 문서화할 때까지
