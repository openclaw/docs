...
ClawHub는 게시 전에 Plugin 패키지를 검증하며 자동화된 패키지 스캔의
발견 사항도 표시할 수 있습니다. 이 페이지에서는 작성자 대상 발견 사항,
즉 Plugin 작성자가 패키지 메타데이터, 매니페스트, SDK 가져오기 또는
게시된 아티팩트에서 수정할 수 있는 발견 사항을 다룹니다.

내부 Plugin Inspector 커버리지 발견 사항은 다루지 않습니다. 전체 보고서에
작성자용 해결 지침이 없는 스캐너 유지 관리 코드가 포함되어 있다면, 이는
Plugin 작성자가 아니라 OpenClaw 유지 관리자를 위한 것입니다.
...
패키지 루트에 `package.json`이 없으므로 ClawHub가 npm 패키지, 버전,
진입점 또는 OpenClaw 메타데이터를 식별할 수 없습니다.

- `name`, `version`, `type`이 포함된 `package.json`을 추가하세요.
- 패키지가 OpenClaw Plugin을 제공하는 경우 `openclaw` 블록을 추가하세요.
- 최소 패키지 예제는 [Plugin 빌드](/ko/plugins/building-plugins)를 사용하고,
  패키지와 매니페스트의 구분은 [Plugin 매니페스트](/ko/plugins/manifest#manifest-versus-packagejson)를 사용하세요.
- `clawhub package validate <path-to-plugin>`을 다시 실행하세요.

### package-openclaw-metadata-missing

패키지에 `package.json`이 있지만 OpenClaw 패키지 메타데이터를 선언하지
않습니다.

- `package.json#openclaw`를 추가하세요.
- `openclaw.extensions` 또는 `openclaw.runtimeExtensions` 같은 진입점
  메타데이터를 포함하세요.
- 패키지를 ClawHub를 통해 게시하거나 설치할 예정이면 호환성 및 설치
  메타데이터를 추가하세요.
...
- [Plugin 진입점](/ko/plugins/sdk-entrypoints)과
  [탐색에 영향을 주는 package.json 필드](/ko/plugins/manifest#packagejson-fields-that-affect-discovery)를 참조하세요.
- `clawhub package validate <path-to-plugin>`을 다시 실행하세요.

### package-entrypoint-missing

패키지가 OpenClaw 진입점을 선언하지만, 참조된 파일이 검증 중인 패키지에
없습니다.
...
- `clawhubSpec`, `npmSpec`, `localPath` 같은 지원되는 설치 소스로
  `openclaw.install`을 채우세요.
- 둘 이상의 설치 소스를 사용할 수 있는 경우 `openclaw.install.defaultChoice`를
  설정하세요.
...
- 빌드하고 테스트한 대상 OpenClaw Plugin API 버전 또는 semver 하한을 사용하세요.
- 이를 패키지 버전과 분리하세요. 패키지 버전은 Plugin 릴리스를 설명하고,
  `openclaw.compat.pluginApi`는 호스트 API 계약을 설명합니다.
- [탐색에 영향을 주는 package.json 필드](/ko/plugins/manifest#packagejson-fields-that-affect-discovery)를 참조하세요.
- `clawhub package validate <path-to-plugin>`을 다시 실행하세요.

### package-min-host-version-drift

패키지의 최소 호스트 버전이 패키지를 빌드한 기준 OpenClaw 버전
메타데이터와 일치하지 않습니다.

- `openclaw.install.minHostVersion`을 확인하세요.
- 릴리스 중 사용된 OpenClaw 버전 같은 패키지의 OpenClaw 빌드 메타데이터를
  확인하세요.
...
- `openclaw.plugin.json`에도 `version`이 있는 경우, 패키지 메타데이터가
  권위 있는 정보라면 이를 일치하도록 업데이트하거나 오래된 매니페스트
  버전 메타데이터를 제거하세요.
...
- 패키지 진입점, 호환성, 설치, 설정, 카탈로그 메타데이터는 지원되는
  `package.json#openclaw` 필드에 유지하세요.
- [탐색에 영향을 주는 package.json 필드](/ko/plugins/manifest#packagejson-fields-that-affect-discovery)를 참조하세요.
- `clawhub package validate <path-to-plugin>`을 다시 실행하세요.
...
- 패킹 실패를 일으키는 잘못된 패키지 메타데이터, 깨진 수명 주기 스크립트
  또는 files 항목을 수정하세요.
- 이 패키지가 공개 게시용인 경우 `private: true`를 제거하세요.
- `clawhub package validate <path-to-plugin>`을 다시 실행하세요.

### package-npm-pack-entrypoint-missing

패키지를 패킹할 수는 있지만, 패킹된 아티팩트에 `package.json#openclaw`에
선언된 진입점 파일이 포함되어 있지 않습니다.

- `npm pack --dry-run`을 실행하고 포함될 파일을 검사하세요.
- 패킹 전에 생성된 진입점을 빌드하세요.
- 선언된 진입점이 포함되도록 `files`, `.npmignore` 또는 빌드 출력을
  업데이트하세요.
- [Plugin 진입점](/ko/plugins/sdk-entrypoints)을 참조하세요.
- `clawhub package validate <path-to-plugin>`을 다시 실행하세요.
...
- 루트 배럴 import를 범위가 좁은 공개 하위 경로 import로 교체하세요.
...
- 전체 세션 저장소 객체를 로드하고, 변경하고, 저장하지 마세요.
...
- 세션 항목을 교체하거나 만들 때는 `upsertSessionEntry(...)`를 사용하세요.
- 전체 세션 저장소 객체를 로드하고, 변경하고, 저장하지 마세요.
- 선언한 호환성 범위가 이를 요구하는 이전 OpenClaw 버전을 여전히 지원하는 동안에만
  전체 저장소 쓰기 헬퍼를 유지하세요.
- [Runtime API](/ko/plugins/sdk-runtime#agent-session-state) 및
...
지원 중단된 세션 파일 경로 헬퍼를 사용합니다.

- 에이전트 및 세션 ID별 세션 메타데이터를 읽으려면 `getSessionEntry(...)`를 사용하세요.
...
지원 중단된 저수준 transcript 헬퍼를 사용합니다.
...
- prompt 또는 context 변경 작업은 `before_prompt_build`로 이동하세요.
- 선언한 호환성 범위가 이를 요구하는 이전 OpenClaw 버전을 여전히 지원하는 동안에만
...
- provider env-var 메타데이터를 `setup.providers[].envVars`에 반영하세요.
- 지원하는 OpenClaw 범위에서 여전히 필요할 때만 `providerAuthEnvVars`를 호환성 메타데이터로 유지하세요.
...
- OpenClaw가 채널 런타임을 로드하지 않고 setup 상태를 검사할 수 있도록 채널 env-var 메타데이터를 선언적으로 유지하세요.
- env 기반 채널 setup을 Plugin 형태에서 사용하는 현재 setup, 채널 config 또는 패키지 채널 메타데이터에 반영하세요.
- 이전에 지원되는 OpenClaw 버전에서 여전히 필요한 동안에만 `channelEnvVars`를 호환성 메타데이터로 유지하세요.
