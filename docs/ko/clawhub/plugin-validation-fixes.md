...
summary: 게시하기 전에 ClawHub Plugin 패키지 검증 문제를 수정하십시오
...
ClawHub는 게시 전에 Plugin 패키지를 검증하며 자동화된 패키지 검사에서 발견된 문제도 표시할 수 있습니다. 이 페이지에서는 작성자가 패키지 메타데이터, 매니페스트, SDK 가져오기 또는 게시된 아티팩트에서 수정할 수 있는 문제를 다룹니다.

내부 Plugin Inspector 검사 범위 문제는 다루지 않습니다. 전체 보고서에 작성자를 위한 해결 지침 없이 스캐너 유지 관리 코드가 포함되어 있다면, 이는 Plugin 작성자가 아니라 OpenClaw 유지 관리자를 위한 것입니다.
...
- 최소 패키지 예제는 [Plugin 빌드](/ko/plugins/building-plugins)를, 패키지와 매니페스트의 역할 구분은 [Plugin 매니페스트](/ko/plugins/manifest#manifest-versus-packagejson)를 참조하십시오.
...
- ClawHub를 통해 패키지를 게시하거나 설치할 예정이라면 호환성 및 설치 메타데이터를 추가하십시오.
...
- 게시된 패키지가 빌드된 JavaScript를 로드해야 하는 경우 `openclaw.runtimeExtensions`를 추가하십시오.
- 모든 진입점 경로를 패키지 디렉터리 내부에 유지하십시오.
- [Plugin 진입점](/ko/plugins/sdk-entrypoints)과 [검색에 영향을 주는 package.json 필드](/ko/plugins/manifest#packagejson-fields-that-affect-discovery)를 참조하십시오.
- `clawhub package validate <path-to-plugin>`을 다시 실행하십시오.
...
- 진입점이 이동했다면 메타데이터를 업데이트하십시오.
- [Plugin 진입점](/ko/plugins/sdk-entrypoints)을 참조하십시오.
- `clawhub package validate <path-to-plugin>`을 다시 실행하십시오.

### package-install-metadata-incomplete

ClawHub에서 패키지를 설치하거나 업데이트하는 방법을 판단할 수 없습니다.

- `openclaw.install`에 `clawhubSpec`, `npmSpec`, `localPath`와 같은 지원되는 설치 소스를 입력하십시오.
- 사용할 수 있는 설치 소스가 둘 이상이면 `openclaw.install.defaultChoice`를 설정하십시오.
...
- 빌드하고 테스트할 때 기준으로 삼은 OpenClaw Plugin API 버전 또는 semver 최저 버전을 사용하십시오.
- 이를 패키지 버전과 구분하여 유지하십시오. 패키지 버전은 Plugin 릴리스를 설명하며, `openclaw.compat.pluginApi`는 호스트 API 계약을 설명합니다.
- [검색에 영향을 주는 package.json 필드](/ko/plugins/manifest#packagejson-fields-that-affect-discovery)를 참조하십시오.
- `clawhub package validate <path-to-plugin>`을 다시 실행하십시오.
...
- 릴리스 중 사용된 OpenClaw 버전과 같이 패키지에 포함된 OpenClaw 빌드 메타데이터를 확인하십시오.
- 최소 호스트 버전을 패키지가 실제로 지원하는 호스트 버전 범위와 일치시키십시오.
- [검색에 영향을 주는 package.json 필드](/ko/plugins/manifest#packagejson-fields-that-affect-discovery)를 참조하십시오.
- `clawhub package validate <path-to-plugin>`을 다시 실행하십시오.
...
- `openclaw.plugin.json`에도 `version`이 있다면 일치하도록 업데이트하거나, 패키지 메타데이터가 기준인 경우 오래된 매니페스트 버전 메타데이터를 제거하십시오.
- 게시된 메타데이터를 변경한 후에는 새 패키지 버전을 게시하십시오.
...
- 이 패키지를 공개적으로 게시하려는 경우 `private: true`를 제거하십시오.
- `clawhub package validate <path-to-plugin>`을 다시 실행하십시오.

### package-npm-pack-entrypoint-missing

패키지를 패킹할 수 있지만 패킹된 아티팩트에 `package.json#openclaw`에서 선언한 진입점 파일이 포함되어 있지 않습니다.

- `npm pack --dry-run`을 실행하고 포함될 파일을 확인하십시오.
...
- `npm pack --dry-run`을 실행하고 포함된 메타데이터 파일을 확인하십시오.
...
네이티브 플러그인 매니페스트에 표시 이름이 포함되어 있지 않습니다.

- `openclaw.plugin.json`에 비어 있지 않은 `name` 필드를 추가하십시오.
- `name`은 사람이 읽을 수 있게 유지하고 `id`는 안정적인 머신 ID로 유지하십시오.
- [플러그인 매니페스트](/ko/plugins/manifest)를 참조하십시오.
- `clawhub package validate <path-to-plugin>`을 다시 실행하십시오.

### manifest-unknown-fields

플러그인 매니페스트에 OpenClaw가 지원하지 않는 최상위 필드가 있습니다.
...
- 런타임 동작을 플러그인 등록 코드로 이동하고, `contracts`는
...
플러그인이 더 이상 권장되지 않는 루트 SDK 배럴인
`openclaw/plugin-sdk`에서 가져옵니다.

- 루트 배럴 가져오기를 용도별 공개 하위 경로 가져오기로 교체하십시오.
- `definePluginEntry`에는 `openclaw/plugin-sdk/plugin-entry`를 사용하십시오.
- 채널 진입점 헬퍼에는 `openclaw/plugin-sdk/channel-core`를 사용하십시오.
- [가져오기 규칙](/ko/plugins/building-plugins#import-conventions)과
  [플러그인 SDK 하위 경로](/ko/plugins/sdk-subpaths)를 사용하여 범위가 좁은 가져오기를 찾으십시오.
- `clawhub package validate <path-to-plugin>`을 다시 실행하십시오.

### reserved-sdk-import

플러그인이 번들 플러그인 또는 내부 호환성을 위해 예약된 SDK 경로에서
가져옵니다.
...
- [플러그인 SDK 하위 경로](/ko/plugins/sdk-subpaths)와
  [SDK 마이그레이션](/ko/plugins/sdk-migration)을 사용하여 지원되는 가져오기를 선택하십시오.
- `clawhub package validate <path-to-plugin>`을 다시 실행하십시오.

### sdk-load-session-store

플러그인이 더 이상 권장되지 않는 전체 세션 저장소 헬퍼
`loadSessionStore`를 여전히 사용합니다.

- 세션 상태를 읽을 때 `getSessionEntry(...)` 또는 `listSessionEntries(...)`를
  사용하십시오.
- 세션 상태를 쓸 때 `patchSessionEntry(...)` 또는 `upsertSessionEntry(...)`를
  사용하십시오.
- 전체 세션 저장소 객체를 로드하고 변경한 후 저장하지 마십시오.
- 선언한 호환성 범위가 해당 기능을 요구하는 이전 OpenClaw 버전을
  계속 지원하는 동안에만 `loadSessionStore(...)`를 유지하십시오.
- [런타임 API](/ko/plugins/sdk-runtime#agent-session-state)와
  [플러그인 SDK 하위 경로](/ko/plugins/sdk-subpaths)를 참조하십시오.
- `clawhub package validate <path-to-plugin>`을 다시 실행하십시오.

### sdk-session-store-write

플러그인이 `saveSessionStore` 또는 `updateSessionStore`와 같은 더 이상 권장되지 않는
전체 세션 저장소 쓰기 헬퍼를 여전히 사용합니다.

- 기존 세션 항목의 필드를 업데이트할 때 `patchSessionEntry(...)`를
  사용하십시오.
- 세션 항목을 교체하거나 생성할 때 `upsertSessionEntry(...)`를 사용하십시오.
- 전체 세션 저장소 객체를 로드하고 변경한 후 저장하지 마십시오.
- 선언한 호환성 범위가 해당 기능을 요구하는 이전 OpenClaw 버전을
...
플러그인이 `resolveSessionFilePath` 또는 `resolveAndPersistSessionFile`과 같은 더 이상 권장되지 않는
세션 파일 경로 헬퍼를 여전히 사용합니다.

- 에이전트 및 세션 ID별로 세션 메타데이터를 읽으려면
  `getSessionEntry(...)`를 사용하십시오.
- 세션 메타데이터를 유지하려면 `patchSessionEntry(...)` 또는
  `upsertSessionEntry(...)`를 사용하십시오.
- 코드가 트랜스크립트 작업을 준비하는 경우 트랜스크립트 ID 또는 대상 헬퍼를 사용하십시오.
- 레거시 트랜스크립트 파일 경로를 유지하거나 이에 의존하지 마십시오.
- [런타임 API](/ko/plugins/sdk-runtime#agent-session-state)와
  [플러그인 SDK 하위 경로](/ko/plugins/sdk-subpaths)를 참조하십시오.
- `clawhub package validate <path-to-plugin>`을 다시 실행하십시오.
...
플러그인이 더 이상 권장되지 않는 트랜스크립트 파일 대상 헬퍼
`resolveSessionTranscriptLegacyFileTarget`을 여전히 사용합니다.
...
플러그인이 `appendSessionTranscriptMessage` 또는 `emitSessionTranscriptUpdate`와 같은
더 이상 권장되지 않는 저수준 트랜스크립트 헬퍼를 여전히 사용합니다.

- 트랜스크립트에 추가할 때 `appendSessionTranscriptMessageByIdentity(...)`를 사용하십시오.
- 트랜스크립트 업데이트 알림에는
  `publishSessionTranscriptUpdateByIdentity(...)`를 사용하십시오.
- OpenClaw가 올바른 트랜잭션 경계와 ID 처리를 적용할 수 있도록
  구조화된 트랜스크립트 런타임 표면을 사용하십시오.
- 선언한 호환성 범위가 해당 기능을 요구하는 이전 OpenClaw 버전을
...
플러그인이 레거시 `before_agent_start` 훅을 여전히 사용합니다.

- 모델 또는 제공자 재정의 작업을 `before_model_resolve`로 이동하십시오.
- 프롬프트 또는 컨텍스트 변경 작업을 `before_prompt_build`로 이동하십시오.
- 선언한 호환성 범위가 해당 기능을 요구하는 이전 OpenClaw 버전을
  계속 지원하는 동안에만 `before_agent_start`를 유지하십시오.
- [훅](/ko/plugins/hooks)과
  [플러그인 호환성](/ko/plugins/compatibility)을 참조하십시오.
- `clawhub package validate <path-to-plugin>`을 다시 실행하십시오.
...
- 제공자 환경 변수 메타데이터를 `setup.providers[].envVars`에도 반영하십시오.
- 지원하는 OpenClaw 버전 범위에서 여전히 필요한 동안에만
  `providerAuthEnvVars`를 호환성 메타데이터로 유지하십시오.
...
- 환경 변수 기반 채널 설정을 플러그인 형태에서 사용하는 현재 설정,
  채널 구성 또는 패키지 채널 메타데이터에 반영하십시오.
- 지원하는 이전 OpenClaw 버전에서 여전히 필요한 동안에만
  `channelEnvVars`를 호환성 메타데이터로 유지하십시오.
- [플러그인 매니페스트](/ko/plugins/manifest)와
  [채널 플러그인](/ko/plugins/sdk-channel-plugins)을 참조하십시오.
- `clawhub package validate <path-to-plugin>`을 다시 실행하십시오.
...
패키지에 지원되지 않는 보안 매니페스트 파일이 제공됩니다.
...
- [플러그인 진입점](/ko/plugins/sdk-entrypoints)
- [플러그인 호환성](/ko/plugins/compatibility)
