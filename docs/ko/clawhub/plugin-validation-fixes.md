...
| `provider-auth-env-vars`                | [공급자 환경 변수를 설정 메타데이터로 이동](/ko/clawhub/plugin-validation-fixes#provider-auth-env-vars)                             |
...
패키지 루트에 `package.json`이 없어서 ClawHub가 npm 패키지, 버전, 엔트리포인트 또는 OpenClaw 메타데이터를 식별할 수 없습니다.
...
- 패키지가 ClawHub를 통해 게시되거나 설치될 경우 호환성 및 설치 메타데이터를 추가합니다.
...
- 이를 패키지 버전과 별도로 유지합니다. 패키지 버전은 Plugin 릴리스를 설명하고, `openclaw.compat.pluginApi`는 호스트 API 계약을 설명합니다.
- [검색에 영향을 주는 package.json 필드](/ko/plugins/manifest#packagejson-fields-that-affect-discovery)를 참고하세요.
- `clawhub package validate <path-to-plugin>`를 다시 실행합니다.

### package-min-host-version-drift

패키지 최소 호스트 버전이 패키지가 빌드될 때 기준으로 삼은 OpenClaw 버전 메타데이터와 일치하지 않습니다.

- `openclaw.install.minHostVersion`을 확인합니다.
- 릴리스 중에 사용된 OpenClaw 버전 같은 패키지의 OpenClaw 빌드 메타데이터를 확인합니다.
...
- `openclaw.plugin.json`에도 `version`이 있다면 일치하도록 업데이트하거나, 패키지 메타데이터가 authoritative한 경우 오래된 매니페스트 버전 메타데이터를 제거합니다.
...
- 패킹 실패를 유발하는 잘못된 패키지 메타데이터, 깨진 수명 주기 스크립트 또는 files 항목을 수정합니다.
- 이 패키지를 공개 게시하려는 경우 `private: true`를 제거합니다.
- `clawhub package validate <path-to-plugin>`를 다시 실행합니다.

### package-npm-pack-entrypoint-missing

패키지는 패킹할 수 있지만, 패킹된 아티팩트에 `package.json#openclaw`에 선언된 엔트리포인트 파일이 포함되지 않습니다.
...
- 루트 배럴 가져오기를 목적에 맞는 공개 하위 경로 가져오기로 교체하세요.
- `definePluginEntry`에는 `openclaw/plugin-sdk/plugin-entry`를 사용하세요.
- 채널 진입 헬퍼에는 `openclaw/plugin-sdk/channel-core`를 사용하세요.
...
Plugin이 아직 레거시 `before_agent_start` 후크를 사용합니다.

- 모델 또는 공급자 재정의 작업은 `before_model_resolve`로 이동하세요.
- 프롬프트 또는 컨텍스트 변경 작업은 `before_prompt_build`로 이동하세요.
- 선언한 호환성 범위가 이를 필요로 하는 이전 OpenClaw 버전을 아직 지원하는 동안에만
  `before_agent_start`를 유지하세요.
- [후크](/ko/plugins/hooks) 및
...
- 환경 변수 기반 채널 setup을 현재 setup, 채널 config 또는 Plugin 형태에서 사용하는
...
패키지가 ClawHub가 사용 가능하다고 인식하지 않는 스키마 참조가 있는
