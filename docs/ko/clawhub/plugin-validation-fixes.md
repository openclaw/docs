...
title: Plugin 검증 수정
x-i18n:
    generated_at: "2026-07-02T22:26:12Z"
    model: gpt-5.5
...
ClawHub는 게시 전에 Plugin 패키지를 검증하며, 자동화된 패키지 스캔의 발견 사항도 표시할 수 있습니다. 이 페이지는 작성자 대상 발견 사항을 다룹니다. 즉, Plugin 작성자가 패키지 메타데이터, 매니페스트, SDK import 또는 게시된 아티팩트에서 수정할 수 있는 발견 사항을 의미합니다.

내부 Plugin Inspector 커버리지 발견 사항은 다루지 않습니다. 전체 보고서에 작성자 수정 지침 없이 스캐너 유지 관리 코드가 포함되어 있다면, 이는 Plugin 작성자가 아니라 OpenClaw 유지관리자를 위한 것입니다.
...
| `package-install-metadata-incomplete`   | [설치 메타데이터 완료](/ko/clawhub/plugin-validation-fixes#package-install-metadata-incomplete)                               |
...
| `legacy-root-sdk-import`                | [루트 SDK import 교체](/ko/clawhub/plugin-validation-fixes#legacy-root-sdk-import)                                             |
| `reserved-sdk-import`                   | [예약된 SDK import 제거](/ko/clawhub/plugin-validation-fixes#reserved-sdk-import)                                             |
...
| `provider-auth-env-vars`                | [provider env var를 setup 메타데이터로 이동](/ko/clawhub/plugin-validation-fixes#provider-auth-env-vars)                             |
| `channel-env-vars`                      | [channel env var를 현재 메타데이터에 미러링](/ko/clawhub/plugin-validation-fixes#channel-env-vars)                                |
...
패키지가 OpenClaw 진입점을 선언했지만, 참조된 파일이 검증 중인 패키지에 없습니다.
...
- 패키지 진입점, 호환성, 설치, setup 및 catalog 메타데이터는 지원되는 `package.json#openclaw` 필드에 유지하세요.
- [검색에 영향을 주는 package.json 필드](/ko/plugins/manifest#packagejson-fields-that-affect-discovery)를 참고하세요.
- `clawhub package validate <path-to-plugin>`을 다시 실행하세요.
...
- 패키지 또는 설치 메타데이터는 매니페스트 대신 지원되는
  `package.json#openclaw` 필드로 이동하세요.
- `clawhub package validate <path-to-plugin>`을 다시 실행하세요.
...
- 지원되지 않는 contract 키를 제거하세요.
- 런타임 동작은 Plugin 등록 코드로 이동하고, `contracts`는 정적 기능 소유권 메타데이터로 제한하세요.
- `clawhub package validate <path-to-plugin>`을 다시 실행하세요.
...
- 루트 배럴 가져오기를 범위가 좁은 공개 하위 경로 가져오기로 교체하세요.
...
- 예약된 OpenClaw 내부 SDK 가져오기를 문서화된 공개
  `openclaw/plugin-sdk/*` 하위 경로로 교체하세요.
- 해당 동작에 공개 SDK가 없으면 헬퍼를 패키지 안에 유지하거나
...
Plugin이 아직 더 이상 권장되지 않는 전체 세션 저장소 헬퍼
...
- 전체 세션 저장소 객체를 로드, 변경, 저장하는 방식을 피하세요.
...
Plugin이 아직 `saveSessionStore` 또는 `updateSessionStore` 같은 더 이상 권장되지 않는 전체 세션 저장소 쓰기 헬퍼를 사용합니다.
...
Plugin이 아직 `resolveSessionFilePath` 또는 `resolveAndPersistSessionFile` 같은 더 이상 권장되지 않는 세션 파일 경로 헬퍼를 사용합니다.
...
Plugin이 아직 더 이상 권장되지 않는 transcript 파일 대상 헬퍼
...
Plugin이 아직 `appendSessionTranscriptMessage` 또는 `emitSessionTranscriptUpdate` 같은 더 이상 권장되지 않는 저수준 transcript 헬퍼를 사용합니다.
...
- 모델 또는 제공자 재정의 작업은 `before_model_resolve`로 이동하세요.
...
- 환경 변수 기반 채널 setup을 현재 setup, 채널 config 또는 Plugin 형태에서 사용하는 패키지 채널 메타데이터로 미러링하세요.
...
- OpenClaw가 버전이 지정된 스키마를 게시한 뒤에만 문서화된 버전 스키마를 사용하세요.
- `clawhub package validate <path-to-plugin>`을 다시 실행하세요.
