...
x-i18n:
    generated_at: "2026-07-02T00:48:09Z"
    model: gpt-5.5
...
발견 사항도 표시할 수 있습니다. 이 페이지는 작성자 대상 발견 사항, 즉
Plugin 작성자가 패키지 메타데이터, 매니페스트, SDK 가져오기 또는 게시된
아티팩트에서 수정할 수 있는 발견 사항을 다룹니다.

내부 Plugin Inspector 커버리지 발견 사항은 다루지 않습니다. 전체 보고서에
작성자 수정 지침이 없는 스캐너 유지 관리 코드가 포함되어 있다면, 이는
...
패키지 루트에 `package.json`이 포함되어 있지 않아 ClawHub가 npm 패키지,
버전, 엔트리포인트 또는 OpenClaw 메타데이터를 식별할 수 없습니다.

- `name`, `version`, `type`이 포함된 `package.json`을 추가합니다.
- 패키지가 OpenClaw Plugin을 제공하는 경우 `openclaw` 블록을 추가합니다.
- 최소 패키지 예시는 [Plugin 빌드](/ko/plugins/building-plugins)를,
...
- `openclaw.extensions` 또는 `openclaw.runtimeExtensions` 같은
  엔트리포인트 메타데이터를 포함합니다.
...
패키지 메타데이터는 있지만 OpenClaw 런타임 엔트리포인트를 선언하지
않습니다.

- 네이티브 Plugin 엔트리포인트에는 `openclaw.extensions`를 추가합니다.
- 게시된 패키지가 빌드된 JavaScript를 로드해야 하는 경우
  `openclaw.runtimeExtensions`를 추가합니다.
- 모든 엔트리포인트 경로를 패키지 디렉터리 내부에 유지합니다.
...
패키지가 OpenClaw 엔트리포인트를 선언하지만, 검증 중인 패키지에서 참조된
파일이 누락되었습니다.

- `openclaw.extensions`, `openclaw.runtimeExtensions`,
  `openclaw.setupEntry`, `openclaw.runtimeSetupEntry`의 각 경로를 확인합니다.
- 엔트리포인트가 `dist`로 생성되는 경우 패키지를 빌드합니다.
...
- `clawhubSpec`, `npmSpec` 또는 `localPath` 같은 지원되는 설치 소스로
  `openclaw.install`을 채웁니다.
...
- 이를 패키지 버전과 분리해 유지합니다. 패키지 버전은 Plugin 릴리스를
  설명하고, `openclaw.compat.pluginApi`는 호스트 API 계약을 설명합니다.
- [탐색에 영향을 주는 package.json 필드](/ko/plugins/manifest#packagejson-fields-that-affect-discovery)를 참조하세요.
- `clawhub package validate <path-to-plugin>`을 다시 실행합니다.

### package-min-host-version-drift

패키지 최소 호스트 버전이 패키지가 기준으로 빌드된 OpenClaw 버전
메타데이터와 일치하지 않습니다.

- `openclaw.install.minHostVersion`을 확인합니다.
- 릴리스 중에 사용된 OpenClaw 버전 같은 패키지의 OpenClaw 빌드 메타데이터를
  확인합니다.
...
- `openclaw.plugin.json`에도 `version`이 있다면, 패키지 메타데이터가
  권위 있는 경우 일치하도록 업데이트하거나 오래된 매니페스트 버전
  메타데이터를 제거합니다.
...
- 패키지 엔트리포인트, 호환성, 설치, 설정, 카탈로그 메타데이터는 지원되는
  `package.json#openclaw` 필드에 유지합니다.
- [탐색에 영향을 주는 package.json 필드](/ko/plugins/manifest#packagejson-fields-that-affect-discovery)를 참조하세요.
- `clawhub package validate <path-to-plugin>`을 다시 실행합니다.
...
- 패킹이 실패하게 만드는 잘못된 패키지 메타데이터, 깨진 수명 주기 스크립트
  또는 files 항목을 수정합니다.
...
패키지는 패킹할 수 있지만, 패킹된 아티팩트에 `package.json#openclaw`에
선언된 엔트리포인트 파일이 포함되어 있지 않습니다.
...
패킹된 아티팩트에 소스 패키지에 있는 OpenClaw 메타데이터가 누락되었습니다.
...
- 루트 배럴 import를 집중된 공개 하위 경로 import로 바꾸세요.
...
- 선언한 호환성 범위가 여전히 이를 요구하는 이전 OpenClaw 버전을 지원하는 동안에만
...
- 공급자 환경 변수 메타데이터를 `setup.providers[].envVars`로 미러링하세요.
...
- OpenClaw가 스키마를 게시한 후에만 문서화된 버전 관리 스키마를 사용하세요.
- `clawhub package validate <path-to-plugin>`를 다시 실행하세요.
...
- OpenClaw가 버전 관리되는 보안 매니페스트 스키마와 ClawHub 동작을 문서화할 때까지
