...
summary: ClawHub Plugin 패키지 검증 결과를 게시 전에 수정
title: Plugin 검증 수정
x-i18n:
    generated_at: "2026-07-01T18:09:52Z"
    model: gpt-5.5
...
내부 Plugin Inspector 적용 범위 발견 사항은 다루지 않습니다. 전체 보고서에 작성자 수정 지침 없이 스캐너 유지관리 코드가 포함되어 있다면, 이는 Plugin 작성자가 아니라 OpenClaw 유지관리자를 위한 것입니다.
...
| `package-min-host-version-drift`        | [최소 호스트 버전 맞춤](/ko/clawhub/plugin-validation-fixes#package-min-host-version-drift)                                   |
| `package-manifest-version-drift`        | [패키지와 매니페스트 버전 맞춤](/ko/clawhub/plugin-validation-fixes#package-manifest-version-drift)                          |
...
- 최소 패키지 예시는 [Plugin 빌드](/ko/plugins/building-plugins)를 사용하고, 패키지와 매니페스트 분리는 [Plugin 매니페스트](/ko/plugins/manifest#manifest-versus-packagejson)를 사용하세요.
...
- 모든 엔트리포인트 경로를 패키지 디렉터리 안에 유지합니다.
- [Plugin 엔트리 포인트](/ko/plugins/sdk-entrypoints) 및 [검색에 영향을 주는 package.json 필드](/ko/plugins/manifest#packagejson-fields-that-affect-discovery)를 참조하세요.
- `clawhub package validate <path-to-plugin>`를 다시 실행합니다.
...
- [Plugin 엔트리 포인트](/ko/plugins/sdk-entrypoints)를 참조하세요.
- `clawhub package validate <path-to-plugin>`를 다시 실행합니다.
...
- 패키지가 실제로 지원하는 호스트 버전 범위에 맞게 최소 호스트 버전을 맞춥니다.
- [검색에 영향을 주는 package.json 필드](/ko/plugins/manifest#packagejson-fields-that-affect-discovery)를 참조하세요.
- `clawhub package validate <path-to-plugin>`를 다시 실행합니다.
...
- 패키지 릴리스 버전으로 `package.json#version`을 선호합니다.
- `openclaw.plugin.json`에도 `version`이 있는 경우, 패키지 메타데이터가 권위 있는 소스라면 이를 일치하도록 업데이트하거나 오래된 매니페스트 버전 메타데이터를 제거합니다.
...
- 패키지 엔트리포인트, 호환성, 설치, 설정, 카탈로그 메타데이터는 지원되는 `package.json#openclaw` 필드에 유지합니다.
- [검색에 영향을 주는 package.json 필드](/ko/plugins/manifest#packagejson-fields-that-affect-discovery)를 참조하세요.
- `clawhub package validate <path-to-plugin>`를 다시 실행합니다.
...
- 패킹을 실패하게 만드는 잘못된 패키지 메타데이터, 깨진 생명주기 스크립트 또는 files 항목을 수정합니다.
- 이 패키지를 공개 게시할 의도라면 `private: true`를 제거합니다.
- `clawhub package validate <path-to-plugin>`를 다시 실행합니다.
...
- 런타임 동작은 Plugin 등록 코드로 옮기고, `contracts`는 정적 기능 소유권 메타데이터로 제한합니다.
- `clawhub package validate <path-to-plugin>`을 다시 실행합니다.
...
- 루트 배럴 import를 범위가 좁은 공개 서브패스 import로 바꿉니다.
...
- 좁은 import를 찾으려면 [Import 규칙](/ko/plugins/building-plugins#import-conventions) 및
  [Plugin SDK 서브패스](/ko/plugins/sdk-subpaths)를 사용합니다.
- `clawhub package validate <path-to-plugin>`을 다시 실행합니다.
...
- 예약된 OpenClaw 내부 SDK import를 문서화된 공개
  `openclaw/plugin-sdk/*` 서브패스로 바꿉니다.
- 해당 동작에 공개 SDK가 없다면 헬퍼를 패키지 안에 유지하거나
  공개 OpenClaw API를 요청합니다.
- 지원되는 import를 선택하려면 [Plugin SDK 서브패스](/ko/plugins/sdk-subpaths) 및
...
Plugin이 여전히 `saveSessionStore` 또는 `updateSessionStore` 같은 사용 중단된 전체 세션 저장소 쓰기 헬퍼를 사용합니다.

- 기존 세션 항목의 필드를 업데이트할 때는 `patchSessionEntry(...)`를 사용합니다.
- 세션 항목을 바꾸거나 만들 때는 `upsertSessionEntry(...)`를 사용합니다.
- 전체 세션 저장소 객체를 로드하고, 변경하고, 저장하는 방식을 피합니다.
- 선언한 호환성 범위가 이를 요구하는 이전 OpenClaw 버전을 아직 지원하는 동안에만
  전체 저장소 쓰기 헬퍼를 유지합니다.
- [런타임 API](/ko/plugins/sdk-runtime#agent-session-state) 및
...
Plugin이 여전히 `resolveSessionFilePath` 또는 `resolveAndPersistSessionFile` 같은 사용 중단된 세션 파일 경로 헬퍼를 사용합니다.
...
Plugin이 여전히 사용 중단된 transcript 파일 대상 헬퍼
...
Plugin이 여전히 `appendSessionTranscriptMessage` 또는 `emitSessionTranscriptUpdate` 같은 사용 중단된 저수준 transcript 헬퍼를 사용합니다.
...
- OpenClaw가 올바른 트랜잭션 경계와 식별자 처리를 적용할 수 있도록 구조화된 transcript 런타임 표면을 선호합니다.
- 선언한 호환성 범위가 이를 요구하는 이전 OpenClaw 버전을 아직 지원하는 동안에만
...
- 모델 또는 제공자 재정의 작업은 `before_model_resolve`로 옮깁니다.
- 프롬프트 또는 컨텍스트 변경 작업은 `before_prompt_build`로 옮깁니다.
- 선언한 호환성 범위가 이를 요구하는 이전 OpenClaw 버전을 아직 지원하는 동안에만
...
- 지원하는 OpenClaw 범위에 여전히 필요할 때만 `providerAuthEnvVars`를 호환성 메타데이터로 유지합니다.
...
- 지원하는 이전 OpenClaw 버전에 여전히 필요할 때만 `channelEnvVars`를 호환성 메타데이터로 유지합니다.
...
패키지가 ClawHub가 사용 가능하다고 인식하지 않는 스키마 참조가 포함된 `openclaw.security.json`을 제공합니다.

- 스키마 URL이 권고용에 불과하면 제거합니다.
- OpenClaw가 문서화된 버전 지정 스키마를 게시한 뒤에만 사용합니다.
- `clawhub package validate <path-to-plugin>`을 다시 실행합니다.
...
- 매니페스트 계약이 생길 때까지 보안에 민감한 동작은 공개 패키지 문서 또는 README에 문서화합니다.
- `clawhub package validate <path-to-plugin>`을 다시 실행합니다.
