...
ClawHub는 게시 전에 Plugin 패키지를 검증하며 자동 패키지 스캔의 발견 사항도 표시할 수 있습니다. 이 페이지는 작성자 대상 발견 사항을 다룹니다. 즉, Plugin 작성자가 패키지 메타데이터, 매니페스트, SDK 가져오기 또는 게시된 아티팩트에서 수정할 수 있는 발견 사항입니다.
...
- `openclaw.install`을 `clawhubSpec`, `npmSpec`, `localPath`와 같은 지원되는 설치 소스로 채웁니다.
...
- 릴리스 중 사용된 OpenClaw 버전과 같은 패키지의 OpenClaw 빌드 메타데이터를 확인합니다.
...
- `openclaw.bundle`과 같은 지원되지 않는 필드를 제거합니다.
...
- 패킹 실패를 유발하는 잘못된 패키지 메타데이터, 깨진 라이프사이클 스크립트 또는 files 항목을 수정합니다.
- 이 패키지를 공개 게시할 예정이라면 `private: true`를 제거합니다.
- `clawhub package validate <path-to-plugin>`을 다시 실행합니다.
...
- 코드에 공개 세션 ID만 필요할 때는 `resolveSessionTranscriptIdentity(...)`를 사용합니다.
- 코드에 구조화된 transcript 작업 대상이 필요할 때는 `resolveSessionTranscriptTarget(...)`를 사용합니다.
...
- OpenClaw가 올바른 트랜잭션 경계와 ID 처리를 적용할 수 있도록 구조화된 transcript 런타임 표면을
  선호합니다.
- 선언한 호환성 범위가 이를 필요로 하는 이전 OpenClaw 버전을 계속 지원하는 동안에만
...
- 모델 또는 제공자 override 작업은 `before_model_resolve`로 이동합니다.
...
- 제공자 환경 변수 메타데이터를 `setup.providers[].envVars`로 미러링합니다.
...
- 이전에 지원되는 OpenClaw 버전에 아직 필요한 동안에만 `channelEnvVars`를 호환성 메타데이터로 유지합니다.
...
- 스키마 URL이 권고용일 뿐이면 제거합니다.
- OpenClaw가 문서화된 버전 지정 스키마를 게시한 뒤에만 이를 사용합니다.
- `clawhub package validate <path-to-plugin>`를 다시 실행합니다.
