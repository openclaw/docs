...
ClawHub는 게시 전에 Plugin 패키지를 검증하며, 자동 패키지 스캔의 발견 사항도 표시할 수 있습니다. 이 페이지는 작성자 대상 발견 사항을 다룹니다. 즉, Plugin 작성자가 패키지 메타데이터, 매니페스트, SDK 가져오기 또는 게시된 아티팩트에서 수정할 수 있는 발견 사항을 의미합니다.

내부 Plugin Inspector 커버리지 발견 사항은 다루지 않습니다. 전체 보고서에 작성자용 수정 안내가 없는 스캐너 유지 관리 코드가 포함되어 있다면, 이는 Plugin 작성자가 아니라 OpenClaw 유지 관리자를 위한 것입니다.
...
- `openclaw.plugin.json`에도 `version`이 있는 경우, 패키지 메타데이터가 권위 있는 기준이면 일치하도록 업데이트하거나 오래된 매니페스트 버전 메타데이터를 제거합니다.
...
- 패키지 엔트리포인트, 호환성, 설치, 설정 및 카탈로그 메타데이터는 지원되는 `package.json#openclaw` 필드에 유지합니다.
- [검색에 영향을 주는 package.json 필드](/ko/plugins/manifest#packagejson-fields-that-affect-discovery)를 참조하세요.
- `clawhub package validate <path-to-plugin>`을 다시 실행합니다.
...
- 패킹을 실패하게 만드는 잘못된 패키지 메타데이터, 깨진 수명 주기 스크립트 또는 files 항목을 수정합니다.
...
- 지원되지 않는 계약 키를 제거하세요.
- 런타임 동작은 Plugin 등록 코드로 옮기고, `contracts`는
  정적 기능 소유권 메타데이터로만 제한하세요.
- `clawhub package validate <path-to-plugin>`을 다시 실행하세요.
...
- 범위가 좁은 가져오기를 찾으려면 [가져오기 규칙](/ko/plugins/building-plugins#import-conventions)과
...
- 전체 세션 저장소 객체를 로드하고 변경한 뒤 저장하는 방식은 피하세요.
...
- 모델 또는 공급자 오버라이드 작업은 `before_model_resolve`로 이동하세요.
...
- 공급자 env-var 메타데이터를 `setup.providers[].envVars`에 미러링하세요.
- 지원하는 OpenClaw 범위가 이를 아직 필요로 하는 동안에만
...
- OpenClaw가 채널 런타임을 로드하지 않고도 setup 상태를 검사할 수 있도록
  채널 env-var 메타데이터를 선언형으로 유지하세요.
- env 기반 채널 setup을 Plugin 형태에서 사용하는 현재 setup, 채널 config 또는
  패키지 채널 메타데이터에 미러링하세요.
- 지원하는 이전 OpenClaw 버전이 이를 아직 요구하는 동안에만
...
- OpenClaw가 문서화된 버전 지정 스키마를 게시한 뒤에만 사용하세요.
- `clawhub package validate <path-to-plugin>`을 다시 실행하세요.
