...
summary: 게시하기 전에 ClawHub Plugin 패키지 검증에서 발견된 문제 수정하기
title: Plugin 검증 수정 사항
x-i18n:
    generated_at: "2026-07-12T00:38:59Z"
    model: gpt-5.6
...
ClawHub는 게시 전에 Plugin 패키지를 검증하며 자동 패키지 검사에서 발견된 문제도 표시할 수 있습니다. 이 페이지에서는 Plugin 작성자가 패키지 메타데이터, 매니페스트, SDK 가져오기 또는 게시된 아티팩트에서 수정할 수 있는 작성자 대상 발견 사항을 다룹니다.

내부 Plugin Inspector 검사 범위 발견 사항은 다루지 않습니다. 전체 보고서에 작성자 수정 지침 없이 스캐너 유지관리 코드가 포함되어 있다면, 이는 Plugin 작성자가 아닌 OpenClaw 유지관리자를 위한 것입니다.

수정 사항을 적용한 후 다음 명령을 다시 실행합니다.
...
| `package-manifest-version-drift`        | [패키지와 매니페스트 버전 일치시키기](/ko/clawhub/plugin-validation-fixes#package-manifest-version-drift)                          |
...
패키지 루트에 `package.json`이 없어 ClawHub가 npm 패키지, 버전, 진입점 또는 OpenClaw 메타데이터를 식별할 수 없습니다.

- `name`, `version`, `type`을 포함하는 `package.json`을 추가합니다.
- 패키지에 OpenClaw Plugin이 포함되는 경우 `openclaw` 블록을 추가합니다.
- 최소 패키지 예시는 [Plugin 빌드](/ko/plugins/building-plugins)를, 패키지와 매니페스트의 구분은 [Plugin 매니페스트](/ko/plugins/manifest#manifest-versus-packagejson)를 참조하세요.
- `clawhub package validate <path-to-plugin>`을 다시 실행합니다.

### package-openclaw-metadata-missing

패키지에 `package.json`이 있지만 OpenClaw 패키지 메타데이터가 선언되어 있지 않습니다.

- `package.json#openclaw`를 추가합니다.
- `openclaw.extensions` 또는 `openclaw.runtimeExtensions`와 같은 진입점 메타데이터를 포함합니다.
...
패키지 메타데이터는 존재하지만 OpenClaw 런타임 진입점이 선언되어 있지 않습니다.
...
패키지에 OpenClaw 진입점이 선언되어 있지만 참조된 파일이 검증 중인 패키지에 없습니다.
...
- `openclaw.install`에 `clawhubSpec`, `npmSpec`, `localPath` 등의 지원되는 설치 소스를 입력합니다.
...
- 빌드하고 테스트할 때 기준으로 사용한 OpenClaw Plugin API 버전 또는 semver 최저 버전을 사용합니다.
- 이를 패키지 버전과 구분합니다. 패키지 버전은 Plugin 릴리스를 설명하고, `openclaw.compat.pluginApi`는 호스트 API 계약을 설명합니다.
- [검색에 영향을 주는 package.json 필드](/ko/plugins/manifest#packagejson-fields-that-affect-discovery)를 참조하세요.
- `clawhub package validate <path-to-plugin>`을 다시 실행합니다.

### package-min-host-version-drift

패키지의 최소 호스트 버전이 패키지를 빌드할 때 기준으로 사용한 OpenClaw 버전 메타데이터와 일치하지 않습니다.

- `openclaw.install.minHostVersion`을 확인합니다.
- 릴리스 중 사용한 OpenClaw 버전 등 패키지의 모든 OpenClaw 빌드 메타데이터를 확인합니다.
- 최소 호스트 버전을 패키지가 실제로 지원하는 호스트 버전 범위와 일치시킵니다.
- [검색에 영향을 주는 package.json 필드](/ko/plugins/manifest#packagejson-fields-that-affect-discovery)를 참조하세요.
- `clawhub package validate <path-to-plugin>`을 다시 실행합니다.
...
- `openclaw.bundle`과 같이 지원되지 않는 필드를 제거합니다.
...
- 패키징에 실패하게 만드는 잘못된 패키지 메타데이터, 손상된 수명 주기 스크립트 또는 files 항목을 수정합니다.
- 이 패키지를 공개 게시하려는 경우 `private: true`를 제거합니다.
- `clawhub package validate <path-to-plugin>`을 다시 실행합니다.

### package-npm-pack-entrypoint-missing

패키지를 패키징할 수 있지만 패키징된 아티팩트에 `package.json#openclaw`에 선언된 진입점 파일이 포함되어 있지 않습니다.

- `npm pack --dry-run`을 실행하고 포함될 파일을 검사합니다.
- 패키징 전에 생성되는 진입점을 빌드합니다.
...
패키징된 아티팩트에 소스 패키지에 존재하는 OpenClaw 메타데이터가 없습니다.
...
- `name`은 사람이 읽을 수 있게 유지하고, `id`는 안정적인 시스템 ID로 유지하세요.
- [Plugin 매니페스트](/ko/plugins/manifest)를 참조하세요.
...
- `openclaw.plugin.json`에서 사용자 정의 필드를 제거하세요.
...
- `contracts` 아래의 각 키를
  [계약 참조](/ko/plugins/manifest#contracts-reference)와 비교하세요.
- 지원되지 않는 계약 키를 제거하세요.
...
- 루트 배럴 가져오기를 범위가 명확한 공개 하위 경로 가져오기로 교체하세요.
- `definePluginEntry`에는 `openclaw/plugin-sdk/plugin-entry`를 사용하세요.
- 채널 진입점 도우미에는 `openclaw/plugin-sdk/channel-core`를 사용하세요.
- [가져오기 규칙](/ko/plugins/building-plugins#import-conventions)과
  [Plugin SDK 하위 경로](/ko/plugins/sdk-subpaths)를 사용하여 범위가 좁은 가져오기 경로를 찾으세요.
- `clawhub package validate <path-to-plugin>`을 다시 실행하세요.
...
- 해당 동작에 공개 SDK가 없다면 도우미를 패키지 내부에 유지하거나
  공개 OpenClaw API를 요청하세요.
- [Plugin SDK 하위 경로](/ko/plugins/sdk-subpaths)와
  [SDK 마이그레이션](/ko/plugins/sdk-migration)을 사용하여 지원되는 가져오기 경로를 선택하세요.
- `clawhub package validate <path-to-plugin>`을 다시 실행하세요.

### sdk-load-session-store

Plugin이 여전히 사용 중단된 전체 세션 저장소 도우미
...
- 세션 상태를 쓸 때는 `patchSessionEntry(...)` 또는 `upsertSessionEntry(...)`를
  사용하세요.
- 전체 세션 저장소 객체를 불러와 변경한 후 저장하지 마세요.
- 선언된 호환성 범위가 이 도우미를 필요로 하는 이전 OpenClaw 버전을
  계속 지원하는 동안에만 `loadSessionStore(...)`를 유지하세요.
...
Plugin이 여전히 `saveSessionStore` 또는 `updateSessionStore` 같은 사용 중단된
전체 세션 저장소 쓰기 도우미를 사용합니다.

- 기존 세션 항목의 필드를 업데이트할 때는 `patchSessionEntry(...)`를
  사용하세요.
- 세션 항목을 교체하거나 생성할 때는 `upsertSessionEntry(...)`를 사용하세요.
- 전체 세션 저장소 객체를 불러와 변경한 후 저장하지 마세요.
- 선언된 호환성 범위가 이러한 도우미를 필요로 하는 이전 OpenClaw 버전을
  계속 지원하는 동안에만 전체 저장소 쓰기 도우미를 유지하세요.
- [런타임 API](/ko/plugins/sdk-runtime#agent-session-state)와
  [Plugin SDK 하위 경로](/ko/plugins/sdk-subpaths)를 참조하세요.
- `clawhub package validate <path-to-plugin>`을 다시 실행하세요.
...
Plugin이 여전히 `resolveSessionFilePath` 또는 `resolveAndPersistSessionFile` 같은
사용 중단된 세션 파일 경로 도우미를 사용합니다.

- 에이전트 및 세션 ID를 기준으로 세션 메타데이터를 읽으려면
  `getSessionEntry(...)`를 사용하세요.
- 세션 메타데이터를 영구 저장하려면 `patchSessionEntry(...)` 또는
  `upsertSessionEntry(...)`를 사용하세요.
- 코드가 트랜스크립트 작업을 준비하는 경우 트랜스크립트 ID 또는 대상
  도우미를 사용하세요.
- 레거시 트랜스크립트 파일 경로를 영구 저장하거나 이에 의존하지 마세요.
- [런타임 API](/ko/plugins/sdk-runtime#agent-session-state)와
  [Plugin SDK 하위 경로](/ko/plugins/sdk-subpaths)를 참조하세요.
- `clawhub package validate <path-to-plugin>`을 다시 실행하세요.
...
Plugin이 여전히 사용 중단된 트랜스크립트 파일 대상 도우미
...
- 코드에 구조화된 트랜스크립트 작업 대상이 필요한 경우
  `resolveSessionTranscriptTarget(...)`을 사용하세요.
- 레거시 트랜스크립트 파일 대상을 직접 읽거나 구성하지 마세요.
- 선언된 호환성 범위가 이 도우미를 필요로 하는 이전 OpenClaw 버전을
  계속 지원하는 동안에만 레거시 도우미를 유지하세요.
- [런타임 API](/ko/plugins/sdk-runtime#agent-session-state)와
  [Plugin SDK 하위 경로](/ko/plugins/sdk-subpaths)를 참조하세요.
- `clawhub package validate <path-to-plugin>`을 다시 실행하세요.
...
`emitSessionTranscriptUpdate` 같은 사용 중단된 저수준 트랜스크립트 도우미를 사용합니다.

- 트랜스크립트에 추가할 때는 `appendSessionTranscriptMessageByIdentity(...)`를 사용하세요.
- 트랜스크립트 업데이트 알림에는
  `publishSessionTranscriptUpdateByIdentity(...)`를 사용하세요.
- OpenClaw가 올바른 트랜잭션 경계와 ID 처리를 적용할 수 있도록 구조화된
  트랜스크립트 런타임 인터페이스를 사용하세요.
- 선언된 호환성 범위가 이러한 도우미를 필요로 하는 이전 OpenClaw 버전을
  계속 지원하는 동안에만 저수준 트랜스크립트 도우미를 유지하세요.
- [런타임 API](/ko/plugins/sdk-runtime#agent-session-state)와
  [Plugin SDK 하위 경로](/ko/plugins/sdk-subpaths)를 참조하세요.
- `clawhub package validate <path-to-plugin>`을 다시 실행하세요.
...
- 모델 또는 공급자 재정의 작업을 `before_model_resolve`로 이동하세요.
- 프롬프트 또는 컨텍스트 변경 작업을 `before_prompt_build`로 이동하세요.
- 선언된 호환성 범위가 이 훅을 필요로 하는 이전 OpenClaw 버전을
  계속 지원하는 동안에만 `before_agent_start`를 유지하세요.
...
- 공급자 환경 변수 메타데이터를 `setup.providers[].envVars`에도 반영하세요.
- 지원되는 OpenClaw 버전 범위에서 여전히 필요한 동안에만
  `providerAuthEnvVars`를 호환성 메타데이터로 유지하세요.
...
- 환경 변수 기반 채널 설정을 Plugin 구조에서 사용하는 현재 설정, 채널 구성
  또는 패키지 채널 메타데이터에 반영하세요.
- 지원되는 이전 OpenClaw 버전에서 여전히 필요한 동안에만 `channelEnvVars`를
  호환성 메타데이터로 유지하세요.
...
패키지에 ClawHub가 사용 가능한 것으로 인식하지 않는 스키마 참조가 포함된
`openclaw.security.json`이 제공됩니다.

- 스키마 URL이 참고용일 뿐이라면 제거하세요.
- OpenClaw가 버전이 지정된 스키마를 게시한 후에만 문서화된 스키마를 사용하세요.
- `clawhub package validate <path-to-plugin>`을 다시 실행하세요.

### unrecognized-security-manifest

패키지에 지원되지 않는 보안 매니페스트 파일이 포함되어 있습니다.

- OpenClaw가 버전이 지정된 보안 매니페스트 스키마와 ClawHub 동작을 문서화할
  때까지 `openclaw.security.json`을 제거하세요.
