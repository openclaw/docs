...
title: Plugin 검증 수정
x-i18n:
    generated_at: "2026-06-28T05:30:26Z"
    model: gpt-5.5
...
ClawHub는 게시 전에 Plugin 패키지를 검증하며, 자동 패키지 스캔의 발견 항목도 표시할 수 있습니다. 이 페이지는 작성자 대상 발견 항목, 즉 Plugin 작성자가 패키지 메타데이터, 매니페스트, SDK 가져오기 또는 게시된 아티팩트에서 수정할 수 있는 발견 항목을 다룹니다.
...
- `openclaw.plugin.json`에도 `version`이 있다면, 패키지 메타데이터가 권위 있는 경우 일치하도록 업데이트하거나 오래된 매니페스트 버전 메타데이터를 제거하세요.
...
패키지를 패킹할 수 있지만, 패킹된 아티팩트에 `package.json#openclaw`에 선언된 진입점 파일이 포함되지 않습니다.
...
- 런타임 동작은 Plugin 등록 코드로 이동하고, `contracts`는 정적 capability 소유권 메타데이터로만
  제한하세요.
- `clawhub package validate <path-to-plugin>`를 다시 실행하세요.
...
- 채널 entry helper에는 `openclaw/plugin-sdk/channel-core`를 사용하세요.
...
- 해당 동작에 공개 SDK가 없다면 helper를 패키지 안에 유지하거나
...
Plugin이 아직 더 이상 사용되지 않는 전체 session-store helper인
...
- 모델 또는 제공자 override 작업을 `before_model_resolve`로 이동하세요.
...
- 제공자 env-var 메타데이터를 `setup.providers[].envVars`로 미러링하세요.
- 지원하는 OpenClaw 범위에서 아직 필요로 하는 동안에만 `providerAuthEnvVars`를 호환성 메타데이터로
  유지하세요.
...
- OpenClaw가 채널 런타임을 로드하지 않고도 setup 상태를 검사할 수 있도록 채널 env-var 메타데이터를 선언형으로 유지하세요.
- env 기반 채널 setup을 Plugin 형태에서 사용하는 현재 setup, 채널 config 또는 패키지 채널 메타데이터로
  미러링하세요.
- 이전에 지원되는 OpenClaw 버전에서 아직 필요로 하는 동안에만 `channelEnvVars`를 호환성 메타데이터로 유지하세요.
...
패키지가 ClawHub에서 사용 가능하다고 인식하지 못하는 스키마 참조가 포함된 `openclaw.security.json`을 제공합니다.

- 스키마 URL이 참고용일 뿐이라면 제거하세요.
- OpenClaw가 버전이 지정된 스키마를 게시한 후에만 문서화된 버전 스키마를 사용하세요.
- `clawhub package validate <path-to-plugin>`를 다시 실행하세요.
