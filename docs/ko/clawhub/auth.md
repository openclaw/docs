...
x-i18n:
    generated_at: "2026-06-30T13:51:28Z"
    model: gpt-5.5
...
ClawHub는 웹 로그인을 위해 GitHub를 사용합니다. CLI는 로그인한 계정으로 생성한 ClawHub API 토큰을 사용합니다.

## 웹 로그인

[clawhub.ai](https://clawhub.ai)에서 GitHub로 로그인하세요.

삭제, 차단 또는 비활성화된 계정은 일반적인 ClawHub 로그인을 완료할 수 없습니다. 로그인 후 로그아웃 상태로 돌아간다면 계정 상태가 양호하지 않을 수 있습니다. 계정이 차단되었거나 비활성화되었다면, 이것이 실수라고 생각하는 경우 [ClawHub 이의 신청 양식](https://appeals.openclaw.ai/)을 사용하세요.
...
삭제, 차단 또는 비활성화된 계정은 기존 API 토큰을 계속 사용할 수 없습니다. 계정이 차단되었거나 비활성화되었다면, 이것이 실수라고 생각하는 경우 [ClawHub 이의 신청 양식](https://appeals.openclaw.ai/)을 사용하세요.
