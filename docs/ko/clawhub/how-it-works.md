...
x-i18n:
    generated_at: "2026-06-28T05:29:12Z"
    model: gpt-5.5
...
목록 페이지는 사용자가 Skill 또는 Plugin을 설치하기 전에 그것이 수행한다고
주장하는 내용을 확인하는 표준 위치입니다.

## Skills

Skill은 `SKILL.md`를 중심으로 하는 버전 관리된 텍스트 번들입니다. 여기에는
지원 파일, 예제, 템플릿, 스크립트가 포함될 수 있습니다.

ClawHub는 `SKILL.md` 프런트매터를 읽어 Skill 이름, 설명, 요구 사항,
환경 변수, 메타데이터를 이해합니다. 정확한 메타데이터는 사용자가 Skill을
설치할지 결정하는 데 도움이 되고, 자동화된 검사가 선언된 동작과 관찰된 동작
...
레지스트리를 신뢰할 수 있는 원본으로 사용하려면 명시적인 ClawHub 설치
소스를 사용하세요.
...
업로드 전에 해석된 페이로드를 미리 보려면 드라이 런을 사용하세요. 그러면
공개 페이지에 게시된 메타데이터, 파일, 소스 출처, 검사 상태가 표시됩니다.
...
작업 영역 밖에서 레지스트리 관리형 Skill 폴더를 원하는 사용자를 위해 직접
Skill 설치 및 업데이트 워크플로도 지원합니다.

## 보안 상태

ClawHub는 게시에 열려 있지만, 릴리스는 여전히 업로드 게이트, 자동화된
검사, 사용자 신고, 관리자 조치의 적용을 받습니다.

공개 페이지는 사용 가능한 경우 검사 요약을 표시합니다. 보류, 숨김 또는
차단된 콘텐츠는 진단을 위해 소유자에게는 계속 표시되지만, 공개 검색 및 설치
흐름에서는 사라질 수 있습니다.
...
속도 제한을 준수하며, 보증을 암시하지 않는 경우 이러한 API를 사용할 수
있습니다.
