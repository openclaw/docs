...
summary: ClawHub 등록 항목, 버전, 설치, 게시, 검사 및 업데이트가 작동하는 방식.
x-i18n:
...
Plugin이 수행한다고 주장하는 내용을 확인하는 표준 위치입니다.

## Skills

skill은 `SKILL.md`를 중심으로 구성된 버전이 지정된 텍스트 번들입니다. 여기에는
...
자동 스캔이 선언된 동작과 관찰된 동작 간의 불일치를 감지하는 데 도움이 되기 때문에 중요합니다.
...
레지스트리를 신뢰 가능한 원본으로 사용하려면 명시적인 ClawHub 설치 소스를 사용하세요.
...
업로드 전에 해석된 페이로드를 미리 보려면 드라이 런을 사용하세요. 그런 다음 공개 페이지에는
...
레지스트리 패키지를 해석할 수 있습니다. 또한 ClawHub CLI는 전체 OpenClaw 워크스페이스 외부에서
...
자동 확인, 사용자 신고, 모더레이터 조치가 적용됩니다.

공개 페이지는 사용 가능한 경우 스캔 요약을 표시합니다. 보류, 숨김 또는
차단된 콘텐츠는 진단을 위해 소유자에게는 계속 표시되지만,
...
서드 파티 카탈로그는 표준 ClawHub 목록으로 다시 연결하고,
