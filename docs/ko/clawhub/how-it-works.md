...
패키지를 찾을 수 있는 공간을 제공하고, 게시자가 버전을 릴리스할 수 있는
...
목록 페이지는 사용자가 설치하기 전에 Skills 또는 Plugin이 수행한다고 주장하는
내용을 확인하는 표준 위치입니다.

## Skills

Skills는 `SKILL.md`를 중심으로 구성된 버전 관리 텍스트 번들입니다. 지원 파일,
예제, 템플릿 및 스크립트를 포함할 수 있습니다.

ClawHub는 `SKILL.md` frontmatter를 읽어 Skills 이름, 설명, 요구 사항, 환경 변수
및 메타데이터를 이해합니다. 정확한 메타데이터는 사용자가 Skills 설치 여부를
결정하는 데 도움을 주고, 자동 스캔이 선언된 동작과 관찰된 동작 사이의 불일치를
...
레지스트리를 진실 공급원으로 사용하려면 명시적인 ClawHub 설치 소스를
사용하세요.
...
페이지에 게시된 메타데이터, 파일, 소스 출처 및 스캔 상태가 표시됩니다.
...
공간 외부에서 레지스트리 관리형 Skills 폴더를 사용하려는 사용자를 위해 직접
Skills 설치 및 업데이트 워크플로도 지원합니다.

## 보안 상태

ClawHub는 게시에 열려 있지만, 릴리스에는 여전히 업로드 게이트, 자동 검사,
사용자 신고 및 중재자 조치가 적용됩니다.
...
ClawHub는 발견, 검색, 패키지 세부 정보 및 다운로드를 위한 공개 읽기 API를
제공합니다. 서드 파티 카탈로그는 표준 ClawHub 목록으로 다시 링크하고, 속도
