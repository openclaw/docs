...
목록 페이지는 사용자가 스킬이나 Plugin을 설치하기 전에 그것이 수행한다고
...
메타데이터를 파악합니다. 정확한 메타데이터는 사용자가 스킬 설치 여부를
...
업로드 전에 해석된 페이로드를 미리 보려면 dry run을 사용하세요. 그러면 공개
...
공간 밖에서 레지스트리 관리 스킬 폴더를 원하는 사용자를 위해 직접 스킬 설치 및
업데이트 워크플로도 지원합니다.
...
공개 페이지는 가능한 경우 스캔 요약을 표시합니다. 보류, 숨김 또는 차단된
