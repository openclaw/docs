...
Skill은 `SKILL.md`를 중심으로 하는 버전 관리 텍스트 번들입니다. 여기에는
지원 파일, 예제, 템플릿, 스크립트가 포함될 수 있습니다.

ClawHub는 Skill 이름, 설명, 요구 사항, 환경 변수, 메타데이터를 이해하기 위해
`SKILL.md` frontmatter를 읽습니다. 정확한 메타데이터는 사용자가 Skill 설치
...
OpenClaw가 ClawHub에서 Plugin을 설치할 때, 설치 전에 표시된 호환성 메타데이터를
확인합니다. 패키지 레코드에는 API 호환성, 최소 Gateway 버전, 호스트 대상,
환경 요구 사항, 아티팩트 다이제스트가 포함될 수 있습니다.

레지스트리를 신뢰할 수 있는 기준으로 삼고 싶을 때는 명시적인 ClawHub 설치
소스를 사용하세요.
...
업로드 전에 해석된 페이로드를 미리 보려면 dry run을 사용하세요. 그런 다음
...
외부에서 레지스트리 관리 Skill 폴더를 사용하려는 사용자를 위해 직접 Skill 설치
및 업데이트 워크플로도 지원합니다.
