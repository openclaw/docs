...
ClawHub는 OpenClaw Skills와 plugins를 위한 레지스트리 계층입니다. 사용자가
...
plugin이 설치 전에 수행한다고 주장하는 내용을 확인할 수 있는 표준 위치입니다.

## Skills

skill은 `SKILL.md`를 중심으로 하는 버전 관리되는 텍스트 번들입니다. 지원
파일, 예시, 템플릿, 스크립트를 포함할 수 있습니다.
...
레지스트리를 신뢰할 수 있는 출처로 사용하려는 경우 명시적인 ClawHub 설치 소스를 사용하세요.
...
게시는 새로운 불변 버전 레코드를 생성합니다. 게시자는 인증된 레지스트리 워크플로에 `clawhub`
CLI를 사용합니다.
...
레지스트리 패키지를 해석할 수 있습니다. ClawHub CLI는 전체 OpenClaw
작업 공간 외부에서 레지스트리로 관리되는 skill 폴더를 원하는 사용자를 위해 직접 skill 설치 및
업데이트 워크플로도 지원합니다.

## 보안 상태

ClawHub는 게시에 열려 있지만, 릴리스는 여전히 업로드 게이트,
자동화된 검사, 사용자 신고, 모더레이터 조치의 적용을 받습니다.

공개 페이지는 사용 가능한 경우 스캔 요약을 표시합니다. 보류, 숨김,
또는 차단된 콘텐츠는 진단을 위해 소유자에게는 계속 표시되면서
...
[공개 API](/clawhub/api)와 [HTTP API](/clawhub/http-api)를 참조하세요.
