...
장소를 제공하며, OpenClaw가 해당 패키지를 안전하게 설치하고 업데이트할 수
있도록 충분한 메타데이터를 제공합니다.
...
Plugin이 설치 전에 무엇을 한다고 주장하는지 확인할 수 있는 표준 위치입니다.

## Skills

Skill은 `SKILL.md`를 중심으로 한 버전 관리되는 텍스트 번들입니다. 지원 파일,
예제, 템플릿, 스크립트를 포함할 수 있습니다.

ClawHub는 `SKILL.md` 프런트매터를 읽어 Skill 이름, 설명, 요구 사항, 환경 변수,
메타데이터를 이해합니다. 정확한 메타데이터는 사용자가 Skill 설치 여부를
...
레지스트리를 신뢰 소스로 사용하려면 명시적인 ClawHub 설치 소스를 사용하세요.
...
업로드 전에 해석된 페이로드를 미리 보려면 드라이런을 사용하세요. 그런 다음
...
설치 소스 메타데이터를 기록합니다. ClawHub CLI는 전체 OpenClaw 작업 영역
외부에서 레지스트리가 관리하는 Skill 폴더를 원하는 사용자를 위해 직접 Skill
설치 및 업데이트 워크플로도 지원합니다.

## 보안 상태

ClawHub는 게시에 열려 있지만, 릴리스는 여전히 업로드 게이트, 자동화된 검사,
사용자 신고, 모더레이터 조치의 적용을 받습니다.

공개 페이지는 사용 가능한 경우 스캔 요약을 표시합니다. 보류, 숨김 또는 차단된
콘텐츠는 소유자에게는 진단용으로 계속 표시되지만 공개 검색 및 설치 흐름에서는
사라질 수 있습니다.
