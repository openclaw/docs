...
summary: ClawHub 목록, 버전, 설치, 게시, 검사 및 업데이트의 작동 방식.
x-i18n:
    generated_at: "2026-07-12T21:33:23Z"
    model: gpt-5.6
...
패키지를 탐색할 수 있는 공간, 게시자가 버전을 릴리스할 수 있는 공간을 제공하며,
...
환경 변수 및 메타데이터를 파악합니다. 정확한 메타데이터는 사용자가 Skills 설치
여부를 결정하는 데 도움을 주고, 자동 검사에서 선언된 동작과 관찰된 동작 간의
불일치를 감지하는 데도 도움이 되므로 중요합니다.
...
OpenClaw가 ClawHub에서 Plugin을 설치할 때는 설치하기 전에 명시된 호환성
메타데이터를 확인합니다. 패키지 레코드에는 API 호환성, 최소 Gateway 버전,
호스트 대상, 환경 요구 사항 및 아티팩트 다이제스트가 포함될 수 있습니다.

레지스트리를 기준 정보 소스로 사용하려면 명시적인 ClawHub 설치 소스를 사용하십시오.
...
업로드 전에 해석된 페이로드를 미리 보려면 시험 실행을 사용하십시오. 그러면 공개
페이지에 게시된 메타데이터, 파일, 소스 출처 표시 및 검사 상태가 표시됩니다.
...
OpenClaw는 나중에 업데이트할 때 동일한 레지스트리 패키지를 확인할 수 있도록 설치
소스 메타데이터를 기록합니다. ClawHub CLI는 전체 OpenClaw 작업 공간 외부에
레지스트리가 관리하는 Skills 폴더를 두려는 사용자를 위해 Skills 직접 설치 및
업데이트 워크플로도 지원합니다.

## 보안 상태

ClawHub는 누구나 게시할 수 있지만, 릴리스에는 여전히 업로드 게이트, 자동 검사,
사용자 신고 및 검토자 조치가 적용됩니다.

공개 페이지에는 가능한 경우 검사 요약이 표시됩니다. 보류, 숨김 또는 차단된
콘텐츠는 소유자에게 진단 목적으로 계속 표시되지만 공개 검색 및 설치 흐름에서는
사라질 수 있습니다.

[보안](/clawhub/security), [보안 감사](/clawhub/security-audits),
[검토 및 계정 안전](/ko/clawhub/moderation) 및
[허용되는 사용 방식](/clawhub/acceptable-usage)을 참조하십시오.

## API 액세스

ClawHub는 탐색, 검색, 패키지 세부 정보 및 다운로드를 위한 공개 읽기 API를
제공합니다. 서드 파티 카탈로그는 기준 ClawHub 목록으로 다시 연결하고, 사용량
제한을 준수하며, ClawHub가 보증한다는 인상을 주지 않는 경우 이러한 API를
사용할 수 있습니다.
