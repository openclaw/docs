...
plugin이 수행한다고 주장하는 내용을 살펴볼 수 있는 표준 위치입니다.
...
설치할지 결정하는 데 도움이 되고, 자동 스캔이 선언된 동작과 관찰된 동작 간의
...
업로드 전에 확인된 페이로드를 미리 보려면 dry run을 사용하세요. 그러면 공개
페이지에 게시된 메타데이터, 파일, 소스 출처 표시, 스캔 상태가 표시됩니다.
...
공간 외부에서 레지스트리 관리 skill 폴더를 원하는 사용자를 위해 직접 skill
...
콘텐츠는 진단을 위해 소유자에게 계속 표시되더라도 공개 검색 및 설치 흐름에서
사라질 수 있습니다.
...
제공합니다. 서드파티 카탈로그는 표준 ClawHub 목록으로 다시 연결하고, 속도
