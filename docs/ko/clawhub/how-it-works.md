...
패키지를 발견할 수 있는 곳을 제공하고, 게시자가 버전을 릴리스할 수 있는
곳을 제공하며, OpenClaw가 해당 패키지를 안전하게 설치하고 업데이트하는 데
...
목록 페이지는 사용자가 Skills 또는 Plugin을 설치하기 전에 그 기능 주장을
검토하는 표준 위치입니다.

## Skills

Skills는 `SKILL.md`를 중심으로 한 버전 관리 텍스트 번들입니다. 지원 파일,
예제, 템플릿, 스크립트를 포함할 수 있습니다.

ClawHub는 `SKILL.md` frontmatter를 읽어 Skills 이름, 설명, 요구 사항,
환경 변수, 메타데이터를 이해합니다. 정확한 메타데이터는 사용자가 Skills를
설치할지 결정하는 데 도움이 되고, 자동 스캔이 선언된 동작과 관찰된 동작
...
OpenClaw가 ClawHub에서 Plugin을 설치할 때, 설치하기 전에 게시된 호환성
메타데이터를 확인합니다. 패키지 레코드에는 API 호환성, 최소 gateway 버전,
호스트 대상, 환경 요구 사항, 아티팩트 다이제스트가 포함될 수 있습니다.

레지스트리를 신뢰할 수 있는 원천으로 사용하려면 명시적인 ClawHub 설치
소스를 사용하세요.
...
업로드 전에 해석된 페이로드를 미리 보려면 dry run을 사용하세요. 그러면
공개 페이지에 게시된 메타데이터, 파일, 출처 표시, 스캔 상태가 표시됩니다.
...
작업 영역 외부에서 레지스트리 관리 Skills 폴더를 원하는 사용자를 위해
...
공개 페이지는 사용 가능한 경우 스캔 요약을 표시합니다. 보류, 숨김 또는
차단된 콘텐츠는 소유자에게는 진단 목적으로 계속 표시되지만, 공개 검색 및
설치 흐름에서는 사라질 수 있습니다.
...
노출합니다. 서드파티 카탈로그는 표준 ClawHub 목록으로 다시 연결하고,
rate limit을 준수하며, 보증을 암시하지 않는 경우 이러한 API를 사용할 수
있습니다.
