...
x-i18n:
    generated_at: "2026-06-30T22:08:33Z"
    model: gpt-5.5
...
목록 페이지는 사용자가 Skills 또는 Plugin이 수행한다고 주장하는 내용을
설치 전에 확인하는 표준 위치입니다.

## Skills

Skills는 `SKILL.md`를 중심으로 하는 버전 관리 텍스트 번들입니다. 여기에는
지원 파일, 예제, 템플릿, 스크립트가 포함될 수 있습니다.

ClawHub는 `SKILL.md` frontmatter를 읽어 Skills 이름, 설명, 요구 사항,
환경 변수, 메타데이터를 이해합니다. 정확한 메타데이터는 사용자가 Skills 설치 여부를
...
레지스트리를 신뢰할 수 있는 소스로 삼으려면 명시적인 ClawHub 설치 소스를 사용하세요.
...
업로드 전에 확인된 페이로드를 미리 보려면 dry run을 사용하세요. 그런 다음 공개
...
외부에서 레지스트리 관리 Skills 폴더를 원하는 사용자를 위해 직접 Skills 설치 및
업데이트 워크플로도 지원합니다.
...
소유자에게 진단용으로 계속 표시되는 동안 공개 검색 및 설치 흐름에서 사라질 수 있습니다.
...
서드 파티 카탈로그는 표준 ClawHub 목록으로 다시 연결하고, 속도 제한을 준수하며,
