...
summary: ClawHub 목록, 버전, 설치, 게시, 검사, 업데이트의 작동 방식.
x-i18n:
    generated_at: "2026-07-01T12:51:59Z"
    model: gpt-5.5
...
목록 페이지는 사용자가 스킬이나 Plugin을 설치하기 전에 그것이 무엇을 한다고
주장하는지 확인할 수 있는 정식 장소입니다.
...
ClawHub는 스킬 이름, 설명, 요구 사항, 환경 변수, 메타데이터를 이해하기 위해
`SKILL.md` frontmatter를 읽습니다. 정확한 메타데이터는 사용자가 스킬 설치 여부를
...
업로드 전에 해석된 페이로드를 미리 보려면 dry run을 사용하세요. 그런 다음 공개
...
제공합니다. 타사 카탈로그는 정식 ClawHub 목록으로 다시 연결하고, 속도 제한을
