...
ClawHub의 공개 읽기 API를 기반으로 타사 카탈로그, 디렉터리 또는 검색 화면을 만들 수 있습니다. 공개 스킬 메타데이터와 스킬 파일은 ClawHub의 스킬 라이선스 규칙에 따라 게시되며, API 자체에는 속도 제한이 적용되므로 책임 있게 사용해야 합니다.
...
- 목록을 표시할 때 사용자가 소스 레지스트리 레코드를 확인할 수 있도록 표준 ClawHub 스킬 URL로 다시 링크하세요.
