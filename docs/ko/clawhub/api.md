...
x-i18n:
    generated_at: "2026-06-28T05:28:50Z"
    model: gpt-5.5
...
- 목록을 표시할 때 사용자가 소스 레지스트리 레코드를 확인할 수 있도록 정식 ClawHub Skill URL로 다시 연결하세요.
...
- `X-RateLimit-Remaining` / `RateLimit-Remaining`: 있을 경우 정확한 남은 한도입니다.
...
- `GET /api/v1/download?slug=&version=&tag=`
  ...
  - 현재 GitHub 기반 Skills에서 `clean` 또는 `suspicious` 스캔 결과가 있으면 ClawHub 바이트 대신
    JSON `public-github` 인계 설명자를 반환합니다.
- `GET /api/v1/skills/export?startDate=&endDate=&limit=&cursor=`
  - 호스팅된 Skills는 저장된 파일로 내보내집니다.
  - 현재 GitHub 기반 Skills에서 `clean` 또는 `suspicious` 스캔 결과가 있으면
