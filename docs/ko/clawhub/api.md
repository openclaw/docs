...
ClawHub의 공개 읽기 API를 기반으로 타사 카탈로그, 디렉터리 또는 검색 화면을 구축할 수 있습니다. 공개 스킬 메타데이터와 스킬 파일은 ClawHub의 스킬 라이선스 규칙에 따라 게시되며, API 자체에는 속도 제한이 적용되므로 책임 있게 사용해야 합니다.
...
- 목록을 표시할 때 사용자가 소스 레지스트리 레코드를 확인할 수 있도록 정식 ClawHub 스킬 URL로 다시 연결하세요.
- `https://clawhub.ai/<owner>/<slug>` 형식의 정식 페이지 URL을 사용하세요.
- ClawHub가 타사 사이트를 보증, 검증 또는 운영한다는 인상을 주지 마세요.
- 공개 API 필터나 인증 경계를 우회하여 숨겨진 콘텐츠, 비공개 콘텐츠 또는 검토 차단된 콘텐츠를 미러링하지 마세요.
