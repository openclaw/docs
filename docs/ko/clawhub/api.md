...
ClawHub의 공개 읽기 API를 기반으로 서드파티 카탈로그, 디렉터리 또는 검색 화면을 구축할 수 있습니다. 공개 스킬 메타데이터와 스킬 파일은 ClawHub의 스킬 라이선스 규칙에 따라 게시되며, API 자체에는 속도 제한이 적용되므로 책임 있게 사용해야 합니다.
...
- `https://clawhub.ai/<owner>/<slug>` 형식의 정식 페이지 URL을 사용하세요.
...
- `POST /api/v1/users/reserve`는 소유자 핸들의 루트 슬러그와 비공개 릴리스 없는 패키지 자리표시자를 예약합니다.
