...
x-i18n:
    generated_at: "2026-07-02T08:05:55Z"
    model: gpt-5.5
...
ClawHub의 공개 읽기 API 위에 서드파티 카탈로그, 디렉터리 또는 검색 화면을 만들 수 있습니다. 공개 Skills 메타데이터와 Skills 파일은 ClawHub의 Skills 라이선스 규칙에 따라 게시되며, API 자체에는 속도 제한이 적용되므로 책임 있게 사용해야 합니다.
...
- 공개 API 필터나 인증 경계를 우회하여 숨김, 비공개 또는 검토 차단 콘텐츠를 미러링하지 마세요.
...
- `X-RateLimit-Remaining` / `RateLimit-Remaining`: 존재하는 경우 정확한 잔여 예산입니다.
