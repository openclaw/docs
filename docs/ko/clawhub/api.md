...
ClawHub의 공개 읽기 API 위에 서드파티 카탈로그, 디렉터리 또는 검색 화면을 구축할 수 있습니다. 공개 Skills 메타데이터와 Skills 파일은 ClawHub의 Skills 라이선스 규칙에 따라 게시되며, API 자체에는 속도 제한이 적용되므로 책임 있게 사용해야 합니다.
...
- `X-RateLimit-Remaining` / `RateLimit-Remaining`: 존재하는 경우 정확한 남은 한도입니다. 샤딩된 성공 요청은 대략적인 전역 값을 반환하는 대신 이를 생략합니다.
...
- `GET /api/v1/download?slug=&version=&tag=`
  ...
  - 현재 GitHub 기반 Skills 중 `clean` 또는 `suspicious` 스캔이 있는 경우 ClawHub 바이트 대신 JSON `public-github` 인계 설명자를 반환합니다.
- `GET /api/v1/skills/export?startDate=&endDate=&limit=&cursor=`
  - 호스팅된 Skills는 저장된 파일로 내보내집니다.
  - 현재 GitHub 기반 Skills 중 `clean` 또는 `suspicious` 스캔이 있는 경우 `public-github` 인계 설명자로 내보내집니다.
