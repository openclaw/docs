...
x-i18n:
    generated_at: "2026-07-04T20:27:57Z"
    model: gpt-5.5
...
ClawHub의 공개 읽기 API를 기반으로 서드파티 카탈로그, 디렉터리 또는 검색 화면을 만들 수 있습니다. 공개 Skills 메타데이터와 Skills 파일은 ClawHub의 Skills 라이선스 규칙에 따라 게시되며, API 자체에는 속도 제한이 적용되므로 책임 있게 사용해야 합니다.
...
- 누락되었거나 유효하지 않은 토큰은 IP 기반 적용으로 폴백됩니다.
...
- `X-RateLimit-Remaining` / `RateLimit-Remaining`: 존재할 경우 정확한 남은 예산입니다. 샤딩된 성공 요청은 근사 전역 값을 반환하는 대신 이를 생략합니다.
...
- `POST /api/v1/users/reserve`는 소유자 핸들의 루트 슬러그와 비공개 무릴리스 패키지 플레이스홀더를 예약합니다.
