...
ClawHub의 공개 읽기 API 위에 서드파티 카탈로그, 디렉터리 또는 검색 화면을 구축할 수 있습니다. 공개 스킬 메타데이터와 스킬 파일은 ClawHub의 스킬 라이선스 규칙에 따라 게시되며, API 자체에는 속도 제한이 적용되므로 책임감 있게 사용해야 합니다.
...
- 표준 페이지 URL은 `https://clawhub.ai/<owner>/<slug>` 형식으로 사용하세요.
- ClawHub가 해당 서드파티 사이트를 보증, 검증 또는 운영한다고 암시하지 마세요.
- 공개 API 필터 또는 인증 경계를 우회하여 숨김, 비공개 또는 검토 차단된 콘텐츠를 미러링하지 마세요.
...
- 누락되었거나 유효하지 않은 토큰은 IP 기준 적용으로 대체됩니다.
