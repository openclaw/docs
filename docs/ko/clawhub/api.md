...
x-i18n:
    generated_at: "2026-07-03T13:21:13Z"
    model: gpt-5.5
...
ClawHub의 공개 읽기 API 위에 서드파티 카탈로그, 디렉터리 또는 검색 표면을 구축할 수 있습니다. 공개 skill 메타데이터와 skill 파일은 ClawHub의 skill 라이선스 규칙에 따라 게시되며, API 자체에는 속도 제한이 적용되므로 책임 있게 사용해야 합니다.
...
- 사용자가 소스 레지스트리 레코드를 확인할 수 있도록 목록을 표시할 때 정식 ClawHub skill URL로 다시 링크하세요.
...
- `X-RateLimit-Remaining` / `RateLimit-Remaining`: 있을 경우 정확한 남은 예산입니다. 샤딩된 성공 요청은 근사 전역 값을 반환하는 대신 이를 생략합니다.
...
- `POST /api/v1/users/reserve`는 소유자 핸들을 위해 루트 slug와 비공개 미릴리스 패키지 자리표시자를 예약합니다.
