...
ClawHub의 공개 읽기 API 위에 서드파티 카탈로그, 디렉터리 또는 검색 화면을 구축할 수 있습니다. 공개 skill 메타데이터와 skill 파일은 ClawHub의 skill 라이선스 규칙에 따라 게시되며, API 자체에는 속도 제한이 있으므로 책임감 있게 사용해야 합니다.
...
- `Retry-After`: `429`에서 대기해야 하는 지연 시간(초)
...
- `POST /api/v1/users/reserve`는 소유자 핸들을 위해 루트 slugs와 비공개 무릴리스 패키지 플레이스홀더를 예약합니다.
