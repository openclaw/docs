...
x-i18n:
    generated_at: "2026-07-05T05:33:49Z"
    model: gpt-5.5
...
ClawHub의 공개 읽기 API를 기반으로 서드파티 카탈로그, 디렉터리 또는 검색 표면을 만들 수 있습니다. 공개 스킬 메타데이터와 스킬 파일은 ClawHub의 스킬 라이선스 규칙에 따라 게시되며, API 자체에는 속도 제한이 적용되므로 책임 있게 사용해야 합니다.
...
- 사용자가 원본 레지스트리 레코드를 확인할 수 있도록 목록을 표시할 때 표준 ClawHub 스킬 URL로 다시 링크하세요.
- `https://clawhub.ai/<owner>/skills/<slug>` 형식의 표준 페이지 URL을 사용하세요.
- ClawHub이 서드파티 사이트를 보증, 검증 또는 운영한다고 암시하지 마세요.
- 공개 API 필터 또는 인증 경계를 우회하여 숨겨진 콘텐츠, 비공개 콘텐츠 또는 모더레이션으로 차단된 콘텐츠를 미러링하지 마세요.
...
- `X-RateLimit-Remaining` / `RateLimit-Remaining`: 있는 경우 정확한 남은 예산입니다. 샤딩된 성공 요청은 대략적인 전역 값을 반환하는 대신 이를 생략합니다.
- `Retry-After`: `429`에서 대기해야 할 지연 시간(초)
...
- `GET /api/v1/download?slug=&version=&tag=`
  ...
  - `clean` 또는 `suspicious` 스캔이 있는 현재 GitHub 기반 스킬은 ClawHub 바이트 대신 JSON `public-github` 핸드오프 설명자를 반환합니다.
- `GET /api/v1/skills/export?startDate=&endDate=&limit=&cursor=`
  - 호스팅된 스킬은 저장된 파일로 내보내집니다.
  - `clean` 또는 `suspicious` 스캔이 있는 현재 GitHub 기반 스킬은 `public-github` 핸드오프 설명자로 내보내집니다.
...
- `POST /api/v1/users/reserve`는 소유자 핸들을 위해 루트 슬러그와 비공개 미릴리스 패키지 자리표시자를 예약합니다.
