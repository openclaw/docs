...
ClawHub의 공개 읽기 API를 기반으로 타사 카탈로그, 디렉터리 또는 검색 표면을 구축할 수 있습니다. 공개 Skills 메타데이터와 Skills 파일은 ClawHub의 Skills 라이선스 규칙에 따라 게시되며, API 자체에는 속도 제한이 적용되므로 책임감 있게 사용해야 합니다.
...
- 목록을 표시할 때 사용자가 소스 레지스트리 레코드를 확인할 수 있도록 정식 ClawHub Skills URL로 다시 연결하세요.
...
- `GET /api/v1/download?slug=&version=&tag=`
  ...
  - 현재 GitHub 기반 Skills 중 `clean` 또는 `suspicious` 스캔이 있는 항목은 ClawHub 바이트 대신 JSON `public-github` 인계 설명자를 반환합니다.
- `GET /api/v1/skills/export?startDate=&endDate=&limit=&cursor=`
  - 호스팅된 Skills는 저장된 파일로 내보내집니다.
  - 현재 GitHub 기반 Skills 중 `clean` 또는 `suspicious` 스캔이 있는 항목은 `public-github` 인계 설명자로 내보내집니다.
