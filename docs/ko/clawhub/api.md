...
x-i18n:
    generated_at: "2026-07-03T09:27:16Z"
    model: gpt-5.5
...
ClawHub의 공개 읽기 API를 기반으로 타사 카탈로그, 디렉터리 또는 검색 화면을 만들 수 있습니다. 공개 Skill 메타데이터와 Skill 파일은 ClawHub의 Skill 라이선스 규칙에 따라 게시되며, API 자체에는 속도 제한이 적용되므로 책임 있게 사용해야 합니다.
...
- `X-RateLimit-Remaining` / `RateLimit-Remaining`: 제공되는 경우 정확한 남은 예산입니다. 샤딩된 성공 요청은 근사 전역 값을 반환하는 대신 이를 생략합니다.
...
- 제공되는 경우 `Retry-After`를 우선 사용하세요.
