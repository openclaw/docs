...
- `https://clawhub.ai/<owner>/<slug>` 형식의 표준 페이지 URL을 사용하세요.
...
헤더: `X-RateLimit-Limit`, `X-RateLimit-Remaining`, `X-RateLimit-Reset`, `RateLimit-Limit`, `RateLimit-Remaining`, `RateLimit-Reset`, `Retry-After` (`429` 시).
...
- `POST /api/v1/users/reserve`는 소유자 핸들을 위해 루트 slugs와 릴리스 없는 비공개 패키지 플레이스홀더를 예약합니다.
