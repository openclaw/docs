...
명령은 변경되지 않은 콘텐츠를 건너뜁니다. 새 Skills는 `1.0.0`에서 시작하며, 이후 변경 사항은 다음 패치 버전으로 자동 게시됩니다. 미리 보려면 `--dry-run`을 사용하고, 명시적 버전을 선택하려면 `--version`을 사용하세요.
...
Code Plugin은 `package.json`에 `openclaw.compat.pluginApi` 및 `openclaw.build.openclawVersion`을 포함한 OpenClaw 호환성 메타데이터를 포함해야 합니다.
