...
OpenClaw는 Skill이 어디에서 왔는지 기록하므로 이후 업데이트도 ClawHub를 통해 계속 확인할 수 있습니다.
...
게시하기 전에 `SKILL.md`의 메타데이터를 확인하세요. 사용자가 설치하기 전에 Skill에 필요한 것을 이해할 수 있도록 필요한 환경 변수, 도구, 권한을 선언하세요. [Skill 형식](/ko/clawhub/skill-format)을 참조하세요.
...
`sync`는 Skill 폴더를 스캔하고 아직 동기화되지 않은 새 Skill 또는 변경된 Skill을 게시합니다.
