...
명시적인 ClawHub 소스로 ClawHub에서 호스팅되는 플러그인 설치:
...
이 명령은 변경되지 않은 콘텐츠를 건너뜁니다. 새 스킬은 `1.0.0`에서 시작하며, 이후 변경 사항은 다음 패치 버전을 자동으로 게시합니다. 미리 보려면 `--dry-run`을 사용하고, 명시적 버전을 선택하려면 `--version`을 사용하세요.
