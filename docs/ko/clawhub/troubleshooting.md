...
- 브라우저에서 `http://127.0.0.1:<port>/callback`에 접근할 수 있는지 확인하십시오.
...
- `clawhub login`으로 다시 로그인하십시오.
- 사용자 지정 구성 경로를 사용하는 경우 `CLAWHUB_CONFIG_PATH`가 현재 토큰이
  포함된 파일을 가리키는지 확인하십시오.
- API 토큰을 사용하는 경우 웹 UI에서 해당 토큰이 취소되지 않았는지 확인하십시오.

## 검색 또는 설치가 `Rate limit exceeded` (429)를 반환함

응답의 재시도 정보를 확인하십시오.

- `Retry-After`: 재시도하기 전에 대기할 시간(초)입니다.
- `RateLimit-Limit`: 이 요청에 적용되는 한도입니다.
- `RateLimit-Remaining`: 헤더가 있는 경우 정확한 잔여 할당량입니다. `429`에서는 `0`입니다.
- `RateLimit-Reset` 또는 `X-RateLimit-Reset`: 재설정 시점입니다.

많은 사용자가 하나의 송신 IP를 공유하면 각 사용자가 요청을 몇 번만 보내더라도
익명 IP 한도에 도달할 수 있습니다. 가능하면 로그인하고 보고된 지연 시간이 지난 후
다시 시도하십시오.
...
- 정확한 슬러그나 소유자 페이지를 알고 있다면 확인하십시오.
- 릴리스가 공개 상태이며 검사 또는 검토로 보류되지 않았는지 확인하십시오.
- Skills의 소유자인 경우 로그인하여 검사하십시오.

```bash
clawhub inspect @openclaw/demo
```

소유자에게 표시되는 진단 정보에 검사, 업로드 게이트 또는 검토 상태가 설명되어 있을 수 있습니다.

## 필수 메타데이터 누락으로 게시가 실패함

Skills의 경우 `SKILL.md` frontmatter를 확인하십시오. 사용자와 스캐너가 패키지를
이해할 수 있도록 필수 환경 변수와 도구를 선언해야 합니다.

Plugin의 경우 `package.json` 호환성 메타데이터를 확인하십시오. 코드 Plugin을
게시하려면 `openclaw.compat.pluginApi` 및 `openclaw.build.openclawVersion`과 같은
...
게시될 내용을 확신할 수 없다면 먼저 미리 확인하십시오.
...
로컬 파일이 ClawHub에서 인식하는 어떤 버전과도 일치하지 않습니다. 다음 중 하나를 선택하십시오.
...
- OpenClaw 버전이 패키지에 명시된 호환성 범위를 충족하는지 확인하십시오.
- 패키지가 숨김, 보류 또는 차단 상태이면 소유자가 문제를 해결할 때까지 설치하지
  못할 수 있습니다.

## 공개 API 요청이 실패함

- `429` 재시도 헤더를 준수하고 공개 목록/검색 응답을 캐시하십시오.
- 사용자를 정식 ClawHub 목록으로 다시 연결하십시오.
- 숨김, 비공개, 보류 또는 검토로 차단된 콘텐츠를 공개 API 범위 외부에
  미러링하지 마십시오.
