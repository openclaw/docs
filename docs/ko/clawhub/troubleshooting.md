...
- `RateLimit-Remaining` 및 `RateLimit-Limit`: 현재 사용 가능 한도.
- `RateLimit-Reset` 또는 `X-RateLimit-Reset`: 초기화 시점.

여러 사용자가 하나의 송신 IP를 공유하면 각 사용자가 몇 개의 요청만 보내더라도 익명 IP 제한에 도달할 수 있습니다. 가능하면 로그인하고 보고된 지연 시간 후에 다시 시도하세요.
...
소유자에게 표시되는 진단 정보가 스캔, 업로드 게이트, 검토 상태를 설명할 수 있습니다.

## 필수 메타데이터가 없어 게시가 실패함

skills의 경우 `SKILL.md` frontmatter를 확인하세요. 사용자가 패키지를 이해하고 스캐너가 분석할 수 있도록 필수 환경 변수와 도구를 선언해야 합니다.

Plugin의 경우 `package.json` 호환성 메타데이터를 확인하세요. 코드 Plugin 게시는 `openclaw.compat.pluginApi` 및 `openclaw.build.openclawVersion` 같은 OpenClaw 호환성 필드가 필요합니다.
...
- 숨김, 비공개, 보류 또는 검토 차단 콘텐츠를 공개 API 표면 밖으로 미러링하지 마세요.
