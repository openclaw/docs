...
여러 사용자가 하나의 이그레스 IP를 공유하는 경우, 각 사용자가 몇 개의 요청만 보내더라도 익명 IP 제한에 도달할 수 있습니다. 가능하면 로그인하고 보고된 지연 시간 이후 다시 시도하세요.
...
Skills의 경우 `SKILL.md` frontmatter를 확인하세요. 사용자가 패키지를 이해하고 스캐너가 검사할 수 있도록 필요한 환경 변수와 도구를 선언해야 합니다.
...
- OpenClaw 버전이 패키지가 공지한 호환성 범위를 만족하는지 확인하세요.
...
- 숨김, 비공개, 보류 또는 검토로 차단된 콘텐츠를 공개 API 표면 외부에 미러링하지 마세요.
