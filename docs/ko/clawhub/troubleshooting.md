...
x-i18n:
    generated_at: "2026-06-28T05:32:10Z"
    model: gpt-5.5
...
CLI는 브라우저 로그인 중 짧게 동작하는 로컬 콜백 서버를 시작합니다.
...
- `RateLimit-Remaining`: 헤더가 있을 때의 정확한 남은 한도입니다. `429`에서는 `0`입니다.
- `RateLimit-Reset` 또는 `X-RateLimit-Reset`: 재설정 시점입니다.

여러 사용자가 하나의 송신 IP를 공유하는 경우, 각 사람이 몇 개의 요청만 보내더라도
익명 IP 제한에 걸릴 수 있습니다. 가능하면 로그인하고 보고된 지연 시간이 지난 뒤 다시
시도하세요.
...
- 릴리스가 공개 상태이고 스캔 또는 검토에 보류되어 있지 않은지 확인하세요.
...
스킬의 경우 `SKILL.md` frontmatter를 확인하세요. 사용자가 패키지를 이해하고 스캐너가
...
청구되었거나 예약되어 게시가 실패한 경우, 먼저 해당 네임스페이스와 일치하는 소유자로
...
[조직 및 네임스페이스 청구](/ko/clawhub/namespace-claims)를 참고하세요.
