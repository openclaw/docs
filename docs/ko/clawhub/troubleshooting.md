...
많은 사용자가 하나의 송신 IP를 공유하면 각 사용자가 몇 개의 요청만 보내더라도
...
- 해당 Skills의 소유자인 경우 로그인해서 검사하세요.
...
Skills의 경우 `SKILL.md` 프런트매터를 확인하세요. 필수 환경 변수와 도구는
사용자와 스캐너가 패키지를 이해할 수 있도록 선언되어야 합니다.
...
ClawHub는 GitHub 신원과 소스 속성을 사용해 패키지를 게시자와 연결합니다.
...
- 스캔 상태와 호환성 메타데이터는 패키지 세부 정보 페이지에서 확인하세요.
- 사용 중인 OpenClaw 버전이 패키지에서 표시한 호환성 범위를 충족하는지 확인하세요.
- 패키지가 숨김, 보류 또는 차단 상태인 경우 소유자가 문제를 해결할 때까지 설치하지 못할 수
  있습니다.
