...
- 사용자 지정 config 경로를 사용하는 경우 `CLAWHUB_CONFIG_PATH`가 현재 토큰이
...
익명 IP 제한에 도달할 수 있습니다. 가능한 경우 로그인하고 보고된 지연 시간 이후에
다시 시도하세요.
...
Skills의 경우 `SKILL.md` frontmatter를 확인하세요. 필수 환경 변수와 도구를
선언해야 사용자와 스캐너가 패키지를 이해할 수 있습니다.
...
- 패키지가 숨겨졌거나, 보류되었거나, 차단된 경우 소유자가 문제를 해결할 때까지
  설치하지 못할 수 있습니다.
...
- 숨겨진 콘텐츠, 비공개 콘텐츠, 보류된 콘텐츠 또는 검토에 의해 차단된 콘텐츠를
  공개 API 표면 외부에 미러링하지 마세요.
