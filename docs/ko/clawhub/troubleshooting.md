...
- 헤드리스 환경에서는 ClawHub 웹 UI에서 API 토큰을 만든 뒤 다음을 실행하세요.
...
- `RateLimit-Remaining`: 헤더가 있을 때 정확히 남아 있는 사용량입니다. `429`에서는 `0`입니다.
- `RateLimit-Reset` 또는 `X-RateLimit-Reset`: 재설정 시점입니다.

여러 사용자가 하나의 송신 IP를 공유하면 각 사용자가 몇 번만 요청하더라도
...
- 릴리스가 공개 상태이며 스캔이나 검토 때문에 보류되지 않았는지 확인하세요.
...
도구를 선언해 사용자와 스캐너가 패키지를 이해할 수 있게 해야 합니다.

Plugin의 경우 `package.json` 호환성 메타데이터를 확인하세요. code-plugin 게시는
...
ClawHub는 GitHub 신원과 소스 출처 표시를 사용해 패키지를 게시자와 연결합니다.
...
청구되었거나 예약되어 게시가 실패하는 경우 먼저 해당 네임스페이스와 일치하는
...
- 숨김, 비공개, 보류 또는 검토 차단된 콘텐츠를 공개 API 표면 밖으로
  미러링하지 마세요.
