...
x-i18n:
    generated_at: "2026-06-30T13:52:30Z"
    model: gpt-5.5
...
익명 IP 한도에 도달할 수 있습니다. 가능하면 로그인하고 보고된 지연 시간 후에
다시 시도하세요.
...
- 릴리스가 공개 상태이며 스캔 또는 조정에 보류되지 않았는지 확인하세요.
...
소유자에게 표시되는 진단 정보가 스캔, 업로드 게이트, 조정 상태를 설명할 수 있습니다.
...
플러그인의 경우 `package.json` 호환성 메타데이터를 확인하세요. 코드 플러그인 게시에는
...
ClawHub는 GitHub ID와 소스 출처 표시를 사용해 패키지를 게시자와
연결합니다.
...
소유자 핸들, 조직 네임스페이스, 패키지 스코프, 스킬 슬러그 또는 패키지 이름이 이미
청구되었거나 예약되어 게시가 실패한 경우, 먼저 해당 네임스페이스와 일치하는 소유자로
게시하고 있는지 확인하세요. 플러그인 패키지의 경우
`@example-org/example-plugin` 같은 스코프 이름은 일치하는 `example-org` 소유자로
게시해야 합니다.

조직, 프로젝트 또는 브랜드가 정당한 네임스페이스 소유자라고 생각하지만
