...
- `RateLimit-Remaining` 및 `RateLimit-Limit`: 현재 예산.
- `RateLimit-Reset` 또는 `X-RateLimit-Reset`: 재설정 시점.

여러 사용자가 하나의 송신 IP를 공유하는 경우 각 사용자가 몇 개의 요청만 보내더라도
익명 IP 제한에 도달할 수 있습니다. 가능하면 로그인하고 보고된 지연 시간 이후에 다시 시도하세요.
...
- 릴리스가 공개 상태이고 스캔 또는 검토에 보류되지 않았는지 확인하세요.
- 해당 skill의 소유자라면 로그인하고 검사하세요.
...
skills의 경우 `SKILL.md` frontmatter를 확인하세요. 사용자가 패키지를 이해하고 스캐너가 검사할 수 있도록
...
- 숨김, 비공개, 보류 또는 검토 차단된 콘텐츠를 공개 API 표면 외부에
  미러링하지 마세요.
