...
summary: ClawHub 로그인, 설치, 게시, 동기화, 업데이트 및 API 문제를 해결합니다.
x-i18n:
...
CLI는 브라우저 로그인 중 짧은 수명의 로컬 콜백 서버를 시작합니다.
...
여러 사용자가 하나의 송신 IP를 공유하는 경우 각 사람이 요청을 몇 개만 보내더라도
...
- 정확한 슬러그를 확인하거나, 알고 있다면 소유자 페이지를 확인하세요.
- 릴리스가 공개 상태이며 스캔 또는 조정으로 보류되지 않았는지 확인하세요.
- Skill의 소유자라면 로그인한 뒤 검사하세요.
...
plugins의 경우 `package.json` 호환성 메타데이터를 확인하세요. 코드 Plugin 게시에는
...
ClawHub는 GitHub ID와 소스 속성을 사용하여 패키지를 게시자와 연결합니다.
...
- 공개 API 표면 밖에서 숨김, 비공개, 보류 또는 조정으로 차단된 콘텐츠를 미러링하지 마세요.

엔드포인트 자세한 내용은 [HTTP API](/ko/clawhub/http-api)를 참조하세요.
