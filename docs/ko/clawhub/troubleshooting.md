...
x-i18n:
    generated_at: "2026-07-03T00:54:42Z"
    model: gpt-5.5
...
CLI는 브라우저 로그인 중 짧게 실행되는 로컬 콜백 서버를 시작합니다.
...
많은 사용자가 하나의 송신 IP를 공유하면 각 사용자가 몇 번만 요청하더라도
익명 IP 한도에 도달할 수 있습니다. 가능하면 로그인하고, 보고된 지연 시간 뒤에
다시 시도하세요.
...
- 알고 있다면 정확한 슬러그나 소유자 페이지를 확인하세요.
- 릴리스가 공개 상태이며 스캔 또는 검토로 보류되지 않았는지 확인하세요.
- Skills 소유자인 경우 로그인한 뒤 검사하세요.
...
청구되었거나 예약되어 게시가 실패한다면, 먼저 해당 네임스페이스와 일치하는 소유자로
...
- OpenClaw 버전이 패키지에 명시된 호환성 범위를 충족하는지 확인하세요.
...
- 숨김, 비공개, 보류 또는 검토 차단 상태의 콘텐츠를 공개 API 표면 밖으로
  미러링하지 마세요.
