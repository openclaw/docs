...
x-i18n:
    generated_at: "2026-07-02T22:26:29Z"
    model: gpt-5.5
...
CLI는 브라우저 로그인 중에 짧게 동작하는 로컬 콜백 서버를 시작합니다.
...
- 릴리스가 공개 상태이며 스캔 또는 모더레이션으로 보류되지 않았는지 확인하세요.
- 스킬의 소유자인 경우 로그인한 뒤 검사하세요.

```bash
clawhub inspect @openclaw/demo
```

소유자에게 표시되는 진단 정보가 스캔, 업로드 게이트 또는 모더레이션 상태를 설명할 수 있습니다.

## 필수 메타데이터가 누락되어 게시가 실패함

스킬의 경우 `SKILL.md` frontmatter를 확인하세요. 필수 환경 변수와
도구를 선언하여 사용자와 스캐너가 패키지를 이해할 수 있게 해야 합니다.

Plugin의 경우 `package.json` 호환성 메타데이터를 확인하세요. 코드 Plugin 게시는
...
ClawHub는 GitHub ID와 소스 출처를 사용하여 패키지와 게시자를 연결합니다.
...
사용 중이거나 예약되어 게시가 실패하는 경우, 먼저 해당 네임스페이스와 일치하는
...
증거 안내와 공개 이슈에 포함하지 말아야 할 내용은
[조직 및 네임스페이스 클레임](/clawhub/namespace-claims)을 참고하세요.

## `sync`에서 스킬을 찾지 못했다고 표시함
...
- 숨김, 비공개, 보류 또는 모더레이션으로 차단된 콘텐츠를 공개 API 표면 밖에
  미러링하지 마세요.
