...
x-i18n:
    generated_at: "2026-05-13T02:52:20Z"
    model: gpt-5.5
    provider: openai
...
- `Retry-After`: 다시 시도하기 전에 기다릴 초 단위 시간.
- `RateLimit-Remaining` 및 `RateLimit-Limit`: 현재 할당량.
- `RateLimit-Reset` 또는 `X-RateLimit-Reset`: 재설정 시점.

여러 사용자가 하나의 이그레스 IP를 공유하면 각 사람이 몇 번만 요청을 보내더라도 익명 IP 제한에 걸릴 수 있습니다. 가능하면 로그인하고 보고된 지연 시간이 지난 뒤 다시 시도하세요.
...
Skills의 경우 `SKILL.md` frontmatter를 확인하세요. 사용자가 패키지를 이해하고 스캐너가 검사할 수 있도록 필수 환경 변수와 도구를 선언해야 합니다.

Plugins의 경우 `package.json` 호환성 메타데이터를 확인하세요. Code-plugin 게시에는 `openclaw.compat.pluginApi` 및 `openclaw.build.openclawVersion` 같은 OpenClaw 호환성 필드가 필요합니다.
...
- 공개 API 표면 밖에서 숨김, 비공개, 보류 또는 검토 차단 콘텐츠를 미러링하지 마세요.
