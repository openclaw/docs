...
x-i18n:
    generated_at: "2026-07-12T15:04:13Z"
    model: gpt-5.6
...
- `RateLimit-Remaining`: 헤더가 있을 때 정확한 잔여 허용량입니다. `429`에서는 `0`입니다.
- `RateLimit-Reset` 또는 `X-RateLimit-Reset`: 한도 초기화 시점입니다.

여러 사용자가 하나의 송신 IP를 공유하면 각 사용자가 요청을 몇 번만 보내더라도
...
- 릴리스가 공개 상태이며 검사 또는 조정으로 보류되지 않았는지 확인하십시오.
- Skills를 소유하고 있다면 로그인하여 검사하십시오.

```bash
clawhub inspect @openclaw/demo
```

소유자에게 표시되는 진단 정보에서 검사, 업로드 게이트 또는 조정 상태를 설명할 수 있습니다.
...
ClawHub는 GitHub ID 및 소스 귀속 정보를 사용하여 패키지를 게시자와 연결합니다.

- 패키지를 소유하거나 게시할 수 있는 GitHub 계정으로 로그인했는지
  확인하십시오.
- 소스 URL이 공개되어 있거나 ClawHub에서 접근 가능한지 확인하십시오.
- GitHub 소스에는 `owner/repo`, `owner/repo@ref` 또는 전체 GitHub URL을 사용하십시오.

## 네임스페이스가 선점되었거나 예약되어 게시가 실패함

소유자 핸들, 조직 네임스페이스, 패키지 범위, Skills 슬러그 또는 패키지 이름이
이미 선점되었거나 예약되어 게시가 실패하면 먼저 네임스페이스와 일치하는 소유자로
게시하고 있는지 확인하십시오. Plugin 패키지의 경우
`@example-org/example-plugin`과 같은 범위 지정 이름은 일치하는
`example-org` 소유자로 게시해야 합니다.

조직, 프로젝트 또는 브랜드가 정당한 네임스페이스 소유자라고 판단하지만 현재
ClawHub 소유자를 관리할 수 없다면 공개된 민감하지 않은 증거와 함께
[조직/네임스페이스 소유권 주장 이슈](https://github.com/openclaw/clawhub/issues/new?template=org-namespace-claim.yml)를
여십시오. 증거 지침과 공개 이슈에 포함하지 말아야 할 내용은
...
- OpenClaw 버전이 패키지에 명시된 호환성 범위를 충족하는지
  확인하십시오.
...
- 숨김, 비공개, 보류 또는 조정으로 차단된 콘텐츠를 공개 API 영역 외부에
  미러링하지 마십시오.

엔드포인트에 관한 자세한 내용은 [HTTP API](/clawhub/http-api)를 참조하십시오.
