...
read_when:
    - ClawHub CLI 사용
    - 설치, 업데이트 또는 배포 디버깅
summary: 'CLI 참조: 명령, 플래그, 구성, 잠금 파일 동작.'
x-i18n:
    generated_at: "2026-07-03T09:27:13Z"
    model: gpt-5.5
...
이는 직접 아웃바운드 연결이 차단된 시스템에서 필요합니다(예: Docker 컨테이너, 프록시 전용 인터넷을 사용하는 Hetzner VPS, 기업 방화벽).
...
- 출력에는 스킬 slug, 소유자 핸들, 표시 이름, 관련도 점수가 포함됩니다.
...
- 스킬이 표시되어야 하는데 표시되지 않으면 메타데이터 이름을 변경하기 전에 로그인한 상태에서 `clawhub inspect @owner/slug`를 실행해 소유자에게 보이는 검토 진단을 확인하세요.
...
- 고정된 스킬을 덮어쓰지 않습니다. 먼저 `clawhub unpin <skill>`을 실행하세요.
- 작성 항목:
...
- 로그인된 상태에서는 현재 설치 수를 비활성화할 수 있도록 최선 노력 방식의 텔레메트리를 보냅니다.
- 대화형: 확인을 요청합니다.
...
- 선택적 이유를 포함해 `clawhub pin`으로 동결된 스킬 옆에 `pinned`를 표시합니다.
...
- 설치된 스킬에서 lockfile 고정을 제거해 이후 업데이트가 수정할 수 있게 합니다.
...
- `update <skill>`은 고정된 스킬에 대해 빠르게 실패하며 먼저 `clawhub unpin <skill>`을 실행하라고 안내합니다.
- `update --all`은 고정된 slug를 건너뛰고 동결 상태로 남은 항목의 요약을 출력합니다.

### `skill publish <path>`

- 로컬 번들 지문을 ClawHub와 비교하고 콘텐츠가 이미 게시된 경우 성공적으로 종료합니다.
- 새 스킬의 기본값은 `1.0.0`입니다. 변경된 스킬의 기본값은 다음 패치 버전입니다.
- `--version <version>`은 버전을 명시적으로 선택하며 콘텐츠가 기존 버전과 일치하더라도 게시합니다.
- `--dry-run`은 업로드 없이 게시를 해석합니다. `--json`은 기계 판독 가능 결과를 출력합니다.
- `--owner <handle>`은 행위자에게 게시자 접근 권한이 있을 때 org/user 게시자 핸들 아래에 게시합니다.
- `--migrate-owner`는 새 버전을 게시하면서 기존 스킬을 `--owner`로 이동합니다. 두 게시자 모두에 대한 admin/owner 접근 권한이 필요합니다.
- 소유자 및 검토 동작은 `docs/publishing.md`에 설명되어 있습니다.
- 스킬을 게시한다는 것은 ClawHub에서 `MIT-0`로 릴리스된다는 뜻입니다.
...
워크플로는 하나의 `skill_path`에 대해, 또는 `root`(기본값: `skills`) 아래의 각 직접 스킬 폴더에 대해 `skill publish`를 호출합니다. 변경되지 않은 스킬은 건너뛰며 동일한 자동 패치 버전 동작을 사용합니다.

토큰 없이 미리 보려면 `dry_run: true`를 설정합니다. 실제 게시는 `clawhub_token` 비밀이 필요합니다.

### `sync`

- 현재 workdir, 구성된 skills 디렉터리, 그리고 `--root <dir>` 폴더에서 `SKILL.md` 또는 `skill.md`를 포함하는 로컬 스킬 폴더를 스캔합니다.
...
- `clawhub login`이 필요합니다.
- ClawHub 보안 검사로 차단되거나 숨겨진 버전을 포함해 제출된 스킬 또는 Plugin 버전에 대해 저장된 스캔 보고서 ZIP을 다운로드합니다.
- 스킬 다운로드는 스킬 slug를 사용하며 기본값은 `--kind skill`입니다.
...
ClawHub는 스킬 저장소와 카탈로그 저장소를 위한 공식 재사용 가능 워크플로를
...
- `root`는 카탈로그 저장소의 경우 기본값이 `skills`입니다.
...
- `--version` 없이는 스킬을 소프트 삭제합니다(소유자, 중재자 또는 관리자).
- `DELETE /api/v1/skills/{slug}`를 호출합니다.
- 소유자가 시작한 소프트 삭제는 30일 동안 slug를 예약하며, 명령은 만료 시간을 출력합니다.
- `--version <version>`은 실패 시 차단되는 버전별 경로를 통해 소유한 최신이 아닌 버전 하나를 영구 삭제합니다.
...
- 소유한 스킬의 이름을 바꾸고 이전 slug를 리디렉션 별칭으로 유지합니다.
...
- 원본 slug는 공개 목록에 표시되지 않고 대상에 대한 리디렉션 별칭이 됩니다.
...
- 조직/게시자 핸들로 이전하는 경우, 행위자가 현재 소유자와 대상 게시자 모두에 대한 관리자 액세스 권한을 가진 경우에만 즉시 적용됩니다.
- 하위 명령:
...
- Plugin과 기타 패키지 계열 항목에는 이것을 사용하세요. 최상위 `search`는 계속 스킬 검색 표면으로 남습니다.
- 플래그:
...
- `--package`를 사용하면 ClawHub에서 예상 메타데이터를 확인하고 로컬 파일을 게시된 아티팩트 메타데이터와 비교합니다.
...
- 중대한 호환성 오류는 0이 아닌 코드로 종료합니다. 경고만 있는 발견 사항은 출력되지만 0으로 종료합니다.
- 플래그:
  ...
  - `--allow-execute`: 격리된 작업 영역에서 런타임 캡처를 허용합니다.
...
검증에서 패키지, 매니페스트, SDK 가져오기 또는 아티팩트 발견 사항을 보고하면 [Plugin 검증 수정](/clawhub/plugin-validation-fixes)을 참조한 다음 명령을 다시 실행하세요.

### `package delete <name>`

- `--version` 없이는 패키지와 모든 릴리스를 소프트 삭제합니다.
- `--version <version>`은 실패 시 차단되는 버전별 경로를 통해 소유한 최신이 아닌 릴리스 하나를 영구 삭제합니다.
  삭제된 버전은 복원하거나 다시 게시할 수 없습니다. 현재 최신 버전을 삭제하기 전에 대체 버전을 게시하세요. 이 버전 전용 흐름에는 패키지 소유자 또는 조직 게시자 관리자가 필요하며, 플랫폼 스태프는 패키지 소유권을 우회하지 않습니다.
...
- 패키지를 다른 게시자로 이전합니다.
- 플랫폼 관리자가 수행하는 경우를 제외하고, 현재 패키지 소유자와 대상 게시자 모두에 대한 관리자 액세스 권한이 필요합니다.
...
- 플래그:
  - `--version <version>`: 신고에 연결할 선택적 패키지 버전.
...
- 공식 상태, ClawPack 사용 가능 여부, 아티팩트 다이제스트, 소스 출처, OpenClaw 호환성, 호스트 대상, 환경 메타데이터, 스캔 상태에 대한 차단 요소를 보고합니다.
- 플래그:
  - `--json`: 기계가 읽을 수 있는 출력입니다.
...
- 번들된 OpenClaw Plugin을 대체할 수 있는 패키지의 운영자 중심 마이그레이션 상태를 표시합니다.
...
- 인증된 사용자가 소유한 조직 게시자를 생성합니다.
- 핸들은 소문자로 정규화되며 `@`를 포함하거나 제외하고 전달할 수 있습니다.
...
- `POST /api/v1/packages`를 통해 code Plugin 또는 bundle Plugin을 게시합니다.
- `<source>`는 다음을 허용합니다:
  - 로컬 폴더 경로: `./my-plugin`
...
- 메타데이터는 `package.json`, `openclaw.plugin.json`, 그리고
  ...
  실제 OpenClaw bundle 마커에서 자동으로 감지됩니다.
- `.tgz` 소스는 ClawPack으로 처리됩니다. CLI는 정확한 npm-pack
  바이트를 업로드하고, 추출된 `package/` 콘텐츠는 검증 및
  메타데이터 사전 채우기에만 사용합니다.
- code Plugin 폴더는 업로드 전에 ClawPack npm tarball로 패키징되므로
  OpenClaw 설치 환경에서 정확한 아티팩트를 검증할 수 있습니다. bundle Plugin 폴더는 계속
  추출 파일 게시 경로를 사용합니다.
- GitHub 소스의 경우 소스 귀속 정보는 저장소, 확인된 커밋, ref, 하위 경로에서 자동으로 채워집니다.
- 로컬 폴더의 경우 origin remote가 GitHub를 가리키면 로컬 git에서 소스 귀속 정보가 자동으로 감지됩니다.
- 외부 code Plugin은 `openclaw.compat.pluginApi`와
...
- scoped 패키지 이름은 선택한 소유자와 일치해야 합니다. `docs/publishing.md`를 참고하세요.
- 기존 플래그(`--family`, `--name`, `--version`, `--source-repo`, `--source-commit`, `--source-ref`, `--source-path`)도 계속 override로 작동합니다.
...
code Plugin의 경우 폴더 게시가 패키지 폴더에서 ClawPack 아티팩트를 빌드하고 업로드합니다:
...
외부 code Plugin은 `package.json`에 소량의 OpenClaw 메타데이터가 필요합니다.
이 최소 manifest만으로도 게시에 성공할 수 있습니다:
...
- `openclaw.hostTargets`와 `openclaw.environment`는 선택적 메타데이터입니다.
  ClawHub는 값이 있으면 이를 표시할 수 있지만, 게시에 필수는 아닙니다.
- 더 자세한 호환성 메타데이터를 게시하려면 `openclaw.compat.minGatewayVersion`과
  `openclaw.build.pluginSdkVersion`을 선택적으로 추가할 수 있습니다.
- 오래된 `clawhub` CLI 릴리스를 사용 중이라면 업로드 전에
  로컬 preflight 검사가 실행되도록 게시 전에 업그레이드하세요.
- 검증에서 remediation code가 보고되면
  [Plugin 검증 수정 사항](/clawhub/plugin-validation-fixes)을 참고하세요.

#### GitHub Actions

ClawHub는 Plugin 저장소용 공식 reusable workflow도
[`/.github/workflows/package-publish.yml`](https://github.com/openclaw/clawhub/blob/a95f470a588ea9fe4c4b4c258c8c4ca5f02c2836/.github/workflows/package-publish.yml)에 제공합니다.
...
- reusable workflow는 기본적으로 `source`를 호출자 저장소로 설정합니다.
- monorepo의 경우 workflow가 Plugin 패키지 폴더를 게시하도록 `source_path`를 전달하세요.
  예: `source_path: extensions/codex`.
- reusable workflow는 안정적인 태그 또는 전체 커밋 SHA에 고정하세요. `@main`에서 릴리스 게시를 실행하지 마세요.
- `pull_request`는 CI가 오염되지 않도록 `dry_run: true`를 사용해야 합니다.
- 실제 게시는 `workflow_dispatch` 또는 태그 push 같은 신뢰할 수 있는 이벤트로 제한해야 합니다.
- secret 없는 trusted publishing은 `workflow_dispatch`에서만 작동합니다. 태그 push에는 여전히 `clawhub_token`이 필요합니다.
- 첫 게시, 신뢰할 수 없는 패키지, 또는 긴급 게시를 위해 `clawhub_token`을 계속 사용할 수 있게 유지하세요.
...
- 구성을 설정한 뒤 저장소, workflow 파일 이름,
  선택적 environment 고정을 확인할 때 사용하세요.
- 플래그:
  - `--json`: 기계 판독 가능 출력.
...
- 패키지는 일반 수동 게시 또는 토큰 인증
  `clawhub package publish`를 통해 먼저 생성되어야 합니다.
...
- `--workflow-filename <file>`은 `.github/workflows/` 안의 workflow 파일 이름과 일치해야 합니다.
- `--environment <name>`은 선택 사항입니다. 구성된 경우 OIDC claim의 GitHub Actions
  environment가 정확히 일치해야 합니다.
...
- 플래그:
  ...
  - `--environment <name>`: 선택적 exact-match GitHub Actions environment.
  - `--json`: 기계 판독 가능 출력.
...
- workflow, 저장소, 또는 environment 고정을 비활성화하거나 다시 생성해야 할 때 rollback으로 사용하세요.
...
- 보고는 best-effort 방식입니다. 텔레메트리를 사용할 수 없어도 install 명령은 실패하지 않습니다.
