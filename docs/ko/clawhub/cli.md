...
- `--workdir <dir>`: 작업 디렉터리(기본값: cwd; 구성된 경우 Clawdbot 작업 영역으로 폴백)
...
CLI는 기업 프록시 또는 제한된 네트워크 뒤에 있는 시스템을 위해 표준 HTTP 프록시 환경 변수를 준수합니다.

- `HTTPS_PROXY` / `https_proxy`
- `HTTP_PROXY` / `http_proxy`
- `NO_PROXY` / `no_proxy`

이 변수 중 하나라도 설정되어 있으면 CLI는 지정된 프록시를 통해 아웃바운드 요청을 라우팅합니다. `HTTPS_PROXY`는 HTTPS 요청에 사용되고, `HTTP_PROXY`는 일반 HTTP에 사용됩니다. `NO_PROXY` / `no_proxy`는 특정 호스트 또는 도메인에 대해 프록시를 우회하도록 준수됩니다.
...
- 스킬이 표시되어야 하는데 표시되지 않으면, 메타데이터 이름을 바꾸기 전에 로그인한 상태에서 `clawhub inspect @owner/slug`를 실행하여 소유자에게 보이는 검토 진단을 확인하세요.
...
- 현재 설치 수가 비활성화될 수 있도록 로그인된 상태에서 최선 노력 방식으로 텔레메트리를 보냅니다.
- 대화형: 확인을 요청합니다.
...
- 고정된 스킬은 `install --force`도 거부하여 로컬 바이트가 실수로 교체되지 않게 합니다.
...
- `update <skill>`은 고정된 스킬에 대해 빠르게 실패하고 먼저 `clawhub unpin <skill>`을 실행하라고 알려줍니다.
...
- 현재 workdir, 구성된 스킬 디렉터리, 그리고 `SKILL.md` 또는 `skill.md`가 포함된 로컬 스킬 폴더를 찾기 위한 모든 `--root <dir>` 폴더를 스캔합니다.
...
- 게시된 스캔에는 소유권 또는 게시자 관리 권한이 필요합니다. 검토자/관리자는 `clawhub-admin`을 통해 동일한 백엔드를 사용할 수 있습니다.
...
- `clawhub login`이 필요합니다.
- ClawHub 보안 검사에 의해 차단되거나 숨겨진 버전을 포함하여, 제출된 스킬 또는 Plugin 버전에 대해 저장된 스캔 보고서 ZIP을 다운로드합니다.
- 스킬 다운로드는 스킬 슬러그를 사용하며 기본값은 `--kind skill`입니다.
- Plugin 다운로드는 패키지 이름을 사용하며 `--kind plugin`이 필요합니다.
- 작성자가 ClawHub에 의해 차단된 정확한 제출 버전을 검사할 수 있도록 `--version`이 필요합니다.
...
ClawHub는 스킬 저장소 및 카탈로그 저장소를 위한 공식 재사용 가능 워크플로를
...
- V1 스킬 게시는 `clawhub_token`을 사용합니다. GitHub OIDC 신뢰할 수 있는 게시는 현재 패키지 전용입니다.

### `delete <skill>`

- `--version` 없이는 skill을 소프트 삭제합니다(소유자, 모더레이터 또는 관리자).
...
- 소스 slug는 공개 목록 표시가 중지되고 대상으로 향하는 리디렉션 별칭이 됩니다.
...
- 조직/게시자 핸들로 이전하는 경우, 행위자가 현재 소유자와 대상 게시자 양쪽 모두에 대한
  관리자 액세스 권한을 가진 경우에만 즉시 적용됩니다.
- 하위 명령:
...
- Plugin 및 기타 패키지 패밀리 항목에는 이것을 사용하세요. 최상위 `search`는 skill 검색 표면으로 유지됩니다.
- 플래그:
...
- 로컬 아티팩트에 대해 ClawHub SHA-256, npm `sha512` 무결성 및 npm shasum을
  계산합니다.
...
- 로컬 Plugin 패키지 폴더에 대해 ClawHub CLI에 번들된 Plugin Inspector를
  실행합니다.
- 기본값은 로컬 OpenClaw 체크아웃을 찾거나 가져오지 않는 오프라인/정적 검증입니다.
- 하드 호환성 오류는 0이 아닌 코드로 종료합니다. 경고만 있는 발견 사항은 출력되지만
  0으로 종료합니다.
- 플래그:
  - `--out <dir>`: Plugin Inspector 보고서를 이 디렉터리에 작성합니다.
...
검증에서 패키지, 매니페스트, SDK import 또는 아티팩트 발견 사항을 보고하면
[Plugin 검증 수정](/clawhub/plugin-validation-fixes)을 참조한 다음 명령을 다시 실행하세요.
...
- 패키지를 다른 게시자로 이전합니다.
- 플랫폼 관리자가 수행하는 경우가 아니라면 현재 패키지 소유자와 대상
  게시자 양쪽 모두에 대한 관리자 액세스 권한이 필요합니다.
- 범위가 지정된 패키지 이름은 일치하는 범위 소유자로 이전되어야 합니다.
...
- 신고는 패키지 수준이며, 선택적으로 버전에 연결될 수 있고, 검토를 위해
  모더레이터에게 표시됩니다.
- 신고 자체만으로는 패키지가 자동으로 숨겨지거나 다운로드가 차단되지 않습니다.
- 플래그:
...
- 공식 상태, ClawPack 가용성, 아티팩트 digest,
  소스 출처, OpenClaw 호환성, 호스트 대상, 환경 메타데이터
  및 스캔 상태에 대한 차단 요소를 보고합니다.
- 플래그:
  - `--json`: 기계가 읽을 수 있는 출력입니다.
...
- 번들된 OpenClaw Plugin을 대체할 수 있는 패키지에 대해 운영자 중심 마이그레이션 상태를
  표시합니다.
- `package readiness`와 동일한 계산된 readiness 엔드포인트를 호출하지만,
...
- 외부 코드 Plugin은 `openclaw.compat.pluginApi`와
  ...
  최상위 `package.json.version`은 게시 유효성 검사의 대체값으로 사용되지 않습니다.
- `--dry-run`은 업로드하지 않고 확인된 게시 페이로드를 미리 보여줍니다.
- `--json`은 CI용 기계 판독 가능 출력을 내보냅니다.
- `--owner <handle>`은 행위자에게 게시자 접근 권한이 있을 때 사용자 또는 조직 게시자 핸들로 게시합니다.
...
코드 Plugin의 경우, 폴더 게시가 패키지 폴더에서 ClawPack 아티팩트를 빌드하고 업로드합니다.
...
- `openclaw.hostTargets`와 `openclaw.environment`는 선택적 메타데이터입니다.
  ClawHub는 해당 값이 있을 때 이를 표시할 수 있지만, 게시에 필수는 아닙니다.
- 더 자세한 호환성 메타데이터를 게시하려면 `openclaw.compat.minGatewayVersion`과
  `openclaw.build.pluginSdkVersion`을 선택적으로 추가할 수 있습니다.
- 이전 `clawhub` CLI 릴리스를 사용 중이라면, 업로드 전에 로컬 사전 점검이 실행되도록
  게시 전에 업그레이드하세요.
- 유효성 검사에서 개선 코드가 보고되면
  [Plugin 유효성 검사 수정](/clawhub/plugin-validation-fixes)을 참조하세요.

#### GitHub Actions

ClawHub는 Plugin 저장소를 위한 공식 재사용 가능 워크플로도
[`/.github/workflows/package-publish.yml`](https://github.com/openclaw/clawhub/blob/2ef5aebc5d2f78630d6fc8fedb7d4e829cf83532/.github/workflows/package-publish.yml)에 제공합니다.
...
- 재사용 가능 워크플로는 `source`를 기본적으로 호출자 저장소로 설정합니다.
- 모노레포의 경우 워크플로가 Plugin 패키지 폴더를 게시하도록 `source_path`를 전달하세요.
  예: `source_path: extensions/codex`.
- 재사용 가능 워크플로는 안정적인 태그 또는 전체 커밋 SHA에 고정하세요. `@main`에서 릴리스 게시를 실행하지 마세요.
- `pull_request`는 CI에 영향을 남기지 않도록 `dry_run: true`를 사용해야 합니다.
...
- 패키지는 먼저 일반 수동 게시 또는 토큰 인증
  `clawhub package publish`를 통해 생성되어야 합니다.
- 구성이 설정되면, 이후 지원되는 GitHub Actions 게시에서
...
- `--environment <name>`은 선택 사항입니다. 구성하면 OIDC 클레임의 GitHub Actions
  환경이 정확히 일치해야 합니다.
- ClawHub는 이 명령이 실행될 때 구성된 GitHub 저장소를 검증합니다.
  공개 저장소는 공개 GitHub 메타데이터를 통해 검증할 수 있습니다. 비공개
  저장소의 경우, 예를 들어 향후 ClawHub GitHub App 설치 또는 다른 승인된
...
- 워크플로, 저장소 또는 환경 고정을 비활성화하거나 다시 생성해야 할 때 롤백으로 사용하세요.
...
- 로그인 상태에서 `clawhub install <slug>` 이후 전송됩니다. 단,
