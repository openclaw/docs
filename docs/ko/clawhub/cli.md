...
summary: 'CLI 참조: 명령, 플래그, 설정, 락파일, 동기화 동작.'
x-i18n:
...
CLI는 기업 프록시나 제한된 네트워크 뒤에 있는 시스템을 위해 표준 HTTP 프록시 환경 변수를 준수합니다.

- `HTTPS_PROXY` / `https_proxy`
- `HTTP_PROXY` / `http_proxy`
- `NO_PROXY` / `no_proxy`

이러한 변수 중 하나라도 설정되어 있으면 CLI는 지정된 프록시를 통해 아웃바운드 요청을 라우팅합니다. `HTTPS_PROXY`는 HTTPS 요청에 사용되고, `HTTP_PROXY`는 일반 HTTP에 사용됩니다. `NO_PROXY` / `no_proxy`는 특정 호스트나 도메인에 대해 프록시를 우회하는 데 적용됩니다.
...
- skill이 표시되어야 하는데 표시되지 않으면, 메타데이터 이름을 바꾸기 전에 로그인한 상태에서 `clawhub inspect <slug>`를 실행하여 소유자에게 표시되는 중재 진단을 확인하세요.

### `explore`

- `/api/v1/skills?limit=...&sort=createdAt`를 통해 최신 skill을 나열합니다(`createdAt` 내림차순 정렬).
- 플래그:
...
- 고정된 skill은 덮어쓰기를 거부합니다. 먼저 `clawhub unpin <slug>`를 실행하세요.
- 다음을 씁니다.
...
- `clawhub pin`으로 동결된 Skills 옆에 선택적 이유를 포함해 `pinned`를 표시합니다.

### `pin <slug>`

- 설치된 skill을 lockfile에서 pinned 상태로 표시합니다.
- `--reason <text>`는 skill이 동결된 이유를 기록합니다.
- Pinned Skills는 `update --all`에서 건너뛰며, 직접 `update <slug>`를 실행하면 거부됩니다.
- Pinned Skills는 로컬 바이트가 실수로 교체되지 않도록 `install --force`도 거부합니다.

### `unpin <slug>`

- 설치된 skill에서 lockfile pin을 제거하여 이후 업데이트가 수정할 수 있게 합니다.
...
- Pinned Skills는 `--force`로도 업데이트되지 않습니다.
- `update <slug>`는 pinned slug에 대해 빠르게 실패하며 먼저 `clawhub unpin <slug>`를 실행하라고 안내합니다.
- `update --all`은 pinned slug를 건너뛰고 동결된 상태로 남은 항목의 요약을 출력합니다.
...
- skill을 게시하면 ClawHub에서 `MIT-0`로 릴리스됩니다.
- 게시된 Skills는 출처 표시 없이 자유롭게 사용, 수정, 재배포할 수 있습니다.
- ClawHub는 유료 Skills나 skill별 가격 책정을 지원하지 않습니다.
- `--clawscan-note <text>`는 ClawScan 메모를 추가합니다. 이 메모는 네트워크 접근, native host 접근, provider별 자격 증명처럼 달리 보면 비정상적으로 보일 수 있는 동작에 대한 맥락을 ClawScan에 제공합니다. 이 메모는 게시된 버전에 저장됩니다.
- 레거시 alias: `publish <path>`.
...
- source slug는 공개 목록 표시가 중단되고 target으로 향하는 redirect alias가 됩니다.
- `POST /api/v1/skills/{sourceSlug}/merge`를 호출합니다.
- `--yes`는 확인을 건너뜁니다.

### `transfer`

- 소유권 transfer 워크플로입니다.
- 사용자 handle로의 transfer는 수신자가 수락하는 pending request를 생성합니다.
- org/publisher handle로의 transfer는 행위자가 현재 소유자와 대상 publisher 모두에 대한 admin 접근 권한을 가진 경우에만 즉시 적용됩니다.
- 하위 명령:
...
- Plugin 및 다른 package-family 항목에는 이것을 사용합니다. 최상위 `search`는 skill 검색 surface로 남습니다.
- 플래그:
...
- Plugin metadata, 호환성, verification, source 및 version/file inspection에 사용합니다.
- `--version <version>`: 특정 버전을 검사합니다(기본값: 최신).
- `--tag <tag>`: 태그가 지정된 버전을 검사합니다(예: `latest`).
- `--versions`: 버전 기록을 나열합니다(첫 페이지).
- `--limit <n>`: 나열할 최대 버전 수(1-100).
- `--files`: 선택된 버전의 파일을 나열합니다.
...
- `--package`를 사용하면 ClawHub에서 예상 metadata를 resolve하고 로컬 파일을 게시된 artifact metadata와 비교합니다.
- 직접 digest 플래그를 사용하면 네트워크 조회 없이 검증합니다.
- 플래그:
  - `--package <name>`: 예상 artifact metadata를 resolve할 package 이름입니다.
...
- 패키지 moderation 표시 상태를 확인하는 소유자 명령입니다.
- `GET /api/v1/packages/{name}/moderation`을 호출합니다.
- 현재 패키지 스캔 상태, 열린 신고 수, 최신 릴리스의 수동
...
- 번들된 OpenClaw Plugin을 대체할 수 있는 패키지에 대해 운영자 지향
  마이그레이션 상태를 표시합니다.
...
- `<source>`는 다음을 허용합니다.
  ...
  - GitHub 리포지토리: `owner/repo` 또는 `owner/repo@ref`
  - GitHub URL: `https://github.com/owner/repo`
- 메타데이터는 `package.json`, `openclaw.plugin.json`, 그리고
  `.codex-plugin/plugin.json`, `.claude-plugin/plugin.json`,
  `.cursor-plugin/plugin.json` 같은 실제 OpenClaw 번들 마커에서 자동 감지됩니다.
- `.tgz` 소스는 ClawPack으로 처리됩니다. CLI는 정확한 npm-pack
  바이트를 업로드하고, 추출된 `package/` 콘텐츠는 검증과 메타데이터
  사전 채우기에만 사용합니다.
- 코드 Plugin 폴더는 업로드 전에 ClawPack npm tarball로 패키징되어
  OpenClaw 설치가 정확한 아티팩트를 검증할 수 있게 합니다. 번들 Plugin
  폴더는 계속 추출된 파일 게시 경로를 사용합니다.
- GitHub 소스의 경우, 소스 속성은 리포지토리, 확인된 커밋, ref, 하위 경로에서 자동으로 채워집니다.
- 로컬 폴더의 경우, origin 원격이 GitHub를 가리키면 로컬 git에서 소스 속성이 자동 감지됩니다.
...
- `--clawscan-note <text>`는 ClawScan 참고 사항을 추가합니다. 이 참고 사항은 네트워크 액세스,
  네이티브 호스트 액세스, 공급자별 자격 증명처럼 일반적이지 않아 보일 수 있는 동작에 대해
  ClawScan에 맥락을 제공합니다. 이 참고 사항은 게시된 릴리스에 저장됩니다.
- 범위가 지정된 패키지 이름은 선택한 소유자와 일치해야 합니다. `docs/publishing.md`를 참조하세요.
- 기존 플래그(`--family`, `--name`, `--version`, `--source-repo`, `--source-commit`, `--source-ref`, `--source-path`)는 계속 오버라이드로 작동합니다.
- 비공개 GitHub 리포지토리에는 `GITHUB_TOKEN`이 필요합니다.
...
라이브 릴리스를 만들기 전에 먼저 `--dry-run`을 사용하여 확인된 패키지 메타데이터와
소스 속성을 확인하세요.
...
- `package.json.version`은 패키지 릴리스 버전이지만, OpenClaw 호환성/빌드 검증의
  대체값으로 사용되지 않습니다.
- `openclaw.hostTargets`와 `openclaw.environment`는 선택 사항인 메타데이터입니다.
  ClawHub는 존재할 때 이를 표시할 수 있지만, 게시에 필수는 아닙니다.
- 더 자세한 호환성 메타데이터를 게시하려는 경우 `openclaw.compat.minGatewayVersion`과
  `openclaw.build.pluginSdkVersion`은 선택 사항인 추가 항목입니다.
- 이전 `clawhub` CLI 릴리스를 사용 중이라면, 로컬 preflight 검사가 업로드 전에 실행되도록
  게시 전에 업그레이드하세요.

#### GitHub Actions

ClawHub는 Plugin 리포지토리용 공식 재사용 가능 workflow도
[`/.github/workflows/package-publish.yml`](https://github.com/openclaw/clawhub/blob/53b64d1d911106dab570eb6260e6ee977e9eefcd/.github/workflows/package-publish.yml)에 제공합니다.
...
- 재사용 가능 workflow는 `source`를 기본적으로 호출자 리포지토리로 설정합니다.
- 모노리포의 경우 workflow가 Plugin 패키지 폴더를 게시하도록 `source_path`를 전달하세요.
...
- 로그인 상태에서는 `sync` 중 전송됩니다. 단, `CLAWHUB_DISABLE_TELEMETRY=1`(레거시 `CLAWDHUB_DISABLE_TELEMETRY=1`)인 경우는 제외됩니다.
