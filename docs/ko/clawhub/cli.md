...
CLI는 기업 프록시 또는 제한된 네트워크 뒤에 있는 시스템을 위해
...
아웃바운드 요청을 라우팅합니다. `HTTPS_PROXY`는 HTTPS 요청에 사용되고, `HTTP_PROXY`는
일반 HTTP에 사용됩니다. `NO_PROXY` / `no_proxy`는 특정 호스트 또는 도메인에 대해
프록시를 우회하도록 준수됩니다.
...
- 검색은 다운로드 인기도보다 정확한 슬러그/이름 토큰 일치를 우선합니다. `map` 같은 독립 슬러그 토큰은 `amap` 내부의 부분 문자열보다 `personal-map`에 더 강하게 일치합니다.
- 다운로드 수는 작은 인기도 사전값일 뿐, 상위 배치를 보장하지 않습니다.
- 스킬이 표시되어야 하지만 표시되지 않는 경우, 메타데이터 이름을 바꾸기 전에 로그인한 상태에서 `clawhub inspect <slug>`를 실행해 소유자에게 표시되는 모더레이션 진단을 확인합니다.
...
- 고정된 스킬을 덮어쓰지 않습니다. 먼저 `clawhub unpin <slug>`를 실행합니다.
- 다음을 씁니다:
...
- `clawhub pin`으로 고정된 스킬 옆에 `pinned`를 표시하며, 선택적 이유도 포함합니다.

### `pin <slug>`

- 설치된 스킬을 잠금 파일에서 고정된 것으로 표시합니다.
- `--reason <text>`는 스킬이 고정된 이유를 기록합니다.
- 고정된 스킬은 `update --all`에서 건너뛰며 직접 `update <slug>`로 업데이트하려 하면 거부됩니다.
- 고정된 스킬은 `install --force`도 거부하므로 로컬 바이트가 실수로 교체되지 않습니다.

### `unpin <slug>`

- 설치된 스킬에서 잠금 파일 고정을 제거하여 향후 업데이트가 수정할 수 있게 합니다.
...
- `update <slug>`는 고정된 슬러그에 대해 빠르게 실패하며 먼저 `clawhub unpin <slug>`를 실행하라고 알려줍니다.
- `update --all`은 고정된 슬러그를 건너뛰고 고정 상태로 남은 항목의 요약을 출력합니다.

### `skill publish <path>`

- `POST /api/v1/skills`(멀티파트)를 통해 게시합니다.
- semver가 필요합니다: `--version 1.2.3`.
- `--owner <handle>`은 행위자에게 게시자 액세스 권한이 있을 때 조직/사용자 게시자 핸들 아래에 게시합니다.
- `--migrate-owner`는 새 버전을 게시하는 동안 기존 스킬을 `--owner`로 이동합니다. 두 게시자 모두에 대한 관리자/소유자 액세스 권한이 필요합니다.
...
- `--clawscan-note <text>`는 ClawScan 메모를 추가합니다. 이 메모는 네트워크 액세스, 네이티브 호스트 액세스, 공급자별 자격 증명처럼 그렇지 않으면 이례적으로 보일 수 있는 동작에 대한 컨텍스트를 ClawScan에 제공합니다. 이 메모는 게시된 버전에 저장됩니다.
- 레거시 별칭: `publish <path>`.
...
- 소스 슬러그는 공개 목록 표시를 중지하고 대상에 대한 리디렉션 별칭이 됩니다.
...
- 소유권 이전 워크플로.
- 사용자 핸들로 이전하면 수신자가 수락할 보류 중인 요청이 생성됩니다.
- 조직/게시자 핸들로 이전하는 경우 행위자가 현재 소유자와 대상 게시자 모두에 대한 관리자 액세스 권한을 가질 때만 즉시 적용됩니다.
- 하위 명령:
...
- Plugin 및 기타 패키지 계열 항목에는 이것을 사용하세요. 최상위 `search`는 계속 스킬 검색 표면입니다.
- 플래그:
...
- ClawPack npm-pack 아티팩트의 경우 npm `sha512` 무결성, npm shasum, 타볼의 `package.json` 이름/버전도 검증합니다.
...
- 패키지 소유자, 조직 퍼블리셔 소유자/관리자, 플랫폼 모더레이터,
  또는 플랫폼 관리자가 필요합니다.
- 플래그:
  - `--yes`: 확인을 건너뜁니다.
...
- 코드 Plugin 폴더는 업로드 전에 ClawPack npm tarball로 패킹되어
  ...
  추출된 파일 게시 경로를 사용합니다.
- GitHub 소스의 경우 소스 귀속 정보는 리포지토리, 확인된 커밋, ref, 하위 경로에서 자동으로 채워집니다.
- 로컬 폴더의 경우 origin 원격이 GitHub를 가리키면 소스 귀속 정보가 로컬 git에서 자동 감지됩니다.
...
- `--owner <handle>`은 행위자에게 퍼블리셔 접근 권한이 있을 때 사용자 또는 조직 퍼블리셔 핸들 아래에 게시합니다.
- `--clawscan-note <text>`는 ClawScan 메모를 추가합니다. 이 메모는 네트워크 접근,
  네이티브 호스트 접근, 또는 제공자별 자격 증명처럼 그렇지 않으면 비정상적으로 보일 수 있는 동작에 대한
...
- `openclaw.hostTargets`와 `openclaw.environment`는 선택적 메타데이터입니다.
  ClawHub가 존재할 때 이를 표시할 수 있지만, 게시에 필수는 아닙니다.
- 더 자세한 호환성 메타데이터를 게시하려면 `openclaw.compat.minGatewayVersion`과
  `openclaw.build.pluginSdkVersion`을 선택적 추가 항목으로 사용할 수 있습니다.
- 오래된 `clawhub` CLI 릴리스를 사용 중이라면, 로컬 사전 검사가 업로드 전에 실행되도록
  게시하기 전에 업그레이드하세요.

#### GitHub Actions

ClawHub는 Plugin 리포지토리를 위한 공식 재사용 가능 워크플로도
[`/.github/workflows/package-publish.yml`](https://github.com/openclaw/clawhub/blob/af96221ebb197e2af09f44870046ced4ded4aea0/.github/workflows/package-publish.yml)에 제공합니다.
...
- 재사용 가능 워크플로의 기본 `source`는 호출자 리포지토리입니다.
...
- 로컬 Skills 폴더를 스캔하고 새 항목 또는 변경된 항목을 게시합니다.
- 루트는 어떤 폴더든 될 수 있습니다: Skills 디렉터리 또는 `SKILL.md`가 있는 단일 Skills 폴더.
- `~/.clawdbot/clawdbot.json`이 있을 때 Clawdbot Skills 루트를 자동으로 추가합니다:
...
- 로그인한 상태에서 `sync` 중 전송됩니다. 단, `CLAWHUB_DISABLE_TELEMETRY=1`(레거시 `CLAWDHUB_DISABLE_TELEMETRY=1`)인 경우는 제외합니다.
