...
summary: 'CLI 참조: 명령, 플래그, 구성, 잠금 파일, 동기화 동작.'
x-i18n:
...
이 변수 중 하나라도 설정되어 있으면 CLI는 지정된 프록시를 통해 아웃바운드 요청을 라우팅합니다. `HTTPS_PROXY`는 HTTPS 요청에 사용되고, `HTTP_PROXY`는 일반 HTTP에 사용됩니다. `NO_PROXY` / `no_proxy`는 특정 호스트나 도메인에 대해 프록시를 우회하도록 준수됩니다.
...
- 레거시 대체: `clawhub/config.json`이 아직 없지만 `clawdhub/config.json`이 있으면 CLI는 레거시 경로를 재사용합니다.
...
- 검색은 다운로드 인기도보다 정확한 slug/name 토큰 일치를 우선합니다. `map`처럼 독립된 slug 토큰은 `amap` 안의 하위 문자열보다 `personal-map`과 더 강하게 일치합니다.
- 다운로드 수는 작은 인기도 사전 가중치일 뿐이며, 최상위 배치를 보장하지 않습니다.
- 표시되어야 하는 skill이 표시되지 않으면 메타데이터 이름을 바꾸기 전에 로그인한 상태에서 `clawhub inspect <slug>`를 실행하여 소유자에게 표시되는 검토 진단을 확인합니다.
...
- 고정된 skill은 덮어쓰기를 거부합니다. 먼저 `clawhub unpin <slug>`를 실행합니다.
- 다음을 기록합니다.
...
- `clawhub pin`으로 고정된 Skills 옆에 선택적 사유와 함께 `pinned`를 표시합니다.

### `pin <slug>`

- 설치된 Skills를 lockfile에서 pinned로 표시합니다.
- `--reason <text>`는 Skills가 고정된 이유를 기록합니다.
- pinned된 Skills는 `update --all`에서 건너뛰며, 직접 `update <slug>`로 업데이트하려 하면 거부됩니다.
- pinned된 Skills는 `install --force`도 거부하므로 로컬 바이트가 실수로 대체될 수 없습니다.

### `unpin <slug>`

- 설치된 Skills에서 lockfile pin을 제거하여 향후 업데이트가 수정할 수 있게 합니다.

### `update [slug]` / `update --all`

- 로컬 파일에서 fingerprint를 계산합니다.
- fingerprint가 알려진 버전과 일치하면 프롬프트가 표시되지 않습니다.
- fingerprint가 일치하지 않으면:
  - 기본적으로 거부합니다
  - `--force`로 덮어씁니다(또는 대화형이면 프롬프트)
- pinned된 Skills는 `--force`로도 업데이트되지 않습니다.
- `update <slug>`는 pinned된 slug에 대해 빠르게 실패하고 먼저 `clawhub unpin <slug>`를 실행하라고 안내합니다.
- `update --all`은 pinned된 slug를 건너뛰고 무엇이 고정된 상태로 남았는지 요약을 출력합니다.

### `skill publish <path>`

- `POST /api/v1/skills`(multipart)를 통해 게시합니다.
- semver가 필요합니다: `--version 1.2.3`.
- 행위자에게 게시자 접근 권한이 있으면 `--owner <handle>`은 org/user 게시자 handle 아래에 게시합니다.
- `--migrate-owner`는 새 버전을 게시하는 동안 기존 Skills를 `--owner`로 이동합니다. 두 게시자 모두에 대한 admin/owner 접근 권한이 필요합니다.
- 소유자 및 리뷰 동작은 `docs/publishing.md`에 설명되어 있습니다.
- Skills를 게시한다는 것은 ClawHub에서 `MIT-0`로 릴리스된다는 의미입니다.
- 게시된 Skills는 저작자 표시 없이 자유롭게 사용, 수정, 재배포할 수 있습니다.
- ClawHub는 유료 Skills 또는 Skills별 가격 책정을 지원하지 않습니다.
- `--clawscan-note <text>`는 ClawScan 메모를 추가합니다. 이 메모는 네트워크 접근, 네이티브 호스트 접근 또는 공급자별 자격 증명처럼 그렇지 않으면 비정상적으로 보일 수 있는 동작에 대한 컨텍스트를 ClawScan에 제공합니다. 이 메모는 게시된 버전에 저장됩니다.
- 레거시 별칭: `publish <path>`.
...
- Skills를 soft-delete합니다(owner, moderator 또는 admin).
- `DELETE /api/v1/skills/{slug}`를 호출합니다.
- 소유자가 시작한 soft delete는 slug를 30일 동안 예약하며, 명령은 만료 시간을 출력합니다.
- `--reason <text>`는 Skills와 audit log에 moderation 메모를 기록합니다.
- `--note <text>`는 `--reason`의 별칭입니다.
- `--yes`는 확인을 건너뜁니다.

### `undelete <slug>`

- 숨겨진 Skills를 복원합니다(owner, moderator 또는 admin).
...
- Skills를 숨깁니다(owner, moderator 또는 admin).
- `delete`의 별칭입니다.

### `unhide <slug>`

- Skills 숨김을 해제합니다(owner, moderator 또는 admin).
- `undelete`의 별칭입니다.

### `skill rename <slug> <new-slug>`

- 소유한 Skills의 이름을 바꾸고 이전 slug를 리디렉션 별칭으로 유지합니다.
- `POST /api/v1/skills/{slug}/rename`를 호출합니다.
- `--yes`는 확인을 건너뜁니다.

### `skill merge <source-slug> <target-slug>`

- 소유한 Skills 하나를 다른 소유한 Skills로 병합합니다.
- 원본 slug는 공개 목록에 표시되지 않으며 대상에 대한 리디렉션 별칭이 됩니다.
...
- org/publisher handle로 이전하는 경우 행위자가 현재 소유자와 대상 게시자 모두에 대한 admin 접근 권한을 가질 때만 즉시 적용됩니다.
- 하위 명령:
...
- plugins 및 다른 package-family 항목에는 이것을 사용하세요. 최상위 `search`는 Skills 검색 표면으로 유지됩니다.
- 플래그:
...
- `--files`: 선택한 버전의 파일을 나열합니다.
- `--file <path>`: 원시 파일 내용을 가져옵니다(텍스트 파일만 해당, 200KB 제한).
- `--json`: 기계 판독 가능 출력입니다.
...
- `--package`와 함께 사용하면 ClawHub에서 예상 메타데이터를 해석하고 로컬 파일을 게시된 artifact 메타데이터와 비교합니다.
- 직접 digest 플래그를 사용하면 네트워크 조회 없이 검증합니다.
- 플래그:
  - `--package <name>`: 예상 artifact 메타데이터를 해석할 패키지 이름입니다.
...
- 모더레이터에게 패키지를 신고하는 인증된 명령입니다.
- `POST /api/v1/packages/{name}/report`를 호출합니다.
- 신고는 패키지 수준이며, 선택적으로 버전과 연결될 수 있고 검토를 위해
  모더레이터에게 표시됩니다.
- 신고 자체만으로 패키지가 자동으로 숨겨지거나 다운로드가 차단되지는 않습니다.
- 플래그:
  - `--version <version>`: 신고에 첨부할 선택 사항인 패키지 버전입니다.
...
- 코드 plugin 폴더는 업로드 전에 ClawPack npm 타르볼로 패킹되어
  ...
  추출된 파일 게시 경로를 사용합니다.
- GitHub 소스의 경우 저장소, 확인된 커밋, ref, 하위 경로에서 소스 속성이 자동으로 채워집니다.
...
- `--json`은 CI용으로 기계가 읽을 수 있는 출력을 내보냅니다.
- `--owner <handle>`은 행위자에게 게시자 접근 권한이 있을 때 사용자 또는 조직 게시자 핸들 아래에 게시합니다.
- `--clawscan-note <text>`는 ClawScan 메모를 추가합니다. 이 메모는 네트워크 접근,
  네이티브 호스트 접근, 공급자별 자격 증명처럼 그렇지 않으면 비정상적으로 보일 수 있는 동작에 대한
  컨텍스트를 ClawScan에 제공합니다. 메모는 게시된 릴리스에 저장됩니다.
...
- 더 자세한 호환성 메타데이터를 게시하려면
  ...
  `openclaw.build.pluginSdkVersion`을 선택 사항으로 추가할 수 있습니다.
- 오래된 `clawhub` CLI 릴리스를 사용하는 경우, 로컬 사전 점검이 업로드 전에 실행되도록
  게시하기 전에 업그레이드하세요.

#### GitHub Actions

ClawHub는 plugin 저장소를 위한 공식 재사용 가능 워크플로도
[`/.github/workflows/package-publish.yml`](https://github.com/openclaw/clawhub/blob/2ddaad62cc7852eb8274022ae8a6d7527d169ae8/.github/workflows/package-publish.yml)에 제공합니다.
...
- 비밀 없이 신뢰할 수 있는 게시는 `workflow_dispatch`에서만 작동하며, 태그 푸시에는 여전히 `clawhub_token`이 필요합니다.
...
- 로그인되어 있을 때 `sync` 중 전송되며, `CLAWHUB_DISABLE_TELEMETRY=1`(레거시 `CLAWDHUB_DISABLE_TELEMETRY=1`)인 경우는 제외됩니다.
