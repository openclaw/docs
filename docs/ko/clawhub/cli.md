...
이 변수 중 하나라도 설정되어 있으면 CLI는 아웃바운드 요청을 지정된 프록시를 통해 라우팅합니다. `HTTPS_PROXY`는 HTTPS 요청에 사용되고, `HTTP_PROXY`는 일반 HTTP에 사용됩니다. 특정 호스트나 도메인에 대해 프록시를 우회하도록 `NO_PROXY` / `no_proxy`가 적용됩니다.
...
- 원격/헤드리스 대화형: `clawhub login --device`가 코드를 출력하고, 사용자가 `<site>/cli/device`에서 승인하는 동안 대기합니다.
...
- 검색은 다운로드 인기도보다 정확한 slug/name 토큰 일치를 우선합니다. `map`과 같은 독립 slug 토큰은 `amap` 내부의 부분 문자열보다 `personal-map`과 더 강하게 일치합니다.
- 다운로드 수는 작은 인기도 사전값일 뿐, 상위 배치를 보장하지 않습니다.
- skill이 표시되어야 하는데 표시되지 않으면 메타데이터 이름을 바꾸기 전에 로그인한 상태에서 `clawhub inspect <slug>`를 실행하여 소유자에게 보이는 모더레이션 진단을 확인하세요.

### `explore`

- `/api/v1/skills?limit=...&sort=createdAt`를 통해 최신 skill 목록을 표시합니다(`createdAt` 내림차순 정렬).
- 플래그:
...
- 고정된 skill 덮어쓰기를 거부합니다. 먼저 `clawhub unpin <slug>`를 실행하세요.
- 다음을 씁니다.
...
- `clawhub pin`으로 고정된 스킬 옆에 선택적 사유를 포함해 `pinned`를 표시합니다.

### `pin <slug>`

- 설치된 스킬을 lockfile에서 고정된 것으로 표시합니다.
- `--reason <text>`는 스킬이 고정된 이유를 기록합니다.
- 고정된 스킬은 `update --all`에서 건너뛰며 직접 `update <slug>`를 실행하면 거부됩니다.
- 고정된 스킬은 `install --force`도 거부하므로 로컬 바이트가 실수로 교체되지 않습니다.
...
- 로컬 파일에서 지문을 계산합니다.
- 지문이 알려진 버전과 일치하는 경우: 프롬프트가 없습니다.
- 지문이 일치하지 않는 경우:
  - 기본적으로 거부합니다
...
- `--owner <handle>`은 행위자에게 게시자 접근 권한이 있을 때
  조직/사용자 게시자 핸들 아래에 게시합니다.
- `--migrate-owner`는 새 버전을 게시하는 동안 기존 스킬을 `--owner`로 이동합니다.
  두 게시자 모두에 대한 admin/owner 접근 권한이 필요합니다.
- 소유자 및 검토 동작은 `docs/publishing.md`에 설명되어 있습니다.
- 스킬을 게시한다는 것은 ClawHub에서 `MIT-0`에 따라 릴리스된다는 뜻입니다.
- 게시된 스킬은 출처 표시 없이 자유롭게 사용, 수정, 재배포할 수 있습니다.
- ClawHub는 유료 스킬 또는 스킬별 가격 책정을 지원하지 않습니다.
- `--clawscan-note <text>`는 ClawScan 참고를 추가합니다. 이 참고는 네트워크 접근,
  네이티브 호스트 접근, 공급자별 자격 증명처럼 그렇지 않으면 비정상적으로 보일 수 있는 동작에 대한
  맥락을 ClawScan에 제공합니다. 참고는 게시된 버전에 저장됩니다.
- 레거시 별칭: `publish <path>`.
...
- 소유자가 시작한 소프트 삭제는 slug를 30일 동안 예약합니다. 명령은 만료 시간을 출력합니다.
- `--reason <text>`는 스킬과 audit log에 중재 참고를 기록합니다.
...
- 소유한 스킬 하나를 다른 소유 스킬로 병합합니다.
- 소스 slug는 공개 목록 표시를 중단하고 대상으로 가는 리디렉션 별칭이 됩니다.
...
- 소유권 이전 워크플로입니다.
- 사용자 핸들로 이전하면 수신자가 수락하는 보류 중 요청이 생성됩니다.
- 조직/게시자 핸들로 이전하는 경우 행위자에게 현재 소유자와 대상 게시자 모두에 대한
  admin 접근 권한이 있을 때만 즉시 적용됩니다.
- 하위 명령:
...
- 모더레이터에게 패키지를 신고하기 위한 인증된 명령입니다.
...
- 번들된 OpenClaw 플러그인을 대체할 수 있는 패키지에 대해
  운영자 지향 마이그레이션 상태를 표시합니다.
...
- `POST /api/v1/packages`를 통해 코드 플러그인 또는 번들 플러그인을 게시합니다.
- `<source>`는 다음을 허용합니다.
  - 로컬 폴더 경로: `./my-plugin`
...
- 코드 플러그인 폴더는 업로드 전에 ClawPack npm 타르볼로 패키징되어
  OpenClaw 설치가 정확한 아티팩트를 검증할 수 있게 합니다. 번들 플러그인 폴더는 여전히
  추출 파일 게시 경로를 사용합니다.
- GitHub 소스의 경우, 소스 속성은 저장소, 확인된 커밋, ref, 하위 경로에서 자동으로 채워집니다.
- 로컬 폴더의 경우, 원본 원격이 GitHub를 가리키면 로컬 git에서 소스 속성이 자동 감지됩니다.
- 외부 코드 플러그인은 `openclaw.compat.pluginApi`와
...
- `--clawscan-note <text>`는 ClawScan 메모를 추가합니다. 이 메모는 네트워크 액세스,
  네이티브 호스트 액세스 또는 공급자별 자격 증명처럼 그렇지 않으면 특이해 보일 수 있는 동작에 대한
  맥락을 ClawScan에 제공합니다. 메모는
  게시된 릴리스에 저장됩니다.
- 범위가 지정된 패키지 이름은 선택된 소유자와 일치해야 합니다. `docs/publishing.md`를 참조하세요.
...
코드 플러그인의 경우, 폴더 게시가 패키지 폴더에서 ClawPack 아티팩트를 빌드하고 업로드합니다.
...
외부 코드 플러그인은 `package.json`에 소량의 OpenClaw 메타데이터가
필요합니다. 이 최소 매니페스트만으로 성공적으로 게시할 수 있습니다.
...
- `openclaw.hostTargets`와 `openclaw.environment`는 선택적 메타데이터입니다.
  ClawHub는 이들이 있을 때 표시할 수 있지만, 게시에 필수는 아닙니다.
- `openclaw.compat.minGatewayVersion`과
  `openclaw.build.pluginSdkVersion`은 더 자세한 호환성 메타데이터를
  게시하려는 경우 사용할 수 있는 선택적 추가 항목입니다.
- 이전 `clawhub` CLI 릴리스를 사용 중이라면, 업로드 전에 로컬 사전 검사가 실행되도록
  게시 전에 업그레이드하세요.

#### GitHub Actions

ClawHub는 플러그인 저장소용 공식 재사용 가능 워크플로도
...
- 로컬 스킬 폴더를 스캔하고 새 항목 또는 변경된 항목을 게시합니다.
- 루트는 어떤 폴더든 될 수 있습니다. 스킬 디렉터리이거나 `SKILL.md`가 있는 단일 스킬 폴더입니다.
- `~/.clawdbot/clawdbot.json`이 있으면 Clawdbot 스킬 루트를 자동 추가합니다.
  - `agent.workspace/skills`(주 에이전트)
