...
직접 아웃바운드 연결이 차단된 시스템(예: Docker 컨테이너, 프록시 전용 인터넷을 사용하는 Hetzner VPS, 기업 방화벽)에서는 이것이 필요합니다.
...
- 레거시 대체 경로: 아직 `clawhub/config.json`은 없지만 `clawdhub/config.json`이 있으면 CLI가 레거시 경로를 재사용합니다
...
- 검색은 다운로드 인기도보다 정확한 slug/이름 토큰 일치를 우선합니다. `map` 같은 독립 slug 토큰은 `amap` 내부의 부분 문자열보다 `personal-map`에 더 강하게 일치합니다.
- 인기도는 작은 순위 사전 가중치일 뿐, 최상위 배치를 보장하지 않습니다.
- 스킬이 표시되어야 하는데 표시되지 않으면, 메타데이터 이름을 바꾸기 전에 로그인한 상태에서 `clawhub inspect @owner/slug`를 실행하여 소유자에게 보이는 검토 진단을 확인합니다.
...
- 플래그:
  ...
  - `--sort newest|updated|rating|downloads|trending`(기본값: newest). 호환성을 위해 레거시 설치 정렬 별칭도 계속 동작합니다.
  - `--json`(기계가 읽을 수 있는 출력)
...
- 고정된 스킬 덮어쓰기를 거부합니다. 먼저 `clawhub unpin <skill>`을 실행합니다.
- 다음을 씁니다.
...
- 로그인되어 있으면 현재 설치 수를 비활성화할 수 있도록 최선형 텔레메트리를 전송합니다.
- 대화형: 확인을 요청합니다.
...
- 선택적 이유를 포함하여 `clawhub pin`으로 동결된 스킬 옆에 `pinned`를 표시합니다.

### `pin <skill>`

- 설치된 스킬을 lockfile에서 고정됨으로 표시합니다.
...
- 향후 업데이트가 수정할 수 있도록 설치된 스킬에서 lockfile 고정을 제거합니다.
...
- 스킬을 게시한다는 것은 ClawHub에서 `MIT-0`로 릴리스된다는 의미입니다.
...
워크플로는 하나의 `skill_path`에 대해, 또는 `root`(기본값: `skills`) 아래의 각 즉시 하위 스킬 폴더에 대해 `skill publish`를 호출합니다. 변경되지 않은 스킬은 건너뛰며 동일한 자동 패치 버전 동작을 사용합니다.

토큰 없이 미리 보려면 `dry_run: true`를 설정합니다. 실제 게시에는 `clawhub_token` 시크릿이 필요합니다.

### `sync`

- 현재 workdir, 구성된 스킬 디렉터리, 그리고 `SKILL.md` 또는 `skill.md`를 포함하는 로컬 스킬 폴더를 찾기 위한 모든 `--root <dir>` 폴더를 스캔합니다.
...
- 게시된 스캔에는 소유권 또는 게시자 관리 접근 권한이 필요합니다. moderator/admin은 `clawhub-admin`을 통해 동일한 백엔드를 사용할 수 있습니다.
...
- 로컬 경로 스캔은 더 이상 지원되지 않습니다. 새 버전을 업로드한 다음 `scan download`를 사용하여 제출된 해당 버전에 저장된 스캔 결과를 검색합니다.
...
- `clawhub login`이 필요합니다.
- ClawHub 보안 검사에 의해 차단되었거나 숨겨진 버전을 포함하여, 제출된 스킬 또는 Plugin 버전에 저장된 스캔 보고서 ZIP을 다운로드합니다.
...
- 카탈로그 저장소의 `root` 기본값은 `skills`입니다.
...
- `--version` 없이 스킬을 소프트 삭제합니다(소유자, 중재자 또는 관리자).
- `DELETE /api/v1/skills/{slug}`를 호출합니다.
- 소유자가 시작한 소프트 삭제는 30일 동안 slug를 예약하며, 명령은 만료 시간을 출력합니다.
- `--version <version>`은 fail-closed,
  버전별 경로를 통해 소유한 최신이 아닌 버전 하나를 영구 삭제합니다.
...
- 원본 slug는 공개 목록에 더 이상 표시되지 않고 대상에 대한 리디렉션 별칭이 됩니다.
...
- 소유권 이전 워크플로입니다.
- 사용자 핸들로의 이전은 수신자가 수락하는 대기 중 요청을 생성합니다.
- 조직/게시자 핸들로의 이전은 행위자가 현재 소유자와 대상 게시자 모두에 대해
...
- 플러그인과 기타 패키지 계열 항목에는 이것을 사용하세요. 최상위 `search`는 계속 스킬 검색 표면입니다.
- 플래그:
...
- 기존 ZIP 버전은 기존 ZIP 경로를 통해 다운로드됩니다.
- 플래그:
...
- 로컬 아티팩트에 대해 ClawHub SHA-256, npm `sha512` 무결성, npm shasum을
  계산합니다.
- `--package`와 함께 사용하면 ClawHub에서 예상 메타데이터를 해석하고
...
- 로컬 플러그인 패키지 폴더에 대해 ClawHub CLI에 번들된 Plugin Inspector를
  실행합니다.
- 로컬 OpenClaw 체크아웃을 찾거나 가져오지 않고 오프라인/정적 검증을 기본값으로 사용합니다.
- 강한 호환성 오류는 0이 아닌 코드로 종료합니다. 경고만 있는 결과는 출력되지만
  0으로 종료합니다.
- 플래그:
  ...
  - `--allow-execute`: 격리된 작업공간에서 런타임 캡처를 허용합니다.
...
검증에서 패키지, 매니페스트, SDK import 또는 아티팩트 결과가 보고되면
[Plugin 검증 수정](/clawhub/plugin-validation-fixes)을 확인한 다음 명령을 다시 실행하세요.

### `package delete <name>`

- `--version` 없이 패키지와 모든 릴리스를 소프트 삭제합니다.
- `--version <version>`은 fail-closed,
  버전별 경로를 통해 소유한 최신이 아닌 릴리스 하나를 영구 삭제합니다.
  삭제된 버전은 복원하거나 다시 게시할 수 없습니다. 현재 최신 버전을 삭제하기 전에
  대체 버전을 게시하세요. 이 버전 전용 흐름에는 패키지 소유자 또는 조직 게시자
  관리자가 필요합니다. 플랫폼 스태프는 패키지 소유권을 우회하지 않습니다.
...
- scoped 패키지 이름은 일치하는 scope 소유자로 이전해야 합니다.
...
- 신고는 패키지 수준이며 선택적으로 버전에 연결될 수 있고, 검토를 위해
  중재자에게 표시됩니다.
- 신고 자체만으로 패키지를 자동으로 숨기거나 다운로드를 차단하지는 않습니다.
- 플래그:
...
- 번들된 OpenClaw 플러그인을 대체할 수 있는 패키지의 운영자 지향
  마이그레이션 상태를 표시합니다.
...
- 새로 생성된 조직 게시자는 기본적으로 신뢰/공식 상태가 아닙니다.
...
- `--owner <handle>`은 행위자에게 게시자 액세스 권한이 있을 때 사용자 또는 조직 게시자 핸들 아래에 게시합니다.
...
- `openclaw.hostTargets` 및 `openclaw.environment`는 선택적 메타데이터입니다.
  존재할 경우 ClawHub에서 표시할 수 있지만 게시에 필수는 아닙니다.
- 더 자세한 호환성 메타데이터를 게시하려면 `openclaw.compat.minGatewayVersion` 및
...
- 재사용 가능 워크플로의 기본 `source`는 호출자 저장소입니다.
...
- 이 명령이 실행될 때 ClawHub는 구성된 GitHub 저장소를 검증합니다.
  ...
  GitHub 통합 등을 통해 ClawHub가 해당 저장소에 대한 GitHub 액세스 권한을 가져야 합니다.
- 플래그:
...
- 로그인된 상태에서 `CLAWHUB_DISABLE_TELEMETRY=1`이 설정되지 않은 경우
  `clawhub install <slug>` 이후 전송됩니다.
- 보고는 최선형으로 수행됩니다. 원격 분석을 사용할 수 없어도 설치 명령은 실패하지 않습니다.
