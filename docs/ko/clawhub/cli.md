...
CLI는 회사 프록시나 제한된 네트워크 뒤에 있는 시스템을 위해 표준 HTTP 프록시 환경 변수를 준수합니다.
...
- `/api/v1/whoami`를 통해 저장된 토큰을 검증합니다.
...
- 검색은 다운로드 인기도보다 정확한 slug/이름 토큰 일치를 우선합니다. `map` 같은 독립형 slug 토큰은 `amap` 내부의 부분 문자열보다 `personal-map`에 더 강하게 일치합니다.
- 다운로드 수는 작은 인기도 사전 가중치일 뿐, 최상위 배치를 보장하지 않습니다.
- 표시되어야 할 스킬이 표시되지 않으면 메타데이터 이름을 바꾸기 전에 로그인한 상태에서 `clawhub inspect <slug>`를 실행하여 소유자에게 표시되는 검토 진단 정보를 확인합니다.
...
- 고정된 스킬 덮어쓰기를 거부합니다. 먼저 `clawhub unpin <slug>`를 실행합니다.
- 다음을 씁니다.
...
- `update <slug>`는 고정된 슬러그에 대해 빠르게 실패하며 먼저 `clawhub unpin <slug>`를 실행하라고 안내합니다.
- `update --all`은 고정된 슬러그를 건너뛰고 무엇이 계속 고정되어 있는지 요약을 출력합니다.
...
- 스킬을 게시한다는 것은 해당 스킬이 ClawHub에서 `MIT-0`으로 릴리스된다는 뜻입니다.
- 게시된 스킬은 저작자 표시 없이 자유롭게 사용, 수정, 재배포할 수 있습니다.
- ClawHub는 유료 스킬이나 스킬별 가격 책정을 지원하지 않습니다.
- `--clawscan-note <text>`는 ClawScan 메모를 추가합니다. 이 메모는 네트워크 접근, 네이티브 호스트 접근, 공급자별 자격 증명처럼 그렇지 않으면 특이해 보일 수 있는 동작에 대한 맥락을 ClawScan에 제공합니다. 메모는 게시된 버전에 저장됩니다.
- 레거시 별칭: `publish <path>`.
...
- `--reason <text>`는 스킬 및 감사 로그에 중재 메모를 기록합니다.
...
- 소스 슬러그는 공개 목록에서 제외되고 대상에 대한 리디렉션 별칭이 됩니다.
...
- 조직/게시자 핸들로 이전하는 경우 행위자가 현재 소유자와 대상 게시자 모두에 대한 관리자 접근 권한을 가진 경우에만 즉시 적용됩니다.
- 하위 명령:
...
- Plugin 및 기타 패키지 계열 항목에는 이것을 사용합니다. 최상위 `search`는 계속 스킬 검색 표면입니다.
- 플래그:
...
- 패키지 소유자, 조직 게시자 소유자/관리자, 플랫폼 중재자 또는
  플랫폼 관리자가 필요합니다.
- 플래그:
  - `--yes`: 확인을 건너뜁니다.
  - `--json`: 기계가 읽을 수 있는 출력입니다.
...
- 번들 OpenClaw 플러그인을 대체할 수 있는 패키지의 운영자 지향 마이그레이션 상태를 표시합니다.
...
- `.tgz` 소스는 ClawPack으로 처리됩니다. CLI는 정확한 npm-pack
  ...
  메타데이터 미리 채우기에만 사용합니다.
- 코드 플러그인 폴더는 업로드 전에 ClawPack npm tarball로 패키징되어
  OpenClaw 설치가 정확한 아티팩트를 검증할 수 있게 합니다. 번들 플러그인 폴더는 여전히
  추출 파일 게시 경로를 사용합니다.
- GitHub 소스의 경우 소스 귀속 정보는 저장소, 해석된 커밋, ref, 하위 경로에서 자동으로 채워집니다.
- 로컬 폴더의 경우 원격 origin이 GitHub를 가리킬 때 로컬 git에서 소스 귀속 정보가 자동 감지됩니다.
...
- `--dry-run`은 업로드 없이 해석된 게시 페이로드를 미리 봅니다.
...
- `--clawscan-note <text>`는 ClawScan 참고 사항을 추가합니다. 이 참고 사항은 네트워크 액세스,
  네이티브 호스트 액세스 또는 제공자별 자격 증명처럼 일반적이지 않게 보일 수 있는 동작에 대한
  ClawScan 컨텍스트를 제공합니다. 참고 사항은
  게시된 릴리스에 저장됩니다.
...
외부 코드 플러그인은 `package.json`에 소량의 OpenClaw 메타데이터가 필요합니다.
...
- 더 자세한 호환성 메타데이터를 게시하려는 경우
  `openclaw.compat.minGatewayVersion`과
  `openclaw.build.pluginSdkVersion`은 선택적 추가 항목입니다.
- 더 오래된 `clawhub` CLI 릴리스를 사용 중이라면, 업로드 전에
  로컬 사전 검사가 실행되도록 게시하기 전에 업그레이드하세요.

#### GitHub Actions

ClawHub는 플러그인 저장소용 공식 재사용 가능 워크플로도
[`/.github/workflows/package-publish.yml`](https://github.com/openclaw/clawhub/blob/c51cfe2459f3482c315a7c8c71b2efd2637bb0e8/.github/workflows/package-publish.yml)에 제공합니다.
...
- 모노레포의 경우 워크플로가 플러그인 패키지 폴더를 게시하도록
