...
- `--dir <dir>`: workdir 아래 설치 디렉터리(기본값: `skills`)
...
- skill이 표시되어야 하는데 표시되지 않으면, 메타데이터 이름을 바꾸기 전에 로그인한 상태에서 `clawhub inspect <slug>`를 실행하여 소유자에게 보이는 모더레이션 진단을 확인합니다.
...
- 고정된 skills를 덮어쓰지 않습니다. 먼저 `clawhub unpin <slug>`를 실행합니다.
- 다음을 씁니다.
...
- `clawhub pin`으로 고정된 스킬 옆에 `pinned`를 표시하며, 선택 사항인 사유도 함께 표시합니다.

### `pin <slug>`

- 설치된 스킬을 lockfile에서 pinned로 표시합니다.
- `--reason <text>`는 스킬이 고정된 이유를 기록합니다.
- Pinned 스킬은 `update --all`에서 건너뛰며 직접 `update <slug>`를 실행하면 거부됩니다.
- Pinned 스킬은 `install --force`도 거부하므로 로컬 바이트가 실수로 교체되지 않습니다.

### `unpin <slug>`

- 설치된 스킬에서 lockfile pin을 제거해 이후 업데이트가 수정할 수 있게 합니다.
...
- fingerprint가 일치하지 않으면:
  - 기본적으로 거부합니다
  - `--force`로 덮어씁니다(또는 interactive인 경우 프롬프트)
- Pinned 스킬은 `--force`로도 업데이트되지 않습니다.
- `update <slug>`는 pinned slug에 대해 빠르게 실패하며 먼저 `clawhub unpin <slug>`를 실행하라고 안내합니다.
- `update --all`은 pinned slug를 건너뛰고 고정 상태로 남은 항목의 요약을 출력합니다.

### `skill publish <path>`

- `POST /api/v1/skills`(multipart)를 통해 게시합니다.
- semver가 필요합니다: `--version 1.2.3`.
- `--owner <handle>`은 actor에게 publisher 접근 권한이 있을 때 org/user publisher handle 아래에 게시합니다.
- `--migrate-owner`는 새 버전을 게시하는 동안 기존 스킬을 `--owner`로 이동합니다. 두 publisher 모두에 admin/owner 접근 권한이 필요합니다.
...
- ClawHub는 유료 스킬이나 스킬별 pricing을 지원하지 않습니다.
- `--clawscan-note <text>`는 ClawScan note를 추가합니다. 이 note는 network access, native host access, provider-specific credentials처럼 달리 보기에 비정상적으로 보일 수 있는 동작에 대한 컨텍스트를 ClawScan에 제공합니다. note는 게시된 버전에 저장됩니다.
...
- 소유한 스킬의 이름을 변경하고 이전 slug를 redirect alias로 유지합니다.
- `POST /api/v1/skills/{slug}/rename`를 호출합니다.
- `--yes`는 confirmation을 건너뜁니다.

### `skill merge <source-slug> <target-slug>`

- 소유한 한 스킬을 소유한 다른 스킬로 병합합니다.
- source slug는 공개 목록에서 제외되고 target에 대한 redirect alias가 됩니다.
- `POST /api/v1/skills/{sourceSlug}/merge`를 호출합니다.
- `--yes`는 confirmation을 건너뜁니다.

### `transfer`

- 소유권 transfer workflow입니다.
- user handle로 transfer하면 recipient가 accept하는 pending request가 생성됩니다.
- org/publisher handle로 transfer하는 경우 actor에게 현재 owner와 destination publisher 양쪽 모두에 대한 admin access가 있을 때만 즉시 적용됩니다.
- Subcommands:
...
- plugins와 기타 package-family entries에는 이것을 사용하세요. top-level `search`는 계속 skill search surface로 남습니다.
- Flags:
...
- `--files`: 선택한 버전의 파일을 나열합니다.
- `--file <path>`: raw file content를 가져옵니다(text files only; 200KB limit).
...
- 레거시 ZIP 버전은 legacy ZIP route를 통해 다운로드됩니다.
- Flags:
...
- `--package`를 사용하면 ClawHub에서 expected metadata를 resolve하고 로컬 파일을 published artifact metadata와 비교합니다.
...
- 패키지 중재 공개 상태를 확인하기 위한 소유자 명령입니다.
...
- 코드 plugin 폴더는 업로드 전에 ClawPack npm tarball로 패키징되어
  ...
  추출된 파일 게시 경로를 사용합니다.
- GitHub 소스의 경우 소스 출처가 저장소, 확인된 커밋, ref, 하위 경로에서 자동 입력됩니다.
- 로컬 폴더의 경우 origin 원격이 GitHub를 가리키면 로컬 git에서 소스 출처를 자동 감지합니다.
...
- `--clawscan-note <text>`는 ClawScan 메모를 추가합니다. 이 메모는 네트워크 액세스,
  네이티브 호스트 액세스 또는 제공자별 자격 증명처럼 그렇지 않으면 특이해 보일 수 있는 동작에 대한
  컨텍스트를 ClawScan에 제공합니다. 이 메모는 게시된 릴리스에 저장됩니다.
- 범위가 지정된 패키지 이름은 선택한 소유자와 일치해야 합니다. `docs/publishing.md`를 참조하세요.
- 기존 플래그(`--family`, `--name`, `--version`, `--source-repo`, `--source-commit`, `--source-ref`, `--source-path`)도 여전히 재정의로 작동합니다.
...
이 최소 매니페스트만으로 성공적으로 게시할 수 있습니다.
...
- `openclaw.hostTargets`와 `openclaw.environment`는 선택적 메타데이터입니다.
  존재할 경우 ClawHub가 표시할 수 있지만 게시에 필수는 아닙니다.
...
- 비밀 없이 신뢰할 수 있는 게시를 하려면 `workflow_dispatch`에서만 작동합니다. 태그 push에는 여전히 `clawhub_token`이 필요합니다.
...
- 로그인한 경우 `sync` 중 전송됩니다. 단, `CLAWHUB_DISABLE_TELEMETRY=1`(레거시 `CLAWDHUB_DISABLE_TELEMETRY=1`)이면 제외됩니다.
