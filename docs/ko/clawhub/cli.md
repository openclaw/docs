...
summary: 'CLI 참조: 명령, 플래그, 구성, lockfile, 동기화 동작.'
x-i18n:
...
이 변수 중 하나라도 설정되어 있으면 CLI는 지정된 프록시를 통해 아웃바운드 요청을 라우팅합니다. `HTTPS_PROXY`는 HTTPS 요청에 사용되고, `HTTP_PROXY`는 일반 HTTP에 사용됩니다. 특정 호스트 또는 도메인에 대해 프록시를 우회하도록 `NO_PROXY` / `no_proxy`가 적용됩니다.

직접 아웃바운드 연결이 차단된 시스템(예: Docker 컨테이너, 프록시 전용 인터넷을 사용하는 Hetzner VPS, 회사 방화벽)에서는 이것이 필요합니다.
...
- 검색은 다운로드 인기도보다 정확한 슬러그/이름 토큰 일치를 우선합니다. `map` 같은 독립 슬러그 토큰은 `amap` 안의 부분 문자열보다 `personal-map`에 더 강하게 일치합니다.
- 다운로드 수는 작은 인기도 사전 정보일 뿐, 최상위 배치를 보장하지 않습니다.
- 표시되어야 할 스킬이 표시되지 않으면 메타데이터 이름을 바꾸기 전에 로그인한 상태에서 `clawhub inspect <slug>`를 실행해 소유자에게 표시되는 검토 진단을 확인하세요.
...
- 설치된 skill을 잠금 파일에서 고정됨으로 표시합니다.
- `--reason <text>`는 skill이 고정된 이유를 기록합니다.
- 고정된 skills는 `update --all`에서 건너뛰며, 직접 `update <slug>`를 실행하면 거부됩니다.
- 고정된 skills는 `install --force`도 거부하여 로컬 바이트가 실수로 교체되지 않게 합니다.

### `unpin <slug>`

- 설치된 skill에서 잠금 파일 고정을 제거하여 이후 업데이트가 수정할 수 있게 합니다.
...
- 지문이 일치하지 않는 경우:
  - 기본적으로 거부합니다
  - `--force`로 덮어씁니다(대화형이면 프롬프트)
- 고정된 skills는 `--force`로도 업데이트되지 않습니다.
- `update <slug>`는 고정된 slugs에 대해 즉시 실패하며, 먼저 `clawhub unpin <slug>`를 실행하라고 안내합니다.
- `update --all`은 고정된 slugs를 건너뛰고 고정 상태로 남은 항목의 요약을 출력합니다.
...
- `--migrate-owner`는 새 버전을 게시하는 동안 기존 skill을 `--owner`로 이동합니다. 두 게시자 모두에 대한 관리자/소유자 접근 권한이 필요합니다.
- 소유자 및 검토 동작은 `docs/publishing.md`에 설명되어 있습니다.
- skill을 게시하면 ClawHub에서 `MIT-0`으로 릴리스됩니다.
- 게시된 skills는 저작자 표시 없이 자유롭게 사용, 수정, 재배포할 수 있습니다.
- ClawHub는 유료 skills 또는 skill별 가격 책정을 지원하지 않습니다.
- `--clawscan-note <text>`는 ClawScan 메모를 추가합니다. 이 메모는 네트워크 접근, 네이티브 호스트 접근, 제공자별 자격 증명처럼 그렇지 않으면 특이해 보일 수 있는 동작에 대한 맥락을 ClawScan에 제공합니다. 메모는 게시된 버전에 저장됩니다.
- 레거시 별칭: `publish <path>`.
...
- skill을 소프트 삭제합니다(소유자, 모더레이터 또는 관리자).
...
- 소유한 skill의 이름을 변경하고 이전 slug를 리디렉션 별칭으로 유지합니다.
...
- 원본 slug는 공개 목록에 더 이상 표시되지 않으며 대상에 대한 리디렉션 별칭이 됩니다.
...
- 소유권 이전 워크플로입니다.
- 사용자 핸들로의 이전은 수신자가 수락하는 보류 중 요청을 생성합니다.
- 조직/게시자 핸들로의 이전은 행위자에게 현재 소유자와 대상 게시자 모두에 대한 관리자 접근 권한이 있을 때만 즉시 적용됩니다.
- 하위 명령:
...
- plugins 및 기타 패키지 계열 항목에는 이것을 사용합니다. 최상위 `search`는 계속 skill 검색 화면으로 유지됩니다.
- 플래그:
...
- Plugin 메타데이터, 호환성, 검증, 소스, 버전/파일 검사에 이것을 사용합니다.
- `--version <version>`: 특정 버전을 검사합니다(기본값: 최신).
...
- `--package`와 함께 사용하면 ClawHub에서 예상 메타데이터를 해석하고 로컬 파일을 게시된 아티팩트 메타데이터와 비교합니다.
- 직접 다이제스트 플래그와 함께 사용하면 네트워크 조회 없이 검증합니다.
- 플래그:
  - `--package <name>`: 예상 아티팩트 메타데이터를 해석할 패키지 이름.
...
- 패키지 소유자, 조직 퍼블리셔 소유자/관리자, 플랫폼 조정자
  또는 플랫폼 관리자가 필요합니다.
- 플래그:
  - `--yes`: 확인을 건너뜁니다.
...
- 번들로 제공되는 OpenClaw Plugin을 대체할 수 있는 패키지의 운영자 지향
  마이그레이션 상태를 표시합니다.
...
- 코드 Plugin 폴더는 업로드 전에 ClawPack npm tarball로 패킹되어
  OpenClaw 설치에서 정확한 아티팩트를 검증할 수 있게 합니다. 번들 Plugin 폴더는 여전히
  추출된 파일 게시 경로를 사용합니다.
- GitHub 소스의 경우, 소스 출처는 리포지토리, 확인된 커밋, ref, 하위 경로에서 자동으로 채워집니다.
...
- 외부 코드 Plugin은 `openclaw.compat.pluginApi` 및
  ...
  최상위 `package.json.version`은 게시 검증의 폴백으로 사용되지 않습니다.
...
- `--clawscan-note <text>`는 ClawScan 메모를 추가합니다. 이 메모는 네트워크 접근,
  네이티브 호스트 접근, 제공자별 자격 증명처럼 그렇지 않으면 특이해 보일 수 있는 동작에 대한
  컨텍스트를 ClawScan에 제공합니다. 메모는 게시된 릴리스에 저장됩니다.
- scoped 패키지 이름은 선택한 소유자와 일치해야 합니다. `docs/publishing.md`를 참고하세요.
- 기존 플래그(`--family`, `--name`, `--version`, `--source-repo`, `--source-commit`, `--source-ref`, `--source-path`)는 여전히 오버라이드로 동작합니다.
...
다음 최소 매니페스트만으로 성공적으로 게시할 수 있습니다.
...
- `package.json.version`은 패키지 릴리스 버전이지만,
  OpenClaw 호환성/빌드 검증의 폴백으로 사용되지 않습니다.
- `openclaw.hostTargets` 및 `openclaw.environment`는 선택적 메타데이터입니다.
  존재할 경우 ClawHub가 이를 표시할 수 있지만, 게시에 필수는 아닙니다.
- 더 자세한 호환성 메타데이터를 게시하려면 `openclaw.compat.minGatewayVersion` 및
  `openclaw.build.pluginSdkVersion`을 선택적으로 추가할 수 있습니다.
- 이전 `clawhub` CLI 릴리스를 사용 중이라면, 업로드 전에 로컬 사전 검사들이 실행되도록
  게시 전에 업그레이드하세요.

#### GitHub Actions

ClawHub는 Plugin 리포지토리용 공식 재사용 가능 workflow도
[`/.github/workflows/package-publish.yml`](https://github.com/openclaw/clawhub/blob/af96221ebb197e2af09f44870046ced4ded4aea0/.github/workflows/package-publish.yml)에서 제공합니다.
...
- 재사용 가능 workflow는 기본적으로 `source`를 호출자 리포지토리로 설정합니다.
- 모노리포의 경우, workflow가 Plugin 패키지 폴더를 게시하도록
...
- 로그인한 상태에서 `sync` 중 전송됩니다. 단, `CLAWHUB_DISABLE_TELEMETRY=1`(레거시 `CLAWDHUB_DISABLE_TELEMETRY=1`)인 경우는 제외됩니다.
