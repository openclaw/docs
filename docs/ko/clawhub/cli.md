...
CLI는 기업 프록시 뒤나 제한된 네트워크에 있는 시스템을 위해 표준 HTTP 프록시 환경 변수를 따릅니다.
...
- 검색은 다운로드 인기도보다 정확한 slug/name 토큰 일치를 우선합니다. `map` 같은 독립형 slug 토큰은 `amap` 내부의 부분 문자열보다 `personal-map`에 더 강하게 일치합니다.
- 다운로드 수는 상위 배치를 보장하는 값이 아니라 작은 인기도 사전값입니다.
- 스킬이 표시되어야 하는데 표시되지 않으면 메타데이터 이름을 변경하기 전에 로그인한 상태에서 `clawhub inspect <slug>`를 실행하여 소유자에게 보이는 조정 진단을 확인합니다.
...
- 고정된 스킬은 덮어쓰기를 거부합니다. 먼저 `clawhub unpin <slug>`를 실행합니다.
- 다음을 씁니다.
...
- 설치된 스킬을 lockfile에서 pinned로 표시합니다.
- `--reason <text>`는 스킬이 고정된 이유를 기록합니다.
- 고정된 스킬은 `update --all`에서 건너뛰며, 직접 `update <slug>`로 업데이트하면 거부됩니다.
- 고정된 스킬은 로컬 바이트가 실수로 교체되지 않도록 `install --force`도 거부합니다.

### `unpin <slug>`

- 설치된 스킬에서 lockfile pin을 제거하여 향후 업데이트가 수정할 수 있게 합니다.
...
- `update <slug>`는 고정된 slug에 대해 즉시 실패하며 먼저 `clawhub unpin <slug>`을 실행하라고 안내합니다.
...
- 액터에게 publisher 접근 권한이 있으면 `--owner <handle>`이 조직/사용자 publisher handle 아래에 게시합니다.
- `--migrate-owner`는 새 버전을 게시하는 동안 기존 스킬을 `--owner`로 이동합니다. 양쪽 publisher 모두에 admin/owner 접근 권한이 필요합니다.
- 소유자 및 review 동작은 `docs/publishing.md`에 설명되어 있습니다.
- 스킬을 게시하면 ClawHub에서 `MIT-0`으로 릴리스됩니다.
- 게시된 스킬은 attribution 없이 자유롭게 사용, 수정, 재배포할 수 있습니다.
- ClawHub는 유료 스킬이나 스킬별 가격 책정을 지원하지 않습니다.
- `--clawscan-note <text>`는 ClawScan note를 추가합니다. 이 note는 네트워크 접근, 네이티브 호스트 접근 또는 provider별 자격 증명처럼 다르게 보면 비정상적으로 보일 수 있는 동작에 대한 컨텍스트를 ClawScan에 제공합니다. note는 게시된 버전에 저장됩니다.
- 레거시 별칭: `publish <path>`.
...
- 스킬을 soft-delete합니다(owner, moderator 또는 admin).
- `DELETE /api/v1/skills/{slug}`를 호출합니다.
- 소유자가 시작한 soft delete는 slug를 30일 동안 예약하며, 명령은 만료 시간을 출력합니다.
- `--reason <text>`는 스킬과 audit log에 moderation note를 기록합니다.
...
- 소유한 스킬의 이름을 바꾸고 이전 slug를 redirect alias로 유지합니다.
...
- 소스 slug는 공개 목록 표시를 중단하고 대상에 대한 redirect alias가 됩니다.
...
- 소유권 이전 workflow입니다.
- 사용자 handle로 이전하면 수신자가 수락하는 pending request가 생성됩니다.
- 조직/publisher handle로 이전하는 경우, 액터가 현재 소유자와 대상 publisher 모두에 admin 접근 권한을 가진 경우에만 즉시 적용됩니다.
- 하위 명령:
...
- Plugin 및 다른 package-family 항목에는 이것을 사용하세요. 최상위 `search`는 스킬 검색 surface로 유지됩니다.
- Flags:
...
- Plugin metadata, 호환성, 검증, source, version/file 검토에 이것을 사용하세요.
- `--version <version>`: 특정 버전을 검사합니다(기본값: latest).
- `--tag <tag>`: tagged version을 검사합니다(예: `latest`).
...
- `GET /api/v1/packages/{name}/versions/{version}/artifact`를 통해 package version을 해석합니다.
...
- `--package`와 함께 사용하면 ClawHub에서 예상 metadata를 해석하고 로컬 파일을 게시된 artifact metadata와 비교합니다.
- direct digest flag를 사용하면 network lookup 없이 검증합니다.
- Flags:
  - `--package <name>`: 예상 artifact metadata를 해석할 package name입니다.
  - `--version <version>` 또는 `--tag <tag>`: 예상 package version입니다.
...
- 패키지 소유자, 조직 게시자 소유자/관리자, 플랫폼 moderator,
  또는 플랫폼 admin이 필요합니다.
- Flags:
  - `--yes`: 확인을 건너뜁니다.
  - `--json`: 기계 판독 가능 출력입니다.
...
- 패키지를 다른 게시자로 이전합니다.
- 플랫폼 admin이 수행하는 경우가 아니라면 현재 패키지 소유자와 대상
  게시자 모두에 대한 admin 액세스가 필요합니다.
- 범위 지정 패키지 이름은 일치하는 범위 소유자에게 이전되어야 합니다.
...
- moderator에게 패키지를 신고하는 인증된 명령입니다.
- `POST /api/v1/packages/{name}/report`를 호출합니다.
- 신고는 패키지 수준이며, 선택적으로 버전에 연결될 수 있고, 검토를 위해
  moderator에게 표시됩니다.
- 신고 자체가 패키지를 자동으로 숨기거나 다운로드를 차단하지는 않습니다.
- Flags:
  - `--version <version>`: 신고에 연결할 선택적 패키지 버전입니다.
...
- 패키지 moderation 표시 여부를 확인하는 소유자 명령입니다.
- `GET /api/v1/packages/{name}/moderation`를 호출합니다.
- 현재 패키지 스캔 상태, 열린 신고 수, 최신 릴리스 수동
  moderation 상태, 다운로드 차단 상태, moderation 사유를 표시합니다.
- Flags:
  - `--json`: 기계 판독 가능 출력입니다.
...
- `<source>`는 다음을 허용합니다:
  ...
  - GitHub repo: `owner/repo` 또는 `owner/repo@ref`
...
- 코드 Plugin 폴더는 업로드 전에 ClawPack npm tarball로 패킹되어
  ...
  추출 파일 게시 경로를 사용합니다.
- GitHub 소스의 경우 소스 출처가 repo, 확인된 commit, ref, subpath에서 자동으로 채워집니다.
...
- `--dry-run`은 업로드하지 않고 확인된 게시 payload를 미리 보여줍니다.
- `--json`은 CI용 기계 판독 가능 출력을 내보냅니다.
- `--owner <handle>`은 행위자가 게시자 액세스 권한을 가진 경우 사용자 또는 조직 게시자 핸들 아래에 게시합니다.
- `--clawscan-note <text>`는 ClawScan 메모를 추가합니다. 이 메모는 ClawScan에
  네트워크 액세스, 네이티브 호스트 액세스, provider별 자격 증명처럼
  그렇지 않으면 특이하게 보일 수 있는 동작에 대한 맥락을 제공합니다. 메모는
  게시된 릴리스에 저장됩니다.
- 범위 지정 패키지 이름은 선택한 소유자와 일치해야 합니다. `docs/publishing.md`를 참조하세요.
- 기존 flags(`--family`, `--name`, `--version`, `--source-repo`, `--source-commit`, `--source-ref`, `--source-path`)는 여전히 재정의로 작동합니다.
- 비공개 GitHub repo에는 `GITHUB_TOKEN`이 필요합니다.
...
외부 코드 Plugin에는 `package.json`에 소량의 OpenClaw 메타데이터가 필요합니다. 이 최소 manifest만으로도 성공적인 게시에 충분합니다:
...
- `package.json.version`은 패키지 릴리스 버전이지만,
  OpenClaw 호환성/빌드 검증의 fallback으로 사용되지 않습니다.
...
ClawHub는 Plugin repo용 공식 재사용 가능 workflow도
...
- 재사용 가능 workflow는 `source`를 호출자 repo로 기본 설정합니다.
- monorepo의 경우 workflow가 Plugin 패키지 폴더를 게시하도록 `source_path`를 전달하세요.
  예: `source_path: extensions/codex`.
- 재사용 가능 workflow를 안정적인 tag 또는 전체 commit SHA에 pin하세요. `@main`에서 릴리스 게시를 실행하지 마세요.
- `pull_request`는 CI가 오염되지 않도록 `dry_run: true`를 사용해야 합니다.
- 실제 게시는 `workflow_dispatch` 또는 tag push 같은 신뢰할 수 있는 이벤트로 제한해야 합니다.
- secret 없는 신뢰할 수 있는 게시는 `workflow_dispatch`에서만 작동합니다. tag push에는 여전히 `clawhub_token`이 필요합니다.
- 최초 게시, 신뢰할 수 없는 패키지, 또는 긴급 게시를 위해 `clawhub_token`을 사용할 수 있게 유지하세요.
- workflow는 JSON 결과를 아티팩트로 업로드하고 workflow 출력으로 노출합니다.

### `sync`

- 로컬 skill 폴더를 스캔하고 새 항목/변경된 항목을 게시합니다.
- 루트는 skills 디렉터리 또는 `SKILL.md`가 있는 단일 skill 폴더 등 어떤 폴더든 될 수 있습니다.
- `~/.clawdbot/clawdbot.json`이 있으면 Clawdbot skill 루트를 자동으로 추가합니다:
  - `agent.workspace/skills`(메인 에이전트)
  - `routing.agents.*.workspace/skills`(에이전트별)
  - `~/.clawdbot/skills`(공유)
  - `skills.load.extraDirs`(공유 pack)
- `CLAWDBOT_CONFIG_PATH` / `CLAWDBOT_STATE_DIR` 및 `OPENCLAW_CONFIG_PATH` / `OPENCLAW_STATE_DIR`를 준수합니다.
- Flags:
  - `--root <dir...>` 추가 스캔 루트
  - `--all` 프롬프트 없이 업로드
  - `--dry-run` 계획만 표시
...
- 로그인된 상태에서는 `sync` 중 전송됩니다. 단, `CLAWHUB_DISABLE_TELEMETRY=1`(legacy `CLAWDHUB_DISABLE_TELEMETRY=1`)인 경우는 제외합니다.
