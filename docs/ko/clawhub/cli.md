...
read_when:
    - ClawHub CLI 사용하기
    - 설치, 업데이트 또는 게시 디버깅
summary: 'CLI 참조: 명령, 플래그, 구성, lockfile 동작.'
x-i18n:
...
- `--registry <url>`: API 기본 URL(기본값: 탐색된 값, 없으면 `https://clawhub.ai`)
- `--no-input`: 프롬프트 비활성화
...
CLI는 기업 프록시 또는 제한된 네트워크 뒤에 있는 시스템을 위해 표준 HTTP 프록시 환경 변수를 따릅니다.

- `HTTPS_PROXY` / `https_proxy`
- `HTTP_PROXY` / `http_proxy`
- `NO_PROXY` / `no_proxy`

이러한 변수 중 하나라도 설정되어 있으면 CLI는 지정된 프록시를 통해 아웃바운드 요청을 라우팅합니다. `HTTPS_PROXY`는 HTTPS 요청에 사용되고, `HTTP_PROXY`는 일반 HTTP에 사용됩니다. `NO_PROXY` / `no_proxy`는 특정 호스트 또는 도메인에 대해 프록시를 우회하도록 적용됩니다.

이는 직접 아웃바운드 연결이 차단된 시스템(예: Docker 컨테이너, 프록시 전용 인터넷을 사용하는 Hetzner VPS, 기업 방화벽)에서 필요합니다.
...
- 레거시 대체 경로: `clawhub/config.json`이 아직 없지만 `clawdhub/config.json`이 있으면 CLI는 레거시 경로를 재사용합니다.
...
- 로컬 로그인 토큰을 CI 비밀 설정 명령으로 파이프하는 데 유용합니다.
...
- 출력에는 스킬 슬러그, 소유자 핸들, 표시 이름, 관련도 점수가 포함됩니다.
...
- 표시되어야 하는 스킬이 표시되지 않으면 메타데이터 이름을 바꾸기 전에 로그인한 상태에서 `clawhub inspect @owner/slug`를 실행해 소유자에게 보이는 조정 진단을 확인하세요.
...
- 지정된 소유자와 스킬의 최신 버전을 해석합니다.
- `/api/v1/download`를 통해 zip을 다운로드합니다.
- `<workdir>/<dir>/<slug>`로 추출합니다.
- 고정된 스킬은 덮어쓰기를 거부합니다. 먼저 `clawhub unpin <skill>`을 실행하세요.
- 다음을 작성합니다.
...
- 로그인되어 있으면 현재 설치 수를 비활성화할 수 있도록 최선 노력 방식의 텔레메트리를 전송합니다.
- 대화형: 확인을 요청합니다.
...
- 선택적 이유를 포함해 `clawhub pin`으로 고정된 스킬 옆에 `pinned`를 표시합니다.

### `pin <skill>`

- 설치된 스킬을 lockfile에서 고정된 것으로 표시합니다.
- `--reason <text>`는 스킬이 동결된 이유를 기록합니다.
- 고정된 스킬은 `update --all`에서 건너뛰며 직접 `update <skill>`에서는 거부됩니다.
...
- 새 스킬은 기본적으로 `1.0.0`이 되며, 변경된 스킬은 기본적으로 다음 패치 버전이 됩니다.
- `--version <version>`은 버전을 명시적으로 선택하고 콘텐츠가 기존 버전과 일치하더라도 게시합니다.
- `--dry-run`은 업로드하지 않고 게시를 해석합니다. `--json`은 기계가 읽을 수 있는 결과를 출력합니다.
- `--owner <handle>`은 행위자에게 게시자 권한이 있을 때 조직/사용자 게시자 핸들 아래에 게시합니다.
- `--migrate-owner`는 새 버전을 게시하면서 기존 스킬을 `--owner`로 이동합니다. 두 게시자 모두에 대한 관리자/소유자 권한이 필요합니다.
...
워크플로는 하나의 `skill_path`에 대해, 또는 `root` 아래의 각 즉시 하위 스킬 폴더(기본값: `skills`)에 대해 `skill publish`를 호출합니다. 변경되지 않은 스킬은 건너뛰며 동일한 자동 패치 버전 동작을 사용합니다.

토큰 없이 미리 보려면 `dry_run: true`를 설정하세요. 실제 게시에는 `clawhub_token` 비밀이 필요합니다.
...
- 새 스킬은 `1.0.0`으로 게시됩니다. 변경된 스킬은 기본적으로 다음 패치 버전을 게시합니다. 더 큰 semver 단계로 이동해야 하는 업데이트 배치에는 `--bump minor|major`를 사용하세요.
- `--dry-run`은 업로드하지 않고 게시 계획을 보여줍니다. `--json`은 기계가 읽을 수 있는 계획을 출력합니다.
- `--all`은 모든 새 스킬 또는 변경된 스킬을 프롬프트 없이 게시합니다. `--all`이 없으면 대화형 터미널에서 게시할 스킬을 선택할 수 있습니다.
- `--owner <handle>`은 행위자에게 게시자 권한이 있을 때 조직/사용자 게시자 핸들 아래에 게시합니다.
- `sync`는 단방향 게시 전용입니다. 설치, 업데이트, 다운로드, 설치/다운로드 텔레메트리 보고를 하지 않습니다.
...
- `POST /api/v1/skills/-/scan`을 통해 ClawHub ClawScan을 실행한 다음 스캔이 최종 상태가 될 때까지 폴링합니다.
- 스캔은 비동기이며 완료하는 데 시간이 걸릴 수 있습니다. 대기열에 있는 동안 터미널 스피너는 현재 우선순위 스캔 위치와 앞에 있는 스캔 수를 표시합니다.
- 게시된 스캔에는 소유권 또는 게시자 관리 권한이 필요합니다. 조정자/관리자는 `clawhub-admin`을 통해 동일한 백엔드를 사용할 수 있습니다.
...
ClawHub는 스킬 저장소와 카탈로그 저장소를 위해 다음 위치에 공식 재사용 가능 워크플로를 제공합니다.
...
- `root`의 기본값은 카탈로그 저장소의 경우 `skills`입니다.
...
- `--version` 없이 skill을 소프트 삭제합니다(소유자, 모더레이터 또는 관리자).
...
- `--version <version>`은 fail-closed 방식의 버전별 라우트를 통해 소유한 최신이 아닌 버전 하나를 영구 삭제합니다.
  삭제된 버전은 복원하거나 다시 게시할 수 없습니다. 현재 최신 버전을 삭제하기 전에 대체 버전을 게시하세요. 플랫폼 스태프는 이 버전 전용 플로에서 소유권을 우회하지 않습니다.
- `--reason <text>`는 전체 skill 소프트 삭제 및 감사 로그에 모더레이션 메모를 기록합니다.
- `--note <text>`는 `--reason`의 별칭입니다.
- `--yes`는 확인을 건너뜁니다.

### `undelete <skill>`

- 숨겨진 skill을 복원합니다(소유자, 모더레이터 또는 관리자).
- 버전 삭제 취소는 없습니다. 영구 삭제된 버전은 복원할 수 없습니다.
- `POST /api/v1/skills/{slug}/undelete`를 호출합니다.
- `--reason <text>`는 skill 및 감사 로그에 모더레이션 메모를 기록합니다.
- `--note <text>`는 `--reason`의 별칭입니다.
- `--yes`는 확인을 건너뜁니다.

### `hide <skill>`

- skill을 숨깁니다(소유자, 모더레이터 또는 관리자).
- `delete`의 별칭입니다.

### `unhide <skill>`

- skill 숨김을 해제합니다(소유자, 모더레이터 또는 관리자).
- `undelete`의 별칭입니다.

### `skill rename <skill> <new-name>`

- 소유한 skill의 이름을 바꾸고 이전 슬러그를 리디렉션 별칭으로 유지합니다.
...
- 원본 슬러그는 공개 목록에 표시되지 않고 대상에 대한 리디렉션 별칭이 됩니다.
...
- 소유권 이전 워크플로입니다.
- 사용자 핸들로 이전하면 수신자가 수락하는 대기 중인 요청이 생성됩니다.
- 조직/게시자 핸들로 이전하는 경우 행위자가 현재 소유자와 대상 게시자 모두에 대한 관리자 권한을 가지고 있을 때만 즉시 적용됩니다.
- 하위 명령:
...
- Plugin 및 기타 패키지 계열 항목에는 이것을 사용하세요. 최상위 `search`는 skill 검색 표면으로 남습니다.
- 플래그:
...
- Plugin 메타데이터, 호환성, 검증, 소스, 버전/파일 검사에 사용하세요.
...
- `GET /api/v1/packages/{name}/versions/{version}/artifact`를 통해 패키지 버전을 해석합니다.
- 해석기의 `downloadUrl`에서 아티팩트를 다운로드합니다.
...
- 기존 ZIP 버전은 기존 ZIP 라우트를 통해 다운로드됩니다.
- 플래그:
...
- `--package`를 사용하면 ClawHub에서 예상 메타데이터를 해석하고 로컬 파일을 게시된 아티팩트 메타데이터와 비교합니다.
- 직접 다이제스트 플래그를 사용하면 네트워크 조회 없이 검증합니다.
- 플래그:
  - `--package <name>`: 예상 아티팩트 메타데이터를 해석할 패키지 이름입니다.
...
- 로컬 Plugin 패키지 폴더에 대해 ClawHub CLI에 번들된 Plugin Inspector를 실행합니다.
- 로컬 OpenClaw 체크아웃을 찾거나 가져오지 않고 오프라인/정적 검증이 기본값입니다.
- 심각한 호환성 오류는 0이 아닌 값으로 종료합니다. 경고만 있는 결과는 출력되지만 0으로 종료합니다.
- 플래그:
  - `--out <dir>`: Plugin Inspector 보고서를 이 디렉터리에 씁니다.
  - `--openclaw <path>`: 명시적인 로컬 OpenClaw 체크아웃을 대상으로 검사합니다.
  - `--runtime`: 런타임 캡처를 활성화합니다. Plugin 코드를 가져옵니다.
  - `--allow-execute`: 격리된 작업 공간에서 런타임 캡처를 허용합니다.
  - `--no-mock-sdk`: 런타임 캡처 중 모킹된 OpenClaw SDK를 비활성화합니다.
  - `--json`: 기계 판독 가능 출력입니다.
...
검증에서 패키지, 매니페스트, SDK 가져오기 또는 아티팩트 결과가 보고되면 [Plugin 검증 수정](/ko/clawhub/plugin-validation-fixes)을 확인한 다음 명령을 다시 실행하세요.

### `package delete <name>`

- `--version` 없이 패키지와 모든 릴리스를 소프트 삭제합니다.
- `--version <version>`은 fail-closed 방식의 버전별 라우트를 통해 소유한 최신이 아닌 릴리스 하나를 영구 삭제합니다.
  삭제된 버전은 복원하거나 다시 게시할 수 없습니다. 현재 최신 버전을 삭제하기 전에 대체 버전을 게시하세요. 이 버전 전용 플로에는 패키지 소유자 또는 조직 게시자 관리자가 필요합니다. 플랫폼 스태프는 패키지 소유권을 우회하지 않습니다.
- 전체 패키지 소프트 삭제에는 패키지 소유자, 조직 게시자 소유자/관리자, 플랫폼 모더레이터 또는 플랫폼 관리자가 필요합니다.
- 플래그:
...
- 버전 삭제 취소는 없습니다. 영구 삭제된 버전은 복원할 수 없습니다.
- 패키지 소유자, 조직 게시자 소유자/관리자, 플랫폼 모더레이터 또는 플랫폼 관리자가 필요합니다.
...
- 패키지를 다른 게시자로 이전합니다.
- 플랫폼 관리자가 수행하는 경우를 제외하고, 현재 패키지 소유자와 대상 게시자 모두에 대한 관리자 권한이 필요합니다.
...
- 공식 상태, ClawPack 가용성, 아티팩트 다이제스트, 소스 출처, OpenClaw 호환성, 호스트 대상, 환경 메타데이터, 스캔 상태에 대한 차단 요소를 보고합니다.
- 플래그:
  - `--json`: 기계 판독 가능 출력입니다.
...
- 번들된 OpenClaw Plugin을 대체할 수 있는 패키지에 대해 운영자 지향 마이그레이션 상태를 표시합니다.
- `package readiness`와 동일한 계산된 준비 상태 엔드포인트를 호출하지만, 마이그레이션 중심 상태, 최신 버전, 공식 패키지 상태, 검사 및 차단 요소를 출력합니다.
- 플래그:
  - `--json`: 기계 판독 가능 출력입니다.
...
- 핸들이 기존 게시자, 사용자 또는 예약된 라우트에서 이미 사용 중이면 실패합니다.
...
- 코드 Plugin 폴더는 업로드 전에 ClawPack npm tarball로 패키징되어
  ...
  추출된 파일 게시 경로를 사용합니다.
- GitHub 소스의 경우, 소스 출처가 저장소, 해석된 커밋, ref, 하위 경로에서 자동으로 채워집니다.
...
외부 코드 Plugin은 `package.json`에 소량의 OpenClaw 메타데이터가 필요합니다. 이 최소 매니페스트만으로도 게시에 성공할 수 있습니다.
...
- `package.json.version`은 패키지 릴리스 버전이지만, OpenClaw 호환성/빌드 검증의 대체값으로 사용되지 않습니다.
- `openclaw.hostTargets` 및 `openclaw.environment`는 선택적 메타데이터입니다.
  ClawHub는 이들이 있으면 표시할 수 있지만, 게시에 필수는 아닙니다.
- `openclaw.compat.minGatewayVersion` 및
  `openclaw.build.pluginSdkVersion`은 더 자세한 호환성 메타데이터를 게시하려는 경우의 선택적 추가 항목입니다.
- 이전 `clawhub` CLI 릴리스를 사용 중이라면, 로컬 사전 점검이 업로드 전에 실행되도록 게시 전에 업그레이드하세요.
- 검증에서 해결 코드가 보고되면
...
[`/.github/workflows/package-publish.yml`](https://github.com/openclaw/clawhub/blob/bdb23c3a9ffe77cb4184fdd13897ce535fb2d703/.github/workflows/package-publish.yml)에 제공합니다.
...
- secret 없이 신뢰할 수 있는 게시는 `workflow_dispatch`에서만 동작합니다. 태그 푸시에는 여전히 `clawhub_token`이 필요합니다.
...
- 구성이 설정된 뒤에는 향후 지원되는 GitHub Actions 게시에서
...
- 플래그:
  ...
  - `--environment <name>`: 선택적 정확히 일치하는 GitHub Actions 환경입니다.
  - `--json`: 기계 판독 가능 출력입니다.
...
- 워크플로, 저장소, 또는 환경 고정을 비활성화하거나 다시 만들어야 할 때 롤백으로 사용하세요.
...
- 로그인한 상태에서 `clawhub install <slug>` 이후 전송됩니다. 단,
  `CLAWHUB_DISABLE_TELEMETRY=1`이 설정된 경우는 제외됩니다.
- 보고는 최선 노력 방식입니다. 텔레메트리를 사용할 수 없어도 설치 명령은 실패하지 않습니다.
