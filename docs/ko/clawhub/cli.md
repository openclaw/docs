...
read_when:
    - ClawHub CLI 사용하기
    - 설치, 업데이트 또는 게시 디버깅
summary: 'CLI 참조: 명령, 플래그, 구성 및 락파일 동작.'
x-i18n:
    generated_at: "2026-06-30T13:52:28Z"
    model: gpt-5.5
...
- `--workdir <dir>`: 작업 디렉터리(기본값: cwd, 구성된 경우 Clawdbot 작업공간으로 대체)
...
- 기본값: 브라우저를 `<site>/cli/auth`로 열고 루프백 콜백을 통해 완료합니다.
- 헤드리스: `clawhub login --token clh_...`
- 원격/헤드리스 대화형: `clawhub login --device`가 코드를 출력하고 `<site>/cli/device`에서 승인하는 동안 기다립니다.
...
- 검색은 다운로드 인기도보다 정확한 슬러그/이름 토큰 일치를 우선합니다. `map` 같은 독립 슬러그 토큰은 `amap` 내부의 부분 문자열보다 `personal-map`과 더 강하게 일치합니다.
- 인기도는 작은 순위 사전값일 뿐, 최상위 배치를 보장하지 않습니다.
- 표시되어야 하는 스킬이 표시되지 않으면 메타데이터 이름을 변경하기 전에 로그인한 상태에서 `clawhub inspect @owner/slug`를 실행하여 소유자에게 표시되는 검토 진단을 확인합니다.
...
- 고정된 스킬은 덮어쓰기를 거부합니다. 먼저 `clawhub unpin <skill>`을 실행합니다.
- 다음을 씁니다:
...
- 현재 설치 수가 비활성화될 수 있도록 로그인한 상태에서 최선 노력 방식으로 텔레메트리를 보냅니다.
- 대화형: 확인을 요청합니다.
...
- `clawhub pin`으로 동결된 스킬 옆에 선택적 이유와 함께 `pinned`를 표시합니다.

### `pin <skill>`

- 설치된 스킬을 lockfile에서 고정으로 표시합니다.
...
- 설치된 스킬에서 lockfile 고정을 제거하여 이후 업데이트가 수정할 수 있게 합니다.
...
- `update <skill>`은 고정된 스킬에 대해 즉시 실패하고 먼저 `clawhub unpin <skill>`을 실행하라고 안내합니다.
- `update --all`은 고정된 슬러그를 건너뛰고 동결 상태로 남은 항목의 요약을 출력합니다.

### `skill publish <path>`

- 로컬 번들 지문을 ClawHub와 비교하고, 콘텐츠가 이미 게시되어 있으면 성공적으로 종료합니다.
- 새 스킬의 기본값은 `1.0.0`이고, 변경된 스킬의 기본값은 다음 패치 버전입니다.
...
- 스킬을 게시한다는 것은 ClawHub에서 `MIT-0`으로 릴리스된다는 뜻입니다.
...
워크플로는 하나의 `skill_path`에 대해, 또는 `root` 아래의 각 즉시 하위 스킬 폴더에 대해 `skill publish`를 호출합니다(기본값: `skills`). 변경되지 않은 스킬은 건너뛰고 동일한 자동 패치 버전 동작을 사용합니다.

토큰 없이 미리 보려면 `dry_run: true`를 설정합니다. 실제 게시는 `clawhub_token` 시크릿이 필요합니다.

### `sync`

- 현재 workdir, 구성된 스킬 디렉터리, 그리고 `SKILL.md` 또는 `skill.md`를 포함하는 로컬 스킬 폴더가 있는 모든 `--root <dir>` 폴더를 스캔합니다.
- 각 로컬 스킬 지문을 ClawHub와 비교하고 새 스킬 또는 변경된 스킬만 게시합니다.
- 새 스킬은 `1.0.0`으로 게시되고, 변경된 스킬은 기본적으로 다음 패치 버전으로 게시됩니다. 더 큰 semver 단계로 이동해야 하는 업데이트 배치에는 `--bump minor|major`를 사용합니다.
...
- `--owner <handle>`은 행위자에게 게시자 접근 권한이 있을 때 조직/사용자 게시자 핸들 아래에 게시합니다.
- `sync`는 단방향 게시만 수행합니다. 설치, 업데이트, 다운로드 또는 설치/다운로드 텔레메트리 보고는 하지 않습니다.
...
- 스캔은 비동기이며 완료하는 데 시간이 걸릴 수 있습니다. 대기열에 있는 동안 터미널 스피너는 현재 우선순위 스캔 위치와 앞에 있는 스캔 수를 표시합니다.
- 게시된 스캔에는 소유권 또는 게시자 관리 접근 권한이 필요합니다. 운영자/관리자는 `clawhub-admin`을 통해 동일한 백엔드를 사용할 수 있습니다.
- `--update`는 `--slug`와 함께 사용할 때만 유효하며, 성공한 게시 스캔 결과를 선택한 버전에 다시 씁니다.
...
- 로컬 경로 스캔은 더 이상 지원되지 않습니다. 새 버전을 업로드한 다음 `scan download`를 사용하여 제출된 해당 버전에 저장된 스캔 결과를 가져오세요.
...
- `clawhub login`이 필요합니다.
- ClawHub 보안 검사로 차단되거나 숨겨진 버전을 포함하여 제출된 스킬 또는 Plugin 버전에 대해 저장된 스캔 보고서 ZIP을 다운로드합니다.
...
- V1 스킬 게시는 `clawhub_token`을 사용합니다. GitHub OIDC 신뢰 게시 기능은 현재 패키지 전용입니다.

### `delete <skill>`

- `--version` 없이 skill을 소프트 삭제합니다(소유자, 중재자 또는 관리자).
...
- `--reason <text>`는 전체 skill 소프트 삭제와 감사 로그에 중재 메모를 기록합니다.
...
- `--reason <text>`는 skill과 감사 로그에 중재 메모를 기록합니다.
- `--note <text>`는 `--reason`의 alias입니다.
- `--yes`는 확인을 건너뜁니다.
...
- 소유한 skill의 이름을 바꾸고 이전 slug를 리디렉션 alias로 유지합니다.
...
- source slug는 공개 목록 표시를 중단하고 대상에 대한 리디렉션 alias가 됩니다.
...
- 소유권 이전 workflow입니다.
- 사용자 handle로 이전하면 수신자가 수락하는 대기 중인 요청이 생성됩니다.
- org/publisher handle로 이전하는 경우 행위자가 현재 소유자와 대상 publisher 모두에 대한 admin access를 가진 경우에만 즉시 적용됩니다.
- 하위 명령:
...
- plugins 및 기타 package-family 항목에는 이것을 사용하세요. 최상위 `search`는 skill 검색 surface로 유지됩니다.
- 플래그:
...
- `--version <version>`: 특정 버전을 검사합니다(기본값: latest).
- `--tag <tag>`: 태그가 지정된 버전을 검사합니다(예: `latest`).
- `--versions`: 버전 history를 나열합니다(첫 페이지).
- `--limit <n>`: 나열할 최대 버전 수(1-100).
- `--files`: 선택한 버전의 파일을 나열합니다.
- `--file <path>`: 원시 파일 content를 가져옵니다(text 파일만, 200KB 제한).
- `--json`: machine-readable 출력.

### `package download <name>`

- `GET /api/v1/packages/{name}/versions/{version}/artifact`를 통해 package version을 확인합니다.
- resolver의 `downloadUrl`에서 artifact를 다운로드합니다.
- 모든 artifact에 대해 ClawHub SHA-256을 검증합니다.
- ClawPack npm-pack artifact의 경우 npm `sha512` integrity, npm shasum, tarball의 `package.json` name/version도 검증합니다.
- Legacy ZIP 버전은 legacy ZIP route를 통해 다운로드됩니다.
- 플래그:
  - `--version <version>`: 특정 버전을 다운로드합니다.
  - `--tag <tag>`: 태그가 지정된 버전을 다운로드합니다(기본값: `latest`).
  - `-o, --output <path>`: 출력 파일 또는 directory.
...
- `--package`를 사용하면 ClawHub에서 expected metadata를 확인하고 로컬 파일을 게시된 artifact metadata와 비교합니다.
- 직접 digest flags를 사용하면 network lookup 없이 검증합니다.
- 플래그:
  - `--package <name>`: expected artifact metadata를 확인할 package name.
...
- 로컬 Plugin package 폴더에 대해 ClawHub CLI에 bundled된 Plugin Inspector를 실행합니다.
- 로컬 OpenClaw checkout을 찾거나 가져오지 않고 offline/static validation을 기본으로 사용합니다.
- Hard compatibility 오류는 0이 아닌 코드로 종료합니다. Warning-only findings는 출력되지만 0으로 종료합니다.
- 플래그:
  - `--out <dir>`: Plugin Inspector report를 이 directory에 작성합니다.
  - `--openclaw <path>`: 명시적 로컬 OpenClaw checkout에 대해 검사합니다.
  - `--runtime`: runtime capture를 활성화합니다. Plugin code를 가져옵니다.
...
validation이 package, manifest, SDK import 또는 artifact finding을 보고하면
[Plugin validation fixes](/clawhub/plugin-validation-fixes)를 참조한 다음 명령을 다시 실행하세요.

### `package delete <name>`

- `--version` 없이 package와 모든 release를 소프트 삭제합니다.
- `--version <version>`은 fail-closed 방식의 버전별 route를 통해 소유한 최신이 아닌 release 하나를 영구 삭제합니다.
  삭제된 버전은 복원하거나 다시 게시할 수 없습니다. 현재 최신 버전을 삭제하기 전에 대체 버전을 게시하세요. 이 버전 전용 flow에는 package owner 또는 org publisher admin이 필요하며, 플랫폼 staff는 package ownership을 우회하지 않습니다.
- 전체 package 소프트 삭제에는 package owner, org publisher owner/admin, platform moderator 또는 platform admin이 필요합니다.
- 플래그:
...
- 버전 undelete는 없습니다. 영구 삭제된 버전은 복원할 수 없습니다.
- package owner, org publisher owner/admin, platform moderator 또는 platform admin이 필요합니다.
...
- platform admin이 수행하는 경우가 아니면 현재 package owner와 대상 publisher 모두에 대한 admin access가 필요합니다.
- Scoped package names는 일치하는 scope owner로 이전해야 합니다.
- `POST /api/v1/packages/{name}/transfer`를 호출합니다.
- 플래그:
  - `--to <owner>`: 대상 publisher handle.
...
- package를 moderator에게 신고하기 위한 인증된 명령입니다.
- `POST /api/v1/packages/{name}/report`를 호출합니다.
- 신고는 package-level이며, 선택적으로 버전에 연결되고, 검토를 위해 moderator에게 표시됩니다.
- 신고 자체만으로는 package를 자동으로 숨기거나 download를 차단하지 않습니다.
- 플래그:
...
- package moderation visibility를 확인하기 위한 owner 명령입니다.
- `GET /api/v1/packages/{name}/moderation`을 호출합니다.
- 현재 package scan state, open report count, latest release manual moderation state, download block state, moderation reasons를 표시합니다.
- 플래그:
  - `--json`: machine-readable 출력.
...
- official status, ClawPack availability, artifact digest, source provenance, OpenClaw compatibility, host targets, environment metadata, scan state에 대한 blocker를 보고합니다.
- 플래그:
  - `--json`: machine-readable 출력.
...
- bundled OpenClaw plugin을 대체할 수 있는 package에 대해 operator-oriented migration status를 표시합니다.
- `package readiness`와 동일한 계산된 readiness endpoint를 호출하지만, migration-focused status, latest version, official-package state, checks, blockers를 출력합니다.
- 플래그:
  - `--json`: machine-readable 출력.
...
- 인증된 사용자가 소유하는 org publisher를 생성합니다.
- handle은 lowercase로 정규화되며 `@`를 포함하거나 포함하지 않고 전달할 수 있습니다.
...
- 코드 Plugin 폴더는 업로드 전에 ClawPack npm tarball로 패키징되어
  ...
  추출 파일 게시 경로를 사용합니다.
- GitHub 소스의 경우 저장소, 확인된 커밋, ref, 하위 경로에서 출처 표시가 자동으로 채워집니다.
- 로컬 폴더의 경우 origin 원격이 GitHub를 가리키면 로컬 git에서 출처 표시가 자동 감지됩니다.
- 외부 코드 Plugin은 `openclaw.compat.pluginApi`와
  `openclaw.build.openclawVersion`을 명시적으로 선언해야 합니다.
  최상위 `package.json.version`은 게시 유효성 검사의 fallback으로 사용되지 않습니다.
...
- scoped 패키지 이름은 선택한 소유자와 일치해야 합니다. `docs/publishing.md`를 참조하세요.
...
출처 표시를 확인할 수 있도록 먼저 `--dry-run`을 사용하세요:
...
필요합니다. 이 최소 manifest만으로도 성공적으로 게시할 수 있습니다:
...
- `package.json.version`은 패키지 릴리스 버전이지만 OpenClaw 호환성/빌드 유효성 검사의
  fallback으로 사용되지 않습니다.
- `openclaw.hostTargets`와 `openclaw.environment`는 선택적 메타데이터입니다.
  ClawHub는 존재할 때 이를 표시할 수 있지만, 게시에 필수는 아닙니다.
- 더 자세한 호환성 메타데이터를 게시하려는 경우 `openclaw.compat.minGatewayVersion`과
  `openclaw.build.pluginSdkVersion`은 선택적 추가 항목입니다.
- 이전 `clawhub` CLI 릴리스를 사용 중이라면 업로드 전에
  로컬 preflight 검사가 실행되도록 게시 전에 업그레이드하세요.
- 유효성 검사가 수정 코드를 보고하면
  [Plugin 유효성 검사 수정](/clawhub/plugin-validation-fixes)을 참조하세요.

#### GitHub Actions

ClawHub는 Plugin 저장소용 공식 재사용 가능 workflow도
[`/.github/workflows/package-publish.yml`](https://github.com/openclaw/clawhub/blob/919f047373fb1836301c5e42f20ad8c2c2201fc5/.github/workflows/package-publish.yml)에 제공합니다.
...
- 모노레포의 경우 workflow가 Plugin 패키지 폴더를 게시하도록
  `source_path`를 전달하세요. 예: `source_path: extensions/codex`.
- 재사용 가능 workflow는 안정적인 태그 또는 전체 커밋 SHA에 고정하세요. `@main`에서 릴리스 게시를 실행하지 마세요.
...
- 패키지의 GitHub Actions 신뢰할 수 있는 게시자 설정을 표시합니다.
- 설정을 지정한 후 저장소, workflow 파일 이름,
...
- 패키지에서 신뢰할 수 있는 게시자 설정을 제거합니다.
- workflow, 저장소 또는 환경 고정을 비활성화하거나 다시 만들어야 할 때 rollback으로 사용하세요.
- 향후 실제 게시는 설정이 다시 지정될 때까지 일반 인증 게시를 사용해야 합니다.
- 플래그:
  - `--json`: 기계 판독 가능 출력.
...
- 로그인된 상태에서 `clawhub install <slug>` 후 전송됩니다. 단,
  `CLAWHUB_DISABLE_TELEMETRY=1`이 설정된 경우는 제외됩니다.
- 보고는 최선 노력 방식입니다. telemetry를 사용할 수 없어도 설치 명령은
  실패하지 않습니다.
