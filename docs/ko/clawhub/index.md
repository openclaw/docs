...
title: ClawHub
x-i18n:
    generated_at: "2026-07-05T04:54:17Z"
    model: gpt-5.5
...
별표, 보안 스캔 요약을 추적합니다. 공개 페이지에는 현재 레지스트리
상태가 표시되므로 사용자는 설치 전에 Skill 또는 Plugin을 검토할 수 있습니다.

## 네이티브 OpenClaw 흐름

네이티브 OpenClaw 명령은 활성 OpenClaw 작업 공간에 설치하고 소스
메타데이터를 유지하여 이후 업데이트 명령이 ClawHub에 계속 머무를 수 있게 합니다.

Plugin 설치가 ClawHub를 통해 확인되어야 할 때는 `clawhub:<package>`를 사용합니다.
출시 전환 기간에는 npm에 안전한 원시 Plugin 사양이 npm을 통해 확인될 수 있으며,
