...
title: ClawHub
x-i18n:
    generated_at: "2026-07-02T08:04:00Z"
    model: gpt-5.5
...
ClawHub는 semver 버전, `latest` 같은 태그, 변경 로그, 파일, 다운로드, 별표, 보안 스캔 요약을 추적합니다. 공개 페이지에는 현재 레지스트리 상태가 표시되어 사용자가 설치 전에 Skill이나 Plugin을 확인할 수 있습니다.

## 기본 OpenClaw 흐름

기본 OpenClaw 명령은 활성 OpenClaw 작업 공간에 설치하고 소스 메타데이터를 보존하므로 이후 업데이트 명령이 ClawHub에 계속 머무를 수 있습니다.
...
CLI에는 직접 레지스트리 워크플로용 Skill 설치/업데이트 명령도 있습니다.
...
ClawHub는 게시된 Skills와 Plugin 릴리스에 자동 검사를 실행합니다. 스캔 보류 또는 차단된 릴리스는 공개 카탈로그와 설치 표면에서 사라질 수 있지만, `/dashboard`에서는 소유자에게 계속 표시됩니다.

로그인한 사용자는 Skills와 패키지를 신고할 수 있습니다. 관리자는 신고를 검토하고, 콘텐츠를 숨기거나 복원하며, 악용 계정을 차단할 수 있습니다. 정책 및 집행 세부 정보는 [보안](/ko/clawhub/security), [보안 감사](/clawhub/security-audits), [관리 및 계정 안전](/clawhub/moderation), [허용되는 사용](/ko/clawhub/acceptable-usage)을 참조하세요.

## 텔레메트리 및 환경

로그인한 상태에서 `clawhub install`을 실행하면 CLI는 ClawHub가 집계 설치 수를 계산할 수 있도록 최선의 노력으로 설치 이벤트를 보낼 수 있습니다. 다음으로 비활성화합니다.
