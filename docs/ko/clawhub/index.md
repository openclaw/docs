...
title: ClawHub
x-i18n:
    generated_at: "2026-07-03T17:16:28Z"
    model: gpt-5.5
...
다운로드, 스타, 보안 스캔 요약을 추적합니다. 공개 페이지에는 현재 레지스트리
상태가 표시되므로 사용자는 Skills나 Plugin을 설치하기 전에 검토할 수 있습니다.

## 기본 OpenClaw 흐름

기본 OpenClaw 명령은 활성 OpenClaw 워크스페이스에 설치하고 소스
메타데이터를 유지하여 이후 업데이트 명령이 ClawHub에 계속 머물 수 있게 합니다.

Plugin 설치가 ClawHub를 통해 해석되어야 할 때는 `clawhub:<package>`를 사용합니다.
순수 npm 안전 Plugin 사양은 출시 전환 중 npm을 통해 해석될 수 있으며,
...
정확히 업로드된 npm-pack `.tgz`를 선호하고, ClawHub 다이제스트 헤더와 다운로드된
바이트를 검증하며, 이후 업데이트를 위해 아티팩트 메타데이터를 기록합니다.
...
Skills 메타데이터는 [Skills 형식](/clawhub/skill-format)을 참조하세요.
...
설치 이벤트를 전송하여 ClawHub가 집계 설치 수를 계산할 수 있습니다. 다음으로 비활성화합니다.
