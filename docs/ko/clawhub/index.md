...
title: ClawHub
x-i18n:
    generated_at: "2026-07-02T22:25:58Z"
    model: gpt-5.5
...
ClawHub는 semver 버전, `latest` 같은 태그, 변경 로그, 파일, 다운로드 수, 별표, 보안 스캔 요약을 추적합니다. 공개 페이지에는 현재 레지스트리 상태가 표시되므로 사용자가 설치 전에 Skills나 Plugin을 검토할 수 있습니다.

## 기본 OpenClaw 흐름

기본 OpenClaw 명령은 활성 OpenClaw 작업 영역에 설치하고 소스 메타데이터를 유지하므로 이후 업데이트 명령이 ClawHub를 계속 사용할 수 있습니다.

Plugin 설치가 ClawHub를 통해 해석되어야 할 때는 `clawhub:<package>`를 사용합니다. 단순 npm 안전 Plugin 사양은 출시 전환 중에 npm을 통해 해석될 수 있으며, 소스를 명시해야 할 때 `npm:<package>`는 npm 전용으로 유지됩니다.
...
ClawHub는 게시된 Skills와 Plugin 릴리스에 대해 자동 검사를 실행합니다. 스캔으로 보류되었거나 차단된 릴리스는 `/dashboard`에서 소유자에게는 계속 표시되지만 공개 카탈로그 및 설치 표면에서는 사라질 수 있습니다.

로그인한 사용자는 Skills와 패키지를 신고할 수 있습니다. 모더레이터는 신고를 검토하고, 콘텐츠를 숨기거나 복원하며, 악용 계정을 차단할 수 있습니다. 정책 및 집행 세부 정보는 [보안](/ko/clawhub/security), [보안 감사](/clawhub/security-audits), [모더레이션 및 계정 안전](/clawhub/moderation), [허용 가능한 사용](/ko/clawhub/acceptable-usage)을 참조하세요.

## 텔레메트리 및 환경

로그인한 상태에서 `clawhub install`을 실행하면 CLI가 최선의 노력 방식으로 설치 이벤트를 보낼 수 있으며, 이를 통해 ClawHub가 집계 설치 수를 계산할 수 있습니다. 다음으로 비활성화합니다.
