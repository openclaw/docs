...
title: ClawHub
x-i18n:
    generated_at: "2026-05-12T04:09:45Z"
    model: gpt-5.5
    provider: openai
...
상태가 표시되어 사용자가 설치 전에 Skills나 Plugin을 검토할 수 있습니다.

## 기본 OpenClaw 흐름

기본 OpenClaw 명령은 활성 OpenClaw 작업 공간에 설치하고 소스 메타데이터를
유지하여 이후 업데이트 명령이 ClawHub를 계속 사용할 수 있도록 합니다.

Plugin 설치가 ClawHub를 통해 확인되어야 할 때는 `clawhub:<package>`를 사용합니다.
출시 전환 기간에는 npm에 안전한 기본 Plugin 지정이 npm을 통해 확인될 수 있으며,
...
또는 다운로드 전에 최신 스캔 상태가 요약됩니다.

ClawHub는 게시된 Skills와 Plugin 릴리스에 대해 자동 검사를 실행합니다. 스캔으로
보류되거나 차단된 릴리스는 공개 카탈로그와 설치 표면에서 사라질 수 있지만,
...
로그인한 상태에서 `clawhub sync`를 실행하면 CLI가 최소 스냅샷을 보내
...
더 자세한 참조 자료는 [Telemetry](/ko/clawhub/telemetry), [HTTP API](/ko/clawhub/http-api),
[Troubleshooting](/ko/clawhub/troubleshooting)을 참조하세요.
