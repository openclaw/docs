...
---
name: my-skill
description: 이 스킬이 수행하는 작업에 대한 짧은 요약입니다.
version: 1.0.0
---
```
...
선택적 환경 변수는 `metadata.openclaw.envVars` 아래에 선언하고 `required: false`를 설정하세요. 선택적 항목을 `requires.env`에 추가하지 마세요. `requires.env`는 해당 항목이 없으면 스킬을 실행할 수 없다는 의미입니다.
...
ClawHub의 보안 분석은 스킬이 선언한 내용이 실제 수행 내용과 일치하는지 확인합니다. 코드가 `TODOIST_API_KEY`를 참조하지만 프런트매터가 이를 `requires.env`, `primaryEnv` 또는 `envVars` 아래에 선언하지 않으면 분석에서 메타데이터 불일치로 표시합니다. 선언을 정확하게 유지하면 스킬이 검토를 통과하는 데 도움이 되고, 사용자가 설치하려는 항목을 이해하는 데도 도움이 됩니다.
...
- 누구나 게시된 스킬을 상업적 용도를 포함하여 사용, 수정, 재배포할 수 있습니다.
- 저작자 표시는 필요하지 않습니다.
...
- 스킬이 유료 타사 서비스와 통합되는 경우, 외부 비용과 필요한 계정을 스킬 지침 및 환경 변수 선언에 명확히 문서화하세요(필수 변수에는 `requires.env`, 선택적 변수에는 `required: false`가 있는 `envVars` 사용).
