...
x-i18n:
    generated_at: "2026-07-03T09:27:34Z"
    model: gpt-5.5
...
스킬 메타데이터는 `SKILL.md` 맨 위의 YAML frontmatter에 선언합니다. 이는 레지스트리(및 보안 분석)에 스킬 실행에 필요한 사항을 알려줍니다.
...
스킬 실행 전에 반드시 있어야 하는 환경 변수에는 `requires.env`를 사용합니다. 선택적 변수와 `required: false`를 포함한 변수별 메타데이터가 필요할 때는 `envVars`를 사용합니다.
...
스킬에 설치해야 할 종속성이 필요한 경우 `install` 배열에 선언합니다.
...
선택적 환경 변수는 `metadata.openclaw.envVars` 아래에 선언하고 `required: false`를 설정합니다. `requires.env`에는 선택적 항목을 추가하지 마세요. `requires.env`는 해당 항목 없이는 스킬을 실행할 수 없다는 뜻이기 때문입니다.
...
ClawHub의 보안 분석은 스킬이 선언한 내용이 실제 동작과 일치하는지 확인합니다. 코드가 `TODOIST_API_KEY`를 참조하지만 frontmatter가 이를 `requires.env`, `primaryEnv` 또는 `envVars` 아래에 선언하지 않으면 분석에서 메타데이터 불일치로 표시합니다. 선언을 정확하게 유지하면 스킬이 검토를 통과하는 데 도움이 되고, 사용자가 무엇을 설치하는지 이해하는 데도 도움이 됩니다.
...
- 전체 번들 크기: 50MB.
- 임베딩 텍스트에는 `SKILL.md` + 최대 약 40개의 비-`.md` 파일이 포함됩니다(최선 노력 기준 제한).
...
- 태그는 버전에 대한 문자열 포인터입니다. `latest`가 일반적으로 사용됩니다.

## 라이선스

- ClawHub에 게시된 모든 스킬은 `MIT-0` 라이선스가 적용됩니다.
- 누구나 상업적 목적을 포함하여 게시된 스킬을 사용, 수정, 재배포할 수 있습니다.
- 저작자 표기는 필요하지 않습니다.
