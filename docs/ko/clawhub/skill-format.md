...
ClawHub의 보안 분석은 Skill이 선언한 내용과 실제 동작이 일치하는지 확인합니다. 코드가 `TODOIST_API_KEY`를 참조하지만 frontmatter에서 `requires.env`, `primaryEnv`, 또는 `envVars` 아래에 선언하지 않으면 분석에서 메타데이터 불일치로 표시됩니다. 선언을 정확하게 유지하면 Skill이 검토를 통과하는 데 도움이 되고, 사용자가 무엇을 설치하는지 이해하는 데도 도움이 됩니다.
...
- `text/`로 시작하는 콘텐츠 타입은 텍스트로 처리되며, 작은 허용 목록(JSON/YAML/TOML/JS/TS/Markdown/SVG)도 추가로 적용됩니다.
...
- ClawHub에 게시된 모든 Skills는 `MIT-0` 라이선스가 적용됩니다.
...
- Skill이 유료 서드파티 서비스와 통합되는 경우, 외부 비용과 필요한 계정을 Skill 지침 및 환경 변수 선언에 명확히 문서화하세요(필수 변수는 `requires.env`, 선택적 변수는 `required: false`가 있는 `envVars`).
