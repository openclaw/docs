...
x-i18n:
    generated_at: "2026-07-04T03:41:05Z"
    model: gpt-5.5
...
스킬 메타데이터는 `SKILL.md` 상단의 YAML 프런트매터에 선언됩니다. 이는 레지스트리(및 보안 분석)에 스킬 실행에 필요한 항목을 알려줍니다.
...
ClawHub의 보안 분석은 스킬이 선언한 내용이 실제 동작과 일치하는지 확인합니다. 코드가 `TODOIST_API_KEY`를 참조하지만 프런트매터에서 이를 `requires.env`, `primaryEnv`, 또는 `envVars` 아래에 선언하지 않으면, 분석에서 메타데이터 불일치로 표시됩니다. 선언을 정확하게 유지하면 스킬이 검토를 통과하는 데 도움이 되고, 사용자가 무엇을 설치하는지 이해하는 데 도움이 됩니다.
