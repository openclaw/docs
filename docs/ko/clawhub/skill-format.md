...
x-i18n:
    generated_at: "2026-07-02T14:00:21Z"
    model: gpt-5.5
...
보관/비활성화된 저장소, 또는 제3자 공개 저장소는 가져오지 않습니다.
...
스킬 메타데이터는 `SKILL.md` 상단의 YAML frontmatter에 선언합니다. 이는 레지스트리(및 보안 분석)에 스킬 실행에 필요한 항목을 알려 줍니다.
...
| `always`           | `boolean`  | `true`이면 스킬이 항상 활성화됩니다(명시적 설치 불필요).                                                                                    |
...
| `nix`              | `object`   | Nix 플러그인 사양입니다(README 참조).                                                                                                        |
| `config`           | `object`   | Clawdbot 구성 사양입니다(README 참조).                                                                                                       |

### 설치 사양

스킬에 설치해야 할 의존성이 필요하면 `install` 배열에 선언합니다.
...
ClawHub의 보안 분석은 스킬이 선언한 내용이 실제 동작과 일치하는지 확인합니다. 코드가 `TODOIST_API_KEY`를 참조하지만 frontmatter에서 `requires.env`, `primaryEnv`, 또는 `envVars` 아래에 선언하지 않으면 분석이 메타데이터 불일치로 표시합니다. 선언을 정확하게 유지하면 스킬이 검토를 통과하는 데 도움이 되고, 사용자가 설치하는 항목을 이해하는 데도 도움이 됩니다.
...
- 전체 번들 크기: 50MB.
- 임베딩 텍스트에는 `SKILL.md` + 최대 약 40개의 비-`.md` 파일이 포함됩니다(최선형 상한).
...
- 스킬이 유료 제3자 서비스와 통합되는 경우, 외부 비용과 필요한 계정을 스킬 지침 및 환경 변수 선언에 명확히 문서화하세요(필수 변수는 `requires.env`, 선택적 변수는 `required: false`가 있는 `envVars` 사용).
