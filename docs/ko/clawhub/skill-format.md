...
웹 GitHub 가져오기 기능은 로컬 게시/동기화보다 더 엄격합니다. 로그인한 GitHub 계정이 소유한 공개 비포크 저장소에서 `SKILL.md` 또는 레거시 `skills.md` 파일만 찾습니다. 비공개 저장소, 포크, 보관/비활성화된 저장소, 서드파티 공개 저장소는 가져오지 않습니다.
...
스킬 실행 전에 반드시 있어야 하는 환경 변수에는 `requires.env`를 사용합니다. 선택 변수에 `required: false`를 포함하는 등 변수별 메타데이터가 필요할 때는 `envVars`를 사용합니다.
...
| `envVars`          | `array`    | `name`, 선택적 `required`, 선택적 `description`이 포함된 환경 변수 선언입니다. 선택 환경 변수에는 `required: false`를 설정합니다. |
...
스킬에 종속성 설치가 필요하면 `install` 배열에 선언합니다.
...
선택 환경 변수는 `metadata.openclaw.envVars` 아래에 선언하고 `required: false`를 설정합니다. `requires.env`는 해당 변수가 없으면 스킬을 실행할 수 없다는 뜻이므로, 선택 항목을 `requires.env`에 추가하지 마세요.
...
ClawHub의 보안 분석은 스킬이 선언한 내용이 실제 동작과 일치하는지 확인합니다. 코드가 `TODOIST_API_KEY`를 참조하지만 frontmatter에서 `requires.env`, `primaryEnv` 또는 `envVars` 아래에 선언하지 않으면, 분석은 메타데이터 불일치를 표시합니다. 선언을 정확하게 유지하면 스킬이 검토를 통과하는 데 도움이 되고, 사용자가 무엇을 설치하는지 이해하는 데도 도움이 됩니다.
...
- 전체 번들 크기: 50MB.
- 임베딩 텍스트에는 `SKILL.md`와 최대 약 40개의 비-`.md` 파일이 포함됩니다(최선 노력 상한).
...
- ClawHub은 유료 스킬, 스킬별 가격 책정, 페이월, 수익 배분을 지원하지 않습니다.
