...
Skill 메타데이터는 `SKILL.md` 맨 위의 YAML frontmatter에 선언됩니다. 이는 레지스트리(및 보안 분석)에 Skill 실행에 필요한 항목을 알려줍니다.
...
Skill에 설치해야 하는 종속성이 필요하다면 `install` 배열에 선언하세요.
...
선택적 환경 변수는 `metadata.openclaw.envVars` 아래에 선언하고 `required: false`를 설정하세요. 선택적 항목을 `requires.env`에 추가하지 마세요. `requires.env`는 해당 항목 없이는 Skill을 실행할 수 없다는 의미입니다.
...
ClawHub의 보안 분석은 Skill이 선언한 내용이 실제 동작과 일치하는지 확인합니다. 코드가 `TODOIST_API_KEY`를 참조하지만 frontmatter가 이를 `requires.env`, `primaryEnv` 또는 `envVars` 아래에 선언하지 않으면 분석에서 메타데이터 불일치로 표시합니다. 선언을 정확하게 유지하면 Skill이 검토를 통과하는 데 도움이 되고, 사용자가 무엇을 설치하는지 이해하는 데에도 도움이 됩니다.
...
- 전체 번들 크기: 50MB.
- 임베딩 텍스트에는 `SKILL.md` + 최대 약 40개의 `.md`가 아닌 파일이 포함됩니다(최선 노력 상한).
...
- 태그는 버전에 대한 문자열 포인터이며, `latest`가 흔히 사용됩니다.

## 라이선스

- ClawHub에 게시된 모든 Skill은 `MIT-0` 라이선스가 적용됩니다.
- 누구나 상업적 용도를 포함해 게시된 Skill을 사용, 수정, 재배포할 수 있습니다.
- 저작자 표시는 필요하지 않습니다.
