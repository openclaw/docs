...
x-i18n:
    generated_at: "2026-07-03T17:16:57Z"
    model: gpt-5.5
...
스킬 메타데이터는 `SKILL.md` 맨 위의 YAML 프런트매터에 선언됩니다. 이는 레지스트리(및 보안 분석)에 스킬 실행에 필요한 사항을 알려 줍니다.
...
스킬 실행 전에 반드시 있어야 하는 환경 변수에는 `requires.env`를 사용합니다. 선택 변수를 포함해 변수별 메타데이터가 필요할 때는 `envVars`를 사용하고, 선택 변수에는 `required: false`를 포함합니다.
...
스킬에 설치해야 하는 종속성이 필요한 경우 `install` 배열에 선언합니다.
...
선택적 환경 변수는 `metadata.openclaw.envVars` 아래에 선언하고 `required: false`를 설정합니다. `requires.env`는 해당 변수 없이는 스킬을 실행할 수 없다는 뜻이므로, 선택 항목을 `requires.env`에 추가하지 마세요.
...
ClawHub의 보안 분석은 스킬이 선언한 내용이 실제 동작과 일치하는지 확인합니다. 코드가 `TODOIST_API_KEY`를 참조하지만 프런트매터에서 이를 `requires.env`, `primaryEnv` 또는 `envVars` 아래에 선언하지 않으면, 분석에서 메타데이터 불일치로 표시됩니다. 선언을 정확하게 유지하면 스킬이 검토를 통과하는 데 도움이 되고 사용자가 무엇을 설치하는지 이해하는 데도 도움이 됩니다.
...
- ClawHub에 게시되는 모든 스킬은 `MIT-0`에 따라 라이선스가 부여됩니다.
...
- 스킬이 유료 타사 서비스와 통합되는 경우, 스킬 지침과 환경 변수 선언에서 외부 비용 및 필요한 계정을 명확히 문서화하세요(필수 변수는 `requires.env`, 선택 변수는 `required: false`가 있는 `envVars`).
