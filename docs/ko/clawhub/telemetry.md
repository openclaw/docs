...
read_when:
    ...
    - 수집되는 데이터에 관한 질문
summary: ClawHub CLI에서 수집하는 설치 원격 측정 데이터와 수집을 거부하는 방법입니다.
x-i18n:
...
보고 대상인 `clawhub install`이 실행될 때마다 CLI는 최선형 방식으로 설치 이벤트 하나를 전송합니다.
...
이를 설정하면 CLI에서 설치 텔레메트리를 전송하지 않습니다.
