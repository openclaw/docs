...
read_when:
    ...
    - 수집되는 데이터에 관한 질문
summary: ClawHub CLI가 수집하는 설치 텔레메트리 및 옵트아웃 방법.
x-i18n:
...
- `installsAllTime`: 해당 skill에 대해 CLI 설치를 하나 이상 보고한 고유 사용자입니다.
- `installsCurrent`: 설치를 보고했고 자신의 텔레메트리를 삭제하지 않은 고유 사용자입니다.
...
이 값이 설정되어 있으면 CLI는 설치 텔레메트리를 전송하지 않습니다.
