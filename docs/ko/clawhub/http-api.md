...
x-i18n:
    generated_at: "2026-07-03T13:21:09Z"
    model: gpt-5.5
...
타사 디렉터리는 공개 읽기 엔드포인트를 사용해 ClawHub Skills를 나열하거나 검색할 수 있습니다. 결과를 캐시하고, `429`/`Retry-After`를 준수하고, 사용자를 정식 ClawHub 목록(`https://clawhub.ai/<owner>/skills/<slug>`)으로 다시 연결하며, 타사 사이트를 ClawHub가 보증한다는 인상을 주지 마세요. 공개 API 표면 외부에서 숨김, 비공개 또는 운영 검토로 차단된 콘텐츠를 미러링하려고 시도하지 마세요.
...
- 인증된 쓰기 엔드포인트는 서버가 이유를 알고 있을 때 단순한 `Unauthorized`만
  ...
  계정은 각각 CLI 클라이언트가 사용자에게 차단 원인을 알려줄 수 있도록 실행 가능한
  텍스트를 받아야 합니다.
...
여기에는 유효성 검사 실패(`400`), 누락된 공개 리소스(`404`), 인증 및 권한 실패
(`401`/`403`), 속도 제한(`429`), 차단된 다운로드가 포함됩니다. 클라이언트는 응답 본문을
사람이 읽을 수 있는 문자열로 읽어야 합니다. 알 수 없는 쿼리 매개변수는 호환성을 위해
무시되지만, 인식된 쿼리 매개변수에 유효하지 않은 값이 있으면 `400`을 반환합니다.
...
- ASCII 텍스트는 단어 및 문장부호 경계에서 토큰화됩니다. 예를 들어 `personal-map`에는 독립형 `map` 토큰이 포함되지만, `amap-jsapi-skill`에는 `amap`, `jsapi`, `skill`이 포함됩니다. 따라서 `map`을 검색하면 `personal-map`이 `amap-jsapi-skill`보다 더 강한 어휘 일치를 얻습니다.
- 인기도는 로그 스케일로 조정되고 상한이 적용됩니다. 참여도가 높은 Skills라도 쿼리 텍스트 일치가 더 약하면 더 낮게 순위가 매겨질 수 있습니다.
- 의심스럽거나 숨겨진 운영 검토 상태는 호출자 필터와 현재 운영 검토 상태에 따라 공개 검색에서 Skill을 제거할 수 있습니다.

게시자 검색 가능성 지침:

- 사용자가 실제로 검색할 용어를 표시 이름, 요약, 태그에 넣으세요. 독립형 슬러그 토큰은 유지하려는 안정적인 ID이기도 할 때만 사용하세요.
- 새 슬러그가 더 나은 장기 정식 이름이 아닌 한, 특정 쿼리 하나를 좇기 위해 슬러그를 변경하지 마세요. 이전 슬러그는 리디렉션 별칭이 되지만, 정식 URL, 표시되는 슬러그, 향후 검색 다이제스트는 새 슬러그를 사용합니다.
- 이름 변경 별칭은 레지스트리를 통해 확인되는 이전 URL과 설치의 해석을 보존하지만, 검색 순위는 이름 변경이 인덱싱된 후 정식 Skill 메타데이터를 기반으로 합니다. 기존 통계는 Skill에 유지됩니다.
- Skill이 예상치 못하게 보이지 않으면 순위 관련 메타데이터를 변경하기 전에 로그인한 상태에서 `clawhub inspect @owner/slug`로 운영 검토 상태를 먼저 확인하세요.
...
- `nonSuspiciousOnly=true`인 경우, 커서 기반 정렬은 페이지 검색 후 의심스러운 Skills가 필터링되므로 한 페이지에서 `limit`보다 적은 항목을 반환할 수 있습니다.
...
- `metadata.os`: Skill 프런트매터에 선언된 OS 제한(예: `["macos"]`, `["linux"]`). 선언되지 않은 경우 `null`.
...
- 소유자와 운영 검토자는 숨겨진 Skills의 운영 검토 세부 정보에 접근할 수 있습니다.
- 공개 호출자는 이미 플래그가 지정된 표시 가능 Skills에 대해서만 `200`을 받습니다.
- 증거는 공개 호출자에게는 수정되어 제공되며, 원시 스니펫은 소유자/운영 검토자에게만 포함됩니다.

### `POST /api/v1/skills/{slug}/report`

운영 검토자 검토를 위해 Skill을 신고합니다. 신고는 Skill 수준이며 선택적으로
버전에 연결되고 Skill 신고 대기열에 전달됩니다.
...
Skill 신고 접수를 위한 운영 검토자/관리자 엔드포인트입니다.
...
Skill 신고를 해결하거나 다시 여는 운영 검토자/관리자 엔드포인트입니다.
...
설정할 때는 생략할 수 있습니다. 동일하게 감사 가능한 워크플로에서 Skill을 숨기려면
...
- 게시된 스캔에는 소유자/게시자 관리 접근 권한 또는 플랫폼 모더레이터/관리자 권한이 필요합니다.
...
제출된 버전에 대한 인증된 저장 보고서 아카이브 엔드포인트입니다.

- 스킬 또는 Plugin에 대한 소유자/게시자 관리 접근 권한, 또는 플랫폼 모더레이터/관리자 권한이 필요합니다.
...
관리자 전용 표준 배치 재스캔 라우트입니다. 레거시 `POST /api/v1/skills/-/rescan-batch`와 동일한 페이로드 형태를 받습니다.

### `POST /api/v1/skills/-/scan/batch/status`

관리자 전용 표준 배치 상태 라우트입니다. `{ "jobIds": ["..."] }`를 받고 레거시 `POST /api/v1/skills/-/rescan-batch/status`와 동일한 집계 카운터를 반환합니다.
...
- `ok`는 선택된 버전에 생성된 Skill Card가 있고, 모더레이션에 의해 멀웨어로 차단되지 않았으며, ClawScan 검증이 clean인 경우에만 `true`입니다.
- 스킬 ID, 게시자 ID, 선택된 버전 메타데이터는 최상위 엔벨로프 필드(`slug`, `displayName`, `publisherHandle`, `version`, `resolvedFrom`, `tag`, `createdAt`)이므로 셸 자동화에서 중첩 래퍼를 풀지 않고 읽을 수 있습니다.
- `security`는 최상위 ClawScan/보안 판정입니다. 자동화는 `ok`, `decision`, `reasons`, `security.status`를 기준으로 삼아야 합니다.
- `security.signals`에는 `staticScan`, `virusTotal`, `skillSpector` 같은 보조 스캐너 증거가 포함됩니다.
- `security.signals.dependencyRegistry`는 v1 응답 호환성을 위해 유지되지만, 의존성 레지스트리 존재 스캐너는 폐기되었으며 이 키는 항상 `null`입니다.
- `provenance`는 ClawHub가 게시 또는 가져오기 중 GitHub 저장소/ref/커밋/경로를 해석하고 저장한 경우에만 `server-resolved-github-import`입니다. 그 외에는 `unavailable`입니다.

### `POST /api/v1/skills/-/security-verdicts`

정확한 스킬 버전에 대한 현재의 압축된 보안 판정을 반환합니다. 이 컬렉션 엔드포인트는 OpenClaw Control UI처럼 표시해야 할 설치된 ClawHub 스킬 버전을 이미 알고 있는 클라이언트를 위한 것입니다.
...
- 결과는 항목별로 반환됩니다. 하나의 스킬 또는 버전이 없어도 전체 응답은 실패하지 않습니다.
- 응답은 보안 전용입니다. Skill Card 데이터, 생성된 카드 상태, 아티팩트 파일 목록, 상세 스캐너 페이로드는 포함하지 않습니다.
- `security.signals`에는 상태 수준의 보조 증거만 포함됩니다. 전체 스캐너 세부 정보가 필요하면 `/scan` 또는 ClawHub 보안 감사 페이지를 사용하세요.
- `security.signals.dependencyRegistry`는 v1 응답 호환성을 위해 유지되지만, 의존성 레지스트리 존재 스캐너는 폐기되었으며 이 키는 항상 `null`입니다.
- Skill Card가 없어도 이 엔드포인트의 `ok`, `decision`, `reasons`에는 영향을 주지 않습니다. 카드 콘텐츠가 필요할 때 클라이언트는 설치된 `skill-card.md`를 로컬에서 읽어야 합니다.
- 단일 스킬 Skill Card 검증 엔벨로프가 필요하면 `/verify`를, 생성된 카드 마크다운이 필요하면 `/card`를, 상세 스캐너 데이터가 필요하면 `/scan`을 사용하세요.
...
- 비공개 패키지는 호출자가 소유 퍼블리셔를 읽을 수 없는 한 `404`를 반환합니다.
...
파일 메타데이터, 호환성, 검증, 아티팩트 메타데이터, 스캔 데이터를 포함해 패키지 버전 하나를 반환합니다.
...
- ClawPack 릴리스에는 npm 호환 `npmIntegrity`, `npmShasum`, `npmTarballName`
  필드가 포함됩니다.
- `version.sha256hash`는 이전 클라이언트를 위한 폐기 예정 호환성 메타데이터입니다. 이는
...
- `release.artifactKind`, `release.artifactSha256`, `release.npmIntegrity`,
  ...
  알려진 경우 존재합니다.
- `trust.scanStatus`는 스캐너 입력과 수동 릴리스 모더레이션에서 도출된
  유효 신뢰 상태입니다.
- `trust.moderationState`는 nullable입니다. 수동 릴리스 모더레이션이 없으면
  `null`입니다.
...
- `trust.reasons`는 사용자에게 표시되고 감사에 쓰이는 설명 목록입니다. 이유 코드는
...
- `trust.stale`은 신뢰 요약이 오래된 입력에서 계산되었으며,
  높은 확신의 허용 결정을 내리기 전에 새로고침이 필요한 것으로 취급해야 함을 의미합니다.
...
- 이 엔드포인트는 소유자/모더레이터 모더레이션 엔드포인트보다 의도적으로 더 좁습니다.
  보고자 신원, 보고 본문, 비공개 증거 또는 내부 검토 타임라인이 아니라
  설치 결정과 공개 설명을 노출합니다.
...
- 기존 패키지 버전은 `legacy-zip` 아티팩트와 기존 ZIP `downloadUrl`을 반환합니다.
...
- 기존 ZIP 버전은 `/api/v1/packages/{name}/download?version=`으로 리디렉션합니다.
- 다운로드 속도 버킷을 사용합니다.
...
버전에 연결됩니다. 신고는 모더레이션 큐에 반영되지만 그 자체로 다운로드를 자동 숨김 처리하거나
차단하지 않습니다. 모더레이터는 릴리스 모더레이션을 사용하여
아티팩트를 승인, 격리 또는 철회해야 합니다.
...
패키지 조정 표시 여부를 위한 소유자/모더레이터 엔드포인트입니다.

인증:

- 패키지 소유자, 게시자 멤버, 모더레이터 또는 관리자 사용자용 API 토큰이
  필요합니다.
...
동일한 감사 가능한 워크플로에서 릴리스 조정을 적용합니다.
...
- 범위가 지정된 패키지 packument는 `/api/npm/@scope/name` 및 npm의
...
`suspicious` 검사가 있는 현재 GitHub 기반 스킬에 대해 GitHub 소스 인계를
반환합니다.
...
- 다운로드 통계는 UTC 일자별 고유 신원으로 계산됩니다(API 토큰이 유효하면 `userId`, 그렇지 않으면 IP).
...
- 선택적 페이로드 필드: `ownerHandle`. 있으면 API가 해당
  게시자를 서버 측에서 확인하고 행위자에게 게시자 접근 권한이 있어야 합니다.
- 선택적 페이로드 필드: `migrateOwner`. `ownerHandle`과 함께 `true`이면
  행위자가 현재 게시자와 대상 게시자 모두에서 관리자/소유자인 경우 기존 스킬을
  해당 소유자로 이동할 수 있습니다. 이 옵트인이 없으면 소유자 변경은
  거부됩니다.
...
- 동일한 요청에서 `files` 또는 `clawpack` 중 하나만 사용해야 하며, 둘 다 사용하면 안 됩니다.
...
- Plugin 패키지에는 `openclaw.plugin.json`이 필요합니다. ClawPack `.tgz` 업로드에는
  `package/openclaw.plugin.json`에 이 파일이 포함되어야 합니다.
...
있으면 `reason`이 스킬 조정 참고로 저장되고 감사 로그에 복사됩니다.
소유자가 시작한 소프트 삭제는 30일 동안 slug를 예약한 다음, 다른 게시자가
해당 slug를 요청할 수 있습니다. 이 만료가 적용되는 경우 삭제 응답에는 `slugReservedUntil`이 포함됩니다.
...
새로 생성된 조직의 경우 `memberHandle`을 제공합니다. 작업을 수행하는 관리자는 멤버로 추가되지 않습니다.
...
관리자 전용입니다. 릴리스를 게시하지 않고 정당한 소유자를 위해 루트 slug 및 패키지 이름을 예약합니다.
패키지 이름은 릴리스 행이 없는 비공개 자리표시자 패키지가 되므로, 같은
...
개인 게시자를 복구합니다. 요청은 변경 불가능한 두 GitHub
제공자 계정 ID를 모두 지정해야 합니다. 변경 가능한 핸들은 운영자 대면 가드로만 사용됩니다.

엔드포인트는 기본적으로 드라이 런입니다. 복구를 적용하려면 직원이 두
GitHub 주체 간의 연속성을 독립적으로 확인한 뒤 `dryRun: false` 및
`confirmIdentityVerified: true`가 필요합니다. 대상 사용자의 현재 개인
게시자에 skills, packages 또는 GitHub skill sources가 있으면 복구는 실패 시 닫힙니다.
복구는 또한 복구된 게시자의 skills, skill slug aliases, packages, package inspector warnings 및 파생된 search digest rows의 레거시 `ownerUserId` 필드를 마이그레이션하여
직접 소유자 경로가 새 게시자 권한과 일치하도록 합니다. 복구된 핸들에 대한 활성 protected-handle
reservation도 대체 사용자에게 재할당되어 이후 프로필 동기화가 이전 사용자의 경쟁 권한을 복원할 수 없게 합니다. 각 기본 테이블은 적용 트랜잭션당
...
사용자 차단을 해제하고 적합한 skills를 복원합니다(관리자 전용).
...
차단 해제나 콘텐츠 복원 없이 기존 차단에 저장된 사유를 변경합니다(관리자 전용).
`dryRun`이 `false`가 아니면 기본값은 드라이 런입니다.
