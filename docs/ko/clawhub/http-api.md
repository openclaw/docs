...
x-i18n:
    generated_at: "2026-07-02T17:33:50Z"
    model: gpt-5.5
...
타사 디렉터리는 공개 읽기 엔드포인트를 사용해 ClawHub Skills를 나열하거나 검색할 수 있습니다. 결과를 캐시하고, `429`/`Retry-After`를 준수하며, 사용자를 정식 ClawHub 목록(`https://clawhub.ai/<owner>/skills/<slug>`)으로 다시 연결하고, 타사 사이트를 ClawHub가 보증한다는 인상을 주지 마세요. 공개 API 표면 밖에 있는 숨김, 비공개 또는 모더레이션 차단 콘텐츠를 미러링하려고 시도하지 마세요.

웹 slug 바로 가기는 레지스트리 계열 전체에서 해석되지만, API 클라이언트는 라우트
...
- 인증된 쓰기 엔드포인트는 서버가 이유를 알고 있을 때 단순한 `Unauthorized`만 반환해서는 안 됩니다. 누락된 토큰, 유효하지 않거나 폐기된 토큰, 삭제/차단/비활성화된 계정은 각각 실행 가능한 텍스트를 받아야 하며, 그래야 CLI 클라이언트가 사용자에게 무엇이 차단했는지 알려줄 수 있습니다.
...
- `X-RateLimit-Remaining` / `RateLimit-Remaining`: 존재할 경우 정확한 잔여 예산.
...
- 신뢰할 수 있는 클라이언트 IP를 사용할 수 없으면 익명 요청은 속도 제한 종류로만 범위가 지정된 대체 버킷을 사용합니다. 이러한 대체 버킷에는 호출자가 제공한 경로, slug, 패키지 이름, 버전, 쿼리 문자열 또는 기타 아티팩트 매개변수가 포함되지 않습니다.
...
- 관련성은 인기보다 더 강합니다. 정확한 slug 또는 표시 이름 토큰 일치는 참여도가 훨씬 높은 느슨한 일치보다 높은 순위를 차지할 수 있습니다.
- ASCII 텍스트는 단어 및 문장 부호 경계에서 토큰화됩니다. 예를 들어 `personal-map`에는 독립적인 `map` 토큰이 포함되지만, `amap-jsapi-skill`에는 `amap`, `jsapi`, `skill`이 포함됩니다. 따라서 `map`을 검색하면 `personal-map`이 `amap-jsapi-skill`보다 더 강한 어휘 일치를 얻습니다.
- 인기는 로그 스케일이 적용되고 상한이 있습니다. 참여도가 높은 Skills도 쿼리 텍스트 일치가 약하면 더 낮은 순위가 될 수 있습니다.
- 의심스럽거나 숨겨진 모더레이션 상태는 호출자 필터 및 현재 모더레이션 상태에 따라 공개 검색에서 Skill을 제거할 수 있습니다.

게시자 검색 가능성 지침:

- 사용자가 실제로 검색할 용어를 표시 이름, 요약, 태그에 넣으세요. 독립적인 slug 토큰은 유지하려는 안정적인 ID이기도 할 때만 사용하세요.
- 새 slug가 더 나은 장기 정식 이름이 아닌 한, 단일 쿼리를 따라가기 위해서만 slug를 변경하지 마세요. 이전 slug는 리디렉션 별칭이 되지만, 정식 URL, 표시되는 slug, 향후 검색 다이제스트는 새 slug를 사용합니다.
- 이름 변경 별칭은 레지스트리를 통해 해석되는 이전 URL 및 설치에 대한 해석을 보존하지만, 검색 순위는 이름 변경이 인덱싱된 후 정식 Skill 메타데이터를 기준으로 합니다. 기존 통계는 해당 Skill에 유지됩니다.
- Skill이 예상치 못하게 보이지 않으면 순위 관련 메타데이터를 변경하기 전에 로그인한 상태에서 `clawhub inspect @owner/slug`로 먼저 모더레이션 상태를 확인하세요.
...
- `sort`(선택): `updated`(기본값), `recommended`(별칭: `default`), `createdAt`(별칭: `newest`), `downloads`, `stars`(별칭: `rating`), 레거시 설치 별칭 `installsCurrent`/`installs`/`installsAllTime`은 `downloads`에 매핑, `trending`
- `nonSuspiciousOnly`(선택): 의심스러운(`flagged.suspicious`) Skills를 숨기려면 `true`
- `nonSuspicious`(선택): `nonSuspiciousOnly`의 레거시 별칭
...
- `createdAt`은 새 Skill 크롤에 안정적입니다. `updated`는 기존 Skills가 다시 게시될 때 변경됩니다.
- `nonSuspiciousOnly=true`일 때, 커서 기반 정렬은 의심스러운 Skills가 페이지 검색 후 필터링되므로 한 페이지에서 `limit`보다 적은 항목을 반환할 수 있습니다.
- 존재할 경우 `nextCursor`를 사용해 페이지네이션을 계속하세요. 짧은 페이지만으로 결과 끝을 의미하지는 않습니다.
...
- `moderation`은 Skill에 플래그가 지정되었거나 소유자가 보고 있을 때만 포함됩니다.
...
- 증거는 공개 호출자에게는 편집되며, 원시 스니펫은 소유자/모더레이터에게만 포함됩니다.
...
Skill 신고를 해결하거나 다시 열기 위한 모더레이터/관리자 엔드포인트입니다.
...
설정할 때는 생략할 수 있습니다. 같은 감사 가능한 워크플로에서 Skill을 숨기려면 분류된
...
- 스캔 요청 페이로드와 다운로드 가능한 보고서는 보존 기간이 지난 후 scan-request 저장소에서 만료됩니다.
...
- 대기 중에는 `queue.queuedAhead`와 `queue.position`을 반환하여 클라이언트가 요청보다 앞서 있는 우선순위 수동 스캔 수를 표시할 수 있게 합니다. 매우 큰 큐는 제한되며 `queuedAheadIsEstimate: true`로 보고됩니다.
...
- `kind`의 기본값은 `skill`입니다. 플러그인/패키지 스캔에는 `kind=plugin`을 사용하세요.
- 스캔 요청 다운로드와 동일한 ZIP 형식을 반환합니다.

### `POST /api/v1/skills/-/scan/batch`

관리자 전용 정식 일괄 재스캔 경로입니다. 레거시 `POST /api/v1/skills/-/rescan-batch`와 동일한 페이로드 형식을 받습니다.

### `POST /api/v1/skills/-/scan/batch/status`

관리자 전용 정식 일괄 상태 경로입니다. `{ "jobIds": ["..."] }`를 받고 레거시 `POST /api/v1/skills/-/rescan-batch/status`와 동일한 집계 카운터를 반환합니다.

### `GET /api/v1/skills/{slug}/verify`

`clawhub skill verify`에서 사용하는 Skill Card 검증 엔벨로프를 반환합니다.
...
- 셸 자동화가 중첩 래퍼를 풀지 않고 읽을 수 있도록 스킬 ID, 게시자 ID, 선택한 버전 메타데이터는 최상위 엔벨로프 필드(`slug`, `displayName`, `publisherHandle`, `version`, `resolvedFrom`, `tag`, `createdAt`)입니다.
...
- `provenance`는 ClawHub가 게시 또는 가져오기 중에 GitHub 저장소/참조/커밋/경로를 해석하고 저장한 경우에만 `server-resolved-github-import`이며, 그렇지 않으면 `unavailable`입니다.

### `POST /api/v1/skills/-/security-verdicts`

정확한 스킬 버전에 대한 현재의 간결한 보안 판정을 반환합니다. 이 컬렉션 엔드포인트는 OpenClaw Control UI처럼 표시해야 하는 설치된 ClawHub 스킬 버전을 이미 알고 있는 클라이언트를 위한 것입니다.
...
- 결과는 항목별로 제공되며, 하나의 누락된 스킬 또는 버전이 전체 응답을 실패시키지는 않습니다.
...
- 단일 스킬 Skill Card 검증 엔벨로프가 필요하면 `/verify`를, 생성된 카드 Markdown이 필요하면 `/card`를, 자세한 스캐너 데이터가 필요하면 `/scan`을 사용하세요.
...
- 인증된 호출자는 자신이 속한 퍼블리셔의 비공개 패키지를 목록/검색 결과에서 볼 수 있습니다.
- `channel=private`은 인증된 호출자가 읽을 수 있는 패키지만 반환합니다.

### `GET /api/v1/packages/search`

Skills와 plugin 패키지 전반에 걸친 통합 카탈로그 검색입니다.
...
- 익명 호출자는 공개 패키지 채널만 볼 수 있습니다.
- 인증된 호출자는 자신이 속한 퍼블리셔의 비공개 패키지를 검색할 수 있습니다.
- `channel=private`은 인증된 호출자가 읽을 수 있는 패키지만 반환합니다.
...
- `clean` 또는 `suspicious` 스캔이 있는 현재 GitHub 기반 Skills는
  ...
  content hash 및 아카이브 URL을 포함합니다. ClawHub 호스팅 소스 파일은 포함하지 않습니다.
...
- `version.artifact.kind`는 구형 패키지 아카이브의 경우 `legacy-zip`, ClawPack 기반 릴리스의 경우
  `npm-pack`입니다.
...
설치 클라이언트를 위한 정확한 패키지 릴리스 보안 및 신뢰 요약을 반환합니다. 이는 해석된 릴리스를
설치할 수 있는지 결정하기 위한 공개 OpenClaw 소비 표면입니다.

인증:

- 공개 읽기 엔드포인트입니다. 소유자, 게시자, moderator 또는 admin 토큰이
  필요하지 않습니다.
...
- `package.name`, `package.displayName`, 및 `package.family`는 해석된
  레지스트리 패키지를 식별합니다.
- `release.releaseId`, `release.version`, 및 `release.createdAt`는 평가된
  정확한 릴리스를 식별합니다.
- `release.artifactKind`, `release.artifactSha256`, `release.npmIntegrity`,
  `release.npmShasum`, 및 `release.npmTarballName`은 릴리스 아티팩트에 대해 알려진 경우
  존재합니다.
...
- `trust.blockedFromDownload`는 설치 차단 신호입니다. OpenClaw 및 기타 설치 클라이언트는
  스캐너 또는 moderation 필드에서 차단 규칙을 다시 파생하는 대신 이 값이 `true`일 때
  설치를 차단해야 합니다.
- `trust.reasons`는 사용자에게 표시되는 감사 설명 목록입니다. 이유 코드는
  `manual:quarantined`, `scan:malicious`, 및 `package:malicious`와 같은 안정적이고 간결한 문자열입니다.
...
- 이 엔드포인트는 버전 단위로 정확합니다. 클라이언트는 단순히 최신 패키지 메타데이터를 읽은 뒤가 아니라,
  설치하려는 패키지 버전을 해석한 후 호출해야 합니다.
- 비공개 패키지는 호출자가 소유 게시자를 읽을 수 없는 한 `404`를 반환합니다.
- 이 엔드포인트는 의도적으로 소유자/moderator moderation 엔드포인트보다 범위가 좁습니다. 보고자 ID,
...
- ClawPack 버전은 `npm-pack` 아티팩트, npm 무결성 필드, `tarballUrl`, 및 레거시 ZIP 호환성 URL을
  반환합니다.
- 이는 OpenClaw 해석기 표면이며, 공유 URL에서 아카이브 형식을 추측하지 않게 합니다.
...
공식 OpenClaw Plugin 마이그레이션 행을 나열하기 위한 moderator 엔드포인트입니다.
...
공식 Plugin 마이그레이션 행을 생성하거나 업데이트하기 위한 admin 엔드포인트입니다.
...
- `manual`: 수동 moderation 재정의가 있는 모든 릴리스입니다.
- `all`: 수동 재정의, 깨끗하지 않은 스캔 상태 또는 패키지 보고서가 있는 모든 릴리스입니다.
...
moderator 검토를 위해 패키지를 보고합니다. 보고서는 패키지 수준이며, 선택적으로 버전에
연결될 수 있습니다. 이는 moderation 대기열에 반영되지만, 자체적으로 다운로드를 자동 숨김하거나
차단하지는 않습니다. moderator는 릴리스 moderation을 사용해 아티팩트를 승인, 격리 또는 취소해야 합니다.
...
패키지 조정 가시성을 위한 소유자/조정자 엔드포인트.

인증:

- 패키지 소유자, 게시자 멤버, 조정자 또는 관리자 사용자용 API 토큰이
  필요합니다.
...
패키지 신고를 해결하거나 다시 여는 조정자/관리자 엔드포인트.
...
`finalAction: "revoke"`를 전달하면 동일한 감사 가능한 워크플로에서 릴리스 조정을 적용합니다.
...
- 파일 크기 제한: 200KB.
- 대기 중인 VirusTotal 검사는 읽기를 차단하지 않습니다. 악성 릴리스는 다른 곳에서 여전히 보류될 수 있습니다.
- 비공개 패키지는 호출자가 소유 게시자를 읽을 수 없는 한 `404`를 반환합니다.
...
호스팅된 skill 버전 ZIP을 다운로드하거나, 호스팅된 버전 없이 `clean` 또는 `suspicious` 검사를 받은
현재 GitHub 기반 skill에 대해 GitHub 소스 인계를 반환합니다.
...
- 선택적 페이로드 필드: `ownerHandle`. 존재하면 API가 해당
  게시자를 서버 측에서 확인하고 행위자에게 게시자 접근 권한이 있어야 합니다.
...
존재하면 `reason`은 skill 조정 메모로 저장되고 감사 로그에 복사됩니다.
소유자가 시작한 소프트 삭제는 slug를 30일 동안 예약한 다음, 다른 게시자가
해당 slug를 청구할 수 있습니다. 이 만료가 적용되면 삭제 응답에는 `slugReservedUntil`이 포함됩니다.
조정자/관리자 숨김 및 보안 제거는 이런 방식으로 만료되지 않습니다.
...
관리자 전용. 핸들에 대한 org 게시자가 존재하도록 보장합니다. 핸들이 아직
레거시 공유 사용자/개인 게시자를 가리키는 경우, 엔드포인트가 먼저 이를 org 게시자로 마이그레이션합니다.
새로 생성된 org의 경우 `memberHandle`을 제공하세요. 행위 관리자 자신은 멤버로 추가되지 않습니다.
...
인증된 셀프서비스 org 게시자 생성. 새 org 게시자를 만들고
...
관리자 전용. 릴리스를 게시하지 않고 정당한 소유자를 위해 루트 slug 및 패키지 이름을 예약합니다.
...
관리자 전용. Convex Auth 계정 행을 편집하지 않고 검증된 대체 GitHub OAuth 주체의
개인 게시자를 복구합니다. 요청에는 변경 불가능한 두 GitHub
제공자 계정 ID를 모두 명시해야 합니다. 변경 가능한 핸들은 운영자용 가드로만 사용됩니다.
...
게시자에 스킬, 패키지 또는 GitHub 스킬 소스가 있으면 복구는 실패 시 차단 상태로 종료됩니다.
...
사용자를 차단하고 소유한 스킬을 하드 삭제합니다(중재자/관리자 전용).
...
`POST /api/cli/upload-url`은 `uploadUrl` 및 `uploadTicket`을 반환합니다. ClawPack
타르볼을 스테이징하는 패키지 게시 작업은 결과 저장소 id를
