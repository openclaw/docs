...
타사 디렉터리는 공개 읽기 엔드포인트를 사용해 ClawHub Skills를 나열하거나 검색할 수 있습니다. 결과를 캐시하고, `429`/`Retry-After`를 준수하며, 사용자를 표준 ClawHub 목록(`https://clawhub.ai/<owner>/<slug>`)으로 다시 연결하고, ClawHub가 해당 타사 사이트를 보증한다는 인상을 주지 마세요. 공개 API 표면 밖에서 숨김, 비공개 또는 모더레이션으로 차단된 콘텐츠를 미러링하려고 시도하지 마세요.
...
- 인증된 쓰기 엔드포인트는 서버가 이유를 알고 있을 때 단순한 `Unauthorized`만 반환해서는 안 됩니다. 토큰 누락, 유효하지 않거나 취소된 토큰, 삭제/차단/비활성화된 계정은 각각 실행 가능한 텍스트를 받아야 CLI 클라이언트가 사용자에게 무엇이 차단했는지 알려줄 수 있습니다.
...
- `Retry-After`: `429`에서 재시도 전에 대기할 초(지연)
...
- ClawHub는 신뢰할 수 있는 전달 헤더를 사용해 엣지에서 클라이언트 IP를 식별합니다.
- 신뢰할 수 있는 클라이언트 IP가 없으면 익명 다운로드 요청은 하나의 전역 `ip:unknown` 버킷 대신 엔드포인트 범위의 폴백 버킷을 사용합니다. 익명 읽기/쓰기 요청은 계속 공유 unknown 버킷을 사용하므로 IP 누락 라우팅이 계속 보이고 보수적으로 유지됩니다.
...
- 결과는 관련도 순서로 반환됩니다(임베딩 유사도 + 정확한 slug/이름 토큰 부스트 + 다운로드에서 가져온 인기도 사전값).
- 관련도는 인기도보다 강합니다. 정확한 slug 또는 표시 이름 토큰 일치는 다운로드 수가 훨씬 많은 더 느슨한 일치보다 높은 순위를 가질 수 있습니다.
...
- 다운로드는 주요 순위 신호가 아니라 작은 로그 스케일 사전값 및 동점 해소 기준으로 사용됩니다. 쿼리 텍스트 일치가 약하면 다운로드가 많은 Skills도 더 낮게 순위가 매겨질 수 있습니다.
- 의심스럽거나 숨겨진 모더레이션 상태는 호출자 필터와 현재 모더레이션 상태에 따라 공개 검색에서 Skill을 제거할 수 있습니다.

게시자 검색 가능성 지침:

- 사용자가 문자 그대로 검색할 용어를 표시 이름, 요약, 태그에 넣으세요. 독립된 slug 토큰은 유지하려는 안정적인 식별자이기도 할 때만 사용하세요.
- 새 slug가 더 나은 장기 표준 이름이 아니라면 특정 쿼리 하나를 쫓기 위해 slug 이름을 바꾸지 마세요. 이전 slug는 리디렉션 별칭이 되지만, 표준 URL, 표시되는 slug, 향후 검색 다이제스트는 새 slug를 사용합니다.
- 이름 변경 별칭은 이전 URL과 레지스트리를 통해 해석되는 설치에 대한 해석을 보존하지만, 검색 순위는 이름 변경이 인덱싱된 후의 표준 Skill 메타데이터를 기준으로 합니다. 기존 통계는 Skill에 유지됩니다.
- Skill이 예상치 못하게 보이지 않으면 순위 관련 메타데이터를 변경하기 전에 로그인한 상태에서 `clawhub inspect <slug>`로 먼저 모더레이션 상태를 확인하세요.
...
- `trending`은 최근 7일의 설치 수를 기준으로 순위를 매깁니다(텔레메트리 기반).
- `createdAt`은 새 Skill 크롤링에 안정적이며, `updated`는 기존 Skills가 다시 게시될 때 변경됩니다.
- `nonSuspiciousOnly=true`인 경우, 커서 기반 정렬은 페이지 검색 후 의심스러운 Skills가 필터링되기 때문에 한 페이지에서 `limit`보다 적은 항목을 반환할 수 있습니다.
- `nextCursor`가 있으면 이를 사용해 페이지네이션을 계속하세요. 짧은 페이지 자체만으로 결과가 끝났다는 의미는 아닙니다.
...
- 소유자 이름 변경/병합 흐름으로 생성된 이전 slug는 표준 Skill로 해석됩니다.
...
`note`는 `confirmed` 및 `dismissed`에 필수입니다. `status`를 다시 `open`으로 설정할 때는 생략할 수 있습니다. 감사 가능한 동일한 워크플로에서 Skill을 숨기려면 분류된 신고와 함께 `finalAction: "hide"`를 전달하세요.
...
- 패키지 소유자, 조직 게시자 소유자/관리자, 플랫폼 조정자 또는 플랫폼 관리자용 API 토큰이 필요합니다.
...
- `version.artifact.kind`는 기존 방식의 패키지 아카이브의 경우 `legacy-zip`,
...
조정자 검토를 위해 패키지를 신고합니다. 신고는 패키지 수준이며, 선택적으로 버전에 연결됩니다. 신고는 조정 대기열에 반영되지만 그 자체로 다운로드를 자동으로 숨기거나 차단하지는 않습니다. 조정자는 릴리스 조정을 사용하여 아티팩트를 승인, 격리 또는 취소해야 합니다.
...
- 패키지 소유자, 게시자 구성원, 조정자 또는 관리자 사용자용 API 토큰이 필요합니다.
...
`finalAction: "revoke"`를 전달하면 동일한 감사 가능한 워크플로에서 릴리스 조정을 적용할 수 있습니다.
...
이전 패키지 릴리스에 명시적 아티팩트 종류 메타데이터를 지정하기 위한
...
ClawPack 기반 패키지 버전에 대해 npm 호환 패큐먼트를 반환합니다.
...
npm 미러 클라이언트에 정확히 업로드된 ClawPack tarball 바이트를 스트리밍합니다.
...
- 선택적 페이로드 필드: `ownerHandle`. 제공되면 API가 해당 게시자를 서버 측에서 해석하고
  행위자에게 게시자 접근 권한이 있어야 합니다.
- 선택적 페이로드 필드: `migrateOwner`. `ownerHandle`과 함께 `true`이면, 행위자가 현재 게시자와
  대상 게시자 모두의 관리자/소유자인 경우 기존 스킬이 해당 소유자로 이동할 수 있습니다.
  이 명시적 선택 없이는 소유자 변경이 거부됩니다.
...
- 대리 게시는 여전히 공식 채널 자격을 대상 소유자 계정 기준으로 검증합니다.
...
제공되면 `reason`은 스킬 중재 메모로 저장되고 감사 로그에 복사됩니다.
소유자가 시작한 소프트 삭제는 30일 동안 슬러그를 예약하며, 이후 다른 게시자가
해당 슬러그를 클레임할 수 있습니다. 이 만료가 적용되는 경우 삭제 응답에는 `slugReservedUntil`이 포함됩니다.
중재자/관리자의 숨김 및 보안 제거에는 이러한 만료가 적용되지 않습니다.
...
관리자 전용입니다. 릴리스를 게시하지 않고 정당한 소유자를 위해 루트 슬러그와 패키지 이름을 예약합니다.
패키지 이름은 릴리스 행이 없는 비공개 자리 표시자 패키지가 되므로, 같은 소유자가 나중에
실제 코드 Plugin 또는 번들 Plugin 릴리스를 해당 이름으로 게시할 수 있습니다.
