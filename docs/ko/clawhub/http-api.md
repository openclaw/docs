...
x-i18n:
    generated_at: "2026-05-12T00:56:58Z"
    model: gpt-5.5
    provider: openai
...
서드파티 디렉터리는 공개 읽기 엔드포인트를 사용해 ClawHub Skills를 나열하거나 검색할 수 있습니다. 결과를 캐시하고, `429`/`Retry-After`를 준수하며, 사용자를 정식 ClawHub 목록(`https://clawhub.ai/<owner>/<slug>`)으로 다시 연결하고, ClawHub가 해당 서드파티 사이트를 보증한다는 인상을 주지 마세요. 숨김, 비공개 또는 모더레이션으로 차단된 콘텐츠를 공개 API 표면 밖에서 미러링하려고 시도하지 마세요.
...
- 인증된 쓰기 엔드포인트는 서버가 이유를 알고 있을 때 단순한 `Unauthorized`만 반환해서는 안 됩니다. 누락된 토큰, 유효하지 않거나 폐기된 토큰, 삭제/차단/비활성화된 계정은 각각 CLI 클라이언트가 사용자에게 차단 원인을 알려줄 수 있도록 실행 가능한 문구를 받아야 합니다.
...
- 신뢰할 수 있는 클라이언트 IP를 사용할 수 없는 경우 익명 다운로드 요청은 하나의 전역 `ip:unknown` 버킷 대신 엔드포인트 범위 폴백 버킷을 사용합니다. 익명 읽기/쓰기 요청은 여전히 공유 unknown 버킷을 사용하므로 IP 누락 라우팅이 계속 표시되고 보수적으로 유지됩니다.
...
- 결과는 관련도 순서로 반환됩니다(임베딩 유사도 + 정확한 슬러그/이름 토큰 부스트 + 다운로드 기반 인기도 사전값).
- 관련도는 인기도보다 더 강합니다. 정확한 슬러그 또는 표시 이름 토큰 일치는 다운로드가 훨씬 많은 더 느슨한 일치보다 높은 순위를 가질 수 있습니다.
...
- 다운로드는 기본 순위 신호가 아니라 작은 로그 스케일 사전값 및 동점 결정 요소로 사용됩니다. 쿼리 텍스트 일치가 약하면 다운로드가 많은 Skills도 더 낮은 순위를 가질 수 있습니다.
- 의심스럽거나 숨겨진 모더레이션 상태는 호출자 필터와 현재 모더레이션 상태에 따라 공개 검색에서 Skill을 제거할 수 있습니다.

게시자 검색 가능성 지침:

- 사용자가 문자 그대로 검색할 용어를 표시 이름, 요약, 태그에 넣으세요. 독립형 슬러그 토큰은 유지하려는 안정적인 정체성이기도 한 경우에만 사용하세요.
- 새 슬러그가 더 나은 장기 정식 이름이 아닌 한, 단일 쿼리를 쫓기 위해 슬러그 이름을 바꾸지 마세요. 이전 슬러그는 리디렉션 별칭이 되지만, 정식 URL, 표시되는 슬러그, 향후 검색 다이제스트는 새 슬러그를 사용합니다.
- 이름 변경 별칭은 이전 URL과 레지스트리를 통해 해석되는 설치의 해석을 보존하지만, 검색 순위는 이름 변경이 인덱싱된 후 정식 Skill 메타데이터를 기준으로 합니다. 기존 통계는 해당 Skill에 유지됩니다.
- Skill이 예상치 못하게 보이지 않는 경우, 순위 관련 메타데이터를 변경하기 전에 로그인한 상태에서 `clawhub inspect <slug>`로 먼저 모더레이션 상태를 확인하세요.
...
- 있는 경우 `nextCursor`를 사용해 페이지네이션을 계속하세요. 짧은 페이지만으로 결과의 끝을 의미하지는 않습니다.
...
- 소유자 이름 변경/병합 플로우로 생성된 이전 슬러그는 정식 Skill로 해석됩니다.
...
- 증거는 공개 호출자에게는 편집되며 소유자/모더레이터에게만 원시 스니펫이 포함됩니다.

### `POST /api/v1/skills/{slug}/report`

모더레이터 검토를 위해 Skill을 신고합니다. 신고는 Skill 수준이며, 선택적으로 버전에 연결되고 Skill 신고 큐로 공급됩니다.
...
`note`는 `confirmed` 및 `dismissed`에 필요하며, `status`를 다시 `open`으로 설정할 때는 생략할 수 있습니다. 동일한 감사 가능한 워크플로에서 Skill을 숨기려면 분류된 신고와 함께 `finalAction: "hide"`를 전달하세요.
...
- `security.capabilityTags`에는 감지된 경우 `crypto`, `requires-wallet`, `can-make-purchases`, `can-sign-transactions`, `requires-oauth-token`, `posts-externally`와 같은 결정론적 기능/위험 레이블이 포함됩니다.
- `security.hasScanResult`는 스캐너가 확정 판정(`clean`, `suspicious` 또는 `malicious`)을 생성한 경우에만 `true`입니다.
- `moderation`은 최신 버전에서 파생된 현재 Skill 수준 모더레이션 스냅샷입니다.
...
- Skills 항목은 Skills 레지스트리를 계속 기반으로 하며 여전히 `POST /api/v1/skills`를 통해서만 게시할 수 있습니다.
...
Skills + Plugin 패키지 전체의 통합 카탈로그 검색입니다.
...
파일 메타데이터, 호환성, 기능, 검증, 아티팩트 메타데이터 및 스캔 데이터를 포함해 하나의 패키지 버전을 반환합니다.
...
모더레이터 검토를 위해 패키지를 신고합니다. 신고는 패키지 수준이며, 선택적으로 버전에 연결될 수 있습니다. 신고는 모더레이션 대기열에 반영되지만 그 자체로 다운로드를 자동 숨김 처리하거나 차단하지 않습니다. 모더레이터는 릴리스 모더레이션을 사용해 아티팩트를 승인, 격리 또는 취소해야 합니다.
...
설정할 때는 생략할 수 있습니다. 확인된 보고서와 함께 `finalAction: "quarantine"` 또는
...
이전 패키지 릴리스에 명시적인 아티팩트 종류 메타데이터를 라벨링하기 위한
관리자 전용 유지관리 엔드포인트입니다.
...
- `dist.tarball`, `dist.integrity`, `dist.shasum`은 사용자가 원하면 npm이 미러를 가리키도록 할 수 있게 npm 호환 필드를 사용합니다.
...
- 선택적 페이로드 필드: `ownerHandle`. 존재하면 API가 서버 측에서 해당 게시자를 확인하고 행위자에게 게시자 접근 권한이 있어야 합니다.
- 선택적 페이로드 필드: `migrateOwner`. `ownerHandle`과 함께 `true`이면, 행위자가 현재 및 대상 게시자 모두에서 관리자/소유자인 경우 기존 Skills가 해당 소유자로 이동할 수 있습니다. 이 옵트인이 없으면 소유자 변경은 거부됩니다.
...
- Plugin 패키지에는 `openclaw.plugin.json`이 필요합니다. ClawPack `.tgz` 업로드에는 `package/openclaw.plugin.json` 위치에 이를 포함해야 합니다.
...
- 대리 게시도 대상 소유자 계정을 기준으로 공식 채널 적격성을 검증합니다.
...
존재하면 `reason`은 Skills 조정 메모로 저장되고 감사 로그에 복사됩니다.
소유자가 시작한 소프트 삭제는 30일 동안 slug를 예약하며, 그 후 다른 게시자가
slug를 요청할 수 있습니다. 이 만료가 적용되면 삭제 응답에 `slugReservedUntil`이 포함됩니다.
...
패키지 이름은 릴리스 행이 없는 비공개 플레이스홀더 패키지가 되므로, 같은 소유자가 나중에
...
사용자를 차단하고 소유한 Skills를 하드 삭제합니다(조정자/관리자 전용).
