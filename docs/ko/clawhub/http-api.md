...
x-i18n:
    generated_at: "2026-07-02T22:24:41Z"
    model: gpt-5.5
...
타사 디렉터리는 공개 읽기 엔드포인트를 사용해 ClawHub Skills를 나열하거나 검색할 수 있습니다. 결과를 캐시하고, `429`/`Retry-After`를 준수하며, 사용자를 표준 ClawHub 목록(`https://clawhub.ai/<owner>/skills/<slug>`)으로 다시 연결하고, 타사 사이트를 ClawHub가 보증한다는 인상을 주지 마세요. 공개 API 표면 외부에서 숨김, 비공개 또는 모더레이션으로 차단된 콘텐츠를 미러링하려고 시도하지 마세요.

웹 슬러그 바로 가기는 레지스트리 계열 전반에서 해석되지만, API 클라이언트는 라우트 우선순위를 재구성하는 대신 읽기 엔드포인트가 반환하는 표준 URL을 사용해야 합니다.
...
- 관련성은 인기도보다 더 강합니다. 정확한 슬러그 또는 표시 이름 토큰 일치는 참여도가 훨씬 더 강한 느슨한 일치보다 높은 순위를 차지할 수 있습니다.
- ASCII 텍스트는 단어 및 문장 부호 경계에서 토큰화됩니다. 예를 들어 `personal-map`에는 독립형 `map` 토큰이 포함되어 있고, `amap-jsapi-skill`에는 `amap`, `jsapi`, `skill`이 포함됩니다. 따라서 `map`을 검색하면 `amap-jsapi-skill`보다 `personal-map`이 더 강한 어휘 일치를 얻습니다.
- 인기도는 로그 스케일로 조정되고 상한이 적용됩니다. 참여도가 높은 Skills라도 쿼리 텍스트 일치가 더 약하면 낮은 순위를 차지할 수 있습니다.
...
- 새 슬러그가 더 나은 장기 표준 이름이 아닌 한, 하나의 쿼리를 따라잡기 위해 슬러그 이름을 바꾸지 마세요. 이전 슬러그는 리디렉션 별칭이 되지만, 표준 URL, 표시되는 슬러그, 향후 검색 다이제스트는 새 슬러그를 사용합니다.
- 이름 변경 별칭은 레지스트리를 통해 해석되는 이전 URL과 설치의 해석을 보존하지만, 검색 순위는 이름 변경이 인덱싱된 후 표준 Skills 메타데이터를 기반으로 합니다. 기존 통계는 Skills에 유지됩니다.
...
- `limit`(선택): 정수(1–200)
- `cursor`(선택): `trending`이 아닌 모든 정렬에 대한 페이지 매김 커서
...
- `createdAt`은 새 Skills 크롤에 안정적입니다. `updated`는 기존 Skills가 다시 게시될 때 변경됩니다.
- `nonSuspiciousOnly=true`일 때 커서 기반 정렬은 페이지 검색 후 의심스러운 Skills가 필터링되므로 한 페이지에서 `limit`보다 적은 항목을 반환할 수 있습니다.
- 존재하는 경우 `nextCursor`를 사용해 페이지 매김을 계속하세요. 짧은 페이지만으로 결과 끝을 의미하지는 않습니다.
...
- `metadata.os`: Skills frontmatter에 선언된 OS 제한(예: `["macos"]`, `["linux"]`). 선언되지 않았으면 `null`.
- `metadata.systems`: Nix 시스템 대상(예: `["aarch64-darwin", "x86_64-linux"]`). 선언되지 않았으면 `null`.
- Skills에 플랫폼 메타데이터가 없으면 `metadata`는 `null`입니다.
- `moderation`은 Skills가 플래그되었거나 소유자가 보고 있을 때만 포함됩니다.
...
- 공개 호출자는 이미 플래그된 표시 가능한 Skills에 대해서만 `200`을 받습니다.
- 증거는 공개 호출자에게는 수정 처리되며, 원본 스니펫은 소유자/모더레이터에게만 포함됩니다.

### `POST /api/v1/skills/{slug}/report`

모더레이터 검토를 위해 Skills를 신고합니다. 신고는 Skills 수준이며, 선택적으로 버전에 연결되고, Skills 신고 큐에 반영됩니다.
...
`note`는 `confirmed` 및 `dismissed`에 필수입니다. `status`를 다시 `open`으로 설정할 때는 생략할 수 있습니다. 감사 가능한 동일 워크플로에서 Skills를 숨기려면 분류된 신고와 함께 `finalAction: "hide"`를 전달하세요.
...
- 과거 버전을 쿼리할 때는 `moderation`과 `security`를 같은 버전 컨텍스트로 취급하기 전에 `moderation.matchesRequestedVersion`와 `moderation.sourceVersion`를 확인하세요.
...
- 응답은 `{ "ok": true, "scanId": "...", "jobId": "...", "status": "queued", "sourceKind": "published", "update": false, "queue": { "queuedAhead": 0, "queuedAheadIsEstimate": false, "position": 1, "running": 0, "runningIsEstimate": false, "note": "스캔은 비동기식이며 완료되는 데 시간이 걸릴 수 있습니다." } }`와 함께 `202`입니다.
...
- 대기 중에는 `queue.queuedAhead`와 `queue.position`을 반환하므로 클라이언트가 해당 요청보다 앞선 우선순위 수동 스캔 수를 표시할 수 있습니다. 매우 큰 큐는 제한되며 `queuedAheadIsEstimate: true`로 보고됩니다.
...
- 차단되었거나 숨겨진 버전을 포함하여 정확히 제출된 버전에 대한 저장된 스캔 결과를 반환합니다.
- `kind`의 기본값은 `skill`입니다. plugin/package 스캔에는 `kind=plugin`을 사용하세요.
- 스캔 요청 다운로드와 동일한 ZIP 형태를 반환합니다.

### `POST /api/v1/skills/-/scan/batch`

admin 전용 표준 배치 재스캔 라우트입니다. 레거시 `POST /api/v1/skills/-/rescan-batch`와 동일한 페이로드 형태를 받습니다.

### `POST /api/v1/skills/-/scan/batch/status`

admin 전용 표준 배치 상태 라우트입니다. `{ "jobIds": ["..."] }`를 받고 레거시 `POST /api/v1/skills/-/rescan-batch/status`와 동일한 집계 카운터를 반환합니다.
...
- `ok`는 선택된 버전에 생성된 Skill Card가 있고, moderation에 의해 악성코드로 차단되지 않았으며, ClawScan 검증이 clean인 경우에만 `true`입니다.
- Skill ID, 게시자 ID, 선택된 버전 메타데이터는 최상위 엔벨로프 필드(`slug`, `displayName`, `publisherHandle`, `version`, `resolvedFrom`, `tag`, `createdAt`)이므로 셸 자동화가 중첩 래퍼를 풀지 않고 읽을 수 있습니다.
...
- `security.signals.dependencyRegistry`는 v1 응답 호환성을 위해 유지되지만, 종속성 레지스트리 존재 스캐너는 사용 중단되었으며 이 키는 항상 `null`입니다.
...
정확한 skill 버전에 대한 현재 압축 security 판정을 반환합니다. 이 컬렉션 엔드포인트는 OpenClaw Control UI처럼 표시해야 할 설치된 ClawHub skill 버전을 이미 알고 있는 클라이언트를 위한 것입니다.
...
- 응답은 security 전용입니다. Skill Card 데이터, 생성된 카드 상태, 아티팩트 파일 목록 또는 자세한 스캐너 페이로드는 포함하지 않습니다.
- `security.signals`에는 상태 수준의 지원 증거만 포함됩니다. 전체 스캐너 세부 정보는 `/scan` 또는 ClawHub security-audit 페이지를 사용하세요.
- `security.signals.dependencyRegistry`는 v1 응답 호환성을 위해 유지되지만, 종속성 레지스트리 존재 스캐너는 사용 중단되었으며 이 키는 항상 `null`입니다.
- Skill Card 부재는 이 엔드포인트의 `ok`, `decision`, `reasons`에 영향을 주지 않습니다. 카드 콘텐츠가 필요한 경우 클라이언트는 설치된 `skill-card.md`를 로컬에서 읽어야 합니다.
- 단일 skill Skill Card 검증 엔벨로프가 필요하면 `/verify`를, 생성된 카드 Markdown이 필요하면 `/card`를, 자세한 스캐너 데이터가 필요하면 `/scan`을 사용하세요.
...
- `category` (선택 사항): plugin 카테고리 필터. 요청 범위가 plugin 패키지(`/api/v1/plugins`,
  `/api/v1/code-plugins`, `/api/v1/bundle-plugins` 또는 `family=code-plugin`/`family=bundle-plugin`이 있는 패키지 엔드포인트)로 지정된 경우에만 지원됩니다. 제어되는 카테고리 및
...
- `startDate` (필수): Skill `updatedAt`에 대한 Unix 밀리초 하한입니다.
- `endDate` (필수): Skill `updatedAt`에 대한 Unix 밀리초 상한입니다.
...
- `clean` 또는 `suspicious` 스캔이 있는 현재 GitHub 기반 Skills에는
  ...
  content hash 및 archive URL과 함께 포함됩니다. ClawHub 호스팅 소스 파일은 포함하지 않습니다.
...
- `family` (선택): `code-plugin` 또는 `bundle-plugin`. 생략하면 두 Plugin
  제품군을 모두 의미합니다.
...
- `version.sha256hash`는 오래된 클라이언트를 위한 더 이상 사용되지 않는 호환성 메타데이터입니다. 이는
  `/api/v1/packages/{name}/download`가 반환하는 정확한 ZIP 바이트를 해시합니다.
  최신 클라이언트는 표준 릴리스 아티팩트를 식별하는
...
- `release.releaseId`, `release.version`, `release.createdAt`은 평가된
  정확한 릴리스를 식별합니다.
- 릴리스 아티팩트에 대해 알려져 있으면 `release.artifactKind`, `release.artifactSha256`, `release.npmIntegrity`,
  `release.npmShasum`, `release.npmTarballName`이 존재합니다.
- `trust.scanStatus`는 스캐너 입력과 수동 릴리스 모더레이션에서 파생된
  유효 신뢰 상태입니다.
- `trust.moderationState`는 nullable입니다. 수동 릴리스
  모더레이션이 없으면 `null`입니다.
- `trust.blockedFromDownload`는 설치 차단 신호입니다. OpenClaw와 기타
  설치 클라이언트는 스캐너 또는 모더레이션 필드에서 차단 규칙을 다시 도출하는 대신,
  이 값이 `true`일 때 설치를 차단해야 합니다.
- `trust.reasons`는 사용자 표시 및 감사용 설명 목록입니다. 사유 코드는
...
- 이 엔드포인트는 버전 정확합니다. 클라이언트는 최신 패키지
  메타데이터를 읽은 직후가 아니라, 설치하려는 패키지 버전을 해석한 후 호출해야 합니다.
- 비공개 패키지는 호출자가 소유 게시자를 읽을 수 없는 한 `404`를 반환합니다.
- 이 엔드포인트는 의도적으로 소유자/모더레이터 모더레이션
  엔드포인트보다 범위가 좁습니다. 설치 결정과 공개 설명을 노출하며,
...
- 기존 패키지 버전은 `legacy-zip` 아티팩트와 기존 ZIP
  `downloadUrl`을 반환합니다.
- ClawPack 버전은 `npm-pack` 아티팩트, npm 무결성 필드,
  `tarballUrl`, 기존 ZIP 호환성 URL을 반환합니다.
- 이는 OpenClaw 리졸버 표면이며, 공유 URL에서 아카이브 형식을
  추측하지 않도록 합니다.
...
- 기존 ZIP 버전은 `/api/v1/packages/{name}/download?version=`으로 리디렉션됩니다.
- 다운로드 속도 버킷을 사용합니다.
...
- `open`: 의심됨, 악성, 보류 중, 격리됨, 취소됨 또는 신고된 릴리스.
...
모더레이터 검토를 위해 패키지를 신고합니다. 신고는 패키지 수준이며,
선택적으로 버전에 연결됩니다. 이는 모더레이션 큐에 반영되지만
그 자체로 다운로드를 자동으로 숨기거나 차단하지는 않습니다. 모더레이터는 릴리스 모더레이션을 사용하여
...
패키지 모더레이션 가시성을 위한 소유자/중재자 엔드포인트.
...
설정할 때는 생략할 수 있습니다. 확인된 신고에 릴리스 모더레이션을 같은 감사 가능한
워크플로에서 적용하려면 `finalAction: "quarantine"` 또는
...
- `quarantined`: 후속 조치 대기 중 차단되었습니다.
- `revoked`: 이전에 신뢰되었던 릴리스가 차단되었습니다.
...
- 비공개 패키지는 호출자가 소유자인 경우를 제외하고 `404`를 반환합니다.
...
호스팅된 skill 버전 ZIP을 다운로드하거나, 호스팅된 버전이 없고
`clean` 또는 `suspicious` 검사가 있는 현재 GitHub 기반 skill에 대한 GitHub 소스 인계를
반환합니다.
...
- 선택적 페이로드 필드: `ownerHandle`. 존재하면 API가 서버 측에서 해당
  게시자를 확인하고 행위자에게 게시자 접근 권한을 요구합니다.
- 선택적 페이로드 필드: `migrateOwner`. `ownerHandle`과 함께 `true`이면,
  행위자가 현재 및 대상 게시자 모두에서 관리자/소유자인 경우 기존 skill을 해당
  소유자로 이동할 수 있습니다. 이 명시적 선택이 없으면 소유자 변경은
  거부됩니다.
...
- `openclaw` 조직 게시자와 현재 `openclaw` 조직 멤버의
...
존재하면 `reason`은 skill 모더레이션 메모로 저장되고 감사 로그에 복사됩니다.
소유자가 시작한 소프트 삭제는 slug를 30일 동안 예약한 뒤, 다른 게시자가
해당 slug를 주장할 수 있습니다. 이 만료가 적용되면 삭제 응답에 `slugReservedUntil`이 포함됩니다.
...
인증된 셀프 서비스 조직 게시자 생성입니다. 새 조직 게시자를 만들고
...
패키지 이름은 릴리스 행이 없는 비공개 플레이스홀더 패키지가 되므로, 같은
...
개인 게시자를 복구합니다. 요청은 두 개의 불변 GitHub
제공자 계정 ID를 모두 지정해야 합니다. 변경 가능한 핸들은 운영자용 가드로만 사용됩니다.

엔드포인트는 기본적으로 dry-run으로 동작합니다. 복구를 적용하려면 직원이 두 GitHub 주체 간의 연속성을 독립적으로 확인한 뒤 `dryRun: false`와
`confirmIdentityVerified: true`가 필요합니다. 대상 사용자의 현재 개인
게시자에 Skills, 패키지 또는 GitHub skill 소스가 있으면 복구는 fail-closed로 실패합니다.
복구는 또한 복구된 게시자의 Skills, skill 슬러그 별칭, 패키지, 패키지 검사기 경고, 파생 검색 다이제스트 행의 레거시 `ownerUserId` 필드를 마이그레이션하여
직접 소유자 경로가 새 게시자 권한과 일치하도록 합니다. 복구된 핸들의 활성 보호 핸들
...
사용자를 차단하고 소유한 Skills를 hard-delete합니다(모더레이터/관리자만).
...
사용자 차단을 해제하고 자격이 있는 Skills를 복원합니다(관리자만).
...
(관리자만). `dryRun`이 `false`가 아니면 기본값은 dry-run입니다.
...
`POST /api/cli/upload-url`는 `uploadUrl`과 `uploadTicket`을 반환합니다. ClawPack tarball을 스테이징하는 패키지
