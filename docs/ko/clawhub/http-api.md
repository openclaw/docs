...
x-i18n:
    generated_at: "2026-07-02T08:08:51Z"
    model: gpt-5.5
...
타사 디렉터리는 공개 읽기 엔드포인트를 사용해 ClawHub skills를 나열하거나 검색할 수 있습니다. 결과를 캐시하고, `429`/`Retry-After`를 준수하며, 사용자를 정식 ClawHub 목록(`https://clawhub.ai/<owner>/skills/<slug>`)으로 다시 연결하고, 타사 사이트를 ClawHub가 보증한다는 인상을 주지 마세요. 공개 API 표면 밖에서 숨김, 비공개 또는 검토 차단된 콘텐츠를 미러링하려고 시도하지 마세요.

웹 slug 바로가기는 레지스트리 패밀리 전체에서 해석되지만, API 클라이언트는 라우트
우선순위를 재구성하는 대신 읽기 엔드포인트가 반환한 정식 URL을 사용해야 합니다.
...
- 신뢰할 수 있는 클라이언트 IP가 없으면 익명 요청은 속도 제한 종류로만
...
- 관련성은 인기도보다 더 강합니다. 정확한 slug 또는 표시 이름 토큰 일치는 참여도가 훨씬 높은 느슨한 일치보다 높은 순위에 오를 수 있습니다.
...
- 사용자가 실제로 검색할 용어를 표시 이름, 요약 및 태그에 넣으세요. 독립형 slug 토큰은 유지하려는 안정적인 식별자이기도 할 때만 사용하세요.
- 새 slug가 더 나은 장기 정식 이름이 아닌 한, 단일 쿼리를 쫓기 위해 slug를 변경하지 마세요. 이전 slug는 리디렉션 별칭이 되지만, 정식 URL, 표시되는 slug 및 향후 검색 다이제스트는 새 slug를 사용합니다.
- 이름 변경 별칭은 이전 URL과 레지스트리를 통해 해석되는 설치의 해석을 보존하지만, 검색 순위는 이름 변경이 인덱싱된 후의 정식 skill 메타데이터를 기준으로 합니다. 기존 통계는 skill에 유지됩니다.
- skill이 예기치 않게 보이지 않으면 순위 관련 메타데이터를 변경하기 전에 로그인한 상태에서 `clawhub inspect @owner/slug`로 검토 상태를 먼저 확인하세요.
...
- `nonSuspiciousOnly=true`일 때는 의심스러운 skills가 페이지 검색 후 필터링되므로 커서 기반 정렬이 한 페이지에서 `limit`보다 적은 항목을 반환할 수 있습니다.
...
- `metadata.os`: skill frontmatter에 선언된 OS 제한(예: `["macos"]`, `["linux"]`). 선언되지 않으면 `null`입니다.
- `metadata.systems`: Nix 시스템 대상(예: `["aarch64-darwin", "x86_64-linux"]`). 선언되지 않으면 `null`입니다.
...
- 증거는 공개 호출자에게 수정 처리되며 소유자/검토자에게만 원본 스니펫이 포함됩니다.

### `POST /api/v1/skills/{slug}/report`

검토자 검토를 위해 skill을 신고합니다. 신고는 skill 수준이며, 선택적으로
...
설정할 때는 생략할 수 있습니다. 동일한 감사 가능한 워크플로에서 skill을 숨기려면
...
- `moderation`은 최신 버전에서 파생된 현재 스킬 수준 조정 스냅샷입니다.
...
- 스킬 또는 Plugin에 대한 소유자/게시자 관리 액세스 권한, 또는 플랫폼 모더레이터/관리자 권한이 필요합니다.
...
관리자 전용 정식 배치 상태 경로입니다. `{ "jobIds": ["..."] }`를 허용하며 레거시 `POST /api/v1/skills/-/rescan-batch/status`와 동일한 집계 카운터를 반환합니다.

### `GET /api/v1/skills/{slug}/verify`

`clawhub skill verify`에서 사용하는 스킬 카드 검증 엔벌로프를 반환합니다.
...
- 선택된 버전에 생성된 스킬 카드가 있고, 조정에 의해 멀웨어로 차단되지 않았으며, ClawScan 검증이 깨끗한 경우에만 `ok`가 `true`입니다.
...
정확한 스킬 버전에 대한 현재 압축 보안 판정을 반환합니다. 이 컬렉션 엔드포인트는 OpenClaw Control UI처럼 표시해야 하는 설치된 ClawHub 스킬 버전을 이미 알고 있는 클라이언트를 대상으로 합니다.
...
- 결과는 항목별로 제공됩니다. 하나의 스킬 또는 버전이 없어도 전체 응답이 실패하지 않습니다.
- 응답은 보안 전용입니다. 스킬 카드 데이터, 생성된 카드 상태, 아티팩트 파일 목록, 자세한 스캐너 페이로드는 포함하지 않습니다.
- `security.signals`에는 상태 수준의 보조 증거만 포함됩니다. 전체 스캐너 세부 정보는 `/scan` 또는 ClawHub 보안 감사 페이지를 사용하세요.
- `security.signals.dependencyRegistry`는 v1 응답 호환성을 위해 유지되지만, 의존성 레지스트리 존재 스캐너는 폐기되었으며 이 키는 항상 `null`입니다.
- 스킬 카드 부재는 이 엔드포인트의 `ok`, `decision`, `reasons`에 영향을 주지 않습니다. 클라이언트는 카드 콘텐츠가 필요할 때 설치된 `skill-card.md`를 로컬에서 읽어야 합니다.
- 단일 스킬 스킬 카드 검증 엔벌로프가 필요하면 `/verify`를, 생성된 카드 마크다운이 필요하면 `/card`를, 자세한 스캐너 데이터가 필요하면 `/scan`을 사용하세요.
...
- `family`, `channel`, `isOfficial`, `featured`, `highlightedOnly` 또는
  `sort`의 값이 유효하지 않으면 `400`을 반환합니다. 알 수 없는 쿼리 매개변수는 무시됩니다.
...
- `family`, `channel`, `isOfficial`, `featured` 또는 `highlightedOnly`의 값이
  유효하지 않으면 `400`을 반환합니다. 알 수 없는 쿼리 매개변수는 무시됩니다.
- 익명 호출자는 공개 패키지 채널만 볼 수 있습니다.
...
- `startDate` (필수): Skill `updatedAt`에 대한 Unix 밀리초 하한값.
- `endDate` (필수): Skill `updatedAt`에 대한 Unix 밀리초 상한값.
...
- `clean` 또는 `suspicious` 스캔이 있는 현재 GitHub 기반 Skills에는
  ...
  콘텐츠 해시, 아카이브 URL이 포함된 `_source_handoff.json`이 포함됩니다. ClawHub 호스팅 소스 파일은 포함하지 않습니다.
...
- `_manifest.json`은 항상 ZIP 루트에 포함됩니다.
- 개별 plugins 또는 파일을 내보낼 수 없을 때 `_errors.json`이 포함됩니다.
...
- `trust.moderationState`는 null 가능입니다. 수동 릴리스 중재가 없으면 `null`입니다.
- `trust.blockedFromDownload`는 설치 차단 신호입니다. OpenClaw 및 기타 설치 클라이언트는
  스캐너 또는 중재 필드에서 차단 규칙을 다시 파생하는 대신 이 값이 `true`일 때 설치를 차단해야 합니다.
...
- `trust.stale`은 신뢰 요약이 오래된 입력에서 계산되었으며,
  높은 신뢰도의 허용 결정을 내리기 전에 새로 고침이 필요하다고 취급해야 함을 의미합니다.

참고:

- 이 엔드포인트는 버전 정확합니다. 클라이언트는 단순히 최신 패키지 메타데이터를 읽은 뒤가 아니라
...
연결됩니다. 신고는 중재 대기열에 반영되지만 그 자체로 다운로드를 자동 숨김 처리하거나
차단하지 않습니다. 중재자는 릴리스 중재를 사용하여 아티팩트를 승인, 격리 또는 폐기해야 합니다.
...
`note`는 `confirmed`와 `dismissed`에 필요합니다. `status`를 다시 `open`으로
...
호스팅된 skill 버전 ZIP을 다운로드하거나, `clean` 또는 `suspicious` 스캔을 갖고
...
- 선택적 페이로드 필드: `ownerHandle`. 존재하는 경우 API가 서버 측에서 해당
  게시자를 해석하고 행위자에게 게시자 접근 권한이 필요합니다.
- 선택적 페이로드 필드: `migrateOwner`. `ownerHandle`과 함께 `true`이면,
  행위자가 현재 게시자와 대상 게시자 모두에서 관리자/소유자인 경우 기존 skill이
  해당 소유자로 이동할 수 있습니다. 이 옵트인이 없으면 소유자 변경은 거부됩니다.
...
소유자가 시작한 소프트 삭제는 30일 동안 slug를 예약한 후 다른 게시자가
그 slug를 요청할 수 있습니다. 이 만료가 적용되는 경우 삭제 응답에는
`slugReservedUntil`이 포함됩니다. 중재자/관리자 숨김 및 보안 제거는 이런 방식으로 만료되지 않습니다.
...
새로 생성된 조직의 경우 `memberHandle`을 제공하세요. 실행 중인 관리자는 멤버로 추가되지 않습니다.
...
패키지 이름은 릴리스 행이 없는 비공개 자리표시자 패키지가 되므로, 같은 소유자가 나중에
...
개인 게시자를 복구합니다. 요청은 변경 불가능한 두 GitHub provider account id를 모두 지정해야 합니다.
...
스킬, 패키지 또는 GitHub 스킬 소스가 있으면 복구는 fail-closed됩니다.
복구는 또한 복구된 게시자의 스킬, 스킬 슬러그 별칭, 패키지, 패키지 검사기 경고,
파생 검색 다이제스트 행의 레거시 `ownerUserId` 필드도 마이그레이션하여
직접 소유자 경로가 새 게시자 권한과 일치하도록 합니다. 복구된 핸들에 대한 활성
보호 핸들 예약도 대체 사용자에게 재할당되어 이후 프로필 동기화가 이전 사용자의
경합 권한을 복원할 수 없게 합니다. 각 기본 테이블은 적용 트랜잭션당
100개 행으로 제한됩니다. 더 큰 복구는 먼저 재개 가능한 소유자 마이그레이션을
사용해야 합니다. GitHub 스킬 소스는 게시자 범위이며 다시 작성되지 않고 확인된
것으로 보고됩니다.
...
(관리자 전용). `dryRun`이 `false`가 아니면 기본적으로 dry-run입니다.
...
스테이징하는 패키지 게시는 결과 스토리지 ID를 `clawpack`으로, 반환된 티켓을
