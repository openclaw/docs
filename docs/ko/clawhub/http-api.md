...
x-i18n:
    generated_at: "2026-07-01T15:21:18Z"
    model: gpt-5.5
...
타사 디렉터리는 공개 읽기 엔드포인트를 사용하여 ClawHub Skills를 나열하거나 검색할 수 있습니다. 결과를 캐시하고, `429`/`Retry-After`를 준수하며, 사용자를 표준 ClawHub 목록(`https://clawhub.ai/<owner>/skills/<slug>`)으로 다시 연결하고, 타사 사이트를 ClawHub가 보증한다고 암시하지 마세요. 공개 API 표면 밖의 숨김, 비공개 또는 모더레이션으로 차단된 콘텐츠를 미러링하려고 시도하지 마세요.

웹 슬러그 바로 가기는 레지스트리 패밀리 전반에서 해석되지만, API 클라이언트는 라우트 우선순위를 재구성하는 대신 읽기 엔드포인트가 반환하는 표준 URL을 사용해야 합니다.
...
- 인증된 쓰기 엔드포인트는 서버가 이유를 알고 있을 때 단순한 `Unauthorized`만 반환해서는 안 됩니다. 누락된 토큰, 유효하지 않거나 폐기된 토큰, 삭제/차단/비활성화된 계정은 각각 CLI 클라이언트가 사용자에게 무엇이 차단했는지 알릴 수 있도록 실행 가능한 텍스트를 받아야 합니다.
...
- 관련도는 인기도보다 더 강합니다. 정확한 슬러그 또는 표시 이름 토큰 일치는 참여도가 훨씬 높은 느슨한 일치보다 더 높은 순위를 차지할 수 있습니다.
- ASCII 텍스트는 단어 및 구두점 경계에서 토큰화됩니다. 예를 들어 `personal-map`에는 독립적인 `map` 토큰이 포함되지만, `amap-jsapi-skill`에는 `amap`, `jsapi`, `skill`이 포함됩니다. 따라서 `map`을 검색하면 `amap-jsapi-skill`보다 `personal-map`이 더 강한 어휘 일치를 얻습니다.
- 인기도는 로그 스케일로 조정되고 상한이 적용됩니다. 참여도가 높은 Skills라도 쿼리 텍스트 일치가 약하면 더 낮은 순위가 될 수 있습니다.
...
- 이름 변경 별칭은 이전 URL 및 레지스트리를 통해 해석되는 설치에 대한 해석을 보존하지만, 검색 순위는 이름 변경이 인덱싱된 후의 표준 Skill 메타데이터를 기준으로 합니다. 기존 통계는 Skill에 유지됩니다.
...
- `cursor`(선택 사항): `trending`이 아닌 모든 정렬을 위한 페이지 매김 커서
...
- `nonSuspiciousOnly=true`일 때 의심스러운 Skills가 페이지 조회 후 필터링되므로 커서 기반 정렬은 한 페이지에서 `limit`보다 적은 항목을 반환할 수 있습니다.
- 존재할 경우 `nextCursor`를 사용하여 페이지 매김을 계속하세요. 짧은 페이지 자체가 결과의 끝을 의미하지는 않습니다.
...
- 공개 호출자에게는 증거가 편집되며, 원시 스니펫은 소유자/모더레이터에게만 포함됩니다.

### `POST /api/v1/skills/{slug}/report`

모더레이터 검토를 위해 Skill을 신고합니다. 신고는 Skill 수준이며, 선택적으로 버전에 연결되고 Skill 신고 큐로 전달됩니다.
...
`note`는 `confirmed` 및 `dismissed`에 필요합니다. `status`를 다시 `open`으로 설정할 때는 생략할 수 있습니다. 같은 감사 가능한 워크플로에서 Skill을 숨기려면 분류된 신고와 함께 `finalAction: "hide"`를 전달하세요.
...
- 스캔 작업은 비동기입니다. 수동 스캔 요청은 일반 게시/백필 작업보다 우선순위가 높지만, 완료는 여전히 워커 가용성에 따라 달라집니다.
...
- 대기 중에는 `queue.queuedAhead`와 `queue.position`을 반환하므로 클라이언트가 요청보다 앞선 우선순위 수동 스캔 수를 표시할 수 있습니다. 매우 큰 큐는 제한되며 `queuedAheadIsEstimate: true`로 보고됩니다.
...
- Skill 또는 Plugin에 대한 소유자/게시자 관리 권한 또는 플랫폼 모더레이터/관리자 권한이 필요합니다.
...
- 선택한 버전에 생성된 Skill Card가 있고, 모더레이션에 의해 악성 코드로 차단되지 않았으며, ClawScan 검증이 깨끗한 경우에만 `ok`가 `true`입니다.
- Skill ID, 게시자 ID, 선택한 버전 메타데이터는 최상위 엔벨로프 필드(`slug`, `displayName`, `publisherHandle`, `version`, `resolvedFrom`, `tag`, `createdAt`)이므로 셸 자동화에서 중첩 래퍼를 풀지 않고 읽을 수 있습니다.
...
- `provenance`는 게시 또는 가져오기 중 ClawHub가 GitHub 저장소/ref/commit/path를 확인하고 저장한 경우에만 `server-resolved-github-import`이며, 그렇지 않으면 `unavailable`입니다.

### `POST /api/v1/skills/-/security-verdicts`

정확한 Skill 버전에 대한 현재 압축 보안 판정을 반환합니다. 이 컬렉션 엔드포인트는 OpenClaw Control UI처럼 표시해야 하는 설치된 ClawHub Skill 버전을 이미 알고 있는 클라이언트를 위한 것입니다.
...
- 결과는 항목별입니다. Skill 또는 버전 하나가 누락되어도 전체 응답이 실패하지 않습니다.
...
- `security.signals.dependencyRegistry`는 v1 응답 호환성을 위해 유지되지만, 의존성 레지스트리 존재 스캐너는 폐기되었으며 이 키는 항상 `null`입니다.
- Skill Card 부재는 이 엔드포인트의 `ok`, `decision` 또는 `reasons`에 영향을 주지 않습니다. 카드 콘텐츠가 필요할 때 클라이언트는 설치된 `skill-card.md`를 로컬에서 읽어야 합니다.
- 단일 Skill Skill Card 검증 엔벨로프가 필요할 때는 `/verify`, 생성된 카드 마크다운이 필요할 때는 `/card`, 상세 스캐너 데이터가 필요할 때는 `/scan`을 사용하세요.
...
- `clean` 또는 `suspicious` 스캔이 있는 현재 GitHub 기반 skills에는
  repo, commit, path, content hash 및 archive URL과 함께
  `_source_handoff.json`에 `sourceRef: "public-github"`가 포함됩니다. ClawHub 호스팅 소스 파일은 포함하지 않습니다.
...
- `version.artifact.kind`는 기존 방식 패키지 아카이브의 경우 `legacy-zip`,
...
- `trust.scanStatus`는 스캐너 입력과 수동 릴리스 중재에서 파생된 유효 신뢰
  상태입니다.
- `trust.moderationState`는 null 가능 값입니다. 수동 릴리스 중재가 없으면
  `null`입니다.
- `trust.blockedFromDownload`는 설치 차단 신호입니다. OpenClaw 및 기타 설치
  클라이언트는 스캐너 또는 중재 필드에서 차단 규칙을 다시 파생하지 말고, 이 값이
...
- 이 엔드포인트는 버전 정확합니다. 클라이언트는 최신 패키지 메타데이터를 읽은 직후가
  아니라, 설치하려는 패키지 버전을 확인한 뒤 호출해야 합니다.
- 호출자가 소유 게시자를 읽을 수 없는 경우 비공개 패키지는 `404`를 반환합니다.
- 이 엔드포인트는 소유자/중재자 중재 엔드포인트보다 의도적으로 더 좁습니다.
  제보자 신원, 신고 본문, 비공개 증거 또는 내부 검토 일정이 아니라 설치 결정과
  공개 설명을 노출합니다.
...
- `open`: 의심스러운, 악성, 대기 중, 격리된, 철회된 또는 신고된 릴리스입니다.
- `blocked`: 격리된, 철회된 또는 악성 릴리스입니다.
- `manual`: 수동 중재 오버라이드가 있는 모든 릴리스입니다.
- `all`: 수동 오버라이드, 깨끗하지 않은 스캔 상태 또는 패키지 신고가 있는 모든 릴리스입니다.
...
자동 숨김 처리하거나 차단하지는 않습니다. 중재자는 릴리스 중재를 사용해 아티팩트를
승인, 격리 또는 철회해야 합니다.
...
패키지 중재 공개 상태를 위한 소유자/중재자 엔드포인트입니다.

인증:

- 패키지 소유자, 게시자 구성원, 중재자 또는 관리자 사용자용 API 토큰이 필요합니다.
...
`note`는 `confirmed` 및 `dismissed`에 필요합니다. `status`를 다시 `open`으로 설정할 때는 생략할 수 있습니다. 확인된 신고에 `finalAction: "quarantine"` 또는 `finalAction: "revoke"`를 전달하면 동일한 감사 가능한 워크플로에서 릴리스 중재를 적용합니다.
...
격리되거나 취소된 릴리스는 아티팩트 다운로드 경로에서 `403`을 반환합니다. 모든 변경 사항은 감사 로그 항목을 기록합니다.
...
- `slug`(필수)
- `hash`(필수): 번들 fingerprint의 64자 16진수 sha256
...
호스팅된 skill 버전 ZIP을 다운로드하거나, `clean` 또는 `suspicious` 스캔이 있고 호스팅된 버전이 없는 현재 GitHub 기반 skill에 대한 GitHub 소스 인계를 반환합니다.
...
- GitHub 기반 skill 인계는 바이트를 프록시하거나 미러링하지 않습니다. JSON 응답에는 `sourceRef: "public-github"`, `repo`, `commit`, `path`, `contentHash`, `archiveUrl`이 포함됩니다. 스캔/현재 상태는 게이트이며 성공 페이로드 메타데이터로 포함되지 않습니다.
...
- 선택적 페이로드 필드: `ownerHandle`. 있으면 API가 서버 측에서 해당 게시자를 확인하고 행위자에게 게시자 액세스 권한이 있어야 합니다.
- 선택적 페이로드 필드: `migrateOwner`. `ownerHandle`과 함께 `true`이면, 행위자가 현재 게시자와 대상 게시자 모두에서 관리자/소유자인 경우 기존 skill을 해당 소유자로 이동할 수 있습니다. 이 옵트인이 없으면 소유자 변경이 거부됩니다.
...
- 허용되는 폼 필드는 `payload`, 반복되는 `files` blob 또는 하나의 `clawpack` tarball 참조입니다. `clawpack`은 `.tgz` blob이거나 upload-url 플로에서 반환된 storage id일 수 있습니다. 스테이징된 storage-id 게시에는 해당 업로드 URL과 함께 반환된 `clawpackUploadTicket`도 포함해야 합니다.
- `files` 또는 `clawpack` 중 하나만 사용하고, 같은 요청에 둘 다 사용하지 마세요.
- JSON 본문 및 호출자가 제공한 `payload.files` / `payload.artifact` 메타데이터는 거부됩니다.
- 직접 multipart 게시 요청은 18MB로 제한됩니다. ClawPack tarball은 upload-url 플로를 사용해 120MB tarball 한도까지 사용할 수 있습니다.
...
- `openclaw` 조직 게시자 및 현재 `openclaw` 조직 구성원의 개인 게시자만 `official` 채널에 게시할 수 있습니다.
- 대리 게시는 대상 소유자 계정을 기준으로 official-channel 자격을 계속 검증합니다.
...
있으면 `reason`은 skill 중재 메모로 저장되고 감사 로그에 복사됩니다.
소유자가 시작한 soft-delete는 slug를 30일 동안 예약한 뒤, 다른 게시자가 해당 slug를 요청할 수 있습니다. 이 만료가 적용되면 삭제 응답에 `slugReservedUntil`이 포함됩니다.
...
관리자 전용입니다. 핸들에 대한 조직 게시자가 존재하는지 보장합니다. 핸들이 여전히 레거시 공유 사용자/개인 게시자를 가리키는 경우, 엔드포인트는 먼저 이를 조직 게시자로 마이그레이션합니다.
...
인증된 셀프서비스 조직 게시자 생성입니다. 새 조직 게시자를 생성하고 호출자를 소유자로 추가합니다. 이 엔드포인트는 기존 사용자/개인 핸들을 마이그레이션하지 않으며 게시자를 신뢰됨/공식으로 표시하지 않습니다.

- 본문: `{ "handle": "opik", "displayName": "Opik" }`
- 응답: `{ "ok": true, "publisherId": "...", "handle": "opik", "created": true, "trusted": false }`
- 핸들이 게시자, 사용자 또는 개인 게시자에 이미 사용 중이면 `409`를 반환합니다.

### `POST /api/v1/users/reserve`

관리자 전용입니다. 릴리스를 게시하지 않고 정당한 소유자를 위해 루트 slug와 패키지 이름을 예약합니다. 패키지 이름은 릴리스 행이 없는 비공개 placeholder 패키지가 되므로, 동일한 소유자가 나중에 실제 code-plugin 또는 bundle-plugin 릴리스를 해당 이름으로 게시할 수 있습니다.
...
관리자 전용입니다. Convex Auth 계정 행을 편집하지 않고 검증된 대체 GitHub OAuth 주체에 대한 개인 게시자를 복구합니다. 요청에는 변경 불가능한 GitHub provider account id 둘 다를 명시해야 합니다. 변경 가능한 핸들은 운영자 대상 가드로만 사용됩니다.
...
또한 복구는 복구된 게시자의 스킬, 스킬 슬러그 별칭, 패키지, 패키지 검사기 경고,
파생 검색 다이제스트 행의 레거시 `ownerUserId` 필드도 마이그레이션하여
...
스테이징하는 패키지 게시에서는 결과 저장소 ID를 `clawpack`으로,
