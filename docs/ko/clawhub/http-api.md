...
x-i18n:
    generated_at: "2026-07-01T20:15:04Z"
    model: gpt-5.5
...
타사 디렉터리는 공개 읽기 엔드포인트를 사용해 ClawHub Skills를 나열하거나 검색할 수 있습니다. 결과를 캐시하고, `429`/`Retry-After`를 준수하며, 사용자를 표준 ClawHub 목록(`https://clawhub.ai/<owner>/skills/<slug>`)으로 다시 연결하고, 타사 사이트가 ClawHub의 보증을 받는다는 인상을 주지 마세요. 숨겨진 콘텐츠, 비공개 콘텐츠 또는 모더레이션으로 차단된 콘텐츠를 공개 API 표면 밖에서 미러링하려고 시도하지 마세요.

웹 slug 바로가기는 레지스트리 계열 전반에서 확인되지만, API 클라이언트는 라우트
...
- 신뢰할 수 있는 클라이언트 IP를 사용할 수 없으면 익명 요청은
  속도 제한 종류로만 범위가 지정된 폴백 버킷을 사용합니다. 이러한 폴백 버킷에는
...
여기에는 검증 실패(`400`), 누락된 공개 리소스(`404`), 인증 및
...
- 결과는 관련도 순서(임베딩 유사도 + 정확한 slug/name 토큰 부스트 + 작은 인기도 사전값)로 반환됩니다.
- 관련도는 인기도보다 더 강합니다. 정확한 slug 또는 표시 이름 토큰 일치는 참여도가 훨씬 높은 느슨한 일치보다 더 높은 순위를 차지할 수 있습니다.
...
- 인기도는 로그 스케일로 조정되고 상한이 적용됩니다. 참여도가 높은 Skills라도 쿼리 텍스트와의 일치가 약하면 더 낮은 순위를 차지할 수 있습니다.
...
- 새 slug가 장기적으로 더 나은 표준 이름이 아닌 한, 단일 쿼리를 쫓기 위해 slug를 바꾸지 마세요. 이전 slug는 리디렉션 별칭이 되지만, 표준 URL, 표시되는 slug, 향후 검색 다이제스트는 새 slug를 사용합니다.
- 이름 변경 별칭은 이전 URL과 레지스트리를 통해 확인되는 설치의 확인을 보존하지만, 검색 순위는 이름 변경이 인덱싱된 후의 표준 Skill 메타데이터를 기반으로 합니다. 기존 통계는 Skill에 유지됩니다.
...
- `nonSuspiciousOnly=true`일 때 커서 기반 정렬은 페이지 검색 후 의심스러운 Skills가 필터링되기 때문에 한 페이지에서 `limit`보다 적은 항목을 반환할 수 있습니다.
...
- 소유자 이름 변경/병합 흐름으로 생성된 이전 slug는 표준 Skill로 확인됩니다.
...
- 증거는 공개 호출자에게는 수정되어 제공되며, 소유자/모더레이터에게만 원시 스니펫이 포함됩니다.
...
설정할 때는 생략할 수 있습니다. 감사 가능한 동일 워크플로에서 Skill을 숨기려면 분류된
...
- 기록 버전을 쿼리할 때는 `moderation`과 `security`를 같은 버전 컨텍스트로 취급하기 전에 `moderation.matchesRequestedVersion` 및 `moderation.sourceVersion`을 확인하세요.
...
- 대기 중에는 `queue.queuedAhead`와 `queue.position`을 반환하여 클라이언트가 해당 요청보다 앞선 우선순위 수동 스캔이 몇 개인지 표시할 수 있게 합니다. 매우 큰 큐는 제한되며 `queuedAheadIsEstimate: true`로 보고됩니다.
...
제출된 버전에 대해 저장된 보고서 아카이브를 제공하는 인증된 엔드포인트입니다.
...
- `ok`는 선택한 버전에 생성된 스킬 카드가 있고, 모더레이션에 의해 멀웨어로 차단되지 않았으며, ClawScan 검증이 깨끗한 경우에만 `true`입니다.
- 스킬 ID, 게시자 ID, 선택한 버전 메타데이터는 최상위 엔벨로프 필드(`slug`, `displayName`, `publisherHandle`, `version`, `resolvedFrom`, `tag`, `createdAt`)이므로 셸 자동화가 중첩 래퍼를 풀지 않고도 읽을 수 있습니다.
...
- `provenance`는 게시 또는 가져오기 중 ClawHub가 GitHub 저장소/ref/커밋/경로를 해석하고 저장한 경우에만 `server-resolved-github-import`입니다. 그렇지 않으면 `unavailable`입니다.
...
- 결과는 항목별로 제공됩니다. 하나의 스킬 또는 버전이 누락되어도 전체 응답은 실패하지 않습니다.
- 응답은 보안 전용입니다. 스킬 카드 데이터, 생성된 카드 상태, 아티팩트 파일 목록 또는 상세 스캐너 페이로드를 포함하지 않습니다.
- `security.signals`에는 상태 수준의 지원 증거만 포함됩니다. 전체 스캐너 세부 정보가 필요하면 `/scan` 또는 ClawHub 보안 감사 페이지를 사용하세요.
- `security.signals.dependencyRegistry`는 v1 응답 호환성을 위해 유지되지만, 의존성 레지스트리 존재 스캐너는 폐기되었으며 이 키는 항상 `null`입니다.
- 스킬 카드가 없어도 이 엔드포인트의 `ok`, `decision`, `reasons`에는 영향을 주지 않습니다. 카드 콘텐츠가 필요한 클라이언트는 설치된 `skill-card.md`를 로컬에서 읽어야 합니다.
- 단일 스킬 스킬 카드 검증 엔벨로프가 필요하면 `/verify`를, 생성된 카드 Markdown이 필요하면 `/card`를, 상세 스캐너 데이터가 필요하면 `/scan`을 사용하세요.
...
- `category`(선택 사항): plugin 카테고리 필터. 요청 범위가 plugin 패키지(`/api/v1/plugins`,
  `/api/v1/code-plugins`, `/api/v1/bundle-plugins` 또는 `family=code-plugin`/`family=bundle-plugin`이 있는 패키지 엔드포인트)로 지정된 경우에만 지원됩니다. 제어된 카테고리와
...
파일 메타데이터, 호환성, 검증, 아티팩트 메타데이터, 스캔 데이터를 포함하여 하나의 패키지 버전을 반환합니다.
...
- `version.sha256hash`는 오래된 클라이언트를 위한 폐기 예정 호환성 메타데이터입니다. 이는
...
- `trust.scanStatus`는 스캐너 입력과 수동 릴리스 모더레이션에서 도출된
  유효 신뢰 상태입니다.
- `trust.moderationState`는 null일 수 있습니다. 수동 릴리스 모더레이션이
  없으면 `null`입니다.
...
- `trust.reasons`는 사용자에게 표시되는 감사 설명 목록입니다. 이유 코드는
  `manual:quarantined`, `scan:malicious`, `package:malicious`처럼 안정적이고
  간결한 문자열입니다.
...
- 이 엔드포인트는 버전 단위로 정확합니다. 클라이언트는 단순히 최신
  패키지 메타데이터를 읽은 뒤가 아니라, 설치하려는 패키지 버전을 확인한 뒤
  이 엔드포인트를 호출해야 합니다.
- 호출자가 소유 게시자를 읽을 수 없으면 비공개 패키지는 `404`를 반환합니다.
- 이 엔드포인트는 소유자/모더레이터 모더레이션 엔드포인트보다 의도적으로 더 좁습니다. 신고자 신원, 신고 본문, 비공개 증거 또는 내부 검토 타임라인이 아니라
  설치 결정과 공개 설명을 노출합니다.
...
버전에 연결됩니다. 신고는 모더레이션 대기열에 반영되지만 그 자체만으로 다운로드를 자동으로 숨기거나
차단하지 않습니다. 모더레이터는 릴리스 모더레이션을 사용해
...
`open`으로 설정할 때는 생략할 수 있습니다. 동일한 감사 가능 워크플로에서 릴리스
모더레이션을 적용하려면 확인된 신고와 함께 `finalAction: "quarantine"` 또는
...
- `dist.tarball`, `dist.integrity`, `dist.shasum`은 사용자가 원할 경우
...
호스팅된 skill 버전 ZIP을 다운로드하거나, `clean` 또는 `suspicious` 스캔을
통과했고 호스팅된 버전이 없는 현재 GitHub 기반 skill에 대한 GitHub 소스 인계를
반환합니다.
...
- 선택적 페이로드 필드: `migrateOwner`. `ownerHandle`과 함께 `true`이면,
  행위자가 현재 게시자와 대상 게시자 모두의 관리자/소유자인 경우 기존 skill이
  해당 소유자로 이동할 수 있습니다. 이 명시적 동의가 없으면 소유자 변경은
  거부됩니다.
...
- 허용되는 폼 필드는 `payload`, 반복되는 `files` blob 또는 하나의 `clawpack`
  ...
  스토리지 ID일 수 있습니다. 스테이징된 스토리지 ID 게시는 해당 업로드 URL과 함께
...
- 직접 multipart 게시 요청은 18MB로 제한됩니다. ClawPack tarball은
  upload-url 플로를 사용해 최대 120MB tarball 제한까지 사용할 수 있습니다.
- 선택적 페이로드 필드: `ownerHandle`. 존재하는 경우 관리자만 해당 소유자를 대신해 게시할 수 있습니다.

검증 주요 사항:

- `family`는 `code-plugin` 또는 `bundle-plugin`이어야 합니다.
- Plugin 패키지는 `openclaw.plugin.json`이 필요합니다. ClawPack `.tgz` 업로드에는
...
존재하는 경우 `reason`은 skill 모더레이션 메모로 저장되고 감사 로그에 복사됩니다.
소유자가 시작한 소프트 삭제는 slug를 30일 동안 예약하며, 이후 다른 게시자가 해당
slug를 청구할 수 있습니다. 이 만료가 적용되는 경우 삭제 응답에는 `slugReservedUntil`이
포함됩니다. 모더레이터/관리자 숨김 및 보안 제거는 이런 방식으로 만료되지 않습니다.
...
마이그레이션합니다. 새로 생성된 조직의 경우 `memberHandle`을 제공하세요. 작업을 수행하는
...
추가합니다. 이 엔드포인트는 기존 사용자/개인 핸들을 마이그레이션하지 않으며 게시자를
신뢰됨/공식으로 표시하지 않습니다.
...
예약합니다. 패키지 이름은 릴리스 행이 없는 비공개 placeholder 패키지가 되므로, 동일한
소유자가 나중에 실제 code-plugin 또는 bundle-plugin 릴리스를 해당 이름으로 게시할 수
있습니다.
...
대한 개인 게시자를 복구합니다. 요청은 두 불변 GitHub 제공자 계정 ID를 모두 명명해야
합니다. 변경 가능한 핸들은 운영자 대상 보호 장치로만 사용됩니다.

엔드포인트는 기본적으로 드라이런으로 동작합니다. 복구를 적용하려면 직원이 두
GitHub 주체 간의 연속성을 독립적으로 확인한 뒤 `dryRun: false`와
`confirmIdentityVerified: true`가 필요합니다. 대상 사용자의 현재 개인 게시자에
Skills, 패키지 또는 GitHub Skills 소스가 있으면 복구는 실패 시 닫힘 방식으로 실패합니다.
복구는 복구된 게시자의 Skills, Skills 슬러그 별칭, 패키지, 패키지 검사기 경고 및 파생 검색 다이제스트 행의 기존 `ownerUserId` 필드도 마이그레이션하여
직접 소유자 경로가 새 게시자 권한과 일치하도록 합니다. 복구된 핸들에 대한 활성 보호 핸들
예약도 대체 사용자에게 재할당되어 이후 프로필 동기화가 이전 사용자의 경쟁 권한을 복원할 수 없도록 합니다. 각 기본 테이블은 적용 트랜잭션당
...
별표(강조 표시)를 추가/제거합니다. 두 엔드포인트 모두 멱등입니다.
...
`POST /api/cli/upload-url`는 `uploadUrl` 및 `uploadTicket`을 반환합니다. ClawPack 타르볼을 준비하는 패키지
