...
x-i18n:
    generated_at: "2026-07-12T15:01:42Z"
    model: gpt-5.6
...
서드파티 디렉터리는 공개 읽기 엔드포인트를 사용하여 ClawHub Skills를 나열하거나 검색할 수 있습니다. 결과를 캐시하고, `429`/`Retry-After`를 준수하며, 사용자가 표준 ClawHub 목록(`https://clawhub.ai/<owner>/skills/<slug>`)으로 이동할 수 있도록 링크하고, ClawHub가 서드파티 사이트를 보증한다는 인상을 주지 마십시오. 공개 API 범위 밖에서 숨김, 비공개 또는 검토 차단된 콘텐츠를 미러링하려고 시도하지 마십시오.

웹 슬러그 바로 가기는 레지스트리 계열 전반에서 해석되지만, API 클라이언트는 경로 우선순위를 재구성하는 대신 읽기 엔드포인트에서 반환된 표준 URL을 사용해야 합니다.
...
- 서버가 이유를 알고 있다면 인증이 필요한 쓰기 엔드포인트는 단순한 `Unauthorized`만 반환해서는 안 됩니다. 누락된 토큰, 유효하지 않거나 취소된 토큰, 삭제되거나 차단되거나 비활성화된 계정에는 각각 조치 가능한 텍스트를 제공하여 CLI 클라이언트가 사용자에게 무엇 때문에 차단되었는지 알려줄 수 있어야 합니다.
...
- `Retry-After`: `429` 발생 시 재시도하기 전에 대기할 시간(초)
...
- `Retry-After`가 있으면 해당 초만큼 기다린 후 재시도하십시오.
- 재시도가 동시에 몰리지 않도록 지터를 적용한 백오프를 사용하십시오.
...
- 신뢰할 수 있는 클라이언트 IP를 사용할 수 없는 경우 익명 요청은 요청 속도 제한 종류로만 범위가 지정된 대체 버킷을 사용합니다. 이러한 대체 버킷에는 호출자가 제공한 경로, 슬러그, 패키지 이름, 버전, 쿼리 문자열 또는 기타 아티팩트 매개변수가 포함되지 않습니다.

## 오류 응답

공개 v1 오류 응답은 `content-type: text/plain; charset=utf-8`인 일반 텍스트입니다.
여기에는 유효성 검사 실패(`400`), 누락된 공개 리소스(`404`), 인증 및 권한 실패(`401`/`403`), 요청 속도 제한(`429`), 차단된 다운로드가 포함됩니다. 클라이언트는 응답 본문을 사람이 읽을 수 있는 문자열로 읽어야 합니다. 알 수 없는 쿼리 매개변수는 호환성을 위해 무시되지만, 인식되는 쿼리 매개변수에 유효하지 않은 값이 지정되면 `400`을 반환합니다.
...
- 결과는 관련성 순서로 반환됩니다(임베딩 유사도 + 정확한 슬러그/이름 토큰 가중치 + 작은 인기도 사전 가중치).
- 관련성은 인기도보다 우선합니다. 정확한 슬러그 또는 표시 이름 토큰 일치는 참여도가 훨씬 높은 느슨한 일치보다 상위에 표시될 수 있습니다.
- ASCII 텍스트는 단어 및 문장 부호 경계에서 토큰화됩니다. 예를 들어 `personal-map`에는 독립된 `map` 토큰이 포함되지만, `amap-jsapi-skill`에는 `amap`, `jsapi`, `skill`이 포함됩니다. 따라서 `map`을 검색하면 `personal-map`이 `amap-jsapi-skill`보다 더 강한 어휘 일치로 평가됩니다.
- 인기도는 로그 스케일로 조정되고 상한이 적용됩니다. 참여도가 높은 Skills도 쿼리 텍스트와의 일치도가 낮으면 순위가 더 낮을 수 있습니다.
- 호출자 필터와 현재 검토 상태에 따라 의심스럽거나 숨겨진 검토 상태의 Skills가 공개 검색에서 제외될 수 있습니다.

게시자 검색 노출 지침:

- 사용자가 실제로 검색할 용어를 표시 이름, 요약 및 태그에 넣으십시오. 독립된 슬러그 토큰은 유지하려는 안정적인 식별자이기도 한 경우에만 사용하십시오.
- 새 슬러그가 장기적으로 더 나은 표준 이름이 아니라면 단일 쿼리의 순위를 높이기 위해 슬러그 이름을 바꾸지 마십시오. 이전 슬러그는 리디렉션 별칭이 되지만, 표준 URL, 표시되는 슬러그 및 향후 검색 요약에는 새 슬러그가 사용됩니다.
- 이름 변경 별칭은 이전 URL 및 레지스트리를 통해 해석되는 설치의 해석 기능을 유지하지만, 검색 순위는 이름 변경이 인덱싱된 후 표준 Skills 메타데이터를 기준으로 결정됩니다. 기존 통계는 해당 Skills에 유지됩니다.
- Skills가 예기치 않게 표시되지 않으면 순위 관련 메타데이터를 변경하기 전에 로그인한 상태에서 `clawhub inspect @owner/slug`를 사용하여 먼저 검토 상태를 확인하십시오.
...
- `cursor`(선택 사항): `trending` 이외의 정렬을 위한 페이지 매김 커서
...
- `createdAt`은 신규 Skills 크롤링에서 안정적이며, `updated`는 기존 Skills가 다시 게시될 때 변경됩니다.
- `nonSuspiciousOnly=true`이면 페이지를 가져온 후 의심스러운 Skills가 필터링되므로 커서 기반 정렬에서 한 페이지에 `limit`보다 적은 항목이 반환될 수 있습니다.
- `nextCursor`가 있으면 이를 사용하여 페이지 매김을 계속하십시오. 페이지가 짧다는 사실만으로 결과가 끝났음을 의미하지는 않습니다.
...
- 소유자의 이름 변경/병합 흐름에서 생성된 이전 슬러그는 표준 Skills로 해석됩니다.
- `metadata.os`: Skills frontmatter에 선언된 OS 제한(예: `["macos"]`, `["linux"]`). 선언되지 않은 경우 `null`입니다.
- `metadata.systems`: Nix 시스템 대상(예: `["aarch64-darwin", "x86_64-linux"]`). 선언되지 않은 경우 `null`입니다.
- Skills에 플랫폼 메타데이터가 없으면 `metadata`는 `null`입니다.
- `moderation`은 Skills가 신고되었거나 소유자가 보고 있는 경우에만 포함됩니다.
...
- 공개 호출자는 이미 신고된 표시 가능한 Skills에 대해서만 `200`을 받습니다.
- 공개 호출자에게는 증거가 가려지며, 원시 스니펫은 소유자/검토자에게만 포함됩니다.

### `POST /api/v1/skills/{slug}/report`

검토자의 검토를 위해 Skills를 신고합니다. 신고는 Skills 수준이며 선택적으로 버전에 연결되고 Skills 신고 대기열로 전달됩니다.
...
Skills 신고를 해결하거나 다시 열기 위한 검토자/관리자 엔드포인트입니다.
...
`confirmed` 및 `dismissed`에는 `note`가 필요하며, `status`를 다시 `open`으로 설정할 때는 생략할 수 있습니다. 동일한 감사 가능한 워크플로에서 Skills를 숨기려면 분류된 신고와 함께 `finalAction: "hide"`를 전달하십시오.
...
- 게시된 스캔에는 소유자/게시자 관리 액세스 권한 또는 플랫폼 중재자/관리자 권한이 필요합니다.
...
- 대기 중에는 클라이언트가 요청보다 앞에 있는 우선순위 수동 스캔 수를 표시할 수 있도록 `queue.queuedAhead`와 `queue.position`을 반환합니다. 대기열이 매우 크면 상한이 적용되며 `queuedAheadIsEstimate: true`로 보고됩니다.
...
- 스킬 또는 플러그인에 대한 소유자/게시자 관리 액세스 권한이나 플랫폼 중재자/관리자 권한이 필요합니다.
- 차단되거나 숨겨진 버전을 포함하여 정확히 제출된 버전의 저장된 스캔 결과를 반환합니다.
- `kind`의 기본값은 `skill`이며, 플러그인/패키지 스캔에는 `kind=plugin`을 사용합니다.
...
- 선택한 버전에 생성된 Skill 카드가 있고, 중재에 의해 악성 코드로 차단되지 않았으며, ClawScan 검증 결과가 안전한 경우에만 `ok`가 `true`입니다.
- 셸 자동화에서 중첩된 래퍼를 해제하지 않고도 읽을 수 있도록 Skill ID, 게시자 ID, 선택한 버전 메타데이터는 최상위 엔벌로프 필드(`slug`, `displayName`, `publisherHandle`, `version`, `resolvedFrom`, `tag`, `createdAt`)에 있습니다.
- `security`는 최상위 ClawScan/보안 판정입니다. 자동화에서는 `ok`, `decision`, `reasons`, `security.status`를 기준으로 처리해야 합니다.
- `security.signals`에는 `staticScan`, `virusTotal`, `skillSpector`와 같은 스캐너의 근거 증거가 포함됩니다.
- `security.signals.dependencyRegistry`는 v1 응답 호환성을 위해 유지되지만, 종속성 레지스트리 존재 여부 스캐너는 폐기되었으며 이 키는 항상 `null`입니다.
- 게시 또는 가져오기 중에 ClawHub가 GitHub 저장소/참조/커밋/경로를 확인하고 저장한 경우에만 `provenance`가 `server-resolved-github-import`이며, 그 외에는 `unavailable`입니다.

### `POST /api/v1/skills/-/security-verdicts`

정확한 Skill 버전에 대한 현재의 간략한 보안 판정을 반환합니다. 이
...
- 결과는 항목별로 제공되며, 하나의 Skill 또는 버전이 없어도 전체 응답은 실패하지 않습니다.
- 응답에는 보안 정보만 포함됩니다. Skill 카드 데이터, 생성된 카드 상태, 아티팩트 파일 목록 또는 상세 스캐너 페이로드는 포함되지 않습니다.
- `security.signals`에는 상태 수준의 근거 증거만 포함됩니다. 전체 스캐너 세부 정보는 `/scan` 또는 ClawHub 보안 감사 페이지를 사용하십시오.
- `security.signals.dependencyRegistry`는 v1 응답 호환성을 위해 유지되지만, 종속성 레지스트리 존재 여부 스캐너는 폐기되었으며 이 키는 항상 `null`입니다.
- Skill 카드가 없어도 이 엔드포인트의 `ok`, `decision` 또는 `reasons`에는 영향을 주지 않습니다. 카드 콘텐츠가 필요한 경우 클라이언트는 설치된 `skill-card.md`를 로컬에서 읽어야 합니다.
- 단일 Skill의 Skill 카드 검증 엔벌로프가 필요하면 `/verify`를, 생성된 카드 Markdown이 필요하면 `/card`를, 상세 스캐너 데이터가 필요하면 `/scan`을 사용하십시오.
...
- `category` (선택 사항): Plugin 카테고리 필터입니다. 요청 범위가 Plugin 패키지(`/api/v1/plugins`,
  `/api/v1/code-plugins`, `/api/v1/bundle-plugins` 또는
  `family=code-plugin`/`family=bundle-plugin`이 지정된 패키지 엔드포인트)로
  한정된 경우에만 지원됩니다. 제어되는 카테고리와 레거시 v1 필터 별칭은
  `GET /api/v1/plugins`에 설명되어 있습니다.

참고:

- `family`, `channel`, `isOfficial`, `featured`, `highlightedOnly` 또는 `sort`의
  값이 잘못되면 `400`을 반환합니다. 알 수 없는 쿼리 매개변수는 무시됩니다.
...
- `category` (선택 사항): Plugin 카테고리 필터입니다. 요청 범위가
  Plugin 패키지로 한정된 경우에만 지원됩니다. 제어되는 카테고리와 레거시
  v1 필터 별칭은 `GET /api/v1/plugins`에 설명되어 있습니다.
...
- 호스팅된 Skills에는 최근 저장된 버전의 파일이 포함되며
  `_manifest.json`에 `sourceRef: "public-clawhub"`로 나열됩니다.
- `clean` 또는 `suspicious` 스캔 결과가 있는 현재 GitHub 기반 Skills에는
  `sourceRef: "public-github"`, 저장소, 커밋, 경로, 콘텐츠 해시 및 아카이브 URL이
  포함된 `_source_handoff.json`이 포함됩니다. ClawHub에서 호스팅되는 소스 파일은 포함되지 않습니다.
- 각 Skill에는 `_export_skill_meta.json`이 포함됩니다.
- `_manifest.json`은 항상 ZIP 루트에 포함됩니다.
- 개별 Skills 또는 파일을 내보낼 수 없으면 `_errors.json`이
  포함됩니다.
...
- `cursor`(선택 사항): 이전 응답의 페이지네이션 커서입니다.
- `family`(선택 사항): `code-plugin` 또는 `bundle-plugin`입니다. 생략하면 두 Plugin
  계열을 모두 의미합니다.
...
- `_manifest.json`은 항상 ZIP 루트에 포함됩니다.
- 개별 Plugin 또는 파일을 내보낼 수 없으면 `_errors.json`이
  포함됩니다.
...
- `limit`(선택 사항): 정수(1-100)
- `isOfficial`(선택 사항): `true` 또는 `false`
- `category`(선택 사항): Plugin 카테고리 필터입니다. 현재 값은 다음과 같습니다.
  `channels`, `models`, `memory`, `context`, `voice`, `media`, `web`,
  `tools`, `runtime`, `gateway`, `security`, `other`.
...
- `GET /api/v1/plugins`에 문서화된 레거시 v1 필터 별칭도
  허용됩니다.
- 카테고리 필터링은 검색 쿼리를 다시 작성하는 방식이 아니라 Plugin 카테고리 다이제스트
...
파일 메타데이터, 호환성, 검증, 아티팩트 메타데이터 및 검사 데이터를 포함하여
패키지 버전 하나를 반환합니다.

참고:

- `version.artifact.kind`는 기존 방식의 패키지 아카이브인 경우 `legacy-zip`이고,
  ClawPack 기반 릴리스인 경우 `npm-pack`입니다.
- ClawPack 릴리스에는 npm 호환 `npmIntegrity`, `npmShasum` 및
  `npmTarballName` 필드가 포함됩니다.
- `version.sha256hash`는 이전 클라이언트를 위한 더 이상 사용되지 않는 호환성 메타데이터입니다. 이 값은
  `/api/v1/packages/{name}/download`가 반환하는 정확한 ZIP 바이트를 해시합니다.
  최신 클라이언트는 정규 릴리스 아티팩트를 식별하는
  `version.artifact.sha256`을 사용해야 합니다.
- 검사 데이터가 있으면 `version.vtAnalysis`, `version.llmAnalysis` 및 `version.staticScan`이
  포함됩니다.
- 호출자가 소유 게시자를 읽을 수 없으면 비공개 패키지는 `404`를 반환합니다.

### `GET /api/v1/packages/{name}/versions/{version}/security`

설치 클라이언트를 위해 정확한 패키지 릴리스의 보안 및 신뢰 요약을 반환합니다.
...
- `trust.scanStatus`는 스캐너 입력과 수동 릴리스 중재에서 도출된
  유효 신뢰 상태입니다.
- `trust.moderationState`는 null이 될 수 있습니다. 수동 릴리스 중재가
  없으면 `null`입니다.
- `trust.blockedFromDownload`는 설치 차단 신호입니다. OpenClaw 및 기타
  설치 클라이언트는 스캐너 또는 중재 필드에서 차단 규칙을 다시 도출하지 말고
...
- `trust.stale`은 오래된 입력을 사용하여 신뢰 요약이 계산되었으며,
  높은 신뢰도의 허용 결정을 내리기 전에 새로 고침이 필요한 것으로 취급해야 함을 의미합니다.

참고:

- 이 엔드포인트는 정확한 버전을 대상으로 합니다. 클라이언트는 최신
  패키지 메타데이터를 읽은 직후가 아니라 설치하려는 패키지 버전을 확인한 후
  이 엔드포인트를 호출해야 합니다.
- 호출자가 소유 게시자를 읽을 수 없으면 비공개 패키지는 `404`를 반환합니다.
- 이 엔드포인트는 의도적으로 소유자/중재자용 중재
  엔드포인트보다 범위가 좁습니다. 설치 결정과 공개 설명만 노출하며,
  신고자 신원, 신고 내용, 비공개 증거 또는 내부 검토
  일정은 노출하지 않습니다.
...
- `packageName`은 npm 이름 형식으로 정규화됩니다. 계획된
...
- `all`: 수동 재정의, 정상 상태가 아닌 검사 상태 또는 패키지 신고가 있는 모든 릴리스입니다.
...
버전에 연결됩니다. 신고는 중재 대기열에 추가되지만 그 자체로 다운로드를 자동 숨김하거나
...
- `approved`: 수동으로 검토했으며 허용되었습니다.
- `quarantined`: 후속 조치가 완료될 때까지 차단됩니다.
- `revoked`: 이전에 신뢰했던 릴리스를 이후 차단합니다.
...
- 이전 OpenClaw 클라이언트가 계속 작동하도록 Plugin/패키지 아카이브는 `package/` 루트가 있는 zip 파일입니다.
- 이 경로는 ZIP만 지원합니다. ClawPack `.tgz` 파일을 스트리밍하지 않습니다.
- 리졸버 무결성 검사를 위해 응답에 `ETag`, `Digest`, `X-ClawHub-Artifact-Type` 및
...
- 사용자가 원할 경우 npm이 미러를 가리키도록 할 수 있도록 `dist.tarball`, `dist.integrity` 및 `dist.shasum`은 npm 호환 필드를 사용합니다.
...
CLI에서 로컬 지문을 알려진 버전에 매핑하는 데 사용합니다.
...
`suspicious` 검사 결과를 받은 현재 GitHub 기반 skill에 대해 GitHub 소스 인계 정보를 반환합니다.
...
- `version`과 `tag`를 모두 제공하지 않으면 최신 버전을 사용합니다.
- 소프트 삭제된 버전은 `410`을 반환합니다.
- GitHub 기반 skill 인계는 바이트를 프록시하거나 미러링하지 않습니다. JSON 응답에는
  `sourceRef: "public-github"`, `repo`, `commit`, `path`, `contentHash` 및
  `archiveUrl`이 포함됩니다. 검사/현재 상태는 게이트이며 성공 페이로드 메타데이터에는
  포함되지 않습니다.
...
- 권장 방식: `payload` JSON과 `files[]` blob이 포함된 `multipart/form-data`.
- `files`(storageId 기반)가 포함된 JSON 본문도 허용됩니다.
- 선택적 페이로드 필드: `ownerHandle`. 이 필드가 있으면 API는 서버 측에서 해당 게시자를 확인하고 행위자에게 게시자 접근 권한이 있는지 요구합니다.
- 선택적 페이로드 필드: `migrateOwner`. `ownerHandle`과 함께 `true`이면 행위자가 현재 게시자와 대상 게시자 모두에서 관리자/소유자인 경우 기존 skill을 해당 소유자로 이동할 수 있습니다. 명시적으로 동의하지 않으면 소유자 변경이 거부됩니다.
...
- 허용되는 양식 필드는 `payload`, 반복되는 `files` blob 또는 하나의 `clawpack`
  tarball 참조입니다. `clawpack`은 `.tgz` blob이거나 upload-url 흐름에서 반환된 스토리지 ID일 수 있습니다. 스테이징된 스토리지 ID 게시에는 해당 업로드 URL과 함께 반환된
  `clawpackUploadTicket`도 포함해야 합니다.
- `files` 또는 `clawpack` 중 하나만 사용해야 하며, 동일한 요청에 둘 다 사용할 수 없습니다.
- JSON 본문 및 호출자가 제공한 `payload.files` / `payload.artifact`
  메타데이터는 거부됩니다.
- 직접 멀티파트 게시 요청의 최대 크기는 18MB입니다. ClawPack tarball은 최대 120MB의 tarball 제한까지 upload-url 흐름을 사용할 수 있습니다.
...
해당 slug를 차지할 수 있습니다. 이 만료가 적용되는 경우 삭제 응답에
`slugReservedUntil`이 포함됩니다.
중재자/관리자의 숨김 및 보안 제거는 이 방식으로 만료되지 않습니다.
...
새로 생성하는 조직의 경우 `memberHandle`을 제공해야 하며, 작업을 수행하는 관리자는 구성원으로 추가되지 않습니다.
...
패키지 이름은 릴리스 행이 없는 비공개 자리 표시자 패키지가 되므로, 동일한 소유자는 나중에 실제
...
관리자 전용입니다. Convex Auth 계정 행을 수정하지 않고 검증된 대체 GitHub OAuth 주체의
...
게시자가 Skills, 패키지 또는 GitHub Skill 소스를 보유한 경우 복구는 안전하게 실패합니다.
복구 시 복구된 게시자의 Skills, Skill 슬러그 별칭, 패키지, 패키지 검사기 경고 및
파생 검색 다이제스트 행의 레거시 `ownerUserId` 필드도 마이그레이션하여 직접 소유자
경로가 새 게시자 권한과 일치하도록 합니다. 복구된 핸들에 활성 상태인 보호된 핸들
예약도 대체 사용자에게 재할당하여, 이후 프로필 동기화가 이전 사용자의 경합 권한을
복원하지 못하도록 합니다. 각 기본 테이블은 적용 트랜잭션당 100개 행으로 제한되며,
규모가 더 큰 복구에서는 먼저 재개 가능한 소유자 마이그레이션을 사용해야 합니다.
GitHub Skill 소스는 게시자 범위에 속하며 다시 작성되지 않고 확인된 것으로 보고됩니다.
...
(관리자만 가능). `dryRun`이 `false`가 아니면 기본적으로 드라이런으로 실행됩니다.
...
별표를 추가하거나 제거합니다(하이라이트). 두 엔드포인트 모두 멱등성을 보장합니다.
...
tarball을 스테이징하는 패키지 게시 요청은 결과 스토리지 ID를 `clawpack`으로,
...
직접 호스팅하는 경우 이 파일을 제공하거나 `CLAWHUB_REGISTRY`를 명시적으로 설정하십시오(레거시: `CLAWDHUB_REGISTRY`).
