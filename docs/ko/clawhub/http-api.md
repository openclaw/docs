...
서드 파티 디렉터리는 공개 읽기 엔드포인트를 사용해 ClawHub Skills를 나열하거나 검색할 수 있습니다. 결과를 캐시하고, `429`/`Retry-After`를 준수하며, 사용자를 표준 ClawHub 목록(`https://clawhub.ai/<owner>/<slug>`)으로 다시 연결하고, ClawHub가 서드 파티 사이트를 보증한다는 인상을 주지 마세요. 공개 API 표면 밖의 숨김, 비공개 또는 검토 차단 콘텐츠를 미러링하려고 시도하지 마세요.

웹 slug 바로 가기는 레지스트리 계열 전반에서 해석되지만, API 클라이언트는 라우트 우선순위를 재구성하는 대신 읽기 엔드포인트가 반환하는 표준 URL을 사용해야 합니다.
...
- 인증된 쓰기 엔드포인트는 서버가 이유를 알고 있을 때 단순한 `Unauthorized`만 반환해서는 안 됩니다. 누락된 토큰, 유효하지 않거나 폐기된 토큰, 삭제/차단/비활성화된 계정은 각각 CLI 클라이언트가 사용자에게 차단 원인을 알려줄 수 있도록 실행 가능한 텍스트를 받아야 합니다.
...
- 신뢰할 수 있는 클라이언트 IP가 없으면, 익명 다운로드 요청은 전역 `ip:unknown` 버킷 하나 대신 엔드포인트 범위 폴백 버킷을 사용합니다. 익명 읽기/쓰기 요청은 여전히 공유 unknown 버킷을 사용하므로 IP 누락 라우팅이 계속 보이고 보수적으로 유지됩니다.
...
- 결과는 관련성 순서로 반환됩니다(임베딩 유사도 + 정확한 slug/이름 토큰 부스트 + 다운로드 기반 인기 사전값).
- 관련성은 인기보다 더 강합니다. 정확한 slug 또는 표시 이름 토큰 일치는 다운로드가 훨씬 많은 더 느슨한 일치보다 높은 순위를 차지할 수 있습니다.
- ASCII 텍스트는 단어 및 구두점 경계에서 토큰화됩니다. 예를 들어 `personal-map`에는 독립형 `map` 토큰이 포함되어 있고, `amap-jsapi-skill`에는 `amap`, `jsapi`, `skill`이 포함됩니다. 따라서 `map`을 검색하면 `amap-jsapi-skill`보다 `personal-map`이 더 강한 어휘 일치를 얻습니다.
- 다운로드는 기본 순위 신호가 아니라 작은 로그 스케일 사전값과 동점 결정 기준으로 사용됩니다. 쿼리 텍스트 일치가 약하면 다운로드가 많은 Skills도 낮은 순위를 받을 수 있습니다.
- 호출자 필터와 현재 검토 상태에 따라 의심스럽거나 숨겨진 검토 상태가 Skills를 공개 검색에서 제거할 수 있습니다.

게시자 검색 가능성 지침:

- 사용자가 문자 그대로 검색할 용어를 표시 이름, 요약, 태그에 넣으세요. 독립형 slug 토큰은 유지하려는 안정적인 정체성이기도 할 때만 사용하세요.
- 새 slug가 더 나은 장기 표준 이름이 아닌 한, 특정 쿼리 하나를 좇기 위해 slug를 변경하지 마세요. 이전 slug는 리디렉션 별칭이 되지만, 표준 URL, 표시되는 slug, 향후 검색 다이제스트는 새 slug를 사용합니다.
- 이름 변경 별칭은 이전 URL과 레지스트리를 통해 해석되는 설치의 해석을 보존하지만, 검색 순위는 이름 변경이 인덱싱된 후의 표준 Skills 메타데이터를 기반으로 합니다. 기존 통계는 Skills와 함께 유지됩니다.
- Skills가 예기치 않게 보이지 않으면, 순위 관련 메타데이터를 변경하기 전에 로그인한 상태에서 `clawhub inspect <slug>`로 먼저 검토 상태를 확인하세요.
...
- `trending`은 최근 7일 설치 수(텔레메트리 기반)로 순위를 매깁니다.
...
- 소유자 이름 변경/병합 흐름으로 생성된 이전 slug는 표준 Skills로 해석됩니다.
...
- 증거는 공개 호출자에게는 수정 처리되며, 원시 스니펫은 소유자/검토자에게만 포함됩니다.

### `POST /api/v1/skills/{slug}/report`

검토자 리뷰를 위해 Skills를 신고합니다. 신고는 Skills 수준이며, 선택적으로 버전에 연결되고, Skills 신고 큐에 반영됩니다.
...
`note`는 `confirmed` 및 `dismissed`에 필수입니다. `status`를 다시 `open`으로 설정할 때는 생략할 수 있습니다. 감사 가능한 동일한 워크플로에서 Skills를 숨기려면 분류된 신고와 함께 `finalAction: "hide"`를 전달하세요.
...
- `version.security`는 사용 가능할 때 정규화된 스캔 검증 상태와 스캐너 세부 정보(VirusTotal + LLM)를 포함합니다.
...
- `security.capabilityTags`는 감지될 때 `crypto`, `requires-wallet`, `can-make-purchases`, `can-sign-transactions`, `requires-oauth-token`, `posts-externally` 같은 결정적 capability/risk 레이블을 포함합니다.
- `security.hasScanResult`는 스캐너가 확정 판정(`clean`, `suspicious` 또는 `malicious`)을 생성한 경우에만 `true`입니다.
- `moderation`은 최신 버전에서 파생된 현재 Skills 수준 검토 스냅샷입니다.
...
- ClawPack 버전은 `npm-pack` 아티팩트, npm 무결성 필드,
  `tarballUrl`, 그리고 레거시 ZIP 호환 URL을 반환합니다.
- 이것은 OpenClaw 리졸버 표면입니다. 공유 URL에서 아카이브 형식을 추측하지 않습니다.
...
- `open`: 의심스러운, 악성, 대기 중, 격리됨, 철회됨 또는 신고된 릴리스.
...
버전에 연결됩니다. 신고는 모더레이션 대기열에 전달되지만 그 자체로 다운로드를 자동으로 숨기거나
...
`note`는 `confirmed` 및 `dismissed`에 필요합니다. `status`를 다시 `open`으로 설정할 때는 생략할 수 있습니다.
확정된 보고서에 `finalAction: "quarantine"` 또는
`finalAction: "revoke"`를 함께 전달하면 동일한 감사 가능한 워크플로에서 릴리스 검토 조치를 적용할 수 있습니다.
...
이전 패키지 릴리스에 명시적인 아티팩트 종류 메타데이터를 라벨링하기 위한 관리자 전용 유지 관리 엔드포인트입니다.
...
- 기본값은 드라이런입니다.
- ClawPack 저장소가 없는 릴리스는 `legacy-zip`으로 라벨링됩니다.
...
- `dist.tarball`, `dist.integrity`, 및 `dist.shasum`은 사용자가 원할 경우 npm이 미러를 가리키도록 할 수 있게 npm 호환
  필드를 사용합니다.
...
- 선택적 페이로드 필드: `ownerHandle`. 존재하는 경우 API가 해당
  게시자를 서버 측에서 해석하고 행위자에게 게시자 접근 권한이 있어야 합니다.
- 선택적 페이로드 필드: `migrateOwner`. `ownerHandle`과 함께 `true`이면,
  행위자가 현재 게시자와 대상 게시자 모두에서 관리자/소유자인 경우 기존 스킬을 해당 소유자로 이동할 수 있습니다.
  이 명시적 동의가 없으면 소유자 변경은
  거부됩니다.
...
존재하는 경우 `reason`은 스킬 검토 참고로 저장되고 감사 로그에 복사됩니다.
소유자가 시작한 소프트 삭제는 slug를 30일 동안 예약하며, 이후 해당 slug는
다른 게시자가 요청할 수 있습니다. 삭제 응답에는 이 만료가 적용될 때 `slugReservedUntil`이 포함됩니다.
모더레이터/관리자의 숨김 및 보안 제거는 이런 방식으로 만료되지 않습니다.
