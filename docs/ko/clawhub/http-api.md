...
서드파티 디렉터리는 공개 읽기 엔드포인트를 사용하여 ClawHub Skills를 나열하거나 검색할 수 있습니다. 결과를 캐시하고, `429`/`Retry-After`를 준수하며, 사용자를 정식 ClawHub 목록(`https://clawhub.ai/<owner>/<slug>`)으로 다시 연결하고, ClawHub가 해당 서드파티 사이트를 보증한다는 암시를 피하세요. 공개 API 표면 밖에서 숨김, 비공개 또는 검토 차단된 콘텐츠를 미러링하려고 시도하지 마세요.

웹 slug 바로가기는 레지스트리 패밀리 전반에서 확인되지만, API 클라이언트는 라우트 우선순위를 재구성하는 대신 읽기 엔드포인트가 반환한 정식 URL을 사용해야 합니다.
...
- 토큰이 없거나 유효하지 않으면 동작은 IP 강제 적용으로 폴백됩니다.
- 인증된 쓰기 엔드포인트는 서버가 이유를 알고 있을 때 단순한 `Unauthorized`를 반환해서는 안 됩니다. 토큰 누락, 유효하지 않거나 취소된 토큰, 삭제/차단/비활성화된 계정은 각각 실행 가능한 텍스트를 받아야 하므로 CLI 클라이언트가 사용자에게 무엇이 차단했는지 알려줄 수 있습니다.
...
- 기본적으로 클라이언트 IP에는 `cf-connecting-ip`(Cloudflare)를 사용합니다.
- ClawHub는 에지에서 클라이언트 IP를 식별하기 위해 신뢰할 수 있는 전달 헤더를 사용합니다.
- 신뢰할 수 있는 클라이언트 IP를 사용할 수 없는 경우, 익명 다운로드 요청은 전역 `ip:unknown` 버킷 하나 대신 엔드포인트 범위 폴백 버킷을 사용합니다. 익명 읽기/쓰기 요청은 여전히 공유 unknown 버킷을 사용하므로 누락된 IP 라우팅이 계속 표시되고 보수적으로 유지됩니다.
...
- 결과는 관련성 순서(임베딩 유사도 + 정확한 slug/이름 토큰 부스트 + 다운로드 기반 인기 사전값)로 반환됩니다.
- 관련성은 인기보다 더 강합니다. 정확한 slug 또는 표시 이름 토큰 일치는 다운로드가 훨씬 많은 느슨한 일치보다 더 높은 순위를 차지할 수 있습니다.
- ASCII 텍스트는 단어 및 문장부호 경계에서 토큰화됩니다. 예를 들어 `personal-map`에는 독립적인 `map` 토큰이 포함되고, `amap-jsapi-skill`에는 `amap`, `jsapi`, `skill`이 포함됩니다. 따라서 `map`을 검색하면 `amap-jsapi-skill`보다 `personal-map`에 더 강한 어휘 일치가 부여됩니다.
- 다운로드는 기본 순위 신호가 아니라 작은 로그 스케일 사전값 및 동점 결정 요소로 사용됩니다. 쿼리 텍스트 일치가 약하면 다운로드가 많은 Skills도 낮게 순위가 매겨질 수 있습니다.
...
- 새 slug가 더 나은 장기 정식 이름이 아닌 한, 하나의 쿼리를 쫓기 위해 slug 이름을 바꾸지 마세요. 이전 slug는 리디렉션 별칭이 되지만 정식 URL, 표시되는 slug, 향후 검색 다이제스트는 새 slug를 사용합니다.
- 이름 변경 별칭은 레지스트리를 통해 확인되는 이전 URL 및 설치의 확인을 보존하지만, 검색 순위는 이름 변경이 인덱싱된 후의 정식 Skill 메타데이터를 기준으로 합니다. 기존 통계는 Skill에 유지됩니다.
...
- 존재하는 경우 `nextCursor`를 사용하여 페이지네이션을 계속하세요. 짧은 페이지만으로 결과 끝을 의미하지는 않습니다.
...
- 소유자와 중재자는 숨겨진 Skills의 검토 세부 정보에 접근할 수 있습니다.
- 공개 호출자는 이미 플래그 지정된 표시 가능한 Skills에 대해서만 `200`을 받습니다.
- 증거는 공개 호출자에게는 수정 처리되며, 원시 스니펫은 소유자/중재자에게만 포함됩니다.

### `POST /api/v1/skills/{slug}/report`

중재자 검토를 위해 Skill을 신고합니다. 신고는 Skill 수준이며, 선택적으로 버전에 연결되고 Skill 신고 큐에 공급됩니다.
...
Skill 신고 수신을 위한 중재자/관리자 엔드포인트입니다.
...
Skill 신고를 해결하거나 다시 열기 위한 중재자/관리자 엔드포인트입니다.
...
`note`는 `confirmed` 및 `dismissed`에 필요합니다. `status`를 다시 `open`으로 설정할 때는 생략할 수 있습니다. 심사된 신고와 함께 `finalAction: "hide"`를 전달하면 동일한 감사 가능한 워크플로에서 Skill을 숨깁니다.
...
- `GET /api/v1/code-plugins` 및 `GET /api/v1/bundle-plugins`는 고정된 family 별칭으로 유지됩니다.
...
- `version.artifact.kind`는 기존 방식의 패키지 아카이브에는 `legacy-zip`,
...
중재자 검토를 위해 패키지를 신고합니다. 신고는 패키지 수준이며, 선택적으로 버전에 연결될 수 있습니다. 신고는 중재 대기열에 반영되지만 그 자체만으로 다운로드를 자동 숨김 처리하거나 차단하지 않습니다. 중재자는 릴리스 중재를 사용하여 아티팩트를 승인, 격리 또는 취소해야 합니다.
...
- `artifactKind`가 누락된 기존 ClawPack 기반 행은 `npm-pack`으로
  복구됩니다.
...
- 기본값은 최신 릴리스입니다.
- 스킬은 `GET /api/v1/download`로 리디렉션됩니다.
...
- 스코프가 지정된 패키지 packument는 `/api/npm/@scope/name`과 npm의
...
소유자가 시작한 소프트 삭제는 30일 동안 slug를 예약하며, 이후 다른 게시자가
해당 slug를 가져갈 수 있습니다. 삭제 응답에는 이 만료가 적용될 때 `slugReservedUntil`이 포함됩니다.
...
사용자를 차단하고 소유한 스킬을 하드 삭제합니다(모더레이터/관리자만).
...
사용자 차단을 해제하고 복원 가능한 스킬을 복원합니다(관리자만).
