...
서드파티 디렉터리는 공개 읽기 엔드포인트를 사용하여 ClawHub Skills를 나열하거나 검색할 수 있습니다. 결과를 캐시하고, `429`/`Retry-After`를 준수하며, 사용자를 정식 ClawHub 목록(`https://clawhub.ai/<owner>/skills/<slug>`)으로 다시 연결하고, 서드파티 사이트가 ClawHub의 보증을 받는다는 인상을 주지 마세요. 공개 API 표면 밖에서 숨김, 비공개 또는 모더레이션 차단 콘텐츠를 미러링하려고 시도하지 마세요.

웹 slug 바로가기는 레지스트리 패밀리 전체에서 확인되지만, API 클라이언트는 라우트
...
- 인증된 쓰기 엔드포인트는 서버가 이유를 알고 있을 때 단순한 `Unauthorized`를
  반환해서는 안 됩니다. 토큰 누락, 유효하지 않거나 폐기된 토큰, 삭제/차단/비활성화된
  계정은 각각 실행 가능한 문구를 받아야 CLI 클라이언트가 사용자에게 무엇이 차단했는지
  알릴 수 있습니다.
...
- 신뢰할 수 있는 클라이언트 IP를 사용할 수 없는 경우, 익명 요청은 속도 제한 종류로만
  범위가 지정된 폴백 버킷을 사용합니다. 이러한 폴백 버킷에는 호출자가 제공한 경로, slug,
  패키지 이름, 버전, 쿼리 문자열 또는 기타 아티팩트 매개변수가 포함되지 않습니다.
...
- 관련성은 인기도보다 더 강합니다. 정확한 slug 또는 표시 이름 토큰 일치는 참여도가 훨씬 높은 느슨한 일치보다 높은 순위를 차지할 수 있습니다.
...
- 인기도는 로그 스케일이 적용되고 상한이 있습니다. 쿼리 텍스트 일치가 약하면 참여도가 높은 Skills도 더 낮은 순위가 될 수 있습니다.
...
- 새 slug가 더 나은 장기 정식 이름이 아닌 한, 하나의 쿼리를 쫓기 위해 slug를 변경하지 마세요. 이전 slug는 리디렉션 별칭이 되지만, 정식 URL, 표시된 slug, 향후 검색 다이제스트는 새 slug를 사용합니다.
- 이름 변경 별칭은 레지스트리를 통해 확인되는 이전 URL 및 설치의 확인을 보존하지만, 검색 순위는 이름 변경이 색인된 후의 정식 Skill 메타데이터를 기반으로 합니다. 기존 통계는 Skill에 유지됩니다.
...
- `trending`은 지난 7일간의 설치 수(텔레메트리 기반)로 순위를 매깁니다.
...
- 있을 때 `nextCursor`를 사용하여 페이지 매김을 계속하세요. 짧은 페이지 자체가 결과 끝을 의미하지는 않습니다.
...
- `moderation`은 Skill이 플래그 지정되었거나 소유자가 보는 경우에만 포함됩니다.
...
- 증거는 공개 호출자에게는 수정되어 표시되며, 원시 스니펫은 소유자/모더레이터에게만 포함됩니다.
...
설정할 때는 생략할 수 있습니다. 동일한 감사 가능한 워크플로에서 Skill을 숨기려면 분류된
...
- `moderation`은 최신 버전에서 파생된 현재 스킬 수준의 moderation 스냅샷입니다.
...
- 게시된 스캔에는 소유자/게시자 관리 액세스 권한 또는 플랫폼 moderator/admin 권한이 필요합니다.
...
- 대기 중일 때 `queue.queuedAhead`와 `queue.position`을 반환하므로 클라이언트가 요청보다 앞선 우선순위 수동 스캔 수를 표시할 수 있습니다. 매우 큰 큐는 제한되며 `queuedAheadIsEstimate: true`로 보고됩니다.
...
- Skill 또는 Plugin에 대한 소유자/게시자 관리 액세스 권한, 또는 플랫폼 moderator/admin 권한이 필요합니다.
...
- `ok`는 선택된 버전에 생성된 Skill Card가 있고, moderation에 의해 악성 코드로 차단되지 않았으며, ClawScan 검증이 clean인 경우에만 `true`입니다.
- Skill ID, 게시자 ID 및 선택된 버전 메타데이터는 최상위 엔벨로프 필드(`slug`, `displayName`, `publisherHandle`, `version`, `resolvedFrom`, `tag`, `createdAt`)이므로 셸 자동화가 중첩 래퍼를 풀지 않고도 읽을 수 있습니다.
...
- `security.signals.dependencyRegistry`는 v1 응답 호환성을 위해 유지되지만, dependency registry 존재 여부 스캐너는 사용 중지되었으며 이 키는 항상 `null`입니다.
- `provenance`는 ClawHub가 게시 또는 가져오기 중에 GitHub repo/ref/commit/path를 해석하고 저장한 경우에만 `server-resolved-github-import`이며, 그 외에는 `unavailable`입니다.
...
- `security.signals.dependencyRegistry`는 v1 응답 호환성을 위해 유지되지만, dependency registry 존재 여부 스캐너는 사용 중지되었으며 이 키는 항상 `null`입니다.
- Skill Card의 부재는 이 엔드포인트의 `ok`, `decision` 또는 `reasons`에 영향을 주지 않습니다. 클라이언트는 카드 콘텐츠가 필요할 때 설치된 `skill-card.md`를 로컬에서 읽어야 합니다.
- 단일 Skill의 Skill Card 검증 엔벨로프가 필요하면 `/verify`를, 생성된 카드 Markdown이 필요하면 `/card`를, 상세 스캐너 데이터가 필요하면 `/scan`을 사용하세요.
...
- `category` (선택 사항): Plugin 카테고리 필터. 요청이 Plugin 패키지
  (`/api/v1/plugins`, `/api/v1/code-plugins`, `/api/v1/bundle-plugins` 또는
  `family=code-plugin`/`family=bundle-plugin`이 있는 패키지 엔드포인트)로
  범위가 지정된 경우에만 지원됩니다. 제어되는 카테고리와 레거시 v1
  필터 별칭은 `GET /api/v1/plugins` 아래에 문서화되어 있습니다.

참고:

- `family`, `channel`, `isOfficial`, `featured`, `highlightedOnly` 또는
  `sort`에 잘못된 값이 있으면 `400`을 반환합니다. 알 수 없는 쿼리 매개변수는 무시됩니다.
...
- `channel` (선택 사항): `official`, `community` 또는 `private`
- `isOfficial` (선택 사항): `true` 또는 `false`
- `category` (선택 사항): Plugin 카테고리 필터. 요청이 Plugin 패키지로
  범위가 지정된 경우에만 지원됩니다. 제어되는 카테고리와 레거시 v1
  필터 별칭은 `GET /api/v1/plugins` 아래에 문서화되어 있습니다.
...
- `version.artifact.kind`는 이전 방식의 패키지 아카이브의 경우 `legacy-zip`이고,
...
설치 클라이언트를 위한 정확한 패키지 릴리스 보안 및 신뢰 요약을 반환합니다. 이는 확인된 릴리스를 설치할 수 있는지 결정하기 위한 공개 OpenClaw 소비 표면입니다.
...
- `release.artifactKind`, `release.artifactSha256`, `release.npmIntegrity`,
  ...
  존재합니다.
- `trust.scanStatus`는 스캐너 입력 및 수동 릴리스 모더레이션에서 파생된 유효 신뢰 상태입니다.
...
- 이 엔드포인트는 버전에 정확히 대응합니다. 클라이언트는 최신 패키지 메타데이터를 읽은 직후가 아니라,
  설치하려는 패키지 버전을 확인한 뒤 호출해야 합니다.
- 호출자가 소유 게시자를 읽을 수 없는 경우 비공개 패키지는 `404`를 반환합니다.
- 이 엔드포인트는 소유자/모더레이터 모더레이션 엔드포인트보다 의도적으로 더 좁습니다. 신고자 신원,
...
- 이는 OpenClaw 확인자 표면입니다. 공유 URL에서 아카이브 형식을 추측하지 않도록 합니다.
...
- `all`: 수동 재정의, 비정상 스캔 상태 또는 패키지 신고가 있는 모든 릴리스입니다.
...
모더레이터 검토를 위해 패키지를 신고합니다. 신고는 패키지 수준이며, 선택적으로 버전에
연결될 수 있습니다. 신고는 모더레이션 대기열에 전달되지만, 그 자체로 다운로드를 자동 숨김 처리하거나
차단하지는 않습니다. 모더레이터는 릴리스 모더레이션을 사용하여 아티팩트를 승인, 격리 또는 취소해야 합니다.
...
패키지 중재 표시 여부를 위한 소유자/중재자 엔드포인트입니다.
...
- 응답에는 리졸버 무결성 검사용 `ETag`, `Digest`, `X-ClawHub-Artifact-Type` 및
...
- 선택적 페이로드 필드: `ownerHandle`. 존재하는 경우 API가 해당
  게시자를 서버 측에서 해석하고 행위자에게 게시자 액세스 권한을 요구합니다.
- 선택적 페이로드 필드: `migrateOwner`. `ownerHandle`과 함께 `true`인 경우,
  행위자가 현재 및 대상 게시자 모두에서 관리자/소유자인 경우 기존 skill을 해당 소유자로
  이동할 수 있습니다. 이 옵트인이 없으면 소유자 변경은
  거부됩니다.
...
- 허용되는 폼 필드는 `payload`, 반복되는 `files` blob 또는 하나의 `clawpack`
  ...
  upload-url 흐름에서 반환된 storage id일 수 있습니다. 스테이징된 storage-id 게시는 해당 업로드 URL과 함께 반환된
...
존재하는 경우 `reason`은 skill 중재 노트로 저장되고 감사 로그에 복사됩니다.
소유자가 시작한 소프트 삭제는 slug를 30일 동안 예약하며, 이후 해당 slug는
다른 게시자가 사용할 수 있습니다. 이 만료가 적용될 때 삭제 응답에는 `slugReservedUntil`이 포함됩니다.
...
관리자 전용입니다. handle에 대한 조직 게시자가 존재하도록 보장합니다. handle이 여전히
...
- handle이 이미 게시자, 사용자 또는 개인 게시자에 의해 사용 중이면 `409`를 반환합니다.

### `POST /api/v1/users/reserve`

관리자 전용입니다. 릴리스를 게시하지 않고 정당한 소유자를 위해 루트 slug와 패키지 이름을 예약합니다.
패키지 이름은 릴리스 행이 없는 비공개 placeholder 패키지가 되므로, 동일한
소유자는 나중에 실제 code-plugin 또는 bundle-plugin 릴리스를 해당 이름으로 게시할 수 있습니다.
...
관리자 전용입니다. Convex Auth 계정 행을 편집하지 않고 검증된 대체 GitHub OAuth principal에 대한
개인 게시자를 복구합니다. 요청은 변경 불가능한 두 GitHub
provider account id를 모두 지정해야 합니다. 변경 가능한 handle은 operator-facing guard로만 사용됩니다.
...
복구는 또한 복구된 게시자의 스킬, 스킬 슬러그 별칭, 패키지, 패키지 검사기 경고, 파생 검색 다이제스트 행의 기존 `ownerUserId` 필드를 마이그레이션하여
...
차단을 해제하거나 콘텐츠를 복원하지 않고 기존 차단에 저장된 사유를 변경합니다(관리자 전용). `dryRun`이 `false`가 아니면 기본적으로 드라이런으로 동작합니다.
