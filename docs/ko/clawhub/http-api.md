...
서드파티 디렉터리는 공개 읽기 엔드포인트를 사용해 ClawHub 스킬을 나열하거나 검색할 수 있습니다. 결과를 캐시하고, `429`/`Retry-After`를 준수하며, 사용자를 표준 ClawHub 목록(`https://clawhub.ai/<owner>/skills/<slug>`)으로 다시 연결하고, ClawHub가 해당 서드파티 사이트를 보증한다는 인상을 주지 마세요. 공개 API 표면 밖에서 숨김, 비공개 또는 모더레이션으로 차단된 콘텐츠를 미러링하려고 시도하지 마세요.

웹 슬러그 바로가기는 레지스트리 패밀리 전반에서 해석되지만, API 클라이언트는 라우트 우선순위를 재구성하지 말고 읽기 엔드포인트가 반환한 표준 URL을 사용해야 합니다.
...
- 토큰이 없거나 유효하지 않으면 동작은 IP 기반 적용으로 대체됩니다.
- 인증된 쓰기 엔드포인트는 서버가 이유를 알고 있을 때 단순한 `Unauthorized`만 반환해서는 안 됩니다. 누락된 토큰, 유효하지 않거나 취소된 토큰, 삭제/차단/비활성화된 계정은 각각 실행 가능한 문구를 받아야 CLI 클라이언트가 사용자에게 무엇이 차단했는지 알릴 수 있습니다.
...
- 동기화된 재시도를 피하기 위해 지터가 적용된 백오프를 사용하세요.
...
- 결과는 관련도 순서(임베딩 유사도 + 정확한 슬러그/이름 토큰 부스트 + 작은 인기 사전값)로 반환됩니다.
- 관련도는 인기보다 더 강합니다. 정확한 슬러그 또는 표시 이름 토큰 일치는 참여도가 훨씬 높은 느슨한 일치보다 더 높은 순위를 차지할 수 있습니다.
- ASCII 텍스트는 단어 및 문장 부호 경계에서 토큰화됩니다. 예를 들어 `personal-map`에는 독립적인 `map` 토큰이 포함되고, `amap-jsapi-skill`에는 `amap`, `jsapi`, `skill`이 포함됩니다. 따라서 `map`을 검색하면 `amap-jsapi-skill`보다 `personal-map`이 더 강한 어휘 일치를 얻습니다.
- 인기는 로그 스케일로 조정되고 상한이 적용됩니다. 참여도가 높은 스킬도 쿼리 텍스트 일치가 약하면 더 낮은 순위를 차지할 수 있습니다.
...
- 새 슬러그가 장기적으로 더 나은 표준 이름이 아닌 한, 단일 쿼리를 좇기 위해 슬러그 이름을 바꾸지 마세요. 이전 슬러그는 리디렉션 별칭이 되지만, 표준 URL, 표시되는 슬러그, 향후 검색 다이제스트는 새 슬러그를 사용합니다.
- 이름 변경 별칭은 레지스트리를 통해 해석되는 이전 URL 및 설치의 해석을 보존하지만, 검색 순위는 이름 변경이 인덱싱된 후의 표준 스킬 메타데이터를 기준으로 합니다. 기존 통계는 해당 스킬에 유지됩니다.
- 스킬이 예상과 달리 보이지 않으면 순위 관련 메타데이터를 변경하기 전에 로그인한 상태에서 `clawhub inspect @owner/slug`로 먼저 모더레이션 상태를 확인하세요.
...
- 있을 경우 페이지 매김을 계속하려면 `nextCursor`를 사용하세요. 짧은 페이지 자체가 결과의 끝을 의미하지는 않습니다.
...
- 소유자 이름 변경/병합 플로에서 생성된 이전 슬러그는 표준 스킬로 해석됩니다.
- `metadata.os`: 스킬 프런트매터에 선언된 OS 제한(예: `["macos"]`, `["linux"]`). 선언되지 않은 경우 `null`.
...
- 증거는 공개 호출자에게는 수정 처리되며 소유자/모더레이터에게만 원시 스니펫을 포함합니다.

### `POST /api/v1/skills/{slug}/report`

모더레이터 검토를 위해 스킬을 신고합니다. 신고는 스킬 수준이며, 선택적으로 버전에 연결되고 스킬 신고 대기열로 전달됩니다.
...
로컬 업로드 검사는 더 이상 지원되지 않습니다. `multipart/form-data` 또는 `{ "source": { "kind": "upload" } }`를 사용하는 요청은 `410`을 반환합니다.
...
- 게시된 검사에는 소유자/게시자 관리 권한 또는 플랫폼 모더레이터/관리자 권한이 필요합니다.
- 게시된 검사는 `update: true`이고 검사가 성공적으로 완료된 경우에만 다시 기록합니다.
...
- 검사 작업은 비동기입니다. 수동 검사 요청은 일반 게시/백필 작업보다 우선 처리되지만, 완료 여부는 여전히 워커 가용성에 따라 달라집니다.
...
- 대기 중에는 `queue.queuedAhead`와 `queue.position`을 반환하여 클라이언트가 요청보다 앞선 우선순위 수동 검사의 수를 표시할 수 있게 합니다. 매우 큰 큐는 제한되며 `queuedAheadIsEstimate: true`로 보고됩니다.
- 사용 가능한 경우 `report`에는 `clawscan`, `skillspector`, `staticAnalysis`, `virustotal` 섹션이 포함됩니다.
- 실패한 검사 작업은 `lastError`와 함께 `status: "failed"`를 반환합니다.

### `GET /api/v1/skills/-/scan/{scanId}/download`

인증된 보고서 아카이브 엔드포인트입니다.

- 성공한 검사가 필요하며, 종료 상태가 아닌 검사는 `409`를 반환합니다.
...
- `kind`의 기본값은 `skill`입니다. Plugin/패키지 검사에는 `kind=plugin`을 사용하세요.
- 검사 요청 다운로드와 같은 ZIP 형식을 반환합니다.

### `POST /api/v1/skills/-/scan/batch`

관리자 전용 표준 배치 재검사 경로입니다. 레거시 `POST /api/v1/skills/-/rescan-batch`와 동일한 페이로드 형식을 허용합니다.
...
`clawhub skill verify`에서 사용하는 스킬 카드 검증 봉투를 반환합니다.
...
- `ok`는 선택한 버전에 생성된 스킬 카드가 있고, 모더레이션에서 악성 코드로 차단되지 않았으며, ClawScan 검증이 깨끗한 경우에만 `true`입니다.
- 스킬 ID, 게시자 ID, 선택된 버전 메타데이터는 최상위 봉투 필드(`slug`, `displayName`, `publisherHandle`, `version`, `resolvedFrom`, `tag`, `createdAt`)이므로 셸 자동화가 중첩된 래퍼를 풀지 않고도 읽을 수 있습니다.
...
- `provenance`는 ClawHub가 게시 또는 가져오기 중 GitHub 저장소/ref/커밋/경로를 해석하고 저장한 경우에만 `server-resolved-github-import`이며, 그렇지 않으면 `unavailable`입니다.
...
- 결과는 항목별로 제공되며, 하나의 스킬 또는 버전이 누락되어도 전체 응답은 실패하지 않습니다.
- 응답은 보안 전용입니다. 스킬 카드 데이터, 생성된 카드 상태, 아티팩트 파일 목록 또는 자세한 스캐너 페이로드는 포함하지 않습니다.
- `security.signals`에는 상태 수준의 지원 증거만 포함됩니다. 전체 스캐너 세부 정보는 `/scan` 또는 ClawHub 보안 감사 페이지를 사용하세요.
- `security.signals.dependencyRegistry`는 v1 응답 호환성을 위해 유지되지만, 의존성 레지스트리 존재 스캐너는 폐기되었으며 이 키는 항상 `null`입니다.
- 스킬 카드의 부재는 이 엔드포인트의 `ok`, `decision`, `reasons`에 영향을 주지 않습니다. 카드 내용이 필요한 클라이언트는 설치된 `skill-card.md`를 로컬에서 읽어야 합니다.
- 단일 스킬 스킬 카드 검증 봉투가 필요하면 `/verify`를, 생성된 카드 Markdown이 필요하면 `/card`를, 자세한 스캐너 데이터가 필요하면 `/scan`을 사용하세요.
...
- `mcp-tooling`, `data`, `automation`은 `tools`로 확인됩니다.
- `observability`와 `deployment`는 `gateway`로 확인됩니다.
...
해결된 릴리스를 설치할 수 있는지 결정하기 위한 공개 OpenClaw 소비 표면입니다.
...
- `package.name`, `package.displayName`, `package.family`은 해결된
  레지스트리 패키지를 식별합니다.
...
- `trust.reasons`는 사용자 대상 및 감사 설명 목록입니다. 이유 코드는
  `manual:quarantined`, `scan:malicious`, `package:malicious` 같은
  안정적이고 간결한 문자열입니다.
...
- 이 엔드포인트는 버전에 정확히 대응합니다. 클라이언트는 최신 패키지 메타데이터만 읽은 뒤가 아니라,
...
버전에 연결됩니다. 이는 중재 큐에 입력되지만 자체적으로 다운로드를 자동 숨김하거나
차단하지는 않습니다. 중재자는 릴리스 중재를 사용해 아티팩트를 승인, 격리 또는 철회해야 합니다.
...
- 파일 크기 제한: 200KB.
- 보류 중인 VirusTotal 스캔은 읽기를 차단하지 않습니다. 악성 릴리스는 다른 곳에서 계속 보류될 수 있습니다.
...
- 보류 중인 VirusTotal 스캔은 다운로드를 차단하지 않습니다. 악성 릴리스는 `403`을 반환합니다.
...
- 선택적 페이로드 필드: `migrateOwner`. `ownerHandle`과 함께 `true`이면
  행위자가 현재 및 대상 게시자 모두에서 관리자/소유자인 경우 기존 skill이 해당 소유자로
...
- 허용되는 폼 필드는 `payload`, 반복되는 `files` blob 또는 하나의 `clawpack`
  ...
  저장소 ID일 수 있습니다. 스테이징된 storage-id 게시는 해당 업로드 URL과 함께 반환된
...
- 직접 multipart 게시 요청은 18MB로 제한됩니다. ClawPack tarball은
  upload-url 플로를 사용해 120MB tarball 제한까지 사용할 수 있습니다.
...
- 대리 게시는 대상 소유자 계정에 대해 official 채널 자격을 계속 검증합니다.
...
관리자 전용입니다. Convex Auth 계정 행을 편집하지 않고 검증된 대체 GitHub OAuth 보안 주체에 대해
개인 게시자를 복구합니다. 요청은 변경 불가능한 두 GitHub 제공자 계정 ID를 모두 지정해야 합니다.
변경 가능한 핸들은 운영자 대상 가드로만 사용됩니다.

엔드포인트는 기본적으로 dry-run으로 설정됩니다. 복구를 적용하려면 직원이 두 GitHub 주체 간의 연속성을 독립적으로 확인한 뒤 `dryRun: false`와
`confirmIdentityVerified: true`가 필요합니다. 대상 사용자의 현재 개인
게시자에 skills, packages 또는 GitHub skill sources가 있으면 복구는 실패 상태로 닫힙니다.
복구는 복구된 게시자의 skills, skill slug aliases, packages, package inspector warnings 및 파생된 search digest rows에 대한 레거시 `ownerUserId` 필드도 마이그레이션하여
...
사용자를 차단하고 소유한 skills를 하드 삭제합니다(모더레이터/관리자만).
...
사용자 차단을 해제하고 적격한 skills를 복원합니다(관리자만).
...
별표(highlights)를 추가/제거합니다. 두 엔드포인트는 모두 멱등적입니다.
