...
타사 디렉터리는 공개 읽기 엔드포인트를 사용하여 ClawHub Skills를 나열하거나 검색할 수 있습니다. 결과를 캐시하고, `429`/`Retry-After`를 준수하며, 사용자를 정식 ClawHub 목록(`https://clawhub.ai/<owner>/<slug>`)으로 다시 연결하고, ClawHub가 타사 사이트를 보증한다는 인상을 주지 마세요. 공개 API 표면 외부에서 숨김, 비공개 또는 조정 차단된 콘텐츠를 미러링하려고 시도하지 마세요.

웹 슬러그 바로 가기는 레지스트리 패밀리 전반에서 해석되지만, API 클라이언트는
라우트 우선순위를 재구성하는 대신 읽기 엔드포인트가 반환하는 정식 URL을 사용해야 합니다.
...
- 인증된 쓰기 엔드포인트는 서버가 이유를 알고 있을 때 단순한 `Unauthorized`를
  반환해서는 안 됩니다. 토큰 누락, 유효하지 않거나 철회된 토큰, 삭제/차단/비활성화된
  계정은 각각 CLI 클라이언트가 사용자에게 무엇이 차단했는지 알려줄 수 있도록 실행 가능한 텍스트를 받아야 합니다.
...
- ClawHub는 신뢰할 수 있는 전달 헤더를 사용하여 엣지에서 클라이언트 IP를 식별합니다.
- 신뢰할 수 있는 클라이언트 IP를 사용할 수 없으면, 익명 다운로드 요청은 하나의 전역 `ip:unknown` 버킷 대신 엔드포인트 범위의 대체 버킷을 사용합니다. 익명 읽기/쓰기 요청은 여전히 공유 unknown 버킷을 사용하므로 누락된 IP 라우팅이 계속 보이고 보수적으로 유지됩니다.
...
- 관련도는 인기도보다 더 강하게 작용합니다. 정확한 슬러그 또는 표시 이름 토큰 일치는 훨씬 더 많은 다운로드가 있는 더 느슨한 일치보다 높은 순위를 차지할 수 있습니다.
- ASCII 텍스트는 단어 및 구두점 경계에서 토큰화됩니다. 예를 들어 `personal-map`에는 독립적인 `map` 토큰이 포함되고, `amap-jsapi-skill`에는 `amap`, `jsapi`, `skill`이 포함됩니다. 따라서 `map`을 검색하면 `personal-map`이 `amap-jsapi-skill`보다 더 강한 어휘 일치를 갖습니다.
- 다운로드는 주요 순위 신호가 아니라 작은 로그 스케일 사전값 및 동률 결정 요소로 사용됩니다. 쿼리 텍스트와의 일치가 더 약하면 다운로드가 많은 Skills도 낮은 순위를 차지할 수 있습니다.
- 의심스럽거나 숨겨진 조정 상태는 호출자 필터와 현재 조정 상태에 따라 공개 검색에서 Skills를 제거할 수 있습니다.

게시자 검색 가능성 지침:

- 사용자가 실제로 검색할 용어를 표시 이름, 요약 및 태그에 넣으세요. 독립형 슬러그 토큰은 유지하려는 안정적인 ID이기도 할 때만 사용하세요.
- 새 슬러그가 더 나은 장기 정식 이름이 아니라면 단일 쿼리를 쫓기 위해 슬러그를 변경하지 마세요. 이전 슬러그는 리디렉션 별칭이 되지만, 정식 URL, 표시되는 슬러그, 향후 검색 다이제스트는 새 슬러그를 사용합니다.
- 이름 변경 별칭은 이전 URL과 레지스트리를 통해 해석되는 설치의 해석을 보존하지만, 검색 순위는 이름 변경이 인덱싱된 후 정식 Skill 메타데이터를 기반으로 합니다. 기존 통계는 해당 Skill에 유지됩니다.
- Skill이 예기치 않게 보이지 않으면 순위 관련 메타데이터를 변경하기 전에 로그인한 상태에서 `clawhub inspect <slug>`로 먼저 조정 상태를 확인하세요.
...
- `nonSuspiciousOnly=true`일 때, 커서 기반 정렬은 페이지를 가져온 후 의심스러운 Skills가 필터링되므로 한 페이지에서 `limit`보다 적은 항목을 반환할 수 있습니다.
- 있을 경우 페이지네이션을 계속하려면 `nextCursor`를 사용하세요. 짧은 페이지만으로 결과 끝을 의미하지는 않습니다.
...
- 증거는 공개 호출자에게 수정되어 제공되며, 소유자/조정자에게만 원시 스니펫을 포함합니다.

### `POST /api/v1/skills/{slug}/report`

조정자 검토를 위해 Skill을 신고합니다. 신고는 Skill 수준이며, 선택적으로
버전에 연결되고 Skill 신고 대기열에 입력됩니다.
...
- `security.capabilityTags`는 감지될 때
  `crypto`, `requires-wallet`, `can-make-purchases`, `can-sign-transactions`,
  `requires-oauth-token`, `posts-externally`와 같은 결정적 기능/위험 레이블을 포함합니다.
- `security.hasScanResult`는 스캐너가 확정적 판정(`clean`, `suspicious` 또는 `malicious`)을 생성한 경우에만 `true`입니다.
- `moderation`은 최신 버전에서 파생된 현재 Skill 수준 조정 스냅샷입니다.
- 과거 버전을 쿼리할 때는 `moderation`과 `security`를 같은 버전 컨텍스트로 처리하기 전에 `moderation.matchesRequestedVersion`과 `moderation.sourceVersion`을 확인하세요.
...
- `capabilityTag` (선택 사항): Plugin 패키지의 기능 필터
...
- 이는 OpenClaw 해석기 표면이며, 공유 URL에서 아카이브 형식을 추측하지 않습니다.
...
차단하지 않습니다. 중재자는 릴리스 중재를 사용해 아티팩트를
승인, 격리 또는 철회해야 합니다.
...
`finalAction: "revoke"`를 전달하면 동일한 감사 가능한 워크플로에서 릴리스 모더레이션을 적용합니다.
...
이전 패키지 릴리스에 명시적인 아티팩트 종류 메타데이터를 지정하기 위한
...
- 기본값은 드라이런입니다.
- ClawPack 스토리지가 없는 릴리스는 `legacy-zip`으로 레이블이 지정됩니다.
...
- 파일 크기 제한: 200KB.
- 대기 중인 VirusTotal 검사는 읽기를 차단하지 않습니다. 악성 릴리스는 다른 곳에서 보류될 수 있습니다.
...
- Plugin/패키지 아카이브는 이전 OpenClaw 클라이언트가 계속 동작하도록 `package/` 루트가 있는 zip 파일입니다.
...
- 선택적 페이로드 필드: `ownerHandle`. 존재하는 경우 API가 서버 측에서 해당 게시자를 확인하고 행위자에게 게시자 접근 권한이 있어야 합니다.
- 선택적 페이로드 필드: `migrateOwner`. `ownerHandle`과 함께 `true`이면 행위자가 현재 게시자와 대상 게시자 모두에서 관리자/소유자인 경우 기존 스킬이 해당 소유자로 이동할 수 있습니다. 이 옵트인이 없으면 소유자 변경은 거부됩니다.
...
- 코드 Plugin에는 `package.json`, 소스 repo 메타데이터, 소스 커밋 메타데이터, config 스키마 메타데이터, `openclaw.compat.pluginApi`, `openclaw.build.openclawVersion`이 필요합니다.
- `openclaw.hostTargets`와 `openclaw.environment`는 선택적 메타데이터입니다.
- 신뢰된 게시자만 `official` 채널에 게시할 수 있습니다.
- 대리 게시도 대상 소유자 계정에 대해 official 채널 자격을 검증합니다.
...
다른 게시자가 요구할 수 있습니다. 삭제 응답에는 이 만료가 적용될 때 `slugReservedUntil`이 포함됩니다.
...
star를 추가/제거합니다(highlights). 두 엔드포인트 모두 멱등적입니다.
