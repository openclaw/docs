...
x-i18n:
    generated_at: "2026-07-01T05:32:00Z"
    model: gpt-5.5
...
타사 디렉터리는 공개 읽기 엔드포인트를 사용해 ClawHub Skills를 나열하거나 검색할 수 있습니다. 결과를 캐시하고, `429`/`Retry-After`를 준수하며, 사용자를 표준 ClawHub 목록(`https://clawhub.ai/<owner>/skills/<slug>`)으로 다시 연결하고, 타사 사이트가 ClawHub의 보증을 받는다는 인상을 주지 마세요. 공개 API 표면 외부에서 숨김, 비공개 또는 모더레이션으로 차단된 콘텐츠를 미러링하려고 시도하지 마세요.

웹 슬러그 바로가기는 레지스트리 패밀리 전반에서 해석되지만, API 클라이언트는 라우트 우선순위를 재구성하는 대신 읽기 엔드포인트가 반환하는 표준 URL을 사용해야 합니다.
...
- 관련성은 인기도보다 강합니다. 정확한 슬러그 또는 표시 이름 토큰 일치는 참여도가 훨씬 높은 느슨한 일치보다 높은 순위를 차지할 수 있습니다.
- ASCII 텍스트는 단어 및 문장 부호 경계에서 토큰화됩니다. 예를 들어 `personal-map`에는 독립형 `map` 토큰이 포함되어 있지만 `amap-jsapi-skill`에는 `amap`, `jsapi`, `skill`이 포함됩니다. 따라서 `map`을 검색하면 `personal-map`이 `amap-jsapi-skill`보다 더 강한 어휘 일치를 얻습니다.
- 인기도는 로그 스케일로 조정되고 상한이 적용됩니다. 참여도가 높은 Skills도 쿼리 텍스트 일치가 더 약하면 낮은 순위를 차지할 수 있습니다.
...
- 새 슬러그가 더 나은 장기 표준 이름이 아닌 한, 하나의 쿼리를 좇기 위해 슬러그 이름을 바꾸지 마세요. 이전 슬러그는 리디렉션 별칭이 되지만, 표준 URL, 표시되는 슬러그, 향후 검색 다이제스트는 새 슬러그를 사용합니다.
- 이름 변경 별칭은 레지스트리를 통해 해석되는 이전 URL과 설치에 대한 해석을 보존하지만, 검색 순위는 이름 변경이 인덱싱된 후 표준 Skill 메타데이터를 기준으로 합니다. 기존 통계는 Skill에 유지됩니다.
...
- `nonSuspiciousOnly=true`인 경우 의심스러운 Skills가 페이지 검색 후 필터링되므로 커서 기반 정렬은 페이지에서 `limit`보다 적은 항목을 반환할 수 있습니다.
...
`note`는 `confirmed` 및 `dismissed`에 필요하며, `status`를 `open`으로 다시 설정할 때는 생략할 수 있습니다. 동일한 감사 가능한 워크플로에서 Skill을 숨기려면 분류된 신고와 함께 `finalAction: "hide"`를 전달하세요.
...
- `moderation`은 최신 버전에서 파생된 현재 Skills 수준의 중재 스냅샷입니다.
...
- 게시된 스캔에는 소유자/게시자 관리 권한 또는 플랫폼 중재자/관리자 권한이 필요합니다.
...
- 스캔 작업은 비동기입니다. 수동 스캔 요청은 일반 게시/백필 작업보다 우선 처리되지만, 완료는 여전히 워커 가용성에 따라 달라집니다.
...
- 대기 중에는 클라이언트가 요청보다 앞서 있는 우선순위 수동 스캔 수를 표시할 수 있도록 `queue.queuedAhead`와 `queue.position`을 반환합니다. 매우 큰 큐는 제한되며 `queuedAheadIsEstimate: true`로 보고됩니다.
...
- Skills 또는 Plugin에 대한 소유자/게시자 관리 권한 또는 플랫폼 중재자/관리자 권한이 필요합니다.
...
관리자 전용 표준 배치 재스캔 라우트입니다. 레거시 `POST /api/v1/skills/-/rescan-batch`와 동일한 페이로드 형식을 허용합니다.
...
- `ok`는 선택된 버전에 생성된 Skill Card가 있고, 중재에 의해 악성 코드로 차단되지 않았으며, ClawScan 검증이 깨끗한 경우에만 `true`입니다.
- 셸 자동화가 중첩 래퍼를 풀지 않고도 읽을 수 있도록 Skills ID, 게시자 ID, 선택한 버전 메타데이터는 최상위 봉투 필드(`slug`, `displayName`, `publisherHandle`, `version`, `resolvedFrom`, `tag`, `createdAt`)입니다.
...
- `provenance`는 ClawHub가 게시 또는 가져오기 중 GitHub 리포지토리/ref/커밋/경로를 해석하고 저장한 경우에만 `server-resolved-github-import`입니다. 그렇지 않으면 `unavailable`입니다.

### `POST /api/v1/skills/-/security-verdicts`

정확한 Skills 버전에 대한 현재의 압축된 보안 판정을 반환합니다. 이 컬렉션 엔드포인트는 OpenClaw Control UI처럼 표시해야 하는 설치된 ClawHub Skills 버전을 이미 알고 있는 클라이언트를 위한 것입니다.
...
- 결과는 항목별입니다. 하나의 누락된 Skills 또는 버전이 전체 응답을 실패시키지는 않습니다.
...
- `security.signals.dependencyRegistry`는 v1 응답 호환성을 위해 유지되지만, 의존성 레지스트리 존재 스캐너는 폐기되었으며 이 키는 항상 `null`입니다.
- Skill Card 부재는 이 엔드포인트의 `ok`, `decision` 또는 `reasons`에 영향을 주지 않습니다. 클라이언트가 카드 콘텐츠가 필요할 때는 설치된 `skill-card.md`를 로컬에서 읽어야 합니다.
- 단일 Skills Skill Card 검증 봉투가 필요할 때는 `/verify`, 생성된 카드 Markdown이 필요할 때는 `/card`, 상세 스캐너 데이터가 필요할 때는 `/scan`을 사용하세요.
...
- `trust.scanStatus`는 스캐너 입력과 수동 릴리스 중재에서 파생된 유효 신뢰
  상태입니다.
- `trust.moderationState`는 null이 가능하며, 수동 릴리스 중재가 없으면
  `null`입니다.
- `trust.blockedFromDownload`는 설치 차단 신호입니다. OpenClaw와 기타 설치
  클라이언트는 스캐너 또는 중재 필드에서 차단 규칙을 다시 파생하는 대신 이 값이
  `true`이면 설치를 차단해야 합니다.
...
- 이 엔드포인트는 소유자/중재자 중재 엔드포인트보다 의도적으로 더 좁습니다.
  신고자 신원, 신고 본문, 비공개 증거 또는 내부 검토 타임라인이 아니라 설치
  결정과 공개 설명을 노출합니다.
...
연결됩니다. 신고는 중재 대기열로 전달되지만 그 자체로 다운로드를 자동으로 숨기거나
...
패키지 검토 공개 상태를 위한 소유자/모더레이터 엔드포인트입니다.

인증:

- 패키지 소유자, 게시자 구성원, 모더레이터 또는 관리자 사용자용 API
  토큰이 필요합니다.
...
`open`으로 설정할 때는 생략할 수 있습니다. 확인된 신고에 대해
동일한 감사 가능한 워크플로에서 릴리스 검토를 적용하려면
`finalAction: "quarantine"` 또는 `finalAction: "revoke"`를 전달합니다.
...
- `revoked`: 이전에 신뢰된 릴리스가 이후 차단되었습니다.
...
- 응답에는 리졸버 무결성 검사를 위한 `ETag`, `Digest`,
  `X-ClawHub-Artifact-Type`, `X-ClawHub-Artifact-Sha256` 헤더가 포함됩니다.
...
- 사용자가 원할 경우 npm이 미러를 가리키도록 할 수 있도록
  `dist.tarball`, `dist.integrity`, `dist.shasum`은 npm 호환 필드를 사용합니다.
- 스코프가 지정된 패키지 packument는 `/api/npm/@scope/name` 및 npm의
...
호스팅된 Skills 버전 ZIP을 다운로드하거나, `clean` 또는 `suspicious` 검사 결과가 있고
...
- 선택적 페이로드 필드: `migrateOwner`. `ownerHandle`과 함께 `true`이면,
  행위자가 현재 및 대상 게시자 모두에서 관리자/소유자인 경우 기존 스킬을 해당
  소유자로 이동할 수 있습니다. 이 명시적 동의가 없으면 소유자 변경은 거부됩니다.
...
- 허용되는 폼 필드는 `payload`, 반복되는 `files` blob 또는 하나의 `clawpack`
  tarball 참조입니다. `clawpack`은 `.tgz` blob이거나 upload-url 플로에서
  반환된 storage id일 수 있습니다. 스테이징된 storage-id 게시에는 해당 업로드 URL과
  함께 반환된 `clawpackUploadTicket`도 포함해야 합니다.
...
- 직접 multipart 게시 요청은 18MB로 제한됩니다. ClawPack tarball은
  upload-url 플로를 사용해 최대 120MB tarball 한도까지 사용할 수 있습니다.
...
- `openclaw` 조직 게시자 및 현재 `openclaw` 조직 구성원의
...
제공되면 `reason`이 스킬 검토 메모로 저장되고 감사 로그에 복사됩니다.
소유자가 시작한 소프트 삭제는 slug를 30일 동안 예약한 뒤, 다른 게시자가
해당 slug를 요청할 수 있습니다. 이 만료가 적용될 때 삭제 응답에는
`slugReservedUntil`이 포함됩니다. 모더레이터/관리자 숨김 및 보안 제거에는
이 방식의 만료가 적용되지 않습니다.
...
마이그레이션합니다. 새로 생성된 조직에는 `memberHandle`을 제공하세요. 작업을 수행하는
관리자는 구성원으로 추가되지 않습니다. `memberRole`의 기본값은 `owner`입니다.
...
변경 가능한 핸들은 운영자 대상 보호 장치로만 사용됩니다.

엔드포인트는 기본적으로 드라이런으로 동작합니다. 복구를 적용하려면 직원이 두 GitHub 주체 간의 연속성을 독립적으로 확인한 뒤 `dryRun: false` 및
`confirmIdentityVerified: true`가 필요합니다. 대상 사용자의 현재 개인
게시자에 Skills, 패키지 또는 GitHub Skill 소스가 있으면 복구는 닫힌 상태로 실패합니다.
복구는 복구된 게시자의 Skills, Skill 슬러그 별칭, 패키지, 패키지 검사기 경고, 파생 검색 다이제스트 행에 대한 기존 `ownerUserId` 필드도 마이그레이션하여
...
차단 해제 또는 콘텐츠 복원 없이 기존 차단에 저장된 사유를 변경합니다(관리자 전용). `dryRun`이 `false`가 아니면 기본값은 드라이런입니다.
...
`POST /api/cli/upload-url`은 `uploadUrl` 및 `uploadTicket`을 반환합니다. ClawPack 타르볼을 스테이징하는 패키지
