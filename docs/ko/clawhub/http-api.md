...
x-i18n:
    generated_at: "2026-07-01T07:55:13Z"
    model: gpt-5.5
...
타사 디렉터리는 공개 읽기 엔드포인트를 사용해 ClawHub Skills를 나열하거나 검색할 수 있습니다. 결과를 캐시하고, `429`/`Retry-After`를 준수하며, 사용자를 표준 ClawHub 목록(`https://clawhub.ai/<owner>/skills/<slug>`)으로 다시 연결하고, ClawHub가 타사 사이트를 보증한다고 암시하지 마세요. 공개 API 표면 외부에서 숨김, 비공개 또는 검토 차단 콘텐츠를 미러링하려고 시도하지 마세요.

웹 슬러그 바로 가기는 레지스트리 제품군 전반에서 확인되지만, API 클라이언트는 라우트 우선순위를 재구성하는 대신 읽기 엔드포인트가 반환한 표준 URL을 사용해야 합니다.
...
- 인증된 쓰기 엔드포인트는 서버가 이유를 알고 있을 때 단순한 `Unauthorized`만 반환해서는 안 됩니다. 토큰 누락, 유효하지 않거나 취소된 토큰, 삭제/차단/비활성화된 계정은 각각 실행 가능한 텍스트를 받아야 CLI 클라이언트가 사용자에게 차단 이유를 알려줄 수 있습니다.
...
- `X-RateLimit-Remaining` / `RateLimit-Remaining`: 존재할 때 정확한 남은 예산입니다.
...
- 신뢰할 수 있는 클라이언트 IP를 사용할 수 없으면 익명 요청은 속도 제한 종류로만 범위가 지정된 대체 버킷을 사용합니다. 이러한 대체 버킷에는 호출자가 제공한 경로, 슬러그, 패키지 이름, 버전, 쿼리 문자열 또는 기타 아티팩트 매개변수가 포함되지 않습니다.
...
- 관련도는 인기도보다 더 강합니다. 정확한 슬러그 또는 표시 이름 토큰 일치는 참여도가 훨씬 높은 느슨한 일치보다 높은 순위에 오를 수 있습니다.
- ASCII 텍스트는 단어 및 문장 부호 경계에서 토큰화됩니다. 예를 들어 `personal-map`에는 독립적인 `map` 토큰이 포함되고, `amap-jsapi-skill`에는 `amap`, `jsapi`, `skill`이 포함됩니다. 따라서 `map`을 검색하면 `amap-jsapi-skill`보다 `personal-map`에 더 강한 어휘 일치가 부여됩니다.
...
- 사용자가 실제로 검색할 용어를 표시 이름, 요약, 태그에 넣으세요. 독립적인 슬러그 토큰은 유지하려는 안정적인 식별자이기도 할 때만 사용하세요.
- 새 슬러그가 더 나은 장기 표준 이름이 아닌 한, 하나의 쿼리를 추적하기 위해 슬러그를 변경하지 마세요. 이전 슬러그는 리디렉션 별칭이 되지만, 표준 URL, 표시되는 슬러그, 향후 검색 다이제스트는 새 슬러그를 사용합니다.
- 이름 변경 별칭은 이전 URL 및 레지스트리를 통해 확인되는 설치의 해석을 보존하지만, 검색 순위는 이름 변경이 인덱싱된 후 표준 Skill 메타데이터를 기반으로 합니다. 기존 통계는 Skill에 유지됩니다.
- Skill이 예기치 않게 보이지 않으면 순위 관련 메타데이터를 변경하기 전에 로그인한 상태에서 `clawhub inspect @owner/slug`로 먼저 검토 상태를 확인하세요.
...
- 소유자 이름 변경/병합 흐름으로 생성된 이전 슬러그는 표준 Skill로 확인됩니다.
...
- 소유자와 중재자는 숨겨진 Skills에 대한 검토 세부 정보에 접근할 수 있습니다.
- 공개 호출자는 이미 플래그 지정된 표시 Skills에 대해서만 `200`을 받습니다.
- 증거는 공개 호출자에게는 수정 처리되며, 원시 스니펫은 소유자/중재자에게만 포함됩니다.

### `POST /api/v1/skills/{slug}/report`

중재자 검토를 위해 Skill을 신고합니다. 신고는 Skill 수준이며, 선택적으로 버전에 연결되고 Skill 신고 대기열에 공급됩니다.
...
Skill 신고 접수를 위한 중재자/관리자 엔드포인트입니다.
...
Skill 신고를 해결하거나 다시 여는 중재자/관리자 엔드포인트입니다.
...
`note`는 `confirmed` 및 `dismissed`에 필요합니다. `status`를 다시 `open`으로 설정할 때는 생략할 수 있습니다. 감사 가능한 동일 워크플로에서 Skill을 숨기려면 분류된 신고와 함께 `finalAction: "hide"`를 전달하세요.
...
- `version.security`는 사용 가능한 경우 정규화된 스캔 확인 상태 및 스캐너 세부 정보(VirusTotal + LLM)를 포함합니다.

### `GET /api/v1/skills/{slug}/scan`

Skill 버전에 대한 보안 스캔 확인 세부 정보를 반환합니다.
...
- `security.hasScanResult`는 스캐너가 명확한 판정(`clean`, `suspicious` 또는 `malicious`)을 생성한 경우에만 `true`입니다.
- `moderation`은 최신 버전에서 파생된 현재 Skill 수준의 검토 스냅샷입니다.
- 과거 버전을 쿼리할 때는 `moderation`과 `security`를 같은 버전 컨텍스트로 처리하기 전에 `moderation.matchesRequestedVersion`와 `moderation.sourceVersion`을 확인하세요.
...
- 게시된 스캔에는 소유자/게시자 관리 액세스 권한 또는 플랫폼 검토자/관리자 권한이 필요합니다.
...
- Skill 또는 Plugin에 대한 소유자/게시자 관리 액세스 권한 또는 플랫폼 검토자/관리자 권한이 필요합니다.
...
관리자 전용 정식 배치 재스캔 경로입니다. 레거시 `POST /api/v1/skills/-/rescan-batch`와 동일한 페이로드 형태를 허용합니다.

### `POST /api/v1/skills/-/scan/batch/status`

관리자 전용 정식 배치 상태 경로입니다. `{ "jobIds": ["..."] }`를 허용하고 레거시 `POST /api/v1/skills/-/rescan-batch/status`와 동일한 집계 카운터를 반환합니다.
...
- `ok`는 선택된 버전에 생성된 Skill Card가 있고, 검토에 의해 악성 코드로 차단되지 않았으며, ClawScan 검증이 깨끗한 경우에만 `true`입니다.
- Skill ID, 게시자 ID, 선택된 버전 메타데이터는 최상위 엔벨로프 필드(`slug`, `displayName`, `publisherHandle`, `version`, `resolvedFrom`, `tag`, `createdAt`)이므로 셸 자동화가 중첩 래퍼를 풀지 않고도 읽을 수 있습니다.
...
- `security.signals.dependencyRegistry`는 v1 응답 호환성을 위해 유지되지만, 의존성 레지스트리 존재 스캐너는 폐기되었으며 이 키는 항상 `null`입니다.
- Skill Card가 없더라도 이 엔드포인트의 `ok`, `decision` 또는 `reasons`에는 영향을 주지 않습니다. 카드 콘텐츠가 필요한 클라이언트는 설치된 `skill-card.md`를 로컬에서 읽어야 합니다.
- 단일 Skill의 Skill Card 검증 엔벨로프가 필요하면 `/verify`를, 생성된 카드 Markdown이 필요하면 `/card`를, 자세한 스캐너 데이터가 필요하면 `/scan`을 사용하세요.
...
- 본문: ZIP 아카이브.
- 내보낸 각 Skills는 `{publisher}/{slug}/`를 루트로 합니다.
...
- `clean` 또는 `suspicious` 스캔이 있는 현재 GitHub 기반 Skills에는
  `sourceRef: "public-github"`, 저장소, 커밋, 경로, 콘텐츠 해시, 아카이브 URL이 포함된
  `_source_handoff.json`이 포함됩니다. ClawHub 호스팅 소스 파일은 포함하지 않습니다.
...
파일 메타데이터, 호환성, 검증, 아티팩트 메타데이터 및 스캔 데이터를 포함한 하나의 패키지 버전을 반환합니다.
...
- `version.sha256hash`는 구형 클라이언트를 위한 사용 중단된 호환성 메타데이터입니다. 이는
...
- `trust.blockedFromDownload`는 설치 차단 신호입니다. OpenClaw 및 기타 설치 클라이언트는
  스캐너 또는 중재 필드에서 차단 규칙을 다시 도출하는 대신 이 값이 `true`일 때 설치를 차단해야 합니다.
...
- 이 엔드포인트는 버전 정확성을 보장합니다. 클라이언트는 최신 패키지 메타데이터를 읽은 뒤가 아니라,
  설치하려는 패키지 버전을 확인한 뒤 호출해야 합니다.
- 호출자가 소유 게시자를 읽을 수 없는 경우 비공개 패키지는 `404`를 반환합니다.
- 이 엔드포인트는 소유자/중재자 중재 엔드포인트보다 의도적으로 좁습니다. 설치 결정과 공개 설명만 노출하며,
  신고자 ID, 신고 본문, 비공개 증거 또는 내부 검토 일정은 노출하지 않습니다.
...
중재자 검토를 위해 패키지를 신고합니다. 신고는 패키지 수준이며 선택적으로 버전에 연결됩니다. 신고는 중재 대기열에 반영되지만 그 자체로 다운로드를 자동 숨김 처리하거나 차단하지는 않습니다. 중재자는 릴리스 중재를 사용하여 아티팩트를 승인, 격리 또는 취소해야 합니다.
...
패키지 모더레이션 표시 여부를 위한 소유자/모더레이터 엔드포인트입니다.
...
`finalAction: "revoke"`를 전달하면 동일한 감사 가능한 워크플로에서 릴리스
모더레이션을 적용할 수 있습니다.
...
- 기존 ZIP 전용 버전은 의도적으로 제외됩니다.
- 사용자가 원할 경우 npm이 미러를 가리키도록 할 수 있도록 `dist.tarball`, `dist.integrity` 및 `dist.shasum`은 npm 호환
  필드를 사용합니다.
- 범위가 지정된 패키지 packument는 `/api/npm/@scope/name`과 npm의
...
호스팅된 Skills 버전 ZIP을 다운로드하거나, `clean` 또는 `suspicious` 스캔이 있고 호스팅된
버전이 없는 현재 GitHub 기반 Skills에 대한 GitHub 소스 인계를 반환합니다.
...
- 선택적 페이로드 필드: `ownerHandle`. 제공되면 API가 서버 측에서 해당
  게시자를 확인하고 행위자에게 게시자 접근 권한이 있어야 합니다.
- 선택적 페이로드 필드: `migrateOwner`. `ownerHandle`과 함께 `true`이면
  행위자가 현재 게시자와 대상 게시자 모두에서 관리자/소유자인 경우
  기존 Skills가 해당 소유자로 이동할 수 있습니다. 이 옵트인이 없으면 소유자 변경은
  거부됩니다.
...
- 같은 요청에서는 `files` 또는 `clawpack` 중 하나만 사용하고 둘 다 사용하지 마세요.
- JSON 본문과 호출자가 제공한 `payload.files` / `payload.artifact`
  메타데이터는 거부됩니다.
- 직접 multipart 게시 요청은 18MB로 제한됩니다. ClawPack tarball은
  upload-url 흐름을 사용하여 최대 120MB tarball 한도까지 사용할 수 있습니다.
- 선택적 페이로드 필드: `ownerHandle`. 제공되면 관리자만 해당 소유자를 대신해 게시할 수 있습니다.
...
- 대리 게시는 여전히 대상 소유자 계정 기준으로 official-channel 자격을 검증합니다.

### `DELETE /api/v1/skills/{slug}` / `POST /api/v1/skills/{slug}/undelete`

Skills를 소프트 삭제/복원합니다(소유자, 모더레이터 또는 관리자).
...
제공되면 `reason`은 Skills 모더레이션 메모로 저장되고 감사 로그에 복사됩니다.
소유자가 시작한 소프트 삭제는 slug를 30일 동안 예약한 다음, 다른 게시자가
해당 slug를 클레임할 수 있습니다. 이 만료가 적용되면 삭제 응답에 `slugReservedUntil`이 포함됩니다.
모더레이터/관리자의 숨김 및 보안 제거에는 이 방식으로 만료가 적용되지 않습니다.
...
관리자 전용입니다. 릴리스를 게시하지 않고 적법한 소유자를 위해 루트 slug와 패키지 이름을 예약합니다.
...
제공자 계정 ID가 모두 명시되어야 합니다. 변경 가능한 핸들은 운영자 대상 가드로만 사용됩니다.
...
게시자에 skills, packages 또는 GitHub skill sources가 있으면 복구는 닫힌 상태로 실패합니다.
복구는 또한 복구된 게시자의 skills, skill slug aliases, packages, package inspector warnings,
파생된 search digest rows에 있는 레거시 `ownerUserId` 필드를 마이그레이션하여
직접 소유자 경로가 새 게시자 권한과 일치하도록 합니다. 복구된 handle에 대한 활성 protected-handle
...
스테이징하는 Package 게시 작업은 결과 storage id를
