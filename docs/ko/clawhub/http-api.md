...
서드파티 디렉터리는 공개 읽기 엔드포인트를 사용하여 ClawHub Skills를 나열하거나 검색할 수 있습니다. 결과를 캐시하고, `429`/`Retry-After`를 준수하며, 사용자를 표준 ClawHub 목록(`https://clawhub.ai/<owner>/<slug>`)으로 다시 연결하고, 서드파티 사이트를 ClawHub가 보증한다는 암시를 피하세요. 공개 API 표면 밖에서 숨김, 비공개 또는 검토 차단된 콘텐츠를 미러링하려고 시도하지 마세요.
...
- `Retry-After`: `429` 시 재시도 전 대기할 초(지연)
...
- ClawHub는 신뢰할 수 있는 전달 헤더를 사용하여 에지에서 클라이언트 IP를 식별합니다.
- 신뢰할 수 있는 클라이언트 IP를 사용할 수 없으면, 익명 다운로드 요청은 하나의 전역 `ip:unknown` 버킷 대신 엔드포인트 범위의 대체 버킷을 사용합니다. 익명 읽기/쓰기 요청은 여전히 공유 unknown 버킷을 사용하므로 누락 IP 라우팅이 계속 보이고 보수적으로 유지됩니다.
...
- 결과는 관련도 순서로 반환됩니다(임베딩 유사도 + 정확한 슬러그/이름 토큰 가중치 + 다운로드 기반 인기 사전값).
- 관련도는 인기보다 더 강합니다. 정확한 슬러그 또는 표시 이름 토큰 일치는 다운로드가 훨씬 많은 느슨한 일치보다 높은 순위를 차지할 수 있습니다.
...
- 다운로드는 기본 순위 신호가 아니라 작은 로그 스케일 사전값 및 동률 결정 요소로 사용됩니다. 쿼리 텍스트 일치가 더 약하면 다운로드가 많은 Skills도 더 낮은 순위를 차지할 수 있습니다.
- 의심스럽거나 숨김 처리된 검토 상태는 호출자 필터와 현재 검토 상태에 따라 공개 검색에서 Skill을 제거할 수 있습니다.

게시자 검색 가능성 지침:

- 사용자가 실제로 검색할 용어를 표시 이름, 요약, 태그에 넣으세요. 독립형 슬러그 토큰은 유지하려는 안정적인 정체성이기도 한 경우에만 사용하세요.
- 새 슬러그가 더 나은 장기 표준 이름이 아니라면 단일 쿼리를 노리고 슬러그를 변경하지 마세요. 이전 슬러그는 리디렉션 별칭이 되지만, 표준 URL, 표시되는 슬러그 및 이후 검색 다이제스트는 새 슬러그를 사용합니다.
- 이름 변경 별칭은 이전 URL과 레지스트리를 통해 해석되는 설치의 해석을 유지하지만, 검색 순위는 이름 변경이 인덱싱된 후의 표준 Skill 메타데이터를 기반으로 합니다. 기존 통계는 Skill에 유지됩니다.
...
- `nonSuspiciousOnly=true`인 경우, 페이지를 가져온 후 의심스러운 Skills가 필터링되므로 커서 기반 정렬은 한 페이지에서 `limit`보다 적은 항목을 반환할 수 있습니다.
- 존재하는 경우 `nextCursor`를 사용하여 페이지 매김을 계속합니다. 짧은 페이지 자체만으로 결과 끝을 의미하지는 않습니다.
...
- `moderation`은 Skill이 플래그 지정되었거나 소유자가 볼 때만 포함됩니다.
...
- 증거는 공개 호출자에게는 편집되어 제공되며, 소유자/검토자에게만 원본 스니펫을 포함합니다.

### `POST /api/v1/skills/{slug}/report`

검토자 검토를 위해 Skill을 신고합니다. 신고는 Skill 수준이며, 선택적으로 버전에 연결되고 Skill 신고 대기열에 입력됩니다.
...
- `security.capabilityTags`는 감지된 경우 `crypto`, `requires-wallet`, `can-make-purchases`, `can-sign-transactions`, `requires-oauth-token`, `posts-externally`와 같은 결정적 기능/위험 레이블을 포함합니다.
...
- 과거 버전을 쿼리할 때는 `moderation`과 `security`를 동일한 버전 컨텍스트로 처리하기 전에 `moderation.matchesRequestedVersion` 및 `moderation.sourceVersion`을 확인하세요.
...
- `target` / `hostTarget`(선택 사항): `host:<target>`의 단축 표기
- `os`, `arch`, `libc`(선택 사항): 호스트 기능 필터의 단축 표기
- `requiresBrowser`, `requiresDesktop`, `requiresNativeDeps`,
  `requiresExternalService`, `requiresBinary`, `requiresOsPermission`
  (선택 사항): 환경 요구 사항 태그에 대한 `true`/`1` 단축 표기
- `externalService`, `binary`, `osPermission`(선택 사항): 이름이 지정된
  환경 요구 사항 태그의 단축 표기
- `artifactKind`(선택 사항): `legacy-zip` 또는 `npm-pack`
- `npmMirror`(선택 사항): npm 미러를 통해 사용할 수 있는 ClawPack 기반
  패키지 버전을 표시하려면 `true`/`1`
...
Skills + Plugin 패키지를 아우르는 통합 카탈로그 검색입니다.
...
- `target` / `hostTarget`, `os`, `arch`, `libc`, `requiresBrowser`,
  ...
  `osPermission`은 일반적인 기능 태그의 단축 표기로 허용됩니다
- `artifactKind`(선택 사항): `legacy-zip` 또는 `npm-pack`
- `npmMirror`(선택 사항): npm 미러를 통해 사용할 수 있는 ClawPack 기반
  패키지 버전을 검색하려면 `true`/`1`
...
- private 패키지는 호출자가 소유 게시자를 읽을 수 없는 경우 `404`를 반환합니다.
...
설치 클라이언트용 정확한 패키지 릴리스 보안 및 신뢰 요약을 반환합니다. 이는
해결된 릴리스를 설치할 수 있는지 판단하기 위한 공개 OpenClaw 소비 표면입니다.
...
- `package.name`, `package.displayName` 및 `package.family`는 해결된
  레지스트리 패키지를 식별합니다.
...
- `trust.reasons`는 사용자 표시 및 감사용 설명 목록입니다. 사유 코드는
  `manual:quarantined`, `scan:malicious`, `static:malicious`, `vt:suspicious`,
  `package:malicious` 같은 안정적이고 간결한 문자열입니다.
- `trust.pending`은 하나 이상의 신뢰 입력이 아직 완료를 기다리고 있음을 의미합니다.
- `trust.stale`은 신뢰 요약이 오래된 입력으로 계산되었으며, 높은 신뢰도의
  허용 결정을 내리기 전에 새로 고침이 필요한 것으로 처리해야 함을 의미합니다.

참고 사항:

- 이 엔드포인트는 버전 정확성을 보장합니다. 클라이언트는 최신 패키지 메타데이터를
  읽은 직후가 아니라, 설치하려는 패키지 버전을 해결한 뒤 이 엔드포인트를 호출해야 합니다.
- private 패키지는 호출자가 소유 게시자를 읽을 수 없는 경우 `404`를 반환합니다.
- 이 엔드포인트는 소유자/모더레이터 중재 엔드포인트보다 의도적으로 좁습니다.
  설치 결정과 공개 설명을 노출하며, 신고자 신원, 신고 본문, private 증거 또는
...
- ClawPack 버전은 `npm-pack` 아티팩트, npm 무결성 필드, `tarballUrl` 및
  기존 ZIP 호환성 URL을 반환합니다.
- 이는 OpenClaw 해석기 표면입니다. 공유 URL에서 아카이브 형식을 추측하지 않도록 합니다.
...
검토자를 위한 패키지를 신고합니다. 신고는 패키지 수준이며, 선택적으로 버전에
연결할 수 있습니다. 신고는 조정 대기열로 전달되지만, 그 자체만으로 자동 숨김이나
다운로드 차단을 수행하지 않습니다. 조정자는 릴리스 조정을 사용해 아티팩트를
승인, 격리 또는 취소해야 합니다.
...
패키지 신고 접수를 위한 조정자/관리자 엔드포인트입니다.
...
- 패키지 소유자, 게시자 구성원, 조정자 또는 관리자 사용자의 API 토큰이 필요합니다.
...
설정할 때는 생략할 수 있습니다. 확인된 신고와 함께 `finalAction: "quarantine"`
또는 `finalAction: "revoke"`를 전달하면 동일한 감사 가능한 워크플로에서 릴리스
조정을 적용합니다.
...
이전 패키지 릴리스에 명시적 아티팩트 종류 메타데이터를 라벨링하기 위한
...
- 기본값은 드라이 런입니다.
- ClawPack 스토리지가 없는 릴리스는 `legacy-zip`으로 라벨링됩니다.
...
- 파일 크기 제한: 200KB.
- 보류 중인 VirusTotal 검사는 읽기를 차단하지 않습니다. 악성 릴리스는 다른 곳에서 보류될 수 있습니다.
...
- 보류 중인 VirusTotal 검사는 다운로드를 차단하지 않습니다. 악성 릴리스는 `403`을 반환합니다.
...
- 선택적 페이로드 필드: `migrateOwner`. `ownerHandle`과 함께 `true`이면 행위자가
  현재 게시자와 대상 게시자 모두에서 관리자/소유자인 경우 기존 스킬을 해당
...
- Plugin 패키지에는 `openclaw.plugin.json`이 필요합니다. ClawPack `.tgz` 업로드에는
  `package/openclaw.plugin.json` 위치에 이를 포함해야 합니다.
...
스킬을 소프트 삭제/복원합니다(소유자, 조정자 또는 관리자).
...
있는 경우 `reason`은 스킬 조정 참고 사항으로 저장되고 감사 로그에 복사됩니다.
소유자가 시작한 소프트 삭제는 slug를 30일 동안 예약하며, 이후 다른 게시자가
해당 slug를 사용할 수 있습니다. 이 만료가 적용되는 경우 삭제 응답에는
`slugReservedUntil`이 포함됩니다. 조정자/관리자 숨김 및 보안 제거는 이러한 방식으로
만료되지 않습니다.
...
이름을 예약합니다. 패키지 이름은 릴리스 행이 없는 비공개 자리 표시자 패키지가 되므로,
...
사용자를 차단하고 소유한 Skills를 완전 삭제합니다(중재자/관리자만).
