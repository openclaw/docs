...
타사 디렉터리는 ClawHub Skills를 나열하거나 검색하기 위해 공개 읽기 엔드포인트를 사용할 수 있습니다. 결과를 캐시하고, `429`/`Retry-After`를 준수하며, 사용자를 정식 ClawHub 목록(`https://clawhub.ai/<owner>/<slug>`)으로 다시 연결하고, 타사 사이트에 대한 ClawHub의 보증을 암시하지 마십시오. 공개 API 표면 밖에서 숨김, 비공개 또는 검토 차단된 콘텐츠를 미러링하려고 시도하지 마십시오.

웹 slug 바로가기는 레지스트리 패밀리 전반에서 확인되지만, API 클라이언트는 라우트 우선순위를 재구성하는 대신 읽기 엔드포인트가 반환하는 정식 URL을 사용해야 합니다.
...
- 인증된 쓰기 엔드포인트는 서버가 이유를 알고 있을 때 단순한 `Unauthorized`만 반환해서는 안 됩니다. 토큰 누락, 유효하지 않거나 해지된 토큰, 삭제/차단/비활성화된 계정에는 각각 실행 가능한 텍스트가 포함되어야 CLI 클라이언트가 사용자에게 무엇이 차단했는지 알려줄 수 있습니다.
...
- 동기화된 재시도를 피하기 위해 지터가 적용된 백오프를 사용합니다.
...
- 신뢰할 수 있는 클라이언트 IP를 사용할 수 없으면 익명 다운로드 요청은 하나의 전역 `ip:unknown` 버킷 대신 엔드포인트 범위의 대체 버킷을 사용합니다. 익명 읽기/쓰기 요청은 여전히 공유 unknown 버킷을 사용하므로 IP 누락 라우팅이 계속 표시되고 보수적으로 유지됩니다.
...
- 결과는 관련도 순서로 반환됩니다(임베딩 유사도 + 정확한 slug/이름 토큰 부스트 + 다운로드의 인기도 사전값).
- 관련도는 인기도보다 더 강합니다. 정확한 slug 또는 표시 이름 토큰 일치는 훨씬 더 많은 다운로드가 있는 느슨한 일치보다 높은 순위를 차지할 수 있습니다.
- ASCII 텍스트는 단어 및 문장부호 경계에서 토큰화됩니다. 예를 들어 `personal-map`에는 독립형 `map` 토큰이 포함되는 반면, `amap-jsapi-skill`에는 `amap`, `jsapi`, `skill`이 포함됩니다. 따라서 `map`을 검색하면 `amap-jsapi-skill`보다 `personal-map`이 더 강한 어휘 일치를 얻습니다.
- 다운로드는 기본 순위 신호가 아니라 작은 로그 스케일 사전값 및 동점 해결 기준으로 사용됩니다. 쿼리 텍스트 일치가 약하면 다운로드가 많은 Skills도 더 낮은 순위를 차지할 수 있습니다.
- 의심스럽거나 숨김 처리된 검토 상태는 호출자 필터와 현재 검토 상태에 따라 공개 검색에서 Skills를 제거할 수 있습니다.

게시자 발견 가능성 지침:

- 사용자가 실제로 검색할 용어를 표시 이름, 요약, 태그에 넣으십시오. 독립형 slug 토큰은 유지하려는 안정적인 식별자이기도 할 때만 사용하십시오.
- 새 slug가 더 나은 장기 정식 이름이 아닌 한, 단일 쿼리를 좇기 위해 slug 이름을 바꾸지 마십시오. 이전 slug는 리디렉션 별칭이 되지만, 정식 URL, 표시되는 slug, 향후 검색 digest는 새 slug를 사용합니다.
- 이름 변경 별칭은 레지스트리를 통해 확인되는 이전 URL과 설치에 대한 확인을 보존하지만, 검색 순위는 이름 변경이 인덱싱된 후 정식 Skills 메타데이터를 기반으로 합니다. 기존 통계는 Skills에 유지됩니다.
- Skills가 예기치 않게 보이지 않으면 순위 관련 메타데이터를 변경하기 전에 로그인한 상태에서 `clawhub inspect <slug>`로 먼저 검토 상태를 확인하십시오.
...
- `nextCursor`가 있으면 이를 사용해 페이지네이션을 계속하십시오. 짧은 페이지 자체가 결과의 끝을 의미하지는 않습니다.
...
- 소유자 이름 변경/병합 흐름으로 생성된 이전 slug는 정식 Skills로 확인됩니다.
- `metadata.os`: Skills frontmatter에 선언된 OS 제한(예: `["macos"]`, `["linux"]`). 선언되지 않으면 `null`입니다.
...
- 소유자와 moderator는 숨김 Skills의 검토 세부 정보에 액세스할 수 있습니다.
- 공개 호출자는 이미 플래그 지정된 표시 Skills에 대해서만 `200`을 받습니다.
- 공개 호출자에 대해서는 증거가 편집되며, 원시 snippet은 소유자/moderator에게만 포함됩니다.

### `POST /api/v1/skills/{slug}/report`

moderator 검토를 위해 Skills를 신고합니다. 신고는 Skills 수준이며, 선택적으로 버전에 연결되고 Skills 신고 큐에 전달됩니다.
...
Skills 신고 접수를 위한 moderator/admin 엔드포인트입니다.
...
Skills 신고를 해결하거나 다시 여는 moderator/admin 엔드포인트입니다.
...
`note`는 `confirmed` 및 `dismissed`에 필요합니다. `status`를 다시 `open`으로 설정할 때는 생략할 수 있습니다. 감사 가능한 동일한 workflow에서 Skills를 숨기려면 triage된 신고와 함께 `finalAction: "hide"`를 전달하십시오.
...
- `version.security`는 사용 가능한 경우 정규화된 스캔 검증 상태와 scanner 세부 정보(VirusTotal + LLM)를 포함합니다.
...
- `tag`(선택): 태그가 지정된 버전을 확인합니다(예: `latest`).

참고:

- `version`과 `tag`가 모두 제공되지 않으면 최신 버전을 사용합니다.
- 정규화된 검증 상태와 scanner별 세부 정보를 포함합니다.
- `security.capabilityTags`는 감지된 경우 `crypto`, `requires-wallet`, `can-make-purchases`, `can-sign-transactions`, `requires-oauth-token`, `posts-externally`와 같은 결정적 capability/risk label을 포함합니다.
- `security.hasScanResult`는 scanner가 확정 판정(`clean`, `suspicious` 또는 `malicious`)을 생성한 경우에만 `true`입니다.
- `moderation`은 최신 버전에서 파생된 현재 Skills 수준 검토 snapshot입니다.
- 과거 버전을 쿼리할 때는 `moderation`과 `security`를 동일한 버전 context로 취급하기 전에 `moderation.matchesRequestedVersion` 및 `moderation.sourceVersion`을 확인하십시오.
...
- `version.artifact.kind`는 구형 패키지 아카이브의 경우 `legacy-zip`이고
...
- 이것은 OpenClaw 리졸버 표면입니다. 공유 URL에서 아카이브 형식을 추측하지 않도록 합니다.
...
중재자 검토를 위해 패키지를 신고합니다. 신고는 패키지 수준이며, 선택적으로 버전에 연결됩니다. 신고는 중재 대기열에 반영되지만 그 자체로 다운로드를 자동으로 숨기거나 차단하지 않습니다. 중재자는 릴리스 중재를 사용해 아티팩트를 승인, 격리 또는 취소해야 합니다.
...
`note`는 `confirmed` 및 `dismissed`에 필요합니다. `status`를 다시 `open`으로 설정할 때는 생략할 수 있습니다. 확인된 보고서와 함께 `finalAction: "quarantine"` 또는 `finalAction: "revoke"`를 전달하면 동일한 감사 가능한 워크플로에서 릴리스 조정을 적용할 수 있습니다.
...
격리되거나 폐기된 릴리스는 아티팩트 다운로드 라우트에서 `403`을 반환합니다.
모든 변경 사항은 감사 로그 항목을 작성합니다.

### `POST /api/v1/packages/backfill/artifacts`

이전 패키지 릴리스에 명시적인 아티팩트 종류 메타데이터를 레이블링하기 위한 관리자 전용 유지보수 엔드포인트입니다.
...
- 기본값은 드라이 런입니다.
- ClawPack 저장소가 없는 릴리스는 `legacy-zip`으로 레이블링됩니다.
...
- `dist.tarball`, `dist.integrity`, `dist.shasum`은 npm 호환 필드를 사용하므로 사용자는 원할 경우 npm이 미러를 가리키도록 할 수 있습니다.
...
- 선택적 페이로드 필드: `ownerHandle`. 이 값이 있으면 API는 서버 측에서 해당 게시자를 해석하고 행위자에게 게시자 접근 권한이 있어야 합니다.
- 선택적 페이로드 필드: `migrateOwner`. `ownerHandle`과 함께 `true`이면 행위자가 현재 게시자와 대상 게시자 모두에서 관리자/소유자인 경우 기존 스킬을 해당 소유자로 이동할 수 있습니다. 이 옵트인이 없으면 소유자 변경은 거부됩니다.
...
이 값이 있으면 `reason`은 스킬 조정 참고로 저장되고 감사 로그에 복사됩니다.
소유자가 시작한 소프트 삭제는 30일 동안 slug를 예약하며, 이후에는 다른 게시자가 해당 slug를 가져갈 수 있습니다. 이 만료가 적용되면 삭제 응답에는 `slugReservedUntil`이 포함됩니다.
...
관리자 전용입니다. 핸들에 대한 조직 게시자가 있는지 확인합니다. 핸들이 여전히 레거시 공유 사용자/개인 게시자를 가리키면, 엔드포인트는 먼저 이를 조직 게시자로 마이그레이션합니다.
...
관리자 전용입니다. 릴리스를 게시하지 않고 정당한 소유자를 위해 루트 slug와 패키지 이름을 예약합니다. 패키지 이름은 릴리스 행이 없는 비공개 자리표시자 패키지가 되므로, 동일한 소유자가 나중에 실제 코드 Plugin 또는 번들 Plugin 릴리스를 해당 이름으로 게시할 수 있습니다.
...
사용자를 차단하고 소유한 스킬을 하드 삭제합니다(조정자/관리자 전용).
