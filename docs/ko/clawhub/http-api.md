...
x-i18n:
    generated_at: "2026-07-04T20:27:40Z"
    model: gpt-5.5
...
타사 디렉터리는 공개 읽기 엔드포인트를 사용하여 ClawHub skills를 나열하거나 검색할 수 있습니다. 결과를 캐시하고, `429`/`Retry-After`를 준수하며, 사용자를 정식 ClawHub 목록(`https://clawhub.ai/<owner>/skills/<slug>`)으로 다시 연결하고, ClawHub가 타사 사이트를 보증한다고 암시하지 마세요. 공개 API 표면 밖에서 숨김, 비공개 또는 모더레이션으로 차단된 콘텐츠를 미러링하려고 시도하지 마세요.

웹 슬러그 단축 경로는 레지스트리 패밀리 전반에서 확인되지만, API 클라이언트는 라우트
...
- 인증된 쓰기 엔드포인트는 서버가 이유를 알고 있을 때 단순한 `Unauthorized`를 반환해서는 안 됩니다. 누락된 토큰, 유효하지 않거나 폐기된 토큰, 삭제/차단/비활성화된 계정에는 각각 실행 가능한 텍스트를 제공하여 CLI 클라이언트가 사용자에게 무엇이 차단했는지 알릴 수 있어야 합니다.
...
- `q`(필수): 쿼리 문자열
- `limit`(선택): 정수
- `highlightedOnly`(선택): 하이라이트된 skills로 필터링하려면 `true`
...
- 관련성은 인기도보다 강합니다. 정확한 슬러그 또는 표시 이름 토큰 일치는 참여도가 훨씬 높은 느슨한 일치보다 더 높은 순위를 차지할 수 있습니다.
...
- 인기도는 로그 스케일로 조정되고 상한이 적용됩니다. 참여도가 높은 skills도 쿼리 텍스트 일치가 약하면 더 낮은 순위를 차지할 수 있습니다.
- 의심스럽거나 숨김 모더레이션 상태는 호출자 필터와 현재 모더레이션 상태에 따라 공개 검색에서 skill을 제거할 수 있습니다.

게시자 검색 가능성 지침:

- 사용자가 실제로 검색할 용어를 표시 이름, 요약, 태그에 넣으세요. 독립적인 슬러그 토큰은 유지하려는 안정적인 식별자이기도 할 때만 사용하세요.
- 새 슬러그가 더 나은 장기 정식 이름이 아닌 한, 한 쿼리를 따라잡기 위해 슬러그를 변경하지 마세요. 이전 슬러그는 리디렉션 별칭이 되지만 정식 URL, 표시되는 슬러그, 향후 검색 다이제스트는 새 슬러그를 사용합니다.
- 이름 변경 별칭은 이전 URL과 레지스트리를 통해 확인되는 설치의 확인을 보존하지만, 검색 순위는 이름 변경이 인덱싱된 뒤 정식 skill 메타데이터를 기준으로 합니다. 기존 통계는 skill에 그대로 유지됩니다.
...
- `sort`(선택): `updated`(기본값), `recommended`(별칭: `default`), `createdAt`(별칭: `newest`), `downloads`, `stars`(별칭: `rating`), 레거시 설치 별칭 `installsCurrent`/`installs`/`installsAllTime`은 `downloads`로 매핑, `trending`
- `nonSuspiciousOnly`(선택): 의심스러운(`flagged.suspicious`) skills를 숨기려면 `true`
- `nonSuspicious`(선택): `nonSuspiciousOnly`의 레거시 별칭
...
- `nonSuspiciousOnly=true`인 경우 의심스러운 skills가 페이지 검색 후 필터링되므로 커서 기반 정렬은 한 페이지에서 `limit`보다 적은 항목을 반환할 수 있습니다.
- 존재하는 경우 `nextCursor`를 사용해 페이지네이션을 계속하세요. 짧은 페이지 자체만으로 결과 끝을 의미하지는 않습니다.
...
- 증거는 공개 호출자에게 편집되어 제공되며, 원시 스니펫은 소유자/모더레이터에게만 포함됩니다.
...
설정할 때는 생략할 수 있습니다. 감사 가능한 동일 워크플로에서 skill을 숨기려면 triage된
...
- `version.security`는 사용할 수 있는 경우 정규화된 스캔 검증 상태와 스캐너 세부 정보
...
- 과거 버전을 쿼리할 때는 `moderation`과 `security`를 동일한 버전 컨텍스트로 취급하기 전에 `moderation.matchesRequestedVersion`와 `moderation.sourceVersion`을 확인하세요.
...
- 대기 중일 때 `queue.queuedAhead`와 `queue.position`을 반환하므로 클라이언트는 요청보다 앞선 우선순위 수동 스캔 수를 표시할 수 있습니다. 매우 큰 큐는 제한되며 `queuedAheadIsEstimate: true`로 보고됩니다.
...
- Skill 또는 Plugin에 대한 소유자/게시자 관리 액세스 권한 또는 플랫폼 중재자/관리자 권한이 필요합니다.
...
관리자 전용 정식 배치 재스캔 라우트입니다. 레거시 `POST /api/v1/skills/-/rescan-batch`와 동일한 페이로드 형태를 받습니다.

### `POST /api/v1/skills/-/scan/batch/status`

관리자 전용 정식 배치 상태 라우트입니다. `{ "jobIds": ["..."] }`를 받고 레거시 `POST /api/v1/skills/-/rescan-batch/status`와 동일한 집계 카운터를 반환합니다.
...
- 선택한 버전에 생성된 Skill Card가 있고, 조정에 의해 악성 코드로 차단되지 않았으며, ClawScan 검증이 깨끗한 경우에만 `ok`가 `true`입니다.
- Skill ID, 게시자 ID 및 선택한 버전 메타데이터는 최상위 엔벌로프 필드(`slug`, `displayName`, `publisherHandle`, `version`, `resolvedFrom`, `tag`, `createdAt`)이므로 셸 자동화에서 중첩 래퍼를 풀지 않고 읽을 수 있습니다.
...
- `provenance`는 게시 또는 가져오기 중에 ClawHub가 GitHub repo/ref/commit/path를 확인하고 저장한 경우에만 `server-resolved-github-import`이며, 그 외에는 `unavailable`입니다.
...
- `items`는 고유한 `{ slug, version }` 쌍 1~100개를 포함해야 합니다.
- 결과는 항목별로 제공됩니다. 누락된 Skill 또는 버전 하나가 전체 응답을 실패시키지는 않습니다.
- 응답은 보안 전용입니다. Skill Card 데이터, 생성된 카드 상태, 아티팩트 파일 목록 또는 상세 스캐너 페이로드는 포함하지 않습니다.
- `security.signals`는 상태 수준의 보조 증거만 포함합니다. 전체 스캐너 세부 정보는 `/scan` 또는 ClawHub 보안 감사 페이지를 사용하세요.
- `security.signals.dependencyRegistry`는 v1 응답 호환성을 위해 유지되지만, 의존성 레지스트리 존재 스캐너는 폐기되었으며 이 키는 항상 `null`입니다.
- Skill Card가 없어도 이 엔드포인트의 `ok`, `decision` 또는 `reasons`에는 영향을 주지 않습니다. 클라이언트가 카드 콘텐츠가 필요할 때는 설치된 `skill-card.md`를 로컬에서 읽어야 합니다.
- 단일 Skill의 Skill Card 검증 엔벌로프가 필요할 때는 `/verify`를, 생성된 카드 마크다운이 필요할 때는 `/card`를, 상세 스캐너 데이터가 필요할 때는 `/scan`을 사용하세요.
...
- `category`(선택 사항): plugin 카테고리 필터. 요청 범위가 plugin 패키지(`/api/v1/plugins`,
  ...
  `family=code-plugin`/`family=bundle-plugin`이 포함된 패키지 엔드포인트)로 지정된 경우에만
  지원됩니다. 제어된 카테고리와 레거시 v1 필터 별칭은 `GET /api/v1/plugins` 아래에 문서화되어 있습니다.
...
Skills + plugin 패키지 전반에 걸친 통합 카탈로그 검색입니다.
...
- `clean` 또는 `suspicious` 스캔이 있는 현재 GitHub 기반 skills에는
  ...
  콘텐츠 해시 및 아카이브 URL과 함께 포함됩니다. ClawHub 호스팅 소스 파일은 포함되지 않습니다.
...
오프라인 분석을 위해 최신 공개 Plugin 릴리스를 대량으로 내보냅니다.
...
- `GET /api/v1/plugins`에 문서화된 기존 v1 필터 별칭도 허용됩니다.
...
파일 메타데이터, 호환성, 검증, 아티팩트 메타데이터, 스캔 데이터를 포함한 단일 패키지 버전을 반환합니다.
...
- `packageName`은 npm 이름으로 정규화되며, 계획된 마이그레이션의 경우 패키지가
  없을 수 있습니다.
...
버전에 연결됩니다. 이는 중재 큐에 제공되지만 자체적으로 다운로드를 자동 숨김하거나
차단하지 않습니다. 중재자는 릴리스 중재를 사용하여 아티팩트를 승인, 격리 또는
철회해야 합니다.
...
`note`는 `confirmed`와 `dismissed`에 필요하며, `status`를 다시 `open`으로
...
- 파일 크기 제한: 200KB.
- 대기 중인 VirusTotal 검사는 읽기를 차단하지 않으며, 악성 릴리스는 다른 곳에서 계속 보류될 수 있습니다.
...
- Plugin/패키지 아카이브는 이전 OpenClaw 클라이언트가 계속 작동하도록 `package/`
  루트가 있는 zip 파일입니다.
- 이 경로는 ZIP 전용으로 유지됩니다. ClawPack `.tgz` 파일을 스트리밍하지 않습니다.
- 응답에는 resolver 무결성 검사용 `ETag`, `Digest`, `X-ClawHub-Artifact-Type`,
  `X-ClawHub-Artifact-Sha256` 헤더가 포함됩니다.
- 레지스트리 전용 메타데이터는 다운로드된 아카이브에 삽입되지 않습니다.
- 대기 중인 VirusTotal 검사는 다운로드를 차단하지 않으며, 악성 릴리스는 `403`을 반환합니다.
...
npm 미러 클라이언트를 위해 업로드된 ClawPack 타르볼의 정확한 바이트를 스트리밍합니다.
...
호스팅된 skill 버전 ZIP을 다운로드하거나, `clean` 또는 `suspicious` 검사를 통과했고 호스팅된
...
- 허용되는 양식 필드는 `payload`, 반복되는 `files` blob 또는 하나의 `clawpack`
  타르볼 참조입니다. `clawpack`은 `.tgz` blob이거나 upload-url 흐름에서 반환된
  스토리지 ID일 수 있습니다. 스테이징된 storage-id 게시는 해당 업로드 URL과 함께 반환된
...
- JSON 본문과 호출자가 제공한 `payload.files` / `payload.artifact`
  메타데이터는 거부됩니다.
- 직접 multipart 게시 요청은 18MB로 제한됩니다. ClawPack 타르볼은
  upload-url 흐름을 사용하여 최대 120MB 타르볼 제한까지 사용할 수 있습니다.
- 선택적 페이로드 필드: `ownerHandle`. 있는 경우 관리자만 해당 소유자를 대신해 게시할 수 있습니다.
...
- 대리 게시는 여전히 대상 소유자 계정을 기준으로 공식 채널 자격을 검증합니다.
...
있는 경우 `reason`은 skill 조정 메모로 저장되고 감사 로그에 복사됩니다.
...
개인 게시자를 복구합니다. 요청은 변경 불가능한 두 GitHub 제공자 계정 ID를 모두 지정해야 하며,
...
게시자에 Skills, 패키지 또는 GitHub Skill 소스가 있으면 복구는 실패 시 닫힌 상태로 종료됩니다.
복구는 또한 복구된 게시자의 Skills, Skill 슬러그 별칭, 패키지, 패키지 검사기 경고, 파생 검색 다이제스트 행에 대한 기존 `ownerUserId` 필드를 마이그레이션하여
