...
x-i18n:
    generated_at: "2026-07-03T00:53:55Z"
    model: gpt-5.5
...
타사 디렉터리는 공개 읽기 엔드포인트를 사용해 ClawHub Skills를 나열하거나 검색할 수 있습니다. 결과를 캐시하고, `429`/`Retry-After`를 준수하며, 사용자를 표준 ClawHub 목록(`https://clawhub.ai/<owner>/skills/<slug>`)으로 다시 연결하고, 타사 사이트가 ClawHub의 보증을 받는다는 인상을 주지 마세요. 공개 API 표면 밖의 숨김, 비공개 또는 조정 차단된 콘텐츠를 미러링하려고 시도하지 마세요.

웹 슬러그 바로 가기는 레지스트리 계열 전체에서 해석되지만, API 클라이언트는 라우트
...
- 인증된 쓰기 엔드포인트는 서버가 이유를 알고 있을 때 단순한 `Unauthorized`를 반환해서는 안 됩니다. 토큰 누락, 유효하지 않거나 폐기된 토큰, 삭제/차단/비활성화된 계정은 각각 CLI
  클라이언트가 사용자를 차단한 원인을 알려줄 수 있도록 실행 가능한 텍스트를 받아야 합니다.
...
- 신뢰할 수 있는 클라이언트 IP를 사용할 수 없으면 익명 요청은 속도 제한 종류로만 범위가 지정된 대체 버킷을 사용합니다. 이러한 대체 버킷에는
...
- 관련성은 인기보다 더 강합니다. 정확한 슬러그 또는 표시 이름 토큰 일치는 참여도가 훨씬 더 높은 느슨한 일치보다 높은 순위를 가질 수 있습니다.
- ASCII 텍스트는 단어 및 구두점 경계에서 토큰화됩니다. 예를 들어 `personal-map`에는 독립형 `map` 토큰이 포함되는 반면, `amap-jsapi-skill`에는 `amap`, `jsapi`, `skill`이 포함됩니다. 따라서 `map`을 검색하면 `personal-map`이 `amap-jsapi-skill`보다 더 강한 어휘 일치를 얻습니다.
- 인기는 로그 스케일링되고 상한이 적용됩니다. 참여도가 높은 Skills도 쿼리 텍스트와의 일치가 약하면 더 낮은 순위를 가질 수 있습니다.
- 의심스럽거나 숨겨진 조정 상태는 호출자 필터와 현재 조정 상태에 따라 공개 검색에서 Skill을 제거할 수 있습니다.

게시자 검색 가능성 지침:

- 사용자가 실제로 검색할 용어를 표시 이름, 요약 및 태그에 넣으세요. 독립형 슬러그 토큰은 유지하려는 안정적인 ID이기도 한 경우에만 사용하세요.
- 새 슬러그가 더 나은 장기 표준 이름이 아닌 한, 단일 쿼리를 추적하려고 슬러그를 변경하지 마세요. 이전 슬러그는 리디렉션 별칭이 되지만, 표준 URL, 표시되는 슬러그 및 이후 검색 다이제스트는 새 슬러그를 사용합니다.
- 이름 변경 별칭은 이전 URL과 레지스트리를 통해 해석되는 설치에 대한 해석을 보존하지만, 검색 순위는 이름 변경이 색인된 후의 표준 Skill 메타데이터를 기반으로 합니다. 기존 통계는 Skill에 유지됩니다.
...
- `limit`(선택): 정수(1–200)
- `cursor`(선택): `trending`이 아닌 모든 정렬의 페이지 매김 커서
...
- `createdAt`은 새 Skill 크롤에 대해 안정적이며, `updated`는 기존 Skills가 다시 게시될 때 변경됩니다.
- `nonSuspiciousOnly=true`이면 의심스러운 Skills가 페이지 검색 후 필터링되므로 커서 기반 정렬은 한 페이지에서 `limit`보다 적은 항목을 반환할 수 있습니다.
...
- 소유자와 조정자는 숨겨진 Skills의 조정 세부정보에 접근할 수 있습니다.
- 공개 호출자는 이미 플래그 지정된 표시 Skills에 대해서만 `200`을 받습니다.
- 증거는 공개 호출자에게는 수정되어 제공되며, 원시 스니펫은 소유자/조정자에게만 포함됩니다.

### `POST /api/v1/skills/{slug}/report`

조정자 검토를 위해 Skill을 신고합니다. 신고는 Skill 수준이며, 선택적으로
버전에 연결되고 Skill 신고 큐에 전달됩니다.
...
설정할 때는 생략할 수 있습니다. 심사된
신고와 함께 `finalAction: "hide"`를 전달하면 동일한 감사 가능한 워크플로에서 Skill을 숨깁니다.
...
- `version.security`는 사용 가능한 경우 정규화된 스캔 확인 상태와 스캐너 세부정보
  (VirusTotal + LLM)를 포함합니다.

### `GET /api/v1/skills/{slug}/scan`

Skill 버전에 대한 보안 스캔 확인 세부정보를 반환합니다.
...
- 대기 중에는 `queue.queuedAhead`와 `queue.position`을 반환하므로 클라이언트가 요청보다 앞선 우선순위 수동 스캔 수를 표시할 수 있습니다. 매우 큰 큐는 상한이 적용되며 `queuedAheadIsEstimate: true`로 보고됩니다.
...
관리자 전용 표준 batch rescan 경로입니다. 레거시 `POST /api/v1/skills/-/rescan-batch`와 같은 페이로드 형태를 받습니다.

### `POST /api/v1/skills/-/scan/batch/status`

관리자 전용 표준 batch 상태 경로입니다. `{ "jobIds": ["..."] }`를 받고 레거시 `POST /api/v1/skills/-/rescan-batch/status`와 동일한 집계 카운터를 반환합니다.
...
- Skill 식별자, 게시자 식별자, 선택한 버전 메타데이터는 최상위 envelope 필드(`slug`, `displayName`, `publisherHandle`, `version`, `resolvedFrom`, `tag`, `createdAt`)이므로 셸 자동화가 중첩된 래퍼를 풀지 않고도 읽을 수 있습니다.
...
- `provenance`는 게시 또는 가져오기 중 ClawHub가 GitHub repo/ref/commit/path를 해석하고 저장한 경우에만 `server-resolved-github-import`이며, 그 외에는 `unavailable`입니다.
...
- `items`에는 고유한 `{ slug, version }` 쌍 1~100개가 포함되어야 합니다.
...
- `security.signals`에는 상태 수준의 보조 증거만 포함됩니다. 전체 스캐너 세부 정보는 `/scan` 또는 ClawHub security-audit 페이지를 사용하세요.
- `security.signals.dependencyRegistry`는 v1 응답 호환성을 위해 유지되지만, dependency registry 존재 스캐너는 폐기되었으며 이 키는 항상 `null`입니다.
- Skill Card 부재는 이 엔드포인트의 `ok`, `decision`, `reasons`에 영향을 주지 않습니다. 클라이언트는 card 콘텐츠가 필요할 때 설치된 `skill-card.md`를 로컬에서 읽어야 합니다.
- 단일 skill Skill Card 검증 envelope가 필요할 때는 `/verify`, 생성된 card markdown이 필요할 때는 `/card`, 상세 스캐너 데이터가 필요할 때는 `/scan`을 사용하세요.
...
- `category` (선택 사항): plugin 카테고리 필터. 요청 범위가 plugin 패키지(`/api/v1/plugins`,
  `/api/v1/code-plugins`, `/api/v1/bundle-plugins` 또는 `family=code-plugin`/`family=bundle-plugin`이 있는 패키지 엔드포인트)로 지정된 경우에만 지원됩니다. 제어되는 카테고리와
...
- 호스팅된 Skills는 저장된 최신 버전 파일을 포함하며
  `_manifest.json`에 `sourceRef: "public-clawhub"`로 나열됩니다.
- `clean` 또는 `suspicious` 스캔이 있는 현재 GitHub 기반 Skills는
  `_source_handoff.json`에 `sourceRef: "public-github"`, 리포지토리, 커밋, 경로,
  콘텐츠 해시, 아카이브 URL을 포함합니다. ClawHub 호스팅 소스 파일은 포함하지 않습니다.
...
- `version.sha256hash`는 이전 클라이언트를 위한 사용 중단된 호환성
  메타데이터입니다. `/api/v1/packages/{name}/download`가 반환하는 정확한
...
- `trust.scanStatus`는 스캐너 입력과 수동 릴리스 중재에서 파생된 유효 신뢰
  상태입니다.
- `trust.moderationState`는 null 가능 필드입니다. 수동 릴리스 중재가 없으면
  `null`입니다.
...
- `trust.reasons`는 사용자 표시 및 감사 설명 목록입니다. 사유 코드는
...
- 이 엔드포인트는 버전 정확합니다. 클라이언트는 최신 패키지 메타데이터를 읽은
  직후가 아니라, 설치하려는 패키지 버전을 해석한 후 호출해야 합니다.
- 비공개 패키지는 호출자가 소유 게시자를 읽을 수 없는 한 `404`를 반환합니다.
- 이 엔드포인트는 소유자/중재자 중재 엔드포인트보다 의도적으로 범위가 좁습니다.
  신고자 ID, 신고 본문, 비공개 증거 또는 내부 검토 일정이 아니라 설치 결정과
  공개 설명을 노출합니다.
...
연결됩니다. 신고는 중재 큐에 반영되지만 그 자체로 다운로드를 자동 숨김 처리하거나
차단하지는 않습니다. 중재자는 릴리스 중재를 사용해 아티팩트를 승인, 격리 또는
철회해야 합니다.
...
소유자/모더레이터용 패키지 모더레이션 가시성 엔드포인트입니다.
...
패키지 신고를 해결하거나 다시 여는 모더레이터/관리자용 엔드포인트입니다.
...
설정할 때는 생략할 수 있습니다. 동일한 감사 가능한 워크플로에서 릴리스 모더레이션을
...
패키지 릴리스 검토용 모더레이터/관리자 엔드포인트입니다.
...
- `dist.tarball`, `dist.integrity` 및 `dist.shasum`은 사용자가 원할 경우 npm이 미러를
...
버전이 없는 현재 GitHub 기반 스킬에 대해 GitHub 소스 핸드오프를 반환합니다.
...
- 선택적 페이로드 필드: `ownerHandle`. 이 값이 있으면 API는 서버 측에서 해당
  게시자를 확인하고 행위자에게 게시자 접근 권한을 요구합니다.
- 선택적 페이로드 필드: `migrateOwner`. `ownerHandle`과 함께 `true`이면 행위자가
  현재 및 대상 게시자 모두에서 관리자/소유자인 경우 기존 스킬을 해당 소유자로 이동할 수
  있습니다. 이 옵트인이 없으면 소유자 변경은 거부됩니다.
...
- `files` 또는 `clawpack` 중 하나만 사용하고, 같은 요청에서 둘을 함께 사용하지 마세요.
...
`reason`이 있으면 스킬 모더레이션 메모로 저장되고 감사 로그에 복사됩니다.
소유자가 시작한 소프트 삭제는 slug를 30일 동안 예약하며, 이후 다른 게시자가
해당 slug를 사용할 수 있습니다. 이 만료가 적용되면 삭제 응답에 `slugReservedUntil`이 포함됩니다.
...
개인 게시자를 복구합니다. 요청은 변경 불가능한 두 GitHub provider account id를 모두 지정해야 하며,
변경 가능한 핸들은 운영자 대면 가드로만 사용됩니다.

엔드포인트는 기본적으로 드라이런입니다. 복구를 적용하려면 직원이 두 GitHub 주체 간의 연속성을 독립적으로 확인한 뒤 `dryRun: false` 및
...
Skills 슬러그 별칭, 패키지, 패키지 검사기 경고 및 파생 검색 다이제스트 행의 기존 `ownerUserId` 필드를 마이그레이션하여
...
사용자를 차단하고 소유한 Skills를 영구 삭제합니다(중재자/관리자 전용).
...
차단을 해제하거나
콘텐츠를 복원하지 않고 기존 차단에 저장된 사유를 변경합니다(관리자 전용). `dryRun`이 `false`가 아니면 기본값은 드라이런입니다.
