...
x-i18n:
    generated_at: "2026-06-30T13:53:09Z"
    model: gpt-5.5
...
서드파티 디렉터리는 공개 읽기 엔드포인트를 사용하여 ClawHub Skills를 나열하거나 검색할 수 있습니다. 결과를 캐시하고, `429`/`Retry-After`를 준수하고, 사용자를 정식 ClawHub 목록(`https://clawhub.ai/<owner>/skills/<slug>`)으로 다시 연결하며, ClawHub가 해당 서드파티 사이트를 보증한다고 암시하지 마세요. 공개 API 표면 밖에서 숨김, 비공개 또는 조정 차단된 콘텐츠를 미러링하려고 시도하지 마세요.

웹 슬러그 단축 경로는 레지스트리 패밀리 전반에서 해석되지만, API 클라이언트는 라우트 우선순위를 재구성하는 대신 읽기 엔드포인트가 반환한 정식 URL을 사용해야 합니다.
...
- 인증된 쓰기 엔드포인트는 서버가 이유를 알고 있을 때 단순한 `Unauthorized`만 반환해서는 안 됩니다. 누락된 토큰, 유효하지 않거나 폐기된 토큰, 삭제/차단/비활성화된 계정에는 각각 실행 가능한 텍스트가 제공되어 CLI 클라이언트가 사용자에게 무엇이 차단했는지 알려줄 수 있어야 합니다.
...
- 관련성은 인기도보다 더 강합니다. 정확한 슬러그 또는 표시 이름 토큰 일치는 참여도가 훨씬 강한 느슨한 일치보다 더 높은 순위를 가질 수 있습니다.
- ASCII 텍스트는 단어 및 문장 부호 경계에서 토큰화됩니다. 예를 들어 `personal-map`에는 독립형 `map` 토큰이 포함되지만, `amap-jsapi-skill`에는 `amap`, `jsapi`, `skill`이 포함됩니다. 따라서 `map`을 검색하면 `amap-jsapi-skill`보다 `personal-map`에 더 강한 어휘 일치가 부여됩니다.
- 인기도는 로그 스케일로 조정되고 상한이 적용됩니다. 참여도가 높은 Skills도 쿼리 텍스트 일치가 더 약하면 더 낮은 순위를 가질 수 있습니다.
...
- 새 슬러그가 더 나은 장기 정식 이름이 아닌 한, 단일 쿼리를 따라잡기 위해 슬러그 이름을 바꾸지 마세요. 이전 슬러그는 리디렉션 별칭이 되지만, 정식 URL, 표시되는 슬러그, 향후 검색 다이제스트는 새 슬러그를 사용합니다.
- 이름 변경 별칭은 레지스트리를 통해 해석되는 이전 URL 및 설치의 해석을 보존하지만, 검색 순위는 이름 변경이 인덱싱된 후의 정식 Skill 메타데이터를 기반으로 합니다. 기존 통계는 Skill에 그대로 유지됩니다.
...
- `nonSuspiciousOnly=true`인 경우, 페이지 검색 후 의심스러운 Skills가 필터링되므로 커서 기반 정렬은 한 페이지에서 `limit`보다 적은 항목을 반환할 수 있습니다.
- 존재하는 경우 `nextCursor`를 사용하여 페이지네이션을 계속하세요. 짧은 페이지 자체가 결과 끝을 의미하지는 않습니다.
...
- 소유자와 조정자는 숨겨진 Skills의 조정 세부 정보에 액세스할 수 있습니다.
- 공개 호출자는 이미 플래그된 표시 가능한 Skills에 대해서만 `200`을 받습니다.
- 증거는 공개 호출자에게 수정되어 제공되며, 원시 스니펫은 소유자/조정자에게만 포함됩니다.
...
Skill 신고를 해결하거나 다시 여는 조정자/관리자 엔드포인트입니다.
...
- 과거 버전을 쿼리할 때는 `moderation`과 `security`를 같은 버전 컨텍스트로 취급하기 전에 `moderation.matchesRequestedVersion`와 `moderation.sourceVersion`을 확인하세요.
...
- 대기 중에는 클라이언트가 요청보다 앞선 우선순위 수동 스캔 수를 표시할 수 있도록 `queue.queuedAhead`와 `queue.position`을 반환합니다. 매우 큰 대기열은 제한되며 `queuedAheadIsEstimate: true`로 보고됩니다.
...
제출된 버전에 대해 인증된 저장 보고서 아카이브 엔드포인트입니다.
...
- scan-request 다운로드와 동일한 ZIP 구조를 반환합니다.

### `POST /api/v1/skills/-/scan/batch`

관리자 전용 표준 일괄 재스캔 라우트입니다. 레거시 `POST /api/v1/skills/-/rescan-batch`와 동일한 페이로드 구조를 허용합니다.

### `POST /api/v1/skills/-/scan/batch/status`

관리자 전용 표준 일괄 상태 라우트입니다. `{ "jobIds": ["..."] }`를 허용하며 레거시 `POST /api/v1/skills/-/rescan-batch/status`와 동일한 집계 카운터를 반환합니다.
...
- `ok`는 선택된 버전에 생성된 Skill Card가 있고, 조정에 의해 악성코드로 차단되지 않았으며, ClawScan 검증이 clean인 경우에만 `true`입니다.
- 셸 자동화가 중첩된 래퍼를 풀지 않고 읽을 수 있도록 Skill ID, 게시자 ID, 선택한 버전 메타데이터는 최상위 엔벨로프 필드(`slug`, `displayName`, `publisherHandle`, `version`, `resolvedFrom`, `tag`, `createdAt`)입니다.
...
- `provenance`는 ClawHub가 게시 또는 가져오기 중에 GitHub 저장소/ref/commit/path를 확인하고 저장한 경우에만 `server-resolved-github-import`입니다. 그 외에는 `unavailable`입니다.

### `POST /api/v1/skills/-/security-verdicts`

정확한 skill 버전에 대한 현재의 간결한 보안 판정을 반환합니다. 이 컬렉션 엔드포인트는 OpenClaw Control UI처럼 표시해야 하는 설치된 ClawHub skill 버전을 이미 알고 있는 클라이언트를 대상으로 합니다.
...
- 결과는 항목별입니다. 하나의 skill 또는 버전이 누락되어도 전체 응답은 실패하지 않습니다.
...
- `security.signals.dependencyRegistry`는 v1 응답 호환성을 위해 유지되지만, 종속성 레지스트리 존재 스캐너는 폐기되었으며 이 키는 항상 `null`입니다.
- Skill Card가 없어도 이 엔드포인트의 `ok`, `decision` 또는 `reasons`에는 영향을 주지 않습니다. 클라이언트는 카드 콘텐츠가 필요할 때 설치된 `skill-card.md`를 로컬에서 읽어야 합니다.
- 단일 skill Skill Card 검증 엔벨로프가 필요하면 `/verify`를, 생성된 카드 Markdown이 필요하면 `/card`를, 상세 스캐너 데이터가 필요하면 `/scan`을 사용하세요.
...
- `category` (선택 사항): plugin 카테고리 필터. 요청 범위가 plugin 패키지(`/api/v1/plugins`,
  ...
  `family=code-plugin`/`family=bundle-plugin`이 포함된 패키지 엔드포인트)로
  한정된 경우에만 지원됩니다. 관리되는 카테고리와 레거시 v1 필터 별칭은
...
- Skills 항목은 계속 Skills 레지스트리를 기반으로 하며 `POST /api/v1/skills`를 통해서만 게시할 수 있습니다.
...
- `channel` (선택 사항): `official`, `community` 또는 `private`
- `isOfficial` (선택 사항): `true` 또는 `false`
- `category` (선택 사항): plugin 카테고리 필터. 요청 범위가 plugin 패키지로 한정된 경우에만 지원됩니다. 관리되는 카테고리와 레거시 v1
...
code-plugin 및 bundle-plugin 패키지를 대상으로 하는 Plugin 전용 카탈로그 탐색입니다.
...
- `clean` 또는 `suspicious` 스캔이 있는 현재 GitHub 기반 Skills에는
  `_source_handoff.json`이 포함되며, 여기에는 `sourceRef: "public-github"`, repo, commit, path,
  content hash, archive URL이 들어 있습니다. ClawHub 호스팅 소스 파일은 포함하지 않습니다.
...
- 패키지 소유자, 조직 게시자 소유자/관리자, 플랫폼 모더레이터 또는 플랫폼 관리자용 API 토큰이 필요합니다.
...
- `version.sha256hash`는 기존 클라이언트를 위한 더 이상 권장되지 않는 호환성 메타데이터입니다.
...
- `trust.moderationState`는 null일 수 있습니다. 수동 릴리스 모더레이션이 없으면 `null`입니다.
- `trust.blockedFromDownload`는 설치 차단 신호입니다. OpenClaw 및 다른 설치 클라이언트는
  스캐너 또는 모더레이션 필드에서 차단 규칙을 다시 파생하는 대신 이 값이 `true`일 때
  설치를 차단해야 합니다.
...
- 이는 OpenClaw 리졸버 표면입니다. 공유 URL에서 아카이브 형식을 추측하지 않도록 합니다.
...
버전에 연결됩니다. 신고는 모더레이션 대기열에 반영되지만 자체적으로 다운로드를
자동 숨김 처리하거나 차단하지 않습니다. 모더레이터는 릴리스 모더레이션을 사용하여
...
패키지 모더레이션 표시 여부를 위한 소유자/모더레이터 엔드포인트.
...
패키지 신고를 해결하거나 다시 여는 모더레이터/관리자 엔드포인트.
...
패키지 릴리스 검토를 위한 모더레이터/관리자 엔드포인트.
...
`suspicious` 스캔을 받은 현재 GitHub 기반 skill에 대해 GitHub 소스 인계를 반환합니다.
...
- 선택적 페이로드 필드: `migrateOwner`. `ownerHandle`과 함께 `true`이면,
  행위자가 현재 및 대상 게시자 모두의 관리자/소유자인 경우 기존 skill을 해당 소유자로
  이동할 수 있습니다. 이 옵트인이 없으면 소유자 변경은 거부됩니다.
...
- 허용되는 폼 필드는 `payload`, 반복되는 `files` blob 또는 하나의 `clawpack`
  ...
  스토리지 ID일 수 있습니다. 스테이징된 스토리지 ID 게시에는 해당 업로드 URL과 함께 반환된
...
관리자 전용. 핸들에 대한 조직 게시자가 존재하도록 보장합니다. 핸들이 여전히
레거시 공유 사용자/개인 게시자를 가리키는 경우, 엔드포인트가 먼저 이를 조직 게시자로 마이그레이션합니다.
새로 생성된 조직에는 `memberHandle`을 제공하세요. 작업 중인 관리자는 멤버로 추가되지 않습니다.
...
인증된 셀프 서비스 조직 게시자 생성. 새 조직 게시자를 생성하고
...
관리자 전용. 릴리스를 게시하지 않고 정당한 소유자를 위해 루트 slug와 패키지 이름을 예약합니다.
...
관리자 전용. Convex Auth 계정 행을 편집하지 않고 검증된 대체 GitHub OAuth 주체에 대해
개인 게시자를 복구합니다. 요청은 변경 불가능한 두 GitHub 제공자 계정 ID를 모두 명명해야 합니다.
변경 가능한 핸들은 운영자 대상 가드로만 사용됩니다.

엔드포인트는 기본적으로 시험 실행으로 동작합니다. 복구를 적용하려면 직원이 두 GitHub 주체 간의 연속성을 독립적으로 확인한 후 `dryRun: false`와
`confirmIdentityVerified: true`가 필요합니다. 대상 사용자의 현재 개인
publisher에 skills, packages 또는 GitHub skill sources가 있으면 복구는 실패로 종료됩니다.
복구는 복구된 publisher의 skills, skill slug aliases, packages, package inspector warnings, 파생된 search digest rows의 레거시 `ownerUserId` 필드도 마이그레이션하여
직접 소유자 경로가 새 publisher 권한과 일치하도록 합니다. 복구된 handle에 대한 활성 protected-handle
예약도 대체 사용자에게 재할당되어, 이후 profile synchronization이 이전 사용자의 경쟁 권한을 복원할 수 없게 합니다. 각 기본 테이블은 적용 트랜잭션당
100개 행으로 제한되며, 더 큰 복구는 먼저 재개 가능한 owner migration을 사용해야 합니다.
GitHub skill sources는 publisher 범위이며 다시 작성되지 않고 확인된 것으로 보고됩니다.
...
- `merge`는 source 목록을 숨기고 source slug를 target 목록으로 리디렉션합니다.
...
사용자를 차단하고 소유한 skills를 하드 삭제합니다(moderator/admin만 가능).
...
사용자 차단을 해제하고 적격한 skills를 복원합니다(admin만 가능).
...
(admin만 가능). `dryRun`이 `false`가 아니면 기본적으로 시험 실행으로 동작합니다.
...
`POST /api/cli/upload-url`는 `uploadUrl`과 `uploadTicket`을 반환합니다. ClawPack tarball을 스테이징하는 package
publish는 결과 storage id를 `clawpack`으로, 반환된 ticket을 `clawpackUploadTicket`으로 보내야 합니다.
...
self-host하는 경우 이 파일을 제공하세요(또는 `CLAWHUB_REGISTRY`를 명시적으로 설정하세요. 레거시 `CLAWDHUB_REGISTRY`).
