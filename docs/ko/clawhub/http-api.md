...
x-i18n:
    generated_at: "2026-07-03T17:15:21Z"
    model: gpt-5.5
...
서드 파티 디렉터리는 공개 읽기 엔드포인트를 사용하여 ClawHub Skills를 나열하거나 검색할 수 있습니다. 결과를 캐시하고, `429`/`Retry-After`를 준수하며, 사용자를 표준 ClawHub 목록(`https://clawhub.ai/<owner>/skills/<slug>`)으로 다시 연결하고, 서드 파티 사이트가 ClawHub의 보증을 받는다는 인상을 주지 마세요. 공개 API 표면 밖의 숨김, 비공개 또는 모더레이션으로 차단된 콘텐츠를 미러링하려고 시도하지 마세요.

웹 슬러그 바로가기는 레지스트리 패밀리 전반에서 해석되지만, API 클라이언트는 라우트
...
- 인증된 쓰기 엔드포인트는 서버가 이유를 알고 있을 때 단순한 `Unauthorized`를 반환해서는 안 됩니다. 누락된 토큰, 유효하지 않거나 폐기된 토큰, 삭제/차단/비활성화된 계정에는 각각 CLI
  클라이언트가 사용자에게 무엇이 차단했는지 알려줄 수 있도록 실행 가능한 텍스트를 제공해야 합니다.
...
- 관련성은 인기도보다 더 강합니다. 정확한 슬러그 또는 표시 이름 토큰 일치는 참여도가 훨씬 높은 느슨한 일치보다 높은 순위를 차지할 수 있습니다.
...
- 인기도는 로그 스케일이 적용되고 상한이 있습니다. 참여도가 높은 Skills도 쿼리 텍스트 일치가 약하면 더 낮은 순위가 될 수 있습니다.
- 의심스럽거나 숨겨진 모더레이션 상태는 호출자 필터와 현재 모더레이션 상태에 따라 공개 검색에서 Skill을 제거할 수 있습니다.

게시자 검색 가능성 지침:

- 사용자가 문자 그대로 검색할 용어를 표시 이름, 요약, 태그에 넣으세요. 독립형 슬러그 토큰은 유지하려는 안정적인 ID이기도 할 때만 사용하세요.
- 새 슬러그가 더 나은 장기 표준 이름이 아닌 한, 하나의 쿼리를 좇기 위해 슬러그 이름을 바꾸지 마세요. 이전 슬러그는 리디렉션 별칭이 되지만, 표준 URL, 표시되는 슬러그, 향후 검색 다이제스트는 새 슬러그를 사용합니다.
- 이름 변경 별칭은 레지스트리를 통해 해석되는 이전 URL 및 설치의 해석을 보존하지만, 검색 순위는 이름 변경이 인덱싱된 후 표준 Skill 메타데이터를 기반으로 합니다. 기존 통계는 Skill에 유지됩니다.
...
- `createdAt`은 새 Skill 크롤링에 대해 안정적입니다. `updated`는 기존 Skills가 다시 게시될 때 변경됩니다.
- `nonSuspiciousOnly=true`인 경우 의심스러운 Skills가 페이지 검색 후 필터링되기 때문에 커서 기반 정렬은 한 페이지에서 `limit`보다 적은 항목을 반환할 수 있습니다.
...
- 스킬 또는 Plugin에 대한 소유자/게시자 관리 액세스 권한 또는 플랫폼 moderator/admin 권한이 필요합니다.
...
관리자 전용 표준 배치 재스캔 라우트입니다. 기존 `POST /api/v1/skills/-/rescan-batch`와 동일한 페이로드 구조를 허용합니다.

### `POST /api/v1/skills/-/scan/batch/status`

관리자 전용 표준 배치 상태 라우트입니다. `{ "jobIds": ["..."] }`를 허용하며 기존 `POST /api/v1/skills/-/rescan-batch/status`와 동일한 집계 카운터를 반환합니다.
...
- 선택한 버전에 생성된 Skill Card가 있고, 검토에 의해 악성 코드로 차단되지 않았으며, ClawScan 검증이 clean인 경우에만 `ok`가 `true`입니다.
- 스킬 ID, 게시자 ID, 선택된 버전 메타데이터는 최상위 envelope 필드(`slug`, `displayName`, `publisherHandle`, `version`, `resolvedFrom`, `tag`, `createdAt`)이므로 셸 자동화가 중첩 wrapper를 풀지 않고 읽을 수 있습니다.
...
- `provenance`는 ClawHub가 게시 또는 가져오기 중에 GitHub 저장소/ref/commit/path를 확인하고 저장한 경우에만 `server-resolved-github-import`입니다. 그렇지 않으면 `unavailable`입니다.

### `POST /api/v1/skills/-/security-verdicts`

정확한 스킬 버전에 대한 현재 compact 보안 판정을 반환합니다. 이 컬렉션 엔드포인트는 OpenClaw Control UI처럼 표시해야 하는 설치된 ClawHub 스킬 버전을 이미 알고 있는 클라이언트를 위한 것입니다.
...
- 결과는 항목별로 제공됩니다. 하나의 스킬 또는 버전이 누락되어도 전체 응답이 실패하지 않습니다.
...
- `security.signals.dependencyRegistry`는 v1 응답 호환성을 위해 유지되지만, 의존성 레지스트리 존재 스캐너는 폐기되었으며 이 키는 항상 `null`입니다.
- Skill Card 부재는 이 엔드포인트의 `ok`, `decision`, `reasons`에 영향을 주지 않습니다. 카드 콘텐츠가 필요할 때 클라이언트는 설치된 `skill-card.md`를 로컬에서 읽어야 합니다.
- 단일 스킬 Skill Card 검증 envelope이 필요하면 `/verify`를, 생성된 카드 Markdown이 필요하면 `/card`를, 자세한 스캐너 데이터가 필요하면 `/scan`을 사용하세요.
...
- `startDate` (필수): Skill `updatedAt`의 하한값(Unix 밀리초).
- `endDate` (필수): Skill `updatedAt`의 상한값(Unix 밀리초).
...
- 결과는 관련도 순서로 반환되며 현재 페이지네이션하지 않습니다.
...
- `release.releaseId`, `release.version`, `release.createdAt`은 평가된
  정확한 릴리스를 식별합니다.
- 릴리스 아티팩트에 대해 알려진 경우 `release.artifactKind`,
  `release.artifactSha256`, `release.npmIntegrity`, `release.npmShasum`,
  `release.npmTarballName`이 존재합니다.
- `trust.scanStatus`는 스캐너 입력 및 수동 릴리스 조정에서 파생된 유효 신뢰 상태입니다.
- `trust.moderationState`는 null일 수 있습니다. 수동 릴리스 조정이 없으면
  `null`입니다.
- `trust.blockedFromDownload`는 설치 차단 신호입니다. OpenClaw 및 기타
  설치 클라이언트는 스캐너 또는 조정 필드에서 차단 규칙을 다시 도출하는 대신
...
- 이 엔드포인트는 버전에 정확히 대응합니다. 클라이언트는 단순히 최신
  패키지 메타데이터를 읽은 뒤가 아니라 설치하려는 패키지 버전을 해석한 뒤에 이를 호출해야 합니다.
- 비공개 패키지는 호출자가 소유 게시자를 읽을 수 없는 한 `404`를 반환합니다.
- 이 엔드포인트는 의도적으로 소유자/조정자 조정 엔드포인트보다 범위가 좁습니다.
...
- 이는 OpenClaw 해석기 표면입니다. 공유 URL에서 아카이브 형식을 추측하지 않습니다.
...
패키지 릴리스 검토 대기열을 위한 조정자/관리자 엔드포인트입니다.
...
조정자 검토를 위해 패키지를 신고합니다. 신고는 패키지 수준이며 선택적으로
버전에 연결됩니다. 이는 조정 대기열에 반영되지만 자체적으로 다운로드를 자동 숨김하거나
...
`finalAction: "revoke"`를 전달하면 동일한 감사 가능한 워크플로에서 릴리스 중재를 적용할 수 있습니다.
...
- 파일 크기 제한: 200KB.
- 대기 중인 VirusTotal 스캔은 읽기를 차단하지 않으며, 악성 릴리스는 다른 곳에서 계속 보류될 수 있습니다.
...
- 대기 중인 VirusTotal 스캔은 다운로드를 차단하지 않으며, 악성 릴리스는 `403`을 반환합니다.
...
- `dist.tarball`, `dist.integrity` 및 `dist.shasum`은 사용자가 원하면 npm이 미러를 가리키도록
  npm 호환 필드를 사용합니다.
...
- 다운로드 통계는 UTC 일자별 고유 ID로 집계됩니다(API 토큰이 유효하면 `userId`, 아니면 IP).
...
- 허용되는 폼 필드는 `payload`, 반복되는 `files` blob 또는 하나의 `clawpack`
  tarball 참조입니다. `clawpack`은 `.tgz` blob이거나 upload-url 흐름에서 반환된 스토리지 ID일 수 있습니다.
...
- 직접 multipart 게시 요청은 18MB로 제한됩니다. ClawPack tarball은
  upload-url 흐름을 사용하여 120MB tarball 제한까지 사용할 수 있습니다.
...
있으면 `reason`이 Skill 중재 메모로 저장되고 감사 로그에 복사됩니다.
소유자가 시작한 소프트 삭제는 slug를 30일 동안 예약하며, 이후 다른 게시자가
slug를 사용할 수 있습니다. 이 만료가 적용되는 경우 삭제 응답에는 `slugReservedUntil`이 포함됩니다.
...
새로 생성된 조직의 경우 `memberHandle`을 제공하세요. 실행 관리자는 구성원으로 추가되지 않습니다.
...
패키지 이름은 릴리스 행이 없는 비공개 자리 표시자 패키지가 되므로, 동일한
...
제공자 계정 ID를 모두 지정해야 하며, 변경 가능한 핸들은 운영자 대상 가드로만 사용됩니다.

엔드포인트는 기본적으로 드라이런입니다. 복구를 적용하려면 직원이 두 GitHub 주체 간 연속성을 독립적으로 확인한 뒤 `dryRun: false` 및
`confirmIdentityVerified: true`가 필요합니다. 대상 사용자의 현재 개인
게시자에 skills, packages 또는 GitHub skill 소스가 있으면 복구는 실패 시 닫힌 상태가 됩니다.
복구는 또한 복구된 게시자의 skills, skill slug 별칭, packages, package inspector 경고, 파생 검색 digest 행의 기존 `ownerUserId` 필드를 마이그레이션하여
...
- `merge`는 원본 목록을 숨기고 원본 slug를 대상 목록으로 리디렉션합니다.
...
게시 작업은 결과 storage ID를 `clawpack`으로, 반환된 티켓을 `clawpackUploadTicket`으로 보내야 합니다.
