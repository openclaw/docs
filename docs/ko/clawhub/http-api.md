...
x-i18n:
    generated_at: "2026-07-02T00:46:34Z"
    model: gpt-5.5
...
타사 디렉터리는 공개 읽기 엔드포인트를 사용해 ClawHub Skills를 나열하거나 검색할 수 있습니다. 결과를 캐시하고, `429`/`Retry-After`를 준수하며, 사용자를 정식 ClawHub 목록(`https://clawhub.ai/<owner>/skills/<slug>`)으로 다시 연결하고, 타사 사이트가 ClawHub의 보증을 받는다는 인상을 주지 마세요. 공개 API 표면 밖의 숨김, 비공개 또는 검토 차단된 콘텐츠를 미러링하려고 시도하지 마세요.
...
- token이 없거나 유효하지 않으면 동작은 IP 적용으로 폴백됩니다.
- 인증된 쓰기 엔드포인트는 서버가 이유를 알고 있을 때 단순한 `Unauthorized`만 반환해서는 안 됩니다. token 누락, 유효하지 않거나 폐기된 token, 삭제/차단/비활성화된 계정은 각각 CLI 클라이언트가 사용자에게 무엇이 막았는지 알려줄 수 있도록 실행 가능한 텍스트를 받아야 합니다.

- 읽기: IP당 3000/min, key당 12000/min
- 쓰기: IP당 300/min, key당 3000/min
- 다운로드: IP당 1200/min, key당 6000/min(다운로드 엔드포인트)
...
- 결과는 관련성 순서로 반환됩니다(임베딩 유사도 + 정확한 slug/name token 부스트 + 작은 인기 사전값).
- 관련성은 인기보다 더 강합니다. 정확한 slug 또는 표시 이름 token 일치는 참여도가 훨씬 강한 더 느슨한 일치보다 높은 순위를 차지할 수 있습니다.
- ASCII 텍스트는 단어 및 문장부호 경계에서 토큰화됩니다. 예를 들어 `personal-map`에는 독립형 `map` token이 포함되는 반면, `amap-jsapi-skill`에는 `amap`, `jsapi`, `skill`이 포함됩니다. 따라서 `map`을 검색하면 `amap-jsapi-skill`보다 `personal-map`에 더 강한 어휘 일치를 부여합니다.
- 인기는 로그 스케일로 조정되고 상한이 적용됩니다. 참여도가 높은 Skills라도 쿼리 텍스트 일치가 더 약하면 낮은 순위를 차지할 수 있습니다.
- 의심스럽거나 숨겨진 검토 상태는 호출자 필터와 현재 검토 상태에 따라 공개 검색에서 Skill을 제거할 수 있습니다.

게시자 검색 가능성 지침:

- 사용자가 실제로 검색할 용어를 표시 이름, 요약, 태그에 넣으세요. 독립형 slug token은 유지하려는 안정적인 정체성이기도 할 때만 사용하세요.
- 새 slug가 더 나은 장기 정식 이름이 아닌 한, 단일 쿼리를 쫓기 위해 slug를 변경하지 마세요. 이전 slug는 리디렉션 별칭이 되지만, 정식 URL, 표시되는 slug, 향후 검색 다이제스트는 새 slug를 사용합니다.
- 이름 변경 별칭은 이전 URL과 레지스트리를 통해 해석되는 설치의 해석을 보존하지만, 검색 순위는 이름 변경이 인덱싱된 후 정식 Skill 메타데이터를 기반으로 합니다. 기존 통계는 Skill에 그대로 유지됩니다.
- Skill이 예상치 못하게 보이지 않는 경우, 순위 관련 메타데이터를 변경하기 전에 로그인한 상태에서 `clawhub inspect @owner/slug`로 먼저 검토 상태를 확인하세요.
...
- `trending`은 최근 7일간 설치 수(telemetry 기반)로 순위를 매깁니다.
...
- `moderation`은 Skill에 플래그가 지정되었거나 소유자가 보는 경우에만 포함됩니다.
...
- 증거는 공개 호출자에게는 수정 처리되며, 소유자/검토자에게만 원시 스니펫을 포함합니다.

### `POST /api/v1/skills/{slug}/report`

검토자 검토를 위해 Skill을 신고합니다. 신고는 Skill 수준이며, 선택적으로 버전에 연결되고 Skill 신고 큐로 전달됩니다.
...
Skill 신고 접수를 위한 검토자/admin 엔드포인트입니다.
...
Skill 신고를 해결하거나 다시 여는 검토자/admin 엔드포인트입니다.
...
`note`는 `confirmed` 및 `dismissed`에 필요합니다. `status`를 다시 `open`으로 설정할 때는 생략할 수 있습니다. 감사 가능한 동일한 워크플로에서 Skill을 숨기려면 분류된 신고와 함께 `finalAction: "hide"`를 전달하세요.
...
- 게시된 스캔은 `update: true`이고 스캔이 성공적으로 완료된 경우에만 다시 씁니다.
...
- 대기 중에는 `queue.queuedAhead`와 `queue.position`을 반환하여 클라이언트가 해당 요청보다 앞선 우선순위 수동 스캔 수를 표시할 수 있게 합니다. 매우 큰 큐는 제한되어 `queuedAheadIsEstimate: true`로 보고됩니다.
...
관리자 전용 표준 일괄 재스캔 경로입니다. 레거시 `POST /api/v1/skills/-/rescan-batch`와 동일한 페이로드 형태를 받습니다.

### `POST /api/v1/skills/-/scan/batch/status`

관리자 전용 표준 일괄 상태 경로입니다. `{ "jobIds": ["..."] }`를 받고 레거시 `POST /api/v1/skills/-/rescan-batch/status`와 동일한 집계 카운터를 반환합니다.
...
- `version`(선택 사항): 특정 버전 문자열.
- `tag`(선택 사항): 태그가 지정된 버전(예: `latest`)을 확인합니다.

참고:

- `ok`는 선택된 버전에 생성된 Skill Card가 있고, 모더레이션에 의해 악성 코드로 차단되지 않았으며, ClawScan 검증이 깨끗한 경우에만 `true`입니다.
...
- `provenance`는 ClawHub가 게시 또는 가져오기 중 GitHub repo/ref/commit/path를 확인하고 저장한 경우에만 `server-resolved-github-import`이며, 그 외에는 `unavailable`입니다.

### `POST /api/v1/skills/-/security-verdicts`

정확한 skill 버전에 대한 현재 압축 보안 판정을 반환합니다. 이 컬렉션 엔드포인트는 OpenClaw Control UI처럼 표시해야 할 설치된 ClawHub skill 버전을 이미 알고 있는 클라이언트를 위한 것입니다.
...
- 응답은 보안 전용입니다. Skill Card 데이터, 생성된 카드 상태, 아티팩트 파일 목록 또는 상세 스캐너 페이로드를 포함하지 않습니다.
...
- `category` (선택 사항): Plugin 카테고리 필터. 요청 범위가 Plugin 패키지(`/api/v1/plugins`,
  ...
  `family=code-plugin`/`family=bundle-plugin`이 있는 패키지 엔드포인트)로 한정된 경우에만
...
- `channel` (선택 사항): `official`, `community` 또는 `private`
- `isOfficial` (선택 사항): `true` 또는 `false`
- `category` (선택 사항): Plugin 카테고리 필터. 요청 범위가 Plugin 패키지로 한정된 경우에만
...
- 본문: ZIP 아카이브입니다.
- 내보낸 각 Plugin은 `{family}/{packageName}/`를 루트로 합니다.
...
- `package.name`, `package.displayName`, 및 `package.family`는 확인된
  레지스트리 패키지를 식별합니다.
...
- 이 엔드포인트는 버전 정확성을 가집니다. 클라이언트는 최신 패키지 메타데이터를 읽은
  직후가 아니라, 설치하려는 패키지 버전을 확인한 뒤 호출해야 합니다.
- 비공개 패키지는 호출자가 소유 게시자를 읽을 수 없는 한 `404`를 반환합니다.
...
- `open`: 의심스럽거나, 악성, 대기 중, 격리됨, 폐기됨 또는 신고된 릴리스입니다.
...
연결됩니다. 이는 모더레이션 큐에 공급되지만 자체적으로 다운로드를 자동 숨김 처리하거나
차단하지 않습니다. 모더레이터는 릴리스 모더레이션을 사용하여 아티팩트를 승인, 격리 또는
폐기해야 합니다.
...
- 선택적 페이로드 필드: `migrateOwner`. `ownerHandle`과 함께 `true`이면,
  행위자가 현재 및 대상 게시자 양쪽에서 관리자/소유자인 경우 기존 skill을 해당 소유자로
...
해당 slug를 차지할 수 있습니다. 이 만료가 적용되는 경우 삭제 응답에는 `slugReservedUntil`이 포함됩니다.
중재자/관리자 숨김 및 보안 제거에는 이러한 방식의 만료가 적용되지 않습니다.
...
새로 생성된 조직의 경우 `memberHandle`을 제공합니다. 실행 중인 관리자는 구성원으로 추가되지 않습니다.
...
- 핸들이 이미 게시자, 사용자 또는 개인 게시자가 사용 중인 경우 `409`를 반환합니다.
...
제공자 계정 ID 둘 다를 지정해야 합니다. 변경 가능한 핸들은 운영자 대상 보호 장치로만 사용됩니다.
...
복구는 복구된 게시자의 스킬, 스킬 slug 별칭, 패키지, 패키지 검사기 경고, 파생 검색 다이제스트 행에 있는 기존 `ownerUserId` 필드도 마이그레이션하여
...
사용자를 차단하고 소유한 스킬을 영구 삭제합니다(중재자/관리자 전용).
...
`dryRun`이 `false`가 아니면 기본값은 드라이런입니다.
