...
실제 사용자 영향의 예에는 확인된 악용, 사용자 데이터 또는 비밀 정보 노출, 플랫폼 장애로 인해 악성 콘텐츠가 사용자에게 도달한 경우, 또는 사용자가 자격 증명을 교체하거나 로컬 소프트웨어를 업데이트하거나 기타 보호 조치를 취해야 하는 모든 문제가 포함됩니다.

사용자가 설치한 소프트웨어의 취약점은 공개됩니다. 예를 들어 사용자가 로컬에서 업데이트해야 하는 ClawHub CLI 패키지, 바이너리, 라이브러리 또는 기타 릴리스 아티팩트가 해당됩니다.

## 관련 페이지

설치 시 감사 라벨, 위험 수준, 발견 사항 및 해석은 [보안 감사](/ko/clawhub/security-audits)를 참조하세요.

마켓플레이스 보고, 조정 보류, 숨겨진 목록, 차단 및 계정 상태는 [조정 및 계정 안전](/ko/clawhub/moderation)을 참조하세요.
