...
title: 설치
x-i18n:
    generated_at: "2026-07-12T00:54:45Z"
    model: gpt-5.6
...
<Note>
Windows 데스크톱 사용자는 설정, 트레이 상태, 채팅, Node 모드 및 로컬 MCP 모드를 포함하는 네이티브 [Windows Hub](/ko/platforms/windows#recommended-windows-hub) 컴패니언 앱도 설치할 수 있습니다.
</Note>
...
`~/.openclaw` 같은 로컬 접두사 아래에 유지하려면 이 방법을 사용하세요.
...
이미 설치했나요? `openclaw update --channel dev`와
`openclaw update --channel stable`을 사용하여 패키지 설치와 git 설치 간에 전환할 수 있습니다.
[업데이트](/ko/install/updating#switch-between-npm-and-git-installs)를 참조하세요.
...
<Tabs>
  <Tab title="npm">
    ...
    <Note>
    호스팅된 설치 프로그램은 OpenClaw 패키지를 설치할 때 `min-release-age`와
    같은 npm 최신성 필터를 해제합니다. npm으로 수동 설치하는 경우에는
    사용자의 npm 정책이 그대로 적용됩니다.
    </Note>
  ...
  <Tab title="pnpm">
    ...
    <Note>
    pnpm에서는 빌드 스크립트가 포함된 패키지를 명시적으로 승인해야 합니다. 최초 설치 후 `pnpm approve-builds -g`를 실행하세요.
    </Note>
...
또는 링크를 생략하고 저장소 내부에서 `pnpm openclaw ...`을 사용하세요. 전체 개발 워크플로는 [설정](/ko/start/setup)을 참조하세요.
...
- 네이티브 Windows: 우선 예약된 작업을 사용하고, 작업 생성이 거부되면 사용자별 시작 폴더 로그인 항목으로 대체

## 호스팅 및 배포

클라우드 서버 또는 VPS에 OpenClaw를 배포하세요. 전체 제공업체 선택 목록(DigitalOcean, Hetzner, Hostinger, Fly.io, GCP, Azure, Railway,
Northflank, Oracle Cloud, Raspberry Pi 등)은 [Linux 서버](/ko/vps)를 참조하거나,
[Render](/ko/install/render)에 선언적으로 배포하세요.
...
거의 항상 PATH 문제입니다. npm의 전역 바이너리 디렉터리가 셸의 `PATH`에 포함되어 있지 않은 것입니다. Windows 경로를 포함한 전체 해결 방법은 [Node.js 문제 해결](/ko/install/node#troubleshooting)을 참조하세요.

```bash
node -v           # Node가 설치되어 있나요?
npm prefix -g     # 전역 패키지는 어디에 있나요?
echo "$PATH"      # 전역 바이너리 디렉터리가 PATH에 있나요?
```
