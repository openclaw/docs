...
- **Node 22.19+, 23.11+ 또는 24+** - Node 24가 기본 대상이며, 설치 프로그램 스크립트가 이를 자동으로 처리합니다.
...
가장 빠른 설치 방법입니다. 운영 체제를 감지하고, 필요한 경우 Node를 설치하며, OpenClaw를 설치한 후 온보딩을 시작합니다.

<Note>
Windows 데스크톱 사용자는 설정, 트레이 상태, 채팅, 노드 모드 및 로컬 MCP 모드가 포함된 네이티브 [Windows Hub](/ko/platforms/windows#recommended-windows-hub) 컴패니언 앱도 설치할 수 있습니다.
</Note>
...
온보딩을 실행하지 않고 설치하려면 다음과 같이 하십시오.
...
모든 플래그와 CI/자동화 옵션은 [설치 프로그램 내부 구조](/ko/install/installer)를 참조하십시오.
...
기본적으로 npm 설치를 지원하며, 동일한 접두사 흐름에서 git 체크아웃
설치도 지원합니다. 전체 참고 자료: [설치 프로그램 내부 구조](/ko/install/installer#install-clish).

이미 설치하셨습니까? `openclaw update --channel dev`와
`openclaw update --channel stable`을 사용하여 패키지 설치와 git 설치 간에 전환하십시오.
[업데이트](/ko/install/updating#switch-between-npm-and-git-installs)를 참조하십시오.

### npm, pnpm 또는 bun

Node를 직접 관리하고 있다면 다음과 같이 하십시오.
...
<Tabs>
  <Tab title="npm">
    ...
    <Note>
    호스팅된 설치 프로그램은 OpenClaw 패키지를 설치할 때 `min-release-age`
    같은 npm 최신성 필터를 해제합니다. npm으로 수동 설치하는 경우에는 자체
    npm 정책이 계속 적용됩니다.
    </Note>
  ...
  <Tab title="pnpm">
    ...
    <Note>
    pnpm에서는 빌드 스크립트가 있는 패키지를 명시적으로 승인해야 합니다. 처음 설치한 후 `pnpm approve-builds -g`를 실행하십시오.
    </Note>
  ...
  <Tab title="bun">
    ...
    <Note>
    Bun은 전역 CLI 설치 경로에서 지원됩니다. Gateway 런타임의 경우에는 Node가 여전히 권장 데몬 런타임입니다.
    </Note>
...
기여자 또는 로컬 체크아웃에서 실행하려는 사용자는 다음과 같이 하십시오.
...
또는 링크를 생략하고 저장소 내부에서 `pnpm openclaw ...`를 사용하십시오. 전체 개발 워크플로는 [설정](/ko/start/setup)을 참조하십시오.
...
설치 후 관리형 시작을 사용하려면 다음과 같이 하십시오.
...
클라우드 서버 또는 VPS에 OpenClaw를 배포합니다. 전체 공급자 선택 목록
(DigitalOcean, Hetzner, Hostinger, Fly.io, GCP, Azure, Railway,
Northflank, Oracle Cloud, Raspberry Pi 등)은 [Linux 서버](/ko/vps)를 참조하거나,
[Render](/ko/install/render)에 선언적으로 배포하십시오.
...
거의 항상 PATH 문제입니다. npm의 전역 바이너리 디렉터리가 셸의 `PATH`에 없습니다. Windows 경로를 포함한 전체 해결 방법은 [Node.js 문제 해결](/ko/install/node#troubleshooting)을 참조하십시오.
