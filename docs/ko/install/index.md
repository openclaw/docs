...
title: 설치
x-i18n:
    generated_at: "2026-05-06T06:30:21Z"
    model: gpt-5.5
    provider: openai
...
- **macOS, Linux 또는 Windows** - 네이티브 Windows와 WSL2가 모두 지원되며, WSL2가 더 안정적입니다. [Windows](/ko/platforms/windows)를 참고하세요.
...
모든 플래그와 CI/자동화 옵션은 [설치 관리자 내부 구조](/ko/install/installer)를 참고하세요.
...
기본적으로 npm 설치를 지원하며, 동일한 프리픽스 흐름에서 git 체크아웃 설치도 지원합니다. 전체 참조 문서: [설치 관리자 내부 구조](/ko/install/installer#install-clish).

이미 설치했나요? `openclaw update --channel dev`와 `openclaw update --channel stable`로 패키지 설치와 git 설치를 전환할 수 있습니다. [업데이트](/ko/install/updating#switch-between-npm-and-git-installs)를 참고하세요.
...
<Tabs>
  ...
  <Tab title="bun">
    ...
    <Note>
    Bun은 전역 CLI 설치 경로에서 지원됩니다. Gateway 런타임에는 Node가 여전히 권장 데몬 런타임입니다.
    </Note>
...
또는 링크를 건너뛰고 저장소 내부에서 `pnpm openclaw ...`를 사용하세요. 전체 개발 워크플로는 [설정](/ko/start/setup)을 참고하세요.
...
그런 다음 새 터미널을 여세요. 자세한 내용은 [Node 설정](/ko/install/node)을 참고하세요.
