...
title: Docker
x-i18n:
    generated_at: "2026-05-06T06:30:00Z"
    model: gpt-5.5
    provider: openai
...
- VPS/공개 호스트에서 실행하는 경우
  [네트워크 노출을 위한 보안 강화](/ko/gateway/security),
...
<Steps>
  ...
  <Step title="Control UI 열기">
    ...
    공유 비밀 값을 Settings에 붙여넣으세요. 설정 스크립트는 기본적으로 `.env`에 토큰을
    기록합니다. 컨테이너 config를 비밀번호 인증으로 전환했다면 대신 해당
    비밀번호를 사용하세요.
...
또는 `OPENCLAW_HOME_VOLUME`을 활성화했다면 설정 스크립트가 `docker-compose.extra.yml`을
작성합니다. `-f docker-compose.yml -f docker-compose.extra.yml`로 포함하세요.
</Note>

<Note>
`openclaw-cli`는 `openclaw-gateway`의 네트워크 네임스페이스를 공유하므로
시작 후 도구입니다. `docker compose up -d openclaw-gateway`를 실행하기 전에는 온보딩과
설정 시점의 config 쓰기를 `--no-deps --entrypoint node`와 함께
...
| `OPENCLAW_HOME_VOLUME`                     | 이름 있는 Docker 볼륨에 `/home/node` 유지                       |
...
| `OTEL_EXPORTER_OTLP_ENDPOINT`              | OpenTelemetry 내보내기를 위한 공유 OTLP/HTTP 컬렉터 엔드포인트  |
| `OTEL_EXPORTER_OTLP_*_ENDPOINT`            | traces, metrics 또는 logs용 신호별 OTLP 엔드포인트              |
| `OTEL_EXPORTER_OTLP_PROTOCOL`              | OTLP 프로토콜 재정의. 현재 `http/protobuf`만 지원               |
...
패키지된 이미지에 대해 번들 Plugin 소스를 테스트할 수 있습니다. 예:
`OPENCLAW_EXTRA_MOUNTS=/path/to/fork/extensions/synology-chat:/app/extensions/synology-chat:ro`.
마운트된 해당 소스 디렉터리는 동일한 Plugin ID에 대해 일치하는 컴파일된
`/app/dist/extensions/synology-chat` 번들을 재정의합니다.

### 관측 가능성

OpenTelemetry 내보내기는 Gateway 컨테이너에서 OTLP 컬렉터로 나가는 방향입니다.
게시된 Docker 포트가 필요하지 않습니다. 이미지를 로컬에서 빌드하고
번들 OpenTelemetry exporter를 이미지 내부에서 사용할 수 있게 하려면
해당 런타임 의존성을 포함하세요.
...
패키지된 Docker 설치에서는 내보내기를 활성화하기 전에 ClawHub에서
...
내보내기](/ko/gateway/opentelemetry)의 config 예시를 사용하세요. 컬렉터 인증 헤더는
...
macOS와 Windows에서 이미 동일한 호스트 이름을 제공합니다.
...
브리지가 멀티캐스트 트래픽을 떨어뜨릴 때 Gateway가 crash-loop에 빠지거나 광고를 반복적으로
다시 시작하지 않도록 기본값을 `OPENCLAW_DISABLE_BONJOUR=1`로 설정합니다.

Docker 호스트에는 게시된 Gateway URL, Tailscale 또는 wide-area DNS-SD를 사용하세요.
호스트 네트워킹, macvlan 또는 mDNS 멀티캐스트가 작동하는 것으로 알려진 다른 네트워크에서
실행하는 경우에만 `OPENCLAW_DISABLE_BONJOUR=0`을 설정하세요.

주의 사항과 문제 해결은 [Bonjour discovery](/ko/gateway/bonjour)를 참조하세요.
...
해당 경로는 컨테이너 교체 후에도 유지됩니다. 두 변수 중 하나가 설정되지 않은 경우 번들
`docker-compose.yml`은 `${HOME}/.openclaw`(workspace 마운트의 경우
`${HOME}/.openclaw/workspace`)로 폴백하고, `HOME` 자체도 없는 경우 `/tmp/.openclaw`로
폴백합니다. 이렇게 하면 최소 환경에서 `docker compose up`이 빈 소스 volume spec을
내보내지 않습니다.

마운트된 해당 config 디렉터리는 OpenClaw가 다음을 보관하는 위치입니다.

- 동작 config용 `openclaw.json`
- 저장된 provider OAuth/API-key 인증용 `agents/<agentId>/agent/auth-profiles.json`
- `OPENCLAW_GATEWAY_TOKEN` 같은 env 기반 런타임 비밀 값용 `.env`

설치된 다운로드 가능 Plugin은 마운트된 OpenClaw 홈 아래에 패키지 상태를 저장하므로
Plugin 설치 기록과 패키지 루트는 컨테이너 교체 후에도 유지됩니다. Gateway 시작은
번들 Plugin 의존성 트리를 생성하지 않습니다.

VM 배포의 전체 지속성 세부 정보는
[Docker VM Runtime - 무엇이 어디에 유지되는가](/ko/install/docker-vm-runtime#what-persists-where)를 참조하세요.

**디스크 증가 핫스팟:** `/tmp/openclaw/` 아래의 `media/`, 세션 JSONL 파일,
`cron/runs/*.jsonl`, 설치된 Plugin 패키지 루트, 순환 파일 로그를 확인하세요.
...
<AccordionGroup>
  <Accordion title="Docker gateway용 에이전트 샌드박스 활성화">
    ...
    스크립트는 샌드박스 사전 조건을 통과한 뒤에만 `docker.sock`을 마운트합니다. 샌드박스 설정을 완료할 수 없으면 스크립트가 `agents.defaults.sandbox.mode`를
    `off`로 재설정합니다.
  ...
  <Accordion title="공유 네트워크 보안 참고 사항">
    `openclaw-cli`는 CLI 명령이 `127.0.0.1`을 통해 Gateway에 연결할 수 있도록 `network_mode: "service:openclaw-gateway"`를 사용합니다. 이를 공유 신뢰 경계로 취급하세요. Compose 구성은 `openclaw-gateway`와 `openclaw-cli` 모두에서 `NET_RAW`/`NET_ADMIN`을 제거하고
  ...
  <Accordion title="권한 및 EACCES">
    ...
    같은 Plugin 경고 뒤에 `plugin present but blocked`가 표시되는 형태로 나타날 수 있습니다. 이는 프로세스 uid와 마운트된 Plugin 디렉터리 소유자가 서로 다르다는 뜻입니다. 컨테이너를 기본 uid 1000으로 실행하고 바인드 마운트 소유권을 고치는 방식을 권장합니다. 장기적으로 OpenClaw를 root로 실행하려는 경우에만
    `/path/to/openclaw-config/npm`을 `root:root`로 chown하세요.
  ...
  <Accordion title="고급 사용자용 컨테이너 옵션">
    기본 이미지는 보안을 우선하며 비root `node`로 실행됩니다. 더 많은 기능을 갖춘 컨테이너를 원한다면:
  ...
  <Accordion title="기본 이미지 메타데이터">
    기본 Docker 런타임 이미지는 `node:24-bookworm-slim`을 사용하며 `org.opencontainers.image.base.name`,
    `org.opencontainers.image.source` 등을 포함한 OCI 기본 이미지 주석을 게시합니다. Node 기본 다이제스트는 Dependabot Docker 기본 이미지 PR을 통해 갱신되며, 릴리스 빌드는 배포판 업그레이드 레이어를 실행하지 않습니다. 참조:
...
바이너리 포함, 지속성, 업데이트를 포함한 공유 VM 배포 단계는 [Hetzner (Docker VPS)](/ko/install/hetzner) 및
[Docker VM Runtime](/ko/install/docker-vm-runtime)을 참조하세요.

## 에이전트 샌드박스

Docker 백엔드와 함께 `agents.defaults.sandbox`를 활성화하면 Gateway 자체는 호스트에 유지하면서 Gateway가 격리된 Docker 컨테이너 안에서 에이전트 도구 실행(셸, 파일 읽기/쓰기 등)을 실행합니다. 이렇게 하면 전체 Gateway를 컨테이너화하지 않고도 신뢰할 수 없거나 멀티테넌트 에이전트 세션 주위에 강한 경계를 둘 수 있습니다.

샌드박스 범위는 에이전트별(기본값), 세션별 또는 공유로 설정할 수 있습니다. 각 범위는 `/workspace`에 마운트된 자체 워크스페이스를 가집니다. 또한 도구 허용/거부 정책, 네트워크 격리, 리소스 제한, 브라우저 컨테이너를 구성할 수 있습니다.
...
기본 샌드박스 이미지를 빌드하세요(소스 checkout에서).

```bash
scripts/sandbox-setup.sh
```

소스 checkout이 없는 npm 설치의 경우, 인라인 `docker build` 명령은 [샌드박싱 § 이미지 및 설정](/ko/gateway/sandboxing#images-and-setup)을 참조하세요.
...
<AccordionGroup>
  <Accordion title="이미지가 없거나 샌드박스 컨테이너가 시작되지 않음">
    ...
    (소스 checkout) 또는 [샌드박싱 § 이미지 및 설정](/ko/gateway/sandboxing#images-and-setup)의 인라인 `docker build` 명령(npm 설치)으로 빌드하거나,
  ...
  <Accordion title="샌드박스에서 사용자 지정 도구를 찾을 수 없음">
    OpenClaw는 `/etc/profile`을 소싱하고 PATH를 재설정할 수 있는 `sh -lc`(로그인 셸)로 명령을 실행합니다. 사용자 지정 도구 경로를 앞에 추가하도록
    `docker.env.PATH`를 설정하거나 Dockerfile의 `/etc/profile.d/` 아래에 스크립트를 추가하세요.
  </Accordion>
  ...
  <Accordion title="Gateway 대상이 ws://172.x.x.x로 표시되거나 Docker CLI에서 페어링 오류가 발생함">
