---
read_when:
    - 보안 강화를 포함한 자동화된 서버 배포를 원합니다
    - VPN 접근이 가능한 방화벽 격리 설정이 필요합니다
    - 원격 Debian/Ubuntu 서버에 배포하는 경우
summary: Ansible, Tailscale VPN 및 방화벽 격리를 통한 자동화되고 보안이 강화된 OpenClaw 설치
title: Ansible
x-i18n:
...
백엔드이므로 Docker를 설치합니다. 자세한 내용과 다른 백엔드는 [Sandboxing](/ko/gateway/sandboxing)을 참조하세요.
</Note>
