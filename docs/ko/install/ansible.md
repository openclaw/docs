...
summary: Ansible, Tailscale VPN 및 방화벽 격리를 사용한 자동화되고 강화된 OpenClaw 설치
title: Ansible
x-i18n:
    generated_at: "2026-05-02T20:54:51Z"
    model: gpt-5.5
    provider: openai
...
**[openclaw-ansible](https://github.com/openclaw/openclaw-ansible)**로 OpenClaw를 프로덕션 서버에 배포하세요. 보안을 우선하는 아키텍처를 갖춘 자동 설치 프로그램입니다.

<Info>
[openclaw-ansible](https://github.com/openclaw/openclaw-ansible) 저장소는 Ansible 배포의 기준 소스입니다. 이 페이지는 간단한 개요입니다.
</Info>
...
5. **OpenClaw** -- 컨테이너화되지 않고 호스트 기반
6. **Systemd 서비스** -- 보안 강화와 함께 자동 시작

<Note>
Gateway는 호스트에서 직접 실행됩니다(Docker 내부가 아님). 에이전트 샌드박싱은
선택 사항입니다. 이 플레이북은 기본 샌드박스
...
Docker는 Gateway 자체를 실행하기 위한 것이 아니라 에이전트 샌드박스(격리된 도구 실행)를 위해 설치됩니다. 샌드박스 구성은 [멀티 에이전트 샌드박스 및 도구](/ko/tools/multi-agent-sandbox-tools)를 참조하세요.
...
- [멀티 에이전트 샌드박스 및 도구](/ko/tools/multi-agent-sandbox-tools) -- 에이전트별 격리
