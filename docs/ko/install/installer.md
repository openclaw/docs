...
| [`install-cli.sh`](#install-clish) | macOS / Linux / WSL  | npm 또는 git을 통해 로컬 접두사(`~/.openclaw`)에 Node + OpenClaw을 설치합니다. 루트 권한이 필요하지 않습니다. |
...
<Tip>
대부분의 macOS/Linux/WSL 대화형 설치에 권장합니다.
</Tip>
...
<Steps>
  <Step title="OS 감지">
    macOS 및 Linux(WSL 포함)를 지원합니다.
  </Step>
  <Step title="기본적으로 Node.js 24 보장">
    Node 버전을 확인하고 필요한 경우 Node 24를 설치합니다(macOS에서는 Homebrew, Linux에서는 apt/dnf/yum용 NodeSource 설정 스크립트 사용). macOS에서는 설치 프로그램이 Node 또는 Git을 위해 Homebrew가 필요할 때만 Homebrew를 설치합니다. 호환성을 위해 Node 22.19+ 및 23.11+도 계속 지원합니다.
    Alpine/musl Linux에서는 설치 프로그램이 NodeSource 대신 apk 패키지를 사용합니다. 구성된 Alpine 저장소에서 지원되는 Node 버전을 제공해야 합니다(작성 시점 기준 Alpine 3.21 이상).
  </Step>
  <Step title="Git 보장">
  ...
  <Step title="OpenClaw 설치">
    ...
    - `git` 방식: 저장소를 복제/업데이트하고 pnpm으로 종속성을 설치한 후 빌드한 다음 `~/.local/bin/openclaw`에 래퍼를 설치합니다.

  </Step>
  <Step title="설치 후 작업">
    - 후속 명령에 사용할 방금 설치된 `openclaw` 바이너리를 확인합니다.
    - 구성되지 않은 설치에서는 doctor 또는 Gateway 검사보다 먼저 온보딩을 시작합니다. `--no-onboard`를 사용하거나 TTY가 없으면 나중에 설정을 완료할 명령을 출력합니다.
    - 구성된 설치에서는 로드된 Gateway 서비스를 가능한 범위에서 새로 고치고 다시 시작한 후 doctor를 실행합니다. 업그레이드 시 가능한 경우 Plugin을 업데이트하며, 헤드리스 프롬프트 활성 실행에서는 수동 명령을 출력합니다.
    - `--verify`를 실행하면 설치된 버전을 확인하고, 구성이 존재하는 경우에만 Gateway 상태를 확인합니다.
...
TTY를 사용할 수 없고 설치 방식도 설정되지 않은 경우 기본값으로 `npm`을 사용하고 경고합니다.

잘못된 방식 선택 또는 잘못된 `--install-method` 값의 경우 스크립트가 코드 `2`로 종료됩니다.
...
| `--version <version\|dist-tag\|spec>`   | npm 버전, dist-tag 또는 패키지 명세(기본값: `latest`)                   |
| `--beta`                                | 사용 가능한 경우 beta dist-tag를 사용하고, 없으면 `latest`로 대체      |
| `--git-dir \| --dir <path>`             | 체크아웃 디렉터리(기본값: `~/openclaw`)                                 |
| `--no-git-update`                       | 기존 체크아웃에서 `git pull` 건너뛰기                                   |
| `--no-prompt`                           | 프롬프트 비활성화                                                       |
| `--no-onboard`                          | 온보딩 건너뛰기                                                         |
| `--onboard`                             | 온보딩 활성화                                                           |
| `--verify`                              | 설치 후 스모크 확인 실행(`--version`, 로드된 경우 Gateway 상태)         |
...
| `OPENCLAW_VERSION=latest\|next\|<semver>\|<spec>` | npm 버전, dist-tag 또는 패키지 명세                               |
...
| `OPENCLAW_NPM_LOGLEVEL=error\|warn\|notice`       | npm 로그 수준(기본값: `error`, npm 지원 중단 관련 불필요한 로그 숨김) |
...
<Info>
모든 항목을 로컬 접두사(기본값 `~/.openclaw`) 아래에 두고
시스템 Node 종속성을 사용하지 않으려는 환경을 위해 설계되었습니다. 기본적으로
npm 설치를 지원하며, 동일한 접두사 흐름에서 git 체크아웃 설치도 지원합니다.
</Info>
...
<Steps>
  <Step title="로컬 Node 런타임 설치">
    고정된 지원 Node LTS tarball(버전은 스크립트에 포함되며 독립적으로 업데이트됨, 기본값 `22.22.2`)을 `<prefix>/tools/node-v<version>`에 다운로드하고 SHA-256을 확인합니다.
    Node가 고정 런타임과 호환되는 tarball을 게시하지 않는 Alpine/musl Linux에서는 `apk`로 `nodejs`와 `npm`을 설치하고 해당 런타임을 접두사 래퍼 경로에 연결합니다. Alpine 저장소에서 지원되는 Node 버전(22.19+, 23.11+ 또는 24+)을 제공해야 합니다. 이전 저장소에서 Node 20 또는 21만 제공하는 경우 Alpine 3.21 이상을 사용하십시오.
  </Step>
  <Step title="Git 보장">
    Git이 없으면 Linux에서는 apt/dnf/yum/apk, macOS에서는 Homebrew를 통해 설치를 시도합니다.
  </Step>
  <Step title="접두사 아래에 OpenClaw 설치">
    - `npm` 방식(기본값): npm을 사용하여 접두사 아래에 설치한 다음 `<prefix>/bin/openclaw`에 래퍼를 작성합니다.
    - `git` 방식: 체크아웃(기본값 `~/openclaw`)을 복제/업데이트하고 마찬가지로 `<prefix>/bin/openclaw`에 래퍼를 작성합니다.

  </Step>
  <Step title="로드된 Gateway 서비스 새로 고침">
    동일한 접두사에서 Gateway 서비스가 이미 로드된 경우 스크립트가
    `openclaw gateway install --force`를 실행한 다음 `openclaw gateway restart`를 실행하고
    가능한 범위에서 Gateway 상태를 검사합니다.
  </Step>
</Steps>
...
| `--set-npm-prefix`                      | Linux에서 현재 접두 경로에 쓸 수 없으면 npm 접두 경로를 `~/.npm-global`로 강제 설정          |
| `--help \| -h`                          | 사용법 표시                                                                                  |
...
<Steps>
  ...
  <Step title="기본적으로 Node.js 24 확인">
    설치되어 있지 않으면 winget, Chocolatey, Scoop 순으로 설치를 시도합니다. 사용할 수 있는 패키지 관리자가 없으면 스크립트가 공식 Node.js 24 Windows zip을 `%LOCALAPPDATA%\OpenClaw\deps\portable-node`에 다운로드하고 현재 프로세스 및 사용자 PATH에 추가합니다. 호환성을 위해 Node 22.19 이상과 23.11 이상도 계속 지원됩니다.
  </Step>
  <Step title="OpenClaw 설치">
    - `npm` 방법(기본값): 선택한 `-Tag`를 사용한 전역 npm 설치입니다. `C:\`처럼 보호된 폴더에서 연 셸에서도 작동하도록 쓰기 가능한 설치 프로그램 임시 디렉터리에서 실행됩니다.
  ...
  <Step title="설치 후 작업">
    ...
    - 로드된 Gateway 서비스를 최선형으로 새로 고칩니다(`openclaw gateway install --force` 실행 후 재시작).
    - 업그레이드 및 git 설치 시 `openclaw doctor --non-interactive`를 실행합니다(최선형).

  </Step>
  <Step title="실패 처리">
    `iwr ... | iex` 및 스크립트 블록 설치는 현재 PowerShell 세션을 닫지 않고 종료 오류를 보고합니다. 직접 실행한 `powershell -File` / `pwsh -File` 설치는 자동화를 위해 여전히 0이 아닌 코드로 종료됩니다.
  </Step>
</Steps>
...
<Note>
`-InstallMethod git`을 사용했는데 Git이 없으면 스크립트는 Git for Windows 링크를 표시하기 전에 사용자 로컬 MinGit 부트스트랩을 시도합니다.
</Note>
...
<AccordionGroup>
  <Accordion title="Git이 필요한 이유는 무엇입니까?">
    `git` 설치 방법에는 Git이 필요합니다. `npm` 설치에서도 종속성이 git URL을 사용할 때 발생하는 `spawn git ENOENT` 오류를 방지하기 위해 Git을 확인/설치합니다.
  </Accordion>

  <Accordion title="Linux에서 npm에 EACCES가 발생하는 이유는 무엇입니까?">
    일부 Linux 설정은 npm의 전역 접두 경로를 root 소유 경로로 지정합니다. `install.sh`는 접두 경로를 `~/.npm-global`로 전환하고 셸 rc 파일이 있는 경우 해당 파일에 PATH 내보내기를 추가할 수 있습니다.
  </Accordion>

  <Accordion title='Windows: "npm error spawn git / ENOENT"'>
    사용자 로컬 MinGit을 부트스트랩할 수 있도록 설치 프로그램을 다시 실행하거나 Git for Windows를 설치한 다음 PowerShell을 다시 여십시오.
  </Accordion>

  <Accordion title='Windows: "openclaw is not recognized"'>
    `npm config get prefix`를 실행하고 해당 디렉터리를 사용자 PATH에 추가한 다음(Windows에서는 `\bin` 접미사가 필요하지 않음) PowerShell을 다시 여십시오.
  </Accordion>
