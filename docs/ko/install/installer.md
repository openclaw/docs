...
title: 설치 프로그램 내부 구조
x-i18n:
    generated_at: "2026-05-02T20:55:56Z"
    model: gpt-5.5
    provider: openai
...
| [`install-cli.sh`](#install-clish) | macOS / Linux / WSL  | npm 또는 git 체크아웃 모드로 Node + OpenClaw를 로컬 접두사(`~/.openclaw`)에 설치합니다. root가 필요 없습니다.       |
...
<Note>
설치가 성공했지만 새 터미널에서 `openclaw`를 찾을 수 없다면 [Node.js 문제 해결](/ko/install/node#troubleshooting)을 참고하세요.
</Note>
...
<Steps>
  ...
  <Step title="OpenClaw 설치">
    ...
    - `git` 방식: repo를 clone/update하고, pnpm으로 deps를 설치하고, 빌드한 다음 `~/.local/bin/openclaw`에 wrapper를 설치합니다.

  </Step>
  <Step title="설치 후 작업">
    - 로드된 Gateway 서비스를 최선의 노력으로 새로 고칩니다(`openclaw gateway install --force`, 그다음 restart)
    - 업그레이드와 git 설치에서 `openclaw doctor --non-interactive`를 실행합니다(최선의 노력)
    - 적절한 경우 온보딩을 시도합니다(TTY 사용 가능, 온보딩 비활성화 아님, bootstrap/config 검사 통과)
...
TTY를 사용할 수 없고 설치 방식도 설정되지 않은 경우 기본값은 `npm`이며 경고합니다.

스크립트는 잘못된 방식 선택 또는 잘못된 `--install-method` 값에 대해 코드 `2`로 종료됩니다.
...
| `--beta`                              | 사용 가능한 경우 beta dist-tag를 사용하고, 아니면 `latest`로 fallback |
...
<Info>
모든 항목을 로컬 접두사(기본값 `~/.openclaw`) 아래에 두고 시스템 Node 의존성은 원하지 않는 환경을 위해 설계되었습니다. 기본적으로 npm 설치를 지원하며, 동일한 접두사 흐름 아래에서 git-checkout 설치도 지원합니다.
</Info>
...
<Steps>
  ...
  <Step title="Git 보장">
    Git이 없으면 Linux에서는 apt/dnf/yum으로, macOS에서는 Homebrew로 설치를 시도합니다.
  </Step>
  <Step title="접두사 아래에 OpenClaw 설치">
    - `npm` 방식(기본값): 접두사 아래에 npm으로 설치한 다음 `<prefix>/bin/openclaw`에 wrapper를 작성합니다.
    - `git` 방식: 체크아웃(기본값 `~/openclaw`)을 clone/update하고 wrapper는 계속 `<prefix>/bin/openclaw`에 작성합니다.

  </Step>
  <Step title="로드된 Gateway 서비스 새로 고침">
    Gateway 서비스가 같은 접두사에서 이미 로드되어 있으면 스크립트는 `openclaw gateway install --force`를 실행한 다음 `openclaw gateway restart`를 실행하고, 최선의 노력으로 Gateway 상태를 검사합니다.
  </Step>
</Steps>
...
| `--set-npm-prefix`          | Linux에서 현재 접두사에 쓸 수 없는 경우 npm 접두사를 `~/.npm-global`로 강제 설정 |
| `--help`                    | 사용법 표시(`-h`)                                                               |
...
<Steps>
  ...
  <Step title="기본적으로 Node.js 24 확인">
    없으면 winget, 그다음 Chocolatey, 그다음 Scoop을 통해 설치를 시도합니다. Node 22 LTS(현재 `22.14+`)도 호환성을 위해 계속 지원됩니다.
  </Step>
  <Step title="OpenClaw 설치">
    - `npm` 방법(기본값): 선택한 `-Tag`를 사용해 전역 npm 설치를 수행하며, `C:\` 같은 보호된 폴더에서 열린 셸도 계속 작동하도록 쓰기 가능한 설치 프로그램 임시 디렉터리에서 실행됩니다
    - `git` 방법: 저장소를 클론/업데이트하고, pnpm으로 설치/빌드한 다음, `%USERPROFILE%\.local\bin\openclaw.cmd`에 래퍼를 설치합니다

  </Step>
  <Step title="설치 후 작업">
    - 가능하면 필요한 bin 디렉터리를 사용자 PATH에 추가합니다
    - 로드된 Gateway 서비스를 최선의 방식으로 새로 고칩니다(`openclaw gateway install --force`, 그다음 재시작)
    - 업그레이드와 git 설치에서 `openclaw doctor --non-interactive`를 실행합니다(최선 노력)

  </Step>
  <Step title="실패 처리">
    `iwr ... | iex` 및 scriptblock 설치는 현재 PowerShell 세션을 닫지 않고 종료 오류를 보고합니다. 직접 `powershell -File` / `pwsh -File`로 설치하는 경우 자동화를 위해 여전히 0이 아닌 코드로 종료됩니다.
  </Step>
</Steps>
...
<AccordionGroup>
  <Accordion title="Git이 왜 필요한가요?">
    Git은 `git` 설치 방법에 필요합니다. `npm` 설치의 경우에도 의존성이 git URL을 사용할 때 `spawn git ENOENT` 실패를 방지하기 위해 Git을 확인/설치합니다.
  </Accordion>

  <Accordion title="Linux에서 npm이 EACCES를 만나는 이유는 무엇인가요?">
    일부 Linux 설정은 npm 전역 접두사가 root 소유 경로를 가리킵니다. `install.sh`는 접두사를 `~/.npm-global`로 전환하고 셸 rc 파일에 PATH export를 추가할 수 있습니다(해당 파일이 있을 때).
  </Accordion>

  <Accordion title="sharp/libvips 문제">
    스크립트는 sharp가 시스템 libvips를 기준으로 빌드되지 않도록 기본적으로 `SHARP_IGNORE_GLOBAL_LIBVIPS=1`을 사용합니다. 재정의하려면:
