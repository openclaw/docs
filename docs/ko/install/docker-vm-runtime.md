---
read_when:
    - Docker를 사용하여 클라우드 가상 머신에 OpenClaw를 배포하고 있습니다
    - 공유 바이너리 빌드, 영속성, 업데이트 흐름이 필요합니다
summary: 장기 운영 OpenClaw Gateway 호스트를 위한 공유 Docker VM 런타임 단계
title: Docker VM 런타임
x-i18n:
    generated_at: "2026-05-02T20:55:13Z"
    model: gpt-5.5
    provider: openai
...
VM 기반 Docker 설치(예: GCP, Hetzner 및 유사 VPS 제공업체)를 위한 공유 런타임 단계입니다.

## 필요한 바이너리를 이미지에 베이크

실행 중인 컨테이너 안에서 바이너리를 설치하는 것은 함정입니다.
런타임에 설치한 것은 무엇이든 재시작 시 손실됩니다.
...
<Note>
위 URL은 예시입니다. ARM 기반 VM의 경우 `arm64` 애셋을 선택하세요. 재현 가능한 빌드를 위해서는 버전이 지정된 릴리스 URL을 고정하세요.
</Note>
...
`pnpm install --frozen-lockfile` 중에 빌드가 `Killed` 또는 `exit code 137`로 실패하면 VM의 메모리가 부족한 것입니다.
...
모든 장기 상태는 재시작, 재빌드, 재부팅 후에도 살아남아야 합니다.
...
| 외부 바이너리       | `/usr/local/bin/`                                      | Docker 이미지          | 빌드 시점에 베이크해야 함                                     |
