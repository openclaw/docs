...
title: Hostinger
x-i18n:
    generated_at: "2026-07-12T15:22:23Z"
    model: gpt-5.6
...
[Hostinger](https://www.hostinger.com/openclaw)에서 영구 OpenClaw Gateway를 실행합니다. **1-Click** 관리형 배포를 사용하거나 직접 관리하는 **VPS**에 설치할 수 있습니다.
...
Hostinger가 인프라, Docker 및 자동 업데이트를 관리합니다. 실행 중인 인스턴스를 가장 빠르게 구축할 수 있는 방법입니다.
...
<Steps>
  <Step title="구매 및 실행">
    ...
    <Note>
    결제 중에 미리 구매되어 OpenClaw에 즉시 통합되는 **Ready-to-Use AI** 크레딧을 선택할 수 있습니다. 다른 제공업체의 외부 계정이나 API 키가 필요하지 않으며 바로 채팅을 시작할 수 있습니다. 또는 설정 중에 Anthropic, OpenAI, Google Gemini 또는 xAI의 자체 키를 제공할 수 있습니다.
    </Note>
  ...
  <Step title="설치 완료">
    **Finish**를 클릭하여 인스턴스를 배포합니다. 준비가 완료되면 hPanel의 **OpenClaw Overview**에서 OpenClaw 대시보드에 액세스합니다.
  </Step>
...
<Steps>
  <Step title="VPS 구매">
    1. [Hostinger OpenClaw 페이지](https://www.hostinger.com/openclaw)에서 OpenClaw on VPS 요금제를 선택하고 결제를 완료합니다.
...
연결한 채널에서 어시스턴트에게 "안녕하세요"라고 보냅니다. OpenClaw가 응답하고 초기 기본 설정 과정을 안내합니다.

## 문제 해결

**대시보드가 로드되지 않음** -- 컨테이너 프로비저닝이 완료될 때까지 몇 분 기다린 다음 hPanel에서 Docker Manager 로그를 확인합니다.

**Docker 컨테이너가 계속 재시작됨** -- Docker Manager 로그를 열어 구성 오류(누락된 토큰, 유효하지 않은 API 키)를 찾습니다.

**Telegram 봇이 응답하지 않음** -- DM 페어링이 필요한 경우 알 수 없는 발신자에게는 응답 대신 짧은 페어링 코드가 전송됩니다. OpenClaw 대시보드 채팅에서 승인하거나, 컨테이너에 셸로 액세스할 수 있다면 `openclaw pairing approve telegram <CODE>`로 승인합니다. [페어링](/ko/channels/pairing)을 참조하십시오.
