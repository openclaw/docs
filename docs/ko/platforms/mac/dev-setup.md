...
read_when:
    - macOS 개발 환경 설정하기
summary: OpenClaw macOS 앱 작업을 하는 개발자를 위한 설정 가이드
title: macOS 개발 환경 설정
x-i18n:
    generated_at: "2026-05-06T09:03:05Z"
    model: gpt-5.5
    provider: openai
...
2. **Node.js 24 및 pnpm**: Gateway, CLI, 패키징 스크립트에 권장됩니다. 현재 `22.14+`인 Node 22 LTS는 호환성을 위해 계속 지원됩니다.
...
Gateway 런타임에는 Node 경로가 계속 권장됩니다.

## 문제 해결

### 빌드 실패: 도구 체인 또는 SDK 불일치

macOS 앱 빌드는 최신 macOS SDK와 Swift 6.2 도구 체인을 기대합니다.
...
2. 실패하면 [`scripts/package-mac-app.sh`](https://github.com/openclaw/openclaw/blob/main/scripts/package-mac-app.sh)의 `BUNDLE_ID`를 임시로 변경하여 macOS에서 "clean slate"를 강제로 적용합니다.

### Gateway가 "Starting..." 상태로 무기한 유지됨

Gateway 상태가 "Starting..."에 머무르면 좀비 프로세스가 포트를 점유하고 있는지 확인하세요.

```bash
openclaw gateway status
openclaw gateway stop

# LaunchAgent를 사용하지 않는 경우(dev mode / manual runs) 리스너를 찾습니다.
