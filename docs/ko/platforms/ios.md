...
title: iOS 앱
x-i18n:
    generated_at: "2026-07-02T08:05:08Z"
    model: gpt-5.5
...
가용성: iPhone 앱 빌드는 릴리스에 대해 활성화된 경우 Apple 채널을 통해 배포됩니다. 로컬 개발 빌드도 소스에서 실행할 수 있습니다.

## 기능

- WebSocket(LAN 또는 tailnet)을 통해 Gateway에 연결합니다.
- Node 기능을 노출합니다: Canvas, 화면 스냅샷, 카메라 캡처, 위치, Talk 모드, Voice wake.
...
명시적 CIDR 또는 정확한 IP를 사용하여 최초 Node 자동 승인을 선택적으로 활성화할 수 있습니다.
...
이는 기본적으로 비활성화되어 있습니다. 요청된 범위가 없는 새로운 `role: node` 페어링에만 적용됩니다. 운영자/브라우저 페어링과 역할, 범위, 메타데이터 또는 공개 키 변경은 여전히 수동 승인이 필요합니다.
...
- Gateway는 저장된 릴레이 핸들을 `push.test`, 백그라운드 깨우기, wake nudge에 사용합니다.
- 사용자 지정 Gateway 릴레이 URL은 iOS 빌드에 내장된 릴레이 URL과 일치해야 합니다.
- 앱이 나중에 다른 Gateway 또는 다른 릴레이 기본 URL을 가진 빌드에 연결하면, 이전 바인딩을 재사용하는 대신 릴레이 등록을 새로 고칩니다.
...
5. 그 이후 `push.test`, 재연결 깨우기, wake nudge는 저장된 릴레이 기반 등록을 사용할 수 있습니다.

## 백그라운드 활성 비콘

iOS가 무음 푸시, 백그라운드 새로 고침 또는 중요한 위치 이벤트로 앱을 깨우면, 앱은 짧은 Node 재연결을 시도한 뒤 `event: "node.presence.alive"`로 `node.event`를 호출합니다.
Gateway는 인증된 Node 장치 ID가 알려진 뒤에만 이를 페어링된 Node/장치 메타데이터의 `lastSeenAtMs`/`lastSeenReason`으로 기록합니다.

앱은 Gateway 응답에 `handled: true`가 포함된 경우에만 백그라운드 깨우기가 성공적으로 기록된 것으로 처리합니다. 이전 Gateway는 `{ "ok": true }`로 `node.event`를 승인할 수 있습니다. 해당 응답은 호환되지만 지속적인 마지막 확인 업데이트로 계산되지는 않습니다.

호환성 참고:

- `OPENCLAW_APNS_RELAY_BASE_URL`은 Gateway의 임시 환경 변수 재정의로 계속 작동합니다.
- 공개 App Store 릴리스 레인은 iOS 빌드의 `OPENCLAW_PUSH_RELAY_BASE_URL`을 거부합니다.
...
- Gateway는 해당 특정 Gateway와 페어링된 iOS 장치에 대해서만 릴레이 기반 푸시를 보낼 수 있습니다.
...
2. `iOS app -> relay`
   ...
   - 이것이 로컬 Xcode/개발 빌드가 호스팅 릴레이를 사용하지 못하게 막는 요소입니다. 로컬 빌드는 서명되어 있을 수 있지만 릴레이가 기대하는 공식 Apple 배포 증명을 충족하지 않습니다.
...
4. `gateway -> relay`
   ...
   - `push.test`, 재연결 깨우기, wake nudge에서 Gateway는 자체 장치 ID로 전송 요청에 서명합니다.
   - 릴레이는 저장된 전송 권한과 Gateway 서명을 등록 시 위임된 Gateway ID와 대조해 모두 검증합니다.
   - 다른 Gateway는 어떻게든 핸들을 얻더라도 해당 저장된 등록을 재사용할 수 없습니다.

5. `relay -> APNs`
   - 릴레이는 공식 빌드의 프로덕션 APNs 자격 증명과 원시 APNs 토큰을 소유합니다.
   - Gateway는 릴레이 기반 공식 빌드의 원시 APNs 토큰을 절대 저장하지 않습니다.
...
- 공식 OpenClaw iOS 빌드에만 호스팅 릴레이 사용을 허용하기 위해.
- 한 Gateway가 다른 Gateway에 속한 iOS 장치로 wake 푸시를 보내지 못하게 하기 위해.
...
이는 Gateway 호스트 런타임 환경 변수이며 Fastlane 설정이 아닙니다. `apps/ios/fastlane/.env`는 `APP_STORE_CONNECT_KEY_ID` 및 `APP_STORE_CONNECT_ISSUER_ID` 같은 App Store Connect 인증만 저장합니다. 로컬 iOS 빌드의 직접 APNs 전달은 구성하지 않습니다.
...
iOS 앱은 `local.`에서 `_openclaw-gw._tcp`를 탐색하며, 구성된 경우 동일한 광역 DNS-SD 검색 도메인도 탐색합니다. 동일 LAN Gateway는 `local.`에서 자동으로 나타납니다. 네트워크 간 검색은 비콘 유형을 변경하지 않고 구성된 광역 도메인을 사용할 수 있습니다.

### Tailnet(네트워크 간)

mDNS가 차단된 경우 유니캐스트 DNS-SD 영역(도메인 선택, 예:
`openclaw.internal.`)과 Tailscale 분할 DNS를 사용하세요.
...
- iOS Node는 기본 제공 스캐폴드를 연결된 기본 보기로 유지합니다. `canvas.a2ui.push` 및 `canvas.a2ui.reset`은 번들된 앱 소유 A2UI 페이지를 사용합니다.
- 원격 Gateway A2UI 페이지는 iOS에서 렌더링 전용입니다. 네이티브 A2UI 버튼 작업은 번들된 앱 소유 페이지에서만 허용됩니다.
- `canvas.navigate`와 `{"url":""}`로 기본 제공 스캐폴드로 돌아갑니다.

## Computer Use 관계

iOS 앱은 모바일 Node 표면이며 Codex Computer Use 백엔드가 아닙니다. Codex Computer Use 및 `cua-driver mcp`는 MCP 도구를 통해 로컬 macOS 데스크톱을 제어합니다. iOS 앱은 `canvas.*`, `camera.*`, `screen.*`, `location.*`, `talk.*` 같은 OpenClaw Node 명령을 통해 iPhone 기능을 노출합니다.
...
- Talk 가능 iOS Node는 `talk` 기능을 광고하며 `talk.ptt.start`, `talk.ptt.stop`, `talk.ptt.cancel`, `talk.ptt.once`를 선언할 수 있습니다. Gateway는 신뢰할 수 있는 Talk 가능 Node에 대해 이러한 push-to-talk 명령을 기본적으로 허용합니다.
- iOS는 백그라운드 오디오를 일시 중단할 수 있습니다. 앱이 활성 상태가 아닐 때는 음성 기능을 최선 노력 방식으로 취급하세요.

## 일반적인 오류

- `NODE_BACKGROUND_UNAVAILABLE`: iOS 앱을 포그라운드로 가져오세요(Canvas/카메라/화면 명령에 필요).
- `A2UI_HOST_UNAVAILABLE`: 번들된 A2UI 페이지에 앱 WebView에서 접근할 수 없습니다. 앱을 Screen 탭에서 포그라운드로 유지한 뒤 다시 시도하세요.
