...
summary: Gateway 프로토콜의 단일 진실 공급원으로서의 TypeBox 스키마
title: TypeBox
x-i18n:
...
| 메시징     | `send`, `agent`, `agent.wait`, `system-event`, `logs.tail` | 부작용에는 `idempotencyKey`가 필요 |
...
- 소스: `packages/gateway-protocol/src/schema.ts`
- 런타임 검증기(AJV): `packages/gateway-protocol/src/index.ts`
- 공지되는 기능/탐색 레지스트리: `src/gateway/server-methods-list.ts`
...
- **클라이언트 측**: JS 클라이언트는 이벤트 및 응답 프레임을 사용하기 전에
  검증합니다.
- **기능 탐색**: Gateway는 `listGatewayMethods()` 및 `GATEWAY_EVENTS`에서 가져온
  보수적인 `features.methods` 및 `features.events` 목록을 `hello-ok`로 보냅니다.
- 이 탐색 목록은 `coreGatewayHandlers`의 호출 가능한 모든 헬퍼를 생성해서 덤프한
  것이 아닙니다. 일부 헬퍼 RPC는 공지되는 기능 목록에 열거되지 않은 채
  `src/gateway/server-methods/*.ts`에 구현되어 있습니다.
...
`packages/gateway-protocol/src/schema.ts`에 추가합니다.
...
해당 메서드가 운영자 또는 노드 클라이언트에서 호출 가능하다면, 범위 적용과
`hello-ok` 기능 공지가 일치하도록 `src/gateway/method-scopes.ts`에서도 분류하세요.
...
알 수 없는 프레임 타입은 이후 호환성을 위해 원시 페이로드로 보존됩니다.
...
- ID와 메서드/이벤트 이름의 기본값은 `NonEmptyString`입니다.
- 최상위 `GatewayFrame`은 `type`의 **discriminator**를 사용합니다.
- 부작용이 있는 메서드는 일반적으로 params에 `idempotencyKey`가 필요합니다
  (예: `send`, `poll`, `agent`, `chat.send`).
- `agent`는 런타임 생성 오케스트레이션 컨텍스트를 위해 선택적 `internalEvents`를
  허용합니다(예: 하위 에이전트/cron 작업 완료 핸드오프). 이를 내부 API 표면으로
  취급하세요.
...
3. 새 RPC에 운영자 또는 노드 범위 분류가 필요하면 `src/gateway/method-scopes.ts`를
   업데이트합니다.
