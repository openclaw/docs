---
read_when:
    - OpenClaw와 통신하는 외부 앱, 스크립트, 대시보드, CI 작업 또는 IDE 확장 프로그램을 빌드하고 있습니다
    - App SDK와 Plugin SDK 중에서 선택하기
    - Gateway 에이전트 실행, 세션, 이벤트, 승인, 모델 또는 도구와 통합하는 경우
sidebarTitle: App SDK
summary: 외부 앱, 스크립트, 대시보드, CI 작업 및 IDE 확장 프로그램을 위한 공개 OpenClaw 앱 SDK
title: OpenClaw 앱 SDK
x-i18n:
    generated_at: "2026-05-02T20:48:28Z"
    model: gpt-5.5
    provider: openai
...
**OpenClaw App SDK**는 OpenClaw 프로세스 외부의 앱을 위한 공개 클라이언트 API입니다. 스크립트, 대시보드, CI 작업, IDE 확장 기능 또는 기타 외부 앱이 Gateway에 연결하거나, 에이전트 실행을 시작하거나, 이벤트를 스트리밍하거나, 결과를 기다리거나, 작업을 취소하거나, Gateway 리소스를 검사하려면 `@openclaw/sdk`를 사용하세요.

<Note>
  App SDK는 [Plugin SDK](/ko/plugins/sdk-overview)와 다릅니다.
  `@openclaw/sdk`는 OpenClaw 외부에서 Gateway와 통신합니다.
  `openclaw/plugin-sdk/*`는 OpenClaw 내부에서 실행되며 공급자, 채널, 도구, 훅 또는 신뢰할 수 있는 런타임을 등록하는 Plugin에만 사용됩니다.
</Note>
...
| `GatewayClientTransport`  | 준비됨  | Gateway 클라이언트가 지원하는 WebSocket 전송입니다.                          |
...
| `oc.sessions`             | 준비됨  | 세션 핸들을 생성, 해석, 전송, 패치, 압축, 조회합니다.  |
...
| `oc.artifacts`            | 준비됨  | Gateway 트랜스크립트 아티팩트를 나열, 조회, 다운로드합니다.                   |
| `oc.approvals`            | 준비됨  | Gateway 승인 RPC를 통해 실행 승인을 나열하고 해결합니다.           |
| `oc.rawEvents()`          | 준비됨  | 고급 소비자를 위해 원시 Gateway 이벤트를 노출합니다.                         |
| `normalizeGatewayEvent()` | 준비됨  | 원시 Gateway 이벤트를 안정적인 SDK 이벤트 형식으로 변환합니다.               |

SDK는 이러한 Surface에서 사용하는 핵심 타입도 내보냅니다.
...
`new OpenClaw({ gateway: "ws://..." })`는 `url`과 동일합니다. 생성자는
`gateway: "auto"` 옵션을 허용하지만, 자동 Gateway 검색은 아직 별도의 SDK 기능이 아닙니다. 앱이 Gateway를 검색하는 방법을 이미 알고 있지 않다면 `url`을 전달하세요.
...
앱에서 에이전트 핸들이 필요하면 `oc.agents.get(id)`를 사용한 다음
`agent.run()`을 호출하세요.
...
`openai/gpt-5.5`와 같은 공급자 한정 모델 참조는 Gateway `provider`와 `model` 오버라이드로 분리됩니다. `timeoutMs`는 SDK에서 밀리초로 유지되며 `agent` RPC용 Gateway 시간 제한 초 단위로 변환됩니다.

`run.wait()`는 Gateway `agent.wait` RPC를 사용합니다. 실행이 아직 활성 상태인 동안 대기 기한이 만료되면 실행 자체가 시간 초과된 것처럼 처리하지 않고 `status: "accepted"`를 반환합니다. 런타임 시간 초과, 중단된 실행, 취소된 실행은 `timed_out` 또는 `cancelled`로 정규화됩니다.

## 세션 생성 및 재사용하기

앱에서 지속적인 트랜스크립트 상태가 필요하면 세션을 사용하세요.
...
아티팩트 헬퍼는 세션, 실행 또는 작업 컨텍스트에 대한 Gateway 아티팩트 투영을 노출합니다. 각 호출에는 명시적인 `sessionKey`, `runId` 또는
`taskId` 범위가 하나 필요합니다.
...
SDK에는 우리가 원하는 제품 모델을 위한 이름이 포함되어 있지만, Gateway RPC가 존재하는 것처럼 조용히 가장하지는 않습니다. 현재 다음 호출은 명시적인 지원되지 않음 오류를 던집니다.
...
실행별 `workspace`, `runtime`, `environment`, `approvals` 필드는 향후 형식으로 타입이 지정되어 있지만, 현재 Gateway는 `agent` RPC에서 이러한 오버라이드를 지원하지 않습니다. 호출자가 이를 전달하면, 기본 워크스페이스, 런타임, 환경 또는 승인 동작으로 작업이 실수로 실행되지 않도록 SDK가 실행 제출 전에 예외를 던집니다.
...
App SDK 코드는 `@openclaw/sdk`에서 가져와야 합니다. Plugin 코드는 문서화된 `openclaw/plugin-sdk/*` 하위 경로에서 가져와야 합니다. 두 계약을 혼용하지 마세요.
