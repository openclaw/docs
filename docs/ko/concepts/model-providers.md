...
read_when:
    ...
    - 모델 제공자를 위한 예시 구성이나 CLI 온보딩 명령이 필요한 경우
sidebarTitle: Model providers
summary: 예시 설정 + CLI 흐름이 포함된 모델 제공자 개요
title: 모델 제공자
x-i18n:
    generated_at: "2026-05-03T06:16:15Z"
    model: gpt-5.5
    provider: openai
...
LLM/모델 공급자(WhatsApp/Telegram 같은 채팅 채널이 아님)에 대한 참조입니다. 모델 선택 규칙은 [모델](/ko/concepts/models)을 참조하세요.
...
<AccordionGroup>
  <Accordion title="Model refs and CLI helpers">
    ...
    - `models.providers.*.contextWindow` / `contextTokens` / `maxTokens`는 공급자 수준 기본값을 설정합니다. `models.providers.*.models[].contextWindow` / `contextTokens` / `maxTokens`는 모델별로 이를 재정의합니다.
  ...
  <Accordion title="Adding provider auth does not change your primary model">
    `openclaw configure`는 공급자를 추가하거나 다시 인증할 때 기존 `agents.defaults.model.primary`를 유지합니다. 공급자 Plugin은 인증 구성 패치에서 권장 기본 모델을 여전히 반환할 수 있지만, 기본 모델이 이미 있으면 configure는 이를 "이 모델을 사용 가능하게 만들기"로 처리하며, "현재 기본 모델 교체"로 처리하지 않습니다.
  ...
  <Accordion title="OpenAI provider/runtime split">
    ...
    - `openai/<model>`에 `agents.defaults.agentRuntime.id: "codex"`를 함께 사용하면 네이티브 Codex 앱 서버 하네스를 사용합니다. 일반적인 ChatGPT/Codex 구독 설정입니다.
    - `openai-codex/<model>`은 PI에서 Codex OAuth를 사용합니다.
    - Codex 런타임 재정의가 없는 `openai/<model>`은 PI의 직접 OpenAI API 키 공급자를 사용합니다.

    [OpenAI](/ko/providers/openai) 및 [Codex 하네스](/ko/plugins/codex-harness)를 참조하세요. 공급자/런타임 분리가 혼란스럽다면 먼저 [에이전트 런타임](/ko/concepts/agent-runtimes)을 읽으세요.
  ...
  <Accordion title="CLI runtimes">
    CLI 런타임은 동일한 분리를 사용합니다. `anthropic/claude-*`, `google/gemini-*`, `openai/gpt-*` 같은 표준 모델 참조를 선택한 다음, 로컬 CLI 백엔드를 원할 때 `agents.defaults.agentRuntime.id`를 `claude-cli`, `google-gemini-cli`, 또는 `codex-cli`로 설정하세요.

    레거시 `claude-cli/*`, `google-gemini-cli/*`, `codex-cli/*` 참조는 런타임이 별도로 기록된 표준 공급자 참조로 다시 마이그레이션됩니다.
...
대부분의 공급자별 로직은 공급자 Plugin(`registerProvider(...)`)에 있으며, OpenClaw는 일반 추론 루프를 유지합니다. Plugin은 온보딩, 모델 카탈로그, 인증 환경 변수 매핑, 전송/구성 정규화, 도구 스키마 정리, 장애 조치 분류, OAuth 새로 고침, 사용량 보고, 사고/추론 프로필 등을 소유합니다.

공급자 SDK 훅과 번들 Plugin 예제의 전체 목록은 [공급자 Plugin](/ko/plugins/sdk-provider-plugins)에 있습니다. 완전히 사용자 지정된 요청 실행기가 필요한 공급자는 별도의 더 깊은 확장 표면입니다.

<Note>
공급자 소유 러너 동작은 재생 정책, 도구 스키마 정규화, 스트림 래핑, 전송/요청 헬퍼 같은 명시적 공급자 훅에 있습니다. 레거시 `ProviderPlugin.capabilities` 정적 백은 호환성 전용이며 더 이상 공유 러너 로직에서 읽지 않습니다.
</Note>
...
<AccordionGroup>
  <Accordion title="Key sources and priority">
    ...
    Google 공급자의 경우 `GOOGLE_API_KEY`도 폴백으로 포함됩니다. 키 선택 순서는 우선순위를 유지하고 값을 중복 제거합니다.

  </Accordion>
  <Accordion title="When rotation kicks in">
    - 요청은 속도 제한 응답(예: `429`, `rate_limit`, `quota`, `resource exhausted`, `Too many concurrent requests`, `ThrottlingException`, `concurrency limit reached`, `workers_ai ... quota limit exceeded`, 또는 주기적인 사용량 제한 메시지)에서만 다음 키로 재시도됩니다.
...
## 기본 제공 공급자(pi-ai 카탈로그)

OpenClaw에는 pi‑ai 카탈로그가 포함되어 있습니다. 이러한 공급자는 `models.providers` 구성이 **필요 없습니다**. 인증을 설정하고 모델을 선택하기만 하면 됩니다.
...
- 특정 설치나 API 키가 다르게 동작하면 `openclaw models list --provider openai`로 계정/모델 사용 가능 여부를 확인하세요.
...
- 숨겨진 OpenClaw 기여 헤더(`originator`, `version`, `User-Agent`)는 `api.openai.com`으로 향하는 네이티브 OpenAI 트래픽에만 적용되며, 일반 OpenAI 호환 프록시에는 적용되지 않습니다.
...
- 선호되는 Claude CLI 구성은 모델 참조를 표준으로 유지하고 CLI
  ...
  `agents.defaults.agentRuntime.id: "claude-cli"`를 함께 사용합니다. 레거시
  `claude-cli/claude-opus-4-7` 참조도 호환성을 위해 계속 작동합니다.

<Note>
Anthropic 직원이 OpenClaw 방식의 Claude CLI 사용이 다시 허용된다고 알려왔으므로, Anthropic이 새 정책을 게시하지 않는 한 OpenClaw는 이 통합에서 Claude CLI 재사용과 `claude -p` 사용을 승인된 것으로 처리합니다. Anthropic setup-token은 지원되는 OpenClaw 토큰 경로로 계속 사용할 수 있지만, OpenClaw는 이제 사용 가능한 경우 Claude CLI 재사용과 `claude -p`를 선호합니다.
</Note>
...
- 숨겨진 OpenClaw 기여 헤더(`originator`, `version`, `User-Agent`)는 `chatgpt.com/backend-api`로 향하는 네이티브 Codex 트래픽에만 첨부되며, 일반 OpenAI 호환 프록시에는 첨부되지 않습니다.
...
- 일반적인 구독과 네이티브 Codex 런타임 경로의 경우 `openai-codex` 인증으로 로그인하되, `openai/gpt-5.5`와 `agents.defaults.agentRuntime.id: "codex"`를 함께 구성하세요.
- PI를 통한 Codex OAuth/구독 경로를 원할 때만 `openai-codex/gpt-5.5`를 사용하세요. API 키 설정과 로컬 카탈로그가 공개 API 경로를 노출하는 경우에는 Codex 런타임 재정의 없이 `openai/gpt-5.5`를 사용하세요.
...
<CardGroup cols={3}>
  ...
  <Card title="Qwen Cloud" href="/ko/providers/qwen">
    Qwen Cloud 공급자 표면과 Alibaba DashScope 및 Coding Plan 엔드포인트 매핑입니다.
  </Card>
</CardGroup>
...
- 별칭: `google/gemini-3.1-pro`가 허용되며 Google의 실시간 Gemini API ID인 `google/gemini-3.1-pro-preview`로 정규화됩니다.
...
<Warning>
OpenClaw의 Gemini CLI OAuth는 비공식 통합입니다. 일부 사용자는 타사 클라이언트를 사용한 뒤 Google 계정 제한을 보고했습니다. 계속 진행하기로 했다면 Google 약관을 검토하고 중요하지 않은 계정을 사용하세요.
</Warning>
...
- 정적 대체 카탈로그는 `kilocode/kilo/auto`를 제공합니다. 실시간 `https://api.kilo.ai/api/gateway/models` 탐색은 런타임 카탈로그를 더 확장할 수 있습니다.
...
<AccordionGroup>
  <Accordion title="OpenRouter">
    검증된 `openrouter.ai` 경로에서만 앱 출처 표시 헤더와 Anthropic `cache_control` 마커를 적용합니다. DeepSeek, Moonshot, ZAI 참조는 OpenRouter가 관리하는 프롬프트 캐싱의 cache-TTL 대상이지만 Anthropic 캐시 마커는 받지 않습니다. 프록시 스타일의 OpenAI 호환 경로이므로 네이티브 OpenAI 전용 조정(`serviceTier`, Responses `store`, 프롬프트 캐시 힌트, OpenAI reasoning 호환성)은 건너뜁니다. Gemini 기반 참조는 프록시 Gemini thought-signature 정리만 유지합니다.
  </Accordion>
  <Accordion title="Kilo Gateway">
    Gemini 기반 참조는 동일한 프록시 Gemini 정리 경로를 따릅니다. `kilocode/kilo/auto` 및 기타 프록시 reasoning 미지원 참조는 프록시 reasoning 주입을 건너뜁니다.
  </Accordion>
  <Accordion title="MiniMax">
    API 키 온보딩은 명시적인 텍스트 전용 M2.7 채팅 모델 정의를 기록합니다. 이미지 이해는 plugin이 소유한 `MiniMax-VL-01` 미디어 공급자에 유지됩니다.
  </Accordion>
  <Accordion title="NVIDIA">
    모델 ID는 `nvidia/<vendor>/<model>` 네임스페이스를 사용합니다(예: `nvidia/moonshotai/kimi-k2.5`와 함께 `nvidia/nvidia/nemotron-...`). 선택기는 리터럴 `<provider>/<model-id>` 구성을 보존하지만 API로 전송되는 정식 키는 단일 접두사 상태를 유지합니다.
  </Accordion>
  <Accordion title="xAI">
...
아래의 번들 공급자 plugin 중 다수는 이미 기본 카탈로그를 게시합니다. 기본 기본 URL, 헤더 또는 모델 목록을 재정의하려는 경우에만 명시적인 `models.providers.<id>` 항목을 사용하세요.

Gateway 모델 기능 검사도 명시적인 `models.providers.<id>.models[]` 메타데이터를 읽습니다. 사용자 지정 또는 프록시 모델이 이미지를 허용하는 경우, WebChat과 Node 원본 첨부 파일 경로가 이미지를 텍스트 전용 미디어 참조 대신 네이티브 모델 입력으로 전달하도록 해당 모델에 `input: ["text", "image"]`를 설정하세요.

### Moonshot AI (Kimi)

Moonshot은 번들 공급자 plugin으로 제공됩니다. 기본적으로 내장 공급자를 사용하고, 기본 URL 또는 모델 메타데이터를 재정의해야 할 때만 명시적인 `models.providers.moonshot` 항목을 추가하세요.
...
온보딩/모델 구성 선택기에서 Volcengine 인증 선택은 `volcengine/*` 및 `volcengine-plan/*` 행을 모두 우선합니다. 해당 모델이 아직 로드되지 않은 경우 OpenClaw는 비어 있는 공급자 범위 선택기를 표시하는 대신 필터링되지 않은 카탈로그로 폴백합니다.
...
온보딩/모델 구성 선택기에서 BytePlus 인증 선택은 `byteplus/*` 및 `byteplus-plan/*` 행을 모두 우선합니다. 해당 모델이 아직 로드되지 않은 경우 OpenClaw는 비어 있는 공급자 범위 선택기를 표시하는 대신 필터링되지 않은 카탈로그로 폴백합니다.
...
<Note>
MiniMax의 Anthropic 호환 스트리밍 경로에서는 명시적으로 설정하지 않는 한 OpenClaw가 기본적으로 thinking을 비활성화하며, `/fast on`은 `MiniMax-M2.7`을 `MiniMax-M2.7-highspeed`로 다시 씁니다.
</Note>
...
`OLLAMA_API_KEY`로 옵트인하면 Ollama가 로컬의 `http://127.0.0.1:11434`에서 감지되며, 번들 제공자 Plugin이 Ollama를 `openclaw onboard`와 모델 선택기에 직접 추가합니다. 온보딩, 클라우드/로컬 모드, 사용자 지정 구성은 [/providers/ollama](/ko/providers/ollama)를 참조하세요.
...
<AccordionGroup>
  <Accordion title="기본 선택적 필드">
    사용자 지정 공급자에서 `reasoning`, `input`, `cost`, `contextWindow`, `maxTokens`는 선택 사항입니다. 생략하면 OpenClaw는 다음을 기본값으로 사용합니다.
  ...
  <Accordion title="프록시 라우트 형태 지정 규칙">
    - 네이티브가 아닌 엔드포인트(호스트가 `api.openai.com`이 아닌 비어 있지 않은 `baseUrl`)에서 `api: "openai-completions"`를 사용하는 경우, OpenClaw는 지원되지 않는 `developer` 역할로 인한 공급자 400 오류를 피하기 위해 `compat.supportsDeveloperRole: false`를 강제합니다.
    - 프록시 스타일 OpenAI 호환 라우트는 네이티브 OpenAI 전용 요청 형태 지정도 건너뜁니다. `service_tier` 없음, Responses `store` 없음, Completions `store` 없음, 프롬프트 캐시 힌트 없음, OpenAI reasoning 호환 페이로드 형태 지정 없음, 숨겨진 OpenClaw 어트리뷰션 헤더 없음.
    ...
    - 느린 로컬 모델이나 원격 LAN/tailnet 호스트의 경우 `models.providers.<id>.timeoutSeconds`를 설정하세요. 이는 전체 에이전트 런타임 타임아웃을 늘리지 않고도 연결, 헤더, 본문 스트리밍, 전체 보호된 fetch 중단을 포함한 공급자 모델 HTTP 요청 처리를 확장합니다.
    ...
    - 비직접 엔드포인트(정식 `anthropic`이 아닌 모든 공급자 또는 호스트가 공개 `api.anthropic.com` 엔드포인트가 아닌 사용자 지정 `models.providers.anthropic.baseUrl`)에서 `api: "anthropic-messages"`를 사용하는 경우, OpenClaw는 사용자 지정 Anthropic 호환 프록시가 지원되지 않는 베타 플래그를 거부하지 않도록 `claude-code-20250219`, `interleaved-thinking-2025-05-14`, OAuth 마커와 같은 암시적 Anthropic 베타 헤더를 억제합니다. 프록시에 특정 베타 기능이 필요한 경우 `models.providers.<id>.headers["anthropic-beta"]`를 명시적으로 설정하세요.
