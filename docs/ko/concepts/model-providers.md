...
read_when:
    ...
    - 모델 제공자를 위한 예시 구성 또는 CLI 온보딩 명령을 원합니다
sidebarTitle: Model providers
summary: 예시 구성과 CLI 흐름을 포함한 모델 제공자 개요
title: 모델 제공업체
x-i18n:
    generated_at: "2026-05-03T21:30:05Z"
    model: gpt-5.5
    provider: openai
...
LLM/모델 제공업체(WhatsApp/Telegram 같은 채팅 채널이 아님)에 대한 참고 자료입니다. 모델 선택 규칙은 [모델](/ko/concepts/models)을 참조하세요.
...
<AccordionGroup>
  <Accordion title="모델 참조와 CLI 도우미">
    ...
    - `models.providers.*.contextWindow` / `contextTokens` / `maxTokens`는 제공업체 수준 기본값을 설정합니다. `models.providers.*.models[].contextWindow` / `contextTokens` / `maxTokens`는 모델별로 이를 재정의합니다.
    - 폴백 규칙, 쿨다운 프로브, 세션 재정의 지속성: [모델 장애 조치](/ko/concepts/model-failover).

  </Accordion>
  <Accordion title="제공업체 인증을 추가해도 기본 모델은 바뀌지 않습니다">
    `openclaw configure`는 제공업체를 추가하거나 재인증할 때 기존 `agents.defaults.model.primary`를 보존합니다. 제공업체 Plugin은 인증 구성 패치에서 권장 기본 모델을 반환할 수 있지만, 기본 모델이 이미 있는 경우 configure는 이를 "이 모델을 사용할 수 있게 함"으로 처리하며 "현재 기본 모델 교체"로 처리하지 않습니다.

    기본 모델을 의도적으로 전환하려면 `openclaw models set <provider/model>` 또는 `openclaw models auth login --provider <id> --set-default`를 사용하세요.

  </Accordion>
  <Accordion title="OpenAI 제공업체/런타임 분리">
    OpenAI 계열 경로는 접두사별로 구분됩니다.

    - `openai/<model>`에 `agents.defaults.agentRuntime.id: "codex"`를 함께 사용하면 네이티브 Codex 앱 서버 하네스를 사용합니다. 이는 일반적인 ChatGPT/Codex 구독 설정입니다.
    - `openai-codex/<model>`은 PI에서 Codex OAuth를 사용합니다.
    - Codex 런타임 재정의 없이 `openai/<model>`을 사용하면 PI의 직접 OpenAI API 키 제공업체를 사용합니다.

    [OpenAI](/ko/providers/openai)와 [Codex 하네스](/ko/plugins/codex-harness)를 참조하세요. 제공업체/런타임 분리가 혼란스럽다면 먼저 [에이전트 런타임](/ko/concepts/agent-runtimes)을 읽으세요.
    ...
    GPT-5.5는 `agentRuntime.id: "codex"`가 설정된 경우 네이티브 Codex 앱 서버 하네스를 통해 사용할 수 있고, Codex OAuth용 PI에서는 `openai-codex/gpt-5.5`로, 계정에서 노출되는 경우 직접 API 키 트래픽용 PI에서는 `openai/gpt-5.5`로 사용할 수 있습니다.
  ...
  <Accordion title="CLI 런타임">
    ...
    레거시 `claude-cli/*`, `google-gemini-cli/*`, `codex-cli/*` 참조는 런타임을 별도로 기록한 상태로 표준 제공업체 참조로 다시 마이그레이션됩니다.
...
대부분의 제공업체별 로직은 제공업체 Plugin(`registerProvider(...)`)에 있고, OpenClaw는 일반 추론 루프를 유지합니다. Plugin은 온보딩, 모델 카탈로그, 인증 환경 변수 매핑, 전송/구성 정규화, 도구 스키마 정리, 장애 조치 분류, OAuth 갱신, 사용량 보고, 사고/추론 프로필 등을 소유합니다.

제공업체 SDK 훅과 번들 Plugin 예제의 전체 목록은 [제공업체 Plugin](/ko/plugins/sdk-provider-plugins)에 있습니다. 완전히 사용자 지정 요청 실행기가 필요한 제공업체는 별도의 더 깊은 확장 표면입니다.

<Note>
제공업체 소유 러너 동작은 재생 정책, 도구 스키마 정규화, 스트림 래핑, 전송/요청 도우미 같은 명시적 제공업체 훅에 있습니다. 레거시 `ProviderPlugin.capabilities` 정적 백은 호환성 전용이며 공유 러너 로직에서 더 이상 읽지 않습니다.
</Note>
...
<AccordionGroup>
  <Accordion title="키 소스와 우선순위">
    ...
    Google 제공업체의 경우 `GOOGLE_API_KEY`도 폴백으로 포함됩니다. 키 선택 순서는 우선순위를 보존하고 값을 중복 제거합니다.

  </Accordion>
  <Accordion title="교체가 시작되는 시점">
    ...
    - 속도 제한이 아닌 실패는 즉시 실패합니다. 키 교체는 시도되지 않습니다.
...
OpenClaw에는 pi-ai 카탈로그가 포함되어 제공됩니다. 이러한 제공업체에는 `models.providers` 구성이 **필요하지 않습니다**. 인증을 설정하고 모델을 선택하기만 하면 됩니다.

### OpenAI

- 제공업체: `openai`
- 인증: `OPENAI_API_KEY`
- 선택적 교체: `OPENAI_API_KEYS`, `OPENAI_API_KEY_1`, `OPENAI_API_KEY_2`, 그리고 `OPENCLAW_LIVE_OPENAI_KEY`(단일 재정의)
...
- OpenAI 우선 처리(priority processing)는 `agents.defaults.models["openai/<model>"].params.serviceTier`로 활성화할 수 있습니다.
...
- 숨겨진 OpenClaw 귀속 헤더(`originator`, `version`, `User-Agent`)는 `api.openai.com`에 대한 네이티브 OpenAI 트래픽에만 적용되며, 일반 OpenAI 호환 프록시에는 적용되지 않습니다.
- 네이티브 OpenAI 경로는 Responses `store`, 프롬프트 캐시 힌트, OpenAI 추론 호환 페이로드 형태 조정도 유지합니다. 프록시 경로는 그렇지 않습니다.
- `openai/gpt-5.3-codex-spark`는 라이브 OpenAI API 요청에서 거부되고 현재 Codex 카탈로그가 노출하지 않으므로 OpenClaw에서 의도적으로 억제됩니다.
...
- 선택적 교체: `ANTHROPIC_API_KEYS`, `ANTHROPIC_API_KEY_1`, `ANTHROPIC_API_KEY_2`, 그리고 `OPENCLAW_LIVE_ANTHROPIC_KEY`(단일 재정의)
...
<Note>
Anthropic 직원이 OpenClaw 스타일의 Claude CLI 사용이 다시 허용된다고 알려왔으므로, Anthropic이 새 정책을 게시하지 않는 한 OpenClaw는 이 통합에서 Claude CLI 재사용과 `claude -p` 사용을 승인된 것으로 처리합니다. Anthropic setup-token은 지원되는 OpenClaw 토큰 경로로 계속 사용할 수 있지만, 이제 OpenClaw는 사용 가능한 경우 Claude CLI 재사용과 `claude -p`를 우선합니다.
</Note>
...
- `params.serviceTier`도 네이티브 Codex Responses 요청(`chatgpt.com/backend-api`)으로 전달됩니다.
- 숨겨진 OpenClaw 귀속 헤더(`originator`, `version`, `User-Agent`)는 `chatgpt.com/backend-api`에 대한 네이티브 Codex 트래픽에만 첨부되며, 일반 OpenAI 호환 프록시에는 첨부되지 않습니다.
- 직접 `openai/*`와 같은 `/fast` 토글 및 `params.fastMode` 구성을 공유합니다. OpenClaw는 이를 `service_tier=priority`로 매핑합니다.
...
<CardGroup cols={3}>
  ...
  <Card title="Qwen Cloud" href="/ko/providers/qwen">
    Qwen Cloud 제공업체 표면과 Alibaba DashScope 및 Coding Plan 엔드포인트 매핑입니다.
  </Card>
</CardGroup>
...
- 공급자: `google`
- 인증: `GEMINI_API_KEY`
- 선택적 로테이션: `GEMINI_API_KEYS`, `GEMINI_API_KEY_1`, `GEMINI_API_KEY_2`, `GOOGLE_API_KEY` 폴백, 및 `OPENCLAW_LIVE_GEMINI_KEY`(단일 재정의)
...
<Warning>
OpenClaw의 Gemini CLI OAuth는 비공식 통합입니다. 일부 사용자는 타사 클라이언트를 사용한 후 Google 계정 제한을 보고했습니다. 진행하기로 선택한 경우 Google 약관을 검토하고 중요하지 않은 계정을 사용하세요.
</Warning>
...
- 정적 폴백 카탈로그는 `kilocode/kilo/auto`를 제공합니다. 라이브 `https://api.kilo.ai/api/gateway/models` 검색은 런타임 카탈로그를 더 확장할 수 있습니다.
...
<AccordionGroup>
  <Accordion title="OpenRouter">
    확인된 `openrouter.ai` 경로에만 앱 귀속 헤더와 Anthropic `cache_control` 마커를 적용합니다. DeepSeek, Moonshot, ZAI 참조는 OpenRouter 관리형 프롬프트 캐싱에서 캐시 TTL 대상이지만 Anthropic 캐시 마커는 받지 않습니다. 프록시 스타일의 OpenAI 호환 경로이므로 네이티브 OpenAI 전용 셰이핑(`serviceTier`, Responses `store`, 프롬프트 캐시 힌트, OpenAI reasoning 호환성)은 건너뜁니다. Gemini 기반 참조는 프록시 Gemini 사고 서명 정리만 유지합니다.
  </Accordion>
  <Accordion title="Kilo Gateway">
    Gemini 기반 참조는 동일한 프록시 Gemini 정리 경로를 따릅니다. `kilocode/kilo/auto` 및 기타 프록시 reasoning 미지원 참조는 프록시 reasoning 주입을 건너뜁니다.
  </Accordion>
  <Accordion title="MiniMax">
    API 키 온보딩은 명시적인 텍스트 전용 M2.7 채팅 모델 정의를 작성합니다. 이미지 이해는 Plugin 소유 `MiniMax-VL-01` 미디어 제공자에 유지됩니다.
  </Accordion>
  <Accordion title="NVIDIA">
    모델 ID는 `nvidia/<vendor>/<model>` 네임스페이스를 사용합니다(예: `nvidia/moonshotai/kimi-k2.5`와 함께 `nvidia/nvidia/nemotron-...`). 선택기는 리터럴 `<provider>/<model-id>` 구성을 보존하지만, API로 전송되는 정규 키는 단일 접두사만 유지합니다.
  </Accordion>
  <Accordion title="xAI">
    xAI Responses 경로를 사용합니다. `grok-4.3`은 번들 기본 채팅 모델입니다. `/fast` 또는 `params.fastMode: true`는 `grok-3`, `grok-3-mini`, `grok-4`, `grok-4-0709`를 각각의 `*-fast` 변형으로 다시 씁니다. `tool_stream`은 기본적으로 켜져 있습니다. `agents.defaults.models["xai/<model>"].params.tool_stream=false`로 비활성화하세요.
  </Accordion>
...
`models.providers`(또는 `models.json`)를 사용해 **사용자 지정** 제공자 또는 OpenAI/Anthropic 호환 프록시를 추가하세요.

아래의 번들 제공자 Plugin 중 다수는 이미 기본 카탈로그를 게시합니다. 기본 기본 URL, 헤더 또는 모델 목록을 재정의하려는 경우에만 명시적인 `models.providers.<id>` 항목을 사용하세요.

Gateway 모델 기능 검사도 명시적인 `models.providers.<id>.models[]` 메타데이터를 읽습니다. 사용자 지정 또는 프록시 모델이 이미지를 허용하는 경우, 해당 모델에 `input: ["text", "image"]`를 설정하여 WebChat과 노드 출처 첨부 파일 경로가 이미지를 텍스트 전용 미디어 참조 대신 네이티브 모델 입력으로 전달하도록 하세요.
...
온보딩/구성 모델 선택기에서 Volcengine 인증 선택은 `volcengine/*` 및 `volcengine-plan/*` 행을 모두 우선합니다. 해당 모델이 아직 로드되지 않은 경우 OpenClaw는 빈 제공자 범위 선택기를 표시하는 대신 필터링되지 않은 카탈로그로 폴백합니다.
...
온보딩/구성 모델 선택기에서 BytePlus 인증 선택은 `byteplus/*` 및 `byteplus-plan/*` 행을 모두 우선합니다. 해당 모델이 아직 로드되지 않은 경우 OpenClaw는 빈 제공자 범위 선택기를 표시하는 대신 필터링되지 않은 카탈로그로 폴백합니다.
...
- MiniMax OAuth (전역): `--auth-choice minimax-global-oauth`
- MiniMax OAuth (CN): `--auth-choice minimax-cn-oauth`
- MiniMax API 키 (전역): `--auth-choice minimax-global-api`
...
OpenClaw는 발견 및 자동 로드에 LM Studio의 네이티브 `/api/v1/models` 및 `/api/v1/models/load`를 사용하고, 기본적으로 추론에는 `/v1/chat/completions`를 사용합니다. LM Studio JIT 로딩, TTL, 자동 제거가 모델 수명 주기를 소유하게 하려면 `models.providers.lmstudio.params.preload: false`를 설정하세요. 설정 및 문제 해결은 [/providers/lmstudio](/ko/providers/lmstudio)를 참조하세요.
...
로컬에서 자동 발견에 옵트인하려면(서버가 인증을 강제하지 않는 경우 아무 값이나 동작함):
...
<AccordionGroup>
  <Accordion title="기본 선택 필드">
    사용자 지정 제공자의 경우 `reasoning`, `input`, `cost`, `contextWindow`, `maxTokens`는 선택 사항입니다. 생략하면 OpenClaw는 기본값으로 다음을 사용합니다.
  ...
  <Accordion title="프록시 경로 형태 지정 규칙">
    - 네이티브가 아닌 엔드포인트(호스트가 `api.openai.com`이 아닌, 비어 있지 않은 `baseUrl`)에서 `api: "openai-completions"`를 사용하는 경우, OpenClaw는 지원되지 않는 `developer` 역할로 인한 제공자 400 오류를 피하기 위해 `compat.supportsDeveloperRole: false`를 강제합니다.
    - 프록시 스타일 OpenAI 호환 경로는 네이티브 OpenAI 전용 요청 형태 지정도 건너뜁니다. `service_tier` 없음, Responses `store` 없음, Completions `store` 없음, 프롬프트 캐시 힌트 없음, OpenAI reasoning 호환 페이로드 형태 지정 없음, 숨겨진 OpenClaw 속성 헤더 없음.
    - 벤더별 필드가 필요한 OpenAI 호환 Completions 프록시의 경우, 발신 요청 본문에 추가 JSON을 병합하려면 `agents.defaults.models["provider/model"].params.extra_body`(또는 `extraBody`)를 설정하세요.
    ...
    - 느린 로컬 모델 또는 원격 LAN/tailnet 호스트의 경우 `models.providers.<id>.timeoutSeconds`를 설정하세요. 이는 전체 에이전트 런타임 타임아웃을 늘리지 않고, 연결, 헤더, 본문 스트리밍, 전체 보호된 fetch 중단을 포함한 제공자 모델 HTTP 요청 처리를 확장합니다.
    - 모델 제공자 HTTP 호출은 구성된 제공자 `baseUrl` 호스트 이름에 대해서만 `198.18.0.0/15` 및 `fc00::/7`의 Surge, Clash, sing-box fake-IP DNS 응답을 허용합니다. 다른 사설, loopback, link-local, 메타데이터 대상은 여전히 명시적 `models.providers.<id>.request.allowPrivateNetwork: true` 옵트인이 필요합니다.
    - `baseUrl`이 비어 있거나 생략된 경우 OpenClaw는 기본 OpenAI 동작을 유지합니다(이는 `api.openai.com`으로 확인됨).
    - 안전을 위해 명시적 `compat.supportsDeveloperRole: true`도 네이티브가 아닌 `openai-completions` 엔드포인트에서는 여전히 재정의됩니다.
    - 직접 엔드포인트가 아닌 곳(정식 `anthropic`이 아닌 모든 제공자, 또는 호스트가 공개 `api.anthropic.com` 엔드포인트가 아닌 사용자 지정 `models.providers.anthropic.baseUrl`)에서 `api: "anthropic-messages"`를 사용하는 경우, OpenClaw는 `claude-code-20250219`, `interleaved-thinking-2025-05-14`, OAuth 마커와 같은 암시적 Anthropic beta 헤더를 억제하여 사용자 지정 Anthropic 호환 프록시가 지원되지 않는 beta 플래그를 거부하지 않도록 합니다. 프록시에 특정 beta 기능이 필요한 경우 `models.providers.<id>.headers["anthropic-beta"]`를 명시적으로 설정하세요.
