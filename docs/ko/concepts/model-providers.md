---
read_when:
    - 제공자별 모델 설정 참조 문서가 필요합니다
    - 모델 제공자를 위한 예시 구성 또는 CLI 온보딩 명령이 필요한 경우
sidebarTitle: Model providers
summary: 예제 구성 + CLI 흐름이 포함된 모델 제공자 개요
title: 모델 제공자
x-i18n:
    generated_at: "2026-05-02T20:48:03Z"
    model: gpt-5.5
    provider: openai
...
LLM/모델 제공자에 대한 참조입니다(WhatsApp/Telegram 같은 채팅 채널이 아님). 모델 선택 규칙은 [모델](/ko/concepts/models)을 참조하세요.
...
<AccordionGroup>
  <Accordion title="모델 참조와 CLI 도우미">
    ...
    - 대체 규칙, 쿨다운 프로브, 세션 재정의 유지: [모델 장애 조치](/ko/concepts/model-failover).

  </Accordion>
  <Accordion title="제공자 인증 추가는 기본 모델을 변경하지 않음">
    `openclaw configure`는 제공자를 추가하거나 재인증할 때 기존 `agents.defaults.model.primary`를 보존합니다. 제공자 Plugin이 인증 구성 패치에서 권장 기본 모델을 반환할 수는 있지만, 기본 모델이 이미 있으면 configure는 이를 "이 모델을 사용할 수 있게 함"으로 처리하며, "현재 기본 모델을 교체함"으로 처리하지 않습니다.
  ...
  <Accordion title="OpenAI 제공자/런타임 분리">
    OpenAI 계열 경로는 접두사별로 구분됩니다.

    - `openai/<model>`에 `agents.defaults.agentRuntime.id: "codex"`를 더하면 네이티브 Codex 앱 서버 하네스를 사용합니다. 이는 일반적인 ChatGPT/Codex 구독 설정입니다.
    - `openai-codex/<model>`은 PI에서 Codex OAuth를 사용합니다.
    - Codex 런타임 재정의가 없는 `openai/<model>`은 PI에서 직접 OpenAI API 키 제공자를 사용합니다.

    [OpenAI](/ko/providers/openai)와 [Codex 하네스](/ko/plugins/codex-harness)를 참조하세요. 제공자/런타임 분리가 혼란스럽다면 먼저 [에이전트 런타임](/ko/concepts/agent-runtimes)을 읽으세요.
    ...
    GPT-5.5는 `agentRuntime.id: "codex"`가 설정된 경우 네이티브 Codex 앱 서버 하네스를 통해 사용할 수 있고, Codex OAuth용 PI에서는 `openai-codex/gpt-5.5`를 통해, 계정에서 노출하는 경우 직접 API 키 트래픽용 PI에서는 `openai/gpt-5.5`를 통해 사용할 수 있습니다.
  ...
  <Accordion title="CLI 런타임">
    ...
    레거시 `claude-cli/*`, `google-gemini-cli/*`, `codex-cli/*` 참조는 런타임이 별도로 기록된 표준 제공자 참조로 다시 마이그레이션됩니다.
...
대부분의 제공자별 로직은 제공자 Plugin(`registerProvider(...)`)에 있으며, OpenClaw는 일반 추론 루프를 유지합니다. Plugin은 온보딩, 모델 카탈로그, 인증 환경 변수 매핑, 전송/구성 정규화, 도구 스키마 정리, 장애 조치 분류, OAuth 새로고침, 사용량 보고, 사고/추론 프로필 등을 소유합니다.

제공자 SDK 훅과 번들 Plugin 예제의 전체 목록은 [제공자 Plugin](/ko/plugins/sdk-provider-plugins)에 있습니다. 완전히 맞춤형 요청 실행기가 필요한 제공자는 별도의 더 깊은 확장 표면입니다.

<Note>
제공자 소유 러너 동작은 리플레이 정책, 도구 스키마 정규화, 스트림 래핑, 전송/요청 도우미 같은 명시적 제공자 훅에 있습니다. 레거시 `ProviderPlugin.capabilities` 정적 백은 호환성 전용이며 더 이상 공유 러너 로직에서 읽지 않습니다.
</Note>
...
OpenClaw는 pi-ai 카탈로그와 함께 제공됩니다. 이러한 제공자는 `models.providers` 구성이 **필요하지 않습니다**. 인증을 설정하고 모델만 선택하면 됩니다.
...
- 특정 설치나 API 키가 다르게 동작하면 `openclaw models list --provider openai`로 계정/모델 가용성을 확인하세요.
...
- OpenAI Responses WebSocket 워밍업은 `params.openaiWsWarmup`(`true`/`false`)를 통해 기본적으로 활성화됩니다.
...
- 공유 `/fast` 토글 대신 명시적 티어를 원할 때 `params.serviceTier`를 사용하세요.
- 숨겨진 OpenClaw 기여 표시 헤더(`originator`, `version`, `User-Agent`)는 일반 OpenAI 호환 프록시가 아니라 `api.openai.com`으로 가는 네이티브 OpenAI 트래픽에만 적용됩니다.
- 네이티브 OpenAI 경로는 Responses `store`, 프롬프트 캐시 힌트, OpenAI 추론 호환 페이로드 구성도 유지합니다. 프록시 경로는 그렇지 않습니다.
- `openai/gpt-5.3-codex-spark`는 라이브 OpenAI API 요청에서 거부되고 현재 Codex 카탈로그가 이를 노출하지 않기 때문에 OpenClaw에서 의도적으로 억제됩니다.
...
- 권장 Claude CLI 구성은 모델 참조를 표준으로 유지하고 CLI
  백엔드를 별도로 선택합니다. `anthropic/claude-opus-4-7`과
  `agents.defaults.agentRuntime.id: "claude-cli"`를 함께 사용하세요. 레거시
  `claude-cli/claude-opus-4-7` 참조도 호환성을 위해 계속 작동합니다.

<Note>
Anthropic 직원은 OpenClaw 스타일 Claude CLI 사용이 다시 허용된다고 알려왔으므로, Anthropic이 새 정책을 게시하지 않는 한 OpenClaw는 이 통합에서 Claude CLI 재사용과 `claude -p` 사용을 승인된 것으로 취급합니다. Anthropic 설정 토큰은 지원되는 OpenClaw 토큰 경로로 계속 사용할 수 있지만, OpenClaw는 이제 가능할 때 Claude CLI 재사용과 `claude -p`를 선호합니다.
</Note>
...
- 숨겨진 OpenClaw 기여 표시 헤더(`originator`, `version`, `User-Agent`)는 일반 OpenAI 호환 프록시가 아니라 `chatgpt.com/backend-api`로 가는 네이티브 Codex 트래픽에만 첨부됩니다.
...
- 정책 참고: OpenAI Codex OAuth는 OpenClaw 같은 외부 도구/워크플로에서 명시적으로 지원됩니다.
- 일반적인 구독 및 네이티브 Codex 런타임 경로의 경우 `openai-codex` 인증으로 로그인하되, `openai/gpt-5.5`와 `agents.defaults.agentRuntime.id: "codex"`를 함께 구성하세요.
- PI를 통한 Codex OAuth/구독 경로를 원할 때만 `openai-codex/gpt-5.5`를 사용하세요. API 키 설정과 로컬 카탈로그가 공개 API 경로를 노출할 때는 Codex 런타임 재정의 없이 `openai/gpt-5.5`를 사용하세요.
...
<CardGroup cols={3}>
  ...
  <Card title="MiniMax" href="/ko/providers/minimax">
    MiniMax Coding Plan OAuth 또는 API 키 접근.
  </Card>
...
- 직접 Gemini 실행은 공급자 네이티브 `cachedContents/...` 핸들을 전달하기 위해 `agents.defaults.models["google/<model>"].params.cachedContent`(또는 기존 `cached_content`)도 허용합니다. Gemini 캐시 적중은 OpenClaw `cacheRead`로 표시됩니다
...
<Warning>
OpenClaw의 Gemini CLI OAuth는 비공식 통합입니다. 일부 사용자는 타사 클라이언트 사용 후 Google 계정 제한을 보고했습니다. 계속 진행하기로 선택한 경우 Google 약관을 검토하고 중요하지 않은 계정을 사용하세요.
</Warning>

Gemini CLI OAuth는 기본 제공 `google` Plugin의 일부로 제공됩니다.
...
<Steps>
  ...
  <Step title="로그인">
    ...
    기본 모델: `google-gemini-cli/gemini-3-flash-preview`. `openclaw.json`에 클라이언트 ID나 비밀 값을 붙여넣지 **않습니다**. CLI 로그인 흐름은 Gateway 호스트의 인증 프로필에 토큰을 저장합니다.
...
- CLI: `openclaw onboard --auth-choice zai-api-key`
  ...
  - `zai-api-key`는 일치하는 Z.AI 엔드포인트를 자동 감지합니다. `zai-coding-global`, `zai-coding-cn`, `zai-global`, `zai-cn`은 특정 표면을 강제로 사용합니다
...
- 정적 대체 카탈로그는 `kilocode/kilo/auto`를 제공합니다. 라이브 `https://api.kilo.ai/api/gateway/models` 탐색은 런타임 카탈로그를 더 확장할 수 있습니다.
- `kilocode/kilo/auto` 뒤의 정확한 업스트림 라우팅은 OpenClaw에 하드코딩된 것이 아니라 Kilo Gateway가 소유합니다.

설정 세부 정보는 [/providers/kilocode](/ko/providers/kilocode)를 참조하세요.

### 기본 제공되는 기타 공급자 Plugin
...
<AccordionGroup>
  <Accordion title="OpenRouter">
    검증된 `openrouter.ai` 경로에서만 앱 속성 헤더와 Anthropic `cache_control` 마커를 적용합니다. DeepSeek, Moonshot, ZAI 참조는 OpenRouter 관리형 프롬프트 캐싱의 cache-TTL 대상이지만 Anthropic 캐시 마커는 받지 않습니다. 프록시 스타일의 OpenAI 호환 경로이므로 네이티브 OpenAI 전용 셰이핑(`serviceTier`, Responses `store`, 프롬프트 캐시 힌트, OpenAI reasoning 호환성)은 건너뜁니다. Gemini 기반 참조는 프록시 Gemini thought-signature 정리만 유지합니다.
  </Accordion>
  <Accordion title="Kilo Gateway">
    Gemini 기반 참조는 동일한 프록시 Gemini 정리 경로를 따릅니다. `kilocode/kilo/auto` 및 기타 프록시 reasoning 미지원 참조는 프록시 reasoning 주입을 건너뜁니다.
  </Accordion>
  <Accordion title="MiniMax">
    API 키 온보딩은 명시적인 텍스트 전용 M2.7 채팅 모델 정의를 작성합니다. 이미지 이해는 Plugin 소유 `MiniMax-VL-01` 미디어 provider에 유지됩니다.
  </Accordion>
  <Accordion title="NVIDIA">
    모델 ID는 `nvidia/<vendor>/<model>` 네임스페이스를 사용합니다(예: `nvidia/moonshotai/kimi-k2.5`와 함께 `nvidia/nvidia/nemotron-...`). 선택기는 리터럴 `<provider>/<model-id>` 구성을 보존하고, API로 전송되는 정식 키는 단일 접두사 상태로 유지됩니다.
  </Accordion>
  <Accordion title="xAI">
  ...
  <Accordion title="Cerebras">
    번들 `cerebras` provider Plugin으로 제공됩니다. GLM은 `zai-glm-4.7`을 사용하며, OpenAI 호환 base URL은 `https://api.cerebras.ai/v1`입니다.
  </Accordion>
</AccordionGroup>

## `models.providers`를 통한 provider(커스텀/base URL)

`models.providers`(또는 `models.json`)를 사용하여 **커스텀** provider 또는 OpenAI/Anthropic 호환 프록시를 추가하세요.

아래의 많은 번들 provider Plugin은 이미 기본 카탈로그를 게시합니다. 기본 base URL, 헤더 또는 모델 목록을 재정의하려는 경우에만 명시적인 `models.providers.<id>` 항목을 사용하세요.

Gateway 모델 기능 검사도 명시적인 `models.providers.<id>.models[]` 메타데이터를 읽습니다. 커스텀 또는 프록시 모델이 이미지를 허용하는 경우 WebChat과 node-origin 첨부 경로가 이미지를 텍스트 전용 미디어 참조 대신 네이티브 모델 입력으로 전달하도록 해당 모델에 `input: ["text", "image"]`를 설정하세요.

### Moonshot AI (Kimi)

Moonshot은 번들 provider Plugin으로 제공됩니다. 기본적으로 내장 provider를 사용하고, base URL 또는 모델 메타데이터를 재정의해야 할 때만 명시적인 `models.providers.moonshot` 항목을 추가하세요.
...
온보딩/구성 모델 선택기에서 Volcengine 인증 선택은 `volcengine/*` 및 `volcengine-plan/*` 행을 모두 선호합니다. 해당 모델들이 아직 로드되지 않은 경우 OpenClaw는 비어 있는 제공자 범위 선택기를 표시하는 대신 필터링되지 않은 카탈로그로 대체합니다.
...
온보딩/구성 모델 선택기에서 BytePlus 인증 선택은 `byteplus/*` 및 `byteplus-plan/*` 행을 모두 선호합니다. 해당 모델들이 아직 로드되지 않은 경우 OpenClaw는 비어 있는 제공자 범위 선택기를 표시하는 대신 필터링되지 않은 카탈로그로 대체합니다.
...
<Note>
MiniMax의 Anthropic 호환 스트리밍 경로에서 OpenClaw는 명시적으로 설정하지 않는 한 기본적으로 사고를 비활성화하며, `/fast on`은 `MiniMax-M2.7`을 `MiniMax-M2.7-highspeed`로 다시 작성합니다.
</Note>
...
OpenClaw는 기본적으로 추론에 `/v1/chat/completions`를 사용하며, 발견 및 자동 로드에는 LM Studio의 네이티브 `/api/v1/models` 및 `/api/v1/models/load`를 사용합니다. LM Studio JIT 로딩, TTL, 자동 제거가 모델 수명 주기를 소유하게 하려면 `models.providers.lmstudio.params.preload: false`를 설정하세요. 설정 및 문제 해결은 [/providers/lmstudio](/ko/providers/lmstudio)를 참조하세요.
...
Ollama는 `OLLAMA_API_KEY`로 옵트인하면 `http://127.0.0.1:11434`에서 로컬로 감지되며, 번들 제공자 Plugin은 Ollama를 `openclaw onboard`와 모델 선택기에 직접 추가합니다. 온보딩, 클라우드/로컬 모드, 사용자 지정 구성은 [/providers/ollama](/ko/providers/ollama)를 참조하세요.
...
로컬 자동 발견에 옵트인하려면(서버가 인증을 강제하지 않는 경우 어떤 값이든 작동):
...
<AccordionGroup>
  <Accordion title="기본 선택 필드">
    사용자 지정 제공자의 경우 `reasoning`, `input`, `cost`, `contextWindow`, `maxTokens`는 선택 사항입니다. 생략하면 OpenClaw는 기본적으로 다음을 사용합니다.
  ...
  <Accordion title="프록시 라우트 구성 규칙">
    - 비네이티브 엔드포인트(호스트가 `api.openai.com`이 아닌 비어 있지 않은 모든 `baseUrl`)에서 `api: "openai-completions"`를 사용하는 경우, OpenClaw는 지원되지 않는 `developer` 역할로 인한 제공자 400 오류를 피하기 위해 `compat.supportsDeveloperRole: false`를 강제합니다.
    - 프록시 스타일 OpenAI 호환 라우트는 네이티브 OpenAI 전용 요청 구성도 건너뜁니다. `service_tier` 없음, Responses `store` 없음, Completions `store` 없음, 프롬프트 캐시 힌트 없음, OpenAI reasoning 호환 페이로드 구성 없음, 숨겨진 OpenClaw 어트리뷰션 헤더 없음.
    - 공급업체별 필드가 필요한 OpenAI 호환 Completions 프록시의 경우, `agents.defaults.models["provider/model"].params.extra_body`(또는 `extraBody`)를 설정하여 추가 JSON을 아웃바운드 요청 본문에 병합하세요.
    - vLLM 채팅 템플릿 컨트롤의 경우 `agents.defaults.models["provider/model"].params.chat_template_kwargs`를 설정하세요. 번들 vLLM Plugin은 세션 사고 수준이 꺼져 있을 때 `vllm/nemotron-3-*`에 대해 `enable_thinking: false` 및 `force_nonempty_content: true`를 자동으로 전송합니다.
    - 느린 로컬 모델 또는 원격 LAN/tailnet 호스트의 경우 `models.providers.<id>.timeoutSeconds`를 설정하세요. 이렇게 하면 전체 에이전트 런타임 제한 시간을 늘리지 않고도 연결, 헤더, 본문 스트리밍, 전체 보호된 fetch 중단을 포함한 제공자 모델 HTTP 요청 처리가 확장됩니다.
    - `baseUrl`이 비어 있거나 생략된 경우 OpenClaw는 기본 OpenAI 동작(`api.openai.com`으로 해석됨)을 유지합니다.
    - 안전을 위해, 비네이티브 `openai-completions` 엔드포인트에서는 명시적인 `compat.supportsDeveloperRole: true`도 여전히 재정의됩니다.
    - 비직접 엔드포인트(정식 `anthropic`이 아닌 모든 제공자, 또는 호스트가 공개 `api.anthropic.com` 엔드포인트가 아닌 사용자 지정 `models.providers.anthropic.baseUrl`)에서 `api: "anthropic-messages"`를 사용하는 경우, OpenClaw는 `claude-code-20250219`, `interleaved-thinking-2025-05-14`, OAuth 마커 같은 암시적 Anthropic 베타 헤더를 억제하여 사용자 지정 Anthropic 호환 프록시가 지원되지 않는 베타 플래그를 거부하지 않도록 합니다. 프록시에 특정 베타 기능이 필요한 경우 `models.providers.<id>.headers["anthropic-beta"]`를 명시적으로 설정하세요.
