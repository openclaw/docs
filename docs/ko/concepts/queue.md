...
title: 명령 큐
x-i18n:
    generated_at: "2026-05-06T06:22:52Z"
    model: gpt-5.5
    provider: openai
...
`steer`는 활성 모델 턴의 응답성을 유지하면서 두 번째 세션 실행을
...
응답을 하나만 원한다면 `collect`/`steer`를 선호하세요.

런타임별 타이밍과 의존성 동작은
[스티어링 큐](/ko/concepts/queue-steering)를 참조하세요. 명시적인 `/steer <message>`
명령은 [스티어링](/ko/tools/steer)을 참조하세요.
...
적용됩니다. `cap`과 `drop`은 전역/세션 옵션이며, 채널별 구성
키가 아닙니다.
