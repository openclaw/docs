...
x-i18n:
    generated_at: "2026-05-06T06:23:10Z"
    model: gpt-5.5
    provider: openai
...
OpenClaw는 일반 세션에서 이를 주입하므로 실제로 큰 영향을 미칩니다. 에이전트가
밋밋하거나, 애매하게 말하거나, 이상하게 기업체 같은 말투를 쓴다면 보통 이 파일을 고치면 됩니다.
...
- 프롬프트 엔지니어링 가이드는 고수준 행동, 어조, 목표, 예시는
  사용자 턴에 묻어두는 것이 아니라 우선순위가 높은 지시 계층에 있어야 한다고 말합니다.
- 같은 가이드는 프롬프트를 한 번 쓰고 잊어버리는 마법 같은 문장이 아니라,
  반복 개선하고, 고정하고, 평가하는 대상으로 다루라고 권장합니다.

OpenClaw에서 `SOUL.md`가 바로 그 계층입니다.

더 나은 개성을 원한다면 더 강한 지시를 작성하세요. 안정적인
개성을 원한다면 지시를 간결하게 유지하고 버전을 관리하세요.
...
운영 규칙은 `AGENTS.md`에 두세요. 목소리, 입장, 스타일은 `SOUL.md`에 두세요.
에이전트가 공유 채널, 공개 답변, 고객 접점에서 작동한다면
그 어조가 여전히 상황에 맞는지 확인하세요.
...
<CardGroup cols={2}>
  <Card title="에이전트 워크스페이스" href="/ko/concepts/agent-workspace" icon="folder-open">
    OpenClaw가 시스템 프롬프트에 주입하는 워크스페이스 파일입니다.
  </Card>
  <Card title="시스템 프롬프트" href="/ko/concepts/system-prompt" icon="message-lines">
    `SOUL.md`가 각 턴의 시스템 프롬프트에 구성되는 방식입니다.
  </Card>
