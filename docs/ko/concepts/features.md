---
read_when:
    - OpenClaw가 지원하는 항목의 전체 목록을 원합니다
summary: 채널, 라우팅, 미디어, UX 전반에 걸친 OpenClaw 기능.
title: 기능
x-i18n:
    generated_at: "2026-05-06T06:20:58Z"
    model: gpt-5.5
    provider: openai
...
<Columns>
  ...
  <Card title="Plugin" icon="plug" href="/ko/tools/plugin">
    일반적인 최신 릴리스에서는 별도 설치 없이 번들 Plugin으로 Matrix, Nextcloud Talk, Nostr, Twitch, Zalo 등을 추가할 수 있습니다.
  </Card>
...
- 기본 제공 채널에는 Discord, Google Chat, iMessage(레거시), IRC, Signal, Slack, Telegram, WebChat, WhatsApp이 포함됩니다.
...
- 선택적으로 별도 설치하는 채널 Plugin에는 Voice Call과 WeChat 같은 타사 패키지가 포함됩니다.
...
- 세션: 직접 채팅은 공유 `main`으로 통합되고, 그룹은 격리됩니다.
- 긴 응답을 위한 스트리밍 및 청킹
...
- 사용자 지정 및 셀프 호스팅 제공자 지원(vLLM, SGLang, Ollama 및 모든 OpenAI 호환 또는 Anthropic 호환 엔드포인트)
...
<CardGroup cols={2}>
  ...
  <Card title="Plugin" href="/ko/tools/plugin" icon="plug">
    OpenClaw를 확장하는 번들 및 타사 Plugin입니다.
  </Card>
</CardGroup>
