---
read_when:
    - OpenClaw이 지원하는 전체 목록을 확인하려는 경우
summary: 채널, 라우팅, 미디어 및 사용자 경험 전반의 OpenClaw 기능.
title: 기능
x-i18n:
    generated_at: "2026-07-12T00:40:38Z"
    model: gpt-5.6
...
<Columns>
  ...
  <Card title="Plugin" icon="plug" href="/ko/tools/plugin">
    공식 Plugin은 한 번의 설치 명령으로 Matrix, Nextcloud Talk, Nostr, Twitch, Zalo를 비롯한 수십 가지 서비스를 추가합니다.
  </Card>
  ...
  <Card title="앱 및 UI" icon="monitor" href="/ko/platforms">
    Windows Hub, 브라우저 Control UI, macOS 메뉴 막대 앱 및 모바일 Node를 제공합니다.
  </Card>
  <Card title="모바일 Node" icon="smartphone" href="/ko/nodes">
    페어링, 음성/채팅 및 다양한 기기 명령을 지원하는 iOS 및 Android Node입니다.
  </Card>
</Columns>
...
- iMessage, Telegram 및 WebChat은 핵심 설치에 포함되며, 그 외 모든 채널은
  `openclaw plugins install @openclaw/<id>`로 설치하는 공식 Plugin입니다(`openclaw onboard` /
  `openclaw channels add` 실행 중 필요할 때 설치할 수도 있음).
...
- 작업 공간 또는 발신자별 격리 세션을 사용하는 멀티 에이전트 라우팅
- 세션: 직접 채팅은 공유 `main`으로 통합되며 그룹은 격리됨
...
- 사용자 지정 및 자체 호스팅 제공자 지원(vLLM, SGLang, Ollama, llama.cpp, LM Studio 및
  모든 OpenAI 호환 또는 Anthropic 호환 엔드포인트)

**미디어:**

- 이미지, 오디오, 동영상 및 문서의 송수신
- 공유 이미지 생성 및 동영상 생성 기능 인터페이스
- 음성 메모 전사
...
- macOS 메뉴 막대 컴패니언 앱
- 페어링, Canvas, 카메라, 화면 녹화, 위치 및 음성을 지원하는 iOS Node
- 페어링, 채팅, 음성, Canvas, 카메라 및 기기 명령을 지원하는 Android Node
...
<CardGroup cols={2}>
  <Card title="실험적 기능" href="/ko/concepts/experimental-features" icon="flask">
    아직 기본 기능 영역에 출시되지 않은 선택형 기능입니다.
  </Card>
  <Card title="에이전트 런타임" href="/ko/concepts/agent" icon="robot">
    에이전트 런타임 모델과 실행이 전달되는 방식을 설명합니다.
  </Card>
