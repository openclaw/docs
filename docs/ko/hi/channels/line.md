...
read_when:
    ...
    - LINE별 메시지 옵션이 필요합니다
summary: LINE 메시징 API Plugin 설정, 구성 및 사용
title: LINE
x-i18n:
...
수신기로 실행되며 인증에 사용자의 채널 액세스 토큰 + 채널 시크릿을
사용합니다.

상태: 다운로드 가능한 Plugin. 다이렉트 메시지, 그룹 채팅, 미디어, 위치, Flex
messages, template messages, quick replies가 지원됩니다. Reactions와 threads는
지원되지 않습니다.
...
2. Provider를 만들거나 선택하고 **Messaging API** 채널을 추가하세요.
3. 채널 설정에서 **Channel access token** 및 **Channel secret**을 복사하세요.
4. Messaging API 설정에서 **Use webhook**을 활성화하세요.
5. Webhook URL을 사용자의 Gateway 엔드포인트로 설정하세요(HTTPS 필요).
...
- LINE signature verification은 body-dependent(raw body에 대한 HMAC)이므로 OpenClaw는 verification 전에 strict pre-auth body limits와 timeout을 적용합니다.
- OpenClaw는 verified raw request bytes에서 Webhook events를 process합니다. signature-integrity safety를 위해 upstream middleware-transformed `req.body` values는 무시됩니다.
...
`tokenFile` 및 `secretFile`은 regular files를 가리켜야 합니다. Symlinks는 거부됩니다.
...
Direct messages는 기본적으로 pairing 상태입니다. 알 수 없는 senders는 pairing code를 받으며, 해당
...
- Runtime note: `channels.line`이 완전히 missing인 경우 runtime은 group checks에 대해 `groupPolicy="allowlist"`로 fallback합니다(`channels.defaults.groupPolicy`가 설정되어 있더라도).

LINE IDs는 대소문자를 구분합니다. 유효한 IDs는 다음과 같습니다.
...
- Inbound media는 agent로 전달되기 전에 `~/.openclaw/media/inbound/` 아래에 save되며,
  이는 다른 bundled channel
  plugins에서 사용하는 shared media store와 일치합니다.

## Channel data(rich messages)

quick replies, locations, Flex cards 또는 template
messages를 보내려면 `channelData.line`을 사용하세요.
...
LINE Plugin은 Flex message presets용 `/card` command도 제공합니다.
...
자세한 내용은 [ACP agents](/ko/tools/acp-agents)를 참조하세요.

## Outbound media

LINE Plugin은 agent message tool을 통해 images, videos, audio files 전송을 지원합니다. Media는 appropriate preview 및 tracking handling과 함께 LINE-specific delivery path를 통해 전송됩니다.

- **Images**: automatic preview generation과 함께 LINE image messages로 전송됩니다.
- **Videos**: explicit preview 및 content-type handling과 함께 전송됩니다.
...
- **No inbound events:** Webhook path가 `channels.line.webhookPath`와 일치하고
  Gateway가 LINE에서 reachable인지 확인하세요.
- **Media download errors:** media가 default limit를 초과하는 경우 `channels.line.mediaMaxMb`를 늘리세요.
...
- [Channel Routing](/ko/channels/channel-routing) — messages용 session routing
