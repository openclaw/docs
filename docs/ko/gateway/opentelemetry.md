...
read_when:
    ...
    - 대시보드 또는 알림을 만들려면 정확한 메트릭 이름, 스팬 이름 또는 속성 형식이 필요합니다.
summary: diagnostics-otel Plugin을 통해 OpenClaw 진단 정보를 OpenTelemetry collector 또는 stdout JSONL로 내보내기
title: OpenTelemetry 내보내기
x-i18n:
    generated_at: "2026-07-01T05:34:33Z"
    model: gpt-5.5
...
OpenClaw는 공식 `diagnostics-otel` Plugin을 통해 **OTLP/HTTP (protobuf)**로 진단 정보를 내보냅니다. 로그는 컨테이너 및 샌드박스 로그 파이프라인을 위해 stdout JSONL로도 기록할 수 있습니다. OTLP/HTTP를 수신하는 모든 컬렉터나 백엔드는 코드 변경 없이 작동합니다. 로컬 파일 로그와 읽는 방법은 [로깅](/ko/logging)을 참조하세요.

## 전체 구성 방식

- **진단 이벤트**는 Gateway와 번들 Plugin이 모델 실행, 메시지 흐름, 세션, 큐, exec에 대해 내보내는 구조화된 프로세스 내부 레코드입니다.
- **`diagnostics-otel` Plugin**은 해당 이벤트를 구독하고 OTLP/HTTP를 통해 OpenTelemetry **메트릭**, **트레이스**, **로그**로 내보냅니다. 진단 로그 레코드를 stdout JSONL로 미러링할 수도 있습니다.
- **Provider 호출**은 provider 전송이 사용자 지정 헤더를 허용할 때 OpenClaw의 신뢰된 모델 호출 span 컨텍스트에서 W3C `traceparent` 헤더를 받습니다. Plugin이 내보낸 트레이스 컨텍스트는 전파되지 않습니다.
- 내보내기는 진단 표면과 Plugin이 모두 활성화된 경우에만 연결되므로, 프로세스 내부 비용은 기본적으로 거의 0에 가깝게 유지됩니다.
...
| 신호        | 포함되는 내용                                                                                                                                                                                                 |
| ----------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **메트릭**  | 토큰 사용량, 비용, 실행 시간, 장애 조치, skill 사용량, 메시지 흐름, Talk 이벤트, 큐 레인, 세션 상태/복구, 도구 실행, 과대 페이로드, exec, 메모리 압력에 대한 카운터와 히스토그램. |
| **트레이스** | 모델 사용량, 모델 호출, 하네스 수명 주기, skill 사용량, 도구 실행, exec, webhook/메시지 처리, 컨텍스트 조립, 도구 루프에 대한 span.                                                   |
...
`traces`, `metrics`, `logs`를 독립적으로 전환할 수 있습니다. `diagnostics.otel.enabled`가 true이면 트레이스와 메트릭은 기본적으로 켜집니다. 로그는 기본적으로 꺼져 있으며 `diagnostics.otel.logs`가 명시적으로 `true`일 때만 내보냅니다. 로그 내보내기는 기본적으로 OTLP입니다. stdout에 JSONL을 출력하려면 `diagnostics.otel.logsExporter`를 `stdout`으로 설정하고, 각 진단 로그 레코드를 OTLP와 stdout 모두로 보내려면 `both`로 설정하세요.
...
| `OTEL_EXPORTER_OTLP_PROTOCOL`                                                                                     | 전송 프로토콜을 재정의합니다. 현재는 `http/protobuf`만 적용됩니다.                                                                                                                                                                                                                                                                   |
| `OTEL_SEMCONV_STABILITY_OPT_IN`                                                                                   | `{gen_ai.operation.name} {gen_ai.request.model}` span 이름, `CLIENT` span 종류, 레거시 `gen_ai.system` 대신 `gen_ai.provider.name`을 포함하는 최신 실험적 GenAI 추론 span 형태를 내보내려면 `gen_ai_latest_experimental`로 설정합니다. GenAI 메트릭은 항상 제한된 저카디널리티 의미 속성을 사용합니다. |
...
원시 모델/도구 콘텐츠는 기본적으로 내보내지 **않습니다**. Span은 제한된 식별자(채널, provider, 모델, 오류 범주, 해시 전용 요청 ID, 도구 소스, 도구 소유자, skill 이름/소스)를 포함하며, 프롬프트 텍스트, 응답 텍스트, 도구 입력, 도구 출력, skill 파일 경로, 세션 키는 절대 포함하지 않습니다. OTLP 로그 레코드는 기본적으로 심각도, 로거, 코드 위치, 신뢰된 트레이스 컨텍스트, 정제된 속성을 유지하지만, 원시 로그 메시지 본문은 `diagnostics.otel.captureContent`가 boolean `true`로 설정된 경우에만 내보냅니다. 세분화된 `captureContent.*` 하위 키는 로그 본문을 활성화하지 않습니다. 범위가 지정된 에이전트 세션 키처럼 보이는 라벨은 `unknown`으로 대체됩니다.
Talk 메트릭은 모드, 전송, provider, 이벤트 유형과 같은 제한된 이벤트 메타데이터만 내보냅니다. 전사, 오디오 페이로드, 세션 ID, 턴 ID, 호출 ID, 룸 ID, handoff 토큰은 포함하지 않습니다.

아웃바운드 모델 요청에는 W3C `traceparent` 헤더가 포함될 수 있습니다. 이 헤더는 활성 모델 호출에 대한 OpenClaw 소유 진단 트레이스 컨텍스트에서만 생성됩니다. 기존 호출자 제공 `traceparent` 헤더는 대체되므로, Plugin이나 사용자 지정 provider 옵션이 서비스 간 트레이스 계보를 위조할 수 없습니다.
...
하위 키가 하나라도 활성화되면 모델 및 도구 span은 해당 클래스에 대해서만 제한되고 수정된 `openclaw.content.*` 속성을 받습니다. boolean `captureContent: true`는 OTLP 로그 메시지 본문도 내보내기로 승인된 광범위한 진단 캡처에만 사용하세요.

`toolInputs`/`toolOutputs` 콘텐츠는 기본 제공 에이전트 런타임의 도구 실행에 대해 캡처됩니다(완료/오류 span의 `openclaw.content.tool_input`, 완료 span의 `openclaw.content.tool_output`). 외부 하네스 도구 호출(Codex, Claude CLI)은 콘텐츠 페이로드 없이 `tool.execution.*` span을 내보냅니다. 캡처된 콘텐츠는 신뢰된 리스너 전용 채널로 전달되며 공개 진단 이벤트 버스에 배치되지 않습니다.

## 샘플링 및 플러시

- **추적:** `diagnostics.otel.sampleRate`(루트 span만 해당, `0.0`은 모두 버리고,
  `1.0`은 모두 유지).
- **메트릭:** `diagnostics.otel.flushIntervalMs`(최소 `1000`).
- **로그:** OTLP 로그는 `logging.level`(파일 로그 수준)을 따릅니다. 콘솔 포맷팅이 아니라
  진단 로그 레코드 수정 경로를 사용합니다. 대용량 설치 환경에서는 로컬 샘플링보다
  OTLP collector 샘플링/필터링을 선호해야 합니다. 플랫폼이 이미 stdout/stderr를
  로그 프로세서로 보내고 OTLP logs collector가 없는 경우
  `diagnostics.otel.logsExporter: "stdout"`을 설정하세요. Stdout 레코드는 줄마다 하나의
  JSON 객체이며, 사용 가능한 경우 `ts`, `signal`, `service.name`, 심각도, 본문,
  수정된 속성, 신뢰할 수 있는 추적 필드를 포함합니다.
- **파일 로그 상관관계:** JSONL 파일 로그는 로그 호출에 유효한 진단 추적 컨텍스트가
  포함된 경우 최상위 `traceId`, `spanId`, `parentSpanId`, `traceFlags`를 포함하므로
  로그 프로세서가 로컬 로그 줄을 내보낸 span과 결합할 수 있습니다.
- **요청 상관관계:** Gateway HTTP 요청과 WebSocket 프레임은 내부 요청 추적 범위를
  만듭니다. 해당 범위 안의 로그와 진단 이벤트는 기본적으로 요청 추적을 상속하며,
  에이전트 실행 및 모델 호출 span은 자식으로 생성되어 provider `traceparent` 헤더가
  같은 추적에 유지됩니다.
- **모델 호출 상관관계:** `openclaw.model.call` span은 기본적으로 안전한 프롬프트
  구성 요소 크기를 포함하며, provider 결과가 사용량을 노출할 때 호출별 토큰 속성을
  포함합니다. `openclaw.model.usage`는 집계 비용, 컨텍스트, 채널 대시보드를 위한
  실행 수준 회계 span으로 유지되며, 내보내는 런타임에 신뢰할 수 있는 추적 컨텍스트가
  있을 때 같은 진단 추적에 유지됩니다.
...
- `openclaw.model_call.response_bytes`(히스토그램, 스트리밍된 응답 청크 페이로드의 UTF-8 바이트 크기, 고빈도 텍스트, 사고, 도구 호출 delta는 증분 `delta` 바이트만 계산, 원시 응답 내용 없음)
...
- `openclaw.talk.event.duration_ms`(히스토그램, 속성: `openclaw.talk.event`와 동일, Talk 이벤트가 지속 시간을 보고할 때 내보냄)
- `openclaw.talk.audio.bytes`(히스토그램, 속성: `openclaw.talk.event`와 동일, 바이트 길이를 보고하는 Talk 오디오 프레임 이벤트에 대해 내보냄)
...
- `openclaw.session.stuck`(카운터, 속성: `openclaw.state`, 복구 가능한 오래된 세션 장부 기록에 대해 내보냄)
- `openclaw.session.stuck_age_ms`(히스토그램, 속성: `openclaw.state`, 복구 가능한 오래된 세션 장부 기록에 대해 내보냄)
...
`diagnostics.stuckSessionWarnMs`는 세션 활성 상태 진단을 위한 진행 없음 기간 임계값입니다.
`processing` 세션은 OpenClaw가 응답, 도구, 상태, 블록 또는 ACP 런타임 진행을 관찰하는 동안
이 임계값을 향해 시간이 누적되지 않습니다. 입력 keepalive는 진행으로 계산되지 않으므로,
조용한 모델이나 harness도 여전히 감지될 수 있습니다.

OpenClaw는 계속 관찰할 수 있는 작업에 따라 세션을 분류합니다.

- `session.long_running`: 활성 임베디드 작업, 모델 호출 또는 도구 호출이
  여전히 진행 중입니다. `diagnostics.stuckSessionWarnMs`를 지나도록 조용한
  소유된 모델 호출도 `diagnostics.stuckSessionAbortMs` 전에는 long-running으로
  보고되므로, 느리거나 스트리밍하지 않는 모델 provider가 abort 관찰 가능한 상태로
  남아 있는 동안 중단된 gateway 세션처럼 보이지 않습니다.
- `session.stalled`: 활성 작업은 있지만, 활성 실행이 최근 진행 상황을 보고하지 않았습니다.
  소유된 모델 호출은 `diagnostics.stuckSessionAbortMs` 시점 또는 이후에
  `session.long_running`에서 `session.stalled`로 전환됩니다. 소유자 없는 오래된
  모델/도구 활동은 무해한 장기 실행 작업으로 취급되지 않습니다. 중단된 임베디드 실행은
  처음에는 관찰 전용으로 유지된 뒤, 진행 없이 `diagnostics.stuckSessionAbortMs`가
  지난 후 abort-drain되어 해당 lane 뒤의 대기 중인 turn이 재개될 수 있습니다. 설정되지
  않은 경우 abort 임계값은 최소 5분 및 `diagnostics.stuckSessionWarnMs`의 3배인 더
  안전한 확장 창으로 기본 설정됩니다.
- `session.stuck`: 활성 작업이 없는 오래된 세션 장부 기록, 또는 소유자 없는 오래된
  모델/도구 활동이 있는 유휴 대기 세션입니다. 이는 복구 게이트를 통과한 직후 영향을
  받는 세션 lane을 해제합니다.

복구는 구조화된 `session.recovery.requested` 및
`session.recovery.completed` 이벤트를 내보냅니다. 진단 세션 상태는 변경을 수행한
복구 결과(`aborted` 또는 `released`) 이후에만, 그리고 동일한 처리 generation이
여전히 현재일 때만 idle로 표시됩니다.

`session.stuck`만 `openclaw.session.stuck` 카운터,
`openclaw.session.stuck_age_ms` 히스토그램, `openclaw.session.stuck` span을
내보냅니다. 반복되는 `session.stuck` 진단은 세션이 변경되지 않은 동안 백오프되므로,
대시보드는 모든 Heartbeat 틱이 아니라 지속적인 증가에 대해 경고해야 합니다. 구성 노브와
기본값은 [구성 참조](/ko/gateway/configuration-reference#diagnostics)를 참조하세요.
...
- `openclaw.tool.execution.duration_ms`(히스토그램, 속성: `gen_ai.tool.name`, `openclaw.toolName`, `openclaw.tool.source`, `openclaw.tool.owner`, `openclaw.tool.params.kind`, 오류 시 `openclaw.errorCategory` 추가)
...
- `openclaw.model.call`
  ...
  - `openclaw.model_call.prompt.input_messages_count`, `openclaw.model_call.prompt.input_messages_chars`, `openclaw.model_call.prompt.system_prompt_chars`, `openclaw.model_call.prompt.tool_definitions_count`, `openclaw.model_call.prompt.tool_definitions_chars`, `openclaw.model_call.prompt.total_chars` (안전한 구성 요소 크기만 포함, 프롬프트 텍스트 없음)
  - 모델 호출 결과가 해당 개별 호출의 제공자 사용량을 포함하는 경우 `openclaw.model_call.usage.*` 및 `gen_ai.usage.*`
  - `openclaw.provider.request_id_hash` (업스트림 제공자 요청 ID의 제한된 SHA 기반 해시; 원시 ID는 내보내지 않음)
  - `OTEL_SEMCONV_STABILITY_OPT_IN=gen_ai_latest_experimental`를 사용하면 모델 호출 span은 `openclaw.model.call` 대신 최신 GenAI 추론 span 이름 `{gen_ai.operation.name} {gen_ai.request.model}` 및 `CLIENT` span 종류를 사용합니다.
...
콘텐츠 캡처가 명시적으로 활성화된 경우, 모델 및 도구 span은 선택한
특정 콘텐츠 클래스에 대해 제한되고 수정된 `openclaw.content.*` 속성도
포함할 수 있습니다.

## 진단 이벤트 카탈로그

아래 이벤트는 위의 메트릭과 span을 뒷받침합니다. Plugin은 OTLP 내보내기 없이도
이를 직접 구독할 수 있습니다.

**모델 사용량**

- `model.usage` - 토큰, 비용, 기간, 컨텍스트, 제공자/모델/채널,
  세션 ID. `usage`는 비용 및 원격 측정을 위한 제공자/턴 계정이며,
  `context.used`는 현재 프롬프트/컨텍스트 스냅샷입니다. 캐시된 입력 또는
...
- `exec.process.completed` - 터미널 결과, 기간, 대상, 모드, 종료
  ...
  않습니다.
- `exec.approval.followup_suppressed` - 세션 리바운드 후 오래된 승인 후속 조치가 삭제되었습니다.
  `approvalId`, `reason` (`session_rebound`),
  `phase` (`direct_delivery` 또는 `gateway_preflight`), 및 디스패처
  타임스탬프를 포함합니다. 세션 키, 경로 및 명령 텍스트는 포함되지 않습니다.
...
`logging.level`을 올리지 않고 대상 디버그 출력을 사용하려면 진단
