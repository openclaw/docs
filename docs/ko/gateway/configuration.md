...
read_when:
    ...
    - 일반적인 구성 패턴을 찾는 중
    - 특정 설정 섹션으로 이동하기
summary: '구성 개요: 일반 작업, 빠른 설정 및 전체 참조 문서 링크'
title: 구성
x-i18n:
...
레이아웃은 OpenClaw 소유 쓰기에서 지원되지 않습니다. 원자적 쓰기가 심볼릭 링크를 보존하는 대신
해당 경로를 대체할 수 있습니다. 기본 상태 디렉터리 밖에 구성을 두는 경우
`OPENCLAW_CONFIG_PATH`가 실제 파일을 직접 가리키도록 설정하세요.

파일이 없으면 OpenClaw는 안전한 기본값을 사용합니다. 구성을 추가하는 일반적인 이유는 다음과 같습니다.

- 채널을 연결하고 bot에 메시지를 보낼 수 있는 사용자를 제어
...
<Tip>
**구성이 처음인가요?** 대화형 설정은 `openclaw onboard`로 시작하거나, 바로 복사해 붙여넣을 수 있는 전체 구성은 [구성 예제](/ko/gateway/configuration-examples) 가이드를 확인하세요.
</Tip>
...
<Tabs>
  ...
  <Tab title="Control UI">
    ...
    Control UI는 실시간 구성 스키마에서 폼을 렌더링합니다. 필드
    `title` / `description` 문서 메타데이터와 사용 가능한 경우 Plugin 및 채널 스키마를 포함하며,
    탈출구로 **Raw JSON** 편집기를 제공합니다. 드릴다운
    UI와 기타 도구를 위해 Gateway는 `config.schema.lookup`도 노출하여
    경로 범위 스키마 노드 하나와 직계 하위 요약을 가져올 수 있게 합니다.
  </Tab>
  <Tab title="직접 편집">
...
`openclaw config schema`는 Control UI와 검증에서 사용하는 정식 JSON Schema를 출력합니다.
`config.schema.lookup`은 드릴다운 도구를 위해 경로 범위 노드 하나와
하위 요약을 가져옵니다. 필드 `title`/`description` 문서 메타데이터는
중첩 객체, 와일드카드(`*`), 배열 항목(`[]`) 및 `anyOf`/
`oneOf`/`allOf` 분기를 통해 전달됩니다. 런타임 Plugin 및 채널 스키마는
manifest registry가 로드될 때 병합됩니다.
...
나중에 `openclaw.json`이 검증에 실패하거나(`gateway.mode`가 제거되거나, 급격히
작아지거나, 앞에 불필요한 로그 줄이 추가된 경우), OpenClaw는 손상된 파일을
`.clobbered.*`로 보존하고 마지막 정상 사본을 복원하며 복구
이유를 기록합니다. 다음 에이전트 턴도 시스템 이벤트 경고를 수신하므로 주
에이전트가 복원된 구성을 무작정 다시 쓰지 않습니다. 후보에 `***` 같은 삭제 처리된
비밀 placeholder가 포함된 경우 마지막 정상 사본으로 승격하지 않습니다.
모든 검증 문제가 `plugins.entries.<id>...` 범위에만 해당하는 경우 OpenClaw는
전체 파일 복구를 수행하지 않습니다. 현재 구성을 활성 상태로 유지하고
Plugin 로컬 실패를 표시하여 Plugin 스키마 또는 호스트 버전 불일치가
관련 없는 사용자 설정을 롤백하지 못하게 합니다.
...
<AccordionGroup>
  ...
  <Accordion title="모델 선택 및 구성">
    ...
    - `agents.defaults.models`는 모델 catalog를 정의하며 `/model`의 allowlist 역할을 합니다.
    ...
    - `agents.defaults.imageMaxDimensionPx`는 transcript/tool 이미지 다운스케일링을 제어합니다(기본값 `1200`). 값이 낮을수록 일반적으로 스크린샷이 많은 실행에서 vision-token 사용량이 줄어듭니다.
    - 채팅에서 모델을 전환하는 방법은 [모델 CLI](/ko/concepts/models)를, 인증 순환 및 fallback 동작은 [모델 장애 조치](/ko/concepts/model-failover)를 참고하세요.
    - 사용자 지정/자체 호스팅 provider는 참조의 [사용자 지정 provider](/ko/gateway/config-tools#custom-providers-and-base-urls)를 참고하세요.

  </Accordion>

  <Accordion title="bot에 메시지를 보낼 수 있는 사용자 제어">
    DM 접근은 채널별로 `dmPolicy`를 통해 제어됩니다.

    - `"pairing"`(기본값): 알 수 없는 발신자는 승인할 일회성 페어링 코드를 받습니다
  ...
  <Accordion title="그룹 채팅 멘션 게이팅 설정">
    그룹 메시지는 기본적으로 **멘션 필요**입니다. 에이전트별 trigger 패턴을 구성하고, 기존 자동 최종 답장을 의도적으로 원하지 않는 한 표시되는 방 답장은 기본 message-tool 경로에 유지하세요.
    ...
    - **표시되는 답장**: `messages.visibleReplies`는 전역적으로 message-tool 전송을 요구할 수 있으며, `messages.groupChat.visibleReplies`는 그룹/채널에 대해 이를 재정의합니다.
    - 표시 답장 모드, 채널별 재정의 및 self-chat 모드는 [전체 참조](/ko/gateway/config-channels#group-chat-mention-gating)를 참고하세요.
  ...
  <Accordion title="Gateway 채널 상태 모니터링 조정">
    오래된 것처럼 보이는 채널을 Gateway가 얼마나 적극적으로 재시작할지 제어합니다.
    ...
    - 상태 모니터 재시작을 전역으로 비활성화하려면 `gateway.channelHealthCheckMinutes: 0`으로 설정하세요.
  ...
  <Accordion title="Gateway WebSocket handshake timeout 조정">
    로드가 많거나 저전력인 호스트에서 로컬 클라이언트가 인증 전 WebSocket handshake를 완료할 시간을
    더 많이 제공합니다.
    ...
    - 기본값은 `15000`밀리초입니다.
    - `OPENCLAW_HANDSHAKE_TIMEOUT_MS`는 일회성 서비스 또는 shell 재정의에 여전히 우선합니다.
    - 먼저 시작/이벤트 루프 stall을 수정하는 것을 권장합니다. 이 knob는 정상적이지만 warmup 중 느린 호스트를 위한 것입니다.
  ...
  <Accordion title="세션 및 reset 구성">
    ...
    - `threadBindings`: 스레드 바인딩된 세션 라우팅의 전역 기본값(Discord는 `/focus`, `/unfocus`, `/agents`, `/session idle`, `/session max-age`를 지원).
    - 범위 지정, ID 링크, 전송 정책은 [세션 관리](/ko/concepts/session)를 참조하세요.
  ...
  <Accordion title="공식 iOS 빌드용 릴레이 기반 푸시 활성화">
    ...
    - 공식/TestFlight iOS 빌드에 포함된 릴레이 기본 URL과 일치해야 등록 및 전송 트래픽이 같은 릴레이 배포에 도달합니다.

    엔드투엔드 흐름:

    1. 같은 릴레이 기본 URL로 컴파일된 공식/TestFlight iOS 빌드를 설치합니다.
    ...
    5. Gateway는 릴레이 핸들과 전송 권한을 저장한 다음, 이를 `push.test`, 깨우기 넛지, 재연결 깨우기에 사용합니다.

    운영 참고 사항:

    - iOS 앱을 다른 Gateway로 전환하는 경우, 앱을 다시 연결해 해당 Gateway에 바인딩된 새 릴레이 등록을 게시할 수 있게 하세요.
    ...
    - `OPENCLAW_APNS_RELAY_ALLOW_HTTP=true`는 loopback 전용 개발 우회 수단으로 남아 있습니다. HTTP 릴레이 URL을 구성에 영구 저장하지 마세요.
  ...
  <Accordion title="Webhook(훅) 설정">
    ...
    - 엄격히 범위가 지정된 디버깅을 하는 경우가 아니라면 안전하지 않은 콘텐츠 우회 플래그(`hooks.gmail.allowUnsafeExternalContent`, `hooks.mappings[].allowUnsafeExternalContent`)를 비활성화해 두세요.
    - `hooks.allowRequestSessionKey`를 활성화하는 경우, 호출자가 선택한 세션 키를 제한하도록 `hooks.allowedSessionKeyPrefixes`도 설정하세요.
    - 훅으로 구동되는 에이전트에는 강력한 최신 모델 티어와 엄격한 도구 정책(예: 가능하면 메시징 전용과 샌드박싱)을 권장합니다.
  ...
  <Accordion title="다중 에이전트 라우팅 구성">
    별도 워크스페이스와 세션을 가진 여러 격리된 에이전트를 실행합니다.
    ...
    바인딩 규칙과 에이전트별 접근 프로필은 [다중 에이전트](/ko/concepts/multi-agent) 및 [전체 참조](/ko/gateway/config-agents#multi-agent-routing)를 참조하세요.
  ...
  <Accordion title="구성을 여러 파일로 분할($include)">
    ...
    - **OpenClaw 소유 쓰기**: 쓰기가 `plugins: { $include: "./plugins.json5" }`처럼
      단일 파일 include가 뒷받침하는 최상위 섹션 하나만 변경하는 경우,
      OpenClaw는 해당 포함 파일을 업데이트하고 `openclaw.json`은 그대로 둡니다
    - **지원되지 않는 write-through**: 루트 include, include 배열, 형제 재정의가
      있는 include는 OpenClaw 소유 쓰기에서 구성을 평탄화하는 대신
      닫힌 상태로 실패합니다
    - **제한**: `$include` 경로는 `openclaw.json`이 있는 디렉터리 아래로
      해석되어야 합니다. 여러 머신이나 사용자 간에 트리를 공유하려면,
      include가 참조할 수 있는 추가 디렉터리의 경로 목록(POSIX에서는 `:`, Windows에서는 `;`)으로
      `OPENCLAW_INCLUDE_ROOTS`를 설정하세요. 심볼릭 링크는 해석된 뒤
      다시 검사되므로, 어휘상 구성 디렉터리 안에 있지만 실제 대상이
      허용된 모든 루트를 벗어나는 경로는 여전히 거부됩니다.
...
마지막 정상 구성을 복원하여 잘못된 외부 편집을 거부합니다. OpenClaw 소유
구성 쓰기는 쓰기 전에 같은 스키마 게이트를 사용합니다. `gateway.mode`를 삭제하거나
파일 크기를 절반 넘게 줄이는 것 같은 파괴적 덮어쓰기는 거부되며,
검사를 위해 `.rejected.*`로 저장됩니다.

Plugin 로컬 검증 실패는 예외입니다. 모든 문제가
`plugins.entries.<id>...` 아래에 있는 경우, 리로드는 현재 구성을 유지하고
`.last-good`을 복원하는 대신 Plugin 문제를 보고합니다.

로그에 `Config auto-restored from last-known-good` 또는
`config reload restored last-known-good config`가 보이면, `openclaw.json` 옆의
...
| **`hot`**              | 안전한 변경 사항만 핫 적용합니다. 재시작이 필요하면 경고를 기록합니다. 처리는 사용자가 합니다. |
...
<Note>
`gateway.reload`와 `gateway.remote`는 예외입니다. 이를 변경해도 재시작은 트리거되지 **않습니다**.
</Note>

### 다시 로드 계획

`$include`를 통해 참조되는 소스 파일을 편집하면, OpenClaw는 평탄화된 인메모리 뷰가 아니라 소스가 작성된 레이아웃을 기준으로 다시 로드를 계획합니다.
따라서 `plugins: { $include: "./plugins.json5" }`처럼 단일 최상위 섹션이 자체 포함 파일에 있는 경우에도 핫 리로드 결정(핫 적용과 재시작)이 예측 가능하게 유지됩니다. 소스 레이아웃이 모호하면 다시 로드 계획은 안전하게 실패합니다.

## 구성 RPC(프로그래밍 방식 업데이트)

Gateway API를 통해 구성을 쓰는 도구의 경우 다음 흐름을 권장합니다.

- `config.schema.lookup`으로 하나의 하위 트리 검사(얕은 스키마 노드 + 자식 요약)
- `config.get`으로 현재 스냅샷과 `hash` 가져오기
- `config.patch`로 부분 업데이트(JSON 병합 패치: 객체는 병합, `null`은 삭제, 배열은 교체)
- 전체 구성을 교체하려는 경우에만 `config.apply` 사용
- 명시적 자체 업데이트와 재시작에는 `update.run` 사용
- 최신 업데이트 재시작 센티널을 검사하고 재시작 후 실행 중인 버전을 확인하려면 `update.status` 사용

에이전트는 정확한 필드 수준 문서와 제약 조건을 확인하는 첫 단계로 `config.schema.lookup`을 사용해야 합니다. 더 넓은 구성 맵, 기본값 또는 전용 하위 시스템 참조 링크가 필요한 경우 [구성 참조](/ko/gateway/configuration-reference)를 사용하세요.

<Note>
컨트롤 플레인 쓰기(`config.apply`, `config.patch`, `update.run`)는 `deviceId+clientIp`당 60초에 3개 요청으로 속도가 제한됩니다. 재시작 요청은 병합된 다음 재시작 주기 사이에 30초 쿨다운을 적용합니다.
`update.status`는 읽기 전용이지만, 재시작 센티널에 업데이트 단계 요약과 명령 출력 꼬리가 포함될 수 있으므로 관리자 범위가 적용됩니다.
</Note>
...
<Accordion title="구성 값의 환경 변수 치환">
  `${VAR_NAME}`을 사용해 모든 구성 문자열 값에서 환경 변수를 참조하세요.
