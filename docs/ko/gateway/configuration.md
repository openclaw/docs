...
title: 구성
x-i18n:
    generated_at: "2026-05-07T13:17:13Z"
    model: gpt-5.5
    provider: openai
...
OpenClaw은 `~/.openclaw/openclaw.json`에서 선택적 <Tooltip tip="JSON5 supports comments and trailing commas">**JSON5**</Tooltip> 구성을 읽습니다.
활성 구성 경로는 일반 파일이어야 합니다. 심볼릭 링크된 `openclaw.json`
레이아웃은 OpenClaw 소유 쓰기에 지원되지 않습니다. 원자적 쓰기는 심볼릭 링크를 보존하는 대신
해당 경로를 대체할 수 있습니다. 기본 상태 디렉터리 밖에 구성을 보관하는 경우
...
<Tip>
**구성이 처음인가요?** 대화형 설정은 `openclaw onboard`로 시작하거나, 완성된 복사-붙여넣기용 구성을 보려면 [구성 예시](/ko/gateway/configuration-examples) 가이드를 확인하세요.
</Tip>
...
<Tabs>
  ...
  <Tab title="Control UI">
    ...
    Control UI는 라이브 구성 스키마에서 양식을 렌더링합니다. 사용 가능한 경우 필드
    `title` / `description` 문서 메타데이터와 Plugin 및 채널 스키마를 포함하며,
    우회 수단으로 **Raw JSON** 편집기를 제공합니다. 드릴다운
    UI와 기타 도구를 위해 Gateway는 경로 범위 스키마 노드 하나와
    직계 하위 요약을 가져오는 `config.schema.lookup`도 제공합니다.
  </Tab>
...
<Warning>
OpenClaw은 스키마와 완전히 일치하는 구성만 허용합니다. 알 수 없는 키, 잘못된 형식의 타입 또는 유효하지 않은 값이 있으면 Gateway가 **시작을 거부**합니다. 유일한 루트 수준 예외는 `$schema`(문자열)이며, 이를 통해 편집기가 JSON Schema 메타데이터를 연결할 수 있습니다.
</Warning>
...
접두사가 붙었거나 덮어써진 구성을 복구하거나 마지막 정상 사본을
복원하려면 `openclaw doctor --fix`(또는 `--yes`)를 실행하세요. 후보에
`***` 같은 편집된 비밀 자리 표시자가 포함된 경우 마지막 정상 상태로의 승격은 건너뜁니다.
...
<AccordionGroup>
  ...
  <Accordion title="Choose and configure models">
    ...
    - `agents.defaults.models`는 모델 카탈로그를 정의하고 `/model`의 허용 목록으로 동작합니다.
    ...
    - 채팅에서 모델을 전환하려면 [모델 CLI](/ko/concepts/models)를, 인증 순환과 폴백 동작은 [모델 장애 조치](/ko/concepts/model-failover)를 확인하세요.
    - 사용자 지정/자체 호스팅 공급자는 참조의 [사용자 지정 공급자](/ko/gateway/config-tools#custom-providers-and-base-urls)를 확인하세요.
  ...
  <Accordion title="봇에 메시지를 보낼 수 있는 사람 제어">
    ...
    채널별 세부 정보는 [전체 참고 자료](/ko/gateway/config-channels#dm-and-group-access)를 참조하세요.

  </Accordion>

  <Accordion title="그룹 채팅 멘션 게이팅 설정">
    그룹 메시지는 기본적으로 **멘션 필요**로 설정됩니다. 에이전트별로 트리거 패턴을 구성하고, 기존 자동 최종 답장을 의도적으로 사용하려는 경우가 아니라면 보이는 방 답장은 기본 message-tool 경로에 유지하세요.
    ...
    - **보이는 답장**: `messages.visibleReplies`는 전역에서 message-tool 전송을 요구할 수 있으며, `messages.groupChat.visibleReplies`는 그룹/채널에 대해 이를 재정의합니다.
    - 보이는 답장 모드, 채널별 재정의, 셀프 채팅 모드는 [전체 참고 자료](/ko/gateway/config-channels#group-chat-mention-gating)를 참조하세요.

  </Accordion>

  <Accordion title="에이전트별 Skills 제한">
    공유 기준선에는 `agents.defaults.skills`를 사용한 다음, 특정
    에이전트는 `agents.list[].skills`로 재정의합니다.
    ...
    - [Skills](/ko/tools/skills), [Skills 구성](/ko/tools/skills-config), 그리고
      [구성 참고 자료](/ko/gateway/config-agents#agents-defaults-skills)를 참조하세요.

  </Accordion>

  <Accordion title="Gateway 채널 상태 모니터링 조정">
    오래된 것처럼 보이는 채널을 Gateway가 얼마나 적극적으로 다시 시작할지 제어합니다.
    ...
    - 상태 모니터 재시작을 전역에서 비활성화하려면 `gateway.channelHealthCheckMinutes: 0`으로 설정합니다.
    - `channelStaleEventThresholdMinutes`는 검사 간격보다 크거나 같아야 합니다.
    - 전역 모니터를 비활성화하지 않고 한 채널 또는 계정의 자동 재시작을 비활성화하려면 `channels.<provider>.healthMonitor.enabled` 또는 `channels.<provider>.accounts.<id>.healthMonitor.enabled`를 사용합니다.
    - 운영 디버깅은 [상태 검사](/ko/gateway/health)를, 모든 필드는 [전체 참고 자료](/ko/gateway/configuration-reference#gateway)를 참조하세요.

  </Accordion>

  <Accordion title="Gateway WebSocket 핸드셰이크 제한 시간 조정">
    부하가 있거나 저전력인 호스트에서 로컬 클라이언트가 인증 전 WebSocket 핸드셰이크를 완료할 시간을 더 부여합니다.
    ...
    - 먼저 시작/이벤트 루프 정체를 해결하는 것을 권장합니다. 이 조절값은 정상 상태이지만 예열 중 느린 호스트를 위한 것입니다.
  ...
  <Accordion title="세션 및 초기화 구성">
    ...
    - `threadBindings`: 스레드 바인딩 세션 라우팅의 전역 기본값입니다(Discord는 `/focus`, `/unfocus`, `/agents`, `/session idle`, `/session max-age`를 지원합니다).
    - 범위 지정, ID 연결, 전송 정책은 [세션 관리](/ko/concepts/session)를 참조하세요.
    - 모든 필드는 [전체 참고 자료](/ko/gateway/config-agents#session)를 참조하세요.
  ...
  <Accordion title="샌드박싱 활성화">
    ...
    먼저 이미지를 빌드하세요. 소스 체크아웃에서는 `scripts/sandbox-setup.sh`를 실행하고, npm 설치에서는 [Sandboxing § Images and setup](/ko/gateway/sandboxing#images-and-setup)의 인라인 `docker build` 명령을 참고하세요.
  ...
  <Accordion title="공식 iOS 빌드용 릴레이 기반 푸시 활성화">
    ...
    - 페어링된 iOS 앱이 전달한 등록 범위의 전송 권한을 사용합니다. Gateway에는 배포 전체 릴레이 토큰이 필요하지 않습니다.
    ...
    - 등록 및 전송 트래픽이 같은 릴레이 배포로 도달하도록 공식/TestFlight iOS 빌드에 포함된 릴레이 기본 URL과 일치해야 합니다.

    엔드 투 엔드 흐름:

    1. 동일한 릴레이 기본 URL로 컴파일된 공식/TestFlight iOS 빌드를 설치합니다.
    2. Gateway에서 `gateway.push.apns.relay.baseUrl`을 구성합니다.
    3. iOS 앱을 Gateway에 페어링하고 node 및 운영자 세션이 모두 연결되도록 합니다.
    4. iOS 앱은 Gateway ID를 가져오고, App Attest와 앱 영수증을 사용해 릴레이에 등록한 다음, 릴레이 기반 `push.apns.register` 페이로드를 페어링된 Gateway에 게시합니다.
    5. Gateway는 릴레이 핸들과 전송 권한을 저장한 다음, 이를 `push.test`, 깨우기 알림, 재연결 깨우기에 사용합니다.

    운영 참고 사항:

    - iOS 앱을 다른 Gateway로 전환하는 경우, 앱을 다시 연결하여 해당 Gateway에 바인딩된 새 릴레이 등록을 게시할 수 있게 하세요.
    - 다른 릴레이 배포를 가리키는 새 iOS 빌드를 배포하면, 앱은 이전 릴레이 출처를 재사용하는 대신 캐시된 릴레이 등록을 새로 고칩니다.

    호환성 참고 사항:

    - `OPENCLAW_APNS_RELAY_BASE_URL` 및 `OPENCLAW_APNS_RELAY_TIMEOUT_MS`는 임시 env 재정의로 계속 작동합니다.
    - `OPENCLAW_APNS_RELAY_ALLOW_HTTP=true`는 local loopback 전용 개발용 탈출구로 유지됩니다. HTTP 릴레이 URL을 구성에 영구 저장하지 마세요.

    엔드 투 엔드 흐름은 [iOS 앱](/ko/platforms/ios#relay-backed-push-for-official-builds)을, 릴레이 보안 모델은 [인증 및 신뢰 흐름](/ko/platforms/ios#authentication-and-trust-flow)을 참고하세요.
  ...
  <Accordion title="Webhook 설정(훅)">
    ...
    - 엄격히 범위가 제한된 디버깅을 하는 경우가 아니면 안전하지 않은 콘텐츠 우회 플래그(`hooks.gmail.allowUnsafeExternalContent`, `hooks.mappings[].allowUnsafeExternalContent`)를 비활성화 상태로 유지하세요.
    - `hooks.allowRequestSessionKey`를 활성화하는 경우, 호출자가 선택한 세션 키를 제한하도록 `hooks.allowedSessionKeyPrefixes`도 설정하세요.
    - 훅 기반 에이전트에는 강력한 최신 모델 티어와 엄격한 도구 정책을 권장합니다(예: 메시징 전용과 가능한 경우 샌드박싱).
  ...
  <Accordion title="구성을 여러 파일로 분리($include)">
    ...
    - **제한**: `$include` 경로는 `openclaw.json`이 있는 디렉터리 아래로 해석되어야 합니다. 여러 머신 또는 사용자 간에 트리를 공유하려면 include가 참조할 수 있는 추가 디렉터리의 경로 목록(POSIX에서는 `:`, Windows에서는 `;`)을 `OPENCLAW_INCLUDE_ROOTS`에 설정하세요. 심볼릭 링크는 해석된 후 다시 검사되므로, 문법적으로는 구성 디렉터리 안에 있지만 실제 대상이 허용된 모든 루트 밖으로 벗어나는 경로는 여전히 거부됩니다.
...
직접 파일 편집은 검증될 때까지 신뢰할 수 없는 것으로 취급됩니다. 감시자는 편집기의 임시 쓰기/이름 변경 변동이 안정될 때까지 기다리고, 최종 파일을 읽은 뒤, 유효하지 않은 외부 편집을 `openclaw.json`을 다시 쓰지 않고 거부합니다. OpenClaw 소유 구성 쓰기는 쓰기 전에 동일한 스키마 게이트를 사용합니다. `gateway.mode`를 삭제하거나 파일 크기를 절반 이상 줄이는 등의 파괴적 덮어쓰기는 거부되고 검사를 위해 `.rejected.*`로 저장됩니다.

`config reload skipped (invalid config)`가 표시되거나 시작 시 `Invalid
config`가 보고되면 구성을 검사하고 `openclaw config validate`를 실행한 다음, 수리를 위해 `openclaw
...
| **`hot`**              | 안전한 변경 사항만 핫 적용합니다. 재시작이 필요하면 경고를 로그에 남깁니다. 사용자가 처리합니다. |
...
`$include`를 통해 참조되는 소스 파일을 편집하면 OpenClaw는 평탄화된 인메모리 뷰가 아니라 소스에 작성된 레이아웃에서 리로드를 계획합니다. 따라서 `plugins: { $include: "./plugins.json5" }`처럼 단일 최상위 섹션이 자체 포함 파일에 있더라도 핫 리로드 결정(핫 적용과 재시작)을 예측 가능하게 유지할 수 있습니다. 소스 레이아웃이 모호하면 리로드 계획은 안전하게 실패합니다.

## 구성 RPC(프로그래밍 방식 업데이트)

Gateway API를 통해 구성을 작성하는 도구에는 다음 흐름을 권장합니다.

- `config.schema.lookup`으로 하나의 하위 트리를 검사합니다(얕은 스키마 노드 + 하위 항목 요약).
...
- 명시적 자체 업데이트와 재시작에는 `update.run`을 사용합니다. 재시작 후 세션이 후속 턴 하나를 실행해야 하면 `continuationMessage`를 포함하세요.
- `update.status`로 최신 업데이트 재시작 센티널을 검사하고 재시작 후 실행 중인 버전을 확인합니다.

Agent는 정확한 필드 수준 문서와 제약 조건을 확인할 때 `config.schema.lookup`을 첫 번째 진입점으로 취급해야 합니다. 더 넓은 구성 맵, 기본값, 또는 전용 하위 시스템 참조 링크가 필요할 때는 [구성 참조](/ko/gateway/configuration-reference)를 사용하세요.

<Note>
컨트롤 플레인 쓰기(`config.apply`, `config.patch`, `update.run`)는 `deviceId+clientIp`당 60초에 3개 요청으로 속도가 제한됩니다. 재시작 요청은 병합된 다음 재시작 주기 사이에 30초 쿨다운을 적용합니다. `update.status`는 읽기 전용이지만, 재시작 센티널에 업데이트 단계 요약과 명령 출력 끝부분이 포함될 수 있으므로 관리자 범위입니다.
</Note>
...
<Accordion title="구성 값의 환경 변수 치환">
  `${VAR_NAME}`으로 모든 구성 문자열 값에서 환경 변수를 참조합니다.
...
- 일치하는 대문자 이름만 사용합니다: `[A-Z_][A-Z0-9_]*`
...
SecretRef 세부 정보(`env`/`file`/`exec`용 `secrets.providers` 포함)는 [시크릿 관리](/ko/gateway/secrets)에 있습니다. 지원되는 자격 증명 경로는 [SecretRef 자격 증명 표면](/ko/reference/secretref-credential-surface)에 나열되어 있습니다.
</Accordion>
