...
레이아웃은 OpenClaw가 소유한 쓰기에서 지원되지 않습니다. 원자적 쓰기는
심볼릭 링크를 보존하는 대신 해당 경로를 대체할 수 있습니다. 구성을 기본
상태 디렉터리 밖에 유지한다면 `OPENCLAW_CONFIG_PATH`가 실제 파일을 직접 가리키도록 하세요.

파일이 없으면 OpenClaw는 안전한 기본값을 사용합니다. 구성을 추가하는 일반적인 이유는 다음과 같습니다.

- 채널을 연결하고 봇에게 메시지를 보낼 수 있는 사용자를 제어
...
`config.schema.lookup`을 사용해야 합니다. 작업 중심 안내에는 이 페이지를 사용하고,
더 넓은 필드 맵과 기본값은 [구성 참조](/ko/gateway/configuration-reference)를 사용하세요.

<Tip>
**구성이 처음이신가요?** 대화형 설정은 `openclaw onboard`로 시작하거나, 완전한 복사-붙여넣기 구성은 [구성 예시](/ko/gateway/configuration-examples) 가이드를 확인하세요.
</Tip>
...
<Tabs>
  ...
  <Tab title="제어 UI">
    [http://127.0.0.1:18789](http://127.0.0.1:18789)를 열고 **구성** 탭을 사용하세요.
    제어 UI는 실시간 구성 스키마에서 양식을 렌더링합니다. 여기에는 필드
    `title` / `description` 문서 메타데이터와 사용 가능한 경우 Plugin 및 채널 스키마가
    포함되며, 탈출구로 **원시 JSON** 편집기도 제공합니다. 드릴다운
    UI와 다른 도구를 위해 Gateway는 `config.schema.lookup`도 노출하여
    경로 범위 스키마 노드 하나와 바로 아래 자식 요약을 가져올 수 있게 합니다.
  </Tab>
  <Tab title="직접 편집">
...
<Warning>
OpenClaw는 스키마와 완전히 일치하는 구성만 허용합니다. 알 수 없는 키, 잘못된 형식의 타입 또는 유효하지 않은 값이 있으면 Gateway가 **시작을 거부**합니다. 루트 수준의 유일한 예외는 `$schema`(문자열)이므로 편집기가 JSON Schema 메타데이터를 연결할 수 있습니다.
</Warning>
...
- 복구를 적용하려면 `openclaw doctor --fix`(또는 `--yes`) 실행

Gateway는 성공적으로 시작할 때마다 신뢰할 수 있는 마지막 정상 사본을 보관하지만,
시작과 핫 리로드는 이를 자동으로 복원하지 않습니다. `openclaw.json`이
검증에 실패하면(Plugin 로컬 검증 포함) Gateway 시작이 실패하거나
리로드를 건너뛰며 현재 런타임은 마지막으로 승인된 구성을 유지합니다.
접두사가 붙었거나 덮어쓴 구성을 복구하거나 마지막 정상 사본을
복원하려면 `openclaw doctor --fix`(또는 `--yes`)를 실행하세요. 후보에
`***` 같은 마스킹된 비밀 placeholder가 포함되어 있으면 마지막 정상 사본으로 승격하지 않습니다.
...
<AccordionGroup>
  ...
  <Accordion title="모델 선택 및 구성">
    ...
    - 기존 모델을 제거하지 않고 허용 목록 항목을 추가하려면 `openclaw config set agents.defaults.models '<json>' --strict-json --merge`를 사용하세요. 항목을 제거하게 되는 단순 교체는 `--replace`를 전달하지 않는 한 거부됩니다.
    - 모델 참조는 `provider/model` 형식을 사용합니다(예: `anthropic/claude-opus-4-6`).
    - `agents.defaults.imageMaxDimensionPx`는 대화 기록/도구 이미지 축소를 제어합니다(기본값 `1200`). 더 낮은 값은 보통 스크린샷이 많은 실행에서 비전 토큰 사용량을 줄입니다.
    - 채팅에서 모델을 전환하려면 [모델 CLI](/ko/concepts/models)를, 인증 순환과 대체 동작은 [모델 장애 조치](/ko/concepts/model-failover)를 확인하세요.
    - 사용자 지정/셀프 호스팅 제공자는 참조의 [사용자 지정 제공자](/ko/gateway/config-tools#custom-providers-and-base-urls)를 확인하세요.

  </Accordion>

  <Accordion title="봇에게 메시지를 보낼 수 있는 사용자 제어">
    DM 접근은 채널별로 `dmPolicy`를 통해 제어됩니다.

    - `"pairing"`(기본값): 알 수 없는 발신자에게 승인용 일회성 페어링 코드 제공
  ...
  <Accordion title="그룹 채팅 멘션 게이트 설정">
    그룹 메시지는 기본적으로 **멘션 필요**입니다. 에이전트별 트리거 패턴을 구성하고, 의도적으로 레거시 자동 최종 답장을 원하지 않는 한 보이는 방 답장은 기본 message-tool 경로에 유지하세요.
    ...
    - 보이는 답장 모드, 채널별 재정의, 자기 채팅 모드는 [전체 참조](/ko/gateway/config-channels#group-chat-mention-gating)를 확인하세요.
  ...
  <Accordion title="에이전트별 Skills 제한">
    ...
    - [Skills](/ko/tools/skills), [Skills 구성](/ko/tools/skills-config),
  ...
  <Accordion title="Gateway 채널 상태 모니터링 조정">
    ...
    - 전역 모니터를 비활성화하지 않고 한 채널이나 계정의 자동 재시작을 비활성화하려면 `channels.<provider>.healthMonitor.enabled` 또는 `channels.<provider>.accounts.<id>.healthMonitor.enabled`를 사용하세요.
    - 운영 디버깅은 [상태 검사](/ko/gateway/health)를, 모든 필드는 [전체 참조](/ko/gateway/configuration-reference#gateway)를 확인하세요.
  ...
  <Accordion title="Gateway WebSocket 핸드셰이크 타임아웃 조정">
    ...
    - 먼저 시작/이벤트 루프 멈춤을 해결하는 것을 선호하세요. 이 조정값은 정상적이지만 워밍업 중 느린 호스트를 위한 것입니다.
  ...
  <Accordion title="세션 및 재설정 구성">
    ...
    - 범위 지정, ID 링크, 전송 정책은 [세션 관리](/ko/concepts/session)를 확인하세요.
    - 모든 필드는 [전체 참조](/ko/gateway/config-agents#session)를 확인하세요.
  ...
  <Accordion title="공식 iOS 빌드용 릴레이 기반 푸시 활성화">
    ...
    - Gateway가 외부 릴레이를 통해 `push.test`, 웨이크 알림, 재연결 웨이크를 보낼 수 있게 합니다.
    ...
    - 등록 및 전송 트래픽이 동일한 릴레이 배포에 도달하도록 공식/TestFlight iOS 빌드에 포함된 릴레이 기본 URL과 일치해야 합니다.
    ...
    5. Gateway가 릴레이 핸들과 전송 권한을 저장한 뒤 `push.test`, 웨이크 알림, 재연결 웨이크에 사용합니다.

    운영 참고 사항:

    - iOS 앱을 다른 Gateway로 전환하는 경우, 앱이 해당 Gateway에 바인딩된 새 릴레이 등록을 게시할 수 있도록 앱을 다시 연결하세요.
    - 다른 릴레이 배포를 가리키는 새 iOS 빌드를 배포하면, 앱은 이전 릴레이 원점을 재사용하는 대신 캐시된 릴레이 등록을 새로 고칩니다.

    호환성 참고 사항:

    - `OPENCLAW_APNS_RELAY_BASE_URL` 및 `OPENCLAW_APNS_RELAY_TIMEOUT_MS`는 임시 환경 재정의로 계속 작동합니다.
    - `OPENCLAW_APNS_RELAY_ALLOW_HTTP=true`는 local loopback 전용 개발 우회로로 유지됩니다. HTTP 릴레이 URL을 구성에 영구 저장하지 마세요.
  ...
  <Accordion title="Cron 작업 구성">
    ...
    - 기능 개요 및 CLI 예시는 [Cron 작업](/ko/automation/cron-jobs)을 참고하세요.
  ...
  <Accordion title="Webhook 설정(후크)">
    ...
    보안 참고 사항:
    - 모든 후크/Webhook 페이로드 콘텐츠를 신뢰할 수 없는 입력으로 취급하세요.
    - 전용 `hooks.token`을 사용하고 공유 Gateway 토큰을 재사용하지 마세요.
    - 후크 인증은 헤더 전용입니다(`Authorization: Bearer ...` 또는 `x-openclaw-token`). 쿼리 문자열 토큰은 거부됩니다.
    - `hooks.path`는 `/`일 수 없습니다. Webhook 인그레스는 `/hooks`와 같은 전용 하위 경로에 유지하세요.
    - 엄격하게 범위가 제한된 디버깅을 수행하는 경우가 아니라면 안전하지 않은 콘텐츠 우회 플래그(`hooks.gmail.allowUnsafeExternalContent`, `hooks.mappings[].allowUnsafeExternalContent`)를 비활성화 상태로 유지하세요.
    - `hooks.allowRequestSessionKey`를 활성화하는 경우, 호출자가 선택한 세션 키의 범위를 제한하도록 `hooks.allowedSessionKeyPrefixes`도 설정하세요.
    - 후크 기반 에이전트에는 강력한 최신 모델 티어와 엄격한 도구 정책을 사용하는 것이 좋습니다(예: 메시징 전용 및 가능한 경우 샌드박싱).

    모든 매핑 옵션 및 Gmail 통합은 [전체 참조](/ko/gateway/configuration-reference#hooks)를 참고하세요.
  ...
  <Accordion title="다중 에이전트 라우팅 구성">
    ...
    바인딩 규칙 및 에이전트별 액세스 프로필은 [다중 에이전트](/ko/concepts/multi-agent)와 [전체 참조](/ko/gateway/config-agents#multi-agent-routing)를 참고하세요.
  ...
  <Accordion title="구성을 여러 파일로 분할($include)">
    ...
    - **OpenClaw 소유 쓰기**: 쓰기가 `plugins: { $include: "./plugins.json5" }`처럼 단일 파일 include로 뒷받침되는 하나의 최상위 섹션만 변경하는 경우, OpenClaw는 해당 포함 파일을 업데이트하고 `openclaw.json`은 그대로 둡니다.
    - **지원되지 않는 쓰기 관통**: 루트 include, include 배열, 형제 재정의가 있는 include는 OpenClaw 소유 쓰기에서 구성을 평탄화하는 대신 닫힌 상태로 실패합니다.
    - **제한**: `$include` 경로는 `openclaw.json`을 포함하는 디렉터리 아래로 확인되어야 합니다. 여러 머신 또는 사용자 간에 트리를 공유하려면 include가 참조할 수 있는 추가 디렉터리의 경로 목록(POSIX에서는 `:`, Windows에서는 `;`)을 `OPENCLAW_INCLUDE_ROOTS`로 설정하세요. 심볼릭 링크는 확인된 뒤 다시 검사되므로, 경로 문자열상 구성 디렉터리 안에 있더라도 실제 대상이 모든 허용된 루트를 벗어나면 계속 거부됩니다.
...
직접 파일 편집은 검증될 때까지 신뢰할 수 없는 것으로 취급됩니다. 감시기는 편집기의 임시 쓰기/이름 변경 변동이 안정될 때까지 기다린 뒤 최종 파일을 읽고, 잘못된 외부 편집은 `openclaw.json`을 다시 쓰지 않고 거부합니다. OpenClaw 소유 구성 쓰기도 쓰기 전에 동일한 스키마 게이트를 사용합니다. `gateway.mode` 삭제 또는 파일 크기를 절반 이하로 줄이는 것과 같은 파괴적인 덮어쓰기는 거부되고 검사를 위해 `.rejected.*`로 저장됩니다.
...
대부분의 필드는 다운타임 없이 핫 적용됩니다. `hybrid` 모드에서는 재시작이 필요한 변경 사항이 자동으로 처리됩니다.
...
소스 파일이 `$include`를 통해 참조될 때 해당 파일을 편집하면, OpenClaw는 평탄화된 인메모리 뷰가 아니라 소스에 작성된 레이아웃을 기준으로 다시 로드를 계획합니다.
따라서 `plugins: { $include: "./plugins.json5" }`처럼 단일 최상위 섹션이 자체 포함 파일에 있더라도 핫 리로드 결정(핫 적용 vs 재시작)을 예측 가능하게 유지합니다. 소스 레이아웃이 모호하면 다시 로드 계획은 실패 시 닫힌 상태로 처리됩니다.

## 구성 RPC(프로그래밍 방식 업데이트)

Gateway API를 통해 구성을 쓰는 도구의 경우 다음 흐름을 권장합니다.

- `config.schema.lookup`: 하나의 하위 트리를 검사합니다(얕은 스키마 노드 + 하위 항목 요약).
- `config.get`: 현재 스냅샷과 `hash`를 가져옵니다.
- `config.patch`: 부분 업데이트에 사용합니다(JSON 병합 패치: 객체는 병합, `null`은 삭제, 배열은 교체).
- `config.apply`: 전체 구성을 교체하려는 경우에만 사용합니다.
- `update.run`: 명시적인 자체 업데이트와 재시작에 사용합니다. 재시작 후 세션이 후속 턴 하나를 실행해야 한다면 `continuationMessage`를 포함합니다.
- `update.status`: 최신 업데이트 재시작 센티널을 검사하고 재시작 후 실행 중인 버전을 확인합니다.

에이전트는 정확한 필드 수준 문서와 제약 조건을 확인하기 위한 첫 단계로 `config.schema.lookup`을 사용해야 합니다. 더 넓은 구성 맵, 기본값, 또는 전용 하위 시스템 참조 링크가 필요할 때는 [구성 참조](/ko/gateway/configuration-reference)를 사용하세요.

<Note>
제어 플레인 쓰기(`config.apply`, `config.patch`, `update.run`)는 `deviceId+clientIp`당 60초에 3개 요청으로 속도가 제한됩니다. 재시작 요청은 병합된 뒤 재시작 주기 사이에 30초의 쿨다운을 적용합니다.
...
OpenClaw는 부모 프로세스와 다음 위치에서 env var를 읽습니다.

- 현재 작업 디렉터리의 `.env`(있는 경우)
- `~/.openclaw/.env`(전역 폴백)

두 파일 모두 기존 env var를 재정의하지 않습니다. 구성에서 인라인 env var도 설정할 수 있습니다.
...
<Accordion title="구성 값의 env var 치환">
  모든 구성 문자열 값에서 `${VAR_NAME}`으로 env var를 참조합니다.
...
<Accordion title="시크릿 참조(env, file, exec)">
...
SecretRef 세부 정보(`env`/`file`/`exec`의 `secrets.providers` 포함)는 [시크릿 관리](/ko/gateway/secrets)에 있습니다.
지원되는 자격 증명 경로는 [SecretRef 자격 증명 표면](/ko/reference/secretref-credential-surface)에 나와 있습니다.
</Accordion>
