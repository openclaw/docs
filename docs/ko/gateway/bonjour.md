...
plugin이 LAN 광고를 담당합니다. macOS 호스트에서는 자동으로 시작되며,
Linux, Windows, 컨테이너화된 Gateway 배포에서는 명시적으로 선택해야 합니다. 네트워크 간 검색의 경우, 동일한
비컨을 구성된 광역 DNS-SD 도메인을 통해 게시할 수도 있습니다. 검색은
여전히 최선 노력 방식이며 SSH 또는 Tailnet 기반 연결을 대체하지 **않습니다**.

## Tailscale를 통한 광역 Bonjour(유니캐스트 DNS-SD)

노드와 Gateway가 서로 다른 네트워크에 있으면 멀티캐스트 mDNS는 경계를
넘지 못합니다. Tailscale에서 **유니캐스트 DNS‑SD**
("광역 Bonjour")로 전환하면 동일한 검색 UX를 유지할 수 있습니다.
...
2. 전용 존 아래에 `_openclaw-gw._tcp`용 DNS‑SD 레코드를 게시합니다
   (예: `openclaw.internal.`).
3. 클라이언트(iOS 포함)에 대해 선택한 도메인이 해당 DNS 서버를 통해
   확인되도록 Tailscale **분할 DNS**를 구성합니다.

OpenClaw는 모든 검색 도메인을 지원합니다. `openclaw.internal.`은 단지 예시입니다.
...
- Gateway의 Tailscale 인터페이스에서만 포트 53에서 수신
...
검색 도메인의 `_openclaw-gw._tcp`를 탐색할 수 있습니다.
...
DNS-SD 게시 책임은 계속 Gateway에 있습니다.
...
- TLS 핀 고정은 광고된 `gatewayTlsSha256`가 이전에 저장된 핀을 재정의하도록 허용해서는 절대 안 됩니다.
- iOS/Android 노드는 검색 기반 직접 연결을 **TLS 전용**으로 취급하고, 처음 보는 지문을 신뢰하기 전에 명시적인 사용자 확인을 요구해야 합니다.
...
Bonjour는 시스템 호스트 이름이 유효한 DNS 라벨이면 광고되는 `.local` 호스트에
시스템 호스트 이름을 사용합니다. 시스템 호스트 이름에 공백, 밑줄 또는 기타
잘못된 DNS 라벨 문자가 포함되어 있으면 OpenClaw는 `openclaw.local`로 대체합니다. 명시적
호스트 라벨이 필요하면 Gateway를 시작하기 전에
...
로컬 앱과 근처의 iOS/Android 노드는 같은 LAN 검색에 의존하는 경우가 많으므로,
macOS 호스트에서 빈 구성으로 Gateway를 시작하면 Bonjour가 자동으로 시작됩니다.

Linux, Windows 또는 다른 비 macOS 호스트에서 같은 LAN 자동 검색이 유용한 경우
...
활성화되면 Bonjour는 `discovery.mdns.mode`를 사용해 게시할 TXT 메타데이터의
양을 결정합니다. 기본 모드는 `minimal`입니다. 로컬 클라이언트에
`cliPath` 또는 `sshPort` 힌트가 필요할 때만 `full`을 사용하고, plugin 활성화 상태를
변경하지 않고 LAN 멀티캐스트를 억제하려면 `off`를 사용하세요.

## Bonjour를 비활성화해야 하는 경우

LAN 멀티캐스트 광고가 필요 없거나, 사용할 수 없거나,
...
DNS-SD를 통해 접근 가능하지만, LAN 자동 검색은 신뢰할 수 없습니다.
...
컨테이너에서 LAN 멀티캐스트 광고를 자동으로 비활성화합니다. Docker 브리지 네트워크는
일반적으로 컨테이너와 LAN 사이에서 mDNS 멀티캐스트(`224.0.0.251:5353`)를
전달하지 않으므로, 컨테이너에서 광고해도 검색이 작동하는 경우는 드뭅니다.

중요한 주의 사항:

- Bonjour는 macOS 호스트에서는 자동 시작되고, 그 외 환경에서는 명시적으로 선택해야 합니다. 비활성화된 상태로
  두어도 Gateway는 중지되지 않으며, LAN 멀티캐스트 광고만 건너뜁니다.
- Bonjour를 비활성화해도 `gateway.bind`는 변경되지 않습니다. Docker는 게시된 호스트 포트가 작동하도록
  여전히 기본값으로 `OPENCLAW_GATEWAY_BIND=lan`을 사용합니다.
- Bonjour를 비활성화해도 광역 DNS-SD는 비활성화되지 않습니다. Gateway와 노드가 같은 LAN에 있지 않을 때는 광역 검색
  또는 Tailnet을 사용하세요.
...
1. Gateway가 자동, 강제 켜짐 또는 강제 꺼짐 모드 중 어느 모드에서 실행 중인지 확인합니다.
...
4. Docker에서 Bonjour plugin을 의도적으로 활성화하고
   `OPENCLAW_DISABLE_BONJOUR=0`으로 광고를 강제한 경우, 호스트에서 멀티캐스트를 테스트합니다.
   ...
   취소가 표시되면 `OPENCLAW_DISABLE_BONJOUR=1`로 복원하고 직접 경로 또는
   Tailnet 경로를 사용하세요.
...
- **광고자가 probing/announcing에 멈춤**: 멀티캐스트가 차단된 호스트,
  ...
  발표되지 않은 상태에 남을 수 있습니다. OpenClaw는 몇 번 재시도한 다음 광고자를 계속 다시 시작하는 대신
  현재 Gateway 프로세스에 대해 Bonjour를 비활성화합니다.
- **Docker 브리지 네트워킹**: 감지된 컨테이너에서 Bonjour가 자동으로 비활성화됩니다.
  호스트, macvlan 또는 기타 mDNS 가능 네트워크에서만
  `OPENCLAW_DISABLE_BONJOUR=0`을 설정하세요.
- **절전 / 인터페이스 변동**: macOS가 mDNS 결과를 일시적으로 잃을 수 있습니다. 다시 시도하세요.
- **탐색은 되지만 확인 실패**: 머신 이름을 단순하게 유지하세요(이모지나
  문장 부호 피하기). 그런 다음 Gateway를 다시 시작하세요. 서비스 인스턴스 이름은
...
- macOS 호스트는 기본적으로 번들된 LAN 검색 plugin을 자동 시작합니다.
- `openclaw plugins enable bonjour`는 기본적으로 활성화되지 않는 호스트에서 번들된 LAN 검색 plugin을 활성화합니다.
