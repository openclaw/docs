...
title: 인증
x-i18n:
    generated_at: "2026-05-06T06:24:15Z"
    model: gpt-5.5
    provider: openai
...
<Note>
이 페이지는 **모델 제공자** 인증 참조(API 키, OAuth, Claude CLI 재사용, Anthropic 설정 토큰)입니다. **Gateway 연결** 인증(토큰, 비밀번호, trusted-proxy)은 [구성](/ko/gateway/configuration) 및 [신뢰할 수 있는 프록시 인증](/ko/gateway/trusted-proxy-auth)을 참고하세요.
</Note>

OpenClaw는 모델 제공자를 위한 OAuth와 API 키를 지원합니다. 항상 실행되는 Gateway
호스트의 경우 API 키가 일반적으로 가장 예측 가능한 옵션입니다. 구독/OAuth
흐름도 제공자 계정 모델과 일치할 때 지원됩니다.

전체 OAuth 흐름과 저장소 레이아웃은 [/concepts/oauth](/ko/concepts/oauth)를
참고하세요.
SecretRef 기반 인증(`env`/`file`/`exec` 제공자)은 [비밀 정보 관리](/ko/gateway/secrets)를 참고하세요.
`models status --probe`에서 사용하는 자격 증명 적격성/이유 코드 규칙은
[인증 자격 증명 의미 체계](/ko/auth-credential-semantics)를 참고하세요.

## 권장 설정(API 키, 모든 제공자)

장기간 실행되는 Gateway를 운영한다면 선택한 제공자의 API 키로 시작하세요.
특히 Anthropic의 경우 API 키 인증은 여전히 가장 예측 가능한 서버 설정이지만,
OpenClaw는 로컬 Claude CLI 로그인을 재사용하는 것도 지원합니다.
...
그런 다음 데몬(또는 Gateway 프로세스)을 다시 시작하고 다시 확인합니다.
...
env var를 직접 관리하고 싶지 않다면 온보딩에서 데몬 사용을 위한
API 키를 저장할 수 있습니다: `openclaw onboard`.

env 상속(`env.shellEnv`, `~/.openclaw/.env`, systemd/launchd)에 대한 자세한 내용은
[도움말](/ko/help)을 참고하세요.

## Anthropic: Claude CLI 및 토큰 호환성

Anthropic 설정 토큰 인증은 지원되는 토큰 경로로 OpenClaw에서 계속 사용할 수 있습니다.
이후 Anthropic 직원이 OpenClaw 방식의 Claude CLI 사용이 다시 허용된다고 알려왔으므로,
Anthropic이 새 정책을 게시하지 않는 한 OpenClaw는 이 통합에서 Claude CLI 재사용과
`claude -p` 사용을 승인된 것으로 취급합니다. 호스트에서 Claude CLI 재사용이 가능하다면
이제 해당 경로가 권장됩니다.

장기간 실행되는 Gateway 호스트의 경우 Anthropic API 키가 여전히 가장 예측 가능한
설정입니다. 같은 호스트의 기존 Claude 로그인을 재사용하려면 onboarding/configure에서
...
OpenClaw는 런타임에 표준 `version` + `profiles` 형태를 기대합니다. 이전 설치에 `{ "openrouter": { "apiKey": "..." } }` 같은 플랫 파일이 아직 있다면 `openclaw doctor --fix`를 실행하여 `openrouter:default` API 키 프로필로 다시 작성하세요. doctor는 원본 옆에 `.legacy-flat.*.bak` 복사본을 보관합니다. `baseUrl`, `api`, 모델 ID, 헤더, 타임아웃 같은 엔드포인트 세부 정보는 `auth-profiles.json`이 아니라 `openclaw.json` 또는 `models.json`의 `models.providers.<id>` 아래에 있어야 합니다.
...
자동화 친화적 확인(만료됨/누락 시 종료 코드 `1`, 곧 만료 시 `2`):
...
- 인증은 있지만 OpenClaw가 해당 제공자에 대해 프로브 가능한 모델 후보를 확인할 수 없으면,
  프로브는 `status: no_model`을 보고합니다.
- 속도 제한 쿨다운은 모델 범위일 수 있습니다. 한 모델에서 쿨다운 중인 프로필도
  같은 제공자의 형제 모델에서는 여전히 사용할 수 있습니다.
...
- 따라서 OpenClaw는 Anthropic이 새 정책을 게시하지 않는 한 Anthropic 기반 실행에서
  Claude CLI 재사용과 `claude -p` 사용을 승인된 것으로 취급합니다.
- 장기간 실행되는 Gateway 호스트와 명시적인 서버 측 과금 제어에는 Anthropic API 키가
  가장 예측 가능한 선택지로 남아 있습니다.
...
일부 제공자는 API 호출이 제공자 속도 제한에 걸렸을 때 대체 키로 요청을 다시 시도하는 것을
지원합니다.
...
쿨다운 문제를 디버그할 때 속도 제한 쿨다운이 전체 제공자 프로필이 아니라
...
Anthropic 프로필이 누락된 경우 **Gateway 호스트**에 Anthropic API 키를 구성하거나
Anthropic 설정 토큰 경로를 설정한 다음 다시 확인하세요.
...
어떤 프로필이 만료되는지 확인하려면 `openclaw models status`를 실행하세요.
Anthropic 토큰 프로필이 없거나 만료된 경우 setup-token을 통해 해당 설정을 갱신하거나
Anthropic API 키로 마이그레이션하세요.
