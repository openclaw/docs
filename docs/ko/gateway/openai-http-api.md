...
title: OpenAI 채팅 완성
x-i18n:
    generated_at: "2026-05-06T06:26:26Z"
    model: gpt-5.5
    provider: openai
...
내부적으로 요청은 일반 Gateway 에이전트 실행(`openclaw agent`와 동일한 코드 경로)으로 실행되므로, 라우팅/권한/구성이 Gateway와 일치합니다.
...
- 신뢰할 수 있는 ID 포함 HTTP 인증(`gateway.auth.mode="trusted-proxy"`):
  구성된 ID 인식 프록시를 통해 라우팅하고, 필요한 ID 헤더를 삽입하게 합니다.
...
- `gateway.auth.mode="trusted-proxy"`일 때 HTTP 요청은 구성된 신뢰할 수 있는 프록시 소스에서 와야 합니다. 동일 호스트 loopback 프록시는 명시적으로 `gateway.auth.trustedProxy.allowLoopback = true`가 필요합니다.
...
- 신뢰할 수 있는 ID 포함 HTTP 모드(예: 신뢰할 수 있는 프록시 인증 또는 `gateway.auth.mode="none"`)는 `x-openclaw-scopes`가 있으면 이를 존중하고, 없으면 일반 운영자 기본 범위 집합으로 폴백합니다.
...
기본적으로 엔드포인트는 **요청별로 상태 비저장**입니다(호출할 때마다 새 세션 키가 생성됩니다).
...
<AccordionGroup>
  ...
  <Accordion title="백엔드 모델을 어떻게 재정의하나요?">
    ...
    이를 생략하면 선택한 에이전트는 일반적으로 구성된 모델 선택으로 실행됩니다.
  ...
  <Accordion title="임베딩은 이 계약에 어떻게 맞나요?">
    ...
    해당 헤더가 없으면 요청은 선택한 에이전트의 일반 임베딩 설정으로 전달됩니다.
...
서버 전송 이벤트(Server-Sent Events, SSE)를 받으려면 `stream: true`를 설정하세요.
...
- 해당 에이전트에 특정 백엔드 공급자/모델을 원하면 에이전트의 일반 기본 모델을 설정하거나 `x-openclaw-model`을 보내세요.
...
이것이 `openclaw/default`를 반환하면, 대부분의 Open WebUI 설정은 동일한 기준 URL과 토큰으로 연결할 수 있습니다.
...
- `/v1/models`는 원시 공급자 카탈로그가 아니라 OpenClaw 에이전트 대상을 반환합니다.
- `openclaw/default`는 항상 존재하므로, 하나의 안정적인 ID가 여러 환경에서 작동합니다.
- 백엔드 공급자/모델 재정의는 OpenAI `model` 필드가 아니라 `x-openclaw-model`에 있어야 합니다.
