...
summary: 에이전트 기본값, 멀티 에이전트 라우팅, 세션, 메시지, 대화 구성
title: 구성 — 에이전트
x-i18n:
    generated_at: "2026-05-12T23:30:38Z"
    model: gpt-5.5
    provider: openai
...
- `agents.defaults.startupContext.*`:
  최근 일일 `memory/*.md` 파일을 포함한 일회성 재설정/시작 모델 실행 프렐류드입니다. 단순 채팅 `/new` 및 `/reset` 명령은 모델을 호출하지 않고 확인 응답만 합니다.
- `skills.limits.*`:
...
- `postCompactionSections`: Compaction 후 다시 주입할 선택적 AGENTS.md H2/H3 섹션 이름입니다. 기본값은 `["Session Startup", "Red Lines"]`이며, 다시 주입을 비활성화하려면 `[]`로 설정하세요. 설정하지 않았거나 명시적으로 해당 기본 쌍으로 설정한 경우, 이전 `Every Session`/`Safety` 제목도 레거시 폴백으로 허용됩니다.
...
`remote` 모드에서는 시드 단계 이후 OpenClaw 외부에서 수행된 호스트 로컬 편집 사항이 샌드박스로 자동 동기화되지 않습니다.
...
**컨테이너 기본값은 `network: "none"`입니다** — 에이전트에 아웃바운드 접근이 필요한 경우 `"bridge"`(또는 사용자 지정 브리지 네트워크)로 설정하세요.
...
- `messages.statusReactions.enabled`: Slack, Discord, Telegram에서 수명 주기 상태 반응을 활성화합니다.
  Slack 및 Discord에서는 설정하지 않으면 확인 반응이 활성화된 경우 상태 반응도 활성 상태로 유지됩니다.
