...
- LINE 서명 검증은 본문에 의존하므로(raw body에 대한 HMAC), OpenClaw는 검증 전에 엄격한 사전 인증 본문 제한과 시간 제한을 적용합니다.
...
- `channels.line.allowFrom`: DM에 허용된 LINE 사용자 ID. `dmPolicy: "open"`에는 `["*"]`가 필요합니다.
- `channels.line.groupPolicy`: `allowlist | open | disabled`
- `channels.line.groupAllowFrom`: 그룹에 허용된 LINE 사용자 ID
- 그룹별 재정의: `channels.line.groups.<groupId>.allowFrom`
- 런타임 참고: `channels.line`이 완전히 누락된 경우, 런타임은 그룹 검사에 대해 `groupPolicy="allowlist"`로 대체합니다(`channels.defaults.groupPolicy`가 설정되어 있어도).
...
일반 미디어 전송은 LINE 전용 경로를 사용할 수 없는 경우 기존 이미지 전용 경로로 대체됩니다.
