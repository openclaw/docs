...
read_when:
    - iMessage 지원 설정하기
    - iMessage 송수신 디버깅
summary: imsg를 통한 네이티브 iMessage 지원(stdio를 통한 JSON-RPC). 호스트 요구 사항이 맞는 경우 새로운 OpenClaw iMessage 설정에 권장됩니다.
title: iMessage
x-i18n:
...
<Note>
새 OpenClaw iMessage 배포에서는 로그인된 macOS Messages 호스트에서 `imsg`를 실행할 수 있을 때 여기서 시작하세요. BlueBubbles는 HTTP 서버, Webhook 또는 더 풍부한 비공개 API 작업에 의존하는 기존 설정을 위한 레거시 폴백으로 계속 사용할 수 있습니다.
</Note>

상태: 네이티브 외부 CLI 통합. Gateway가 `imsg rpc`를 실행하고 stdio의 JSON-RPC로 통신합니다(별도 데몬/포트 없음).

<CardGroup cols={3}>
  <Card title="BlueBubbles (legacy fallback)" icon="message-circle" href="/ko/channels/bluebubbles">
...
- Messages는 `imsg`를 실행하는 Mac에서 로그인되어 있어야 합니다.
...
<Tabs>
  ...
  <Tab title="Group policy + mentions">
    ...
    런타임 참고: `channels.imessage`가 완전히 없으면 런타임은 `groupPolicy="allowlist"`로 폴백하고 경고를 기록합니다(`channels.defaults.groupPolicy`가 설정되어 있어도 마찬가지).
...
<AccordionGroup>
  <Accordion title="Dedicated bot macOS user (separate iMessage identity)">
    ...
    5. `channels.imessage.accounts.<id>.cliPath` 및 `.dbPath`가 해당 사용자 프로필을 가리키도록 합니다.
...
```
    ...
    먼저 호스트 키가 신뢰되도록 하여(예: `ssh bot@mac-mini.tailnet-1234.ts.net`) `known_hosts`가 채워졌는지 확인하세요.
...
iMessage는 기본적으로 채널에서 시작되는 구성 쓰기를 허용합니다(`commands.config: true`일 때 `/config set|unset`).
...
```
    ...
    프로브가 RPC 미지원이라고 보고하면 `imsg`를 업데이트하세요.
