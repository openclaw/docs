---
read_when:
    - BlueBubbles에서 번들 iMessage Plugin으로 이전 계획하기
    - BlueBubbles 구성 키를 iMessage에 해당하는 항목으로 변환하기
    - iMessage Plugin을 활성화하기 전에 imsg 확인하기
summary: 페어링, 허용 목록 또는 그룹 바인딩을 잃지 않고 이전 BlueBubbles 설정을 번들 iMessage Plugin으로 마이그레이션합니다.
title: BlueBubbles에서 오기
x-i18n:
...
| _(해당 없음)_                                                    | `channels.imessage.remoteAttachmentRoots` | SCP 가져오기에 `remoteHost`가 설정된 경우에만 사용됩니다.                                                                                                                                                                                                                                                                                                                                  |
...
- 특정 그룹이 런타임에서 처음 드롭될 때 `chat_id`별로 한 번 발생하는 `warn`입니다. chat_id와 이를 허용하기 위해 `groups`에 추가할 정확한 키를 명시합니다.
...
| 재시작 후 인바운드 복구                             | ✅ (Webhook 재생 + 기록 가져오기)   | ✅ (자동: since_rowid + 중복 제거로 누락분 재생; 로컬에서는 더 넓은 창)       |
