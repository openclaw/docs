...
이 Plugin은 Synology Chat 발신 Webhook에서 인바운드 메시지를 받고,
Synology Chat 수신 Webhook을 통해 답장을 보냅니다.

## 번들 Plugin

Synology Chat은 현재 OpenClaw 릴리스에 번들 Plugin으로 포함되어 있으므로, 일반
패키지 빌드에서는 별도 설치가 필요하지 않습니다.

Synology Chat이 제외된 이전 빌드 또는 사용자 지정 설치를 사용하는 경우,
수동으로 설치하세요.
...
- OpenClaw는 발신 Webhook 토큰을 `body.token`, 그다음
  `?token=...`, 그다음 헤더에서 받습니다.
- 허용되는 헤더 형식:
  - `x-synology-token`
  - `x-webhook-token`
...
URL 기반 파일 전달을 통한 미디어 전송이 지원됩니다.
아웃바운드 파일 URL은 `http` 또는 `https`를 사용해야 하며, 비공개 또는 그 밖의 차단된 네트워크 대상은 OpenClaw가 URL을 NAS Webhook으로 전달하기 전에 거부됩니다.

## 다중 계정

여러 Synology Chat 계정은 `channels.synology-chat.accounts` 아래에서 지원됩니다.
각 계정은 토큰, 수신 URL, Webhook 경로, DM 정책, 제한을 재정의할 수 있습니다.
다이렉트 메시지 세션은 계정 및 사용자별로 격리되므로, 서로 다른 두 Synology 계정의 같은 숫자 `user_id`는 대화 기록 상태를 공유하지 않습니다.
활성화된 각 계정에 고유한 `webhookPath`를 지정하세요. 이제 OpenClaw는 정확히 중복되는 경로를 거부하고,
다중 계정 설정에서 공유 Webhook 경로만 상속하는 명명된 계정은 시작하지 않습니다.
명명된 계정에 레거시 상속이 의도적으로 필요한 경우 해당 계정 또는 `channels.synology-chat`에
`dangerouslyAllowInheritedWebhookPath: true`를 설정하세요. 하지만 정확히 중복되는 경로는 여전히 안전하게 거부됩니다. 계정별 명시적 경로를 권장합니다.
