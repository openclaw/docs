...
공식 Discord Gateway를 통해 다이렉트 메시지와 길드 채널을 사용할 준비가 되어 있습니다.

<CardGroup cols={3}>
  <Card title="페어링" icon="link" href="/ko/channels/pairing">
    Discord 다이렉트 메시지는 기본적으로 페어링 모드로 설정됩니다.
  </Card>
...
봇이 포함된 새 애플리케이션을 만들고, 봇을 서버에 추가한 뒤, OpenClaw에 페어링해야 합니다. 봇은 본인의 비공개 서버에 추가하는 것을 권장합니다. 아직 서버가 없다면 [먼저 만드세요](https://support.discord.com/hc/en-us/articles/204849977-How-do-I-create-a-server) (**Create My Own > For me and my friends** 선택).
...
<Steps>
  ...
  <Step title="권한 있는 인텐트 활성화">
    계속 **Bot** 페이지에서 아래로 스크롤해 **Privileged Gateway Intents**로 이동한 뒤 다음을 활성화합니다.

    - **Message Content Intent** (필수)
    - **Server Members Intent** (권장, 역할 허용 목록 및 이름-아이디 매칭에 필요)
    - **Presence Intent** (선택 사항, 현재 상태 업데이트에만 필요)

  </Step>

  <Step title="봇 토큰 복사">
    **Bot** 페이지에서 다시 위로 스크롤해 **Reset Token**을 클릭합니다.

    <Note>
    이름과 달리, 이 작업은 첫 토큰을 생성합니다. 실제로 "재설정"되는 것은 없습니다.
    </Note>
  ...
  <Step title="초대 URL 생성 및 서버에 봇 추가">
    사이드바에서 **OAuth2**를 클릭합니다. 봇을 서버에 추가할 수 있도록 올바른 권한이 포함된 초대 URL을 생성합니다.
    ...
    **Text Permissions**
    ...
    이는 일반 텍스트 채널용 기본 권한 세트입니다. 포럼 또는 미디어 채널 워크플로처럼 스레드를 만들거나 이어가는 작업을 포함해 Discord 스레드에 게시할 계획이라면 **Send Messages in Threads**도 활성화합니다.
    하단에 생성된 URL을 복사해 브라우저에 붙여넣고, 서버를 선택한 뒤 **Continue**를 클릭해 연결합니다. 이제 Discord 서버에서 봇을 볼 수 있어야 합니다.

  </Step>

  <Step title="개발자 모드 활성화 및 아이디 수집">
    Discord 앱으로 돌아가 내부 아이디를 복사할 수 있도록 개발자 모드를 활성화해야 합니다.
  ...
  <Step title="서버 멤버의 다이렉트 메시지 허용">
    페어링이 작동하려면 Discord가 봇이 사용자에게 다이렉트 메시지를 보낼 수 있도록 허용해야 합니다. **서버 아이콘**을 오른쪽 클릭 → **Privacy Settings** → **Direct Messages**를 켭니다.

    이렇게 하면 서버 멤버(봇 포함)가 사용자에게 다이렉트 메시지를 보낼 수 있습니다. OpenClaw와 함께 Discord 다이렉트 메시지를 사용하려면 이 설정을 켜 둡니다. 길드 채널만 사용할 계획이라면 페어링 후 다이렉트 메시지를 비활성화해도 됩니다.

  </Step>

  <Step title="봇 토큰을 안전하게 설정(채팅으로 보내지 마세요)">
    Discord 봇 토큰은 비밀 값(비밀번호와 같음)입니다. 에이전트에게 메시지를 보내기 전에 OpenClaw를 실행하는 머신에 설정합니다.
...
```
    ...
    호스트가 Discord의 시작 애플리케이션 조회에 의해 차단되거나 속도 제한을 받는 경우, 시작 시 해당 REST 호출을 건너뛸 수 있도록 Developer Portal의 Discord 애플리케이션/클라이언트 아이디를 설정합니다. 기본 계정에는 `channels.discord.applicationId`를 사용하고, 여러 Discord 봇을 실행할 때는 `channels.discord.accounts.<accountId>.applicationId`를 사용합니다.
  ...
  <Step title="OpenClaw 설정 및 페어링">
    ...
    <Tabs>
      <Tab title="에이전트에게 요청">
        기존 채널(예: Telegram)에서 OpenClaw 에이전트와 채팅하고 다음과 같이 말합니다. Discord가 첫 채널이라면 대신 CLI / 설정 탭을 사용합니다.

        > "이미 설정에서 Discord 봇 토큰을 설정했습니다. User ID `<user_id>` 및 Server ID `<server_id>`로 Discord 설정을 완료해 주세요."
      </Tab>
      <Tab title="CLI / 설정">
        파일 기반 설정을 선호한다면 다음을 설정합니다.
...
```
        ...
        스크립트 기반 또는 원격 설정의 경우 `openclaw config patch --file ./discord.patch.json5 --dry-run`으로 동일한 JSON5 블록을 작성한 다음 `--dry-run` 없이 다시 실행합니다. 일반 텍스트 `token` 값이 지원됩니다. env/file/exec 제공자 전반에서 `channels.discord.token`에 SecretRef 값도 지원됩니다. [비밀 관리](/ko/gateway/secrets)를 참조하세요.

        여러 Discord 봇의 경우 각 봇 토큰과 애플리케이션 아이디를 해당 계정 아래에 둡니다. 최상위 `channels.discord.applicationId`는 계정에 상속되므로 모든 계정이 같은 애플리케이션 아이디를 사용해야 할 때만 그 위치에 설정합니다.
...
```
  ...
  <Step title="첫 다이렉트 메시지 페어링 승인">
    Gateway가 실행될 때까지 기다린 다음 Discord에서 봇에게 다이렉트 메시지를 보냅니다. 봇이 페어링 코드를 응답합니다.
...
```
    ...
    이제 Discord에서 다이렉트 메시지를 통해 에이전트와 채팅할 수 있어야 합니다.

  </Step>
</Steps>

<Note>
토큰 해석은 계정을 인식합니다. 설정 토큰 값이 env 폴백보다 우선합니다. `DISCORD_BOT_TOKEN`은 기본 계정에만 사용됩니다.
활성화된 두 Discord 계정이 같은 봇 토큰으로 해석되면, OpenClaw는 해당 토큰에 대해 하나의 Gateway 모니터만 시작합니다. 설정에서 제공된 토큰이 기본 env 폴백보다 우선합니다. 그렇지 않으면 첫 번째 활성 계정이 우선하며 중복 계정은 비활성화된 것으로 보고됩니다.
...
다이렉트 메시지가 작동하면 Discord 서버를 전체 작업 공간으로 설정해 각 채널이 고유한 컨텍스트를 가진 자체 에이전트 세션을 갖게 할 수 있습니다. 사용자와 봇만 있는 비공개 서버에 권장됩니다.

<Steps>
  <Step title="서버를 길드 허용 목록에 추가">
    이렇게 하면 에이전트가 다이렉트 메시지만이 아니라 서버의 모든 채널에서 응답할 수 있습니다.
...
```
  ...
  <Step title="@멘션 없이 응답 허용">
    기본적으로 에이전트는 길드 채널에서 @멘션될 때만 응답합니다. 비공개 서버라면 모든 메시지에 응답하기를 원할 가능성이 큽니다.

    길드 채널에서 일반 어시스턴트 최종 답변은 기본적으로 비공개로 유지됩니다. 표시되는 Discord 출력은 `message` 도구로 명시적으로 보내야 하므로, 에이전트는 기본적으로 지켜보다가 채널 답변이 유용하다고 판단할 때만 게시할 수 있습니다.

    이는 선택한 모델이 도구를 안정적으로 호출해야 함을 의미합니다. Discord에 입력 중 표시가 나타나고 로그에 토큰 사용량이 표시되지만 게시된 메시지가 없다면 세션 로그에서 `didSendViaMessagingTool: false`가 있는 어시스턴트 텍스트를 확인하세요. 이는 모델이 `message(action=send)`를 호출하는 대신 비공개 최종 답변을 생성했다는 의미입니다. 더 강력한 도구 호출 모델로 전환하거나 아래 설정을 사용해 레거시 자동 최종 답변을 복원하세요.

    <Tabs>
      <Tab title="에이전트에게 요청">
        > "이 서버에서 @멘션 없이 내 에이전트가 응답하도록 허용해 주세요"
      </Tab>
      <Tab title="설정">
...
```
        ...
        그룹/채널 방에 대한 레거시 자동 최종 답변을 복원하려면 `messages.groupChat.visibleReplies: "automatic"`을 설정합니다.
  ...
  <Step title="길드 채널의 메모리 계획">
    기본적으로 장기 메모리(MEMORY.md)는 다이렉트 메시지 세션에서만 로드됩니다. 길드 채널은 MEMORY.md를 자동으로 로드하지 않습니다.
    ...
    <Tabs>
      ...
      <Tab title="수동">
        모든 채널에서 공유 컨텍스트가 필요하다면 안정적인 지침을 `AGENTS.md` 또는 `USER.md`에 넣습니다(모든 세션에 주입됩니다). 장기 노트는 `MEMORY.md`에 보관하고 필요할 때 메모리 도구로 접근합니다.
      </Tab>
    </Tabs>
...
이제 Discord 서버에 채널을 몇 개 만들고 채팅을 시작하세요. 에이전트는 채널 이름을 볼 수 있으며, 각 채널은 고유한 격리 세션을 갖습니다. 따라서 워크플로에 맞게 `#coding`, `#home`, `#research` 등을 설정할 수 있습니다.

## 런타임 모델

- Gateway가 Discord 연결을 소유합니다.
- 답장 라우팅은 결정적입니다. Discord 인바운드 답장은 다시 Discord로 전달됩니다.
- Discord 길드/채널 메타데이터는 사용자에게 보이는 답장 접두사가 아니라 신뢰할 수 없는
  컨텍스트로 모델 프롬프트에 추가됩니다. 모델이 해당 엔벌로프를 다시 복사하면
...
<Tabs>
  <Tab title="DM policy">
    ...
    DM 정책이 open이 아니면 알 수 없는 사용자는 차단됩니다(`pairing` 모드에서는 페어링하라는 메시지가 표시됨).

    다중 계정 우선순위:

    - `channels.discord.accounts.default.allowFrom`은 `default` 계정에만 적용됩니다.
    - 계정이 하나인 경우 `allowFrom`이 기존 `dm.allowFrom`보다 우선합니다.
    - 명명된 계정은 자체 `allowFrom`과 기존 `dm.allowFrom`이 설정되지 않았을 때 `channels.discord.allowFrom`을 상속합니다.
    - 명명된 계정은 `channels.discord.accounts.default.allowFrom`을 상속하지 않습니다.

    기존 `channels.discord.dm.policy` 및 `channels.discord.dm.allowFrom`은 호환성을 위해 계속 읽힙니다. 접근을 변경하지 않고 수행할 수 있는 경우 `openclaw doctor --fix`가 이를 `dmPolicy` 및 `allowFrom`으로 마이그레이션합니다.
    ...
    기본 채널이 활성화된 경우 순수 숫자 ID는 일반적으로 채널 ID로 해석되지만, 계정의 유효 DM `allowFrom`에 나열된 ID는 호환성을 위해 사용자 DM 대상으로 취급됩니다.
  ...
  <Tab title="DM access groups">
    ...
    접근 그룹 이름은 메시지 채널 간에 공유됩니다. 멤버가 각 채널의 일반 `allowFrom` 구문으로 표현되는 정적 그룹에는 `type: "message.senders"`를 사용하거나, Discord 채널의 현재 `ViewChannel` 대상자가 동적으로 멤버십을 정의해야 할 때는 `type: "discord.channelAudience"`를 사용하세요. 공유 접근 그룹 동작은 여기에서 문서화되어 있습니다. [접근 그룹](/ko/channels/access-groups)
...
```
    ...
    Discord 텍스트 채널에는 별도의 멤버 목록이 없습니다. `type: "discord.channelAudience"`는 멤버십을 다음과 같이 모델링합니다. DM 발신자가 구성된 길드의 멤버이며, 역할 및 채널 덮어쓰기가 적용된 후 구성된 채널에 대한 유효 `ViewChannel` 권한을 현재 가지고 있습니다.

    예: 다른 모든 사용자에게는 DM을 닫아 두면서 `#maintainers`를 볼 수 있는 사람은 누구나 봇에 DM할 수 있도록 허용합니다.
...
```
    ...
    조회는 실패 시 닫힌 상태로 처리됩니다. Discord가 `Missing Access`를 반환하거나, 멤버 조회가 실패하거나, 채널이 다른 길드에 속한 경우 DM 발신자는 권한이 없는 것으로 취급됩니다.

    채널 대상자 접근 그룹을 사용할 때는 Discord Developer Portal에서 봇에 대해 **Server Members Intent**를 활성화하세요. DM에는 길드 멤버 상태가 포함되지 않으므로 OpenClaw는 권한 부여 시점에 Discord REST를 통해 멤버를 확인합니다.
  ...
  <Tab title="Guild policy">
    ...
    `channels.discord`가 존재할 때의 보안 기준값은 `allowlist`입니다.
...
```
    ...
    `DISCORD_BOT_TOKEN`만 설정하고 `channels.discord` 블록을 만들지 않으면 `channels.defaults.groupPolicy`가 `open`이어도 런타임 폴백은 `groupPolicy="allowlist"`입니다(로그에 경고가 표시됨).
  ...
  <Tab title="Mentions and group DMs">
    ...
    아웃바운드 Discord 메시지를 작성할 때는 정식 멘션 구문을 사용하세요. 사용자는 `<@USER_ID>`, 채널은 `<#CHANNEL_ID>`, 역할은 `<@&ROLE_ID>`입니다. 기존 `<@!USER_ID>` 닉네임 멘션 형식은 사용하지 마세요.
...
역할 ID별로 Discord 길드 멤버를 서로 다른 에이전트로 라우팅하려면 `bindings[].match.roles`를 사용하세요. 역할 기반 바인딩은 역할 ID만 허용하며, 피어 또는 상위 피어 바인딩 이후, 길드 전용 바인딩 이전에 평가됩니다. 바인딩이 다른 매치 필드도 설정하는 경우(예: `peer` + `guildId` + `roles`) 구성된 모든 필드가 일치해야 합니다.
...
<AccordionGroup>
  <Accordion title="답장 태그와 네이티브 답장">
    ...
    `batched`는 수신 턴이 여러 메시지의 디바운스된 배치였을 때만 Discord의 암시적 네이티브 답장 참조를 연결합니다. 이는 모든 단일 메시지 턴이 아니라, 주로 모호하고 짧은 시간에 몰려오는 채팅에 네이티브 답장을 사용하려는 경우 유용합니다.
  ...
  <Accordion title="실시간 스트림 미리보기">
    OpenClaw는 임시 메시지를 보내고 텍스트가 도착하는 대로 편집하여 초안 답장을 스트리밍할 수 있습니다. `channels.discord.streaming`은 `off` | `partial` | `block` | `progress`(기본값)를 받습니다. `progress`는 편집 가능한 상태 초안 하나를 유지하고 최종 전달 전까지 도구 진행 상황으로 업데이트합니다. `streamMode`는 레거시 런타임 별칭입니다. 지속된 설정을 표준 키로 다시 쓰려면 `openclaw doctor --fix`를 실행하세요.
...
```
    ...
    - `block`은 초안 크기의 청크를 내보냅니다(`draftChunk`를 사용해 크기와 분기점을 조정하며, `textChunkLimit`까지로 제한됨).
    - 미디어, 오류, 명시적 답장 최종 메시지는 대기 중인 미리보기 편집을 취소합니다.
    - `streaming.preview.toolProgress`(기본값 `true`)는 도구/진행 업데이트가 미리보기 메시지를 재사용할지 제어합니다.
    - `streaming.preview.commandText` / `streaming.progress.commandText`는 압축 진행 줄의 명령/실행 세부 정보를 제어합니다: `raw`(기본값) 또는 `status`(도구 레이블만).

    압축 진행 줄은 유지하면서 원시 명령/실행 텍스트를 숨기기:
  ...
  <Accordion title="기록, 컨텍스트 및 스레드 동작">
    ...
    - Discord 스레드는 채널 세션으로 라우팅되며, 재정의하지 않는 한 부모 채널 설정을 상속합니다.
    - 스레드 세션은 부모 채널의 세션 수준 `/model` 선택을 모델 전용 대체값으로 상속합니다. 스레드 로컬 `/model` 선택은 여전히 우선하며, 전사 기록 상속이 활성화되지 않은 한 부모 전사 기록은 복사되지 않습니다.
    - `channels.discord.thread.inheritParent`(기본값 `false`)는 새 자동 스레드가 부모 전사에서 시드되도록 선택합니다. 계정별 재정의는 `channels.discord.accounts.<id>.thread.inheritParent` 아래에 있습니다.
    - 메시지 도구 반응은 `user:<id>` DM 대상을 확인할 수 있습니다.
    - `guilds.<guild>.channels.<channel>.requireMention: false`는 답장 단계 활성화 대체 중에도 보존됩니다.

    채널 주제는 **신뢰할 수 없는** 컨텍스트로 주입됩니다. 허용 목록은 누가 에이전트를 트리거할 수 있는지만 제한하며, 전체 보조 컨텍스트 삭제 경계가 아닙니다.

  </Accordion>

  <Accordion title="하위 에이전트용 스레드 바인딩 세션">
    Discord는 스레드를 세션 대상에 바인딩하여 해당 스레드의 후속 메시지가 동일한 세션(하위 에이전트 세션 포함)으로 계속 라우팅되게 할 수 있습니다.
...
```
    ...
    [하위 에이전트](/ko/tools/subagents), [ACP 에이전트](/ko/tools/acp-agents), [설정 참조](/ko/gateway/configuration-reference)를 참조하세요.

  </Accordion>

  <Accordion title="영구 ACP 채널 바인딩">
    안정적인 "상시 실행" ACP 작업 공간의 경우 Discord 대화를 대상으로 하는 최상위 타입 지정 ACP 바인딩을 설정하세요.
...
```
    ...
    - `/acp spawn codex --bind here`는 현재 채널 또는 스레드를 그 자리에서 바인딩하고 향후 메시지를 동일한 ACP 세션에 유지합니다. 스레드 메시지는 부모 채널 바인딩을 상속합니다.
    - 바인딩된 채널 또는 스레드에서 `/new` 및 `/reset`은 동일한 ACP 세션을 그 자리에서 재설정합니다. 임시 스레드 바인딩은 활성 상태일 때 대상 확인을 재정의할 수 있습니다.
...
```
  ...
  <Accordion title="PluralKit 지원">
    프록시된 메시지를 시스템 멤버 아이덴티티에 매핑하려면 PluralKit 확인을 활성화하세요.
...
```
  ...
  <Accordion title="발신 멘션 별칭">
    에이전트가 알려진 Discord 사용자에 대해 결정적 발신 멘션이 필요할 때 `mentionAliases`를 사용하세요. 키는 앞의 `@`가 없는 핸들이며, 값은 Discord 사용자 ID입니다. 알 수 없는 핸들, `@everyone`, `@here`, Markdown 코드 스팬 안의 멘션은 변경되지 않습니다.
...
```
    ...
    자동 상태 표시 기능은 런타임 가용성을 Discord 상태에 매핑합니다. healthy => online, degraded 또는 unknown => idle, exhausted 또는 unavailable => dnd. 선택적 텍스트 재정의:
  ...
  <Accordion title="Approvals in Discord">
    ...
    Discord는 `enabled`가 설정되지 않았거나 `"auto"`이고 승인자를 하나 이상 확인할 수 있을 때 기본 exec 승인을 자동으로 활성화합니다. 승인자는 `execApprovals.approvers` 또는 `commands.ownerAllowFrom`에서 확인됩니다. Discord는 채널 `allowFrom`, 레거시 `dm.allowFrom`, 또는 직접 메시지 `defaultTo`에서 exec 승인자를 추론하지 않습니다. Discord를 기본 승인 클라이언트로 명시적으로 비활성화하려면 `enabled: false`를 설정하세요.

    `/diagnostics` 및 `/export-trajectory` 같은 민감한 소유자 전용 그룹 명령의 경우 OpenClaw는 승인 프롬프트와 최종 결과를 비공개로 보냅니다. 호출한 소유자에게 Discord 소유자 경로가 있으면 먼저 Discord DM을 시도합니다. 사용할 수 없으면 Telegram 같은 `commands.ownerAllowFrom`의 첫 번째 사용 가능한 소유자 경로로 대체합니다.

    `target`이 `channel` 또는 `both`이면 승인 프롬프트가 채널에 표시됩니다. 확인된 승인자만 버튼을 사용할 수 있으며, 다른 사용자는 임시 거부 메시지를 받습니다. 승인 프롬프트에는 명령 텍스트가 포함되므로 신뢰할 수 있는 채널에서만 채널 전송을 활성화하세요. 세션 키에서 채널 ID를 파생할 수 없으면 OpenClaw는 DM 전송으로 대체합니다.

    Discord는 다른 채팅 채널에서 사용하는 공유 승인 버튼도 렌더링합니다. 기본 Discord 어댑터는 주로 승인자 DM 라우팅과 채널 팬아웃을 추가합니다.
    이러한 버튼이 있으면 기본 승인 UX입니다. OpenClaw는
    도구 결과가 채팅 승인을 사용할 수 없다고 하거나 수동 승인이 유일한 경로라고 말할 때만
    수동 `/approve` 명령을 포함해야 합니다.
    Discord 기본 승인 런타임이 활성 상태가 아니면 OpenClaw는
    로컬 결정적 `/approve <id> <decision>` 프롬프트를 계속 표시합니다. 런타임이
    활성 상태이지만 기본 카드를 어떤 대상에도 전달할 수 없으면
...
Discord 메시지 작업에는 메시징, 채널 관리, 검토, 상태 표시, 메타데이터 작업이 포함됩니다.
...
`event-create` 작업은 예약된 이벤트 표지 이미지를 설정하기 위해 선택적 `image` 매개변수(URL 또는 로컬 파일 경로)를 허용합니다.
...
4. 대상 음성 채널에서 Connect, Speak, Send Messages, Read Message History를 허용합니다.
5. 기본 명령(`commands.native` 또는 `channels.discord.commands.native`)을 활성화합니다.
...
- Discord 음성은 텍스트 전용 설정에서는 옵트인입니다. `/vc` 명령, 음성 런타임, `GuildVoiceStates` gateway intent를 활성화하려면 `channels.discord.voice.enabled=true`를 설정하세요(또는 기존 `channels.discord.voice` 블록을 유지하세요).
- `channels.discord.intents.voiceStates`는 음성 상태 intent 구독을 명시적으로 재정의할 수 있습니다. intent가 유효한 음성 활성화 상태를 따르도록 하려면 설정하지 않은 상태로 두세요.
...
- `voice.reconnectGraceMs`는 연결이 끊긴 음성 세션이 재연결을 시작하기까지 OpenClaw가 기다리는 시간을 제어한 후 세션을 제거합니다. 기본값: `15000`.
- OpenClaw는 수신 복호화 실패도 감시하며, 짧은 시간 안에 실패가 반복되면 음성 채널을 나갔다가 다시 참여해 자동 복구합니다.
- 업데이트 후 수신 로그에 `DecryptionFailed(UnencryptedWhenPassthroughDisabled)`가 반복해서 표시되면 의존성 보고서와 로그를 수집하세요. 번들된 `@discordjs/voice` 라인에는 discord.js PR #11449의 upstream padding 수정이 포함되어 있으며, 이 수정은 discord.js issue #11419를 종료했습니다.

음성 채널 파이프라인:

- Discord PCM 캡처가 WAV 임시 파일로 변환됩니다.
- `tools.media.audio`가 STT를 처리합니다. 예: `openai/gpt-4o-mini-transcribe`.
- 전사는 Discord 수신 및 라우팅을 거쳐 전송되며, 응답 LLM은 agent `tts` 도구를 숨기고 반환 텍스트를 요청하는 음성 출력 정책으로 실행됩니다. 최종 TTS 재생은 Discord 음성이 소유하기 때문입니다.
- `voice.model`이 설정된 경우 이 음성 채널 턴의 응답 LLM만 재정의합니다.
- `voice.tts`가 `messages.tts` 위에 병합됩니다. 결과 오디오는 참여한 채널에서 재생됩니다.

자격 증명은 컴포넌트별로 확인됩니다. `voice.model`용 LLM 경로 인증, `tools.media.audio`용 STT 인증, `messages.tts`/`voice.tts`용 TTS 인증입니다.
...
- 텍스트 콘텐츠를 생략하세요(Discord는 같은 페이로드의 텍스트 + 음성 메시지를 거부함).
...
<AccordionGroup>
  ...
  <Accordion title="Guild messages blocked unexpectedly">
    ...
    - `groupPolicy` 확인
    - `channels.discord.guilds` 아래의 guild 허용 목록 확인
...
```
  ...
  <Accordion title="Require mention false but still blocked">
    ...
    - 일치하는 guild/channel 허용 목록 없이 `groupPolicy="allowlist"` 사용
    - `requireMention`이 잘못된 위치에 설정됨(`channels.discord.guilds` 또는 채널 항목 아래에 있어야 함)
    - 발신자가 guild/channel `users` 허용 목록에 의해 차단됨
  ...
  <Accordion title="Long-running Discord turns or duplicate replies">
    ...
    - 이것은 에이전트 턴 수명이 아니라 Discord gateway listener 작업만 제어합니다.

    Discord는 큐에 들어간 에이전트 턴에 채널 소유 timeout을 적용하지 않습니다. 메시지 listener는 즉시 넘겨주며, 큐에 들어간 Discord 실행은 session/tool/runtime lifecycle이 완료되거나 작업이 중단될 때까지 세션별 순서를 보존합니다.
...
```
  ...
  <Accordion title="Gateway 메타데이터 조회 시간 초과 경고">
    OpenClaw는 연결하기 전에 Discord `/gateway/bot` 메타데이터를 가져옵니다. 일시적인 실패는 Discord의 기본 Gateway URL로 폴백하며 로그에서 비율 제한됩니다.
  ...
  <Accordion title="Gateway READY 시간 초과 재시작">
    OpenClaw는 시작 중과 런타임 재연결 후 Discord의 Gateway `READY` 이벤트를 기다립니다. 시작 시차 적용을 사용하는 다중 계정 설정은 기본값보다 더 긴 시작 READY 대기 시간이 필요할 수 있습니다.
  ...
  <Accordion title="권한 감사 불일치">
    ...
    slug 키를 사용하는 경우에도 런타임 매칭은 계속 작동할 수 있지만, probe가 권한을 완전히 검증할 수는 없습니다.
  ...
  <Accordion title="Bot 간 루프">
    ...
    bot을 멘션하는 bot 메시지만 허용하려면 `channels.discord.allowBots="mentions"`를 선호하세요.
...
```
  ...
  <Accordion title="DecryptionFailed(...)가 발생하는 Voice STT 드롭">
...
- bot 토큰은 비밀로 취급하세요(관리형 환경에서는 `DISCORD_BOT_TOKEN` 권장).
