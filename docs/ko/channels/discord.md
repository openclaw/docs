...
<Steps>
  ...
  <Step title="초대 URL 생성 및 서버에 봇 추가">
    사이드바에서 **OAuth2**를 클릭합니다. 봇을 서버에 추가하기 위한 올바른 권한이 포함된 초대 URL을 생성합니다.
    ...
    **텍스트 권한**
    ...
    이는 일반 텍스트 채널을 위한 기본 권한 세트입니다. 포럼 또는 미디어 채널 워크플로처럼 스레드를 만들거나 이어 가는 경우를 포함해 Discord 스레드에 게시할 계획이라면 **Send Messages in Threads**도 활성화합니다.
  ...
  <Step title="개발자 모드 활성화 및 ID 수집">
    ...
    **Server ID**와 **User ID**를 Bot Token과 함께 저장합니다. 다음 단계에서 이 세 가지를 모두 OpenClaw로 보냅니다.

  </Step>

  <Step title="서버 멤버의 DM 허용">
    페어링이 작동하려면 Discord가 봇이 당신에게 DM을 보낼 수 있도록 허용해야 합니다. **서버 아이콘**을 오른쪽 클릭 → **Privacy Settings** → **Direct Messages** 토글 켜기.

    이렇게 하면 서버 멤버(봇 포함)가 당신에게 DM을 보낼 수 있습니다. OpenClaw와 함께 Discord DM을 사용하려면 이 설정을 켜 둡니다. 길드 채널만 사용할 계획이라면 페어링 후 DM을 비활성화할 수 있습니다.

  </Step>

  <Step title="봇 토큰을 안전하게 설정(채팅으로 보내지 마세요)">
    Discord 봇 토큰은 비밀번호와 같은 비밀입니다. 에이전트에 메시지를 보내기 전에 OpenClaw가 실행되는 머신에 설정합니다.
...
```
    ...
    관리형 서비스 설치의 경우 `DISCORD_BOT_TOKEN`이 존재하는 셸에서 `openclaw gateway install`을 실행하거나 변수를 `~/.openclaw/.env`에 저장해, 재시작 후 서비스가 env SecretRef를 확인할 수 있게 합니다.
    호스트가 Discord의 시작 애플리케이션 조회에 의해 차단되었거나 속도 제한을 받는 경우, 시작 시 해당 REST 호출을 건너뛸 수 있도록 Developer Portal의 Discord 애플리케이션/클라이언트 ID를 설정합니다. 기본 계정에는 `channels.discord.applicationId`를 사용하고, 여러 Discord 봇을 실행할 때는 `channels.discord.accounts.<accountId>.applicationId`를 사용합니다.
  ...
  <Step title="OpenClaw 구성 및 페어링">
    ...
    <Tabs>
      <Tab title="에이전트에게 요청">
        기존 채널(예: Telegram)에서 OpenClaw 에이전트와 채팅하고 다음과 같이 알려 줍니다. Discord가 첫 채널이라면 대신 CLI / 구성 탭을 사용합니다.
...
```
        ...
        스크립트 기반 또는 원격 설정의 경우 `openclaw config patch --file ./discord.patch.json5 --dry-run`으로 동일한 JSON5 블록을 작성한 뒤 `--dry-run` 없이 다시 실행합니다. 일반 텍스트 `token` 값이 지원됩니다. SecretRef 값도 env/file/exec 제공자 전반에서 `channels.discord.token`에 대해 지원됩니다. [비밀 관리](/ko/gateway/secrets)를 참조하세요.

        여러 Discord 봇의 경우 각 봇 토큰과 애플리케이션 ID를 해당 계정 아래에 둡니다. 최상위 `channels.discord.applicationId`는 계정에 상속되므로, 모든 계정이 같은 애플리케이션 ID를 사용해야 할 때만 그곳에 설정합니다.
...
활성화된 두 Discord 계정이 같은 봇 토큰으로 확인되면 OpenClaw는 해당 토큰에 대해 Gateway 모니터를 하나만 시작합니다. 구성에서 가져온 토큰은 기본 env 대체값보다 우선합니다. 그렇지 않으면 첫 번째로 활성화된 계정이 우선하며 중복 계정은 비활성화된 것으로 보고됩니다.
...
DM이 작동하면 Discord 서버를 전체 작업 공간으로 설정할 수 있습니다. 각 채널에는 고유한 컨텍스트가 있는 자체 에이전트 세션이 제공됩니다. 이는 당신과 봇만 있는 비공개 서버에 권장됩니다.
...
```
  ...
  <Step title="@멘션 없이 응답 허용">
    기본적으로 에이전트는 길드 채널에서 @멘션된 경우에만 응답합니다. 비공개 서버라면 모든 메시지에 응답하기를 원할 가능성이 높습니다.

    길드 채널에서는 일반 어시스턴트 최종 응답이 기본적으로 비공개로 유지됩니다. 표시되는 Discord 출력은 `message` 도구로 명시적으로 보내야 하므로, 에이전트는 기본적으로 조용히 지켜보다가 채널 응답이 유용하다고 판단할 때만 게시할 수 있습니다.
...
```
        ...
        그룹/채널 방에서 레거시 자동 최종 응답을 복원하려면 `messages.groupChat.visibleReplies: "automatic"`을 설정합니다.
  ...
  <Step title="길드 채널의 메모리 계획">
    ...
    <Tabs>
      <Tab title="에이전트에게 요청">
        > "Discord 채널에서 질문할 때 MEMORY.md의 장기 컨텍스트가 필요하면 memory_search 또는 memory_get을 사용하세요."
      </Tab>
      <Tab title="수동">
        모든 채널에서 공유 컨텍스트가 필요하다면 안정적인 지침을 `AGENTS.md` 또는 `USER.md`에 넣습니다(모든 세션에 주입됩니다). 장기 메모는 `MEMORY.md`에 보관하고 필요할 때 메모리 도구로 접근합니다.
      </Tab>
    </Tabs>
...
이제 Discord 서버에 몇 가지 채널을 만들고 채팅을 시작하세요. 에이전트는 채널 이름을 볼 수 있으며, 각 채널은 자체 격리된 세션을 갖습니다. 따라서 `#coding`, `#home`, `#research` 또는 워크플로에 맞는 어떤 채널이든 설정할 수 있습니다.

## 런타임 모델

- Gateway가 Discord 연결을 소유합니다.
- 응답 라우팅은 결정적입니다. Discord 인바운드 응답은 Discord로 돌아갑니다.
- Discord 길드/채널 메타데이터는 사용자가 볼 수 있는 응답 접두사가 아니라 신뢰할 수 없는
  컨텍스트로 모델 프롬프트에 추가됩니다. 모델이 해당 래퍼를 다시 복사하면
...
- Discord로 텍스트 전용 Cron/Heartbeat 알림을 전달할 때는 최종
  ...
  에이전트가 여러 전달 가능 페이로드를 내보내는 경우 여러 메시지로 유지됩니다.
...
`/model` 및 `/models` 슬래시 명령은 제공자, 모델, 호환 런타임 드롭다운과 제출 단계가 있는 대화형 모델 선택기를 엽니다. `/models add`는 더 이상 사용되지 않으며 이제 채팅에서 모델을 등록하는 대신 지원 중단 메시지를 반환합니다. 선택기 응답은 임시 메시지이며 호출한 사용자만 사용할 수 있습니다.

파일 첨부:

- `file` 블록은 첨부 참조(`attachment://<filename>`)를 가리켜야 합니다
- `media`/`path`/`filePath`를 통해 첨부를 제공합니다(단일 파일). 여러 파일에는 `media-gallery`를 사용하세요
- 업로드 이름이 첨부 참조와 일치해야 할 때는 `filename`을 사용해 업로드 이름을 재정의하세요
...
<Tabs>
  <Tab title="DM 정책">
    ...
    DM 정책이 open이 아니면 알 수 없는 사용자는 차단됩니다(`pairing` 모드에서는 페어링 안내가 표시됨).

    다중 계정 우선순위:

    - `channels.discord.accounts.default.allowFrom`은 `default` 계정에만 적용됩니다.
    - 계정이 하나인 경우 `allowFrom`이 레거시 `dm.allowFrom`보다 우선합니다.
    - 이름이 있는 계정은 자체 `allowFrom`과 레거시 `dm.allowFrom`이 설정되지 않은 경우 `channels.discord.allowFrom`을 상속합니다.
    - 이름이 있는 계정은 `channels.discord.accounts.default.allowFrom`을 상속하지 않습니다.

    레거시 `channels.discord.dm.policy` 및 `channels.discord.dm.allowFrom`은 호환성을 위해 계속 읽습니다. 접근을 변경하지 않고 처리할 수 있는 경우 `openclaw doctor --fix`가 이를 `dmPolicy` 및 `allowFrom`으로 마이그레이션합니다.
    ...
    단순 숫자 ID는 일반적으로 채널 기본값이 활성화된 경우 채널 ID로 해석되지만, 계정의 유효 DM `allowFrom`에 나열된 ID는 호환성을 위해 사용자 DM 대상으로 처리됩니다.
  ...
  <Tab title="DM 접근 그룹">
    ...
    접근 그룹 이름은 메시지 채널 전반에서 공유됩니다. 각 채널의 일반 `allowFrom` 구문으로 구성원이 표현되는 정적 그룹에는 `type: "message.senders"`를 사용하고, Discord 채널의 현재 `ViewChannel` 대상이 멤버십을 동적으로 정의해야 할 때는 `type: "discord.channelAudience"`를 사용하세요. 공유 접근 그룹 동작은 여기 문서에 설명되어 있습니다: [접근 그룹](/ko/channels/access-groups).
...
```
    ...
    Discord 텍스트 채널에는 별도의 구성원 목록이 없습니다. `type: "discord.channelAudience"`는 멤버십을 다음과 같이 모델링합니다. DM 발신자는 구성된 길드의 구성원이며, 역할 및 채널 덮어쓰기가 적용된 후 구성된 채널에 대해 현재 유효한 `ViewChannel` 권한을 가지고 있습니다.

    예: 다른 모든 사용자에게는 DM을 닫아 둔 상태에서 `#maintainers`를 볼 수 있는 모든 사람이 봇에 DM을 보낼 수 있게 허용합니다.
...
```
    ...
    조회는 실패 시 닫힌 상태가 됩니다. Discord가 `Missing Access`를 반환하거나, 구성원 조회가 실패하거나, 채널이 다른 길드에 속한 경우 DM 발신자는 권한이 없는 것으로 처리됩니다.

    채널 대상 접근 그룹을 사용할 때 봇에 대해 Discord Developer Portal **Server Members Intent**를 활성화하세요. DM에는 길드 구성원 상태가 포함되지 않으므로 OpenClaw는 권한 부여 시점에 Discord REST를 통해 구성원을 확인합니다.
...
```
    ...
    `DISCORD_BOT_TOKEN`만 설정하고 `channels.discord` 블록을 만들지 않으면 런타임 폴백은 `groupPolicy="allowlist"`입니다(로그에 경고 표시). 이는 `channels.defaults.groupPolicy`가 `open`인 경우에도 마찬가지입니다.
  ...
  <Tab title="멘션 및 그룹 DM">
    ...
    아웃바운드 Discord 메시지를 작성할 때는 표준 멘션 구문을 사용하세요. 사용자는 `<@USER_ID>`, 채널은 `<#CHANNEL_ID>`, 역할은 `<@&ROLE_ID>`입니다. 레거시 `<@!USER_ID>` 닉네임 멘션 형식을 사용하지 마세요.

    `requireMention`은 길드/채널별로 구성됩니다(`channels.discord.guilds...`).
    `ignoreOtherMentions`는 선택적으로 봇이 아닌 다른 사용자/역할을 멘션한 메시지를 삭제합니다(@everyone/@here 제외).
...
역할 ID별로 Discord 길드 구성원을 다른 에이전트로 라우팅하려면 `bindings[].match.roles`를 사용하세요. 역할 기반 바인딩은 역할 ID만 허용하며 피어 또는 상위 피어 바인딩 이후, 길드 전용 바인딩 이전에 평가됩니다. 바인딩이 다른 매치 필드도 설정하는 경우(예: `peer` + `guildId` + `roles`) 구성된 모든 필드가 일치해야 합니다.
...
- `commands.native=false`는 시작 중 Discord 슬래시 명령 등록과 정리를 건너뜁니다. 이전에 등록된 명령은 Discord 앱에서 제거할 때까지 Discord에 계속 표시될 수 있습니다.
...
<AccordionGroup>
  <Accordion title="답장 태그 및 네이티브 답장">
    ...
    `batched`는 수신 턴이 여러 메시지의 디바운스된 배치였을 때만 Discord의 암시적 네이티브 답장 참조를 첨부합니다. 이는 모든 단일 메시지 턴이 아니라, 주로 모호한 짧은 연속 채팅에 네이티브 답장을 사용하려는 경우 유용합니다.
  ...
  <Accordion title="실시간 스트림 미리 보기">
    OpenClaw는 임시 메시지를 보내고 텍스트가 도착할 때마다 편집하여 초안 답장을 스트리밍할 수 있습니다. `channels.discord.streaming`은 `off`(기본값) | `partial` | `block` | `progress`를 받습니다. `progress`는 편집 가능한 상태 초안 하나를 유지하고 최종 전달 전까지 도구 진행 상황으로 업데이트합니다. `streamMode`는 레거시 별칭이며 자동으로 마이그레이션됩니다.

    Discord 미리 보기 편집은 여러 봇 또는 Gateway가 계정을 공유할 때 속도 제한에 빠르게 걸리므로 기본값은 `off`로 유지됩니다.
...
```
    ...
    - `partial`은 토큰이 도착할 때 단일 미리 보기 메시지를 편집합니다.
    - `block`은 초안 크기의 청크를 내보냅니다(`draftChunk`를 사용해 크기와 중단 지점을 조정하며, `textChunkLimit`로 제한됨).
    - 미디어, 오류, 명시적 답장 최종 메시지는 대기 중인 미리 보기 편집을 취소합니다.
    - `streaming.preview.toolProgress`(기본값 `true`)는 도구/진행 상황 업데이트가 미리 보기 메시지를 재사용할지 제어합니다.

    미리 보기 스트리밍은 텍스트 전용입니다. 미디어 답장은 일반 전달로 대체됩니다. `block` 스트리밍이 명시적으로 활성화되면 OpenClaw는 이중 스트리밍을 피하기 위해 미리 보기 스트림을 건너뜁니다.
  ...
  <Accordion title="기록, 컨텍스트 및 스레드 동작">
    ...
    - Discord 스레드는 채널 세션으로 라우팅되며, 재정의하지 않으면 부모 채널 구성을 상속합니다.
    - 스레드 세션은 부모 채널의 세션 수준 `/model` 선택을 모델 전용 대체값으로 상속합니다. 스레드 로컬 `/model` 선택은 여전히 우선하며, 트랜스크립트 상속이 활성화되지 않으면 부모 트랜스크립트 기록은 복사되지 않습니다.
    - `channels.discord.thread.inheritParent`(기본값 `false`)는 새 자동 스레드가 부모 트랜스크립트에서 시드되도록 선택합니다. 계정별 재정의는 `channels.discord.accounts.<id>.thread.inheritParent` 아래에 있습니다.
    - 메시지 도구 반응은 `user:<id>` DM 대상을 해석할 수 있습니다.
    - `guilds.<guild>.channels.<channel>.requireMention: false`는 답장 단계 활성화 대체 중에도 유지됩니다.

    채널 주제는 **신뢰되지 않는** 컨텍스트로 주입됩니다. 허용 목록은 누가 에이전트를 트리거할 수 있는지를 제한하지만, 전체 보충 컨텍스트 삭제 경계는 아닙니다.

  </Accordion>

  <Accordion title="하위 에이전트용 스레드 바인딩 세션">
    Discord는 스레드를 세션 대상에 바인딩할 수 있으므로, 해당 스레드의 후속 메시지는 같은 세션(하위 에이전트 세션 포함)으로 계속 라우팅됩니다.
    ...
    - `/session max-age <duration|off>` 포커스된 바인딩의 최대 수명 조회/업데이트
...
```
    ...
    - `defaultSpawnContext`는 스레드 바인딩 생성에 대한 네이티브 하위 에이전트 컨텍스트를 제어합니다. 기본값: `"fork"`.
  ...
  <Accordion title="영구 ACP 채널 바인딩">
    안정적인 "항상 켜짐" ACP 작업 공간의 경우 Discord 대화를 대상으로 하는 최상위 유형화 ACP 바인딩을 구성하세요.
...
```
    ...
    - 바인딩된 채널 또는 스레드에서 `/new`와 `/reset`은 같은 ACP 세션을 그 자리에서 재설정합니다. 임시 스레드 바인딩은 활성 상태일 때 대상 해석을 재정의할 수 있습니다.
...
```
    ...
    - `channels.discord.dangerouslyAllowNameMatching: true`일 때만 멤버 표시 이름이 이름/슬러그로 일치됩니다.
  ...
  <Accordion title="발신 멘션 별칭">
    에이전트가 알려진 Discord 사용자에게 결정적인 발신 멘션이 필요할 때 `mentionAliases`를 사용하세요. 키는 앞의 `@`가 없는 핸들이며, 값은 Discord 사용자 ID입니다. 알 수 없는 핸들, `@everyone`, `@here`, Markdown 코드 스팬 내부의 멘션은 변경되지 않습니다.
...
```
  ...
  <Accordion title="Discord의 승인">
    ...
    Discord는 `enabled`가 설정되지 않았거나 `"auto"`이고 `execApprovals.approvers` 또는 `commands.ownerAllowFrom`에서 하나 이상의 승인자를 확인할 수 있으면 네이티브 exec 승인을 자동으로 활성화합니다. Discord는 채널 `allowFrom`, 레거시 `dm.allowFrom`, 또는 다이렉트 메시지 `defaultTo`에서 exec 승인자를 추론하지 않습니다. Discord를 네이티브 승인 클라이언트로 명시적으로 비활성화하려면 `enabled: false`를 설정하세요.
    ...
    도구 결과가 채팅 승인을 사용할 수 없다고 말하거나 수동 승인이 유일한 경로일 때만
    수동 `/approve` 명령을 포함해야 합니다.
    Discord 네이티브 승인 런타임이 활성 상태가 아니면 OpenClaw는
    로컬 결정적 `/approve <id> <decision>` 프롬프트를 표시 상태로 유지합니다. 런타임이
    활성 상태지만 네이티브 카드를 어떤 대상에도 전달할 수 없으면,
...
OpenClaw는 exec 승인과 교차 컨텍스트 마커에 Discord components v2를 사용합니다. Discord 메시지 작업은 사용자 지정 UI를 위해 `components`도 받을 수 있으며(고급; discord 도구를 통해 컴포넌트 페이로드를 구성해야 함), 레거시 `embeds`는 계속 사용할 수 있지만 권장되지 않습니다.
...
- `voice.model`은 Discord 음성 채널 응답에 사용되는 LLM에만 적용됩니다. 설정하지 않으면 라우팅된 에이전트 모델을 상속합니다.
- STT는 `tools.media.audio`를 사용합니다. `voice.model`은 전사에 영향을 주지 않습니다.
- 채널별 Discord `systemPrompt` 재정의는 해당 음성 채널의 음성 전사 턴에 적용됩니다.
- 음성 전사 턴은 Discord `allowFrom`(또는 `dm.allowFrom`)에서 소유자 상태를 파생합니다. 소유자가 아닌 화자는 소유자 전용 도구(예: `gateway` 및 `cron`)에 접근할 수 없습니다.
- Discord 음성은 텍스트 전용 구성에서는 옵트인입니다. `/vc` 명령, 음성 런타임, `GuildVoiceStates` Gateway 인텐트를 활성화하려면 `channels.discord.voice.enabled=true`를 설정하세요(또는 기존 `channels.discord.voice` 블록을 유지하세요).
- `channels.discord.intents.voiceStates`는 음성 상태 인텐트 구독을 명시적으로 재정의할 수 있습니다. 인텐트가 유효한 음성 활성화 상태를 따르도록 하려면 설정하지 않은 상태로 두세요.
...
- `voice.reconnectGraceMs`는 연결이 끊긴 음성 세션이 재연결을 시작하기를 OpenClaw가 파기 전에 얼마나 오래 기다릴지 제어합니다. 기본값: `15000`.
- OpenClaw는 수신 복호화 실패도 감시하며, 짧은 시간 창 안에서 실패가 반복되면 음성 채널을 나갔다가 다시 참여해 자동 복구합니다.
...
- `voice.tts`는 `messages.tts` 위에 병합됩니다. 결과 오디오는 참여한 채널에서 재생됩니다.
...
- 모든 오디오 형식을 허용합니다. OpenClaw는 필요에 따라 OGG/Opus로 변환합니다.
...
```
  ...
  <Accordion title="requireMention이 false인데도 여전히 차단됨">
    일반적인 원인:
  ...
  <Accordion title="장시간 실행되는 Discord 턴 또는 중복 응답">
    ...
    - 이것은 Discord Gateway 리스너 작업만 제어하며, 에이전트 턴 수명은 제어하지 않습니다

    Discord는 큐에 있는 에이전트 턴에 채널 소유 제한 시간을 적용하지 않습니다. 메시지 리스너는 즉시 넘겨주며, 큐에 있는 Discord 실행은 세션/도구/런타임 수명 주기가 완료되거나 작업을 중단할 때까지 세션별 순서를 유지합니다.
...
```
  ...
  <Accordion title="Gateway 메타데이터 조회 제한 시간 경고">
  ...
  <Accordion title="Gateway READY 시간 초과 재시작">
    OpenClaw는 시작 중 및 런타임 재연결 후 Discord의 gateway `READY` 이벤트를 기다립니다. 시작 시차를 두는 다중 계정 설정은 기본값보다 더 긴 시작 READY 창이 필요할 수 있습니다.

    READY 시간 초과 조정 항목:

    - 시작 단일 계정: `channels.discord.gatewayReadyTimeoutMs`
    - 시작 다중 계정: `channels.discord.accounts.<accountId>.gatewayReadyTimeoutMs`
    - 설정이 지정되지 않은 경우 시작 환경 변수 대체값: `OPENCLAW_DISCORD_READY_TIMEOUT_MS`
    ...
    - 설정이 지정되지 않은 경우 런타임 환경 변수 대체값: `OPENCLAW_DISCORD_RUNTIME_READY_TIMEOUT_MS`
  ...
  <Accordion title="권한 감사 불일치">
    ...
    슬러그 키를 사용하는 경우 런타임 매칭은 계속 작동할 수 있지만, 프로브는 권한을 완전히 확인할 수 없습니다.
...
```
  ...
  <Accordion title="음성 STT가 DecryptionFailed(...)와 함께 누락됨">
...
- 미디어/재시도: `mediaMaxMb`(나가는 Discord 업로드 제한, 기본값 `100MB`), `retry`
- 작업: `actions.*`
