...
x-i18n:
    generated_at: "2026-04-30T06:18:30Z"
    model: gpt-5.5
    provider: openai
...
이전 빌드를 사용 중이거나 번들 Teams를 제외한 사용자 지정 설치를 사용하는 경우, 게시된 최신 npm 패키지가 있으면 설치하세요.

```bash
openclaw plugins install @openclaw/msteams
```

npm이 OpenClaw 소유 패키지를 deprecated로 보고하면, 더 최신 npm 패키지가 게시될 때까지 현재 패키지된 OpenClaw 빌드 또는 로컬 체크아웃 경로를 사용하세요.
...
[`@microsoft/teams.cli`](https://www.npmjs.com/package/@microsoft/teams.cli)는 봇 등록, 매니페스트 생성, 자격 증명 생성을 하나의 명령으로 처리합니다.
...
<Note>
Teams CLI는 현재 preview입니다. 명령과 플래그는 릴리스 간에 변경될 수 있습니다.
</Note>

**2. 터널 시작**(Teams는 localhost에 연결할 수 없습니다)
...
<Note>
Teams가 devtunnels로 인증할 수 없으므로 `--allow-anonymous`가 필요합니다. 각 수신 봇 요청은 여전히 Teams SDK에서 자동으로 검증됩니다.
</Note>
...
출력에는 `CLIENT_ID`, `CLIENT_SECRET`, `TENANT_ID`, 그리고 **Teams App ID**가 표시됩니다. 다음 단계에서 사용할 수 있도록 기록해 두세요. 또한 앱을 Teams에 직접 설치할 수도 있습니다.

**4. 출력의 자격 증명을 사용해 OpenClaw 구성:**
...
`teams app create`는 앱 설치를 묻습니다. "Install in Teams"를 선택하세요. 건너뛰었다면 나중에 링크를 받을 수 있습니다.
...
프로덕션 배포의 경우 클라이언트 암호 대신 [페더레이션 인증](/ko/channels/msteams#federated-authentication-certificate-plus-managed-identity)(인증서 또는 관리 ID)을 사용하는 것을 고려하세요.

<Note>
그룹 채팅은 기본적으로 차단됩니다(`channels.msteams.groupPolicy: "allowlist"`). 그룹 답장을 허용하려면 `channels.msteams.groupAllowFrom`을 설정하거나 `groupPolicy: "open"`을 사용해 모든 멤버를 허용하세요(멘션 게이트 적용).
</Note>
...
- 허용 목록에는 UPN/표시 이름 매칭에 의존하지 마세요. 변경될 수 있습니다. OpenClaw는 기본적으로 직접 이름 매칭을 비활성화합니다. `channels.msteams.dangerouslyAllowNameMatching: true`로 명시적으로 옵트인하세요.
...
- `channels.msteams.groupAllowFrom`은 그룹 채팅/채널에서 어떤 보낸 사람이 트리거할 수 있는지 제어합니다(`channels.msteams.allowFrom`으로 대체됨).
...
- `groupPolicy="allowlist"`이고 팀 허용 목록이 있으면 나열된 팀/채널만 허용됩니다(멘션 게이트 적용).
- 구성 마법사는 `Team/Channel` 항목을 받아 대신 저장합니다.
- 시작 시 OpenClaw는 팀/채널 및 사용자 허용 목록 이름을 ID로 확인하고(Graph 권한이 허용하는 경우)
  매핑을 로그에 남깁니다. 확인되지 않은 팀/채널 이름은 입력한 대로 유지되지만, `channels.msteams.dangerouslyAllowNameMatching: true`가 활성화되지 않는 한 기본적으로 라우팅에서 무시됩니다.
...
1. Microsoft Teams Plugin이 사용 가능한지 확인합니다(현재 릴리스에 번들됨).
2. **Azure Bot**(App ID + 암호 + 테넌트 ID)을 만듭니다.
...
<Warning>
새 다중 테넌트 봇 생성은 2025-07-31 이후 deprecated되었습니다. 새 봇에는 **Single Tenant**를 사용하세요.
</Warning>
...
2. **Messaging endpoint**를 Webhook URL로 설정합니다.
   ...
   - 로컬 개발: 터널을 사용합니다(아래 [로컬 개발](#local-development-tunneling) 참조).
...
1. 인증서를 생성하거나 받습니다(개인 키가 포함된 PEM 형식).
...
비밀번호 없는 인증에는 Azure Managed Identity를 사용합니다. 이는 관리 ID를 사용할 수 있는 Azure 인프라(AKS, App Service, Azure VM) 배포에 적합합니다.
...
5. IMDS(`169.254.169.254`)에 대한 **네트워크 액세스를 보장**합니다. NetworkPolicy를 사용하는 경우 포트 80에서 `169.254.169.254/32`로 가는 트래픽을 허용하는 egress 규칙을 추가합니다.
...
Teams는 `localhost`에 접근할 수 없습니다. 세션 간에도 URL이 동일하게 유지되도록 지속적인 개발 터널을 사용하세요.
...
OpenClaw는 에이전트와 자동화가 Microsoft Graph에서 직접 채널 멤버 세부 정보(표시 이름, 이메일, 역할)를 확인할 수 있도록 Microsoft Teams용 Graph 기반 `member-info` 액션을 제공합니다.
...
- `channels.msteams.historyLimit`는 최근 채널/그룹 메시지 몇 개를 프롬프트에 래핑할지 제어합니다.
...
다음은 Teams 앱 매니페스트의 **기존 resourceSpecific 권한**입니다. 이 권한은 앱이 설치된 팀/채팅 내부에만 적용됩니다.
...
업데이트 후 새 권한이 적용되도록 각 팀에서 앱을 다시 설치하고, 캐시된 앱 메타데이터를 지우려면 **Teams를 완전히 종료한 뒤 다시 실행**하세요(창만 닫는 것이 아님).
...
- 메시지 기록 읽기(라이브 Webhook 이벤트 이후는 제외).
...
**핵심:** RSC는 실시간 수신용이고, Graph API는 기록 접근용입니다. 오프라인 상태에서 놓친 메시지를 따라잡으려면 관리자 동의가 필요한 `ChannelMessage.Read.All`이 포함된 Graph API가 필요합니다.
...
3. Teams 앱 **매니페스트 버전**을 올리고, 다시 업로드한 뒤, **Teams에 앱을 다시 설치**합니다.
4. 캐시된 앱 메타데이터를 지우려면 **Teams를 완전히 종료한 뒤 다시 실행**합니다.

**사용자 멘션을 위한 추가 권한:** 사용자 @멘션은 대화에 있는 사용자에게는 기본적으로 작동합니다. 그러나 **현재 대화에 없는** 사용자를 동적으로 검색하고 멘션하려면 `User.Read.All`(애플리케이션) 권한을 추가하고 관리자 동의를 부여하세요.
...
OpenClaw는 빠르게 반환하고 능동적으로 답장을 보내는 방식으로 이를 처리하지만, 매우 느린 응답은 여전히 문제를 일으킬 수 있습니다.
...
- 설문과 시맨틱 프레젠테이션 전송에는 Adaptive Cards가 지원됩니다(아래 참고)
...
- `channels.msteams.allowFrom`: DM 허용 목록(AAD 객체 ID 권장). Graph 액세스를 사용할 수 있으면 설정 중 마법사가 이름을 ID로 확인합니다.
...
- `channels.msteams.mediaAllowHosts`: 인바운드 첨부 파일 호스트의 허용 목록입니다(기본값은 Microsoft/Teams 도메인).
...
- `channels.msteams.actions.memberInfo`: Graph 기반 멤버 정보 액션을 활성화하거나 비활성화합니다(기본값: Graph 자격 증명을 사용할 수 있으면 활성화).
...
- `channels.msteams.sharePointSiteId`: 그룹 채팅/채널에서 파일 업로드에 사용할 SharePoint 사이트 ID([그룹 채팅에서 파일 보내기](#sending-files-in-group-chats) 참고).
...
| ------------------------ | --------------------------------------------------------- | ------------------------ |
| **게시물**(클래식)       | 메시지가 아래에 스레드 답장이 달린 카드로 표시됨          | `thread`(기본값)         |
...
- 게시물 스타일 채널에서 `top-level` → 답장이 스레드 안이 아니라 별도의 최상위 게시물로 표시됨
...
- **DM:** 이미지는 Teams 봇 파일 API를 통해 동작합니다.
...
- 명시적인 파일 우선 전송에는 `media` / `filePath` / `path`와 함께 `action=upload-file`을 사용합니다. 선택적 `message`는 함께 전송되는 텍스트/댓글이 되고, `filename`은 업로드된 이름을 재정의합니다.

Graph 권한이 없으면 이미지가 있는 채널 메시지는 텍스트로만 수신됩니다(이미지 콘텐츠는 봇이 액세스할 수 없음).
...
봇은 FileConsentCard 흐름(내장)을 사용해 DM에서 파일을 보낼 수 있습니다. 그러나 **그룹 채팅/채널에서 파일 보내기**에는 추가 설정이 필요합니다.
...
- 설문은 아직 결과 요약을 자동 게시하지 않습니다(필요하면 저장소 파일을 확인하세요).

## 프레젠테이션 카드

`message` 도구 또는 CLI를 사용해 Teams 사용자나 대화에 시맨틱 프레젠테이션 페이로드를 보냅니다. OpenClaw는 일반 프레젠테이션 계약에 따라 이를 Teams Adaptive Cards로 렌더링합니다.
...
대상 형식의 자세한 내용은 아래 [대상 형식](#target-formats)을 참고하세요.
...
- OpenClaw 라우팅에서는 `groupId` 쿼리 매개변수를 **무시**하세요. 이는 Microsoft Entra 그룹 ID이며, 들어오는 Teams 활동에서 사용되는 Bot Framework 대화 ID가 아닙니다.
...
1. 봇 상호작용에는 표준 채널 사용
2. DM 사용 - 사용자는 언제든 봇에게 직접 메시지를 보낼 수 있음
...
- **버전 불일치(Teams에 여전히 이전 매니페스트가 표시됨):** 앱을 제거한 뒤 다시 추가하고 Teams를 완전히 종료해 새로고침하세요.
...
- **업로드 시 "Something went wrong":** 대신 [https://admin.teams.microsoft.com](https://admin.teams.microsoft.com)를 통해 업로드하고, 브라우저 DevTools(F12) → Network 탭을 열어 실제 오류가 담긴 응답 본문을 확인하세요.
