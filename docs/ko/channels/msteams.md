...
x-i18n:
    generated_at: "2026-05-10T19:22:45Z"
    model: gpt-5.5
    provider: openai
...
상태: 텍스트 + DM 첨부 파일이 지원됩니다. 채널/그룹 파일 전송에는 `sharePointSiteId` + Graph 권한이 필요합니다([그룹 채팅에서 파일 보내기](#sending-files-in-group-chats) 참조). 투표는 Adaptive Cards를 통해 전송됩니다. 메시지 작업은 파일 우선 전송을 위한 명시적 `upload-file`을 노출합니다.

## 번들 Plugin

Microsoft Teams는 현재 OpenClaw 릴리스에서 번들 Plugin으로 제공되므로, 일반적인 패키지 빌드에서는 별도 설치가 필요하지 않습니다.

이전 빌드를 사용 중이거나 번들 Teams를 제외한 사용자 지정 설치를 사용하는 경우, npm 패키지를 직접 설치하세요.
...
<Note>
Teams CLI는 현재 미리 보기 상태입니다. 명령과 플래그는 릴리스 간에 변경될 수 있습니다.
</Note>
...
<Note>
Teams가 devtunnels로 인증할 수 없기 때문에 `--allow-anonymous`가 필요합니다. 들어오는 각 봇 요청은 여전히 Teams SDK에서 자동으로 검증됩니다.
</Note>

대안: `ngrok http 3978` 또는 `tailscale funnel 3978`(다만 세션마다 URL이 변경될 수 있음).
...
출력에는 `CLIENT_ID`, `CLIENT_SECRET`, `TENANT_ID`, **Teams App ID**가 표시됩니다. 다음 단계를 위해 이를 기록해 두세요. 또한 앱을 Teams에 직접 설치할지 묻습니다.

**4. 출력의 자격 증명을 사용하여 OpenClaw 구성하기:**
...
`teams app create`는 앱 설치를 묻습니다. "Install in Teams"를 선택하세요. 건너뛴 경우 나중에 링크를 가져올 수 있습니다.
...
프로덕션 배포의 경우 클라이언트 시크릿 대신 [페더레이션 인증](/ko/channels/msteams#federated-authentication-certificate-plus-managed-identity)(인증서 또는 관리 ID) 사용을 고려하세요.

<Note>
그룹 채팅은 기본적으로 차단됩니다(`channels.msteams.groupPolicy: "allowlist"`). 그룹 응답을 허용하려면 `channels.msteams.groupAllowFrom`을 설정하거나, `groupPolicy: "open"`을 사용하여 모든 멤버를 허용하세요(멘션 게이트 적용).
</Note>
...
기본적으로 Microsoft Teams는 `/config set|unset`으로 트리거된 구성 업데이트 쓰기가 허용됩니다(`commands.config: true` 필요).
...
- `channels.msteams.allowFrom`은 안정적인 AAD 개체 ID 또는 `accessGroup:core-team` 같은 정적 발신자 접근 그룹을 사용해야 합니다.
- 허용 목록에 UPN/표시 이름 매칭을 신뢰하지 마세요. 변경될 수 있습니다. OpenClaw는 기본적으로 직접 이름 매칭을 비활성화하며, `channels.msteams.dangerouslyAllowNameMatching: true`로 명시적으로 선택해야 합니다.
- 자격 증명이 허용하는 경우 마법사는 Microsoft Graph를 통해 이름을 ID로 확인할 수 있습니다.

**그룹 접근**

- 기본값: `channels.msteams.groupPolicy = "allowlist"`(`groupAllowFrom`을 추가하지 않는 한 차단됨). 설정되지 않은 경우 기본값을 재정의하려면 `channels.defaults.groupPolicy`를 사용하세요.
- `channels.msteams.groupAllowFrom`은 그룹 채팅/채널에서 어떤 발신자 또는 정적 발신자 접근 그룹이 트리거할 수 있는지 제어합니다(`channels.msteams.allowFrom`으로 폴백).
- 모든 멤버를 허용하려면 `groupPolicy: "open"`을 설정하세요(여전히 기본적으로 멘션 게이트 적용).
...
- 시작 시 OpenClaw는 팀/채널 및 사용자 허용 목록 이름을 ID로 확인하고(Graph 권한이 허용하는 경우)
  매핑을 로그에 남깁니다. 확인되지 않은 팀/채널 이름은 입력한 그대로 유지되지만, `channels.msteams.dangerouslyAllowNameMatching: true`가 활성화되지 않은 한 기본적으로 라우팅에서는 무시됩니다.
...
2. **Azure Bot**(App ID + 시크릿 + tenant ID)을 만듭니다.
...
- 세 파일 `manifest.json`, `outline.png`, `color.png`를 함께 zip으로 묶습니다.
...
프로덕션 배포의 경우 OpenClaw는 클라이언트 시크릿보다 더 안전한 대안으로 **페더레이션 인증**을 지원합니다. 두 가지 방법을 사용할 수 있습니다.
...
1. 인증서를 생성하거나 확보합니다(프라이빗 키가 포함된 PEM 형식).
...
암호 없는 인증을 위해 Azure Managed Identity를 사용합니다. 이는 관리 ID를 사용할 수 있는 Azure 인프라(AKS, App Service, Azure VM) 배포에 적합합니다.
...
4. 토큰은 봇 인증을 위해 Teams SDK로 전달됩니다.
...
5. IMDS(`169.254.169.254`)에 대한 **네트워크 접근을 보장**합니다. NetworkPolicy를 사용하는 경우, 포트 80에서 `169.254.169.254/32`로 향하는 트래픽을 허용하는 이그레스 규칙을 추가합니다.

### 인증 유형 비교

| 방식                 | 구성                                           | 장점                               | 단점                                  |
| -------------------- | ---------------------------------------------- | ---------------------------------- | ------------------------------------- |
| **클라이언트 비밀**  | `appPassword`                                  | 간단한 설정                        | 비밀 순환이 필요하며 보안성이 낮음    |
| **인증서**           | `authType: "federated"` + `certificatePath`    | 네트워크를 통한 공유 비밀 없음     | 인증서 관리 부담                      |
| **관리 ID**          | `authType: "federated"` + `useManagedIdentity` | 비밀번호 없음, 관리할 비밀 없음    | Azure 인프라 필요                     |

**기본 동작:** `authType`이 설정되지 않으면 OpenClaw는 기본적으로 클라이언트 비밀 인증을 사용합니다. 기존 구성은 변경 없이 계속 작동합니다.

## 로컬 개발(터널링)

Teams는 `localhost`에 접근할 수 없습니다. 세션 간 URL이 동일하게 유지되도록 영구 개발 터널을 사용하세요.
...
3. 수신 활동에 대한 Gateway 로그를 확인합니다.

## 환경 변수

모든 구성 키는 대신 환경 변수를 통해 설정할 수 있습니다.
...
OpenClaw는 Microsoft Teams용 Graph 기반 `member-info` 액션을 노출하므로 에이전트와 자동화가 Microsoft Graph에서 직접 채널 구성원 세부 정보(표시 이름, 이메일, 역할)를 확인할 수 있습니다.
...
다음은 Teams 앱 매니페스트에 있는 **기존 resourceSpecific 권한**입니다. 앱이 설치된 팀/채팅 내부에서만 적용됩니다.
...
- `bots[].supportsFiles: true`는 개인 범위에서 파일 처리를 위해 필요합니다.
...
업데이트 후 새 권한이 적용되도록 각 팀에서 앱을 다시 설치하고, 캐시된 앱 메타데이터를 지우기 위해 **Teams를 완전히 종료한 뒤 다시 실행**하세요(창을 닫기만 하는 것이 아님).
...
### **Teams RSC + Microsoft Graph Application 권한**을 사용하는 경우
...
| 기능                    | RSC 권한             | Graph API                           |
| ----------------------- | -------------------- | ----------------------------------- |
| **실시간 메시지**       | 예(Webhook 경유)     | 아니요(폴링만)                      |
| **기록 메시지**         | 아니요               | 예(기록 쿼리 가능)                  |
| **설정 복잡도**         | 앱 매니페스트만      | 관리자 동의 + 토큰 흐름 필요        |
| **오프라인 작동**       | 아니요(실행 중이어야 함) | 예(언제든 쿼리 가능)             |

**요점:** RSC는 실시간 수신용이고, Graph API는 기록 접근용입니다. 오프라인 동안 놓친 메시지를 따라잡으려면 `ChannelMessage.Read.All`이 포함된 Graph API가 필요합니다(관리자 동의 필요).

## Graph 활성화 미디어 + 기록(채널에 필요)
...
OpenClaw는 빠르게 반환하고 사전에 답장을 보내는 방식으로 이를 처리하지만, 매우 느린 응답은 여전히 문제를 일으킬 수 있습니다.
...
- `channels.msteams.allowFrom`: DM 허용 목록(AAD 객체 ID 권장). Graph 액세스를 사용할 수 있으면 설정 마법사가 설정 중 이름을 ID로 확인합니다.
...
- `channels.msteams.chunkMode`: 길이 기준 청크 처리 전에 빈 줄(단락 경계)에서 분할하려면 `length`(기본값) 또는 `newline`입니다.
- `channels.msteams.mediaAllowHosts`: 수신 첨부 파일 호스트의 허용 목록입니다(기본값은 Microsoft/Teams 도메인).
...
- `channels.msteams.actions.memberInfo`: Graph 기반 멤버 정보 작업을 활성화하거나 비활성화합니다(기본값: Graph 자격 증명이 사용 가능하면 활성화).
...
| ------------------------ | --------------------------------------------------------- | ---------------- |
| **게시물**(클래식)       | 메시지가 아래에 스레드형 답글이 달린 카드로 표시됩니다    | `thread`(기본값) |
| **스레드**(Slack 유사)   | 메시지가 Slack처럼 더 선형적으로 흐릅니다                 | `top-level`      |

**문제:** Teams API는 채널이 어떤 UI 스타일을 사용하는지 노출하지 않습니다. 잘못된 `replyStyle`을 사용하면 다음과 같습니다.

- 스레드 스타일 채널에서 `thread` → 답글이 어색하게 중첩되어 표시됩니다
- 게시물 스타일 채널에서 `top-level` → 답글이 스레드 안이 아니라 별도의 최상위 게시물로 표시됩니다
...
봇이 채널에 답장을 보낼 때 `replyStyle`은 가장 구체적인 재정의부터 기본값까지 순서대로 확인됩니다. 첫 번째 non-`undefined` 값이 적용됩니다.
...
명시적 `replyStyle` 없이 전역으로 `requireMention: false`를 설정하면, 수신 메시지가 스레드 답글이었더라도 게시물 스타일 채널의 멘션이 최상위 게시물로 표시됩니다. 예기치 않은 동작을 피하려면 전역, 팀 또는 채널 수준에서 `replyStyle: "thread"`를 고정하세요.

### 스레드 컨텍스트 보존

`replyStyle: "thread"`가 적용 중이고 봇이 채널 스레드 안에서 @mentioned된 경우, OpenClaw는 원래 스레드 루트를 발신 대화 참조(`19:…@thread.tacv2;messageid=<root>`)에 다시 첨부하여 답장이 같은 스레드 안에 도착하도록 합니다. 이는 실시간(턴 내부) 전송과 Bot Framework 턴 컨텍스트가 만료된 뒤 수행되는 사전 전송(예: 오래 실행되는 에이전트, `mcp__openclaw__message`를 통한 대기 중인 도구 호출 답장) 모두에 적용됩니다.

스레드 루트는 대화 참조에 저장된 `threadId`에서 가져옵니다. `threadId` 이전의 오래된 저장 참조는 `activityId`(대화를 마지막으로 시드한 수신 활동)로 폴백하므로, 기존 배포는 다시 시드하지 않아도 계속 작동합니다.

`replyStyle: "top-level"`이 적용 중이면 채널 스레드 수신 메시지는 의도적으로 새 최상위 게시물로 답변됩니다. 스레드 접미사는 첨부되지 않습니다. 이는 스레드 스타일 채널의 올바른 동작입니다. 스레드형 답글을 예상했는데 최상위 게시물이 보인다면, 해당 채널의 `replyStyle` 설정이 잘못된 것입니다.
...
- **채널/그룹:** 첨부 파일은 M365 스토리지(SharePoint/OneDrive)에 있습니다. Webhook 페이로드에는 실제 파일 바이트가 아니라 HTML 스텁만 포함됩니다. **채널 첨부 파일을 다운로드하려면 Graph API 권한이 필요합니다**.
- 명시적인 파일 우선 전송에는 `media` / `filePath` / `path`와 함께 `action=upload-file`을 사용하세요. 선택적 `message`는 함께 전송되는 텍스트/댓글이 되고, `filename`은 업로드되는 이름을 재정의합니다.

Graph 권한이 없으면 이미지가 포함된 채널 메시지는 텍스트만으로 수신됩니다(이미지 콘텐츠는 봇이 액세스할 수 없음).
기본적으로 OpenClaw는 Microsoft/Teams 호스트 이름에서만 미디어를 다운로드합니다. `channels.msteams.mediaAllowHosts`로 재정의하세요(모든 호스트를 허용하려면 `["*"]` 사용).
Authorization 헤더는 `channels.msteams.mediaAuthAllowHosts`에 있는 호스트에만 첨부됩니다(기본값은 Graph + Bot Framework 호스트). 이 목록은 엄격하게 유지하세요(다중 테넌트 접미사는 피하세요).

## 그룹 채팅에서 파일 보내기

봇은 FileConsentCard 흐름(내장)을 사용해 DM에서 파일을 보낼 수 있습니다. 그러나 **그룹 채팅/채널에서 파일을 보내려면** 추가 설정이 필요합니다.
...
OpenClaw는 Teams 설문을 Adaptive Cards로 전송합니다(네이티브 Teams 설문 API는 없음).

- CLI: `openclaw message poll --channel msteams --target conversation:<id> ...`
- 투표는 Gateway가 `~/.openclaw/msteams-polls.json`에 기록합니다.
- 투표를 기록하려면 Gateway가 온라인 상태를 유지해야 합니다.
- 설문은 아직 결과 요약을 자동 게시하지 않습니다(필요한 경우 저장소 파일을 확인하세요).

## 프레젠테이션 카드

`message` 도구 또는 CLI를 사용하여 의미론적 프레젠테이션 페이로드를 Teams 사용자 또는 대화에 보냅니다. OpenClaw는 일반 프레젠테이션 계약에서 이를 Teams Adaptive Cards로 렌더링합니다.

`presentation` 매개변수는 의미론적 블록을 받습니다. `presentation`이 제공되면 메시지 텍스트는 선택 사항입니다.
...
- 선제적 메시지는 사용자가 상호작용한 **후에만** 가능합니다. 그 시점에 대화 참조를 저장하기 때문입니다.
...
- OpenClaw 라우팅에서는 `groupId` 쿼리 매개변수를 **무시**하세요. 이는 Microsoft Entra 그룹 ID이며, 들어오는 Teams 활동에 사용되는 Bot Framework 대화 ID가 아닙니다.
...
- **버전 불일치(Teams에 여전히 이전 매니페스트가 표시됨):** 앱을 제거했다가 다시 추가하고 Teams를 완전히 종료하여 새로고침하세요.
- **Webhook에서 401 Unauthorized:** Azure JWT 없이 수동으로 테스트할 때 예상되는 동작입니다. 엔드포인트에 도달할 수 있지만 인증에 실패했다는 의미입니다. 올바르게 테스트하려면 Azure Web Chat을 사용하세요.

### 매니페스트 업로드 오류

- **"Icon file cannot be empty":** 매니페스트가 0바이트인 아이콘 파일을 참조합니다. 유효한 PNG 아이콘(`outline.png`는 32x32, `color.png`는 192x192)을 만드세요.
- **"webApplicationInfo.Id already in use":** 앱이 다른 팀/채팅에 아직 설치되어 있습니다. 먼저 찾아서 제거하거나 전파될 때까지 5~10분 기다리세요.
- **업로드 시 "Something went wrong":** 대신 [https://admin.teams.microsoft.com](https://admin.teams.microsoft.com)을 통해 업로드하고, 브라우저 DevTools(F12) → Network 탭을 열어 실제 오류의 응답 본문을 확인하세요.
...
4. 올바른 범위를 사용 중인지 확인: 팀의 경우 `ChannelMessage.Read.Group`, 그룹 채팅의 경우 `ChatMessage.Read.Chat`
