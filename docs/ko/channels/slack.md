...
title: Slack
x-i18n:
    generated_at: "2026-05-11T20:21:40Z"
    model: gpt-5.5
    provider: openai
...
<Note>
  단일 Gateway 호스트, 개발 노트북, `*.slack.com`으로 아웃바운드는 가능하지만 인바운드 HTTPS를 받을 수 없는 온프레미스 네트워크에는 **Socket Mode를 선택하세요**.

로드 밸런서 뒤에서 여러 Gateway 복제본을 실행하거나, 아웃바운드 WSS는 차단되었지만 인바운드 HTTPS는 허용되는 경우, 또는 이미 리버스 프록시에서 Slack Webhook을 종료하는 경우에는 **HTTP Request URLs를 선택하세요**.
</Note>
...
```
        ...
        <Note>
          **Recommended**는 번들 Slack plugin의 전체 기능 세트와 일치합니다: App Home, slash commands, 파일, 반응, 핀, 그룹 DM, 이모지/사용자 그룹 읽기. 워크스페이스 정책이 scope를 제한하는 경우 **Minimal**을 선택하세요. DM, 채널/그룹 기록, 멘션, slash commands를 포함하지만 파일, 반응, 핀, 그룹 DM(`mpim:*`), `emoji:read`, `usergroups:read`는 제외합니다. scope별 근거와 추가 slash commands 같은 추가 옵션은 [매니페스트 및 scope 체크리스트](#manifest-and-scope-checklist)를 참고하세요.
        </Note>
...
<AccordionGroup>
  <Accordion title="Optional native slash commands">
    ...
    여러 [기본 슬래시 명령](#commands-and-slash-behavior)은 다음 세부 사항을 고려하여 하나의 구성된 명령 대신 사용할 수 있습니다.
...
<Tabs>
  ...
  <Tab title="채널 정책">
    ...
    항상 Slack 채널 ID를 키로 사용하세요. 찾는 방법: Slack에서 채널을 마우스 오른쪽 버튼으로 클릭 → **링크 복사** — ID(`C...`)가 URL 끝에 나타납니다.
...
<Note>
`replyToMode="off"`는 명시적 `[[reply_to_*]]` 태그를 포함해 Slack의 **모든** 답글 스레딩을 비활성화합니다. 이는 `"off"` 모드에서도 명시적 태그가 여전히 적용되는 Telegram과 다릅니다. Slack 스레드는 채널에서 메시지를 숨기는 반면 Telegram 답글은 인라인으로 계속 표시됩니다.
</Note>
...
이는 다른 채널과 동일한 공유 승인 버튼 표면을 사용합니다. Slack 앱 설정에서 `interactivity`가 활성화되면 승인 프롬프트가 대화 안에 직접 Block Kit 버튼으로 렌더링됩니다.
해당 버튼이 있으면 기본 승인 UX가 됩니다. 도구 결과가 채팅
승인을 사용할 수 없거나 수동 승인이 유일한 경로라고 할 때만 OpenClaw는 수동 `/approve` 명령을 포함해야 합니다.
...
`enabled`가 설정되지 않았거나 `"auto"`이고 하나 이상의 승인자가 해결되면 Slack은 네이티브 exec 승인을 자동으로 활성화합니다. Slack을 네이티브 승인 클라이언트로 명시적으로 비활성화하려면 `enabled: false`를 설정하세요.
승인자가 해결될 때 네이티브 승인을 강제로 켜려면 `enabled: true`를 설정하세요.
...
```
  ...
  <Accordion title="DM 메시지가 무시됨">
    ...
    - 페어링 승인 / 허용 목록 항목
    - Slack 어시스턴트 DM 이벤트: `drop message_changed`를 언급하는 자세한 로그는
      보통 Slack이 메시지 메타데이터에서 복구 가능한 사람 발신자 없이
      편집된 어시스턴트 스레드 이벤트를 보냈음을 의미합니다.
...
```
  ...
  <Accordion title="Socket Mode가 연결되지 않음">
    ...
    구성되어 있지만 현재 런타임이 SecretRef 기반 값을 해결할 수 없었던 것입니다.
  ...
  <Accordion title="HTTP 모드가 이벤트를 수신하지 않음">
    ...
    표시되면, HTTP 계정은 구성되어 있지만 현재 런타임이 SecretRef 기반
    서명 시크릿을 확인할 수 없었다는 뜻입니다.
