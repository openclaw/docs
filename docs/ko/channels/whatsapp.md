...
title: WhatsApp
x-i18n:
    generated_at: "2026-05-05T06:06:23Z"
    model: gpt-5.5
    provider: openai
...
- `openclaw channels login --channel whatsapp`도 아직 Plugin이 없으면 설치 흐름을
  제공합니다.
...
Windows에서는 WhatsApp Plugin이 npm 설치 중 `PATH`의 Git을 필요로 합니다. Baileys/libsignal 종속성 중
...
<Note>
OpenClaw는 가능하면 별도 번호에서 WhatsApp을 실행하는 것을 권장합니다. (채널 메타데이터와 설정 흐름은 해당 설정에 최적화되어 있지만, 개인 번호 설정도 지원됩니다.)
</Note>
...
인바운드 WhatsApp 메시지 내용과 식별자를 수신해도 된다고 신뢰하는 Plugin에만
이를 활성화하세요.
...
확인 반응은 `reactionLevel`에 의해 제어됩니다. `reactionLevel`이 `"off"`이면 억제됩니다.
...
<AccordionGroup>
  ...
  <Accordion title="답장이 기록에는 보이지만 WhatsApp에는 보이지 않음">
