...
title: WhatsApp
x-i18n:
    generated_at: "2026-05-03T06:15:55Z"
    model: gpt-5.5
    provider: openai
...
현재 공식 릴리스 태그를 따르려면 베어 패키지를 사용하세요. 재현 가능한 설치가
...
<Note>
OpenClaw는 가능하면 WhatsApp을 별도 번호에서 실행하는 것을 권장합니다. (채널 메타데이터와 설정 흐름은 이 설정에 최적화되어 있지만, 개인 번호 설정도 지원됩니다.)
</Note>
...
<AccordionGroup>
  ...
  <Accordion title="개인 번호 대체 모드">
    온보딩은 개인 번호 모드를 지원하며 자기 자신과의 채팅에 적합한 기준값을 작성합니다:

    - `dmPolicy: "allowlist"`
    - `allowFrom`에 개인 번호 포함
    - `selfChatMode: true`

    런타임에서는 연결된 자기 번호와 `allowFrom`을 기준으로 자기 자신과의 채팅 보호가 동작합니다.
...
- 재연결 워치독은 인바운드 앱 메시지 양만이 아니라 WhatsApp Web 전송 활동을 사용하므로, 조용한 연결 기기 세션은 최근에 아무도 메시지를 보내지 않았다는 이유만으로 재시작되지 않습니다. 전송 프레임이 계속 도착하지만 워치독 기간 동안 애플리케이션 메시지가 처리되지 않으면 더 긴 애플리케이션 무음 상한이 여전히 재연결을 강제합니다. 최근 활성 세션의 일시적 재연결 이후에는 해당 애플리케이션 무음 검사가 첫 복구 기간에 일반 메시지 타임아웃을 사용합니다.
- Baileys 소켓 타이밍은 `web.whatsapp.*` 아래에 명시됩니다. `keepAliveIntervalMs`는 WhatsApp Web 애플리케이션 ping을 제어하고, `connectTimeoutMs`는 여는 핸드셰이크 타임아웃을 제어하며, `defaultQueryTimeoutMs`는 Baileys 쿼리 타임아웃을 제어합니다.
- 아웃바운드 전송에는 대상 계정의 활성 WhatsApp 리스너가 필요합니다.
- 그룹 전송은 토큰이 LID 기반 그룹을 포함한 현재 WhatsApp 참가자 메타데이터와 일치할 때 텍스트 및 미디어 캡션의 `@+<digits>` 및 `@<digits>` 토큰에 네이티브 멘션 메타데이터를 첨부합니다.
- 상태 및 브로드캐스트 채팅은 무시됩니다(`@status`, `@broadcast`).
- 재연결 워치독은 인바운드 앱 메시지 양만이 아니라 WhatsApp Web 전송 활동을 따릅니다. 조용한 연결 기기 세션은 전송 프레임이 계속되는 동안 유지되지만, 전송 정지는 이후의 원격 연결 해제 경로보다 훨씬 먼저 재연결을 강제합니다.
- 직접 채팅은 DM 세션 규칙을 사용합니다(`session.dmScope`; 기본값 `main`은 DM을 에이전트 기본 세션으로 축소합니다).
- 그룹 세션은 격리됩니다(`agent:<agentId>:whatsapp:group:<jid>`).
- WhatsApp 채널/뉴스레터는 네이티브 `@newsletter` JID를 가진 명시적 아웃바운드 대상이 될 수 있습니다. 아웃바운드 뉴스레터 전송은 DM 세션 의미 체계가 아니라 채널 세션 메타데이터(`agent:<agentId>:whatsapp:channel:<jid>`)를 사용합니다.
...
인바운드 WhatsApp 메시지 내용과 식별자를 받도록 신뢰하는 Plugin에 대해서만 이 기능을 활성화하세요.
...
<Tabs>
  <Tab title="DM 정책">
    ...
    다중 계정 오버라이드: `channels.whatsapp.accounts.<id>.dmPolicy`(및 `allowFrom`)가 해당 계정에 대해 채널 수준 기본값보다 우선합니다.

    런타임 동작 세부 정보:

    - 페어링은 채널 허용 저장소에 유지되고 구성된 `allowFrom`과 병합됩니다
    - 예약 자동화 및 Heartbeat 수신자 대체는 명시적 전달 대상 또는 구성된 `allowFrom`을 사용합니다. DM 페어링 승인은 암시적 Cron 또는 Heartbeat 수신자가 아닙니다
  ...
  <Tab title="멘션 + /activation">
    ...
    - 인용/답장은 멘션 게이트만 충족하며, 발신자 권한을 부여하지 **않습니다**
...
연결된 자기 번호가 `allowFrom`에도 있으면 WhatsApp 자기 자신과의 채팅 보호가 활성화됩니다:

- 자기 자신과의 채팅 턴에 대해 읽음 확인 건너뛰기
- 그렇지 않으면 자신을 ping할 수 있는 mention-JID 자동 트리거 동작 무시
- `messages.responsePrefix`가 설정되지 않은 경우 자기 자신과의 채팅 답장은 기본적으로 `[{identity.name}]` 또는 `[openclaw]`로 설정됩니다
...
<AccordionGroup>
  <Accordion title="인바운드 엔벨로프 + 답장 컨텍스트">
    ...
    인용된 답장 대상이 다운로드 가능한 미디어인 경우 OpenClaw는 일반 인바운드 미디어 저장소를 통해 저장하고 `MediaPath`/`MediaType`으로 노출하여 에이전트가
    `<media:image>`만 보는 대신 참조된 이미지를 검사할 수 있게 합니다.
  ...
  <Accordion title="미디어 플레이스홀더 및 위치/연락처 추출">
    ...
    승인된 그룹 음성 메모는 본문이 `<media:audio>`뿐인 경우 멘션 게이트 전에 전사되므로, 음성 메모에서 봇 멘션을 말하면
    답장을 트리거할 수 있습니다. 전사문에 여전히 봇 멘션이 없으면
    전사문은 원시 플레이스홀더 대신 대기 중인 그룹 기록에 보관됩니다.
  ...
  <Accordion title="대기 중인 그룹 기록 주입">
    그룹의 경우 처리되지 않은 메시지를 버퍼링한 뒤 봇이 최종적으로 트리거될 때 컨텍스트로 주입할 수 있습니다.
  ...
  <Accordion title="읽음 확인">
    ...
    자체 채팅 턴은 전역으로 활성화되어 있어도 읽음 확인을 건너뜁니다.
...
<AccordionGroup>
  <Accordion title="텍스트 청크 분할">
    ...
    - `newline` 모드는 문단 경계(빈 줄)를 우선 사용한 다음, 길이 제한에 안전한 청크 분할로 대체합니다.

  </Accordion>

  <Accordion title="아웃바운드 미디어 동작">
    - 이미지, 동영상, 오디오(PTT 음성 메모), 문서 페이로드를 지원합니다.
    - 오디오 미디어는 Baileys `audio` 페이로드와 `ptt: true`를 통해 전송되므로, WhatsApp 클라이언트가 이를 푸시 투 토크 음성 메모로 렌더링합니다.
    - 답장 페이로드는 `audioAsVoice`를 보존합니다. 공급자가 MP3 또는 WebM을 반환하더라도 WhatsApp용 TTS 음성 메모 출력은 이 PTT 경로를 유지합니다.
    - 네이티브 Ogg/Opus 오디오는 음성 메모 호환성을 위해 `audio/ogg; codecs=opus`로 전송됩니다.
    - Microsoft Edge TTS MP3/WebM 출력을 포함한 Ogg가 아닌 오디오는 PTT 전송 전에 `ffmpeg`로 48 kHz 모노 Ogg/Opus로 트랜스코딩됩니다.
    - `/tts latest`는 최신 어시스턴트 답장을 하나의 음성 메모로 전송하고 같은 답장에 대한 반복 전송을 억제합니다. `/tts chat on|off|default`는 현재 WhatsApp 채팅의 자동 TTS를 제어합니다.
    - 동영상 전송에서 `gifPlayback: true`를 통해 애니메이션 GIF 재생을 지원합니다.
    - 다중 미디어 답장 페이로드를 보낼 때 캡션은 첫 번째 미디어 항목에 적용됩니다. 단, PTT 음성 메모는 WhatsApp 클라이언트가 음성 메모 캡션을 일관되게 렌더링하지 않으므로 오디오를 먼저 보내고 표시 텍스트는 별도로 보냅니다.
  ...
  <Accordion title="미디어 크기 제한 및 대체 동작">
    ...
    - 이미지는 제한에 맞도록 자동 최적화됩니다(크기 조정/품질 조정).
...
| `"batched"` | 대기열에 들어간 일괄 답장은 인용하고 즉시 답장은 인용하지 않음        |
...
WhatsApp은 `channels.whatsapp.ackReaction`을 통해 인바운드 수신 시 즉각적인 확인 반응을 지원합니다.
확인 반응은 `reactionLevel`로 게이트됩니다. `reactionLevel`이 `"off"`이면 억제됩니다.
...
- 실패는 로그에 기록되지만 정상적인 답장 전송을 차단하지 않습니다.
- 그룹 모드 `mentions`는 멘션으로 트리거된 턴에서 반응합니다. 그룹 활성화 `always`는 이 검사를 우회합니다.
...
<AccordionGroup>
  <Accordion title="계정 선택 및 기본값">
    ...
    - 기본 계정 선택: `default`가 있으면 이를 사용하고, 그렇지 않으면 처음 구성된 계정 ID(정렬 기준)를 사용합니다.
  ...
  <Accordion title="자격 증명 경로 및 레거시 호환성">
    ...
    - `~/.openclaw/credentials/`의 레거시 기본 인증은 기본 계정 흐름에서 여전히 인식되거나 마이그레이션됩니다.
  ...
  <Accordion title="로그아웃 동작">
    ...
    Gateway에 연결할 수 있으면, 로그아웃은 먼저 선택한 계정의 라이브 WhatsApp 리스너를 중지하여 연결된 세션이 다음 재시작 전까지 메시지를 계속 수신하지 않도록 합니다. `openclaw channels remove --channel whatsapp`도 계정 구성을 비활성화하거나 삭제하기 전에 라이브 리스너를 중지합니다.
...
<AccordionGroup>
  ...
  <Accordion title="연결되었지만 연결 끊김 / 재연결 루프">
    ...
    조용한 계정은 일반 메시지 시간 제한을 지나서도 연결 상태를 유지할 수 있습니다. Watchdog은 WhatsApp Web 전송 활동이 중지되거나, 소켓이 닫히거나, 애플리케이션 수준 활동이 더 긴 안전 시간 범위를 넘어 조용히 유지될 때 다시 시작합니다.

    로그에 `status=408 Request Time-out Connection was lost`가 반복해서 표시되면 `web.whatsapp` 아래의 Baileys 소켓 타이밍을 조정하세요. 먼저 `keepAliveIntervalMs`를 네트워크의 유휴 시간 제한보다 짧게 줄이고, 느리거나 손실이 있는 링크에서는 `connectTimeoutMs`를 늘리세요.
    ...
    `~/.openclaw/logs/whatsapp-health.log`에 `Gateway inactive`라고 표시되지만 `openclaw gateway status`와 `openclaw channels status --probe`에서 Gateway와 WhatsApp이 정상이라고 표시되면 `openclaw doctor`를 실행하세요. Linux에서 doctor는 여전히 `~/.openclaw/bin/ensure-whatsapp.sh`를 호출하는 레거시 crontab 항목에 대해 경고합니다. cron에는 systemd 사용자 버스 환경이 없을 수 있어 이전 스크립트가 Gateway 상태를 잘못 보고할 수 있으므로, `crontab -e`로 오래된 항목을 제거하세요.
  ...
  <Accordion title="프록시 뒤에서 QR 로그인이 시간 초과됨">
    ...
    WhatsApp Web 로그인은 Gateway 호스트의 표준 프록시 환경(`HTTPS_PROXY`, `HTTP_PROXY`, 소문자 변형 및 `NO_PROXY`)을 사용합니다. Gateway 프로세스가 프록시 환경 변수를 상속하는지, 그리고 `NO_PROXY`가 `mmg.whatsapp.net`과 일치하지 않는지 확인하세요.

  </Accordion>

  <Accordion title="전송 시 활성 리스너 없음">
    대상 계정에 대한 활성 Gateway 리스너가 없으면 아웃바운드 전송은 빠르게 실패합니다.

    Gateway가 실행 중이고 계정이 연결되어 있는지 확인하세요.

  </Accordion>

  <Accordion title="답장이 기록에는 나타나지만 WhatsApp에는 나타나지 않음">
    기록 행은 에이전트가 생성한 내용을 기록합니다. WhatsApp 전송은 별도로 확인됩니다. OpenClaw는 Baileys가 하나 이상의 표시 텍스트 또는 미디어 전송에 대해 아웃바운드 메시지 ID를 반환한 뒤에만 자동 답장이 전송된 것으로 처리합니다.

    확인 반응은 독립적인 답장 전 수신 확인입니다. 반응이 성공했다고 해서 이후 텍스트 또는 미디어 답장이 WhatsApp에 수락되었다는 뜻은 아닙니다.
  ...
  <Accordion title="그룹 메시지가 예기치 않게 무시됨">
    ...
    - `openclaw.json`(JSON5)의 중복 키: 나중 항목이 이전 항목을 재정의하므로 범위마다 `groupPolicy`를 하나만 유지하세요.
...
WhatsApp은 `groups` 및 `direct` 맵을 통해 그룹 및 직접 채팅에 대해 Telegram 스타일 시스템 프롬프트를 지원합니다.
...
1. **직접 채팅별 시스템 프롬프트**(`direct["<peerId>"].systemPrompt`): 특정 피어 항목이 맵에 존재하고 **그리고** 해당 `systemPrompt` 키가 정의되어 있을 때 사용됩니다. `systemPrompt`가 빈 문자열(`""`)이면 와일드카드가 억제되고 시스템 프롬프트가 적용되지 않습니다.
2. **직접 채팅 와일드카드 시스템 프롬프트**(`direct["*"].systemPrompt`): 특정 피어 항목이 맵에 전혀 없거나, 존재하지만 `systemPrompt` 키를 정의하지 않을 때 사용됩니다.

<Note>
`dms`는 DM별 경량 기록 재정의 버킷(`dms.<id>.historyLimit`)으로 남아 있습니다. 프롬프트 재정의는 `direct` 아래에 있습니다.
</Note>

**Telegram 다중 계정 동작과의 차이점:** Telegram에서는 다중 계정 설정에서 루트 `groups`가 모든 계정에 대해 의도적으로 억제됩니다. 자체 `groups`를 정의하지 않은 계정까지 포함됩니다. 이는 봇이 자신이 속하지 않은 그룹의 그룹 메시지를 수신하지 못하게 하기 위한 것입니다. WhatsApp에는 이 보호 장치가 적용되지 않습니다. 루트 `groups`와 루트 `direct`는 구성된 계정 수와 관계없이 계정 수준 재정의를 정의하지 않은 계정에 항상 상속됩니다. 다중 계정 WhatsApp 설정에서 계정별 그룹 또는 직접 프롬프트를 원한다면 루트 수준 기본값에 의존하지 말고 각 계정 아래에 전체 맵을 명시적으로 정의하세요.
...
- 그룹 허용과 발신자 권한 부여는 별개의 검사입니다. `groups["*"]`는 그룹 처리가 도달할 수 있는 그룹 집합을 넓히지만, 그 자체만으로 해당 그룹의 모든 발신자를 권한 부여하지는 않습니다. 발신자 접근은 여전히 `channels.whatsapp.groupPolicy`와 `channels.whatsapp.groupAllowFrom`에 의해 별도로 제어됩니다.
- `channels.whatsapp.direct`에는 DM에 대해 같은 부작용이 없습니다. `direct["*"]`는 `dmPolicy`와 `allowFrom` 또는 페어링 저장소 규칙에 의해 DM이 이미 허용된 후 기본 직접 채팅 구성만 제공합니다.
