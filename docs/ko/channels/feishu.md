...
**상태:** bot DM + 그룹 채팅에 프로덕션 준비 완료. WebSocket이 기본 모드이며, Webhook 모드는 선택 사항입니다.
...
<Steps>
  ...
  ```
  Feishu/Lark 모바일 앱으로 QR 코드를 스캔하여 Feishu/Lark bot을 자동으로 생성합니다.
  </Step>
...
bot에게 DM을 보낼 수 있는 사용자를 제어하려면 `dmPolicy`를 구성하세요.

- `"pairing"` - 알 수 없는 사용자는 페어링 코드를 받으며, CLI를 통해 승인합니다
- `"allowlist"` - `allowFrom`에 나열된 사용자만 채팅할 수 있습니다(기본값: bot 소유자만)
...
Gateway를 시작하고 bot에게 DM을 보낸 다음 로그를 확인하세요.
...
`accounts.<id>.tts`는 `messages.tts`와 동일한 형태를 사용하며 전역 TTS 구성 위에 깊은 병합을 수행하므로, 여러 bot을 사용하는 Feishu 설정은 공유 provider 자격 증명을 전역으로 유지하면서 계정별로 음성, 모델, 페르소나 또는 자동 모드만 재정의할 수 있습니다.
...
`--thread here`는 DM 및 Feishu/Lark 스레드 메시지에서 동작합니다. 바인딩된 대화의 후속 메시지는 해당 ACP 세션으로 직접 라우팅됩니다.
...
조회 팁은 [그룹/사용자 ID 가져오기](#get-groupuser-ids)를 참고하세요.
...
| `channels.feishu.requireMention`                  | 그룹에서 @멘션 요구                                                              | `true`           |
...
| `channels.feishu.blockStreaming`                  | 완료된 블록 답글 스트리밍                                                        | `false`          |
...
인바운드 Feishu/Lark 오디오 메시지는 원시 `file_key` JSON 대신 미디어 자리 표시자로 정규화됩니다. `tools.media.audio`가 구성되어 있으면 OpenClaw는 음성 메모 리소스를 다운로드하고 에이전트 턴 전에 공유 오디오 전사를 실행하므로, 에이전트는 음성 전사문을 받습니다. Feishu가 오디오 페이로드에 전사 텍스트를 직접 포함하는 경우 해당 텍스트가 추가 ASR 호출 없이 사용됩니다. 오디오 전사 제공자가 없더라도 에이전트는 원시 Feishu 리소스 페이로드가 아니라 저장된 첨부 파일과 함께 `<media:audio>` 자리 표시자를 받습니다.
...
네이티브 Feishu/Lark 오디오 말풍선은 Feishu `audio` 메시지 유형을 사용하며 Ogg/Opus 업로드 미디어(`file_type: "opus"`)가 필요합니다. 기존 `.opus` 및 `.ogg` 미디어는 네이티브 오디오로 직접 전송됩니다. MP3/WAV/M4A 및 기타 오디오로 보이는 형식은 답글이 음성 전달을 요청할 때만(`audioAsVoice` / 메시지 도구 `asVoice`, TTS 음성 메모 답글 포함) `ffmpeg`로 48kHz Ogg/Opus로 트랜스코딩됩니다. 일반 MP3 첨부 파일은 일반 파일로 유지됩니다. `ffmpeg`가 없거나 변환에 실패하면 OpenClaw는 파일 첨부로 대체하고 이유를 로그에 기록합니다.

### 스레드 및 답글

- ✅ 인라인 답글
- ✅ 스레드 답글
- ✅ 스레드 메시지에 답글을 달 때 미디어 답글은 스레드를 인식한 상태로 유지됩니다

`groupSessionScope: "group_topic"` 및 `"group_topic_sender"`의 경우 네이티브 Feishu/Lark 주제 그룹은 이벤트 `thread_id`(`omt_*`)를 표준 주제 세션 키로 사용합니다. 네이티브 주제 시작 이벤트에 `thread_id`가 없으면 OpenClaw는 턴을 라우팅하기 전에 Feishu에서 이를 보강합니다. OpenClaw가 스레드로 변환하는 일반 그룹 답글은 계속 답글 루트 메시지 ID(`om_*`)를 사용하므로 첫 번째 턴과 후속 턴이 같은 세션에 유지됩니다.
