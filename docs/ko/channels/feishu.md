...
`allowlist` 모드에서는 명시적 `groups.<chat_id>` 항목을 추가하여 그룹을 허용할 수도 있습니다. 명시적 항목은 `groupPolicy: "disabled"`를 재정의하지 않습니다. `groups.*` 아래의 와일드카드 기본값은 일치하는 그룹을 구성하지만, 그 자체로 그룹을 허용하지는 않습니다.
