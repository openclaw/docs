...
summary: Node의 위치 명령(`location.get`), 권한 모드 및 Android 포그라운드 동작
title: 위치 명령어
x-i18n:
    generated_at: "2026-07-12T15:24:06Z"
    model: gpt-5.6
...
- 기본적으로 꺼져 있습니다.
- Android 서드 파티 빌드에서는 끄기 / 사용 중 / 항상 선택기를 사용합니다. Play 빌드에서는 끄기 / 사용 중만 제공됩니다.
- 정밀한 위치는 별도의 토글입니다.

## 단순한 스위치가 아닌 선택기를 사용하는 이유

OS 위치 권한은 여러 수준으로 나뉩니다. 정밀한 위치도 별도의 OS 권한입니다(iOS 14+의 "Precise", Android의 "fine"과 "coarse"). 앱 내 선택기는 요청할 모드를 결정하지만, 실제로 부여할 권한은 여전히 OS가 결정합니다.
...
- Android 서드 파티 빌드에서 `always`를 선택하면 먼저 포그라운드 권한을 요청하고 백그라운드 액세스를 설명한 다음, 별도의 **Allow all the time** 권한을 설정할 수 있도록 Android 앱 설정을 엽니다.
...
- `LOCATION_BACKGROUND_UNAVAILABLE`: 앱이 백그라운드 상태이지만 사용 중 권한만 부여되어 있습니다.
- `LOCATION_TIMEOUT`: 제한 시간 내에 위치를 확인하지 못했습니다.
- `LOCATION_UNAVAILABLE`: 시스템 오류가 발생했거나 사용 가능한 제공자가 없습니다.

## 백그라운드 동작

- Android 서드 파티 빌드는 사용자가 `Always`를 선택하고 Android가 백그라운드 위치 권한을 부여한 경우에만 백그라운드 `location.get`을 허용합니다. 기존의 지속 실행 Node 서비스는 `location` 서비스 유형을 추가하고 활성 상태인 동안 `Location: Always`를 명시합니다.
- Android Play 빌드와 `While Using` 모드에서는 백그라운드 상태일 때 `location.get`을 거부합니다.
- 다른 Node 플랫폼에서는 동작이 다를 수 있습니다.

## 모델/도구 연동

- 에이전트 도구: `nodes` 도구의 `location_get` 작업입니다(Node 필수).
...
- 정밀: "정밀한 GPS 위치를 사용합니다. 대략적인 위치를 공유하려면 토글을 끄십시오."
