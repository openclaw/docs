...
summary: 전송, Gateway, 에이전트 응답을 위한 이미지 및 미디어 처리 규칙
title: 이미지 및 미디어 지원
x-i18n:
...
- `openclaw message send --media`로 선택적 캡션이 있는 미디어를 전송합니다.
- 웹 받은편지함의 자동 응답이 텍스트와 함께 미디어를 포함할 수 있게 합니다.
...
- 흐름: Buffer로 로드하고, 미디어 종류를 감지한 다음, 올바른 페이로드를 빌드합니다.
  - **이미지:** `channels.whatsapp.mediaMaxMb`(기본값: 50 MB)를 목표로 JPEG로 크기 조정 및 재압축(최대 변 2048px).
  - **오디오/음성/비디오:** 최대 16 MB까지 그대로 전달하며, 오디오는 음성 노트(`ptt: true`)로 전송됩니다.
  - **문서:** 그 밖의 모든 항목은 최대 100 MB까지 허용되며, 가능한 경우 파일 이름을 보존합니다.
- WhatsApp GIF 스타일 재생: 모바일 클라이언트에서 인라인 반복 재생되도록 `gifPlayback: true`가 있는 MP4를 전송합니다(CLI: `--gif-playback`).
...
- 수신 웹 메시지에 미디어가 포함되면 OpenClaw는 임시 파일로 다운로드하고 템플릿 변수를 노출합니다.
  - 수신 미디어용 `{{MediaUrl}}` 의사 URL.
  - 명령을 실행하기 전에 기록되는 `{{MediaPath}}` 로컬 임시 경로.
- 세션별 Docker 샌드박스가 활성화되어 있으면 수신 미디어가 샌드박스 워크스페이스로 복사되고, `MediaPath`/`MediaUrl`은 `media/inbound/<filename>` 같은 상대 경로로 다시 작성됩니다.
- 미디어 이해가 `tools.media.*` 또는 공유 `tools.media.models`를 통해 구성된 경우 템플릿 처리 전에 실행되며, `[Image]`, `[Audio]`, `[Video]` 블록을 `Body`에 삽입할 수 있습니다.
  ...
  - 활성 기본 이미지 모델이 이미 네이티브 비전을 지원하는 경우 OpenClaw는 `[Image]` 요약 블록을 건너뛰고 대신 원본 이미지를 모델에 전달합니다.
- 기본적으로 일치하는 첫 번째 이미지/오디오/비디오 첨부 파일만 처리됩니다. 여러 첨부 파일을 처리하려면 `tools.media.<cap>.attachments`를 설정하세요.

## 제한 및 오류

**발신 전송 한도(WhatsApp 웹 전송)**
...
- 이미지 재압축(크기 제한)과 오디오의 음성 노트 플래그를 검증합니다.
- 여러 미디어 응답이 순차 전송으로 펼쳐지는지 확인합니다.
