...
Google Plugin은 Google AI Studio를 통한 Gemini 모델 액세스와 이미지 생성, 미디어 이해(이미지/오디오/동영상), 텍스트 음성 변환 및 Gemini Grounding을 통한 웹 검색 기능을 제공합니다.
...
<Tabs>
  <Tab title="API 키">
    **권장 용도:** Google AI Studio를 통한 표준 Gemini API 액세스.
    ...
    <Tip>
    `GEMINI_API_KEY`와 `GOOGLE_API_KEY`를 모두 사용할 수 있습니다. 이미 구성된 키를 사용하십시오.
    </Tip>

  </Tab>

  <Tab title="Gemini CLI (OAuth)">
    **권장 용도:** 별도의 API 키 대신 PKCE OAuth를 통해 기존 Gemini CLI 로그인을 재사용합니다.

    <Warning>
    `google-gemini-cli` 제공자는 비공식 통합입니다. 일부 사용자는 이 방식으로
    OAuth를 사용할 때 계정이 제한된다고 보고합니다. 위험을 감수하고 사용하십시오.
    </Warning>
    ...
    <Steps>
      <Step title="Gemini CLI 설치">
        ...
        OpenClaw는 일반적인 Windows/npm 구성을 포함하여 Homebrew 설치와 전역 npm
        설치를 모두 지원합니다.
      </Step>
    ...
    Gemini 3.1 Pro의 Gemini API 모델 ID는 `gemini-3.1-pro-preview`입니다. OpenClaw는 편의를 위해 더 짧은 `google/gemini-3.1-pro`를 별칭으로 허용하며 제공자 호출 전에 이를 정규화합니다.
    ...
    **환경 변수:**
    ...
    `GOOGLE_CLOUD_PROJECT` 또는 `GOOGLE_CLOUD_PROJECT_ID`를 설정하고 다시 시도하십시오.
    </Note>
    ...
    `google-gemini-cli/*` 모델 참조는 레거시 호환성 별칭입니다. 로컬
    Gemini CLI 실행을 사용하려는 새 구성은 `google/*` 모델 참조와
...
<Note>
`google/gemini-3-pro-preview`는 2026-03-09에 사용 중단되었으므로 대신 `google/gemini-3.1-pro-preview`를 사용하십시오. Gemini API 키 설정을 다시 실행하면(`openclaw onboard --auth-choice gemini-api-key` 또는 `openclaw models auth login --provider google`) 오래된 기본 모델 구성이 현재 모델로 다시 작성됩니다.
</Note>
...
`GEMINI_API_KEY` 다음으로 `models.providers.google.apiKey`를 재사용하게 하십시오.
...
`models.providers.google.apiKey` 순서입니다. `webSearch.baseUrl`은 선택 사항이며
운영자 프록시 또는 호환되는 Gemini API 엔드포인트용으로 제공됩니다. 생략하면
Gemini 웹 검색에서 `models.providers.google.baseUrl`을 재사용합니다. 제공자별 도구 동작은
...
`/think adaptive`는 고정된 OpenClaw 수준을 선택하는 대신 Google의 동적 사고
의미 체계를 유지합니다. Gemini 3 및 Gemini 3.1은 고정된 `thinkingLevel`을 생략하여
Google이 수준을 선택할 수 있도록 하며, Gemini 2.5는 Google의 동적 센티널 값인
...
사고 기능을 `off`로 설정하면 `MINIMAL`에 매핑하지 않고 사고 기능을 비활성화된 상태로
유지합니다.
...
- 기하 구조 제어: `size`, `aspectRatio`, `resolution`
...
번들 `google` Plugin은 공유 `video_generate` 도구를 통한 동영상 생성도
등록합니다.

- 기본 동영상 모델: `google/veo-3.1-fast-generate-preview`
- 모드: 텍스트에서 동영상 생성, 이미지에서 동영상 생성 및 단일 동영상 참조 흐름
- `aspectRatio`(`16:9`, `9:16`) 및 `resolution`(`720P`, `1080P`)을 지원하며, 현재 Veo에서는 오디오 출력을 지원하지 않습니다.
...
번들 `google` Plugin은 공유 `music_generate` 도구를 통한 음악 생성도
등록합니다.

- 기본 음악 모델: `google/lyria-3-clip-preview`
- `google/lyria-3-pro-preview`도 지원합니다.
- 프롬프트 제어: `lyrics` 및 `instrumental`
- 출력 형식: 기본값은 `mp3`이며 `google/lyria-3-pro-preview`에서는 `wav`도 지원
- 참조 입력: 최대 10개 이미지
...
- 음성 메모 출력: Google PCM은 WAV로 래핑된 후 `ffmpeg`를 사용하여 48 kHz Opus로 트랜스코딩됩니다.

Google의 배치 Gemini TTS 경로는 완료된 `generateContent` 응답에서 생성된
오디오를 반환합니다. 지연 시간이 가장 짧은 음성 대화를 위해서는 배치
TTS 대신 Gemini Live API 기반 Google 실시간 음성 제공자를 사용하십시오.
...
음성으로 읽을 텍스트 앞에 재사용 가능한 스타일 프롬프트를 추가하려면
`audioProfile`을 설정하십시오. 프롬프트 텍스트에서 이름이 지정된 화자를
언급할 때는 `speakerName`을 설정하십시오.

Gemini API TTS는 텍스트에서 `[whispers]` 또는 `[laughs]`와 같은 표현력 있는
대괄호 오디오 태그도 허용합니다. 태그를 TTS에 전송하면서 표시되는 채팅
응답에서는 제외하려면 `[[tts:text]]...[[/tts:text]]` 블록 안에 넣으십시오.

```text
다음은 깔끔한 응답 텍스트입니다.

[[tts:text]][whispers] 다음은 음성으로 읽는 버전입니다.[[/tts:text]]
```

<Note>
Gemini API로 제한된 Google Cloud Console API 키를 이 제공자에 사용할 수
있습니다. 이는 별도의 Cloud Text-to-Speech API 경로가 아닙니다.
</Note>

## 실시간 음성

번들 `google` Plugin은 Voice Call 및 Google Meet와 같은 백엔드 오디오
브리지를 위해 Gemini Live API 기반 실시간 음성 제공자를 등록합니다.

| 설정                  | 구성 경로                                                           | 기본값                                                                                |
| --------------------- | ------------------------------------------------------------------- | ------------------------------------------------------------------------------------- |
| 모델                  | `plugins.entries.voice-call.config.realtime.providers.google.model` | `gemini-3.1-flash-live-preview`                                                       |
| 음성                  | `...google.voice`                                                   | `Kore`                                                                                |
| Temperature           | `...google.temperature`                                             | (설정되지 않음)                                                                       |
...
`temperature`를 설정하지 마십시오. Google Live는 `temperature: 0`일 때 오디오 없이
...
<Note>
Gemini 3.1 Live는 실시간 입력을 통한 대화형 텍스트를 허용하며 순차적 함수 호출을
사용합니다. OpenClaw는 이 모델에서 이전의 `NON_BLOCKING`, 함수 응답 예약 및
감정 대화 필드를 생략합니다. `thinkingLevel`을 사용하는 것이 좋습니다. 구성된 양수
`thinkingBudget` 값은 가장 가까운 지원 수준으로 매핑되며, `-1`은 Google의 기본값을
그대로 유지합니다. [Gemini Live 기능 비교](https://ai.google.dev/gemini-api/docs/live-api/capabilities)를
참조하십시오.
</Note>

<Note>
Control UI Talk는 제약이 적용된 일회용 토큰을 사용하는 Google Live 브라우저 세션을
지원합니다. 백엔드 전용 실시간 음성 제공자는 일반 Gateway 릴레이 전송을 통해서도
실행할 수 있으며, 이 경우 제공자 자격 증명은 Gateway에 유지됩니다.
</Note>

관리자가 실시간으로 검증하려면
`OPENAI_API_KEY=... GEMINI_API_KEY=... node --import tsx scripts/dev/realtime-talk-live-smoke.ts`를 실행하십시오.
이 스모크 테스트는 OpenAI 백엔드/WebRTC 경로도 다룹니다. Google 구간은 Control UI Talk에서
사용하는 것과 동일한 제약된 Live API 토큰 형식을 발급하고, 브라우저 WebSocket 엔드포인트를
열고, 초기 설정 페이로드를 전송한 다음 `setupComplete`를 기다립니다.

## 고급 구성

<AccordionGroup>
  <Accordion title="Gemini 캐시 직접 재사용">
    Gemini API를 직접 실행할 때(`api: "google-generative-ai"`) OpenClaw는
    구성된 `cachedContent` 핸들을 Gemini 요청에 전달합니다.

    - 모델별 또는 전역 매개변수는 `cachedContent` 또는 레거시
      `cached_content` 중 하나로 구성합니다.
    - 더 구체적인 범위의 매개변수(전역보다 모델 수준)가 항상 우선합니다.
      같은 범위에서 두 키가 모두 설정된 경우 `cached_content`가 우선합니다.
      예상치 못한 동작을 방지하려면 범위마다 하나의 키만 사용하십시오.
  ...
  <Accordion title="Gemini CLI 사용 참고 사항">
    ...
    정규화합니다. 레거시 `--output-format json` 재정의는 계속 JSON 파서를
    사용합니다.

    - 스트리밍된 응답 텍스트는 어시스턴트 `message` 이벤트에서 가져옵니다.
    - 레거시 JSON 출력에서는 응답 텍스트를 CLI JSON의 `response` 필드에서 가져옵니다.
  ...
  <Accordion title="환경 및 데몬 설정">
    ...
    해당 프로세스에서 사용할 수 있는지 확인하십시오(예: `~/.openclaw/.env` 또는
