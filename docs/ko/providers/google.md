...
title: Google (Gemini)
x-i18n:
    generated_at: "2026-05-10T19:48:58Z"
    model: gpt-5.5
    provider: openai
...
- 런타임 옵션: provider/model `agentRuntime.id: "google-gemini-cli"`는
  모델 참조를 `google/*`로 표준화한 상태로 유지하면서 Gemini CLI OAuth를 재사용합니다.
...
<Tabs>
  <Tab title="API 키">
    **권장 대상:** Google AI Studio를 통한 표준 Gemini API 접근.
  ...
  <Tab title="Gemini CLI (OAuth)">
    **권장 대상:** 별도의 API 키 대신 PKCE OAuth를 통해 기존 Gemini CLI 로그인을 재사용.
    ...
    <Steps>
      <Step title="Gemini CLI 설치">
        ...
        OpenClaw는 일반적인 Windows/npm 레이아웃을 포함해 Homebrew 설치와
        전역 npm 설치를 모두 지원합니다.
      </Step>
    ...
    **환경 변수:**
    ...
    <Note>
    브라우저 플로가 시작되기 전에 로그인이 실패하면 로컬 `gemini`
    명령이 설치되어 있고 `PATH`에 있는지 확인하세요.
    </Note>

    `google-gemini-cli/*` 모델 참조는 레거시 호환성 별칭입니다. 새
    구성에서는 로컬 Gemini CLI 실행을 원할 때 `google/*` 모델 참조와
...
운영자 프록시 또는 호환되는 Gemini API 엔드포인트를 위한 것입니다. 생략하면
...
사고를 `off`로 설정하면 `MINIMAL`로 매핑하는 대신 사고 비활성화 상태를 유지합니다.
</Tip>
...
- 모드: 텍스트-비디오, 이미지-비디오, 단일 비디오 참조 플로
...
- 참조 입력: 최대 10개 이미지
- 세션 기반 실행은 `action: "status"`를 포함한 공유 작업/상태 플로를 통해 분리됩니다.
...
TTS 대신 Gemini Live API를 기반으로 하는
...
Gemini API TTS는 스타일 제어에 자연어 프롬프트를 사용합니다. 발화 텍스트 앞에 재사용 가능한 스타일 프롬프트를 붙이려면
`audioProfile`을 설정하세요. 프롬프트 텍스트가 이름 있는 화자를 참조할 때는
`speakerName`을 설정하세요.

Gemini API TTS는 텍스트 안에서
`[whispers]` 또는 `[laughs]` 같은 표현적인 대괄호 오디오 태그도 허용합니다. TTS로 전송하면서
보이는 채팅 답변에는 태그가 나타나지 않도록 하려면 이를 `[[tts:text]]...[[/tts:text]]`
블록 안에 넣으세요.
...
도구 호출은 공유 실시간 음성 계약에 유지합니다. 샘플링 변경이 필요하지 않다면 `temperature`를
설정하지 마세요. Google Live는 `temperature: 0`에서 오디오 없이 대본을 반환할 수 있기 때문에
...
<Note>
Control UI Talk는 제약된 1회용 토큰으로 Google Live 브라우저 세션을 지원합니다.
백엔드 전용 실시간 음성 제공자도 일반 Gateway 릴레이 전송을 통해 실행할 수 있으며,
이 방식은 제공자 자격 증명을 Gateway에 유지합니다.
</Note>

관리자 라이브 검증은
`OPENAI_API_KEY=... GEMINI_API_KEY=... node --import tsx scripts/dev/realtime-talk-live-smoke.ts`를 실행하세요.
이 스모크는 OpenAI 백엔드/WebRTC 경로도 포함합니다. Google 구간은 Control UI Talk에서 사용하는 것과 동일한
제약된 Live API 토큰 형태를 발급하고, 브라우저
WebSocket 엔드포인트를 열고, 초기 설정 페이로드를 전송한 뒤
`setupComplete`를 기다립니다.

## 고급 구성

<AccordionGroup>
  <Accordion title="Gemini 직접 캐시 재사용">
