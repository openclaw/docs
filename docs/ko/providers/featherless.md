...
[Featherless AI](https://featherless.ai)는 OpenAI 호환 API를 통해 오픈 모델을 제공합니다. OpenClaw는 Featherless를 공식 외부 제공자 Plugin으로 설치하며, 내장 카탈로그는 작게 유지하면서 런타임에 Featherless의 정확한 모델 ID를 허용합니다.
...
Featherless가 Qwen 3 제품군의 네이티브 도구 호출을 명시하고 있으므로, Plugin은 `Qwen/Qwen3-32B`를 설정 기본값으로 사용합니다. OpenClaw는 이 모델에 32,768토큰 컨텍스트 창, 보수적인 4,096토큰 출력 제한, Qwen 채팅 템플릿의 사고 제어 기능을 구성합니다.

Featherless는 여러 청구 모드를 지원하고 OpenClaw는 계정별 요금제 또는 요청별 가격을 포함하지 않으므로, 카탈로그 비용 필드는 0입니다.
...
OpenClaw는 의도적으로 Featherless의 전체 공개 모델 인덱스를 선택기에 복사하지 않습니다. 인덱스가 방대하고 모든 텍스트, 비전, 임베딩, 추론 모델을 안전하게 분류하기에 충분한 구조화된 기능 메타데이터를 제공하지 않기 때문입니다. 따라서 알 수 없는 ID에는 보수적인 텍스트 전용 비추론 기본값인 4,096토큰 컨텍스트 창과 1,024토큰 출력 제한이 적용됩니다.
...
사용자 지정 메타데이터를 추가하기 전에 Featherless의 모델 카탈로그에서 현재 모델 가용성과 기능 태그를 확인합니다.

## 문제 해결

- `401` 또는 `403`: `FEATHERLESS_API_KEY`가 Gateway 프로세스에 표시되는지 확인하거나 온보딩을 다시 실행합니다.
- 알 수 없는 모델: `featherless/` 접두사 뒤에 Featherless에서 제공하는 정확한 대소문자 구분 ID를 사용합니다.
- 도구 호출이 텍스트로 반환됨: Qwen 3처럼 Featherless가 네이티브 함수 호출을 지원한다고 명시한 모델 제품군을 선택합니다.
- 관리형 Gateway에서 키를 인식하지 못함: `~/.openclaw/.env` 또는 서비스가 로드하는 다른 환경 소스에 키를 저장한 다음 Gateway를 다시 시작합니다.
