...
title: Cerebras
x-i18n:
    generated_at: "2026-07-12T15:38:29Z"
    model: gpt-5.6
...
[Cerebras](https://www.cerebras.ai)는 맞춤형 추론 하드웨어에서 고속 OpenAI 호환 추론을 제공합니다. 이 Plugin은 정적 모델 4개로 구성된 카탈로그와 함께 제공됩니다(실시간 검색 없음).
...
4개 모델 모두 128k 컨텍스트 창과 최대 8,192개의 출력 토큰을 지원합니다.
...
<Warning>
Cerebras는 `zai-glm-4.7`과 `qwen-3-235b-a22b-instruct-2507`을 프리뷰 모델로 표시하며, `llama3.1-8b`와 `qwen-3-235b-a22b-instruct-2507`은 2026년 5월 27일에 지원 중단될 예정이라고 문서화되어 있습니다. 프로덕션 워크로드에서 이 모델들을 사용하기 전에 Cerebras의 [지원 모델 페이지](https://inference-docs.cerebras.ai/models/overview)를 확인하십시오.
</Warning>

## 수동 구성

대부분의 설정에는 API 키만 필요합니다. 모델 메타데이터를 재정의하거나 정적 카탈로그를 대상으로 `mode: "merge"`로 실행하려면 명시적인 `models.providers.cerebras` 구성을 사용하십시오.
...
<Note>
Gateway가 데몬(launchd, systemd, Docker)으로 실행되는 경우 해당 프로세스에서 `CEREBRAS_API_KEY`를 사용할 수 있는지 확인하십시오. 예를 들어 `~/.openclaw/.env` 또는 `env.shellEnv`를 통해 제공할 수 있습니다. 대화형 셸에서만 내보낸 키는 환경을 별도로 가져오지 않는 한 관리형 서비스에 적용되지 않습니다.
</Note>
...
<CardGroup cols={2}>
  ...
  <Card title="사고 모드" href="/ko/tools/thinking" icon="brain">
    추론을 지원하는 Cerebras 모델 2개의 추론 노력 수준입니다.
  </Card>
