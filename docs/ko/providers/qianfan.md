---
read_when:
    - 여러 LLM에 대해 하나의 API 키를 사용하고 싶은 경우
    - Baidu Qianfan 설정 안내가 필요합니다
summary: OpenClaw에서 Qianfan의 통합 API를 사용해 다양한 모델에 액세스하세요
title: Qianfan
x-i18n:
...
<Tip>
기본 모델 참조는 `qianfan/deepseek-v3.2`입니다. 사용자 지정 기본 URL이나 모델 메타데이터가 필요할 때만 `models.providers.qianfan`을 재정의하면 됩니다.
</Tip>
...
<AccordionGroup>
  <Accordion title="전송 및 호환성">
    Qianfan은 네이티브 OpenAI 요청 형식이 아니라 OpenAI 호환 전송 경로를 통해 실행됩니다. 즉, 표준 OpenAI SDK 기능은 작동하지만 제공자별 매개변수는 전달되지 않을 수 있습니다.
  </Accordion>

  <Accordion title="카탈로그 및 재정의">
    정적 카탈로그에는 현재 `deepseek-v3.2` 및 `ernie-5.0-thinking-preview`가 포함되어 있습니다. 사용자 지정 기본 URL이나 모델 메타데이터가 필요할 때만 `models.providers.qianfan`을 추가하거나 재정의하세요.
  ...
  <Accordion title="문제 해결">
    ...
    - 모델이 나열되지 않으면 계정에서 Qianfan 서비스가 활성화되어 있는지 확인하세요.
