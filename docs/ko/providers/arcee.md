...
[Arcee AI](https://arcee.ai)는 OpenAI 호환 API를 통해 전문가 혼합 모델인 Trinity 제품군을 제공합니다. 모든 Trinity 모델에는 Apache 2.0 라이선스가 적용됩니다. Arcee는 공식 OpenClaw Plugin이며 코어에 번들로 포함되지 않으므로, 온보딩 전에 설치 단계가 필요합니다.
...
<Tabs>
  <Tab title="직접 연결(Arcee 플랫폼)">
    <Steps>
      <Step title="API 키 받기">
        [Arcee AI](https://chat.arcee.ai/)에서 API 키를 생성하십시오.
      </Step>
      <Step title="온보딩 실행">
        ```bash
  ...
  <Tab title="OpenRouter 경유">
    <Steps>
      <Step title="API 키 받기">
        [OpenRouter](https://openrouter.ai/keys)에서 API 키를 생성하십시오.
      </Step>
      <Step title="온보딩 실행">
        ```bash
...
| 모델 참조                      | 이름                   | 입력   | 컨텍스트 | 최대 출력 | 비용(입력/출력, 1M당) | 도구  | 참고                                      |
| ------------------------------ | ---------------------- | ------ | -------- | --------- | --------------------- | ----- | ----------------------------------------- |
| `arcee/trinity-large-thinking` | Trinity Large Thinking | 텍스트 | 256K     | 80K       | $0.25 / $0.90         | 아니요 | 기본 모델, 확장 사고                      |
| `arcee/trinity-large-preview`  | Trinity Large Preview  | 텍스트 | 128K     | 16K       | $0.25 / $1.00         | 예    | 범용, 400B 매개변수 중 13B 활성화         |
| `arcee/trinity-mini`           | Trinity Mini 26B       | 텍스트 | 128K     | 80K       | $0.045 / $0.15        | 예    | 빠르고 비용 효율적, 함수 호출             |

<Tip>
온보딩 사전 설정은 `arcee/trinity-large-thinking`을 기본 모델로 설정합니다.
</Tip>
...
<AccordionGroup>
  <Accordion title="환경 참고 사항">
    ...
    (또는 `OPENROUTER_API_KEY`)를 해당 프로세스에서 사용할 수 있는지 확인하십시오. 예를 들어
    `~/.openclaw/.env` 또는 `env.shellEnv`를 통해 제공할 수 있습니다.
  </Accordion>

  <Accordion title="OpenRouter 라우팅">
    OpenRouter를 통해 Arcee 모델을 사용하는 경우에도 동일한 `arcee/*` 모델 참조가 적용됩니다.
    OpenClaw는 선택한 인증 방식에 따라 투명하게 라우팅합니다. OpenRouter 관련
    구성 세부 정보는 [OpenRouter 제공자 문서](/ko/providers/openrouter)를
    참조하십시오.
  </Accordion>
</AccordionGroup>
...
<CardGroup cols={2}>
  <Card title="OpenRouter" href="/ko/providers/openrouter" icon="shuffle">
    단일 API 키로 Arcee 모델을 비롯한 여러 모델에 액세스할 수 있습니다.
  </Card>
