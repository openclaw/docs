...
read_when:
    ...
    - Voice Call에 Deepgram 스트리밍 음성 변환을 사용하려는 경우
    - 빠르게 참고할 수 있는 Deepgram 구성 예시가 필요합니다
summary: 수신 음성 메모를 위한 Deepgram 전사
title: Deepgram
x-i18n:
    generated_at: "2026-07-12T01:07:23Z"
    model: gpt-5.6
...
수신 오디오/음성 메모 전사와 `plugins.entries.voice-call.config.streaming`을 통한
Voice Call 스트리밍 STT에 이 API를 사용합니다.

일괄 전사는 전체 오디오 파일을 Deepgram에 업로드하고 전사문을
응답 파이프라인에 삽입합니다(`{{Transcript}}` + `[Audio]` 블록).
Voice Call 스트리밍은 실시간 G.711 u-law 프레임을 Deepgram의
WebSocket `listen` 엔드포인트로 전달하고, Deepgram에서 반환되는
부분/최종 전사문을 내보냅니다.
...
<Steps>
  ...
  <Step title="음성 메모 보내기">
    연결된 채널을 통해 오디오 메시지를 보내세요. OpenClaw은 Deepgram을 통해 이를
    전사하고 전사문을 응답 파이프라인에 삽입합니다.
  </Step>
</Steps>
...
`providerOptions.deepgram`은 추가 쿼리 매개변수를 Deepgram `/listen`
요청에 직접 병합하므로, Deepgram에서 지원하는 모든 매개변수 이름을 사용할 수 있습니다
...
번들로 제공되는 `deepgram` Plugin은 Voice Call Plugin용 실시간 전사 제공자도
등록합니다.
...
| 샘플링 레이트   | `...deepgram.sampleRate`                                                | `8000`                           |
| 엔드포인트 감지 | `...deepgram.endpointingMs`                                             | `800`                            |
...
<Note>
Voice Call은 전화 오디오를 8kHz G.711 u-law 형식으로 수신합니다. Deepgram
