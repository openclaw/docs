...
OpenClaw은 이를 `synthetic` 제공자로 번들로 제공하며 Anthropic
Messages API를 사용합니다.
...
<Steps>
  <Step title="API 키 가져오기">
    Synthetic 계정에서 `SYNTHETIC_API_KEY`를 가져오거나, 온보딩 과정에서
...
모든 Synthetic 모델의 비용은 `0`입니다(입력/출력/캐시).
...
모든 모델을 확인하려면 `openclaw models list --provider synthetic`을 사용하세요.
</Tip>

<AccordionGroup>
  <Accordion title="모델 허용 목록">
    모델 허용 목록(`agents.defaults.models`)을 활성화하는 경우 사용하려는
    모든 Synthetic 모델을 추가하세요. 허용 목록에 없는 모델은 에이전트에
    표시되지 않습니다.
  </Accordion>

  <Accordion title="기본 URL 재정의">
    Synthetic이 API 엔드포인트를 변경하면 기본 URL을 재정의하세요.
    ...
    OpenClaw은 계속해서 `/v1`을 자동으로 추가합니다.
