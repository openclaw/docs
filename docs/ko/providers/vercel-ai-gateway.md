...
x-i18n:
    generated_at: "2026-07-12T01:08:58Z"
    model: gpt-5.6
...
<Tip>
OpenClaw은 Gateway의 `/v1/models` 카탈로그를 자동으로 검색하므로
...
OpenClaw은 런타임에 Claude 축약형 모델 참조를 정규화합니다.
...
<Tip>
구성에서 어느 형식이든 사용할 수 있으며, OpenClaw이 표준
...
<AccordionGroup>
  <Accordion title="데몬 프로세스용 환경 변수">
    ...
    `AI_GATEWAY_API_KEY`를 사용할 수 있는지 확인하세요.

    <Warning>
    대화형 셸에서만 내보낸 키는 해당 환경을 명시적으로 가져오지 않는 한
    launchd/systemd 데몬에서 사용할 수 없습니다. Gateway 프로세스가 키를
    읽을 수 있도록 `~/.openclaw/.env` 또는 `env.shellEnv`를 통해 키를
    설정하세요.
    </Warning>
  ...
  <Accordion title="제공자 라우팅">
    ...
    Anthropic을 통해, `vercel-ai-gateway/openai/gpt-5.5`는 OpenAI를 통해,
    `vercel-ai-gateway/moonshotai/kimi-k2.6`은 MoonshotAI를 통해 라우팅됩니다.
    하나의 `AI_GATEWAY_API_KEY`로 모든 업스트림 제공자를 인증합니다.
  </Accordion>
  <Accordion title="사고 수준">
    OpenClaw이 업스트림 모델 접두사를 인식하면 `/think` 옵션은 해당 접두사를
    ...
    `gpt-5.1-codex`까지의 Codex 변형)는 `/think xhigh`를 제공합니다. 그 밖의
    네임스페이스가 지정된 참조는 카탈로그 메타데이터에 더 높은 수준이
    선언되어 있지 않은 한 표준 추론 수준을 유지합니다.
  </Accordion>
</AccordionGroup>
