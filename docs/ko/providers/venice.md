...
title: Venice AI
x-i18n:
    generated_at: "2026-07-12T01:13:15Z"
    model: gpt-5.6
...
로깅 없이 실행되며, Claude, GPT, Gemini, Grok에 대한 익명화 프록시 액세스도 제공합니다.
...
| 모드           | 동작                                                         | 모델                                                        |
| -------------- | ---------------------------------------------------------------- | ------------------------------------------------------------- |
| **비공개**    | 프롬프트와 응답을 저장하거나 기록하지 않습니다. 일시적으로만 유지됩니다.         | Llama, Qwen, DeepSeek, Kimi, MiniMax, Venice Uncensored 등 |
| **익명화** | 전달 전에 메타데이터를 제거한 후 Venice를 통해 프록시됩니다. | Claude, GPT, Gemini, Grok                                     |

<Warning>
익명화 모델은 완전히 비공개가 아닙니다. Venice는 전달 전에 메타데이터를 제거하지만, 기반 제공자(OpenAI, Anthropic, Google, xAI)는 여전히 요청을 처리합니다. 완전한 개인정보 보호가 필요한 경우 비공개 모델을 사용하세요.
</Warning>
  ...
  <Step title="OpenClaw 구성">
    <Tabs>
      <Tab title="대화형(권장)">
        ...
        API 키를 입력하라는 메시지를 표시하거나 기존 `VENICE_API_KEY`를 재사용하고, 사용 가능한 Venice 모델을 나열한 후 기본 모델을 설정합니다.
      </Tab>
      <Tab title="환경 변수">
        ```bash
...
| 사용 사례                 | 모델                             | 이유                                       |
| ------------------------- | ---------------------------------- | ------------------------------------------ |
| 일반 대화(기본값)    | `kimi-k2-5`                        | 강력한 비공개 추론과 비전       |
| 최고의 전반적 품질      | `claude-opus-4-6`                  | 가장 강력한 Venice 익명화 옵션         |
...
<AccordionGroup>
  <Accordion title="비공개 모델(26개) — 완전한 비공개, 로깅 없음">
    | 모델 ID                               | 이름                                 | 컨텍스트 | 참고                      |
    | -------------------------------------- | ------------------------------------- | ------- | --------------------------- |
...
Grok 기반 Venice 모델(`grok-41-fast` 및 유사 모델)에는 네이티브 xAI 제공자와
동일한 도구 스키마 호환성 패치가 적용됩니다. 동일한 업스트림
도구 호출 형식을 공유하기 때문입니다.

## 모델 검색

위의 번들 카탈로그는 매니페스트를 기반으로 하는 초기 목록입니다. 런타임에 OpenClaw는
Venice `/models` API에서 이를 새로 고치며, API에 연결할 수 없으면 초기 목록으로
대체합니다. `/models` 엔드포인트는 공개되어 있어 목록 조회에는 인증이 필요하지 않지만,
추론에는 유효한 API 키가 필요합니다.

## DeepSeek V4 재생 동작

Venice가 `deepseek-v4-pro` 또는 `deepseek-v4-flash`와 같은 DeepSeek V4 모델을
제공하는 경우, Venice가 어시스턴트 메시지에서 필수 `reasoning_content` 재생
필드를 생략하면 OpenClaw가 이를 채우고, 요청 페이로드에서 `thinking`/
`reasoning`/`reasoning_effort`를 제거합니다(Venice는 이러한 모델에서
DeepSeek의 네이티브 `thinking` 제어를 거부합니다). 이 재생 수정은
네이티브 DeepSeek 제공자 자체의 사고 제어와 별개입니다.
...
| ---------------- | ------------------------------------------------- |
| 스트리밍        | 모든 모델                                        |
| 함수 호출 | 대부분의 모델, 위에 표시된 경우 모델별로 비활성화 |
| 비전/이미지    | 위에서 "비전"으로 표시된 모델                      |
| JSON 모드        | `response_format` 사용                             |

## 요금

Venice는 크레딧 기반 시스템을 사용합니다. 익명화 모델 비용은 대략
직접 API 요금에 소액의 Venice 수수료를 더한 수준입니다. 현재 요금은
[venice.ai/pricing](https://venice.ai/pricing)을 참조하세요.
...
<AccordionGroup>
  ...
  <Accordion title="모델을 사용할 수 없음">
    현재 사용 가능한 모델을 확인하려면 `openclaw models list --all --provider venice`를
    실행하세요. Venice에서 모델을 추가하거나 폐기함에 따라 카탈로그가 변경됩니다.
  </Accordion>

  <Accordion title="연결 문제">
    Venice API는 `https://api.venice.ai/api/v1`에 있습니다. 네트워크에서 해당 호스트로의 HTTPS 연결을 허용하는지 확인하세요.
  </Accordion>
</AccordionGroup>
...
<CardGroup cols={2}>
  ...
  <Card title="요금제" href="https://venice.ai/pricing" icon="credit-card">
