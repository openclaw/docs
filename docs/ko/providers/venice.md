...
summary: OpenClaw에서 Venice AI 개인정보 보호 중심 모델 사용하기
title: Venice AI
x-i18n:
...
Venice AI는 검열되지 않은 모델 지원과 익명화 프록시를 통한 주요 독점 모델 접근을 포함하는 **개인정보 보호 중심 AI 추론**을 제공합니다. 모든 추론은 기본적으로 비공개입니다. 즉, 사용자 데이터로 학습하지 않고 로깅하지 않습니다.
...
| 모드           | 설명                                                                                                                       | 모델                                                        |
| -------------- | --------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------- |
| **비공개**    | 완전 비공개입니다. 프롬프트/응답은 **절대 저장되거나 로깅되지 않습니다**. 임시 처리됩니다.                                                       | Llama, Qwen, DeepSeek, Kimi, MiniMax, Venice Uncensored 등 |
| **익명화** | 메타데이터가 제거된 상태로 Venice를 통해 프록시됩니다. 기본 제공자(OpenAI, Anthropic, Google, xAI)는 익명화된 요청을 봅니다. | Claude, GPT, Gemini, Grok                                     |

<Warning>
익명화 모델은 완전한 비공개가 **아닙니다**. Venice는 전달 전에 메타데이터를 제거하지만, 기본 제공자(OpenAI, Anthropic, Google, xAI)가 여전히 요청을 처리합니다. 완전한 개인정보 보호가 필요하면 **비공개** 모델을 선택하세요.
</Warning>

## 기능

- **개인정보 보호 중심**: "비공개"(완전 비공개) 모드와 "익명화"(프록시) 모드 중 선택
- **검열되지 않은 모델**: 콘텐츠 제한이 없는 모델에 접근
...
- **강제 속도 제한 없음**: 극단적인 사용량에는 공정 사용 제한이 적용될 수 있음
...
- **개인정보 보호**: 완전한 비공개 추론에는 "비공개" 모델을 선택하세요.
- **기능**: Venice 프록시를 통해 Claude, GPT, Gemini에 접근하려면 "익명화" 모델을 선택하세요.
...
| **최고의 전반적 품질**   | `claude-opus-4-6`                | 가장 강력한 익명화 Venice 옵션           |
...
`venice/deepseek-v4-flash` 같은 DeepSeek V4 모델을 노출하는 경우, 프록시가 이를
생략하면 OpenClaw는 어시스턴트 메시지에 필요한 DeepSeek V4
`reasoning_content` 재생 자리표시자를 채웁니다. Venice는 DeepSeek의 기본 최상위
`thinking` 제어를 거부하므로, OpenClaw는 이 제공자별 재생 수정을 기본
DeepSeek 제공자의 thinking 제어와 분리해 둡니다.
...
<AccordionGroup>
  <Accordion title="비공개 모델(26개) — 완전 비공개, 로깅 없음">
    ...
    | `venice-uncensored`                    | Venice Uncensored (Dolphin-Mistral) | 32k     | 검열되지 않음, 도구 비활성화 |
...
OpenClaw는 읽기 전용 모델 목록을 위해 매니페스트 기반 Venice 시드 카탈로그를 제공합니다. 런타임 새로고침은 여전히 Venice API에서 모델을 탐색할 수 있으며, API에 연결할 수 없으면 매니페스트 카탈로그로 대체됩니다.

`/models` 엔드포인트는 공개되어 있지만(목록 조회에는 인증 불필요), 추론에는 유효한 API 키가 필요합니다.
...
Venice는 크레딧 기반 시스템을 사용합니다. 현재 요율은 [venice.ai/pricing](https://venice.ai/pricing)에서 확인하세요.

- **비공개 모델**: 일반적으로 더 낮은 비용
- **익명화된 모델**: 직접 API 가격 + 소액의 Venice 수수료와 유사

### Venice(익명화)와 직접 API 비교

| 항목         | Venice(익명화)                    | 직접 API          |
| ------------ | ---------------------------------- | ----------------- |
| **개인정보 보호** | 메타데이터 제거 및 익명화        | 계정이 연결됨     |
