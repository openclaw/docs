...
title: OpenAI
x-i18n:
    generated_at: "2026-05-02T21:12:03Z"
    model: gpt-5.5
    provider: openai
...
OpenClaw는 세 가지 OpenAI 계열 경로를 지원합니다. Codex 동작을 원하는 대부분의 ChatGPT/Codex 구독자는 기본 Codex 앱 서버 런타임을 사용해야 합니다. 모델 접두사는 공급자/모델 이름을 선택하며, 별도의 런타임 설정은 포함된 에이전트 루프를 누가 실행할지 선택합니다.

- **API 키** - 사용량 기반 과금이 적용되는 직접 OpenAI Platform 액세스(`openai/*` 모델)
- **기본 Codex 런타임을 사용하는 Codex 구독** - ChatGPT/Codex 로그인과 Codex 앱 서버 실행(`openai/*` 모델 및 `agents.defaults.agentRuntime.id: "codex"`)
...
공급자, 모델, 런타임, 채널은 별도의 계층입니다. 이러한 레이블이 서로 섞이고 있다면 설정을 변경하기 전에 [에이전트 런타임](/ko/concepts/agent-runtimes)을 읽어 보세요.

## 빠른 선택

| 목표                                                 | 사용                                             | 참고                                                                     |
| ---------------------------------------------------- | ------------------------------------------------ | ------------------------------------------------------------------------ |
| 기본 Codex 런타임을 사용하는 ChatGPT/Codex 구독 | `openai/gpt-5.5` 및 `agentRuntime.id: "codex"` | 대부분의 사용자에게 권장되는 Codex 설정입니다. `openai-codex` 인증으로 로그인하세요. |
| 직접 API 키 과금                               | `openai/gpt-5.5`                                 | `OPENAI_API_KEY`를 설정하거나 OpenAI API 키 온보딩을 실행하세요.                    |
| PI를 통한 ChatGPT/Codex 구독 인증           | `openai-codex/gpt-5.5`                           | 의도적으로 일반 PI 러너를 사용하려는 경우에만 사용하세요.                |
...
| `openai-codex`                     | 공급자 접두사   | 일반 OpenClaw PI 러너를 통과하는 OpenAI Codex OAuth/구독 경로입니다.                      |
| `codex` plugin                     | Plugin           | 기본 Codex 앱 서버 런타임과 `/codex` 채팅 제어를 제공하는 번들 OpenClaw Plugin입니다. |
| `agentRuntime.id: codex`           | 에이전트 런타임     | 포함된 턴에 기본 Codex 앱 서버 하니스를 강제합니다.                                     |
| `/codex ...`                       | 채팅 명령 세트  | 대화에서 Codex 앱 서버 스레드를 바인딩/제어합니다.                                        |
| `runtime: "acp", agentId: "codex"` | ACP 세션 경로 | ACP/acpx를 통해 Codex를 실행하는 명시적 폴백 경로입니다.                                          |

즉, 설정에는 의도적으로 `openai-codex/*`와 `codex` plugin이 모두 포함될 수 있습니다. PI를 통한 Codex OAuth를 원하면서 기본 `/codex` 채팅 제어도 사용 가능하게 하고 싶을 때 유효합니다. `openclaw doctor`는 해당 조합에 대해 경고하므로 의도한 것인지 확인할 수 있으며, 이를 다시 작성하지는 않습니다.

<Note>
GPT-5.5는 직접 OpenAI Platform API 키 액세스와 구독/OAuth 경로 모두에서 사용할 수 있습니다. ChatGPT/Codex 구독과 기본 Codex 실행을 함께 사용하려면 `agentRuntime.id: "codex"`와 함께 `openai/gpt-5.5`를 사용하세요. PI를 통한 Codex OAuth에는 `openai-codex/gpt-5.5`만 사용하고, 직접 `OPENAI_API_KEY` 트래픽에는 Codex 런타임 재정의 없이 `openai/gpt-5.5`를 사용하세요.
</Note>

<Note>
OpenAI plugin을 활성화하거나 `openai-codex/*` 모델을 선택해도 번들 Codex 앱 서버 plugin은 활성화되지 않습니다. OpenClaw는 `agentRuntime.id: "codex"`로 기본 Codex 하니스를 명시적으로 선택하거나 레거시 `codex/*` 모델 참조를 사용할 때만 해당 plugin을 활성화합니다.
번들 `codex` plugin이 활성화되어 있지만 `openai-codex/*`가 여전히 PI를 통해 확인되는 경우, `openclaw doctor`는 경고하고 경로를 변경하지 않습니다.
</Note>
...
| Codex 앱 서버 하니스  | `openai/<model>` 및 `agentRuntime.id: codex`             | 예                                                    |
| 서버 측 웹 검색    | 기본 OpenAI Responses 도구                               | 예, 웹 검색이 활성화되어 있고 공급자가 고정되지 않은 경우 |
...
비대칭 임베딩 레이블이 필요한 OpenAI 호환 엔드포인트의 경우 `memorySearch` 아래에 `queryInputType` 및 `documentInputType`을 설정하세요. OpenClaw는 이를 공급자별 `input_type` 요청 필드로 전달합니다. 쿼리 임베딩은 `queryInputType`을 사용하고, 인덱싱된 메모리 청크와 배치 인덱싱은 `documentInputType`을 사용합니다. 전체 예제는 [메모리 설정 참조](/ko/reference/memory-config#provider-specific-config)를 참조하세요.
...
<Tabs>
  <Tab title="API 키(OpenAI Platform)">
    **가장 적합한 용도:** 직접 API 액세스 및 사용량 기반 과금.
    ...
    | `openai/gpt-5.5`       | `agentRuntime.id: "codex"`           | Codex 앱 서버 하니스    | Codex 앱 서버 |

    <Note>
    `openai/*`는 Codex 앱 서버 하니스를 명시적으로 강제하지 않는 한 직접 OpenAI API 키 경로입니다. 기본 PI 러너를 통한 Codex OAuth에는 `openai-codex/*`를 사용하거나, 기본 Codex 앱 서버 실행에는 `agentRuntime.id: "codex"`와 함께 `openai/gpt-5.5`를 사용하세요.
    </Note>
  ...
  <Tab title="Codex 구독">
    **가장 적합한 용도:** 별도의 API 키 대신 ChatGPT/Codex 구독을 기본 Codex 앱 서버 실행과 함께 사용하는 경우. Codex 클라우드에는 ChatGPT 로그인이 필요합니다.
    ...
    <Steps>
      <Step title="Codex OAuth 실행">
        ...
        헤드리스 또는 콜백에 불리한 설정의 경우, 로컬호스트 브라우저 콜백 대신 ChatGPT 디바이스 코드 흐름으로 로그인하려면 `--device-code`를 추가하세요.

        ```bash
        openclaw models auth login --provider openai-codex --device-code
        ```
      </Step>
      <Step title="기본 Codex 런타임 사용">
        ```bash
      ...
      <Step title="Codex 인증을 사용할 수 있는지 확인">
        ...
        Gateway가 실행 중이면 채팅에서 `/codex status` 또는 `/codex models`를 보내 기본 앱 서버 런타임을 확인하세요.
      </Step>
    </Steps>
    ...
    | 모델 참조 | 런타임 설정 | 경로 | 인증 |
    |-----------|----------------|-------|------|
    | `openai/gpt-5.5` | `agentRuntime.id: "codex"` | 기본 Codex 앱 서버 하니스 | Codex 로그인 또는 선택한 `openai-codex` 프로필 |
    ...
    | `openai-codex/gpt-5.5` | `runtime: "auto"` | plugin이 명시적으로 `openai-codex`를 클레임하지 않는 한 여전히 PI | Codex 로그인 |

    <Note>
    인증/프로필 명령에는 계속 `openai-codex` provider id를 사용하세요.
    `openai-codex/*` 모델 접두사는 Codex OAuth를 위한 명시적 PI 경로이기도 합니다.
    이는 번들 Codex 앱 서버 하네스를 선택하거나 자동으로 활성화하지 않습니다. 일반적인
    구독과 네이티브 런타임 설정을 함께 사용하는 경우 `openai-codex`로 로그인하되
    모델 참조는 `openai/gpt-5.5`로 유지하고 `agentRuntime.id: "codex"`를 설정하세요.
    </Note>
    ...
    대신 일반 PI 실행기에서 Codex OAuth를 유지하려면
    `openai-codex/gpt-5.5`를 사용하고 Codex 런타임 재정의를 생략하세요.
    ...
    기본 PI 하네스는 `Runtime: OpenClaw Pi Default`로 표시됩니다. 번들
    Codex 앱 서버 하네스가 선택되면 `/status`는 `Runtime: OpenAI Codex`를
    표시합니다. 기존 세션은 기록된 하네스 id를 유지하므로 `agentRuntime`을
    변경한 뒤 `/status`에 새 PI/Codex 선택이 반영되게 하려면 `/new` 또는
    `/reset`을 사용하세요.

    ### Doctor 경고

    번들 `codex` Plugin이 활성화되어 있고 `openai-codex/*` 경로가
    선택되어 있으면, `openclaw doctor`는 모델이 여전히 PI를 통해 해석된다고
    경고합니다. 해당 PI 구독 인증 경로가 의도된 경우에만 구성을 그대로
    유지하세요. 네이티브 Codex 앱 서버 실행을 원하면 `openai/<model>`과
    ...
    더 작은 기본 상한은 실제 사용에서 더 나은 지연 시간과 품질 특성을 보입니다. `contextTokens`로 재정의하세요.
    ...
    OpenClaw는 `gpt-5.5`가 존재할 때 업스트림 Codex 카탈로그 메타데이터를
    사용합니다. 계정이 인증되어 있는데 라이브 Codex 검색에서
    `openai-codex/gpt-5.5` 행이 누락되면, OpenClaw는 해당 OAuth 모델 행을
    합성하여 cron, 하위 에이전트, 구성된 기본 모델 실행이 `Unknown model`로
    실패하지 않도록 합니다.
...
3. 로컬 stdio 앱 서버 실행에 한해서만, 앱 서버가 계정을 보고하지 않고 여전히
   OpenAI 인증을 요구할 때 `CODEX_API_KEY`, 그다음 `OPENAI_API_KEY`.

즉, Gateway 프로세스에도 직접 OpenAI 모델이나 임베딩을 위한
`OPENAI_API_KEY`가 있다고 해서 로컬 ChatGPT/Codex 구독 로그인이 대체되지는
않습니다. 환경 API 키 폴백은 로컬 stdio 무계정 경로에만 해당하며 WebSocket
앱 서버 연결로 전송되지 않습니다. 구독형 Codex 프로필이 선택되면 OpenClaw는
생성된 stdio 앱 서버 자식 프로세스에서 `CODEX_API_KEY`와 `OPENAI_API_KEY`도
...
계속 사용할 수 있습니다. 투명 배경 PNG/WebP 출력에는 `openai/gpt-image-1.5`를
사용하세요. 현재 `gpt-image-2` API는 `background: "transparent"`를 거부합니다.

투명 배경 요청의 경우 에이전트는 `model: "openai/gpt-image-1.5"`,
`outputFormat: "png"` 또는 `"webp"`, `background: "transparent"`와 함께
`image_generate`를 호출해야 합니다. 이전 `openai.background` provider 옵션도
계속 허용됩니다. OpenClaw는 기본 `openai/gpt-image-2` 투명 요청을
`gpt-image-1.5`로 재작성하여 공개 OpenAI 및 OpenAI Codex OAuth 경로도
보호합니다. Azure와 사용자 지정 OpenAI 호환 엔드포인트는 구성된 배포/모델 이름을
유지합니다.
...
입력 파일에서 시작할 때는 `openclaw infer image edit`에 동일한
...
해당 요청에 대해 먼저 `OPENAI_API_KEY`를 시도하거나 조용히 API 키로 폴백하지
않습니다. 대신 직접 OpenAI Images API 경로를 원하면 API 키, 사용자 지정 기본 URL
...
이 옵트인이 없으면 비공개/내부 OpenAI 호환 이미지 엔드포인트를 차단된 상태로 유지합니다.
...
OpenClaw는 provider 전반의 GPT-5 계열 실행에 공유 GPT-5 프롬프트 기여를 추가합니다. 이는 모델 id로 적용되므로 `openai-codex/gpt-5.5`, `openai/gpt-5.5`, `openrouter/openai/gpt-5.5`, `opencode/gpt-5.5` 및 기타 호환 GPT-5 참조는 동일한 오버레이를 받습니다. 이전 GPT-4.x 모델에는 적용되지 않습니다.

번들 네이티브 Codex 하네스는 Codex 앱 서버 개발자 지침을 통해 동일한 GPT-5 동작과 Heartbeat 오버레이를 사용하므로, `agentRuntime.id: "codex"`를 통해 강제로 실행되는 `openai/gpt-5.x` 세션은 Codex가 나머지 하네스 프롬프트를 소유하더라도 동일한 후속 처리 및 선제적 Heartbeat 지침을 유지합니다.

GPT-5 기여는 페르소나 지속성, 실행 안전성, 도구 규율, 출력 형태, 완료 확인, 검증을 위한 태그가 지정된 동작 계약을 추가합니다. 채널별 답장 및 무음 메시지 동작은 공유 OpenClaw 시스템 프롬프트와 아웃바운드 전달 정책에 남아 있습니다. GPT-5 지침은 일치하는 모델에 항상 활성화됩니다. 친근한 상호작용 스타일 레이어는 별도이며 구성할 수 있습니다.
...
<Tip>
런타임에서는 값의 대소문자를 구분하지 않으므로 `"Off"`와 `"off"` 모두 친근한 스타일 레이어를 비활성화합니다.
</Tip>
...
<AccordionGroup>
  ...
  <Accordion title="음성 텍스트 변환">
    번들 `openai` Plugin은 OpenClaw의 미디어 이해 트랜스크립션 표면을 통해
    배치 음성 텍스트 변환을 등록합니다.

    - 기본 모델: `gpt-4o-transcribe`
    - 엔드포인트: OpenAI REST `/v1/audio/transcriptions`
    - 입력 경로: multipart 오디오 파일 업로드
    - `tools.media.audio`를 사용하는 인바운드 오디오 트랜스크립션이 있는 모든 OpenClaw 위치에서 지원됩니다.
      여기에는 Discord 음성 채널 세그먼트와 채널 오디오 첨부 파일이 포함됩니다.

    인바운드 오디오 트랜스크립션에 OpenAI를 강제로 사용하려면:
    ...
    언어 및 프롬프트 힌트는 공유 오디오 미디어 config 또는 호출별 트랜스크립션 요청에서 제공될 때 OpenAI로 전달됩니다.

  </Accordion>

  <Accordion title="실시간 트랜스크립션">
    번들 `openai` Plugin은 Voice Call Plugin용 실시간 트랜스크립션을 등록합니다.
    ...
    | VAD 임곗값 | `...openai.vadThreshold` | `0.5` |
    | API 키 | `...openai.apiKey` | `OPENAI_API_KEY`로 폴백 |

    <Note>
    G.711 u-law(`g711_ulaw` / `audio/pcmu`) 오디오와 함께 `wss://api.openai.com/v1/realtime`에 대한 WebSocket 연결을 사용합니다. 이 스트리밍 제공자는 Voice Call의 실시간 트랜스크립션 경로용입니다. 현재 Discord 음성은 짧은 세그먼트를 녹음한 뒤 대신 배치 `tools.media.audio` 트랜스크립션 경로를 사용합니다.
    </Note>
  ...
  <Accordion title="실시간 음성">
    ...
    <Note>
    백엔드 실시간 브리지용 `azureEndpoint` 및 `azureDeployment` config 키를 통해 Azure OpenAI를 지원합니다. 양방향 도구 호출을 지원합니다. G.711 u-law 오디오 형식을 사용합니다.
    </Note>

    <Note>
    Control UI Talk는 Gateway가 발급한 임시 클라이언트 시크릿과 OpenAI Realtime API에 대한 직접 브라우저 WebRTC SDP 교환을 사용해 OpenAI 브라우저 실시간 세션을 사용합니다. Maintainer 실시간 검증은 `OPENAI_API_KEY=... GEMINI_API_KEY=... node --import tsx scripts/dev/realtime-talk-live-smoke.ts`로 사용할 수 있습니다. OpenAI 구간은 Node에서 클라이언트 시크릿을 발급하고, 가짜 마이크 미디어로 브라우저 SDP offer를 생성하고, 이를 OpenAI에 게시한 뒤, 시크릿을 로깅하지 않고 SDP answer를 적용합니다.
    </Note>
...
번들 `openai` 제공자는 기본 URL을 재정의하여 이미지 생성을 위해 Azure OpenAI 리소스를 대상으로 지정할 수 있습니다. 이미지 생성 경로에서 OpenClaw는 `models.providers.openai.baseUrl`의 Azure 호스트 이름을 감지하고 자동으로 Azure의 요청 형태로 전환합니다.

<Note>
실시간 음성은 별도의 구성 경로
(`plugins.entries.voice-call.config.realtime.providers.openai.azureEndpoint`)를 사용하며
`models.providers.openai.baseUrl`의 영향을 받지 않습니다. 해당 Azure 설정은 [음성 및 말하기](#voice-and-speech) 아래의 **실시간 음성** 아코디언을 참조하세요.
</Note>
...
번들 `openai` 제공자를 통한 Azure 이미지 생성을 위해
...
<Note>
`openai` 제공자의 이미지 생성 경로에 대한 Azure 라우팅에는 OpenClaw 2026.4.22 이상이 필요합니다. 이전 버전은 사용자 지정 `openai.baseUrl`을 모두 공개 OpenAI 엔드포인트처럼 처리하며 Azure 이미지 배포에 대해 실패합니다.
</Note>

### API 버전

Azure 이미지 생성 경로에 특정 Azure preview 또는 GA 버전을 고정하려면 `AZURE_OPENAI_API_VERSION`을 설정하세요.
...
Azure OpenAI는 모델을 배포에 바인딩합니다. 번들 `openai` 제공자를 통해 라우팅되는 Azure 이미지 생성 요청의 경우 OpenClaw의 `model` 필드는 공개 OpenAI 모델 ID가 아니라 Azure 포털에서 구성한 **Azure 배포 이름**이어야 합니다.
...
동일한 배포 이름 규칙이 번들 `openai` 제공자를 통해 라우팅되는 이미지 생성 호출에도 적용됩니다.
...
Azure 요청이 검증 오류로 실패하면 Azure 포털에서 특정 배포 및 API 버전이 지원하는
매개변수 세트를 확인하세요.

<Note>
Azure OpenAI는 네이티브 전송 및 호환 동작을 사용하지만 OpenClaw의 숨겨진 attribution 헤더를 받지 않습니다. [고급 구성](#advanced-configuration) 아래의 **네이티브와 OpenAI 호환 라우트** 아코디언을 참조하세요.

Azure의 채팅 또는 Responses 트래픽(이미지 생성을 넘어서는 경우)에는 온보딩 흐름 또는 전용 Azure 제공자 config를 사용하세요. `openai.baseUrl`만으로는 Azure API/auth 형태를 적용하지 않습니다. 별도의 `azure-openai-responses/*` 제공자가 있습니다. 아래 Server-side compaction 아코디언을 참조하세요.
</Note>
...
<AccordionGroup>
  <Accordion title="전송(WebSocket vs SSE)">
    OpenClaw는 `openai/*`와 `openai-codex/*` 모두에 대해 SSE 폴백(`"auto"`)이 있는 WebSocket 우선 방식을 사용합니다.
  ...
  <Accordion title="WebSocket 워밍업">
    OpenClaw는 첫 turn 지연 시간을 줄이기 위해 `openai/*` 및 `openai-codex/*`에 대해 기본적으로 WebSocket 워밍업을 활성화합니다.
  ...
  <Accordion title="빠른 모드">
    OpenClaw는 `openai/*` 및 `openai-codex/*`에 대한 공유 빠른 모드 토글을 노출합니다.
    ...
    <Note>
    세션 재정의가 config보다 우선합니다. Sessions UI에서 세션 재정의를 지우면 세션이 구성된 기본값으로 돌아갑니다.
    </Note>
  ...
  <Accordion title="우선 처리(service_tier)">
    ...
    <Warning>
    `serviceTier`는 네이티브 OpenAI 엔드포인트(`api.openai.com`)와 네이티브 Codex 엔드포인트(`chatgpt.com/backend-api`)에만 전달됩니다. 두 제공자 중 하나를 프록시를 통해 라우팅하면 OpenClaw는 `service_tier`를 그대로 둡니다.
    </Warning>
  ...
  <Accordion title="서버 측 Compaction (Responses API)">
    ...
    - `store: true`를 강제합니다(모델 호환성이 `supportsStore: false`로 설정된 경우 제외)
    - `context_management: [{ type: "compaction", compact_threshold: ... }]`를 주입합니다
    - 기본 `compact_threshold`: `contextWindow`의 70%(사용할 수 없으면 `80000`)

    이는 내장 Pi 하네스 경로와 임베디드 실행에서 사용하는 OpenAI 제공자 훅에 적용됩니다. 네이티브 Codex 앱 서버 하네스는 Codex를 통해 자체 컨텍스트를 관리하며 `agents.defaults.agentRuntime.id`로 별도로 구성됩니다.
    ...
    <Note>
    `responsesServerCompaction`는 `context_management` 주입만 제어합니다. 직접 OpenAI Responses 모델은 호환성이 `supportsStore: false`로 설정되지 않는 한 여전히 `store: true`를 강제합니다.
    </Note>
  ...
  <Accordion title="엄격한 에이전트형 GPT 모드">
    ...
    - 도구 동작이 가능할 때 계획만 있는 턴을 더 이상 성공적인 진행으로 취급하지 않습니다
    - 즉시 실행하도록 유도하며 턴을 다시 시도합니다
    - 규모가 있는 작업에 대해 `update_plan`을 자동으로 활성화합니다
    - 모델이 계속 실행하지 않고 계획만 세우는 경우 명시적인 차단 상태를 표시합니다

    <Note>
    OpenAI 및 Codex GPT-5 계열 실행에만 범위가 지정됩니다. 다른 제공자와 이전 모델 계열은 기본 동작을 유지합니다.
    </Note>

  </Accordion>

  <Accordion title="네이티브 라우트와 OpenAI 호환 라우트">
    OpenClaw는 직접 OpenAI, Codex, Azure OpenAI 엔드포인트를 일반 OpenAI 호환 `/v1` 프록시와 다르게 취급합니다.

    **네이티브 라우트**(`openai/*`, Azure OpenAI):
    - OpenAI `none` 추론 강도를 지원하는 모델에 대해서만 `reasoning: { effort: "none" }`을 유지합니다
    - `reasoning.effort: "none"`을 거부하는 모델이나 프록시에 대해서는 비활성화된 reasoning을 생략합니다
    - 도구 스키마의 기본값을 엄격 모드로 설정합니다
    - 검증된 네이티브 호스트에만 숨겨진 attribution 헤더를 첨부합니다
    - OpenAI 전용 요청 구성(`service_tier`, `store`, reasoning 호환성, 프롬프트 캐시 힌트)을 유지합니다
