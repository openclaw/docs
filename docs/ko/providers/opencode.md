...
title: OpenCode
x-i18n:
    generated_at: "2026-07-12T15:40:39Z"
    model: gpt-5.6
...
올바르게 유지되도록 런타임 제공자 ID를 분리하지만, 온보딩과 문서에서는 이를
...
<Tabs>
  <Tab title="Zen 카탈로그">
    **적합한 용도:** 엄선된 OpenCode 다중 모델 프록시(Claude, GPT, Gemini, GLM,
  ...
  <Tab title="Go 카탈로그">
    **적합한 용도:** OpenCode에서 호스팅하는 Kimi, GLM, MiniMax, Qwen 및 DeepSeek 모델군.
...
현재 전체 목록을 보려면 `openclaw models list --provider opencode`를 실행하십시오.
...
전체 Go 모델 표는 [OpenCode Go](/ko/providers/opencode-go)를 참조하십시오.
...
<AccordionGroup>
  ...
  <Accordion title="API 키 발급">
    OpenCode 계정을 만들고
    [opencode.ai/auth](https://opencode.ai/auth)에서 API 키를 생성하십시오. 결제 및
    카탈로그 사용 가능 여부는 OpenCode 대시보드에서 관리합니다.
  </Accordion>

  <Accordion title="Gemini 재생 동작">
    Gemini 기반 OpenCode 참조는 프록시 Gemini 경로를 계속 사용하므로, OpenClaw는
    네이티브 Gemini 재생 검증 또는 부트스트랩 재작성을 활성화하지 않고 해당
