  ...
  <Tab title="OAuth">
    <Steps>
      <Step title="OAuth 온보딩 흐름 실행">
        ...
        ```
        OpenClaw는 로컬에서 브라우저 흐름을 시작하거나, 원격/헤드리스 호스트에서 URL과 리디렉션 결과 붙여넣기 흐름을 표시합니다. OAuth 토큰은 OpenClaw 인증 프로필을 통해 자동으로 갱신됩니다.
      </Step>
    </Steps>
  </Tab>
  <Tab title="API 키">
...
Chutes 인증을 사용할 수 있으면 OpenClaw는 해당 자격 증명으로 `GET /v1/models`를 요청하고 검색된 모델을 사용하며, 자격 증명별로 5분 동안 캐시합니다. 키가 만료되었거나 승인되지 않은 경우(HTTP 401), OpenClaw는 자격 증명 없이 한 번 다시 시도합니다. 검색 결과에 여전히 행이 없거나, 검색이 실패하거나, 다른 비-2xx 상태가 반환되면 번들 정적 카탈로그로 대체합니다(API 키와 OAuth 검색 모두 동일한 경로를 사용합니다). 시작 시 검색이 실패하면 정적 카탈로그가 자동으로 사용됩니다.

## 기본 별칭

OpenClaw는 Chutes 카탈로그에 편의를 위한 별칭 3개를 등록합니다.
...
번들 대체 카탈로그에는 47개의 모델이 있습니다. 다음은 현재 참조의 대표적인 예입니다.
...
전체 목록을 보려면 `openclaw models list --all --provider chutes`를 실행하십시오.
...
<AccordionGroup>
  <Accordion title="OAuth 재정의">
    선택적 환경 변수를 사용하여 OAuth 흐름을 사용자 지정합니다.

    | 변수 | 용도 |
    | -------- | ------- |
    | `CHUTES_CLIENT_ID` | OAuth 클라이언트 ID(설정되지 않은 경우 입력 요청) |
    | `CHUTES_CLIENT_SECRET` | OAuth 클라이언트 비밀 |
    ...
    리디렉션 앱 요구 사항과 도움말은 [Chutes OAuth 문서](https://chutes.ai/docs/sign-in-with-chutes/overview)를 참조하십시오.

  </Accordion>

  <Accordion title="참고 사항">
    - Chutes 모델은 `chutes/<model-id>` 형식으로 등록됩니다.
    - Chutes는 스트리밍 중에 토큰 사용량을 보고하지 않습니다(`supportsUsageInStreaming: false`). 스트림이 완료되면 사용량 합계가 계속 표시됩니다.
