...
StepFun은 두 개의 공급자 ID를 사용하는 외부 공식 Plugin(`@openclaw/stepfun-provider`)으로 제공됩니다.

- 표준 엔드포인트용 `stepfun`
- Step Plan 엔드포인트용 `stepfun-plan`

<Warning>
표준 및 Step Plan은 엔드포인트와 모델 참조 접두사(`stepfun/...` 및 `stepfun-plan/...`)가 서로 다른 **별도의 공급자**입니다. `.com` 엔드포인트에는 중국 키를 사용하고 `.ai` 엔드포인트에는 글로벌 키를 사용하십시오.
</Warning>
...
한 번의 인증 흐름으로 `stepfun`과 `stepfun-plan` 모두에 리전이 일치하는 프로필을 작성하므로, 온보딩을 한 번 실행하면 두 인터페이스가 함께 검색됩니다.

## 고급 구성

<AccordionGroup>
  <Accordion title="전체 구성: 표준 공급자">
    ```json5
    {
  ...
  <Accordion title="전체 구성: Step Plan 공급자">
    ```json5
    {
      env: { STEPFUN_API_KEY: "your-key" },
  ...
  <Accordion title="참고">
    - `step-3.7-flash`는 OpenClaw를 통해 텍스트 및 이미지 입력을 받습니다. StepFun API는 동영상도 지원하지만, 아직 OpenClaw의 모델 입력 모달리티로 지원되지는 않습니다.
    - Step 3.7은 `low`, `medium`, `high` 추론 노력을 지원합니다. 이 모델에는 비추론 모드가 없으므로 `/think off`는 `low`에 매핑됩니다.
    - `step-3.5-flash-2603`은 현재 `stepfun-plan`에서만 제공됩니다.
    - 모델을 확인하거나 전환하려면 `openclaw models list` 및 `openclaw models set <provider/model>`을 사용하십시오.
...
<CardGroup cols={2}>
  <Card title="모델 공급자" href="/ko/concepts/model-providers" icon="layers">
    모든 공급자, 모델 참조 및 장애 조치 동작에 대한 개요입니다.
  </Card>
  <Card title="구성 참조" href="/ko/gateway/configuration-reference" icon="gear">
    공급자, 모델 및 Plugin의 전체 구성 스키마입니다.
  </Card>
