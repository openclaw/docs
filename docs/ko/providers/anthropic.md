...
title: Anthropic
x-i18n:
    generated_at: "2026-05-10T19:48:12Z"
    model: gpt-5.5
    provider: openai
...
Anthropic이 새 정책을 게시하지 않는 한 OpenClaw는 Claude CLI 재사용과 `claude -p` 사용을
승인된 것으로 취급합니다.

장기간 실행되는 Gateway 호스트의 경우에도 Anthropic API 키가 여전히 가장 명확하고
예측 가능한 프로덕션 경로입니다.
...
<Tabs>
  ...
  <Tab title="Claude CLI">
    **권장 대상:** 별도의 API 키 없이 기존 Claude CLI 로그인을 재사용.
    ...
    기존 `claude-cli/claude-opus-4-7` 모델 참조는 호환성을 위해 계속 작동하지만,
    새 구성에서는 provider/model 선택을 `anthropic/*`로 유지하고 실행 백엔드를
    provider/model 런타임 정책에 넣어야 합니다.
...
<AccordionGroup>
  <Accordion title="빠른 모드">
    ...
    - 명시적인 `serviceTier` 또는 `service_tier` 파라미터가 둘 다 설정된 경우 `/fast`를 오버라이드합니다.
  ...
  <Accordion title="미디어 이해(이미지 및 PDF)">
    ...
    구성된 Anthropic 인증에서 미디어 기능을 자동으로 해석하므로
    추가 구성이 필요하지 않습니다.
  ...
  <Accordion title="1M 컨텍스트 창(베타)">
    ...
    <Warning>
    Anthropic 자격 증명에 장문 컨텍스트 액세스 권한이 필요합니다. 기존 토큰 인증(`sk-ant-oat-*`)은 1M 컨텍스트 요청에서 거부됩니다. OpenClaw는 경고를 기록하고 표준 컨텍스트 창으로 폴백합니다.
    </Warning>
