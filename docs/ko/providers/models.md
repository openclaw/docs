...
read_when:
    - 모델 제공자를 선택하려고 합니다
    - LLM 인증 및 모델 선택을 위한 빠른 설정 예시가 필요합니다
summary: OpenClaw에서 지원하는 모델 제공업체(LLM)
title: 모델 제공업체 빠른 시작
x-i18n:
    generated_at: "2026-07-12T15:36:52Z"
    model: gpt-5.6
...
제공자를 선택하고 인증한 다음, 기본 모델을 `provider/model` 형식으로 설정하십시오.

## 빠른 시작(2단계)

1. 제공자에 인증합니다(일반적으로 `openclaw onboard`를 사용).
2. 기본 모델을 설정합니다.
...
- [BytePlus(글로벌)](/ko/concepts/model-providers#byteplus-international)
...
전체 제공자 카탈로그와 고급 구성은
[제공자 디렉터리](/ko/providers/index) 및 [모델 제공자](/ko/concepts/model-providers)를 참조하십시오.

## 추가 제공자 변형

- `anthropic-vertex` - Vertex 자격 증명을 사용할 수 있을 때 Google Vertex에서 암시적으로 Anthropic을 지원하려면 `@openclaw/anthropic-vertex-provider`를 설치하십시오. 별도의 온보딩 인증 선택지는 없습니다.
- `copilot-proxy` - 로컬 VS Code Copilot Proxy 브리지입니다. `openclaw onboard --auth-choice copilot-proxy`를 사용하십시오.
- `google-gemini-cli` - 비공식 Gemini CLI OAuth 흐름입니다. 로컬에 `gemini`가 설치되어 있어야 합니다(`brew install gemini-cli` 또는 `npm install -g @google/gemini-cli`). 기본 모델은 `google-gemini-cli/gemini-3-flash-preview`입니다. `openclaw onboard --auth-choice google-gemini-cli` 또는 `openclaw models auth login --provider google-gemini-cli --set-default`를 사용하십시오.
