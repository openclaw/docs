...
title: MiniMax
x-i18n:
    generated_at: "2026-05-02T21:11:35Z"
    model: gpt-5.5
    provider: openai
...
<Tabs>
  <Tab title="OAuth (Coding Plan)">
    **적합한 용도:** API 키 없이 OAuth를 통해 MiniMax Coding Plan을 빠르게 설정합니다.
    ...
    <Note>
    OAuth 설정은 `minimax-portal` 공급자 ID를 사용합니다. 모델 참조는 `minimax-portal/MiniMax-M2.7` 형식을 따릅니다.
    </Note>
  ...
  <Tab title="API key">
    **적합한 용도:** Anthropic 호환 API를 사용하는 호스팅 MiniMax.
    ...
    <Warning>
    Anthropic 호환 스트리밍 경로에서 OpenClaw는 사용자가 직접 `thinking`을 명시적으로 설정하지 않는 한 기본적으로 MiniMax thinking을 비활성화합니다. MiniMax의 스트리밍 엔드포인트는 네이티브 Anthropic thinking 블록 대신 OpenAI 스타일 델타 청크로 `reasoning_content`를 내보내며, 암시적으로 활성화된 상태로 두면 내부 추론이 표시 출력에 노출될 수 있습니다.
    </Warning>

    <Note>
    API 키 설정은 `minimax` 공급자 ID를 사용합니다. 모델 참조는 `minimax/MiniMax-M2.7` 형식을 따릅니다.
    </Note>
...
`minimax`와 `minimax-portal` 모두 동일한 `image-01` 모델로
`image_generate`를 등록합니다. API 키 설정은 `MINIMAX_API_KEY`를 사용하고, OAuth 설정은
...
CN 엔드포인트를 통해 라우팅하려면 `MINIMAX_API_HOST=https://api.minimaxi.com`을 설정합니다.
기본 글로벌 엔드포인트는 `https://api.minimax.io`입니다.

온보딩 또는 API 키 설정이 명시적인 `models.providers.minimax`
항목을 작성하면 OpenClaw는 `MiniMax-M2.7`과
`MiniMax-M2.7-highspeed`를 텍스트 전용 채팅 모델로 구체화합니다. 이미지 이해는
...
- 인증 확인 순서는 `messages.tts.providers.minimax.apiKey`, 그다음
  `minimax-portal` OAuth/토큰 인증 프로필, 그다음 토큰 플랜 환경
...
- Feishu 및 Telegram 같은 음성 메모 대상은 MiniMax
  MP3에서 `ffmpeg`를 사용해 48kHz Opus로 트랜스코딩됩니다. Feishu/Lark 파일 API가
  네이티브 오디오 메시지에 `file_type: "opus"`만 허용하기 때문입니다.
...
번들된 MiniMax Plugin은 `minimax`와 `minimax-portal` 모두에 대해 공유
...
<Note>
전체 웹 검색 구성 및 사용법은 [MiniMax 검색](/ko/tools/minimax-search)을 참조하세요.
</Note>
