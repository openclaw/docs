...
빠른 시작, QA 러너, 유닛/통합 제품군, Docker 흐름은
[테스트](/ko/help/testing)를 참조하세요. 이 페이지에서는 **라이브**(네트워크에 접속하는) 테스트
제품군을 다룹니다. 모델 매트릭스, CLI 백엔드, ACP, 미디어 제공자 라이브 테스트와
자격 증명 처리를 포함합니다.

## 라이브: 로컬 프로필 스모크 명령

임시 라이브 확인 전에 `~/.profile`을 소스로 불러와 제공자 키와 로컬 도구
경로가 셸과 일치하도록 하세요.
...
`voicecall smoke`는 `--yes`가 함께 있을 때를 제외하면 드라이런입니다. 실제 알림 전화를 걸려는 의도가 있을 때만 `--yes`를 사용하세요. Twilio, Telnyx, Plivo의 경우 준비 상태 확인에 성공하려면 공개 Webhook URL이 필요합니다. 로컬 전용 loopback/비공개 대체 경로는 설계상 거부됩니다.
...
- “직접 모델”은 주어진 키로 제공자/모델이 응답할 수 있는지를 알려줍니다.
- “Gateway 스모크”는 해당 모델에 대해 전체 Gateway+에이전트 파이프라인(세션, 기록, 도구, 샌드박스 정책 등)이 작동하는지를 알려줍니다.
...
- 목표:
  ...
  - 모델마다 작은 완성을 실행(필요한 경우 대상 회귀 포함)
- 활성화 방법:
  - `pnpm test:live`(또는 Vitest를 직접 호출하는 경우 `OPENCLAW_LIVE_TEST=1`)
- 이 제품군을 실제로 실행하려면 `OPENCLAW_LIVE_MODELS=modern`(또는 `all`, modern의 별칭)을 설정하세요. 그렇지 않으면 `pnpm test:live`가 Gateway 스모크에 집중되도록 건너뜁니다.
- 모델 선택 방법:
  ...
  - modern/all 스윕은 기본적으로 엄선된 고신호 상한을 사용합니다. 전체 최신 스윕에는 `OPENCLAW_LIVE_MAX_MODELS=0`을 설정하고, 더 작은 상한에는 양수를 설정하세요.
  - 전체 스윕은 전체 직접 모델 테스트 제한 시간에 `OPENCLAW_LIVE_TEST_TIMEOUT_MS`를 사용합니다. 기본값: 60분.
  - 직접 모델 프로브는 기본적으로 20개 병렬 처리로 실행됩니다. 재정의하려면 `OPENCLAW_LIVE_MODEL_CONCURRENCY`를 설정하세요.
- 제공자 선택 방법:
  - `OPENCLAW_LIVE_PROVIDERS="google,google-antigravity,google-gemini-cli"`(쉼표 허용 목록)
- 키 출처:
  - 기본값: 프로필 저장소 및 env 대체값
  - **프로필 저장소**만 강제하려면 `OPENCLAW_LIVE_REQUIRE_PROFILE_KEYS=1`을 설정하세요.
- 존재 이유:
  - “제공자 API가 손상됨 / 키가 유효하지 않음”을 “Gateway 에이전트 파이프라인이 손상됨”과 분리합니다.
  - 작고 격리된 회귀를 포함합니다(예: OpenAI Responses/Codex Responses 추론 리플레이 + 도구 호출 흐름).
...
- 목표:
  ...
  - 키가 있는 모델을 반복하고 다음을 검증:
    ...
    - OpenAI 회귀 경로(도구 호출 전용 → 후속)가 계속 작동
...
- 모델 선택 방법:
  ...
  - 또는 `OPENCLAW_LIVE_GATEWAY_MODELS="provider/model"`(또는 쉼표 목록)을 설정해 범위를 좁힙니다.
  - modern/all Gateway 스윕은 기본적으로 엄선된 고신호 상한을 사용합니다. 전체 최신 스윕에는 `OPENCLAW_LIVE_GATEWAY_MAX_MODELS=0`을 설정하고, 더 작은 상한에는 양수를 설정하세요.
...
- 도구 + 이미지 프로브는 이 라이브 테스트에서 항상 켜져 있습니다.
  ...
  - 모델이 이미지 입력 지원을 알리면 이미지 프로브 실행
  - 흐름(상위 수준):
    - 테스트가 “CAT” + 무작위 코드가 포함된 작은 PNG를 생성(`src/gateway/live-image-probe.ts`)
    - `agent`를 통해 `attachments: [{ mimeType: "image/png", content: "<base64>" }]`로 전송
    ...
    - 검증: 응답에 `cat` + 코드가 포함됨(OCR 허용 범위: 사소한 실수 허용)
...
- 재정의(선택 사항):
  ...
  - 실제 이미지 첨부를 보내려면 `OPENCLAW_LIVE_CLI_BACKEND_IMAGE_PROBE=1`을 설정하세요(경로는 프롬프트에 주입됩니다). Docker 레시피는 명시적으로 요청하지 않는 한 기본적으로 이를 끕니다.
  - 프롬프트 주입 대신 이미지 파일 경로를 CLI 인수로 전달하려면 `OPENCLAW_LIVE_CLI_BACKEND_IMAGE_ARG="--image"`를 설정하세요.
  - `IMAGE_ARG`가 설정되어 있을 때 이미지 인수를 전달하는 방식을 제어하려면 `OPENCLAW_LIVE_CLI_BACKEND_IMAGE_MODE="repeat"`(또는 `"list"`)를 설정하세요.
  - 두 번째 턴을 보내고 재개 흐름을 검증하려면 `OPENCLAW_LIVE_CLI_BACKEND_RESUME_PROBE=1`을 설정하세요.
  - 선택한 모델이 전환 대상을 지원할 때 Claude Sonnet -> Opus 동일 세션 연속성 프로브를 사용하려면 `OPENCLAW_LIVE_CLI_BACKEND_MODEL_SWITCH_PROBE=1`을 설정하세요. Docker 레시피는 집계 안정성을 위해 기본적으로 이를 끕니다.
...
이는 Gemini에 응답 생성을 요청하지 않습니다. OpenClaw가 Gemini에 제공하는 것과 동일한 시스템
...
- 소유 확장에서 CLI 스모크 메타데이터를 해석한 다음, 일치하는 Linux CLI 패키지(`@anthropic-ai/claude-code`, `@openai/codex` 또는 `@google/gemini-cli`)를 `OPENCLAW_DOCKER_CLI_TOOLS_DIR`(기본값: `~/.cache/openclaw/docker-cli-tools`)의 캐시된 쓰기 가능 prefix에 설치합니다.
- `pnpm test:docker:live-cli-backend:claude-subscription`에는 `claudeAiOauth.subscriptionType`이 있는 `~/.claude/.credentials.json` 또는 `claude setup-token`의 `CLAUDE_CODE_OAUTH_TOKEN`을 통한 이식 가능한 Claude Code 구독 OAuth가 필요합니다. 먼저 Docker에서 직접 `claude -p`를 증명한 뒤, Anthropic API 키 env var를 보존하지 않고 두 번의 Gateway CLI 백엔드 턴을 실행합니다. 이 구독 레인은 Claude MCP/도구 및 이미지 프로브를 기본적으로 비활성화합니다. 현재 Claude가 서드파티 앱 사용을 일반 구독 플랜 한도 대신 추가 사용량 과금으로 라우팅하기 때문입니다.
- 라이브 CLI 백엔드 스모크는 이제 Claude, Codex, Gemini에 대해 동일한 엔드투엔드 흐름을 실행합니다. 텍스트 턴, 이미지 분류 턴, 이후 Gateway CLI를 통해 검증되는 MCP `cron` 도구 호출입니다.
...
- 목표: 로컬 HTTP CONNECT 프록시를 통해 Apple의 sandbox APNs 엔드포인트로 터널링하고, APNs HTTP/2 검증 요청을 보낸 뒤 Apple의 실제 `403 InvalidProviderToken` 응답이 프록시 경로를 통해 돌아오는지 검증합니다.
- 활성화:
...
- 목표: 실제 ACP 에이전트로 실제 ACP 대화 바인드 흐름을 검증합니다.
  ...
  - 합성 메시지 채널 대화를 그 자리에서 바인드
  - 같은 대화에서 일반 후속 메시지 전송
  - 후속 메시지가 바인드된 ACP 세션 트랜스크립트에 도착하는지 확인
- 활성화:
...
- 참고:
  - 이 레인은 Gateway `chat.send` 표면을 관리자 전용 합성 원본 경로 필드와 함께 사용하므로, 테스트가 외부 전달을 가장하지 않고 메시지 채널 컨텍스트를 붙일 수 있습니다.
  - `OPENCLAW_LIVE_ACP_BIND_AGENT_COMMAND`가 설정되지 않은 경우, 테스트는 선택된 ACP 하네스 에이전트에 대해 내장 `acpx` Plugin의 기본 에이전트 레지스트리를 사용합니다.
  - 외부 ACP 하네스가 바인드/이미지 증거가 통과된 뒤 MCP 호출을 취소할 수 있으므로, 바인드된 세션의 Cron MCP 생성은 기본적으로 최선 노력 방식입니다. 바인드 후 Cron 프로브를 엄격하게 만들려면 `OPENCLAW_LIVE_ACP_BIND_REQUIRE_CRON=1`을 설정하세요.
...
- 기본적으로 ACP 바인드 스모크를 집계 라이브 CLI 에이전트에 대해 순서대로 실행합니다: `claude`, `codex`, 그다음 `gemini`.
...
- `~/.profile`을 소스로 읽고, 일치하는 CLI 인증 자료를 컨테이너에 스테이징한 다음, 요청된 라이브 CLI(`@anthropic-ai/claude-code`, `@openai/codex`, `https://app.factory.ai/cli`를 통한 Factory Droid, `@google/gemini-cli`, 또는 `opencode-ai`)가 없으면 설치합니다. ACP 백엔드 자체는 공식 `acpx` Plugin의 내장 `acpx/runtime` 패키지입니다.
- Droid Docker 변형은 설정을 위해 `~/.factory`를 스테이징하고, `FACTORY_API_KEY`를 전달하며, 로컬 Factory OAuth/키링 인증은 컨테이너로 이식할 수 없으므로 해당 API 키가 필요합니다. ACPX의 기본 `droid exec --output-format acp` 레지스트리 항목을 사용합니다.
- OpenCode Docker 변형은 엄격한 단일 에이전트 회귀 레인입니다. `~/.profile`을 소스로 읽은 뒤 `OPENCLAW_LIVE_ACP_BIND_OPENCODE_MODEL`(기본값 `opencode/kimi-k2.6`)에서 임시 `OPENCODE_CONFIG_CONTENT` 기본 모델을 작성하며, `pnpm test:docker:live-acp-bind:opencode`는 일반적인 바인드 후 건너뛰기를 허용하지 않고 바인드된 어시스턴트 트랜스크립트를 요구합니다.
...
- 목표: 정상 Gateway `agent` 메서드를 통해 Plugin 소유 Codex 하네스를 검증합니다.
  - 번들된 `codex` Plugin 로드
  - `OPENCLAW_AGENT_RUNTIME=codex` 선택
  - Codex 하네스를 강제하여 `openai/gpt-5.5`에 첫 번째 Gateway 에이전트 턴 전송
  - 같은 OpenClaw 세션에 두 번째 턴을 보내고 앱 서버 스레드가 재개될 수 있는지 확인
  - 같은 Gateway 명령 경로를 통해 `/codex status` 및 `/codex models` 실행
  - 선택적으로 Guardian 검토를 거친 권한 상승 셸 프로브 두 개 실행: 승인되어야 하는 무해한 명령 하나와 거부되어 에이전트가 되물어야 하는 가짜 비밀 업로드 하나
...
- 스모크는 `agentRuntime.id: "codex"`를 사용하므로, 깨진 Codex 하네스가 조용히 PI로 폴백하여 통과할 수 없습니다.
- 인증: 로컬 Codex 구독 로그인의 Codex 앱 서버 인증. Docker 스모크는 적용 가능한 경우 Codex가 아닌 프로브를 위해 `OPENAI_API_KEY`도 제공할 수 있으며, 선택적으로 복사된 `~/.codex/auth.json` 및 `~/.codex/config.toml`도 사용할 수 있습니다.
...
- 마운트된 `~/.profile`을 소스로 읽고, `OPENAI_API_KEY`를 전달하며, Codex CLI 인증 파일이 있으면 복사하고, 쓰기 가능한 마운트된 npm 접두사에 `@openai/codex`를 설치한 뒤, 소스 트리를 스테이징하고 Codex 하네스 라이브 테스트만 실행합니다.
...
- Docker는 같은 명시적 Codex 런타임 구성을 사용하므로, 레거시 별칭이나 PI 폴백이 Codex 하네스 회귀를 숨길 수 없습니다.
...
- `google-antigravity/...`는 Antigravity OAuth 브리지(Cloud Code Assist 방식 에이전트 엔드포인트)를 사용합니다.
- `google-gemini-cli/...`는 머신의 로컬 Gemini CLI를 사용합니다(별도 인증 + 도구 특이점).
- Gemini API와 Gemini CLI:
  - API: OpenClaw가 HTTP를 통해 Google의 호스팅 Gemini API를 호출합니다(API 키 / 프로필 인증). 대부분의 사용자가 “Gemini”라고 말할 때 의미하는 방식입니다.
  - CLI: OpenClaw가 로컬 `gemini` 바이너리를 셸로 실행합니다. 자체 인증이 있으며 다르게 동작할 수 있습니다(스트리밍/도구 지원/버전 차이).

## 라이브: 모델 매트릭스(적용 범위)

고정된 “CI 모델 목록”은 없지만(라이브는 선택 사항), 키가 있는 개발 머신에서 정기적으로 다루기를 권장하는 모델은 다음과 같습니다.

### 최신 스모크 세트(도구 호출 + 이미지)

계속 작동해야 한다고 기대하는 “공통 모델” 실행입니다.
...
### 기준: 도구 호출(Read + 선택적 Exec)

제공자 계열별로 최소 하나를 선택하세요.
...
자격 증명/구성이 있으면 라이브 매트릭스에 포함할 수 있는 추가 제공자:
...
라이브 테스트는 CLI와 같은 방식으로 자격 증명을 검색합니다. 실제 영향은 다음과 같습니다:

- CLI가 작동하면 live 테스트도 동일한 키를 찾아야 합니다.
- live 테스트에서 “자격 증명 없음”이라고 나오면 `openclaw models list` / 모델 선택을 디버그하는 것과 같은 방식으로 디버그하세요.

- 에이전트별 인증 프로필: `~/.openclaw/agents/<agentId>/agent/auth-profiles.json`(live 테스트에서 “프로필 키”가 의미하는 항목)
- 구성: `~/.openclaw/openclaw.json`(또는 `OPENCLAW_CONFIG_PATH`)
- 레거시 상태 디렉터리: `~/.openclaw/credentials/`(있으면 스테이징된 live 홈으로 복사되지만, 기본 프로필 키 저장소는 아님)
- live 로컬 실행은 기본적으로 활성 구성, 에이전트별 `auth-profiles.json` 파일, 레거시 `credentials/`, 지원되는 외부 CLI 인증 디렉터리를 임시 테스트 홈으로 복사합니다. 스테이징된 live 홈은 `workspace/`와 `sandboxes/`를 건너뛰며, `agents.*.workspace` / `agentDir` 경로 오버라이드는 제거되어 프로브가 실제 호스트 워크스페이스에 접근하지 않게 합니다.

env 키(예: `~/.profile`에 내보낸 키)에 의존하려면 `source ~/.profile` 후 로컬 테스트를 실행하거나 아래 Docker 실행기를 사용하세요(`~/.profile`을 컨테이너에 마운트할 수 있음).
...
- 범위:
  - 등록된 모든 이미지 생성 공급자 Plugin을 열거합니다
  - 프로브 전에 로그인 셸(`~/.profile`)에서 누락된 공급자 env var를 로드합니다
  - 기본적으로 저장된 인증 프로필보다 live/env API 키를 우선 사용하므로, `auth-profiles.json`의 오래된 테스트 키가 실제 셸 자격 증명을 가리지 않습니다
  - 사용 가능한 인증/프로필/모델이 없는 공급자를 건너뜁니다
  - 구성된 각 공급자를 공유 이미지 생성 런타임을 통해 실행합니다:
    - `<provider>:generate`
...
- 선택적 인증 동작:
  - `OPENCLAW_LIVE_REQUIRE_PROFILE_KEYS=1`로 프로필 저장소 인증을 강제하고 env 전용 오버라이드를 무시합니다

제공된 CLI 경로의 경우 공급자/런타임 live 테스트가 통과한 후 `infer` 스모크를 추가하세요:
...
이는 CLI 인자 파싱, 구성/기본 에이전트 해석, 번들
Plugin 활성화, 공유 이미지 생성 런타임, live 공급자
요청을 다룹니다. Plugin 의존성은 런타임 로드 전에 존재해야 합니다.
...
- 범위:
  ...
  - 프로브 전에 로그인 셸(`~/.profile`)에서 공급자 env var를 로드합니다
  - 기본적으로 저장된 인증 프로필보다 live/env API 키를 우선 사용하므로, `auth-profiles.json`의 오래된 테스트 키가 실제 셸 자격 증명을 가리지 않습니다
  - 사용 가능한 인증/프로필/모델이 없는 공급자를 건너뜁니다
  ...
  - 현재 공유 레인 적용 범위:
    ...
    - `minimax`: `generate`
    - `comfy`: 이 공유 스윕이 아니라 별도 Comfy live 파일
- 선택적 범위 축소:
...
- 범위:
  - 공유 번들 비디오 생성 공급자 경로를 실행합니다
  - 기본값은 릴리스에 안전한 스모크 경로입니다: FAL이 아닌 공급자, 공급자당 텍스트-투-비디오 요청 하나, 1초짜리 바닷가재 프롬프트, `OPENCLAW_LIVE_VIDEO_GENERATION_TIMEOUT_MS`의 공급자별 작업 상한(기본값 `180000`)
  - 공급자 측 큐 지연 시간이 릴리스 시간을 지배할 수 있으므로 기본적으로 FAL을 건너뜁니다. 명시적으로 실행하려면 `--video-providers fal` 또는 `OPENCLAW_LIVE_VIDEO_GENERATION_PROVIDERS="fal"`을 전달하세요
  - 프로브 전에 로그인 셸(`~/.profile`)에서 공급자 env var를 로드합니다
  - 기본적으로 저장된 인증 프로필보다 live/env API 키를 우선 사용하므로, `auth-profiles.json`의 오래된 테스트 키가 실제 셸 자격 증명을 가리지 않습니다
  - 사용 가능한 인증/프로필/모델이 없는 공급자를 건너뜁니다
  ...
  - 사용 가능한 경우 선언된 변환 모드도 실행하려면 `OPENCLAW_LIVE_VIDEO_GENERATION_FULL_MODES=1`을 설정하세요:
    ...
    - 공급자가 `capabilities.videoToVideo.enabled`를 선언하고 선택한 공급자/모델이 공유 스윕에서 버퍼 기반 로컬 비디오 입력을 허용하는 경우 `videoToVideo`
  - 공유 스윕에서 현재 선언되었지만 건너뛰는 `imageToVideo` 공급자:
    - `vydra`: 번들 `veo3`는 텍스트 전용이고 번들 `kling`에는 원격 이미지 URL이 필요하기 때문입니다
  - Vydra 공급자별 적용 범위:
    - `OPENCLAW_LIVE_TEST=1 OPENCLAW_LIVE_VYDRA_VIDEO=1 pnpm test:live -- extensions/vydra/vydra.live.test.ts`
    - 해당 파일은 기본적으로 원격 이미지 URL 픽스처를 사용하는 `kling` 레인과 함께 `veo3` 텍스트-투-비디오를 실행합니다
  - 현재 `videoToVideo` live 적용 범위:
    - 선택한 모델이 `runway/gen4_aleph`인 경우에만 `runway`
  - 공유 스윕에서 현재 선언되었지만 건너뛰는 `videoToVideo` 공급자:
    - `alibaba`, `qwen`, `xai`: 해당 경로는 현재 원격 `http(s)` / MP4 참조 URL이 필요하기 때문입니다
    - `google`: 현재 공유 Gemini/Veo 레인이 로컬 버퍼 기반 입력을 사용하며 해당 경로는 공유 스윕에서 허용되지 않기 때문입니다
    - `openai`: 현재 공유 레인에 조직별 비디오 인페인트/리믹스 접근 보장이 없기 때문입니다
- 선택적 범위 축소:
  - `OPENCLAW_LIVE_VIDEO_GENERATION_PROVIDERS="deepinfra,google,openai,runway"`
  - `OPENCLAW_LIVE_VIDEO_GENERATION_MODELS="google/veo-3.1-fast-generate-preview,openai/sora-2,runway/gen4_aleph"`
  - FAL을 포함해 기본 스윕의 모든 공급자를 포함하려면 `OPENCLAW_LIVE_VIDEO_GENERATION_SKIP_PROVIDERS=""`
  - 공격적인 스모크 실행을 위해 각 공급자 작업 상한을 줄이려면 `OPENCLAW_LIVE_VIDEO_GENERATION_TIMEOUT_MS=60000`
- 선택적 인증 동작:
  - `OPENCLAW_LIVE_REQUIRE_PROFILE_KEYS=1`로 프로필 저장소 인증을 강제하고 env 전용 오버라이드를 무시합니다
...
- 목적:
  - 하나의 저장소 네이티브 엔트리포인트를 통해 공유 이미지, 음악, 비디오 live 제품군을 실행합니다
  - `~/.profile`에서 누락된 공급자 env var를 자동 로드합니다
  - 기본적으로 현재 사용 가능한 인증이 있는 공급자로 각 제품군의 범위를 자동 축소합니다
