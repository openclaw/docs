...
title: '테스트: 라이브 스위트'
x-i18n:
    generated_at: "2026-05-02T20:54:23Z"
    model: gpt-5.5
    provider: openai
...
빠른 시작, QA 러너, 단위/통합 스위트, Docker 플로우는
[테스트](/ko/help/testing)를 참고하세요. 이 페이지는 **라이브**(네트워크를 건드리는) 테스트
스위트인 모델 매트릭스, CLI 백엔드, ACP, 미디어 제공자 라이브 테스트와
자격 증명 처리를 다룹니다.

## 라이브: 로컬 프로필 스모크 명령

임시 라이브 확인 전에 `~/.profile`을 source하여 제공자 키와 로컬 도구
경로가 셸과 일치하도록 하세요.
...
`voicecall smoke`는 `--yes`도 함께 있지 않으면 드라이 런입니다. 실제 알림 전화를 걸려는 의도가
있을 때만 `--yes`를 사용하세요. Twilio, Telnyx, Plivo의 경우 성공적인 준비 상태 확인에는 공개 Webhook URL이 필요하며, 로컬 전용 루프백/비공개 대체 경로는 의도적으로 거부됩니다.
...
- 필수 사전 설정:
  ...
  - 앱을 포그라운드에 유지해야 합니다.
  - 통과할 것으로 예상하는 기능에 대해 권한/캡처 동의가 부여되어 있어야 합니다.
- 선택적 대상 재정의:
...
- “Gateway 스모크”는 해당 모델에 대해 전체 Gateway+에이전트 파이프라인이 동작하는지 알려줍니다(세션, 기록, 도구, 샌드박스 정책 등).
...
- 목표:
  ...
  - 모델별로 작은 완료 실행(필요한 경우 대상 회귀도 실행)
- 활성화 방법:
...
- 프로브 세부 정보(실패를 빠르게 설명할 수 있도록):
  ...
  - `exec+read` 프로브: 테스트가 에이전트에게 임시 파일에 nonce를 `exec`로 쓰고, 다시 `read`하도록 요청합니다.
...
- 도구 + 이미지 프로브는 이 라이브 테스트에서 항상 켜져 있습니다.
  ...
  - 모델이 이미지 입력 지원을 알릴 때 이미지 프로브 실행
  - 플로우(상위 수준):
...
- 재정의(선택 사항):
  ...
  - 실제 이미지 첨부를 보내려면 `OPENCLAW_LIVE_CLI_BACKEND_IMAGE_PROBE=1`을 설정합니다(경로는 프롬프트에 주입됩니다). Docker 레시피는 명시적으로 요청하지 않는 한 기본적으로 이를 끕니다.
  - 프롬프트 주입 대신 이미지 파일 경로를 CLI 인수로 전달하려면 `OPENCLAW_LIVE_CLI_BACKEND_IMAGE_ARG="--image"`를 설정합니다.
  - `IMAGE_ARG`가 설정되었을 때 이미지 인수가 전달되는 방식을 제어하려면 `OPENCLAW_LIVE_CLI_BACKEND_IMAGE_MODE="repeat"`(또는 `"list"`)를 설정합니다.
  - 두 번째 턴을 보내고 재개 플로우를 검증하려면 `OPENCLAW_LIVE_CLI_BACKEND_RESUME_PROBE=1`을 설정합니다.
  - 선택한 모델이 전환 대상을 지원할 때 Claude Sonnet -> Opus 동일 세션 연속성 프로브를 선택적으로 사용하려면 `OPENCLAW_LIVE_CLI_BACKEND_MODEL_SWITCH_PROBE=1`을 설정합니다. Docker 레시피는 집계 신뢰성을 위해 기본적으로 이를 끕니다.
  - MCP/도구 루프백 프로브를 선택적으로 사용하려면 `OPENCLAW_LIVE_CLI_BACKEND_MCP_PROBE=1`을 설정합니다. Docker 레시피는 명시적으로 요청하지 않는 한 기본적으로 이를 끕니다.
...
- 소유 확장에서 CLI 스모크 메타데이터를 해석한 다음, 일치하는 Linux CLI 패키지(`@anthropic-ai/claude-code`, `@openai/codex` 또는 `@google/gemini-cli`)를 `OPENCLAW_DOCKER_CLI_TOOLS_DIR`의 캐시된 쓰기 가능 prefix(기본값: `~/.cache/openclaw/docker-cli-tools`)에 설치합니다.
- `pnpm test:docker:live-cli-backend:claude-subscription`에는 `claudeAiOauth.subscriptionType`이 있는 `~/.claude/.credentials.json` 또는 `claude setup-token`의 `CLAUDE_CODE_OAUTH_TOKEN`을 통한 이식 가능한 Claude Code 구독 OAuth가 필요합니다. 먼저 Docker에서 직접 `claude -p`를 증명한 다음, Anthropic API 키 env vars를 보존하지 않고 두 번의 Gateway CLI 백엔드 턴을 실행합니다. 이 구독 레인은 Claude가 현재 제3자 앱 사용을 일반 구독 플랜 제한 대신 추가 사용량 과금으로 라우팅하기 때문에 기본적으로 Claude MCP/도구 및 이미지 프로브를 비활성화합니다.
- 라이브 CLI 백엔드 스모크는 이제 Claude, Codex, Gemini에 대해 동일한 엔드투엔드 플로우를 실행합니다: 텍스트 턴, 이미지 분류 턴, 그리고 Gateway CLI를 통해 검증되는 MCP `cron` 도구 호출.
- Claude의 기본 스모크도 세션을 Sonnet에서 Opus로 패치하고, 재개된 세션이 이전 메모를 여전히 기억하는지 검증합니다.
...
- 기본값:
  ...
  - 직접 `pnpm test:live ...` 실행용 ACP 에이전트: `claude`
...
- 참고:
  ...
  - `OPENCLAW_LIVE_ACP_BIND_AGENT_COMMAND`가 설정되지 않은 경우, 테스트는 선택된 ACP 하니스 에이전트에 대해 내장 `acpx` Plugin의 기본 제공 에이전트 레지스트리를 사용합니다.
  - 외부 ACP 하니스가 바인딩/이미지 증명 통과 후 MCP 호출을 취소할 수 있으므로, 바인딩된 세션의 Cron MCP 생성은 기본적으로 최선 노력 방식입니다. 바인딩 후 Cron 탐침을 엄격하게 만들려면 `OPENCLAW_LIVE_ACP_BIND_REQUIRE_CRON=1`을 설정하세요.
...
- 기본적으로 집계된 라이브 CLI 에이전트를 순서대로 대상으로 ACP 바인딩 스모크를 실행합니다: `claude`, `codex`, 그다음 `gemini`.
...
- `~/.profile`을 소싱하고, 일치하는 CLI 인증 자료를 컨테이너에 준비한 다음, 없으면 요청된 라이브 CLI(`@anthropic-ai/claude-code`, `@openai/codex`, `https://app.factory.ai/cli`를 통한 Factory Droid, `@google/gemini-cli` 또는 `opencode-ai`)를 설치합니다. ACP backend 자체는 공식 `acpx` Plugin의 내장 `acpx/runtime` 패키지입니다.
- Droid Docker 변형은 설정을 위해 `~/.factory`를 준비하고, `FACTORY_API_KEY`를 전달하며, 로컬 Factory OAuth/keyring 인증은 컨테이너로 이식할 수 없으므로 해당 API 키가 필요합니다. ACPX의 기본 제공 `droid exec --output-format acp` 레지스트리 항목을 사용합니다.
- OpenCode Docker 변형은 엄격한 단일 에이전트 회귀 레인입니다. `~/.profile`을 소싱한 뒤 `OPENCLAW_LIVE_ACP_BIND_OPENCODE_MODEL`(기본값 `opencode/kimi-k2.6`)에서 임시 `OPENCODE_CONFIG_CONTENT` 기본 모델을 작성하며, `pnpm test:docker:live-acp-bind:opencode`는 일반적인 바인딩 후 건너뛰기를 허용하지 않고 바인딩된 assistant transcript를 요구합니다.
- 직접 `acpx` CLI 호출은 Gateway 외부에서 동작을 비교하기 위한 수동/우회 경로일 뿐입니다. Docker ACP 바인딩 스모크는 OpenClaw의 내장 `acpx` 런타임 backend를 실행합니다.
...
- 목표: 일반 Gateway `agent` 메서드를 통해 Plugin 소유 Codex 하니스를 검증합니다.
  ...
  - Codex 하니스를 강제한 상태로 첫 Gateway 에이전트 턴을 `openai/gpt-5.5`에 전송
  - 같은 OpenClaw 세션에 두 번째 턴을 전송하고 app-server 스레드가 재개될 수 있는지 확인
  - 같은 Gateway 명령 경로를 통해 `/codex status` 및 `/codex models` 실행
  - 선택적으로 Guardian 검토를 거친 escalated shell 탐침 두 개 실행: 승인되어야 하는 무해한 명령 하나와, 거부되어 에이전트가 다시 물어야 하는 가짜 secret 업로드 하나
- 테스트: `src/gateway/gateway-codex-harness.live.test.ts`
- 활성화: `OPENCLAW_LIVE_CODEX_HARNESS=1`
- 기본 모델: `openai/gpt-5.5`
- 선택적 이미지 탐침: `OPENCLAW_LIVE_CODEX_HARNESS_IMAGE_PROBE=1`
- 선택적 MCP/도구 탐침: `OPENCLAW_LIVE_CODEX_HARNESS_MCP_PROBE=1`
- 선택적 Guardian 탐침: `OPENCLAW_LIVE_CODEX_HARNESS_GUARDIAN_PROBE=1`
- 스모크는 `OPENCLAW_AGENT_HARNESS_FALLBACK=none`을 설정하므로, 깨진 Codex 하니스가 조용히 PI로 폴백하여 통과할 수 없습니다.
- 인증: 로컬 Codex 구독 로그인에서 제공되는 Codex app-server 인증입니다. Docker 스모크는 해당되는 경우 비 Codex 탐침을 위해 `OPENAI_API_KEY`도 제공할 수 있으며, 선택적으로 복사된 `~/.codex/auth.json` 및 `~/.codex/config.toml`도 사용할 수 있습니다.
...
- 마운트된 `~/.profile`을 소싱하고, `OPENAI_API_KEY`를 전달하며, 존재하는 경우 Codex CLI 인증 파일을 복사하고, `@openai/codex`를 쓰기 가능한 마운트된 npm prefix에 설치하고, 소스 트리를 준비한 뒤 Codex 하니스 라이브 테스트만 실행합니다.
- Docker는 기본적으로 이미지, MCP/도구, Guardian 탐침을 활성화합니다. 더 좁은 디버그 실행이 필요할 때는 `OPENCLAW_LIVE_CODEX_HARNESS_IMAGE_PROBE=0` 또는 `OPENCLAW_LIVE_CODEX_HARNESS_MCP_PROBE=0` 또는 `OPENCLAW_LIVE_CODEX_HARNESS_GUARDIAN_PROBE=0`을 설정하세요.
- Docker는 라이브 테스트 구성과 일치하도록 `OPENCLAW_AGENT_HARNESS_FALLBACK=none`도 export하므로, 레거시 alias 또는 PI 폴백이 Codex 하니스 회귀를 숨길 수 없습니다.

### 권장 라이브 레시피

좁고 명시적인 allowlist가 가장 빠르고 불안정성이 가장 적습니다.
...
- Gemini API와 Gemini CLI:
  - API: OpenClaw는 HTTP를 통해 Google의 호스팅 Gemini API를 호출합니다(API 키 / profile 인증). 대부분의 사용자가 “Gemini”라고 할 때 의미하는 것입니다.
  - CLI: OpenClaw는 로컬 `gemini` 바이너리를 shell로 실행합니다. 자체 인증을 가지며 다르게 동작할 수 있습니다(스트리밍/도구 지원/버전 차이).

## 라이브: 모델 매트릭스(포함 범위)

고정된 “CI 모델 목록”은 없습니다(라이브는 opt-in). 하지만 키가 있는 개발 머신에서 정기적으로 포함하기를 **권장**하는 모델은 다음과 같습니다.

### 최신 스모크 세트(도구 호출 + 이미지)

계속 작동하도록 유지할 것으로 기대하는 “일반 모델” 실행입니다.
...
이미지 탐침을 실행하려면 `OPENCLAW_LIVE_GATEWAY_MODELS`에 이미지 지원 모델을 하나 이상 포함하세요(Claude/Gemini/OpenAI vision 지원 변형 등).
...
- `models.providers`를 통해(사용자 지정 endpoint): `minimax`(cloud/API), 그리고 모든 OpenAI/Anthropic 호환 proxy(LM Studio, vLLM, LiteLLM 등)

<Tip>
문서에 "all models"를 하드코딩하지 마세요. 권위 있는 목록은 사용자의 머신에서 `discoverModels(...)`가 반환하는 것과 사용 가능한 키입니다.
</Tip>

## 자격 증명(절대 커밋하지 마세요)

라이브 테스트는 CLI와 같은 방식으로 자격 증명을 탐색합니다. 실제 영향:

- CLI가 작동하면 라이브 테스트도 같은 키를 찾아야 합니다.
- 라이브 테스트에서 “자격 증명 없음”이라고 나오면 `openclaw models list` / 모델 선택을 디버그할 때와 같은 방식으로 디버그하세요.

- 에이전트별 인증 프로필: `~/.openclaw/agents/<agentId>/agent/auth-profiles.json` (라이브 테스트에서 “프로필 키”가 의미하는 것이 이것입니다)
...
- 라이브 로컬 실행은 기본적으로 활성 구성, 에이전트별 `auth-profiles.json` 파일, 레거시 `credentials/`, 지원되는 외부 CLI 인증 디렉터리를 임시 테스트 홈으로 복사합니다. 스테이징된 라이브 홈은 `workspace/`와 `sandboxes/`를 건너뛰며, 프로브가 실제 호스트 워크스페이스에 접근하지 않도록 `agents.*.workspace` / `agentDir` 경로 재정의가 제거됩니다.

환경 키(예: `~/.profile`에 내보낸 키)에 의존하려면 `source ~/.profile` 이후 로컬 테스트를 실행하거나, 아래 Docker 러너를 사용하세요(컨테이너 안에 `~/.profile`을 마운트할 수 있습니다).
...
- 범위:
  ...
  - 현재 공유 레인 범위:
    ...
    - `minimax`: `generate`
    - `comfy`: 별도의 Comfy 라이브 파일이며, 이 공유 스윕은 아닙니다.
- 선택적 축소:
...
- 범위:
  ...
  - 릴리스에 안전한 스모크 경로가 기본값입니다. FAL이 아닌 제공자, 제공자당 텍스트-비디오 요청 1개, 1초짜리 랍스터 프롬프트, 그리고 `OPENCLAW_LIVE_VIDEO_GENERATION_TIMEOUT_MS`의 제공자별 작업 상한(기본값 `180000`)을 사용합니다.
  ...
  - 공유 스윕에서 현재 선언되었지만 건너뛰는 `videoToVideo` 제공자:
    - `alibaba`, `qwen`, `xai`: 해당 경로들은 현재 원격 `http(s)` / MP4 참조 URL이 필요하기 때문입니다.
    - `google`: 현재 공유 Gemini/Veo 레인이 로컬 버퍼 기반 입력을 사용하며, 그 경로는 공유 스윕에서 허용되지 않기 때문입니다.
...
- 목적:
  ...
  - 기본적으로 현재 사용할 수 있는 인증이 있는 제공자로 각 제품군을 자동 축소합니다.
...
- [테스트](/ko/help/testing) — 유닛, 통합, QA 및 Docker 제품군
