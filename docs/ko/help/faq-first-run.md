...
read_when:
    ...
    - 인증 및 제공자 구독 선택
    - docs.openclaw.ai에 접속할 수 없음, 대시보드를 열 수 없음, 설치 중 멈춤
...
---
  ...
  <Accordion title="막혔을 때 가장 빠르게 해결하는 방법">
    **사용자 머신을 볼 수 있는** 로컬 AI 에이전트를 사용하세요. 대부분의 "막혔어요" 사례는 원격 도우미가 검사할 수 없는 **로컬 설정 또는 환경 문제**이기 때문에
    Discord에서 묻는 것보다 훨씬 효과적입니다.

    - **Claude Code**: [https://www.anthropic.com/claude-code/](https://www.anthropic.com/claude-code/)
    - **OpenAI Codex**: [https://openai.com/codex/](https://openai.com/codex/)

    이러한 도구는 리포지토리를 읽고, 명령을 실행하고, 로그를 검사하고, 머신 수준의
    설정(PATH, 서비스, 권한, 인증 파일)을 고치는 데 도움을 줄 수 있습니다. 해킹 가능한(git) 설치 방식으로
    **전체 소스 체크아웃**을 제공하세요.
    ...
    실행 중인 정확한 버전에 대해 판단할 수 있습니다. 나중에 `--install-method git` 없이 설치 프로그램을 다시 실행하면 언제든지 안정 버전으로 돌아갈 수 있습니다.

    팁: 에이전트에게 수정 과정을 **계획하고 감독**하게 한 뒤(단계별), 필요한 명령만 실행하도록 요청하세요. 이렇게 하면 변경 사항이 작고 감사하기 쉬워집니다.

    실제 버그나 수정 사항을 발견했다면 GitHub 이슈를 만들거나 PR을 보내 주세요.
  ...
  <Accordion title="Heartbeat가 계속 건너뜁니다. 건너뛰기 이유는 무엇을 의미하나요?">
    ...
    - `empty-heartbeat-file`: `HEARTBEAT.md`가 있지만 빈 내용/헤더만 있는 골격만 포함합니다.
    - `no-tasks-due`: `HEARTBEAT.md` 작업 모드가 활성화되어 있지만 아직 기한이 된 작업 간격이 없습니다.
    - `alerts-disabled`: 모든 Heartbeat 표시가 비활성화되어 있습니다(`showOk`, `showAlerts`, `useIndicator`가 모두 꺼짐).

    작업 모드에서는 실제 Heartbeat 실행이
    완료된 뒤에만 기한 타임스탬프가 갱신됩니다. 건너뛴 실행은 작업을 완료로 표시하지 않습니다.
  ...
  <Accordion title="OpenClaw 설치 및 설정 권장 방법">
    리포지토리는 소스에서 실행하고 온보딩을 사용하는 방식을 권장합니다.
  ...
  <Accordion title="온보딩 후 대시보드는 어떻게 여나요?">
    마법사는 온보딩 직후 깔끔한(토큰화되지 않은) 대시보드 URL로 브라우저를 열고, 요약에도 링크를 출력합니다. 해당 탭을 열어 두세요. 실행되지 않았다면 같은 머신에서 출력된 URL을 복사/붙여넣기 하세요.
  </Accordion>

  <Accordion title="localhost와 원격에서 대시보드는 어떻게 인증하나요?">
    **Localhost(같은 머신):**

    - `http://127.0.0.1:18789/`를 엽니다.
    - 공유 비밀 인증을 요구하면 설정된 토큰 또는 비밀번호를 Control UI 설정에 붙여넣습니다.
    - 토큰 출처: `gateway.auth.token`(또는 `OPENCLAW_GATEWAY_TOKEN`).
    - 비밀번호 출처: `gateway.auth.password`(또는 `OPENCLAW_GATEWAY_PASSWORD`).
    - 아직 공유 비밀이 설정되어 있지 않다면 `openclaw doctor --generate-gateway-token`으로 토큰을 생성합니다.

    **localhost가 아닌 경우:**

    - **Tailscale Serve**(권장): 바인드는 loopback으로 유지하고, `openclaw gateway --tailscale serve`를 실행한 뒤 `https://<magicdns>/`를 엽니다. `gateway.auth.allowTailscale`이 `true`이면 ID 헤더가 Control UI/WebSocket 인증을 충족합니다(붙여넣은 공유 비밀 없음, 신뢰할 수 있는 Gateway 호스트라고 가정). HTTP API는 private-ingress `none` 또는 trusted-proxy HTTP 인증을 의도적으로 사용하지 않는 한 여전히 공유 비밀 인증이 필요합니다.
      같은 클라이언트의 잘못된 동시 Serve 인증 시도는 실패 인증 제한기가 기록하기 전에 직렬화되므로, 두 번째 잘못된 재시도가 이미 `retry later`를 표시할 수 있습니다.
    - **Tailnet 바인드**: `openclaw gateway --bind tailnet --token "<token>"`을 실행하거나 비밀번호 인증을 설정하고, `http://<tailscale-ip>:18789/`를 연 다음 일치하는 공유 비밀을 대시보드 설정에 붙여넣습니다.
    - **ID 인식 리버스 프록시**: Gateway를 신뢰할 수 있는 프록시 뒤에 두고, `gateway.auth.mode: "trusted-proxy"`를 설정한 뒤 프록시 URL을 엽니다. 같은 호스트의 loopback 프록시는 명시적으로 `gateway.auth.trustedProxy.allowLoopback = true`가 필요합니다.
    - **SSH 터널**: `ssh -N -L 18789:127.0.0.1:18789 user@host`를 실행한 뒤 `http://127.0.0.1:18789/`를 엽니다. 터널을 통해서도 공유 비밀 인증이 적용됩니다. 요청되면 설정된 토큰 또는 비밀번호를 붙여넣으세요.
  ...
  <Accordion title="채팅 승인에 exec 승인 설정이 두 개인 이유는 무엇인가요?">
    ...
    - 채팅이 이미 명령과 답장을 지원한다면, 같은 채팅의 `/approve`가 공유 경로를 통해 동작합니다.
    ...
    - 네이티브 승인 카드/버튼을 사용할 수 있으면 해당 네이티브 UI가 기본 경로입니다. 에이전트는 도구 결과가 채팅 승인을 사용할 수 없거나 수동 승인만 가능한 경로라고 말할 때만 수동 `/approve` 명령을 포함해야 합니다.
    - 프롬프트를 다른 채팅이나 명시적 운영 방에도 전달해야 할 때만 `approvals.exec`를 사용하세요.
    - 승인 프롬프트를 원래 방/주제로 다시 게시하기를 명시적으로 원할 때만 `channels.<channel>.execApprovals.target: "channel"` 또는 `"both"`를 사용하세요.
    - Plugin 승인은 또 별개입니다. 기본적으로 같은 채팅의 `/approve`를 사용하고, 선택적으로 `approvals.plugin` 전달을 사용하며, 일부 네이티브 채널만 그 위에 plugin-approval-native 처리를 유지합니다.
  ...
  <Accordion title="Raspberry Pi에서 실행되나요?">
    ...
    디스크면 충분하다고 되어 있으며, **Raspberry Pi 4에서 실행할 수 있다**고 설명합니다.

    여유 공간(로그, 미디어, 기타 서비스)을 더 원한다면 **2GB를 권장**하지만,
    엄격한 최소 요구 사항은 아닙니다.

    팁: 작은 Pi/VPS가 Gateway를 호스팅할 수 있고, 노트북/휴대폰의 **노드**를 페어링해
    로컬 화면/카메라/캔버스 또는 명령 실행을 사용할 수 있습니다. [노드](/ko/nodes)를 참조하세요.
  ...
  <Accordion title="wake up my friend에서 멈췄거나 온보딩이 부화하지 않습니다. 이제 어떻게 하나요?">
    해당 화면은 Gateway에 연결할 수 있고 인증되어 있어야 동작합니다. TUI도 첫 부화 시
    "Wake up, my friend!"를 자동으로 보냅니다. 해당 줄이 **응답 없이** 표시되고
    ...
    Gateway가 원격이라면 터널/Tailscale 연결이 활성화되어 있고 UI가 올바른 Gateway를
    가리키는지 확인하세요. [원격 액세스](/ko/gateway/remote)를 참조하세요.

  </Accordion>

  <Accordion title="온보딩을 다시 하지 않고 설정을 새 머신(Mac mini)으로 마이그레이션할 수 있나요?">
    예. **상태 디렉터리**와 **작업 공간**을 복사한 다음 Doctor를 한 번 실행하세요. **두** 위치를 모두 복사하는 한
    ...
    Gateway 호스트가 세션 저장소와 작업 공간을 소유한다는 점을 기억하세요.

    **중요:** 작업 공간만 GitHub에 커밋/푸시하면
    **메모리 + bootstrap 파일**은 백업되지만, 세션 기록이나 인증은 백업되지 **않습니다**. 이러한 항목은
    `~/.openclaw/` 아래에 있습니다(예: `~/.openclaw/agents/<agentId>/sessions/`).

    관련: [마이그레이션](/ko/install/migrating), [디스크에서 항목이 저장되는 위치](/ko/help/faq#where-things-live-on-disk),
    [에이전트 작업 공간](/ko/concepts/agent-workspace), [Doctor](/ko/gateway/doctor),
  ...
  <Accordion title="최신 버전의 새로운 내용은 어디에서 확인하나요?">
    ...
    최신 항목은 맨 위에 있습니다. 맨 위 섹션이 **Unreleased**로 표시되어 있다면, 그다음 날짜가 있는
    섹션이 최신 출시 버전입니다. 항목은 **하이라이트**, **변경 사항**, **수정 사항**별로 그룹화됩니다(필요한 경우 문서/기타 섹션도 포함).
  ...
  <Accordion title="docs.openclaw.ai에 액세스할 수 없습니다(SSL 오류)">
    ...
    차단 해제를 위해 여기에서 신고해 주세요: [https://spa.xfinity.com/check_url_status](https://spa.xfinity.com/check_url_status).
  ...
  <Accordion title="안정 버전과 베타의 차이">
    **안정 버전**과 **베타**는 별도의 코드 라인이 아니라 **npm dist-tags**입니다:

    - `latest` = 안정 버전
    - `beta` = 테스트용 초기 빌드

    일반적으로 안정 릴리스는 먼저 **베타**에 올라간 다음, 명시적인
    승격 단계에서 같은 버전을 `latest`로 이동합니다. 유지관리자는 필요할 때
    바로 `latest`에 게시할 수도 있습니다. 그래서 승격 후에는 베타와 안정 버전이
    **같은 버전**을 가리킬 수 있습니다.

    변경된 내용 보기:
    [https://github.com/openclaw/openclaw/blob/main/CHANGELOG.md](https://github.com/openclaw/openclaw/blob/main/CHANGELOG.md)

    설치 한 줄 명령과 베타와 Dev의 차이는 아래 아코디언을 참고하세요.

  </Accordion>

  <Accordion title="베타 버전은 어떻게 설치하고, 베타와 Dev의 차이는 무엇인가요?">
    **베타**는 npm dist-tag `beta`입니다(승격 후에는 `latest`와 일치할 수 있음).
    **Dev**는 `main`의 이동하는 최신 상태(git)입니다. 게시될 때는 npm dist-tag `dev`를 사용합니다.
  ...
  <Accordion title="최신 비트를 어떻게 사용해 볼 수 있나요?">
    ...
    그러면 편집할 수 있는 로컬 repo가 생기며, 이후 git으로 업데이트할 수 있습니다.

    수동으로 깨끗하게 clone하려면 다음을 사용하세요:
  ...
  <Accordion title="문서가 제 질문에 답하지 못했습니다. 더 나은 답변을 얻으려면 어떻게 하나요?">
    ...
    bot(또는 Claude/Codex)에게 _그 폴더에서_ 물어보면 repo를 읽고 정확히 답할 수 있습니다.
  ...
  <Accordion title="클라우드/VPS 설치 가이드는 어디에 있나요?">
    일반적인 제공업체를 모은 **호스팅 허브**를 유지합니다. 하나를 선택하고 가이드를 따르세요:
    ...
    클라우드에서 작동하는 방식: **Gateway는 서버에서 실행**되며, 노트북/휴대폰에서
    ...
    **노드**(Mac/iOS/Android/headless)를 해당 클라우드 Gateway에 페어링하여
    Gateway는 클라우드에 유지하면서 로컬 화면/카메라/canvas에 접근하거나
  ...
  <Accordion title="OpenClaw에게 자체 업데이트를 요청할 수 있나요?">
    ...
    확인을 요청할 수 있습니다. 더 안전한 방법: 운영자로서 셸에서 업데이트를 실행하세요.
  ...
  <Accordion title="이것을 실행하려면 Claude 또는 OpenAI 구독이 필요한가요?">
    ...
    **로컬 전용 모델**로 OpenClaw를 실행하여 데이터를 기기에 유지할 수 있습니다. 구독(Claude
    ...
    - **OpenClaw의 Claude CLI / Claude 구독 인증**: Anthropic 직원이
      이 사용이 다시 허용된다고 알려주었으며, Anthropic이 새 정책을 게시하지 않는 한
      OpenClaw는 이 통합에서 `claude -p` 사용을 승인된 것으로 취급합니다
  ...
  <Accordion title="API 키 없이 Claude Max 구독을 사용할 수 있나요?">
    ...
    Anthropic이 새 정책을 게시하지 않는 한 OpenClaw는 Claude 구독 인증과
    `claude -p` 사용을 이 통합에서 승인된 것으로 취급합니다. 가장 예측 가능한
  ...
  <Accordion title="Claude 구독 인증(Claude Pro 또는 Max)을 지원하나요?">
    ...
    Anthropic 직원이 이 사용이 다시 허용된다고 알려주었으므로, OpenClaw는
    Anthropic이 새 정책을 게시하지 않는 한 Claude CLI 재사용과 `claude -p`
    사용을 이 통합에서 승인된 것으로 취급합니다.
...
<AccordionGroup>
  <Accordion title="Anthropic에서 HTTP 429 rate_limit_error가 표시되는 이유는 무엇인가요?">
    이는 현재 시간 창에서 **Anthropic quota/rate limit**이 소진되었다는 뜻입니다. **Claude CLI**를
    ...
    Extra Usage가 활성화된 OpenClaw Claude 로그인 경로)에 적합한 경우에만 작동합니다.

    팁: OpenClaw가 공급자의 속도 제한 중에도 계속 응답할 수 있도록 **대체 모델**을 설정하세요.
    [모델](/ko/cli/models), [OAuth](/ko/concepts/oauth) 및
  ...
  <Accordion title="AWS Bedrock이 지원되나요?">
    예. OpenClaw에는 번들로 제공되는 **Amazon Bedrock (Converse)** 공급자가 있습니다. AWS env 마커가 있으면 OpenClaw가 스트리밍/텍스트 Bedrock 카탈로그를 자동으로 발견하고 암시적 `amazon-bedrock` 공급자로 병합할 수 있습니다. 그렇지 않으면 `plugins.entries.amazon-bedrock.config.discovery.enabled`를 명시적으로 활성화하거나 수동 공급자 항목을 추가할 수 있습니다. [Amazon Bedrock](/ko/providers/bedrock) 및 [모델 공급자](/ko/providers/models)를 참조하세요. 관리형 키 흐름을 선호한다면 Bedrock 앞에 OpenAI 호환 프록시를 두는 것도 여전히 유효한 옵션입니다.
  </Accordion>
  ...
  <Accordion title="Codex 인증은 어떻게 작동하나요?">
    ...
    ChatGPT/Codex 구독 인증과 네이티브 Codex 앱 서버 실행입니다.
    기본 PI 러너를 통해 Codex OAuth를 사용하려는 경우에만
    `openai-codex/gpt-5.5`를 사용하세요. 직접 OpenAI API 키 접근에는
    Codex 런타임 오버라이드 없이 `openai/gpt-5.5`를 사용하세요.
    [모델 공급자](/ko/concepts/model-providers) 및 [온보딩(CLI)](/ko/start/wizard)을 참조하세요.
  </Accordion>

  <Accordion title="OpenClaw가 왜 아직 openai-codex를 언급하나요?">
    `openai-codex`는 ChatGPT/Codex OAuth의 공급자 및 auth-profile id입니다.
    ...
    - Codex 런타임 오버라이드 없는 `openai/gpt-5.5` = PI의 직접 OpenAI API 키 경로
    ...
    런타임의 경우 모델 참조는 `openai/gpt-5.5`로 유지하고
  ...
  <Accordion title="Codex OAuth 한도가 ChatGPT 웹과 다를 수 있는 이유는 무엇인가요?">
    ...
    둘 다 같은 계정에 연결되어 있더라도 이 한도가 ChatGPT 웹사이트/앱 경험과
    다를 수 있습니다.

    OpenClaw는 `openclaw models status`에서 현재 표시되는 공급자 사용량/할당량 창을
    보여줄 수 있지만, ChatGPT 웹 권한을 직접 API 접근으로
    만들어내거나 정규화하지는 않습니다. 직접 OpenAI Platform
  ...
  <Accordion title="OpenAI 구독 인증(Codex OAuth)을 지원하나요?">
    ...
    명시적으로 허용합니다. 온보딩이 OAuth 흐름을 대신 실행할 수 있습니다.

    [OAuth](/ko/concepts/oauth), [모델 공급자](/ko/concepts/model-providers) 및 [온보딩(CLI)](/ko/start/wizard)을 참조하세요.
  ...
  <Accordion title="캐주얼 채팅에 로컬 모델을 사용해도 괜찮나요?">
    보통은 아닙니다. OpenClaw에는 큰 컨텍스트와 강력한 안전성이 필요합니다. 작은 카드는 잘리고 유출됩니다. 꼭 필요하다면 로컬에서 실행할 수 있는 **가장 큰** 모델 빌드(LM Studio)를 실행하고 [/gateway/local-models](/ko/gateway/local-models)를 참조하세요. 더 작거나 양자화된 모델은 프롬프트 인젝션 위험을 높입니다. [보안](/ko/gateway/security)을 참조하세요.
  </Accordion>

  <Accordion title="호스팅 모델 트래픽을 특정 리전에 유지하려면 어떻게 해야 하나요?">
    리전에 고정된 엔드포인트를 선택하세요. OpenRouter는 MiniMax, Kimi, GLM의 미국 호스팅 옵션을 제공합니다. 데이터를 리전 내에 유지하려면 미국 호스팅 변형을 선택하세요. 선택한 리전 공급자를 존중하면서 대체 모델을 계속 사용할 수 있도록 `models.mode: "merge"`를 사용해 Anthropic/OpenAI를 함께 나열할 수도 있습니다.
  </Accordion>

  <Accordion title="이것을 설치하려면 Mac Mini를 사야 하나요?">
    아니요. OpenClaw는 macOS 또는 Linux(WSL2를 통한 Windows)에서 실행됩니다. Mac mini는 선택 사항입니다. 항상 켜져 있는 호스트로 하나를 구매하는 사람도 있지만 작은 VPS, 홈 서버 또는 Raspberry Pi급 장비도 작동합니다.

    Mac은 **macOS 전용 도구**에만 필요합니다. iMessage의 경우 [BlueBubbles](/ko/channels/bluebubbles)를 사용하세요(권장). BlueBubbles 서버는 어떤 Mac에서나 실행되고, Gateway는 Linux나 다른 곳에서 실행할 수 있습니다. 다른 macOS 전용 도구를 원한다면 Mac에서 Gateway를 실행하거나 macOS Node를 페어링하세요.
  ...
  <Accordion title="iMessage 지원에 Mac mini가 필요한가요?">
    ...
    - Linux/VPS에서 Gateway를 실행하고, Messages에 로그인된 아무 Mac에서나 BlueBubbles 서버를 실행합니다.
  ...
  <Accordion title="OpenClaw를 실행하기 위해 Mac mini를 사면 MacBook Pro에 연결할 수 있나요?">
    ...
    **Node**(동반 기기)로 연결할 수 있습니다. Nodes는 Gateway를 실행하지 않습니다. 대신 해당 기기에서
    화면/카메라/캔버스 및 `system.run` 같은 추가 기능을 제공합니다.

    일반적인 패턴:

    - Mac mini에서 Gateway 실행(항상 켜짐).
    - MacBook Pro가 macOS 앱 또는 Node 호스트를 실행하고 Gateway에 페어링됩니다.
    - 이를 보려면 `openclaw nodes status` / `openclaw nodes list`를 사용하세요.
  ...
  <Accordion title="Telegram: allowFrom에는 무엇이 들어가나요?">
    ...
    설정은 숫자 사용자 ID만 요청합니다. 구성에 기존 `@username` 항목이 이미 있으면 `openclaw doctor --fix`가 이를 해결하려고 시도할 수 있습니다.
  ...
  <Accordion title="여러 사람이 서로 다른 OpenClaw 인스턴스에서 하나의 WhatsApp 번호를 사용할 수 있나요?">
    예, **멀티 에이전트 라우팅**을 통해 가능합니다. 각 발신자의 WhatsApp **DM**(peer `kind: "direct"`, 발신자 E.164 예: `+15551234567`)을 서로 다른 `agentId`에 바인딩하면, 각 사람이 자신의 워크스페이스와 세션 저장소를 갖게 됩니다. 답장은 여전히 **같은 WhatsApp 계정**에서 나가며, DM 접근 제어(`channels.whatsapp.dmPolicy` / `channels.whatsapp.allowFrom`)는 WhatsApp 계정별 전역입니다. [멀티 에이전트 라우팅](/ko/concepts/multi-agent) 및 [WhatsApp](/ko/channels/whatsapp)을 참조하세요.
  </Accordion>

  <Accordion title='“빠른 채팅” 에이전트와 “코딩용 Opus” 에이전트를 실행할 수 있나요?'>
    예. 멀티 에이전트 라우팅을 사용하세요. 각 에이전트에 자체 기본 모델을 지정한 다음, 인바운드 라우트(공급자 계정 또는 특정 peer)를 각 에이전트에 바인딩합니다. 예시 구성은 [멀티 에이전트 라우팅](/ko/concepts/multi-agent)에 있습니다. [모델](/ko/concepts/models) 및 [구성](/ko/gateway/configuration)도 참조하세요.
  </Accordion>
  ...
  <Accordion title="Homebrew는 Linux에서 작동하나요?">
    ...
    systemd를 통해 OpenClaw를 실행하는 경우, 비로그인 셸에서도 `brew`로 설치한 도구가 해석되도록 서비스 PATH에 `/home/linuxbrew/.linuxbrew/bin`(또는 brew 접두사)이 포함되어 있는지 확인하세요.
  ...
  <Accordion title="Gateway를 노트북에서 실행해야 하나요, 아니면 VPS에서 실행해야 하나요?">
    짧은 답: **24/7 안정성을 원한다면 VPS를 사용하세요**. 마찰을 최소화하고 절전/재시작을 감수할 수 있다면 로컬에서 실행하세요.
    ...
    **VPS / 클라우드**
    ...
    - **장점:** 항상 켜져 있음, 안정적인 네트워크, 노트북 절전 문제 없음, 계속 실행 상태로 유지하기 쉬움.
    - **단점:** 대개 헤드리스로 실행됨(스크린샷 사용), 원격 파일 접근만 가능, 업데이트하려면 SSH를 사용해야 함.

    **OpenClaw 관련 참고:** WhatsApp/Telegram/Slack/Mattermost/Discord는 모두 VPS에서 잘 작동합니다. 실제 절충점은 **헤드리스 브라우저**와 보이는 창 중 무엇을 쓰느냐뿐입니다. [브라우저](/ko/tools/browser)를 참조하세요.

    **권장 기본값:** 이전에 Gateway 연결 끊김을 겪었다면 VPS를 사용하세요. Mac을 적극적으로 사용 중이고 로컬 파일 접근이나 보이는 브라우저를 통한 UI 자동화를 원한다면 로컬도 좋습니다.
  ...
  <Accordion title="전용 머신에서 OpenClaw를 실행하는 것이 얼마나 중요한가요?">
    ...
    - **전용 호스트(VPS/Mac mini/Pi):** 항상 켜져 있음, 절전/재부팅 중단이 적음, 권한이 더 깔끔함, 계속 실행 상태로 유지하기 쉬움.
    - **공유 노트북/데스크톱:** 테스트와 적극적인 사용에는 전혀 문제없지만, 머신이 절전 모드로 들어가거나 업데이트될 때 일시 중지가 발생할 수 있습니다.

    두 가지 장점을 모두 원한다면 Gateway는 전용 호스트에 두고, 로컬 화면/카메라/실행 도구용 **Node**로 노트북을 페어링하세요. [Nodes](/ko/nodes)를 참조하세요.
  ...
  <Accordion title="최소 VPS 요구 사항과 권장 OS는 무엇인가요?">
    ...
    OS: **Ubuntu LTS**(또는 최신 Debian/Ubuntu)를 사용하세요. Linux 설치 경로는 해당 환경에서 가장 잘 테스트되었습니다.
  ...
  <Accordion title="OpenClaw를 VM에서 실행할 수 있나요? 요구 사항은 무엇인가요?">
    예. VM을 VPS와 동일하게 다루세요. 항상 켜져 있고, 접근 가능하며, Gateway와 활성화한 모든 채널에 충분한
    RAM이 필요합니다.
    ...
    호환성이 가장 좋습니다. [Windows](/ko/platforms/windows), [VPS 호스팅](/ko/vps)을 참조하세요.
