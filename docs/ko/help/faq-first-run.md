...
---
  ...
  빠른 시작 및 최초 실행 Q&A입니다. 일상적인 운영, 모델, 인증, 세션,
  ...
  <Accordion title="막혔을 때 가장 빠르게 해결하는 방법">
    **내 머신을 볼 수 있는** 로컬 AI 에이전트를 사용하세요. 대부분의 "막혔어요" 사례는 원격 도우미가 확인할 수 없는
    **로컬 구성 또는 환경 문제**이기 때문에 Discord에서 묻는 것보다 훨씬 효과적입니다.

    - **Claude Code**: [https://www.anthropic.com/claude-code/](https://www.anthropic.com/claude-code/)
    - **OpenAI Codex**: [https://openai.com/codex/](https://openai.com/codex/)

    이러한 도구는 repo를 읽고, 명령을 실행하고, 로그를 검사하며, 머신 수준
    설정(PATH, 서비스, 권한, 인증 파일)을 고치는 데 도움을 줄 수 있습니다. 해킹 가능한 (git) 설치로
    **전체 소스 체크아웃**을 제공하세요.
    ...
    이렇게 하면 OpenClaw가 **git checkout에서** 설치되므로, 에이전트가 코드와 문서를 읽고
    실행 중인 정확한 버전에 대해 추론할 수 있습니다. 나중에 언제든지 `--install-method git` 없이
    설치 프로그램을 다시 실행해 안정 버전으로 되돌릴 수 있습니다.

    팁: 에이전트에게 수정 작업을 **계획하고 감독**하게 한 다음(단계별), 필요한 명령만 실행하게 하세요.
    그러면 변경 사항이 작고 감사하기 쉬워집니다.

    실제 버그나 수정 사항을 발견했다면 GitHub issue를 만들거나 PR을 보내 주세요.
    ...
    - `openclaw status`: gateway/agent 상태와 기본 구성의 빠른 스냅샷입니다.
    ...
    빠른 디버그 루프: [무언가 고장 났을 때 첫 60초](#first-60-seconds-if-something-is-broken).
  ...
  <Accordion title="Heartbeat가 계속 건너뜁니다. 건너뛰기 이유는 무엇을 의미하나요?">
    ...
    - `empty-heartbeat-file`: `HEARTBEAT.md`가 있지만 빈 내용/헤더뿐인 스캐폴딩만 포함합니다.
    - `no-tasks-due`: `HEARTBEAT.md` 작업 모드가 활성 상태이지만 아직 도래한 작업 간격이 없습니다.
    - `alerts-disabled`: 모든 heartbeat 표시가 비활성화되어 있습니다(`showOk`, `showAlerts`, `useIndicator`가 모두 꺼져 있음).

    작업 모드에서는 실제 heartbeat 실행이 완료된 뒤에만 만료 예정 타임스탬프가 진행됩니다.
  ...
  <Accordion title="OpenClaw 설치 및 설정 권장 방법">
    repo는 소스에서 실행하고 온보딩을 사용하는 방식을 권장합니다.
    ...
    마법사는 UI asset도 자동으로 빌드할 수 있습니다. 온보딩 후에는 보통 **18789** 포트에서 Gateway를 실행합니다.
  ...
  <Accordion title="온보딩 후 대시보드는 어떻게 열나요?">
    마법사는 온보딩 직후 깔끔한(토큰화되지 않은) 대시보드 URL로 브라우저를 열고, 요약에도 링크를 출력합니다. 해당 탭을 열어 두세요. 실행되지 않았다면 같은 머신에서 출력된 URL을 복사/붙여넣기하세요.
  </Accordion>
  ...
  <Accordion title="localhost와 원격에서 대시보드 인증은 어떻게 하나요?">
    **Localhost(같은 머신):**
    ...
    - 공유 비밀 인증을 요청하면 구성된 토큰이나 비밀번호를 Control UI 설정에 붙여넣으세요.
    ...
    **localhost가 아닌 경우:**
    ...
    - **Tailscale Serve**(권장): bind를 loopback으로 유지하고 `openclaw gateway --tailscale serve`를 실행한 뒤 `https://<magicdns>/`를 여세요. `gateway.auth.allowTailscale`이 `true`이면 identity header가 Control UI/WebSocket 인증을 충족합니다(붙여넣는 공유 비밀 없음, 신뢰된 gateway host라고 가정). HTTP API는 private-ingress `none` 또는 trusted-proxy HTTP 인증을 의도적으로 사용하지 않는 한 여전히 공유 비밀 인증이 필요합니다.
      같은 클라이언트의 잘못된 동시 Serve 인증 시도는 실패 인증 제한기가 기록하기 전에 직렬화되므로, 두 번째 잘못된 재시도는 이미 `retry later`를 표시할 수 있습니다.
    - **Tailnet bind**: `openclaw gateway --bind tailnet --token "<token>"`을 실행하거나 비밀번호 인증을 구성하고, `http://<tailscale-ip>:18789/`를 연 다음 대시보드 설정에 일치하는 공유 비밀을 붙여넣으세요.
    - **ID 인식 리버스 프록시**: Gateway를 신뢰된 프록시 뒤에 두고 `gateway.auth.mode: "trusted-proxy"`를 구성한 다음 프록시 URL을 여세요. 같은 host의 loopback 프록시는 명시적으로 `gateway.auth.trustedProxy.allowLoopback = true`가 필요합니다.
    - **SSH 터널**: `ssh -N -L 18789:127.0.0.1:18789 user@host`를 실행한 다음 `http://127.0.0.1:18789/`를 여세요. 공유 비밀 인증은 터널을 통해서도 적용됩니다. 요청되면 구성된 토큰이나 비밀번호를 붙여넣으세요.

    bind 모드와 인증 세부 정보는 [대시보드](/ko/web/dashboard)와 [Web surface](/ko/web)를 참조하세요.

  </Accordion>

  <Accordion title="채팅 승인에 exec 승인 구성이 두 개인 이유는 무엇인가요?">
    서로 다른 계층을 제어합니다.

    - `approvals.exec`: 승인 프롬프트를 채팅 대상에 전달합니다.
    - `channels.<channel>.execApprovals`: 해당 채널이 exec 승인용 네이티브 승인 클라이언트처럼 동작하게 합니다.

    host exec 정책은 여전히 실제 승인 게이트입니다. 채팅 구성은 승인
    프롬프트가 표시되는 위치와 사람들이 답변하는 방식만 제어합니다.
    ...
    - 네이티브 승인 카드/버튼을 사용할 수 있으면 해당 네이티브 UI가 기본 경로입니다. 도구 결과에서 채팅 승인을 사용할 수 없거나 수동 승인이 유일한 경로라고 할 때만 에이전트가 수동 `/approve` 명령을 포함해야 합니다.
    - 프롬프트도 다른 채팅이나 명시적 운영 방으로 전달되어야 할 때만 `approvals.exec`를 사용하세요.
    - 승인 프롬프트를 원래 방/토픽에 다시 게시하고 싶을 때만 `channels.<channel>.execApprovals.target: "channel"` 또는 `"both"`를 사용하세요.
    - Plugin 승인은 또 별도입니다. 기본적으로 같은 채팅의 `/approve`, 선택적 `approvals.plugin` 전달을 사용하며, 일부 네이티브 채널만 그 위에 plugin 승인 네이티브 처리를 유지합니다.
  ...
  <Accordion title="Raspberry Pi에서 실행되나요?">
    예. Gateway는 가볍습니다. 문서에는 개인용으로 충분한 사양으로 **512MB-1GB RAM**, **1 core**, 약 **500MB**
    디스크를 제시하며, **Raspberry Pi 4에서 실행할 수 있음**도 언급합니다.

    추가 여유 공간(로그, 미디어, 기타 서비스)을 원한다면 **2GB를 권장**하지만,
    엄격한 최소 사양은 아닙니다.

    팁: 작은 Pi/VPS가 Gateway를 host할 수 있고, 노트북/휴대폰에서 **노드**를 페어링해
    로컬 화면/카메라/canvas 또는 명령 실행을 사용할 수 있습니다. [노드](/ko/nodes)를 참조하세요.

  </Accordion>

  <Accordion title="Raspberry Pi 설치 팁이 있나요?">
    짧게 말하면 작동하지만, 거친 부분은 예상하세요.
  ...
  <Accordion title="wake up my friend에서 멈췄습니다 / 온보딩이 hatch되지 않습니다. 이제 어떻게 하나요?">
    해당 화면은 Gateway에 연결할 수 있고 인증되어야 합니다. TUI도 첫 hatch 때
    "Wake up, my friend!"를 자동으로 보냅니다. **응답 없이** 이 줄이 보이고
    ...
    3. 그래도 멈춰 있다면 다음을 실행합니다.

    ```bash
    openclaw doctor
    ```

    Gateway가 원격에 있다면 터널/Tailscale 연결이 활성 상태인지, UI가 올바른 Gateway를 가리키는지 확인하세요.
    [원격 액세스](/ko/gateway/remote)를 참조하세요.

  </Accordion>

  <Accordion title="온보딩을 다시 하지 않고 설정을 새 머신(Mac mini)으로 마이그레이션할 수 있나요?">
    예. **상태 디렉터리**와 **workspace**를 복사한 다음 Doctor를 한 번 실행하세요. 다음
    두 위치를 **모두** 복사하면 봇이 "정확히 동일하게"(memory, session history, auth, channel
    state) 유지됩니다.
    ...
    이렇게 하면 config, auth profile, WhatsApp creds, session, memory가 보존됩니다. 원격 모드라면
    gateway host가 session store와 workspace를 소유한다는 점을 기억하세요.

    **중요:** workspace만 GitHub에 commit/push하면
    **memory + bootstrap files**는 백업되지만, session history나 auth는 백업되지 않습니다. 이들은
    `~/.openclaw/` 아래에 있습니다(예: `~/.openclaw/agents/<agentId>/sessions/`).

    관련 문서: [마이그레이션](/ko/install/migrating), [디스크에서 항목이 위치하는 곳](#where-things-live-on-disk),
    [Agent workspace](/ko/concepts/agent-workspace), [Doctor](/ko/gateway/doctor),
    [원격 모드](/ko/gateway/remote).

  </Accordion>

  <Accordion title="최신 버전의 새로운 내용은 어디서 볼 수 있나요?">
    GitHub changelog를 확인하세요.
    [https://github.com/openclaw/openclaw/blob/main/CHANGELOG.md](https://github.com/openclaw/openclaw/blob/main/CHANGELOG.md)

    최신 항목은 맨 위에 있습니다. 맨 위 섹션이 **Unreleased**로 표시되어 있으면 다음 날짜가 있는
    섹션이 최신 출시 버전입니다. 항목은 **Highlights**, **Changes**, **Fixes**(필요 시 docs/other 섹션 포함)로
    그룹화됩니다.
  ...
  <Accordion title="docs.openclaw.ai에 접근할 수 없습니다(SSL 오류)">
    ...
    Advanced Security를 통해 `docs.openclaw.ai`를 잘못 차단합니다. 이를 비활성화하거나 `docs.openclaw.ai`를 허용 목록에 추가한 다음 다시 시도하세요.
    여기에서 신고해 차단 해제에 도움을 주세요: [https://spa.xfinity.com/check_url_status](https://spa.xfinity.com/check_url_status).

    그래도 사이트에 접속할 수 없다면 문서는 GitHub에 미러링되어 있습니다.
  ...
  <Accordion title="안정 버전과 베타의 차이">
    **안정 버전**과 **베타**는 별도의 코드 라인이 아니라 **npm dist-tag**입니다.
    ...
    일반적으로 안정 릴리스는 먼저 **베타**에 올라간 뒤, 명시적인
    승격 단계에서 같은 버전을 `latest`로 이동합니다. 유지관리자는 필요한 경우
  ...
  <Accordion title="최신 빌드는 어떻게 사용해 볼 수 있나요?">
    ...
    직접 깨끗하게 클론하는 방식을 선호한다면 다음을 사용하세요.
  ...
  <Accordion title="설치 프로그램이 멈췄나요? 더 많은 피드백을 얻으려면 어떻게 하나요?">
  ...
  <Accordion title="Windows 설치에서 git을 찾을 수 없거나 openclaw가 인식되지 않는다고 표시됩니다">
    ...
    **2) 설치 후 openclaw가 인식되지 않음**
    ...
    - 해당 디렉터리를 사용자 PATH에 추가하세요(Windows에서는 `\bin` 접미사가 필요하지 않음. 대부분의 시스템에서는 `%AppData%\npm`입니다).
  ...
  <Accordion title="Windows exec 출력에 깨진 중국어 텍스트가 표시됩니다. 어떻게 해야 하나요?">
    이는 일반적으로 네이티브 Windows 셸의 콘솔 코드 페이지 불일치 때문입니다.
  ...
  <Accordion title="문서가 제 질문에 답하지 못했습니다. 더 나은 답변을 얻으려면 어떻게 하나요?">
    ...
    봇(또는 Claude/Codex)에게 _그 폴더에서_ 질문하여 저장소를 읽고 정확히 답할 수 있게 하세요.
  ...
  <Accordion title="Linux에 OpenClaw를 어떻게 설치하나요?">
    간단한 답: Linux 가이드를 따른 뒤 온보딩을 실행하세요.
  ...
  <Accordion title="VPS에 OpenClaw를 어떻게 설치하나요?">
    어떤 Linux VPS든 사용할 수 있습니다. 서버에 설치한 다음 SSH/Tailscale로 Gateway에 접근하세요.
  ...
  <Accordion title="클라우드/VPS 설치 가이드는 어디에 있나요?">
    일반적인 제공업체를 모아 둔 **호스팅 허브**를 유지하고 있습니다. 하나를 선택해 가이드를 따르세요.
    ...
    Control UI(또는 Tailscale/SSH)를 통해 접근합니다. 상태와 작업 공간은
    서버에 있으므로 호스트를 진실 공급원으로 취급하고 백업하세요.
  ...
  <Accordion title="OpenClaw에 자체 업데이트를 요청할 수 있나요?">
    간단한 답: **가능하지만 권장하지 않습니다**. 업데이트 흐름은
    Gateway를 다시 시작할 수 있고(활성 세션이 끊김), 깨끗한 git checkout이 필요할 수 있으며,
  ...
  <Accordion title="온보딩은 실제로 무엇을 하나요?">
    ...
    - **모델/인증 설정**(제공업체 OAuth, API 키, Anthropic setup-token, LM Studio 같은 로컬 모델 옵션 포함)
    - **작업 공간** 위치 + 부트스트랩 파일
  ...
  <Accordion title="이를 실행하려면 Claude 또는 OpenAI 구독이 필요한가요?">
    아니요. **API 키**(Anthropic/OpenAI/기타) 또는 **로컬 전용 모델**로
    OpenClaw를 실행하여 데이터를 장치에 유지할 수 있습니다. 구독(Claude
    ...
    - **OpenClaw의 Claude CLI / Claude 구독 인증**: Anthropic 직원이
      이 사용이 다시 허용된다고 알려 주었으며, OpenClaw는 Anthropic이 새
      정책을 게시하지 않는 한 이 통합에 대해 `claude -p` 사용을 승인된 것으로 취급합니다

    오래 실행되는 Gateway 호스트에는 Anthropic API 키가 여전히 더
  ...
  <Accordion title="API 키 없이 Claude Max 구독을 사용할 수 있나요?">
    ...
    OpenClaw는 Anthropic이 새 정책을 게시하지 않는 한 이 통합에 대해
  ...
  <Accordion title="Claude 구독 인증(Claude Pro 또는 Max)을 지원하나요?">
    ...
    Anthropic 직원이 이 사용이 다시 허용된다고 알려 주었으므로, OpenClaw는
    Anthropic이 새 정책을 게시하지 않는 한 이 통합에 대해
    Claude CLI 재사용과 `claude -p` 사용을 승인된 것으로 취급합니다.

    Anthropic setup-token은 여전히 지원되는 OpenClaw 토큰 경로로 사용할 수 있지만, 이제 OpenClaw는 가능할 때 Claude CLI 재사용과 `claude -p`를 선호합니다.
...
<AccordionGroup>
  <Accordion title="Anthropic에서 HTTP 429 rate_limit_error가 표시되는 이유는 무엇인가요?">
    이는 현재 창에서 **Anthropic 할당량/속도 제한**이 소진되었음을 의미합니다. **Claude CLI**를
    사용한다면 창이 재설정될 때까지 기다리거나 플랜을 업그레이드하세요. **Anthropic API 키**를
    사용한다면 Anthropic Console에서 사용량/결제를 확인하고 필요에 따라 한도를 높이세요.

    메시지가 구체적으로
    `Extra usage is required for long context requests`라면, 요청이
    Anthropic의 1M 컨텍스트 베타(`context1m: true`)를 사용하려는 것입니다. 이는
    자격 증명이 긴 컨텍스트 과금(API 키 과금 또는 Extra Usage가 활성화된
    OpenClaw Claude-login 경로)에 적합한 경우에만 작동합니다.
  ...
  <Accordion title="AWS Bedrock이 지원되나요?">
    예. OpenClaw에는 번들 **Amazon Bedrock (Converse)** 제공자가 포함되어 있습니다. AWS 환경 표시자가 있으면 OpenClaw가 스트리밍/텍스트 Bedrock 카탈로그를 자동으로 찾아 암시적 `amazon-bedrock` 제공자로 병합할 수 있습니다. 그렇지 않으면 `plugins.entries.amazon-bedrock.config.discovery.enabled`를 명시적으로 활성화하거나 수동 제공자 항목을 추가할 수 있습니다. [Amazon Bedrock](/ko/providers/bedrock) 및 [Model providers](/ko/providers/models)를 참조하세요. 관리형 키 흐름을 선호한다면 Bedrock 앞에 OpenAI 호환 프록시를 두는 것도 여전히 유효한 선택입니다.
  </Accordion>
  ...
  <Accordion title="Codex 인증은 어떻게 작동하나요?">
    ...
    ChatGPT/Codex 구독 인증과 네이티브 Codex 앱 서버 실행을 함께 사용합니다.
    기본 PI 러너를 통해 Codex OAuth를 사용하려는 경우에만
    `openai-codex/gpt-5.5`를 사용하세요. 직접 OpenAI API 키 액세스에는
    Codex 런타임 재정의 없이 `openai/gpt-5.5`를 사용하세요.
    [Model providers](/ko/concepts/model-providers) 및 [Onboarding (CLI)](/ko/start/wizard)을 참조하세요.
  </Accordion>

  <Accordion title="OpenClaw가 왜 아직도 openai-codex를 언급하나요?">
    `openai-codex`는 ChatGPT/Codex OAuth용 제공자 및 인증 프로필 id입니다.
    또한 Codex OAuth의 명시적 PI 모델 접두사이기도 합니다.

    - `openai/gpt-5.5` + `agentRuntime.id: "codex"` = 네이티브 Codex 런타임을 사용하는 ChatGPT/Codex 구독 인증
    - `openai-codex/gpt-5.5` = PI의 Codex OAuth 경로
    - Codex 런타임 재정의 없는 `openai/gpt-5.5` = PI의 직접 OpenAI API 키 경로
    - `openai-codex:...` = 모델 참조가 아닌 인증 프로필 id

    직접 OpenAI Platform 청구/한도 경로를 원하면
    `OPENAI_API_KEY`를 설정하세요. ChatGPT/Codex 구독 인증을 원하면
    `openclaw models auth login --provider openai-codex`로 로그인하세요. 네이티브 Codex
    런타임의 경우 모델 참조를 `openai/gpt-5.5`로 유지하고
    `agentRuntime.id: "codex"`를 설정하세요. `openai-codex/*` 모델 참조는 PI
    실행에만 사용하세요.

  </Accordion>

  <Accordion title="Codex OAuth 한도가 ChatGPT 웹과 다를 수 있는 이유는 무엇인가요?">
    Codex OAuth는 OpenAI가 관리하는 플랜 종속 할당량 창을 사용합니다. 실제로는
    둘 다 같은 계정에 연결되어 있더라도 이러한 한도가 ChatGPT 웹사이트/앱 경험과
    다를 수 있습니다.

    OpenClaw는 `openclaw models status`에서 현재 표시되는 제공자 사용량/할당량 창을
    보여줄 수 있지만, ChatGPT 웹 권한을 직접 API 액세스로 만들어내거나 정규화하지는
    않습니다. 직접 OpenAI Platform 청구/한도 경로를 원하면 API 키와 함께 `openai/*`를 사용하세요.
  ...
  <Accordion title="OpenAI 구독 인증(Codex OAuth)을 지원하나요?">
    ...
    Onboarding에서 OAuth 흐름을 실행할 수 있습니다.

    [OAuth](/ko/concepts/oauth), [Model providers](/ko/concepts/model-providers), 그리고 [Onboarding (CLI)](/ko/start/wizard)을 참조하세요.

  </Accordion>

  <Accordion title="Gemini CLI OAuth는 어떻게 설정하나요?">
    Gemini CLI는 `openclaw.json`의 클라이언트 id나 secret이 아니라 **Plugin 인증 흐름**을 사용합니다.
    ...
    5. 요청이 실패하면 Gateway 호스트에서 `GOOGLE_CLOUD_PROJECT` 또는 `GOOGLE_CLOUD_PROJECT_ID`를 설정합니다

    이렇게 하면 OAuth 토큰이 Gateway 호스트의 인증 프로필에 저장됩니다. 자세한 내용: [Model providers](/ko/concepts/model-providers).

  </Accordion>

  <Accordion title="일상적인 채팅에 로컬 모델을 사용해도 괜찮나요?">
    대체로 그렇지 않습니다. OpenClaw에는 큰 컨텍스트와 강력한 안전성이 필요합니다. 작은 카드는 잘리고 누출됩니다. 꼭 사용해야 한다면 로컬에서 실행할 수 있는 **가장 큰** 모델 빌드(LM Studio)를 실행하고 [/gateway/local-models](/ko/gateway/local-models)를 참조하세요. 더 작거나 양자화된 모델은 프롬프트 인젝션 위험을 높입니다. [Security](/ko/gateway/security)를 참조하세요.
  </Accordion>

  <Accordion title="호스팅 모델 트래픽을 특정 지역에 유지하려면 어떻게 하나요?">
    지역 고정 엔드포인트를 선택하세요. OpenRouter는 MiniMax, Kimi, GLM에 대해 미국 호스팅 옵션을 제공합니다. 데이터를 해당 지역에 유지하려면 미국 호스팅 변형을 선택하세요. 선택한 지역 제공자를 존중하면서도 대체 항목을 계속 사용할 수 있도록 `models.mode: "merge"`를 사용해 Anthropic/OpenAI를 함께 나열할 수도 있습니다.
  </Accordion>

  <Accordion title="이것을 설치하려면 Mac Mini를 사야 하나요?">
    아니요. OpenClaw는 macOS 또는 Linux(Windows는 WSL2를 통해)에서 실행됩니다. Mac mini는 선택 사항입니다. 일부 사용자는 항상 켜져 있는 호스트로 사용하려고 구매하지만, 작은 VPS, 홈 서버, 또는 Raspberry Pi급 장치도 사용할 수 있습니다.

    **macOS 전용 도구**를 사용하려는 경우에만 Mac이 필요합니다. iMessage의 경우 [BlueBubbles](/ko/channels/bluebubbles)(권장)를 사용하세요. BlueBubbles 서버는 어떤 Mac에서든 실행되며, Gateway는 Linux나 다른 곳에서 실행할 수 있습니다. 다른 macOS 전용 도구를 원한다면 Mac에서 Gateway를 실행하거나 macOS Node를 페어링하세요.
  ...
  <Accordion title="iMessage 지원에 Mac mini가 필요한가요?">
    Messages에 로그인된 **어떤 macOS 기기**가 필요합니다. 꼭 Mac mini일 필요는 없습니다.
    어떤 Mac이든 작동합니다. iMessage에는 **[BlueBubbles](/ko/channels/bluebubbles)를 사용하세요**(권장). BlueBubbles 서버는 macOS에서 실행되고, Gateway는 Linux나 다른 곳에서 실행할 수 있습니다.

    일반적인 설정:

    - Linux/VPS에서 Gateway를 실행하고, Messages에 로그인된 아무 Mac에서 BlueBubbles 서버를 실행합니다.
    - 가장 단순한 단일 머신 설정을 원하면 Mac에서 모든 것을 실행합니다.
  ...
  <Accordion title="OpenClaw를 실행하려고 Mac mini를 사면 MacBook Pro에 연결할 수 있나요?">
    예. **Mac mini는 Gateway를 실행할 수 있고**, MacBook Pro는
    **Node**(보조 기기)로 연결할 수 있습니다. Nodes는 Gateway를 실행하지 않습니다. 대신 해당 기기에서 화면/카메라/캔버스 및 `system.run` 같은 추가 기능을 제공합니다.

    일반적인 패턴:

    - Mac mini에서 Gateway를 실행합니다(항상 켜짐).
    - MacBook Pro에서 macOS 앱 또는 Node 호스트를 실행하고 Gateway에 페어링합니다.
    - `openclaw nodes status` / `openclaw nodes list`로 확인합니다.
  ...
  <Accordion title="Telegram: allowFrom에는 무엇이 들어가나요?">
    ...
    - 봇에게 DM을 보낸 다음 `openclaw logs --follow`를 실행하고 `from.id`를 확인합니다.

    공식 Bot API:

    - 봇에게 DM을 보낸 다음 `https://api.telegram.org/bot<bot_token>/getUpdates`를 호출하고 `message.from.id`를 확인합니다.
  ...
  <Accordion title="여러 사람이 서로 다른 OpenClaw 인스턴스로 하나의 WhatsApp 번호를 사용할 수 있나요?">
    예, **다중 에이전트 라우팅**을 통해 가능합니다. 각 발신자의 WhatsApp **DM**(피어 `kind: "direct"`, `+15551234567` 같은 발신자 E.164)을 서로 다른 `agentId`에 바인딩하면, 각 사람이 자신의 워크스페이스와 세션 저장소를 갖게 됩니다. 응답은 여전히 **같은 WhatsApp 계정**에서 전송되며, DM 액세스 제어(`channels.whatsapp.dmPolicy` / `channels.whatsapp.allowFrom`)는 WhatsApp 계정별로 전역입니다. [Multi-Agent Routing](/ko/concepts/multi-agent) 및 [WhatsApp](/ko/channels/whatsapp)을 참조하세요.
  </Accordion>

  <Accordion title='“빠른 채팅” 에이전트와 “코딩용 Opus” 에이전트를 실행할 수 있나요?'>
    예. 다중 에이전트 라우팅을 사용하세요. 각 에이전트에 고유한 기본 모델을 지정한 다음, 인바운드 경로(제공자 계정 또는 특정 피어)를 각 에이전트에 바인딩합니다. 예시 구성은 [Multi-Agent Routing](/ko/concepts/multi-agent)에 있습니다. [Models](/ko/concepts/models) 및 [Configuration](/ko/gateway/configuration)도 참조하세요.
  </Accordion>
  ...
  <Accordion title="Homebrew는 Linux에서 작동하나요?">
    ...
    systemd를 통해 OpenClaw를 실행하는 경우, 비로그인 셸에서도 `brew`로 설치한 도구를 확인할 수 있도록 서비스 PATH에 `/home/linuxbrew/.linuxbrew/bin`(또는 brew 접두사)이 포함되어 있는지 확인하세요.
  ...
  <Accordion title="해킹 가능한 git 설치와 npm 설치의 차이">
    - **해킹 가능한(git) 설치:** 전체 소스 체크아웃, 편집 가능, 기여자에게 가장 적합합니다.
  ...
  <Accordion title="나중에 npm 설치와 git 설치를 전환할 수 있나요?">
    ...
    이 작업은 **데이터를 삭제하지 않습니다**. OpenClaw 코드 설치만 변경합니다.
    ...
    `--no-restart`를 전달하지 않는 한 Gateway를 재시작합니다.

    설치 프로그램도 어느 모드든 강제할 수 있습니다.
    ...
    백업 팁: [Backup strategy](#where-things-live-on-disk)를 참조하세요.

  </Accordion>

  <Accordion title="Gateway를 노트북에서 실행해야 하나요, 아니면 VPS에서 실행해야 하나요?">
    짧은 답: **24/7 안정성을 원한다면 VPS를 사용하세요**. 마찰이 가장 적고
    절전/재시작을 감수할 수 있다면 로컬에서 실행하세요.

    **노트북(로컬 Gateway)**

    - **장점:** 서버 비용 없음, 로컬 파일에 직접 액세스, 실시간 브라우저 창.
    - **단점:** 절전/네트워크 끊김 = 연결 해제, OS 업데이트/재부팅으로 중단, 계속 깨어 있어야 함.

    **VPS / 클라우드**

    - **장점:** 항상 켜져 있고, 네트워크가 안정적이며, 노트북 절전 문제가 없고, 계속 실행 상태로 유지하기 쉽습니다.
    - **단점:** 헤드리스로 실행하는 경우가 많고(스크린샷 사용), 원격 파일 접근만 가능하며, 업데이트에는 SSH가 필요합니다.

    **OpenClaw 관련 참고:** WhatsApp/Telegram/Slack/Mattermost/Discord는 모두 VPS에서 문제없이 작동합니다. 실제 트레이드오프는 **헤드리스 브라우저**와 보이는 창 중 무엇을 쓸지뿐입니다. [브라우저](/ko/tools/browser)를 참고하세요.

    **권장 기본값:** 이전에 Gateway 연결 끊김을 겪었다면 VPS를 사용하세요. Mac을 적극적으로 사용 중이고 로컬 파일 접근이나 보이는 브라우저로 UI 자동화를 원한다면 로컬도 좋습니다.
  ...
  <Accordion title="OpenClaw를 전용 머신에서 실행하는 것이 얼마나 중요한가요?">
    ...
    - **전용 호스트(VPS/Mac mini/Pi):** 항상 켜져 있고, 절전/재부팅으로 인한 중단이 적으며, 권한이 더 깔끔하고, 계속 실행 상태로 유지하기 쉽습니다.
    - **공유 노트북/데스크톱:** 테스트와 적극적인 사용에는 전혀 문제가 없지만, 머신이 절전 모드로 들어가거나 업데이트될 때 일시 중지가 발생할 수 있습니다.

    두 가지 장점을 모두 원한다면 Gateway는 전용 호스트에 두고, 로컬 화면/카메라/exec 도구용 **Node**로 노트북을 페어링하세요. [Node](/ko/nodes)를 참고하세요.
  ...
  <Accordion title="OpenClaw를 VM에서 실행할 수 있나요? 요구 사항은 무엇인가요?">
    예. VM은 VPS와 동일하게 취급하세요. 항상 켜져 있고, 접근 가능하며, Gateway와 활성화한 채널을 실행할 충분한
    RAM이 필요합니다.
