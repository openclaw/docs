...
title: 'FAQ: 모델 및 인증'
x-i18n:
    generated_at: "2026-05-06T06:28:02Z"
    model: gpt-5.5
    provider: openai
...
---
  ...
  모델 및 인증 프로필 Q&A입니다. 설정, 세션, Gateway, 채널, 문제 해결은
  기본 [FAQ](/ko/help/faq)를 참고하세요.
  ...
  <Accordion title='"기본 모델"이란 무엇인가요?'>
    ...
    모델은 `provider/model` 형식으로 참조됩니다(예: `openai/gpt-5.5` 또는 `openai-codex/gpt-5.5`). 공급자를 생략하면 OpenClaw는 먼저 별칭을 시도한 다음, 해당 정확한 모델 ID와 일치하는 고유한 구성된 공급자를 찾고, 그 다음에야 더 이상 권장되지 않는 호환 경로로 구성된 기본 공급자로 대체합니다. 해당 공급자가 더 이상 구성된 기본 모델을 제공하지 않으면, OpenClaw는 오래된 제거된 공급자 기본값을 표시하는 대신 처음 구성된 공급자/모델로 대체합니다. 그래도 `provider/model`을 **명시적으로** 설정해야 합니다.
  ...
  <Accordion title="어떤 모델을 추천하나요?">
    ...
    **일상적/낮은 위험도의 채팅:** 더 저렴한 대체 모델을 사용하고 에이전트 역할에 따라 라우팅하세요.

    MiniMax에는 자체 문서가 있습니다: [MiniMax](/ko/providers/minimax) 및
    [로컬 모델](/ko/gateway/local-models).

    경험칙: 높은 위험도의 작업에는 **감당할 수 있는 최고의 모델**을 사용하고, 일상적인 채팅이나 요약에는 더 저렴한
    ...
    강력한 경고: 약하거나 과도하게 양자화된 모델은 프롬프트
  ...
  <Accordion title="구성을 지우지 않고 모델을 전환하려면 어떻게 하나요?">
    **모델 명령**을 사용하거나 **모델** 필드만 편집하세요. 전체 구성 교체는 피하세요.
    ...
    RPC 편집의 경우 먼저 `config.schema.lookup`으로 검사하고 `config.patch`를 선호하세요. 조회 페이로드는 정규화된 경로, 얕은 스키마 문서/제약 조건, 즉시 하위 항목 요약을 제공합니다.
    부분 업데이트용입니다.
    구성을 덮어쓴 경우 백업에서 복원하거나 `openclaw doctor`를 다시 실행해 복구하세요.
  ...
  <Accordion title="자체 호스팅 모델(llama.cpp, vLLM, Ollama)을 사용할 수 있나요?">
    ...
    3. 클라우드 모델도 원하면 `ollama signin` 실행
  ...
  <Accordion title="OpenClaw, Flawd, Krill은 어떤 모델을 사용하나요?">
    - 이러한 배포는 서로 다를 수 있고 시간이 지나며 변경될 수 있습니다. 고정된 공급자 권장은 없습니다.
  ...
  <Accordion title="재시작 없이 즉시 모델을 전환하려면 어떻게 하나요?">
    ...
    `/model` 및 `/model list`는 간결한 번호가 매겨진 선택기를 표시합니다. 번호로 선택하세요.

    ```
    /model 3
    ```

    공급자에 대해 특정 인증 프로필을 강제로 지정할 수도 있습니다(세션별).
  ...
  <Accordion title="일상 작업에는 GPT 5.5를, 코딩에는 Codex 5.5를 사용할 수 있나요?">
    ...
    - **네이티브 Codex 코딩 에이전트:** `agents.defaults.model.primary`를 `openai/gpt-5.5`로 설정하고 `agents.defaults.agentRuntime.id`를 `"codex"`로 설정하세요. ChatGPT/Codex 구독 인증을 원할 때는 `openclaw models auth login --provider openai-codex`로 로그인하세요.
    - **PI를 통한 직접 OpenAI API 작업:** Codex 런타임 재정의 없이 `/model openai/gpt-5.5`를 사용하고 `OPENAI_API_KEY`를 구성하세요.
    - **PI를 통한 Codex OAuth:** 일반 PI 러너와 Codex OAuth를 의도적으로 사용하려는 경우에만 `/model openai-codex/gpt-5.5`를 사용하세요.
    - **하위 에이전트:** 코딩 작업을 자체 모델과 `agentRuntime` 기본값이 있는 Codex 전용 에이전트로 라우팅하세요.
  ...
  <Accordion title="GPT 5.5의 빠른 모드는 어떻게 구성하나요?">
    ...
    OpenAI의 경우 빠른 모드는 지원되는 네이티브 Responses 요청에서 `service_tier = "priority"`에 매핑됩니다. 세션 `/fast` 재정의가 구성 기본값보다 우선합니다.

    [Thinking 및 빠른 모드](/ko/tools/thinking) 및 [OpenAI 빠른 모드](/ko/providers/openai#fast-mode)를 참고하세요.

  </Accordion>

  <Accordion title='"Model ... is not allowed"가 표시된 뒤 답장이 없는 이유는 무엇인가요?'>
    ...
    해당 오류는 일반 답장 **대신** 반환됩니다. 해결: 모델을
    `agents.defaults.models`에 추가하거나, 허용 목록을 제거하거나, `/model list`에서 모델을 선택하세요.
    명령에 `--runtime codex`도 포함되어 있었다면 먼저 모델을 추가한 뒤
    동일한 `/model provider/model --runtime codex` 명령을 다시 시도하세요.

  </Accordion>

  <Accordion title='"Unknown model: minimax/MiniMax-M2.7"이 표시되는 이유는 무엇인가요?'>
    이는 **공급자가 구성되지 않았음**을 의미합니다(MiniMax 공급자 구성 또는 인증
    프로필을 찾을 수 없음). 따라서 모델을 해석할 수 없습니다.

    해결 체크리스트:

    1. 최신 OpenClaw 릴리스로 업그레이드하거나 소스 `main`에서 실행한 뒤 Gateway를 재시작하세요.
    2. MiniMax가 구성되어 있는지(마법사 또는 JSON), 또는 MiniMax 인증이
       env/인증 프로필에 있어 일치하는 공급자가 주입될 수 있는지 확인하세요
  ...
  <Accordion title="MiniMax를 기본값으로 사용하고 복잡한 작업에는 OpenAI를 사용할 수 있나요?">
    ...
    대체는 "어려운 작업"이 아니라 **오류**를 위한 것이므로 `/model` 또는 별도 에이전트를 사용하세요.
  ...
  <Accordion title="opus / sonnet / gpt는 기본 제공 바로 가기인가요?">
    예. OpenClaw는 몇 가지 기본 약칭을 제공합니다(`agents.defaults.models`에 모델이 있을 때만 적용됨).

    - `opus` → `anthropic/claude-opus-4-6`
    - `sonnet` → `anthropic/claude-sonnet-4-6`
    - `gpt` → API 키 설정에서는 `openai/gpt-5.5`, Codex OAuth용으로 구성된 경우 `openai-codex/gpt-5.5`
    ...
    같은 이름으로 자체 별칭을 설정하면 사용자 값이 우선합니다.

  </Accordion>

  <Accordion title="모델 바로 가기(별칭)를 정의/재정의하려면 어떻게 하나요?">
    ...
    그러면 `/model sonnet` 또는 지원되는 경우 `/<alias>`가 해당 모델 ID로 해석됩니다.

  </Accordion>

  <Accordion title="OpenRouter나 Z.AI 같은 다른 공급자의 모델을 추가하려면 어떻게 하나요?">
    ...
    공급자/모델을 참조했지만 필요한 공급자 키가 없으면 런타임 인증 오류가 발생합니다(예: `No API key found for provider "zai"`).

    **새 에이전트를 추가한 후 공급자에 대한 API 키를 찾을 수 없음**
    ...
    - OAuth 프로필의 경우 새 에이전트에 자체 계정이 필요할 때 해당 에이전트에서 로그인합니다. 그렇지 않으면 OpenClaw는 새로고침 토큰을 복제하지 않고 기본/메인 에이전트를 통해 읽을 수 있습니다.
...
<AccordionGroup>
  <Accordion title="장애 조치는 어떻게 작동하나요?">
    ...
    1. 같은 공급자 내 **인증 프로필 순환**.
    2. `agents.defaults.model.fallbacks`의 다음 모델로 **모델 폴백**.

    실패한 프로필에는 쿨다운이 적용되므로(지수 백오프), 공급자가 속도 제한에 걸리거나 일시적으로 실패하더라도 OpenClaw가 계속 응답할 수 있습니다.
    ...
    청구처럼 보이는 일부 응답은 `402`가 아니며, 일부 HTTP `402`
    응답도 이 일시적 버킷에 남습니다. 공급자가 `401` 또는 `403`에서
    명시적인 청구 관련 텍스트를 반환하면 OpenClaw는 여전히 이를
    청구 레인에 둘 수 있지만, 공급자별 텍스트 매처는 해당 매처를
    소유한 공급자로 범위가 제한됩니다(예: OpenRouter `Key limit exceeded`). `402`
    ...
    장기 청구 비활성화가 아니라 `rate_limit`으로 처리합니다.

    컨텍스트 오버플로 오류는 다릅니다. `request_too_large`,
    `input exceeds the maximum number of tokens`,
    `input token count exceeds the maximum number of input tokens`,
    `input is too long for the model`, 또는 `ollama error: context length
    exceeded` 같은 시그니처는 모델 폴백으로 진행하지 않고
    Compaction/재시도 경로에 남습니다.

    일반 서버 오류 텍스트는 의도적으로 "unknown/error가 포함된 모든 것"보다
    더 좁게 처리됩니다. OpenClaw는 Anthropic의 단순 `An unknown error occurred`,
    OpenRouter의 단순 `Provider returned error`, `Unhandled stop reason:
    error` 같은 중지 사유 오류, 일시적인 서버 텍스트
    (`internal server error`, `unknown error, 520`, `upstream error`, `backend
    error`)가 포함된 JSON `api_error` 페이로드, 그리고 `ModelNotReadyException` 같은
    공급자 사용 중 오류처럼 공급자 범위의 일시적 형태를
    공급자 컨텍스트가 일치할 때 장애 조치할 만한 타임아웃/과부하 신호로
    처리합니다.
  ...
  <Accordion title='"No credentials found for profile anthropic:default"는 무엇을 의미하나요?'>
    이는 시스템이 인증 프로필 ID `anthropic:default`를 사용하려 했지만, 예상된 인증 저장소에서 해당 자격 증명을 찾을 수 없었다는 뜻입니다.
    ...
    **해결 체크리스트:**
    ...
    - **환경 변수가 Gateway에 로드되었는지 확인**
      - 셸에서 `ANTHROPIC_API_KEY`를 설정했지만 systemd/launchd로 Gateway를 실행하는 경우 이를 상속하지 않을 수 있습니다. `~/.openclaw/.env`에 넣거나 `env.shellEnv`를 활성화하세요.
    - **올바른 에이전트를 편집하고 있는지 확인**
      - 다중 에이전트 설정에서는 `auth-profiles.json` 파일이 여러 개 있을 수 있습니다.
    - **모델/인증 상태 기본 점검**
      - 구성된 모델과 공급자의 인증 여부를 보려면 `openclaw models status`를 사용하세요.
  ...
  <Accordion title="왜 Google Gemini도 시도한 뒤 실패했나요?">
    모델 구성에 Google Gemini가 폴백으로 포함되어 있거나 Gemini 약칭으로 전환한 경우, OpenClaw는 모델 폴백 중 이를 시도합니다. Google 자격 증명을 구성하지 않았다면 `No API key found for provider "google"`이 표시됩니다.

    해결: Google 인증을 제공하거나, `agents.defaults.model.fallbacks` / 별칭에서 Google 모델을 제거하거나 피해서 폴백이 그쪽으로 라우팅되지 않게 하세요.

    **LLM 요청 거부됨: thinking 서명 필요(Google Antigravity)**

    원인: 세션 기록에 **서명 없는 thinking 블록**이 포함되어 있습니다(대개
    중단되었거나 부분적인 스트림에서 발생). Google Antigravity는 thinking 블록에 서명을 요구합니다.

    해결: 이제 OpenClaw는 Google Antigravity Claude에 대해 서명 없는 thinking 블록을 제거합니다. 그래도 계속 나타나면 **새 세션**을 시작하거나 해당 에이전트에 `/thinking off`를 설정하세요.
...
<AccordionGroup>
  <Accordion title="인증 프로필이란 무엇인가요?">
    인증 프로필은 공급자에 연결된 이름 있는 자격 증명 레코드(OAuth 또는 API 키)입니다. 프로필은 다음 위치에 있습니다.

    ```
    ~/.openclaw/agents/<agentId>/agent/auth-profiles.json
    ```

    비밀을 출력하지 않고 저장된 프로필을 검사하려면 `openclaw models auth list`를 실행하세요(선택적으로 `--provider <id>` 또는 `--json`). 자세한 내용은 [모델 CLI](/ko/cli/models#auth-profiles)를 참조하세요.

  </Accordion>

  <Accordion title="일반적인 프로필 ID는 무엇인가요?">
    OpenClaw는 다음과 같은 공급자 접두사가 붙은 ID를 사용합니다.
  ...
  <Accordion title="어떤 인증 프로필을 먼저 시도할지 제어할 수 있나요?">
    예. 구성은 프로필에 대한 선택적 메타데이터와 공급자별 순서(`auth.order.<provider>`)를 지원합니다. 이는 비밀을 저장하지 않으며, ID를 공급자/모드에 매핑하고 순환 순서를 설정합니다.

    프로필이 짧은 **쿨다운**(속도 제한/타임아웃/인증 실패) 또는 더 긴 **비활성화** 상태(청구/크레딧 부족)에 있으면 OpenClaw가 일시적으로 해당 프로필을 건너뛸 수 있습니다. 이를 검사하려면 `openclaw models status --json`을 실행하고 `auth.unusableProfiles`를 확인하세요. 조정: `auth.cooldowns.billingBackoffHours*`.

    속도 제한 쿨다운은 모델 범위일 수 있습니다. 한 모델에 대해 쿨다운 중인
    프로필도 같은 공급자의 형제 모델에는 계속 사용할 수 있지만,
    청구/비활성화 기간은 여전히 전체 프로필을 차단합니다.
    ...
    # 순환을 단일 프로필로 고정합니다(이 프로필만 시도)
    openclaw models auth order set --provider anthropic anthropic:default

    # 또는 명시적 순서를 설정합니다(공급자 내 폴백)
    openclaw models auth order set --provider anthropic anthropic:work anthropic:default

    # 재정의를 지웁니다(config auth.order / 라운드 로빈으로 폴백)
    openclaw models auth order clear --provider anthropic
    ```
  ...
  <Accordion title="OAuth와 API 키의 차이는 무엇인가요?">
    ...
    - **OAuth**는 적용 가능한 경우 구독 접근 권한을 활용하는 경우가 많습니다.
...
- [자주 묻는 질문](/ko/help/faq) — 주요 자주 묻는 질문
