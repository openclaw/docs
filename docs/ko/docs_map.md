...
title: 문서 맵
x-i18n:
    generated_at: "2026-06-30T13:54:45Z"
    model: gpt-5.5
...
- 제목:
  ...
  - H2: 비공개 API 작업
  - H2: 구성 쓰기
  - H2: 분할 전송 DM 병합(하나의 작성 내 명령 + URL)
...
- 제목:
  ...
  - H3: /context map
  - H2: 컨텍스트 창에 포함되는 항목
  - H2: OpenClaw가 시스템 프롬프트를 만드는 방식
  - H2: 삽입된 작업공간 파일(프로젝트 컨텍스트)
  - H2: Skills: 삽입됨 vs 필요 시 로드됨
  - H2: 도구: 비용은 두 가지
  - H2: 명령, 지시문, "인라인 바로가기"
  - H2: 세션, Compaction, 프루닝(지속되는 항목)
...
- 제목:
  ...
  - H3: 도구 프로필
  - H3: 도구 그룹
  - H3: 샌드박스 도구 정책 안의 MCP 및 plugin 도구
  - H3: tools.codeMode
...
- 제목:
  ...
  - H3: "도구 X가 sandbox 도구 정책에 의해 차단됨"
  - H3: "이것이 main이라고 생각했는데 왜 sandbox 처리되었나요?"
  - H2: 관련 항목
...
- 제목:
  ...
  - H2: macOS Gateway가 조용히 응답을 멈췄다가 대시보드를 건드리면 재개됨
...
- 제목:
  ...
  - H3: context engine assembly는 Codex 입력으로 투영되어야 함
  - H3: prompt-cache 안정성이 중요함
  - H3: 런타임 선택 의미는 변경되지 않음
  - H2: 구현 계획
  - H3: 1. 재사용 가능한 context-engine attempt 헬퍼를 내보내거나 재배치
  - H3: 2. Codex context projection 헬퍼 추가
...
- 제목:
  ...
  - H3: 1) Gateway 시작
  - H3: 2) 검색 확인(선택 사항)
  - H4: Tailnet(Vienna ⇄ London) 검색 via 유니캐스트 DNS-SD
  - H3: 3) Android에서 연결
...
- 제목:
  ...
  - H3: Twilio 설정 검사 실패
  - H3: Twilio 통화는 시작되지만 회의에 들어가지 않음
  - H2: 참고 사항
  - H2: 관련 항목
...
- 제목:
  ...
  - H3: openclaw.plugin.json 생성된 메타데이터가 오래됨
...
- 제목:
  ...
  - H3: 4단계: Transcript, ACP 스트림, 궤적, VFS 이동
...
- 제목:
  ...
  - H2: 두 개의 지속성 레이어
  - H2: 디스크 위치
  - H2: 저장소 유지 관리와 디스크 제어
  - H2: Cron 세션 및 실행 로그
  - H2: 세션 키(sessionKey)
  - H2: 세션 ID(sessionId)
  - H2: 세션 저장소 스키마(sessions.json)
  - H2: Transcript 구조(.jsonl)
  - H2: 컨텍스트 창과 추적 토큰
  - H2: Compaction: 정의
...
- 제목:
  ...
  - H2: 중요한 주의 사항
  - H2: 결과 재현
  - H3: Gateway 노출 및 열린 Gateway 구성 오류
