...
이 파일은 에이전트가 문서 트리를 탐색할 수 있도록 `docs/**/*.md` 및 `docs/**/*.mdx` 제목에서 생성됩니다.
...
- 제목:
  ...
  - H2: 음성 메시지
  - H2: 승인 메타데이터
  - H3: 조용한 최종 미리 보기를 위한 자체 호스팅 푸시 규칙
  - H2: 봇 간 방
  - H2: 암호화 및 확인
  - H3: 암호화 활성화
  - H3: 상태 및 신뢰 신호
...
- 제목:
  ...
  - H3: 내부 hook(Gateway hook)
  - H3: Plugin hook(에이전트 + Gateway 수명 주기)
  - H2: 스트리밍 + 부분 응답
  - H2: 도구 실행 + 메시징 도구
  - H2: 응답 형태 지정 + 억제
  - H2: Compaction + 재시도
...
- 제목:
  ...
  - H2: 주입된 작업공간 파일(Project Context)
...
- 제목:
  ...
  - H3: 감사 추적
  - H2: delegate 설정
  - H3: 1. delegate 에이전트 생성
  - H3: 2. ID provider 위임 구성
  - H4: Microsoft 365
  - H4: Google Workspace
  - H3: 3. delegate를 채널에 바인딩
  - H3: 4. delegate 에이전트에 자격 증명 추가
  - H2: 예시: 조직 보조자
  - H2: 확장 패턴
  - H2: 관련 항목
...
- 제목:
  ...
  - H2: 모델 fallback
  - H3: 후보 체인 규칙
  - H3: 어떤 오류가 fallback을 진행시키는가
  - H3: Cooldown 건너뛰기와 probe 동작 비교
  - H2: 세션 override 및 실시간 모델 전환
  - H2: 관측 가능성과 장애 요약
  - H2: 관련 구성
...
- 제목:
  ...
  - H3: 기타 번들 provider Plugin
  - H4: 알아둘 만한 특이 사항
  - H2: models.providers를 통한 provider(사용자 지정/base URL)
  - H3: Moonshot AI(Kimi)
  - H3: Kimi coding
...
- 제목:
  ...
  - H2: 검색 입력(클라이언트가 Gateway 위치를 파악하는 방법)
...
- 제목:
  ...
  - H3: "이게 메인이라고 생각했는데 왜 샌드박스 처리되나요?"
  - H2: 관련 항목
...
- 제목:
  ...
  - H2: macOS 게이트웨이가 조용히 응답을 중지한 다음 대시보드를 터치하면 다시 시작됨
...
- 제목:
  ...
  - H2: 요구 사항
  - H2: 빠른 시작(페어링 + 연결)
  - H2: 공식 빌드를 위한 relay 지원 push
  - H2: 백그라운드 alive beacon
  - H2: 인증 및 신뢰 흐름
  - H2: 검색 경로
  - H3: Bonjour(LAN)
  - H3: Tailnet(크로스 네트워크)
...
- 제목:
  ...
  - H3: Twilio 설정 검사 실패
  - H3: Twilio 통화가 시작되지만 회의에 들어가지 못함
  - H2: 참고 사항
  - H2: 관련 항목
...
- 제목:
  ...
  - H3: 4단계: 대화 기록, ACP 스트림, 궤적, VFS 이동
...
- 제목:
  - H2: 문제: "포트 18800에서 Chrome CDP를 시작하지 못했습니다"
  - H3: 근본 원인
  - H3: 해결 방법 1: Google Chrome 설치(권장)
  - H3: 해결 방법 2: 연결 전용 모드로 Snap Chromium 사용
  - H3: 브라우저 작동 확인
  - H3: 구성 참조
  - H3: 문제: "profile=\"user\"에 대한 Chrome 탭을 찾을 수 없습니다"
  - H2: 관련 항목
...
- 제목:
  ...
  - H3: 중요 제한 사항: 임베디드 Lobster와 openclaw.invoke 비교
  - H2: 워크플로 파일(.lobster)
  - H2: Lobster 설치
  - H2: 도구 활성화
  - H2: 예시: 이메일 분류
