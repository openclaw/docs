...
title: 문서 맵
x-i18n:
    generated_at: "2026-07-12T15:13:33Z"
    model: gpt-5.6
...
- 제목:
  ...
  - H3: 봇이 메시지를 수신하지 않음
  - H3: Feishu 모바일 앱에서 QR 설정이 반응하지 않음
  - H3: App Secret 유출
  - H2: 고급 구성
  - H3: 여러 계정
  - H3: 메시지 제한
...
- 제목:
  ...
  - H2: 활성화(소유자 전용)
  - H2: 컨텍스트 필드
  - H2: iMessage 관련 세부 사항
  - H2: WhatsApp 시스템 프롬프트
  - H2: WhatsApp 관련 세부 사항
  - H2: 관련 문서
...
- 제목:
  ...
  - H2: 보안 기본값
  - H2: 접근 제어
  - H3: 흔한 주의 사항: allowFrom은 채널이 아닌 DM용입니다
  - H2: 응답 트리거(멘션)
...
- 제목:
  ...
  - H2: 음성 메시지
  - H2: 승인 메타데이터
  - H3: 확정된 미리보기 알림을 최소화하기 위한 자체 호스팅 푸시 규칙
  - H2: 봇 간 대화방
  - H2: 암호화 및 인증
  - H3: 암호화 활성화
  - H3: 상태 및 신뢰 신호
...
- 제목:
  ...
  - H2: SMS 보내기
  - H2: 설정 확인
  - H3: macOS iMessage/SMS에서 수행하는 엔드투엔드 테스트
  - H2: Webhook 보안
  - H2: 다중 계정 구성
  - H2: 문제 해결
...
- 제목:
  ...
  - H3: 영속적 수신 확인
  - H3: 공개 SDK 축소
  - H2: 구현이 원래 설계와 달라진 부분
  - H2: 구체적인 마이그레이션 위험(여전히 관련 있음)
  - H2: 실패 분류
  - H2: 미결 질문
  - H2: 관련 항목
...
- 제목:
  ...
  - H2: 여러 계정(프로필) 및 라우팅
  - H3: 1) 권장 방식: 에이전트 분리
  - H3: 2) 고급 방식: 하나의 에이전트에서 여러 프로필 사용
  - H2: 관련 항목
...
- 제목:
  ...
  - H3: 1) Gateway 자체 항목
  - H3: 2) WebSocket 연결
  - H4: 임시 제어 평면 연결이 표시되지 않는 이유
  - H3: 3) system-event 비콘
  - H3: 4) Node 연결(role: node)
  - H2: 병합 및 중복 제거 규칙(instanceId가 중요한 이유)
  - H2: TTL 및 크기 제한
  - H2: 원격/터널 관련 주의 사항(루프백 IP)
  - H2: 소비자
  - H3: Control UI의 Devices 페이지
  - H3: macOS의 Instances 탭
  - H2: 디버깅 팁
  - H2: 관련 항목
...
- 경로: /date-time
- 제목:
  - H2: 메시지 엔벌로프(기본적으로 현지 시간)
  - H3: 예시
  - H2: 시스템 프롬프트: 현재 날짜 및 시간
  - H2: 시스템 이벤트 줄(기본적으로 현지 시간)
  - H3: 사용자 시간대 및 형식 구성
  - H2: 시간 형식 감지(자동)
...
- 제목:
  ...
  - H2: 일반적인 패턴
  - H3: 하나의 재정의를 포함한 공유 Skills 기준선
  - H3: 다중 플랫폼 설정
  - H3: 신뢰할 수 있는 Node 네트워크 자동 승인
  - H3: 안전한 DM 모드(공유 받은편지함/다중 사용자 DM)
  - H3: Anthropic API 키 + MiniMax 대체
  - H3: 업무용 봇(제한된 액세스)
  - H3: 로컬 모델만 사용
  - H2: 팁
  - H2: 관련 문서
...
- 제목:
  - H2: 구조 봇 빠른 시작
  - H3: --profile rescue onboard가 변경하는 사항
...
- 제목:
  ...
  - H2: 일반적인 "샌드박스 격리" 문제 해결
  - H3: "샌드박스 도구 정책에 의해 도구 X가 차단됨"
  - H3: "기본 에이전트라고 생각했는데 왜 샌드박스에서 실행되나요?"
  - H2: 관련 문서
...
- 제목:
  ...
  - H2: openclaw 보안 감사
  - H3: 감사에서 검사하는 항목(개요)
  - H3: 발견 사항을 분류할 때의 우선순위
  - H2: 60초 만에 적용하는 강화된 기준선
  - H2: 신뢰 경계 매트릭스
  - H2: 설계상 취약점이 아닌 항목
  - H2: Gateway와 Node 신뢰
  - H2: 위협 모델
  ...
  - H2: 프롬프트 인젝션
  - H3: 외부 콘텐츠와 신뢰할 수 없는 입력 래핑
  - H3: 우회 플래그(프로덕션에서는 끄십시오)
  - H3: 그룹에서의 추론과 상세 출력
  - H2: 명령 권한 부여
  - H2: 제어 플레인 도구
  - H2: Node 실행(system.run)
  - H2: 동적 Skills(감시기/원격 Node)
  - H2: Plugin
  - H2: 샌드박싱
  - H3: 하위 에이전트 위임 가드레일
  - H3: 읽기 전용 모드
  ...
  - H3: 전체 액세스(샌드박스 없음)
  - H3: 읽기 전용 도구 + 읽기 전용 워크스페이스
  - H3: 파일 시스템/셸 액세스 없음(공급자 메시징 허용)
  - H2: 브라우저 제어 위험
  - H3: 브라우저 SSRF 정책(기본적으로 엄격함)
  - H2: 네트워크 노출
  - H3: 바인드, 포트, 방화벽
...
- 제목:
  - H2: 명령 진단 단계
  - H2: 업데이트 후
  - H2: 분리된 설치 상태와 최신 구성 보호
  - H2: 롤백 후 프로토콜 불일치
  - H2: 경로 이탈로 간주되어 건너뛴 Skill 심볼릭 링크
  - H2: 긴 컨텍스트에 추가 사용량이 필요할 때 발생하는 Anthropic 429
  - H2: 업스트림 403 차단 응답
  ...
  - H2: macOS Gateway가 조용히 응답을 멈췄다가 대시보드를 조작하면 재개됨
...
- 제목:
  ...
  - H3: 라이브(실제 제공자 + 실제 모델)
  - H2: 어떤 테스트 모음을 실행해야 합니까?
  - H2: 라이브(네트워크를 사용하는) 테스트
  - H2: Docker 실행기(선택적 "Linux에서 작동" 확인)
  - H2: 문서 무결성 검사
...
- 제목:
  - H2: 처음 60초
  - H2: 어시스턴트의 기능이 제한적이거나 도구가 누락된 경우
  - H2: Anthropic 긴 컨텍스트 429
  - H2: 로컬 OpenAI 호환 백엔드는 직접 실행하면 작동하지만 OpenClaw에서는 실패하는 경우
  - H2: openclaw 확장이 없어 Plugin 설치가 실패하는 경우
  - H2: 설치 정책이 Plugin 설치 또는 업데이트를 차단하는 경우
  - H2: Plugin이 있지만 의심스러운 소유권으로 인해 차단된 경우
  - H2: 의사 결정 트리
  - H2: 관련 문서
...
- 제목:
  - H2: 필요한 바이너리를 이미지에 미리 포함
  - H2: 빌드 및 실행
  - H2: 각 항목이 영속적으로 유지되는 위치
  - H2: 업데이트
  - H2: 관련 문서
...
- 제목:
  - H2: 간편한 방법(CLI가 여전히 설치된 경우)
  - H2: 수동 서비스 제거(CLI가 설치되지 않은 경우)
  - H3: macOS(launchd)
  - H3: Linux(systemd 사용자 단위)
  - H3: Windows(예약된 작업)
...
- 제목:
  ...
  - H2: 자동 감지(기본값)
  - H2: 구성 예시
  - H3: 제공자 및 CLI 폴백(OpenAI + Whisper CLI)
  - H3: 범위 제한을 적용한 제공자 전용
  - H3: 제공자 전용(Deepgram)
  - H3: 제공자 전용(Mistral Voxtral)
  - H3: 제공자 전용(SenseAudio)
  - H3: 채팅에 트랜스크립트 표시(옵트인)
  - H2: 참고 사항 및 제한
  - H3: 상주형 로컬 STT
  - H3: 프록시 환경 지원
  - H2: 그룹에서 멘션 감지
...
- 제목:
  - H2: iOS Node
  - H3: iOS 사용자 설정
  - H3: iOS 명령어(Gateway node.invoke를 통해)
  - H3: iOS 포그라운드 요구 사항
  - H3: CLI 도우미
  - H2: Android Node
  - H3: Android 사용자 설정
  ...
  - H3: Android 명령어(Gateway node.invoke를 통해)
  - H2: macOS 앱
  - H3: macOS 사용자 설정
...
- 제목:
  ...
  - H2: 목표
  - H2: 비목표(v1)
  - H2: 선행 사례(가져올 부분과 반대로 적용할 부분)
  - H2: 아키텍처 결정: 워커에서 루프 실행, Gateway를 통한 추론
  - H2: 구성 요소
  - H3: 1. 환경 상태 머신 및 제공자 계약
  - H3: 2. 워커 부트스트랩: Box에 OpenClaw 설치
  - H3: 3. 전송: 모든 작업을 SSH로 처리
  - H3: 4. 워커 프로토콜(전용, Node 프로토콜과 다름)
  - H3: 5. 세션 백엔드 RPC
  - H3: 6. 워크스페이스 동기화
...
- 제목:
  ...
  - H3: 8. 음성 및 확장된 Android 명령어 인터페이스
...
- 제목:
  ...
  - H3: 권한 부여 시 앱 충돌
  - H3: Gateway가 "Starting..." 상태로 무기한 유지됨
  - H2: 관련 문서
...
- 제목:
  ...
  - H2: macOS 통합 로깅의 비공개 데이터
  - H2: OpenClaw(ai.openclaw)에서 활성화
  - H2: 디버깅 후 비활성화
  - H2: 관련 문서
...
- 제목:
  ...
  - H3: Twilio 설정 확인 실패
  - H3: Twilio 통화가 시작되지만 회의에 진입하지 못함
  - H2: 참고 사항
  - H2: 관련 항목
...
- 제목:
  ...
  - H3: openclaw.plugin.json에서 생성된 메타데이터가 오래됨
  - H3: package.json openclaw.extensions에 ./dist/index.js를 포함해야 함
  - H3: 패키지 'typebox'를 찾을 수 없음
  - H3: 설치 후 도구가 표시되지 않음
  - H2: 함께 보기
...
- 제목:
  ...
  - H2: Gateway RPC
  - H2: 문제 해결
  - H3: 설정 시 Webhook 노출에 실패함
  - H3: 제공자 자격 증명 인증에 실패함
  - H3: 통화는 시작되지만 제공자 Webhook이 도착하지 않음
  - H3: 서명 검증에 실패함
  - H3: Google Meet의 Twilio 참여에 실패함
  - H3: 실시간 통화에서 음성이 나오지 않음
  - H2: 관련 문서
...
- 제목:
  ...
  - H3: 4단계: 트랜스크립트, ACP 스트림, 궤적 및 VFS 이전
  - H3: 5단계: 백업, 복원, Vacuum 및 확인
  - H3: 6단계: 워커 런타임
  - H3: 7단계: 기존 체계 삭제
  - H2: 백업 및 복원
  - H2: 런타임 리팩터링 계획
...
- 제목:
  ...
  - H2: 대상 그룹 프로젝션
  - H2: 전달된 표면의 수렴
  - H2: 재시작, 타임아웃 및 경로 의미 체계
  - H2: 호환성 계획
  - H2: 출시
  - H3: PR 1: 내구성 있는 수명 주기
  - H3: PR 2: 형식화된 작업 및 채널 콜백
  - H3: PR 3: 제어 UI 딥 링크
  - H3: PR 4: 네이티브 클라이언트
  - H3: PR 5: 상위 항목 수명 주기 전파
  - H3: PR 6: 실패 시 차단 동작
  - H3: 후속 작업: 내구성 있는 원격 메시지 정리
  - H2: 테스트
  - H2: 관측 가능성
  - H2: 미결정 사항
...
- 제목:
  ...
  - H3: 제공자 사용량 스냅샷(상태/건전성)
  - H3: Compaction 보호용 요약
  - H3: 모델 스캔/프로브
  - H3: 대화(음성)
  - H3: Skills(서드파티 API)
  - H2: 관련 문서
...
- 제목:
  ...
  - H2: Control UI, TUI 및 확장 기능 레인
  - H2: Gateway 및 E2E
  - H2: 전체 Docker 스위트(pnpm test:docker:all)
  - H3: 주요 Docker 레인
  - H2: 로컬 PR 게이트
  - H2: 테스트 성능 도구
  - H2: 벤치마크
...
- 제목:
  - H2: 구성
  - H3: 사설 CA를 사용하는 HTTPS 프록시 엔드포인트
  - H2: 라우팅 작동 방식
  - H3: Gateway 루프백 모드
  - H3: 컨테이너
  - H2: 관련 프록시 용어
...
- 제목:
  - H2: 문제: 18800 포트에서 Chrome CDP를 시작하지 못함
  - H3: 근본 원인
...
- 제목:
  ...
  - H2: 로컬 제어와 원격 제어
  - H2: Node 브라우저 프록시(구성이 필요 없는 기본값)
...
- 제목:
  ...
  - H2: YOLO 모드(승인 없음)
  - H3: 영구 Gateway 호스트 "프롬프트를 표시하지 않음" 설정
  - H3: 로컬 바로 가기
  - H3: Node 호스트
  - H3: 세션 전용 바로 가기
  - H2: 허용 목록(에이전트별)
...
- 제목:
  ...
  - H2: Firecrawl 검색 구성
  - H2: Firecrawl webfetch 대체 경로 구성
  - H3: 자체 호스팅 Firecrawl
...
- 제목:
  ...
  - H2: 제공자 매핑(공급업체가 기능 영역별로 분리되는 방식)
  - H2: 관련 문서
...
- 제목:
  ...
  - H3: 번들 Plugin 명령
  - H3: Skill 명령
  - H2: /tools: 에이전트가 현재 사용할 수 있는 도구
  - H2: /model: 모델 선택
  - H2: /config: 디스크 내 구성 쓰기
  - H2: /mcp: MCP 서버 구성
  - H2: /debug: 런타임 전용 재정의
