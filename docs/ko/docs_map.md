...
title: 문서 맵
x-i18n:
    generated_at: "2026-07-01T12:52:15Z"
    model: gpt-5.5
...
- 제목:
  ...
  - H2: macOS Gateway가 조용히 응답을 중지했다가 대시보드를 건드리면 다시 시작됨
  - H2: 메모리 사용량이 높을 때 Gateway 종료
  - H2: Gateway가 유효하지 않은 구성을 거부함
  - H2: Gateway 프로브 경고
  - H2: 채널이 연결되었지만 메시지가 흐르지 않음
  - H2: Cron 및 Heartbeat 전달
  - H2: 노드가 페어링되었지만 도구 실패
  - H2: 브라우저 도구 실패
  - H2: 업그레이드 후 무언가 갑자기 중단된 경우
  - H2: 관련 항목
...
- 제목:
  ...
  - H3: 계층 2: Gateway + 개발 에이전트 스모크("@openclaw"이 실제로 수행하는 작업)
...
- 제목:
  ...
  - H2: 권장: 설치 스크립트
  - H2: 대체 설치 방법
  - H3: 로컬 prefix 설치 프로그램(install-cli.sh)
  - H3: npm, pnpm 또는 bun
  - H3: 소스에서 설치
...
- 제목:
  ...
  - H3: 권한 부여 시 앱 충돌
  - H3: Gateway가 계속 "Starting..." 상태로 유지됨
  - H2: 관련 항목
...
- 제목:
  ...
  - H2: 구성 형태
  - H2: 고급 백엔드 훅
  - H3: ownsNativeCompaction: OpenClaw Compaction 사용하지 않기
  - H2: MCP 도구 브리지
  - H2: 사용자 구성
  - H2: 검증
  - H2: 체크리스트
...
- 제목:
  - H2: 개요
  - H2: 스레드 바인딩 및 모델 변경
  - H2: 표시 응답 및 Heartbeat
  - H2: 훅 경계
  - H2: V1 지원 계약
  - H2: 네이티브 권한 및 MCP 요청
  - H2: 큐 조정
  - H2: Codex 피드백 업로드
  - H2: Compaction 및 대화 기록 미러
  - H2: 미디어 및 전달
  - H2: 관련 항목
...
- 제목:
  ...
  - H2: 라이브 테스트 체크리스트
  - H2: 문제 해결
  - H3: Agent가 Google Meet 도구를 볼 수 없음
  - H3: 연결된 Google Meet 지원 Node가 없음
  - H3: 브라우저가 열리지만 Agent가 참여할 수 없음
  - H3: 회의 생성 실패
  - H3: Agent가 참여하지만 말하지 않음
  - H3: Twilio 설정 검사 실패
...
- 제목:
  ...
  - H3: 4단계: 대화 기록, ACP 스트림, 궤적 및 VFS 이동
...
- 제목:
  ...
  - H2: YOLO 모드(승인 없음)
  - H3: 영구 Gateway 호스트 "절대 프롬프트하지 않음" 설정
  - H3: 로컬 바로가기
  - H3: Node 호스트
  - H3: 세션 전용 바로가기
  - H2: 허용 목록(에이전트별)
