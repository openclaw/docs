...
title: 문서 맵
x-i18n:
    generated_at: "2026-07-03T09:27:37Z"
    model: gpt-5.5
...
- 제목:
  ...
  - H2: 비공개 API 작업
  - H2: 구성 쓰기
  - H2: 분할 전송 DM 병합(명령 + URL을 하나의 작성에 포함)
...
- 제목:
  ...
  - H3: 재사용 가능한 발신자 그룹
  - H3: 상태가 저장되는 위치
  - H2: 2) Node 장치 페어링(iOS/Android/macOS/헤드리스 노드)
...
- 제목:
  - H1: openclaw node
  - H2: node host를 사용하는 이유
  - H2: 브라우저 프록시(제로 구성)
  - H2: 실행(포그라운드)
  - H2: node host용 Gateway 인증
  - H2: 서비스(백그라운드)
  - H2: 페어링
  - H2: 실행 승인
  - H2: 관련
...
- 제목:
  ...
  - H2: 주입된 작업 영역 파일(Project Context)
...
- 제목:
  ...
  - H2: Rescue-Bot 빠른 시작
  - H2: 이것이 작동하는 이유
  - H2: --profile rescue onboard 변경 사항
  - H2: 일반 다중 Gateway 설정
  - H2: 격리 체크리스트
  - H2: 포트 매핑(파생)
...
- 제목:
  ...
  - H3: 예시: 파일 시스템/셸 액세스 없음(제공자 메시징 허용)
  - H2: 사고 대응
  - H3: 격리
  - H3: 교체(비밀이 유출되었으면 침해로 간주)
  - H3: 감사
  - H3: 보고서용 수집
  - H2: 비밀 스캐닝
  - H2: 보안 문제 보고
...
- 제목:
  ...
  - H2: macOS gateway가 조용히 응답을 중단한 뒤 대시보드를 터치하면 재개됨
...
- 제목:
  - H2: 첫 60초
  - H2: 어시스턴트가 제한적이거나 도구가 없는 것처럼 느껴짐
  - H2: Anthropic 긴 컨텍스트 429
  - H2: 로컬 OpenAI 호환 백엔드는 직접 작동하지만 OpenClaw에서는 실패
  - H2: Plugin 설치가 누락된 openclaw extensions 때문에 실패
...
- 제목:
  ...
  - H3: 8) 음성 + 확장된 Android 명령 Surface
  - H2: Assistant 진입점
  - H2: 알림 전달
  - H2: 관련 항목
...
- 제목:
  ...
  - H3: 탐색에 영향을 주는 package.json 필드
  - H2: 탐색 우선순위(중복 Plugin ID)
...
- 제목:
  ...
  - H3: package.json openclaw.extensions는 ./dist/index.js를 포함해야 함
