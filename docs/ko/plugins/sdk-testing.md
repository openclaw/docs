...
title: Plugin 테스트
x-i18n:
    generated_at: "2026-07-16T12:57:29Z"
    model: gpt-5.6
...
OpenClaw Plugin의 테스트 유틸리티, 패턴 및 린트 적용에 대한 참고 자료입니다.

<Tip>
  **테스트 예시를 찾고 계십니까?** 사용 방법 가이드에는 완성된 테스트 예시가 포함되어 있습니다.
  [채널 Plugin 테스트](/ko/plugins/sdk-channel-plugins#step-6-test) 및
  [공급자 Plugin 테스트](/ko/plugins/sdk-provider-plugins#step-6-test).
</Tip>

## 테스트 유틸리티

이 하위 경로들은 OpenClaw 자체 번들 Plugin 테스트를 위한 저장소 로컬 소스 진입점입니다.
타사 Plugin에 게시되는 `package.json` 내보내기가 아니며,
Vitest 또는 기타 저장소 전용 테스트 종속성을 가져올 수 있습니다.
...
번들 Plugin 테스트에는 이러한 목적별 하위 경로를 사용하십시오. 이전
`openclaw/plugin-sdk/testing` 배럴은 저장소 로컬 전용이었고 배포 패키지에서
제외되었으며, 이제 제거되었습니다. 레거시 `openclaw/plugin-sdk/test-utils`
별칭은 저장소 로컬로 유지됩니다. `pnpm run lint:plugins:no-extension-test-core-imports`
(`scripts/check-no-extension-test-core-imports.ts`)은 해당 별칭을 새 확장 테스트에서
가져오는 것을 거부합니다.
...
| `expectChannelInboundContextContract`                | 채널 수신 컨텍스트 형태를 검증합니다. `plugin-sdk/channel-contract-testing`에서 가져옵니다.                                                  |
| `installChannelOutboundPayloadContractSuite`         | 채널 송신 페이로드 계약 사례를 설치합니다. `plugin-sdk/channel-contract-testing`에서 가져옵니다.                                       |
...
| `registerSingleProviderPlugin`                       | 로더 스모크 테스트에 하나의 공급자 Plugin을 등록합니다. `plugin-sdk/plugin-test-runtime`에서 가져옵니다.                                         |
| `registerProviderPlugin`                             | 하나의 Plugin에서 모든 공급자 종류를 캡처합니다. `plugin-sdk/plugin-test-runtime`에서 가져옵니다.                                                 |
| `registerProviderPlugins`                            | 여러 Plugin의 공급자 등록을 캡처합니다. `plugin-sdk/plugin-test-runtime`에서 가져옵니다.                                     |
| `requireRegisteredProvider`                          | 공급자 컬렉션에 ID가 포함되어 있는지 검증합니다. `plugin-sdk/plugin-test-runtime`에서 가져옵니다.                                           |
| `createRuntimeEnv`                                   | 모의 CLI/Plugin 런타임 환경을 빌드합니다. `plugin-sdk/plugin-test-runtime`에서 가져옵니다.                                              |
| `createPluginRuntimeMock`                            | 모의 Plugin 런타임 표면을 빌드합니다. `plugin-sdk/plugin-test-runtime`에서 가져옵니다.                                                      |
| `createPluginSetupWizardStatus`                      | 채널 Plugin용 설정 상태 도우미를 빌드합니다. `plugin-sdk/plugin-test-runtime`에서 가져옵니다.                                             |
| `createTestWizardPrompter`                           | 모의 설정 마법사 프롬프터를 빌드합니다. `plugin-sdk/plugin-test-runtime`에서 가져옵니다.                                                       |
| `createRuntimeTaskFlow`                              | 격리된 런타임 작업 흐름 상태를 생성합니다. `plugin-sdk/plugin-test-runtime`에서 가져옵니다.                                                    |
| `runProviderCatalog`                                 | 테스트 종속성을 사용하여 공급자 카탈로그 훅을 실행합니다. `plugin-sdk/plugin-test-runtime`에서 가져옵니다.                                     |
| `resolveProviderWizardOptions`                       | 계약 테스트에서 공급자 설정 마법사의 선택 항목을 해석합니다. `plugin-sdk/plugin-test-runtime`에서 가져옵니다.                                    |
| `resolveProviderModelPickerEntries`                  | 계약 테스트에서 공급자 모델 선택기 항목을 해석합니다. `plugin-sdk/plugin-test-runtime`에서 가져옵니다.                                    |
| `buildProviderPluginMethodChoice`                    | 검증에 사용할 공급자 마법사 선택 항목 ID를 빌드합니다. `plugin-sdk/plugin-test-runtime`에서 가져옵니다.                                            |
| `setProviderWizardProvidersResolverForTest`          | 격리된 테스트에 공급자 마법사 공급자를 주입합니다. `plugin-sdk/plugin-test-runtime`에서 가져옵니다.                                        |
| `describeOpenAIProviderRuntimeContract`              | 공급자 제품군 런타임 계약 검사를 설치합니다. `plugin-sdk/provider-test-contracts`에서 가져옵니다.                                        |
| `expectPassthroughReplayPolicy`                      | 공급자 재생 정책이 공급자 소유 도구와 메타데이터를 그대로 통과하는지 검증합니다. `plugin-sdk/provider-test-contracts`에서 가져옵니다.         |
| `runRealtimeSttLiveTest`                             | 공유 오디오 픽스처를 사용하여 실시간 STT 공급자 라이브 테스트를 실행합니다. `plugin-sdk/provider-test-contracts`에서 가져옵니다.                       |
| `normalizeTranscriptForMatch`                        | 퍼지 검증 전에 라이브 트랜스크립트 출력을 정규화합니다. `plugin-sdk/provider-test-contracts`에서 가져옵니다.                               |
| `expectExplicitVideoGenerationCapabilities`          | 비디오 공급자가 명시적인 생성 모드 기능을 선언하는지 검증합니다. `plugin-sdk/provider-test-contracts`에서 가져옵니다.                   |
| `expectExplicitMusicGenerationCapabilities`          | 음악 공급자가 명시적인 생성/편집 기능을 선언하는지 검증합니다. `plugin-sdk/provider-test-contracts`에서 가져옵니다.                   |
| `mockSuccessfulDashscopeVideoTask`                   | 성공한 DashScope 호환 비디오 작업 응답을 설치합니다. `plugin-sdk/provider-test-contracts`에서 가져옵니다.                          |
| `getProviderHttpMocks`                               | 옵트인 공급자 HTTP/인증 Vitest 모의에 접근합니다. `plugin-sdk/provider-http-test-mocks`에서 가져옵니다.                                         |
| `installProviderHttpMockCleanup`                     | 각 테스트 후 공급자 HTTP/인증 모의를 재설정합니다. `plugin-sdk/provider-http-test-mocks`에서 가져옵니다.                                        |
| `installCommonResolveTargetErrorCases`               | 대상 해석 오류 처리를 위한 공유 테스트 사례입니다. `plugin-sdk/channel-target-testing`에서 가져옵니다.                                  |
...
| `createMockIncomingRequest`                          | 최소 수신 HTTP 요청 객체를 빌드합니다. `plugin-sdk/test-env`에서 가져옵니다.                                                          |
...
| `createProviderUsageFetch`                           | 공급자 사용량 가져오기 픽스처를 빌드합니다. `plugin-sdk/test-env`에서 가져옵니다.                                                                   |
...
| `bundledPluginRoot` / `bundledPluginFile`            | 번들 Plugin 소스 또는 배포판 픽스처 경로를 해석합니다. `plugin-sdk/test-fixtures`에서 가져옵니다.                                              |
| `mockNodeBuiltinModule`                              | 범위가 좁은 Node 내장 Vitest 모의를 설치합니다. `plugin-sdk/test-node-mocks`에서 가져옵니다.                                                       |
...
번들 Plugin 계약 스위트는 테스트 전용 레지스트리, 매니페스트, 공개 아티팩트 및 런타임 픽스처 도우미를 위해 이러한 SDK 테스트 하위 경로도 사용합니다.
번들 OpenClaw 인벤토리에 의존하는 코어 전용 스위트는 대신
`src/plugins/contracts` 아래에 유지됩니다.

### 타입

집중 테스트 하위 경로에서도 테스트 파일에 유용한 타입을 다시 내보냅니다.
...
채널 대상 확인에 표준 오류 사례를 추가하려면
`installCommonResolveTargetErrorCases`을 사용하십시오.
...
직접 작성한 `api` 모의를 `register(api)`에 전달하는 단위 테스트는
OpenClaw의 로더 승인 게이트를 실행하지 않습니다. Plugin이 의존하는 각 등록 표면마다
로더 기반 스모크 테스트를 하나 이상 추가하십시오. 특히 훅과 메모리 같은
독점 기능에 반드시 추가해야 합니다.

필수 메타데이터가 없거나 Plugin이 소유하지 않은 기능 API를 호출하면 실제 로더는
Plugin 등록에 실패합니다. 예를 들어 `api.registerHook(...)`에는 훅 이름이 필요하며,
`api.registerMemoryCapability(...)`에는 Plugin 매니페스트 또는 내보낸
엔트리가 `kind: "memory"`을 선언해야 합니다.

### 런타임 구성 접근 테스트

`openclaw/plugin-sdk/plugin-test-runtime`의 공유 Plugin 런타임 모의를 사용하는 것이 좋습니다.
이 모의의 `runtime.config.loadConfig()` 및 `runtime.config.writeConfigFile(...)`은
기본적으로 예외를 발생시키므로 테스트에서 사용 중단된 호환성 API의 새로운 사용을
감지할 수 있습니다. 테스트가 레거시 호환성 동작을 명시적으로 다루는 경우에만
이 모의를 재정의하십시오.
...
`createPluginRuntimeStore`을 사용하는 코드의 경우 테스트에서 런타임을 모의하십시오.
...
프로토타입 변경보다 인스턴스별 스텁을 사용하는 것이 좋습니다.
...
`scripts/run-additional-boundary-checks.mjs`은 CI에서 일련의 `lint:plugins:*`
가져오기 경계 검사를 실행하며, 각 검사는 로컬에서도 독립적으로 실행할 수 있습니다.

| 명령어                                                        | 적용 사항                                                                                    |
| -------------------------------------------------------------- | ------------------------------------------------------------------------------------------- |
| `pnpm run lint:plugins:no-monolithic-plugin-sdk-entry-imports` | 번들 Plugin은 모놀리식 `openclaw/plugin-sdk` 루트 배럴을 가져올 수 없습니다.             |
| `pnpm run lint:plugins:no-extension-src-imports`               | 프로덕션 확장 파일은 저장소의 `src/**` 트리를 직접 가져올 수 없습니다(`../../src/...`). |
| `pnpm run lint:plugins:no-extension-test-core-imports`         | 확장 테스트 파일은 `plugin-sdk/test-utils` 또는 기타 코어 전용 테스트 헬퍼를 가져올 수 없습니다. |
