...
read_when:
    ...
    - 패키지된 OpenClaw 설치 또는 번들 Plugin 매니페스트를 유지 관리하고 있습니다
...
`openclaw plugins install npm-pack:<path.tgz>`는 로컬 npm-pack tarball에
동일한 관리형 npm 루트를 사용합니다. OpenClaw는 tarball의 npm 메타데이터를 읽고,
복사된 `file:` 의존성으로 관리형 루트에 추가한 뒤, 일반 npm 설치를 실행하고,
설치된 lockfile 메타데이터를 검증한 후에 Plugin을 신뢰합니다.
이는 로컬 pack 아티팩트가 시뮬레이션하는 레지스트리 아티팩트처럼 동작해야 하는
패키지 수락 및 릴리스 후보 증명을 위한 것입니다.
...
런타임 의존성은 관리형 정리 경계 안에 남습니다.

`openclaw/plugin-sdk/*`를 가져오는 Plugin은 `openclaw`를 피어
의존성으로 선언합니다. OpenClaw는 npm이 호스트 패키지의 별도 레지스트리 복사본을
관리형 루트에 설치하도록 허용하지 않습니다. 오래된 호스트 패키지가 이후 Plugin
설치 중 npm 피어 해석에 영향을 줄 수 있기 때문입니다. 관리형 npm 설치는 공유
루트에 대해 npm 피어 해석/구체화를 건너뛰며, OpenClaw는 설치, 업데이트 또는
제거 후 호스트 피어를 선언한 설치 패키지에 대해 Plugin 로컬
`node_modules/openclaw` 링크를 다시 보장합니다.

git 설치는 저장소를 복제하거나 새로 고친 다음 다음을 실행합니다.
...
`node_modules` 해석은 일반 Node 패키지와 같은 방식으로 동작합니다.

## 로컬 Plugin

로컬 Plugin은 개발자가 제어하는 디렉터리로 취급됩니다. OpenClaw는 로컬 Plugin에
대해 `npm install`, `pnpm install` 또는 의존성 복구를 실행하지 않습니다. 로컬
Plugin에 의존성이 있으면 로드하기 전에 해당 Plugin 안에 설치하세요.

서드 파티 TypeScript 로컬 Plugin은 긴급 Jiti 경로를 사용할 수 있습니다. 패키징된
JavaScript Plugin과 번들 내부 Plugin은 Jiti 대신 네이티브 import/require를 통해
로드됩니다.

## 시작 및 다시 로드

Gateway 시작과 구성 다시 로드는 절대 Plugin 의존성을 설치하지 않습니다. 이들은
Plugin 설치 기록을 읽고, 진입점을 계산한 뒤, 로드합니다.

런타임에 의존성이 누락되면 Plugin 로드가 실패하며, 오류는 운영자에게 명시적인
수정 방법을 안내해야 합니다.
...
`doctor --fix`는 레거시 OpenClaw 생성 의존성 상태를 정리하고, 구성에서
...
무거운 런타임 의존성 트리가 없거나, ClawHub/npm의 다운로드 가능한 패키지로
이동되어야 합니다.
...
Plugin 기능은 일반 Plugin으로 패키징하고 서드 파티 Plugin과 동일한
...
워크스페이스 의존성을 사용할 수 있고 편집 내용이 직접 반영됩니다. 소스 체크아웃
개발은 pnpm 전용입니다. 저장소 루트에서 일반 `npm install`을 실행하는 것은 번들
Plugin 의존성을 준비하는 지원되는 방법이 아닙니다.

| 설치 형태                         | 번들 Plugin 위치                     | 의존성 소유자                                                        |
| -------------------------------- | ------------------------------------- | -------------------------------------------------------------------- |
| `npm install -g openclaw`        | 패키지 내부의 빌드된 런타임 트리      | OpenClaw 패키지 및 명시적인 Plugin 설치/업데이트/doctor 흐름         |
| Git 체크아웃과 `pnpm install`    | `extensions/<id>` 워크스페이스 패키지 | 각 Plugin 패키지의 자체 의존성을 포함한 pnpm 워크스페이스            |
| `openclaw plugins install ...`   | 관리형 npm/git/ClawHub Plugin 루트    | Plugin 설치/업데이트 흐름                                            |

## 레거시 정리

이전 OpenClaw 버전은 시작 시점 또는 doctor 복구 중에 번들 Plugin 의존성 루트를
생성했습니다. 현재 doctor 정리는 `--fix`가 사용될 때 이러한 오래된 디렉터리와
심볼릭 링크를 제거합니다. 여기에는 오래된 `plugin-runtime-deps` 루트,
정리된 `plugin-runtime-deps` 대상를 가리키는 전역 Node-prefix 패키지 심볼릭 링크,
`.openclaw-runtime-deps*` 매니페스트, 생성된 Plugin `node_modules`, 설치
스테이지 디렉터리, 패키지 로컬 pnpm 저장소가 포함됩니다. 패키징된 postinstall도
