...
그리고 **Cursor**에서 Plugin을 설치할 수 있습니다. 이를 **번들**이라고 합니다. 번들은
OpenClaw가 Skills, 훅, MCP 도구 같은 네이티브 기능으로 매핑하는 콘텐츠와 메타데이터 팩입니다.

<Info>
  번들은 네이티브 OpenClaw Plugin과 **같지 않습니다**. 네이티브 Plugin은
  인프로세스로 실행되며 모든 기능을 등록할 수 있습니다. 번들은 선택적 기능 매핑과 더 좁은
  신뢰 경계를 가진 콘텐츠 팩입니다.
</Info>

## 번들이 존재하는 이유

유용한 많은 Plugin은 Codex, Claude 또는 Cursor 형식으로 배포됩니다. 작성자가 이를
...
- OpenClaw는 번들 MCP 구성을 유효한 내장 Pi 설정에
  `mcpServers`로 병합합니다
- OpenClaw는 stdio 서버를 실행하거나 HTTP 서버에 연결하여 내장 Pi 에이전트 턴 중에
...
- 번들 MCP 도구 카탈로그는 등록 전에 결정적으로 정렬되므로,
  업스트림 `listTools()` 순서 변경으로 인해 프롬프트 캐시 도구 블록이 불필요하게 흔들리지 않습니다
...
OpenClaw는 번들 MCP 도구를 `serverName__toolName` 형식의
제공자 안전 이름으로 등록합니다. 예를 들어 `"vigil-harbor"` 키의 서버가
`memory_search` 도구를 노출하면 `vigil-harbor__memory_search`로 등록됩니다.

- `A-Za-z0-9_-` 외의 문자는 `-`로 대체됩니다
- 문자가 아닌 것으로 시작하는 조각에는 문자 접두사가 추가되므로, `12306` 같은 숫자
  서버 키도 제공자 안전 도구 접두사가 됩니다
- 서버 접두사는 30자로 제한됩니다
- 전체 도구 이름은 64자로 제한됩니다
- 빈 서버 이름은 `mcp`로 대체됩니다
- 충돌하는 정제된 이름은 숫자 접미사로 구분됩니다
- 최종 노출 도구 순서는 안전 이름 기준으로 결정적이어서 반복되는 Pi
  턴의 캐시가 안정적으로 유지됩니다
- 프로필 필터링은 하나의 번들 MCP 서버에서 온 모든 도구를 `bundle-mcp`가 소유한
  Plugin으로 처리하므로, 프로필 허용 목록과 거부 목록에는 개별 노출 도구 이름 또는
  `bundle-mcp` Plugin 키를 포함할 수 있습니다
...
- 타사 호환 번들은 시작 시 `npm install` 복구를 받지 않습니다. 이러한 번들은
  `openclaw plugins install`을 통해 설치되어야 하며 필요한 모든 것을 설치된
  Plugin 디렉터리에 포함해야 합니다.
- OpenClaw 소유 번들 Plugin은 코어에 가볍게 포함되어 배포되거나
  Plugin 설치 관리자를 통해 다운로드할 수 있습니다. Gateway 시작 시 이들을 위해
  패키지 관리자를 실행하지 않습니다.
- `openclaw doctor --fix`는 레거시 스테이징 의존성 디렉터리를 제거하고,
  구성에서 참조하지만 로컬 Plugin 인덱스에 없는 다운로드 가능 Plugin을 복구할 수 있습니다.
...
이로 인해 번들은 기본적으로 더 안전하지만, 그래도 타사
...
<AccordionGroup>
  ...
  <Accordion title="Claude 훅이 실행되지 않음">
    `hooks/hooks.json`은 감지만 됩니다. 실행 가능한 훅이 필요하면
