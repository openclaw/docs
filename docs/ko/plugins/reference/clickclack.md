---
read_when:
    - clickclack 플러그인을 설치, 구성 또는 감사하고 있습니다.
summary: OpenClaw 메시지를 송수신하기 위한 Clickclack 채널 인터페이스를 추가합니다.
title: Clickclack Plugin
x-i18n:
    generated_at: "2026-07-12T01:05:17Z"
    model: gpt-5.6
    postprocess_version: locale-links-v1
    provider: openai
    source_hash: d939f5291de0510eaac361732f93fb79cac94656ac56a653f3c9e97394c6a9ae
    source_path: plugins/reference/clickclack.md
    workflow: 16
---

# Clickclack Plugin

OpenClaw 메시지를 보내고 받기 위한 Clickclack 채널 인터페이스를 추가합니다.

## 배포

- 패키지: `@openclaw/clickclack`
- 설치 경로: npm; ClawHub: `clawhub:@openclaw/clickclack`

## 인터페이스

채널: clickclack

## 관련 문서

- [clickclack](/ko/channels/clickclack)
