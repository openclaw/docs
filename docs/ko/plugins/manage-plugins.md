...
read_when:
    ...
    - ClawHub과 npm Plugin 배포 중에서 선택하려는 경우
    - Plugin 패키지를 게시하고 있습니다
sidebarTitle: Manage plugins
summary: OpenClaw Plugin 설치, 목록 보기, 제거, 업데이트 및 게시를 위한 빠른 예제
title: Plugin 관리
x-i18n:
...
대부분의 Plugin 워크플로는 몇 가지 명령으로 구성됩니다. 검색, 설치, Gateway 재시작, 확인, 그리고 더 이상 Plugin이 필요하지 않을 때 제거합니다.
...
Plugin 레지스트리에서 발견할 수 있는 항목을 보여 주지만, 이미 실행 중인
Gateway 프로세스가 Plugin 런타임을 가져왔다는 증명은 아닙니다.
...
Plugin이 도구, 훅, 서비스, Gateway 메서드, 또는 Plugin 소유 CLI 명령과
같은 런타임 표면을 등록했다는 증거가 필요할 때는 `inspect --runtime`을
사용하세요.
...
스펙을 전달하면 향후 업데이트를 위해 추적되는 설치 대상이 해당 스펙으로
전환됩니다.
...
릴리스가 없으면 OpenClaw는 기록된 기본/latest 스펙으로 대체합니다. 정확한
버전과 `@rc` 또는 `@beta` 같은 명시적 태그는 유지됩니다.
...
제거는 Plugin의 구성 항목, Plugin 인덱스 기록, 허용/거부 목록 항목, 그리고
해당하는 경우 연결된 로드 경로를 제거합니다. `--keep-files`를 전달하지 않으면
관리형 설치 디렉터리가 제거됩니다.

## Plugin 게시

외부 Plugin을 [ClawHub](https://clawhub.ai), npmjs.com, 또는 둘 모두에
게시할 수 있습니다.

### ClawHub에 게시

ClawHub는 OpenClaw Plugin의 기본 공개 검색 표면입니다. 사용자가 설치 전에
검색 가능한 메타데이터, 버전 기록, 레지스트리 스캔 결과를 볼 수 있게 합니다.
...
간단한 형식도 여전히 ClawHub를 먼저 확인합니다.

### npmjs.com에 게시

네이티브 npm Plugin은 Plugin 매니페스트와 `package.json` OpenClaw 엔트리포인트
메타데이터를 포함해야 합니다.
...
동일한 패키지가 ClawHub에도 제공되는 경우, `npm:`은 ClawHub 조회를 건너뛰고
npm 해석을 강제합니다.

## 소스 선택

- **ClawHub**: OpenClaw 네이티브 검색, 스캔 요약, 버전, 설치 힌트가 필요할 때 사용하세요.
- **npmjs.com**: JavaScript 패키지를 이미 배포하고 있거나 npm dist-tag/비공개 레지스트리 워크플로가 필요할 때 사용하세요.
