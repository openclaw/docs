...
read_when:
    ...
    - '`openclaw tasks flow` 아래의 Task Flow 명령어를 문서화하고 있습니다.'
summary: '`openclaw tasks`의 CLI 참조(백그라운드 작업 원장 및 TaskFlow 상태)'
title: '`openclaw tasks`'
x-i18n:
    generated_at: "2026-07-12T00:43:12Z"
    model: gpt-5.6
...
수명 주기 및 전달 모델은 [백그라운드 작업](/ko/automation/tasks)을 참조하고,
...
Task Flow 레코드를 표시합니다. `cleanupAfter`까지 유지되는 유실 작업은 경고이며,
...
심각도와 트리거에 대한 자세한 내용은 [백그라운드 작업](/ko/automation/tasks)을
참조하세요.
...
작업 및 Task Flow 조정, 정리 시점 기록, 가지치기, 오래된 Cron 실행 세션
레지스트리 정리를 미리 보거나 적용합니다.

Cron 작업의 경우 조정 시 이전 활성 작업을 `lost`로 표시하기 전에 저장된 실행
...
Gateway 실행 컨텍스트가 사라지면 `lost`로 표시됩니다.
...
작업 원장의 지속성 있는 Task Flow 상태를 검사하거나 취소합니다.
