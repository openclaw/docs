...
summary: '`openclaw health` CLI 참조(RPC를 통한 Gateway 상태 스냅샷)'
title: 상태
x-i18n:
    generated_at: "2026-07-12T00:39:08Z"
    model: gpt-5.6
...
| 플래그           | 기본값  | 설명                                                                                         |
| ---------------- | ------- | -------------------------------------------------------------------------------------------- |
| `--json`         | `false` | 텍스트 대신 머신 판독 가능한 JSON을 출력합니다.                                              |
...
- `--verbose`를 사용하지 않으면 Gateway는 캐시된 스냅샷(최대 60초 동안 최신 상태이며 실시간 채널 런타임 상태와 동일함)을 반환하고 다음 호출자를 위해 백그라운드에서 이를 새로 고칠 수 있습니다.
- `--verbose`는 실시간 프로브(채널별 계정 프로브)를 강제하고 Gateway 연결 세부 정보를 출력하며, 기본 에이전트만 표시하는 대신 구성된 모든 계정과 에이전트에 대한 사람이 읽을 수 있는 출력을 확장합니다.
- `--json`은 항상 전체 스냅샷을 반환합니다. 여기에는 채널, 계정별 프로브, Plugin 로드 상태, 컨텍스트 엔진 격리 상태, 모델 가격 책정 캐시 상태, 이벤트 루프 상태 및 에이전트별 세션 저장소가 포함됩니다.

## 관련 항목

- [CLI 참조](/ko/cli)
- [`openclaw status`](/ko/cli/status) — 전체 상태 스냅샷 없이 수행하는 로컬 진단 및 채널 프로브
