...
title: 메시지
x-i18n:
    generated_at: "2026-06-27T17:18:25Z"
    model: gpt-5.5
...
- `openclaw message`는 `--channel` 또는 채널 접두사가 붙은 대상이 있으면 선택된 채널을 해당 채널을 소유한 Plugin으로 확인합니다. 그렇지 않으면 기본 채널 추론을 위해 구성된 채널 Plugin을 로드합니다.
...
- 캐시 미스가 발생하면, 제공자가 지원하는 경우 OpenClaw가 라이브 디렉터리 조회를 시도합니다.

## 공통 플래그

- `--channel <name>`
- `--account <id>`
- `--target <dest>`(보내기/투표/읽기 등의 대상 채널 또는 사용자)
...
- `openclaw message`는 선택된 작업을 실행하기 전에 지원되는 채널 SecretRef를 확인합니다.
- 가능한 경우 확인 범위는 활성 작업 대상에 한정됩니다.
...
- 관련 없는 채널의 확인되지 않은 SecretRef는 대상 지정 메시지 작업을 차단하지 않습니다.
- 선택된 채널/계정 SecretRef가 확인되지 않으면, 해당 작업에 대해 명령이 실패-폐쇄 방식으로 실패합니다.
...
- `send`
  ...
  - 공유 프레젠테이션 페이로드: `--presentation`은 코어가 선택된 채널의 선언된 기능을 통해 렌더링하는 의미론적 블록(`text`, `context`, `divider`, `buttons`, `select`)을 보냅니다. [메시지 프레젠테이션](/ko/plugins/message-presentation)을 참조하세요.
  - 일반 전달 기본 설정: `--delivery`는 `{ "pin": true }` 같은 전달 힌트를 허용합니다. `--pin`은 채널이 지원하는 경우 고정 전달의 축약형입니다.
  - Telegram + WhatsApp: `--force-document`(채널 압축을 피하기 위해 이미지, GIF, 동영상을 문서로 전송)
  - Telegram 전용: `--thread-id`(포럼 주제 ID)
  - Slack 전용: `--thread-id`(스레드 타임스탬프; `--reply-to`는 같은 필드를 사용)
  - Telegram + Discord: `--silent`
  - WhatsApp 전용: `--gif-playback`; WhatsApp Channels/Newsletters는 기본 `@newsletter` JID로 주소 지정됩니다.
...
- `react`
  - 채널: Discord/Google Chat/Matrix/Nextcloud Talk/Signal/Slack/Telegram/WhatsApp
  ...
  - Nextcloud Talk: 반응 추가만 가능; `--remove`는 명확한 오류와 함께 거부됩니다(/tools/reactions 참조)
...
일반 프레젠테이션을 통해 Telegram 인라인 버튼을 보냅니다:
...
일반 프레젠테이션을 통해 Telegram Mini App 버튼을 보냅니다:
...
Telegram 웹 앱 버튼은 사용자와 봇 간의 비공개 채팅에서만 지원됩니다.
`web_app`을 사용하는 이전 JSON 페이로드도 여전히 파싱되지만, `webApp`이
표준 프레젠테이션 필드입니다.

일반 프레젠테이션을 통해 Teams 카드를 보냅니다:
...
압축을 피하기 위해 Telegram 또는 WhatsApp 이미지를 문서로 보냅니다:
