...
- OSC-8 하이퍼링크는 지원되는 곳에서 클릭 가능한 링크로 렌더링됩니다. 그렇지 않으면
  CLI가 일반 URL로 대체합니다.
...
OAuth/API 자격 증명을 사용할 수 있을 때 공급자 사용량/할당량을 표시합니다. 데이터는 공급자 사용량
엔드포인트에서 직접 가져오며 `X% left`로 정규화됩니다. 현재 사용량
창이 있는 공급자: Anthropic, GitHub Copilot, Gemini CLI, OpenAI Codex, MiniMax,
Xiaomi, z.ai.
