...
`openclaw`는 기본 CLI 진입점입니다. 각 핵심 명령에는 전용 참조 페이지가 있거나 별칭 대상 명령과 함께 문서화되어 있습니다. 이 색인은 CLI 전반에 적용되는 명령, 전역 플래그, 출력 스타일 규칙을 나열합니다.

의도에 따라 설정 명령을 사용하세요.

- `openclaw setup`은 전체 안내식 온보딩 흐름을 거치지 않고 기본 구성과 작업 영역을 만듭니다.
- `openclaw onboard`는 Gateway, 모델 인증, 작업 영역, 채널, Skills, 상태 점검을 위한 전체 안내식 최초 실행 경로입니다.
- `openclaw configure`는 모델 인증, Gateway, 채널, Plugin, Skills처럼 기존 설정의 특정 부분을 변경합니다.
- `openclaw channels add`는 기본 설정이 존재한 후 채널 계정을 구성합니다. 안내식 채널 설정에는 플래그 없이 실행하고, 스크립트에는 채널별 플래그와 함께 실행하세요.
...
- `--json`(지원되는 경우 `--plain`도 포함)은 깔끔한 출력을 위해 스타일링을 비활성화합니다.
...
Plugin은 추가 최상위 명령을 더할 수 있습니다(예: `openclaw voicecall`).
