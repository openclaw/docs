...
title: CLI 참조서
x-i18n:
    generated_at: "2026-07-16T12:28:51Z"
    model: gpt-5.6
...
참조 페이지가 있거나 별칭이 가리키는 명령과 함께 문서화되어 있습니다. 이 색인에는
CLI 전반에 적용되는 명령, 전역 플래그 및 출력 스타일 규칙이 나열되어 있습니다.

목적별 설정 명령:

- `openclaw setup` 및 `openclaw onboard`은 먼저 추론을 검증한 다음 Gateway, 워크스페이스, 채널, Skills 및 상태 확인을 설정하기 위해 OpenClaw를 시작합니다.
- `openclaw setup --baseline`은 안내형 온보딩 흐름을 진행하지 않고 기본 구성과 워크스페이스를 생성합니다.
- `openclaw configure`은 기존 설정에서 모델 인증, Gateway, 채널, Plugin 또는 Skills와 같은 특정 부분을 변경합니다.
- `openclaw channels add`은 기본 설정이 존재한 후 채널 계정을 구성합니다. 안내형 설정에는 플래그 없이 실행하고, 스크립트에는 채널별 플래그와 함께 실행하십시오.

## 명령 페이지

| 영역                         | 명령                                                                                                                                                                                                                              |
| ---------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 설정 및 온보딩         | [`openclaw`](/ko/cli/openclaw) · [`setup`](/ko/cli/setup) · [`onboard`](/ko/cli/onboard) · [`configure`](/ko/cli/configure) · [`config`](/ko/cli/config) · [`completion`](/ko/cli/completion) · [`doctor`](/ko/cli/doctor) · [`dashboard`](/ko/cli/dashboard) |
...
| 상태 및 세션          | [`status`](/ko/cli/status) · [`health`](/ko/cli/health) · [`sessions`](/ko/cli/sessions) · [`audit`](/ko/cli/audit)                                                                                                                               |
...
| 네트워크 및 Node            | [`directory`](/ko/cli/directory) · [`nodes`](/ko/cli/nodes) · [`devices`](/ko/cli/devices) · [`node`](/ko/cli/node) · [`worker`](/cli/worker)                                                                                                     |
...
| `--container <name>`    | `<name>`이라는 이름으로 실행 중인 Podman/Docker 컨테이너 내부에서 CLI를 실행합니다(기본값: 환경 변수 `OPENCLAW_CONTAINER`) |
| `--log-level <level>`   | 파일 및 콘솔 출력의 전역 로그 수준을 재정의합니다                                                 |
| `--no-color`            | ANSI 색상을 비활성화합니다(`NO_COLOR=1`도 적용됨)                                                    |
| `--update`              | [`openclaw update`](/ko/cli/update)의 축약형이며, 소스 체크아웃과 패키지 설치 모두에서 작동합니다    |
...
이 맵은 핵심 명령과 주요 하위 명령을 다룹니다. Plugin에서 추가한
하위 명령(예: `skills`, `plugins`, `wiki` 아래)은
독립적으로 변경됩니다. 신뢰할 수 있는 최신 목록을 확인하려면 `<command> --help`을 실행하십시오.
...
- `/status` - 빠른 진단입니다.
- `/trace` - 세션 범위의 Plugin 추적/디버그 줄입니다.
- `/config` - 영구 저장되는 구성 변경 사항입니다.
- `/debug` - 런타임 전용 구성 재정의입니다(메모리에만 저장되고 디스크에는 저장되지 않으며, `commands.debug: true`이 필요합니다).

## 사용량 추적

OAuth/API 자격 증명을 사용할 수 있는 경우 `openclaw status --usage` 및 Control UI에 제공자 사용량/할당량이 표시됩니다. 데이터는 제공자의 사용량 엔드포인트에서 직접 가져오며 `X% left`으로 정규화됩니다. 현재 사용량 기간을 제공하는 제공자는 Anthropic, Gemini CLI, GitHub Copilot, MiniMax, OpenAI Codex, Xiaomi, z.ai입니다.
