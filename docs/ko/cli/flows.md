...
read_when:
    ...
    - 빠르게 TaskFlow를 점검할 수 있는 참조 자료가 필요합니다
summary: '리디렉션: 흐름 명령은 `openclaw tasks flow` 아래에 있습니다.'
title: 플로우(리디렉션)
x-i18n:
    generated_at: "2026-07-12T15:03:47Z"
    model: gpt-5.6
...
`<lookup>`에는 흐름 ID(`list` / `show`가 반환) 또는 흐름의 소유자 키(소유 하위 시스템이 흐름을 추적하는 데 사용하는 안정적인 식별자)를 사용할 수 있습니다.
...
TaskFlow 개념과 작성 방법은 [TaskFlow](/ko/automation/taskflow)를 참조하십시오. 상위 `tasks` 명령은 [tasks CLI 참조](/ko/cli/tasks)를 참조하십시오.
