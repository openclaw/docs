...
summary: Google Lyria, MiniMax 및 ComfyUI 워크플로에서 music_generate를 통해 음악 생성
title: 음악 생성
x-i18n:
...
작업으로 시작하고, 작업 원장에서 추적한 다음, 트랙이 준비되면 에이전트를
다시 깨워 에이전트가 사용자에게 알리고 완성된 오디오를 첨부할 수 있게
합니다. 메시지 도구로만 보이는 전달을 사용하는 그룹/채널 채팅에서는
에이전트가 메시지 도구를 통해 결과를 전달합니다.

<Note>
기본 제공 공유 도구는 하나 이상의 음악 생성 제공자를 사용할 수 있을 때만
나타납니다. 에이전트의 도구에 `music_generate`가 보이지 않으면
  ...
  <Tab title="Shared provider-backed">
    <Steps>
      <Step title="Configure auth">
        하나 이상의 제공자에 대한 API 키를 설정하세요. 예를 들어
        `GEMINI_API_KEY` 또는 `MINIMAX_API_KEY`를 설정합니다.
      </Step>
      ...
      <Step title="Ask the agent">
        ...
        에이전트가 `music_generate`를 자동으로 호출합니다. 도구 허용 목록
        설정은 필요하지 않습니다.
      </Step>
    </Steps>

    세션 기반 에이전트 실행이 없는 직접 동기 컨텍스트에서는 기본 제공
    도구가 여전히 인라인 생성으로 대체되고 도구 결과에 최종 미디어 경로를
    반환합니다.
  ...
  <Tab title="ComfyUI workflow">
    <Steps>
      <Step title="Configure the workflow">
        ...
        `plugins.entries.comfy.config.music`를 구성하세요.
      </Step>
...
| -------- | :--------: | :----: | ---------- | ------------------------------------------------------------------------- |
| ComfyUI  |     ✓      |   ✓    | 이미지 1개    | 공유 스윕에는 포함되지 않음; `extensions/comfy/comfy.live.test.ts`에서 다룸 |
...
<ParamField path="lyrics" type="string">
  제공자가 명시적 가사 입력을 지원할 때 사용할 수 있는 선택적 가사입니다.
</ParamField>
<ParamField path="instrumental" type="boolean">
  제공자가 지원할 때 악기 전용 출력을 요청합니다.
</ParamField>
<ParamField path="image" type="string">
  단일 참조 이미지 경로 또는 URL입니다.
</ParamField>
<ParamField path="images" type="string[]">
  여러 참조 이미지입니다(지원 제공자에서는 최대 10개).
</ParamField>
<ParamField path="durationSeconds" type="number">
  제공자가 길이 힌트를 지원할 때의 목표 길이(초)입니다.
</ParamField>
<ParamField path="format" type='"mp3" | "wav"'>
  제공자가 지원할 때의 출력 형식 힌트입니다.
</ParamField>
<ParamField path="filename" type="string">출력 파일 이름 힌트입니다.</ParamField>
<ParamField path="timeoutMs" type="number">선택적 제공자 요청 제한 시간(밀리초)입니다. 10000ms보다 작은 값은 10000ms로 올려지고 도구 결과에 보고됩니다.</ParamField>

<Note>
모든 제공자가 모든 매개변수를 지원하는 것은 아닙니다. OpenClaw는 제출 전에
입력 개수 같은 하드 제한은 계속 검증합니다. 제공자가 길이를 지원하지만
요청값보다 짧은 최대값을 사용하는 경우, OpenClaw는 가장 가까운 지원
길이로 제한합니다. 선택한 제공자나 모델이 실제로 적용할 수 없는 선택적
힌트는 경고와 함께 무시됩니다. 도구 결과는 적용된 설정을 보고하며,
`details.normalization`은 요청값에서 적용값으로의 매핑을 캡처합니다.
</Note>
...
- **백그라운드 작업:** `music_generate`는 백그라운드 작업을 만들고, 시작됨/작업 응답을
  즉시 반환하며, 나중에 후속 에이전트 메시지로 완성된 트랙을 게시합니다.
- **중복 방지:** 작업이 `queued` 또는 `running` 상태인 동안 같은 세션의 이후
  `music_generate` 호출은 다른 생성을 시작하는 대신 작업 상태를 반환합니다.
...
- **완료 깨우기:** OpenClaw는 내부 완료 이벤트를 같은 세션에 다시 주입하여
  모델이 사용자에게 보이는 후속 메시지를 직접 작성할 수 있게 합니다.
- **프롬프트 힌트:** 같은 세션의 이후 사용자/수동 턴에는 음악 작업이 이미
  진행 중일 때 작은 런타임 힌트가 제공되어 모델이 `music_generate`를
  다시 무작정 호출하지 않게 합니다.
...
4. 인증 기반 제공자 기본값만 사용한 자동 감지:
   - 현재 기본 제공자를 먼저 사용;
   - 나머지 등록된 음악 생성 제공자를 제공자 ID 순서로 사용.
...
<AccordionGroup>
  <Accordion title="ComfyUI">
    워크플로 기반이며 프롬프트/출력 필드에 대해 구성된 그래프와 노드
    매핑에 따라 달라집니다. 번들된 `comfy` Plugin은 음악 생성 제공자
    레지스트리를 통해 공유 `music_generate` 도구에 연결됩니다.
  </Accordion>
  <Accordion title="Google (Lyria 3)">
    Lyria 3 배치 생성을 사용합니다. 현재 번들된 흐름은 프롬프트, 선택적
    가사 텍스트, 선택적 참조 이미지를 지원합니다.
  </Accordion>
  <Accordion title="MiniMax">
    배치 `music_generation` 엔드포인트를 사용합니다. `minimax` API 키 인증
    또는 `minimax-portal` OAuth를 통해 프롬프트, 선택적 가사, 악기 모드,
    길이 조정, mp3 출력을 지원합니다.
  </Accordion>
</AccordionGroup>
...
[ComfyUI](/ko/providers/comfy)를 참조하세요. 공유 제공자 동작을 디버깅하는
경우 [Google (Gemini)](/ko/providers/google) 또는
...
`maxInputImages`, `supportsLyrics`, `supportsFormat` 같은 레거시 평면 필드만으로는
편집 지원을 알리기에 **충분하지 않습니다**. 제공자는 `generate`와 `edit`를
명시적으로 선언해야 라이브 테스트, 계약 테스트, 공유 `music_generate`
도구가 모드 지원을 결정적으로 검증할 수 있습니다.
...
기본적으로 저장된 인증 프로필보다 live/env API 키를 우선하며, 제공자가
편집 모드를 활성화한 경우 `generate`와 선언된 `edit` 커버리지를 모두
실행합니다. 현재 커버리지:

- `google`: `generate` 및 `edit`
- `minimax`: `generate`만
- `comfy`: 별도의 Comfy 라이브 커버리지, 공유 제공자 스윕 아님
