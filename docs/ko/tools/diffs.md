---
read_when:
    - 에이전트가 코드 또는 Markdown 편집 내용을 차이 형식으로 표시하도록 하려는 경우
    - 캔버스에서 바로 사용할 수 있는 뷰어 URL 또는 렌더링된 diff 파일을 원합니다
    - 안전한 기본값을 갖춘 제어 가능한 임시 차이 산출물이 필요합니다
sidebarTitle: Diffs
summary: 에이전트용 읽기 전용 차이 보기 도구 및 파일 렌더러(선택 사항 Plugin 도구)
title: 차이점
x-i18n:
    generated_at: "2026-05-11T20:37:52Z"
    model: gpt-5.5
    provider: openai
