...
OpenClaw는 `web_search` 제공자로 [Exa AI](https://exa.ai/)를 지원합니다. Exa는
...
**환경 대안:** Gateway 환경에 `EXA_API_KEY`를 설정합니다.
Gateway 설치의 경우 `~/.openclaw/.env`에 넣습니다.

## 기본 URL 재정의

Exa 검색 요청이 호환 프록시나 대체 Exa 엔드포인트를 거쳐야 할 때
`plugins.entries.exa.config.webSearch.baseUrl`을 설정합니다. OpenClaw는
기본 호스트 앞에 `https://`를 붙여 정규화하고, 경로가 이미 해당 위치에서 끝나지 않는 한
`/search`를 추가합니다. 해석된 엔드포인트는 검색 캐시 키에 포함되므로
...
- `contents` 옵션이 제공되지 않으면 Exa는 기본적으로 `{ highlights: true }`를 사용하므로
  결과에 핵심 문장 발췌가 포함됩니다.
