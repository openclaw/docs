...
title: ACP 에이전트
x-i18n:
    generated_at: "2026-05-11T20:36:36Z"
    model: gpt-5.5
    provider: openai
...
사용할 수 있습니다. 준비 상태 확인은 `/acp doctor`를 실행하세요.
...
정책은 여전히 디스패치 전에 `acp.allowedAgents`와 모든
...
<AccordionGroup>
  ...
  <Accordion title="네이티브 Codex 라우팅 규칙">
    ...
    Native Codex 대화 바인딩은 기본 채팅 제어 경로입니다.
    OpenClaw 동적 도구는 여전히 OpenClaw를 통해 실행되는 반면,
    shell/apply-patch 같은 Codex 네이티브 도구는 Codex 안에서 실행됩니다.
    ...
    지원 경계는
    [Codex 하니스 v1 지원 계약](/ko/plugins/codex-harness-runtime#v1-support-contract)에 문서화되어 있습니다.
  ...
  <Accordion title="ACP 라우팅 자연어 트리거">
    ...
    OpenClaw는 `runtime: "acp"`를 선택하고, 하니스 `agentId`를 해석하며,
    ...
    `sessions_spawn`의 경우 ACP가 활성화되어 있고, 요청자가 샌드박스 처리되지 않았으며, ACP 런타임
...
세션을 제자리에서 재설정하고, `/acp close`는 바인딩을 제거합니다.
...
비일시적 워크플로의 경우 최상위 `bindings[]` 항목에서
지속성 ACP 바인딩을 구성하세요.
...
<Tabs>
  <Tab title="From sessions_spawn">
    ...
    <Note>
    `runtime`의 기본값은 `subagent`이므로 ACP 세션에는 `runtime: "acp"`를 명시적으로 설정하세요. `agentId`가 생략되면 OpenClaw는 구성된 경우 `acp.defaultAgent`를 사용합니다. `mode: "session"`은 지속적인 바인딩 대화를 유지하기 위해 `thread: true`가 필요합니다.
    </Note>
...
- OpenClaw는 여전히 ACP 기능 게이트, 허용된 에이전트, 세션 소유권, 채널 바인딩 및 Gateway 전달 정책을 적용합니다.
...
대상을 해석할 수 없으면 OpenClaw는 명확한 오류를 반환합니다
...
acpx 하니스 설정(Claude Code / Codex / Gemini CLI
