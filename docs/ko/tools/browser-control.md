...
read_when:
    ...
    - '`openclaw browser` CLI 참조를 찾고 계신가요'
    - 스냅샷과 참조를 사용한 사용자 지정 브라우저 자동화 추가
...
CLI, 그리고 스크립팅 패턴(스냅샷, 참조, 대기, 디버그 흐름)에 대한 참조입니다.

## 제어 API(선택 사항)

로컬 통합 전용으로 Gateway는 작은 local loopback HTTP API를 노출합니다.
...
모든 엔드포인트는 `?profile=<name>`을 받습니다. `POST /start?headless=true`는 저장된
브라우저 구성을 변경하지 않고 로컬 관리 프로필에 대해 일회성 헤드리스 실행을
요청합니다. 연결 전용, 원격 CDP, 기존 세션 프로필은 해당 재정의를 거부합니다.
OpenClaw가 그런 브라우저 프로세스를 실행하지 않기 때문입니다.
...
- 이 독립 실행형 local loopback 브라우저 API는 신뢰된 프록시 또는
  Tailscale Serve ID 헤더를 사용하지 않습니다.
- `gateway.auth.mode`가 `none` 또는 `trusted-proxy`인 경우, 이러한 local loopback 브라우저
  라우트는 ID를 포함하는 해당 모드를 상속하지 않습니다. local loopback 전용으로 유지하세요.
...
일부 기능(탐색/작업/AI 스냅샷/역할 스냅샷, 요소 스크린샷,
...
- 탭별 CDP WebSocket을 사용할 수 있을 때 역할 스타일 접근성 스냅샷(`--interactive`, `--compact`,
  `--depth`, `--efficient`). 이는 검사 및 참조 발견을 위한
  폴백이며, Playwright는 여전히 기본 작업 엔진입니다.
- 탭별 CDP
  WebSocket을 사용할 수 있을 때 관리되는 `openclaw` 브라우저의 페이지 스크린샷
...
Gateway에 핵심 브라우저 런타임 의존성이 누락된 것입니다. OpenClaw를 다시 설치하거나 업데이트한
다음 Gateway를 다시 시작하세요. Docker의 경우 아래와 같이 Chromium
브라우저 바이너리도 설치하세요.
...
`OPENCLAW_HOME_VOLUME` 또는 바인드 마운트를 통해 유지되는지 확인하세요. [Docker](/ko/install/docker)를 참조하세요.

## 작동 방식(내부)

작은 local loopback 제어 서버가 HTTP 요청을 수락하고 CDP를 통해 Chromium 기반 브라우저에 연결합니다. 고급 작업(클릭/입력/스냅샷/PDF)은 CDP 위에서 Playwright를 통해 처리됩니다. Playwright가 없으면 Playwright를 사용하지 않는 작업만 사용할 수 있습니다. 로컬/원격 브라우저와 프로필이 내부에서 자유롭게 교체되는 동안 에이전트는 하나의 안정적인 인터페이스를 봅니다.

## CLI 빠른 참조

모든 명령은 특정 프로필을 대상으로 지정하는 `--browser-profile <name>`과 기계가 읽을 수 있는 출력을 위한 `--json`을 받습니다.
...
- `click`/`type`/등은 `snapshot`의 `ref`가 필요합니다(숫자 `12`, 역할 참조 `e12`, 또는 실행 가능한 ARIA 참조 `ax12`). CSS 선택자는 작업에 대해 의도적으로 지원되지 않습니다. 보이는 뷰포트 위치만 신뢰할 수 있는 대상인 경우 `click-coords`를 사용하세요.
- 다운로드, 추적, 업로드 경로는 OpenClaw 임시 루트로 제한됩니다. `/tmp/openclaw{,/downloads,/uploads}`(폴백: `${os.tmpdir()}/openclaw/...`).
- `upload`는 `--input-ref` 또는 `--element`를 통해 파일 입력을 직접 설정할 수도 있습니다.

OpenClaw가 교체 탭을 증명할 수 있는 경우, 예를 들어 같은 URL이거나 양식 제출 후 단일 이전 탭이
단일 새 탭이 되는 경우, 안정적인 탭 ID와 레이블은 Chromium 원시 대상 교체 후에도 유지됩니다.
원시 대상 ID는 여전히 변동될 수 있으므로 스크립트에서는
`tabs`의 `suggestedTargetId`를 선호하세요.

스냅샷 플래그 한눈에 보기:

- `--format ai`(Playwright 사용 시 기본값): 숫자 참조(`aria-ref="<n>"`)가 있는 AI 스냅샷입니다.
- `--format aria`: `axN` 참조가 있는 접근성 트리입니다. Playwright를 사용할 수 있으면 OpenClaw는 후속 작업에서 사용할 수 있도록 백엔드 DOM ID를 사용해 참조를 라이브 페이지에 바인딩합니다. 그렇지 않으면 출력을 검사 전용으로 취급하세요.
- `--efficient`(또는 `--mode efficient`): 간결한 역할 스냅샷 프리셋입니다. 이를 기본값으로 만들려면 `browser.snapshotDefaults.mode: "efficient"`를 설정하세요([Gateway 구성](/ko/gateway/configuration-reference#browser) 참조).
...
- **AI 스냅샷(숫자 참조)**: `openclaw browser snapshot`(기본값; `--format ai`)
  ...
  - 내부적으로 참조는 Playwright의 `aria-ref`를 통해 확인됩니다.
...
- **역할 스냅샷(`e12` 같은 역할 참조)**: `openclaw browser snapshot --interactive`(또는 `--compact`, `--depth`, `--selector`, `--frame`)
  ...
  - 내부적으로 참조는 `getByRole(...)`(중복 항목에는 `nth()` 추가)를 통해 확인됩니다.
...
- Playwright를 사용할 수 없는 경우에도 ARIA 스냅샷은
  검사에 유용할 수 있지만, 참조가 실행 가능하지 않을 수 있습니다. 작업 참조가 필요할 때는
  `--format ai` 또는 `--interactive`로 다시 스냅샷을 찍으세요.
- 원시 CDP 폴백 경로에 대한 Docker 증명: `pnpm test:docker:browser-cdp-snapshot`은
  CDP로 Chromium을 시작하고, `browser doctor --deep`을 실행하며, 역할
  스냅샷에 링크 URL, 커서로 승격된 클릭 가능 항목, iframe 메타데이터가 포함되는지 확인합니다.
...
- `/act`는 교체 탭을 증명할 수 있을 때 액션으로 트리거된 교체 후 현재 원시 `targetId`를 반환합니다.
  후속 명령에는 안정적인 탭 id/label을 계속 사용하세요.
- 역할 스냅샷을 `--frame`으로 찍은 경우, 다음 역할 스냅샷까지 역할 ref는 해당 iframe으로 범위가 제한됩니다.
- 알 수 없거나 오래된 `axN` ref는 Playwright의 `aria-ref` 셀렉터로 넘어가지 않고 빠르게 실패합니다. 이런 경우
  같은 탭에서 새 스냅샷을 실행하세요.

## 대기 기능 강화

시간/텍스트뿐 아니라 더 많은 대상에 대해 대기할 수 있습니다.
...
3. 그래도 실패하면: `openclaw browser highlight <ref>`로 Playwright가 대상으로 삼는 항목을 확인
4. 페이지가 이상하게 동작하는 경우:
   - `openclaw browser errors --clear`
   - `openclaw browser requests --filter api --clear`
5. 심층 디버깅: trace 기록:
...
- 기기 / viewport:
  - `set device "iPhone 14"`(Playwright 기기 preset)
  - `set viewport 1280 720`

## 보안 및 개인정보

- openclaw 브라우저 프로필에는 로그인된 세션이 포함될 수 있으므로 민감하게 다루세요.
...
- 로그인 및 봇 방지 관련 참고 사항(X/Twitter 등)은 [브라우저 로그인 + X/Twitter 게시](/ko/tools/browser-login)를 참조하세요.
- Gateway/node 호스트는 비공개로 유지하세요(local loopback 또는 tailnet 전용).
- 원격 CDP endpoint는 강력합니다. 터널링하고 보호하세요.
