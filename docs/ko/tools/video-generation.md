...
title: 동영상 생성
x-i18n:
    generated_at: "2026-05-05T06:10:07Z"
    model: gpt-5.5
    provider: openai
...
OpenClaw 에이전트는 텍스트 프롬프트, 참조 이미지 또는 기존 동영상으로부터 동영상을 생성할 수 있습니다. 16개의 제공자 백엔드를 지원하며, 각 백엔드는 서로 다른 모델 옵션, 입력 모드 및 기능 세트를 제공합니다. 에이전트는 구성과 사용 가능한 API 키를 기준으로 적절한 제공자를 자동으로 선택합니다.

<Note>
`video_generate` 도구는 하나 이상의 동영상 생성 제공자를 사용할 수 있을 때만 표시됩니다. 에이전트 도구에 보이지 않는다면 제공자 API 키를 설정하거나 `agents.defaults.videoGenerationModel`을 구성하세요.
</Note>
...
제공자는 이러한 모드의 일부 또는 전부를 지원할 수 있습니다. 도구는 제출 전에 활성 모드를 검증하고 `action=list`에서 지원되는 모드를 보고합니다.
...
<Steps>
  ...
  <Step title="에이전트에게 요청">
    > 해질녘에 친근한 랍스터가 서핑하는 5초짜리 시네마틱 동영상을 생성해 줘.

    에이전트가 `video_generate`를 자동으로 호출합니다. 도구 허용 목록 설정은 필요하지 않습니다.
...
동영상 생성은 비동기 방식입니다. 에이전트가 세션에서 `video_generate`를 호출하면 다음이 수행됩니다.
...
3. 동영상이 준비되면 OpenClaw가 내부 완료 이벤트로 같은 세션을 깨웁니다.
4. 에이전트가 사용자에게 알리고 완성된 동영상을 첨부합니다. 메시지 도구 전용 가시 전달을 사용하는 그룹/채널 채팅에서는 OpenClaw가 직접 게시하는 대신 에이전트가 메시지 도구를 통해 결과를 전달합니다.

작업이 진행 중인 동안 같은 세션에서 중복 `video_generate` 호출이 들어오면 다른 생성을 시작하지 않고 현재 작업 상태를 반환합니다. CLI에서 진행 상황을 확인하려면 `openclaw tasks list` 또는 `openclaw tasks show <taskId>`를 사용하세요.

세션 기반 에이전트 실행 외부의 경우(예: 직접 도구 호출), 도구는 인라인 생성으로 대체되며 같은 턴에서 최종 미디어 경로를 반환합니다.

제공자가 바이트를 반환하면 생성된 동영상 파일은 OpenClaw가 관리하는 미디어 저장소 아래에 저장됩니다. 기본 생성 동영상 저장 한도는 동영상 미디어 제한을 따르며, `agents.defaults.mediaMaxMb`는 더 큰 렌더링을 위해 이 한도를 높입니다. 제공자가 호스팅된 출력 URL도 반환하는 경우, 로컬 영속성이 너무 큰 파일을 거부하더라도 OpenClaw는 작업을 실패시키는 대신 해당 URL을 전달할 수 있습니다.
...
현재 세션에 이미 `queued` 또는 `running` 상태의 동영상 작업이 있으면 `video_generate`는 새 작업을 시작하지 않고 기존 작업 상태를 반환합니다. 새 생성을 트리거하지 않고 명시적으로 확인하려면 `action: "status"`를 사용하세요.
...
| fal                   | `fal-ai/minimax/video-01-live`  |   ✓    | 이미지 1개, Seedance reference-to-video 사용 시 최대 9개 | Seedance reference-to-video 사용 시 최대 3개 동영상 | `FAL_KEY`                                |
...
일부 제공자는 추가 또는 대체 API 키 환경 변수를 허용합니다. 자세한 내용은 개별 [제공자 페이지](#related)를 참조하세요.

런타임에서 사용 가능한 제공자, 모델 및 런타임 모드를 확인하려면 `video_generate action=list`를 실행하세요.

### 기능 매트릭스

`video_generate`, 계약 테스트, 공유 라이브 스윕에서 사용하는 명시적 모드 계약은 다음과 같습니다.
...
| ComfyUI    |     ✓      |       ✓        |       —        | 공유 스윕에는 포함되지 않습니다. 워크플로별 적용 범위는 Comfy 테스트에 있습니다                                                         |
| DeepInfra  |     ✓      |       —        |       —        | `generate`; 네이티브 DeepInfra 동영상 스키마는 번들 계약에서 텍스트-동영상입니다                                                        |
...
| OpenAI     |     ✓      |       ✓        |       ✓        | `generate`, `imageToVideo`; 이 조직/입력 경로에는 현재 제공자 측 inpaint/remix 액세스가 필요하므로 공유 `videoToVideo`는 건너뜁니다     |
| OpenRouter |     ✓      |       ✓        |       —        | `generate`, `imageToVideo`                                                                                                               |
| Qwen       |     ✓      |       ✓        |       ✓        | `generate`, `imageToVideo`; 이 제공자는 원격 `http(s)` 동영상 URL이 필요하므로 `videoToVideo`는 건너뜁니다                              |
| Runway     |     ✓      |       ✓        |       ✓        | `generate`, `imageToVideo`; 선택한 모델이 `runway/gen4_aleph`일 때만 `videoToVideo`가 실행됩니다                                        |
| Together   |     ✓      |       ✓        |       —        | `generate`, `imageToVideo`                                                                                                               |
| Vydra      |     ✓      |       ✓        |       —        | `generate`; 번들된 `veo3`는 텍스트 전용이고 번들된 `kling`은 원격 이미지 URL이 필요하므로 공유 `imageToVideo`는 건너뜁니다              |
...
결합된 비디오 목록과 병렬인 선택적 위치별 역할 힌트입니다.
표준 값: `reference_video`.
</ParamField>
<ParamField path="audioRef" type="string">
단일 참조 오디오(경로 또는 URL). provider가 오디오 입력을 지원할 때
...
<Note>
역할 힌트는 있는 그대로 provider에 전달됩니다. 표준 값은
`VideoGenerationAssetRole` 유니언에서 오지만, provider가 추가 역할
문자열을 허용할 수도 있습니다. `*Roles` 배열은 해당 참조 목록보다
항목이 많으면 안 됩니다. 한 칸 밀린 실수는 명확한 오류와 함께 실패합니다.
...
<ParamField path="aspectRatio" type="string">
  `1:1`, `16:9`, `9:16`, `adaptive` 또는 provider별 값 같은 종횡비 힌트입니다. OpenClaw는 provider별로 지원되지 않는 값을 정규화하거나 무시합니다.
</ParamField>
<ParamField path="resolution" type="string">`480P`, `720P`, `768P`, `1080P`, `4K` 또는 provider별 값 같은 해상도 힌트입니다. OpenClaw는 provider별로 지원되지 않는 값을 정규화하거나 무시합니다.</ParamField>
<ParamField path="durationSeconds" type="number">
  목표 지속 시간(초)입니다(provider가 지원하는 가장 가까운 값으로 반올림됨).
</ParamField>
<ParamField path="size" type="string">provider가 지원할 때의 크기 힌트입니다.</ParamField>
<ParamField path="audio" type="boolean">
  지원되는 경우 출력에서 생성된 오디오를 활성화합니다. `audioRef*`(입력)와는 별개입니다.
</ParamField>
<ParamField path="watermark" type="boolean">지원되는 경우 provider 워터마킹을 전환합니다.</ParamField>

`adaptive`는 provider별 센티널입니다. 기능에 `adaptive`를 선언한
provider에 있는 그대로 전달됩니다(예: BytePlus Seedance는 입력 이미지
크기에서 비율을 자동 감지하는 데 이를 사용). 이를 선언하지 않은 provider는
tool 결과의 `details.ignoredOverrides`를 통해 값을 노출하여 값이 제외되었음을
확인할 수 있게 합니다.

### 고급

<ParamField path="action" type='"generate" | "status" | "list"' default="generate">
  `"status"`는 현재 세션 작업을 반환하고, `"list"`는 provider를 검사합니다.
</ParamField>
...
<ParamField path="timeoutMs" type="number">선택적 provider 작업 타임아웃(밀리초).</ParamField>
<ParamField path="providerOptions" type="object">
  JSON 객체로 된 provider별 옵션(예: `{"seed": 42, "draft": true}`).
  타입이 지정된 스키마를 선언한 provider는 키와 타입을 검증합니다. 알 수 없는
  키나 불일치가 있으면 fallback 중에 해당 후보를 건너뜁니다. 선언된 스키마가
  없는 provider는 옵션을 있는 그대로 받습니다. 각 provider가 허용하는 항목을
  보려면 `video_generate action=list`를 실행하세요.
</ParamField>

<Note>
모든 provider가 모든 매개변수를 지원하는 것은 아닙니다. OpenClaw는 지속 시간을
가장 가까운 provider 지원 값으로 정규화하고, fallback provider가 다른 제어
표면을 노출할 때 크기-종횡비 같은 번역된 지오메트리 힌트를 다시 매핑합니다.
실제로 지원되지 않는 재정의는 최선의 노력 기준으로 무시되며 tool 결과에
경고로 보고됩니다. 너무 많은 참조 입력 같은 엄격한 기능 제한은 제출 전에
실패합니다. tool 결과는 적용된 설정을 보고하며, `details.normalization`은
요청에서 적용까지의 모든 변환을 캡처합니다.
</Note>
...
- 참조 오디오 입력은 확인된 모드를 변경하지 **않습니다**. 이미지/비디오 참조가
  선택한 모드 위에 적용되며, `maxInputAudios`를 선언한 provider에서만
  작동합니다.
...
일부 기능 검사는 tool 경계가 아니라 fallback 계층에서 적용되므로,
기본 provider의 제한을 초과하는 요청도 가능한 fallback에서 계속 실행될 수 있습니다.
...
- 요청에 `providerOptions`가 포함되어 있고 활성 후보가 타입이 지정된
  ...
  옵션을 있는 그대로 받습니다(하위 호환 pass-through). provider는 빈 스키마
  (`capabilities.providerOptions: {}`)를 선언하여 모든 provider 옵션을
  거부할 수 있으며, 이 경우 타입 불일치와 동일하게 건너뜁니다.

요청에서 첫 번째 건너뛰기 이유는 `warn`으로 로그에 남겨 운영자가 기본
provider가 제외된 시점을 볼 수 있게 합니다. 이후 건너뛰기는 긴 fallback 체인이
조용하도록 `debug`로 로그에 남깁니다. 모든 후보를 건너뛰면 집계된 오류에
...
| 작업       | 수행 내용                                                                                                 |
| ---------- | -------------------------------------------------------------------------------------------------------- |
| `generate` | 기본값. 주어진 프롬프트와 선택적 참조 입력에서 비디오를 생성합니다.                                      |
...
1. **`model` tool 매개변수** — agent가 호출에서 지정한 경우.
2. 구성의 **`videoGenerationModel.primary`**.
3. 순서대로 **`videoGenerationModel.fallbacks`**.
4. **자동 감지** — 현재 기본 provider로 시작해, 유효한 인증이 있는 provider를
   알파벳순으로 나머지 provider까지 확인합니다.

provider가 실패하면 다음 후보가 자동으로 시도됩니다. 모든 후보가 실패하면
...
<AccordionGroup>
  <Accordion title="Alibaba">
    DashScope / Model Studio 비동기 endpoint를 사용합니다. 참조 이미지와
  ...
  <Accordion title="BytePlus (1.0)">
    ...
    이미지를 위치 인자로 전달하거나 `role: "first_frame"`을 설정하세요.
    이미지가 제공되면 T2V 모델 ID는 해당하는 I2V variant로 자동 전환됩니다.
  ...
  <Accordion title="BytePlus Seedance 1.5">
    ...
    통합 `content[]` API를 사용합니다. 최대 2개의 입력 이미지를 지원합니다
    (`first_frame` + `last_frame`). 모든 입력은 원격 `https://`
    URL이어야 합니다. 각 이미지에 `role: "first_frame"` / `"last_frame"`을
    설정하거나 이미지를 위치 인자로 전달하세요.
  ...
  <Accordion title="fal">
    장시간 실행되는 작업에 큐 기반 흐름을 사용합니다. OpenClaw는 기본적으로 최대 20분까지
    기다린 뒤 진행 중인 fal 큐 작업을 시간 초과로 처리합니다. 대부분의 fal 비디오 모델은
    단일 이미지 참조를 허용합니다. Seedance 2.0 참조-비디오
    모델은 최대 9개 이미지, 3개 비디오, 3개 오디오 참조를 허용하며,
    전체 참조 파일은 최대 12개입니다.
  </Accordion>
  <Accordion title="Google (Gemini / Veo)">
    하나의 이미지 또는 하나의 비디오 참조를 지원합니다. 생성 오디오 요청은
    Gemini API 경로에서 경고와 함께 무시됩니다. 해당 API가 현재 Veo 비디오 생성의
  ...
  <Accordion title="Vydra">
    인증을 누락시키는 리디렉션을 피하기 위해 `https://www.vydra.ai/api/v1`을
    직접 사용합니다. `veo3`는 텍스트-비디오 전용으로 번들되며, `kling`은
    원격 이미지 URL이 필요합니다.
  </Accordion>
  <Accordion title="xAI">
...
공유 비디오 생성 계약은 단순한 평면 집계 제한 대신 모드별 기능을
지원합니다. 새 제공자 구현은 명시적인 모드 블록을
우선해야 합니다.
...
`maxInputImages` 및 `maxInputVideos` 같은 평면 집계 필드는
변환 모드 지원을 광고하기에 **충분하지 않습니다**. 제공자는
`generate`, `imageToVideo`, `videoToVideo`를 명시적으로 선언해야 하며, 그래야 라이브
테스트, 계약 테스트, 공유 `video_generate` 도구가 모드 지원을
결정적으로 검증할 수 있습니다.

제공자 내 한 모델이 나머지보다 더 넓은 참조 입력 지원을 갖는 경우,
모드 전체 제한을 높이는 대신 `maxInputImagesByModel`, `maxInputVideosByModel` 또는
`maxInputAudiosByModel`을 사용하세요.

## 라이브 테스트

공유 번들 제공자에 대한 옵트인 라이브 범위:
...
이 라이브 파일은 누락된 제공자 환경 변수를 `~/.profile`에서 로드하고, 기본적으로
저장된 인증 프로필보다 live/env API 키를 우선하며, 기본적으로
릴리스에 안전한 스모크를 실행합니다.

- 스윕의 모든 FAL 제외 제공자에 대한 `generate`.
- 1초 lobster 프롬프트.
- `OPENCLAW_LIVE_VIDEO_GENERATION_TIMEOUT_MS`에서 오는 제공자별 작업 한도
  (기본값 `180000`).

FAL은 제공자 측 큐 지연 시간이 릴리스 시간을 지배할 수 있기 때문에
옵트인입니다.
...
`OPENCLAW_LIVE_VIDEO_GENERATION_FULL_MODES=1`을 설정하면 공유 스윕이 로컬 미디어로
...
현재 공유 `videoToVideo` 라이브 레인은 `runway/gen4_aleph`를 선택한 경우에만
`runway`를 포함합니다.
