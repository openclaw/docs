...
title: ACP 에이전트 — 설정
x-i18n:
    generated_at: "2026-05-02T21:14:20Z"
    model: gpt-5.5
    provider: openai
...
app-server 런타임 구성에는 [Codex harness](/ko/plugins/codex-harness)를 사용하세요.
OpenAI API 키 또는 Codex OAuth 모델 제공자 구성에는
...
OpenClaw가 acpx 백엔드를 사용할 때는 acpx 구성에서 사용자 지정 에이전트 별칭을 정의하지 않는 한 `agentId`에 이 값들을 선호하세요.
로컬 Cursor 설치가 여전히 ACP를 `agent acp`로 노출하는 경우, 내장 기본값을 변경하는 대신 acpx 구성에서 `cursor` 에이전트 명령을 재정의하세요.

직접 acpx CLI 사용은 `--agent <command>`를 통해 임의의 어댑터를 대상으로 할 수도 있지만, 그 원시 우회 수단은 acpx CLI 기능입니다(일반적인 OpenClaw `agentId` 경로가 아님).
...
기본적으로 `acpx` Plugin은 Gateway 시작 중 ACP 에이전트를 생성하지 않고
내장 ACP 백엔드를 등록합니다. 명시적 라이브 프로브에는 `/acp doctor`를 실행하세요.
시작 시 Gateway가 구성된 에이전트를 프로브해야 할 때만
`OPENCLAW_ACPX_RUNTIME_STARTUP_PROBE=1`을 설정하세요.
...
- `command`는 절대 경로, 상대 경로(OpenClaw 워크스페이스에서 해석됨) 또는 명령 이름을 허용합니다.
...
자동 설치가 실패해도 Gateway는 정상적으로 시작되며 누락된 종속성을
...
Codex 또는 Claude Code 같은 ACP 에이전트가 memory recall/store 같은 설치된
...
ACP 에이전트에 `cron` 같은 선택된 내장 도구가 필요할 때 별도의 핵심 도구 브리지를 활성화하세요.
...
- 핵심 도구 노출을 명시적이며 기본값 꺼짐으로 유지합니다.

### 런타임 제한 시간 구성

`acpx` Plugin은 기본적으로 내장 런타임 턴에 120초 제한 시간을 사용합니다.
이를 통해 Gemini CLI 같은 느린 하네스가 ACP 시작 및 초기화를 완료할 충분한 시간을 얻습니다.
호스트에 다른 런타임 제한이 필요하면 재정의하세요.
...
`/acp doctor` 또는 옵트인 시작 프로브가 백엔드를 확인할 때 번들된
`acpx` Plugin은 하나의 하네스 에이전트를 프로브합니다. `acp.allowedAgents`가 설정되어 있으면
첫 번째 허용 에이전트가 기본값입니다. 그렇지 않으면 `codex`가 기본값입니다. 배포에서
...
이 ACPX 하네스 권한은 OpenClaw 실행 승인과 별개이며, Claude CLI `--permission-mode bypassPermissions` 같은 CLI 백엔드 공급업체 우회 플래그와도 별개입니다. ACPX `approve-all`은 ACP 세션을 위한 하네스 수준 비상 스위치입니다.
...
권한 프롬프트가 표시되어야 하지만 대화형 TTY를 사용할 수 없는 경우(ACP 세션에서는 항상 해당)에 어떤 일이 발생하는지 제어합니다.

| 값     | 동작                                                                 |
| ------ | -------------------------------------------------------------------- |
| `fail` | `AcpRuntimeError`로 세션을 중단합니다. **(기본값)**                  |
| `deny` | 권한을 조용히 거부하고 계속합니다(우아한 기능 저하).                 |
...
이 값들을 변경한 후 Gateway를 다시 시작하세요.

<Warning>
OpenClaw 기본값은 `permissionMode=approve-reads` 및 `nonInteractivePermissions=fail`입니다. 비대화형 ACP 세션에서 권한 프롬프트를 트리거하는 쓰기 또는 실행은 `AcpRuntimeError: Permission prompt unavailable in non-interactive mode`로 실패할 수 있습니다.

권한을 제한해야 한다면 세션이 충돌하는 대신 우아하게 기능 저하되도록 `nonInteractivePermissions`를 `deny`로 설정하세요.
</Warning>
