...
summary: 발신 답장을 위한 텍스트 음성 변환 — 프로바이더, 페르소나, 슬래시 명령, 채널별 출력
title: 텍스트 음성 변환
x-i18n:
...
OpenClaw는 발신 답장을 **14개의 음성 제공자** 전반에서 오디오로 변환하고 Feishu, Matrix, Telegram, WhatsApp에서는 네이티브 음성 메시지로, 그 외 모든 곳에서는 오디오 첨부 파일로, telephony 및 Talk에서는 PCM/Ulaw 스트림으로 전달할 수 있습니다.
...
OpenClaw는 registry 자동 선택 순서에서 처음 설정된 제공자를 선택합니다.
기본 제공 `tts` 에이전트 도구는 명시적 의도 전용입니다. 일반 채팅은 사용자가
오디오를 요청하거나, `/tts`를 사용하거나, Auto-TTS/지시어 음성을 활성화하지 않는 한
텍스트로 유지됩니다.
</Note>
...
| 제공자          | 인증                                                                                                             | 참고                                                                   |
| ----------------- | ---------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------- |
| **Azure Speech**  | `AZURE_SPEECH_KEY` + `AZURE_SPEECH_REGION` (`AZURE_SPEECH_API_KEY`, `SPEECH_KEY`, `SPEECH_REGION`도 가능)          | 네이티브 Ogg/Opus 음성 메모 출력 및 telephony.                        |
| **DeepInfra**     | `DEEPINFRA_API_KEY`                                                                                              | OpenAI 호환 TTS. 기본값은 `hexgrad/Kokoro-82M`입니다.                |
| **ElevenLabs**    | `ELEVENLABS_API_KEY` 또는 `XI_API_KEY`                                                                             | 음성 복제, 다국어, `seed`를 통한 결정적 출력.                  |
| **Google Gemini** | `GEMINI_API_KEY` 또는 `GOOGLE_API_KEY`                                                                             | Gemini API TTS; `promptTemplate: "audio-profile-v1"`를 통한 페르소나 인식. |
| **Gradium**       | `GRADIUM_API_KEY`                                                                                                | 음성 메모 및 telephony 출력.                                        |
| **Inworld**       | `INWORLD_API_KEY`                                                                                                | 스트리밍 TTS API. 네이티브 Opus 음성 메모 및 PCM telephony.            |
...
여러 제공자가 설정된 경우 선택된 제공자가 먼저 사용되고 나머지는
대체 옵션이 됩니다. 자동 요약은 `summaryModel`(또는
`agents.defaults.model.primary`)을 사용하므로, 요약을 활성화 상태로 유지하는 경우
해당 제공자도 인증되어 있어야 합니다.

<Warning>
번들 제공 **Microsoft** 제공자는 `node-edge-tts`를 통해 Microsoft Edge의 온라인 neural TTS
서비스를 사용합니다. 공개된 SLA나 할당량이 없는 공개 웹 서비스이므로
최선 노력 방식으로 취급하세요. 레거시 제공자 id `edge`는
`microsoft`로 정규화되며 `openclaw doctor --fix`는 저장된 설정을 다시 씁니다.
새 설정은 항상 `microsoft`를 사용해야 합니다.
</Warning>

## 설정

TTS 설정은 `~/.openclaw/openclaw.json`의 `messages.tts` 아래에 있습니다. 프리셋을
선택하고 제공자 블록을 조정하세요.
...
하나의 에이전트가 다른 제공자, 음성, 모델, 페르소나 또는 Auto-TTS 모드로
말해야 할 때는 `agents.list[].tts`를 사용하세요. 에이전트 블록은
`messages.tts` 위에 깊은 병합을 수행하므로 제공자 자격 증명은 전역 제공자 설정에
그대로 둘 수 있습니다.
...
에이전트별 페르소나를 고정하려면 제공자 설정과 함께 `agents.list[].tts.persona`를
설정하세요. 이는 해당 에이전트에 대해서만 전역 `messages.tts.persona`를 재정의합니다.

자동 답장, `/tts audio`, `/tts status`, 그리고
...
이전 계층 위에 심층 병합되므로, 공유 프로바이더 자격 증명은
`messages.tts`에 유지하면서 채널이나 봇 계정은 음성, 모델, 페르소나,
...
**페르소나**는 프로바이더 전반에 결정적으로 적용할 수 있는 안정적인 음성 정체성입니다.
하나의 프로바이더를 선호하고, 프로바이더 중립적인 프롬프트 의도를 정의하며,
음성, 모델, 프롬프트 템플릿, 시드, 음성 설정에 대한 프로바이더별 바인딩을 담을 수 있습니다.
...
각 프로바이더 시도마다 OpenClaw는 다음 순서로 구성을 병합합니다.
...
### 프로바이더가 페르소나 프롬프트를 사용하는 방식

페르소나 프롬프트 필드(`profile`, `scene`, `sampleContext`, `style`, `accent`,
`pacing`, `constraints`)는 **프로바이더 중립적**입니다. 각 프로바이더가
이를 어떻게 사용할지 결정합니다.

<AccordionGroup>
  <Accordion title="Google Gemini">
    유효한 Google 프로바이더 구성이 `promptTemplate: "audio-profile-v1"`
    ...
    `[whispers]` 또는 `[laughs]` 같은 인라인 오디오 태그는 Gemini 전사 안에 보존됩니다.
    OpenClaw는 이러한 태그를 생성하지 않습니다.
  </Accordion>
  <Accordion title="OpenAI">
    명시적인 OpenAI `instructions`가 구성되어 있지 않은 경우에만
    페르소나 프롬프트 필드를 요청 `instructions` 필드에 매핑합니다.
    명시적인 `instructions`가 항상 우선합니다.
  </Accordion>
  <Accordion title="기타 프로바이더">
    `personas.<id>.providers.<provider>` 아래의 프로바이더별 페르소나 바인딩만 사용합니다.
    프로바이더가 자체 페르소나 프롬프트 매핑을 구현하지 않는 한
    페르소나 프롬프트 필드는 무시됩니다.
  </Accordion>
</AccordionGroup>
...
`fallbackPolicy`는 페르소나에 시도된 프로바이더에 대한 **바인딩이 없을 때**의 동작을 제어합니다.

| 정책                | 동작                                                                                                                                             |
| ------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------ |
| `preserve-persona`  | **기본값.** 프로바이더 중립 프롬프트 필드는 계속 사용할 수 있으며, 프로바이더가 이를 사용하거나 무시할 수 있습니다.                              |
| `provider-defaults` | 해당 시도의 프롬프트 준비에서 페르소나가 생략됩니다. 프로바이더는 중립 기본값을 사용하며 다른 프로바이더로의 대체는 계속됩니다.                 |
| `fail`              | `reasonCode: "not_configured"` 및 `personaBinding: "missing"`으로 해당 프로바이더 시도를 건너뜁니다. 대체 프로바이더는 여전히 시도됩니다.        |

전체 TTS 요청은 시도한 **모든** 프로바이더가 건너뛰어지거나 실패한 경우에만 실패합니다.

## 모델 기반 지시문

기본적으로 어시스턴트는 단일 응답의 음성, 모델, 속도를 재정의하는
...
스트리밍 블록 전달은 인접한 블록에 나뉘어 있어도 채널이 보기 전에
표시 텍스트에서 지시문을 제거합니다.

`modelOverrides.allowProvider: true`가 아니면 `provider=...`는 무시됩니다.
응답이 `provider=...`를 선언하면 해당 지시문의 다른 키는
그 프로바이더만 파싱합니다. 지원되지 않는 키는 제거되고 TTS
지시문 경고로 보고됩니다.

**사용 가능한 지시문 키:**

- `provider`(등록된 프로바이더 ID, `allowProvider: true` 필요)
...
**다른 조정값은 구성 가능하게 유지하면서 프로바이더 전환 허용:**
...
단일 명령 `/tts`. Discord에서는 `/tts`가 내장 Discord 명령이기 때문에
OpenClaw가 `/voice`도 등록합니다. 텍스트 `/tts ...`는 여전히 동작합니다.
...
<Note>
명령은 인증된 발신자(허용 목록/소유자 규칙 적용)가 필요하며,
...
- `/tts latest`는 현재 세션 전사에서 최신 어시스턴트 응답을 읽고 오디오로 한 번 전송합니다. 중복 음성 전송을 억제하기 위해 세션 항목에 해당 응답의 해시만 저장합니다.
- `/tts audio`는 일회성 오디오 응답을 생성합니다(TTS를 켜지 **않음**).
- `limit` 및 `summary`는 기본 구성 파일이 아니라 **로컬 환경설정**에 저장됩니다.
- `/tts status`에는 최신 시도에 대한 대체 진단이 포함됩니다. `Fallback: <primary> -> <used>`, `Attempts: ...`, 그리고 시도별 세부 정보(`provider:outcome(reasonCode) latency`)입니다.
- `/status`는 TTS가 활성화된 경우 활성 TTS 모드와 구성된 프로바이더, 모델, 음성, 정리된 사용자 지정 엔드포인트 메타데이터를 표시합니다.

## 사용자별 환경설정

슬래시 명령은 로컬 재정의를 `prefsPath`에 기록합니다. 기본값은
`~/.openclaw/settings/tts.json`입니다. `OPENCLAW_TTS_PREFS` env var
또는 `messages.tts.prefsPath`로 재정의합니다.

| 저장된 필드 | 효과                                         |
| ------------ | -------------------------------------------- |
| `auto`       | 로컬 자동 TTS 재정의(`always`, `off`, …)     |
| `provider`   | 로컬 기본 프로바이더 재정의                  |
| `persona`    | 로컬 페르소나 재정의                         |
| `maxLength`  | 요약 임계값(기본값 `1500`자)                 |
| `summarize`  | 요약 토글(기본값 `true`)                     |

이는 `messages.tts`의 유효 구성과 해당 호스트의 활성
`agents.list[].tts` 블록을 재정의합니다.

## 출력 형식(고정)

TTS 음성 전달은 채널 기능에 의해 구동됩니다. 채널 Plugin은
음성 스타일 TTS가 프로바이더에 네이티브 `voice-note` 대상을 요청해야 하는지,
또는 일반 `audio-file` 합성을 유지하고 호환되는 출력만 음성 전달용으로 표시해야 하는지를 알립니다.
...
- **Feishu / WhatsApp**: 음성 메모 답장이 MP3/WebM/WAV/M4A
  ...
  `ffmpeg`로 이를 48kHz Ogg/Opus로 트랜스코딩합니다. WhatsApp은
  결과물을 Baileys `audio` 페이로드를 통해 `ptt: true` 및
  `audio/ogg; codecs=opus`로 보냅니다. 변환에 실패하면 Feishu는 원본
  파일을 첨부 파일로 받습니다. WhatsApp 전송은 호환되지 않는
  PTT 페이로드를 게시하는 대신 실패합니다.
- **BlueBubbles**: 공급자 합성은 일반 오디오 파일 경로에 유지합니다. MP3
...
- **xAI**: 기본적으로 MP3를 사용합니다. `responseFormat`은 `mp3`, `wav`, `pcm`, `mulaw` 또는 `alaw`일 수 있습니다. OpenClaw는 xAI의 배치 REST TTS 엔드포인트를 사용하며 완전한 오디오 첨부 파일을 반환합니다. 이 공급자 경로에서는 xAI의 스트리밍 TTS WebSocket을 사용하지 않습니다. 네이티브 Opus 음성 메모 형식은 이 경로에서 지원되지 않습니다.
- **Microsoft**: `microsoft.outputFormat`을 사용합니다(기본값 `audio-24khz-48kbitrate-mono-mp3`).
  ...
  - Telegram `sendVoice`는 OGG/MP3/M4A를 받습니다. Opus 음성 메시지를
    보장해야 하는 경우 OpenAI/ElevenLabs를 사용하세요.
...
- 답장에 이미 미디어 또는 `MEDIA:` 지시문이 포함되어 있으면 TTS를 건너뜁니다.
- 매우 짧은 답장(10자 미만)을 건너뜁니다.
- 요약이 활성화되어 있으면 `summaryModel`(또는 `agents.defaults.model.primary`)을 사용하여
  긴 답장을 요약합니다.
- 생성된 오디오를 답장에 첨부합니다.
- `mode: "final"`에서는 텍스트 스트림이 완료된 후 스트리밍된 최종 답장에도
  오디오 전용 TTS를 계속 보냅니다. 생성된 미디어는 일반 답장 첨부 파일과 동일한
  채널 미디어 정규화를 거칩니다.

답장이 `maxLength`를 초과하고 요약이 꺼져 있거나(또는 요약 모델에 대한 API 키가
없으면), 오디오는 건너뛰고 일반 텍스트 답장이 전송됩니다.
...
## 채널별 출력 형식
  ...
  | 대상                                  | 형식                                                                                                                                       |
  | ------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------ |
  | Feishu / Matrix / Telegram / WhatsApp | 음성 메모 답장은 **Opus**(ElevenLabs의 `opus_48000_64`, OpenAI의 `opus`)를 우선 사용합니다. 48 kHz / 64 kbps는 선명도와 크기의 균형이 좋습니다. |
  | 기타 채널                             | **MP3**(ElevenLabs의 `mp3_44100_128`, OpenAI의 `mp3`). 44.1 kHz / 128 kbps는 음성의 기본값입니다.                                             |
  | Talk / 전화                           | 제공자 네이티브 **PCM**(Inworld 22050 Hz, Google 24 kHz) 또는 전화용 Gradium의 `ulaw_8000`입니다.                                             |

  제공자별 참고 사항:

  - **Feishu / WhatsApp 트랜스코딩:** 음성 메모 답장이 MP3/WebM/WAV/M4A로 도착하면 채널 Plugin이 `ffmpeg`로 48 kHz Ogg/Opus로 트랜스코딩합니다. WhatsApp은 Baileys를 통해 `ptt: true` 및 `audio/ogg; codecs=opus`로 전송합니다. 변환이 실패하는 경우: Feishu는 원본 파일 첨부로 폴백하고, WhatsApp은 호환되지 않는 PTT 페이로드를 게시하는 대신 전송에 실패합니다.
  - **MiniMax / Xiaomi MiMo:** 기본 MP3(MiniMax `speech-2.8-hd`의 경우 32 kHz)입니다. 음성 메모 대상에는 `ffmpeg`를 통해 48 kHz Opus로 트랜스코딩됩니다.
  - **로컬 CLI:** 구성된 `outputFormat`을 사용합니다. 음성 메모 대상은 Ogg/Opus로 변환되고, 전화 출력은 원시 16 kHz 모노 PCM으로 변환됩니다.
  - **Google Gemini:** 원시 24 kHz PCM을 반환합니다. OpenClaw는 첨부 파일용으로 WAV로 감싸고, 음성 메모 대상용으로 48 kHz Opus로 트랜스코딩하며, Talk/전화용으로는 PCM을 직접 반환합니다.
  - **Inworld:** MP3 첨부 파일, 네이티브 `OGG_OPUS` 음성 메모, Talk/전화용 원시 `PCM` 22050 Hz입니다.
  - **xAI:** 기본값은 MP3입니다. `responseFormat`은 `mp3|wav|pcm|mulaw|alaw`일 수 있습니다. xAI의 배치 REST 엔드포인트를 사용하며, 스트리밍 WebSocket TTS는 사용하지 **않습니다**. 네이티브 Opus 음성 메모 형식은 지원되지 **않습니다**.
  - **Microsoft:** `microsoft.outputFormat`을 사용합니다(기본값 `audio-24khz-48kbitrate-mono-mp3`). Telegram `sendVoice`는 OGG/MP3/M4A를 허용합니다. 보장된 Opus 음성 메시지가 필요하면 OpenAI/ElevenLabs를 사용하세요. 구성된 Microsoft 형식이 실패하면 OpenClaw는 MP3로 다시 시도합니다.
  ...
  <Accordion title="최상위 messages.tts.*">
    ...
    <ParamField path="provider" type="string">
      음성 제공자 ID입니다. 설정되지 않은 경우 OpenClaw는 레지스트리 자동 선택 순서에서 첫 번째로 구성된 제공자를 사용합니다. 레거시 `provider: "edge"`는 `openclaw doctor --fix`에 의해 `"microsoft"`로 다시 작성됩니다.
    </ParamField>
    ...
    <ParamField path="summaryModel" type="string">
      자동 요약용 저렴한 모델입니다. 기본값은 `agents.defaults.model.primary`입니다. `provider/model` 또는 구성된 모델 별칭을 허용합니다.
    </ParamField>
    <ParamField path="modelOverrides" type="object">
      모델이 TTS 지시문을 내보내도록 허용합니다. `enabled`의 기본값은 `true`이고, `allowProvider`의 기본값은 `false`입니다.
    </ParamField>
    <ParamField path="providers.<id>" type="object">
      음성 제공자 ID를 키로 하는 제공자 소유 설정입니다. 레거시 직접 블록(`messages.tts.openai`, `.elevenlabs`, `.microsoft`, `.edge`)은 `openclaw doctor --fix`에 의해 다시 작성됩니다. `messages.tts.providers.<id>`만 커밋하세요.
    </ParamField>
    <ParamField path="maxTextLength" type="number">
      TTS 입력 문자 수의 엄격한 상한입니다. 초과하면 `/tts audio`가 실패합니다.
    </ParamField>
  ...
  <Accordion title="ElevenLabs">
    ...
    <ParamField path="voiceSettings" type="object">
      `stability`, `similarityBoost`, `style`(각각 `0..1`), `useSpeakerBoost`(`true|false`), `speed`(`0.5..2.0`, `1.0` = 보통)입니다.
    </ParamField>
    <ParamField path="applyTextNormalization" type='"auto" | "on" | "off"'>텍스트 정규화 모드입니다.</ParamField>
    <ParamField path="languageCode" type="string">2글자 ISO 639-1입니다(예: `en`, `de`).</ParamField>
    <ParamField path="seed" type="number">최선 노력 결정성을 위한 정수 `0..4294967295`입니다.</ParamField>
  ...
  <Accordion title="Google Gemini">
    ...
    <ParamField path="speakerName" type="string">프롬프트가 이름이 지정된 화자를 사용할 때 발화 텍스트 앞에 추가되는 선택적 화자 레이블입니다.</ParamField>
    <ParamField path="promptTemplate" type='"audio-profile-v1"'>활성 페르소나 프롬프트 필드를 결정적인 Gemini TTS 프롬프트 구조로 감싸려면 `audio-profile-v1`로 설정하세요.</ParamField>
  ...
  <Accordion title="Gradium">
    ...
    <ParamField path="baseUrl" type="string">기본값은 `https://api.gradium.ai`입니다.</ParamField>
    <ParamField path="voiceId" type="string">기본값은 Emma(`YTpq7expH9539ERJ`)입니다.</ParamField>
  </Accordion>
  ...
  <Accordion title="Inworld">
    ...
    <ParamField path="modelId" type="string">기본값은 `inworld-tts-1.5-max`입니다. 그 외: `inworld-tts-1.5-mini`, `inworld-tts-1-max`, `inworld-tts-1`.</ParamField>
    <ParamField path="voiceId" type="string">기본값은 `Sarah`입니다.</ParamField>
  ...
  <Accordion title="Local CLI (tts-local-cli)">
    ...
    <ParamField path="outputFormat" type='"mp3" | "opus" | "wav"'>예상 CLI 출력 형식입니다. 오디오 첨부 파일의 기본값은 `mp3`입니다.</ParamField>
    ...
    <ParamField path="env" type="Record<string, string>">명령에 대한 선택적 환경 재정의입니다.</ParamField>
  </Accordion>

  <Accordion title="Microsoft (no API key)">
    <ParamField path="enabled" type="boolean" default="true">Microsoft 음성 사용을 허용합니다.</ParamField>
    <ParamField path="voice" type="string">Microsoft neural voice 이름(예: `en-US-MichelleNeural`)입니다.</ParamField>
    <ParamField path="lang" type="string">언어 코드(예: `en-US`)입니다.</ParamField>
    <ParamField path="outputFormat" type="string">Microsoft 출력 형식입니다. 기본값은 `audio-24khz-48kbitrate-mono-mp3`입니다. 번들 Edge 기반 전송에서 모든 형식을 지원하지는 않습니다.</ParamField>
    <ParamField path="rate / pitch / volume" type="string">퍼센트 문자열(예: `+10%`, `-5%`)입니다.</ParamField>
    <ParamField path="saveSubtitles" type="boolean">오디오 파일 옆에 JSON 자막을 씁니다.</ParamField>
    ...
    <ParamField path="edge.*" type="object" deprecated>레거시 별칭입니다. 유지된 구성을 `providers.microsoft`로 다시 쓰려면 `openclaw doctor --fix`를 실행하세요.</ParamField>
  </Accordion>

  <Accordion title="MiniMax">
    <ParamField path="apiKey" type="string">`MINIMAX_API_KEY`로 폴백합니다. Token Plan 인증은 `MINIMAX_OAUTH_TOKEN`, `MINIMAX_CODE_PLAN_KEY` 또는 `MINIMAX_CODING_API_KEY`를 사용합니다.</ParamField>
  ...
  <Accordion title="OpenAI">
    ...
    <ParamField path="model" type="string">OpenAI TTS 모델 ID(예: `gpt-4o-mini-tts`)입니다.</ParamField>
    <ParamField path="voice" type="string">음성 이름(예: `alloy`, `cedar`)입니다.</ParamField>
    <ParamField path="instructions" type="string">명시적 OpenAI `instructions` 필드입니다. 설정된 경우 페르소나 프롬프트 필드는 자동으로 매핑되지 **않습니다**.</ParamField>
    <ParamField path="extraBody / extra_body" type="Record<string, unknown>">생성된 OpenAI TTS 필드 뒤에 `/audio/speech` 요청 본문으로 병합되는 추가 JSON 필드입니다. `lang` 같은 공급자별 키가 필요한 Kokoro와 같은 OpenAI 호환 엔드포인트에 사용하세요. 안전하지 않은 프로토타입 키는 무시됩니다.</ParamField>
  ...
  <Accordion title="OpenRouter">
    ...
    <ParamField path="speed" type="number">공급자 네이티브 속도 재정의입니다.</ParamField>
  </Accordion>

  <Accordion title="Volcengine (BytePlus Seed Speech)">
    <ParamField path="apiKey" type="string">Env: `VOLCENGINE_TTS_API_KEY` 또는 `BYTEPLUS_SEED_SPEECH_API_KEY`.</ParamField>
    <ParamField path="resourceId" type="string">기본값은 `seed-tts-1.0`입니다. Env: `VOLCENGINE_TTS_RESOURCE_ID`. 프로젝트에 TTS 2.0 권한이 있는 경우 `seed-tts-2.0`을 사용하세요.</ParamField>
    <ParamField path="appKey" type="string">앱 키 헤더입니다. 기본값은 `aGjiRDfUWi`입니다. Env: `VOLCENGINE_TTS_APP_KEY`.</ParamField>
    <ParamField path="baseUrl" type="string">Seed Speech TTS HTTP 엔드포인트를 재정의합니다. Env: `VOLCENGINE_TTS_BASE_URL`.</ParamField>
    <ParamField path="voice" type="string">음성 유형입니다. 기본값은 `en_female_anna_mars_bigtts`입니다. Env: `VOLCENGINE_TTS_VOICE`.</ParamField>
    <ParamField path="speedRatio" type="number">공급자 네이티브 속도 비율입니다.</ParamField>
  ...
  <Accordion title="xAI">
    ...
    <ParamField path="voiceId" type="string">기본값은 `eve`입니다. 라이브 음성: `ara`, `eve`, `leo`, `rex`, `sal`, `una`.</ParamField>
  ...
  <Accordion title="Xiaomi MiMo">
    ...
    <ParamField path="style" type="string">사용자 메시지로 전송되는 선택적 자연어 스타일 지침입니다. 음성으로 말해지지는 않습니다.</ParamField>
  </Accordion>
</AccordionGroup>

## 에이전트 도구

`tts` 도구는 텍스트를 음성으로 변환하고 응답 전달용 오디오 첨부 파일을
반환합니다. Feishu, Matrix, Telegram, WhatsApp에서는 오디오가
파일 첨부가 아니라 음성 메시지로 전달됩니다. Feishu와
WhatsApp은 `ffmpeg`를 사용할 수 있을 때 이 경로에서 Opus가 아닌 TTS 출력을
트랜스코딩할 수 있습니다.

WhatsApp은 Baileys를 통해 PTT 음성 노트(`ptt: true`가 포함된 `audio`)로 오디오를 보내며,
클라이언트가 음성 노트의 캡션을 일관되게 렌더링하지 않기 때문에 표시되는 텍스트는 PTT 오디오와 **별도로** 보냅니다.

이 도구는 선택적 `channel` 및 `timeoutMs` 필드를 받습니다. `timeoutMs`는
