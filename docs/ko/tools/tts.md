...
summary: 발신 답장을 위한 텍스트 음성 변환 — 프로바이더, 페르소나, 슬래시 명령어, 채널별 출력
title: 텍스트 음성 변환
x-i18n:
    generated_at: "2026-05-02T21:17:08Z"
    model: gpt-5.5
    provider: openai
...
OpenClaw는 아웃바운드 답장을 **14개의 음성 제공자**를 통해 오디오로 변환할 수 있으며,
Feishu, Matrix, Telegram, WhatsApp에서는 네이티브 음성 메시지로,
그 외 모든 곳에서는 오디오 첨부 파일로, 전화 통신 및 Talk용으로는 PCM/Ulaw 스트림으로 전달할 수 있습니다.
...
<Steps>
  ...
  <Step title="구성에서 활성화">
    `messages.tts.auto: "always"` 및 `messages.tts.provider`를 설정합니다.
...
| 제공자          | 인증                                                                                                             | 참고                                                                   |
| ----------------- | ---------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------- |
| **Azure Speech**  | `AZURE_SPEECH_KEY` + `AZURE_SPEECH_REGION` (또한 `AZURE_SPEECH_API_KEY`, `SPEECH_KEY`, `SPEECH_REGION`)          | 네이티브 Ogg/Opus 음성 메모 출력 및 전화 통신.                        |
...
| **MiniMax**       | `MINIMAX_API_KEY` (또는 토큰 플랜: `MINIMAX_OAUTH_TOKEN`, `MINIMAX_CODE_PLAN_KEY`, `MINIMAX_CODING_API_KEY`)      | T2A v2 API. 기본값은 `speech-2.8-hd`입니다.                                |
| **OpenAI**        | `OPENAI_API_KEY`                                                                                                 | 자동 요약에도 사용됩니다. 페르소나 `instructions`를 지원합니다.            |
...
하나의 에이전트가 다른 제공자, 음성, 모델, 페르소나 또는 Auto-TTS 모드로 말해야 하는 경우
`agents.list[].tts`를 사용하세요. 에이전트 블록은 `messages.tts` 위에 심층 병합되므로
...
에이전트별 페르소나를 고정하려면 제공자 구성과 함께 `agents.list[].tts.persona`를 설정하세요.
이 값은 해당 에이전트에 대해서만 전역 `messages.tts.persona`를 재정의합니다.
...
이전 계층 위에 깊은 병합되므로, 공유 공급자 자격 증명은
`messages.tts`에 유지하면서 채널 또는 봇 계정은 음성, 모델, 페르소나,
...
**페르소나**는 공급자 전반에 결정적으로 적용할 수 있는 안정적인
음성 정체성입니다. 하나의 공급자를 선호하고, 공급자 중립 프롬프트
의도를 정의하며, 음성, 모델, 프롬프트 템플릿, 시드, 음성 설정에 대한
공급자별 바인딩을 포함할 수 있습니다.
...
`pacing`, `constraints`)는 **공급자 중립**입니다. 각 공급자는 이를 사용하는
방식을 직접 결정합니다.

<AccordionGroup>
  <Accordion title="Google Gemini">
    유효한 Google 공급자 구성이 `promptTemplate: "audio-profile-v1"`
    또는 `personaPrompt`를 설정한 경우에만 페르소나 프롬프트 필드를
    Gemini TTS 프롬프트 구조로 감쌉니다. 이전 `audioProfile` 및
    `speakerName` 필드는 여전히 Google 전용 프롬프트 텍스트로 앞에
    추가됩니다. `[[tts:text]]` 블록 안의 `[whispers]` 또는 `[laughs]` 같은
    인라인 오디오 태그는 Gemini transcript 안에 보존되며, OpenClaw는
    이러한 태그를 생성하지 않습니다.
  </Accordion>
  <Accordion title="OpenAI">
    명시적인 OpenAI `instructions`가 구성되지 않은 경우에만 페르소나
    프롬프트 필드를 요청 `instructions` 필드에 매핑합니다. 명시적인
    `instructions`가 항상 우선합니다.
  </Accordion>
  <Accordion title="Other providers">
    `personas.<id>.providers.<provider>` 아래의 공급자별 페르소나 바인딩만
    사용합니다. 공급자가 자체 페르소나 프롬프트 매핑을 구현하지 않는 한
    페르소나 프롬프트 필드는 무시됩니다.
  </Accordion>
</AccordionGroup>
...
`fallbackPolicy`는 페르소나에 시도 중인 공급자에 대한 **바인딩이 없을 때**
동작을 제어합니다.

| 정책                | 동작                                                                                                                                       |
| ------------------- | ------------------------------------------------------------------------------------------------------------------------------------------ |
| `preserve-persona`  | **기본값입니다.** 공급자 중립 프롬프트 필드는 계속 사용할 수 있으며, 공급자는 이를 사용하거나 무시할 수 있습니다.                         |
| `provider-defaults` | 해당 시도의 프롬프트 준비에서 페르소나가 생략됩니다. 다른 공급자로의 폴백은 계속되는 동안 공급자는 중립 기본값을 사용합니다.             |
| `fail`              | `reasonCode: "not_configured"` 및 `personaBinding: "missing"`으로 해당 공급자 시도를 건너뜁니다. 폴백 공급자는 계속 시도됩니다.            |

전체 TTS 요청은 시도한 **모든** 공급자가 건너뛰어지거나 실패한 경우에만
실패합니다.
...
`messages.tts.auto`가 `"tagged"`이면 오디오를 트리거하려면 **지시문이
필수**입니다. 스트리밍 블록 전달은 채널이 보기 전에 표시 텍스트에서
지시문을 제거하며, 인접한 블록에 나뉘어 있어도 제거합니다.

`modelOverrides.allowProvider: true`가 아니면 `provider=...`는 무시됩니다.
응답이 `provider=...`를 선언하면 해당 지시문의 다른 키는 그 공급자만
파싱합니다. 지원되지 않는 키는 제거되고 TTS 지시문 경고로 보고됩니다.
...
- `pitch`(MiniMax 정수 피치, −12에서 12, 소수 값은 버림)
...
단일 명령어 `/tts`. Discord에서는 `/tts`가 내장 Discord 명령어이므로
OpenClaw는 `/voice`도 등록합니다. 텍스트 `/tts ...`는 계속 작동합니다.
...
<Note>
명령어에는 권한이 있는 발신자가 필요하며(allowlist/소유자 규칙 적용),
...
- `/tts latest`는 현재 세션 기록에서 최신 어시스턴트 응답을 읽어 오디오로 한 번 전송합니다. 중복 음성 전송을 억제하기 위해 해당 응답의 해시만 세션 항목에 저장합니다.
...
- `/status`는 TTS가 활성화된 경우 활성 TTS 모드와 구성된 공급자, 모델, 음성, 정제된 사용자 지정 엔드포인트 메타데이터를 표시합니다.

## 사용자별 환경설정

슬래시 명령어는 로컬 재정의를 `prefsPath`에 기록합니다. 기본값은
`~/.openclaw/settings/tts.json`입니다. `OPENCLAW_TTS_PREFS` env var 또는
`messages.tts.prefsPath`로 재정의할 수 있습니다.
...
이는 `messages.tts`와 해당 호스트의 활성 `agents.list[].tts` 블록을 합친
유효 구성보다 우선합니다.

## 출력 형식(고정)

TTS 음성 전달은 채널 기능에 따라 결정됩니다. 채널 Plugin은 음성 스타일
TTS가 공급자에 네이티브 `voice-note` 대상을 요청해야 하는지, 아니면
일반 `audio-file` 합성을 유지하고 호환되는 출력만 음성 전달용으로
표시해야 하는지를 알립니다.
...
- **Feishu / WhatsApp**: 음성 메모 답장이 MP3/WebM/WAV/M4A
  또는 그 밖의 오디오 파일로 보이는 형식으로 생성되면, 채널 Plugin은 네이티브 음성 메시지를 보내기 전에 `ffmpeg`로 48kHz
...
- **MiniMax**: 일반 오디오 첨부 파일에는 MP3(`speech-2.8-hd` 모델, 32kHz 샘플 레이트)를 사용합니다. 채널이 알린 음성 메모 대상의 경우, 채널이 트랜스코딩을 알리면 OpenClaw가 전달 전에 MiniMax MP3를 `ffmpeg`로 48kHz Opus로 트랜스코딩합니다.
- **Xiaomi MiMo**: 기본적으로 MP3를 사용하거나, 구성된 경우 WAV를 사용합니다. 채널이 알린 음성 메모 대상의 경우, 채널이 트랜스코딩을 알리면 OpenClaw가 전달 전에 Xiaomi 출력을 `ffmpeg`로 48kHz Opus로 트랜스코딩합니다.
- **로컬 CLI**: 구성된 `outputFormat`을 사용합니다. 음성 메모 대상은
  Ogg/Opus로 변환되고, 전화 통신 출력은 `ffmpeg`로 원시 16 kHz 모노 PCM으로 변환됩니다.
- **Google Gemini**: Gemini API TTS는 원시 24kHz PCM을 반환합니다. OpenClaw는 이를 오디오 첨부 파일용 WAV로 감싸고, 음성 메모 대상용으로 48kHz Opus로 트랜스코딩하며, Talk/전화 통신용으로는 PCM을 직접 반환합니다.
...
- **xAI**: 기본적으로 MP3를 사용합니다. `responseFormat`은 `mp3`, `wav`, `pcm`, `mulaw` 또는 `alaw`일 수 있습니다. OpenClaw는 xAI의 배치 REST TTS 엔드포인트를 사용하고 완전한 오디오 첨부 파일을 반환합니다. 이 공급자 경로에서는 xAI의 스트리밍 TTS WebSocket을 사용하지 않습니다. 이 경로에서는 네이티브 Opus 음성 메모 형식이 지원되지 않습니다.
...
- `mode: "final"`에서는 텍스트 스트림이 완료된 후에도 스트리밍된 최종 답장에 대해
  오디오 전용 TTS를 계속 전송합니다. 생성된 미디어는 일반 답장 첨부 파일과 동일한
  채널 미디어 정규화를 거칩니다.
...
## 채널별 출력 형식
  ...
  - **Feishu / WhatsApp 트랜스코딩:** 음성 메모 답장이 MP3/WebM/WAV/M4A로 도착하면 채널 Plugin이 `ffmpeg`로 48 kHz Ogg/Opus로 트랜스코딩합니다. WhatsApp은 Baileys를 통해 `ptt: true` 및 `audio/ogg; codecs=opus`로 전송합니다. 변환에 실패하면 Feishu는 원본 파일 첨부로 대체하고, WhatsApp 전송은 호환되지 않는 PTT 페이로드를 게시하는 대신 실패합니다.
  ...
  - **Google Gemini:** 원시 24 kHz PCM을 반환합니다. OpenClaw는 첨부 파일용으로 WAV로 감싸고, 음성 메모 대상용으로 48 kHz Opus로 트랜스코딩하며, Talk/전화 통신에는 PCM을 직접 반환합니다.
  ...
  <Accordion title="최상위 messages.tts.*">
    <ParamField path="auto" type='"off" | "always" | "inbound" | "tagged"'>
      자동 TTS 모드입니다. `inbound`는 인바운드 음성 메시지 이후에만 오디오를 전송하고, `tagged`는 답장에 `[[tts:...]]` 지시문 또는 `[[tts:text]]` 블록이 포함된 경우에만 오디오를 전송합니다.
    </ParamField>
    ...
    <ParamField path="provider" type="string">
      음성 공급자 ID입니다. 설정되지 않은 경우 OpenClaw는 레지스트리 자동 선택 순서에서 처음 구성된 공급자를 사용합니다. 레거시 `provider: "edge"`는 `openclaw doctor --fix`에 의해 `"microsoft"`로 다시 작성됩니다.
    </ParamField>
  ...
  <Accordion title="ElevenLabs">
    <ParamField path="apiKey" type="string">`ELEVENLABS_API_KEY` 또는 `XI_API_KEY`로 대체됩니다.</ParamField>
    ...
    <ParamField path="seed" type="number">최선형 결정성을 위한 정수 `0..4294967295`.</ParamField>
    <ParamField path="baseUrl" type="string">ElevenLabs API 기본 URL을 재정의합니다.</ParamField>
  </Accordion>

  <Accordion title="Google Gemini">
    <ParamField path="apiKey" type="string">`GEMINI_API_KEY` / `GOOGLE_API_KEY`로 대체됩니다. 생략하면 TTS는 환경 변수 대체 전에 `models.providers.google.apiKey`를 재사용할 수 있습니다.</ParamField>
    <ParamField path="model" type="string">Gemini TTS 모델입니다. 기본값은 `gemini-3.1-flash-tts-preview`입니다.</ParamField>
    <ParamField path="voiceName" type="string">Gemini 사전 구축 음성 이름입니다. 기본값은 `Kore`입니다. 별칭: `voice`.</ParamField>
    <ParamField path="audioProfile" type="string">말할 텍스트 앞에 추가되는 자연어 스타일 프롬프트입니다.</ParamField>
    <ParamField path="speakerName" type="string">프롬프트가 이름 있는 화자를 사용할 때 말할 텍스트 앞에 추가되는 선택적 화자 레이블입니다.</ParamField>
  ...
  <Accordion title="MiniMax">
    <ParamField path="apiKey" type="string">`MINIMAX_API_KEY`로 폴백합니다. `MINIMAX_OAUTH_TOKEN`, `MINIMAX_CODE_PLAN_KEY` 또는 `MINIMAX_CODING_API_KEY`를 통한 Token Plan 인증.</ParamField>
  ...
  <Accordion title="OpenAI">
    ...
    <ParamField path="instructions" type="string">명시적인 OpenAI `instructions` 필드. 설정하면 페르소나 프롬프트 필드는 자동 매핑되지 **않습니다**.</ParamField>
    <ParamField path="extraBody / extra_body" type="Record<string, unknown>">생성된 OpenAI TTS 필드 뒤에 `/audio/speech` 요청 본문으로 병합되는 추가 JSON 필드. `lang` 같은 제공자별 키가 필요한 Kokoro와 같은 OpenAI 호환 엔드포인트에 사용하세요. 안전하지 않은 프로토타입 키는 무시됩니다.</ParamField>
  ...
  <Accordion title="Xiaomi MiMo">
    ...
    <ParamField path="style" type="string">사용자 메시지로 전송되는 선택적 자연어 스타일 지시문이며, 음성으로 말해지지는 않습니다.</ParamField>
  </Accordion>
</AccordionGroup>

## 에이전트 도구

`tts` 도구는 텍스트를 음성으로 변환하고 답장 전달을 위한 오디오 첨부 파일을 반환합니다. Feishu, Matrix, Telegram, WhatsApp에서는 오디오가 파일 첨부가 아니라 음성 메시지로 전달됩니다. Feishu와 WhatsApp은 `ffmpeg`를 사용할 수 있을 때 이 경로에서 Opus가 아닌 TTS 출력을 트랜스코딩할 수 있습니다.

WhatsApp은 Baileys를 통해 오디오를 PTT 음성 메모(`audio`와 `ptt: true`)로 보내며, 클라이언트가 음성 메모의 캡션을 일관되게 렌더링하지 않기 때문에 표시되는 텍스트는 PTT 오디오와 **별도로** 보냅니다.

이 도구는 선택적 `channel` 및 `timeoutMs` 필드를 허용합니다. `timeoutMs`는 호출별 제공자 요청 시간 제한(밀리초)입니다.
