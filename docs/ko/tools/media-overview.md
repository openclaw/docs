...
title: 미디어 개요
x-i18n:
    generated_at: "2026-07-12T01:15:40Z"
    model: gpt-5.6
...
OpenClaw은 이미지, 동영상, 음악을 생성하고 수신 미디어
...
`stt-tts`, 관찰 전용 음성 캡처를 위한 `transcription`의 세 가지 모드가 있습니다. 이러한 모드는
...
<CardGroup cols={2}>
  ...
  <Card title="동영상 생성" href="/ko/tools/video-generation" icon="video">
    `video_generate`를 통해 텍스트-동영상, 이미지-동영상, 동영상-동영상 변환을 수행합니다.
    비동기로 백그라운드에서 실행되며 준비되면 결과를 게시합니다.
  </Card>
  <Card title="음악 생성" href="/ko/tools/music-generation" icon="music">
    `music_generate`를 통해 음악 또는 오디오 트랙을 생성합니다. 채팅
    세션에서는 공유 미디어 생성 작업 수명 주기에 따라 비동기로 실행됩니다.
  </Card>
...
[미디어 이해](/ko/nodes/media-understanding#provider-support-matrix)를 참조하세요.
</Note>
...
[음성 텍스트 변환 및 Voice Call](#speech-to-text-and-voice-call)을 참조하세요.
...
| 텍스트 음성 변환 | 동기   | 제공자 응답은 몇 초 안에 반환되며 응답 오디오에 첨부됩니다.                                         |
| 동영상         | 비동기   | 제공자 처리에는 30초에서 수 분이 걸리며, 느린 대기열은 구성된 제한 시간까지 실행될 수 있습니다.     |
...
메시지 도구가 필요한 경우 `message(action="send")`를 사용합니다. 요청자 세션이 비활성 상태이거나 활성 깨우기가
