---
read_when:
    - '`openclaw onboard`에 대한 자세한 동작이 필요합니다'
    - 온보딩 결과를 디버깅하거나 온보딩 클라이언트를 통합하는 경우
sidebarTitle: CLI reference
summary: |-
    OpenClaw Docs 국제화 입력>
    CLI 설정 흐름, 인증/모델 설정, 출력 및 내부 동작에 대한 전체 참조
title: CLI 설정 참조
x-i18n:
    generated_at: "2026-07-04T06:27:00Z"
    model: gpt-5.5
...
- 데몬 설치(LaunchAgent, systemd 사용자 유닛 또는 시작 폴더 대체 경로가 있는 네이티브 Windows Scheduled Task)
- 상태 확인
- Skills 설정
...
<Steps>
  <Step title="기존 구성 감지">
    ...
    - 마법사를 다시 실행해도 명시적으로 재설정을 선택하거나 `--reset`을 전달하지 않는 한 아무것도 지우지 않습니다.
  ...
  <Step title="워크스페이스">
    ...
    - 최초 실행 부트스트랩 절차에 필요한 워크스페이스 파일을 시드합니다.
    - 워크스페이스 레이아웃: [Agent 워크스페이스](/ko/concepts/agent-workspace).

  </Step>
  <Step title="Gateway">
    - 포트, 바인드, 인증 모드 및 tailscale 노출을 묻습니다.
    - 권장: 루프백에서도 토큰 인증을 활성화해 로컬 WS 클라이언트가 인증하도록 유지하세요.
  ...
  <Step title="데몬 설치">
    - macOS: LaunchAgent
      - 로그인한 사용자 세션이 필요합니다. 헤드리스 환경에서는 사용자 지정 LaunchDaemon을 사용하세요(제공되지 않음).
    ...
    - 네이티브 Windows: Scheduled Task 우선
      - 작업 생성이 거부되면 OpenClaw는 사용자별 시작 폴더 로그인 항목으로 대체하고 Gateway를 즉시 시작합니다.
      - Scheduled Tasks는 더 나은 감독자 상태를 제공하므로 여전히 선호됩니다.
  ...
  <Step title="Skills">
    ...
    - 사용할 수 없는 Homebrew, uv 및 Go 설치 프로그램은 건너뛴 다음 영향을 받는 Skills를 수동 설정 안내와 함께 그룹화합니다. 누락된 필수 구성 요소를 설치한 후 `openclaw doctor`를 실행하세요.
...
Control UI 애셋이 없으면 마법사가 빌드를 시도합니다. 대체 명령은 `pnpm ui:build`입니다(UI 종속성을 자동 설치).
</Note>
...
<AccordionGroup>
  ...
  <Accordion title="OpenAI Code 구독(OAuth)">
    브라우저 플로입니다. `code#state`를 붙여넣으세요.
  ...
  <Accordion title="OpenAI Code 구독(기기 페어링)">
    수명이 짧은 기기 코드를 사용하는 브라우저 페어링 플로입니다.

    모델이 설정되지 않았거나 이미 OpenAI 계열이면 Codex 런타임을 통해 `agents.defaults.model`을 `openai/gpt-5.5`로 설정합니다.

  </Accordion>
  <Accordion title="OpenAI API 키">
    `OPENAI_API_KEY`가 있으면 사용하고, 없으면 키를 요청한 다음 자격 증명을 인증 프로필에 저장합니다.

    모델이 설정되지 않았거나 `openai/*` 또는 레거시 Codex 모델 참조이면 `agents.defaults.model`을 `openai/gpt-5.5`로 설정합니다.

  </Accordion>
  <Accordion title="xAI(Grok) OAuth">
    자격이 있는 SuperGrok 또는 X Premium 계정용 브라우저 로그인입니다. 대부분의 사용자에게 권장되는 xAI 경로입니다. OpenClaw는 Grok 모델, Grok `web_search`, `x_search` 및 `code_execution`에 사용할 결과 인증 프로필을 저장합니다.
  </Accordion>
  ...
  <Accordion title="xAI(Grok) API 키">
    `XAI_API_KEY`를 요청하고 xAI를 모델 제공자로 구성합니다. 구독 OAuth 대신 xAI Console API 키를 원할 때 사용하세요.
  </Accordion>
  ...
  <Accordion title="StepFun">
    StepFun 표준 또는 Step Plan에 대해 중국 또는 글로벌 엔드포인트용 구성이 자동으로 작성됩니다.
  ...
  <Accordion title="Ollama(클라우드 및 로컬 오픈 모델)">
    ...
    호스트 기반 모드는 기본 URL(기본값 `http://127.0.0.1:11434`)을 요청하고, 사용 가능한 모델을 발견하며, 기본값을 제안합니다.
  ...
  <Accordion title="사용자 지정 제공자">
    ...
    대화형 온보딩은 다른 제공자 API 키 플로와 동일한 API 키 저장 선택지를 지원합니다.
    - **지금 API 키 붙여넣기**(평문)
...
- 사용자 지정 제공자 온보딩은 일반적인 모델 ID의 이미지 지원을 추론하고 모델 이름을 알 수 없는 경우에만 묻습니다.
- 온보딩이 제공자 인증 선택에서 시작되면 모델 선택기는 해당 제공자를 자동으로 우선합니다. Volcengine 및 BytePlus의 경우 동일한 선호도는 해당 코딩 플랜 변형(`volcengine-plan/*`, `byteplus-plan/*`)도 일치시킵니다.
- 해당 선호 제공자 필터가 비어 있으면 선택기는 모델이 없다고 표시하는 대신 전체 카탈로그로 대체합니다.
...
- 비대화형 모드에서 `--secret-input-mode ref`는 env 기반만 지원합니다.
  ...
  - 이 사용자 지정 제공자 사례에서는 `--custom-api-key`에 `CUSTOM_API_KEY`가 설정되어 있어야 하며, 그렇지 않으면 온보딩이 빠르게 실패합니다.
...
- `tools.profile`(설정되지 않은 경우 로컬 온보딩 기본값은 `"coding"`이며, 기존 명시적 값은 보존됩니다)
- `gateway.*`(mode, bind, auth, tailscale)
- `session.dmScope`(설정되지 않은 경우 로컬 온보딩은 이를 `per-channel-peer`로 기본 설정하며, 기존 명시적 값은 보존됩니다)
- `channels.telegram.botToken`, `channels.discord.token`, `channels.matrix.*`, `channels.signal.*`, `channels.imessage.*`
- 프롬프트 중 선택한 경우 채널 허용 목록(Slack, Discord, Matrix, Microsoft Teams)(가능한 경우 이름이 ID로 해석됨)
- `skills.install.nodeManager`
  - `setup --node-manager` 플래그는 `npm`, `pnpm` 또는 `bun`을 허용합니다.
  - 수동 구성에서는 나중에 `skills.install.nodeManager: "yarn"`을 계속 설정할 수 있습니다.
- `wizard.lastRunAt`
...
<Note>
일부 채널은 plugins로 제공됩니다. 설정 중 선택하면 마법사가
채널 구성 전에 plugin(npm 또는 로컬 경로)을 설치하라는 메시지를 표시합니다.
</Note>
