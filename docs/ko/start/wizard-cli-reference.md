...
title: CLI 설정 참조
x-i18n:
    generated_at: "2026-06-30T22:11:16Z"
    model: gpt-5.5
...
<Steps>
  <Step title="기존 설정 감지">
    ...
    - 설정이 유효하지 않거나 레거시 키를 포함하면, 마법사는 계속하기 전에 `openclaw doctor`를 실행하라고 요청하고 중지합니다.
  ...
  <Step title="Gateway">
    ...
    - 권장: local loopback에서도 로컬 WS 클라이언트가 인증해야 하도록 토큰 인증을 켜 둡니다.
  ...
  <Step title="채널">
    ...
    - [Signal](/ko/channels/signal): 선택적 `signal-cli` 설치 + 계정 설정
    - [iMessage](/ko/channels/imessage): `imsg` CLI 경로 + Messages DB 접근 권한. Gateway가 Mac 외부에서 실행될 때는 SSH 래퍼를 사용하세요.
    - DM 보안: 기본값은 페어링입니다. 첫 DM은 코드를 보냅니다. 다음으로 승인하세요.
      `openclaw pairing approve <channel> <code>` 또는 허용 목록을 사용하세요.
  </Step>
  <Step title="데몬 설치">
    - macOS: LaunchAgent
      - 로그인된 사용자 세션이 필요합니다. 헤드리스 환경에서는 사용자 지정 LaunchDaemon을 사용하세요(제공되지 않음).
  ...
  <Step title="Skills">
    ...
    - 노드 관리자를 선택하게 합니다: npm, pnpm 또는 bun.
...
Control UI 자산이 없으면 마법사는 이를 빌드하려고 시도합니다. 폴백은 `pnpm ui:build`입니다(UI 종속성을 자동 설치).
</Note>
...
<AccordionGroup>
  <Accordion title="Anthropic API 키">
    있으면 `ANTHROPIC_API_KEY`를 사용하고, 없으면 키를 요청한 뒤 데몬 사용을 위해 저장합니다.
  </Accordion>
  ...
  <Accordion title="OpenAI Code 구독(기기 페어링)">
    수명이 짧은 기기 코드가 있는 브라우저 페어링 흐름입니다.

    모델이 설정되지 않았거나 이미 OpenAI 계열이면 Codex 런타임을 통해 `agents.defaults.model`을 `openai/gpt-5.5`로 설정합니다.

  </Accordion>
  <Accordion title="OpenAI API 키">
    있으면 `OPENAI_API_KEY`를 사용하고, 없으면 키를 요청한 뒤 자격 증명을 인증 프로필에 저장합니다.

    모델이 설정되지 않았거나 `openai/*` 또는 레거시 Codex 모델 참조이면 `agents.defaults.model`을 `openai/gpt-5.5`로 설정합니다.

  </Accordion>
  <Accordion title="xAI(Grok) OAuth">
    자격이 있는 SuperGrok 또는 X Premium 계정의 브라우저 로그인입니다. 대부분의 사용자에게
    권장되는 xAI 경로입니다. OpenClaw는 Grok 모델, Grok `web_search`,
    `x_search`, `code_execution`에 사용할 인증 프로필을 저장합니다.
  </Accordion>
  <Accordion title="xAI(Grok) 기기 코드">
    localhost 콜백 대신 짧은 코드를 사용하는 원격 친화적 브라우저 로그인입니다.
    SSH, Docker 또는 VPS 호스트에서 사용하세요.
  </Accordion>
  <Accordion title="xAI(Grok) API 키">
    `XAI_API_KEY`를 요청하고 xAI를 모델 제공자로 구성합니다. 구독 OAuth 대신
    xAI Console API 키를 사용하려는 경우 사용하세요.
  </Accordion>
  ...
  <Accordion title="MiniMax">
    설정이 자동으로 작성됩니다. 호스팅 기본값은 `MiniMax-M3`이고, API 키 설정은
    `minimax/...`를 사용하며, OAuth 설정은 `minimax-portal/...`을 사용합니다.
    자세한 내용: [MiniMax](/ko/providers/minimax).
  </Accordion>
  <Accordion title="StepFun">
    China 또는 글로벌 엔드포인트의 StepFun standard 또는 Step Plan에 대한 설정이 자동으로 작성됩니다.
  ...
  <Accordion title="Ollama(클라우드 및 로컬 오픈 모델)">
    ...
    호스트 기반 모드는 기본 URL(기본값 `http://127.0.0.1:11434`)을 요청하고, 사용 가능한 모델을 탐색한 뒤 기본값을 제안합니다.
  ...
  <Accordion title="Moonshot 및 Kimi Coding">
    Moonshot(Kimi K2) 및 Kimi Coding 설정이 자동으로 작성됩니다.
  ...
  <Accordion title="사용자 지정 제공자">
    ...
    - **지금 API 키 붙여넣기**(평문)
    - **비밀 참조 사용**(사전 점검 검증이 있는 환경 참조 또는 구성된 제공자 참조)
...
- 사용자 지정 제공자 온보딩은 일반적인 모델 ID의 이미지 지원을 추론하며, 모델 이름을 알 수 없을 때만 질문합니다.
- 온보딩이 제공자 인증 선택에서 시작되면 모델 선택기는 해당 제공자를
  자동으로 우선합니다. Volcengine 및 BytePlus의 경우 동일한 선호도는
  해당 코딩 플랜 변형(`volcengine-plan/*`, `byteplus-plan/*`)에도
  일치합니다.
- 선호 제공자 필터가 비어 있게 되면 선택기는 모델을 표시하지 않는 대신
  전체 카탈로그로 폴백합니다.
...
- 기본 온보딩 동작은 API 키를 auth 프로필에 일반 텍스트 값으로 유지합니다.
...
- 비대화형 모드에서 `--secret-input-mode ref`는 env 기반만 지원합니다.
  ...
  - 이 사용자 지정 provider 사례에서는 `--custom-api-key`에 `CUSTOM_API_KEY`가 설정되어 있어야 하며, 그렇지 않으면 온보딩이 즉시 실패합니다.
...
<Note>
헤드리스 및 서버 팁: 브라우저가 있는 머신에서 OAuth를 완료한 다음, 해당
agent의 `auth-profiles.json`(예:
`~/.openclaw/agents/<agentId>/agent/auth-profiles.json` 또는 일치하는
`$OPENCLAW_STATE_DIR/...` 경로)을 gateway 호스트에 복사하세요. `credentials/oauth.json`은
레거시 가져오기 소스로만 사용됩니다.
</Note>
...
- `--skip-bootstrap`이 전달될 때 `agents.defaults.skipBootstrap`
- `agents.defaults.model` / `models.providers`(Minimax를 선택한 경우)
- `tools.profile`(설정되지 않은 경우 로컬 온보딩은 기본값으로 `"coding"`을 사용하며, 기존 명시적 값은 보존됩니다)
- `gateway.*`(모드, 바인드, auth, tailscale)
- `session.dmScope`(설정되지 않은 경우 로컬 온보딩은 기본값으로 `per-channel-peer`를 사용하며, 기존 명시적 값은 보존됩니다)
- `channels.telegram.botToken`, `channels.discord.token`, `channels.matrix.*`, `channels.signal.*`, `channels.imessage.*`
- 프롬프트 중 옵트인할 때의 채널 허용 목록(Slack, Discord, Matrix, Microsoft Teams)(가능한 경우 이름은 ID로 해석됩니다)
- `skills.install.nodeManager`
  - `setup --node-manager` 플래그는 `npm`, `pnpm` 또는 `bun`을 허용합니다.
  - 수동 구성은 나중에 계속 `skills.install.nodeManager: "yarn"`을 설정할 수 있습니다.
- `wizard.lastRunAt`
...
- Windows는 WSL2를 사용하며 WSL 내부의 Linux signal-cli 흐름을 따름
