...
Esta página cobre atualizações do plugin público `matrix` anterior para a implementação atual.
...
- instalações a partir do código-fonte executam `openclaw doctor --fix` durante o fluxo de atualização e, em seguida, reiniciam o gateway por padrão
- instalações por gerenciador de pacotes atualizam o pacote, executam uma passada não interativa do doctor e depois dependem da reinicialização padrão do gateway para que a inicialização possa concluir a migração do Matrix
- se você usar `openclaw update --no-restart`, a migração do Matrix dependente da inicialização será adiada até que você execute depois `openclaw doctor --fix` e reinicie o gateway

A migração automática cobre:

- criação ou reutilização de um snapshot pré-migração em `~/Backups/openclaw-migrations/`
- reutilização de suas credenciais Matrix em cache
- manutenção da mesma seleção de conta e da configuração `channels.matrix`
- movimentação do armazenamento de sincronização Matrix plano mais antigo para o local atual com escopo por conta
- movimentação do armazenamento de criptografia Matrix plano mais antigo para o local atual com escopo por conta quando a conta de destino puder ser resolvida com segurança
- extração de uma chave de descriptografia de backup de chave de sala do Matrix salva anteriormente do armazenamento criptográfico rust antigo, quando essa chave existir localmente
- reutilização da raiz de armazenamento de hash de token existente mais completa para a mesma conta Matrix, homeserver e usuário quando o token de acesso mudar depois
...
- restauração de chaves de sala com backup no novo armazenamento criptográfico na próxima inicialização do Matrix
...
- Se o Matrix tiver apenas estado de migração com avisos, por exemplo porque `userId` ou `accessToken` ainda está ausente, o OpenClaw ainda não cria o snapshot porque nenhuma mutação do Matrix é acionável.
...
- o armazenamento plano Matrix mais antigo (`~/.openclaw/matrix/bot-storage.json` e `~/.openclaw/matrix/crypto/`) veio de um layout de armazenamento único, então o OpenClaw só pode migrá-lo para um único destino de conta Matrix resolvido
...
O plugin público Matrix anterior **não** criava automaticamente backups de chave de sala do Matrix. Ele persistia o estado criptográfico local e solicitava verificação do dispositivo, mas não garantia que suas chaves de sala fossem salvas no homeserver.
...
- instalações de plugin Matrix em caminho personalizado são exibidas tanto na inicialização do gateway quanto em `openclaw doctor`

Se sua instalação antiga tinha histórico criptografado somente local que nunca foi salvo em backup, algumas mensagens criptografadas mais antigas podem continuar ilegíveis após a atualização.
...
7. Se este dispositivo ainda não estiver verificado, execute o comando para a conta correspondente:
   ...
   e digite `yes` somente quando corresponderem. O comando só é concluído com sucesso
   depois que `Cross-signing verified` se tornar `yes`.

8. Se você estiver abandonando intencionalmente um histórico antigo irrecuperável e quiser uma nova linha de base de backup para mensagens futuras, execute:

   ```bash
   openclaw matrix verify backup reset --yes
   ```

9. Se ainda não existir backup de chave no servidor, crie um para recuperações futuras:
...
A migração criptografada é um processo de dois estágios:
...
2. A inicialização ou `openclaw doctor --fix` inspeciona o armazenamento criptográfico Matrix antigo por meio da instalação ativa do plugin Matrix.
...
Se o armazenamento antigo reportar chaves de sala que nunca foram salvas em backup, o OpenClaw emite um aviso em vez de fingir que a recuperação foi bem-sucedida.
...
- O que fazer: defina `channels.matrix.defaultAccount` como a conta pretendida, então execute novamente `openclaw doctor --fix` ou reinicie o gateway.

`Matrix legacy sync store not migrated because the target already exists (...)`

- Significado: o local novo com escopo por conta já tem um armazenamento de sincronização ou criptografia, então o OpenClaw não o sobrescreveu automaticamente.
...
- Significado: o OpenClaw tentou mover o estado Matrix antigo, mas a operação no sistema de arquivos falhou.
- O que fazer: inspecione permissões do sistema de arquivos e o estado do disco, então execute novamente `openclaw doctor --fix`.
...
- Significado: o OpenClaw detectou estado Matrix antigo, mas a migração ainda está bloqueada por falta de dados de identidade ou credenciais.
- O que fazer: conclua o login ou a configuração do Matrix, então execute novamente `openclaw doctor --fix` ou reinicie o gateway.
...
- O que fazer: reinstale ou repare o plugin Matrix (`openclaw plugins install @openclaw/matrix`, ou `openclaw plugins install ./path/to/local/matrix-plugin` para um checkout do repositório), então execute novamente `openclaw doctor --fix` ou reinicie o gateway.
...
- O que fazer: reinstale o plugin Matrix a partir de um caminho confiável, então execute novamente `openclaw doctor --fix` ou reinicie o gateway.
...
- O que fazer: resolva o erro de backup, então execute novamente `openclaw doctor --fix` ou reinicie o gateway.
...
- Significado: algumas chaves de sala antigas existiam apenas no armazenamento local antigo e nunca haviam sido enviadas para o backup do Matrix.
- O que fazer: espere que algum histórico criptografado antigo permaneça indisponível, a menos que você consiga recuperar essas chaves manualmente de outro cliente verificado.
...
- O que fazer: execute novamente `openclaw doctor --fix`. Se isso se repetir, mantenha intacto o diretório de estado antigo e recupere usando outro cliente Matrix verificado mais `printf '%s\n' "$MATRIX_RECOVERY_KEY" | openclaw matrix verify backup restore --recovery-key-stdin`.
...
- O que fazer: verifique qual chave de recuperação está correta antes de repetir qualquer comando de restauração.

`Legacy Matrix encrypted state for account "..." cannot be fully converted automatically because the old rust crypto store does not expose all local room keys for export.`

- Significado: este é o limite rígido do formato de armazenamento antigo.
- O que fazer: chaves salvas em backup ainda podem ser restauradas, mas histórico criptografado somente local pode continuar indisponível.

`matrix: failed restoring room keys from legacy encrypted-state backup: ...`

- Significado: o novo plugin tentou restaurar, mas o Matrix retornou um erro.
- O que fazer: execute `openclaw matrix verify backup status`, então tente novamente com `printf '%s\n' "$MATRIX_RECOVERY_KEY" | openclaw matrix verify backup restore --recovery-key-stdin` se necessário.
...
- O que fazer: defina `MATRIX_RECOVERY_KEY`, execute `printf '%s\n' "$MATRIX_RECOVERY_KEY" | openclaw matrix verify device --recovery-key-stdin` e então restaure o backup.

`Backup key mismatch on this device. Re-run 'openclaw matrix verify device --recovery-key-stdin' with the matching recovery key.`

- Significado: a chave armazenada não corresponde ao backup Matrix ativo.
- O que fazer: defina `MATRIX_RECOVERY_KEY` com a chave correta e execute `printf '%s\n' "$MATRIX_RECOVERY_KEY" | openclaw matrix verify device --recovery-key-stdin`.
...
segredo de backup armazenado estiver quebrado, essa redefinição também poderá recriar o armazenamento secreto para que a
...
- O que fazer: execute `openclaw matrix verify self`, aceite a solicitação em outro
  ...
  comando espera confiança total de identidade Matrix antes de reportar sucesso. Use
  `printf '%s\n' "$MATRIX_RECOVERY_KEY" | openclaw matrix verify bootstrap --recovery-key-stdin --force-reset-cross-signing`
  somente quando você intencionalmente quiser substituir a identidade atual de cross-signing.

`Matrix key backup is not active on this device after loading from secret storage.`

- Significado: o armazenamento secreto não produziu uma sessão de backup ativa neste dispositivo.
- O que fazer: verifique primeiro o dispositivo e depois confira novamente com `openclaw matrix verify backup status`.

`Matrix crypto backend cannot load backup keys from secret storage. Verify this device with 'openclaw matrix verify device --recovery-key-stdin' first.`

- Significado: este dispositivo não consegue restaurar do armazenamento secreto até que a verificação do dispositivo seja concluída.
...
Se você aceitar perder o histórico criptografado antigo irrecuperável e quiser apenas uma linha de base limpa de backup daqui para frente, execute estes comandos nesta ordem:
...
Se o dispositivo ainda não estiver verificado depois disso, conclua a verificação no seu cliente Matrix comparando os emojis SAS ou os códigos decimais e confirmando que correspondem.

## Páginas relacionadas

- [Matrix](/pt-BR/channels/matrix)
- [Doctor](/pt-BR/gateway/doctor)
- [Migrando](/pt-BR/install/migrating)
