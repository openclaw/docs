...
x-i18n:
    generated_at: "2026-05-06T05:59:55Z"
    model: gpt-5.5
    provider: openai
...
Normalmente publicamos builds estáveis primeiro em **beta**, testamos ali e então executamos uma
...
diretamente em `latest` quando necessário. Dist-tags são a fonte da verdade para instalações
via npm.
...
- **`stable`** (instalações de pacote): atualiza via npm dist-tag `latest`.
- **`beta`** (instalações de pacote): prefere o npm dist-tag `beta`, mas volta para
  `latest` quando `beta` está ausente ou é mais antigo que a tag estável atual.
- **`stable`** (instalações git): faz checkout da tag git estável mais recente.
- **`beta`** (instalações git): prefere a tag git beta mais recente, mas volta para
...
# Instalar a partir da beta dist-tag (pontual, não persiste)
...
- `--tag` se aplica **apenas a instalações de pacote (npm)**. Instalações git a ignoram.
...
- `--channel beta` é diferente de `--tag beta`: o fluxo de canal pode voltar para
  stable/latest quando beta estiver ausente ou for mais antigo, enquanto `--tag beta` direciona a
  dist-tag `beta` bruta para aquela execução única.
...
Quando você alterna canais com `openclaw update`, o OpenClaw também sincroniza as fontes de plugins:
...
- Marque as versões nas quais você quer que checkouts git parem (`vYYYY.M.D` para stable,
  `vYYYY.M.D-beta.N` para beta).
- `vYYYY.M.D.beta.N` também é reconhecida para compatibilidade, mas prefira `-beta.N`.
...
Builds beta e dev podem **não** incluir uma versão do app macOS. Tudo bem:
