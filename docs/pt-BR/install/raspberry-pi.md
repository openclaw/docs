...
read_when:
    ...
    - Criando uma IA pessoal barata sempre ativa
...
Execute um OpenClaw Gateway persistente e sempre ativo em um Raspberry Pi. Como o Pi é apenas o gateway (os modelos rodam na nuvem via API), até mesmo um Pi modesto lida bem com a carga de trabalho — o custo típico de hardware é **US$ 35–80 uma vez**, sem mensalidades.
...
| Pi 3B+       | 1 GB   | Lento     | Funciona, mas fica arrastado.        |
| Pi Zero 2 W  | 512 MB | Não       | Não recomendado.                     |

**Mínimo:** 1 GB de RAM, 1 núcleo, 500 MB de disco livre, SO de 64 bits.
...
<Steps>
  ...
  <Step title="Acesse a Control UI">
    ...
    Abra a URL impressa no seu navegador local. Para acesso remoto sempre ativo, veja a [integração com Tailscale](/pt-BR/gateway/tailscale).
...
**Use um SSD USB** -- Cartões SD são lentos e se desgastam. Um SSD USB melhora drasticamente o desempenho. Veja o [guia de inicialização USB do Pi](https://www.raspberrypi.com/documentation/computers/raspberry-pi.html#usb-mass-storage-boot).
...
`OPENCLAW_NO_RESPAWN=1` mantém reinicializações rotineiras do Gateway dentro do processo, o que evita repasses extras de processo e mantém o rastreamento de PID simples em hosts pequenos.
...
**Drop-in do systemd para reinicializações estáveis** -- Se este Pi estiver rodando principalmente o OpenClaw, adicione um drop-in de serviço:
...
Depois execute `systemctl --user daemon-reload && systemctl --user restart openclaw-gateway.service`. Em um Pi headless, também habilite o lingering uma vez para que o serviço do usuário sobreviva ao logout: `sudo loginctl enable-linger "$(whoami)"`.

## Configuração de modelo recomendada

Como o Pi roda apenas o gateway, use modelos de API hospedados na nuvem:
...
Não rode LLMs locais em um Pi — até modelos pequenos são lentos demais para serem úteis. Deixe Claude ou GPT fazerem o trabalho de modelo.

## Observações sobre binários ARM

A maioria dos recursos do OpenClaw funciona em ARM64 sem alterações (Node.js, Telegram, WhatsApp/Baileys, Chromium). Os binários que ocasionalmente não têm builds ARM normalmente são ferramentas de CLI opcionais em Go/Rust enviadas por skills. Verifique a página de releases do binário ausente em busca de artefatos `linux-arm64` / `aarch64` antes de recorrer à compilação a partir do código-fonte.

## Persistência e backups

O estado do OpenClaw fica em:

- `~/.openclaw/` — `openclaw.json`, `auth-profiles.json` por agente, estado de canal/provedor, sessões.
...
Se você mantiver esses dados em um SSD, tanto o desempenho quanto a longevidade melhoram em relação ao cartão SD.

## Solução de problemas

**Sem memória** -- Verifique se a swap está ativa com `free -h`. Desabilite serviços não usados (`sudo systemctl disable cups bluetooth avahi-daemon`). Use apenas modelos baseados em API.

**Desempenho lento** -- Use um SSD USB em vez de um cartão SD. Verifique se há throttling de CPU com `vcgencmd get_throttled` (deve retornar `0x0`).

**O serviço não inicia** -- Verifique os logs com `journalctl --user -u openclaw-gateway.service --no-pager -n 100` e execute `openclaw doctor --non-interactive`. Se este for um Pi headless, também verifique se o lingering está habilitado: `sudo loginctl enable-linger "$(whoami)"`.
