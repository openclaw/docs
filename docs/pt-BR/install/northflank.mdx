---
read_when:
    - Implantando o OpenClaw no Northflank
    - Você quer uma implantação em nuvem com um clique e Interface de controle no navegador
summary: Implantar o OpenClaw no Northflank com modelo de um clique
title: Northflank
x-i18n:
    generated_at: "2026-04-23T14:03:55Z"
    model: gpt-5.4
    provider: openai
...
Implante o OpenClaw no Northflank com um modelo de um clique e acesse-o pela Interface de controle web.
Este é o caminho mais fácil de “sem terminal no servidor”: o Northflank executa o Gateway para você.
...
- Armazenamento persistente via Volume do Northflank (`/data`) para que `openclaw.json`,
  `auth-profiles.json` por agente, estado de canal/provider, sessões e
