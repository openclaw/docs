---
read_when:
    - Você quer instalações reproduzíveis e que permitam reversão
    - Você já usa Nix/NixOS/Home Manager
    - Você quer que tudo fique fixado e seja gerenciado de forma declarativa
summary: Instale o OpenClaw declarativamente com o Nix
title: Nix
x-i18n:
    generated_at: "2026-07-12T00:03:05Z"
    model: gpt-5.6
...
<Steps>
  ...
  <Step title="Criar um flake local">
    Use o modelo voltado para agentes do repositório nix-openclaw:
    ```bash
  ...
  <Step title="Configurar segredos">
    Configure o token do bot de mensagens e a chave de API do provedor de modelos. Arquivos simples em `~/.secrets/` funcionam normalmente.
  </Step>
  <Step title="Preencher os espaços reservados do modelo e aplicar">
    ```bash
    home-manager switch
    ```
  </Step>
  <Step title="Verificar">
    Confirme se o serviço launchd está em execução e se o bot responde às mensagens.
  </Step>
</Steps>
...
- O arquivo `openclaw.json` é tratado como imutável. Os padrões derivados durante a inicialização permanecem apenas no ambiente de execução, e os gravadores de configuração (configuração, integração inicial, `openclaw update` com alterações, instalação/atualização/desinstalação/ativação de Plugins, `doctor --fix`, `doctor --generate-gateway-token`, `openclaw config set`) se recusam a editar o arquivo.
- Em vez disso, edite o código-fonte Nix. Para o nix-openclaw, use o [Início rápido](https://github.com/openclaw/nix-openclaw#quick-start) voltado para agentes e defina a configuração em `programs.openclaw.config` ou `instances.<name>.config`.
...
O serviço Gateway do launchd/systemd descobre automaticamente os binários dos perfis Nix para que Plugins e ferramentas que executam externamente executáveis instalados pelo `nix` funcionem sem configuração manual do PATH:

- Quando `NIX_PROFILES` está definido, cada entrada é adicionada ao PATH do serviço com precedência da direita para a esquerda (corresponde à precedência do shell do Nix: a entrada mais à direita prevalece).
- Quando `NIX_PROFILES` não está definido, `~/.nix-profile/bin` é adicionado como alternativa.

Isso se aplica aos ambientes de serviço do launchd no macOS e do systemd no Linux.
...
<CardGroup cols={2}>
  ...
  <Card title="Assistente de configuração" href="/pt-BR/start/wizard" icon="wand-magic-sparkles">
    Guia passo a passo de configuração pela CLI sem Nix.
  </Card>
