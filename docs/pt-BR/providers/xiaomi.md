...
provedores de texto e um provedor de fala (TTS):
...
| Variáveis de ambiente de auth | `XIAOMI_API_KEY`, `XIAOMI_TOKEN_PLAN_API_KEY`                                                                                                      |
...
| API                           | conclusões de chat compatíveis com a OpenAI (`openai-completions`)                                                                                 |
...
<Steps>
  <Step title="Get the right key">
    Crie uma chave com pagamento conforme o uso no [console do Xiaomi MiMo](https://platform.xiaomimimo.com/#/console/api-keys) ou abra a página da sua assinatura do Token Plan e copie a URL base regional compatível com a OpenAI junto com a chave `tp-...` correspondente.
  </Step>
...
<Tip>
A integração inicial valida o formato da chave e emite um aviso quando uma chave `tp-...` é inserida no fluxo de pagamento conforme o uso ou quando uma chave `sk-...` é inserida no fluxo do Token Plan.
</Tip>
...
`xiaomi-token-plan` precisa de uma URL base regional para ser resolvido. O caminho
compatível é uma opção de integração inicial do Token Plan incluída ou um bloco
de configuração `models.providers.xiaomi-token-plan` explícito com `baseUrl`
definido; o provedor não é oferecido sem uma dessas opções.

## Modelos de raciocínio

`mimo-v2-pro`, `mimo-v2-omni`, `mimo-v2.5` e `mimo-v2.5-pro` oferecem suporte
à [diretiva `/think` do OpenClaw](/pt-BR/tools/thinking) com os níveis `off`,
...
para `messages.tts`. Ele chama o contrato de TTS de conclusões de chat da Xiaomi
com o texto como uma mensagem `assistant` e orientações opcionais de estilo como
uma mensagem `user`.
...
uma instrução de estilo em linguagem natural, em vez de usar um ID de voz
predefinido. Defina `style` como a descrição de voz desejada; o OpenClaw a envia
como a mensagem `user`, envia o texto falado como a mensagem `assistant` e omite
`audio.voice` para esse modelo.
...
Para canais que solicitam um destino de síntese de nota de voz (Discord, Feishu,
Matrix, Telegram e WhatsApp), o OpenClaw transcodifica a saída da Xiaomi para
Opus mono de 48 kHz com `ffmpeg` antes da entrega.
...
Os preços e os sinalizadores de compatibilidade vêm do manifesto do plugin incluído; por isso, o exemplo de configuração omite `cost` e `compat` para evitar divergências em relação ao comportamento em tempo de execução.
...
Os preços vêm do manifesto incluído (os modelos do Token Plan incluem preços em camadas para leituras de cache); por isso, o exemplo de configuração omite `cost`.

<AccordionGroup>
  <Accordion title="Auto-injection behavior">
    O provedor `xiaomi` é habilitado automaticamente quando `XIAOMI_API_KEY` está definido no seu ambiente ou quando existe um perfil de autenticação. `xiaomi-token-plan` precisa de uma URL base regional; portanto, o caminho compatível é a opção de integração inicial do Token Plan incluída ou um bloco de configuração `models.providers.xiaomi-token-plan` explícito.
  </Accordion>

  <Accordion title="Model details">
    - **mimo-v2-flash** - leve e rápido, ideal para tarefas de texto de uso geral. Sem suporte a raciocínio.
    - **mimo-v2-pro** - oferece suporte a raciocínio com uma janela de contexto de 1 milhão de tokens para cargas de trabalho com documentos longos.
  ...
  <Accordion title="Troubleshooting">
    - Se os modelos não aparecerem, confirme que a variável de ambiente da chave relevante ou o perfil de autenticação está presente e é válido.
    - Para o Token Plan, confirme que a região escolhida na integração inicial corresponde à URL base da página de assinatura e que a chave começa com `tp-`.
    - Quando o Gateway for executado como daemon, verifique se a chave está disponível para esse processo (por exemplo, em `~/.openclaw/.env` ou por meio de `env.shellEnv`).

    <Warning>
    Chaves definidas apenas no seu shell interativo não ficam visíveis para processos do Gateway gerenciados como daemon. Use `~/.openclaw/.env` ou a configuração `env.shellEnv` para disponibilidade persistente.
    </Warning>
