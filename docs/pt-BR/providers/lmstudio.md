...
title: LM Studio
x-i18n:
    generated_at: "2026-05-02T05:54:55Z"
    model: gpt-5.5
    provider: openai
...
LM Studio é um aplicativo amigável e poderoso para executar modelos de pesos abertos no seu próprio hardware. Ele permite executar modelos llama.cpp (GGUF) ou MLX (Apple Silicon). Disponível como pacote GUI ou daemon sem interface (`llmster`). Para a documentação do produto e da configuração, consulte [lmstudio.ai](https://lmstudio.ai/).

## Início rápido

1. Instale o LM Studio (desktop) ou `llmster` (sem interface) e, em seguida, inicie o servidor local:
...
Use a integração inicial não interativa quando quiser automatizar a configuração com script (CI, provisionamento, bootstrap remoto):
...
A configuração interativa pode solicitar um comprimento de contexto de carregamento preferencial opcional e aplicá-lo a todos os modelos LM Studio descobertos que ela salva na configuração.
A configuração do Plugin LM Studio confia no endpoint LM Studio configurado para solicitações de modelo, incluindo hosts de loopback, LAN e tailnet. Você pode desativar isso definindo `models.providers.lmstudio.request.allowPrivateNetwork: false`.
...
`usage` no formato da OpenAI, o OpenClaw recupera as contagens de tokens a partir dos metadados
...
Quando a descoberta `/api/v1/models` do LM Studio informa opções de raciocínio
específicas do modelo, o OpenClaw preserva esses valores nativos nos metadados de compatibilidade do modelo. Para
modelos de raciocínio binário que anunciam `allowed_options: ["off", "on"]`,
o OpenClaw mapeia o raciocínio desabilitado para `off` e níveis `/think` habilitados para `on`,
em vez de enviar valores exclusivos da OpenAI, como `low` ou `medium`.
...
Certifique-se de que o LM Studio esteja em execução. Se a autenticação estiver habilitada, também defina `LM_API_TOKEN`:
...
O LM Studio oferece suporte ao carregamento de modelo just-in-time (JIT), no qual os modelos são carregados na primeira solicitação. O OpenClaw pré-carrega modelos por meio do endpoint de carregamento nativo do LM Studio por padrão, o que ajuda quando o JIT está desabilitado. Para permitir que o JIT, o TTL ocioso e o comportamento de remoção automática do LM Studio controlem o ciclo de vida do modelo, desabilite a etapa de pré-carregamento do OpenClaw:
...
Use o endereço alcançável do host LM Studio, mantenha `/v1` e certifique-se de que o LM Studio esteja vinculado além do loopback nessa máquina:
...
Ao contrário dos provedores genéricos compatíveis com OpenAI, `lmstudio` confia automaticamente no endpoint local/privado configurado para solicitações de modelo protegidas. IDs de provedores personalizados de loopback, como `localhost` ou `127.0.0.1`, também são confiáveis automaticamente; para IDs de provedores personalizados de LAN, tailnet ou DNS privado, defina `models.providers.<id>.request.allowPrivateNetwork: true` explicitamente.
