...
x-i18n:
    generated_at: "2026-04-24T06:08:05Z"
    model: gpt-5.4
    provider: openai
...
O OpenClaw pode usar muitos provedores de LLM. Escolha um, autentique-se e então defina o
modelo padrão como `provider/model`.

## Início rápido (duas etapas)

1. Autentique-se com o provedor (normalmente via `openclaw onboard`).
...
- `anthropic-vertex` - suporte implícito a Anthropic no Google Vertex quando credenciais do Vertex estão disponíveis; sem escolha separada de autenticação no onboarding
...
Para ver o catálogo completo de provedores (xAI, Groq, Mistral etc.) e configuração avançada,
...
- [CLI de Models](/pt-BR/cli/models)
