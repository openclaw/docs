...
title: Groq
x-i18n:
    generated_at: "2026-07-12T00:18:44Z"
    model: gpt-5.6
...
[Groq](https://groq.com) oferece inferência ultrarrápida em modelos de pesos abertos (Llama, Gemma, Kimi, Qwen, GPT OSS e outros) usando hardware LPU personalizado. O Plugin da Groq registra tanto um provedor de chat compatível com OpenAI quanto um provedor de compreensão de mídia de áudio.
...
<Tip>
  O catálogo evolui a cada versão do OpenClaw. `openclaw models list --provider groq` mostra as linhas conhecidas pela sua versão instalada; compare com [console.groq.com/docs/models](https://console.groq.com/docs/models) para verificar modelos recém-adicionados ou descontinuados.
</Tip>

## Modelos de raciocínio

Os modelos de raciocínio da Groq (`reasoning: true` na tabela acima) mapeiam os níveis compartilhados de `/think` do OpenClaw para valores de `reasoning_effort` como `low`, `medium` ou `high`. `/think off` ou `/think none` omite `reasoning_effort` da solicitação, em vez de enviar um valor desativado.

Consulte [Modos de raciocínio](/pt-BR/tools/thinking) para conhecer os níveis compartilhados de `/think` e como o OpenClaw os traduz para cada provedor.
...
<AccordionGroup>
  <Accordion title="Disponibilidade do ambiente para o daemon">
    ...
    <Warning>
      Uma chave exportada somente em um shell interativo não estará disponível para um daemon do launchd ou systemd, a menos que esse ambiente também seja importado nele. Defina a chave em `~/.openclaw/.env` ou por meio de `env.shellEnv` para que ela possa ser lida pelo processo do Gateway.
    </Warning>
...
<CardGroup cols={2}>
  ...
  <Card title="Referência de configuração" href="/pt-BR/gateway/configuration-reference" icon="gear">
    Esquema de configuração completo, incluindo definições de provedor e áudio.
  </Card>
