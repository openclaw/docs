...
summary: Use o Qwen Cloud por meio do Plugin do OpenClaw
title: Qwen
x-i18n:
    generated_at: "2026-07-12T00:19:03Z"
    model: gpt-5.6
...
Qwen Cloud é um plugin oficial externo de provedor do OpenClaw com o id canônico `qwen`. Ele é destinado aos endpoints Standard e Coding Plan do Qwen Cloud / Alibaba DashScope, expõe o Token Plan como `qwen-token-plan`, mantém `modelstudio` como um alias de compatibilidade, gerencia de forma independente o id de provedor personalizado `bailian-token-plan` documentado pela Alibaba e expõe o fluxo de token do Qwen Portal como [`qwen-oauth`](/pt-BR/providers/qwen-oauth).
...
O `qwen` é distribuído como um plugin oficial externo, não incluído no núcleo. Instale-o e reinicie o Gateway:
...
<Tabs>
  <Tab title="Coding Plan (assinatura)">
    **Mais indicado para:** acesso por assinatura por meio do Qwen Coding Plan.
    ...
    <Note>
    Os ids legados de opção de autenticação `modelstudio-*` e as referências de modelo `modelstudio/...` ainda
    funcionam como aliases de compatibilidade, mas os novos fluxos de configuração devem dar preferência aos ids canônicos
    de opção de autenticação `qwen-*` e às referências de modelo `qwen/...`. Se você definir uma entrada personalizada exata
    `models.providers.modelstudio` com outro valor de `api`, esse
    provedor personalizado gerenciará as referências `modelstudio/...` em vez do alias de compatibilidade
    do Qwen.
    </Note>
    ...
    **Mais indicado para:** acesso por assinatura de equipe baseado em créditos ao Qwen e a modelos de terceiros compatíveis por meio do Alibaba Cloud Model Studio.
    ...
    <Steps>
      ...
      <Step title="Execute a integração inicial">
        ...
        Para o endpoint da **China** em Pequim:
    ...
    <Note>
    O guia do OpenClaw da Alibaba usa `bailian-token-plan` para um
    provedor personalizado manual. O plugin registra esse id como responsável pela compatibilidade, mas as novas
    configurações devem usar `qwen-token-plan`. Uma entrada personalizada exata
    `models.providers.bailian-token-plan` mantém o controle sobre o transporte e o catálogo
    configurados; ela nunca é mesclada ao catálogo canônico da OpenAI.
    </Note>

    <Warning>
    Use o Token Plan somente em sessões interativas do OpenClaw. Não o selecione para
    tarefas Cron, scripts autônomos ou backends de aplicações. A Alibaba informa que
    o uso não interativo pode suspender a assinatura ou revogar sua chave de API.
    </Warning>

  </Tab>

  <Tab title="Qwen OAuth / Portal">
    **Mais indicado para:** um token do Qwen Portal para `https://portal.qwen.ai/v1`.

    Consulte [Qwen OAuth / Portal](/pt-BR/providers/qwen-oauth) para acessar a página dedicada do provedor
    e as observações sobre migração.
    ...
    <Note>
    O `qwen-oauth` usa o mesmo nome de variável de ambiente `QWEN_API_KEY` que o provedor
    Qwen Cloud, mas armazena a autenticação sob o id de provedor `qwen-oauth` quando configurado
    por meio da integração inicial do OpenClaw.
    </Note>
...
O provedor seleciona automaticamente o endpoint com base na opção de autenticação. As opções
canônicas usam a família `qwen-*`; `modelstudio-*` permanece apenas para compatibilidade.
Substitua essa seleção com um `baseUrl` personalizado na configuração.
...
O OpenClaw distribui este catálogo estático do Qwen. O catálogo considera o endpoint: as configurações do Coding
Plan omitem modelos que funcionam somente no endpoint Standard.
...
<Note>
A disponibilidade ainda pode variar conforme o endpoint e o plano de cobrança, mesmo quando um modelo está
presente no catálogo estático.
</Note>

### Catálogo do Token Plan

O Token Plan usa uma lista de permissões separada com correspondência exata de strings. Os modelos do plano
exclusivos para geração de imagens não estão incluídos aqui porque usam APIs diferentes.
...
raciocínio habilitado no catálogo integrado. Para modelos de raciocínio da família
`qwen`, o provedor mapeia os níveis de pensamento do OpenClaw para o sinalizador
de solicitação de nível superior `enable_thinking` do DashScope: o pensamento
desabilitado envia `enable_thinking: false`; qualquer outro nível envia
`enable_thinking: true`. Modelos personalizados podem optar por uma carga de
pensamento alternativa no modelo de chat definindo
`compat.thinkingFormat: "qwen-chat-template"` na entrada do modelo.

Os modelos do Token Plan também são marcados como compatíveis com raciocínio.
`kimi-k2.7-code` e `MiniMax-M2.5` funcionam somente com pensamento, portanto o
OpenClaw mantém o pensamento habilitado mesmo quando a sessão solicita
`/think off`. O DeepSeek V4 mapeia de `minimal` a `high` para o esforço `high`
do serviço e mapeia `xhigh` ou `max` para `max`. O GLM 5.2 aceita todo o
intervalo de `minimal` a `max`; o GLM 5.1 e o GLM 5 aceitam até `xhigh`, e os
três usam `high` como padrão. Outros modelos híbridos seguem o estado
ligado/desligado solicitado.
...
A compreensão de mídia é resolvida automaticamente com base na autenticação
Qwen configurada; nenhuma configuração adicional é necessária. Certifique-se
de usar um endpoint Standard (pagamento conforme o uso) para que a compreensão
de mídia funcione.
...
Limites de geração de vídeo: 1 vídeo de saída por solicitação, até 1 imagem de
entrada (imagem para vídeo), até 4 vídeos de entrada (vídeo para vídeo) e
duração máxima de 10 segundos. Compatível com `size`, `aspectRatio`,
`resolution`, `audio` e `watermark`. Entradas de imagem/vídeo de referência
exigem URLs http(s) remotas; caminhos de arquivos locais são rejeitados
antecipadamente porque o endpoint de vídeo do DashScope não aceita buffers
locais enviados para essas referências.
...
<AccordionGroup>
  <Accordion title="Disponibilidade do Qwen 3.6 e 3.7">
    ...
    mude para o endpoint Standard correspondente e sua respectiva chave.

  </Accordion>

  <Accordion title="Roteamento regional da geração de vídeo">
    O OpenClaw mapeia a região Qwen configurada para o host AIGC correspondente
    do DashScope antes de enviar uma tarefa de vídeo:

    - Global/Internacional: `https://dashscope-intl.aliyuncs.com`
    - China: `https://dashscope.aliyuncs.com`

    Um `models.providers.qwen.baseUrl` normal que aponte para hosts Qwen do
    Coding Plan ou Standard ainda encaminha a geração de vídeo para o endpoint
    de vídeo regional correspondente do DashScope.

  </Accordion>

  <Accordion title="Compatibilidade do uso com streaming">
    Os endpoints nativos do Qwen anunciam compatibilidade do uso com streaming
    no transporte compartilhado `openai-completions`. Assim, IDs de provedores
    personalizados compatíveis com o DashScope que apontem para os mesmos hosts
    nativos herdam o mesmo comportamento sem exigir especificamente o ID de
    provedor integrado `qwen`. Isso se aplica aos endpoints Coding Plan,
  ...
  <Accordion title="Plano de recursos">
    O Plugin `qwen` está sendo posicionado como a solução do fornecedor para
    toda a plataforma Qwen Cloud, não apenas para modelos de programação/texto.

    - **Modelos de texto/chat:** disponíveis por meio do Plugin
    - **Chamadas de ferramentas, saída estruturada e pensamento:** herdados do transporte compatível com OpenAI
    ...
    - **Fala/áudio:** planejados na camada de Plugin do provedor
    - **Embeddings/reordenação de memória:** planejados por meio da interface do adaptador de embeddings
    - **Geração de vídeos:** disponível por meio do Plugin usando o recurso compartilhado de geração de vídeos

  </Accordion>

  <Accordion title="Configuração do ambiente e do daemon">
    Se o Gateway for executado como um daemon (launchd/systemd), certifique-se
    de que `QWEN_API_KEY` ou `QWEN_TOKEN_PLAN_API_KEY` esteja disponível para
    esse processo (por exemplo, em `~/.openclaw/.env` ou por meio de
    `env.shellEnv`).
  </Accordion>
</AccordionGroup>
