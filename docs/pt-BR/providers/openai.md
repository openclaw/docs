...
O OpenClaw oferece suporte a três rotas da família OpenAI. A maioria dos assinantes do ChatGPT/Codex que deseja o comportamento do Codex deve usar o runtime nativo de servidor de aplicativo do Codex. O prefixo do modelo seleciona o nome do provedor/modelo; uma configuração de runtime separada seleciona quem executa o loop de agente embutido:

- **Chave de API** - acesso direto à OpenAI Platform com cobrança baseada em uso (modelos `openai/*`)
- **Assinatura do Codex com runtime nativo do Codex** - login ChatGPT/Codex mais execução pelo servidor de aplicativo do Codex (modelos `openai/*` mais `agents.defaults.agentRuntime.id: "codex"`)
...
| ---------------------------------------------------- | ------------------------------------------------ | ------------------------------------------------------------------------- |
| Assinatura ChatGPT/Codex com runtime nativo do Codex | `openai/gpt-5.5` mais `agentRuntime.id: "codex"` | Configuração recomendada do Codex para a maioria dos usuários. Faça login com autenticação `openai-codex`. |
| Cobrança direta por chave de API                     | `openai/gpt-5.5`                                 | Defina `OPENAI_API_KEY` ou execute o onboarding de chave de API da OpenAI. |
| Autenticação de assinatura ChatGPT/Codex por meio do PI | `openai-codex/gpt-5.5`                           | Use somente quando você quiser intencionalmente o executor PI normal.     |
...
| Plugin `codex`                     | Plugin            | Plugin incluído do OpenClaw que fornece runtime nativo de servidor de aplicativo do Codex e controles de chat `/codex`. |
| `agentRuntime.id: codex`           | Runtime de agente | Força o harness nativo de servidor de aplicativo do Codex para turnos embutidos.                  |
| `/codex ...`                       | Conjunto de comandos de chat | Vincula/controla threads do servidor de aplicativo do Codex a partir de uma conversa.             |
| `runtime: "acp", agentId: "codex"` | Rota de sessão ACP | Caminho de fallback explícito que executa o Codex por ACP/acpx.                                   |

Isso significa que uma configuração pode conter intencionalmente tanto `openai-codex/*` quanto o Plugin `codex`. Isso é válido quando você quer OAuth do Codex por meio do PI e também quer que os controles de chat nativos `/codex` estejam disponíveis. `openclaw doctor` avisa sobre essa combinação para que você possa confirmar que ela é intencional; ele não a reescreve.
...
<Note>
Habilitar o Plugin da OpenAI, ou selecionar um modelo `openai-codex/*`, não habilita o Plugin de servidor de aplicativo do Codex incluído. O OpenClaw habilita esse Plugin somente quando você seleciona explicitamente o harness nativo do Codex com `agentRuntime.id: "codex"` ou usa uma referência de modelo legada `codex/*`.
Se o Plugin `codex` incluído estiver habilitado, mas `openai-codex/*` ainda resolver por meio do PI, `openclaw doctor` avisa e deixa a rota inalterada.
</Note>
...
| Harness de servidor de aplicativo do Codex | `openai/<model>` com `agentRuntime.id: codex` | Sim                                                    |
...
O OpenClaw pode usar a OpenAI, ou um endpoint de embeddings compatível com a OpenAI, para indexação e embeddings de consulta de `memory_search`:
...
<Tabs>
  <Tab title="Chave de API (OpenAI Platform)">
    **Melhor para:** acesso direto à API e cobrança baseada em uso.
    ...
    | `openai/gpt-5.5`       | `agentRuntime.id: "codex"`           | Harness de servidor de aplicativo do Codex | Servidor de aplicativo do Codex |

    <Note>
    `openai/*` é a rota direta com chave de API da OpenAI, a menos que você force explicitamente o harness de servidor de aplicativo do Codex. Use `openai-codex/*` para OAuth do Codex por meio do executor PI padrão, ou use `openai/gpt-5.5` com `agentRuntime.id: "codex"` para execução nativa do servidor de aplicativo do Codex.
    </Note>
  ...
  <Tab title="Assinatura do Codex">
    **Melhor para:** usar sua assinatura ChatGPT/Codex com execução nativa pelo servidor de aplicativo do Codex em vez de uma chave de API separada. A nuvem do Codex exige login ChatGPT.
    ...
    <Steps>
      <Step title="Execute o OAuth do Codex">
        ...
        Para configurações headless ou hostis a callback, adicione `--device-code` para fazer login com um fluxo de código de dispositivo do ChatGPT em vez do callback de navegador localhost:
      ...
      <Step title="Verifique se a autenticação do Codex está disponível">
        ...
        Depois que o gateway estiver em execução, envie `/codex status` ou `/codex models` no chat para verificar o runtime nativo de servidor de aplicativo.
      </Step>
    </Steps>
    ...
    | Referência de modelo | Configuração de runtime | Rota | Autenticação |
    |-----------|----------------|-------|------|
    | `openai/gpt-5.5` | `agentRuntime.id: "codex"` | Harness nativo de servidor de aplicativo do Codex | Login do Codex ou perfil `openai-codex` selecionado |
    | `openai-codex/gpt-5.5` | omitido / `runtime: "pi"` | OAuth ChatGPT/Codex por meio do PI | Login do Codex |
    | `openai-codex/gpt-5.4-mini` | omitido / `runtime: "pi"` | OAuth ChatGPT/Codex por meio do PI | Login do Codex |
    | `openai-codex/gpt-5.5` | `runtime: "auto"` | Ainda PI, a menos que um Plugin reivindique explicitamente `openai-codex` | Login do Codex |

    <Warning>
    Não configure referências de modelo antigas `openai-codex/gpt-5.1*`, `openai-codex/gpt-5.2*` ou `openai-codex/gpt-5.3*`. Contas OAuth ChatGPT/Codex agora rejeitam esses modelos. Use `openai-codex/gpt-5.5` para a rota OAuth do PI, ou `openai/gpt-5.5` com `agentRuntime.id: "codex"` para execução nativa do runtime do Codex.
    </Warning>
    ...
    Ele não seleciona nem habilita automaticamente o ambiente de execução do app-server do Codex incluído. Para
    ...
    Para manter o OAuth do Codex no executor PI normal em vez disso, use
    ...
    ### Verificar e recuperar o roteamento de OAuth do Codex
    ...
    Se uma execução de `doctor --fix` da versão 2026.5.5 alterou uma configuração de assinatura GPT-5.5 de
    `openai-codex/gpt-5.5` para `openai/gpt-5.5`, retorne o agente padrão
    para a rota PI de OAuth do Codex:
    ...
    `openai-codex/*` significa OAuth do ChatGPT/Codex via PI. `openai/*` com
    `agentRuntime.id: "codex"` significa execução nativa do app-server do Codex.

    ### Indicador de status

    O `/status` do chat mostra qual runtime de modelo está ativo para a sessão atual.
    O ambiente PI padrão aparece como `Runtime: OpenClaw Pi Default`. Quando o
    ambiente de app-server do Codex incluído é selecionado, `/status` mostra
    `Runtime: OpenAI Codex`. Sessões existentes mantêm seu id de ambiente registrado; portanto, use
    ...
    selecionada, `openclaw doctor` avisa que o modelo ainda é resolvido via PI.
    Mantenha a configuração inalterada somente quando essa rota PI de autenticação por assinatura for
    intencional. Mude para `openai/<model>` mais `agentRuntime.id: "codex"` quando
    quiser execução nativa do app-server do Codex.
...
O ambiente nativo de app-server do Codex usa referências de modelo `openai/*` mais
`agentRuntime.id: "codex"`, mas sua autenticação ainda é baseada em conta. O OpenClaw
seleciona a autenticação nesta ordem:

1. Um perfil de autenticação explícito `openai-codex` do OpenClaw vinculado ao agente.
2. A conta existente do app-server, como um login local do Codex CLI no ChatGPT.
...
não é enviado a conexões de app-server por WebSocket. Quando um perfil Codex
no estilo assinatura é selecionado, o OpenClaw também mantém `CODEX_API_KEY` e `OPENAI_API_KEY`
fora do filho app-server stdio gerado e envia as credenciais selecionadas
...
Ele oferece suporte à geração de imagens com chave de API da OpenAI e à geração de imagens com OAuth do Codex
...
| Proporção / resolução     | Não encaminhadas para a API Images da OpenAI | Mapeadas para um tamanho compatível quando seguro |
...
<Note>
Consulte [Geração de Imagens](/pt-BR/tools/image-generation) para parâmetros compartilhados de ferramentas, seleção de provedor e comportamento de failover.
</Note>
...
em vez disso.
Se esse endpoint de imagem personalizado estiver em um endereço confiável de LAN/privado, também defina
`browser.ssrfPolicy.dangerouslyAllowPrivateNetwork: true`; o OpenClaw mantém
endpoints de imagem privados/internos compatíveis com OpenAI bloqueados, a menos que essa adesão explícita esteja
presente.
...
<Note>
Consulte [Geração de Vídeo](/pt-BR/tools/video-generation) para parâmetros compartilhados de ferramentas, seleção de provedor e comportamento de failover.
</Note>

## Contribuição de prompt do GPT-5

O OpenClaw adiciona uma contribuição compartilhada de prompt do GPT-5 para execuções da família GPT-5 entre provedores. Ela se aplica por id de modelo, então `openai-codex/gpt-5.5`, `openai/gpt-5.5`, `openrouter/openai/gpt-5.5`, `opencode/gpt-5.5` e outras referências compatíveis de GPT-5 recebem a mesma sobreposição. Modelos GPT-4.x mais antigos não recebem.

O ambiente nativo Codex incluído usa o mesmo comportamento do GPT-5 e a mesma sobreposição de Heartbeat por meio de instruções de desenvolvedor do app-server do Codex, então sessões `openai/gpt-5.x` forçadas por `agentRuntime.id: "codex"` mantêm a mesma orientação de acompanhamento e Heartbeat proativo, mesmo que o Codex controle o restante do prompt do ambiente.

A contribuição do GPT-5 adiciona um contrato de comportamento marcado para persistência de persona, segurança de execução, disciplina de ferramentas, formato de saída, verificações de conclusão e verificação. O comportamento de resposta específico por canal e de mensagens silenciosas permanece no prompt de sistema compartilhado do OpenClaw e na política de entrega de saída. A orientação do GPT-5 está sempre habilitada para modelos correspondentes. A camada de estilo de interação amigável é separada e configurável.
...
<AccordionGroup>
  ...
  <Accordion title="Speech-to-text">
    O Plugin `openai` incluído registra fala para texto em lote por meio da superfície de transcrição de compreensão de mídia do OpenClaw.
    ...
    - Compatível com OpenClaw sempre que a transcrição de áudio de entrada usa
      `tools.media.audio`, incluindo segmentos de canal de voz do Discord e anexos de áudio de canais
  ...
  <Accordion title="Realtime voice">
    ...
    <Note>
    Compatível com Azure OpenAI por meio das chaves de configuração `azureEndpoint` e `azureDeployment` para pontes backend em tempo real. Compatível com chamada bidirecional de ferramentas. Usa o formato de áudio G.711 u-law.
    </Note>

    <Note>
    O Control UI Talk usa sessões em tempo real da OpenAI no navegador com um segredo efêmero de cliente emitido pelo Gateway e uma troca direta de SDP WebRTC do navegador com a OpenAI Realtime API. A verificação live para mantenedores está disponível com `OPENAI_API_KEY=... GEMINI_API_KEY=... node --import tsx scripts/dev/realtime-talk-live-smoke.ts`; a etapa da OpenAI emite um segredo de cliente no Node, gera uma oferta SDP do navegador com mídia de microfone falsa, envia-a para a OpenAI e aplica a resposta SDP sem registrar segredos.
    </Note>
...
O provedor `openai` incluído pode apontar para um recurso Azure OpenAI para geração de imagens ao substituir a URL base. No caminho de geração de imagens, o OpenClaw detecta nomes de host do Azure em `models.providers.openai.baseUrl` e alterna automaticamente para o formato de solicitação do Azure.

<Note>
Voz em tempo real usa um caminho de configuração separado
(`plugins.entries.voice-call.config.realtime.providers.openai.azureEndpoint`)
e não é afetada por `models.providers.openai.baseUrl`. Consulte o acordeão **Voz em tempo real** em [Voz e fala](#voice-and-speech) para suas configurações do Azure.
</Note>
...
- Você quer manter o tráfego dentro de um locatário existente do Azure

### Configuração

Para geração de imagens pelo Azure por meio do provedor `openai` incluído, aponte
...
- Usa um timeout padrão de solicitação de 600s para chamadas de geração de imagens do Azure.
...
`openai.baseUrl` personalizado como o endpoint público da OpenAI e falharão com implantações de imagem do Azure.
</Note>

### Versão da API

Defina `AZURE_OPENAI_API_VERSION` para fixar uma versão preview ou GA específica do Azure para o caminho de geração de imagens do Azure:
...
A mesma regra de nome de implantação se aplica a chamadas de geração de imagens roteadas pelo provedor `openai` incluído.
...
`uaenorth`). Verifique a lista atual de regiões da Microsoft antes de criar uma implantação e confirme se o modelo específico é oferecido na sua região.
...
valores de `background` em `gpt-image-2`) ou expô-las apenas em versões específicas de modelo. Essas diferenças vêm do Azure e do modelo subjacente, não do OpenClaw. Se uma solicitação do Azure falhar com erro de validação, verifique o conjunto de parâmetros compatível com sua implantação específica e versão da API no portal do Azure.

<Note>
O Azure OpenAI usa transporte nativo e comportamento de compatibilidade, mas não recebe os cabeçalhos ocultos de atribuição do OpenClaw — consulte o acordeão **Rotas nativas vs compatíveis com OpenAI** em [Configuração avançada](#advanced-configuration).

Para tráfego de chat ou Responses no Azure (além de geração de imagens), use o fluxo de onboarding ou uma configuração dedicada de provedor Azure — apenas `openai.baseUrl` não ativa o formato de API/autenticação do Azure. Existe um provedor separado
`azure-openai-responses/*`; consulte o acordeão de compactação do lado do servidor abaixo.
</Note>
...
<AccordionGroup>
  <Accordion title="Transport (WebSocket vs SSE)">
    ...
    - Após uma falha, marca o WebSocket como degradado por cerca de 60 segundos e usa SSE durante o período de resfriamento
  ...
  <Accordion title="Fast mode">
    ...
    <Note>
    Substituições da sessão têm precedência sobre a configuração. Limpar a substituição da sessão na UI de Sessões retorna a sessão ao padrão configurado.
    </Note>
  ...
  <Accordion title="Processamento prioritário (service_tier)">
    ...
    <Warning>
    `serviceTier` só é encaminhado para endpoints nativos da OpenAI (`api.openai.com`) e endpoints nativos do Codex (`chatgpt.com/backend-api`). Se você rotear qualquer um dos provedores por um proxy, o OpenClaw deixa `service_tier` inalterado.
    </Warning>

  </Accordion>

  <Accordion title="Compaction no lado do servidor (Responses API)">
    Para modelos OpenAI Responses diretos (`openai/*` em `api.openai.com`), o wrapper de stream Pi-harness do Plugin da OpenAI ativa automaticamente a Compaction no lado do servidor:
    ...
    Isso se aplica ao caminho do harness Pi integrado e aos hooks do provedor OpenAI usados por execuções incorporadas. O harness nativo do servidor de app Codex gerencia seu próprio contexto por meio do Codex e é configurado separadamente com `agents.defaults.agentRuntime.id`.
  ...
  <Accordion title="Modo GPT strict-agentic">
    Para execuções da família GPT-5 em `openai/*`, o OpenClaw pode usar um contrato de execução incorporada mais rigoroso:
  ...
  <Accordion title="Rotas nativas vs. compatíveis com OpenAI">
    ...
    **Rotas nativas** (`openai/*`, Azure OpenAI):
    ...
    - Mantêm a formatação de requisições exclusiva da OpenAI (`service_tier`, `store`, compatibilidade de raciocínio, dicas de cache de prompt)
