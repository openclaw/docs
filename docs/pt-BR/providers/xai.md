...
summary: Use os modelos xAI Grok no OpenClaw
title: xAI
x-i18n:
    generated_at: "2026-05-02T05:55:35Z"
    model: gpt-5.5
    provider: openai
...
o provedor de modelo xAI integrado também reutiliza essa chave como alternativa.
...
O Plugin também resolve para frente IDs mais novos `grok-4*` e
`grok-code-fast*` quando eles seguem o mesmo formato de API.

<Tip>
`grok-4.3`, `grok-4-fast`, `grok-4-1-fast` e as variantes
`grok-4.20-beta-*` são as referências atuais do Grok com suporte a imagens no
catálogo integrado.
</Tip>
...
contratos compartilhados de provedor e ferramentas do OpenClaw. Recursos que não
se encaixam no contrato compartilhado (por exemplo, TTS por streaming e voz em
tempo real) não são expostos — veja a tabela abaixo.
...
| Arquivos / lotes           | Compatibilidade somente com API genérica de modelo | Não é uma ferramenta OpenClaw nativa                         |

<Note>
O OpenClaw usa as APIs REST de imagem/vídeo/TTS/STT da xAI para geração de
mídia, fala e transcrição em lote, o WebSocket STT por streaming da xAI para
transcrição de chamadas de voz ao vivo e a API Responses para ferramentas de
...
como capacidades upstream, e não como comportamento oculto do Plugin.
</Note>
...
<AccordionGroup>
  ...
  <Accordion title="Geração de vídeo">
    ...
    - Duração: 1 a 15 segundos para geração/imagem para vídeo, 1 a 10 segundos ao
      usar funções `reference_image`, 2 a 10 segundos para extensão
    ...
    Para usar a xAI como o provedor de vídeo padrão:
  ...
  <Accordion title="Geração de imagem">
    ...
    armazenada e entregue pelo caminho normal de anexo do canal. Imagens de
    referência locais são convertidas em URLs de dados; referências remotas `http(s)` são
    repassadas.

    Para usar a xAI como o provedor de imagem padrão:
    ...
    controles de imagem compartilhados entre provedores; controles nativos sem suporte
    são intencionalmente não expostos por `image_generate`.
    </Note>
  ...
  <Accordion title="Texto para fala">
    ...
    Para usar a xAI como o provedor de TTS padrão:
  ...
  <Accordion title="Fala para texto">
    O Plugin `xai` integrado registra fala para texto em lote por meio da superfície de
    transcrição de compreensão de mídia do OpenClaw.

    - Modelo padrão: `grok-stt`
    - Endpoint: REST xAI `/v1/stt`
    - Caminho de entrada: upload multipart de arquivo de áudio
    - Compatível no OpenClaw onde quer que a transcrição de áudio de entrada use
    ...
    O idioma pode ser fornecido pela configuração compartilhada de mídia de áudio ou por uma
    solicitação de transcrição por chamada. Dicas de prompt são aceitas pela superfície
    compartilhada do OpenClaw, mas a integração REST STT da xAI encaminha apenas arquivo, modelo e
    idioma porque eles correspondem claramente ao endpoint público atual da xAI.

  </Accordion>

  <Accordion title="Fala para texto por streaming">
    O Plugin `xai` integrado também registra um provedor de transcrição em tempo real
    ...
    - Transcrições parciais: habilitadas por padrão

    O stream de mídia Twilio do Voice Call envia quadros de áudio G.711 µ-law, então o
    provedor xAI pode encaminhar esses quadros diretamente sem transcodificação:
    ...
    A configuração de propriedade do provedor fica em
    ...
    Atualmente, a voz do Discord grava segmentos curtos e usa o caminho de transcrição
  ...
  <Accordion title="Configuração de x_search">
    O Plugin xAI incluído expõe `x_search` como uma ferramenta OpenClaw para pesquisar
  ...
  <Accordion title="Configuração de execução de código">
    O Plugin xAI incluído expõe `code_execution` como uma ferramenta OpenClaw para
  ...
  <Accordion title="Limites conhecidos">
    - A autenticação hoje é apenas por chave de API. Ainda não há fluxo OAuth ou
      de código de dispositivo da xAI no OpenClaw.
    - `grok-4.20-multi-agent-experimental-beta-0304` não é compatível no caminho
      normal do provedor xAI porque exige uma superfície de API upstream
      diferente do transporte xAI padrão do OpenClaw.
    - A voz em tempo real da xAI ainda não está registrada como um provedor OpenClaw. Ela
      precisa de um contrato de sessão de voz bidirecional diferente de STT em lote ou
      transcrição por streaming.
    - `quality` de imagem da xAI, `mask` de imagem e proporções extras exclusivas
      do nativo não são expostas até que a ferramenta compartilhada `image_generate`
      tenha os controles correspondentes entre provedores.
  </Accordion>

  <Accordion title="Notas avançadas">
    - O OpenClaw aplica automaticamente correções de compatibilidade específicas da xAI
      para esquemas de ferramentas e chamadas de ferramentas no caminho do executor compartilhado.
    - As solicitações nativas da xAI usam `tool_stream: true` por padrão. Defina
      ...
      desabilitá-lo.
    - O wrapper xAI incluído remove sinalizadores strict de esquema de ferramentas não compatíveis e
      chaves de payload de raciocínio antes de enviar solicitações nativas da xAI.
    - `web_search`, `x_search` e `code_execution` são expostos como ferramentas OpenClaw.
      O OpenClaw habilita o recurso nativo específico da xAI de que precisa dentro de cada
    ...
    - `x_search` e `code_execution` pertencem ao Plugin xAI incluído, em vez de
      serem codificados diretamente no runtime central do modelo.
    - `code_execution` é execução remota no sandbox da xAI, não o
...
Os caminhos de mídia da xAI são cobertos por testes de unidade e suítes ao vivo opcionais. Os comandos ao vivo carregam segredos do seu shell de login, incluindo `~/.profile`, antes de consultar `XAI_API_KEY`.
...
por STT em tempo real da xAI, gera saída de texto para imagem e edita uma imagem de referência. O
arquivo ao vivo de imagem compartilhado verifica o mesmo provedor xAI por meio do caminho de
