...
read_when:
    ...
    - Você precisa da variável de ambiente da chave da API da Cohere ou da opção de autenticação da CLI
summary: Configuração da Cohere (autenticação + seleção de modelo)
title: Cohere
x-i18n:
    generated_at: "2026-07-12T15:32:17Z"
    model: gpt-5.6
...
[Cohere](https://cohere.com) fornece inferência compatível com a OpenAI por meio de sua API de Compatibilidade. O OpenClaw inclui o provedor Cohere durante sua transição para externalização e também o publica como um plugin externo oficial.
...
| Sinalizador de integração    | `--auth-choice cohere-api-key`                            |
| Sinalizador direto da CLI    | `--cohere-api-key <key>`                                  |
...
Os modelos Cohere com capacidade de raciocínio oferecem suporte a dois modos de raciocínio da API de Compatibilidade. O OpenClaw mapeia **desativado** para `none` e todos os níveis de pensamento habilitados para `high`. O Command A Vision não oferece suporte ao uso de ferramentas, portanto o OpenClaw mantém as ferramentas do agente desabilitadas para esse modelo.

## Primeiros passos

1. A Cohere é fornecida com os pacotes atuais do OpenClaw. Se estiver ausente, instale o pacote externo e reinicie o Gateway:
...
A integração só define a Cohere como o modelo principal quando nenhum modelo principal já está configurado.

## Configuração somente por ambiente

Disponibilize `COHERE_API_KEY` para o processo do Gateway e selecione o modelo Cohere:
