...
title: Mistral
x-i18n:
    generated_at: "2026-07-12T15:40:34Z"
    model: gpt-5.6
...
| Flag de integração   | `--auth-choice mistral-api-key`              |
...
| `mistral/mistral-small-latest`     | texto, imagem | 262,144 | 16,384     | Mistral Small 4 mais recente; `reasoning_effort` ajustável |
| `mistral/mistral-small-2603`       | texto, imagem | 262,144 | 16,384     | Mistral Small 4 fixado; `reasoning_effort` ajustável  |
...
Consulte a linha do catálogo incluído antes de alterar a configuração:
...
openclaw infer model run --local \
  ...
  --prompt "Responda exatamente com: mistral-ok" \
  --json
```

## Transcrição de áudio (Voxtral)

Use o Voxtral para transcrição de áudio em lote pelo pipeline de compreensão de mídia:
...
<Note>
Por padrão, o OpenClaw configura o STT em tempo real da Mistral como `pcm_mulaw` a 8 kHz para que o Voice Call possa encaminhar diretamente os quadros de mídia da Twilio. Use `encoding: "pcm_s16le"` e um `sampleRate` correspondente somente se o seu fluxo de origem já estiver em PCM bruto.
</Note>
...
<AccordionGroup>
  <Accordion title="Raciocínio ajustável">
    `mistral/mistral-small-latest`, `mistral/mistral-small-2603` e `mistral/mistral-medium-3-5` são compatíveis com [raciocínio ajustável](https://docs.mistral.ai/studio-api/conversations/reasoning/adjustable) na API Chat Completions por meio de `reasoning_effort` (`none` minimiza o raciocínio adicional na saída; `high` exibe todos os rastros de raciocínio antes da resposta final).
    ...
    <Warning>
    Evite combinar o modo de raciocínio do Medium 3.5 com `temperature: 0`; há relatos de que a API HTTP da Mistral rejeita `reasoning_effort="high"` junto com `temperature: 0`, retornando uma resposta 400. Deixe a temperatura sem definição ou desative/reduza ao mínimo o raciocínio para que o OpenClaw envie `reasoning_effort: "none"` antes de definir uma temperatura baixa.
    </Warning>

    Exemplo de configuração com escopo de modelo para o raciocínio do Medium 3.5:
    ...
    <Note>
    Os outros modelos incluídos no catálogo da Mistral não usam esse parâmetro. Continue usando modelos `magistral-*` quando quiser o comportamento nativo da Mistral que prioriza o raciocínio.
    </Note>
  ...
  <Accordion title="Autenticação e URL base">
    ...
    - A URL base do provedor usa `https://api.mistral.ai/v1` por padrão e aceita o formato padrão de solicitação de conclusões de chat compatível com OpenAI.
    - O modelo padrão da integração é `mistral/mistral-large-latest`.
...
<CardGroup cols={2}>
  ...
  <Card title="Compreensão de mídia" href="/pt-BR/nodes/media-understanding" icon="microphone">
    Configuração da transcrição de áudio e seleção de provedor.
  </Card>
</CardGroup>
