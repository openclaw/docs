...
summary: Use imagens, vídeos e fala do Vydra no OpenClaw
title: Vydra
x-i18n:
    generated_at: "2026-07-12T00:19:47Z"
    model: gpt-5.6
...
- Síntese de fala por meio da rota TTS da Vydra, baseada no ElevenLabs

O OpenClaw usa a mesma `VYDRA_API_KEY` para as três funcionalidades.
...
| Opção de integração inicial | `--auth-choice vydra-api-key`                                             |
| Opção direta da CLI         | `--vydra-api-key <key>`                                                   |
...
<Warning>
Use `https://www.vydra.ai/api/v1` como URL base. Atualmente, o host raiz da Vydra (`https://vydra.ai/api/v1`) redireciona para `www`. Alguns clientes HTTP removem o cabeçalho `Authorization` nesse redirecionamento entre hosts, fazendo com que uma chave de API válida resulte em uma falha de autenticação enganosa. Para evitar isso, o Plugin incluído normaliza qualquer URL base `vydra.ai` configurada para `www.vydra.ai`.
</Warning>
...
<Steps>
  ...
  <Step title="Escolha uma funcionalidade padrão">
    Escolha uma ou mais das funcionalidades abaixo (imagem, vídeo ou fala) e aplique a configuração correspondente.
  </Step>
</Steps>
...
<AccordionGroup>
  <Accordion title="Geração de imagens">
    ...
    O suporte incluído abrange apenas texto para imagem, com no máximo uma imagem por solicitação. As rotas de edição hospedadas da Vydra esperam URLs de imagens remotas, e o Plugin incluído não adiciona uma ponte de upload específica para a Vydra.
  ...
  <Accordion title="Geração de vídeos">
    ...
    - `vydra/veo3` para texto para vídeo (rejeita entradas de referência de imagem)
    - `vydra/kling` para imagem para vídeo (requer exatamente uma URL de imagem remota)
    ...
    - `vydra/kling` rejeita antecipadamente uploads de arquivos locais; apenas uma referência por URL de imagem remota funciona.
    - A rota HTTP `kling` da Vydra tem apresentado inconsistência quanto à exigência de `image_url` ou `video_url`; o provedor incluído envia a mesma URL de imagem remota nos dois campos.
    - O Plugin incluído mantém uma abordagem conservadora e não encaminha opções de estilo não documentadas, como proporção da imagem, resolução, marca-d'água ou áudio gerado.

    <Note>
    Consulte [Geração de vídeos](/pt-BR/tools/video-generation) para ver os parâmetros compartilhados da ferramenta, a seleção de provedor e o comportamento de failover.
    </Note>
  ...
  <Accordion title="Testes ao vivo de vídeo">
    ...
    Substitua o recurso de teste de imagem remota quando necessário:
...
<CardGroup cols={2}>
  <Card title="Diretório de provedores" href="/pt-BR/providers/index" icon="list">
    Explore todos os provedores disponíveis.
  </Card>
