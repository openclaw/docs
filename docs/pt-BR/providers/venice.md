...
read_when:
    ...
    - Você quer orientações para configurar a Venice AI
summary: Use modelos da Venice AI com foco em privacidade no OpenClaw
title: Venice AI
x-i18n:
    generated_at: "2026-07-12T00:20:59Z"
    model: gpt-5.6
...
[Venice AI](https://venice.ai) oferece inferência com foco em privacidade: modelos abertos são executados
sem registro de dados, além de acesso por proxy anonimizado ao Claude, GPT, Gemini e Grok.
Todos os endpoints são compatíveis com OpenAI (`/v1`).

## Modos de privacidade

| Modo           | Comportamento                                                               | Modelos                                                       |
| -------------- | --------------------------------------------------------------------------- | ------------------------------------------------------------- |
| **Privado**    | Prompts e respostas nunca são armazenados nem registrados. São efêmeros.    | Llama, Qwen, DeepSeek, Kimi, MiniMax, Venice Uncensored etc.  |
| **Anonimizado** | Encaminhado pela Venice, com os metadados removidos antes do envio.         | Claude, GPT, Gemini, Grok                                     |

<Warning>
Os modelos anonimizados não são totalmente privados. A Venice remove os metadados antes do encaminhamento, mas o provedor subjacente (OpenAI, Anthropic, Google, xAI) ainda processa a solicitação. Use modelos Privados quando for necessária privacidade total.
</Warning>
  ...
  <Step title="Configurar o OpenClaw">
    <Tabs>
      <Tab title="Interativo (recomendado)">
        ...
        Solicita a chave de API (ou reutiliza uma `VENICE_API_KEY` existente), lista os modelos disponíveis da Venice e define seu modelo padrão.
      </Tab>
...
- **Opção anonimizada mais avançada**: `venice/claude-opus-4-6`.
...
Você também pode executar `openclaw configure` e selecionar **Provedor de modelo/autenticação > Venice AI**.

<Tip>
| Caso de uso                | Modelo                             | Motivo                                               |
| -------------------------- | ---------------------------------- | ---------------------------------------------------- |
| Conversa geral (padrão)    | `kimi-k2-5`                        | Raciocínio privado avançado com visão                |
| Melhor qualidade geral     | `claude-opus-4-6`                  | Opção anonimizada mais avançada da Venice            |
| Privacidade + programação  | `qwen3-coder-480b-a35b-instruct`   | Modelo privado de programação com contexto amplo     |
| Rápido + econômico         | `qwen3-4b`                         | Modelo leve de raciocínio                            |
| Tarefas privadas complexas | `deepseek-v3.2`                    | Raciocínio avançado; chamadas de ferramentas desativadas |
...
Os modelos da Venice baseados no Grok (`grok-41-fast` e similares) recebem o mesmo patch de compatibilidade
de esquema de ferramentas que o provedor nativo da xAI, pois compartilham o mesmo formato upstream
de chamada de ferramentas.

## Descoberta de modelos

O catálogo integrado acima é uma lista inicial baseada em manifesto. Durante a execução, o OpenClaw
o atualiza por meio da API `/models` da Venice e recorre à lista inicial caso
a API esteja inacessível. O endpoint `/models` é público (não é necessária autenticação para
a listagem), mas a inferência exige uma chave de API válida.

## Comportamento de reprodução do DeepSeek V4

Se a Venice disponibilizar modelos DeepSeek V4, como `deepseek-v4-pro` ou
`deepseek-v4-flash`, o OpenClaw preenche o campo obrigatório de reprodução
`reasoning_content` nas mensagens do assistente quando a Venice o omite e remove `thinking`/
`reasoning`/`reasoning_effort` do payload da solicitação (a Venice rejeita
o controle nativo `thinking` do DeepSeek nesses modelos). Essa correção de reprodução é
separada dos controles de pensamento do próprio provedor nativo do DeepSeek.
...
| Chamada de funções  | Maioria dos modelos; desativada por modelo quando indicado acima |
...
o preço direto da API, acrescido de uma pequena taxa da Venice. Consulte
...
<AccordionGroup>
  ...
  <Accordion title="Modelo indisponível">
    ...
    disponíveis no momento; o catálogo muda conforme a Venice adiciona ou descontinua modelos.
  </Accordion>
...
<CardGroup cols={2}>
  ...
  <Card title="Preços" href="https://venice.ai/pricing" icon="credit-card">
    Tarifas de créditos e planos atuais da Venice.
  </Card>
</CardGroup>
