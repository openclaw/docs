...
read_when:
    ...
    - Você quer orientação para configurar a Venice AI
summary: Usar modelos focados em privacidade da Venice AI no OpenClaw
title: Venice AI
x-i18n:
    generated_at: "2026-04-24T06:09:35Z"
    model: gpt-5.4
    provider: openai
...
A Venice AI oferece **inferência de IA focada em privacidade**, com suporte a modelos sem censura e acesso a grandes modelos proprietários por meio do proxy anonimizado deles. Toda inferência é privada por padrão — sem treinamento com seus dados, sem logging.
...
| **Anonymized**  | Encaminhado pela Venice com metadados removidos. O provedor subjacente (OpenAI, Anthropic, Google, xAI) vê requisições anonimizadas. | Claude, GPT, Gemini, Grok                                  |

<Warning>
Modelos anonymized **não** são totalmente privados. A Venice remove metadados antes do encaminhamento, mas o provedor subjacente (OpenAI, Anthropic, Google, xAI) ainda processa a solicitação. Escolha modelos **Private** quando a privacidade total for obrigatória.
</Warning>
...
- **Acesso a grandes modelos**: use Claude, GPT, Gemini e Grok via proxy anonimizado da Venice
...
- **Sem limites rígidos de taxa**: throttling de uso justo pode se aplicar em uso extremo
...
| **Privacidade + código**    | `qwen3-coder-480b-a35b-instruct` | Modelo privado de código com grande contexto  |
...
<AccordionGroup>
  <Accordion title="Modelos Private (26) — totalmente privados, sem logging">
  ...
  <Accordion title="Modelos Anonymized (15) — via proxy da Venice">
    | ID do modelo                     | Nome                            | Contexto | Recursos                  |
    | -------------------------------- | ------------------------------- | -------- | ------------------------- |
...
O OpenClaw descobre automaticamente modelos a partir da API da Venice quando `VENICE_API_KEY` está definido. Se a API estiver inacessível, ele usa fallback para um catálogo estático.

O endpoint `/models` é público (não precisa de autenticação para listagem), mas a inferência exige uma chave de API válida.
...
| **Vision/Imagens**    | Modelos marcados com o recurso "Vision"                |
...
- **Modelos Private**: custo geralmente menor
- **Modelos Anonymized**: semelhantes ao preço direto da API + pequena taxa da Venice
...
# Usar modelo de vision com imagem
...
<AccordionGroup>
  ...
  <Accordion title="Problemas de conexão">
    A API da Venice fica em `https://api.venice.ai/api/v1`. Certifique-se de que sua rede permite conexões HTTPS.
  </Accordion>
</AccordionGroup>
