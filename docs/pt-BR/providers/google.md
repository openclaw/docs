...
title: Google (Gemini)
x-i18n:
    generated_at: "2026-05-10T19:47:27Z"
    model: gpt-5.5
    provider: openai
...
- Opção de runtime: provider/model `agentRuntime.id: "google-gemini-cli"`
  reutiliza o OAuth do Gemini CLI enquanto mantém as refs de modelo canônicas como `google/*`.
...
<Tabs>
  ...
  <Tab title="Gemini CLI (OAuth)">
    **Ideal para:** reutilizar um login existente do Gemini CLI via OAuth PKCE em vez de uma chave de API separada.
    ...
    O ID de modelo da API Gemini para o Gemini 3.1 Pro é `gemini-3.1-pro-preview`. O OpenClaw aceita o `google/gemini-3.1-pro` mais curto como um alias de conveniência e o normaliza antes das chamadas ao provedor.
    ...
    **Variáveis de ambiente:**
    ...
    <Note>
    Se o login falhar antes do fluxo do navegador começar, verifique se o comando local `gemini`
    está instalado e em `PATH`.
    </Note>

    As refs de modelo `google-gemini-cli/*` são aliases de compatibilidade legada. Novas
      ...
      google: {
        config: {
          webSearch: {
            ...
            baseUrl: "https://generativelanguage.googleapis.com/v1beta", // usa models.providers.google.baseUrl como fallback
...
existe para proxies de operador ou endpoints compatíveis da API Gemini; quando omitido,
...
- Execuções apoiadas por sessão se destacam pelo fluxo compartilhado de tarefa/status, incluindo `action: "status"`
...
- Saída: WAV para anexos TTS regulares, Opus para destinos de recado de voz, PCM para Talk/telefonia
- Saída de recado de voz: o PCM do Google é encapsulado como WAV e transcodificado para Opus a 48 kHz com `ffmpeg`

O caminho TTS Gemini em lote do Google retorna áudio gerado na resposta concluída
`generateContent`. Para conversas faladas com a menor latência, use o
...
O OpenClaw adapta o áudio da ponte de telefonia/Meet ao fluxo da PCM Live API
do Gemini e mantém chamadas de ferramenta no contrato compartilhado de voz em
tempo real. Deixe `temperature` indefinido, a menos que você precise de
alterações de amostragem; o OpenClaw omite valores não positivos porque o Google
Live pode retornar transcrições sem áudio para `temperature: 0`. A transcrição
da API Gemini é ativada sem `languageCodes`; o SDK atual do Google rejeita dicas
de código de idioma neste caminho de API.
</Note>

<Note>
A Control UI Talk oferece suporte a sessões do Google Live no navegador com
tokens restritos de uso único. Provedores de voz em tempo real somente de backend
também podem ser executados pelo transporte de retransmissão genérico do
...
O smoke também cobre caminhos de backend/WebRTC da OpenAI; a etapa do Google
emite o mesmo formato de token restrito da Live API usado pela Control UI Talk,
abre o endpoint WebSocket do navegador, envia a carga útil inicial de configuração
...
<AccordionGroup>
  <Accordion title="Reutilização direta do cache do Gemini">
    Para execuções diretas da API Gemini (`api: "google-generative-ai"`), o
    OpenClaw repassa um identificador `cachedContent` configurado para as
    ...
    - O uso de acertos de cache do Gemini é normalizado para `cacheRead` do
      OpenClaw a partir de `cachedContentTokenCount` upstream
  ...
  <Accordion title="Notas de uso de JSON do Gemini CLI">
    Ao usar o provedor OAuth `google-gemini-cli`, o OpenClaw normaliza a saída
    JSON da CLI da seguinte forma:
  ...
  <Accordion title="Configuração de ambiente e daemon">
    ...
    `GEMINI_API_KEY` esteja disponível para esse processo (por exemplo, em
