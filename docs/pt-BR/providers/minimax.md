...
summary: Use modelos da MiniMax no OpenClaw
title: MiniMax
x-i18n:
    generated_at: "2026-05-02T05:55:04Z"
    model: gpt-5.5
    provider: openai
...
<Tabs>
  ...
  <Tab title="API key">
    **Melhor para:** MiniMax hospedado com API compatível com Anthropic.
    ...
    <Warning>
    No caminho de streaming compatível com Anthropic, o OpenClaw desativa o pensamento da MiniMax por padrão, a menos que você defina explicitamente `thinking`. O endpoint de streaming da MiniMax emite `reasoning_content` em fragmentos delta no estilo OpenAI, em vez de blocos de pensamento nativos da Anthropic, o que pode vazar raciocínio interno para a saída visível se ficar habilitado implicitamente.
    </Warning>
...
O Plugin `minimax` integrado registra o MiniMax T2A v2 como provedor de fala para
`messages.tts`.

- Modelo TTS padrão: `speech-2.8-hd`
- Voz padrão: `English_expressive_narrator`
- IDs de modelos integrados compatíveis incluem `speech-2.8-hd`, `speech-2.8-turbo`,
...
- A resolução de autenticação é `messages.tts.providers.minimax.apiKey`, depois
  perfis de autenticação OAuth/token `minimax-portal`, depois chaves de ambiente do
  Plano de Token (`MINIMAX_OAUTH_TOKEN`, `MINIMAX_CODE_PLAN_KEY`,
  `MINIMAX_CODING_API_KEY`) e, por fim, `MINIMAX_API_KEY`.
...
- Destinos de notas de voz, como Feishu e Telegram, são transcodificados de MP3 do MiniMax
  para Opus a 48 kHz com `ffmpeg`, porque a API de arquivos do Feishu/Lark só
...
O Plugin MiniMax integrado registra a geração de música por meio da ferramenta compartilhada
...
O Plugin MiniMax integrado registra a geração de vídeo por meio da ferramenta compartilhada
...
O Plugin MiniMax registra a compreensão de imagem separadamente do catálogo de
texto:
...
quando o catálogo de provedor de texto incluído ainda mostra refs de chat M2.7 somente texto.
...
<Note>
Consulte [Pesquisa MiniMax](/pt-BR/tools/minimax-search) para configuração e uso completos da pesquisa na web.
</Note>
