...
summary: Referência da CLI para `openclaw health` (resumo da integridade do Gateway via RPC)
title: Saúde
x-i18n:
    generated_at: "2026-07-11T23:48:41Z"
    model: gpt-5.6
...
Obtém um instantâneo de integridade do Gateway em execução por meio de RPC via WebSocket (sem conexões diretas da CLI aos canais).
...
- Sem `--verbose`, o Gateway pode retornar um instantâneo em cache (válido por até 60 segundos e sem alterações em relação ao estado de execução dos canais em tempo real) e atualizá-lo em segundo plano para o próximo solicitante.
- `--verbose` força uma sondagem em tempo real (sondagens de contas por canal), exibe detalhes da conexão com o Gateway e expande a saída legível por humanos para todas as contas e todos os agentes configurados, em vez de mostrar apenas o agente padrão.
