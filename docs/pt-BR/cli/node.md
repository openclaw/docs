...
Execute um **host Node sem interface gráfica** que se conecta ao WebSocket do Gateway e expõe
...
`openclaw node run` pode publicar ferramentas fornecidas por plugins ou baseadas em MCP após se conectar.
...
não exige novo emparelhamento. Consulte
...
Se necessário, desative-o no Node:
...
- `--tls`: usar TLS na conexão com o Gateway
...
- `--node-id <id>`: substituir o ID legado da instância do cliente armazenado em `node.json` (não redefine o emparelhamento)
- `--display-name <name>`: substituir o nome de exibição do Node

## Autenticação do Gateway para o host Node

`openclaw node run` e `openclaw node install` resolvem a autenticação do Gateway pela configuração/ambiente (não há flags `--token`/`--password` nos comandos do Node):

- `OPENCLAW_GATEWAY_TOKEN` / `OPENCLAW_GATEWAY_PASSWORD` são verificados primeiro.
- Em seguida, usa-se o fallback da configuração local: `gateway.auth.token` / `gateway.auth.password`.
- No modo local, o host Node não herda intencionalmente `gateway.remote.token` / `gateway.remote.password`.
- Se `gateway.auth.token` / `gateway.auth.password` estiver configurado explicitamente via SecretRef e não puder ser resolvido, a resolução da autenticação do Node falhará de forma segura (sem fallback remoto que mascare a falha).
...
Para um Node que se conecta a um Gateway `ws://` em texto simples, são aceitos
loopback, literais de IP privado, `.local` e hosts Tailnet `*.ts.net`. Para outros
nomes DNS privados confiáveis, defina `OPENCLAW_ALLOW_INSECURE_PRIVATE_WS=1`; sem
isso, a inicialização do Node falhará de forma segura e solicitará o uso de `wss://`, um túnel SSH ou
...
O host Node tenta novamente, dentro do processo, após reinicializações do Gateway e encerramentos de rede. Se o
Gateway relatar uma pausa terminal na autenticação por token/senha/bootstrap, o host Node
registra os detalhes do encerramento e termina com código diferente de zero, para que launchd/systemd/Agendador de Tarefas possa
reiniciá-lo com configurações e credenciais atualizadas. Pausas que exigem emparelhamento permanecem no
...
Quando o host do Gateway consegue se conectar ao host Node via SSH de forma não interativa (mesmo usuário,
chave de host confiável), a solicitação pendente é aprovada automaticamente: o Gateway
executa `openclaw node identity --json` no host Node por SSH e aprova quando há
correspondência exata da chave do dispositivo. Isso fica habilitado por padrão; consulte
[Aprovação automática de dispositivo verificada por SSH](/pt-BR/gateway/pairing#ssh-verified-device-auto-approval-default)
para ver os requisitos e como desativá-la (`gateway.nodes.pairing.sshVerify: false`).
...
Em redes de nós rigidamente controladas, o operador do Gateway pode habilitar explicitamente
a aprovação automática do primeiro emparelhamento de nós provenientes de CIDRs confiáveis:
...
Isso fica desativado por padrão (`autoApproveCidrs` não está definido). Aplica-se apenas a
novos emparelhamentos com `role: node`, sem escopos solicitados e provenientes de um IP de cliente em que o
...
### Estado de identidade e emparelhamento

O Node sem interface gráfica separa o ID legado da instância do cliente da identidade assinada do dispositivo
que o Gateway usa para emparelhamento e roteamento. Esses arquivos ficam no diretório de estado do
OpenClaw (`~/.openclaw` por padrão, ou `$OPENCLAW_STATE_DIR`
quando definido):

| Arquivo                     | Finalidade                                                                                                                                                                          |
| --------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `node.json`                 | ID da instância do cliente na chave legada `nodeId`, nome de exibição e metadados de conexão do Gateway. O cliente envia esse valor como `instanceId`.                               |
| `identity/device.json`      | Par de chaves Ed25519 assinado e ID de dispositivo derivado. Para conexões assinadas, esse ID de dispositivo é o ID do Node usado no roteamento e a identidade de emparelhamento.    |
| `identity/device-auth.json` | Tokens de dispositivos emparelhados, indexados pelo ID criptográfico do dispositivo e pela função.                                                                                  |

`--node-id` altera somente o ID da instância do cliente em `node.json`. Ele não
altera o ID criptográfico do dispositivo nem limpa a autenticação do emparelhamento. Da mesma forma, excluir apenas
...
2. No Node, reinicie o serviço instalado com `openclaw node restart` ou
   pare e execute novamente o comando em primeiro plano `openclaw node run`. Isso inicia o
   fluxo de emparelhamento do dispositivo. Se `openclaw devices list` não mostrar uma solicitação
   e o Node relatar `AUTH_DEVICE_TOKEN_MISMATCH`, reinicie-o ou execute-o novamente mais
   uma vez. A tentativa rejeitada limpa o token local agora revogado; a próxima
...
Versões mais antigas do OpenClaw podiam deixar um campo legado `token` em `node.json`.
...
Para execução assíncrona aprovada no Node, o OpenClaw prepara um `systemRunPlan` canônico
antes da solicitação de aprovação. O encaminhamento posterior e aprovado de `system.run` reutiliza esse
plano armazenado; portanto, edições nos campos de comando/cwd/sessão após a criação da solicitação de aprovação
são rejeitadas, em vez de alterar o que o Node executa.
