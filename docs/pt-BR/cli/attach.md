...
title: Anexar CLI
x-i18n:
    generated_at: "2026-07-12T14:59:12Z"
    model: gpt-5.6
...
`openclaw attach` inicia o Claude Code com uma configuração MCP temporária estrita vinculada a uma sessão do Gateway.
...
O token bearer é passado por variáveis de ambiente, não por argv. O OpenClaw inicia o Claude Code com `--strict-mcp-config --mcp-config <path>` para que servidores MCP do Claude presentes no ambiente não sejam adicionados à sessão anexada. Inicializações normais (sem `--print-config`) revogam a concessão quando o processo do Claude Code é encerrado.

Veja também: [CLI do Gateway](/pt-BR/cli/gateway), [CLI do MCP](/pt-BR/cli/mcp) e [CLI do ACP](/pt-BR/cli/acp).
