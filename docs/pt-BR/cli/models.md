...
read_when:
    ...
    - Você deseja examinar os modelos/provedores disponíveis e depurar perfis de autenticação
...
- Conceitos de seleção de modelo + comando slash `/models`: [Conceito de modelos](/pt-BR/concepts/models)
...
enquanto `auth.oauth` é apenas a integridade dos perfis do store de autenticação.
...
- `models list` é somente leitura: ele lê a configuração, os perfis de autenticação, o estado de catálogo
  existente e as linhas de catálogo pertencentes ao provedor, mas não reescreve
  `models.json`.
- A coluna `Auth` é em nível de provedor e somente leitura. Ela é calculada a partir de
  metadados de perfil de autenticação local, marcadores de env, chaves de provedor configuradas, marcadores
  de provedor local, marcadores de env/perfil do AWS Bedrock e metadados de autenticação sintética de Plugin;
  ela não carrega o runtime do provedor, lê segredos do keychain, chama APIs de provedor
...
- `models list --all --provider <id>` pode incluir linhas de catálogo estático pertencentes ao provedor
  de manifestos de Plugin ou metadados de catálogo de provedores integrados, mesmo quando você
...
- `models list` mantém o plano de controle responsivo enquanto a descoberta do catálogo do provedor
  está lenta. As visualizações padrão e configuradas recorrem a linhas de modelo configuradas ou
  sintéticas após uma espera curta e deixam a descoberta terminar em
  segundo plano. Use `--all` quando precisar do catálogo descoberto completo e exato e
...
- `models list --all` amplo mescla linhas de catálogo de manifesto sobre linhas do registro
  ...
  permanecem baseados em registro/cache e acrescentam linhas de manifesto como suplementos, enquanto
...
- Se você omitir o provedor, o OpenClaw resolve a entrada primeiro como um alias, depois
  como uma correspondência exclusiva de provedor configurado para esse id de modelo exato, e só então
...
- `--no-probe` (somente metadados; sem busca de configuração/segredos)
- `--min-params <b>`
- `--max-age-days <days>`
- `--provider <name>`
- `--max-candidates <n>`
...
- `--json`
- `--plain`
- `--check` (códigos de saída 1=expirado/ausente, 2=expirando)
- `--probe` (probe ao vivo de perfis de autenticação configurados)
...
e inicialização são roteados para stderr para que scripts possam encadear stdout diretamente
em ferramentas como `jq`.
...
Casos de detalhe/código de motivo de probe esperados:
...
`models auth add` é o assistente interativo de autenticação. Ele pode iniciar um fluxo de autenticação
de provedor (OAuth/chave de API) ou orientar você na colagem manual de token, dependendo do
...
- `setup-token` exige um TTY interativo e executa o método de autenticação por token
  do provedor (usando por padrão o método `setup-token` desse provedor quando ele expõe
  um).
...
- Observação sobre Anthropic: a equipe da Anthropic nos informou que o uso do Claude CLI no estilo do OpenClaw é permitido novamente, então o OpenClaw trata a reutilização do Claude CLI e o uso de `claude -p` como sancionados para esta integração, a menos que a Anthropic publique uma nova política.
- Anthropic `setup-token` / `paste-token` continuam disponíveis como um caminho de token do OpenClaw com suporte, mas o OpenClaw agora prefere a reutilização do Claude CLI e `claude -p` quando disponíveis.
