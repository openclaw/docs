...
read_when:
    ...
    - Você precisa da opção de configuração exclusiva da linha de base para scripts
...
`openclaw setup` executa o mesmo fluxo guiado de integração que `openclaw onboard`:
primeiro, ele verifica e persiste a inferência e, em seguida, inicia o Crestodian para configurar
o espaço de trabalho, o Gateway, os canais, as Skills e a integridade. Use `--baseline` quando você
precisar apenas inicializar as pastas de configuração/espaço de trabalho sem o assistente.

No modo guiado, `--workspace <dir>` é o espaço de trabalho proposto ao Crestodian;
ele é persistido somente após você aprovar essa proposta. As configurações básica, clássica e
não interativa persistem o espaço de trabalho fornecido por meio de seus fluxos normais.

`setup` aceita as mesmas opções de integração que `openclaw onboard`, incluindo
...
[Automação da CLI](/pt-BR/start/wizard-cli-automation) para ver a referência completa das opções e
exemplos não interativos. `openclaw onboard --modern` é o alias de compatibilidade
para o assistente Crestodian condicionado à inferência e não tem equivalente em `setup`.
...
| `--baseline`               | Cria as pastas básicas de configuração, espaço de trabalho e sessão sem integração.                               |
| `--wizard`                 | Aceita para compatibilidade; a configuração executa a integração por padrão.                                      |
| `--non-interactive`        | Executa a integração sem prompts.                                                                                  |
| `--accept-risk`            | Confirma o risco de acesso do agente a todo o sistema; obrigatória com `--non-interactive`.                        |
| `--mode <mode>`            | Modo de integração: `local` ou `remote`.                                                                           |
| `--flow <flow>`            | Fluxo de integração: `quickstart`, `advanced`, `manual` ou `import`.                                               |
| `--reset`                  | Redefine a configuração + credenciais + sessões antes da integração (o espaço de trabalho somente com `--reset-scope full`). |
...
| `--import-secrets`         | Importa os segredos compatíveis durante a migração da integração.                                                 |
...
`openclaw setup --baseline` preserva o comportamento anterior, restrito à configuração básica: ele
...
- Após a configuração básica, execute `openclaw setup` ou `openclaw onboard` para realizar todo o processo guiado, `openclaw configure` para alterações específicas ou `openclaw channels add` para adicionar contas de canais.
- Se o estado do Hermes for detectado, a integração interativa poderá oferecer a migração automaticamente. A integração por importação requer uma configuração nova; use [Migrar](/pt-BR/cli/migrate) para planos de simulação, backups e modo de substituição fora da integração.
