...
summary: Referência da CLI para `openclaw tui` (interface de usuário de terminal com suporte do Gateway ou incorporada localmente)
title: TUI
x-i18n:
    generated_at: "2026-07-12T15:03:43Z"
    model: gpt-5.6
...
Abra a interface de terminal conectada ao Gateway ou execute-a no modo
local incorporado.
...
| ---------------------------- | ----------------------------------------- | ------------------------------------------------------------------------------------------ |
| `--local`                    | `false`                                   | Executa usando o runtime local incorporado do agente em vez de um Gateway.                 |
...
| `--session <key>`            | `main` (ou `global` quando o escopo é global) | Chave da sessão. Dentro do espaço de trabalho de um agente, seleciona automaticamente esse agente, a menos que haja um prefixo. |
...
| `--history-limit <n>`        | `200`                                     | Entradas do histórico a serem carregadas ao conectar.                                      |
...
- `--local` não pode ser combinado com `--url`, `--token`, `--password` ou `--tls-fingerprint`.
- `tui` resolve as SecretRefs de autenticação configuradas do Gateway para autenticação
  por token/senha quando possível (provedores `env`/`file`/`exec`).
- Sem uma URL ou porta explícita, `tui` usa a porta ativa do Gateway local
...
- Quando iniciada dentro do diretório de um espaço de trabalho de agente configurado, a TUI seleciona
  automaticamente esse agente como padrão da chave da sessão (a menos que `--session` seja explicitamente
  `agent:<id>:...`).
- Para mostrar o nome do host do Gateway no rodapé em conexões não locais
  baseadas em URL, execute `openclaw config set tui.footer.showRemoteHost true`. Desativado por
  padrão; nunca é mostrado para conexões de loopback ou locais incorporadas.
- O modo local usa diretamente o runtime incorporado do agente. A maioria das ferramentas locais funciona,
  mas os recursos exclusivos do Gateway ficam indisponíveis.
- O modo local adiciona `/auth [provider]` à superfície de comandos da TUI.
- As verificações de aprovação de Plugin ainda se aplicam no modo local: ferramentas que exigem aprovação
...
com a documentação e ajude a repará-la no mesmo terminal.
