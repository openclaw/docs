...
x-i18n:
    generated_at: "2026-05-02T05:49:38Z"
    model: gpt-5.5
    provider: openai
...
Para início rápido, executores de QA, suítes de unidade/integração e fluxos do Docker, consulte
...
## Ao vivo: comandos de verificação rápida do perfil local

Use `~/.profile` como fonte antes de verificações ao vivo ad hoc, para que as chaves de provedores e os caminhos de ferramentas locais correspondam ao seu shell:

```bash
source ~/.profile
```

Verificação rápida segura de mídia:
...
Verificação rápida segura de prontidão para chamada de voz:
...
privados/locais de local loopback são rejeitados por definição.
...
## Ao vivo: verificação rápida de modelos (chaves de perfil)

Os testes ao vivo são divididos em duas camadas para que possamos isolar falhas:

- “Modelo direto” indica se o provedor/modelo consegue responder de alguma forma com a chave fornecida.
- “Verificação rápida do Gateway” indica se o pipeline completo de Gateway+agente funciona para esse modelo (sessões, histórico, ferramentas, política de sandbox etc.).
...
- Defina `OPENCLAW_LIVE_MODELS=modern` (ou `all`, alias para modern) para realmente executar esta suíte; caso contrário, ela é ignorada para manter `pnpm test:live` focado na verificação rápida do Gateway
- Como selecionar modelos:
  ...
  - As sondagens de modelo direto rodam com paralelismo de 20 vias por padrão; defina `OPENCLAW_LIVE_MODEL_CONCURRENCY` para substituir.
...
- Por que isso existe:
  ...
  - Contém regressões pequenas e isoladas (exemplo: replay de raciocínio do OpenAI Responses/Codex Responses + fluxos de chamadas de ferramentas)

### Camada 2: Gateway + verificação rápida do agente de desenvolvimento (o que "@openclaw" realmente faz)
...
- Objetivo:
  ...
  - Iterar por modelos com chaves e validar:
    ...
    - sondagens opcionais extras de ferramentas (sondagem de execução+leitura)
...
- Detalhes das sondagens (para você explicar falhas rapidamente):
  - Sondagem `read`: o teste grava um arquivo com nonce no workspace e pede ao agente para fazer `read` dele e ecoar o nonce de volta.
  - Sondagem `exec+read`: o teste pede ao agente para usar `exec` para gravar um nonce em um arquivo temporário e depois fazer `read` dele.
...
- Como selecionar modelos:
  ...
  - Varreduras de Gateway modern/all usam por padrão um limite selecionado de alto sinal; defina `OPENCLAW_LIVE_GATEWAY_MAX_MODELS=0` para uma varredura moderna exaustiva ou um número positivo para um limite menor.
...
## Ao vivo: verificação rápida de backend de CLI (Claude, Codex, Gemini ou outras CLIs locais)

- Teste: `src/gateway/gateway-cli-backend.live.test.ts`
- Objetivo: validar o pipeline de Gateway + agente usando um backend de CLI local, sem tocar na sua configuração padrão.
- Os padrões de verificação rápida específicos de backend ficam na definição `cli-backend.ts` do Plugin proprietário.
- Habilitar:
  - `pnpm test:live` (ou `OPENCLAW_LIVE_TEST=1` se invocar o Vitest diretamente)
...
- Substituições (opcionais):
  ...
  - `OPENCLAW_LIVE_CLI_BACKEND_MODEL_SWITCH_PROBE=1` para optar pela sondagem de continuidade na mesma sessão de Claude Sonnet -> Opus quando o modelo selecionado aceita um alvo de troca. Receitas Docker deixam isso desativado por padrão para confiabilidade agregada.
  - `OPENCLAW_LIVE_CLI_BACKEND_MCP_PROBE=1` para optar pela sondagem MCP/ferramenta de local loopback. Receitas Docker deixam isso desativado por padrão, a menos que solicitado explicitamente.
...
Verificação rápida barata de configuração MCP do Gemini:
...
e consegue se conectar a um servidor MCP local de HTTP transmitível.
...
- Ele executa a verificação rápida ao vivo de backend de CLI dentro da imagem Docker do repositório como o usuário não root `node`.
- Ele resolve os metadados de verificação rápida de CLI a partir do Plugin proprietário e então instala o pacote de CLI Linux correspondente (`@anthropic-ai/claude-code`, `@openai/codex` ou `@google/gemini-cli`) em um prefixo gravável em cache em `OPENCLAW_DOCKER_CLI_TOOLS_DIR` (padrão: `~/.cache/openclaw/docker-cli-tools`).
- `pnpm test:docker:live-cli-backend:claude-subscription` exige OAuth portátil de assinatura do Claude Code por meio de `~/.claude/.credentials.json` com `claudeAiOauth.subscriptionType` ou `CLAUDE_CODE_OAUTH_TOKEN` de `claude setup-token`. Primeiro ele comprova `claude -p` direto no Docker, depois executa duas rodadas de backend de CLI do Gateway sem preservar variáveis de ambiente de chave de API da Anthropic. Esta faixa de assinatura desativa por padrão as sondagens MCP/ferramenta e de imagem do Claude porque atualmente o Claude encaminha o uso por apps de terceiros por cobrança de uso extra em vez dos limites normais do plano de assinatura.
- A verificação rápida ao vivo de backend de CLI agora exercita o mesmo fluxo de ponta a ponta para Claude, Codex e Gemini: rodada de texto, rodada de classificação de imagem e então chamada de ferramenta `cron` MCP verificada pela CLI do Gateway.
- A verificação rápida padrão do Claude também corrige a sessão de Sonnet para Opus e verifica que a sessão retomada ainda se lembra de uma anotação anterior.

## Ao vivo: verificação rápida de bind ACP (`/acp spawn ... --bind here`)
...
- Observações:
  - Esta lane usa a superfície `chat.send` do Gateway com campos de rota de origem sintética exclusivos de admin para que os testes possam anexar contexto de canal de mensagens sem fingir entrega externa.
  ...
  - A criação de MCP Cron de sessão vinculada é de melhor esforço por padrão porque harnesses ACP externos podem cancelar chamadas MCP depois que a prova de bind/imagem passou; defina `OPENCLAW_LIVE_ACP_BIND_REQUIRE_CRON=1` para tornar essa sondagem de Cron pós-bind estrita.
...
- Por padrão, ele executa o smoke de bind ACP contra os agentes CLI ao vivo agregados em sequência: `claude`, `codex`, depois `gemini`.
...
- Ele carrega `~/.profile`, prepara o material de autenticação da CLI correspondente no contêiner e instala a CLI ao vivo solicitada (`@anthropic-ai/claude-code`, `@openai/codex`, Factory Droid via `https://app.factory.ai/cli`, `@google/gemini-cli` ou `opencode-ai`) se estiver ausente. O próprio backend ACP é o pacote incorporado `acpx/runtime` incluído no Plugin `acpx`.
- A variante Docker do Droid prepara `~/.factory` para configurações, encaminha `FACTORY_API_KEY` e exige essa chave de API porque a autenticação OAuth/keyring local do Factory não é portátil para dentro do contêiner. Ela usa a entrada de registro integrada do ACPX `droid exec --output-format acp`.
- A variante Docker do OpenCode é uma lane de regressão estrita de agente único. Ela grava um modelo padrão temporário em `OPENCODE_CONFIG_CONTENT` a partir de `OPENCLAW_LIVE_ACP_BIND_OPENCODE_MODEL` (padrão `opencode/kimi-k2.6`) depois de carregar `~/.profile`, e `pnpm test:docker:live-acp-bind:opencode` exige uma transcrição de assistente vinculada em vez de aceitar o salto genérico pós-bind.
- Chamadas diretas à CLI `acpx` são apenas um caminho manual/de contorno para comparar comportamento fora do Gateway. O smoke Docker de bind ACP exercita o backend runtime `acpx` incorporado do OpenClaw.
...
- Objetivo: validar o harness Codex de propriedade do Plugin pelo método normal
  ...
  - opcionalmente executar duas sondagens de shell escaladas revisadas pelo Guardian: um comando benigno
    que deve ser aprovado e um upload de segredo falso que deve ser negado para que o agente pergunte de volta
...
- O smoke define `OPENCLAW_AGENT_HARNESS_FALLBACK=none` para que um harness Codex quebrado
  não possa passar fazendo fallback silencioso para PI.
- Autenticação: autenticação de app-server do Codex pelo login da assinatura local do Codex. Smokes Docker
...
- O Docker também exporta `OPENCLAW_AGENT_HARNESS_FALLBACK=none`, correspondendo à configuração do teste
  ao vivo para que aliases legados ou fallback para PI não possam ocultar uma regressão do harness Codex.
...
- API Gemini vs CLI Gemini:
  - API: OpenClaw chama a API Gemini hospedada do Google via HTTP (chave de API / autenticação de perfil); é isso que a maioria dos usuários quer dizer com “Gemini”.
  - CLI: OpenClaw chama por shell um binário `gemini` local; ele tem sua própria autenticação e pode se comportar de forma diferente (suporte a streaming/ferramentas/desalinhamento de versão).
...
Cobertura adicional opcional (boa de ter):
...
<Tip>
Não codifique "todos os modelos" nos docs. A lista autoritativa é o que `discoverModels(...)` retorna na sua máquina mais as chaves disponíveis.
</Tip>
...
- Se a CLI funcionar, os testes ao vivo devem encontrar as mesmas chaves.
...
- Diretório de estado legado: `~/.openclaw/credentials/` (copiado para o diretório inicial live preparado quando presente, mas não para o armazenamento principal de chaves de perfil)
- Execuções locais ao vivo copiam a configuração ativa, os arquivos `auth-profiles.json` por agente, `credentials/` legados e diretórios de autenticação de CLI externa compatíveis para um diretório inicial temporário de teste por padrão; diretórios iniciais live preparados ignoram `workspace/` e `sandboxes/`, e sobrescritas de caminho `agents.*.workspace` / `agentDir` são removidas para que as sondagens fiquem fora do workspace real do seu host.

Se quiser depender de chaves de ambiente (por exemplo, exportadas no seu `~/.profile`), execute os testes locais depois de `source ~/.profile`, ou use os executores Docker abaixo (eles podem montar `~/.profile` no contêiner).
...
- Escopo:
  - Exercita os caminhos de imagem, vídeo e `music_generate` comfy incluídos
...
- Comportamento opcional de autenticação:
  - `OPENCLAW_LIVE_REQUIRE_PROFILE_KEYS=1` para forçar autenticação pelo armazenamento de perfis e ignorar sobrescritas apenas por ambiente

Para o caminho da CLI distribuído, adicione um smoke `infer` depois que o teste ao vivo do provedor/runtime passar:
...
Isso cobre análise de argumentos da CLI, resolução de configuração/agente padrão, ativação de plugins incluídos, o runtime compartilhado de geração de imagem e a solicitação ao provedor ao vivo. Espera-se que as dependências de Plugin estejam presentes antes do carregamento do runtime.
...
- Escopo:
  - Exercita o caminho compartilhado do provedor incluído de geração de música
...
- Escopo:
  - Exercita o caminho compartilhado do provedor incluído de geração de vídeo
  - Usa como padrão o caminho smoke seguro para release: provedores não FAL, uma solicitação de texto para vídeo por provedor, prompt de lagosta de um segundo e um limite de operação por provedor de `OPENCLAW_LIVE_VIDEO_GENERATION_TIMEOUT_MS` (`180000` por padrão)
  ...
  - Provedores `imageToVideo` atualmente declarados, mas ignorados, na varredura compartilhada:
    - `vydra` porque o `veo3` incluído é apenas texto e o `kling` incluído exige uma URL remota de imagem
  - Cobertura específica do provedor Vydra:
    - `OPENCLAW_LIVE_TEST=1 OPENCLAW_LIVE_VYDRA_VIDEO=1 pnpm test:live -- extensions/vydra/vydra.live.test.ts`
    - esse arquivo executa texto para vídeo `veo3` mais uma faixa `kling` que usa uma fixture de URL remota de imagem por padrão
...
- Propósito:
  ...
  - Refina automaticamente cada suíte para provedores que atualmente têm autenticação utilizável por padrão
  - Reutiliza `scripts/test-live.mjs`, de modo que o comportamento de Heartbeat e modo silencioso permaneça consistente
- Exemplos:
