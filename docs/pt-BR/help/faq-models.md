...
read_when:
    ...
    - Entenda os perfis de autenticação e como gerenciá-los
sidebarTitle: Models FAQ
summary: 'Perguntas frequentes: valores padrão de modelo, seleção, apelidos, troca, alternância por falha e perfis de autenticação'
...
---
  ...
  <Accordion title="Qual modelo vocês recomendam?">
    ...
    **Para chat rotineiro/de baixo risco:** use modelos de fallback mais baratos e roteie por função do agente.
    ...
    Aviso importante: modelos mais fracos/superquantizados são mais vulneráveis a prompt
  ...
  <Accordion title="Como troco de modelo sem apagar minha configuração?">
    ...
    Para edições via RPC, inspecione primeiro com `config.schema.lookup` e prefira `config.patch`. O payload de lookup fornece o caminho normalizado, documentação/restrições rasas do schema e resumos dos filhos imediatos.
  ...
  <Accordion title="Posso usar modelos auto-hospedados (llama.cpp, vLLM, Ollama)?">
    ...
    - `Cloud + Local` oferece modelos em nuvem além dos seus modelos Ollama locais
    ...
    Nota de segurança: modelos menores ou fortemente quantizados são mais vulneráveis a prompt
  ...
  <Accordion title="Como troco de modelo em tempo real (sem reiniciar)?">
    ...
    Estes são os aliases integrados. Aliases personalizados podem ser adicionados por meio de `agents.defaults.models`.
  ...
  <Accordion title="Se dois provedores expõem o mesmo ID de modelo, qual deles o /model usa?">
    ...
    Por exemplo, `qianfan/deepseek-v4-flash` e `deepseek/deepseek-v4-flash` são referências de modelo diferentes, embora ambas contenham `deepseek-v4-flash`. O OpenClaw não deve alternar silenciosamente de um provedor para outro apenas porque o ID de modelo sem provedor corresponde.

    Uma referência `/model` selecionada pelo usuário também é estrita para a política de fallback. Se esse provedor/modelo selecionado estiver indisponível, a resposta falhará de forma visível em vez de responder a partir de `agents.defaults.model.fallbacks`. Cadeias de fallback configuradas ainda se aplicam a padrões configurados, modelos primários de jobs cron e estado de fallback selecionado automaticamente.

    Se uma execução iniciada a partir de uma substituição que não é de sessão puder usar fallback, o OpenClaw tenta primeiro o provedor/modelo solicitado, depois os fallbacks configurados e só então o primário configurado. Isso impede que IDs de modelo duplicados sem provedor saltem diretamente de volta para o provedor padrão.
  ...
  <Accordion title="Como configuro o modo rápido para GPT 5.5?">
    ...
    - **Por padrão de modelo:** defina `agents.defaults.models["openai/gpt-5.5"].params.fastMode` como `true`.
    ...
    Para OpenAI, o modo rápido mapeia para `service_tier = "priority"` em solicitações Responses nativas compatíveis. Substituições de sessão por `/fast` têm precedência sobre padrões de configuração.
  ...
  <Accordion title='Por que vejo "Model ... is not allowed" e depois nenhuma resposta?'>
    ...
    `agents.defaults.models`, adicione um wildcard de provedor como `"provider/*": {}` para catálogos dinâmicos de provedores, remova a allowlist ou escolha um modelo em `/model list`.
    Se o comando também incluiu `--runtime codex`, atualize a allowlist primeiro e tente novamente
  ...
  <Accordion title='Por que vejo "Unknown model: minimax/MiniMax-M2.7"?'>
    ...
    2. Certifique-se de que o MiniMax esteja configurado (assistente ou JSON), ou que a autenticação do MiniMax
       ...
       para `minimax-portal`).
    3. Use o ID de modelo exato (com diferenciação de maiúsculas/minúsculas) para seu caminho de autenticação:
  ...
  <Accordion title="Como defino/substituo atalhos de modelo (aliases)?">
    ...
    Então `/model sonnet` (ou `/<alias>` quando houver suporte) resolve para esse ID de modelo.
  ...
  <Accordion title="Como adiciono modelos de outros provedores, como OpenRouter ou Z.AI?">
    ...
    **Nenhuma chave de API encontrada para o provedor após adicionar um novo agente**
    ...
    - Ou copie apenas perfis estáticos portáveis de `api_key` / `token` do armazenamento de autenticação do agente principal para o armazenamento de autenticação do novo agente.
    - Para perfis OAuth, faça login a partir do novo agente quando ele precisar de sua própria conta; caso contrário, o OpenClaw pode ler o agente padrão/principal sem clonar tokens de atualização.
...
<AccordionGroup>
  <Accordion title="Como o failover funciona?">
    ...
    Cooldowns se aplicam a perfis com falha (backoff exponencial), então o OpenClaw pode continuar respondendo mesmo quando um provedor está limitado por taxa ou falhando temporariamente.
    ...
    na faixa de cobrança, mas correspondências de texto específicas do provedor permanecem restritas ao
    ...
    exceeded` permanecem no caminho de Compaction/tentativa novamente, em vez de avançar o fallback de modelo.

    Texto genérico de erro do servidor é intencionalmente mais restrito do que "qualquer coisa com
    unknown/error". O OpenClaw trata formatos transitórios com escopo de provedor
    como `An unknown error occurred` simples da Anthropic, `Provider returned error`
  ...
  <Accordion title="Por que ele também tentou o Google Gemini e falhou?">
    Se sua configuração de modelo incluir o Google Gemini como fallback (ou você mudou para um atalho Gemini), o OpenClaw tentará usá-lo durante o fallback de modelo. Se você não configurou credenciais do Google, verá `No API key found for provider "google"`.
    ...
    **Solicitação LLM rejeitada: assinatura de raciocínio obrigatória (Google Antigravity)**

    Causa: o histórico da sessão contém **blocos de raciocínio sem assinaturas** (muitas vezes de
    um stream abortado/parcial). O Google Antigravity exige assinaturas para blocos de raciocínio.

    Correção: o OpenClaw agora remove blocos de raciocínio não assinados para o Claude do Google Antigravity. Se ainda aparecer, inicie uma **nova sessão** ou defina `/thinking off` para esse agente.
...
<AccordionGroup>
  <Accordion title="O que é um perfil de autenticação?">
    Um perfil de autenticação é um registro de credenciais nomeado (OAuth ou chave de API) vinculado a um provedor. Os perfis ficam em:
  ...
  <Accordion title="Posso controlar qual perfil de autenticação é tentado primeiro?">
    ...
    Cooldowns de limite de taxa podem ter escopo de modelo. Um perfil que está em cooldown
  ...
  <Accordion title="OAuth vs chave de API - qual é a diferença?">
    ...
    O assistente oferece suporte explícito à CLI do Claude da Anthropic, ao OAuth do OpenAI Codex e a chaves de API.
