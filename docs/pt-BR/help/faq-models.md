...
read_when:
    ...
    - Depuração da alternância em caso de falha de modelos / "Todos os modelos falharam"
    - Entendendo perfis de autenticação e como gerenciá-los
sidebarTitle: Models FAQ
summary: 'Perguntas frequentes: padrões de modelo, seleção, apelidos, alternância, failover e perfis de autenticação'
...
---
  ...
  <Accordion title="Qual modelo vocês recomendam?">
    **Padrão recomendado:** use o modelo mais forte de geração mais recente disponível na sua pilha de provedores.
    ...
    **Para chat rotineiro/de baixo risco:** use modelos de fallback mais baratos e roteie por função do agente.
    ...
    Aviso forte: modelos mais fracos ou excessivamente quantizados são mais vulneráveis a prompt
  ...
  <Accordion title="Posso usar modelos auto-hospedados (llama.cpp, vLLM, Ollama)?">
    ...
    - `Cloud + Local` fornece modelos em nuvem mais seus modelos locais do Ollama
  ...
  <Accordion title="Quais modelos o OpenClaw, Flawd e Krill usam?">
    ...
    - Para agentes sensíveis à segurança/com ferramentas habilitadas, use o modelo mais forte de geração mais recente disponível.
  ...
  <Accordion title="Posso usar GPT 5.5 para tarefas diárias e Codex 5.5 para programação?">
    ...
    - **Tarefas diretas da API da OpenAI fora do loop do agente:** configure `OPENAI_API_KEY` para imagens, embeddings, fala, tempo real e outras superfícies da API da OpenAI que não sejam de agentes.
  ...
  <Accordion title="Como configuro o modo rápido para GPT 5.5?">
    ...
    Para OpenAI, o modo rápido é mapeado para `service_tier = "priority"` em solicitações nativas Responses compatíveis. Substituições de sessão com `/fast` prevalecem sobre padrões de configuração.

    Consulte [Thinking e modo rápido](/pt-BR/tools/thinking) e [Modo rápido da OpenAI](/pt-BR/providers/openai#fast-mode).
  ...
  <Accordion title='Por que vejo "Unknown model: minimax/MiniMax-M2.7"?'>
    ...
    1. Atualize para uma versão atual do OpenClaw (ou execute a partir de `main` no código-fonte) e depois reinicie o gateway.
  ...
  <Accordion title="Como adiciono modelos de outros provedores, como OpenRouter ou Z.AI?">
    ...
    **Nenhuma chave de API encontrada para o provedor após adicionar um novo agente**
    ...
    - Para perfis OAuth, entre pelo novo agente quando ele precisar da própria conta; caso contrário, o OpenClaw pode ler a partir do agente padrão/principal sem clonar tokens de atualização.
...
<AccordionGroup>
  <Accordion title="Como o failover funciona?">
    ...
    parecer, em vez disso, um limite recuperável de janela de uso ou
    ...
    exceeded` permanecem no caminho de compaction/nova tentativa em vez de avançar para o
    fallback de modelo.
  ...
  <Accordion title="Por que ele também tentou o Google Gemini e falhou?">
    Se sua configuração de modelo incluir Google Gemini como fallback (ou se você trocou para um atalho Gemini), o OpenClaw vai tentá-lo durante o fallback de modelo. Se você não configurou credenciais do Google, verá `No API key found for provider "google"`.
...
<AccordionGroup>
  ...
  <Accordion title="OAuth versus chave de API: qual é a diferença?">
    ...
    - **OAuth** costuma aproveitar acesso por assinatura (quando aplicável).
