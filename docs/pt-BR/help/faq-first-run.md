...
summary: 'FAQ: configuração de início rápido e primeira execução — instalação, integração, autenticação, assinaturas, falhas iniciais'
...
---
  ...
  <AccordionGroup>
  <Accordion title="Estou travado, maneira mais rápida de destravar">
    ...
    - `openclaw models status`: verifica a autenticação do provedor + disponibilidade de modelos.
  ...
  <Accordion title="Heartbeat continua sendo pulado. O que significam os motivos de pulo?">
    ...
    - `empty-heartbeat-file`: `HEARTBEAT.md` existe, mas contém apenas estrutura em branco, comentário, cabeçalho, fence ou checklist vazio
  ...
  <Accordion title="Maneira recomendada de instalar e configurar o OpenClaw">
    ...
    O assistente também pode construir assets da UI automaticamente. Depois do onboarding, você normalmente executa o Gateway na porta **18789**.
  ...
  <Accordion title="Como autentico o dashboard em localhost versus remoto?">
    ...
    **Não em localhost:**
    ...
    - **Tailscale Serve** (recomendado): mantenha o bind em loopback, execute `openclaw gateway --tailscale serve`, abra `https://<magicdns>/`. Se `gateway.auth.allowTailscale` for `true`, os cabeçalhos de identidade satisfazem a autenticação da Control UI/WebSocket (sem colar segredo compartilhado, pressupõe host de gateway confiável); APIs HTTP ainda exigem autenticação por segredo compartilhado, a menos que você use deliberadamente private-ingress `none` ou autenticação HTTP por proxy confiável.
      Tentativas simultâneas ruins de autenticação do Serve a partir do mesmo cliente são serializadas antes de o limitador de autenticação falha registrá-las, então a segunda nova tentativa ruim já pode mostrar `retry later`.
    ...
    - **Proxy reverso ciente de identidade**: mantenha o Gateway atrás de um proxy confiável, configure `gateway.auth.mode: "trusted-proxy"` e então abra a URL do proxy. Proxies de loopback no mesmo host exigem `gateway.auth.trustedProxy.allowLoopback = true` explícito.
  ...
  <Accordion title="Por que há duas configurações de aprovação de exec para aprovações por chat?">
    ...
    A política de exec do host ainda é o verdadeiro gate de aprovação. A configuração de chat controla apenas onde os
    ...
    - Se um canal nativo compatível puder inferir aprovadores com segurança, o OpenClaw agora habilita automaticamente aprovações nativas priorizando DM quando `channels.<channel>.execApprovals.enabled` está indefinido ou é `"auto"`.
    - Quando cards/botões de aprovação nativos estão disponíveis, essa UI nativa é o caminho principal; o agente só deve incluir um comando manual `/approve` se o resultado da ferramenta disser que aprovações por chat estão indisponíveis ou que a aprovação manual é o único caminho.
    ...
    - Aprovações de Plugin são separadas novamente: elas usam `/approve` no mesmo chat por padrão, encaminhamento opcional por `approvals.plugin`, e apenas alguns canais nativos mantêm tratamento nativo de aprovação de Plugin além disso.
  ...
  <Accordion title="Alguma dica para instalações em Raspberry Pi?">
    Versão curta: funciona, mas espere arestas ásperas.
  ...
  <Accordion title="Onde vejo o que há de novo na versão mais recente?">
    ...
    com data é a versão lançada mais recente. As entradas são agrupadas por **Destaques**, **Mudanças** e
  ...
  <Accordion title="Não consigo acessar docs.openclaw.ai (erro de SSL)">
    ...
    Advanced Security. Desative-o ou adicione `docs.openclaw.ai` à lista de permissões e tente novamente.
  ...
  <Accordion title="Quanto tempo a instalação e o onboarding geralmente levam?">
    ...
    - **Onboarding do QuickStart:** geralmente alguns minutos
    - **Onboarding completo:** mais tempo quando login do provedor, pareamento de canal, instalação do daemon,
      downloads de rede, Skills ou Plugins opcionais precisam de configuração extra

    O assistente da CLI mostra essa linha do tempo antecipadamente. Você pode pular etapas opcionais e voltar
  ...
  <Accordion title="A instalação no Windows diz que git não foi encontrado ou openclaw não é reconhecido">
    ...
    **2) openclaw não é reconhecido após a instalação**
    ...
    - Adicione esse diretório ao PATH do seu usuário (não é necessário sufixo `\bin` no Windows; na maioria dos sistemas, é `%AppData%\npm`).
    - Feche e reabra o PowerShell depois de atualizar o PATH.

    Para configuração de desktop, use o app nativo **Windows Hub**. Para configuração
    apenas por terminal, o instalador do PowerShell e os caminhos do WSL2 Gateway são ambos compatíveis.
    Documentação: [Windows](/pt-BR/platforms/windows).

  </Accordion>

  <Accordion title="A saída exec do Windows mostra texto em chinês corrompido - o que devo fazer?">
    ...
    Se você ainda conseguir reproduzir isso no OpenClaw mais recente, acompanhe/relate em:
  ...
  <Accordion title="O que o onboarding realmente faz?">
    ...
    - **Configuração de modelo/auth** (OAuth do provedor, chaves de API, setup-token da Anthropic, além de opções de modelo local como LM Studio)
    ...
    Ele também define expectativas de duração antes dos prompts principais começarem e avisa se seu
    modelo configurado é desconhecido ou está sem auth.
  ...
  <Accordion title="Preciso de uma assinatura Claude ou OpenAI para executar isto?">
    ...
    **modelos apenas locais** para que seus dados permaneçam no seu dispositivo. Assinaturas (Claude
    ...
    - **CLI do Claude / autenticação de assinatura Claude no OpenClaw**: a equipe da Anthropic
    ...
    OpenClaw também oferece suporte a outras opções hospedadas em estilo de assinatura, incluindo
  ...
  <Accordion title="Vocês oferecem suporte à autenticação por assinatura Claude (Claude Pro ou Max)?">
    ...
    a reutilização da Claude CLI e o uso de `claude -p` como sancionados para essa integração
    a menos que a Anthropic publique uma nova política.

    O setup-token da Anthropic ainda está disponível como caminho de token compatível no OpenClaw, mas agora o OpenClaw prefere a reutilização da Claude CLI e `claude -p` quando disponíveis.
    ...
    em estilo de assinatura no OpenClaw, consulte [OpenAI](/pt-BR/providers/openai), [Qwen / Model
...
<a id="why-am-i-seeing-http-429-ratelimiterror-from-anthropic"></a>
  ...
  <Accordion title="Por que estou vendo HTTP 429 rate_limit_error da Anthropic?">
    ...
    Dica: defina um **modelo reserva** para que o OpenClaw possa continuar respondendo enquanto um provedor estiver com limite de taxa.
  ...
  <Accordion title="Como funciona a autenticação do Codex?">
    ...
    execução nativa do servidor de aplicativo do Codex. Referências GPT legadas do Codex são
    configuração legada reparada por `openclaw doctor --fix`. O acesso direto por chave de API da OpenAI
    continua disponível para superfícies de API OpenAI que não sejam de agente e para modelos de agente
  ...
  <Accordion title="Por que o OpenClaw ainda menciona o prefixo legado OpenAI Codex?">
    `openai` é o provedor e o id do perfil de autenticação tanto para chaves de API da OpenAI quanto para
    OAuth do ChatGPT/Codex. Você ainda pode ver o prefixo legado OpenAI Codex em configurações legadas e
    avisos de migração.
    ...
    - ids legados de perfil de autenticação do Codex = id legado de perfil de autenticação migrado por `openclaw doctor --fix`
    ...
    `openclaw models auth login --provider openai`. Mantenha a referência do modelo como
  ...
  <Accordion title="Preciso comprar um Mac Mini para instalar isso?">
    ...
    compram um como host sempre ligado, mas um VPS pequeno, servidor doméstico ou uma caixa da classe Raspberry Pi também funciona.
  ...
  <Accordion title="Preciso de um Mac mini para suporte a iMessage?">
    ...
    - Execute o Gateway em Linux/VPS e defina `channels.imessage.cliPath` como um wrapper SSH que executa `imsg` em um Mac conectado ao Messages.
  ...
  <Accordion title="Telegram: o que vai em allowFrom?">
    ...
    A configuração solicita apenas IDs de usuário numéricos. Se você já tem entradas legadas `@username` na configuração, `openclaw doctor --fix` pode tentar resolvê-las.
  ...
  <Accordion title='Posso executar um agente de "chat rápido" e um agente "Opus para programação"?'>
    Sim. Use roteamento multiagente: dê a cada agente seu próprio modelo padrão e, em seguida, vincule rotas de entrada (conta de provedor ou peers específicos) a cada agente. Um exemplo de configuração está em [Roteamento multiagente](/pt-BR/concepts/multi-agent). Consulte também [Modelos](/pt-BR/concepts/models) e [Configuração](/pt-BR/gateway/configuration).
  </Accordion>

  <Accordion title="Homebrew funciona no Linux?">
    Sim. Homebrew oferece suporte ao Linux (Linuxbrew). Configuração rápida:
  ...
  <Accordion title="Diferença entre a instalação git hackeável e a instalação npm">
    ...
    - **Instalação npm:** instalação global da CLI, sem repositório, ideal para "apenas executar".
  ...
  <Accordion title="Posso alternar entre instalações npm e git depois?">
    ...
    acompanhamentos do Doctor, atualiza as fontes do Plugin para o canal de destino e
  ...
  <Accordion title="Devo executar o Gateway no meu laptop ou em um VPS?">
    ...
    menor atrito e não se importa com suspensões/reinicializações, execute localmente.

    **Laptop (Gateway local)**

    - **Prós:** sem custo de servidor, acesso direto a arquivos locais, janela do navegador ativa.
  ...
  <Accordion title="Qual é a importância de executar o OpenClaw em uma máquina dedicada?">
    ...
    - **Host dedicado (VPS/Mac mini/Raspberry Pi):** sempre ativo, menos interrupções por suspensão/reinicialização, permissões mais limpas, mais fácil de manter em execução.
    ...
    Se você quer o melhor dos dois mundos, mantenha o Gateway em um host dedicado e pareie seu laptop como um **nó** para ferramentas locais de tela/câmera/execução. Consulte [Nós](/pt-BR/nodes).
  ...
  <Accordion title="Quais são os requisitos mínimos de VPS e o sistema operacional recomendado?">
    ...
    - **Recomendado:** 1-2 vCPU, 2 GB de RAM ou mais para folga (logs, mídia, múltiplos canais). Ferramentas de Node e automação de navegador podem consumir muitos recursos.

    SO: use **Ubuntu LTS** (ou qualquer Debian/Ubuntu moderno). O caminho de instalação no Linux é mais bem testado nele.

    Docs: [Linux](/pt-BR/platforms/linux), [Hospedagem em VPS](/pt-BR/vps).

  </Accordion>

  <Accordion title="Posso executar o OpenClaw em uma VM e quais são os requisitos?">
    Sim. Trate uma VM da mesma forma que um VPS: ela precisa estar sempre ativa, acessível e ter RAM suficiente
    ...
    de ferramentas. Consulte [Windows](/pt-BR/platforms/windows), [Hospedagem em VPS](/pt-BR/vps).
