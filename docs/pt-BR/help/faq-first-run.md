...
x-i18n:
    generated_at: "2026-05-02T05:48:59Z"
    model: gpt-5.5
    provider: openai
...
---
  ...
  Início rápido e perguntas frequentes da primeira execução. Para operações do dia a dia, modelos, autenticação, sessões
  ...
  <Accordion title="Estou travado, qual é a forma mais rápida de destravar">
    ...
    Essas ferramentas conseguem ler o repositório, executar comandos, inspecionar logs e ajudar a corrigir sua
    configuração no nível da máquina (PATH, serviços, permissões, arquivos de autenticação). Dê a elas o **checkout completo do código-fonte** por meio
    ...
    Isso instala o OpenClaw **a partir de um checkout git**, então o agente consegue ler o código + a documentação e
    ...
    - `openclaw status`: visão rápida da integridade do gateway/agente + configuração básica.
  ...
  <Accordion title="Heartbeat continua sendo ignorado. O que significam os motivos de ignorar?">
    ...
    - `empty-heartbeat-file`: `HEARTBEAT.md` existe, mas contém apenas um esqueleto em branco/somente cabeçalho
    ...
    No modo de tarefas, os carimbos de data/hora de vencimento só avançam depois que uma execução real de Heartbeat
  ...
  <Accordion title="Forma recomendada de instalar e configurar o OpenClaw">
    ...
    O assistente também pode criar assets da UI automaticamente. Depois do onboarding, você normalmente executa o Gateway na porta **18789**.
  ...
  <Accordion title="Como abro o painel depois do onboarding?">
    O assistente abre seu navegador com uma URL limpa do painel (sem token) logo depois do onboarding e também imprime o link no resumo. Mantenha essa aba aberta; se ela não abrir, copie/cole a URL impressa na mesma máquina.
  </Accordion>
  ...
  <Accordion title="Como autentico o painel em localhost versus remoto?">
    ...
    **Não está em localhost:**
    ...
    - **Tailscale Serve** (recomendado): mantenha o bind em loopback, execute `openclaw gateway --tailscale serve`, abra `https://<magicdns>/`. Se `gateway.auth.allowTailscale` for `true`, os cabeçalhos de identidade satisfazem a autenticação da Control UI/WebSocket (sem colar segredo compartilhado, presume host do gateway confiável); APIs HTTP ainda exigem autenticação por segredo compartilhado, a menos que você use deliberadamente ingresso privado `none` ou autenticação HTTP por proxy confiável.
      Tentativas ruins simultâneas de autenticação do Serve vindas do mesmo cliente são serializadas antes que o limitador de autenticação com falha as registre, então a segunda nova tentativa ruim já pode mostrar `retry later`.
    - **Bind de tailnet**: execute `openclaw gateway --bind tailnet --token "<token>"` (ou configure autenticação por senha), abra `http://<tailscale-ip>:18789/` e cole o segredo compartilhado correspondente nas configurações do painel.
    - **Proxy reverso com reconhecimento de identidade**: mantenha o Gateway atrás de um proxy confiável, configure `gateway.auth.mode: "trusted-proxy"` e abra a URL do proxy. Proxies de loopback no mesmo host exigem `gateway.auth.trustedProxy.allowLoopback = true` explícito.
  ...
  <Accordion title="Por que há duas configurações de aprovação de exec para aprovações no chat?">
    ...
    - Se um canal nativo compatível consegue inferir aprovadores com segurança, o OpenClaw agora ativa automaticamente aprovações nativas primeiro por DM quando `channels.<channel>.execApprovals.enabled` não está definido ou é `"auto"`.
    - Quando cartões/botões de aprovação nativos estão disponíveis, essa UI nativa é o caminho principal; o agente só deve incluir um comando manual `/approve` se o resultado da ferramenta disser que aprovações por chat não estão disponíveis ou que aprovação manual é o único caminho.
    ...
    - Use `channels.<channel>.execApprovals.target: "channel"` ou `"both"` apenas quando você explicitamente quiser que os prompts de aprovação sejam publicados de volta na sala/tópico de origem.
    - Aprovações de Plugin são separadas novamente: elas usam `/approve` no mesmo chat por padrão, encaminhamento opcional de `approvals.plugin`, e apenas alguns canais nativos mantêm tratamento nativo de aprovação de Plugin por cima.
  ...
  <Accordion title="Ele roda em Raspberry Pi?">
    ...
    Se você quiser folga extra (logs, mídia, outros serviços), **2 GB são recomendados**, mas isso
  ...
  <Accordion title="Alguma dica para instalações em Raspberry Pi?">
    ...
    - Use um sistema operacional **64-bit** e mantenha Node >= 22.
  ...
  <Accordion title="Está travado em wake up my friend / o onboarding não vai chocar. E agora?">
    ...
    Se o Gateway for remoto, confira se o túnel/conexão Tailscale está ativo e se a UI
    está apontada para o Gateway correto. Consulte [Acesso remoto](/pt-BR/gateway/remote).
  ...
  <Accordion title="Onde vejo o que há de novo na versão mais recente?">
    Verifique o changelog no GitHub:
    ...
    é a versão mais recente já lançada. As entradas são agrupadas por **Highlights**, **Changes** e
  ...
  <Accordion title="Não consigo acessar docs.openclaw.ai (erro de SSL)">
    ...
    Advanced Security. Desative ou coloque `docs.openclaw.ai` na lista de permissões e tente novamente.
  ...
  <Accordion title="Como instalo a versão beta e qual é a diferença entre beta e dev?">
    ...
    **Dev** é o head móvel de `main` (git); quando publicado, usa a dist-tag do npm `dev`.
  ...
  <Accordion title="Como testo os bits mais recentes?">
    ...
    2. **Instalação modificável (a partir do site do instalador):**
  ...
  <Accordion title="A instalação no Windows diz que git não foi encontrado ou que openclaw não é reconhecido">
  ...
  <Accordion title="A saída de exec no Windows mostra texto chinês ilegível - o que devo fazer?">
    ...
    Depois reinicie o Gateway e tente executar seu comando novamente:
  ...
  <Accordion title="A documentação não respondeu à minha pergunta - como obtenho uma resposta melhor?">
    Use a **instalação modificável (git)** para ter todo o código-fonte e a documentação localmente, depois pergunte
  ...
  <Accordion title="Onde estão os guias de instalação na nuvem/VPS?">
    ...
    do seu notebook/celular pela Control UI (ou Tailscale/SSH). Seu estado + workspace
    ...
    tela/câmera/canvas locais ou executar comandos no seu notebook enquanto mantém o
    Gateway na nuvem.
  ...
  <Accordion title="O que o onboarding realmente faz?">
    ...
    - **Configuração de modelo/autenticação** (OAuth de provedor, chaves de API, token de configuração da Anthropic, além de opções de modelo local como LM Studio)
  ...
  <Accordion title="Preciso de uma assinatura Claude ou OpenAI para executar isto?">
    ...
    **modelos apenas locais** para que seus dados permaneçam no seu dispositivo. Assinaturas (Claude
    ...
    O OpenClaw também dá suporte a outras opções hospedadas em estilo de assinatura, incluindo
  ...
  <Accordion title="Posso usar a assinatura Claude Max sem uma chave de API?">
    ...
    A equipe da Anthropic nos disse que o uso da Claude CLI no estilo OpenClaw é permitido novamente, então
    o OpenClaw trata a autenticação de assinatura Claude e o uso de `claude -p` como autorizados
  ...
  <Accordion title="Vocês oferecem suporte à autenticação de assinatura Claude (Claude Pro ou Max)?">
    ...
    a reutilização da Claude CLI e o uso de `claude -p` como autorizados para esta integração,
    a menos que a Anthropic publique uma nova política.

    O token de configuração da Anthropic ainda está disponível como um caminho de token compatível no OpenClaw, mas agora o OpenClaw prefere reutilizar a Claude CLI e `claude -p` quando disponível.
    Para workloads de produção ou multiusuário, a autenticação por chave de API da Anthropic ainda é a
    opção mais segura e previsível. Se quiser outras opções hospedadas em estilo de assinatura
...
<AccordionGroup>
  ...
  <Accordion title="Como a autenticação do Codex funciona?">
    ...
    autenticação de assinatura ChatGPT/Codex mais execução nativa do servidor de app do Codex. Use
    `openai-codex/gpt-5.5` somente quando quiser OAuth do Codex pelo runner
  ...
  <Accordion title="Por que o OpenClaw ainda menciona openai-codex?">
    ...
    Ele também é o prefixo explícito de modelo do PI para OAuth do Codex:

    - `openai/gpt-5.5` + `agentRuntime.id: "codex"` = autenticação de assinatura ChatGPT/Codex com runtime nativo do Codex
    ...
    Se quiser o caminho direto de cobrança/limite da OpenAI Platform, defina
  ...
  <Accordion title="Por que os limites do OAuth do Codex podem ser diferentes dos do ChatGPT web?">
    ...
    em acesso direto à API. Se quiser o caminho direto de cobrança/limite da OpenAI Platform,
  ...
  <Accordion title="Vocês oferecem suporte à autenticação por assinatura da OpenAI (OAuth do Codex)?">
    ...
    externos como o OpenClaw. O onboarding pode executar o fluxo OAuth para você.
  ...
  <Accordion title="Como configuro o OAuth do Gemini CLI?">
    O Gemini CLI usa um **fluxo de autenticação de plugin**, não um id de cliente nem um segredo em `openclaw.json`.
  ...
  <Accordion title="Um modelo local é adequado para conversas casuais?">
    Geralmente, não. O OpenClaw precisa de contexto grande + segurança forte; placas pequenas truncam e vazam. Se for indispensável, execute localmente a **maior** build de modelo que puder (LM Studio) e consulte [/gateway/local-models](/pt-BR/gateway/local-models). Modelos menores/quantizados aumentam o risco de injeção de prompt - consulte [Segurança](/pt-BR/gateway/security).
  </Accordion>

  <Accordion title="Como mantenho o tráfego de modelos hospedados em uma região específica?">
    Escolha endpoints fixados por região. O OpenRouter expõe opções hospedadas nos EUA para MiniMax, Kimi e GLM; escolha a variante hospedada nos EUA para manter os dados na região. Você ainda pode listar Anthropic/OpenAI junto com esses usando `models.mode: "merge"` para que os fallbacks permaneçam disponíveis enquanto respeitam o provedor regional selecionado.
  </Accordion>
  ...
  <Accordion title="Preciso comprar um Mac Mini para instalar isto?">
    ...
    compram um como host sempre ativo, mas um VPS pequeno, servidor doméstico ou equipamento da classe Raspberry Pi também funciona.
  ...
  <Accordion title="Se eu comprar um Mac mini para executar o OpenClaw, posso conectá-lo ao meu MacBook Pro?">
    ...
    **nó** (dispositivo complementar). Nós não executam o Gateway - eles fornecem recursos
  ...
  <Accordion title='Posso executar um agente de "conversa rápida" e um agente "Opus para programação"?'>
    Sim. Use roteamento multiagente: dê a cada agente seu próprio modelo padrão e então vincule rotas de entrada (conta de provedor ou peers específicos) a cada agente. A configuração de exemplo está em [Roteamento multiagente](/pt-BR/concepts/multi-agent). Veja também [Modelos](/pt-BR/concepts/models) e [Configuração](/pt-BR/gateway/configuration).
  </Accordion>
  ...
  <Accordion title="O Homebrew funciona no Linux?">
    ...
    Builds recentes também prefixam diretórios bin comuns do usuário em serviços Linux systemd (por exemplo `~/.local/bin`, `~/.npm-global/bin`, `~/.local/share/pnpm`, `~/.bun/bin`) e respeitam `PNPM_HOME`, `NPM_CONFIG_PREFIX`, `BUN_INSTALL`, `VOLTA_HOME`, `ASDF_DATA_DIR`, `NVM_DIR` e `FNM_DIR` quando definidos.
  ...
  <Accordion title="Devo executar o Gateway no meu laptop ou em um VPS?">
    ...
    **VPS / nuvem**
    ...
    - **Prós:** sempre ativo, rede estável, sem problemas de laptop em suspensão, mais fácil de manter em execução.
    ...
    **Observação específica do OpenClaw:** WhatsApp/Telegram/Slack/Mattermost/Discord funcionam bem a partir de um VPS. A única troca real é **navegador sem interface gráfica** vs uma janela visível. Consulte [Navegador](/pt-BR/tools/browser).
  ...
  <Accordion title="Qual é a importância de executar o OpenClaw em uma máquina dedicada?">
    ...
    Se você quer o melhor dos dois mundos, mantenha o Gateway em um host dedicado e emparelhe seu laptop como um **node** para ferramentas locais de tela/câmera/exec. Consulte [Nodes](/pt-BR/nodes).
  ...
  <Accordion title="Quais são os requisitos mínimos de VPS e o SO recomendado?">
    ...
    - **Recomendado:** 1-2 vCPU, 2 GB de RAM ou mais para margem (logs, mídia, múltiplos canais). Ferramentas de Node e automação de navegador podem consumir muitos recursos.

    SO: use **Ubuntu LTS** (ou qualquer Debian/Ubuntu moderno). O caminho de instalação no Linux é melhor testado ali.
  ...
  <Accordion title="Posso executar o OpenClaw em uma VM e quais são os requisitos?">
    ...
    Se você está no Windows, **WSL2 é a configuração no estilo VM mais fácil** e tem a melhor compatibilidade
