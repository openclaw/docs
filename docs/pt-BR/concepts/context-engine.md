...
summary: 'Mecanismo de contexto: montagem de contexto plugável, Compaction e ciclo de vida de subagentes'
...
Um **mecanismo de contexto** controla como o OpenClaw constrói o contexto do modelo para cada execução: quais mensagens incluir, como resumir o histórico mais antigo e como gerenciar o contexto entre limites de subagentes.
...
O método `assemble` pode retornar uma string `systemPromptAddition`. O OpenClaw antepõe isso ao prompt do sistema para a execução. Isso permite que mecanismos injetem orientações dinâmicas de recuperação, instruções de busca ou dicas sensíveis ao contexto sem exigir arquivos estáticos no workspace.
...
- **Montagem**: passagem direta (o pipeline existente sanitize → validate → limit no runtime lida com a montagem de contexto).
...
Em seguida, habilite-o na configuração:
...
| ------------------ | ----------- | -------------------------------------------------------------- |
| `info`             | Propriedade | ID, nome, versão do mecanismo e se ele é responsável por Compaction |
...
<ParamField path="promptAuthority" type='"assembled" | "preassembly_may_overflow"'>
  ...
  preventivas de estouro. O padrão é `"assembled"`, o que significa que somente
  a estimativa do prompt montado é verificada - adequado para mecanismos que retornam um
  contexto em janela e autocontido. Defina como `"preassembly_may_overflow"` somente
  quando sua visão montada puder ocultar risco de estouro na transcrição
  subjacente; então o executor usa o máximo entre a estimativa montada
  e a estimativa do histórico de sessão pré-montagem (sem janela) ao decidir
...
`compact` retorna um `CompactResult`. Quando a Compaction alterna a
...
| `ingestBatch(params)`          | Método | Ingerir um turno concluído como um lote. Chamado após uma execução ser concluída, com todas as mensagens daquele turno de uma vez. |
...
`runtimeSettings` opcional. Ele é uma superfície interna versionada, somente leitura,
de API produtor/consumidor: o OpenClaw o produz para o mecanismo de contexto
selecionado, e o mecanismo de contexto o consome dentro dos hooks de ciclo de vida. Ele não é
renderizado diretamente para usuários e não cria uma superfície dedicada de relatório.

- `schemaVersion`: atualmente `1`
- `runtime`: host OpenClaw, modo de runtime (`normal`, `fallback` ou
  `degraded`) e ids opcionais de harness/runtime
- `contextEngineSelection`: id do mecanismo de contexto selecionado e fonte da seleção
...
- `diagnostics`: códigos fechados de motivo de fallback e degradação quando conhecidos

Campos que podem ser desconhecidos são representados como `null`; campos discriminadores, como
modo de runtime e fonte da seleção, permanecem não anuláveis. Mecanismos mais antigos permanecem
compatíveis: se um mecanismo legacy estrito rejeitar `runtimeSettings` como uma
propriedade desconhecida, o OpenClaw tenta novamente a chamada de ciclo de vida sem ele, em vez de colocar
o mecanismo em quarentena.

### Requisitos de host

Mecanismos de contexto podem declarar requisitos de capacidade de host em `info.hostRequirements`.
O OpenClaw verifica esses requisitos antes de iniciar a operação e falha de forma fechada
...
Execuções de agente nativas do Codex e incorporadas do OpenClaw satisfazem `assemble-before-prompt`.
Backends de CLI genéricos não satisfazem, então mecanismos que exigem isso são rejeitados antes que o
processo da CLI comece.

### Isolamento de falhas

O OpenClaw isola o mecanismo de Plugin selecionado do caminho principal de resposta. Se um
mecanismo não legacy estiver ausente, falhar na validação de contrato, lançar durante a criação
da fábrica ou lançar a partir de um método de ciclo de vida, o OpenClaw coloca esse mecanismo em quarentena
para o processo atual do Gateway e rebaixa o trabalho de mecanismo de contexto para o
...
não tem uma capacidade obrigatória, o OpenClaw falha em modo fechado antes de iniciar a execução. Isso
protege mecanismos que corromperiam o estado se fossem executados em um host sem suporte.

### ownsCompaction

`ownsCompaction` controla se a auto-Compaction embutida durante a tentativa do runtime do OpenClaw permanece habilitada para a execução:

<AccordionGroup>
  <Accordion title="ownsCompaction: true">
    O mecanismo controla o comportamento de Compaction. O OpenClaw desabilita a auto-Compaction embutida do runtime do OpenClaw para essa execução, e a implementação de `compact()` do mecanismo é responsável por `/compact`, pela Compaction de recuperação de estouro e por qualquer Compaction proativa que ele queira fazer em `afterTurn()`. O OpenClaw ainda pode executar a proteção de estouro pré-prompt; quando ela prevê que a transcrição completa estourará o limite, o caminho de recuperação chama `compact()` do mecanismo ativo antes de enviar outro prompt.
  </Accordion>
  <Accordion title="ownsCompaction: false or unset">
    A auto-Compaction embutida do runtime do OpenClaw ainda pode ser executada durante a execução do prompt, mas o método `compact()` do mecanismo ativo ainda é chamado para `/compact` e recuperação de estouro.
  </Accordion>
</AccordionGroup>

<Warning>
`ownsCompaction: false` **não** significa que o OpenClaw automaticamente volta para o caminho de Compaction do mecanismo legado.
</Warning>
...
<Tabs>
  ...
  <Tab title="Delegating mode">
    Defina `ownsCompaction: false` e faça `compact()` chamar `delegateCompactionToRuntime(...)` de `openclaw/plugin-sdk/core` para usar o comportamento de Compaction embutido do OpenClaw.
  </Tab>
</Tabs>

Um `compact()` sem operação é inseguro para um mecanismo ativo que não controla a Compaction, porque desabilita o caminho normal de Compaction de `/compact` e recuperação de estouro para esse slot de mecanismo.
...
<AccordionGroup>
  <Accordion title="Compaction">
    Compaction é uma responsabilidade do mecanismo de contexto. O mecanismo legado delega para a sumarização embutida do OpenClaw. Mecanismos de plugin podem implementar qualquer estratégia de Compaction (resumos em DAG, recuperação vetorial etc.).
  </Accordion>
  <Accordion title="Memory plugins">
    Plugins de memória (`plugins.slots.memory`) são separados dos mecanismos de contexto. Plugins de memória fornecem busca/recuperação; mecanismos de contexto controlam o que o modelo vê. Eles podem trabalhar juntos - um mecanismo de contexto pode usar dados de plugin de memória durante a montagem. Mecanismos de plugin que desejam o caminho de prompt de memória ativo devem preferir `buildMemorySystemPromptAddition(...)` de `openclaw/plugin-sdk/core`, que converte as seções do prompt de memória ativo em um `systemPromptAddition` pronto para ser acrescentado no início. Se um mecanismo precisar de controle de nível mais baixo, ele ainda pode extrair linhas brutas de `openclaw/plugin-sdk/memory-host-core` via `buildActiveMemoryPromptSection(...)`.
  </Accordion>
  <Accordion title="Session pruning">
    O corte de resultados antigos de ferramentas em memória ainda é executado independentemente de qual mecanismo de contexto esteja ativo.
  </Accordion>
</AccordionGroup>
...
- Se trocar de mecanismo, as sessões existentes continuam com seu histórico atual. O novo mecanismo assume para execuções futuras.
- Erros do mecanismo são registrados, e o mecanismo de plugin selecionado fica em quarentena para o processo atual do Gateway. O OpenClaw volta para `legacy` nos turnos do usuário para que as respostas possam continuar, mas você ainda deve reparar, atualizar, desabilitar ou desinstalar o plugin quebrado.
...
- [Contexto](/pt-BR/concepts/context) - como o contexto é construído para turnos de agente
- [Arquitetura de Plugin](/pt-BR/plugins/architecture) - registrando plugins de mecanismo de contexto
- [Manifesto de plugin](/pt-BR/plugins/manifest) - campos do manifesto do plugin
