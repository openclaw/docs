...
read_when:
    ...
    - Esclarecimento de sessões, modos de enfileiramento ou comportamento de streaming
...
title: Mensagens
x-i18n:
    generated_at: "2026-05-06T05:50:59Z"
    model: gpt-5.5
    provider: openai
...
turno do agente via `messages.inbound`. O debounce é escopado por canal + conversa
...
- Comandos de controle ignoram o debounce para permanecerem independentes — **exceto** quando um canal opta explicitamente por coalescência de DMs do mesmo remetente (por exemplo, [BlueBubbles `coalesceSameSenderDms`](/pt-BR/channels/bluebubbles#coalescing-split-send-dms-command--url-in-one-composition)), em que comandos de DM aguardam dentro da janela de debounce para que uma carga enviada em partes possa entrar no mesmo turno do agente.
...
longas para evitar contexto divergente. A UI de Controle e a TUI sempre mostram a
...
- `Body`: fallback legado de prompt. Ele pode incluir envelopes de canal e
...
acionaram uma execução (por exemplo, mensagens filtradas por menção) e **excluem** mensagens
...
blocos de contexto não confiáveis com papel de usuário durante a montagem do prompt.
...
sessão, trabalhos de longa duração são regidos pela sessão, ferramenta e ciclo de vida de
...
A divisão em chunks respeita os limites de texto do canal e evita dividir código cercado por cercas.
...
Detalhes: [Streaming + divisão em chunks](/pt-BR/concepts/streaming).
...
- Conversas diretas proíbem silêncio por padrão e reescrevem uma resposta silenciosa
  isolada para um fallback curto e visível.
...
texto padrão de erro do Gateway. Conversas diretas mostram texto compacto de falha por padrão;
...
- [Refatoração do ciclo de vida de mensagens](/pt-BR/concepts/message-lifecycle-refactor) - design-alvo durável de envio e recebimento
