...
read_when:
    ...
    - Você quer configurações de exemplo ou comandos de integração inicial da CLI para provedores de modelos
sidebarTitle: Model providers
summary: Visão geral dos provedores de modelo com configurações de exemplo + fluxos da CLI
title: Provedores de modelos
x-i18n:
    generated_at: "2026-05-02T05:45:17Z"
    model: gpt-5.5
    provider: openai
...
<AccordionGroup>
  ...
  <Accordion title="Separação entre provedor e runtime da OpenAI">
    ...
    - `openai/<model>` mais `agents.defaults.agentRuntime.id: "codex"` usa o harness nativo de app-server do Codex. Essa é a configuração comum de assinatura ChatGPT/Codex.
    ...
    Consulte [OpenAI](/pt-BR/providers/openai) e [harness do Codex](/pt-BR/plugins/codex-harness). Se a separação entre provedor e runtime estiver confusa, leia [Runtimes de agente](/pt-BR/concepts/agent-runtimes) primeiro.
    ...
    GPT-5.5 está disponível pelo harness nativo de app-server do Codex quando `agentRuntime.id: "codex"` está definido, por `openai-codex/gpt-5.5` no PI para OAuth do Codex, e por `openai/gpt-5.5` no PI para tráfego direto com chave de API quando sua conta o expõe.

  </Accordion>
  <Accordion title="Runtimes da CLI">
    Runtimes da CLI usam a mesma separação: escolha referências de modelo canônicas como `anthropic/claude-*`, `google/gemini-*` ou `openai/gpt-*`, depois defina `agents.defaults.agentRuntime.id` como `claude-cli`, `google-gemini-cli` ou `codex-cli` quando quiser um backend de CLI local.
...
A lista completa de hooks do SDK de provedor e exemplos de Plugin empacotado fica em [Plugins de provedor](/pt-BR/plugins/sdk-provider-plugins). Um provedor que precisa de um executor de requisições totalmente personalizado é uma superfície de extensão separada e mais profunda.

<Note>
O comportamento de runner pertencente ao provedor vive em hooks explícitos de provedor, como política de replay, normalização de esquema de ferramentas, empacotamento de stream e auxiliares de transporte/requisição. O pacote estático legado `ProviderPlugin.capabilities` existe apenas por compatibilidade e não é mais lido pela lógica compartilhada do runner.
</Note>
...
<AccordionGroup>
  ...
  <Accordion title="Quando a rotação entra em ação">
    ...
    - Falhas que não são de limite de taxa falham imediatamente; nenhuma rotação de chaves é tentada.
...
O OpenClaw é fornecido com o catálogo pi‑ai. Esses provedores **não** exigem configuração de `models.providers`; basta definir a autenticação e escolher um modelo.
...
- O aquecimento do WebSocket de Responses da OpenAI vem habilitado por padrão via `params.openaiWsWarmup` (`true`/`false`)
...
- Cabeçalhos ocultos de atribuição do OpenClaw (`originator`, `version`, `User-Agent`) aplicam-se apenas ao tráfego nativo da OpenAI para `api.openai.com`, não a proxies genéricos compatíveis com OpenAI
...
- `openai/gpt-5.3-codex-spark` é suprimido intencionalmente no OpenClaw porque requisições live à API da OpenAI o rejeitam e o catálogo atual do Codex não o expõe
...
- A configuração preferida da CLI do Claude mantém a referência de modelo canônica e seleciona o
  backend da CLI separadamente: `anthropic/claude-opus-4-7` com
...
- Limite de Plugin: `openai-codex/*` carrega o Plugin da OpenAI; o Plugin nativo de app-server do Codex é selecionado apenas pelo runtime de harness do Codex ou por referências legadas `codex/*`.
...
- Compartilha o mesmo alternador `/fast` e a configuração `params.fastMode` de `openai/*` direto; o OpenClaw mapeia isso para `service_tier=priority`
...
- Use `openai-codex/gpt-5.5` apenas quando quiser a rota de OAuth/assinatura do Codex por meio do PI; use `openai/gpt-5.5` sem a substituição de runtime do Codex quando sua configuração de chave de API e catálogo local expuserem a rota de API pública.
...
<CardGroup cols={3}>
  ...
  <Card title="Qwen Cloud" href="/pt-BR/providers/qwen">
    Superfície de provedor Qwen Cloud mais mapeamento de endpoints do Alibaba DashScope e Coding Plan.
  </Card>
</CardGroup>
...
O OAuth do Gemini CLI é enviado como parte do Plugin `google` incluído.
...
<Steps>
  ...
  <Step title="Defina o projeto (se necessário)">
...
- O catálogo fallback estático inclui `kilocode/kilo/auto`; a descoberta ativa em `https://api.kilo.ai/api/gateway/models` pode expandir ainda mais o catálogo em tempo de execução.
...
| Provedor                | ID                               | Variável de ambiente de autenticação                         | Modelo de exemplo                             |
| ----------------------- | -------------------------------- | ------------------------------------------------------------ | --------------------------------------------- |
...
<AccordionGroup>
  <Accordion title="OpenRouter">
    Aplica seus cabeçalhos de atribuição de app e marcadores Anthropic `cache_control` somente em rotas `openrouter.ai` verificadas. Referências DeepSeek, Moonshot e ZAI são elegíveis a cache-TTL para cache de prompts gerenciado pelo OpenRouter, mas não recebem marcadores de cache da Anthropic. Como um caminho compatível com OpenAI em estilo de proxy, ele ignora a conformação exclusiva da OpenAI nativa (`serviceTier`, Responses `store`, dicas de cache de prompt, compatibilidade de raciocínio com OpenAI). Referências baseadas em Gemini mantêm apenas a sanitização de assinaturas de pensamento proxy-Gemini.
  </Accordion>
  ...
  <Accordion title="MiniMax">
    O onboarding por chave de API grava definições explícitas de modelos de chat M2.7 somente texto; o entendimento de imagem permanece no provedor de mídia `MiniMax-VL-01` pertencente ao plugin.
  </Accordion>
  <Accordion title="NVIDIA">
    IDs de modelo usam um namespace `nvidia/<vendor>/<model>` (por exemplo, `nvidia/nvidia/nemotron-...` junto de `nvidia/moonshotai/kimi-k2.5`); os seletores preservam a composição literal `<provider>/<model-id>`, enquanto a chave canônica enviada à API permanece com um único prefixo.
  </Accordion>
  <Accordion title="xAI">
...
Muitos dos plugins de provedor incluídos abaixo já publicam um catálogo padrão. Use entradas explícitas `models.providers.<id>` somente quando quiser substituir a URL base, cabeçalhos ou lista de modelos padrão.

As verificações de capacidade de modelo do Gateway também leem metadados explícitos de `models.providers.<id>.models[]`. Se um modelo personalizado ou de proxy aceitar imagens, defina `input: ["text", "image"]` nesse modelo para que o WebChat e os caminhos de anexos originados em Node enviem imagens como entradas nativas do modelo, em vez de referências de mídia somente texto.
...
Nos seletores de modelo de integração inicial/configuração, a opção de autenticação do Volcengine prefere tanto linhas `volcengine/*` quanto `volcengine-plan/*`. Se esses modelos ainda não estiverem carregados, o OpenClaw recorre ao catálogo sem filtro em vez de mostrar um seletor vazio com escopo de provedor.
...
Nos seletores de modelo de integração inicial/configuração, a opção de autenticação do BytePlus prefere tanto linhas `byteplus/*` quanto `byteplus-plan/*`. Se esses modelos ainda não estiverem carregados, o OpenClaw recorre ao catálogo sem filtro em vez de mostrar um seletor vazio com escopo de provedor.
...
- A compreensão de imagem é `MiniMax-VL-01`, de propriedade do Plugin, em ambos os caminhos de autenticação do MiniMax
...
O OpenClaw usa os endpoints nativos `/api/v1/models` e `/api/v1/models/load` do LM Studio para descoberta + carregamento automático, com `/v1/chat/completions` para inferência por padrão. Se você quiser que carregamento JIT, TTL e remoção automática do LM Studio sejam responsáveis pelo ciclo de vida do modelo, defina `models.providers.lmstudio.params.preload: false`. Veja [/providers/lmstudio](/pt-BR/providers/lmstudio) para configuração e solução de problemas.
...
Ollama é detectado localmente em `http://127.0.0.1:11434` quando você opta por usar `OLLAMA_API_KEY`, e o Plugin de provedor incluído adiciona Ollama diretamente ao `openclaw onboard` e ao seletor de modelos. Veja [/providers/ollama](/pt-BR/providers/ollama) para integração inicial, modo nuvem/local e configuração personalizada.
...
SGLang é distribuído como um Plugin de provedor incluído para servidores auto-hospedados rápidos compatíveis com OpenAI:
...
<AccordionGroup>
  ...
  <Accordion title="Proxy-route shaping rules">
    ...
    - Para controles de template de chat do vLLM, defina `agents.defaults.models["provider/model"].params.chat_template_kwargs`. O Plugin vLLM incluído envia automaticamente `enable_thinking: false` e `force_nonempty_content: true` para `vllm/nemotron-3-*` quando o nível de thinking da sessão está desligado.
    - Para modelos locais lentos ou hosts LAN/tailnet remotos, defina `models.providers.<id>.timeoutSeconds`. Isso estende o tratamento de requisições HTTP do modelo do provedor, incluindo conexão, cabeçalhos, streaming do corpo e o abort total de busca protegida, sem aumentar o timeout de todo o runtime do agente.
