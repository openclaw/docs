...
x-i18n:
    generated_at: "2026-05-10T19:31:07Z"
    model: gpt-5.5
    provider: openai
...
<AccordionGroup>
  ...
  <Accordion title="Separação entre provedor/runtime da OpenAI">
    ...
    - `openai/<model>` usa o harness nativo de servidor de app do Codex para turnos de agente por padrão. Esta é a configuração usual de assinatura ChatGPT/Codex.
    - `openai-codex/<model>` é uma configuração legada que o doctor reescreve para `openai/<model>`.
    - `openai/<model>` mais provider/model `agentRuntime.id: "pi"` usa PI para rotas explícitas de chave de API ou compatibilidade.
    ...
    A ativação automática de Plugin segue o mesmo limite: referências de agente `openai/*` ativam o Plugin Codex para a rota padrão, e referências explícitas provider/model `agentRuntime.id: "codex"` ou referências legadas `codex/<model>` também o exigem.

    GPT-5.5 está disponível por meio do harness nativo de servidor de app do Codex por padrão em `openai/gpt-5.5`, e por meio do PI somente quando a política de runtime provider/model seleciona explicitamente `pi`.

  </Accordion>
  <Accordion title="Runtimes de CLI">
    Runtimes de CLI usam a mesma separação: escolha referências de modelo canônicas como `anthropic/claude-*`, `google/gemini-*` ou `openai/gpt-*`, depois defina a política de runtime provider/model para `claude-cli`, `google-gemini-cli` ou `codex-cli` quando quiser um backend de CLI local.
...
A maior parte da lógica específica de provedor vive em Plugins de provedor (`registerProvider(...)`), enquanto o OpenClaw mantém o loop genérico de inferência. Plugins são responsáveis por onboarding, catálogos de modelos, mapeamento de variáveis de ambiente de autenticação, normalização de transporte/configuração, limpeza de esquema de ferramentas, classificação de failover, atualização OAuth, relatórios de uso, perfis de pensamento/raciocínio e mais.

A lista completa de hooks do SDK de provedor e exemplos de Plugins incluídos está em [Plugins de provedor](/pt-BR/plugins/sdk-provider-plugins). Um provedor que precisa de um executor de requisições totalmente personalizado é uma superfície de extensão separada e mais profunda.

<Note>
O comportamento de runner pertencente ao provedor vive em hooks explícitos de provedor, como política de replay, normalização de esquema de ferramentas, empacotamento de stream e auxiliares de transporte/requisição. O pacote estático legado `ProviderPlugin.capabilities` existe apenas para compatibilidade e não é mais lido pela lógica compartilhada de runner.
</Note>
...
O OpenClaw inclui o catálogo pi-ai. Esses provedores **não** exigem configuração `models.providers`; basta definir a autenticação e escolher um modelo.
...
- O transporte padrão é `auto`; o OpenClaw passa a escolha de transporte para o pi-ai.
...
- `/fast` e `params.fastMode` mapeiam requisições diretas Responses `openai/*` para `service_tier=priority` em `api.openai.com`
...
- Rotas nativas da OpenAI também mantêm `store` de Responses, dicas de cache de prompt e modelagem de payload compatível com raciocínio da OpenAI; rotas de proxy não
...
- Requisições públicas diretas da Anthropic dão suporte ao alternador compartilhado `/fast` e `params.fastMode`, incluindo tráfego com chave de API e autenticado por OAuth enviado para `api.anthropic.com`; o OpenClaw mapeia isso para `service_tier` da Anthropic (`auto` vs `standard_only`)
- A configuração preferida da Claude CLI mantém a referência de modelo canônica e seleciona o backend de CLI separadamente: `anthropic/claude-opus-4-7` com `agentRuntime.id: "claude-cli"` com escopo de modelo. Referências legadas `claude-cli/claude-opus-4-7` ainda funcionam por compatibilidade.

<Note>
A equipe da Anthropic nos informou que o uso da Claude CLI no estilo OpenClaw voltou a ser permitido, então o OpenClaw trata a reutilização da Claude CLI e o uso de `claude -p` como sancionados para esta integração, a menos que a Anthropic publique uma nova política. O setup-token da Anthropic continua disponível como um caminho de token do OpenClaw com suporte, mas agora o OpenClaw prefere a reutilização da Claude CLI e `claude -p` quando disponíveis.
</Note>
...
- Referência de modelo legada do PI: `openai-codex/gpt-5.5`
- Referência do harness nativo de servidor de app do Codex: `openai/gpt-5.5`
- Documentação do harness nativo de servidor de app do Codex: [harness do Codex](/pt-BR/plugins/codex-harness)
- Referências de modelo legadas: `codex/gpt-*`
- Limite de Plugin: `openai-codex/*` carrega o Plugin OpenAI; o Plugin nativo de servidor de app do Codex é selecionado somente pelo runtime do harness do Codex ou por referências legadas `codex/*`.
...
- `params.serviceTier` também é encaminhado em requisições nativas Responses do Codex (`chatgpt.com/backend-api`)
...
- Compartilha o mesmo alternador `/fast` e a configuração `params.fastMode` das rotas diretas `openai/*`; o OpenClaw mapeia isso para `service_tier=priority`
...
- Para a rota comum de assinatura mais runtime nativo do Codex, entre com a autenticação `openai-codex`, mas configure `openai/gpt-5.5`; turnos de agente da OpenAI selecionam Codex por padrão.
- Use provider/model `agentRuntime.id: "pi"` somente quando quiser uma rota de compatibilidade por meio do PI; caso contrário, mantenha `openai/gpt-5.5` no harness Codex padrão.
- Referências antigas `openai-codex/gpt-5.1*`, `openai-codex/gpt-5.2*` e `openai-codex/gpt-5.3*` são suprimidas porque contas OAuth ChatGPT/Codex as rejeitam; use `openai-codex/gpt-5.5` ou a rota de runtime nativa do Codex em vez disso.
...
<CardGroup cols={3}>
  <Card title="Modelos GLM" href="/pt-BR/providers/glm">
    Plano de Coding da Z.AI ou endpoints gerais de API.
  </Card>
  <Card title="MiniMax" href="/pt-BR/providers/minimax">
    OAuth do Plano de Coding da MiniMax ou acesso por chave de API.
  </Card>
  <Card title="Qwen Cloud" href="/pt-BR/providers/qwen">
    Superfície do provedor Qwen Cloud mais mapeamento de endpoints Alibaba DashScope e Plano de Coding.
  </Card>
</CardGroup>
...
<Warning>
O OAuth do Gemini CLI no OpenClaw é uma integração não oficial. Alguns usuários relataram restrições em contas Google após usar clientes de terceiros. Revise os termos do Google e use uma conta não crítica se optar por prosseguir.
</Warning>
...
- CLI: `openclaw onboard --auth-choice zai-api-key`
  - Alias: `z.ai/*` e `z-ai/*` são normalizados para `zai/*`
...
- O catálogo estático de fallback distribui `kilocode/kilo/auto`; a descoberta ativa em `https://api.kilo.ai/api/gateway/models` pode expandir ainda mais o catálogo de runtime.
...
<AccordionGroup>
  <Accordion title="OpenRouter">
    Aplica seus cabeçalhos de atribuição de app e marcadores Anthropic `cache_control` somente em rotas `openrouter.ai` verificadas. Referências DeepSeek, Moonshot e ZAI são elegíveis a TTL de cache para cache de prompt gerenciado pelo OpenRouter, mas não recebem marcadores de cache Anthropic. Como um caminho compatível com OpenAI em estilo proxy, ele ignora formatação exclusiva de OpenAI nativa (`serviceTier`, `store` de Responses, dicas de cache de prompt, compatibilidade de raciocínio da OpenAI). Referências baseadas em Gemini mantêm apenas a sanitização de assinatura de pensamento do proxy-Gemini.
  </Accordion>
  <Accordion title="Kilo Gateway">
    Referências baseadas em Gemini seguem o mesmo caminho de sanitização de proxy-Gemini; `kilocode/kilo/auto` e outras referências de proxy sem suporte a raciocínio ignoram a injeção de raciocínio por proxy.
  </Accordion>
  ...
  <Accordion title="NVIDIA">
    IDs de modelo usam um namespace `nvidia/<vendor>/<model>` (por exemplo, `nvidia/nvidia/nemotron-...` junto de `nvidia/moonshotai/kimi-k2.5`); seletores preservam a composição literal `<provider>/<model-id>`, enquanto a chave canônica enviada para a API permanece com prefixo único.
  </Accordion>
  <Accordion title="xAI">
    Usa o caminho Responses da xAI. `grok-4.3` é o modelo de chat padrão incluído. `/fast` ou `params.fastMode: true` reescreve `grok-3`, `grok-3-mini`, `grok-4` e `grok-4-0709` para suas variantes `*-fast`. `tool_stream` vem ativado por padrão; desative via `agents.defaults.models["xai/<model>"].params.tool_stream=false`.
  </Accordion>
...
As verificações de capacidade de modelo do Gateway também leem metadados explícitos de `models.providers.<id>.models[]`. Se um modelo personalizado ou proxy aceitar imagens, defina `input: ["text", "image"]` nesse modelo para que os caminhos de anexos do WebChat e originados em nós transmitam imagens como entradas nativas do modelo, em vez de referências de mídia somente texto.

`agents.defaults.models["provider/model"]` controla apenas visibilidade de modelos, aliases e metadados por modelo para agentes. Ele não registra um novo modelo de runtime por si só. Para modelos de provedor personalizados, adicione também `models.providers.<provider>.models[]` com pelo menos o `id` correspondente.
...
Os modelos legados `kimi/kimi-code` e `kimi/k2p5` continuam aceitos como IDs de modelo de compatibilidade e são normalizados para o ID de modelo de API estável do Kimi.
...
A configuração inicial usa a superfície de codificação por padrão, mas o catálogo geral `volcengine/*` é registrado ao mesmo tempo.

Nos seletores de modelo de configuração inicial/configuração, a opção de autenticação do Volcengine prefere as linhas `volcengine/*` e `volcengine-plan/*`. Se esses modelos ainda não estiverem carregados, o OpenClaw volta para o catálogo sem filtro em vez de mostrar um seletor vazio com escopo de provedor.
...
A configuração inicial usa a superfície de codificação por padrão, mas o catálogo geral `byteplus/*` é registrado ao mesmo tempo.

Nos seletores de modelo de configuração inicial/configuração, a opção de autenticação do BytePlus prefere as linhas `byteplus/*` e `byteplus-plan/*`. Se esses modelos ainda não estiverem carregados, o OpenClaw volta para o catálogo sem filtro em vez de mostrar um seletor vazio com escopo de provedor.
...
<Note>
No caminho de streaming compatível com Anthropic do MiniMax, o OpenClaw desabilita o pensamento por padrão, a menos que você o defina explicitamente, e `/fast on` reescreve `MiniMax-M2.7` para `MiniMax-M2.7-highspeed`.
</Note>
...
- A compreensão de imagens é `MiniMax-VL-01`, pertencente ao Plugin, nos dois caminhos de autenticação do MiniMax
...
O Ollama é detectado localmente em `http://127.0.0.1:11434` quando você habilita explicitamente com `OLLAMA_API_KEY`, e o Plugin de provedor incluído adiciona o Ollama diretamente ao `openclaw onboard` e ao seletor de modelos. Consulte [/providers/ollama](/pt-BR/providers/ollama) para configuração inicial, modo em nuvem/local e configuração personalizada.

### vLLM

vLLM é fornecido como um Plugin de provedor incluído para servidores locais/hospedados pelo próprio usuário compatíveis com OpenAI:
...
SGLang é fornecido como um Plugin de provedor incluído para servidores rápidos hospedados pelo próprio usuário compatíveis com OpenAI:
...
<AccordionGroup>
  ...
  <Accordion title="Regras de formatação de rotas de proxy">
    ...
    - Para controles de template de chat do vLLM, defina `agents.defaults.models["provider/model"].params.chat_template_kwargs`. O Plugin vLLM incluído envia automaticamente `enable_thinking: false` e `force_nonempty_content: true` para `vllm/nemotron-3-*` quando o nível de pensamento da sessão está desligado.
    - Para modelos locais lentos ou hosts remotos de LAN/tailnet, defina `models.providers.<id>.timeoutSeconds`. Isso estende o tratamento de requisições HTTP do modelo do provedor, incluindo conexão, cabeçalhos, streaming do corpo e o abort total de guarded-fetch, sem aumentar o timeout de todo o runtime do agente.
    - Chamadas HTTP do provedor de modelos permitem respostas DNS de fake-IP do Surge, Clash e sing-box em `198.18.0.0/15` e `fc00::/7` somente para o nome de host `baseUrl` configurado do provedor. Outros destinos privados, de loopback, link-local e de metadados ainda exigem uma habilitação explícita de `models.providers.<id>.request.allowPrivateNetwork: true`.
