...
x-i18n:
    generated_at: "2026-05-02T05:45:57Z"
    model: gpt-5.5
    provider: openai
...
| `subagents`        | Lista, orienta ou encerra subagentes gerados para esta sessão                 |
...
chamador já pode ver segundo a política configurada de visibilidade de
ferramentas de sessão, então sessões não relacionadas permanecem ocultas.
...
- o texto do assistente é normalizado antes da recuperação:
  ...
  - blocos de payload XML de chamada de ferramenta em texto puro, como `<tool_call>...</tool_call>`,
  ...
  - tokens vazados de controle de modelo, como `<|assistant|>`, outros tokens
    ASCII `<|...|>` e variantes em largura total `<｜...｜>`, são removidos
...
Se você precisar da transcrição exata byte por byte, inspecione o arquivo de
...
Sessões de chat com escopo de thread, como chaves do Slack ou Discord que
terminam em `:thread:<id>`, não são destinos válidos para `sessions_send`. Use a
chave da sessão do canal pai para coordenação entre agentes, para que mensagens
roteadas por ferramentas não apareçam dentro de uma thread ativa voltada a
humanos.
...
roteados por ferramenta, não como uma instrução diretamente escrita pelo usuário
final.
...
ou outra sessão visível. Ela relata uso, tempo, estado de modelo/runtime e
contexto vinculado de tarefa em segundo plano quando presente. Assim como
`/status`, ela pode preencher contadores esparsos de token/cache a partir da
entrada de uso mais recente da transcrição, e `model=default` limpa uma
...
depois de gerar subagentes quando quiser que os resultados de conclusão cheguem
...
- `context: "fork"` para subagentes nativos quando o filho precisa da transcrição
  atual do solicitante; omita ou use `context: "isolated"` para um filho limpo.
...
folha ainda não recebem ferramentas recursivas de orquestração.
...
O padrão é `tree`. Sessões em sandbox são limitadas a `tree` independentemente
da configuração.
