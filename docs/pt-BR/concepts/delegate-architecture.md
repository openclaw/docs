...
- Segue **[ordens permanentes](/pt-BR/automation/standing-orders)** - regras definidas no `AGENTS.md` do agente que especificam o que ele pode fazer de forma autônoma vs. o que exige aprovação humana (veja [Tarefas Cron](/pt-BR/automation/cron-jobs) para execução agendada).

O modelo de delegado corresponde diretamente a como assistentes executivos trabalham: eles têm suas próprias credenciais, enviam emails "em nome de" seu principal e seguem um escopo de autoridade definido.
...
Comece pelo nível mais baixo que atende às suas necessidades. Escale apenas quando o caso de uso exigir.
...
- Email: ler caixa de entrada, resumir threads, sinalizar itens para ação humana.
- Calendário: ler eventos, mostrar conflitos, resumir o dia.
- Arquivos: ler documentos compartilhados, resumir conteúdo.

Esse nível exige apenas permissões de leitura do provedor de identidade. O agente não escreve em nenhuma caixa de correio ou calendário - rascunhos e propostas são entregues via chat para que o humano aja.
...
Esse nível exige permissões de envio em nome de (ou delegado).
...
- Informativos matinais entregues a um canal.
- Publicação automatizada em redes sociais via filas de conteúdo aprovadas.
- Triagem de caixa de entrada com categorização e sinalização automáticas.

Esse nível combina permissões de Nível 2 com [Tarefas Cron](/pt-BR/automation/cron-jobs) e [Ordens Permanentes](/pt-BR/automation/standing-orders).
...
<Note>
**Faça isso primeiro.** Antes de conceder credenciais ou acesso ao provedor de identidade, restrinja os limites do delegado. As etapas desta seção definem o que o agente **não pode** fazer. Estabeleça essas restrições antes de dar a ele a capacidade de fazer qualquer coisa.
</Note>

### Bloqueios rígidos (inegociáveis)

Defina isso no `SOUL.md` e no `AGENTS.md` do delegado antes de conectar quaisquer contas externas:
...
Essas regras são carregadas em cada sessão. Elas são a última linha de defesa, independentemente das instruções que o agente receba.
...
Para implantações de alta segurança, coloque o agente delegado em sandbox para que ele não possa acessar o sistema de arquivos do host ou a rede além das ferramentas permitidas:
...
Configure o registro antes que o delegado lide com dados reais:
...
Com a proteção em vigor, prossiga para conceder ao delegado sua identidade e permissões.
...
- `USER.md`: informações sobre o(s) principal(is) que o delegado atende.

### 2. Configurar delegação no provedor de identidade

O delegado precisa de sua própria conta no seu provedor de identidade com permissões explícitas de delegação. **Aplique o princípio do menor privilégio** - comece com o Nível 1 (somente leitura) e escale apenas quando o caso de uso exigir.
...
<Warning>
A delegação em todo o domínio permite que a conta de serviço se passe por **qualquer usuário em todo o domínio**. Restrinja os escopos ao mínimo necessário e limite o ID de cliente da conta de serviço apenas aos escopos listados acima no Admin Console (Security > API controls > Domain-wide delegation). Uma chave de conta de serviço vazada com escopos amplos concede acesso total a todas as caixas de correio e calendários da organização. Rotacione chaves em uma agenda e monitore o log de auditoria do Admin Console para eventos inesperados de representação.
</Warning>

### 3. Vincular o delegado aos canais

Roteie mensagens recebidas para o agente delegado usando vínculos de [Roteamento Multiagente](/pt-BR/concepts/multi-agent):
...
Se você conceder `sessions_history`, lembre-se de que essa é uma visão de recordação limitada e filtrada por segurança. O OpenClaw redige textos semelhantes a credenciais/tokens, trunca conteúdos longos, remove tags de raciocínio / estruturas de `<relevant-memories>` / cargas XML de chamadas de ferramenta em texto simples (incluindo `<tool_call>...</tool_call>`, `<function_call>...</function_call>`, `<tool_calls>...</tool_calls>`, `<function_calls>...</function_calls>` e blocos truncados de chamadas de ferramenta) / estruturas rebaixadas de chamadas de ferramenta / tokens de controle de modelo ASCII/largura completa vazados / XML de chamada de ferramenta MiniMax malformado da recordação do assistente, e pode substituir linhas grandes demais por `[sessions_history omitted: message too large]` em vez de retornar um despejo bruto da transcrição.
...
3. **Conceda permissões com escopo definido** por meio do provedor de identidade (menor privilégio).
...
Várias organizações podem compartilhar um servidor Gateway usando roteamento multiagente - cada organização recebe seu próprio agente, workspace e credenciais isolados.
