...
x-i18n:
    generated_at: "2026-07-01T12:51:21Z"
    model: gpt-5.5
...
- Em ambientes headless, crie um token de API na UI web do ClawHub e execute:
...
- Se você usa um token de API, confirme que ele não foi revogado na UI web.

## Pesquisar ou instalar retorna `Rate limit exceeded` (429)
...
## Pesquisar ou instalar falha atrás de um proxy
...
- Confirme que a versão está pública e não retida por varredura ou moderação.
...
Se uma publicação falhar porque o handle do proprietário, namespace da org, escopo do pacote, slug da skill
...
Se você acredita que sua org, projeto ou marca é o proprietário legítimo do namespace, mas
não consegue gerenciar o proprietário atual do ClawHub, abra uma
[issue de Reivindicação de Org / Namespace](https://github.com/openclaw/clawhub/issues/new?template=org-namespace-claim.yml)
com prova pública e não sensível. Consulte
[Reivindicações de Org e Namespace](/clawhub/namespace-claims) para orientação sobre evidências e o que
...
- Verifique a página de detalhes do pacote para ver o status de varredura e os metadados de compatibilidade.
