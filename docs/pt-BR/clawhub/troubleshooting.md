...
Para plugins, verifique os metadados de compatibilidade de `package.json`. Publicações de Plugin de código
precisam de campos de compatibilidade do OpenClaw, como `openclaw.compat.pluginApi` e
`openclaw.build.openclawVersion`.

Pré-visualize primeiro a carga útil da publicação:
...
## `update` se recusa por causa de alterações locais
...
- Verifique a página de detalhes do pacote para ver o estado da verificação e os metadados de compatibilidade.
