...
summary: Solucionando problemas de login, instalação, publicação, sincronização, atualização e API do ClawHub.
x-i18n:
...
A CLI inicia um servidor de retorno local de curta duração durante o login pelo navegador.

- Certifique-se de que seu navegador consegue acessar `http://127.0.0.1:<port>/callback`.
- Verifique as regras de firewall local, VPN e proxy se o retorno nunca chegar.
...
- Substitua pela versão publicada:
