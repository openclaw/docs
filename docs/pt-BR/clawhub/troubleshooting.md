...
x-i18n:
    generated_at: "2026-07-01T20:15:28Z"
    model: gpt-5.5
...
Os nomes aceitos incluem `HTTPS_PROXY`, `HTTP_PROXY`, `https_proxy` e
`http_proxy`.

## Um Skill não aparece na busca
...
- Se você é o proprietário do Skill, entre e inspecione-o:
...
Se uma publicação falhar porque o identificador do proprietário, namespace da organização, escopo do pacote, slug do Skill
...
## `sync` diz que nenhum Skill foi encontrado
...
Pré-visualize primeiro se você não tem certeza do que será publicado:
...
- Se o pacote estiver oculto, retido ou bloqueado, talvez ele não seja instalável até
...
- Direcione usuários de volta à listagem canônica do ClawHub.
