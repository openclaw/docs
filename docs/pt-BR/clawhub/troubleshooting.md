...
x-i18n:
    generated_at: "2026-07-03T13:21:24Z"
    model: gpt-5.5
...
ferramentas obrigatórias devem ser declaradas para que usuários e verificadores entendam o pacote.
...
- Certifique-se de que você entrou com a conta do GitHub que é proprietária ou pode publicar
  o pacote.
...
Se uma publicação falhar porque o identificador do proprietário, namespace da org, escopo do pacote, slug da skill
...
com comprovação pública e não sensível. Consulte
[Reivindicações de org e namespace](/clawhub/namespace-claims) para orientações sobre evidências e o que
manter fora de issues públicas.

## `sync` diz que nenhuma skill foi encontrada

`sync` procura pastas contendo `SKILL.md` ou `skill.md`.
