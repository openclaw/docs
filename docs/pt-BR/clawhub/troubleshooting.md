---
read_when:
    - Os comandos da CLI do ClawHub ou do registro do OpenClaw falham
...
x-i18n:
    generated_at: "2026-07-05T04:52:45Z"
    model: gpt-5.5
...
- Faça login novamente com `clawhub login`.
...
- `RateLimit-Remaining`: seu orçamento restante exato quando o cabeçalho está presente. Em `429`, é `0`.
...
pessoa envia apenas algumas solicitações. Faça login quando possível e tente novamente após o
atraso informado.
...
- Verifique o slug exato ou a página do proprietário se você a conhece.
- Confirme que a versão é pública e não está retida por varredura ou moderação.
- Se você é proprietário da skill, faça login e inspecione-a:

```bash
clawhub inspect @openclaw/demo
```

Diagnósticos visíveis ao proprietário podem explicar o estado de varredura, bloqueio de envio ou moderação.
...
## A publicação falha com erro de proprietário do GitHub ou de origem
...
nomes com escopo, como `@example-org/example-plugin`, devem ser publicados como o proprietário
...
[Reivindicações de Organização e Namespace](/clawhub/namespace-claims) para orientação sobre evidências e sobre o que
...
Aponte-o para as raízes que você deseja escanear:
...
Os arquivos locais não correspondem a nenhuma versão que o ClawHub conhece. Escolha uma opção:

- Manter edições locais e ignorar a atualização.
...
- Verifique a página de detalhes do pacote para estado de varredura e metadados de compatibilidade.
...
- Respeite os cabeçalhos de nova tentativa de `429` e armazene em cache respostas públicas de lista/busca.
