...
x-i18n:
    generated_at: "2026-06-30T13:51:59Z"
    model: gpt-5.5
...
- Verifique regras locais de firewall, VPN e proxy se o callback nunca chegar.
...
## Uma habilidade não aparece na busca

- Verifique o slug exato ou a página do proprietário, se você souber.
- Confirme que o lançamento é público e não está retido por varredura ou moderação.
- Se você é proprietário da habilidade, entre e inspecione-a:
...
Para habilidades, verifique o frontmatter de `SKILL.md`. Variáveis de ambiente e
ferramentas obrigatórias devem ser declaradas para que usuários e scanners possam entender o pacote.

Para plugins, verifique os metadados de compatibilidade de `package.json`. Publicações de plugins de código
precisam de campos de compatibilidade do OpenClaw, como `openclaw.compat.pluginApi` e
`openclaw.build.openclawVersion`.

Visualize primeiro a carga útil de publicação:
...
Se uma publicação falhar porque o identificador do proprietário, namespace da organização, escopo do pacote, slug da habilidade
ou nome do pacote já foi reivindicado ou reservado, primeiro confirme que você está
publicando com o proprietário que corresponde ao namespace. Para pacotes de plugins,
...
[Reivindicações de Organização e Namespace](/clawhub/namespace-claims) para orientações sobre evidências e sobre o que
manter fora de issues públicas.

## `sync` diz que nenhuma habilidade foi encontrada

`sync` procura pastas que contenham `SKILL.md` ou `skill.md`.

Aponte para as raízes que você deseja verificar:
...
- Verifique a página de detalhes do pacote para consultar o status da varredura e os metadados de compatibilidade.
...
- Se o pacote estiver oculto, retido ou bloqueado, talvez ele não possa ser instalado até
  o proprietário resolver o problema.
