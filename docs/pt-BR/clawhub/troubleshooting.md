...
summary: Solução de problemas de login, instalação, publicação, atualização e problemas de API do ClawHub.
x-i18n:
    generated_at: "2026-07-04T03:38:58Z"
    model: gpt-5.5
...
Diagnósticos visíveis ao proprietário podem explicar o estado de varredura, gate de upload ou moderação.
...
Para plugins, verifique os metadados de compatibilidade de `package.json`. Publicações de code-plugin
precisam de campos de compatibilidade do OpenClaw, como `openclaw.compat.pluginApi` e
`openclaw.build.openclawVersion`.

Visualize primeiro a carga de publicação:
...
O ClawHub usa a identidade do GitHub e a atribuição de origem para conectar pacotes aos seus
publicadores.
...
[Reivindicações de Organização e Namespace](/clawhub/namespace-claims) para orientações de evidências e o que
...
- Manter as edições locais e ignorar a atualização.
...
- Respeite cabeçalhos de nova tentativa `429` e armazene em cache respostas públicas de listagem/busca.
