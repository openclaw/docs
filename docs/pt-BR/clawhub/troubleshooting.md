...
x-i18n:
    generated_at: "2026-07-04T17:51:39Z"
    model: gpt-5.5
...
- Verifique se o seu navegador consegue acessar `http://127.0.0.1:<port>/callback`.
...
## Uma habilidade não aparece na pesquisa
...
- Se você é o proprietário da habilidade, entre e inspecione-a:
...
Para plugins, verifique os metadados de compatibilidade de `package.json`. Publicações de
code-plugin precisam de campos de compatibilidade do OpenClaw, como `openclaw.compat.pluginApi` e
...
- Verifique se você entrou com a conta do GitHub que é proprietária ou pode publicar
  o pacote.
...
[Reivindicações de Organização e Namespace](/clawhub/namespace-claims) para orientações de evidência e o que
...
- Substituir pela versão publicada:
