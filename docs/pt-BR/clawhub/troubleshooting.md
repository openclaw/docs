...
x-i18n:
    generated_at: "2026-06-30T22:09:08Z"
    model: gpt-5.5
...
- Em ambientes sem interface gráfica, crie um token de API na UI web do ClawHub e execute:
...
## A publicação falha com erro de proprietário do GitHub ou origem
...
Se uma publicação falhar porque o handle do proprietário, namespace da organização, escopo do pacote, slug da Skill
...
Pré-visualize primeiro se não tiver certeza do que será publicado:
...
- Verifique a página de detalhes do pacote para estado de verificação e metadados de compatibilidade.
- Confirme que sua versão do OpenClaw atende ao intervalo de compatibilidade
  anunciado pelo pacote.
- Se o pacote estiver oculto, retido ou bloqueado, talvez ele não seja instalável até que
...
- Encaminhe usuários de volta para a listagem canônica do ClawHub.
