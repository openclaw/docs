...
summary: Corrija os achados de validação do pacote de Plugin do ClawHub antes da publicação
title: Plugin validation fixes
x-i18n:
    generated_at: "2026-07-05T04:52:02Z"
    model: gpt-5.5
...
achados que o autor do Plugin pode corrigir nos metadados do pacote, manifesto, importações
do SDK ou artefato publicado.
...
| `sdk-session-transcript-file-target`    | [Substituir destinos de arquivo de transcrição legados](/pt-BR/clawhub/plugin-validation-fixes#sdk-session-transcript-file-target)                   |
...
O pacote não declara o intervalo da API de Plugin do OpenClaw ao qual oferece suporte.

- Adicione `openclaw.compat.pluginApi` ao `package.json`.
- Use a versão da API de Plugin do OpenClaw ou o piso semver contra o qual você desenvolveu e testou.
...
contra os quais o pacote foi desenvolvido.
...
- Alinhe a versão mínima do host com o intervalo de versões de host ao qual o pacote
  realmente oferece suporte.
- Consulte [campos de package.json que afetam a descoberta](/pt-BR/plugins/manifest#packagejson-fields-that-affect-discovery).
- Execute novamente `clawhub package validate <path-to-plugin>`.
...
O bloco `package.json#openclaw` contém campos que não são metadados de pacote OpenClaw
compatíveis.
...
- Atualize `files`, `.npmignore` ou a saída de build para que os pontos de entrada
  declarados sejam incluídos.
- Consulte [Pontos de entrada de Plugin](/pt-BR/plugins/sdk-entrypoints).
- Execute novamente `clawhub package validate <path-to-plugin>`.
...
- Substitua imports do barrel raiz por imports de subcaminhos públicos focados.
...
- Use [Convenções de import](/pt-BR/plugins/building-plugins#import-conventions) e
  [Subcaminhos do SDK do Plugin](/pt-BR/plugins/sdk-subpaths) para encontrar o import restrito.
- Execute novamente `clawhub package validate <path-to-plugin>`.
...
- Substitua imports internos reservados do SDK do OpenClaw por subcaminhos públicos
...
- Use [Subcaminhos do SDK do Plugin](/pt-BR/plugins/sdk-subpaths) e
  [Migração do SDK](/pt-BR/plugins/sdk-migration) para escolher um import compatível.
- Execute novamente `clawhub package validate <path-to-plugin>`.
...
- Evite carregar, modificar e salvar o objeto inteiro de armazenamento de sessão.
- Mantenha helpers de gravação de armazenamento completo apenas enquanto seu intervalo de compatibilidade declarado
...
- Use helpers de identidade de transcrição ou destino quando o código estiver preparando uma
...
O Plugin ainda usa helpers de transcrição de baixo nível obsoletos, como
...
- Prefira a superfície estruturada de runtime de transcrição para que o OpenClaw possa aplicar
  os limites corretos de transação e o tratamento de identidade.
...
- Espelhe os metadados de env-var do provedor em `setup.providers[].envVars`.
...
O manifesto usa metadados legados ou antigos de env-var de canal sem os metadados atuais
...
- Espelhe o setup de canal orientado por env para o setup atual, configuração de canal ou
...
- Remova `openclaw.security.json` até que o OpenClaw documente um esquema de manifesto
  de segurança versionado e o comportamento do ClawHub.
