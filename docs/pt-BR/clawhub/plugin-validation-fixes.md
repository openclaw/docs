...
read_when:
    ...
    - ClawHub rejeitou ou alertou em uma publicação de pacote de Plugin
    - Você está atualizando os metadados do pacote do plugin antes do lançamento
summary: Corrigir as descobertas de validação do pacote de Plugin do ClawHub antes da publicação
title: Correções de validação de Plugin
x-i18n:
    generated_at: "2026-07-05T05:26:26Z"
    model: gpt-5.5
...
varreduras automatizadas de pacotes. Esta página aborda constatações voltadas ao autor, ou seja,
...
Ela não aborda constatações internas de cobertura do Inspetor de Plugin. Se um relatório completo
...
| `package-plugin-api-compat-missing`     | [Declare a compatibilidade da API do Plugin](/pt-BR/clawhub/plugin-validation-fixes#package-plugin-api-compat-missing)                          |
...
| `package-openclaw-unsupported-metadata` | [Remova metadados de pacote OpenClaw incompatíveis](/pt-BR/clawhub/plugin-validation-fixes#package-openclaw-unsupported-metadata)          |
...
| `manifest-unknown-fields`               | [Remova campos de manifesto incompatíveis](/pt-BR/clawhub/plugin-validation-fixes#manifest-unknown-fields)                                  |
| `manifest-unknown-contracts`            | [Remova chaves de contrato incompatíveis](/pt-BR/clawhub/plugin-validation-fixes#manifest-unknown-contracts)                                 |
...
| `unrecognized-security-manifest`        | [Remova arquivos de manifesto de segurança incompatíveis](/pt-BR/clawhub/plugin-validation-fixes#unrecognized-security-manifest)                   |
...
O pacote não declara o intervalo da API de Plugin do OpenClaw que suporta.

- Adicione `openclaw.compat.pluginApi` a `package.json`.
- Use a versão da API de Plugin do OpenClaw ou o piso semver com que você criou e testou
  o pacote.
...
com os quais o pacote foi criado.
...
O artefato empacotado não tem metadados do OpenClaw que existem no pacote
de origem.
...
## Migração de compatibilidade e SDK
...
- Use [Convenções de importação](/pt-BR/plugins/building-plugins#import-conventions) e
  [Subcaminhos do Plugin SDK](/pt-BR/plugins/sdk-subpaths) para encontrar a importação estreita.
- Execute novamente `clawhub package validate <path-to-plugin>`.
...
- Use `appendSessionTranscriptMessageByIdentity(...)` para anexações de transcrição.
...
- Espelhe metadados de variáveis de ambiente de provedor em `setup.providers[].envVars`.
...
- Espelhe o setup de canal orientado por ambiente no setup atual, na configuração do canal ou nos
...
- Mantenha comportamentos sensíveis à segurança documentados na documentação pública do seu pacote ou no
  README até que o contrato de manifesto exista.
- Execute novamente `clawhub package validate <path-to-plugin>`.
