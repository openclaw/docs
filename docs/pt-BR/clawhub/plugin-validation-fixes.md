...
x-i18n:
    generated_at: "2026-07-01T15:21:29Z"
    model: gpt-5.5
...
para mantenedores do OpenClaw, não para autores de Plugin.
...
- Use a versão da API de Plugin do OpenClaw ou a versão mínima semver contra a qual você
  criou e testou.
...
- Espelhe a configuração de canal conduzida por variáveis de ambiente no setup, na configuração de canal ou
  nos metadados de canal do pacote atuais usados pelo formato do seu plugin.
- Mantenha `channelEnvVars` somente como metadados de compatibilidade enquanto versões mais antigas
  compatíveis do OpenClaw ainda os exigirem.
