---
read_when:
    - Você executou clawhub package validate e precisa corrigir os achados de Plugin
...
summary: Corrigir os apontamentos de validação do pacote do Plugin do ClawHub antes da publicação
title: Correções de validação de Plugin
x-i18n:
    generated_at: "2026-07-01T18:09:15Z"
    model: gpt-5.5
...
varreduras automatizadas de pacotes. Esta página cobre achados voltados a autores, ou seja,
...
são destinados a mantenedores do OpenClaw, não a autores de Plugin.
...
| `package-openclaw-entry-missing`        | [Declarar pontos de entrada de pacote OpenClaw](/pt-BR/clawhub/plugin-validation-fixes#package-openclaw-entry-missing)            |
...
| `sdk-load-session-store`                | [Substituir acesso ao armazenamento de sessão completo](/pt-BR/clawhub/plugin-validation-fixes#sdk-load-session-store)            |
| `sdk-session-store-write`               | [Substituir gravações no armazenamento de sessão completo](/pt-BR/clawhub/plugin-validation-fixes#sdk-session-store-write)        |
...
- Veja [campos de package.json que afetam a descoberta](/pt-BR/plugins/manifest#packagejson-fields-that-affect-discovery).
- Execute novamente `clawhub package validate <path-to-plugin>`.
...
- Veja [Pontos de entrada de Plugin](/pt-BR/plugins/sdk-entrypoints) e
...
- Veja [Pontos de entrada de Plugin](/pt-BR/plugins/sdk-entrypoints).
- Execute novamente `clawhub package validate <path-to-plugin>`.
...
- Defina `openclaw.install.defaultChoice` quando mais de uma origem de instalação
  estiver disponível.
...
- Use a versão da API de plugin do OpenClaw ou o piso semver contra o qual você criou e testou
  o pacote.
- Mantenha isso separado da versão do pacote. A versão do pacote descreve o
  lançamento do plugin; `openclaw.compat.pluginApi` descreve o contrato da API do host.
- Veja [campos de package.json que afetam a descoberta](/pt-BR/plugins/manifest#packagejson-fields-that-affect-discovery).
- Execute novamente `clawhub package validate <path-to-plugin>`.

### package-min-host-version-drift

A versão mínima do host do pacote não corresponde aos metadados da versão do OpenClaw
contra a qual o pacote foi criado.
...
- Se `openclaw.plugin.json` também tiver `version`, atualize-o para corresponder ou remova
...
- Veja [Manifesto de Plugin](/pt-BR/plugins/manifest).
- Execute novamente `clawhub package validate <path-to-plugin>`.

### package-openclaw-unsupported-metadata

O bloco `package.json#openclaw` contém campos que não são metadados de pacote do
...
- Mantenha pontos de entrada do pacote, compatibilidade, instalação, configuração e metadados
  de catálogo em campos compatíveis de `package.json#openclaw`.
- Veja [campos de package.json que afetam a descoberta](/pt-BR/plugins/manifest#packagejson-fields-that-affect-discovery).
- Execute novamente `clawhub package validate <path-to-plugin>`.
...
O artefato empacotado não inclui metadados do OpenClaw que existem no seu pacote
de origem.
...
- Garanta que `openclaw.plugin.json` seja incluído quando o pacote for um plugin nativo do
  OpenClaw.
- Atualize `files` ou `.npmignore` para que os metadados do pacote não sejam excluídos.
- Veja [Criar plugins](/pt-BR/plugins/building-plugins).
- Execute novamente `clawhub package validate <path-to-plugin>`.
...
- Mova o comportamento de runtime para o código de registro do plugin e mantenha `contracts`
  limitado a metadados estáticos de posse de capacidades.
- Execute novamente `clawhub package validate <path-to-plugin>`.
...
- Use [Convenções de importação](/pt-BR/plugins/building-plugins#import-conventions) e
  [subcaminhos do SDK do plugin](/pt-BR/plugins/sdk-subpaths) para encontrar a importação restrita.
- Execute novamente `clawhub package validate <path-to-plugin>`.
...
- Substitua importações internas reservadas do SDK do OpenClaw por subcaminhos públicos
...
- Use [Subcaminhos do SDK do plugin](/pt-BR/plugins/sdk-subpaths) e
...
- Mantenha `loadSessionStore(...)` apenas enquanto seu intervalo de compatibilidade declarado
...
- Consulte [API de runtime](/pt-BR/plugins/sdk-runtime#agent-session-state) e
  [Subcaminhos do SDK do plugin](/pt-BR/plugins/sdk-subpaths).
- Execute novamente `clawhub package validate <path-to-plugin>`.

### sdk-session-store-write

O plugin ainda usa um helper obsoleto de escrita de armazenamento de sessão inteiro, como
...
- Evite carregar, modificar e salvar o objeto inteiro do armazenamento de sessão.
- Mantenha helpers de escrita de armazenamento inteiro apenas enquanto seu intervalo de compatibilidade declarado
...
- Use a identidade da transcrição ou helpers de alvo quando o código estiver preparando uma
...
O plugin ainda usa o helper obsoleto de alvo de arquivo de transcrição
`resolveSessionTranscriptLegacyFileTarget`.

- Use `resolveSessionTranscriptIdentity(...)` quando o código precisar apenas da identidade pública
  da sessão.
- Use `resolveSessionTranscriptTarget(...)` quando o código precisar de um alvo estruturado
  de operação de transcrição.
- Evite ler ou construir alvos de arquivo de transcrição legados diretamente.
- Mantenha o helper legado apenas enquanto seu intervalo de compatibilidade declarado ainda
...
O plugin ainda usa helpers obsoletos de baixo nível para transcrição, como
...
- Use `publishSessionTranscriptUpdateByIdentity(...)` para notificações de atualização da transcrição.
...
- Mantenha helpers de transcrição de baixo nível apenas enquanto seu intervalo de compatibilidade declarado
  ainda oferecer suporte a versões mais antigas do OpenClaw que os exigem.
- Consulte [API de runtime](/pt-BR/plugins/sdk-runtime#agent-session-state) e
  [Subcaminhos do SDK do plugin](/pt-BR/plugins/sdk-subpaths).
- Execute novamente `clawhub package validate <path-to-plugin>`.
...
- Mantenha `before_agent_start` apenas enquanto seu intervalo de compatibilidade declarado ainda
  oferecer suporte a versões mais antigas do OpenClaw que o exigem.
- Consulte [Hooks](/pt-BR/plugins/hooks) e
  [Compatibilidade do plugin](/pt-BR/plugins/compatibility).
- Execute novamente `clawhub package validate <path-to-plugin>`.
...
- Mantenha `providerAuthEnvVars` apenas como metadados de compatibilidade enquanto seu intervalo
  de OpenClaw compatível ainda precisar dele.
...
- Espelhe o setup de canal orientado por ambiente nos metadados atuais de setup, configuração de canal ou
...
O pacote envia `openclaw.security.json` com uma referência de esquema que o ClawHub
...
O pacote envia um arquivo de manifesto de segurança sem suporte.
