...
read_when:
    ...
    - Você está atualizando os metadados do pacote do Plugin antes da release
...
varreduras automatizadas de pacotes. Esta página cobre constatações voltadas a autores, ou seja,
...
destinados aos mantenedores do OpenClaw, não aos autores de Plugins.
...
| `package-openclaw-unsupported-metadata` | [Remover metadados de pacote OpenClaw incompatíveis](/pt-BR/clawhub/plugin-validation-fixes#package-openclaw-unsupported-metadata)          |
...
| `manifest-unknown-fields`               | [Remover campos de manifesto incompatíveis](/pt-BR/clawhub/plugin-validation-fixes#manifest-unknown-fields)                                  |
| `manifest-unknown-contracts`            | [Remover chaves de contrato incompatíveis](/pt-BR/clawhub/plugin-validation-fixes#manifest-unknown-contracts)                                 |
...
| `unrecognized-security-manifest`        | [Remover arquivos de manifesto de segurança incompatíveis](/pt-BR/clawhub/plugin-validation-fixes#unrecognized-security-manifest)                   |
...
Os metadados do pacote existem, mas não declaram um ponto de entrada de tempo de execução
do OpenClaw.
...
- Use a versão da API de Plugin do OpenClaw ou a versão mínima semver com a qual você
  criou e testou.
...
- Verifique qualquer metadado de build do OpenClaw no pacote, como a versão do OpenClaw
...
- Remova campos incompatíveis, como `openclaw.bundle`.
- Mantenha metadados de Plugin nativo em `openclaw.plugin.json`.
- Mantenha metadados de pontos de entrada, compatibilidade, instalação, configuração e catálogo
  do pacote em campos compatíveis de `package.json#openclaw`.
- Consulte [campos de package.json que afetam a descoberta](/pt-BR/plugins/manifest#packagejson-fields-that-affect-discovery).
- Execute novamente `clawhub package validate <path-to-plugin>`.
...
- Corrija metadados inválidos do pacote, scripts de ciclo de vida quebrados ou entradas de arquivos que
...
## Migração do SDK e compatibilidade
...
O plugin ainda usa o helper obsoleto de repositório de sessão completo
`loadSessionStore`.
...
- Evite carregar, modificar e salvar o objeto completo do repositório de sessão.
...
O plugin ainda usa um helper obsoleto de escrita do repositório de sessão completo, como
...
- Evite carregar, modificar e salvar o objeto completo do repositório de sessão.
- Mantenha helpers de escrita do repositório completo apenas enquanto seu intervalo de compatibilidade declarado
  ainda oferecer suporte a versões mais antigas do OpenClaw que exigem isso.
- Consulte [API de runtime](/pt-BR/plugins/sdk-runtime#agent-session-state) e
  [subcaminhos do SDK de Plugin](/pt-BR/plugins/sdk-subpaths).
- Execute novamente `clawhub package validate <path-to-plugin>`.
...
- Use `getSessionEntry(...)` para ler metadados de sessão por agente e identidade
  de sessão.
- Use `patchSessionEntry(...)` ou `upsertSessionEntry(...)` para persistir metadados
  de sessão.
...
- Use `resolveSessionTranscriptIdentity(...)` quando o código precisar apenas da identidade
  pública da sessão.
...
- Mantenha `providerAuthEnvVars` apenas como metadados de compatibilidade enquanto o intervalo
  do OpenClaw compatível ainda precisar disso.
...
- Mantenha os metadados de variáveis de ambiente de canal declarativos para que o OpenClaw possa inspecionar o status
  de setup sem carregar o runtime do canal.
- Espelhe o setup de canal orientado por variáveis de ambiente no setup atual, na configuração de canal ou nos
...
O pacote envia um arquivo de manifesto de segurança não compatível.
...
- Mantenha o comportamento sensível à segurança documentado nos documentos públicos do seu pacote ou
