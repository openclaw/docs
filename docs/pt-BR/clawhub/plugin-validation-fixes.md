...
x-i18n:
    generated_at: "2026-07-04T17:51:23Z"
    model: gpt-5.5
...
- Mantenha isso separado da versão do pacote. A versão do pacote descreve o lançamento do
  Plugin; `openclaw.compat.pluginApi` descreve o contrato da API do host.
- Consulte [campos de package.json que afetam a descoberta](/pt-BR/plugins/manifest#packagejson-fields-that-affect-discovery).
- Execute novamente `clawhub package validate <path-to-plugin>`.
...
- Corrija metadados de pacote inválidos, scripts de ciclo de vida quebrados ou entradas de arquivos
  que fazem o empacotamento falhar.
- Remova `private: true` se este pacote for destinado à publicação pública.
- Execute novamente `clawhub package validate <path-to-plugin>`.

### package-npm-pack-entrypoint-missing

O pacote pode ser empacotado, mas o artefato empacotado não inclui os arquivos de ponto de
entrada declarados em `package.json#openclaw`.
...
O artefato empacotado não contém metadados do OpenClaw que existem no seu pacote-fonte.
...
- Mova metadados de pacote ou instalação para campos `package.json#openclaw`
  compatíveis em vez de colocá-los no manifesto.
- Execute novamente `clawhub package validate <path-to-plugin>`.
...
- Use [Convenções de importação](/pt-BR/plugins/building-plugins#import-conventions) e
  [subcaminhos do SDK de Plugin](/pt-BR/plugins/sdk-subpaths) para encontrar a importação mais restrita.
- Execute novamente `clawhub package validate <path-to-plugin>`.
...
O Plugin ainda usa um auxiliar obsoleto de escrita no armazenamento de sessão completo, como
...
- Evite carregar, modificar e salvar o objeto inteiro do armazenamento de sessão.
- Mantenha auxiliares de escrita no armazenamento completo somente enquanto o intervalo de compatibilidade declarado
...
- Não persista nem dependa de caminhos legados de arquivo de transcrição.
- Consulte [API de Runtime](/pt-BR/plugins/sdk-runtime#agent-session-state) e
  [subcaminhos do SDK de Plugin](/pt-BR/plugins/sdk-subpaths).
- Execute novamente `clawhub package validate <path-to-plugin>`.
...
- Evite ler ou construir destinos legados de arquivo de transcrição diretamente.
...
- Mantenha auxiliares de transcrição de baixo nível somente enquanto o intervalo de compatibilidade declarado
  ainda oferecer suporte a versões mais antigas do OpenClaw que os exigem.
- Consulte [API de Runtime](/pt-BR/plugins/sdk-runtime#agent-session-state) e
  [subcaminhos do SDK de Plugin](/pt-BR/plugins/sdk-subpaths).
- Execute novamente `clawhub package validate <path-to-plugin>`.
...
- Espelhe o setup de canal orientado por variáveis de ambiente para o setup atual, a configuração de canal ou
  os metadados de canal do pacote usados pelo formato do seu Plugin.
...
- Remova a URL do schema se ela for apenas informativa.
