---
read_when:
    - Você executou clawhub package validate e precisa corrigir findings do plugin
...
summary: Corrigir os achados de validação do pacote de Plugin do ClawHub antes da publicação
...
- Use a versão da API de Plugin do OpenClaw ou o piso de semver contra o qual você criou
  e testou.
...
- Remova `private: true` se este pacote for destinado a publicação pública.
- Execute novamente `clawhub package validate <path-to-plugin>`.
...
- Mova metadados de pacote ou instalação para campos compatíveis em `package.json#openclaw`
...
- Use `openclaw/plugin-sdk/channel-core` para auxiliares de entrada de canais.
...
- Substitua importações reservadas do SDK interno do OpenClaw por subcaminhos públicos
  documentados em `openclaw/plugin-sdk/*`.
...
- Evite carregar, modificar e salvar o objeto inteiro de armazenamento de sessão.
- Mantenha auxiliares de gravação do armazenamento inteiro somente enquanto seu intervalo de compatibilidade declarado
  ainda oferecer suporte a versões mais antigas do OpenClaw que exigem isso.
- Consulte [API de runtime](/pt-BR/plugins/sdk-runtime#agent-session-state) e
  [Subcaminhos do SDK de Plugin](/pt-BR/plugins/sdk-subpaths).
- Execute novamente `clawhub package validate <path-to-plugin>`.
...
- Use auxiliares de identidade de transcrição ou de destino quando o código estiver preparando uma
...
- Mantenha o auxiliar legado somente enquanto seu intervalo de compatibilidade declarado ainda
...
- Mantenha auxiliares de transcrição de baixo nível somente enquanto seu intervalo de compatibilidade declarado
  ainda oferecer suporte a versões mais antigas do OpenClaw que exigem isso.
- Consulte [API de runtime](/pt-BR/plugins/sdk-runtime#agent-session-state) e
  [Subcaminhos do SDK de Plugin](/pt-BR/plugins/sdk-subpaths).
- Execute novamente `clawhub package validate <path-to-plugin>`.
...
- Mantenha `providerAuthEnvVars` somente como metadados de compatibilidade enquanto o intervalo
  do OpenClaw com suporte ainda precisar disso.
...
- Mantenha `channelEnvVars` somente como metadados de compatibilidade enquanto versões mais antigas
  do OpenClaw com suporte ainda exigirem isso.
...
O pacote fornece `openclaw.security.json` com uma referência de esquema que o ClawHub
...
O pacote fornece um arquivo de manifesto de segurança sem suporte.
