...
x-i18n:
    generated_at: "2026-07-03T00:54:21Z"
    model: gpt-5.5
...
O ClawHub valida pacotes de Plugin antes da publicação e também pode mostrar constatações de varreduras automatizadas de pacotes. Esta página cobre constatações voltadas ao autor, ou seja, constatações que o autor do Plugin pode corrigir nos metadados do pacote, no manifesto, nas importações do SDK ou no artefato publicado.

Ela não cobre constatações internas de cobertura do Plugin Inspector. Se um relatório completo contiver códigos de manutenção do scanner sem orientação de correção para o autor, eles são destinados aos mantenedores do OpenClaw, não aos autores de Plugin.
...
| `package-openclaw-unsupported-metadata` | [Remova metadados de pacote do OpenClaw incompatíveis](/pt-BR/clawhub/plugin-validation-fixes#package-openclaw-unsupported-metadata) |
...
A raiz do pacote não inclui `package.json`, portanto o ClawHub não consegue identificar o pacote npm, a versão, os pontos de entrada ou os metadados do OpenClaw.

- Adicione `package.json` com `name`, `version` e `type`.
- Adicione um bloco `openclaw` quando o pacote distribuir um Plugin do OpenClaw.
- Use [Criando Plugins](/pt-BR/plugins/building-plugins) para ver um exemplo mínimo de pacote e [Manifesto de Plugin](/pt-BR/plugins/manifest#manifest-versus-packagejson) para a divisão entre pacote e manifesto.
...
O pacote declara um ponto de entrada do OpenClaw, mas o arquivo referenciado está ausente do pacote que está sendo validado.
...
- Alinhe a versão mínima do host com o intervalo de versões do host que o pacote realmente suporta.
- Consulte [campos de package.json que afetam a descoberta](/pt-BR/plugins/manifest#packagejson-fields-that-affect-discovery).
- Execute novamente `clawhub package validate <path-to-plugin>`.
...
- Mantenha metadados de pontos de entrada do pacote, compatibilidade, instalação, configuração e catálogo em campos compatíveis de `package.json#openclaw`.
- Consulte [campos de package.json que afetam a descoberta](/pt-BR/plugins/manifest#packagejson-fields-that-affect-discovery).
- Execute novamente `clawhub package validate <path-to-plugin>`.
...
O artefato empacotado está sem metadados do OpenClaw que existem no seu pacote de origem.
...
- Mantenha `channelEnvVars` somente como metadados de compatibilidade enquanto versões mais antigas compatíveis
