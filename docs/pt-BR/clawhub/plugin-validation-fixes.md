...
summary: Corrigir achados de validação de pacotes de Plugin do ClawHub antes da publicação
title: Correções de validação de Plugin
x-i18n:
    generated_at: "2026-07-03T17:15:50Z"
    model: gpt-5.5
...
varreduras automatizadas de pacotes. Esta página cobre achados voltados ao autor, ou seja,
achados que o autor do plugin pode corrigir nos metadados do pacote, manifesto, imports do SDK
...
são destinados aos mantenedores do OpenClaw, não aos autores de plugin.
...
| `package-openclaw-entry-missing`        | [Declarar pontos de entrada de pacote do OpenClaw](/pt-BR/clawhub/plugin-validation-fixes#package-openclaw-entry-missing)                         |
...
| `package-install-metadata-incomplete`   | [Concluir metadados de instalação](/pt-BR/clawhub/plugin-validation-fixes#package-install-metadata-incomplete)                               |
...
| `legacy-root-sdk-import`                | [Substituir imports raiz do SDK](/pt-BR/clawhub/plugin-validation-fixes#legacy-root-sdk-import)                                             |
| `reserved-sdk-import`                   | [Remover imports reservados do SDK](/pt-BR/clawhub/plugin-validation-fixes#reserved-sdk-import)                                             |
...
| `sdk-session-file-helper`               | [Substituir helpers de caminho de arquivo de sessão](/pt-BR/clawhub/plugin-validation-fixes#sdk-session-file-helper)                                   |
...
| `sdk-session-transcript-low-level`      | [Substituir helpers de transcrição de baixo nível](/pt-BR/clawhub/plugin-validation-fixes#sdk-session-transcript-low-level)                       |
...
| `security-manifest-schema-unavailable`  | [Remover referências indisponíveis de schema de manifesto de segurança](/pt-BR/clawhub/plugin-validation-fixes#security-manifest-schema-unavailable) |
...
- Consulte [campos do package.json que afetam a descoberta](/pt-BR/plugins/manifest#packagejson-fields-that-affect-discovery).
- Execute novamente `clawhub package validate <path-to-plugin>`.
...
- Consulte [Pontos de entrada de Plugin](/pt-BR/plugins/sdk-entrypoints) e
  [campos do package.json que afetam a descoberta](/pt-BR/plugins/manifest#packagejson-fields-that-affect-discovery).
- Execute novamente `clawhub package validate <path-to-plugin>`.
...
O pacote não declara o intervalo da API de plugin do OpenClaw ao qual dá suporte.
...
A versão mínima do host no pacote não corresponde aos metadados de versão do OpenClaw
...
O bloco `package.json#openclaw` contém campos que não têm suporte como metadados de pacote
do OpenClaw.

- Remova campos sem suporte, como `openclaw.bundle`.
- Mantenha metadados de plugin nativo em `openclaw.plugin.json`.
- Mantenha pontos de entrada de pacote, compatibilidade, instalação, configuração e metadados
...
O plugin ainda usa um auxiliar obsoleto de escrita do armazenamento de sessão inteiro, como
...
- Evite carregar, modificar e salvar o objeto inteiro de armazenamento de sessão.
- Mantenha auxiliares de escrita de armazenamento inteiro apenas enquanto seu intervalo de compatibilidade declarado
...
O plugin ainda usa auxiliares obsoletos de caminhos de arquivo de sessão, como
...
- Mantenha o auxiliar legado apenas enquanto seu intervalo de compatibilidade declarado ainda
...
O plugin ainda usa auxiliares obsoletos de baixo nível de transcrição, como
...
- Mantenha auxiliares de transcrição de baixo nível apenas enquanto seu intervalo de compatibilidade declarado
  ainda oferecer suporte a versões mais antigas do OpenClaw que os exigem.
- Consulte [API de Runtime](/pt-BR/plugins/sdk-runtime#agent-session-state) e
  [Subcaminhos do SDK de Plugin](/pt-BR/plugins/sdk-subpaths).
- Execute novamente `clawhub package validate <path-to-plugin>`.
...
O pacote distribui `openclaw.security.json` com uma referência de schema que o ClawHub
não reconhece como disponível.

- Remova a URL do schema se ela for apenas consultiva.
- Use um schema versionado documentado somente depois que o OpenClaw publicar um.
- Execute novamente `clawhub package validate <path-to-plugin>`.
...
- Remova `openclaw.security.json` até que o OpenClaw documente um schema de manifesto de segurança
