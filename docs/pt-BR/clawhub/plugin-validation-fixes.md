...
summary: Corrija os achados de validação do pacote do Plugin ClawHub antes da publicação
...
| `package-plugin-api-compat-missing`     | [Declare compatibilidade da API de Plugin](/pt-BR/clawhub/plugin-validation-fixes#package-plugin-api-compat-missing)                          |
...
- Preencha `openclaw.install` com a origem de instalação com suporte, como
...
- Mantenha metadados de pontos de entrada, compatibilidade, instalação, configuração e catálogo
  do pacote em campos com suporte de `package.json#openclaw`.
- Consulte [campos de package.json que afetam a descoberta](/pt-BR/plugins/manifest#packagejson-fields-that-affect-discovery).
- Execute novamente `clawhub package validate <path-to-plugin>`.
...
O artefato empacotado está sem metadados do OpenClaw que existem no seu pacote de
origem.
...
- Mova o comportamento de runtime para o código de registro do plugin e mantenha `contracts`
  limitado aos metadados estáticos de propriedade de capacidades.
- Execute novamente `clawhub package validate <path-to-plugin>`.
...
- Use [Convenções de importação](/pt-BR/plugins/building-plugins#import-conventions) e
  [subpaths do SDK do Plugin](/pt-BR/plugins/sdk-subpaths) para encontrar a importação restrita.
- Execute novamente `clawhub package validate <path-to-plugin>`.
...
- Use [subpaths do SDK do Plugin](/pt-BR/plugins/sdk-subpaths) e
...
- Use `getSessionEntry(...)` ou `listSessionEntries(...)` ao ler o estado de sessão.
- Use `patchSessionEntry(...)` ou `upsertSessionEntry(...)` ao gravar o estado de sessão.
- Evite carregar, modificar e salvar todo o objeto de armazenamento de sessão.
...
- Consulte [API de Runtime](/pt-BR/plugins/sdk-runtime#agent-session-state) e
  [subpaths do SDK do Plugin](/pt-BR/plugins/sdk-subpaths).
- Execute novamente `clawhub package validate <path-to-plugin>`.
...
- Use a identidade da transcrição ou auxiliares de destino quando o código estiver preparando uma
...
- Mantenha `providerAuthEnvVars` apenas como metadados de compatibilidade enquanto o intervalo
  de OpenClaw com suporte ainda precisar disso.
...
- Espelhe a configuração de canal orientada por env na configuração atual, na configuração de canal ou nos
  metadados de canal do pacote usados pelo formato do seu plugin.
- Mantenha `channelEnvVars` apenas como metadados de compatibilidade enquanto versões mais antigas do
  OpenClaw com suporte ainda o exigirem.
