...
varreduras automatizadas de pacote. Esta página cobre achados voltados ao autor, ou seja,
...
Ela não cobre achados internos de cobertura do Inspetor de Plugin. Se um relatório completo
...
| `provider-auth-env-vars`                | [Mova variáveis de ambiente de provedor para metadados de setup](/pt-BR/clawhub/plugin-validation-fixes#provider-auth-env-vars)                             |
...
| `security-manifest-schema-unavailable`  | [Remova referências indisponíveis de schema de manifesto de segurança](/pt-BR/clawhub/plugin-validation-fixes#security-manifest-schema-unavailable) |
...
do OpenClaw compatíveis.

- Remova campos sem suporte, como `openclaw.bundle`.
- Mantenha metadados de Plugin nativo em `openclaw.plugin.json`.
- Mantenha pontos de entrada, compatibilidade, instalação, setup e metadados de catálogo do pacote
...
- Mantenha `name` legível por humanos e mantenha `id` como o id estável para máquina.
- Consulte [Manifesto de Plugin](/pt-BR/plugins/manifest).
- Execute novamente `clawhub package validate <path-to-plugin>`.
...
- Mova o comportamento de runtime para o código de registro do Plugin e mantenha
  `contracts` limitado a metadados estáticos de propriedade de capacidade.
- Execute novamente `clawhub package validate <path-to-plugin>`.
...
- Substitua importações do barrel raiz por importações de subpaths públicos focados.
...
- Use [convenções de importação](/pt-BR/plugins/building-plugins#import-conventions) e
  [subpaths do Plugin SDK](/pt-BR/plugins/sdk-subpaths) para encontrar a importação estreita.
- Execute novamente `clawhub package validate <path-to-plugin>`.
...
O manifesto usa metadados legados ou mais antigos de variáveis de ambiente de canal sem os
metadados atuais de setup ou configuração que o ClawHub espera.
