...
summary: Corrigir achados de validação de pacote de Plugin do ClawHub antes da publicação
title: Correções de validação de Plugin
x-i18n:
    generated_at: "2026-07-02T00:46:49Z"
    model: gpt-5.5
...
varreduras automatizadas de pacote. Esta página aborda achados voltados ao autor, o que significa
...
Ela não aborda achados internos de cobertura do Plugin Inspector. Se um relatório completo
contiver códigos de manutenção do scanner sem orientação de correção para o autor, esses
...
| `security-manifest-schema-unavailable`  | [Remova referências indisponíveis a esquema de manifesto de segurança](/pt-BR/clawhub/plugin-validation-fixes#security-manifest-schema-unavailable) |
...
- Adicione um bloco `openclaw` quando o pacote incluir um Plugin do OpenClaw.
...
O pacote tem `package.json`, mas não declara metadados de pacote do
OpenClaw.
...
A versão do pacote e a versão do manifesto de Plugin divergem.
...
O Plugin importa um caminho do SDK reservado para Plugins integrados ou
...
O Plugin ainda usa o auxiliar obsoleto de repositório de sessão inteiro
`loadSessionStore`.
...
O Plugin ainda usa um auxiliar obsoleto de gravação de repositório de sessão inteiro, como
...
- Evite carregar, modificar e salvar o objeto inteiro do repositório de sessão.
- Mantenha auxiliares de gravação do repositório inteiro apenas enquanto seu intervalo de compatibilidade declarado
...
- Use `resolveSessionTranscriptIdentity(...)` quando o código precisar apenas de identidade pública
  da sessão.
...
- Mantenha auxiliares de baixo nível de transcrição apenas enquanto seu intervalo de compatibilidade declarado
  ainda oferecer suporte a versões antigas do OpenClaw que os exigem.
- Consulte [API de runtime](/pt-BR/plugins/sdk-runtime#agent-session-state) e
  [Subcaminhos do SDK de Plugin](/pt-BR/plugins/sdk-subpaths).
- Execute novamente `clawhub package validate <path-to-plugin>`.
...
- Mantenha metadados de variáveis de ambiente de canal declarativos para que o OpenClaw possa inspecionar o status
  de setup sem carregar o runtime do canal.
- Espelhe o setup de canal orientado por variáveis de ambiente no setup atual, na configuração de canal ou
