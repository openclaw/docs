...
read_when:
    ...
    - O ClawHub rejeitou ou exibiu um aviso ao publicar um pacote de plugin
...
x-i18n:
    generated_at: "2026-07-16T12:18:36Z"
    model: gpt-5.6
...
O ClawHub valida pacotes de plugins antes da publicação e também pode mostrar constatações de
verificações automatizadas de pacotes. Esta página aborda constatações voltadas para autores, ou seja,
...
## Constatações voltadas para autores
...
| `package-install-metadata-incomplete`   | [Completar os metadados de instalação](/pt-BR/clawhub/plugin-validation-fixes#package-install-metadata-incomplete)                               |
| `package-plugin-api-compat-missing`     | [Declarar compatibilidade com a API de plugins](/pt-BR/clawhub/plugin-validation-fixes#package-plugin-api-compat-missing)                          |
...
| `package-manifest-version-drift`        | [Alinhar as versões do pacote e do manifesto](/pt-BR/clawhub/plugin-validation-fixes#package-manifest-version-drift)                          |
...
| `package-npm-pack-entrypoint-missing`   | [Incluir pontos de entrada na saída do empacotamento npm](/pt-BR/clawhub/plugin-validation-fixes#package-npm-pack-entrypoint-missing)                  |
| `package-npm-pack-metadata-missing`     | [Incluir metadados na saída do empacotamento npm](/pt-BR/clawhub/plugin-validation-fixes#package-npm-pack-metadata-missing)                       |
...
| `legacy-root-sdk-import`                | [Substituir importações da raiz do SDK](/pt-BR/clawhub/plugin-validation-fixes#legacy-root-sdk-import)                                             |
| `reserved-sdk-import`                   | [Remover importações reservadas do SDK](/pt-BR/clawhub/plugin-validation-fixes#reserved-sdk-import)                                             |
| `sdk-load-session-store`                | [Substituir o acesso ao armazenamento completo de sessões](/pt-BR/clawhub/plugin-validation-fixes#sdk-load-session-store)                                   |
| `sdk-session-store-write`               | [Substituir gravações no armazenamento completo de sessões](/pt-BR/clawhub/plugin-validation-fixes#sdk-session-store-write)                                  |
| `sdk-session-file-helper`               | [Substituir auxiliares de caminho de arquivos de sessão](/pt-BR/clawhub/plugin-validation-fixes#sdk-session-file-helper)                                   |
| `sdk-session-transcript-file-target`    | [Substituir destinos legados de arquivos de transcrição](/pt-BR/clawhub/plugin-validation-fixes#sdk-session-transcript-file-target)                   |
| `sdk-session-transcript-low-level`      | [Substituir auxiliares de baixo nível para transcrições](/pt-BR/clawhub/plugin-validation-fixes#sdk-session-transcript-low-level)                       |
| `legacy-before-agent-start`             | [Substituir before_agent_start](/pt-BR/clawhub/plugin-validation-fixes#legacy-before-agent-start)                                        |
| `provider-auth-env-vars`                | [Mover variáveis de ambiente do provedor para os metadados de configuração](/pt-BR/clawhub/plugin-validation-fixes#provider-auth-env-vars)                             |
...
| `security-manifest-schema-unavailable`  | [Remover referências indisponíveis ao esquema do manifesto de segurança](/pt-BR/clawhub/plugin-validation-fixes#security-manifest-schema-unavailable) |
...
- Consulte [Criação de plugins](/pt-BR/plugins/building-plugins) para obter um exemplo mínimo de pacote
  e [Manifesto do plugin](/pt-BR/plugins/manifest#manifest-versus-packagejson)
  para entender a divisão entre pacote e manifesto.
- Execute novamente `clawhub package validate <path-to-plugin>`.

### package-openclaw-metadata-missing

O pacote contém `package.json`, mas não declara metadados de pacote do
OpenClaw.
...
- Adicione `openclaw.extensions` para pontos de entrada nativos de plugins.
...
- Use a versão ou o limite mínimo semver da API de plugins do OpenClaw com a qual o pacote foi compilado e testado.
...
A versão mínima do host no pacote não corresponde aos metadados da versão do OpenClaw
com a qual o pacote foi compilado.

- Verifique `openclaw.install.minHostVersion`.
- Verifique quaisquer metadados de compilação do OpenClaw no pacote, como a versão do OpenClaw
  usada durante o lançamento.
- Alinhe a versão mínima do host ao intervalo de versões do host que o pacote
  realmente aceita.
- Consulte [campos de package.json que afetam a descoberta](/pt-BR/plugins/manifest#packagejson-fields-that-affect-discovery).
- Execute novamente `clawhub package validate <path-to-plugin>`.
...
- Se `openclaw.plugin.json` também contiver `version`, atualize-o para corresponder ou remova
  metadados obsoletos de versão do manifesto quando os metadados do pacote forem a fonte autoritativa.
- Publique uma nova versão do pacote após alterar os metadados publicados.
...
- Mantenha os metadados nativos do plugin em `openclaw.plugin.json`.
- Mantenha os pontos de entrada, a compatibilidade, a instalação, a configuração e os metadados de catálogo do pacote
...
- Corrija metadados inválidos do pacote, scripts de ciclo de vida com falhas ou entradas de arquivos que
  causem falha no empacotamento.
...
O artefato empacotado não contém os metadados do OpenClaw presentes no pacote
de origem.
...
- Verifique se `openclaw.plugin.json` está incluído quando o pacote é um Plugin nativo do
  OpenClaw.
...
- Mantenha `name` legível para humanos e `id` como o id estável para máquinas.
- Consulte [Manifesto do Plugin](/pt-BR/plugins/manifest).
- Execute `clawhub package validate <path-to-plugin>` novamente.

### manifest-unknown-fields

O manifesto do Plugin contém campos de nível superior não compatíveis com o OpenClaw.
...
- Remova os campos personalizados de `openclaw.plugin.json`.
- Mova os metadados do pacote ou da instalação para campos `package.json#openclaw` compatíveis,
...
- Mova o comportamento de runtime para o código de registro do Plugin e mantenha `contracts`
  limitado aos metadados estáticos de propriedade de recursos.
- Execute `clawhub package validate <path-to-plugin>` novamente.

## Migração do SDK e de compatibilidade
...
- Use `openclaw/plugin-sdk/channel-core` para os auxiliares de entrada de canal.
...
- Mantenha `loadSessionStore(...)` somente enquanto o intervalo de compatibilidade declarado
  ainda oferecer suporte a versões antigas do OpenClaw que o exigem.
...
- Mantenha os auxiliares de gravação do armazenamento completo somente enquanto o intervalo de compatibilidade declarado
  ainda oferecer suporte a versões antigas do OpenClaw que os exigem.
- Consulte [API de runtime](/pt-BR/plugins/sdk-runtime#agent-session-state) e
  [Subcaminhos do SDK de Plugins](/pt-BR/plugins/sdk-subpaths).
- Execute `clawhub package validate <path-to-plugin>` novamente.
...
- Use `getSessionEntry(...)` para ler metadados de sessão por identidade de agente e de
  sessão.
- Use `patchSessionEntry(...)` ou `upsertSessionEntry(...)` para persistir metadados de
  sessão.
...
- Use `resolveSessionTranscriptIdentity(...)` quando o código precisar apenas da identidade pública da
  sessão.
- Use `resolveSessionTranscriptTarget(...)` quando o código precisar de um destino estruturado de
  operação de transcrição.
...
- Mantenha o auxiliar legado somente enquanto o intervalo de compatibilidade declarado ainda
  oferecer suporte a versões antigas do OpenClaw que o exigem.
- Consulte [API de runtime](/pt-BR/plugins/sdk-runtime#agent-session-state) e
  [Subcaminhos do SDK de Plugins](/pt-BR/plugins/sdk-subpaths).
- Execute `clawhub package validate <path-to-plugin>` novamente.
...
- Use `appendSessionTranscriptMessageByIdentity(...)` para anexar dados à transcrição.
- Use `publishSessionTranscriptUpdateByIdentity(...)` para notificações de atualização da
  transcrição.
- Prefira a interface estruturada de runtime de transcrição para que o OpenClaw possa aplicar os
...
- Mantenha `providerAuthEnvVars` apenas como metadados de compatibilidade enquanto o intervalo de versões do
  OpenClaw compatível ainda precisar dele.
...
O manifesto usa metadados legados ou antigos de variáveis de ambiente de canal sem os metadados atuais de
configuração inicial ou configuração que o ClawHub espera.

- Mantenha os metadados de variáveis de ambiente do canal declarativos para que o OpenClaw possa inspecionar o status da configuração inicial
  sem carregar o runtime do canal.
- Replique a configuração inicial do canal orientada por variáveis de ambiente nos metadados atuais de configuração inicial, configuração de canal ou
  canal do pacote usados pelo formato do seu Plugin.
- Mantenha `channelEnvVars` apenas como metadados de compatibilidade enquanto versões antigas compatíveis do
  OpenClaw ainda o exigirem.
