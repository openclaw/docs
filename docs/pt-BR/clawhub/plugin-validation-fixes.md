...
summary: Corrigir apontamentos de validação do pacote de Plugin do ClawHub antes da publicação
title: Correções de validação de Plugin
x-i18n:
    generated_at: "2026-06-30T13:51:19Z"
    model: gpt-5.5
...
ClawHub valida pacotes de Plugin antes da publicação e também pode mostrar descobertas de varreduras automatizadas de pacotes. Esta página aborda descobertas voltadas ao autor, ou seja, descobertas que o autor do Plugin pode corrigir nos metadados do pacote, no manifesto, nas importações do SDK ou no artefato publicado.

Ela não aborda descobertas internas de cobertura do Plugin Inspector. Se um relatório completo contiver códigos de manutenção do scanner sem orientação de remediação para o autor, eles são destinados aos mantenedores do OpenClaw, não aos autores de Plugins.
...
| `package-plugin-api-compat-missing`     | [Declarar compatibilidade com a API de Plugin](/pt-BR/clawhub/plugin-validation-fixes#package-plugin-api-compat-missing)                          |
...
A versão mínima do host do pacote não corresponde aos metadados de versão do OpenClaw contra os quais o pacote foi criado.
...
- Se `openclaw.plugin.json` também tiver `version`, atualize-a para corresponder ou remova metadados de versão de manifesto obsoletos quando os metadados do pacote forem autoritativos.
...
- Mantenha os metadados de Plugin nativo em `openclaw.plugin.json`.
- Mantenha pontos de entrada do pacote, compatibilidade, instalação, configuração e metadados de catálogo nos campos compatíveis de `package.json#openclaw`.
- Consulte [campos de package.json que afetam a descoberta](/pt-BR/plugins/manifest#packagejson-fields-that-affect-discovery).
- Execute novamente `clawhub package validate <path-to-plugin>`.
...
- Mova metadados de pacote ou instalação para campos `package.json#openclaw`
  compatíveis, em vez de deixá-los no manifesto.
- Execute novamente `clawhub package validate <path-to-plugin>`.
...
- Evite carregar, modificar e salvar o objeto completo de armazenamento de sessão.
...
- Evite carregar, modificar e salvar o objeto completo de armazenamento de sessão.
- Mantenha auxiliares de escrita no armazenamento completo apenas enquanto o intervalo de compatibilidade declarado
...
- Mantenha o comportamento sensível à segurança documentado nos documentos públicos do seu pacote ou no
