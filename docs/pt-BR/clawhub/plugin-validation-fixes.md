...
summary: Corrigir os achados de validação do pacote do Plugin ClawHub antes da publicação
title: Correções de validação de Plugin
x-i18n:
    generated_at: "2026-07-05T02:01:31Z"
    model: gpt-5.5
...
varreduras automatizadas de pacotes. Esta página cobre achados voltados ao autor, o que significa
achados que o autor do Plugin pode corrigir nos metadados do pacote, manifesto, importações do SDK
ou artefato publicado.

Ela não cobre achados internos de cobertura do Plugin Inspector. Se um relatório completo
contiver códigos de manutenção do scanner sem orientação de remediação para o autor, esses
...
| `package-openclaw-unsupported-metadata` | [Remover metadados de pacote do OpenClaw sem suporte](/pt-BR/clawhub/plugin-validation-fixes#package-openclaw-unsupported-metadata) |
...
| `security-manifest-schema-unavailable`  | [Remover referências indisponíveis ao esquema de manifesto de segurança](/pt-BR/clawhub/plugin-validation-fixes#security-manifest-schema-unavailable) |
...
pacote npm, versão, pontos de entrada ou metadados do OpenClaw.

- Adicione `package.json` com `name`, `version` e `type`.
- Adicione um bloco `openclaw` quando o pacote entrega um Plugin do OpenClaw.
...
- Atualize os metadados se o ponto de entrada mudou.
...
- Se `openclaw.plugin.json` também tiver `version`, atualize para corresponder ou remova
...
- Mantenha pontos de entrada, compatibilidade, instalação, configuração e metadados de catálogo
...
O artefato empacotado está sem metadados do OpenClaw que existem no pacote
de origem.
...
- Substitua importações do barrel raiz por importações de subcaminhos públicos específicos.
...
O plugin importa um caminho do SDK reservado para plugins incluídos no pacote ou compatibilidade
interna.

- Substitua importações reservadas do SDK interno do OpenClaw por subcaminhos públicos documentados de
  `openclaw/plugin-sdk/*`.
...
- Use [subcaminhos do SDK de Plugin](/pt-BR/plugins/sdk-subpaths) e
  [migração de SDK](/pt-BR/plugins/sdk-migration) para escolher uma importação aceita.
- Execute novamente `clawhub package validate <path-to-plugin>`.
...
- Evite carregar, alterar e salvar o objeto completo do armazenamento de sessão.
- Mantenha `loadSessionStore(...)` apenas enquanto o intervalo de compatibilidade declarado
  ainda aceitar versões mais antigas do OpenClaw que o exigem.
...
O plugin ainda usa um helper obsoleto de gravação do armazenamento de sessão completo, como
...
- Evite carregar, alterar e salvar o objeto completo do armazenamento de sessão.
- Mantenha helpers de gravação do armazenamento completo apenas enquanto o intervalo de compatibilidade declarado
  ainda aceitar versões mais antigas do OpenClaw que os exigem.
- Consulte [API de runtime](/pt-BR/plugins/sdk-runtime#agent-session-state) e
  [subcaminhos do SDK de Plugin](/pt-BR/plugins/sdk-subpaths).
- Execute novamente `clawhub package validate <path-to-plugin>`.
...
- Mantenha o helper legado apenas enquanto o intervalo de compatibilidade declarado ainda
  aceitar versões mais antigas do OpenClaw que o exigem.
- Consulte [API de runtime](/pt-BR/plugins/sdk-runtime#agent-session-state) e
  [subcaminhos do SDK de Plugin](/pt-BR/plugins/sdk-subpaths).
- Execute novamente `clawhub package validate <path-to-plugin>`.
...
- Mova o trabalho de alteração de prompt ou contexto para `before_prompt_build`.
...
- Mantenha `providerAuthEnvVars` apenas como metadados de compatibilidade enquanto o intervalo
  aceito do OpenClaw ainda precisar disso.
...
- Mantenha metadados de variáveis de ambiente de canal declarativos para que o OpenClaw possa inspecionar o status de setup
  sem carregar o runtime de canal.
- Espelhe a configuração de canal orientada por variáveis de ambiente no setup atual, na configuração de canal ou
  nos metadados de canal do pacote usados pelo formato do seu plugin.
- Mantenha `channelEnvVars` apenas como metadados de compatibilidade enquanto versões mais antigas aceitas do
...
- Use um esquema versionado documentado apenas depois que o OpenClaw publicar um.
- Execute novamente `clawhub package validate <path-to-plugin>`.

### unrecognized-security-manifest

O pacote distribui um arquivo de manifesto de segurança não aceito.
