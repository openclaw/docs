---
read_when:
    - Você executou clawhub package validate e precisa corrigir achados do plugin
...
summary: Corrigir os apontamentos de validação do pacote do Plugin ClawHub antes da publicação
title: Correções de validação de Plugin
x-i18n:
    generated_at: "2026-07-03T09:27:29Z"
    model: gpt-5.5
...
códigos são para mantenedores do OpenClaw, não para autores de plugins.
...
| `sdk-load-session-store`                | [Substitua o acesso ao armazenamento de sessão completo](/pt-BR/clawhub/plugin-validation-fixes#sdk-load-session-store)                                   |
| `sdk-session-store-write`               | [Substitua gravações no armazenamento de sessão completo](/pt-BR/clawhub/plugin-validation-fixes#sdk-session-store-write)                                  |
...
| `provider-auth-env-vars`                | [Mova variáveis de ambiente de provedor para os metadados de configuração](/pt-BR/clawhub/plugin-validation-fixes#provider-auth-env-vars)                             |
...
- Adicione um bloco `openclaw` quando o pacote distribuir um plugin OpenClaw.
...
O pacote não declara o intervalo da API de plugin OpenClaw compatível.

- Adicione `openclaw.compat.pluginApi` ao `package.json`.
- Use a versão da API de plugin OpenClaw ou o piso semver com que você criou e testou
  o pacote.
...
O plugin importa um caminho do SDK reservado para plugins incorporados ou
...
- Mantenha `loadSessionStore(...)` apenas enquanto seu intervalo declarado de compatibilidade
  ainda oferecer suporte a versões antigas do OpenClaw que exigem isso.
...
O plugin ainda usa um helper obsoleto de gravação do armazenamento de sessão inteiro, como
...
- Evite carregar, modificar e salvar o objeto inteiro de armazenamento de sessão.
- Mantenha helpers de gravação do armazenamento inteiro apenas enquanto seu intervalo declarado de compatibilidade
  ainda oferecer suporte a versões antigas do OpenClaw que exigem isso.
- Consulte [API de runtime](/pt-BR/plugins/sdk-runtime#agent-session-state) e
  [Subcaminhos do SDK de Plugin](/pt-BR/plugins/sdk-subpaths).
- Execute novamente `clawhub package validate <path-to-plugin>`.
...
- Mantenha o helper legado apenas enquanto seu intervalo declarado de compatibilidade ainda
  oferecer suporte a versões antigas do OpenClaw que exigem isso.
- Consulte [API de runtime](/pt-BR/plugins/sdk-runtime#agent-session-state) e
  [Subcaminhos do SDK de Plugin](/pt-BR/plugins/sdk-subpaths).
- Execute novamente `clawhub package validate <path-to-plugin>`.
...
- Mantenha helpers de transcrição de baixo nível apenas enquanto seu intervalo declarado de compatibilidade
  ainda oferecer suporte a versões antigas do OpenClaw que exigem isso.
- Consulte [API de runtime](/pt-BR/plugins/sdk-runtime#agent-session-state) e
  [Subcaminhos do SDK de Plugin](/pt-BR/plugins/sdk-subpaths).
- Execute novamente `clawhub package validate <path-to-plugin>`.
...
- Mantenha `before_agent_start` apenas enquanto seu intervalo declarado de compatibilidade ainda
  oferecer suporte a versões antigas do OpenClaw que exigem isso.
...
- Espelhe o setup de canal baseado em variáveis de ambiente nos metadados atuais de setup, configuração de canal ou
