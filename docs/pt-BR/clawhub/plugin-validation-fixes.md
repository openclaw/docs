...
summary: Corrija os achados de validação do pacote de Plugin do ClawHub antes de publicar
title: Correções de validação de Plugin
x-i18n:
    generated_at: "2026-07-02T08:00:28Z"
    model: gpt-5.5
...
| `sdk-load-session-store`                | [Substitua o acesso ao armazenamento de sessão inteira](/pt-BR/clawhub/plugin-validation-fixes#sdk-load-session-store)          |
| `sdk-session-store-write`               | [Substitua gravações no armazenamento de sessão inteira](/pt-BR/clawhub/plugin-validation-fixes#sdk-session-store-write)        |
...
- Adicione um bloco `openclaw` quando o pacote fornecer um Plugin OpenClaw.
...
- Defina `openclaw.install.defaultChoice` quando mais de uma fonte de instalação
  estiver disponível.
...
O pacote não declara o intervalo da API de Plugin OpenClaw com o qual é compatível.

- Adicione `openclaw.compat.pluginApi` ao `package.json`.
- Use a versão da API de Plugin OpenClaw ou o piso semver contra o qual você compilou e testou.
...
- Atualize `files`, `.npmignore` ou a saída de build para que os pontos de entrada declarados
  sejam incluídos.
- Consulte [Pontos de entrada de Plugin](/pt-BR/plugins/sdk-entrypoints).
- Execute novamente `clawhub package validate <path-to-plugin>`.
...
- Substitua importações reservadas do SDK interno do OpenClaw por subcaminhos
...
- Mantenha `loadSessionStore(...)` apenas enquanto sua faixa de compatibilidade declarada
...
- Evite carregar, modificar e salvar o objeto completo do armazenamento de sessão.
- Mantenha helpers de gravação do armazenamento completo apenas enquanto sua faixa de compatibilidade declarada
...
- Mantenha o helper legado apenas enquanto sua faixa de compatibilidade declarada ainda
...
- Prefira a superfície estruturada de runtime de transcrição para que o OpenClaw possa aplicar
  os limites de transação e o tratamento de identidade corretos.
- Mantenha helpers de transcrição de baixo nível apenas enquanto sua faixa de compatibilidade declarada
  ainda oferecer suporte a versões mais antigas do OpenClaw que os exigem.
- Consulte [API de runtime](/pt-BR/plugins/sdk-runtime#agent-session-state) e
  [Subcaminhos do SDK do Plugin](/pt-BR/plugins/sdk-subpaths).
- Execute novamente `clawhub package validate <path-to-plugin>`.
...
- Mantenha `before_agent_start` apenas enquanto sua faixa de compatibilidade declarada ainda
  oferecer suporte a versões mais antigas do OpenClaw que o exigem.
...
- Mantenha `providerAuthEnvVars` apenas como metadados de compatibilidade enquanto sua faixa
  de OpenClaw compatível ainda precisar deles.
...
- Espelhe setup de canal orientado por variáveis de ambiente nos metadados atuais de setup, configuração de canal ou
