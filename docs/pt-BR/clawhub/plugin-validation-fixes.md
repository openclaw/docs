...
x-i18n:
    generated_at: "2026-07-01T07:55:50Z"
    model: gpt-5.5
...
varreduras automatizadas de pacotes. Esta página aborda descobertas voltadas a autores, ou seja,
...
## Descobertas voltadas a autores
...
| `sdk-session-store-write`               | [Substitua gravações do armazenamento de sessão inteiro](/pt-BR/clawhub/plugin-validation-fixes#sdk-session-store-write)         |
...
- Alinhe a versão mínima do host ao intervalo de versões do host ao qual o pacote
...
- Se `openclaw.plugin.json` também tiver `version`, atualize-o para corresponder ou remova
  metadados de versão do manifesto obsoletos quando os metadados do pacote forem autoritativos.
...
- Consulte [Manifesto do Plugin](/pt-BR/plugins/manifest).
- Reexecute `clawhub package validate <path-to-plugin>`.
...
- Mova metadados de pacote ou instalação para campos aceitos de `package.json#openclaw`
  em vez do manifesto.
- Reexecute `clawhub package validate <path-to-plugin>`.
...
- Remova chaves de contrato não aceitas.
...
- Use [Subcaminhos do SDK do Plugin](/pt-BR/plugins/sdk-subpaths) e
  [Migração do SDK](/pt-BR/plugins/sdk-migration) para escolher uma importação aceita.
- Reexecute `clawhub package validate <path-to-plugin>`.
...
- Mantenha `providerAuthEnvVars` apenas como metadados de compatibilidade enquanto o intervalo
  aceito do OpenClaw ainda precisar dele.
...
- Mantenha `channelEnvVars` apenas como metadados de compatibilidade enquanto versões mais antigas aceitas
...
- Remova `openclaw.security.json` até que o OpenClaw documente um esquema versionado de manifesto
  de segurança e o comportamento do ClawHub.
