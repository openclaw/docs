...
summary: Corrija as constatações de validação do pacote do Plugin ClawHub antes de publicar
title: Correções de validação de Plugin
x-i18n:
    generated_at: "2026-07-01T20:15:12Z"
    model: gpt-5.5
...
são para mantenedores do OpenClaw, e não para autores de Plugin.
...
| `sdk-load-session-store`                | [Substitua o acesso ao armazenamento completo da sessão](/pt-BR/clawhub/plugin-validation-fixes#sdk-load-session-store)                                   |
| `sdk-session-store-write`               | [Substitua gravações no armazenamento completo da sessão](/pt-BR/clawhub/plugin-validation-fixes#sdk-session-store-write)                                  |
...
- Garanta que `openclaw.plugin.json` esteja incluído quando o pacote for um Plugin nativo
  do OpenClaw.
...
- Mova comportamento de runtime para o código de registro do plugin e mantenha `contracts`
...
- Use [subcaminhos do SDK do Plugin](/pt-BR/plugins/sdk-subpaths) e
  [migração do SDK](/pt-BR/plugins/sdk-migration) para escolher uma importação com suporte.
- Execute novamente `clawhub package validate <path-to-plugin>`.
...
- Evite carregar, modificar e salvar o objeto completo de armazenamento de sessão.
- Mantenha auxiliares de escrita de armazenamento completo apenas enquanto o intervalo de compatibilidade declarado
...
- Prefira a superfície estruturada de runtime de transcrição para que o OpenClaw possa aplicar os
  limites de transação corretos e o tratamento de identidade.
...
- Espelhe a configuração de canal baseada em variáveis de ambiente no setup, na configuração de canal ou nos
  metadados de canal do pacote atuais usados pelo formato do seu plugin.
