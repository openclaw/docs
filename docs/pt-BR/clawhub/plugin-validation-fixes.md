...
read_when:
    ...
    - ClawHub rejeitou ou avisou sobre uma publicação de pacote de Plugin
    - Você está atualizando os metadados do pacote do Plugin antes da versão
summary: Corrigir os achados de validação do pacote do Plugin ClawHub antes de publicar
...
ClawHub valida pacotes de Plugin antes da publicação e também pode mostrar constatações de
varreduras automatizadas de pacotes. Esta página cobre constatações voltadas ao autor, ou seja,
constatações que o autor do Plugin pode corrigir nos metadados do pacote, manifesto, importações
...
códigos são para mantenedores do OpenClaw, não para autores de Plugin.
...
| `package-plugin-api-compat-missing`     | [Declarar compatibilidade da API do Plugin](/pt-BR/clawhub/plugin-validation-fixes#package-plugin-api-compat-missing)                          |
...
contra os quais o pacote foi construído.
...
- Mantenha metadados de pontos de entrada do pacote, compatibilidade, instalação, setup e catálogo
...
- Evite carregar, modificar e salvar o objeto inteiro de armazenamento de sessão.
- Mantenha auxiliares de gravação de armazenamento inteiro apenas enquanto o intervalo de compatibilidade declarado
