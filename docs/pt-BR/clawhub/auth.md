...
x-i18n:
    generated_at: "2026-07-01T20:14:47Z"
    model: gpt-5.5
...
Se o login retornar você a um estado desconectado, sua conta pode não estar em boa
...
3. Após o login pelo GitHub, o ClawHub cria um token da API.
4. O navegador redireciona de volta para o callback local.
5. A CLI armazena o token no arquivo de configuração do ClawHub.

Se seu navegador não conseguir alcançar o callback local por causa de regras de firewall, VPN ou
...
Use esse fluxo para servidores, jobs de CI ou ambientes somente com terminal.
...
Você pode revogar tokens da API na interface web do ClawHub.
...
Contas excluídas, banidas ou desativadas não podem continuar usando tokens da API existentes.
