...
x-i18n:
    generated_at: "2026-05-12T00:56:33Z"
    model: gpt-5.5
    provider: openai
...
no ClawHub. Se o login retornar você a um estado desconectado, talvez sua conta
não esteja em situação regular.

## Login pela CLI

O fluxo de login padrão da CLI abre seu navegador:
...
1. A CLI inicia um servidor temporário de retorno de chamada em `127.0.0.1`.
...
Se seu navegador não conseguir acessar o retorno de chamada local por causa de
...
Use este fluxo para servidores, trabalhos de CI ou ambientes somente de terminal.
...
Tokens revogados, inválidos ou ausentes retornam `401 Unauthorized`. Entre
