...
x-i18n:
    generated_at: "2026-07-01T20:15:37Z"
    model: gpt-5.5
...
- o nível de risco
- quaisquer constatações listadas
...
Um `Pass` é tranquilizador, mas não substitui seu próprio julgamento. Isso é
mais importante para ferramentas que podem publicar conteúdo, editar dados, executar comandos, ler arquivos ou
...
Constatações explicam por que um resultado de auditoria foi exibido. Cada constatação geralmente inclui:
...
Constatações podem ser rotuladas como `Info`, `Low`, `Medium`, `High` ou `Critical`. Constatações de maior
...
solicitada e o conteúdo real se alinham ao que os usuários razoavelmente esperariam?
...
As páginas de artefato vinculam à auditoria completa em:
...
seção pública autônoma da auditoria nem um veredito de bloqueio de instalação.
