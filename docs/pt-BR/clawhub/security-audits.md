...
read_when:
    ...
    - Decidindo se deve instalar um skill ou plugin
    - Explicando o status da auditoria, o nível de risco ou as descobertas do ClawHub
...
As auditorias de segurança do ClawHub ajudam você a decidir se uma skill ou um Plugin é segura o suficiente
...
- proprietário, fonte, versão, changelog, downloads, estrelas e outros sinais de confiança
...
agir com autoridade significativa sobre uma conta.
...
solicitada e o conteúdo real estão alinhados ao que os usuários esperariam razoavelmente?
...
```text
62/62 fornecedores sinalizaram esta skill como limpa.
```

ou:

```text
2/64 fornecedores sinalizaram esta skill como maliciosa, 1/64 a sinalizou como suspeita, e 61/64 a sinalizaram como limpa.
```

Quando o ClawHub não tem telemetria de contagem de fornecedores para resumir, a auditoria diz:

```text
Nenhum achado do VirusTotal
```

O VirusTotal continua sendo telemetria. Ele não substitui a própria análise de risco
consciente de artefatos do ClawHub.

## Análise de risco

A análise de risco é alimentada internamente pelo ClawScan, o próprio sistema de auditoria de segurança
...
O ClawScan não trata uma capacidade que parece assustadora como automaticamente maliciosa.
