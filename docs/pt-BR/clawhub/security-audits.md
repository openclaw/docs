---
read_when:
    - Compreendendo os resultados da auditoria de segurança do ClawHub
...
x-i18n:
    generated_at: "2026-07-03T13:22:13Z"
    model: gpt-5.5
...
para instalar. Elas mostram o que uma versão faz, quais permissões ela solicita e
se algo merece atenção extra antes que ela possa acessar arquivos, contas,
credenciais, código ou serviços externos.

Auditorias são fortes sinais de segurança, mas não garantem que uma versão esteja
...
não significa que ela é maliciosa. Significa que a skill parece alinhada ao propósito, mas pode
agir com autoridade significativa sobre a conta.

## Achados

Achados explicam por que um resultado de auditoria foi exibido. Cada achado geralmente inclui:
...
Achados podem ser rotulados como `Info`, `Low`, `Medium`, `High` ou `Critical`. Achados de maior
severidade contribuem mais fortemente para o nível de risco e o status da auditoria.
...
A principal questão é a coerência: o nome, o resumo, os metadados, as permissões
solicitadas e o conteúdo real estão alinhados com o que os usuários razoavelmente esperariam?
...
O ClawHub usa VirusTotal como telemetria de malware na pilha de auditoria. VirusTotal é um
padrão confiável da indústria para reputação de arquivos e varredura de malware, e nossa
parceria permite que o ClawHub adicione inteligência de segurança mais ampla à revisão de skills e Plugins.

VirusTotal é especialmente útil para artefatos maliciosos conhecidos, detecções de mecanismos e
sinais de reputação que complementam a revisão do ClawHub ciente de agentes. Quando contagens de mecanismos
de fornecedores estão disponíveis, a auditoria as resume em linguagem simples, como:
...
VirusTotal continua sendo telemetria. Ele não substitui a análise de risco própria do ClawHub
ciente de artefatos.
...
seção de auditoria pública independente nem um veredito que bloqueia a instalação.
