...
summary: Como entender os resultados da auditoria de segurança do ClawHub antes de instalar um skill ou plugin.
title: Auditorias de segurança
x-i18n:
    generated_at: "2026-07-01T07:55:22Z"
    model: gpt-5.5
...
As auditorias de segurança do ClawHub ajudam você a decidir se uma skill ou plugin é seguro o bastante
para instalar. Elas mostram o que um lançamento faz, que autoridade ele solicita e
se algo merece atenção extra antes de poder acessar arquivos, contas,
credenciais, código ou serviços externos.

Auditorias são sinais fortes de segurança, mas não garantem que um lançamento seja
...
O nível de risco descreve o raio de impacto: quanto poder o lançamento parece ter se
você o usar como pretendido.
...
Achados podem ser rotulados como `Info`, `Low`, `Medium`, `High` ou `Critical`. Achados de
...
Páginas de artefatos vinculam para a auditoria completa em:
...
sinais de reputação que complementam a revisão do ClawHub com consciência de agentes. Quando as
...
O VirusTotal continua sendo telemetria. Ele não substitui a análise de risco própria do ClawHub
com consciência de artefatos.

## Análise de risco

A análise de risco é alimentada internamente pelo ClawScan, o sistema próprio de auditoria de segurança
do ClawHub. Ele revisa cada lançamento como um artefato voltado a agentes: instruções,
...
pelo caso de uso declarado do lançamento.
