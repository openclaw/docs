...
summary: Como entender os resultados de auditoria de segurança do ClawHub antes de instalar um skill ou plugin.
title: Auditorias de segurança
x-i18n:
    generated_at: "2026-06-30T13:52:43Z"
    model: gpt-5.5
...
As auditorias de segurança do ClawHub ajudam você a decidir se uma skill ou Plugin é segura o suficiente para instalar. Elas mostram o que uma versão faz, qual autoridade ela solicita e se algo merece atenção extra antes de poder acessar arquivos, contas, credenciais, código ou serviços externos.
...
Veja também [Segurança](/clawhub/security), [Uso aceitável](/pt-BR/clawhub/acceptable-usage) e [Moderação e segurança da conta](/clawhub/moderation).
...
Por exemplo, uma skill de publicação pode mostrar `Review` com risco `Medium`. Isso não significa que ela é maliciosa. Significa que a skill parece alinhada ao propósito, mas pode agir com autoridade significativa sobre a conta.
...
Achados de baixa confiança são ocultados da consolidação pública da auditoria para que a página
permaneça focada em evidências úteis.

## O que o ClawHub verifica

O ClawHub audita artefatos de release enviados, incluindo:
...
A questão principal é a coerência: o nome, o resumo, os metadados, a autoridade
solicitada e o conteúdo real correspondem ao que os usuários razoavelmente esperariam?
...
sinais de reputação que complementam a revisão do ClawHub consciente de agentes. Quando as
...
própria do ClawHub, consciente dos artefatos.

## Análise de risco

A análise de risco é alimentada internamente pelo ClawScan, o sistema próprio de auditoria de segurança
do ClawHub. Ele revisa cada release como um artefato voltado para agentes: instruções,
metadados, permissões declaradas, arquivos, sinais de capacidade, sinais de varredura estática,
achados do SkillSpector, telemetria do VirusTotal e contexto fornecido pelo publisher.
...
Ele pergunta se a capacidade é divulgada, alinhada ao propósito e sustentada pelo
caso de uso declarado do release.
