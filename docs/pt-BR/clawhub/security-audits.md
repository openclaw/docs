...
read_when:
    ...
    - Decidindo se deve instalar uma habilidade ou Plugin
...
x-i18n:
    generated_at: "2026-07-04T15:10:41Z"
    model: gpt-5.5
...
para instalar. Elas mostram o que uma versão faz, quais autoridades ela solicita e
se algo merece atenção extra antes que possa acessar arquivos, contas,
credenciais, código ou serviços externos.

As auditorias são sinais fortes de segurança, mas não garantem que uma versão seja
...
O status da auditoria informa como você deve reagir ao resultado da auditoria:
...
Um `Pass` é tranquilizador, mas não substitui seu próprio julgamento. Isso é mais
...
maior severidade contribuem mais fortemente para o nível de risco e o status da auditoria.

Achados de baixa confiança são ocultados do resumo público da auditoria para que a página
...
- instruções de habilidades ou metadados de plugin
...
A pergunta principal é coerência: nome, resumo, metadados, autoridade solicitada
e conteúdo real estão alinhados com o que os usuários razoavelmente esperariam?
...
sinais de reputação que complementam a revisão consciente de agentes do ClawHub. Quando contagens de
...
O VirusTotal continua sendo telemetria. Ele não substitui a própria
análise de risco do ClawHub consciente de artefatos.

## Análise de risco

A análise de risco é alimentada internamente pelo ClawScan, o sistema próprio de auditoria de segurança
do ClawHub. Ela revisa cada versão como um artefato voltado a agentes: instruções,
...
Sinais de verificação estática são contexto interno para essa revisão; eles não são uma
