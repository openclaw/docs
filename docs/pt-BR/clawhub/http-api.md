...
x-i18n:
    generated_at: "2026-06-30T22:08:27Z"
    model: gpt-5.5
...
- Solicitações anônimas: impostas por IP.
- Solicitações autenticadas (token Bearer válido): impostas por bucket de usuário.
- Se o token estiver ausente/inválido, o comportamento volta para imposição por IP.
...
- `highlightedOnly` (opcional): `true` para filtrar para habilidades em destaque
- `nonSuspiciousOnly` (opcional): `true` para ocultar habilidades suspeitas (`flagged.suspicious`)
...
- A relevância é mais forte que a popularidade. Uma correspondência precisa de slug ou token de nome exibido pode superar uma correspondência mais frouxa com engajamento muito maior.
...
- A popularidade é escalada logaritmicamente e limitada. Habilidades com alto engajamento podem ficar abaixo quando o texto da consulta é uma correspondência mais fraca.
- Estado de moderação suspeito ou oculto pode remover uma habilidade da pesquisa pública dependendo dos filtros do chamador e do status atual de moderação.

Orientação de descoberta para publicadores:

- Coloque os termos que os usuários pesquisarão literalmente no nome exibido, resumo e tags. Use um token de slug independente somente quando ele também for uma identidade estável que você deseja manter.
...
- Aliases de renomeação preservam a resolução para URLs antigas e instalações que resolvem pelo registro, mas o ranqueamento de pesquisa é baseado nos metadados canônicos da habilidade depois que a renomeação foi indexada. Estatísticas existentes permanecem com a habilidade.
- Se uma habilidade estiver inesperadamente invisível, verifique primeiro o estado de moderação com `clawhub inspect @owner/slug` enquanto estiver conectado antes de alterar metadados relacionados ao ranqueamento.
...
- `createdAt` é estável para rastreamentos de novas habilidades; `updated` muda quando habilidades existentes são republicadas.
- Quando `nonSuspiciousOnly=true`, ordenações baseadas em cursor podem retornar menos que `limit` itens em uma página porque habilidades suspeitas são filtradas após a recuperação da página.
...
- Slugs antigos criados por fluxos de renomeação/mesclagem de proprietário resolvem para a habilidade canônica.
- `metadata.os`: restrições de SO declaradas no frontmatter da habilidade (por exemplo, `["macos"]`, `["linux"]`). `null` se não declarado.
- `metadata.systems`: alvos de sistema Nix (por exemplo, `["aarch64-darwin", "x86_64-linux"]`). `null` se não declarado.
- `metadata` é `null` se a habilidade não tiver metadados de plataforma.
- `moderation` é incluído somente quando a habilidade está sinalizada ou o proprietário a está visualizando.
...
- Proprietários e moderadores podem acessar detalhes de moderação para habilidades ocultas.
- Chamadores públicos só recebem `200` para habilidades visíveis já sinalizadas.
- Evidências são redigidas para chamadores públicos e incluem trechos brutos somente para proprietários/moderadores.

### `POST /api/v1/skills/{slug}/report`

Denuncia uma habilidade para análise de moderador. Denúncias são no nível da habilidade, opcionalmente vinculadas
a uma versão, e alimentam a fila de denúncias de habilidades.
...
Endpoint de moderador/admin para entrada de denúncias de habilidades.
...
Endpoint de moderador/admin para resolver ou reabrir denúncias de habilidades.
...
para ocultar a habilidade no mesmo fluxo de trabalho auditável.
...
Retorna detalhes de verificação de varredura de segurança para uma versão de habilidade.
...
- Trabalhos de scan são assíncronos. Solicitações manuais de scan são priorizadas antes do trabalho normal de publicação/preenchimento retroativo, mas a conclusão ainda depende da disponibilidade dos workers.

### `GET /api/v1/skills/-/scan/{scanId}`

Endpoint autenticado de polling para um scan enviado.

- Retorna status enfileirado/em execução/bem-sucedido/falhou.
- Retorna `queue.queuedAhead` e `queue.position` enquanto estiver enfileirado para que os clientes possam mostrar quantos scans manuais priorizados estão à frente da solicitação. Filas muito grandes são limitadas e relatadas com `queuedAheadIsEstimate: true`.
...
- `security` é o veredito de segurança/ClawScan de nível superior. A automação deve se basear em `ok`, `decision`, `reasons` e `security.status`.
- `security.signals` contém evidências de scanner de suporte, como `staticScan`, `virusTotal` e `skillSpector`.
- `security.signals.dependencyRegistry` é mantido para compatibilidade da resposta v1, mas o scanner de existência do registro de dependências foi aposentado e essa chave é sempre `null`.
...
- A resposta é somente de segurança. Ela não inclui dados do Skill Card, status do card gerado, listas de arquivos de artefato nem payloads detalhados de scanner.
...
- Requer um token de API para o proprietário do pacote, um proprietário/admin de publicador da organização,
...
- Lançamentos ClawPack incluem campos `npmIntegrity`, `npmShasum` e
...
- Endpoint de leitura pública. Nenhum token de proprietário, publicador, moderador ou admin é
  obrigatório.
...
- Este endpoint é intencionalmente mais restrito que endpoints de moderação de proprietário/moderador.
  ...
  identidades de denunciantes, corpos de denúncias, evidências privadas ou cronogramas
  internos de revisão.
...
- Usa o bucket de taxa de downloads.
...
- resumo do artefato
- proveniência do repositório-fonte e commit
...
bloqueiam downloads por si só; moderadores devem usar a moderação de lançamentos para
...
Endpoint de moderador/administrador para entrada de relatórios de pacotes.
...
- `revoked`: bloqueado depois que uma release foi anteriormente confiável.
...
- Arquivos de Plugin/pacote são arquivos zip com uma raiz `package/` para que clientes
...
- `dist.tarball`, `dist.integrity` e `dist.shasum` usam campos compatíveis com npm
  para que usuários possam apontar o npm para o espelho, se quiserem.
...
- Transferências de skills baseadas no GitHub não fazem proxy nem espelham bytes. A resposta JSON
  ...
  e `archiveUrl`; o estado de varredura/atual é um gate e não é incluído como metadados
...
- Os campos de formulário permitidos são `payload`, blobs repetidos em `files` ou uma referência de tarball
  ...
  fluxo upload-url. Publicações com storage-id em estágio também devem incluir o
...
- Solicitações de publicação multipart diretas são limitadas a 18 MB. Tarballs ClawPack podem
...
- Pacotes de Plugin exigem `openclaw.plugin.json`. Envios `.tgz` de ClawPack devem
  contê-lo em `package/openclaw.plugin.json`.
- Code plugins exigem `package.json`, metadados do repositório-fonte, metadados do commit-fonte,
...
- Somente o publicador da organização `openclaw` e publicadores pessoais dos membros atuais
...
Somente administradores. Garante que exista um publicador de organização para um identificador. Se o identificador ainda apontar para um
...
Somente administradores. Recupera um publicador pessoal para um principal substituto verificado do GitHub OAuth
...
aliases de slugs de Skills, pacotes, avisos do inspetor de pacotes e linhas derivadas de resumo de busca, para que
os caminhos de proprietário direto concordem com a nova autoridade do publicador. Uma reserva ativa de handle protegido
para o handle recuperado também é reatribuída ao usuário substituto, para que sincronizações futuras de perfil não possam
restaurar a autoridade concorrente do usuário anterior. Cada tabela primária é limitada a
100 linhas por transação de aplicação; recuperações maiores devem primeiro usar uma migração de proprietário retomável.
Fontes de Skills do GitHub são escopadas ao publicador e relatadas como verificadas, em vez de reescritas.
...
Bane um usuário e exclui permanentemente as Skills que ele possui (somente moderador/admin).
