...
- Endpoints de escrita autenticados não devem retornar um `Unauthorized` isolado quando
...
- Se nenhum IP de cliente confiável estiver disponível, requisições anônimas usam buckets de fallback
  ...
  caminhos, slugs, nomes de pacote, versões, strings de consulta ou outros parâmetros
...
- Os resultados são retornados em ordem de relevância (similaridade de embedding + reforços de tokens exatos de slug/nome + uma pequena prioridade de popularidade).
...
- Texto ASCII é tokenizado em limites de palavra e pontuação. Por exemplo, `personal-map` contém um token independente `map`, enquanto `amap-jsapi-skill` contém `amap`, `jsapi` e `skill`; portanto, pesquisar por `map` dá a `personal-map` uma correspondência lexical mais forte que `amap-jsapi-skill`.
- A popularidade é escalada em log e limitada. Skills com alto engajamento podem ficar em posição inferior quando o texto da consulta é uma correspondência mais fraca.
...
- Coloque os termos que os usuários literalmente pesquisarão no nome de exibição, resumo e tags. Use um token de slug independente somente quando ele também for uma identidade estável que você queira manter.
- Não renomeie um slug apenas para perseguir uma consulta, a menos que o novo slug seja um nome canônico melhor no longo prazo. Slugs antigos se tornam aliases de redirecionamento, mas a URL canônica, o slug exibido e resumos de pesquisa futuros usam o novo slug.
...
Reporte uma Skill para revisão de moderador. Relatórios são no nível da Skill, opcionalmente vinculados
...
Varreduras de upload local não são mais compatíveis. Solicitações que usam
...
- Trabalhos de varredura são assíncronos. Solicitações manuais de varredura são priorizadas antes do trabalho normal de publicação/backfill, mas a conclusão ainda depende da disponibilidade dos workers.
...
- Retorna o mesmo formato de ZIP dos downloads de solicitações de varredura.

### `POST /api/v1/skills/-/scan/batch`

Rota canônica de nova varredura em lote exclusiva para administradores. Ela aceita o mesmo formato de payload que o legado `POST /api/v1/skills/-/rescan-batch`.

### `POST /api/v1/skills/-/scan/batch/status`

Rota canônica de status em lote exclusiva para administradores. Ela aceita `{ "jobIds": ["..."] }` e retorna os mesmos contadores agregados que o legado `POST /api/v1/skills/-/rescan-batch/status`.
...
- `security.signals.dependencyRegistry` é mantido para compatibilidade da resposta v1, mas o scanner de existência do registro de dependências foi desativado e essa chave é sempre `null`.
...
No endpoint unificado `/api/v1/packages`, ela é somente de plugins; use
...
Pesquisa somente de plugins entre pacotes code-plugin e bundle-plugin.
...
- Requer um token de API do proprietário do pacote, de um proprietário/admin do editor da organização,
...
- Este endpoint é intencionalmente mais restrito que os endpoints de moderação de
  ...
  identidades de denunciantes, corpos de relatos, evidências privadas ou cronogramas internos
  de revisão.
...
- Esta é a superfície de resolvedor do OpenClaw; ela evita adivinhar o formato do arquivo
...
- `revoked`: bloqueado depois que uma versão já havia sido considerada confiável.
...
Transmite exatamente os bytes do tarball ClawPack enviado para clientes de espelho npm.
...
skill atual baseada no GitHub com varredura `clean` ou `suspicious` e sem versão
hospedada.
...
- Campo opcional de payload: `migrateOwner`. Quando `true` com `ownerHandle`, uma
  skill existente pode ser movida para esse proprietário se o ator for administrador/proprietário nos publicadores
...
- Os campos de formulário permitidos são `payload`, blobs `files` repetidos ou uma referência de tarball
  ...
  fluxo upload-url. Publicações com ID de armazenamento em staging também devem incluir o
...
Somente administrador. Garante que exista um publicador de org para um identificador. Se o identificador ainda apontar para um
...
principals do GitHub. A recuperação falha de forma fechada quando o publicador pessoal atual do usuário de destino
...
As fontes de Skill do GitHub têm escopo de publicador e são relatadas como verificadas, em vez de reescritas.
...
Bane um usuário e exclui definitivamente as Skills possuídas (somente moderador/admin).
