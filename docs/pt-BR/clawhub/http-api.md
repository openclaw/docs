...
Diretórios de terceiros podem usar os endpoints públicos de leitura para listar ou pesquisar Skills do ClawHub. Armazene os resultados em cache, respeite `429`/`Retry-After`, direcione os usuários de volta para a listagem canônica do ClawHub (`https://clawhub.ai/<owner>/skills/<slug>`) e evite insinuar endosso do ClawHub ao site de terceiros. Não tente espelhar conteúdo oculto, privado ou bloqueado por moderação fora da superfície da API pública.
...
- `X-RateLimit-Remaining` / `RateLimit-Remaining`: orçamento restante exato quando presente.
  Solicitações bem-sucedidas particionadas omitem este cabeçalho em vez de retornar um valor global aproximado.
...
- Os resultados são retornados em ordem de relevância (similaridade de embeddings + boosts de token exato de slug/nome + um pequeno prior de popularidade).
- A relevância é mais forte que a popularidade. Uma correspondência precisa de slug ou token de nome de exibição pode superar uma correspondência mais frouxa com engajamento muito mais forte.
...
- A popularidade é escalada por log e limitada. Skills com alto engajamento podem ter classificação mais baixa quando o texto da consulta é uma correspondência mais fraca.
...
- Aliases de renomeação preservam a resolução para URLs antigas e instalações que resolvem pelo registro, mas a classificação da pesquisa é baseada nos metadados canônicos da Skill depois que a renomeação foi indexada. Estatísticas existentes permanecem com a Skill.
...
- Quando `nonSuspiciousOnly=true`, ordenações baseadas em cursor podem retornar menos itens do que `limit` em uma página porque Skills suspeitas são filtradas após a recuperação da página.
...
Reporte uma Skill para análise por moderador. Relatórios são em nível de Skill, opcionalmente vinculados
...
Endpoint de moderador/admin para recebimento de relatórios de Skills.
...
- Retorna o mesmo formato de ZIP que os downloads de requisições de scan.
...
- A resposta é somente de segurança. Ela não inclui dados de Skill Card, status de card gerado, listas de arquivos de artefatos ou payloads detalhados de scanner.
- `security.signals` contém somente evidência de suporte em nível de status; use `/scan` ou a página de auditoria de segurança do ClawHub para detalhes completos do scanner.
- `security.signals.dependencyRegistry` é mantido para compatibilidade de resposta v1, mas o scanner de existência do registro de dependências foi aposentado e esta chave é sempre `null`.
...
- Use `/verify` quando precisar do envelope de verificação de Skill Card de uma única skill, `/card` quando precisar do Markdown de card gerado, e `/scan` quando precisar de dados detalhados de scanner.
...
- `category` (opcional): filtro de categoria de plugin. Suportado somente quando a
...
- As entradas de Skill continuam baseadas no registro de Skills e ainda só podem ser publicadas por meio de `POST /api/v1/skills`.
...
Pesquisa somente de Plugin entre pacotes code-plugin e bundle-plugin.
...
- Os aliases de filtro legados da v1 documentados em `GET /api/v1/plugins` também são
  aceitos.
- A filtragem por categoria é um filtro real de API respaldado por linhas de digest de categoria de Plugin,
  não uma reescrita de consulta de pesquisa.
- Os resultados são retornados por ordem de relevância e atualmente não têm paginação.
- Os controles de ordenação da UI do navegador para pesquisa de Plugin reordenam os resultados de relevância carregados,
...
Exclui reversivelmente um pacote e todos os lançamentos.

Observações:

- Exige um token de API para o proprietário do pacote, proprietário/admin do publicador da organização,
...
- `version.artifact.kind` é `legacy-zip` para arquivos de pacote do mundo antigo ou
  `npm-pack` para lançamentos respaldados por ClawPack.
...
- `trust.scanStatus` é o status de confiança efetivo derivado de entradas de scanners
  e moderação manual de lançamento.
...
- Pacotes privados retornam `404`, a menos que o chamador possa ler o publicador proprietário.
- Este endpoint é intencionalmente mais restrito que os endpoints de moderação
  de proprietário/moderador. Ele expõe a decisão de instalação e a explicação pública, não
  identidades de relatores, corpos de relatos, evidências privadas ou cronogramas internos de
  revisão.
...
- `all`: qualquer lançamento com uma substituição manual, estado de varredura não limpo ou relato de pacote.
...
Endpoint de moderador/admin para entrada de denúncias de pacotes.
...
`finalAction: "revoke"` com uma denúncia confirmada para aplicar moderação de versão no
...
- `revoked`: bloqueado depois que uma versão havia sido confiada anteriormente.
...
- Metadados apenas de registro não são injetados no arquivo baixado.
...
- Transferências de skills baseadas no GitHub não fazem proxy nem espelham bytes. A resposta JSON
  ...
  e `archiveUrl`; a varredura/estado atual é uma barreira e não é incluída como metadados
...
- Campo opcional de payload: `migrateOwner`. Quando `true` com `ownerHandle`, uma
  ...
  publicadores atuais quanto nos de destino. Sem essa adesão explícita, alterações de proprietário são
  rejeitadas.
...
- Pacotes de Plugin exigem `openclaw.plugin.json`. Envios `.tgz` ClawPack devem
...
- Publicações em nome de outro proprietário ainda validam a elegibilidade do canal oficial contra a conta do proprietário de destino.
...
versão. Nomes de pacote se tornam pacotes de espaço reservado privados sem linhas de versão, para que o mesmo
...
não consiga restaurar a autoridade concorrente do antigo usuário. Cada tabela primária é limitada a
100 linhas por transação de aplicação; recuperações maiores devem primeiro usar uma migração de proprietário retomável.
Fontes de Skills do GitHub têm escopo de publicador e são relatadas como verificadas, em vez de regravadas.
...
Bane um usuário e exclui permanentemente as Skills próprias (somente moderador/admin).
...
que prepararam um tarball ClawPack devem enviar o id de armazenamento resultante como
