...
x-i18n:
    generated_at: "2026-06-30T13:51:46Z"
    model: gpt-5.5
...
Os legados `/api/...` e `/api/cli/...` permanecem por compatibilidade (veja `DEPRECATIONS.md`).
...
- Se o token estiver ausente/inválido, o comportamento volta à aplicação por IP.
- Endpoints autenticados de escrita não devem retornar um `Unauthorized` puro quando
  ...
  CLI consigam informar aos usuários o que os bloqueou.
...
- Se nenhum IP confiável do cliente estiver disponível, requisições anônimas usam buckets de fallback
...
- Os resultados são retornados em ordem de relevância (similaridade de embeddings + reforços por tokens exatos de slug/nome + um pequeno fator prévio de popularidade).
...
- Texto ASCII é tokenizado em limites de palavras e pontuação. Por exemplo, `personal-map` contém um token `map` independente, enquanto `amap-jsapi-skill` contém `amap`, `jsapi` e `skill`; portanto, pesquisar por `map` dá a `personal-map` uma correspondência lexical mais forte do que a `amap-jsapi-skill`.
...
- Coloque os termos que os usuários literalmente pesquisarão no nome de exibição, no resumo e nas tags. Use um token de slug independente somente quando ele também for uma identidade estável que você quer manter.
- Não renomeie um slug apenas para perseguir uma consulta, a menos que o novo slug seja um nome canônico melhor no longo prazo. Slugs antigos viram aliases de redirecionamento, mas a URL canônica, o slug exibido e futuros resumos de pesquisa usam o novo slug.
- Aliases de renomeação preservam a resolução para URLs antigas e instalações resolvidas pelo registro, mas o ranqueamento de pesquisa se baseia nos metadados canônicos da Skill depois que a renomeação tiver sido indexada. Estatísticas existentes permanecem com a Skill.
...
- `cursor` (opcional): cursor de paginação para qualquer ordenação não `trending`
...
- Quando `nonSuspiciousOnly=true`, ordenações baseadas em cursor podem retornar menos itens que `limit` em uma página porque Skills suspeitas são filtradas depois da recuperação da página.
...
Denuncia uma Skill para revisão por moderadores. Denúncias são no nível da Skill, opcionalmente vinculadas
...
- `moderation` é um instantâneo atual de moderação no nível da Skill derivado da versão mais recente.
...
Ponto de extremidade autenticado de envio para novos trabalhos do ClawScan.

Varreduras por envio local não são mais compatíveis. Solicitações usando
...
- As cargas úteis de solicitação de varredura e os relatórios baixáveis expiram do armazenamento de solicitações de varredura após a janela de retenção.
...
- Trabalhos de varredura são assíncronos. Solicitações manuais de varredura são priorizadas à frente do trabalho normal de publicação/preenchimento retroativo, mas a conclusão ainda depende da disponibilidade dos workers.

### `GET /api/v1/skills/-/scan/{scanId}`

Ponto de extremidade autenticado de sondagem para uma varredura enviada.

- Retorna o status em fila/em execução/bem-sucedido/com falha.
- Retorna `queue.queuedAhead` e `queue.position` enquanto estiver em fila para que clientes possam mostrar quantas varreduras manuais priorizadas estão à frente da solicitação. Filas muito grandes são limitadas e relatadas com `queuedAheadIsEstimate: true`.
...
Ponto de extremidade autenticado de arquivo de relatórios.
...
Ponto de extremidade autenticado de arquivo de relatórios armazenados para versões enviadas.
...
- `kind` usa `skill` como padrão; use `kind=plugin` para varreduras de Plugin/pacote.
- Retorna o mesmo formato ZIP dos downloads de solicitação de varredura.

### `POST /api/v1/skills/-/scan/batch`

Rota canônica somente para administradores para nova varredura em lote. Ela aceita o mesmo formato de carga útil que o legado `POST /api/v1/skills/-/rescan-batch`.

### `POST /api/v1/skills/-/scan/batch/status`

Rota canônica somente para administradores para status de lote. Ela aceita `{ "jobIds": ["..."] }` e retorna os mesmos contadores agregados que o legado `POST /api/v1/skills/-/rescan-batch/status`.
...
- A identidade da Skill, a identidade do publicador e os metadados da versão selecionada são campos de envelope de nível superior (`slug`, `displayName`, `publisherHandle`, `version`, `resolvedFrom`, `tag`, `createdAt`) para que automações de shell possam lê-los sem desempacotar wrappers aninhados.
...
- `security.signals` contém evidências de suporte do scanner, como `staticScan`, `virusTotal` e `skillSpector`.
- `security.signals.dependencyRegistry` é mantido para compatibilidade de resposta v1, mas o scanner de existência do registro de dependências foi aposentado e esta chave é sempre `null`.
...
ponto de extremidade de coleção é destinado a clientes que já sabem quais versões
de Skills do ClawHub instaladas precisam exibir, como a OpenClaw Control UI.
...
- A resposta é somente de segurança. Ela não inclui dados do Skill Card, status do card gerado, listas de arquivos de artefatos ou cargas úteis detalhadas de scanner.
- `security.signals` contém somente evidências de suporte em nível de status; use `/scan` ou a página de auditoria de segurança do ClawHub para detalhes completos do scanner.
- `security.signals.dependencyRegistry` é mantido para compatibilidade de resposta v1, mas o scanner de existência do registro de dependências foi aposentado e esta chave é sempre `null`.
- A ausência do Skill Card não afeta `ok`, `decision` ou `reasons` deste ponto de extremidade; clientes devem ler o `skill-card.md` instalado localmente quando precisarem do conteúdo do card.
- Use `/verify` quando precisar do envelope de verificação de Skill Card de uma única Skill, `/card` quando precisar do Markdown do card gerado e `/scan` quando precisar de dados detalhados do scanner.
...
- `category` (opcional): filtro de categoria de plugin. Compatível apenas quando a
  solicitação está limitada a pacotes de plugin (`/api/v1/plugins`,
  ...
  aliases legados de filtros da v1 estão documentados em `GET /api/v1/plugins`.
...
- Entradas de habilidades continuam respaldadas pelo registro de habilidades e ainda podem ser publicadas apenas por meio de `POST /api/v1/skills`.
...
Pesquisa de catálogo unificado entre habilidades + pacotes de plugin.
...
- `isOfficial` (opcional): `true` ou `false`
- `category` (opcional): filtro de categoria de plugin. Compatível apenas quando a
  solicitação está limitada a pacotes de plugin. Categorias controladas e aliases
  legados de filtros da v1 estão documentados em `GET /api/v1/plugins`.
...
Aliases legados de filtros da v1 continuam aceitos em endpoints de leitura:
...
`/api/v1/skills?sort=trending` para o catálogo de habilidades.

Aliases legados não são aceitos como valores de categoria armazenados ou declarados pelo autor.

### `GET /api/v1/skills/export`

Exportação em massa das habilidades públicas mais recentes para análise offline.
...
- `startDate` (obrigatório): limite inferior em milissegundos Unix para `updatedAt` da habilidade.
- `endDate` (obrigatório): limite superior em milissegundos Unix para `updatedAt` da habilidade.
...
- Corpo: arquivo ZIP.
- Cada habilidade exportada tem raiz em `{publisher}/{slug}/`.
- Habilidades hospedadas incluem os arquivos da versão armazenada mais recente e são listadas em
  `_manifest.json` com `sourceRef: "public-clawhub"`.
- Habilidades atuais respaldadas pelo GitHub com uma varredura `clean` ou `suspicious` incluem
...
- Cada habilidade inclui `_export_skill_meta.json`.
- `_manifest.json` é sempre incluído na raiz do ZIP.
- `_errors.json` é incluído quando habilidades ou arquivos individuais não puderam ser
  exportados.
...
- Os aliases de filtros legados da v1 documentados em `GET /api/v1/plugins` também são
  aceitos.
- A filtragem por categoria é um filtro real da API, baseado em linhas de digest de
  categoria de Plugin, não uma reescrita da consulta de busca.
- Os resultados são retornados em ordem de relevância e atualmente não têm paginação.
- Os controles de ordenação da UI do navegador para busca de Plugins reordenam os resultados de relevância carregados,
...
- `trust.moderationState` é anulável. Ele é `null` quando não existe moderação manual
  da versão.
- `trust.blockedFromDownload` é o sinal de bloqueio de instalação. O OpenClaw e outros
...
- `trust.stale` significa que o resumo de confiança foi computado a partir de entradas desatualizadas e
...
- Este endpoint é intencionalmente mais restrito que endpoints de moderação de proprietário/moderador.
  ...
  identidades de denunciantes, corpos de denúncia, evidências privadas ou cronogramas internos de revisão.
...
- Versões legadas de pacote retornam um artefato `legacy-zip` e uma URL de download ZIP legada
  `downloadUrl`.
...
- `packageName` é normalizado como nome npm; o pacote pode estar ausente para migrações
  planejadas.
- Isto acompanha apenas a prontidão da migração. Não modifica o OpenClaw nem gera
  ClawPacks.
...
- `manual`: qualquer versão com substituição de moderação manual.
- `all`: qualquer versão com substituição manual, estado de varredura não limpo ou denúncia de pacote.
...
Denuncia um pacote para revisão por moderador. As denúncias são no nível do pacote, opcionalmente
...
- `revoked`: bloqueado depois que uma versão havia sido anteriormente confiável.

Versões em quarentena e revogadas retornam `403` nas rotas de download de artefato.
Cada alteração grava uma entrada de log de auditoria.
...
skill atual baseada em GitHub com uma verificação `clean` ou `suspicious` e sem versão
hospedada.
...
- Transferências de skill baseadas em GitHub não fazem proxy nem espelham bytes. A resposta JSON
  ...
  e `archiveUrl`; o estado de verificação/atual é uma barreira e não é incluído como metadados de
  payload de sucesso.
...
- Campo opcional de payload: `migrateOwner`. Quando `true` com `ownerHandle`, uma
  skill existente pode mudar para esse proprietário se o ator for admin/proprietário tanto nos
  publicadores atual quanto de destino. Sem esta adesão explícita, alterações de proprietário são
  rejeitadas.
...
- Os campos de formulário permitidos são `payload`, blobs repetidos `files` ou uma referência de tarball
  ...
  fluxo de upload-url. Publicações preparadas com id de armazenamento também devem incluir o
...
- Pacotes de Plugin exigem `openclaw.plugin.json`. Uploads `.tgz` ClawPack devem
...
- Somente o publicador da org `openclaw` e os publicadores pessoais de membros atuais da org `openclaw`
...
usuário compartilhado legado/publicador pessoal, o endpoint o migra primeiro para um publicador de org.
...
Criação autenticada e por autoatendimento de publicador de org. Cria um novo publicador de org e adiciona o
chamador como proprietário. Este endpoint não migra identificadores existentes de usuário/pessoal e
...
Somente admin. Reserva slugs raiz e nomes de pacotes para um proprietário legítimo sem publicar uma
versão. Nomes de pacotes se tornam pacotes de espaço reservado privados sem linhas de versão, para que o mesmo
...
sem editar linhas de conta do Convex Auth. A solicitação deve nomear ambos os ids imutáveis de conta de
...
`confirmIdentityVerified: true` depois que a equipe verifica de forma independente a continuidade entre ambos
os principais do GitHub. A recuperação falha de modo fechado quando o publicador pessoal atual do usuário de destino
tem skills, pacotes ou fontes de skill do GitHub.
A recuperação também migra campos legados `ownerUserId` para as skills do publicador recuperado,
aliases de slug de skill, pacotes, avisos do inspetor de pacotes e linhas derivadas do resumo de busca para que
...
Bane um usuário e exclui permanentemente as skills pertencentes a ele (apenas moderador/admin).
...
Remove o banimento de um usuário e restaura as skills qualificadas (apenas admin).
...
conteúdo (apenas admin). Usa simulação por padrão, a menos que `dryRun` seja `false`.
...
Altera a função de um usuário (apenas admin).
...
Lista ou busca usuários (apenas admin).
