...
x-i18n:
    generated_at: "2026-07-02T22:24:19Z"
    model: gpt-5.5
...
- Requisições autenticadas (token Bearer válido): aplicadas por balde de usuário.
- Se o token estiver ausente/inválido, o comportamento recai para aplicação por IP.
- Pontos de extremidade de escrita autenticados não devem retornar um `Unauthorized` simples quando
...
- Download: 1200/min por IP, 6000/min por chave (pontos de extremidade de download)
...
- Se nenhum IP confiável do cliente estiver disponível, requisições anônimas usam baldes de contingência
  escopados apenas pelo tipo de limite de taxa. Esses baldes de contingência não incluem
...
## Pontos de extremidade públicos (sem autenticação)
...
- Os resultados são retornados em ordem de relevância (similaridade de embedding + reforços por token exato de slug/nome + um pequeno fator prévio de popularidade).
...
- Texto ASCII é tokenizado em limites de palavra e pontuação. Por exemplo, `personal-map` contém um token independente `map`, enquanto `amap-jsapi-skill` contém `amap`, `jsapi` e `skill`; portanto, pesquisar por `map` dá a `personal-map` uma correspondência lexical mais forte do que a `amap-jsapi-skill`.
- A popularidade é escalada logaritmicamente e limitada. Skills com alto engajamento podem ficar em posição inferior quando o texto da consulta tem uma correspondência mais fraca.
...
- Coloque os termos que os usuários pesquisarão literalmente no nome de exibição, no resumo e nas tags. Use um token de slug independente apenas quando ele também for uma identidade estável que você deseja manter.
...
- `moderation` é incluído apenas quando a Skill está sinalizada ou quando o proprietário a está visualizando.
...
Ponto de extremidade de moderador/admin para recebimento de denúncias de Skills.
...
Ponto de extremidade de moderador/admin para resolver ou reabrir denúncias de Skills.
...
- `moderation` é um instantâneo de moderação atual no nível da skill derivado da versão mais recente.
...
As varreduras por upload local não são mais compatíveis. Solicitações usando
...
- Trabalhos de varredura são assíncronos. Solicitações manuais de varredura têm prioridade sobre trabalho normal de publicação/preenchimento retroativo, mas a conclusão ainda depende da disponibilidade dos workers.
...
- Retorna `queue.queuedAhead` e `queue.position` enquanto estiver na fila para que os clientes possam mostrar quantas varreduras manuais priorizadas estão à frente da solicitação. Filas muito grandes são limitadas e relatadas com `queuedAheadIsEstimate: true`.
...
- Exige uma varredura concluída com sucesso; varreduras não terminais retornam `409`.
...
- Retorna o mesmo formato de ZIP que os downloads de solicitação de varredura.

### `POST /api/v1/skills/-/scan/batch`

Rota canônica de nova varredura em lote somente para administradores. Ela aceita o mesmo formato de payload que a rota legada `POST /api/v1/skills/-/rescan-batch`.

### `POST /api/v1/skills/-/scan/batch/status`

Rota canônica de status de lote somente para administradores. Ela aceita `{ "jobIds": ["..."] }` e retorna os mesmos contadores agregados que a rota legada `POST /api/v1/skills/-/rescan-batch/status`.
...
- Identidade da skill, identidade do publicador e metadados da versão selecionada são campos de nível superior do envelope (`slug`, `displayName`, `publisherHandle`, `version`, `resolvedFrom`, `tag`, `createdAt`) para que a automação de shell possa lê-los sem desempacotar wrappers aninhados.
...
- `security.signals.dependencyRegistry` é mantido para compatibilidade de resposta v1, mas o scanner de existência do registro de dependências foi desativado e esta chave é sempre `null`.
- `provenance` é `server-resolved-github-import` somente quando ClawHub resolveu e armazenou um repo/ref/commit/path do GitHub durante a publicação ou importação; caso contrário, é `unavailable`.
...
- `items` deve conter 1 a 100 pares únicos `{ slug, version }`.
- Os resultados são por item; uma skill ou versão ausente não faz a resposta inteira falhar.
- A resposta é somente de segurança. Ela não inclui dados do Skill Card, status do cartão gerado, listas de arquivos de artefatos ou payloads detalhados de scanner.
- `security.signals` contém apenas evidências de suporte em nível de status; use `/scan` ou a página de auditoria de segurança do ClawHub para detalhes completos do scanner.
- `security.signals.dependencyRegistry` é mantido para compatibilidade de resposta v1, mas o scanner de existência do registro de dependências foi desativado e esta chave é sempre `null`.
- A ausência do Skill Card não afeta `ok`, `decision` ou `reasons` deste endpoint; os clientes devem ler o `skill-card.md` instalado localmente quando precisarem do conteúdo do cartão.
- Use `/verify` quando precisar do envelope de verificação do Skill Card de uma única skill, `/card` quando precisar do markdown do cartão gerado e `/scan` quando precisar de dados detalhados do scanner.
...
- Chamadores autenticados podem ver pacotes privados de publishers aos quais pertencem nos resultados de listagem/pesquisa.
- `channel=private` retorna apenas pacotes que o chamador autenticado pode ler.

### `GET /api/v1/packages/search`

Pesquisa de catálogo unificada entre Skills + pacotes de plugin.
...
- Skills atuais baseadas no GitHub com uma verificação `clean` ou `suspicious` incluem
...
Exportação em lote das versões públicas mais recentes de plugins para análise offline.
...
- Os aliases de filtro v1 legados documentados em `GET /api/v1/plugins` também são
  aceitos.
- A filtragem por categoria é um filtro de API real apoiado por linhas de resumo
...
- Requer um token de API do proprietário do pacote, um proprietário/admin do publicador da organização,
...
- `version.artifact.kind` é `legacy-zip` para arquivos de pacote do mundo antigo ou
  `npm-pack` para versões apoiadas por ClawPack.
...
- `trust.moderationState` é anulável. É `null` quando não existe moderação manual da
  versão.
- `trust.blockedFromDownload` é o sinal de bloqueio de instalação. OpenClaw e outros
  ...
  derivar novamente regras de bloqueio a partir de campos de scanner ou moderação.
...
Baixa o artefato da versão por meio do caminho explícito do resolvedor.
...
- disponibilidade de artefato npm-pack ClawPack
- resumo do artefato
...
- `bundledPluginId` é normalizado para minúsculas e é a chave estável para upsert.
...
Endpoint de moderador/admin para filas de revisão de versões de pacote.
...
Denuncia um pacote para revisão de moderador. Denúncias são em nível de pacote, opcionalmente
vinculadas a uma versão. Elas alimentam a fila de moderação, mas não ocultam automaticamente nem
bloqueiam downloads por conta própria; moderadores devem usar moderação de versão para
...
Endpoint de moderador/administrador para recebimento de relatórios de pacote.
...
Endpoint de moderador/administrador para resolver ou reabrir relatórios de pacote.
...
Endpoint de moderador/administrador para revisão de release de pacote.
...
- `revoked`: bloqueado depois que uma release foi previamente confiada.

Releases em quarentena e revogadas retornam `403` das rotas de download de artefato.
...
- Verificações pendentes do VirusTotal não bloqueiam leituras; releases maliciosas ainda podem ser retidas em outro lugar.
- Pacotes privados retornam `404`, a menos que o chamador possa ler o publicador proprietário.
...
- Verificações pendentes do VirusTotal não bloqueiam downloads; releases maliciosas retornam `403`.
...
- Packuments de pacote com escopo são compatíveis tanto com `/api/npm/@scope/name` quanto com o caminho de requisição
...
- `tag` (opcional): nome da tag (ex.: `latest`)
...
- Transferências de skill baseada no GitHub não fazem proxy nem espelham bytes. A resposta JSON
  inclui `sourceRef: "public-github"`, `repo`, `commit`, `path`, `contentHash`
  e `archiveUrl`; o estado de verificação/atual é um gate e não é incluído como metadado de payload
  de sucesso.
...
- Campo opcional do payload: `migrateOwner`. Quando `true` com `ownerHandle`, uma
  skill existente pode se mover para esse proprietário se o ator for administrador/proprietário tanto nos
  publicadores atual quanto de destino. Sem essa adesão explícita, alterações de proprietário são
  rejeitadas.
...
- Os campos de formulário permitidos são `payload`, blobs `files` repetidos ou uma referência de tarball
  ...
  fluxo de upload-url. Publicações com storage-id preparado também devem incluir o
...
- Publicações em nome de outra pessoa ainda validam a elegibilidade para canal oficial em relação à conta do proprietário de destino.
...
Somente administrador. Garante que exista um publicador de org para um handle. Se o handle ainda apontar para um
usuário compartilhado legado/publicador pessoal, o endpoint o migra primeiro para um publicador de org.
Para uma org recém-criada, forneça `memberHandle`; o administrador atuante não é adicionado como membro.
`memberRole` assume `owner` por padrão.
...
Somente administrador. Reserva slugs raiz e nomes de pacote para um proprietário legítimo sem publicar uma
release. Nomes de pacote se tornam pacotes placeholder privados sem linhas de release, para que o mesmo
...
Somente administrador. Recupera um publicador pessoal para um principal OAuth do GitHub substituto verificado
sem editar linhas de conta do Convex Auth. A requisição deve nomear ambos os ids imutáveis de conta de
provedor do GitHub; handles mutáveis são usados apenas como uma proteção voltada ao operador.
...
tem habilidades, pacotes ou fontes de habilidades do GitHub.
...
para o handle recuperado também é reatribuída ao usuário substituto para que uma sincronização posterior de perfil
não consiga restaurar a autoridade concorrente do usuário anterior. Cada tabela primária é limitada a
100 linhas por transação de aplicação; recuperações maiores devem primeiro usar uma migração de proprietário retomável.
Fontes de habilidades do GitHub têm escopo de publicador e são relatadas como verificadas, em vez de reescritas.
...
Banir um usuário e excluir permanentemente as habilidades pertencentes a ele (somente moderador/admin).
...
Remover o banimento de um usuário e restaurar habilidades elegíveis (somente admin).
...
Alterar o motivo armazenado para um banimento existente sem remover o banimento nem restaurar
...
Se você faz auto-hospedagem, sirva este arquivo (ou defina `CLAWHUB_REGISTRY` explicitamente; `CLAWDHUB_REGISTRY` legado).
