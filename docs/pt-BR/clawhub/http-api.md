...
read_when:
    ...
    - Depuração de solicitações da CLI ↔ registro
summary: Referência da API HTTP (endpoints públicos + CLI + autenticação).
x-i18n:
    generated_at: "2026-07-04T20:27:20Z"
    model: gpt-5.5
...
- Os resultados são retornados em ordem de relevância (similaridade de embedding + impulsos por tokens exatos de slug/nome + um pequeno prior de popularidade).
- A relevância é mais forte que a popularidade. Uma correspondência precisa de token de slug ou nome de exibição pode superar uma correspondência mais ampla com engajamento muito mais forte.
- Texto ASCII é tokenizado em limites de palavras e pontuação. Por exemplo, `personal-map` contém um token `map` autônomo, enquanto `amap-jsapi-skill` contém `amap`, `jsapi` e `skill`; portanto, pesquisar por `map` dá a `personal-map` uma correspondência lexical mais forte do que `amap-jsapi-skill`.
- A popularidade é escalada logaritmicamente e limitada. Skills de alto engajamento podem classificar pior quando o texto da consulta tem uma correspondência mais fraca.
...
- Coloque os termos que os usuários pesquisarão literalmente no nome de exibição, resumo e tags. Use um token de slug autônomo somente quando ele também for uma identidade estável que você deseja manter.
- Não renomeie um slug apenas para buscar uma consulta, a menos que o novo slug seja um nome canônico melhor no longo prazo. Slugs antigos tornam-se aliases de redirecionamento, mas a URL canônica, o slug exibido e futuros resumos de pesquisa usam o novo slug.
- Aliases de renomeação preservam a resolução de URLs antigas e instalações que resolvem pelo registro, mas a classificação de pesquisa é baseada nos metadados canônicos da Skill depois que a renomeação foi indexada. Estatísticas existentes permanecem com a Skill.
...
Reporte uma Skill para análise de moderador. Relatórios são em nível de Skill, opcionalmente vinculados
...
Retorna detalhes de verificação da varredura de segurança para uma versão da Skill.
...
Endpoint autenticado de sondagem para um scan enviado.

- Retorna status em fila/em execução/bem-sucedido/com falha.
- Retorna `queue.queuedAhead` e `queue.position` enquanto estiver em fila, para que clientes possam mostrar quantos scans manuais priorizados estão à frente da requisição. Filas muito grandes são limitadas e relatadas com `queuedAheadIsEstimate: true`.
...
- `kind` assume `skill` por padrão; use `kind=plugin` para scans de plugin/pacote.
...
- `security.signals.dependencyRegistry` é mantido para compatibilidade de resposta v1, mas o scanner de existência de registro de dependências foi desativado e essa chave é sempre `null`.
...
- `security.signals.dependencyRegistry` é mantido para compatibilidade de resposta v1, mas o scanner de existência de registro de dependências foi desativado e essa chave é sempre `null`.
- A ausência de Skill Card não afeta `ok`, `decision` ou `reasons` deste endpoint; clientes devem ler o `skill-card.md` instalado localmente quando precisarem do conteúdo do card.
...
- `isOfficial` (opcional): `true` ou `false`
- `category` (opcional): filtro de categoria de plugin. Compatível somente quando a
  solicitação tem escopo para pacotes de plugin. Categorias controladas e aliases legados de filtro v1
...
- Os aliases de filtros legados da v1 documentados em `GET /api/v1/plugins` também são
  aceitos.
- A filtragem por categoria é um filtro real da API baseado em linhas de resumo de categoria
  de Plugin, não uma reescrita da consulta de busca.
...
- Requer um token de API do proprietário do pacote, de um proprietário/admin do publicador da organização,
...
- `trust.scanStatus` é o status efetivo de confiança derivado de entradas do scanner
...
- Este endpoint é intencionalmente mais restrito do que endpoints de moderação de proprietário/moderador.
  ...
  identidades de relatores, corpos de relatórios, evidências privadas ou cronogramas internos
  de revisão.
...
Baixa um ZIP de versão de habilidade hospedada ou retorna uma transferência de fonte do GitHub para uma
habilidade atual baseada no GitHub com uma varredura `clean` ou `suspicious` e sem versão
hospedada.
...
- Transferências de habilidades baseadas no GitHub não fazem proxy nem espelham bytes. A resposta JSON
  inclui `sourceRef: "public-github"`, `repo`, `commit`, `path`, `contentHash`
  e `archiveUrl`; o estado atual/de varredura é uma barreira e não é incluído como metadado
...
- Campo opcional do payload: `migrateOwner`. Quando `true` com `ownerHandle`, uma
  habilidade existente pode ser movida para esse proprietário se o ator for admin/proprietário tanto nos
...
Exclui de forma reversível / restaura uma habilidade (proprietário, moderador ou admin).
...
Quando presente, `reason` é armazenado como a observação de moderação da habilidade e copiado para o log de auditoria.
...
- `401`: não autorizado
- `403`: proibido
- `404`: habilidade/usuário não encontrado
...
chamador como proprietário. Este endpoint não migra handles existentes de usuário/pessoal e
...
lançamento. Nomes de pacote se tornam pacotes de placeholder privados sem linhas de lançamento, para que o mesmo
...
As fontes de Skills do GitHub são escopadas ao publicador e relatadas como verificadas, em vez de reescritas.
...
Alterar uma função de usuário (somente admin).
