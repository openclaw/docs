...
x-i18n:
    generated_at: "2026-07-02T00:46:04Z"
    model: gpt-5.5
...
- Endpoints autenticados de escrita não devem retornar um simples `Unauthorized` quando
...
- Se nenhum IP confiável do cliente estiver disponível, requisições anônimas usam baldes de fallback
  escopados apenas por tipo de limite de taxa. Esses baldes de fallback não incluem
...
- Os resultados são retornados em ordem de relevância (similaridade de embeddings + reforços de token exato de slug/nome + uma pequena prioridade de popularidade).
- Relevância é mais forte que popularidade. Uma correspondência precisa de slug ou token de nome de exibição pode superar uma correspondência mais imprecisa com engajamento muito maior.
...
- Popularidade é escalada logaritmicamente e limitada. Skills com alto engajamento podem ficar abaixo quando o texto da consulta tem uma correspondência mais fraca.
...
- Aliases de renome preservam a resolução para URLs antigas e instalações que resolvem pelo registro, mas a classificação de pesquisa se baseia nos metadados canônicos da Skill depois que a renomeação foi indexada. Estatísticas existentes permanecem com a Skill.
...
- Use `nextCursor` para continuar a paginação quando presente. Uma página curta, por si só, não significa o fim dos resultados.
...
Denuncia uma Skill para análise por moderador. Denúncias são em nível de Skill, opcionalmente vinculadas
...
- Retorna status na fila/em execução/bem-sucedido/com falha.
...
- `security.signals.dependencyRegistry` é mantido para compatibilidade de resposta v1, mas o scanner de existência no registro de dependências foi aposentado e essa chave é sempre `null`.
...
- Use `/verify` quando precisar do envelope de verificação de Skill Card de uma única skill, `/card` quando precisar do markdown do card gerado e `/scan` quando precisar de dados detalhados de scanner.
...
- `isOfficial` (opcional): `true` ou `false`
- `category` (opcional): filtro de categoria de Plugin. Compatível somente quando a
  solicitação está limitada a pacotes de Plugin. Categorias controladas e aliases legados
  de filtro v1 estão documentados em `GET /api/v1/plugins`.
...
- A filtragem por categoria é um filtro de API real baseado em linhas de resumo
  de categorias de Plugins, não uma reescrita de consulta de busca.
...
- Requer um token de API para o proprietário do pacote, proprietário/admin de publicador de organização,
...
- `version.sha256hash` é metadado de compatibilidade obsoleto para clientes antigos. Ele
  gera o hash dos bytes exatos do ZIP retornados por `/api/v1/packages/{name}/download`.
...
- `trust.reasons` é a lista de explicações para usuários e auditoria. Os códigos de motivo
...
- Este endpoint é intencionalmente mais restrito que os endpoints de moderação de
  ...
  identidades de relatores, corpos de relatos, evidências privadas ou cronogramas internos
  de revisão.
...
- Somente versões com tarballs ClawPack npm-pack carregados são listadas.
...
- Usa o bucket de taxa de download.
- Os cabeçalhos de download incluem SHA-256 do ClawHub, além de metadados de integridade/shasum do npm.
...
- Os campos de formulário permitidos são `payload`, blobs repetidos em `files` ou uma referência de tarball
  ...
  fluxo de URL de upload. Publicações com storage-id em preparo também devem incluir o
...
- Requisições diretas de publicação multipart são limitadas a 18 MB. Tarballs ClawPack podem
  usar o fluxo de URL de upload até o limite de 120 MB por tarball.
...
Ocultações de moderador/admin e remoções de segurança não expiram dessa forma.
...
proprietário possa depois publicar a versão real de code-plugin ou bundle-plugin com esse nome.
...
os principais do GitHub. A recuperação falha de modo fechado quando o publisher pessoal atual do usuário de destino
...
As fontes de Skills do GitHub têm escopo de publisher e são relatadas como verificadas em vez de reescritas.
