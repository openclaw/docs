...
x-i18n:
    generated_at: "2026-07-01T15:21:04Z"
    model: gpt-5.5
...
- Endpoints autenticados de escrita não devem retornar apenas `Unauthorized` quando
  ...
  de CLI possam informar aos usuários o que os bloqueou.
...
- Se nenhum IP de cliente confiável estiver disponível, requisições anônimas usam buckets alternativos
  escopados apenas pelo tipo de limite de taxa. Esses buckets alternativos não incluem
...
- Os resultados são retornados em ordem de relevância (similaridade de embeddings + reforços por tokens exatos de slug/nome + um pequeno prior de popularidade).
- A relevância é mais forte que a popularidade. Uma correspondência precisa de token de slug ou nome de exibição pode superar uma correspondência mais frouxa com engajamento muito maior.
- Texto ASCII é tokenizado em limites de palavras e pontuação. Por exemplo, `personal-map` contém um token independente `map`, enquanto `amap-jsapi-skill` contém `amap`, `jsapi` e `skill`; portanto, buscar por `map` dá a `personal-map` uma correspondência lexical mais forte que `amap-jsapi-skill`.
- A popularidade é escalada logaritmicamente e limitada. Skills com alto engajamento podem ficar em posição inferior quando o texto da consulta tem correspondência mais fraca.
...
- Coloque os termos que os usuários literalmente buscarão no nome de exibição, no resumo e nas tags. Use um token de slug independente somente quando ele também for uma identidade estável que você deseja manter.
...
- Aliases de renomeação preservam a resolução para URLs antigas e instalações que resolvem pelo registro, mas o ranking de busca é baseado nos metadados canônicos do Skill depois que a renomeação foi indexada. Estatísticas existentes permanecem com o Skill.
...
- Quando `nonSuspiciousOnly=true`, ordenações baseadas em cursor podem retornar menos que `limit` itens em uma página porque Skills suspeitos são filtrados após a recuperação da página.
...
Denuncia um Skill para revisão por moderador. Denúncias são no nível do Skill, opcionalmente vinculadas
...
- Se nem `version` nem `tag` for fornecido, usa a versão mais recente.
...
- Jobs de scan são assíncronos. Requisições manuais de scan são priorizadas à frente do trabalho normal de publicação/backfill, mas a conclusão ainda depende da disponibilidade dos workers.
...
- `kind` tem padrão `skill`; use `kind=plugin` para scans de Plugin/pacote.
- Retorna o mesmo formato ZIP dos downloads de requisição de scan.
...
- `security.signals` contém somente evidências de suporte no nível de status; use `/scan` ou a página de auditoria de segurança do ClawHub para obter detalhes completos do scanner.
- `security.signals.dependencyRegistry` é mantido para compatibilidade de resposta v1, mas o scanner de existência do registro de dependências foi aposentado e esta chave é sempre `null`.
...
- `category` (opcional): filtro de categoria de plugin. Compatível somente quando a
  ...
  aliases de filtro legados da v1 são documentados em `GET /api/v1/plugins`.
...
- Entradas de skill continuam apoiadas pelo registro de skills e ainda podem ser publicadas somente por meio de `POST /api/v1/skills`.
...
- Chamadores autenticados podem ver pacotes privados de publishers aos quais pertencem em resultados de lista/pesquisa.
...
- `category` (opcional): filtro de categoria de plugin. Compatível somente quando a
  ...
  de filtro legados da v1 são documentados em `GET /api/v1/plugins`.
...
Navegação de catálogo somente de plugins em pacotes code-plugin e bundle-plugin.
...
Busca somente de plugins entre pacotes code-plugin e bundle-plugin.
...
- Os aliases de filtros legados da v1 documentados em `GET /api/v1/plugins` também são
  aceitos.
- A filtragem por categoria é um filtro real da API, respaldado por linhas de resumo de
  categorias de plugin, não uma reescrita da consulta de busca.
...
- `version.artifact.kind` é `legacy-zip` para arquivos de pacote do mundo antigo ou
  `npm-pack` para versões respaldadas por ClawPack.
...
Retorna o resumo exato de segurança e confiança da versão do pacote para clientes de instalação.
Esta é a superfície pública de consumo do OpenClaw para decidir se uma
versão resolvida pode ser instalada.

Autenticação:

- Endpoint de leitura público. Nenhum token de proprietário, publisher, moderador ou admin é
  necessário.
...
- `trust.moderationState` pode ser nulo. Ele é `null` quando não existe moderação manual da versão.
...
- Este endpoint é intencionalmente mais restrito do que endpoints de moderação de proprietário/moderador.
  ...
  identidades de denunciantes, corpos de denúncias, evidências privadas ou cronogramas internos de revisão.
...
- `packageName` é normalizado como nome npm; o pacote pode estar ausente para migrações
  planejadas.
- Isso rastreia apenas a prontidão de migração. Não altera o OpenClaw nem gera
  ClawPacks.
...
- Exige um token de API para o proprietário do pacote, membro do publicador,
...
- `dist.tarball`, `dist.integrity` e `dist.shasum` usam campos compatíveis com npm
  para que os usuários possam apontar npm para o espelho se quiserem.
...
- Transferências de skills baseadas no GitHub não fazem proxy nem espelham bytes. A resposta JSON
  ...
  e `archiveUrl`; o estado de verificação/atual é um gate e não é incluído como metadados de payload
  de sucesso.
...
- Campo opcional do payload: `migrateOwner`. Quando `true` com `ownerHandle`, uma
  ...
  publicadores atual quanto de destino. Sem essa aceitação explícita, alterações de proprietário são
  rejeitadas.
...
- Os campos de formulário permitidos são `payload`, blobs `files` repetidos ou uma referência de tarball
  ...
  fluxo de upload-url. Publicações com storage-id preparados também devem incluir o
...
- Publicações em nome de outro usuário ainda validam a elegibilidade do canal oficial contra a conta do proprietário de destino.
...
usuário compartilhado legado/publicador pessoal, o endpoint primeiro o migra para um publicador de org.
...
chamador como proprietário. Este endpoint não migra handles de usuário/pessoais existentes e
...
release. Nomes de pacotes tornam-se pacotes privados de placeholder sem linhas de release, para que o mesmo
...
aliases de slug de skill, pacotes, avisos do inspetor de pacotes e linhas derivadas do digest de pesquisa, para que
...
Veja `DEPRECATIONS.md` para o plano de remoção.
