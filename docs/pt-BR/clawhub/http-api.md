...
x-i18n:
    generated_at: "2026-07-04T03:38:18Z"
    model: gpt-5.5
...
Todos os caminhos v1 ficam sob `/api/v1/...`.
Os legados `/api/...` e `/api/cli/...` permanecem para compatibilidade (veja `DEPRECATIONS.md`).
...
Atalhos de slug web resolvem entre famílias de registro, mas clientes de API devem usar
...
- Solicitações anônimas: aplicado por IP.
- Solicitações autenticadas (token Bearer válido): aplicado por bucket de usuário.
...
- Se nenhum IP confiável de cliente estiver disponível, solicitações anônimas usam buckets de fallback
  com escopo apenas pelo tipo de limite de taxa. Esses buckets de fallback não incluem
...
- `highlightedOnly` (opcional): `true` para filtrar apenas skills destacadas
...
- Os resultados são retornados em ordem de relevância (similaridade de embeddings + reforços por token exato de slug/nome + um pequeno fator prévio de popularidade).
- A relevância é mais forte que a popularidade. Uma correspondência precisa de token de slug ou nome de exibição pode superar uma correspondência mais vaga com engajamento muito maior.
...
- A popularidade é escalada logaritmicamente e limitada. Skills com alto engajamento podem ranquear abaixo quando o texto da consulta é uma correspondência mais fraca.
...
- Coloque os termos que os usuários literalmente pesquisarão no nome de exibição, resumo e tags. Use um token de slug autônomo somente quando ele também for uma identidade estável que você deseja manter.
...
- Aliases de renomeação preservam a resolução para URLs antigas e instalações que resolvem pelo registro, mas o ranking de pesquisa é baseado nos metadados canônicos da skill depois que a renomeação é indexada. Estatísticas existentes permanecem com a skill.
- Se uma skill estiver inesperadamente invisível, verifique primeiro o estado de moderação com `clawhub inspect @owner/slug` enquanto estiver conectado antes de alterar metadados relacionados a ranking.
...
Retorna o estado estruturado de moderação.
...
Relate uma skill para análise de moderador. Relatos são no nível da skill, opcionalmente vinculados
...
Escaneamentos de upload local não são mais compatíveis. Requisições que usam
`multipart/form-data` ou `{ "source": { "kind": "upload" } }` retornam `410`.

Escaneamentos publicados usam JSON:
...
- Payloads de requisição de escaneamento e relatórios baixáveis expiram do armazenamento de requisições de escaneamento após a janela de retenção.
- Escaneamentos publicados exigem acesso de gerenciamento de proprietário/publicador ou autoridade de moderador/administrador da plataforma.
- Escaneamentos publicados gravam de volta somente quando `update: true` e o escaneamento é concluído com sucesso.
...
- Trabalhos de escaneamento são assíncronos. Requisições manuais de escaneamento são priorizadas à frente do trabalho normal de publicação/preenchimento retroativo, mas a conclusão ainda depende da disponibilidade dos workers.

### `GET /api/v1/skills/-/scan/{scanId}`

Endpoint autenticado de polling para um escaneamento enviado.

- Retorna status queued/running/succeeded/failed.
- Retorna `queue.queuedAhead` e `queue.position` enquanto estiver na fila, para que clientes possam mostrar quantos escaneamentos manuais priorizados estão à frente da requisição. Filas muito grandes são limitadas e reportadas com `queuedAheadIsEstimate: true`.
- Quando disponível, `report` contém seções `clawscan`, `skillspector`, `staticAnalysis` e `virustotal`.
- Trabalhos de escaneamento com falha retornam `status: "failed"` com `lastError`.

### `GET /api/v1/skills/-/scan/{scanId}/download`

Endpoint autenticado de arquivo de relatório.

- Exige um escaneamento concluído com sucesso; escaneamentos não terminais retornam `409`.
...
- Retorna resultados de escaneamento armazenados para a versão exata enviada, incluindo versões bloqueadas ou ocultas.
- `kind` tem `skill` como padrão; use `kind=plugin` para escaneamentos de plugin/pacote.
- Retorna o mesmo formato de ZIP dos downloads de requisições de escaneamento.

### `POST /api/v1/skills/-/scan/batch`

Rota canônica de reescaneamento em lote somente para administradores. Ela aceita o mesmo formato de payload que a rota legada `POST /api/v1/skills/-/rescan-batch`.
...
- `security.signals.dependencyRegistry` é mantido para compatibilidade da resposta v1, mas o scanner de existência do registro de dependências foi descontinuado e esta chave é sempre `null`.
- `provenance` é `server-resolved-github-import` somente quando o ClawHub resolveu e armazenou um repo/ref/commit/caminho do GitHub durante a publicação ou importação; caso contrário, é `unavailable`.

### `POST /api/v1/skills/-/security-verdicts`

Retorna vereditos compactos de segurança atuais para versões exatas de skills. Este
...
- `security.signals` contém apenas evidências de apoio no nível de status; use `/scan` ou a página de auditoria de segurança do ClawHub para detalhes completos do scanner.
- `security.signals.dependencyRegistry` é mantido para compatibilidade da resposta v1, mas o scanner de existência do registro de dependências foi descontinuado e esta chave é sempre `null`.
...
- Use `/verify` quando precisar do envelope de verificação do Skill Card de uma única skill, `/card` quando precisar do markdown de card gerado e `/scan` quando precisar de dados detalhados de scanner.
...
- `POST /api/v1/packages` continua sendo somente para releases de code-plugin e bundle-plugin.
...
Pesquisa de catálogo unificado em Skills + pacotes de Plugin.
...
- `category` (opcional): filtro de categoria de Plugin. Compatível somente quando a
  ...
  de filtro v1 legados estão documentados em `GET /api/v1/plugins`.
...
- Skills atuais apoiadas pelo GitHub com uma varredura `clean` ou `suspicious` incluem
  `_source_handoff.json` com `sourceRef: "public-github"`, repo, commit, caminho,
...
Exportação em massa das versões públicas mais recentes de Plugin para análise offline.
...
- Exige um token de API do proprietário do pacote, de um proprietário/administrador do publisher da organização,
...
- `version.artifact.kind` é `legacy-zip` para arquivos de pacotes do mundo antigo ou
  `npm-pack` para versões baseadas em ClawPack.
- Versões ClawPack incluem campos compatíveis com npm `npmIntegrity`, `npmShasum` e
  `npmTarballName`.
- `version.sha256hash` são metadados de compatibilidade obsoletos para clientes antigos. Ele
...
- Endpoint de leitura pública. Nenhum token de proprietário, publisher, moderador ou administrador é
  obrigatório.
...
- `trust.moderationState` é anulável. É `null` quando não existe moderação manual da versão.
...
- Este endpoint é intencionalmente mais restrito do que endpoints de moderação de proprietário/moderador.
  ...
  identidades de denunciantes, corpos de denúncias, evidências privadas ou linhas do tempo internas
  de revisão.

### `GET /api/v1/packages/{name}/versions/{version}/artifact`

Retorna os metadados explícitos do resolvedor de artefatos para uma versão de pacote.
...
- Exige um token de API de usuário moderador ou administrador.
...
- Exige um token de API de usuário administrador.
...
vinculadas a uma versão. Elas alimentam a fila de moderação, mas não ocultam nem
bloqueiam downloads automaticamente por si só; moderadores devem usar moderação de versão para
...
`finalAction: "revoke"` com um relatório confirmado para aplicar moderação da versão no
...
- `revoked`: bloqueado depois que uma versão havia sido considerada confiável.
...
- Packuments de pacotes com escopo oferecem suporte tanto a `/api/npm/@scope/name` quanto ao caminho de requisição
...
- Transferências de skills baseadas no GitHub não fazem proxy nem espelham bytes. A resposta JSON
  ...
  de sucesso.
- Estatísticas de download são contabilizadas como identidades únicas por dia UTC (`userId` quando o token de API é válido, caso contrário IP).
...
- Os campos de formulário permitidos são `payload`, blobs `files` repetidos ou uma referência de tarball
  ...
  fluxo de upload-url. Publicações com ID de armazenamento preparado também devem incluir o
...
- Campo opcional do payload: `ownerHandle`. Quando presente, apenas admins podem publicar em nome desse proprietário.
...
- Apenas o publicador da org `openclaw` e publicadores pessoais dos membros atuais da org `openclaw`
...
Somente admin. Garante que exista um publicador de org para um handle. Se o handle ainda apontar para um
usuário/publicador pessoal compartilhado legado, o endpoint o migra primeiro para um publicador de org.
...
proprietário possa depois publicar a versão real de code-plugin ou bundle-plugin nesse nome.
...
principais do GitHub. A recuperação falha em modo fechado quando o publicador pessoal atual do usuário de destino
tem Skills, pacotes ou fontes de Skill do GitHub.
A recuperação também migra campos legados `ownerUserId` para os Skills do publicador recuperado,
aliases de slug de Skill, pacotes, avisos do inspetor de pacotes e linhas derivadas de resumo de busca, para que
os caminhos de proprietário direto concordem com a nova autoridade do publicador. Uma reserva de handle protegido
ativa para o handle recuperado também é reatribuída ao usuário substituto, para que uma sincronização posterior de
perfil não possa restaurar a autoridade concorrente do usuário anterior. Cada tabela primária é limitada a
...
### Endpoints de gerenciamento de slug de proprietário
...
- Ambos os endpoints exigem autenticação por token de API e funcionam apenas para o proprietário do Skill.
...
Bane um usuário e exclui permanentemente os Skills de sua propriedade (somente moderador/admin).
...
Se você fizer self-host, sirva este arquivo (ou defina `CLAWHUB_REGISTRY` explicitamente; `CLAWDHUB_REGISTRY` legado).
