...
Diretórios de terceiros podem usar os endpoints públicos de leitura para listar ou pesquisar Skills do ClawHub. Armazene os resultados em cache, respeite `429`/`Retry-After`, direcione os usuários para a listagem canônica do ClawHub (`https://clawhub.ai/<owner>/skills/<slug>`) e evite sugerir que o ClawHub endossa o site de terceiros. Não tente espelhar conteúdo oculto, privado ou bloqueado pela moderação fora da superfície da API pública.

Os atalhos de slug da Web são resolvidos entre famílias de registros, mas os clientes da API devem usar
...
- Solicitações anônimas: limite aplicado por IP.
- Solicitações autenticadas (token Bearer válido): limite aplicado por grupo de usuário.
- Se o token estiver ausente ou for inválido, o comportamento volta à aplicação por IP.
- Os endpoints autenticados de gravação não devem retornar apenas `Unauthorized` quando
  o servidor souber o motivo. Tokens ausentes, tokens inválidos/revogados e
  contas excluídas/banidas/desativadas devem receber textos orientativos distintos, para que os clientes
  CLI possam informar aos usuários o que os bloqueou.

- Leitura: 3.000/min por IP, 12.000/min por chave
- Gravação: 300/min por IP, 3.000/min por chave
- Download: 1.200/min por IP, 6.000/min por chave (endpoints de download)

Cabeçalhos:

- Compatibilidade legada: `X-RateLimit-Limit`, `X-RateLimit-Reset`
- Padronizados: `RateLimit-Limit`, `RateLimit-Reset`
- Em caso de `429`: `X-RateLimit-Remaining: 0` e `RateLimit-Remaining: 0`
- Em caso de `429`: `Retry-After`
...
- `Retry-After`: segundos a aguardar antes de tentar novamente (atraso) em caso de `429`
...
- Use recuo com aleatoriedade para evitar novas tentativas sincronizadas.
...
- Se nenhum IP confiável do cliente estiver disponível, as solicitações anônimas usarão grupos alternativos
  cujo escopo se limita ao tipo de limite de taxa. Esses grupos alternativos não incluem
  caminhos, slugs, nomes de pacotes, versões, strings de consulta ou outros
  parâmetros de artefatos fornecidos pelo chamador.
...
- A relevância tem mais peso que a popularidade. Uma correspondência precisa de token de slug ou nome de exibição pode superar uma correspondência menos exata com engajamento muito maior.
...
- Inclua os termos que os usuários pesquisarão literalmente no nome de exibição, no resumo e nas tags. Use um token de slug independente somente quando ele também for uma identidade estável que você queira manter.
...
- Os aliases de renomeação preservam a resolução de URLs antigas e instalações resolvidas por meio do registro, mas a classificação da pesquisa se baseia nos metadados canônicos da Skill depois que a renomeação é indexada. As estatísticas existentes permanecem com a Skill.
- Se uma Skill estiver inesperadamente invisível, verifique primeiro o estado de moderação com `clawhub inspect @owner/slug` enquanto estiver conectado, antes de alterar metadados relacionados à classificação.
...
- `trending` classifica pelo número de instalações nos últimos 7 dias (com base em telemetria).
- `createdAt` é estável para rastreamentos de Skills novas; `updated` muda quando Skills existentes são republicadas.
- Quando `nonSuspiciousOnly=true`, ordenações baseadas em cursor podem retornar menos de `limit` itens em uma página porque as Skills suspeitas são filtradas depois que a página é recuperada.
- Use `nextCursor` para continuar a paginação quando estiver presente. Uma página curta, por si só, não significa que os resultados terminaram.
...
- `metadata.systems`: sistemas de destino do Nix (por exemplo, `["aarch64-darwin", "x86_64-linux"]`). `null` se não declarados.
...
Denuncia uma Skill para análise por moderadores. As denúncias se aplicam à Skill como um todo, podem opcionalmente ser vinculadas
a uma versão e são enviadas à fila de denúncias de Skills.
...
Endpoint de moderador/administrador para receber denúncias de Skills.
...
Retorna os metadados da versão e a lista de arquivos.

- `version.security` inclui o estado normalizado de verificação da varredura e os detalhes dos mecanismos de varredura
  (VirusTotal + LLM), quando disponíveis.

### `GET /api/v1/skills/{slug}/scan`

Retorna os detalhes de verificação da varredura de segurança de uma versão da Skill.

Parâmetros de consulta:

- `version` (opcional): string de uma versão específica.
...
- Inclui o status de verificação normalizado e detalhes específicos de cada scanner.
...
- As verificações publicadas gravam os resultados de volta somente quando `update: true` e a verificação é concluída com êxito.
- A resposta é `202` com `{ "ok": true, "scanId": "...", "jobId": "...", "status": "queued", "sourceKind": "published", "update": false, "queue": { "queuedAhead": 0, "queuedAheadIsEstimate": false, "position": 1, "running": 0, "runningIsEstimate": false, "note": "As verificações são assíncronas e podem levar algum tempo para serem concluídas." } }`.
- Os trabalhos de verificação são assíncronos. As solicitações de verificação manual têm prioridade sobre o trabalho normal de publicação/preenchimento retroativo, mas a conclusão ainda depende da disponibilidade dos workers.

### `GET /api/v1/skills/-/scan/{scanId}`

Endpoint autenticado de consulta para uma verificação enviada.

- Retorna o status na fila/em execução/concluída com êxito/com falha.
- Retorna `queue.queuedAhead` e `queue.position` enquanto estiver na fila, para que os clientes possam mostrar quantas verificações manuais prioritárias estão à frente da solicitação. Filas muito grandes são limitadas e informadas com `queuedAheadIsEstimate: true`.
...
- Exige uma verificação concluída com êxito; verificações não terminais retornam `409`.
...
- Retorna o mesmo formato de ZIP dos downloads de solicitações de verificação.

### `POST /api/v1/skills/-/scan/batch`

Rota canônica de nova verificação em lote, exclusiva para administradores. Ela aceita o mesmo formato de carga útil que a rota legada `POST /api/v1/skills/-/rescan-batch`.
...
Retorna o envelope de verificação do cartão da Skill usado por `clawhub skill verify`.
...
- `ok` é `true` somente quando a versão selecionada tem um cartão da Skill gerado, não está bloqueada pela moderação por conter malware e a verificação do ClawScan está limpa.
- A identidade da Skill, a identidade do publicador e os metadados da versão selecionada são campos de nível superior do envelope (`slug`, `displayName`, `publisherHandle`, `version`, `resolvedFrom`, `tag`, `createdAt`), para que automações de shell possam lê-los sem desempacotar invólucros aninhados.
...
- `security.signals` contém evidências complementares dos scanners, como `staticScan`, `virusTotal` e `skillSpector`.
- `security.signals.dependencyRegistry` é mantido para compatibilidade com a resposta v1, mas o scanner de existência no registro de dependências foi descontinuado e esta chave é sempre `null`.
- `provenance` é `server-resolved-github-import` somente quando o ClawHub resolveu e armazenou um repositório/referência/commit/caminho do GitHub durante a publicação ou importação; caso contrário, é `unavailable`.
...
- Os resultados são por item; uma Skill ou versão ausente não causa falha na resposta inteira.
- A resposta contém apenas informações de segurança. Ela não inclui dados do cartão da Skill, status do cartão gerado, listas de arquivos do artefato nem cargas úteis detalhadas dos scanners.
- `security.signals` contém somente evidências complementares no nível do status; use `/scan` ou a página de auditoria de segurança do ClawHub para obter todos os detalhes dos scanners.
- `security.signals.dependencyRegistry` é mantido para compatibilidade com a resposta v1, mas o scanner de existência no registro de dependências foi descontinuado e esta chave é sempre `null`.
- A ausência do cartão da Skill não afeta `ok`, `decision` nem `reasons` deste endpoint; os clientes devem ler localmente o `skill-card.md` instalado quando precisarem do conteúdo do cartão.
- Use `/verify` quando precisar do envelope de verificação do cartão de uma única Skill, `/card` quando precisar do Markdown do cartão gerado e `/scan` quando precisar de dados detalhados dos scanners.
...
- `category` (opcional): filtro de categoria de plugin. Compatível somente quando a
  solicitação está restrita a pacotes de plugins (`/api/v1/plugins`,
...
- As entradas de Skills continuam respaldadas pelo registro de Skills e ainda podem ser publicadas somente por meio de `POST /api/v1/skills`.
- `POST /api/v1/packages` continua sendo destinado somente a lançamentos de plugins de código e plugins de pacote.
- Chamadores anônimos veem somente canais públicos de pacotes.
- Chamadores autenticados podem ver nos resultados de listagem/pesquisa os pacotes privados de publicadores aos quais pertencem.
- `channel=private` retorna somente pacotes que o chamador autenticado pode ler.

### `GET /api/v1/packages/search`

Pesquisa unificada no catálogo de Skills e pacotes de plugins.
...
- `isOfficial` (opcional): `true` ou `false`
- `category` (opcional): filtro de categoria de plugin. Compatível somente quando a
  solicitação está restrita a pacotes de plugins. As categorias controladas e os
  aliases de filtro legados da v1 estão documentados em `GET /api/v1/plugins`.
...
- Corpo: arquivo ZIP.
- Cada Skill exportada fica na raiz `{publisher}/{slug}/`.
...
- `limit` (opcional): inteiro (1-250), padrão `250`.
- `cursor` (opcional): cursor de paginação da resposta anterior.
- `family` (opcional): `code-plugin` ou `bundle-plugin`. Se omitido, inclui ambas as famílias de plugins.
...
- `_manifest.json` é sempre incluído na raiz do ZIP.
- `_errors.json` é incluído quando não foi possível exportar plugins ou arquivos individuais.
...
Pesquisa exclusiva de plugins nos pacotes code-plugin e bundle-plugin.
...
- Os aliases legados de filtros da v1 documentados em `GET /api/v1/plugins` também são aceitos.
- A filtragem por categoria é um filtro real da API, baseado nas linhas de resumo de categoria dos plugins, e não uma reformulação da consulta de pesquisa.
- Os resultados são retornados em ordem de relevância e atualmente não têm paginação.
- Os controles de ordenação da interface do navegador para pesquisa de plugins reordenam os resultados de relevância carregados, correspondendo ao comportamento atual de navegação em `/skills`.
...
Exclui de forma reversível um pacote e todas as suas versões.

Observações:

- Exige um token de API do proprietário do pacote, de um proprietário/administrador da organização publicadora, de um moderador da plataforma ou de um administrador da plataforma.
...
Retorna uma versão do pacote, incluindo metadados de arquivos, compatibilidade, verificação, metadados do artefato e dados de varredura.

Observações:

- `version.artifact.kind` é `legacy-zip` para arquivos de pacote do formato antigo ou `npm-pack` para versões baseadas em ClawPack.
- As versões ClawPack incluem os campos compatíveis com npm `npmIntegrity`, `npmShasum` e `npmTarballName`.
- `version.sha256hash` é um metadado de compatibilidade obsoleto para clientes antigos. Ele calcula o hash dos bytes exatos do ZIP retornado por `/api/v1/packages/{name}/download`. Clientes modernos devem usar `version.artifact.sha256`, que identifica o artefato canônico da versão.
- `version.vtAnalysis`, `version.llmAnalysis` e `version.staticScan` são incluídos quando há dados de varredura.
- Pacotes privados retornam `404`, a menos que o chamador possa ler o publicador proprietário.

### `GET /api/v1/packages/{name}/versions/{version}/security`

Retorna o resumo exato de segurança e confiança da versão do pacote para clientes de instalação. Esta é a interface pública de consumo do OpenClaw para decidir se uma versão resolvida pode ser instalada.

Autenticação:

- Endpoint público de leitura. Não é necessário um token de proprietário, publicador, moderador ou administrador.
...
- `package.name`, `package.displayName` e `package.family` identificam o pacote resolvido no registro.
- `release.releaseId`, `release.version` e `release.createdAt` identificam a versão exata que foi avaliada.
- `release.artifactKind`, `release.artifactSha256`, `release.npmIntegrity`, `release.npmShasum` e `release.npmTarballName` estão presentes quando são conhecidos para o artefato da versão.
- `trust.scanStatus` é o status efetivo de confiança derivado das entradas dos mecanismos de varredura e da moderação manual da versão.
- `trust.moderationState` pode ser nulo. É `null` quando não há moderação manual da versão.
- `trust.blockedFromDownload` é o sinal de bloqueio da instalação. O OpenClaw e outros clientes de instalação devem bloquear a instalação quando esse valor for `true`, em vez de recalcular as regras de bloqueio com base nos campos de varredura ou moderação.
- `trust.reasons` é a lista de explicações exibida ao usuário e usada para auditoria. Os códigos de motivo são strings estáveis e compactas, como `manual:quarantined`, `scan:malicious` e `package:malicious`.
- `trust.pending` significa que uma ou mais entradas de confiança ainda aguardam conclusão.
- `trust.stale` significa que o resumo de confiança foi calculado com entradas desatualizadas e deve ser tratado como exigindo atualização antes de uma decisão de permissão com alta confiança.

Observações:

- Este endpoint é específico para uma versão exata. Os clientes devem chamá-lo após resolver a versão do pacote que pretendem instalar, e não apenas após ler os metadados mais recentes do pacote.
- Pacotes privados retornam `404`, a menos que o chamador possa ler o publicador proprietário.
- Este endpoint é intencionalmente mais restrito do que os endpoints de moderação de proprietários/moderadores. Ele expõe a decisão de instalação e a explicação pública, não as identidades dos denunciantes, o conteúdo das denúncias, evidências privadas ou cronogramas internos de revisão.
...
- Versões legadas de pacotes retornam um artefato `legacy-zip` e uma `downloadUrl` de ZIP legado.
- Versões ClawPack retornam um artefato `npm-pack`, campos de integridade do npm, uma `tarballUrl` e a URL de compatibilidade do ZIP legado.
- Esta é a interface de resolução do OpenClaw; ela evita inferir o formato do arquivo a partir de uma URL compartilhada.
...
- Versões ClawPack transmitem os bytes exatos do arquivo `.tgz` npm-pack enviado.
- Versões ZIP legadas redirecionam para `/api/v1/packages/{name}/download?version=`.
- Usa o grupo de limitação de taxa de downloads.
...
- Exige um token de API de um usuário moderador ou administrador.
...
- Exige um token de API de um usuário administrador.
...
- `packageName` é normalizado como nome npm; o pacote pode estar ausente para migrações planejadas.
- Isso rastreia apenas a prontidão da migração. Não modifica o OpenClaw nem gera ClawPacks.
...
Denuncia um pacote para revisão por moderadores. As denúncias são referentes ao pacote e podem, opcionalmente, ser vinculadas a uma versão. Elas alimentam a fila de moderação, mas, por si só, não ocultam automaticamente nem bloqueiam downloads; os moderadores devem usar a moderação de versões para aprovar, colocar em quarentena ou revogar artefatos.
...
- `status` (opcional): `open` (padrão), `confirmed`, `dismissed` ou `all`
- `limit` (opcional): número inteiro (1-100)
- `cursor` (opcional): cursor de paginação
...
Endpoint do proprietário/moderador para visibilidade da moderação de pacotes.

Autenticação:

- Requer um token de API do proprietário do pacote, de um membro do publicador, de um moderador ou de um
...
Endpoint de moderador/administrador para análise de versões de pacotes.
...
- `approved`: analisada manualmente e permitida.
- `quarantined`: bloqueada enquanto aguarda acompanhamento.
- `revoked`: bloqueada depois que uma versão foi considerada confiável anteriormente.
...
- Usa por padrão a versão mais recente.
- Usa o grupo de limite de taxa de leitura, não o grupo de download.
...
- Esta rota permanece exclusivamente para ZIP. Ela não transmite arquivos `.tgz` do ClawPack.
...
- Versões legadas disponíveis apenas em ZIP são omitidas intencionalmente.
...
- Packuments de pacotes com escopo são compatíveis tanto com `/api/npm/@scope/name` quanto com o
  caminho de solicitação codificado do npm, `/api/npm/@scope%2Fname`.

### `GET /api/npm/{package}/-/{tarball}.tgz`

Transmite exatamente os bytes do tarball do ClawPack enviado para clientes de espelho npm.

Observações:

- Usa o grupo de limite de taxa de download.
...
Baixa o ZIP de uma versão hospedada de uma skill ou retorna um encaminhamento para o código-fonte do GitHub de uma
...
- Os encaminhamentos de skills baseadas no GitHub não atuam como proxy nem espelham bytes. A resposta JSON
  inclui `sourceRef: "public-github"`, `repo`, `commit`, `path`, `contentHash`
  e `archiveUrl`; o estado da verificação/atual funciona como uma barreira e não é incluído como metadado
  da carga útil de sucesso.
- As estatísticas de download são contadas por identidades únicas por dia UTC (`userId` quando o token de API é válido; caso contrário, IP).
...
- Um corpo JSON com `files` (baseados em storageId) também é aceito.
- Campo opcional da carga útil: `ownerHandle`. Quando presente, a API resolve esse
  publicador no lado do servidor e exige que o agente tenha acesso ao publicador.
- Campo opcional da carga útil: `migrateOwner`. Quando `true` com `ownerHandle`, uma
  skill existente pode ser movida para esse proprietário se o agente for administrador/proprietário nos publicadores
...
- Corpos JSON e metadados `payload.files` / `payload.artifact` fornecidos pelo chamador
  são rejeitados.
- Solicitações de publicação multipart direta são limitadas a 18 MB. Tarballs do ClawPack podem
  usar o fluxo de URL de upload até o limite de 120 MB para tarballs.
- Campo opcional da carga útil: `ownerHandle`. Quando presente, somente administradores podem publicar em nome desse proprietário.
...
outro publicador. A resposta da exclusão inclui `slugReservedUntil` quando esse prazo se aplica.
Ocultações realizadas por moderador/administrador e remoções de segurança não expiram dessa forma.
...
Criação autenticada e autônoma de um publicador de organização. Cria um novo publicador de organização e adiciona o
chamador como proprietário. Este endpoint não migra identificadores existentes de usuários/pessoais e
...
versão. Os nomes de pacotes tornam-se pacotes privados de espaço reservado sem linhas de versão, para que o mesmo
...
Somente para administradores. Recupera um publicador pessoal para uma identidade principal substituta verificada do OAuth do GitHub
sem editar as linhas de conta do Convex Auth. A solicitação deve nomear os dois IDs imutáveis de conta do
provedor GitHub; identificadores mutáveis são usados apenas como proteção voltada para o operador.

O endpoint usa simulação por padrão. A aplicação da recuperação exige `dryRun: false` e
`confirmIdentityVerified: true` após a equipe verificar de forma independente a continuidade entre ambas as
identidades principais do GitHub. A recuperação falha de forma segura quando o publicador pessoal atual
do usuário de destino possui skills, pacotes ou fontes de skills do GitHub.
A recuperação também migra os campos legados `ownerUserId` das skills do publicador recuperado,
dos aliases de slug das skills, dos pacotes, dos avisos do inspetor de pacotes e das linhas derivadas do resumo de pesquisa, para que
...
do perfil não possa restaurar a autoridade concorrente do usuário anterior. Cada tabela principal é limitada a
100 linhas por transação de aplicação; recuperações maiores devem primeiro usar uma migração de proprietário retomável.
As fontes de skills do GitHub têm escopo por publicador e são relatadas como verificadas, em vez de reescritas.
...
### Endpoints de gerenciamento de slugs pelo proprietário
...
Remove o banimento de um usuário e restaura as skills elegíveis (somente administrador).
...
o conteúdo (somente administrador). Usa simulação por padrão, a menos que `dryRun` seja `false`.
...
Adiciona/remove uma estrela (destaque). Ambos os endpoints são idempotentes.
