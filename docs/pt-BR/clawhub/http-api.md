...
- Padronizado: `RateLimit-Limit`, `RateLimit-Remaining`, `RateLimit-Reset`
- Em `429`: `Retry-After`
...
- Downloads são usados como um pequeno prior em escala logarítmica e critério de desempate, não como o principal sinal de ranqueamento. Skills com muitos downloads podem ranquear abaixo quando o texto da consulta tem uma correspondência mais fraca.
...
- Aliases de renomeação preservam a resolução para URLs antigas e instalações que resolvem pelo registro, mas o ranqueamento de pesquisa é baseado nos metadados canônicos da Skill após a renomeação ter sido indexada. Estatísticas existentes permanecem com a Skill.
...
- `requiresBrowser`, `requiresDesktop`, `requiresNativeDeps`,
  ...
  (opcional): abreviação `true`/`1` para tags de requisitos de ambiente
- `externalService`, `binary`, `osPermission` (opcional): abreviação para tags
  nomeadas de requisitos de ambiente
...
- Entradas de Skill continuam respaldadas pelo registro de Skills e ainda podem ser publicadas somente por meio de `POST /api/v1/skills`.
- `POST /api/v1/packages` continua sendo somente para versões de code-plugin e bundle-plugin.
...
Retorna metadados de detalhes do pacote.
...
Denuncia um pacote para revisão do moderador. Denúncias são em nível de pacote, opcionalmente
...
Endpoint de moderador/admin para recebimento de denúncias de pacote.
...
Endpoint de manutenção somente para admins para rotular lançamentos de pacotes mais antigos com
...
- Usa o lançamento mais recente por padrão.
...
- A moderação e as verificações de acesso a pacotes privados ainda se aplicam.
...
- Campo opcional de payload: `migrateOwner`. Quando `true` com `ownerHandle`, uma
  skill existente pode ser movida para esse proprietário se o ator for admin/proprietário nos dois
  publicadores atual e de destino. Sem essa adesão explícita, alterações de proprietário são
  rejeitadas.
...
- Pacotes Plugin exigem `openclaw.plugin.json`. Envios de ClawPack `.tgz` devem
...
lançamento. Nomes de pacotes se tornam pacotes privados de placeholder sem linhas de lançamento, para que o mesmo
