...
x-i18n:
    generated_at: "2026-07-04T15:10:14Z"
    model: gpt-5.5
...
Diretórios de terceiros podem usar os endpoints públicos de leitura para listar ou buscar Skills do ClawHub. Armazene os resultados em cache, respeite `429`/`Retry-After`, vincule os usuários de volta à listagem canônica do ClawHub (`https://clawhub.ai/<owner>/skills/<slug>`) e evite insinuar endosso do ClawHub ao site de terceiros. Não tente espelhar conteúdo oculto, privado ou bloqueado por moderação fora da superfície pública da API.

Atalhos de slug da web resolvem entre famílias de registro, mas clientes da API devem usar
...
- `X-RateLimit-Remaining` / `RateLimit-Remaining`: orçamento restante exato quando presente.
  Requisições bem-sucedidas fragmentadas omitem esse cabeçalho em vez de retornar um valor global aproximado.
...
- Se `Retry-After` existir, espere essa quantidade de segundos antes de tentar novamente.
...
- ClawHub usa cabeçalhos de encaminhamento confiáveis para identificar IPs de cliente na borda.
...
- Os resultados são retornados em ordem de relevância (similaridade de embedding + reforços de token de slug/nome exato + um pequeno prior de popularidade).
- A relevância é mais forte que a popularidade. Uma correspondência precisa de token de slug ou nome de exibição pode superar uma correspondência mais ampla com engajamento muito maior.
- Texto ASCII é tokenizado em limites de palavras e pontuação. Por exemplo, `personal-map` contém um token `map` autônomo, enquanto `amap-jsapi-skill` contém `amap`, `jsapi` e `skill`; portanto, buscar `map` dá a `personal-map` uma correspondência lexical mais forte que `amap-jsapi-skill`.
- A popularidade é escalada logaritmicamente e limitada. Skills de alto engajamento podem ficar em posição inferior quando o texto da consulta tem uma correspondência mais fraca.
...
- Coloque os termos que os usuários literalmente buscarão no nome de exibição, no resumo e nas tags. Use um token de slug autônomo somente quando ele também for uma identidade estável que você deseja manter.
- Não renomeie um slug apenas para perseguir uma consulta, a menos que o novo slug seja um nome canônico melhor a longo prazo. Slugs antigos se tornam aliases de redirecionamento, mas a URL canônica, o slug exibido e resumos de busca futuros usam o novo slug.
- Aliases de renomeação preservam a resolução para URLs antigas e instalações que resolvem pelo registro, mas o ranqueamento da busca se baseia nos metadados canônicos da Skill depois que a renomeação foi indexada. Estatísticas existentes permanecem com a Skill.
- Se uma Skill estiver inesperadamente invisível, verifique primeiro o estado de moderação com `clawhub inspect @owner/slug` enquanto estiver conectado antes de alterar metadados relacionados ao ranqueamento.
...
Denuncie uma Skill para revisão de moderador. Denúncias são no nível da Skill, opcionalmente vinculadas
...
- Scans publicados exigem acesso de gerenciamento do proprietário/publicador, ou autoridade de moderador/administrador da plataforma.
...
- Trabalhos de scan são assíncronos. Requisições manuais de scan são priorizadas à frente do trabalho normal de publicação/preenchimento retroativo, mas a conclusão ainda depende da disponibilidade dos workers.
...
Rota canônica de novo scan em lote exclusiva para administradores. Ela aceita o mesmo formato de payload que a rota legada `POST /api/v1/skills/-/rescan-batch`.

### `POST /api/v1/skills/-/scan/batch/status`

Rota canônica de status em lote exclusiva para administradores. Ela aceita `{ "jobIds": ["..."] }` e retorna os mesmos contadores agregados que a rota legada `POST /api/v1/skills/-/rescan-batch/status`.
...
- A resposta é somente de segurança. Ela não inclui dados do Skill Card, status de cartão gerado, listas de arquivos de artefatos nem payloads detalhados de scanners.
- `security.signals` contém somente evidências de apoio no nível de status; use `/scan` ou a página de auditoria de segurança do ClawHub para detalhes completos dos scanners.
- `security.signals.dependencyRegistry` é mantido para compatibilidade de resposta v1, mas o scanner de existência do registro de dependências foi aposentado e esta chave é sempre `null`.
- A ausência do Skill Card não afeta `ok`, `decision` nem `reasons` deste endpoint; clientes devem ler o `skill-card.md` instalado localmente quando precisarem do conteúdo do cartão.
- Use `/verify` quando precisar do envelope de verificação do Skill Card de uma única skill, `/card` quando precisar do markdown do cartão gerado, e `/scan` quando precisar de dados detalhados de scanners.
...
- Entradas de Skill continuam baseadas no registro de Skills e ainda só podem ser publicadas por meio de `POST /api/v1/skills`.
...
`trending` é um ranking de instalações/downloads dos últimos sete dias e não usa totais históricos.
...
- Skills atuais baseadas no GitHub com uma verificação `clean` ou `suspicious` incluem
  `_source_handoff.json` com `sourceRef: "public-github"`, repo, commit, path,
...
Busca apenas de Plugins em pacotes code-plugin e bundle-plugin.
...
- Os aliases de filtro legados da v1 documentados em `GET /api/v1/plugins` também são
  aceitos.
- A filtragem por categoria é um filtro real de API apoiado por linhas de resumo de categoria
  de Plugin, não uma reescrita de consulta de busca.
...
- Requer um token de API do proprietário do pacote, de um proprietário/admin de editor de organização,
...
Retorna o resumo exato de segurança e confiança do lançamento do pacote para clientes
de instalação. Esta é a superfície pública de consumo do OpenClaw para decidir se um
lançamento resolvido pode ser instalado.

Autenticação:

- Endpoint de leitura pública. Nenhum token de proprietário, editor, moderador ou admin é
  obrigatório.
...
- `trust.blockedFromDownload` é o sinal de bloqueio de instalação. OpenClaw e outros
  ...
  derivar novamente as regras de bloqueio dos campos de scanner ou moderação.
...
- Pacotes privados retornam `404`, a menos que o chamador possa ler o editor proprietário.
- Este endpoint é intencionalmente mais restrito do que endpoints de moderação de
  proprietário/moderador. Ele expõe a decisão de instalação e a explicação pública, não
  identidades de relatores, corpos de relatório, evidências privadas ou cronogramas internos
  de revisão.
...
- Versões de pacote legadas retornam um artefato `legacy-zip` e um
...
- resumo do artefato
- proveniência de repositório de origem e commit
...
- `open`: lançamentos suspeitos, maliciosos, pendentes, em quarentena, revogados ou relatados.
...
Relata um pacote para revisão de moderador. Relatórios são em nível de pacote, opcionalmente
...
- `revoked`: bloqueado depois que um lançamento já havia sido considerado confiável.

Lançamentos em quarentena e revogados retornam `403` nas rotas de download de artefato.
...
- Transferências de skills baseadas no GitHub não fazem proxy nem espelham bytes. A resposta JSON
  inclui `sourceRef: "public-github"`, `repo`, `commit`, `path`, `contentHash`
  e `archiveUrl`; o estado da varredura/atual é um gate e não é incluído como metadados de payload
  de sucesso.
...
- Campo de payload opcional: `ownerHandle`. Quando presente, a API resolve esse
  publicador no servidor e exige que o ator tenha acesso ao publicador.
- Campo de payload opcional: `migrateOwner`. Quando `true` com `ownerHandle`, uma
  skill existente pode ser movida para esse proprietário se o ator for administrador/proprietário tanto no
...
- Os campos de formulário permitidos são `payload`, blobs `files` repetidos ou uma referência de tarball
  ...
  fluxo de upload-url. Publicações com id de armazenamento em staging também devem incluir o
...
- Solicitações de publicação multipart diretas são limitadas a 18MB. Tarballs ClawPack podem
  usar o fluxo de upload-url até o limite de tarball de 120MB.
- Campo de payload opcional: `ownerHandle`. Quando presente, somente administradores podem publicar em nome desse proprietário.
...
- Plugins de código exigem `package.json`, metadados de repositório de origem, metadados de commit
...
- Somente o publicador da org `openclaw` e os publicadores pessoais dos membros atuais da org `openclaw`
...
Criação autenticada e autônoma de publicador de org. Cria um novo publicador de org e adiciona o
chamador como proprietário. Este endpoint não migra handles de usuários/pessoais existentes e
...
Somente administrador. Reserva slugs raiz e nomes de pacote para um proprietário legítimo sem publicar um
lançamento. Nomes de pacote se tornam pacotes privados de placeholder sem linhas de lançamento, para que o mesmo
...
Somente administrador. Recupera um publicador pessoal para um principal substituto verificado do GitHub OAuth
sem editar linhas de conta do Convex Auth. A solicitação deve nomear ambos os ids imutáveis de conta
...
`confirmIdentityVerified: true` depois que a equipe verificar independentemente a continuidade entre os dois
...
aliases de slug de skill, pacotes, avisos do inspetor de pacotes e linhas derivadas de resumo de busca para que
...
para o identificador recuperado também é reatribuída ao usuário substituto para que sincronizações posteriores de perfil
não possam restaurar a autoridade concorrente do usuário anterior. Cada tabela primária é limitada a
...
Remover o banimento de um usuário e restaurar skills elegíveis (somente admin).
