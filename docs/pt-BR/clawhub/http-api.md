...
summary: Referência da API HTTP (pontos de extremidade públicos + pontos de extremidade da CLI + autenticação).
x-i18n:
    generated_at: "2026-07-01T18:08:57Z"
    model: gpt-5.5
...
- Se o token estiver ausente/inválido, o comportamento recorre à aplicação por IP.
...
- `X-RateLimit-Remaining` / `RateLimit-Remaining`: orçamento restante exato quando presente.
  Solicitações fragmentadas bem-sucedidas omitem este cabeçalho em vez de retornar um valor global aproximado.
...
- Texto ASCII é tokenizado em limites de palavras e pontuação. Por exemplo, `personal-map` contém um token `map` independente, enquanto `amap-jsapi-skill` contém `amap`, `jsapi` e `skill`; portanto, pesquisar por `map` dá a `personal-map` uma correspondência lexical mais forte do que `amap-jsapi-skill`.
...
Relata uma Skill para revisão por moderador. Relatórios são no nível da Skill, opcionalmente vinculados
...
- Trabalhos de scan são assíncronos. Requisições manuais de scan são priorizadas à frente do trabalho normal de publicação/backfill, mas a conclusão ainda depende da disponibilidade dos workers.

### `GET /api/v1/skills/-/scan/{scanId}`

Endpoint autenticado de polling para um scan enviado.

- Retorna status enfileirado/em execução/bem-sucedido/com falha.
- Retorna `queue.queuedAhead` e `queue.position` enquanto está enfileirado, para que clientes possam mostrar quantos scans manuais priorizados estão à frente da requisição. Filas muito grandes são limitadas e relatadas com `queuedAheadIsEstimate: true`.
...
- Retorna o mesmo formato ZIP que downloads de requisições de scan.

### `POST /api/v1/skills/-/scan/batch`

Rota canônica de novo scan em lote somente para admins. Ela aceita o mesmo formato de payload que a rota legada `POST /api/v1/skills/-/rescan-batch`.
...
- `security.signals.dependencyRegistry` é mantido para compatibilidade da resposta v1, mas o scanner de existência de registro de dependências foi desativado e esta chave é sempre `null`.
...
- A resposta é apenas de segurança. Ela não inclui dados do Skill Card, status do card gerado, listas de arquivos de artefato ou payloads detalhados de scanner.
- `security.signals` contém apenas evidências de suporte no nível de status; use `/scan` ou a página de auditoria de segurança do ClawHub para detalhes completos do scanner.
- `security.signals.dependencyRegistry` é mantido para compatibilidade da resposta v1, mas o scanner de existência de registro de dependências foi desativado e esta chave é sempre `null`.
...
- Use `/verify` quando precisar do envelope de verificação do Skill Card de uma única skill, `/card` quando precisar do markdown de card gerado e `/scan` quando precisar de dados detalhados do scanner.
...
- Entradas de Skills continuam baseadas no registro de Skills e ainda podem ser publicadas somente por meio de `POST /api/v1/skills`.
...
- `isOfficial` (opcional): `true` ou `false`
- `category` (opcional): filtro de categoria de Plugin. Compatível somente quando a
  solicitação está escopada para pacotes de Plugin. Categorias controladas e aliases legados v1
  de filtro estão documentados em `GET /api/v1/plugins`.
...
No endpoint unificado `/api/v1/packages`, ele é somente para Plugin; use
...
- `trust.blockedFromDownload` é o sinal de bloqueio de instalação. OpenClaw e outros
  ...
  rederivar regras de bloqueio dos campos de scanner ou moderação.
...
- Este endpoint é exato por versão. Os clientes devem chamá-lo depois de resolver a
  ...
  do pacote.
- Pacotes privados retornam `404`, a menos que o chamador possa ler o publisher proprietário.
- Este endpoint é intencionalmente mais estreito do que os endpoints de moderação de proprietário/moderador.
  Ele expõe a decisão de instalação e a explicação pública, não identidades de relatores,
  corpos de relatório, evidências privadas ou cronogramas de revisão internos.
...
- Esta é a superfície resolvedora do OpenClaw; ela evita inferir o formato de arquivo a partir
...
- Versões ClawPack transmitem os bytes `.tgz` exatos de npm-pack enviados.
...
Denuncia um pacote para revisão de moderador. Relatórios são no nível do pacote, opcionalmente
...
- `revoked`: bloqueado depois que uma versão foi considerada confiável anteriormente.
...
Baixa um ZIP de versão de Skill hospedada ou retorna um encaminhamento de origem do GitHub para uma
Skill atual baseada em GitHub com uma varredura `clean` ou `suspicious` e nenhuma versão
hospedada.
...
- Encaminhamentos de Skill baseada em GitHub não fazem proxy nem espelham bytes. A resposta JSON
  inclui `sourceRef: "public-github"`, `repo`, `commit`, `path`, `contentHash`
  e `archiveUrl`; o estado de varredura/atual é um gate e não é incluído como metadado
...
- Campo opcional do payload: `migrateOwner`. Quando `true` com `ownerHandle`, uma
  Skill existente pode passar para esse proprietário se o ator for administrador/proprietário tanto no
...
- Corpos JSON e metadados `payload.files` / `payload.artifact` fornecidos pelo chamador
  são rejeitados.
- Requisições de publicação multipart direta são limitadas a 18 MB. Tarballs ClawPack podem
...
`confirmIdentityVerified: true` depois que a equipe verificar de forma independente a continuidade entre ambos
os principais do GitHub. A recuperação falha fechada quando o publisher pessoal atual do usuário de destino
tem Skills, pacotes ou fontes de skill do GitHub.
A recuperação também migra campos legados `ownerUserId` para as Skills,
aliases de slug de skill, pacotes, avisos do inspetor de pacotes e linhas derivadas de resumo de pesquisa do publisher recuperado, para que
...
As fontes de skill do GitHub têm escopo de publisher e são relatadas como verificadas, em vez de reescritas.
