...
x-i18n:
    generated_at: "2026-07-03T09:26:10Z"
    model: gpt-5.5
...
- Os resultados são retornados em ordem de relevância (similaridade de embedding + reforços por tokens exatos de slug/nome + um pequeno prior de popularidade).
...
- Texto ASCII é tokenizado em limites de palavras e pontuação. Por exemplo, `personal-map` contém um token autônomo `map`, enquanto `amap-jsapi-skill` contém `amap`, `jsapi` e `skill`; portanto, buscar por `map` dá a `personal-map` uma correspondência lexical mais forte do que `amap-jsapi-skill`.
- A popularidade é escalada por log e limitada. Skills com alto engajamento podem ranquear mais baixo quando o texto da consulta é uma correspondência mais fraca.
...
- Coloque os termos que os usuários literalmente buscarão no nome de exibição, no resumo e nas tags. Use um token de slug autônomo somente quando ele também for uma identidade estável que você queira manter.
...
- Aliases de renomeação preservam a resolução para URLs antigas e instalações que resolvem pelo registro, mas o ranqueamento de busca se baseia nos metadados canônicos da Skill depois que a renomeação for indexada. Estatísticas existentes permanecem com a Skill.
...
Denuncie uma Skill para revisão por moderadores. Denúncias são no nível da Skill, opcionalmente vinculadas
...
- Jobs de scan são assíncronos. Requisições manuais de scan são priorizadas à frente do trabalho normal de publicação/backfill, mas a conclusão ainda depende da disponibilidade de workers.
...
- Retorna o mesmo formato de ZIP que os downloads de requisição de scan.
...
- A identidade da Skill, a identidade do publicador e os metadados da versão selecionada são campos de envelope de nível superior (`slug`, `displayName`, `publisherHandle`, `version`, `resolvedFrom`, `tag`, `createdAt`), para que automação de shell possa lê-los sem desempacotar wrappers aninhados.
- `security` é o veredito de nível superior de ClawScan/segurança. Automações devem se basear em `ok`, `decision`, `reasons` e `security.status`.
- `security.signals` contém evidências de suporte de scanners, como `staticScan`, `virusTotal` e `skillSpector`.
- `security.signals.dependencyRegistry` é mantido para compatibilidade de resposta v1, mas o scanner de existência no registro de dependências foi descontinuado e esta chave é sempre `null`.
...
- A resposta é somente de segurança. Ela não inclui dados do Skill Card, status de cartão gerado, listas de arquivos de artefato nem payloads detalhados de scanners.
- `security.signals` contém apenas evidências de suporte no nível de status; use `/scan` ou a página de auditoria de segurança do ClawHub para obter todos os detalhes dos scanners.
- `security.signals.dependencyRegistry` é mantido para compatibilidade de resposta v1, mas o scanner de existência no registro de dependências foi descontinuado e esta chave é sempre `null`.
...
- Use `/verify` quando precisar do envelope de verificação do Skill Card para uma única Skill, `/card` quando precisar do markdown do cartão gerado e `/scan` quando precisar de dados detalhados dos scanners.
...
`trending` é um placar de instalações/downloads de sete dias e não usa totais históricos.
...
- Skills atuais apoiadas pelo GitHub com uma varredura `clean` ou `suspicious` incluem
  ...
  hash de conteúdo e URL do arquivo. Elas não incluem arquivos-fonte hospedados pelo ClawHub.
...
Exportação em massa das releases públicas mais recentes de Plugin para análise offline.
...
- Corpo: arquivo ZIP.
- Cada Plugin exportado fica enraizado em `{family}/{packageName}/`.
...
- Os controles de ordenação da UI do navegador para busca de Plugin reordenam os resultados de relevância carregados,
...
- `version.artifact.kind` é `legacy-zip` para arquivos de pacote legados ou
...
Retorna o resumo exato de segurança e confiança da release do pacote para clientes de
...
- `trust.moderationState` é anulável. É `null` quando não existe moderação manual da release.
...
- Versões de pacote legadas retornam um artefato `legacy-zip` e uma URL de download ZIP legada
  `downloadUrl`.
...
- Esta é a superfície do resolvedor OpenClaw; ela evita adivinhar o formato do arquivo a partir
...
- `packageName` é normalizado como nome npm; o pacote pode estar ausente para migrações
  planejadas.
- Isso rastreia apenas a prontidão de migração. Não modifica o OpenClaw nem gera
  ClawPacks.

### `GET /api/v1/packages/moderation/queue`

Endpoint de moderador/administrador para filas de revisão de release de pacote.
...
- `revoked`: bloqueado depois que uma versão havia sido confiável anteriormente.
...
- Cabeçalhos de download incluem SHA-256 do ClawHub mais metadados integrity/shasum do npm.
...
skill atual baseada em GitHub com verificação `clean` ou `suspicious` e sem versão
hospedada.
...
- Transferências de skill baseadas em GitHub não fazem proxy nem espelham bytes. A resposta JSON
  ...
  e `archiveUrl`; o estado de verificação/atual é uma barreira e não é incluído como metadado
...
- Os campos de formulário permitidos são `payload`, blobs `files` repetidos ou uma referência de tarball
  ...
  fluxo de upload-url. Publicações preparadas por storage-id também devem incluir o
...
- Retorna `409` quando o handle já está em uso por um publicador, usuário ou publicador pessoal.
...
sem editar linhas de conta do Convex Auth. A requisição deve nomear ambos os ids de conta de provedor GitHub
imutáveis; handles mutáveis são usados apenas como uma proteção voltada ao operador.
...
tem skills, pacotes ou origens de skills do GitHub.
...
Origens de skills do GitHub têm escopo de publicador e são relatadas como verificadas, em vez de reescritas.
