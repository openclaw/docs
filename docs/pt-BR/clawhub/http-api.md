...
- Aliases de renomeação preservam a resolução para URLs antigas e instalações que resolvem pelo registro, mas o ranqueamento da pesquisa se baseia nos metadados canônicos da skill depois que a renomeação foi indexada. Estatísticas existentes permanecem com a skill.
...
- `recommended` usa sinais de engajamento e recenticidade.
...
Relate uma skill para revisão por moderador. Relatos são em nível de skill, opcionalmente vinculados
...
Endpoint autenticado de envio para novas tarefas ClawScan.
...
- Tarefas de varredura são assíncronas. Solicitações manuais de varredura são priorizadas à frente do trabalho normal de publicação/preenchimento retroativo, mas a conclusão ainda depende da disponibilidade de workers.
...
- Tarefas de varredura com falha retornam `status: "failed"` com `lastError`.
...
- Retorna o mesmo formato de ZIP dos downloads de solicitação de varredura.

### `POST /api/v1/skills/-/scan/batch`

Rota canônica de nova varredura em lote somente para administradores. Ela aceita o mesmo formato de payload do `POST /api/v1/skills/-/rescan-batch` legado.

### `POST /api/v1/skills/-/scan/batch/status`

Rota canônica de status de lote somente para administradores. Ela aceita `{ "jobIds": ["..."] }` e retorna os mesmos contadores agregados do `POST /api/v1/skills/-/rescan-batch/status` legado.
...
- `security.signals.dependencyRegistry` é mantido para compatibilidade de resposta v1, mas o scanner de existência de registro de dependências foi aposentado, e essa chave é sempre `null`.
...
- `security.signals` contém apenas evidências de suporte em nível de status; use `/scan` ou a página de auditoria de segurança do ClawHub para detalhes completos dos scanners.
- `security.signals.dependencyRegistry` é mantido para compatibilidade de resposta v1, mas o scanner de existência de registro de dependências foi aposentado, e essa chave é sempre `null`.
- A ausência do Skill Card não afeta o `ok`, `decision` ou `reasons` deste endpoint; clientes devem ler o `skill-card.md` instalado localmente quando precisarem do conteúdo do cartão.
- Use `/verify` quando precisar do envelope de verificação de Skill Card de uma única skill, `/card` quando precisar do markdown de cartão gerado, e `/scan` quando precisar de dados detalhados dos scanners.
...
- Entradas de Skill continuam tendo suporte do registro de Skills e ainda podem ser publicadas apenas por meio de `POST /api/v1/skills`.
...
- Skills atuais com suporte do GitHub com uma varredura `clean` ou `suspicious` incluem
...
- Os controles de ordenação da interface do navegador para busca de Plugins reordenam os resultados carregados por relevância,
...
- Requer um token de API para o proprietário do pacote, um proprietário/admin de publicador de organização,
...
- `packageName` é normalizado como nome npm; o pacote pode estar ausente para migrações
  planejadas.
- Isto acompanha apenas a prontidão de migração. Não altera o OpenClaw nem gera
  ClawPacks.

### `GET /api/v1/packages/moderation/queue`

Endpoint de moderador/admin para filas de revisão de lançamentos de pacote.
...
- Campo opcional de payload: `migrateOwner`. Quando `true` com `ownerHandle`, uma
  skill existente pode ser movida para esse proprietário se o ator for administrador/proprietário tanto nos
...
Ocultações de moderador/administrador e remoções de segurança não expiram dessa forma.
...
Somente administrador. Reserva slugs raiz e nomes de pacotes para um proprietário legítimo sem publicar uma
versão. Nomes de pacotes se tornam pacotes placeholder privados sem linhas de versão, para que o mesmo
proprietário possa publicar depois a versão code-plugin ou bundle-plugin real nesse nome.
...
provedor GitHub; identificadores mutáveis são usados somente como uma proteção voltada ao operador.
