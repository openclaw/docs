...
read_when:
    ...
    - Depurando solicitações CLI ↔ registro
...
- Endpoints de gravação autenticados não devem retornar um simples `Unauthorized` quando
...
- Os resultados são retornados em ordem de relevância (similaridade de embedding + reforços por token exato de slug/nome + uma pequena prioridade de popularidade).
...
- Coloque os termos que os usuários vão literalmente pesquisar no nome de exibição, resumo e tags. Use um token de slug independente somente quando ele também for uma identidade estável que você deseja manter.
- Não renomeie um slug apenas para perseguir uma consulta, a menos que o novo slug seja um nome canônico melhor a longo prazo. Slugs antigos se tornam aliases de redirecionamento, mas a URL canônica, o slug exibido e futuros resumos de busca usam o novo slug.
- Aliases de renomeação preservam a resolução para URLs antigas e instalações que resolvem pelo registro, mas o ranqueamento de busca é baseado nos metadados canônicos da Skill depois que a renomeação foi indexada. Estatísticas existentes permanecem com a Skill.
...
Reporte uma Skill para revisão de moderador. Relatórios são em nível de Skill, opcionalmente vinculados
...
triado para ocultar a Skill no mesmo fluxo auditável.
...
Retorna metadados de versão + lista de arquivos.
...
Scans de upload local não são mais compatíveis. Requisições que usam
...
- Jobs de scan são assíncronos. Requisições de scan manuais são priorizadas à frente do trabalho normal de publicação/backfill, mas a conclusão ainda depende da disponibilidade dos workers.
...
Rota canônica de rescan em lote exclusiva para admins. Ela aceita o mesmo formato de payload do legado `POST /api/v1/skills/-/rescan-batch`.

### `POST /api/v1/skills/-/scan/batch/status`

Rota canônica de status em lote exclusiva para admins. Ela aceita `{ "jobIds": ["..."] }` e retorna os mesmos contadores agregados do legado `POST /api/v1/skills/-/rescan-batch/status`.

### `GET /api/v1/skills/{slug}/verify`

Retorna o envelope de verificação do cartão de Skill usado por `clawhub skill verify`.
...
- `ok` é `true` somente quando a versão selecionada tem um cartão de Skill gerado, não está bloqueada como malware pela moderação e a verificação do ClawScan está limpa.
- Identidade da skill, identidade do publicador e metadados da versão selecionada são campos de envelope de nível superior (`slug`, `displayName`, `publisherHandle`, `version`, `resolvedFrom`, `tag`, `createdAt`) para que automação de shell possa lê-los sem desempacotar wrappers aninhados.
...
- `items` deve conter 1-100 pares únicos de `{ slug, version }`.
- Os resultados são por item; uma skill ou versão ausente não faz a resposta inteira falhar.
- A resposta é somente de segurança. Ela não inclui dados de cartão de Skill, status de cartão gerado, listas de arquivos de artefato ou payloads detalhados de scanner.
...
- `security.signals.dependencyRegistry` é mantido para compatibilidade de resposta v1, mas o scanner de existência do registro de dependências foi aposentado e esta chave é sempre `null`.
- A ausência de cartão de Skill não afeta `ok`, `decision` ou `reasons` deste endpoint; os clientes devem ler o `skill-card.md` instalado localmente quando precisarem do conteúdo do cartão.
- Use `/verify` quando precisar do envelope de verificação de cartão de Skill para uma única skill, `/card` quando precisar do markdown do cartão gerado e `/scan` quando precisar de dados detalhados do scanner.
...
- `POST /api/v1/packages` continua sendo apenas para lançamentos de code-plugin e bundle-plugin.
- Chamadores anônimos veem apenas canais públicos de pacote.
...
- Skills atuais com backend no GitHub com uma verificação `clean` ou `suspicious` incluem
...
Busca somente de Plugin em pacotes code-plugin e bundle-plugin.
...
- Os aliases de filtro v1 legados documentados em `GET /api/v1/plugins` também são
  aceitos.
- A filtragem por categoria é um filtro real de API respaldado por linhas de digest
...
- Exige um token de API do proprietário do pacote, de um proprietário/administrador do publicador da organização,
...
- `trust.moderationState` é anulável. Ele é `null` quando não há moderação manual da versão.
- `trust.blockedFromDownload` é o sinal de bloqueio de instalação. O OpenClaw e outros
  ...
  derivar novamente regras de bloqueio de campos de scanner ou moderação.
...
- Este endpoint é exato por versão. Clientes devem chamá-lo após resolver a
  versão do pacote que pretendem instalar, não apenas após ler os metadados
  mais recentes do pacote.
- Pacotes privados retornam `404`, a menos que o chamador possa ler o publicador proprietário.
- Este endpoint é intencionalmente mais estreito do que endpoints de moderação de proprietário/moderador.
...
- digest do artefato
- repositório de origem e procedência do commit
...
- `packageName` é normalizado como nome npm; o pacote pode estar ausente para migrações
  planejadas.
- Isto acompanha somente a prontidão da migração. Não modifica o OpenClaw nem gera
  ClawPacks.
...
Denuncia um pacote para revisão por moderador. As denúncias são em nível de pacote, opcionalmente
vinculadas a uma versão. Elas alimentam a fila de moderação, mas não ocultam automaticamente nem
bloqueiam downloads por si só; moderadores devem usar a moderação de versão para
...
- `revoked`: bloqueado depois que uma release foi anteriormente considerada confiável.
...
- Os campos de formulário permitidos são `payload`, blobs repetidos `files` ou uma referência de tarball
  ...
  fluxo de upload-url. Publicações de id de armazenamento preparado também devem incluir o
...
- Solicitações de publicação multipart direta são limitadas a 18 MB. Tarballs ClawPack podem
...
Exclusões lógicas iniciadas pelo proprietário reservam o slug por 30 dias; depois, o slug pode ser reivindicado por
...
usuário compartilhado legado/publicador pessoal, o endpoint primeiro o migra para um publicador de organização.
...
Criação autenticada por autoatendimento de publicador de organização. Cria um novo publicador de organização e adiciona o
...
release. Nomes de pacotes se tornam pacotes privados placeholder sem linhas de release, para que o mesmo
...
de perfil não consiga restaurar a autoridade concorrente do usuário anterior. Cada tabela primária é limitada a
100 linhas por transação de aplicação; recuperações maiores devem primeiro usar uma migração retomável de proprietário.
Fontes de habilidade do GitHub têm escopo de publicador e são relatadas como verificadas, em vez de reescritas.
