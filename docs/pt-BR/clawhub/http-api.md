...
x-i18n:
    generated_at: "2026-07-02T13:59:35Z"
    model: gpt-5.5
...
- Coloque os termos que os usuários pesquisarão literalmente no nome de exibição, resumo e tags. Use um token de slug autônomo apenas quando ele também for uma identidade estável que você deseja manter.
- Não renomeie um slug apenas para perseguir uma consulta, a menos que o novo slug seja um nome canônico melhor a longo prazo. Slugs antigos tornam-se aliases de redirecionamento, mas a URL canônica, o slug exibido e futuros resumos de pesquisa usam o novo slug.
...
- Quando `nonSuspiciousOnly=true`, ordenações baseadas em cursor podem retornar menos do que `limit` itens em uma página porque skills suspeitas são filtradas após a recuperação da página.
...
- `moderation` é um snapshot de moderação atual no nível do Skill derivado da versão mais recente.
...
- Trabalhos de scan são assíncronos. Solicitações manuais de scan são priorizadas antes do trabalho normal de publicação/preenchimento retroativo, mas a conclusão ainda depende da disponibilidade de workers.
...
- Retorna `queue.queuedAhead` e `queue.position` enquanto estiver em fila para que os clientes possam mostrar quantos scans manuais priorizados estão à frente da solicitação. Filas muito grandes são limitadas e relatadas com `queuedAheadIsEstimate: true`.
...
Rota canônica somente para administradores de novo scan em lote. Ela aceita o mesmo formato de payload que o legado `POST /api/v1/skills/-/rescan-batch`.

### `POST /api/v1/skills/-/scan/batch/status`

Rota canônica somente para administradores de status em lote. Ela aceita `{ "jobIds": ["..."] }` e retorna os mesmos contadores agregados que o legado `POST /api/v1/skills/-/rescan-batch/status`.
...
- A identidade do Skill, a identidade do publicador e os metadados da versão selecionada são campos de envelope de nível superior (`slug`, `displayName`, `publisherHandle`, `version`, `resolvedFrom`, `tag`, `createdAt`) para que automações de shell possam lê-los sem desempacotar wrappers aninhados.
...
- `security.signals.dependencyRegistry` é mantido para compatibilidade de resposta v1, mas o scanner de existência de registro de dependências foi desativado e esta chave é sempre `null`.
- `provenance` é `server-resolved-github-import` somente quando ClawHub resolveu e armazenou um repositório/ref/commit/caminho do GitHub durante publicação ou importação; caso contrário, é `unavailable`.
...
do ClawHub instaladas precisam exibir, como OpenClaw Control UI.
...
- A resposta é somente de segurança. Ela não inclui dados do Skill Card, status de card gerado, listas de arquivos de artefato ou payloads detalhados de scanner.
...
- `security.signals.dependencyRegistry` é mantido para compatibilidade de resposta v1, mas o scanner de existência de registro de dependências foi desativado e esta chave é sempre `null`.
- A ausência de Skill Card não afeta `ok`, `decision` ou `reasons` deste endpoint; os clientes devem ler o `skill-card.md` instalado localmente quando precisarem do conteúdo do card.
- Use `/verify` quando precisar do envelope de verificação de Skill Card de um único Skill, `/card` quando precisar de markdown de card gerado e `/scan` quando precisar de dados detalhados de scanner.
...
- As entradas de Skills continuam apoiadas pelo registro de Skills e ainda só podem ser publicadas por meio de `POST /api/v1/skills`.
...
Pesquisa somente de plugins em pacotes code-plugin e bundle-plugin.
...
- Requer um token de API para o proprietário do pacote, um proprietário/admin de editor de organização,
...
- `trust.moderationState` é anulável. Ele é `null` quando não existe
  moderação manual da versão.
...
- Este endpoint é exato por versão. Clientes devem chamá-lo depois de resolver a
  ...
  do pacote.
- Pacotes privados retornam `404`, a menos que o chamador possa ler o editor proprietário.
- Este endpoint é intencionalmente mais estreito do que endpoints de moderação de
  proprietário/moderador. Ele expõe a decisão de instalação e a explicação pública, não
  identidades de relatores, corpos de relatos, evidências privadas ou linhas do tempo internas de
  revisão.
...
- `manual`: qualquer versão com substituição manual de moderação.
- `all`: qualquer versão com substituição manual, estado de varredura não limpo ou relato de pacote.
...
Relata um pacote para revisão de moderador. Relatos são em nível de pacote, opcionalmente
...
- Usa o bucket de taxa de download.
- Cabeçalhos de download incluem SHA-256 do ClawHub mais metadados npm de integrity/shasum.
...
- Campos de formulário permitidos são `payload`, blobs `files` repetidos ou uma referência de tarball
  ...
  fluxo upload-url. Publicações preparadas por storage-id também devem incluir o
...
Exclusão reversível / restauração de uma skill (proprietário, moderador ou admin).
...
- Retorna `409` quando o identificador já está usado por um publicador, usuário ou publicador pessoal.
...
aliases de slug de skill, packages, avisos do inspetor de package e linhas derivadas de resumo de busca do publisher recuperado, para que
...
não possa restaurar a autoridade concorrente do antigo usuário. Cada tabela primária é limitada a
100 linhas por transação de aplicação; recuperações maiores devem primeiro usar uma migração de proprietário retomável.
Fontes de skill do GitHub têm escopo por publisher e são relatadas como verificadas, em vez de reescritas.
...
Se você fizer self-host, sirva este arquivo (ou defina `CLAWHUB_REGISTRY` explicitamente; legado `CLAWDHUB_REGISTRY`).
