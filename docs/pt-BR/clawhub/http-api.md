...
- Os resultados são retornados em ordem de relevância (similaridade de embedding + reforços de token exato de slug/nome + uma pequena prioridade de popularidade).
- A relevância é mais forte que a popularidade. Uma correspondência precisa de slug ou token de nome de exibição pode ficar acima de uma correspondência mais ampla com engajamento muito maior.
- Texto ASCII é tokenizado em limites de palavras e pontuação. Por exemplo, `personal-map` contém um token independente `map`, enquanto `amap-jsapi-skill` contém `amap`, `jsapi` e `skill`; portanto, pesquisar por `map` dá a `personal-map` uma correspondência lexical mais forte que `amap-jsapi-skill`.
- A popularidade é escalada logaritmicamente e limitada. Skills com alto engajamento podem ficar abaixo quando o texto da consulta for uma correspondência mais fraca.
...
- Coloque os termos que os usuários pesquisarão literalmente no nome de exibição, resumo e tags. Use um token de slug independente somente quando ele também for uma identidade estável que você deseja manter.
- Não renomeie um slug apenas para perseguir uma consulta, a menos que o novo slug seja um nome canônico melhor a longo prazo. Slugs antigos se tornam aliases de redirecionamento, mas a URL canônica, o slug exibido e os resumos de pesquisa futuros usam o novo slug.
- Aliases de renomeação preservam a resolução para URLs antigas e instalações que resolvem por meio do registro, mas a classificação de pesquisa se baseia nos metadados canônicos da Skill após a renomeação ser indexada. Estatísticas existentes permanecem com a Skill.
...
Valores de `sort` inválidos retornam `400`.
...
Reporte uma Skill para análise de moderador. Relatórios são no nível da Skill, opcionalmente vinculados
...
Endpoint de moderador/administrador para recebimento de relatórios de Skills.
...
definir `status` de volta para `open`. Passe `finalAction: "hide"` com um relatório triado
...
- Retorna `queue.queuedAhead` e `queue.position` enquanto estiver na fila para que os clientes possam mostrar quantos scans manuais priorizados estão à frente da requisição. Filas muito grandes são limitadas e relatadas com `queuedAheadIsEstimate: true`.
...
- Retorna o mesmo formato ZIP dos downloads de requisições de scan.

### `POST /api/v1/skills/-/scan/batch`

Rota canônica de rescan em lote somente para admins. Ela aceita o mesmo formato de payload que a rota legada `POST /api/v1/skills/-/rescan-batch`.
...
- `ok` é `true` somente quando a versão selecionada tem um Skill Card gerado, não está bloqueada como malware pela moderação e a verificação ClawScan está limpa.
...
- `security` é o veredito de segurança/ClawScan de nível superior. Automações devem se basear em `ok`, `decision`, `reasons` e `security.status`.
- `security.signals` contém evidências de scanner de suporte, como `staticScan`, `virusTotal` e `skillSpector`.
- `security.signals.dependencyRegistry` é mantido por compatibilidade com respostas v1, mas o scanner de existência do registro de dependências foi aposentado e esta chave é sempre `null`.
...
Retorna vereditos compactos atuais de segurança para versões exatas de skills. Este
...
- A resposta é somente de segurança. Ela não inclui dados do Skill Card, status do card gerado, listas de arquivos de artefato ou payloads detalhados de scanner.
- `security.signals` contém apenas evidência de suporte em nível de status; use `/scan` ou a página de auditoria de segurança do ClawHub para detalhes completos do scanner.
- `security.signals.dependencyRegistry` é mantido por compatibilidade com respostas v1, mas o scanner de existência do registro de dependências foi aposentado e esta chave é sempre `null`.
...
- Use `/verify` quando precisar do envelope de verificação do Skill Card de uma única skill, `/card` quando precisar do markdown do card gerado e `/scan` quando precisar de dados detalhados do scanner.
...
- Entradas de Skill continuam apoiadas pelo registro de Skills e ainda só podem ser publicadas por meio de `POST /api/v1/skills`.
...
Navegação de catálogo somente de Plugin em pacotes code-plugin e bundle-plugin.
...
Exportação em massa dos lançamentos públicos mais recentes de plugins para análise offline.
...
Busca exclusiva de plugins em pacotes code-plugin e bundle-plugin.
...
- Requer um token de API do proprietário do pacote, de um proprietário/admin de publisher de organização,
...
Retorna o resumo exato de segurança e confiança do lançamento do pacote para clientes de instalação. Esta é a superfície pública de consumo do OpenClaw para decidir se um
...
- `trust.moderationState` é anulável. Ele é `null` quando não existe moderação manual de lançamento.
...
- Este endpoint é intencionalmente mais restrito do que endpoints de moderação de proprietário/moderador.
  ...
  identidades de denunciantes, corpos de relatos, evidências privadas ou cronogramas internos de revisão.
...
- Versões de pacote legadas retornam um artefato `legacy-zip` e uma URL legada de ZIP
  `downloadUrl`.
...
- Versões do ClawPack transmitem os bytes `.tgz` exatos de npm-pack enviados.
...
Endpoint de moderador/admin para filas de revisão de lançamento de pacote.
...
bloqueiam downloads automaticamente por si só; moderadores devem usar a moderação de lançamento para
...
Ponto de extremidade de moderador/admin para recebimento de relatórios de pacote.
...
Ponto de extremidade de proprietário/moderador para visibilidade da moderação de pacote.
...
Ponto de extremidade de moderador/admin para resolver ou reabrir relatórios de pacote.
...
Ponto de extremidade de moderador/admin para revisão de lançamento de pacote.
...
Lançamentos em quarentena e revogados retornam `403` das rotas de download de artefato.
...
skill atual baseada em GitHub com uma verificação `clean` ou `suspicious` e nenhuma versão
hospedada.
...
- Transferências de skill baseada em GitHub não fazem proxy nem espelham bytes. A resposta JSON
  inclui `sourceRef: "public-github"`, `repo`, `commit`, `path`, `contentHash`
  e `archiveUrl`; o estado de verificação/atual é uma barreira e não é incluído como metadado de payload
  de sucesso.
...
## Pontos de extremidade de autenticação (token Bearer)

Todos os pontos de extremidade exigem:
...
- Campo opcional de payload: `migrateOwner`. Quando `true` com `ownerHandle`, uma
  skill existente pode passar para esse proprietário se o ator for admin/proprietário tanto nos
  publicadores atual quanto de destino. Sem essa opção explícita, alterações de proprietário são
  rejeitadas.
...
usuário/publicador pessoal compartilhado legado, o ponto de extremidade primeiro o migra para um publicador de org.
...
Criação autenticada de publicador de org por autoatendimento. Cria um novo publicador de org e adiciona o
chamador como proprietário. Este ponto de extremidade não migra identificadores existentes de usuário/pessoal e não
...
Somente admin. Reserva slugs raiz e nomes de pacote para um proprietário legítimo sem publicar um
lançamento. Nomes de pacote se tornam pacotes privados placeholder sem linhas de lançamento, para que o mesmo
...
do provedor GitHub; identificadores mutáveis são usados somente como proteção voltada ao operador.

O endpoint usa execução de teste por padrão. Aplicar a recuperação exige `dryRun: false` e
`confirmIdentityVerified: true` depois que a equipe verifica independentemente a continuidade entre os dois
principais do GitHub. A recuperação falha em modo fechado quando o editor pessoal atual do usuário de destino
...
aliases de slug de Skills, pacotes, avisos do inspetor de pacotes e linhas derivadas de resumo de busca para que
os caminhos de proprietário direto concordem com a nova autoridade do editor. Uma reserva ativa de handle protegido
para o handle recuperado também é reatribuída ao usuário substituto para que uma sincronização de perfil posterior
...
Banir um usuário e excluir definitivamente Skills próprias (somente moderador/admin).
...
conteúdo (somente admin). Usa execução de teste por padrão, a menos que `dryRun` seja `false`.
