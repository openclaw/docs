...
x-i18n:
    generated_at: "2026-07-03T23:28:11Z"
    model: gpt-5.5
...
- Se nenhum IP confiável do cliente estiver disponível, requisições anônimas usam buckets de fallback
  definidos apenas pelo tipo de limite de taxa. Esses buckets de fallback não incluem
  caminhos, slugs, nomes de pacote, versões, strings de consulta nem outros
  parâmetros de artefato fornecidos pelo chamador.

## Respostas de erro

As respostas públicas de erro v1 são texto simples com `content-type: text/plain; charset=utf-8`.
...
- Os resultados são retornados em ordem de relevância (similaridade de embedding + reforços por correspondência exata de token de slug/nome + um pequeno prior de popularidade).
- A relevância é mais forte que a popularidade. Uma correspondência precisa de token de slug ou nome de exibição pode superar uma correspondência mais frouxa com engajamento muito mais forte.
- Texto ASCII é tokenizado em limites de palavras e pontuação. Por exemplo, `personal-map` contém um token independente `map`, enquanto `amap-jsapi-skill` contém `amap`, `jsapi` e `skill`; pesquisar por `map`, portanto, dá a `personal-map` uma correspondência lexical mais forte do que `amap-jsapi-skill`.
- A popularidade é escalada por log e limitada. Skills com alto engajamento podem ficar abaixo quando o texto da consulta tem uma correspondência mais fraca.
...
- Coloque os termos que os usuários pesquisarão literalmente no nome de exibição, no resumo e nas tags. Use um token de slug independente somente quando ele também for uma identidade estável que você deseja manter.
- Não renomeie um slug apenas para perseguir uma consulta, a menos que o novo slug seja um nome canônico melhor a longo prazo. Slugs antigos viram aliases de redirecionamento, mas a URL canônica, o slug exibido e futuros resumos de busca usam o novo slug.
- Aliases de renomeação preservam a resolução de URLs antigas e instalações que resolvem pelo registro, mas o ranqueamento de busca é baseado nos metadados canônicos da Skill depois que a renomeação for indexada. As estatísticas existentes permanecem com a Skill.
...
- `createdAt` é estável para crawls de Skills novas; `updated` muda quando Skills existentes são republicadas.
...
- Evidências são redigidas para chamadores públicos e incluem snippets brutos apenas para proprietários/moderadores.
...
Retorna detalhes de verificação da varredura de segurança de uma versão da Skill.
...
Scans de upload local não são mais compatíveis. Requisições usando
...
- Scans publicados exigem acesso de gerenciamento de proprietário/publicador, ou autoridade de moderador/admin da plataforma.
...
- Jobs de scan são assíncronos. Requisições manuais de scan são priorizadas antes do trabalho normal de publicação/backfill, mas a conclusão ainda depende da disponibilidade de workers.
...
- Retorna `queue.queuedAhead` e `queue.position` enquanto estiver na fila, para que clientes possam mostrar quantos scans manuais priorizados estão à frente da requisição. Filas muito grandes são limitadas e reportadas com `queuedAheadIsEstimate: true`.
...
- Exige acesso de gerenciamento de proprietário/publicador à skill ou ao plugin, ou autoridade de moderador/admin da plataforma.
- Retorna resultados de scan armazenados para a versão enviada exata, incluindo versões bloqueadas ou ocultas.
- `kind` usa `skill` por padrão; use `kind=plugin` para scans de plugin/pacote.
- Retorna o mesmo formato de ZIP dos downloads de requisições de scan.

### `POST /api/v1/skills/-/scan/batch`

Rota canônica de rescan em lote somente para admins. Ela aceita o mesmo formato de payload que o legado `POST /api/v1/skills/-/rescan-batch`.

### `POST /api/v1/skills/-/scan/batch/status`

Rota canônica de status de lote somente para admins. Ela aceita `{ "jobIds": ["..."] }` e retorna os mesmos contadores agregados que o legado `POST /api/v1/skills/-/rescan-batch/status`.
...
- Identidade da skill, identidade do publicador e metadados da versão selecionada são campos de envelope de nível superior (`slug`, `displayName`, `publisherHandle`, `version`, `resolvedFrom`, `tag`, `createdAt`) para que automações de shell possam lê-los sem descompactar wrappers aninhados.
- `security` é o veredito ClawScan/segurança de nível superior. Automações devem se basear em `ok`, `decision`, `reasons` e `security.status`.
...
- `provenance` é `server-resolved-github-import` somente quando o ClawHub resolveu e armazenou um repo/ref/commit/path do GitHub durante a publicação ou importação; caso contrário, é `unavailable`.

### `POST /api/v1/skills/-/security-verdicts`

Retorna vereditos de segurança compactos atuais para versões exatas de skills. Este
endpoint de coleção é destinado a clientes que já sabem quais versões de skills do
ClawHub instaladas precisam exibir, como a OpenClaw Control UI.
...
- A resposta é somente de segurança. Ela não inclui dados do Skill Card, status do cartão gerado, listas de arquivos de artefato ou payloads detalhados de scanner.
- `security.signals` contém somente evidências de apoio em nível de status; use `/scan` ou a página de auditoria de segurança do ClawHub para detalhes completos do scanner.
- `security.signals.dependencyRegistry` é mantido para compatibilidade da resposta v1, mas o scanner de existência no registro de dependências foi aposentado e esta chave é sempre `null`.
- A ausência do Skill Card não afeta `ok`, `decision` ou `reasons` deste endpoint; clientes devem ler o `skill-card.md` instalado localmente quando precisarem do conteúdo do cartão.
- Use `/verify` quando precisar do envelope de verificação do Skill Card de uma única skill, `/card` quando precisar do markdown do cartão gerado, e `/scan` quando precisar de dados detalhados de scanner.
...
- Chamadores autenticados podem ver pacotes privados de publicadores aos quais pertencem em resultados de listagem/busca.
...
Navegação de catálogo somente de Plugin entre pacotes code-plugin e bundle-plugin.
...
Aliases de filtro legados da v1 continuam aceitos nos endpoints de leitura:
...
`trending` é uma classificação de instalações/downloads de sete dias e não usa totais de todo o período.
...
Pesquisa somente de Plugin em pacotes code-plugin e bundle-plugin.
...
- Os aliases de filtro legados da v1 documentados em `GET /api/v1/plugins` também são
  aceitos.
- A filtragem por categoria é um filtro de API real respaldado por linhas de resumo
  de categoria de Plugin, não uma reescrita da consulta de pesquisa.
- Os resultados são retornados em ordem de relevância e atualmente não são paginados.
- Os controles de ordenação da interface do navegador para pesquisa de Plugin reordenam os resultados de relevância carregados,
...
- Exige um token de API do proprietário do pacote, de um proprietário/administrador do editor da organização,
...
- Lançamentos ClawPack incluem os campos compatíveis com npm `npmIntegrity`, `npmShasum` e
  `npmTarballName`.
- `version.sha256hash` é metadado de compatibilidade obsoleto para clientes antigos. Ele
  gera hash dos bytes exatos do ZIP retornados por `/api/v1/packages/{name}/download`.
  Clientes modernos devem usar `version.artifact.sha256`, que identifica o
  artefato de lançamento canônico.
...
- Endpoint de leitura público. Nenhum token de proprietário, editor, moderador ou administrador é
  necessário.
...
- `release.artifactKind`, `release.artifactSha256`, `release.npmIntegrity`,
  ...
  o artefato de lançamento.
- `trust.scanStatus` é o status de confiança efetivo derivado de entradas de scanner
...
- `trust.blockedFromDownload` é o sinal de bloqueio de instalação. OpenClaw e outros
  ...
  derivar novamente as regras de bloqueio a partir dos campos de scanner ou moderação.
- `trust.reasons` é a lista de explicações voltada ao usuário e à auditoria. Códigos de motivo
...
- Este endpoint é intencionalmente mais restrito que os endpoints de moderação de
  proprietário/moderador. Ele expõe a decisão de instalação e a explicação pública, não
  identidades de denunciantes, corpos de relatórios, evidências privadas ou cronogramas internos
  de revisão.
...
- Versões ClawPack retornam um artefato `npm-pack`, campos de integridade npm, uma
  `tarballUrl` e a URL de compatibilidade de ZIP legado.
- Esta é a superfície do resolvedor OpenClaw; ela evita inferir o formato do arquivo
...
- Versões ClawPack transmitem os bytes exatos `.tgz` de npm-pack enviados.
- Versões de ZIP legado redirecionam para `/api/v1/packages/{name}/download?version=`.
...
- Requer um token de API do proprietário do pacote, membro publicador, moderador ou
  usuário admin.
...
- `revoked`: bloqueado depois que uma versão foi previamente confiável.

Versões em quarentena e revogadas retornam `403` nas rotas de download de artefatos.
Toda alteração grava uma entrada de log de auditoria.
...
- Esta rota permanece somente ZIP. Ela não transmite arquivos `.tgz` do ClawPack.
...
- `dist.tarball`, `dist.integrity` e `dist.shasum` usam campos compatíveis com npm
  para que os usuários possam apontar npm para o espelho se escolherem.
...
- Usa o bucket de taxa de download.
- Os cabeçalhos de download incluem o SHA-256 do ClawHub mais metadados de integridade/shasum do npm.
...
Baixa um ZIP de versão de skill hospedado ou retorna uma transferência de origem do GitHub para uma
skill atual com backend no GitHub com uma varredura `clean` ou `suspicious` e sem versão
hospedada.
...
- Transferências de skills com backend no GitHub não fazem proxy nem espelham bytes. A resposta JSON
...
- Preferencial: `multipart/form-data` com JSON em `payload` + blobs `files[]`.
...
- Campo opcional do payload: `ownerHandle`. Quando presente, a API resolve esse
  publicador no lado do servidor e exige que o ator tenha acesso de publicador.
- Campo opcional do payload: `migrateOwner`. Quando `true` com `ownerHandle`, uma
  ...
  publicador atual quanto no de destino. Sem essa adesão explícita, alterações de proprietário são
  rejeitadas.
...
- Solicitações diretas de publicação multipart são limitadas a 18 MB. Tarballs ClawPack podem
  usar o fluxo de URL de upload até o limite de tarball de 120 MB.
...
- Code plugins exigem `package.json`, metadados do repositório de origem, metadados do commit
  de origem, metadados de esquema de configuração, `openclaw.compat.pluginApi` e
  `openclaw.build.openclawVersion`.
- `openclaw.hostTargets` e `openclaw.environment` são metadados opcionais.
- Somente o publicador da org `openclaw` e os publicadores pessoais de membros atuais da org
  `openclaw` podem publicar no canal `official`.
...
Somente para admin. Garante que um publicador de org exista para um identificador. Se o identificador ainda apontar para um
publicador legado compartilhado de usuário/pessoal, o endpoint primeiro o migrará para um publicador de org.
...
Criação autenticada self-service de publicador de org. Cria um novo publicador de org e adiciona o
...
Somente para admin. Reserva slugs raiz e nomes de pacotes para um proprietário legítimo sem publicar uma
versão. Nomes de pacote se tornam pacotes reservados privados sem linhas de versão, para que o mesmo
...
Somente para admin. Recupera um publicador pessoal para um principal substituto verificado do GitHub OAuth
...
O endpoint usa dry-run por padrão. Aplicar a recuperação requer `dryRun: false` e
`confirmIdentityVerified: true` depois que a equipe verificar independentemente a continuidade entre ambos os
principais do GitHub. A recuperação falha de forma fechada quando o editor pessoal atual do usuário de destino
tem Skills, pacotes ou fontes de Skills do GitHub.
A recuperação também migra campos legados `ownerUserId` para as Skills do editor recuperado,
aliases de slug de Skill, pacotes, avisos do inspetor de pacotes e linhas derivadas do digest de busca, para que
os caminhos de proprietário direto concordem com a nova autoridade do editor. Uma reserva ativa de handle protegido
para o handle recuperado também é reatribuída ao usuário substituto para que a sincronização de perfil posterior
...
Fontes de Skills do GitHub têm escopo de editor e são relatadas como verificadas, em vez de reescritas.
