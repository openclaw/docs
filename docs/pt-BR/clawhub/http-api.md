...
x-i18n:
    generated_at: "2026-07-03T02:44:11Z"
    model: gpt-5.5
...
- Endpoints autenticados de escrita não devem retornar um `Unauthorized` simples quando
  o servidor sabe o motivo. Tokens ausentes, tokens inválidos/revogados e contas
  excluídas/banidas/desativadas devem receber texto acionável para que clientes de CLI
  possam informar aos usuários o que os bloqueou.
...
- Se `Retry-After` estiver ausente, faça fallback para `RateLimit-Reset` (ou calcule a partir de `X-RateLimit-Reset`).
...
- A relevância é mais forte do que a popularidade. Uma correspondência precisa de token de slug ou nome de exibição pode superar uma correspondência mais vaga com engajamento muito maior.
...
- A popularidade é escalonada em log e limitada. Skills com alto engajamento podem ficar em posição inferior quando o texto da consulta é uma correspondência mais fraca.
...
- Coloque os termos que os usuários pesquisarão literalmente no nome de exibição, no resumo e nas tags. Use um token de slug autônomo somente quando ele também for uma identidade estável que você deseja manter.
- Não renomeie um slug apenas para perseguir uma consulta, a menos que o novo slug seja um nome canônico melhor de longo prazo. Slugs antigos se tornam aliases de redirecionamento, mas a URL canônica, o slug exibido e os futuros resumos de pesquisa usam o novo slug.
- Aliases de renomeação preservam a resolução para URLs antigas e instalações que resolvem por meio do registro, mas o ranqueamento de pesquisa é baseado nos metadados canônicos da Skill depois que a renomeação é indexada. Estatísticas existentes permanecem com a Skill.
...
Reporte uma Skill para análise por moderadores. Relatórios são no nível da Skill, opcionalmente vinculados
...
- Trabalhos de scan são assíncronos. Requisições manuais de scan são priorizadas antes do trabalho normal de publicação/backfill, mas a conclusão ainda depende da disponibilidade dos workers.
...
Rota canônica de novo scan em lote exclusiva para administradores. Ela aceita o mesmo formato de payload do legado `POST /api/v1/skills/-/rescan-batch`.

### `POST /api/v1/skills/-/scan/batch/status`

Rota canônica de status de lote exclusiva para administradores. Ela aceita `{ "jobIds": ["..."] }` e retorna os mesmos contadores agregados do legado `POST /api/v1/skills/-/rescan-batch/status`.
...
- Skills atuais baseadas no GitHub com uma varredura `clean` ou `suspicious` incluem
  ...
  hash do conteúdo e URL do arquivo. Elas não incluem arquivos-fonte hospedados pelo ClawHub.
...
- Os aliases de filtros legados da v1 documentados em `GET /api/v1/plugins` também são
  aceitos.
- A filtragem por categoria é um filtro real de API respaldado por linhas de resumo
...
- Versões do ClawPack incluem campos compatíveis com npm `npmIntegrity`, `npmShasum` e
  `npmTarballName`.
...
- Este endpoint é intencionalmente mais restrito que os endpoints de moderação de
  ...
  identidades de relatores, corpos de relatórios, evidências privadas ou cronogramas
  internos de revisão.
...
- Versões do ClawPack transmitem os bytes exatos `.tgz` npm-pack enviados.
...
- resumo do artefato
- proveniência do repositório-fonte e do commit
...
`finalAction: "revoke"` com um relatório confirmado para aplicar moderação da release no
...
- `quarantined`: bloqueada aguardando acompanhamento.
- `revoked`: bloqueada após uma release ter sido previamente confiável.
...
skill atual baseada no GitHub com verificação `clean` ou `suspicious` e sem versão
hospedada.
...
- Campo opcional do payload: `ownerHandle`. Quando presente, a API resolve esse
  publicador no servidor e exige que o ator tenha acesso de publicador.
- Campo opcional do payload: `migrateOwner`. Quando `true` com `ownerHandle`, uma
  ...
  os publicadores atual e de destino. Sem esta adesão explícita, alterações de proprietário são
  rejeitadas.
...
- Campos de formulário permitidos são `payload`, blobs `files` repetidos ou uma referência a tarball
...
- Somente o publicador da organização `openclaw` e os publicadores pessoais de membros atuais da organização `openclaw`
...
`confirmIdentityVerified: true` depois que a equipe verifica independentemente a continuidade entre ambos os
principais do GitHub. A recuperação falha de forma fechada quando o publisher pessoal atual do usuário de destino
tem skills, packages ou fontes de skill do GitHub.
A recuperação também migra campos legados `ownerUserId` das skills do publisher recuperado,
aliases de slug de skill, packages, avisos do inspetor de package e linhas derivadas de resumo de busca para que
os caminhos de proprietário direto concordem com a nova autoridade do publisher. Uma reserva ativa de handle protegido
...
Ainda há suporte para versões antigas da CLI:
...
## Descoberta do Registry (`/.well-known/clawhub.json`)
