...
x-i18n:
    generated_at: "2026-07-01T20:14:45Z"
    model: gpt-5.5
...
- Os resultados são retornados em ordem de relevância (similaridade de embeddings + reforços de token exato de slug/nome + um pequeno fator prévio de popularidade).
- A relevância é mais forte que a popularidade. Uma correspondência precisa de slug ou token de nome exibido pode superar uma correspondência mais vaga com engajamento muito maior.
- Texto ASCII é tokenizado em limites de palavras e pontuação. Por exemplo, `personal-map` contém um token independente `map`, enquanto `amap-jsapi-skill` contém `amap`, `jsapi` e `skill`; portanto, pesquisar por `map` dá a `personal-map` uma correspondência lexical mais forte do que `amap-jsapi-skill`.
- A popularidade é escalada em log e limitada. Skills de alto engajamento podem ranquear abaixo quando o texto da consulta é uma correspondência mais fraca.
...
- Coloque os termos que os usuários pesquisarão literalmente no nome exibido, no resumo e nas tags. Use um token de slug independente somente quando ele também for uma identidade estável que você deseja manter.
- Não renomeie um slug apenas para perseguir uma consulta, a menos que o novo slug seja um nome canônico melhor a longo prazo. Slugs antigos tornam-se aliases de redirecionamento, mas a URL canônica, o slug exibido e os futuros resumos de pesquisa usam o novo slug.
- Aliases de renomeação preservam a resolução para URLs antigas e instalações que resolvem pelo registro, mas o ranqueamento de pesquisa se baseia nos metadados canônicos da Skill depois que a renomeação tiver sido indexada. Estatísticas existentes permanecem com a Skill.
...
- Slugs antigos criados por fluxos de renomeação/fusão de proprietário resolvem para a Skill canônica.
...
Denuncie uma Skill para análise de moderador. Denúncias são no nível da Skill, opcionalmente vinculadas
...
Endpoint de moderador/administrador para entrada de denúncias de Skills.
...
Varreduras de uploads locais não são mais compatíveis. Requisições usando
...
- Trabalhos de varredura são assíncronos. Requisições manuais de varredura são priorizadas à frente do trabalho normal de publicação/preenchimento retroativo, mas a conclusão ainda depende da disponibilidade dos workers.
...
- Retorna `queue.queuedAhead` e `queue.position` enquanto estiver na fila, para que os clientes possam mostrar quantas varreduras manuais priorizadas estão à frente da requisição. Filas muito grandes são limitadas e relatadas com `queuedAheadIsEstimate: true`.
...
- Retorna o mesmo formato ZIP dos downloads de requisições de varredura.

### `POST /api/v1/skills/-/scan/batch`

Rota canônica somente para administradores de nova varredura em lote. Ela aceita o mesmo formato de payload que o legado `POST /api/v1/skills/-/rescan-batch`.

### `POST /api/v1/skills/-/scan/batch/status`

Rota canônica somente para administradores de status de lote. Ela aceita `{ "jobIds": ["..."] }` e retorna os mesmos contadores agregados que o legado `POST /api/v1/skills/-/rescan-batch/status`.
...
- A identidade da skill, a identidade do publicador e os metadados da versão selecionada são campos de nível superior do envelope (`slug`, `displayName`, `publisherHandle`, `version`, `resolvedFrom`, `tag`, `createdAt`) para que automações de shell possam lê-los sem desembrulhar wrappers aninhados.
...
- `security.signals.dependencyRegistry` é mantido para compatibilidade de resposta v1, mas o scanner de existência de registro de dependências foi descontinuado e esta chave é sempre `null`.
...
- A resposta é somente de segurança. Ela não inclui dados de Skill Card, status de cartão gerado, listas de arquivos de artefato ou payloads detalhados de scanner.
- `security.signals` contém apenas evidências de apoio em nível de status; use `/scan` ou a página de auditoria de segurança do ClawHub para detalhes completos do scanner.
- `security.signals.dependencyRegistry` é mantido para compatibilidade de resposta v1, mas o scanner de existência de registro de dependências foi descontinuado e esta chave é sempre `null`.
- A ausência de Skill Card não afeta `ok`, `decision` ou `reasons` deste endpoint; clientes devem ler o `skill-card.md` instalado localmente quando precisarem do conteúdo do cartão.
- Use `/verify` quando precisar do envelope de verificação de Skill Card de uma única skill, `/card` quando precisar do markdown do cartão gerado e `/scan` quando precisar de dados detalhados de scanner.
...
- As entradas de skill continuam apoiadas pelo registro de skills e ainda podem ser publicadas apenas por meio de `POST /api/v1/skills`.
...
Aliases de filtro legados da v1 continuam sendo aceitos em endpoints de leitura:
...
Exportação em massa das releases públicas mais recentes de Plugins para análise offline.
...
- Os aliases de filtro legados da v1 documentados em `GET /api/v1/plugins` também são
  aceitos.
- A filtragem por categoria é um filtro real de API baseado em linhas de resumo de
...
- Exige um token de API do proprietário do pacote, de um proprietário/admin do publisher da organização,
  de um moderador da plataforma ou de um admin da plataforma.
...
- Endpoint público de leitura. Nenhum token de proprietário, publisher, moderador ou admin é
  necessário.
...
- `trust.moderationState` pode ser nulo. Ele é `null` quando não existe moderação
  manual da release.
- `trust.blockedFromDownload` é o sinal de bloqueio de instalação. O OpenClaw e outros
  ...
  recalcular regras de bloqueio a partir de campos de scanner ou moderação.
...
- Este endpoint é intencionalmente mais restrito que endpoints de moderação de
  ...
  identidades de denunciantes, corpos de denúncias, evidências privadas ou cronologias internas de revisão.
...
- Versões de pacote legadas retornam um artefato `legacy-zip` e um `downloadUrl`
  de ZIP legado.
- Versões do ClawPack retornam um artefato `npm-pack`, campos de integridade npm, uma
  `tarballUrl` e a URL de compatibilidade do ZIP legado.
...
- Versões do ClawPack transmitem os bytes exatos `.tgz` de `npm-pack` enviados.
...
- Exige um token de API de um usuário moderador ou admin.
...
- Exige um token de API de um usuário admin.
...
Endpoint de proprietário/moderador para visibilidade da moderação de pacote.
...
`finalAction: "revoke"` com um relatório confirmado para aplicar a moderação da release no
...
- `revoked`: bloqueado depois que uma release foi anteriormente confiada.
...
Baixa o arquivo ZIP determinístico legado de uma release de pacote.
...
Baixa um ZIP de versão de skill hospedada ou retorna uma transferência de fonte do GitHub para uma
...
- Campo opcional de payload: `ownerHandle`. Quando presente, a API resolve esse
  publisher no servidor e exige que o ator tenha acesso ao publisher.
- Campo opcional de payload: `migrateOwner`. Quando `true` com `ownerHandle`, uma
  skill existente pode ser movida para esse proprietário se o ator for admin/proprietário em ambos
  os publishers atual e de destino. Sem esta adesão explícita, alterações de proprietário são
  rejeitadas.
...
- Os campos de formulário permitidos são `payload`, blobs repetidos em `files` ou uma referência de tarball
  `clawpack`. `clawpack` pode ser um blob `.tgz` ou um id de armazenamento retornado pelo
  fluxo de upload-url. Publicações preparadas com storage-id também devem incluir o
...
- Requisições de publicação multipart diretas têm limite de 18 MB. Tarballs ClawPack podem
  usar o fluxo de upload-url até o limite de 120 MB para tarball.
...
- Code plugins exigem `package.json`, metadados de repositório-fonte, metadados de commit-fonte,
...
- Somente o publisher da org `openclaw` e publishers pessoais de membros atuais da org `openclaw`
...
Somente admin. Garante que exista um publisher de org para um identificador. Se o identificador ainda apontar para um
usuário/publisher pessoal compartilhado legado, o endpoint o migra primeiro para um publisher de org.
Para uma org recém-criada, forneça `memberHandle`; o admin em atuação não é adicionado como membro.
...
Criação autenticada de publisher de org por autoatendimento. Cria um novo publisher de org e adiciona o
chamador como proprietário. Este endpoint não migra identificadores existentes de usuário/pessoais e
não marca o publisher como confiável/oficial.

- Corpo: `{ "handle": "opik", "displayName": "Opik" }`
- Resposta: `{ "ok": true, "publisherId": "...", "handle": "opik", "created": true, "trusted": false }`
- Retorna `409` quando o identificador já está em uso por um publisher, usuário ou publisher pessoal.
...
proprietário possa publicar depois a release real de code-plugin ou bundle-plugin nesse nome.
...
Somente admin. Recupera um publisher pessoal para um principal substituto verificado do GitHub OAuth
sem editar linhas de conta do Convex Auth. A requisição deve nomear ambos os ids imutáveis de conta do
...
`confirmIdentityVerified: true` depois que a equipe verifica de forma independente a continuidade entre as duas
identidades GitHub. A recuperação falha de modo fechado quando o publicador pessoal atual do usuário de destino
...
para o identificador recuperado também é reatribuída ao usuário substituto para que sincronizações posteriores
de perfil não possam restaurar a autoridade concorrente do usuário anterior. Cada tabela primária é limitada a
100 linhas por transação de aplicação; recuperações maiores devem primeiro usar uma migração retomável de proprietário.
Fontes de habilidades do GitHub têm escopo do publicador e são relatadas como verificadas em vez de reescritas.
...
Bane um usuário e exclui permanentemente as habilidades pertencentes a ele (somente moderador/admin).
