...
x-i18n:
    generated_at: "2026-07-01T07:55:03Z"
    model: gpt-5.5
...
Respostas de erro v1 públicas são texto simples com `content-type: text/plain; charset=utf-8`.
...
- Relevância é mais forte que popularidade. Uma correspondência precisa de slug ou token de nome de exibição pode superar uma correspondência mais ampla com engajamento muito maior.
...
- Popularidade é escalada logaritmicamente e limitada. Skills de alto engajamento podem ficar abaixo quando o texto da consulta tem uma correspondência mais fraca.
...
- Aliases de renomeação preservam a resolução para URLs antigas e instalações que resolvem pelo registro, mas a classificação de pesquisa é baseada nos metadados canônicos da Skill depois que a renomeação é indexada. Estatísticas existentes permanecem com a Skill.
...
- Quando `nonSuspiciousOnly=true`, ordenações baseadas em cursor podem retornar menos de `limit` itens em uma página porque Skills suspeitas são filtradas depois da recuperação da página.
...
- Retorna `queue.queuedAhead` e `queue.position` enquanto estiver enfileirado, para que os clientes possam mostrar quantos scans manuais priorizados estão à frente da requisição. Filas muito grandes são limitadas e informadas com `queuedAheadIsEstimate: true`.
...
Rota canônica de rescan em lote somente para administradores. Ela aceita o mesmo formato de payload da rota legada `POST /api/v1/skills/-/rescan-batch`.

### `POST /api/v1/skills/-/scan/batch/status`

Rota canônica de status em lote somente para administradores. Ela aceita `{ "jobIds": ["..."] }` e retorna os mesmos contadores agregados da rota legada `POST /api/v1/skills/-/rescan-batch/status`.
...
- `security.signals` contém apenas evidências de suporte no nível de status; use `/scan` ou a página de auditoria de segurança do ClawHub para detalhes completos de scanner.
- `security.signals.dependencyRegistry` é mantido para compatibilidade da resposta v1, mas o scanner de existência do registro de dependências foi aposentado e esta chave é sempre `null`.
...
- Entradas de skill continuam baseadas no registro de skills e ainda podem ser publicadas somente por meio de `POST /api/v1/skills`.
...
- `isOfficial` (opcional): `true` ou `false`
- `category` (opcional): filtro de categoria de plugin. Compatível somente quando a
  solicitação está no escopo de pacotes de plugin. Categorias controladas e aliases
...
- Skills atuais baseadas no GitHub com verificação `clean` ou `suspicious` incluem
...
- `limit` (opcional): inteiro (1-250), padrão `250`.
- `cursor` (opcional): cursor de paginação da resposta anterior.
- `family` (opcional): `code-plugin` ou `bundle-plugin`. Se omitido, significa ambas
...
- Os resultados são retornados por ordem de relevância e atualmente não são paginados.
...
Retorna os metadados de detalhes do pacote.
...
- Requer um token de API para o proprietário do pacote, um proprietário/administrador do editor da organização,
...
- `version.artifact.kind` é `legacy-zip` para arquivos de pacote do modelo antigo ou
  `npm-pack` para lançamentos apoiados pelo ClawPack.
...
- `revoked`: bloqueado depois que uma release havia sido previamente confiável.
...
- Transferências de skills baseadas no GitHub não fazem proxy nem espelham bytes. A resposta JSON
  ...
  e `archiveUrl`; o estado da varredura/atual é uma barreira e não é incluído como metadados de
  payload de sucesso.
...
- Campo opcional de payload: `migrateOwner`. Quando `true` com `ownerHandle`, uma
  skill existente pode ser movida para esse proprietário se o ator for admin/proprietário nos publicadores
  atual e de destino. Sem esta adesão explícita, alterações de proprietário são
  rejeitadas.
...
release. Nomes de pacotes se tornam pacotes privados de placeholder sem linhas de release, para que o mesmo
...
identidades GitHub. A recuperação falha de forma fechada quando o publicador pessoal atual do usuário de destino
