...
- Os resultados são retornados em ordem de relevância (similaridade de embedding + impulsos por token exato de slug/nome + prior de popularidade a partir de downloads).
...
- Downloads são usados como um prior pequeno em escala logarítmica e critério de desempate, não como o principal sinal de ranqueamento. Skills com muitos downloads podem ranquear abaixo quando o texto da consulta é uma correspondência mais fraca.
...
- Coloque os termos que os usuários procurarão literalmente no nome de exibição, no resumo e nas tags. Use um token de slug autônomo somente quando ele também for uma identidade estável que você deseja manter.
- Não renomeie um slug apenas para perseguir uma consulta, a menos que o novo slug seja um nome canônico melhor a longo prazo. Slugs antigos viram aliases de redirecionamento, mas a URL canônica, o slug exibido e os futuros resumos de pesquisa usam o novo slug.
...
Denuncie uma Skill para revisão por moderador. Denúncias são em nível de Skill, opcionalmente vinculadas
...
Retorna conteúdo bruto de texto.
...
- `externalService`, `binary`, `osPermission` (opcional): atalho para tags de
  requisito de ambiente nomeadas
...
- `artifactKind` (opcional): `legacy-zip` ou `npm-pack`
- `npmMirror` (opcional): `true`/`1` para pesquisar versões de pacotes baseadas no ClawPack
  disponíveis por meio do espelho npm
...
- Skills também podem ser resolvidas por meio desta rota no catálogo unificado.
...
- Requer um token de API do proprietário do pacote, um proprietário/admin de publicador da organização,
...
Relate um pacote para revisão de moderador. Os relatos são em nível de pacote, opcionalmente
...
Endpoint de moderador/admin para recebimento de relatos de pacote.
...
Endpoint de moderador/admin para resolver ou reabrir relatos de pacote.
...
`finalAction: "revoke"` com um relato confirmado para aplicar moderação de lançamento no
...
Endpoint de manutenção somente para admins para rotular lançamentos de pacote mais antigos com
...
- Arquivos Plugin/pacote são arquivos zip com uma raiz `package/` para que clientes OpenClaw
...
- Packuments de pacote com escopo aceitam tanto `/api/npm/@scope/name` quanto o caminho de requisição
...
- Plugins de código exigem `package.json`, metadados do repositório-fonte, metadados do commit-fonte,
  metadados do schema de configuração, `openclaw.compat.pluginApi` e
...
Somente para admins. Garante que exista um publicador de organização para um handle. Se o handle ainda apontar para um
...
Somente para admins. Reserva slugs raiz e nomes de pacote para um proprietário legítimo sem publicar um
...
Bane um usuário e exclua permanentemente Skills de propriedade dele (somente moderador/admin).
...
Remova o banimento de um usuário e restaure Skills elegíveis (somente admin).
...
Altere a função de um usuário (somente admin).
...
Liste ou pesquise usuários (somente admin).
...
Adicione/remova uma estrela (destaques). Ambos os endpoints são idempotentes.
...
Se você auto-hospedar, sirva este arquivo (ou defina `CLAWHUB_REGISTRY` explicitamente; `CLAWDHUB_REGISTRY` legado).
