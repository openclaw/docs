...
x-i18n:
    generated_at: "2026-07-01T05:31:42Z"
    model: gpt-5.5
...
- Se nenhum IP de cliente confiável estiver disponível, solicitações anônimas usam buckets de fallback
  escopados somente pelo tipo de limite de taxa. Esses buckets de fallback não incluem
  caminhos, slugs, nomes de pacotes, versões, strings de consulta ou outros parâmetros
  de artefato fornecidos pelo chamador.
...
- Os resultados são retornados em ordem de relevância (similaridade de embeddings + reforços de token de slug/nome exatos + uma pequena prioridade de popularidade).
...
- Texto ASCII é tokenizado em limites de palavras e pontuação. Por exemplo, `personal-map` contém um token `map` independente, enquanto `amap-jsapi-skill` contém `amap`, `jsapi` e `skill`; portanto, buscar por `map` dá a `personal-map` uma correspondência lexical mais forte do que `amap-jsapi-skill`.
- A popularidade usa escala logarítmica e é limitada. Skills com alto engajamento podem ficar em posição inferior quando o texto da consulta tem correspondência mais fraca.
...
- Coloque os termos que os usuários literalmente buscarão no nome de exibição, resumo e tags. Use um token de slug independente somente quando ele também for uma identidade estável que você quer manter.
- Não renomeie um slug apenas para perseguir uma consulta, a menos que o novo slug seja um nome canônico melhor a longo prazo. Slugs antigos viram aliases de redirecionamento, mas a URL canônica, o slug exibido e os futuros resumos de busca usam o novo slug.
...
Denuncia uma Skill para revisão por moderador. Denúncias são no nível da Skill, opcionalmente vinculadas
...
Scans por upload local não são mais compatíveis. Solicitações usando
...
- Trabalhos de scan são assíncronos. Solicitações manuais de scan são priorizadas à frente do trabalho normal de publicação/backfill, mas a conclusão ainda depende da disponibilidade dos workers.

### `GET /api/v1/skills/-/scan/{scanId}`

Endpoint autenticado de polling para um scan enviado.

- Retorna status em fila/em execução/bem-sucedido/falhou.
- Retorna `queue.queuedAhead` e `queue.position` enquanto estiver em fila para que clientes possam mostrar quantos scans manuais priorizados estão à frente da solicitação. Filas muito grandes são limitadas e relatadas com `queuedAheadIsEstimate: true`.
...
- Exige acesso de gerenciamento de proprietário/publicador ao skill ou plugin, ou autoridade de moderador/administrador da plataforma.
...
Rota canônica de novo scan em lote, somente para administradores. Ela aceita o mesmo formato de payload que o `POST /api/v1/skills/-/rescan-batch` legado.

### `POST /api/v1/skills/-/scan/batch/status`

Rota canônica de status em lote, somente para administradores. Ela aceita `{ "jobIds": ["..."] }` e retorna os mesmos contadores agregados que o `POST /api/v1/skills/-/rescan-batch/status` legado.
...
- Identidade do skill, identidade do publicador e metadados da versão selecionada são campos de envelope de nível superior (`slug`, `displayName`, `publisherHandle`, `version`, `resolvedFrom`, `tag`, `createdAt`) para que automações de shell possam lê-los sem desempacotar wrappers aninhados.
...
- Os resultados são por item; um skill ou versão ausente não faz a resposta inteira falhar.
- A resposta é somente de segurança. Ela não inclui dados de Skill Card, status de card gerado, listas de arquivos de artefato nem payloads detalhados de scanners.
- `security.signals` contém apenas evidências de apoio em nível de status; use `/scan` ou a página de auditoria de segurança do ClawHub para detalhes completos dos scanners.
- `security.signals.dependencyRegistry` é mantido para compatibilidade com respostas v1, mas o scanner de existência do registro de dependências foi aposentado e esta chave é sempre `null`.
...
- Use `/verify` quando precisar do envelope de verificação do Skill Card de um único skill, `/card` quando precisar do markdown do card gerado e `/scan` quando precisar de dados detalhados de scanners.
...
- Os aliases de filtro legados da v1 documentados em `GET /api/v1/plugins` também são
  aceitos.
- A filtragem por categoria é um filtro de API real respaldado por linhas de resumo de categoria de Plugin,
  não uma reescrita de consulta de busca.
...
- Requer um token de API do proprietário do pacote, proprietário/administrador do publicador da organização,
...
Retorna o resumo exato de segurança e confiança do lançamento do pacote para clientes de instalação.
Esta é a superfície pública de consumo do OpenClaw para decidir se um
...
- Este endpoint é intencionalmente mais restrito que endpoints de moderação de proprietário/moderador.
  ...
  identidades de denunciantes, corpos de denúncias, evidências privadas ou cronogramas de revisão
  internos.
...
- Versões ClawPack transmitem os bytes exatos de `.tgz` npm-pack enviados.
...
- Requer um token de API para usuário moderador ou administrador.
...
- Requer um token de API para usuário administrador.
...
- Requer um token de API de usuário moderador ou admin.
...
Endpoint de moderador/admin para análise de versão de pacote.
...
- `revoked`: bloqueada depois que uma versão anteriormente era confiável.
...
- Apenas versões com tarballs npm-pack do ClawPack carregados são listadas.
...
- Packuments de pacotes com escopo são compatíveis com `/api/npm/@scope/name` e o caminho de requisição
  codificado do npm `/api/npm/@scope%2Fname`.

### `GET /api/npm/{package}/-/{tarball}.tgz`

Transmite os bytes exatos do tarball ClawPack carregado para clientes de espelho npm.
...
- Se nem `version` nem `tag` for fornecido, a versão mais recente será usada.
...
- Apenas o publicador da org `openclaw` e os publicadores pessoais de membros atuais da org `openclaw`
...
publicador pessoal/usuário compartilhado legado, o endpoint o migra primeiro para um publicador de org.
...
Criação autenticada por autosserviço de publicador de org. Cria um novo publicador de org e adiciona o
...
Somente admin. Recupera um publicador pessoal para um principal OAuth do GitHub substituto verificado
...
para o handle recuperado também é reatribuída ao usuário substituto para que a sincronização posterior do perfil
...
Listar ou pesquisar usuários (somente admin).
...
Se você fizer hospedagem própria, sirva este arquivo (ou defina `CLAWHUB_REGISTRY` explicitamente; legado `CLAWDHUB_REGISTRY`).
