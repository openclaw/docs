...
summary: Referência da API HTTP (endpoints públicos + endpoints de CLI + autenticação).
x-i18n:
...
- Os resultados são retornados em ordem de relevância (similaridade de embedding + boosts de token por slug/nome exatos + prioridade de popularidade por downloads).
- A relevância é mais forte que a popularidade. Uma correspondência precisa de token de slug ou nome de exibição pode superar uma correspondência mais ampla com muito mais downloads.
...
- Downloads são usados como uma pequena prioridade em escala logarítmica e critério de desempate, não como o principal sinal de ranqueamento. Skills com muitos downloads podem ranquear abaixo quando o texto da consulta é uma correspondência mais fraca.
...
- Aliases de renomeação preservam a resolução para URLs antigas e instalações que resolvem pelo registro, mas o ranqueamento de pesquisa é baseado nos metadados canônicos do Skill depois que a renomeação foi indexada. Estatísticas existentes permanecem com o Skill.
- Se um Skill estiver inesperadamente invisível, verifique primeiro o estado de moderação com `clawhub inspect <slug>` enquanto estiver conectado antes de alterar metadados relacionados ao ranqueamento.
...
- `createdAt` é estável para varreduras de novos Skills; `updated` muda quando Skills existentes são republicados.
...
- Chamadores públicos recebem `200` somente para Skills visíveis já sinalizados.
- Evidências são redigidas para chamadores públicos e incluem trechos brutos somente para proprietários/moderadores.

### `POST /api/v1/skills/{slug}/report`

Reporta um Skill para análise de moderador. Relatos são no nível do Skill, opcionalmente vinculados
...
- `version.security` inclui status normalizado de verificação da varredura e detalhes do scanner
...
- `npmMirror` (opcional): `true`/`1` para mostrar versões de pacotes baseadas em ClawPack
  disponíveis pelo espelho npm

Observações:

- `GET /api/v1/code-plugins` e `GET /api/v1/bundle-plugins` permanecem aliases de família fixa.
- Entradas de Skill continuam baseadas no registro de Skill e ainda podem ser publicadas somente por `POST /api/v1/skills`.
...
- `artifactKind` (opcional): `legacy-zip` ou `npm-pack`
- `npmMirror` (opcional): `true`/`1` para pesquisar versões de pacotes baseadas em ClawPack
  disponíveis pelo espelho npm
...
- Requer um token de API para o proprietário do pacote, um proprietário/admin de publicador de org,
...
- Esta é a superfície do resolvedor OpenClaw; ela evita adivinhar o formato do arquivo
...
- Versões ClawPack transmitem exatamente os bytes `.tgz` do npm-pack enviado.
...
- `revoked`: bloqueado depois que um lançamento havia sido confiável anteriormente.
...
Endpoint de manutenção exclusivo para administradores para rotular lançamentos de pacotes antigos com
...
- Linhas existentes baseadas em ClawPack sem `artifactKind` são reparadas como
  `npm-pack`.
- Isso não gera ClawPacks nem modifica bytes de artefatos.
...
- Usa por padrão o lançamento mais recente.
...
- Somente versões com tarballs npm-pack ClawPack carregados são listadas.
...
Publica um lançamento code-plugin ou bundle-plugin.
...
publicador pessoal/de usuário compartilhado legado, o endpoint o migra primeiro para um publicador de organização.
...
Exclusivo para administradores. Reserva slugs raiz e nomes de pacotes para um proprietário legítimo sem publicar um
lançamento. Nomes de pacote tornam-se pacotes de placeholder privados sem linhas de lançamento, para que o mesmo
proprietário possa publicar posteriormente o lançamento real code-plugin ou bundle-plugin nesse nome.
...
Bane um usuário e exclui permanentemente Skills de sua propriedade (somente moderador/administrador).
...
Altera o papel de um usuário (somente administrador).
