...
x-i18n:
    generated_at: "2026-07-04T06:24:48Z"
    model: gpt-5.5
...
- Os resultados são retornados em ordem de relevância (similaridade de embeddings + impulsos de token exato de slug/nome + um pequeno prior de popularidade).
- Relevância é mais forte que popularidade. Uma correspondência precisa de slug ou token de nome de exibição pode superar uma correspondência mais frouxa com engajamento muito maior.
...
- Popularidade usa escala logarítmica e é limitada. Skills de alto engajamento podem ranquear abaixo quando o texto da consulta é uma correspondência mais fraca.
...
Orientação de descobribilidade para publicadores:
...
- Aliases de renomeação preservam a resolução para URLs antigas e instalações que resolvem pelo registro, mas o ranqueamento de pesquisa se baseia nos metadados canônicos do Skill depois que a renomeação foi indexada. Estatísticas existentes permanecem com o Skill.
- Se um Skill estiver inesperadamente invisível, verifique primeiro o estado de moderação com `clawhub inspect @owner/slug` enquanto estiver conectado antes de alterar metadados relacionados ao ranqueamento.
...
- Quando `nonSuspiciousOnly=true`, ordenações baseadas em cursor podem retornar menos que `limit` itens em uma página porque Skills suspeitos são filtrados depois da recuperação da página.
...
Relata um Skill para análise por moderador. Relatos são em nível de Skill, opcionalmente vinculados
...
- Trabalhos de scan são assíncronos. Solicitações manuais de scan são priorizadas à frente do trabalho normal de publicação/preenchimento retroativo, mas a conclusão ainda depende da disponibilidade de workers.

### `GET /api/v1/skills/-/scan/{scanId}`

Endpoint autenticado de consulta para um scan enviado.

- Retorna status em fila/em execução/bem-sucedido/com falha.
- Retorna `queue.queuedAhead` e `queue.position` enquanto está em fila para que clientes possam mostrar quantos scans manuais priorizados estão à frente da solicitação. Filas muito grandes são limitadas e relatadas com `queuedAheadIsEstimate: true`.
...
- Retorna o mesmo formato de ZIP dos downloads de solicitação de scan.

### `POST /api/v1/skills/-/scan/batch`

Rota canônica de novo scan em lote somente para administradores. Ela aceita o mesmo formato de payload do legado `POST /api/v1/skills/-/rescan-batch`.

### `POST /api/v1/skills/-/scan/batch/status`

Rota canônica de status em lote somente para administradores. Ela aceita `{ "jobIds": ["..."] }` e retorna os mesmos contadores agregados do legado `POST /api/v1/skills/-/rescan-batch/status`.
...
- A resposta é somente de segurança. Ela não inclui dados de Skill Card, status de cartão gerado, listas de arquivos de artefatos ou payloads detalhados de scanner.
- `security.signals` contém apenas evidências de suporte em nível de status; use `/scan` ou a página de auditoria de segurança do ClawHub para obter detalhes completos do scanner.
- `security.signals.dependencyRegistry` é mantido para compatibilidade da resposta v1, mas o scanner de existência do registro de dependências foi aposentado e esta chave é sempre `null`.
...
- `category` (opcional): filtro de categoria de plugin. Compatível somente quando a
  solicitação está no escopo de pacotes de plugins (`/api/v1/plugins`,
...
- Chamadores autenticados podem ver pacotes privados de publicadores aos quais pertencem em resultados de listagem/pesquisa.
- `channel=private` retorna apenas pacotes que o chamador autenticado pode ler.

### `GET /api/v1/packages/search`

Pesquisa de catálogo unificado entre Skills + pacotes de plugins.
...
- `isOfficial` (opcional): `true` ou `false`
- `category` (opcional): filtro de categoria de plugin. Compatível somente quando a
  solicitação está no escopo de pacotes de plugins. Categorias controladas e aliases
...
`trending` é um ranking de instalações/downloads de sete dias e não usa totais de todo o período.
...
- Os aliases legados de filtro v1 documentados em `GET /api/v1/plugins` também são
  aceitos.
- A filtragem por categoria é um filtro real de API com suporte por linhas de digest
...
Exclui temporariamente um pacote e todas as releases.

Observações:

- Exige um token de API para o proprietário do pacote, proprietário/admin do publisher da organização,
...
- Endpoint de leitura pública. Nenhum token de proprietário, publisher, moderador ou admin é
  obrigatório.
...
- `trust.scanStatus` é o status de confiança efetivo derivado de entradas do scanner
  e moderação manual da release.
...
- `trust.reasons` é a lista de explicações voltada ao usuário e para auditoria. Os códigos de motivo
...
- Este endpoint é exato por versão. Os clientes devem chamá-lo depois de resolver a
...
- disponibilidade do artefato ClawPack npm-pack
- digest do artefato
...
Endpoint de moderador/admin para filas de revisão de releases de pacote.
...
- `open`: releases suspeitas, maliciosas, pendentes, em quarentena, revogadas ou relatadas.
- `blocked`: releases em quarentena, revogadas ou maliciosas.
- `manual`: qualquer release com uma substituição manual de moderação.
- `all`: qualquer release com substituição manual, estado de varredura não limpo ou relato de pacote.
...
Relata um pacote para revisão por moderador. Relatos são em nível de pacote, opcionalmente
vinculados a uma versão. Eles alimentam a fila de moderação, mas não ocultam automaticamente nem
bloqueiam downloads por si só; moderadores devem usar a moderação de releases para
...
Endpoint de moderador/admin para revisão de versões de pacotes.
...
Retorna um packument compatível com npm para versões de pacote baseadas no ClawPack.
...
- Packuments de pacotes com escopo dão suporte tanto a `/api/npm/@scope/name` quanto ao caminho de requisição
  codificado do npm `/api/npm/@scope%2Fname`.

### `GET /api/npm/{package}/-/{tarball}.tgz`

Transmite os bytes exatos do tarball enviado do ClawPack para clientes de espelho npm.

Observações:

- Usa o bucket de limite de download.
- Cabeçalhos de download incluem SHA-256 do ClawHub, além de metadados de integridade/shasum do npm.
...
- Transferências de skills baseadas no GitHub não fazem proxy nem espelham bytes. A resposta JSON
  ...
  e `archiveUrl`; o estado de varredura/atual é um portão e não é incluído como metadados
...
- Os campos de formulário permitidos são `payload`, blobs `files` repetidos ou uma referência de tarball
  ...
  fluxo de URL de upload. Publicações preparadas com storage-id também devem incluir o
...
- Requisições diretas de publicação multipart são limitadas a 18 MB. Tarballs do ClawPack podem
...
- Apenas o publicador da org `openclaw` e os publicadores pessoais dos membros atuais da org `openclaw`
...
usuário/publicador pessoal compartilhado legado, o endpoint primeiro o migra para um publicador de org.
...
chamador como proprietário. Este endpoint não migra handles de usuário/pessoais existentes e não
...
versão. Nomes de pacote tornam-se pacotes privados de espaço reservado sem linhas de versão, para que o mesmo
proprietário possa posteriormente publicar a versão real de code-plugin ou bundle-plugin nesse nome.
...
de provedor do GitHub; handles mutáveis são usados apenas como uma proteção voltada ao operador.
...
- `rename` preserva o slug anterior como alias de redirecionamento.
