...
read_when:
    ...
    - Depurando solicitações CLI ↔ registry
summary: Referência da API HTTP (endpoints públicos + CLI + autenticação).
x-i18n:
    generated_at: "2026-07-03T13:21:08Z"
    model: gpt-5.5
...
Atalhos de slug da web resolvem entre famílias do registro, mas clientes de API devem usar
...
- A relevância é mais forte que a popularidade. Uma correspondência precisa de slug ou token de nome de exibição pode ficar acima de uma correspondência mais vaga com engajamento muito maior.
- Texto ASCII é tokenizado em limites de palavras e pontuação. Por exemplo, `personal-map` contém um token independente `map`, enquanto `amap-jsapi-skill` contém `amap`, `jsapi` e `skill`; portanto, buscar por `map` dá a `personal-map` uma correspondência lexical mais forte que a `amap-jsapi-skill`.
- A popularidade é escalada em log e limitada. Skills de alto engajamento podem ficar abaixo quando o texto da consulta tem correspondência mais fraca.
...
- Coloque os termos que os usuários literalmente buscarão no nome de exibição, resumo e tags. Use um token de slug independente somente quando ele também for uma identidade estável que você deseja manter.
...
- Aliases de renomeação preservam a resolução para URLs antigas e instalações que resolvem pelo registro, mas a classificação de busca é baseada nos metadados canônicos da skill depois que a renomeação tiver sido indexada. Estatísticas existentes permanecem com a skill.
...
Reporte uma skill para revisão por moderador. Relatórios são no nível da skill, opcionalmente vinculados
...
Varreduras de upload local não são mais compatíveis. Requisições que usam
...
- Varreduras publicadas exigem acesso de gerenciamento do proprietário/publicador ou autoridade de moderador/admin da plataforma.
...
- Trabalhos de varredura são assíncronos. Solicitações manuais de varredura são priorizadas antes do trabalho normal de publicação/preenchimento retroativo, mas a conclusão ainda depende da disponibilidade dos workers.
...
Rota canônica de nova varredura em lote somente para admins. Ela aceita o mesmo formato de payload do legado `POST /api/v1/skills/-/rescan-batch`.

### `POST /api/v1/skills/-/scan/batch/status`

Rota canônica de status de lote somente para admins. Ela aceita `{ "jobIds": ["..."] }` e retorna os mesmos contadores agregados do legado `POST /api/v1/skills/-/rescan-batch/status`.
...
- A resposta é somente de segurança. Ela não inclui dados do Skill Card, status do cartão gerado, listas de arquivos de artefatos nem payloads detalhados dos scanners.
- `security.signals` contém apenas evidências de suporte no nível de status; use `/scan` ou a página de auditoria de segurança do ClawHub para obter detalhes completos dos scanners.
- `security.signals.dependencyRegistry` é mantido para compatibilidade da resposta v1, mas o scanner de existência do registro de dependências foi aposentado e esta chave é sempre `null`.
...
- `category` (opcional): filtro de categoria de plugin. Compatível somente quando a
  solicitação é escopada para pacotes de plugin (`/api/v1/plugins`,
...
- `isOfficial` (opcional): `true` ou `false`
- `category` (opcional): filtro de categoria de plugin. Compatível somente quando a
  solicitação é escopada para pacotes de plugin. Categorias controladas e aliases
...
Busca apenas de Plugin entre pacotes code-plugin e bundle-plugin.
...
Faz exclusão lógica de um pacote e todas as versões.

Observações:

- Exige um token de API do proprietário do pacote, proprietário/administrador do publicador da organização,
...
- `trust.reasons` é a lista de explicações voltada ao usuário e de auditoria. Os códigos de motivo
...
- Este endpoint é exato por versão. Os clientes devem chamá-lo após resolver a
...
- Este endpoint é intencionalmente mais restrito do que endpoints de moderação de proprietário/moderador.
  ...
  identidades de denunciantes, corpos de denúncias, evidências privadas ou cronogramas internos
  de revisão.
...
- Versões ClawPack transmitem os bytes exatos `.tgz` do npm-pack enviado.
...
- Exige um token de API para usuário moderador ou administrador.
...
- Exige um token de API para usuário administrador.
...
`finalAction: "revoke"` com uma denúncia confirmada para aplicar moderação de release no
...
Endpoint de moderador/admin para revisão de releases de pacotes.
...
- Code plugins exigem `package.json`, metadados de repositório de origem, metadados de commit
...
- Somente o publicador da organização `openclaw` e publicadores pessoais de membros atuais da organização `openclaw`
  podem publicar no canal `official`.
- Publicações em nome de terceiros ainda validam a elegibilidade do canal oficial em relação à conta proprietária de destino.
...
release. Nomes de pacotes se tornam pacotes placeholder privados sem linhas de release, para que o mesmo
...
O endpoint usa execução simulada por padrão. Aplicar a recuperação exige `dryRun: false` e
...
aliases de slugs de skills, pacotes, avisos do inspetor de pacotes e linhas derivadas de resumo de pesquisa para que
...
conteúdo (somente admin). Usa execução simulada por padrão, a menos que `dryRun` seja `false`.
