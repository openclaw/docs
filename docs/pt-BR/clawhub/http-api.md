...
- Se nenhum IP de cliente confiável estiver disponível, solicitações anônimas de download usam um bucket de fallback com escopo de endpoint em vez de um único bucket global `ip:unknown`. Solicitações anônimas de leitura/escrita ainda usam o bucket desconhecido compartilhado para que o roteamento com IP ausente permaneça visível e conservador.
...
- Os resultados são retornados em ordem de relevância (similaridade de embeddings + reforços de tokens exatos de slug/nome + prior de popularidade por downloads).
- A relevância é mais forte que a popularidade. Uma correspondência precisa de token de slug ou nome de exibição pode superar uma correspondência mais vaga com muito mais downloads.
- Texto ASCII é tokenizado nos limites de palavras e pontuação. Por exemplo, `personal-map` contém um token `map` autônomo, enquanto `amap-jsapi-skill` contém `amap`, `jsapi` e `skill`; pesquisar por `map`, portanto, dá a `personal-map` uma correspondência lexical mais forte do que a `amap-jsapi-skill`.
- Downloads são usados como um prior pequeno em escala logarítmica e critério de desempate, não como o sinal primário de ranqueamento. Skills com muitos downloads podem ficar em posição inferior quando o texto da consulta tem uma correspondência mais fraca.
...
- Coloque os termos que os usuários pesquisarão literalmente no nome de exibição, no resumo e nas tags. Use um token de slug autônomo apenas quando ele também for uma identidade estável que você deseja manter.
...
- Aliases de renomeação preservam a resolução para URLs antigas e instalações que resolvem pelo registro, mas o ranqueamento de pesquisa se baseia nos metadados canônicos da Skill depois que a renomeação for indexada. Estatísticas existentes permanecem com a Skill.
...
- `npmMirror` (opcional): `true`/`1` para mostrar versões de pacote baseadas em
  ClawPack disponíveis pelo espelho npm

Observações:

- `GET /api/v1/code-plugins` e `GET /api/v1/bundle-plugins` permanecem aliases de família fixa.
- As entradas de Skills continuam respaldadas pelo registro de Skills e ainda podem ser publicadas apenas por `POST /api/v1/skills`.
...
- `artifactKind` (opcional): `legacy-zip` ou `npm-pack`
- `npmMirror` (opcional): `true`/`1` para pesquisar versões de pacote baseadas
  em ClawPack disponíveis pelo espelho npm
...
- Requer um token de API do proprietário do pacote, um proprietário/administrador
  do publicador da organização, moderador da plataforma ou administrador da plataforma.
...
- Versões ClawPack incluem campos compatíveis com npm `npmIntegrity`, `npmShasum` e
  `npmTarballName`.
- `version.sha256hash`, `version.vtAnalysis`, `version.llmAnalysis` e `version.staticScan` são incluídos quando há dados de varredura.
- Pacotes privados retornam `404`, a menos que o chamador possa ler o publicador proprietário.

### `GET /api/v1/packages/{name}/versions/{version}/security`

Retorna o resumo exato de segurança e confiança da versão do pacote para clientes
de instalação. Esta é a superfície pública de consumo do OpenClaw para decidir se
uma versão resolvida pode ser instalada.

Autenticação:

- Endpoint de leitura pública. Nenhum token de proprietário, publicador,
  moderador ou administrador é necessário.
...
- `trust.moderationState` pode ser nulo. Ele é `null` quando não existe
...
- `trust.reasons` é a lista de explicações para o usuário e para auditoria. Códigos de motivo
...
- Versões ClawPack transmitem exatamente os bytes `.tgz` npm-pack enviados.
...
Relate um pacote para análise da moderação. Os relatórios são em nível de pacote, opcionalmente
vinculados a uma versão. Eles alimentam a fila de moderação, mas não ocultam automaticamente nem
bloqueiam downloads por si só; os moderadores devem usar a moderação de lançamento para
...
Endpoint de moderador/admin para entrada de relatórios de pacotes.
...
`finalAction: "revoke"` com um relatório confirmado para aplicar a moderação de lançamento no
...
Endpoint de moderador/admin para análise de lançamento de pacote.
...
- `revoked`: bloqueado após um lançamento ter sido anteriormente confiável.
...
Endpoint de manutenção exclusivo para admins para rotular lançamentos de pacotes mais antigos com
...
- Linhas existentes com suporte do ClawPack sem `artifactKind` são reparadas como
  `npm-pack`.
...
Retorna um packument compatível com npm para versões de pacote com suporte do ClawPack.

Observações:

- Somente versões com tarballs npm-pack do ClawPack carregados são listadas.
...
- Packuments de pacote com escopo oferecem suporte tanto ao caminho de solicitação `/api/npm/@scope/name` quanto ao
  caminho codificado `/api/npm/@scope%2Fname` do npm.

### `GET /api/npm/{package}/-/{tarball}.tgz`

Transmite exatamente os bytes do tarball ClawPack carregado para clientes de espelho npm.
...
- Preferencial: `multipart/form-data` com JSON `payload` + blobs `files[]`.
- Corpo JSON com `files` (baseado em storageId) também é aceito.
- Campo de payload opcional: `ownerHandle`. Quando presente, somente admins podem publicar em nome desse proprietário.
...
- Plugins de código exigem `package.json`, metadados do repositório-fonte, metadados do commit de origem,
...
- Publicações em nome de outra pessoa ainda validam a elegibilidade para o canal oficial em relação à conta do proprietário de destino.
...
publicador legado compartilhado de usuário/pessoal, o endpoint primeiro o migra para um publicador de organização.
...
lançamento. Nomes de pacotes tornam-se pacotes de espaço reservado privados sem linhas de lançamento, para que o mesmo
...
Banir um usuário e excluir permanentemente Skills pertencentes a ele (somente moderador/admin).
...
## Endpoints da CLI legada (obsoletos)
