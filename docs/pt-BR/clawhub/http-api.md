...
x-i18n:
    generated_at: "2026-07-04T17:52:15Z"
    model: gpt-5.5
...
- `X-RateLimit-Remaining` / `RateLimit-Remaining`: orçamento restante exato quando presente.
  Requisições bem-sucedidas fragmentadas omitem este header em vez de retornar um valor global aproximado.
...
- Usa headers confiáveis de IP do cliente, incluindo `cf-connecting-ip`, somente quando a
  implantação habilita explicitamente headers encaminhados confiáveis.
- O ClawHub usa headers de encaminhamento confiáveis para identificar IPs de clientes na borda.
...
As respostas públicas de erro v1 são texto puro com `content-type: text/plain; charset=utf-8`.
...
- Texto ASCII é tokenizado em limites de palavra e pontuação. Por exemplo, `personal-map` contém um token `map` independente, enquanto `amap-jsapi-skill` contém `amap`, `jsapi` e `skill`; portanto, pesquisar por `map` dá a `personal-map` uma correspondência lexical mais forte que `amap-jsapi-skill`.
- A popularidade é escalada por log e limitada. Skills de alto engajamento podem ficar abaixo no ranking quando o texto da consulta tem uma correspondência mais fraca.
...
- Aliases de renomeação preservam a resolução para URLs antigas e instalações resolvidas pelo registro, mas o ranking de pesquisa se baseia nos metadados canônicos da skill depois que a renomeação for indexada. Estatísticas existentes permanecem com a skill.
- Se uma skill estiver inesperadamente invisível, verifique primeiro o estado de moderação com `clawhub inspect @owner/slug` enquanto estiver logado antes de alterar metadados relacionados ao ranking.
...
- Slugs antigos criados por fluxos de renomeação/mesclagem de owner resolvem para a skill canônica.
- `metadata.os`: restrições de OS declaradas no frontmatter da skill (por exemplo, `["macos"]`, `["linux"]`). `null` se não declarado.
...
- `moderation` é incluído somente quando a skill está sinalizada ou quando o owner a está visualizando.
...
- Owners e moderadores podem acessar detalhes de moderação para skills ocultas.
- Chamadores públicos só recebem `200` para skills visíveis já sinalizadas.
- Evidências são redigidas para chamadores públicos e incluem trechos brutos somente para owners/moderadores.

### `POST /api/v1/skills/{slug}/report`

Reporta uma skill para análise de moderador. Reports são no nível da skill, opcionalmente vinculados
...
definir `status` de volta para `open`. Passe `finalAction: "hide"` com um report triado
para ocultar a skill no mesmo workflow auditável.
...
Retorna detalhes de verificação por varredura de segurança para uma versão de skill.
...
- Trabalhos de scan são assíncronos. Requisições manuais de scan são priorizadas antes do trabalho normal de publicação/backfill, mas a conclusão ainda depende da disponibilidade de workers.
...
Rota canônica de rescan em lote somente para administradores. Ela aceita o mesmo formato de payload que a rota legada `POST /api/v1/skills/-/rescan-batch`.
...
- `security.signals.dependencyRegistry` é mantido para compatibilidade com respostas v1, mas o scanner de existência do registro de dependências foi aposentado e essa chave é sempre `null`.
...
endpoint de coleção é destinado a clientes que já sabem quais versões instaladas
de skills do ClawHub precisam exibir, como a OpenClaw Control UI.
...
Busca unificada no catálogo em Skills + pacotes de plugin.
...
- Skills atuais baseadas no GitHub com varredura `clean` ou `suspicious` incluem
...
Pesquisa apenas de plugins em pacotes code-plugin e bundle-plugin.
...
- Os aliases de filtro legados da v1 documentados em `GET /api/v1/plugins` também são
  aceitos.
- A filtragem por categoria é um filtro de API real, baseado em linhas de resumo
...
- Requer um token de API para o proprietário do pacote, um proprietário/administrador do publisher da organização,
...
- `version.sha256hash` é metadado de compatibilidade obsoleto para clientes antigos. Ele
  gera hash dos bytes exatos do ZIP retornado por `/api/v1/packages/{name}/download`.
...
- Endpoint público de leitura. Nenhum token de proprietário, publisher, moderador ou administrador é
  necessário.
...
- Esta é a superfície do resolvedor OpenClaw; ela evita inferir o formato do arquivo a partir de
  uma URL compartilhada.
...
- Transferências de skills baseadas no GitHub não fazem proxy nem espelham bytes. A resposta JSON
  ...
  e `archiveUrl`; o estado de varredura/atual é uma barreira e não é incluído como metadados de carga útil
  de sucesso.
...
## Endpoints de Auth (token Bearer)
...
- Campo opcional da carga útil: `ownerHandle`. Quando presente, apenas admins podem publicar em nome desse proprietário.
...
- Apenas o publicador da organização `openclaw` e os publicadores pessoais de membros atuais da organização
  `openclaw` podem publicar no canal `official`.
- Publicações em nome de outro usuário ainda validam a qualificação para o canal oficial em relação à conta do proprietário de destino.
...
Apenas admin. Garante que exista um publicador de organização para um handle. Se o handle ainda apontar para um
...
Apenas admin. Reserva slugs raiz e nomes de pacote para um proprietário legítimo sem publicar uma
versão. Nomes de pacote se tornam pacotes privados de placeholder sem linhas de versão, para que o mesmo
proprietário possa publicar depois a versão real code-plugin ou bundle-plugin nesse nome.
...
Apenas admin. Recupera um publicador pessoal para um principal GitHub OAuth substituto verificado
...
tem skills, pacotes ou origens de skill do GitHub.
A recuperação também migra campos legados `ownerUserId` para as skills do publicador recuperado,
aliases de slug de skill, pacotes, avisos do inspetor de pacotes e linhas derivadas de digest de busca para que
...
para o identificador recuperado também é reatribuída ao usuário substituto para que sincronizações de perfil posteriores
não consigam restaurar a autoridade concorrente do usuário anterior. Cada tabela primária é limitada a
100 linhas por transação de aplicação; recuperações maiores devem primeiro usar uma migração retomável de proprietário.
Origens de skill do GitHub têm escopo de publicador e são relatadas como verificadas, em vez de reescritas.
...
Se você fizer auto-hospedagem, sirva este arquivo (ou defina `CLAWHUB_REGISTRY` explicitamente; legado `CLAWDHUB_REGISTRY`).
