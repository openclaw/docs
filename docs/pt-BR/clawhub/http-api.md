...
x-i18n:
    generated_at: "2026-07-05T04:51:03Z"
    model: gpt-5.5
...
Diretórios de terceiros podem usar os endpoints públicos de leitura para listar ou pesquisar Skills do ClawHub. Armazene resultados em cache, respeite `429`/`Retry-After`, direcione usuários de volta para a listagem canônica do ClawHub (`https://clawhub.ai/<owner>/skills/<slug>`) e evite sugerir endosso do ClawHub ao site de terceiros. Não tente espelhar conteúdo oculto, privado ou bloqueado por moderação fora da superfície da API pública.

Atalhos de slugs da Web são resolvidos entre famílias de registro, mas clientes da API devem usar
...
- `X-RateLimit-Remaining` / `RateLimit-Remaining`: orçamento restante exato quando presente.
  Requisições bem-sucedidas particionadas omitem este cabeçalho em vez de retornar um valor global aproximado.
...
- Os resultados são retornados em ordem de relevância (similaridade de embedding + reforços de token exato de slug/nome + um pequeno fator prévio de popularidade).
- A relevância é mais forte do que a popularidade. Uma correspondência precisa de slug ou token de nome de exibição pode superar uma correspondência mais ampla com engajamento muito maior.
...
- A popularidade é escalada logaritmicamente e limitada. Skills com alto engajamento podem ter classificação menor quando o texto da consulta é uma correspondência mais fraca.
...
- Coloque os termos que os usuários literalmente pesquisarão no nome de exibição, no resumo e nas tags. Use um token de slug independente somente quando ele também for uma identidade estável que você deseja manter.
...
- Aliases de renomeação preservam a resolução para URLs antigas e instalações que resolvem pelo registro, mas a classificação de pesquisa se baseia nos metadados canônicos da Skill depois que a renomeação é indexada. Estatísticas existentes permanecem com a Skill.
- Se uma Skill ficar inesperadamente invisível, verifique primeiro o estado de moderação com `clawhub inspect @owner/slug` enquanto estiver conectado, antes de alterar metadados relacionados à classificação.
...
- Slugs antigos criados por fluxos de renomeação/mesclagem de proprietário são resolvidos para a Skill canônica.
...
- `moderation` é incluído somente quando a Skill está sinalizada ou quando o proprietário está visualizando.
...
- Payloads de solicitação de scan e relatórios baixáveis expiram do repositório de solicitações de scan após a janela de retenção.
...
- Scans publicados fazem write-back somente quando `update: true` e o scan é concluído com sucesso.
...
- Retorna o mesmo formato ZIP dos downloads de solicitação de scan.
...
- `security.signals.dependencyRegistry` é mantido para compatibilidade de resposta v1, mas o scanner de existência de registro de dependências foi aposentado e esta chave é sempre `null`.
...
- `items` deve conter 1-100 pares `{ slug, version }` exclusivos.
- Resultados são por item; uma skill ou versão ausente não faz a resposta inteira falhar.
- A resposta é somente de segurança. Ela não inclui dados do Skill Card, status de cartão gerado, listas de arquivos de artefato nem payloads detalhados de scanner.
- `security.signals` contém somente evidências de apoio no nível de status; use `/scan` ou a página de auditoria de segurança do ClawHub para detalhes completos do scanner.
- `security.signals.dependencyRegistry` é mantido para compatibilidade de resposta v1, mas o scanner de existência de registro de dependências foi aposentado e esta chave é sempre `null`.
...
- Use `/verify` quando precisar do envelope de verificação de Skill Card de uma única skill, `/card` quando precisar do markdown de cartão gerado, e `/scan` quando precisar de dados detalhados de scanner.
...
- `category` (opcional): filtro de categoria de plugin. Compatível apenas quando a
  ...
  aliases legados do filtro v1 estão documentados em `GET /api/v1/plugins`.
...
- Entradas de Skill continuam respaldadas pelo registro de Skill e ainda só podem ser publicadas por meio de `POST /api/v1/skills`.
...
- `isOfficial` (opcional): `true` ou `false`
- `category` (opcional): filtro de categoria de plugin. Compatível apenas quando a
  solicitação está no escopo de pacotes de plugin. Categorias controladas e aliases legados do filtro v1
...
Aliases legados do filtro v1 continuam aceitos em endpoints de leitura:
...
- Skills atuais apoiadas pelo GitHub com uma verificação `clean` ou `suspicious` incluem
...
Exportação em lote das versões públicas mais recentes de Plugin para análise offline.
...
Busca apenas de Plugins entre pacotes code-plugin e bundle-plugin.
...
- Os aliases legados de filtro v1 documentados em `GET /api/v1/plugins` também são
  aceitos.
...
- Controles de ordenação da interface do navegador para busca de Plugin reordenam os resultados de relevância carregados,
...
- Requer um token de API para o proprietário do pacote, um proprietário/administrador do publicador da organização,
...
Retorna o resumo exato de segurança e confiança da versão do pacote para clientes de instalação. Esta é a superfície pública de consumo do OpenClaw para decidir se uma
versão resolvida pode ser instalada.

Autenticação:

- Endpoint de leitura público. Nenhum token de proprietário, publicador, moderador ou administrador é
  obrigatório.
...
- Versões de pacote legadas retornam um artefato `legacy-zip` e uma `downloadUrl`
  ZIP legada.
...
Verificações de prontidão abrangem:
...
Denuncia um pacote para revisão por moderadores. Denúncias são em nível de pacote, opcionalmente
...
Ponto de extremidade de moderador/admin para recebimento de denúncias de pacotes.
...
Ponto de extremidade de proprietário/moderador para visibilidade de moderação de pacotes.
...
Ponto de extremidade de moderador/admin para resolver ou reabrir denúncias de pacotes.
...
Ponto de extremidade de moderador/admin para revisão de versão de pacote.
...
- `revoked`: bloqueado depois que uma versão foi previamente considerada confiável.
...
- As verificações de moderação e acesso a pacote privado ainda se aplicam.
...
Publica uma versão code-plugin ou bundle-plugin.

- Requer autenticação por token Bearer.
- Requer `multipart/form-data`.
- Os campos de formulário permitidos são `payload`, blobs `files` repetidos ou uma referência a tarball
...
- Requisições diretas de publicação multipart são limitadas a 18 MB. Tarballs ClawPack podem
  usar o fluxo de upload-url até o limite de 120 MB para tarballs.
...
- Apenas o publicador da organização `openclaw` e publicadores pessoais dos membros atuais da organização `openclaw`
...
Exclui logicamente / restaura uma skill (proprietário, moderador ou admin).
...
usuário compartilhado legado/publicador pessoal, o ponto de extremidade primeiro o migra para um publicador de organização.
...
chamador como proprietário. Este ponto de extremidade não migra identificadores de usuário/pessoais existentes e
...
- Retorna `409` quando o identificador já está em uso por um publicador, usuário ou publicador pessoal.

### `POST /api/v1/users/reserve`

Somente admin. Reserva slugs raiz e nomes de pacote para um proprietário legítimo sem publicar uma
versão. Nomes de pacote se tornam pacotes privados de espaço reservado sem linhas de versão, para que o mesmo
proprietário possa posteriormente publicar a versão real code-plugin ou bundle-plugin nesse nome.
...
do provedor GitHub; identificadores mutáveis são usados apenas como uma proteção voltada ao operador.
...
Fontes de skill do GitHub têm escopo por publicador e são relatadas como verificadas em vez de reescritas.
...
Bane um usuário e exclui permanentemente as skills pertencentes a ele (somente moderador/administrador).
...
conteúdo (somente administrador). Usa simulação por padrão, a menos que `dryRun` seja `false`.
