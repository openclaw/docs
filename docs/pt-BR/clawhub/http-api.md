...
x-i18n:
    generated_at: "2026-07-04T10:31:15Z"
    model: gpt-5.5
...
As respostas de erro públicas da v1 são texto puro com `content-type: text/plain; charset=utf-8`.
...
- Texto ASCII é tokenizado em limites de palavras e pontuação. Por exemplo, `personal-map` contém um token `map` independente, enquanto `amap-jsapi-skill` contém `amap`, `jsapi` e `skill`; pesquisar por `map`, portanto, dá a `personal-map` uma correspondência lexical mais forte do que `amap-jsapi-skill`.
...
- Coloque os termos que os usuários literalmente pesquisarão no nome de exibição, no resumo e nas tags. Use um token de slug independente apenas quando ele também for uma identidade estável que você queira manter.
...
- Aliases de renomeação preservam a resolução para URLs antigas e instalações que resolvem pelo registro, mas o ranqueamento de busca se baseia nos metadados canônicos da Skill depois que a renomeação foi indexada. Estatísticas existentes permanecem com a Skill.
...
Reporte uma Skill para revisão por moderadores. Relatórios são no nível da Skill, opcionalmente vinculados
...
Escaneamentos de upload local não são mais compatíveis. Solicitações usando
...
- Payloads de solicitação de escaneamento e relatórios baixáveis expiram do armazenamento de solicitações de escaneamento após a janela de retenção.
- Escaneamentos publicados exigem acesso de gerenciamento do proprietário/publicador, ou autoridade de moderador/admin da plataforma.
...
- Trabalhos de escaneamento são assíncronos. Solicitações manuais de escaneamento são priorizadas antes do trabalho normal de publicação/backfill, mas a conclusão ainda depende da disponibilidade dos workers.
...
- Retorna `queue.queuedAhead` e `queue.position` enquanto estiver na fila, para que clientes possam mostrar quantos escaneamentos manuais priorizados estão à frente da solicitação. Filas muito grandes são limitadas e relatadas com `queuedAheadIsEstimate: true`.
...
- `kind` usa `skill` por padrão; use `kind=plugin` para escaneamentos de plugin/pacote.
- Retorna o mesmo formato de ZIP que os downloads de solicitação de escaneamento.

### `POST /api/v1/skills/-/scan/batch`

Rota canônica somente para admins para reescaneamento em lote. Ela aceita o mesmo formato de payload que o legado `POST /api/v1/skills/-/rescan-batch`.

### `POST /api/v1/skills/-/scan/batch/status`

Rota canônica somente para admins para status de lote. Ela aceita `{ "jobIds": ["..."] }` e retorna os mesmos contadores agregados que o legado `POST /api/v1/skills/-/rescan-batch/status`.
...
- `security.signals.dependencyRegistry` é mantido para compatibilidade com respostas v1, mas o scanner de existência do registro de dependências foi aposentado e esta chave é sempre `null`.
...
- `security.signals` contém apenas evidências de apoio no nível de status; use `/scan` ou a página de auditoria de segurança do ClawHub para obter detalhes completos dos scanners.
- `security.signals.dependencyRegistry` é mantido para compatibilidade com respostas v1, mas o scanner de existência do registro de dependências foi aposentado e esta chave é sempre `null`.
...
- Entradas de Skills continuam respaldadas pelo registro de Skills e ainda só podem ser publicadas por meio de `POST /api/v1/skills`.
...
Pesquisa de catálogo unificada entre Skills + pacotes de plugins.
...
- `isOfficial` (opcional): `true` ou `false`
- `category` (opcional): filtro de categoria de plugin. Compatível somente quando a
  solicitação está limitada a pacotes de plugins. Categorias controladas e aliases
...
- Skills atuais respaldadas pelo GitHub com uma varredura `clean` ou `suspicious` incluem
...
Busca exclusiva de Plugin em pacotes code-plugin e bundle-plugin.
...
- `version.vtAnalysis`, `version.llmAnalysis` e `version.staticScan` são
  incluídos quando há dados de varredura.
- Pacotes privados retornam `404`, a menos que o chamador possa ler o publicador proprietário.
...
- Esta é a superfície de resolução do OpenClaw; ela evita adivinhar o formato do arquivo a partir
...
- `revoked`: bloqueado depois que um lançamento foi previamente confiável.
...
skill atual baseada no GitHub com uma verificação `clean` ou `suspicious` e nenhuma versão
hospedada.
...
- Campo opcional do payload: `migrateOwner`. Quando `true` com `ownerHandle`, uma
  ...
  os publicadores atual e de destino. Sem esta opção explícita, alterações de proprietário são
  rejeitadas.
...
- Os campos de formulário permitidos são `payload`, blobs `files` repetidos ou uma referência de tarball
  ...
  fluxo de upload-url. Publicações preparadas por ID de armazenamento também devem incluir o
...
- Corpos JSON e metadados `payload.files` / `payload.artifact` fornecidos pelo chamador
  são rejeitados.
- Solicitações multipart diretas de publicação são limitadas a 18MB. Tarballs ClawPack podem
...
- Somente o publicador da organização `openclaw` e publicadores pessoais dos membros atuais da organização `openclaw`
...
outro publicador. A resposta de exclusão inclui `slugReservedUntil` quando esta expiração se aplica.
...
Somente administrador. Garante que exista um publicador de organização para um identificador. Se o identificador ainda apontar para um
usuário compartilhado legado/publicador pessoal, o endpoint o migra primeiro para um publicador de organização.
Para uma organização recém-criada, forneça `memberHandle`; o administrador em ação não é adicionado como membro.
...
lançamento. Nomes de pacote se tornam pacotes privados de espaço reservado sem linhas de lançamento, para que o mesmo
...
Somente administrador. Recupera um publicador pessoal para um principal substituto verificado de GitHub OAuth
...
A recuperação também migra campos legados `ownerUserId` para as skills,
aliases de slug de skill, pacotes, avisos do inspetor de pacotes e linhas derivadas do resumo de busca do publicador recuperado para que
...
de perfil não possa restaurar a autoridade concorrente do usuário anterior. Cada tabela primária é limitada a
100 linhas por transação de aplicação; recuperações maiores devem primeiro usar uma migração de proprietário retomável.
Fontes de Skills do GitHub têm escopo por publicador e são relatadas como verificadas em vez de reescritas.
...
Se você fizer self-host, disponibilize este arquivo (ou defina `CLAWHUB_REGISTRY` explicitamente; `CLAWDHUB_REGISTRY` legado).
