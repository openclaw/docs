...
summary: Referência da API HTTP (pública + endpoints da CLI + autenticação).
x-i18n:
...
Atalhos de slug da Web são resolvidos entre famílias do registro, mas clientes de API devem usar
...
- Endpoints de escrita autenticados não devem retornar um simples `Unauthorized` quando
...
- Se nenhum IP de cliente confiável estiver disponível, solicitações anônimas usam buckets de fallback
...
- A relevância é mais forte que a popularidade. Uma correspondência precisa de slug ou token de nome de exibição pode superar uma correspondência mais frouxa com engajamento muito maior.
- Texto ASCII é tokenizado em limites de palavra e pontuação. Por exemplo, `personal-map` contém um token `map` independente, enquanto `amap-jsapi-skill` contém `amap`, `jsapi` e `skill`; portanto, pesquisar por `map` dá a `personal-map` uma correspondência lexical mais forte do que `amap-jsapi-skill`.
- A popularidade é escalada logaritmicamente e limitada. Skills de alto engajamento podem ranquear abaixo quando o texto da consulta é uma correspondência mais fraca.
...
- Não renomeie um slug apenas para perseguir uma consulta, a menos que o novo slug seja um nome canônico melhor a longo prazo. Slugs antigos se tornam aliases de redirecionamento, mas a URL canônica, o slug exibido e os futuros resumos de busca usam o novo slug.
- Aliases de renomeação preservam a resolução para URLs antigas e instalações que resolvem pelo registro, mas o ranqueamento de busca se baseia nos metadados canônicos da Skill depois que a renomeação tiver sido indexada. Estatísticas existentes permanecem com a Skill.
- Se uma Skill estiver inesperadamente invisível, verifique primeiro o estado de moderação com `clawhub inspect @owner/slug` enquanto estiver conectado antes de alterar metadados relacionados a ranqueamento.
...
- Slugs antigos criados por fluxos de renomeação/mesclagem de proprietários resolvem para a Skill canônica.
- `metadata.os`: restrições de SO declaradas no frontmatter da Skill (por exemplo, `["macos"]`, `["linux"]`). `null` se não declaradas.
- `metadata.systems`: destinos de sistema Nix (por exemplo, `["aarch64-darwin", "x86_64-linux"]`). `null` se não declarados.
...
Reporte uma Skill para revisão de moderador. Reports são no nível da Skill, opcionalmente vinculados
a uma versão, e alimentam a fila de reports de Skills.
...
Endpoint de moderador/admin para entrada de reports de Skills.
...
Endpoint de moderador/admin para resolver ou reabrir reports de Skills.
...
definir `status` de volta para `open`. Passe `finalAction: "hide"` com um report
...
- `moderation` é um snapshot atual de moderação no nível da habilidade derivado da versão mais recente.
...
Endpoint autenticado de envio para novos trabalhos ClawScan.

Varreduras de upload local não são mais compatíveis. Solicitações usando
...
- Payloads de solicitação de varredura e relatórios baixáveis expiram do armazenamento de solicitações de varredura após a janela de retenção.
- Varreduras publicadas exigem acesso de gerenciamento do proprietário/publicador ou autoridade de moderador/administrador da plataforma.
- Varreduras publicadas gravam de volta somente quando `update: true` e a varredura é concluída com sucesso.
...
- Trabalhos de varredura são assíncronos. Solicitações manuais de varredura são priorizadas antes do trabalho normal de publicação/preenchimento retroativo, mas a conclusão ainda depende da disponibilidade de workers.

### `GET /api/v1/skills/-/scan/{scanId}`

Endpoint autenticado de consulta para uma varredura enviada.

- Retorna status em fila/em execução/bem-sucedido/falho.
- Retorna `queue.queuedAhead` e `queue.position` enquanto estiver em fila, para que os clientes possam mostrar quantas varreduras manuais priorizadas estão à frente da solicitação. Filas muito grandes são limitadas e relatadas com `queuedAheadIsEstimate: true`.
...
- Exige acesso de gerenciamento do proprietário/publicador à habilidade ou ao Plugin, ou autoridade de moderador/administrador da plataforma.
...
- Retorna o mesmo formato ZIP dos downloads de solicitação de varredura.

### `POST /api/v1/skills/-/scan/batch`

Rota canônica exclusiva para administradores para nova varredura em lote. Ela aceita o mesmo formato de payload que a rota legada `POST /api/v1/skills/-/rescan-batch`.

### `POST /api/v1/skills/-/scan/batch/status`

Rota canônica exclusiva para administradores de status de lote. Ela aceita `{ "jobIds": ["..."] }` e retorna os mesmos contadores agregados da rota legada `POST /api/v1/skills/-/rescan-batch/status`.
...
- Identidade da habilidade, identidade do publicador e metadados da versão selecionada são campos de envelope de nível superior (`slug`, `displayName`, `publisherHandle`, `version`, `resolvedFrom`, `tag`, `createdAt`) para que a automação de shell possa lê-los sem desempacotar wrappers aninhados.
- `security` é o veredito ClawScan/segurança de nível superior. A automação deve se basear em `ok`, `decision`, `reasons` e `security.status`.
- `security.signals` contém evidências de scanner de apoio, como `staticScan`, `virusTotal` e `skillSpector`.
- `security.signals.dependencyRegistry` é mantido para compatibilidade de resposta v1, mas o scanner de existência do registro de dependências foi desativado e essa chave é sempre `null`.
...
Retorna vereditos de segurança compactos atuais para versões exatas de habilidades. Este
endpoint de coleção é destinado a clientes que já sabem quais versões de habilidades
...
- `items` deve conter 1-100 pares `{ slug, version }` únicos.
- Os resultados são por item; uma habilidade ou versão ausente não faz a resposta inteira falhar.
- A resposta é somente de segurança. Ela não inclui dados do Skill Card, status do cartão gerado, listas de arquivos de artefato nem payloads detalhados de scanner.
- `security.signals` contém somente evidências de apoio no nível de status; use `/scan` ou a página de auditoria de segurança do ClawHub para obter todos os detalhes do scanner.
- `security.signals.dependencyRegistry` é mantido para compatibilidade de resposta v1, mas o scanner de existência do registro de dependências foi desativado e essa chave é sempre `null`.
...
- Use `/verify` quando precisar do envelope de verificação do Skill Card de uma única habilidade, `/card` quando precisar do markdown do cartão gerado e `/scan` quando precisar de dados detalhados do scanner.
...
- `category` (opcional): filtro de categoria de plugin. Compatível somente quando a
  ...
  aliases de filtro v1 legados estão documentados em `GET /api/v1/plugins`.
...
- As entradas de skill continuam sendo respaldadas pelo registro de skills e ainda podem ser publicadas somente por meio de `POST /api/v1/skills`.
...
- Chamadores autenticados podem ver pacotes privados de publishers aos quais pertencem nos resultados de lista/pesquisa.
...
- `isOfficial` (opcional): `true` ou `false`
- `category` (opcional): filtro de categoria de plugin. Compatível somente quando a
  solicitação está no escopo de pacotes de plugin. Categorias controladas e aliases de filtro v1 legados
...
- Chamadores anônimos veem somente canais de pacotes públicos.
- Chamadores autenticados podem pesquisar pacotes privados de publishers aos quais pertencem.
- `channel=private` retorna somente pacotes que o chamador autenticado pode ler.

### `GET /api/v1/plugins`

Navegação no catálogo somente de Plugins entre pacotes code-plugin e bundle-plugin.
...
Aliases de filtro v1 legados continuam aceitos em endpoints de leitura:
...
`trending` é uma classificação de instalações/downloads de sete dias e não usa totais de todos os tempos.
No endpoint unificado `/api/v1/packages`, ela é somente para plugins; use
...
- Skills atuais com origem no GitHub com varredura `clean` ou `suspicious` incluem
...
- Os controles de ordenação da UI do navegador para pesquisa de plugins reordenam os resultados de relevância carregados,
...
Exclui de forma reversível um pacote e todas as versões.

Observações:

- Exige um token de API para o proprietário do pacote, um proprietário/admin de publicador de organização,
...
- `version.sha256hash` é metadado de compatibilidade obsoleto para clientes antigos. Ele
  aplica hash aos bytes ZIP exatos retornados por `/api/v1/packages/{name}/download`.
...
- Endpoint público de leitura. Nenhum token de proprietário, publicador, moderador ou admin é
  necessário.
...
- `trust.scanStatus` é o status efetivo de confiança derivado de entradas de scanners
  e moderação manual da versão.
- `trust.moderationState` é anulável. Ele é `null` quando não existe moderação manual da versão.
...
- `trust.reasons` é a lista de explicações voltada ao usuário e para auditoria. Códigos de motivo
  ...
  e `package:malicious`.
- `trust.pending` significa que uma ou mais entradas de confiança ainda estão aguardando conclusão.
...
- Este endpoint é exato por versão. Clientes devem chamá-lo depois de resolver a
  ...
  do pacote.
- Pacotes privados retornam `404`, a menos que o chamador possa ler o publicador proprietário.
- Este endpoint é intencionalmente mais restrito que endpoints de moderação de
  proprietários/moderadores. Ele expõe a decisão de instalação e a explicação pública, não
  identidades de relatores, corpos de relatos, evidências privadas ou cronogramas internos de revisão.
...
- Versões de pacotes legados retornam um artefato `legacy-zip` e uma `downloadUrl`
  ZIP legada.
...
- Versões ClawPack transmitem os bytes `.tgz` exatos do npm-pack enviado.
...
Retorna a prontidão calculada para consumo futuro do OpenClaw.
...
Endpoint de moderador/admin para filas de revisão de versões de pacotes.
...
- `open`: versões suspeitas, maliciosas, pendentes, em quarentena, revogadas ou relatadas.
- `blocked`: versões em quarentena, revogadas ou maliciosas.
- `manual`: qualquer versão com uma substituição de moderação manual.
- `all`: qualquer versão com uma substituição manual, estado de varredura não limpo ou relato de pacote.
...
Relata um pacote para revisão por moderador. Relatos são no nível do pacote, opcionalmente
vinculados a uma versão. Eles alimentam a fila de moderação, mas não ocultam automaticamente nem
bloqueiam downloads por si só; moderadores devem usar moderação de versão para
...
- Exige um token de API para o proprietário do pacote, membro publicador,
  moderador ou usuário admin.
...
`finalAction: "revoke"` com uma denúncia confirmada para aplicar moderação à
release no mesmo fluxo auditável.
...
- `revoked`: bloqueada após uma release ter sido considerada confiável anteriormente.

Releases em quarentena e revogadas retornam `403` nas rotas de download de artefatos.
...
- Varreduras VirusTotal pendentes não bloqueiam leituras; releases maliciosas ainda podem ser retidas em outros lugares.
- Pacotes privados retornam `404`, a menos que o chamador possa ler o publicador proprietário.
...
- Arquivos Plugin/pacote são arquivos zip com uma raiz `package/` para que clientes
  OpenClaw antigos continuem funcionando.
...
- Varreduras VirusTotal pendentes não bloqueiam downloads; releases maliciosas retornam `403`.
...
- Cabeçalhos de download incluem o SHA-256 do ClawHub mais metadados de integridade/shasum do npm.
...
Baixa um ZIP de versão de Skill hospedada, ou retorna uma transferência de fonte GitHub para uma
Skill atual baseada em GitHub com uma varredura `clean` ou `suspicious` e sem versão
hospedada.
...
- Transferências de Skill baseadas em GitHub não fazem proxy nem espelham bytes. A resposta JSON
  inclui `sourceRef: "public-github"`, `repo`, `commit`, `path`, `contentHash`
  e `archiveUrl`; o estado de varredura/atual é uma barreira e não é incluído como metadados de payload
  de sucesso.
...
- Preferido: `multipart/form-data` com JSON em `payload` + blobs `files[]`.
...
- Os campos de formulário permitidos são `payload`, blobs `files` repetidos ou uma referência de tarball
  ...
  fluxo upload-url. Publicações com ID de armazenamento em estágio também devem incluir o
...
- Requisições diretas de publicação multipart são limitadas a 18 MB. Tarballs ClawPack podem
...
- Pacotes Plugin exigem `openclaw.plugin.json`. Uploads ClawPack `.tgz` devem
...
- Publicações em nome de terceiros ainda validam a elegibilidade ao canal oficial contra a conta do proprietário de destino.
...
`memberRole` usa `owner` como padrão.
...
Criação autenticada e self-service de publicador de organização. Cria um novo publicador de organização e adiciona o
...
- Retorna `409` quando o handle já é usado por um publicador, usuário ou publicador pessoal.

### `POST /api/v1/users/reserve`

Somente admin. Reserva slugs raiz e nomes de pacote para um proprietário legítimo sem publicar uma
release. Nomes de pacote se tornam pacotes de placeholder privados sem linhas de release, para que o mesmo
...
Somente admin. Recupera um publicador pessoal para um principal GitHub OAuth substituto verificado
sem editar linhas de conta do Convex Auth. A requisição deve nomear ambos os IDs imutáveis de conta
de provedor GitHub; handles mutáveis são usados apenas como uma proteção voltada ao operador.

O endpoint usa simulação por padrão. Aplicar a recuperação exige `dryRun: false` e
`confirmIdentityVerified: true` depois que a equipe verificar independentemente a continuidade entre ambas as
identidades do GitHub. A recuperação falha em modo fechado quando o publicador pessoal atual do usuário de destino
...
aliases de slugs de skills, pacotes, avisos do inspetor de pacotes e linhas derivadas de digest de busca para que
os caminhos de proprietário direto concordem com a nova autoridade do publicador. Uma reserva ativa de handle protegido
para o handle recuperado também é reatribuída ao usuário substituto para que a sincronização posterior de perfil
...
Fontes de skills do GitHub têm escopo de publicador e são relatadas como verificadas, em vez de reescritas.
...
Bane um usuário e exclui permanentemente as skills pertencentes a ele (somente moderador/admin).
...
Se você auto-hospedar, sirva este arquivo (ou defina `CLAWHUB_REGISTRY` explicitamente; legado `CLAWDHUB_REGISTRY`).
