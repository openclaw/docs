...
x-i18n:
    generated_at: "2026-07-05T05:24:53Z"
    model: gpt-5.5
...
Atalhos de slugs da Web são resolvidos entre famílias de registro, mas clientes de API devem usar
...
- Os resultados são retornados em ordem de relevância (similaridade de embeddings + boosts de token de slug/nome exato + um pequeno prior de popularidade).
...
- Texto ASCII é tokenizado em limites de palavras e pontuação. Por exemplo, `personal-map` contém um token autônomo `map`, enquanto `amap-jsapi-skill` contém `amap`, `jsapi` e `skill`; portanto, pesquisar por `map` dá a `personal-map` uma correspondência lexical mais forte do que a `amap-jsapi-skill`.
- Popularidade é escalada logaritmicamente e limitada. Skills com alto engajamento podem ter classificação inferior quando o texto da consulta é uma correspondência mais fraca.
...
- Coloque os termos que os usuários literalmente pesquisarão no nome de exibição, resumo e tags. Use um token de slug autônomo somente quando ele também for uma identidade estável que você queira manter.
- Não renomeie um slug apenas para perseguir uma consulta, a menos que o novo slug seja um nome canônico melhor em longo prazo. Slugs antigos viram aliases de redirecionamento, mas a URL canônica, o slug exibido e os futuros resumos de pesquisa usam o novo slug.
- Aliases de renomeação preservam a resolução para URLs antigas e instalações que resolvem pelo registro, mas a classificação de pesquisa é baseada nos metadados canônicos da Skill depois que a renomeação tiver sido indexada. Estatísticas existentes permanecem com a Skill.
...
- `moderation` é incluído somente quando a Skill está sinalizada ou quando o proprietário está visualizando-a.
...
- Trabalhos de scan são assíncronos. Solicitações manuais de scan são priorizadas à frente do trabalho normal de publicação/preenchimento retroativo, mas a conclusão ainda depende da disponibilidade dos workers.

### `GET /api/v1/skills/-/scan/{scanId}`

Endpoint autenticado de polling para um scan enviado.

- Retorna status de enfileirado/em execução/bem-sucedido/com falha.
...
- `security.signals.dependencyRegistry` é mantido para compatibilidade da resposta v1, mas o scanner de existência de registro de dependências foi aposentado e esta chave é sempre `null`.
...
- A resposta é somente de segurança. Ela não inclui dados de Skill Card, status de cartão gerado, listas de arquivos de artefato nem payloads detalhados de scanners.
- `security.signals` contém somente evidências de suporte no nível de status; use `/scan` ou a página de auditoria de segurança do ClawHub para detalhes completos dos scanners.
- `security.signals.dependencyRegistry` é mantido para compatibilidade da resposta v1, mas o scanner de existência de registro de dependências foi aposentado e esta chave é sempre `null`.
- A ausência de Skill Card não afeta `ok`, `decision` nem `reasons` deste endpoint; clientes devem ler o `skill-card.md` instalado localmente quando precisarem do conteúdo do cartão.
...
- `category` (opcional): filtro de categoria de plugin. Compatível somente quando a
  solicitação está escopada para pacotes de plugin (`/api/v1/plugins`,
...
- As entradas de skill continuam apoiadas pelo registro de skills e ainda só podem ser publicadas por meio de `POST /api/v1/skills`.
- `POST /api/v1/packages` continua sendo apenas para releases de code-plugin e bundle-plugin.
...
- `isOfficial` (opcional): `true` ou `false`
- `category` (opcional): filtro de categoria de plugin. Compatível somente quando a
  solicitação está escopada para pacotes de plugin. Categorias controladas e aliases
...
`trending` é um ranking de instalações/downloads de sete dias e não usa totais de todos os tempos.
No endpoint unificado `/api/v1/packages`, ele é exclusivo para plugins; use
...
- `limit` (opcional): inteiro (1-250), padrão `250`.
- `cursor` (opcional): cursor de paginação da resposta anterior.
- `family` (opcional): `code-plugin` ou `bundle-plugin`. Omitido significa ambas as
  famílias de plugins.
...
- Exige um token de API para o proprietário do pacote, um proprietário/administrador do publicador da organização,
...
- `version.artifact.kind` é `legacy-zip` para arquivos de pacote do mundo antigo ou
  `npm-pack` para lançamentos respaldados pelo ClawPack.
...
- `trust.scanStatus` é o status de confiança efetivo derivado das entradas do scanner
  e da moderação manual de lançamento.
- `trust.moderationState` é anulável. É `null` quando não existe moderação manual de
  lançamento.
- `trust.blockedFromDownload` é o sinal de bloqueio de instalação. O OpenClaw e outros
  ...
  rederivar regras de bloqueio a partir de campos de scanner ou moderação.
...
- Este endpoint é intencionalmente mais restrito que os endpoints de moderação de
  ...
  identidades de denunciantes, corpos de denúncia, evidências privadas ou cronogramas internos de
  revisão.
...
- Esta é a superfície do resolvedor do OpenClaw; ela evita inferir o formato do arquivo a partir de
...
Endpoint de moderador/administrador para filas de revisão de lançamento de pacotes.
...
Endpoint de moderador/admin para ingestão de relatórios de pacote.
...
- `revoked`: bloqueado depois que uma versão foi anteriormente considerada confiável.
...
- `dist.tarball`, `dist.integrity` e `dist.shasum` usam campos compatíveis com npm
  para que os usuários possam apontar o npm para o espelho se optarem por isso.
- Packuments de pacote com escopo dão suporte tanto a `/api/npm/@scope/name` quanto ao caminho de requisição
...
skill atual baseada no GitHub com uma varredura `clean` ou `suspicious` e nenhuma versão
hospedada.
...
- Campo opcional do payload: `migrateOwner`. Quando `true` com `ownerHandle`, uma
  skill existente pode mudar para esse proprietário se o ator for admin/proprietário tanto no publicador
  atual quanto no de destino. Sem essa adesão explícita, alterações de proprietário são
  rejeitadas.
...
- Os campos de formulário permitidos são `payload`, blobs `files` repetidos ou uma referência de tarball
  ...
  fluxo de upload-url. Publicações staged com id de armazenamento também devem incluir o
...
- Requisições multipart diretas de publicação são limitadas a 18 MB. Tarballs ClawPack podem
...
usuário/publicador pessoal compartilhado legado, o endpoint primeiro o migra para um publicador de organização.
...
`confirmIdentityVerified: true` depois que a equipe verifica de forma independente a continuidade entre ambos os
principais do GitHub. A recuperação falha de modo fechado quando o editor pessoal atual do usuário de destino
tem skills, pacotes ou fontes de skill do GitHub.
A recuperação também migra campos legados `ownerUserId` para as skills,
aliases de slug de skill, pacotes, avisos do inspetor de pacotes e linhas derivadas de resumo de pesquisa do editor recuperado, para que
os caminhos de proprietário direto concordem com a nova autoridade do editor. Uma reserva ativa de identificador protegido
para o identificador recuperado também é reatribuída ao usuário substituto, para que uma sincronização de perfil posterior
...
Fontes de skill do GitHub têm escopo por editor e são relatadas como verificadas, em vez de reescritas.
