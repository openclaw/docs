...
x-i18n:
    generated_at: "2026-07-02T17:32:38Z"
    model: gpt-5.5
...
- Se `Retry-After` estiver ausente, use `RateLimit-Reset` (ou calcule a partir de `X-RateLimit-Reset`).
...
- Se nenhum IP confiável de cliente estiver disponível, requisições anônimas usam buckets de fallback
...
- A popularidade é escalada logaritmicamente e limitada. Skills com alto engajamento podem ter classificação mais baixa quando o texto da consulta tem correspondência mais fraca.
...
Relata uma Skill para revisão de moderador. Relatórios são no nível da Skill, opcionalmente vinculados
...
- Trabalhos de scan são assíncronos. Solicitações manuais de scan são priorizadas antes do trabalho normal de publicação/backfill, mas a conclusão ainda depende da disponibilidade dos workers.
...
- Retorna o mesmo formato ZIP dos downloads de solicitações de scan.

### `POST /api/v1/skills/-/scan/batch`

Rota canônica somente para admins de novo scan em lote. Ela aceita o mesmo formato de payload que o legado `POST /api/v1/skills/-/rescan-batch`.

### `POST /api/v1/skills/-/scan/batch/status`

Rota canônica somente para admins de status de lote. Ela aceita `{ "jobIds": ["..."] }` e retorna os mesmos contadores agregados que o legado `POST /api/v1/skills/-/rescan-batch/status`.
...
- A identidade da Skill, a identidade do publicador e os metadados da versão selecionada são campos de envelope de nível superior (`slug`, `displayName`, `publisherHandle`, `version`, `resolvedFrom`, `tag`, `createdAt`) para que automações de shell possam lê-los sem desembrulhar wrappers aninhados.
...
- Use `/verify` quando precisar do envelope de verificação de Skill Card de uma única Skill, `/card` quando precisar do markdown do cartão gerado e `/scan` quando precisar de dados detalhados do scanner.
...
- Entradas de Skills continuam sendo respaldadas pelo registro de Skills e ainda só podem ser publicadas por meio de `POST /api/v1/skills`.
...
Pesquisa de catálogo unificada em Skills + pacotes de plugins.
...
- `isOfficial` (opcional): `true` ou `false`
- `category` (opcional): filtro de categoria de plugin. Compatível somente quando a
  solicitação está restrita a pacotes de plugins. Categorias controladas e aliases
...
- Requer um token de API do proprietário do pacote, de um proprietário/admin de publicador da organização,
...
- Versões ClawPack incluem campos `npmIntegrity`, `npmShasum` e
...
- `trust.blockedFromDownload` é o sinal de bloqueio de instalação. OpenClaw e outros
  ...
  derivar novamente as regras de bloqueio a partir de campos de scanner ou moderação.
...
- Versões legadas de pacote retornam um artefato `legacy-zip` e uma URL legada de ZIP
  `downloadUrl`.
...
- Exige um token de API para o proprietário do pacote, membro publicador, moderador ou
  usuário admin.
...
- `revoked`: bloqueado depois que um lançamento foi considerado confiável anteriormente.
...
- `dist.tarball`, `dist.integrity` e `dist.shasum` usam campos compatíveis com npm
  para que usuários possam apontar npm para o espelho, se quiserem.
...
- Campo opcional do payload: `migrateOwner`. Quando `true` com `ownerHandle`, uma
  skill existente pode se mover para esse proprietário se o ator for admin/proprietário nos publicadores
  atual e de destino. Sem essa adesão explícita, mudanças de proprietário são
  rejeitadas.
...
- Exige autenticação com token Bearer.
...
lançamento. Nomes de pacote tornam-se pacotes privados de placeholder sem linhas de lançamento, para que o mesmo
...
caminhos de proprietário direto concordem com a nova autoridade do publicador. Uma reserva ativa de handle protegido
