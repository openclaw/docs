...
x-i18n:
    generated_at: "2026-07-12T21:29:27Z"
    model: gpt-5.6
...
O ClawHub lê o frontmatter de `SKILL.md` para identificar o nome da Skill,
...
informações de compatibilidade, links para o código-fonte, artefatos e registros de versão.

Quando o OpenClaw instala um plugin do ClawHub, ele verifica os metadados
de compatibilidade anunciados antes da instalação. Os registros de pacotes podem incluir compatibilidade de API,
...
exibem os metadados publicados, os arquivos, a atribuição de origem e o status da verificação.
...
O ClawHub permite a publicação aberta, mas os lançamentos ainda estão sujeitos a barreiras de envio,
verificações automatizadas, denúncias de usuários e ações de moderadores.

As páginas públicas mostram resumos das verificações quando disponíveis. Conteúdos retidos, ocultos
ou bloqueados podem desaparecer da pesquisa pública e dos fluxos de instalação, embora permaneçam
...
downloads. Catálogos de terceiros podem usar essas APIs quando incluem um link para a
