...
x-i18n:
    generated_at: "2026-07-12T21:30:03Z"
    model: gpt-5.6
...
O importador web do GitHub é mais rigoroso do que a publicação/sincronização local. Ele só encontra
...
o nome de exibição no catálogo, portanto, nomes existentes de outros clientes continuam
...
| `requires.bins`    | `string[]` | Binários da CLI que precisam estar todos instalados.                                                                                                        |
| `requires.anyBins` | `string[]` | Binários da CLI dos quais pelo menos um precisa existir.                                                                                                    |
...
A análise de segurança do ClawHub verifica se o que sua Skill declara corresponde ao que ela realmente faz. Se o código fizer referência a `TODOIST_API_KEY`, mas o frontmatter não a declarar em `requires.env`, `primaryEnv` ou `envVars`, a análise sinalizará uma incompatibilidade nos metadados. Manter as declarações corretas ajuda sua Skill a passar pela revisão e ajuda os usuários a entender o que estão instalando.
...
- O texto para embedding inclui `SKILL.md` + até aproximadamente 40 arquivos que não sejam `.md` (limite aplicado na medida do possível).
...
- Os slugs de pacote devem estar em minúsculas e ser compatíveis com npm, por exemplo, `@example.tools/demo-plugin` ou `demo-plugin`.

## Versionamento + tags

- Cada publicação cria uma nova versão (semver).
- Tags são ponteiros de string para uma versão; `latest` é usada com frequência.
...
- Não adicione termos de licença conflitantes ao `SKILL.md`; o ClawHub não oferece suporte à substituição da licença por Skill.

## Skills pagas

- O ClawHub não oferece suporte a Skills pagas, preços por Skill, paywalls ou compartilhamento de receita.
- Não adicione metadados de preços ao `SKILL.md`; eles não fazem parte do formato da Skill e não transformarão uma Skill publicada em uma Skill paga.
