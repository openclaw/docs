...
x-i18n:
    generated_at: "2026-07-01T12:51:46Z"
    model: gpt-5.5
...
O importador web do GitHub é mais rígido do que a publicação/sincronização local. Ele só descobre arquivos
`SKILL.md` ou legados `skills.md` em repositórios públicos, que não sejam forks, pertencentes à
...
Os metadados da habilidade são declarados no frontmatter YAML no início do seu `SKILL.md`. Isso informa ao registro (e à análise de segurança) do que sua habilidade precisa para executar.
...
| `always`           | `boolean`  | Se `true`, a habilidade está sempre ativa (nenhuma instalação explícita é necessária).                                                                              |
| `skillKey`         | `string`   | Substitui a chave de invocação da habilidade.                                                                                                         |
| `emoji`            | `string`   | Emoji de exibição da habilidade.                                                                                                                 |
| `homepage`         | `string`   | URL para a página inicial ou documentação da habilidade.                                                                                                         |
...
| `nix`              | `object`   | Especificação de Plugin Nix (consulte README).                                                                                                                |
...
A análise de segurança do ClawHub verifica se o que sua habilidade declara corresponde ao que ela realmente faz. Se seu código referencia `TODOIST_API_KEY`, mas seu frontmatter não a declara em `requires.env`, `primaryEnv` ou `envVars`, a análise sinalizará uma incompatibilidade de metadados. Manter as declarações precisas ajuda sua habilidade a passar na revisão e ajuda os usuários a entender o que estão instalando.
...
- Escopos de pacote devem corresponder exatamente ao identificador de publicador do ClawHub. Identificadores de publicador podem usar letras minúsculas, números, hifens, pontos e underscores; eles devem começar e terminar com uma letra minúscula ou número.
