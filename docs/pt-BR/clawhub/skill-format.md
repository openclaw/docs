...
x-i18n:
    generated_at: "2026-07-05T04:53:32Z"
    model: gpt-5.5
...
- `description` é usado como o resumo da Skill na interface/busca.
...
### Metadados de tempo de execução (`metadata.openclaw`)

Declare os requisitos de tempo de execução da sua Skill em `metadata.openclaw` (aliases: `metadata.clawdbot`, `metadata.clawdis`).
...
- Se sua skill integrar-se a um serviço de terceiros pago, documente claramente o custo externo e a conta necessária nas instruções da skill e nas declarações de env (`requires.env` para variáveis obrigatórias, ou `envVars` com `required: false` para variáveis opcionais).
