...
x-i18n:
    generated_at: "2026-07-04T03:39:09Z"
    model: gpt-5.5
...
O importador web do GitHub é mais rigoroso que a publicação/sincronização local. Ele descobre apenas arquivos
...
| `requires.bins`    | `string[]` | Binários de CLI que todos precisam estar instalados.                                                                                                        |
...
- Arquivos de script ainda são examinados após o upload; arquivos PowerShell `.ps1`, `.psm1` e `.psd1` são aceitos como texto.
...
- Escopos de pacote precisam corresponder exatamente ao identificador do publicador no ClawHub. Identificadores de publicador podem usar letras minúsculas, números, hifens, pontos e sublinhados; eles precisam começar e terminar com uma letra minúscula ou número.
...
- Se sua Skill integra com um serviço pago de terceiros, documente claramente o custo externo e a conta necessária nas instruções da Skill e nas declarações de ambiente (`requires.env` para variáveis obrigatórias, ou `envVars` com `required: false` para variáveis opcionais).
