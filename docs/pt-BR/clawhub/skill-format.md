...
Estado de instalação no diretório de trabalho (gravado pela CLI):
...
- `description` é usado como o resumo da skill na IU/busca.
...
| `envVars`          | `array`    | Declarações de variáveis de ambiente com `name`, `required` opcional e `description` opcional. Defina `required: false` para vars de ambiente opcionais. |
...
- O texto de embedding inclui `SKILL.md` + até cerca de 40 arquivos que não são `.md` (limite de melhor esforço).
...
- Se sua skill integra com um serviço pago de terceiros, documente claramente o custo externo e a conta necessária nas instruções da skill e nas declarações de env (`requires.env` para variáveis obrigatórias, ou `envVars` com `required: false` para variáveis opcionais).
