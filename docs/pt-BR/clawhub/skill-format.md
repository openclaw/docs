...
x-i18n:
    generated_at: "2026-07-02T13:59:56Z"
    model: gpt-5.5
...
O importador web do GitHub é mais rígido do que a publicação/sincronização local. Ele descobre apenas arquivos
`SKILL.md` ou arquivos legados `skills.md` em repositórios públicos que não sejam forks e pertençam
...
| `primaryEnv`       | `string`   | A variável de ambiente de credencial principal da sua skill.                                                                                                  |
...
A análise de segurança do ClawHub verifica se o que sua skill declara corresponde ao que ela realmente faz. Se seu código referencia `TODOIST_API_KEY`, mas seu frontmatter não a declara em `requires.env`, `primaryEnv` ou `envVars`, a análise sinalizará uma incompatibilidade de metadados. Manter as declarações precisas ajuda sua skill a passar pela revisão e ajuda os usuários a entenderem o que estão instalando.
...
- O texto para embeddings inclui `SKILL.md` + até cerca de 40 arquivos que não sejam `.md` (limite de melhor esforço).
...
- Não adicione metadados de preço a `SKILL.md`; isso não faz parte do formato da skill e não tornará paga uma skill publicada.
