...
- Escopos de pacote precisam corresponder exatamente ao identificador do publicador no ClawHub. Identificadores de publicador podem usar letras minúsculas, números, hífens, pontos e underscores; eles precisam começar e terminar com uma letra minúscula ou número.
...
- Se sua Skill integra com um serviço pago de terceiros, documente claramente o custo externo e a conta exigida nas instruções da Skill e nas declarações de env (`requires.env` para variáveis obrigatórias, ou `envVars` com `required: false` para variáveis opcionais).
