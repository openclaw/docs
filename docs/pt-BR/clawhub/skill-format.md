...
x-i18n:
    generated_at: "2026-06-28T05:31:46Z"
    model: gpt-5.5
...
O importador web do GitHub é mais rigoroso que a publicação/sincronização local. Ele só descobre
arquivos `SKILL.md` ou `skills.md` legados em repositórios públicos, que não sejam forks, pertencentes à
...
| `primaryEnv`       | `string`   | A variável de ambiente da credencial principal da sua skill.                                                                                                  |
...
| `skillKey`         | `string`   | Sobrescreve a chave de invocação da skill.                                                                                                                    |
...
A análise de segurança do ClawHub verifica se o que sua skill declara corresponde ao que ela realmente faz. Se seu código referencia `TODOIST_API_KEY`, mas seu frontmatter não a declara em `requires.env`, `primaryEnv` ou `envVars`, a análise sinalizará uma incompatibilidade de metadados. Manter as declarações precisas ajuda sua skill a passar na revisão e ajuda os usuários a entenderem o que estão instalando.
...
- Escopos de pacote devem corresponder exatamente ao handle do publicador no ClawHub. Handles de publicador podem usar letras minúsculas, números, hifens, pontos e underscores; eles devem começar e terminar com uma letra minúscula ou número.
...
- Não adicione metadados de preços a `SKILL.md`; isso não faz parte do formato de skill e não tornará uma skill publicada paga.
- Se sua skill se integra a um serviço de terceiros pago, documente claramente o custo externo e a conta necessária nas instruções da skill e nas declarações de env (`requires.env` para variáveis obrigatórias ou `envVars` com `required: false` para variáveis opcionais).
