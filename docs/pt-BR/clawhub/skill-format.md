...
x-i18n:
    generated_at: "2026-07-03T02:44:28Z"
    model: gpt-5.5
...
A análise de segurança do ClawHub verifica se o que sua skill declara corresponde ao que ela realmente faz. Se seu código referencia `TODOIST_API_KEY`, mas seu frontmatter não a declara em `requires.env`, `primaryEnv` ou `envVars`, a análise sinalizará uma incompatibilidade de metadados. Manter as declarações corretas ajuda sua skill a passar na revisão e ajuda os usuários a entenderem o que estão instalando.
...
- O texto de embedding inclui `SKILL.md` + até cerca de 40 arquivos que não sejam `.md` (limite por melhor esforço).
...
- Slugs de pacote precisam estar em minúsculas e ser compatíveis com npm, por exemplo `@example.tools/demo-plugin` ou `demo-plugin`.
...
- Se sua skill se integra a um serviço de terceiros pago, documente claramente o custo externo e a conta exigida nas instruções da skill e nas declarações de ambiente (`requires.env` para variáveis obrigatórias, ou `envVars` com `required: false` para variáveis opcionais).
