...
- Tipos de conteúdo que começam com `text/` são tratados como texto; mais uma pequena lista permitida (JSON/YAML/TOML/JS/TS/Markdown/SVG).

Limites (lado do servidor):

- Tamanho total do pacote: 50 MB.
- O texto de incorporação inclui `SKILL.md` + até cerca de 40 arquivos não `.md` (limite de melhor esforço).
...
- Se sua skill se integra a um serviço de terceiros pago, documente claramente o custo externo e a conta exigida nas instruções da skill e nas declarações de env (`requires.env` para variáveis obrigatórias, ou `envVars` com `required: false` para variáveis opcionais).
