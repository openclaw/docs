...
- `.clawhubignore` (padrões de ignorar para publicação/sincronização, `.clawdhubignore` legado)
...
- `description` é usada como o resumo da skill na UI/busca.
...
- O texto para embedding inclui `SKILL.md` + até aproximadamente 40 arquivos que não sejam `.md` (limite de melhor esforço).

## Slugs

- Derivados do nome da pasta por padrão.
- Devem estar em minúsculas e serem seguros para URL: `^[a-z0-9][a-z0-9-]*$`.
