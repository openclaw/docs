...
x-i18n:
    generated_at: "2026-07-02T22:24:58Z"
    model: gpt-5.5
...
O importador web do GitHub é mais rigoroso do que a publicação/sincronização local. Ele só descobre
...
| `always`           | `boolean`  | Se `true`, a skill está sempre ativa (sem necessidade de instalação explícita).                                                           |
...
| `config`           | `object`   | Especificação de configuração Clawdbot (veja README).                                                                                     |
...
A análise de segurança do ClawHub verifica se o que sua skill declara corresponde ao que ela realmente faz. Se o seu código referenciar `TODOIST_API_KEY`, mas seu frontmatter não a declarar em `requires.env`, `primaryEnv` ou `envVars`, a análise sinalizará uma incompatibilidade de metadados. Manter declarações precisas ajuda sua skill a passar na revisão e ajuda os usuários a entender o que estão instalando.
...
- Escopos de pacote devem corresponder exatamente ao handle do publicador no ClawHub. Handles de publicador podem usar letras minúsculas, números, hífens, pontos e underscores; eles devem começar e terminar com uma letra minúscula ou número.
...
- Não adicione metadados de preço a `SKILL.md`; isso não faz parte do formato de skill e não fará uma skill publicada se tornar paga.
- Se sua skill se integrar a um serviço pago de terceiros, documente claramente o custo externo e a conta exigida nas instruções da skill e nas declarações de env (`requires.env` para variáveis obrigatórias, ou `envVars` com `required: false` para variáveis opcionais).
