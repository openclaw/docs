...
O importador web do GitHub é mais rígido que a publicação/sincronização local. Ele só descobre
...
| `always`           | `boolean`  | Se `true`, a skill fica sempre ativa (nenhuma instalação explícita é necessária).                                                                       |
...
- Escopos de pacote precisam corresponder exatamente ao identificador do publisher do ClawHub. Identificadores de publisher podem usar letras minúsculas, números, hifens, pontos e sublinhados; eles precisam começar e terminar com uma letra minúscula ou número.
