...
O ClawHub usa **telemetria mínima** para calcular **contagens de instalações** (o que está realmente em uso) e viabilizar melhor ordenação/filtragem.
...
A cada `clawhub sync`, a CLI reporta uma **captura completa** do que encontrou, agrupada por raiz de varredura (“pasta/raiz”).

Para cada raiz, armazenamos:

- `rootId`: um **hash SHA-256** do caminho canônico da raiz (o servidor nunca vê o caminho bruto).
- `label`: um rótulo legível derivado dos dois últimos segmentos do caminho (caminhos da pasta inicial são mostrados com `~`).
...
- Nenhum rastreamento de skills que não foram enviadas ao registro (slugs desconhecidos são ignorados).
...
- Se uma skill desaparecer de uma raiz na próxima sincronização, marcamos sua remoção para essa raiz.
...
- Mostra as raízes exatas + as skills instaladas que armazenamos.
