...
Isso é baseado no comando `clawhub sync` da CLI.

## Quando a telemetria é coletada

A telemetria só é enviada quando:

- Você está **autenticado** na CLI (já exigimos autenticação para fluxos de sync/publish).
...
Se você fizer sync a partir de várias pastas, tratamos cada raiz de varredura independentemente. Uma Skill está “atualmente instalada” se existir em **qualquer** raiz ativa.

### Detecção de desinstalação

Como `sync` reporta o conjunto completo por raiz:

- Se uma Skill desaparecer de uma raiz no próximo sync, marcamos que ela foi removida dessa raiz.
