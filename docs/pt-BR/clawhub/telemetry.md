...
x-i18n:
    generated_at: "2026-05-12T15:42:58Z"
    model: gpt-5.5
    provider: openai
...
O ClawHub usa **telemetria mínima** para calcular **contagens de instalações** (o que está realmente em uso) e habilitar uma ordenação/filtragem melhor.
...
Em cada `clawhub sync`, a CLI informa um **snapshot completo** do que encontrou, agrupado pela raiz de varredura (“pasta/raiz”).

Para cada raiz, armazenamos:

- `rootId`: um **hash SHA-256** do caminho canônico da raiz (o servidor nunca vê o caminho bruto).
- `label`: um rótulo legível por humanos derivado dos dois últimos segmentos do caminho (caminhos da pasta inicial são mostrados com `~`).
- `firstSeenAt`, `lastSeenAt`, `expiredAt` opcional.

Para cada skill encontrada sob uma raiz, armazenamos:

- `skillId` (resolvido pelo slug; somente skills existentes no registro são rastreadas).
- `firstSeenAt`, `lastSeenAt`.
- `lastVersion` (melhor esforço; atualmente a versão correspondente no registro, se conhecida).
- `removedAt` opcional quando uma instalação informada anteriormente desaparece de uma raiz.
...
- `installsAllTime`: usuários únicos que já informaram que a skill estava instalada.

### Várias raízes

Se você sincronizar a partir de várias pastas, tratamos cada raiz de varredura de forma independente. Uma skill está “instalada atualmente” se existir em **qualquer** raiz ativa.

### Detecção de desinstalação

Como `sync` informa o conjunto completo por raiz:
...
Raízes que não informam telemetria por **120 dias** são marcadas como obsoletas, e suas instalações deixam de contar para `installsCurrent`.
Isso é avaliado sob demanda (no próximo relatório de telemetria) para evitar jobs em segundo plano.

## Transparência + controles do usuário

O ClawHub oferece uma aba privada “Instaladas” no seu próprio perfil:
