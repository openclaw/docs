...
ClawHub usa **telemetria mínima** para calcular **contagens de instalação** (o que está realmente em uso) e permitir melhor ordenação/filtragem.
...
Se você não estiver conectado, nada é reportado.

## O que coletamos

A cada `clawhub sync`, a CLI reporta um **instantâneo completo** do que encontrou, agrupado por raiz de varredura (“pasta/raiz”).

Para cada raiz, armazenamos:

- `rootId`: um **hash SHA-256** do caminho canônico da raiz (o servidor nunca vê o caminho bruto).
- `label`: um rótulo legível por humanos derivado dos dois últimos segmentos do caminho (caminhos da pasta pessoal são exibidos com `~`).
...
Isso é avaliado de forma preguiçosa (no próximo relatório de telemetria) para evitar trabalhos em segundo plano.

## Transparência + controles do usuário

ClawHub fornece uma aba privada “Instalados” no seu próprio perfil:

- Mostra as raízes + skills instaladas exatas que armazenamos.
...
Todas as outras pessoas veem apenas **contadores de instalação agregados**; ninguém mais consegue ver suas raízes/pastas.
