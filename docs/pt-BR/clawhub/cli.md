...
summary: 'Referência da CLI: comandos, sinalizadores, configuração, lockfile, comportamento de sincronização.'
x-i18n:
    generated_at: "2026-05-12T00:56:42Z"
    model: gpt-5.5
    provider: openai
...
- Transferências para identificadores de org/publicador são aplicadas imediatamente apenas quando o ator tem
...
- Use isto para plugins e outras entradas de famílias de pacotes; o `search` de nível superior permanece a superfície de pesquisa de Skills.
- Flags:
...
- Exclui temporariamente um pacote e todos os releases.
- Requer o proprietário do pacote, um proprietário/admin de publicador da org, moderador da plataforma
...
- Restaura um pacote excluído temporariamente e seus releases.
- Requer o proprietário do pacote, um proprietário/admin de publicador da org, moderador da plataforma
  ou admin da plataforma.
...
- Mostra o estado atual de varredura do pacote, a contagem de denúncias abertas, o estado de moderação
  manual do release mais recente, o estado de bloqueio de download e os motivos de moderação.
- Sinalizadores:
  - `--json`: saída legível por máquina.
...
- Relata bloqueadores para status oficial, disponibilidade do ClawPack, digest do artefato,
  proveniência do código-fonte, compatibilidade com o OpenClaw, destinos de host, metadados de ambiente
...
- Fontes `.tgz` são tratadas como ClawPack. A CLI envia os bytes npm-pack exatos
...
- Para fontes do GitHub, a atribuição de origem é preenchida automaticamente com o repositório, commit resolvido, ref e subcaminho.
...
- `--owner <handle>` publica sob o identificador de publicador de usuário ou org quando o ator tem acesso de publicador.
...
- Sinalizadores existentes (`--family`, `--name`, `--version`, `--source-repo`, `--source-commit`, `--source-ref`, `--source-path`) continuam funcionando como substituições.
...
a atribuição de origem antes de criar um release ativo:
...
- Se você estiver usando uma versão mais antiga da CLI `clawhub`, atualize antes de publicar para que
  as verificações locais de pré-validação sejam executadas antes do upload.
