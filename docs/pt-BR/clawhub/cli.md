...
summary: 'Referência da CLI: comandos, opções, configuração, arquivo de bloqueio, comportamento de sincronização.'
x-i18n:
...
Isso é necessário em sistemas em que conexões diretas de saída são bloqueadas
...
Armazena seu token de API + a URL do registro em cache.
...
- A busca favorece correspondências exatas de tokens de slug/nome antes da popularidade de downloads. Um token de slug independente, como `map`, corresponde a `personal-map` mais fortemente do que à substring dentro de `amap`.
...
- Habilidades fixadas são ignoradas por `update --all` e rejeitadas por `update <slug>` direto.
...
- `--owner <handle>` publica sob o identificador de editor de uma organização/usuário quando o
  ator tem acesso de editor.
- `--migrate-owner` move uma habilidade existente para `--owner` ao publicar uma nova
  versão. Exige acesso de admin/proprietário em ambos os editores.
...
- Habilidades publicadas são livres para usar, modificar e redistribuir sem atribuição.
- ClawHub não oferece suporte a habilidades pagas nem a preços por habilidade.
...
- Exclui uma habilidade de forma reversível (proprietário, moderador ou admin).
...
- Restaura uma habilidade oculta (proprietário, moderador ou admin).
...
- Oculta uma habilidade (proprietário, moderador ou admin).
- Alias para `delete`.

### `unhide <slug>`

- Desoculta uma habilidade (proprietário, moderador ou admin).
- Alias para `undelete`.

### `skill rename <slug> <new-slug>`

- Renomeia uma habilidade de sua propriedade e mantém o slug anterior como um alias de redirecionamento.
- Chama `POST /api/v1/skills/{slug}/rename`.
- `--yes` ignora a confirmação.

### `skill merge <source-slug> <target-slug>`

- Mescla uma habilidade de sua propriedade em outra habilidade de sua propriedade.
...
- Transferências para identificadores de organização/editor são aplicadas imediatamente apenas quando o ator tem
  acesso de admin tanto ao proprietário atual quanto ao editor de destino.
- Subcomandos:
...
- Use isto para plugins e outras entradas da família de pacotes; `search` no nível superior permanece como a superfície de pesquisa de habilidades.
- Flags:
...
- Para artefatos ClawPack npm-pack, também verifica a integridade `sha512` do npm,
  o shasum do npm e o nome/versão do `package.json` do tarball.
...
- Calcula o SHA-256 do ClawHub, a integridade `sha512` do npm e o shasum do npm para um artefato
  local.
...
- Requer o proprietário do pacote, um proprietário/admin de publicador de organização, moderador da plataforma
...
- Restaura um pacote excluído reversivelmente e seus lançamentos.
- Requer o proprietário do pacote, um proprietário/admin de publicador de organização, moderador da plataforma
  ou admin da plataforma.
...
- Requer acesso de admin tanto ao proprietário atual do pacote quanto ao publicador
...
- Mostra o estado atual de verificação do pacote, contagem de denúncias abertas, estado de moderação
  manual do lançamento mais recente, estado de bloqueio de download e motivos de moderação.
- Flags:
  - `--json`: saída legível por máquina.
...
- Os metadados são detectados automaticamente de `package.json`, `openclaw.plugin.json` e
  marcadores reais de bundle OpenClaw, como `.codex-plugin/plugin.json`,
...
- Para pastas locais, a atribuição de fonte é detectada automaticamente a partir do git local quando o remote de origem aponta para o GitHub.
...
a atribuição de fonte antes de criar um lançamento real:
...
- Fixe o workflow reutilizável a uma tag estável ou SHA de commit completo. Não execute publicação de lançamentos a partir de `@main`.
...
- Publicação confiável sem um secret funciona apenas em `workflow_dispatch`; pushes de tags ainda precisam de `clawhub_token`.
- Mantenha `clawhub_token` disponível para primeira publicação, pacotes não confiáveis ou publicações de emergência.
...
- Flags:
  ...
  - `--all` enviar sem solicitar confirmação
