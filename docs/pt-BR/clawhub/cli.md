...
para HTTP simples. `NO_PROXY` / `no_proxy` é respeitado para contornar o proxy para
hosts ou domínios específicos.

Isso é necessário em sistemas onde conexões diretas de saída estão bloqueadas
...
- Habilidades fixadas também rejeitam `install --force`, para que os bytes locais não sejam substituídos por acidente.
...
- O ClawHub não oferece suporte a habilidades pagas nem a preços por habilidade.
...
- Exclui logicamente uma habilidade (proprietário, moderador ou administrador).
...
- Use isto para plugins e outras entradas de famílias de pacotes; `search` no nível superior continua sendo a superfície de pesquisa de habilidades.
- Flags:
...
- Requer o proprietário do pacote, um proprietário/administrador do publicador da org, moderador da plataforma
...
- Requer acesso de administrador tanto ao proprietário atual do pacote quanto ao publicador de destino,
  exceto quando executado por um administrador da plataforma.
...
- Mostra o estado atual de verificação do pacote, a contagem de denúncias abertas, o estado de moderação manual da versão mais recente,
  o estado de bloqueio de download e os motivos de moderação.
- Opções:
  - `--json`: saída legível por máquina.
...
- Fontes `.tgz` são tratadas como ClawPack. A CLI envia os bytes exatos do npm-pack
  e usa o conteúdo `package/` extraído apenas para validação e preenchimento prévio de
  metadados.
...
- Para pastas locais, a atribuição de fonte é detectada automaticamente a partir do git local quando o remote origin aponta para o GitHub.
...
- `--dry-run` pré-visualiza a carga de publicação resolvida sem enviar.
...
- `--clawscan-note <text>` adiciona uma observação do ClawScan. Essa observação dá ao ClawScan
  contexto para comportamentos que poderiam parecer incomuns, como acesso à rede,
  acesso ao host nativo ou credenciais específicas de provedor. A observação é armazenada na
  versão publicada.
- Nomes de pacote com escopo devem corresponder ao proprietário selecionado. Consulte `docs/publishing.md`.
- Opções existentes (`--family`, `--name`, `--version`, `--source-repo`, `--source-commit`, `--source-ref`, `--source-path`) ainda funcionam como substituições.
...
a atribuição de fonte antes de criar uma versão ativa:
...
- `openclaw.hostTargets` e `openclaw.environment` são metadados opcionais.
  O ClawHub pode exibi-los quando presentes, mas eles não são necessários para publicar.
...
- Opções:
  ...
  - `--all` enviar sem pedir confirmação
