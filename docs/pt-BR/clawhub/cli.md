...
- `--dir <dir>`: diretório de instalação sob o diretório de trabalho (padrão: `skills`)
...
Quando qualquer uma dessas variáveis está definida, a CLI encaminha solicitações de saída pelo
...
- `--file <path>`: busca conteúdo bruto do arquivo (apenas arquivos de texto; limite de 200 KB).
- `--json`: saída legível por máquina.
...
- Use isso para plugins e outras entradas da família de pacotes; `search` de nível superior continua sendo a superfície de busca de Skills.
- Flags:
...
- Use isso para metadados de Plugin, compatibilidade, verificação, origem e inspeção de versão/arquivo.
- `--version <version>`: inspeciona uma versão específica (padrão: mais recente).
- `--tag <tag>`: inspeciona uma versão marcada (por exemplo, `latest`).
- `--versions`: lista o histórico de versões (primeira página).
...
- Exclui temporariamente um pacote e todos os lançamentos.
- Exige o proprietário do pacote, um proprietário/administrador de publicador de org, moderador da plataforma
...
- Restaura um pacote e lançamentos excluídos temporariamente.
- Exige o proprietário do pacote, um proprietário/administrador de publicador de org, moderador da plataforma
  ou administrador da plataforma.
...
- Exige acesso de administrador ao proprietário atual do pacote e ao publicador
  de destino, salvo quando executado por um administrador da plataforma.
- Nomes de pacote com escopo devem ser transferidos para o proprietário de escopo correspondente.
...
- Denúncias são no nível do pacote, podem ser vinculadas a uma versão opcionalmente
  e ficam visíveis para os moderadores revisarem.
...
- Mostra o estado atual da varredura do pacote, a contagem de denúncias abertas, o estado de moderação
  manual da versão mais recente, o estado de bloqueio de downloads e os motivos de moderação.
- Flags:
  - `--json`: saída legível por máquina.
...
- Fontes `.tgz` são tratadas como ClawPack. A CLI envia exatamente os bytes do npm-pack
...
- Para pastas locais, a atribuição de origem é detectada automaticamente pelo git local quando o remote origin aponta para o GitHub.
...
- `--clawscan-note <text>` adiciona uma nota do ClawScan. Essa nota dá contexto ao ClawScan
  sobre comportamentos que poderiam parecer incomuns, como acesso à rede,
  acesso nativo ao host ou credenciais específicas de provedor. A nota é armazenada no
...
a atribuição de origem antes de criar um lançamento ativo:
...
- O fluxo de trabalho reutilizável usa o repositório chamador como `source` por padrão.
...
- Fixe o fluxo de trabalho reutilizável em uma tag estável ou SHA de commit completo. Não execute publicação de lançamento a partir de `@main`.
...
- O fluxo de trabalho envia o resultado JSON como artefato e o expõe como outputs do fluxo de trabalho.
...
- Adiciona automaticamente raízes de Skill do Clawdbot quando `~/.clawdbot/clawdbot.json` está presente:
...
- Enviada durante `sync` quando autenticado, salvo se `CLAWHUB_DISABLE_TELEMETRY=1` (legado `CLAWDHUB_DISABLE_TELEMETRY=1`).
