...
- Downloads são um pequeno fator prévio de popularidade, não uma garantia de posicionamento no topo.
- Se uma Skill deveria aparecer, mas não aparece, execute `clawhub inspect <slug>` enquanto estiver autenticado para verificar diagnósticos de moderação visíveis ao proprietário antes de renomear metadados.
...
- Solicita uma nova verificação de segurança para a versão mais recente da skill publicada.
- Proprietários e administradores de publicadores podem verificar novamente suas próprias Skills até o limite
  de recuperação por versão.
- Moderadores e administradores da plataforma podem verificar novamente qualquer skill e não são bloqueados pelo
  limite de recuperação do proprietário, embora apenas uma nova verificação possa ser executada por vez por versão.
...
- Use isto para plugins e outras entradas de famílias de pacotes; `search` de nível superior continua sendo a superfície de pesquisa de Skills.
- Flags:
...
- Solicita uma nova varredura de segurança para a versão publicada mais recente do pacote.
- Proprietários e administradores do publicador podem fazer nova varredura de seus próprios pacotes até o limite de
  recuperação por versão.
- Moderadores e administradores da plataforma podem fazer nova varredura de qualquer pacote e não são bloqueados pelo
...
- Comando autenticado para reportar um pacote aos moderadores.
- Chama `POST /api/v1/packages/{name}/report`.
- Relatórios são em nível de pacote, opcionalmente vinculados a uma versão, e ficam visíveis
  para revisão pelos moderadores.
- Relatórios não ocultam pacotes automaticamente nem bloqueiam downloads por conta própria.
- Flags:
  - `--version <version>`: versão opcional do pacote a ser anexada ao relatório.
  - `--reason <text>`: motivo obrigatório do relatório.
  - `--json`: saída legível por máquina.
...
- Comando de proprietário/publicador para recorrer da moderação de versão.
...
- Comando de proprietário para verificar a visibilidade da moderação do pacote.
- Chama `GET /api/v1/packages/{name}/moderation`.
- Mostra o estado atual da varredura do pacote, contagem de relatórios abertos, estado de moderação manual da versão
  mais recente, estado de bloqueio de download e motivos de moderação.
- Flags:
  - `--json`: saída legível por máquina.
...
- Repositórios privados do GitHub requerem `GITHUB_TOKEN`.
...
- Se você estiver usando uma versão mais antiga da CLI `clawhub`, atualize antes de publicar para que
  as verificações locais prévias sejam executadas antes do envio.
...
- `pull_request` deve usar `dry_run: true` para que o CI permaneça sem efeitos colaterais.
