...
summary: 'Referência da CLI: comandos, flags, configuração, arquivo de bloqueio, comportamento de sincronização.'
x-i18n:
...
- Remove do arquivo de bloqueio a fixação de uma skill instalada, para que atualizações futuras possam modificá-la.
...
- O ClawHub não oferece suporte a Skills pagas ou preços por skill.
...
- Transferências para identificadores de organização/publicador são aplicadas imediatamente apenas quando o ator tem
  acesso de admin ao proprietário atual e ao publicador de destino.
- Subcomandos:
...
- Use isso para metadados de Plugin, compatibilidade, verificação, fonte e inspeção de versão/arquivo.
...
- Com flags de digest direto, verifica sem uma consulta de rede.
- Flags:
...
- Requer o proprietário do pacote, um proprietário/administrador de publicador da organização, moderador da plataforma
...
- Comando para proprietários verificarem a visibilidade de moderação do pacote.
- Chama `GET /api/v1/packages/{name}/moderation`.
- Mostra o estado atual da varredura do pacote, contagem de denúncias abertas, estado de moderação manual
  da versão mais recente, estado de bloqueio de downloads e motivos de moderação.
- Flags:
  - `--json`: saída legível por máquina.
...
- `--clawscan-note <text>` adiciona uma nota do ClawScan. Essa nota dá contexto ao ClawScan
  sobre comportamento que poderia parecer incomum, como acesso à rede,
  acesso ao host nativo ou credenciais específicas de provedor. A nota é armazenada na
  versão publicada.
...
atribuição de fonte antes de criar uma versão ativa:
...
- Se você estiver usando uma versão mais antiga da CLI `clawhub`, atualize antes de publicar para que
  as verificações locais de pré-voo sejam executadas antes do upload.
...
- O workflow reutilizável usa como padrão `source` definido para o repositório chamador.
...
- Fixe o workflow reutilizável em uma tag estável ou em um SHA de commit completo. Não execute publicação de release a partir de `@main`.
- `pull_request` deve usar `dry_run: true` para que o CI não polua o ambiente.
