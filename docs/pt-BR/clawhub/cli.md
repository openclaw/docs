...
x-i18n:
    generated_at: "2026-07-02T17:32:43Z"
    model: gpt-5.5
...
- `--workdir <dir>`: diretório de trabalho (padrão: cwd; recua para o workspace do Clawdbot se configurado)
...
- Padrão: abre o navegador em `<site>/cli/auth` e conclui por callback de loopback.
- Sem interface: `clawhub login --token clh_...`
- Interativo remoto/sem interface: `clawhub login --device` imprime um código e aguarda enquanto você o autoriza em `<site>/cli/device`.
...
- Útil para canalizar um token de login local para comandos de configuração de segredos de CI.
...
- ClawHub não oferece suporte a Skills pagas nem a preços por skill.
...
- Varreduras são assíncronas e podem levar tempo para concluir. Enquanto estão na fila, o spinner do terminal mostra a posição atual priorizada da varredura e quantas varreduras estão à frente.
...
- Varreduras de caminho local não são mais aceitas. Envie uma nova versão e depois use `scan download` para recuperar os resultados de varredura armazenados para essa versão enviada.
...
- Exige `clawhub login`.
- Baixa o ZIP de relatório de varredura armazenado para uma versão enviada de skill ou Plugin, incluindo versões que foram bloqueadas ou ocultadas por verificações de segurança do ClawHub.
...
- `owner` mapeia para a flag CLI `--owner`; omita para publicar como o usuário autenticado.
...
- `--version <version>` exclui permanentemente uma versão possuída que não é a mais recente por meio de uma rota com falha fechada
  e específica da versão.
  Versões excluídas não podem ser restauradas nem republicadas. Publique uma substituição antes de excluir a
  versão mais recente atual. A equipe da plataforma não ignora a propriedade nesse fluxo somente de versão.
...
- Com flags de resumo diretas, verifica sem consulta de rede.
- Flags:
...
- Executa o Plugin Inspector integrado da CLI do ClawHub contra uma pasta de pacote de plugin
  local.
...
- `--version <version>` exclui permanentemente uma versão possuída que não é a mais recente por meio de uma rota com falha fechada
  e específica da versão.
  Versões excluídas não podem ser restauradas nem republicadas. Publique uma substituição antes de excluir a
  versão mais recente atual. Esse fluxo somente de versão exige o proprietário do pacote ou um administrador do publicador da organização;
...
- Mostra o estado atual de verificação do pacote, a contagem de denúncias abertas, o estado de moderação manual da versão
  mais recente, o estado de bloqueio de download e os motivos de moderação.
- Flags:
  - `--json`: saída legível por máquina.
...
- Relata bloqueadores para status oficial, disponibilidade do ClawPack, resumo do artefato,
  procedência da origem, compatibilidade com OpenClaw, destinos de host, metadados de ambiente
...
- O identificador é normalizado para letras minúsculas e pode ser passado com ou sem `@`.
...
- `--dry-run` pré-visualiza a carga útil de publicação resolvida sem enviar.
...
- Se você estiver usando uma versão mais antiga da CLI `clawhub`, atualize antes de publicar para que
  as verificações preliminares locais sejam executadas antes do envio.
- Se a validação relatar um código de remediação, consulte
  [Correções de validação de Plugin](/clawhub/plugin-validation-fixes).

#### GitHub Actions

ClawHub também distribui um workflow reutilizável oficial em
...
- Anexa ou substitui a configuração de publicador confiável do GitHub Actions para um pacote
  existente.
- O pacote deve ser criado primeiro por meio da publicação `clawhub package publish`
  manual normal ou autenticada por token.
- Depois que a configuração for definida, publicações futuras compatíveis pelo GitHub Actions podem usar
...
- ClawHub verifica o repositório GitHub configurado quando este comando é executado.
  Repositórios públicos podem ser verificados por metadados públicos do GitHub. Repositórios
  privados exigem que ClawHub tenha acesso do GitHub a esse repositório, por
...
- Use isto como rollback se o pin de workflow, repositório ou ambiente precisar ser
...
- Enviada depois de `clawhub install <slug>` quando conectado, a menos que
