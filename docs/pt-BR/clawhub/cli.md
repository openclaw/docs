...
x-i18n:
    generated_at: "2026-05-11T20:23:19Z"
    model: gpt-5.5
    provider: openai
...
- A busca favorece correspondências exatas de token de slug/nome antes da popularidade por downloads. Um token de slug isolado como `map` corresponde a `personal-map` com mais força do que à substring dentro de `amap`.
- Downloads são um pequeno indicativo prévio de popularidade, não uma garantia de posição no topo.
- Se um Skill deveria aparecer mas não aparece, execute `clawhub inspect <slug>` enquanto estiver logado para verificar diagnósticos de moderação visíveis ao proprietário antes de renomear metadados.
...
- Busca metadados do Skill e arquivos de versão sem instalar.
...
- Recusa sobrescrever Skills fixados; execute `clawhub unpin <slug>` primeiro.
- Escreve:
...
- Solicita uma nova varredura de segurança para a versão de skill publicada mais recente.
- Proprietários e administradores do publicador podem fazer nova varredura de suas próprias skills até o limite de
  recuperação por versão.
- Moderadores e administradores da plataforma podem fazer nova varredura de qualquer skill e não são bloqueados pelo
  limite de recuperação do proprietário, embora apenas uma nova varredura possa ser executada por vez por versão.
- Chama `POST /api/v1/skills/{slug}/rescan`.
- Flags:
  - `--yes`: pula a confirmação.
  - `--json`: saída legível por máquina.
...
- Faz a exclusão reversível de um pacote e de todas as versões.
- Requer o proprietário do pacote, um proprietário/administrador do publisher da organização, moderador da plataforma,
...
- Transfere um pacote para outro publisher.
- Requer acesso de administrador tanto ao proprietário atual do pacote quanto ao publisher
...
- Flags:
  - `--to <owner>`: identificador do publisher de destino.
...
- Proprietários e administradores de publisher podem verificar novamente seus próprios pacotes até o limite de recuperação
  por versão.
- Moderadores e administradores da plataforma podem verificar novamente qualquer pacote e não são bloqueados
  pelo limite de recuperação do proprietário, embora apenas uma nova verificação possa ser executada por vez por versão.
- Chama `POST /api/v1/packages/{name}/rescan`.
- Flags:
  - `--yes`: pula a confirmação.
  - `--json`: saída legível por máquina.
...
- As denúncias não ocultam pacotes automaticamente nem bloqueiam downloads por conta própria.
- Flags:
...
- Comando de proprietário/publisher para recorrer da moderação de uma versão.
- Chama `POST /api/v1/packages/{name}/appeal`.
- Recursos são aceitos para versões em quarentena, revogadas, suspeitas ou maliciosas.
- Flags:
  - `--version <version>`: versão obrigatória do pacote.
  - `--message <text>`: mensagem obrigatória do recurso.
  - `--json`: saída legível por máquina.
...
- Mostra o estado atual de verificação do pacote, a contagem de denúncias abertas, o estado de moderação manual
...
- Fontes `.tgz` são tratadas como ClawPack. A CLI envia exatamente os bytes npm-pack
  e usa o conteúdo extraído de `package/` apenas para validação e
  pré-preenchimento de metadados.
- Pastas de Plugins de código são empacotadas em um tarball npm do ClawPack antes do envio para que
  instalações do OpenClaw possam verificar o artefato exato. Pastas de Plugins de pacote ainda
  usam o caminho de publicação por arquivos extraídos.
- Para fontes do GitHub, a atribuição de código-fonte é preenchida automaticamente a partir do repositório, commit resolvido, ref e subcaminho.
- Para pastas locais, a atribuição de código-fonte é detectada automaticamente a partir do git local quando o remoto origin aponta para o GitHub.
...
- `--owner <handle>` publica sob um identificador de publisher de usuário ou organização quando o ator tem acesso ao publisher.
...
a atribuição de código-fonte antes de criar uma versão ativa:
...
Para Plugins de código, a publicação por pasta cria e envia um artefato ClawPack a partir
da pasta do pacote:
...
- Se você está usando uma versão mais antiga da CLI `clawhub`, atualize antes de publicar para que
  as verificações locais de pré-voo sejam executadas antes do envio.
...
- Fixe o workflow reutilizável em uma tag estável ou SHA de commit completo. Não execute publicações de versão a partir de `@main`.
- `pull_request` deve usar `dry_run: true` para que o CI não gere efeitos colaterais.
...
- Verifica pastas locais de Skills e publica as novas/alteradas.
