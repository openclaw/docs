...
O ClawHub é um registro de Skills e plugins para o OpenClaw.

Use o OpenClaw ao instalar itens no OpenClaw. Use a CLI `clawhub`
ao entrar na sua conta, publicar, gerenciar suas próprias listagens ou usar
...
O OpenClaw registra a origem da Skill para que atualizações posteriores possam
...
O comando ignora conteúdo inalterado. Novas Skills começam na versão `1.0.0`; alterações
posteriores publicam automaticamente a próxima versão de correção. Use `--dry-run` para visualizar ou
`--version` para escolher uma versão específica.

Antes de publicar, verifique os metadados em `SKILL.md`. Declare as
variáveis de ambiente, ferramentas e permissões necessárias para que os usuários entendam do que a
...
metadados, links de origem, versões, registros de alterações e o status da verificação:
...
moderação podem permanecer ocultas das áreas de pesquisa e instalação até que a situação seja resolvida.
