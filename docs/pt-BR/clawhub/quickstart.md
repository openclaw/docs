...
x-i18n:
    generated_at: "2026-07-11T23:47:27Z"
    model: gpt-5.6
...
O ClawHub é um registro de Skills e plugins do OpenClaw.
...
O OpenClaw registra a origem da Skill para que atualizações futuras continuem
...
Uma Skill é uma pasta com um arquivo `SKILL.md` obrigatório e arquivos auxiliares
opcionais.
...
metadados, links da origem, versões, históricos de alterações e o status da verificação:
...
moderação podem ficar ocultas das interfaces de pesquisa e instalação até que a situação seja resolvida.
