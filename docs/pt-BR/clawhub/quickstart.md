...
x-i18n:
    generated_at: "2026-07-04T10:31:31Z"
    model: gpt-5.5
...
Ambientes headless podem usar um token de API da UI web do ClawHub:
...
publicam automaticamente a próxima versão de patch. Use `--dry-run` para visualizar uma prévia ou
...
ferramentas e permissões obrigatórias para que os usuários entendam do que a
...
Use `--dry-run` primeiro para visualizar uma prévia dos metadados resolvidos do pacote, campos de compatibilidade,
...
Listagens públicas mostram o estado mais recente da varredura. Releases retidas ou bloqueadas por
