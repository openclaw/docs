...
title: ClawHub
x-i18n:
    generated_at: "2026-07-03T17:15:26Z"
    model: gpt-5.5
...
Instale a CLI do ClawHub quando quiser fluxos autenticados pelo registro, como
...
downloads, estrelas e resumos de verificações de segurança. Páginas públicas mostram
o estado atual do registro para que usuários possam inspecionar uma Skill ou um plugin antes de instalá-lo.
...
`npm:<package>` permanece apenas npm quando uma origem precisa ser explícita.

Instalações de plugins validam a compatibilidade anunciada de `pluginApi` e `minGatewayVersion`
antes que a instalação do arquivo seja executada. Quando uma versão de pacote publica um
...
A CLI do ClawHub é para trabalho autenticado pelo registro:
...
antiga o suficiente para passar pelo portão de envio. Páginas públicas de detalhes resumem o
estado mais recente da verificação antes da instalação ou do download.

O ClawHub executa verificações automatizadas em Skills publicadas e lançamentos de plugins. Lançamentos retidos por verificação
...
Ao executar `clawhub install` enquanto estiver conectado, a CLI pode enviar um evento de instalação
