...
title: ClawHub
x-i18n:
    generated_at: "2026-07-04T17:51:09Z"
    model: gpt-5.5
...
downloads, estrelas e resumos de varredura de segurança. As páginas públicas mostram o estado atual do registro
...
metadados de origem para que comandos posteriores de atualização possam permanecer no ClawHub.
...
A CLI do ClawHub é para trabalhos autenticados no registro:
...
O ClawHub é aberto por padrão: qualquer pessoa pode fazer upload, mas publicar exige uma conta do GitHub
antiga o suficiente para passar pelo portão de upload. Páginas públicas de detalhes resumem o
estado mais recente da varredura antes da instalação ou do download.

O ClawHub executa verificações automatizadas em Skills publicadas e versões de plugins. Versões retidas por varredura
