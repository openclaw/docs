...
read_when:
    ...
    - Buscar, instalar ou atualizar Skills ou plugins
...
title: ClawHub
x-i18n:
    generated_at: "2026-07-03T23:28:29Z"
    model: gpt-5.5
...
ClawHub é o registry público para Skills e plugins do OpenClaw.

- Use comandos nativos do `openclaw` para buscar, instalar e atualizar Skills e instalar plugins do ClawHub.
- Use a CLI `clawhub` separada para fluxos de autenticação no registry, publicação e exclusão/restauração.
...
Instale a CLI do ClawHub quando quiser fluxos autenticados no registry, como
...
o estado atual do registry para que os usuários possam inspecionar uma skill ou
Plugin antes de instalá-lo.

## Fluxos nativos do OpenClaw

Os comandos nativos do OpenClaw instalam no workspace ativo do OpenClaw e
persistem metadados de origem para que comandos de atualização posteriores possam
continuar no ClawHub.

Use `clawhub:<package>` quando uma instalação de Plugin precisar ser resolvida
pelo ClawHub. Especificações de Plugin simples e compatíveis com npm podem ser
resolvidas pelo npm durante transições de lançamento, e `npm:<package>` permanece
somente npm quando a origem precisa ser explícita.
...
de pacote publica um artefato ClawPack, o OpenClaw prefere o `.tgz` exato enviado
por npm-pack, verifica o cabeçalho de resumo do ClawHub e os bytes baixados, e
registra metadados do artefato para atualizações posteriores.

## CLI do ClawHub

A CLI do ClawHub é para trabalho autenticado no registry:
...
Use `--dry-run` para criar o plano exato de publicação sem enviar nada, e
`--json` para uma saída adequada para CI.
...
exige uma conta do GitHub antiga o suficiente para passar pelo portão de upload.
Páginas públicas de detalhes resumem o estado da varredura mais recente antes da
instalação ou do download.

O ClawHub executa verificações automatizadas em Skills publicadas e releases de
plugins. Releases retidas por varredura ou bloqueadas podem desaparecer do catálogo
público e das superfícies de instalação enquanto permanecem visíveis para seu
proprietário em `/dashboard`.

Usuários conectados podem denunciar Skills e pacotes. Moderadores podem revisar
...
[Uso aceitável](/pt-BR/clawhub/acceptable-usage) para detalhes sobre políticas e aplicação.
...
[Solução de problemas](/pt-BR/clawhub/troubleshooting) para material de referência mais
aprofundado.
