...
o estado atual do registro para que usuários possam inspecionar uma skill ou plugin
antes de instalá-lo.
...
Especificações simples de plugin seguras para npm podem ser resolvidas pelo npm durante transições de lançamento, e
`npm:<package>` permanece somente npm quando uma fonte precisa ser explícita.

Instalações de plugins validam a compatibilidade anunciada de `pluginApi` e `minGatewayVersion`
antes da instalação do arquivo ser executada. Quando uma versão de pacote publica um
artefato ClawPack, o OpenClaw prefere o `.tgz` npm-pack exato enviado, verifica
...
Plugins de código devem incluir os metadados de compatibilidade exigidos pelo OpenClaw em
...
O ClawHub executa verificações automatizadas em Skills publicadas e lançamentos de plugins. Lançamentos retidos por varredura
...
Quando você executa `clawhub install` estando autenticado, a CLI pode enviar um evento de instalação de melhor esforço
para que o ClawHub possa calcular contagens agregadas de instalação. Desative isso com:
