...
title: ClawHub
x-i18n:
    generated_at: "2026-07-02T08:00:14Z"
    model: gpt-5.5
...
publica um artefato ClawPack, o OpenClaw prefere o `.tgz` exato enviado por
npm-pack, verifica o cabeçalho de resumo do ClawHub e os bytes baixados, e registra
...
exige uma conta do GitHub antiga o suficiente para passar pelo bloqueio de upload.
Páginas públicas de detalhes resumem o estado da varredura mais recente antes da
instalação ou download.

O ClawHub executa verificações automatizadas em Skills publicadas e versões de
Plugin. Versões retidas por varredura ou bloqueadas podem desaparecer do catálogo
público e das superfícies de instalação, permanecendo visíveis para seu proprietário
em `/dashboard`.
...
Quando você executa `clawhub install` conectado, a CLI pode enviar um evento de
instalação em modo de melhor esforço para que o ClawHub possa calcular contagens
agregadas de instalações. Desative isso com:
