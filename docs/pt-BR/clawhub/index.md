...
title: ClawHub
x-i18n:
    generated_at: "2026-07-01T05:32:10Z"
    model: gpt-5.5
...
metadados de origem para que comandos posteriores de atualização possam permanecer
no ClawHub.

Use `clawhub:<package>` quando a instalação de um plugin deve ser resolvida pelo
ClawHub. Especificações de plugins simples compatíveis com npm podem ser
...
o OpenClaw em `package.json`, incluindo `openclaw.compat.pluginApi` e
...
ClawHub é aberto por padrão: qualquer pessoa pode enviar, mas a publicação exige
uma conta do GitHub com idade suficiente para passar pelo controle de envio.
Páginas públicas de detalhes resumem o estado da varredura mais recente antes da
instalação ou download.

ClawHub executa verificações automatizadas em Skills publicadas e lançamentos de
plugins. Lançamentos retidos por varredura ou bloqueados podem desaparecer do
catálogo público e das superfícies de instalação enquanto continuam visíveis para
seu proprietário em `/dashboard`.
