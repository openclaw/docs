...
title: ClawHub
x-i18n:
    generated_at: "2026-07-05T05:25:51Z"
    model: gpt-5.5
...
pelo npm durante transições de lançamento, e `npm:<package>` permanece somente npm
quando uma origem precisa ser explícita.

Instalações de plugins validam a compatibilidade anunciada de `pluginApi` e
`minGatewayVersion` antes que a instalação do arquivo seja executada. Quando uma
versão de pacote publica um artefato ClawPack, o OpenClaw prefere o `.tgz` exato
do npm-pack enviado, verifica o cabeçalho de digest do ClawHub e os bytes baixados,
...
Esses comandos instalam Skills em `./skills` dentro do diretório de trabalho atual
...
públicas de detalhes resumem o estado mais recente da varredura antes da
...
[Uso aceitável](/clawhub/acceptable-usage) para detalhes de políticas e aplicação.
...
contagens agregadas de instalação. Desabilite isso com:
...
| `CLAWHUB_DISABLE_TELEMETRY=1` | Desabilita a telemetria de instalação.             |

Consulte [Telemetria](/pt-BR/clawhub/telemetry), [API HTTP](/clawhub/http-api) e
[Solução de problemas](/clawhub/troubleshooting) para material de referência mais aprofundado.
