...
title: ClawHub
x-i18n:
    generated_at: "2026-07-05T02:01:25Z"
    model: gpt-5.5
...
- Use a CLI `clawhub` separada para fluxos de trabalho de autenticação no registro, publicação e exclusão/restauração.
...
Especificações de plugin simples compatíveis com npm podem ser resolvidas pelo npm durante transições de lançamento, e
...
antes da execução da instalação do arquivo. Quando uma versão de pacote publica um artefato
ClawPack, o OpenClaw prefere o `.tgz` exato de npm-pack enviado, verifica
...
ClawHub é aberto por padrão: qualquer pessoa pode fazer upload, mas a publicação exige uma conta do GitHub
...
ClawHub executa verificações automatizadas em Skills publicadas e versões de plugins. Versões retidas por varredura
...
Usuários conectados podem denunciar Skills e pacotes. Moderadores podem analisar denúncias,
...
em melhor esforço para que o ClawHub possa calcular contagens agregadas de instalação. Desative isso com:
