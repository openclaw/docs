...
title: ClawHub
x-i18n:
    generated_at: "2026-07-02T17:33:26Z"
    model: gpt-5.5
...
o estado atual do registro para que usuários possam inspecionar uma skill ou
...
permanecer no ClawHub.

Use `clawhub:<package>` quando a instalação de um plugin deve ser resolvida pelo
ClawHub. Especificações de plugin simples e seguras para npm podem ser resolvidas
pelo npm durante transições de lançamento, e `npm:<package>` permanece somente
npm quando uma origem precisa ser explícita.
...
publica um artefato ClawPack, o OpenClaw prefere o `.tgz` exato de npm-pack
...
Use `--dry-run` para construir o plano exato de publicação sem enviar, e `--json`
para uma saída adequada para CI.
...
O ClawHub é aberto por padrão: qualquer pessoa pode enviar, mas a publicação
...
de plugin. Lançamentos retidos por varredura ou bloqueados podem desaparecer do
catálogo público e das superfícies de instalação enquanto permanecem visíveis ao
proprietário em `/dashboard`.

Usuários autenticados podem denunciar Skills e pacotes. Moderadores podem revisar
...
[Uso aceitável](/clawhub/acceptable-usage) para detalhes de política e aplicação.

## Telemetria e ambiente

Quando você executa `clawhub install` enquanto está autenticado, a CLI pode enviar
...
Sobrescritas úteis de ambiente:
...
| `CLAWHUB_CONFIG_PATH`         | Sobrescreve onde a CLI armazena o estado de token/configuração. |
