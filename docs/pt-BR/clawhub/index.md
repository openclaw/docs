...
title: ClawHub
x-i18n:
    generated_at: "2026-06-30T22:08:45Z"
    model: gpt-5.5
...
- Use comandos nativos do `openclaw` para pesquisar, instalar e atualizar Skills e para instalar plugins pelo ClawHub.
...
Use `clawhub:<package>` quando uma instalação de plugin deve resolver pelo ClawHub.
Especificações simples de plugins compatíveis com npm podem resolver pelo npm durante transições de lançamento, e
...
`minGatewayVersion` antes que a instalação do arquivo seja executada. Quando uma versão de pacote publica um
...
Quando você executa `clawhub install` estando conectado, a CLI pode enviar um evento de instalação de melhor esforço
para que o ClawHub possa computar contagens agregadas de instalações. Desative isso com:
