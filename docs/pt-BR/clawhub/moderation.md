---
read_when:
    - Como reportar um skill, Plugin ou pacote
...
x-i18n:
    generated_at: "2026-07-02T08:01:09Z"
    model: gpt-5.5
...
ClawHub é aberta para publicação, mas as superfícies públicas de descoberta e instalação ainda
...
Use relatórios da ClawHub somente para conteúdo inseguro do marketplace, como:
...
Não use relatórios da ClawHub para vulnerabilidades no código-fonte próprio de uma skill ou
Plugin de terceiros. Relate-as diretamente ao publicador ou ao repositório-fonte
vinculado na listagem. A ClawHub não mantém nem corrige código de Skills ou plugins
de terceiros.

GitHub Security Advisories para `openclaw/clawhub` são para vulnerabilidades na
própria ClawHub. Exemplos incluem bugs no site, API, CLI, registro, autenticação,
varredura, moderação ou limites de confiança de download/instalação. Não use advisories da ClawHub
...
Disputas sobre organização, marca, escopo de pacote, identificador de proprietário ou propriedade de espaço de nomes devem
usar o processo de [Reivindicações de Organização e Espaço de Nomes](/clawhub/namespace-claims), não o
fluxo de denúncia no produto nem o formulário de recurso de conta.

Use esse processo quando precisar que a equipe da ClawHub revise provas não sensíveis de que um
espaço de nomes deve ser reservado, transferido, renomeado, ocultado, colocado em quarentena, receber alias
ou ser revisado de outra forma. Não inclua segredos, documentos privados, arquivos legais privados,
...
Retenções de moderação servem para proteger os usuários enquanto a ClawHub resolve casos de alto risco.
...
Contas que violam a política da ClawHub podem perder acesso de publicação. Abusos graves podem
...
limite de possível banimento da ClawHub podem acionar um aviso automático. Se a próxima
varredura elegível após o prazo do aviso ainda colocar o publicador no
limite de possível banimento, a ClawHub poderá aplicar a ação de conta automaticamente.
Sinais de revisão de menor confiança e temporais delimitados ficam fora da
aplicação automática.

Contas excluídas, banidas ou desativadas não podem usar tokens de API da ClawHub. Se a autenticação da CLI
...
use o [formulário de recurso da ClawHub](https://appeals.openclaw.ai/) para revisão de recuperação.
