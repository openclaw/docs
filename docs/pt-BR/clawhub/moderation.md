...
x-i18n:
    generated_at: "2026-07-02T17:33:21Z"
    model: gpt-5.5
...
O ClawHub é aberto para publicação, mas as superfícies públicas de descoberta e instalação ainda
...
de origem vinculado na listagem. O ClawHub não mantém nem corrige
...
Bons relatórios são específicos e acionáveis. O abuso do sistema de denúncias pode, por si só, levar a
uma ação de conta.
...
ou passar por outra análise. Não inclua segredos, documentos privados, arquivos jurídicos privados,
...
ou publicações futuras podem começar ocultas até que o problema seja analisado.

As retenções de moderação têm como objetivo proteger os usuários enquanto o ClawHub resolve casos
...
Se um email acionado por scanner identificar uma versão de skill ou plugin como maliciosa,
