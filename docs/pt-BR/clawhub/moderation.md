---
read_when:
    - Denunciando uma skill, plugin ou pacote
...
x-i18n:
    generated_at: "2026-06-30T22:08:21Z"
    model: gpt-5.5
...
Boas denúncias são específicas e acionáveis. O abuso de denúncias pode levar a
ações contra a conta.

## Reivindicações de organizações e namespaces

Disputas sobre organizações, marcas, escopos de pacote, identificadores de proprietários ou
propriedade de namespaces devem usar o processo de [Reivindicações de organizações e namespaces](/clawhub/namespace-claims), não o fluxo de denúncia no produto nem o formulário de recurso de conta.
...
documentos de identidade pessoais, tokens de API ou tokens de desafio de DNS em uma issue pública.
...
ou futuras publicações podem começar ocultas até que o problema seja revisado.

As retenções de moderação servem para proteger os usuários enquanto o ClawHub resolve casos de alto risco.
...
Se você vir um desses estados, não instale a versão a menos que o proprietário resolva o problema
ou a moderação a restaure.

Proprietários ainda podem ver diagnósticos das próprias listagens retidas ou ocultas. Esses
diagnósticos ajudam a explicar o que aconteceu e o que precisa mudar antes que a listagem possa
retornar às superfícies públicas.
...
o limite de banimento potencial do ClawHub podem acionar um aviso automático. Se a próxima
varredura elegível após o prazo do aviso ainda colocar o publicador no limite de
banimento potencial, o ClawHub pode aplicar a ação de conta automaticamente.
Sinais de revisão de menor confiança e temporais delimitados ficam fora da aplicação automática.
