...
x-i18n:
    generated_at: "2026-07-03T02:44:06Z"
    model: gpt-5.5
...
Você pode criar um catálogo, diretório ou superfície de busca de terceiros sobre as APIs públicas de leitura do ClawHub. Os metadados públicos de Skills e os arquivos de Skills são publicados sob as regras de licença de Skills do ClawHub, enquanto a API em si tem limite de taxa e deve ser consumida com responsabilidade.
...
- Crie link de volta para a URL canônica da Skill no ClawHub ao exibir listagens, para que os usuários possam inspecionar o registro de origem no registro.
...
- Os erros v1 são texto simples (`text/plain; charset=utf-8`), incluindo `400`,
  `401`, `403`, `404`, `429` e respostas de download bloqueado.
- Parâmetros de consulta desconhecidos são ignorados para compatibilidade.
...
- `GET /api/v1/download?slug=&version=&tag=`
  ...
  - Skills atuais com backend no GitHub e uma varredura `clean` ou `suspicious` retornam um
    descritor de repasse JSON `public-github` em vez de bytes do ClawHub.
- `GET /api/v1/skills/export?startDate=&endDate=&limit=&cursor=`
  - Skills hospedadas são exportadas como arquivos armazenados.
  - Skills atuais com backend no GitHub e uma varredura `clean` ou `suspicious` são exportadas
    como descritores de repasse `public-github`.
...
- `POST /api/v1/users/reserve` reserva slugs raiz e placeholders privados de pacote sem release para um identificador de proprietário.
