...
x-i18n:
    generated_at: "2026-07-05T04:52:44Z"
    model: gpt-5.5
...
- Vincule de volta à URL canônica de Skill do ClawHub ao exibir listagens, para que os usuários possam inspecionar o registro de origem no registro.
...
- `X-RateLimit-Reset`: segundos do Unix epoch (horário absoluto de redefinição)
...
- `X-RateLimit-Remaining` / `RateLimit-Remaining`: orçamento restante exato quando
  presente; requisições bem-sucedidas fragmentadas o omitem em vez de retornar um valor global
  aproximado
...
- `GET /api/v1/download?slug=&version=&tag=`
  ...
  - Skills atuais baseadas no GitHub com uma varredura `clean` ou `suspicious` retornam um
    descritor de entrega JSON `public-github` em vez de bytes do ClawHub.
- `GET /api/v1/skills/export?startDate=&endDate=&limit=&cursor=`
  ...
  - Skills atuais baseadas no GitHub com uma varredura `clean` ou `suspicious` são exportadas
    como descritores de entrega `public-github`.
