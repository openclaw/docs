...
read_when:
    ...
    - Adicionando pontos de extremidade ou esquemas
...
Você pode criar um catálogo, diretório ou superfície de busca de terceiros sobre as APIs públicas de leitura do ClawHub. Os metadados públicos de habilidades e os arquivos de habilidades são publicados sob as regras de licença de habilidades do ClawHub, enquanto a própria API tem limite de taxa e deve ser consumida com responsabilidade.
...
- Armazene respostas em cache e respeite `429`, `Retry-After` e cabeçalhos de limite de taxa em vez de consultar agressivamente.
- Ao exibir listagens, inclua um link de volta para a URL canônica da habilidade no ClawHub para que os usuários possam inspecionar o registro de origem no registro.
...
- Requisições autenticadas (token Bearer válido): por compartimento de usuário.
- Token ausente/inválido recorre à aplicação por IP.
...
- `POST /api/v1/users/reserve` reserva slugs raiz e espaços reservados de pacotes privados sem release para um identificador de proprietário.
