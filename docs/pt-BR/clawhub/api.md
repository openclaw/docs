...
x-i18n:
    generated_at: "2026-07-04T10:31:08Z"
    model: gpt-5.5
...
Você pode criar um catálogo, diretório ou superfície de busca de terceiros sobre as APIs públicas de leitura do ClawHub. Os metadados públicos de skill e os arquivos de skill são publicados sob as regras de licença de skills do ClawHub, enquanto a API em si tem limite de taxa e deve ser consumida com responsabilidade.
...
- Armazene respostas em cache e respeite os cabeçalhos `429`, `Retry-After` e de limite de taxa em vez de fazer consultas agressivas.
- Inclua link para a URL canônica da skill no ClawHub ao exibir listagens, para que os usuários possam inspecionar o registro de origem.
...
- Solicitações autenticadas (token Bearer válido): por compartimento de usuário.
...
- `X-RateLimit-Reset`: segundos desde a época Unix (tempo absoluto de redefinição)
...
- `GET /api/v1/download?slug=&version=&tag=`
  ...
  - Skills atuais com backend do GitHub e varredura `clean` ou `suspicious` retornam um
...
- `GET /api/v1/skills/export?startDate=&endDate=&limit=&cursor=`
  ...
  - Skills atuais com backend do GitHub e varredura `clean` ou `suspicious` são exportadas
...
- `POST /api/v1/users/reserve` reserva slugs raiz e espaços reservados privados de pacote sem lançamento para um identificador de proprietário.
