...
- Adicione link para a URL canônica da Skill no ClawHub ao exibir listagens, para que os usuários possam inspecionar o registro de origem no registro.
- Use URLs de página canônicas no formato `https://clawhub.ai/<owner>/skills/<slug>`.
- Não insinue que o ClawHub endossa, verifica ou opera o site de terceiros.
...
Aplicação com reconhecimento de autenticação:
...
- Caso contrário, use `RateLimit-Reset` ou derive o atraso a partir de `X-RateLimit-Reset`.
...
- `GET /api/v1/skills?limit=&cursor=&sort=`
  ...
  - `cursor` se aplica a ordenações que não são `trending`
...
- `GET /api/v1/download?slug=&version=&tag=`
  ...
  - Skills atuais baseadas no GitHub com varredura `clean` ou `suspicious` retornam um
    descritor JSON de handoff `public-github` em vez de bytes do ClawHub.
- `GET /api/v1/skills/export?startDate=&endDate=&limit=&cursor=`
  - Skills hospedadas são exportadas como arquivos armazenados.
  - Skills atuais baseadas no GitHub com varredura `clean` ou `suspicious` são exportadas
    como descritores de handoff `public-github`.
...
`/api/*` e `/api/cli/*` legados ainda estão disponíveis. Consulte `DEPRECATIONS.md`.
