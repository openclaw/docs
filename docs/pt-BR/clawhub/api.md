...
read_when:
    ...
    - Adição de endpoints ou esquemas
summary: Visão geral e convenções da API REST pública (v1).
x-i18n:
    generated_at: "2026-07-16T12:17:42Z"
    model: gpt-5.6
...
É possível criar um catálogo, diretório ou mecanismo de pesquisa de terceiros sobre as APIs públicas de leitura do ClawHub. Os metadados e arquivos públicos de Skills são publicados conforme as regras de licença de Skills do ClawHub, enquanto a própria API tem limites de requisições e deve ser utilizada de forma responsável.
...
- Armazene as respostas em cache e respeite `429`, `Retry-After` e os cabeçalhos de limite de requisições, em vez de fazer consultas frequentes de forma agressiva.
...
- Requisições autenticadas (token Bearer válido): por grupo de usuário.
- Um token ausente ou inválido usa a aplicação por IP.
...
- `X-RateLimit-Remaining` / `RateLimit-Remaining`: orçamento restante exato quando
  presente; requisições distribuídas bem-sucedidas o omitem em vez de retornar um valor
  global aproximado
- `Retry-After`: segundos de espera em `429`
...
- Adicione variação aleatória às novas tentativas.

## Erros

- Os erros da v1 são texto simples (`text/plain; charset=utf-8`), incluindo `400`,
  `401`, `403`, `404`, `429` e respostas de download bloqueado.
- Parâmetros de consulta desconhecidos são ignorados para fins de compatibilidade.
...
- `GET /api/v1/skills?limit=&cursor=&sort=`
  - `sort`: `updated` (padrão), `recommended` (`default`), `createdAt` (`newest`), `downloads`, `stars` (`rating`), os aliases legados de instalação `installsCurrent`/`installs`/`installsAllTime` correspondem a `downloads`, `trending`
...
- `GET /api/v1/download?slug=&version=&tag=`
  ...
  - Skills atuais baseadas no GitHub com uma verificação `clean` ou `suspicious` retornam um
    descritor JSON de transferência `public-github` em vez de bytes do ClawHub.
...
- `POST /api/v1/users/reserve` reserva slugs raiz e espaços reservados para pacotes privados sem versão para um identificador de proprietário.

## Legado

Os antigos `/api/*` e `/api/cli/*` continuam disponíveis. Consulte `DEPRECATIONS.md`.
