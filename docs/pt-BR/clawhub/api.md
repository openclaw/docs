...
x-i18n:
    generated_at: "2026-07-11T23:48:29Z"
    model: gpt-5.6
...
Você pode criar um catálogo, diretório ou mecanismo de busca de terceiros com base nas APIs públicas de leitura do ClawHub. Os metadados e arquivos públicos de Skills são publicados de acordo com as regras de licença de Skills do ClawHub, enquanto a própria API tem limites de taxa e deve ser consumida com responsabilidade.
...
- Armazene as respostas em cache e respeite `429`, `Retry-After` e os cabeçalhos de limite de taxa, em vez de realizar consultas frequentes e agressivas.
- Ao exibir listagens, inclua um link para a URL canônica da Skill no ClawHub, para que os usuários possam consultar o registro no catálogo de origem.
- Use URLs canônicas de página no formato `https://clawhub.ai/<owner>/skills/<slug>`.
...
Aplicação baseada em autenticação:

- Requisições anônimas: por IP.
- Requisições autenticadas (token Bearer válido): por cota de usuário.
- Um token ausente ou inválido faz com que a aplicação por IP seja usada.
...
- `X-RateLimit-Remaining` / `RateLimit-Remaining`: cota restante exata quando
  presentes; requisições distribuídas bem-sucedidas omitem esse valor, em vez de retornar
  um valor global aproximado
...
- Prefira `Retry-After` quando estiver presente.
- Caso contrário, use `RateLimit-Reset` ou calcule o tempo de espera com base em `X-RateLimit-Reset`.
...
- `GET /api/v1/skills?limit=&cursor=&sort=`
  - `sort`: `updated` (padrão), `recommended` (`default`), `createdAt` (`newest`), `downloads`, `stars` (`rating`), os aliases legados de instalação `installsCurrent`/`installs`/`installsAllTime` são mapeados para `downloads`, `trending`
  - Valores inválidos de `sort` retornam `400`
  - `cursor` aplica-se a ordenações diferentes de `trending`
...
- `GET /api/v1/download?slug=&version=&tag=`
  ...
  - As Skills atuais provenientes do GitHub com uma verificação `clean` ou `suspicious` retornam um
    descritor JSON de encaminhamento `public-github`, em vez dos bytes do ClawHub.
- `GET /api/v1/skills/export?startDate=&endDate=&limit=&cursor=`
  - Skills hospedadas são exportadas como arquivos armazenados.
  - As Skills atuais provenientes do GitHub com uma verificação `clean` ou `suspicious` são exportadas
...
- `POST /api/v1/skills` (publicação, preferencialmente multipart)
...
- `POST /api/v1/users/reserve` reserva slugs raiz e espaços reservados privados de pacotes sem versão para um identificador de proprietário.
