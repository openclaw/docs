...
x-i18n:
    generated_at: "2026-07-01T20:14:27Z"
    model: gpt-5.5
...
- Inclua um link para a URL canônica da Skill no ClawHub ao exibir listagens para que usuários possam inspecionar o registro de origem no registro.
...
- `X-RateLimit-Reset`: segundos desde a época Unix (horário absoluto de reinicialização)
- `RateLimit-Reset`: segundos de atraso até a reinicialização
- `X-RateLimit-Remaining` / `RateLimit-Remaining`: orçamento restante exato quando
  presente; requisições bem-sucedidas fragmentadas omitem isso em vez de retornar um valor
  global aproximado
