...
x-i18n:
    generated_at: "2026-07-03T09:26:55Z"
    model: gpt-5.5
...
- `X-RateLimit-Reset`: segundos de época Unix (horário absoluto de reinício)
- `RateLimit-Reset`: segundos de atraso até o reinício
- `X-RateLimit-Remaining` / `RateLimit-Remaining`: orçamento restante exato quando
  presente; requisições bem-sucedidas particionadas o omitem em vez de retornar um valor
  global aproximado
...
- `GET /api/v1/download?slug=&version=&tag=`
  ...
  - Skills atuais com backend do GitHub e uma verificação `clean` ou `suspicious` retornam um
...
- `GET /api/v1/skills/export?startDate=&endDate=&limit=&cursor=`
  ...
  - Skills atuais com backend do GitHub e uma verificação `clean` ou `suspicious` são exportadas
