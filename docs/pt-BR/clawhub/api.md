...
x-i18n:
    generated_at: "2026-07-03T13:20:14Z"
    model: gpt-5.5
...
- Inclua um link para a URL canônica da Skill no ClawHub ao exibir listagens, para que os usuários possam inspecionar o registro de origem no registry.
...
- `X-RateLimit-Reset`: segundos Unix epoch (horário absoluto de redefinição)
...
- `GET /api/v1/download?slug=&version=&tag=`
  ...
  - Skills atuais apoiadas pelo GitHub com uma verificação `clean` ou `suspicious` retornam um
    descritor de encaminhamento JSON `public-github` em vez de bytes do ClawHub.
- `GET /api/v1/skills/export?startDate=&endDate=&limit=&cursor=`
  - Skills hospedadas são exportadas como arquivos armazenados.
  - Skills atuais apoiadas pelo GitHub com uma verificação `clean` ou `suspicious` são exportadas
...
- `POST /api/v1/users/reserve` reserva slugs raiz e placeholders de pacotes privados sem lançamento para um identificador de proprietário.

## Legado

Legados `/api/*` e `/api/cli/*` ainda estão disponíveis. Consulte `DEPRECATIONS.md`.
