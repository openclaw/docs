...
x-i18n:
    generated_at: "2026-07-02T13:59:29Z"
    model: gpt-5.5
...
- `X-RateLimit-Reset`: segundos do Unix epoch (tempo absoluto de redefinição)
...
- `GET /api/v1/download?slug=&version=&tag=`
  ...
  - Skills atuais baseadas no GitHub com uma verificação `clean` ou `suspicious` retornam um
    descritor JSON de repasse `public-github` em vez de bytes do ClawHub.
...
Legado `/api/*` e `/api/cli/*` ainda disponíveis. Veja `DEPRECATIONS.md`.
