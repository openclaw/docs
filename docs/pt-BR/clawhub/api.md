...
- Use pontos de extremidade públicos de leitura, como `GET /api/v1/skills`, `GET /api/v1/search` e `GET /api/v1/skills/{slug}`, para listagens de catálogo.
- Armazene respostas em cache e respeite os cabeçalhos `429`, `Retry-After` e de limite de taxa, em vez de consultar agressivamente.
- Inclua um link de volta para a URL canônica de Skill do ClawHub ao exibir listagens, para que os usuários possam inspecionar o registro de origem do registro.
...
- `X-RateLimit-Remaining` / `RateLimit-Remaining`: orçamento restante exato quando
  presente; solicitações bem-sucedidas particionadas o omitem em vez de retornar um valor
  global aproximado
...
- `POST /api/v1/skills` (publicação, multipart preferível)
