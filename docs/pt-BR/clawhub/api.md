...
x-i18n:
    generated_at: "2026-07-03T00:54:03Z"
    model: gpt-5.5
...
- Inclua um link de volta para a URL canônica da Skill no ClawHub ao exibir listagens, para que os usuários possam inspecionar o registro de origem no registro.
...
- `X-RateLimit-Reset`: segundos de época Unix (tempo absoluto de redefinição)
...
- `X-RateLimit-Remaining` / `RateLimit-Remaining`: orçamento restante exato quando
  presente; solicitações bem-sucedidas fragmentadas omitem isso em vez de retornar um valor
  global aproximado
