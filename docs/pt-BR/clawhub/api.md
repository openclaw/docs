...
- Adicione links para a URL canônica da Skill no ClawHub ao exibir listagens, para que os usuários possam inspecionar o registro de origem no registry.
- Use URLs de página canônicas no formato `https://clawhub.ai/<owner>/<slug>`.
- Não implique que o ClawHub endossa, verifica ou opera o site de terceiros.
...
Aplicação considerando autenticação:

- Requisições anônimas: por IP.
- Requisições autenticadas (token Bearer válido): por bucket de usuário.
- Token ausente/inválido usa a aplicação por IP como fallback.
