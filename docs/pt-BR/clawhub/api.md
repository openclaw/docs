...
Você pode criar um catálogo, diretório ou superfície de busca de terceiros sobre as APIs públicas de leitura do ClawHub. Metadados públicos de skill e arquivos de skill são publicados sob as regras de licença de skill do ClawHub, enquanto a própria API tem limite de taxa e deve ser consumida de forma responsável.
...
- Use URLs de páginas canônicas no formato `https://clawhub.ai/<owner>/skills/<slug>`.
...
- `GET /api/v1/download?slug=&version=&tag=`
  ...
  - Skills atuais com suporte do GitHub com uma verificação `clean` ou `suspicious` retornam um
...
- `GET /api/v1/skills/export?startDate=&endDate=&limit=&cursor=`
  ...
  - Skills atuais com suporte do GitHub com uma verificação `clean` ou `suspicious` são exportadas
