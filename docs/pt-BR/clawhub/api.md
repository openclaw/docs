...
x-i18n:
    generated_at: "2026-07-04T15:10:32Z"
    model: gpt-5.5
...
Você pode criar um catálogo, diretório ou superfície de busca de terceiros sobre as APIs públicas de leitura do ClawHub. Metadados públicos de skill e arquivos de skill são publicados sob as regras de licença de skills do ClawHub, enquanto a API em si tem limite de taxa e deve ser consumida com responsabilidade.
...
- Inclua link de volta para a URL canônica da skill no ClawHub ao exibir listagens, para que os usuários possam inspecionar o registro de origem no registry.
...
- Requisições autenticadas (token Bearer válido): por bucket de usuário.
...
- `X-RateLimit-Reset`: segundos de época Unix (horário absoluto de redefinição)
- `RateLimit-Reset`: segundos de atraso até redefinir
- `X-RateLimit-Remaining` / `RateLimit-Remaining`: orçamento restante exato quando
  presente; requisições bem-sucedidas fragmentadas o omitem em vez de retornar um valor
  global aproximado
...
- Erros da v1 são texto simples (`text/plain; charset=utf-8`), incluindo `400`,
...
- `GET /api/v1/skills?limit=&cursor=&sort=`
  ...
  - `cursor` aplica-se a ordenações que não sejam `trending`
  - Filtro opcional: `nonSuspiciousOnly=true`
  - Alias legado: `nonSuspicious=true`
  - Com `nonSuspiciousOnly=true`, páginas baseadas em cursor podem conter menos itens do que `limit`; use `nextCursor` para continuar.
...
- `GET /api/v1/download?slug=&version=&tag=`
  ...
  - Skills atuais respaldadas pelo GitHub com uma varredura `clean` ou `suspicious` retornam um
    descritor de transferência `public-github` JSON em vez de bytes do ClawHub.
- `GET /api/v1/skills/export?startDate=&endDate=&limit=&cursor=`
  - Skills hospedadas são exportadas como arquivos armazenados.
  - Skills atuais respaldadas pelo GitHub com uma varredura `clean` ou `suspicious` são exportadas
...
- `POST /api/v1/users/reserve` reserva slugs raiz e placeholders de pacotes privados sem release para um identificador de proprietário.
