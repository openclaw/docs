...
- Adicione link para a URL canônica da Skill no ClawHub ao exibir listagens para que os usuários possam inspecionar o registro de origem no registry.
...
- `X-RateLimit-Reset`: segundos de época Unix (hora absoluta de redefinição)
...
- `GET /api/v1/download?slug=&version=&tag=`
  ...
  - Skills atuais com back-end do GitHub com uma verificação `clean` ou `suspicious` retornam um
...
- `GET /api/v1/skills/export?startDate=&endDate=&limit=&cursor=`
  ...
  - Skills atuais com back-end do GitHub com uma verificação `clean` ou `suspicious` são exportadas
...
Legacy `/api/*` e `/api/cli/*` ainda disponíveis. Consulte `DEPRECATIONS.md`.
