...
x-i18n:
    generated_at: "2026-07-12T21:28:27Z"
    model: gpt-5.6
...
Você pode criar um catálogo, diretório ou mecanismo de busca de terceiros sobre as APIs públicas de leitura do ClawHub. Os metadados e arquivos públicos de Skills são publicados de acordo com as regras de licença de Skills do ClawHub, enquanto a própria API tem limites de taxa e deve ser utilizada com responsabilidade.
...
- Um token ausente ou inválido usa a aplicação por IP como alternativa.
...
- Os erros da v1 são texto simples (`text/plain; charset=utf-8`), incluindo respostas `400`,
...
- `GET /api/v1/download?slug=&version=&tag=`
  ...
  - Skills atuais com origem no GitHub e uma verificação `clean` ou `suspicious` retornam um
...
- `GET /api/v1/skills/export?startDate=&endDate=&limit=&cursor=`
  ...
  - Skills atuais com origem no GitHub e uma verificação `clean` ou `suspicious` são exportadas
...
- `POST /api/v1/skills` (publicação, de preferência multipart)
...
As APIs legadas `/api/*` e `/api/cli/*` continuam disponíveis. Consulte `DEPRECATIONS.md`.
