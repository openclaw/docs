...
read_when:
    ...
    - Decidindo entre usar uma denúncia, apelação ou reivindicação de namespace
sidebarTitle: Org and Namespace Claims
summary: Como solicitar uma revisão do ClawHub para disputas de propriedade de organização, marca, identificador de proprietário, escopo de pacote, slug de Skill ou namespace.
...
Use este caminho para revisão pública e não sensível de propriedade. Não use relatórios dentro do produto nem o formulário de apelação de conta para reivindicações de namespace.

## Quando Abrir uma Reivindicação

Abra uma reivindicação de namespace quando você acreditar que a equipe do ClawHub deve revisar se um namespace deve ser reservado, transferido, renomeado, ocultado, colocado em quarentena, associado a um alias ou alterado de outra forma por causa de propriedade no mundo real.

Exemplos incluem:

- um identificador de organização que corresponde à sua organização, projeto, empresa ou comunidade no GitHub
- um escopo de pacote como `@example-org/*` que deve publicar somente sob o proprietário correspondente no ClawHub
...
Se a listagem for insegura, maliciosa ou enganosa além da disputa de propriedade, siga também a orientação de moderação ou segurança relevante. O formulário de reivindicação de namespace é para revisão de propriedade, não para divulgação emergencial de vulnerabilidades.

## Antes de Enviar

Primeiro, confirme que você está publicando com o proprietário que corresponde ao namespace. Para pacotes de Plugin, nomes com escopo como `@example-org/example-plugin` devem ser publicados como o proprietário correspondente `example-org`.
...
- links para o proprietário, Skills, Plugin, pacote ou issue disputado no ClawHub
...
A revisão de namespace não garante que todo nome correspondente será transferido. A equipe avalia evidências públicas, uso existente, risco de segurança e impacto para usuários.
