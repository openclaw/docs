...
read_when:
    ...
    - Decidindo se deve usar uma denúncia, um recurso ou uma reivindicação de namespace
...
O ClawHub usa identificadores de proprietário, identificadores de organização, slugs de Skills, nomes de pacotes de Plugin e
escopos de pacote como namespaces públicos. Se um namespace parece pertencer a um
projeto, marca, ecossistema de pacotes ou organização do mundo real, mas já está
...
- um identificador de organização que corresponde à sua organização do GitHub, projeto, empresa ou comunidade
