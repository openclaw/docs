...
x-i18n:
    generated_at: "2026-07-16T12:19:08Z"
    model: gpt-5.6
...
reivindicado, reservado, for enganoso ou estiver em disputa no ClawHub, solicite uma análise da equipe
por meio do
...
Abra uma reivindicação de namespace quando considerar que a equipe do ClawHub deve analisar se um
...
- um escopo de pacote como `@example-org/*` que só deve ser publicado pelo
...
- um proprietário excluído, inativo ou inacessível que impeça o proprietário legítimo do namespace
...
Primeiro, confirme que a publicação está sendo feita com o proprietário correspondente ao namespace.
Para pacotes de plugins, nomes com escopo como `@example-org/example-plugin` devem ser
publicados pelo proprietário `example-org` correspondente.

Se for possível gerenciar o proprietário atual, corrija o namespace diretamente publicando,
renomeando, transferindo, ocultando ou excluindo o recurso afetado. Use uma reivindicação
quando não for possível gerenciar o proprietário atual ou quando a equipe precisar resolver uma
disputa.

## Evidências a incluir

Use evidências públicas e não confidenciais. Provas úteis incluem:

- histórico da organização, do repositório, de versões ou de mantenedores no GitHub
...
- evidência de propriedade de marca registrada, marca ou projeto que possa ser discutida
...
Não publique segredos nem provas privadas em uma issue pública do GitHub. Não inclua:
...
O formulário de reivindicação pergunta se as evidências confidenciais precisam de um canal privado com a equipe.
Use essa opção em vez de publicar material confidencial publicamente.
...
A equipe avalia as evidências públicas, o uso existente, o risco de segurança e o impacto para os usuários.
