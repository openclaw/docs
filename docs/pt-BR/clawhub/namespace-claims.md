...
read_when:
    ...
    - Decidindo entre usar uma denúncia, uma apelação ou uma reivindicação de namespace
...
x-i18n:
    generated_at: "2026-07-12T21:30:42Z"
    model: gpt-5.6
...
ou ser alterado de outra forma devido à propriedade no mundo real.

Alguns exemplos:

- um identificador de organização que corresponde à sua organização, projeto, empresa ou comunidade no GitHub
- um escopo de pacote, como `@example-org/*`, que deve ser publicado somente pelo
...
- um proprietário excluído, inativo ou inacessível que impede o acesso do proprietário legítimo ao namespace
...
destina-se à análise de propriedade, não à divulgação emergencial de vulnerabilidades.

## Antes de registrar

Primeiro, confirme que você está publicando com o proprietário correspondente ao namespace.
...
Use evidências públicas e não confidenciais. Evidências úteis incluem:
...
- evidências de propriedade de marca registrada, marca ou projeto que possam ser discutidas com segurança
  publicamente
...
Explique o que cada link comprova. A equipe deve conseguir compreender a
relação sem precisar de credenciais privadas ou segredos.

## O que não incluir

Não coloque segredos nem evidências privadas em uma issue pública do GitHub. Não inclua:
...
adicionar um alias ou redirecionamento, solicitar mais evidências ou recusar a solicitação.
