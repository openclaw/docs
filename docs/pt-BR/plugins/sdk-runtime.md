...
Referência para o objeto `api.runtime` injetado em todo plugin durante o registro. Use estes helpers em vez de importar internos do host diretamente.

<CardGroup cols={2}>
  <Card title="Plugins de canal" href="/pt-BR/plugins/sdk-channel-plugins">
    Guia passo a passo que usa estes helpers em contexto para plugins de canal.
  </Card>
  <Card title="Plugins de provedor" href="/pt-BR/plugins/sdk-provider-plugins">
    Guia passo a passo que usa estes helpers em contexto para plugins de provedor.
  </Card>
</CardGroup>
...
Use `api.runtime.config.current()` apenas quando um manipulador de longa duração precisa do snapshot atual do processo e nenhuma configuração foi passada para essa função. O valor retornado é somente leitura; clone ou use um helper de mutação antes de editar.
...
Persista mudanças com `api.runtime.config.mutateConfigFile(...)` ou `api.runtime.config.replaceConfigFile(...)`. Cada gravação deve escolher uma política `afterWrite` explícita:
...
- `afterWrite: { mode: "none", reason: "..." }` suprime o recarregamento/reinicialização automático apenas quando o chamador é dono do acompanhamento.

Os helpers de mutação retornam `afterWrite` mais um resumo `followUp` tipado para que os chamadores possam registrar em log ou testar se solicitaram uma reinicialização. O Gateway ainda é dono de quando essa reinicialização realmente acontece.

`api.runtime.config.loadConfig()` e `api.runtime.config.writeConfigFile(...)` são helpers de compatibilidade obsoletos em `runtime-config-load-write`. Eles emitem um aviso uma vez em tempo de execução e continuam disponíveis para plugins externos antigos durante a janela de migração. Plugins integrados não devem usá-los; as proteções de limite de configuração falham se o código do plugin os chamar ou importar esses helpers de subcaminhos do SDK de plugins.
...
tipos, `plugin-config-runtime` para asserções de configuração já carregada e pesquisa de entrada de plugin, `runtime-config-snapshot` para snapshots atuais do processo e
`config-mutation` para gravações. Testes de plugins integrados devem simular esses subcaminhos focados diretamente em vez de simular o barrel amplo de compatibilidade.

O código interno de runtime do OpenClaw segue a mesma direção: carregar a configuração uma vez no limite da CLI, do Gateway ou do processo, e então passar esse valor adiante. Gravações de mutação bem-sucedidas atualizam o snapshot de runtime do processo e avançam sua revisão interna; caches de longa duração devem usar a chave de cache de propriedade do runtime em vez de serializar a configuração localmente. Módulos de runtime de longa duração têm um scanner de tolerância zero para chamadas ambientes de `loadConfig()`; use um `cfg` passado, um `context.getRuntimeConfig()` de requisição ou `getRuntimeConfig()` em um limite explícito de processo.

Caminhos de execução de provedor e canal devem usar o snapshot de configuração de runtime ativo, não um snapshot de arquivo retornado para leitura ou edição de configuração. Snapshots de arquivo preservam valores de origem, como marcadores SecretRef, para UI e gravações; callbacks de provedor precisam da visão de runtime resolvida. Quando um helper puder ser chamado com o snapshot de origem ativo ou com o snapshot de runtime ativo, roteie por `selectApplicableRuntimeConfig()` antes de ler credenciais.

## Utilitários reutilizáveis de runtime

Use fatos `botLoopProtection` de entrada para mensagens de entrada criadas por bot. O core aplica a proteção compartilhada em memória de janela deslizante antes do registro de sessão e do despacho, sem vincular a política a um único canal. A proteção rastreia chaves `(scopeId, conversationId, participant pair)`, conta as duas direções de um par juntas, aplica um cooldown quando o orçamento da janela é excedido e remove entradas inativas oportunisticamente.

Plugins de canal que expõem esse comportamento a operadores devem preferir a forma compartilhada `channels.defaults.botLoopProtection` para orçamentos de base e depois aplicar substituições específicas de canal/provedor por cima. A configuração compartilhada usa segundos porque é voltada ao usuário:
...
Passe fatos normalizados de pares de bots com o turno resolvido. O core resolve padrões, conversão de unidades e semântica de `enabled`:
...
Use `openclaw/plugin-sdk/pair-loop-guard-runtime` diretamente apenas para loops de eventos personalizados de duas partes que não passam pelo runner compartilhado de respostas de entrada.
...
<AccordionGroup>
  <Accordion title="api.runtime.agent">
    ...
    `runEmbeddedAgent(...)` é o helper neutro para iniciar um turno normal de agente do OpenClaw a partir do código de plugin. Ele usa a mesma resolução de provedor/modelo e seleção de harness de agente que respostas acionadas por canal.
    ...
    `resolveThinkingPolicy(...)` retorna os níveis de raciocínio compatíveis do provedor/modelo e o padrão opcional. Plugins de provedor são donos do perfil específico do modelo por meio de seus hooks de raciocínio, então plugins de ferramenta devem chamar este helper de runtime em vez de importar ou duplicar listas de provedores.
    ...
    **Helpers de armazenamento de sessão** ficam em `api.runtime.agent.session`:
    ...
    `loadSessionStore(...)`, `saveSessionStore(...)`, `updateSessionStore(...)`, `resolveSessionFilePath(...)` e `resolveAndPersistSessionFile(...)` são helpers de compatibilidade obsoletos para plugins que ainda dependem intencionalmente da forma legada de armazenamento inteiro ou arquivo de transcrição. Código novo de plugin não deve usar esses helpers, e chamadores existentes devem migrar para helpers de entrada e helpers de identidade de transcrição.
  ...
  <Accordion title="api.runtime.llm">
    Execute uma conclusão de texto pertencente ao host sem importar internos de provedor nem
    ...
    O helper usa o mesmo caminho de preparação de conclusão simples que o runtime
  ...
  <Accordion title="api.runtime.nodes">
    ...
    Dentro do Gateway, este runtime é executado no mesmo processo. Em comandos de CLI do plugin, ele chama o Gateway configurado por RPC, então comandos como `openclaw googlemeet recover-tab` podem inspecionar nós pareados pelo terminal. Comandos de nó ainda passam pelo pareamento normal de nós do Gateway, listas de permissões de comandos, políticas de invocação de nós por plugin e tratamento de comandos local ao nó.

    Plugins que expõem comandos perigosos hospedados em nó devem registrar uma política de invocação de nó com `api.registerNodeInvokePolicy(...)`. A política é executada no Gateway após as verificações de lista de permissões de comandos e antes que o comando seja encaminhado ao nó, então chamadas diretas de `node.invoke` e ferramentas de plugin de nível mais alto compartilham o mesmo caminho de aplicação.

  </Accordion>
  <Accordion title="api.runtime.tasks.managedFlows">
    Vincule um runtime de fluxo de tarefas a uma chave de sessão existente do OpenClaw ou a um contexto de ferramenta confiável e, então, crie e gerencie fluxos de tarefas sem passar um proprietário em cada chamada.

    O fluxo de tarefas rastreia estado durável de workflows com várias etapas. Ele não é um agendador:
    use Cron ou `api.session.workflow.scheduleSessionTurn(...)` para despertares futuros
    e, então, use `managedFlows` a partir do turno agendado quando esse trabalho
  ...
  <Accordion title="api.runtime.system">
    ...
    que não são timeout ainda podem retornar `code: null`, então use `termination` e
  ...
  <Accordion title="api.runtime.state">
    Resolução de diretório de estado e armazenamento chaveado com suporte em SQLite.
  ...
  <Accordion title="api.runtime.channel">
    ...
    `api.runtime.channel.media` é a superfície preferencial para downloads e armazenamento de mídia de canal:
    ...
    `api.runtime.channel.mentions` intencionalmente não expõe os auxiliares de compatibilidade mais antigos `resolveMentionGating*`. Prefira o caminho normalizado `{ facts, policy }`.
...
<ParamField path="api.config" type="OpenClawConfig">
  Snapshot de configuração atual (snapshot ativo do runtime em memória quando disponível).
</ParamField>
