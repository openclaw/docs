---
read_when:
    - Você está instalando, configurando ou auditando o Plugin lmstudio
summary: Adiciona suporte ao provedor de modelos LM Studio ao OpenClaw.
title: Plugin do LM Studio
x-i18n:
    generated_at: "2026-05-02T20:57:29Z"
    model: gpt-5.5
    provider: openai
    source_hash: ec6c84ec422c0795d00f744239fcad96a5a01ef4fddd4823ba0b6ed93444a75b
    source_path: plugins/reference/lmstudio.md
    workflow: 16
---

# Plugin do LM Studio

Adiciona suporte ao provedor de modelos LM Studio ao OpenClaw.

## Distribuição

- Pacote: `@openclaw/lmstudio-provider`
- Rota de instalação: incluído no OpenClaw

## Superfície

provedores: lmstudio; contratos: memoryEmbeddingProviders

## Documentos relacionados

- [lmstudio](/pt-BR/providers/lmstudio)
