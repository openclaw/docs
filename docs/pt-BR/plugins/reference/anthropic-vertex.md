...
x-i18n:
    generated_at: "2026-07-12T15:29:16Z"
    model: gpt-5.6
...
Plugin do provedor Anthropic Vertex do OpenClaw para modelos Claude no Google Vertex AI.
...
`/think minimal` usam o esforço `low` porque o modelo não permite desativar o raciocínio.
...
`/think off` ou os níveis nativos `/think xhigh|max`. O OpenClaw publica automaticamente sua
janela de contexto de 1.000.000 de tokens e o limite de saída de 128.000 tokens.

Os preços do catálogo seguem a tarifa global introdutória do Vertex de `$2/$10` por
milhão de tokens de entrada/saída até 31 de agosto de 2026 e, depois, passam a `$3/$15` a partir de
1º de setembro. Os endpoints multirregionais `us` e `eu` usam o
