...
summary: Migrar da camada legada de retrocompatibilidade para o SDK de Plugin moderno
title: Migração do SDK de Plugin
x-i18n:
    generated_at: "2026-07-01T12:52:29Z"
    model: gpt-5.5
...
O OpenClaw passou de uma ampla camada de compatibilidade retroativa para uma arquitetura moderna de Plugin
...
- **`openclaw/plugin-sdk/compat`** - uma única importação que reexportava dezenas de
  helpers. Ela foi introduzida para manter Plugins antigos baseados em hooks funcionando enquanto a
  nova arquitetura de Plugins estava sendo criada.
- **`openclaw/plugin-sdk/infra-runtime`** - um barrel amplo de helpers de runtime que
  misturava eventos do sistema, estado de Heartbeat, filas de entrega, helpers de fetch/proxy,
  helpers de arquivo, tipos de aprovação e utilitários não relacionados.
- **`openclaw/plugin-sdk/config-runtime`** - um barrel amplo de compatibilidade de configuração
  que ainda carrega helpers diretos obsoletos de carregamento/gravação durante a janela de migração.
...
- **`api.registerEmbeddedExtensionFactory(...)`** - um hook de extensão incluída, exclusivo do executor embutido e removido,
...
exclusiva do executor embutido foi removida; use middleware de resultado de ferramenta em vez disso.

O OpenClaw não remove nem reinterpreta comportamento documentado de Plugins na mesma
alteração que introduz uma substituição. Alterações que quebram contratos devem primeiro passar
...
As interfaces legadas de conveniência de provedores para canais incluídos também foram removidas.
Interfaces auxiliares com marca de canal eram atalhos privados do monorepo, não contratos estáveis
de Plugins. Use subcaminhos genéricos e estreitos do SDK em vez disso. Dentro do workspace de Plugins
incluídos, mantenha helpers pertencentes ao provedor no próprio `api.ts` ou
`runtime-api.ts` desse Plugin.

Exemplos atuais de provedores incluídos:

- Anthropic mantém helpers de stream específicos do Claude na sua própria interface `api.ts` /
  `contract-api.ts`
...
Código de voz em tempo real, telefonia, reuniões e Talk no navegador está migrando da
contabilidade local de turnos em cada superfície para um controlador compartilhado de sessão Talk exportado por
`openclaw/plugin-sdk/realtime-voice`. O novo controlador possui o envelope comum de eventos do Talk,
...
sessões em tempo real específicas de fornecedor; Plugins de superfície devem continuar sendo responsáveis por captura,
reprodução, telefonia e particularidades de reuniões.

Esta migração do Talk é intencionalmente limpa e com quebra:

1. Mantenha o controlador compartilhado e os primitivos de runtime em
   `plugin-sdk/realtime-voice`.
2. Mova as superfícies incluídas para o controlador compartilhado: retransmissão do navegador,
   handoff de sala gerenciada, tempo real de chamada de voz, STT por streaming de chamada de voz, Google
   Meet em tempo real e pressionar para falar nativo.
3. Substitua as famílias antigas de RPC do Talk pela API final `talk.session.*` e
   `talk.client.*`.
4. Anuncie um canal de eventos Talk ativo no Gateway
   `hello-ok.features.events`: `talk.event`.
5. Exclua o endpoint HTTP antigo de tempo real e qualquer caminho de sobrescrita de instruções
...
de áudio de saída permaneçam consistentes entre telefonia, reuniões, retransmissão do navegador, handoff
de sala gerenciada e clientes nativos do Talk.
...
e sessões nativas de STT/TTS em sala gerenciada.

Configurações legadas que colocavam seletores de tempo real ao lado de `talk.provider` /
`talk.providers` devem ser reparadas com `openclaw doctor --fix`; o Talk em runtime
não reinterpreta configuração de provedor de fala/TTS como configuração de provedor de tempo real.
...
| `stt-tts`       | `managed-room`  | `agent-consult` | Sala nativa/cliente | Salas no estilo pressionar para falar e walkie-talkie em que o cliente é responsável por captura/reprodução e o Gateway pelo estado de turno. |
| `stt-tts`       | `managed-room`  | `direct-tools`  | Sala nativa/cliente | Modo de sala apenas para administradores em superfícies primárias confiáveis que executam ações de ferramenta do Gateway diretamente. |
...
O vocabulário unificado de controle também é deliberadamente estreito:
  ...
  | `talk.session.startTurn`        | `stt-tts/managed-room`                                  | Inicia um turno de usuário de sala gerenciada.                                                                                                                                           |
  | `talk.session.endTurn`          | `stt-tts/managed-room`                                  | Encerra o turno ativo após a validação de turno obsoleto.                                                                                                                                |
  | `talk.session.cancelTurn`       | todas as sessões pertencentes ao Gateway                | Cancela o trabalho ativo de captura/provedor/agente/TTS para um turno.                                                                                                                   |
  ...
  | `talk.session.close`            | todas as sessões unificadas                             | Interrompe sessões de relay ou revoga o estado de sala gerenciada e, então, esquece o id da sessão unificada.                                                                            |

  Não introduza casos especiais de provedor ou plataforma no core para fazer isso funcionar.
  O core é responsável pela semântica das sessões Talk. Plugins de provedor são responsáveis pela configuração de sessões do fornecedor.
  ...
  `pnpm plugins:boundary-report:ci` quando uma trava de CI deve falhar em registros
  de compatibilidade vencidos, importações de SDK reservadas entre proprietários ou subcaminhos
  reservados de SDK não utilizados. O relatório agrupa registros de compatibilidade
  descontinuados por data de remoção, conta referências locais em código/docs,
  expõe importações de SDK reservadas entre proprietários e resume a ponte privada
  do SDK de host de memória para que a limpeza de compatibilidade permaneça explícita em vez de
  depender de buscas ad hoc. Subcaminhos reservados de SDK devem ter uso de proprietário rastreado;
  exports de helpers reservados não utilizados devem ser removidos do SDK público.

  Se um campo de manifesto ainda for aceito, autores de plugins podem continuar usando-o até
  que a documentação e os diagnósticos digam o contrário. Código novo deve preferir o substituto
  documentado, mas plugins existentes não devem quebrar durante versões minor
  comuns.
  ...
  <Step title="Migrate runtime config load/write helpers">
    ...
    snapshot atual do processo podem usar `api.runtime.config.current()`. Ferramentas
    de agente de longa duração devem usar `ctx.getRuntimeConfig()` do contexto da ferramenta dentro de
    ...
    `afterWrite: { mode: "none", reason: "..." }` somente quando o chamador é responsável pelo
    acompanhamento e quer deliberadamente suprimir o planejador de recarga.
    ...
    o código de compatibilidade `runtime-config-load-write`. Plugins incluídos e código
    de runtime do repositório são protegidos por barreiras de scanner em
    ...
    o snapshot de runtime da solicitação, helpers de envio/ação/cliente de canal de runtime
    ...
    `openclaw/plugin-sdk/config-runtime`. Use o subcaminho estreito
    do SDK que corresponde ao trabalho:
  ...
  <Step title="Migrate embedded tool-result extensions to middleware">
    ...
    explicitamente habilitados e declaram todos os runtimes segmentados em
  ...
  <Step title="Migrate approval-native handlers to capability facts">
    Plugins de canal com suporte a aprovações agora expõem comportamento nativo de aprovação por meio de
    ...
    - Mova auth/entrega específicos de aprovação para fora da fiação legada `plugin.auth` /
    ...
    - Não envie avisos de reroteamento pertencentes ao Plugin a partir de handlers nativos de aprovação;
      o core agora é responsável por avisos de roteado-para-outro-lugar a partir dos resultados reais de entrega
  ...
  <Step title="Find deprecated imports">
    Pesquise no seu Plugin por importações de qualquer uma das superfícies descontinuadas:
  ...
  <Step title="Replace with focused imports">
    ...
    | auxiliares de armazenamento de sessões | `api.runtime.agent.session.*` |

  </Step>

  <Step title="Substituir importações amplas de infra-runtime">
    ...
    | Auxiliares seguros de caminho para arquivo/mídia local | `openclaw/plugin-sdk/file-access-runtime` |
    | Fetch ciente do dispatcher | `openclaw/plugin-sdk/runtime-fetch` |
    ...
    | Auxiliares de payload de resposta de aprovação e comando | `openclaw/plugin-sdk/approval-reply-runtime` |
    ...
    Plugins empacotados são protegidos por scanner contra `infra-runtime`,
    então o código do repositório não pode regredir para o barrel amplo.

  </Step>

  <Step title="Migrar auxiliares de rota de canal">
    Código novo de rota de canal deve usar `openclaw/plugin-sdk/channel-route`.
    Os nomes mais antigos de route-key e comparable-target permanecem como aliases
    ...
    deduplicação de entrada, entrega por Cron e roteamento de sessões.
    ...
    Esses hooks estão obsoletos e permanecem apenas para plugins mais antigos durante a
    ...
    e fallback de ausência no diretório, `messaging.inferTargetChatType(...)` quando o núcleo
    precisar de um tipo de par antecipado, e `messaging.resolveOutboundSessionRoute(...)`
    para identidade de sessão e thread nativa do provedor.
...
## Referência de caminhos de importação
  ...
  | --- | --- | --- |
  | `plugin-sdk/plugin-entry` | Auxiliar de entrada canônica de plugin | `definePluginEntry` |
  ...
  | `plugin-sdk/setup-runtime` | Auxiliares de tempo de execução durante a configuração | `createSetupTranslator`, adaptadores de patch de configuração seguros para importação, auxiliares de notas de busca, `promptResolvedAllowFrom`, `splitSetupEntries`, proxies de configuração delegada |
  ...
  | `plugin-sdk/account-core` | Auxiliares de múltiplas contas | Auxiliares de lista/configuração/portão de ação de contas |
  | `plugin-sdk/account-id` | Auxiliares de ID de conta | `DEFAULT_ACCOUNT_ID`, normalização de ID de conta |
  | `plugin-sdk/account-resolution` | Auxiliares de busca de conta | Auxiliares de busca de conta + fallback padrão |
  ...
  | `plugin-sdk/messaging-targets` | Caminho de importação obsoleto para análise de destino | Use `plugin-sdk/channel-targets` para auxiliares genéricos de análise de destino, `plugin-sdk/channel-route` para comparação de rotas e `messaging.targetResolver` / `messaging.resolveOutboundSessionRoute` pertencentes ao plugin para resolução de destino específica do provedor |
  ...
  | `plugin-sdk/channel-outbound` | Auxiliares de ciclo de vida de mensagens de saída | Adaptadores de mensagem, recibos, auxiliares de envio durável, auxiliares de pré-visualização/transmissão ao vivo, opções de resposta, auxiliares de ciclo de vida, identidade de saída e planejamento de payload |
  ...
  | `plugin-sdk/agent-media-payload` | Auxiliares legados de payload de mídia | Construtor de payload de mídia de agente para layouts de campos legados |
  | `plugin-sdk/channel-runtime` | Shim de compatibilidade obsoleto | Apenas utilitários legados de tempo de execução de canal |
  ...
  | `plugin-sdk/runtime-env` | Auxiliares estreitos de ambiente de tempo de execução | Auxiliares de logger/ambiente de tempo de execução, tempo limite, retentativa e backoff |
  ...
  | `plugin-sdk/lazy-runtime` | Auxiliares preguiçosos de tempo de execução | `createLazyRuntimeModule`, `createLazyRuntimeMethod`, `createLazyRuntimeMethodBinder`, `createLazyRuntimeNamedExport`, `createLazyRuntimeSurface` |
  ...
  | `plugin-sdk/gateway-runtime` | Auxiliares do Gateway | Cliente Gateway, auxiliar de inicialização pronto para loop de eventos, resolução de host LAN anunciado e auxiliares de patch de status de canal |
  ...
  | `plugin-sdk/telegram-command-config` | Auxiliares de comandos do Telegram | Auxiliares de validação de comandos do Telegram estáveis para fallback quando a superfície de contrato do Telegram empacotado está indisponível |
  | `plugin-sdk/approval-runtime` | Auxiliares de prompt de aprovação | Payload de aprovação de exec/plugin, auxiliares de capacidade/perfil de aprovação, auxiliares nativos de roteamento/tempo de execução de aprovação e formatação estruturada de caminho de exibição de aprovação |
  ...
  | `plugin-sdk/approval-handler-runtime` | Auxiliares de manipulador de aprovação | Auxiliares mais amplos de tempo de execução do manipulador de aprovação; prefira as costuras mais estreitas de adaptador/Gateway quando forem suficientes |
  ...
  | `plugin-sdk/security-runtime` | Auxiliares de segurança | Auxiliares compartilhados de confiança, bloqueio de DM, arquivo/caminho limitado à raiz, conteúdo externo e coleta de segredos |
  | `plugin-sdk/ssrf-policy` | Auxiliares de política SSRF | Auxiliares de lista de permissões de hosts e política de rede privada |
  | `plugin-sdk/ssrf-runtime` | Auxiliares de tempo de execução SSRF | Despachante fixado, fetch protegido, auxiliares de política SSRF |
  | `plugin-sdk/system-event-runtime` | Auxiliares de evento de sistema | `enqueueSystemEvent`, `peekSystemEventEntries` |
  ...
  | `plugin-sdk/file-access-runtime` | Auxiliares de acesso a arquivo | Auxiliares seguros de caminho de arquivo/mídia local |
  ...
  | `plugin-sdk/diagnostic-runtime` | Auxiliares de bloqueio diagnóstico | `isDiagnosticFlagEnabled`, `isDiagnosticsEnabled` |
  ...
  | `plugin-sdk/fetch-runtime` | Auxiliares de fetch/proxy encapsulados | `resolveFetch`, auxiliares de proxy, auxiliares de opção EnvHttpProxyAgent |
  | `plugin-sdk/host-runtime` | Auxiliares de normalização de host | `normalizeHostname`, `normalizeScpRemoteHost` |
  | `plugin-sdk/retry-runtime` | Auxiliares de retentativa | `RetryConfig`, `retryAsync`, executores de política |
  ...
  | `plugin-sdk/command-auth` | Auxiliares de bloqueio de comandos e superfície de comandos | `resolveControlCommandGate`, auxiliares de autorização de remetente, auxiliares de registro de comandos incluindo formatação dinâmica de menu de argumentos |
  ...
  | `plugin-sdk/reply-runtime` | Tempo de execução compartilhado de resposta | Despacho de entrada, Heartbeat, planejador de respostas, fragmentação |
  ...
  | `plugin-sdk/status-helpers` | Auxiliares de status de canal | Construtores de resumo de status de canal/conta, padrões de estado de tempo de execução, auxiliares de metadados de problema |
  ...
  | `plugin-sdk/request-url` | Auxiliares de URL de requisição | Extrai URLs de string de entradas semelhantes a requisições |
  ...
  | `plugin-sdk/provider-setup` | Auxiliares selecionados para configuração de provedor local/auto-hospedado | Auxiliares de descoberta/configuração de provedor auto-hospedado |
  | `plugin-sdk/self-hosted-provider-setup` | Auxiliares focados para configuração de provedor auto-hospedado compatível com OpenAI | Os mesmos auxiliares de descoberta/configuração de provedor auto-hospedado |
  | `plugin-sdk/provider-auth-runtime` | Auxiliares de autenticação de tempo de execução de provedor | Auxiliares de resolução de chave de API em tempo de execução |
  | `plugin-sdk/provider-auth-api-key` | Auxiliares de configuração de chave de API de provedor | Auxiliares de onboarding/gravação de perfil para chave de API |
  ...
  | `plugin-sdk/provider-web-search-config-contract` | Auxiliares de configuração de web-search de provedor | Auxiliares estreitos de configuração/credencial de web-search para provedores que não precisam de conexão de habilitação de Plugin |
  ...
  | `plugin-sdk/provider-web-search` | Auxiliares de web-search de provedor | Auxiliares de registro/cache/tempo de execução de provedor de web-search |
  | `plugin-sdk/provider-tools` | Auxiliares de compatibilidade de ferramenta/schema de provedor | `ProviderToolCompatFamily`, `buildProviderToolCompatFamilyHooks` e limpeza + diagnósticos de schemas DeepSeek/Gemini/OpenAI |
  ...
  | `plugin-sdk/provider-transport-runtime` | Auxiliares de transporte de provedor | Auxiliares de transporte nativo de provedor, como fetch protegido, extração de texto de resultado de ferramenta, transformações de mensagem de transporte e streams de eventos de transporte graváveis |
  ...
  | `plugin-sdk/media-understanding` | Auxiliares de compreensão de mídia | Tipos de provedor de compreensão de mídia e exportações de auxiliares de imagem/áudio voltadas a provedores |
  ...
  | `plugin-sdk/text-chunking` | Auxiliares de divisão de texto | Auxiliar de divisão de texto de saída |
  | `plugin-sdk/speech` | Auxiliares de fala | Tipos de provedor de fala e auxiliares voltados a provedores para diretiva, registro e validação, além do construtor de TTS compatível com OpenAI |
  ...
  | `plugin-sdk/music-generation-core` | Núcleo compartilhado de geração de música | Tipos de geração de música, auxiliares de failover, consulta de provedor e parsing de ref de modelo |
  | `plugin-sdk/video-generation` | Auxiliares de geração de vídeo | Tipos de provedor/requisição/resultado de geração de vídeo |
  | `plugin-sdk/video-generation-core` | Núcleo compartilhado de geração de vídeo | Tipos de geração de vídeo, auxiliares de failover, consulta de provedor e parsing de ref de modelo |
  ...
  | `plugin-sdk/channel-plugin-common` | Prelúdio compartilhado de canal | Exportações de prelúdio compartilhado de Plugin de canal |
  ...
  | `plugin-sdk/direct-dm`, `plugin-sdk/direct-dm-access` | Fachadas obsoletas de compatibilidade | Use `plugin-sdk/channel-inbound` |
  | `plugin-sdk/direct-dm-guard-policy` | Auxiliares de guarda de Direct-DM | Auxiliares estreitos de política de guarda pré-criptografia |
  ...
  | `plugin-sdk/webhook-targets` | Auxiliares de destino de Webhook | Registro de destinos de Webhook e auxiliares de instalação de rota |
  ...
  | `plugin-sdk/memory-core-engine-runtime` | Fachada de tempo de execução do mecanismo de memória | Fachada de tempo de execução de índice/busca de memória |
  ...
  | `plugin-sdk/memory-core-host-engine-embeddings` | Mecanismo de embeddings do host de memória | Contratos de embeddings de memória, acesso ao registro, provedor local e auxiliares genéricos de lote/remoto; provedores remotos concretos vivem nos plugins proprietários correspondentes |
  ...
  | `plugin-sdk/memory-core-host-runtime-cli` | Tempo de execução CLI do host de memória | Auxiliares de tempo de execução CLI do host de memória |
  ...
  | `plugin-sdk/memory-host-core` | Alias de tempo de execução central do host de memória | Alias neutro em relação a fornecedor para auxiliares de tempo de execução central do host de memória |
  ...
  | `plugin-sdk/memory-host-search` | Fachada de busca do Active Memory | Fachada preguiçosa de tempo de execução do gerenciador de busca do Active Memory |
  | `plugin-sdk/memory-host-status` | Alias obsoleto de status do host de memória | Use `plugin-sdk/memory-core-host-status` |
  | `plugin-sdk/testing` | Utilitários de teste | Barrel obsoleto de compatibilidade local do repositório; use subcaminhos de teste locais do repositório focados, como `plugin-sdk/plugin-test-runtime`, `plugin-sdk/channel-test-helpers`, `plugin-sdk/channel-target-testing`, `plugin-sdk/test-env` e `plugin-sdk/test-fixtures` |
</Accordion>
...
a partir do subconjunto público.

As interfaces auxiliares reservadas para bundled plugins foram removidas do
mapa de exportação público do SDK, exceto facades de compatibilidade
documentadas explicitamente, como o shim obsoleto `plugin-sdk/discord` mantido
para o pacote publicado `@openclaw/discord@2026.3.13`. Auxiliares específicos
do proprietário ficam dentro do pacote do plugin proprietário; o comportamento
compartilhado do host deve passar por contratos genéricos do SDK, como
...
uma exportação, confira o código-fonte em `src/plugin-sdk/` ou pergunte aos
mantenedores qual contrato genérico deve ser responsável por ela.

## Depreciações ativas

Depreciações mais estreitas que se aplicam ao SDK de plugin, contrato de
provedor, superfície de runtime e manifesto. Cada uma ainda funciona hoje, mas
será removida em uma versão major futura. A entrada abaixo de cada item mapeia
a API antiga para sua substituição canônica.
...
<AccordionGroup>
  ...
  <Accordion title="Auxiliares de controle por menção → resolveInboundMentionDecision">
  ...
  <Accordion title="Shim de runtime de canal e auxiliares de ações de canal">
    ...
    estão obsoletos junto com exportações brutas de canal de "actions". Exponha
    capacidades pela superfície semântica `presentation` em vez disso - plugins
    de canal declaram o que renderizam (cards, botões, seletores), em vez de
  ...
  <Accordion title="Envelopes de canal em texto simples → BodyForAgent">
    ...
    **Novo**: `BodyForAgent` mais blocos estruturados de contexto do usuário.
    ...
    envelopes sintetizados voltados ao assistente, mas envelopes de entrada em
    texto simples estão sendo descontinuados.

    Áreas afetadas: `inbound_claim`, `message_received` e qualquer plugin de
    canal personalizado que pós-processava o texto `channelEnvelope`.
  ...
  <Accordion title="hook subagent_spawning → vinculação de thread do core">
    ...
    **Novo**: deixe o core preparar vinculações de subagente `thread: true`
    ...
    `api.on("subagent_spawned", handler)` apenas para observação pós-inicialização.
  ...
  <Accordion title="Tipos de descoberta de provedor → tipos de catálogo de provedor">
    ...
    Mais o pacote estático legado `ProviderCapabilities` - plugins de provedor
  ...
  <Accordion title="Hooks de política de raciocínio → resolveThinkingProfile">
    ...
    **Novo**: um único `resolveThinkingProfile(ctx)` que retorna um
    ...
    apenas quando o contrato de requisição configurado oferece suporte a ele.
  ...
  <Accordion title="Busca de env-var do provedor → setup.providers[].envVars">
    Campo de manifesto **antigo**: `providerAuthEnvVars: { anthropic: ["ANTHROPIC_API_KEY"] }`.

    **Novo**: espelhe a mesma busca de env-var em `setup.providers[].envVars`
    no manifesto. Isso consolida metadados de ambiente de setup/status em um
    só lugar e evita iniciar o runtime do plugin apenas para responder buscas
    de env-var.
  ...
  <Accordion title="Registro de plugin de memória → registerMemoryCapability">
    ...
    **Novo**: uma chamada na API de estado de memória -
    ...
    Mesmos slots, única chamada de registro. Auxiliares aditivos de prompt e
  ...
  <Accordion title="API de provedor de embeddings de memória">
    ...
    **Novo**: `api.registerEmbeddingProvider(...)` mais
    ...
    enquanto provedores existentes migram. A inspeção de plugins informa uso
    não bundled como dívida de compatibilidade.
  ...
  <Accordion title="Tipos de mensagens de sessão de subagente renomeados">
    ...
    `getSessionMessages`. Mesma assinatura; o método antigo encaminha a
    chamada para o novo.
  ...
  <Accordion title="runtime.tasks.flow → runtime.tasks.managedFlows">
    ...
    **Novo**: `runtime.tasks.managedFlows` mantém o runtime de mutação
    gerenciada do TaskFlow para plugins que criam, atualizam, cancelam ou
    executam tarefas filhas a partir de um fluxo. Use `runtime.tasks.flows`
    quando o plugin só precisar de leituras baseadas em DTO.
  ...
  <Accordion title="Factories de extensão embutida → middleware de resultado de ferramenta do agente">
    Coberto em "Como migrar → Migrar extensões embutidas de resultado de
    ferramenta para middleware" acima. Incluído aqui para completude: o caminho
    removido `api.registerEmbeddedExtensionFactory(...)`, exclusivo do runner
    embutido, é substituído por `api.registerAgentToolResultMiddleware(...)`
    com uma lista de runtimes explícita em `contracts.agentToolResultMiddleware`.
  </Accordion>
...
<Note>
Depreciações no nível da extensão (dentro de plugins bundled de canal/provedor
em `extensions/`) são rastreadas dentro dos próprios barrels `api.ts` e
`runtime-api.ts`. Elas não afetam contratos de plugins de terceiros e não são
listadas aqui. Se você consumir diretamente o barrel local de um plugin bundled,
leia os comentários de depreciação nesse barrel antes de atualizar.
</Note>
