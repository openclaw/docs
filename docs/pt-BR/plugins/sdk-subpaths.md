...
x-i18n:
    generated_at: "2026-05-03T21:36:31Z"
    model: gpt-5.5
    provider: openai
...
---
  ...
  O SDK de plugins é exposto como um conjunto de subcaminhos estreitos sob `openclaw/plugin-sdk/`.
  ...
  subcaminhos auxiliares reservados de plugins integrados aparecem ali, mas são detalhes de
  ...
  | `plugin-sdk/testing`                      | Barrel amplo de compatibilidade para testes legados de plugins; prefira subcaminhos de teste focados para novos testes de plugins                                           |
  | `plugin-sdk/plugin-test-api`              | Construtor mínimo de mock `OpenClawPluginApi` para testes unitários de registro direto de plugins                                                                             |
  | `plugin-sdk/agent-runtime-test-contracts` | Fixtures de contrato de adaptador agent-runtime nativo para perfis de autenticação, supressão de entrega, classificação de fallback, hooks de ferramentas, sobreposições de prompts, esquemas e reparo de transcrição |
  ...
  | `plugin-sdk/plugin-test-contracts`        | Auxiliares de contrato para registro de plugins, manifesto de pacote, artefato público, API de runtime, efeito colateral de importação e importação direta                   |
  | `plugin-sdk/plugin-test-runtime`          | Fixtures para testes de runtime de plugin, registro, registro de provider, assistente de configuração e fluxo de tarefas de runtime                                          |
  | `plugin-sdk/provider-test-contracts`      | Auxiliares de contrato para runtime de provider, autenticação, descoberta, integração, catálogo, capacidade de mídia, política de replay, áudio ao vivo de STT em tempo real, pesquisa/busca na web e assistente |
  | `plugin-sdk/provider-http-test-mocks`     | Mocks HTTP/autenticação opt-in do Vitest para testes de provider que exercitam `plugin-sdk/provider-http`                                                                    |
  | `plugin-sdk/test-env`                     | Fixtures de ambiente de teste, fetch/rede, servidor HTTP descartável, requisição recebida, teste ao vivo, sistema de arquivos temporário e controle de tempo                 |
  ...
  | `plugin-sdk/test-node-mocks`              | Auxiliares focados de mock de built-ins do Node para uso dentro de factories Vitest `vi.mock("node:*")`                                                                      |
  | `plugin-sdk/migration`                    | Auxiliares de itens de provider de migração, como `createMigrationItem`, constantes de motivo, marcadores de status de item, auxiliares de redação e `summarizeMigrationItems` |
  | `plugin-sdk/migration-runtime`            | Auxiliares de migração de runtime, como `copyMigrationFileItem`, `withCachedMigrationConfigRuntime` e `writeMigrationReport`                                                |
  ...
  <Accordion title="Subcaminhos de canal">
    ...
    | `plugin-sdk/account-core` | Auxiliares de configuração multi-conta/gate de ações, auxiliares de fallback de conta padrão |
    ...
    | `plugin-sdk/channel-config-schema` | Primitivas compartilhadas de esquema de configuração de canal, mais construtores Zod e JSON/TypeBox diretos |
    | `plugin-sdk/bundled-channel-config-schema` | Esquemas de configuração de canal integrado do OpenClaw somente para plugins integrados mantidos |
    ...
    | `plugin-sdk/telegram-command-config` | Auxiliares de normalização/validação de comandos personalizados do Telegram com fallback de contrato integrado |
    | `plugin-sdk/command-gating` | Auxiliares estreitos de gate de autorização de comandos |
    | `plugin-sdk/channel-policy` | `resolveChannelGroupRequireMention` |
    | `plugin-sdk/channel-lifecycle` | `createAccountStatusSink`, `createChannelRunQueue`, auxiliares de ciclo de vida/finalização de stream de rascunho |
    | `plugin-sdk/inbound-envelope` | Auxiliares compartilhados de rota de entrada + construção de envelope |
    ...
    | `plugin-sdk/outbound-runtime` | Auxiliares de entrega de saída, identidade, delegate de envio, sessão, formatação e planejamento de payload |
    | `plugin-sdk/poll-runtime` | Auxiliares estreitos de normalização de poll |
    | `plugin-sdk/thread-bindings-runtime` | Auxiliares de ciclo de vida de vinculação de threads e adaptadores |
    ...
    | `plugin-sdk/direct-dm` | Auxiliares compartilhados de autenticação/guard de DM direto |
    ...
    | `plugin-sdk/interactive-runtime` | Auxiliares de apresentação semântica de mensagens, entrega e resposta interativa legada. Consulte [Apresentação de Mensagens](/pt-BR/plugins/message-presentation) |
    ...
    | `plugin-sdk/channel-actions` | Auxiliares de ações de mensagem de canal, mais auxiliares obsoletos de esquema nativo mantidos para compatibilidade de plugins |
    | `plugin-sdk/channel-route` | Auxiliares compartilhados de normalização de rota, resolução de alvo orientada por parser, stringificação de thread-id, chaves de rota dedupe/compactas, tipos de alvo analisado e comparação de rota/alvo |
    | `plugin-sdk/channel-targets` | Auxiliares de análise de alvos; chamadores de comparação de rotas devem usar `plugin-sdk/channel-route` |
    | `plugin-sdk/channel-contract` | Tipos de contrato de canal |
    | `plugin-sdk/channel-feedback` | Fiação de feedback/reação |
    | `plugin-sdk/channel-secret-runtime` | Auxiliares estreitos de contrato de segredo, como `collectSimpleChannelFieldAssignments`, `getChannelSurface`, `pushAssignment` e tipos de alvo secreto |
  </Accordion>
  ...
  <Accordion title="Subcaminhos de provedor">
    ...
    | `plugin-sdk/lmstudio-runtime` | Fachada compatível do runtime do LM Studio para padrões do servidor local, descoberta de modelos, cabeçalhos de solicitação e auxiliares de modelos carregados |
    ...
    | `plugin-sdk/provider-model-shared` | `ProviderReplayFamily`, `buildProviderReplayFamilyHooks`, `normalizeModelCompat`, construtores compartilhados de políticas de replay, auxiliares de endpoint de provedor e auxiliares de normalização de ID de modelo, como `normalizeNativeXaiModelId` |
    | `plugin-sdk/provider-catalog-runtime` | Hook de runtime de ampliação de catálogo de provedor e seams de registro plugin-provedor para testes de contrato |
    ...
    | `plugin-sdk/provider-tools` | `ProviderToolCompatFamily`, `buildProviderToolCompatFamilyHooks`, limpeza + diagnósticos de esquema do Gemini e auxiliares de compatibilidade do xAI, como `resolveXaiModelCompatPatch` / `applyXaiModelCompat` |
  ...
  <Accordion title="Subcaminhos de autenticação e segurança">
    ...
    | `plugin-sdk/approval-delivery-runtime` | Adaptadores nativos de recurso/entrega de aprovação |
    ...
    | `plugin-sdk/approval-runtime` | Auxiliares de payload de aprovação de execução/Plugin, auxiliares nativos de roteamento/runtime de aprovação e auxiliares de exibição estruturada de aprovação, como `formatApprovalDisplayPath` |
    | `plugin-sdk/reply-dedupe` | Auxiliares estreitos de redefinição de deduplicação de resposta recebida |
    ...
    | `plugin-sdk/secret-ref-runtime` | Auxiliares estreitos de tipagem `coerceSecretRef` e SecretRef para parsing de contrato/configuração de segredo |
    ...
    | `plugin-sdk/ssrf-policy` | Auxiliares de allowlist de hosts e política de SSRF de rede privada |
    | `plugin-sdk/ssrf-dispatcher` | Auxiliares estreitos de dispatcher fixado sem a ampla superfície de runtime de infraestrutura |
    ...
    | `plugin-sdk/webhook-request-guards` | Auxiliares de tamanho/timeout de corpo de solicitação |
  </Accordion>

  <Accordion title="Runtime e subcaminhos de armazenamento">
    | Subcaminho | Exportações principais |
    | --- | --- |
    ...
    | `plugin-sdk/cli-runtime` | Helpers de formatação da CLI, espera, versão, invocação de argumentos e grupos de comandos lazy |
    ...
    | `plugin-sdk/config-mutation` | Helpers transacionais de mutação de configuração, como `mutateConfigFile`, `replaceConfigFile` e `logConfigUpdated` |
    ...
    | `plugin-sdk/text-autolink-runtime` | Detecção de links automáticos de referência de arquivo sem o barrel amplo de text-runtime |
    | `plugin-sdk/approval-runtime` | Helpers de aprovação de execução/Plugin, builders de capacidade de aprovação, helpers de autenticação/perfil, helpers nativos de roteamento/runtime e formatação de caminho de exibição de aprovação estruturada |
    | `plugin-sdk/reply-runtime` | Helpers compartilhados de runtime de entrada/resposta, divisão em partes, despacho, Heartbeat, planejador de resposta |
    | `plugin-sdk/reply-dispatch-runtime` | Helpers restritos de despacho/finalização de resposta e rótulo de conversa |
    | `plugin-sdk/reply-history` | Helpers compartilhados de histórico de respostas de janela curta e marcadores como `buildHistoryContext`, `HISTORY_CONTEXT_MARKER`, `recordPendingHistoryEntry` e `clearHistoryEntriesIfEnabled` |
    | `plugin-sdk/reply-reference` | `createReplyReferencePlanner` |
    | `plugin-sdk/reply-chunking` | Helpers restritos de divisão de texto/markdown em partes |
    | `plugin-sdk/session-store-runtime` | Helpers de caminho do armazenamento de sessão, chave de sessão, atualizado-em e mutação de armazenamento |
    ...
    | `plugin-sdk/talk-config-runtime` | Helpers de resolução de configuração de provedor de conversa |
    | `plugin-sdk/json-store` | Pequenos helpers de leitura/gravação de estado JSON |
    | `plugin-sdk/file-lock` | Helpers de bloqueio de arquivo reentrantes |
    | `plugin-sdk/persistent-dedupe` | Helpers de cache de desduplicação com suporte em disco |
    ...
    | `plugin-sdk/acp-binding-resolve-runtime` | Resolução somente leitura de vinculação ACP sem importações de inicialização do ciclo de vida |
    | `plugin-sdk/agent-config-primitives` | Primitivas restritas de schema de configuração de runtime do agente |
    ...
    | `plugin-sdk/models-provider-runtime` | Helpers de resposta de comando/provedor de `/models` |
    ...
    | `plugin-sdk/agent-harness` | Superfície experimental de Plugin confiável para harnesses de agente de baixo nível: tipos de harness, helpers de direcionamento/abortamento de execução ativa, helpers de ponte de ferramentas do OpenClaw, helpers de política de ferramentas de plano de runtime, classificação de resultado de terminal, helpers de formatação/detalhe de progresso de ferramentas e utilitários de resultado de tentativa |
    | `plugin-sdk/provider-zai-endpoint` | Helpers de detecção de endpoint Z.AI |
    | `plugin-sdk/async-lock-runtime` | Helper de bloqueio assíncrono local do processo para pequenos arquivos de estado de runtime |
    ...
    | `plugin-sdk/transport-ready-runtime` | Helper de espera por prontidão de transporte |
    ...
    | `plugin-sdk/fetch-runtime` | Fetch encapsulado, proxy, opção EnvHttpProxyAgent e helpers de lookup fixo |
    ...
    | `plugin-sdk/session-binding-runtime` | Estado atual de vinculação da conversa sem roteamento de vinculação configurado nem armazenamentos de pareamento |
    ...
    | `plugin-sdk/string-coerce-runtime` | Helpers restritos de coerção e normalização de registro primitivo/string sem importações de markdown/logging |
    | `plugin-sdk/host-runtime` | Helpers de normalização de nome de host e host SCP |
    ...
    | `plugin-sdk/directory-runtime` | Consulta/desduplicação de diretório com suporte de configuração |
  ...
  <Accordion title="Subpaths de capacidade e testes">
    ...
    | `plugin-sdk/media-understanding` | Tipos de provedores de compreensão de mídia, além de exportações de helpers de imagem/áudio voltadas para provedores |
    | `plugin-sdk/text-runtime` | Helpers compartilhados de texto/markdown/logging, como remoção de texto visível para assistente, helpers de renderização/fragmentação/tabelas em markdown, helpers de redação, helpers de tags de diretiva e utilitários de texto seguro |
    | `plugin-sdk/text-chunking` | Helper de fragmentação de texto de saída |
    | `plugin-sdk/speech` | Tipos de provedores de fala, além de exportações de helpers de diretiva, registro, validação, construtor TTS compatível com OpenAI e fala voltadas para provedores |
    ...
    | `plugin-sdk/image-generation` | Tipos de provedores de geração de imagem, além de helpers de ativos de imagem/URLs de dados e o construtor de provedor de imagem compatível com OpenAI |
    ...
    | `plugin-sdk/music-generation-core` | Tipos compartilhados de geração de música, helpers de failover, busca de provedor e análise de model-ref |
    | `plugin-sdk/video-generation` | Tipos de provedor/solicitação/resultado de geração de vídeo |
    | `plugin-sdk/video-generation-core` | Tipos compartilhados de geração de vídeo, helpers de failover, busca de provedor e análise de model-ref |
    ...
    | `plugin-sdk/testing` | Módulo agregador amplo de compatibilidade para testes legados de plugins. Novos testes de extensões devem importar subpaths focados do SDK, como `plugin-sdk/agent-runtime-test-contracts`, `plugin-sdk/plugin-test-runtime`, `plugin-sdk/channel-test-helpers`, `plugin-sdk/test-env` ou `plugin-sdk/test-fixtures` |
    ...
    | `plugin-sdk/agent-runtime-test-contracts` | Fixtures nativas de contrato de adaptador agent-runtime para testes de autenticação, entrega, fallback, hook de ferramenta, sobreposição de prompt, schema e projeção de transcrição |
    ...
    | `plugin-sdk/provider-test-contracts` | Helpers de contrato de runtime de provedor, autenticação, descoberta, integração, catálogo, assistente, capacidade de mídia, política de replay, áudio ao vivo de STT em tempo real, pesquisa/busca na Web e stream |
    ...
    | `plugin-sdk/test-fixtures` | Fixtures genéricas de captura de runtime de CLI, contexto de sandbox, gravador de Skills, mensagem de agente, evento de sistema, recarregamento de módulo, caminho de plugin incluído, texto de terminal, fragmentação, token de autenticação e caso tipado |
    | `plugin-sdk/test-node-mocks` | Helpers focados de mock de built-ins do Node para uso dentro de factories Vitest `vi.mock("node:*")` |
  </Accordion>

  <Accordion title="Subpaths de memória">
    | Subpath | Exportações principais |
    | --- | --- |
    | `plugin-sdk/memory-core` | Superfície de helpers memory-core incluída para helpers de gerenciador/configuração/arquivo/CLI |
    ...
    | `plugin-sdk/memory-host-markdown` | Helpers compartilhados de markdown gerenciado para plugins adjacentes a memória |
  ...
  <Accordion title="Subpaths reservados de helpers incluídos">
    Atualmente não há subpaths reservados do SDK para helpers incluídos. Helpers
    específicos de proprietários ficam dentro do pacote do plugin proprietário,
    enquanto contratos de host reutilizáveis usam subpaths genéricos do SDK, como
