...
O protocolo WS do Gateway é o **plano de controle único + transporte de nó** para
OpenClaw. Todos os clientes (CLI, interface web, app macOS, nós iOS/Android, nós
sem interface) se conectam por WebSocket e declaram seu **papel** + **escopo** no
...
- Frames pré-conexão são limitados a 64 KiB. Após um handshake bem-sucedido, os clientes
  ...
  conteúdo de anexos, corpo bruto do frame, tokens, cookies nem valores secretos.
...
Enquanto o Gateway ainda está concluindo os sidecars de inicialização, a solicitação `connect` pode
...
Clientes de backend confiáveis do mesmo processo (`client.id: "gateway-client"`,
...
clientes com origem em navegador, clientes de nó e clientes explícitos de token de dispositivo/identidade de dispositivo
...
adicionais de papel delimitado em `deviceTokens`:
...
Para o fluxo de bootstrap integrado de nó/operador, o token primário de nó permanece
`scopes: []` e qualquer token de operador transferido permanece limitado à lista de permissões do operador
...
O escopo do método é apenas o primeiro bloqueio. Alguns comandos slash acessados por
`chat.send` aplicam verificações mais estritas em nível de comando além disso. Por exemplo, gravações persistentes
...
O Gateway trata isso como **reivindicações** e aplica listas de permissões no servidor.

## Presença

- `system-presence` retorna entradas indexadas por identidade do dispositivo.
- Entradas de presença incluem `deviceId`, `roles` e `scopes` para que as interfaces possam mostrar uma única linha por dispositivo
...
- `node.list` inclui campos opcionais `lastSeenAtMs` e `lastSeenReason`. Nós conectados informam
  seu horário de conexão atual como `lastSeenAtMs` com motivo `connect`; nós pareados também podem informar
...
Eventos de broadcast WebSocket enviados pelo servidor são controlados por escopo para que sessões com escopo de pareamento ou apenas de nó não recebam passivamente conteúdo de sessão.
...
A superfície WS pública é mais ampla que os exemplos de handshake/autenticação acima. Esta
não é uma listagem gerada — `hello-ok.features.methods` é uma lista conservadora
de descoberta criada a partir de `src/gateway/server-methods-list.ts` mais exportações carregadas
...
<AccordionGroup>
  <Accordion title="Sistema e identidade">
    ...
    - `diagnostics.stability` retorna o registrador recente e delimitado de estabilidade de diagnóstico. Ele mantém metadados operacionais como nomes de eventos, contagens, tamanhos em bytes, leituras de memória, estado de fila/sessão, nomes de canal/plugin e ids de sessão. Ele não mantém texto de chat, corpos de webhook, saídas de ferramentas, corpos brutos de solicitações ou respostas, tokens, cookies nem valores secretos. Escopo de leitura de operador é obrigatório.
  ...
  <Accordion title="Modelos e uso">
    - `models.list` retorna o catálogo de modelos permitido pelo tempo de execução. Passe `{ "view": "configured" }` para modelos configurados em tamanho adequado para seletores (`agents.defaults.models` primeiro, depois `models.providers.*.models`), ou `{ "view": "all" }` para o catálogo completo.
    - `usage.status` retorna resumos das janelas de uso/cota restante do provedor.
    - `usage.cost` retorna resumos agregados de uso de custo para um intervalo de datas.
    - `doctor.memory.status` retorna a prontidão da memória vetorial / embedding em cache para o workspace ativo do agente padrão. Passe `{ "probe": true }` ou `{ "deep": true }` somente quando o chamador quiser explicitamente um ping ativo no provedor de embeddings.
  ...
  <Accordion title="Mensagens e logs">
    ...
    - `logs.tail` retorna o tail configurado do log de arquivo do Gateway com controles de cursor/limite e máximo de bytes.
  ...
  <Accordion title="Talk e TTS">
    ...
    - `talk.speak` sintetiza fala pelo provedor de fala ativo do Talk.
  ...
  <Accordion title="Segredos, configuração, atualização e assistente">
    ...
    - `config.schema` retorna o payload do esquema de configuração ativo usado pela Control UI e pelas ferramentas de CLI: esquema, `uiHints`, versão e metadados de geração, incluindo metadados de esquema de Plugin + canal quando o tempo de execução consegue carregá-los. O esquema inclui metadados de campo `title` / `description` derivados dos mesmos rótulos e textos de ajuda usados pela UI, incluindo branches de composição de objeto aninhado, curinga, item de array e `anyOf` / `oneOf` / `allOf` quando existe documentação de campo correspondente.
    - `config.schema.lookup` retorna um payload de consulta com escopo por caminho para um caminho de configuração: caminho normalizado, um nó de esquema superficial, hint correspondente + `hintPath` e resumos de filhos imediatos para detalhamento na UI/CLI. Os nós de esquema da consulta mantêm a documentação voltada ao usuário e os campos comuns de validação (`title`, `description`, `type`, `enum`, `const`, `format`, `pattern`, limites numéricos/de string/de array/de objeto e flags como `additionalProperties`, `deprecated`, `readOnly`, `writeOnly`). Os resumos de filhos expõem `key`, `path` normalizado, `type`, `required`, `hasChildren`, além de `hint` / `hintPath` correspondentes.
    - `update.run` executa o fluxo de atualização do Gateway e agenda uma reinicialização somente quando a própria atualização for bem-sucedida. Atualizações do gerenciador de pacotes forçam uma reinicialização de atualização sem adiamento e sem cooldown após a troca do pacote, para que o processo antigo do Gateway não continue carregando sob demanda a partir de uma árvore `dist` substituída.
  ...
  <Accordion title="Controle de sessão">
    - `sessions.list` retorna o índice atual de sessões, incluindo metadados `agentRuntime` por linha quando um backend de tempo de execução de agente está configurado.
  ...
  <Accordion title="Pareamento de Node, invocação e trabalho pendente">
    - `node.pair.request`, `node.pair.list`, `node.pair.approve`, `node.pair.reject`, `node.pair.remove` e `node.pair.verify` cobrem pareamento de Node e verificação de inicialização.
    ...
    - `node.pending.pull` e `node.pending.ack` são as APIs de fila de Nodes conectados.
  ...
  <Accordion title="Famílias de aprovação">
    ...
    - `exec.approvals.node.get` e `exec.approvals.node.set` gerenciam a política de aprovação de exec local do Node por comandos de retransmissão do Node.
...
- `voicewake.changed`: configuração de acionadores de palavra de ativação alterada.
...
- Operadores podem chamar `tools.invoke` (`operator.write`) para invocar uma ferramenta disponível pelo mesmo caminho de política do gateway que `/tools/invoke`.
  ...
  - A resposta é um envelope voltado ao SDK com `ok`, `toolName`, `output` opcional e campos `error` tipados. Recusas de aprovação ou política retornam `ok:false` no payload, em vez de contornar o pipeline de política de ferramentas do gateway.
...
- Operadores podem chamar `skills.update` (`operator.admin`) em dois modos:
  ...
  - O modo de configuração aplica patches em valores de `skills.entries.<skillKey>`, como `enabled`, `apiKey` e `env`.
...
- `"configured"`: comportamento dimensionado para seletor. Se `agents.defaults.models` estiver configurado, ele ainda prevalece. Caso contrário, a resposta usa entradas explícitas de `models.providers.*.models`, recorrendo ao catálogo completo apenas quando não houver nenhuma linha de modelo configurada.
...
- Se um chamador alterar `command`, `rawCommand`, `cwd`, `agentId` ou `sessionKey` entre o preparo e o encaminhamento final aprovado de `system.run`, o gateway rejeita a execução em vez de confiar no payload alterado.
...
- `bestEffortDeliver=true` permite fallback para execução somente na sessão quando nenhuma rota entregável externa puder ser resolvida (por exemplo, sessões internas/webchat ou configurações ambíguas de múltiplos canais).
...
O cliente de referência em `src/gateway/client.ts` usa estes padrões. Os valores são estáveis no protocolo v3 e são a baseline esperada para clientes de terceiros.

| Constante                                 | Padrão                                                | Fonte                                                                                      |
| ----------------------------------------- | ----------------------------------------------------- | ------------------------------------------------------------------------------------------ |
| `PROTOCOL_VERSION`                        | `3`                                                   | `src/gateway/protocol/schema/protocol-schemas.ts`                                          |
| Timeout da solicitação (por RPC)          | `30_000` ms                                           | `src/gateway/client.ts` (`requestTimeoutMs`)                                               |
| Timeout de pré-autenticação / desafio de conexão | `15_000` ms                                    | `src/gateway/handshake-timeouts.ts` (config/env pode elevar o orçamento pareado servidor/cliente) |
...
- Modos que carregam identidade, como Tailscale Serve
  ...
  `gateway.auth.mode: "trusted-proxy"` não loopback, satisfazem a verificação de autenticação de conexão a partir dos
  cabeçalhos da requisição em vez de `connect.params.auth.*`.
- A entrada privada `gateway.auth.mode: "none"` ignora totalmente a autenticação de conexão por segredo compartilhado;
  não exponha esse modo em entrada pública/não confiável.
- Após o emparelhamento, o Gateway emite um **token de dispositivo** com escopo restrito à função da conexão
  + escopos. Ele é retornado em `hello-ok.auth.deviceToken` e deve ser
...
- Reconectar com esse token de dispositivo **armazenado** também deve reutilizar o conjunto de escopos aprovados
  armazenado para esse token. Isso preserva o acesso de leitura/sondagem/status
...
- Entradas adicionais de `hello-ok.auth.deviceTokens` são tokens de transferência de bootstrap.
  Persista-os somente quando a conexão usou autenticação de bootstrap em um transporte confiável,
  como `wss://` ou emparelhamento por loopback/local.
...
- `device.token.rotate` retorna metadados de rotação. Ele ecoa o token de portador substituto
  somente para chamadas do mesmo dispositivo que já estão autenticadas com
  esse token de dispositivo, para que clientes somente com token possam persistir a substituição antes de
  reconectar. Rotações compartilhadas/de administrador não ecoam o token de portador.
- Emissão, rotação e revogação de tokens permanecem limitadas ao conjunto de funções aprovadas
  registrado na entrada de emparelhamento desse dispositivo; a mutação de token não pode expandir nem
  direcionar uma função de dispositivo que a aprovação de emparelhamento nunca concedeu.
- Para sessões de token de dispositivo emparelhado, o gerenciamento de dispositivos é autoescopado, a menos que o
  chamador também tenha `operator.admin`: chamadores que não são administradores podem remover/revogar/rotacionar
  somente sua **própria** entrada de dispositivo.
- `device.token.rotate` e `device.token.revoke` também verificam o conjunto de escopos do token de operador alvo
  contra os escopos da sessão atual do chamador. Chamadores que não são administradores
...
- Nós devem incluir uma identidade de dispositivo estável (`device.id`) derivada de uma
  impressão digital do par de chaves.
- Gateways emitem tokens por dispositivo + função.
- Aprovações de emparelhamento são necessárias para novos IDs de dispositivo, a menos que a aprovação automática local
  esteja habilitada.
- A aprovação automática de emparelhamento é centrada em conexões diretas via local loopback.
...
- Conexões de mesma máquina via tailnet ou LAN ainda são tratadas como remotas para emparelhamento e
  exigem aprovação.
- Clientes WS normalmente incluem identidade `device` durante `connect` (operador +
  ...
  - autenticação de operador da UI de Controle bem-sucedida com `gateway.auth.mode: "trusted-proxy"`.
  - `gateway.controlUi.dangerouslyDisableDeviceAuth=true` (quebra de emergência, rebaixamento severo de segurança).
  - RPCs de backend `gateway-client` via loopback direto autenticados com o token/senha compartilhado
    do Gateway.
...
- Assinaturas legadas `v2` continuam aceitas por compatibilidade, mas a fixação de
