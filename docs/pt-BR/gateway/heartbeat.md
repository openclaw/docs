---
read_when:
    - Ajustar a cadência ou as mensagens de Heartbeat
...
title: Heartbeat
x-i18n:
    generated_at: "2026-05-02T20:47:18Z"
    model: gpt-5.5
    provider: openai
...
<Note>
**Heartbeat vs Cron?** Consulte [Automação e Tarefas](/pt-BR/automation) para orientações sobre quando usar cada um.
</Note>
...
Heartbeat é um turno agendado da sessão principal — ele **não** cria registros de [tarefa em segundo plano](/pt-BR/automation/tasks). Registros de tarefas são para trabalho separado (execuções ACP, subagentes, trabalhos Cron isolados).
...
<Steps>
  ...
  <Step title="Decida para onde as mensagens de heartbeat devem ir">
    `target: "none"` é o padrão; defina `target: "last"` para encaminhar para o último contato.
  </Step>
...
- O prompt de heartbeat é enviado **verbatim** como a mensagem do usuário. O prompt do sistema inclui uma seção "Heartbeat" somente quando heartbeats estão ativados para o agente padrão, e a execução é sinalizada internamente.
...
- O horário ativo (`heartbeat.activeHours`) é verificado no fuso horário configurado. Fora da janela, heartbeats são ignorados até o próximo tick dentro da janela.
- Heartbeats são adiados automaticamente enquanto trabalho Cron está ativo ou na fila. Defina `heartbeat.skipWhenBusy: true` para adiar também em vias ocupadas extras (subagente ou trabalho de comando aninhado); isso é útil para Ollama local e outros hosts de runtime único restritos.

## Para que serve o prompt de heartbeat

O prompt padrão é intencionalmente amplo:

- **Tarefas em segundo plano**: "Consider outstanding tasks" incentiva o agente a revisar acompanhamentos (caixa de entrada, calendário, lembretes, trabalho na fila) e destacar qualquer coisa urgente.
- **Check-in humano**: "Checkup sometimes on your human during day time" incentiva uma mensagem leve ocasional de "precisa de algo?", mas evita spam noturno usando seu fuso horário local configurado (consulte [Fuso horário](/pt-BR/concepts/timezone)).
...
Se você quiser que um heartbeat faça algo muito específico (por exemplo, "verificar estatísticas do Gmail PubSub" ou "verificar a integridade do Gateway"), defina `agents.defaults.heartbeat.prompt` (ou `agents.list[].heartbeat.prompt`) para um corpo personalizado (enviado verbatim).

## Contrato de resposta

- Se nada precisar de atenção, responda com **`HEARTBEAT_OK`**.
- Execuções de heartbeat com suporte a ferramentas podem chamar `heartbeat_respond` com `notify: false` para nenhuma atualização visível, ou `notify: true` mais `notificationText` para um alerta. Quando presente, a resposta estruturada da ferramenta tem precedência sobre o fallback de texto.
...
Fora dos heartbeats, um `HEARTBEAT_OK` solto no início/fim de uma mensagem é removido e registrado em log; uma mensagem que contém apenas `HEARTBEAT_OK` é descartada.
...
Fora desta janela (antes das 9h ou depois das 22h no horário do Leste), heartbeats são ignorados. O próximo tick agendado dentro da janela será executado normalmente.
...
<Warning>
Não defina os mesmos horários de `start` e `end` (por exemplo, de `08:00` a `08:00`). Isso é tratado como uma janela de largura zero, então heartbeats são sempre ignorados.
</Warning>
...
<ParamField path="lightContext" type="boolean" default="false">
  Quando true, execuções de heartbeat usam contexto de inicialização leve e mantêm apenas `HEARTBEAT.md` dos arquivos de inicialização do workspace.
</ParamField>
...
<ParamField path="skipWhenBusy" type="boolean" default="false">
  Quando true, execuções de heartbeat são adiadas em vias ocupadas extras: subagente ou trabalho de comando aninhado. As vias de Cron sempre adiam heartbeats, mesmo sem esta flag, para que hosts de modelos locais não executem prompts de Cron e heartbeat ao mesmo tempo.
</ParamField>
...
<ParamField path="accountId" type="string">
  Id de conta opcional para canais com várias contas. Quando `target: "last"`, o id da conta se aplica ao último canal resolvido se ele der suporte a contas; caso contrário, é ignorado. Se o id da conta não corresponder a uma conta configurada para o canal resolvido, a entrega será ignorada.
...
<ParamField path="suppressToolErrorWarnings" type="boolean">
  Quando verdadeiro, suprime cargas de aviso de erro de ferramenta durante execuções de Heartbeat.
...
- Omitido ou `"user"`: usa seu `agents.defaults.userTimezone` se definido; caso contrário, usa o fuso horário do sistema host como fallback.
- `"local"`: sempre usa o fuso horário do sistema host.
- Qualquer identificador IANA (por exemplo, `America/New_York`): usado diretamente; se inválido, usa como fallback o comportamento `"user"` acima.
...
<AccordionGroup>
  <Accordion title="Roteamento de sessão e destino">
    ...
    - Se a fila principal, a faixa da sessão de destino, a faixa de Cron ou um trabalho de Cron ativo estiver ocupado, o Heartbeat será ignorado e tentado novamente mais tarde.
    - Se `skipWhenBusy: true`, subagentes e faixas aninhadas também adiam execuções de Heartbeat.
  ...
  <Accordion title="Visibilidade e comportamento de salto">
    - Se `showOk`, `showAlerts` e `useIndicator` estiverem todos desativados, a execução é ignorada de início como `reason=alerts-disabled`.
    - Se apenas a entrega de alertas estiver desativada, o OpenClaw ainda pode executar o Heartbeat, atualizar timestamps de tarefas vencidas, restaurar o timestamp de inatividade da sessão e suprimir a carga de alerta externa.
    - Se o destino de Heartbeat resolvido der suporte a digitação, o OpenClaw mostra digitação enquanto a execução de Heartbeat está ativa. Isso usa o mesmo destino para o qual o Heartbeat enviaria a saída do chat, e é desativado por `typingMode: "never"`.

  </Accordion>
  <Accordion title="Ciclo de vida da sessão e auditoria">
    - Respostas somente de Heartbeat **não** mantêm a sessão viva. Metadados de Heartbeat podem atualizar a linha da sessão, mas a expiração por inatividade usa `lastInteractionAt` da última mensagem real de usuário/canal, e a expiração diária usa `sessionStartedAt`.
    - O histórico da Control UI e do WebChat oculta prompts de Heartbeat e confirmações somente OK. A transcrição da sessão subjacente ainda pode conter esses turnos para auditoria/reprodução.
    - [Tarefas em segundo plano](/pt-BR/automation/tasks) destacadas podem enfileirar um evento do sistema e acordar o Heartbeat quando a sessão principal deve perceber algo rapidamente. Esse despertar não faz a execução de Heartbeat virar uma tarefa em segundo plano.
...
Se um arquivo `HEARTBEAT.md` existir no workspace, o prompt padrão instrui o agente a lê-lo. Pense nele como sua "checklist de heartbeat": pequeno, estável e seguro para incluir a cada 30 minutos.
...
Mantenha-o minúsculo (checklist curta ou lembretes) para evitar inchaço de prompt.
...
`HEARTBEAT.md` também dá suporte a um pequeno bloco estruturado `tasks:` para verificações baseadas em intervalo dentro do próprio Heartbeat.
...
<AccordionGroup>
  <Accordion title="Comportamento">
    ...
    - Se nenhuma tarefa estiver vencida, o Heartbeat é completamente ignorado (`reason=no-tasks-due`) para evitar uma chamada desperdiçada ao modelo.
    ...
    - Timestamps da última execução da tarefa são armazenados no estado da sessão (`heartbeatTaskState`), então intervalos sobrevivem a reinicializações normais.
    - Timestamps de tarefas só avançam depois que uma execução de Heartbeat conclui seu caminho normal de resposta. Execuções ignoradas por `empty-heartbeat-file` / `no-tasks-due` não marcam tarefas como concluídas.
...
Se quiser que isso aconteça proativamente, você também pode incluir uma linha explícita no seu prompt de Heartbeat, como: "Se a checklist ficar desatualizada, atualize HEARTBEAT.md com uma melhor."
...
Quando habilitado, Heartbeats também entregarão uma mensagem separada prefixada com `Reasoning:` (mesmo formato que `/reasoning on`). Isso pode ser útil quando o agente está gerenciando várias sessões/codexes e você quer ver por que ele decidiu enviar um ping — mas também pode vazar mais detalhes internos do que você deseja. Prefira mantê-lo desativado em chats em grupo.
...
- Use `target: "none"` se você quiser apenas atualizações de estado interno.
...
Heartbeats atuais preservam o modelo de runtime existente da sessão compartilhada depois que a execução é concluída. Você ainda pode usar `isolatedSession: true` para executar Heartbeats em uma nova sessão, combiná-lo com `lightContext: true` para o menor prompt ou escolher um modelo de Heartbeat com uma janela de contexto grande o suficiente para a sessão compartilhada.

## Relacionado

- [Automação e tarefas](/pt-BR/automation) — todos os mecanismos de automação em resumo
- [Tarefas em segundo plano](/pt-BR/automation/tasks) — como o trabalho destacado é rastreado
