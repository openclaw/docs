...
x-i18n:
    generated_at: "2026-04-07T05:28:04Z"
    model: gpt-5.4
    provider: openai
...
- `openclaw doctor` não relata problemas de configuração/serviço que bloqueiem o uso.
- `openclaw channels status --probe` mostra status de transporte ao vivo por conta e,
  quando compatível, resultados de sonda/auditoria como `works` ou `audit ok`.

## Uso extra exigido pelo Anthropic 429 para contexto longo
...
- Incompatibilidades em listas de permissão de canal/grupo.

Assinaturas comuns:

- `drop guild message (mention required` → mensagem de grupo ignorada até receber menção.
...
Quando a dashboard/control UI não se conectar, valide URL, modo de autenticação e premissas de contexto seguro.
...
- URL de sonda e URL da dashboard corretas.
...
- `origin not allowed` → `Origin` do navegador não está em `gateway.controlUi.allowedOrigins`
  (ou você está se conectando de uma origem de navegador não loopback sem uma
  allowlist explícita).
...
- `device signature invalid` / `device signature expired` → o cliente assinou a carga
  errada (ou um timestamp desatualizado) para o handshake atual.
...
- Essa nova tentativa com token em cache reutiliza o conjunto de escopos armazenado com o
  token de dispositivo pareado. Chamadores com `deviceToken` explícito / `scopes` explícitos mantêm o
  conjunto de escopos solicitado.
- Fora desse caminho de nova tentativa, a precedência de autenticação de conexão é token/senha
  compartilhado explícito primeiro, depois `deviceToken` explícito, depois token de dispositivo armazenado,
  depois token bootstrap.
- No caminho assíncrono da Control UI via Tailscale Serve, tentativas com falha para o mesmo
  `{scope, ip}` são serializadas antes que o limitador registre a falha. Duas novas tentativas
  simultâneas inválidas do mesmo cliente podem, portanto, exibir `retry later`
...
- `unauthorized` repetido após essa nova tentativa → divergência de token compartilhado/token de dispositivo; atualize a configuração do token e reaprove/revogue e gere um novo token de dispositivo, se necessário.
...
| `AUTH_TOKEN_MISMATCH`        | O token compartilhado não corresponde ao token de autenticação do gateway. | Se `canRetryWithDeviceToken=true`, permita uma tentativa confiável. Novas tentativas com token em cache reutilizam escopos aprovados armazenados; chamadores com `deviceToken` / `scopes` explícitos mantêm os escopos solicitados. Se ainda falhar, execute o [checklist de recuperação de divergência de token](/cli/devices#token-drift-recovery-checklist). |
| `AUTH_DEVICE_TOKEN_MISMATCH` | O token por dispositivo em cache está desatualizado ou revogado. | Gire/reaprove o token de dispositivo usando a [CLI de dispositivos](/cli/devices), depois reconecte.                                                                                                                                                                                   |
...
## Serviço de gateway não está em execução
...
## Avisos de sonda do gateway

Use isto quando `openclaw gateway probe` alcança algum destino, mas ainda exibe um bloco de aviso.
...
- `multiple reachable gateways detected` → mais de um alvo respondeu. Normalmente isso significa uma configuração intencional com vários gateways ou listeners antigos/duplicados.
...
- texto de aviso de SecretRef não resolvida em `gateway.auth.*` / `gateway.remote.*` → o material de autenticação não estava disponível nesse caminho de comando para o alvo com falha.
...
Se cron ou heartbeat não foi executado ou não foi entregue, verifique primeiro o estado do agendador e depois o destino da entrega.
...
- Status do histórico de execuções do job (`ok`, `skipped`, `error`).
...
- `heartbeat skipped` com `reason=dm-blocked` → o destino do heartbeat foi resolvido como um destino no estilo DM enquanto `agents.defaults.heartbeat.directPolicy` (ou a substituição por agente) está definido como `block`.
...
Se um nó estiver pareado, mas as ferramentas falharem, isole estado de primeiro plano, permissões e aprovação.
...
- Aprovações de execução e estado de allowlist.
...
- `browser.cdpUrl must be http(s) or ws(s)` → a URL de CDP configurada usa um esquema incompatível, como `file:` ou `ftp:`.
...
- `Browser attachOnly is enabled ... not reachable` ou `Browser attachOnly is enabled and CDP websocket ... is not reachable` → o perfil somente anexação não tem um alvo acessível, ou o endpoint HTTP respondeu, mas o WebSocket de CDP ainda não pôde ser aberto.
- `Playwright is not available in this gateway build; '<feature>' is unsupported.` → a instalação atual do gateway não inclui o pacote completo do Playwright; snapshots de ARIA e capturas de tela básicas da página ainda podem funcionar, mas navegação, snapshots com IA, capturas de elementos por seletor CSS e exportação para PDF continuam indisponíveis.
- `fullPage is not supported for element screenshots` → a solicitação de captura misturou `--full-page` com `--ref` ou `--element`.
- `element screenshots are not supported for existing-session profiles; use ref from snapshot.` → chamadas de captura para Chrome MCP / `existing-session` devem usar captura de página ou um `--ref` de snapshot, não `--element` em CSS.
- `existing-session file uploads do not support element selectors; use ref/inputRef.` → hooks de upload no Chrome MCP precisam de refs de snapshot, não de seletores CSS.
...
- substituições antigas de viewport / modo escuro / localidade / offline em perfis somente anexação ou CDP remoto → execute `openclaw browser stop --browser-profile <name>` para fechar a sessão de controle ativa e liberar o estado de emulação Playwright/CDP sem reiniciar o gateway inteiro.
...
A maior parte das quebras pós-atualização vem de deriva de configuração ou defaults mais rígidos agora sendo aplicados.
...
- Se `gateway.mode=remote`, chamadas da CLI podem estar mirando o remoto enquanto seu serviço local está funcionando normalmente.
...
### 2) Guardrails de bind e autenticação são mais rígidos
...
- Aprovações pendentes de dispositivo para dashboard/nós.
- Aprovações pendentes de pareamento de DM após mudanças de política ou identidade.

Assinaturas comuns:

- `device identity required` → autenticação de dispositivo não atendida.
