---
read_when:
    - Você precisa da semântica exata ou dos valores padrão da configuração em nível de campo
...
Cobre as principais superfícies de configuração do OpenClaw e aponta para fora quando um subsistema tem sua própria referência mais detalhada. Catálogos de comandos pertencentes a canais e plugins e controles avançados de memória/QMD ficam em suas próprias páginas, em vez desta.

Fonte de verdade no código:

- `openclaw config schema` imprime o JSON Schema ativo usado para validação e pela UI de Controle, com metadados de itens incluídos/plugins/canais mesclados quando disponíveis
- `config.schema.lookup` retorna um nó de schema com escopo por caminho para ferramentas de detalhamento
- `pnpm config:docs:check` / `pnpm config:docs:gen` validam o hash da linha de base da documentação de configuração contra a superfície atual do schema

Caminho de consulta do agente: use a ação de ferramenta `config.schema.lookup` do
`gateway` para obter documentação e restrições exatas em nível de campo antes de edições. Use
...
- páginas do canal/plugin responsável para superfícies de comandos específicas de canal
...
Chaves de configuração por canal foram movidas para uma página dedicada - consulte
...
canais incluídos (auth, controle de acesso, várias contas, restrição por menção).
...
- `agents.defaults.*` (área de trabalho, modelo, raciocínio, Heartbeat, memória, mídia, Skills, sandbox)
...
- `talk.*` (modo Talk)
  - `talk.consultThinkingLevel`: sobreposição do nível de raciocínio para a execução completa do agente OpenClaw por trás das consultas em tempo real do Talk da UI de Controle
  - `talk.consultFastMode`: sobreposição pontual de modo rápido para consultas em tempo real do Talk da UI de Controle
  - `talk.speechLocale`: id de localidade BCP 47 opcional para reconhecimento de fala do Talk em iOS/macOS
...
Política de ferramentas, alternâncias experimentais, configuração de ferramentas respaldadas por provedores e configuração de
...
- `models.providers.*.localService`: gerenciador de processos sob demanda opcional para
  servidores de modelos locais. O OpenClaw testa o endpoint de saúde configurado, inicia
  o `command` absoluto quando necessário, aguarda a prontidão e então envia a solicitação
  do modelo. Consulte [Serviços de modelos locais](/pt-BR/gateway/local-model-services).
- `models.pricing.enabled`: controla a inicialização de preços em segundo plano que
  começa depois que sidecars e canais chegam ao caminho de prontidão do Gateway. Quando `false`,
  o Gateway pula buscas do catálogo de preços do OpenRouter e LiteLLM; valores
...
consumidas pelo Pi embutido e outros adaptadores de runtime. Os comandos `openclaw mcp list`,
...
- `mcp.sessionIdleTtlMs`: TTL ocioso para runtimes MCP incluídos com escopo de sessão.
  Execuções embutidas pontuais solicitam limpeza ao fim da execução; este TTL é a salvaguarda para
  sessões de longa duração e chamadores futuros.
- Alterações em `mcp.*` aplicam-se a quente ao descartar runtimes MCP de sessão em cache.
  A próxima descoberta/uso de ferramentas os recria a partir da nova configuração, então entradas
  `mcp.servers` removidas são limpas imediatamente, em vez de aguardar o TTL ocioso.

Consulte [MCP](/pt-BR/cli/mcp#openclaw-as-an-mcp-client-registry) e
[backends da CLI](/pt-BR/gateway/cli-backends#bundle-mcp-overlays) para comportamento de runtime.
...
- `allowBundled`: lista de permissão opcional apenas para Skills incluídas (Skills gerenciadas/de área de trabalho não são afetadas).
- `load.extraDirs`: raízes extras de Skills compartilhadas (menor precedência).
- `load.allowSymlinkTargets`: raízes de destino reais confiáveis nas quais links simbólicos de Skills podem
...
- `install.nodeManager`: preferência de instalador Node para especificações `metadata.openclaw.install`
  (`npm` | `pnpm` | `yarn` | `bun`).
- `install.allowUploadedArchives`: permite que clientes Gateway `operator.admin`
  confiáveis instalem arquivos zip privados preparados por meio de `skills.upload.*`
...
- A descoberta aceita plugins nativos do OpenClaw mais bundles Codex compatíveis e bundles Claude, incluindo bundles Claude de layout padrão sem manifesto.
- **Alterações de configuração exigem uma reinicialização do Gateway.**
- `allow`: lista de permissão opcional (somente os plugins listados carregam). `deny` prevalece.
- `bundledDiscovery`: o padrão é `"allowlist"` para novas configurações, então um
  `plugins.allow` não vazio também restringe plugins de provedores incluídos, incluindo provedores de runtime
  de pesquisa na web. O Doctor grava `"compat"` para configurações legadas migradas de lista de permissão
...
- `plugins.entries.<id>.hooks.allowPromptInjection`: quando `false`, o núcleo bloqueia `before_prompt_build` e ignora campos que alteram o prompt do `before_agent_start` legado, preservando `modelOverride` e `providerOverride` legados. Aplica-se a hooks de plugins nativos e diretórios de hooks fornecidos por bundles compatíveis.
- `plugins.entries.<id>.hooks.allowConversationAccess`: quando `true`, plugins não incluídos confiáveis podem ler conteúdo bruto da conversa de hooks tipados como `llm_input`, `llm_output`, `before_model_resolve`, `before_agent_reply`, `before_agent_run`, `before_agent_finalize` e `agent_end`.
...
- `plugins.entries.<id>.subagent.allowedModels`: lista de permissão opcional de destinos canônicos `provider/model` para substituições de subagentes confiáveis. Use `"*"` apenas quando você intencionalmente quiser permitir qualquer modelo.
...
- `plugins.entries.<id>.llm.allowedModels`: lista de permissão opcional de destinos canônicos `provider/model` para substituições confiáveis de conclusão LLM de plugin. Use `"*"` apenas quando você intencionalmente quiser permitir qualquer modelo.
...
O plugin `codex` incluído é responsável pelas configurações do harness app-server nativo do Codex em
...
Ele não habilita plugins Codex para Pi, execuções normais do provedor OpenAI, vínculos de conversa ACP
ou qualquer harness não Codex.
...
- `plugins.entries.codex.config.codexPlugins.enabled`: habilita o suporte nativo a
  plugins/apps do Codex para o harness do Codex. Padrão: `false`.
- `plugins.entries.codex.config.codexPlugins.allow_destructive_actions`:
  política padrão de ação destrutiva para elicitações migradas de apps de plugin.
  Padrão: `false`.
...
- `plugins.entries.codex.config.codexPlugins.plugins.<key>.pluginName`: identidade
  estável do plugin Codex da migração, por exemplo `"google-calendar"`.
- `plugins.entries.codex.config.codexPlugins.plugins.<key>.allow_destructive_actions`:
  substituição de ação destrutiva por plugin. Quando omitida, o valor global de
  `allow_destructive_actions` é usado.

`codexPlugins.enabled` é a diretiva global de habilitação. Entradas explícitas de
plugin escritas pela migração são o conjunto durável de instalação e elegibilidade
para reparo. `plugins["*"]` não é compatível, não há opção `install`, e valores
locais de `marketplacePath` intencionalmente não são campos de configuração porque
são específicos do host.

As verificações de prontidão de `app/list` são armazenadas em cache por uma hora e atualizadas
de forma assíncrona quando ficam obsoletas. A configuração de app de thread do Codex é calculada no
estabelecimento da sessão do harness Codex, não a cada turno; use `/new`, `/reset` ou uma reinicialização
do Gateway após alterar a configuração de plugin nativo.

- `plugins.entries.firecrawl.config.webFetch`: configurações do provedor de web-fetch Firecrawl.
  - `apiKey`: chave de API do Firecrawl (aceita SecretRef). Recua para `plugins.entries.firecrawl.config.webSearch.apiKey`, `tools.web.fetch.firecrawl.apiKey` legado ou a variável de ambiente `FIRECRAWL_API_KEY`.
...
- Plugins habilitados do pacote Claude também podem contribuir padrões Pi incorporados de `settings.json`; o OpenClaw aplica isso como configurações de agente sanitizadas, não como patches brutos de configuração do OpenClaw.
...
- `tabCleanup` recupera abas rastreadas do agente principal após o tempo ocioso ou quando uma
...
- Se um serviço CDP gerenciado externamente estiver alcançável via loopback, defina
  `attachOnly: true` nesse perfil; caso contrário, o OpenClaw trata a porta de loopback como um
  perfil local gerenciado de navegador e pode relatar erros de propriedade de porta local.
...
- Perfis `openclaw` gerenciados localmente atribuem `cdpPort` e `cdpUrl` automaticamente; defina
...
- Perfis gerenciados localmente usam `browser.localLaunchTimeoutMs` para descoberta HTTP de CDP do Chrome
  após o início do processo e `browser.localCdpReadyTimeoutMs` para
  prontidão pós-inicialização do websocket CDP. Aumente-os em hosts mais lentos onde o Chrome
...
- `browser.executablePath` e `browser.profiles.<name>.executablePath` ambos
  aceitam `~` e `~/...` para o diretório home do seu SO antes da inicialização do Chromium.
...
- `seamColor`: cor de destaque para chrome de UI de app nativo (tint da bolha do Talk Mode etc.).
...
- `gateway.auth.mode: "none"`: modo explícito sem autenticação. Use apenas para configurações confiáveis de local loopback; isso é intencionalmente não oferecido pelos prompts de onboarding.
...
- `gateway.auth.rateLimit`: limitador opcional de falhas de autenticação. Aplica-se por IP de cliente e por escopo de autenticação (shared-secret e device-token são rastreados independentemente). Tentativas bloqueadas retornam `429` + `Retry-After`.
  - No caminho assíncrono da UI de Controle do Tailscale Serve, tentativas com falha para o mesmo `{scope, clientIp}` são serializadas antes da escrita da falha. Assim, tentativas inválidas concorrentes do mesmo cliente podem acionar o limitador na segunda solicitação, em vez de ambas passarem simultaneamente como simples incompatibilidades.
  - `gateway.auth.rateLimit.exemptLoopback` tem padrão `true`; defina como `false` quando você quiser intencionalmente que o tráfego de localhost também sofra limitação de taxa (para configurações de teste ou implantações de proxy estritas).
- Tentativas de autenticação WS originadas do navegador são sempre limitadas, com isenção de loopback desativada (defesa em profundidade contra força bruta baseada em navegador contra localhost).
- Em loopback, esses bloqueios originados do navegador são isolados por valor normalizado de `Origin`, então falhas repetidas de uma origem localhost não bloqueiam automaticamente uma origem diferente.
- `tailscale.mode`: `serve` (somente tailnet, bind loopback) ou `funnel` (público, exige autenticação).
- `tailscale.preserveFunnel`: quando `true` e `tailscale.mode = "serve"`, o OpenClaw verifica `tailscale funnel status` antes de reaplicar o Serve na inicialização e ignora isso se uma rota Funnel configurada externamente já cobrir a porta do Gateway. Padrão `false`.
- `controlUi.allowedOrigins`: allowlist explícita de origens de navegador para conexões WebSocket do Gateway. Obrigatória quando clientes de navegador são esperados a partir de origens não loopback.
...
- `OPENCLAW_ALLOW_INSECURE_PRIVATE_WS=1`: override de emergência no ambiente de processo do lado do cliente que permite `ws://` em texto claro para IPs confiáveis de rede privada; o padrão permanece somente loopback para texto claro. Não há equivalente em `openclaw.json`, e configurações de rede privada do navegador, como `browser.ssrfPolicy.dangerouslyAllowPrivateNetwork`, não afetam clientes WebSocket do Gateway.
...
- Registros apoiados por relay são delegados a uma identidade específica do Gateway. O app iOS pareado busca `gateway.identity.get`, inclui essa identidade no registro do relay e encaminha uma concessão de envio com escopo de registro para o Gateway. Outro Gateway não pode reutilizar esse registro armazenado.
- `OPENCLAW_APNS_RELAY_BASE_URL` / `OPENCLAW_APNS_RELAY_TIMEOUT_MS`: overrides temporários de ambiente para a configuração de relay acima.
...
- `gateway.handshakeTimeoutMs`: tempo limite de handshake WebSocket do Gateway antes da autenticação, em milissegundos. Padrão: `15000`. `OPENCLAW_HANDSHAKE_TIMEOUT_MS` tem precedência quando definido. Aumente isso em hosts carregados ou de baixa potência em que clientes locais conseguem conectar enquanto o aquecimento da inicialização ainda está se estabilizando.
...
- `channels.<provider>.healthMonitor.enabled`: exclusão por canal das reinicializações pelo monitor de integridade, mantendo o monitor global habilitado.
- `channels.<provider>.accounts.<accountId>.healthMonitor.enabled`: override por conta para canais com várias contas. Quando definido, ele tem precedência sobre o override no nível do canal.
...
- `gateway.nodes.pairing.autoApproveCidrs`: allowlist opcional de CIDR/IP para aprovar automaticamente o pareamento inicial de dispositivos de nó sem escopos solicitados. Fica desabilitada quando não definida. Isso não aprova automaticamente pareamento de operador/navegador/UI de Controle/WebChat, e não aprova automaticamente upgrades de função, escopo, metadados ou chave pública.
- `gateway.nodes.allowCommands` / `gateway.nodes.denyCommands`: moldagem global de allow/deny para comandos de nó declarados após pareamento e avaliação da allowlist da plataforma. Use `allowCommands` para optar por comandos de nó perigosos, como `camera.snap`, `camera.clip` e `screen.record`; `denyCommands` remove um comando mesmo se um padrão da plataforma ou permissão explícita o incluísse de outra forma. Depois que um nó alterar sua lista de comandos declarados, rejeite e aprove novamente esse pareamento de dispositivo para que o Gateway armazene o snapshot de comandos atualizado.
...
- Reforço de segurança para entrada por URL da Responses:
  - `gateway.http.endpoints.responses.maxUrlParts`
  - `gateway.http.endpoints.responses.files.urlAllowlist`
  - `gateway.http.endpoints.responses.images.urlAllowlist`
    Allowlists vazias são tratadas como não definidas; use `gateway.http.endpoints.responses.files.allowUrl=false` e/ou `gateway.http.endpoints.responses.images.allowUrl=false` para desabilitar busca por URL.
- Cabeçalho opcional de reforço da resposta:
...
- `autoGenerate`: gera automaticamente um par de certificado/chave autoassinado local quando arquivos explícitos não estão configurados; apenas para uso local/desenvolvimento.
...
- `caPath`: caminho opcional para o pacote de CA para verificação de cliente ou cadeias de confiança personalizadas.
...
- `deferralTimeoutMs`: tempo máximo opcional em ms para aguardar operações em andamento antes de forçar uma reinicialização ou recarregamento a quente de canal. Omita para usar a espera limitada padrão (`300000`); defina `0` para aguardar indefinidamente e registrar avisos periódicos de ainda pendente.
...
- Se um mapeamento ou predefinição usar um `sessionKey` modelado, defina `hooks.allowedSessionKeyPrefixes` e `hooks.allowRequestSessionKey=true`. Chaves de mapeamento estáticas não exigem essa adesão explícita.
...
- `allowedSessionKeyPrefixes`: lista de permissões opcional de prefixos para valores explícitos de `sessionKey` (solicitação + mapeamento), por exemplo, `["hook:"]`. Torna-se obrigatória quando qualquer mapeamento ou predefinição usa um `sessionKey` modelado.
...
- Se você precisar de `hooks.allowRequestSessionKey: false`, substitua a predefinição por um `sessionKey` estático em vez do padrão modelado.
...
- Vinculações não loopback: rotas canvas exigem autenticação do Gateway (token/senha/proxy confiável), igual às outras superfícies HTTP do Gateway.
...
- `full`: inclui `cliPath` + `sshPort`; o anúncio multicast de LAN ainda exige que o Plugin `bonjour` integrado esteja habilitado.
- `off`: suprime o anúncio multicast de LAN sem alterar a habilitação do Plugin.
- O Plugin `bonjour` integrado inicia automaticamente em hosts macOS e é opcional em implantações de Gateway no Linux, Windows e em contêineres.
...
- Referências a segredos são resolvidas no momento da ativação para um snapshot em memória; depois, os caminhos de requisição leem apenas o snapshot.
- A filtragem de superfícies ativas se aplica durante a ativação: referências não resolvidas em superfícies habilitadas fazem a inicialização/recarga falhar, enquanto superfícies inativas são ignoradas com diagnósticos.
...
- `auth-profiles.json` aceita referências no nível de valor (`keyRef` para `api_key`, `tokenRef` para `token`) para modos de credenciais estáticas.
...
- `rateLimitedProfileRotations`: máximo de rotações de perfis de autenticação do mesmo provedor para erros de limite de taxa antes de alternar para fallback de modelo (padrão: `1`). Esse bucket de limite de taxa inclui texto em formato de provedor, como `Too many concurrent requests`, `ThrottlingException`, `concurrency limit reached`, `workers_ai ... quota limit exceeded` e `resource exhausted`.
...
- `stuckSessionAbortMs`: limite de idade sem progresso em ms antes que trabalho ativo travado qualificado possa ser drenado por abortamento para recuperação. Quando não definido, o OpenClaw usa a janela incorporada estendida mais segura de pelo menos 10 minutos e 5x `stuckSessionWarnMs`.
...
- `otel.captureContent`: captura opcional de conteúdo bruto para atributos de span OTEL. O padrão é desativado. Booleano `true` captura conteúdo de mensagens/ferramentas que não seja do sistema; a forma de objeto permite habilitar explicitamente `inputMessages`, `outputMessages`, `toolInputs`, `toolOutputs` e `systemPrompt`.
- `OTEL_SEMCONV_STABILITY_OPT_IN=gen_ai_latest_experimental`: alternância de ambiente para os atributos experimentais mais recentes de provedor de span GenAI. Por padrão, os spans mantêm o atributo legado `gen_ai.system` para compatibilidade; métricas GenAI usam atributos semânticos limitados.
- `OPENCLAW_OTEL_PRELOADED=1`: alternância de ambiente para hosts que já registraram um SDK OpenTelemetry global. Então o OpenClaw pula a inicialização/desligamento do SDK pertencente ao Plugin, mantendo os listeners de diagnóstico ativos.
...
- `cacheTrace.includeMessages` / `includePrompt` / `includeSystem`: controlam o que é incluído na saída de trace de cache (todos por padrão: `true`).
...
- `auto.stableJitterHours`: janela adicional em horas para espalhar a distribuição no canal estável (padrão: `12`; máximo: `168`).
...
- `stream.maxChunkChars`: tamanho máximo de chunk antes de dividir a projeção de bloco transmitida.
...
- `runtime.ttlMinutes`: TTL ocioso em minutos para workers de sessão ACP antes da limpeza qualificada.
...
- `sessionRetention`: por quanto tempo manter sessões concluídas de execuções Cron isoladas antes de podá-las de `sessions.json`. Também controla a limpeza de transcrições Cron arquivadas que foram excluídas. Padrão: `24h`; defina como `false` para desativar.
...
- `webhook`: URL de Webhook legada e obsoleta de fallback (http/https), usada apenas para trabalhos armazenados que ainda têm `notify: true`.
...
- `maxAttempts`: máximo de novas tentativas para trabalhos pontuais em erros transitórios (padrão: `3`; intervalo: `0`-`10`).
...
Aplica-se apenas a trabalhos Cron pontuais. Trabalhos recorrentes usam tratamento de falha separado.
...
- Quando nenhum destino de falha global nem por trabalho está definido, trabalhos que já entregam via `announce` usam como fallback esse destino principal de anúncio em caso de falha.
