---
read_when:
    - Você precisa da semântica ou dos valores padrão exatos de configuração no nível do campo
...
Abrange as principais superfícies de configuração do OpenClaw e aponta para outras páginas quando um subsistema tem sua própria referência mais detalhada. Catálogos de comandos pertencentes a canais e plugins e ajustes avançados de memória/QMD ficam em suas próprias páginas, em vez de nesta.

Verdade no código:

- `openclaw config schema` imprime o JSON Schema ativo usado para validação e Control UI, com metadados incluídos/de plugin/de canal mesclados quando disponíveis
- `config.schema.lookup` retorna um nó de schema com escopo de caminho para ferramentas de detalhamento
- `pnpm config:docs:check` / `pnpm config:docs:gen` validam o hash de baseline da documentação de configuração contra a superfície atual do schema
...
O formato de configuração é **JSON5** (comentários + vírgulas finais são permitidos). Todos os campos são opcionais — o OpenClaw usa padrões seguros quando omitidos.
...
canais incluídos (autenticação, controle de acesso, múltiplas contas, controle de menções).
...
- `mcp.sessionIdleTtlMs`: TTL ocioso para runtimes MCP incluídos com escopo de sessão.
  Execuções incorporadas de uso único solicitam limpeza ao fim da execução; este TTL é o respaldo para
...
- `allowBundled`: lista de permissões opcional apenas para Skills incluídas (Skills gerenciadas/de workspace não são afetadas).
...
- `entries.<skillKey>.apiKey`: conveniência para Skills que declaram uma variável de ambiente principal (string em texto puro ou objeto SecretRef).
...
- `plugins.entries.<id>.hooks.allowPromptInjection`: quando `false`, o núcleo bloqueia `before_prompt_build` e ignora campos que alteram prompts de `before_agent_start` legado, preservando `modelOverride` e `providerOverride` legados. Aplica-se a hooks de plugins nativos e diretórios de hooks fornecidos por bundle com suporte.
- `plugins.entries.<id>.hooks.allowConversationAccess`: quando `true`, plugins não incluídos e confiáveis podem ler o conteúdo bruto da conversa a partir de hooks tipados como `llm_input`, `llm_output`, `before_agent_finalize` e `agent_end`.
- `plugins.entries.<id>.subagent.allowModelOverride`: confie explicitamente neste plugin para solicitar substituições de `provider` e `model` por execução para execuções de subagentes em segundo plano.
...
- `plugins.entries.firecrawl.config.webFetch`: configurações do provedor de busca web Firecrawl.
  - `apiKey`: chave de API Firecrawl (aceita SecretRef). Recai para `plugins.entries.firecrawl.config.webSearch.apiKey`, `tools.web.fetch.firecrawl.apiKey` legado ou variável de ambiente `FIRECRAWL_API_KEY`.
  - `baseUrl`: URL base da API Firecrawl (padrão: `https://api.firecrawl.dev`; substituições auto-hospedadas devem mirar endpoints privados/internos).
...
- Plugins de bundle Claude habilitados também podem contribuir padrões incorporados do Pi a partir de `settings.json`; o OpenClaw aplica esses valores como configurações de agente sanitizadas, não como patches brutos de configuração do OpenClaw.
...
`commitments` controla a memória de acompanhamento inferida: o OpenClaw pode detectar check-ins a partir de turnos de conversa e entregá-los por meio de execuções de Heartbeat.
...
- `profiles.*.cdpUrl` aceita `http://`, `https://`, `ws://` e `wss://`.
  ...
  quando seu provedor fornecer uma URL direta do WebSocket do DevTools.
- `remoteCdpTimeoutMs` e `remoteCdpHandshakeTimeoutMs` se aplicam a solicitações remotas e
  de alcance CDP `attachOnly`, além de solicitações de abertura de abas. Perfis gerenciados de loopback
...
- Se um serviço CDP gerenciado externamente estiver acessível por loopback, defina
  ...
  perfil local de navegador gerenciado e pode relatar erros de posse de porta local.
...
- Perfis `existing-session` mantêm os limites atuais de rota do Chrome MCP:
  ações orientadas por snapshot/ref em vez de segmentação por seletor CSS, ganchos
  de upload de um arquivo, sem substituições de tempo limite de diálogo, sem
...
- `seamColor`: cor de destaque para o chrome da UI do app nativo (matiz do balão do Modo de Conversa etc.).
- `assistant`: substituição da identidade da UI de controle. Recua para a identidade do agente ativo.
...
- `mode`: `local` (executa Gateway) ou `remote` (conecta a um Gateway remoto). Gateway se recusa a iniciar a menos que seja `local`.
...
- `gateway.auth.mode: "trusted-proxy"`: delega a autenticação de navegador/usuário a um proxy reverso com reconhecimento de identidade e confia nos cabeçalhos de identidade de `gateway.trustedProxies` (consulte [Autenticação por proxy confiável](/pt-BR/gateway/trusted-proxy-auth)). Este modo espera uma origem de proxy **não loopback** por padrão; proxies reversos loopback no mesmo host exigem `gateway.auth.trustedProxy.allowLoopback = true` explícito. Chamadores internos no mesmo host podem usar `gateway.auth.password` como fallback direto local; `gateway.auth.token` continua mutuamente exclusivo com o modo trusted-proxy.
- `gateway.auth.allowTailscale`: quando `true`, cabeçalhos de identidade do Tailscale Serve podem satisfazer a autenticação da Control UI/WebSocket (verificada via `tailscale whois`). Endpoints de API HTTP **não** usam essa autenticação de cabeçalho do Tailscale; eles seguem o modo normal de autenticação HTTP do Gateway. Esse fluxo sem token pressupõe que o host do Gateway seja confiável. O padrão é `true` quando `tailscale.mode = "serve"`.
- `gateway.auth.rateLimit`: limitador opcional de falhas de autenticação. Aplica-se por IP do cliente e por escopo de autenticação (shared-secret e device-token são rastreados independentemente). Tentativas bloqueadas retornam `429` + `Retry-After`.
  ...
  - `gateway.auth.rateLimit.exemptLoopback` tem padrão `true`; defina como `false` quando você intencionalmente quiser que o tráfego localhost também sofra limitação de taxa (para configurações de teste ou implantações de proxy rígidas).
- Tentativas de autenticação WS com origem em navegador sempre sofrem throttling com isenção de loopback desativada (defesa em profundidade contra força bruta em localhost baseada em navegador).
...
- `controlUi.allowedOrigins`: lista explícita de origens de navegador permitidas para conexões WebSocket do Gateway. Obrigatória quando clientes de navegador são esperados de origens que não sejam loopback.
...
- `OPENCLAW_APNS_RELAY_ALLOW_HTTP=true`: saída de escape somente para desenvolvimento para URLs de relay HTTP em loopback. URLs de relay de produção devem permanecer em HTTPS.
- `gateway.handshakeTimeoutMs`: tempo limite de handshake WebSocket pré-autenticação do Gateway em milissegundos. Padrão: `15000`. `OPENCLAW_HANDSHAKE_TIMEOUT_MS` tem precedência quando definido. Aumente isso em hosts carregados ou de baixa potência nos quais clientes locais podem se conectar enquanto o aquecimento da inicialização ainda está se estabilizando.
- `gateway.channelHealthCheckMinutes`: intervalo do monitor de integridade de canal em minutos. Defina `0` para desativar reinicializações do monitor de integridade globalmente. Padrão: `5`.
...
- `channels.<provider>.accounts.<accountId>.healthMonitor.enabled`: substituição por conta para canais de várias contas. Quando definido, tem precedência sobre a substituição em nível de canal.
- Caminhos de chamada do Gateway local só podem usar `gateway.remote.*` como fallback quando `gateway.auth.*` não está definido.
- Se `gateway.auth.token` / `gateway.auth.password` for configurado explicitamente via SecretRef e não for resolvido, a resolução falha de modo fechado (sem mascaramento por fallback remoto).
...
- `gateway.nodes.pairing.autoApproveCidrs`: allowlist opcional de CIDR/IP para aprovar automaticamente o primeiro pareamento de dispositivo Node sem escopos solicitados. Fica desativado quando não definido. Isso não aprova automaticamente pareamento de operador/navegador/Control UI/WebChat e não aprova automaticamente upgrades de função, escopo, metadados ou chave pública.
- `gateway.nodes.allowCommands` / `gateway.nodes.denyCommands`: modelagem global de permissão/negação para comandos Node declarados após pareamento e avaliação da allowlist de plataforma. Use `allowCommands` para aceitar comandos Node perigosos, como `camera.snap`, `camera.clip` e `screen.record`; `denyCommands` remove um comando mesmo que um padrão de plataforma ou permissão explícita o incluísse. Depois que um Node alterar sua lista de comandos declarados, rejeite e aprove novamente esse pareamento de dispositivo para que o Gateway armazene o snapshot atualizado dos comandos.
...
- Reforço de segurança de entrada de URL em Responses:
  ...
  - `gateway.http.endpoints.responses.images.urlAllowlist`
    ...
    e/ou `gateway.http.endpoints.responses.images.allowUrl=false` para desativar busca de URL.
- Cabeçalho opcional de reforço de segurança de resposta:
...
- `deferralTimeoutMs`: tempo máximo opcional em ms para aguardar operações em andamento antes de forçar uma reinicialização. Omita para usar a espera limitada padrão (`300000`); defina `0` para esperar indefinidamente e registrar avisos periódicos de ainda pendente.
...
- Se um mapeamento ou preset usar uma `sessionKey` com template, defina `hooks.allowedSessionKeyPrefixes` e `hooks.allowRequestSessionKey=true`. Chaves de mapeamento estáticas não exigem essa adesão.

**Pontos de extremidade:**

- `POST /hooks/wake` → `{ text, mode?: "now"|"next-heartbeat" }`
- `POST /hooks/agent` → `{ message, name?, agentId?, sessionKey?, wakeMode?, deliver?, channel?, to?, model?, thinking?, timeoutSeconds? }`
  - A `sessionKey` do payload da solicitação é aceita somente quando `hooks.allowRequestSessionKey=true` (padrão: `false`).
...
- `defaultSessionKey`: chave de sessão fixa opcional para execuções de agente por hook sem `sessionKey` explícita.
...
- `allowedSessionKeyPrefixes`: lista opcional de prefixos permitidos para valores explícitos de `sessionKey` (solicitação + mapeamento), por exemplo `["hook:"]`. Ela passa a ser obrigatória quando qualquer mapeamento ou preset usa uma `sessionKey` com template.
- `deliver: true` envia a resposta final para um canal; `channel` tem `last` como padrão.
...
- Se você precisar de `hooks.allowRequestSessionKey: false`, substitua o preset por uma `sessionKey` estática em vez do padrão com template.
...
- Serve HTML/CSS/JS editável por agentes e A2UI via HTTP na porta do Gateway:
  - `http://<gateway-host>:<gateway.port>/__openclaw__/canvas/`
  - `http://<gateway-host>:<gateway.port>/__openclaw__/a2ui/`
- Somente local: mantenha `gateway.bind: "loopback"` (padrão).
- Vinculações que não são loopback: rotas de canvas exigem autenticação do Gateway (token/senha/proxy confiável), como outras superfícies HTTP do Gateway.
- WebViews do Node normalmente não enviam cabeçalhos de autenticação; depois que um nó é pareado e conectado, o Gateway anuncia URLs de capacidade com escopo de nó para acesso ao canvas/A2UI.
...
- O nome de host usa o hostname do sistema como padrão quando ele é um rótulo DNS válido, recorrendo a `openclaw`. Substitua com `OPENCLAW_MDNS_HOSTNAME`.
...
- Alvos de `secrets apply` são caminhos de credenciais compatíveis em `openclaw.json`.
...
- Caminhos de provedores file e exec falham em modo fechado quando a verificação de ACL do Windows não está disponível. Defina `allowInsecurePath: true` somente para caminhos confiáveis que não podem ser verificados.
- O provedor `exec` exige um caminho absoluto de `command` e usa payloads de protocolo em stdin/stdout.
- Por padrão, caminhos de comando por symlink são rejeitados. Defina `allowSymlinkCommand: true` para permitir caminhos por symlink ao validar o caminho de destino resolvido.
- Se `trustedDirs` estiver configurado, a verificação de diretório confiável se aplica ao caminho de destino resolvido.
- O ambiente filho de `exec` é mínimo por padrão; passe as variáveis exigidas explicitamente com `passEnv`.
...
- `auth-profiles.json` é compatível com referências em nível de valor (`keyRef` para `api_key`, `tokenRef` para `token`) para modos de credenciais estáticas.
...
- Perfis no modo OAuth (`auth.profiles.<id>.mode = "oauth"`) não são compatíveis com credenciais de perfil de autenticação baseadas em SecretRef.
...
- `flags`: array de strings de sinalizadores que habilitam saída de log direcionada (suporta curingas como `"telegram.*"` ou `"*"`).
- `stuckSessionWarnMs`: limite de idade sem progresso em ms para classificar sessões de processamento de longa duração como `session.long_running`, `session.stalled` ou `session.stuck`. Resposta, ferramenta, status, bloco e progresso de ACP reiniciam o temporizador; diagnósticos `session.stuck` repetidos aplicam backoff enquanto não houver mudanças.
...
- `otel.captureContent`: captura de conteúdo bruto opt-in para atributos de span OTEL. Desativado por padrão. Booleano `true` captura conteúdo de mensagens/ferramentas que não seja do sistema; a forma de objeto permite habilitar explicitamente `inputMessages`, `outputMessages`, `toolInputs`, `toolOutputs` e `systemPrompt`.
...
- `OPENCLAW_OTEL_PRELOADED=1`: alternância de ambiente para hosts que já registraram um SDK OpenTelemetry global. Então o OpenClaw ignora a inicialização/desligamento do SDK pertencente ao Plugin, mantendo ouvintes de diagnóstico ativos.
...
- `auto.stableJitterHours`: janela extra de distribuição de rollout no canal stable em horas (padrão: `12`; máx.: `168`).
- `auto.betaCheckIntervalHours`: frequência das verificações do canal beta em horas (padrão: `1`; máx.: `24`).
...
- `enabled`: gate global do recurso ACP (padrão: `true`; defina `false` para ocultar affordances de despacho e spawn de ACP).
...
- `stream.maxChunkChars`: tamanho máximo de chunk antes de dividir a projeção de bloco transmitida por stream.
...
As compilações atuais não incluem mais a bridge TCP. Nodes se conectam pelo WebSocket do Gateway. As chaves `bridge.*` não fazem mais parte do esquema de configuração (a validação falha até serem removidas; `openclaw doctor --fix` pode remover chaves desconhecidas).
...
- `sessionRetention`: por quanto tempo manter sessões concluídas de execução isolada de Cron antes de removê-las de `sessions.json`. Também controla a limpeza de transcrições arquivadas de Cron excluídas. Padrão: `24h`; defina `false` para desativar.
- `runLog.maxBytes`: tamanho máximo por arquivo de log de execução (`cron/runs/<jobId>.jsonl`) antes da poda. Padrão: `2_000_000` bytes.
- `runLog.keepLines`: linhas mais recentes mantidas quando a poda de log de execução é acionada. Padrão: `2000`.
...
- `includeSkipped`: contabiliza execuções ignoradas consecutivas para o limite do alerta (padrão: `false`). Execuções ignoradas são rastreadas separadamente e não afetam o backoff de erros de execução.
...
- Quando nem o destino de falha global nem o por tarefa é definido, tarefas que já entregam via `announce` usam esse destino principal de announce como fallback em caso de falha.
- `delivery.failureDestination` só tem suporte para tarefas `sessionTarget="isolated"`, a menos que o `delivery.mode` principal da tarefa seja `"webhook"`.
...
- Escritas pertencentes ao OpenClaw que alteram apenas uma seção de nível superior respaldada por um include de arquivo único são gravadas nesse arquivo incluído. Por exemplo, `plugins install` atualiza `plugins: { $include: "./plugins.json5" }` em `plugins.json5` e deixa `openclaw.json` intacto.
