---
read_when:
    - Você precisa de semântica ou padrões exatos de configuração em nível de campo
...
- `openclaw config schema` imprime o JSON Schema ativo usado para validação e Control UI, com metadados de pacotes incluídos/plugins/canais mesclados quando disponíveis
- `config.schema.lookup` retorna um nó de schema com escopo por caminho para ferramentas de análise detalhada
...
Caminho de consulta do agente: use a ação de ferramenta `gateway` `config.schema.lookup` para obter
...
- [Comandos slash](/pt-BR/tools/slash-commands) para o catálogo atual de comandos integrados + incluídos
...
canais incluídos (autenticação, controle de acesso, várias contas, controle de menções).
...
- `models.pricing.enabled`: controla a inicialização de preços em segundo plano que
  começa depois que sidecars e canais alcançam o caminho de prontidão do Gateway. Quando `false`,
...
- `mcp.sessionIdleTtlMs`: TTL de inatividade para runtimes MCP incluídos com escopo de sessão.
  Execuções incorporadas únicas solicitam limpeza ao fim da execução; este TTL é a garantia para
...
- Alterações em `mcp.*` são aplicadas a quente ao descartar runtimes MCP de sessão em cache.
  ...
  `mcp.servers` removidas são coletadas imediatamente em vez de aguardar o TTL de inatividade.
...
- `allowBundled`: lista opcional de permissões apenas para Skills incluídas (Skills gerenciadas/do workspace não são afetadas).
...
- `install.nodeManager`: preferência de instalador Node para especificações `metadata.openclaw.install`
  (`npm` | `pnpm` | `yarn` | `bun`).
- `entries.<skillKey>.enabled: false` desativa uma Skill mesmo que incluída/instalada.
...
- A descoberta aceita plugins nativos do OpenClaw mais pacotes compatíveis do Codex e pacotes do Claude, incluindo pacotes sem manifesto com layout padrão do Claude.
- **Alterações de configuração exigem reinício do Gateway.**
- `allow`: lista opcional de permissões (somente plugins listados carregam). `deny` vence.
- `bundledDiscovery`: o padrão é `"allowlist"` para novas configurações, então um
  `plugins.allow` não vazio também controla plugins de provedores incluídos, incluindo provedores de runtime
  de pesquisa na web. O Doctor grava `"compat"` para configurações migradas de lista de permissões legada
  para preservar o comportamento existente de provedores incluídos até você optar pelo novo modo.
...
- `plugins.entries.<id>.hooks.allowPromptInjection`: quando `false`, o núcleo bloqueia `before_prompt_build` e ignora campos que modificam prompts de `before_agent_start` legado, preservando `modelOverride` e `providerOverride` legados. Aplica-se a hooks de plugins nativos e diretórios de hooks fornecidos por pacotes compatíveis.
- `plugins.entries.<id>.hooks.allowConversationAccess`: quando `true`, plugins confiáveis não incluídos podem ler conteúdo bruto de conversas a partir de hooks tipados como `llm_input`, `llm_output`, `before_model_resolve`, `before_agent_reply`, `before_agent_run`, `before_agent_finalize` e `agent_end`.
...
- `plugins.entries.<id>.subagent.allowedModels`: lista opcional de permissões de destinos canônicos `provider/model` para substituições confiáveis de subagentes. Use `"*"` somente quando você pretende permitir qualquer modelo.
...
- `plugins.entries.firecrawl.config.webFetch`: configurações do provedor de busca web do Firecrawl.
  - `apiKey`: chave de API do Firecrawl (aceita SecretRef). Recorre a `plugins.entries.firecrawl.config.webSearch.apiKey`, `tools.web.fetch.firecrawl.apiKey` legado ou à variável de ambiente `FIRECRAWL_API_KEY`.
...
- `plugins.entries.memory-core.config.dreaming`: configurações de dreaming de memória. Consulte [Dreaming](/pt-BR/concepts/dreaming) para fases e limites.
  - `enabled`: chave principal de dreaming (padrão `false`).
...
- Plugins de pacotes Claude habilitados também podem contribuir padrões incorporados do Pi a partir de `settings.json`; o OpenClaw aplica isso como configurações sanitizadas de agente, não como patches brutos de configuração do OpenClaw.
...
`commitments` controla memória de acompanhamento inferida: o OpenClaw pode detectar check-ins a partir de turnos de conversa e entregá-los por meio de execuções de Heartbeat.
...
- `commitments.maxPerDay`: máximo de compromissos de acompanhamento inferidos entregues por sessão de agente em um dia móvel. Padrão: `3`.
...
- `tabCleanup` recupera abas rastreadas do agente principal após o tempo ocioso ou quando uma sessão excede seu limite. Defina `idleMinutes: 0` ou `maxTabsPerSession: 0` para desativar esses modos de limpeza individuais.
...
- No modo estrito, os endpoints de perfil CDP remoto (`profiles.*.cdpUrl`) estão sujeitos ao mesmo bloqueio de rede privada durante verificações de alcance/descoberta.
...
- Perfis remotos são apenas de anexação (iniciar/parar/redefinir desativados).
...
- Se um serviço CDP gerenciado externamente estiver acessível por loopback, defina
  ...
  perfil de navegador local gerenciado e pode relatar erros de propriedade da porta local.
...
- Perfis `existing-session` mantêm os limites atuais da rota Chrome MCP:
  ações orientadas por snapshot/ref em vez de seleção por CSS selector, hooks de upload
...
- `browser.executablePath` e `browser.profiles.<name>.executablePath` aceitam
  `~` e `~/...` para o diretório home do seu sistema operacional antes da inicialização do Chromium.
...
- `assistant`: substituição da identidade da UI de Controle. Usa como fallback a identidade do agente ativo.
...
- **Nota sobre Docker**: o bind `loopback` padrão escuta em `127.0.0.1` dentro do contêiner. Com rede bridge do Docker (`-p 18789:18789`), o tráfego chega em `eth0`, portanto o Gateway fica inacessível. Use `--network host` ou defina `bind: "lan"` (ou `bind: "custom"` com `customBindHost: "0.0.0.0"`) para escutar em todas as interfaces.
- **Autenticação**: obrigatória por padrão. Binds que não são loopback exigem autenticação do Gateway. Na prática, isso significa um token/senha compartilhado ou um proxy reverso com reconhecimento de identidade usando `gateway.auth.mode: "trusted-proxy"`. O assistente de onboarding gera um token por padrão.
...
- `gateway.auth.mode: "none"`: modo explícito sem autenticação. Use somente para configurações confiáveis de local loopback; isto intencionalmente não é oferecido pelos prompts de onboarding.
- `gateway.auth.mode: "trusted-proxy"`: delega a autenticação de navegador/usuário a um proxy reverso com reconhecimento de identidade e confia em cabeçalhos de identidade de `gateway.trustedProxies` (veja [Autenticação por Proxy Confiável](/pt-BR/gateway/trusted-proxy-auth)). Este modo espera uma origem de proxy **não loopback** por padrão; proxies reversos loopback no mesmo host exigem `gateway.auth.trustedProxy.allowLoopback = true` explícito. Chamadores internos no mesmo host podem usar `gateway.auth.password` como fallback direto local; `gateway.auth.token` permanece mutuamente exclusivo com o modo trusted-proxy.
...
- `gateway.auth.rateLimit`: limitador opcional de falhas de autenticação. Aplica-se por IP de cliente e por escopo de autenticação (shared-secret e device-token são rastreados independentemente). Tentativas bloqueadas retornam `429` + `Retry-After`.
  ...
  - `gateway.auth.rateLimit.exemptLoopback` tem padrão `true`; defina `false` quando você intencionalmente quiser que o tráfego de localhost também seja limitado por taxa (para configurações de teste ou implantações estritas com proxy).
- Tentativas de autenticação WS com origem no navegador sempre sofrem throttling com a isenção de loopback desativada (defesa em profundidade contra força bruta de localhost baseada em navegador).
- Em loopback, esses bloqueios com origem no navegador são isolados por valor
...
- `controlUi.dangerouslyAllowHostHeaderOriginFallback`: modo perigoso que habilita fallback de origem pelo cabeçalho Host para implantações que intencionalmente dependem de política de origem baseada no cabeçalho Host.
...
- `OPENCLAW_ALLOW_INSECURE_PRIVATE_WS=1`: substituição emergencial no ambiente de processo
  do lado do cliente que permite `ws://` em texto claro para IPs confiáveis de
  rede privada; o padrão permanece somente loopback para texto claro. Não há equivalente em `openclaw.json`,
...
- `gateway.push.apns.relay.baseUrl`: URL HTTPS base para o relé APNs externo usado por builds oficiais/TestFlight do iOS depois que publicam registros respaldados por relé no Gateway. Essa URL deve corresponder à URL do relé compilada no build do iOS.
- `gateway.push.apns.relay.timeoutMs`: tempo limite de envio do Gateway para o relé em milissegundos. Padrão: `10000`.
- Registros respaldados por relé são delegados a uma identidade específica do Gateway. O app iOS pareado busca `gateway.identity.get`, inclui essa identidade no registro do relé e encaminha ao Gateway uma concessão de envio com escopo de registro. Outro Gateway não pode reutilizar esse registro armazenado.
- `OPENCLAW_APNS_RELAY_BASE_URL` / `OPENCLAW_APNS_RELAY_TIMEOUT_MS`: substituições temporárias de env para a configuração de relé acima.
...
- `gateway.handshakeTimeoutMs`: tempo limite do handshake WebSocket pré-autenticação do Gateway em milissegundos. Padrão: `15000`. `OPENCLAW_HANDSHAKE_TIMEOUT_MS` tem precedência quando definido. Aumente isso em hosts carregados ou de baixa potência nos quais clientes locais conseguem conectar enquanto o aquecimento de inicialização ainda está estabilizando.
- `gateway.channelHealthCheckMinutes`: intervalo do monitor de integridade de canais em minutos. Defina `0` para desativar reinicializações pelo monitor de integridade globalmente. Padrão: `5`.
...
- `gateway.channelMaxRestartsPerHour`: máximo de reinicializações pelo monitor de integridade por canal/conta em uma hora móvel. Padrão: `10`.
- `channels.<provider>.healthMonitor.enabled`: opção de exclusão por canal para reinicializações pelo monitor de integridade enquanto mantém o monitor global habilitado.
- `channels.<provider>.accounts.<accountId>.healthMonitor.enabled`: substituição por conta para canais multi-conta. Quando definida, tem precedência sobre a substituição em nível de canal.
...
- `gateway.nodes.allowCommands` / `gateway.nodes.denyCommands`: modelagem global de allow/deny para comandos declarados de Node após pareamento e avaliação da allowlist da plataforma. Use `allowCommands` para optar por comandos perigosos de Node, como `camera.snap`, `camera.clip` e `screen.record`; `denyCommands` remove um comando mesmo que um padrão de plataforma ou allow explícito o incluísse de outra forma. Depois que um Node altera sua lista de comandos declarados, rejeite e aprove novamente o pareamento desse dispositivo para que o Gateway armazene o snapshot de comandos atualizado.
...
- Fortalecimento de entrada de URL do Responses:
...
- Serve HTML/CSS/JS editável por agentes e A2UI por HTTP na porta do Gateway:
  - `http://<gateway-host>:<gateway.port>/__openclaw__/canvas/`
  - `http://<gateway-host>:<gateway.port>/__openclaw__/a2ui/`
- Somente local: mantenha `gateway.bind: "loopback"` (padrão).
- Binds que não são de loopback: rotas de canvas exigem autenticação do Gateway (token/senha/proxy confiável), como outras superfícies HTTP do Gateway.
...
- URLs de capacidade são vinculadas à sessão WS do Node ativo e expiram rapidamente. Fallback baseado em IP não é usado.
...
- O nome do host usa por padrão o nome de host do sistema quando ele é um rótulo DNS válido, com fallback para `openclaw`. Substitua com `OPENCLAW_MDNS_HOSTNAME`.
...
- Variáveis de ambiente embutidas só são aplicadas se a variável de ambiente do processo não tiver a chave.
...
- Refs de segredo são resolvidas no momento da ativação em um snapshot em memória; depois, os caminhos de requisição leem somente o snapshot.
...
- Mapas simples legados de `auth-profiles.json`, como `{ "provider": { "apiKey": "..." } }`, não são um formato em tempo de execução; `openclaw doctor --fix` os reescreve para perfis canônicos de chave de API `provider:default` com um backup `.legacy-flat.*.bak`.
...
- `billingBackoffHours`: recuo base em horas quando um perfil falha devido a erros reais de
  cobrança/crédito insuficiente (padrão: `5`). Texto explícito de cobrança pode
  ainda cair aqui mesmo em respostas `401`/`403`, mas correspondedores de texto
  específicos do provedor permanecem restritos ao provedor que os possui (por exemplo, OpenRouter
...
- `rateLimitedProfileRotations`: máximo de rotações de perfis de autenticação do mesmo provedor para erros de limite de taxa antes de alternar para fallback de modelo (padrão: `1`). Esse grupo de limite de taxa inclui texto no formato do provedor, como `Too many concurrent requests`, `ThrottlingException`, `concurrency limit reached`, `workers_ai ... quota limit exceeded` e `resource exhausted`.
...
- `enabled`: controle mestre para saída de instrumentação (padrão: `true`).
- `flags`: matriz de strings de sinalizadores que ativam saída de log direcionada (compatível com curingas como `"telegram.*"` ou `"*"`).
- `stuckSessionWarnMs`: limite de idade sem progresso em ms para classificar sessões de processamento longas como `session.long_running`, `session.stalled` ou `session.stuck`. Respostas, ferramentas, status, blocos e progresso ACP reiniciam o temporizador; diagnósticos `session.stuck` repetidos recuam enquanto não houver alterações.
- `stuckSessionAbortMs`: limite de idade sem progresso em ms antes que trabalho ativo paralisado elegível possa ser drenado por aborto para recuperação. Quando não definido, o OpenClaw usa a janela incorporada estendida mais segura de pelo menos 10 minutos e 5x `stuckSessionWarnMs`.
...
- `otel.tracesEndpoint` / `otel.metricsEndpoint` / `otel.logsEndpoint`: endpoints OTLP opcionais específicos por sinal. Quando definidos, substituem `otel.endpoint` apenas para aquele sinal.
...
- `otel.captureContent`: captura opcional de conteúdo bruto para atributos de spans OTEL. Desativado por padrão. O booleano `true` captura conteúdo de mensagens/ferramentas que não seja de sistema; a forma de objeto permite ativar explicitamente `inputMessages`, `outputMessages`, `toolInputs`, `toolOutputs` e `systemPrompt`.
- `OTEL_SEMCONV_STABILITY_OPT_IN=gen_ai_latest_experimental`: alternância de ambiente para os atributos de provedor de span GenAI experimentais mais recentes. Por padrão, spans mantêm o atributo legado `gen_ai.system` para compatibilidade; métricas GenAI usam atributos semânticos limitados.
- `OPENCLAW_OTEL_PRELOADED=1`: alternância de ambiente para hosts que já registraram um SDK OpenTelemetry global. O OpenClaw então ignora a inicialização/desligamento do SDK pertencente ao Plugin enquanto mantém ouvintes de diagnóstico ativos.
...
- `cacheTrace.includeMessages` / `includePrompt` / `includeSystem`: controla o que é incluído na saída de trace de cache (todos padrão: `true`).
...
- `auto.enabled`: ativa atualização automática em segundo plano para instalações de pacotes (padrão: `false`).
...
- `enabled`: controle global de recurso ACP (padrão: `true`; defina `false` para ocultar affordances de despacho e spawn ACP).
- `dispatch.enabled`: controle independente para despacho de turnos de sessão ACP (padrão: `true`). Defina `false` para manter comandos ACP disponíveis enquanto bloqueia a execução.
...
- `defaultAgent`: id de agente ACP de destino fallback quando spawns não especificam um destino explícito.
...
- `sessionRetention`: por quanto tempo manter sessões de execução cron isoladas concluídas antes de removê-las de `sessions.json`. Também controla a limpeza de transcrições cron arquivadas e excluídas. Padrão: `24h`; defina como `false` para desativar.
...
- `webhook`: URL de Webhook legado obsoleto de fallback (http/https) usada apenas para trabalhos armazenados que ainda têm `notify: true`.
...
- `maxAttempts`: máximo de novas tentativas para trabalhos únicos em erros transitórios (padrão: `3`; intervalo: `0`-`10`).
- `backoffMs`: matriz de atrasos de recuo em ms para cada tentativa de repetição (padrão: `[30000, 60000, 300000]`; 1-10 entradas).
...
Aplica-se apenas a trabalhos cron únicos. Trabalhos recorrentes usam tratamento de falhas separado.
...
- `includeSkipped`: conta execuções ignoradas consecutivas para o limite de alerta (padrão: `false`). Execuções ignoradas são rastreadas separadamente e não afetam o recuo de erro de execução.
...
- Quando nenhum destino de falha global ou por trabalho é definido, trabalhos que já entregam via `announce` recorrem a esse destino de anúncio primário em caso de falha.
...
- Gravações pertencentes ao OpenClaw que alteram apenas uma seção de nível superior respaldada por uma inclusão de arquivo único gravam nesse arquivo incluído. Por exemplo, `plugins install` atualiza `plugins: { $include: "./plugins.json5" }` em `plugins.json5` e deixa `openclaw.json` intacto.
- Inclusões raiz, matrizes de inclusão e inclusões com substituições irmãs são somente leitura para gravações pertencentes ao OpenClaw; essas gravações falham de modo fechado em vez de achatar a configuração.
