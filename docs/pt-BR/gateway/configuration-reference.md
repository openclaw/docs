---
read_when:
    - Você precisa de semântica de configuração ou valores padrão exatos em nível de campo
...
x-i18n:
    generated_at: "2026-06-30T22:09:10Z"
    model: gpt-5.5
...
Abrange as principais superfícies de configuração do OpenClaw e aponta para outras páginas quando um subsistema tem sua própria referência mais aprofundada. Catálogos de comandos pertencentes a canais e Plugins, além de controles aprofundados de memória/QMD, ficam em suas próprias páginas em vez desta.

Fonte de verdade no código:

- `openclaw config schema` imprime o JSON Schema ativo usado para validação e pela Interface de Controle, com metadados de pacotes/Plugins/canais mesclados quando disponíveis
- `config.schema.lookup` retorna um nó de schema com escopo por caminho para ferramentas de detalhamento
- `pnpm config:docs:check` / `pnpm config:docs:gen` validam o hash de referência da documentação de configuração contra a superfície de schema atual
...
canais empacotados (autenticação, controle de acesso, várias contas, controle de menções).
...
- `talk.*` (modo Talk)
  - `talk.consultThinkingLevel`: substituição do nível de raciocínio para a execução completa do agente OpenClaw por trás das consultas em tempo real do Talk na Interface de Controle
  - `talk.consultFastMode`: substituição única de modo rápido para consultas em tempo real do Talk na Interface de Controle
  ...
  - `talk.realtime.consultRouting`: fallback de retransmissão do Gateway para transcrições finalizadas do Talk em tempo real que pulam `openclaw_agent_consult`

## Ferramentas e provedores personalizados

Política de ferramentas, alternâncias experimentais, configuração de ferramentas baseadas em provedores e
configuração de provedor personalizado / URL base foram movidas para uma página dedicada - veja
...
- `models.providers.*.localService`: gerenciador opcional de processos sob demanda para
  servidores de modelos locais. O OpenClaw sonda o endpoint de saúde configurado, inicia
  o `command` absoluto quando necessário, aguarda a prontidão e então envia a solicitação
  do modelo. Veja [Serviços de modelos locais](/pt-BR/gateway/local-model-services).
- `models.pricing.enabled`: controla a inicialização de preços em segundo plano que
  ...
  o Gateway ignora buscas de catálogos de preços do OpenRouter e LiteLLM; valores
...
- `mcp.servers.<name>.enabled`: defina como `false` para manter uma definição de servidor salva
  enquanto a exclui da descoberta MCP do OpenClaw incorporado e da projeção de ferramentas.
- `mcp.servers.<name>.timeout` / `requestTimeoutMs`: tempo limite por servidor para solicitações MCP
...
- `mcp.servers.<name>.oauth`: escopo OAuth opcional, URL de redirecionamento e substituições de URL
...
- `mcp.servers.<name>.codex`: controles opcionais de projeção do servidor de app Codex.
  ...
  manter o servidor projetado para todos os agentes do servidor de app Codex com o comportamento
  padrão de aprovação MCP do Codex.
- `mcp.sessionIdleTtlMs`: TTL ocioso para runtimes MCP empacotados com escopo de sessão.
  Execuções incorporadas únicas solicitam limpeza no fim da execução; este TTL é a proteção de retaguarda para
...
- A descoberta em runtime também respeita notificações de alteração da lista de ferramentas MCP ao descartar
  o catálogo em cache daquela sessão. Servidores que anunciam recursos ou
  prompts recebem ferramentas utilitárias para listar/ler recursos e listar/buscar
  prompts. Falhas repetidas de chamadas de ferramentas pausam brevemente o servidor afetado antes que
...
- `allowBundled`: lista de permissão opcional apenas para Skills empacotadas (Skills gerenciadas/do workspace não são afetadas).
- `load.extraDirs`: raízes extras compartilhadas de Skills (menor precedência).
- `load.allowSymlinkTargets`: raízes de destino reais confiáveis às quais symlinks de Skills podem
  resolver quando o link fica fora da raiz de origem configurada.
- `workshop.allowSymlinkTargetWrites`: permite que a aplicação do Skill Workshop escreva
  por meio de destinos de symlink já confiáveis (padrão: false).
...
- `plugins.entries.<id>.hooks.allowConversationAccess`: quando `true`, plugins confiáveis não incluídos em bundles podem ler o conteúdo bruto da conversa a partir de hooks tipados como `llm_input`, `llm_output`, `before_model_resolve`, `before_agent_reply`, `before_agent_run`, `before_agent_finalize` e `agent_end`.
...
- `plugins.entries.<id>.subagent.allowedModels`: lista de permissões opcional de destinos canônicos `provider/model` para substituições confiáveis de subagentes. Use `"*"` somente quando você quiser intencionalmente permitir qualquer modelo.
...
- `plugins.entries.<id>.llm.allowedModels`: lista de permissões opcional de destinos canônicos `provider/model` para substituições confiáveis de conclusões LLM de plugins. Use `"*"` somente quando você quiser intencionalmente permitir qualquer modelo.
...
- As configurações de conta/runtime de plugins de canal ficam em `channels.<id>` e devem ser descritas pelos metadados `channelConfigs` do manifesto do plugin proprietário, não por um registro central de opções do OpenClaw.

### Configuração do plugin de harness do Codex

O plugin `codex` incluído é proprietário das configurações do harness nativo do servidor de aplicativo Codex em
...
Ele não habilita plugins do Codex para execuções de provedor OpenClaw, associações de conversa
...
- `plugins.entries.codex.config.codexPlugins.enabled`: habilita o suporte nativo a plugins/aplicativos do Codex para o harness do Codex. Padrão: `false`.
- `plugins.entries.codex.config.codexPlugins.allow_destructive_actions`:
  política padrão de ações destrutivas para elicitações migradas de aplicativos de plugin.
  Use `true` para aceitar esquemas seguros de aprovação do Codex sem solicitar confirmação, `false`
  para recusá-los, `"auto"` para rotear aprovações exigidas pelo Codex por aprovações de
  plugins do OpenClaw, ou `"always"` para solicitar confirmação para cada ação destrutiva/de escrita
  de plugin sem aprovação durável. O modo `"always"` limpa substituições duráveis de aprovação
  por ferramenta do Codex para o aplicativo afetado antes de iniciar a thread.
  Padrão: `true`.
...
- `plugins.entries.codex.config.codexPlugins.plugins.<key>.pluginName`: identidade estável
  do plugin Codex da migração, por exemplo `"google-calendar"`.
- `plugins.entries.codex.config.codexPlugins.plugins.<key>.allow_destructive_actions`:
  substituição de ações destrutivas por plugin. Quando omitida, o valor global
...
assincronamente quando obsoletas. A configuração de aplicativo da thread Codex é calculada no
estabelecimento da sessão do harness Codex, não a cada turno; use `/new`, `/reset` ou uma
reinicialização do gateway depois de alterar a configuração nativa do plugin.
...
- Plugins de bundle do Claude habilitados também podem contribuir padrões incorporados do OpenClaw a partir de `settings.json`; o OpenClaw aplica esses padrões como configurações sanitizadas de agente, não como patches brutos de configuração do OpenClaw.
...
`commitments` controla a memória inferida de acompanhamento: o OpenClaw pode detectar acompanhamentos a partir de turnos de conversa e entregá-los por execuções de Heartbeat.
...
- No modo estrito, endpoints de perfil CDP remotos (`profiles.*.cdpUrl`) ficam sujeitos ao mesmo bloqueio de rede privada durante verificações de acessibilidade/descoberta.
...
- Perfis remotos são apenas de anexação (iniciar/parar/redefinir desabilitados).
...
- `remoteCdpTimeoutMs` e `remoteCdpHandshakeTimeoutMs` se aplicam à acessibilidade de CDP remoto e
  `attachOnly`, além de solicitações de abertura de abas. Perfis gerenciados por loopback
  mantêm os padrões locais de CDP.
- Se um serviço CDP gerenciado externamente estiver acessível via loopback, defina
  `attachOnly: true` nesse perfil; caso contrário, o OpenClaw trata a porta de loopback como um
  perfil de navegador gerenciado localmente e pode relatar erros de propriedade da porta local.
...
- Perfis `openclaw` gerenciados localmente atribuem automaticamente `cdpPort` e `cdpUrl`; defina
  `cdpUrl` explicitamente somente para perfis CDP remotos ou anexação de endpoint
  existing-session.
...
- Perfis gerenciados localmente usam `browser.localLaunchTimeoutMs` para descoberta HTTP do Chrome CDP
  após o início do processo e `browser.localCdpReadyTimeoutMs` para
  prontidão do websocket CDP pós-inicialização. Aumente esses valores em hosts mais lentos onde o Chrome
...
- `browser.executablePath` e `browser.profiles.<name>.executablePath` ambos
  aceitam `~` e `~/...` para o diretório inicial do seu SO antes da inicialização do Chromium.
...
- `extraArgs` acrescenta flags extras de inicialização ao início local do Chromium (por exemplo
...
- `seamColor`: cor de destaque para o chrome da UI do aplicativo nativo (tom do balão do Talk Mode etc.).
...
- **Autenticação**: exigida por padrão. Binds que não são loopback exigem autenticação do Gateway. Na prática, isso significa um token/senha compartilhado ou um proxy reverso com reconhecimento de identidade com `gateway.auth.mode: "trusted-proxy"`. O assistente de onboarding gera um token por padrão.
...
- `gateway.auth.mode: "trusted-proxy"`: delega a autenticação de navegador/usuário a um proxy reverso com reconhecimento de identidade e confia nos cabeçalhos de identidade de `gateway.trustedProxies` (consulte [Autenticação por Proxy Confiável](/pt-BR/gateway/trusted-proxy-auth)). Esse modo espera por padrão uma origem de proxy **não loopback**; proxies reversos loopback no mesmo host exigem `gateway.auth.trustedProxy.allowLoopback = true` explícito. Chamadores internos no mesmo host podem usar `gateway.auth.password` como fallback direto local; `gateway.auth.token` permanece mutuamente exclusivo com o modo trusted-proxy.
...
--service` para que a Control UI possa ser exposta por meio de um Service nomeado em vez
  do nome de host do dispositivo. O valor deve usar o formato de nome de Service
  `svc:<dns-label>` do Tailscale; a inicialização informa a URL de Service derivada.
- `tailscale.preserveFunnel`: quando `true` e `tailscale.mode = "serve"`, o OpenClaw
  ...
  Padrão `false`.
- `controlUi.allowedOrigins`: lista de permissões explícita de origens de navegador para conexões WebSocket do Gateway. Obrigatória para origens de navegador públicas não loopback. Carregamentos de UI privados de mesma origem em LAN/Tailnet a partir de hosts loopback, RFC1918/link-local, `.local`, `.ts.net` ou Tailscale CGNAT são aceitos sem habilitar fallback por cabeçalho Host.
...
- `remote.transport`: `ssh` (padrão) ou `direct` (ws/wss). Para `direct`, `remote.url` deve ser `wss://` para hosts públicos; `ws://` em texto claro é aceito apenas para hosts loopback, LAN, link-local, `.local`, `.ts.net` e Tailscale CGNAT.
- `remote.remotePort`: porta do Gateway no host SSH remoto. O padrão é `18789`; use isso quando a porta local do túnel for diferente da porta remota do Gateway.
...
- `OPENCLAW_APNS_RELAY_ALLOW_HTTP=true`: escape hatch somente para desenvolvimento para URLs de relay HTTP loopback. URLs de relay de produção devem permanecer em HTTPS.
- `gateway.handshakeTimeoutMs`: timeout de handshake WebSocket pré-autenticação do Gateway em milissegundos. Padrão: `15000`. `OPENCLAW_HANDSHAKE_TIMEOUT_MS` tem precedência quando definido. Aumente isso em hosts carregados ou de baixa potência em que clientes locais possam se conectar enquanto o aquecimento da inicialização ainda está se estabilizando.
- `gateway.channelHealthCheckMinutes`: intervalo do monitor de integridade de canais em minutos. Defina `0` para desativar reinícios do monitor de integridade globalmente. Padrão: `5`.
...
- `gateway.channelMaxRestartsPerHour`: reinícios máximos do monitor de integridade por canal/conta em uma hora móvel. Padrão: `10`.
- `channels.<provider>.healthMonitor.enabled`: desativação por canal para reinícios do monitor de integridade, mantendo o monitor global habilitado.
...
- `gateway.nodes.pairing.autoApproveCidrs`: lista de permissões CIDR/IP opcional para aprovar automaticamente pareamento inicial de dispositivo de nó sem escopos solicitados. Ela fica desativada quando não definida. Isso não aprova automaticamente pareamento de operador/navegador/Control UI/WebChat, e não aprova automaticamente upgrades de função, escopo, metadados ou chave pública.
- `gateway.nodes.allowCommands` / `gateway.nodes.denyCommands`: modelagem global de permissão/negação para comandos de nó declarados após o pareamento e a avaliação da lista de permissões da plataforma. Use `allowCommands` para optar por comandos de nó perigosos, como `camera.snap`, `camera.clip` e `screen.record`; `denyCommands` remove um comando mesmo que um padrão de plataforma ou permissão explícita o incluísse de outra forma. Depois que um nó alterar sua lista de comandos declarados, rejeite e reaprove o pareamento desse dispositivo para que o Gateway armazene o snapshot de comandos atualizado.
- `gateway.tools.deny`: nomes de ferramentas extras bloqueados para HTTP `POST /tools/invoke` (estende a lista de negação padrão).
- `gateway.tools.allow`: remove nomes de ferramentas da lista de negação HTTP padrão para
  chamadores owner/admin. Isso não promove chamadores `operator.write` com identidade
  a acesso owner/admin; `cron`, `gateway` e `nodes` permanecem
  indisponíveis para chamadores não owner mesmo quando estão na lista de permissões.

</Accordion>

### Endpoints compatíveis com OpenAI

- RPC HTTP de administrador: desativado por padrão como o Plugin `admin-http-rpc`. Habilite o Plugin para registrar `POST /api/v1/admin/rpc`. Consulte [RPC HTTP de Administrador](/pt-BR/plugins/admin-http-rpc).
...
- `mode`: controla como as edições de configuração são aplicadas em tempo de execução.
  - `"off"`: ignora edições ao vivo; as mudanças exigem uma reinicialização explícita.
  - `"restart"`: sempre reinicia o processo do Gateway quando a configuração muda.
  - `"hot"`: aplica mudanças no processo sem reiniciar.
  - `"hybrid"` (padrão): tenta recarregamento a quente primeiro; volta para reinicialização se necessário.
...
- `openclaw security audit` sinaliza reutilização de autenticação de hook/Gateway como um achado crítico, incluindo autenticação por senha do Gateway fornecida apenas no momento da auditoria (`--auth password --password <password>`). Execute `openclaw doctor --fix` para rotacionar um `hooks.token` reutilizado persistido e, em seguida, atualize os remetentes externos de hooks para usar o novo token de hook.
...
- Se um mapeamento ou predefinição usa um `sessionKey` com template, defina `hooks.allowedSessionKeyPrefixes` e `hooks.allowRequestSessionKey=true`. Chaves de mapeamento estáticas não exigem essa adesão explícita.
...
- `agentId` roteia para um agente específico; IDs desconhecidos voltam para o agente padrão.
...
- `allowedSessionKeyPrefixes`: lista de permissões opcional de prefixos para valores explícitos de `sessionKey` (requisição + mapeamento), por exemplo, `["hook:"]`. Ela se torna obrigatória quando qualquer mapeamento ou predefinição usa um `sessionKey` com template.
...
- Serve HTML/CSS/JS editáveis por agente e A2UI via HTTP sob a porta do Gateway:
  - `http://<gateway-host>:<gateway.port>/__openclaw__/canvas/`
  - `http://<gateway-host>:<gateway.port>/__openclaw__/a2ui/`
- Somente local: mantenha `gateway.bind: "loopback"` (padrão).
- Binds não loopback: rotas de canvas exigem autenticação do Gateway (token/senha/proxy confiável), igual a outras superfícies HTTP do Gateway.
...
- `off`: suprime publicidade multicast na LAN sem alterar a habilitação do Plugin.
- O Plugin `bonjour` incluído inicia automaticamente em hosts macOS e é opt-in no Linux, Windows e implantações do Gateway em contêineres.
- O nome do host usa o hostname do sistema como padrão quando ele é um rótulo DNS válido, voltando para `openclaw`. Substitua com `OPENCLAW_MDNS_HOSTNAME`.
...
Refs de segredos são aditivas: valores em texto simples ainda funcionam.
...
- Padrão de id de `source: "exec"`: `^[A-Za-z0-9][A-Za-z0-9._:/#-]{0,255}$` (oferece suporte a seletores no estilo AWS `secret#json_key`)
...
- Caminhos de provedores de arquivo e exec falham fechados quando a verificação de ACL do Windows está indisponível. Defina `allowInsecurePath: true` apenas para caminhos confiáveis que não podem ser verificados.
- O provedor `exec` exige um caminho `command` absoluto e usa payloads de protocolo em stdin/stdout.
...
- Refs de segredos são resolvidas no momento da ativação em um snapshot na memória; depois, os caminhos de solicitação leem apenas o snapshot.
...
- `auth-profiles.json` oferece suporte a refs no nível do valor (`keyRef` para `api_key`, `tokenRef` para `token`) para modos de credenciais estáticas.
...
- `billingBackoffHours`: backoff base em horas quando um perfil falha devido a erros reais de
  faturamento/crédito insuficiente (padrão: `5`). Texto explícito de faturamento ainda pode
  cair aqui mesmo em respostas `401`/`403`, mas correspondências de texto específicas do provedor
  permanecem limitadas ao provedor que as possui (por exemplo, OpenRouter
  `Key limit exceeded`). Mensagens HTTP `402` repetíveis de janela de uso ou
  limite de gastos da organização/workspace permanecem no caminho `rate_limit`
  em vez disso.
- `billingBackoffHoursByProvider`: substituições opcionais por provedor para horas de backoff de faturamento.
- `billingMaxHours`: limite em horas para crescimento exponencial do backoff de faturamento (padrão: `24`).
...
- `stuckSessionAbortMs`: limite de idade sem progresso em ms antes que trabalho ativo travado qualificado possa ser abortado e drenado para recuperação. Quando não definido, OpenClaw usa a janela incorporada estendida mais segura de pelo menos 5 minutos e 3x `stuckSessionWarnMs`.
- `memoryPressureSnapshot`: captura um snapshot de estabilidade redigido pré-OOM quando a pressão de memória atinge `critical` (padrão: `false`). Defina como `true` para adicionar a varredura/gravação do arquivo do pacote de estabilidade, mantendo eventos normais de pressão de memória.
...
- `OPENCLAW_OTEL_PRELOADED=1`: alternância de ambiente para hosts que já registraram um SDK OpenTelemetry global. OpenClaw então ignora inicialização/desligamento do SDK pertencente ao Plugin, mantendo os listeners de diagnóstico ativos.
...
- `auto.stableJitterHours`: janela extra em horas para espalhar o rollout do canal estável (padrão: `12`; máx.: `168`).
- `auto.betaCheckIntervalHours`: frequência em horas com que verificações do canal beta são executadas (padrão: `1`; máx.: `24`).
...
- `allowedAgents`: lista de permissão de ids de agentes autorizados para sessões de runtime ACP; vazio significa sem restrição adicional.
...
- `stream.maxChunkChars`: tamanho máximo do chunk antes de dividir a projeção de bloco transmitida por stream.
...
- `cli.banner.taglineMode` controla o estilo da frase do banner:
  - `"random"` (padrão): frases divertidas/sazonais rotativas.
  - `"default"`: frase neutra fixa (`All your chats, one OpenClaw.`).
  - `"off"`: nenhum texto de frase (título/versão do banner ainda exibidos).
...
As builds atuais não incluem mais a ponte TCP. Os nós se conectam pelo Gateway WebSocket. As chaves `bridge.*` não fazem mais parte do esquema de configuração (a validação falha até que sejam removidas; `openclaw doctor --fix` pode remover chaves desconhecidas).
...
- `sessionRetention`: por quanto tempo manter sessões concluídas de execução cron isolada antes da limpeza de `sessions.json`. Também controla a limpeza de transcrições cron excluídas e arquivadas. Padrão: `24h`; defina como `false` para desabilitar.
...
- `webhook`: URL de webhook legada e obsoleta (http/https) usada por `openclaw doctor --fix` para migrar trabalhos armazenados que ainda têm `notify: true`; a entrega em tempo de execução usa `delivery.mode="webhook"` por trabalho mais `delivery.to`, ou `delivery.completionDestination` ao preservar a entrega de anúncio.
...
- `maxAttempts`: número máximo de novas tentativas para trabalhos cron em erros transitórios (padrão: `3`; intervalo: `0`-`10`).
...
- `mode`: `"announce"` ou `"webhook"`; assume `"announce"` como padrão quando há dados de destino suficientes.
- `channel`: substituição de canal para entrega de anúncio. `"last"` reutiliza o último canal de entrega conhecido.
- `to`: destino de anúncio explícito ou URL de webhook. Obrigatório para o modo webhook.
...
- Quando nenhum destino de falha global nem por trabalho está definido, trabalhos que já entregam via `announce` voltam para esse destino de anúncio primário em caso de falha.
...
- Escritas de propriedade do OpenClaw que alteram apenas uma seção de nível superior baseada em um include de arquivo único escrevem diretamente nesse arquivo incluído. Por exemplo, `plugins install` atualiza `plugins: { $include: "./plugins.json5" }` em `plugins.json5` e deixa `openclaw.json` intacto.
- Includes raiz, arrays de include e includes com substituições por chaves irmãs são somente leitura para escritas de propriedade do OpenClaw; essas escritas falham de forma fechada em vez de achatar a configuração.
