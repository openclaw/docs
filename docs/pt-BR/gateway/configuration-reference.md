...
x-i18n:
    generated_at: "2026-05-05T05:42:20Z"
    model: gpt-5.5
    provider: openai
...
Abrange as principais superfícies de configuração do OpenClaw e aponta para outras páginas quando um subsistema tem sua própria referência mais aprofundada. Catálogos de comandos pertencentes a canais e plugins e ajustes avançados de memória/QMD ficam em suas próprias páginas, em vez desta.

Fonte de verdade do código:

- `openclaw config schema` imprime o JSON Schema ativo usado para validação e Control UI, com metadados de pacotes/plugins/canais integrados mesclados quando disponíveis
- `config.schema.lookup` retorna um nó de esquema com escopo de caminho para ferramentas de detalhamento
- `pnpm config:docs:check` / `pnpm config:docs:gen` validam o hash de base da documentação de configuração em relação à superfície de esquema atual

Caminho de consulta de agente: use a ação de ferramenta `gateway` `config.schema.lookup` para
documentação e restrições exatas em nível de campo antes de edições. Use
...
- páginas de canais/plugins responsáveis por superfícies de comando específicas do canal
...
- `session.*` (ciclo de vida de sessão, compaction, poda)
...
A raiz `models` também é responsável pelo comportamento global do catálogo de modelos.
...
consumidas pelo Pi integrado e outros adaptadores de runtime. Os comandos `openclaw mcp list`,
...
- `mcp.sessionIdleTtlMs`: TTL ocioso para runtimes MCP incluídos com escopo de sessão.
  Execuções integradas de uso único solicitam limpeza ao fim da execução; este TTL é o recurso de segurança para
...
- Alterações em `mcp.*` são aplicadas a quente ao descartar runtimes MCP de sessão em cache.
  A próxima descoberta/uso de ferramentas os recria a partir da nova configuração, então entradas
...
- `allowBundled`: allowlist opcional apenas para skills incluídas (skills gerenciadas/workspace não afetadas).
- `load.extraDirs`: raízes extras compartilhadas de skills (menor precedência).
- `install.preferBrew`: quando verdadeiro, preferir instaladores Homebrew quando `brew` estiver
...
- `install.nodeManager`: preferência de instalador node para especificações `metadata.openclaw.install`
  (`npm` | `pnpm` | `yarn` | `bun`).
- `entries.<skillKey>.enabled: false` desativa uma skill mesmo se incluída/instalada.
...
- A descoberta aceita plugins nativos do OpenClaw, além de pacotes Codex compatíveis e pacotes Claude, incluindo pacotes Claude sem manifesto com layout padrão.
- **Alterações de configuração exigem reinicialização do gateway.**
- `allow`: allowlist opcional (apenas plugins listados carregam). `deny` prevalece.
- `bundledDiscovery`: o padrão é `"allowlist"` para novas configurações, então um
  `plugins.allow` não vazio também bloqueia plugins de provedores incluídos, incluindo provedores de runtime
  de busca na Web. O Doctor escreve `"compat"` para configurações migradas de allowlist legada
  para preservar o comportamento existente de provedores incluídos até você optar por aderir.
...
- `plugins.entries.<id>.hooks.allowConversationAccess`: quando `true`, plugins não incluídos confiáveis podem ler conteúdo bruto de conversas de hooks tipados como `llm_input`, `llm_output`, `before_agent_finalize` e `agent_end`.
...
- `plugins.entries.firecrawl.config.webFetch`: configurações do provedor de web-fetch Firecrawl.
  ...
  - `baseUrl`: URL base da API Firecrawl (padrão: `https://api.firecrawl.dev`; substituições auto-hospedadas devem direcionar endpoints privados/internos).
...
- `plugins.entries.xai.config.xSearch`: configurações do xAI X Search (busca na Web do Grok).
  - `enabled`: habilitar o provedor X Search.
...
- Plugins de pacote Claude habilitados também podem contribuir padrões integrados do Pi a partir de `settings.json`; o OpenClaw aplica isso como configurações de agente sanitizadas, não como patches brutos de configuração do OpenClaw.
...
- `commitments.enabled`: habilita extração oculta por LLM, armazenamento e entrega por heartbeat de compromissos inferidos de acompanhamento. Padrão: `false`.
...
- `ssrfPolicy.dangerouslyAllowPrivateNetwork` fica desabilitado quando não definido, portanto a navegação do navegador permanece estrita por padrão.
...
- No modo estrito, endpoints de perfis CDP remotos (`profiles.*.cdpUrl`) estão sujeitos ao mesmo bloqueio de rede privada durante verificações de alcance/descoberta.
...
- `remoteCdpTimeoutMs` e `remoteCdpHandshakeTimeoutMs` se aplicam ao alcance de CDP remoto e
...
- Se um serviço CDP gerenciado externamente estiver acessível por loopback, defina
  ...
  perfil local de navegador gerenciado e pode relatar erros de propriedade de porta local.
...
- Perfis `existing-session` mantêm os limites atuais da rota Chrome MCP:
  ...
  de arquivo único, sem substituições de timeout de diálogo, sem `wait --load networkidle` e sem
  `responsebody`, exportação de PDF, interceptação de download ou ações em lote.
- Perfis locais gerenciados `openclaw` atribuem `cdpPort` e `cdpUrl` automaticamente; defina
...
- Perfis locais gerenciados usam `browser.localLaunchTimeoutMs` para descoberta HTTP de CDP do Chrome
  após o início do processo e `browser.localCdpReadyTimeoutMs` para
  prontidão do websocket CDP após a inicialização. Aumente-os em hosts mais lentos nos quais o Chrome
...
- `seamColor`: cor de destaque para o chrome da UI do app nativo (tom da bolha do Modo de Conversa etc.).
- `assistant`: substituição de identidade da UI de controle. Recai para a identidade do agente ativo.
...
- **Autenticação**: exigida por padrão. Binds que não são loopback exigem autenticação do Gateway. Na prática, isso significa um token/senha compartilhado ou um proxy reverso com reconhecimento de identidade usando `gateway.auth.mode: "trusted-proxy"`. O assistente de onboarding gera um token por padrão.
...
- `gateway.auth.mode: "trusted-proxy"`: delega a autenticação de navegador/usuário a um proxy reverso com reconhecimento de identidade e confia nos cabeçalhos de identidade de `gateway.trustedProxies` (veja [Autenticação por proxy confiável](/pt-BR/gateway/trusted-proxy-auth)). Por padrão, esse modo espera uma origem de proxy **não loopback**; proxies reversos loopback no mesmo host exigem `gateway.auth.trustedProxy.allowLoopback = true` explícito. Chamadores internos no mesmo host podem usar `gateway.auth.password` como fallback direto local; `gateway.auth.token` permanece mutuamente exclusivo com o modo trusted-proxy.
- `gateway.auth.allowTailscale`: quando `true`, os cabeçalhos de identidade do Tailscale Serve podem satisfazer a autenticação da Interface de controle/WebSocket (verificada via `tailscale whois`). Endpoints da API HTTP **não** usam essa autenticação por cabeçalho do Tailscale; em vez disso, seguem o modo normal de autenticação HTTP do Gateway. Esse fluxo sem token presume que o host do Gateway é confiável. O padrão é `true` quando `tailscale.mode = "serve"`.
- `gateway.auth.rateLimit`: limitador opcional de falhas de autenticação. Aplica-se por IP do cliente e por escopo de autenticação (shared-secret e device-token são rastreados independentemente). Tentativas bloqueadas retornam `429` + `Retry-After`.
  - No caminho assíncrono da Interface de controle do Tailscale Serve, as tentativas com falha para o mesmo `{scope, clientIp}` são serializadas antes da gravação da falha. Tentativas ruins simultâneas do mesmo cliente podem, portanto, acionar o limitador na segunda solicitação, em vez de ambas avançarem em disputa como simples incompatibilidades.
  - `gateway.auth.rateLimit.exemptLoopback` usa `true` por padrão; defina `false` quando você quiser intencionalmente limitar também o tráfego de localhost (para configurações de teste ou implantações de proxy rigorosas).
- Tentativas de autenticação WS com origem no navegador são sempre limitadas com a isenção de loopback desabilitada (defesa em profundidade contra força bruta baseada em navegador no localhost).
- Em loopback, esses bloqueios com origem no navegador são isolados por valor `Origin`
...
- `controlUi.allowedOrigins`: allowlist explícita de origem do navegador para conexões WebSocket do Gateway. Obrigatória quando clientes de navegador são esperados de origens não loopback.
- `controlUi.chatMessageMaxWidth`: largura máxima opcional para mensagens de chat agrupadas da Interface de controle. Aceita valores restritos de largura CSS, como `960px`, `82%`, `min(1280px, 82%)` e `calc(100% - 2rem)`.
...
- `OPENCLAW_ALLOW_INSECURE_PRIVATE_WS=1`: substituição de emergência no ambiente de processo do lado do cliente
  que permite `ws://` em texto claro para IPs confiáveis de rede privada;
  o padrão permanece somente loopback para texto claro. Não há equivalente em `openclaw.json`,
...
- `gateway.remote.token` / `.password` são campos de credencial de cliente remoto. Eles não configuram autenticação do Gateway por si só.
- `gateway.push.apns.relay.baseUrl`: URL HTTPS base para o relé APNs externo usado por builds iOS oficiais/TestFlight depois que elas publicam registros apoiados por relé no Gateway. Essa URL deve corresponder à URL do relé compilada na build iOS.
- `gateway.push.apns.relay.timeoutMs`: tempo limite de envio do Gateway para o relé em milissegundos. O padrão é `10000`.
- Registros apoiados por relé são delegados a uma identidade específica do Gateway. O app iOS pareado busca `gateway.identity.get`, inclui essa identidade no registro do relé e encaminha ao Gateway uma concessão de envio com escopo de registro. Outro Gateway não pode reutilizar esse registro armazenado.
- `OPENCLAW_APNS_RELAY_BASE_URL` / `OPENCLAW_APNS_RELAY_TIMEOUT_MS`: substituições temporárias de ambiente para a configuração de relé acima.
- `OPENCLAW_APNS_RELAY_ALLOW_HTTP=true`: escape hatch somente para desenvolvimento para URLs de relé HTTP em loopback. URLs de relé de produção devem permanecer em HTTPS.
- `gateway.handshakeTimeoutMs`: tempo limite de handshake WebSocket do Gateway antes da autenticação, em milissegundos. Padrão: `15000`. `OPENCLAW_HANDSHAKE_TIMEOUT_MS` tem precedência quando definido. Aumente isso em hosts carregados ou de baixa potência nos quais clientes locais conseguem se conectar enquanto o aquecimento da inicialização ainda está se estabilizando.
- `gateway.channelHealthCheckMinutes`: intervalo do monitor de saúde de canal em minutos. Defina `0` para desabilitar globalmente reinicializações pelo monitor de saúde. Padrão: `5`.
...
- `gateway.channelMaxRestartsPerHour`: número máximo de reinicializações pelo monitor de saúde por canal/conta em uma hora móvel. Padrão: `10`.
- `channels.<provider>.healthMonitor.enabled`: opt-out por canal para reinicializações pelo monitor de saúde mantendo o monitor global habilitado.
...
- `gateway.nodes.pairing.autoApproveCidrs`: allowlist CIDR/IP opcional para aprovar automaticamente o pareamento inicial de dispositivo de Node sem escopos solicitados. Fica desabilitada quando não definida. Isso não aprova automaticamente pareamento de operador/navegador/Interface de controle/WebChat, e não aprova automaticamente upgrades de função, escopo, metadados ou chave pública.
- `gateway.nodes.allowCommands` / `gateway.nodes.denyCommands`: modelagem global de permissão/negação para comandos de Node declarados após o pareamento e a avaliação da allowlist da plataforma. Use `allowCommands` para optar por comandos perigosos de Node, como `camera.snap`, `camera.clip` e `screen.record`; `denyCommands` remove um comando mesmo que um padrão de plataforma ou permissão explícita o incluísse de outra forma. Depois que um Node alterar sua lista de comandos declarados, rejeite e reaprove esse pareamento de dispositivo para que o Gateway armazene o snapshot de comandos atualizado.
...
- Reforço de entrada de URL da Responses:
...
- `enabled`: habilita encerramento TLS no listener do Gateway (HTTPS/WSS) (padrão: `false`).
- `autoGenerate`: gera automaticamente um par de certificado/chave local autoassinado quando arquivos explícitos não são configurados; somente para uso local/dev.
- `certPath`: caminho do sistema de arquivos para o arquivo de certificado TLS.
- `keyPath`: caminho do sistema de arquivos para o arquivo de chave privada TLS; mantenha com permissões restritas.
...
Tokens de hooks na string de consulta são rejeitados.
...
- Se um mapeamento ou predefinição usa um `sessionKey` com template, defina `hooks.allowedSessionKeyPrefixes` e `hooks.allowRequestSessionKey=true`. Chaves de mapeamento estáticas não exigem essa aceitação explícita.
...
- `transform` pode apontar para um módulo JS/TS que retorna uma ação de hook.
  ...
  - Mantenha `hooks.transformsDir` em `~/.openclaw/hooks/transforms`; diretórios de Skills do espaço de trabalho são rejeitados. Se `openclaw doctor` relatar esse caminho como inválido, mova o módulo de transformação para o diretório de transformações de hooks ou remova `hooks.transformsDir`.
...
- `allowedSessionKeyPrefixes`: lista de permissões de prefixos opcional para valores explícitos de `sessionKey` (requisição + mapeamento), por exemplo `["hook:"]`. Ela passa a ser obrigatória quando qualquer mapeamento ou predefinição usa um `sessionKey` com template.
- `deliver: true` envia a resposta final para um canal; `channel` assume `last` por padrão.
...
- Vinculações que não são loopback: rotas de canvas exigem autenticação do Gateway (token/senha/proxy confiável), igual a outras superfícies HTTP do Gateway.
- WebViews do Node normalmente não enviam cabeçalhos de autenticação; depois que um node é pareado e conectado, o Gateway anuncia URLs de capacidade com escopo do node para acesso a canvas/A2UI.
...
- `full`: inclui `cliPath` + `sshPort`; a publicidade multicast da LAN ainda exige que o Plugin `bonjour` empacotado esteja habilitado.
- `off`: suprime a publicidade multicast da LAN sem alterar a habilitação do Plugin.
- O Plugin `bonjour` empacotado inicia automaticamente em hosts macOS e é opcional em implantações do Gateway no Linux, Windows e em contêineres.
- O nome do host assume como padrão o nome do host do sistema quando ele é um rótulo DNS válido, com fallback para `openclaw`. Substitua com `OPENCLAW_MDNS_HOSTNAME`.
...
- Caminhos de provedores file e exec falham de forma fechada quando a verificação de ACL do Windows está indisponível. Defina `allowInsecurePath: true` somente para caminhos confiáveis que não podem ser verificados.
- O provedor `exec` exige um caminho absoluto em `command` e usa payloads de protocolo em stdin/stdout.
- Por padrão, caminhos de comandos por symlink são rejeitados. Defina `allowSymlinkCommand: true` para permitir caminhos por symlink enquanto valida o caminho de destino resolvido.
- Se `trustedDirs` estiver configurado, a verificação de diretório confiável se aplica ao caminho de destino resolvido.
- O ambiente filho de `exec` é mínimo por padrão; passe variáveis obrigatórias explicitamente com `passEnv`.
- Referências de segredo são resolvidas no momento da ativação para um snapshot em memória; depois, os caminhos de requisição leem somente o snapshot.
...
- `auth-profiles.json` é compatível com refs no nível de valor (`keyRef` para `api_key`, `tokenRef` para `token`) para modos de credenciais estáticas.
- Mapas planos legados de `auth-profiles.json`, como `{ "provider": { "apiKey": "..." } }`, não são um formato de tempo de execução; `openclaw doctor --fix` os reescreve para perfis canônicos de chave de API `provider:default` com um backup `.legacy-flat.*.bak`.
...
- Comportamento de tempo de execução de segredos e ferramentas `audit/configure/apply`: [Gerenciamento de segredos](/pt-BR/gateway/secrets).
...
- `billingBackoffHours`: recuo base em horas quando um perfil falha devido a erros reais de
  faturamento/crédito insuficiente (padrão: `5`). Texto explícito de faturamento
  ainda pode cair aqui mesmo em respostas `401`/`403`, mas os correspondedores
  de texto específicos de provedor permanecem limitados ao provedor que os possui
  (por exemplo, OpenRouter `Key limit exceeded`). Mensagens de HTTP `402`
  retentáveis de janela de uso ou limite de gastos de organização/workspace
  permanecem no caminho `rate_limit`.
...
- `authPermanentMaxMinutes`: limite em minutos para o crescimento do recuo `auth_permanent` (padrão: `60`).
- `failureWindowHours`: janela deslizante em horas usada para contadores de recuo (padrão: `24`).
- `overloadedProfileRotations`: máximo de rotações de perfil de autenticação do mesmo provedor para erros de sobrecarga antes de alternar para o fallback de modelo (padrão: `1`). Formatos de provedor ocupado, como `ModelNotReadyException`, caem aqui.
- `overloadedBackoffMs`: atraso fixo antes de tentar novamente uma rotação de provedor/perfil sobrecarregado (padrão: `0`).
- `rateLimitedProfileRotations`: máximo de rotações de perfil de autenticação do mesmo provedor para erros de limite de taxa antes de alternar para o fallback de modelo (padrão: `1`). Esse grupo de limite de taxa inclui texto em formato de provedor, como `Too many concurrent requests`, `ThrottlingException`, `concurrency limit reached`, `workers_ai ... quota limit exceeded` e `resource exhausted`.
...
- `redactSensitive` / `redactPatterns`: mascaramento de melhor esforço para saída no console, logs de arquivo, registros de log OTLP e texto persistido de transcrição de sessão. `redactSensitive: "off"` desativa apenas esta política geral de log/transcrição; superfícies de segurança de UI/ferramenta/diagnóstico ainda removem segredos antes da emissão.
...
- `stuckSessionWarnMs`: limite de idade sem progresso em ms para classificar sessões de processamento de longa duração como `session.long_running`, `session.stalled` ou `session.stuck`. Resposta, ferramenta, status, bloco e progresso ACP reiniciam o temporizador; diagnósticos `session.stuck` repetidos recuam enquanto permanecerem inalterados.
- `stuckSessionAbortMs`: limite de idade sem progresso em ms antes que trabalho ativo parado elegível possa ser abortado e drenado para recuperação. Quando não definido, OpenClaw usa a janela incorporada estendida mais segura de pelo menos 10 minutos e 5x `stuckSessionWarnMs`.
...
- `otel.captureContent`: captura opcional de conteúdo bruto para atributos de spans OTEL. Desativada por padrão. Booleano `true` captura conteúdo de mensagens/ferramentas que não seja do sistema; a forma de objeto permite habilitar `inputMessages`, `outputMessages`, `toolInputs`, `toolOutputs` e `systemPrompt` explicitamente.
...
- `OPENCLAW_OTEL_PRELOADED=1`: alternância de ambiente para hosts que já registraram um SDK OpenTelemetry global. OpenClaw então ignora a inicialização/desligamento do SDK pertencente ao Plugin, mantendo os ouvintes de diagnóstico ativos.
...
- `auto.stableJitterHours`: janela adicional de distribuição do lançamento no canal estável em horas (padrão: `12`; máx.: `168`).
...
- `enabled`: gate global do recurso ACP (padrão: `true`; defina `false` para ocultar despacho ACP e affordances de spawn).
...
- `allowedAgents`: lista de permissões de ids de agentes permitidos para sessões de runtime ACP; vazia significa sem restrição adicional.
- `maxConcurrentSessions`: máximo de sessões ACP ativas simultaneamente.
- `stream.coalesceIdleMs`: janela de envio ocioso em ms para texto transmitido em stream.
- `stream.maxChunkChars`: tamanho máximo de fragmento antes de dividir a projeção de bloco transmitido em stream.
...
- `stream.tagVisibility`: registro de nomes de tags para substituições booleanas de visibilidade em eventos transmitidos em stream.
...
As compilações atuais não incluem mais a ponte TCP. Nodes se conectam pelo WebSocket do Gateway. Chaves `bridge.*` não fazem mais parte do esquema de configuração (a validação falha até serem removidas; `openclaw doctor --fix` pode remover chaves desconhecidas).

<Accordion title="Configuração da ponte legada (referência histórica)">
...
- `webhook`: URL de Webhook legada obsoleta de fallback (http/https) usada apenas para tarefas armazenadas que ainda têm `notify: true`.
...
Aplica-se apenas a tarefas cron de execução única. Tarefas recorrentes usam tratamento de falhas separado.
...
- `includeSkipped`: conta execuções ignoradas consecutivas para o limite de alerta (padrão: `false`). Execuções ignoradas são rastreadas separadamente e não afetam o backoff de erros de execução.
...
- Quando nem o destino de falha global nem o por tarefa estiver definido, tarefas que já entregam via `announce` usam esse destino primário de anúncio como fallback em caso de falha.
...
Consulte [Tarefas cron](/pt-BR/automation/cron-jobs). Execuções cron isoladas são rastreadas como [tarefas em segundo plano](/pt-BR/automation/tasks).
...
- Matriz de arquivos: mesclada profundamente em ordem (os posteriores substituem os anteriores).
...
- Escritas de propriedade do OpenClaw que alteram apenas uma seção de nível superior apoiada por um include de arquivo único escrevem nesse arquivo incluído. Por exemplo, `plugins install` atualiza `plugins: { $include: "./plugins.json5" }` em `plugins.json5` e deixa `openclaw.json` intacto.
- Includes raiz, matrizes de include e includes com substituições irmãs são somente leitura para escritas de propriedade do OpenClaw; essas escritas falham fechadas em vez de achatar a configuração.
