---
read_when:
    - Você precisa de semântica ou valores padrão exatos de configuração em nível de campo
    - Você está validando blocos de configuração de canal, modelo, Gateway ou ferramenta
summary: Referência de configuração do Gateway para as chaves principais do OpenClaw, valores padrão e links para referências dedicadas dos subsistemas
...
Abrange as principais superfícies de configuração do OpenClaw e aponta para outras referências quando um subsistema tem sua própria documentação mais aprofundada. Catálogos de comandos pertencentes a canais e plugins, bem como ajustes avançados de memória/QMD, ficam em suas próprias páginas em vez desta.

Fonte de verdade no código:

- `openclaw config schema` imprime o JSON Schema ativo usado para validação e Control UI, com metadados de pacotes integrados/plugins/canais mesclados quando disponíveis
- `config.schema.lookup` retorna um nó de esquema com escopo por caminho para ferramentas de detalhamento
- `pnpm config:docs:check` / `pnpm config:docs:gen` validam o hash de linha de base da documentação de configuração em relação à superfície atual do esquema

Caminho de consulta do agente: use a ação de ferramenta `config.schema.lookup` do `gateway` para
obter documentação e restrições exatas em nível de campo antes das edições. Use
...
- [Comandos slash](/pt-BR/tools/slash-commands) para o catálogo atual de comandos integrados + empacotados
- páginas dos canais/plugins proprietários para superfícies de comando específicas de canais
...
Política de ferramentas, alternâncias experimentais, configuração de ferramentas apoiada por provedores e configuração de
...
- `models.pricing.enabled`: controla a inicialização em segundo plano de preços que
  começa depois que os sidecars e canais alcançam o caminho de prontidão do Gateway. Quando `false`,
...
consumidas pelo Pi embarcado e outros adaptadores de runtime. Os comandos `openclaw mcp list`,
...
- `mcp.sessionIdleTtlMs`: TTL de inatividade para runtimes MCP empacotados com escopo de sessão.
  Execuções embarcadas de uso único solicitam limpeza ao final da execução; este TTL é o mecanismo de segurança para
...
- Alterações em `mcp.*` são aplicadas a quente descartando runtimes MCP de sessão em cache.
  ...
  removidas de `mcp.servers` são coletadas imediatamente em vez de esperar pelo TTL de inatividade.
...
- `allowBundled`: lista de permissão opcional apenas para Skills empacotadas (Skills gerenciadas/do espaço de trabalho não são afetadas).
...
- A descoberta aceita plugins nativos do OpenClaw, além de pacotes compatíveis do Codex e pacotes do Claude, incluindo pacotes de layout padrão do Claude sem manifesto.
...
- `bundledDiscovery`: o padrão é `"allowlist"` para novas configurações, então um
  `plugins.allow` não vazio também restringe plugins de provedores empacotados, incluindo provedores de runtime
  de busca na web. O Doctor escreve `"compat"` para configurações de listas de permissão legadas migradas
  a fim de preservar o comportamento existente de provedores empacotados até você optar pelo novo modo.
...
- Plugins de pacote do Claude habilitados também podem contribuir padrões embarcados do Pi a partir de `settings.json`; o OpenClaw aplica esses valores como configurações saneadas de agente, não como patches brutos de configuração do OpenClaw.
...
`commitments` controla memória inferida de acompanhamento: o OpenClaw pode detectar check-ins em turnos de conversa e entregá-los por meio de execuções de Heartbeat.

- `commitments.enabled`: habilita extração oculta por LLM, armazenamento e entrega por Heartbeat para compromissos de acompanhamento inferidos. Padrão: `false`.
- `commitments.maxPerDay`: número máximo de compromissos de acompanhamento inferidos entregues por sessão de agente em um dia contínuo. Padrão: `3`.
...
- `tabCleanup` recupera abas rastreadas do agente primário após tempo ocioso ou quando uma
  ...
  desativar esses modos de limpeza individuais.
...
- Perfis remotos são apenas para anexação (iniciar/parar/redefinir desativados).
...
- `remoteCdpTimeoutMs` e `remoteCdpHandshakeTimeoutMs` se aplicam ao alcance CDP remoto e
  `attachOnly`, além de solicitações de abertura de abas. Perfis gerenciados de loopback
  mantêm os padrões CDP locais.
...
- Perfis `existing-session` podem definir `userDataDir` para apontar para um
  perfil específico de navegador baseado em Chromium, como Brave ou Edge.
- Perfis `existing-session` mantêm os limites atuais de rota do Chrome MCP:
  ações orientadas por snapshot/ref em vez de seleção por seletor CSS, hooks de upload
...
- Perfis `openclaw` gerenciados locais atribuem automaticamente `cdpPort` e `cdpUrl`; apenas
  defina `cdpUrl` explicitamente para CDP remoto.
- Perfis gerenciados locais podem definir `executablePath` para substituir o
  `browser.executablePath` global nesse perfil. Use isso para executar um perfil no
  Chrome e outro no Brave.
- Perfis gerenciados locais usam `browser.localLaunchTimeoutMs` para descoberta HTTP do Chrome CDP
  ...
  inicia com sucesso, mas as verificações de prontidão disputam a inicialização. Ambos os valores devem ser
  inteiros positivos até `120000` ms; valores de configuração inválidos são rejeitados.
- Ordem de autodetecção: navegador padrão se baseado em Chromium → Chrome → Brave → Edge → Chromium → Chrome Canary.
...
- `assistant`: substituição da identidade da UI de controle. Usa a identidade do agente ativo como fallback.
...
- **Autenticação**: exigida por padrão. Binds que não sejam de loopback exigem autenticação do Gateway. Na prática, isso significa um token/senha compartilhado ou um proxy reverso ciente de identidade com `gateway.auth.mode: "trusted-proxy"`. O assistente de onboarding gera um token por padrão.
...
- `gateway.auth.mode: "trusted-proxy"`: delega a autenticação de navegador/usuário a um proxy reverso ciente de identidade e confia em cabeçalhos de identidade vindos de `gateway.trustedProxies` (consulte [Autenticação por proxy confiável](/pt-BR/gateway/trusted-proxy-auth)). Por padrão, esse modo espera uma origem de proxy **não loopback**; proxies reversos de loopback no mesmo host exigem `gateway.auth.trustedProxy.allowLoopback = true` explícito. Chamadores internos no mesmo host podem usar `gateway.auth.password` como fallback direto local; `gateway.auth.token` permanece mutuamente exclusivo com o modo trusted-proxy.
...
- `gateway.auth.rateLimit`: limitador opcional de falhas de autenticação. Aplica-se por IP de cliente e por escopo de autenticação (segredo compartilhado e token de dispositivo são rastreados independentemente). Tentativas bloqueadas retornam `429` + `Retry-After`.
  - No caminho assíncrono da Control UI do Tailscale Serve, tentativas com falha para o mesmo `{scope, clientIp}` são serializadas antes da gravação da falha. Portanto, tentativas incorretas concorrentes do mesmo cliente podem acionar o limitador na segunda requisição, em vez de ambas passarem em paralelo como simples divergências.
  - `gateway.auth.rateLimit.exemptLoopback` usa `true` por padrão; defina `false` quando você intencionalmente quiser que tráfego de localhost também sofra limitação de taxa (para configurações de teste ou implantações estritas com proxy).
- Tentativas de autenticação WS originadas do navegador são sempre limitadas, com isenção de loopback desativada (defesa em profundidade contra força bruta em localhost baseada em navegador).
- Em loopback, esses bloqueios originados do navegador são isolados por valor de `Origin`
...
- `controlUi.allowedOrigins`: lista explícita de permissões de origem do navegador para conexões WebSocket do Gateway. Exigida quando clientes de navegador são esperados a partir de origens que não sejam de loopback.
...
- `controlUi.dangerouslyAllowHostHeaderOriginFallback`: modo perigoso que habilita fallback de origem por cabeçalho Host para implantações que intencionalmente dependem da política de origem do cabeçalho Host.
- `remote.transport`: `ssh` (padrão) ou `direct` (ws/wss). Para `direct`, `remote.url` deve ser `ws://` ou `wss://`.
- `OPENCLAW_ALLOW_INSECURE_PRIVATE_WS=1`: substituição emergencial no ambiente de processo do lado do cliente
  que permite `ws://` em texto claro para IPs confiáveis de rede privada; o padrão permanece somente loopback
  para texto claro. Não há equivalente em `openclaw.json`, e configurações de rede privada do navegador, como
...
- `gateway.push.apns.relay.baseUrl`: URL HTTPS base para o relay APNs externo usado por builds oficiais/TestFlight de iOS depois que publicam registros apoiados por relay no Gateway. Essa URL deve corresponder à URL do relay compilada no build iOS.
- `gateway.push.apns.relay.timeoutMs`: tempo limite de envio do Gateway para o relay, em milissegundos. Padrão: `10000`.
...
- `gateway.handshakeTimeoutMs`: tempo limite do handshake WebSocket pré-autenticação do Gateway, em milissegundos. Padrão: `15000`. `OPENCLAW_HANDSHAKE_TIMEOUT_MS` tem precedência quando definido. Aumente isso em hosts carregados ou de baixa potência nos quais clientes locais conseguem se conectar enquanto o aquecimento da inicialização ainda está estabilizando.
- `gateway.channelHealthCheckMinutes`: intervalo do monitor de saúde do canal, em minutos. Defina `0` para desativar reinicializações do monitor de saúde globalmente. Padrão: `5`.
...
- `gateway.channelMaxRestartsPerHour`: máximo de reinicializações pelo monitor de saúde por canal/conta em uma hora móvel. Padrão: `10`.
- `channels.<provider>.healthMonitor.enabled`: desativação por canal das reinicializações do monitor de saúde, mantendo o monitor global habilitado.
- `channels.<provider>.accounts.<accountId>.healthMonitor.enabled`: substituição por conta para canais com múltiplas contas. Quando definida, tem precedência sobre a substituição no nível do canal.
...
- `gateway.nodes.pairing.autoApproveCidrs`: lista opcional de permissões CIDR/IP para aprovar automaticamente o primeiro emparelhamento de dispositivo de Node sem escopos solicitados. Fica desativada quando não definida. Isso não aprova automaticamente emparelhamento de operador/navegador/Control UI/WebChat e não aprova automaticamente upgrades de função, escopo, metadados ou chave pública.
- `gateway.nodes.allowCommands` / `gateway.nodes.denyCommands`: modelagem global de permissão/negação para comandos de Node declarados após o emparelhamento e a avaliação da lista de permissões da plataforma. Use `allowCommands` para aceitar comandos perigosos de Node, como `camera.snap`, `camera.clip` e `screen.record`; `denyCommands` remove um comando mesmo que um padrão da plataforma ou uma permissão explícita o incluísse de outra forma. Depois que um Node alterar sua lista de comandos declarados, rejeite e aprove novamente o emparelhamento desse dispositivo para que o Gateway armazene o snapshot de comandos atualizado.
...
- Proteção de entrada por URL da Responses:
...
- Cabeçalho opcional de proteção de resposta:
...
- `caPath`: caminho opcional para pacote de CA para verificação de cliente ou cadeias de confiança customizadas.
...
- `POST /hooks/agent` → `{ message, name?, agentId?, sessionKey?, wakeMode?, deliver?, channel?, to?, model?, thinking?, timeoutSeconds? }`
  - `sessionKey` da carga da solicitação só é aceito quando `hooks.allowRequestSessionKey=true` (padrão: `false`).
...
- Serve HTML/CSS/JS editáveis por agentes e A2UI por HTTP na porta do Gateway:
  - `http://<gateway-host>:<gateway.port>/__openclaw__/canvas/`
  - `http://<gateway-host>:<gateway.port>/__openclaw__/a2ui/`
- Somente local: mantenha `gateway.bind: "loopback"` (padrão).
- Vínculos sem loopback: rotas do canvas exigem autenticação do Gateway (token/senha/proxy confiável), igual a outras superfícies HTTP do Gateway.
...
- `minimal` (padrão quando o plugin integrado `bonjour` está habilitado): omite `cliPath` + `sshPort` dos registros TXT.
- `full`: inclui `cliPath` + `sshPort`; a publicidade multicast em LAN ainda exige que o plugin integrado `bonjour` esteja habilitado.
- `off`: suprime a publicidade multicast em LAN sem alterar a habilitação do plugin.
- O plugin integrado `bonjour` inicia automaticamente em hosts macOS e é opcional em Linux, Windows e implantações de Gateway conteinerizadas.
- O hostname usa como padrão o hostname do sistema quando ele é um rótulo DNS válido, usando `openclaw` como fallback. Substitua com `OPENCLAW_MDNS_HOSTNAME`.
...
- Caminhos de provedores file e exec falham de forma fechada quando a verificação de ACL do Windows não está disponível. Defina `allowInsecurePath: true` apenas para caminhos confiáveis que não possam ser verificados.
- O provedor `exec` exige um caminho absoluto de `command` e usa cargas de protocolo em stdin/stdout.
- Por padrão, caminhos de comando por symlink são rejeitados. Defina `allowSymlinkCommand: true` para permitir caminhos de symlink enquanto valida o caminho de destino resolvido.
...
- Referências de segredo são resolvidas no momento da ativação em um snapshot em memória; depois, os caminhos de solicitação leem apenas o snapshot.
...
- `auth-profiles.json` oferece suporte a referências no nível do valor (`keyRef` para `api_key`, `tokenRef` para `token`) para modos de credencial estática.
- Mapas planos legados de `auth-profiles.json`, como `{ "provider": { "apiKey": "..." } }`, não são um formato em tempo de execução; `openclaw doctor --fix` os reescreve para perfis canônicos de chave de API `provider:default` com um backup `.legacy-flat.*.bak`.
- Perfis em modo OAuth (`auth.profiles.<id>.mode = "oauth"`) não oferecem suporte a credenciais de perfil de autenticação apoiadas por SecretRef.
- Credenciais estáticas em tempo de execução vêm de snapshots resolvidos em memória; entradas estáticas legadas de `auth.json` são limpas quando descobertas.
...
- `billingBackoffHours`: recuo base em horas quando um perfil falha devido a erros reais de
  ...
  cair aqui mesmo em respostas `401`/`403`, mas os correspondedores de texto específicos
  do provedor continuam limitados ao provedor ao qual pertencem (por exemplo, OpenRouter
  `Key limit exceeded`). Mensagens HTTP `402` retentáveis de janela de uso ou
...
- `overloadedProfileRotations`: número máximo de rotações de perfis de autenticação do mesmo provedor para erros de sobrecarga antes de alternar para o fallback de modelo (padrão: `1`). Formatos de provedor ocupado, como `ModelNotReadyException`, caem aqui.
...
- `redactSensitive` / `redactPatterns`: mascaramento por melhor esforço para saída do console, logs em arquivo, registros de log OTLP e texto persistido da transcrição da sessão. `redactSensitive: "off"` desativa apenas essa política geral de log/transcrição; superfícies de segurança de UI/ferramentas/diagnóstico ainda ocultam segredos antes da emissão.
...
- `stuckSessionWarnMs`: limite de idade sem progresso em ms para classificar sessões de processamento de longa duração como `session.long_running`, `session.stalled` ou `session.stuck`. Resposta, ferramenta, status, bloco e progresso ACP reiniciam o temporizador; diagnósticos `session.stuck` repetidos fazem recuo enquanto não houver mudanças.
- `stuckSessionAbortMs`: limite de idade sem progresso em ms antes que trabalho ativo travado elegível possa ser drenado por abortamento para recuperação. Quando não definido, OpenClaw usa a janela estendida mais segura de execução embarcada de pelo menos 10 minutos e 5x `stuckSessionWarnMs`.
...
- `otel.traces` / `otel.metrics` / `otel.logs`: habilita exportação de rastreamentos, métricas ou logs.
- `otel.sampleRate`: taxa de amostragem de rastreamento de `0` a `1`.
- `otel.flushIntervalMs`: intervalo periódico de envio de telemetria em ms.
- `otel.captureContent`: captura opcional de conteúdo bruto para atributos de spans OTEL. Desativado por padrão. Booleano `true` captura conteúdo não sistêmico de mensagens/ferramentas; a forma de objeto permite habilitar `inputMessages`, `outputMessages`, `toolInputs`, `toolOutputs` e `systemPrompt` explicitamente.
- `OTEL_SEMCONV_STABILITY_OPT_IN=gen_ai_latest_experimental`: alternância de ambiente para os atributos experimentais mais recentes de provedor de spans GenAI. Por padrão, spans mantêm o atributo legado `gen_ai.system` para compatibilidade; métricas GenAI usam atributos semânticos delimitados.
- `OPENCLAW_OTEL_PRELOADED=1`: alternância de ambiente para hosts que já registraram um SDK OpenTelemetry global. OpenClaw então ignora a inicialização/desligamento do SDK pertencente ao Plugin, mantendo os listeners de diagnóstico ativos.
...
- `cacheTrace.enabled`: registra snapshots de rastreamento de cache para execuções embarcadas (padrão: `false`).
...
- `auto.enabled`: habilita atualização automática em segundo plano para instalações de pacotes (padrão: `false`).
...
- `enabled`: gate global do recurso ACP (padrão: `true`; defina como `false` para ocultar despacho ACP e opções de spawn).
- `dispatch.enabled`: gate independente para despacho de turnos de sessão ACP (padrão: `true`). Defina como `false` para manter comandos ACP disponíveis enquanto bloqueia a execução.
- `backend`: id padrão do backend de runtime ACP (deve corresponder a um Plugin registrado de runtime ACP).
...
- `allowedAgents`: lista de permissão de ids de agentes permitidos para sessões de runtime ACP; vazia significa sem restrição adicional.
...
- `cli.banner.taglineMode` controla o estilo do slogan do banner:
  - `"random"` (padrão): slogans engraçados/sazonais em rotação.
...
As builds atuais não incluem mais a ponte TCP. Nodes se conectam pelo WebSocket do Gateway. As chaves `bridge.*` não fazem mais parte do esquema de configuração (a validação falha até que sejam removidas; `openclaw doctor --fix` pode remover chaves desconhecidas).
...
- `sessionRetention`: por quanto tempo manter sessões concluídas de execuções Cron isoladas antes da remoção de `sessions.json`. Também controla a limpeza de transcrições Cron excluídas arquivadas. Padrão: `24h`; defina como `false` para desativar.
...
- `webhookToken`: token bearer usado para entrega POST do Webhook Cron (`delivery.mode = "webhook"`); se omitido, nenhum cabeçalho de autenticação é enviado.
- `webhook`: URL de Webhook legada de fallback obsoleta (http/https), usada apenas para trabalhos armazenados que ainda têm `notify: true`.
...
Aplica-se apenas a trabalhos Cron de execução única. Trabalhos recorrentes usam tratamento de falhas separado.
...
- `includeSkipped`: conta execuções ignoradas consecutivas para o limite do alerta (padrão: `false`). Execuções ignoradas são rastreadas separadamente e não afetam o backoff de erros de execução.
- `mode`: modo de entrega - `"announce"` envia por meio de uma mensagem de canal; `"webhook"` publica no Webhook configurado.
- `accountId`: conta ou id de canal opcional para limitar o escopo da entrega de alertas.
...
- `delivery.failureDestination` substitui este padrão global por trabalho.
- Quando nenhum destino de falha global nem por trabalho está definido, trabalhos que já fazem entrega via `announce` usam esse destino principal de anúncio como fallback em caso de falha.
...
- Gravações de propriedade do OpenClaw que alteram apenas uma seção de nível superior apoiada por um include de arquivo único são gravadas nesse arquivo incluído. Por exemplo, `plugins install` atualiza `plugins: { $include: "./plugins.json5" }` em `plugins.json5` e deixa `openclaw.json` intacto.
- Includes raiz, arrays de includes e includes com substituições irmãs são somente leitura para gravações de propriedade do OpenClaw; essas gravações falham de forma fechada em vez de achatar a configuração.
