...
read_when:
    ...
    - Auditoria da política de mensagens diretas, da política de grupos ou do controle por menções
...
Todos os canais dão suporte a políticas de DM e políticas de grupo:
...
- `channels.defaults.contextVisibility`: modo padrão de visibilidade de contexto suplementar para todos os canais. Valores: `all` (padrão, inclui todo contexto de citação/thread/histórico), `allowlist` (inclui apenas contexto de remetentes na allowlist), `allowlist_quote` (igual a allowlist, mas mantém contexto explícito de citação/resposta). Substituição por canal: `channels.<channel>.contextVisibility`.
...
- O diretório de autenticação Baileys legado de conta única é migrado pelo `openclaw doctor` para `whatsapp/default`.
...
- `apiRoot` é apenas a raiz da Telegram Bot API. Use `https://api.telegram.org` ou sua raiz auto-hospedada/de proxy, não `https://api.telegram.org/bot<TOKEN>`; `openclaw doctor --fix` remove um sufixo `/bot<TOKEN>` acidental ao final.
...
- `channels.discord.threadBindings` controla o roteamento vinculado a threads do Discord:
  ...
  - `spawnSessions`: alternância para `sessions_spawn({ thread: true })` e criação/vinculação automática de thread em spawn de thread ACP (padrão: `true`)
  - `defaultSpawnContext`: contexto nativo de subagente para spawns vinculados a thread (`"fork"` por padrão)
- Entradas `bindings[]` de nível superior com `type: "acp"` configuram vinculações ACP persistentes para canais e threads (use id de canal/thread em `match.peer.id`). A semântica dos campos é compartilhada em [Agentes ACP](/pt-BR/tools/acp-agents#channel-specific-settings).
- `channels.discord.ui.components.accentColor` define a cor de destaque para contêineres v2 de componentes do Discord.
- `channels.discord.voice` habilita conversas em canais de voz do Discord e substituições opcionais de entrada automática + LLM + TTS. Configurações do Discord somente texto deixam voz desativada por padrão; defina `channels.discord.voice.enabled=true` para habilitar.
- `channels.discord.voice.model` opcionalmente substitui o modelo LLM usado para respostas de canal de voz do Discord.
...
- `channels.discord.streaming` é a chave canônica do modo de stream. Valores legados de `streamMode` e booleanos de `streaming` são migrados automaticamente.
...
- `channels.discord.dangerouslyAllowNameMatching` reabilita a correspondência mutável por nome/tag (modo de compatibilidade break-glass).
...
- `socketMode` repassa o ajuste de transporte do Socket Mode do SDK do Slack para a API pública do receptor Bolt. Use-o apenas ao investigar timeout de ping/pong ou comportamento de websocket obsoleto.
...
- `channels.slack.streaming.mode` é a chave canônica do modo de stream do Slack. `channels.slack.streaming.nativeTransport` controla o transporte de streaming nativo do Slack. Valores legados de `streamMode`, booleanos de `streaming` e `nativeStreaming` são migrados automaticamente.
...
- Streaming nativo do Slack mais o status de thread "is typing..." no estilo assistente do Slack exigem um destino de thread de resposta. DMs de nível superior ficam fora de thread por padrão, então usam `typingReaction` ou entrega normal em vez da prévia no estilo thread.
- `typingReaction` adiciona uma reação temporária à mensagem recebida do Slack enquanto uma resposta está em execução e a remove na conclusão. Use um shortcode de emoji do Slack, como `"hourglass_flowing_sand"`.
...
customizados podem instalar um pacote npm atual com
`openclaw plugins install @openclaw/mattermost`; se o npm relatar que o
pacote de propriedade do OpenClaw está obsoleto, use o Plugin integrado ou um checkout local
...
Modos de chat: `oncall` (responder em @-mention, padrão), `onmessage` (toda mensagem), `onchar` (mensagens que começam com prefixo de gatilho).
...
- `commands.callbackUrl` deve resolver para o ponto de extremidade do Gateway do OpenClaw e ser acessível a partir do servidor Mattermost.
...
- `channels.matrix.autoJoin` usa `off` por padrão, portanto salas convidadas e novos convites no estilo DM são ignorados até que você defina `autoJoin: "allowlist"` com `autoJoinAllowlist` ou `autoJoin: "always"`.
- `channels.matrix.execApprovals`: entrega de aprovações de execução nativas do Matrix e autorização de aprovadores.
...
- Configurações básicas do canal se aplicam a todas as contas, salvo se substituídas por conta.
...
### Bloqueio por menção em conversas em grupo

Mensagens de grupo exigem **menção** por padrão (menção por metadados ou padrões de regex seguros). Aplica-se a WhatsApp, Telegram, Discord, Google Chat e conversas em grupo do iMessage.

Respostas visíveis são controladas separadamente. Salas de grupo/canal usam `messages.groupChat.visibleReplies: "message_tool"` por padrão: o OpenClaw ainda processa o turno, mas respostas finais normais permanecem privadas e a saída visível na sala exige `message(action=send)`. Defina `"automatic"` somente quando quiser o comportamento legado em que respostas normais são publicadas de volta na sala. Para aplicar o mesmo comportamento de resposta visível somente por ferramenta a conversas diretas também, defina `messages.visibleReplies: "message_tool"`; o harness do Codex também usa esse comportamento somente por ferramenta como padrão não definido para conversas diretas.

Se a ferramenta de mensagens estiver indisponível sob a política de ferramentas ativa, o OpenClaw volta para respostas visíveis automáticas em vez de suprimir silenciosamente a resposta. `openclaw doctor` avisa sobre essa incompatibilidade.
...
- **Padrões de texto**: padrões de regex seguros em `agents.list[].groupChat.mentionPatterns`. Padrões inválidos e repetição aninhada insegura são ignorados.
...
`messages.groupChat.historyLimit` define o padrão global. Os canais podem sobrescrevê-lo com `channels.<channel>.historyLimit` (ou por conta). Defina como `0` para desativar.

`messages.visibleReplies` é o padrão global do turno de origem; `messages.groupChat.visibleReplies` o sobrescreve para turnos de origem em grupos/canais. Quando `messages.visibleReplies` não está definido, um harness pode fornecer seu próprio padrão direto/de origem; o harness do Codex usa `message_tool` por padrão. As listas de permissões de canais e o controle por menções ainda decidem se um turno é processado.
...
- Sobrescreva por canal: `channels.discord.commands.native` (booleano ou `"auto"`). `false` limpa comandos registrados anteriormente.
...
- `config: true` ativa `/config` (lê/grava `openclaw.json`). Para clientes `chat.send` do Gateway, gravações persistentes de `/config set|unset` também exigem `operator.admin`; `/config show` somente leitura continua disponível para clientes operadores normais com escopo de gravação.
...
- `ownerDisplay: "hash"` gera hash dos IDs de proprietário no prompt do sistema. Defina `ownerDisplaySecret` para controlar a geração de hash.
...
- Mapa da documentação de comandos:
  - catálogo integrado e incluído no pacote: [Comandos de barra](/pt-BR/tools/slash-commands)
