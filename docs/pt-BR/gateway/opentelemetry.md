---
read_when:
    - Você quer enviar métricas do OpenClaw sobre uso do modelo, fluxo de mensagens ou sessão para um coletor OpenTelemetry
...
x-i18n:
    generated_at: "2026-05-05T05:43:19Z"
    model: gpt-5.5
    provider: openai
...
funciona sem alterações de código. Para logs de arquivos locais e como lê-los, consulte
...
- Exportadores só são anexados quando tanto a superfície de diagnósticos quanto o Plugin estão
  habilitados, então o custo em processo permanece perto de zero por padrão.
...
| `OPENCLAW_OTEL_PRELOADED`                                                                                         | Defina como `1` quando outro preload ou processo host já tiver registrado o SDK OpenTelemetry global. O Plugin então ignora seu próprio ciclo de vida do NodeSDK, mas ainda conecta listeners de diagnóstico e respeita `traces`/`metrics`/`logs`. |

## Privacidade e captura de conteúdo

Conteúdo bruto de modelo/ferramenta **não** é exportado por padrão. Spans carregam
identificadores limitados (canal, provedor, modelo, categoria de erro, ids de solicitação apenas por hash)
...
sua política de retenção estiverem aprovados para texto de prompt, resposta, ferramenta ou
prompt de sistema. Cada subchave é opt-in independentemente:

- `inputMessages` — conteúdo de prompt do usuário.
- `outputMessages` — conteúdo de resposta do modelo.
...
- **Logs:** logs OTLP respeitam `logging.level` (nível de log de arquivo). Eles usam o
  ...
  de alto volume devem preferir amostragem/filtragem no coletor OTLP em vez de amostragem local.
- **Correlação de logs de arquivo:** logs de arquivo JSONL incluem `traceId`,
  `spanId`, `parentSpanId` e `traceFlags` no nível superior quando a chamada de log carrega um
  contexto de trace de diagnóstico válido, o que permite que processadores de log juntem linhas de log locais com
  spans exportados.
- **Correlação de solicitações:** solicitações HTTP do Gateway e frames WebSocket criam um
  escopo de trace de solicitação interno. Logs e eventos de diagnóstico dentro desse escopo
...
- `openclaw.session.stuck` (contador, attrs: `openclaw.state`; emitido apenas para controle de sessões obsoletas sem trabalho ativo)
- `openclaw.session.stuck_age_ms` (histograma, attrs: `openclaw.state`; emitido apenas para controle de sessões obsoletas sem trabalho ativo)
...
atividade de sessão. Uma sessão `processing` não avança em direção a esse limite
...
- `session.stalled`: há trabalho ativo, mas a execução ativa não relatou
  progresso recente. Execuções incorporadas paralisadas permanecem inicialmente apenas em observação e depois
  fazem abort-drain após `diagnostics.stuckSessionAbortMs` sem progresso para que os turnos
  enfileirados atrás da lane possam ser retomados. Quando não definido, o limite de aborto usa por padrão
...
Diagnósticos repetidos de `session.stuck` aplicam backoff enquanto a sessão permanece
...
- `exec.process.completed` — resultado terminal, duração, alvo, modo, código de saída
  e tipo de falha. O texto do comando e os diretórios de trabalho não são
  incluídos.

## Sem um exportador

Você pode manter os eventos de diagnóstico disponíveis para plugins ou destinos personalizados sem
...
- [Logging](/pt-BR/logging) — logs em arquivo, saída do console, tailing pela CLI e a aba Logs da Control UI
...
- [Referência de configuração](/pt-BR/gateway/configuration-reference#diagnostics) — referência completa do campo `diagnostics.*`
