...
read_when:
    ...
    - Implementação das configurações do navegador + ciclo de vida no aplicativo para macOS
summary: Serviço integrado de controle do navegador + comandos de ação
title: Navegador (gerenciado pelo OpenClaw)
x-i18n:
    generated_at: "2026-05-06T09:15:27Z"
    model: gpt-5.5
    provider: openai
...
- Uma skill `browser-automation` incluída que ensina aos agentes o loop de
  recuperação de snapshot, aba estável, referência obsoleta e bloqueador manual
  quando o plugin do navegador está habilitado.
- Suporte opcional a múltiplos perfis (`openclaw`, `work`, `remote`, ...).

Este navegador **não** é seu navegador do dia a dia. Ele é uma superfície segura e isolada para
...
é aplicada depois da filtragem de perfil.

O Plugin de navegador fornece dois níveis de orientação para agentes:

- A descrição da ferramenta `browser` contém o contrato compacto sempre ativo: escolha
  o perfil certo, mantenha as refs na mesma aba, use `tabId`/rótulos para direcionamento
  de abas e carregue a skill de navegador para trabalho em várias etapas.
- A skill `browser-automation` incluída contém o loop operacional mais longo:
  verificar status/abas primeiro, rotular abas da tarefa, fazer snapshot antes de agir, fazer novo snapshot
  depois de mudanças na UI, recuperar refs obsoletas uma vez e relatar bloqueadores de login/2FA/captcha ou
  câmera/microfone como ação manual em vez de adivinhar.

Skills incluídas em Plugins aparecem na lista de Skills disponíveis do agente quando o
...
Se `openclaw browser` for desconhecido após uma atualização, `browser.request` estiver ausente, ou o agente relatar que a ferramenta de navegador não está disponível, a causa usual é uma lista `plugins.allow` que omite `browser` e não existe nenhum bloco raiz de configuração `browser`. Adicione-o:
...
Um bloco raiz `browser` explícito, por exemplo `browser.enabled=true` ou `browser.profiles.<name>`, ativa o Plugin de navegador incluído mesmo sob um `plugins.allow` restritivo, igual ao comportamento de configuração de canais. `plugins.entries.browser.enabled=true` e `tools.alsoAllow: ["browser"]` não substituem, por si só, a participação na allowlist. Remover `plugins.allow` totalmente também restaura o padrão.

## Perfis: `openclaw` vs `user`

- `openclaw`: navegador gerenciado e isolado (nenhuma extensão necessária).
- `user`: perfil integrado de conexão do Chrome MCP para sua sessão **real do Chrome com login**.
...
- Perfis `openclaw` locais atribuem automaticamente `cdpPort`/`cdpUrl`; defina esses valores somente para CDP remoto. `cdpUrl` usa como padrão a porta CDP local gerenciada quando não definido.
- `remoteCdpTimeoutMs` se aplica a verificações de acessibilidade HTTP de CDP remotas e `attachOnly`
  e a solicitações HTTP de abertura de abas; `remoteCdpHandshakeTimeoutMs` se aplica aos
  handshakes CDP WebSocket delas.
...
- Falhas repetidas de inicialização/prontidão do Chrome gerenciado acionam um circuit breaker por
  perfil. Após várias falhas consecutivas, o OpenClaw pausa brevemente novas tentativas de inicialização
  em vez de gerar o Chromium em cada chamada da ferramenta de navegador. Corrija
  o problema de inicialização, desabilite o navegador se ele não for necessário ou reinicie o
  Gateway após o reparo.
- `actionTimeoutMs` é o orçamento padrão para solicitações `act` do navegador quando o chamador não passa `timeoutMs`. O transporte do cliente adiciona uma pequena janela de folga para que esperas longas possam terminar em vez de atingir timeout na fronteira HTTP.
- `tabCleanup` é uma limpeza de melhor esforço para abas abertas por sessões de navegador do agente primário. A limpeza de ciclo de vida de subagente, cron e ACP ainda fecha suas abas rastreadas explícitas ao final da sessão; sessões primárias mantêm abas ativas reutilizáveis e depois fecham abas rastreadas ociosas ou excedentes em segundo plano.

</Accordion>

<Accordion title="SSRF policy">

- A navegação do navegador e a abertura de abas são protegidas contra SSRF antes da navegação e verificadas novamente, em melhor esforço, na URL final `http(s)` depois.
- No modo SSRF estrito, a descoberta de endpoint CDP remoto e sondagens `/json/version` (`cdpUrl`) também são verificadas.
- Variáveis de ambiente Gateway/provedor `HTTP_PROXY`, `HTTPS_PROXY`, `ALL_PROXY` e `NO_PROXY` não aplicam proxy automaticamente ao navegador gerenciado pelo OpenClaw. O Chrome gerenciado inicia direto por padrão para que as configurações de proxy do provedor não enfraqueçam as verificações SSRF do navegador.
...
- `browser.ssrfPolicy.dangerouslyAllowPrivateNetwork` fica desativado por padrão; habilite somente quando o acesso do navegador à rede privada for intencionalmente confiável.
- `browser.ssrfPolicy.allowPrivateNetwork` continua aceito como um alias legado.
...
- `POST /start?headless=true` e `openclaw browser start --headless` solicitam uma
  ...
  `browser.headless` nem a configuração do perfil. Perfis de sessão existente, somente anexação e
...
- Em hosts Linux sem `DISPLAY` ou `WAYLAND_DISPLAY`, perfis gerenciados locais
  assumem headless automaticamente quando nem o ambiente nem a configuração de
  perfil/global escolhem explicitamente o modo com interface. `openclaw browser status --json`
...
- `OPENCLAW_BROWSER_HEADLESS=1` força inicializações gerenciadas locais em modo headless para o
  ...
  uma solicitação explícita `start --headless` ainda vence para essa inicialização única.
...
- `color` (nível superior e por perfil) aplica uma tonalidade à UI do navegador para que você possa ver qual perfil está ativo.
- O perfil padrão é `openclaw` (standalone gerenciado). Use `defaultProfile: "user"` para optar pelo navegador do usuário autenticado.
...
O comportamento de parada difere conforme o modo do perfil:

- perfis gerenciados locais: `openclaw browser stop` para o processo do navegador que
  o OpenClaw iniciou
- perfis somente anexação e CDP remoto: `openclaw browser stop` fecha a sessão de
...
- Os perfis vêm da própria configuração `browser.profiles` do Node (igual à local).
...
defina `EXTERNAL` do Browserless para essa mesma base WebSocket pública-para-OpenClaw, como
...
- **Raízes WebSocket simples** - `ws://host[:port]` ou `wss://host[:port]` sem
  ...
  recua para um handshake WebSocket direto na raiz simples. Se o endpoint
  WebSocket anunciado rejeitar o handshake CDP, mas a raiz simples configurada
  aceitá-lo, o OpenClaw também recua para essa raiz. Isso permite que um `ws://` simples
  apontado para um Chrome local ainda se conecte, já que o Chrome só aceita upgrades WebSocket
...
navegadores headless com resolução de CAPTCHA integrada, modo stealth e proxies residenciais.
...
- [Cadastre-se](https://www.browserbase.com/sign-up) e copie sua **chave de API**
  do [painel de Visão geral](https://www.browserbase.com/overview).
- Substitua `<BROWSERBASE_API_KEY>` pela sua chave de API Browserbase real.
- O Browserbase cria automaticamente uma sessão de navegador ao conectar por WebSocket, então nenhuma
...
- O controle do navegador é apenas via loopback; o acesso flui pela autenticação do Gateway ou pelo pareamento de Node.
- A API HTTP autônoma do navegador de loopback usa **apenas autenticação por segredo compartilhado**:
...
- Cabeçalhos de identidade do Tailscale Serve e `gateway.auth.mode: "trusted-proxy"` **não** autenticam esta API autônoma do navegador de loopback.
...
- O perfil `user` é integrado para anexar uma sessão existente do Chrome MCP.
- Perfis de sessão existente são opcionais além de `user`; crie-os com `--driver existing-session`.
- Portas CDP locais são alocadas de **18800 a 18899** por padrão.
...
- o navegador exibiu o prompt de consentimento de anexação e você o aceitou
- `openclaw doctor` migra configurações antigas de navegador baseadas em plugin e verifica se
  o Chrome está instalado localmente para perfis de conexão automática padrão, mas não pode
  habilitar a depuração remota do lado do navegador para você

Uso por agente:

- Use `profile="user"` quando precisar do estado do navegador conectado do usuário.
- Se você usar um perfil personalizado de sessão existente, passe esse nome de perfil explícito.
- Escolha este modo somente quando o usuário estiver no computador para aprovar o
  prompt de anexação.
- o Gateway ou o host de Node pode gerar `npx chrome-devtools-mcp@latest --autoConnect`

Observações:

- Este caminho tem risco maior que o perfil isolado `openclaw` porque pode
...
- Sessão existente pode se anexar no host selecionado ou por meio de um Node de
...
`userDataDir` é ignorado para a inicialização do Chrome MCP, já que o Chrome MCP se anexa ao
...
Comparados ao perfil gerenciado `openclaw`, os drivers de sessão existente são mais restritos:

- **Capturas de tela** - capturas de página e capturas de elemento `--ref` funcionam; seletores CSS `--element` não. `--full-page` não pode ser combinado com `--ref` ou `--element`. O Playwright não é necessário para capturas de tela de página ou de elementos baseadas em ref.
...
- **Espera / upload / diálogo** - `wait --url` oferece suporte a padrões exatos, de substring e glob; `wait --load networkidle` não tem suporte. Ganchos de upload exigem `ref` ou `inputRef`, um arquivo por vez, sem `element` CSS. Ganchos de diálogo não oferecem suporte a substituições de timeout.
- **Recursos apenas gerenciados** - ações em lote, exportação de PDF, interceptação de download e `responsebody` ainda exigem o caminho de navegador gerenciado.
...
Para scripts e depuração, o Gateway expõe uma pequena **API HTTP de controle apenas via loopback**,
além de uma CLI `openclaw browser` correspondente (snapshots, refs, reforços de espera,
...
Para configurações WSL2 Gateway + Windows Chrome em hosts separados, consulte
...
- Se `start` falhar com `not reachable after start`, solucione a prontidão CDP primeiro.
- Se `start` tiver sucesso, mas `tabs` falhar, o plano de controle ainda não está saudável. Trate isso como um problema de alcançabilidade CDP, não como um problema de navegação de página.
...
- A configuração do navegador usa como padrão um objeto de política SSRF fail-closed mesmo quando você não configura `browser.ssrfPolicy`.
- Para o perfil gerenciado local de loopback `openclaw`, as verificações de integridade CDP ignoram intencionalmente a aplicação de alcançabilidade SSRF do navegador para o próprio plano de controle local do OpenClaw.
...
- Prefira exceções estreitas de host, como `hostnameAllowlist` ou `allowedHostnames`, em vez de amplo acesso à rede privada.
...
- `browser` aceita:
  ...
  - Se um node compatível com navegador estiver conectado, a ferramenta pode encaminhar automaticamente para ele, a menos que você fixe `target="host"` ou `target="node"`.
...
- [Sandboxing](/pt-BR/gateway/sandboxing) - controle de navegador em ambientes com sandbox
- [Segurança](/pt-BR/gateway/security) - riscos e reforço do controle de navegador
