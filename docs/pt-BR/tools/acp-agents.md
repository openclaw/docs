...
read_when:
    ...
    - Solução de problemas do backend ACP, da integração de plugins ou da entrega de conclusões
...
title: Agentes ACP
x-i18n:
    generated_at: "2026-06-27T18:13:21Z"
    model: gpt-5.5
...
plugin de servidor de app nativo do Codex é responsável pelos controles `/codex ...` e pelo runtime
embutido padrão `openai/gpt-*` para turnos de agente; o ACP é responsável pelos
...
| Reutilizar uma CLI de IA local como modelo fallback somente texto                                | [Backends de CLI](/pt-BR/gateway/cli-backends) | Não é ACP. Sem ferramentas do OpenClaw, sem controles ACP, sem runtime de harness                                                                                                            |
...
Checkouts de código-fonte podem usar o plugin de workspace local `extensions/acpx` depois de
`pnpm install`. Execute `/acp doctor` para uma verificação de prontidão.

O OpenClaw só ensina os agentes sobre criação ACP quando o ACP é **realmente
utilizável**: o ACP deve estar habilitado, o despacho não deve estar desabilitado, a sessão
...
<AccordionGroup>
  <Accordion title="Pegadinhas da primeira execução">
    ...
    - Codex ACP roda com um `CODEX_HOME` isolado; o OpenClaw copia entradas de projeto confiáveis e configuração segura de roteamento de modelo/provedor da configuração Codex do host, enquanto autenticação, notificações e hooks permanecem na configuração do host.
    ...
    - Se o host não tiver npm ou acesso à rede, as buscas de adaptadores na primeira execução falharão até que os caches sejam pré-aquecidos ou o adaptador seja instalado de outra forma.

  </Accordion>
  <Accordion title="Pré-requisitos de runtime">
    ACP inicia um processo real de harness externo. O OpenClaw é responsável por roteamento,
    ...
    - O modo de permissão corresponde ao trabalho. Sessões não interativas não conseguem clicar em prompts de permissão nativos, então execuções de codificação com muita escrita/execução geralmente precisam de um perfil de permissão ACPX que possa prosseguir sem interface.
...
| `cursor`      | ACP da CLI do Cursor (`cursor-agent acp`)      | Substitua o comando acpx se uma instalação local expuser um entrypoint ACP diferente. |
...
<AccordionGroup>
  <Accordion title="Detalhes do ciclo de vida">
    - A criação cria ou retoma uma sessão de runtime ACP, registra metadados ACP no armazenamento de sessões do OpenClaw e pode criar uma tarefa em segundo plano quando a execução é de propriedade do pai.
    - Sessões ACP de propriedade do pai são tratadas como trabalho em segundo plano mesmo quando a sessão de runtime é persistente; a conclusão e a entrega entre superfícies passam pelo notificador da tarefa pai em vez de agir como uma sessão de chat normal voltada ao usuário.
    - A manutenção de tarefas fecha sessões ACP one-shot terminais ou órfãs de propriedade do pai. Sessões ACP persistentes são preservadas enquanto um vínculo de conversa ativo permanece; sessões persistentes obsoletas sem vínculo ativo são fechadas para que não possam ser retomadas silenciosamente depois que a tarefa proprietária terminar ou seu registro de tarefa desaparecer.
    ...
    - Workers de runtime ociosos estão qualificados para limpeza após `acp.runtime.ttlMinutes`; os metadados de sessão armazenados permanecem disponíveis para `/acp sessions`.

  </Accordion>
  <Accordion title="Regras de roteamento do Codex nativo">
    Gatilhos em linguagem natural que devem rotear para o **plugin Codex
    nativo** quando ele está habilitado:

    - "Vincule este canal do Discord ao Codex."
    - "Anexe este chat à thread Codex `<id>`."
    - "Mostre as threads do Codex e então vincule esta."

    A vinculação de conversas nativa do Codex é o caminho padrão de controle de chat.
  ...
  <Accordion title="Resumo de seleção de modelo / provedor / runtime">
    - refs de modelo Codex legadas - rota de modelo Codex OAuth/assinatura legada reparada pelo doctor.
    - `openai/*` - runtime incorporado nativo do app-server Codex para turnos de agente OpenAI.
  ...
  <Accordion title="Gatilhos de linguagem natural para roteamento ACP">
    ...
    - "Execute Codex por ACP em uma thread em segundo plano."
    ...
    threads ACP, mas não oculta nem bloqueia chamadas explícitas
...
Use ACP quando quiser um runtime de harness externo. Use o **app-server nativo
do Codex** para vinculação/controle de conversa do Codex quando o Plugin `codex`
estiver habilitado. Use **subagentes** quando quiser execuções delegadas nativas
do OpenClaw.
...
Para Claude Code por ACP, a stack é:
...
Backends CLI são runtimes locais de fallback separados, somente texto - consulte
...
/acp spawn codex --bind here --cwd /workspace/repo       # mesmo vínculo de chat, Codex roda em /workspace/repo
```

<AccordionGroup>
  <Accordion title="Regras de vinculação e exclusividade">
    - `--bind here` e `--thread ...` são mutuamente exclusivos.
    - `--bind here` funciona somente em canais que anunciam vinculação da conversa atual; caso contrário, o OpenClaw retorna uma mensagem clara de não compatível. Vínculos persistem entre reinicializações do gateway.
    - No Discord, `spawnSessions` controla a criação de thread filha para `--thread auto|here` - não `--bind here`.
    - Se você gerar para um agente ACP diferente sem `--cwd`, o OpenClaw herda o workspace do **agente de destino** por padrão. Caminhos herdados ausentes (`ENOENT`/`ENOTDIR`) voltam para o padrão do backend; outros erros de acesso (por exemplo, `EACCES`) aparecem como erros de spawn.
  ...
  <Accordion title="Sessões vinculadas a thread">
    ...
    - Unfocus/fechamento/arquivamento/tempo limite de inatividade ou expiração por idade máxima remove o vínculo.
    ...
    de não compatível/indisponível.
...
- **Canal/DM do Slack:** `match.channel="slack"` + `match.peer.id="<channelId|channel:<channelId>|#<channelId>|userId|user:<userId>|slack:<userId>|<@userId>>"`. Prefira ids estáveis do Slack; vínculos de canal também correspondem a respostas dentro das threads desse canal.
...
<ParamField path="bindings[].acp.backend" type="string">
  Sobrescrita de backend opcional.
</ParamField>
...
- Bindings ACP configurados são donos de sua rota de sessão. A distribuição fan-out de transmissão do canal não substitui a sessão ACP configurada para um binding correspondente.
- Em conversas vinculadas, `/new` e `/reset` redefinem a mesma chave de sessão ACP no lugar.
- Bindings temporários de runtime (por exemplo, criados por fluxos de foco em thread) ainda são aplicados quando presentes.
...
- Caminhos de workspace herdados ausentes recorrem ao cwd padrão do backend; falhas de acesso não ausentes aparecem como erros de inicialização.
...
<ParamField path="thread" type="boolean" default="false">
  Solicita fluxo de binding de thread onde houver suporte.
</ParamField>
<ParamField path="mode" type='"run" | "session"' default="run">
  `"run"` é de execução única; `"session"` é persistente. Se `thread: true` e
  `mode` for omitido, o OpenClaw pode usar por padrão o comportamento persistente conforme
  o caminho de runtime. `mode: "session"` exige `thread: true`.
</ParamField>
<ParamField path="cwd" type="string">
  Diretório de trabalho solicitado para o runtime (validado pela política de
  backend/runtime). Se omitido, a inicialização ACP herda o workspace do agente
  de destino quando configurado; caminhos herdados ausentes recorrem aos
  padrões do backend, enquanto erros reais de acesso são retornados.
</ParamField>
...
<ParamField path="streamTo" type='"parent"'>
  ...
  (`<sessionId>.acp-stream.jsonl`) que você pode acompanhar para ver o histórico completo de repasse.
  Streams de progresso do pai mostram comentários do assistente e progresso de status ACP por
  padrão, a menos que `streaming.progress.commentary=false`. O Discord também usa por padrão
  prévias do pai em modo de progresso quando nenhum modo de stream está configurado. O progresso de
...
seu limite padrão de turno filho. A ferramenta não aceita substituições de timeout
por chamada.

<ParamField path="model" type="string">
  Substituição explícita de modelo para a sessão filha ACP. Inicializações ACP do Codex
  normalizam refs da OpenAI, como `openai/gpt-5.4`, para a configuração de inicialização
  do Codex ACP antes de `session/new`; formas slash, como `openai/gpt-5.4/high`,
  também definem o esforço de raciocínio do Codex ACP.
  Quando omitido, `sessions_spawn({ runtime: "acp" })` usa os padrões existentes
  de modelo de subagente (`agents.defaults.subagents.model` ou
  `agents.list[].subagents.model`) quando configurados; caso contrário, permite que o
  harness ACP use seu próprio modelo padrão.
  Outros harnesses devem anunciar ACP `models` e dar suporte a
...
<ParamField path="thinking" type="string">
  Esforço explícito de pensamento/raciocínio. Para o Codex ACP, `minimal` mapeia para
  ...
  Quando omitidas, inicializações ACP usam os padrões existentes de pensamento de subagente e
  `agents.defaults.models["provider/model"].params.thinking` por modelo
  para o modelo selecionado.
</ParamField>

## Modos de binding e thread de inicialização
...
<Tabs>
  <Tab title="--bind here|off">
    ...
    | `off`  | Não cria um binding para a conversa atual.                                  |

    Observações:

    - `--bind here` é o caminho de operador mais simples para "fazer este canal ou chat ser apoiado pelo Codex."
    - `--bind here` não cria uma thread filha.
    - `--bind here` só está disponível em canais que expõem suporte a binding da conversa atual.
  ...
  <Tab title="--thread auto|here|off">
    ...
    | `off`  | Sem binding. A sessão começa desvinculada.                                                                 |

    Observações:

    - Em superfícies de binding sem thread, o comportamento padrão é efetivamente `off`.
...
<AccordionGroup>
  <Accordion title="Sessões ACP interativas">
    Sessões interativas foram feitas para continuar conversando em uma superfície de chat
    visível:
    ...
    - Entradas de transcrição ACP usam o texto de gatilho visível ao usuário ou o prompt simples de conclusão. Metadados de evento internos permanecem estruturados no OpenClaw quando possível e não são tratados como conteúdo de chat escrito pelo usuário.
  ...
  <Accordion title="Sessões ACP de execução única pertencentes ao pai">
    ...
    - Turnos filhos executam na mesma faixa em segundo plano usada por inicializações de subagentes nativos, então um harness ACP lento não bloqueia trabalho não relacionado da sessão principal.
    - A conclusão é reportada de volta pelo caminho de anúncio de conclusão de tarefa. O OpenClaw converte metadados internos de conclusão em um prompt ACP simples antes de enviá-los a um harness externo, para que harnesses não vejam marcadores de contexto de runtime exclusivos do OpenClaw.
  ...
  <Accordion title="sessions_send e entrega A2A">
    `sessions_send` pode mirar outra sessão após a inicialização. Para sessões pares
    ...
    ver e enviar mensagem para um destino ACP, por exemplo sob configurações amplas de
  ...
  <Accordion title="Retomar uma sessão existente">
    ...
    - Transfira uma sessão do Codex do seu laptop para o seu telefone - diga ao seu agente para continuar de onde você parou.
    - Continue uma sessão de codificação que você iniciou interativamente na CLI, agora sem interface por meio do seu agente.
    - Retome um trabalho que foi interrompido por uma reinicialização do Gateway ou por tempo limite de inatividade.
    ...
    - `resumeSessionId` é um id de retomada ACP/harness local do host, não uma chave de sessão de canal do OpenClaw; o OpenClaw ainda verifica a política de spawn ACP e a política do agente de destino antes do despacho, enquanto o backend ACP ou o harness possui a autorização para carregar esse id upstream.
  ...
  <Accordion title="Teste smoke pós-implantação">
    Após uma implantação do Gateway, execute uma verificação ponta a ponta
    ...
    4. Verifique `accepted=yes`, um `childSessionKey` real e nenhum erro do validador.
    5. Limpe a sessão temporária da ponte.

    Mantenha o gate em `mode: "run"` e pule `streamTo: "parent"` -
    caminhos de `mode: "session"` vinculados a thread e de retransmissão
    de stream são passes de integração mais ricos e separados.
...
As sessões ACP atualmente rodam no runtime do host, **não** dentro da
sandbox do OpenClaw.
...
- O OpenClaw ainda impõe gates de recurso ACP, agentes permitidos, propriedade de sessão, vinculações de canal e política de entrega do Gateway.
- Use `runtime: "subagent"` para trabalho nativo do OpenClaw imposto por sandbox.
...
| `/acp steer`         | Envia instrução de direção à sessão em execução.         | `/acp steer --session support inbox prioritize failing tests` |
...
| `/acp timeout`       | Define tempo limite de runtime (segundos).               | `/acp timeout 120`                                            |
| `/acp model`         | Define substituição de modelo de runtime.                | `/acp model anthropic/claude-opus-4-6`                        |
...
`/acp status` mostra as opções efetivas de runtime, além de
identificadores de sessão em nível de runtime e de backend. Erros de
controle sem suporte aparecem claramente quando um backend não tem um
recurso. `/acp sessions` lê o store da sessão vinculada atual ou
solicitante; tokens de destino (`session-key`, `session-id` ou
`session-label`) são resolvidos por meio da descoberta de sessões do
Gateway, incluindo raízes `session.store` personalizadas por agente.
...
| ---------------------------- | ------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `/acp model <id>`            | chave de configuração de runtime `model` | Para ACP do Codex, o OpenClaw normaliza `openai/<model>` para o id de modelo do adaptador e mapeia sufixos de raciocínio com barra, como `openai/gpt-5.4/high`, para `reasoning_effort`.                    |
| `/acp set thinking <level>`  | opção canônica `thinking`            | O OpenClaw envia o equivalente anunciado pelo backend quando presente, preferindo `thinking`, depois `effort`, `reasoning_effort` ou `thought_level`. Para ACP do Codex, o adaptador mapeia valores para `reasoning_effort`. |
...
| `ACP dispatch is disabled by policy (acp.dispatch.enabled=false)`           | Despacho automático de mensagens de thread normal desabilitado.                                                               | Defina `acp.dispatch.enabled=true` para retomar o roteamento automático de threads; chamadas explícitas a `sessions_spawn({ runtime: "acp" })` ainda funcionam.                                      |
...
| `/acp doctor` reports backend not ready right after startup                 | O Plugin de backend está ausente, desabilitado, bloqueado por política de permissão/negação, ou seu executável configurado está indisponível.        | Instale/habilite o Plugin de backend, execute `/acp doctor` novamente e inspecione o erro de instalação ou política do backend se ele continuar não saudável.                                           |
| Harness command not found                                                   | A CLI do adaptador não está instalada, o Plugin externo está ausente ou a busca de primeira execução do `npx` falhou para um adaptador que não é Codex. | Execute `/acp doctor`, instale/pré-aqueça o adaptador no host do Gateway ou configure explicitamente o comando do agente acpx.                                                      |
...
| Vendor auth error from the harness                                          | O OpenClaw está saudável, mas a CLI/provedor de destino não está conectado.                                                     | Faça login ou forneça a chave de provedor necessária no ambiente do host do Gateway.                                                                                             |
...
| `--bind here requires running /acp spawn inside an active ... conversation` | `--bind here` usado sem uma conversa vinculável ativa.                                                            | Vá para o chat/canal de destino e tente novamente, ou use uma geração sem vínculo.                                                                                                         |
...
| `Only <user-id> can rebind this channel/conversation/thread.`               | Outro usuário é o proprietário do destino de vínculo ativo.                                                                           | Revincule como proprietário ou use uma conversa ou thread diferente.                                                                                                               |
...
| `Sandboxed sessions cannot spawn ACP sessions ...`                          | O runtime ACP fica no lado do host; a sessão solicitante está em sandbox.                                                              | Use `runtime="subagent"` de sessões em sandbox, ou execute a geração ACP de uma sessão sem sandbox.                                                                         |
| `sessions_spawn sandbox="require" is unsupported for runtime="acp" ...`     | `sandbox="require"` solicitado para o runtime ACP.                                                                         | Use `runtime="subagent"` para sandbox obrigatório, ou use ACP com `sandbox="inherit"` de uma sessão sem sandbox.                                                      |
...
| Missing ACP metadata for bound session                                      | Metadados de sessão ACP obsoletos/excluídos.                                                                                    | Recrie com `/acp spawn` e depois revincule/foque a thread.                                                                                                                    |
...
| ACP session stalls indefinitely after completing work                       | O processo do harness terminou, mas a sessão ACP não relatou conclusão.                                                    | Atualize o OpenClaw; a limpeza atual do acpx recolhe processos obsoletos de wrapper e adaptador de propriedade do OpenClaw no fechamento e na inicialização do Gateway.                                             |
| Harness sees `<<<BEGIN_OPENCLAW_INTERNAL_CONTEXT>>>`                        | Envelope de evento interno vazou pelo limite ACP.                                                                | Atualize o OpenClaw e execute novamente o fluxo de conclusão; harnesses externos devem receber apenas prompts de conclusão simples.                                                          |

<Note>
`Command blocked by PreToolUse hook: Native hook relay unavailable` pertence ao
relay de hook nativo do Codex, não ao ACP/acpx. Em um chat Codex vinculado, inicie uma nova
...
repetir `/new`. Consulte [Solução de problemas do harness do Codex](/pt-BR/plugins/codex-harness#troubleshooting).
</Note>
