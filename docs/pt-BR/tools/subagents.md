...
title: Subagentes
x-i18n:
    generated_at: "2026-04-25T13:58:18Z"
    model: gpt-5.4
    provider: openai
...
Esses comandos funcionam em canais que oferecem suporte a vínculos persistentes com thread. Consulte **Canais com suporte a threads** abaixo.
...
Use `sessions_history` para uma visualização limitada e filtrada por segurança da recuperação; inspecione o
caminho da transcrição em disco quando precisar da transcrição completa bruta.

### Comportamento de spawn

`/subagents spawn` inicia um subagente em segundo plano como um comando do usuário, e não como um relay interno, e envia uma atualização final de conclusão de volta ao chat solicitante quando a execução termina.
...
- A entrega da conclusão é baseada em push. Depois de iniciado, não faça polling de `/subagents list`,
  ...
  conclusão; inspecione o status apenas sob demanda para depuração ou intervenção.
- Na conclusão, o OpenClaw fecha por best effort abas/processos do navegador rastreados abertos por essa sessão de subagente antes que o fluxo de limpeza do anúncio continue.
- Para spawns manuais, a entrega é resiliente:
  - O OpenClaw tenta primeiro a entrega direta `agent` com uma chave de idempotência estável.
  - Se a entrega direta falhar, ele faz fallback para roteamento por fila.
  - Se o roteamento por fila ainda não estiver disponível, o anúncio é tentado novamente com um pequeno backoff exponencial antes da desistência final.
- A entrega da conclusão mantém a rota do solicitante resolvida:
...
- O repasse da conclusão para a sessão solicitante é um contexto interno gerado em tempo de execução (não texto escrito pelo usuário) e inclui:
  - `Result` (texto mais recente visível de resposta `assistant`; caso contrário, texto mais recente saneado de tool/toolResult; execuções com falha terminal não reutilizam texto de resposta capturado)
  - `Status` (`completed successfully` / `failed` / `timed out` / `unknown`)
  - estatísticas compactas de runtime/tokens
  - uma instrução de entrega informando ao agente solicitante que reescreva em voz normal de assistant (não encaminhe metadados internos brutos)
...
- Para sessões do harness ACP (Codex, Claude Code, Gemini CLI), use `sessions_spawn` com `runtime: "acp"` e consulte [Agentes ACP](/pt-BR/tools/acp-agents), especialmente o [modelo de entrega ACP](/pt-BR/tools/acp-agents#delivery-model) ao depurar conclusões ou loops de agente para agente.
...
Observação sobre custo: cada subagente tem seu **próprio** contexto e uso de tokens por padrão. Para tarefas pesadas ou
...
Subagentes nativos começam isolados, a menos que o chamador solicite explicitamente um fork da
transcrição atual.

| Mode       | When to use it                                                                                                                         | Behavior                                                                          |
| ---------- | -------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------- |
| `isolated` | Pesquisa nova, implementação independente, trabalho com ferramenta lenta ou qualquer coisa que possa ser descrita no texto da tarefa   | Cria uma transcrição filha limpa. Este é o padrão e mantém o uso de tokens menor. |
| `fork`     | Trabalho que depende da conversa atual, de resultados anteriores de ferramentas ou de instruções detalhadas já presentes na transcrição do solicitante | Ramifica a transcrição do solicitante para a sessão filha antes de ela começar. |
...
- Modelo padrão: herda do chamador, a menos que você defina `agents.defaults.subagents.model` (ou `agents.list[].subagents.model` por agente); um `sessions_spawn.model` explícito ainda tem precedência.
- Thinking padrão: herda do chamador, a menos que você defina `agents.defaults.subagents.thinking` (ou `agents.list[].subagents.thinking` por agente); um `sessions_spawn.thinking` explícito ainda tem precedência.
- Timeout padrão da execução: se `sessions_spawn.runTimeoutSeconds` for omitido, o OpenClaw usará `agents.defaults.subagents.runTimeoutSeconds` quando definido; caso contrário, fará fallback para `0` (sem timeout).
...
- `thread?` (padrão `false`; quando `true`, solicita vínculo com thread do canal para esta sessão de subagente)
- `mode?` (`run|session`)
  - o padrão é `run`
...
- `context?` (`isolated|fork`, padrão `isolated`; apenas subagentes nativos)
  - `isolated` cria uma transcrição filha limpa e é o padrão.
  - `fork` ramifica a transcrição atual do solicitante para a sessão filha para que o filho comece com o mesmo contexto de conversa.
  - Use `fork` apenas quando o filho precisar da transcrição atual. Para trabalho com escopo definido, omita `context`.
...
Quando vínculos com thread estão habilitados para um canal, um subagente pode permanecer vinculado a uma thread para que mensagens posteriores do usuário nessa thread continuem sendo roteadas para a mesma sessão de subagente.
...
4. Use `/session idle` para inspecionar/atualizar a remoção automática de foco por inatividade e `/session max-age` para controlar o limite rígido.
...
- Substituições por canal e chaves de vínculo automático no spawn são específicas do adaptador. Consulte **Canais com suporte a threads** acima.
...
- `agents.defaults.subagents.allowAgents`: lista de permissões padrão de agente de destino usada quando o agente solicitante não define seu próprio `subagents.allowAgents`.
...
- `cleanup: "delete"` arquiva imediatamente após o anúncio (ainda mantém a transcrição por meio da renomeação).
- O arquivamento automático é best effort; timers pendentes são perdidos se o gateway reiniciar.
...
- A limpeza do navegador é separada da limpeza de arquivamento: abas/processos de navegador rastreados são fechados por best effort quando a execução termina, mesmo que o registro de transcrição/sessão seja mantido.
...
| Depth | Formato da chave da sessão                  | Papel                                         | Pode iniciar spawn?          |
| ----- | ------------------------------------------- | --------------------------------------------- | ---------------------------- |
...
- Inicie o trabalho filho uma vez e aguarde eventos de conclusão em vez de criar
  loops de polling em torno de `sessions_list`, `sessions_history`, `/subagents list` ou
  comandos `exec` com sleep.
- `sessions_list` e `/subagents list` mantêm os relacionamentos entre sessões filhas focados
  no trabalho em andamento: filhos ativos permanecem anexados, filhos encerrados continuam visíveis por uma
  janela curta de recência, e links de filhos obsoletos apenas no armazenamento são ignorados após sua
  janela de validade. Isso evita que metadados antigos de `spawnedBy` / `parentSessionKey`
  ressuscitem filhos fantasmas após reinicialização.
...
### Política da ferramenta por profundidade
...
- **Profundidade 2 (worker folha)**: nenhuma ferramenta de sessão — `sessions_spawn` é sempre negada na profundidade 2. Não pode iniciar mais filhos.
...
Observação: a mesclagem é aditiva, então os perfis principais estão sempre disponíveis como fallback. Autenticação totalmente isolada por agente ainda não é compatível.
...
- Caso contrário, a entrega depende da profundidade do solicitante:
  ...
  - sessões solicitantes de subagente aninhadas recebem uma injeção interna de acompanhamento (`deliver=false`) para que o orquestrador possa sintetizar os resultados dos filhos na sessão
  - se uma sessão solicitante de subagente aninhada não existir mais, o OpenClaw faz fallback para o solicitante dessa sessão quando disponível
...
- A agregação de conclusão de filhos é limitada à execução atual do solicitante ao montar achados de conclusão aninhados, evitando que saídas antigas de filhos de execuções anteriores vazem para o anúncio atual.
- Respostas de anúncio preservam o roteamento de thread/tópico quando disponível nos adaptadores de canal.
- O contexto de anúncio é normalizado em um bloco de evento interno estável:
  ...
  - conteúdo do resultado selecionado a partir do texto visível mais recente do assistant, ou então texto saneado mais recente de tool/toolResult; execuções com falha terminal relatam status de falha sem repetir texto de resposta capturado
  - uma instrução de acompanhamento descrevendo quando responder ou permanecer em silêncio
- `Status` não é inferido a partir da saída do modelo; ele vem dos sinais de resultado do runtime.
- Em caso de timeout, se o filho só passou por chamadas de ferramenta, o anúncio pode condensar esse histórico em um breve resumo de progresso parcial em vez de repetir saída bruta da ferramenta.
...
- Metadados internos são destinados apenas à orquestração; respostas voltadas ao usuário devem ser reescritas em voz normal de assistant.

`sessions_history` é o caminho mais seguro para orquestração:
...
- históricos muito grandes podem descartar linhas antigas ou substituir uma linha superdimensionada por
...
`sessions_history` continua sendo uma visualização limitada e saneada de recuperação aqui também; não
é um dump bruto da transcrição.
  ...
  tools: {
    subagents: {
      tools: {
        // deny vence
        deny: ["gateway", "cron"],
        // se allow estiver definido, torna-se apenas allow (deny ainda vence)
...
ainda está vivo. Execuções não encerradas mais antigas que a janela de execução obsoleta deixam de contar como
ativas/pendentes em `/subagents list`, resumos de status, gating de conclusão de descendentes
e verificações de concorrência por sessão.

Após uma reinicialização do gateway, execuções restauradas obsoletas não encerradas são removidas, a menos que sua
sessão filha esteja marcada com `abortedLastRun: true`. Essas sessões filhas abortadas na reinicialização
continuam recuperáveis por meio do fluxo de recuperação de órfãos de subagente, que
envia uma mensagem sintética de retomada antes de limpar o marcador de abortado.
...
- `sessions_spawn` é sempre não bloqueante: retorna imediatamente `{ status: "accepted", runId, childSessionKey }`.
- O contexto do subagente só injeta `AGENTS.md` + `TOOLS.md` (não `SOUL.md`, `IDENTITY.md`, `USER.md`, `HEARTBEAT.md` nem `BOOTSTRAP.md`).
