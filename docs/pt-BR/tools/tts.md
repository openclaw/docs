...
summary: Texto para fala em respostas enviadas — provedores, personas, comandos de barra e saída por canal
...
OpenClaw pode converter respostas enviadas em áudio em **14 provedores de fala**
...
| **Inworld**       | `INWORLD_API_KEY`                                                                                                | API de TTS em streaming. Nota de voz Opus nativa e telefonia PCM.       |
...
| **MiniMax**       | `MINIMAX_API_KEY` (ou Plano de Token: `MINIMAX_OAUTH_TOKEN`, `MINIMAX_CODE_PLAN_KEY`, `MINIMAX_CODING_API_KEY`)  | API T2A v2. O padrão é `speech-2.8-hd`.                                 |
...
| **xAI**           | `XAI_API_KEY`                                                                                                    | TTS em lote da xAI. Nota de voz Opus nativa **não** é compatível.       |
| **Xiaomi MiMo**   | `XIAOMI_API_KEY`                                                                                                 | TTS MiMo por conclusões de chat da Xiaomi.                              |
...
voz, modelo, persona ou modo Auto-TTS diferente. O bloco do agente faz mesclagem profunda sobre
`messages.tts`, então as credenciais do provedor podem permanecer na configuração global de provedores:
...
Para fixar uma persona por agente, defina `agents.list[].tts.persona` junto com a configuração
do provedor — ela substitui a `messages.tts.persona` global apenas para esse agente.
...
6. diretivas inline `[[tts:...]]` quando [substituições controladas pelo modelo](#model-driven-directives) estão habilitadas

Substituições de canal e conta usam o mesmo formato de `messages.tts` e fazem
merge profundo sobre as camadas anteriores, para que credenciais compartilhadas
de provedor possam permanecer em `messages.tts` enquanto um canal ou conta de bot
altera apenas voz, modelo, persona ou modo automático:
...
determinística entre provedores. Ela pode preferir um provedor, definir uma
intenção de prompt independente de provedor e carregar vinculações específicas
de provedor para vozes, modelos, templates de prompt, seeds e configurações de
voz.
...
A seleção de provedor segue a prioridade explícita:
...
4. Substituições de diretiva TTS emitidas pelo modelo permitidas

### Como os provedores usam prompts de persona

Os campos de prompt de persona (`profile`, `scene`, `sampleContext`, `style`, `accent`,
`pacing`, `constraints`) são **independentes de provedor**. Cada provedor decide
como usá-los:

<AccordionGroup>
  <Accordion title="Google Gemini">
    Envolve os campos de prompt de persona em uma estrutura de prompt TTS do Gemini **somente quando**
  ...
  <Accordion title="OpenAI">
    ...
    nenhuma `instructions` explícita da OpenAI está configurada. `instructions`
    explícitas sempre têm prioridade.
  </Accordion>
...
`fallbackPolicy` controla o comportamento quando uma persona **não tem vinculação** para o
provedor tentado:
...
áudio. A entrega de blocos em streaming remove as diretivas do texto visível antes que
o canal as veja, mesmo quando divididas entre blocos adjacentes.
...
**Desabilitar totalmente substituições do modelo:**
...
- `/tts chat on|off|default` grava uma substituição de TTS automático com escopo de sessão para o chat atual.
...
- `/status` mostra o modo TTS ativo, além de provedor, modelo, voz e metadados higienizados de endpoint personalizado configurados quando TTS está habilitado.
...
A entrega de voz TTS é orientada por capacidades do canal. Plugins de canal anunciam
...
- **Feishu / WhatsApp**: quando uma resposta de nota de voz é produzida como MP3/WebM/WAV/M4A
  ou outro provável arquivo de áudio, o Plugin do canal a transcodifica para 48kHz
  Ogg/Opus com `ffmpeg` antes de enviar a mensagem de voz nativa. WhatsApp envia
...
- **CLI local**: usa o `outputFormat` configurado. Destinos de nota de voz são
  ...
  com `ffmpeg`.
- **Google Gemini**: o TTS da API Gemini retorna PCM bruto de 24kHz. O OpenClaw o encapsula como WAV para anexos de áudio, o transcodifica para Opus de 48kHz para destinos de nota de voz e retorna PCM diretamente para Talk/telefonia.
...
- Em `mode: "final"`, ainda envia TTS somente em áudio para respostas finais transmitidas por streaming
  depois que o stream de texto é concluído; a mídia gerada passa pela mesma
...
## Formatos de saída por canal
  ...
  | Destino                               | Formato                                                                                                                               |
  | ------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------- |
  | Feishu / Matrix / Telegram / WhatsApp | Respostas de nota de voz preferem **Opus** (`opus_48000_64` da ElevenLabs, `opus` da OpenAI). 48 kHz / 64 kbps equilibra clareza e tamanho. |
  ...
  - **Transcodificação Feishu / WhatsApp:** Quando uma resposta de nota de voz chega como MP3/WebM/WAV/M4A, o plugin do canal transcodifica para Ogg/Opus de 48 kHz com `ffmpeg`. O WhatsApp envia pelo Baileys com `ptt: true` e `audio/ogg; codecs=opus`. Se a conversão falhar: Feishu volta a anexar o arquivo original; o envio do WhatsApp falha em vez de publicar uma carga PTT incompatível.
  ...
  - **CLI local:** Usa o `outputFormat` configurado. Destinos de nota de voz são convertidos para Ogg/Opus, e saída de telefonia para PCM mono bruto de 16 kHz.
  - **Google Gemini:** Retorna PCM bruto de 24 kHz. OpenClaw encapsula como WAV para anexos, transcodifica para Opus de 48 kHz para destinos de nota de voz, retorna PCM diretamente para Talk/telefonia.
  - **Inworld:** Anexos MP3, nota de voz nativa `OGG_OPUS`, `PCM` bruto de 22050 Hz para Talk/telefonia.
  - **xAI:** MP3 por padrão; `responseFormat` pode ser `mp3|wav|pcm|mulaw|alaw`. Usa o endpoint REST em lote da xAI — TTS por WebSocket em streaming **não** é usado. Formato Opus nativo para nota de voz **não** é compatível.
  - **Microsoft:** Usa `microsoft.outputFormat` (padrão `audio-24khz-48kbitrate-mono-mp3`). O `sendVoice` do Telegram aceita OGG/MP3/M4A; use OpenAI/ElevenLabs se precisar de mensagens de voz em Opus garantidas. Se o formato Microsoft configurado falhar, OpenClaw tenta novamente com MP3.
  ...
  <Accordion title="Top-level messages.tts.*">
    ...
    <ParamField path="provider" type="string">
      ID do provedor de fala. Quando não definido, OpenClaw usa o primeiro provedor configurado na ordem de seleção automática do registro. O legado `provider: "edge"` é reescrito para `"microsoft"` por `openclaw doctor --fix`.
    </ParamField>
  ...
  <Accordion title="Google Gemini">
    <ParamField path="apiKey" type="string">Usa `GEMINI_API_KEY` / `GOOGLE_API_KEY` como fallback. Se omitido, TTS pode reutilizar `models.providers.google.apiKey` antes do fallback de env.</ParamField>
    <ParamField path="model" type="string">Modelo TTS do Gemini. Padrão `gemini-3.1-flash-tts-preview`.</ParamField>
    <ParamField path="voiceName" type="string">Nome da voz pré-criada do Gemini. Padrão `Kore`. Alias: `voice`.</ParamField>
  ...
  <Accordion title="Microsoft (sem chave de API)">
    <ParamField path="enabled" type="boolean" default="true">Permitir uso de fala da Microsoft.</ParamField>
    ...
    <ParamField path="outputFormat" type="string">Formato de saída da Microsoft. Padrão `audio-24khz-48kbitrate-mono-mp3`. Nem todos os formatos são aceitos pelo transporte incluído baseado no Edge.</ParamField>
    <ParamField path="rate / pitch / volume" type="string">Strings percentuais (por exemplo, `+10%`, `-5%`).</ParamField>
    <ParamField path="saveSubtitles" type="boolean">Grava legendas em JSON junto com o arquivo de áudio.</ParamField>
  ...
  <Accordion title="MiniMax">
    <ParamField path="apiKey" type="string">Recorre a `MINIMAX_API_KEY`. Autenticação Token Plan via `MINIMAX_OAUTH_TOKEN`, `MINIMAX_CODE_PLAN_KEY` ou `MINIMAX_CODING_API_KEY`.</ParamField>
    ...
    <ParamField path="voiceId" type="string">Padrão `English_expressive_narrator`. Amb.: `MINIMAX_TTS_VOICE_ID`.</ParamField>
  ...
  <Accordion title="OpenAI">
    ...
    <ParamField path="extraBody / extra_body" type="Record<string, unknown>">Campos JSON extras mesclados aos corpos de solicitação `/audio/speech` após os campos TTS gerados da OpenAI. Use isto para endpoints compatíveis com OpenAI, como Kokoro, que exigem chaves específicas do provedor, como `lang`; chaves de protótipo inseguras são ignoradas.</ParamField>
    <ParamField path="baseUrl" type="string">
      Substitui o endpoint TTS da OpenAI. Ordem de resolução: configuração → `OPENAI_TTS_BASE_URL` → `https://api.openai.com/v1`. Valores diferentes do padrão são tratados como endpoints TTS compatíveis com OpenAI, portanto nomes personalizados de modelo e voz são aceitos.
    </ParamField>
  </Accordion>
  ...
  <Accordion title="Volcengine (BytePlus Seed Speech)">
    ...
    <ParamField path="appKey" type="string">Cabeçalho de chave do app. Padrão `aGjiRDfUWi`. Amb.: `VOLCENGINE_TTS_APP_KEY`.</ParamField>
    <ParamField path="baseUrl" type="string">Substitui o endpoint HTTP TTS do Seed Speech. Amb.: `VOLCENGINE_TTS_BASE_URL`.</ParamField>
    <ParamField path="voice" type="string">Tipo de voz. Padrão `en_female_anna_mars_bigtts`. Amb.: `VOLCENGINE_TTS_VOICE`.</ParamField>
  ...
  <Accordion title="xAI">
    ...
    <ParamField path="voiceId" type="string">Padrão `eve`. Vozes ao vivo: `ara`, `eve`, `leo`, `rex`, `sal`, `una`.</ParamField>
  ...
  <Accordion title="Xiaomi MiMo">
    ...
    <ParamField path="model" type="string">Padrão `mimo-v2.5-tts`. Amb.: `XIAOMI_TTS_MODEL`. Também aceita `mimo-v2-tts`.</ParamField>
    <ParamField path="voice" type="string">Padrão `mimo_default`. Amb.: `XIAOMI_TTS_VOICE`.</ParamField>
    <ParamField path="format" type='"mp3" | "wav"'>Padrão `mp3`. Amb.: `XIAOMI_TTS_FORMAT`.</ParamField>
    <ParamField path="style" type="string">Instrução opcional de estilo em linguagem natural enviada como a mensagem do usuário; não é falada.</ParamField>
  </Accordion>
</AccordionGroup>
...
WhatsApp podem transcodificar saídas TTS que não sejam Opus nesse caminho quando `ffmpeg` estiver
disponível.
...
clientes não renderizam legendas de forma consistente em notas de voz.
