...
read_when:
    ...
    - Escolhendo entre o Chrome MCP local ao host e o CDP remoto bruto em configurações com hosts separados
summary: Solucione problemas do Gateway no WSL2 + CDP remoto do Chrome no Windows em camadas
title: Solução de problemas de WSL2 + Windows + CDP remoto do Chrome
x-i18n:
    generated_at: "2026-07-12T15:47:39Z"
    model: gpt-5.6
...
separadamente, embora produzam erros semelhantes. Percorra as camadas
...
Use um perfil de navegador remoto que aponte do WSL2 para um endpoint CDP
do Chrome no Windows. Escolha essa opção quando o Gateway permanecer dentro do WSL2, o Chrome for executado no
...
exportação para PDF, interceptação de downloads ou ações em lote (os perfis MCP do Chrome não
oferecem suporte a esses recursos).

Para Gateway no WSL2 + Chrome no Windows, use CDP remoto bruto. O MCP do Chrome é
...
Quando a interface for aberta no Windows, use o localhost do Windows, a menos que você tenha uma
...
Não use um IP da LAN como padrão. HTTP sem criptografia em um endereço da LAN ou da tailnet pode
...
Prossiga de cima para baixo; não pule etapas. Corrigir uma camada ainda pode deixar
visível um erro diferente proveniente de uma camada posterior.

### Camada 1: verifique se o Chrome está disponibilizando o CDP no Windows

```powershell
chrome.exe --remote-debugging-port=9222 --user-data-dir="$env:LOCALAPPDATA\OpenClaw\ChromeCDP"
```

O Chrome 136 e versões posteriores ignoram opções de linha de comando de depuração remota para o
diretório de dados padrão do Chrome. Use um diretório de dados separado e não padrão, como
...
O Chromium tenta primeiro vincular a depuração remota a `127.0.0.1` e recorre a
`[::1]` somente se a vinculação IPv4 falhar. Uma regra `v4tov4` persistente escutando em
`127.0.0.1:9222` pode ocupar esse endpoint antes da inicialização do Chrome. O Chrome então
recorre a `[::1]:9222`, enquanto a regra antiga encaminha o tráfego IPv4 de volta ao
próprio listener e retorna uma resposta vazia.

Verifique os listeners e as regras de proxy reais no Windows, em vez de inferi-los
...
- Se `chrome.exe` responder em `127.0.0.1`, remova qualquer regra de portproxy que também
  escute em `127.0.0.1:9222`. Encaminhe somente o endereço do adaptador do Windows
  acessível pelo WSL2 para `127.0.0.1`.
- Se `chrome.exe` responder somente em `[::1]`, aponte o listener acessível pelo WSL2 para
  `::1` com `v4tov6`, em vez de encaminhar para um endereço IPv4 não utilizado:

  ```powershell
  netsh interface portproxy add v4tov6 listenaddress=WINDOWS_HOST_OR_IP listenport=9222 connectaddress=::1 connectport=9222
  ```

Vincule o listener ao endereço do adaptador necessário para o WSL2. Não exponha a porta
...
incorreto para o lado do WSL2 ou faltam firewall/encaminhamento de porta/proxy. Corrija
...
- use o endereço acessível pelo WSL2, não um que funcione somente no Windows
...
- use WS(S) somente quando o provedor do navegador fornecer uma URL direta de socket
  do DevTools
...
### Camada 5: verifique o controle de ponta a ponta do navegador
...
- a guia é aberta no Chrome do Windows
- `browser tabs` retorna o destino
- as ações posteriores (`snapshot`, `screenshot`, `navigate`) funcionam com o mesmo
  perfil
...
| --------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `control-ui-insecure-auth`                                                              | problema de origem da interface/contexto seguro, não um problema de transporte CDP                                                                                                                          |
| `token_missing`                                                                         | problema de configuração de autenticação                                                                                                                                                                    |
...
| resposta CDP vazia / `other side closed` por meio de um portproxy                       | incompatibilidade de listener do Windows ou um loop de autorreferência; inspecione ambas as famílias de loopback e `netsh interface portproxy show all`                                                      |
...
| substituições obsoletas de viewport / modo escuro / localidade / modo offline após uma sessão remota | execute `openclaw browser --browser-profile remote stop` para encerrar a sessão e liberar a conexão Playwright/CDP em cache sem reiniciar o Gateway nem o navegador externo                       |
| tempo limite em torno de `remoteCdpTimeoutMs` (padrão de 1500ms)                        | geralmente ainda é a acessibilidade do CDP ou um endpoint remoto lento/inacessível                                                                                                                          |
| `Playwright page enumeration timed out after 3000ms`                                    | o CDP remoto foi conectado, mas a leitura persistente de guias ficou travada; o prazo é o maior valor entre `remoteCdpTimeoutMs` e `remoteCdpHandshakeTimeoutMs`                                             |
| `No Chrome tabs found for profile="user"`                                               | perfil MCP local do Chrome selecionado onde não há guias locais ao host disponíveis                                                                                                                          |

## Lista de verificação para triagem rápida

1. Windows: qual dos endereços `127.0.0.1` ou `[::1]` responde em `/json/version` e
...
5. Você está tentando usar `existing-session` entre o WSL2 e o Windows em vez
   de CDP remoto bruto?

Primeiro, verifique localmente o endpoint do Chrome no Windows; depois, verifique o mesmo endpoint
no WSL2; e somente então diagnostique a configuração do OpenClaw ou a autenticação da interface de controle.
