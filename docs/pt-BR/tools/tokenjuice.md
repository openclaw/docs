...
read_when:
    ...
    - Você precisa entender o que o tokenjuice altera e o que ele mantém em formato bruto
summary: Compacte resultados ruidosos das ferramentas exec e bash com o Plugin opcional Tokenjuice
title: Tokenjuice
x-i18n:
    generated_at: "2026-07-12T00:27:41Z"
    model: gpt-5.6
...
reescreve a entrada do shell, não executa novamente os comandos nem altera os códigos de saída.

Atualmente, isso se aplica às execuções incorporadas do OpenClaw e às ferramentas dinâmicas do OpenClaw no
harness do app-server do Codex. O Tokenjuice se conecta ao middleware de resultados de ferramentas do OpenClaw e
...
- Compacta resultados ruidosos de `exec` e `bash` antes que sejam enviados de volta à sessão.
- Mantém a execução original do comando intacta.
- Aplica uma política de inventário seguro: leituras exatas do conteúdo de arquivos permanecem sem alterações, comandos independentes de inventário do repositório podem ser compactados e sequências mistas de comandos que não sejam seguras permanecem sem alterações.
- Permanece opcional: desative o plugin se quiser a saída literal em todos os casos.
...
4. Verifique se o resultado retornado pela ferramenta é mais curto e mais estruturado que a saída bruta do shell.
