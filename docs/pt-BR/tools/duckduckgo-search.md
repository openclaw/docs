...
x-i18n:
    generated_at: "2026-07-12T00:27:40Z"
    model: gpt-5.6
...
O OpenClaw oferece suporte ao DuckDuckGo como provedor de `web_search` **sem necessidade de chave**. Nenhuma chave de API ou conta é necessária.
...
Os parâmetros `region` e `safeSearch` da ferramenta substituem os valores da configuração do Plugin acima para cada consulta.

## Observações

- **Sem chave de API** — funciona assim que o DuckDuckGo é selecionado como provedor de `web_search`.
...
- **Risco de desafio contra bots** — o DuckDuckGo pode exibir CAPTCHAs ou bloquear solicitações em caso de uso intenso ou automatizado.
...
- **O valor padrão de SafeSearch é `moderate`** quando não configurado.
