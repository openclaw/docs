...
[Exa AI](https://exa.ai/) é um provedor de `web_search` com modos de busca neural, por palavras-chave e
...
Defina `plugins.entries.exa.config.webSearch.baseUrl` para encaminhar as
solicitações de busca do Exa por um proxy compatível ou endpoint alternativo. O OpenClaw
normaliza hosts sem protocolo, prefixando-os com `https://`, e acrescenta `/search`, a menos que
o caminho já termine assim. O endpoint resolvido faz parte da chave do cache
de busca, portanto os resultados de endpoints diferentes nunca são compartilhados.
...
Resultados a retornar (1–100, sujeito aos limites do tipo de busca do Exa).
</ParamField>
...
Opções de extração de conteúdo (consulte abaixo).
</ParamField>
...
| `highlights`      | `boolean \| { maxCharacters, query, numSentences, highlightsPerUrl }` | Extrai as frases principais      |
| `summary`         | `boolean \| { query }`                                                | Resumo gerado por IA             |

Se `contents` for omitido, o padrão do Exa será `{ highlights: true }`, para que os resultados
incluam trechos com frases principais. As descrições dos resultados são obtidas primeiro dos destaques,
depois do resumo e, por fim, do texto completo — usando o primeiro que estiver disponível. Os resultados
também preservam os campos brutos `highlightScores` e `summary` da resposta da API do Exa,
quando disponíveis.
...
| `deep`           | Busca aprofundada e abrangente           |
| `deep-reasoning` | Busca aprofundada com raciocínio         |
| `instant`        | Resultados mais rápidos                   |

## Observações

- `count` aceita até 100, sujeito aos limites do tipo de busca do Exa.
- Por padrão, os resultados ficam armazenados em cache por 15 minutos. Configure os parâmetros compartilhados
  `tools.web.search.cacheTtlMinutes` (minutos) e
  `tools.web.search.timeoutSeconds` (padrão de 30 s) para alterar o cache e
  o tempo limite das solicitações de todos os provedores de `web_search`, incluindo o Exa.
