...
read_when:
    ...
    - Você quer entender a detecção automática e a troca para provedor alternativo
sidebarTitle: Web Search
summary: web_search, x_search e web_fetch -- pesquise na web, pesquise posts do X ou busque conteúdo da página
title: Busca na web
x-i18n:
    generated_at: "2026-05-02T05:58:53Z"
    model: gpt-5.5
    provider: openai
...
<CardGroup cols={2}>
  ...
  <Card title="Firecrawl" icon="flame" href="/pt-BR/tools/firecrawl">
    Resultados estruturados. Funciona melhor combinado com `firecrawl_search` e `firecrawl_scrape` para extração profunda.
  </Card>
  ...
  <Card title="MiniMax Search" icon="globe" href="/pt-BR/tools/minimax-search">
    Resultados estruturados via API de busca MiniMax Token Plan.
  </Card>
  <Card title="Ollama Web Search" icon="globe" href="/pt-BR/tools/ollama-search">
    Busca via um host Ollama local conectado ou pela API hospedada da Ollama.
  </Card>
...
Modelos OpenAI Responses diretos usam automaticamente a ferramenta `web_search` hospedada pela OpenAI quando a busca web do OpenClaw está habilitada e nenhum provedor gerenciado está fixado. Esse é um comportamento de propriedade do provedor no Plugin OpenAI incluído e se aplica apenas ao tráfego nativo da API da OpenAI, não a URLs base de proxy compatíveis com OpenAI nem a rotas do Azure. Defina `tools.web.search.provider` para outro provedor, como `brave`, para manter a ferramenta `web_search` gerenciada para modelos OpenAI, ou defina `tools.web.search.enabled: false` para desabilitar tanto a busca gerenciada quanto a busca OpenAI nativa.
...
- `web_search` gerenciada ainda se aplica a modelos que não são Codex
...
Se a busca nativa do Codex estiver habilitada, mas o modelo atual não for compatível com Codex, o OpenClaw mantém o comportamento normal de `web_search` gerenciada.

## Configurando a busca web

As listas de provedores na documentação e nos fluxos de configuração estão em ordem alfabética. A detecção automática mantém uma
...
11. **Ollama Web Search** -- alternativa sem chave via seu host Ollama local configurado quando ele está acessível e conectado com `ollama signin`; pode reutilizar a autenticação bearer do provedor Ollama quando o host precisar dela, e pode chamar a busca direta em `https://ollama.com` quando configurada com `OLLAMA_API_KEY` (ordem 110)
...
após sua configuração dedicada de busca web e `GEMINI_API_KEY`. Consulte as
...
- `web_fetch` fora de sandbox pode usar provedores de Plugins instalados que declaram
  `contracts.webFetchProviders`; buscas em sandbox permanecem apenas com os incluídos
...
- o modelo padrão de pesquisa na web do Kimi (padrão: `kimi-k2.6`)
...
| `domain_filter`       | Array de lista de permissões/bloqueios de domínios (somente Perplexity) |
...
<Warning>
  ...
  `date_before` convertendo-os em intervalos de tempo de fundamentação da Pesquisa Google.
  ...
  SearXNG aceita `http://` somente para hosts de rede privada confiável ou local loopback;
...
`x_search` consulta publicações no X (anteriormente Twitter) usando xAI e retorna
...
integrada da xAI na solicitação que atende a esta chamada de ferramenta.

<Note>
  A xAI documenta que `x_search` oferece suporte a pesquisa por palavra-chave, pesquisa semântica, pesquisa de usuário
  ...
  exata da publicação ou o ID de status. Pesquisas amplas por palavra-chave podem encontrar a publicação correta,
  mas retornar metadados por publicação menos completos. Um bom padrão é: localize a publicação primeiro e depois
...
| ---------------------------- | ----------------------------------------------------- |
| `query`                      | Consulta de pesquisa (obrigatório)                    |
| `allowed_x_handles`          | Restringir resultados a identificadores específicos do X |
| `excluded_x_handles`         | Excluir identificadores específicos do X              |
...
- [Web Browser](/pt-BR/tools/browser) -- automação completa do navegador para sites com uso intenso de JS
