...
read_when:
    ...
    - Você quer orientações sobre o host de pesquisa do MiniMax CN/global
summary: Pesquisa do MiniMax pela API de pesquisa do Token Plan
...
<Steps>
  <Step title="Criar uma chave">
    Crie ou copie uma chave do Token Plan do MiniMax na
    [Plataforma MiniMax](https://platform.minimax.io/user-center/basic-information/interface-key).
    Configurações de OAuth podem reutilizar `MINIMAX_OAUTH_TOKEN`.
  </Step>
...
`MINIMAX_CODE_PLAN_KEY`. `MINIMAX_API_KEY` deve apontar para uma credencial do
Token Plan com pesquisa habilitada; chaves comuns da API de modelos do MiniMax podem não ser aceitas pelo
  ...
  plugins: {
    entries: {
      minimax: {
        config: {
          webSearch: {
            apiKey: "sk-cp-...", // opcional se uma variável de ambiente do Token Plan do MiniMax estiver definida
            region: "global", // ou "cn"
          },
        },
      },
...
A Pesquisa do MiniMax usa estes endpoints:
...
Isso significa que a integração inicial para a China ou `MINIMAX_API_HOST=https://api.minimaxi.com/...`
também mantém automaticamente a Pesquisa do MiniMax no host da China.
...
pode fornecer a credencial de portador para a Pesquisa do MiniMax.
...
| `count`   | integer | 1-10, padrão 5  | Número de resultados a retornar. O OpenClaw reduz a lista retornada para esse tamanho. |

No momento, não há suporte a filtros específicos do provedor.
