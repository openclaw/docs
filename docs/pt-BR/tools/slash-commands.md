...
Quando uma conversa está vinculada a uma sessão ACP, o texto normal é encaminhado para o
harness ACP. Os comandos de gerenciamento do Gateway permanecem locais: `/acp ...` sempre chega
ao processador de comandos do OpenClaw, e `/status` e `/unfocus` permanecem locais sempre que
...
<CardGroup cols={3}>
  ...
  <Card title="Diretivas" icon="sliders">
    ...
    a veja. Persistem as configurações da sessão quando enviadas sozinhas; atuam como orientações em linha
...
<AccordionGroup>
  <Accordion title="Detalhes do comportamento das diretivas">
    ...
    - As diretivas se aplicam apenas a **remetentes autorizados**. Se `commands.allowFrom`
      estiver definido, essa será a única lista de permissões usada; caso contrário, a autorização virá das
      listas de permissões/pareamento do canal, além de `commands.useAccessGroups`. Para remetentes não
...
<ParamField path="commands.native" type='boolean | "auto"' default='"auto"'>
  ...
  `channels.<provider>.commands.native`. No Discord, `false` ignora o registro de comandos de barra;
...
<ParamField path="commands.restart" type="boolean" default="true">
  Habilita `/restart` e ações de ferramentas para reiniciar o Gateway.
</ParamField>

<ParamField path="commands.ownerAllowFrom" type="string[]">
  Lista explícita de proprietários permitidos para superfícies de comandos exclusivas do proprietário. Separada de
  `commands.allowFrom` e do acesso por pareamento de mensagens diretas.
</ParamField>
...
<ParamField path="commands.allowFrom" type="object">
  ...
  **única** fonte de autorização para comandos e diretivas. Use `"*"` como padrão
  global; chaves específicas de provedor o substituem.
</ParamField>
...
A disponibilidade depende dos sinalizadores de configuração, da superfície do canal e dos
...
### Comandos do núcleo
  ...
  <Accordion title="Sessões e execuções">
    ...
    <Note>
      A interface de controle intercepta `/new` digitado para criar uma nova
      sessão do painel e alternar para ela, exceto quando `session.dmScope: "main"` está configurado
  ...
  <Accordion title="Controles de modelo e execução">
    ...
    | `/trace on\|off` | Ativa ou desativa a saída de rastreamento do plugin para a sessão atual |
    ...
    | `/login [codex\|openai\|openai-codex]` | Vincula o login do Codex/OpenAI por um chat privado ou uma sessão da interface Web. Somente proprietário/administrador |
    ...
    <AccordionGroup>
      <Accordion title="segurança de verbose / trace / fast / reasoning">
        - `/verbose` é destinado à depuração — mantenha-o **desativado** durante o uso normal.
        - `/trace` revela apenas linhas de rastreamento/depuração pertencentes ao plugin; as mensagens detalhadas normais permanecem desativadas.
        - `/fast auto|on|off` mantém uma substituição da sessão; use a opção `inherit` da interface de Sessões para limpá-la.
        ...
        - `/reasoning`, `/verbose` e `/trace` são arriscados em ambientes de grupo — podem revelar raciocínio interno ou diagnósticos de plugins. Mantenha-os desativados em chats em grupo.
  ...
  <Accordion title="Descoberta e status">
    | Comando | Descrição |
    | --- | --- |
    | `/help` | Exibe o resumo curto da ajuda |
    | `/commands` | Exibe o catálogo de comandos gerado |
    | `/tools [compact\|verbose]` | Exibe o que o agente atual pode usar neste momento |
    | `/status` | Exibe o status da execução e do ambiente de runtime, o tempo de atividade do Gateway e do sistema, a integridade dos plugins e o uso/a cota do provedor |
    | `/status plugins` | Exibe detalhes da integridade dos plugins: erros de carregamento, quarentenas, falhas de plugins de canal, problemas de dependência e avisos de compatibilidade. Requer `commands.plugins: true` |
    ...
    | `/diagnostics [note]` | Fluxo de relatório de suporte exclusivo do proprietário. Sempre solicita aprovação de execução |
    | `/crestodian <request>` | Executa o assistente de configuração e reparo do Crestodian a partir de uma mensagem direta do proprietário |
  ...
  <Accordion title="Skills, listas de permissões e aprovações">
    ...
    | `/learn [request]` | Elabora uma skill revisável com base na conversa atual ou em fontes nomeadas por meio da [Oficina de Skills](/pt-BR/tools/skill-workshop) |
    | `/allowlist [list\|add\|remove] ...` | Gerencia entradas da lista de permissões. Somente texto |
    | `/approve <id> <decision>` | Resolve solicitações de aprovação de execução ou de plugin |
  ...
  <Accordion title="Subagentes e ACP">
    ...
    | `/acp spawn\|cancel\|steer\|close\|sessions\|status\|set-mode\|set\|cwd\|permissions\|timeout\|model\|reset-options\|doctor\|install\|help` | Gerencia sessões ACP e opções de runtime. Os controles de runtime exigem a identidade de um proprietário externo ou administrador interno do Gateway |
  ...
  <Accordion title="Gravações exclusivas do proprietário e administração">
    ...
    | `/mcp show\|get\|set\|unset` | `commands.mcp: true` | Lê ou grava a configuração de servidores MCP gerenciados pelo OpenClaw. Exclusivo do proprietário |
    | `/plugins list\|inspect\|show\|get\|install\|enable\|disable` | `commands.plugins: true` | Inspeciona ou altera o estado de plugins. Gravações exclusivas do proprietário. Alias: `/plugin` |
...
Gerados a partir de plugins de canal com suporte a comandos nativos:
...
| `/phone status\|arm ...\|disarm`                        | Habilita temporariamente comandos de Node de alto risco (câmera/tela/computador/gravações). Consulte [Uso do computador](/nodes/computer-use)                                                  |
...
| `/codex <action> ...`                                   | Vincula, direciona e inspeciona o harness do servidor de aplicativos do Codex (status, threads, retomada, modelo, modo rápido, permissões, compactação, revisão, MCP, Skills e mais). Consulte [Harness do Codex](/pt-BR/plugins/codex-harness) |
...
<AccordionGroup>
  ...
  <Accordion title="Argumentos de comandos nativos">
    ...
    argumentos obrigatórios são omitidos. Telegram e Slack exibem um menu de botões para comandos com
    opções. As opções dinâmicas são resolvidas de acordo com o modelo da sessão de destino, portanto as opções
...
`/tools` responde a uma pergunta sobre o runtime: **o que este agente pode usar agora nesta
conversa** — não um catálogo estático de configurações.
...
Os resultados têm escopo de sessão. Alterar o agente, canal, thread, autorização
...
/model list        # igual ao anterior
/model 3           # seleciona pelo número do seletor
/model openai/gpt-5.4
/model opus@anthropic:default
/model default     # limpa a seleção de modelo da sessão
/model status      # exibição detalhada com endpoint e modo de API
```

No Discord, `/model` e `/models` abrem um seletor interativo com listas suspensas
de provedores e modelos. O seletor respeita `agents.defaults.models`, incluindo
entradas `provider/*`.

## `/config`: gravações de configuração em disco

<Note>
  Exclusivo do proprietário. Desativado por padrão — habilite com `commands.config: true`.
</Note>
...
<Note>
  Exclusivo do proprietário. Desativado por padrão — habilite com `commands.mcp: true`.
</Note>
...
`/mcp` armazena a configuração na configuração do OpenClaw, não nas configurações de projeto
do agente incorporado. `/mcp show` oculta campos que contêm credenciais, valores de
flags de credenciais reconhecidas e argumentos conhecidos com formato de segredo. Quando executado em um grupo, a
configuração é enviada ao proprietário em particular; se nenhuma rota privada para o proprietário estiver
disponível, o comando falha de forma segura e solicita que o proprietário tente novamente em um
chat direto.

## `/debug`: substituições somente para o runtime

<Note>
  Exclusivo do proprietário. Desativado por padrão — habilite com `commands.debug: true`.
...
<Note>
  Gravações exclusivas do proprietário. Desativado por padrão — habilite com `commands.plugins: true`.
</Note>
...
`/plugins enable|disable` atualiza a configuração de plugins e recarrega dinamicamente o runtime
de plugins do Gateway para novas interações do agente. `/plugins install` reinicia automaticamente
os Gateways gerenciados porque os módulos de origem do plugin foram alterados.
...
`/trace` revela linhas de rastreamento/depuração de plugins com escopo de sessão sem o modo detalhado
completo. Ele não substitui `/debug` (substituições de runtime) nem `/verbose` (saída normal
das ferramentas).
...
/side o que mudou enquanto a execução principal continuou?
```

Diferentemente de uma mensagem normal:

- Usa a sessão atual como contexto de fundo.
- Em sessões do harness do Codex, é executado como uma thread paralela efêmera do Codex.
...
<AccordionGroup>
  <Accordion title="Escopo de sessão por superfície">
    - **Comandos de texto:** são executados na sessão normal de chat (mensagens diretas compartilham `main`; grupos têm sua própria sessão).
    ...
    - **Comandos nativos do Telegram:** `telegram:slash:<userId>` (direcionam-se à sessão de chat por meio de `CommandTargetSessionKey`)
    - **`/login codex`** envia códigos de pareamento de dispositivos apenas por chat privado ou caminhos de resposta da Web UI. Invocações em grupos/tópicos do Telegram solicitam que o proprietário envie uma mensagem direta ao bot.
    - **`/stop`** direciona-se à sessão de chat ativa para interromper a execução atual.
  ...
  <Accordion title="Especificidades do Slack">
    ...
    `/status`. O comando de texto `/status` continua funcionando em mensagens do Slack.
  </Accordion>
  <Accordion title="Caminho rápido e atalhos em linha">
    - Mensagens contendo apenas comandos de remetentes na lista de permissões são processadas imediatamente (ignoram a fila e o modelo).
    ...
    - Mensagens não autorizadas contendo apenas comandos são ignoradas silenciosamente; tokens `/...` em linha são tratados como texto simples.

  </Accordion>
  <Accordion title="Observações sobre argumentos">
    - Os comandos aceitam `:` opcional entre o comando e os argumentos (`/think: high`, `/send: on`).
    - `/new <model>` aceita um alias de modelo, `provider/model` ou o nome de um provedor (correspondência aproximada); se não houver correspondência, o texto será tratado como o corpo da mensagem.
...
- **Uso/cota do provedor** (por exemplo, "Claude com 80% restante") aparece em `/status` para o provedor do modelo atual quando o acompanhamento de uso está ativado.
...
- **Execução vs. runtime:** `/status` informa `Execution` para o caminho efetivo do sandbox e `Runtime` para indicar quem está executando a sessão: `OpenClaw Default`, `OpenAI Codex`, um backend de CLI ou um backend ACP.
