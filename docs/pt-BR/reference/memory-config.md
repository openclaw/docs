...
summary: Todas as opções de configuração para pesquisa de memória, provedores de embeddings, QMD, pesquisa híbrida e indexação multimodal
...
<Note>
Se você está procurando a opção para ativar o recurso **Active Memory** e a configuração do subagente, elas ficam em `plugins.entries.active-memory`, e não em `memorySearch`.

Active Memory usa um modelo de duas condições:

1. o plugin deve estar ativado e direcionado ao id do agente atual
2. a solicitação deve ser uma sessão de chat persistente e interativa qualificada

Consulte [Active Memory](/pt-BR/concepts/active-memory) para conhecer o modelo de ativação, a configuração pertencente ao plugin, a persistência da transcrição e o padrão de implantação gradual segura.
</Note>
...
Configurações legadas que ainda contêm `provider: "auto"` são resolvidas como `openai`.

<Warning>
Alterar o provedor de embeddings, o modelo, as configurações do provedor, as fontes, o escopo,
a divisão em segmentos ou o tokenizador pode tornar o índice vetorial SQLite existente incompatível.
O OpenClaw pausa a busca vetorial e informa um aviso de identidade do índice em vez de
refazer automaticamente os embeddings de tudo. Recrie o índice quando estiver pronto com
`openclaw memory status --index --agent <id>` ou
`openclaw memory index --force --agent <id>`.
</Warning>

Quando `provider` não está definido, há um `provider: "auto"` legado ou
`provider: "none"` seleciona intencionalmente o modo somente FTS, a recuperação de memória ainda pode
usar a classificação lexical do FTS quando os embeddings não estão disponíveis.

Provedores não locais explícitos falham de forma fechada. Se você definir `memorySearch.provider` como
um provedor concreto apoiado por serviço remoto, como Bedrock, DeepInfra, Gemini, GitHub
Copilot, LM Studio, Mistral, Ollama, OpenAI, Voyage ou um provedor personalizado
compatível com OpenAI, e esse provedor não estiver disponível em tempo de execução, `memory_search`
retornará um resultado de indisponibilidade em vez de usar silenciosamente a recuperação somente por FTS. Corrija a
configuração do provedor/autenticação, mude para um provedor acessível ou defina
`provider: "none"` se quiser usar deliberadamente a recuperação somente por FTS.

### IDs de provedores personalizados

`memorySearch.provider` pode apontar para uma entrada personalizada `models.providers.<id>` para adaptadores de provedores específicos de memória, como `ollama`, ou para APIs de modelos compatíveis com OpenAI, como `openai-responses` / `openai-completions`. O OpenClaw resolve o proprietário da `api` desse provedor para o adaptador de embeddings enquanto preserva o ID personalizado do provedor para o tratamento do endpoint, da autenticação e do prefixo do modelo. Isso permite que configurações com várias GPUs ou vários hosts dediquem os embeddings de memória a um endpoint local específico:
...
<ParamField path="remote.headers" type="object">
  Cabeçalhos HTTP adicionais (combinados com os padrões do provedor).
</ParamField>
...
<AccordionGroup>
  ...
  <Accordion title="Tipos de entrada compatíveis com OpenAI">
    Endpoints de embeddings compatíveis com OpenAI podem optar por usar campos de solicitação `input_type` específicos do provedor. Isso é útil para modelos de embeddings assimétricos que exigem rótulos diferentes para embeddings de consulta e de documento.
    ...
    Alterar esses valores afeta a identidade do cache de embeddings na indexação em lote do provedor e deve ser seguido por uma reindexação da memória quando o modelo upstream tratar os rótulos de forma diferente.

  </Accordion>
  <Accordion title="Bedrock">
    ### Configuração de embeddings do Bedrock

    O Bedrock usa a cadeia padrão de credenciais do AWS SDK mais um token de portador verificado pelo OpenClaw, portanto nenhuma chave de API é armazenada na configuração. Se o OpenClaw for executado no EC2 com uma função de instância habilitada para o Bedrock, basta definir o provedor e o modelo:
    ...
    **Modelos compatíveis** (com detecção de família e padrões de dimensão):
    ...
    As variantes com sufixo de throughput (por exemplo, `amazon.titan-embed-text-v1:2:8k`) e os IDs de perfil de inferência com prefixo de região (por exemplo, `us.amazon.titan-embed-text-v2:0`) herdam a configuração do modelo base.
    ...
    **Permissões do IAM:** a função ou o usuário do IAM precisa de:
    ...
    Para aplicar o menor privilégio, restrinja `InvokeModel` ao modelo específico:
  ...
  <Accordion title="Local (GGUF + llama.cpp)">
    ...
    | `local.contextSize`   | `number \| "auto"` | `4096`                 | Tamanho da janela de contexto para o contexto de embeddings. 4096 abrange trechos típicos (128-512 tokens) enquanto limita a VRAM não usada pelos pesos. Reduza para 1024-2048 em hosts com recursos limitados. `"auto"` usa o máximo treinado do modelo — não recomendado para modelos 8B+ (Qwen3-Embedding-8B: até 40 960 tokens podem elevar o uso de VRAM para ~32 GB). |
    ...
    Use a CLI independente para verificar o mesmo caminho do provedor usado pelo Gateway:
    ...
    Valores numéricos de `local.contextSize` também orientam o posicionamento automático de camadas na GPU do node-llama-cpp, para que os pesos do modelo e o contexto de embeddings solicitado sejam acomodados em conjunto. `openclaw memory status --deep` informa o backend, o dispositivo, o offload e o contexto solicitado do llama.cpp conhecidos mais recentemente, além de fatos de memória com carimbo de data e hora, depois que o runtime é carregado; o status passivo não carrega um modelo.
...
Quando não definido, usa o padrão do provedor: 600 segundos para provedores locais/auto-hospedados, como `local`, `ollama` e `lmstudio`, e 120 segundos para provedores hospedados. Aumente esse valor quando os lotes de embeddings locais limitados pela CPU estiverem funcionando corretamente, mas forem lentos.
</ParamField>
...
| Chave                          | Tipo      | Padrão | Descrição                                                                      |
| ------------------------------ | --------- | ------ | ------------------------------------------------------------------------------ |
| `onSessionStart`               | `boolean` | `true` | Sincroniza o índice de memória quando uma sessão é iniciada                    |
| `onSearch`                     | `boolean` | `true` | Sincroniza sob demanda durante a busca após detectar alterações no conteúdo    |
| `watch`                        | `boolean` | `true` | Monitora os arquivos de memória (chokidar) e agenda a reindexação após mudanças |
...
<ParamField path="chunking.tokens" type="number">
  Tamanho do bloco em tokens usado ao dividir fontes de memória antes de gerar embeddings (padrão: 400).
</ParamField>
<ParamField path="chunking.overlap" type="number">
  Sobreposição de tokens entre blocos adjacentes para preservar o contexto próximo aos limites de divisão (padrão: 80).
</ParamField>

<Note>
Alterar `chunking.tokens` ou `chunking.overlap` modifica os limites dos blocos e invalida a identidade do índice existente (consulte o Aviso em Seleção de provedor).
</Note>
...
| Chave        | Tipo     | Padrão | Descrição                                                   |
| ------------ | -------- | ------ | ----------------------------------------------------------- |
| `maxResults` | `number` | `6`    | Máximo de resultados de memória retornados antes da injeção |
...
<Tabs>
  <Tab title="MMR (diversidade)">
    | Chave         | Tipo      | Padrão | Descrição                                      |
    | ------------- | --------- | ------ | ---------------------------------------------- |
    | `mmr.enabled` | `boolean` | `false` | Ativa o reordenamento por MMR                  |
    | `mmr.lambda`  | `number`  | `0.7`   | 0 = diversidade máxima, 1 = relevância máxima |
  </Tab>
  <Tab title="Decaimento temporal (recenticidade)">
    | Chave                        | Tipo      | Padrão | Descrição                              |
    | ---------------------------- | --------- | ------ | -------------------------------------- |
    | `temporalDecay.enabled`      | `boolean` | `false` | Ativa o aumento por recenticidade     |
    | `temporalDecay.halfLifeDays` | `number`  | `30`    | A pontuação cai pela metade a cada N dias |

    Arquivos perenes (`MEMORY.md` e arquivos sem data em `memory/`) nunca sofrem decaimento.
...
Os caminhos podem ser absolutos ou relativos ao espaço de trabalho. Os diretórios são verificados recursivamente em busca de arquivos `.md`. O tratamento de links simbólicos depende do backend ativo: o mecanismo integrado ignora links simbólicos, enquanto o QMD segue o comportamento do scanner QMD subjacente.
...
<Note>
Aplica-se somente a arquivos em `extraPaths`. As raízes de memória padrão continuam aceitando apenas Markdown. Requer `gemini-embedding-2-preview`. `fallback` deve ser `"none"`.
</Note>
...
Evita gerar novamente os embeddings de texto inalterado durante a reindexação ou atualizações de transcrições. Deixe `maxEntries` não definido para um cache ilimitado; defina-o quando o crescimento do uso do disco for mais importante que a velocidade máxima de reindexação. Quando definido, as entradas mais antigas (pelo horário da última atualização) são removidas primeiro assim que o cache ultrapassa o limite.
...
| Chave                         | Tipo      | Padrão  | Descrição                              |
| ----------------------------- | --------- | ------- | -------------------------------------- |
| `remote.nonBatchConcurrency`  | `number`  | `4`     | Embeddings inline em paralelo          |
...
Disponível para `gemini`, `openai` e `voyage`. O processamento em lote da OpenAI costuma ser mais rápido e barato para grandes preenchimentos retroativos.

`remote.nonBatchConcurrency` controla as chamadas inline de embeddings usadas por provedores locais/auto-hospedados e provedores hospedados quando as APIs de lote do provedor não estão ativas. O padrão do Ollama é `1` para indexação sem lote, evitando sobrecarregar hosts locais menores; defina um valor maior em máquinas mais robustas.

Isso é separado de `sync.embeddingBatchTimeoutSeconds`, que controla o tempo limite das chamadas inline de embeddings.

---

## Pesquisa na memória da sessão (experimental)

Indexe as transcrições das sessões e disponibilize-as por meio de `memory_search`:
...
padrão `tree` expõe apenas a sessão atual e as sessões iniciadas por ela. Para
recuperar, em uma sessão diferente, uma sessão não relacionada do mesmo agente
despachada pelo Gateway, como uma DM, amplie intencionalmente a visibilidade para
`agent` (ou para `all` somente quando a recuperação entre agentes também for
necessária e a política entre agentes permitir).

Os exemplos abaixo colocam essas configurações em `agents.defaults`. Você também
pode aplicar configurações `memorySearch` equivalentes em uma substituição por
agente quando apenas um agente deve indexar e pesquisar transcrições de sessões.

Para recuperação do Gateway para DM pelo mesmo agente:
...
`sources: ["sessions"]` não exportam, por si só, as transcrições para o QMD.
Defina também `memory.qmd.sessions.enabled: true`.
...
| `store.vector.extensionPath` | `string`  | incluído | Substituir o caminho do sqlite-vec          |

Quando o sqlite-vec não está disponível, o OpenClaw recorre automaticamente à similaridade de cosseno no próprio processo.
...
| Chave                    | Tipo      | Padrão   | Descrição                                                                                              |
| ------------------------ | --------- | -------- | ------------------------------------------------------------------------------------------------------ |
| `command`                | `string`  | `qmd`    | Caminho do executável do QMD; defina um caminho absoluto quando o `PATH` do serviço for diferente do shell |
| `searchMode`             | `string`  | `search` | Comando de busca: `search`, `vsearch`, `query`                                                         |
| `rerank`                 | `boolean` | --       | Defina como `false` com `searchMode: "query"` e QMD 2.1+ para ignorar o reordenamento do QMD           |
...
| `sessions.exportDir`     | `string`  | --       | Diretório de exportação                                                                                |
  ...
  `searchMode: "search"` usa apenas pesquisa lexical/BM25. O OpenClaw não executa sondagens de prontidão de vetores semânticos nem manutenção de embeddings do QMD nesse modo, inclusive durante `memory status --deep`; `vsearch` e `query` continuam exigindo prontidão de vetores e embeddings do QMD.

  `rerank: false` altera apenas o modo `query` do QMD e exige o QMD 2.1 ou mais recente. No modo CLI direto, o OpenClaw passa `--no-rerank`; no modo MCP baseado em mcporter, ele passa `rerank: false` para a ferramenta de consulta unificada do QMD. Deixe essa opção sem definir para usar o comportamento padrão de reranqueamento de consultas do QMD.

  O OpenClaw prefere os formatos atuais de coleções e consultas MCP do QMD, mas mantém as versões mais antigas do QMD funcionando ao tentar sinalizadores de padrão de coleção compatíveis e nomes de ferramentas MCP mais antigos quando necessário. Quando o QMD informa compatibilidade com vários filtros de coleção, as coleções da mesma fonte são pesquisadas com um único processo do QMD; compilações mais antigas do QMD mantêm o caminho de compatibilidade por coleção. Mesma fonte significa que as coleções de memória persistente (arquivos de memória padrão mais caminhos personalizados) são agrupadas, enquanto as coleções de transcrições de sessões permanecem em um grupo separado para que a diversificação de fontes ainda tenha ambas as entradas.
  ...
  ### Integração com mcporter

  Todas as opções ficam em `memory.qmd.mcporter`. Encaminha as pesquisas do QMD por meio de um daemon MCP `mcporter` de longa duração, em vez de iniciar `qmd` a cada consulta, reduzindo a sobrecarga de inicialização a frio para modelos maiores.

  | Chave         | Tipo      | Padrão  | Descrição                                                                      |
  | ------------- | --------- | ------- | ------------------------------------------------------------------------------ |
  | `enabled`     | `boolean` | `false` | Encaminha chamadas do QMD pelo mcporter em vez de iniciar `qmd` a cada requisição |
  | `serverName`  | `string`  | `qmd`   | Nome do servidor mcporter que executa `qmd mcp` com `lifecycle: keep-alive`    |
  | `startDaemon` | `boolean` | `true`  | Inicia automaticamente o daemon mcporter quando `enabled` é verdadeiro         |

  Exige que o `mcporter` esteja instalado e disponível no PATH, além de um servidor mcporter configurado para executar `qmd mcp`. Mantenha desativado em configurações locais mais simples nas quais o custo de iniciar um processo por consulta seja aceitável.
  ...
  <Accordion title="Programação de atualizações">
    ...
    | `update.embedInterval`    | `string`  | `60m`   | Cadência separada de geração de embeddings                |
    | `update.commandTimeoutMs` | `number`  | `30000` | Tempo limite para comandos de manutenção do QMD (listar/adicionar coleções) |
  ...
  <Accordion title="Limites">
    | Chave                       | Tipo     | Padrão  | Descrição                |
    | --------------------------- | -------- | ------- | ------------------------------ |
    | `limits.maxResults`       | `number` | `4`     | Máximo de resultados da pesquisa         |
    | `limits.maxSnippetChars`  | `number` | `450`   | Limita o comprimento do trecho       |
  ...
  <Accordion title="Escopo">
    ...
    O padrão distribuído permite apenas mensagens diretas/conversas diretas, negando grupos e outros tipos de canal. `match.keyPrefix` corresponde à chave normalizada da sessão; `match.rawKeyPrefix` corresponde à chave bruta, incluindo `agent:<id>:`.
...
Quando a inicialização do QMD na inicialização do Gateway está habilitada, o OpenClaw inicia o QMD apenas para agentes qualificados. Se `update.onBoot` for verdadeiro e nenhuma manutenção de intervalo/incorporação estiver configurada, a inicialização usará um gerenciador de execução única para a atualização da inicialização e o fechará. Se um intervalo de atualização ou incorporação estiver configurado, a inicialização abrirá o gerenciador QMD de longa duração para que ele possa controlar o observador e os temporizadores de intervalo; `update.onBoot: false` ignora apenas a atualização imediata da inicialização.
...
| `phases.deep.maxPromotedSnippetTokens` | `number`  | `160`         | Máximo estimado de tokens mantidos de cada trecho de recordação de curto prazo promovido para `MEMORY.md`; os metadados de proveniência permanecem visíveis |
...
- `dreaming.model` usa a barreira de confiança de subagente existente do plugin; defina `plugins.entries.memory-core.subagent.allowModelOverride: true` antes de habilitá-lo.
- O Dream Diary tenta novamente uma vez com o modelo padrão da sessão quando o modelo configurado não está disponível. Falhas de confiança ou de lista de permissões são registradas e não geram novas tentativas silenciosas.
...
- [Pesquisa na memória](/pt-BR/concepts/memory-search)
