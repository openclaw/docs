...
read_when:
    ...
    - Alteração na forma como a interface de Instâncias exibe os nomes dos dispositivos
...
| `NOTICE.md`                            | SHAs fixados dos commits do projeto upstream    |
| `LICENSE.apple-device-identifiers.txt` | Licença MIT do projeto upstream                 |

## Fonte dos dados

Incorporados do repositório do GitHub `kyle-seongwoo-jun/apple-device-identifiers`, licenciado sob a licença MIT. Os arquivos JSON são fixados nos SHAs dos commits registrados em `NOTICE.md` para manter as compilações determinísticas.

## Atualização do banco de dados

1. Escolha os SHAs dos commits upstream a serem fixados (um para iOS e outro para macOS).
...
4. Confirme se `LICENSE.apple-device-identifiers.txt` ainda corresponde à versão upstream; substitua-o se a licença upstream tiver sido alterada.
