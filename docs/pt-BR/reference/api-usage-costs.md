...
read_when:
    ...
    - Você precisa auditar chaves, custos e a visibilidade do uso
...
x-i18n:
    generated_at: "2026-07-12T00:20:23Z"
    model: gpt-5.6
...
- Adiciona um **custo estimado** para a última resposta quando o OpenClaw tem metadados de uso e preços locais para o modelo ativo, incluindo provedores sem chave de API com preços definidos explicitamente, como os modelos `aws-sdk` do Bedrock.
- Se o instantâneo da sessão ativa tiver poucos dados, `/status` recupera os contadores de tokens/cache e o rótulo do modelo ativo da entrada de uso mais recente da transcrição. Valores ativos existentes diferentes de zero prevalecem sobre os dados da transcrição; um total da transcrição equivalente ao tamanho do prompt ainda pode prevalecer quando o total armazenado está ausente ou é menor.

**`/usage`** (rodapé por mensagem)

- `/usage full` acrescenta um rodapé de uso a cada resposta, incluindo o **custo estimado** quando os preços locais estão configurados e os metadados de uso estão disponíveis.
- `/usage tokens` mostra apenas os tokens. Runtimes de OAuth/token e CLI no estilo de assinatura mostram apenas os tokens, a menos que forneçam metadados de uso compatíveis e um preço local explícito.
- `/usage cost` exibe um resumo local dos custos; `/usage off` desativa o rodapé.
- Observação sobre a Gemini CLI: tanto a saída `stream-json` quanto a saída legada `json` incluem o uso em `stats`. O OpenClaw normaliza `stats.cached` como `cacheRead` e, quando necessário, deriva os tokens de entrada de `stats.input_tokens - stats.cached`.
...
- Identifica diretamente a escala do gráfico diário. Um selo `√` significa que a compressão por raiz quadrada está mantendo visíveis os dias de baixo uso.
- Esses totais descrevem o histórico de sessões local disponível, não uma fatura do provedor nem um registro de cobrança de toda a duração. A interface avisa quando faltam preços para algumas entradas.

**Janelas de uso da CLI** (cotas do provedor, não custo por mensagem)

- `openclaw status --usage` e `openclaw channels list` mostram as **janelas de uso** dos provedores como `X% left`.
- Provedores atuais de janelas de uso: Anthropic, ClawRouter, DeepSeek, GitHub Copilot, Gemini CLI, MiniMax, OpenAI (abrange autenticação por OAuth/token do ChatGPT/Codex), Xiaomi e z.ai. Consulte [CLI de modelos](/pt-BR/cli/models) e [CLI de canais](/pt-BR/cli/channels) para ver a lista completa de provedores/opções.
- Os campos brutos `usage_percent` / `usagePercent` da MiniMax informam a cota restante, portanto o OpenClaw os inverte; campos baseados em contagem prevalecem quando presentes. Se a resposta incluir uma matriz `model_remains`, o OpenClaw seleciona a entrada do modelo de chat, deriva o rótulo da janela dos carimbos de data e hora quando necessário e inclui o nome do modelo no rótulo do plano.
- A autenticação de uso vem de hooks específicos do provedor quando disponíveis; caso contrário, o OpenClaw recorre às credenciais OAuth/chave de API correspondentes nos perfis de autenticação, no ambiente ou na configuração.

Consulte [Uso e custos de tokens](/pt-BR/reference/token-use) para ver exemplos detalhados.

<Note>
A Anthropic confirmou que a reutilização da Claude CLI (incluindo `claude -p`) é um padrão de integração autorizado, a menos que publique uma nova política. A Anthropic não fornece uma estimativa monetária por mensagem, portanto `/usage full` não pode mostrar o custo do uso da Claude CLI.
</Note>
...
- **Skills**: `skills.entries.<name>.apiKey`, que pode exportar a chave para o ambiente do processo da Skills.

## Recursos que podem consumir chaves

### Respostas do modelo principal (chat + ferramentas)

Cada resposta ou chamada de ferramenta é executada no provedor do modelo atual. Essa é a principal fonte de uso e custo, incluindo planos hospedados no estilo de assinatura cuja cobrança ocorre fora da interface local do OpenClaw: OpenAI Codex, Alibaba Cloud Model Studio Coding Plan, MiniMax Coding Plan, Z.AI/GLM Coding Plan e o fluxo de login do Claude da Anthropic com Extra Usage ativado.

Consulte [Modelos](/pt-BR/providers/models) para ver a configuração de preços e [Uso e custos de tokens](/pt-BR/reference/token-use) para saber como são exibidos.

### Compreensão de mídia (áudio/imagem/vídeo)

A mídia recebida pode ser resumida ou transcrita por uma API de provedor antes da execução do pipeline de resposta. O suporte a provedores é registrado por Plugin e muda à medida que Plugins são adicionados; consulte [Compreensão de mídia](/pt-BR/nodes/media-understanding) para ver a lista e a configuração atuais.

### Geração de imagens e vídeos

`image_generate` e `video_generate` encaminham para qualquer provedor configurado que esteja disponível. A geração de imagens pode inferir um provedor padrão baseado em autenticação quando `agents.defaults.imageGenerationModel` não está definido; a geração de vídeos exige um `agents.defaults.videoGenerationModel` explícito (por exemplo, `qwen/wan2.6-t2v`).
...
Os caminhos legados de configuração `tools.web.search.*` ainda são carregados por uma camada de compatibilidade, mas deixaram de ser a opção recomendada.
...
Consulte [Ferramentas da web](/pt-BR/tools/web).

### Ferramenta de busca de conteúdo da web (Firecrawl)

`web_fetch` pode chamar o Firecrawl com acesso inicial sem chave; adicione `FIRECRAWL_API_KEY` (ou `plugins.entries.firecrawl.config.webFetch.apiKey`) para obter limites maiores. Se o Firecrawl não estiver configurado, a ferramenta recorre à busca direta mais o Plugin `web-readability` incluído (sem API paga). Desative `plugins.entries.web-readability.enabled` para ignorar a extração local de legibilidade.

Consulte [Ferramentas da web](/pt-BR/tools/web).

### Instantâneos de uso dos provedores (status/integridade)

`openclaw status --usage` e `openclaw models status --json` chamam endpoints de uso dos provedores para mostrar janelas de cota ou a integridade da autenticação. As chamadas têm baixo volume, mas ainda acessam as APIs dos provedores.

Consulte [CLI de modelos](/pt-BR/cli/models).

### Resumo de proteção da Compaction

A proteção da Compaction pode resumir o histórico da sessão usando o modelo atual, o que invoca APIs do provedor quando executada.
...
`openclaw models scan` pode sondar modelos do OpenRouter e usa `OPENROUTER_API_KEY` quando a sondagem está ativada.
...
Skills pode armazenar `apiKey` em `skills.entries.<name>.apiKey`. Se uma Skills usar essa chave em uma API externa, o custo seguirá o provedor da Skills.
