...
x-i18n:
    generated_at: "2026-07-16T12:13:12Z"
    model: gpt-5.6
...
O Nextcloud Talk é um plugin de canal disponível para download (`@openclaw/nextcloud-talk`) que conecta o OpenClaw a uma instância auto-hospedada do Nextcloud por meio de um bot de webhook do Talk. Há suporte para mensagens diretas, salas, reações e mensagens em markdown; mídias são enviadas como URLs.
...
A partir de um checkout local (fluxos de trabalho de desenvolvimento):
...
2. No servidor Nextcloud, crie um bot:
   ...
   Mantenha `--feature response`: sem isso, as respostas de saída falham com 401. Corrija um bot existente com `./occ talk:bot:state --feature webhook --feature response --feature reaction <botId> 1`.
...
4. Configure o OpenClaw:
   ...
   Configuração pela CLI (`--url`/`--token` são aliases para os campos explícitos; `nc-talk` e `nc` funcionam como aliases do canal):
...
- A URL do webhook deve estar acessível a partir do servidor Nextcloud; defina `webhookPublicUrl` quando o Gateway estiver atrás de um proxy. As solicitações do webhook são assinadas com HMAC-SHA256 usando o segredo do bot; assinaturas inválidas são rejeitadas e sujeitas a limitação de taxa.
- A API do bot não oferece suporte ao upload de mídias; mídias de saída são acrescentadas como uma linha `Attachment: <url>`.
- O payload do webhook não diferencia mensagens diretas de salas; defina `apiUser` + `apiPassword` para ativar consultas de tipo de sala (armazenadas em cache por cerca de 5 minutos). Sem essas configurações, todas as conversas são tratadas como salas.
- As solicitações de saída passam pela proteção contra SSRF. Para um host Nextcloud em uma rede privada/interna confiável, habilite essa opção com `channels.nextcloud-talk.network.dangerouslyAllowPrivateNetwork: true`.
...
| Salas             | Compatível        |
| Threads           | Incompatível      |
| Mídia             | Somente por URL   |
| Reações           | Compatível        |
| Comandos nativos  | Incompatível      |
...
- `channels.nextcloud-talk.botSecretFile`: caminho do segredo em arquivo comum. Links simbólicos são rejeitados.
...
- `channels.nextcloud-talk.allowFrom`: lista de permissões para mensagens diretas (IDs de usuário).
- `channels.nextcloud-talk.groupPolicy`: `allowlist | open | disabled` (padrão: allowlist).
- `channels.nextcloud-talk.groupAllowFrom`: lista de remetentes permitidos nas salas (IDs de usuário); usa `allowFrom` como alternativa quando não definido.
...
- `channels.nextcloud-talk.historyLimit`: limite do histórico do grupo (0 desativa).
- `channels.nextcloud-talk.dmHistoryLimit`: limite do histórico de mensagens diretas (0 desativa).
- `channels.nextcloud-talk.dms`: substituições por mensagem direta, usando o ID do usuário como chave (`historyLimit`).
- `channels.nextcloud-talk.textChunkLimit`: tamanho dos blocos de texto de saída em caracteres (padrão: 4000).
- `channels.nextcloud-talk.streaming.chunkMode`: `length` (padrão) ou `newline` para dividir em linhas em branco (limites de parágrafos) antes da divisão por tamanho.
- `channels.nextcloud-talk.streaming.block.enabled`: ativa ou desativa o streaming de blocos para este canal.
- `channels.nextcloud-talk.streaming.block.coalesce`: ajuste da coalescência do streaming de blocos.
...
- `channels.nextcloud-talk.accounts.<id>`: substituições por conta (mesmas chaves); `defaultAccount` seleciona o padrão. As variáveis de ambiente `NEXTCLOUD_TALK_BOT_SECRET` / `NEXTCLOUD_TALK_API_PASSWORD` aplicam-se somente à conta padrão.
