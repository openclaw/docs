...
summary: Suporte nativo ao iMessage via imsg (JSON-RPC sobre stdio), com ações de API privada para respostas, tapbacks, efeitos, anexos e gerenciamento de grupos. Preferencial para novas configurações de iMessage do OpenClaw quando os requisitos do host forem compatíveis.
title: iMessage
x-i18n:
...
<Note>
Para implantações do OpenClaw iMessage, use `imsg` em um host macOS Messages conectado. Se o seu Gateway roda em Linux ou Windows, aponte `channels.imessage.cliPath` para um wrapper SSH que execute `imsg` no Mac.

**A recuperação após indisponibilidade do Gateway é opcional.** Quando ativada (`channels.imessage.catchup.enabled: true`), o Gateway reproduz mensagens recebidas que chegaram ao `chat.db` enquanto ele estava offline (falha, reinicialização, Mac em repouso) na próxima inicialização. Desativada por padrão — veja [Recuperação após indisponibilidade do Gateway](#catching-up-after-gateway-downtime). Fecha [openclaw#78649](https://github.com/openclaw/openclaw/issues/78649).
</Note>

<Warning>
O suporte ao BlueBubbles foi removido. Migre configurações `channels.bluebubbles` para `channels.imessage`; o OpenClaw oferece suporte ao iMessage apenas por meio do `imsg`. Comece por [Remoção do BlueBubbles e o caminho do imsg para iMessage](/pt-BR/announcements/bluebubbles-imessage) para o anúncio curto, ou [Vindo do BlueBubbles](/pt-BR/channels/imessage-from-bluebubbles) para a tabela completa de migração.
</Warning>
...
<CardGroup cols={3}>
  ...
  <Card title="Remote Mac" icon="terminal" href="#remote-mac-over-ssh">
    Use um wrapper SSH quando o Gateway não estiver rodando no Mac com Messages.
  </Card>
  <Card title="Configuration reference" icon="settings" href="/pt-BR/gateway/config-channels#imessage">
...
- Acesso Total ao Disco é obrigatório para o contexto do processo que executa OpenClaw/`imsg` (acesso ao banco de dados do Messages).
- A permissão de Automação é obrigatória para enviar mensagens por meio do Messages.app.
- Para ações avançadas (reagir / editar / desfazer envio / resposta encadeada / efeitos / operações de grupo), a Proteção de Integridade do Sistema deve estar desativada — veja [Ativação da API privada do imsg](#enabling-the-imsg-private-api) abaixo. Envio/recebimento básico de texto e mídia funciona sem isso.

<Tip>
As permissões são concedidas por contexto de processo. Se o Gateway rodar sem interface (LaunchAgent/SSH), execute um comando interativo único nesse mesmo contexto para acionar os prompts:
...
- **Modo básico** (padrão, sem necessidade de alterações no SIP): texto e mídia de saída via `send`, monitoramento/histórico de entrada, lista de chats. É isso que você obtém imediatamente com uma nova instalação `brew install steipete/tap/imsg` mais as permissões padrão do macOS acima.
...
> Recursos avançados como `read`, `typing`, `launch`, envio enriquecido com suporte de ponte, mutação de mensagens e gerenciamento de chats são opcionais. Eles exigem que o SIP esteja desativado e que uma dylib auxiliar seja injetada em `Messages.app`. `imsg launch` se recusa a injetar quando o SIP está ativado.
...
<Warning>
**Desativar o SIP é uma troca real de segurança.** O SIP é uma das proteções centrais do macOS contra a execução de código de sistema modificado; desativá-lo no sistema inteiro abre superfície de ataque adicional e efeitos colaterais. Em especial, **desativar o SIP em Macs Apple Silicon também desativa a capacidade de instalar e executar apps iOS no seu Mac**.

Trate isso como uma escolha operacional deliberada, não como padrão. Se o seu modelo de ameaças não puder tolerar o SIP desativado, o iMessage incluído fica limitado ao modo básico — apenas envio/recebimento de texto e mídia, sem reações / edição / desfazer envio / efeitos / operações de grupo.
</Warning>
...
1. **Instale (ou atualize) `imsg`** no Mac que executa Messages.app:
   ...
   A saída de `imsg status --json` relata `bridge_version`, `rpc_methods` e `selectors` por método para que você veja o que a versão atual oferece suporte antes de começar.

2. **Desative a Proteção de Integridade do Sistema.** Isso é específico da versão do macOS porque o requisito subjacente da Apple depende do SO e do hardware:
   ...
   - **macOS 11+, Apple Silicon:** sequência de inicialização pelo botão liga/desliga para entrar na Recuperação; em versões recentes do macOS, segure a tecla **Shift esquerda** ao clicar em Continuar, depois `csrutil disable`. Configurações de máquina virtual seguem um fluxo separado — tire um snapshot da VM primeiro.
   - **macOS 26 / Tahoe:** políticas de validação de biblioteca e verificações de direitos privados do `imagent` ficaram ainda mais rígidas; `imsg` pode precisar de uma versão atualizada para acompanhar. Se a injeção de `imsg launch` ou `selectors` específicos começarem a retornar falso após uma atualização principal do macOS, verifique as notas de versão do `imsg` antes de presumir que a etapa do SIP foi bem-sucedida.

   Siga o fluxo de Modo de Recuperação da Apple para o seu Mac para desativar o SIP antes de executar `imsg launch`.

3. **Injete o auxiliar.** Com o SIP desativado e Messages.app conectado:
...
4. **Verifique a ponte pelo OpenClaw:**
   ...
   A entrada do iMessage deve relatar `works`, e `imsg status --json | jq '.selectors'` deve mostrar `retractMessagePart: true`, além de quaisquer seletores de edição / digitação / leitura expostos pela sua versão do macOS. O controle por método do Plugin OpenClaw em `actions.ts` anuncia apenas ações cujo seletor subjacente é `true`, então a superfície de ações que você vê na lista de ferramentas do agente reflete o que a ponte realmente consegue fazer neste host.
...
Se SIP desativado não for aceitável para o seu modelo de ameaças:
...
- Você pode executar um Mac não Apple Silicon separado (ou um Mac dedicado para bot) com o SIP desativado para a carga de trabalho do iMessage, mantendo o SIP ativado nos seus dispositivos principais. Veja [Usuário macOS dedicado para bot (identidade iMessage separada)](#deployment-patterns) abaixo.
...
<Tabs>
  <Tab title="DM policy">
    ...
    Entradas da lista de permissões podem ser handles, grupos estáticos de acesso de remetente (`accessGroup:<name>`) ou destinos de chat (`chat_id:*`, `chat_guid:*`, `chat_identifier:*`).
  ...
  <Tab title="Group policy + mentions">
    ...
    Fallback em runtime: se `groupAllowFrom` não estiver definido, as verificações de remetente de grupo do iMessage fazem fallback para `allowFrom` quando disponível.
    Observação de runtime: se `channels.imessage` estiver completamente ausente, o runtime faz fallback para `groupPolicy="allowlist"` e registra um aviso (mesmo se `channels.defaults.groupPolicy` estiver definido).

    <Warning>
    O roteamento de grupo tem **dois** controles de lista de permissões executados em sequência, e ambos devem passar:

    1. **Lista de permissões de remetente / destino de chat** (`channels.imessage.groupAllowFrom`) — handle, `chat_guid`, `chat_identifier` ou `chat_id`.
    2. **Registro de grupos** (`channels.imessage.groups`) — com `groupPolicy: "allowlist"`, este controle exige uma entrada curinga `groups: { "*": { ... } }` (define `allowAll = true`) ou uma entrada explícita por `chat_id` em `groups`.

    Se o controle 2 não tiver nada, toda mensagem de grupo será descartada. O Plugin emite dois sinais de nível `warn` no nível de log padrão:
    ...
    Se essas linhas `warn` aparecerem no log do gateway, a porta 2 está caindo — adicione o bloco `groups`.
    </Warning>

    Mencione o controle de acesso para grupos:
...
Vínculos persistentes configurados são compatíveis por meio de entradas de nível superior `bindings[]` com `type: "acp"` e `match.channel: "imessage"`.
...
<AccordionGroup>
  <Accordion title="Usuário macOS dedicado para bot (identidade iMessage separada)">
    Use um ID Apple e um usuário macOS dedicados para que o tráfego do bot fique isolado do seu perfil pessoal do Messages.

    Fluxo típico:

    1. Crie/faça login em um usuário macOS dedicado.
    2. Entre no Messages com o ID Apple do bot nesse usuário.
...
<AccordionGroup>
  <Accordion title="Anexos e mídia">
    - a ingestão de anexos de entrada fica **desativada por padrão** — defina `channels.imessage.includeAttachments: true` para encaminhar fotos, memorandos de voz, vídeos e outros anexos para o agente. Com isso desativado, iMessages contendo apenas anexos são descartadas antes de chegar ao agente e podem não produzir nenhuma linha de log `Inbound message`.
...
Quando `imsg launch` está em execução e `openclaw channels status --probe` informa `privateApi.available: true`, a ferramenta de mensagens pode usar ações nativas do iMessage além de envios normais de texto.
...
<AccordionGroup>
  <Accordion title="Ações disponíveis">
    - **react**: Adicionar/remover tapbacks do iMessage (`messageId`, `emoji`, `remove`). Tapbacks compatíveis mapeiam para amor, curtir, não curtir, risada, ênfase e pergunta.
    - **reply**: Enviar uma resposta encadeada para uma mensagem existente (`messageId`, `text` ou `message`, mais `chatGuid`, `chatId`, `chatIdentifier` ou `to`).
    ...
    - **renameGroup**, **setGroupIcon**, **addParticipant**, **removeParticipant**, **leaveGroup**: Gerenciar chats de grupo quando o alvo atual é uma conversa de grupo.

  </Accordion>

  <Accordion title="IDs de mensagem">
    O contexto de entrada do iMessage inclui valores `MessageSid` curtos e GUIDs completos de mensagem quando disponíveis. IDs curtos são escopados ao cache recente de respostas em memória e são verificados contra o chat atual antes do uso. Se um ID curto expirou ou pertence a outro chat, tente novamente com o `MessageSidFull` completo.

  </Accordion>

  <Accordion title="Detecção de recursos">
    O OpenClaw oculta ações de API privada somente quando o status em cache da sondagem diz que a bridge está indisponível. Se o status for desconhecido, as ações permanecem visíveis e disparam sondagens de forma preguiçosa para que a primeira ação possa ser bem-sucedida após `imsg launch` sem uma atualização manual de status separada.

  </Accordion>

  <Accordion title="Recibos de leitura e digitação">
    Quando a bridge de API privada está ativa, chats de entrada aceitos são marcados como lidos antes do despacho e um balão de digitação é mostrado ao remetente enquanto o agente gera. Desative a marcação de leitura com:
    ...
    Builds mais antigos do `imsg` que antecedem a lista de recursos por método desativarão silenciosamente digitação/leitura; o OpenClaw registra um aviso único por reinicialização para que a ausência do recibo seja atribuível.
...
As duas linhas chegam ao OpenClaw com ~0,8-2,0 s de intervalo na maioria das configurações. Sem coalescência, o agente recebe o comando sozinho no turno 1, responde (geralmente "envie-me a URL") e só vê a URL no turno 2 — nesse ponto, o contexto do comando já foi perdido. Esse é o pipeline de envio da Apple, não algo introduzido pelo OpenClaw ou pelo `imsg`.

`channels.imessage.coalesceSameSenderDms` faz uma DM optar por mesclar linhas consecutivas do mesmo remetente em um único turno do agente. Chats de grupo continuam a ser despachados por mensagem para que a estrutura de turnos com vários usuários seja preservada.

<Tabs>
  <Tab title="Quando habilitar">
    Habilite quando:

    - Você envia Skills que esperam `command + payload` em uma mensagem (dump, paste, save, queue etc.).
    ...
    - Todos os seus fluxos são comandos de execução única sem acompanhamentos de payload.
  ...
  <Tab title="Habilitação">
    ...
    Com a flag ativada e sem `messages.inbound.byChannel.imessage` explícito, a janela de debounce aumenta para **2500 ms** (o padrão legado é 0 ms — sem debounce). A janela maior é necessária porque a cadência de envio dividido da Apple, de 0,8-2,0 s, não cabe em um padrão mais curto.
  ...
  <Tab title="Compromissos">
    - **Latência adicionada para mensagens de DM.** Com a flag ativada, cada DM (incluindo comandos de controle independentes e respostas de acompanhamento com texto único) espera até a janela de debounce antes do despacho, caso uma linha de payload esteja chegando. Mensagens de chat em grupo mantêm despacho instantâneo.
    ...
    - **Somente DM.** Chats em grupo seguem para despacho por mensagem, para que o bot continue responsivo quando várias pessoas estiverem digitando.
...
| ------------------------------------------------------------------- | --------------------- | ---------------------------------------------- | -------------------------------------------------------------------------- |
| `Dump https://example.com` (um envio)                               | 2 linhas com ~1 s de diferença | Dois turnos do agente: "Dump" sozinho, depois a URL | Um turno: texto mesclado `Dump https://example.com`                        |
...
| `/status` (comando independente)                                    | 1 linha               | Despacho instantâneo                           | **Espera até a janela e então despacha**                                    |
| URL colada sozinha                                                  | 1 linha               | Despacho instantâneo                           | Despacho instantâneo (apenas uma entrada no bucket)                        |
| Texto + URL enviados como duas mensagens separadas deliberadas, minutos de intervalo | 2 linhas fora da janela | Dois turnos                                    | Dois turnos (a janela expira entre elas)                                   |
...
| Duas pessoas digitando em um chat em grupo                          | N linhas de M remetentes | M+ turnos (um por bucket de remetente)         | M+ turnos — chats em grupo não são aglutinados                             |

## Recuperação após indisponibilidade do Gateway

Quando o Gateway está offline (falha, reinicialização, repouso do Mac, máquina desligada), `imsg watch` retoma a partir do estado atual do `chat.db` quando o Gateway volta a ficar ativo — por padrão, qualquer coisa que chegou durante o intervalo nunca é vista. A recuperação reproduz essas mensagens na próxima inicialização para que o agente não perca silenciosamente o tráfego de entrada.

A recuperação fica **desabilitada por padrão**. Habilite por canal:
...
### Semântica de cursor e novas tentativas
...
- Após `maxFailureRetries` exceções consecutivas contra o mesmo `guid`, a recuperação registra um `warn` e força o cursor a avançar para além da mensagem travada, para que inicializações subsequentes possam progredir.
- GUIDs dos quais já se desistiu são ignorados ao serem vistos (sem tentativa de despacho) em execuções posteriores e contabilizados em `skippedGivenUp` no resumo da execução.
...
Uma linha `WARN ... capped to perRunLimit` significa que uma única inicialização não drenou todo o backlog. Aumente `perRunLimit` (máx. 500) se seus intervalos excedem regularmente a passagem padrão de 50 linhas.

### Quando deixá-la desativada

- O Gateway é executado continuamente com reinicialização automática por watchdog e os intervalos são sempre < alguns segundos — o padrão desativado é suficiente.
- O volume de DMs é baixo e mensagens perdidas não alterariam o comportamento do agente — a janela inicial `firstRunLookbackMinutes` pode despachar contexto antigo inesperado na primeira habilitação.

Ao ativar a recuperação, a primeira inicialização sem cursor só olha para trás `firstRunLookbackMinutes` (padrão de 30 min), não a janela completa de `maxAgeMinutes` — isso evita reproduzir um longo histórico de mensagens anteriores à habilitação.
...
<AccordionGroup>
  <Accordion title="imsg não encontrado ou RPC sem suporte">
    ...
    Se a sondagem informar que RPC não tem suporte, atualize `imsg`. Se ações de API privada não estiverem disponíveis, execute `imsg launch` na sessão do usuário conectado ao macOS e faça a sondagem novamente. Se o Gateway não estiver em execução no macOS, use a configuração de Mac remoto por SSH acima em vez do caminho local padrão de `imsg`.

  </Accordion>

  <Accordion title="Gateway não está em execução no macOS">
    O `cliPath: "imsg"` padrão deve ser executado no Mac conectado ao Mensagens. No Linux ou Windows, defina `channels.imessage.cliPath` para um script wrapper que faça SSH para esse Mac e execute `imsg "$@"`.
...
```
  ...
  <Accordion title="Anexos remotos falham">
    ...
    - autenticação de chave SSH/SCP a partir do host do gateway
    - a chave do host existe em `~/.ssh/known_hosts` no host do gateway
    - legibilidade do caminho remoto no Mac que executa Mensagens
...
- [Remoção do BlueBubbles e o caminho do imsg para iMessage](/pt-BR/announcements/bluebubbles-imessage) — anúncio e resumo de migração
