...
title: Discord
x-i18n:
    generated_at: "2026-07-03T02:44:12Z"
    model: gpt-5.5
...
Pronto para DMs e canais de guilda pelo Gateway oficial do Discord.
...
<Steps>
  ...
  <Step title="Gere uma URL de convite e adicione o bot ao seu servidor">
    ...
    **Permissões de texto**
    ...
    Este é o conjunto básico para canais de texto normais. Se você planeja postar em threads do Discord, incluindo fluxos de trabalho de canais de fórum ou mídia que criam ou continuam uma thread, ative também **Send Messages in Threads**.
  ...
  <Step title="Permita DMs de membros do servidor">
    ...
    Isso permite que membros do servidor (incluindo bots) enviem DMs para você. Mantenha isso ativado se quiser usar DMs do Discord com o OpenClaw. Se você planeja usar apenas canais de guilda, pode desativar DMs depois do emparelhamento.
...
```
    ...
    Se seu host for bloqueado ou tiver limite de taxa pela consulta de aplicativo de inicialização do Discord, defina o ID de aplicativo/cliente do Discord no Portal de Desenvolvedores para que a inicialização possa pular essa chamada REST. Use `channels.discord.applicationId` para a conta padrão, ou `channels.discord.accounts.<accountId>.applicationId` quando executar vários bots do Discord.
...
```
        ...
        Para vários bots do Discord, mantenha cada token de bot e ID de aplicativo na respectiva conta. Um `channels.discord.applicationId` de nível superior é herdado pelas contas, portanto defina-o ali apenas quando todas as contas precisarem usar o mesmo ID de aplicativo.
...
```
  ...
  <Step title="Aprove o primeiro emparelhamento por DM">
    Aguarde até que o Gateway esteja em execução e envie uma DM para seu bot no Discord. Ele responderá com um código de emparelhamento.

    <Tabs>
      <Tab title="Pergunte ao seu agente">
        Envie o código de emparelhamento ao seu agente no canal existente:
...
<Note>
A resolução de token reconhece contas. Valores de token na configuração têm prioridade sobre fallback de env. `DISCORD_BOT_TOKEN` é usado apenas para a conta padrão.
...
Para chamadas de saída avançadas (ferramenta de mensagem/ações de canal), um `token` explícito por chamada é usado para essa chamada. Isso se aplica a ações de envio e leitura/sondagem (por exemplo, ler/pesquisar/buscar/thread/pins/permissões). As configurações de política/tentativa da conta ainda vêm da conta selecionada no snapshot de runtime ativo.
</Note>

## Recomendado: configure um workspace de guilda

Depois que as DMs estiverem funcionando, você pode configurar seu servidor Discord como um workspace completo, onde cada canal recebe sua própria sessão de agente com seu próprio contexto. Isso é recomendado para servidores privados em que há apenas você e seu bot.
...
```
  ...
  <Step title="Permita respostas sem @mention">
    ...
    Em canais de guilda, respostas normais são publicadas automaticamente por padrão. Para salas compartilhadas sempre ativas, opte por `messages.groupChat.visibleReplies: "message_tool"` para que o agente possa ficar à espreita e postar apenas quando decidir que uma resposta no canal é útil. Isso funciona melhor com modelos de última geração e confiáveis em ferramentas, como GPT 5.5. Eventos de sala ambiente ficam silenciosos a menos que a ferramenta envie. Consulte [Eventos de sala ambiente](/pt-BR/channels/ambient-room-events) para a configuração completa do modo de observação.

    Se o Discord mostrar digitação e os logs mostrarem uso de token, mas nenhuma mensagem publicada, verifique se o turno foi configurado como um evento de sala ambiente ou optou por respostas visíveis via ferramenta de mensagem.
...
Agora crie alguns canais no seu servidor Discord e comece a conversar. Seu agente consegue ver o nome do canal, e cada canal recebe sua própria sessão isolada — então você pode configurar `#coding`, `#home`, `#research` ou o que se ajustar ao seu fluxo de trabalho.
...
- A entrega de anúncio de Cron/Heartbeat somente texto para o Discord usa a resposta final visível ao assistente uma vez. Payloads de mídia e componentes estruturados continuam em várias mensagens quando o agente emite vários payloads entregáveis.
...
- Envie uma mensagem ao pai do fórum (`channel:<forumId>`) para criar uma thread automaticamente. O título da thread usa a primeira linha não vazia da sua mensagem.
...
- Use `filename` para sobrescrever o nome de upload quando ele deve corresponder à referência de anexo
...
```
    ...
    Um canal de texto do Discord não tem lista de membros separada. `type: "discord.channelAudience"` modela a associação assim: o remetente da DM é membro da guild configurada e atualmente tem permissão efetiva de `ViewChannel` no canal configurado depois que funções e sobrescritas de canal são aplicadas.
...
```
    ...
    Habilite o **Server Members Intent** do Discord Developer Portal para o bot ao usar grupos de acesso por público de canal. DMs não incluem estado de membro de guild, então o OpenClaw resolve o membro pela REST do Discord no momento da autorização.
  ...
  <Tab title="Guild policy">
    ...
    - listas de permissões opcionais de remetentes: `users` (IDs estáveis recomendados) e `roles` (somente IDs de funções); se qualquer uma for configurada, remetentes serão permitidos quando corresponderem a `users` OU `roles`
    - correspondência direta por nome/tag fica desabilitada por padrão; habilite `channels.discord.dangerouslyAllowNameMatching: true` apenas como modo de compatibilidade emergencial
...
```
  ...
  <Tab title="Mentions and group DMs">
    ...
    `ignoreOtherMentions` opcionalmente descarta mensagens que mencionam outro usuário/função, mas não o bot (exceto @everyone/@here).
...
Use `bindings[].match.roles` para rotear membros de guildas do Discord para agentes diferentes por ID de função. Associações baseadas em função aceitam apenas IDs de função e são avaliadas depois de associações de par ou par pai e antes de associações somente de guilda. Se uma associação também definir outros campos de correspondência (por exemplo `peer` + `guildId` + `roles`), todos os campos configurados devem corresponder.
...
<AccordionGroup>
  <Accordion title="Reply tags and native replies">
    ...
    IDs de mensagem são expostos no contexto/histórico para que agentes possam mirar mensagens específicas.

  </Accordion>

  <Accordion title="Link previews">
    O Discord gera embeds ricos de links para URLs por padrão. O OpenClaw suprime esses embeds gerados em mensagens de saída do Discord por padrão, para que URLs enviadas por agentes permaneçam como links simples, a menos que você opte por ativá-los:
...
```
    ...
    Defina `channels.discord.accounts.<id>.suppressEmbeds` para substituir em uma conta. Envios pela ferramenta de mensagens do agente também podem passar `suppressEmbeds: false` para uma única mensagem. Payloads explícitos de `embeds` do Discord não são suprimidos pela configuração padrão de prévia de links.

  </Accordion>

  <Accordion title="Live stream preview">
    O OpenClaw pode transmitir rascunhos de respostas enviando uma mensagem temporária e editando-a conforme o texto chega. `channels.discord.streaming` aceita `off` | `partial` | `block` | `progress` (padrão). `progress` mantém um rascunho de status editável e o atualiza com o progresso de ferramentas até a entrega final; o rótulo inicial compartilhado é uma linha contínua, então ele rola para fora como o restante quando trabalho suficiente aparece. `streamMode` é um alias de runtime legado. Execute `openclaw doctor --fix` para regravar a configuração persistida para a chave canônica.

    Defina `channels.discord.streaming.mode` como `off` para desabilitar edições de prévia do Discord. Se a transmissão em bloco do Discord estiver explicitamente habilitada, o OpenClaw ignora a transmissão de prévia para evitar transmissão duplicada.
...
```
    ...
    - `streaming.progress.maxLineChars` controla o orçamento de prévia de progresso por linha. A prosa é encurtada nos limites de palavras; detalhes de comando e caminho mantêm sufixos úteis.
    ...
    Oculte o texto bruto de comando/execução enquanto mantém linhas compactas de progresso:
    ...
    A transmissão de prévia é somente texto; respostas com mídia voltam para a entrega normal. Quando a transmissão `block` é explicitamente habilitada, o OpenClaw ignora a transmissão de prévia para evitar transmissão duplicada.
  ...
  <Accordion title="History, context, and thread behavior">
    ...
    - Threads do Discord são roteadas como sessões de canal e herdam a configuração do canal pai, a menos que substituídas.
    - Sessões de thread herdam a seleção de `/model` em nível de sessão do canal pai como fallback somente de modelo; seleções de `/model` locais à thread ainda têm precedência, e o histórico de transcrição do pai não é copiado, a menos que a herança de transcrição esteja habilitada.
    - `channels.discord.thread.inheritParent` (padrão `false`) faz com que novas auto-threads sejam semeadas a partir da transcrição pai. Substituições por conta ficam em `channels.discord.accounts.<id>.thread.inheritParent`.
    ...
    Tópicos de canal são injetados como contexto **não confiável**. Listas de permissão controlam quem pode acionar o agente, não uma fronteira completa de redação de contexto suplementar.

  </Accordion>

  <Accordion title="Thread-bound sessions for subagents">
    O Discord pode associar uma thread a um alvo de sessão para que mensagens de acompanhamento nessa thread continuem roteando para a mesma sessão (incluindo sessões de subagente).

    Comandos:

    - `/focus <target>` associa a thread atual/nova a um alvo de subagente/sessão
    - `/unfocus` remove a associação da thread atual
    - `/agents` mostra execuções ativas e o estado de associação
    - `/session idle <duration|off>` inspeciona/atualiza o desfoco automático por inatividade para associações focadas
    - `/session max-age <duration|off>` inspeciona/atualiza a idade máxima rígida para associações focadas
...
```
    ...
    - `spawnSessions` controla a criação/associação automática de threads para `sessions_spawn({ thread: true })` e spawns de thread ACP. Padrão: `true`.
    - `defaultSpawnContext` controla o contexto nativo de subagente para spawns associados a thread. Padrão: `"fork"`.
    - Chaves obsoletas `spawnSubagentSessions`/`spawnAcpSessions` são migradas por `openclaw doctor --fix`.
    - Se associações de thread estiverem desabilitadas para uma conta, `/focus` e operações relacionadas de associação de thread ficam indisponíveis.
  ...
  <Accordion title="Persistent ACP channel bindings">
    Para workspaces ACP estáveis e "sempre ativos", configure associações ACP tipadas de nível superior mirando conversas do Discord.
...
```
    ...
    - `/acp spawn codex --bind here` associa o canal ou a thread atual no local e mantém mensagens futuras na mesma sessão ACP. Mensagens de thread herdam a associação do canal pai.
    - Em um canal ou thread associado, `/new` e `/reset` redefinem a mesma sessão ACP no local. Associações temporárias de thread podem substituir a resolução de alvo enquanto ativas.
    - `spawnSessions` controla a criação/associação de threads filhas via `--thread auto|here`.

    Consulte [Agentes ACP](/pt-BR/tools/acp-agents) para detalhes do comportamento de associação.
  ...
  <Accordion title="Config writes">
    Escritas de configuração iniciadas pelo canal são habilitadas por padrão.
...
```
  ...
  <Accordion title="Gateway proxy">
    Roteie o tráfego WebSocket do Gateway do Discord e consultas REST de inicialização (ID da aplicação + resolução de lista de permissões) por meio de um proxy HTTP(S) com `channels.discord.proxy`.
    O proxy WebSocket do Gateway do Discord é explícito; conexões WebSocket não herdam variáveis de ambiente de proxy do ambiente do processo do Gateway. Consultas REST de inicialização usam esse proxy quando `channels.discord.proxy` está configurado.
...
```
    ...
    A presença automática mapeia a disponibilidade em runtime para o status do Discord: íntegro => online, degradado ou desconhecido => idle, esgotado ou indisponível => dnd. Substituições opcionais de texto:
...
Controles de ação ficam em `channels.discord.actions.*`.
...
O OpenClaw usa componentes v2 do Discord para aprovações de exec e marcadores entre contextos. Ações de mensagem do Discord também podem aceitar `components` para UI personalizada (avançado; requer construir um payload de componente via a ferramenta discord), enquanto `embeds` legados continuam disponíveis, mas não são recomendados.
...
- Pré-visualizações de URL simples são suprimidas por padrão. Defina `suppressEmbeds: false` em uma ação de mensagem quando um único link de saída deve ser expandido.
...
2. Ative Server Members Intent quando listas de permissão por função/usuário forem usadas.
...
Use `/vc join|leave|status` para controlar sessões. O comando usa o agente padrão da conta e segue as mesmas regras de lista de permissão e política de grupo de outros comandos do Discord.
...
- `voice.tts` substitui `messages.tts` apenas para reprodução de voz `stt-tts`. Modos em tempo real usam `voice.realtime.speakerVoice`.
- `voice.mode` controla o caminho da conversa. O padrão é `agent-proxy`: um front-end de voz em tempo real gerencia o tempo dos turnos, interrupção e reprodução, delega o trabalho substantivo ao agente OpenClaw roteado por meio de `openclaw_agent_consult` e trata o resultado como um prompt digitado do Discord daquele falante. `stt-tts` mantém o fluxo em lote mais antigo de STT mais TTS. `bidi` permite que o modelo em tempo real converse diretamente enquanto expõe `openclaw_agent_consult` para o cérebro do OpenClaw.
...
- `voice.followUsers` permite que o bot entre, se mova e saia da voz do Discord com usuários selecionados. Veja [Seguir usuários na voz](#follow-users-in-voice) para regras de comportamento e exemplos.
...
- Em modos em tempo real, `voice.realtime.provider`, `voice.realtime.model` e `voice.realtime.speakerVoice` configuram a sessão de áudio em tempo real. Para OpenAI Realtime 2 mais o cérebro Codex, use `voice.realtime.model: "gpt-realtime-2"` e `voice.model: "openai/gpt-5.5"`.
- Modos de voz em tempo real incluem pequenos arquivos de perfil `IDENTITY.md`, `USER.md` e `SOUL.md` nas instruções do provedor em tempo real por padrão, para que turnos diretos rápidos mantenham a mesma identidade, base do usuário e persona do agente OpenClaw roteado. Defina `voice.realtime.bootstrapContextFiles` como um subconjunto para personalizar isso, ou `[]` para desativar. Os arquivos de inicialização em tempo real compatíveis são limitados a esses arquivos de perfil; `AGENTS.md` permanece no contexto normal do agente. O contexto de perfil injetado não substitui `openclaw_agent_consult` para trabalho no workspace, fatos atuais, busca de memória ou ações apoiadas por ferramentas.
- No modo em tempo real `agent-proxy` da OpenAI, defina `voice.realtime.requireWakeName: true` para manter a voz em tempo real do Discord silenciosa até que uma transcrição comece ou termine com um nome de ativação. Nomes de ativação configurados devem ter uma ou duas palavras. Se `voice.realtime.wakeNames` não estiver definido, o OpenClaw usa o `name` do agente roteado mais `OpenClaw`, recorrendo ao id do agente mais `OpenClaw`. O bloqueio por nome de ativação desativa a resposta automática do provedor em tempo real, roteia turnos aceitos pelo caminho de consulta do agente OpenClaw e dá uma breve confirmação falada quando um nome de ativação inicial é reconhecido pela transcrição parcial antes da transcrição final chegar.
...
- `voice.realtime.bargeIn` controla se eventos de início de fala no Discord interrompem a reprodução ativa em tempo real. Se não estiver definido, segue a configuração de interrupção de áudio de entrada do provedor em tempo real.
...
- Para uma voz da OpenAI na reprodução do Discord, defina `voice.tts.provider: "openai"` e escolha uma voz de Text-to-speech em `voice.tts.providers.openai.speakerVoice`. `cedar` é uma boa escolha com sonoridade masculina no modelo TTS atual da OpenAI.
- Substituições de `systemPrompt` por canal do Discord se aplicam a turnos de transcrição de voz desse canal de voz.
- Turnos de transcrição de voz derivam o status de proprietário de `allowFrom` (ou `dm.allowFrom`) do Discord para comandos protegidos por proprietário e ações de canal. A visibilidade das ferramentas do agente segue a política de ferramentas configurada para a sessão roteada.
- Voz do Discord é opcional para configurações somente texto; defina `channels.discord.voice.enabled=true` (ou mantenha um bloco `channels.discord.voice` existente) para habilitar comandos `/vc`, o runtime de voz e a intent de Gateway `GuildVoiceStates`.
- `channels.discord.intents.voiceStates` pode substituir explicitamente a assinatura de intent de estado de voz. Deixe sem definir para que a intent siga a ativação efetiva de voz.
- Se `voice.autoJoin` tiver várias entradas para o mesmo servidor, o OpenClaw entra no último canal configurado para esse servidor.
- `voice.allowedChannels` é uma lista opcional de permissão de residência. Deixe sem definir para permitir `/vc join` em qualquer canal de voz autorizado do Discord. Quando definida, `/vc join`, entrada automática na inicialização e movimentos de estado de voz do bot ficam restritos às entradas `{ guildId, channelId }` listadas. Defina como um array vazio para negar todas as entradas em voz do Discord. Se o Discord mover o bot para fora da lista de permissão, o OpenClaw sai desse canal e retorna ao destino configurado de entrada automática quando houver um disponível.
...
- `voice.captureSilenceGraceMs` controla por quanto tempo o OpenClaw espera depois que o Discord informa que um falante parou antes de finalizar esse segmento de áudio para STT. Padrão: `2000`; aumente isso se o Discord dividir pausas normais em transcrições parciais picotadas.
...
- Se os logs de recebimento mostrarem repetidamente `DecryptionFailed(UnencryptedWhenPassthroughDisabled)` após a atualização, colete um relatório de dependências e logs. A linha incluída de `@discordjs/voice` contém a correção de padding upstream do PR #11449 do discord.js, que fechou a issue #11419 do discord.js.
...
Use `voice.followUsers` quando você quiser que o bot de voz do Discord fique com um ou mais usuários conhecidos do Discord em vez de entrar em um canal fixo na inicialização ou esperar por `/vc join`.
...
- `followUsers` aceita IDs brutos de usuário do Discord e valores `discord:<id>`. O OpenClaw normaliza as duas formas antes de corresponder eventos de estado de voz.
- `followUsersEnabled` usa `true` como padrão quando `followUsers` está configurado. Defina como `false` para manter a lista salva, mas interromper o acompanhamento automático de voz.
...
- Se vários usuários seguidos estiverem no mesmo servidor e o usuário seguido ativo sair, o OpenClaw se move para o canal de outro usuário seguido rastreado antes de sair do servidor. Se vários usuários seguidos se moverem ao mesmo tempo, o evento de estado de voz observado mais recente vence.
- `allowedChannels` ainda se aplica. Um usuário seguido em um canal não permitido é ignorado, e uma sessão controlada por acompanhamento se move para outro usuário seguido ou sai.
- O OpenClaw reconcilia eventos de estado de voz perdidos na inicialização e em um intervalo limitado. A reconciliação amostra servidores configurados e limita buscas REST por execução, então listas `followUsers` muito grandes podem levar mais de um intervalo para convergir.
- Se o Discord ou um administrador mover o bot enquanto ele está seguindo um usuário, o OpenClaw reconstrói a sessão de voz e preserva a propriedade de acompanhamento quando o destino é permitido. Se o bot for movido para fora de `allowedChannels`, o OpenClaw sai e retorna ao destino configurado quando houver um.
- A recuperação de recebimento DAVE pode sair e entrar novamente no mesmo canal após falhas repetidas de descriptografia. Sessões controladas por acompanhamento mantêm sua propriedade de acompanhamento por esse caminho de recuperação, então uma desconexão posterior do usuário seguido ainda faz o canal ser deixado.
...
- Use `/vc join` para entradas pontuais ou salas em que presença automática na voz seria inesperada.
...
- A transcrição é enviada pelo ingresso e roteamento do Discord enquanto o LLM de resposta é executado com uma política de saída por voz que oculta a ferramenta `tts` do agente e solicita texto retornado, porque a voz do Discord controla a reprodução final de TTS.
- `voice.model`, quando definido, substitui apenas o LLM de resposta para esta rodada de canal de voz.
...
Exemplo padrão de sessão de canal de voz agent-proxy:
...
Voz como uma extensão de uma sessão de canal existente do Discord:
...
No modo `agent-proxy`, o bot entra no canal de voz configurado, mas as rodadas do agente OpenClaw usam a sessão roteada normal e o agente do canal de destino. A sessão de voz em tempo real fala o resultado retornado de volta no canal de voz. O agente supervisor ainda pode usar ferramentas normais de mensagem de acordo com sua política de ferramentas, incluindo enviar uma mensagem separada no Discord se essa for a ação correta.

Enquanto uma execução delegada do OpenClaw estiver ativa, novas transcrições de voz do Discord são tratadas como controle de execução ao vivo antes de iniciar outra rodada do agente. Frases como "status", "cancel that", "use the smaller fix" ou "when you're done also check tests" são classificadas como entrada de status, cancelamento, direcionamento ou acompanhamento para a sessão ativa. Resultados de status, cancelamento, direcionamento aceito e acompanhamento são falados de volta no canal de voz para que quem chamou saiba se o OpenClaw lidou com a solicitação.

Formas de destino úteis:

- `target: "channel:123456789012345678"` é roteado por meio de uma sessão de canal de texto do Discord.
- `target: "123456789012345678"` é tratado como um alvo de canal.
- `target: "dm:123456789012345678"` ou `target: "user:123456789012345678"` é roteado por meio dessa sessão de mensagem direta.
...
Use isto quando o modelo ouvir sua própria reprodução do Discord por um microfone aberto, mas você ainda quiser interrompê-lo falando. O OpenClaw impede que o OpenAI interrompa automaticamente com áudio de entrada bruto, enquanto `bargeIn: true` permite que eventos de início de fala do Discord e áudio de falante já ativo cancelem respostas realtime ativas antes que o próximo turno capturado chegue ao OpenAI. Sinais de interrupção muito iniciais com `audioEndMs` abaixo de `minBargeInAudioEndMs` são tratados como provável eco/ruído e ignorados para que o modelo não seja cortado no primeiro quadro de reprodução.
...
- Ao iniciar realtime: `discord voice: realtime bridge starting ... autoRespond=false interruptResponse=false bargeIn=false minBargeInAudioEndMs=...`
...
- Na conclusão da resposta realtime: `discord voice: realtime audio playback finishing reason=response.done ... audioMs=... chunks=...`
- Ao parar/redefinir a reprodução: `discord voice: realtime audio playback stopped reason=... audioMs=... elapsedMs=... chunks=...`
- Em consulta realtime: `discord voice: realtime consult requested ... voiceSession=... supervisorSession=... question=...`
- Em resposta do agente: `discord voice: agent turn answer ...`
...
Para depurar áudio cortado, leia os logs de voz realtime como uma linha do tempo:

1. `realtime audio playback started` significa que o Discord começou a reproduzir áudio do assistente. A ponte começa a contar blocos de saída do assistente, bytes PCM do Discord, bytes realtime do provedor e duração de áudio sintetizado a partir desse ponto.
2. `realtime speaker turn opened` marca um falante do Discord se tornando ativo. Se a reprodução já estiver ativa e `bargeIn` estiver habilitado, isso pode ser seguido por `barge-in detected source=speaker-start`.
...
5. `barge-in requested reason=...` significa que o OpenClaw pediu ao provedor realtime para cancelar ou truncar a resposta ativa. Ele inclui `outputAudioMs`, `outputActive` e `playbackChunks` para que você possa ver quanto áudio do assistente realmente havia sido reproduzido antes da interrupção.
...
7. `realtime speaker turn closed` resume o turno de entrada capturado. `chunks=0` ou `hasAudio=false` significa que o turno de falante abriu, mas nenhum áudio utilizável chegou à ponte realtime. `interruptedPlayback=true` significa que esse turno de entrada se sobrepôs à saída do assistente e acionou a lógica de barge-in.

Campos úteis:

- `outputAudioMs`: duração do áudio do assistente gerada pelo provedor realtime antes da linha de log.
- `audioMs`: duração do áudio do assistente que o OpenClaw contou antes de a reprodução parar.
- `elapsedMs`: tempo de relógio decorrido entre abrir e fechar o fluxo de reprodução ou o turno de falante.
- `discordBytes`: bytes PCM estéreo de 48 kHz enviados para ou recebidos da voz do Discord.
- `realtimeBytes`: bytes PCM no formato do provedor enviados para ou recebidos do provedor realtime.
...
- `source=speaker-start` seguido por `speaker turn closed ... hasAudio=false` significa que o Discord informou um início de fala, mas nenhum áudio chegou ao OpenClaw. Isso pode ser um evento transitório de voz do Discord, comportamento de gate de ruído ou um cliente acionando brevemente o microfone.
...
- `barge-in ignored ... outputActive=false` significa que o VAD do Discord ou do provedor informou fala, mas o OpenClaw não tinha reprodução ativa para interromper. Isso não deve cortar o áudio.

As credenciais são resolvidas por componente: autenticação da rota LLM para `voice.model`, autenticação STT para `tools.media.audio`, autenticação TTS para `messages.tts`/`voice.tts` e autenticação do provedor realtime para `voice.realtime.providers` ou a configuração normal de autenticação do provedor.
...
<AccordionGroup>
  ...
  <Accordion title="Mensagens de guilda bloqueadas inesperadamente">
    ...
    - verifique a allowlist de guildas em `channels.discord.guilds`
...
```
  ...
  <Accordion title="Turnos longos do Discord ou respostas duplicadas">
    ...
    O Discord não aplica um tempo limite pertencente ao canal a turnos de agente enfileirados. Listeners de mensagem transferem imediatamente, e execuções enfileiradas do Discord preservam a ordenação por sessão até que o ciclo de vida da sessão/ferramenta/runtime conclua ou aborte o trabalho.
...
```
  ...
  <Accordion title="Avisos de tempo limite na busca de metadados do Gateway">
    O OpenClaw busca metadados `/gateway/bot` do Discord antes de conectar. Falhas transitórias recorrem à URL padrão do gateway do Discord e são limitadas por taxa nos logs.
  ...
  <Accordion title="Reinícios por tempo limite de READY do Gateway">
  ...
  <Accordion title="Incompatibilidades na auditoria de permissões">
    ...
    Se você usar chaves slug, a correspondência em runtime ainda pode funcionar, mas o probe não consegue verificar permissões por completo.
  ...
  <Accordion title="Loops de bot para bot">
    ...
    O OpenClaw também inclui [proteção contra loop de bots](/pt-BR/channels/bot-loop-protection) compartilhada. Sempre que `allowBots` permite que mensagens criadas por bots cheguem ao dispatch, o Discord mapeia o evento de entrada para fatos de `(conta, canal, par de bots)` e a proteção genérica de par suprime o par depois que ele ultrapassa o orçamento de eventos configurado. A proteção evita loops descontrolados entre dois bots que antes precisavam ser interrompidos pelos limites de taxa do Discord; ela não afeta implantações com um único bot nem respostas pontuais de bots que permanecem abaixo do orçamento.
    ...
    Configure o padrão compartilhado uma vez em `channels.defaults.botLoopProtection` e então sobrescreva o Discord quando um fluxo de trabalho legítimo precisar de mais margem. A precedência é:
