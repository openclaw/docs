...
summary: Configuração do Slack e comportamento em runtime (Socket Mode + URLs de solicitação HTTP)
title: Slack
x-i18n:
...
Pronto para produção para DMs e canais por meio de integrações de aplicativo do Slack. O modo padrão é Socket Mode; URLs de solicitação HTTP também são compatíveis.

<CardGroup cols={3}>
  <Card title="Pairing" icon="link" href="/pt-BR/channels/pairing">
    As DMs do Slack usam o modo pairing por padrão.
  </Card>
...
<Tabs>
  <Tab title="Socket Mode (padrão)">
    <Steps>
      <Step title="Criar um novo aplicativo do Slack">
...
O manifesto base do aplicativo do Slack é o mesmo para Socket Mode e URLs de solicitação HTTP. Apenas o bloco `settings` (e a `url` do comando de barra) é diferente.
...
Para o modo **URLs de solicitação HTTP**, substitua `settings` pela variante HTTP e adicione `url` a cada comando de barra. É necessária uma URL pública:
...
Exibem recursos diferentes que estendem os padrões acima.
...
```json
    "slash_commands": [
      ...
      {
        "command": "/models",
        "description": "Listar providers/modelos ou adicionar um modelo",
        "usage_hint": "[provider] [page] [limit=<n>|size=<n>|all] | add <provider> <modelId>"
      },
      {
        "command": "/help",
        "description": "Mostrar o breve resumo de ajuda"
      },
      {
        "command": "/commands",
      ...
      {
        "command": "/agentstatus",
        "description": "Mostrar o status do runtime, incluindo uso/cota do provider quando disponível"
      },
      {
        "command": "/tasks",
...
```
  ...
  <Accordion title="Escopos opcionais de autoria (operações de gravação)">
    Adicione o escopo de bot `chat:write.customize` se você quiser que mensagens de saída usem a identidade do agente ativo (nome de usuário e ícone personalizados) em vez da identidade padrão do aplicativo do Slack.
...
<Tip>
Para ações/leituras de diretório, o token de usuário pode ter preferência quando configurado. Para gravações, o token do bot continua sendo preferido; gravações com token de usuário só são permitidas quando `userTokenReadOnly: false` e o token do bot não está disponível.
</Tip>
...
<Tabs>
  <Tab title="Política de DM">
    ...
    - `dm.allowFrom` (legado)
    - `dm.groupEnabled` (DMs de grupo com padrão false)
    - `dm.groupChannels` (lista de permissão opcional de MPIM)

    Precedência para múltiplas contas:
  ...
  <Tab title="Política de canal">
    ...
    A lista de permissão de canais fica em `channels.slack.channels` e deve usar IDs estáveis de canal.

    Observação de runtime: se `channels.slack` estiver completamente ausente (configuração somente por ambiente), o runtime usa `groupPolicy="allowlist"` como fallback e registra um aviso em log (mesmo se `channels.defaults.groupPolicy` estiver definido).

    Resolução de nome/ID:

    - entradas de lista de permissão de canal e de DM são resolvidas na inicialização quando o acesso por token permite
    - entradas não resolvidas por nome de canal são mantidas como configuradas, mas ignoradas para roteamento por padrão
    - autorização de entrada e roteamento de canal usam ID primeiro por padrão; correspondência direta por nome de usuário/slug exige `channels.slack.dangerouslyAllowNameMatching: true`
...
- Com o padrão `session.dmScope=main`, as DMs do Slack são reduzidas à sessão principal do agente.
...
Observação: `replyToMode="off"` desabilita **todo** o encadeamento de respostas no Slack, incluindo tags explícitas `[[reply_to_*]]`. Isso difere do Telegram, onde tags explícitas ainda são respeitadas no modo `"off"` — threads do Slack ocultam mensagens do canal, enquanto respostas do Telegram continuam visíveis inline.
...
- fallback para o emoji de identidade do agente (`agents.list[].identity.emoji`, senão "👀")
...
`channels.slack.streaming` controla o comportamento da pré-visualização em tempo real:

- `off`: desabilita a pré-visualização em tempo real.
...
- `progress`: mostra texto de status de progresso enquanto gera e, em seguida, envia o texto final.
- `streaming.preview.toolProgress`: quando a pré-visualização de rascunho está ativa, encaminha atualizações de ferramenta/progresso para a mesma mensagem de pré-visualização editada (padrão: `true`). Defina `false` para manter mensagens separadas de ferramenta/progresso.
...
- Raízes de chats em canal e grupo ainda podem usar a pré-visualização normal de rascunho quando o streaming nativo não estiver disponível.
- DMs de Slack de nível superior permanecem fora de thread por padrão, portanto não mostram a pré-visualização no estilo thread; use respostas em thread ou `typingReaction` se quiser progresso visível ali.
- Mídia e cargas úteis não textuais recorrem à entrega normal.
- Resultados finais de mídia/erro cancelam edições pendentes de pré-visualização; resultados finais elegíveis de texto/bloco só são descarregados quando podem editar a pré-visualização no mesmo lugar.
- Se o streaming falhar no meio da resposta, o OpenClaw recorre à entrega normal para as cargas úteis restantes.

Use pré-visualização de rascunho em vez do streaming nativo de texto do Slack:
...
- A reação é em melhor esforço, e a limpeza é tentada automaticamente após a resposta ou quando o caminho de falha termina.
...
<AccordionGroup>
  ...
  <Accordion title="Texto e arquivos de saída">
    ...
    - o limite de mídia de saída segue `channels.slack.mediaMaxMb` quando configurado; caso contrário, envios pelo canal usam padrões por tipo MIME do pipeline de mídia
  </Accordion>
...
Comandos nativos exigem [configurações adicionais do manifesto](#additional-manifest-settings) no seu aplicativo do Slack e são habilitados com `channels.slack.commands.native: true` ou `commands.native: true` em configurações globais.
...
Menus nativos de argumentos usam uma estratégia de renderização adaptativa que mostra um modal de confirmação antes de despachar o valor da opção selecionada:
...
O Slack pode atuar como um cliente nativo de aprovação com botões e interações interativas, em vez de recorrer à Web UI ou ao terminal.
...
Isso usa a mesma superfície compartilhada de botões de aprovação que outros canais. Quando `interactivity` está habilitado nas configurações do seu aplicativo do Slack, prompts de aprovação são renderizados como botões Block Kit diretamente na conversa.
Quando esses botões estão presentes, eles são a UX principal de aprovação; o OpenClaw
deve incluir um comando manual `/approve` somente quando o resultado da ferramenta disser que aprovações pelo chat
...
O Slack habilita automaticamente aprovações nativas de exec quando `enabled` está indefinido ou como `"auto"` e pelo menos um
...
- O iniciador da thread e a semeadura inicial do contexto do histórico da thread são filtrados pelas listas de permissão de remetente configuradas, quando aplicável.
- Ações de bloco e interações de modal emitem eventos de sistema estruturados `Slack interaction: ...` com campos de payload avançados:
...
```
  ...
  <Accordion title="Mensagens de DM ignoradas">
    ...
    - aprovações de pairing / entradas de lista de permissão
...
```
  ...
  <Accordion title="Socket mode não conecta">
    Valide os tokens do bot + aplicativo e a habilitação do Socket Mode nas configurações do aplicativo do Slack.
  ...
  <Accordion title="O modo HTTP não recebe eventos">
    ...
    snapshots da conta, a conta HTTP está configurada, mas o runtime atual não conseguiu
    resolver o signing secret sustentado por SecretRef.
  ...
  <Accordion title="Comandos nativos/de barra não são acionados">
    ...
    - modo de comandos nativos (`channels.slack.commands.native: true`) com os comandos de barra correspondentes registrados no Slack
    - ou modo de comando de barra único (`channels.slack.slashCommand.enabled: true`)

    Verifique também `commands.useAccessGroups` e as listas de permissão de canal/usuário.
...
<CardGroup cols={2}>
  <Card title="Pairing" icon="link" href="/pt-BR/channels/pairing">
    Faça o pairing de um usuário do Slack com o gateway.
  </Card>
