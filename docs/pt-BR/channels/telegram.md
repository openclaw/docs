...
summary: Status do suporte ao bot Telegram, recursos e configuração
title: Telegram
x-i18n:
    generated_at: "2026-04-24T05:43:01Z"
    model: gpt-5.4
    provider: openai
...
<Note>
A ordem de resolução do token reconhece contas. Na prática, valores de configuração prevalecem sobre o fallback por env, e `TELEGRAM_BOT_TOKEN` se aplica apenas à conta padrão.
</Note>
...
<AccordionGroup>
  <Accordion title="Modo de privacidade e visibilidade em grupos">
    ...
    Se o bot precisar ver todas as mensagens do grupo, faça um dos seguintes:
...
<Tabs>
  <Tab title="Política de DM">
    ...
    Para bots de um único proprietário, prefira `dmPolicy: "allowlist"` com IDs numéricos explícitos em `allowFrom` para manter a política de acesso persistente na configuração (em vez de depender de aprovações anteriores de pareamento).

    Confusão comum: aprovar o pareamento de DM não significa "este remetente está autorizado em todos os lugares".
    O pareamento concede acesso apenas a DM. A autorização de remetente em grupo ainda vem de allowlists explícitas na configuração.
...
```
  ...
  <Tab title="Política de grupo e allowlists">
    ...
    `groupAllowFrom` é usado para filtragem de remetente em grupos. Se não estiver definido, o Telegram usa `allowFrom` como fallback.
    ...
    Observação de runtime: se `channels.telegram` estiver totalmente ausente, o runtime usa por padrão fail-closed `groupPolicy="allowlist"`, a menos que `channels.defaults.groupPolicy` esteja explicitamente definido.
...
- Mensagens de DM podem carregar `message_thread_id`; o OpenClaw as roteia com chaves de sessão com reconhecimento de thread e preserva o ID da thread para respostas.
...
<AccordionGroup>
  <Accordion title="Prévia de stream ao vivo (edições de mensagem)">
    ...
    - os valores legados `channels.telegram.streamMode` e booleanos de `streaming` são mapeados automaticamente
    ...
    Para respostas complexas (por exemplo, payloads de mídia), o OpenClaw usa fallback para entrega final normal e depois limpa a mensagem de prévia.

    A transmissão de prévia é separada do block streaming. Quando o block streaming é ativado explicitamente para Telegram, o OpenClaw ignora a stream de prévia para evitar transmissão dupla.

    Se o transporte nativo de rascunho estiver indisponível/for rejeitado, o OpenClaw usa automaticamente o fallback `sendMessage` + `editMessageText`.
...
```
    ...
    O código de configuração carrega um token bootstrap de curta duração. O handoff bootstrap integrado mantém o token do Node principal em `scopes: []`; qualquer token de operador transferido permanece limitado a `operator.approvals`, `operator.read`, `operator.talk.secrets` e `operator.write`. As verificações de escopo de bootstrap usam prefixo de função, então essa allowlist de operador atende apenas a solicitações de operador; funções que não sejam de operador ainda precisam de escopos sob seu próprio prefixo de função.
...
```
  ...
  <Accordion title="Ações de mensagem do Telegram para agentes e automação">
    ...
    Envios em runtime usam o snapshot ativo de config/segredos (inicialização/reload), portanto os caminhos de ação não fazem nova resolução ad hoc de SecretRef a cada envio.
  ...
  <Accordion title="Tópicos de fórum e comportamento de thread">
    ...
    **Roteamento de agente por tópico**: cada tópico pode ser roteado para um agente diferente ao definir `agentId` na configuração do tópico. Isso dá a cada tópico seu próprio workspace, memória e sessão isolados. Exemplo:
    ...
    **Spawn ACP vinculado à thread a partir do chat**: `/acp spawn <agent> --thread here|auto` vincula o tópico atual a uma nova sessão ACP; acompanhamentos são roteados diretamente para ela. O OpenClaw fixa a confirmação do spawn no tópico. Exige `channels.telegram.threadBindings.spawnAcpSessions=true`.

    O contexto do template expõe `MessageThreadId` e `IsForum`. Chats de DM com `message_thread_id` mantêm o roteamento de DM, mas usam chaves de sessão com reconhecimento de thread.
...
```
  ...
  <Accordion title="Notificações de reação">
    ...
    Quando ativado, o OpenClaw enfileira eventos do sistema como:
...
```
  ...
  <Accordion title="Limites, novas tentativas e destinos da CLI">
    ...
    - `channels.telegram.chunkMode="newline"` prioriza limites de parágrafo (linhas em branco) antes da divisão por tamanho.
    ...
    - `channels.telegram.pollingStallThresholdMs` tem padrão `120000`; ajuste entre `30000` e `600000` apenas para reinicializações por polling travado com falso positivo.
    ...
    - allowlists do Telegram bloqueiam principalmente quem pode acionar o agente, não um limite completo de redação de contexto suplementar.
...
```
  ...
  <Accordion title="Aprovações de exec no Telegram">
    O Telegram oferece suporte a aprovações de exec em DMs de aprovadores e, opcionalmente, pode publicar prompts no chat ou tópico de origem. Os aprovadores devem ser IDs numéricos de usuário do Telegram.
    ...
    A entrega no canal mostra o texto do comando no chat; ative `channel` ou `both` apenas em grupos/tópicos confiáveis. Quando o prompt chega a um tópico de fórum, o OpenClaw preserva o tópico tanto para o prompt de aprovação quanto para o acompanhamento. Aprovações de exec expiram após 30 minutos por padrão.
...
<AccordionGroup>
  <Accordion title="O bot não responde a mensagens de grupo sem menção">
    ...
    - `openclaw channels status --probe` pode verificar IDs numéricos explícitos de grupo; o curinga `"*"` não pode ser sondado quanto a participação.
  ...
  <Accordion title="Os comandos funcionam parcialmente ou não funcionam">
    ...
    - `setMyCommands failed` com erros de rede/fetch normalmente indica problemas de alcance DNS/HTTPS até `api.telegram.org`
  ...
  <Accordion title="Polling ou instabilidade de rede">
    ...
    - Se os logs incluírem `Polling stall detected`, o OpenClaw reinicia o polling e reconstrói o transporte do Telegram após 120 segundos sem liveness concluído de long polling por padrão.
    - Aumente `channels.telegram.pollingStallThresholdMs` apenas quando chamadas de longa duração de `getUpdates` estiverem saudáveis, mas seu host ainda relatar reinicializações falsas por polling travado. Travamentos persistentes normalmente apontam para problemas de proxy, DNS, IPv6 ou saída TLS entre o host e `api.telegram.org`.
...
```
    ...
    <Warning>
      ...
      proteções SSRF da mídia do Telegram. Use isso apenas em ambientes de proxy
      confiáveis e controlados pelo operador, como roteamento fake-IP de Clash, Mihomo ou Surge, quando eles
      sintetizarem respostas privadas ou de uso especial fora da faixa de benchmark
      RFC 2544. Deixe isso desativado para acesso normal do Telegram pela internet pública.
    </Warning>
...
<Accordion title="Campos de Telegram de maior sinal">
...
<CardGroup cols={2}>
  ...
  <Card title="Roteamento com múltiplos agentes" icon="sitemap" href="/pt-BR/concepts/multi-agent">
