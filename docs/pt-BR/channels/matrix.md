...
title: Matriz
x-i18n:
    generated_at: "2026-05-06T05:47:11Z"
    model: gpt-5.5
    provider: openai
...
Instale o Matrix antes de configurar o canal:
...
`plugins install` registra e habilita o Plugin, então nenhuma etapa separada `openclaw plugins enable matrix` é necessária. O Plugin ainda não faz nada até que você configure o canal abaixo. Consulte [Plugins](/pt-BR/tools/plugin) para ver o comportamento geral de Plugins e as regras de instalação.
...
`channels.matrix.autoJoin` usa `off` como padrão. Com o padrão, o bot não aparecerá em novas salas ou DMs de convites recentes até que você entre manualmente.

OpenClaw não consegue saber, no momento do convite, se uma sala convidada é uma DM ou um grupo, então todos os convites - incluindo convites no estilo DM - passam primeiro por `autoJoin`. `dm.policy` só se aplica depois, após o bot entrar e a sala ser classificada.
...
Allowlists de DM e sala são melhor preenchidas com IDs estáveis:
...
- Salas (`groups`, `autoJoinAllowlist`): use `!room:server` ou `#alias:server`. Nomes são resolvidos com melhor esforço em relação às salas já ingressadas; entradas não resolvidas são ignoradas em tempo de execução.
...
Quando credenciais em cache existem ali, OpenClaw trata Matrix como configurado mesmo que o token de acesso não esteja no arquivo de configuração - isso cobre configuração, `openclaw doctor` e sondagens de status do canal.
...
O streaming de respostas do Matrix é opcional. `streaming` controla como OpenClaw entrega a resposta do assistente em andamento; `blockStreaming` controla se cada bloco concluído é preservado como sua própria mensagem Matrix.
...
| `"quiet"`         | Igual a `"partial"`, mas a mensagem é um aviso sem notificação. Os destinatários só recebem uma notificação quando uma regra de push por usuário corresponde à edição finalizada (veja abaixo). |
...
- Se uma pré-visualização ultrapassar o limite de tamanho por evento do Matrix, OpenClaw interrompe o streaming de pré-visualização e volta para entrega somente final.
- Respostas com mídia sempre enviam anexos normalmente. Se uma pré-visualização obsoleta não puder mais ser reutilizada com segurança, OpenClaw a redige antes de enviar a resposta final com mídia.
- Atualizações de pré-visualização de progresso de ferramenta ficam habilitadas por padrão quando o streaming de pré-visualização do Matrix está ativo. Defina `streaming.preview.toolProgress: false` para manter edições de pré-visualização para o texto da resposta, mas deixar o progresso de ferramenta no caminho de entrega normal.
...
Quando um prompt de aprovação é longo demais para um evento Matrix, OpenClaw divide o texto visível em partes e anexa `com.openclaw.approval` apenas à primeira parte. Reações para decisões de permitir/negar são vinculadas a esse primeiro evento, então prompts longos mantêm o mesmo destino de aprovação que prompts de evento único.

### Regras de push auto-hospedadas para pré-visualizações finalizadas silenciosas

`streaming: "quiet"` só notifica os destinatários quando um bloco ou turno é finalizado - uma regra de push por usuário precisa corresponder ao marcador de pré-visualização finalizada. Consulte [Regras de push do Matrix para pré-visualizações silenciosas](/pt-BR/channels/matrix-push-rules) para a receita completa (token do destinatário, verificação do pusher, instalação da regra, observações por homeserver).
...
- Matrix não expõe uma sinalização nativa de bot aqui; OpenClaw trata "autoria de bot" como "enviado por outra conta Matrix configurada neste Gateway OpenClaw".

Use allowlists estritas de salas e requisitos de menção ao habilitar tráfego bot-a-bot em salas compartilhadas.

## Criptografia e verificação

Em salas criptografadas (E2EE), eventos de imagem de saída usam `thumbnail_file` para que pré-visualizações de imagem sejam criptografadas junto com o anexo completo. Salas não criptografadas ainda usam `thumbnail_url` simples. Nenhuma configuração é necessária - o Plugin detecta automaticamente o estado de E2EE.

Todos os comandos `openclaw matrix` aceitam `--verbose` (diagnósticos completos), `--json` (saída legível por máquina) e `--account <id>` (configurações com várias contas). A saída é concisa por padrão, com registro interno silencioso do SDK. Os exemplos abaixo mostram a forma canônica; adicione as flags conforme necessário.
...
`Verified by owner` se torna `yes` somente quando `Cross-signing verified` é `yes`. Confiança local ou uma assinatura do proprietário por si só não é suficiente.
...
Ele sai com código diferente de zero quando a confiança completa da identidade está incompleta, mesmo que a chave de recuperação tenha desbloqueado o material de backup. Nesse caso, conclua a autoverificação a partir de outro cliente Matrix:
...
`backup status` mostra se existe um backup no servidor e se este dispositivo consegue descriptografá-lo. `backup restore` importa as chaves de sala em backup para o armazenamento criptográfico local; se a chave de recuperação já estiver em disco, você pode omitir `--recovery-key-stdin`.
...
Adicione `--rotate-recovery-key` somente quando quiser intencionalmente que a chave de recuperação anterior pare de desbloquear a nova linha de base de backup.
...
Envia uma solicitação de verificação a partir desta conta OpenClaw. `--own-user` solicita autoverificação (você aceita o prompt em outro cliente Matrix do mesmo usuário); `--user-id`/`--device-id`/`--room-id` direcionam para outra pessoa. `--own-user` não pode ser combinado com as outras flags de direcionamento.
...
<AccordionGroup>
  <Accordion title="Comportamento de inicialização">
    ...
    A inicialização também executa uma passagem conservadora de inicialização criptográfica que reutiliza o armazenamento de segredos e a identidade de assinatura cruzada atuais. Se o estado de inicialização estiver quebrado, o OpenClaw tenta um reparo protegido mesmo sem `channels.matrix.password`; se o homeserver exigir UIA com senha, a inicialização registra um aviso e permanece não fatal. Dispositivos já assinados pelo proprietário são preservados.
...
Você pode passar ambas as opções em uma chamada. O Matrix aceita URLs de avatar `mxc://` diretamente; quando você passa `http://` ou `https://`, o OpenClaw primeiro faz upload do arquivo e armazena a URL `mxc://` resolvida em `channels.matrix.avatarUrl` (ou na substituição por conta).
...
Associações explícitas de conversa sempre prevalecem sobre `sessionScope`, então salas e threads associadas mantêm a sessão de destino escolhida.

### Respostas em thread (`threadReplies`)

`threadReplies` decide onde o bot publica sua resposta:

- `"off"`: as respostas são de nível superior. Mensagens de entrada em thread permanecem na sessão pai.
...
`dm.threadReplies` substitui isto apenas para DMs - por exemplo, manter threads de sala isoladas enquanto mantém DMs sem threads.

### Herança de threads e comandos de barra

- As mensagens encadeadas de entrada incluem a mensagem raiz do thread como contexto extra do agente.
- Envios pela ferramenta de mensagens herdam automaticamente o thread Matrix atual ao mirar a mesma sala (ou o mesmo alvo de usuário por DM), a menos que um `threadId` explícito seja fornecido.
- A reutilização de alvo de usuário por DM só entra em ação quando os metadados da sessão atual comprovam o mesmo par de DM na mesma conta Matrix; caso contrário, o OpenClaw recorre ao roteamento normal com escopo por usuário.
- `/focus`, `/unfocus`, `/agents`, `/session idle`, `/session max-age` e `/acp spawn` vinculado a thread funcionam em salas e DMs do Matrix.
- `/focus` no nível superior cria um novo thread Matrix e o vincula à sessão de destino quando `threadBindings.spawnSessions` está habilitado.
- Executar `/focus` ou `/acp spawn --thread here` dentro de um thread Matrix existente vincula esse thread no lugar.

Quando o OpenClaw detecta uma sala de DM do Matrix em conflito com outra sala de DM na mesma sessão compartilhada, ele publica um `m.notice` único nessa sala apontando para a saída de emergência `/focus` e sugerindo uma alteração em `dm.sessionScope`. O aviso só aparece quando os vínculos de thread estão habilitados.

## Vínculos de conversa ACP

Salas, DMs e threads Matrix existentes podem ser transformados em workspaces ACP duráveis sem alterar a superfície de chat.
...
- Em uma DM ou sala Matrix de nível superior, a DM/sala atual permanece como a superfície de chat e as mensagens futuras são roteadas para a sessão ACP gerada.
- Dentro de um thread Matrix existente, `--bind here` vincula o thread atual no lugar.
- `/new` e `/reset` redefinem a mesma sessão ACP vinculada no lugar.
- `/acp close` fecha a sessão ACP e remove o vínculo.

Observações:

- `--bind here` não cria um thread filho no Matrix.
- `threadBindings.spawnSessions` controla `/acp spawn --thread auto|here`, quando o OpenClaw precisa criar ou vincular um thread filho no Matrix.
...
A geração de sessões vinculadas a threads do Matrix vem habilitada por padrão:
...
- Defina `threadBindings.defaultSpawnContext: "isolated"` quando gerações nativas de threads de subagentes não devem bifurcar a transcrição pai.

## Reações

O Matrix oferece suporte a reações de saída, notificações de reações de entrada e reações de confirmação.
...
- `emoji=""` remove as próprias reações do bot naquele evento.
...
`reactionNotifications: "own"` encaminha eventos `m.reaction` adicionados quando eles miram mensagens Matrix criadas pelo bot; `"off"` desabilita eventos do sistema de reações. Remoções de reações não são sintetizadas em eventos do sistema porque o Matrix as expõe como redações, não como remoções independentes de `m.reaction`.

## Contexto do histórico

- `channels.matrix.historyLimit` controla quantas mensagens recentes da sala são incluídas como `InboundHistory` quando uma mensagem em sala Matrix aciona o agente. Recorre a `messages.groupChat.historyLimit`; se ambos não estiverem definidos, o padrão efetivo é `0`. Defina `0` para desabilitar.
- O histórico de salas Matrix é apenas da sala. DMs continuam usando o histórico normal da sessão.
- O histórico de salas Matrix é somente pendente: o OpenClaw armazena em buffer mensagens da sala que ainda não acionaram uma resposta e captura uma cópia dessa janela quando uma menção ou outro gatilho chega.
- A mensagem de gatilho atual não é incluída em `InboundHistory`; ela permanece no corpo principal de entrada daquele turno.
- Novas tentativas do mesmo evento Matrix reutilizam o instantâneo original do histórico em vez de avançar para mensagens mais novas da sala.

## Visibilidade de contexto

O Matrix oferece suporte ao controle compartilhado `contextVisibility` para contexto suplementar de sala, como texto de resposta buscado, raízes de threads e histórico pendente.
...
Para silenciar DMs completamente mantendo as salas funcionando, defina `dm.enabled: false`:
...
Consulte [Grupos](/pt-BR/channels/groups) para ver o comportamento de controle por menção e allowlist.
...
Se um usuário Matrix não aprovado continuar enviando mensagens antes da aprovação, o OpenClaw reutiliza o mesmo código de pareamento pendente e pode enviar uma resposta de lembrete após um curto período de espera, em vez de gerar um novo código.

Consulte [Pareamento](/pt-BR/channels/pairing) para ver o fluxo compartilhado de pareamento por DM e o layout de armazenamento.

## Reparo de sala direta

Se o estado de mensagens diretas ficar fora de sincronia, o OpenClaw pode acabar com mapeamentos `m.direct` obsoletos que apontam para salas individuais antigas em vez da DM ativa. Inspecione o mapeamento atual para um par:
...
- cria uma nova sala direta e reescreve `m.direct` se não houver uma DM íntegra

Ele não exclui salas antigas automaticamente. Ele escolhe a DM íntegra e atualiza o mapeamento para que envios futuros pelo Matrix, avisos de verificação e outros fluxos de mensagens diretas mirem a sala correta.

## Aprovações de execução

O Matrix pode atuar como um cliente nativo de aprovação. Configure em `channels.matrix.execApprovals` (ou `channels.matrix.accounts.<account>.execApprovals` para uma substituição por conta):
...
- `target`: para onde os prompts vão. `"dm"` (padrão) envia para as DMs dos aprovadores; `"channel"` envia para a sala Matrix ou DM de origem; `"both"` envia para ambos.
- `agentFilter` / `sessionFilter`: allowlists opcionais para quais agentes/sessões acionam a entrega pelo Matrix.
...
Apenas aprovadores resolvidos podem aprovar ou negar. A entrega por canal para aprovações de execução inclui o texto do comando - habilite `channel` ou `both` apenas em salas confiáveis.

Relacionado: [Aprovações de execução](/pt-BR/tools/exec-approvals).

## Comandos slash

Comandos slash (`/new`, `/reset`, `/model`, `/focus`, `/unfocus`, `/agents`, `/session`, `/acp`, `/approve` etc.) funcionam diretamente em DMs. Em salas, o OpenClaw também reconhece comandos prefixados pela própria menção Matrix do bot, de modo que `@bot:server /new` aciona o caminho de comando sem uma regex de menção personalizada. Isso mantém o bot responsivo a publicações no estilo de sala `@mention /command` que o Element e clientes semelhantes emitem quando um usuário usa preenchimento por tabulação no bot antes de digitar o comando.

As regras de autorização ainda se aplicam: os remetentes dos comandos devem satisfazer as mesmas políticas de allowlist/proprietário de DM ou sala que mensagens comuns.
...
- Quando o OpenClaw promove uma configuração de conta única para várias contas durante reparo ou configuração inicial, ele preserva a conta nomeada existente se houver uma ou se `defaultAccount` já apontar para uma. Apenas chaves de autenticação/bootstrap do Matrix são movidas para a conta promovida; chaves compartilhadas de política de entrega permanecem no nível superior.
...
Se seu homeserver roda em localhost, em um IP de LAN/Tailscale ou em um nome de host interno, habilite
...
Esta opção explícita permite apenas destinos privados/internos confiáveis. Homeservers públicos em texto claro como
...
O OpenClaw usa a mesma configuração de proxy para o tráfego Matrix em tempo de execução e sondagens de status de conta.
...
IDs de sala do Matrix diferenciam maiúsculas de minúsculas. Use a capitalização exata do ID da sala do Matrix
ao configurar destinos de entrega explícitos, tarefas de Cron, vinculações ou listas de permissões.
O OpenClaw mantém chaves internas de sessão canônicas para armazenamento, então essas chaves em minúsculas
não são uma fonte confiável para IDs de entrega do Matrix.

A consulta ao diretório em tempo real usa a conta do Matrix conectada:

- Consultas de usuários consultam o diretório de usuários do Matrix nesse homeserver.
- Consultas de salas aceitam IDs de sala e apelidos explícitos diretamente e, em seguida, recorrem à busca nos nomes de salas ingressadas dessa conta.
- A consulta por nome de sala ingressada é de melhor esforço. Se um nome de sala não puder ser resolvido para um ID ou apelido, ele será ignorado pela resolução de lista de permissões em tempo de execução.

## Referência de configuração

Campos do tipo lista de permissões (`groupAllowFrom`, `dm.allowFrom`, `groups.<room>.users`) aceitam IDs completos de usuários do Matrix (mais seguro). Correspondências exatas de diretório são resolvidas na inicialização e sempre que a lista de permissões muda enquanto o monitor está em execução; entradas que não podem ser resolvidas são ignoradas em tempo de execução. Listas de permissões de salas preferem IDs de sala ou apelidos pelo mesmo motivo.
...
- `proxy`: URL opcional de proxy HTTP(S) para o tráfego Matrix. Há suporte a substituição por conta.
- `userId`: ID completo de usuário Matrix (`@bot:example.org`).
- `accessToken`: token de acesso para autenticação baseada em token. Há suporte a valores em texto simples e SecretRef entre provedores de env/file/exec ([Gerenciamento de segredos](/pt-BR/gateway/secrets)).
...
- `avatarUrl`: URL do autoavatar armazenado para sincronização de perfil e atualizações de `profile set`.
...
- `autoJoinAllowlist`: salas/aliases permitidos quando `autoJoin` é `"allowlist"`. Entradas de alias são resolvidas em relação ao servidor doméstico, não em relação ao estado reivindicado pela sala convidada.
- `contextVisibility`: visibilidade de contexto suplementar (padrão `"all"`, `"allowlist"`, `"allowlist_quote"`).
...
- `ackReactionScope`: substituição de escopo (padrão `"group-mentions"`, `"group-all"`, `"direct"`, `"all"`, `"none"`, `"off"`).
- `reactionNotifications`: modo de notificação de reação de entrada (padrão `"own"`, `"off"`).
...
- `groups`: mapa de políticas por sala. A identidade da sessão usa o ID estável da sala após a resolução. (`rooms` é um alias legado.)
  - `groups.<room>.account`: restringe uma entrada herdada de sala a uma conta específica.
  ...
  - `groups.<room>.autoReply`: substituição por sala do controle por menção. `true` desabilita os requisitos de menção para essa sala; `false` força sua reativação.
...
- [Pareamento](/pt-BR/channels/pairing) - fluxo de autenticação e pareamento de DM
