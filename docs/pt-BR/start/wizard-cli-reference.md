...
- Instalação como daemon (LaunchAgent, unidade de usuário systemd ou Scheduled Task nativa do Windows com fallback para a pasta de Inicialização)
...
<Steps>
  <Step title="Detecção de configuração existente">
    - Se `~/.openclaw/openclaw.json` existir, escolha entre Manter, Modificar ou Redefinir.
    - Executar o assistente novamente não apaga nada, a menos que você escolha explicitamente Redefinir (ou passe `--reset`).
    - O `--reset` da CLI usa por padrão `config+creds+sessions`; use `--reset-scope full` para também remover o workspace.
  ...
  <Step title="Gateway">
    ...
    - Caminho SecretRef para token no modo não interativo: `--gateway-token-ref-env <ENV_VAR>`.
    ...
    - Binds fora do loopback ainda exigem autenticação.
  </Step>
  <Step title="Canais">
  ...
  <Step title="Instalação como daemon">
    - macOS: LaunchAgent
      - Exige sessão de usuário conectada; para uso headless, use um LaunchDaemon personalizado (não incluído).
    - Linux e Windows via WSL2: unidade de usuário systemd
      - O assistente tenta executar `loginctl enable-linger <user>` para que o gateway continue ativo após logout.
      - Pode solicitar sudo (grava em `/var/lib/systemd/linger`); ele tenta sem sudo primeiro.
    - Windows nativo: Scheduled Task primeiro
      - Se a criação da tarefa for negada, o OpenClaw recorre a um item de login por usuário na pasta de Inicialização e inicia o gateway imediatamente.
...
<Note>
Se nenhuma GUI for detectada, o assistente exibirá instruções de encaminhamento de porta SSH para o Control UI em vez de abrir um navegador.
Se os assets do Control UI estiverem ausentes, o assistente tentará gerá-los; o fallback é `pnpm ui:build` (instala automaticamente as dependências da UI).
</Note>

## Detalhes do modo remoto

O modo remoto configura esta máquina para se conectar a um gateway em outro local.
...
<Note>
- Se o gateway for somente loopback, use tunelamento SSH ou uma tailnet.
- Dicas de descoberta:
...
<AccordionGroup>
  ...
  <Accordion title="StepFun">
    A configuração é gravada automaticamente para o padrão StepFun ou Step Plan em endpoints da China ou globais.
  ...
  <Accordion title="Ollama (Cloud e modelos abertos locais)">
    ...
    Os modos com host solicitam a URL base (padrão `http://127.0.0.1:11434`), descobrem modelos disponíveis e sugerem padrões.
    `Cloud + Local` também verifica se esse host Ollama está autenticado para acesso cloud.
  ...
  <Accordion title="Provedor personalizado">
    ...
    - **Usar referência de segredo** (referência de env ou referência de provedor configurada, com validação de pré-verificação)
...
- Se esse filtro de provedor preferido ficar vazio, o seletor recorre ao
  catálogo completo em vez de mostrar nenhum modelo.
...
- O modo de referência interativo executa uma validação rápida de pré-verificação antes de salvar.
  - Referências de env: valida o nome da variável + valor não vazio no ambiente atual de onboarding.
  - Referências de provedor: valida a configuração do provedor e resolve o id solicitado.
  - Se a pré-verificação falhar, o onboarding mostra o erro e permite tentar novamente.
...
- As credenciais de autenticação do Gateway oferecem suporte a escolhas de texto simples e SecretRef na configuração interativa:
...
- `tools.profile` (o onboarding local define por padrão `"coding"` quando não está definido; valores explícitos existentes são preservados)
- `gateway.*` (modo, bind, autenticação, Tailscale)
- `session.dmScope` (o onboarding local define por padrão `per-channel-peer` quando não está definido; valores explícitos existentes são preservados)
- `channels.telegram.botToken`, `channels.discord.token`, `channels.matrix.*`, `channels.signal.*`, `channels.imessage.*`
- Allowlists de canal (Slack, Discord, Matrix, Microsoft Teams) quando você opta por elas durante os prompts (os nomes são resolvidos para IDs quando possível)
...
Os clientes (app macOS e Control UI) podem renderizar etapas sem reimplementar a lógica de onboarding.
