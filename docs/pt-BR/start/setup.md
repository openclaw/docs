...
<Note>
Se você estiver configurando pela primeira vez, comece por [Introdução](/pt-BR/start/getting-started).
...
- **Fluxo de ponta (dev):** execute o Gateway por conta própria via `pnpm gateway:watch` e então deixe o app para macOS se conectar em modo Local.

## Pré-requisitos (a partir do código-fonte)

- Node 24 recomendado (Node 22 LTS, atualmente `22.14+`, ainda compatível)
- `pnpm` é necessário para checkouts do código-fonte. O OpenClaw carrega Plugins incluídos a partir dos pacotes de workspace pnpm
  `extensions/*` no modo de desenvolvimento, portanto `npm install` na raiz
  não prepara toda a árvore de código-fonte.
- Docker (opcional; somente para configuração/e2e em contêiner — consulte [Docker](/pt-BR/install/docker))
...
Objetivo: trabalhar no Gateway em TypeScript, obter recarregamento automático e manter a UI do app para macOS conectada.
...
`OPENCLAW_GATEWAY_WATCH_AUTO_DOCTOR=0` para desativar essa etapa de reparo exclusiva de desenvolvimento.
`pnpm openclaw setup` é a etapa única de inicialização da configuração/workspace local para um checkout novo.
`pnpm gateway:watch` não recompila `dist/control-ui`, portanto execute novamente `pnpm ui:build` após alterações em `ui/` ou use `pnpm ui:dev` enquanto desenvolve a Control UI.
...
- O status do Gateway no app deve exibir **“Usando gateway existente …”**
- Ou via CLI:
...
ao fazer logout/ficar inativo, o que encerra o Gateway. O onboarding tenta habilitar
...
serviço de usuário (sem necessidade de lingering). Consulte [Runbook do Gateway](/pt-BR/gateway) para as notas sobre systemd.
