...
read_when:
    ...
    - به نمونه پیکربندی‌ها یا فرمان‌های راه‌اندازی CLI برای ارائه‌دهندگان مدل نیاز دارید
...
مرجعی برای **ارائه‌دهندگان LLM/مدل** (نه کانال‌های چت مانند WhatsApp/Telegram). برای قواعد انتخاب مدل، [مدل‌ها](/fa/concepts/models) را ببینید.
...
<AccordionGroup>
  <Accordion title="ارجاع‌های مدل و کمک‌کننده‌های CLI">
    ...
    - قواعد جایگزینی، کاوش‌های دوره‌ی انتظار، و ماندگاری بازنویسی نشست: [جایگزینی مدل](/fa/concepts/model-failover).
  ...
  <Accordion title="تفکیک ارائه‌دهنده/زمان اجرای OpenAI">
    ...
    [OpenAI](/fa/providers/openai) و [هارنس Codex](/fa/plugins/codex-harness) را ببینید. اگر تفکیک ارائه‌دهنده/زمان اجرا گیج‌کننده است، ابتدا [زمان‌های اجرای عامل](/fa/concepts/agent-runtimes) را بخوانید.
...
فهرست کامل قلاب‌های SDK ارائه‌دهنده و مثال‌های Pluginهای همراه در [Pluginهای ارائه‌دهنده](/fa/plugins/sdk-provider-plugins) قرار دارد. ارائه‌دهنده‌ای که به اجراکننده درخواست کاملا سفارشی نیاز دارد، یک سطح توسعه جداگانه و عمیق‌تر است.

<Note>
رفتار اجراکننده متعلق به ارائه‌دهنده روی قلاب‌های صریح ارائه‌دهنده مانند سیاست بازپخش، نرمال‌سازی طرح‌واره ابزار، پوشش‌دهی جریان، و کمک‌کننده‌های انتقال/درخواست قرار دارد. محفظه ایستای قدیمی `ProviderPlugin.capabilities` فقط برای سازگاری است و دیگر توسط منطق اجراکننده مشترک خوانده نمی‌شود.
</Note>
...
<AccordionGroup>
  ...
  <Accordion title="چرخش چه زمانی شروع می‌شود">
...
<Note>
کارکنان Anthropic به ما گفته‌اند که استفاده به سبک OpenClaw از Claude CLI دوباره مجاز است، بنابراین OpenClaw استفاده مجدد از Claude CLI و استفاده از `claude -p` را برای این یکپارچه‌سازی مجاز تلقی می‌کند، مگر اینکه Anthropic سیاست جدیدی منتشر کند. setup-token مربوط به Anthropic همچنان به‌عنوان یک مسیر توکن پشتیبانی‌شده OpenClaw در دسترس است، اما OpenClaw اکنون در صورت موجود بودن، استفاده مجدد از Claude CLI و `claude -p` را ترجیح می‌دهد.
</Note>
...
- مستندات هارنس بومی سرور برنامه Codex: [هارنس Codex](/fa/plugins/codex-harness)
...
<CardGroup cols={3}>
  ...
  <Card title="Qwen Cloud" href="/fa/providers/qwen">
    سطح ارائه‌دهنده Qwen Cloud به‌همراه نگاشت نقاط پایانی Alibaba DashScope و Coding Plan.
  </Card>
</CardGroup>
...
<Warning>
OAuth در Gemini CLI برای OpenClaw یک یکپارچه‌سازی غیررسمی است. برخی کاربران پس از استفاده از کلاینت‌های شخص ثالث، محدودیت‌هایی در حساب Google خود گزارش کرده‌اند. شرایط Google را بررسی کنید و اگر تصمیم به ادامه دارید، از یک حساب غیرحساس استفاده کنید.
</Warning>
...
<Note>
در مسیر استریمینگ سازگار با Anthropic متعلق به MiniMax، OpenClaw به‌طور پیش‌فرض thinking را غیرفعال می‌کند مگر اینکه آن را صریحاً تنظیم کنید، و `/fast on` مقدار `MiniMax-M2.7` را به `MiniMax-M2.7-highspeed` بازنویسی می‌کند.
</Note>
...
وقتی با `OLLAMA_API_KEY` فعال‌سازی را انتخاب کنید، Ollama به‌صورت محلی در `http://127.0.0.1:11434` شناسایی می‌شود، و Plugin ارائه‌دهنده همراه، Ollama را مستقیماً به `openclaw onboard` و انتخابگر مدل اضافه می‌کند. برای راه‌اندازی، حالت ابری/محلی، و پیکربندی سفارشی، [/providers/ollama](/fa/providers/ollama) را ببینید.
...
- [جابه‌جایی خودکار مدل](/fa/concepts/model-failover) - زنجیره‌های جایگزین و رفتار تلاش دوباره
