...
summary: ویدئوها را از طریق video_generate از ارجاع‌های متنی، تصویری یا ویدئویی در ۱۶ بک‌اند ارائه‌دهنده تولید کنید
title: تولید ویدئو
x-i18n:
    generated_at: "2026-05-05T06:21:42Z"
    model: gpt-5.5
    provider: openai
...
ویدئوهای موجود ویدئو تولید کنند. شانزده backend ارائه‌دهنده پشتیبانی می‌شود، که هرکدام
گزینه‌های مدل، حالت‌های ورودی، و مجموعه قابلیت‌های متفاوتی دارند. عامل بر اساس پیکربندی شما و کلیدهای API
موجود، ارائه‌دهنده مناسب را به‌صورت خودکار انتخاب می‌کند.

<Note>
ابزار `video_generate` فقط زمانی ظاهر می‌شود که دست‌کم یک ارائه‌دهنده تولید ویدئو
...
OpenClaw تولید ویدئو را به‌صورت سه حالت زمان اجرا در نظر می‌گیرد:

- `generate` — درخواست‌های متن به ویدئو بدون رسانه مرجع.
- `imageToVideo` — درخواست شامل یک یا چند تصویر مرجع است.
- `videoToVideo` — درخواست شامل یک یا چند ویدئوی مرجع است.

ارائه‌دهندگان می‌توانند هر زیرمجموعه‌ای از این حالت‌ها را پشتیبانی کنند. ابزار پیش از ارسال، حالت
فعال را اعتبارسنجی می‌کند و حالت‌های پشتیبانی‌شده را در `action=list` گزارش می‌دهد.
...
<Steps>
  ...
  <Step title="درخواست از عامل">
    > یک ویدئوی سینمایی ۵ ثانیه‌ای از یک خرچنگ دریایی دوستانه که هنگام غروب موج‌سواری می‌کند تولید کن.

    عامل به‌صورت خودکار `video_generate` را فراخوانی می‌کند. نیازی به مجازکردن ابزار
    نیست.
...
تولید ویدئو ناهمگام است. وقتی عامل در یک
نشست `video_generate` را فراخوانی می‌کند:

1. OpenClaw درخواست را به ارائه‌دهنده ارسال می‌کند و بلافاصله یک شناسه تسک برمی‌گرداند.
2. ارائه‌دهنده کار را در پس‌زمینه پردازش می‌کند (معمولاً از ۳۰ ثانیه تا چند دقیقه بسته به ارائه‌دهنده و وضوح؛ ارائه‌دهندگان کند مبتنی بر صف می‌توانند تا زمان‌پایان پیکربندی‌شده اجرا شوند).
3. وقتی ویدئو آماده شد، OpenClaw همان نشست را با یک رویداد تکمیل داخلی بیدار می‌کند.
4. عامل به کاربر اطلاع می‌دهد و ویدئوی نهایی را پیوست می‌کند. در چت‌های گروهی/کانالی
   که از تحویل قابل‌مشاهده فقط از طریق ابزار پیام استفاده می‌کنند، عامل نتیجه را
   به‌جای اینکه OpenClaw مستقیماً آن را ارسال کند، از طریق ابزار پیام منتقل می‌کند.

وقتی کاری در حال انجام است، فراخوانی‌های تکراری `video_generate` در همان
نشست به‌جای شروع یک
تولید دیگر، وضعیت تسک فعلی را برمی‌گردانند. برای بررسی پیشرفت از CLI از `openclaw tasks list` یا `openclaw tasks show <taskId>` استفاده کنید.

خارج از اجراهای عاملِ پشتیبانی‌شده با نشست (برای مثال، فراخوانی‌های مستقیم ابزار)،
ابزار به تولید درون‌خطی برمی‌گردد و مسیر رسانه نهایی را
در همان نوبت برمی‌گرداند.

وقتی ارائه‌دهنده بایت برمی‌گرداند، فایل‌های ویدئوی تولیدشده در فضای ذخیره‌سازی رسانه مدیریت‌شده توسط OpenClaw
ذخیره می‌شوند. سقف ذخیره پیش‌فرض ویدئوی تولیدشده از
محدودیت رسانه ویدئو پیروی می‌کند، و `agents.defaults.mediaMaxMb` آن را برای
رندرهای بزرگ‌تر افزایش می‌دهد. وقتی ارائه‌دهنده همچنین یک URL خروجی میزبانی‌شده برمی‌گرداند، OpenClaw
می‌تواند در صورتی که ماندگاری محلی یک فایل بیش‌ازحد بزرگ را رد کند، به‌جای شکست‌دادن تسک
آن URL را تحویل دهد.
...
| وضعیت       | معنی                                                                                                |
| ----------- | ------------------------------------------------------------------------------------------------------ |
| `queued`    | تسک ایجاد شده و منتظر است ارائه‌دهنده آن را بپذیرد.                                                   |
...
اگر یک تسک ویدئو از قبل برای نشست فعلی `queued` یا `running` باشد،
`video_generate` به‌جای شروع یک
تسک جدید، وضعیت تسک موجود را برمی‌گرداند. برای بررسی صریح بدون راه‌اندازی یک
تولید جدید، از `action: "status"` استفاده کنید.
...
| BytePlus Seedance 2.0 | `dreamina-seedance-2-0-260128`  |  ✓   | تا ۹ تصویر مرجع                             | تا ۳ ویدئو                                  | `BYTEPLUS_API_KEY`                       |
| ComfyUI               | `workflow`                      |  ✓   | ۱ تصویر                                              | —                                               | `COMFY_API_KEY` یا `COMFY_CLOUD_API_KEY` |
| DeepInfra             | `Pixverse/Pixverse-T2V`         |  ✓   | —                                                    | —                                               | `DEEPINFRA_API_KEY`                      |
| fal                   | `fal-ai/minimax/video-01-live`  |  ✓   | ۱ تصویر؛ تا ۹ تصویر با Seedance reference-to-video    | تا ۳ ویدئو با Seedance reference-to-video | `FAL_KEY`                                |
| Google                | `veo-3.1-fast-generate-preview` |  ✓   | ۱ تصویر                                              | ۱ ویدئو                                         | `GEMINI_API_KEY`                         |
| MiniMax               | `MiniMax-Hailuo-2.3`            |  ✓   | ۱ تصویر                                              | —                                               | `MINIMAX_API_KEY` یا MiniMax OAuth       |
| OpenAI                | `sora-2`                        |  ✓   | ۱ تصویر                                              | ۱ ویدئو                                         | `OPENAI_API_KEY`                         |
...
| Runway                | `gen4.5`                        |  ✓   | ۱ تصویر                                              | ۱ ویدئو                                         | `RUNWAYML_API_SECRET`                    |
| Together              | `Wan-AI/Wan2.2-T2V-A14B`        |  ✓   | ۱ تصویر                                              | —                                               | `TOGETHER_API_KEY`                       |
| Vydra                 | `veo3`                          |  ✓   | ۱ تصویر (`kling`)                                    | —                                               | `VYDRA_API_KEY`                          |
| xAI                   | `grok-imagine-video`            |  ✓   | ۱ تصویر فریم اول یا تا ۷ `reference_image`    | ۱ ویدئو                                         | `XAI_API_KEY`                            |

برخی ارائه‌دهندگان متغیرهای محیطی کلید API اضافی یا جایگزین را می‌پذیرند. برای جزئیات،
[صفحه‌های ارائه‌دهنده](#related) را ببینید.

برای بررسی ارائه‌دهندگان، مدل‌ها، و حالت‌های زمان اجرای در دسترس در زمان اجرا، `video_generate action=list` را اجرا کنید.

### ماتریس قابلیت‌ها

قرارداد حالت صریح که توسط `video_generate`، آزمون‌های قرارداد، و
پویش زنده مشترک استفاده می‌شود:

| ارائه‌دهنده   | `generate` | `imageToVideo` | `videoToVideo` | مسیرهای زنده مشترک امروز                                                                                                                  |
| ---------- | :--------: | :------------: | :------------: | ---------------------------------------------------------------------------------------------------------------------------------------- |
| Alibaba    |     ✓      |       ✓        |       ✓        | `generate`، `imageToVideo`؛ `videoToVideo` رد می‌شود چون این ارائه‌دهنده به URLهای ویدئویی `http(s)` راه‌دور نیاز دارد                               |
| BytePlus   |     ✓      |       ✓        |       —        | `generate`، `imageToVideo`                                                                                                               |
| ComfyUI    |     ✓      |       ✓        |       —        | در پویش مشترک نیست؛ پوشش مختص workflow همراه آزمون‌های Comfy قرار دارد                                                               |
| DeepInfra  |     ✓      |       —        |       —        | `generate`؛ schemaهای ویدئوی بومی DeepInfra در قرارداد همراه، متن به ویدئو هستند                                                     |
| fal        |     ✓      |       ✓        |       ✓        | `generate`، `imageToVideo`؛ `videoToVideo` فقط هنگام استفاده از Seedance reference-to-video                                                   |
| Google     |     ✓      |       ✓        |       ✓        | `generate`، `imageToVideo`؛ `videoToVideo` مشترک رد می‌شود چون پویش Gemini/Veo فعلی مبتنی بر بافر آن ورودی را نمی‌پذیرد  |
...
| Qwen       |     ✓      |       ✓        |       ✓        | `generate`، `imageToVideo`؛ `videoToVideo` رد می‌شود چون این ارائه‌دهنده به URLهای ویدئویی `http(s)` راه‌دور نیاز دارد                               |
| Runway     |     ✓      |       ✓        |       ✓        | `generate`، `imageToVideo`؛ `videoToVideo` فقط وقتی اجرا می‌شود که مدل انتخاب‌شده `runway/gen4_aleph` باشد                                      |
| Together   |     ✓      |       ✓        |       —        | `generate`، `imageToVideo`                                                                                                               |
| Vydra      |     ✓      |       ✓        |       —        | `generate`؛ `imageToVideo` مشترک رد می‌شود چون `veo3` همراه فقط متنی است و `kling` همراه به یک URL تصویر راه‌دور نیاز دارد            |
| xAI        |     ✓      |       ✓        |       ✓        | `generate`، `imageToVideo`؛ `videoToVideo` رد می‌شود چون این ارائه‌دهنده در حال حاضر به یک URL راه‌دور MP4 نیاز دارد                                |
...
<ParamField path="prompt" type="string" required>
  شرح متنی ویدئویی که باید تولید شود. برای `action: "generate"` الزامی است.
</ParamField>

### ورودی‌های محتوا

<ParamField path="image" type="string">یک تصویر مرجع واحد (مسیر یا URL).</ParamField>
<ParamField path="images" type="string[]">چند تصویر مرجع (تا 9 عدد).</ParamField>
<ParamField path="imageRoles" type="string[]">
راهنماهای اختیاری نقش برای هر موقعیت، موازی با فهرست ترکیبی تصاویر.
...
<ParamField path="videos" type="string[]">چند ویدیوی مرجع (تا 4 عدد).</ParamField>
<ParamField path="videoRoles" type="string[]">
راهنماهای اختیاری نقش برای هر موقعیت، موازی با فهرست ترکیبی ویدیوها.
مقدار استاندارد: `reference_video`.
</ParamField>
<ParamField path="audioRef" type="string">
یک صدای مرجع واحد (مسیر یا URL). برای موسیقی پس‌زمینه یا مرجع صدا
وقتی ارائه‌دهنده از ورودی‌های صوتی پشتیبانی می‌کند استفاده می‌شود.
</ParamField>
<ParamField path="audioRefs" type="string[]">چند صدای مرجع (تا 3 عدد).</ParamField>
...
<Note>
راهنماهای نقش بدون تغییر به ارائه‌دهنده ارسال می‌شوند. مقادیر استاندارد از
union مربوط به `VideoGenerationAssetRole` می‌آیند، اما ارائه‌دهندگان ممکن است
رشته‌های نقش بیشتری را بپذیرند. آرایه‌های `*Roles` نباید ورودی‌های بیشتری از
فهرست مرجع متناظر داشته باشند؛ خطاهای یکی‌کم‌یکی با خطایی روشن شکست می‌خورند.
برای تنظیم‌نشده گذاشتن یک جایگاه، از رشته خالی استفاده کنید. برای xAI، نقش هر تصویر را روی
...
<ParamField path="aspectRatio" type="string">
  راهنمای نسبت تصویر مانند `1:1`، `16:9`، `9:16`، `adaptive`، یا مقداری ویژه ارائه‌دهنده. OpenClaw مقادیر پشتیبانی‌نشده را برای هر ارائه‌دهنده عادی‌سازی یا نادیده می‌گیرد.
</ParamField>
<ParamField path="resolution" type="string">راهنمای وضوح مانند `480P`، `720P`، `768P`، `1080P`، `4K`، یا مقداری ویژه ارائه‌دهنده. OpenClaw مقادیر پشتیبانی‌نشده را برای هر ارائه‌دهنده عادی‌سازی یا نادیده می‌گیرد.</ParamField>
<ParamField path="durationSeconds" type="number">
  مدت هدف بر حسب ثانیه (گردشده به نزدیک‌ترین مقدار پشتیبانی‌شده توسط ارائه‌دهنده).
</ParamField>
<ParamField path="size" type="string">راهنمای اندازه وقتی ارائه‌دهنده از آن پشتیبانی می‌کند.</ParamField>
<ParamField path="audio" type="boolean">
  وقتی پشتیبانی شود، صدای تولیدشده را در خروجی فعال می‌کند. از `audioRef*` (ورودی‌ها) جداست.
</ParamField>
...
ارائه‌دهندگانی ارسال می‌شود که `adaptive` را در قابلیت‌های خود اعلام می‌کنند (مثلاً BytePlus
...
<ParamField path="timeoutMs" type="number">مهلت زمانی اختیاری عملیات ارائه‌دهنده بر حسب میلی‌ثانیه.</ParamField>
<ParamField path="providerOptions" type="object">
  گزینه‌های ویژه ارائه‌دهنده به‌صورت یک شیء JSON (مثلاً `{"seed": 42, "draft": true}`).
  ارائه‌دهندگانی که schema تایپ‌شده اعلام می‌کنند، کلیدها و نوع‌ها را اعتبارسنجی می‌کنند؛ کلیدهای ناشناخته
  یا ناهماهنگی‌ها باعث می‌شوند نامزد در هنگام fallback رد شود. ارائه‌دهندگان بدون
  schema اعلام‌شده، گزینه‌ها را همان‌طور که هستند دریافت می‌کنند. `video_generate action=list` را اجرا کنید
...
مانند اندازه به نسبت تصویر را وقتی یک ارائه‌دهنده fallback سطح کنترل متفاوتی
ارائه می‌دهد بازنگاشت می‌کند. بازنویسی‌های واقعاً پشتیبانی‌نشده تا حد امکان
نادیده گرفته می‌شوند و به‌صورت هشدار در نتیجه ابزار گزارش می‌شوند. محدودیت‌های سخت قابلیت
(مانند ورودی‌های مرجع بیش از حد) پیش از ارسال شکست می‌خورند. نتایج ابزار
...
مرز ابزار، بنابراین درخواستی که از محدودیت‌های ارائه‌دهنده اصلی فراتر می‌رود می‌تواند
همچنان روی یک fallback توانمند اجرا شود:

- نامزد فعال که هیچ `maxInputAudios` اعلام نکرده (یا `0`) وقتی
  درخواست شامل مراجع صوتی باشد رد می‌شود؛ نامزد بعدی امتحان می‌شود.
- `maxDurationSeconds` نامزد فعال پایین‌تر از `durationSeconds` درخواستی
  و بدون فهرست اعلام‌شده `supportedDurationSeconds` → رد می‌شود.
- درخواست شامل `providerOptions` است و نامزد فعال به‌صراحت
  یک schema تایپ‌شده `providerOptions` اعلام می‌کند → اگر کلیدهای ارائه‌شده
  در schema نباشند یا نوع مقدارها مطابقت نداشته باشد رد می‌شود. ارائه‌دهندگان بدون
  schema اعلام‌شده، گزینه‌ها را همان‌طور که هستند دریافت می‌کنند (عبور
  سازگار با نسخه‌های قبلی). یک ارائه‌دهنده می‌تواند با اعلام یک schema خالی
  (`capabilities.providerOptions: {}`) از همه گزینه‌های ارائه‌دهنده انصراف دهد، که
  همان رد شدنِ ناهماهنگی نوع را ایجاد می‌کند.

اولین دلیل رد شدن در یک درخواست با سطح `warn` ثبت می‌شود تا اپراتورها ببینند چه زمانی
ارائه‌دهنده اصلی آن‌ها کنار گذاشته شده است؛ رد شدن‌های بعدی با سطح `debug` ثبت می‌شوند تا
زنجیره‌های fallback طولانی کم‌صدا بمانند. اگر همه نامزدها رد شوند، خطای
...
| ---------- | -------------------------------------------------------------------------------------------------------- |
| `generate` | پیش‌فرض. یک ویدیو از prompt داده‌شده و ورودی‌های مرجع اختیاری ایجاد می‌کند.                             |
...
1. **پارامتر ابزار `model`** — اگر agent یکی را در فراخوانی مشخص کند.
...
<AccordionGroup>
  ...
  <Accordion title="BytePlus (1.0)">
    ...
    پشتیبانی می‌کنند. تصویر را به‌صورت موقعیتی ارسال کنید یا `role: "first_frame"` را تنظیم کنید.
    شناسه‌های مدل T2V وقتی تصویری ارائه شود، به‌صورت خودکار به نسخه متناظر I2V
    تغییر داده می‌شوند.

    کلیدهای پشتیبانی‌شده `providerOptions`: `seed` (عدد)، `draft` (بولین —
    اجبار به 480p)، `camera_fixed` (بولین).
  ...
  <Accordion title="BytePlus Seedance 2.0">
    ...
    از API یکپارچه `content[]` استفاده می‌کند. تا 9 تصویر مرجع،
    3 ویدیوی مرجع، و 3 صدای مرجع را پشتیبانی می‌کند. همه ورودی‌ها باید URLهای راه دور
  ...
  <Accordion title="fal">
    برای کارهای طولانی‌مدت از جریان مبتنی بر صف استفاده می‌کند. OpenClaw به‌طور پیش‌فرض تا ۲۰
    دقیقه منتظر می‌ماند و پس از آن کار در حال اجرای صف fal را منقضی‌شده در نظر می‌گیرد. بیشتر مدل‌های ویدیویی fal
    یک مرجع تصویری واحد را می‌پذیرند. مدل‌های مرجع به ویدیوی Seedance 2.0
    تا ۹ تصویر، ۳ ویدیو و ۳ مرجع صوتی را می‌پذیرند، با
    حداکثر ۱۲ فایل مرجع در مجموع.
  </Accordion>
  <Accordion title="Google (Gemini / Veo)">
    از یک تصویر یا یک مرجع ویدیویی پشتیبانی می‌کند. درخواست‌های صدای تولیدشده
  ...
  <Accordion title="MiniMax">
    فقط یک مرجع تصویری واحد. MiniMax وضوح‌های `768P` و `1080P`
  ...
  <Accordion title="OpenRouter">
    ...
    endpoint مستندشده محتوای کار را دانلود می‌کند. پیش‌فرض همراه `google/veo-3.1-fast`
    مدت‌زمان‌های ۴/۶/۸ ثانیه، وضوح‌های `720P`/`1080P` و
    نسبت‌های تصویر `16:9`/`9:16` را اعلام می‌کند.
  </Accordion>
  <Accordion title="Qwen">
    همان backend DashScope مانند Alibaba. ورودی‌های مرجع باید URLهای راه دور
  ...
  <Accordion title="Vydra">
    برای جلوگیری از redirectهایی که احراز هویت را حذف می‌کنند، مستقیماً از `https://www.vydra.ai/api/v1`
    استفاده می‌کند. `veo3` فقط به‌صورت متن به ویدیو همراه شده است؛ `kling` به
    یک URL تصویر راه دور نیاز دارد.
  </Accordion>
  <Accordion title="xAI">
    از متن به ویدیو، تصویر اولین فریم واحد به ویدیو، تا ۷
    ورودی `reference_image` از طریق `reference_images` در xAI، و جریان‌های
...
ویژه هر حالت پشتیبانی می‌کند. پیاده‌سازی‌های جدید ارائه‌دهنده
باید بلوک‌های حالت صریح را ترجیح دهند:
...
آزمون‌های قرارداد و ابزار مشترک `video_generate` بتوانند
پشتیبانی حالت را به‌صورت قطعی اعتبارسنجی کنند.

وقتی یک مدل در یک ارائه‌دهنده پشتیبانی گسترده‌تری از ورودی مرجع نسبت به
بقیه دارد، به‌جای افزایش محدودیت سراسری حالت، از `maxInputImagesByModel`، `maxInputVideosByModel` یا
`maxInputAudiosByModel` استفاده کنید.

## آزمون‌های زنده

پوشش زنده اختیاری برای ارائه‌دهندگان همراه مشترک:
...
این فایل زنده env varهای گمشده ارائه‌دهنده را از `~/.profile` بارگذاری می‌کند، به‌طور پیش‌فرض
کلیدهای API زنده/env را بر auth profileهای ذخیره‌شده ترجیح می‌دهد، و به‌طور پیش‌فرض
یک smoke امن برای release اجرا می‌کند:

- `generate` برای هر ارائه‌دهنده غیر FAL در sweep.
- prompt خرچنگ یک‌ثانیه‌ای.
- سقف عملیات برای هر ارائه‌دهنده از
  `OPENCLAW_LIVE_VIDEO_GENERATION_TIMEOUT_MS` (`180000` به‌طور پیش‌فرض).

FAL اختیاری است، چون تأخیر صف سمت ارائه‌دهنده می‌تواند بر زمان release
غلبه کند:
...
برای اجرای حالت‌های تبدیل اعلام‌شده‌ای که sweep مشترک می‌تواند با رسانه محلی به‌صورت امن تمرین کند نیز
...
- `videoToVideo` وقتی `capabilities.videoToVideo.enabled` و
  ارائه‌دهنده/مدل ورودی ویدیوی محلی مبتنی بر buffer را در sweep مشترک می‌پذیرد.

امروز lane زنده مشترک `videoToVideo` فقط وقتی `runway` را انتخاب کنید
و `runway/gen4_aleph` را پوشش می‌دهد.
...
- [وظایف پس‌زمینه](/fa/automation/tasks) — رهگیری task برای تولید ویدیوی ناهمگام
