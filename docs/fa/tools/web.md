...
read_when:
    ...
    - باید یک ارائه‌دهندهٔ جست‌وجو را انتخاب کنید
    - می‌خواهید تشخیص خودکار و بازگشت به ارائه‌دهندهٔ جایگزین را بفهمید
sidebarTitle: Web Search
summary: web_search، x_search، و web_fetch -- جست‌وجوی وب، جست‌وجوی پست‌های X، یا دریافت محتوای صفحه
title: جستجوی وب
x-i18n:
    generated_at: "2026-05-07T13:34:42Z"
    model: gpt-5.5
    provider: openai
...
نتایج را برمی‌گرداند. نتایج به‌مدت ۱۵ دقیقه بر اساس پرس‌وجو کش می‌شوند (قابل پیکربندی).
...
<Info>
  ...
  سایت‌های سنگین از نظر JS یا ورودها، از [مرورگر وب](/fa/tools/browser) استفاده کنید. برای
  دریافت یک URL مشخص، از [دریافت وب](/fa/tools/web-fetch) استفاده کنید.
</Info>
...
<Steps>
  <Step title="Choose a provider">
    یک ارائه‌دهنده انتخاب کنید و هر راه‌اندازی لازم را کامل کنید. بعضی ارائه‌دهنده‌ها
    بدون کلید هستند، در حالی که بعضی دیگر از کلیدهای API استفاده می‌کنند. برای
    جزئیات، صفحه‌های ارائه‌دهنده در ادامه را ببینید.
  </Step>
  <Step title="Configure">
    ```bash
    openclaw configure --section web
    ```
    این کار ارائه‌دهنده و هر اعتبار لازم را ذخیره می‌کند. همچنین می‌توانید یک متغیر محیطی
...
<CardGroup cols={2}>
  <Card title="Brave Search" icon="shield" href="/fa/tools/brave-search">
    نتایج ساختاریافته همراه با قطعه‌متن‌ها. از حالت `llm-context` و فیلترهای کشور/زبان پشتیبانی می‌کند. سطح رایگان در دسترس است.
  </Card>
  ...
  <Card title="Firecrawl" icon="flame" href="/fa/tools/firecrawl">
    نتایج ساختاریافته. برای استخراج عمیق، بهترین ترکیب با `firecrawl_search` و `firecrawl_scrape` است.
  </Card>
  <Card title="Gemini" icon="sparkles" href="/fa/tools/gemini-search">
    پاسخ‌های ترکیب‌شده با هوش مصنوعی همراه با ارجاع‌ها از طریق استوارسازی Google Search.
  </Card>
  <Card title="Grok" icon="zap" href="/fa/tools/grok-search">
    پاسخ‌های ترکیب‌شده با هوش مصنوعی همراه با ارجاع‌ها از طریق استوارسازی وب xAI.
  </Card>
  <Card title="Kimi" icon="moon" href="/fa/tools/kimi-search">
    پاسخ‌های ترکیب‌شده با هوش مصنوعی همراه با ارجاع‌ها از طریق جست‌وجوی وب Moonshot؛ جایگزین‌های چت بدون استوارسازی صریحا شکست می‌خورند.
  </Card>
...
| [Grok](/fa/tools/grok-search)                | ترکیب‌شده با هوش مصنوعی + ارجاع‌ها                                     | --                                               | `XAI_API_KEY`                                                                           |
| [Kimi](/fa/tools/kimi-search)                | ترکیب‌شده با هوش مصنوعی + ارجاع‌ها؛ در جایگزین‌های چت بدون استوارسازی شکست می‌خورد | --                                               | `KIMI_API_KEY` / `MOONSHOT_API_KEY`                                                     |
...
| [Ollama Web Search](/fa/tools/ollama-search) | قطعه‌متن‌های ساختاریافته                                            | --                                               | هیچ‌کدام برای میزبان‌های محلی واردشده؛ `OLLAMA_API_KEY` برای جست‌وجوی مستقیم `https://ollama.com` |
...
مدل‌های مستقیم OpenAI Responses وقتی جست‌وجوی وب OpenClaw فعال باشد و هیچ ارائه‌دهنده مدیریت‌شده‌ای پین نشده باشد، به‌طور خودکار از ابزار میزبانی‌شده `web_search` متعلق به OpenAI استفاده می‌کنند. این رفتار متعلق به ارائه‌دهنده در Plugin بسته‌بندی‌شده OpenAI است و فقط برای ترافیک بومی OpenAI API اعمال می‌شود، نه URLهای پایه پروکسی سازگار با OpenAI یا مسیرهای Azure. برای نگه داشتن ابزار مدیریت‌شده `web_search` برای مدل‌های OpenAI، `tools.web.search.provider` را روی ارائه‌دهنده دیگری مثل `brave` تنظیم کنید، یا برای غیرفعال کردن هم جست‌وجوی مدیریت‌شده و هم جست‌وجوی بومی OpenAI، `tools.web.search.enabled: false` را تنظیم کنید.

## جست‌وجوی وب بومی Codex

مدل‌های دارای قابلیت Codex می‌توانند به‌صورت اختیاری به‌جای تابع مدیریت‌شده `web_search` متعلق به OpenClaw، از ابزار بومی ارائه‌دهنده Responses با نام `web_search` استفاده کنند.
...
اگر جست‌وجوی بومی Codex فعال باشد اما مدل فعلی قابلیت Codex نداشته باشد، OpenClaw رفتار عادی مدیریت‌شده `web_search` را حفظ می‌کند.

## ایمنی شبکه

فراخوانی‌های ارائه‌دهنده مدیریت‌شده `web_search` از مسیر دریافت محافظت‌شده OpenClaw استفاده می‌کنند. برای
میزبان‌های API ارائه‌دهنده مورد اعتماد، OpenClaw پاسخ‌های DNS جعلی-IP
Surge، Clash، و sing-box را در `198.18.0.0/15` و `fc00::/7` فقط برای همان نام میزبان ارائه‌دهنده مجاز می‌کند.
مقصدهای خصوصی، loopback، link-local، و فراداده دیگر همچنان مسدود می‌مانند.

این مجوز خودکار برای URLهای دلخواه `web_fetch` اعمال نمی‌شود. برای
`web_fetch`، فقط وقتی پروکسی مورد اعتماد شما مالک آن بازه‌های مصنوعی است،
`tools.web.fetch.ssrfPolicy.allowRfc2544BenchmarkRange` و
`tools.web.fetch.ssrfPolicy.allowIpv6UniqueLocalRange` را صریحا فعال کنید.

## راه‌اندازی جست‌وجوی وب

فهرست‌های ارائه‌دهنده در مستندات و جریان‌های راه‌اندازی الفبایی هستند. تشخیص خودکار یک
...
8. **Exa** -- `EXA_API_KEY` یا `plugins.entries.exa.config.webSearch.apiKey`؛ `plugins.entries.exa.config.webSearch.baseUrl` اختیاری نقطه پایانی Exa را بازنویسی می‌کند (ترتیب ۶۵)
9. **Tavily** -- `TAVILY_API_KEY` یا `plugins.entries.tavily.config.webSearch.apiKey` (ترتیب ۷۰)

جایگزین‌های بدون کلید پس از آن:

10. **DuckDuckGo** -- جایگزین HTML بدون کلید، بدون حساب یا کلید API (ترتیب ۱۰۰)
11. **Ollama Web Search** -- جایگزین بدون کلید از طریق میزبان محلی Ollama پیکربندی‌شده شما وقتی قابل دسترسی باشد و با `ollama signin` وارد شده باشید؛ می‌تواند وقتی میزبان به آن نیاز دارد از احراز هویت bearer ارائه‌دهنده Ollama دوباره استفاده کند، و وقتی با `OLLAMA_API_KEY` پیکربندی شده باشد می‌تواند جست‌وجوی مستقیم `https://ollama.com` را فراخوانی کند (ترتیب ۱۱۰)
12. **SearXNG** -- `SEARXNG_BASE_URL` یا `plugins.entries.searxng.config.webSearch.baseUrl` (ترتیب ۲۰۰)

اگر هیچ ارائه‌دهنده‌ای تشخیص داده نشود، به Brave برمی‌گردد (یک خطای کلیدِ ناموجود
...
<Note>
  ...
  ارائه‌دهنده‌های جست‌وجوی وب مبتنی بر API بسته‌بندی‌شده resolve می‌شوند، از جمله Brave، Exa، Firecrawl،
  Gemini، Grok، Kimi، MiniMax، Perplexity، و Tavily،
  چه ارائه‌دهنده صریحا از طریق `tools.web.search.provider` انتخاب شده باشد و چه
  از طریق تشخیص خودکار. در حالت تشخیص خودکار، OpenClaw فقط کلید ارائه‌دهنده
  انتخاب‌شده را resolve می‌کند -- SecretRefهای انتخاب‌نشده غیرفعال می‌مانند، بنابراین می‌توانید
...
پیکربندی ویژه‌ی ارائه‌دهنده (کلیدهای API، نشانی‌های پایه و حالت‌ها) زیر
`plugins.entries.<plugin>.config.webSearch.*` قرار می‌گیرد. Gemini همچنین می‌تواند
`models.providers.google.apiKey` و `models.providers.google.baseUrl` را به‌عنوان
جایگزین‌هایی با اولویت پایین‌تر، پس از پیکربندی اختصاصی جست‌وجوی وب و `GEMINI_API_KEY` خود دوباره استفاده کند. برای نمونه‌ها، صفحه‌های ارائه‌دهنده را ببینید.

`tools.web.search.provider` در برابر شناسه‌های ارائه‌دهنده‌ی جست‌وجوی وب
اعلام‌شده در مانیفست‌های Pluginهای همراه و نصب‌شده اعتبارسنجی می‌شود. یک غلط املایی مانند `"brvae"`
به‌جای بازگشت بی‌سروصدا به تشخیص خودکار، باعث شکست اعتبارسنجی پیکربندی می‌شود. اگر یک
ارائه‌دهنده‌ی پیکربندی‌شده فقط شواهد Plugin کهنه داشته باشد، مانند بلوک باقی‌مانده‌ی
`plugins.entries.<plugin>` پس از حذف یک Plugin شخص ثالث،
OpenClaw راه‌اندازی را مقاوم نگه می‌دارد و هشداری گزارش می‌کند تا بتوانید
Plugin را دوباره نصب کنید یا برای پاک‌سازی پیکربندی کهنه `openclaw doctor --fix` را اجرا کنید.

انتخاب ارائه‌دهنده‌ی جایگزین `web_fetch` جداگانه است:

- آن را با `tools.web.fetch.provider` انتخاب کنید
- یا آن فیلد را حذف کنید و بگذارید OpenClaw نخستین ارائه‌دهنده‌ی آماده‌ی web-fetch
  را از اعتبارنامه‌های موجود به‌صورت خودکار تشخیص دهد
- `web_fetch` غیرسندباکس‌شده می‌تواند از ارائه‌دهندگان Plugin نصب‌شده‌ای استفاده کند که
  `contracts.webFetchProviders` را اعلام می‌کنند؛ واکشی‌های سندباکس‌شده فقط همراه‌ها را نگه می‌دارند
- امروز ارائه‌دهنده‌ی همراه web-fetch، Firecrawl است که زیر
  `plugins.entries.firecrawl.config.webFetch.*` پیکربندی می‌شود

وقتی هنگام `openclaw onboard` یا
`openclaw configure --section web` **Kimi** را انتخاب می‌کنید، OpenClaw همچنین می‌تواند این‌ها را بپرسد:

- منطقه‌ی Moonshot API (`https://api.moonshot.ai/v1` یا `https://api.moonshot.cn/v1`)
...
جایگزین `XAI_API_KEY` مانند جست‌وجوی وب Grok استفاده می‌کند.
پیکربندی قدیمی `tools.web.x_search.*` توسط `openclaw doctor --fix` به‌صورت خودکار مهاجرت داده می‌شود.
وقتی هنگام `openclaw onboard` یا `openclaw configure --section web`، Grok را انتخاب می‌کنید،
OpenClaw همچنین می‌تواند راه‌اندازی اختیاری `x_search` را با همان کلید ارائه کند.
این یک مرحله‌ی پیگیری جداگانه داخل مسیر Grok است، نه یک انتخاب جداگانه‌ی سطح‌بالای
ارائه‌دهنده‌ی جست‌وجوی وب. اگر ارائه‌دهنده‌ی دیگری را انتخاب کنید، OpenClaw
...
| `freshness`           | فیلتر زمان: `day`، `week`، `month` یا `year`           |
...
| `domain_filter`       | آرایه‌ی فهرست مجاز/ممنوع دامنه‌ها (فقط Perplexity)    |
| `max_tokens`          | بودجه‌ی کل محتوا، پیش‌فرض 25000 (فقط Perplexity)      |
| `max_tokens_per_page` | محدودیت توکن به‌ازای هر صفحه، پیش‌فرض 2048 (فقط Perplexity) |

<Warning>
  همه‌ی پارامترها با همه‌ی ارائه‌دهنده‌ها کار نمی‌کنند. حالت `llm-context` در Brave
  `ui_lang` را رد می‌کند؛ `date_before` همچنین به `date_after` نیاز دارد، چون بازه‌های
  تازگی سفارشی Brave به هر دو تاریخ شروع و پایان نیاز دارند.
  Gemini، Grok و Kimi یک پاسخ ترکیب‌شده با ارجاع‌ها بازمی‌گردانند. آن‌ها
  `count` را برای سازگاری ابزار مشترک می‌پذیرند، اما شکل پاسخ مبتنی بر منبع را تغییر نمی‌دهد.
  Gemini از `freshness`، `date_after` و
  `date_before` با تبدیل آن‌ها به بازه‌های زمانی مبتنی بر Google Search پشتیبانی می‌کند.
  Perplexity وقتی از مسیر سازگاری Sonar/OpenRouter استفاده می‌کنید
  (`plugins.entries.perplexity.config.webSearch.baseUrl` /
  `model` یا `OPENROUTER_API_KEY`) همین رفتار را دارد.
  SearXNG فقط برای میزبان‌های معتمد شبکه‌ی خصوصی یا loopback، `http://` را می‌پذیرد؛
...
`x_search` پست‌های X (توییتر سابق) را با استفاده از xAI جست‌وجو می‌کند و
پاسخ‌های ترکیب‌شده با هوش مصنوعی و همراه با ارجاع‌ها بازمی‌گرداند. این ابزار پرس‌وجوهای زبان طبیعی و
فیلترهای ساختاریافته‌ی اختیاری را می‌پذیرد. OpenClaw فقط در درخواستی که این فراخوانی ابزار را سرو می‌کند،
ابزار داخلی xAI `x_search` را فعال می‌کند.

<Note>
  xAI مستند کرده است که `x_search` از جست‌وجوی کلیدواژه، جست‌وجوی معنایی، جست‌وجوی کاربر
  و واکشی رشته پشتیبانی می‌کند. برای آمار تعامل هر پست مانند بازنشرها،
  پاسخ‌ها، نشانک‌ها یا بازدیدها، جست‌وجوی هدفمند برای URL دقیق پست
  یا شناسه‌ی وضعیت را ترجیح دهید. جست‌وجوهای کلیدواژه‌ای گسترده ممکن است پست درست را پیدا کنند اما فراداده‌ی هر پست را
  کمتر کامل بازگردانند. الگوی خوب این است: ابتدا پست را پیدا کنید، سپس
...
`tools.web.search.grok.baseUrl` قدیمی و در نهایت نقطه‌ی پایانی عمومی xAI بازمی‌گردد.
...
| ---------------------------- | ------------------------------------------------------ |
| `query`                      | پرس‌وجوی جست‌وجو (الزامی)                             |
| `allowed_x_handles`          | محدود کردن نتایج به دسته‌های مشخص X                   |
| `excluded_x_handles`         | حذف دسته‌های مشخص X                                   |
| `from_date`                  | فقط شامل پست‌های در این تاریخ یا پس از آن (YYYY-MM-DD) |
| `to_date`                    | فقط شامل پست‌های در این تاریخ یا پیش از آن (YYYY-MM-DD) |
| `enable_image_understanding` | اجازه دهید xAI تصاویر پیوست‌شده به پست‌های منطبق را بررسی کند |
| `enable_video_understanding` | اجازه دهید xAI ویدیوهای پیوست‌شده به پست‌های منطبق را بررسی کند |
...
- [Web Fetch](/fa/tools/web-fetch) -- واکشی یک URL و استخراج محتوای خوانا
- [Web Browser](/fa/tools/browser) -- اتوماسیون کامل مرورگر برای سایت‌های سنگین از نظر JS
- [Grok Search](/fa/tools/grok-search) -- Grok به‌عنوان ارائه‌دهنده‌ی `web_search`
