...
این مورد در سیستم‌هایی لازم است که اتصال‌های خروجی مستقیم در آن‌ها مسدود است
...
وقتی هیچ متغیر پروکسی تنظیم نشده باشد، رفتار تغییر نمی‌کند (اتصال‌های مستقیم).
...
- پیش‌فرض: مرورگر را به `<site>/cli/auth` باز می‌کند و از طریق callback حلقه برگشتی تکمیل می‌شود.
- بدون رابط گرافیکی: `clawhub login --token clh_...`
- تعاملی راه‌دور/بدون رابط گرافیکی: `clawhub login --device` یک کد چاپ می‌کند و تا زمانی که آن را در `<site>/cli/device` مجاز کنید منتظر می‌ماند.
...
- برای پایپ کردن یک توکن ورود محلی به دستورهای راه‌اندازی secret در CI مفید است.
...
- خروجی شامل slug مهارت، شناسه مالک، نام نمایشی، و امتیاز ارتباط است.
- جست‌وجو پیش از محبوبیت دانلود، تطابق‌های دقیق توکن slug/نام را ترجیح می‌دهد. یک توکن مستقل slug مانند `map` با `personal-map` قوی‌تر از زیررشته داخل `amap` تطابق دارد.
- محبوبیت یک پیش‌فرض کوچک برای رتبه‌بندی است، نه تضمینی برای قرار گرفتن در رتبه اول.
- اگر مهارتی باید ظاهر شود اما ظاهر نمی‌شود، هنگام ورود `clawhub inspect @owner/slug` را اجرا کنید تا پیش از تغییر نام فراداده، عیب‌یابی‌های moderation قابل مشاهده برای مالک را بررسی کنید.
...
- `--tag <tag>`: بررسی یک نسخه برچسب‌دار (مثلاً `latest`).
...
- هنگام ورود، telemetry را با بهترین تلاش ارسال می‌کند تا شمار نصب‌های فعلی
  غیرفعال شوند.
...
- `--reason <text>` دلیل منجمد شدن مهارت را ثبت می‌کند.
- Skills پین‌شده توسط `update --all` نادیده گرفته می‌شوند و توسط `update <skill>` مستقیم رد می‌شوند.
...
- `update --all` از slugهای پین‌شده عبور می‌کند و خلاصه‌ای از مواردی که منجمد ماندند چاپ می‌کند.
...
- `--dry-run` انتشار را بدون آپلود resolve می‌کند؛ `--json` نتیجه‌ای
  قابل‌خواندن برای ماشین چاپ می‌کند.
- `--owner <handle>` وقتی عامل دسترسی ناشر داشته باشد، زیر شناسه ناشر org/user منتشر می‌کند.
- `--migrate-owner` یک مهارت موجود را هنگام انتشار نسخه‌ای جدید به `--owner` منتقل می‌کند. به دسترسی admin/owner در هر دو ناشر نیاز دارد.
...
- ClawHub از Skills پولی یا قیمت‌گذاری به‌ازای هر مهارت پشتیبانی نمی‌کند.
...
برای یک `skill_path`، یا برای هر پوشه مستقیم مهارت زیر `root` (پیش‌فرض: `skills`) `skill publish` را فراخوانی می‌کند. Skills تغییریافته‌نشده را نادیده می‌گیرد و از همان رفتار خودکار نسخه patch استفاده می‌کند.
...
- اثرانگشت هر مهارت محلی را با ClawHub مقایسه می‌کند و فقط Skills جدید یا
  تغییریافته را منتشر می‌کند.
- Skills جدید به‌صورت `1.0.0` منتشر می‌شوند؛ Skills تغییریافته به‌طور پیش‌فرض
  نسخه patch بعدی را منتشر می‌کنند. برای دسته‌های به‌روزرسانی که باید با گام semver
  بزرگ‌تری حرکت کنند، از `--bump minor|major` استفاده کنید.
- `--dry-run` برنامه انتشار را بدون آپلود نشان می‌دهد؛ `--json` برنامه‌ای
  قابل‌خواندن برای ماشین چاپ می‌کند.
- `--all` هر مهارت جدید یا تغییریافته را بدون اعلان منتشر می‌کند. بدون
  `--all`، ترمینال‌های تعاملی به شما اجازه می‌دهند Skills مورد انتشار را انتخاب کنید.
- `--owner <handle>` وقتی عامل دسترسی ناشر داشته باشد، زیر شناسه ناشر org/user منتشر می‌کند.
...
- اسکن‌ها ناهمگام هستند و ممکن است تکمیل آن‌ها زمان ببرد. هنگام قرار گرفتن در صف، spinner ترمینال جایگاه اسکن اولویت‌بندی‌شده فعلی و تعداد اسکن‌های جلوتر را نشان می‌دهد.
...
- `--update` فقط با `--slug` معتبر است؛ نتایج موفق اسکن منتشرشده را به نسخه انتخاب‌شده می‌نویسد.
...
- اسکن‌های مسیر محلی دیگر پشتیبانی نمی‌شوند. یک نسخه جدید آپلود کنید، سپس از `scan download` برای بازیابی نتایج اسکن ذخیره‌شده برای آن نسخه ارسالی استفاده کنید.
...
- `--version` الزامی است تا نویسندگان همان نسخه ارسالی دقیقی را که ClawHub مسدود کرده بررسی کنند.
...
- انتشار مهارت V1 از `clawhub_token` استفاده می‌کند؛ انتشار مورداعتماد GitHub OIDC فعلاً فقط برای بسته‌ها است.
...
- `--version <version>` یک نسخهٔ غیرآخرِ متعلق به شما را از طریق یک مسیر fail-closed و
  مختص نسخه، برای همیشه حذف می‌کند.
  نسخه‌های حذف‌شده را نمی‌توان بازیابی یا دوباره منتشر کرد. پیش از حذف نسخهٔ آخر فعلی،
  یک جایگزین منتشر کنید. کارکنان پلتفرم برای این جریان فقط-نسخه، مالکیت را دور نمی‌زنند.
- `--reason <text>` یک یادداشت نظارتی را روی حذف نرم کل مهارت و گزارش حسابرسی ثبت می‌کند.
...
- undelete نسخه وجود ندارد؛ نسخه‌های حذف‌شدهٔ دائمی را نمی‌توان بازیابی کرد.
...
- یک مهارت متعلق به خود را تغییرنام دهید و slug قبلی را به‌عنوان نام مستعار تغییرمسیر نگه دارید.
...
- انتقال به handleهای سازمان/ناشر فقط زمانی فوراً اعمال می‌شود که کنشگر به هر دو
  مالک فعلی و ناشر مقصد دسترسی مدیر داشته باشد.
- زیرفرمان‌ها:
...
- از این برای Pluginها و ورودی‌های دیگر خانوادهٔ بسته استفاده کنید؛ `search` سطح بالا همچنان سطح جست‌وجوی مهارت باقی می‌ماند.
- پرچم‌ها:
...
- فرادادهٔ بسته را بدون نصب دریافت می‌کند.
...
- برای artifactهای npm-pack در ClawPack، همچنین یکپارچگی `sha512` مربوط به npm،
  shasum مربوط به npm، و نام/نسخهٔ `package.json` در tarball را راستی‌آزمایی می‌کند.
- نسخه‌های ZIP قدیمی از طریق مسیر ZIP قدیمی دانلود می‌شوند.
- پرچم‌ها:
  - `--version <version>`: دانلود یک نسخهٔ مشخص.
  - `--tag <tag>`: دانلود یک نسخهٔ برچسب‌خورده (پیش‌فرض: `latest`).
...
- با `--package`، فرادادهٔ مورد انتظار را از ClawHub حل می‌کند و
...
- پرچم‌ها:
  - `--package <name>`: نام بسته برای حل فرادادهٔ artifact مورد انتظار.
...
- Plugin Inspector همراه CLI ClawHub را در برابر پوشهٔ بستهٔ Plugin محلی
  اجرا می‌کند.
- به‌طور پیش‌فرض، اعتبارسنجی آفلاین/ایستا انجام می‌دهد، بدون مکان‌یابی یا واردکردن checkout محلی
  OpenClaw.
...
- پرچم‌ها:
  ...
  - `--runtime`: ضبط runtime را فعال کنید؛ کد Plugin را وارد می‌کند.
  - `--allow-execute`: اجازهٔ ضبط runtime در یک workspace ایزوله را بدهید.
...
اگر اعتبارسنجی یک یافتهٔ بسته، manifest، import SDK، یا artifact گزارش کرد، به
[رفع‌های اعتبارسنجی Plugin](/fa/clawhub/plugin-validation-fixes) مراجعه کنید، سپس فرمان را دوباره اجرا کنید.

### `package delete <name>`

- بدون `--version`، یک بسته و همهٔ releaseها را به‌صورت حذف نرم حذف می‌کند.
- `--version <version>` یک release غیرآخرِ متعلق به شما را از طریق یک مسیر fail-closed و
  مختص نسخه، برای همیشه حذف می‌کند.
  نسخه‌های حذف‌شده را نمی‌توان بازیابی یا دوباره منتشر کرد. پیش از حذف نسخهٔ آخر فعلی،
...
- مسدودکننده‌های وضعیت رسمی، در دسترس بودن ClawPack، digest مربوط به artifact،
  منشأ منبع، سازگاری OpenClaw، هدف‌های میزبان، فرادادهٔ محیط،
...
- handle به حروف کوچک نرمال‌سازی می‌شود و می‌تواند با `@` یا بدون آن ارسال شود.
- ناشران سازمانی تازه‌ساخته‌شده به‌طور پیش‌فرض مورداعتماد/رسمی نیستند.
...
- `<source>` این موارد را می‌پذیرد:
  ...
  - tarball محلی ClawPack از نوع npm-pack: `./my-plugin-1.2.3.tgz`
...
- پوشه‌های Plugin کد پیش از بارگذاری در یک tarball npm از نوع ClawPack بسته‌بندی می‌شوند تا
  نصب‌های OpenClaw بتوانند آرتیفکت دقیق را تأیید کنند. پوشه‌های Plugin بسته همچنان
...
- `--dry-run` بدون بارگذاری، payload انتشار حل‌شده را پیش‌نمایش می‌کند.
- `--json` خروجی قابل خواندن توسط ماشین را برای CI منتشر می‌کند.
- `--owner <handle>` زمانی که actor دسترسی ناشر داشته باشد، زیر handle ناشر کاربر یا سازمان منتشر می‌کند.
...
ابتدا از `--dry-run` استفاده کنید تا بتوانید فراداده package حل‌شده و
انتساب منبع را پیش از ایجاد یک release واقعی تأیید کنید:
...
- `package.json.version` نسخه release package شماست، اما به‌عنوان
...
- اگر از release قدیمی‌تر CLI به نام `clawhub` استفاده می‌کنید، پیش از انتشار ارتقا دهید تا
  بررسی‌های preflight محلی پیش از بارگذاری اجرا شوند.
- اگر اعتبارسنجی یک کد اصلاح گزارش کرد، به
  [اصلاح‌های اعتبارسنجی Plugin](/fa/clawhub/plugin-validation-fixes) مراجعه کنید.
...
- workflow قابل استفاده مجدد، `source` را به‌صورت پیش‌فرض روی مخزن caller می‌گذارد.
- برای monorepoها، `source_path` را ارسال کنید تا workflow پوشه package
...
- انتشار trusted بدون secret فقط روی `workflow_dispatch` کار می‌کند؛ pushهای tag همچنان به `clawhub_token` نیاز دارند.
- `clawhub_token` را برای اولین انتشار، packageهای غیرقابل اعتماد یا انتشارهای break-glass در دسترس نگه دارید.
- workflow نتیجه JSON را به‌عنوان آرتیفکت بارگذاری می‌کند و آن را به‌عنوان خروجی‌های workflow در معرض می‌گذارد.

### `package trusted-publisher get <name>`

- پیکربندی ناشر trusted در GitHub Actions را برای یک package نشان می‌دهد.
- پس از تنظیم پیکربندی، از این دستور برای تأیید مخزن، نام فایل workflow
...
- پیکربندی ناشر trusted در GitHub Actions را برای یک package موجود
  متصل یا جایگزین می‌کند.
- ابتدا باید package از طریق `clawhub package publish` عادی دستی یا احراز هویت‌شده با token
  ایجاد شده باشد.
- پس از تنظیم پیکربندی، انتشارهای پشتیبانی‌شده آینده در GitHub Actions می‌توانند از
  OIDC/trusted publishing بدون token بلندمدت ClawHub استفاده کنند.
...
- `--environment <name>` اختیاری است. وقتی پیکربندی شود، environment مربوط به GitHub Actions
  در claimهای OIDC باید دقیقاً مطابقت داشته باشد.
- ClawHub هنگام اجرای این دستور مخزن GitHub پیکربندی‌شده را تأیید می‌کند.
  ...
  به این نیاز دارند که ClawHub به آن مخزن دسترسی GitHub داشته باشد، برای
