...
summary: 'مرجع CLI: دستورها، پرچم‌ها، پیکربندی و رفتار فایل قفل.'
x-i18n:
    generated_at: "2026-06-30T22:24:42Z"
    model: gpt-5.5
...
پراکسی‌های سازمانی یا شبکه‌های محدود قرار دارند رعایت می‌کند:
...
پراکسی مشخص‌شده مسیریابی می‌کند. `HTTPS_PROXY` برای درخواست‌های HTTPS و `HTTP_PROXY`
برای HTTP ساده استفاده می‌شود. `NO_PROXY` / `no_proxy` برای دور زدن پراکسی برای
میزبان‌ها یا دامنه‌های مشخص رعایت می‌شود.

این مورد در سیستم‌هایی لازم است که اتصال مستقیم خروجی در آن‌ها مسدود است
...
وقتی هیچ متغیر پراکسی تنظیم نشده باشد، رفتار بدون تغییر است (اتصال مستقیم).
...
- تعاملی راه‌دور/بدون رابط گرافیکی: `clawhub login --device` یک کد چاپ می‌کند و منتظر می‌ماند تا آن را در `<site>/cli/device` مجاز کنید.
...
- یک مهارت را به برجسته‌های شما اضافه یا از آن حذف می‌کند.
...
- جست‌وجو تطابق‌های دقیق توکن slug/name را پیش از محبوبیت دانلود ترجیح می‌دهد. یک توکن slug مستقل مانند `map` با `personal-map` قوی‌تر از زیررشته داخل `amap` تطابق دارد.
- محبوبیت یک prior رتبه‌بندی کوچک است، نه تضمینی برای قرارگیری در رتبه نخست.
- اگر مهارتی باید ظاهر شود اما نمی‌شود، در حالی که وارد شده‌اید `clawhub inspect @owner/slug` را اجرا کنید تا پیش از تغییر نام فراداده، عیب‌یابی‌های moderation قابل‌مشاهده برای مالک را بررسی کنید.
...
- پرچم‌ها:
  ...
  - `--sort newest|updated|rating|downloads|trending` (پیش‌فرض: newest). نام‌های مستعار مرتب‌سازی نصب میراثی همچنان برای سازگاری کار می‌کنند.
...
- هنگام ورود، telemetry به‌صورت best-effort ارسال می‌کند تا شمارش نصب‌های فعلی بتواند
  غیرفعال شود.
...
- مهارت‌های pinned توسط `update --all` رد می‌شوند و با `update <skill>` مستقیم نیز رد می‌شوند.
- مهارت‌های pinned همچنین `install --force` را رد می‌کنند تا بایت‌های محلی تصادفاً جایگزین نشوند.

### `unpin <skill>`

- pin موجود در lockfile را از یک مهارت نصب‌شده حذف می‌کند تا به‌روزرسانی‌های آینده بتوانند آن را تغییر دهند.

### `update [@owner/slug]` / `update --all`

- fingerprint را از فایل‌های محلی محاسبه می‌کند.
- اگر fingerprint با یک نسخه شناخته‌شده تطابق داشته باشد: هیچ اعلانی نمایش داده نمی‌شود.
...
- `update --all` از slugهای pinned عبور می‌کند و خلاصه‌ای از مواردی که منجمد ماندند چاپ می‌کند.
...
- `--dry-run` انتشار را بدون بارگذاری resolve می‌کند؛ `--json` نتیجه‌ای
  قابل‌خواندن برای ماشین چاپ می‌کند.
- `--owner <handle>` وقتی actor دسترسی publisher داشته باشد، زیر یک handle ناشر org/user منتشر می‌کند.
- `--migrate-owner` یک مهارت موجود را هنگام انتشار یک نسخه جدید به `--owner` منتقل می‌کند. به دسترسی admin/owner روی هر دو ناشر نیاز دارد.
- رفتار مالک و review در `docs/publishing.md` توضیح داده شده است.
- انتشار یک مهارت یعنی در ClawHub تحت `MIT-0` منتشر می‌شود.
...
برای یک `skill_path`، یا برای هر پوشه مهارت بلافصل زیر `root` (پیش‌فرض: `skills`) فرمان `skill publish` را فراخوانی می‌کند. مهارت‌های بدون تغییر را رد می‌کند و از همان رفتار خودکار نسخه patch استفاده می‌کند.

برای پیش‌نمایش بدون توکن، `dry_run: true` را تنظیم کنید. انتشارهای واقعی به secret
`clawhub_token` نیاز دارند.
...
- مهارت‌های جدید به‌صورت `1.0.0` منتشر می‌شوند؛ مهارت‌های تغییریافته به‌صورت پیش‌فرض نسخه patch بعدی را منتشر می‌کنند. برای دسته‌های به‌روزرسانی که باید با یک گام semver بزرگ‌تر جابه‌جا شوند، از `--bump minor|major` استفاده کنید.
...
- `--all` هر مهارت جدید یا تغییریافته را بدون اعلان منتشر می‌کند. بدون
  `--all`، ترمینال‌های تعاملی اجازه می‌دهند مهارت‌های موردنظر برای انتشار را انتخاب کنید.
- `--owner <handle>` وقتی actor دسترسی publisher داشته باشد، زیر یک handle ناشر org/user منتشر می‌کند.
...
- ClawHub ClawScan را از طریق `POST /api/v1/skills/-/scan` اجرا می‌کند، سپس تا زمانی که scan نهایی شود poll می‌کند.
- scanها ناهمگام هستند و ممکن است کامل‌شدنشان زمان ببرد. هنگام قرارداشتن در صف، spinner ترمینال موقعیت scan اولویت‌بندی‌شده فعلی و تعداد scanهای جلوتر را نشان می‌دهد.
- scanهای منتشرشده به مالکیت یا دسترسی مدیریت publisher نیاز دارند. moderatorها/adminها می‌توانند از همان backend از طریق `clawhub-admin` استفاده کنند.
...
- `--json` پاسخ کامل poll را برای اتوماسیون چاپ می‌کند.
- scanهای مسیر محلی دیگر پشتیبانی نمی‌شوند. یک نسخه جدید بارگذاری کنید، سپس برای بازیابی نتایج scan ذخیره‌شده برای آن نسخه ارسال‌شده از `scan download` استفاده کنید.
...
- به `clawhub login` نیاز دارد.
- ZIP گزارش scan ذخیره‌شده را برای نسخه ارسال‌شده یک مهارت یا Plugin دانلود می‌کند، از جمله نسخه‌هایی که توسط بررسی‌های امنیتی ClawHub مسدود یا پنهان شده‌اند.
...
- حذف نرم آغازشده توسط مالک، slug را به‌مدت ۳۰ روز رزرو می‌کند؛ فرمان زمان انقضا را چاپ می‌کند.
- `--version <version>` یک نسخه غیرآخرِ متعلق به مالک را از طریق یک مسیر fail-closed
  و مخصوص نسخه، به‌صورت دائمی حذف می‌کند.
  نسخه‌های حذف‌شده قابل بازیابی یا انتشار دوباره نیستند. پیش از حذف نسخه‌ای که در حال حاضر آخرین نسخه است، یک جایگزین منتشر کنید. کارکنان پلتفرم برای این جریان فقط-نسخه، مالکیت را دور نمی‌زنند.
- `--reason <text>` یک یادداشت نظارتی را روی حذف نرم کل مهارت و لاگ ممیزی ثبت می‌کند.
...
- undelete نسخه وجود ندارد؛ نسخه‌های حذف‌شده دائمی قابل بازیابی نیستند.
- `POST /api/v1/skills/{slug}/undelete` را فراخوانی می‌کند.
- `--reason <text>` یک یادداشت نظارتی را روی مهارت و لاگ ممیزی ثبت می‌کند.
- `--note <text>` نام مستعار `--reason` است.
- `--yes` تأیید را رد می‌کند.
...
- یک مهارت متعلق به مالک را تغییر نام دهید و slug قبلی را به‌عنوان نام مستعار تغییرمسیر نگه دارید.
...
- انتقال به handleهای سازمان/ناشر فقط زمانی فوراً اعمال می‌شود که عامل به هر دو ناشر مالک فعلی و مقصد
...
- از این برای plugins و دیگر ورودی‌های خانواده بسته استفاده کنید؛ `search` سطح بالا همچنان سطح جست‌وجوی مهارت است.
- پرچم‌ها:
...
- `--version <version>`: یک نسخه مشخص را بازرسی کنید (پیش‌فرض: آخرین نسخه).
...
- `--file <path>`: محتوای خام فایل را دریافت کنید (فقط فایل‌های متنی؛ حد ۲۰۰KB).
...
- برای artifactهای ClawPack npm-pack، همچنین یکپارچگی `sha512` در npm،
  shasum مربوط به npm، و نام/نسخه `package.json` در tarball را تأیید می‌کند.
...
- SHA-256 مربوط به ClawHub، یکپارچگی `sha512` در npm، و shasum مربوط به npm را برای یک
...
- با پرچم‌های digest مستقیم، بدون جست‌وجوی شبکه‌ای تأیید می‌کند.
- پرچم‌ها:
...
- Plugin Inspector همراه CLI مربوط به ClawHub را روی پوشه بسته Plugin محلی
  اجرا می‌کند.
- به‌صورت پیش‌فرض اعتبارسنجی آفلاین/ایستا انجام می‌دهد، بدون یافتن یا واردکردن یک checkout محلی
  OpenClaw.
- خطاهای سازگاری سخت با کد غیرصفر خارج می‌شوند. یافته‌های فقط-هشدار چاپ می‌شوند اما
...
- پرچم‌ها:
  ...
  - `--no-mock-sdk`: OpenClaw SDK شبیه‌سازی‌شده را هنگام ضبط زمان اجرا غیرفعال کنید.
  - `--json`: خروجی قابل خواندن توسط ماشین.
...
اگر اعتبارسنجی یک یافته مربوط به بسته، manifest، import از SDK، یا artifact گزارش کرد، به
...
- `--version <version>` یک انتشار غیرآخرِ متعلق به مالک را از طریق یک مسیر fail-closed
  و مخصوص نسخه، به‌صورت دائمی حذف می‌کند.
  نسخه‌های حذف‌شده قابل بازیابی یا انتشار دوباره نیستند. پیش از حذف نسخه‌ای که در حال حاضر آخرین نسخه است، یک جایگزین منتشر کنید. این جریان فقط-نسخه به مالک بسته یا مدیر ناشر سازمانی نیاز دارد؛ کارکنان پلتفرم مالکیت بسته را دور نمی‌زنند.
- حذف نرم کل بسته به مالک بسته، مالک/مدیر ناشر سازمانی، ناظر پلتفرم،
  یا مدیر پلتفرم نیاز دارد.
- پرچم‌ها:
  - `--version <version>`: یک نسخه غیرآخر را به‌صورت دائمی حذف کنید.
...
- یک بسته و انتشارهای حذف‌شده نرم را بازیابی می‌کند.
- undelete نسخه وجود ندارد؛ نسخه‌های حذف‌شده دائمی قابل بازیابی نیستند.
...
- به دسترسی مدیر به هر دو مالک فعلی بسته و ناشر مقصد
...
- گزارش‌ها در سطح بسته هستند، می‌توانند به‌صورت اختیاری به یک نسخه متصل شوند، و برای
...
- مسدودکننده‌های وضعیت رسمی، دسترس‌پذیری ClawPack، digest مربوط به artifact،
  منشأ منبع، سازگاری OpenClaw، مقصدهای میزبان، فراداده محیط،
...
- اگر handle قبلاً توسط ناشر، کاربر، یا مسیر رزروشده موجودی استفاده شده باشد، شکست می‌خورد.
...
- پوشه‌های Plugin کد پیش از بارگذاری در یک tarball npm از نوع ClawPack بسته‌بندی می‌شوند تا
...
- برای پوشه‌های محلی، وقتی remote مبدأ git محلی به GitHub اشاره کند، انتساب منبع به‌صورت خودکار شناسایی می‌شود.
...
- `--owner <handle>` وقتی عامل دسترسی ناشر داشته باشد، انتشار را زیر شناسه ناشر کاربر یا سازمان انجام می‌دهد.
...
ابتدا از `--dry-run` استفاده کنید تا بتوانید پیش از ساختن یک انتشار زنده، فراداده package حل‌شده و
...
- `package.json.version` نسخه انتشار package شما است، اما به‌عنوان
...
- `openclaw.hostTargets` و `openclaw.environment` فراداده اختیاری هستند.
  ClawHub ممکن است هنگام وجود آن‌ها را نمایش دهد، اما برای انتشار الزامی نیستند.
...
- اگر اعتبارسنجی یک کد اصلاحی گزارش کرد، به
...
- workflow قابل استفاده مجدد، `source` را به‌طور پیش‌فرض روی مخزن فراخوان تنظیم می‌کند.
- برای monorepoها، `source_path` را پاس دهید تا workflow پوشه package
...
- این workflow نتیجه JSON را به‌عنوان یک artifact بارگذاری می‌کند و آن را به‌عنوان خروجی‌های workflow در معرض قرار می‌دهد.
...
- پیکربندی ناشر مورد اعتماد GitHub Actions را به یک package موجود متصل یا جایگزین می‌کند.
- package باید ابتدا از طریق `clawhub package publish` معمولی با احراز هویت دستی یا token ساخته شده باشد.
...
- ClawHub هنگام اجرای این دستور مخزن GitHub پیکربندی‌شده را تأیید می‌کند.
  ...
  مثال از طریق نصب آینده GitHub App مربوط به ClawHub یا یک یکپارچه‌سازی مجاز دیگر با GitHub.
- flagها:
...
- اگر workflow، مخزن یا pin مربوط به environment باید غیرفعال یا دوباره ساخته شود، از این به‌عنوان rollback استفاده کنید.
- انتشارهای واقعی آینده تا زمانی که پیکربندی دوباره تنظیم شود باید از انتشار احراز هویت‌شده معمولی استفاده کنند.
- flagها:
  - `--json`: خروجی قابل خواندن توسط ماشین.
...
- گزارش‌دهی بر اساس بهترین تلاش است. اگر تله‌متری در دسترس نباشد، دستورهای نصب
  fail نمی‌شوند.
