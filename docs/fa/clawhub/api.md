...
x-i18n:
    generated_at: "2026-06-30T14:11:59Z"
    model: gpt-5.5
...
- درخواست‌های احراز هویت‌شده (Bearer token معتبر): به‌ازای هر bucket کاربر.
...
- `GET /api/v1/download?slug=&version=&tag=`
  ...
  - Skills فعلی مبتنی بر GitHub با scan برابر `clean` یا `suspicious` به‌جای byteهای ClawHub، یک descriptor واگذاری JSON از نوع `public-github` برمی‌گردانند.
- `GET /api/v1/skills/export?startDate=&endDate=&limit=&cursor=`
  - Skills میزبانی‌شده به‌صورت فایل‌های ذخیره‌شده export می‌شوند.
  - Skills فعلی مبتنی بر GitHub با scan برابر `clean` یا `suspicious`
    به‌صورت descriptorهای واگذاری `public-github` export می‌شوند.
...
- `POST /api/v1/users/reserve` برای یک owner handle، slugهای ریشه و placeholderهای خصوصی package بدون release را reserve می‌کند.
