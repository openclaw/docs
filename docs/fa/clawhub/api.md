...
x-i18n:
    generated_at: "2026-07-02T17:42:03Z"
    model: gpt-5.5
...
می‌توانید روی APIهای خواندن عمومی ClawHub یک کاتالوگ، فهرست یا سطح جست‌وجوی شخص ثالث بسازید. فراداده‌های عمومی مهارت و فایل‌های مهارت تحت قواعد مجوز مهارت ClawHub منتشر می‌شوند، در حالی که خود API محدودیت نرخ دارد و باید مسئولانه مصرف شود.
...
- هنگام نمایش فهرست‌ها، به URL مهارت مرجع ClawHub پیوند دهید تا کاربران بتوانند رکورد رجیستری منبع را بررسی کنند.
...
- توکنِ ناموجود/نامعتبر به اعمال بر اساس IP برمی‌گردد.
...
- `X-RateLimit-Remaining` / `RateLimit-Remaining`: بودجهٔ باقی‌ماندهٔ دقیق هنگام
  حضور؛ درخواست‌های موفق شاردشده به‌جای بازگرداندن یک مقدار جهانی تقریبی،
  آن را حذف می‌کنند
...
- `GET /api/v1/download?slug=&version=&tag=`
  ...
  - مهارت‌های فعلی متکی به GitHub با اسکن `clean` یا `suspicious` به‌جای بایت‌های ClawHub،
    یک توصیفگر واگذاری JSON `public-github` برمی‌گردانند.
- `GET /api/v1/skills/export?startDate=&endDate=&limit=&cursor=`
  - مهارت‌های میزبانی‌شده به‌صورت فایل‌های ذخیره‌شده صادر می‌شوند.
  - مهارت‌های فعلی متکی به GitHub با اسکن `clean` یا `suspicious`
    به‌صورت توصیفگرهای واگذاری `public-github` صادر می‌شوند.
...
- `POST /api/v1/skills` (انتشار، multipart ترجیح دارد)
