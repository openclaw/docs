...
می‌توانید یک کاتالوگ، فهرست، یا سطح جست‌وجوی شخص ثالث را بر پایهٔ APIهای خواندن عمومی ClawHub بسازید. فرادادهٔ عمومی skill و فایل‌های skill تحت قوانین مجوز skill در ClawHub منتشر می‌شوند، در حالی که خود API دارای محدودیت نرخ است و باید مسئولانه مصرف شود.
...
- هنگام نمایش فهرست‌ها، به URL اصلی skill در ClawHub لینک دهید تا کاربران بتوانند رکورد رجیستری منبع را بررسی کنند.
- از URLهای صفحهٔ اصلی با قالب `https://clawhub.ai/<owner>/skills/<slug>` استفاده کنید.
...
- درخواست‌های احراز هویت‌شده (token معتبر Bearer): بر اساس bucket کاربر.
...
- `X-RateLimit-Reset`: ثانیه‌های epoch یونیکس (زمان reset مطلق)
...
- خطاهای v1 متن ساده هستند (`text/plain; charset=utf-8`)، از جمله `400`،
  `401`، `403`، `404`، `429`، و پاسخ‌های blocked-download.
...
- `GET /api/v1/download?slug=&version=&tag=`
  - skillهای میزبانی‌شده byteهای ZIP قطعی برمی‌گردانند.
  - skillهای فعلی پشتیبانی‌شده با GitHub که scan با وضعیت `clean` یا `suspicious` دارند، به‌جای byteهای ClawHub، یک توصیف‌گر handoff از نوع JSON `public-github` برمی‌گردانند.
- `GET /api/v1/skills/export?startDate=&endDate=&limit=&cursor=`
  - skillهای میزبانی‌شده به‌صورت فایل‌های ذخیره‌شده export می‌شوند.
  - skillهای فعلی پشتیبانی‌شده با GitHub که scan با وضعیت `clean` یا `suspicious` دارند، به‌صورت توصیف‌گرهای handoff از نوع `public-github` export می‌شوند.
...
مسیرهای legacy یعنی `/api/*` و `/api/cli/*` همچنان در دسترس هستند. `DEPRECATIONS.md` را ببینید.
