...
x-i18n:
    generated_at: "2026-07-01T15:27:56Z"
    model: gpt-5.5
...
می‌توانید یک کاتالوگ، فهرست، یا سطح جست‌وجوی شخص ثالث را بر پایهٔ APIهای خواندن عمومی ClawHub بسازید. فرادادهٔ عمومی Skills و فایل‌های Skills طبق قواعد مجوز Skills در ClawHub منتشر می‌شوند، درحالی‌که خود API دارای محدودیت نرخ است و باید مسئولانه مصرف شود.
...
- از URLهای صفحهٔ متعارف در قالب `https://clawhub.ai/<owner>/skills/<slug>` استفاده کنید.
- القا نکنید که ClawHub سایت شخص ثالث را تأیید، راستی‌آزمایی، یا اداره می‌کند.
- با دور زدن فیلترهای API عمومی یا مرزهای احراز هویت، محتوای پنهان، خصوصی، یا مسدودشده توسط ناظر را آینه نکنید.
...
- توکن ناموجود/نامعتبر به اعمال بر پایهٔ IP بازمی‌گردد.
...
- `X-RateLimit-Remaining` / `RateLimit-Remaining`: بودجهٔ باقی‌ماندهٔ دقیق، وقتی
  وجود داشته باشد؛ درخواست‌های موفق قطعه‌بندی‌شده به‌جای برگرداندن یک مقدار تقریبی
...
- پارامترهای کوئری شناخته‌شده با مقادیر نامعتبر، `400` برمی‌گردانند.
...
- `GET /api/v1/download?slug=&version=&tag=`
  ...
  - Skills فعلی مبتنی بر GitHub با اسکن `clean` یا `suspicious` به‌جای بایت‌های ClawHub،
    یک توصیف‌گر تحویل JSON از نوع `public-github` برمی‌گردانند.
...
- `POST /api/v1/users/reserve` اسلاگ‌های ریشه و جای‌نگهدارهای خصوصی بستهٔ بدون انتشار را برای یک شناسهٔ مالک رزرو می‌کند.
