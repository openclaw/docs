...
x-i18n:
    generated_at: "2026-07-02T08:34:43Z"
    model: gpt-5.5
...
می‌توانید یک کاتالوگ، فهرست‌راهنما، یا سطح جست‌وجوی شخص ثالث را بر پایهٔ APIهای خواندن عمومی ClawHub بسازید. فرادادهٔ عمومی مهارت‌ها و فایل‌های مهارت تحت قوانین مجوز مهارت ClawHub منتشر می‌شوند، در حالی که خود API دارای محدودیت نرخ است و باید مسئولانه مصرف شود.
...
- هنگام نمایش فهرست‌ها، به URL رسمی مهارت در ClawHub لینک دهید تا کاربران بتوانند رکورد رجیستری منبع را بررسی کنند.
- از URLهای صفحهٔ رسمی با شکل `https://clawhub.ai/<owner>/skills/<slug>` استفاده کنید.
...
- درخواست‌های احرازهویت‌شده (Bearer token معتبر): به‌ازای باکت کاربر.
- token مفقود/نامعتبر به اعمال محدودیت بر اساس IP برمی‌گردد.
...
- `RateLimit-Reset`: تأخیر بر حسب ثانیه تا reset
...
- `Retry-After`: تأخیر بر حسب ثانیه برای انتظار روی `429`
...
- `GET /api/v1/download?slug=&version=&tag=`
  ...
  - مهارت‌های فعلی مبتنی بر GitHub با اسکن `clean` یا `suspicious` به‌جای
    بایت‌های ClawHub، یک توصیف‌گر handoff از نوع JSON `public-github` برمی‌گردانند.
- `GET /api/v1/skills/export?startDate=&endDate=&limit=&cursor=`
  - مهارت‌های hosted به‌صورت فایل‌های ذخیره‌شده export می‌شوند.
  - مهارت‌های فعلی مبتنی بر GitHub با اسکن `clean` یا `suspicious`
    به‌صورت توصیف‌گرهای handoff نوع `public-github` export می‌شوند.
...
- `POST /api/v1/users/reserve`، slugهای ریشه و placeholderهای خصوصی package بدون release را برای handle مالک reserve می‌کند.
