...
summary: نمای کلی و قراردادهای API REST عمومی (v1).
x-i18n:
    generated_at: "2026-07-03T23:38:34Z"
    model: gpt-5.5
...
می‌توانید یک کاتالوگ، فهرست، یا سطح جست‌وجوی شخص ثالث را روی APIهای خواندنی عمومی ClawHub بسازید. فرادادهٔ عمومی مهارت‌ها و فایل‌های مهارت‌ها تحت قوانین مجوز مهارت ClawHub منتشر می‌شوند، در حالی که خود API دارای محدودیت نرخ است و باید مسئولانه مصرف شود.
...
- دانلود: 1200/دقیقه به‌ازای هر IP، 6000/دقیقه به‌ازای هر key
...
- `X-RateLimit-Remaining` / `RateLimit-Remaining`: بودجهٔ باقی‌ماندهٔ دقیق وقتی
  وجود داشته باشد؛ درخواست‌های موفق sharded به‌جای برگرداندن مقدار جهانی تقریبی،
  آن را حذف می‌کنند
...
- `GET /api/v1/download?slug=&version=&tag=`
  - مهارت‌های میزبانی‌شده byteهای ZIP قطعی برمی‌گردانند.
  - مهارت‌های فعلی مبتنی بر GitHub با scan مقدار `clean` یا `suspicious` به‌جای byteهای ClawHub،
    یک توصیف‌گر handoff از نوع JSON `public-github` برمی‌گردانند.
- `GET /api/v1/skills/export?startDate=&endDate=&limit=&cursor=`
  - مهارت‌های میزبانی‌شده به‌صورت فایل‌های ذخیره‌شده export می‌شوند.
  - مهارت‌های فعلی مبتنی بر GitHub با scan مقدار `clean` یا `suspicious`
...
- `POST /api/v1/users/reserve` اسلاگ‌های root و placeholderهای private no-release package را برای handle مالک رزرو می‌کند.
