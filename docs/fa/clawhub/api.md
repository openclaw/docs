...
می‌توانید یک کاتالوگ، فهرست، یا سطح جست‌وجوی شخص ثالث را روی APIهای خواندن عمومی ClawHub بسازید. فرادادهٔ عمومی Skills و فایل‌های Skills تحت قواعد مجوز Skills در ClawHub منتشر می‌شوند، در حالی که خود API محدودیت نرخ دارد و باید مسئولانه مصرف شود.
...
- `GET /api/v1/skills?limit=&cursor=&sort=`
  ...
  - `cursor` به مرتب‌سازی‌های غیر از `trending` اعمال می‌شود
...
- `GET /api/v1/download?slug=&version=&tag=`
  ...
  - Skills فعلیِ مبتنی بر GitHub با اسکن `clean` یا `suspicious` به‌جای بایت‌های ClawHub یک توصیف‌گر تحویل JSON از نوع `public-github` برمی‌گردانند.
- `GET /api/v1/skills/export?startDate=&endDate=&limit=&cursor=`
  - Skills میزبانی‌شده به‌صورت فایل‌های ذخیره‌شده صادر می‌شوند.
  - Skills فعلیِ مبتنی بر GitHub با اسکن `clean` یا `suspicious` به‌صورت توصیف‌گرهای تحویل `public-github` صادر می‌شوند.
