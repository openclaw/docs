...
می‌توانید یک کاتالوگ، دایرکتوری، یا سطح جست‌وجوی شخص ثالث را روی APIهای خواندن عمومی ClawHub بسازید. فرادادهٔ عمومی Skills و فایل‌های Skills تحت قواعد مجوز Skills در ClawHub منتشر می‌شوند، در حالی که خود API محدودیت نرخ دارد و باید مسئولانه مصرف شود.

راهنماها:

- برای فهرست‌های کاتالوگ از نقطه‌های پایانی خواندن عمومی مانند `GET /api/v1/skills`، `GET /api/v1/search`، و `GET /api/v1/skills/{slug}` استفاده کنید.
- پاسخ‌ها را کش کنید و به‌جای نظرسنجی تهاجمی، به `429`، `Retry-After`، و سرآیندهای محدودیت نرخ احترام بگذارید.
- هنگام نمایش فهرست‌ها، به URL رسمی Skills در ClawHub لینک دهید تا کاربران بتوانند رکورد رجیستری منبع را بررسی کنند.
...
- `GET /api/v1/skills?limit=&cursor=&sort=`
  ...
  - نام مستعار قدیمی: `nonSuspicious=true`
  - با `nonSuspiciousOnly=true`، صفحه‌های مبتنی بر نشانگر ممکن است کمتر از `limit` مورد داشته باشند؛ برای ادامه از `nextCursor` استفاده کنید.
...
- `POST /api/v1/users/reserve` slugهای ریشه و نگه‌دارنده‌های جای بستهٔ خصوصی بدون انتشار را برای یک شناسهٔ مالک رزرو می‌کند.
