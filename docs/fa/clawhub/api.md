...
summary: مرور کلی و قراردادهای REST API عمومی (v1).
x-i18n:
...
- `X-RateLimit-Remaining` / `RateLimit-Remaining`: بودجهٔ دقیق باقی‌مانده هنگام
  وجود داشتن؛ درخواست‌های موفق شاردشده به‌جای برگرداندن یک مقدار جهانی تقریبی،
  آن را حذف می‌کنند
...
- `GET /api/v1/download?slug=&version=&tag=`
  ...
  - Skills فعلی مبتنی بر GitHub با اسکن `clean` یا `suspicious` به‌جای بایت‌های ClawHub،
    یک توصیفگر واگذاری JSON با `public-github` برمی‌گردانند.
...
- `POST /api/v1/users/reserve` اسلاگ‌های ریشه و جای‌نگهدارهای خصوصی بستهٔ بدون انتشار را برای یک دستهٔ مالک رزرو می‌کند.
