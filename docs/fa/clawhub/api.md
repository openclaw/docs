...
summary: مرور کلی و قراردادهای API REST عمومی (v1).
x-i18n:
    generated_at: "2026-07-01T18:16:24Z"
    model: gpt-5.5
...
می‌توانید روی APIهای خواندن عمومی ClawHub، یک کاتالوگ، فهرست، یا سطح جست‌وجوی شخص ثالث بسازید. فرادادهٔ عمومی Skills و فایل‌های Skills تحت قواعد مجوز Skills در ClawHub منتشر می‌شوند، در حالی که خود API دارای محدودیت نرخ است و باید مسئولانه مصرف شود.
...
- پاسخ‌ها را کش کنید و به‌جای نظرسنجی تهاجمی، به سرآیندهای `429`، `Retry-After`، و محدودیت نرخ احترام بگذارید.
- هنگام نمایش فهرست‌ها، به URL رسمی Skills در ClawHub پیوند بدهید تا کاربران بتوانند رکورد رجیستری منبع را بررسی کنند.
...
- با دور زدن فیلترهای API عمومی یا مرزهای احراز هویت، محتوای پنهان، خصوصی، یا مسدودشده توسط تعدیل‌گری را آینه نکنید.
...
- توکن گمشده/نامعتبر به اعمال بر اساس IP برمی‌گردد.
...
`X-RateLimit-Remaining`، `RateLimit-Remaining`، و `Retry-After` روی `429` درج می‌شوند.
...
- `X-RateLimit-Remaining` / `RateLimit-Remaining`: بودجهٔ دقیق باقی‌مانده وقتی
  حاضر باشد؛ درخواست‌های موفق شاردشده، به‌جای برگرداندن مقدار جهانی تقریبی،
  آن را حذف می‌کنند
...
- پارامترهای کوئری ناشناخته برای سازگاری نادیده گرفته می‌شوند.
- پارامترهای کوئری شناخته‌شده با مقدارهای نامعتبر `400` برمی‌گردانند.
...
- `GET /api/v1/download?slug=&version=&tag=`
  ...
  - Skills فعلی با پشتوانهٔ GitHub که اسکن `clean` یا `suspicious` دارند، به‌جای بایت‌های ClawHub،
    یک توصیف‌گر واگذاری JSON با نوع `public-github` برمی‌گردانند.
- `GET /api/v1/skills/export?startDate=&endDate=&limit=&cursor=`
  - Skills میزبانی‌شده به‌صورت فایل‌های ذخیره‌شده صادر می‌شوند.
  - Skills فعلی با پشتوانهٔ GitHub که اسکن `clean` یا `suspicious` دارند،
    به‌صورت توصیف‌گرهای واگذاری `public-github` صادر می‌شوند.
...
- `POST /api/v1/skills` (انتشار، multipart ترجیحی است)
...
- `POST /api/v1/users/reserve` برای یک شناسهٔ مالک، اسلاگ‌های ریشه و جای‌نگهدارهای خصوصی بستهٔ بدون انتشار را رزرو می‌کند.
