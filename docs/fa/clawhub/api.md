...
x-i18n:
    generated_at: "2026-07-04T10:50:10Z"
    model: gpt-5.5
...
- هنگام نمایش فهرست‌ها، به URL متعارف Skills در ClawHub لینک دهید تا کاربران بتوانند رکورد منبع registry را بررسی کنند.
...
- `GET /api/v1/download?slug=&version=&tag=`
  ...
  - Skills فعلی مبتنی بر GitHub با scan مقدار `clean` یا `suspicious` به‌جای byteهای ClawHub، یک
    توصیفگر handoff از نوع JSON `public-github` برمی‌گردانند.
- `GET /api/v1/skills/export?startDate=&endDate=&limit=&cursor=`
  - Skills میزبانی‌شده به‌صورت فایل‌های ذخیره‌شده export می‌شوند.
  - Skills فعلی مبتنی بر GitHub با scan مقدار `clean` یا `suspicious` به‌صورت
    توصیفگرهای handoff از نوع `public-github` export می‌شوند.
...
- `POST /api/v1/users/reserve` slugهای root و placeholderهای بستهٔ خصوصی بدون release را برای handle مالک reserve می‌کند.
