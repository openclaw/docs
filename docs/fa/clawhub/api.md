...
x-i18n:
    generated_at: "2026-07-03T01:00:40Z"
    model: gpt-5.5
...
می‌توانید روی APIهای خواندن عمومی ClawHub یک کاتالوگ، فهرست، یا سطح جست‌وجوی شخص ثالث بسازید. فراداده‌های عمومی Skills و فایل‌های Skills طبق قواعد مجوز Skills در ClawHub منتشر می‌شوند، درحالی‌که خود API محدودیت نرخ دارد و باید مسئولانه مصرف شود.

راهنماها:

- برای فهرست‌های کاتالوگ از نقطه‌پایان‌های خواندن عمومی مانند `GET /api/v1/skills`، `GET /api/v1/search`، و `GET /api/v1/skills/{slug}` استفاده کنید.
- پاسخ‌ها را کش کنید و به‌جای نظرسنجی تهاجمی، سرآیندهای `429`، `Retry-After`، و محدودیت نرخ را رعایت کنید.
- هنگام نمایش فهرست‌ها به URL رسمی Skill در ClawHub لینک بدهید تا کاربران بتوانند رکورد رجیستری منبع را بررسی کنند.
- از URLهای رسمی صفحه با قالب `https://clawhub.ai/<owner>/skills/<slug>` استفاده کنید.
...
اجرای آگاه از احراز هویت:

- درخواست‌های ناشناس: به‌ازای هر IP.
- درخواست‌های احراز هویت‌شده (توکن Bearer معتبر): به‌ازای باکت هر کاربر.
- توکن ناموجود/نامعتبر به اجرای مبتنی بر IP برمی‌گردد.
...
- `X-RateLimit-Remaining` / `RateLimit-Remaining`: بودجهٔ دقیق باقی‌مانده وقتی
  وجود داشته باشد؛ درخواست‌های موفق شاردشده به‌جای برگرداندن یک مقدار جهانی
...
- در غیر این صورت از `RateLimit-Reset` استفاده کنید یا تأخیر را از `X-RateLimit-Reset` مشتق کنید.
...
- `GET /api/v1/skills?limit=&cursor=&sort=`
  - `sort`: `updated` (پیش‌فرض)، `recommended` (`default`)، `createdAt` (`newest`)، `downloads`، `stars` (`rating`)، نام‌های مستعار قدیمی نصب `installsCurrent`/`installs`/`installsAllTime` به `downloads` نگاشت می‌شوند، `trending`
...
- `GET /api/v1/download?slug=&version=&tag=`
  ...
  - Skills فعلی مبتنی بر GitHub با اسکن `clean` یا `suspicious` به‌جای بایت‌های ClawHub، یک توصیف‌گر واگذاری JSON با نام `public-github` برمی‌گردانند.
- `GET /api/v1/skills/export?startDate=&endDate=&limit=&cursor=`
  - Skills میزبانی‌شده به‌صورت فایل‌های ذخیره‌شده صادر می‌شوند.
  - Skills فعلی مبتنی بر GitHub با اسکن `clean` یا `suspicious` به‌صورت توصیف‌گرهای واگذاری `public-github` صادر می‌شوند.
...
- `POST /api/v1/users/reserve` اسلاگ‌های ریشه و جای‌نگهدارهای خصوصی بسته‌های بدون انتشار را برای یک هندل مالک رزرو می‌کند.

## قدیمی

مسیرهای قدیمی `/api/*` و `/api/cli/*` همچنان در دسترس‌اند. `DEPRECATIONS.md` را ببینید.
