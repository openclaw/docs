...
x-i18n:
    generated_at: "2026-07-02T01:04:30Z"
    model: gpt-5.5
...
می‌توانید یک کاتالوگ، دایرکتوری، یا سطح جست‌وجوی شخص ثالث را روی APIهای خواندنی عمومی ClawHub بسازید. فرادادهٔ عمومی مهارت‌ها و فایل‌های مهارت‌ها تحت قواعد مجوز مهارت ClawHub منتشر می‌شوند، درحالی‌که خود API دارای محدودیت نرخ است و باید مسئولانه مصرف شود.
...
- هنگام نمایش فهرست‌ها، به URL مهارت متعارف ClawHub پیوند دهید تا کاربران بتوانند رکورد رجیستری منبع را بررسی کنند.
...
- درخواست‌های احرازشده (توکن Bearer معتبر): به‌ازای هر سطل کاربر.
...
- `X-RateLimit-Remaining` / `RateLimit-Remaining`: بودجهٔ دقیق باقی‌مانده وقتی
  وجود داشته باشد؛ درخواست‌های موفق شاردشده به‌جای بازگرداندن مقدار تقریبی
...
- پارامترهای ناشناختهٔ پرس‌وجو برای سازگاری نادیده گرفته می‌شوند.
- پارامترهای شناخته‌شدهٔ پرس‌وجو با مقدارهای نامعتبر، `400` بازمی‌گردانند.
...
- `GET /api/v1/skills?limit=&cursor=&sort=`
  ...
  - مقدارهای نامعتبر `sort`، `400` بازمی‌گردانند
...
- `GET /api/v1/download?slug=&version=&tag=`
  ...
  - Skills فعلی با پشتوانهٔ GitHub که اسکن `clean` یا `suspicious` دارند، به‌جای بایت‌های ClawHub یک
    توصیف‌گر تحویل JSON از نوع `public-github` بازمی‌گردانند.
- `GET /api/v1/skills/export?startDate=&endDate=&limit=&cursor=`
  - Skills میزبانی‌شده به‌صورت فایل‌های ذخیره‌شده صادر می‌شوند.
  - Skills فعلی با پشتوانهٔ GitHub که اسکن `clean` یا `suspicious` دارند، به‌صورت
    توصیف‌گرهای تحویل `public-github` صادر می‌شوند.
