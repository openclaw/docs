---
read_when:
    - دستورات ClawHub CLI یا رجیستری OpenClaw با خطا مواجه می‌شوند
...
CLI هنگام ورود از طریق مرورگر، یک سرور بازخوانی محلی کوتاه‌عمر راه‌اندازی می‌کند.

- مطمئن شوید مرورگر شما می‌تواند به `http://127.0.0.1:<port>/callback` دسترسی پیدا کند.
- اگر بازخوانی هرگز نمی‌رسد، قوانین فایروال محلی، VPN و پراکسی را بررسی کنید.
...
- تأیید کنید انتشار عمومی است و توسط اسکن یا تعدیل نگه داشته نشده است.
...
عیب‌یابی‌های قابل مشاهده برای مالک ممکن است وضعیت اسکن، دروازه بارگذاری، یا تعدیل را توضیح دهند.

## انتشار به دلیل نبود فراداده لازم شکست می‌خورد
...
ابتدا بار انتشار را پیش‌نمایش کنید:
...
- بررسی کنید URL منبع عمومی یا برای ClawHub قابل دسترسی باشد.
- برای منابع GitHub، از `owner/repo`، `owner/repo@ref`، یا یک URL کامل GitHub استفاده کنید.

## انتشار به دلیل ادعاشدن یا رزروبودن یک فضای نام شکست می‌خورد

اگر انتشار به این دلیل شکست می‌خورد که handle مالک، فضای نام سازمان، scope بسته، slug مربوط به Skill
یا نام بسته از قبل ادعا یا رزرو شده است، ابتدا تأیید کنید که با مالکی منتشر می‌کنید
که با فضای نام منطبق است. برای بسته‌های Plugin،
نام‌های scoped مانند `@example-org/example-plugin` باید با مالک
منطبق `example-org` منتشر شوند.

اگر باور دارید سازمان، پروژه، یا برند شما مالک قانونی فضای نام است اما
...
با اثبات عمومی و غیرحساس باز کنید. برای راهنمایی شواهد و اینکه چه چیزهایی
...
- محتوای پنهان، خصوصی، نگه‌داشته‌شده، یا مسدودشده توسط تعدیل را خارج از
