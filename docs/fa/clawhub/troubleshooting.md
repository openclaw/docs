---
read_when:
    - دستورهای CLI ClawHub یا رجیستری OpenClaw ناموفق می‌شوند
    - یک بسته نمی‌تواند نصب، منتشر یا به‌روزرسانی شود
summary: عیب‌یابی مشکلات ورود به ClawHub، نصب، انتشار، به‌روزرسانی و رابط برنامه‌نویسی کاربردی.
x-i18n:
    generated_at: "2026-07-01T18:17:34Z"
    model: gpt-5.5
...
- `RateLimit-Remaining`: سهمیه دقیق باقی‌مانده شما وقتی این header وجود دارد. در `429`، مقدار آن `0` است.
- `RateLimit-Reset` یا `X-RateLimit-Reset`: زمان reset.

اگر کاربران زیادی یک IP خروجی مشترک داشته باشند، حتی وقتی هر فرد فقط چند
درخواست می‌فرستد، ممکن است به محدودیت‌های IP ناشناس برسید. هرجا ممکن است وارد شوید و پس از
...
اگر انتشار به این دلیل ناموفق می‌شود که handle مالک، namespace سازمان، scope package، slug مربوط به Skill
یا نام package از قبل claim یا reserved شده است، ابتدا تأیید کنید که با owner متناظر با namespace
...
با مدارک عمومی و غیرحساس باز کنید. برای راهنمایی درباره مدارک و اینکه چه چیزهایی
...
- اگر package پنهان، نگه داشته‌شده یا مسدود شده باشد، ممکن است تا وقتی
