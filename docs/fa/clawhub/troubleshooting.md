...
x-i18n:
    generated_at: "2026-07-03T02:53:01Z"
    model: gpt-5.5
...
نفر فقط چند درخواست می‌فرستد. هر جا ممکن است وارد شوید و پس از تأخیر
...
- تأیید کنید که release عمومی است و به‌دلیل اسکن یا moderation نگه داشته نشده است.
...
برای افزونه‌ها، metadata سازگاری در `package.json` را بررسی کنید. انتشارهای code-plugin
...
## انتشار به‌دلیل claim یا رزرو بودن namespace شکست می‌خورد

اگر انتشار به‌دلیل اینکه handle مالک، namespace سازمان، scope بسته، slug مهارت
یا نام بسته از قبل claim یا رزرو شده است شکست می‌خورد، ابتدا تأیید کنید که با
مالکی منتشر می‌کنید که با namespace مطابقت دارد. برای بسته‌های افزونه،
...
نباید در issueهای عمومی قرار بگیرد، [ادعاهای سازمان و Namespace](/clawhub/namespace-claims) را ببینید.
...
## نصب یک افزونه در OpenClaw شکست می‌خورد
...
- تأیید کنید نسخه OpenClaw شما با بازه سازگاری اعلام‌شده بسته مطابقت دارد.
- اگر بسته hidden، held یا blocked باشد، ممکن است تا وقتی مالک مشکل را حل کند
  قابل نصب نباشد.

## درخواست‌های API عمومی شکست می‌خورند

- سربرگ‌های تلاش دوباره `429` را رعایت کنید و پاسخ‌های فهرست/جست‌وجوی عمومی را cache کنید.
- کاربران را به فهرست canonical ClawHub بازگردانید.
- محتوای hidden، private، held یا moderation-blocked را بیرون از سطح
