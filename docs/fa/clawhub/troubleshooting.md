...
summary: عیب‌یابی مشکلات ورود به سیستم، نصب، انتشار، به‌روزرسانی و API در ClawHub.
x-i18n:
    generated_at: "2026-06-28T08:06:05Z"
    model: gpt-5.5
...
اگر کاربران زیادی از یک IP خروجی مشترک استفاده کنند، ممکن است محدودیت‌های IP ناشناس فعال شود، حتی وقتی هر
...
CLI به متغیرهای استاندارد پروکسی احترام می‌گذارد:
...
- تأیید کنید انتشار عمومی است و به‌دلیل اسکن یا moderation نگه داشته نشده است.
...
عیب‌یابی‌های قابل مشاهده برای مالک ممکن است وضعیت اسکن، دروازه بارگذاری یا moderation را توضیح دهند.

## انتشار به‌دلیل نبود فراداده لازم ناموفق است

برای مهارت‌ها، فرانت‌متر `SKILL.md` را بررسی کنید. متغیرهای محیطی و
ابزارهای لازم باید اعلام شوند تا کاربران و اسکنرها بتوانند بسته را درک کنند.

برای Pluginها، فراداده سازگاری `package.json` را بررسی کنید. انتشارهای
...
## انتشار به‌دلیل claim یا رزرو بودن namespace ناموفق است

اگر انتشار به این دلیل ناموفق می‌شود که شناسه مالک، namespace سازمان، scope بسته، slug مهارت
یا نام بسته از قبل claim یا رزرو شده است، ابتدا تأیید کنید که با
مالکی منتشر می‌کنید که با namespace مطابقت دارد. برای بسته‌های Plugin،
...
با مدارک عمومی و غیرحساس باز کنید. برای راهنمای شواهد و اینکه چه چیزهایی را
نباید در مسائل عمومی قرار دهید، [Claimهای سازمان و Namespace](/fa/clawhub/namespace-claims) را ببینید.
...
- به headerهای تلاش مجدد `429` احترام بگذارید و پاسخ‌های فهرست/جست‌وجوی عمومی را cache کنید.
