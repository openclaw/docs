...
x-i18n:
    generated_at: "2026-07-04T20:39:06Z"
    model: gpt-5.5
...
- `Retry-After`: تعداد ثانیه‌هایی که باید قبل از تلاش مجدد صبر کنید.
- `RateLimit-Limit`: محدودیتی که روی این درخواست اعمال شده است.
- `RateLimit-Remaining`: بودجه دقیق باقی‌مانده شما وقتی این header وجود دارد. روی `429`، مقدار آن `0` است.
...
## جست‌وجو یا نصب پشت پراکسی ناموفق می‌شود
...
## انتشار به‌دلیل نبود فراداده الزامی ناموفق می‌شود
...
## انتشار به‌دلیل ادعا یا رزرو شدن یک namespace ناموفق می‌شود

اگر انتشار به این دلیل ناموفق شد که handle مالک، namespace سازمان، scope بسته، slug مهارت
...
نباید در issueهای عمومی قرار بگیرند، [ادعاهای سازمان و Namespace](/clawhub/namespace-claims) را ببینید.
...
- اگر بسته hidden، held یا blocked باشد، ممکن است تا وقتی مالک مشکل را حل نکند
  قابل نصب نباشد.
