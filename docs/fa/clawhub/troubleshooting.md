...
x-i18n:
    generated_at: "2026-06-30T22:26:31Z"
    model: gpt-5.5
...
اگر کاربران زیادی یک IP خروجی مشترک داشته باشند، حتی وقتی هر شخص فقط چند
درخواست می‌فرستد، ممکن است محدودیت‌های IP ناشناس فعال شوند. هرجا ممکن است
وارد شوید و پس از تأخیر گزارش‌شده دوباره تلاش کنید.
...
## انتشار به‌دلیل تصاحب یا رزرو بودن namespace ناموفق است

اگر انتشار به این دلیل ناموفق است که handle مالک، namespace سازمان، scope بسته، slug مهارت
یا نام بسته قبلاً تصاحب یا رزرو شده است، ابتدا تأیید کنید که با مالکی منتشر
می‌کنید که با namespace مطابقت دارد. برای بسته‌های plugin، نام‌های scoped مانند
`@example-org/example-plugin` باید به‌عنوان مالک متناظر `example-org` منتشر شوند.
...
را نباید در مسائل عمومی قرار دهید، [ادعاهای سازمان و Namespace](/clawhub/namespace-claims) را ببینید.
...
- اگر بسته پنهان، نگه‌داشته‌شده یا مسدود باشد، ممکن است تا زمانی که مالک مشکل را
  حل نکند قابل نصب نباشد.

## درخواست‌های API عمومی ناموفق هستند

- سربرگ‌های تلاش مجدد `429` را رعایت کنید و پاسخ‌های فهرست/جست‌وجوی عمومی را cache کنید.
- کاربران را به فهرست canonical ClawHub برگردانید.
- محتوای پنهان، خصوصی، نگه‌داشته‌شده یا مسدودشده توسط بازبینی را بیرون از سطح
