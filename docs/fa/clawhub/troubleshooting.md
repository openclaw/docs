...
x-i18n:
    generated_at: "2026-06-28T07:42:38Z"
    model: gpt-5.5
...
CLI هنگام ورود با مرورگر، یک سرور بازفراخوان محلی کوتاه‌عمر راه‌اندازی می‌کند.

- مطمئن شوید مرورگر شما می‌تواند به `http://127.0.0.1:<port>/callback` دسترسی پیدا کند.
- اگر بازفراخوان هرگز نمی‌رسد، قوانین فایروال محلی، VPN و پروکسی را بررسی کنید.
...
- `RateLimit-Remaining`: بودجه دقیق باقی‌مانده شما وقتی سرآیند وجود دارد. در `429`، مقدار آن `0` است.
- `RateLimit-Reset` یا `X-RateLimit-Reset`: زمان‌بندی بازنشانی.

اگر کاربران زیادی یک IP خروجی را به اشتراک بگذارند، حتی وقتی هر نفر فقط چند
درخواست می‌فرستد، ممکن است به سقف‌های IP ناشناس برسید. در صورت امکان وارد شوید و پس از
...
- تأیید کنید انتشار عمومی است و به‌دلیل اسکن یا تعدیل متوقف نشده است.
...
ابزارهای الزامی باید اعلام شوند تا کاربران و اسکنرها بتوانند بسته را بفهمند.

برای Plugin‌ها، فراداده سازگاری `package.json` را بررسی کنید. انتشارهای code-plugin
...
## انتشار شکست می‌خورد چون یک فضای نام ادعا یا رزرو شده است

اگر انتشار به این دلیل شکست می‌خورد که handle مالک، فضای نام سازمان، محدوده بسته، slug مهارت
یا نام بسته از قبل ادعا یا رزرو شده است، ابتدا تأیید کنید که با مالکی منتشر می‌کنید
که با فضای نام مطابقت دارد. برای بسته‌های Plugin،
نام‌های دارای scope مانند `@example-org/example-plugin` باید با مالک
...
نباید در مسائل عمومی قرار بگیرد، [ادعاهای سازمان و فضای نام](/fa/clawhub/namespace-claims) را ببینید.
...
- سرآیندهای تلاش دوباره `429` را رعایت کنید و پاسخ‌های فهرست/جست‌وجوی عمومی را کش کنید.
- کاربران را به فهرست canonical ClawHub برگردانید.
- محتوای پنهان، خصوصی، متوقف‌شده یا مسدودشده به‌دلیل تعدیل را بیرون از
