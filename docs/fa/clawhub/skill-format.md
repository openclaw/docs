...
فراداده مهارت در YAML frontmatter بالای `SKILL.md` شما اعلام می‌شود. این به registry (و تحلیل امنیتی) می‌گوید مهارت شما برای اجرا به چه چیزهایی نیاز دارد.
...
از `requires.env` برای متغیرهای محیطی استفاده کنید که باید پیش از اجرای مهارت وجود داشته باشند. زمانی از `envVars` استفاده کنید که برای هر متغیر به فراداده نیاز دارید، از جمله متغیرهای اختیاری با `required: false`.
...
- باید lowercase و برای URL امن باشند: `^[a-z0-9][a-z0-9-]*$`.
...
- اگر مهارت شما با یک سرویس شخص ثالث پولی یکپارچه می‌شود، هزینه خارجی و حساب الزامی را به‌روشنی در دستورالعمل‌های مهارت و اعلام‌های env مستند کنید (`requires.env` برای متغیرهای الزامی، یا `envVars` با `required: false` برای متغیرهای اختیاری).
