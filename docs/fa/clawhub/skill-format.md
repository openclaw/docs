...
summary: قالب پوشهٔ مهارت، فایل‌های الزامی، انواع فایل مجاز، محدودیت‌ها.
x-i18n:
    generated_at: "2026-07-02T22:40:42Z"
    model: gpt-5.5
...
واردکنندهٔ وب GitHub از انتشار/همگام‌سازی محلی سخت‌گیرتر است. فقط فایل‌های
...
- `description` به‌عنوان خلاصهٔ skill در رابط کاربری/جست‌وجو استفاده می‌شود.

## فرادادهٔ frontmatter

فرادادهٔ Skill در frontmatter نوع YAML در بالای `SKILL.md` شما تعریف می‌شود. این به رجیستری (و تحلیل امنیتی) می‌گوید skill شما برای اجرا به چه چیزهایی نیاز دارد.
...
برای متغیرهای محیطی که پیش از اجرای skill باید وجود داشته باشند، از `requires.env` استفاده کنید. زمانی که برای هر متغیر به فراداده نیاز دارید، از جمله متغیرهای اختیاری با `required: false`، از `envVars` استفاده کنید.
...
| `primaryEnv`       | `string`   | متغیر محیطی اعتبارنامهٔ اصلی برای skill شما.                                                                                                  |
...
| `homepage`         | `string`   | URL صفحهٔ اصلی یا مستندات skill.                                                                                                         |
...
اگر skill شما نیاز دارد وابستگی‌هایی نصب شوند، آن‌ها را در آرایهٔ `install` اعلام کنید:
...
- نوع‌های محتوا که با `text/` شروع می‌شوند متن تلقی می‌شوند؛ به‌علاوهٔ یک فهرست مجاز کوچک (JSON/YAML/TOML/JS/TS/Markdown/SVG).

محدودیت‌ها (سمت سرور):

- اندازهٔ کل بسته: 50MB.
- متن embedding شامل `SKILL.md` + تا حدود ۴۰ فایل غیر `.md` است (سقف بر پایهٔ بهترین تلاش).
...
- همهٔ skillهای منتشرشده در ClawHub تحت مجوز `MIT-0` هستند.
- هر کسی می‌تواند skillهای منتشرشده را استفاده، اصلاح، و بازتوزیع کند، از جمله به‌صورت تجاری.
- انتساب لازم نیست.
- شرایط مجوز ناسازگار را در `SKILL.md` اضافه نکنید؛ ClawHub از بازنویسی مجوز برای هر skill پشتیبانی نمی‌کند.

## Skillهای پولی

- ClawHub از skillهای پولی، قیمت‌گذاری به‌ازای هر skill، paywallها، یا اشتراک درآمد پشتیبانی نمی‌کند.
- فرادادهٔ قیمت‌گذاری را به `SKILL.md` اضافه نکنید؛ این بخشی از قالب skill نیست و یک skill منتشرشده را پولی نمی‌کند.
- اگر skill شما با یک سرویس شخص ثالث پولی یکپارچه می‌شود، هزینهٔ خارجی و حساب لازم را به‌روشنی در دستورالعمل‌های skill و اعلان‌های env مستند کنید (`requires.env` برای متغیرهای ضروری، یا `envVars` با `required: false` برای متغیرهای اختیاری).
