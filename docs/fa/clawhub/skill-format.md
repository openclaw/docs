...
read_when:
    - انتشار Skills
    - اشکال‌زدایی خرابی‌های انتشار
summary: قالب پوشه مهارت، فایل‌های الزامی، انواع فایل مجاز، محدودیت‌ها.
x-i18n:
    generated_at: "2026-07-04T15:26:49Z"
    model: gpt-5.5
...
- `.gitignore` (آن نیز رعایت می‌شود)

## وارد کردن از GitHub

واردکننده وب GitHub سخت‌گیرانه‌تر از انتشار/همگام‌سازی محلی است. این ابزار فقط فایل‌های
`SKILL.md` یا `skills.md` قدیمی را در مخزن‌های عمومی، غیر فورک، و متعلق به
حساب GitHub واردشده کشف می‌کند. مخزن‌های خصوصی، فورک‌ها،
...
وضعیت نصب در workdir (نوشته‌شده توسط CLI):
...
فراداده مهارت در فرانت‌متر YAML بالای `SKILL.md` شما اعلام می‌شود. این مورد به رجیستری (و تحلیل امنیتی) می‌گوید مهارت شما برای اجرا به چه چیزهایی نیاز دارد.
...
برای متغیرهای محیطی‌ای که باید پیش از اجرای مهارت وجود داشته باشند از `requires.env` استفاده کنید. وقتی به فراداده جداگانه برای هر متغیر نیاز دارید، از جمله متغیرهای اختیاری با `required: false`، از `envVars` استفاده کنید.
...
- scopeهای بسته باید دقیقاً با شناسه ناشر ClawHub مطابقت داشته باشند. شناسه‌های ناشر می‌توانند از حروف کوچک، اعداد، خط تیره، نقطه و زیرخط استفاده کنند؛ باید با یک حرف کوچک یا عدد شروع و پایان یابند.
