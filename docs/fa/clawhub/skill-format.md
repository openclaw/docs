...
x-i18n:
    generated_at: "2026-07-05T05:22:25Z"
    model: gpt-5.5
...
تحلیل امنیتی ClawHub بررسی می‌کند که آنچه مهارت شما اعلام می‌کند با کاری که واقعاً انجام می‌دهد منطبق باشد. اگر کد شما به `TODOIST_API_KEY` ارجاع دهد اما frontmatter شما آن را زیر `requires.env`، `primaryEnv`، یا `envVars` اعلام نکرده باشد، تحلیل، ناسازگاری فراداده را علامت‌گذاری می‌کند. دقیق نگه داشتن اعلان‌ها به مهارت شما کمک می‌کند بازبینی را بگذراند و به کاربران کمک می‌کند بفهمند چه چیزی را نصب می‌کنند.
...
- scopes بسته باید دقیقاً با handle ناشر ClawHub مطابقت داشته باشند. handleهای ناشر می‌توانند از حروف کوچک، اعداد، خط تیره، نقطه و زیرخط استفاده کنند؛ باید با یک حرف کوچک یا عدد شروع و تمام شوند.
- اسلاگ‌های بسته باید با حروف کوچک و سازگار با npm باشند، برای مثال `@example.tools/demo-plugin` یا `demo-plugin`.
...
- اگر مهارت شما با یک سرویس شخص ثالث پولی یکپارچه می‌شود، هزینه خارجی و حساب موردنیاز را در دستورالعمل‌های مهارت و اعلان‌های env به‌روشنی مستند کنید (`requires.env` برای متغیرهای ضروری، یا `envVars` با `required: false` برای متغیرهای اختیاری).
