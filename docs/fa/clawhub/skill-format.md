...
summary: قالب پوشه Skill، فایل‌های الزامی، نوع‌های مجاز فایل، محدودیت‌ها.
x-i18n:
    generated_at: "2026-07-04T03:57:18Z"
    model: gpt-5.5
...
فراداده Skill در frontmatter YAML در بالای `SKILL.md` شما اعلام می‌شود. این به رجیستری (و تحلیل امنیتی) می‌گوید Skill شما برای اجرا به چه چیزهایی نیاز دارد.
...
برای متغیرهای محیطی که باید پیش از اجرای Skill وجود داشته باشند، از `requires.env` استفاده کنید. وقتی برای هر متغیر به فراداده نیاز دارید، از جمله متغیرهای اختیاری با `required: false`، از `envVars` استفاده کنید.
...
| `install`          | `array`    | مشخصات نصب برای وابستگی‌ها (در ادامه ببینید).                                                                                                  |
...
- اندازه کل بسته: 50MB.
- متن جاسازی‌شده شامل `SKILL.md` + تا حدود ۴۰ فایل غیر `.md` است (سقف به‌شکل best-effort).

## اسلاگ‌ها

- به‌صورت پیش‌فرض از نام پوشه مشتق می‌شوند.
- محدوده‌های بسته باید دقیقاً با شناسه ناشر ClawHub مطابقت داشته باشند. شناسه‌های ناشر می‌توانند از حروف کوچک، اعداد، خط تیره، نقطه، و زیرخط استفاده کنند؛ باید با یک حرف کوچک یا عدد شروع و تمام شوند.
- اسلاگ‌های بسته باید حروف کوچک و برای npm امن باشند، برای مثال `@example.tools/demo-plugin` یا `demo-plugin`.
...
- همه Skills منتشرشده روی ClawHub تحت مجوز `MIT-0` هستند.
...
- اگر Skill شما با یک سرویس شخص ثالث پولی یکپارچه می‌شود، هزینه خارجی و حساب موردنیاز را به‌وضوح در دستورالعمل‌های Skill و اعلان‌های env مستند کنید (`requires.env` برای متغیرهای الزامی، یا `envVars` با `required: false` برای متغیرهای اختیاری).
