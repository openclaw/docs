...
فراداده مهارت در فرانت‌متر YAML بالای `SKILL.md` شما تعریف می‌شود. این به رجیستری (و تحلیل امنیتی) می‌گوید مهارت شما برای اجرا به چه چیزهایی نیاز دارد.
...
از `requires.env` برای متغیرهای محیطی استفاده کنید که پیش از اجرای مهارت باید وجود داشته باشند. وقتی برای هر متغیر به فراداده جداگانه نیاز دارید، از جمله متغیرهای اختیاری با `required: false`، از `envVars` استفاده کنید.
...
- اندازه کل بسته: 50MB.
- متن تعبیه‌سازی شامل `SKILL.md` + تا حدود ۴۰ فایل غیر `.md` است (سقف با بهترین تلاش).

## اسلاگ‌ها

- به‌طور پیش‌فرض از نام پوشه مشتق می‌شوند.
- اسکوپ‌های بسته باید دقیقاً با شناسه ناشر ClawHub مطابقت داشته باشند. شناسه‌های ناشر می‌توانند از حروف کوچک، عددها، خط تیره، نقطه و زیرخط استفاده کنند؛ باید با یک حرف کوچک یا عدد شروع و تمام شوند.
...
- هر کسی می‌تواند از مهارت‌های منتشرشده استفاده کند، آن‌ها را تغییر دهد و دوباره توزیع کند، از جمله به‌صورت تجاری.
- انتساب لازم نیست.
...
- فراداده قیمت‌گذاری را به `SKILL.md` اضافه نکنید؛ بخشی از قالب مهارت نیست و باعث پولی شدن مهارت منتشرشده نمی‌شود.
