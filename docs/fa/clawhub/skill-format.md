...
x-i18n:
    generated_at: "2026-06-30T22:24:09Z"
    model: gpt-5.5
...
واردکننده وب GitHub از انتشار/همگام‌سازی محلی سخت‌گیرتر است. این واردکننده فقط فایل‌های
`SKILL.md` یا `skills.md` قدیمی را در مخازن عمومی و غیر فورک‌شده‌ای پیدا می‌کند که متعلق به
حساب GitHub واردشده هستند. مخازن خصوصی، فورک‌ها،
...
تحلیل امنیتی ClawHub بررسی می‌کند که آنچه مهارت شما اعلام می‌کند با کاری که واقعاً انجام می‌دهد سازگار باشد. اگر کد شما به `TODOIST_API_KEY` ارجاع دهد اما فرانت‌متر شما آن را زیر `requires.env`، `primaryEnv`، یا `envVars` اعلام نکرده باشد، تحلیل یک ناسازگاری فراداده را علامت‌گذاری می‌کند. دقیق نگه داشتن اعلان‌ها به مهارت شما کمک می‌کند بازبینی را بگذراند و به کاربران کمک می‌کند بفهمند چه چیزی را نصب می‌کنند.
