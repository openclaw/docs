...
x-i18n:
    generated_at: "2026-07-05T07:27:01Z"
    model: gpt-5.5
...
`SKILL.md` یا `skills.md` قدیمی را در مخازن عمومی و غیر فورک که متعلق به حساب GitHub
واردشده هستند کشف می‌کند. مخازن خصوصی، فورک‌ها، مخازن
آرشیوشده/غیرفعال، یا مخازن عمومی شخص ثالث را وارد نمی‌کند.
...
فراداده مهارت در frontmatter YAML در ابتدای `SKILL.md` شما تعریف می‌شود. این به رجیستری (و تحلیل امنیتی) می‌گوید مهارت شما برای اجرا به چه چیزهایی نیاز دارد.
...
متغیرهای محیطی اختیاری را زیر `metadata.openclaw.envVars` اعلام کنید و `required: false` را تنظیم کنید. ورودی‌های اختیاری را به `requires.env` اضافه نکنید، چون `requires.env` یعنی مهارت بدون آن‌ها اجرا نمی‌شود.
...
تحلیل امنیتی ClawHub بررسی می‌کند که آنچه مهارت شما اعلام می‌کند با کاری که واقعاً انجام می‌دهد هم‌خوان باشد. اگر کد شما به `TODOIST_API_KEY` ارجاع دهد اما frontmatter شما آن را زیر `requires.env`، `primaryEnv`، یا `envVars` اعلام نکند، تحلیل، ناسازگاری فراداده را گزارش می‌کند. دقیق نگه‌داشتن اعلان‌ها به مهارت شما کمک می‌کند بازبینی را پشت سر بگذارد و به کاربران کمک می‌کند بفهمند چه چیزی نصب می‌کنند.
...
- انواع محتوایی که با `text/` شروع می‌شوند متن در نظر گرفته می‌شوند؛ به‌علاوه یک فهرست مجاز کوچک (JSON/YAML/TOML/JS/TS/Markdown/SVG).

محدودیت‌ها (سمت سرور):

- اندازه کل بسته: 50MB.
- متن جاسازی شامل `SKILL.md` + حداکثر حدود ۴۰ فایل غیر `.md` است (سقف بر مبنای بهترین تلاش).
...
- نامک‌های بسته باید با حروف کوچک و برای npm امن باشند، برای مثال `@example.tools/demo-plugin` یا `demo-plugin`.
...
- ClawHub از مهارت‌های پولی، قیمت‌گذاری برای هر مهارت، دیوار پرداخت، یا اشتراک درآمد پشتیبانی نمی‌کند.
