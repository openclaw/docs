...
summary: فرمت پوشه Skills، فایل‌های الزامی، انواع فایل‌های مجاز، محدودیت‌ها.
x-i18n:
...
- `.gitignore` (این نیز رعایت می‌شود)

## وارد کردن از GitHub

واردکنندهٔ وبی GitHub از انتشار/همگام‌سازی محلی سخت‌گیرتر است. این ابزار فقط فایل‌های
...
فرادادهٔ نصب محلی (نوشته‌شده توسط CLI):

- `<skill>/.clawhub/origin.json` (`.clawdhub` قدیمی)

وضعیت نصب در پوشهٔ کاری (نوشته‌شده توسط CLI):

- `<workdir>/.clawhub/lock.json` (`.clawdhub` قدیمی)

## `SKILL.md`

- Markdown با پیش‌متن YAML اختیاری.
- سرور هنگام انتشار، فراداده را از پیش‌متن استخراج می‌کند.
- `description` به‌عنوان خلاصهٔ مهارت در رابط کاربری/جست‌وجو استفاده می‌شود.

## فرادادهٔ پیش‌متن

فرادادهٔ مهارت در پیش‌متن YAML ابتدای `SKILL.md` شما اعلام می‌شود. این به رجیستری (و تحلیل امنیتی) می‌گوید مهارت شما برای اجرا به چه چیزهایی نیاز دارد.
...
### فرادادهٔ زمان اجرا (`metadata.openclaw`)
...
از `requires.env` برای متغیرهای محیطی‌ای استفاده کنید که باید پیش از اجرای مهارت وجود داشته باشند. وقتی به فرادادهٔ جداگانه برای هر متغیر نیاز دارید، از جمله متغیرهای اختیاری با `required: false`، از `envVars` استفاده کنید.
...
| `primaryEnv`       | `string`   | متغیر محیطی اعتبارنامهٔ اصلی برای مهارت شما.                                                                                                  |
...
| `emoji`            | `string`   | ایموجی نمایش برای مهارت.                                                                                                                 |
| `homepage`         | `string`   | نشانی صفحهٔ اصلی یا مستندات مهارت.                                                                                                         |
...
اگر مهارت شما به وابستگی‌های نصب‌شده نیاز دارد، آن‌ها را در آرایهٔ `install` اعلام کنید:
...
- نوع‌های محتوایی که با `text/` شروع می‌شوند به‌عنوان متن در نظر گرفته می‌شوند؛ به‌علاوهٔ یک فهرست مجاز کوچک (JSON/YAML/TOML/JS/TS/Markdown/SVG).

محدودیت‌ها (سمت سرور):

- اندازهٔ کل بسته: 50MB.
- متن جاسازی‌شده شامل `SKILL.md` + حداکثر حدود ۴۰ فایل غیر `.md` است (سقف با بهترین تلاش).

## اسلاگ‌ها

- به‌طور پیش‌فرض از نام پوشه مشتق می‌شوند.
- دامنه‌های بسته باید دقیقاً با شناسهٔ ناشر ClawHub مطابقت داشته باشند. شناسه‌های ناشر می‌توانند از حروف کوچک، اعداد، خط تیره، نقطه، و زیرخط استفاده کنند؛ باید با یک حرف کوچک یا عدد شروع و تمام شوند.
- اسلاگ‌های بسته باید حروف کوچک و سازگار با npm باشند، برای مثال `@example.tools/demo-plugin` یا `demo-plugin`.
...
- همهٔ مهارت‌های منتشرشده در ClawHub تحت مجوز `MIT-0` هستند.
...
- ClawHub از مهارت‌های پولی، قیمت‌گذاری برای هر مهارت، دیوار پرداخت، یا تقسیم درآمد پشتیبانی نمی‌کند.
- فرادادهٔ قیمت‌گذاری را به `SKILL.md` اضافه نکنید؛ این بخشی از قالب مهارت نیست و یک مهارت منتشرشده را پولی نمی‌کند.
- اگر مهارت شما با یک سرویس شخص ثالث پولی یکپارچه می‌شود، هزینهٔ خارجی و حساب موردنیاز را به‌روشنی در دستورالعمل‌های مهارت و اعلان‌های محیطی مستند کنید (`requires.env` برای متغیرهای الزامی، یا `envVars` با `required: false` برای متغیرهای اختیاری).
