...
- هر فایل _متنی_ پشتیبان (ببینید «فایل‌های مجاز»)
- `.clawhubignore` (الگوهای نادیده‌گیری برای انتشار، `.clawdhubignore` قدیمی)
- `.gitignore` (این نیز رعایت می‌شود)

## درون‌ریزی از GitHub

درون‌ریز وب GitHub از انتشار/همگام‌سازی محلی سخت‌گیرتر است. این ابزار فقط فایل‌های
...
برای متغیرهای محیطی‌ای که باید پیش از اجرای مهارت حاضر باشند، از `requires.env` استفاده کنید. وقتی برای هر متغیر به فراداده نیاز دارید، از جمله متغیرهای اختیاری با `required: false`، از `envVars` استفاده کنید.
...
| `os`               | `string[]` | محدودیت‌های OS (برای مثال `["macos"]`، `["linux"]`).                                                                                             |
...
تحلیل امنیتی ClawHub بررسی می‌کند که آنچه مهارت شما اعلام می‌کند با کاری که واقعاً انجام می‌دهد هم‌خوان باشد. اگر کد شما به `TODOIST_API_KEY` ارجاع دهد اما frontmatter شما آن را زیر `requires.env`، `primaryEnv` یا `envVars` اعلام نکند، تحلیل یک ناهماهنگی فراداده را پرچم‌گذاری می‌کند. دقیق نگه‌داشتن اعلان‌ها به مهارت شما کمک می‌کند بازبینی را بگذراند و به کاربران کمک می‌کند بفهمند چه چیزی نصب می‌کنند.
...
- اندازه کل بسته: 50MB.
- متن embedding شامل `SKILL.md` + تا حدود ۴۰ فایل غیر `.md` است (سقف بر مبنای بهترین تلاش).

## Slugها

- به‌صورت پیش‌فرض از نام پوشه مشتق می‌شوند.
- scopeهای بسته باید دقیقاً با شناسه ناشر ClawHub مطابقت داشته باشند. شناسه‌های ناشر می‌توانند از حروف کوچک، اعداد، خط تیره، نقطه و زیرخط استفاده کنند؛ باید با یک حرف کوچک یا عدد شروع و تمام شوند.
...
- شرایط مجوز ناسازگار را در `SKILL.md` اضافه نکنید؛ ClawHub بازنویسی مجوز برای هر مهارت را پشتیبانی نمی‌کند.

## Skills پولی

- ClawHub از Skills پولی، قیمت‌گذاری برای هر مهارت، paywallها یا اشتراک درآمد پشتیبانی نمی‌کند.
- فراداده قیمت‌گذاری را به `SKILL.md` اضافه نکنید؛ این بخشی از قالب مهارت نیست و یک مهارت منتشرشده را پولی نمی‌کند.
- اگر مهارت شما با یک سرویس شخص ثالث پولی یکپارچه می‌شود، هزینه خارجی و حساب لازم را به‌روشنی در دستورالعمل‌های مهارت و اعلان‌های env مستند کنید (`requires.env` برای متغیرهای ضروری، یا `envVars` با `required: false` برای متغیرهای اختیاری).
