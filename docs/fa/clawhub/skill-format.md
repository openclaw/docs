...
summary: فرمت پوشه Skills، فایل‌های الزامی، انواع فایل مجاز، محدودیت‌ها.
x-i18n:
    generated_at: "2026-07-03T17:30:25Z"
    model: gpt-5.5
...
`SKILL.md` یا فایل‌های قدیمی `skills.md` را در مخازن عمومی، غیر fork، و متعلق به
حساب GitHub واردشده شناسایی می‌کند. مخازن خصوصی، forkها،
...
- اندازه کل بسته: 50MB.
- متن embedding شامل `SKILL.md` + تا حدود 40 فایل غیر `.md` است (سقف best-effort).
...
- اگر مهارت شما با یک سرویس شخص ثالث پولی یکپارچه می‌شود، هزینه خارجی و حساب موردنیاز را به‌روشنی در دستورالعمل‌های مهارت و تعریف‌های env مستند کنید (`requires.env` برای متغیرهای ضروری، یا `envVars` با `required: false` برای متغیرهای اختیاری).
