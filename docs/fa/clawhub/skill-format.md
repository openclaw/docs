...
summary: فرمت پوشهٔ Skills، فایل‌های الزامی، انواع فایل‌های مجاز، محدودیت‌ها.
x-i18n:
    generated_at: "2026-07-01T08:20:30Z"
    model: gpt-5.5
...
`SKILL.md` یا نسخه قدیمی `skills.md` را در مخازن عمومی، غیر فورک، متعلق به
حساب GitHub واردشده کشف می‌کند. مخازن خصوصی، فورک‌ها،
...
از `requires.env` برای متغیرهای محیطی استفاده کنید که پیش از اجرای مهارت باید وجود داشته باشند. وقتی برای هر متغیر به فراداده نیاز دارید، از جمله متغیرهای اختیاری با `required: false`، از `envVars` استفاده کنید.
...
- فراداده قیمت‌گذاری را به `SKILL.md` اضافه نکنید؛ این بخشی از قالب مهارت نیست و باعث پولی شدن یک مهارت منتشرشده نمی‌شود.
