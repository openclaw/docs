...
summary: فرمت پوشهٔ Skill، فایل‌های الزامی، انواع فایل‌های مجاز، محدودیت‌ها.
x-i18n:
    generated_at: "2026-05-12T08:45:04Z"
    model: gpt-5.5
    provider: openai
...
- هر فایل پشتیبان _مبتنی بر متن_ (ببینید «فایل‌های مجاز»)
...
فراداده مهارت در frontmatter نوع YAML در بالای `SKILL.md` شما اعلام می‌شود. این به رجیستری (و تحلیل امنیتی) می‌گوید مهارت شما برای اجرا به چه چیزهایی نیاز دارد.
...
از `requires.env` برای متغیرهای محیطی‌ای استفاده کنید که پیش از اجرای مهارت باید موجود باشند. وقتی برای هر متغیر به فراداده نیاز دارید، از جمله متغیرهای اختیاری با `required: false`، از `envVars` استفاده کنید.
...
فقط فایل‌های «مبتنی بر متن» هنگام انتشار پذیرفته می‌شوند.
...
- انتساب لازم نیست.
- شروط مجوز ناسازگار را در `SKILL.md` اضافه نکنید؛ ClawHub از بازنویسی مجوز برای هر مهارت پشتیبانی نمی‌کند.
...
- اگر مهارت شما با یک سرویس شخص ثالث پولی یکپارچه می‌شود، هزینه خارجی و حساب موردنیاز را به‌روشنی در دستورالعمل‌های مهارت و اعلامیه‌های env مستند کنید (`requires.env` برای متغیرهای الزامی، یا `envVars` همراه با `required: false` برای متغیرهای اختیاری).
