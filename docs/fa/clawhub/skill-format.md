...
summary: قالب پوشهٔ مهارت، فایل‌های ضروری، انواع فایل مجاز، محدودیت‌ها.
x-i18n:
...
- `SKILL.md` (یا `skill.md`؛ نسخه قدیمی `skills.md` نیز پذیرفته می‌شود)

اختیاری:

- هر فایل پشتیبان _متنی_ (بخش «فایل‌های مجاز» را ببینید)
- `.clawhubignore` (الگوهای نادیده‌گیری برای انتشار، نسخه قدیمی `.clawdhubignore`)
- `.gitignore` (آن هم رعایت می‌شود)

## وارد کردن از GitHub

واردکننده وب GitHub از انتشار/همگام‌سازی محلی سخت‌گیرتر است. این ابزار فقط فایل‌های
`SKILL.md` یا نسخه قدیمی `skills.md` را در مخزن‌های عمومی و غیرفورک که متعلق به
...
فراداده Skill در frontmatter نوع YAML در بالای `SKILL.md` شما اعلام می‌شود. این به رجیستری (و تحلیل امنیتی) می‌گوید Skill شما برای اجرا به چه چیزهایی نیاز دارد.
...
از `requires.env` برای متغیرهای محیطی‌ای استفاده کنید که پیش از اجرای Skill باید حاضر باشند. وقتی برای هر متغیر به فراداده نیاز دارید، از جمله متغیرهای اختیاری با `required: false`، از `envVars` استفاده کنید.
...
تحلیل امنیتی ClawHub بررسی می‌کند که آنچه Skill شما اعلام می‌کند با کاری که واقعاً انجام می‌دهد مطابقت داشته باشد. اگر کد شما به `TODOIST_API_KEY` ارجاع دهد اما frontmatter شما آن را زیر `requires.env`، `primaryEnv`، یا `envVars` اعلام نکرده باشد، تحلیل یک ناسازگاری فراداده را گزارش می‌کند. دقیق نگه داشتن اعلان‌ها کمک می‌کند Skill شما بازبینی را با موفقیت بگذراند و به کاربران کمک می‌کند بفهمند چه چیزی نصب می‌کنند.
...
- اندازه کل بسته: 50MB.
- متن embedding شامل `SKILL.md` + تا حدود 40 فایل غیر `.md` است (سقف به‌صورت best-effort).
...
- slugهای بسته باید کوچک‌نویس و npm-safe باشند، برای مثال `@example.tools/demo-plugin` یا `demo-plugin`.
...
- فراداده قیمت‌گذاری را به `SKILL.md` اضافه نکنید؛ این بخشی از قالب Skill نیست و باعث پولی شدن Skill منتشرشده نمی‌شود.
