...
summary: فرمت پوشهٔ Skill، فایل‌های موردنیاز، انواع فایل مجاز، محدودیت‌ها.
x-i18n:
    generated_at: "2026-07-04T20:38:29Z"
    model: gpt-5.5
...
`SKILL.md` یا `skills.md` قدیمی را در مخازن عمومی و غیر fork که مالک آن‌ها
حساب GitHub واردشده است پیدا می‌کند. مخازن خصوصی، forkها،
...
تحلیل امنیتی ClawHub بررسی می‌کند که آنچه مهارت شما اعلام می‌کند با کاری که واقعاً انجام می‌دهد همخوان باشد. اگر کد شما به `TODOIST_API_KEY` ارجاع دهد اما frontmatter شما آن را زیر `requires.env`، `primaryEnv`، یا `envVars` اعلام نکند، تحلیل یک ناسازگاری فراداده را علامت‌گذاری می‌کند. دقیق نگه داشتن اعلان‌ها کمک می‌کند مهارت شما بازبینی را بگذراند و به کاربران کمک می‌کند بفهمند چه چیزی را نصب می‌کنند.
...
- اندازه کل بسته: 50MB.
- متن جاسازی‌شده شامل `SKILL.md` + تا حدود 40 فایل غیر `.md` است (سقف بر پایه بهترین تلاش).
...
- اگر مهارت شما با یک سرویس پولی شخص ثالث یکپارچه می‌شود، هزینه خارجی و حساب موردنیاز را به‌روشنی در دستورالعمل‌های مهارت و اعلان‌های محیطی مستند کنید (`requires.env` برای متغیرهای الزامی، یا `envVars` با `required: false` برای متغیرهای اختیاری).
