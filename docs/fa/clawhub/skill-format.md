...
summary: قالب پوشهٔ مهارت، فایل‌های الزامی، انواع فایل‌های مجاز، محدودیت‌ها.
x-i18n:
    generated_at: "2026-07-01T18:17:21Z"
    model: gpt-5.5
...
از `requires.env` برای متغیرهای محیطی استفاده کنید که باید پیش از اجرای مهارت موجود باشند. وقتی برای هر متغیر به فراداده نیاز دارید، از جمله متغیرهای اختیاری با `required: false`، از `envVars` استفاده کنید.
...
تحلیل امنیتی ClawHub بررسی می‌کند آنچه مهارت شما اعلام می‌کند با کاری که واقعاً انجام می‌دهد منطبق باشد. اگر کد شما به `TODOIST_API_KEY` ارجاع دهد اما frontmatter شما آن را زیر `requires.env`، `primaryEnv` یا `envVars` اعلام نکرده باشد، تحلیل یک ناهماهنگی فراداده را علامت‌گذاری می‌کند. دقیق نگه داشتن اعلان‌ها به مهارت شما کمک می‌کند بازبینی را بگذراند و به کاربران کمک می‌کند بفهمند چه چیزی نصب می‌کنند.
