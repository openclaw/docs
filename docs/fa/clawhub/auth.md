...
x-i18n:
    generated_at: "2026-07-01T18:16:20Z"
    model: gpt-5.5
...
حساب‌های حذف‌شده، مسدودشده یا غیرفعال‌شده نمی‌توانند ورود عادی به ClawHub را تکمیل کنند.
اگر ورود شما را به حالت خارج‌شده از حساب برمی‌گرداند، ممکن است حساب شما در وضعیت مناسبی
نباشد. اگر حساب شما مسدود یا غیرفعال شده است و فکر می‌کنید این موضوع اشتباه است، از
...
1. CLI یک سرور بازگشت‌فراخوان موقت را روی `127.0.0.1` راه‌اندازی می‌کند.
2. مرورگر شما صفحه ورود ClawHub را باز می‌کند.
3. پس از ورود با GitHub، ClawHub یک توکن API ایجاد می‌کند.
4. مرورگر دوباره به بازگشت‌فراخوان محلی هدایت می‌شود.
5. CLI توکن را در فایل پیکربندی ClawHub شما ذخیره می‌کند.

اگر مرورگر شما به‌دلیل دیوار آتش، VPN یا قواعد پراکسی نمی‌تواند به بازگشت‌فراخوان محلی
دسترسی پیدا کند، از جریان توکن بدون واسط گرافیکی استفاده کنید.
...
اگر حساب شما مسدود یا غیرفعال شده است و فکر می‌کنید این موضوع اشتباه است، از
[فرم درخواست تجدیدنظر ClawHub](https://appeals.openclaw.ai/) استفاده کنید.
