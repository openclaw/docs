...
x-i18n:
    generated_at: "2026-07-04T10:50:27Z"
    model: gpt-5.5
...
حساب‌های حذف‌شده، مسدودشده یا غیرفعال‌شده نمی‌توانند فرایند عادی ورود به ClawHub را کامل کنند.
اگر ورود شما را دوباره به حالت خارج‌شده برگرداند، ممکن است حساب شما در وضعیت مناسبی
...
1. CLI یک سرور کال‌بک موقت روی `127.0.0.1` راه‌اندازی می‌کند.
2. مرورگر شما صفحه ورود ClawHub را باز می‌کند.
3. پس از ورود با GitHub، ClawHub یک توکن API ایجاد می‌کند.
4. مرورگر به کال‌بک محلی هدایت می‌شود.
5. CLI توکن را در فایل پیکربندی ClawHub شما ذخیره می‌کند.

اگر مرورگر شما به‌دلیل دیوار آتش، VPN یا
قواعد پراکسی نتواند به کال‌بک محلی دسترسی پیدا کند، از جریان توکن بدون واسط گرافیکی استفاده کنید.
...
برای شل‌های راه‌دور که می‌توانید مرورگر را در جای دیگری باز کنید، اجرا کنید:
