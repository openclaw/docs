...
x-i18n:
    generated_at: "2026-07-05T07:36:19Z"
    model: gpt-5.5
...
ClawHub برای ورود وب از GitHub استفاده می‌کند. CLI از توکن‌های API متعلق به ClawHub استفاده می‌کند که
...
اگر ورود شما را به وضعیت خارج‌شده از حساب برگرداند، ممکن است حساب شما در
وضعیت مناسبی نباشد. اگر حساب شما مسدود یا غیرفعال شده است و فکر می‌کنید این یک
اشتباه است، از
...
1. CLI یک سرور بازفراخوان موقت را روی `127.0.0.1` راه‌اندازی می‌کند.
...
پراکسی نمی‌تواند به بازفراخوان محلی برسد، از جریان توکن بدون واسط گرافیکی استفاده کنید.
...
برای شل‌های راه دور که می‌توانید در جایی دیگر مرورگر باز کنید، اجرا کنید:

```bash
clawhub login --device
```

CLI یک کد یک‌بارمصرف چاپ می‌کند و تا زمانی که آن را در
...
می‌توانید توکن‌های API را در رابط وب ClawHub ابطال کنید.
