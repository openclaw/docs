...
x-i18n:
    generated_at: "2026-07-01T13:11:03Z"
    model: gpt-5.5
...
اگر ورود شما را به وضعیت خارج‌شده از حساب برمی‌گرداند، ممکن است حساب شما در وضعیت
...
اگر مرورگر شما به‌دلیل قوانین firewall، VPN یا proxy نتواند به callback محلی دسترسی پیدا کند،
از جریان توکن بدون واسط گرافیکی استفاده کنید.
...
توکن‌های لغوشده، نامعتبر یا مفقود `401 Unauthorized` برمی‌گردانند. دوباره با
`clawhub login` وارد شوید یا با `clawhub login --token` یک توکن تازه ارائه کنید.

حساب‌های حذف‌شده، مسدودشده یا غیرفعال‌شده نمی‌توانند به استفاده از توکن‌های API موجود ادامه دهند.
اگر حساب شما مسدود یا غیرفعال شده است، در صورتی که فکر می‌کنید این یک اشتباه است، از
