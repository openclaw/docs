...
read_when:
    ...
    - اشکال‌زدایی خطاهای ۴۰۱
summary: ورود به سیستم ClawHub، توکن‌های API، ورود CLI، ذخیره‌سازی توکن و ابطال.
x-i18n:
    generated_at: "2026-07-04T20:37:46Z"
    model: gpt-5.5
...
حساب‌های حذف‌شده، ممنوع‌شده یا غیرفعال‌شده نمی‌توانند ورود عادی ClawHub را کامل کنند.
اگر ورود شما را به وضعیت خارج‌شده از حساب برگرداند، ممکن است حساب شما در وضعیت
مناسبی نباشد. اگر حساب شما ممنوع یا غیرفعال شده است، در صورتی که فکر می‌کنید این یک
اشتباه است، از
[فرم درخواست تجدیدنظر ClawHub](https://appeals.openclaw.ai/) استفاده کنید.

## ورود CLI

جریان ورود پیش‌فرض CLI مرورگر شما را باز می‌کند:
...
4. مرورگر به callback محلی برمی‌گردد.
5. CLI توکن را در فایل پیکربندی ClawHub شما ذخیره می‌کند.

اگر مرورگر شما به‌دلیل قوانین firewall، VPN یا
proxy نتواند به callback محلی دسترسی پیدا کند، از جریان توکن headless استفاده کنید.
...
حساب‌های حذف‌شده، ممنوع‌شده یا غیرفعال‌شده نمی‌توانند به استفاده از توکن‌های API موجود ادامه دهند.
اگر حساب شما ممنوع یا غیرفعال شده است، در صورتی که فکر می‌کنید این یک
اشتباه است، از
[فرم درخواست تجدیدنظر ClawHub](https://appeals.openclaw.ai/) استفاده کنید.
