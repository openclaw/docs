...
x-i18n:
    generated_at: "2026-06-28T08:01:44Z"
    model: gpt-5.5
...
فهرست‌های شخص ثالث می‌توانند از endpointهای خواندنی عمومی برای فهرست کردن یا جست‌وجوی Skills در ClawHub استفاده کنند. لطفاً نتایج را cache کنید، `429`/`Retry-After` را رعایت کنید، کاربران را به فهرست canonical در ClawHub (`https://clawhub.ai/<owner>/skills/<slug>`) برگردانید، و از القای تأیید سایت شخص ثالث توسط ClawHub خودداری کنید. تلاش نکنید محتوای پنهان، خصوصی، یا مسدودشده توسط moderation را خارج از سطح API عمومی mirror کنید.

میان‌برهای slug وب در خانواده‌های registry resolve می‌شوند، اما کلاینت‌های API باید به‌جای بازسازی تقدم مسیرها، از URLهای canonical برگردانده‌شده توسط endpointهای خواندن استفاده کنند.
...
- endpointهای نوشتنِ احراز هویت‌شده نباید وقتی سرور دلیل را می‌داند یک `Unauthorized` تنها برگردانند. توکن‌های مفقود، توکن‌های نامعتبر/لغوشده، و حساب‌های حذف‌شده/مسدودشده/غیرفعال‌شده باید هرکدام متن قابل اقدام دریافت کنند تا کلاینت‌های CLI بتوانند به کاربران بگویند چه چیزی آن‌ها را مسدود کرده است.
...
- `Retry-After`: ثانیه‌های انتظار پیش از تلاش دوباره (delay) در `429`
...
- فقط زمانی از headerهای IP کلاینت مورد اعتماد، از جمله `cf-connecting-ip`، استفاده می‌کند که deployment به‌صراحت headerهای forwarded مورد اعتماد را فعال کرده باشد.
- ClawHub از headerهای trusted forwarding برای شناسایی IPهای کلاینت در edge استفاده می‌کند.
- اگر IP کلاینت مورد اعتمادی در دسترس نباشد، درخواست‌های ناشناس از bucketهای fallback استفاده می‌کنند که فقط بر اساس نوع rate-limit محدود شده‌اند. این bucketهای fallback شامل مسیرهای ارائه‌شده توسط caller، slugها، نام packageها، نسخه‌ها، query stringها، یا سایر پارامترهای artifact نمی‌شوند.

## پاسخ‌های خطا

پاسخ‌های خطای عمومی v1 متن ساده با `content-type: text/plain; charset=utf-8` هستند.
این شامل شکست‌های اعتبارسنجی (`400`)، منابع عمومی مفقود (`404`)، شکست‌های auth و permission (`401`/`403`)، محدودیت‌های نرخ (`429`)، و دانلودهای مسدودشده است. کلاینت‌ها باید بدنهٔ پاسخ را به‌صورت یک رشتهٔ خواندنی برای انسان بخوانند. پارامترهای query ناشناخته برای سازگاری نادیده گرفته می‌شوند، اما پارامترهای query شناخته‌شده با مقدارهای نامعتبر `400` برمی‌گردانند.
...
- نتایج به‌ترتیب ارتباط برگردانده می‌شوند (شباهت embedding + تقویت token دقیق slug/name + یک prior محبوبیت کوچک).
- ارتباط از محبوبیت قوی‌تر است. تطابق دقیق token در slug یا display-name می‌تواند از تطابقی سست‌تر با engagement بسیار بیشتر بالاتر قرار بگیرد.
- متن ASCII بر اساس مرزهای واژه و نشانه‌گذاری tokenسازی می‌شود. برای نمونه، `personal-map` شامل token مستقل `map` است، در حالی که `amap-jsapi-skill` شامل `amap`، `jsapi`، و `skill` است؛ بنابراین جست‌وجوی `map` به `personal-map` تطابق lexical قوی‌تری نسبت به `amap-jsapi-skill` می‌دهد.
- محبوبیت به‌صورت لگاریتمی مقیاس‌بندی و capped می‌شود. Skills با engagement بالا ممکن است وقتی متن query تطابق ضعیف‌تری دارد رتبهٔ پایین‌تری بگیرند.
...
راهنمای discoverability برای منتشرکننده:

- اصطلاحاتی را که کاربران عیناً جست‌وجو خواهند کرد در display name، summary، و tagها قرار دهید. فقط وقتی از یک token مستقل در slug استفاده کنید که همان token هویت پایداری هم باشد که می‌خواهید حفظ کنید.
...
- اگر skill به‌طور غیرمنتظره‌ای نامرئی است، پیش از تغییر metadata مرتبط با رتبه‌بندی، ابتدا در حالت logged in با `clawhub inspect @owner/slug` وضعیت moderation را بررسی کنید.
...
- `moderation` فقط وقتی skill flagged باشد یا مالک آن را مشاهده کند اضافه می‌شود.
...
یک skill را برای بازبینی moderator گزارش دهید. گزارش‌ها در سطح skill هستند، به‌صورت اختیاری به یک نسخه لینک می‌شوند، و صف گزارش skill را تغذیه می‌کنند.
...
`note` برای `confirmed` و `dismissed` الزامی است؛ هنگام برگرداندن `status` به `open` می‌توان آن را حذف کرد. برای پنهان کردن skill در همان workflow قابل audit، همراه با گزارش triageشده `finalAction: "hide"` را پاس دهید.
...
- `version.security` شامل وضعیت تأیید scan عادی‌سازی‌شده و جزئیات scanner است
...
- هنگام کوئری‌گرفتن از یک نسخه تاریخی، پیش از اینکه `moderation` و `security` را در همان زمینه نسخه در نظر بگیرید، `moderation.matchesRequestedVersion` و `moderation.sourceVersion` را بررسی کنید.
...
- کارهای اسکن ناهمگام هستند. درخواست‌های اسکن دستی پیش از کارهای عادی انتشار/پرکردن گذشته اولویت‌بندی می‌شوند، اما تکمیل همچنان به دسترس‌بودن worker بستگی دارد.
...
- هنگام در صف بودن، `queue.queuedAhead` و `queue.position` را برمی‌گرداند تا کلاینت‌ها بتوانند نشان دهند چند اسکن دستی اولویت‌دار پیش از این درخواست قرار دارند. صف‌های بسیار بزرگ محدود می‌شوند و با `queuedAheadIsEstimate: true` گزارش می‌شوند.
...
مسیر بازاسکن دسته‌ای canonical فقط ویژه مدیر. همان شکل محموله `POST /api/v1/skills/-/rescan-batch` قدیمی را می‌پذیرد.

### `POST /api/v1/skills/-/scan/batch/status`

مسیر وضعیت دسته‌ای canonical فقط ویژه مدیر. `{ "jobIds": ["..."] }` را می‌پذیرد و همان شمارنده‌های تجمیعی `POST /api/v1/skills/-/rescan-batch/status` قدیمی را برمی‌گرداند.

### `GET /api/v1/skills/{slug}/verify`

پاکت راستی‌آزمایی کارت Skill را که توسط `clawhub skill verify` استفاده می‌شود، برمی‌گرداند.
...
- `ok` فقط زمانی `true` است که نسخه انتخاب‌شده یک کارت Skill تولیدشده داشته باشد، توسط تعدیل به‌عنوان بدافزار مسدود نشده باشد، و راستی‌آزمایی ClawScan پاک باشد.
- هویت Skill، هویت ناشر، و فراداده نسخه انتخاب‌شده فیلدهای سطح بالای پاکت هستند (`slug`، `displayName`، `publisherHandle`، `version`، `resolvedFrom`، `tag`، `createdAt`) تا خودکارسازی shell بتواند آن‌ها را بدون بازکردن wrapperهای تو‌در‌تو بخواند.
...
- پاسخ فقط امنیتی است. شامل داده‌های کارت Skill، وضعیت کارت تولیدشده، فهرست‌های فایل artifact، یا محموله‌های تفصیلی اسکنر نیست.
...
- `security.signals.dependencyRegistry` برای سازگاری پاسخ v1 حفظ شده است، اما اسکنر وجود رجیستری وابستگی بازنشسته شده و این کلید همیشه `null` است.
- نبود کارت Skill بر `ok`، `decision`، یا `reasons` این نقطه پایانی اثر نمی‌گذارد؛ کلاینت‌ها هنگام نیاز به محتوای کارت باید `skill-card.md` نصب‌شده را به‌صورت محلی بخوانند.
- وقتی به پاکت راستی‌آزمایی کارت Skill برای یک Skill نیاز دارید از `/verify` استفاده کنید، وقتی به markdown کارت تولیدشده نیاز دارید از `/card`، و وقتی به داده‌های تفصیلی اسکنر نیاز دارید از `/scan`.
...
- `category` (اختیاری): فیلتر دسته‌بندی Plugin. فقط زمانی پشتیبانی می‌شود که
  ...
  نام‌های مستعار فیلتر قدیمی نسخه ۱ زیر `GET /api/v1/plugins` مستند شده‌اند.
...
نام‌های مستعار فیلتر قدیمی نسخه ۱ همچنان در نقاط پایانی خواندن پذیرفته می‌شوند:
...
صدور دسته‌جمعی آخرین Skills عمومی برای تحلیل آفلاین.
...
- `startDate` (الزامی): کران پایین به میلی‌ثانیه یونیکس برای `updatedAt` در Skills.
- `endDate` (الزامی): کران بالا به میلی‌ثانیه یونیکس برای `updatedAt` در Skills.
...
- Skills فعلی متکی به GitHub با اسکن `clean` یا `suspicious` شامل
...
- `_manifest.json` همیشه در ریشه ZIP گنجانده می‌شود.
- وقتی Pluginها یا فایل‌های منفرد صادر نشوند، `_errors.json` گنجانده می‌شود.
...
- نام‌های مستعار فیلتر قدیمی v1 که زیر `GET /api/v1/plugins` مستند شده‌اند نیز
  پذیرفته می‌شوند.
- فیلتر دسته‌بندی یک فیلتر واقعی API است که توسط ردیف‌های خلاصه دسته‌بندی Plugin
...
راستی‌آزمایی، فراداده artifact، و داده‌های scan، برمی‌گرداند.
...
- وقتی داده scan وجود داشته باشد، `version.vtAnalysis`، `version.llmAnalysis`، و `version.staticScan`
  گنجانده می‌شوند.
- بسته‌های خصوصی `404` برمی‌گردانند، مگر اینکه فراخواننده بتواند ناشر مالک را بخواند.
...
انتشار resolveشده قابل نصب است یا نه.
...
- `release.artifactKind`، `release.artifactSha256`، `release.npmIntegrity`,
  ...
  artifact انتشار شناخته شده باشند، وجود دارند.
...
- `trust.blockedFromDownload` سیگنال مسدودسازی نصب است. OpenClaw و سایر
  ...
  قوانین مسدودسازی را دوباره از فیلدهای scanner یا moderation استخراج کنند.
- `trust.reasons` فهرست توضیحات قابل نمایش برای کاربر و audit است. کدهای دلیل
...
- `trust.stale` یعنی خلاصه اعتماد از ورودی‌های قدیمی محاسبه شده و
  قبل از تصمیم allow با اطمینان بالا باید نیازمند refresh تلقی شود.
...
- این نقطه پایانی عمداً محدودتر از نقاط پایانی نظارت مالک/ناظر است.
  ...
  هویت گزارش‌دهندگان، متن گزارش‌ها، شواهد خصوصی، یا جدول‌های زمانی review داخلی.
...
- نسخه‌های بسته legacy یک artifact از نوع `legacy-zip` و یک
  `downloadUrl` برای ZIP legacy برمی‌گردانند.
...
- نسخه‌های ZIP legacy به `/api/v1/packages/{name}/download?version=` redirect می‌شوند.
...
نقطه پایانی ناظر برای فهرست کردن ردیف‌های migration مربوط به Plugin رسمی OpenClaw.
...
نقطه پایانی مدیر برای ایجاد یا به‌روزرسانی یک ردیف migration مربوط به Plugin رسمی.
...
- `packageName` به نام npm نرمال‌سازی می‌شود؛ بسته می‌تواند برای migrationهای برنامه‌ریزی‌شده
  وجود نداشته باشد.
- این فقط آمادگی migration را پیگیری می‌کند. OpenClaw را تغییر نمی‌دهد و
  ClawPack تولید نمی‌کند.

### `GET /api/v1/packages/moderation/queue`

نقطه پایانی ناظر/مدیر برای صف‌های review انتشار بسته.
...
- `all`: هر انتشاری با override دستی، وضعیت scan غیر clean، یا گزارش بسته.
...
یک بسته را برای review ناظر گزارش می‌کند. گزارش‌ها در سطح بسته هستند و به‌صورت اختیاری
...
نقطه پایانی مالک/ناظر برای نمایان بودن وضعیت نظارت بسته.
...
برگرداندن `status` به `open` می‌تواند حذف شود. برای اعمال نظارت انتشار در همان
...
- فراداده‌های صرفاً رجیستری به آرشیو دانلودشده تزریق نمی‌شوند.
...
- `dist.tarball`، `dist.integrity`، و `dist.shasum` از فیلدهای سازگار با npm
  استفاده می‌کنند تا کاربران در صورت انتخاب بتوانند npm را به mirror اشاره دهند.
...
پشتیبانی‌شده با GitHub که اسکن `clean` یا `suspicious` دارد و نسخه میزبانی‌شده ندارد،
واگذاری منبع GitHub را برمی‌گرداند.
...
- واگذاری‌های skill پشتیبانی‌شده با GitHub بایت‌ها را proxy یا mirror نمی‌کنند. پاسخ JSON
  ...
  درج نمی‌شود.
- آمار دانلود به‌عنوان هویت‌های یکتا در هر روز UTC شمارش می‌شود (`userId` وقتی توکن API معتبر است، در غیر این صورت IP).
...
- فیلدهای فرم مجاز عبارت‌اند از `payload`، blobهای تکرارشونده `files`، یا یک مرجع tarball
  `clawpack`. `clawpack` می‌تواند یک blob `.tgz` یا یک storage id برگشتی از
  جریان upload-url باشد. انتشارهای stage شده با storage-id باید همچنین
...
- بدنه‌های JSON و فراداده‌های `payload.files` / `payload.artifact`
...
وقتی حاضر باشد، `reason` به‌عنوان یادداشت نظارت skill ذخیره می‌شود و در گزارش حسابرسی کپی می‌شود.
...
برای یک org تازه‌ساخته‌شده، `memberHandle` را ارائه کنید؛ مدیر عامل به‌عنوان عضو اضافه نمی‌شود.
...
ایجاد ناشر org به‌صورت self-serve و احرازشده. یک ناشر org جدید ایجاد می‌کند و
...
provider مربوط به GitHub را نام ببرد؛ handleهای تغییرپذیر فقط به‌عنوان محافظ روبه‌روی operator استفاده می‌شوند.

این نقطه پایانی به‌صورت پیش‌فرض روی اجرای آزمایشی تنظیم است. اعمال بازیابی پس از آنکه کارکنان به‌طور مستقل پیوستگی بین هر دو
اصل GitHub را تأیید کردند، به `dryRun: false` و
`confirmIdentityVerified: true` نیاز دارد. اگر ناشر شخصی فعلی کاربر مقصد
دارای Skills، بسته‌ها، یا منابع Skill در GitHub باشد، بازیابی به‌صورت بسته شکست می‌خورد.
بازیابی همچنین فیلدهای قدیمی `ownerUserId` را برای Skills ناشر بازیابی‌شده،
نام‌های مستعار اسلاگ Skill، بسته‌ها، هشدارهای بازرس بسته، و ردیف‌های خلاصه جست‌وجوی مشتق‌شده مهاجرت می‌دهد تا
مسیرهای مالک مستقیم با مرجع ناشر جدید همسو شوند. یک رزرو دسته محافظت‌شده فعال
برای دسته بازیابی‌شده نیز دوباره به کاربر جایگزین واگذار می‌شود تا همگام‌سازی بعدی
...
منابع Skill در GitHub در دامنه ناشر هستند و به‌جای بازنویسی، به‌عنوان بررسی‌شده گزارش می‌شوند.
...
- `merge` فهرست منبع را پنهان می‌کند و اسلاگ منبع را به فهرست هدف تغییرمسیر می‌دهد.
...
مسدود کردن کاربر و حذف سخت Skills متعلق به او (فقط ناظر/مدیر).
...
محتوا (فقط مدیر). مگر اینکه `dryRun` برابر `false` باشد، به‌صورت پیش‌فرض اجرای آزمایشی است.
...
افزودن/حذف ستاره (برجسته‌سازی‌ها). هر دو نقطه پایانی هم‌نهشت هستند.
...
که یک تاربال ClawPack را آماده‌سازی می‌کنند باید شناسه ذخیره‌سازی حاصل را به‌عنوان
