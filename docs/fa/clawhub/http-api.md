...
x-i18n:
    generated_at: "2026-07-03T17:31:55Z"
    model: gpt-5.5
...
مسیرهای قدیمی `/api/...` و `/api/cli/...` برای سازگاری باقی می‌مانند (ببینید `DEPRECATIONS.md`).
...
میانبرهای slug وب در خانواده‌های registry resolve می‌شوند، اما کلاینت‌های API باید به‌جای بازسازی اولویت route، از URLهای canonical برگردانده‌شده توسط endpointهای خواندن استفاده کنند.
...
- درخواست‌های احراز هویت‌شده (Bearer token معتبر): بر اساس bucket هر کاربر اعمال می‌شود.
- اگر token وجود نداشته باشد یا نامعتبر باشد، رفتار به اعمال بر اساس IP برمی‌گردد.
- endpointهای نوشتن احراز هویت‌شده وقتی سرور دلیل را می‌داند نباید یک `Unauthorized` ساده برگردانند. tokenهای گم‌شده، tokenهای نامعتبر/لغوشده، و حساب‌های حذف‌شده/ممنوع‌شده/غیرفعال‌شده باید هرکدام متن قابل اقدام دریافت کنند تا کلاینت‌های CLI بتوانند به کاربران بگویند چه چیزی آن‌ها را مسدود کرده است.

- خواندن: 3000/min برای هر IP، 12000/min برای هر key
- نوشتن: 300/min برای هر IP، 3000/min برای هر key
- دانلود: 1200/min برای هر IP، 6000/min برای هر key (endpointهای دانلود)
...
- از headerهای IP کلاینت trusted، از جمله `cf-connecting-ip`، فقط وقتی استفاده می‌کند که deployment به‌طور صریح headerهای forwarded trusted را فعال کرده باشد.
- ClawHub از headerهای forwarding trusted برای شناسایی IPهای کلاینت در edge استفاده می‌کند.
- اگر IP کلاینت trusted در دسترس نباشد، درخواست‌های ناشناس از bucketهای fallback استفاده می‌کنند که فقط به نوع rate-limit محدود هستند. این bucketهای fallback شامل مسیرهای ارائه‌شده توسط caller، slugها، نام‌های package، versionها، query stringها، یا پارامترهای artifact دیگر نمی‌شوند.

## پاسخ‌های خطا

پاسخ‌های خطای عمومی v1 متن ساده با `content-type: text/plain; charset=utf-8` هستند.
این شامل failureهای اعتبارسنجی (`400`)، resourceهای عمومی گم‌شده (`404`)، failureهای auth و permission (`401`/`403`)، محدودیت‌های نرخ (`429`)، و دانلودهای مسدودشده می‌شود. کلاینت‌ها باید بدنهٔ پاسخ را به‌عنوان رشته‌ای خوانا برای انسان بخوانند. پارامترهای query ناشناخته برای سازگاری نادیده گرفته می‌شوند، اما پارامترهای query شناخته‌شده با مقدارهای نامعتبر `400` برمی‌گردانند.
...
- نتایج به ترتیب relevance برگردانده می‌شوند (embedding similarity + boostهای دقیق token برای slug/name + یک popularity prior کوچک).
- relevance از popularity قوی‌تر است. تطابق دقیق slug یا token نام نمایشی می‌تواند بالاتر از تطابقی آزادتر با engagement بسیار قوی‌تر قرار بگیرد.
- متن ASCII روی مرزهای واژه و نشانه‌گذاری tokenization می‌شود. برای مثال، `personal-map` یک token مستقل `map` دارد، درحالی‌که `amap-jsapi-skill` شامل `amap`، `jsapi`، و `skill` است؛ بنابراین جست‌وجوی `map` به `personal-map` تطابق lexical قوی‌تری نسبت به `amap-jsapi-skill` می‌دهد.
- popularity با مقیاس لگاریتمی محاسبه و محدود می‌شود. مهارت‌های با engagement بالا وقتی متن query تطابق ضعیف‌تری دارد می‌توانند رتبهٔ پایین‌تری بگیرند.
- وضعیت moderation مشکوک یا پنهان می‌تواند بسته به فیلترهای caller و وضعیت moderation فعلی، یک مهارت را از جست‌وجوی عمومی حذف کند.

راهنمای discoverability برای ناشر:

- اصطلاحاتی را که کاربران واقعاً جست‌وجو خواهند کرد در نام نمایشی، summary، و tagها قرار دهید. از token مستقل slug فقط وقتی استفاده کنید که همچنین یک هویت پایدار است که می‌خواهید نگه دارید.
- slug را فقط برای دنبال کردن یک query تغییر نام ندهید، مگر اینکه slug جدید نام canonical بلندمدت بهتری باشد. slugهای قدیمی به aliasهای redirect تبدیل می‌شوند، اما URL canonical، slug نمایش‌داده‌شده، و digestهای جست‌وجوی آینده از slug جدید استفاده می‌کنند.
- aliasهای rename برای URLهای قدیمی و نصب‌هایی که از طریق registry resolve می‌شوند، resolution را حفظ می‌کنند، اما رتبه‌بندی جست‌وجو پس از index شدن rename بر اساس metadata canonical مهارت است. آمار موجود با مهارت باقی می‌ماند.
- اگر یک مهارت به‌طور غیرمنتظره‌ای نامرئی است، پیش از تغییر metadata مرتبط با رتبه‌بندی، ابتدا وضعیت moderation را با `clawhub inspect @owner/slug` در حالت logged in بررسی کنید.
...
- `createdAt` برای crawlهای مهارت جدید پایدار است؛ `updated` وقتی مهارت‌های موجود دوباره publish می‌شوند تغییر می‌کند.
...
- slugهای قدیمی ایجادشده توسط flowهای rename/merge مالک به مهارت canonical resolve می‌شوند.
- `metadata.os`: محدودیت‌های OS اعلام‌شده در frontmatter مهارت (مثلاً `["macos"]`، `["linux"]`). اگر اعلام نشده باشد `null`.
- `metadata.systems`: targetهای system در Nix (مثلاً `["aarch64-darwin", "x86_64-linux"]`). اگر اعلام نشده باشد `null`.
- اگر مهارت metadata پلتفرم نداشته باشد، `metadata` مقدار `null` است.
- `moderation` فقط وقتی گنجانده می‌شود که مهارت flagged باشد یا مالک آن را مشاهده کند.

### `GET /api/v1/skills/{slug}/moderation`

وضعیت structured moderation را برمی‌گرداند.
...
- callerهای عمومی فقط برای مهارت‌های visible که از قبل flagged شده‌اند `200` می‌گیرند.
- evidence برای callerهای عمومی redacted است و فقط برای مالکان/moderatorها شامل snippetهای خام می‌شود.

### `POST /api/v1/skills/{slug}/report`

یک مهارت را برای بررسی moderator گزارش کنید. گزارش‌ها در سطح مهارت هستند، به‌صورت اختیاری به یک version لینک می‌شوند، و queue گزارش مهارت را تغذیه می‌کنند.
...
endpoint مخصوص moderator/admin برای دریافت گزارش‌های مهارت.
...
endpoint مخصوص moderator/admin برای resolve کردن یا بازگشایی گزارش‌های مهارت.
...
`note` برای `confirmed` و `dismissed` الزامی است؛ هنگام تنظیم دوبارهٔ `status` به `open` می‌تواند حذف شود. برای پنهان کردن مهارت در همان workflow قابل audit، همراه با گزارش triage‌شده `finalAction: "hide"` را ارسال کنید.
...
جزئیات verification اسکن امنیتی برای یک version مهارت را برمی‌گرداند.
...
- شامل وضعیت تأیید عادی‌سازی‌شده به‌همراه جزئیات ویژهٔ پویشگر است.
- `security.hasScanResult` فقط زمانی `true` است که یک پویشگر رأی قطعی (`clean`، `suspicious`، یا `malicious`) تولید کرده باشد.
- `moderation` یک نمای لحظه‌ایِ تعدیل در سطح مهارت است که از آخرین نسخه به‌دست آمده است.
...
پویش‌های بارگذاری محلی دیگر پشتیبانی نمی‌شوند. درخواست‌هایی که از
`multipart/form-data` یا `{ "source": { "kind": "upload" } }` استفاده کنند، `410` برمی‌گردانند.

پویش‌های منتشرشده از JSON استفاده می‌کنند:
...
- بارهای درخواست پویش و گزارش‌های قابل دانلود پس از پنجرهٔ نگهداری از انبار درخواست پویش منقضی می‌شوند.
- پویش‌های منتشرشده به دسترسی مدیریتی مالک/ناشر، یا اختیار ناظر/مدیر پلتفرم نیاز دارند.
- پویش‌های منتشرشده فقط زمانی بازنویسی می‌کنند که `update: true` باشد و پویش با موفقیت کامل شود.
...
- کارهای پویش ناهمگام هستند. درخواست‌های پویش دستی جلوتر از کارهای عادی انتشار/پرکردن عقب‌مانده اولویت‌بندی می‌شوند، اما تکمیل همچنان به دسترس‌بودن worker بستگی دارد.

### `GET /api/v1/skills/-/scan/{scanId}`

نقطهٔ پایانی نظرسنجی احراز هویت‌شده برای یک پویش ارسال‌شده.

- وضعیت در صف/در حال اجرا/موفق/ناموفق را برمی‌گرداند.
- هنگام در صف بودن، `queue.queuedAhead` و `queue.position` را برمی‌گرداند تا کلاینت‌ها بتوانند نشان دهند چند پویش دستی اولویت‌دار جلوتر از درخواست قرار دارند. صف‌های بسیار بزرگ محدود می‌شوند و با `queuedAheadIsEstimate: true` گزارش می‌شوند.
- در صورت وجود، `report` شامل بخش‌های `clawscan`، `skillspector`، `staticAnalysis` و `virustotal` است.
- کارهای پویش ناموفق، `status: "failed"` را همراه با `lastError` برمی‌گردانند.

### `GET /api/v1/skills/-/scan/{scanId}/download`

نقطهٔ پایانی آرشیو گزارش احراز هویت‌شده.

- به یک پویش موفق نیاز دارد؛ پویش‌های غیرنهایی `409` برمی‌گردانند.
...
- نتایج پویش ذخیره‌شده برای نسخهٔ دقیق ارسال‌شده، از جمله نسخه‌های مسدود یا پنهان، را برمی‌گرداند.
- مقدار پیش‌فرض `kind` برابر `skill` است؛ برای پویش‌های plugin/package از `kind=plugin` استفاده کنید.
- همان شکل ZIP دانلودهای درخواست پویش را برمی‌گرداند.

### `POST /api/v1/skills/-/scan/batch`

مسیر بازپویش دسته‌ای canonical فقط برای مدیران. همان شکل بار `POST /api/v1/skills/-/rescan-batch` قدیمی را می‌پذیرد.
...
پاکت تأیید کارت مهارت استفاده‌شده توسط `clawhub skill verify` را برمی‌گرداند.
...
- `ok` فقط زمانی `true` است که نسخهٔ انتخاب‌شده یک کارت مهارت تولیدشده داشته باشد، توسط تعدیل به‌عنوان بدافزار مسدود نشده باشد، و تأیید ClawScan پاک باشد.
- هویت مهارت، هویت ناشر، و فرادادهٔ نسخهٔ انتخاب‌شده فیلدهای سطح بالای پاکت هستند (`slug`، `displayName`، `publisherHandle`، `version`، `resolvedFrom`، `tag`، `createdAt`) تا خودکارسازی shell بتواند آن‌ها را بدون بازکردن wrapperهای تودرتو بخواند.
- `security` رأی سطح بالای ClawScan/امنیت است. خودکارسازی باید بر اساس `ok`، `decision`، `reasons` و `security.status` عمل کند.
- `security.signals` شامل شواهد پشتیبان پویشگر مانند `staticScan`، `virusTotal` و `skillSpector` است.
- `security.signals.dependencyRegistry` برای سازگاری پاسخ v1 حفظ شده است، اما پویشگر وجود رجیستری وابستگی بازنشسته شده و این کلید همیشه `null` است.
- `provenance` فقط زمانی `server-resolved-github-import` است که ClawHub در زمان انتشار یا واردسازی، repo/ref/commit/path مربوط به GitHub را حل و ذخیره کرده باشد؛ در غیر این صورت `unavailable` است.

### `POST /api/v1/skills/-/security-verdicts`

رأی‌های امنیتی فشردهٔ فعلی را برای نسخه‌های دقیق مهارت برمی‌گرداند. این
نقطهٔ پایانی مجموعه برای کلاینت‌هایی در نظر گرفته شده است که از قبل می‌دانند کدام نسخه‌های مهارت ClawHub نصب‌شده را باید نمایش دهند، مانند OpenClaw Control UI.
...
- پاسخ فقط امنیتی است. شامل دادهٔ کارت مهارت، وضعیت کارت تولیدشده، فهرست‌های فایل artifact، یا بارهای تفصیلی پویشگر نیست.
- `security.signals` فقط شامل شواهد پشتیبان در سطح وضعیت است؛ برای جزئیات کامل پویشگر از `/scan` یا صفحهٔ ممیزی امنیتی ClawHub استفاده کنید.
- `security.signals.dependencyRegistry` برای سازگاری پاسخ v1 حفظ شده است، اما پویشگر وجود رجیستری وابستگی بازنشسته شده و این کلید همیشه `null` است.
- نبود کارت مهارت بر `ok`، `decision` یا `reasons` این نقطهٔ پایانی اثر نمی‌گذارد؛ کلاینت‌ها وقتی به محتوای کارت نیاز دارند باید `skill-card.md` نصب‌شده را به‌صورت محلی بخوانند.
- وقتی به پاکت تأیید کارت مهارت تک‌مهارتی نیاز دارید از `/verify` استفاده کنید، وقتی به markdown کارت تولیدشده نیاز دارید از `/card`، و وقتی به دادهٔ تفصیلی پویشگر نیاز دارید از `/scan`.
...
- ورودی‌های مهارت همچنان بر رجیستری مهارت تکیه دارند و هنوز فقط از طریق `POST /api/v1/skills` قابل انتشار هستند.
...
- `isOfficial` (اختیاری): `true` یا `false`
- `category` (اختیاری): فیلتر دسته‌بندی Plugin. فقط زمانی پشتیبانی می‌شود که
  درخواست به بسته‌های Plugin محدود شده باشد. دسته‌بندی‌های کنترل‌شده و نام‌های مستعار فیلتر قدیمی v1
...
`trending` یک جدول رتبه‌بندی هفت‌روزه نصب/دانلود است و از مجموع‌های تمام‌زمان استفاده نمی‌کند.
در نقطه پایانی یکپارچه `/api/v1/packages` فقط مخصوص Plugin است؛ برای کاتالوگ مهارت از
...
خروجی‌گیری انبوه از آخرین مهارت‌های عمومی برای تحلیل آفلاین.
...
- `startDate` (الزامی): کران پایین برحسب میلی‌ثانیه Unix برای `updatedAt` مهارت.
- `endDate` (الزامی): کران بالا برحسب میلی‌ثانیه Unix برای `updatedAt` مهارت.
...
- بدنه: آرشیو ZIP.
- ریشه هر مهارت خروجی‌گرفته‌شده در `{publisher}/{slug}/` قرار دارد.
- مهارت‌های میزبانی‌شده شامل آخرین فایل‌های نسخه ذخیره‌شده هستند و در
  `_manifest.json` با `sourceRef: "public-clawhub"` فهرست می‌شوند.
- مهارت‌های فعلی مبتنی بر GitHub با اسکن `clean` یا `suspicious` شامل
...
- زمانی که مهارت‌ها یا فایل‌های جداگانه قابل
...
- `startDate` (ضروری): کران پایین بر حسب میلی‌ثانیه Unix برای `updatedAt` مربوط به Plugin.
- `endDate` (ضروری): کران بالا بر حسب میلی‌ثانیه Unix برای `updatedAt` مربوط به Plugin.
- `limit` (اختیاری): عدد صحیح (1-250)، پیش‌فرض `250`.
- `cursor` (اختیاری): نشانگر صفحه‌بندی از پاسخ قبلی.
...
- نام‌های مستعار فیلتر قدیمی v1 که زیر `GET /api/v1/plugins` مستند شده‌اند نیز
  پذیرفته می‌شوند.
- فیلتر دسته‌بندی یک فیلتر واقعی API است که توسط ردیف‌های چکیده دسته‌بندی Plugin
  پشتیبانی می‌شود، نه بازنویسی پرس‌وجوی جست‌وجو.
- نتایج به ترتیب مرتبط‌بودن برگردانده می‌شوند و در حال حاضر صفحه‌بندی نمی‌شوند.
- کنترل‌های مرتب‌سازی رابط کاربری مرورگر برای جست‌وجوی Plugin نتایج مرتبط‌بودن بارگذاری‌شده را
  بازمرتب می‌کنند و با رفتار مرور فعلی `/skills` مطابقت دارند.
...
- Skills همچنین می‌تواند از طریق این مسیر در کاتالوگ یکپارچه resolve شود.
...
- `version.artifact.kind` برای آرشیوهای بسته قدیمی `legacy-zip` یا
  برای انتشارهای پشتیبانی‌شده توسط ClawPack برابر `npm-pack` است.
...
انتشار resolve‌شده قابل نصب است یا نه.
...
- `release.artifactKind`، `release.artifactSha256`، `release.npmIntegrity`،
  ...
  مصنوع انتشار شناخته‌شده باشند وجود دارند.
- `trust.scanStatus` وضعیت اعتماد مؤثر است که از ورودی‌های اسکنر
  و نظارت دستی انتشار مشتق شده است.
- `trust.moderationState` می‌تواند nullable باشد. وقتی نظارت دستی انتشار
...
- `trust.stale` یعنی خلاصه اعتماد از ورودی‌های قدیمی محاسبه شده و
  پیش از یک تصمیم اجازه با اطمینان بالا باید نیازمند تازه‌سازی تلقی شود.

یادداشت‌ها:

- این نقطه پایانی دقیقاً مختص نسخه است. کلاینت‌ها باید آن را پس از resolve کردن
  نسخه بسته‌ای که قصد نصبش را دارند فراخوانی کنند، نه فقط پس از خواندن آخرین
  فراداده بسته.
- بسته‌های خصوصی `404` برمی‌گردانند مگر اینکه فراخواننده بتواند ناشر مالک را بخواند.
- این نقطه پایانی عمداً محدودتر از نقطه‌های پایانی نظارت مالک/ناظر است.
  تصمیم نصب و توضیح عمومی را افشا می‌کند، نه
  هویت گزارش‌دهندگان، متن گزارش‌ها، شواهد خصوصی، یا
  جدول‌های زمانی بازبینی داخلی.

### `GET /api/v1/packages/{name}/versions/{version}/artifact`

فراداده resolver صریح مصنوع را برای یک نسخه بسته برمی‌گرداند.
...
مصنوع نسخه را از مسیر صریح resolver دانلود می‌کند.
...
نقطهٔ پایانی مدیر ناظر/ادمین برای دریافت گزارش‌های بسته.

احراز هویت:

- به یک توکن API برای کاربر مدیر ناظر یا ادمین نیاز دارد.
...
نقطهٔ پایانی مالک/مدیر ناظر برای مشاهده‌پذیری نظارت بسته.

احراز هویت:

- به یک توکن API برای مالک بسته، عضو ناشر، مدیر ناظر، یا
...
نقطهٔ پایانی مدیر ناظر/ادمین برای حل‌وفصل یا بازگشایی گزارش‌های بسته.
...
نقطهٔ پایانی مدیر ناظر/ادمین برای بازبینی انتشار بسته.
...
- بسته‌های خصوصی `404` برمی‌گردانند، مگر آن‌که فراخواننده بتواند ناشر مالک را بخواند.
...
- بسته‌های خصوصی `404` برمی‌گردانند، مگر آن‌که فراخواننده مالک باشد.
...
- فقط نسخه‌هایی فهرست می‌شوند که tarballهای npm-pack مربوط به ClawPack آن‌ها آپلود شده باشد.
...
بایت‌های دقیق tarball آپلودشدهٔ ClawPack را برای کلاینت‌های آینهٔ npm استریم می‌کند.

نکات:

- از سطل نرخ دانلود استفاده می‌کند.
- هدرهای دانلود شامل SHA-256 مربوط به ClawHub به‌علاوهٔ فرادادهٔ integrity/shasum مربوط به npm هستند.
...
یک ZIP نسخهٔ Skills میزبانی‌شده را دانلود می‌کند، یا برای یک Skills فعلیِ پشتیبانی‌شده با GitHub که
اسکن `clean` یا `suspicious` دارد و نسخهٔ میزبانی‌شده ندارد، واگذاری منبع GitHub برمی‌گرداند.
...
- واگذاری‌های Skills پشتیبانی‌شده با GitHub بایت‌ها را پروکسی یا آینه نمی‌کنند. پاسخ JSON
  ...
  گنجانده نمی‌شود.
- آمار دانلود به‌صورت هویت‌های یکتا به‌ازای هر روز UTC شمرده می‌شود (`userId` وقتی توکن API معتبر است، در غیر این صورت IP).
...
- فیلد اختیاری payload: `ownerHandle`. در صورت وجود، API آن
  ناشر را در سمت سرور resolve می‌کند و actor باید دسترسی ناشر داشته باشد.
- فیلد اختیاری payload: `migrateOwner`. وقتی همراه با `ownerHandle` برابر `true` باشد،
  یک Skills موجود می‌تواند به آن مالک منتقل شود، اگر actor روی هر دو ناشر
  فعلی و هدف ادمین/مالک باشد. بدون این opt-in، تغییرات مالک رد می‌شوند.
...
- فیلدهای مجاز فرم `payload`، blobهای تکراری `files`، یا یک مرجع tarball
  به نام `clawpack` هستند. `clawpack` می‌تواند یک blob با پسوند `.tgz` یا یک storage id برگردانده‌شده توسط
  جریان upload-url باشد. انتشارهای storage-id مرحله‌بندی‌شده باید
  `clawpackUploadTicket` برگردانده‌شده همراه با آن URL آپلود را نیز شامل شوند.
...
حذف نرم / بازیابی یک Skills (مالک، مدیر ناظر، یا ادمین).
...
در صورت وجود، `reason` به‌عنوان یادداشت نظارت Skills ذخیره و در لاگ حسابرسی کپی می‌شود.
حذف‌های نرم آغازشده توسط مالک، slug را برای ۳۰ روز رزرو می‌کنند، سپس slug می‌تواند توسط
ناشر دیگری claim شود. پاسخ حذف وقتی این انقضا اعمال شود شامل `slugReservedUntil` است.
پنهان‌سازی‌های مدیر ناظر/ادمین و حذف‌های امنیتی به این شکل منقضی نمی‌شوند.
...
فقط ادمین. تضمین می‌کند یک ناشر org برای یک handle وجود داشته باشد. اگر handle هنوز به یک
...
ایجاد ناشر org سلف‌سرویسِ احرازهویت‌شده. یک ناشر org جدید ایجاد می‌کند و
فراخواننده را به‌عنوان مالک اضافه می‌کند. این نقطهٔ پایانی handleهای کاربری/شخصی موجود را مهاجرت نمی‌دهد و
...
فقط ادمین. slugهای ریشه و نام‌های بسته را برای مالک محق، بدون انتشار یک
...
فقط ادمین. یک ناشر شخصی را برای principal جایگزینِ تأییدشدهٔ GitHub OAuth
بدون ویرایش ردیف‌های حساب Convex Auth بازیابی می‌کند. درخواست باید هر دو شناسهٔ تغییرناپذیر حساب
provider مربوط به GitHub را نام ببرد؛ handleهای تغییرپذیر فقط به‌عنوان محافظ رو‌به‌روی اپراتور استفاده می‌شوند.

نقطهٔ پایانی به‌صورت پیش‌فرض در حالت اجرای آزمایشی است. اعمال بازیابی پس از اینکه کارکنان پیوستگی بین هر دو هویت اصلی GitHub را به‌طور مستقل تأیید کردند، به `dryRun: false` و
`confirmIdentityVerified: true` نیاز دارد. اگر ناشر شخصی فعلی کاربر مقصد Skills، بسته‌ها، یا منابع Skill در GitHub داشته باشد، بازیابی با حالت بسته شکست می‌خورد.
بازیابی همچنین فیلدهای قدیمی `ownerUserId` را برای Skills ناشر بازیابی‌شده،
نام‌های مستعار اسلاگ Skill، بسته‌ها، هشدارهای بازرس بسته، و ردیف‌های مشتق‌شدهٔ چکیدهٔ جست‌وجو مهاجرت می‌دهد تا
...
یک کاربر را مسدود می‌کند و Skills تحت مالکیت او را به‌طور قطعی حذف می‌کند (فقط مدیر ناظر/مدیر).
...
مسدودی یک کاربر را لغو می‌کند و Skills واجد شرایط را بازیابی می‌کند (فقط مدیر).
...
دلیل ذخیره‌شده برای یک مسدودی موجود را بدون لغو مسدودی یا بازیابی
محتوا تغییر می‌دهد (فقط مدیر). مگر اینکه `dryRun` برابر `false` باشد، پیش‌فرض اجرای آزمایشی است.
...
یک ستاره (برجسته‌سازی) را اضافه/حذف می‌کند. هر دو نقطهٔ پایانی نسبت به اجرای تکراری بی‌اثر هستند.
