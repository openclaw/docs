...
x-i18n:
    generated_at: "2026-07-04T18:10:56Z"
    model: gpt-5.5
...
دایرکتوری‌های شخص ثالث می‌توانند از endpointهای خواندن عمومی برای فهرست‌کردن یا جست‌وجوی مهارت‌های ClawHub استفاده کنند. لطفاً نتایج را cache کنید، `429`/`Retry-After` را رعایت کنید، کاربران را به فهرست canonical ClawHub (`https://clawhub.ai/<owner>/skills/<slug>`) برگردانید، و از القای تأیید سایت شخص ثالث توسط ClawHub خودداری کنید. تلاش نکنید محتوای پنهان، خصوصی، یا مسدودشده توسط moderation را خارج از سطح API عمومی mirror کنید.

میانبرهای slug وب در سراسر خانواده‌های registry resolve می‌شوند، اما کلاینت‌های API باید به‌جای بازسازی تقدم route، از URLهای canonical بازگردانده‌شده توسط endpointهای خواندن استفاده کنند.
...
- اگر token وجود نداشته باشد/نامعتبر باشد، رفتار به اعمال بر پایهٔ IP برمی‌گردد.
- endpointهای نوشتن احرازهویت‌شده وقتی سرور دلیل را می‌داند نباید یک `Unauthorized` تنها برگردانند. tokenهای غایب، tokenهای نامعتبر/لغوشده، و حساب‌های حذف‌شده/ممنوع‌شده/غیرفعال‌شده باید هرکدام متن قابل‌اقدامی دریافت کنند تا کلاینت‌های CLI بتوانند به کاربران بگویند چه چیزی آن‌ها را مسدود کرده است.
...
- اگر `Retry-After` غایب است، به `RateLimit-Reset` برگردید (یا از `X-RateLimit-Reset` محاسبه کنید).

منبع IP:

- فقط زمانی از headerهای IP کلاینت مورداعتماد، از جمله `cf-connecting-ip`، استفاده می‌کند که deployment به‌طور صریح headerهای forwarded مورداعتماد را فعال کرده باشد.
- ClawHub از headerهای trusted forwarding برای شناسایی IPهای کلاینت در edge استفاده می‌کند.
- اگر IP کلاینت مورداعتماد در دسترس نباشد، درخواست‌های ناشناس از bucketهای fallback استفاده می‌کنند که فقط بر اساس نوع rate-limit scoped شده‌اند. این bucketهای fallback شامل مسیرها، slugها، نام packageها، versionها، query stringها، یا دیگر پارامترهای artifact ارائه‌شده توسط caller نیستند.

## پاسخ‌های خطا

پاسخ‌های خطای عمومی v1 متن ساده با `content-type: text/plain; charset=utf-8` هستند.
این شامل شکست‌های اعتبارسنجی (`400`)، منابع عمومی غایب (`404`)، شکست‌های auth و permission (`401`/`403`)، محدودیت‌های نرخ (`429`)، و دانلودهای مسدودشده است. کلاینت‌ها باید body پاسخ را به‌عنوان رشته‌ای خوانا برای انسان بخوانند. پارامترهای query ناشناخته برای سازگاری نادیده گرفته می‌شوند، اما پارامترهای query شناخته‌شده با مقادیر نامعتبر `400` برمی‌گردانند.
...
- `highlightedOnly` (اختیاری): `true` برای filter کردن به مهارت‌های برجسته‌شده
...
- نتایج به‌ترتیب relevance برگردانده می‌شوند (embedding similarity + boostهای token دقیق slug/name + یک prior کوچک محبوبیت).
- relevance از محبوبیت قوی‌تر است. یک match دقیق token در slug یا display-name می‌تواند از match شل‌تر با engagement بسیار قوی‌تر بالاتر قرار بگیرد.
- متن ASCII بر اساس مرزهای word و punctuation به token تبدیل می‌شود. برای مثال، `personal-map` شامل token مستقل `map` است، در حالی که `amap-jsapi-skill` شامل `amap`، `jsapi`، و `skill` است؛ بنابراین جست‌وجوی `map` به `personal-map` match واژگانی قوی‌تری نسبت به `amap-jsapi-skill` می‌دهد.
- محبوبیت log-scaled و capped است. مهارت‌های با engagement بالا وقتی متن query match ضعیف‌تری باشد می‌توانند رتبهٔ پایین‌تری بگیرند.
- وضعیت moderation مشکوک یا پنهان می‌تواند بسته به filterهای caller و وضعیت moderation فعلی، یک مهارت را از جست‌وجوی عمومی حذف کند.

راهنمای discoverability ناشر:

- اصطلاحاتی را که کاربران دقیقاً جست‌وجو خواهند کرد در display name، summary، و tagها بگذارید. فقط زمانی از token مستقل slug استفاده کنید که آن نیز یک identity پایدار باشد که می‌خواهید نگه دارید.
- صرفاً برای دنبال‌کردن یک query، slug را rename نکنید مگر اینکه slug جدید نام canonical بلندمدت بهتری باشد. slugهای قدیمی به redirect alias تبدیل می‌شوند، اما URL canonical، slug نمایش‌داده‌شده، و digestهای جست‌وجوی آینده از slug جدید استفاده می‌کنند.
- aliasهای rename برای URLهای قدیمی و installهایی که از طریق registry resolve می‌شوند resolution را حفظ می‌کنند، اما رتبه‌بندی جست‌وجو پس از index شدن rename بر اساس metadata canonical مهارت است. stats موجود با مهارت باقی می‌مانند.
- اگر مهارتی به‌طور غیرمنتظره نامرئی است، پیش از تغییر metadata مرتبط با رتبه‌بندی، ابتدا در حالت logged in با `clawhub inspect @owner/slug` وضعیت moderation را بررسی کنید.
...
مقادیر نامعتبر `sort` پاسخ `400` برمی‌گردانند.
...
- وقتی `nonSuspiciousOnly=true` باشد، sortهای cursor-based ممکن است در یک صفحه کمتر از `limit` آیتم برگردانند، چون مهارت‌های مشکوک پس از page retrieval filter می‌شوند.
...
- callerهای عمومی فقط برای مهارت‌های قابل‌مشاهده‌ای که قبلاً flagged شده‌اند `200` دریافت می‌کنند.
- evidence برای callerهای عمومی redacted است و فقط برای مالکان/moderatorها raw snippetها را شامل می‌شود.

### `POST /api/v1/skills/{slug}/report`

گزارش یک مهارت برای review توسط moderator. گزارش‌ها در سطح مهارت هستند، به‌صورت اختیاری به یک version لینک می‌شوند، و صف گزارش مهارت را تغذیه می‌کنند.
...
endpoint مدیر/moderator برای دریافت گزارش‌های مهارت.
...
endpoint مدیر/moderator برای حل‌وفصل یا reopen کردن گزارش‌های مهارت.
...
`note` برای `confirmed` و `dismissed` الزامی است؛ هنگام برگرداندن `status` به `open` می‌تواند حذف شود. برای پنهان‌کردن مهارت در همان workflow قابل audit، همراه با یک گزارش triaged مقدار `finalAction: "hide"` را ارسال کنید.
...
- هنگام پرس‌وجوی یک نسخه تاریخی، پیش از اینکه `moderation` و `security` را در یک زمینه نسخه‌ای یکسان در نظر بگیرید، `moderation.matchesRequestedVersion` و `moderation.sourceVersion` را بررسی کنید.
...
- محموله‌های درخواست اسکن و گزارش‌های قابل دانلود پس از پنجره نگه‌داری از ذخیره‌گاه درخواست اسکن منقضی می‌شوند.
...
- کارهای اسکن ناهمگام هستند. درخواست‌های اسکن دستی جلوتر از کارهای عادی انتشار/بازپرکنی اولویت‌بندی می‌شوند، اما تکمیل همچنان به در دسترس بودن worker بستگی دارد.
...
- هنگام در صف بودن، `queue.queuedAhead` و `queue.position` را برمی‌گرداند تا کلاینت‌ها بتوانند نشان دهند چند اسکن دستی اولویت‌دار جلوتر از این درخواست قرار دارند. صف‌های بسیار بزرگ محدود می‌شوند و با `queuedAheadIsEstimate: true` گزارش می‌شوند.
...
- به دسترسی مدیریت مالک/ناشر به مهارت یا Plugin، یا اختیار ناظر/مدیر پلتفرم نیاز دارد.
- نتایج اسکن ذخیره‌شده را برای نسخه دقیق ارسال‌شده، از جمله نسخه‌های مسدودشده یا پنهان، برمی‌گرداند.
...
مسیر بازاسکن دسته‌ای کانونیکال فقط برای مدیر. همان شکل محموله مسیر قدیمی `POST /api/v1/skills/-/rescan-batch` را می‌پذیرد.
...
- `ok` فقط زمانی `true` است که نسخه انتخاب‌شده یک کارت مهارت تولیدشده داشته باشد، توسط تعدیل به‌عنوان بدافزار مسدود نشده باشد، و راستی‌آزمایی ClawScan پاک باشد.
- هویت مهارت، هویت ناشر و فراداده نسخه انتخاب‌شده فیلدهای سطح بالای پاکت هستند (`slug`، `displayName`، `publisherHandle`، `version`، `resolvedFrom`، `tag`، `createdAt`) تا خودکارسازی shell بتواند آن‌ها را بدون باز کردن wrapperهای تو در تو بخواند.
...
- پاسخ فقط امنیتی است. داده کارت مهارت، وضعیت کارت تولیدشده، فهرست فایل‌های artifact، یا محموله‌های تفصیلی اسکنر را شامل نمی‌شود.
...
- `security.signals.dependencyRegistry` برای سازگاری پاسخ v1 حفظ شده است، اما اسکنر وجود رجیستری وابستگی بازنشسته شده و این کلید همیشه `null` است.
- نبود کارت مهارت بر `ok`، `decision` یا `reasons` این نقطه پایانی تأثیری ندارد؛ کلاینت‌ها وقتی به محتوای کارت نیاز دارند باید `skill-card.md` نصب‌شده را به‌صورت محلی بخوانند.
- وقتی به پاکت راستی‌آزمایی کارت مهارت تک‌مهارته نیاز دارید از `/verify` استفاده کنید، وقتی به markdown کارت تولیدشده نیاز دارید از `/card`، و وقتی به داده تفصیلی اسکنر نیاز دارید از `/scan`.
...
- `category` (اختیاری): فیلتر دسته‌بندی plugin. فقط زمانی پشتیبانی می‌شود که
  ...
  نام‌های مستعار قدیمی فیلتر v1 ذیل `GET /api/v1/plugins` مستند شده‌اند.
...
- `channel=private` فقط بسته‌هایی را برمی‌گرداند که فراخواننده احراز هویت‌شده اجازه خواندنشان را دارد.
...
- `category` (اختیاری): فیلتر دسته‌بندی plugin. فقط زمانی پشتیبانی می‌شود که
  ...
  فیلتر v1 ذیل `GET /api/v1/plugins` مستند شده‌اند.
...
`trending` یک جدول رتبه‌بندی نصب/دانلود هفت‌روزه است و از مجموع‌های همیشگی استفاده نمی‌کند.
...
- `startDate` (الزامی): کران پایین بر حسب میلی‌ثانیه یونیکس برای `updatedAt` مربوط به skill.
- `endDate` (الزامی): کران بالا بر حسب میلی‌ثانیه یونیکس برای `updatedAt` مربوط به skill.
...
- `endDate` (الزامی): کران بالای بر حسب میلی‌ثانیه یونیکس برای `updatedAt` در Plugin.
- `limit` (اختیاری): عدد صحیح (1-250)، پیش‌فرض `250`.
- `cursor` (اختیاری): نشانگر صفحه‌بندی از پاسخ قبلی.
...
- بدنه: آرشیو ZIP.
- ریشهٔ هر Plugin خروجی‌گرفته‌شده در `{family}/{packageName}/` است.
...
- وقتی Pluginها یا فایل‌های جداگانه قابل خروجی‌گیری نبوده‌اند، `_errors.json`
  گنجانده می‌شود.
...
- نام‌های مستعار فیلتر قدیمی v1 که زیر `GET /api/v1/plugins` مستند شده‌اند نیز
  پذیرفته می‌شوند.
- فیلتر کردن دسته‌بندی یک فیلتر واقعی API است که با ردیف‌های خلاصهٔ دسته‌بندی
...
- کنترل‌های مرتب‌سازی رابط کاربری مرورگر برای جست‌وجوی Plugin، نتایج ارتباط بارگذاری‌شده را
  دوباره مرتب می‌کنند و با رفتار فعلی مرور `/skills` همخوان هستند.
...
تأیید، فرادادهٔ artifact، و داده‌های اسکن، برمی‌گرداند.

نکته‌ها:

- `version.artifact.kind` برای آرشیوهای بستهٔ دنیای قدیم `legacy-zip` است یا
...
- `version.vtAnalysis`، `version.llmAnalysis`، و `version.staticScan` وقتی
  دادهٔ اسکن وجود داشته باشد گنجانده می‌شوند.
- بسته‌های خصوصی `404` برمی‌گردانند مگر اینکه فراخواننده بتواند ناشر مالک را بخواند.
...
resolve‌شده قابل نصب است یا نه محسوب می‌شود.

احراز هویت:

- نقطهٔ پایانی خواندن عمومی. توکن مالک، ناشر، ناظر، یا ادمین
  لازم نیست.
...
- `trust.moderationState` می‌تواند تهی باشد. وقتی نظارت دستی انتشار
...
- `trust.reasons` فهرست توضیح کاربرنما و حسابرسی است. کدهای دلیل
...
- این نقطهٔ پایانی عمداً محدودتر از نقطه‌های پایانی نظارت مالک/ناظر است.
  این نقطه تصمیم نصب و توضیح عمومی را افشا می‌کند، نه هویت گزارش‌دهندگان،
...
- نسخه‌های بستهٔ قدیمی یک artifact از نوع `legacy-zip` و یک `downloadUrl`
  قدیمی ZIP برمی‌گردانند.
...
- نسخه‌های ClawPack بایت‌های دقیق `.tgz` آپلودشدهٔ npm-pack را stream می‌کنند.
...
نقطهٔ پایانی ادمین برای ایجاد یا به‌روزرسانی یک ردیف مهاجرت Plugin رسمی.
...
نقطهٔ پایانی ناظر/ادمین برای صف‌های بازبینی انتشار بسته.
...
- `open`: انتشارهای مشکوک، مخرب، معلق، قرنطینه‌شده، لغوشده، یا گزارش‌شده.
...
به یک نسخه پیوند می‌خورند. آن‌ها صف نظارت را تغذیه می‌کنند اما خودشان به‌تنهایی
...
نقطهٔ پایانی مالک/ناظر برای مشاهده‌پذیری تعدیل بسته.
...
- پیش‌فرض، آخرین انتشار است.
- Skills به `GET /api/v1/download` تغییرمسیر می‌دهد.
...
- بسته‌های خصوصی `404` برمی‌گردانند، مگر اینکه فراخوان مالک باشد.
...
- packumentهای بستهٔ scoped هم از مسیر درخواست `/api/npm/@scope/name` و هم از مسیر
...
- `slug` (الزامی)
- `hash` (الزامی): sha256 شانزدهی 64 نویسه‌ای از اثرانگشت bundle
...
با اسکن `clean` یا `suspicious` و بدون نسخهٔ میزبانی‌شده، تحویل‌سپاری منبع GitHub را برمی‌گرداند.
...
- تحویل‌سپاری‌های skill مبتنی بر GitHub بایت‌ها را پراکسی یا mirror نمی‌کنند. پاسخ JSON
...
- فیلد اختیاری payload: `migrateOwner`. وقتی با `ownerHandle` مقدار `true` باشد، یک
...
- فیلدهای فرم مجاز عبارت‌اند از `payload`، blobهای تکرارشوندهٔ `files`، یا یک ارجاع tarball
  `clawpack`. `clawpack` می‌تواند یک blob با پسوند `.tgz` یا یک id ذخیره‌سازی باشد که توسط
  جریان upload-url برگردانده شده است. انتشارهای مرحله‌بندی‌شده با storage-id باید همچنین
  `clawpackUploadTicket` برگردانده‌شده همراه آن URL بارگذاری را شامل شوند.
- از `files` یا `clawpack` استفاده کنید، هرگز هر دو را در یک درخواست یکسان نه.
- بدنه‌های JSON و فرادادهٔ `payload.files` / `payload.artifact`
  ارائه‌شده توسط فراخوان رد می‌شوند.
- درخواست‌های انتشار multipart مستقیم به 18MB محدود می‌شوند. tarballهای ClawPack ممکن است
...
- انتشارهای از طرف دیگران همچنان صلاحیت کانال رسمی را در برابر حساب مالک هدف اعتبارسنجی می‌کنند.
...
ناشر کاربر/شخصی مشترک قدیمی اشاره کند، نقطهٔ پایانی ابتدا آن را به یک ناشر org مهاجرت می‌دهد.
برای یک org تازه ایجادشده، `memberHandle` را ارائه کنید؛ مدیر در حال اقدام به‌عنوان عضو اضافه نمی‌شود.
...
فراخوان را به‌عنوان مالک اضافه می‌کند. این نقطهٔ پایانی handleهای کاربر/شخصی موجود را مهاجرت نمی‌دهد و
...
فقط مدیر. یک ناشر شخصی را برای اصل GitHub OAuth جایگزین و تأییدشده
بدون ویرایش ردیف‌های حساب Convex Auth بازیابی می‌کند. درخواست باید هر دو شناسهٔ تغییرناپذیر حساب
ارائه‌دهندهٔ GitHub را نام ببرد؛ handleهای تغییرپذیر فقط به‌عنوان گارد روبه‌روی اپراتور استفاده می‌شوند.

نقطه پایانی به‌طور پیش‌فرض روی اجرای آزمایشی تنظیم است. اعمال بازیابی پس از اینکه کارکنان به‌طور مستقل پیوستگی بین هر دو
اصل GitHub را تأیید کردند، به `dryRun: false` و
`confirmIdentityVerified: true` نیاز دارد. اگر ناشر شخصی فعلی کاربر مقصد
مهارت‌ها، بسته‌ها، یا منابع مهارت GitHub داشته باشد، بازیابی به‌صورت بسته شکست می‌خورد.
بازیابی همچنین فیلدهای قدیمی `ownerUserId` را برای مهارت‌های ناشر بازیابی‌شده،
نام‌های مستعار slug مهارت، بسته‌ها، هشدارهای بازرس بسته، و ردیف‌های digest جست‌وجوی مشتق‌شده مهاجرت می‌دهد تا
مسیرهای مالک مستقیم با مرجعیت ناشر جدید هم‌خوان شوند. یک رزرو فعال handle محافظت‌شده
برای handle بازیابی‌شده نیز به کاربر جایگزین واگذار می‌شود تا همگام‌سازی بعدی
پروفایل نتواند مرجعیت رقیب کاربر پیشین را بازگرداند. هر جدول اصلی در هر تراکنش اعمال به
...
- `rename`، slug پیشین را به‌عنوان نام مستعار تغییرمسیر حفظ می‌کند.
- `merge`، فهرست مبدا را پنهان می‌کند و slug مبدا را به فهرست مقصد تغییرمسیر می‌دهد.
...
یک کاربر را مسدود می‌کند و مهارت‌های تحت مالکیت او را به‌صورت حذف سخت پاک می‌کند (فقط moderator/admin).
...
مسدودیت یک کاربر را برمی‌دارد و مهارت‌های واجد شرایط را بازیابی می‌کند (فقط admin).
...
محتوا تغییر می‌دهد (فقط admin). مگر اینکه `dryRun` برابر با `false` باشد، به‌طور پیش‌فرض اجرای آزمایشی است.
...
نقش یک کاربر را تغییر می‌دهد (فقط admin).
...
کاربران را فهرست می‌کند یا جست‌وجو می‌کند (فقط admin).
...
که یک tarball مربوط به ClawPack را مرحله‌بندی می‌کنند، باید شناسه storage حاصل را به‌عنوان
