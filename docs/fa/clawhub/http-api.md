...
میانبرهای slug وب در میان خانواده‌های registry resolve می‌شوند، اما کلاینت‌های API باید به‌جای بازسازی تقدم route، از URLهای canonical برگشتی توسط endpointهای خواندن استفاده کنند.
...
- درخواست‌های احراز هویت‌شده (توکن Bearer معتبر): برای هر bucket کاربر اعمال می‌شود.
- اگر توکن موجود نباشد یا نامعتبر باشد، رفتار به اعمال محدودیت بر اساس IP برمی‌گردد.
- endpointهای نوشتن احراز هویت‌شده، وقتی سرور دلیل را می‌داند، نباید یک `Unauthorized` خالی برگردانند. توکن‌های گم‌شده، توکن‌های نامعتبر/باطل‌شده، و حساب‌های حذف‌شده/مسدودشده/غیرفعال‌شده باید هرکدام متن قابل‌اقدامی دریافت کنند تا کلاینت‌های CLI بتوانند به کاربران بگویند چه چیزی جلوی آن‌ها را گرفته است.

- خواندن: 3000/min برای هر IP، ‏12000/min برای هر کلید
- نوشتن: 300/min برای هر IP، ‏3000/min برای هر کلید
- دانلود: 1200/min برای هر IP، ‏6000/min برای هر کلید (endpointهای دانلود)
...
- فقط وقتی deployment صراحتاً headerهای forwarded مورداعتماد را فعال کند، از headerهای IP کلاینت مورداعتماد، از جمله `cf-connecting-ip`، استفاده می‌کند.
- ClawHub از headerهای forwarding مورداعتماد برای شناسایی IPهای کلاینت در edge استفاده می‌کند.
- اگر IP کلاینت مورداعتمادی در دسترس نباشد، درخواست‌های ناشناس از bucketهای fallback استفاده می‌کنند که فقط بر اساس نوع محدودیت نرخ scope شده‌اند. این bucketهای fallback شامل مسیرها، slugها، نام packageها، نسخه‌ها، query stringها، یا دیگر پارامترهای artifact ارائه‌شده توسط caller نمی‌شوند.
...
- `highlightedOnly` (اختیاری): `true` برای فیلتر کردن به مهارت‌های highlighted
- `nonSuspiciousOnly` (اختیاری): `true` برای پنهان کردن مهارت‌های suspicious (`flagged.suspicious`)
...
- نتایج به ترتیب مرتبط بودن برگردانده می‌شوند (شباهت embedding + تقویت tokenهای دقیق slug/name + یک prior کوچک محبوبیت).
- مرتبط بودن از محبوبیت قوی‌تر است. تطابق دقیق slug یا token نام نمایشی می‌تواند از تطابق آزادتر با engagement بسیار قوی‌تر رتبهٔ بالاتری بگیرد.
- متن ASCII بر اساس مرزهای واژه و punctuation به token تبدیل می‌شود. برای نمونه، `personal-map` شامل یک token مستقل `map` است، در حالی که `amap-jsapi-skill` شامل `amap`،‏ `jsapi`، و `skill` است؛ بنابراین جست‌وجوی `map` به `personal-map` تطابق واژگانی قوی‌تری نسبت به `amap-jsapi-skill` می‌دهد.
- محبوبیت به‌صورت لگاریتمی scale و cap می‌شود. مهارت‌های با engagement بالا وقتی متن query تطابق ضعیف‌تری دارد، می‌توانند رتبهٔ پایین‌تری بگیرند.
...
- اصطلاحاتی را که کاربران واقعاً جست‌وجو می‌کنند در نام نمایشی، summary، و tagها بگذارید. فقط وقتی از یک token مستقل slug استفاده کنید که همان هم هویت پایداری باشد که می‌خواهید نگه دارید.
...
- aliasهای تغییر نام، resolve شدن URLهای قدیمی و نصب‌هایی را که از طریق registry resolve می‌شوند حفظ می‌کنند، اما رتبه‌بندی جست‌وجو پس از index شدن تغییر نام، بر اساس metadata canonical مهارت است. آمارهای موجود با مهارت باقی می‌مانند.
- اگر یک مهارت به‌طور غیرمنتظره نامرئی است، پیش از تغییر metadata مرتبط با رتبه‌بندی، ابتدا هنگام ورود به سیستم وضعیت moderation را با `clawhub inspect @owner/slug` بررسی کنید.
...
- وقتی `nonSuspiciousOnly=true` است، sortهای مبتنی بر cursor ممکن است در یک صفحه کمتر از `limit` item برگردانند، چون مهارت‌های suspicious پس از بازیابی صفحه فیلتر می‌شوند.
...
- `moderation` فقط وقتی مهارت flagged شده باشد یا مالک در حال مشاهدهٔ آن باشد گنجانده می‌شود.
...
یک مهارت را برای بازبینی moderator گزارش کنید. گزارش‌ها در سطح مهارت هستند، می‌توانند به‌صورت اختیاری به یک نسخه لینک شوند، و queue گزارش مهارت را تغذیه می‌کنند.
...
endpoint مخصوص moderator/admin برای resolve کردن یا باز کردن دوبارهٔ گزارش‌های مهارت.
...
`note` برای `confirmed` و `dismissed` الزامی است؛ هنگام تنظیم دوبارهٔ `status` به `open` می‌توان آن را حذف کرد. برای پنهان کردن مهارت در همان workflow قابل audit، `finalAction: "hide"` را همراه با یک گزارش triaged ارسال کنید.
...
جزئیات تأیید scan امنیتی برای یک نسخهٔ مهارت را برمی‌گرداند.

پارامترهای query:

- `version` (اختیاری): رشتهٔ نسخهٔ مشخص.
- `tag` (اختیاری): یک نسخهٔ tagged را resolve می‌کند (برای مثال `latest`).
...
- هنگام پرس‌وجوی یک نسخه تاریخی، پیش از اینکه `moderation` و `security` را متعلق به همان بافت نسخه بدانید، `moderation.matchesRequestedVersion` و `moderation.sourceVersion` را بررسی کنید.
...
- بارهای درخواست اسکن و گزارش‌های قابل دانلود پس از بازه نگه‌داری از مخزن درخواست اسکن منقضی می‌شوند.
- اسکن‌های منتشرشده به دسترسی مدیریتی مالک/ناشر، یا اختیار مدیر محتوا/مدیر سکوی پلتفرم نیاز دارند.
...
- کارهای اسکن ناهمگام هستند. درخواست‌های اسکن دستی جلوتر از کارهای معمول انتشار/پرکردن عقب‌افتاده اولویت‌بندی می‌شوند، اما تکمیل همچنان به در دسترس بودن worker وابسته است.
...
- هنگام در صف بودن، `queue.queuedAhead` و `queue.position` را برمی‌گرداند تا کلاینت‌ها بتوانند نشان دهند چه تعداد اسکن دستی اولویت‌دار جلوتر از درخواست هستند. صف‌های بسیار بزرگ محدود می‌شوند و با `queuedAheadIsEstimate: true` گزارش می‌شوند.
...
- به دسترسی مدیریتی مالک/ناشر به skill یا plugin، یا اختیار مدیر محتوا/مدیر سکوی پلتفرم نیاز دارد.
...
مسیر بازاسکن دسته‌ای canonical فقط برای مدیر. همان شکل بار داده legacy `POST /api/v1/skills/-/rescan-batch` را می‌پذیرد.
...
پوش Skill Card verification را که توسط `clawhub skill verify` استفاده می‌شود برمی‌گرداند.
...
- هویت skill، هویت ناشر، و فراداده نسخه انتخاب‌شده، فیلدهای سطح بالای پوش هستند (`slug`، `displayName`، `publisherHandle`، `version`، `resolvedFrom`، `tag`، `createdAt`) تا خودکارسازی shell بتواند آن‌ها را بدون باز کردن wrapperهای تو در تو بخواند.
...
نقطه پایانی مجموعه برای کلاینت‌هایی در نظر گرفته شده که از قبل می‌دانند کدام نسخه‌های skill نصب‌شده از
...
- پاسخ فقط امنیتی است. داده Skill Card، وضعیت کارت تولیدشده، فهرست‌های فایل artifact، یا بارهای داده تفصیلی اسکنر را شامل نمی‌شود.
...
- وقتی به پوش verification تک-skill مربوط به Skill Card نیاز دارید از `/verify` استفاده کنید، وقتی به markdown کارت تولیدشده نیاز دارید از `/card` استفاده کنید، و وقتی به داده تفصیلی اسکنر نیاز دارید از `/scan` استفاده کنید.
...
- ورودی‌های Skill همچنان بر رجیستری Skill متکی هستند و هنوز فقط از طریق `POST /api/v1/skills` قابل انتشارند.
...
- فراخوان‌های احراز هویت‌شده می‌توانند در نتایج فهرست/جست‌وجو، بسته‌های خصوصی ناشرانی را ببینند که به آن‌ها تعلق دارند.
...
- فراخوان‌های ناشناس فقط کانال‌های بسته عمومی را می‌بینند.
- فراخوان‌های احراز هویت‌شده می‌توانند بسته‌های خصوصی ناشرانی را جست‌وجو کنند که به آن‌ها تعلق دارند.
- `channel=private` فقط بسته‌هایی را برمی‌گرداند که فراخوان احراز هویت‌شده می‌تواند بخواند.
...
- `startDate` (الزامی): حد پایین میلی‌ثانیه‌های Unix برای `updatedAt` مربوط به Skill.
- `endDate` (الزامی): حد بالای میلی‌ثانیه‌های Unix برای `updatedAt` مربوط به Skill.
...
- Skills فعلی با پشتوانه GitHub که اسکن `clean` یا `suspicious` دارند، شامل
...
- `_errors.json` زمانی گنجانده می‌شود که Skillها یا فایل‌های جداگانه قابل
  خروجی گرفتن نبوده باشند.
...
- `startDate` (الزامی): کران پایین بر حسب میلی‌ثانیه Unix برای `updatedAt` در Plugin.
- `endDate` (الزامی): کران بالا بر حسب میلی‌ثانیه Unix برای `updatedAt` در Plugin.
- `limit` (اختیاری): عدد صحیح (1-250)، پیش‌فرض `250`.
...
- کنترل‌های مرتب‌سازی رابط کاربری مرورگر برای جست‌وجوی Plugin نتایج مرتبط بارگذاری‌شده را
...
- `trust.reasons` فهرست توضیحی کاربرمحور و قابل ممیزی است. کدهای دلیل
...
- `trust.stale` یعنی خلاصه اعتماد از ورودی‌های قدیمی محاسبه شده است و
  پیش از تصمیم اجازه‌دهی با اطمینان بالا باید نیازمند تازه‌سازی تلقی شود.
...
- این نقطه پایانی عمداً از نقاط پایانی نظارت مالک/ناظر محدودتر است.
  ...
  هویت گزارش‌دهنده‌ها، بدنه گزارش‌ها، شواهد خصوصی، یا خط زمانی‌های بازبینی داخلی.
...
- نسخه‌های ClawPack همان بایت‌های دقیق `.tgz` بارگذاری‌شده npm-pack را stream می‌کنند.
- نسخه‌های ZIP قدیمی به `/api/v1/packages/{name}/download?version=` هدایت می‌شوند.
- از bucket نرخ دانلود استفاده می‌کند.

### `GET /api/v1/packages/{name}/readiness`

آمادگی محاسبه‌شده را برای مصرف آینده OpenClaw بازمی‌گرداند.

بررسی‌های آمادگی شامل موارد زیر است:

- وضعیت کانال رسمی
- در دسترس بودن آخرین نسخه
- در دسترس بودن artifact نوع npm-pack مربوط به ClawPack
...
- `packageName` طبق نام npm نرمال‌سازی می‌شود؛ بسته می‌تواند برای مهاجرت‌های
...
به یک نسخه پیوند می‌خورند. آن‌ها صف نظارت را تغذیه می‌کنند اما خودشان به‌طور خودکار
...
- نسخه‌های قدیمیِ فقط ZIP عمداً حذف شده‌اند.
...
- packumentهای بسته‌های scoped هم از مسیر درخواست `/api/npm/@scope/name` و هم از مسیر کدگذاری‌شدهٔ npm یعنی `/api/npm/@scope%2Fname` پشتیبانی می‌کنند.
...
ZIP نسخهٔ یک skill میزبانی‌شده را دانلود می‌کند، یا برای یک skill فعلی مبتنی بر GitHub با اسکن `clean` یا `suspicious` و بدون نسخهٔ میزبانی‌شده، واگذاری منبع GitHub را برمی‌گرداند.
...
- واگذاری‌های skill مبتنی بر GitHub بایت‌ها را proxy یا mirror نمی‌کنند. پاسخ JSON شامل `sourceRef: "public-github"`، `repo`، `commit`، `path`، `contentHash` و `archiveUrl` است؛ وضعیت اسکن/فعلی یک gate است و به‌عنوان فرادادهٔ payload موفقیت گنجانده نمی‌شود.
- آمار دانلود به‌صورت هویت‌های یکتا در هر روز UTC شمارش می‌شود (`userId` وقتی توکن API معتبر باشد، در غیر این صورت IP).
...
- فیلد اختیاری payload: `ownerHandle`. وقتی وجود داشته باشد، API آن ناشر را سمت سرور resolve می‌کند و نیاز دارد actor دسترسی ناشر داشته باشد.
- فیلد اختیاری payload: `migrateOwner`. وقتی همراه با `ownerHandle` مقدار `true` داشته باشد، یک skill موجود می‌تواند به آن مالک منتقل شود، اگر actor در هر دو ناشر فعلی و هدف مدیر/مالک باشد. بدون این opt-in، تغییرات مالک رد می‌شوند.
...
- فیلدهای مجاز فرم عبارت‌اند از `payload`، blobهای تکرارشوندهٔ `files` یا یک ارجاع tarball به نام `clawpack`. `clawpack` می‌تواند یک blob `.tgz` یا یک شناسهٔ storage برگشتی از جریان upload-url باشد. انتشارهای مرحله‌بندی‌شدهٔ storage-id باید `clawpackUploadTicket` برگشتی با آن URL بارگذاری را نیز شامل شوند.
- یا از `files` استفاده کنید یا از `clawpack`، هرگز هر دو را در یک درخواست استفاده نکنید.
- بدنه‌های JSON و فرادادهٔ `payload.files` / `payload.artifact` تأمین‌شده توسط فراخواننده رد می‌شوند.
- درخواست‌های انتشار multipart مستقیم به 18MB محدودند. tarballهای ClawPack می‌توانند از جریان upload-url تا سقف tarball 120MB استفاده کنند.
...
- بسته‌های Plugin به `openclaw.plugin.json` نیاز دارند. بارگذاری‌های ClawPack `.tgz` باید آن را در `package/openclaw.plugin.json` داشته باشند.
- code pluginها به `package.json`، فرادادهٔ مخزن منبع، فرادادهٔ commit منبع، فرادادهٔ schema پیکربندی، `openclaw.compat.pluginApi` و `openclaw.build.openclawVersion` نیاز دارند.
...
soft-delete / بازگردانی یک skill (مالک، ناظر یا مدیر).
...
soft deleteهایی که مالک آغاز می‌کند slug را برای 30 روز رزرو می‌کنند، سپس slug می‌تواند توسط ناشر دیگری claim شود. وقتی این انقضا اعمال شود، پاسخ حذف شامل `slugReservedUntil` است.
...
فقط مدیر. تضمین می‌کند یک ناشر سازمانی برای یک handle وجود دارد. اگر handle هنوز به یک ناشر مشترک قدیمی کاربر/شخصی اشاره کند، نقطهٔ پایانی ابتدا آن را به یک ناشر سازمانی مهاجرت می‌دهد.
برای سازمان تازه‌ساخته‌شده، `memberHandle` را ارائه کنید؛ مدیر acting به‌عنوان عضو اضافه نمی‌شود.
...
ایجاد ناشر سازمانی به‌صورت self-serve برای کاربر احراز هویت‌شده. یک ناشر سازمانی جدید می‌سازد و فراخواننده را به‌عنوان مالک اضافه می‌کند. این نقطهٔ پایانی handleهای موجود کاربر/شخصی را مهاجرت نمی‌دهد و ناشر را trusted/official علامت‌گذاری نمی‌کند.
...
فقط مدیر. slugهای ریشه و نام‌های بسته را بدون انتشار یک release برای مالک برحق رزرو می‌کند. نام‌های بسته به بسته‌های placeholder خصوصی بدون ردیف release تبدیل می‌شوند، تا همان مالک بتواند بعداً release واقعی code-plugin یا bundle-plugin را در آن نام منتشر کند.
...
فقط مدیر. یک ناشر شخصی را برای principal جایگزین GitHub OAuth تأییدشده، بدون ویرایش ردیف‌های حساب Convex Auth، بازیابی می‌کند. درخواست باید هر دو شناسهٔ تغییرناپذیر حساب provider در GitHub را نام ببرد؛ handleهای قابل تغییر فقط به‌عنوان guard روبه‌روی operator استفاده می‌شوند.

نقطه پایانی به‌طور پیش‌فرض روی اجرای آزمایشی تنظیم است. اعمال بازیابی پس از آنکه کارکنان به‌طور مستقل پیوستگی بین هر دو هویت اصلی
...
نام‌های مستعار اسلاگ مهارت، بسته‌ها، هشدارهای بازرس بسته، و ردیف‌های مشتق‌شده خلاصه جست‌وجو مهاجرت می‌دهد تا
مسیرهای مالک مستقیم با مرجع ناشر جدید هم‌خوان شوند. یک رزرو فعال دسته محافظت‌شده
برای دسته بازیابی‌شده نیز به کاربر جایگزین منتقل می‌شود تا همگام‌سازی‌های بعدی
...
- `rename` اسلاگ قبلی را به‌عنوان یک نام مستعار هدایت حفظ می‌کند.
- `merge` فهرست منبع را پنهان می‌کند و اسلاگ منبع را به فهرست هدف هدایت می‌کند.
...
یک کاربر را مسدود می‌کند و مهارت‌های تحت مالکیت او را به‌صورت سخت حذف می‌کند (فقط ناظر/مدیر).
...
مسدودیت یک کاربر را لغو می‌کند و مهارت‌های واجد شرایط را بازیابی می‌کند (فقط مدیر).
...
دلیل ذخیره‌شده برای یک مسدودیت موجود را بدون لغو مسدودیت یا بازیابی
...
یک ستاره را اضافه/حذف می‌کند (برجسته‌سازی‌ها). هر دو نقطه پایانی هم‌توان هستند.
