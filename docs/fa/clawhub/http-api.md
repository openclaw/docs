...
summary: مرجع HTTP API (نقاط پایانی عمومی + نقاط پایانی CLI + احراز هویت).
x-i18n:
...
دایرکتوری‌های شخص ثالث می‌توانند از endpointهای خواندنی عمومی برای فهرست‌کردن یا جستجوی skills در ClawHub استفاده کنند. لطفاً نتایج را cache کنید، `429`/`Retry-After` را رعایت کنید، کاربران را به فهرست canonical در ClawHub (`https://clawhub.ai/<owner>/<slug>`) برگردانید، و از القای تأیید ClawHub برای سایت شخص ثالث خودداری کنید. تلاش نکنید محتوای مخفی، خصوصی، یا مسدودشده توسط moderation را خارج از سطح API عمومی mirror کنید.

میانبرهای web slug در خانواده‌های registry resolve می‌شوند، اما clientهای API باید به‌جای بازسازی تقدم route، از URLهای canonical بازگردانده‌شده توسط endpointهای خواندن استفاده کنند.
...
- endpointهای نوشتنِ احراز هویت‌شده نباید وقتی server دلیل را می‌داند، فقط یک `Unauthorized` ساده برگردانند. tokenهای غایب، tokenهای نامعتبر/لغوشده، و حساب‌های حذف‌شده/مسدودشده/غیرفعال‌شده باید هرکدام متن قابل‌اقدامی دریافت کنند تا clientهای CLI بتوانند به کاربران بگویند چه چیزی آن‌ها را متوقف کرده است.
...
- `X-RateLimit-Reset`: ثانیه‌های Unix epoch مطلق
...
- اگر هیچ IP client قابل‌اعتمادی در دسترس نباشد، درخواست‌های دانلود ناشناس به‌جای یک bucket سراسری `ip:unknown` از یک bucket fallback محدود به endpoint استفاده می‌کنند. درخواست‌های خواندن/نوشتن ناشناس همچنان از bucket ناشناختهٔ مشترک استفاده می‌کنند تا routing بدون IP همچنان قابل‌مشاهده و محافظه‌کارانه بماند.
...
- `highlightedOnly` (اختیاری): `true` برای محدودکردن به skills برجسته‌شده
...
- نتایج به‌ترتیب ارتباط بازگردانده می‌شوند (شباهت embedding + تقویت‌های دقیق token برای slug/name + prior محبوبیت از دانلودها).
- ارتباط از محبوبیت قوی‌تر است. یک تطابق دقیق token در slug یا display-name می‌تواند از تطابقی آزادتر با دانلودهای بسیار بیشتر رتبهٔ بالاتری بگیرد.
- متن ASCII بر اساس مرزهای کلمه و نشانه‌گذاری tokenسازی می‌شود. برای مثال، `personal-map` شامل یک token مستقل `map` است، درحالی‌که `amap-jsapi-skill` شامل `amap`، `jsapi` و `skill` است؛ بنابراین جستجو برای `map` به `personal-map` یک تطابق واژگانی قوی‌تر از `amap-jsapi-skill` می‌دهد.
- دانلودها به‌عنوان prior کوچک با مقیاس لگاریتمی و tie-breaker استفاده می‌شوند، نه سیگنال اصلی رتبه‌بندی. skills با دانلود بالا وقتی متن query تطابق ضعیف‌تری دارد می‌توانند رتبهٔ پایین‌تری بگیرند.
- وضعیت moderation مشکوک یا مخفی بسته به فیلترهای caller و وضعیت فعلی moderation می‌تواند یک skill را از جستجوی عمومی حذف کند.

راهنمای discoverability برای ناشر:

- واژه‌هایی را که کاربران به‌صورت لفظی جستجو می‌کنند در display name، summary و tagها قرار دهید. فقط زمانی از token مستقل در slug استفاده کنید که همان نیز یک هویت پایدار باشد که می‌خواهید نگه دارید.
...
- aliasهای تغییر نام، resolution را برای URLهای قدیمی و installهایی که از طریق registry resolve می‌شوند حفظ می‌کنند، اما رتبه‌بندی جستجو پس از indexشدن تغییر نام بر اساس metadata canonical skill است. آمار موجود با skill باقی می‌ماند.
- اگر یک skill به‌شکلی غیرمنتظره نامرئی است، پیش از تغییر metadata مرتبط با رتبه‌بندی، ابتدا هنگام ورود با `clawhub inspect <slug>` وضعیت moderation را بررسی کنید.
...
- evidence برای callerهای عمومی redact می‌شود و فقط برای مالکان/moderatorها شامل snippetهای raw است.
...
`note` برای `confirmed` و `dismissed` ضروری است؛ هنگام تنظیم دوبارهٔ `status` به `open` می‌تواند حذف شود. برای مخفی‌کردن skill در همان workflow قابل‌ممیزی، `finalAction: "hide"` را همراه با گزارش triage‌شده ارسال کنید.
...
- اگر نه `version` و نه `tag` ارائه نشده باشد، از آخرین version استفاده می‌کند.
- شامل وضعیت verification normalized به‌همراه جزئیات مخصوص هر scanner است.
...
- `moderation` یک snapshot فعلی از moderation در سطح skill است که از آخرین version مشتق شده است.
- هنگام query کردن یک version تاریخی، پیش از تلقی `moderation` و `security` به‌عنوان context یکسان version، `moderation.matchesRequestedVersion` و `moderation.sourceVersion` را بررسی کنید.

### `GET /api/v1/skills/{slug}/file`

محتوای متن raw را برمی‌گرداند.
...
- `externalService`، `binary`، `osPermission` (اختیاری): میان‌بر برای برچسب‌های نام‌دار
  نیازمندی محیط
- `artifactKind` (اختیاری): `legacy-zip` یا `npm-pack`
- `npmMirror` (اختیاری): `true`/`1` برای نمایش نسخه‌های بسته با پشتیبانی ClawPack
  که از طریق آینه npm در دسترس‌اند

نکات:

- `GET /api/v1/code-plugins` و `GET /api/v1/bundle-plugins` به‌عنوان نام‌های مستعار با خانواده ثابت باقی می‌مانند.
- ورودی‌های Skill همچنان پشتوانه‌شان رجیستری skill است و هنوز فقط از طریق `POST /api/v1/skills` قابل انتشار هستند.
...
- `artifactKind` (اختیاری): `legacy-zip` یا `npm-pack`
- `npmMirror` (اختیاری): `true`/`1` برای جست‌وجوی نسخه‌های بسته با پشتیبانی ClawPack
  که از طریق آینه npm در دسترس‌اند
...
- `version.artifact.kind` برای آرشیوهای بسته دنیای قدیمی `legacy-zip` است یا
  برای انتشارهای با پشتیبانی ClawPack مقدار `npm-pack` دارد.
...
- `open`: انتشارهای مشکوک، مخرب، در انتظار، قرنطینه‌شده، revoked یا گزارش‌شده.
- `blocked`: انتشارهای قرنطینه‌شده، revoked یا مخرب.
...
به یک نسخه پیوند داده می‌شوند. آن‌ها صف moderation را تغذیه می‌کنند، اما خودشان به‌طور خودکار
دانلودها را پنهان یا مسدود نمی‌کنند؛ ناظران باید برای
تأیید، قرنطینه یا revoke کردن آرتیفکت‌ها از moderation انتشار استفاده کنند.
...
`note` برای `confirmed` و `dismissed` الزامی است؛ هنگام تنظیم دوبارهٔ `status` روی `open` می‌توان آن را حذف کرد. برای اعمال تعدیل انتشار در همان گردش‌کار قابل ممیزی، `finalAction: "quarantine"` یا `finalAction: "revoke"` را همراه با یک گزارش تأییدشده ارسال کنید.
...
هر تغییر یک ورودی در گزارش ممیزی می‌نویسد.
...
- ردیف‌های موجودِ پشتیبانی‌شده با ClawPack که `artifactKind` ندارند، به‌صورت `npm-pack` ترمیم می‌شوند.
...
- پاسخ‌ها برای بررسی‌های یکپارچگی حل‌کننده شامل سرآیندهای `ETag`، `Digest`، `X-ClawHub-Artifact-Type` و `X-ClawHub-Artifact-Sha256` هستند.
...
- فقط نسخه‌هایی که تاربال‌های npm-pack مربوط به ClawPack برایشان بارگذاری شده است فهرست می‌شوند.
...
- packumentهای بسته‌های دارای scope هم از `/api/npm/@scope/name` و هم از مسیر درخواست کدگذاری‌شدهٔ npm یعنی `/api/npm/@scope%2Fname` پشتیبانی می‌کنند.
...
- `slug` (الزامی)
- `hash` (الزامی): sha256 شانزدهی ۶۴ کاراکتری از اثرانگشت باندل
...
یک zip از نسخهٔ یک مهارت را دانلود می‌کند.
...
- فیلد اختیاری payload: `ownerHandle`. وقتی حاضر باشد، API آن ناشر را سمت سرور حل می‌کند و نیاز دارد عامل به ناشر دسترسی داشته باشد.
- فیلد اختیاری payload: `migrateOwner`. وقتی همراه با `ownerHandle` برابر `true` باشد، یک مهارت موجود می‌تواند به آن مالک منتقل شود، اگر عامل روی ناشران فعلی و مقصد مدیر/مالک باشد. بدون این انتخاب صریح، تغییرات مالک رد می‌شوند.
...
- Pluginهای کد به `package.json`، فرادادهٔ مخزن منبع، فرادادهٔ کامیت منبع، فرادادهٔ شمای پیکربندی، `openclaw.compat.pluginApi` و `openclaw.build.openclawVersion` نیاز دارند.
...
- انتشارهای ازطرف‌دیگری همچنان صلاحیت کانال رسمی را نسبت به حساب مالک مقصد اعتبارسنجی می‌کنند.

### `DELETE /api/v1/skills/{slug}` / `POST /api/v1/skills/{slug}/undelete`

حذف نرم / بازیابی یک مهارت (مالک، ناظر یا مدیر).
...
وقتی حاضر باشد، `reason` به‌عنوان یادداشت تعدیل مهارت ذخیره می‌شود و در گزارش ممیزی کپی می‌شود.
حذف‌های نرم آغازشده توسط مالک، slug را برای ۳۰ روز رزرو می‌کنند و سپس slug می‌تواند توسط ناشر دیگری ادعا شود. وقتی این انقضا اعمال شود، پاسخ حذف شامل `slugReservedUntil` است.
...
فقط مدیر. تضمین می‌کند که یک ناشر سازمانی برای یک شناسه وجود دارد. اگر شناسه هنوز به یک ناشر کاربر/شخصی مشترک قدیمی اشاره کند، نقطهٔ پایانی ابتدا آن را به یک ناشر سازمانی مهاجرت می‌دهد.
...
فقط مدیر. slugهای ریشه و نام‌های بسته را بدون انتشار یک نسخه برای مالک برحق رزرو می‌کند. نام‌های بسته به بسته‌های جای‌نگهدار خصوصی بدون ردیف انتشار تبدیل می‌شوند، بنابراین همان مالک می‌تواند بعداً انتشار واقعی code-plugin یا bundle-plugin را در همان نام منتشر کند.
...
- `rename`، slug قبلی را به‌عنوان نام مستعار هدایت نگه می‌دارد.
- `merge` فهرست مبدأ را پنهان می‌کند و slug مبدأ را به فهرست مقصد هدایت می‌کند.
...
یک کاربر را مسدود می‌کند و مهارت‌های تحت مالکیت او را حذف سخت می‌کند (فقط ناظر/مدیر).
...
مسدودی یک کاربر را لغو می‌کند و مهارت‌های واجد شرایط را بازیابی می‌کند (فقط مدیر).
...
یک ستاره اضافه/حذف می‌کند (برجسته‌ها). هر دو نقطهٔ پایانی idempotent هستند.
...
اگر خودمیزبان هستید، این فایل را ارائه دهید (یا `CLAWHUB_REGISTRY` را صریح تنظیم کنید؛ `CLAWDHUB_REGISTRY` قدیمی است).
