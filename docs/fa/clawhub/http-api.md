...
x-i18n:
    generated_at: "2026-07-05T05:19:23Z"
    model: gpt-5.5
...
فهرست‌های شخص ثالث می‌توانند از endpointهای خواندن عمومی برای فهرست کردن یا جست‌وجوی Skills های ClawHub استفاده کنند. لطفاً نتایج را cache کنید، `429`/`Retry-After` را رعایت کنید، کاربران را به فهرست canonical در ClawHub برگردانید (`https://clawhub.ai/<owner>/skills/<slug>`)، و از القای تأیید سایت شخص ثالث توسط ClawHub خودداری کنید. تلاش نکنید محتوای پنهان، خصوصی، یا مسدودشده توسط moderation را خارج از سطح API عمومی mirror کنید.

میانبرهای slug وب در خانواده‌های registry resolve می‌شوند، اما clientهای API باید به‌جای بازسازی تقدم route،
از URLهای canonical برگردانده‌شده توسط endpointهای خواندن استفاده کنند.
...
- endpointهای نوشتن احراز هویت‌شده وقتی server دلیل را می‌داند نباید یک `Unauthorized` خالی برگردانند. توکن‌های مفقود، توکن‌های نامعتبر/باطل‌شده، و حساب‌های حذف‌شده/ممنوع‌شده/غیرفعال‌شده باید هرکدام متن قابل اقدام دریافت کنند تا clientهای CLI بتوانند به کاربران بگویند چه چیزی مانع آن‌ها شده است.
...
- `X-RateLimit-Remaining` / `RateLimit-Remaining`: بودجهٔ دقیق باقی‌مانده هنگام حضور.
  درخواست‌های موفق sharded این header را حذف می‌کنند، به‌جای آنکه یک مقدار global تقریبی برگردانند.
- `Retry-After`: ثانیه‌هایی که باید پیش از تلاش دوباره منتظر ماند (تأخیر) در `429`
...
- از headerهای IP معتبر client، از جمله `cf-connecting-ip`، فقط زمانی استفاده می‌کند که deployment به‌صراحت headerهای forwarded معتبر را فعال کرده باشد.
- ClawHub از headerهای forwarding معتبر برای شناسایی IPهای client در edge استفاده می‌کند.
- اگر IP معتبر client در دسترس نباشد، درخواست‌های ناشناس از bucketهای fallback استفاده می‌کنند
  که فقط با نوع rate-limit scoped شده‌اند. این bucketهای fallback شامل
  مسیرها، slugها، نام packageها، نسخه‌ها، query stringها، یا سایر
...
permission (`401`/`403`)، محدودیت‌های نرخ (`429`)، و دانلودهای مسدودشده است. clientها
باید بدنهٔ پاسخ را به‌عنوان رشته‌ای خوانا برای انسان بخوانند. پارامترهای query ناشناخته
برای سازگاری نادیده گرفته می‌شوند، اما پارامترهای query شناخته‌شده با مقدار نامعتبر
`400` برمی‌گردانند.
...
- `highlightedOnly` (اختیاری): `true` برای filter کردن به Skills های highlighted
- `nonSuspiciousOnly` (اختیاری): `true` برای پنهان کردن Skills های مشکوک (`flagged.suspicious`)
- `nonSuspicious` (اختیاری): alias legacy برای `nonSuspiciousOnly`
...
- نتایج به‌ترتیب ارتباط برگردانده می‌شوند (شباهت embedding + تقویت tokenهای دقیق slug/name + یک prior کوچک محبوبیت).
...
- متن ASCII بر اساس مرزهای کلمه و نشانه‌گذاری tokenized می‌شود. برای مثال، `personal-map` شامل token مستقل `map` است، درحالی‌که `amap-jsapi-skill` شامل `amap`، `jsapi`، و `skill` است؛ بنابراین جست‌وجوی `map` به `personal-map` تطابق واژگانی قوی‌تری نسبت به `amap-jsapi-skill` می‌دهد.
- محبوبیت با مقیاس log محاسبه و capped می‌شود. Skills های با engagement بالا وقتی متن query تطابق ضعیف‌تری دارد می‌توانند رتبهٔ پایین‌تری بگیرند.
...
- واژه‌هایی را که کاربران دقیقاً جست‌وجو خواهند کرد در display name، summary، و tags قرار دهید. از token مستقل slug فقط وقتی استفاده کنید که همان‌طور یک هویت پایدار است که می‌خواهید نگه دارید.
- slug را فقط برای دنبال کردن یک query تغییر نام ندهید، مگر آنکه slug جدید نام canonical بلندمدت بهتری باشد. slugهای قدیمی به aliasهای redirect تبدیل می‌شوند، اما URL canonical، slug نمایش‌داده‌شده، و digestهای جست‌وجوی آینده از slug جدید استفاده می‌کنند.
- aliasهای rename، resolution را برای URLهای قدیمی و نصب‌هایی که از طریق registry resolve می‌شوند حفظ می‌کنند، اما رتبه‌بندی جست‌وجو پس از index شدن rename بر اساس metadata canonical Skill است. آمار موجود با Skill باقی می‌ماند.
- اگر یک Skill به‌طور غیرمنتظره نامرئی است، پیش از تغییر metadata مرتبط با رتبه‌بندی، ابتدا وضعیت moderation را با `clawhub inspect @owner/slug` درحالی‌که وارد شده‌اید بررسی کنید.
...
- `cursor` (اختیاری): pagination cursor برای هر sort غیر از `trending`
- `sort` (اختیاری): `updated` (پیش‌فرض)، `recommended` (alias: `default`)، `createdAt` (alias: `newest`)، `downloads`، `stars` (alias: `rating`)، aliasهای legacy نصب `installsCurrent`/`installs`/`installsAllTime` به `downloads` map می‌شوند، `trending`
- `nonSuspiciousOnly` (اختیاری): `true` برای پنهان کردن Skills های مشکوک (`flagged.suspicious`)
- `nonSuspicious` (اختیاری): alias legacy برای `nonSuspiciousOnly`
...
- وقتی `nonSuspiciousOnly=true`، sortهای مبتنی بر cursor ممکن است در یک صفحه کمتر از `limit` مورد برگردانند، چون Skills مشکوک پس از retrieval صفحه filter می‌شوند.
- وقتی `nextCursor` وجود دارد، برای ادامهٔ pagination از آن استفاده کنید. یک صفحهٔ کوتاه به‌تنهایی به‌معنای پایان نتایج نیست.
...
به یک نسخه link می‌شوند، و صف گزارش Skill را تغذیه می‌کنند.
...
- `limit` (اختیاری): عدد صحیح (1-200)
- `cursor` (اختیاری): pagination cursor
...
برگرداندن `status` به `open` می‌توان آن را حذف کرد. `finalAction: "hide"` را همراه با یک گزارش triaged
...
- `security.hasScanResult` فقط زمانی `true` است که یک اسکنر حکم قطعی (`clean`، `suspicious`، یا `malicious`) تولید کرده باشد.
...
- کارهای اسکن ناهمگام هستند. درخواست‌های اسکن دستی جلوتر از کارهای عادی انتشار/پرکردنِ عقب‌مانده اولویت‌بندی می‌شوند، اما تکمیل همچنان به در دسترس بودن worker وابسته است.
...
- هویت مهارت، هویت ناشر، و فراداده نسخه انتخاب‌شده فیلدهای سطح بالای پاکت هستند (`slug`، `displayName`، `publisherHandle`، `version`، `resolvedFrom`، `tag`، `createdAt`) تا اتوماسیون shell بتواند بدون باز کردن wrapperهای تو در تو آن‌ها را بخواند.
...
- `provenance` فقط زمانی `server-resolved-github-import` است که ClawHub هنگام انتشار یا import یک مخزن/رفرنس/commit/مسیر GitHub را حل و ذخیره کرده باشد؛ در غیر این صورت `unavailable` است.

### `POST /api/v1/skills/-/security-verdicts`

حکم‌های امنیتی فشرده فعلی را برای نسخه‌های دقیق مهارت برمی‌گرداند. این
نقطه پایانی مجموعه برای کلاینت‌هایی در نظر گرفته شده است که از قبل می‌دانند کدام نسخه‌های مهارت ClawHub نصب‌شده را باید نمایش دهند، مانند OpenClaw Control UI.
...
- پاسخ فقط امنیتی است. شامل داده کارت مهارت، وضعیت کارت تولیدشده، فهرست‌های فایل artifact، یا بارهای تفصیلی اسکنر نیست.
...
- `security.signals.dependencyRegistry` برای سازگاری پاسخ v1 نگه داشته شده است، اما اسکنر وجود رجیستری وابستگی بازنشسته شده و این کلید همیشه `null` است.
- نبود کارت مهارت بر `ok`، `decision`، یا `reasons` این نقطه پایانی اثر نمی‌گذارد؛ کلاینت‌ها هنگام نیاز به محتوای کارت باید `skill-card.md` نصب‌شده را به‌صورت محلی بخوانند.
- وقتی به پاکت راستی‌آزمایی کارت مهارت تک‌مهارتی نیاز دارید از `/verify` استفاده کنید، وقتی به markdown کارت تولیدشده نیاز دارید از `/card` استفاده کنید، و وقتی به داده تفصیلی اسکنر نیاز دارید از `/scan` استفاده کنید.
...
`trending` یک جدول رتبه‌بندی نصب/دانلود هفت‌روزه است و از مجموع کل دوره استفاده نمی‌کند.
...
- `startDate` (الزامی): حد پایین برحسب میلی‌ثانیه Unix برای `updatedAt` مربوط به Skill.
- `endDate` (الزامی): حد بالا برحسب میلی‌ثانیه Unix برای `updatedAt` مربوط به Skill.
...
- Skills فعلی پشتیبانی‌شده توسط GitHub با اسکن `clean` یا `suspicious` شامل
...
- `startDate` (الزامی): کران پایین میلی‌ثانیه‌های یونیکس برای `updatedAt` مربوط به Plugin.
- `endDate` (الزامی): کران بالای میلی‌ثانیه‌های یونیکس برای `updatedAt` مربوط به Plugin.
- `limit` (اختیاری): عدد صحیح (1-250)، پیش‌فرض `250`.
- `cursor` (اختیاری): نشانگر صفحه‌بندی از پاسخ قبلی.
...
- `_manifest.json` همیشه در ریشه ZIP گنجانده می‌شود.
- وقتی Pluginها یا فایل‌های منفرد قابل خروجی‌گیری نباشند، `_errors.json` گنجانده
  می‌شود.
...
- نام‌های مستعار فیلتر قدیمی v1 که زیر `GET /api/v1/plugins` مستند شده‌اند نیز
  پذیرفته می‌شوند.
- فیلترکردن دسته‌بندی یک فیلتر واقعی API است که با ردیف‌های خلاصه دسته‌بندی Plugin
...
- کنترل‌های مرتب‌سازی رابط کاربری مرورگر برای جست‌وجوی Plugin، نتایج ارتباط بارگذاری‌شده را دوباره مرتب می‌کنند،
  مطابق با رفتار فعلی مرور `/skills`.
...
- `version.sha256hash` فراداده سازگاری منسوخ برای کلاینت‌های قدیمی است. این مقدار
  ...
  مصنوع انتشار کانونی را مشخص می‌کند.
...
برمی‌گرداند. این سطح مصرف عمومی OpenClaw برای تصمیم‌گیری درباره این است که آیا
یک انتشار resolveشده قابل نصب است یا نه.

احراز هویت:

- نقطه پایانی خواندن عمومی. هیچ توکن مالک، ناشر، ناظر، یا ادمین
  لازم نیست.
...
- `trust.reasons` فهرست توضیح قابل نمایش به کاربر و قابل حسابرسی است. کدهای دلیل
...
- این نقطه پایانی عمداً از نقاط پایانی نظارت مالک/ناظر محدودتر است.
  ...
  هویت گزارش‌دهنده‌ها، متن گزارش‌ها، شواهد خصوصی، یا جدول‌های زمانی داخلی بازبینی.

### `GET /api/v1/packages/{name}/versions/{version}/artifact`

فراداده resolver مصنوع صریح را برای یک نسخه بسته برمی‌گرداند.
...
مصنوع نسخه را از طریق مسیر resolver صریح دانلود می‌کند.

یادداشت‌ها:

- نسخه‌های ClawPack بایت‌های دقیق `.tgz` آپلودشده npm-pack را استریم می‌کنند.
...
به یک نسخه پیوند می‌خورند. آن‌ها صف نظارت را تغذیه می‌کنند، اما خودشان به‌طور خودکار دانلودها را
پنهان یا مسدود نمی‌کنند؛ ناظران باید از نظارت انتشار برای
تأیید، قرنطینه، یا ابطال مصنوعات استفاده کنند.
...
نقطهٔ پایانی مدیر ناظر/مدیر برای دریافت گزارش‌های بسته.

احراز هویت:

- به یک توکن API برای کاربر مدیر ناظر یا مدیر نیاز دارد.
...
نقطهٔ پایانی مدیر ناظر/مدیر برای حل‌وفصل یا بازگشایی گزارش‌های بسته.
...
نقطهٔ پایانی مدیر ناظر/مدیر برای بازبینی انتشار بسته.
...
- فقط نسخه‌هایی که tarballهای npm-pack ClawPack آپلودشده دارند فهرست می‌شوند.
...
- فیلد اختیاری payload: `ownerHandle`. وقتی حاضر باشد، API آن
  ناشر را سمت سرور resolve می‌کند و نیاز دارد actor به ناشر دسترسی داشته باشد.
- فیلد اختیاری payload: `migrateOwner`. وقتی همراه با `ownerHandle` مقدار `true` باشد، یک
  skill موجود می‌تواند به آن مالک منتقل شود اگر actor در هر دو ناشر
...
- فیلدهای مجاز فرم عبارت‌اند از `payload`، blobهای تکرارشوندهٔ `files`، یا یک ارجاع tarball
  با نام `clawpack`. `clawpack` می‌تواند یک blob `.tgz` یا یک شناسهٔ storage برگشتی از
...
- انتشارهای از طرف دیگران همچنان صلاحیت کانال رسمی را در برابر حساب مالک مقصد اعتبارسنجی می‌کنند.

### `DELETE /api/v1/skills/{slug}` / `POST /api/v1/skills/{slug}/undelete`

soft-delete / بازیابی یک skill (مالک، مدیر ناظر، یا مدیر).
...
پنهان‌سازی‌های مدیر ناظر/مدیر و حذف‌های امنیتی به این شکل منقضی نمی‌شوند.
...
فقط مدیر. تضمین می‌کند که یک ناشر org برای یک handle وجود دارد. اگر handle هنوز به یک
کاربر مشترک قدیمی/ناشر شخصی اشاره کند، نقطهٔ پایانی ابتدا آن را به یک ناشر org مهاجرت می‌دهد.
برای یک org تازه‌ساخته، `memberHandle` را ارائه کنید؛ مدیر عمل‌کننده به‌عنوان عضو اضافه نمی‌شود.
پیش‌فرض `memberRole` برابر `owner` است.
...
ایجاد ناشر org به‌صورت خودسرویس و با احراز هویت. یک ناشر org جدید ایجاد می‌کند و
...
یک release رزرو می‌کند. نام‌های بسته به بسته‌های placeholder خصوصی بدون سطرهای release تبدیل می‌شوند، تا همان
...
بدون ویرایش سطرهای حساب Convex Auth بازیابی می‌کند. درخواست باید هر دو شناسهٔ account
تغییرناپذیر provider در GitHub را نام ببرد؛ handleهای تغییرپذیر فقط به‌عنوان guard روبه‌روی اپراتور استفاده می‌شوند.

نقطه پایانی به‌طور پیش‌فرض روی اجرای آزمایشی تنظیم است. اعمال بازیابی به `dryRun: false` و
`confirmIdentityVerified: true` نیاز دارد، پس از آنکه کارکنان به‌طور مستقل پیوستگی بین هر دو
اصل GitHub را تأیید کنند. بازیابی در صورتی که ناشر شخصی فعلی کاربر مقصد Skills، بسته‌ها، یا منابع Skill در GitHub داشته باشد، به‌صورت بسته شکست می‌خورد.
بازیابی همچنین فیلدهای قدیمی `ownerUserId` را برای Skills ناشر بازیابی‌شده،
نام‌های مستعار slug مربوط به Skill، بسته‌ها، هشدارهای بازرس بسته، و ردیف‌های مشتق‌شده digest جستجو منتقل می‌کند تا
...
- `merge` فهرست مبدأ را پنهان می‌کند و slug مبدأ را به فهرست هدف تغییرمسیر می‌دهد.
...
مسدود کردن یک کاربر و حذف سخت Skills متعلق به او (فقط گرداننده/مدیر).
...
فهرست کردن یا جستجوی کاربران (فقط مدیر).
...
که یک tarball از ClawPack را در آن مرحله آماده می‌کنند باید شناسه ذخیره‌سازی حاصل را به‌عنوان
...
اگر خودمیزبانی می‌کنید، این فایل را ارائه کنید (یا `CLAWHUB_REGISTRY` را به‌طور صریح تنظیم کنید؛ `CLAWDHUB_REGISTRY` قدیمی است).
