...
دایرکتوری‌های شخص ثالث می‌توانند از نقطه‌های پایانی خواندن عمومی برای فهرست کردن یا جست‌وجوی Skills در ClawHub استفاده کنند. لطفاً نتایج را کش کنید، `429`/`Retry-After` را رعایت کنید، کاربران را به فهرست رسمی ClawHub (`https://clawhub.ai/<owner>/skills/<slug>`) برگردانید، و از القای تأیید سایت شخص ثالث توسط ClawHub خودداری کنید. تلاش نکنید محتوای پنهان، خصوصی، یا مسدودشده توسط نظارت را خارج از سطح API عمومی آینه کنید.

میانبرهای اسلاگ وب در خانواده‌های رجیستری resolve می‌شوند، اما کلاینت‌های API باید به‌جای بازسازی تقدم مسیرها، از
URLهای رسمی برگردانده‌شده توسط نقطه‌های پایانی خواندن استفاده کنند.
...
- درخواست‌های احراز هویت‌شده (توکن Bearer معتبر): بر اساس باکت هر کاربر اعمال می‌شود.
- اگر توکن وجود نداشته باشد یا نامعتبر باشد، رفتار به اعمال بر اساس IP برمی‌گردد.
- نقطه‌های پایانی نوشتن احراز هویت‌شده نباید وقتی
  سرور دلیل را می‌داند، فقط یک `Unauthorized` ساده برگردانند. توکن‌های مفقود، توکن‌های نامعتبر/باطل‌شده، و
...
- دانلود: 1200/min برای هر IP، 6000/min برای هر کلید (نقطه‌های پایانی دانلود)
...
- `RateLimit-Reset`: ثانیه‌ها تا بازنشانی (تأخیر)
- `X-RateLimit-Remaining` / `RateLimit-Remaining`: بودجهٔ دقیق باقی‌مانده وقتی وجود داشته باشد.
  درخواست‌های موفق شاردشده این هدر را حذف می‌کنند، به‌جای اینکه یک مقدار جهانی تقریبی برگردانند.
- `Retry-After`: ثانیه‌هایی که باید پیش از تلاش دوباره منتظر ماند (تأخیر) روی `429`
...
- فقط وقتی استقرار به‌صورت صریح هدرهای forwarded مورد اعتماد را فعال کرده باشد، از هدرهای IP کلاینت مورد اعتماد، از جمله `cf-connecting-ip`، استفاده می‌کند.
- ClawHub از هدرهای forwarding مورد اعتماد برای شناسایی IPهای کلاینت در edge استفاده می‌کند.
- اگر IP کلاینت مورد اعتمادی در دسترس نباشد، درخواست‌های ناشناس از باکت‌های fallback
  استفاده می‌کنند که فقط بر اساس نوع محدودیت نرخ scoped شده‌اند. این باکت‌های fallback شامل
  مسیرهای ارائه‌شده توسط فراخواننده، اسلاگ‌ها، نام‌های پکیج، نسخه‌ها، رشته‌های پرس‌وجو، یا سایر
  پارامترهای artifact نیستند.

## پاسخ‌های خطا

پاسخ‌های خطای عمومی v1 متن ساده با `content-type: text/plain; charset=utf-8` هستند.
این شامل شکست‌های اعتبارسنجی (`400`)، منابع عمومی مفقود (`404`)، شکست‌های احراز هویت و
مجوز (`401`/`403`)، محدودیت‌های نرخ (`429`)، و دانلودهای مسدودشده می‌شود. کلاینت‌ها
باید بدنهٔ پاسخ را به‌عنوان یک رشتهٔ خوانای انسانی بخوانند. پارامترهای پرس‌وجوی ناشناخته
برای سازگاری نادیده گرفته می‌شوند، اما پارامترهای پرس‌وجوی شناخته‌شده با مقادیر نامعتبر
`400` برمی‌گردانند.
...
- ارتباط از محبوبیت قوی‌تر است. تطابق دقیق توکن اسلاگ یا نام نمایشی می‌تواند از تطابق سست‌تر با تعامل بسیار بیشتر رتبهٔ بالاتری بگیرد.
- متن ASCII بر اساس مرزهای واژه و نشانه‌گذاری tokenized می‌شود. برای مثال، `personal-map` شامل یک توکن مستقل `map` است، در حالی که `amap-jsapi-skill` شامل `amap`، `jsapi`، و `skill` است؛ بنابراین جست‌وجوی `map` به `personal-map` یک تطابق واژگانی قوی‌تر از `amap-jsapi-skill` می‌دهد.
- محبوبیت به‌صورت لگاریتمی مقیاس‌دهی و محدود می‌شود. Skills با تعامل بالا وقتی متن پرس‌وجو تطابق ضعیف‌تری دارد می‌توانند رتبهٔ پایین‌تری بگیرند.
- وضعیت نظارت مشکوک یا پنهان می‌تواند بسته به فیلترهای فراخواننده و وضعیت فعلی نظارت، یک Skill را از جست‌وجوی عمومی حذف کند.

راهنمای دیده‌شدن برای ناشر:

- اصطلاحاتی را که کاربران واقعاً جست‌وجو خواهند کرد در نام نمایشی، خلاصه، و تگ‌ها قرار دهید. فقط وقتی از یک توکن اسلاگ مستقل استفاده کنید که همان‌طور یک هویت پایدار نیز هست که می‌خواهید حفظ کنید.
- فقط برای دنبال کردن یک پرس‌وجو، اسلاگ را تغییر نام ندهید، مگر اینکه اسلاگ جدید نام رسمی بلندمدت بهتری باشد. اسلاگ‌های قدیمی به نام‌های مستعار redirect تبدیل می‌شوند، اما URL رسمی، اسلاگ نمایش‌داده‌شده، و خلاصه‌های جست‌وجوی آینده از اسلاگ جدید استفاده می‌کنند.
- نام‌های مستعار تغییر نام، resolve شدن URLهای قدیمی و نصب‌هایی را که از طریق رجیستری resolve می‌شوند حفظ می‌کنند، اما رتبه‌بندی جست‌وجو پس از ایندکس شدن تغییر نام، بر اساس فرادادهٔ رسمی Skill است. آمار موجود با همان Skill باقی می‌ماند.
- اگر یک Skill به‌شکل غیرمنتظره نامرئی است، پیش از تغییر فرادادهٔ مرتبط با رتبه‌بندی، ابتدا وضعیت نظارت را با `clawhub inspect @owner/slug` در حالت واردشده بررسی کنید.
...
- وقتی `nonSuspiciousOnly=true` باشد، مرتب‌سازی‌های مبتنی بر cursor ممکن است کمتر از `limit` مورد در یک صفحه برگردانند، چون Skills مشکوک پس از بازیابی صفحه فیلتر می‌شوند.
...
- اسلاگ‌های قدیمی ایجادشده توسط جریان‌های تغییر نام/ادغام مالک به Skill رسمی resolve می‌شوند.
...
- اگر Skill هیچ فرادادهٔ پلتفرمی نداشته باشد، `metadata` برابر `null` است.
- `moderation` فقط وقتی شامل می‌شود که Skill پرچم‌گذاری شده باشد یا مالک آن را مشاهده کند.
...
- فراخواننده‌های عمومی فقط برای Skills قابل مشاهده‌ای که از پیش پرچم‌گذاری شده‌اند `200` دریافت می‌کنند.
- شواهد برای فراخواننده‌های عمومی redacted می‌شود و فقط برای مالکان/ناظران شامل قطعه‌های خام است.

### `POST /api/v1/skills/{slug}/report`

یک Skill را برای بازبینی ناظر گزارش کنید. گزارش‌ها در سطح Skill هستند، به‌صورت اختیاری
به یک نسخه پیوند می‌خورند، و صف گزارش Skill را تغذیه می‌کنند.
...
نقطهٔ پایانی ناظر/ادمین برای دریافت گزارش‌های Skill.
...
نقطهٔ پایانی ناظر/ادمین برای حل‌وفصل یا بازگشایی گزارش‌های Skill.
...
بازگرداندن `status` به `open` می‌توان آن را حذف کرد. برای پنهان کردن Skill در همان جریان کاری قابل ممیزی، همراه با گزارش triage‌شده
...
فرادادهٔ نسخه + فهرست فایل‌ها را برمی‌گرداند.

- `version.security` شامل وضعیت تأیید اسکن نرمال‌سازی‌شده و جزئیات scanner
  (VirusTotal + LLM)، در صورت وجود، است.

### `GET /api/v1/skills/{slug}/scan`

جزئیات تأیید اسکن امنیتی را برای یک نسخهٔ Skill برمی‌گرداند.

پارامترهای پرس‌وجو:

- `version` (اختیاری): رشتهٔ نسخهٔ مشخص.
- `tag` (اختیاری): resolve کردن یک نسخهٔ تگ‌شده (برای مثال `latest`).
...
- `moderation` یک نمای لحظه‌ای فعلی از moderation در سطح Skill است که از آخرین نسخه مشتق شده است.
- هنگام پرس‌وجوی یک نسخهٔ تاریخی، پیش از یکسان دانستن زمینهٔ نسخهٔ `moderation` و `security`، `moderation.matchesRequestedVersion` و `moderation.sourceVersion` را بررسی کنید.
...
- بارهای دادهٔ درخواست اسکن و گزارش‌های قابل دانلود پس از بازهٔ نگهداری از مخزن درخواست اسکن منقضی می‌شوند.
...
- کارهای اسکن ناهمگام هستند. درخواست‌های اسکن دستی جلوتر از کارهای عادی انتشار/پس‌پرکنی اولویت‌بندی می‌شوند، اما تکمیل همچنان به دسترس‌بودن worker بستگی دارد.

### `GET /api/v1/skills/-/scan/{scanId}`

نقطهٔ پایانی poll احراز هویت‌شده برای یک اسکن ارسال‌شده.

- وضعیت queued/running/succeeded/failed را برمی‌گرداند.
- در زمان queued بودن، `queue.queuedAhead` و `queue.position` را برمی‌گرداند تا clientها بتوانند نشان دهند چند اسکن دستی اولویت‌دار جلوتر از درخواست قرار دارند. صف‌های بسیار بزرگ محدود می‌شوند و با `queuedAheadIsEstimate: true` گزارش می‌شوند.
...
- به دسترسی مدیریتی مالک/ناشر به Skill یا plugin، یا اختیار مدیر/ناظر پلتفرم نیاز دارد.
...
مسیر batch rescan متعارف فقط مخصوص مدیر. همان شکل payload مسیر قدیمی `POST /api/v1/skills/-/rescan-batch` را می‌پذیرد.

### `POST /api/v1/skills/-/scan/batch/status`

مسیر وضعیت batch متعارف فقط مخصوص مدیر. `{ "jobIds": ["..."] }` را می‌پذیرد و همان شمارنده‌های تجمیعی مسیر قدیمی `POST /api/v1/skills/-/rescan-batch/status` را برمی‌گرداند.

### `GET /api/v1/skills/{slug}/verify`

envelope تأیید کارت Skill را که توسط `clawhub skill verify` استفاده می‌شود برمی‌گرداند.

پارامترهای query:

- `version` (اختیاری): رشتهٔ نسخهٔ مشخص.
- `tag` (اختیاری): یک نسخهٔ tagشده را resolve می‌کند (برای مثال `latest`).

نکات:

- `ok` فقط زمانی `true` است که نسخهٔ انتخاب‌شده یک کارت Skill تولیدشده داشته باشد، توسط moderation به‌عنوان malware مسدود نشده باشد، و تأیید ClawScan پاک باشد.
- هویت Skill، هویت ناشر، و فرادادهٔ نسخهٔ انتخاب‌شده فیلدهای سطح بالای envelope هستند (`slug`، `displayName`، `publisherHandle`، `version`، `resolvedFrom`، `tag`، `createdAt`) تا automationهای shell بتوانند بدون باز کردن wrapperهای تو در تو آن‌ها را بخوانند.
- `security` حکم سطح بالای ClawScan/security است. automation باید بر پایهٔ `ok`، `decision`، `reasons`، و `security.status` عمل کند.
- `security.signals` شواهد پشتیبان اسکنر مانند `staticScan`، `virusTotal`، و `skillSpector` را شامل می‌شود.
- `security.signals.dependencyRegistry` برای سازگاری پاسخ v1 حفظ شده است، اما اسکنر وجود registry وابستگی بازنشسته شده و این کلید همیشه `null` است.
- `provenance` فقط زمانی `server-resolved-github-import` است که ClawHub هنگام publish یا import یک repo/ref/commit/path از GitHub را resolve و ذخیره کرده باشد؛ در غیر این صورت `unavailable` است.

### `POST /api/v1/skills/-/security-verdicts`

حکم‌های امنیتی فشردهٔ فعلی را برای نسخه‌های دقیق Skill برمی‌گرداند. این
نقطهٔ پایانی collection برای clientهایی در نظر گرفته شده که از قبل می‌دانند کدام نسخه‌های Skill نصب‌شدهٔ
ClawHub را باید نمایش دهند، مانند رابط کاربری کنترل OpenClaw.
...
- نتایج به‌ازای هر item هستند؛ نبودن یک Skill یا نسخه باعث شکست کل پاسخ نمی‌شود.
- پاسخ فقط امنیتی است. شامل داده‌های کارت Skill، وضعیت کارت تولیدشده، فهرست فایل‌های artifact، یا payloadهای تفصیلی اسکنر نیست.
- `security.signals` فقط شواهد پشتیبان در سطح وضعیت را شامل می‌شود؛ برای جزئیات کامل اسکنر از `/scan` یا صفحهٔ security-audit در ClawHub استفاده کنید.
- `security.signals.dependencyRegistry` برای سازگاری پاسخ v1 حفظ شده است، اما اسکنر وجود registry وابستگی بازنشسته شده و این کلید همیشه `null` است.
- نبود کارت Skill بر `ok`، `decision`، یا `reasons` این نقطهٔ پایانی اثر نمی‌گذارد؛ clientها زمانی که به محتوای کارت نیاز دارند باید `skill-card.md` نصب‌شده را به‌صورت محلی بخوانند.
- زمانی از `/verify` استفاده کنید که به envelope تأیید کارت Skill برای یک Skill واحد نیاز دارید، از `/card` زمانی که به markdown کارت تولیدشده نیاز دارید، و از `/scan` زمانی که به دادهٔ تفصیلی اسکنر نیاز دارید.
...
- `POST /api/v1/packages` هنوز فقط برای انتشارهای code-plugin و bundle-plugin است.
...
- `channel=private` فقط بسته‌هایی را برمی‌گرداند که فراخوان احراز هویت‌شده بتواند بخواند.
...
`trending` یک جدول رتبه‌بندی هفت‌روزه نصب/دانلود است و از مجموع‌های تمام‌دوره استفاده نمی‌کند.
...
- `startDate` (الزامی): کران پایین برحسب میلی‌ثانیه یونیکس برای `updatedAt` مربوط به Skill.
- `endDate` (الزامی): کران بالا برحسب میلی‌ثانیه یونیکس برای `updatedAt` مربوط به Skill.
...
- وقتی Skills یا فایل‌های منفرد قابل خروجی‌گیری نباشند،
...
صدور گروهی تازه‌ترین انتشارهای عمومی Plugin برای تحلیل آفلاین.
...
- `startDate` (الزامی): حد پایین به میلی‌ثانیه Unix برای `updatedAt` در Plugin.
- `endDate` (الزامی): حد بالا به میلی‌ثانیه Unix برای `updatedAt` در Plugin.
- `limit` (اختیاری): عدد صحیح (1-250)، پیش‌فرض `250`.
...
- هر Plugin صادرشده شامل فایل‌های ذخیره‌شده تازه‌ترین انتشار است.
...
- نام‌های مستعار قدیمی فیلتر v1 که زیر `GET /api/v1/plugins` مستند شده‌اند نیز
  پذیرفته می‌شوند.
- فیلتر دسته‌بندی یک فیلتر واقعی API است که با ردیف‌های چکیده دسته‌بندی Plugin
...
- کنترل‌های مرتب‌سازی UI مرورگر برای جست‌وجوی Plugin نتایج ارتباط بارگذاری‌شده را
  بازچینی می‌کنند و با رفتار مرور فعلی `/skills` هم‌خوان هستند.
...
یک نسخه بسته را، شامل فراداده فایل، سازگاری، تأیید، فراداده artifact، و داده‌های اسکن بازمی‌گرداند.
...
- `version.sha256hash` فراداده سازگاری منسوخ‌شده برای کلاینت‌های قدیمی است. این
...
سطح مصرف عمومی OpenClaw برای تصمیم‌گیری درباره قابل نصب بودن یک انتشار resolve‌شده است.

احراز هویت:

- endpoint خواندن عمومی. هیچ توکن مالک، ناشر، ناظر، یا مدیر لازم نیست.
...
- `release.releaseId`، `release.version`، و `release.createdAt` انتشار دقیق
  ارزیابی‌شده را شناسایی می‌کنند.
- `release.artifactKind`، `release.artifactSha256`، `release.npmIntegrity`,
  `release.npmShasum`، و `release.npmTarballName` وقتی برای artifact انتشار
  شناخته‌شده باشند، حاضرند.
- `trust.scanStatus` وضعیت اعتماد مؤثر است که از ورودی‌های اسکنر و نظارت دستی
  انتشار به دست می‌آید.
- `trust.moderationState` می‌تواند nullable باشد. وقتی نظارت دستی انتشار وجود
  ندارد، مقدار آن `null` است.
- `trust.blockedFromDownload` سیگنال مسدودسازی نصب است. OpenClaw و سایر
  ...
  قوانین مسدودسازی را از فیلدهای اسکنر یا نظارت دوباره استخراج کنند.
- `trust.reasons` فهرست توضیح کاربرمحور و حسابرسی است. کدهای دلیل رشته‌های
  پایدار و فشرده‌ای مانند `manual:quarantined`، `scan:malicious`،
...
- `trust.stale` یعنی خلاصه اعتماد از ورودی‌های قدیمی محاسبه شده و پیش از یک
  تصمیم اجازه با اطمینان بالا باید نیازمند تازه‌سازی تلقی شود.
...
- این endpoint دقیقاً وابسته به نسخه است. کلاینت‌ها باید پس از resolve کردن
  ...
  تازه‌ترین فراداده بسته.
- بسته‌های خصوصی `404` بازمی‌گردانند، مگر اینکه فراخواننده بتواند ناشر مالک را بخواند.
- این endpoint عمداً از endpointهای نظارت مالک/ناظر محدودتر است. تصمیم نصب و
  توضیح عمومی را افشا می‌کند، نه هویت گزارش‌دهندگان، متن گزارش‌ها، شواهد خصوصی،
  یا خط زمانی‌های بررسی داخلی.

### `GET /api/v1/packages/{name}/versions/{version}/artifact`

فراداده resolveکننده artifact صریح را برای یک نسخه بسته بازمی‌گرداند.

نکات:

- نسخه‌های بسته legacy یک artifact از نوع `legacy-zip` و یک `downloadUrl`
  برای ZIP قدیمی بازمی‌گردانند.
- نسخه‌های ClawPack یک artifact از نوع `npm-pack`، فیلدهای integrity مربوط به npm،
  یک `tarballUrl`، و URL سازگاری ZIP قدیمی را بازمی‌گردانند.
- این سطح resolver در OpenClaw است؛ از حدس‌زدن قالب آرشیو از روی یک URL مشترک
  پرهیز می‌کند.
...
- نسخه‌های ClawPack بایت‌های دقیق `.tgz` نوع npm-pack بارگذاری‌شده را stream می‌کنند.
...
- وضعیت کانال رسمی
- در دسترس بودن تازه‌ترین نسخه
- در دسترس بودن artifact نوع npm-pack در ClawPack
- چکیده artifact
...
- `packageName` به نام npm normalize می‌شود؛ برای مهاجرت‌های برنامه‌ریزی‌شده ممکن
  است بسته موجود نباشد.
- این فقط آمادگی مهاجرت را ردیابی می‌کند. OpenClaw را تغییر نمی‌دهد و ClawPack
  تولید نمی‌کند.

### `GET /api/v1/packages/moderation/queue`

endpoint ناظر/مدیر برای صف‌های بررسی انتشار بسته.
...
نقطه پایانی مدیر/سرپرست برای دریافت گزارش‌های بسته.

احراز هویت:

- به توکن API برای کاربر مدیر یا سرپرست نیاز دارد.
...
نقطه پایانی مالک/مدیر برای مشاهده وضعیت نظارت بسته.

احراز هویت:

- به توکن API برای مالک بسته، عضو ناشر، مدیر، یا کاربر سرپرست نیاز دارد.
...
نقطه پایانی مدیر/سرپرست برای حل‌وفصل یا بازگشایی گزارش‌های بسته.
...
گردش‌کار قابل حسابرسی، همراه با گزارش تأییدشده `finalAction: "quarantine"` یا
...
نقطه پایانی مدیر/سرپرست برای بازبینی انتشار بسته.
...
- این مسیر فقط ZIP می‌ماند. فایل‌های ClawPack با پسوند `.tgz` را جریان‌دهی نمی‌کند.
- پاسخ‌ها برای بررسی یکپارچگی حل‌کننده، سرآیندهای `ETag`، `Digest`، `X-ClawHub-Artifact-Type`، و
  `X-ClawHub-Artifact-Sha256` را شامل می‌شوند.
- فراداده‌ای که فقط مخصوص رجیستری است به آرشیو دانلودشده تزریق نمی‌شود.
...
- فقط نسخه‌هایی که tarballهای npm-pack مربوط به ClawPack برایشان بارگذاری شده فهرست می‌شوند.
- نسخه‌های قدیمی فقط-ZIP عمداً حذف شده‌اند.
- `dist.tarball`، `dist.integrity`، و `dist.shasum` از فیلدهای سازگار با npm استفاده می‌کنند تا کاربران در صورت تمایل بتوانند npm را به آینه اشاره دهند.
- packumentهای بسته scoped هم مسیر `/api/npm/@scope/name` و هم مسیر درخواست کدگذاری‌شده npm یعنی `/api/npm/@scope%2Fname` را پشتیبانی می‌کنند.

### `GET /api/npm/{package}/-/{tarball}.tgz`

بایت‌های دقیق tarball بارگذاری‌شده ClawPack را برای کلاینت‌های آینه npm جریان‌دهی می‌کند.
...
یک ZIP نسخه مهارت میزبانی‌شده را دانلود می‌کند، یا برای یک مهارت فعلی مبتنی بر GitHub که اسکن `clean` یا `suspicious` دارد و نسخه میزبانی‌شده ندارد، واگذاری منبع GitHub را برمی‌گرداند.
...
- واگذاری‌های مهارت مبتنی بر GitHub بایت‌ها را پروکسی یا آینه نمی‌کنند. پاسخ JSON شامل `sourceRef: "public-github"`، `repo`، `commit`، `path`، `contentHash`،
  و `archiveUrl` است؛ وضعیت اسکن/فعلی یک gate است و به‌عنوان فراداده payload موفقیت شامل نمی‌شود.
- آمار دانلود به‌عنوان هویت‌های یکتا در هر روز UTC شمارش می‌شود (`userId` زمانی که توکن API معتبر است، در غیر این صورت IP).
...
- فیلد اختیاری payload: `ownerHandle`. وقتی وجود داشته باشد، API آن ناشر را در سمت سرور resolve می‌کند و از کنشگر می‌خواهد دسترسی ناشر داشته باشد.
- فیلد اختیاری payload: `migrateOwner`. وقتی با `ownerHandle` برابر `true` باشد، یک مهارت موجود ممکن است به آن مالک منتقل شود، اگر کنشگر در هر دو ناشر فعلی و هدف سرپرست/مالک باشد. بدون این opt-in، تغییرات مالک رد می‌شوند.
...
- فیلدهای مجاز فرم عبارت‌اند از `payload`، blobهای تکرارشونده `files`، یا یک ارجاع tarball با نام `clawpack`.
  `clawpack` می‌تواند یک blob با پسوند `.tgz` یا یک شناسه storage باشد که توسط جریان upload-url برگردانده شده است. انتشارهای مرحله‌بندی‌شده با storage-id همچنین باید `clawpackUploadTicket` برگشتی همراه آن URL بارگذاری را شامل شوند.
- در هر درخواست، یا از `files` استفاده کنید یا از `clawpack`، هرگز هر دو را با هم استفاده نکنید.
- بدنه‌های JSON و فراداده `payload.files` / `payload.artifact` تأمین‌شده توسط فراخواننده رد می‌شوند.
- درخواست‌های انتشار مستقیم multipart به 18MB محدود شده‌اند. tarballهای ClawPack می‌توانند تا سقف 120MB tarball از جریان upload-url استفاده کنند.
- فیلد اختیاری payload: `ownerHandle`. وقتی وجود داشته باشد، فقط سرپرست‌ها می‌توانند از طرف آن مالک منتشر کنند.
...
- code pluginها به `package.json`، فراداده مخزن منبع، فراداده commit منبع، فراداده طرح‌واره پیکربندی، `openclaw.compat.pluginApi`، و
  `openclaw.build.openclawVersion` نیاز دارند.
- `openclaw.hostTargets` و `openclaw.environment` فراداده اختیاری هستند.
- فقط ناشر سازمان `openclaw` و ناشران شخصی اعضای فعلی سازمان `openclaw` می‌توانند در کانال `official` منتشر کنند.
- انتشارهای از طرف دیگران همچنان صلاحیت کانال رسمی را نسبت به حساب مالک هدف اعتبارسنجی می‌کنند.

### `DELETE /api/v1/skills/{slug}` / `POST /api/v1/skills/{slug}/undelete`

حذف نرم / بازیابی یک مهارت (مالک، مدیر، یا سرپرست).
...
وقتی وجود داشته باشد، `reason` به‌عنوان یادداشت نظارت مهارت ذخیره و در گزارش حسابرسی کپی می‌شود.
حذف‌های نرم آغازشده توسط مالک، slug را برای 30 روز رزرو می‌کنند، سپس slug می‌تواند توسط ناشر دیگری ادعا شود.
پاسخ حذف وقتی این انقضا اعمال شود، `slugReservedUntil` را شامل می‌شود.
مخفی‌سازی‌های مدیر/سرپرست و حذف‌های امنیتی به این شکل منقضی نمی‌شوند.
...
فقط سرپرست. تضمین می‌کند یک ناشر سازمانی برای یک handle وجود دارد. اگر handle همچنان به یک ناشر کاربر/شخصی مشترک قدیمی اشاره کند، نقطه پایانی ابتدا آن را به یک ناشر سازمانی مهاجرت می‌دهد.
برای یک سازمان تازه‌ساخته‌شده، `memberHandle` را ارائه کنید؛ سرپرست در حال اقدام به‌عنوان عضو اضافه نمی‌شود.
...
ایجاد ناشر سازمانی سلف‌سرویس احراز هویت‌شده. یک ناشر سازمانی جدید ایجاد می‌کند و فراخواننده را به‌عنوان مالک اضافه می‌کند. این نقطه پایانی handleهای کاربر/شخصی موجود را مهاجرت نمی‌دهد و ناشر را trusted/official علامت‌گذاری نمی‌کند.
...
فقط سرپرست. slugهای ریشه و نام‌های بسته را برای مالک برحق بدون انتشار یک release رزرو می‌کند. نام‌های بسته به بسته‌های placeholder خصوصی بدون ردیف release تبدیل می‌شوند، تا همان مالک بتواند بعداً انتشار واقعی code-plugin یا bundle-plugin را در آن نام منتشر کند.
...
فقط سرپرست. یک ناشر شخصی را برای اصل GitHub OAuth جایگزین تأییدشده بازیابی می‌کند، بدون اینکه ردیف‌های حساب Convex Auth را ویرایش کند. درخواست باید هر دو شناسه تغییرناپذیر حساب ارائه‌دهنده GitHub را نام ببرد؛ handleهای تغییرپذیر فقط به‌عنوان guard رو به اپراتور استفاده می‌شوند.

نقطهٔ پایانی به‌طور پیش‌فرض روی اجرای آزمایشی تنظیم است. اعمال بازیابی پس از آنکه کارکنان به‌صورت مستقل پیوستگی بین هر دو اصل GitHub را تأیید کردند، به `dryRun: false` و
`confirmIdentityVerified: true` نیاز دارد. وقتی ناشر شخصی فعلی کاربر مقصد Skills، بسته‌ها، یا منابع Skill در GitHub داشته باشد، بازیابی در حالت بسته شکست می‌خورد.
بازیابی همچنین فیلدهای قدیمی `ownerUserId` را برای Skills ناشر بازیابی‌شده،
نام‌های مستعار slug مربوط به Skill، بسته‌ها، هشدارهای بازرس بسته، و ردیف‌های digest جست‌وجوی مشتق‌شده مهاجرت می‌دهد تا
مسیرهای مالک مستقیم با اختیار ناشر جدید هم‌خوان شوند. یک رزرو فعال protected-handle
برای handle بازیابی‌شده نیز دوباره به کاربر جایگزین واگذار می‌شود تا همگام‌سازی بعدی
پروفایل نتواند اختیار رقیب کاربر پیشین را بازگرداند. هر جدول اصلی در هر تراکنش اعمال به
...
مسدود کردن یک کاربر و حذف کامل Skills متعلق به او (فقط ناظر/مدیر).
...
محتوا (فقط مدیر). مگر آنکه `dryRun` برابر `false` باشد، به‌طور پیش‌فرض اجرای آزمایشی است.
...
`POST /api/cli/upload-url` مقدارهای `uploadUrl` و `uploadTicket` را برمی‌گرداند. انتشار بسته‌هایی
که یک tarball از ClawPack را stage می‌کنند باید شناسهٔ ذخیره‌سازی حاصل را به‌عنوان
...
اگر خودمیزبانی می‌کنید، این فایل را سرو کنید (یا `CLAWHUB_REGISTRY` را صراحتاً تنظیم کنید؛ مقدار قدیمی `CLAWDHUB_REGISTRY`).
