...
x-i18n:
    generated_at: "2026-07-04T15:24:57Z"
    model: gpt-5.5
...
دایرکتوری‌های شخص ثالث می‌توانند از نقاط پایانی عمومی خواندن برای فهرست‌کردن یا جست‌وجوی Skillsهای ClawHub استفاده کنند. لطفا نتایج را کش کنید، `429`/`Retry-After` را رعایت کنید، کاربران را به فهرست canonical در ClawHub (`https://clawhub.ai/<owner>/skills/<slug>`) برگردانید، و از القای تایید سایت شخص ثالث توسط ClawHub خودداری کنید. تلاش نکنید محتوای پنهان، خصوصی، یا مسدودشده توسط نظارت را بیرون از سطح API عمومی آینه‌سازی کنید.

میان‌برهای slug وب در میان خانواده‌های رجیستری resolve می‌شوند، اما کلاینت‌های API باید به‌جای بازسازی تقدم مسیرها، از URLهای canonical برگردانده‌شده توسط نقاط پایانی خواندن استفاده کنند.
...
- نقاط پایانی نوشتن احرازهویت‌شده وقتی سرور دلیل را می‌داند نباید صرفا `Unauthorized` برگردانند. توکن‌های مفقود، توکن‌های نامعتبر/لغوشده، و حساب‌های حذف‌شده/مسدودشده/غیرفعال هر کدام باید متن قابل اقدام دریافت کنند تا کلاینت‌های CLI بتوانند به کاربران بگویند چه چیزی آن‌ها را مسدود کرده است.
...
- `X-RateLimit-Remaining` / `RateLimit-Remaining`: بودجه باقی‌مانده دقیق، وقتی موجود باشد.
  درخواست‌های موفق sharded این هدر را حذف می‌کنند، به‌جای اینکه مقدار جهانی تقریبی برگردانند.
- `Retry-After`: ثانیه‌هایی که باید قبل از تلاش دوباره منتظر ماند (تاخیر) روی `429`
...
- اگر `Retry-After` وجود دارد، همان تعداد ثانیه قبل از تلاش دوباره صبر کنید.
- برای جلوگیری از تلاش‌های دوباره هم‌زمان، از backoff همراه با jitter استفاده کنید.
- اگر `Retry-After` وجود ندارد، به `RateLimit-Reset` fallback کنید (یا از `X-RateLimit-Reset` محاسبه کنید).

منبع IP:

- از هدرهای قابل اعتماد IP کلاینت، از جمله `cf-connecting-ip`، فقط وقتی استفاده می‌کند که استقرار به‌صراحت هدرهای forwarded قابل اعتماد را فعال کرده باشد.
- ClawHub از هدرهای forwarding قابل اعتماد برای شناسایی IPهای کلاینت در edge استفاده می‌کند.
- اگر IP کلاینت قابل اعتمادی در دسترس نباشد، درخواست‌های ناشناس از bucketهای fallback استفاده می‌کنند که فقط بر اساس نوع محدودیت نرخ scope شده‌اند. این bucketهای fallback شامل مسیرها، slugها، نام بسته‌ها، نسخه‌ها، query stringها، یا سایر پارامترهای artifact ارائه‌شده توسط caller نمی‌شوند.

## پاسخ‌های خطا

پاسخ‌های خطای عمومی v1 متن ساده با `content-type: text/plain; charset=utf-8` هستند.
این شامل شکست‌های اعتبارسنجی (`400`)، منابع عمومی مفقود (`404`)، شکست‌های احراز هویت و مجوز (`401`/`403`)، محدودیت‌های نرخ (`429`)، و دانلودهای مسدودشده می‌شود. کلاینت‌ها باید بدنه پاسخ را به‌عنوان رشته‌ای خوانا برای انسان بخوانند. پارامترهای query ناشناخته برای سازگاری نادیده گرفته می‌شوند، اما پارامترهای query شناخته‌شده با مقادیر نامعتبر `400` برمی‌گردانند.
...
- `highlightedOnly` (اختیاری): `true` برای فیلتر به Skillsهای برجسته
...
- ارتباط از محبوبیت قوی‌تر است. تطابق دقیق token در slug یا display-name می‌تواند از تطابق آزادتر با engagement بسیار قوی‌تر بالاتر قرار بگیرد.
- متن ASCII روی مرزهای واژه و نشانه‌گذاری tokenized می‌شود. برای مثال، `personal-map` شامل token مستقل `map` است، درحالی‌که `amap-jsapi-skill` شامل `amap`، `jsapi`، و `skill` است؛ بنابراین جست‌وجوی `map` به `personal-map` تطابق واژگانی قوی‌تری نسبت به `amap-jsapi-skill` می‌دهد.
- محبوبیت log-scaled و capped است. Skillsهای با engagement بالا وقتی متن query تطابق ضعیف‌تری دارد ممکن است پایین‌تر رتبه‌بندی شوند.
- وضعیت نظارتی مشکوک یا پنهان می‌تواند بسته به فیلترهای caller و وضعیت فعلی نظارت، یک skill را از جست‌وجوی عمومی حذف کند.

راهنمای discoverability برای ناشر:

- اصطلاحاتی را که کاربران عینا جست‌وجو خواهند کرد در display name، summary، و tags قرار دهید. از token مستقل slug فقط وقتی استفاده کنید که همان نیز یک هویت پایدار است که می‌خواهید نگه دارید.
...
- aliasهای تغییر نام resolution را برای URLهای قدیمی و نصب‌هایی که از طریق رجیستری resolve می‌شوند حفظ می‌کنند، اما رتبه‌بندی جست‌وجو بعد از index شدن تغییر نام بر اساس metadata canonical skill است. آمار موجود با همان skill باقی می‌ماند.
- اگر skill به‌طور غیرمنتظره نامرئی است، قبل از تغییر metadata مرتبط با رتبه‌بندی، ابتدا وضعیت نظارت را هنگام ورود با `clawhub inspect @owner/slug` بررسی کنید.
...
- `sort` (اختیاری): `updated` (پیش‌فرض)، `recommended` (alias: `default`)، `createdAt` (alias: `newest`)، `downloads`، `stars` (alias: `rating`)، aliasهای قدیمی نصب `installsCurrent`/`installs`/`installsAllTime` به `downloads` نگاشت می‌شوند، `trending`
- `nonSuspiciousOnly` (اختیاری): `true` برای پنهان‌کردن Skillsهای مشکوک (`flagged.suspicious`)
- `nonSuspicious` (اختیاری): alias قدیمی برای `nonSuspiciousOnly`
...
- وقتی `nonSuspiciousOnly=true` باشد، sortهای مبتنی بر cursor ممکن است در یک صفحه کمتر از `limit` مورد برگردانند، چون Skillsهای مشکوک بعد از بازیابی صفحه فیلتر می‌شوند.
- وقتی `nextCursor` موجود است از آن برای ادامه صفحه‌بندی استفاده کنید. یک صفحه کوتاه به‌تنهایی به معنی پایان نتایج نیست.
...
- `metadata.os`: محدودیت‌های OS اعلام‌شده در frontmatter مربوط به skill (برای مثال `["macos"]`، `["linux"]`). اگر اعلام نشده باشد `null`.
...
- `moderation` فقط وقتی گنجانده می‌شود که skill flagged باشد یا مالک در حال مشاهده آن باشد.
...
- مالکان و moderators می‌توانند به جزئیات نظارت برای Skillsهای پنهان دسترسی داشته باشند.
- callerهای عمومی فقط برای Skillsهای مرئی که از قبل flagged شده‌اند `200` دریافت می‌کنند.
- evidence برای callerهای عمومی redact می‌شود و فقط برای مالکان/moderators شامل snippetهای خام است.

### `POST /api/v1/skills/{slug}/report`

یک skill را برای بازبینی moderator گزارش کنید. گزارش‌ها در سطح skill هستند، به‌صورت اختیاری به یک نسخه پیوند می‌خورند، و صف گزارش skill را تغذیه می‌کنند.
...
نقطه پایانی moderator/admin برای resolve یا بازگشایی گزارش‌های skill.
...
`note` برای `confirmed` و `dismissed` الزامی است؛ هنگام تنظیم دوباره `status` به `open` می‌توان آن را حذف کرد. برای پنهان‌کردن skill در همان workflow قابل audit، `finalAction: "hide"` را همراه با یک گزارش triaged ارسال کنید.
...
- `version.security` شامل وضعیت نرمال‌شده تایید scan و جزئیات scanner است
  (VirusTotal + LLM)، وقتی در دسترس باشد.

### `GET /api/v1/skills/{slug}/scan`

جزئیات تایید scan امنیتی برای یک نسخه skill را برمی‌گرداند.

پارامترهای query:

- `version` (اختیاری): رشته نسخه مشخص.
- `tag` (اختیاری): یک نسخه tagged را resolve می‌کند (برای مثال `latest`).

نکات:

- اگر نه `version` و نه `tag` ارائه شود، از آخرین نسخه استفاده می‌شود.
- وضعیت تأیید عادی‌سازی‌شده را به‌همراه جزئیات اختصاصی هر اسکنر شامل می‌شود.
- مقدار `security.hasScanResult` فقط زمانی `true` است که یک اسکنر رأی قطعی (`clean`، `suspicious`، یا `malicious`) تولید کرده باشد.
- `moderation` یک تصویر لحظه‌ای فعلی از تعدیل در سطح مهارت است که از آخرین نسخه مشتق شده است.
- هنگام پرس‌وجوی یک نسخه تاریخی، پیش از اینکه `moderation` و `security` را در یک زمینه نسخه یکسان در نظر بگیرید، `moderation.matchesRequestedVersion` و `moderation.sourceVersion` را بررسی کنید.

### `POST /api/v1/skills/-/scan`

نقطه پایانی ثبت احراز هویت‌شده برای کارهای جدید ClawScan.
...
- بارهای داده درخواست اسکن و گزارش‌های قابل دانلود پس از پنجره نگهداری از مخزن درخواست اسکن منقضی می‌شوند.
...
- کارهای اسکن ناهمگام هستند. درخواست‌های اسکن دستی پیش از کار عادی انتشار/پر کردن عقب‌مانده‌ها اولویت‌بندی می‌شوند، اما تکمیل همچنان به در دسترس بودن کارگرها بستگی دارد.

### `GET /api/v1/skills/-/scan/{scanId}`

نقطه پایانی نظرسنجی احراز هویت‌شده برای یک اسکن ثبت‌شده.

- وضعیت در صف/در حال اجرا/موفق/ناموفق را برمی‌گرداند.
- هنگام قرار داشتن در صف، `queue.queuedAhead` و `queue.position` را برمی‌گرداند تا کلاینت‌ها بتوانند نشان دهند چند اسکن دستی اولویت‌دار پیش از درخواست قرار دارند. صف‌های بسیار بزرگ محدود می‌شوند و با `queuedAheadIsEstimate: true` گزارش می‌شوند.
...
نقطه پایانی آرشیو گزارش ذخیره‌شده احراز هویت‌شده برای نسخه‌های ثبت‌شده.

- به دسترسی مدیریتی مالک/ناشر به مهارت یا Plugin، یا اختیار ناظر/مدیر پلتفرم نیاز دارد.
- نتایج اسکن ذخیره‌شده را برای نسخه دقیق ثبت‌شده، از جمله نسخه‌های مسدود یا پنهان، برمی‌گرداند.
...
مسیر بازاسکن دسته‌ای متعارف، فقط برای مدیران. همان شکل بار داده مسیر قدیمی `POST /api/v1/skills/-/rescan-batch` را می‌پذیرد.

### `POST /api/v1/skills/-/scan/batch/status`

مسیر وضعیت دسته‌ای متعارف، فقط برای مدیران. `{ "jobIds": ["..."] }` را می‌پذیرد و همان شمارنده‌های تجمیعی مسیر قدیمی `POST /api/v1/skills/-/rescan-batch/status` را برمی‌گرداند.

### `GET /api/v1/skills/{slug}/verify`

پوشش تأیید Skill Card را که توسط `clawhub skill verify` استفاده می‌شود برمی‌گرداند.

پارامترهای پرس‌وجو:

- `version` (اختیاری): رشته نسخه مشخص.
- `tag` (اختیاری): یک نسخه برچسب‌خورده را حل می‌کند (برای نمونه `latest`).

نکات:

- `ok` فقط زمانی `true` است که نسخه انتخاب‌شده یک Skill Card تولیدشده داشته باشد، توسط تعدیل به‌عنوان بدافزار مسدود نشده باشد، و تأیید ClawScan پاک باشد.
- هویت مهارت، هویت ناشر، و فراداده نسخه انتخاب‌شده فیلدهای سطح بالای پوشش هستند (`slug`، `displayName`، `publisherHandle`، `version`، `resolvedFrom`، `tag`، `createdAt`) تا اتوماسیون shell بتواند آن‌ها را بدون باز کردن بسته‌بندهای تو در تو بخواند.
...
- `provenance` فقط زمانی `server-resolved-github-import` است که ClawHub هنگام انتشار یا وارد کردن، repo/ref/commit/path مربوط به GitHub را حل و ذخیره کرده باشد؛ در غیر این صورت `unavailable` است.
...
- پاسخ فقط امنیتی است. شامل داده Skill Card، وضعیت کارت تولیدشده، فهرست فایل‌های مصنوعه، یا بارهای داده جزئی اسکنر نیست.
...
- `security.signals.dependencyRegistry` برای سازگاری پاسخ v1 حفظ شده است، اما اسکنر وجود رجیستری وابستگی بازنشسته شده و این کلید همیشه `null` است.
- نبود Skill Card بر `ok`، `decision`، یا `reasons` این نقطه پایانی اثری ندارد؛ کلاینت‌ها وقتی به محتوای کارت نیاز دارند باید `skill-card.md` نصب‌شده را به‌صورت محلی بخوانند.
- وقتی به پوشش تأیید Skill Card برای یک مهارت نیاز دارید از `/verify`، وقتی به Markdown کارت تولیدشده نیاز دارید از `/card`، و وقتی به داده دقیق اسکنر نیاز دارید از `/scan` استفاده کنید.
...
- ورودی‌های Skill همچنان توسط رجیستری Skill پشتیبانی می‌شوند و فقط از طریق `POST /api/v1/skills` قابل انتشار هستند.
...
- فراخوانندگان احراز هویت‌شده می‌توانند در نتایج فهرست/جست‌وجو بسته‌های خصوصی ناشرانی را ببینند که به آن‌ها تعلق دارند.
...
- فراخوانندگان ناشناس فقط کانال‌های بسته عمومی را می‌بینند.
- فراخوانندگان احراز هویت‌شده می‌توانند بسته‌های خصوصی ناشرانی را جست‌وجو کنند که به آن‌ها تعلق دارند.
- `channel=private` فقط بسته‌هایی را برمی‌گرداند که فراخواننده احراز هویت‌شده می‌تواند بخواند.
...
- `endDate` (الزامی): کران بالای بر حسب میلی‌ثانیه یونیکس برای `updatedAt` Skill.
...
- `_errors.json` زمانی گنجانده می‌شود که Skillها یا فایل‌های منفرد امکان
  خروجی‌گیری نداشته باشند.
...
- `startDate` (الزامی): کران پایین بر حسب میلی‌ثانیه یونیکس برای `updatedAt` در Plugin.
- `endDate` (الزامی): کران بالا بر حسب میلی‌ثانیه یونیکس برای `updatedAt` در Plugin.
...
- `_manifest.json` همیشه در ریشه ZIP گنجانده می‌شود.
- `_errors.json` وقتی گنجانده می‌شود که Pluginها یا فایل‌های منفرد قابل
...
- نام‌های مستعار فیلتر v1 قدیمی که زیر `GET /api/v1/plugins` مستند شده‌اند نیز
  پذیرفته می‌شوند.
- فیلتر دسته یک فیلتر واقعی API است که با ردیف‌های خلاصه دسته Plugin پشتیبانی
...
- کنترل‌های مرتب‌سازی رابط کاربری مرورگر برای جست‌وجوی Plugin نتایج ارتباط
  بارگذاری‌شده را بازمرتب می‌کنند و با رفتار فعلی مرور `/skills` مطابقت دارند.
...
- به توکن API برای مالک بسته، مالک/مدیر ناشر سازمانی، ناظر پلتفرم، یا مدیر
  پلتفرم نیاز دارد.
...
یک نسخه بسته را همراه با فراداده فایل، سازگاری، راستی‌آزمایی، فراداده آرتیفکت،
و داده‌های اسکن برمی‌گرداند.

یادداشت‌ها:

- `version.artifact.kind` برای آرشیوهای بسته قدیمی `legacy-zip` و برای
  انتشارهای مبتنی بر ClawPack برابر `npm-pack` است.
- انتشارهای ClawPack شامل فیلدهای سازگار با npm یعنی `npmIntegrity`،
  `npmShasum`، و `npmTarballName` هستند.
- `version.sha256hash` فراداده سازگاری منسوخ برای کلاینت‌های قدیمی است. این
  مقدار بایت‌های دقیق ZIP برگردانده‌شده توسط `/api/v1/packages/{name}/download`
  را هش می‌کند. کلاینت‌های مدرن باید از `version.artifact.sha256` استفاده کنند
  که آرتیفکت انتشار کانونی را شناسایی می‌کند.
- `version.vtAnalysis`، `version.llmAnalysis`، و `version.staticScan` وقتی داده
  اسکن وجود داشته باشد گنجانده می‌شوند.
- بسته‌های خصوصی `404` برمی‌گردانند، مگر اینکه فراخواننده بتواند ناشر مالک را بخواند.

### `GET /api/v1/packages/{name}/versions/{version}/security`

خلاصه دقیق امنیت و اعتماد انتشار بسته را برای کلاینت‌های نصب برمی‌گرداند. این
سطح مصرف عمومی OpenClaw برای تصمیم‌گیری درباره نصب‌پذیر بودن یک انتشار حل‌شده است.

احراز هویت:

- نقطه پایانی خواندن عمومی. توکن مالک، ناشر، ناظر، یا مدیر لازم نیست.
...
- `trust.moderationState` تهی‌پذیر است. وقتی نظارت دستی انتشار وجود نداشته
  باشد مقدار آن `null` است.
- `trust.blockedFromDownload` سیگنال مسدودسازی نصب است. OpenClaw و دیگر
  کلاینت‌های نصب باید وقتی این مقدار `true` است نصب را مسدود کنند، به‌جای آنکه
  قوانین مسدودسازی را دوباره از فیلدهای اسکنر یا نظارت استخراج کنند.
- `trust.reasons` فهرست توضیح کاربرپسند و حسابرسی است. کدهای دلیل رشته‌هایی
...
- `trust.stale` یعنی خلاصه اعتماد از ورودی‌های منسوخ محاسبه شده است و پیش از
  تصمیم اجازه با اطمینان بالا باید نیازمند تازه‌سازی تلقی شود.

یادداشت‌ها:

- این نقطه پایانی دقیق به نسخه است. کلاینت‌ها باید آن را پس از حل نسخه بسته‌ای
  که قصد نصبش را دارند فراخوانی کنند، نه صرفا پس از خواندن آخرین فراداده بسته.
- بسته‌های خصوصی `404` برمی‌گردانند، مگر اینکه فراخواننده بتواند ناشر مالک را بخواند.
- این نقطه پایانی عمدا از نقاط پایانی نظارت مالک/ناظر محدودتر است. تصمیم نصب و
  توضیح عمومی را افشا می‌کند، نه هویت گزارش‌دهندگان، متن گزارش‌ها، شواهد خصوصی،
  یا جدول‌های زمانی بازبینی داخلی.

### `GET /api/v1/packages/{name}/versions/{version}/artifact`

فراداده حل‌کننده آرتیفکت صریح را برای یک نسخه بسته برمی‌گرداند.
...
- نسخه‌های ClawPack یک آرتیفکت `npm-pack`، فیلدهای یکپارچگی npm، یک
  `tarballUrl`، و نشانی سازگاری ZIP قدیمی برمی‌گردانند.
- این سطح حل‌کننده OpenClaw است؛ از حدس‌زدن قالب آرشیو از یک URL مشترک جلوگیری
  می‌کند.

### `GET /api/v1/packages/{name}/versions/{version}/artifact/download`

آرتیفکت نسخه را از مسیر حل‌کننده صریح دانلود می‌کند.

یادداشت‌ها:

- نسخه‌های ClawPack بایت‌های دقیق `.tgz` بارگذاری‌شده npm-pack را جریان‌دهی می‌کنند.
- نسخه‌های ZIP قدیمی به `/api/v1/packages/{name}/download?version=` تغییرمسیر می‌دهند.
...
- دسترس‌پذیری آرتیفکت npm-pack در ClawPack
- خلاصه آرتیفکت
...
یک بسته را برای بازبینی ناظر گزارش می‌کند. گزارش‌ها در سطح بسته هستند و به‌صورت
...
برای تأیید، قرنطینه، یا لغو آرتیفکت‌ها استفاده کنند.
...
- به یک توکن API برای کاربر مدیر یا ادمین نیاز دارد.
...
نقطهٔ پایانی مالک/مدیر برای مشاهده‌پذیری نظارت بسته.

احراز هویت:

- به یک توکن API برای مالک بسته، عضو ناشر، مدیر، یا
...
نقطهٔ پایانی مدیر/ادمین برای حل‌وفصل یا بازگشایی گزارش‌های بسته.
...
- `quarantined`: در انتظار پیگیری مسدود شده است.
...
آرشیو ZIP قطعیِ قدیمی را برای انتشار یک بسته دانلود می‌کند.
...
- پاسخ‌ها شامل سربرگ‌های `ETag`، `Digest`، `X-ClawHub-Artifact-Type`، و
...
- packumentهای بستهٔ scoped از هر دو مسیر درخواست `/api/npm/@scope/name` و مسیر
...
- سربرگ‌های دانلود شامل SHA-256 مربوط به ClawHub به‌علاوهٔ فرادادهٔ integrity/shasum مربوط به npm هستند.
...
- `slug` (الزامی)
- `hash` (الزامی): sha256 هگز ۶۴نویسه‌ایِ اثرانگشت bundle
...
یک ZIP نسخهٔ skill میزبانی‌شده را دانلود می‌کند، یا برای یک skill فعلی مبتنی بر GitHub با اسکن `clean` یا `suspicious` و بدون نسخهٔ میزبانی‌شده، یک واگذاری منبع GitHub برمی‌گرداند.
...
- اگر نه `version` و نه `tag` ارائه شود، از آخرین نسخه استفاده می‌شود.
- نسخه‌های soft-deleteشده `410` برمی‌گردانند.
- واگذاری‌های skill مبتنی بر GitHub بایت‌ها را proxy یا mirror نمی‌کنند. پاسخ JSON
  ...
  و `archiveUrl` است؛ وضعیت scan/current یک gate است و به‌عنوان فرادادهٔ payload موفقیت
  گنجانده نمی‌شود.
...
- فیلد اختیاری payload: `migrateOwner`. وقتی همراه با `ownerHandle` مقدار `true` باشد،
  یک skill موجود می‌تواند به آن مالک منتقل شود، اگر actor روی ناشران فعلی و مقصد
  ادمین/مالک باشد. بدون این opt-in، تغییرات مالک رد می‌شوند.
...
- فیلدهای مجاز فرم `payload`، blobهای تکرارشوندهٔ `files`، یا یک ارجاع tarball
  `clawpack` هستند. `clawpack` می‌تواند یک blob با پسوند `.tgz` یا یک storage id باشد که توسط
  جریان upload-url برگردانده شده است. انتشارهای staged با storage-id باید همچنین
  `clawpackUploadTicket` برگردانده‌شده همراه آن URL بارگذاری را شامل شوند.
- از `files` یا `clawpack` استفاده کنید، هرگز هر دو را در یک درخواست واحد استفاده نکنید.
...
- فیلد اختیاری payload: `ownerHandle`. در صورت وجود، فقط ادمین‌ها می‌توانند از طرف آن مالک منتشر کنند.
...
- فقط ناشر org با نام `openclaw` و ناشران شخصیِ اعضای فعلی org با نام `openclaw`
  می‌توانند در کانال `official` منتشر کنند.
- انتشارهای ازطرف‌دیگری همچنان شایستگی کانال official را در برابر حساب مالک مقصد اعتبارسنجی می‌کنند.

### `DELETE /api/v1/skills/{slug}` / `POST /api/v1/skills/{slug}/undelete`

soft-delete / بازیابی یک skill (مالک، مدیر، یا ادمین).
...
در صورت وجود، `reason` به‌عنوان یادداشت نظارت skill ذخیره و در گزارش حسابرسی کپی می‌شود.
...
فقط ادمین. تضمین می‌کند یک ناشر org برای یک handle وجود دارد. اگر handle هنوز به یک
کاربر مشترک قدیمی/ناشر شخصی اشاره کند، نقطهٔ پایانی ابتدا آن را به یک ناشر org مهاجرت می‌دهد.
برای یک org تازه‌ساخته‌شده، `memberHandle` را ارائه کنید؛ ادمین عامل به‌عنوان عضو اضافه نمی‌شود.
...
ایجاد ناشر org به‌صورت self-serve احرازشده. یک ناشر org جدید ایجاد می‌کند و
...
فقط ادمین. slugهای ریشه و نام‌های بسته را برای مالک برحق بدون انتشار
...
فقط ادمین. یک ناشر شخصی را برای اصل OAuth جایگزین و تأییدشدهٔ GitHub
...
نقطه پایانی به‌صورت پیش‌فرض در حالت اجرای آزمایشی است. اعمال بازیابی پس از اینکه کارکنان پیوستگی بین هر دو شناسه اصلی GitHub را به‌طور مستقل تأیید کردند، به `dryRun: false` و
...
نام‌های مستعار نامک مهارت، بسته‌ها، هشدارهای بازرس بسته، و ردیف‌های چکیده جست‌وجوی مشتق‌شده منتقل می‌کند تا مسیرهای مالک مستقیم با مرجع ناشر جدید همخوان باشند. یک رزرو فعال دستگیره محافظت‌شده برای دستگیره بازیابی‌شده نیز به کاربر جایگزین واگذار می‌شود تا همگام‌سازی بعدی نمایه نتواند مرجع رقیب کاربر قبلی را بازگرداند. هر جدول اصلی در هر تراکنش اعمال به
...
- `rename` نامک قبلی را به‌عنوان نام مستعار تغییرمسیر حفظ می‌کند.
- `merge` فهرست منبع را پنهان می‌کند و نامک منبع را به فهرست هدف تغییرمسیر می‌دهد.
...
یک کاربر را مسدود می‌کند و مهارت‌های متعلق به او را به‌طور سخت حذف می‌کند (فقط مدیر/ناظر).
...
مسدودی یک کاربر را رفع می‌کند و مهارت‌های واجد شرایط را بازیابی می‌کند (فقط مدیر).
...
دلیل ذخیره‌شده برای یک مسدودی موجود را بدون رفع مسدودی یا بازیابی
محتوا تغییر می‌دهد (فقط مدیر). مگر اینکه `dryRun` برابر `false` باشد، به‌صورت پیش‌فرض در حالت اجرای آزمایشی است.
...
یک ستاره را اضافه/حذف می‌کند (برجسته‌سازی‌ها). هر دو نقطه پایانی idempotent هستند.
...
`POST /api/cli/upload-url` مقدارهای `uploadUrl` و `uploadTicket` را برمی‌گرداند. انتشارهای بسته‌ای که یک tarball از ClawPack را مرحله‌بندی می‌کنند باید شناسه ذخیره‌سازی حاصل را به‌عنوان
...
اگر خودمیزبان هستید، این فایل را ارائه کنید (یا `CLAWHUB_REGISTRY` را صراحتاً تنظیم کنید؛ `CLAWDHUB_REGISTRY` قدیمی است).
