...
x-i18n:
    generated_at: "2026-07-05T07:25:31Z"
    model: gpt-5.5
...
دایرکتوری‌های شخص ثالث می‌توانند از اندپوینت‌های خواندن عمومی برای فهرست‌کردن یا جست‌وجوی Skills در ClawHub استفاده کنند. لطفاً نتایج را cache کنید، `429`/`Retry-After` را رعایت کنید، کاربران را به فهرست رسمی ClawHub (`https://clawhub.ai/<owner>/skills/<slug>`) برگردانید، و از القای تأیید سایت شخص ثالث توسط ClawHub خودداری کنید. برای mirror کردن محتوای پنهان، خصوصی، یا مسدودشده به‌دلیل moderation خارج از سطح API عمومی تلاش نکنید.

میان‌برهای slug وب در میان خانواده‌های registry resolve می‌شوند، اما کلاینت‌های API باید به‌جای بازسازی اولویت route، از URLهای رسمی بازگردانده‌شده توسط اندپوینت‌های خواندن استفاده کنند.
...
- درخواست‌های احرازهویت‌شده (token معتبر Bearer): به‌ازای bucket هر کاربر اعمال می‌شود.
- اگر token وجود نداشته باشد یا نامعتبر باشد، رفتار به اعمال بر اساس IP برمی‌گردد.
- اندپوینت‌های نوشتن احرازهویت‌شده نباید وقتی سرور دلیل را می‌داند، یک `Unauthorized` خالی برگردانند. tokenهای گم‌شده، tokenهای نامعتبر/لغوشده، و حساب‌های حذف‌شده/مسدودشده/غیرفعال‌شده باید هرکدام متن قابل‌اقدام دریافت کنند تا کلاینت‌های CLI بتوانند به کاربران بگویند چه چیزی آن‌ها را مسدود کرده است.

- خواندن: 3000/min به‌ازای هر IP، 12000/min به‌ازای هر کلید
- نوشتن: 300/min به‌ازای هر IP، 3000/min به‌ازای هر کلید
- دانلود: 1200/min به‌ازای هر IP، 6000/min به‌ازای هر کلید (اندپوینت‌های دانلود)
...
- فقط زمانی از headerهای IP کلاینت مورد اعتماد، از جمله `cf-connecting-ip`، استفاده می‌کند که deployment به‌صراحت headerهای forwardشدهٔ مورد اعتماد را فعال کرده باشد.
- ClawHub از headerهای forwarding مورد اعتماد برای شناسایی IPهای کلاینت در edge استفاده می‌کند.
- اگر هیچ IP کلاینت مورد اعتمادی در دسترس نباشد، درخواست‌های ناشناس از bucketهای fallback استفاده می‌کنند که فقط بر اساس نوع rate-limit محدود شده‌اند. این bucketهای fallback شامل pathهای ارائه‌شده توسط فراخواننده، slugها، نام‌های package، نسخه‌ها، رشته‌های query، یا سایر پارامترهای artifact نمی‌شوند.

## پاسخ‌های خطا

پاسخ‌های خطای عمومی v1 متن ساده با `content-type: text/plain; charset=utf-8` هستند.
این شامل شکست‌های validation (`400`)، منابع عمومی گم‌شده (`404`)، شکست‌های auth و permission (`401`/`403`)، محدودیت‌های نرخ (`429`)، و دانلودهای مسدودشده است. کلاینت‌ها باید بدنهٔ پاسخ را به‌عنوان رشته‌ای قابل‌خواندن برای انسان بخوانند. پارامترهای query ناشناخته برای سازگاری نادیده گرفته می‌شوند، اما پارامترهای query شناخته‌شده با مقادیر نامعتبر `400` برمی‌گردانند.
...
- نتایج به‌ترتیب مرتبط‌بودن بازگردانده می‌شوند (شباهت embedding + تقویت‌های token دقیق slug/name + یک prior کوچک برای محبوبیت).
- مرتبط‌بودن از محبوبیت قوی‌تر است. تطابق دقیق token در slug یا display-name می‌تواند از تطابق آزادتر با engagement بسیار قوی‌تر رتبهٔ بالاتری بگیرد.
- متن ASCII روی مرزهای واژه و نشانه‌گذاری tokenized می‌شود. برای مثال، `personal-map` یک token مستقل `map` دارد، در حالی که `amap-jsapi-skill` شامل `amap`، `jsapi`، و `skill` است؛ بنابراین جست‌وجوی `map` به `personal-map` تطابق lexical قوی‌تری نسبت به `amap-jsapi-skill` می‌دهد.
- محبوبیت به‌صورت لگاریتمی مقیاس‌بندی و capped می‌شود. Skills با engagement بالا وقتی متن query تطابق ضعیف‌تری دارد، می‌توانند رتبهٔ پایین‌تری بگیرند.
...
- اصطلاحاتی را که کاربران به‌معنای واقعی جست‌وجو خواهند کرد در display name، summary، و tagها قرار دهید. فقط وقتی از یک token مستقل slug استفاده کنید که همان نیز هویت پایداری باشد که می‌خواهید نگه دارید.
- slug را فقط برای دنبال‌کردن یک query تغییر نام ندهید، مگر اینکه slug جدید نام رسمی بلندمدت بهتری باشد. slugهای قدیمی به aliasهای redirect تبدیل می‌شوند، اما URL رسمی، slug نمایش‌داده‌شده، و digestهای جست‌وجوی آینده از slug جدید استفاده می‌کنند.
- aliasهای تغییر نام resolution را برای URLهای قدیمی و نصب‌هایی که از طریق registry resolve می‌شوند حفظ می‌کنند، اما رتبه‌بندی جست‌وجو پس از index شدن تغییر نام، بر اساس metadata رسمی Skill است. آمار موجود با همان Skill باقی می‌ماند.
- اگر یک Skill به‌طور غیرمنتظره‌ای نامرئی است، پیش از تغییر metadata مرتبط با رتبه‌بندی، ابتدا وضعیت moderation را با `clawhub inspect @owner/slug` در حالت logged in بررسی کنید.
...
- وقتی `nonSuspiciousOnly=true` باشد، sortهای مبتنی بر cursor ممکن است در یک صفحه کمتر از `limit` آیتم برگردانند، چون Skills مشکوک پس از بازیابی صفحه filter می‌شوند.
...
- slugهای قدیمی ایجادشده توسط flowهای تغییر نام/merge مالک به Skill رسمی resolve می‌شوند.
...
- Evidence برای فراخواننده‌های عمومی redacted است و فقط برای مالکان/moderatorها شامل snippetهای خام می‌شود.

### `POST /api/v1/skills/{slug}/report`

گزارش یک Skill برای بررسی moderator. گزارش‌ها در سطح Skill هستند، به‌صورت اختیاری به یک نسخه linked می‌شوند، و صف گزارش Skill را تغذیه می‌کنند.
...
اندپوینت moderator/admin برای resolve کردن یا بازگشایی گزارش‌های Skill.
...
`note` برای `confirmed` و `dismissed` الزامی است؛ وقتی `status` دوباره روی `open` تنظیم می‌شود، می‌توان آن را حذف کرد. `finalAction: "hide"` را با یک گزارش triaged ارسال کنید تا Skill در همان workflow قابل‌ممیزی پنهان شود.
...
- `moderation` یک نمای لحظه‌ای فعلی از تعدیل در سطح مهارت است که از آخرین نسخه استخراج شده است.
- هنگام پرس‌وجوی یک نسخه تاریخی، پیش از در نظر گرفتن `moderation` و `security` به‌عنوان زمینه یک نسخه یکسان، `moderation.matchesRequestedVersion` و `moderation.sourceVersion` را بررسی کنید.
...
- محتوای درخواست‌های اسکن و گزارش‌های قابل دانلود پس از بازه نگهداری از مخزن درخواست اسکن منقضی می‌شوند.
- اسکن‌های منتشرشده به دسترسی مدیریتی مالک/ناشر، یا اختیار تعدیل‌گر/مدیر پلتفرم نیاز دارند.
...
- کارهای اسکن ناهمگام هستند. درخواست‌های اسکن دستی جلوتر از کارهای عادی انتشار/پس‌پرکردن اولویت می‌گیرند، اما تکمیل همچنان به دسترس‌بودن worker وابسته است.
...
- به دسترسی مدیریتی مالک/ناشر به مهارت یا Plugin، یا اختیار تعدیل‌گر/مدیر پلتفرم نیاز دارد.
- نتایج اسکن ذخیره‌شده را برای نسخه دقیق ارسال‌شده، از جمله نسخه‌های مسدود یا پنهان، برمی‌گرداند.
...
مسیر متعارف اسکن مجدد دسته‌ای فقط برای مدیر. همان شکل payload قدیمی `POST /api/v1/skills/-/rescan-batch` را می‌پذیرد.

### `POST /api/v1/skills/-/scan/batch/status`

مسیر متعارف وضعیت دسته‌ای فقط برای مدیر. `{ "jobIds": ["..."] }` را می‌پذیرد و همان شمارنده‌های تجمیعی قدیمی `POST /api/v1/skills/-/rescan-batch/status` را برمی‌گرداند.
...
- `tag` (اختیاری): حل‌کردن یک نسخه برچسب‌خورده (برای مثال `latest`).

نکته‌ها:

- `ok` فقط زمانی `true` است که نسخه انتخاب‌شده یک کارت مهارت تولیدشده داشته باشد، توسط تعدیل به‌دلیل بدافزار مسدود نشده باشد، و راستی‌آزمایی ClawScan پاک باشد.
- هویت مهارت، هویت ناشر و فراداده نسخه انتخاب‌شده، فیلدهای سطح بالای پاکت هستند (`slug`، `displayName`، `publisherHandle`، `version`، `resolvedFrom`، `tag`، `createdAt`) تا خودکارسازی shell بتواند آن‌ها را بدون بازکردن wrapperهای تودرتو بخواند.
...
نقطه پایانی مجموعه‌ای برای کلاینت‌هایی در نظر گرفته شده است که از قبل می‌دانند کدام
نسخه‌های مهارت ClawHub نصب‌شده را باید نمایش دهند، مانند OpenClaw Control UI.
...
- نتایج به‌ازای هر آیتم هستند؛ نبودن یک مهارت یا نسخه باعث شکست کل پاسخ نمی‌شود.
- پاسخ فقط امنیتی است. داده کارت مهارت، وضعیت کارت تولیدشده، فهرست فایل‌های artifact، یا payloadهای تفصیلی اسکنر را شامل نمی‌شود.
- `security.signals` فقط شامل شواهد پشتیبان در سطح وضعیت است؛ برای جزئیات کامل اسکنر از `/scan` یا صفحه بازرسی امنیتی ClawHub استفاده کنید.
- `security.signals.dependencyRegistry` برای سازگاری پاسخ v1 حفظ شده است، اما اسکنر وجود رجیستری وابستگی بازنشسته شده و این کلید همیشه `null` است.
...
- وقتی به پاکت راستی‌آزمایی کارت مهارت تک‌مهارتی نیاز دارید از `/verify` استفاده کنید، وقتی به markdown کارت تولیدشده نیاز دارید از `/card`، و وقتی به داده‌های تفصیلی اسکنر نیاز دارید از `/scan`.
...
در نقطه پایانی یکپارچه `/api/v1/packages` فقط برای Plugin است؛ برای کاتالوگ Skills از
...
خروجی دسته‌ای آخرین Skills عمومی برای تحلیل آفلاین.
...
- Skills فعلی مبتنی بر GitHub با اسکن `clean` یا `suspicious` شامل
  ...
  هش محتوا و URL آرشیو هستند. آن‌ها شامل فایل‌های مبدأ میزبانی‌شده در ClawHub نیستند.
...
- وقتی Pluginها یا فایل‌های منفرد قابل صدور نباشند، `_errors.json` قرار داده
  می‌شود.
...
- نام‌های مستعار قدیمی فیلتر v1 که ذیل `GET /api/v1/plugins` مستند شده‌اند نیز
  پذیرفته می‌شوند.
- فیلتر دسته‌بندی یک فیلتر واقعی API است که بر ردیف‌های چکیده دسته‌بندی Plugin
...
- کنترل‌های مرتب‌سازی رابط مرورگر برای جست‌وجوی Plugin نتایج مرتبط بارگذاری‌شده را
  بازمرتب می‌کنند و با رفتار مرور فعلی `/skills` هم‌خوان هستند.
...
- به توکن API برای مالک بسته، مالک/مدیر ناشر سازمانی، ناظر پلتفرم، یا مدیر پلتفرم
  نیاز دارد.
...
یک نسخه بسته را، شامل فراداده فایل، سازگاری، راستی‌آزمایی، فراداده artifact، و
داده‌های اسکن، برمی‌گرداند.

نکته‌ها:

- `version.artifact.kind` برای آرشیوهای بسته قدیمی `legacy-zip` یا برای انتشارهای
  مبتنی بر ClawPack مقدار `npm-pack` است.
...
- `version.sha256hash` فراداده سازگاری منسوخ برای کلاینت‌های قدیمی است. این مقدار
  همان بایت‌های دقیق ZIP برگردانده‌شده توسط `/api/v1/packages/{name}/download` را
  hash می‌کند. کلاینت‌های مدرن باید از `version.artifact.sha256` استفاده کنند که
  artifact انتشار canonical را مشخص می‌کند.
- `version.vtAnalysis`, `version.llmAnalysis`, و `version.staticScan` وقتی داده اسکن
  وجود داشته باشد قرار داده می‌شوند.
- بسته‌های خصوصی `404` برمی‌گردانند، مگر اینکه فراخوان بتواند ناشر مالک را بخواند.

### `GET /api/v1/packages/{name}/versions/{version}/security`

خلاصه دقیق امنیت و اعتماد انتشار بسته را برای کلاینت‌های نصب برمی‌گرداند. این
سطح مصرف عمومی OpenClaw برای تصمیم‌گیری درباره امکان نصب یک انتشار resolve‌شده
است.
...
- `release.artifactKind`, `release.artifactSha256`, `release.npmIntegrity`,
  `release.npmShasum`, و `release.npmTarballName` وقتی برای artifact انتشار شناخته
  شده باشند حاضر هستند.
- `trust.scanStatus` وضعیت مؤثر اعتماد است که از ورودی‌های اسکنر و ناظر دستی انتشار
  مشتق می‌شود.
- `trust.moderationState` می‌تواند null باشد. وقتی هیچ نظارت دستی انتشار وجود
  نداشته باشد، مقدار آن `null` است.
- `trust.blockedFromDownload` سیگنال مسدودسازی نصب است. OpenClaw و سایر کلاینت‌های
  نصب باید وقتی این مقدار `true` است نصب را مسدود کنند، نه اینکه قواعد مسدودسازی را
  دوباره از فیلدهای اسکنر یا نظارت استخراج کنند.
- `trust.reasons` فهرست توضیح کاربرمحور و ممیزی است. کدهای دلیل رشته‌هایی پایدار و
  فشرده مانند `manual:quarantined`, `scan:malicious`, و `package:malicious` هستند.
- `trust.pending` یعنی یک یا چند ورودی اعتماد هنوز در انتظار تکمیل هستند.
- `trust.stale` یعنی خلاصه اعتماد از ورودی‌های منسوخ محاسبه شده و باید پیش از تصمیم
  اجازه با اطمینان بالا، نیازمند تازه‌سازی تلقی شود.

نکته‌ها:

- این نقطه پایانی دقیقاً وابسته به نسخه است. کلاینت‌ها باید پس از resolve کردن نسخه
...
- نسخه‌های بسته legacy یک artifact از نوع `legacy-zip` و یک `downloadUrl` مربوط به
  ZIP legacy برمی‌گردانند.
- نسخه‌های ClawPack یک artifact از نوع `npm-pack`، فیلدهای integrity مربوط به npm،
  یک `tarballUrl`، و URL سازگاری ZIP legacy را برمی‌گردانند.
...
- نسخه‌های ClawPack همان بایت‌های دقیق `.tgz` بارگذاری‌شده npm-pack را stream
  می‌کنند.
- نسخه‌های ZIP legacy به `/api/v1/packages/{name}/download?version=` redirect
  می‌شوند.
...
- `bundledPluginId` به حروف کوچک normalize می‌شود و کلید پایدار upsert است.
- `packageName` به نام npm normalize می‌شود؛ برای مهاجرت‌های برنامه‌ریزی‌شده ممکن
  است بسته وجود نداشته باشد.
...
گزارش یک بسته برای بررسی ناظر. گزارش‌ها در سطح بسته هستند و به‌صورت اختیاری به یک
نسخه پیوند می‌خورند. آن‌ها صف نظارت را تغذیه می‌کنند، اما به‌تنهایی دانلودها را
به‌صورت خودکار پنهان یا مسدود نمی‌کنند؛ ناظران باید از نظارت انتشار برای تأیید،
قرنطینه، یا لغو artifactها استفاده کنند.
...
- به یک توکن API برای مالک بسته، عضو منتشرکننده، ناظر، یا کاربر مدیر نیاز
  دارد.
...
- آرشیوهای Plugin/بسته فایل‌های zip با ریشه `package/` هستند تا کلاینت‌های
  قدیمی OpenClaw همچنان کار کنند.
- این مسیر فقط ZIP باقی می‌ماند. فایل‌های ClawPack با پسوند `.tgz` را استریم نمی‌کند.
- پاسخ‌ها برای بررسی‌های یکپارچگی resolver شامل سرآیندهای `ETag`، `Digest`،
...
- فقط نسخه‌هایی که tarballهای npm-pack متعلق به ClawPack برای آن‌ها بارگذاری شده‌اند فهرست می‌شوند.
...
- packumentهای بسته scoped هم از مسیر درخواست `/api/npm/@scope/name` و هم از مسیر
  رمزگذاری‌شده npm یعنی `/api/npm/@scope%2Fname` پشتیبانی می‌کنند.
...
- `slug` (الزامی)
- `hash` (الزامی): sha256 شانزده‌شانزدهی ۶۴ نویسه‌ای اثرانگشت bundle
...
یک ZIP نسخه skill میزبانی‌شده را دانلود می‌کند، یا برای یک skill فعلی
پشتیبانی‌شده با GitHub که اسکن `clean` یا `suspicious` دارد و نسخه میزبانی‌شده
ندارد، یک handoff منبع GitHub برمی‌گرداند.
...
- handoffهای skill پشتیبانی‌شده با GitHub بایت‌ها را proxy یا mirror نمی‌کنند. پاسخ JSON
  شامل `sourceRef: "public-github"`، `repo`، `commit`، `path`، `contentHash`
  و `archiveUrl` است؛ وضعیت اسکن/فعلی یک gate است و به‌عنوان فراداده payload
  موفقیت گنجانده نمی‌شود.
- آمار دانلود به‌صورت هویت‌های یکتا در هر روز UTC شمارش می‌شود (`userId` وقتی توکن API معتبر است، وگرنه IP).
...
- فیلد اختیاری payload: `migrateOwner`. وقتی همراه با `ownerHandle` برابر `true` باشد،
  اگر actor روی منتشرکننده فعلی و هدف مدیر/مالک باشد، یک skill موجود می‌تواند به آن مالک
  منتقل شود. بدون این opt-in، تغییرات مالک رد می‌شوند.
...
- فیلدهای مجاز فرم عبارت‌اند از `payload`، blobهای تکرارشونده `files`، یا یک ارجاع tarball
  به نام `clawpack`. `clawpack` می‌تواند یک blob با پسوند `.tgz` یا یک شناسه ذخیره‌سازی باشد
  که توسط جریان upload-url برگردانده شده است. انتشارهای staged مبتنی بر storage-id باید
  `clawpackUploadTicket` برگردانده‌شده همراه با آن URL بارگذاری را نیز شامل شوند.
...
- فیلد اختیاری payload: `ownerHandle`. هنگام حضور، فقط مدیران می‌توانند به نمایندگی از آن مالک منتشر کنند.
...
- انتشارهای از طرف دیگران همچنان شایستگی کانال رسمی را نسبت به حساب مالک هدف اعتبارسنجی می‌کنند.
...
منتشرکننده دیگری claim شود. پاسخ حذف وقتی این انقضا اعمال شود شامل `slugReservedUntil` است.
...
کاربر مشترک قدیمی/منتشرکننده شخصی اشاره کند، نقطه پایانی ابتدا آن را به یک منتشرکننده org
مهاجرت می‌دهد. برای org تازه‌ساخته‌شده، `memberHandle` را ارائه کنید؛ مدیر اجراکننده به‌عنوان عضو اضافه نمی‌شود.
...
فراخواننده را به‌عنوان مالک اضافه می‌کند. این نقطه پایانی handleهای کاربر/شخصی موجود را مهاجرت
نمی‌دهد و منتشرکننده را trusted/official علامت‌گذاری نمی‌کند.
...
فقط مدیر. یک منتشرکننده شخصی را برای principal جایگزین تأییدشده GitHub OAuth
بدون ویرایش ردیف‌های حساب Convex Auth بازیابی می‌کند. درخواست باید هر دو شناسه حساب provider
تغییرناپذیر GitHub را نام ببرد؛ handleهای تغییرپذیر فقط به‌عنوان guard رو‌به‌روی operator استفاده می‌شوند.

نقطهٔ پایانی به‌طور پیش‌فرض در حالت اجرای آزمایشی است. اعمال بازیابی پس از آنکه کارکنان به‌طور مستقل پیوستگی بین هر دو هویت اصلی GitHub را تأیید کردند، به `dryRun: false` و
`confirmIdentityVerified: true` نیاز دارد. اگر منتشرکنندهٔ شخصی فعلی کاربر مقصد دارای مهارت‌ها، بسته‌ها، یا منابع مهارت GitHub باشد، بازیابی به‌صورت بسته و ایمن شکست می‌خورد.
بازیابی همچنین فیلدهای قدیمی `ownerUserId` را برای مهارت‌های منتشرکنندهٔ بازیابی‌شده،
نام‌های مستعار slug مهارت، بسته‌ها، هشدارهای بازرس بسته، و ردیف‌های مشتق‌شدهٔ خلاصهٔ جست‌وجو منتقل می‌کند تا
مسیرهای مالک مستقیم با مرجعیت منتشرکنندهٔ جدید هم‌خوان شوند. رزرو فعال handle محافظت‌شده
برای handle بازیابی‌شده نیز به کاربر جایگزین واگذار می‌شود تا همگام‌سازی‌های بعدی
پروفایل نتوانند مرجعیت رقیب کاربر قبلی را بازگردانند. هر جدول اصلی در هر تراکنش اعمال به
100 ردیف محدود است؛ بازیابی‌های بزرگ‌تر باید ابتدا از مهاجرت مالک قابل‌ازسرگیری استفاده کنند.
منابع مهارت GitHub در محدودهٔ منتشرکننده هستند و به‌جای بازنویسی، به‌عنوان بررسی‌شده گزارش می‌شوند.
...
یک کاربر را مسدود می‌کند و مهارت‌های متعلق به او را به‌صورت سخت حذف می‌کند (فقط ناظر/مدیر).
...
مسدودیت کاربر را برمی‌دارد و مهارت‌های واجد شرایط را بازیابی می‌کند (فقط مدیر).
...
محتوا تغییر می‌دهد (فقط مدیر). مگر اینکه `dryRun` برابر `false` باشد، به‌طور پیش‌فرض در حالت اجرای آزمایشی است.
...
یک ستاره (برجسته‌سازی) اضافه/حذف می‌کند. هر دو نقطهٔ پایانی idempotent هستند.
...
`POST /api/cli/upload-url` مقدارهای `uploadUrl` و `uploadTicket` را برمی‌گرداند. انتشار بسته‌هایی که یک tarball از ClawPack را در مرحله قرار می‌دهند، باید شناسهٔ ذخیره‌سازی حاصل را به‌عنوان
