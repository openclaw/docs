...
x-i18n:
    generated_at: "2026-07-01T15:27:52Z"
    model: gpt-5.5
...
دایرکتوری‌های شخص ثالث می‌توانند از اندپوینت‌های خواندن عمومی برای فهرست‌کردن یا جست‌وجوی Skills در ClawHub استفاده کنند. لطفاً نتایج را cache کنید، `429`/`Retry-After` را رعایت کنید، کاربران را به فهرست canonical در ClawHub (`https://clawhub.ai/<owner>/skills/<slug>`) برگردانید، و از القای تأیید سایت شخص ثالث توسط ClawHub خودداری کنید. تلاش نکنید محتوای پنهان، خصوصی، یا مسدودشده توسط moderation را خارج از سطح API عمومی mirror کنید.

میان‌برهای web slug در خانواده‌های registry resolve می‌شوند، اما کلاینت‌های API باید به‌جای بازسازی تقدم route، از URLهای canonical برگردانده‌شده توسط اندپوینت‌های خواندن استفاده کنند.

## محدودیت‌های نرخ

مدل اعمال:

- درخواست‌های ناشناس: به‌ازای IP اعمال می‌شود.
- درخواست‌های احرازشده (توکن Bearer معتبر): به‌ازای bucket کاربر اعمال می‌شود.
- اگر توکن وجود نداشته باشد یا نامعتبر باشد، رفتار به اعمال بر اساس IP برمی‌گردد.
- اندپوینت‌های نوشتن احرازشده، وقتی سرور دلیل را می‌داند، نباید فقط یک `Unauthorized` خام برگردانند. توکن‌های مفقود، توکن‌های نامعتبر/باطل‌شده، و حساب‌های حذف‌شده/مسدودشده/غیرفعال باید هرکدام متن قابل‌اقدامی دریافت کنند تا کلاینت‌های CLI بتوانند به کاربران بگویند چه چیزی مانع آن‌ها شده است.

- خواندن: 3000/min به‌ازای IP، 12000/min به‌ازای key
- نوشتن: 300/min به‌ازای IP، 3000/min به‌ازای key
- دانلود: 1200/min به‌ازای IP، 6000/min به‌ازای key (اندپوینت‌های دانلود)
...
- `RateLimit-Reset`: ثانیه‌ها تا reset (تأخیر)
...
- `Retry-After`: ثانیه‌های انتظار پیش از retry (تأخیر) روی `429`
...
- برای جلوگیری از retryهای هم‌زمان، از backoff با jitter استفاده کنید.
- اگر `Retry-After` وجود ندارد، به `RateLimit-Reset` fallback کنید (یا از `X-RateLimit-Reset` محاسبه کنید).

منبع IP:

- فقط وقتی deployment به‌صراحت trusted forwarded headers را فعال کرده باشد، از headerهای IP کلاینت مورد اعتماد، از جمله `cf-connecting-ip`، استفاده می‌کند.
- ClawHub از trusted forwarding headers برای شناسایی IPهای کلاینت در edge استفاده می‌کند.
- اگر IP کلاینت مورد اعتماد در دسترس نباشد، درخواست‌های ناشناس از bucketهای fallback استفاده می‌کنند که فقط بر اساس نوع rate-limit scope شده‌اند. این bucketهای fallback شامل مسیرهای ارائه‌شده توسط caller، slugها، نام packageها، versionها، query stringها، یا دیگر پارامترهای artifact نمی‌شوند.

## پاسخ‌های خطا

پاسخ‌های خطای عمومی v1 متن ساده با `content-type: text/plain; charset=utf-8` هستند.
این شامل شکست‌های validation (`400`)، منابع عمومی مفقود (`404`)، شکست‌های auth و permission (`401`/`403`)، محدودیت‌های نرخ (`429`)، و دانلودهای مسدودشده است. کلاینت‌ها باید بدنهٔ پاسخ را به‌عنوان رشته‌ای human-readable بخوانند. پارامترهای query ناشناخته برای سازگاری نادیده گرفته می‌شوند، اما پارامترهای query شناخته‌شده با مقادیر نامعتبر `400` برمی‌گردانند.

## اندپوینت‌های عمومی (بدون auth)
...
- نتایج به ترتیب relevance برگردانده می‌شوند (شباهت embedding + boostهای token دقیق slug/name + یک prior کوچک popularity).
- Relevance قوی‌تر از popularity است. تطبیق دقیق slug یا token نام نمایشی می‌تواند از تطبیق آزادتر با engagement بسیار قوی‌تر رتبهٔ بالاتری بگیرد.
- متن ASCII بر اساس مرزهای واژه و punctuation token می‌شود. برای مثال، `personal-map` شامل token مستقل `map` است، در حالی که `amap-jsapi-skill` شامل `amap`، `jsapi`، و `skill` است؛ بنابراین جست‌وجوی `map` برای `personal-map` یک تطبیق lexical قوی‌تر از `amap-jsapi-skill` می‌دهد.
- Popularity به‌صورت log-scale است و سقف دارد. Skills با engagement بالا می‌توانند وقتی متن query تطبیق ضعیف‌تری دارد، رتبهٔ پایین‌تری بگیرند.
- وضعیت moderation مشکوک یا پنهان می‌تواند بسته به فیلترهای caller و وضعیت moderation فعلی، یک Skill را از جست‌وجوی عمومی حذف کند.

راهنمای discoverability ناشر:

- اصطلاحاتی را که کاربران واقعاً جست‌وجو خواهند کرد در نام نمایشی، خلاصه، و tagها بگذارید. فقط وقتی از token مستقل slug استفاده کنید که همان نیز هویت پایداری باشد که می‌خواهید نگه دارید.
- فقط برای دنبال‌کردن یک query، slug را تغییر نام ندهید مگر اینکه slug جدید نام canonical بلندمدت بهتری باشد. slugهای قدیمی به redirect alias تبدیل می‌شوند، اما URL canonical، slug نمایش‌داده‌شده، و search digestهای آینده از slug جدید استفاده می‌کنند.
- Aliasهای تغییر نام resolution را برای URLهای قدیمی و installهایی که از طریق registry resolve می‌شوند حفظ می‌کنند، اما رتبه‌بندی جست‌وجو پس از index شدن تغییر نام، بر اساس metadata canonical Skill است. آمار موجود با Skill می‌ماند.
- اگر یک Skill به‌طور غیرمنتظره نامرئی است، پیش از تغییر metadata مرتبط با ranking، ابتدا وضعیت moderation را با `clawhub inspect @owner/slug` در حالت logged in بررسی کنید.
...
- وقتی `nextCursor` وجود دارد، از آن برای ادامهٔ صفحه‌بندی استفاده کنید. یک صفحهٔ کوتاه به‌تنهایی به معنای پایان نتایج نیست.
...
- اگر Skill هیچ platform metadata نداشته باشد، `metadata` برابر `null` است.
- `moderation` فقط وقتی Skill flag شده باشد یا مالک آن را مشاهده کند، شامل می‌شود.
...
- Evidence برای callerهای عمومی redacted است و فقط برای مالکان/moderatorها شامل snippetهای خام می‌شود.

### `POST /api/v1/skills/{slug}/report`

گزارش یک Skill برای بازبینی moderator. گزارش‌ها در سطح Skill هستند، به‌صورت اختیاری به یک version لینک می‌شوند، و queue گزارش Skill را تغذیه می‌کنند.
...
اندپوینت moderator/admin برای دریافت گزارش‌های Skill.
...
اندپوینت moderator/admin برای حل‌وفصل یا بازگشایی گزارش‌های Skill.
...
`note` برای `confirmed` و `dismissed` الزامی است؛ هنگام تنظیم دوبارهٔ `status` به `open` می‌توان آن را حذف کرد. برای پنهان‌کردن Skill در همان workflow قابل audit، همراه با یک گزارش triageشده `finalAction: "hide"` را پاس دهید.
...
- `version.security` شامل وضعیت scan verification نرمال‌شده و جزئیات scanner است
  (VirusTotal + LLM)، وقتی در دسترس باشد.

### `GET /api/v1/skills/{slug}/scan`

جزئیات security scan verification را برای یک version از Skill برمی‌گرداند.

پارامترهای query:

- `version` (اختیاری): رشتهٔ version مشخص.
- `tag` (اختیاری): یک version دارای tag را resolve می‌کند (برای مثال `latest`).
...
- `moderation` یک نمای فوری فعلی از moderation در سطح Skill است که از آخرین نسخه مشتق شده است.
- هنگام پرس‌وجو دربارهٔ یک نسخهٔ تاریخی، پیش از اینکه `moderation` و `security` را در یک زمینهٔ نسخه‌ای یکسان در نظر بگیرید، `moderation.matchesRequestedVersion` و `moderation.sourceVersion` را بررسی کنید.

### `POST /api/v1/skills/-/scan`

نقطهٔ ارسال احرازهویت‌شده برای کارهای جدید ClawScan.
...
- payloadهای درخواست اسکن و گزارش‌های قابل دانلود پس از پنجرهٔ نگه‌داری از مخزن درخواست اسکن منقضی می‌شوند.
...
- کارهای اسکن ناهمگام هستند. درخواست‌های اسکن دستی جلوتر از کارهای عادی انتشار/backfill اولویت‌بندی می‌شوند، اما تکمیل همچنان به در دسترس بودن worker بستگی دارد.

### `GET /api/v1/skills/-/scan/{scanId}`

نقطهٔ poll احرازهویت‌شده برای یک اسکن ارسال‌شده.
...
نقطهٔ آرشیو گزارش احرازهویت‌شده.
...
نقطهٔ آرشیو گزارش ذخیره‌شدهٔ احرازهویت‌شده برای نسخه‌های ارسال‌شده.
...
مسیر canonical فقط مخصوص admin برای اسکن مجدد دسته‌ای. همان شکل payload مسیر legacy `POST /api/v1/skills/-/rescan-batch` را می‌پذیرد.

### `POST /api/v1/skills/-/scan/batch/status`

مسیر canonical فقط مخصوص admin برای وضعیت دسته‌ای. `{ "jobIds": ["..."] }` را می‌پذیرد و همان شمارنده‌های تجمیعی مسیر legacy `POST /api/v1/skills/-/rescan-batch/status` را برمی‌گرداند.

### `GET /api/v1/skills/{slug}/verify`

envelope تأیید Skill Card مورد استفادهٔ `clawhub skill verify` را برمی‌گرداند.

پارامترهای query:

- `version` (اختیاری): رشتهٔ نسخهٔ مشخص.
- `tag` (اختیاری): resolve کردن یک نسخهٔ tagشده (برای مثال `latest`).

نکات:

- `ok` فقط زمانی `true` است که نسخهٔ انتخاب‌شده یک Skill Card تولیدشده داشته باشد، توسط moderation به‌عنوان malware مسدود نشده باشد، و تأیید ClawScan پاک باشد.
- هویت Skill، هویت ناشر، و فرادادهٔ نسخهٔ انتخاب‌شده فیلدهای سطح بالای envelope هستند (`slug`، `displayName`، `publisherHandle`، `version`، `resolvedFrom`، `tag`، `createdAt`) تا خودکارسازی shell بتواند آن‌ها را بدون باز کردن wrapperهای تودرتو بخواند.
...
- `security.signals.dependencyRegistry` برای سازگاری پاسخ v1 نگه داشته شده است، اما اسکنر وجود dependency registry بازنشسته شده و این کلید همیشه `null` است.
...
نقطهٔ collection برای کلاینت‌هایی در نظر گرفته شده که از قبل می‌دانند کدام
نسخه‌های نصب‌شدهٔ ClawHub skill را باید نمایش دهند، مانند OpenClaw Control UI.
...
- نتایج برای هر آیتم جداگانه هستند؛ نبودن یک Skill یا نسخه باعث شکست کل پاسخ نمی‌شود.
- پاسخ فقط امنیتی است. شامل دادهٔ Skill Card، وضعیت کارت تولیدشده، فهرست فایل‌های artifact، یا payloadهای تفصیلی اسکنر نیست.
...
- `security.signals.dependencyRegistry` برای سازگاری پاسخ v1 نگه داشته شده است، اما اسکنر وجود dependency registry بازنشسته شده و این کلید همیشه `null` است.
- نبود Skill Card روی `ok`، `decision`، یا `reasons` این endpoint تأثیری ندارد؛ کلاینت‌ها هنگامی که به محتوای کارت نیاز دارند باید `skill-card.md` نصب‌شده را به‌صورت محلی بخوانند.
- هنگامی که به envelope تأیید Skill Card برای یک Skill نیاز دارید از `/verify` استفاده کنید، هنگامی که به markdown کارت تولیدشده نیاز دارید از `/card` استفاده کنید، و هنگامی که به دادهٔ تفصیلی اسکنر نیاز دارید از `/scan` استفاده کنید.
...
- `category` (اختیاری): فیلتر دسته‌بندی Plugin. فقط زمانی پشتیبانی می‌شود که
  ...
  زیر `GET /api/v1/plugins` مستند شده‌اند.
...
مرور کاتالوگ فقط مخصوص Plugin در بسته‌های code-plugin و bundle-plugin.
...
نام‌های مستعار قدیمی فیلتر v1 همچنان در نقاط پایانی خواندنی پذیرفته می‌شوند:
...
`trending` یک جدول رتبه‌بندی نصب/دانلود هفت‌روزه است و از مجموع‌های همه‌زمانه استفاده نمی‌کند.
...
- `startDate` (الزامی): حد پایین میلی‌ثانیه یونیکس برای `updatedAt` مربوط به Skill.
- `endDate` (الزامی): حد بالای میلی‌ثانیه یونیکس برای `updatedAt` مربوط به Skill.
...
خروجی‌گیری انبوه از آخرین انتشارهای عمومی Plugin برای تحلیل آفلاین.
...
- هر Plugin خروجی‌گرفته‌شده شامل فایل‌های ذخیره‌شدهٔ آخرین انتشار است.
- فرادادهٔ خروجی‌گیری برای هر Plugin در
  `__clawhub_export/{family}/{packageName}/plugin_meta.json` ذخیره می‌شود.
- `_manifest.json` همیشه در ریشهٔ ZIP گنجانده می‌شود.
- `_errors.json` زمانی گنجانده می‌شود که بعضی Pluginها یا فایل‌ها قابل
  خروجی‌گیری نبوده‌اند.
...
- `limit` (اختیاری): عدد صحیح (1-100)
- `isOfficial` (اختیاری): `true` یا `false`
- `category` (اختیاری): پالایهٔ دسته‌بندی Plugin. مقادیر فعلی:
  `channels`، `models`، `memory`، `context`، `voice`، `media`، `web`،
  `tools`، `runtime`، `gateway`، `security`، `other`.

نکته‌ها:

- نام‌های مستعار پالایهٔ v1 قدیمی که زیر `GET /api/v1/plugins` مستند شده‌اند نیز
  پذیرفته می‌شوند.
- پالایش دسته‌بندی یک پالایهٔ واقعی API است که با ردیف‌های خلاصهٔ دسته‌بندی Plugin
  پشتیبانی می‌شود، نه بازنویسی پرس‌وجوی جست‌وجو.
- نتایج به ترتیب ارتباط بازگردانده می‌شوند و در حال حاضر صفحه‌بندی ندارند.
- کنترل‌های مرتب‌سازی رابط مرورگر برای جست‌وجوی Plugin، نتایج ارتباط بارگذاری‌شده را بازچینی می‌کنند،
...
یک بسته و همهٔ انتشارها را soft-delete می‌کند.
...
راستی‌آزمایی، فرادادهٔ artifact و دادهٔ اسکن برمی‌گرداند.

نکته‌ها:

- `version.artifact.kind` برای آرشیوهای بستهٔ دنیای قدیمی `legacy-zip` یا
...
- `version.sha256hash` فرادادهٔ سازگاری منسوخ برای کلاینت‌های قدیمی است. این مقدار
...
- `version.vtAnalysis`، `version.llmAnalysis` و `version.staticScan` زمانی
  گنجانده می‌شوند که دادهٔ اسکن وجود داشته باشد.
- بسته‌های خصوصی `404` برمی‌گردانند مگر اینکه فراخواننده بتواند ناشر مالک را بخواند.

### `GET /api/v1/packages/{name}/versions/{version}/security`

خلاصهٔ دقیق امنیت و اعتماد انتشار بسته را برای کلاینت‌های نصب برمی‌گرداند.
این سطح مصرف عمومی OpenClaw برای تصمیم‌گیری دربارهٔ اینکه آیا یک انتشار
resolveشده قابل نصب است یا نه.

احراز هویت:

- نقطهٔ پایانی خواندن عمومی. هیچ توکن مالک، ناشر، ناظر، یا مدیر
  لازم نیست.
...
- `package.name`، `package.displayName` و `package.family` بستهٔ رجیستری
...
- `release.artifactKind`، `release.artifactSha256`، `release.npmIntegrity`،
  `release.npmShasum` و `release.npmTarballName` زمانی که برای artifact
...
- `trust.blockedFromDownload` سیگنال مسدودسازی نصب است. OpenClaw و سایر
  کلاینت‌های نصب باید وقتی این مقدار `true` است، به‌جای استخراج دوبارهٔ قواعد مسدودسازی
  از فیلدهای اسکنر یا نظارت، نصب را مسدود کنند.
...
- `trust.stale` یعنی خلاصهٔ اعتماد از ورودی‌های قدیمی محاسبه شده و
  پیش از تصمیم اجازه‌دهی با اطمینان بالا باید نیازمند نوسازی تلقی شود.

نکته‌ها:

- این نقطهٔ پایانی دقیقاً وابسته به نسخه است. کلاینت‌ها باید پس از resolve کردن
  نسخهٔ بسته‌ای که قصد نصب آن را دارند آن را فراخوانی کنند، نه فقط پس از خواندن آخرین
  فرادادهٔ بسته.
- بسته‌های خصوصی `404` برمی‌گردانند مگر اینکه فراخواننده بتواند ناشر مالک را بخواند.
- این نقطهٔ پایانی عمداً محدودتر از نقطه‌های پایانی نظارت مالک/ناظر است.
...
فرادادهٔ resolver صریح artifact را برای یک نسخهٔ بسته برمی‌گرداند.

نکته‌ها:

- نسخه‌های بستهٔ قدیمی یک artifact از نوع `legacy-zip` و یک
...
آمادگی محاسبه‌شده برای مصرف آیندهٔ OpenClaw را برمی‌گرداند.
...
- `bundledPluginId` به حروف کوچک عادی‌سازی می‌شود و کلید upsert پایدار است.
- `packageName` به نام npm عادی‌سازی می‌شود؛ بسته می‌تواند برای مهاجرت‌های برنامه‌ریزی‌شده
  وجود نداشته باشد.
...
روند قابل ممیزی، همراه با گزارش تأییدشده `finalAction: "quarantine"` یا
`finalAction: "revoke"` ارسال کنید.
...
- `revoked`: پس از آنکه یک انتشار قبلاً مورد اعتماد بود، مسدود شده است.
...
- پاسخ‌ها برای بررسی‌های یکپارچگی resolver شامل سربرگ‌های `ETag`، `Digest`، `X-ClawHub-Artifact-Type`، و
...
برای نسخه‌های بستهٔ پشتیبانی‌شده با ClawPack، یک packument سازگار با npm برمی‌گرداند.
...
- packumentهای بستهٔ دارای scope هم از `/api/npm/@scope/name` و هم از مسیر درخواست
...
- `slug` (الزامی)
- `hash` (الزامی): sha256 هگز 64 کاراکتری از اثرانگشت باندل
...
نسخهٔ ZIP میزبانی‌شدهٔ یک skill را دانلود می‌کند، یا برای یک skill فعلی مبتنی بر GitHub
با اسکن `clean` یا `suspicious` و بدون نسخهٔ میزبانی‌شده، handoff منبع GitHub برمی‌گرداند.
...
- handoffهای skill مبتنی بر GitHub بایت‌ها را پروکسی یا آینه نمی‌کنند. پاسخ JSON
...
- فیلد اختیاری payload: `ownerHandle`. وقتی حاضر باشد، API آن ناشر را در سمت سرور resolve می‌کند
  و از بازیگر می‌خواهد دسترسی ناشر داشته باشد.
- فیلد اختیاری payload: `migrateOwner`. وقتی همراه با `ownerHandle` برابر `true` باشد، اگر
  بازیگر روی هر دو ناشر فعلی و هدف مدیر/مالک باشد، یک skill موجود می‌تواند به آن مالک منتقل شود.
...
- فیلدهای فرم مجاز عبارت‌اند از `payload`، blobهای تکرارشوندهٔ `files`، یا یک ارجاع tarball
  به نام `clawpack`. `clawpack` می‌تواند یک blob با پسوند `.tgz` یا یک شناسهٔ ذخیره‌سازی باشد که
  توسط جریان upload-url برگردانده شده است. انتشارهای مرحله‌بندی‌شده با storage-id باید همچنین
...
وقتی حاضر باشد، `reason` به‌عنوان یادداشت نظارت skill ذخیره و در لاگ ممیزی کپی می‌شود.
...
فقط مدیر. تضمین می‌کند برای یک handle یک ناشر سازمانی وجود دارد. اگر handle همچنان به یک
کاربر مشترک قدیمی/ناشر شخصی اشاره کند، این نقطهٔ پایانی ابتدا آن را به یک ناشر سازمانی مهاجرت می‌دهد.
برای سازمان تازه‌ساخته‌شده، `memberHandle` ارائه کنید؛ مدیرِ عامل به‌عنوان عضو اضافه نمی‌شود.
...
ایجاد ناشر سازمانی به‌صورت self-serve برای کاربر احرازشده. یک ناشر سازمانی جدید ایجاد می‌کند و
فراخواننده را به‌عنوان مالک اضافه می‌کند. این نقطهٔ پایانی handleهای موجود کاربری/شخصی را مهاجرت نمی‌دهد و
ناشر را مورد اعتماد/رسمی علامت‌گذاری نمی‌کند.
...
حساب provider GitHub را نام ببرد؛ handleهای قابل تغییر فقط به‌عنوان نگهبان روبه‌روی اپراتور استفاده می‌شوند.

نقطهٔ پایانی به‌طور پیش‌فرض روی dry-run تنظیم است. اعمال بازیابی نیازمند `dryRun: false` و
`confirmIdentityVerified: true` است، پس از آن‌که کارکنان به‌طور مستقل پیوستگی بین هر دو
اصل GitHub را تأیید کنند. وقتی ناشر شخصی فعلی کاربر مقصد Skills، بسته‌ها، یا منابع Skill در GitHub داشته باشد، بازیابی به‌صورت بسته شکست می‌خورد.
...
مسیرهای مالک مستقیم با اختیار ناشر جدید هم‌خوان شوند. یک رزرو فعالِ دستهٔ محافظت‌شده
برای دستهٔ بازیابی‌شده نیز به کاربر جایگزین واگذار می‌شود تا همگام‌سازی‌های بعدی
پروفایل نتوانند اختیار رقیبِ کاربر پیشین را بازگردانند. هر جدول اصلی در هر تراکنش اعمال به
۱۰۰ ردیف محدود است؛ بازیابی‌های بزرگ‌تر باید ابتدا از یک مهاجرت مالک قابل‌ازسرگیری استفاده کنند.
منابع Skill در GitHub در محدودهٔ ناشر هستند و به‌جای بازنویسی، به‌صورت بررسی‌شده گزارش می‌شوند.

- بدنه: `{ "handle": "gingiris", "nextUserHandle": "gingiris-1031", "previousGitHubProviderAccountId": "123", "nextGitHubProviderAccountId": "456", "reason": "پیوستگی حساب برای issue #2555 تأیید شد", "confirmIdentityVerified": true, "dryRun": false }`
- پاسخ: `{ "ok": true, "dryRun": false, "recovered": true, "publisherId": "...", "handle": "gingiris", "previousUser": { "userId": "...", "handle": "gingiris", "nextHandle": "gingiris-recovered", "githubProviderAccountId": "123", "authAccountCount": 1 }, "nextUser": { "userId": "...", "handle": "gingiris-1031", "nextHandle": "gingiris", "githubProviderAccountId": "456", "authAccountCount": 1 }, "retiredPersonalPublisher": null, "resourceOwnerMigration": { "limitPerTable": 100, "skills": 1, "skillSlugAliases": 1, "packages": 0, "packageInspectorWarnings": 0, "githubSourcesChecked": 1, "handleReservations": 1 }, "identityVerified": true, "reason": "پیوستگی حساب برای issue #2555 تأیید شد" }`
...
- `merge` فهرست مبدأ را پنهان می‌کند و اسلاگ مبدأ را به فهرست هدف تغییرمسیر می‌دهد.

### نقاط پایانی انتقال مالکیت

- `POST /api/v1/skills/{slug}/transfer`
  - بدنه: `{ "toUserHandle": "target_handle", "message": "اختیاری" }`
...
- `GET /api/v1/transfers/outgoing`
  - شکل پاسخ: `{ "transfers": [{ "_id": "...", "skill": { "slug": "demo", "displayName": "نمونه" }, "fromUser"|"toUser": { "handle": "..." }, "message": "...", "requestedAt": 0, "expiresAt": 0 }] }`

### `POST /api/v1/users/ban`

مسدود کردن یک کاربر و حذف سخت Skills تحت مالکیت او (فقط مدیر ارشد/ناظر).
...
```json
{ "handle": "user_handle", "reason": "دلیل اختیاری رفع مسدودی" }
```

یا

```json
{ "userId": "users_...", "reason": "دلیل اختیاری رفع مسدودی" }
```
...
محتوا (فقط مدیر ارشد). مگر این‌که `dryRun` برابر `false` باشد، به‌طور پیش‌فرض dry-run است.
...
```json
{
  ...
  "userId": "users_...",
  "handle": "user_handle",
  "previousReason": "مسدودسازی خودکار به‌دلیل بدافزار",
  "nextReason": "هرزنامهٔ انتشار انبوه",
  "changed": true
}
```
...
```json
{ "handle": "user_handle", "role": "ناظر" }
```

یا

```json
{ "userId": "users_...", "role": "مدیر ارشد" }
```
...
که یک tarball از ClawPack را مرحله‌بندی می‌کنند، باید شناسهٔ ذخیره‌سازی حاصل را به‌صورت
`clawpack` و تیکت برگشتی را به‌صورت `clawpackUploadTicket` ارسال کنند.
