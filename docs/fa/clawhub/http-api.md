...
دایرکتوری‌های شخص ثالث می‌توانند از endpointهای خواندنی عمومی برای فهرست‌کردن یا جست‌وجوی Skills در ClawHub استفاده کنند. لطفاً نتایج را cache کنید، `429`/`Retry-After` را رعایت کنید، کاربران را به فهرست canonical ClawHub (`https://clawhub.ai/<owner>/skills/<slug>`) برگردانید، و از القای تأیید سایت شخص ثالث توسط ClawHub خودداری کنید. تلاش نکنید محتوای پنهان، خصوصی، یا مسدودشده توسط moderation را بیرون از سطح API عمومی mirror کنید.

میان‌برهای slug وب در میان خانواده‌های registry resolve می‌شوند، اما clientهای API باید از URLهای canonical برگردانده‌شده توسط endpointهای خواندنی استفاده کنند، نه اینکه اولویت route را بازسازی کنند.
...
- endpointهای نوشتن احراز هویت‌شده، وقتی server دلیل را می‌داند، نباید یک `Unauthorized` خام برگردانند. tokenهای ناموجود، tokenهای نامعتبر/لغوشده، و حساب‌های حذف‌شده/مسدودشده/غیرفعال‌شده باید هرکدام متن قابل اقدام دریافت کنند تا clientهای CLI بتوانند به کاربران بگویند چه چیزی مانع آن‌ها شده است.
...
- سازگاری legacy: `X-RateLimit-Limit`، `X-RateLimit-Reset`
...
- `X-RateLimit-Remaining` / `RateLimit-Remaining`: بودجهٔ باقی‌ماندهٔ دقیق، وقتی وجود داشته باشد.
  درخواست‌های موفق sharded این header را حذف می‌کنند، به‌جای اینکه مقدار جهانی تقریبی برگردانند.
- `Retry-After`: ثانیه‌هایی که باید پیش از retry در `429` صبر کرد (delay)
...
- فقط وقتی deployment به‌صراحت headerهای forwarded مورد اعتماد را فعال می‌کند، از headerهای IP client مورد اعتماد، از جمله `cf-connecting-ip`، استفاده می‌کند.
- ClawHub از headerهای forwarding مورد اعتماد برای شناسایی IPهای client در edge استفاده می‌کند.
- اگر IP client مورد اعتمادی در دسترس نباشد، درخواست‌های ناشناس از bucketهای fallback استفاده می‌کنند که فقط با نوع rate-limit scoped شده‌اند. این bucketهای fallback شامل pathها، slugها، نام packageها، versionها، query stringها، یا دیگر پارامترهای artifact ارائه‌شده توسط caller نمی‌شوند.

## پاسخ‌های خطا

پاسخ‌های خطای عمومی v1 متن ساده با `content-type: text/plain; charset=utf-8` هستند.
این شامل شکست‌های validation (`400`)، منابع عمومی ناموجود (`404`)، شکست‌های auth و permission (`401`/`403`)، محدودیت‌های نرخ (`429`)، و دانلودهای مسدودشده می‌شود. clientها باید بدنهٔ پاسخ را به‌عنوان یک رشتهٔ قابل‌خواندن برای انسان بخوانند. پارامترهای query ناشناخته برای سازگاری نادیده گرفته می‌شوند، اما پارامترهای query شناخته‌شده با مقدارهای نامعتبر `400` برمی‌گردانند.
...
- نتایج به ترتیب ارتباط برگردانده می‌شوند (شباهت embedding + تقویت‌های exact برای tokenهای slug/name + یک prior کوچک محبوبیت).
- ارتباط از محبوبیت قوی‌تر است. یک تطابق دقیق token در slug یا display-name می‌تواند از تطابق آزادتر با engagement بسیار قوی‌تر بالاتر قرار بگیرد.
- متن ASCII در مرزهای واژه و نشانه‌گذاری tokenسازی می‌شود. برای مثال، `personal-map` شامل یک token مستقل `map` است، درحالی‌که `amap-jsapi-skill` شامل `amap`، `jsapi`، و `skill` است؛ بنابراین جست‌وجوی `map` به `personal-map` تطابق lexical قوی‌تری نسبت به `amap-jsapi-skill` می‌دهد.
- محبوبیت log-scaled و capped است. Skills با engagement بالا ممکن است وقتی متن query تطابق ضعیف‌تری دارد، رتبهٔ پایین‌تری بگیرند.
...
- اصطلاحاتی را که کاربران دقیقاً جست‌وجو خواهند کرد در display name، summary، و tags قرار دهید. فقط وقتی از یک token مستقل slug استفاده کنید که همان‌طور یک هویت پایدار هم باشد که می‌خواهید نگه دارید.
...
- aliasهای rename، resolution را برای URLهای قدیمی و نصب‌هایی که از طریق registry resolve می‌شوند حفظ می‌کنند، اما رتبه‌بندی search بر اساس metadata canonical skill پس از index شدن rename است. آمارهای موجود با skill باقی می‌مانند.
- اگر skill به‌طور غیرمنتظره نامرئی است، پیش از تغییر metadata مرتبط با ranking، ابتدا وضعیت moderation را با `clawhub inspect @owner/slug` در حالت logged in بررسی کنید.
...
- `sort` (اختیاری): `updated` (پیش‌فرض)، `recommended` (alias: `default`)، `createdAt` (alias: `newest`)، `downloads`، `stars` (alias: `rating`)، aliasهای نصب legacy `installsCurrent`/`installs`/`installsAllTime` به `downloads` map می‌شوند، `trending`
- `nonSuspiciousOnly` (اختیاری): `true` برای پنهان‌کردن Skills مشکوک (`flagged.suspicious`)
- `nonSuspicious` (اختیاری): alias قدیمی برای `nonSuspiciousOnly`
...
- وقتی `nonSuspiciousOnly=true`، sortهای مبتنی بر cursor ممکن است در یک صفحه کمتر از `limit` آیتم برگردانند، چون Skills مشکوک بعد از بازیابی صفحه filter می‌شوند.
...
- callerهای عمومی فقط برای Skills نمایانِ از قبل flagged شده `200` می‌گیرند.
- Evidence برای callerهای عمومی redacted می‌شود و فقط برای مالکان/moderatorها شامل snippetهای خام است.

### `POST /api/v1/skills/{slug}/report`

گزارش‌کردن یک skill برای review توسط moderator. گزارش‌ها در سطح skill هستند، به‌صورت اختیاری به یک version لینک می‌شوند، و queue گزارش‌های skill را تغذیه می‌کنند.
...
`note` برای `confirmed` و `dismissed` الزامی است؛ هنگام برگرداندن `status` به `open` می‌توان آن را حذف کرد. برای پنهان‌کردن skill در همان workflow قابل audit، `finalAction: "hide"` را همراه با یک گزارش triage‌شده ارسال کنید.
...
- وضعیت راستی‌آزمایی نرمال‌سازی‌شده به‌همراه جزئیات اختصاصی اسکنر را شامل می‌شود.
...
- هنگام پرس‌وجوی یک نسخه تاریخی، پیش از اینکه `moderation` و `security` را در زمینه نسخه یکسان در نظر بگیرید، `moderation.matchesRequestedVersion` و `moderation.sourceVersion` را بررسی کنید.
...
- کارهای اسکن ناهمگام هستند. درخواست‌های اسکن دستی پیش از کار عادی انتشار/بازپرکردن اولویت‌بندی می‌شوند، اما تکمیل همچنان به در دسترس بودن worker وابسته است.
...
- در زمان در صف بودن، `queue.queuedAhead` و `queue.position` را برمی‌گرداند تا کلاینت‌ها بتوانند نشان دهند چند اسکن دستی اولویت‌دار پیش از درخواست قرار دارد. صف‌های بسیار بزرگ محدود می‌شوند و با `queuedAheadIsEstimate: true` گزارش می‌شوند.
- وقتی موجود باشد، `report` شامل بخش‌های `clawscan`، `skillspector`، `staticAnalysis` و `virustotal` است.
...
- `provenance` فقط زمانی `server-resolved-github-import` است که ClawHub در زمان انتشار یا import یک مخزن/رفرنس/کامیت/مسیر GitHub را حل و ذخیره کرده باشد؛ در غیر این صورت `unavailable` است.
...
- نتایج برای هر مورد جداگانه هستند؛ نبود یک مهارت یا نسخه کل پاسخ را ناموفق نمی‌کند.
- پاسخ فقط امنیتی است. داده کارت مهارت، وضعیت کارت تولیدشده، فهرست‌های فایل artifact، یا محموله‌های تفصیلی اسکنر را شامل نمی‌شود.
- `security.signals` فقط شواهد پشتیبان در سطح وضعیت را شامل می‌شود؛ برای جزئیات کامل اسکنر از `/scan` یا صفحه ممیزی امنیتی ClawHub استفاده کنید.
- `security.signals.dependencyRegistry` برای سازگاری پاسخ v1 حفظ شده است، اما اسکنر وجود رجیستری وابستگی بازنشسته شده و این کلید همیشه `null` است.
- نبود کارت مهارت بر `ok`، `decision` یا `reasons` این نقطه پایانی اثر نمی‌گذارد؛ وقتی کلاینت‌ها به محتوای کارت نیاز دارند، باید `skill-card.md` نصب‌شده را به‌صورت محلی بخوانند.
- وقتی به پاکت راستی‌آزمایی کارت مهارت برای یک مهارت نیاز دارید از `/verify` استفاده کنید، وقتی به Markdown کارت تولیدشده نیاز دارید از `/card`، و وقتی به داده تفصیلی اسکنر نیاز دارید از `/scan`.
...
- ورودی‌های Skill همچنان پشتوانه‌شان رجیستری Skill است و فقط از طریق `POST /api/v1/skills` قابل انتشار هستند.
...
`trending` جدول برترین‌های نصب/دانلود هفت‌روزه است و از مجموع‌های همه‌زمان استفاده نمی‌کند.
...
- `startDate` (الزامی): حد پایین بر حسب میلی‌ثانیه Unix برای `updatedAt` مربوط به Plugin.
- `endDate` (الزامی): حد بالا بر حسب میلی‌ثانیه Unix برای `updatedAt` مربوط به Plugin.
- `limit` (اختیاری): عدد صحیح (1-250)، پیش‌فرض `250`.
...
- `_manifest.json` همیشه در ریشه ZIP گنجانده می‌شود.
- وقتی Pluginها یا فایل‌های منفرد قابل صدور نباشند، `_errors.json` گنجانده می‌شود.
...
- نام‌های مستعار فیلتر v1 قدیمی که زیر `GET /api/v1/plugins` مستند شده‌اند نیز
  پذیرفته می‌شوند.
- فیلترکردن دسته‌بندی یک فیلتر واقعی API است که توسط ردیف‌های چکیده دسته‌بندی Plugin پشتیبانی می‌شود،
...
- `version.sha256hash` فراداده سازگاری منسوخ برای کلاینت‌های قدیمی است. این مقدار
  بایت‌های دقیق ZIP برگشتی توسط `/api/v1/packages/{name}/download` را هش می‌کند.
...
انتشار resolveشده قابل نصب هست یا نه.
...
- `trust.moderationState` nullable است. وقتی هیچ نظارت دستی انتشاری
...
- `trust.stale` یعنی خلاصه اعتماد از ورودی‌های قدیمی محاسبه شده و
  پیش از یک تصمیم allow با اطمینان بالا باید نیازمند تازه‌سازی در نظر گرفته شود.

نکات:

- این endpoint دقیقاً وابسته به نسخه است. کلاینت‌ها باید آن را پس از resolve کردن
...
- این endpoint عمداً از endpointهای نظارت مالک/ناظر محدودتر است.
  ...
  هویت گزارش‌دهندگان، متن گزارش‌ها، شواهد خصوصی، یا جدول‌های زمانی بررسی داخلی.
...
نقطه پایانی مالک/ناظر برای مشاهده وضعیت مدیریت بسته.
...
تنظیم دوباره `status` به `open` می‌توان آن را حذف کرد. برای اعمال مدیریت انتشار در همان گردش کار قابل ممیزی، همراه با یک گزارش تأییدشده
`finalAction: "quarantine"` یا `finalAction: "revoke"` را ارسال کنید.
...
- به‌طور پیش‌فرض از آخرین انتشار استفاده می‌کند.
- Skills به `GET /api/v1/download` بازهدایت می‌شوند.
...
- پاسخ‌ها برای بررسی‌های یکپارچگی حل‌کننده شامل سرآیندهای `ETag`، `Digest`، `X-ClawHub-Artifact-Type`، و
...
- فقط نسخه‌هایی که tarballهای npm-pack مربوط به ClawPack در آن‌ها بارگذاری شده‌اند فهرست می‌شوند.
...
- بررسی‌های مدیریت و دسترسی بسته خصوصی همچنان اعمال می‌شوند.
...
- `slug` (الزامی)
- `hash` (الزامی): sha256 شانزده‌شانزدهی 64 کاراکتری از اثرانگشت bundle
...
یک ZIP نسخه Skill میزبانی‌شده را دانلود می‌کند، یا برای یک Skill فعلی مبتنی بر GitHub با اسکن `clean` یا `suspicious` و بدون نسخه میزبانی‌شده،
یک واگذاری منبع GitHub برمی‌گرداند.
...
- فیلد اختیاری payload: `migrateOwner`. وقتی همراه با `ownerHandle` مقدار `true` باشد،
  یک Skill موجود می‌تواند به آن مالک منتقل شود، اگر actor روی هر دو
  ناشر فعلی و مقصد admin/owner باشد. بدون این opt-in، تغییرات مالک
  رد می‌شوند.
...
- فیلدهای مجاز فرم عبارت‌اند از `payload`، blobهای تکرارشونده `files`، یا یک ارجاع tarball با نام `clawpack`.
  `clawpack` می‌تواند یک blob با پسوند `.tgz` یا یک شناسه storage باشد که توسط
  جریان upload-url برگردانده شده است. انتشارهای staging شده با storage-id باید همچنین
  `clawpackUploadTicket` برگشتی همراه با آن URL بارگذاری را شامل شوند.
- از `files` یا `clawpack` استفاده کنید، هرگز هر دو را در یک درخواست استفاده نکنید.
...
در صورت وجود، `reason` به‌عنوان یادداشت مدیریت Skill ذخیره می‌شود و در گزارش ممیزی کپی می‌شود.
...
فقط مدیر. تضمین می‌کند برای یک handle ناشر سازمان وجود دارد. اگر handle هنوز به یک
کاربر/ناشر شخصی مشترک قدیمی اشاره کند، نقطه پایانی ابتدا آن را به یک ناشر سازمان مهاجرت می‌دهد.
...
ایجاد self-serve ناشر سازمانِ احراز هویت‌شده. یک ناشر سازمان جدید ایجاد می‌کند و
...
فقط مدیر. slugهای ریشه و نام‌های بسته را برای مالک قانونی، بدون انتشار یک
انتشار، رزرو می‌کند. نام‌های بسته به بسته‌های placeholder خصوصی بدون سطرهای انتشار تبدیل می‌شوند، تا همان
...
فقط مدیر. یک ناشر شخصی را برای principal جایگزین و تأییدشده GitHub OAuth
بدون ویرایش سطرهای حساب Convex Auth بازیابی می‌کند. درخواست باید هر دو شناسه immutable حساب
provider مربوط به GitHub را نام ببرد؛ handleهای mutable فقط به‌عنوان guard روبه‌روی operator استفاده می‌شوند.

نقطهٔ پایانی به‌صورت پیش‌فرض روی اجرای آزمایشی تنظیم است. اعمال بازیابی پس از آن‌که کارکنان به‌طور مستقل تداوم بین هر دو شناسهٔ اصلی GitHub را تأیید کردند، به `dryRun: false` و
`confirmIdentityVerified: true` نیاز دارد. اگر ناشر شخصی فعلی کاربر مقصد Skills، بسته‌ها یا منابع Skills در GitHub داشته باشد، بازیابی به‌صورت بسته شکست می‌خورد.
...
برای دستهٔ بازیابی‌شده نیز به کاربر جایگزین بازتخصیص داده می‌شود تا همگام‌سازی بعدی
نمایه نتواند مرجع رقیب کاربر پیشین را بازیابی کند. هر جدول اصلی در هر تراکنش اعمال به
۱۰۰ ردیف محدود است؛ بازیابی‌های بزرگ‌تر باید ابتدا از مهاجرت مالک قابل‌ازسرگیری استفاده کنند.
منابع Skills در GitHub در محدودهٔ ناشر هستند و به‌جای بازنویسی، به‌عنوان بررسی‌شده گزارش می‌شوند.
...
- `rename` اسلاگ پیشین را به‌عنوان نام مستعار تغییرمسیر حفظ می‌کند.
...
یک کاربر را مسدود کنید و Skills تحت مالکیت او را به‌صورت حذف سخت حذف کنید (فقط ناظر/مدیر).
...
محتوا تغییر دهید (فقط مدیر). مگر این‌که `dryRun` برابر `false` باشد، پیش‌فرض اجرای آزمایشی است.
...
نقش کاربر را تغییر دهید (فقط مدیر).
...
فهرست کاربران را نمایش دهید یا در آن جست‌وجو کنید (فقط مدیر).
...
CLI می‌تواند تنظیمات Registry/احراز هویت را از سایت کشف کند:
