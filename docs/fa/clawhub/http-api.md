...
x-i18n:
    generated_at: "2026-07-04T20:37:40Z"
    model: gpt-5.5
...
دایرکتوری‌های شخص ثالث می‌توانند از نقاط پایانی خواندنی عمومی برای فهرست‌کردن یا جست‌وجوی Skills های ClawHub استفاده کنند. لطفاً نتایج را cache کنید، `429`/`Retry-After` را رعایت کنید، کاربران را به فهرست canonical ClawHub (`https://clawhub.ai/<owner>/skills/<slug>`) پیوند دهید، و از القای تأیید سایت شخص ثالث توسط ClawHub خودداری کنید. تلاش نکنید محتوای پنهان، خصوصی، یا مسدودشده توسط moderation را بیرون از سطح API عمومی آینه کنید.

میان‌برهای slug وب در خانواده‌های registry resolve می‌شوند، اما کلاینت‌های API باید به‌جای بازسازی اولویت مسیرها، از URLهای canonical بازگردانده‌شده توسط نقاط پایانی خواندنی استفاده کنند.
...
- نقاط پایانی نوشتن احراز هویت‌شده نباید وقتی سرور دلیل را می‌داند، فقط یک `Unauthorized` خام برگردانند. توکن‌های مفقود، توکن‌های نامعتبر/لغوشده، و حساب‌های حذف‌شده/مسدودشده/غیرفعال هرکدام باید متن قابل‌اقدام دریافت کنند تا کلاینت‌های CLI بتوانند به کاربران بگویند چه چیزی مانع آن‌ها شده است.
...
- `Retry-After`: تعداد ثانیه‌های انتظار پیش از تلاش دوباره (تأخیر) روی `429`
...
- از سرآیندهای IP کلاینت مورد اعتماد، از جمله `cf-connecting-ip`، فقط زمانی استفاده می‌کند که deployment صراحتاً سرآیندهای forwarded مورد اعتماد را فعال کرده باشد.
- ClawHub از سرآیندهای forwarding مورد اعتماد برای شناسایی IP کلاینت‌ها در edge استفاده می‌کند.
- اگر IP کلاینت مورد اعتمادی در دسترس نباشد، درخواست‌های ناشناس از bucketهای fallback استفاده می‌کنند که فقط بر اساس نوع محدودیت نرخ scoped شده‌اند. این bucketهای fallback شامل مسیرهای ارائه‌شده توسط caller، slugها، نام‌های package، نسخه‌ها، query stringها، یا دیگر پارامترهای artifact نمی‌شوند.
...
- `highlightedOnly` (اختیاری): `true` برای فیلترکردن به Skills های highlighted
...
- ارتباط از محبوبیت قوی‌تر است. تطابق دقیق token برای slug یا display-name می‌تواند از یک تطابق آزادتر با engagement بسیار قوی‌تر رتبهٔ بالاتری بگیرد.
- متن ASCII بر اساس مرزهای واژه و نشانه‌گذاری token می‌شود. برای مثال، `personal-map` شامل یک token مستقل `map` است، درحالی‌که `amap-jsapi-skill` شامل `amap`، `jsapi`، و `skill` است؛ بنابراین جست‌وجوی `map` به `personal-map` یک تطابق واژگانی قوی‌تر از `amap-jsapi-skill` می‌دهد.
- محبوبیت با log مقیاس‌بندی و capped می‌شود. Skills های با engagement بالا وقتی متن query تطابق ضعیف‌تری داشته باشد، می‌توانند رتبهٔ پایین‌تری بگیرند.
...
- اصطلاحاتی را که کاربران واقعاً جست‌وجو خواهند کرد در display name، summary، و tags بگذارید. از token مستقل slug فقط وقتی استفاده کنید که هویت پایداری هم هست که می‌خواهید نگه دارید.
...
- aliasهای rename امکان resolve را برای URLهای قدیمی و نصب‌هایی که از طریق registry resolve می‌شوند حفظ می‌کنند، اما رتبه‌بندی جست‌وجو پس از index شدن rename بر اساس metadata canonical Skill است. آمار موجود با Skill باقی می‌ماند.
...
- وقتی `nonSuspiciousOnly=true` باشد، sortهای مبتنی بر cursor ممکن است در یک صفحه کمتر از `limit` آیتم برگردانند، چون Skills های مشکوک پس از بازیابی صفحه فیلتر می‌شوند.
...
- `metadata.systems`: هدف‌های system در Nix (برای مثال `["aarch64-darwin", "x86_64-linux"]`). اگر اعلام نشده باشد `null`.
- اگر Skill هیچ metadata پلتفرمی نداشته باشد، `metadata` برابر `null` است.
- `moderation` فقط زمانی شامل می‌شود که Skill flagged باشد یا مالک در حال مشاهدهٔ آن باشد.
...
- مالکان و moderators می‌توانند به جزئیات moderation برای Skills های پنهان دسترسی داشته باشند.
- callerهای عمومی فقط برای Skills های visible که از قبل flagged شده‌اند `200` دریافت می‌کنند.
...
نقطهٔ پایانی moderator/admin برای resolve کردن یا reopening گزارش‌های Skill.
...
`note` برای `confirmed` و `dismissed` الزامی است؛ هنگام تنظیم دوبارهٔ `status` به `open` می‌توان آن را حذف کرد. برای پنهان‌کردن Skill در همان workflow قابل audit، همراه یک گزارش triaged مقدار `finalAction: "hide"` را ارسال کنید.
...
- `tag` (اختیاری): یک نسخهٔ tagged را resolve کنید (برای مثال `latest`).

یادداشت‌ها:

- اگر نه `version` و نه `tag` ارائه شده باشد، از آخرین نسخه استفاده می‌کند.
- شامل وضعیت راستی‌آزمایی نرمال‌شده به‌همراه جزئیات ویژه‌ی اسکنر است.
- `security.hasScanResult` فقط زمانی `true` است که یک اسکنر حکم قطعی تولید کرده باشد (`clean`، `suspicious`، یا `malicious`).
- `moderation` یک نمای فعلی از تعدیل در سطح Skill است که از آخرین نسخه مشتق شده است.
- هنگام پرس‌وجوی یک نسخه‌ی تاریخی، پیش از اینکه `moderation` و `security` را در یک زمینه‌ی نسخه‌ای یکسان در نظر بگیرید، `moderation.matchesRequestedVersion` و `moderation.sourceVersion` را بررسی کنید.

### `POST /api/v1/skills/-/scan`

نقطه‌ی پایانی ثبت احراز هویت‌شده برای کارهای جدید ClawScan.
...
- بدنه‌های درخواست اسکن و گزارش‌های قابل دانلود پس از بازه‌ی نگه‌داری از مخزن درخواست اسکن منقضی می‌شوند.
...
- کارهای اسکن ناهمگام هستند. درخواست‌های اسکن دستی پیش از کارهای انتشار/پرکردن‌مجدد عادی اولویت‌بندی می‌شوند، اما تکمیل همچنان به در دسترس بودن worker وابسته است.

### `GET /api/v1/skills/-/scan/{scanId}`

نقطه‌ی پایانی نظرسنجی احراز هویت‌شده برای یک اسکن ثبت‌شده.
...
- در صورت دسترس بودن، `report` شامل بخش‌های `clawscan`، `skillspector`، `staticAnalysis`، و `virustotal` است.
- کارهای اسکن ناموفق `status: "failed"` را همراه با `lastError` برمی‌گردانند.

### `GET /api/v1/skills/-/scan/{scanId}/download`

نقطه‌ی پایانی آرشیو گزارش احراز هویت‌شده.
...
نقطه‌ی پایانی آرشیو گزارش ذخیره‌شده‌ی احراز هویت‌شده برای نسخه‌های ثبت‌شده.

- به دسترسی مدیریتی مالک/ناشر به Skill یا Plugin، یا اختیار ناظر/مدیر پلتفرم نیاز دارد.
- نتایج اسکن ذخیره‌شده برای نسخه‌ی دقیق ثبت‌شده، از جمله نسخه‌های مسدودشده یا پنهان، را برمی‌گرداند.
...
مسیر بازاسکن دسته‌ای canonical فقط برای مدیران. این مسیر همان شکل بدنه‌ی مسیر قدیمی `POST /api/v1/skills/-/rescan-batch` را می‌پذیرد.

### `POST /api/v1/skills/-/scan/batch/status`

مسیر وضعیت دسته‌ای canonical فقط برای مدیران. این مسیر `{ "jobIds": ["..."] }` را می‌پذیرد و همان شمارنده‌های تجمیعی مسیر قدیمی `POST /api/v1/skills/-/rescan-batch/status` را برمی‌گرداند.

### `GET /api/v1/skills/{slug}/verify`

پاکت راستی‌آزمایی Skill Card مورد استفاده‌ی `clawhub skill verify` را برمی‌گرداند.

پارامترهای پرس‌وجو:

- `version` (اختیاری): رشته‌ی نسخه‌ی مشخص.
- `tag` (اختیاری): یک نسخه‌ی برچسب‌خورده را resolve می‌کند (برای مثال `latest`).
...
- هویت Skill، هویت ناشر، و فراداده‌ی نسخه‌ی انتخاب‌شده فیلدهای سطح بالای پاکت هستند (`slug`، `displayName`، `publisherHandle`، `version`، `resolvedFrom`، `tag`، `createdAt`) تا اتوماسیون shell بتواند آن‌ها را بدون باز کردن wrapperهای تودرتو بخواند.
- `security` حکم سطح بالای ClawScan/امنیت است. اتوماسیون باید بر اساس `ok`، `decision`، `reasons`، و `security.status` تصمیم بگیرد.
...
حکم‌های امنیتی فشرده‌ی فعلی را برای نسخه‌های دقیق Skill برمی‌گرداند. این
نقطه‌ی پایانی مجموعه برای کلاینت‌هایی در نظر گرفته شده که از قبل می‌دانند کدام نسخه‌های
Skill نصب‌شده‌ی ClawHub را باید نمایش دهند، مانند OpenClaw Control UI.
...
- نتایج به‌ازای هر مورد هستند؛ نبود یک Skill یا نسخه باعث شکست کل پاسخ نمی‌شود.
- پاسخ فقط امنیتی است. این پاسخ شامل داده‌های Skill Card، وضعیت کارت تولیدشده، فهرست فایل‌های artifact، یا بدنه‌های تفصیلی اسکنر نیست.
- `security.signals` فقط شامل شواهد پشتیبان در سطح وضعیت است؛ برای جزئیات کامل اسکنر از `/scan` یا صفحه‌ی ممیزی امنیتی ClawHub استفاده کنید.
- `security.signals.dependencyRegistry` برای سازگاری پاسخ v1 حفظ شده است، اما اسکنر وجود رجیستری وابستگی بازنشسته شده و این کلید همیشه `null` است.
- نبود Skill Card روی `ok`، `decision`، یا `reasons` این نقطه‌ی پایانی اثر نمی‌گذارد؛ کلاینت‌ها وقتی به محتوای کارت نیاز دارند باید `skill-card.md` نصب‌شده را به‌صورت محلی بخوانند.
- وقتی به پاکت راستی‌آزمایی Skill Card برای یک Skill نیاز دارید از `/verify` استفاده کنید، وقتی به markdown کارت تولیدشده نیاز دارید از `/card` استفاده کنید، و وقتی به داده‌های تفصیلی اسکنر نیاز دارید از `/scan` استفاده کنید.
...
- فراخواننده‌های احرازهویت‌شده می‌توانند بسته‌های خصوصی ناشرانی را که به آن‌ها تعلق دارند در نتایج فهرست/جست‌وجو ببینند.
- `channel=private` فقط بسته‌هایی را برمی‌گرداند که فراخواننده احرازهویت‌شده می‌تواند بخواند.
...
- فراخواننده‌های ناشناس فقط کانال‌های بسته عمومی را می‌بینند.
- فراخواننده‌های احرازهویت‌شده می‌توانند بسته‌های خصوصی ناشرانی را که به آن‌ها تعلق دارند جست‌وجو کنند.
- `channel=private` فقط بسته‌هایی را برمی‌گرداند که فراخواننده احرازهویت‌شده می‌تواند بخواند.
...
`trending` یک جدول رتبه‌بندی نصب/دانلود هفت‌روزه است و از مجموع‌های همه‌زمان استفاده نمی‌کند.
...
- زمانی که skills یا فایل‌های جداگانه قابل صدور نباشند،
...
- کنترل‌های مرتب‌سازی رابط کاربری مرورگر برای جست‌وجوی Plugin نتایج بارگذاری‌شده
  بر اساس ارتباط را بازمرتب می‌کنند، مطابق با رفتار مرور فعلی `/skills`.
...
راستی‌آزمایی، فراداده artifact، و داده‌های اسکن را برمی‌گرداند.
...
- `version.sha256hash` فراداده سازگاری منسوخ‌شده برای کلاینت‌های قدیمی است. این فیلد
...
- `release.artifactKind`, `release.artifactSha256`, `release.npmIntegrity`,
  `release.npmShasum` و `release.npmTarballName` زمانی که برای artifact انتشار شناخته شده باشند وجود دارند.
- `trust.scanStatus` وضعیت اعتماد مؤثر مشتق‌شده از ورودی‌های اسکنر
  و نظارت دستی انتشار است.
...
فراداده resolveکننده صریح artifact را برای یک نسخه بسته برمی‌گرداند.
...
- این سطح resolveکننده OpenClaw است؛ از حدس زدن قالب آرشیو از
  یک URL مشترک جلوگیری می‌کند.

### `GET /api/v1/packages/{name}/versions/{version}/artifact/download`

artifact نسخه را از مسیر resolveکننده صریح دانلود می‌کند.
...
- در دسترس بودن artifact npm-pack مربوط به ClawPack
- خلاصه artifact
...
دانلودها را پنهان یا مسدود نمی‌کنند؛ ناظران باید برای
تأیید، قرنطینه، یا لغو artifactها از نظارت انتشار استفاده کنند.
...
`status` به `open` می‌توان آن را حذف کرد. برای اعمال نظارت بر انتشار در همان
...
هر تغییر یک ورودی در گزارش حسابرسی می‌نویسد.
...
- پاسخ‌ها شامل headerهای `ETag`، `Digest`، `X-ClawHub-Artifact-Type`، و
...
- headerهای دانلود شامل SHA-256 مربوط به ClawHub به‌همراه فرادادهٔ integrity/shasum مربوط به npm هستند.
...
- `slug` (الزامی)
- `hash` (الزامی): sha256 هگز ۶۴-نویسه‌ای از اثرانگشت bundle
...
با اسکن `clean` یا `suspicious` و بدون نسخهٔ میزبانی‌شده، یک handoff منبع GitHub برمی‌گرداند.
...
- handoffهای skill مبتنی بر GitHub بایت‌ها را proxy یا mirror نمی‌کنند. پاسخ JSON
  ...
  گنجانده نمی‌شود.
- آمار دانلود به‌صورت هویت‌های یکتا در هر روز UTC شمارش می‌شود (`userId` وقتی توکن API معتبر باشد، وگرنه IP).
...
- فیلد اختیاری payload: `migrateOwner`. وقتی همراه با `ownerHandle` برابر `true` باشد،
  یک skill موجود می‌تواند به آن مالک منتقل شود، اگر actor روی هر دو ناشر فعلی و هدف
...
- فیلدهای form مجاز `payload`، blobهای تکرارشوندهٔ `files`، یا یک ارجاع tarball
  `clawpack` هستند. `clawpack` می‌تواند یک blob `.tgz` یا یک storage id برگشتی از
...
soft deleteهای آغازشده توسط مالک، slug را به مدت ۳۰ روز رزرو می‌کنند، سپس slug می‌تواند توسط
ناشر دیگری ادعا شود. پاسخ حذف وقتی این انقضا اعمال شود، `slugReservedUntil` را شامل می‌شود.
...
فقط مدیر. اطمینان می‌دهد که یک ناشر org برای یک handle وجود دارد. اگر handle همچنان به یک
...
ایجاد self-serve ناشر org برای کاربران احراز هویت‌شده. یک ناشر org جدید ایجاد می‌کند و
فراخواننده را به‌عنوان مالک اضافه می‌کند. این نقطهٔ پایانی handleهای کاربر/شخصی موجود را migrate نمی‌کند و
...
نقطهٔ پایانی به‌صورت پیش‌فرض در حالت اجرای آزمایشی است. اعمال بازیابی پس از آن‌که کارکنان به‌طور مستقل پیوستگی بین هر دو هویت اصلی GitHub را تأیید کردند، به `dryRun: false` و
`confirmIdentityVerified: true` نیاز دارد. وقتی ناشر شخصی فعلی کاربر مقصد Skills، بسته‌ها، یا منابع Skill مربوط به GitHub داشته باشد، بازیابی به‌صورت بسته و ایمن شکست می‌خورد.
بازیابی همچنین فیلدهای قدیمی `ownerUserId` را برای Skills ناشر بازیابی‌شده،
نام‌های مستعار اسلاگ Skill، بسته‌ها، هشدارهای بازرس بسته، و ردیف‌های چکیدهٔ جست‌وجوی مشتق‌شده مهاجرت می‌دهد تا
مسیرهای مالک مستقیم با مرجع ناشر جدید هم‌خوان شوند. یک رزرو فعال دستهٔ محافظت‌شده
برای دستهٔ بازیابی‌شده نیز به کاربر جایگزین دوباره واگذار می‌شود تا همگام‌سازی بعدی
نمایه نتواند مرجع رقیب کاربر قبلی را برگرداند. هر جدول اصلی در هر تراکنش اعمال به
۱۰۰ ردیف محدود است؛ بازیابی‌های بزرگ‌تر باید ابتدا از مهاجرت مالک قابل ادامه استفاده کنند.
منابع Skill مربوط به GitHub در محدودهٔ ناشر هستند و به‌جای بازنویسی، به‌عنوان بررسی‌شده گزارش می‌شوند.

- بدنه: `{ "handle": "gingiris", "nextUserHandle": "gingiris-1031", "previousGitHubProviderAccountId": "123", "nextGitHubProviderAccountId": "456", "reason": "تأیید پیوستگی حساب برای issue #2555", "confirmIdentityVerified": true, "dryRun": false }`
- پاسخ: `{ "ok": true, "dryRun": false, "recovered": true, "publisherId": "...", "handle": "gingiris", "previousUser": { "userId": "...", "handle": "gingiris", "nextHandle": "gingiris-recovered", "githubProviderAccountId": "123", "authAccountCount": 1 }, "nextUser": { "userId": "...", "handle": "gingiris-1031", "nextHandle": "gingiris", "githubProviderAccountId": "456", "authAccountCount": 1 }, "retiredPersonalPublisher": null, "resourceOwnerMigration": { "limitPerTable": 100, "skills": 1, "skillSlugAliases": 1, "packages": 0, "packageInspectorWarnings": 0, "githubSourcesChecked": 1, "handleReservations": 1 }, "identityVerified": true, "reason": "تأیید پیوستگی حساب برای issue #2555" }`
...
یک کاربر را مسدود می‌کند و Skills تحت مالکیت او را به‌صورت سخت حذف می‌کند (فقط ناظر/مدیر).
...
یک ستاره (برجسته‌سازی) اضافه/حذف می‌کند. هر دو نقطهٔ پایانی هم‌توان هستند.
...
`clawpack` و بلیت بازگردانده‌شده را به‌عنوان `clawpackUploadTicket` ارسال کنند.
...
اگر خودمیزبانی می‌کنید، این فایل را ارائه کنید (یا `CLAWHUB_REGISTRY` را صریحاً تنظیم کنید؛ `CLAWDHUB_REGISTRY` قدیمی است).
