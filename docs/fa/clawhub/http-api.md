...
summary: مرجع رابط برنامه‌نویسی کاربردی HTTP (نقطه‌های پایانی عمومی + نقطه‌های پایانی CLI + احراز هویت).
x-i18n:
    generated_at: "2026-07-04T06:40:49Z"
    model: gpt-5.5
...
دایرکتوری‌های شخص ثالث می‌توانند از نقاط پایانی خواندنی عمومی برای فهرست‌کردن یا جست‌وجوی مهارت‌های ClawHub استفاده کنند. لطفاً نتایج را کش کنید، `429`/`Retry-After` را رعایت کنید، کاربران را به فهرست مرجع ClawHub (`https://clawhub.ai/<owner>/skills/<slug>`) برگردانید، و از القای تأیید سایت شخص ثالث توسط ClawHub خودداری کنید. تلاش نکنید محتوای پنهان، خصوصی، یا مسدودشده توسط نظارت را خارج از سطح API عمومی بازتاب دهید.

میان‌برهای اسلاگ وب در خانواده‌های رجیستری resolve می‌شوند، اما کلاینت‌های API باید به‌جای بازسازی اولویت مسیرها، از URLهای مرجعی استفاده کنند که نقاط پایانی خواندن برمی‌گردانند.
...
- اگر توکن وجود نداشته باشد یا نامعتبر باشد، رفتار به اعمال مبتنی بر IP برمی‌گردد.
- نقاط پایانی نوشتن احراز هویت‌شده وقتی سرور دلیل را می‌داند نباید یک `Unauthorized` خالی برگردانند. توکن‌های جاافتاده، توکن‌های نامعتبر/لغوشده، و حساب‌های حذف‌شده/مسدودشده/غیرفعال‌شده باید هرکدام متن قابل اقدام دریافت کنند تا کلاینت‌های CLI بتوانند به کاربران بگویند چه چیزی آن‌ها را مسدود کرده است.
...
- `X-RateLimit-Remaining` / `RateLimit-Remaining`: بودجهٔ باقی‌ماندهٔ دقیق، وقتی وجود داشته باشد.
  درخواست‌های موفق شاردشده به‌جای برگرداندن یک مقدار تقریبی سراسری، این هدر را حذف می‌کنند.
...
- اگر `Retry-After` وجود دارد، همان تعداد ثانیه پیش از تلاش دوباره صبر کنید.
...
- فقط وقتی استقرار به‌طور صریح هدرهای forwarded مورد اعتماد را فعال کرده باشد، از هدرهای IP کلاینت مورد اعتماد، از جمله `cf-connecting-ip`، استفاده می‌کند.
- ClawHub از هدرهای forwarding مورد اعتماد برای شناسایی IPهای کلاینت در لبه استفاده می‌کند.
- اگر IP کلاینت مورد اعتمادی در دسترس نباشد، درخواست‌های ناشناس از باکت‌های fallback استفاده می‌کنند که فقط با نوع محدودیت نرخ scope شده‌اند. این باکت‌های fallback شامل مسیرهای ارائه‌شده توسط فراخوان، اسلاگ‌ها، نام‌های بسته، نسخه‌ها، query stringها، یا دیگر پارامترهای artifact نیستند.

## پاسخ‌های خطا

پاسخ‌های خطای عمومی v1 متن ساده با `content-type: text/plain; charset=utf-8` هستند.
این شامل شکست‌های اعتبارسنجی (`400`)، منابع عمومی جاافتاده (`404`)، شکست‌های احراز هویت و مجوز (`401`/`403`)، محدودیت‌های نرخ (`429`)، و دانلودهای مسدودشده است. کلاینت‌ها باید بدنهٔ پاسخ را به‌عنوان یک رشتهٔ قابل خواندن برای انسان بخوانند. پارامترهای query ناشناخته برای سازگاری نادیده گرفته می‌شوند، اما پارامترهای query شناخته‌شده با مقدارهای نامعتبر `400` برمی‌گردانند.
...
- نتایج به‌ترتیب مرتبط‌بودن برگردانده می‌شوند (شباهت embedding + تقویت‌های توکن اسلاگ/نام دقیق + یک prior کوچک محبوبیت).
- مرتبط‌بودن از محبوبیت قوی‌تر است. یک تطابق دقیق توکن اسلاگ یا نام نمایشی می‌تواند از تطابق آزادتر با تعامل بسیار بیشتر رتبهٔ بالاتری بگیرد.
- متن ASCII روی مرزهای واژه و نشانه‌گذاری tokenized می‌شود. برای مثال، `personal-map` شامل توکن مستقل `map` است، در حالی که `amap-jsapi-skill` شامل `amap`، `jsapi`، و `skill` است؛ بنابراین جست‌وجوی `map` به `personal-map` نسبت به `amap-jsapi-skill` تطابق واژگانی قوی‌تری می‌دهد.
- محبوبیت به‌صورت لگاریتمی مقیاس‌بندی و سقف‌گذاری می‌شود. مهارت‌های با تعامل بالا وقتی متن پرس‌وجو تطابق ضعیف‌تری دارد می‌توانند رتبهٔ پایین‌تری بگیرند.
- وضعیت نظارتی مشکوک یا پنهان می‌تواند بسته به فیلترهای فراخوان و وضعیت فعلی نظارت، یک مهارت را از جست‌وجوی عمومی حذف کند.

راهنمای کشف‌پذیری ناشر:

- اصطلاحاتی را که کاربران عیناً جست‌وجو خواهند کرد در نام نمایشی، خلاصه، و برچسب‌ها بگذارید. فقط وقتی از توکن اسلاگ مستقل استفاده کنید که یک هویت پایدار نیز هست که می‌خواهید نگه دارید.
- فقط برای دنبال‌کردن یک پرس‌وجو، اسلاگ را تغییر نام ندهید مگر اینکه اسلاگ جدید نام مرجع بلندمدت بهتری باشد. اسلاگ‌های قدیمی به aliasهای redirect تبدیل می‌شوند، اما URL مرجع، اسلاگ نمایش‌داده‌شده، و digestهای جست‌وجوی آینده از اسلاگ جدید استفاده می‌کنند.
- aliasهای تغییرنام، resolution را برای URLهای قدیمی و نصب‌هایی که از طریق رجیستری resolve می‌شوند حفظ می‌کنند، اما رتبه‌بندی جست‌وجو بر اساس فرادادهٔ مرجع مهارت پس از ایندکس‌شدن تغییرنام است. آمار موجود با مهارت باقی می‌ماند.
- اگر یک مهارت به‌طور غیرمنتظره نامرئی است، پیش از تغییر فرادادهٔ مرتبط با رتبه‌بندی، ابتدا وضعیت نظارت را با `clawhub inspect @owner/slug` در حالت ورود بررسی کنید.
...
- وقتی `nonSuspiciousOnly=true` است، sortهای مبتنی بر cursor ممکن است در یک صفحه کمتر از `limit` مورد برگردانند، چون مهارت‌های مشکوک پس از بازیابی صفحه فیلتر می‌شوند.
...
- اسلاگ‌های قدیمی ایجادشده توسط جریان‌های تغییرنام/ادغام مالک به مهارت مرجع resolve می‌شوند.
...
- اگر مهارت فرادادهٔ پلتفرم نداشته باشد، `metadata` برابر `null` است.
- `moderation` فقط وقتی درج می‌شود که مهارت پرچم‌گذاری شده باشد یا مالک در حال مشاهدهٔ آن باشد.

### `GET /api/v1/skills/{slug}/moderation`

وضعیت نظارت ساختاریافته را برمی‌گرداند.
...
- مالکان و ناظران می‌توانند به جزئیات نظارت برای مهارت‌های پنهان دسترسی داشته باشند.
- فراخوان‌های عمومی فقط برای مهارت‌های قابل‌مشاهده‌ای که از قبل پرچم‌گذاری شده‌اند `200` می‌گیرند.
- شواهد برای فراخوان‌های عمومی redact می‌شود و فقط برای مالکان/ناظران شامل snippetهای خام است.

### `POST /api/v1/skills/{slug}/report`

یک مهارت را برای بازبینی ناظر گزارش کنید. گزارش‌ها در سطح مهارت هستند، می‌توانند به‌صورت اختیاری به یک نسخه پیوند داشته باشند، و صف گزارش مهارت را تغذیه می‌کنند.
...
نقطهٔ پایانی ناظر/مدیر برای intake گزارش مهارت.
...
`note` برای `confirmed` و `dismissed` الزامی است؛ هنگام تنظیم دوبارهٔ `status` به `open` می‌تواند حذف شود. برای پنهان‌کردن مهارت در همان workflow قابل audit، همراه با یک گزارش triageشده، `finalAction: "hide"` را ارسال کنید.
...
- `version.security` شامل وضعیت تأیید اسکن نرمال‌شده و جزئیات scanner است
...
- وضعیت تأیید نرمال‌سازی‌شده را به‌همراه جزئیات ویژهٔ اسکنر شامل می‌شود.
- `security.hasScanResult` فقط زمانی `true` است که یک اسکنر حکم قطعی تولید کرده باشد (`clean`، `suspicious`، یا `malicious`).
- `moderation` یک عکس فوری فعلی از تعدیل در سطح مهارت است که از آخرین نسخه مشتق شده است.
...
- بدنهٔ درخواست‌های اسکن و گزارش‌های قابل دانلود، پس از بازهٔ نگهداری از مخزن درخواست اسکن منقضی می‌شوند.
...
- کارهای اسکن ناهمگام هستند. درخواست‌های اسکن دستی جلوتر از کارهای معمول انتشار/پرکردن مجدد اولویت‌بندی می‌شوند، اما تکمیل همچنان به دسترس‌بودن worker بستگی دارد.
...
- نتایج اسکن ذخیره‌شده را برای نسخهٔ دقیق ارسال‌شده برمی‌گرداند، از جمله نسخه‌های مسدودشده یا پنهان.
...
مسیر بازاسکن دسته‌ای کانونی فقط برای مدیران. همان شکل بدنهٔ `POST /api/v1/skills/-/rescan-batch` قدیمی را می‌پذیرد.
...
پاکت تأیید کارت مهارت را که توسط `clawhub skill verify` استفاده می‌شود برمی‌گرداند.
...
- `security` حکم سطح بالای ClawScan/امنیت است. خودکارسازی باید بر اساس `ok`، `decision`، `reasons`، و `security.status` تصمیم بگیرد.
...
- `provenance` فقط زمانی `server-resolved-github-import` است که ClawHub هنگام انتشار یا import یک مخزن/ref/commit/path در GitHub را حل و ذخیره کرده باشد؛ در غیر این صورت `unavailable` است.

### `POST /api/v1/skills/-/security-verdicts`

حکم‌های امنیتی فشردهٔ فعلی را برای نسخه‌های دقیق مهارت برمی‌گرداند. این
نقطهٔ پایانی مجموعه برای کلاینت‌هایی در نظر گرفته شده که از قبل می‌دانند کدام نسخه‌های
مهارت ClawHub نصب‌شده را باید نمایش دهند، مانند OpenClaw Control UI.
...
- پاسخ فقط امنیتی است. دادهٔ کارت مهارت، وضعیت کارت تولیدشده، فهرست فایل‌های artifact، یا بدنه‌های تفصیلی اسکنر را شامل نمی‌شود.
...
- `security.signals.dependencyRegistry` برای سازگاری پاسخ v1 نگه داشته شده است، اما اسکنر وجود رجیستری وابستگی بازنشسته شده و این کلید همیشه `null` است.
- نبود کارت مهارت روی `ok`، `decision`، یا `reasons` این نقطهٔ پایانی اثر نمی‌گذارد؛ کلاینت‌ها وقتی به محتوای کارت نیاز دارند باید `skill-card.md` نصب‌شده را به‌صورت محلی بخوانند.
- وقتی به پاکت تأیید کارت مهارت برای یک مهارت نیاز دارید از `/verify` استفاده کنید، وقتی به Markdown کارت تولیدشده نیاز دارید از `/card`، و وقتی به دادهٔ تفصیلی اسکنر نیاز دارید از `/scan`.
...
- `category` (اختیاری): فیلتر دسته‌بندی Plugin. فقط زمانی پشتیبانی می‌شود که
  ...
  نام‌های مستعار فیلتر قدیمی نسخهٔ ۱ زیر `GET /api/v1/plugins` مستند شده‌اند.
...
- مدخل‌های مهارت همچنان به رجیستری مهارت متکی هستند و هنوز فقط از طریق `POST /api/v1/skills` قابل انتشارند.
...
- `category` (اختیاری): فیلتر دسته‌بندی Plugin. فقط زمانی پشتیبانی می‌شود که
  ...
  فیلتر قدیمی نسخهٔ ۱ زیر `GET /api/v1/plugins` مستند شده‌اند.
...
مرور کاتالوگ مخصوص Plugin در میان بسته‌های code-plugin و bundle-plugin.
...
نام‌های مستعار فیلتر قدیمی نسخهٔ ۱ همچنان در نقاط پایانی خواندن پذیرفته می‌شوند:
...
`trending` جدول رتبه‌بندی نصب/دانلود هفت‌روزه است و از مجموع‌های کل زمان استفاده نمی‌کند.
در نقطهٔ پایانی یکپارچهٔ `/api/v1/packages`، فقط مخصوص Plugin است؛ برای کاتالوگ مهارت از
`/api/v1/skills?sort=trending` استفاده کنید.

نام‌های مستعار قدیمی به‌عنوان مقدارهای دسته‌بندی ذخیره‌شده یا اعلام‌شده از سوی نویسنده پذیرفته نمی‌شوند.

### `GET /api/v1/skills/export`

خروجی‌گیری گروهی از آخرین مهارت‌های عمومی برای تحلیل آفلاین.
...
- `startDate` (الزامی): حد پایین به میلی‌ثانیهٔ Unix برای `updatedAt` مهارت.
- `endDate` (الزامی): حد بالا به میلی‌ثانیهٔ Unix برای `updatedAt` مهارت.
...
- مهارت‌های میزبانی‌شده شامل فایل‌های آخرین نسخهٔ ذخیره‌شده هستند و در
  `_manifest.json` با `sourceRef: "public-clawhub"` فهرست می‌شوند.
- مهارت‌های فعلی متکی بر GitHub با اسکن `clean` یا `suspicious` شامل
...
- وقتی مهارت‌ها یا فایل‌های منفرد قابل خروجی‌گیری نباشند، `_errors.json` گنجانده می‌شود.
...
- بدنه: آرشیو ZIP.
- ریشه هر Plugin صادرشده در `{family}/{packageName}/` است.
...
- وقتی Pluginها یا فایل‌های جداگانه قابل صدور نبوده‌اند، `_errors.json`
  گنجانده می‌شود.
...
- نام‌های مستعار فیلتر نسخه قدیمی v1 که زیر `GET /api/v1/plugins` مستند شده‌اند نیز
  پذیرفته می‌شوند.
- فیلتر دسته‌بندی یک فیلتر واقعی API است که با ردیف‌های digest دسته‌بندی Plugin
  پشتیبانی می‌شود، نه بازنویسی عبارت جست‌وجو.
- نتایج به ترتیب ارتباط برگردانده می‌شوند و در حال حاضر صفحه‌بندی نمی‌شوند.
- کنترل‌های مرتب‌سازی رابط کاربری مرورگر برای جست‌وجوی Plugin، نتایج مرتبط بارگذاری‌شده را بازمرتب می‌کنند،
...
راستی‌آزمایی، فراداده artifact، و داده‌های اسکن، برمی‌گرداند.

نکته‌ها:

- `version.artifact.kind` برای آرشیوهای بسته دنیای قدیم `legacy-zip` است یا
...
- endpoint خواندن عمومی. هیچ توکن مالک، ناشر، ناظر، یا ادمین
  لازم نیست.
...
- `trust.scanStatus` وضعیت اعتماد مؤثر است که از ورودی‌های اسکنر
  و تعدیل دستی انتشار مشتق می‌شود.
- `trust.moderationState` می‌تواند null باشد. وقتی هیچ تعدیل دستی انتشار
...
- `trust.stale` یعنی خلاصه اعتماد از ورودی‌های منسوخ محاسبه شده است و
  پیش از یک تصمیم allow با اطمینان بالا باید نیازمند تازه‌سازی تلقی شود.
...
- این endpoint دقیقاً وابسته به نسخه است. کلاینت‌ها باید پس از resolve کردن
  ...
  فراداده بسته.
- بسته‌های خصوصی `404` برمی‌گردانند مگر اینکه فراخواننده بتواند ناشر مالک را بخواند.
- این endpoint عمداً از endpointهای تعدیل مالک/ناظر محدودتر است.
...
- نسخه‌های ClawPack یک artifact از نوع `npm-pack`، فیلدهای integrity در npm، یک
...
endpoint ادمین برای ایجاد یا به‌روزرسانی یک ردیف migration مربوط به Plugin رسمی.
...
- `packageName` به نام npm نرمال‌سازی می‌شود؛ بسته می‌تواند برای migrationهای برنامه‌ریزی‌شده
  وجود نداشته باشد.
- این فقط آمادگی migration را دنبال می‌کند. OpenClaw را تغییر نمی‌دهد یا
  ClawPack تولید نمی‌کند.

### `GET /api/v1/packages/moderation/queue`

endpoint ناظر/ادمین برای صف‌های بازبینی انتشار بسته.
...
- `manual`: هر انتشاری با بازنویسی تعدیل دستی.
- `all`: هر انتشاری با بازنویسی دستی، وضعیت اسکن غیر clean، یا گزارش بسته.
...
به یک نسخه پیوند می‌خورند. آن‌ها صف تعدیل را تغذیه می‌کنند اما به‌تنهایی دانلودها را
به‌صورت خودکار مخفی یا مسدود نمی‌کنند؛ ناظران باید از تعدیل انتشار برای
...
- `revoked`: پس از آن‌که انتشار قبلاً قابل اعتماد بوده، مسدود شده است.
...
- فقط نسخه‌هایی فهرست می‌شوند که tarballهای npm-pack از نوع ClawPack آن‌ها بارگذاری شده باشد.
...
- packumentهای بستهٔ scoped هم از مسیر `/api/npm/@scope/name` و هم از مسیر درخواست
...
- `slug` (الزامی)
- `hash` (الزامی): sha256 هگز 64-کاراکتری اثرانگشت bundle
...
یک ZIP نسخهٔ skill میزبانی‌شده را دانلود می‌کند، یا برای skill فعلی مبتنی بر GitHub که اسکن
`clean` یا `suspicious` دارد و نسخهٔ میزبانی‌شده ندارد، تحویل منبع GitHub را برمی‌گرداند.
...
- فیلد اختیاری payload: `ownerHandle`. وقتی وجود داشته باشد، API آن
  ناشر را در سمت سرور resolve می‌کند و از actor دسترسی ناشر می‌خواهد.
- فیلد اختیاری payload: `migrateOwner`. وقتی همراه با `ownerHandle` برابر `true` باشد، یک
  skill موجود می‌تواند به آن مالک منتقل شود، اگر actor روی هر دو ناشر فعلی و مقصد
...
- فیلدهای فرم مجاز عبارت‌اند از `payload`، blobهای تکرارشوندهٔ `files`، یا یک ارجاع tarball
  `clawpack`. `clawpack` می‌تواند یک blob با پسوند `.tgz` یا یک شناسهٔ storage باشد که توسط
  جریان upload-url برگردانده شده است. انتشارهای staged با storage-id باید همچنین
  `clawpackUploadTicket` برگشتی همراه با آن URL بارگذاری را شامل شوند.
...
- انتشار از طرف دیگری همچنان واجد شرایط بودن کانال رسمی را نسبت به حساب مالک مقصد اعتبارسنجی می‌کند.
...
وقتی وجود داشته باشد، `reason` به‌عنوان یادداشت نظارت skill ذخیره و در لاگ ممیزی کپی می‌شود.
...
فقط مدیر. اطمینان می‌دهد که یک ناشر سازمانی برای یک handle وجود دارد. اگر handle هنوز به یک
کاربر مشترک قدیمی/ناشر شخصی اشاره کند، نقطهٔ پایانی ابتدا آن را به یک ناشر سازمانی مهاجرت می‌دهد.
برای یک سازمان تازه‌ساخته‌شده، `memberHandle` را ارائه کنید؛ مدیر اجراکننده به‌عنوان عضو اضافه نمی‌شود.
...
ایجاد self-serve ناشر سازمانی با احراز هویت. یک ناشر سازمانی جدید ایجاد می‌کند و
...
release برای مالک برحق رزرو می‌کند. نام‌های بسته به بسته‌های placeholder خصوصی بدون ردیف‌های release تبدیل می‌شوند، تا همان
...
فقط مدیر. یک ناشر شخصی را برای اصل GitHub OAuth جایگزینِ تأییدشده
...
نقطهٔ پایانی به‌طور پیش‌فرض روی اجرای آزمایشی تنظیم است. اعمال بازیابی پس از آن‌که کارکنان به‌صورت مستقل تداوم میان هر دو هویت اصلی GitHub را تأیید کردند، به `dryRun: false` و
`confirmIdentityVerified: true` نیاز دارد. اگر ناشر شخصی فعلی کاربر مقصد دارای skills، packages، یا GitHub skill sources باشد، بازیابی به‌صورت بسته‌-در-خطا شکست می‌خورد.
...
برای handle بازیابی‌شده نیز به کاربر جایگزین بازتخصیص داده می‌شود تا همگام‌سازی بعدی
نمایه نتواند اختیار رقیب کاربر قبلی را بازگرداند. هر جدول اصلی در هر تراکنش اعمال به
۱۰۰ ردیف محدود است؛ بازیابی‌های بزرگ‌تر باید ابتدا از یک مهاجرت مالک قابل‌ازسرگیری استفاده کنند.
GitHub skill sources در محدودهٔ ناشر هستند و به‌جای بازنویسی، به‌عنوان بررسی‌شده گزارش می‌شوند.
...
یک کاربر را مسدود کنید و skills متعلق به او را سخت‌حذف کنید (فقط ناظر/مدیر).
...
مسدودی کاربر را بردارید و skills واجد شرایط را بازیابی کنید (فقط مدیر).
...
محتوا تغییر دهید (فقط مدیر). مگر آن‌که `dryRun` برابر `false` باشد، به‌طور پیش‌فرض اجرای آزمایشی است.
...
`POST /api/cli/upload-url` مقدارهای `uploadUrl` و `uploadTicket` را برمی‌گرداند. انتشارهای package
