...
x-i18n:
    generated_at: "2026-07-03T09:49:12Z"
    model: gpt-5.5
...
اسکن‌های خودکار بسته را نشان دهد. این صفحه یافته‌های مختص نویسنده را پوشش می‌دهد، یعنی
یافته‌هایی که نویسنده Plugin می‌تواند در فراداده بسته، مانیفست، importهای SDK
یا آرتیفکت منتشرشده خود اصلاح کند.

این صفحه یافته‌های پوشش داخلی Plugin Inspector را پوشش نمی‌دهد. اگر یک گزارش کامل
کدهای نگهداری اسکنر را بدون راهنمای اصلاح برای نویسنده داشته باشد، آن‌ها برای
...
| `sdk-session-transcript-low-level`     | [کمک‌کننده‌های سطح‌پایین transcript را جایگزین کنید](/fa/clawhub/plugin-validation-fixes#sdk-session-transcript-low-level)        |
...
| `provider-auth-env-vars`               | [متغیرهای env ارائه‌دهنده را به فراداده setup منتقل کنید](/fa/clawhub/plugin-validation-fixes#provider-auth-env-vars)             |
| `channel-env-vars`                     | [متغیرهای env کانال را در فراداده فعلی mirror کنید](/fa/clawhub/plugin-validation-fixes#channel-env-vars)                         |
| `security-manifest-schema-unavailable` | [ارجاع‌های unavailable به شمای مانیفست امنیتی را حذف کنید](/fa/clawhub/plugin-validation-fixes#security-manifest-schema-unavailable) |
...
- وقتی بسته یک Plugin برای OpenClaw ارسال می‌کند، یک بلوک `openclaw` اضافه کنید.
...
- وقتی بسته از طریق ClawHub منتشر یا نصب خواهد شد، فراداده سازگاری و نصب
  را اضافه کنید.
...
بسته یک نقطه ورود OpenClaw اعلام می‌کند، اما فایل ارجاع‌داده‌شده در
بسته‌ای که اعتبارسنجی می‌شود وجود ندارد.
...
- وقتی بیش از یک منبع نصب در دسترس است، `openclaw.install.defaultChoice`
  را تنظیم کنید.
...
- از نسخه API Plugin در OpenClaw یا کف semver که بر اساس آن ساخته و آزمایش کرده‌اید
  استفاده کنید.
...
- هر فراداده build مربوط به OpenClaw در بسته را بررسی کنید، مانند نسخه OpenClaw
...
- اگر `openclaw.plugin.json` نیز `version` دارد، آن را برای تطابق به‌روزرسانی کنید
  یا وقتی فراداده بسته مرجع معتبر است، فراداده نسخه مانیفست قدیمی را حذف کنید.
...
- نقاط ورود بسته، سازگاری، نصب، setup و فراداده کاتالوگ را در فیلدهای پشتیبانی‌شده
  `package.json#openclaw` نگه دارید.
- [فیلدهای package.json که بر discovery اثر می‌گذارند](/fa/plugins/manifest#packagejson-fields-that-affect-discovery) را ببینید.
- `clawhub package validate <path-to-plugin>` را دوباره اجرا کنید.
...
بسته نمی‌تواند به آرتیفکتی بسته‌بندی شود که ClawHub آن را بررسی یا منتشر
می‌کند.
...
بسته قابل بسته‌بندی است، اما آرتیفکت بسته‌بندی‌شده فایل‌های نقطه ورود اعلام‌شده
در `package.json#openclaw` را شامل نمی‌شود.
...
- importهای barrel ریشه را با importهای زیرمسیر عمومی متمرکز جایگزین کنید.
...
- برای انتخاب یک import پشتیبانی‌شده، از [زیرمسیرهای SDK Plugin](/fa/plugins/sdk-subpaths) و
...
- هنگام به‌روزرسانی فیلدهای یک entry موجود session از `patchSessionEntry(...)` استفاده کنید.
...
- برای پایدارسازی فراداده session از `patchSessionEntry(...)` یا `upsertSessionEntry(...)` استفاده کنید.
- وقتی کد در حال آماده‌سازی یک عملیات transcript است، از هویت transcript یا کمک‌کننده‌های هدف استفاده کنید.
- مسیرهای فایل transcript قدیمی را persist نکنید و به آن‌ها وابسته نشوید.
- [API زمان اجرا](/fa/plugins/sdk-runtime#agent-session-state) و
  [زیرمسیرهای SDK Plugin](/fa/plugins/sdk-subpaths) را ببینید.
- `clawhub package validate <path-to-plugin>` را دوباره اجرا کنید.
...
Plugin هنوز از کمک‌کننده منسوخ‌شده هدف فایل transcript یعنی
...
- از خواندن یا ساختن مستقیم هدف‌های فایل transcript قدیمی خودداری کنید.
...
- سطح زمان اجرای ساختاریافته transcript را ترجیح دهید تا OpenClaw بتواند
...
مانیفست هنوز از فراداده auth قدیمی provider یعنی `providerAuthEnvVars` استفاده می‌کند.
...
مانیفست از فراداده env-var قدیمی یا قدیمی‌تر کانال استفاده می‌کند، بدون فراداده setup
