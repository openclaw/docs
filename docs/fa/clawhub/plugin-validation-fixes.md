...
ClawHub بسته‌های Plugin را پیش از انتشار اعتبارسنجی می‌کند و می‌تواند یافته‌های
اسکن‌های خودکار بسته را نیز نشان دهد. این صفحه یافته‌های مربوط به نویسنده را پوشش
می‌دهد؛ یعنی یافته‌هایی که نویسنده Plugin می‌تواند در فراداده بسته، مانیفست، واردسازی‌های SDK
یا آرتیفکت منتشرشده خود رفع کند.

این صفحه یافته‌های پوشش داخلی Plugin Inspector را پوشش نمی‌دهد. اگر یک گزارش کامل
شامل کدهای نگهداری اسکنر بدون راهنمای رفع توسط نویسنده باشد، آن‌ها برای نگهدارندگان
...
| `package-openclaw-unsupported-metadata` | [حذف فراداده بسته OpenClaw پشتیبانی‌نشده](/fa/clawhub/plugin-validation-fixes#package-openclaw-unsupported-metadata)              |
...
| `manifest-unknown-fields`               | [حذف فیلدهای مانیفست پشتیبانی‌نشده](/fa/clawhub/plugin-validation-fixes#manifest-unknown-fields)                                  |
...
| `sdk-session-file-helper`               | [جایگزینی کمک‌کننده‌های مسیر فایل نشست](/fa/clawhub/plugin-validation-fixes#sdk-session-file-helper)                              |
...
- برای نمونه حداقلی بسته از [ساخت Pluginها](/fa/plugins/building-plugins) و برای تفکیک
  بسته از مانیفست از [مانیفست Plugin](/fa/plugins/manifest#manifest-versus-packagejson)
  استفاده کنید.
...
- اگر `openclaw.plugin.json` نیز `version` دارد، آن را به‌روزرسانی کنید تا مطابق شود یا وقتی فراداده بسته مرجع است،
  فراداده نسخه مانیفست قدیمی را حذف کنید.
...
- نقاط ورود بسته، سازگاری، نصب، راه‌اندازی و فراداده کاتالوگ را در فیلدهای پشتیبانی‌شده
...
بسته را نمی‌توان به آرتیفکتی تبدیل کرد که ClawHub بررسی یا منتشر می‌کند.

- `npm pack --dry-run` را از ریشه بسته اجرا کنید.
- فراداده نامعتبر بسته، اسکریپت‌های چرخه‌عمر خراب، یا ورودی‌های files را که باعث شکست بسته‌بندی می‌شوند رفع کنید.
- اگر این بسته برای انتشار عمومی در نظر گرفته شده است، `private: true` را حذف کنید.
- دوباره `clawhub package validate <path-to-plugin>` را اجرا کنید.

### package-npm-pack-entrypoint-missing

بسته قابل بسته‌بندی است، اما آرتیفکت بسته‌بندی‌شده فایل‌های نقطه ورود اعلام‌شده در
`package.json#openclaw` را شامل نمی‌شود.
...
- importهای root-barrel را با importهای متمرکز از زیرمسیرهای عمومی جایگزین کنید.
...
- برای پیدا کردن import محدودتر، از [قراردادهای import](/fa/plugins/building-plugins#import-conventions) و
...
- هنگام به‌روزرسانی فیلدها روی یک entry موجود session از `patchSessionEntry(...)` استفاده کنید.
...
- برای خواندن فراداده session بر اساس هویت agent و session از `getSessionEntry(...)` استفاده کنید.
...
- وقتی کد به target ساختاریافته عملیات transcript نیاز دارد، از `resolveSessionTranscriptTarget(...)` استفاده کنید.
...
Plugin هنوز از کمک‌کننده‌های منسوخ‌شده سطح‌پایین transcript مانند
...
- سطح زمان اجرای ساختاریافته transcript را ترجیح دهید تا OpenClaw بتواند
  مرزهای درست transaction و مدیریت هویت را اعمال کند.
...
مانیفست از فراداده env-var قدیمی یا قدیمی‌تر کانال بدون فراداده setup یا config فعلی
...
آن را به‌عنوان schema موجود نمی‌شناسد.

- اگر URL schema فقط جنبه توصیه‌ای دارد، آن را حذف کنید.
- فقط پس از اینکه OpenClaw یک schema نسخه‌دار منتشر کرد، از schema مستندشده نسخه‌دار استفاده کنید.
- `clawhub package validate <path-to-plugin>` را دوباره اجرا کنید.
