...
x-i18n:
    generated_at: "2026-07-04T06:42:32Z"
    model: gpt-5.5
...
شامل کدهای نگهداری اسکنر بدون راهنمای اصلاح برای نویسنده باشد، آن‌ها مربوط به
نگهدارندگان OpenClaw هستند، نه نویسندگان Plugin.
...
| `package-min-host-version-drift`        | [هماهنگ‌سازی حداقل نسخه میزبان](/fa/clawhub/plugin-validation-fixes#package-min-host-version-drift)                               |
| `package-manifest-version-drift`        | [هماهنگ‌سازی نسخه‌های بسته و manifest](/fa/clawhub/plugin-validation-fixes#package-manifest-version-drift)                        |
...
| `package-npm-pack-entrypoint-missing`   | [گنجاندن entrypointها در خروجی بسته npm](/fa/clawhub/plugin-validation-fixes#package-npm-pack-entrypoint-missing)                  |
...
| `reserved-sdk-import`                   | [حذف importهای رزرو‌شده SDK](/fa/clawhub/plugin-validation-fixes#reserved-sdk-import)                                             |
...
| `security-manifest-schema-unavailable`  | [حذف ارجاع‌های schema manifest امنیتی دردسترس‌نبودنی](/fa/clawhub/plugin-validation-fixes#security-manifest-schema-unavailable)   |
...
ریشه بسته شامل `package.json` نیست، بنابراین ClawHub نمی‌تواند
بسته npm، نسخه، entrypointها یا فراداده OpenClaw را شناسایی کند.
...
- برای نمونه حداقلی بسته از [ساخت Pluginها](/fa/plugins/building-plugins)
  و برای تفکیک بسته از manifest از [manifest Plugin](/fa/plugins/manifest#manifest-versus-packagejson)
  استفاده کنید.
...
- از نسخه API Plugin در OpenClaw یا کف semver که بر اساس آن ساخته و آزموده‌اید استفاده کنید.
...
بر اساس آن ساخته شده است همخوانی ندارد.
...
- حداقل نسخه میزبان را با بازه نسخه میزبان که بسته
  واقعاً پشتیبانی می‌کند هماهنگ کنید.
- [فیلدهای package.json که بر کشف اثر می‌گذارند](/fa/plugins/manifest#packagejson-fields-that-affect-discovery) را ببینید.
- `clawhub package validate <path-to-plugin>` را دوباره اجرا کنید.

### package-manifest-version-drift

نسخه بسته و نسخه manifest Plugin با هم اختلاف دارند.

- `package.json#version` را به‌عنوان نسخه انتشار بسته ترجیح دهید.
- اگر `openclaw.plugin.json` نیز `version` دارد، آن را به‌روزرسانی کنید تا مطابقت داشته باشد، یا وقتی فراداده بسته مرجع است
  فراداده نسخه manifest قدیمی را حذف کنید.
...
بسته را نمی‌توان به artifactای بسته‌بندی کرد که ClawHub آن را بررسی یا
منتشر می‌کند.

- `npm pack --dry-run` را از ریشه بسته اجرا کنید.
- فراداده نامعتبر بسته، lifecycle scriptهای خراب، یا ورودی‌های files را که
...
بسته قابل بسته‌بندی است، اما artifact بسته‌بندی‌شده شامل
...
- entrypointهای تولیدشده را پیش از بسته‌بندی بسازید.
- `files`، `.npmignore` یا خروجی ساخت را به‌روزرسانی کنید تا entrypointهای اعلام‌شده
  گنجانده شوند.
- [نقاط ورود Plugin](/fa/plugins/sdk-entrypoints) را ببینید.
- `clawhub package validate <path-to-plugin>` را دوباره اجرا کنید.
...
- برای راهنماهای نقطه ورود کانال از `openclaw/plugin-sdk/channel-core` استفاده کنید.
...
Plugin یک مسیر SDK را import می‌کند که برای Pluginهای همراه یا سازگاری داخلی
رزرو شده است.

- importهای SDK داخلی رزروشده OpenClaw را با زیرمسیرهای عمومی مستندشده
  `openclaw/plugin-sdk/*` جایگزین کنید.
- اگر رفتار هیچ SDK عمومی ندارد، راهنما را داخل بسته خود نگه دارید یا
...
Plugin هنوز از راهنمای منسوخ‌شده کل session-store یعنی
...
Plugin هنوز از یک راهنمای نوشتن کل session-store منسوخ‌شده مانند
...
- از بارگذاری، تغییر دادن، و ذخیره کردن کل شیء session store خودداری کنید.
- راهنماهای نوشتن کل store را فقط تا زمانی نگه دارید که بازه سازگاری اعلام‌شده شما
...
Plugin هنوز از راهنماهای منسوخ‌شده مسیر فایل نشست مانند
...
- برای ماندگار کردن فراداده نشست از `patchSessionEntry(...)` یا `upsertSessionEntry(...)`
  استفاده کنید.
- وقتی کد در حال آماده‌سازی یک عملیات transcript است، از هویت transcript یا راهنماهای target
  استفاده کنید.
- مسیرهای فایل transcript قدیمی را ماندگار نکنید و به آن‌ها وابسته نشوید.
- [API زمان اجرا](/fa/plugins/sdk-runtime#agent-session-state) و
  [زیرمسیرهای Plugin SDK](/fa/plugins/sdk-subpaths) را ببینید.
- `clawhub package validate <path-to-plugin>` را دوباره اجرا کنید.
...
Plugin هنوز از راهنمای منسوخ‌شده target فایل transcript یعنی
...
- وقتی کد به یک target ساختاریافته برای عملیات transcript نیاز دارد، از `resolveSessionTranscriptTarget(...)`
  استفاده کنید.
- از خواندن یا ساختن مستقیم targetهای فایل transcript قدیمی خودداری کنید.
- راهنمای قدیمی را فقط تا زمانی نگه دارید که بازه سازگاری اعلام‌شده شما همچنان
...
Plugin هنوز از راهنماهای سطح‌پایین transcript منسوخ‌شده مانند
...
- راهنماهای سطح‌پایین transcript را فقط تا زمانی نگه دارید که بازه سازگاری اعلام‌شده شما
  همچنان از نسخه‌های قدیمی‌تر OpenClaw که به آن‌ها نیاز دارند پشتیبانی می‌کند.
- [API زمان اجرا](/fa/plugins/sdk-runtime#agent-session-state) و
  [زیرمسیرهای Plugin SDK](/fa/plugins/sdk-subpaths) را ببینید.
- `clawhub package validate <path-to-plugin>` را دوباره اجرا کنید.
...
مانیفست هنوز از فراداده احراز هویت provider قدیمی `providerAuthEnvVars` استفاده می‌کند.
...
مانیفست از فراداده env-var قدیمی یا کهنه‌تر کانال استفاده می‌کند، بدون فراداده setup یا config
فعلی که ClawHub انتظار دارد.
...
- اگر URL schema فقط جنبه مشورتی دارد، آن را حذف کنید.
