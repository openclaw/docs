...
ClawHub بسته‌های Plugin را پیش از انتشار اعتبارسنجی می‌کند و همچنین می‌تواند یافته‌های حاصل از پویش‌های خودکار بسته را نشان دهد. این صفحه یافته‌های مربوط به نویسنده را پوشش می‌دهد؛ یعنی یافته‌هایی که نویسنده Plugin می‌تواند در فراداده بسته، مانیفست، واردکردن‌های SDK یا آرتیفکت منتشرشده خود اصلاح کند.

این صفحه یافته‌های پوشش داخلی Plugin Inspector را پوشش نمی‌دهد. اگر یک گزارش کامل شامل کدهای نگهداری پویشگر بدون راهنمای اصلاح برای نویسنده باشد، آن موارد برای نگه‌دارندگان OpenClaw هستند، نه نویسندگان Plugin.
...
| `package-npm-pack-entrypoint-missing`  | [نقاط ورود را در خروجی بسته npm بگنجانید](/fa/clawhub/plugin-validation-fixes#package-npm-pack-entrypoint-missing)              |
| `package-npm-pack-metadata-missing`    | [فراداده را در خروجی بسته npm بگنجانید](/fa/clawhub/plugin-validation-fixes#package-npm-pack-metadata-missing)                  |
...
| `legacy-root-sdk-import`               | [واردکردن‌های ریشه SDK را جایگزین کنید](/fa/clawhub/plugin-validation-fixes#legacy-root-sdk-import)                            |
...
بسته یک نقطه ورود OpenClaw اعلام می‌کند، اما فایل ارجاع‌داده‌شده در بسته‌ای که اعتبارسنجی می‌شود وجود ندارد.
...
بسته محدوده API Plugin OpenClaw را که پشتیبانی می‌کند اعلام نمی‌کند.
...
- هرگونه فراداده ساخت OpenClaw در بسته را بررسی کنید، مانند نسخه OpenClaw استفاده‌شده هنگام انتشار.
- حداقل نسخه میزبان را با محدوده نسخه میزبان که بسته واقعا پشتیبانی می‌کند هم‌راستا کنید.
- [فیلدهای package.json که بر کشف اثر می‌گذارند](/fa/plugins/manifest#packagejson-fields-that-affect-discovery) را ببینید.
- دوباره `clawhub package validate <path-to-plugin>` را اجرا کنید.
...
Plugin از barrel ریشه منسوخ SDK وارد می‌کند:
`openclaw/plugin-sdk`.
...
Plugin یک مسیر SDK را وارد می‌کند که برای Pluginهای بسته‌بندی‌شده یا سازگاری
داخلی رزرو شده است.
...
Plugin هنوز از کمک‌کننده منسوخ کل مخزن نشست
...
- هنگام نوشتن وضعیت نشست، از `patchSessionEntry(...)` یا `upsertSessionEntry(...)`
  استفاده کنید.
- از بارگذاری، تغییر، و ذخیره کل شیء مخزن نشست پرهیز کنید.
...
Plugin هنوز از یک کمک‌کننده نوشتن کل مخزن نشست منسوخ مانند
...
- برای ماندگار کردن فراداده نشست، از `patchSessionEntry(...)` یا `upsertSessionEntry(...)`
  استفاده کنید.
- وقتی کد در حال آماده‌سازی عملیات رونوشت است، از هویت رونوشت یا کمک‌کننده‌های هدف
  استفاده کنید.
...
Plugin هنوز از کمک‌کننده منسوخ هدف فایل رونوشت
...
Plugin هنوز از کمک‌کننده‌های سطح پایین منسوخ رونوشت مانند
...
- کار مربوط به بازنویسی مدل یا تامین‌کننده را به `before_model_resolve` منتقل کنید.
- کار مربوط به تغییر prompt یا زمینه را به `before_prompt_build` منتقل کنید.
...
مانیفست هنوز از فراداده احراز هویت تامین‌کننده قدیمی `providerAuthEnvVars` استفاده می‌کند.

- فراداده env-var تامین‌کننده را در `setup.providers[].envVars` منعکس کنید.
...
- تا زمانی که OpenClaw یک schema مانیفست امنیت نسخه‌دار و رفتار ClawHub را مستند کند،
  `openclaw.security.json` را حذف کنید.
- تا زمانی که قرارداد مانیفست وجود پیدا کند، رفتار حساس به امنیت را در مستندات عمومی بسته
