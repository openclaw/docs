...
x-i18n:
    generated_at: "2026-07-04T03:57:20Z"
    model: gpt-5.5
...
# رفع ایرادهای اعتبارسنجی Plugin
...
یافته‌هایی که نویسنده Plugin می‌تواند در فراداده بسته، مانیفست، ایمپورت‌های SDK
یا آرتیفکت منتشرشده خود رفع کند.

این صفحه یافته‌های پوشش داخلی Plugin Inspector را پوشش نمی‌دهد. اگر یک گزارش کامل
کدهای نگهداشت اسکنر را بدون راهنمایی رفع برای نویسنده شامل شود، آن‌ها برای نگه‌دارندگان
...
| `package-openclaw-metadata-missing`    | [بخش openclaw بسته را اضافه کنید](/fa/clawhub/plugin-validation-fixes#package-openclaw-metadata-missing)                         |
...
| `package-npm-pack-entrypoint-missing`  | [نقاط ورود را در خروجی npm pack وارد کنید](/fa/clawhub/plugin-validation-fixes#package-npm-pack-entrypoint-missing)             |
| `package-npm-pack-metadata-missing`    | [فراداده را در خروجی npm pack وارد کنید](/fa/clawhub/plugin-validation-fixes#package-npm-pack-metadata-missing)                 |
...
| `legacy-root-sdk-import`               | [ایمپورت‌های ریشه SDK را جایگزین کنید](/fa/clawhub/plugin-validation-fixes#legacy-root-sdk-import)                              |
| `reserved-sdk-import`                  | [ایمپورت‌های رزرو‌شده SDK را حذف کنید](/fa/clawhub/plugin-validation-fixes#reserved-sdk-import)                                 |
...
| `security-manifest-schema-unavailable` | [ارجاع‌های غیرقابل‌دسترس به شِمای مانیفست امنیتی را حذف کنید](/fa/clawhub/plugin-validation-fixes#security-manifest-schema-unavailable) |
...
- وقتی بسته یک Plugin مربوط به OpenClaw را ارائه می‌کند، یک بخش `openclaw` اضافه کنید.
...
بسته یک نقطه ورود OpenClaw را اعلام می‌کند، اما فایل ارجاع‌شده در بسته‌ای که
...
- این مورد را از نسخه بسته جدا نگه دارید. نسخه بسته انتشار Plugin را توصیف می‌کند؛
...
- هر فراداده ساخت OpenClaw در بسته را، مانند نسخه OpenClaw استفاده‌شده هنگام انتشار،
  بررسی کنید.
- حداقل نسخه میزبان را با بازه نسخه میزبانی که بسته واقعاً پشتیبانی می‌کند همسو کنید.
- [فیلدهای package.json که بر کشف اثر می‌گذارند](/fa/plugins/manifest#packagejson-fields-that-affect-discovery) را ببینید.
- `clawhub package validate <path-to-plugin>` را دوباره اجرا کنید.
...
- اگر `openclaw.plugin.json` نیز `version` دارد، آن را برای تطبیق به‌روزرسانی کنید
  یا وقتی فراداده بسته مرجع است، فراداده نسخه مانیفست کهنه را حذف کنید.
...
بخش `package.json#openclaw` شامل فیلدهایی است که به‌عنوان فراداده بسته OpenClaw
پشتیبانی نمی‌شوند.
...
- فراداده نامعتبر بسته، اسکریپت‌های چرخه عمر خراب یا ورودی‌های files را که باعث
  شکست بسته‌بندی می‌شوند اصلاح کنید.
...
- مطمئن شوید `package.json` بخش `openclaw` را در آرتیفکت بسته‌بندی‌شده شامل می‌کند.
- وقتی بسته یک Plugin بومی OpenClaw است، مطمئن شوید `openclaw.plugin.json`
  شامل شده است.
...
Plugin از barrel ریشه SDK منسوخ‌شده وارد می‌کند:
`openclaw/plugin-sdk`.

- واردکردن‌ها از root-barrel را با واردکردن‌های زیرمسیر عمومی متمرکز جایگزین کنید.
...
- برای یافتن واردکردن محدودتر، از [قراردادهای واردکردن](/fa/plugins/building-plugins#import-conventions) و
...
- واردکردن‌های SDK داخلی رزرو‌شده OpenClaw را با زیرمسیرهای عمومی مستندشده
...
- برای انتخاب واردکردن پشتیبانی‌شده، از [زیرمسیرهای Plugin SDK](/fa/plugins/sdk-subpaths) و
...
- از بارگذاری، جهش، و ذخیره‌سازی کل شیء مخزن نشست پرهیز کنید.
...
Plugin هنوز از یک کمک‌کننده نوشتن کل مخزن نشست منسوخ‌شده مانند
`saveSessionStore` یا `updateSessionStore` استفاده می‌کند.

- هنگام به‌روزرسانی فیلدها در یک ورودی نشست موجود از `patchSessionEntry(...)` استفاده کنید.
...
- برای خواندن فراداده نشست بر اساس هویت عامل و نشست از `getSessionEntry(...)` استفاده کنید.
...
- وقتی کد فقط به هویت نشست عمومی نیاز دارد، از `resolveSessionTranscriptIdentity(...)` استفاده کنید.
- وقتی کد به یک هدف ساختاریافته عملیات رونوشت نیاز دارد، از `resolveSessionTranscriptTarget(...)` استفاده کنید.
...
- سطح ساختاریافته زمان اجرای رونوشت را ترجیح دهید تا OpenClaw بتواند مرزهای تراکنش و مدیریت هویت درست را اعمال کند.
...
- کار بازنویسی مدل یا ارائه‌دهنده را به `before_model_resolve` منتقل کنید.
- کار جهش prompt یا زمینه را به `before_prompt_build` منتقل کنید.
...
مانیفست از فراداده env-var کانال قدیمی یا کهنه استفاده می‌کند، بدون فراداده setup یا config فعلی که ClawHub انتظار دارد.
...
- فقط پس از اینکه OpenClaw یک schema نسخه‌گذاری‌شده منتشر کرد، از آن استفاده کنید.
- `clawhub package validate <path-to-plugin>` را دوباره اجرا کنید.
...
- `openclaw.security.json` را حذف کنید تا زمانی که OpenClaw یک schema مانیفست امنیتی
  نسخه‌گذاری‌شده و رفتار ClawHub را مستند کند.
- رفتار حساس به امنیت را تا زمان وجود قرارداد مانیفست، در مستندات عمومی بسته یا
