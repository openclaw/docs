...
summary: پیش از انتشار، یافته‌های اعتبارسنجی بسته Plugin در ClawHub را برطرف کنید
title: رفع‌های اعتبارسنجی Plugin
x-i18n:
    generated_at: "2026-07-03T02:52:21Z"
    model: gpt-5.5
...
# رفع مشکلات اعتبارسنجی Plugin

ClawHub بسته‌های Plugin را پیش از انتشار اعتبارسنجی می‌کند و همچنین می‌تواند یافته‌های اسکن‌های خودکار بسته را نمایش دهد. این صفحه یافته‌های نویسنده‌محور را پوشش می‌دهد؛ یعنی یافته‌هایی که نویسنده Plugin می‌تواند در فراداده بسته، مانیفست، واردسازی‌های SDK، یا آرتیفکت منتشرشده خود اصلاح کند.

این صفحه یافته‌های پوشش داخلی Plugin Inspector را پوشش نمی‌دهد. اگر یک گزارش کامل شامل کدهای نگهداری اسکنر بدون راهنمای اصلاح برای نویسنده باشد، آن‌ها مربوط به نگه‌دارندگان OpenClaw هستند، نه نویسندگان Plugin.
...
| `security-manifest-schema-unavailable`  | [حذف ارجاع‌های طرح مانیفست امنیتی ناموجود](/fa/clawhub/plugin-validation-fixes#security-manifest-schema-unavailable)              |
...
- اگر `openclaw.plugin.json` نیز `version` دارد، آن را برای مطابقت به‌روزرسانی کنید یا وقتی فراداده بسته مرجع معتبر است، فراداده نسخه مانیفست منسوخ را حذف کنید.
...
- فراداده نامعتبر بسته، اسکریپت‌های چرخه عمر خراب، یا entries مربوط به files را که باعث شکست بسته‌بندی می‌شوند اصلاح کنید.
...
- واردسازی‌های root-barrel را با واردسازی‌های زیرمسیر عمومی و متمرکز جایگزین کنید.
- برای `definePluginEntry` از `openclaw/plugin-sdk/plugin-entry` استفاده کنید.
- برای کمک‌کننده‌های نقطهٔ ورود کانال از `openclaw/plugin-sdk/channel-core` استفاده کنید.
- برای یافتن واردسازی محدودتر، از [قراردادهای واردسازی](/fa/plugins/building-plugins#import-conventions) و
...
- واردسازی‌های SDK داخلی و رزروشدهٔ OpenClaw را با زیرمسیرهای عمومی مستندشدهٔ
...
Plugin هنوز از کمک‌کنندهٔ منسوخ‌شدهٔ کل انبار نشست
...
- از بارگذاری، جهش، و ذخیرهٔ کل شیء انبار نشست خودداری کنید.
...
Plugin هنوز از یک کمک‌کنندهٔ نوشتن منسوخ‌شدهٔ کل انبار نشست مانند
...
- از بارگذاری، جهش، و ذخیرهٔ کل شیء انبار نشست خودداری کنید.
- کمک‌کننده‌های نوشتن کل انبار را فقط تا زمانی نگه دارید که بازهٔ سازگاری اعلام‌شدهٔ شما
...
- برای خواندن فرادادهٔ نشست بر اساس هویت agent و نشست از `getSessionEntry(...)` استفاده کنید.
- برای پایدارسازی فرادادهٔ نشست از `patchSessionEntry(...)` یا `upsertSessionEntry(...)` استفاده کنید.
...
Plugin هنوز از کمک‌کننده‌های سطح‌پایین منسوخ‌شدهٔ رونوشت مانند
...
- سطح زمان اجرای ساختاریافتهٔ رونوشت را ترجیح دهید تا OpenClaw بتواند
...
مانیفست از فرادادهٔ env-var قدیمی یا قدیمی‌تر کانال استفاده می‌کند که فرادادهٔ setup یا config فعلی مورد انتظار ClawHub را ندارد.
...
بسته `openclaw.security.json` را با ارجاعی به schema منتشر می‌کند که ClawHub
...
- تا زمانی که OpenClaw یک schema نسخه‌دار مانیفست امنیتی و رفتار ClawHub را مستند نکرده است،
