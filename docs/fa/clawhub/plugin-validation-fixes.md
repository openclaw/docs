---
read_when:
    - دستور `clawhub package validate` را اجرا کرده‌اید و باید یافته‌های Plugin را برطرف کنید
    - ClawHub انتشار یک بستهٔ Plugin را رد کرد یا دربارهٔ آن هشدار داد
    - پیش از انتشار، در حال به‌روزرسانی فراداده‌های بستهٔ Plugin هستید
summary: یافته‌های اعتبارسنجی بستهٔ Plugin در ClawHub را پیش از انتشار برطرف کنید
title: اصلاحات اعتبارسنجی Plugin
x-i18n:
    generated_at: "2026-07-16T15:38:35Z"
    model: gpt-5.6
...
اسکن خودکار بسته‌ها را نمایش دهد. این صفحه یافته‌های مربوط به نویسنده را پوشش می‌دهد؛ یعنی
یافته‌هایی که نویسنده Plugin می‌تواند در فراداده بسته، مانیفست، درون‌ریزی‌های SDK
یا مصنوع منتشرشده اصلاح کند.

این صفحه یافته‌های داخلی پوشش Plugin Inspector را در بر نمی‌گیرد. اگر گزارشی کامل
شامل کدهای نگهداشت اسکنر باشد که راهنمایی‌ای برای اصلاح توسط نویسنده ندارند، آن‌ها
مربوط به نگه‌دارندگان OpenClaw هستند، نه نویسندگان Plugin.
...
| `package-openclaw-entry-missing`        | [تعریف نقاط ورود بسته OpenClaw](/fa/clawhub/plugin-validation-fixes#package-openclaw-entry-missing)                         |
| `package-entrypoint-missing`            | [انتشار نقطه ورود تعریف‌شده](/fa/clawhub/plugin-validation-fixes#package-entrypoint-missing)                                  |
...
| `package-plugin-api-compat-missing`     | [تعریف سازگاری API مربوط به Plugin](/fa/clawhub/plugin-validation-fixes#package-plugin-api-compat-missing)                          |
...
| `legacy-root-sdk-import`                | [جایگزینی درون‌ریزی‌های SDK از ریشه](/fa/clawhub/plugin-validation-fixes#legacy-root-sdk-import)                                             |
| `reserved-sdk-import`                   | [حذف درون‌ریزی‌های رزروشده SDK](/fa/clawhub/plugin-validation-fixes#reserved-sdk-import)                                             |
...
| `security-manifest-schema-unavailable`  | [حذف ارجاع‌های دردسترس‌نبودنی طرح‌واره مانیفست امنیتی](/fa/clawhub/plugin-validation-fixes#security-manifest-schema-unavailable) |
...
- اگر بسته یک Plugin برای OpenClaw ارائه می‌کند، بلوک `openclaw` را اضافه کنید.
...
بسته دارای `package.json` است، اما فراداده بسته
OpenClaw را تعریف نمی‌کند.

- `package.json#openclaw` را اضافه کنید.
- فراداده نقطه ورود، مانند `openclaw.extensions` یا
  `openclaw.runtimeExtensions`، را بگنجانید.
- اگر بسته از طریق ClawHub منتشر یا نصب خواهد شد، فراداده سازگاری و نصب را
  اضافه کنید.
- [فیلدهای package.json مؤثر بر کشف](/fa/plugins/manifest#packagejson-fields-that-affect-discovery) را ببینید.
- `clawhub package validate <path-to-plugin>` را دوباره اجرا کنید.

### package-openclaw-entry-missing

فراداده بسته وجود دارد، اما نقطه ورود زمان اجرای
OpenClaw را تعریف نمی‌کند.

- برای نقاط ورود بومی Plugin، `openclaw.extensions` را اضافه کنید.
- اگر بسته منتشرشده باید JavaScript ساخته‌شده را بارگذاری کند،
...
بسته یک نقطه ورود OpenClaw تعریف می‌کند، اما فایل ارجاع‌شده
...
- اگر بیش از یک منبع نصب دردسترس است،
...
بسته محدوده API مربوط به Plugin در OpenClaw را که پشتیبانی می‌کند تعریف نکرده است.

- `openclaw.compat.pluginApi` را به `package.json` اضافه کنید.
- از نسخه API مربوط به Plugin در OpenClaw یا حد پایین semver که بسته را بر اساس آن ساخته و
  آزمایش کرده‌اید استفاده کنید.
...
حداقل نسخه میزبان بسته با فراداده نسخه OpenClaw که بسته
بر اساس آن ساخته شده است مطابقت ندارد.

- `openclaw.install.minHostVersion` را بررسی کنید.
- هرگونه فراداده ساخت OpenClaw در بسته، مانند نسخه OpenClaw
  استفاده‌شده هنگام انتشار، را بررسی کنید.
- حداقل نسخه میزبان را با محدوده نسخه میزبانی که بسته
...
نسخه بسته و نسخه مانیفست Plugin با یکدیگر ناسازگارند.
...
- اگر `openclaw.plugin.json` نیز دارای `version` است، آن را برای مطابقت به‌روزرسانی کنید یا
  هنگامی که فراداده بسته مرجع معتبر است، فراداده قدیمی نسخه مانیفست را حذف کنید.
...
بسته را نمی‌توان به مصنوعی تبدیل کرد که ClawHub آن را بررسی یا
منتشر می‌کند.

- `npm pack --dry-run` را از ریشه بسته اجرا کنید.
- فراداده نامعتبر بسته، اسکریپت‌های چرخه عمر خراب یا ورودی‌های files که
  باعث شکست بسته‌بندی می‌شوند را اصلاح کنید.
- اگر این بسته برای انتشار عمومی در نظر گرفته شده است، `private: true` را حذف کنید.
- `clawhub package validate <path-to-plugin>` را دوباره اجرا کنید.

### package-npm-pack-entrypoint-missing

بسته قابل‌بسته‌بندی است، اما مصنوع بسته‌بندی‌شده فایل‌های
نقطه ورود تعریف‌شده در `package.json#openclaw` را شامل نمی‌شود.

- `npm pack --dry-run` را اجرا و فایل‌هایی را که گنجانده خواهند شد بررسی کنید.
- پیش از بسته‌بندی، نقاط ورود تولیدشده را بسازید.
- `files`، `.npmignore` یا خروجی ساخت را به‌روزرسانی کنید تا نقاط ورود تعریف‌شده
  گنجانده شوند.
- [نقاط ورود Plugin](/fa/plugins/sdk-entrypoints) را ببینید.
- `clawhub package validate <path-to-plugin>` را دوباره اجرا کنید.
...
مصنوع بسته‌بندی‌شده، فراداده OpenClaw موجود در بسته
منبع شما را ندارد.

- `npm pack --dry-run` را اجرا و فایل‌های فرادادهٔ موجود را بررسی کنید.
- مطمئن شوید `package.json` شامل بلوک `openclaw` در بستهٔ ساخته‌شده است.
- اگر بسته یک Plugin بومی OpenClaw است، مطمئن شوید
...
- `name` را خوانا برای انسان و `id` را به‌عنوان شناسهٔ پایدار ماشینی حفظ کنید.
- [مانیفست Plugin](/fa/plugins/manifest) را ببینید.
- `clawhub package validate <path-to-plugin>` را دوباره اجرا کنید.
...
مانیفست درون `contracts` کلیدهای پشتیبانی‌نشده‌ای تعریف کرده است.
...
- رفتار زمان اجرا را به کد ثبت Plugin منتقل کنید و `contracts` را
  به فرادادهٔ ایستای مالکیت قابلیت‌ها محدود نگه دارید.
- `clawhub package validate <path-to-plugin>` را دوباره اجرا کنید.
...
Plugin از barrel منسوخ‌شدهٔ ریشهٔ SDK وارد می‌کند:
`openclaw/plugin-sdk`.
...
- برای ابزارهای کمکی نقطهٔ ورود کانال از `openclaw/plugin-sdk/channel-core` استفاده کنید.
...
- واردسازی‌های رزروشدهٔ SDK داخلی OpenClaw را با زیرمسیرهای عمومی مستندشدهٔ
  `openclaw/plugin-sdk/*` جایگزین کنید.
- اگر این رفتار SDK عمومی ندارد، ابزار کمکی را درون بستهٔ خود نگه دارید یا
...
Plugin همچنان از ابزار کمکی منسوخ‌شدهٔ کل مخزن نشست
`loadSessionStore` استفاده می‌کند.

- هنگام خواندن وضعیت نشست، از `getSessionEntry(...)` یا
  `listSessionEntries(...)` استفاده کنید.
- هنگام نوشتن وضعیت نشست، از `patchSessionEntry(...)` یا
  `upsertSessionEntry(...)` استفاده کنید.
- از بارگذاری، تغییر و ذخیرهٔ کل شیء مخزن نشست خودداری کنید.
- `loadSessionStore(...)` را فقط تا زمانی نگه دارید که بازهٔ سازگاری اعلام‌شدهٔ شما
  همچنان نسخه‌های قدیمی‌تر OpenClaw را که به آن نیاز دارند پشتیبانی کند.
...
- هنگام به‌روزرسانی فیلدهای یک ورودی نشست موجود، از
  `patchSessionEntry(...)` استفاده کنید.
- هنگام جایگزینی یا ایجاد یک ورودی نشست، از `upsertSessionEntry(...)` استفاده کنید.
- از بارگذاری، تغییر و ذخیرهٔ کل شیء مخزن نشست خودداری کنید.
- ابزارهای کمکی نوشتن کل مخزن را فقط تا زمانی نگه دارید که بازهٔ سازگاری اعلام‌شدهٔ شما
  همچنان نسخه‌های قدیمی‌تر OpenClaw را که به آن‌ها نیاز دارند پشتیبانی کند.
- [API زمان اجرا](/fa/plugins/sdk-runtime#agent-session-state) و
  [زیرمسیرهای SDK مربوط به Plugin](/fa/plugins/sdk-subpaths) را ببینید.
- `clawhub package validate <path-to-plugin>` را دوباره اجرا کنید.
...
Plugin همچنان از ابزارهای کمکی منسوخ‌شدهٔ مسیر فایل نشست، مانند
`resolveSessionFilePath` یا `resolveAndPersistSessionFile`، استفاده می‌کند.

- برای خواندن فرادادهٔ نشست بر اساس هویت عامل و نشست، از
  `getSessionEntry(...)` استفاده کنید.
- برای ماندگارکردن فرادادهٔ نشست، از `patchSessionEntry(...)` یا
  `upsertSessionEntry(...)` استفاده کنید.
- هنگامی که کد در حال آماده‌سازی یک عملیات رونوشت است، از ابزارهای کمکی هویت
  یا مقصد رونوشت استفاده کنید.
...
Plugin همچنان از ابزار کمکی منسوخ‌شدهٔ مقصد فایل رونوشت
`resolveSessionTranscriptLegacyFileTarget` استفاده می‌کند.

- هنگامی که کد فقط به هویت عمومی نشست نیاز دارد، از
  `resolveSessionTranscriptIdentity(...)` استفاده کنید.
...
- ابزار کمکی قدیمی را فقط تا زمانی نگه دارید که بازهٔ سازگاری اعلام‌شدهٔ شما همچنان
  نسخه‌های قدیمی‌تر OpenClaw را که به آن نیاز دارند پشتیبانی کند.
- [API زمان اجرا](/fa/plugins/sdk-runtime#agent-session-state) و
  [زیرمسیرهای SDK مربوط به Plugin](/fa/plugins/sdk-subpaths) را ببینید.
- `clawhub package validate <path-to-plugin>` را دوباره اجرا کنید.
...
Plugin همچنان از ابزارهای سطح پایین منسوخ‌شدهٔ رونوشت، مانند
...
- برای اعلان‌های به‌روزرسانی رونوشت، از
  `publishSessionTranscriptUpdateByIdentity(...)` استفاده کنید.
- سطح ساختاریافتهٔ زمان اجرای رونوشت را ترجیح دهید تا OpenClaw بتواند
  مرزهای تراکنش و مدیریت هویت صحیح را اعمال کند.
- ابزارهای سطح پایین رونوشت را فقط تا زمانی نگه دارید که بازهٔ سازگاری اعلام‌شدهٔ شما
  همچنان نسخه‌های قدیمی‌تر OpenClaw را که به آن‌ها نیاز دارند پشتیبانی کند.
- [API زمان اجرا](/fa/plugins/sdk-runtime#agent-session-state) و
  [زیرمسیرهای SDK مربوط به Plugin](/fa/plugins/sdk-subpaths) را ببینید.
- `clawhub package validate <path-to-plugin>` را دوباره اجرا کنید.
...
مانیفست همچنان از فرادادهٔ قدیمی احراز هویت ارائه‌دهندهٔ
`providerAuthEnvVars` استفاده می‌کند.

- فرادادهٔ متغیرهای محیطی ارائه‌دهنده را در `setup.providers[].envVars` نیز منعکس کنید.
- `providerAuthEnvVars` را فقط به‌عنوان فرادادهٔ سازگاری نگه دارید، تا زمانی که بازهٔ
  پشتیبانی‌شدهٔ OpenClaw شما همچنان به آن نیاز دارد.
...
مانیفست از فرادادهٔ قدیمی یا کهنه‌تر متغیرهای محیطی کانال استفاده می‌کند، بدون فرادادهٔ
راه‌اندازی یا پیکربندی کنونی که ClawHub انتظار دارد.

- فرادادهٔ متغیرهای محیطی کانال را اعلانی نگه دارید تا OpenClaw بتواند وضعیت راه‌اندازی را
  بدون بارگذاری زمان اجرای کانال بررسی کند.
- راه‌اندازی مبتنی بر محیط کانال را در فرادادهٔ کنونی راه‌اندازی، پیکربندی کانال یا
...
- `channelEnvVars` را فقط به‌عنوان فرادادهٔ سازگاری نگه دارید، تا زمانی که نسخه‌های قدیمی‌تر
  پشتیبانی‌شدهٔ OpenClaw همچنان به آن نیاز دارند.
...
بسته، `openclaw.security.json` را با ارجاعی به طرح‌واره عرضه می‌کند که ClawHub
آن را در دسترس نمی‌شناسد.

- اگر URL طرح‌واره صرفاً جنبهٔ راهنمایی دارد، آن را حذف کنید.
- فقط پس از انتشار یک طرح‌وارهٔ نسخه‌دار توسط OpenClaw، از طرح‌وارهٔ نسخه‌دار مستندشده استفاده کنید.
- `clawhub package validate <path-to-plugin>` را دوباره اجرا کنید.
...
- `openclaw.security.json` را تا زمانی که OpenClaw یک طرح‌وارهٔ نسخه‌دار برای مانیفست امنیتی
