...
x-i18n:
    generated_at: "2026-06-28T08:03:19Z"
    model: gpt-5.5
...
ClawHub بسته‌های Plugin را پیش از انتشار اعتبارسنجی می‌کند و همچنین می‌تواند یافته‌های اسکن‌های خودکار بسته را نشان دهد. این صفحه یافته‌های روبه‌روی نویسنده را پوشش می‌دهد؛ یعنی یافته‌هایی که نویسنده Plugin می‌تواند در فراداده بسته، مانیفست، واردسازی‌های SDK، یا مصنوع منتشرشده خود اصلاح کند.
...
| `sdk-load-session-store`                | [جایگزینی دسترسی به کل ذخیره نشست](/fa/clawhub/plugin-validation-fixes#sdk-load-session-store)                                 |
...
- اگر `openclaw.plugin.json` نیز `version` دارد، آن را برای تطبیق به‌روزرسانی کنید یا وقتی فراداده بسته مرجع است، فراداده نسخه مانیفست کهنه را حذف کنید.
...
بسته نمی‌تواند به مصنوعی بسته‌بندی شود که ClawHub آن را بررسی یا منتشر می‌کند.
...
بسته می‌تواند بسته‌بندی شود، اما مصنوع بسته‌بندی‌شده شامل فایل‌های نقطه ورود اعلام‌شده در `package.json#openclaw` نیست.
...
- مطمئن شوید `package.json` شامل بلوک `openclaw` در مصنوع بسته‌بندی‌شده است.
...
- هر فیلد سطح بالا را با
  [مرجع فیلدهای سطح بالا در مانیفست](/fa/plugins/manifest#top-level-field-reference) مقایسه کنید.
...
- importهای barrel ریشه را با importهای زیرمسیر عمومی و متمرکز جایگزین کنید.
- برای `definePluginEntry` از `openclaw/plugin-sdk/plugin-entry` استفاده کنید.
- برای راهنماهای نقطه‌ی ورود کانال از `openclaw/plugin-sdk/channel-core` استفاده کنید.
...
- importهای SDK داخلی و رزرو‌شده‌ی OpenClaw را با زیرمسیرهای عمومی مستندشده‌ی
  `openclaw/plugin-sdk/*` جایگزین کنید.
- اگر این رفتار SDK عمومی ندارد، راهنما را داخل بسته‌ی خود نگه دارید یا
  یک API عمومی OpenClaw درخواست کنید.
- برای انتخاب یک import پشتیبانی‌شده، از [زیرمسیرهای SDK مربوط به Plugin](/fa/plugins/sdk-subpaths) و
  [مهاجرت SDK](/fa/plugins/sdk-migration) استفاده کنید.
- `clawhub package validate <path-to-plugin>` را دوباره اجرا کنید.

### sdk-load-session-store

Plugin همچنان از راهنمای منسوخ‌شده‌ی کل مخزن نشست
...
- از بارگذاری، جهش دادن و ذخیره کردن کل شیء مخزن نشست خودداری کنید.
...
- `before_agent_start` را فقط تا زمانی نگه دارید که بازه‌ی سازگاری اعلام‌شده‌ی شما همچنان
...
مانیفست همچنان از فراداده‌ی قدیمی احراز هویت ارائه‌دهنده با `providerAuthEnvVars` استفاده می‌کند.

- فراداده‌ی متغیرهای محیطی ارائه‌دهنده را در `setup.providers[].envVars` بازتاب دهید.
...
مانیفست از فراداده‌ی قدیمی یا کهنه‌ی متغیر محیطی کانال استفاده می‌کند، بدون فراداده‌ی setup یا config فعلی که ClawHub انتظار دارد.

- فراداده‌ی متغیر محیطی کانال را declarative نگه دارید تا OpenClaw بتواند وضعیت setup را
  بدون بارگذاری زمان اجرای کانال بررسی کند.
- setup کانال مبتنی بر env را در setup فعلی، config کانال، یا
  فراداده‌ی کانال بسته که توسط شکل Plugin شما استفاده می‌شود بازتاب دهید.
...
بسته، `openclaw.security.json` را با ارجاع schemaای عرضه می‌کند که ClawHub
آن را به‌عنوان موجود تشخیص نمی‌دهد.

- اگر URL مربوط به schema فقط جنبه‌ی راهنمایی دارد، آن را حذف کنید.
- فقط پس از اینکه OpenClaw یک schema نسخه‌دار منتشر کرد، از schema مستندشده‌ی نسخه‌دار استفاده کنید.
- `clawhub package validate <path-to-plugin>` را دوباره اجرا کنید.
...
- `openclaw.security.json` را تا زمانی که OpenClaw یک schema مانیفست امنیتی نسخه‌دار و
  رفتار ClawHub را مستند کند حذف کنید.
- رفتارهای حساس از نظر امنیتی را تا زمان وجود قرارداد مانیفست، در مستندات عمومی بسته یا
