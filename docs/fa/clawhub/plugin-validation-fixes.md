...
این صفحه یافته‌های پوشش داخلی Plugin Inspector را پوشش نمی‌دهد. اگر یک گزارش کامل شامل کدهای نگهداشت اسکنر بدون راهنمایی اصلاح برای نویسنده باشد، آن‌ها برای نگه‌دارندگان OpenClaw هستند نه نویسندگان Plugin.
...
- فرادادهٔ package یا install را به‌جای manifest به فیلدهای پشتیبانی‌شدهٔ `package.json#openclaw`
  منتقل کنید.
...
- رفتار runtime را به کد ثبت plugin منتقل کنید و `contracts`
  را فقط به فرادادهٔ ایستای مالکیت قابلیت محدود نگه دارید.
- دوباره `clawhub package validate <path-to-plugin>` را اجرا کنید.
...
- اگر این رفتار SDK عمومی ندارد، helper را داخل package خود نگه دارید یا
...
Plugin همچنان از helper منسوخ‌شدهٔ whole-session-store یعنی
...
manifest همچنان از فرادادهٔ احراز هویت provider قدیمی `providerAuthEnvVars` استفاده می‌کند.
...
manifest از فرادادهٔ قدیمی یا کهنه‌تر env-var کانال بدون فرادادهٔ setup یا config فعلی که ClawHub انتظار دارد استفاده می‌کند.
...
- setup کانال مبتنی بر env را در setup فعلی، config کانال یا
  فرادادهٔ کانال package که شکل plugin شما استفاده می‌کند منعکس کنید.
...
package فایل `openclaw.security.json` را با ارجاع schemaای عرضه می‌کند که ClawHub
...
package یک فایل manifest امنیتی پشتیبانی‌نشده عرضه می‌کند.

- تا زمانی که OpenClaw یک schema نسخه‌دار manifest امنیتی و رفتار ClawHub را مستند نکرده است،
  `openclaw.security.json` را حذف کنید.
- تا زمان وجود contract مربوط به manifest، رفتار حساس از نظر امنیتی را در docs عمومی package خود یا
