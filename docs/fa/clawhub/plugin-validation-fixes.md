...
x-i18n:
    generated_at: "2026-07-01T15:28:17Z"
    model: gpt-5.5
...
اسکن‌های خودکار بسته را نشان دهد. این صفحه یافته‌های ویژه نویسنده را پوشش می‌دهد؛ یعنی
...
| `sdk-load-session-store`                | [جایگزینی دسترسی به کل ذخیره‌ساز نشست](/fa/clawhub/plugin-validation-fixes#sdk-load-session-store)                                   |
| `sdk-session-store-write`               | [جایگزینی نوشتن‌های کل ذخیره‌ساز نشست](/fa/clawhub/plugin-validation-fixes#sdk-session-store-write)                                  |
| `sdk-session-file-helper`               | [جایگزینی راهنماهای مسیر فایل نشست](/fa/clawhub/plugin-validation-fixes#sdk-session-file-helper)                                   |
...
| `sdk-session-transcript-low-level`      | [جایگزینی راهنماهای سطح پایین رونوشت](/fa/clawhub/plugin-validation-fixes#sdk-session-transcript-low-level)                       |
...
- از نسخه API مربوط به Plugin در OpenClaw یا کف semver که بر اساس آن ساخته و آزموده‌اید
  استفاده کنید.
...
- هر فراداده ساخت OpenClaw را در بسته بررسی کنید، مانند نسخه OpenClaw
...
نسخه بسته و نسخه مانیفست Plugin با هم ناسازگار هستند.
...
بسته نمی‌تواند در قالب آرتیفکتی بسته‌بندی شود که ClawHub آن را بررسی یا
منتشر می‌کند.
...
Plugin از barrel ریشه منسوخ‌شده SDK ایمپورت می‌کند:
`openclaw/plugin-sdk`.

- ایمپورت‌های root-barrel را با ایمپورت‌های زیرمسیر عمومی متمرکز جایگزین کنید.
...
Plugin یک مسیر SDK را ایمپورت می‌کند که برای Pluginهای همراه یا سازگاری داخلی
رزرو شده است.
...
- برای انتخاب یک ایمپورت پشتیبانی‌شده، از [زیرمسیرهای SDK Plugin](/fa/plugins/sdk-subpaths) و
...
- وقتی کد در حال آماده‌سازی یک عملیات رونوشت است، از هویت رونوشت یا helperهای هدف استفاده کنید.
- مسیرهای فایل رونوشت legacy را پایدار نکنید و به آن‌ها وابسته نباشید.
- [API زمان اجرا](/fa/plugins/sdk-runtime#agent-session-state) و
  [زیرمسیرهای SDK Plugin](/fa/plugins/sdk-subpaths) را ببینید.
- `clawhub package validate <path-to-plugin>` را دوباره اجرا کنید.
...
- از خواندن یا ساختن مستقیم هدف‌های فایل رونوشت legacy خودداری کنید.
...
مانیفست هنوز از فراداده احراز هویت ارائه‌دهنده legacy با نام `providerAuthEnvVars` استفاده می‌کند.
...
آن را به‌عنوان مورد موجود تشخیص نمی‌دهد.
...
- تا زمانی که قرارداد مانیفست وجود ندارد، رفتار حساس از نظر امنیتی را در مستندات عمومی بسته
