...
title: ClawHub
x-i18n:
    generated_at: "2026-07-04T03:56:47Z"
    model: gpt-5.5
...
| Pluginهای کد | بسته‌های Plugin در OpenClaw با فراداده سازگاری | `openclaw plugins install clawhub:<package>` |
...
پایدار می‌کنند تا فرمان‌های به‌روزرسانی بعدی بتوانند روی ClawHub بمانند.

وقتی نصب یک Plugin باید از طریق ClawHub حل شود، از `clawhub:<package>` استفاده کنید.
مشخصات ساده و امن برای npm مربوط به Plugin ممکن است هنگام گذارهای راه‌اندازی از طریق npm
حل شوند، و وقتی منبع باید صریح باشد، `npm:<package>` فقط npm باقی می‌ماند.

نصب‌های Plugin پیش از اجرای نصب آرشیو، سازگاری `pluginApi` و `minGatewayVersion`
اعلام‌شده را اعتبارسنجی می‌کنند. وقتی نسخه‌ای از بسته یک artifact از نوع ClawPack منتشر می‌کند،
OpenClaw دقیق‌ترین `.tgz` بارگذاری‌شده با npm-pack را ترجیح می‌دهد، سرآیند digest در ClawHub
و بایت‌های دانلودشده را تأیید می‌کند، و فراداده artifact را برای به‌روزرسانی‌های بعدی ثبت می‌کند.
...
Pluginها را از یک پوشه محلی، `owner/repo`، `owner/repo@ref`، یا یک URL در GitHub
منتشر کنید:
...
از `--dry-run` برای ساخت برنامه دقیق انتشار بدون بارگذاری، و از `--json`
برای خروجی مناسب CI استفاده کنید.

Pluginهای کد باید فراداده سازگاری الزامی OpenClaw را در
`package.json` شامل `openclaw.compat.pluginApi` و
`openclaw.build.openclawVersion` داشته باشند. برای مرجع کامل فرمان‌ها [CLI](/fa/clawhub/cli)
و برای فراداده Skill [قالب Skill](/clawhub/skill-format) را ببینید.

## امنیت و نظارت

ClawHub به‌طور پیش‌فرض باز است: هرکسی می‌تواند بارگذاری کند، اما انتشار به یک حساب GitHub
...
ClawHub بررسی‌های خودکار را روی Skills منتشرشده و انتشارهای Plugin اجرا می‌کند. انتشارهای
نگه‌داشته‌شده به‌دلیل اسکن یا مسدودشده ممکن است از کاتالوگ عمومی و سطوح نصب ناپدید شوند،
...
محتوا را پنهان یا بازیابی کنند، و حساب‌های سوءاستفاده‌گر را ممنوع کنند. برای جزئیات سیاست و اجرا
[امنیت](/fa/clawhub/security)،
[ممیزی‌های امنیتی](/clawhub/security-audits)،
[نظارت و ایمنی حساب](/clawhub/moderation)، و
[کاربری پذیرفتنی](/fa/clawhub/acceptable-usage) را ببینید.

## تله‌متری و محیط

وقتی در حالت ورود، `clawhub install` را اجرا می‌کنید، CLI ممکن است یک رویداد نصب best-effort
ارسال کند تا ClawHub بتواند شمارش‌های تجمیعی نصب را محاسبه کند. این را با مورد زیر غیرفعال کنید:
...
| متغیر | اثر |
| ----------------------------- | ------------------------------------------------- |
| `CLAWHUB_SITE` | بازنویسی URL سایت استفاده‌شده برای ورود مرورگر. |
| `CLAWHUB_REGISTRY` | بازنویسی URL API رجیستری. |
| `CLAWHUB_CONFIG_PATH` | بازنویسی جایی که CLI وضعیت توکن/پیکربندی را ذخیره می‌کند. |
| `CLAWHUB_WORKDIR` | بازنویسی دایرکتوری کاری پیش‌فرض. |
| `CLAWHUB_DISABLE_TELEMETRY=1` | غیرفعال کردن تله‌متری نصب. |
