...
title: ClawHub
x-i18n:
    generated_at: "2026-07-02T22:39:53Z"
    model: gpt-5.5
...
وقتی گردش‌کارهای دارای احراز هویت رجیستری مانند انتشار یا حذف/بازگردانی حذف
را می‌خواهید، CLI مربوط به ClawHub را نصب کنید:
...
مشخصات Plugin ساده و سازگار با npm ممکن است هنگام جابه‌جایی‌های زمان راه‌اندازی از طریق npm resolve شوند، و
وقتی منبع باید صریح باشد، `npm:<package>` فقط مخصوص npm باقی می‌ماند.

نصب Pluginها پیش از اجرای نصب آرشیو، سازگاری اعلام‌شده `pluginApi` و `minGatewayVersion`
را اعتبارسنجی می‌کند. وقتی یک نسخه بسته یک مصنوع ClawPack منتشر می‌کند،
OpenClaw بسته دقیق npm-pack آپلودشده `.tgz` را ترجیح می‌دهد، هدر digest ClawHub
و بایت‌های دانلودشده را راستی‌آزمایی می‌کند، و فراداده مصنوع را برای به‌روزرسانی‌های بعدی ثبت می‌کند.
...
برای ساخت برنامه دقیق انتشار بدون آپلود، از `--dry-run` استفاده کنید، و برای خروجی مناسب CI
...
نیاز دارد که برای عبور از دروازه آپلود به‌اندازه کافی قدیمی باشد. صفحه‌های جزئیات عمومی
...
در اسکن نگه داشته شده‌اند یا مسدود شده‌اند ممکن است از کاتالوگ عمومی و سطح‌های نصب ناپدید شوند،
...
وقتی در حالت ورودشده `clawhub install` را اجرا می‌کنید، CLI ممکن است یک رویداد نصب best-effort
ارسال کند تا ClawHub بتواند شمار نصب‌های تجمیعی را محاسبه کند. این مورد را با این دستور غیرفعال کنید:
...
| متغیر                        | اثر                                               |
| ----------------------------- | ------------------------------------------------- |
| `CLAWHUB_SITE`                | URL سایت مورد استفاده برای ورود از طریق مرورگر را override می‌کند. |
