...
title: ClawHub
x-i18n:
    generated_at: "2026-07-01T08:19:11Z"
    model: gpt-5.5
...
- برای جست‌وجو، نصب، و به‌روزرسانی Skills و نصب Pluginها از ClawHub، از فرمان‌های بومی `openclaw` استفاده کنید.
- برای گردش‌کارهای احراز هویت رجیستری، انتشار، و حذف/بازگردانی حذف، از CLI جداگانه `clawhub` استفاده کنید.
...
وقتی نصب یک Plugin باید از طریق ClawHub resolve شود، از `clawhub:<package>` استفاده کنید. مشخصه‌های Plugin ساده و سازگار با npm ممکن است هنگام جابه‌جایی‌های زمان عرضه از طریق npm resolve شوند، و وقتی منبع باید صریح باشد، `npm:<package>` فقط npm باقی می‌ماند.

نصب‌های Plugin پیش از اجرای نصب آرشیو، سازگاری اعلام‌شده `pluginApi` و `minGatewayVersion` را اعتبارسنجی می‌کنند. وقتی نسخه‌ای از یک بسته یک آرتیفکت ClawPack منتشر می‌کند، OpenClaw همان npm-pack `.tgz` آپلودشده را ترجیح می‌دهد، سرآیند digest مربوط به ClawHub و بایت‌های دانلودشده را تأیید می‌کند، و فراداده آرتیفکت را برای به‌روزرسانی‌های بعدی ثبت می‌کند.
...
از `--dry-run` برای ساخت دقیق برنامه انتشار بدون آپلود، و از `--json` برای خروجی مناسب CI استفاده کنید.
...
ClawHub به‌صورت پیش‌فرض باز است: هر کسی می‌تواند آپلود کند، اما انتشار به یک حساب گیت‌هاب نیاز دارد که سن آن برای عبور از دروازه آپلود کافی باشد. صفحه‌های جزئیات عمومی، پیش از نصب یا دانلود، وضعیت آخرین اسکن را خلاصه می‌کنند.

ClawHub روی Skills منتشرشده و انتشارهای Plugin بررسی‌های خودکار اجرا می‌کند. انتشارهایی که به‌دلیل اسکن نگه داشته شده‌اند یا مسدود شده‌اند ممکن است از کاتالوگ عمومی و سطوح نصب ناپدید شوند، در حالی که همچنان برای مالکشان در `/dashboard` قابل مشاهده می‌مانند.
...
وقتی در حالت واردشده `clawhub install` را اجرا می‌کنید، CLI ممکن است یک رویداد نصب best-effort ارسال کند تا ClawHub بتواند شمارش‌های نصب تجمیعی را محاسبه کند. این را با مورد زیر غیرفعال کنید:
...
برای مطالب مرجع عمیق‌تر، [Telemetry](/clawhub/telemetry)، [HTTP API](/clawhub/http-api)، و [عیب‌یابی](/fa/clawhub/troubleshooting) را ببینید.
