...
x-i18n:
    generated_at: "2026-07-03T01:01:29Z"
    model: gpt-5.5
...
هنگامی که چیزی را در OpenClaw نصب می‌کنید، از OpenClaw استفاده کنید. هنگامی که در حال ورود، انتشار، مدیریت فهرست‌های خودتان، یا استفاده از گردش‌کارهای ویژهٔ رجیستری هستید، از CLI `clawhub` استفاده کنید.
...
این فرمان محتوای بدون تغییر را رد می‌کند. skillهای جدید از `1.0.0` شروع می‌شوند؛ تغییرات بعدی به‌طور خودکار نسخهٔ patch بعدی را منتشر می‌کنند. برای پیش‌نمایش از `--dry-run` یا برای انتخاب نسخه‌ای صریح از `--version` استفاده کنید.
...
برای مخزن‌هایی که شامل چندین skill هستند، گردش‌کار قابل‌استفادهٔ مجدد GitHub برای هر پوشهٔ مستقیم skill زیر `skills/` فرمان `skill publish` را فراخوانی می‌کند:
...
ابتدا از `--dry-run` استفاده کنید تا فرادادهٔ بستهٔ resolve‌شده، فیلدهای سازگاری، انتساب منبع، و طرح بارگذاری را بدون انتشار پیش‌نمایش کنید.
...
فهرست‌های عمومی آخرین وضعیت اسکن را نشان می‌دهند. انتشارهایی که توسط moderation نگه داشته یا مسدود شده‌اند ممکن است تا زمان رفع مشکل از سطوح جست‌وجو و نصب پنهان شوند.
