...
x-i18n:
    generated_at: "2026-07-01T08:20:03Z"
    model: gpt-5.5
...
وقتی چیزی را داخل OpenClaw نصب می‌کنید، از OpenClaw استفاده کنید. وقتی در حال ورود، انتشار، مدیریت فهرست‌های خودتان، یا استفاده از گردش‌کارهای ویژهٔ رجیستری هستید، از CLI `clawhub` استفاده کنید.
...
محیط‌های headless می‌توانند از یک توکن API از رابط وب ClawHub استفاده کنند:
...
این فرمان محتوای بدون تغییر را رد می‌کند. مهارت‌های جدید از `1.0.0` شروع می‌شوند؛ تغییرات بعدی به‌طور خودکار نسخهٔ patch بعدی را منتشر می‌کنند. برای پیش‌نمایش از `--dry-run` یا برای انتخاب یک نسخهٔ صریح از `--version` استفاده کنید.
...
برای مخزن‌هایی که چند مهارت دارند، گردش‌کار قابل‌استفادهٔ مجدد GitHub برای هر پوشهٔ مستقیم مهارت در زیر `skills/`، `skill publish` را فراخوانی می‌کند:
...
ابتدا از `--dry-run` استفاده کنید تا فرادادهٔ بستهٔ resolveشده، فیلدهای سازگاری، انتساب منبع و برنامهٔ بارگذاری را بدون انتشار پیش‌نمایش کنید.
...
پیش از نصب، از صفحهٔ وب ClawHub یا فرمان‌های جزئیات CLI استفاده کنید تا فراداده، پیوندهای منبع، نسخه‌ها، تغییرات نسخه و وضعیت اسکن را بررسی کنید:
...
فهرست‌های عمومی آخرین وضعیت اسکن را نشان می‌دهند. انتشارهایی که توسط نظارت نگه داشته یا مسدود شده‌اند ممکن است تا زمان رفع مشکل از سطوح جست‌وجو و نصب پنهان بمانند.
