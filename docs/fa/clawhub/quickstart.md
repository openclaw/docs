...
x-i18n:
    generated_at: "2026-07-03T02:52:36Z"
    model: gpt-5.5
...
محیط‌های بدون رابط کاربری می‌توانند از یک توکن API از UI وب ClawHub استفاده کنند:
...
این دستور محتوای بدون تغییر را رد می‌کند. Skills جدید از `1.0.0` شروع می‌شوند؛ تغییرات بعدی به‌طور خودکار نسخهٔ patch بعدی را منتشر می‌کنند. برای پیش‌نمایش از `--dry-run` یا برای انتخاب یک نسخهٔ صریح از `--version` استفاده کنید.
...
برای مخازنی که چندین skill دارند، گردش‌کار قابل‌استفادهٔ مجدد GitHub برای هر پوشهٔ skill مستقیم زیر `skills/` دستور `skill publish` را فراخوانی می‌کند:
