...
x-i18n:
    generated_at: "2026-07-02T22:40:34Z"
    model: gpt-5.5
...
وقتی می‌خواهید OpenClaw بسته را از طریق ClawHub، نه npm یا منبعی دیگر، حل‌وفصل کند، از پیشوند `clawhub:` استفاده کنید.
...
این دستور محتوای بدون تغییر را رد می‌کند. Skills جدید از `1.0.0` شروع می‌شوند؛ تغییرات بعدی به‌طور خودکار نسخه patch بعدی را منتشر می‌کنند. برای پیش‌نمایش از `--dry-run` یا برای انتخاب یک نسخه صریح از `--version` استفاده کنید.
...
برای مخازنی که چندین Skill دارند، گردش‌کار قابل استفاده‌مجدد GitHub برای هر پوشه Skill مستقیم زیر `skills/` دستور `skill publish` را فراخوانی می‌کند:
...
ابتدا از `--dry-run` استفاده کنید تا بدون انتشار، پیش‌نمایشی از فراداده بسته حل‌شده، فیلدهای سازگاری، انتساب منبع، و برنامه بارگذاری ببینید.
...
پیش از نصب، از صفحه وب ClawHub یا دستورهای جزئیات CLI برای بررسی فراداده، پیوندهای منبع، نسخه‌ها، changelogها و وضعیت اسکن استفاده کنید:
...
فهرست‌های عمومی آخرین وضعیت اسکن را نشان می‌دهند. انتشارهایی که توسط نظارت نگه داشته یا مسدود شده‌اند ممکن است تا زمان حل‌وفصل، از سطوح جست‌وجو و نصب پنهان بمانند.
