...
x-i18n:
    generated_at: "2026-07-04T18:10:41Z"
    model: gpt-5.5
...
این فرمان محتوای بدون تغییر را نادیده می‌گیرد. Skills جدید از `1.0.0` شروع می‌شوند؛ تغییرات بعدی به‌طور خودکار نسخهٔ patch بعدی را منتشر می‌کنند. برای پیش‌نمایش از `--dry-run` یا برای انتخاب نسخهٔ صریح از `--version` استفاده کنید.
...
برای مخازنی که چندین Skill دارند، گردش‌کار قابل استفادهٔ دوبارهٔ GitHub برای هر پوشهٔ Skill مستقیم زیر `skills/` فرمان `skill publish` را فراخوانی می‌کند:
...
ابتدا از `--dry-run` استفاده کنید تا پیش از انتشار، فرادادهٔ بستهٔ حل‌شده، فیلدهای سازگاری، انتساب منبع، و برنامهٔ بارگذاری را پیش‌نمایش کنید.
...
فهرست‌های عمومی آخرین وضعیت اسکن را نشان می‌دهند. نسخه‌هایی که به‌دلیل نظارت نگه داشته یا مسدود شده‌اند، ممکن است تا زمان رفع مشکل از سطوح جست‌وجو و نصب پنهان بمانند.
