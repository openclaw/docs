...
x-i18n:
    generated_at: "2026-07-04T03:57:15Z"
    model: gpt-5.5
...
محیط‌های بدون رابط می‌توانند از یک توکن API از رابط وب ClawHub استفاده کنند:
...
مهارت پوشه‌ای است با فایل الزامی `SKILL.md` و فایل‌های پشتیبان اختیاری.
...
برای مخزن‌هایی که چند مهارت دارند، گردش‌کار قابل استفادهٔ مجدد GitHub برای هر پوشهٔ مهارت مستقیم زیر `skills/`، `skill publish` را فراخوانی می‌کند:
...
پیش از نصب، از صفحهٔ وب ClawHub یا دستورهای جزئیات CLI استفاده کنید تا فراداده، پیوندهای منبع، نسخه‌ها، گزارش‌های تغییرات، و وضعیت اسکن را بررسی کنید:
...
فهرست‌های عمومی آخرین وضعیت اسکن را نشان می‌دهند. انتشارهایی که به‌دلیل نظارت نگه داشته یا مسدود شده‌اند، ممکن است تا زمان رفع مشکل از سطح‌های جست‌وجو و نصب پنهان بمانند.
