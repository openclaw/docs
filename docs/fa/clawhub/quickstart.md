...
x-i18n:
    generated_at: "2026-06-28T08:04:03Z"
    model: gpt-5.5
...
وقتی چیزی را در OpenClaw نصب می‌کنید، از OpenClaw استفاده کنید. وقتی در حال ورود، انتشار، مدیریت فهرست‌های خودتان، یا استفاده از جریان‌های کاری اختصاصی رجیستری هستید، از CLI `clawhub` استفاده کنید.
...
این دستور محتوای بدون تغییر را نادیده می‌گیرد. مهارت‌های جدید از `1.0.0` شروع می‌شوند؛ تغییرات بعدی به‌طور خودکار نسخه patch بعدی را منتشر می‌کنند. برای پیش‌نمایش از `--dry-run` یا برای انتخاب یک نسخه صریح از `--version` استفاده کنید.
...
برای مخزن‌هایی که چند مهارت دارند، جریان کاری قابل‌استفاده‌مجدد GitHub برای هر پوشه مهارت مستقیم زیر `skills/` دستور `skill publish` را فراخوانی می‌کند:
...
ابتدا از `--dry-run` استفاده کنید تا فراداده حل‌شده بسته، فیلدهای سازگاری، انتساب منبع و طرح بارگذاری را بدون انتشار پیش‌نمایش کنید.
