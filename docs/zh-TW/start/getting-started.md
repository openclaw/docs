...
**Windows 使用者：**支援原生 Windows 與 WSL2。WSL2 較穩定，建議用於完整體驗。請參閱 [Windows](/zh-TW/platforms/windows)。
...
/>
  ...
  <Step title="傳送你的第一則訊息">
    ...
    想改用手機聊天嗎？最快設定的頻道是
    [Telegram](/zh-TW/channels/telegram)（只需要一個機器人權杖）。所有選項請參閱[頻道](/zh-TW/channels)。
