...
OpenClaw 需要 **節點 22.19 或更新版本**。**節點 24 是安裝、CI 與發布工作流程的預設且建議執行環境**。節點 22 仍透過目前的 LTS 系列受到支援。[安裝程式腳本](/zh-TW/install#alternative-install-methods)會自動偵測並安裝節點 - 本頁適用於你想自行設定節點，並確認一切都已正確串接時（版本、PATH、全域安裝）。
...
如果這會印出 `v24.x.x` 或更高版本，表示你正在使用建議的預設版本。如果印出 `v22.19.x` 或更高版本，表示你正在使用受支援的節點 22 LTS 路徑，但我們仍建議在方便時升級到節點 24。如果尚未安裝節點，或版本太舊，請從下方選擇一種安裝方法。
...
這幾乎一定表示 npm 的全域 bin 目錄不在你的 PATH 中。
...
<Steps>
  ...
  <Step title="將它加入你的 shell 啟動檔">
    <Tabs>
      ...
      <Tab title="Windows">
        透過「設定」→「系統」→「環境變數」，將 `npm prefix -g` 的輸出加入你的系統 PATH。
      </Tab>
    </Tabs>
...
將 `export PATH=...` 這一行加入你的 `~/.bashrc` 或 `~/.zshrc`，讓它永久生效。
