...
這三者都支援 Node **22.19+、23.11+ 或 24+**；全新安裝預設以 Node 24 為目標版本。
...
<Steps>
  ...
  <Step title="預設確保使用 Node.js 24">
    檢查 Node 版本，並視需要安裝 Node 24（macOS 使用 Homebrew，Linux apt/dnf/yum 使用 NodeSource 設定指令碼）。在 macOS 上，只有當安裝程式需要 Homebrew 來安裝 Node 或 Git 時，才會安裝 Homebrew。為了相容性，仍支援 Node 22.19+ 和 23.11+。
    在 Alpine/musl Linux 上，安裝程式會使用 apk 套件而非 NodeSource；設定的 Alpine 軟體庫必須提供受支援的 Node 版本（撰寫本文時為 Alpine 3.21 或更新版本）。
  </Step>
  <Step title="確保已安裝 Git">
    如果缺少 Git，便使用偵測到的套件管理器進行安裝，包括 macOS 上的 Homebrew 和 Alpine 上的 apk。
  </Step>
  <Step title="安裝 OpenClaw">
    - `npm` 方式（預設）：全域 npm 安裝
    - `git` 方式：複製/更新儲存庫、使用 pnpm 安裝相依套件、建置，然後在 `~/.local/bin/openclaw` 安裝包裝程式

  </Step>
  <Step title="安裝後工作">
    - 解析剛安裝的 `openclaw` 二進位檔，以供後續命令使用
    - 對於尚未設定的安裝，會在 doctor 或閘道探測之前啟動初始設定。使用 `--no-onboard` 或沒有 TTY 時，會顯示稍後完成設定所需的命令。
    - 對於已設定的安裝，會盡力重新整理並重新啟動已載入的閘道服務，然後執行 doctor。升級時會盡可能更新外掛；若在已啟用提示但無介面的執行環境中，則會顯示手動命令。
    - 執行 `--verify` 時，會檢查已安裝的版本，且僅在已有設定後才檢查閘道健康狀態。
...
如果在 OpenClaw 簽出目錄（`package.json` + `pnpm-workspace.yaml`）內執行，指令碼會提供以下選項：
...
選取無效的方式或 `--install-method` 值無效時，指令碼會以代碼 `2` 結束。
...
| `OPENCLAW_NPM_LOGLEVEL=error\|warn\|notice`       | npm 記錄層級（預設：`error`，隱藏 npm 棄用訊息雜訊）         |
...
<Info>
專為希望將所有內容置於本機前綴路徑下的環境而設計
（預設為 `~/.openclaw`），且不依賴系統 Node。預設支援 npm 安裝，
...
<Steps>
  <Step title="安裝本機 Node 執行環境">
    將固定版本且受支援的 Node LTS tarball（版本嵌入於指令碼中並獨立更新，預設為 `22.22.2`）下載至 `<prefix>/tools/node-v<version>`，並驗證 SHA-256。
    在 Alpine/musl Linux 上，由於 Node 不會為該固定執行環境發布相容的 tarball，因此會使用 `apk` 安裝 `nodejs` 和 `npm`，並將該執行環境連結至前綴路徑的包裝程式路徑。Alpine 軟體庫必須提供受支援的 Node 版本（22.19+、23.11+ 或 24+）；如果較舊的軟體庫僅提供 Node 20 或 21，請使用 Alpine 3.21 或更新版本。
  </Step>
  <Step title="確保已安裝 Git">
    如果缺少 Git，會嘗試在 Linux 上透過 apt/dnf/yum/apk 安裝，或在 macOS 上透過 Homebrew 安裝。
  </Step>
  <Step title="在前綴路徑下安裝 OpenClaw">
    - `npm` 方式（預設）：使用 npm 安裝至前綴路徑下，然後將包裝程式寫入 `<prefix>/bin/openclaw`
    - `git` 方式：複製/更新簽出目錄（預設為 `~/openclaw`），且仍會將包裝程式寫入 `<prefix>/bin/openclaw`

  </Step>
  <Step title="重新整理已載入的閘道服務">
    如果已有閘道服務從相同前綴路徑載入，指令碼會執行
...
| `--no-onboard`                          | 略過新手引導（預設）                                                       |
| `--set-npm-prefix`                      | 在 Linux 上，如果目前的 npm 前綴不可寫入，則強制設為 `~/.npm-global` |
| `--help \| -h`                          | 顯示用法                                                                      |
...
| `OPENCLAW_HOME=<path>`                      | OpenClaw 狀態及預設 git／新手引導路徑的基礎目錄 |
...
<Steps>
  ...
  <Step title="確保預設使用 Node.js 24">
    如果缺少，會依序嘗試透過 winget、Chocolatey、Scoop 安裝。如果沒有可用的套件管理器，指令碼會將官方 Node.js 24 Windows zip 下載至 `%LOCALAPPDATA%\OpenClaw\deps\portable-node`，並將其加入目前處理程序及使用者的 PATH。為維持相容性，仍支援 Node 22.19+ 和 23.11+。
  </Step>
  <Step title="安裝 OpenClaw">
    - `npm` 方式（預設）：使用所選的 `-Tag` 進行全域 npm 安裝，並從可寫入的安裝程式暫存目錄啟動，因此即使在 `C:\` 等受保護資料夾中開啟的殼層仍可運作
    - `git` 方式：複製／更新儲存庫、使用 pnpm 安裝／建置，並將包裝程式安裝至 `%USERPROFILE%\.local\bin\openclaw.cmd`。如果缺少 Git，指令碼會在 `%LOCALAPPDATA%\OpenClaw\deps\portable-git` 下啟動使用者本機 MinGit，並將其加入目前處理程序及使用者的 PATH。

  </Step>
  <Step title="安裝後工作">
    - 盡可能將所需的 bin 目錄加入使用者 PATH
    - 以盡力而為方式重新整理已載入的閘道服務（`openclaw gateway install --force`，接著重新啟動）
    - 升級及 git 安裝時執行 `openclaw doctor --non-interactive`（盡力而為）

  </Step>
  <Step title="處理失敗">
    `iwr ... | iex` 和指令碼區塊安裝會回報終止錯誤，但不會關閉目前的 PowerShell 工作階段。直接使用 `powershell -File`／`pwsh -File` 安裝時，仍會以非零狀態結束，供自動化流程判斷。
  </Step>
</Steps>
...
<AccordionGroup>
  ...
  <Accordion title="為什麼 npm 在 Linux 上遇到 EACCES？">
    某些 Linux 設定會將 npm 的全域前綴指向 root 擁有的路徑。`install.sh` 可將前綴切換至 `~/.npm-global`，並將 PATH 匯出設定附加至殼層 rc 檔案（如果這些檔案存在）。
  </Accordion>
