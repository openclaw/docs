...
read_when:
    - 你想要具備安全強化的自動化伺服器部署
    - 你需要具防火牆隔離並可透過 VPN 存取的設定
    - 你正在部署到遠端 Debian/Ubuntu 伺服器
summary: 使用 Ansible、Tailscale VPN 與防火牆隔離的自動化強化 OpenClaw 安裝
title: Ansible
x-i18n:
    generated_at: "2026-06-27T19:26:02Z"
    model: gpt-5.5
...
- **Tailscale VPN** -- 安全的遠端存取，不需公開暴露服務
- **Docker** -- 隔離的沙盒容器，僅綁定 localhost
- **縱深防禦** -- 4 層安全架構
- **Systemd 整合** -- 開機自動啟動並具備強化設定
- **單一命令設定** -- 數分鐘內完成部署
...
3. **Docker CE + Compose V2** -- 用於預設代理沙盒後端
4. **Node.js 24 + pnpm** -- 執行階段相依性（Node 22 LTS，目前為 `22.19+`，仍受支援）
5. **OpenClaw** -- 以主機為基礎，不容器化
6. **Systemd 服務** -- 具備安全強化的自動啟動

<Note>
閘道會直接在主機上執行（不在 Docker 中）。代理沙盒是
選用功能；此 playbook 會安裝 Docker，因為它是預設沙盒
後端。請參閱 [沙盒](/zh-TW/gateway/sandboxing) 了解詳細資訊與其他後端。
</Note>
...
只有連接埠 22 (SSH) 應該開啟。所有其他服務（閘道、Docker）都會被鎖定。

Docker 是為代理沙盒（隔離的工具執行）而安裝，不是用來執行閘道本身。請參閱 [多代理沙盒與工具](/zh-TW/tools/multi-agent-sandbox-tools) 了解沙盒設定。
...
Ansible 安裝程式會將 OpenClaw 設定為手動更新。請參閱 [更新](/zh-TW/install/updating) 了解標準更新流程。
...
- [沙盒](/zh-TW/gateway/sandboxing) -- 代理沙盒設定
- [多代理沙盒與工具](/zh-TW/tools/multi-agent-sandbox-tools) -- 每個代理的隔離
