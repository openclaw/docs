...
read_when:
    ...
    - 你需要具備 VPN 存取能力且由防火牆隔離的設定
    - 你正在部署至遠端 Debian/Ubuntu 伺服器
summary: 使用 Ansible、Tailscale VPN 與防火牆隔離，自動化且經過強化的 OpenClaw 安裝方式
title: Ansible
x-i18n:
    generated_at: "2026-07-11T21:24:32Z"
    model: gpt-5.6
...
使用以安全性為優先架構的自動化安裝程式 **[openclaw-ansible](https://github.com/openclaw/openclaw-ansible)**，將 OpenClaw 部署至正式環境伺服器。
...
- 防火牆優先的安全防護：UFW + Docker 隔離（僅可連線至 SSH + Tailscale）
- 使用 Tailscale VPN 進行遠端存取，不需將服務公開至網際網路
- 使用 Docker 建立隔離的沙箱容器，且僅繫結至 localhost
- 整合具備強化安全性的 systemd，並在開機時自動啟動
- 單一命令即可完成設定
...
5. OpenClaw，以主機型態安裝，不進行容器化
6. 具備安全性強化設定的 systemd 服務

<Note>
閘道直接在主機上執行，而非在 Docker 中執行。代理程式沙箱為
選用功能；此 playbook 會安裝 Docker，因為它是預設的沙箱
後端。如需其他後端，請參閱[沙箱](/zh-TW/gateway/sandboxing)。
</Note>
...
1. 防火牆（UFW）：僅將 SSH（22）和 Tailscale（41641/udp）公開至網際網路
2. VPN（Tailscale）：僅能透過 VPN 網狀網路連線至閘道
3. Docker 隔離：`DOCKER-USER` iptables 鏈可防止連接埠暴露於外部
4. Systemd 安全性強化：`NoNewPrivileges`、`PrivateTmp`、非特權使用者
...
應僅開放連接埠 22（SSH）。閘道和 Docker 會維持封鎖狀態。

Docker 用於代理程式沙箱（隔離的工具執行環境），而非用於執行閘道。如需沙箱設定，請參閱[多代理程式沙箱與工具](/zh-TW/tools/multi-agent-sandbox-tools)。
...
<AccordionGroup>
  ...
  <Accordion title="Docker 沙箱問題">
    ...
    # 若缺少沙箱映像檔，請建置該映像檔（需要原始碼檢出）
    cd /opt/openclaw/openclaw
    sudo -u openclaw ./scripts/sandbox-setup.sh
    # 若使用 npm 安裝且沒有原始碼檢出，請參閱
    # https://docs.openclaw.ai/gateway/sandboxing#images-and-setup
    ```

  </Accordion>
  <Accordion title="通道登入失敗">
    請確定你是以 `openclaw` 使用者身分執行：
    ```bash
    sudo -i -u openclaw
    openclaw channels login --channel <name>
    ```
...
如需詳細的安全性架構和疑難排解資訊，請參閱 openclaw-ansible 儲存庫：
