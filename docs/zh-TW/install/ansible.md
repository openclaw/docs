...
使用 **[openclaw-ansible](https://github.com/openclaw/openclaw-ansible)** 將 OpenClaw 部署到生產伺服器 -- 一個採用安全優先架構的自動化安裝程式。
...
- **Tailscale VPN** -- 安全的遠端存取，不公開暴露服務
- **Docker** -- 隔離的沙箱容器，僅限 localhost 綁定
- **縱深防禦** -- 4 層安全架構
- **Systemd 整合** -- 開機時自動啟動並加固
- **單一命令設定** -- 數分鐘內完成部署
...
3. **Docker CE + Compose V2** -- 用於預設的代理程式沙箱後端
4. **Node.js 24 + pnpm** -- 執行階段相依項（Node 22 LTS，目前為 `22.14+`，仍受支援）
5. **OpenClaw** -- 在主機上執行，非容器化
6. **Systemd 服務** -- 自動啟動並進行安全加固

<Note>
Gateway 直接在主機上執行（不在 Docker 中）。代理程式沙箱是
選用的；此 playbook 會安裝 Docker，因為它是預設的沙箱
後端。詳情與其他後端請參閱[沙箱](/zh-TW/gateway/sandboxing)。
</Note>
...
Docker 是為代理程式沙箱（隔離的工具執行）而安裝，不是用來執行 Gateway 本身。沙箱設定請參閱[多代理程式沙箱與工具](/zh-TW/tools/multi-agent-sandbox-tools)。
