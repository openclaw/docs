---
read_when:
    - 將 OpenClaw 部署到 Northflank
    - 你想要一鍵式雲端部署，並使用以瀏覽器為基礎的控制 UI
summary: 使用一鍵式範本在 Northflank 上部署 OpenClaw
title: Northflank
x-i18n:
    generated_at: "2026-04-30T03:16:20Z"
    model: gpt-5.5
    provider: openai
...
使用一鍵式範本在 Northflank 上部署 OpenClaw，並透過網頁版控制 UI 存取。
這是最簡單的「伺服器上無需終端機」路徑：Northflank 會為你執行 Gateway。
...
- 透過 Northflank Volume (`/data`) 提供持久化儲存空間，讓 `openclaw.json`、
