...
外掛、檢查 `computer-use` MCP 伺服器是否可用，接著讓
Codex 在 Codex 模式的回合期間負責原生 MCP 工具呼叫。

當 OpenClaw 已使用原生 Codex 執行框架時，請使用此頁面。若要了解
執行環境本身的設定，請參閱 [Codex 執行框架](/zh-TW/plugins/codex-harness)。

這與 OpenClaw 內建的[節點支援電腦工具](/zh-TW/nodes/computer-use)不同。若相同的代理程式合約應控制已配對的 Mac，無論代理程式是在閘道還是另一個節點上執行，都請使用內建工具。若應由 Codex app-server 負責本機 MCP 安裝、權限和原生工具呼叫，請使用 Codex Computer Use。

## OpenClaw.app 與 Peekaboo

OpenClaw.app 的 Peekaboo 整合與 Codex Computer Use 彼此獨立。
macOS 應用程式可以託管 PeekabooBridge 通訊端，讓 `peekaboo` 命令列介面重複使用
應用程式的本機「輔助使用」和「螢幕錄製」授權，以執行 Peekaboo 自己的
自動化工具。該橋接器不會安裝或代理 Codex Computer Use，而
Codex Computer Use 也不會透過 PeekabooBridge 通訊端呼叫。

若你希望 OpenClaw.app 成為能感知權限的主機，以執行 Peekaboo 命令列介面自動化，
請使用 [Peekaboo 橋接器](/zh-TW/platforms/mac/peekaboo)。若 Codex 模式的
OpenClaw 代理程式應在回合開始前就能使用 Codex 原生的 `computer-use` MCP 外掛，
請使用此頁面。

## iOS 應用程式

iOS 應用程式與 Codex Computer Use 彼此獨立。它不會安裝或代理
Codex `computer-use` MCP 伺服器，也不是桌面控制後端。
iOS 應用程式會改以 OpenClaw 節點身分連線，並透過
`canvas.*`、`camera.*`、`screen.*`、
`location.*` 和 `talk.*` 等節點命令公開行動裝置功能。

若你希望代理程式透過閘道控制 iPhone 節點，
請使用 [iOS](/zh-TW/platforms/ios)。若 Codex 模式代理程式應透過 Codex 原生的 Computer Use 外掛
控制本機 macOS 桌面，請使用此頁面。

## 直接使用 cua-driver MCP

Codex Computer Use 並不是公開桌面控制的唯一方式。若你希望
OpenClaw 管理的執行環境直接呼叫 TryCua 的驅動程式，請透過 OpenClaw 的 MCP 登錄檔使用上游
`cua-driver mcp` 伺服器，而不是 Codex 專用的市集流程。
...
結構描述和結構化 MCP 回應。若你希望將 CUA 驅動程式
當作一般 OpenClaw MCP 伺服器使用，請選擇此方式。若應由 Codex app-server 負責外掛安裝、MCP 重新載入，
以及 Codex 模式回合內的原生工具呼叫，請使用本頁面的 Codex Computer Use 設定。

CUA 的驅動程式僅適用於 macOS，且仍需要其應用程式提示的本機 macOS 權限，
例如「輔助使用」和「螢幕錄製」。OpenClaw 不會
安裝 `cua-driver`、授予這些權限，或繞過上游
驅動程式的安全模型。

## 快速設定

若 Codex 模式回合必須在討論串開始前就能使用
Computer Use，請設定 `plugins.entries.codex.config.computerUse`。`autoInstall: true` 會
選用 Computer Use，並允許 OpenClaw 在回合前安裝或重新啟用它：
...
回合前檢查 Codex app-server。若 Computer Use 遺失，但 Codex app-server 已探索到
可安裝的市集，OpenClaw 會要求 Codex app-server 安裝或
重新啟用外掛，並重新載入 MCP 伺服器。在 macOS 上，若未登錄相符的
市集，且存在標準桌面應用程式套件，OpenClaw
也會嘗試從
`/Applications/ChatGPT.app/Contents/Resources/plugins/openai-bundled` 登錄隨附的 Codex 市集，並保留
`/Applications/Codex.app/Contents/Resources/plugins/openai-bundled`
作為舊版獨立安裝的備援。若設定後仍無法讓
MCP 伺服器可用，回合會在討論串開始前失敗。

變更 Computer Use 設定後，若現有 Codex 討論串已經開始，
請先在受影響的聊天中使用 `/new` 或 `/reset`，再進行測試。

在 macOS 上，Computer Use 的受管理啟動會優先使用位於
`/Applications/ChatGPT.app/Contents/Resources/codex` 的桌面應用程式執行檔，接著針對舊版
獨立安裝，備援至 `/Applications/Codex.app/Contents/Resources/codex`。
這也適用於會自行啟動用戶端的一次性 Computer Use 狀態與
安裝命令。這能讓桌面控制維持在
擁有本機 macOS 權限的應用程式套件之下。若未安裝桌面應用程式，
OpenClaw 會備援至安裝在外掛旁的受管理 Codex 執行檔。
使用預設隔離代理程式主目錄的一般受管理 Codex 回合會優先採用
該固定版本的套件，以免較舊的桌面應用程式遮蔽目前的模型支援。
使用者範圍的主目錄仍會優先使用桌面應用程式，因為它們可以載入原生
Computer Use 狀態。若隔離代理程式主目錄的有效 Codex 設定已啟用
...
閘道外變更原生 Codex 外掛設定後，請重新啟動閘道並開始新的聊天，
再依賴新的選擇。

## 命令

你可以在任何提供 `codex` 外掛命令介面的聊天介面中使用
`/codex computer-use` 命令。這些是 OpenClaw 聊天／執行環境
...
`status` 是預設操作，且為唯讀：它不會新增市集
來源、安裝外掛，或啟用 Codex 外掛支援。若沒有設定選用
...
| `marketplaceSource`  | 你有 app-server 可以新增的 Codex 市集來源。                     | 是，適用於明確的 `/codex computer-use install`。         |
| `marketplacePath`    | 你已知道主機上的本機市集檔案路徑。                              | 是，適用於明確安裝和回合開始時的自動安裝。               |
| `marketplaceName`    | 你想依名稱選擇已登錄的市集。                                    | 僅當所選市集具有本機路徑時才支援。                       |

新的 Codex 主目錄可能需要一小段時間來建立其官方
市集。安裝期間，OpenClaw 會輪詢 `plugin/list`，最長
`marketplaceDiscoveryTimeoutMs` 毫秒（預設 60 秒）。

若多個已知市集包含 Computer Use，OpenClaw 會依序偏好
`openai-bundled`、`openai-curated`，接著是 `local`。無法辨識且不明確的
相符項目會採取封閉式失敗，並要求你設定 `marketplaceName` 或
`marketplacePath`。

## 隨附的 macOS 市集

目前的 ChatGPT 桌面版組建會在此處隨附 Computer Use；舊版獨立
...
當 `computerUse.autoInstall` 為 true，且尚未登錄包含
`computer-use` 的市集時，OpenClaw 會嘗試新增第一個
存在的標準隨附市集根目錄：
...
| 欄位                            | 預設值         | 意義                                                                               |
| ------------------------------- | -------------- | ---------------------------------------------------------------------------------- |
| `enabled`                       | 推斷           | 要求使用「電腦操作」。設定其他「電腦操作」欄位時，預設為 true。                    |
| `autoInstall`                   | false          | 在回合開始時，從已探索到的市集安裝或重新啟用。                                     |
| `marketplaceDiscoveryTimeoutMs` | 60000          | 安裝等待 Codex app-server 完成市集探索的時間。                                      |
| `liveTestTimeoutMs`             | 60000          | 暫時性就緒執行緒及其清理要求的逾時時間。                                            |
...
| `pluginCacheMode`               | `independent`  | 使用 `shared`，透過隨附的桌面外掛重新整理 Codex 主目錄快取。                        |
| `strictReadiness`               | false          | 即時探測失敗時停止啟動，而不是顯示警告後繼續。                                     |
| `autoRepair`                    | false          | 終止範圍內過時的「電腦操作」MCP 子程序，並對失敗的探測重試一次。                    |
...
| 原因                         | 意義                                                   | 下一步                                        |
| ---------------------------- | ------------------------------------------------------ | --------------------------------------------- |
| `disabled`                   | `computerUse.enabled` 解析為 false。                   | 設定 `enabled` 或其他「電腦操作」欄位。       |
...
| `mcp_missing`                | 外掛已啟用，但 MCP 伺服器無法使用。                    | 檢查 Codex「電腦操作」和作業系統權限。        |
| `ready`                      | 外掛和 MCP 工具均可使用。                              | 開始 Codex 模式回合。                         |
| `check_failed`               | 狀態檢查期間的 Codex app-server 要求失敗。             | 檢查 app-server 連線和記錄。                  |
| `auto_install_blocked`       | 回合開始時的設定需要新增來源。                         | 先執行明確安裝。                              |

聊天輸出包括外掛狀態、MCP 伺服器狀態、市集、
可用時的工具，以及設定失敗步驟的特定訊息。

## macOS 權限

「電腦操作」僅適用於 macOS。Codex 所屬的 MCP 伺服器可能需要本機作業系統
權限，才能檢查或控制應用程式。如果 OpenClaw 顯示「電腦操作」
已安裝，但 MCP 伺服器無法使用，請先確認 Codex 端的
「電腦操作」設定：

- Codex app-server 正在應執行桌面控制的同一台主機上執行。
- 「電腦操作」外掛已在 Codex 設定中啟用。
- `computer-use` MCP 伺服器出現在 Codex app-server MCP 狀態中。
- macOS 已授予桌面控制應用程式所需的權限。
- 目前的主機工作階段可以存取受控制的桌面。

當 `computerUse.enabled` 為 true 時，OpenClaw 會刻意採取失敗關閉。
Codex 模式回合不應在缺少設定所要求的原生桌面工具時
無提示地繼續。
...
**狀態顯示不支援遠端安裝。** 使用本機市集
來源或路徑。僅限遠端的目錄項目可以檢查，但無法
透過目前的 app-server API 安裝。

**狀態顯示 MCP 伺服器無法使用。** 重新執行一次安裝，讓 MCP
伺服器重新載入。如果仍無法使用，請修正 Codex「電腦操作」應用程式、
Codex app-server MCP 狀態或 macOS 權限。

**狀態或探測在 `computer-use.list_apps` 上逾時。** 外掛和
MCP 伺服器都存在，但本機「電腦操作」橋接器沒有回應。
結束或重新啟動 Codex「電腦操作」，必要時重新啟動 Codex Desktop，然後
在新的 OpenClaw 工作階段中重試。如果主機先前曾透過
較舊的受管理 Codex app-server 執行「電腦操作」，請從
桌面隨附的市集重新整理已安裝的外掛（獨立安裝的
Codex 桌面版請使用 `Codex.app` 路徑）：

```text
/codex computer-use install --source /Applications/ChatGPT.app/Contents/Resources/plugins/openai-bundled
```

**「電腦操作」工具顯示 `Native hook relay unavailable`。** 
Codex 原生工具掛鉤無法透過本機橋接器或閘道後援，
連線至作用中的 OpenClaw 轉送器。使用 `/new`
或 `/reset` 啟動新的 OpenClaw 工作階段。如果它成功一次，但之後的工具呼叫
再次失敗，`/new` 只會清除目前的嘗試；請重新啟動 Codex app-server 或
OpenClaw 閘道，以移除舊的執行緒和掛鉤註冊，然後
在新的工作階段中重試。
