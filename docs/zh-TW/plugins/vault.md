...
內建的 Vault 外掛可讓 OpenClaw 在閘道啟動及重新載入時，從 HashiCorp Vault 解析 `exec` SecretRef。OpenClaw 會將 Vault 參照儲存在設定中，將解析後的值保留於記憶體內的密鑰快照，且不會把解析後的 API 金鑰寫回 `openclaw.json`。

若你已經在執行 Vault，或希望模型供應商金鑰存放在 OpenClaw 設定檔之外，請使用此功能。如需了解 SecretRef 執行階段模型，請參閱[密鑰管理](/zh-TW/gateway/secrets)。
...
- 可連線的 Vault 伺服器
- 能產生用戶端權杖的 Vault 驗證，且該權杖必須具有 OpenClaw 所需解析之密鑰路徑的讀取權限
- 啟動閘道的環境必須包含 `VAULT_ADDR`，以及下列其中一種設定：`VAULT_TOKEN`、搭配 `VAULT_TOKEN_FILE` 的 `OPENCLAW_VAULT_AUTH_METHOD=token_file`，或已設定的 JWT/Kubernetes 登入

解析器會從 Node 透過 HTTP 與 Vault 通訊。閘道解析 SecretRef 時不需要 Vault 命令列介面。
...
OpenClaw 預設使用掛載於 `secret` 的 KV v2，與 Vault 開發伺服器範例相符。若使用正式環境 Vault，請在建立 SecretRef ID 前，將 `OPENCLAW_VAULT_KV_MOUNT` 設為實際的 KV 掛載路徑。採用 OpenClaw 預設值時，此 SecretRef ID：
...
請為 OpenClaw 使用限定範圍的用戶端權杖，而非根權杖。對於預設 KV v2 配置，模型供應商金鑰所需的最小原則如下：
...
對於未容器化的本機閘道，請在啟動 OpenClaw 的同一個 shell 中匯出 Vault 設定。預設驗證方式會從 `VAULT_TOKEN` 讀取 Vault 用戶端權杖：
...
如果 Vault Agent 將權杖寫入接收檔案，請使用權杖檔案驗證：
...
若 Vault 伺服器由私有 CA 簽署，請將該 CA 安裝到主機信任存放區，並啟用 Node 系統信任：
...
若要使用非互動式 JWT 驗證，請使用工作負載 JWT 檔案及類型為 `jwt` 的 Vault 角色：
...
JWT 檔案應為投射的工作負載權杖，例如受 Vault 角色接受其目標對象的 Kubernetes 服務帳戶權杖。
互動式 OIDC 瀏覽器登入適合人員使用，但閘道執行階段需要非互動式 JWT 登入或權杖檔案。

若要使用 Vault 的 Kubernetes 驗證方式，請使用 `kubernetes`。此方式適用於以 Pod 執行的閘道；預設掛載點為 `kubernetes`，預設 JWT 檔案則是標準服務帳戶權杖路徑：
...
只有當 Vault 將 Kubernetes 驗證掛載於 `auth/kubernetes` 以外的位置時，才設定 `OPENCLAW_VAULT_AUTH_MOUNT`。只有當服務帳戶權杖投射至自訂路徑時，才設定 `OPENCLAW_VAULT_JWT_FILE`。
...
<Warning>
如果閘道以服務、LaunchAgent、systemd 單元、排程工作或容器的形式執行，該執行階段環境必須取得相同的 Vault 變數。在互動式 shell 中設定變數只能證明該 shell 的設定，無法證明已在執行的閘道也有這些設定。
</Warning>

## 產生並套用 SecretRef 計畫

建立一份將 OpenRouter 模型供應商 API 金鑰對應至 Vault 的計畫：
...
沒有捷徑的內建供應商，或已設定的 OpenAI 相容與自訂模型供應商，請使用 `--provider-key`：
...
每個 `--provider-key <provider=id>` 都會將 SecretRef 寫入 `models.providers.<provider>.apiKey`。對於自訂供應商，此命令不會建立該供應商的 `baseUrl`、`api` 或 `models` 設定；請先設定這些項目。
...
不含前綴的目標路徑會套用至 `openclaw.json`。若要指定現有的 `auth-profiles.json` 目標，請使用 `auth-profiles:<agentId>:<path>`。
目標路徑必須是已註冊的 OpenClaw SecretRef 目標。設定命令不會在 OpenClaw 中建立任意具名密鑰；Vault 仍是密鑰存放區，OpenClaw 只會在受支援的設定欄位中儲存 SecretRef。

## SecretRef ID 格式

Vault SecretRef ID 採用以下慣例：
...
| SecretRef ID                  | 預設 KV v2 Vault 讀取路徑           | 傳回欄位 |
| ----------------------------- | ---------------------------------- | -------------- |
...
之後，`providers/openrouter/apiKey` 會讀取：
...
- 一個驗證來源：
  ...
  - `OPENCLAW_VAULT_AUTH_METHOD=jwt` 加上 `OPENCLAW_VAULT_AUTH_MOUNT`、
    `OPENCLAW_VAULT_AUTH_ROLE` 及 `OPENCLAW_VAULT_JWT_FILE`
...
- 選用的 `VAULT_NAMESPACE`、`OPENCLAW_VAULT_KV_MOUNT` 及
  `OPENCLAW_VAULT_KV_VERSION`

使用 Kubernetes 時，如果 Vault 已為叢集設定 Kubernetes 驗證，建議使用 `OPENCLAW_VAULT_AUTH_METHOD=kubernetes`。只有在 Vault 設定為將叢集視為一般 JWT/OIDC 簽發者時，才使用 `OPENCLAW_VAULT_AUTH_METHOD=jwt`。這兩個選項都比將長效 Vault 權杖存放在 Kubernetes Secret 中更好。Vault Agent Sidecar 或注入器部署則可改用 `token_file`。

對於多租戶 Vault 設定，請將租戶路由保留在 Vault 原則與部署設定中。OpenClaw 不要求固定的掛載點、角色或路徑：每個閘道環境都可以設定自己的 `OPENCLAW_VAULT_KV_MOUNT`、`OPENCLAW_VAULT_AUTH_ROLE` 與 SecretRef ID。若單一共用閘道必須同時解析不同 Vault 使用者的密鑰，請使用手動設定的 exec 供應商來包裝不同的驗證環境，或將租戶分散至具有個別 Vault 環境變數的閘道環境。
