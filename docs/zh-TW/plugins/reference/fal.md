---
read_when:
    - 您正在安裝、設定或稽核 fal 外掛
summary: 為 OpenClaw 新增 fal 模型供應商支援。
title: fal 外掛
x-i18n:
    generated_at: "2026-07-11T21:35:59Z"
    model: gpt-5.6
    postprocess_version: locale-links-v1
    provider: openai
    source_hash: f62d6a642a6edaa581fb23fa27dade77486c827d460f19aa23750959f5aa6f1c
    source_path: plugins/reference/fal.md
    workflow: 16
---

# fal 外掛

為 OpenClaw 新增 fal 模型供應商支援。

## 發佈

- 套件：`@openclaw/fal-provider`
- 安裝方式：隨附於 OpenClaw

## 介面

供應商：fal；合約：imageGenerationProviders、musicGenerationProviders、videoGenerationProviders

## 相關文件

- [fal](/zh-TW/providers/fal)
