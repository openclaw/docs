...
LLM/模型提供者（不是 WhatsApp/Telegram 這類聊天通道）的參考資料。模型選擇規則請參閱 [模型](/zh-TW/concepts/models)。
...
<AccordionGroup>
  <Accordion title="模型參照與 CLI 輔助工具">
    ...
    - `models.providers.*.contextWindow` / `contextTokens` / `maxTokens` 設定提供者層級的預設值；`models.providers.*.models[].contextWindow` / `contextTokens` / `maxTokens` 會逐一模型覆寫它們。
    - 備援規則、冷卻探測，以及工作階段覆寫持久化：[模型容錯移轉](/zh-TW/concepts/model-failover)。

  </Accordion>
  <Accordion title="新增提供者驗證不會變更你的主要模型">
    當你新增或重新驗證提供者時，`openclaw configure` 會保留既有的 `agents.defaults.model.primary`。提供者 Plugin 仍可能在其驗證設定修補中傳回建議的預設模型，但當主要模型已存在時，configure 會將其視為「讓這個模型可用」，而不是「取代目前的主要模型」。
  ...
  <Accordion title="OpenAI 提供者/執行階段分離">
    ...
    設定 `agentRuntime.id: "codex"` 時，可透過原生 Codex app-server harness 使用 GPT-5.5；針對 Codex OAuth，可在 PI 中透過 `openai-codex/gpt-5.5` 使用；當你的帳戶開放時，針對直接 API 金鑰流量，可在 PI 中透過 `openai/gpt-5.5` 使用。

  </Accordion>
  <Accordion title="CLI 執行階段">
    CLI 執行階段使用相同的分離方式：選擇標準模型參照，例如 `anthropic/claude-*`、`google/gemini-*` 或 `openai/gpt-*`，然後在你需要本機 CLI 後端時，將 `agents.defaults.agentRuntime.id` 設為 `claude-cli`、`google-gemini-cli` 或 `codex-cli`。

    舊版 `claude-cli/*`、`google-gemini-cli/*` 和 `codex-cli/*` 參照會遷移回標準提供者參照，並將執行階段分開記錄。
...
多數提供者專屬邏輯位於提供者 Plugin（`registerProvider(...)`）中，而 OpenClaw 保留通用推論迴圈。Plugin 負責 onboarding、模型目錄、驗證環境變數對應、傳輸/設定正規化、工具結構描述清理、容錯移轉分類、OAuth 重新整理、用量回報、thinking/reasoning 設定檔等。

提供者 SDK hook 和內建 Plugin 範例的完整清單位於 [提供者 Plugin](/zh-TW/plugins/sdk-provider-plugins)。需要完全自訂請求執行器的提供者屬於另一個更深層的擴充介面。

<Note>
提供者擁有的 runner 行為存在於明確的提供者 hook 上，例如重播原則、工具結構描述正規化、串流包裝，以及傳輸/請求輔助工具。舊版 `ProviderPlugin.capabilities` 靜態包僅用於相容性，且共用 runner 邏輯不再讀取它。
</Note>
...
- 透過 `agents.defaults.models["openai/<model>"].params.transport` 逐一模型覆寫（`"sse"`、`"websocket"` 或 `"auto"`）
...
- `/fast` 和 `params.fastMode` 會將直接的 `openai/*` Responses 請求在 `api.openai.com` 上對應到 `service_tier=priority`
- 當你想要明確層級而非共用的 `/fast` 切換時，請使用 `params.serviceTier`
- 隱藏的 OpenClaw 歸因標頭（`originator`、`version`、`User-Agent`）只套用於傳送到 `api.openai.com` 的原生 OpenAI 流量，不套用於通用 OpenAI 相容代理
- 原生 OpenAI 路由也會保留 Responses `store`、提示快取提示，以及 OpenAI reasoning 相容酬載塑形；代理路由則不會
- `openai/gpt-5.3-codex-spark` 在 OpenClaw 中被刻意抑制，因為即時 OpenAI API 請求會拒絕它，且目前 Codex 目錄未公開它
...
- 直接的公開 Anthropic 請求支援共用的 `/fast` 切換與 `params.fastMode`，包括傳送到 `api.anthropic.com` 的 API 金鑰與 OAuth 驗證流量；OpenClaw 會將其對應到 Anthropic `service_tier`（`auto` 相對於 `standard_only`）
- 偏好的 Claude CLI 設定會保留標準模型參照，並分開選擇 CLI
  後端：`anthropic/claude-opus-4-7` 搭配
  `agents.defaults.agentRuntime.id: "claude-cli"`。舊版
  `claude-cli/claude-opus-4-7` 參照仍可用於相容性。

<Note>
Anthropic 員工告訴我們，OpenClaw 風格的 Claude CLI 使用已再次被允許，因此除非 Anthropic 發布新政策，OpenClaw 會將此整合中的 Claude CLI 重用和 `claude -p` 使用視為已核准。Anthropic setup-token 仍可作為受支援的 OpenClaw token 路徑，但 OpenClaw 現在偏好在可用時重用 Claude CLI 和 `claude -p`。
</Note>
...
- 預設傳輸為 `auto`（WebSocket 優先，SSE 備援）
- 透過 `agents.defaults.models["openai-codex/<model>"].params.transport` 逐一 PI 模型覆寫（`"sse"`、`"websocket"` 或 `"auto"`）
- `params.serviceTier` 也會在原生 Codex Responses 請求（`chatgpt.com/backend-api`）上轉送
- 隱藏的 OpenClaw 歸因標頭（`originator`、`version`、`User-Agent`）只會附加在傳送到 `chatgpt.com/backend-api` 的原生 Codex 流量上，不會附加在通用 OpenAI 相容代理上
- 與直接 `openai/*` 共用相同的 `/fast` 切換和 `params.fastMode` 設定；OpenClaw 會將其對應到 `service_tier=priority`
- `openai-codex/gpt-5.5` 使用 Codex 目錄原生 `contextWindow = 400000` 和預設執行階段 `contextTokens = 272000`；可使用 `models.providers.openai-codex.models[].contextTokens` 覆寫執行階段上限
...
- 只有當你要透過 PI 使用 Codex OAuth/訂閱路由時，才使用 `openai-codex/gpt-5.5`；當你的 API 金鑰設定和本機目錄公開公共 API 路由時，請使用沒有 Codex 執行階段覆寫的 `openai/gpt-5.5`。
...
- 思考：`/think adaptive` 使用 Google dynamic thinking。Gemini 3/3.1 省略固定的 `thinkingLevel`；Gemini 2.5 會傳送 `thinkingBudget: -1`。
...
- CLI：`openclaw onboard --auth-choice zai-api-key`
  ...
  - `zai-api-key` 會自動偵測相符的 Z.AI endpoint；`zai-coding-global`、`zai-coding-cn`、`zai-global` 和 `zai-cn` 會強制使用特定 surface
...
- 靜態備援目錄會提供 `kilocode/kilo/auto`；即時 `https://api.kilo.ai/api/gateway/models` 探索可進一步擴充執行階段目錄。
- `kilocode/kilo/auto` 背後的確切上游路由由 Kilo Gateway 擁有，並非在 OpenClaw 中硬編碼。

如需設定詳細資訊，請參閱 [/providers/kilocode](/zh-TW/providers/kilocode)。
...
<AccordionGroup>
  <Accordion title="OpenRouter">
    只有在已驗證的 `openrouter.ai` 路由上，才會套用其應用程式歸因標頭與 Anthropic `cache_control` 標記。DeepSeek、Moonshot 和 ZAI 參照可用於 OpenRouter 管理的提示快取 TTL，但不會接收 Anthropic 快取標記。作為代理風格的 OpenAI 相容路徑，它會略過僅限原生 OpenAI 的塑形（`serviceTier`、Responses `store`、提示快取提示、OpenAI 推理相容性）。Gemini 後端參照只保留代理 Gemini 思考簽章清理。
  </Accordion>
  <Accordion title="Kilo Gateway">
    Gemini 後端參照遵循相同的代理 Gemini 清理路徑；`kilocode/kilo/auto` 和其他不支援代理推理的參照會略過代理推理注入。
  </Accordion>
  <Accordion title="MiniMax">
    API 金鑰上線流程會寫入明確的純文字 M2.7 聊天模型定義；影像理解仍保留在 Plugin 擁有的 `MiniMax-VL-01` 媒體供應商。
  </Accordion>
  <Accordion title="NVIDIA">
    模型 ID 使用 `nvidia/<vendor>/<model>` 命名空間（例如 `nvidia/nvidia/nemotron-...` 與 `nvidia/moonshotai/kimi-k2.5` 並列）；選擇器會保留字面上的 `<provider>/<model-id>` 組合，而送往 API 的標準鍵仍維持單一前綴。
  </Accordion>
  <Accordion title="xAI">
...
使用 `models.providers`（或 `models.json`）來新增**自訂**供應商或 OpenAI/Anthropic 相容代理。

下方許多內建供應商 Plugin 已經發布預設目錄。只有在你想覆寫預設基底 URL、標頭或模型清單時，才使用明確的 `models.providers.<id>` 項目。

Gateway 模型能力檢查也會讀取明確的 `models.providers.<id>.models[]` 中繼資料。如果自訂或代理模型接受影像，請在該模型上設定 `input: ["text", "image"]`，讓 WebChat 和節點來源附件路徑能將影像作為原生模型輸入傳遞，而不是純文字媒體參照。

### Moonshot AI（Kimi）

Moonshot 以內建供應商 Plugin 出貨。預設使用內建供應商，只有在需要覆寫基底 URL 或模型中繼資料時，才新增明確的 `models.providers.moonshot` 項目：
...
- 提供者：`volcengine`（程式撰寫：`volcengine-plan`）
...
入門設定預設使用程式撰寫介面，但一般的 `volcengine/*` 目錄也會同時註冊。

在入門設定/設定模型選擇器中，Volcengine 驗證選項會優先顯示 `volcengine/*` 和 `volcengine-plan/*` 列。如果這些模型尚未載入，OpenClaw 會改為回退到未篩選的目錄，而不是顯示空的提供者範圍選擇器。
...
<Tabs>
  ...
  <Tab title="程式撰寫模型 (volcengine-plan)">
...
- 提供者：`byteplus`（程式撰寫：`byteplus-plan`）
- 驗證：`BYTEPLUS_API_KEY`
...
入門設定預設使用程式撰寫介面，但一般的 `byteplus/*` 目錄也會同時註冊。

在入門設定/設定模型選擇器中，BytePlus 驗證選項會優先顯示 `byteplus/*` 和 `byteplus-plan/*` 列。如果這些模型尚未載入，OpenClaw 會改為回退到未篩選的目錄，而不是顯示空的提供者範圍選擇器。
...
<Tabs>
  ...
  <Tab title="程式撰寫模型 (byteplus-plan)">
...
<Note>
在 MiniMax 的 Anthropic 相容串流路徑上，除非你明確設定，OpenClaw 預設會停用 thinking，而 `/fast on` 會將 `MiniMax-M2.7` 改寫為 `MiniMax-M2.7-highspeed`。
</Note>
...
- 圖像理解是在兩條 MiniMax 驗證路徑上皆由 Plugin 擁有的 `MiniMax-VL-01`
...
OpenClaw 使用 LM Studio 的原生 `/api/v1/models` 和 `/api/v1/models/load` 進行探索與自動載入，並預設使用 `/v1/chat/completions` 進行推論。如果你想讓 LM Studio JIT 載入、TTL 和自動逐出掌控模型生命週期，請設定 `models.providers.lmstudio.params.preload: false`。請參閱 [/providers/lmstudio](/zh-TW/providers/lmstudio) 以取得設定與疑難排解。
...
當你使用 `OLLAMA_API_KEY` 選擇加入時，Ollama 會在本機 `http://127.0.0.1:11434` 被偵測到，而內建提供者 Plugin 會將 Ollama 直接加入 `openclaw onboard` 和模型選擇器。請參閱 [/providers/ollama](/zh-TW/providers/ollama) 以取得入門設定、雲端/本機模式和自訂設定。

### vLLM

vLLM 以內建提供者 Plugin 形式提供，適用於本機/自行託管的 OpenAI 相容伺服器：

- 提供者：`vllm`
- 驗證：選用（取決於你的伺服器）
- 預設基底 URL：`http://127.0.0.1:8000/v1`

若要在本機選擇加入自動探索（如果你的伺服器不強制驗證，任何值皆可）：
...
SGLang 以內建提供者 Plugin 形式提供，適用於快速的自行託管 OpenAI 相容伺服器：
...
<AccordionGroup>
  <Accordion title="預設選用欄位">
    對於自訂提供者，`reasoning`、`input`、`cost`、`contextWindow` 和 `maxTokens` 都是選用欄位。省略時，OpenClaw 預設為：
  ...
  <Accordion title="代理路由塑形規則">
    - 對於非原生端點上的 `api: "openai-completions"`（任何非空 `baseUrl` 且其主機不是 `api.openai.com`），OpenClaw 會強制 `compat.supportsDeveloperRole: false`，以避免提供者因不支援的 `developer` 角色而產生 400 錯誤。
    - 代理樣式的 OpenAI 相容路由也會略過僅限原生 OpenAI 的請求塑形：沒有 `service_tier`、沒有 Responses `store`、沒有 Completions `store`、沒有提示快取提示、沒有 OpenAI reasoning 相容負載塑形，也沒有隱藏的 OpenClaw attribution 標頭。
    - 對於需要廠商特定欄位的 OpenAI 相容 Completions 代理，請設定 `agents.defaults.models["provider/model"].params.extra_body`（或 `extraBody`），將額外 JSON 合併到輸出的請求本文中。
    - 對於 vLLM 聊天範本控制，請設定 `agents.defaults.models["provider/model"].params.chat_template_kwargs`。當工作階段 thinking 等級關閉時，內建 vLLM Plugin 會自動為 `vllm/nemotron-3-*` 傳送 `enable_thinking: false` 和 `force_nonempty_content: true`。
    - 對於較慢的本機模型或遠端 LAN/tailnet 主機，請設定 `models.providers.<id>.timeoutSeconds`。這會延長提供者模型 HTTP 請求處理，包括連線、標頭、本文串流，以及整體受保護擷取中止，而不會增加整個代理執行階段逾時。
    - 模型提供者 HTTP 呼叫僅針對已設定提供者 `baseUrl` 主機名稱，允許 Surge、Clash 和 sing-box 在 `198.18.0.0/15` 與 `fc00::/7` 中的 fake-IP DNS 回應。其他 private、loopback、link-local 和 metadata 目的地仍需要明確選擇加入 `models.providers.<id>.request.allowPrivateNetwork: true`。
...
- [模型容錯移轉](/zh-TW/concepts/model-failover) — 回退鏈與重試行為
