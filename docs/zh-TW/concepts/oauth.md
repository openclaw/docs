...
title: OAuth
x-i18n:
    generated_at: "2026-07-16T11:37:08Z"
    model: gpt-5.6
...
尤其是 **OpenAI Codex（ChatGPT OAuth）** 與 **Anthropic Claude 命令列介面重複使用**。
對 Anthropic 而言，實際上的區分如下：
...
- **OpenClaw 內的 Anthropic Claude 命令列介面／訂閱驗證**：Anthropic 員工
  ...
  `claude -p` 用法視為此整合的核准方式。對正式環境中的 Anthropic 而言，API 金鑰驗證仍是
  較安全的建議方式。
...
部分供應商在為同一使用者／應用程式核發新權杖時，會使先前的重新整理權杖
失效。實際症狀是：同時透過 OpenClaw _以及_
Claude Code／Codex 命令列介面登入，之後其中一方會隨機遭到登出。

為減少此情況，OpenClaw 將驗證設定檔存放區視為**權杖匯集處**：

- 執行階段會從每個代理程式的單一位置讀取認證資訊
- 多個設定檔可以共存並以確定性方式路由
- 外部命令列介面重複使用取決於供應商：一旦 OpenClaw 擁有某供應商的本機 OAuth
  設定檔，本機重新整理權杖即為標準來源。如果該本機
  重新整理權杖遭拒，OpenClaw 會回報需要
  重新驗證的設定檔，而不會改用外部命令列介面的權杖資料。
  Codex 命令列介面的啟動程序範圍更窄：在 OpenClaw 擁有該
  供應商的 OAuth 之前，它只能植入空白的
  `openai:default` 樣式設定檔；此後，OpenClaw 擁有的重新整理結果會維持為標準來源
- 狀態／啟動路徑會將外部命令列介面探索限制在
...
僅供舊版匯入的檔案（仍受支援，但不是主要存放區）：

- `~/.openclaw/credentials/oauth.json`（首次使用時匯入驗證設定檔存放區）

以上項目也都遵循 `$OPENCLAW_STATE_DIR`（狀態目錄覆寫）。完整參考資料：[/gateway/configuration-reference#auth-storage](/zh-TW/gateway/configuration-reference#auth-storage)

如需靜態密鑰參照與執行階段快照啟用行為，請參閱[密鑰管理](/zh-TW/gateway/secrets)。

當次要代理程式沒有本機驗證設定檔時，OpenClaw 會透過讀取穿透
繼承預設／主要代理程式存放區；它不會在讀取時複製主要
代理程式的存放區。OAuth 重新整理權杖尤其敏感：一般
複製流程預設會略過它們，因為部分供應商會在使用後輪替重新整理權杖或使其
失效。當代理程式需要獨立帳號時，請為其設定個別的 OAuth 登入。

## Anthropic Claude 命令列介面重複使用

OpenClaw 支援 Anthropic Claude 命令列介面重複使用與 `claude -p`，將其視為核准的
驗證路徑。如果主機上已有本機 Claude 登入，
導引／設定可以直接重複使用。Anthropic setup-token 仍可
作為受支援的權杖驗證路徑，但 OpenClaw 會在可用時優先採用 Claude 命令列介面
重複使用。

<Warning>
Anthropic 的公開 Claude Code 文件指出，直接使用 Claude Code 仍受
Claude 訂閱限制約束，而 Anthropic 員工告知我們，OpenClaw 類型的 Claude
命令列介面用法已再次獲准。因此，除非 Anthropic
發布新政策，否則 OpenClaw 會將 Claude 命令列介面重複使用與
`claude -p` 用法視為此整合的核准方式。
...
1. 在任何裝有 Claude Code 的機器上執行 `claude setup-token` 以建立權杖，接著從 OpenClaw 啟動 Anthropic setup-token 或 paste-token
2. OpenClaw 將產生的 Anthropic 認證資訊儲存在驗證設定檔中
3. 模型選擇仍維持在 `anthropic/...`
...
若要在一個代理程式中使用多個 ChatGPT/Codex OAuth 帳號，請使用
`--profile-id openai:<name>`。新設定檔請勿使用 `openai-codex:<name>`。Doctor 會將
該較舊前綴移轉為不會衝突的 `openai:*` 設定檔 ID；修復後請先執行
`openclaw models auth list --provider openai`，再將設定檔 ID 複製到
`auth.order` 或 `/model ...@<profileId>`。

流程形式（PKCE）：

1. 產生 PKCE 驗證器／挑戰與隨機的 `state`
2. 開啟 `https://auth.openai.com/oauth/authorize?...`（範圍
   `openid profile email offline_access`）
3. 嘗試在 `http://localhost:1455/auth/callback` 擷取回呼（
   回呼主機預設為 `localhost`，且只接受迴路主機；
   可使用 `OPENCLAW_OAUTH_CALLBACK_HOST` 覆寫）
4. 如果你能在回呼抵達前貼上代碼（或你處於
   遠端／無頭環境且回呼無法繫結），則改為貼上重新導向 URL／代碼
   ——手動貼上會與瀏覽器回呼競速，先完成的一方生效
...
- 如果已到期，則重新整理（受檔案鎖定保護）並覆寫已儲存的認證資訊
- 如果次要代理程式讀取繼承自主要代理程式的 OAuth 設定檔，
  重新整理結果會寫回主要代理程式存放區，而不會將重新整理
  權杖複製到次要代理程式存放區
- 由外部管理的命令列介面認證資訊（Claude 命令列介面、範圍有限的 Codex 命令列介面啟動程序；
  請參閱[權杖匯集處](#the-token-sink-why-it-exists)）會重新讀取，而不會
  使用複製的重新整理權杖。如果受管理的重新整理失敗，OpenClaw
  會回報受影響且需要重新驗證的設定檔，而不會傳回
  外部命令列介面的權杖資料。

重新整理流程會自動執行；通常不需要手動管理權杖。
...
如果你希望「個人」與「工作」永不互相影響，請使用隔離的代理程式（分開的工作階段＋認證資訊＋工作區）：
...
接著按代理程式設定驗證（精靈），並將聊天路由至正確的代理程式。

### 2) 進階：單一代理程式中的多個設定檔

驗證設定檔存放區支援同一供應商的多個設定檔 ID。
選擇要使用的設定檔：

- 透過設定順序在全域選擇（`auth.order`）
- 透過 `/model ...@<profileId>` 按工作階段選擇
...
- [密鑰](/zh-TW/gateway/secrets) — 認證資訊儲存與 SecretRef
