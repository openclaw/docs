...
TypeBox 是 TypeScript 優先的結構描述函式庫。我們使用它來定義 **Gateway
WebSocket 協定**（交握、請求/回應、伺服器事件）。這些結構描述會驅動
**執行階段驗證**、**JSON Schema 匯出**，以及 macOS 應用程式的 **Swift 程式碼產生**。單一真相來源；其他所有內容皆由此產生。
...
第一個框架**必須**是 `connect` 請求。之後，客戶端可以呼叫
...
- **伺服器端**：每個傳入框架都會使用 AJV 驗證。交握只接受
  params 符合 `ConnectParams` 的 `connect` 請求。
- **客戶端**：JS 客戶端會在使用事件與回應框架前先驗證它們。
- **功能探索**：Gateway 會在 `hello-ok` 中從 `listGatewayMethods()` 和
  `GATEWAY_EVENTS` 傳送保守的 `features.methods` 與 `features.events`
  清單。
- 該探索清單不是 `coreGatewayHandlers` 中每個可呼叫 helper 的產生式傾印；
  有些 helper RPC 實作於 `src/gateway/server-methods/*.ts`，但未列舉在已宣告的
  功能清單中。
...
然後將 `"system.echo"` 加入 `src/gateway/server-methods-list.ts` 中的
`listGatewayMethods` 輸入。

如果該方法可由操作者或節點客戶端呼叫，也要在
`src/gateway/method-scopes.ts` 中分類，使範圍強制執行與 `hello-ok` 功能宣告
保持一致。
...
- `agent` 接受選用的 `internalEvents`，用於執行階段產生的協調脈絡
  （例如子代理/Cron 任務完成交接）；請將此視為內部 API 介面。
