...
summary: 用於外部 CLI（signal-cli、imsg）與 Gateway 模式的 RPC 配接器
title: RPC 配接器
x-i18n:
...
- OpenClaw 會產生 `imsg rpc` 作為子行程（舊版 iMessage 整合）。
- JSON-RPC 透過 stdin/stdout 以行分隔傳輸（每行一個 JSON 物件）。
...
請參閱 [iMessage](/zh-TW/channels/imessage) 了解舊版設定與定址（建議使用 `chat_id`）。

## 轉接器指南

- Gateway 負責行程（啟動/停止與提供者生命週期綁定）。
- 讓 RPC 用戶端保持韌性：逾時、退出時重新啟動。
