---
read_when:
    - 你想了解專案背景故事或貢獻者致謝
summary: 專案起源、貢獻者與授權。
title: 致謝
x-i18n:
    generated_at: "2026-05-06T09:18:26Z"
    model: gpt-5.5
    provider: openai
    source_hash: 4cda0d96a66d4a6fcf81553d39315ebe01e7ebb4a21bb2994e9a503825bc7fb6
    source_path: reference/credits.md
    workflow: 16
---

## 名稱

OpenClaw = CLAW + TARDIS，因為每隻太空龍蝦都需要一台時空機器。

## 致謝

- **Peter Steinberger** ([@steipete](https://x.com/steipete)) - 創作者、龍蝦語者
- **Mario Zechner** ([@badlogicc](https://x.com/badlogicgames)) - Pi 創作者、安全性滲透測試人員
- **Clawd** - 要求更好名稱的太空龍蝦

## 核心貢獻者

- **Maxim Vovshin** (@Hyaxia, [36747317+Hyaxia@users.noreply.github.com](mailto:36747317+Hyaxia@users.noreply.github.com)) - Blogwatcher 技能
- **Nacho Iacovino** (@nachoiacovino, [nacho.iacovino@gmail.com](mailto:nacho.iacovino@gmail.com)) - 位置解析（Telegram 和 WhatsApp）
- **Vincent Koc** ([@vincentkoc](https://github.com/vincentkoc), [@vincent_koc](https://x.com/vincent_koc)) - Agents、Telemetry、Hooks、安全性

## 授權

MIT - 像海洋中的龍蝦一樣自由。

> 「我們都只是在玩自己的提示。」（某個 AI，大概是 token 嗑太多）

## 相關

- [Token 使用量與成本](/zh-TW/reference/token-use)
- [發布政策](/zh-TW/reference/RELEASING)
