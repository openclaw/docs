...
- `--registry <url>`：API 基底 URL（預設：探索取得，否則為 `https://clawhub.ai`）
- `--no-input`：停用提示
...
設定其中任何變數時，CLI 會透過指定的代理路由對外請求。`HTTPS_PROXY` 用於 HTTPS 請求，`HTTP_PROXY` 用於一般 HTTP。`NO_PROXY` / `no_proxy` 會用於略過特定主機或網域的代理。

這對於直接對外連線遭封鎖的系統是必要的（例如 Docker 容器、僅能透過代理上網的 Hetzner VPS、企業防火牆）。
...
- 舊版後備：如果 `clawhub/config.json` 尚不存在但 `clawdhub/config.json` 存在，CLI 會重用舊版路徑
...
- 呼叫 `/api/v1/search?q=...`。
- 搜尋會優先採用精確的 slug/名稱 token 相符，然後才是下載人氣。像 `map` 這樣的獨立 slug token，會比 `amap` 內的子字串更強烈地匹配 `personal-map`。
- 下載量只是小幅的人氣先驗，不保證排名在最前面。
- 如果某個技能應該出現但沒有，請在登入狀態下執行 `clawhub inspect <slug>`，先檢查擁有者可見的審核診斷，再重新命名中繼資料。
...
- 針對使用 `clawhub pin` 凍結的 Skills，在旁邊顯示 `pinned`，包含選填原因。

### `pin <slug>`

- 在 lockfile 中將已安裝的 Skills 標記為 pinned。
- `--reason <text>` 會記錄 Skills 為何被凍結。
- Pinned Skills 會被 `update --all` 略過，且直接執行 `update <slug>` 會被拒絕。
- Pinned Skills 也會拒絕 `install --force`，因此本機位元組不會被意外取代。

### `unpin <slug>`

- 從已安裝的 Skills 移除 lockfile pin，讓未來更新可以修改它。
...
- `update <slug>` 會對 pinned slug 快速失敗，並告訴你先執行 `clawhub unpin <slug>`。
- `update --all` 會略過 pinned slug，並印出保留凍結項目的摘要。

### `skill publish <path>`

- 透過 `POST /api/v1/skills`（multipart）發布。
- 需要 semver：`--version 1.2.3`。
- 當執行者具有 publisher 存取權時，`--owner <handle>` 會以組織/使用者 publisher handle 發布。
- `--migrate-owner` 會在發布新版本時，將既有 Skills 移至 `--owner`。需要兩個 publisher 的 admin/owner 存取權。
- 擁有者與審查行為說明於 `docs/publishing.md`。
- 發布 Skills 表示它會在 ClawHub 上以 `MIT-0` 發行。
- 已發布的 Skills 可自由使用、修改與重新散布，且無需署名。
- ClawHub 不支援付費 Skills 或依 Skills 計價。
- `--clawscan-note <text>` 會加入 ClawScan 註記。此註記會提供 ClawScan 脈絡，說明可能看起來異常的行為，例如網路存取、原生主機存取，或供應商特定憑證。此註記會儲存在已發布版本上。
- 舊版別名：`publish <path>`。
...
- 軟刪除 Skills（擁有者、moderator 或 admin）。
- 呼叫 `DELETE /api/v1/skills/{slug}`。
- 由擁有者發起的軟刪除會保留 slug 30 天；此命令會印出到期時間。
- `--reason <text>` 會在 Skills 和稽核記錄上記錄 moderation 註記。
- `--note <text>` 是 `--reason` 的別名。
- `--yes` 會略過確認。

### `undelete <slug>`

- 還原隱藏的 Skills（擁有者、moderator 或 admin）。
...
- 隱藏 Skills（擁有者、moderator 或 admin）。
- `delete` 的別名。

### `unhide <slug>`

- 取消隱藏 Skills（擁有者、moderator 或 admin）。
- `undelete` 的別名。

### `skill rename <slug> <new-slug>`

- 重新命名擁有的 Skills，並將先前的 slug 保留為重新導向別名。
- 呼叫 `POST /api/v1/skills/{slug}/rename`。
- `--yes` 會略過確認。

### `skill merge <source-slug> <target-slug>`

- 將一個擁有的 Skills 合併到另一個擁有的 Skills。
...
- 擁有權轉移工作流程。
- 轉移到使用者 handle 會建立待處理請求，由收件者接受。
- 轉移到組織/publisher handle 時，只有執行者同時具有目前擁有者與目標 publisher 的 admin 存取權，才會立即套用。
- 子命令：
...
- 這適用於 Plugin 和其他套件系列項目；頂層 `search` 仍是 Skills 搜尋介面。
- 旗標：
...
- 擷取套件中繼資料而不安裝。
- 這適用於 Plugin 中繼資料、相容性、驗證、來源，以及版本/檔案檢查。
- `--version <version>`：檢查特定版本（預設：latest）。
...
- 驗證所有 artifact 的 ClawHub SHA-256。
- 對於 ClawPack npm-pack artifact，還會驗證 npm `sha512` integrity、npm shasum，以及 tarball 的 `package.json` 名稱/版本。
...
- 使用 `--package` 時，會從 ClawHub 解析預期中繼資料，並將本機檔案與已發布 artifact 中繼資料比較。
- 使用直接 digest 旗標時，無需網路查詢即可驗證。
- 旗標：
...
- 呼叫與 `package readiness` 相同的計算式就緒端點，但列印
  以遷移為重點的狀態、最新版本、官方套件狀態、檢查項目，
  以及阻擋因素。
- 旗標：
  - `--json`：機器可讀輸出。
...
- 透過 `POST /api/v1/packages` 發布程式碼 Plugin 或組合 Plugin。
- `<source>` 接受：
  - 本機資料夾路徑：`./my-plugin`
...
- 中繼資料會從 `package.json`、`openclaw.plugin.json`，以及
  真正的 OpenClaw 組合標記自動偵測，例如 `.codex-plugin/plugin.json`、
  `.claude-plugin/plugin.json` 和 `.cursor-plugin/plugin.json`。
- `.tgz` 來源會被視為 ClawPack。CLI 會上傳精確的 npm-pack
  位元組，並只使用擷取出的 `package/` 內容進行驗證與
  中繼資料預填。
- 程式碼 Plugin 資料夾會先封裝成 ClawPack npm tarball 再上傳，
  讓 OpenClaw 安裝可以驗證精確成品。組合 Plugin 資料夾仍然
  使用擷取檔案的發布路徑。
- 對於 GitHub 來源，來源歸屬會從儲存庫、解析後的 commit、ref 與子路徑自動填入。
- 對於本機資料夾，當 origin remote 指向 GitHub 時，來源歸屬會從本機 git 自動偵測。
- 外部程式碼 Plugin 必須明確宣告 `openclaw.compat.pluginApi` 與
  `openclaw.build.openclawVersion`。
  頂層 `package.json.version` 不會作為發布驗證的備援值。
- `--dry-run` 會預覽解析後的發布酬載，而不上傳。
- `--json` 會為 CI 輸出機器可讀內容。
- `--owner <handle>` 會在執行者具有發布者存取權時，以使用者或組織發布者識別名發布。
- `--clawscan-note <text>` 會新增 ClawScan 註記。此註記會提供 ClawScan
  關於行為的脈絡，這些行為否則可能看起來不尋常，例如網路存取、
  原生主機存取，或提供者特定認證。此註記會儲存在
  已發布版本上。
...
先使用 `--dry-run`，讓你可以在建立即時發布版本前確認解析後的套件中繼資料與
來源歸屬：
...
外部程式碼 Plugin 需要在
`package.json` 中加入少量 OpenClaw 中繼資料。這個最小 manifest 足以成功發布：
...
- `openclaw.compat.minGatewayVersion` 和
  `openclaw.build.pluginSdkVersion` 是選用的額外項目，適合在你想發布
  更詳細的相容性中繼資料時使用。
- 如果你使用的是較舊的 `clawhub` CLI 發布版本，請在發布前升級，讓
  本機預檢能在上傳前執行。

#### GitHub Actions

ClawHub 也為 Plugin 儲存庫提供官方可重用 workflow，位於
...
- 可重用 workflow 預設會將 `source` 設為呼叫端儲存庫。
...
- 真正發布應限制於受信任事件，例如 `workflow_dispatch` 或標籤推送。
- 不使用 secret 的受信任發布只適用於 `workflow_dispatch`；標籤推送仍需要 `clawhub_token`。
- 保留 `clawhub_token` 可用於首次發布、不受信任套件，或緊急發布。
