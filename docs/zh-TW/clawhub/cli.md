...
x-i18n:
    generated_at: "2026-07-02T00:42:23Z"
    model: gpt-5.5
...
設定其中任何變數時，命令列介面會透過指定的 Proxy 路由對外請求。`HTTPS_PROXY` 用於 HTTPS 請求，`HTTP_PROXY` 用於一般 HTTP。`NO_PROXY` / `no_proxy` 會被遵循，以便針對特定主機或網域略過 Proxy。

這在直接對外連線被封鎖的系統上是必要的（例如 Docker 容器、僅能透過 Proxy 上網的 Hetzner VPS、企業防火牆）。
...
- 遠端/無頭互動模式：`clawhub login --device` 會列印代碼，並在你於 `<site>/cli/device` 授權期間等待。
...
- 搜尋會先偏好完全相符的 slug/名稱 token，再考量下載熱門度。像 `map` 這樣的獨立 slug token，會比 `amap` 內的子字串更強地匹配 `personal-map`。
- 熱門度只是小幅排名先驗，不保證位於最前。
...
- 在使用 `clawhub pin` 凍結的技能旁顯示 `pinned`，包含可選的原因。

### `pin <skill>`

- 在 lockfile 中將已安裝技能標記為已釘選。
- `--reason <text>` 記錄技能凍結的原因。
- 已釘選的技能會被 `update --all` 略過，且會拒絕直接 `update <skill>`。
...
工作流程會對一個 `skill_path` 呼叫 `skill publish`，或對 `root` 底下每個直接技能資料夾呼叫（預設：`skills`）。它會略過未變更技能，並使用相同的自動修補版本行為。
...
- 新技能發布為 `1.0.0`；已變更技能預設發布下一個修補版本。對於應提高較大 semver 階段的更新批次，請使用 `--bump minor|major`。
- `--dry-run` 會顯示發布計畫但不上傳；`--json` 會列印機器可讀計畫。
- `--all` 會發布每個新的或已變更的技能而不提示。沒有 `--all` 時，互動式終端機可讓你選擇要發布的技能。
- `--owner <handle>` 會在執行者具有發布者存取權時，以組織/使用者發布者 handle 發布。
...
- 需要 `clawhub login`。
- 透過 `POST /api/v1/skills/-/scan` 執行 ClawHub ClawScan，然後輪詢直到掃描達到終止狀態。
- 掃描是非同步的，可能需要一些時間才能完成。排隊時，終端機旋轉指示器會顯示目前的優先掃描位置，以及前方還有多少掃描。
- 已發布掃描需要擁有權或發布者管理存取權。審核員/管理員可以透過 `clawhub-admin` 使用相同後端。
- `--update` 只能與 `--slug` 搭配使用；它會將成功的已發布掃描結果寫回所選版本。
...
- `owner` 會對應到命令列介面的 `--owner` 旗標；省略它則以已驗證使用者發布。
- V1 技能發布使用 `clawhub_token`；GitHub OIDC 受信任發布目前僅適用於套件。

### `delete <skill>`

- 不加 `--version` 時，軟刪除技能（擁有者、版主或管理員）。
- 呼叫 `DELETE /api/v1/skills/{slug}`。
- 由擁有者發起的軟刪除會保留 slug 30 天；命令會列印到期時間。
- `--version <version>` 會透過失敗即關閉、
  版本專屬的路由，永久刪除一個已擁有且非最新版的版本。
  已刪除的版本無法還原或重新發布。刪除目前最新版之前，請先發布替代版本。平台人員不會在這個僅限版本的流程中略過擁有權。
- `--reason <text>` 會在整個技能的軟刪除和稽核記錄上記錄審核備註。
...
- 擁有權轉移工作流程。
- 轉移到使用者帳號代稱會建立待處理請求，由收件者接受。
- 轉移到組織/發布者帳號代稱時，只有在執行者同時擁有目前擁有者和目的地發布者的
  管理員存取權時才會立即套用。
- 子命令：
...
- 預設為離線/靜態驗證，不會定位或匯入本機
  OpenClaw checkout。
- 嚴重相容性錯誤會以非零代碼結束。只有警告的發現項目會列印，但
  以零代碼結束。
- 旗標：
...
- `--version <version>` 會透過失敗即關閉、
  版本專屬的路由，永久刪除一個已擁有且非最新版的發行版。
  已刪除的版本無法還原或重新發布。刪除目前最新版之前，請先發布替代版本。這個僅限版本的流程需要套件擁有者或組織發布者
  管理員；平台人員不會略過套件擁有權。
- 整個套件的軟刪除需要套件擁有者、組織發布者擁有者/管理員、平台
  版主或平台管理員。
- 旗標：
  - `--version <version>`：永久刪除一個非最新版版本。
  - `--yes`：跳過確認。
  - `--json`：機器可讀輸出。
...
- 檢舉屬於套件層級，可選擇繫結到某個版本，並會顯示給版主
  進行審查。
- 檢舉本身不會自動隱藏套件或封鎖下載。
- 旗標：
  - `--version <version>`：可選擇附加到檢舉的套件版本。
  - `--reason <text>`：必要的檢舉原因。
  - `--json`：機器可讀輸出。
...
- 顯示可能取代
  捆綁 OpenClaw 外掛的套件之操作員導向遷移狀態。
- 呼叫與 `package readiness` 相同的計算就緒度端點，但列印
...
- 建立由已驗證使用者擁有的組織發布者。
- 帳號代稱會正規化為小寫，並且可加上或不加 `@` 傳入。
- 新建立的組織發布者預設不受信任/非官方。
- 如果帳號代稱已由現有發布者、使用者或保留路由使用，則會失敗。
...
- `.tgz` 來源會被視為 ClawPack。命令列介面會上傳完整的 npm-pack
  位元組，且只使用解壓縮出的 `package/` 內容進行驗證與
  中繼資料預填。
- 程式碼外掛資料夾會先封裝成 ClawPack npm tarball 再上傳，讓
  OpenClaw 安裝可以驗證精確的成品。套件包外掛資料夾仍然
  使用解壓縮檔案的發布路徑。
...
- 外部程式碼外掛必須明確宣告 `openclaw.compat.pluginApi` 和
  ...
  頂層 `package.json.version` 不會用作發布驗證的備援。
...
先使用 `--dry-run`，這樣你可以在建立正式發布前確認解析後的套件中繼資料和
來源歸屬：
