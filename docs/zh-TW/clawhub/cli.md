...
x-i18n:
    generated_at: "2026-05-11T20:23:01Z"
    model: gpt-5.5
    provider: openai
...
- `--workdir <dir>`：工作目錄（預設：cwd；若已設定，會回退至 Clawdbot 工作區）
...
設定其中任何變數時，CLI 會透過指定的 proxy 路由對外請求。`HTTPS_PROXY` 用於 HTTPS 請求，`HTTP_PROXY` 用於純 HTTP。`NO_PROXY` / `no_proxy` 會被遵循，用來針對特定主機或網域略過 proxy。
...
- 遠端/無介面互動式：`clawhub login --device` 會列印代碼，並在你於 `<site>/cli/device` 授權時等待。
...
- 呼叫 `/api/v1/search?q=...`。
- 搜尋會優先考慮精確的 slug/name 權杖相符，然後才是下載熱門度。像 `map` 這樣的獨立 slug 權杖，會比 `amap` 裡的子字串更強烈地符合 `personal-map`。
- 下載數只是小幅的人氣先驗，不保證排在最前面。
- 如果某個 skill 應該出現但沒有出現，請在已登入狀態下執行 `clawhub inspect <slug>`，先檢查擁有者可見的審核診斷，再重新命名中繼資料。
...
- 對使用 `clawhub pin` 凍結的 Skills 顯示 `pinned`，包括選填原因。
...
- 已釘選的 Skills 也會拒絕 `install --force`，因此本機位元組不會被意外替換。

### `unpin <slug>`

- 從已安裝的 Skill 移除鎖定檔釘選，讓未來更新可以修改它。
...
- `update <slug>` 會對已釘選的 slugs 快速失敗，並告訴你先執行 `clawhub unpin <slug>`。
- `update --all` 會略過已釘選的 slugs，並列印哪些項目保持凍結的摘要。

### `skill publish <path>`

- 透過 `POST /api/v1/skills` 發佈（multipart）。
- 需要 semver：`--version 1.2.3`。
- 當執行者擁有發佈者存取權時，`--owner <handle>` 會在組織/使用者發佈者 handle 底下發佈。
- `--migrate-owner` 會在發佈新版本時，將現有 Skill 移至 `--owner`。需要對兩個發佈者都有管理員/擁有者存取權。
- 擁有者與審查行為在 `docs/publishing.md` 中說明。
- 發佈 Skill 表示它會在 ClawHub 上以 `MIT-0` 授權釋出。
- 已發佈的 Skills 可免費使用、修改與再散佈，且無需署名。
- ClawHub 不支援付費 Skills 或個別 Skill 定價。
- 舊版別名：`publish <path>`。

### `delete <slug>`

- 軟刪除 Skill（擁有者、版主或管理員）。
- 呼叫 `DELETE /api/v1/skills/{slug}`。
- 擁有者發起的軟刪除會保留該 slug 30 天；此命令會列印到期時間。
...
- 重新命名已擁有的 Skill，並保留先前的 slug 作為重新導向別名。
...
- 要求對最新發佈的 Skill 版本進行安全性重新掃描。
- 擁有者和發佈者管理員可以在每個版本的復原限制內，重新掃描自己的 Skills。
- 平台版主和管理員可以重新掃描任何 Skill，且不受擁有者復原限制阻擋，但每個版本一次只能執行一次重新掃描。
...
- 擁有權轉移工作流程。
- 轉移至使用者 handles 會建立待處理要求，由接收者接受。
- 轉移至組織/發佈者 handles 時，只有在執行者對目前擁有者與目標發佈者都有管理員存取權時才會立即套用。
- 子命令：
...
- 搭配 `--package` 時，會從 ClawHub 解析預期中繼資料，並將本機檔案與已發佈的成品中繼資料比較。
...
- 將套件移轉給另一個發布者。
- 需要目前套件擁有者與目的地發布者雙方的管理員存取權，
  除非由平台管理員執行。
- 帶範圍的套件名稱必須移轉給相符的範圍擁有者。
...
- 檢舉以套件層級為單位，可選擇綁定到某個版本，並會顯示給版主審查。
- 檢舉本身不會自動隱藏套件或阻擋下載。
- 旗標：
...
- 擁有者/發布者用來針對發布版本審核提出申訴的指令。
- 呼叫 `POST /api/v1/packages/{name}/appeal`。
- 隔離、撤銷、可疑或惡意的發布版本可提出申訴。
- 旗標：
  - `--version <version>`：必填的套件版本。
  - `--message <text>`：必填的申訴訊息。
  - `--json`：機器可讀輸出。
...
- 顯示目前套件掃描狀態、開啟的檢舉數量、最新發布版本的人工審核狀態、
  下載阻擋狀態，以及審核原因。
- 旗標：
  - `--json`：機器可讀輸出。
...
- 回報官方狀態、ClawPack 可用性、成品摘要、來源出處、
  OpenClaw 相容性、主機目標、環境中繼資料，以及掃描狀態的阻擋因素。
- 旗標：
  - `--json`：機器可讀輸出。
...
- 顯示可能取代內建 OpenClaw Plugin 的套件，其偏向操作員的移轉狀態。
- 呼叫與 `package readiness` 相同的計算式就緒狀態端點，但會列印偏重移轉的狀態、
  最新版本、官方套件狀態、檢查項目和阻擋因素。
- 旗標：
  - `--json`：機器可讀輸出。
...
- 中繼資料會從 `package.json`、`openclaw.plugin.json`，以及真正的 OpenClaw 組合包標記自動偵測，
  例如 `.codex-plugin/plugin.json`、`.claude-plugin/plugin.json` 和 `.cursor-plugin/plugin.json`。
- `.tgz` 來源會被視為 ClawPack。CLI 會上傳精確的 npm-pack
  位元組，並僅使用解出的 `package/` 內容進行驗證和中繼資料預填。
- 程式碼 Plugin 資料夾會在上傳前封裝成 ClawPack npm tarball，
  讓 OpenClaw 安裝項目可以驗證精確成品。組合包 Plugin 資料夾仍使用解出檔案的發布路徑。
...
- `--owner <handle>` 會在執行者具有發布者存取權時，發布到使用者或組織發布者代稱底下。
- 帶範圍的套件名稱必須符合選取的擁有者。請參閱 `docs/publishing.md`。
- 既有旗標（`--family`、`--name`、`--version`、`--source-repo`、`--source-commit`、`--source-ref`、`--source-path`）仍可作為覆寫使用。
- 私有 GitHub repo 需要 `GITHUB_TOKEN`。

#### 建議的本機流程

先使用 `--dry-run`，如此你可以在建立實際發布版本前確認解析後的套件中繼資料和
來源歸屬：
...
- `openclaw.hostTargets` 和 `openclaw.environment` 是選用中繼資料。
  ClawHub 可能會在存在時顯示它們，但發布時並非必填。
- `openclaw.compat.minGatewayVersion` 和
  `openclaw.build.pluginSdkVersion` 是選用的額外項目，若你想發布
  更詳細的相容性中繼資料即可使用。
- 如果你使用較舊的 `clawhub` CLI 發布版本，請先升級再發布，
  以便本機預檢查能在上傳前執行。

#### GitHub Actions

ClawHub 也為 Plugin repo 提供官方可重複使用的 workflow，位於
...
- 可重複使用的 workflow 預設會將 `source` 設為呼叫端 repo。
- 對於 monorepo，傳入 `source_path`，讓 workflow 發布 Plugin
  套件資料夾，例如 `source_path: extensions/codex`。
- 將可重複使用的 workflow 固定到穩定標籤或完整 commit SHA。不要從 `@main` 執行發布。
...
- 保留可用的 `clawhub_token`，供首次發布、不受信任套件或緊急發布使用。
