...
當設定其中任何變數時，命令列介面會透過指定的代理路由對外請求。`HTTPS_PROXY` 用於 HTTPS 請求，`HTTP_PROXY` 用於純 HTTP。`NO_PROXY` / `no_proxy` 會受到遵循，以針對特定主機或網域略過代理。
...
- 適合用來將本機登入權杖透過管線傳入 CI secret 設定命令。
...
- 搜尋會在下載人氣之前，優先採用精確 slug/名稱 token 相符。一個獨立 slug token（例如 `map`）與 `personal-map` 的相符強度會高於 `amap` 內的子字串。
- 人氣只是小幅排名先驗，不保證排在最前。
- 如果某個技能應該出現但沒有，請在登入後執行 `clawhub inspect @owner/slug`，先檢查擁有者可見的審核診斷，再重新命名中繼資料。

### `explore`

- 透過 `/api/v1/skills?limit=...&sort=createdAt` 列出最新技能（依 `createdAt` desc 排序）。
- 旗標：
  - `--limit <n>`（1-200，預設：25）
...
- `--files`：列出所選版本的檔案。
- `--file <path>`：取得原始檔案內容（僅文字檔；200KB 限制）。
- `--json`：機器可讀輸出。
...
- 在透過 `clawhub pin` 凍結的技能旁顯示 `pinned`，包含選填原因。

### `pin <skill>`

- 在 lockfile 中將已安裝技能標記為已釘選。
- `--reason <text>` 記錄技能被凍結的原因。
- 已釘選技能會被 `update --all` 略過，也會被直接 `update <skill>` 拒絕。
- 已釘選技能也會拒絕 `install --force`，以免本機位元組遭意外取代。
...
workflow 會對一個 `skill_path` 呼叫 `skill publish`，或對 `root` 下的每個直接技能資料夾呼叫（預設：`skills`）。它會略過未變更技能，並使用相同的自動 patch-version 行為。
...
- 將每個本機技能指紋與 ClawHub 比較，且只發布新的或已變更的技能。
- 新技能會發布為 `1.0.0`；已變更技能預設會發布下一個 patch 版本。針對應該移動較大 semver 階段的更新批次，請使用 `--bump minor|major`。
- `--dry-run` 會顯示發布計畫但不上傳；`--json` 會列印機器可讀計畫。
- `--all` 會發布每個新的或已變更的技能而不提示。未使用 `--all` 時，互動式終端機可讓你選擇要發布的技能。
- 當操作者有發布者存取權時，`--owner <handle>` 會以組織/使用者發布者 handle 發布。
- `sync` 僅是單向發布。它不會安裝、更新、下載，也不會回報安裝/下載遙測。
...
- 需要 `clawhub login`。
- 透過 `POST /api/v1/skills/-/scan` 執行 ClawHub ClawScan，然後輪詢直到掃描終止。
- 掃描是非同步的，可能需要一些時間完成。排隊期間，終端機 spinner 會顯示目前的優先掃描位置，以及前方還有多少掃描。
- 已發布掃描需要擁有權或發布者管理存取權。版主/管理員可以透過 `clawhub-admin` 使用相同後端。
- `--update` 只有搭配 `--slug` 時有效；它會將成功的已發布掃描結果寫回所選版本。
- `--output <file.zip>` 會下載完整報告封存，內含 `manifest.json`、`clawscan.json`、`skillspector.json`、`static-analysis.json`、`virustotal.json` 和 `README.md`。
- `--json` 會列印完整輪詢回應以供自動化使用。
- 不再支援本機路徑掃描。請上傳新版本，然後使用 `scan download` 取回該提交版本儲存的掃描結果。
...
- `owner` 對應至命令列介面的 `--owner` 旗標；省略它則以已驗證使用者身分發布。
- V1 技能發布使用 `clawhub_token`；GitHub OIDC 受信任發布目前僅適用於套件。

### `delete <skill>`

- 不加 `--version` 時，軟刪除一個技能（擁有者、版主或管理員）。
- 呼叫 `DELETE /api/v1/skills/{slug}`。
- 由擁有者發起的軟刪除會保留 slug 30 天；命令會印出到期時間。
- `--version <version>` 會透過失敗即關閉、
  版本專屬的路由，永久刪除一個擁有的非最新版。
  已刪除的版本無法還原或重新發布。刪除目前最新版前，請先發布替代版本。
  平台工作人員不會在這個僅限版本的流程中繞過所有權。
- `--reason <text>` 會在整個技能的軟刪除和稽核日誌中記錄一則審核備註。
...
- `--reason <text>` 會在技能和稽核日誌中記錄一則審核備註。
- `--note <text>` 是 `--reason` 的別名。
- `--yes` 會略過確認。
...
- 所有權轉移工作流程。
- 轉移到使用者 handle 時，會建立一個待處理請求，由收件者接受。
- 轉移到組織/發布者 handle 時，只有在執行者對目前擁有者和目的地發布者
  都有管理員存取權時，才會立即套用。
- 子命令：
...
- 不加 `--version` 時，軟刪除套件和所有發行版。
- `--version <version>` 會透過失敗即關閉、
  版本專屬的路由，永久刪除一個擁有的非最新版發行版。
  已刪除的版本無法還原或重新發布。刪除目前最新版前，請先發布替代版本。
  這個僅限版本的流程需要套件擁有者或組織發布者管理員；平台工作人員不會繞過套件所有權。
...
- 顯示可能取代
  OpenClaw 內建外掛之套件的營運人員導向遷移狀態。
- 呼叫與 `package readiness` 相同的已計算 readiness 端點，但會印出
...
- 建立由已驗證使用者擁有的組織發布者。
- handle 會正規化為小寫，且可加上或不加 `@` 傳入。
- 新建立的組織發布者預設不受信任/非官方。
- 如果 handle 已被既有發布者、使用者或保留路由使用，則會失敗。
...
- 外部程式碼外掛必須明確宣告 `openclaw.compat.pluginApi` 和
  ...
  頂層 `package.json.version` 不會作為發布驗證的 fallback。
- `--dry-run` 會預覽解析後的發布 payload，而不會上傳。
- `--json` 會輸出供 CI 使用的機器可讀內容。
- `--owner <handle>` 會在執行者具備發布者存取權時，以使用者或組織發布者 handle 發布。
...
先使用 `--dry-run`，讓你可以在建立正式發布前確認解析後的套件中繼資料與
來源歸屬：
...
- `openclaw.compat.minGatewayVersion` 和
  `openclaw.build.pluginSdkVersion` 是選用的額外欄位，適合你想發布
  更詳細的相容性中繼資料時使用。
...
ClawHub 也提供官方 reusable workflow，位置在
...
- reusable workflow 預設會將 `source` 設為呼叫端儲存庫。
- 對於 monorepo，請傳入 `source_path`，讓 workflow 發布外掛
  套件資料夾，例如 `source_path: extensions/codex`。
- 請將 reusable workflow pin 到穩定標籤或完整 commit SHA。不要從 `@main` 執行發布。
- `pull_request` 應使用 `dry_run: true`，讓 CI 保持不產生污染。
- 真正發布應限制在受信任事件，例如 `workflow_dispatch` 或 tag push。
- 不含 secret 的 trusted publishing 只適用於 `workflow_dispatch`；tag push 仍需要 `clawhub_token`。
- 請保留可用的 `clawhub_token`，用於首次發布、不受信任的套件，或緊急發布。
- workflow 會將 JSON 結果作為 artifact 上傳，並將它公開為 workflow outputs。

### `package trusted-publisher get <name>`

- 顯示套件的 GitHub Actions 受信任發布者設定。
- 設定 config 後使用此命令，以確認儲存庫、workflow 檔名，
...
- `--environment <name>` 是選用項。設定時，OIDC claim 中的 GitHub Actions
  environment 必須完全相符。
- ClawHub 會在此命令執行時驗證已設定的 GitHub 儲存庫。
  公開儲存庫可以透過公開 GitHub 中繼資料驗證。私有
  儲存庫需要 ClawHub 具備該儲存庫的 GitHub 存取權，例如透過未來的
  ClawHub GitHub App 安裝，或另一個已授權的
  GitHub 整合。
- 旗標：
  - `--repository <repo>`：GitHub 儲存庫，例如 `openclaw/example-plugin`。
  - `--workflow-filename <file>`：workflow 檔名，例如 `package-publish.yml`。
  - `--environment <name>`：選用的精確比對 GitHub Actions environment。
  - `--json`：機器可讀輸出。
...
- 從套件移除受信任發布者設定。
- 如果 workflow、儲存庫或 environment pin 需要停用或重新建立，請將此作為 rollback 使用。
- 未來的正式發布必須使用一般驗證發布，直到再次設定 config。
- 旗標：
  - `--json`：機器可讀輸出。
...
- 登入時，會在 `clawhub install <slug>` 之後送出，除非
  已設定 `CLAWHUB_DISABLE_TELEMETRY=1`。
- 回報會盡力執行。如果 telemetry 無法使用，安裝命令不會失敗。
