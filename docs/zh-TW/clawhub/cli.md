...
read_when:
    - 使用 ClawHub 命令列介面
    - 偵錯安裝、更新或發布
summary: 命令列介面參考：指令、旗標、設定與 lockfile 行為。
x-i18n:
...
命令列介面會遵循標準 HTTP proxy 環境變數，適用於位於企業代理或受限網路後方的系統：

- `HTTPS_PROXY` / `https_proxy`
- `HTTP_PROXY` / `http_proxy`
- `NO_PROXY` / `no_proxy`

設定其中任一變數時，命令列介面會透過指定的 proxy 路由外送請求。`HTTPS_PROXY` 用於 HTTPS 請求，`HTTP_PROXY` 用於一般 HTTP。`NO_PROXY` / `no_proxy` 會被遵循，以針對特定主機或網域略過 proxy。

這在直接外送連線遭封鎖的系統上是必要的（例如 Docker 容器、僅能透過 proxy 連網的 Hetzner VPS、企業防火牆）。
...
- 舊版退回：如果 `clawhub/config.json` 尚不存在，但 `clawdhub/config.json` 存在，命令列介面會重用舊版路徑
...
- 遠端/headless 互動式：`clawhub login --device` 會列印代碼，並在你於 `<site>/cli/device` 授權時等待。
...
- 適合用來把本機登入權杖透過管線傳入 CI secret 設定命令。
...
- 搜尋會優先考慮精確的 slug/名稱 token 相符，然後才是下載熱門度。像 `map` 這樣的獨立 slug token，會比 `amap` 內的子字串更強烈地匹配 `personal-map`。
- 熱門度只是小幅排名先驗，不保證排在最前。
- 如果某個技能應該出現但沒有，請在登入狀態下執行 `clawhub inspect @owner/slug`，先檢查擁有者可見的審核診斷，再重新命名中繼資料。
...
- 旗標：
  - `--limit <n>`（1-200，預設：25）
  - `--sort newest|updated|rating|downloads|trending`（預設：newest）。舊版 install 排序別名仍可為相容性使用。
  - `--json`（機器可讀輸出）
...
- 在登入狀態下送出 best-effort 遙測，以便停用目前的安裝計數。
- 互動式：要求確認。
...
- 已釘選技能會被 `update --all` 略過，且直接 `update <skill>` 會遭拒。
- 已釘選技能也會拒絕 `install --force`，因此本機位元組不會被意外取代。
...
- 如果 fingerprint 不符合：
  - 預設拒絕
  - 以 `--force` 覆寫（若為互動式，則提示）
- 已釘選技能永遠不會被 `--force` 更新。
- `update <skill>` 對已釘選技能會快速失敗，並提示你先執行 `clawhub unpin <skill>`。
- `update --all` 會略過已釘選的 slug，並列印哪些項目保持凍結的摘要。

### `skill publish <path>`

- 將本機 bundle fingerprint 與 ClawHub 比較，內容已發布時會成功結束。
- 新技能預設為 `1.0.0`；已變更技能預設為下一個 patch 版本。
- `--version <version>` 會明確選擇版本，即使內容符合既有版本也會發布。
- `--dry-run` 會解析發布但不上傳；`--json` 會列印機器可讀結果。
- `--owner <handle>` 會在執行者具有 publisher 存取權時，以組織/使用者 publisher handle 發布。
- `--migrate-owner` 會在發布新版本時，將既有技能移至 `--owner`。需要兩邊 publisher 的 admin/owner 存取權。
...
ClawHub 的可重用 [`skill-publish.yml`](https://github.com/openclaw/clawhub/blob/main/.github/workflows/skill-publish.yml) workflow 會對一個 `skill_path` 呼叫 `skill publish`，或對 `root` 下每個直接的技能資料夾呼叫（預設：`skills`）。它會略過未變更的技能，並使用相同的自動 patch 版本行為。

設定 `dry_run: true` 可在沒有權杖的情況下預覽。真正發布需要 `clawhub_token` secret。

### `sync`

- 掃描目前 workdir、已設定的 skills 目錄，以及任何 `--root <dir>` 資料夾，尋找包含 `SKILL.md` 或 `skill.md` 的本機技能資料夾。
- 將每個本機技能 fingerprint 與 ClawHub 比較，只發布新的或已變更的技能。
- 新技能發布為 `1.0.0`；已變更技能預設發布下一個 patch 版本。對於應以較大 semver 步幅前進的更新批次，使用 `--bump minor|major`。
- `--dry-run` 顯示發布計畫但不上傳；`--json` 列印機器可讀計畫。
- `--all` 會發布每個新的或已變更的技能而不提示。沒有 `--all` 時，互動式終端會讓你選擇要發布的技能。
- `--owner <handle>` 會在執行者具有 publisher 存取權時，以組織/使用者 publisher handle 發布。
- `sync` 僅為單向發布。它不會安裝、更新、下載，或回報安裝/下載遙測。
...
- 已發布掃描需要擁有權或 publisher 管理存取權。Moderator/admin 可透過 `clawhub-admin` 使用相同後端。
...
- 需要 `clawhub login`。
- 下載已提交技能或外掛版本的已儲存掃描報告 ZIP，包含被 ClawHub 安全性檢查封鎖或隱藏的版本。
- 技能下載使用技能 slug，且預設為 `--kind skill`。
- 外掛下載使用套件名稱，且需要 `--kind plugin`。
- `--version` 是必要的，讓作者檢查被 ClawHub 封鎖的確切提交版本。
...
ClawHub 在 [`/.github/workflows/skill-publish.yml`](https://github.com/openclaw/clawhub/blob/bdb23c3a9ffe77cb4184fdd13897ce535fb2d703/.github/workflows/skill-publish.yml) 提供官方可重用 workflow，供技能 repo 與 catalog repo 使用。
...
- 傳入 `skill_path: skills/review-helper` 以處理單一技能資料夾。
- `owner` 對應命令列介面的 `--owner` 旗標；省略時會以已驗證使用者身分發布。
...
- `--version <version>` 會透過失敗即封閉、版本專屬的路由，永久刪除一個已擁有的非最新版。
  已刪除的版本無法還原或重新發布。刪除目前最新版之前，請先發布替代版本。平台人員不會在此僅版本流程中繞過擁有權。
...
- 擁有權轉移工作流程。
- 轉移給使用者帳號會建立待處理請求，由接收者接受。
- 轉移給組織／發布者帳號時，只有在執行者同時擁有目前擁有者與目標發布者的管理員存取權時，才會立即套用。
- 子命令：
...
- 將此用於外掛和其他套件系列項目；頂層 `search` 仍是技能搜尋介面。
- 旗標：
...
- `--version <version>`：檢查特定版本（預設值：latest）。
...
- 搭配 `--package` 時，會從 ClawHub 解析預期中繼資料，並將本機檔案與已發布的成品中繼資料比對。
...
- 預設為離線／靜態驗證，不會尋找或匯入本機 OpenClaw checkout。
- 嚴重相容性錯誤會以非零狀態碼結束。僅警告的發現會列印出來，但以零狀態碼結束。
- 旗標：
...
- 不使用 `--version` 時，軟刪除套件和所有發行版。
- `--version <version>` 會透過失敗即封閉、版本專屬的路由，永久刪除一個已擁有的非最新版。
  已刪除的版本無法還原或重新發布。刪除目前最新版之前，請先發布替代版本。此僅版本流程需要套件擁有者或組織發布者管理員；平台人員不會繞過套件擁有權。
...
- 將套件轉移給另一個發布者。
- 除非由平台管理員執行，否則需要同時擁有目前套件擁有者與目標發布者的管理員存取權。
...
- 回報屬於套件層級，可選擇繫結到某個版本，並會顯示給版主審查。
- 回報本身不會自動隱藏套件或封鎖下載。
- 旗標：
  - `--version <version>`：要附加到回報的選填套件版本。
  - `--reason <text>`：必要的回報原因。
  - `--json`：機器可讀輸出。
...
- 顯示目前套件掃描狀態、未結回報數、最新發行版手動審核狀態、下載封鎖狀態，以及審核原因。
- 旗標：
  - `--json`：機器可讀輸出。
...
- 顯示可能取代內建 OpenClaw 外掛之套件的操作員導向遷移狀態。
- 呼叫與 `package readiness` 相同的計算就緒狀態端點，但會列印以遷移為重點的狀態、最新版本、官方套件狀態、檢查和阻擋項目。
- 旗標：
  - `--json`：機器可讀輸出。
...
- `<source>` 接受：
  - 本機資料夾路徑：`./my-plugin`
  - 本機 ClawPack npm-pack tar球：`./my-plugin-1.2.3.tgz`
...
- `.tgz` 來源會被視為 ClawPack。命令列介面會上傳精確的 npm-pack
  ...
  中繼資料預填。
- 程式碼外掛資料夾會在上傳前封裝成 ClawPack npm tarball，以便
  OpenClaw 安裝可以驗證精確的成品。bundle 外掛資料夾仍然
  使用解出檔案的發布路徑。
...
- `--json` 會為 CI 輸出機器可讀的內容。
- 當執行者具有發布者存取權時，`--owner <handle>` 會以使用者或組織發布者 handle 發布。
- scoped package 名稱必須符合所選 owner。請參閱 `docs/publishing.md`。
...
請先使用 `--dry-run`，以便在建立實際發布前確認解析後的套件中繼資料和
來源歸屬：
...
外部程式碼外掛需要在
`package.json` 中提供少量 OpenClaw 中繼資料。這份最小 manifest 足以成功發布：
...
- 如果你使用較舊的 `clawhub` 命令列介面版本，請在發布前升級，以便
  本機預檢查可以在上傳前執行。
...
- 將可重用工作流程釘選到穩定 tag 或完整 commit SHA。不要從 `@main` 執行發布。
- `pull_request` 應使用 `dry_run: true`，讓 CI 保持不產生污染。
- 實際發布應限制於可信任事件，例如 `workflow_dispatch` 或 tag push。
- 不使用 secret 的可信任發布只適用於 `workflow_dispatch`；tag push 仍需要 `clawhub_token`。
- 保持 `clawhub_token` 可用，以供首次發布、不受信任的套件，或緊急發布使用。
- 工作流程會將 JSON 結果上傳為成品，並將其公開為工作流程輸出。

### `package trusted-publisher get <name>`

- 顯示套件的 GitHub Actions 可信任發布者設定。
- 設定 config 後使用此命令，以確認儲存庫、工作流程檔名，
...
- `--environment <name>` 是選用的。設定後，OIDC claim 中的 GitHub Actions
  environment 必須完全相符。
- ClawHub 會在此命令執行時驗證設定的 GitHub 儲存庫。
  公開儲存庫可以透過公開 GitHub 中繼資料驗證。私有
  儲存庫需要 ClawHub 擁有該 GitHub 儲存庫的存取權，例如
...
- 旗標：
  ...
  - `--environment <name>`：選用的精確相符 GitHub Actions environment。
  - `--json`：機器可讀輸出。
...
- 從套件移除可信任發布者設定。
- 如果工作流程、儲存庫或 environment pin 需要
  停用或重新建立，請使用此作為回復。
...
- 登入時，在 `clawhub install <slug>` 之後傳送，除非
