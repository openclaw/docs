---
read_when:
    - 你執行了 clawhub package validate，並需要修正外掛發現的問題
    - ClawHub 拒絕或警告外掛套件發布
    - 你正在發布前更新外掛套件中繼資料
summary: 修正在發布前發現的 ClawHub 外掛套件驗證問題
title: 外掛驗證修正
x-i18n:
    generated_at: "2026-07-01T18:06:43Z"
    model: gpt-5.5
...
ClawHub 會在發布前驗證外掛套件，也可以顯示自動化套件掃描的發現項目。此頁涵蓋面向作者的發現項目，也就是外掛作者可以在其套件中繼資料、manifest、SDK 匯入或已發布成品中修正的發現項目。

此頁不涵蓋內部 Plugin Inspector 覆蓋率發現項目。如果完整報告包含沒有作者補救指引的掃描器維護代碼，這些代碼是給 OpenClaw 維護者，而不是外掛作者。
...
| `package-manifest-version-drift`        | [對齊套件與 manifest 版本](/zh-TW/clawhub/plugin-validation-fixes#package-manifest-version-drift)                          |
...
| `package-npm-pack-unavailable`          | [讓 npm 成品可被打包](/zh-TW/clawhub/plugin-validation-fixes#package-npm-pack-unavailable)                                 |
...
| `manifest-name-missing`                 | [新增 manifest 顯示名稱](/zh-TW/clawhub/plugin-validation-fixes#manifest-name-missing)                                           |
| `manifest-unknown-fields`               | [移除不支援的 manifest 欄位](/zh-TW/clawhub/plugin-validation-fixes#manifest-unknown-fields)                                  |
...
| `sdk-load-session-store`                | [取代整個 session store 存取](/zh-TW/clawhub/plugin-validation-fixes#sdk-load-session-store)                                   |
| `sdk-session-store-write`               | [取代整個 session store 寫入](/zh-TW/clawhub/plugin-validation-fixes#sdk-session-store-write)                                  |
| `sdk-session-file-helper`               | [取代 session 檔案路徑輔助工具](/zh-TW/clawhub/plugin-validation-fixes#sdk-session-file-helper)                                   |
| `sdk-session-transcript-file-target`    | [取代舊版 transcript 檔案目標](/zh-TW/clawhub/plugin-validation-fixes#sdk-session-transcript-file-target)                   |
| `sdk-session-transcript-low-level`      | [取代低階 transcript 輔助工具](/zh-TW/clawhub/plugin-validation-fixes#sdk-session-transcript-low-level)                       |
| `legacy-before-agent-start`             | [取代 before_agent_start](/zh-TW/clawhub/plugin-validation-fixes#legacy-before-agent-start)                                        |
| `provider-auth-env-vars`                | [將 provider env vars 移至設定中繼資料](/zh-TW/clawhub/plugin-validation-fixes#provider-auth-env-vars)                             |
| `channel-env-vars`                      | [在目前中繼資料中鏡像 channel env vars](/zh-TW/clawhub/plugin-validation-fixes#channel-env-vars)                                |
| `security-manifest-schema-unavailable`  | [移除無法使用的安全性 manifest schema 參照](/zh-TW/clawhub/plugin-validation-fixes#security-manifest-schema-unavailable) |
| `unrecognized-security-manifest`        | [移除不支援的安全性 manifest 檔案](/zh-TW/clawhub/plugin-validation-fixes#unrecognized-security-manifest)                   |
...
- 使用[建置外掛](/zh-TW/plugins/building-plugins)查看最小套件範例，並使用[外掛 manifest](/zh-TW/plugins/manifest#manifest-versus-packagejson)了解套件與 manifest 的分工。
...
套件中繼資料存在，但未宣告 OpenClaw runtime 進入點。
...
套件宣告了 OpenClaw 進入點，但在正在驗證的套件中找不到參照的檔案。
...
- 將此項與套件版本分開。套件版本描述外掛發行版；`openclaw.compat.pluginApi` 描述主機 API 契約。
- 請參閱[影響探索的 package.json 欄位](/zh-TW/plugins/manifest#packagejson-fields-that-affect-discovery)。
- 重新執行 `clawhub package validate <path-to-plugin>`。
...
套件版本與外掛 manifest 版本不一致。

- 優先使用 `package.json#version` 作為套件發行版本。
- 如果 `openclaw.plugin.json` 也有 `version`，請更新它以符合套件版本，或在套件中繼資料具權威性時移除過時的 manifest 版本中繼資料。
...
- 將套件進入點、相容性、安裝、設定和 catalog 中繼資料保留在支援的 `package.json#openclaw` 欄位中。
- 請參閱[影響探索的 package.json 欄位](/zh-TW/plugins/manifest#packagejson-fields-that-affect-discovery)。
- 重新執行 `clawhub package validate <path-to-plugin>`。
...
套件無法被打包成 ClawHub 會檢查或發布的成品。

- 從套件根目錄執行 `npm pack --dry-run`。
- 修正無效的套件中繼資料、損壞的 lifecycle scripts，或會導致打包失敗的 files entries。
- 如果此套件打算公開發布，請移除 `private: true`。
- 重新執行 `clawhub package validate <path-to-plugin>`。

### package-npm-pack-entrypoint-missing

套件可以被打包，但打包後的成品未包含 `package.json#openclaw` 中宣告的進入點檔案。
...
打包後的成品缺少來源套件中存在的 OpenClaw 中繼資料。
...
- 在 `openclaw.plugin.json` 中新增非空的 `name` 欄位。
- 讓 `name` 保持人類可讀，並讓 `id` 作為穩定的機器識別碼。
...
此外掛從已棄用的根 SDK barrel 匯入：
`openclaw/plugin-sdk`。

- 將根 barrel 匯入替換為聚焦的公開子路徑匯入。
- 對 `definePluginEntry` 使用 `openclaw/plugin-sdk/plugin-entry`。
- 對通道進入點輔助工具使用 `openclaw/plugin-sdk/channel-core`。
- 使用[匯入慣例](/zh-TW/plugins/building-plugins#import-conventions)和
  [外掛 SDK 子路徑](/zh-TW/plugins/sdk-subpaths)來找出精確的匯入。
- 重新執行 `clawhub package validate <path-to-plugin>`。

### reserved-sdk-import

此外掛匯入了保留給內建外掛或內部相容性的 SDK 路徑。

- 將保留的 OpenClaw 內部 SDK 匯入替換為文件化的公開
...
此外掛仍使用已棄用的工作階段檔案路徑輔助工具，例如
`resolveSessionFilePath` 或 `resolveAndPersistSessionFile`。

- 使用 `getSessionEntry(...)`，依代理程式和工作階段身分讀取工作階段中繼資料。
- 使用 `patchSessionEntry(...)` 或 `upsertSessionEntry(...)` 來保存工作階段
  中繼資料。
...
- 優先使用結構化的逐字稿執行階段介面，讓 OpenClaw 可以套用正確的
  交易邊界和身分處理。
- 只有在你宣告的相容性範圍仍支援需要它們的較舊 OpenClaw 版本時，
  才保留低階逐字稿輔助工具。
- 請參閱[執行階段 API](/zh-TW/plugins/sdk-runtime#agent-session-state)和
...
此外掛仍使用舊版 `before_agent_start` hook。
...
資訊清單使用舊版或較舊的通道環境變數中繼資料，缺少 ClawHub 期望的目前
設定或組態中繼資料。

- 讓通道環境變數中繼資料保持宣告式，使 OpenClaw 可以在不載入通道執行階段的情況下
  檢查設定狀態。
- 將環境驅動的通道設定鏡像到你的外掛形態所使用的目前設定、通道組態或
  套件通道中繼資料。
...
套件隨附的 `openclaw.security.json` 包含一個 ClawHub 不認為可用的結構描述參照。

- 如果結構描述 URL 僅供建議，請移除它。
- 只有在 OpenClaw 發布版本化結構描述後，才使用文件化的版本化結構描述。
- 重新執行 `clawhub package validate <path-to-plugin>`。
...
- 在 OpenClaw 文件化版本化安全性資訊清單結構描述和 ClawHub 行為之前，
  移除 `openclaw.security.json`。
- 在資訊清單合約存在之前，將安全性敏感行為記錄在你的公開套件文件或
  README 中。
- 重新執行 `clawhub package validate <path-to-plugin>`。
