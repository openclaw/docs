...
title: 外掛驗證修正
x-i18n:
    generated_at: "2026-07-03T13:15:30Z"
    model: gpt-5.5
...
ClawHub 會在發布前驗證外掛套件，也可以顯示自動套件掃描的發現項目。本頁涵蓋面向作者的發現項目，也就是外掛作者可以在其套件中繼資料、清單、SDK 匯入或已發布成品中修正的發現項目。

本頁不涵蓋內部 Plugin Inspector 覆蓋率發現項目。如果完整報告包含沒有作者補救指引的掃描器維護代碼，這些是給 OpenClaw 維護者處理，而不是給外掛作者。
...
| `legacy-root-sdk-import`                | [替換根 SDK 匯入](/zh-TW/clawhub/plugin-validation-fixes#legacy-root-sdk-import)                                             |
| `reserved-sdk-import`                   | [移除保留的 SDK 匯入](/zh-TW/clawhub/plugin-validation-fixes#reserved-sdk-import)                                             |
| `sdk-load-session-store`                | [替換整個工作階段儲存存取](/zh-TW/clawhub/plugin-validation-fixes#sdk-load-session-store)                                   |
| `sdk-session-store-write`               | [替換整個工作階段儲存寫入](/zh-TW/clawhub/plugin-validation-fixes#sdk-session-store-write)                                  |
| `sdk-session-file-helper`               | [替換工作階段檔案路徑輔助工具](/zh-TW/clawhub/plugin-validation-fixes#sdk-session-file-helper)                                   |
| `sdk-session-transcript-file-target`    | [替換舊版逐字稿檔案目標](/zh-TW/clawhub/plugin-validation-fixes#sdk-session-transcript-file-target)                   |
| `sdk-session-transcript-low-level`      | [替換低階逐字稿輔助工具](/zh-TW/clawhub/plugin-validation-fixes#sdk-session-transcript-low-level)                       |
| `legacy-before-agent-start`             | [替換 before_agent_start](/zh-TW/clawhub/plugin-validation-fixes#legacy-before-agent-start)                                        |
...
| `security-manifest-schema-unavailable`  | [移除不可用的安全性清單架構參照](/zh-TW/clawhub/plugin-validation-fixes#security-manifest-schema-unavailable) |
...
- 請參閱 [建置外掛](/zh-TW/plugins/building-plugins) 取得最小套件範例，並參閱 [外掛清單](/zh-TW/plugins/manifest#manifest-versus-packagejson) 了解套件與清單的分工。
...
套件宣告了 OpenClaw 進入點，但被參照的檔案不存在於正在驗證的套件中。
...
- 將它與套件版本分開。套件版本描述外掛發行；`openclaw.compat.pluginApi` 描述主機 API 合約。
- 請參閱 [影響探索的 package.json 欄位](/zh-TW/plugins/manifest#packagejson-fields-that-affect-discovery)。
- 重新執行 `clawhub package validate <path-to-plugin>`。
...
- 如果 `openclaw.plugin.json` 也有 `version`，請更新它以符合套件版本，或在套件中繼資料具權威性時移除過時的清單版本中繼資料。
...
- 修正無效的套件中繼資料、損壞的生命週期指令碼，或導致封裝失敗的檔案項目。
...
封裝後的成品缺少存在於來源套件中的 OpenClaw 中繼資料。

- 執行 `npm pack --dry-run` 並檢查被包含的中繼資料檔案。
...
- 使用[匯入慣例](/zh-TW/plugins/building-plugins#import-conventions)和
  [外掛 SDK 子路徑](/zh-TW/plugins/sdk-subpaths)尋找範圍較窄的匯入。
- 重新執行 `clawhub package validate <path-to-plugin>`。
...
- 將保留的 OpenClaw 內部 SDK 匯入替換為已記載的公開
...
- 更新既有工作階段項目上的欄位時，使用 `patchSessionEntry(...)`。
...
- 使用 `patchSessionEntry(...)` 或 `upsertSessionEntry(...)` 持久保存工作階段
  中繼資料。
...
- 優先使用結構化逐字稿執行階段介面，讓 OpenClaw 可以套用正確的交易邊界與身分處理。
- 只有在你宣告的相容性範圍仍支援需要它們的舊版 OpenClaw 時，
  才保留低階逐字稿輔助工具。
- 請參閱[執行階段 API](/zh-TW/plugins/sdk-runtime#agent-session-state)和
  [外掛 SDK 子路徑](/zh-TW/plugins/sdk-subpaths)。
...
- 將提供者環境變數中繼資料鏡像至 `setup.providers[].envVars`。
...
- 讓通道環境變數中繼資料保持宣告式，讓 OpenClaw 無需載入通道執行階段即可檢查設定狀態。
- 將由環境變數驅動的通道設定鏡像至目前的設定、通道組態，或
  你的外掛形態所使用的套件通道中繼資料。
- 只有在較舊的受支援 OpenClaw 版本仍需要它時，才將 `channelEnvVars`
  保留為相容性中繼資料。
...
套件隨附 `openclaw.security.json`，其中包含 ClawHub 不認定為可用的結構描述參照。

- 如果結構描述 URL 僅供建議用途，請移除它。
- 只有在 OpenClaw 發布文件化的版本化結構描述後，才使用它。
- 重新執行 `clawhub package validate <path-to-plugin>`。
...
- 在資訊清單 contract 存在之前，請將安全性敏感行為記載於你的公開套件文件或
  README 中。
- 重新執行 `clawhub package validate <path-to-plugin>`。
