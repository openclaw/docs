---
read_when:
    - 您已執行 clawhub package validate，並需要修正外掛發現項目
...
ClawHub 會在發布前驗證外掛套件，也可以顯示自動套件掃描的發現項目。本頁說明面向作者的發現項目，也就是外掛作者可在其套件中繼資料、資訊清單、SDK 匯入或已發布成品中修正的發現項目。

本頁不涵蓋內部 Plugin Inspector 覆蓋率發現項目。如果完整報告包含沒有作者補救指引的掃描器維護代碼，這些是給 OpenClaw 維護者，而非外掛作者使用。
...
| `package-manifest-version-drift`        | [對齊套件和資訊清單版本](/zh-TW/clawhub/plugin-validation-fixes#package-manifest-version-drift)                          |
...
| `package-npm-pack-unavailable`          | [使 npm 成品可打包](/zh-TW/clawhub/plugin-validation-fixes#package-npm-pack-unavailable)                                 |
...
- 當套件將透過 ClawHub 發布或安裝時，請新增相容性與安裝中繼資料。
...
- 使用你建置並測試依據的 OpenClaw 外掛 API 版本或 semver 下限。
- 將此與套件版本分開。套件版本描述外掛發布；`openclaw.compat.pluginApi` 描述主機 API 契約。
- 請參閱 [影響探索的 package.json 欄位](/zh-TW/plugins/manifest#packagejson-fields-that-affect-discovery)。
- 重新執行 `clawhub package validate <path-to-plugin>`。

### package-min-host-version-drift

套件最低主機版本與套件建置依據的 OpenClaw 版本中繼資料不相符。
...
- 如果 `openclaw.plugin.json` 也有 `version`，請將其更新為相符；或在套件中繼資料具權威性時，移除過時的資訊清單版本中繼資料。
- 變更已發布中繼資料後，發布新的套件版本。
...
套件可以打包，但打包成品未包含 `package.json#openclaw` 中宣告的進入點檔案。
...
此外掛從已棄用的根 SDK 彙整匯出入口匯入：
`openclaw/plugin-sdk`。

- 將根層彙整匯入取代為聚焦的公開子路徑匯入。
...
- 使用[匯入慣例](/zh-TW/plugins/building-plugins#import-conventions)和
  [外掛 SDK 子路徑](/zh-TW/plugins/sdk-subpaths)尋找較窄的匯入。
- 重新執行 `clawhub package validate <path-to-plugin>`。
...
- 將提示或脈絡修改工作移至 `before_prompt_build`。
- 只有在你宣告的相容性範圍仍支援需要它的較舊 OpenClaw 版本時，
...
資訊清單使用舊版或較舊的頻道環境變數中繼資料，缺少 ClawHub 預期的目前設定或設定中繼資料。

- 讓頻道環境變數中繼資料保持宣告式，讓 OpenClaw 可以在不載入頻道執行階段的情況下檢查設定狀態。
- 將環境變數驅動的頻道設定鏡像到目前的設定、頻道設定，或
  你的外掛形態所使用的套件頻道中繼資料。
