---
read_when:
    - 你執行了 clawhub package validate，並且需要修正外掛發現項目
    - ClawHub 拒絕或警告外掛套件發布
    - 你正在發行前更新外掛套件中繼資料
summary: 發佈前修正 ClawHub 外掛套件驗證發現事項
title: 外掛驗證修正
x-i18n:
    generated_at: "2026-07-05T11:05:55Z"
    model: gpt-5.5
...
本頁不涵蓋內部 Plugin Inspector 涵蓋率發現項目。如果完整報告包含沒有作者補救指引的掃描器維護代碼，這些是給 OpenClaw 維護者看的，而不是給外掛作者看的。
...
- 執行 `npm pack --dry-run` 並檢查會包含的檔案。
- 打包前建置產生的進入點。
- 更新 `files`、`.npmignore` 或建置輸出，讓宣告的進入點包含在內。
- 請參閱[外掛進入點](/zh-TW/plugins/sdk-entrypoints)。
- 重新執行 `clawhub package validate <path-to-plugin>`。
...
外掛仍在使用已淘汰的整個 session store 輔助工具
`loadSessionStore`。
...
外掛仍在使用已淘汰的整個 session store 寫入輔助工具，例如
`saveSessionStore` 或 `updateSessionStore`。

- 更新現有 session 項目的欄位時，使用 `patchSessionEntry(...)`。
- 取代或建立 session 項目時，使用 `upsertSessionEntry(...)`。
- 避免載入、變更並儲存整個 session store 物件。
...
外掛仍在使用已淘汰的 session 檔案路徑輔助工具，例如
`resolveSessionFilePath` 或 `resolveAndPersistSessionFile`。

- 使用 `getSessionEntry(...)` 依 agent 與 session 身分讀取 session 中繼資料。
- 使用 `patchSessionEntry(...)` 或 `upsertSessionEntry(...)` 持久化 session
  中繼資料。
- 當程式碼正在準備 transcript 操作時，使用 transcript 身分或 target 輔助工具。
...
外掛仍在使用已淘汰的 transcript 檔案 target 輔助工具
`resolveSessionTranscriptLegacyFileTarget`。

- 當程式碼只需要公開 session 身分時，使用 `resolveSessionTranscriptIdentity(...)`。
- 當程式碼需要結構化的 transcript 操作 target 時，使用
  `resolveSessionTranscriptTarget(...)`。
- 避免直接讀取或建構舊版 transcript 檔案 target。
- 只有在你宣告的相容性範圍仍支援需要它的較舊 OpenClaw 版本時，
  才保留舊版輔助工具。
- 請參閱[執行階段 API](/zh-TW/plugins/sdk-runtime#agent-session-state)和
  [外掛 SDK 子路徑](/zh-TW/plugins/sdk-subpaths)。
...
外掛仍在使用已淘汰的低階 transcript 輔助工具，例如
`appendSessionTranscriptMessage` 或 `emitSessionTranscriptUpdate`。

- 使用 `appendSessionTranscriptMessageByIdentity(...)` 進行 transcript 附加。
- 使用 `publishSessionTranscriptUpdateByIdentity(...)` 發送 transcript 更新通知。
- 優先使用結構化的 transcript 執行階段介面，讓 OpenClaw 能套用正確的交易邊界與身分處理。
- 只有在你宣告的相容性範圍仍支援需要它們的較舊 OpenClaw 版本時，
...
manifest 仍在使用舊版 `providerAuthEnvVars` provider auth 中繼資料。

- 將 provider env-var 中繼資料鏡像至 `setup.providers[].envVars`。
...
manifest 使用舊版或較舊的頻道 env-var 中繼資料，且缺少 ClawHub 預期的目前
setup 或 config 中繼資料。

- 讓頻道 env-var 中繼資料保持宣告式，使 OpenClaw 無需載入頻道執行階段即可檢查 setup 狀態。
- 將由 env 驅動的頻道 setup 鏡像至你的外掛形態所使用的目前 setup、頻道 config 或
  套件頻道中繼資料。
- 只有在較舊且仍受支援的 OpenClaw 版本仍需要時，才將 `channelEnvVars`
  保留為相容性中繼資料。
...
套件隨附 `openclaw.security.json`，其中的 schema 參考未被 ClawHub
識別為可用。
