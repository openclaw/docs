---
read_when:
    - 你已執行 clawhub package validate，並需要修正外掛檢查結果
    - ClawHub 拒絕外掛套件發布或發出警告
    - 你正在發布前更新外掛套件中繼資料
summary: 修正 ClawHub 外掛套件驗證結果後再發布
title: 外掛驗證修正
x-i18n:
    generated_at: "2026-07-06T10:47:44Z"
    model: gpt-5.5
...
ClawHub 會在發布前驗證外掛套件，也可以顯示自動套件掃描的發現項目。本頁說明面向作者的發現項目，也就是外掛作者可以在其套件中繼資料、資訊清單、SDK 匯入或已發布成品中修正的發現項目。
...
- 將這與套件版本分開。套件版本描述外掛版本；`openclaw.compat.pluginApi` 描述主機 API 合約。
- 請參閱 [會影響探索的 package.json 欄位](/zh-TW/plugins/manifest#packagejson-fields-that-affect-discovery)。
- 重新執行 `clawhub package validate <path-to-plugin>`。
...
- 如果 `openclaw.plugin.json` 也有 `version`，請更新為相符版本，或在套件中繼資料具權威性時移除過時的資訊清單版本中繼資料。
...
- 將提示或情境修改工作移至 `before_prompt_build`。
- 只有在你宣告的相容性範圍仍支援需要它的較舊 OpenClaw 版本時，
...
資訊清單使用舊版或較舊的頻道環境變數中繼資料，缺少 ClawHub 目前預期的設定或設定中繼資料。

- 讓頻道環境變數中繼資料保持宣告式，讓 OpenClaw 能在不載入頻道執行階段的情況下檢查設定狀態。
- 將環境變數驅動的頻道設定鏡像到你的外掛形態使用的目前設定、頻道組態或套件頻道中繼資料。
...
- 在 OpenClaw 文件化版本化安全性資訊清單結構描述和 ClawHub 行為之前，請移除 `openclaw.security.json`。
