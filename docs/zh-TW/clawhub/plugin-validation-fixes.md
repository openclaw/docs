...
read_when:
    ...
    - ClawHub 拒絕或警告外掛套件發布
    - 你正在發行前更新外掛套件中繼資料
summary: 修正在發布前的 ClawHub 外掛套件驗證發現事項
title: 外掛驗證修正
x-i18n:
    generated_at: "2026-07-05T20:17:51Z"
    model: gpt-5.5
...
本頁不涵蓋內部 Plugin Inspector 涵蓋率發現項目。如果完整報告包含沒有作者補救指引的掃描器維護代碼，這些是給 OpenClaw 維護者看的，而不是外掛作者。
...
| `reserved-sdk-import`                   | [移除保留 SDK 匯入](/zh-TW/clawhub/plugin-validation-fixes#reserved-sdk-import)                                             |
...
| `sdk-session-transcript-file-target`    | [取代舊版轉錄檔目標](/zh-TW/clawhub/plugin-validation-fixes#sdk-session-transcript-file-target)                   |
| `sdk-session-transcript-low-level`      | [取代低階轉錄輔助工具](/zh-TW/clawhub/plugin-validation-fixes#sdk-session-transcript-low-level)                       |
...
| `channel-env-vars`                      | [在目前中繼資料中鏡像通道 env vars](/zh-TW/clawhub/plugin-validation-fixes#channel-env-vars)                                |
| `security-manifest-schema-unavailable`  | [移除無法使用的安全資訊清單 schema 參照](/zh-TW/clawhub/plugin-validation-fixes#security-manifest-schema-unavailable) |
...
套件最低主機版本與套件建置時依據的 OpenClaw 版本中繼資料不符。
...
- 如果 `openclaw.plugin.json` 也有 `version`，請更新以相符，或在套件中繼資料具權威性時移除過時的資訊清單版本中繼資料。
- 變更已發布中繼資料後，發布新的套件版本。
...
封裝後的成品遺失了存在於來源套件中的 OpenClaw 中繼資料。
...
- 將套件或安裝中繼資料移到受支援的 `package.json#openclaw` 欄位，
  而不是放在資訊清單中。
- 重新執行 `clawhub package validate <path-to-plugin>`。
...
- 使用[匯入慣例](/zh-TW/plugins/building-plugins#import-conventions)和
  [外掛 SDK 子路徑](/zh-TW/plugins/sdk-subpaths)來找出範圍更窄的匯入。
- 重新執行 `clawhub package validate <path-to-plugin>`。
...
外掛仍在使用已淘汰的工作階段檔案路徑輔助工具，例如
`resolveSessionFilePath` 或 `resolveAndPersistSessionFile`。

- 使用 `getSessionEntry(...)` 依代理和工作階段身分讀取工作階段中繼資料。
...
- 將提示詞或脈絡變更工作移到 `before_prompt_build`。
- 只有在你宣告的相容範圍仍支援需要它的較舊 OpenClaw 版本時，
...
資訊清單使用舊版或較舊的通道環境變數中繼資料，缺少 ClawHub 預期的
目前設定或組態中繼資料。

- 保持通道環境變數中繼資料為宣告式，讓 OpenClaw 可在不載入通道執行階段的情況下
  檢查設定狀態。
- 將環境變數驅動的通道設定鏡像到你的外掛形態所使用的目前設定、通道組態或
  套件通道中繼資料。
...
套件隨附的 `openclaw.security.json` 含有 ClawHub 不識別為可用的結構描述參照。

- 如果結構描述 URL 僅供建議用途，請將它移除。
- 只有在 OpenClaw 發佈已記錄的版本化結構描述後，才使用該結構描述。
- 重新執行 `clawhub package validate <path-to-plugin>`。
...
- 移除 `openclaw.security.json`，直到 OpenClaw 記錄版本化安全性
  資訊清單結構描述和 ClawHub 行為為止。
- 在資訊清單合約存在前，請將安全性敏感行為記錄在你的公開套件文件或
  README 中。
- 重新執行 `clawhub package validate <path-to-plugin>`。
