---
read_when:
    - 你已執行 `clawhub package validate`，並需要修正外掛發現項。
    - ClawHub 在發布外掛套件時拒絕或發出警告
    - 你正在發布前更新外掛套件中繼資料
summary: 發布前修正 ClawHub 外掛套件驗證發現的問題
title: 外掛驗證修正
x-i18n:
    generated_at: "2026-07-04T10:26:42Z"
    model: gpt-5.5
...
ClawHub 會在發布前驗證外掛套件，也可以顯示自動化套件掃描的發現項目。此頁說明面向作者的發現項目，也就是外掛作者可以在其套件中繼資料、資訊清單、SDK 匯入或已發布成品中修正的發現項目。

此頁不涵蓋內部 Plugin Inspector 覆蓋率發現項目。如果完整報告包含沒有作者補救指引的掃描器維護代碼，這些是給 OpenClaw 維護者看的，而不是給外掛作者看的。
...
- 將所有進入點路徑保留在套件目錄內。
- 請參閱 [外掛進入點](/zh-TW/plugins/sdk-entrypoints) 與 [會影響探索的 package.json 欄位](/zh-TW/plugins/manifest#packagejson-fields-that-affect-discovery)。
- 重新執行 `clawhub package validate <path-to-plugin>`。
...
- 使用[匯入慣例](/zh-TW/plugins/building-plugins#import-conventions)與
  [外掛 SDK 子路徑](/zh-TW/plugins/sdk-subpaths)來找到窄範圍匯入。
- 重新執行 `clawhub package validate <path-to-plugin>`。
...
- 只有在你宣告的相容性範圍仍支援需要它的舊版 OpenClaw 時，才保留
  `loadSessionStore(...)`。
...
- 避免載入、變更並儲存整個工作階段儲存區物件。
- 只有在你宣告的相容性範圍仍支援需要它們的舊版 OpenClaw 時，才保留整個儲存區寫入輔助工具。
- 請參閱[執行階段 API](/zh-TW/plugins/sdk-runtime#agent-session-state)與
  [外掛 SDK 子路徑](/zh-TW/plugins/sdk-subpaths)。
- 重新執行 `clawhub package validate <path-to-plugin>`。
...
- 避免直接讀取或建構舊版逐字稿檔案目標。
- 只有在你宣告的相容性範圍仍支援需要它的舊版 OpenClaw 時，才保留舊版輔助工具。
- 請參閱[執行階段 API](/zh-TW/plugins/sdk-runtime#agent-session-state)與
  [外掛 SDK 子路徑](/zh-TW/plugins/sdk-subpaths)。
- 重新執行 `clawhub package validate <path-to-plugin>`。
...
- 逐字稿附加請使用 `appendSessionTranscriptMessageByIdentity(...)`。
- 逐字稿更新通知請使用 `publishSessionTranscriptUpdateByIdentity(...)`。
- 優先使用結構化逐字稿執行階段介面，讓 OpenClaw 能套用正確的交易邊界與身分處理。
- 只有在你宣告的相容性範圍仍支援需要它們的舊版 OpenClaw 時，才保留低階逐字稿輔助工具。
- 請參閱[執行階段 API](/zh-TW/plugins/sdk-runtime#agent-session-state)與
  [外掛 SDK 子路徑](/zh-TW/plugins/sdk-subpaths)。
- 重新執行 `clawhub package validate <path-to-plugin>`。
...
- 請參閱[setup 參考](/zh-TW/plugins/manifest#setup-reference)與
  [SDK 遷移](/zh-TW/plugins/sdk-migration)。
- 重新執行 `clawhub package validate <path-to-plugin>`。

### channel-env-vars

資訊清單使用舊版或較舊的頻道環境變數中繼資料，缺少目前 ClawHub 預期的
setup 或 config 中繼資料。

- 讓頻道環境變數中繼資料保持宣告式，使 OpenClaw 能在不載入頻道執行階段的情況下檢查設定狀態。
- 將環境驅動的頻道設定鏡像到你外掛形態所使用的目前 setup、頻道 config 或
  套件頻道中繼資料。
...
套件隨附的 `openclaw.security.json` 使用 ClawHub 無法辨識為可用的 schema 參照。
