---
read_when:
    - 你執行了 clawhub package validate，現在需要修正外掛檢查結果
...
ClawHub 會在發布前驗證外掛套件，也可以顯示自動化套件掃描的發現。此頁面涵蓋面向作者的發現，也就是外掛作者可在套件中修正的中繼資料、資訊清單、SDK 匯入或發布成品。

此頁面不涵蓋內部 Plugin Inspector 的涵蓋範圍發現。如果完整報告包含沒有作者修正指引的掃描器維護代碼，這些項目是供 OpenClaw 維護者處理，而非外掛作者。
...
- 參閱[建置外掛](/zh-TW/plugins/building-plugins)以取得最小套件範例，並參閱[外掛資訊清單](/zh-TW/plugins/manifest#manifest-versus-packagejson)以了解套件與資訊清單的分工。
...
- 使用 `openclaw.install.minHostVersion` 指定 OpenClaw 主機的最低版本。
- 請參閱[影響探索的 package.json 欄位](/zh-TW/plugins/manifest#packagejson-fields-that-affect-discovery)。
- 重新執行 `clawhub package validate <path-to-plugin>`。
...
- 使用你進行建置與測試時所依據的 OpenClaw 外掛 API 版本或 semver 下限。
- 將此項目與套件版本分開。套件版本描述外掛版本；`openclaw.compat.pluginApi` 描述主機 API 契約。
- 請參閱[影響探索的 package.json 欄位](/zh-TW/plugins/manifest#packagejson-fields-that-affect-discovery)。
- 重新執行 `clawhub package validate <path-to-plugin>`。

### package-min-host-version-drift

套件的最低主機版本與套件建置時所依據的 OpenClaw 版本中繼資料不符。

- 檢查 `openclaw.install.minHostVersion`。
- 檢查套件中的所有 OpenClaw 建置中繼資料，例如發布期間所使用的 OpenClaw 版本。
...
- 如果 `openclaw.plugin.json` 也有 `version`，請更新使其相符；若以套件中繼資料為準，則移除過時的資訊清單版本中繼資料。
...
- `name` 應保持人類可讀，而 `id` 應保留為穩定的機器識別碼。
- 請參閱[外掛資訊清單](/zh-TW/plugins/manifest)。
- 重新執行 `clawhub package validate <path-to-plugin>`。
...
- 將每個頂層欄位與
  [資訊清單欄位參考資料](/zh-TW/plugins/manifest#top-level-field-reference)進行比較。
...
- 將 `contracts` 下的每個鍵與
  [合約參考資料](/zh-TW/plugins/manifest#contracts-reference)進行比較。
- 移除不受支援的合約鍵。
...
- 將根匯出入口匯入替換為範圍明確的公開子路徑匯入。
- 對 `definePluginEntry` 使用 `openclaw/plugin-sdk/plugin-entry`。
- 對頻道進入點輔助函式使用 `openclaw/plugin-sdk/channel-core`。
...
- 當程式碼準備進行對話記錄操作時，使用對話記錄身分或目標輔助函式。
- 請勿保存或依賴舊式對話記錄檔案路徑。
- 請參閱[執行階段 API](/zh-TW/plugins/sdk-runtime#agent-session-state)和
  [外掛 SDK 子路徑](/zh-TW/plugins/sdk-subpaths)。
- 重新執行 `clawhub package validate <path-to-plugin>`。
...
外掛仍使用已棄用的對話記錄檔案目標輔助函式
`resolveSessionTranscriptLegacyFileTarget`。

- 當程式碼只需要公開的工作階段身分時，使用 `resolveSessionTranscriptIdentity(...)`。
- 當程式碼需要結構化的對話記錄操作目標時，使用 `resolveSessionTranscriptTarget(...)`。
- 避免直接讀取或建構舊式對話記錄檔案目標。
- 只有在你宣告的相容性範圍仍支援需要此舊式輔助函式的舊版
  OpenClaw 時，才保留此函式。
- 請參閱[執行階段 API](/zh-TW/plugins/sdk-runtime#agent-session-state)和
  [外掛 SDK 子路徑](/zh-TW/plugins/sdk-subpaths)。
- 重新執行 `clawhub package validate <path-to-plugin>`。
...
外掛仍使用已棄用的低階對話記錄輔助函式，例如
`appendSessionTranscriptMessage` 或 `emitSessionTranscriptUpdate`。

- 使用 `appendSessionTranscriptMessageByIdentity(...)` 附加對話記錄。
- 使用 `publishSessionTranscriptUpdateByIdentity(...)` 發布對話記錄更新通知。
- 優先使用結構化的對話記錄執行階段介面，讓 OpenClaw 能套用
  正確的交易邊界與身分處理。
- 只有在你宣告的相容性範圍仍支援需要這些函式的舊版
  OpenClaw 時，才保留低階對話記錄輔助函式。
- 請參閱[執行階段 API](/zh-TW/plugins/sdk-runtime#agent-session-state)和
  [外掛 SDK 子路徑](/zh-TW/plugins/sdk-subpaths)。
- 重新執行 `clawhub package validate <path-to-plugin>`。
...
外掛仍使用舊式 `before_agent_start` 掛鉤。

- 將模型或供應商覆寫工作移至 `before_model_resolve`。
- 將提示或上下文修改工作移至 `before_prompt_build`。
- 只有在你宣告的相容性範圍仍支援需要 `before_agent_start` 的舊版
  OpenClaw 時，才保留此掛鉤。
...
資訊清單仍使用舊式 `providerAuthEnvVars` 供應商驗證中繼資料。

- 將供應商環境變數中繼資料同步至 `setup.providers[].envVars`。
- 只有在你支援的 OpenClaw 版本範圍仍需要時，才保留 `providerAuthEnvVars`
  作為相容性中繼資料。
- 請參閱[設定參考資料](/zh-TW/plugins/manifest#setup-reference)和
  [SDK 遷移](/zh-TW/plugins/sdk-migration)。
- 重新執行 `clawhub package validate <path-to-plugin>`。

### channel-env-vars

資訊清單使用舊式或較舊的頻道環境變數中繼資料，但缺少 ClawHub 預期的目前
設定或組態中繼資料。

- 將頻道環境變數中繼資料保持為宣告式，讓 OpenClaw 無須載入頻道執行階段
  即可檢查設定狀態。
- 將環境變數驅動的頻道設定同步至你的外掛形態所使用的目前設定、頻道組態或
  套件頻道中繼資料。
- 只有在仍受支援的舊版 OpenClaw 需要時，才保留 `channelEnvVars`
  作為相容性中繼資料。
...
套件隨附的 `openclaw.security.json` 含有 ClawHub 無法辨識為可用的
結構描述參考。

- 如果結構描述 URL 僅供參考，請將其移除。
- 只有在 OpenClaw 發布具版本的結構描述後，才使用文件記載的該結構描述。
- 重新執行 `clawhub package validate <path-to-plugin>`。
...
- 在 OpenClaw 記載具版本的安全性資訊清單結構描述與 ClawHub 行為之前，
  請移除 `openclaw.security.json`。
- 在資訊清單合約存在之前，請於你的公開套件文件或 README 中記載
  涉及安全性的行為。
- 重新執行 `clawhub package validate <path-to-plugin>`。
