...
ClawHub 會在發布前驗證外掛套件，也可以顯示自動化套件掃描的檢查結果。本頁涵蓋面向作者的檢查結果，也就是外掛作者可在其套件中繼資料、資訊清單、SDK 匯入或已發布成品中修正的檢查結果。

本頁不涵蓋內部 Plugin Inspector 覆蓋率檢查結果。如果完整報告包含沒有作者補救指引的掃描器維護代碼，這些代碼是給 OpenClaw 維護者使用，而不是給外掛作者使用。
...
- 參閱 [影響探索的 package.json 欄位](/zh-TW/plugins/manifest#packagejson-fields-that-affect-discovery)。
- 重新執行 `clawhub package validate <path-to-plugin>`。
...
- 讓所有進入點路徑保持在套件目錄內。
- 參閱 [外掛進入點](/zh-TW/plugins/sdk-entrypoints) 和 [影響探索的 package.json 欄位](/zh-TW/plugins/manifest#packagejson-fields-that-affect-discovery)。
- 重新執行 `clawhub package validate <path-to-plugin>`。
...
- 如果進入點產生到 `dist`，請建置套件。
- 如果進入點已移動，請更新中繼資料。
- 參閱 [外掛進入點](/zh-TW/plugins/sdk-entrypoints)。
- 重新執行 `clawhub package validate <path-to-plugin>`。
...
- 變更已發布中繼資料後，發布新的套件版本。
- 參閱 [外掛資訊清單](/zh-TW/plugins/manifest)。
- 重新執行 `clawhub package validate <path-to-plugin>`。
...
- 參閱 [建置外掛](/zh-TW/plugins/building-plugins)。
- 重新執行 `clawhub package validate <path-to-plugin>`。
...
清單仍使用舊版 `providerAuthEnvVars` 提供者驗證中繼資料。
...
套件隨附的 `openclaw.security.json` 使用了 ClawHub 不認可為可用的結構描述參考。
