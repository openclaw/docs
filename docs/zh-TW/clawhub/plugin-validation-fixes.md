---
read_when:
    - 你已執行 clawhub package validate，並需要修正外掛發現項目
    - ClawHub 拒絕或警告外掛套件發布
    - 你正在發行前更新外掛套件中繼資料
summary: 發布前修正 ClawHub 外掛套件驗證發現事項
title: 外掛驗證修正
x-i18n:
    generated_at: "2026-07-05T01:54:32Z"
    model: gpt-5.5
...
ClawHub 會在發布前驗證外掛套件，也可以顯示來自自動化套件掃描的發現。此頁涵蓋面向作者的發現，也就是外掛作者可以在其套件中修正的套件中繼資料、資訊清單、SDK 匯入或已發布成品相關發現。

此頁不涵蓋內部 Plugin Inspector 覆蓋率發現。如果完整報告包含沒有作者補救指引的掃描器維護代碼，那些是給 OpenClaw 維護者看的，而不是給外掛作者看的。
...
- 使用 [建置外掛](/zh-TW/plugins/building-plugins) 查看最小套件範例，並使用 [外掛資訊清單](/zh-TW/plugins/manifest#manifest-versus-packagejson) 了解套件與資訊清單的分工。
...
- 參見 [影響探索的 package.json 欄位](/zh-TW/plugins/manifest#packagejson-fields-that-affect-discovery)。
- 重新執行 `clawhub package validate <path-to-plugin>`。
...
- 將所有進入點路徑保持在套件目錄內。
- 參見 [外掛進入點](/zh-TW/plugins/sdk-entrypoints) 和 [影響探索的 package.json 欄位](/zh-TW/plugins/manifest#packagejson-fields-that-affect-discovery)。
- 重新執行 `clawhub package validate <path-to-plugin>`。

### package-entrypoint-missing

套件宣告了 OpenClaw 進入點，但被參照的檔案在正在驗證的套件中遺失。
...
- 如果進入點已移動，請更新中繼資料。
- 參見 [外掛進入點](/zh-TW/plugins/sdk-entrypoints)。
- 重新執行 `clawhub package validate <path-to-plugin>`。
...
- 使用你建置並測試時依據的 OpenClaw 外掛 API 版本或 semver 最低版本。
- 將此與套件版本分開。套件版本描述外掛發布；`openclaw.compat.pluginApi` 描述主機 API 合約。
- 參見 [影響探索的 package.json 欄位](/zh-TW/plugins/manifest#packagejson-fields-that-affect-discovery)。
- 重新執行 `clawhub package validate <path-to-plugin>`。

### package-min-host-version-drift

套件最低主機版本與套件建置時所依據的 OpenClaw 版本中繼資料不符。
...
- 如果 `openclaw.plugin.json` 也有 `version`，請更新它以相符，或在套件中繼資料為權威來源時移除過時的資訊清單版本中繼資料。
- 變更已發布的中繼資料後，發布新的套件版本。
- 參見 [外掛資訊清單](/zh-TW/plugins/manifest)。
- 重新執行 `clawhub package validate <path-to-plugin>`。
...
- 修正無效的套件中繼資料、損壞的生命週期指令碼，或導致打包失敗的 files entries。
- 如果此套件是要公開發布，請移除 `private: true`。
- 重新執行 `clawhub package validate <path-to-plugin>`。

### package-npm-pack-entrypoint-missing

套件可以打包，但打包後的成品不包含 `package.json#openclaw` 中宣告的進入點檔案。

- 執行 `npm pack --dry-run` 並檢查將會包含的檔案。
- 打包前先建置產生的進入點。
- 更新 `files`、`.npmignore` 或建置輸出，讓已宣告的進入點被包含進去。
- 參見 [外掛進入點](/zh-TW/plugins/sdk-entrypoints)。
- 重新執行 `clawhub package validate <path-to-plugin>`。

### package-npm-pack-metadata-missing

打包後的成品遺失了來源套件中存在的 OpenClaw 中繼資料。

- 執行 `npm pack --dry-run` 並檢查包含的中繼資料檔案。
- 確保打包後的成品中，`package.json` 包含 `openclaw` 區塊。
- 當套件是原生 OpenClaw 外掛時，確保包含 `openclaw.plugin.json`。
- 更新 `files` 或 `.npmignore`，讓套件中繼資料不會被排除。
- 參見 [建置外掛](/zh-TW/plugins/building-plugins)。
- 重新執行 `clawhub package validate <path-to-plugin>`。
...
- 讓 `name` 維持人類可讀，並讓 `id` 作為穩定的機器 ID。
- 請參閱[外掛資訊清單](/zh-TW/plugins/manifest)。
- 重新執行 `clawhub package validate <path-to-plugin>`。

### manifest-unknown-fields

外掛資訊清單有 OpenClaw 不支援的頂層欄位。
...
- 將 root-barrel 匯入取代為聚焦的公開子路徑匯入。
- 使用 `openclaw/plugin-sdk/plugin-entry` 取得 `definePluginEntry`。
- 使用 `openclaw/plugin-sdk/channel-core` 取得頻道進入點輔助工具。
- 使用[匯入慣例](/zh-TW/plugins/building-plugins#import-conventions)和
  [外掛 SDK 子路徑](/zh-TW/plugins/sdk-subpaths)尋找精準匯入。
- 重新執行 `clawhub package validate <path-to-plugin>`。
...
- 將保留的 OpenClaw 內部 SDK 匯入取代為已文件化的公開
...
- 避免直接讀取或建構舊版轉錄檔案目標。
- 只有在你宣告的相容性範圍仍支援需要它的較舊 OpenClaw 版本時，才保留舊版輔助工具。
- 請參閱[執行階段 API](/zh-TW/plugins/sdk-runtime#agent-session-state)和
  [外掛 SDK 子路徑](/zh-TW/plugins/sdk-subpaths)。
- 重新執行 `clawhub package validate <path-to-plugin>`。
...
外掛仍使用已棄用的低階轉錄輔助工具，例如
`appendSessionTranscriptMessage` 或 `emitSessionTranscriptUpdate`。

- 使用 `appendSessionTranscriptMessageByIdentity(...)` 附加轉錄。
- 使用 `publishSessionTranscriptUpdateByIdentity(...)` 發布轉錄更新通知。
- 優先使用結構化轉錄執行階段介面，讓 OpenClaw 能套用正確的交易邊界與身分處理。
- 只有在你宣告的相容性範圍仍支援需要它們的較舊 OpenClaw 版本時，才保留低階轉錄輔助工具。
- 請參閱[執行階段 API](/zh-TW/plugins/sdk-runtime#agent-session-state)和
  [外掛 SDK 子路徑](/zh-TW/plugins/sdk-subpaths)。
- 重新執行 `clawhub package validate <path-to-plugin>`。
...
- 只有在你宣告的相容性範圍仍支援需要它的較舊 OpenClaw 版本時，才保留 `before_agent_start`。
...
資訊清單使用舊版或較舊的頻道環境變數中繼資料，缺少 ClawHub 預期的目前
setup 或 config 中繼資料。

- 保持頻道環境變數中繼資料為宣告式，讓 OpenClaw 可以在不載入頻道執行階段的情況下檢查設定狀態。
- 將環境變數驅動的頻道設定鏡像到你的外掛形態所使用的目前 setup、頻道 config 或
  套件頻道中繼資料。
- 只有在較舊且受支援的 OpenClaw 版本仍需要它時，才將 `channelEnvVars` 保留為相容性中繼資料。
...
套件隨附的 `openclaw.security.json` 有 ClawHub
不認得為可用的結構描述參照。

- 如果結構描述 URL 僅供建議，請移除它。
- 只有在 OpenClaw 發布版本化結構描述後，才使用已文件化的版本化結構描述。
- 重新執行 `clawhub package validate <path-to-plugin>`。
...
- 在資訊清單契約存在之前，請將安全性敏感行為記錄在你的公開套件文件或
  README 中。
- 重新執行 `clawhub package validate <path-to-plugin>`。
