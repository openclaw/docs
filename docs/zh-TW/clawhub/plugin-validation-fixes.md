---
read_when:
    - 你執行了 clawhub package validate，需要修正外掛發現項目
    - ClawHub 拒絕外掛套件發布或顯示警告
    - 你正在發行前更新外掛套件中繼資料
summary: 修正發布前的 ClawHub 外掛套件驗證發現事項
title: 外掛驗證修正
x-i18n:
    generated_at: "2026-07-03T02:42:04Z"
    model: gpt-5.5
...
本頁不涵蓋內部 Plugin Inspector 覆蓋率發現。如果完整報告包含沒有作者修復指引的掃描器維護代碼，這些是給 OpenClaw 維護者，而不是外掛作者使用。
...
- 使用 [建置外掛](/zh-TW/plugins/building-plugins) 查看最小套件範例，並參閱 [外掛資訊清單](/zh-TW/plugins/manifest#manifest-versus-packagejson) 了解套件與資訊清單的分工。
...
- 將此與套件版本分開。套件版本描述外掛發行；`openclaw.compat.pluginApi` 描述主機 API 合約。
- 請參閱 [會影響探索的 package.json 欄位](/zh-TW/plugins/manifest#packagejson-fields-that-affect-discovery)。
- 重新執行 `clawhub package validate <path-to-plugin>`。
...
`package.json#openclaw` 區塊包含不支援的 OpenClaw 套件中繼資料欄位。
...
- 將非空白的 `name` 欄位加入 `openclaw.plugin.json`。
- 保持 `name` 便於人類閱讀，並保持 `id` 作為穩定的機器 ID。
- 請參閱[外掛資訊清單](/zh-TW/plugins/manifest)。
- 重新執行 `clawhub package validate <path-to-plugin>`。
...
- 將 `contracts` 底下的每個鍵與
  [合約參考](/zh-TW/plugins/manifest#contracts-reference)比對。
- 移除不支援的合約鍵。
...
- 將根 barrel 匯入取代為聚焦的公開子路徑匯入。
- 對 `definePluginEntry` 使用 `openclaw/plugin-sdk/plugin-entry`。
- 對頻道進入輔助工具使用 `openclaw/plugin-sdk/channel-core`。
...
- 將保留的 OpenClaw 內部 SDK 匯入取代為已記錄的公開
...
外掛仍在使用已棄用的低階文字記錄輔助工具，例如
`appendSessionTranscriptMessage` 或 `emitSessionTranscriptUpdate`。

- 對文字記錄附加使用 `appendSessionTranscriptMessageByIdentity(...)`。
- 對文字記錄更新通知使用 `publishSessionTranscriptUpdateByIdentity(...)`。
- 優先使用結構化文字記錄執行階段介面，讓 OpenClaw 能套用正確的
  交易邊界與身分處理。
- 只有在你宣告的相容性範圍仍支援需要它們的較舊 OpenClaw 版本時，
  才保留低階文字記錄輔助工具。
- 請參閱[執行階段 API](/zh-TW/plugins/sdk-runtime#agent-session-state)和
...
資訊清單使用了舊版或較早的頻道環境變數中繼資料，缺少 ClawHub
預期的目前設定或組態中繼資料。

- 保持頻道環境變數中繼資料為宣告式，讓 OpenClaw 能在不載入頻道執行階段的情況下
  檢查設定狀態。
- 將環境驅動的頻道設定鏡像到你的外掛形態所使用的目前設定、頻道組態或
  套件頻道中繼資料。
- 只有在較舊且仍支援的 OpenClaw 版本仍需要它時，才將 `channelEnvVars`
  保留為相容性中繼資料。
...
套件隨附的 `openclaw.security.json` 包含 ClawHub 未識別為可用的架構參考。

- 如果架構 URL 只是建議用途，請將其移除。
- 只有在 OpenClaw 發布版本化架構之後，才使用已記錄的版本化架構。
- 重新執行 `clawhub package validate <path-to-plugin>`。
