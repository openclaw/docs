---
read_when:
    - 您已執行 clawhub package validate，並需要修正外掛發現項
    - ClawHub 拒絕外掛套件發布或對其提出警告
    - 你正在發布前更新外掛套件中繼資料
summary: 發布前修正 ClawHub 外掛套件驗證發現的問題
title: 外掛驗證修正
x-i18n:
    generated_at: "2026-07-05T04:50:01Z"
    model: gpt-5.5
...
本頁不涵蓋內部外掛檢查器覆蓋率發現項目。如果完整報告包含沒有作者修復指引的掃描器維護代碼，這些代碼是給 OpenClaw 維護者使用，而不是給外掛作者使用。
...
| `provider-auth-env-vars`                | [將供應者環境變數移至設定中繼資料](/zh-TW/clawhub/plugin-validation-fixes#provider-auth-env-vars)                             |
...
- 如果 `openclaw.plugin.json` 也有 `version`，請更新為相符值，或在套件中繼資料具權威性時移除過時的資訊清單版本中繼資料。
...
- 修正無效的套件中繼資料、損壞的生命週期腳本，或導致封裝失敗的 files 項目。
...
此外掛仍使用已淘汰的低階轉錄輔助函式，例如
`appendSessionTranscriptMessage` 或 `emitSessionTranscriptUpdate`。

- 使用 `appendSessionTranscriptMessageByIdentity(...)` 追加轉錄內容。
- 使用 `publishSessionTranscriptUpdateByIdentity(...)` 傳送轉錄更新通知。
...
- 將由環境變數驅動的頻道 setup 鏡像至你的外掛形態所使用的目前 setup、頻道設定或套件頻道中繼資料。
...
- 在 OpenClaw 文件化版本化安全性資訊清單 schema 和 ClawHub 行為之前，請移除 `openclaw.security.json`。
- 在資訊清單合約存在之前，請將安全性敏感行為記錄於你的公開套件文件或
  README。
- 重新執行 `clawhub package validate <path-to-plugin>`。
