...
title: 外掛驗證修正
x-i18n:
    generated_at: "2026-07-01T15:19:30Z"
    model: gpt-5.5
...
ClawHub 會在發布前驗證外掛套件，也可以顯示自動化套件掃描的發現事項。本頁涵蓋面向作者的發現事項，也就是外掛作者可在其套件中繼資料、資訊清單、SDK 匯入或已發布成品中修正的發現事項。

本頁不涵蓋內部 Plugin Inspector 覆蓋率發現事項。如果完整報告包含沒有作者補救指引的掃描器維護代碼，這些是給 OpenClaw 維護者看的，而不是給外掛作者看的。
...
- 當套件會透過 ClawHub 發布或安裝時，新增相容性和安裝中繼資料。
...
- 使用[匯入慣例](/zh-TW/plugins/building-plugins#import-conventions)和
  [外掛 SDK 子路徑](/zh-TW/plugins/sdk-subpaths)來尋找窄範圍匯入。
- 重新執行 `clawhub package validate <path-to-plugin>`。
...
外掛仍在使用已淘汰的轉錄稿檔案目標輔助工具
`resolveSessionTranscriptLegacyFileTarget`。

- 當程式碼只需要公開工作階段身分時，使用 `resolveSessionTranscriptIdentity(...)`。
- 當程式碼需要結構化的轉錄稿操作目標時，使用 `resolveSessionTranscriptTarget(...)`。
- 避免直接讀取或建構舊版轉錄稿檔案目標。
- 只有在你宣告的相容範圍仍支援需要它的較舊 OpenClaw 版本時，
  才保留舊版輔助工具。
- 請參閱[執行階段 API](/zh-TW/plugins/sdk-runtime#agent-session-state)和
...
外掛仍在使用已淘汰的低階轉錄稿輔助工具，例如
`appendSessionTranscriptMessage` 或 `emitSessionTranscriptUpdate`。

- 使用 `appendSessionTranscriptMessageByIdentity(...)` 追加轉錄稿。
- 使用 `publishSessionTranscriptUpdateByIdentity(...)` 發布轉錄稿更新通知。
- 優先使用結構化轉錄稿執行階段介面，讓 OpenClaw 能套用正確的交易邊界與身分處理。
- 只有在你宣告的相容範圍仍支援需要它們的較舊 OpenClaw 版本時，
  才保留低階轉錄稿輔助工具。
- 請參閱[執行階段 API](/zh-TW/plugins/sdk-runtime#agent-session-state)和
  [外掛 SDK 子路徑](/zh-TW/plugins/sdk-subpaths)。
...
外掛仍在使用舊版 `before_agent_start` 鉤子。
...
- 將供應商環境變數中繼資料鏡射到 `setup.providers[].envVars`。
...
資訊清單使用舊版或較舊的頻道環境變數中繼資料，未包含 ClawHub 預期的目前設定或組態中繼資料。

- 讓頻道環境變數中繼資料保持宣告式，使 OpenClaw 能在不載入頻道執行階段的情況下檢查設定狀態。
- 將環境驅動的頻道設定鏡射到目前設定、頻道組態，或你的外掛形態所使用的套件頻道中繼資料。
...
套件附帶的 `openclaw.security.json` 含有 ClawHub 未識別為可用的綱要參照。

- 如果綱要 URL 僅供建議用途，請移除它。
- 只有在 OpenClaw 發布文件化的版本化綱要後，才使用該綱要。
- 重新執行 `clawhub package validate <path-to-plugin>`。
...
- 在 OpenClaw 文件化版本化安全性資訊清單綱要與 ClawHub 行為之前，移除 `openclaw.security.json`。
