...
read_when:
    ...
    - ClawHub 在發布外掛套件時拒絕或發出警告
    - 你正在發布前更新外掛套件中繼資料
summary: 修正發布前的 ClawHub 外掛套件驗證發現
title: 外掛驗證修正
x-i18n:
    generated_at: "2026-07-01T20:11:11Z"
    model: gpt-5.5
...
這不涵蓋內部 Plugin Inspector 覆蓋率發現項目。如果完整報告包含沒有作者補救指引的掃描器維護代碼，這些是給 OpenClaw 維護者看的，而不是外掛作者。
...
- 使用 [建置外掛](/zh-TW/plugins/building-plugins) 查看最小套件範例，並使用 [外掛資訊清單](/zh-TW/plugins/manifest#manifest-versus-packagejson) 查看套件與資訊清單的分工。
...
- 將此與套件版本分開。套件版本描述外掛版本；`openclaw.compat.pluginApi` 描述主機 API 合約。
- 請參閱 [影響探索的 package.json 欄位](/zh-TW/plugins/manifest#packagejson-fields-that-affect-discovery)。
- 重新執行 `clawhub package validate <path-to-plugin>`。
...
- 如果 `openclaw.plugin.json` 也有 `version`，請更新它以相符；如果套件中繼資料具權威性，則移除過時的資訊清單版本中繼資料。
- 變更已發布中繼資料後，發布新的套件版本。
...
- 對 channel entry helpers 使用 `openclaw/plugin-sdk/channel-core`。
- 使用[匯入慣例](/zh-TW/plugins/building-plugins#import-conventions)和
  [外掛 SDK 子路徑](/zh-TW/plugins/sdk-subpaths)來找出範圍較窄的匯入。
- 重新執行 `clawhub package validate <path-to-plugin>`。
...
外掛仍使用已棄用的整體 session store helper
`loadSessionStore`。
...
外掛仍使用已棄用的整體 session store 寫入 helper，例如
`saveSessionStore` 或 `updateSessionStore`。

- 更新現有 session entry 的欄位時，使用 `patchSessionEntry(...)`。
- 取代或建立 session entry 時，使用 `upsertSessionEntry(...)`。
- 避免載入、修改並儲存整個 session store 物件。
- 只有在你宣告的相容性範圍仍支援需要它們的舊版 OpenClaw 時，
  才保留 whole-store write helpers。
- 請參閱[執行階段 API](/zh-TW/plugins/sdk-runtime#agent-session-state)和
  [外掛 SDK 子路徑](/zh-TW/plugins/sdk-subpaths)。
- 重新執行 `clawhub package validate <path-to-plugin>`。
...
外掛仍使用已棄用的 session file-path helpers，例如
`resolveSessionFilePath` 或 `resolveAndPersistSessionFile`。

- 使用 `getSessionEntry(...)` 依 agent 與 session 身分讀取 session 中繼資料。
- 使用 `patchSessionEntry(...)` 或 `upsertSessionEntry(...)` 保存 session
  中繼資料。
- 當程式碼正在準備 transcript 作業時，使用 transcript identity 或 target helpers。
...
外掛仍使用已棄用的 transcript file target helper
`resolveSessionTranscriptLegacyFileTarget`。

- 當程式碼只需要公開 session identity 時，使用 `resolveSessionTranscriptIdentity(...)`。
- 當程式碼需要結構化 transcript operation target 時，使用 `resolveSessionTranscriptTarget(...)`。
- 避免直接讀取或建構舊版 transcript file targets。
- 只有在你宣告的相容性範圍仍支援需要它的舊版 OpenClaw 時，
  才保留 legacy helper。
- 請參閱[執行階段 API](/zh-TW/plugins/sdk-runtime#agent-session-state)和
  [外掛 SDK 子路徑](/zh-TW/plugins/sdk-subpaths)。
...
外掛仍使用已棄用的低階 transcript helpers，例如
`appendSessionTranscriptMessage` 或 `emitSessionTranscriptUpdate`。

- 使用 `appendSessionTranscriptMessageByIdentity(...)` 進行 transcript 附加。
- 使用 `publishSessionTranscriptUpdateByIdentity(...)` 發送 transcript update
  通知。
- 優先使用結構化的 transcript runtime surface，讓 OpenClaw 可以套用正確的
  transaction boundaries 與 identity handling。
- 只有在你宣告的相容性範圍仍支援需要它們的舊版 OpenClaw 時，
  才保留低階 transcript helpers。
- 請參閱[執行階段 API](/zh-TW/plugins/sdk-runtime#agent-session-state)和
  [外掛 SDK 子路徑](/zh-TW/plugins/sdk-subpaths)。
- 重新執行 `clawhub package validate <path-to-plugin>`。
...
- 將 model 或 provider override 工作移至 `before_model_resolve`。
- 將 prompt 或 context mutation 工作移至 `before_prompt_build`。
- 只有在你宣告的相容性範圍仍支援需要它的舊版 OpenClaw 時，
...
manifest 仍使用舊版 `providerAuthEnvVars` provider auth 中繼資料。

- 將 provider env-var 中繼資料鏡射到 `setup.providers[].envVars`。
...
manifest 使用舊版或較舊的 channel env-var 中繼資料，缺少 ClawHub 預期的目前
setup 或 config 中繼資料。

- 讓 channel env-var 中繼資料保持宣告式，以便 OpenClaw 不必載入 channel
  runtime 即可檢查 setup 狀態。
- 將 env-driven channel setup 鏡射到你的外掛形態所使用的目前 setup、channel config
  或 package channel 中繼資料。
- 只有在較舊且受支援的 OpenClaw 版本仍需要它時，才將 `channelEnvVars`
  保留為相容性中繼資料。
- 請參閱[外掛 manifest](/zh-TW/plugins/manifest)和
  [Channel 外掛](/zh-TW/plugins/sdk-channel-plugins)。
- 重新執行 `clawhub package validate <path-to-plugin>`。
...
套件隨附的 `openclaw.security.json` 含有 ClawHub 不識別為可用的 schema 參考。

- 如果 schema URL 僅供建議，請移除它。
...
- 在 manifest contract 存在前，請將安全性敏感行為記錄在你的公開套件文件或
  README 中。
- 重新執行 `clawhub package validate <path-to-plugin>`。
...
- [外掛 entry points](/zh-TW/plugins/sdk-entrypoints)
- [外掛相容性](/zh-TW/plugins/compatibility)
