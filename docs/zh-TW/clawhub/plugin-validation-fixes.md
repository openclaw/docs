---
read_when:
    - 你已執行 clawhub package validate，且需要修正外掛發現項目
    - ClawHub 拒絕或警告外掛套件發布
    - 你正在發行前更新外掛套件中繼資料
summary: 發布前修正 ClawHub 外掛套件驗證發現項目
title: 外掛驗證修正
x-i18n:
    generated_at: "2026-06-27T19:02:28Z"
    model: gpt-5.5
...
ClawHub 會在發布前驗證外掛套件，也可以顯示來自自動套件掃描的發現。此頁面涵蓋面向作者的發現，也就是外掛作者可以在其套件中繼資料、manifest、SDK 匯入或已發布成品中修正的發現。

它不涵蓋內部外掛檢查器的涵蓋率發現。如果完整報告包含沒有作者補救指引的掃描器維護代碼，這些是給 OpenClaw 維護者看的，而不是給外掛作者。
...
| `security-manifest-schema-unavailable`  | [移除不可用的安全性 manifest schema 參照](/zh-TW/clawhub/plugin-validation-fixes#security-manifest-schema-unavailable) |
...
- 當套件出貨 OpenClaw 外掛時，新增 `openclaw` 區塊。
...
套件宣告了 OpenClaw 進入點，但被參照的檔案不在正在驗證的套件中。
...
- 使用受支援的安裝來源填入 `openclaw.install`，例如 `clawhubSpec`、`npmSpec` 或 `localPath`。
...
套件未宣告它支援的 OpenClaw 外掛 API 範圍。

- 將 `openclaw.compat.pluginApi` 新增至 `package.json`。
- 使用你建置並測試所依據的 OpenClaw 外掛 API 版本或 semver 下限。
- 將此與套件版本分開。套件版本描述外掛發行版；`openclaw.compat.pluginApi` 描述主機 API 合約。
- 請參閱 [影響探索的 package.json 欄位](/zh-TW/plugins/manifest#packagejson-fields-that-affect-discovery)。
- 重新執行 `clawhub package validate <path-to-plugin>`。
...
- 如果 `openclaw.plugin.json` 也有 `version`，請更新它以相符；或在套件中繼資料具權威性時，移除過時的 manifest 版本中繼資料。
...
- 修正無效的套件中繼資料、損壞的生命週期指令碼，或導致打包失敗的 files 項目。
...
- 使用[匯入慣例](/zh-TW/plugins/building-plugins#import-conventions)和
  [外掛 SDK 子路徑](/zh-TW/plugins/sdk-subpaths)找出範圍更窄的匯入。
- 重新執行 `clawhub package validate <path-to-plugin>`。
...
- 如果該行為沒有公開 SDK，請將輔助程式保留在你的 package 內，或
  要求提供公開的 OpenClaw API。
...
外掛仍使用已棄用的完整 session store 輔助程式
`loadSessionStore`。

- 讀取 session 狀態時，請使用 `getSessionEntry(...)` 或 `listSessionEntries(...)`。
- 寫入 session 狀態時，請使用 `patchSessionEntry(...)` 或 `upsertSessionEntry(...)`。
...
- 將 prompt 或 context 變更工作移到 `before_prompt_build`。
- 只有在你宣告的相容性範圍仍支援需要它的舊版 OpenClaw 時，
...
manifest 仍使用舊版 `providerAuthEnvVars` 提供者驗證中繼資料。

- 將提供者環境變數中繼資料鏡像到 `setup.providers[].envVars`。
- 只有在你支援的 OpenClaw 範圍仍需要時，
...
manifest 使用舊版或較舊的通道環境變數中繼資料，沒有 ClawHub 目前預期的
setup 或 config 中繼資料。

- 讓通道環境變數中繼資料保持宣告式，使 OpenClaw 無需載入通道執行階段
  即可檢查 setup 狀態。
- 將環境變數驅動的通道 setup 鏡像到你的外掛形態所使用的目前 setup、通道 config 或
  package 通道中繼資料。
- 只有在較舊且受支援的 OpenClaw 版本仍需要時，
...
package 隨附的 `openclaw.security.json` 具有 ClawHub 無法辨識為可用的 schema 參考。

- 如果 schema URL 只是建議性質，請將其移除。
- 只有在 OpenClaw 發布文件化的版本化 schema 後，才使用它。
- 重新執行 `clawhub package validate <path-to-plugin>`。

### unrecognized-security-manifest

package 隨附了不支援的安全性 manifest 檔案。

- 在 OpenClaw 文件化版本化安全性 manifest schema 和 ClawHub 行為之前，
  請移除 `openclaw.security.json`。
- 在 manifest contract 存在之前，請將安全性敏感行為記錄在你的公開 package 文件或
  README 中。
- 重新執行 `clawhub package validate <path-to-plugin>`。
