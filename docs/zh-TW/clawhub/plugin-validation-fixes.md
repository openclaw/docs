...
read_when:
    ...
    - 你正在發行前更新外掛套件中繼資料
summary: 修正發布前 ClawHub 外掛套件驗證發現的問題
title: 外掛驗證修正
x-i18n:
    generated_at: "2026-07-01T10:56:45Z"
    model: gpt-5.5
...
ClawHub 會在發布前驗證外掛套件，也可以顯示自動套件掃描的發現。此頁涵蓋面向作者的發現，也就是外掛作者可在其套件中繼資料、資訊清單、SDK 匯入或已發布成品中修正的發現。

這不涵蓋內部外掛檢查器涵蓋率發現。如果完整報告包含沒有作者補救指引的掃描器維護代碼，這些是給 OpenClaw 維護者而非外掛作者看的。
...
套件宣告了 OpenClaw 進入點，但參照的檔案在正在驗證的套件中遺失。
...
- 讓它與套件版本保持分離。套件版本描述外掛發行版；`openclaw.compat.pluginApi` 描述主機 API 合約。
- 請參閱[影響探索的 package.json 欄位](/zh-TW/plugins/manifest#packagejson-fields-that-affect-discovery)。
- 重新執行 `clawhub package validate <path-to-plugin>`。
...
- 如果 `openclaw.plugin.json` 也有 `version`，請更新為相符版本，或在套件中繼資料具有權威性時移除過時的資訊清單版本中繼資料。
...
- 將套件進入點、相容性、安裝、設定與目錄中繼資料保留在支援的 `package.json#openclaw` 欄位中。
- 請參閱[影響探索的 package.json 欄位](/zh-TW/plugins/manifest#packagejson-fields-that-affect-discovery)。
- 重新執行 `clawhub package validate <path-to-plugin>`。
...
- 修正無效的套件中繼資料、損壞的生命週期腳本，或會導致打包失敗的 files 項目。
- 如果此套件預計公開發布，請移除 `private: true`。
- 重新執行 `clawhub package validate <path-to-plugin>`。

### package-npm-pack-entrypoint-missing

套件可打包，但打包後的成品不包含 `package.json#openclaw` 中宣告的進入點檔案。
...
- 讓 `name` 保持適合人類閱讀，並讓 `id` 作為穩定的機器識別碼。
- 請參閱[外掛資訊清單](/zh-TW/plugins/manifest)。
- 重新執行 `clawhub package validate <path-to-plugin>`。
...
- 將套件或安裝中繼資料移入支援的 `package.json#openclaw` 欄位，
  而不是放在資訊清單中。
- 重新執行 `clawhub package validate <path-to-plugin>`。
...
- 優先使用結構化逐字稿執行階段表面，讓 OpenClaw 能套用正確的
  交易邊界和身分處理。
- 只有在你宣告的相容性範圍仍支援需要它們的舊版 OpenClaw 時，
  才保留低階逐字稿輔助工具。
- 請參閱[執行階段 API](/zh-TW/plugins/sdk-runtime#agent-session-state)和
...
- 將供應商環境變數中繼資料鏡像至 `setup.providers[].envVars`。
...
- 將環境驅動的頻道設定鏡像至你的外掛形態使用的目前設定、頻道組態或
  套件頻道中繼資料。
- 只有在較舊且受支援的 OpenClaw 版本仍需要時，才將 `channelEnvVars`
  保留為相容性中繼資料。
...
套件隨附 `openclaw.security.json`，其中含有 ClawHub 無法辨識為可用的
結構描述參考。

- 如果結構描述 URL 僅供諮詢，請移除它。
...
- 移除 `openclaw.security.json`，直到 OpenClaw 記載版本化安全性
  資訊清單結構描述和 ClawHub 行為。
