---
read_when:
    - 你執行了 clawhub package validate，需要修正外掛檢查發現的問題
...
自動套件掃描的發現事項。本頁涵蓋面向作者的發現事項，也就是
外掛作者可以在其套件中繼資料、清單、SDK
匯入或已發布成品中修正的發現事項。

本頁不涵蓋內部外掛檢查器涵蓋率發現事項。如果完整報告
包含沒有作者補救指引的掃描器維護代碼，這些代碼是給
...
- 當套件會透過 ClawHub 發布或安裝時，新增相容性與安裝
  中繼資料。
...
- 使用支援的安裝來源填入 `openclaw.install`，例如
...
套件最低主機版本與建置套件時所依據的 OpenClaw 版本中繼資料
不相符。
...
- 如果 `openclaw.plugin.json` 也有 `version`，請更新它使其相符；或在套件中繼資料為權威來源時，
  移除過時的清單版本中繼資料。
- 變更已發布中繼資料後，發布新的套件版本。
...
- 將套件進入點、相容性、安裝、設定和目錄中繼資料
  保留在受支援的 `package.json#openclaw` 欄位中。
- 請參閱 [影響探索的 package.json 欄位](/zh-TW/plugins/manifest#packagejson-fields-that-affect-discovery)。
- 重新執行 `clawhub package validate <path-to-plugin>`。
...
- 在封裝前建置產生的進入點。
- 更新 `files`、`.npmignore` 或建置輸出，讓已宣告的進入點
  被包含。
- 請參閱 [外掛進入點](/zh-TW/plugins/sdk-entrypoints)。
- 重新執行 `clawhub package validate <path-to-plugin>`。
...
- 當程式碼正在準備轉錄紀錄操作時，使用轉錄紀錄身分或目標輔助工具。
- 不要持久化或依賴舊版轉錄紀錄檔案路徑。
- 請參閱[執行階段 API](/zh-TW/plugins/sdk-runtime#agent-session-state)與
  [外掛 SDK 子路徑](/zh-TW/plugins/sdk-subpaths)。
- 重新執行 `clawhub package validate <path-to-plugin>`。
...
外掛仍在使用已棄用的轉錄紀錄檔案目標輔助工具
`resolveSessionTranscriptLegacyFileTarget`。

- 當程式碼只需要公開工作階段身分時，使用 `resolveSessionTranscriptIdentity(...)`。
- 當程式碼需要結構化的轉錄紀錄操作目標時，使用 `resolveSessionTranscriptTarget(...)`。
- 避免直接讀取或建構舊版轉錄紀錄檔案目標。
- 只有在你宣告的相容性範圍仍支援需要它的舊版 OpenClaw 時，
  才保留舊版輔助工具。
- 請參閱[執行階段 API](/zh-TW/plugins/sdk-runtime#agent-session-state)與
...
外掛仍在使用已棄用的低階轉錄紀錄輔助工具，例如
`appendSessionTranscriptMessage` 或 `emitSessionTranscriptUpdate`。

- 使用 `appendSessionTranscriptMessageByIdentity(...)` 追加轉錄紀錄。
- 使用 `publishSessionTranscriptUpdateByIdentity(...)` 傳送轉錄紀錄更新通知。
- 優先使用結構化的轉錄紀錄執行階段介面，讓 OpenClaw 能套用正確的
  交易邊界與身分處理。
- 只有在你宣告的相容性範圍仍支援需要這些工具的舊版 OpenClaw 時，
  才保留低階轉錄紀錄輔助工具。
- 請參閱[執行階段 API](/zh-TW/plugins/sdk-runtime#agent-session-state)與
...
清單仍在使用舊版 `providerAuthEnvVars` 提供者驗證中繼資料。
...
清單使用舊版或較舊的通道環境變數中繼資料，且缺少 ClawHub 預期的目前
setup 或設定中繼資料。

- 讓通道環境變數中繼資料保持宣告式，使 OpenClaw 能在不載入通道執行階段的情況下檢查 setup 狀態。
- 將環境驅動的通道 setup 鏡像至你的外掛形態所使用的目前 setup、通道設定或
  套件通道中繼資料。
- 只有在較舊的受支援 OpenClaw 版本仍需要它時，才將 `channelEnvVars`
  保留為相容性中繼資料。
- 請參閱[外掛清單](/zh-TW/plugins/manifest)與
...
套件隨附的 `openclaw.security.json` 包含 ClawHub 無法辨識為可用的結構描述參照。
...
- 在 OpenClaw 文件化版本化安全性清單結構描述與 ClawHub 行為之前，
  移除 `openclaw.security.json`。
- 在清單契約存在之前，請將安全性敏感行為記錄於你的公開套件文件或
  README 中。
- 重新執行 `clawhub package validate <path-to-plugin>`。
