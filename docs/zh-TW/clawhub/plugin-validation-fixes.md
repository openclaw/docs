...
title: 外掛驗證修正
x-i18n:
    generated_at: "2026-07-04T20:24:10Z"
    model: gpt-5.5
...
套件宣告了 OpenClaw 進入點，但所參照的檔案在正在驗證的套件中遺失。
...
- 用支援的安裝來源填寫 `openclaw.install`，例如
  `clawhubSpec`、`npmSpec` 或 `localPath`。
- 當有多個安裝來源可用時，設定 `openclaw.install.defaultChoice`。
- 使用 `openclaw.install.minHostVersion` 作為最低 OpenClaw 主機版本。
- 請參閱[影響探索的 package.json 欄位](/zh-TW/plugins/manifest#packagejson-fields-that-affect-discovery)。
- 重新執行 `clawhub package validate <path-to-plugin>`。
...
- 將其與套件版本分開。套件版本描述外掛發行版；`openclaw.compat.pluginApi` 描述主機 API 合約。
- 請參閱[影響探索的 package.json 欄位](/zh-TW/plugins/manifest#packagejson-fields-that-affect-discovery)。
- 重新執行 `clawhub package validate <path-to-plugin>`。
...
- 如果 `openclaw.plugin.json` 也有 `version`，請更新使其相符，或在套件中繼資料具有權威性時移除過時的資訊清單版本中繼資料。
- 變更已發布中繼資料後，發布新的套件版本。
...
- 保持 `name` 為人類可讀，並保持 `id` 作為穩定的機器識別碼。
- 請參閱[外掛資訊清單](/zh-TW/plugins/manifest)。
- 重新執行 `clawhub package validate <path-to-plugin>`。
...
- 使用 `publishSessionTranscriptUpdateByIdentity(...)` 進行轉錄更新通知。
- 優先使用結構化轉錄執行階段介面，讓 OpenClaw 能套用正確的交易邊界和身分處理。
- 僅在你宣告的相容性範圍仍支援需要它們的較舊 OpenClaw 版本時，
  才保留低階轉錄輔助工具。
- 請參閱[執行階段 API](/zh-TW/plugins/sdk-runtime#agent-session-state)和
  [外掛 SDK 子路徑](/zh-TW/plugins/sdk-subpaths)。
...
- 將提示詞或內容變更工作移至 `before_prompt_build`。
- 僅在你宣告的相容性範圍仍支援需要它的較舊 OpenClaw 版本時，
...
- 僅在你支援的 OpenClaw 範圍仍需要它時，才保留 `providerAuthEnvVars`
  作為相容性中繼資料。
...
- 將由環境變數驅動的頻道 setup 鏡像到目前 setup、頻道設定或
  你的外掛形態所使用的套件頻道中繼資料。
...
- 如果結構描述 URL 僅供建議使用，請移除它。
- 只有在 OpenClaw 發布已記錄的版本化結構描述後，才使用它。
- 重新執行 `clawhub package validate <path-to-plugin>`。
...
- 移除 `openclaw.security.json`，直到 OpenClaw 記錄版本化安全性資訊清單結構描述和 ClawHub 行為。
- 在資訊清單契約存在之前，將安全性敏感行為記錄在你的公開套件文件或
  README 中。
- 重新執行 `clawhub package validate <path-to-plugin>`。
