...
title: 外掛驗證修正
x-i18n:
    generated_at: "2026-07-04T15:07:37Z"
    model: gpt-5.5
...
ClawHub 會在發布前驗證外掛套件，也可以顯示來自自動化套件掃描的發現項目。此頁涵蓋面向作者的發現項目，也就是外掛作者可在其套件中繼資料、資訊清單、SDK 匯入或已發布成品中修正的發現項目。

它不涵蓋內部外掛檢查器涵蓋率發現項目。如果完整報告包含沒有作者修復指引的掃描器維護代碼，這些是給 OpenClaw 維護者看的，而不是給外掛作者看的。
...
| `sdk-session-transcript-low-level`      | [取代低階文字記錄輔助函式](/zh-TW/clawhub/plugin-validation-fixes#sdk-session-transcript-low-level)                       |
...
- 讓它與套件版本分開。套件版本描述外掛版本；`openclaw.compat.pluginApi` 描述主機 API 合約。
- 請參閱[影響探索的 package.json 欄位](/zh-TW/plugins/manifest#packagejson-fields-that-affect-discovery)。
- 重新執行 `clawhub package validate <path-to-plugin>`。
...
套件可以封裝，但封裝後的成品未包含在 `package.json#openclaw` 中宣告的進入點檔案。
...
封裝後的成品缺少你原始套件中存在的 OpenClaw 中繼資料。
...
- 將提供者環境變數中繼資料鏡射到 `setup.providers[].envVars`。
...
- 讓頻道環境變數中繼資料保持宣告式，讓 OpenClaw 能在不載入頻道執行階段的情況下檢查 setup 狀態。
- 將由環境變數驅動的頻道 setup 鏡射到目前 setup、頻道設定，或
  你的外掛形態所使用的套件頻道中繼資料。
- 僅在較舊的受支援 OpenClaw 版本仍需要它時，才將 `channelEnvVars`
  保留為相容性中繼資料。
...
套件隨附的 `openclaw.security.json` 包含 ClawHub 未辨識為可用的 schema 參考。

- 如果 schema URL 只是建議性質，請移除它。
- 僅在 OpenClaw 發布文件記載的版本化 schema 後才使用它。
- 重新執行 `clawhub package validate <path-to-plugin>`。
...
- 在 OpenClaw 記載版本化安全性清單 schema 和 ClawHub 行為之前，
  移除 `openclaw.security.json`。
- 在清單合約存在之前，將安全性敏感行為記載於你的公開套件文件或
  README。
- 重新執行 `clawhub package validate <path-to-plugin>`。
