---
read_when:
    - 你執行了 clawhub package validate，並需要修正外掛發現事項
    - ClawHub 在發布外掛套件時拒絕或發出警告
    - 你正在發布前更新外掛套件中繼資料
summary: 發佈前修正 ClawHub 外掛套件驗證發現事項
title: 外掛驗證修正
x-i18n:
...
ClawHub 會在發布前驗證外掛套件，也可以顯示來自自動化套件掃描的發現項目。本頁涵蓋面向作者的發現項目，也就是外掛作者可以在其套件中繼資料、清單、SDK 匯入或已發布成品中修正的發現項目。

它不涵蓋內部外掛 Inspector 覆蓋率發現項目。如果完整報告包含沒有作者補救指引的掃描器維護代碼，這些是給 OpenClaw 維護者而非外掛作者使用。
...
| `package-npm-pack-unavailable`          | [讓 npm 成品可以打包](/zh-TW/clawhub/plugin-validation-fixes#package-npm-pack-unavailable)                                 |
...
| `security-manifest-schema-unavailable`  | [移除無法使用的安全性清單 schema 參照](/zh-TW/clawhub/plugin-validation-fixes#security-manifest-schema-unavailable) |
...
- 使用 [建置外掛](/zh-TW/plugins/building-plugins) 查看最小套件範例，並使用 [外掛清單](/zh-TW/plugins/manifest#manifest-versus-packagejson) 了解套件與清單的分工。
...
- 將其與套件版本分開。套件版本描述外掛發布；`openclaw.compat.pluginApi` 描述主機 API 合約。
- 請參閱 [影響探索的 package.json 欄位](/zh-TW/plugins/manifest#packagejson-fields-that-affect-discovery)。
- 重新執行 `clawhub package validate <path-to-plugin>`。
...
- 如果 `openclaw.plugin.json` 也有 `version`，請更新它以相符，或在套件中繼資料具有權威性時移除過時的清單版本中繼資料。
- 變更已發布中繼資料後，發布新的套件版本。
...
- 讓 `name` 易於人類閱讀，並讓 `id` 作為穩定的機器 ID。
- 請參閱 [外掛清單](/zh-TW/plugins/manifest)。
- 重新執行 `clawhub package validate <path-to-plugin>`。
...
- 使用[匯入慣例](/zh-TW/plugins/building-plugins#import-conventions)和
  [外掛 SDK 子路徑](/zh-TW/plugins/sdk-subpaths)來找到範圍更窄的匯入。
- 重新執行 `clawhub package validate <path-to-plugin>`。
...
外掛仍使用已淘汰的整個 session store 輔助工具
`loadSessionStore`。
...
- 將提示或 context 變更工作移至 `before_prompt_build`。
- 只有在你宣告的相容性範圍仍支援需要它的舊版 OpenClaw 時，
...
- 讓頻道環境變數中繼資料保持宣告式，讓 OpenClaw 可以在不載入頻道執行階段的情況下檢查 setup 狀態。
- 將由環境變數驅動的頻道 setup 鏡像到目前 setup、頻道 config，或
  你的外掛形態所使用的套件頻道中繼資料。
...
套件附帶的 `openclaw.security.json` 含有 ClawHub 無法識別為可用的 schema 參照。

- 如果 schema URL 只是建議性質，請移除它。
- 只有在 OpenClaw 發布已記錄的版本化 schema 之後，才使用它。
- 重新執行 `clawhub package validate <path-to-plugin>`。

### unrecognized-security-manifest

套件附帶不支援的安全性 manifest 檔案。

- 在 OpenClaw 記錄版本化安全性 manifest schema 與 ClawHub 行為之前，
  請移除 `openclaw.security.json`。
- 在 manifest contract 存在之前，請將安全性敏感行為記錄在你的公開套件文件或
  README 中。
- 重新執行 `clawhub package validate <path-to-plugin>`。
