---
read_when:
    - 你執行了 `clawhub package validate`，且需要修正外掛發現的問題
    - ClawHub 拒絕或警告外掛套件發布
    - 你正在發行前更新外掛套件中繼資料
...
ClawHub 會在發布前驗證外掛套件，也可以顯示來自自動化套件掃描的發現。本頁涵蓋作者可見的發現，也就是外掛作者可在其套件中繼資料、資訊清單、SDK 匯入或已發布成品中修正的發現。

它不涵蓋內部 Plugin Inspector 覆蓋率發現。如果完整報告包含沒有作者補救指引的掃描器維護程式碼，這些是給 OpenClaw 維護者而不是外掛作者使用。
...
- 使用[建置外掛](/zh-TW/plugins/building-plugins)取得最小套件範例，並使用[外掛資訊清單](/zh-TW/plugins/manifest#manifest-versus-packagejson)了解套件與資訊清單的分工。
...
- 使用 `openclaw.install.minHostVersion` 設定最低 OpenClaw 主機版本。
- 請參閱[影響探索的 package.json 欄位](/zh-TW/plugins/manifest#packagejson-fields-that-affect-discovery)。
- 重新執行 `clawhub package validate <path-to-plugin>`。
...
- 使用您建置並測試所依據的 OpenClaw 外掛 API 版本或 semver 下限。
- 將這與套件版本分開。套件版本描述外掛發布；`openclaw.compat.pluginApi` 描述主機 API 合約。
- 請參閱[影響探索的 package.json 欄位](/zh-TW/plugins/manifest#packagejson-fields-that-affect-discovery)。
- 重新執行 `clawhub package validate <path-to-plugin>`。
...
- 如果 `openclaw.plugin.json` 也有 `version`，請更新它以相符；如果套件中繼資料具有權威性，則移除過時的資訊清單版本中繼資料。
- 變更已發布中繼資料後，發布新的套件版本。
...
- 如果此套件 intended for public publishing，請移除 `private: true`。
- 重新執行 `clawhub package validate <path-to-plugin>`。
...
- 封裝前建置產生的進入點。
- 更新 `files`、`.npmignore` 或建置輸出，使已宣告的進入點被包含。
- 請參閱[外掛進入點](/zh-TW/plugins/sdk-entrypoints)。
- 重新執行 `clawhub package validate <path-to-plugin>`。
...
- 更新 `files` 或 `.npmignore`，使套件中繼資料不會被排除。
...
- 使用 `openclaw/plugin-sdk/channel-core` 取得通道進入點輔助工具。
- 使用[匯入慣例](/zh-TW/plugins/building-plugins#import-conventions)和
  [外掛 SDK 子路徑](/zh-TW/plugins/sdk-subpaths)尋找狹窄匯入。
- 重新執行 `clawhub package validate <path-to-plugin>`。
...
- 當程式碼正在準備 transcript 操作時，使用 transcript 身分或目標輔助工具。
...
外掛仍在使用已棄用的 transcript 檔案目標輔助工具
...
外掛仍在使用已棄用的低階 transcript 輔助工具，例如
...
- 優先使用結構化 transcript 執行階段介面，讓 OpenClaw 能套用正確的交易邊界與身分處理。
- 只有在你宣告的相容性範圍仍支援需要它們的舊版 OpenClaw 時，
  才保留低階 transcript 輔助工具。
- 請參閱[執行階段 API](/zh-TW/plugins/sdk-runtime#agent-session-state)和
  [外掛 SDK 子路徑](/zh-TW/plugins/sdk-subpaths)。
- 重新執行 `clawhub package validate <path-to-plugin>`。
...
- 將模型或 provider 覆寫工作移到 `before_model_resolve`。
...
資訊清單仍在使用舊版 `providerAuthEnvVars` provider 驗證中繼資料。

- 將 provider env-var 中繼資料鏡像到 `setup.providers[].envVars`。
...
資訊清單使用舊版或較舊的通道 env-var 中繼資料，缺少 ClawHub 預期的目前
setup 或 config 中繼資料。

- 保持通道 env-var 中繼資料為宣告式，讓 OpenClaw 可以在不載入通道執行階段的情況下檢查 setup 狀態。
- 將 env 驅動的通道 setup 鏡像到你的外掛形態所使用的目前 setup、通道 config 或
  套件通道中繼資料。
...
套件隨附的 `openclaw.security.json` 包含 ClawHub 不認定為可用的 schema 參照。

- 如果 schema URL 僅供參考，請將其移除。
...
- 移除 `openclaw.security.json`，直到 OpenClaw 文件化版本化安全性
  資訊清單 schema 和 ClawHub 行為。
- 在資訊清單 contract 存在之前，請將安全性敏感行為記錄在你的公開套件文件或
  README 中。
- 重新執行 `clawhub package validate <path-to-plugin>`。
