---
read_when:
    - 你執行了 clawhub package validate，並需要修正外掛檢查發現事項
...
本頁不涵蓋內部 Plugin Inspector 覆蓋率發現事項。如果完整報告包含掃描器維護代碼，且沒有作者補救指引，這些代碼是給 OpenClaw 維護者使用，而不是給外掛作者使用。
...
| `security-manifest-schema-unavailable`  | [移除不可用的安全資訊清單架構參照](/zh-TW/clawhub/plugin-validation-fixes#security-manifest-schema-unavailable) |
...
- 這應與套件版本分開。套件版本描述外掛發布；`openclaw.compat.pluginApi` 描述主機 API 合約。
- 請參閱[影響探索的 package.json 欄位](/zh-TW/plugins/manifest#packagejson-fields-that-affect-discovery)。
- 重新執行 `clawhub package validate <path-to-plugin>`。
...
封裝後的成品缺少存在於原始碼套件中的 OpenClaw 中繼資料。

- 執行 `npm pack --dry-run`，並檢查包含的中繼資料檔案。
- 確保封裝後的成品中，`package.json` 包含 `openclaw` 區塊。
...
- 保持 `name` 可供人閱讀，並將 `id` 保持為穩定的機器識別碼。
- 請參閱[外掛資訊清單](/zh-TW/plugins/manifest)。
- 重新執行 `clawhub package validate <path-to-plugin>`。
...
- 移除不支援的 contract 鍵。
- 將執行階段行為移到外掛註冊程式碼中，並將 `contracts`
  限定為靜態功能擁有權中繼資料。
- 重新執行 `clawhub package validate <path-to-plugin>`。
...
- 使用[匯入慣例](/zh-TW/plugins/building-plugins#import-conventions)和
  [外掛 SDK 子路徑](/zh-TW/plugins/sdk-subpaths)尋找窄範圍匯入。
- 重新執行 `clawhub package validate <path-to-plugin>`。
...
外掛仍在使用已棄用的 whole-session-store 輔助工具
`loadSessionStore`。
...
外掛仍在使用已棄用的 whole-session-store 寫入輔助工具，例如
...
- 偏好結構化的 transcript 執行階段介面，讓 OpenClaw 可以套用正確的
  transaction 邊界與身分處理。
- 只有在你宣告的相容性範圍仍支援需要它們的舊版 OpenClaw 時，
...
- 將由 env 驅動的頻道 setup 鏡像到目前 setup、頻道 config，或
  你的外掛形態所使用的套件頻道中繼資料。
...
套件隨附的 `openclaw.security.json` 包含 ClawHub 不認可為可用的 schema 參照。
