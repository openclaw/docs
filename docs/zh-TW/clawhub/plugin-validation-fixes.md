...
read_when:
    ...
    - 你正在發布前更新外掛套件中繼資料
summary: 修正 ClawHub 外掛套件驗證發現的問題後再發布
title: 外掛驗證修正
x-i18n:
...
ClawHub 會在發布前驗證外掛套件，也能顯示自動套件掃描的發現項目。本頁涵蓋作者面向的發現項目，也就是外掛作者可以在其套件中繼資料、資訊清單、SDK 匯入或已發布成品中修正的發現項目。

它不涵蓋內部 Plugin Inspector 覆蓋率發現項目。如果完整報告包含沒有作者補救指引的掃描器維護代碼，這些是給 OpenClaw 維護者，而不是外掛作者。
...
套件最低主機版本與套件建置時所依據的 OpenClaw 版本中繼資料不相符。
...
已封裝成品缺少存在於原始碼套件中的 OpenClaw 中繼資料。
...
- 保持 `name` 可供人閱讀，並將 `id` 保持為穩定的機器 ID。
- 請參閱 [外掛資訊清單](/zh-TW/plugins/manifest)。
- 重新執行 `clawhub package validate <path-to-plugin>`。
...
- 使用[匯入慣例](/zh-TW/plugins/building-plugins#import-conventions)和
  [外掛 SDK 子路徑](/zh-TW/plugins/sdk-subpaths)尋找更精準的匯入。
- 重新執行 `clawhub package validate <path-to-plugin>`。
...
外掛仍在使用已棄用的完整 session store 輔助工具
`loadSessionStore`。
...
- 將 prompt 或 context 修改工作移到 `before_prompt_build`。
- 只有在你宣告的相容性範圍仍支援需要它的舊版 OpenClaw 時，
...
manifest 使用舊版或較舊的頻道 env-var 中繼資料，缺少 ClawHub 預期的目前
setup 或 config 中繼資料。

- 保持頻道 env-var 中繼資料的宣告式格式，讓 OpenClaw 無需載入頻道執行階段
  即可檢查 setup 狀態。
- 將 env 驅動的頻道 setup 鏡像到你的外掛形態所使用的目前 setup、頻道 config，
  或套件頻道中繼資料。
...
套件隨附的 `openclaw.security.json` 使用了 ClawHub 不認得為可用的 schema 參照。
