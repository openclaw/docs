---
read_when:
    - 你已執行 clawhub package validate，且需要修正外掛發現項。
...
套件宣告了 OpenClaw 進入點，但驗證中的套件缺少參照的檔案。
...
- 將它與套件版本分開。套件版本描述外掛版本；`openclaw.compat.pluginApi` 描述主機 API 合約。
- 請參閱 [影響探索的 package.json 欄位](/zh-TW/plugins/manifest#packagejson-fields-that-affect-discovery)。
- 重新執行 `clawhub package validate <path-to-plugin>`。
...
套件可以封裝，但已封裝成品未包含 `package.json#openclaw` 中宣告的進入點檔案。

- 執行 `npm pack --dry-run` 並檢查將包含的檔案。
- 在封裝前建置產生的進入點。
- 更新 `files`、`.npmignore` 或建置輸出，使宣告的進入點包含在內。
- 請參閱 [外掛進入點](/zh-TW/plugins/sdk-entrypoints)。
- 重新執行 `clawhub package validate <path-to-plugin>`。

### package-npm-pack-metadata-missing

已封裝成品缺少來源套件中存在的 OpenClaw 中繼資料。

- 執行 `npm pack --dry-run` 並檢查包含的中繼資料檔案。
- 確保 `package.json` 在已封裝成品中包含 `openclaw` 區塊。
...
- 使用[匯入慣例](/zh-TW/plugins/building-plugins#import-conventions)和
  [外掛 SDK 子路徑](/zh-TW/plugins/sdk-subpaths)來尋找較窄的匯入。
- 重新執行 `clawhub package validate <path-to-plugin>`。
...
- 優先使用結構化轉錄執行階段介面，讓 OpenClaw 可以套用正確的交易邊界和身分處理。
- 只有在你宣告的相容性範圍仍支援需要它們的舊版 OpenClaw 時，
  才保留低階轉錄輔助工具。
- 請參閱[執行階段 API](/zh-TW/plugins/sdk-runtime#agent-session-state)和
  [外掛 SDK 子路徑](/zh-TW/plugins/sdk-subpaths)。
...
清單使用舊版或較舊的頻道環境變數中繼資料，缺少 ClawHub 預期的目前設定或組態中繼資料。

- 讓頻道環境變數中繼資料保持宣告式，讓 OpenClaw 可以在不載入頻道執行階段的情況下檢查設定狀態。
- 將環境變數驅動的頻道設定鏡像到你的外掛形態所使用的目前設定、頻道組態或套件頻道中繼資料。
- 只有在較舊但仍支援的 OpenClaw 版本仍需要它時，才將 `channelEnvVars` 保留為相容性中繼資料。
...
套件隨附 `openclaw.security.json`，但其中的結構描述參考未被 ClawHub 識別為可用。

- 如果結構描述 URL 僅供建議用途，請移除它。
- 只有在 OpenClaw 發布已記錄的版本化結構描述後，才使用該結構描述。
- 重新執行 `clawhub package validate <path-to-plugin>`。
...
- 在 OpenClaw 記錄版本化安全性清單結構描述和 ClawHub 行為之前，請移除 `openclaw.security.json`。
- 在清單合約存在之前，請將安全性敏感行為記錄在你的公開套件文件或
  README 中。
- 重新執行 `clawhub package validate <path-to-plugin>`。
