---
read_when:
    - 你執行了 `clawhub package validate`，需要修正外掛發現項。
    - ClawHub 拒絕或警告外掛套件發布
    - 你正在發行前更新外掛套件中繼資料
summary: 修正發布前的 ClawHub 外掛套件驗證發現
title: 外掛驗證修正
x-i18n:
    generated_at: "2026-07-05T04:53:59Z"
    model: gpt-5.5
...
ClawHub 會在發布前驗證外掛套件，也可以顯示自動化套件掃描的發現項目。此頁涵蓋面向作者的發現項目，也就是外掛作者可在其套件中繼資料、清單、SDK 匯入或已發布成品中修正的項目。

它不涵蓋內部 Plugin Inspector 覆蓋率發現項目。如果完整報告包含沒有作者補救指引的掃描器維護代碼，這些項目是給 OpenClaw 維護者，而不是外掛作者。
...
| `security-manifest-schema-unavailable`  | [移除不可用的安全性清單結構描述參照](/zh-TW/clawhub/plugin-validation-fixes#security-manifest-schema-unavailable) |
...
- 讓它與套件版本分開。套件版本描述外掛發布；`openclaw.compat.pluginApi` 描述主機 API 合約。
- 請參閱 [影響探索的 package.json 欄位](/zh-TW/plugins/manifest#packagejson-fields-that-affect-discovery)。
- 重新執行 `clawhub package validate <path-to-plugin>`。
...
- 如果 `openclaw.plugin.json` 也有 `version`，請更新它以相符，或在套件中繼資料具權威性時移除過時的清單版本中繼資料。
- 變更已發布中繼資料後，發布新的套件版本。
...
- 在 `openclaw.plugin.json` 加入非空的 `name` 欄位。
- 保持 `name` 可供人閱讀，並保持 `id` 作為穩定的機器識別碼。
...
- 使用 `getSessionEntry(...)` 依代理與工作階段身分讀取工作階段中繼資料。
- 使用 `patchSessionEntry(...)` 或 `upsertSessionEntry(...)` 持久保存工作階段中繼資料。
...
- 使用 `appendSessionTranscriptMessageByIdentity(...)` 追加逐字稿。
- 使用 `publishSessionTranscriptUpdateByIdentity(...)` 發布逐字稿更新通知。
- 優先使用結構化逐字稿執行階段介面，讓 OpenClaw 可以套用正確的
  交易邊界與身分處理。
- 只有在你宣告的相容性範圍仍支援需要它們的舊版 OpenClaw 時，
  才保留低階逐字稿輔助工具。
- 請參閱[執行階段 API](/zh-TW/plugins/sdk-runtime#agent-session-state)和
...
- 保持頻道環境變數中繼資料為宣告式，讓 OpenClaw 可在不載入頻道執行階段的情況下檢查設定狀態。
- 將環境變數驅動的頻道設定鏡像到你的外掛形態所使用的目前設定、頻道設定，或
  套件頻道中繼資料。
...
套件隨附的 `openclaw.security.json` 含有 ClawHub 不識別為可用的 schema 參照。

- 如果 schema URL 僅作為建議，請移除它。
