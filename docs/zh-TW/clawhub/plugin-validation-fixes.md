...
title: 外掛驗證修正
x-i18n:
    generated_at: "2026-07-04T17:48:12Z"
    model: gpt-5.5
...
ClawHub 會在發布前驗證外掛套件，也可以顯示自動化套件掃描的發現項目。本頁涵蓋作者可見的發現項目，也就是外掛作者可以在其套件中繼資料、資訊清單、SDK 匯入或已發布成品中修正的發現項目。

本頁不涵蓋內部 Plugin Inspector 覆蓋率發現項目。如果完整報告包含沒有作者修復指引的掃描器維護代碼，這些項目是給 OpenClaw 維護者處理，而不是給外掛作者處理。
...
| `package-npm-pack-unavailable`          | [讓 npm 成品可被封裝](/zh-TW/clawhub/plugin-validation-fixes#package-npm-pack-unavailable)                                 |
...
外掛從已棄用的根層 SDK 統一匯出入口匯入：
`openclaw/plugin-sdk`。

- 將根層統一匯出入口匯入取代為聚焦的公開子路徑匯入。
...
- 使用[匯入慣例](/zh-TW/plugins/building-plugins#import-conventions)和
  [外掛 SDK 子路徑](/zh-TW/plugins/sdk-subpaths)來找到精確的匯入。
- 重新執行 `clawhub package validate <path-to-plugin>`。
...
清單仍使用舊版 `providerAuthEnvVars` 供應商驗證中繼資料。
...
清單使用舊版或較舊的頻道環境變數中繼資料，缺少 ClawHub 預期的目前
setup 或設定中繼資料。

- 讓頻道環境變數中繼資料保持宣告式，使 OpenClaw 能在不載入頻道執行階段的情況下檢查 setup 狀態。
- 將環境變數驅動的頻道 setup 鏡像至目前 setup、頻道設定，或
  你的外掛形態所使用的套件頻道中繼資料。
...
- 在 OpenClaw 文件化版本化安全性清單 schema 與 ClawHub 行為之前，
