...
title: 外掛驗證修正
x-i18n:
    generated_at: "2026-07-02T22:21:56Z"
    model: gpt-5.5
...
ClawHub 會在發布前驗證外掛套件，也可以顯示自動化套件掃描的發現項目。本頁涵蓋作者面向的發現項目，也就是外掛作者可以在其套件中繼資料、資訊清單、SDK 匯入或已發布成品中修正的發現項目。

本頁不涵蓋內部 Plugin Inspector 覆蓋率發現項目。如果完整報告包含沒有作者修復指引的掃描器維護代碼，這些是給 OpenClaw 維護者看的，而不是給外掛作者看的。
...
套件宣告了 OpenClaw 進入點，但正在驗證的套件缺少所參照的檔案。
...
- 優先將 `package.json#version` 作為套件發行版本。
- 如果 `openclaw.plugin.json` 也有 `version`，請將其更新為相符，或在套件中繼資料具權威性時移除過時的資訊清單版本中繼資料。
...
- 保持 `name` 為人類可讀，並將 `id` 保持為穩定的機器 ID。
- 請參閱[外掛資訊清單](/zh-TW/plugins/manifest)。
- 重新執行 `clawhub package validate <path-to-plugin>`。
...
- 使用[匯入慣例](/zh-TW/plugins/building-plugins#import-conventions)和
  [外掛 SDK 子路徑](/zh-TW/plugins/sdk-subpaths)尋找精準的匯入。
- 重新執行 `clawhub package validate <path-to-plugin>`。
...
外掛仍在使用已棄用的逐字稿檔案目標輔助函式
`resolveSessionTranscriptLegacyFileTarget`。

- 當程式碼只需要公開工作階段身分時，使用 `resolveSessionTranscriptIdentity(...)`。
- 當程式碼需要結構化的逐字稿作業目標時，使用 `resolveSessionTranscriptTarget(...)`。
- 避免直接讀取或建構舊版逐字稿檔案目標。
- 僅在你宣告的相容性範圍仍支援需要它的較舊 OpenClaw 版本時，
  才保留舊版輔助函式。
- 請參閱[執行階段 API](/zh-TW/plugins/sdk-runtime#agent-session-state)和
...
外掛仍在使用已棄用的低階逐字稿輔助函式，例如
`appendSessionTranscriptMessage` 或 `emitSessionTranscriptUpdate`。

- 使用 `appendSessionTranscriptMessageByIdentity(...)` 進行逐字稿追加。
...
- 保持頻道環境變數中繼資料為宣告式，讓 OpenClaw 能在不載入頻道執行階段的情況下
  檢查 setup 狀態。
- 將環境變數驅動的頻道 setup 鏡像到你的外掛形態使用的目前 setup、頻道設定或
  套件頻道中繼資料。
- 僅在較舊且仍受支援的 OpenClaw 版本仍需要它時，才將 `channelEnvVars`
  保留為相容性中繼資料。
...
- 僅在 OpenClaw 發布文件化的版本化結構描述後，才使用它。
- 重新執行 `clawhub package validate <path-to-plugin>`。
