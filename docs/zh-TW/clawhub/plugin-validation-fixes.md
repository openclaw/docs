...
read_when:
    ...
    - 你正在發布前更新外掛套件中繼資料
summary: 修正 ClawHub 外掛套件驗證發現的問題，再發布
title: 外掛驗證修正
x-i18n:
    generated_at: "2026-07-06T21:47:05Z"
    model: gpt-5.5
...
ClawHub 會在發布前驗證外掛套件，也可以顯示來自自動化套件掃描的發現。本頁涵蓋面向作者的發現，也就是外掛作者可以在其套件中繼資料、資訊清單、SDK 匯入或已發布成品中修正的發現。

本頁不涵蓋內部 Plugin Inspector 覆蓋率發現。如果完整報告包含沒有作者補救指引的掃描器維護代碼，這些是給 OpenClaw 維護者，而不是外掛作者。
...
- 將其與套件版本分開。套件版本描述外掛發行；`openclaw.compat.pluginApi` 描述主機 API 合約。
- 請參閱 [影響探索的 package.json 欄位](/zh-TW/plugins/manifest#packagejson-fields-that-affect-discovery)。
- 重新執行 `clawhub package validate <path-to-plugin>`。
...
- 保持 `name` 易於人類閱讀，並保持 `id` 作為穩定的機器識別碼。
- 請參閱[外掛資訊清單](/zh-TW/plugins/manifest)。
- 重新執行 `clawhub package validate <path-to-plugin>`。
...
- 使用[匯入慣例](/zh-TW/plugins/building-plugins#import-conventions)和
  [外掛 SDK 子路徑](/zh-TW/plugins/sdk-subpaths)來找到範圍較窄的匯入。
- 重新執行 `clawhub package validate <path-to-plugin>`。
...
外掛仍使用已淘汰的 whole-session-store 輔助工具
`loadSessionStore`。
...
外掛仍使用已淘汰的 whole-session-store 寫入輔助工具，例如
...
外掛仍使用已淘汰的低階 transcript 輔助工具，例如
...
資訊清單仍使用舊版 `providerAuthEnvVars` 提供者 auth 中繼資料。

- 將提供者 env-var 中繼資料鏡像到 `setup.providers[].envVars`。
...
資訊清單使用舊版或較舊的頻道 env-var 中繼資料，沒有 ClawHub 預期的目前 setup 或 config 中繼資料。

- 保持頻道 env-var 中繼資料為宣告式，讓 OpenClaw 能在不載入頻道執行階段的情況下檢查 setup 狀態。
- 將 env 驅動的頻道 setup 鏡像到目前 setup、頻道 config，或你的外掛形態所使用的套件頻道中繼資料。
...
- 在 OpenClaw 記錄版本化安全性資訊清單 schema 與 ClawHub 行為前，移除 `openclaw.security.json`。
- 在資訊清單契約存在前，將安全性敏感行為記錄在你的公開套件文件或
  README 中。
- 重新執行 `clawhub package validate <path-to-plugin>`。
