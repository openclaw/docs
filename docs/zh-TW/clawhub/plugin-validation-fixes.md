...
read_when:
    ...
    - 你正在發行前更新外掛套件中繼資料
summary: 發布前修正 ClawHub 外掛套件的驗證問題
title: 外掛驗證修正
x-i18n:
    generated_at: "2026-07-14T13:30:20Z"
    model: gpt-5.6
...
ClawHub 會在發布前驗證外掛套件，也能顯示自動化套件掃描的發現。此頁涵蓋面向作者的發現，也就是外掛作者可透過修正套件中繼資料、資訊清單、SDK 匯入或已發布成品來解決的發現。

此頁不涵蓋 Plugin Inspector 的內部涵蓋範圍發現。如果完整報告包含沒有作者修正指引的掃描器維護代碼，這些代碼是供 OpenClaw 維護者處理，而非外掛作者。
...
- 當套件包含 OpenClaw 外掛時，新增 `openclaw` 區塊。
- 請參閱[建置外掛](/zh-TW/plugins/building-plugins)中的最小套件範例，以及[外掛資訊清單](/zh-TW/plugins/manifest#manifest-versus-packagejson)中套件與資訊清單的職責劃分。
...
- 如果套件將透過 ClawHub 發布或安裝，請新增相容性與安裝中繼資料。
...
- 如果有多個安裝來源可用，請設定 `openclaw.install.defaultChoice`。
...
- 使用建置及測試時所依據的 OpenClaw 外掛 API 版本或語意化版本下限。
- 請將此資訊與套件版本分開。套件版本描述外掛版本；`openclaw.compat.pluginApi` 描述主機 API 契約。
- 請參閱[影響探索的 package.json 欄位](/zh-TW/plugins/manifest#packagejson-fields-that-affect-discovery)。
- 重新執行 `clawhub package validate <path-to-plugin>`。

### package-min-host-version-drift

套件的最低主機版本與建置該套件時所依據的 OpenClaw 版本中繼資料不一致。

- 檢查 `openclaw.install.minHostVersion`。
- 檢查套件中的所有 OpenClaw 建置中繼資料，例如發布時使用的 OpenClaw 版本。
- 讓最低主機版本與套件實際支援的主機版本範圍一致。
- 請參閱[影響探索的 package.json 欄位](/zh-TW/plugins/manifest#packagejson-fields-that-affect-discovery)。
- 重新執行 `clawhub package validate <path-to-plugin>`。
...
- 如果 `openclaw.plugin.json` 也具有 `version`，請更新它以保持一致；若套件中繼資料是權威來源，則移除過時的資訊清單版本中繼資料。
...
- 確保 `name` 便於人類閱讀，並將 `id` 保持為穩定的機器識別碼。
- 請參閱[外掛資訊清單](/zh-TW/plugins/manifest)。
- 重新執行 `clawhub package validate <path-to-plugin>`。
...
- 將套件或安裝中繼資料移至受支援的 `package.json#openclaw` 欄位，而非放在資訊清單中。
- 重新執行 `clawhub package validate <path-to-plugin>`。
...
- 將 `contracts` 下的每個鍵與[契約參考](/zh-TW/plugins/manifest#contracts-reference)進行比較。
- 移除不受支援的契約鍵。
- 將執行階段行為移至外掛註冊程式碼，並將 `contracts` 限制為靜態功能擁有權中繼資料。
- 重新執行 `clawhub package validate <path-to-plugin>`。
...
- 使用[匯入慣例](/zh-TW/plugins/building-plugins#import-conventions)與[外掛 SDK 子路徑](/zh-TW/plugins/sdk-subpaths)尋找範圍精確的匯入。
- 重新執行 `clawhub package validate <path-to-plugin>`。
...
- 如果此行為沒有公開 SDK，請將輔助函式保留在你的套件中，或要求提供公開的 OpenClaw API。
...
- 僅在你宣告的相容範圍仍支援需要 `loadSessionStore(...)` 的舊版 OpenClaw 時，才保留它。
...
- 避免載入、修改並儲存整個工作階段儲存區物件。
- 僅在你宣告的相容範圍仍支援需要完整儲存區寫入輔助函式的舊版 OpenClaw 時，才保留這些函式。
- 請參閱[執行階段 API](/zh-TW/plugins/sdk-runtime#agent-session-state)與[外掛 SDK 子路徑](/zh-TW/plugins/sdk-subpaths)。
- 重新執行 `clawhub package validate <path-to-plugin>`。
...
- 避免直接讀取或建構舊版逐字稿檔案目標。
- 僅在你宣告的相容範圍仍支援需要舊版輔助函式的舊版 OpenClaw 時，才保留該函式。
- 請參閱[執行階段 API](/zh-TW/plugins/sdk-runtime#agent-session-state)與[外掛 SDK 子路徑](/zh-TW/plugins/sdk-subpaths)。
- 重新執行 `clawhub package validate <path-to-plugin>`。
...
- 僅在你宣告的相容範圍仍支援需要低階逐字稿輔助函式的舊版 OpenClaw 時，才保留這些函式。
- 請參閱[執行階段 API](/zh-TW/plugins/sdk-runtime#agent-session-state)與[外掛 SDK 子路徑](/zh-TW/plugins/sdk-subpaths)。
- 重新執行 `clawhub package validate <path-to-plugin>`。
...
- 僅在你宣告的相容範圍仍支援需要 `before_agent_start` 的舊版 OpenClaw 時，才保留它。
...
資訊清單使用舊版或較舊的頻道環境變數中繼資料，但未包含 ClawHub 預期的目前設定或組態中繼資料。

- 保持頻道環境變數中繼資料的宣告式形式，讓 OpenClaw 無須載入頻道執行階段即可檢查設定狀態。
- 將環境變數驅動的頻道設定同步至你的外掛形態所使用的目前設定、頻道組態或套件頻道中繼資料。
- 僅在支援的舊版 OpenClaw 仍需要 `channelEnvVars` 時，才將其保留為相容性中繼資料。
- 請參閱[外掛資訊清單](/zh-TW/plugins/manifest)與[頻道外掛](/zh-TW/plugins/sdk-channel-plugins)。
- 重新執行 `clawhub package validate <path-to-plugin>`。
...
套件隨附的 `openclaw.security.json` 含有 ClawHub 無法辨識為可用的結構描述參考。

- 如果結構描述 URL 僅供參考，請將其移除。
- 僅在 OpenClaw 發布有文件記載且已版本化的結構描述後，才使用該結構描述。
- 重新執行 `clawhub package validate <path-to-plugin>`。
...
- 在 OpenClaw 記載已版本化的安全性資訊清單結構描述與 ClawHub 行為之前，請移除 `openclaw.security.json`。
- 在資訊清單契約建立之前，請持續在公開套件文件或 README 中記載安全性敏感行為。
- 重新執行 `clawhub package validate <path-to-plugin>`。
