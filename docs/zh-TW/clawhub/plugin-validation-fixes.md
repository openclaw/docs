---
read_when:
    - 你已執行 `clawhub package validate`，需要修正外掛發現項目
    - ClawHub 拒絕或警告外掛套件發布
    - 你正在發布前更新外掛套件中繼資料
summary: 發布前修正 ClawHub 外掛套件驗證發現的問題
title: 外掛驗證修正
x-i18n:
    generated_at: "2026-07-01T05:27:55Z"
    model: gpt-5.5
...
ClawHub 會在發布前驗證外掛套件，也可以顯示自動化套件掃描的發現項目。本頁涵蓋面向作者的發現項目，也就是外掛作者可在其套件中繼資料、manifest、SDK 匯入或已發布成品中修正的發現項目。

本頁不涵蓋內部 Plugin Inspector 覆蓋率發現項目。如果完整報告包含沒有作者補救指引的掃描器維護代碼，這些代碼是給 OpenClaw 維護者使用，而不是給外掛作者使用。
...
| `provider-auth-env-vars`                | [將 provider 環境變數移至設定中繼資料](/zh-TW/clawhub/plugin-validation-fixes#provider-auth-env-vars)                             |
| `channel-env-vars`                      | [在目前中繼資料中鏡像 channel 環境變數](/zh-TW/clawhub/plugin-validation-fixes#channel-env-vars)                                |
| `security-manifest-schema-unavailable`  | [移除無法使用的安全 manifest schema 參照](/zh-TW/clawhub/plugin-validation-fixes#security-manifest-schema-unavailable) |
| `unrecognized-security-manifest`        | [移除不支援的安全 manifest 檔案](/zh-TW/clawhub/plugin-validation-fixes#unrecognized-security-manifest)                   |
...
套件宣告了 OpenClaw 進入點，但被參照的檔案在正在驗證的套件中缺失。
...
套件最低主機版本與套件建置所依據的 OpenClaw 版本中繼資料不相符。
...
- 修正無效的套件中繼資料、損壞的 lifecycle script，或會導致封裝失敗的 files 項目。
...
- 在 `openclaw.plugin.json` 中加入非空的 `name` 欄位。
...
- 將套件或安裝中繼資料移到支援的 `package.json#openclaw` 欄位，
  而不是放在資訊清單中。
- 重新執行 `clawhub package validate <path-to-plugin>`。
...
- 移除不支援的合約鍵。
- 將執行階段行為移到外掛註冊程式碼中，並讓 `contracts`
  僅限於靜態能力所有權中繼資料。
- 重新執行 `clawhub package validate <path-to-plugin>`。
...
- 使用[匯入慣例](/zh-TW/plugins/building-plugins#import-conventions)和
  [外掛 SDK 子路徑](/zh-TW/plugins/sdk-subpaths)來尋找精確的匯入。
- 重新執行 `clawhub package validate <path-to-plugin>`。
...
外掛仍在使用已棄用的整體工作階段儲存輔助工具
`loadSessionStore`。

- 讀取工作階段狀態時使用 `getSessionEntry(...)` 或 `listSessionEntries(...)`。
- 寫入工作階段狀態時使用 `patchSessionEntry(...)` 或 `upsertSessionEntry(...)`。
...
外掛仍在使用已棄用的整體工作階段儲存寫入輔助工具，例如
`saveSessionStore` 或 `updateSessionStore`。

- 更新現有工作階段項目的欄位時使用 `patchSessionEntry(...)`。
- 替換或建立工作階段項目時使用 `upsertSessionEntry(...)`。
- 避免載入、變更並儲存整個工作階段儲存物件。
...
外掛仍在使用已棄用的逐字稿檔案目標輔助工具
`resolveSessionTranscriptLegacyFileTarget`。

- 當程式碼只需要公開工作階段身分時，使用 `resolveSessionTranscriptIdentity(...)`。
- 當程式碼需要結構化逐字稿作業目標時，使用 `resolveSessionTranscriptTarget(...)`。
- 避免直接讀取或建構舊版逐字稿檔案目標。
- 只有在你宣告的相容性範圍仍支援需要它的較舊 OpenClaw 版本時，
  才保留舊版輔助工具。
- 請參閱[執行階段 API](/zh-TW/plugins/sdk-runtime#agent-session-state)和
...
- 優先使用結構化逐字稿執行階段介面，讓 OpenClaw 能套用正確的交易邊界和身分處理。
- 只有在你宣告的相容性範圍仍支援需要它們的較舊 OpenClaw 版本時，
  才保留低階逐字稿輔助工具。
- 請參閱[執行階段 API](/zh-TW/plugins/sdk-runtime#agent-session-state)和
  [外掛 SDK 子路徑](/zh-TW/plugins/sdk-subpaths)。
...
- 將模型或提供者覆寫工作移到 `before_model_resolve`。
- 將提示或脈絡變更工作移到 `before_prompt_build`。
- 只有在你宣告的相容性範圍仍支援需要它的較舊 OpenClaw 版本時，
...
資訊清單使用舊版或較舊的通道環境變數中繼資料，缺少 ClawHub 預期的目前設定或設定檔中繼資料。

- 讓通道環境變數中繼資料保持宣告式，讓 OpenClaw 能在不載入通道執行階段的情況下檢查設定狀態。
- 將環境變數驅動的通道設定鏡像到你的外掛形態所使用的目前 setup、通道設定檔或套件通道中繼資料。
- 只有在較舊且受支援的 OpenClaw 版本仍需要時，才將 `channelEnvVars` 保留為相容性中繼資料。
...
套件隨附的 `openclaw.security.json` 使用了 ClawHub 未識別為可用的 schema 參照。

- 如果 schema URL 只具建議性，請移除它。
- 只有在 OpenClaw 發布文件化的版本化 schema 後，才使用它。
- 重新執行 `clawhub package validate <path-to-plugin>`。
...
- 在 OpenClaw 文件化版本化安全性資訊清單 schema 和 ClawHub 行為之前，移除 `openclaw.security.json`。
- 在資訊清單合約存在之前，請將安全性敏感行為記錄於你的公開套件文件或 README 中。
- 重新執行 `clawhub package validate <path-to-plugin>`。
