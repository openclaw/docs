---
read_when:
    - 你已執行 clawhub package validate，需要修正外掛檢查結果
    - ClawHub 拒絕發布外掛套件或對其發出警告
    - 你正在發佈前更新外掛套件中繼資料
summary: 發布前修正 ClawHub 外掛套件驗證問題
title: 外掛驗證修正
x-i18n:
    generated_at: "2026-07-11T21:12:37Z"
    model: gpt-5.6
...
ClawHub 會在發布前驗證外掛套件，也能顯示自動化套件掃描的發現。本頁涵蓋面向作者的發現，也就是外掛作者可在套件中修正的套件中繼資料、資訊清單、SDK 匯入或已發布成品相關問題。

本頁不涵蓋 Plugin Inspector 的內部涵蓋範圍發現。若完整報告包含沒有提供作者修正指引的掃描器維護代碼，這些代碼是供 OpenClaw 維護者處理，而非外掛作者。
...
| `sdk-load-session-store`                | [取代整體工作階段儲存區存取](/zh-TW/clawhub/plugin-validation-fixes#sdk-load-session-store)                                   |
| `sdk-session-store-write`               | [取代整體工作階段儲存區寫入](/zh-TW/clawhub/plugin-validation-fixes#sdk-session-store-write)                                  |
...
| `channel-env-vars`                      | [在目前的中繼資料中同步通道環境變數](/zh-TW/clawhub/plugin-validation-fixes#channel-env-vars)                                |
...
- 套件提供 OpenClaw 外掛時，請新增 `openclaw` 區塊。
- 如需最小套件範例，請參閱[建置外掛](/zh-TW/plugins/building-plugins)；如需了解套件與資訊清單的職責區分，請參閱[外掛資訊清單](/zh-TW/plugins/manifest#manifest-versus-packagejson)。
...
- 若套件將透過 ClawHub 發布或安裝，請新增相容性與安裝中繼資料。
...
- 當已發布的套件應載入建置後的 JavaScript 時，請新增 `openclaw.runtimeExtensions`。
- 所有進入點路徑都必須位於套件目錄內。
- 請參閱[外掛進入點](/zh-TW/plugins/sdk-entrypoints)和[會影響探索的 package.json 欄位](/zh-TW/plugins/manifest#packagejson-fields-that-affect-discovery)。
- 重新執行 `clawhub package validate <path-to-plugin>`。
...
- 有多個可用的安裝來源時，請設定 `openclaw.install.defaultChoice`。
...
- 使用您建置及測試時所依據的 OpenClaw 外掛 API 版本或語意化版本下限。
- 請將此項與套件版本分開。套件版本描述外掛版本；`openclaw.compat.pluginApi` 描述主機 API 合約。
- 請參閱[會影響探索的 package.json 欄位](/zh-TW/plugins/manifest#packagejson-fields-that-affect-discovery)。
- 重新執行 `clawhub package validate <path-to-plugin>`。

### package-min-host-version-drift

套件的最低主機版本與套件建置時所依據的 OpenClaw 版本中繼資料不相符。
...
- 如果 `openclaw.plugin.json` 也有 `version`，請更新該值使其一致；若套件中繼資料具有權威性，則移除過時的資訊清單版本中繼資料。
...
- 將套件進入點、相容性、安裝、設定和目錄中繼資料保留在受支援的 `package.json#openclaw` 欄位中。
- 請參閱[會影響探索的 package.json 欄位](/zh-TW/plugins/manifest#packagejson-fields-that-affect-discovery)。
- 重新執行 `clawhub package validate <path-to-plugin>`。
...
套件可以封裝，但封裝後的成品不包含 `package.json#openclaw` 中宣告的進入點檔案。

- 執行 `npm pack --dry-run`，並檢查將包含的檔案。
- 在封裝前建置產生的進入點。
- 更新 `files`、`.npmignore` 或建置輸出，確保包含已宣告的進入點。
- 請參閱[外掛進入點](/zh-TW/plugins/sdk-entrypoints)。
- 重新執行 `clawhub package validate <path-to-plugin>`。
...
- 確保封裝後成品中的 `package.json` 包含 `openclaw` 區塊。
- 當套件是原生 OpenClaw 外掛時，請確保包含 `openclaw.plugin.json`。
...
- `name` 應保持易於人類閱讀，`id` 則應作為穩定的機器識別碼。
- 請參閱[外掛資訊清單](/zh-TW/plugins/manifest)。
- 重新執行 `clawhub package validate <path-to-plugin>`。
...
- 從 `openclaw.plugin.json` 中移除自訂欄位。
...
此外掛從已淘汰的根 SDK 匯出彙總入口匯入：
`openclaw/plugin-sdk`。

- 將根匯出彙總入口匯入替換為聚焦的公開子路徑匯入。
...
此外掛匯入了保留供內建外掛或內部相容性使用的 SDK 路徑。

- 將保留的 OpenClaw 內部 SDK 匯入替換為文件所列的公開
  `openclaw/plugin-sdk/*` 子路徑。
- 如果該行為沒有公開 SDK，請將輔助函式保留在您的套件中，或
  請求提供公開的 OpenClaw API。
...
此外掛仍在使用已淘汰的完整工作階段儲存區輔助函式
`loadSessionStore`。

- 讀取工作階段狀態時，請使用 `getSessionEntry(...)` 或 `listSessionEntries(...)`。
- 寫入工作階段狀態時，請使用 `patchSessionEntry(...)` 或 `upsertSessionEntry(...)`。
- 避免載入、修改及儲存整個工作階段儲存區物件。
- 僅當您宣告的相容性範圍仍支援需要此函式的舊版 OpenClaw 時，
...
此外掛仍在使用已淘汰的完整工作階段儲存區寫入輔助函式，例如
`saveSessionStore` 或 `updateSessionStore`。

- 更新現有工作階段項目的欄位時，請使用 `patchSessionEntry(...)`。
- 替換或建立工作階段項目時，請使用 `upsertSessionEntry(...)`。
- 避免載入、修改及儲存整個工作階段儲存區物件。
- 僅當您宣告的相容性範圍仍支援需要這些函式的舊版 OpenClaw 時，
  才保留完整儲存區寫入輔助函式。
- 請參閱[執行階段 API](/zh-TW/plugins/sdk-runtime#agent-session-state)和
  [外掛 SDK 子路徑](/zh-TW/plugins/sdk-subpaths)。
- 重新執行 `clawhub package validate <path-to-plugin>`。
...
- 當程式碼準備執行逐字稿操作時，請使用逐字稿身分或目標輔助函式。
- 請勿保存或依賴舊版逐字稿檔案路徑。
- 請參閱[執行階段 API](/zh-TW/plugins/sdk-runtime#agent-session-state)和
  [外掛 SDK 子路徑](/zh-TW/plugins/sdk-subpaths)。
- 重新執行 `clawhub package validate <path-to-plugin>`。
...
此外掛仍在使用已淘汰的逐字稿檔案目標輔助函式
`resolveSessionTranscriptLegacyFileTarget`。

- 當程式碼只需要公開工作階段身分時，請使用 `resolveSessionTranscriptIdentity(...)`。
- 當程式碼需要結構化逐字稿操作目標時，請使用 `resolveSessionTranscriptTarget(...)`。
- 避免直接讀取或建構舊版逐字稿檔案目標。
- 僅當您宣告的相容性範圍仍支援需要此函式的舊版 OpenClaw 時，
  才保留舊版輔助函式。
- 請參閱[執行階段 API](/zh-TW/plugins/sdk-runtime#agent-session-state)和
...
此外掛仍在使用已淘汰的低階逐字稿輔助函式，例如
`appendSessionTranscriptMessage` 或 `emitSessionTranscriptUpdate`。

- 使用 `appendSessionTranscriptMessageByIdentity(...)` 附加逐字稿內容。
- 使用 `publishSessionTranscriptUpdateByIdentity(...)` 傳送逐字稿更新
  通知。
- 優先使用結構化逐字稿執行階段介面，讓 OpenClaw 能套用正確的
  交易邊界與身分處理。
- 僅當您宣告的相容性範圍仍支援需要這些函式的舊版 OpenClaw 時，
  才保留低階逐字稿輔助函式。
- 請參閱[執行階段 API](/zh-TW/plugins/sdk-runtime#agent-session-state)和
...
- 僅當您宣告的相容性範圍仍支援需要此掛鉤的舊版 OpenClaw 時，
  才保留 `before_agent_start`。
- 請參閱[掛鉤](/zh-TW/plugins/hooks)和
...
- 僅當您支援的 OpenClaw 版本範圍仍需要時，才保留 `providerAuthEnvVars`
  作為相容性中繼資料。
...
資訊清單使用了舊版或較舊的頻道環境變數中繼資料，但未包含 ClawHub
預期的目前設定或組態中繼資料。

- 讓頻道環境變數中繼資料保持宣告式，使 OpenClaw 無須載入頻道執行階段
  即可檢查設定狀態。
- 將以環境變數驅動的頻道設定同步至您的外掛形式所使用的目前設定、
  頻道組態或套件頻道中繼資料。
- 僅當支援的舊版 OpenClaw 仍需要時，才保留 `channelEnvVars`
  作為相容性中繼資料。
...
套件提供的 `openclaw.security.json` 包含 ClawHub 無法辨識為可用的
結構描述參考。

- 如果結構描述 URL 僅供參考，請將其移除。
- 僅在 OpenClaw 發布有文件說明的版本化結構描述後才使用它。
- 重新執行 `clawhub package validate <path-to-plugin>`。
...
- 在 OpenClaw 提供版本化安全性資訊清單結構描述及 ClawHub 行為的文件前，
  請移除 `openclaw.security.json`。
- 在資訊清單合約建立前，請將安全性敏感行為記錄於公開套件文件或
  README 中。
- 重新執行 `clawhub package validate <path-to-plugin>`。
