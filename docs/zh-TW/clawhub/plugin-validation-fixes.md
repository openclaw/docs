...
read_when:
    ...
    - 你正在發布前更新外掛套件中繼資料
summary: 修正 ClawHub 外掛套件驗證發現的問題再發布
title: 外掛驗證修正
x-i18n:
    generated_at: "2026-07-03T17:13:08Z"
    model: gpt-5.5
...
ClawHub 會在發布前驗證外掛套件，也可以顯示自動化套件掃描的發現項目。此頁涵蓋面向作者的發現項目，也就是外掛作者可以在其套件中繼資料、資訊清單、SDK 匯入或已發布成品中修正的發現項目。

它不涵蓋內部外掛檢查器覆蓋率發現項目。如果完整報告包含掃描器維護代碼，且沒有作者修復指引，這些是給 OpenClaw 維護者看的，而不是外掛作者。
...
| `package-npm-pack-entrypoint-missing`   | [在 npm 打包輸出中包含進入點](/zh-TW/clawhub/plugin-validation-fixes#package-npm-pack-entrypoint-missing)                  |
| `package-npm-pack-metadata-missing`     | [在 npm 打包輸出中包含中繼資料](/zh-TW/clawhub/plugin-validation-fixes#package-npm-pack-metadata-missing)                       |
...
| `security-manifest-schema-unavailable`  | [移除不可用的安全資訊清單結構描述參照](/zh-TW/clawhub/plugin-validation-fixes#security-manifest-schema-unavailable) |
...
- 當可用的安裝來源超過一個時，設定 `openclaw.install.defaultChoice`。
...
- 偏好以 `package.json#version` 作為套件發布版本。
- 如果 `openclaw.plugin.json` 也有 `version`，請更新它以符合套件版本，或在套件中繼資料具權威性時移除過時的資訊清單版本中繼資料。
- 變更已發布中繼資料後，發布新的套件版本。
...
- 修正無效的套件中繼資料、損壞的生命週期指令碼，或會導致打包失敗的 files 項目。
...
- 將根匯出彙整匯入取代為聚焦的公開子路徑匯入。
- 針對 `definePluginEntry` 使用 `openclaw/plugin-sdk/plugin-entry`。
- 針對頻道進入點輔助工具使用 `openclaw/plugin-sdk/channel-core`。
...
- 將保留的 OpenClaw 內部 SDK 匯入取代為文件化的公開
...
- 當程式碼正在準備文字記錄操作時，使用文字記錄身分或目標輔助工具。
- 不要保存或依賴舊版文字記錄檔案路徑。
- 請參閱[執行階段 API](/zh-TW/plugins/sdk-runtime#agent-session-state) 與
  [外掛 SDK 子路徑](/zh-TW/plugins/sdk-subpaths)。
- 重新執行 `clawhub package validate <path-to-plugin>`。
...
外掛仍使用已淘汰的文字記錄檔案目標輔助工具
`resolveSessionTranscriptLegacyFileTarget`。

- 當程式碼只需要公開工作階段身分時，使用 `resolveSessionTranscriptIdentity(...)`。
- 當程式碼需要結構化文字記錄操作目標時，使用 `resolveSessionTranscriptTarget(...)`。
- 避免直接讀取或建構舊版文字記錄檔案目標。
- 只有在你宣告的相容性範圍仍支援需要它的較舊 OpenClaw 版本時，
  才保留舊版輔助工具。
- 請參閱[執行階段 API](/zh-TW/plugins/sdk-runtime#agent-session-state) 與
...
外掛仍使用已淘汰的低階文字記錄輔助工具，例如
`appendSessionTranscriptMessage` 或 `emitSessionTranscriptUpdate`。

- 使用 `appendSessionTranscriptMessageByIdentity(...)` 進行文字記錄附加。
- 使用 `publishSessionTranscriptUpdateByIdentity(...)` 發佈文字記錄更新通知。
- 優先使用結構化文字記錄執行階段介面，讓 OpenClaw 可以套用正確的交易邊界與身分處理。
- 只有在你宣告的相容性範圍仍支援需要它們的較舊 OpenClaw 版本時，
  才保留低階文字記錄輔助工具。
- 請參閱[執行階段 API](/zh-TW/plugins/sdk-runtime#agent-session-state) 與
  [外掛 SDK 子路徑](/zh-TW/plugins/sdk-subpaths)。
...
外掛仍使用舊版 `before_agent_start` 鉤子。

- 將模型或提供者覆寫工作移至 `before_model_resolve`。
- 將提示或情境變更工作移至 `before_prompt_build`。
- 只有在你宣告的相容性範圍仍支援需要它的較舊 OpenClaw 版本時，
  才保留 `before_agent_start`。
- 請參閱[鉤子](/zh-TW/plugins/hooks)與
...
- 只有在你支援的 OpenClaw 範圍仍需要它時，
  才將 `providerAuthEnvVars` 保留作為相容性中繼資料。
...
資訊清單使用舊版或較舊的頻道環境變數中繼資料，缺少 ClawHub 預期的目前設定或組態中繼資料。

- 讓頻道環境變數中繼資料保持宣告式，使 OpenClaw 可以在不載入頻道執行階段的情況下檢查設定狀態。
- 將由環境變數驅動的頻道設定鏡像到目前設定、頻道組態，或你的外掛形態所使用的套件頻道中繼資料。
- 只有在較舊且受支援的 OpenClaw 版本仍需要它時，
  才將 `channelEnvVars` 保留作為相容性中繼資料。
...
套件隨附 `openclaw.security.json`，其中包含 ClawHub 無法辨識為可用的架構參考。

- 如果架構 URL 只是建議性質，請將其移除。
- 只有在 OpenClaw 發佈文件化的版本化架構後，才使用它。
- 重新執行 `clawhub package validate <path-to-plugin>`。
...
- 在 OpenClaw 文件化版本化安全性資訊清單架構與 ClawHub 行為之前，
  移除 `openclaw.security.json`。
- 在資訊清單合約存在之前，請將安全性敏感行為記錄在你的公開套件文件或
  README 中。
- 重新執行 `clawhub package validate <path-to-plugin>`。
