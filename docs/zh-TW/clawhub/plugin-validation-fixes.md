---
read_when:
    - 你已執行 clawhub package validate，且需要修正外掛發現事項
    - ClawHub 拒絕外掛套件發布或發出警告
    - 你正在發布前更新外掛套件中繼資料
summary: 發布前修正 ClawHub 外掛套件驗證發現的問題
title: 外掛驗證修正
x-i18n:
    generated_at: "2026-06-28T20:41:54Z"
    model: gpt-5.5
...
ClawHub 會在發布前驗證外掛套件，也可以顯示自動化套件掃描的發現項目。本頁涵蓋面向作者的發現項目，也就是外掛作者可以在其套件中繼資料、manifest、SDK 匯入或已發布成品中修正的發現項目。

本頁不涵蓋內部 Plugin Inspector 覆蓋率發現項目。如果完整報告包含沒有作者補救指引的掃描器維護代碼，這些項目是給 OpenClaw 維護者，而不是外掛作者。
...
- 請參閱 [建置外掛](/zh-TW/plugins/building-plugins) 取得最小套件範例，並參閱 [外掛 manifest](/zh-TW/plugins/manifest#manifest-versus-packagejson) 了解套件與 manifest 的分工。
...
- 將此項與套件版本分開。套件版本描述外掛發行；`openclaw.compat.pluginApi` 描述主機 API 契約。
- 請參閱 [影響探索的 package.json 欄位](/zh-TW/plugins/manifest#packagejson-fields-that-affect-discovery)。
- 重新執行 `clawhub package validate <path-to-plugin>`。
...
- 對通道入口輔助工具使用 `openclaw/plugin-sdk/channel-core`。
- 使用[匯入慣例](/zh-TW/plugins/building-plugins#import-conventions)和
  [外掛 SDK 子路徑](/zh-TW/plugins/sdk-subpaths)尋找更窄的匯入。
- 重新執行 `clawhub package validate <path-to-plugin>`。
...
此外掛仍使用已棄用的整個 session store 輔助工具
`loadSessionStore`。
...
- 保持通道環境變數中繼資料為宣告式，讓 OpenClaw 能在不載入通道執行階段的情況下檢查設定狀態。
- 將環境變數驅動的通道設定鏡像到此外掛形態使用的目前設定、通道組態或
  套件通道中繼資料。
...
套件隨附 `openclaw.security.json`，其中的 schema 參照未被 ClawHub
識別為可用。

- 如果 schema URL 僅供建議用途，請移除它。
- 僅在 OpenClaw 發布文件化的版本化 schema 後才使用。
- 重新執行 `clawhub package validate <path-to-plugin>`。
...
- 在 OpenClaw 文件化版本化安全性資訊清單 schema 和 ClawHub 行為之前，
...
- [外掛入口點](/zh-TW/plugins/sdk-entrypoints)
- [外掛相容性](/zh-TW/plugins/compatibility)
