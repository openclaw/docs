...
title: 外掛驗證修正
x-i18n:
    generated_at: "2026-07-01T07:50:57Z"
    model: gpt-5.5
...
本頁不涵蓋內部外掛檢查器涵蓋率發現項目。如果完整報告包含沒有作者修復指引的掃描器維護代碼，這些是給 OpenClaw 維護者處理，而不是給外掛作者。
...
| `sdk-load-session-store`                | [取代整個工作階段存放區存取](/zh-TW/clawhub/plugin-validation-fixes#sdk-load-session-store)                                   |
| `sdk-session-store-write`               | [取代整個工作階段存放區寫入](/zh-TW/clawhub/plugin-validation-fixes#sdk-session-store-write)                                  |
...
套件宣告 OpenClaw 進入點，但驗證中的套件缺少參照的檔案。
...
- 如果 `openclaw.plugin.json` 也有 `version`，請更新它以相符，或在套件中繼資料具有權威性時移除過時的資訊清單版本中繼資料。
...
- 打包前建置產生的進入點。
- 更新 `files`、`.npmignore` 或建置輸出，讓已宣告的進入點包含在內。
- 請參閱[外掛進入點](/zh-TW/plugins/sdk-entrypoints)。
- 重新執行 `clawhub package validate <path-to-plugin>`。

### package-npm-pack-metadata-missing

打包後成品缺少來源套件中存在的 OpenClaw 中繼資料。

- 執行 `npm pack --dry-run` 並檢查包含的中繼資料檔案。
- 確保 `package.json` 在打包後成品中包含 `openclaw` 區塊。
...
- 使用 `patchSessionEntry(...)` 或 `upsertSessionEntry(...)` 來持久化工作階段
  中繼資料。
...
- 請參閱[Hook](/zh-TW/plugins/hooks)與
...
- 只有在你支援的 OpenClaw 範圍仍需要時，才將 `providerAuthEnvVars`
  保留為相容性中繼資料。
...
- 讓頻道環境變數中繼資料保持宣告式，讓 OpenClaw 可以在不載入頻道執行階段的情況下檢查設定狀態。
- 將環境變數驅動的頻道設定鏡像到你的外掛形態所使用的目前設定、頻道組態或套件頻道中繼資料。
- 只有在較舊但仍受支援的 OpenClaw 版本仍需要它時，才將 `channelEnvVars` 保留為相容性中繼資料。
...
套件隨附的 `openclaw.security.json` 包含 ClawHub 不認可為可用的結構描述參考。

- 如果結構描述 URL 僅供建議用途，請移除它。
- 只有在 OpenClaw 發佈文件化的版本化結構描述後，才使用它。
- 重新執行 `clawhub package validate <path-to-plugin>`。
...
- 在 OpenClaw 文件化版本化安全性資訊清單結構描述與 ClawHub 行為之前，
