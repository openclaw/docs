...
title: 外掛驗證修正
x-i18n:
    generated_at: "2026-07-02T00:43:02Z"
    model: gpt-5.5
...
ClawHub 會在發布前驗證外掛套件，也可以顯示自動套件掃描的發現項目。本頁涵蓋面向作者的發現項目，也就是外掛作者可以在其套件中繼資料、manifest、SDK 匯入或已發布成品中修正的發現項目。

本頁不涵蓋內部 Plugin Inspector 覆蓋率發現項目。如果完整報告包含沒有作者補救指引的掃描器維護代碼，這些項目是給 OpenClaw 維護者使用，而不是給外掛作者使用。
...
| `package-entrypoint-missing`            | [發布宣告的進入點](/zh-TW/clawhub/plugin-validation-fixes#package-entrypoint-missing)                                  |
...
| `manifest-unknown-contracts`            | [移除不支援的 contract 鍵](/zh-TW/clawhub/plugin-validation-fixes#manifest-unknown-contracts)                                 |
...
| `sdk-session-file-helper`               | [取代 session 檔案路徑輔助函式](/zh-TW/clawhub/plugin-validation-fixes#sdk-session-file-helper)                                   |
...
| `sdk-session-transcript-low-level`      | [取代低階 transcript 輔助函式](/zh-TW/clawhub/plugin-validation-fixes#sdk-session-transcript-low-level)                       |
...
- 使用 [建置外掛](/zh-TW/plugins/building-plugins) 取得最小套件範例，並使用 [外掛 manifest](/zh-TW/plugins/manifest#manifest-versus-packagejson) 了解套件與 manifest 的分工。
...
- 當已發布套件應載入已建置的 JavaScript 時，新增 `openclaw.runtimeExtensions`。
- 將所有進入點路徑保留在套件目錄內。
...
- 以支援的安裝來源填入 `openclaw.install`，例如 `clawhubSpec`、`npmSpec` 或 `localPath`。
...
- 使用你建置並測試時所依據的 OpenClaw 外掛 API 版本或 semver 下限。
- 將此與套件版本分開。套件版本描述外掛版本；`openclaw.compat.pluginApi` 描述主機 API contract。
- 請參閱[影響探索的 package.json 欄位](/zh-TW/plugins/manifest#packagejson-fields-that-affect-discovery)。
- 重新執行 `clawhub package validate <path-to-plugin>`。
...
- 偏好將 `package.json#version` 作為套件發布版本。
- 如果 `openclaw.plugin.json` 也有 `version`，請更新它以相符，或在套件中繼資料具權威性時移除過時的 manifest 版本中繼資料。
- 變更已發布中繼資料後，發布新的套件版本。
...
- 修正無效的套件中繼資料、損壞的 lifecycle scripts，或會造成打包失敗的 files entries。
...
- 打包前先建置產生的進入點。
- 更新 `files`、`.npmignore` 或建置輸出，使宣告的進入點被包含。
- 請參閱[外掛進入點](/zh-TW/plugins/sdk-entrypoints)。
- 重新執行 `clawhub package validate <path-to-plugin>`。
...
- 更新 `files` 或 `.npmignore`，使套件中繼資料不被排除。
...
外掛資訊清單包含 OpenClaw 不支援的頂層欄位。

- 將每個頂層欄位與
  [資訊清單欄位參考](/zh-TW/plugins/manifest#top-level-field-reference)比對。
...
- 將 `contracts` 底下的每個鍵與
  [contracts 參考](/zh-TW/plugins/manifest#contracts-reference)比對。
- 移除不支援的合約鍵。
...
- 使用[匯入慣例](/zh-TW/plugins/building-plugins#import-conventions)和
  [外掛 SDK 子路徑](/zh-TW/plugins/sdk-subpaths)找出狹窄匯入。
- 重新執行 `clawhub package validate <path-to-plugin>`。
...
- 使用 `patchSessionEntry(...)` 或 `upsertSessionEntry(...)` 持久化工作階段
  中繼資料。
...
- 優先使用結構化逐字稿執行階段介面，讓 OpenClaw 可以套用
  正確的交易邊界與身分處理。
- 僅在你宣告的相容性範圍仍支援需要它們的較舊 OpenClaw 版本時，
  才保留低階逐字稿輔助工具。
- 請參閱[執行階段 API](/zh-TW/plugins/sdk-runtime#agent-session-state)和
...
- 將提示或內容變更工作移至 `before_prompt_build`。
- 僅在你宣告的相容性範圍仍支援需要它的較舊 OpenClaw 版本時，
...
資訊清單使用舊版或較舊的頻道環境變數中繼資料，缺少 ClawHub
目前預期的設定或組態中繼資料。

- 讓頻道環境變數中繼資料保持宣告式，以便 OpenClaw 可以在不載入頻道執行階段的情況下
  檢查設定狀態。
- 將由環境變數驅動的頻道設定鏡像到目前的設定、頻道組態，或
  你的外掛形態使用的套件頻道中繼資料。
- 僅在較舊且受支援的 OpenClaw 版本仍需要它時，才保留 `channelEnvVars`
  作為相容性中繼資料。
...
套件隨附 `openclaw.security.json`，其中的結構描述參考無法被 ClawHub
辨識為可用。

- 如果結構描述 URL 僅具建議性，請移除它。
- 僅在 OpenClaw 發佈已文件化的版本化結構描述後，才使用它。
- 重新執行 `clawhub package validate <path-to-plugin>`。
...
- 移除 `openclaw.security.json`，直到 OpenClaw 文件化版本化安全性
  資訊清單結構描述與 ClawHub 行為。
- 在資訊清單合約存在前，將安全性敏感行為記錄在你的公開套件文件或
  README 中。
- 重新執行 `clawhub package validate <path-to-plugin>`。
...
- [ClawHub 發佈](/zh-TW/clawhub/publishing)
