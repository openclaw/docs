---
read_when:
    - 你執行了 `clawhub package validate`，並需要修正外掛發現項
    - ClawHub 拒絕或警告外掛套件發布
    - 你正在發布前更新外掛套件中繼資料
...
ClawHub 會在發布前驗證外掛套件，也可以顯示自動化套件掃描的發現。本頁涵蓋面向作者的發現，也就是外掛作者可以在其套件中繼資料、資訊清單、SDK 匯入或已發布成品中修正的發現。

本頁不涵蓋內部 Plugin Inspector 涵蓋率發現。如果完整報告包含沒有作者補救指引的掃描器維護代碼，這些是給 OpenClaw 維護者看的，而不是給外掛作者看的。
...
- 使用[建置外掛](/zh-TW/plugins/building-plugins)取得最小套件範例，並使用[外掛資訊清單](/zh-TW/plugins/manifest#manifest-versus-packagejson)了解套件與資訊清單的拆分。
...
- 如果 `openclaw.plugin.json` 也有 `version`，請更新它以符合，或在套件中繼資料具權威性時移除過時的資訊清單版本中繼資料。
...
套件可打包，但打包後的成品未包含 `package.json#openclaw` 中宣告的進入點檔案。
...
打包後的成品缺少原始碼套件中存在的 OpenClaw 中繼資料。
...
- 將非空白的 `name` 欄位新增至 `openclaw.plugin.json`。
- 讓 `name` 保持為人類可讀，並讓 `id` 作為穩定的機器 ID。
- 請參閱[外掛資訊清單](/zh-TW/plugins/manifest)。
- 重新執行 `clawhub package validate <path-to-plugin>`。
...
- 當程式碼需要結構化的轉錄操作目標時，使用 `resolveSessionTranscriptTarget(...)`。
- 避免直接讀取或建構舊版轉錄檔案目標。
- 只有在你宣告的相容性範圍仍支援需要它的較舊 OpenClaw 版本時，
  才保留舊版輔助工具。
- 請參閱[執行階段 API](/zh-TW/plugins/sdk-runtime#agent-session-state)和
...
外掛仍使用已淘汰的低階轉錄輔助工具，例如
`appendSessionTranscriptMessage` 或 `emitSessionTranscriptUpdate`。

- 使用 `appendSessionTranscriptMessageByIdentity(...)` 進行轉錄附加。
- 使用 `publishSessionTranscriptUpdateByIdentity(...)` 發布轉錄更新通知。
- 優先使用結構化的轉錄執行階段介面，讓 OpenClaw 可以套用正確的交易邊界與身分處理。
- 只有在你宣告的相容性範圍仍支援需要它們的較舊 OpenClaw 版本時，
  才保留低階轉錄輔助工具。
- 請參閱[執行階段 API](/zh-TW/plugins/sdk-runtime#agent-session-state)和
  [外掛 SDK 子路徑](/zh-TW/plugins/sdk-subpaths)。
...
資訊清單使用舊版或較舊的通道環境變數中繼資料，但缺少 ClawHub 預期的目前設定或組態中繼資料。

- 讓通道環境變數中繼資料保持宣告式，使 OpenClaw 可以在不載入通道執行階段的情況下檢查設定狀態。
...
套件隨附的 `openclaw.security.json` 具有 ClawHub 無法辨識為可用的結構描述參照。

- 如果結構描述 URL 僅作為建議用途，請移除它。
- 只有在 OpenClaw 發布版本化結構描述之後，才使用已記錄的版本化結構描述。
- 重新執行 `clawhub package validate <path-to-plugin>`。
