---
read_when:
    - 你執行了 clawhub package validate，且需要修正外掛發現項目
    - ClawHub 在外掛套件發布時拒絕或發出警告
    - 你正在發行前更新外掛套件中繼資料
summary: 修正 ClawHub 外掛套件發布前的驗證發現
title: 外掛驗證修正
x-i18n:
    generated_at: "2026-07-03T23:26:03Z"
    model: gpt-5.5
...
ClawHub 會在發布前驗證外掛套件，也可以顯示來自自動化套件掃描的發現。此頁涵蓋面向作者的發現，也就是外掛作者可在其套件中繼資料、資訊清單、SDK 匯入或已發布成品中修正的發現。

這不涵蓋內部 Plugin Inspector 覆蓋率發現。如果完整報告包含沒有作者修復指引的掃描器維護代碼，這些代碼是給 OpenClaw 維護者使用，而不是給外掛作者使用。
...
| `sdk-session-transcript-file-target`    | [取代舊版文字記錄檔目標](/zh-TW/clawhub/plugin-validation-fixes#sdk-session-transcript-file-target)                   |
| `sdk-session-transcript-low-level`      | [取代低階文字記錄輔助工具](/zh-TW/clawhub/plugin-validation-fixes#sdk-session-transcript-low-level)                       |
...
- 將此項與套件版本分開。套件版本描述外掛發行；`openclaw.compat.pluginApi` 描述主機 API 合約。
- 請參閱 [會影響探索的 package.json 欄位](/zh-TW/plugins/manifest#packagejson-fields-that-affect-discovery)。
- 重新執行 `clawhub package validate <path-to-plugin>`。
...
- 確保 `package.json` 在打包成品中包含 `openclaw` 區塊。
...
- 將 `contracts` 下的每個鍵與
  [契約參考](/zh-TW/plugins/manifest#contracts-reference)比較。
- 移除不支援的契約鍵。
...
外掛從已棄用的根層 SDK barrel 匯入：
`openclaw/plugin-sdk`。

- 將根層 barrel 匯入替換為聚焦的公開子路徑匯入。
- 針對 `definePluginEntry` 使用 `openclaw/plugin-sdk/plugin-entry`。
- 針對通道進入點輔助工具使用 `openclaw/plugin-sdk/channel-core`。
...
- 針對逐字稿附加使用 `appendSessionTranscriptMessageByIdentity(...)`。
- 針對逐字稿更新通知使用 `publishSessionTranscriptUpdateByIdentity(...)`。
- 優先使用結構化的逐字稿執行階段介面，讓 OpenClaw 能套用正確的
  交易邊界和身分處理。
- 只有在你宣告的相容性範圍仍支援需要它們的舊版 OpenClaw 時，
  才保留低階逐字稿輔助工具。
- 請參閱[執行階段 API](/zh-TW/plugins/sdk-runtime#agent-session-state)和
...
資訊清單使用舊版或較舊的通道環境變數中繼資料，缺少 ClawHub
預期的目前設定或設定檔中繼資料。

- 保持通道環境變數中繼資料為宣告式，讓 OpenClaw 能在不載入通道執行階段的情況下
  檢查設定狀態。
- 將環境變數驅動的通道設定鏡像到你的外掛形態使用的目前設定、通道設定檔或
  套件通道中繼資料。
- 只有在較舊但仍受支援的 OpenClaw 版本仍需要它時，才將 `channelEnvVars`
  保留為相容性中繼資料。
...
套件隨附的 `openclaw.security.json` 包含 ClawHub 未辨識為可用的
結構描述參考。

- 如果結構描述 URL 只具建議性，請移除它。
...
- 在資訊清單契約存在之前，將安全性敏感行為記錄於你的公開套件文件或
  README。
- 重新執行 `clawhub package validate <path-to-plugin>`。
