...
title: 安全性
x-i18n:
    generated_at: "2026-06-30T13:46:24Z"
    model: gpt-5.5
...
請針對 ClawHub 本身的弱點使用 GitHub Security Advisories。良好的 ClawHub advisory 回報包括以下方面的錯誤：
...
請勿使用 ClawHub advisories 回報第三方 skill 或外掛自身原始碼中的弱點。請直接回報給 ClawHub listing 所連結的發布者或原始碼儲存庫。

## 弱點揭露

由於 ClawHub 是託管的雲端應用程式，ClawHub 服務弱點預設不會公開揭露。當有實際使用者影響的證據，或使用者需要採取行動時，才會公開揭露。

實際使用者影響的範例包括已確認的利用、使用者資料或祕密外洩、因平台失效而使惡意內容觸及使用者，或任何需要使用者輪替憑證、更新本機軟體，或採取其他保護措施的問題。
...
若要了解市集回報、審核暫停、隱藏 listing、封鎖與帳戶狀態，請參閱[審核與帳戶安全](/clawhub/moderation)。
