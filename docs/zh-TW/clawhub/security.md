...
- `malicious`：此版本被視為不安全。
- `pending`：檢查尚未完成。
- `held`、`quarantined`、`revoked` 或 `hidden`：此版本並未在公開安裝介面上完全
  可用。

確切措辭可能因介面而異，但實際含義相同：如果某個
版本被保留或阻擋，使用者不應安裝，直到擁有者解決
問題或審核恢復該版本。

## Skills

Skill 掃描會檢查已發布的 Skill 套件包、中繼資料、宣告的
需求，以及可疑指示。

ClawHub 特別注意 Skill 宣告內容與其實際行為之間的不一致。
例如，引用必要 API 金鑰的 Skill 應在 `SKILL.md` 中宣告該需求，
讓使用者能在安裝前看到。

掃描發現以成品為基礎。預期的供應商行為，例如宣告的
API 認證、本機主機 OAuth 回呼、限定範圍的解除安裝清理、Basic Auth
編碼，或使用者選取檔案上傳至已說明的供應商，會與隱藏式憑證轉送、
廣泛存取私人檔案、無關的網路目的地，或隱密的瀏覽器濫用
區別處理。
...
成品。ClawScan 在審查 Plugin 版本時會包含宣告的套件 `openclaw.environment` 環境/設定
中繼資料，讓宣告的執行階段需求能與觀察到的行為比較。

## 回報

已登入的使用者可以回報 Skill、套件與留言。

回報應具體且可行。濫用回報本身也可能導致
帳戶處置。
...
發布 Skill 或 Plugin 時，發布者可以提供選用的 ClawScan 備註。
此備註會為 ClawScan 提供脈絡，用於說明否則可能看起來不尋常的行為，
例如網路存取、原生主機存取，或供應商特定
憑證。
...
因使用者層級保留而隱藏的 Skill，並寫入一筆 `user.moderation.lift` 稽核
記錄項目。因其他原因被隱藏，或其自身靜態掃描仍為
惡意的 Skill，會維持隱藏。

## 停權與帳戶狀態

違反 ClawHub 政策的帳戶可能會失去發布權限。嚴重濫用
可能導致帳戶停權、權杖撤銷、內容隱藏或列表
移除。

已刪除、遭停權或停用的帳戶無法使用 ClawHub API 權杖。如果 CLI 驗證
在帳戶處置後開始失敗，請登入網頁 UI 檢查帳戶
狀態。如果登入或一般 CLI 存取被阻擋，請聯絡
...
- 保持名稱、摘要、標籤與變更記錄準確
- 宣告必要環境變數與權限
- 當版本具有不尋常但有意設計的行為時，加入發布者 ClawScan 備註
- 避免混淆處理過的安裝命令
- 盡可能連結到原始碼
