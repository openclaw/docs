...
title: 安全性
x-i18n:
    generated_at: "2026-07-01T15:19:38Z"
    model: gpt-5.5
...
請使用 GitHub 安全性公告回報 ClawHub 本身的漏洞。良好的 ClawHub 公告報告包含以下錯誤：
...
請勿使用 ClawHub 公告回報第三方技能或外掛自身原始碼中的漏洞。請直接向 ClawHub 清單連結的發布者或原始碼儲存庫回報。

## 漏洞揭露

由於 ClawHub 是託管的雲端應用程式，ClawHub 服務漏洞預設不會公開揭露。當有實際使用者影響的證據，或使用者需要採取行動時，才會公開揭露。

實際使用者影響的範例包括已確認的利用、使用者資料或密鑰外洩、因平台故障而讓惡意內容觸及使用者，或任何需要使用者輪換憑證、更新本機軟體，或採取其他保護行動的問題。
...
如需市集報告、審核保留、隱藏清單、停權和帳號狀態，請參閱[審核與帳號安全](/clawhub/moderation)。
