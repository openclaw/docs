...
ClawHub 追蹤 semver 版本、`latest` 等標籤、變更記錄、檔案、下載數、星標與安全掃描摘要。公開頁面會顯示目前的登錄庫狀態，讓使用者在安裝前檢視 Skill 或外掛。
...
外掛安裝會在封存安裝執行前，驗證宣告的 `pluginApi` 與 `minGatewayVersion` 相容性。當套件版本發布 ClawPack 成品時，OpenClaw 會優先使用精確上傳的 npm-pack `.tgz`、驗證 ClawHub 摘要標頭與下載位元組，並記錄成品中繼資料以供後續更新使用。
...
命令列介面也提供 Skill 安裝/更新命令，可用於直接登錄庫工作流程：
...
ClawHub 預設開放：任何人都能上傳，但發布需要足夠舊且能通過上傳門檻的 GitHub 帳戶。公開詳細資料頁會在安裝或下載前摘要最新掃描狀態。

ClawHub 會對已發布的 Skills 與外掛版本執行自動化檢查。被掃描保留或封鎖的版本，可能會從公開目錄與安裝介面中消失，但其擁有者仍可在 `/dashboard` 中看到。

已登入使用者可以檢舉 Skills 與套件。審核員可以審閱檢舉、隱藏或還原內容，並封鎖濫用帳戶。政策與執行細節請見
...
當你在已登入狀態下執行 `clawhub install` 時，命令列介面可能會盡力傳送安裝事件，讓 ClawHub 能計算彙總安裝數。可使用以下方式停用：
