...
title: ClawHub
x-i18n:
    generated_at: "2026-05-12T15:42:48Z"
    model: gpt-5.5
    provider: openai
...
- 使用原生 `openclaw` 命令搜尋、安裝與更新 Skills，並從 ClawHub 安裝 Plugin。
...
當你需要登錄檔驗證的工作流程，例如發布、同步，或刪除/取消刪除時，請安裝 ClawHub CLI：
...
| 介面           | 儲存內容                                                     | 常用命令                                     |
| -------------- | ------------------------------------------------------------ | -------------------------------------------- |
| Skills         | 版本化文字套件，包含 `SKILL.md` 與支援檔案                  | `openclaw skills install <slug>`             |
| 程式碼 Plugin  | 含相容性中繼資料的 OpenClaw Plugin 套件                     | `openclaw plugins install clawhub:<package>` |
| 套件組 Plugin  | 用於 OpenClaw 發行的封裝 Plugin 套件組                      | `clawhub package publish <source>`           |
| Souls          | 顯示於 onlycrabs.ai 的 `SOUL.md` 套件組                      | Web 與 API 發布流程                         |

ClawHub 會追蹤 semver 版本、`latest` 等標籤、變更記錄、檔案、下載次數、星標，以及安全掃描摘要。公開頁面會顯示目前的登錄檔狀態，讓使用者在安裝 Skill 或 Plugin 前可以檢查內容。

## 原生 OpenClaw 流程

原生 OpenClaw 命令會安裝到作用中的 OpenClaw 工作區，並保存來源中繼資料，讓後續更新命令能持續使用 ClawHub。

當 Plugin 安裝應透過 ClawHub 解析時，請使用 `clawhub:<package>`。裸露的 npm 安全 Plugin 規格可能會在啟動切換期間透過 npm 解析，而在來源必須明確時，`npm:<package>` 會維持僅使用 npm。

Plugin 安裝會在封存檔安裝執行前，驗證宣告的 `pluginApi` 與 `minGatewayVersion` 相容性。當套件版本發布 ClawPack 成品時，OpenClaw 會優先使用確切上傳的 npm-pack `.tgz`，驗證 ClawHub digest 標頭與下載的位元組，並記錄成品中繼資料以供後續更新使用。
...
CLI 也提供 Skill 安裝/更新命令，可用於直接的登錄檔工作流程：
...
ClawHub 預設開放：任何人都可以上傳，但發布需要 GitHub 帳戶足夠久，才能通過上傳門檻。公開詳細資料頁會在安裝或下載前摘要顯示最新掃描狀態。

ClawHub 會對已發布的 Skills 和 Plugin 發行版本執行自動檢查。被掃描保留或封鎖的發行版本可能會從公開目錄和安裝介面消失，但仍可由其擁有者在 `/dashboard` 中看到。

已登入的使用者可以檢舉 Skills 和套件。審核者可以審查檢舉、隱藏或還原內容，並封鎖濫用帳戶。政策與執行細節請參閱 [可接受使用方式](/zh-TW/clawhub/acceptable-usage) 和 [安全性 + 審核](/zh-TW/clawhub/security)。
