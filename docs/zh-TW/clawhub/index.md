...
read_when:
    ...
    - 搜尋、安裝或更新 Skills 或外掛
    - 將 Skills 或外掛發布到登錄庫
    - 選擇 openclaw 與 clawhub 命令列介面流程之間的取捨
sidebarTitle: ClawHub
summary: 用於探索、安裝、發布、安全性與 clawhub 命令列介面的公開 ClawHub 概覽。
title: ClawHub
x-i18n:
    generated_at: "2026-06-28T20:41:39Z"
    model: gpt-5.5
...
ClawHub 會追蹤語意化版本、`latest` 等標籤、變更記錄、檔案、下載次數、星標，以及安全掃描摘要。公開頁面會顯示目前的登錄檔狀態，讓使用者可在安裝前檢視 Skill 或外掛。
...
程式碼外掛必須在 `package.json` 中包含必要的 OpenClaw 相容性中繼資料，包括 `openclaw.compat.pluginApi` 與 `openclaw.build.openclawVersion`。完整命令參考請見 [命令列介面](/zh-TW/clawhub/cli)，Skill 中繼資料請見 [Skill 格式](/zh-TW/clawhub/skill-format)。

## 安全與審核

ClawHub 預設開放：任何人都可以上傳，但發布需要 GitHub 帳號年齡足以通過上傳閘門。公開詳細資料頁面會在安裝或下載前摘要最新掃描狀態。

ClawHub 會對已發布的 Skills 和外掛版本執行自動化檢查。遭掃描保留或封鎖的版本可能會從公開目錄和安裝介面消失，但仍可由其擁有者在 `/dashboard` 中看見。

已登入使用者可以回報 Skills 和套件。審核者可以檢閱回報、隱藏或還原內容，並停權濫用帳號。政策與執行細節請見
...
當你在登入狀態下執行 `clawhub install` 時，命令列介面可能會盡力傳送安裝事件，讓 ClawHub 能計算彙總安裝數。可用以下方式停用：
...
更深入的參考資料請見 [遙測](/zh-TW/clawhub/telemetry)、[HTTP API](/zh-TW/clawhub/http-api) 和
