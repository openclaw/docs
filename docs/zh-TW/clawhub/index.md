...
read_when:
    ...
    - 搜尋、安裝或更新 Skills 或外掛
    - 將 Skills 或外掛發布到登錄檔
    - 選擇 openclaw 與 clawhub 命令列介面流程之間的差異
...
| 組合外掛 | 供 OpenClaw 發行使用的已封裝外掛組合 | `clawhub package publish <source>` |

ClawHub 追蹤 semver 版本、`latest` 等標籤、變更記錄、檔案、下載次數、星標，以及安全掃描摘要。公開頁面會顯示目前的登錄檔狀態，讓使用者在安裝 Skill 或外掛前能夠檢查。

## 原生 OpenClaw 流程

原生 OpenClaw 命令會安裝到目前作用中的 OpenClaw 工作區，並保存來源中繼資料，讓後續更新命令能維持使用 ClawHub。

當外掛安裝應透過 ClawHub 解析時，請使用 `clawhub:<package>`。裸 npm 安全外掛規格在啟動切換期間可能會透過 npm 解析，而當來源必須明確指定時，`npm:<package>` 會維持僅使用 npm。
...
ClawHub 預設開放：任何人都可以上傳，但發布需要 GitHub 帳號足夠久，以通過上傳門檻。公開詳細資料頁面會在安裝或下載前摘要最新掃描狀態。

ClawHub 會對已發布的 Skills 與外掛版本執行自動化檢查。被掃描保留或封鎖的版本可能會從公開目錄與安裝介面中消失，但其擁有者仍可在 `/dashboard` 中看到。

已登入的使用者可以檢舉 Skills 與套件。審核者可以審查檢舉、隱藏或還原內容，並封鎖濫用帳號。政策與執行細節請參閱
...
更深入的參考資料請參閱 [遙測](/clawhub/telemetry)、[HTTP API](/clawhub/http-api)，以及
