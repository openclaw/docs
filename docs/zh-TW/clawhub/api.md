...
x-i18n:
    generated_at: "2026-06-28T00:10:36Z"
    model: gpt-5.5
...
你可以在 ClawHub 的公開讀取 API 之上建置第三方目錄、清單或搜尋介面。公開技能中繼資料與技能檔案會依 ClawHub 的技能授權規則發布，而 API 本身有速率限制，應負責任地使用。

指南：

- 使用公開讀取端點，例如 `GET /api/v1/skills`、`GET /api/v1/search` 和 `GET /api/v1/skills/{slug}` 取得目錄列表。
- 快取回應並遵守 `429`、`Retry-After` 和速率限制標頭，而不是激進地輪詢。
- 顯示列表時連回標準 ClawHub 技能 URL，讓使用者可以檢查來源註冊記錄。
...
- 下載：每 IP 1200/min，每 key 6000/min
...
- v1 錯誤是純文字（`text/plain; charset=utf-8`），包含 `400`、`401`、`403`、`404`、`429` 和遭封鎖下載回應。
- 未知查詢參數會因相容性而被忽略。
- 已知查詢參數若值無效，會回傳 `400`。
...
- `GET /api/v1/skills?limit=&cursor=&sort=`
  ...
  - 舊版別名：`nonSuspicious=true`
  - 使用 `nonSuspiciousOnly=true` 時，游標型頁面可能包含少於 `limit` 個項目；使用 `nextCursor` 繼續。
...
- `GET /api/v1/download?slug=&version=&tag=`
  - 託管技能會回傳確定性的 ZIP 位元組。
  - 目前由 GitHub 支援且掃描結果為 `clean` 或 `suspicious` 的技能，會回傳 JSON `public-github` 交接描述元，而不是 ClawHub 位元組。
