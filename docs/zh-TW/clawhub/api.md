...
read_when:
    - 建置 API 用戶端
    - 新增端點或結構描述
summary: 公共 REST API (v1) 概觀與慣例。
x-i18n:
    generated_at: "2026-07-04T20:23:35Z"
    model: gpt-5.5
...
你可以在 ClawHub 的公開讀取 API 之上建置第三方目錄、清單或搜尋介面。公開 skill 中繼資料和 skill 檔案會依 ClawHub 的 skill 授權規則發布，而 API 本身有速率限制，應負責任地使用。

指南：

- 使用公開讀取端點，例如 `GET /api/v1/skills`、`GET /api/v1/search` 和 `GET /api/v1/skills/{slug}` 來取得目錄列表。
- 快取回應，並遵守 `429`、`Retry-After` 和速率限制標頭，而不是積極輪詢。
- 顯示列表時連回標準 ClawHub skill URL，讓使用者可以檢查來源登錄記錄。
- 使用格式為 `https://clawhub.ai/<owner>/skills/<slug>` 的標準頁面 URL。
- 不要暗示 ClawHub 背書、驗證或營運該第三方網站。
- 不要透過繞過公開 API 篩選器或驗證邊界來鏡像隱藏、私人或因審核而封鎖的內容。
...
- v1 錯誤是純文字（`text/plain; charset=utf-8`），包括 `400`、`401`、`403`、`404`、`429` 和被封鎖的下載回應。
- 未知查詢參數會基於相容性而忽略。
- 已知查詢參數若值無效，會回傳 `400`。
...
- `GET /api/v1/skills?limit=&cursor=&sort=`
  ...
  - 舊版別名：`nonSuspicious=true`
  - 使用 `nonSuspiciousOnly=true` 時，以游標為基礎的頁面可能包含少於 `limit` 的項目；使用 `nextCursor` 繼續。
...
- `GET /api/v1/download?slug=&version=&tag=`
  ...
  - 目前以 GitHub 為後端且掃描結果為 `clean` 或 `suspicious` 的 skills，會回傳 JSON `public-github` 交接描述元，而不是 ClawHub 位元組。
- `GET /api/v1/skills/export?startDate=&endDate=&limit=&cursor=`
  - 託管的 skills 會匯出為儲存的檔案。
