...
- 快取回應並遵守 `429`、`Retry-After` 和速率限制標頭，不要激進輪詢。
...
- `GET /api/v1/download?slug=&version=&tag=`
  - 託管技能會回傳確定性的 ZIP 位元組。
  - 目前以 GitHub 為後端且掃描結果為 `clean` 或 `suspicious` 的技能，會回傳
    JSON `public-github` 交接描述元，而不是 ClawHub 位元組。
- `GET /api/v1/skills/export?startDate=&endDate=&limit=&cursor=`
  - 託管技能會匯出為已儲存檔案。
  - 目前以 GitHub 為後端且掃描結果為 `clean` 或 `suspicious` 的技能，會匯出
...
- `POST /api/v1/users/reserve` 會為擁有者代號保留根 slug 和私人無發布套件預留位置。
