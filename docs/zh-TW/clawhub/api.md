...
x-i18n:
    generated_at: "2026-07-03T17:12:35Z"
    model: gpt-5.5
...
- `GET /api/v1/skills?limit=&cursor=&sort=`
  ...
  - 舊版別名：`nonSuspicious=true`
  - 使用 `nonSuspiciousOnly=true` 時，游標式頁面可能包含少於 `limit` 的項目；請使用 `nextCursor` 繼續。
...
- `POST /api/v1/users/reserve` 會為擁有者 handle 保留根 slug 和私人未發布套件佔位符。
