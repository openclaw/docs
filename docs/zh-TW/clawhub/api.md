...
- `GET /api/v1/download?slug=&version=&tag=`
  - 託管技能會回傳確定性的 ZIP 位元組。
  - 目前由 GitHub 支援、且掃描結果為 `clean` 或 `suspicious` 的技能，會回傳 JSON `public-github` 交接描述，而不是 ClawHub 位元組。
- `GET /api/v1/skills/export?startDate=&endDate=&limit=&cursor=`
  - 託管技能會匯出為已儲存的檔案。
  - 目前由 GitHub 支援、且掃描結果為 `clean` 或 `suspicious` 的技能，會匯出為 `public-github` 交接描述。
