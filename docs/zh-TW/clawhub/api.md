...
x-i18n:
    generated_at: "2026-07-01T18:06:16Z"
    model: gpt-5.5
...
- 快取回應，並遵守 `429`、`Retry-After` 和速率限制標頭，不要積極輪詢。
...
- 匿名請求：按 IP。
- 已驗證請求（有效 Bearer 權杖）：按使用者 bucket。
- 缺少/無效權杖會回退為 IP 強制執行。
...
- `GET /api/v1/download?slug=&version=&tag=`
  ...
  - 目前由 GitHub 支援且掃描結果為 `clean` 或 `suspicious` 的 skills，會回傳 JSON `public-github` 交接描述子，而不是 ClawHub 位元組。
- `GET /api/v1/skills/export?startDate=&endDate=&limit=&cursor=`
  - 託管 skills 會匯出為已儲存的檔案。
  - 目前由 GitHub 支援且掃描結果為 `clean` 或 `suspicious` 的 skills，會匯出為 `public-github` 交接描述子。
...
- `POST /api/v1/users/reserve` 會為擁有者 handle 保留根 slug 和私人無發行套件佔位符。
