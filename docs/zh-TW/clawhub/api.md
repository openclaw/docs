...
- 顯示清單時連回正式的 ClawHub Skills URL，讓使用者可以檢查來源註冊記錄。
- 使用格式為 `https://clawhub.ai/<owner>/skills/<slug>` 的正式頁面 URL。
...
- `GET /api/v1/download?slug=&version=&tag=`
  ...
  - 目前由 GitHub 支援，且掃描結果為 `clean` 或 `suspicious` 的 Skills，會回傳 JSON `public-github` 交接描述，而不是 ClawHub 位元組。
- `GET /api/v1/skills/export?startDate=&endDate=&limit=&cursor=`
  - 託管的 Skills 會以儲存檔案匯出。
  - 目前由 GitHub 支援，且掃描結果為 `clean` 或 `suspicious` 的 Skills，會以 `public-github` 交接描述匯出。
...
- `POST /api/v1/users/reserve` 會為擁有者代號保留根 slug 與私人無發布套件佔位符。
