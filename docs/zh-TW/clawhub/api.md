...
x-i18n:
    generated_at: "2026-05-11T20:22:20Z"
    model: gpt-5.5
    provider: openai
...
你可以在 ClawHub 的公開讀取 API 之上建立第三方目錄、名錄或搜尋介面。公開的 skill 中繼資料與 skill 檔案會依 ClawHub 的 skill 授權規則發布，而 API 本身有速率限制，應負責任地使用。
...
- `POST /api/v1/users/reserve` 會為擁有者 handle 保留根 slug 和私有無發行 package 預留項。
