...
read_when:
    - 建置 API 用戶端
    - 新增端點或架構
summary: 公開 REST 應用程式介面 (v1) 概觀與慣例。
x-i18n:
...
你可以在 ClawHub 的公開讀取 API 之上建置第三方目錄、索引或搜尋介面。公開技能中繼資料和技能檔案會依 ClawHub 的技能授權規則發布，而 API 本身有速率限制，應負責任地使用。

指引：

- 使用公開讀取端點，例如 `GET /api/v1/skills`、`GET /api/v1/search` 和 `GET /api/v1/skills/{slug}` 取得目錄清單。
- 快取回應，並尊重 `429`、`Retry-After` 和速率限制標頭，而不是積極輪詢。
- 顯示清單時連回正式的 ClawHub 技能 URL，讓使用者可以檢視來源登錄紀錄。
...
- `GET /api/v1/download?slug=&version=&tag=`
  - 託管技能會回傳確定性的 ZIP 位元組。
  - 目前由 GitHub 支援且掃描結果為 `clean` 或 `suspicious` 的技能，會回傳 JSON `public-github` 交接描述子，而不是 ClawHub 位元組。
- `GET /api/v1/skills/export?startDate=&endDate=&limit=&cursor=`
  - 託管技能會匯出為已儲存檔案。
  - 目前由 GitHub 支援且掃描結果為 `clean` 或 `suspicious` 的技能，會匯出為 `public-github` 交接描述子。
...
- `POST /api/v1/users/reserve` 會為擁有者帳號保留根 slug 和私人未發布套件預留位置。
