...
x-i18n:
    generated_at: "2026-07-02T13:57:23Z"
    model: gpt-5.5
...
你可以在 ClawHub 的公開讀取 API 之上建置第三方目錄、索引或搜尋介面。公開 skill 中繼資料與 skill 檔案會依 ClawHub 的 skill 授權規則發布，而 API 本身有速率限制，應負責任地使用。

指引：

- 使用公開讀取端點，例如 `GET /api/v1/skills`、`GET /api/v1/search` 和 `GET /api/v1/skills/{slug}` 取得目錄清單。
- 快取回應並尊重 `429`、`Retry-After` 與速率限制標頭，而不是積極輪詢。
...
- v1 錯誤為純文字（`text/plain; charset=utf-8`），包含 `400`、`401`、`403`、`404`、`429` 以及被封鎖的下載回應。
- 未知查詢參數會為了相容性而忽略。
- 已知查詢參數帶有無效值時會回傳 `400`。
...
- `POST /api/v1/users/reserve` 會為擁有者 handle 保留根 slugs 與私人無發布 package 預留位置。
