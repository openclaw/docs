...
- 顯示列表時連回標準的 ClawHub Skills URL，讓使用者可以檢查來源註冊記錄。
...
`X-RateLimit-Remaining`、`RateLimit-Remaining` 和 `Retry-After` 會在 `429` 中包含。
...
- v1 錯誤為純文字（`text/plain; charset=utf-8`），包括 `400`、`401`、`403`、`404`、`429` 和遭封鎖下載回應。
- 為了相容性，未知查詢參數會被忽略。
- 已知查詢參數若值無效會回傳 `400`。
...
- `POST /api/v1/users/reserve` 會為擁有者 handle 保留根 slugs 和私人未發布套件佔位符。
