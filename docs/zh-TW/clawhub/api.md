...
x-i18n:
    generated_at: "2026-07-02T07:55:56Z"
    model: gpt-5.5
...
- 不要透過繞過公開 API 篩選器或驗證邊界來鏡像隱藏、私有或被審核封鎖的內容。
...
- 讀取：每個 IP 3000/min，每個金鑰 12000/min
- 寫入：每個 IP 300/min，每個金鑰 3000/min
- 下載：每個 IP 1200/min，每個金鑰 6000/min

標頭：`X-RateLimit-Limit`、`X-RateLimit-Reset`、`RateLimit-Limit`、`RateLimit-Reset`；
`X-RateLimit-Remaining`、`RateLimit-Remaining` 和 `Retry-After` 會在 `429` 上包含。
...
- `GET /api/v1/skills/export?startDate=&endDate=&limit=&cursor=`
  - 託管技能會匯出為已儲存檔案。
  - 目前以 GitHub 為後端且掃描結果為 `clean` 或 `suspicious` 的技能，會匯出為 `public-github` 交接描述元。
...
- `POST /api/v1/users/reserve` 會為擁有者控制代碼保留根 slug 和私有無發布套件預留位置。
