...
- 快取回應，並遵守 `429`、`Retry-After` 與速率限制標頭，不要過度頻繁輪詢。
...
- 匿名請求：依 IP。
- 已驗證請求（有效的 Bearer token）：依使用者配額桶。
...
- `GET /api/v1/download?slug=&version=&tag=`
  - 託管技能會回傳確定性的 ZIP 位元組。
  - 目前以 GitHub 支援、且掃描結果為 `clean` 或 `suspicious` 的技能會回傳
    JSON `public-github` 轉交描述元，而不是 ClawHub 位元組。
- `GET /api/v1/skills/export?startDate=&endDate=&limit=&cursor=`
  - 託管技能會匯出為儲存的檔案。
  - 目前以 GitHub 支援、且掃描結果為 `clean` 或 `suspicious` 的技能會匯出
    為 `public-github` 轉交描述元。
...
- `POST /api/v1/users/reserve` 會為擁有者 handle 保留根 slug 與私人未發布套件佔位符。
