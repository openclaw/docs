...
x-i18n:
    generated_at: "2026-07-01T10:56:29Z"
    model: gpt-5.5
...
- 快取回應，並遵守 `429`、`Retry-After` 和速率限制標頭，而不是激進輪詢。
- 顯示列表時連回標準 ClawHub 技能 URL，讓使用者可以檢查來源登錄紀錄。
...
- `X-RateLimit-Remaining` / `RateLimit-Remaining`：出現時為精確的剩餘配額；分片成功請求會省略它，而不是傳回近似的全域值
...
- `GET /api/v1/download?slug=&version=&tag=`
  - 託管技能會傳回決定性的 ZIP 位元組。
  - 目前由 GitHub 支援且掃描結果為 `clean` 或 `suspicious` 的技能，會傳回 JSON `public-github` 交接描述器，而不是 ClawHub 位元組。
- `GET /api/v1/skills/export?startDate=&endDate=&limit=&cursor=`
  - 託管技能會以已儲存檔案匯出。
  - 目前由 GitHub 支援且掃描結果為 `clean` 或 `suspicious` 的技能，會匯出為 `public-github` 交接描述器。
...
- `POST /api/v1/users/reserve` 會為擁有者帳號保留根 slugs 和私有無發布套件預留位置。
