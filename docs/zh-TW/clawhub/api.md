...
你可以在 ClawHub 的公開讀取 API 之上建置第三方目錄、索引或搜尋介面。公開 Skills 中繼資料與 Skills 檔案會依照 ClawHub 的 Skills 授權規則發布，而 API 本身有速率限制，應負責任地使用。
...
- `GET /api/v1/download?slug=&version=&tag=`
  ...
  - 目前由 GitHub 支援且具有 `clean` 或 `suspicious` 掃描結果的 Skills，會回傳 JSON `public-github` 交接描述子，而不是 ClawHub 位元組。
- `GET /api/v1/skills/export?startDate=&endDate=&limit=&cursor=`
  - 託管的 Skills 會以儲存檔案匯出。
  - 目前由 GitHub 支援且具有 `clean` 或 `suspicious` 掃描結果的 Skills，會以 `public-github` 交接描述子匯出。
...
- `POST /api/v1/users/reserve` 會為擁有者帳號保留根 slugs 與私有無發布套件預留位置。
