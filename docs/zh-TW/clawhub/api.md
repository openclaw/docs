...
x-i18n:
    generated_at: "2026-07-03T23:25:41Z"
    model: gpt-5.5
...
你可以在 ClawHub 的公開讀取 API 之上建置第三方目錄、索引或搜尋介面。公開技能中繼資料和技能檔案會依照 ClawHub 的技能授權規則發布，而 API 本身有速率限制，應負責任地使用。

指南：

- 使用公開讀取端點，例如 `GET /api/v1/skills`、`GET /api/v1/search` 和 `GET /api/v1/skills/{slug}` 來取得目錄清單。
- 快取回應，並尊重 `429`、`Retry-After` 和速率限制標頭，而不是積極輪詢。
- 顯示清單時連回標準 ClawHub 技能 URL，讓使用者可以檢視來源註冊記錄。
...
- `X-RateLimit-Remaining` / `RateLimit-Remaining`：存在時代表精確剩餘額度；分片的成功請求會省略它，而不是回傳近似的全域值
...
- `GET /api/v1/skills?limit=&cursor=&sort=`
  ...
  - 舊版別名：`nonSuspicious=true`
  - 使用 `nonSuspiciousOnly=true` 時，游標式頁面可能包含少於 `limit` 個項目；使用 `nextCursor` 繼續。
  - `recommended` 使用互動度和新近度訊號。
...
- `POST /api/v1/users/reserve` 會為擁有者 handle 保留根 slug 和私人無發行版本套件預留位置。
