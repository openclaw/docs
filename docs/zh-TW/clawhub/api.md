...
x-i18n:
    generated_at: "2026-07-05T11:08:55Z"
    model: gpt-5.5
...
- `GET /api/v1/download?slug=&version=&tag=`
  - 託管技能會回傳確定性的 ZIP 位元組。
  - 目前由 GitHub 支援且具有 `clean` 或 `suspicious` 掃描結果的技能，會回傳 JSON `public-github` 交接描述元，而不是 ClawHub 位元組。
- `GET /api/v1/skills/export?startDate=&endDate=&limit=&cursor=`
  - 託管技能會匯出為已儲存的檔案。
  - 目前由 GitHub 支援且具有 `clean` 或 `suspicious` 掃描結果的技能，會匯出為 `public-github` 交接描述元。
