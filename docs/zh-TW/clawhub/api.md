...
x-i18n:
    generated_at: "2026-07-04T15:07:12Z"
    model: gpt-5.5
...
你可以在 ClawHub 的公開讀取 API 之上建置第三方型錄、目錄或搜尋介面。公開 skill 中繼資料與 skill 檔案會依 ClawHub 的 skill 授權規則發布，而 API 本身有速率限制，應負責任地使用。

指南：

- 使用公開讀取端點，例如 `GET /api/v1/skills`、`GET /api/v1/search` 和 `GET /api/v1/skills/{slug}` 取得型錄清單。
- 快取回應並遵守 `429`、`Retry-After` 與速率限制標頭，而不是積極輪詢。
- 顯示清單時連回標準 ClawHub skill URL，讓使用者可檢查來源註冊記錄。
- 使用格式為 `https://clawhub.ai/<owner>/skills/<slug>` 的標準頁面 URL。
- 不要暗示 ClawHub 背書、驗證或營運該第三方網站。
- 不要透過繞過公開 API 篩選器或驗證邊界，鏡像隱藏、私人或因審核遭封鎖的內容。
...
- v1 錯誤是純文字（`text/plain; charset=utf-8`），包括 `400`、`401`、`403`、`404`、`429`，以及遭封鎖的下載回應。
- 未知查詢參數會基於相容性而被忽略。
- 已知查詢參數若值無效，會回傳 `400`。
...
- `GET /api/v1/download?slug=&version=&tag=`
  ...
  - 目前由 GitHub 支援、且掃描為 `clean` 或 `suspicious` 的 skills 會回傳 JSON `public-github` 交接描述元，而不是 ClawHub 位元組。
- `GET /api/v1/skills/export?startDate=&endDate=&limit=&cursor=`
  - 託管 skills 會匯出為儲存的檔案。
  - 目前由 GitHub 支援、且掃描為 `clean` 或 `suspicious` 的 skills 會匯出為 `public-github` 交接描述元。
...
- `POST /api/v1/users/reserve` 會為擁有者 handle 保留根 slugs 與私人無發布套件佔位符。
