...
- `X-RateLimit-Remaining` / `RateLimit-Remaining`：存在時為精確的剩餘配額；分片的成功請求會省略它，而不是回傳近似的全域值
...
- `GET /api/v1/download?slug=&version=&tag=`
  ...
  - 目前由 GitHub 支援且掃描為 `clean` 或 `suspicious` 的 Skills，會回傳 JSON `public-github` 交接描述元，而不是 ClawHub 位元組。
- `GET /api/v1/skills/export?startDate=&endDate=&limit=&cursor=`
  - 託管的 Skills 會匯出為已儲存的檔案。
  - 目前由 GitHub 支援且掃描為 `clean` 或 `suspicious` 的 Skills，會匯出為 `public-github` 交接描述元。
...
- `POST /api/v1/users/reserve` 會為擁有者帳號保留根 slug 與私有的無發布套件預留位置。
