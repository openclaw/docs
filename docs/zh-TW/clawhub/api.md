...
x-i18n:
    generated_at: "2026-07-01T20:10:45Z"
    model: gpt-5.5
...
你可以在 ClawHub 的公開讀取 API 之上建置第三方目錄、索引或搜尋介面。公開技能中繼資料與技能檔案會依 ClawHub 的技能授權規則發布，而 API 本身有速率限制，應負責任地使用。

指引：

- 使用公開讀取端點，例如 `GET /api/v1/skills`、`GET /api/v1/search` 和 `GET /api/v1/skills/{slug}` 來取得目錄列表。
- 快取回應並遵守 `429`、`Retry-After` 和速率限制標頭，而不是激進地輪詢。
- 顯示列表時連回標準 ClawHub 技能 URL，讓使用者可以檢視來源登錄記錄。
...
- v1 錯誤是純文字（`text/plain; charset=utf-8`），包含 `400`、
  `401`、`403`、`404`、`429`，以及遭封鎖的下載回應。
- 未知查詢參數會為了相容性而忽略。
- 已知查詢參數若值無效，會回傳 `400`。
...
- `GET /api/v1/download?slug=&version=&tag=`
  - 託管技能會回傳確定性的 ZIP 位元組。
  - 目前由 GitHub 支援且掃描結果為 `clean` 或 `suspicious` 的技能會回傳 JSON `public-github` 交接描述器，而不是 ClawHub 位元組。
- `GET /api/v1/skills/export?startDate=&endDate=&limit=&cursor=`
  - 託管技能會匯出為已儲存的檔案。
  - 目前由 GitHub 支援且掃描結果為 `clean` 或 `suspicious` 的技能會匯出為 `public-github` 交接描述器。
...
- `POST /api/v1/users/reserve` 會為擁有者控制代碼保留根 slug 和私人無發布套件預留位置。
