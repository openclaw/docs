...
x-i18n:
    generated_at: "2026-07-05T04:56:05Z"
    model: gpt-5.5
...
你可以在 ClawHub 的公開讀取 API 之上建置第三方目錄、索引或搜尋介面。公開 Skills 中繼資料與 Skills 檔案會依 ClawHub 的 Skills 授權規則發布，而 API 本身有速率限制，應以負責任的方式使用。

指南：

- 使用公開讀取端點，例如 `GET /api/v1/skills`、`GET /api/v1/search` 和 `GET /api/v1/skills/{slug}` 取得目錄列表。
- 快取回應並遵守 `429`、`Retry-After` 與速率限制標頭，而不是積極輪詢。
- 顯示列表時連回標準 ClawHub Skills URL，讓使用者可以檢查來源登錄紀錄。
...
- `X-RateLimit-Remaining` / `RateLimit-Remaining`：存在時表示確切剩餘配額；分片的成功請求會省略它，而不是回傳近似的全域值
...
- v1 錯誤為純文字（`text/plain; charset=utf-8`），包括 `400`、`401`、`403`、`404`、`429` 和被封鎖下載的回應。
- 為了相容性，未知查詢參數會被忽略。
- 已知查詢參數若值無效，會回傳 `400`。
...
- `POST /api/v1/users/reserve` 會為擁有者 handle 保留根 slugs 和私人無發布套件佔位符。
