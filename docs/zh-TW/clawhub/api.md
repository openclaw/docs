...
x-i18n:
    generated_at: "2026-07-02T22:21:38Z"
    model: gpt-5.5
...
你可以在 ClawHub 的公開讀取 API 之上建立第三方目錄、索引或搜尋介面。公開技能中繼資料與技能檔案會依 ClawHub 的技能授權規則發布，而 API 本身有速率限制，應負責任地取用。

指南：

- 使用公開讀取端點，例如 `GET /api/v1/skills`、`GET /api/v1/search` 和 `GET /api/v1/skills/{slug}` 來取得目錄清單。
- 快取回應並遵守 `429`、`Retry-After` 與速率限制標頭，而不是激進輪詢。
- 顯示清單時連回標準 ClawHub 技能 URL，讓使用者可以檢視來源登錄記錄。
- 使用 `https://clawhub.ai/<owner>/skills/<slug>` 形式的標準頁面 URL。
- 不要暗示 ClawHub 背書、驗證或營運該第三方網站。
- 不要透過繞過公開 API 篩選器或驗證邊界，鏡像隱藏、私人或被審核封鎖的內容。
...
- `X-RateLimit-Remaining` / `RateLimit-Remaining`：存在時為精確剩餘額度；分片的成功請求會省略它，而不是回傳近似的全域值
- `Retry-After`：在 `429` 上等待的延遲秒數
...
- `GET /api/v1/download?slug=&version=&tag=`
  - 代管技能會回傳確定性的 ZIP 位元組。
  - 目前由 GitHub 支援、且掃描結果為 `clean` 或 `suspicious` 的技能，會回傳 JSON `public-github` 交接描述元，而不是 ClawHub 位元組。
- `GET /api/v1/skills/export?startDate=&endDate=&limit=&cursor=`
  - 代管技能會匯出為已儲存的檔案。
  - 目前由 GitHub 支援、且掃描結果為 `clean` 或 `suspicious` 的技能，會匯出為 `public-github` 交接描述元。
...
- `POST /api/v1/users/reserve` 會為擁有者帳號保留根 slug 和私有的未發布套件預留位置。
