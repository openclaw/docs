...
你可以在 ClawHub 的公開讀取 API 之上建立第三方目錄、索引或搜尋介面。公開 Skills 中繼資料和 Skills 檔案會依 ClawHub 的 Skills 授權規則發布，而 API 本身有速率限制，應負責任地使用。

指南：

- 使用公開讀取端點，例如 `GET /api/v1/skills`、`GET /api/v1/search` 和 `GET /api/v1/skills/{slug}` 來取得目錄列表。
- 快取回應，並遵守 `429`、`Retry-After` 和速率限制標頭，而不是頻繁輪詢。
- 顯示列表時連回標準 ClawHub Skills URL，讓使用者可以檢查來源登錄記錄。
...
- `X-RateLimit-Remaining` / `RateLimit-Remaining`：存在時表示精確的剩餘配額；分片成功請求會省略它，而不是回傳近似的全域值
...
- v1 錯誤為純文字（`text/plain; charset=utf-8`），包括 `400`、`401`、`403`、`404`、`429` 和被封鎖的下載回應。
- 未知查詢參數會為了相容性而被忽略。
- 已知查詢參數若值無效會回傳 `400`。
...
- `GET /api/v1/download?slug=&version=&tag=`
  - 託管 Skills 會回傳確定性的 ZIP 位元組。
  - 目前由 GitHub 支援且具有 `clean` 或 `suspicious` 掃描的 Skills，會回傳 JSON `public-github` 交接描述符，而不是 ClawHub 位元組。
- `GET /api/v1/skills/export?startDate=&endDate=&limit=&cursor=`
  - 託管 Skills 會匯出為儲存的檔案。
  - 目前由 GitHub 支援且具有 `clean` 或 `suspicious` 掃描的 Skills，會匯出為 `public-github` 交接描述符。
...
- `POST /api/v1/users/reserve` 會為擁有者 handle 保留根 slug 和私人未發布套件預留位置。
