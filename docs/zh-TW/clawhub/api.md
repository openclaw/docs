...
- 顯示清單時連回標準的 ClawHub 技能 URL，讓使用者可以檢查來源登錄記錄。
...
- `X-RateLimit-Remaining` / `RateLimit-Remaining`：存在時表示精確的剩餘配額；分片的成功請求會省略它，而不是傳回近似的全域值
...
- v1 錯誤為純文字（`text/plain; charset=utf-8`），包括 `400`、`401`、`403`、`404`、`429` 與遭封鎖的下載回應。
- 未知的查詢參數會為了相容性而忽略。
- 已知查詢參數若值無效，會傳回 `400`。
...
- `GET /api/v1/download?slug=&version=&tag=`
  - 託管技能會傳回決定性的 ZIP 位元組。
  - 目前由 GitHub 支援且掃描結果為 `clean` 或 `suspicious` 的技能，會傳回 JSON `public-github` 交接描述元，而不是 ClawHub 位元組。
- `GET /api/v1/skills/export?startDate=&endDate=&limit=&cursor=`
  - 託管技能會匯出為已儲存檔案。
  - 目前由 GitHub 支援且掃描結果為 `clean` 或 `suspicious` 的技能，會匯出為 `public-github` 交接描述元。
...
- `POST /api/v1/users/reserve` 會為擁有者代號保留根 slug 與私人無發行版本套件預留項。
