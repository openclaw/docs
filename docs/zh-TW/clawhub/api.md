...
x-i18n:
    generated_at: "2026-06-30T13:45:07Z"
    model: gpt-5.5
...
你可以在 ClawHub 的公開唯讀 API 之上建置第三方目錄、索引或搜尋介面。公開的 Skills 中繼資料與 Skills 檔案會依 ClawHub 的 Skills 授權規則發布，而 API 本身有速率限制，應以負責任的方式取用。

指引：

- 使用公開唯讀端點，例如 `GET /api/v1/skills`、`GET /api/v1/search` 和 `GET /api/v1/skills/{slug}` 來產生目錄清單。
- 快取回應並遵守 `429`、`Retry-After` 與速率限制標頭，不要過度頻繁輪詢。
- 顯示清單時連回標準的 ClawHub Skills URL，讓使用者可以檢查來源註冊表記錄。
...
- 匿名請求：依 IP。
- 已驗證請求（有效的 Bearer 權杖）：依使用者桶。
- 遺漏或無效權杖會退回使用 IP 強制執行。
...
- `X-RateLimit-Remaining` / `RateLimit-Remaining`：存在時為精確剩餘配額；分片的成功請求會省略它，而不是回傳近似的全域值
...
- v1 錯誤是純文字（`text/plain; charset=utf-8`），包含 `400`、`401`、`403`、`404`、`429` 和被封鎖的下載回應。
- 未知查詢參數會為了相容性而被忽略。
- 已知查詢參數若值無效，會回傳 `400`。
...
- `GET /api/v1/download?slug=&version=&tag=`
  ...
  - 目前以 GitHub 為後端、掃描結果為 `clean` 或 `suspicious` 的 Skills，會回傳 JSON `public-github` 交接描述，而不是 ClawHub 位元組。
- `GET /api/v1/skills/export?startDate=&endDate=&limit=&cursor=`
  - 託管的 Skills 會以已儲存檔案的形式匯出。
  - 目前以 GitHub 為後端、掃描結果為 `clean` 或 `suspicious` 的 Skills，會以 `public-github` 交接描述的形式匯出。
...
- `POST /api/v1/users/reserve` 會為擁有者 handle 保留根 slug 和私有無發行套件佔位符。
