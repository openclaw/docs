...
你可以在 ClawHub 的公開讀取 API 之上建置第三方目錄、索引或搜尋介面。公開的 skill 中繼資料與 skill 檔案會依照 ClawHub 的 skill 授權規則發布，而 API 本身有速率限制，應負責任地使用。

指南：

- 使用公開讀取端點，例如 `GET /api/v1/skills`、`GET /api/v1/search` 和 `GET /api/v1/skills/{slug}` 取得目錄清單。
- 快取回應，並遵守 `429`、`Retry-After` 與速率限制標頭，而不是積極輪詢。
- 顯示清單時連回正式的 ClawHub skill URL，讓使用者可以檢視來源 registry 記錄。
...
- 匿名請求：依 IP。
- 已驗證請求（有效的 Bearer 權杖）：依使用者 bucket。
- 缺少/無效權杖會退回 IP 強制執行。
...
- `X-RateLimit-Remaining` / `RateLimit-Remaining`：存在時為精確的剩餘額度；分片的成功請求會省略它，而不是回傳近似的全域值
- `Retry-After`：在 `429` 上應等待的延遲秒數
...
- v1 錯誤是純文字（`text/plain; charset=utf-8`），包含 `400`、`401`、`403`、`404`、`429` 與遭封鎖下載的回應。
- 未知查詢參數會為了相容性而被忽略。
- 已知查詢參數若值無效，會回傳 `400`。
...
- `GET /api/v1/search?q=...`
  - 可選篩選器：`highlightedOnly=true`、`nonSuspiciousOnly=true`
...
- `GET /api/v1/download?slug=&version=&tag=`
  ...
  - 目前由 GitHub 支援且掃描結果為 `clean` 或 `suspicious` 的 skills，會回傳 JSON `public-github` 交接描述，而非 ClawHub 位元組。
- `GET /api/v1/skills/export?startDate=&endDate=&limit=&cursor=`
  - 託管的 skills 會匯出為已儲存檔案。
  - 目前由 GitHub 支援且掃描結果為 `clean` 或 `suspicious` 的 skills，會匯出為 `public-github` 交接描述。
...
- `POST /api/v1/users/reserve` 會為擁有者 handle 保留根 slugs 與私人未發布 package 預留位置。
