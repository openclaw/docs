...
你可以在 ClawHub 的公開唯讀 API 之上建置第三方目錄、清單或搜尋介面。公開技能中繼資料與技能檔案會依 ClawHub 的技能授權規則發布，而 API 本身具有速率限制，應負責任地使用。
...
- 不要透過繞過公開 API 篩選器或身分驗證邊界來鏡像隱藏、私人或遭審核封鎖的內容。
...
- `POST /api/v1/users/reserve` 會為擁有者控制代碼保留根 slug 與私人無發布套件預留位置。
