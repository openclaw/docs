...
x-i18n:
    generated_at: "2026-07-06T21:46:25Z"
    model: gpt-5.5
...
- 使用公開讀取端點，例如 `GET /api/v1/skills`、`GET /api/v1/search` 和 `GET /api/v1/skills/{slug}` 來提供目錄清單。
- 快取回應，並遵守 `429`、`Retry-After` 與速率限制標頭，不要積極輪詢。
- 顯示清單時連回標準 ClawHub Skills URL，讓使用者可以檢視來源登錄記錄。
- 使用 `https://clawhub.ai/<owner>/skills/<slug>` 形式的標準頁面 URL。
- 不要暗示 ClawHub 背書、驗證或營運該第三方網站。
- 不要藉由繞過公開 API 篩選器或身分驗證邊界，鏡像隱藏、私人或遭審核封鎖的內容。
...
`429` 時會包含 `X-RateLimit-Remaining`、`RateLimit-Remaining` 和 `Retry-After`。
...
- v1 錯誤為純文字（`text/plain; charset=utf-8`），包含 `400`、`401`、`403`、`404`、`429` 與遭封鎖的下載回應。
- 未知查詢參數會為了相容性而忽略。
- 已知查詢參數若值無效會回傳 `400`。
...
- `POST /api/v1/users/reserve` 會為擁有者帳號保留根 slug 與私人未發布套件預留位置。
