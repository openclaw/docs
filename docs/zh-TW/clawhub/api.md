...
x-i18n:
    generated_at: "2026-07-05T04:48:20Z"
    model: gpt-5.5
...
你可以在 ClawHub 的公開讀取 API 之上建置第三方目錄、清單或搜尋介面。公開 skill 中繼資料與 skill 檔案會依照 ClawHub 的 skill 授權規則發布，而 API 本身有速率限制，應負責任地使用。

指南：

- 使用公開讀取端點，例如 `GET /api/v1/skills`、`GET /api/v1/search` 和 `GET /api/v1/skills/{slug}` 來取得目錄列表。
- 快取回應並遵守 `429`、`Retry-After` 和速率限制標頭，而不是積極輪詢。
- 顯示列表時連回正式的 ClawHub skill URL，讓使用者可以檢查來源登錄記錄。
...
- `GET /api/v1/download?slug=&version=&tag=`
  ...
  - 目前由 GitHub 支援且掃描結果為 `clean` 或 `suspicious` 的 skills，會回傳 JSON `public-github` 交接描述元，而不是 ClawHub 位元組。
- `GET /api/v1/skills/export?startDate=&endDate=&limit=&cursor=`
  - 託管 skills 會匯出為已儲存的檔案。
  - 目前由 GitHub 支援且掃描結果為 `clean` 或 `suspicious` 的 skills，會匯出為 `public-github` 交接描述元。
...
- `POST /api/v1/users/reserve` 會為擁有者 handle 保留根 slugs 和私人無發行版本套件預留位置。
