...
你可以在 ClawHub 的公開唯讀 API 之上建構第三方目錄、索引或搜尋介面。公開 skill 中繼資料與 skill 檔案依 ClawHub 的 skill 授權規則發布，而 API 本身有速率限制，應負責任地使用。

指引：

- 使用公開唯讀端點，例如 `GET /api/v1/skills`、`GET /api/v1/search` 和 `GET /api/v1/skills/{slug}` 取得目錄清單。
- 快取回應並遵守 `429`、`Retry-After` 與速率限制標頭，避免積極輪詢。
...
- `GET /api/v1/skills?limit=&cursor=&sort=`
  ...
  - 舊版別名：`nonSuspicious=true`
  - 使用 `nonSuspiciousOnly=true` 時，游標式頁面可能包含少於 `limit` 個項目；請使用 `nextCursor` 繼續。
...
- `POST /api/v1/users/reserve` 會為擁有者代號保留根 slug 與私人未發布套件預留位置。
