...
- 顯示清單時連回標準 ClawHub Skills URL，讓使用者可以檢視來源登錄紀錄。
...
- `X-RateLimit-Remaining` / `RateLimit-Remaining`：存在時為精確剩餘配額；分片成功請求會省略它，而不是傳回近似的全域值
...
- v1 錯誤是純文字（`text/plain; charset=utf-8`），包括 `400`、`401`、`403`、`404`、`429` 和遭封鎖下載回應。
- 未知查詢參數會因相容性而被忽略。
- 具有無效值的已知查詢參數會傳回 `400`。
...
- `GET /api/v1/download?slug=&version=&tag=`
  - 託管 Skills 會傳回確定性的 ZIP 位元組。
  - 目前由 GitHub 支援、掃描結果為 `clean` 或 `suspicious` 的 Skills，會傳回 JSON `public-github` 交接描述元，而不是 ClawHub 位元組。
- `GET /api/v1/skills/export?startDate=&endDate=&limit=&cursor=`
  - 託管 Skills 會匯出為已儲存檔案。
  - 目前由 GitHub 支援、掃描結果為 `clean` 或 `suspicious` 的 Skills，會匯出為 `public-github` 交接描述元。
...
- `POST /api/v1/users/reserve` 會為擁有者代號保留根 slug 和私人無發布套件佔位符。
