...
x-i18n:
    generated_at: "2026-06-28T05:08:19Z"
    model: gpt-5.5
...
- 快取回應，並遵守 `429`、`Retry-After` 與速率限制標頭，避免過度頻繁輪詢。
...
- `X-RateLimit-Remaining` / `RateLimit-Remaining`：存在時為精確剩餘配額；分片成功請求會省略它，而不是回傳近似的全域值
...
- `GET /api/v1/download?slug=&version=&tag=`
  - 託管技能會回傳確定性的 ZIP 位元組。
  - 目前由 GitHub 支援、且有 `clean` 或 `suspicious` 掃描的技能，會回傳 JSON `public-github` 交接描述元，而不是 ClawHub 位元組。
- `GET /api/v1/skills/export?startDate=&endDate=&limit=&cursor=`
  - 託管技能會匯出為已儲存檔案。
  - 目前由 GitHub 支援、且有 `clean` 或 `suspicious` 掃描的技能，會匯出為 `public-github` 交接描述元。
...
- `POST /api/v1/users/reserve` 會為擁有者 handle 保留根 slug 與私人未發行套件預留位置。
