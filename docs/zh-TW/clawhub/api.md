...
你可以在 ClawHub 的公開讀取 API 之上建立第三方目錄、索引或搜尋介面。公開技能中繼資料與技能檔案會依照 ClawHub 的技能授權規則發布，而 API 本身有速率限制，應負責任地使用。
...
- `GET /api/v1/download?slug=&version=&tag=`
  - 託管技能會回傳確定性的 ZIP 位元組。
  - 目前由 GitHub 支援、且掃描結果為 `clean` 或 `suspicious` 的技能會回傳 JSON `public-github` 交接描述，而不是 ClawHub 位元組。
- `GET /api/v1/skills/export?startDate=&endDate=&limit=&cursor=`
  - 託管技能會以儲存檔案匯出。
  - 目前由 GitHub 支援、且掃描結果為 `clean` 或 `suspicious` 的技能會匯出為 `public-github` 交接描述。
...
- `POST /api/v1/users/reserve` 會為擁有者 handle 保留根 slugs 和私人未發布套件預留位置。
