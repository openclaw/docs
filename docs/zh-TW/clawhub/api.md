...
x-i18n:
    generated_at: "2026-07-01T15:18:45Z"
    model: gpt-5.5
...
你可以在 ClawHub 的公開讀取 API 之上建構第三方目錄、索引或搜尋介面。公開 Skill 中繼資料與 Skill 檔案會依 ClawHub 的 Skill 授權規則發布，而 API 本身有速率限制，應負責任地取用。

指南：

- 使用公開讀取端點，例如 `GET /api/v1/skills`、`GET /api/v1/search` 和 `GET /api/v1/skills/{slug}` 來取得目錄列表。
- 快取回應並遵守 `429`、`Retry-After` 和速率限制標頭，而不是積極輪詢。
- 顯示列表時連回標準 ClawHub Skill URL，讓使用者可以檢視來源登錄記錄。
...
- v1 錯誤是純文字（`text/plain; charset=utf-8`），包含 `400`、`401`、`403`、`404`、`429` 和遭封鎖的下載回應。
- 未知查詢參數會基於相容性被忽略。
- 已知查詢參數若值無效會回傳 `400`。
...
- `GET /api/v1/skills?limit=&cursor=&sort=`
  ...
  - 舊版別名：`nonSuspicious=true`
  - 使用 `nonSuspiciousOnly=true` 時，游標型頁面可能包含少於 `limit` 的項目；使用 `nextCursor` 繼續。
  - `recommended` 使用互動度與時近性訊號。
...
- `POST /api/v1/users/reserve` 會為擁有者帳號保留根 slug 和私有無發布套件預留位置。
