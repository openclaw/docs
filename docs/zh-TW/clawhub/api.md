...
x-i18n:
    generated_at: "2026-06-27T19:00:31Z"
    model: gpt-5.5
...
- 使用公開讀取端點，例如 `GET /api/v1/skills`、`GET /api/v1/search` 與 `GET /api/v1/skills/{slug}` 來取得目錄清單。
- 快取回應，並遵守 `429`、`Retry-After` 與速率限制標頭，而不是積極輪詢。
- 顯示清單時連回標準的 ClawHub Skills URL，讓使用者可以檢視來源登錄紀錄。
- 使用 `https://clawhub.ai/<owner>/skills/<slug>` 格式的標準頁面 URL。
...
`X-RateLimit-Remaining`、`RateLimit-Remaining` 與 `Retry-After` 會包含在 `429` 中。
...
- v1 錯誤是純文字（`text/plain; charset=utf-8`），包括 `400`、
  `401`、`403`、`404`、`429` 與遭封鎖的下載回應。
- 未知查詢參數會為了相容性而忽略。
- 已知查詢參數若值無效，會回傳 `400`。
...
- `GET /api/v1/download?slug=&version=&tag=`
  ...
  - 目前由 GitHub 支援、且掃描結果為 `clean` 或 `suspicious` 的 Skills，會回傳
    JSON `public-github` 交接描述元，而不是 ClawHub 位元組。
- `GET /api/v1/skills/export?startDate=&endDate=&limit=&cursor=`
  - 託管 Skills 會匯出為已儲存檔案。
  - 目前由 GitHub 支援、且掃描結果為 `clean` 或 `suspicious` 的 Skills，會匯出
    為 `public-github` 交接描述元。
...
- `POST /api/v1/users/reserve` 會為擁有者帳號保留根 slug 與私人無發布套件預留項。
