...
網頁版 GitHub 匯入器比本機發布/同步更嚴格。它只會在已登入 GitHub 帳號擁有的公開、非 fork 儲存庫中探索
...
技能中繼資料會在你的 `SKILL.md` 頂端以 YAML frontmatter 宣告。這會告訴登錄檔（以及安全性分析）你的技能執行時需要什麼。
...
針對技能執行前必須存在的環境變數，使用 `requires.env`。當你需要每個變數的中繼資料時，請使用 `envVars`，包含帶有 `required: false` 的可選變數。
...
| `envVars`          | `array`    | 環境變數宣告，包含 `name`、可選的 `required`，以及可選的 `description`。針對可選環境變數設定 `required: false`。                         |
...
在 `metadata.openclaw.envVars` 下宣告可選環境變數，並設定 `required: false`。不要將可選項目加入 `requires.env`，因為 `requires.env` 表示技能沒有它們就無法執行。
...
ClawHub 的安全性分析會檢查你的技能宣告內容是否符合它實際執行的行為。如果你的程式碼引用 `TODOIST_API_KEY`，但 frontmatter 沒有在 `requires.env`、`primaryEnv` 或 `envVars` 下宣告它，分析就會標記中繼資料不相符。保持宣告準確，有助於你的技能通過審查，也能協助使用者了解自己正在安裝什麼。
...
- 以 `text/` 開頭的內容類型會被視為文字；另有小型允許清單（JSON/YAML/TOML/JS/TS/Markdown/SVG）。

限制（伺服器端）：

- 總套件大小：50MB。
- 嵌入文字包含 `SKILL.md` + 最多約 40 個非 `.md` 檔案（盡力限制）。

## Slugs

- 預設從資料夾名稱衍生。
- 套件 scope 必須與 ClawHub 發布者 handle 完全相符。發布者 handle 可以使用小寫字母、數字、連字號、點和底線；必須以小寫字母或數字開頭與結尾。
...
- 所有發布到 ClawHub 的技能都依 `MIT-0` 授權。
- 任何人都可以使用、修改並重新散布已發布的技能，包括商業用途。
- 不需要署名。
...
- 不要將定價中繼資料加入 `SKILL.md`；它不是技能格式的一部分，也不會讓已發布技能變成付費。
- 如果你的技能整合付費第三方服務，請在技能指示與環境變數宣告中清楚記錄外部成本與必要帳號（必要變數使用 `requires.env`，可選變數使用帶有 `required: false` 的 `envVars`）。
