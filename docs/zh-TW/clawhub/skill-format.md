...
網頁版 GitHub 匯入器比本機發布/同步更嚴格。它只會在登入的 GitHub 帳號所擁有的公開、非 fork 儲存庫中探索
...
Skill 中繼資料是在 `SKILL.md` 頂端的 YAML frontmatter 中宣告。這會告訴登錄檔（以及安全性分析）你的 Skill 需要什麼才能執行。
...
針對 Skill 執行前必須存在的環境變數使用 `requires.env`。當你需要每個變數的中繼資料，包括帶有 `required: false` 的選用變數時，請使用 `envVars`。
...
如果你的 Skill 需要安裝相依項，請在 `install` 陣列中宣告：
...
ClawHub 的安全性分析會檢查你的 Skill 宣告內容是否符合它實際執行的行為。如果你的程式碼參照 `TODOIST_API_KEY`，但 frontmatter 沒有在 `requires.env`、`primaryEnv` 或 `envVars` 下宣告它，分析會標記中繼資料不相符。保持宣告準確，有助於你的 Skill 通過審查，也能幫助使用者了解他們正在安裝什麼。
...
- 上傳後仍會掃描指令碼檔案；PowerShell `.ps1`、`.psm1` 和 `.psd1` 檔案會被視為文字而接受。
...
- 預設從資料夾名稱衍生。
- 套件 scope 必須與 ClawHub 發布者 handle 完全相符。發布者 handle 可以使用小寫字母、數字、連字號、點和底線；必須以小寫字母或數字開頭和結尾。
...
- 不需要署名。
- 不要在 `SKILL.md` 中新增衝突的授權條款；ClawHub 不支援逐 Skill 覆寫授權。

## 付費 Skills

- ClawHub 不支援付費 Skills、逐 Skill 定價、付費牆或收益分潤。
- 不要將定價中繼資料新增到 `SKILL.md`；它不是 Skill 格式的一部分，也不會讓已發布的 Skill 變成付費。
- 如果你的 Skill 與付費第三方服務整合，請在 Skill 指示和 env 宣告中清楚記錄外部成本和必要帳號（必要變數使用 `requires.env`，選用變數則使用帶有 `required: false` 的 `envVars`）。
