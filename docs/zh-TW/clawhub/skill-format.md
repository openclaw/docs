...
- `.clawhubignore`（發布/同步用的忽略模式，舊版 `.clawdhubignore`）
- `.gitignore`（同樣會套用）
...
技能中繼資料宣告於 `SKILL.md` 頂端的 YAML frontmatter。這會告訴登錄檔（以及安全性分析）你的技能需要什麼才能執行。
...
對技能執行前必須存在的環境變數使用 `requires.env`。當你需要逐一變數的中繼資料時，請使用 `envVars`，包含帶有 `required: false` 的選填變數。
...
| `envVars`          | `array`    | 帶有 `name`、選填 `required` 和選填 `description` 的環境變數宣告。若為選填環境變數，請設定 `required: false`。 |
...
在 `metadata.openclaw.envVars` 下宣告選填環境變數，並設定 `required: false`。不要將選填項目加入 `requires.env`，因為 `requires.env` 表示技能沒有它們就無法執行。
...
ClawHub 的安全性分析會檢查你的技能宣告是否與其實際行為相符。如果你的程式碼參照 `TODOIST_API_KEY`，但 frontmatter 沒有在 `requires.env`、`primaryEnv` 或 `envVars` 下宣告它，分析就會標記中繼資料不相符。保持宣告準確，有助於技能通過審查，也能幫助使用者了解自己正在安裝什麼。
...
- 如果你的技能整合付費第三方服務，請在技能指示和環境變數宣告中清楚記錄外部費用與必要帳號（必要變數使用 `requires.env`，選填變數使用帶有 `required: false` 的 `envVars`）。
