...
x-i18n:
    generated_at: "2026-07-01T05:28:11Z"
    model: gpt-5.5
...
- 任何支援用的_文字型_檔案（請見「允許的檔案」）
- `.clawhubignore`（發布時的忽略模式，舊版為 `.clawdhubignore`）
- `.gitignore`（也會被採用）

## GitHub 匯入

網頁版 GitHub 匯入器比本機發布/同步更嚴格。它只會探索由已登入 GitHub 帳號擁有、公開、非 fork 儲存庫中的
...
對技能執行前必須存在的環境變數使用 `requires.env`。需要每個變數的中繼資料時，請使用 `envVars`，包括帶有 `required: false` 的可選變數。
...
| `nix`              | `object`   | Nix 外掛規格（請見 README）。                                                                                                                |
| `config`           | `object`   | Clawdbot 設定規格（請見 README）。                                                                                                           |

### 安裝規格

如果技能需要安裝相依項目，請在 `install` 陣列中宣告：
...
ClawHub 的安全性分析會檢查技能宣告的內容是否符合實際行為。如果你的程式碼參照 `TODOIST_API_KEY`，但 frontmatter 沒有在 `requires.env`、`primaryEnv` 或 `envVars` 下宣告它，分析會標記中繼資料不相符。維持宣告準確有助於技能通過審查，也能幫助使用者了解他們正在安裝什麼。
...
- ClawHub 不支援付費技能、每技能定價、付費牆或收入分潤。
- 不要將定價中繼資料加入 `SKILL.md`；它不是技能格式的一部分，也不會讓已發布的技能變成付費技能。
- 如果你的技能整合付費第三方服務，請在技能指示和環境變數宣告中清楚記錄外部成本和必要帳號（必要變數使用 `requires.env`，或對可選變數使用帶有 `required: false` 的 `envVars`）。
