...
x-i18n:
    generated_at: "2026-07-04T20:24:01Z"
    model: gpt-5.5
...
網頁版 GitHub 匯入工具比本機發布/同步更嚴格。它只會在由已登入 GitHub 帳戶擁有的公開、非 fork 儲存庫中探索
...
針對技能執行前必須存在的環境變數使用 `requires.env`。需要每個變數的中繼資料時，請使用 `envVars`，包含帶有 `required: false` 的選用變數。
...
ClawHub 的安全性分析會檢查技能宣告的內容是否符合它實際執行的行為。如果你的程式碼參照 `TODOIST_API_KEY`，但 frontmatter 沒有在 `requires.env`、`primaryEnv` 或 `envVars` 下宣告它，分析會標記中繼資料不相符。保持宣告準確，有助於技能通過審查，也能協助使用者了解他們正在安裝什麼。
...
- 總套件大小：50MB。
- 嵌入文字包含 `SKILL.md` 加上最多約 40 個非 `.md` 檔案（盡力上限）。
...
- 如果技能整合付費的第三方服務，請在技能說明和環境變數宣告中清楚記錄外部費用與必要帳戶（必要變數使用 `requires.env`，選用變數則使用帶有 `required: false` 的 `envVars`）。
