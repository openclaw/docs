...
網頁版 GitHub 匯入工具比本機發布/同步更嚴格。它只會在已登入 GitHub 帳號所擁有的公開、非 fork 儲存庫中探索
...
技能中繼資料宣告在 `SKILL.md` 頂端的 YAML frontmatter 中。這會告訴登錄系統（以及安全性分析）你的技能執行時需要什麼。
...
在 `metadata.openclaw` 底下宣告你的技能執行階段需求（別名：`metadata.clawdbot`、`metadata.clawdis`）。
...
對技能執行前必須存在的環境變數使用 `requires.env`。當你需要個別變數的中繼資料時，請使用 `envVars`，包括帶有 `required: false` 的選用變數。
...
在 `metadata.openclaw.envVars` 底下宣告選用環境變數，並設定 `required: false`。不要將選用項目加入 `requires.env`，因為 `requires.env` 表示沒有它們時技能無法執行。
...
ClawHub 的安全性分析會檢查你的技能宣告內容是否符合它實際執行的內容。如果你的程式碼參照 `TODOIST_API_KEY`，但你的 frontmatter 沒有在 `requires.env`、`primaryEnv` 或 `envVars` 底下宣告它，分析會標記中繼資料不相符。保持宣告準確，有助於你的技能通過審查，也能協助使用者了解他們正在安裝什麼。
...
- 預設從資料夾名稱衍生。
- 套件 scope 必須完全符合 ClawHub 發布者 handle。發布者 handle 可以使用小寫字母、數字、連字號、句點和底線；必須以小寫字母或數字開頭和結尾。
...
- 如果你的技能整合了付費第三方服務，請在技能說明與環境變數宣告中清楚記錄外部費用和所需帳號（必要變數使用 `requires.env`，選用變數則使用帶有 `required: false` 的 `envVars`）。
