...
x-i18n:
    generated_at: "2026-07-16T11:30:20Z"
    model: gpt-5.6
...
- `SKILL.md`（或 `skill.md`；亦接受舊版 `skills.md`）
...
網頁版 GitHub 匯入工具比本機發布／同步更嚴格。它只會探索已登入 GitHub 帳號所擁有之公開、非分支儲存庫中的
...
對於可攜式 Agent Skills，`name` 應與上層目錄相符，並使用
1–64 個小寫字母、數字或連字號。ClawHub 會將可路由的 slug 與
目錄顯示名稱分開，因此其他用戶端的現有名稱仍可發布，且不會被
無提示地改寫。目錄清單可能會在視覺上縮短過長的名稱，
但不會變更儲存的名稱。

## Frontmatter 中繼資料

Skill 中繼資料是在 `SKILL.md` 頂端的 YAML frontmatter 中宣告。這會告知登錄檔（及安全性分析）Skill 執行時需要哪些項目。
...
對於 Skill 執行前必須存在的環境變數，請使用 `requires.env`。需要個別變數的中繼資料時，請使用 `envVars`，包括透過 `required: false` 宣告的選用變數。
...
| `envVars`          | `array`    | 包含 `name`、選用 `required` 及選用 `description` 的環境變數宣告。選用環境變數請設定 `required: false`。 |
| `always`           | `boolean`  | 若為 `true`，Skill 將一律啟用（不需要明確安裝）。                                                                                 |
...
若 Skill 需要安裝相依套件，請在 `install` 陣列中宣告：
...
請在 `metadata.openclaw.envVars` 下宣告選用環境變數，並設定 `required: false`。請勿將選用項目加入 `requires.env`，因為 `requires.env` 表示缺少它們時 Skill 無法執行。
...
ClawHub 的安全性分析會檢查 Skill 宣告的內容是否與其實際行為相符。如果程式碼參照 `TODOIST_API_KEY`，但 frontmatter 未在 `requires.env`、`primaryEnv` 或 `envVars` 下宣告，分析便會標記中繼資料不相符。維持宣告的正確性有助於 Skill 通過審查，也能協助使用者瞭解其安裝內容。
...
- 指令碼檔案在上傳後仍會接受掃描；PowerShell `.ps1`、`.psm1` 和 `.psd1` 檔案會視為文字接受。
- 以 `text/` 開頭的內容類型會視為文字；另有一份小型允許清單（JSON／YAML／TOML／JS／TS／Markdown／SVG）。

限制（伺服器端）：

- 套件總大小：50MB。
- 嵌入文字包括 `SKILL.md` 加上最多約 40 個非 `.md` 檔案（盡力而為的上限）。

## Slug

- 預設從資料夾名稱衍生。
- 套件範圍必須與 ClawHub 發布者代稱完全相符。發布者代稱可使用小寫字母、數字、連字號、句點及底線；開頭和結尾必須是小寫字母或數字。
...
- 所有發布至 ClawHub 的 Skill 均採用 `MIT-0` 授權。
- 任何人都可以使用、修改及轉散布已發布的 Skill，包括商業用途。
- 不需要標示出處。
...
- 如果 Skill 整合付費第三方服務，請在 Skill 指示及環境變數宣告中，清楚說明外部費用與必要帳號（必要變數使用 `requires.env`，選用變數則使用 `envVars` 搭配 `required: false`）。
