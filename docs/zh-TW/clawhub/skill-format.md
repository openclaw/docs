...
- Markdown，可選用 YAML 前置資料。
- 伺服器會在發布時從前置資料擷取中繼資料。
- `description` 會在 UI/搜尋中作為 Skill 摘要。

## 前置資料中繼資料

Skill 中繼資料會在 `SKILL.md` 頂端的 YAML 前置資料中宣告。這會告訴註冊表（以及安全性分析）你的 Skill 需要什麼才能執行。
...
對於 Skill 執行前必須存在的環境變數，請使用 `requires.env`。當你需要逐一變數的中繼資料，包括含有 `required: false` 的選用變數時，請使用 `envVars`。
...
在 `metadata.openclaw.envVars` 下宣告選用環境變數，並設定 `required: false`。請勿將選用項目加入 `requires.env`，因為 `requires.env` 表示 Skill 沒有它們就無法執行。
...
ClawHub 的安全性分析會檢查 Skill 宣告的內容是否與實際行為相符。如果你的程式碼參照 `TODOIST_API_KEY`，但前置資料沒有在 `requires.env`、`primaryEnv` 或 `envVars` 下宣告它，分析就會標記中繼資料不符。維持宣告準確有助於 Skill 通過審查，也能協助使用者了解自己正在安裝什麼。
...
- 總套件大小：50MB。
- 內嵌文字包含 `SKILL.md` + 最多約 40 個非 `.md` 檔案（盡力上限）。
...
- 所有發布到 ClawHub 的 Skills 都採用 `MIT-0` 授權。
...
- ClawHub 不支援付費 Skills、逐 Skill 定價、付費牆或營收分潤。
- 請勿將定價中繼資料加入 `SKILL.md`；它不是 Skill 格式的一部分，也不會讓已發布的 Skill 變成付費項目。
- 如果你的 Skill 整合付費的第三方服務，請在 Skill 說明和環境宣告中清楚記錄外部費用和必要帳號（必要變數使用 `requires.env`，選用變數則使用含 `required: false` 的 `envVars`）。
