...
- `.clawhubignore`（發布時使用的忽略模式，舊版 `.clawdhubignore`）
- `.gitignore`（也會遵循）
...
技能中繼資料是在你的 `SKILL.md` 頂端 YAML frontmatter 中宣告。這會告訴登錄檔（以及安全性分析）你的技能執行時需要什麼。
...
針對技能執行前必須存在的環境變數，請使用 `requires.env`。如果需要每個變數的中繼資料，包括 `required: false` 的選用變數，請使用 `envVars`。
...
ClawHub 的安全性分析會檢查你的技能宣告內容是否符合它實際執行的動作。如果你的程式碼參照 `TODOIST_API_KEY`，但 frontmatter 沒有在 `requires.env`、`primaryEnv` 或 `envVars` 下宣告它，分析就會標示中繼資料不符。保持宣告準確有助於你的技能通過審查，也能協助使用者了解他們正在安裝什麼。
...
- 上傳後仍會掃描指令碼檔案；PowerShell `.ps1`、`.psm1` 和 `.psd1` 檔案會被視為文字接受。
...
- 預設從資料夾名稱衍生。
- 套件 scope 必須完全符合 ClawHub 發布者 handle。發布者 handle 可以使用小寫字母、數字、連字號、點和底線；必須以小寫字母或數字開頭與結尾。
...
- 如果你的技能整合付費第三方服務，請在技能說明和環境變數宣告中清楚記錄外部費用與必要帳號（必要變數使用 `requires.env`，或選用變數使用含 `required: false` 的 `envVars`）。
