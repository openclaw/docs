...
使用 `requires.env` 指定技能執行前必須存在的環境變數。需要逐變數中繼資料時，請使用 `envVars`，包括具有 `required: false` 的選用變數。
...
在 `metadata.openclaw.envVars` 下宣告選用環境變數，並設定 `required: false`。不要將選用項目加入 `requires.env`，因為 `requires.env` 表示沒有它們時技能無法執行。
...
ClawHub 的安全分析會檢查你的技能宣告內容是否符合它實際執行的內容。如果你的程式碼參照 `TODOIST_API_KEY`，但 frontmatter 未在 `requires.env`、`primaryEnv` 或 `envVars` 下宣告它，分析就會標記中繼資料不相符。保持宣告準確有助於你的技能通過審查，也能幫助使用者了解他們正在安裝什麼。
...
- 每次發布都會建立新版本（semver）。
- 標籤是指向版本的字串指標；`latest` 常用於此用途。

## 授權

- 所有在 ClawHub 發布的技能都以 `MIT-0` 授權。
- 任何人都可以使用、修改及重新散布已發布技能，包括商業用途。
- 不需要署名。
...
- 不要將定價中繼資料加入 `SKILL.md`；它不是技能格式的一部分，也不會讓已發布技能變成付費技能。
- 如果你的技能整合了付費第三方服務，請在技能說明和環境變數宣告中清楚記載外部費用與必要帳戶（必要變數使用 `requires.env`，或選用變數使用具有 `required: false` 的 `envVars`）。
