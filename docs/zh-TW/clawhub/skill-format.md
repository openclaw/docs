...
針對技能執行前必須存在的環境變數，使用 `requires.env`。當你需要每個變數的中繼資料時，請使用 `envVars`，包括帶有 `required: false` 的選用變數。
...
在 `metadata.openclaw.envVars` 下宣告選用環境變數，並設定 `required: false`。不要將選用項目加入 `requires.env`，因為 `requires.env` 表示沒有它們技能就無法執行。
...
ClawHub 的安全性分析會檢查你的技能宣告是否符合實際行為。如果你的程式碼參照 `TODOIST_API_KEY`，但 frontmatter 沒有在 `requires.env`、`primaryEnv` 或 `envVars` 下宣告它，分析就會標記中繼資料不相符。保持宣告準確有助於你的技能通過審查，也能幫助使用者理解他們正在安裝什麼。
...
- 以 `text/` 開頭的內容類型會視為文字；此外還有一小份允許清單（JSON/YAML/TOML/JS/TS/Markdown/SVG）。
...
- 不要將定價中繼資料加入 `SKILL.md`；這不是技能格式的一部分，也不會讓已發布的技能變成付費。
- 如果你的技能整合付費第三方服務，請在技能指示和環境變數宣告中清楚記錄外部費用和所需帳戶（必要變數使用 `requires.env`，選用變數則使用帶有 `required: false` 的 `envVars`）。
