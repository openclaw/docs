...
x-i18n:
    generated_at: "2026-07-01T15:19:13Z"
    model: gpt-5.5
...
網頁版 GitHub 匯入工具比本機發布/同步更嚴格。它只會在已登入 GitHub 帳戶擁有的公開、非 fork 儲存庫中探索
...
- `description` 會作為介面/搜尋中的技能摘要。

## Frontmatter 中繼資料

技能中繼資料宣告在 `SKILL.md` 頂端的 YAML frontmatter 中。這會告訴登錄檔（以及安全性分析）你的技能執行時需要什麼。
...
使用 `requires.env` 表示技能執行前必須存在的環境變數。需要逐變數中繼資料時，請使用 `envVars`，包含帶有 `required: false` 的選用變數。
...
在 `metadata.openclaw.envVars` 底下宣告選用環境變數，並設定 `required: false`。不要把選用項目加入 `requires.env`，因為 `requires.env` 表示技能缺少它們就無法執行。
...
ClawHub 的安全性分析會檢查技能宣告的內容是否符合它實際執行的動作。如果你的程式碼參照 `TODOIST_API_KEY`，但 frontmatter 沒有在 `requires.env`、`primaryEnv` 或 `envVars` 底下宣告它，分析就會標記中繼資料不一致。保持宣告準確有助於技能通過審查，也能幫助使用者理解他們正在安裝什麼。
...
- 指令碼檔案在上傳後仍會被掃描；PowerShell `.ps1`、`.psm1` 和 `.psd1` 檔案會被接受為文字。
...
- 預設從資料夾名稱衍生。
- 套件 scope 必須完全符合 ClawHub 發布者 handle。發布者 handle 可使用小寫字母、數字、連字號、句點和底線；必須以小寫字母或數字開頭和結尾。
- 套件 slug 必須為小寫且 npm 安全，例如 `@example.tools/demo-plugin` 或 `demo-plugin`。
...
- 不要在 `SKILL.md` 中加入定價中繼資料；它不是技能格式的一部分，也不會讓已發布技能變成付費技能。
- 如果你的技能整合付費第三方服務，請在技能說明和環境變數宣告中清楚記錄外部費用和必要帳戶（必要變數用 `requires.env`，選用變數則用帶有 `required: false` 的 `envVars`）。
