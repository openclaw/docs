...
x-i18n:
    generated_at: "2026-07-05T04:47:26Z"
    model: gpt-5.5
...
由已登入 GitHub 帳號擁有、公開、非 fork 儲存庫中的
...
對於技能執行前必須存在的環境變數，請使用 `requires.env`。當你需要每個變數的中繼資料時，請使用 `envVars`，包含具備 `required: false` 的選用變數。
...
在 `metadata.openclaw.envVars` 底下宣告選用環境變數，並設定 `required: false`。不要將選用項目加入 `requires.env`，因為 `requires.env` 代表缺少它們時技能無法執行。
...
ClawHub 的安全性分析會檢查技能宣告的內容是否符合它實際執行的動作。如果你的程式碼引用 `TODOIST_API_KEY`，但 frontmatter 沒有在 `requires.env`、`primaryEnv` 或 `envVars` 底下宣告它，分析就會標記中繼資料不相符。保持宣告準確有助於技能通過審查，也能幫助使用者了解自己正在安裝什麼。
...
- 每次發布都會建立新版本（semver）。
- 標籤是指向版本的字串指標；常見用法是 `latest`。

## 授權

- 所有發布在 ClawHub 上的技能皆依 `MIT-0` 授權。
...
- 如果你的技能整合付費的第三方服務，請在技能指示和環境變數宣告中清楚記錄外部費用與必要帳號（必要變數使用 `requires.env`，選用變數使用含有 `required: false` 的 `envVars`）。
