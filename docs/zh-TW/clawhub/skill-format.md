...
網頁版 GitHub 匯入器比本機發布/同步更嚴格。它只會探索登入 GitHub 帳號所擁有的公開、非 fork 儲存庫中的
...
在 `metadata.openclaw.envVars` 底下宣告選用環境變數，並設定 `required: false`。不要將選用項目加入 `requires.env`，因為 `requires.env` 表示沒有它們技能就無法執行。
...
ClawHub 的安全性分析會檢查技能宣告的內容是否符合它實際執行的行為。如果你的程式碼參照 `TODOIST_API_KEY`，但 frontmatter 未在 `requires.env`、`primaryEnv` 或 `envVars` 底下宣告它，分析就會標記中繼資料不相符。保持宣告準確有助於技能通過審查，也能幫助使用者了解他們正在安裝什麼。
...
- 預設從資料夾名稱衍生。
- 套件 scope 必須與 ClawHub 發布者 handle 完全相符。發布者 handle 可使用小寫字母、數字、連字號、句點和底線；它們必須以小寫字母或數字開頭與結尾。
...
- 如果你的技能整合付費第三方服務，請在技能指示和 env 宣告中清楚記錄外部成本與必要帳號（必要變數使用 `requires.env`，或選用變數使用帶有 `required: false` 的 `envVars`）。
