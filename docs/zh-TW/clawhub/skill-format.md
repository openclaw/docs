...
x-i18n:
    generated_at: "2026-07-06T21:47:08Z"
    model: gpt-5.5
...
技能中繼資料會宣告在 `SKILL.md` 頂端的 YAML frontmatter 中。這會告訴註冊表（和安全性分析）你的技能需要什麼才能執行。
...
ClawHub 的安全性分析會檢查技能宣告的內容是否符合它實際執行的行為。如果你的程式碼參照 `TODOIST_API_KEY`，但 frontmatter 沒有在 `requires.env`、`primaryEnv` 或 `envVars` 下宣告它，分析會標示中繼資料不相符。保持宣告準確有助於技能通過審查，也能幫助使用者了解他們正在安裝什麼。
...
- 如果你的技能整合付費第三方服務，請在技能說明和環境變數宣告中清楚記錄外部費用和必要帳號（必要變數使用 `requires.env`，或針對選用變數使用帶有 `required: false` 的 `envVars`）。
