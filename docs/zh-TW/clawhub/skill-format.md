...
網頁版 GitHub 匯入器比本機發布/同步更嚴格。它只會在登入的 GitHub 帳號所擁有、公開且非 fork 的儲存庫中探索
...
- `description` 會在 UI/搜尋中用作技能摘要。

## Frontmatter 中繼資料

技能中繼資料會宣告在 `SKILL.md` 頂端的 YAML frontmatter 中。這會告訴註冊表（以及安全性分析）你的技能執行時需要什麼。
...
| `primaryEnv`       | `string`   | 你的技能主要的憑證環境變數。                                                                                                                 |
...
ClawHub 的安全性分析會檢查你的技能宣告內容是否符合它實際執行的內容。如果你的程式碼參照 `TODOIST_API_KEY`，但 frontmatter 沒有在 `requires.env`、`primaryEnv` 或 `envVars` 下宣告它，分析就會標記中繼資料不相符。保持宣告準確有助於你的技能通過審查，也能協助使用者了解他們正在安裝什麼。
...
- 以 `text/` 開頭的內容類型會被視為文字；另有一小組允許清單（JSON/YAML/TOML/JS/TS/Markdown/SVG）。
...
- 預設由資料夾名稱衍生。
- 套件 scope 必須與 ClawHub 發布者 handle 完全相符。發布者 handle 可以使用小寫字母、數字、連字號、點和底線；且必須以小寫字母或數字開頭和結尾。
...
- 不需要署名。
- 不要在 `SKILL.md` 中加入衝突的授權條款；ClawHub 不支援逐技能授權覆寫。

## 付費技能

- ClawHub 不支援付費技能、逐技能定價、付費牆或收益分潤。
- 不要將定價中繼資料加入 `SKILL.md`；它不是技能格式的一部分，也不會讓已發布的技能變成付費技能。
- 如果你的技能整合付費第三方服務，請在技能指示和環境變數宣告中清楚記錄外部費用與必要帳號（必要變數使用 `requires.env`，或選用變數使用帶有 `required: false` 的 `envVars`）。
