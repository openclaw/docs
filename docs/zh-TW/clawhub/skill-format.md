...
網頁版 GitHub 匯入工具比本機發布/同步更嚴格。它只會發現
登入 GitHub 帳戶擁有的公開、非 fork 儲存庫中的
...
使用 `requires.env` 宣告技能執行前必須存在的環境變數。當你需要每個變數的中繼資料時，使用 `envVars`，包括帶有 `required: false` 的選用變數。
...
ClawHub 的安全性分析會檢查你的技能宣告內容是否符合它實際執行的行為。如果你的程式碼參照 `TODOIST_API_KEY`，但 frontmatter 沒有在 `requires.env`、`primaryEnv` 或 `envVars` 下宣告它，分析會標記中繼資料不相符。維持準確的宣告有助於你的技能通過審查，也能幫助使用者了解他們正在安裝什麼。
...
- 上傳後仍會掃描指令碼檔案；PowerShell `.ps1`、`.psm1` 和 `.psd1` 檔案會作為文字接受。
...
- 預設從資料夾名稱衍生。
- 套件 scope 必須與 ClawHub 發布者 handle 完全相符。發布者 handle 可以使用小寫字母、數字、連字號、句點和底線；它們必須以小寫字母或數字開頭和結尾。
...
- 所有發布在 ClawHub 上的技能均採用 `MIT-0` 授權。
...
- 如果你的技能整合了付費第三方服務，請在技能指示和環境變數宣告中清楚記錄外部費用與必要帳戶（必要變數使用 `requires.env`，或選用變數使用帶有 `required: false` 的 `envVars`）。
