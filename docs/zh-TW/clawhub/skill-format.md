...
x-i18n:
    generated_at: "2026-06-28T05:05:44Z"
    model: gpt-5.5
...
網頁版 GitHub 匯入工具比本機發布/同步更嚴格。它只會探索由
已登入 GitHub 帳號擁有、公開且非 fork 的儲存庫中的
...
技能中繼資料宣告在 `SKILL.md` 頂端的 YAML frontmatter 中。這會告訴登錄檔（以及安全性分析）你的技能需要什麼才能執行。
...
對於技能執行前必須存在的環境變數，請使用 `requires.env`。當你需要每個變數各自的中繼資料（包括 `required: false` 的選用變數）時，請使用 `envVars`。
...
ClawHub 的安全性分析會檢查你的技能宣告是否符合它實際執行的內容。如果你的程式碼參照了 `TODOIST_API_KEY`，但 frontmatter 沒有在 `requires.env`、`primaryEnv` 或 `envVars` 下宣告它，分析就會標記中繼資料不相符。維持宣告準確有助於技能通過審查，也能協助使用者了解他們正在安裝什麼。
...
- 腳本檔案在上傳後仍會被掃描；PowerShell `.ps1`、`.psm1` 和 `.psd1` 檔案會被當作文字接受。
- 以 `text/` 開頭的內容類型會被視為文字；另有一小份允許清單（JSON/YAML/TOML/JS/TS/Markdown/SVG）。

限制（伺服器端）：

- 套件總大小：50MB。
- 嵌入文字包含 `SKILL.md` + 最多約 40 個非 `.md` 檔案（盡力而為的上限）。

## Slugs

- 預設從資料夾名稱衍生。
- 套件 scope 必須與 ClawHub 發布者 handle 完全相符。發布者 handle 可使用小寫字母、數字、連字號、點號和底線；必須以小寫字母或數字開頭和結尾。
...
- 不需要標示出處。
- 不要在 `SKILL.md` 中加入衝突的授權條款；ClawHub 不支援個別技能覆寫授權。

## 付費技能

- ClawHub 不支援付費技能、個別技能定價、付費牆或收益分潤。
- 不要將定價中繼資料加入 `SKILL.md`；它不是技能格式的一部分，也不會讓已發布的技能變成付費技能。
- 如果你的技能整合了付費第三方服務，請在技能指引和環境變數宣告中清楚記載外部費用與必要帳號（必要變數使用 `requires.env`，選用變數則使用 `envVars` 並設定 `required: false`）。
