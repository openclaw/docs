...
x-i18n:
    generated_at: "2026-07-01T20:11:19Z"
    model: gpt-5.5
...
網頁版 GitHub 匯入器比本機發布/同步更嚴格。它只會在已登入 GitHub 帳號擁有的公開、非 fork 儲存庫中，探索
...
對於技能執行前必須存在的環境變數，請使用 `requires.env`。需要每個變數的中繼資料時，請使用 `envVars`，包括帶有 `required: false` 的選用變數。
...
| `requires.bins`    | `string[]` | 全部都必須安裝的命令列介面二進位檔。                                                                                                         |
...
ClawHub 的安全性分析會檢查技能宣告的內容是否符合它實際執行的動作。如果你的程式碼參照 `TODOIST_API_KEY`，但 frontmatter 沒有在 `requires.env`、`primaryEnv` 或 `envVars` 下宣告它，分析就會標記中繼資料不相符。保持宣告準確有助於技能通過審查，也能幫助使用者了解他們正在安裝什麼。
...
- 預設從資料夾名稱衍生。
- 套件 scope 必須與 ClawHub 發布者 handle 完全相符。發布者 handle 可以使用小寫字母、數字、連字號、句點和底線；必須以小寫字母或數字開頭和結尾。
...
- 如果你的技能整合付費的第三方服務，請在技能指示和環境變數宣告中清楚記錄外部費用與必要帳號（必要變數使用 `requires.env`，選用變數則使用帶有 `required: false` 的 `envVars`）。
