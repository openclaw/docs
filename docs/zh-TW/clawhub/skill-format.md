...
- 任何支援的_文字型_檔案（見「允許的檔案」）
- `.clawhubignore`（發佈時的忽略模式，舊版 `.clawdhubignore`）
- `.gitignore`（也會遵循）

## GitHub 匯入

網頁版 GitHub 匯入器比本機發佈/同步更嚴格。它只會在已登入 GitHub 帳號擁有的公開、非 fork 儲存庫中探索
...
- Markdown，可選用 YAML 前置內容。
- 伺服器會在發佈期間從前置內容擷取中繼資料。
- `description` 會在 UI/搜尋中作為技能摘要使用。

## 前置內容中繼資料

技能中繼資料宣告在 `SKILL.md` 頂端的 YAML 前置內容中。這會告訴登錄檔（以及安全性分析）你的技能執行時需要什麼。
...
需要在技能執行前存在的環境變數請使用 `requires.env`。當你需要每個變數的中繼資料時，請使用 `envVars`，包括帶有 `required: false` 的選用變數。
...
ClawHub 的安全性分析會檢查你的技能宣告內容是否符合它實際執行的動作。如果你的程式碼參照 `TODOIST_API_KEY`，但你的前置內容沒有在 `requires.env`、`primaryEnv` 或 `envVars` 下宣告它，分析會標記中繼資料不相符。保持宣告準確能幫助你的技能通過審查，也能幫助使用者了解自己正在安裝什麼。
...
- 指令碼檔案上傳後仍會被掃描；PowerShell `.ps1`、`.psm1` 和 `.psd1` 檔案會以文字形式接受。
- 以 `text/` 開頭的內容類型會視為文字；另有一小份允許清單（JSON/YAML/TOML/JS/TS/Markdown/SVG）。
...
- 預設從資料夾名稱衍生。
- 套件 scope 必須與 ClawHub 發佈者 handle 完全相符。發佈者 handle 可使用小寫字母、數字、連字號、點和底線；開頭與結尾必須是小寫字母或數字。
...
- 所有在 ClawHub 上發佈的技能皆以 `MIT-0` 授權。
- 任何人都可以使用、修改並重新散佈已發佈的技能，包括商業用途。
- 不需要署名。
- 不要在 `SKILL.md` 中加入衝突的授權條款；ClawHub 不支援個別技能的授權覆寫。

## 付費技能

- ClawHub 不支援付費技能、個別技能定價、付費牆或收益分潤。
- 不要將定價中繼資料加入 `SKILL.md`；它不是技能格式的一部分，也不會讓已發佈的技能變成付費技能。
- 如果你的技能整合付費的第三方服務，請在技能說明與環境變數宣告中清楚記錄外部費用與必要帳號（必要變數使用 `requires.env`，選用變數則使用帶有 `required: false` 的 `envVars`）。
