...
x-i18n:
    generated_at: "2026-05-12T12:49:22Z"
    model: gpt-5.5
    provider: openai
...
- `.clawhubignore`（用於發布/同步的忽略模式，舊版 `.clawdhubignore`）
- `.gitignore`（也會受到遵循）
...
在 `metadata.openclaw`（別名：`metadata.clawdbot`、`metadata.clawdis`）下宣告技能的執行階段需求。
...
對於技能執行前必須存在的環境變數，使用 `requires.env`。當你需要逐一變數的中繼資料，包括帶有 `required: false` 的選用變數時，使用 `envVars`。

### 完整欄位參考

| 欄位               | 類型       | 說明                                                                                                                                            |
| ------------------ | ---------- | ----------------------------------------------------------------------------------------------------------------------------------------------- |
| `requires.env`     | `string[]` | 你的技能預期必要的環境變數。                                                                                                                    |
...
| `envVars`          | `array`    | 環境變數宣告，包含 `name`、選用的 `required`，以及選用的 `description`。對於選用環境變數，設定 `required: false`。                              |
...
如果你的技能需要安裝依賴項目，請在 `install` 陣列中宣告：
...
ClawHub 的安全性分析會檢查你的技能所宣告的內容是否符合其實際行為。如果你的程式碼參照 `TODOIST_API_KEY`，但 frontmatter 沒有在 `requires.env`、`primaryEnv` 或 `envVars` 下宣告它，分析會標示中繼資料不相符。保持宣告準確，有助於你的技能通過審查，也能幫助使用者了解自己正在安裝什麼。
...
- 指令碼檔案在上傳後仍會掃描；PowerShell `.ps1`、`.psm1` 和 `.psd1` 檔案會以文字檔接受。
...
- 如果你的技能整合了付費第三方服務，請在技能說明和 env 宣告中清楚記載外部費用與必要帳號（必要變數使用 `requires.env`，選用變數則使用帶有 `required: false` 的 `envVars`）。
