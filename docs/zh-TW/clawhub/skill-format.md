...
x-i18n:
    generated_at: "2026-06-28T20:42:31Z"
    model: gpt-5.5
...
網頁版 GitHub 匯入器比本機發布/同步更嚴格。它只會在已登入 GitHub 帳戶擁有的公開、非 fork 儲存庫中發現
...
技能中繼資料會宣告在 `SKILL.md` 頂端的 YAML frontmatter 中。這會告訴登錄檔（以及安全性分析）你的技能執行時需要什麼。
...
對技能執行前必須存在的環境變數使用 `requires.env`。當你需要逐一變數的中繼資料時，請使用 `envVars`，包含具有 `required: false` 的選用變數。
...
在 `metadata.openclaw.envVars` 下宣告選用環境變數，並設定 `required: false`。不要將選用項目加入 `requires.env`，因為 `requires.env` 表示技能缺少它們就無法執行。
...
ClawHub 的安全性分析會檢查你的技能所宣告的內容是否符合它實際執行的內容。如果你的程式碼參照 `TODOIST_API_KEY`，但你的 frontmatter 沒有在 `requires.env`、`primaryEnv` 或 `envVars` 下宣告它，分析會標記中繼資料不相符。保持宣告準確有助於你的技能通過審查，也能協助使用者了解他們正在安裝什麼。
...
- 指令碼檔案上傳後仍會掃描；PowerShell `.ps1`、`.psm1` 和 `.psd1` 檔案會作為文字接受。
- 以 `text/` 開頭的內容類型會視為文字；另加一小份允許清單（JSON/YAML/TOML/JS/TS/Markdown/SVG）。
...
- 套件 slug 必須是小寫且 npm 安全，例如 `@example.tools/demo-plugin` 或 `demo-plugin`。

## 版本控制 + 標籤

- 每次發布都會建立新版本（semver）。
- 標籤是指向版本的字串指標；常用的是 `latest`。
...
- ClawHub 不支援付費技能、逐技能定價、付費牆或收入分潤。
- 不要將定價中繼資料加入 `SKILL.md`；它不是技能格式的一部分，也不會讓已發布的技能變成付費技能。
- 如果你的技能整合付費第三方服務，請在技能指示與環境變數宣告中清楚記載外部費用與必要帳戶（必要變數使用 `requires.env`，或針對選用變數使用含有 `required: false` 的 `envVars`）。
