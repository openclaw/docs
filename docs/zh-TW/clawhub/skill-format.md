...
x-i18n:
    generated_at: "2026-07-05T04:57:05Z"
    model: gpt-5.5
...
網頁版 GitHub 匯入器比本機發布/同步更嚴格。它只會在登入的 GitHub 帳號所擁有、公開且非分支的儲存庫中探索
...
- Markdown，可選用 YAML 前置中繼資料。
- 伺服器會在發布期間從前置中繼資料擷取中繼資料。
- `description` 會作為 UI/搜尋中的技能摘要。

## 前置中繼資料

技能中繼資料宣告於 `SKILL.md` 頂端的 YAML 前置中繼資料。這會告訴登錄檔（以及安全性分析）你的技能執行時需要什麼。
...
對技能執行前必須存在的環境變數使用 `requires.env`。當你需要每個變數的中繼資料時，請使用 `envVars`，包含帶有 `required: false` 的選用變數。

### 完整欄位參考

| 欄位               | 類型       | 說明                                                                                                                                       |
| ------------------ | ---------- | ------------------------------------------------------------------------------------------------------------------------------------------ |
| `requires.env`     | `string[]` | 技能預期的必要環境變數。                                                                                                                   |
...
| `envVars`          | `array`    | 具有 `name`、選用 `required` 和選用 `description` 的環境變數宣告。對選用環境變數設定 `required: false`。                                  |
| `always`           | `boolean`  | 若為 `true`，技能永遠啟用（不需要明確安裝）。                                                                                              |
...
在 `metadata.openclaw.envVars` 下宣告選用環境變數，並設定 `required: false`。不要將選用項目加入 `requires.env`，因為 `requires.env` 表示技能缺少這些變數就無法執行。
...
ClawHub 的安全性分析會檢查你的技能所宣告的內容是否符合它實際執行的動作。如果你的程式碼參照 `TODOIST_API_KEY`，但你的前置中繼資料沒有在 `requires.env`、`primaryEnv` 或 `envVars` 下宣告它，分析會標記中繼資料不相符。保持宣告準確有助於技能通過審查，也能幫助使用者了解他們正在安裝什麼。
...
- 預設從資料夾名稱衍生。
- 套件範圍必須與 ClawHub 發布者代號完全相符。發布者代號可使用小寫字母、數字、連字號、點和底線；必須以小寫字母或數字開頭和結尾。
- 套件 slug 必須是小寫且符合 npm 安全，例如 `@example.tools/demo-plugin` 或 `demo-plugin`。
...
- 所有發布在 ClawHub 上的技能皆採用 `MIT-0` 授權。
...
- ClawHub 不支援付費技能、按技能定價、付費牆或收益分潤。
- 不要將定價中繼資料加入 `SKILL.md`；這不是技能格式的一部分，也不會讓已發布的技能變成付費技能。
- 如果你的技能整合付費第三方服務，請在技能說明和環境變數宣告中清楚記載外部成本與必要帳號（必要變數使用 `requires.env`，選用變數使用帶有 `required: false` 的 `envVars`）。
