...
- `.clawhubignore`（發布時忽略的模式，舊版 `.clawdhubignore`）
- `.gitignore`（也會被遵循）

## GitHub 匯入

網頁版 GitHub 匯入器比本機發布/同步更嚴格。它只會在已登入 GitHub 帳號所擁有的公開、非 fork 儲存庫中，探索 `SKILL.md` 或舊版 `skills.md` 檔案。它不會匯入私有儲存庫、fork、封存/停用的儲存庫，或第三方公開儲存庫。
...
技能中繼資料會宣告在 `SKILL.md` 頂端的 YAML frontmatter 中。這會告訴註冊表（以及安全性分析）你的技能需要什麼才能執行。
...
在 `metadata.openclaw` 底下宣告技能的執行階段需求（別名：`metadata.clawdbot`、`metadata.clawdis`）。
...
針對技能執行前必須存在的環境變數使用 `requires.env`。當你需要每個變數的中繼資料時，請使用 `envVars`，包含帶有 `required: false` 的選用變數。
...
| `always`           | `boolean`  | 如果為 `true`，技能永遠啟用（不需要明確安裝）。                                                                                             |
...
在 `metadata.openclaw.envVars` 底下宣告選用環境變數，並設定 `required: false`。不要將選用項目加入 `requires.env`，因為 `requires.env` 代表沒有它們技能就無法執行。
...
ClawHub 的安全性分析會檢查你的技能宣告是否與它實際執行的內容相符。如果你的程式碼參照 `TODOIST_API_KEY`，但 frontmatter 沒有在 `requires.env`、`primaryEnv` 或 `envVars` 底下宣告它，分析就會標記中繼資料不相符。保持宣告準確有助於技能通過審查，也能協助使用者了解他們正在安裝什麼。
...
- 預設從資料夾名稱衍生。
- 套件 scope 必須與 ClawHub 發布者 handle 完全相符。發布者 handle 可使用小寫字母、數字、連字號、點和底線；且必須以小寫字母或數字開頭與結尾。
- 套件 slug 必須為小寫且符合 npm 安全格式，例如 `@example.tools/demo-plugin` 或 `demo-plugin`。

## 版本控管 + 標籤

- 每次發布都會建立新版本（semver）。
- 標籤是指向某個版本的字串指標；常見用法是 `latest`。

## 授權

- 所有發布到 ClawHub 的技能都採用 `MIT-0` 授權。
- 任何人都可以使用、修改及再散布已發布的技能，包括商業用途。
- 不需要署名。
- 不要在 `SKILL.md` 中新增衝突的授權條款；ClawHub 不支援逐技能授權覆寫。

## 付費技能

- ClawHub 不支援付費技能、逐技能定價、付費牆或收益分潤。
- 不要將定價中繼資料加入 `SKILL.md`；它不是技能格式的一部分，也不會讓已發布的技能變成付費。
- 如果你的技能整合付費第三方服務，請在技能說明與環境變數宣告中清楚記錄外部成本與所需帳號（必要變數使用 `requires.env`，或選用變數使用帶有 `required: false` 的 `envVars`）。
