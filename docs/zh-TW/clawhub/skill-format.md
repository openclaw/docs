...
x-i18n:
    generated_at: "2026-06-28T07:42:23Z"
    model: gpt-5.5
...
網頁版 GitHub 匯入器比本機發布/同步更嚴格。它只會探索
已登入 GitHub 帳號所擁有的公開、非 fork 儲存庫中的
...
針對技能執行前必須存在的環境變數，使用 `requires.env`。需要每個變數的中繼資料時，請使用 `envVars`，包括帶有 `required: false` 的選用變數。
...
| `envVars`          | `array`    | 帶有 `name`、選用 `required` 和選用 `description` 的環境變數宣告。針對選用環境變數設定 `required: false`。                                    |
...
ClawHub 的安全性分析會檢查你的技能宣告內容是否符合它實際執行的內容。如果你的程式碼參照 `TODOIST_API_KEY`，但 frontmatter 沒有在 `requires.env`、`primaryEnv` 或 `envVars` 下宣告它，分析會標記中繼資料不相符。保持宣告準確有助於技能通過審查，也能幫助使用者了解他們正在安裝什麼。
...
- 以 `text/` 開頭的內容型別會被視為文字；另外還有一小份允許清單（JSON/YAML/TOML/JS/TS/Markdown/SVG）。
...
- 套件 slug 必須是小寫且符合 npm 安全規則，例如 `@example.tools/demo-plugin` 或 `demo-plugin`。
...
- ClawHub 不支援付費技能、每個技能定價、付費牆或收益分潤。
- 不要將定價中繼資料加入 `SKILL.md`；它不是技能格式的一部分，也不會讓已發布的技能變成付費。
- 如果你的技能整合付費第三方服務，請在技能指示和環境變數宣告中清楚記錄外部成本與必要帳號（必要變數使用 `requires.env`，選用變數使用帶有 `required: false` 的 `envVars`）。
