...
網頁版 GitHub 匯入器比本機發布/同步更嚴格。它只會在登入的 GitHub 帳戶所擁有的公開、非 fork 儲存庫中，發現
...
ClawHub 的安全性分析會檢查技能宣告的內容是否符合它實際執行的動作。如果你的程式碼參照 `TODOIST_API_KEY`，但 frontmatter 未在 `requires.env`、`primaryEnv` 或 `envVars` 下宣告它，分析會標記為中繼資料不相符。保持宣告準確可協助技能通過審查，也能協助使用者了解他們正在安裝什麼。
...
- 如果你的技能整合付費第三方服務，請在技能說明和環境變數宣告中清楚記錄外部費用和所需帳戶（必要變數使用 `requires.env`，或選用變數使用帶有 `required: false` 的 `envVars`）。
