...
網頁版 GitHub 匯入器比本機發布/同步更嚴格。它只會探索登入的 GitHub 帳號所擁有、公開且非分支儲存庫中的
...
- Markdown，並可選用 YAML frontmatter。
...
| 欄位               | 類型       | 說明                                                                                                                                         |
| ------------------ | ---------- | -------------------------------------------------------------------------------------------------------------------------------------------- |
| `requires.env`     | `string[]` | 技能預期需要的必填環境變數。                                                                                                                 |
...
ClawHub 的安全性分析會檢查你的技能宣告內容是否符合其實際行為。如果你的程式碼引用 `TODOIST_API_KEY`，但 frontmatter 未在 `requires.env`、`primaryEnv` 或 `envVars` 下宣告它，分析就會標示中繼資料不相符。保持宣告準確可協助你的技能通過審查，也能協助使用者了解他們正在安裝什麼。
...
- 腳本檔案在上傳後仍會被掃描；PowerShell `.ps1`、`.psm1` 和 `.psd1` 檔案會被視為文字接受。
...
- 預設從資料夾名稱衍生。
- 套件 scope 必須與 ClawHub 發布者 handle 完全相符。發布者 handle 可以使用小寫字母、數字、連字號、句點和底線；且必須以小寫字母或數字開頭與結尾。
...
- 所有發布到 ClawHub 的技能皆採用 `MIT-0` 授權。
- 任何人都可以使用、修改並重新散布已發布的技能，包括商業用途。
- 不需要署名。
- 不要在 `SKILL.md` 中加入衝突的授權條款；ClawHub 不支援為個別技能覆寫授權。

## 付費技能

- ClawHub 不支援付費技能、個別技能定價、付費牆或收益分潤。
- 不要將定價中繼資料加入 `SKILL.md`；它不是技能格式的一部分，也不會讓已發布的技能變成付費。
- 如果你的技能整合了付費第三方服務，請在技能說明和環境變數宣告中清楚記載外部成本與所需帳號（必填變數使用 `requires.env`，選用變數則使用帶有 `required: false` 的 `envVars`）。
