...
- 任何支援的_文字型_檔案（請參閱「允許的檔案」）
- `.clawhubignore`（發布時的忽略模式，舊版 `.clawdhubignore`）
- `.gitignore`（也會採用）

## GitHub 匯入

網頁版 GitHub 匯入工具比本機發布/同步更嚴格。它只會在已登入 GitHub 帳號擁有的公開、非 fork 儲存庫中探索
...
- 可選 YAML frontmatter 的 Markdown。
...
對於技能執行前必須存在的環境變數，使用 `requires.env`。當你需要每個變數的中繼資料時，請使用 `envVars`，包括帶有 `required: false` 的可選變數。

### 完整欄位參考

| 欄位               | 類型       | 說明                                                                                                                                         |
| ------------------ | ---------- | -------------------------------------------------------------------------------------------------------------------------------------------- |
| `requires.env`     | `string[]` | 你的技能預期存在的必要環境變數。                                                                                                             |
...
| `envVars`          | `array`    | 環境變數宣告，包含 `name`、可選的 `required` 和可選的 `description`。對可選環境變數設定 `required: false`。                                  |
...
在 `metadata.openclaw.envVars` 下宣告可選環境變數，並設定 `required: false`。不要將可選項目加入 `requires.env`，因為 `requires.env` 表示沒有它們時技能無法執行。
...
ClawHub 的安全性分析會檢查你的技能宣告內容是否符合它實際執行的動作。如果你的程式碼參照 `TODOIST_API_KEY`，但 frontmatter 未在 `requires.env`、`primaryEnv` 或 `envVars` 下宣告它，分析會標記中繼資料不相符。保持宣告準確有助於你的技能通過審查，也能幫助使用者了解他們正在安裝什麼。
...
- 預設由資料夾名稱衍生。
- 套件 scope 必須與 ClawHub 發布者 handle 完全相符。發布者 handle 可以使用小寫字母、數字、連字號、點和底線；開頭和結尾必須是小寫字母或數字。
...
- 所有發布在 ClawHub 上的 Skills 都以 `MIT-0` 授權。
...
- ClawHub 不支援付費 Skills、個別技能定價、付費牆或收益分潤。
- 不要將定價中繼資料加入 `SKILL.md`；它不是技能格式的一部分，也不會讓已發布的技能變成付費。
- 如果你的技能整合付費第三方服務，請在技能說明和環境變數宣告中清楚記錄外部費用和必要帳號（必要變數使用 `requires.env`，或可選變數使用帶有 `required: false` 的 `envVars`）。
