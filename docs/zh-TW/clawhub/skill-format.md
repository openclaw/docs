...
- Markdown，並可選擇加入 YAML frontmatter。
...
在 `metadata.openclaw`（別名：`metadata.clawdbot`、`metadata.clawdis`）底下宣告你的技能執行階段需求。
...
針對技能執行前必須存在的環境變數，使用 `requires.env`。當你需要每個變數的中繼資料時，請使用 `envVars`，其中包括含有 `required: false` 的選用變數。
...
| `envVars`          | `array`    | 環境變數宣告，含 `name`、選用的 `required`，以及選用的 `description`。若為選用環境變數，請設定 `required: false`。                  |
...
ClawHub 的安全性分析會檢查你的技能宣告內容是否符合實際行為。如果你的程式碼引用了 `TODOIST_API_KEY`，但 frontmatter 沒有在 `requires.env`、`primaryEnv` 或 `envVars` 底下宣告它，分析就會標記中繼資料不符。保持宣告準確，有助於你的技能通過審查，也能幫助使用者了解他們正在安裝什麼。
...
- ClawHub 不支援付費技能、單一技能定價、付費牆或收益分潤。
- 不要將定價中繼資料加入 `SKILL.md`；這不是技能格式的一部分，也不會讓已發布的技能變成付費技能。
- 如果你的技能整合付費第三方服務，請在技能說明和環境宣告中清楚記錄外部成本與必要帳號（必要變數使用 `requires.env`，選用變數則使用含 `required: false` 的 `envVars`）。
