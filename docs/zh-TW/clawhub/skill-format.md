...
x-i18n:
    generated_at: "2026-07-12T21:22:57Z"
    model: gpt-5.6
...
網頁版 GitHub 匯入工具比本機發佈／同步更嚴格。它只會探索已登入 GitHub 帳號所擁有之公開、非分支倉庫中的
`SKILL.md` 或舊版 `skills.md` 檔案。它不會匯入私人倉庫、分支倉庫、
已封存／停用的倉庫，或第三方公開倉庫。
...
目錄顯示名稱分開保存，因此來自其他用戶端的現有名稱仍可
發佈，且不會在未告知的情況下被改寫。目錄清單可能會在視覺上縮短過長的名稱，
但不會變更儲存的名稱。

## Frontmatter 中繼資料

Skill 中繼資料是在 `SKILL.md` 頂端的 YAML frontmatter 中宣告。這會告訴登錄檔（以及安全性分析）你的 Skill 執行時需要什麼。
...
使用 `requires.env` 指定 Skill 執行前必須存在的環境變數。需要個別變數的中繼資料時，請使用 `envVars`，包括設定為 `required: false` 的選用變數。
...
| `envVars`          | `array`    | 包含 `name`、選用 `required` 和選用 `description` 的環境變數宣告。選用環境變數請設定 `required: false`。                                           |
...
如果你的 Skill 需要安裝相依套件，請在 `install` 陣列中宣告：
...
在 `metadata.openclaw.envVars` 下宣告選用環境變數，並設定 `required: false`。請勿將選用項目加入 `requires.env`，因為 `requires.env` 表示缺少這些項目時 Skill 無法執行。
...
ClawHub 的安全性分析會檢查 Skill 宣告的內容是否與實際行為相符。如果程式碼參照 `TODOIST_API_KEY`，但 frontmatter 未在 `requires.env`、`primaryEnv` 或 `envVars` 下宣告，分析便會將其標記為中繼資料不相符。保持宣告準確有助於 Skill 通過審查，也能協助使用者瞭解他們正在安裝什麼。
...
- 指令碼檔案在上傳後仍會受到掃描；PowerShell `.ps1`、`.psm1` 和 `.psd1` 檔案可作為文字檔接受。
- 以 `text/` 開頭的內容類型會被視為文字；此外還有一小部分允許清單（JSON／YAML／TOML／JS／TS／Markdown／SVG）。

限制（伺服器端）：

- 套件總大小：50MB。
- 內嵌文字包括 `SKILL.md` 加上最多約 40 個非 `.md` 檔案（盡力而為的上限）。

## Slug

- 預設從資料夾名稱衍生。
- 套件範圍必須與 ClawHub 發佈者代稱完全相符。發佈者代稱可使用小寫字母、數字、連字號、句點和底線；開頭和結尾必須是小寫字母或數字。
...
- 所有發佈至 ClawHub 的 Skill 均採用 `MIT-0` 授權。
- 任何人都可以使用、修改及重新散布已發佈的 Skill，包括商業用途。
- 不要求標示出處。
- 請勿在 `SKILL.md` 中加入衝突的授權條款；ClawHub 不支援針對個別 Skill 覆寫授權條款。

## 付費 Skill

- ClawHub 不支援付費 Skill、個別 Skill 定價、付費牆或收益分成。
- 請勿將定價中繼資料加入 `SKILL.md`；它不屬於 Skill 格式，也不會讓已發佈的 Skill 成為付費項目。
- 如果你的 Skill 整合付費的第三方服務，請在 Skill 指示和環境變數宣告中，清楚說明外部費用和必要帳號（必要變數使用 `requires.env`，選用變數則使用設定了 `required: false` 的 `envVars`）。
