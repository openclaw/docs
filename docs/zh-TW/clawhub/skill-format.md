...
網頁版 GitHub 匯入器比本機發布/同步更嚴格。它只會在登入的 GitHub 帳戶所擁有、公開且非分支的儲存庫中探索
...
使用 `requires.env` 宣告技能執行前必須存在的環境變數。需要逐一變數的中繼資料時，使用 `envVars`，包括帶有 `required: false` 的選用變數。

### 完整欄位參考

| 欄位               | 類型       | 說明                                                                                                                                         |
| ------------------ | ---------- | -------------------------------------------------------------------------------------------------------------------------------------------- |
| `requires.env`     | `string[]` | 技能預期存在的必要環境變數。                                                                                                                 |
...
在 `metadata.openclaw.envVars` 下宣告選用環境變數，並設定 `required: false`。不要把選用項目加入 `requires.env`，因為 `requires.env` 表示技能沒有它們就無法執行。
...
ClawHub 的安全性分析會檢查技能宣告的內容是否符合它實際執行的行為。如果你的程式碼參照 `TODOIST_API_KEY`，但 frontmatter 沒有在 `requires.env`、`primaryEnv` 或 `envVars` 下宣告它，分析會標記中繼資料不相符。保持宣告準確有助於技能通過審查，也能幫助使用者理解他們正在安裝什麼。
...
- 指令碼檔案仍會在上傳後掃描；PowerShell `.ps1`、`.psm1` 和 `.psd1` 檔案會作為文字接受。
...
- ClawHub 不支援付費 skills、逐技能定價、付費牆或收益分潤。
- 不要將定價中繼資料加入 `SKILL.md`；它不是技能格式的一部分，也不會讓已發布的技能變成付費。
- 如果你的技能整合付費第三方服務，請在技能指示和環境變數宣告中清楚記錄外部費用與必要帳戶（必要變數使用 `requires.env`，選用變數則使用帶有 `required: false` 的 `envVars`）。
