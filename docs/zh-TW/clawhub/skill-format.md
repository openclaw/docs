...
x-i18n:
    generated_at: "2026-07-02T13:57:49Z"
    model: gpt-5.5
...
已登入 GitHub 帳戶擁有的公開、非 fork 儲存庫中的
...
使用 `requires.env` 宣告技能執行前必須存在的環境變數。當你需要逐一提供變數中繼資料時，請使用 `envVars`，包括帶有 `required: false` 的選用變數。
...
ClawHub 的安全性分析會檢查技能宣告的內容是否符合它實際執行的內容。如果你的程式碼參照 `TODOIST_API_KEY`，但 frontmatter 沒有在 `requires.env`、`primaryEnv` 或 `envVars` 下宣告它，分析就會標示中繼資料不符。保持宣告準確可協助你的技能通過審查，也協助使用者了解他們正在安裝什麼。
...
- 任何人都可以使用、修改及重新散布已發佈的技能，包括商業用途。
- 不需要署名。
- 不要在 `SKILL.md` 中加入衝突的授權條款；ClawHub 不支援每個技能自訂授權覆寫。

## 付費技能

- ClawHub 不支援付費技能、單一技能定價、付費牆或收益分潤。
- 不要將定價中繼資料加入 `SKILL.md`；它不是技能格式的一部分，也不會讓已發佈的技能變成付費。
- 如果你的技能整合付費第三方服務，請在技能指示和環境變數宣告中清楚記錄外部成本與所需帳戶（必要變數使用 `requires.env`，選用變數則使用帶有 `required: false` 的 `envVars`）。
