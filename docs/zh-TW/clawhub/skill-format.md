...
x-i18n:
    generated_at: "2026-06-27T19:02:39Z"
    model: gpt-5.5
...
網頁版 GitHub 匯入器比本機發布/同步更嚴格。它只會在已登入 GitHub 帳號所擁有的公開、非 fork 儲存庫中探索 `SKILL.md` 或舊版 `skills.md` 檔案。它不會匯入私有 repo、fork、已封存/停用的 repo，或第三方公開 repo。
...
- Markdown，可包含選用的 YAML frontmatter。
...
針對技能執行前必須存在的環境變數使用 `requires.env`。當你需要每個變數的中繼資料時，請使用 `envVars`，包括帶有 `required: false` 的選用變數。
...
| `envVars`          | `array`    | 帶有 `name`、選用 `required` 與選用 `description` 的環境變數宣告。針對選用環境變數設定 `required: false`。                 |
  ...
  openclaw:
    ...
    envVars:
      ...
      - name: TODOIST_PROJECT_ID
        required: false
        description: 使用者未指定時的選用預設專案 ID。
```

### 這為什麼重要

ClawHub 的安全性分析會檢查你的技能宣告內容是否符合它實際執行的動作。如果你的程式碼參照 `TODOIST_API_KEY`，但你的 frontmatter 沒有在 `requires.env`、`primaryEnv` 或 `envVars` 下宣告它，分析就會標記中繼資料不相符。保持宣告準確有助於你的技能通過審查，也能協助使用者了解他們正在安裝什麼。
...
- 以 `text/` 開頭的內容類型會被視為文字；另外還有一份小型允許清單（JSON/YAML/TOML/JS/TS/Markdown/SVG）。
...
- 預設從資料夾名稱衍生。
- 套件 scope 必須與 ClawHub 發布者 handle 完全相符。發布者 handle 可使用小寫字母、數字、連字號、點和底線；且必須以小寫字母或數字開頭和結尾。
...
- 如果你的技能整合付費第三方服務，請在技能指示與環境變數宣告中清楚記錄外部成本和必要帳號（必要變數使用 `requires.env`，選用變數則使用帶有 `required: false` 的 `envVars`）。
