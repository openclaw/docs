...
網頁版 GitHub 匯入器比本機發布/同步更嚴格。它只會探索已登入 GitHub 帳號所擁有、公開且非 fork 的儲存庫中的
...
- Markdown，帶有選用的 YAML frontmatter。
- 伺服器會在發布期間從 frontmatter 擷取中繼資料。
- `description` 會作為 UI/搜尋中的技能摘要。

## Frontmatter 中繼資料

技能中繼資料會宣告在你的 `SKILL.md` 頂端的 YAML frontmatter 中。這會告訴登錄檔（以及安全性分析）你的技能執行時需要什麼。
...
在 `metadata.openclaw`（別名：`metadata.clawdbot`、`metadata.clawdis`）下宣告你的技能執行階段需求。
...
使用 `requires.env` 宣告技能執行前必須存在的環境變數。當你需要逐一變數的中繼資料時，請使用 `envVars`，包括帶有 `required: false` 的選用變數。
...
| `primaryEnv`       | `string`   | 你的技能主要認證用環境變數。                                                                                                              |
| `envVars`          | `array`    | 環境變數宣告，包含 `name`、選用的 `required`，以及選用的 `description`。請為選用環境變數設定 `required: false`。                         |
...
如果你的技能需要安裝依賴項，請在 `install` 陣列中宣告：
...
在 `metadata.openclaw.envVars` 下宣告選用環境變數，並設定 `required: false`。不要把選用項目加到 `requires.env`，因為 `requires.env` 表示沒有它們技能就無法執行。
...
ClawHub 的安全性分析會檢查你的技能宣告內容是否符合它實際執行的行為。如果你的程式碼引用 `TODOIST_API_KEY`，但你的 frontmatter 沒有在 `requires.env`、`primaryEnv` 或 `envVars` 下宣告它，分析就會標記中繼資料不相符。保持宣告準確可協助你的技能通過審查，也協助使用者理解自己正在安裝什麼。
...
- 預設從資料夾名稱衍生。
- 套件 scope 必須與 ClawHub 發布者 handle 完全相符。發布者 handle 可使用小寫字母、數字、連字號、點和底線；必須以小寫字母或數字開頭與結尾。
...
- 在 ClawHub 上發布的所有技能都採用 `MIT-0` 授權。
- 任何人都可以使用、修改和重新散布已發布技能，包括商業用途。
- 不需要署名。
...
- 不要在 `SKILL.md` 中加入定價中繼資料；它不是技能格式的一部分，也不會讓已發布技能變成付費。
- 如果你的技能整合付費第三方服務，請在技能指示與環境變數宣告中清楚記載外部費用和必要帳號（必要變數使用 `requires.env`，選用變數則使用帶有 `required: false` 的 `envVars`）。
