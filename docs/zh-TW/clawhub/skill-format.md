...
x-i18n:
    generated_at: "2026-07-02T17:30:41Z"
    model: gpt-5.5
...
網頁版 GitHub 匯入器比本機發佈/同步更嚴格。它只會在已登入 GitHub 帳號擁有的公開、非 fork 儲存庫中，探索
...
- 伺服器在發佈期間會從 frontmatter 擷取中繼資料。
- `description` 會作為 UI/搜尋中的技能摘要。

## Frontmatter 中繼資料

技能中繼資料會宣告在 `SKILL.md` 頂端的 YAML frontmatter。這會告訴登錄檔（以及安全性分析）你的技能需要什麼才能執行。
...
請在 `metadata.openclaw` 下宣告技能的執行階段需求（別名：`metadata.clawdbot`、`metadata.clawdis`）。
...
針對技能執行前必須存在的環境變數，請使用 `requires.env`。當你需要每個變數的中繼資料時，請使用 `envVars`，包括帶有 `required: false` 的選用變數。
...
請在 `metadata.openclaw.envVars` 下宣告選用環境變數，並設定 `required: false`。不要將選用項目加入 `requires.env`，因為 `requires.env` 表示技能沒有它們就無法執行。
...
ClawHub 的安全性分析會檢查技能宣告的內容是否符合它實際執行的行為。如果你的程式碼參照 `TODOIST_API_KEY`，但 frontmatter 沒有在 `requires.env`、`primaryEnv` 或 `envVars` 下宣告它，分析就會標記為中繼資料不一致。保持宣告準確有助於技能通過審查，也能幫助使用者了解他們正在安裝什麼。
...
- 腳本檔案在上傳後仍會被掃描；PowerShell `.ps1`、`.psm1` 和 `.psd1` 檔案會以文字形式接受。
- 以 `text/` 開頭的內容類型會被視為文字；另有一小份允許清單（JSON/YAML/TOML/JS/TS/Markdown/SVG）。

限制（伺服器端）：

- 總套件大小：50MB。
- 嵌入文字包含 `SKILL.md` + 最多約 40 個非 `.md` 檔案（盡力而為上限）。

## Slug

- 預設從資料夾名稱衍生。
- 套件 scope 必須與 ClawHub 發佈者 handle 完全相符。發佈者 handle 可以使用小寫字母、數字、連字號、點和底線；開頭與結尾必須是小寫字母或數字。
...
- 在 ClawHub 上發佈的所有技能都採用 `MIT-0` 授權。
- 任何人都可以使用、修改及重新散佈已發佈的技能，包括商業用途。
- 不需要署名。
...
- 如果你的技能整合付費第三方服務，請在技能說明與環境變數宣告中清楚記載外部費用與必要帳號（必要變數使用 `requires.env`，或針對選用變數使用帶有 `required: false` 的 `envVars`）。
