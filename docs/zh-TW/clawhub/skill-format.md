...
read_when:
    - 發布 Skills
    - 偵錯發布/同步失敗
summary: 技能資料夾格式、必要檔案、允許的檔案類型與限制。
x-i18n:
...
技能中繼資料宣告在 `SKILL.md` 頂部的 YAML frontmatter 中。這會告訴登錄檔（以及安全性分析）你的技能執行時需要什麼。
...
在 `metadata.openclaw`（別名：`metadata.clawdbot`、`metadata.clawdis`）底下宣告技能的執行階段需求。
...
使用 `requires.env` 表示技能執行前必須存在的環境變數。當你需要每個變數的中繼資料（包括具有 `required: false` 的選用變數）時，請使用 `envVars`。
...
| `requires.config`  | `string[]` | 技能讀取的設定檔路徑。                                                                                                          |
...
如果技能需要安裝相依性，請在 `install` 陣列中宣告它們：
...
在 `metadata.openclaw.envVars` 底下宣告選用環境變數，並設定 `required: false`。不要將選用項目加入 `requires.env`，因為 `requires.env` 表示技能沒有它們就無法執行。
...
ClawHub 的安全性分析會檢查技能宣告的內容是否符合它實際執行的內容。如果你的程式碼參照 `TODOIST_API_KEY`，但 frontmatter 沒有在 `requires.env`、`primaryEnv` 或 `envVars` 底下宣告它，分析就會標記中繼資料不相符。保持宣告準確有助於技能通過審查，也能協助使用者了解他們正在安裝什麼。
...
- 指令碼檔案在上傳後仍會掃描；PowerShell `.ps1`、`.psm1` 和 `.psd1` 檔案會被接受為文字。
...
- ClawHub 不支援付費技能、逐技能定價、付費牆或營收分潤。
- 不要將定價中繼資料加入 `SKILL.md`；它不是技能格式的一部分，也不會讓已發布的技能變成付費技能。
- 如果你的技能整合付費第三方服務，請在技能指示和 env 宣告中清楚記錄外部費用與所需帳號（必要變數使用 `requires.env`，或選用變數使用具有 `required: false` 的 `envVars`）。
