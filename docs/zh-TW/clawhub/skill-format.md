...
x-i18n:
    generated_at: "2026-06-28T05:09:11Z"
    model: gpt-5.5
...
對於技能執行前必須存在的環境變數，請使用 `requires.env`。當你需要每個變數的中繼資料，包括具有 `required: false` 的選用變數時，請使用 `envVars`。
...
ClawHub 的安全性分析會檢查技能宣告的內容是否符合它實際執行的行為。如果你的程式碼參照 `TODOIST_API_KEY`，但 frontmatter 沒有在 `requires.env`、`primaryEnv` 或 `envVars` 下宣告它，分析會標示中繼資料不相符。保持宣告準確，有助於技能通過審查，也能協助使用者了解他們正在安裝什麼。
...
- 預設由資料夾名稱衍生。
- 套件 scope 必須與 ClawHub 發布者 handle 完全相符。發布者 handle 可使用小寫字母、數字、連字號、句點和底線；開頭和結尾必須是小寫字母或數字。
...
- 如果你的技能整合付費第三方服務，請在技能說明和環境變數宣告中清楚記載外部費用和必要帳號（必要變數使用 `requires.env`，選用變數使用含 `required: false` 的 `envVars`）。
