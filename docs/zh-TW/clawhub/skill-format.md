...
read_when:
    - 發布 Skills
    - 偵錯發布失敗
summary: Skill 資料夾格式、必需檔案、允許的檔案類型、限制。
x-i18n:
...
網頁 GitHub 匯入器比本機發布/同步更嚴格。它只會在已登入 GitHub 帳號擁有的公開、非 fork 儲存庫中探索
...
使用 `requires.env` 表示技能執行前必須存在的環境變數。當你需要每個變數的中繼資料時，請使用 `envVars`，包含帶有 `required: false` 的選用變數。
...
| `envVars`          | `array`    | 帶有 `name`、選用 `required` 和選用 `description` 的環境變數宣告。若為選用環境變數，請設定 `required: false`。                              |
...
ClawHub 的安全分析會檢查你的技能宣告內容是否符合它實際執行的行為。如果你的程式碼參照 `TODOIST_API_KEY`，但你的 frontmatter 未在 `requires.env`、`primaryEnv` 或 `envVars` 下宣告它，分析會標記中繼資料不相符。保持宣告準確有助於你的技能通過審查，也能幫助使用者了解他們正在安裝什麼。
...
- 腳本檔案在上傳後仍會被掃描；PowerShell `.ps1`、`.psm1` 和 `.psd1` 檔案會被接受為文字。
...
- 預設從資料夾名稱衍生。
- Package scope 必須與 ClawHub 發布者 handle 完全相符。發布者 handle 可使用小寫字母、數字、連字號、點和底線；且必須以小寫字母或數字開頭與結尾。
- Package slug 必須是小寫且符合 npm 安全命名，例如 `@example.tools/demo-plugin` 或 `demo-plugin`。
...
- 在 ClawHub 上發布的所有技能皆以 `MIT-0` 授權。
- 任何人都可以使用、修改和重新散布已發布的技能，包含商業用途。
- 不需要署名。
...
- 如果你的技能整合付費第三方服務，請在技能說明和環境變數宣告中清楚記載外部成本與必要帳號（必要變數使用 `requires.env`，或選用變數使用帶有 `required: false` 的 `envVars`）。
