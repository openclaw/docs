...
x-i18n:
    generated_at: "2026-06-28T05:04:48Z"
    model: gpt-5.5
...
此命令會略過未變更的內容。新技能會從 `1.0.0` 開始；之後的變更會自動發布下一個修補版本。使用 `--dry-run` 可預覽，或使用 `--version` 選擇明確版本。

發布前，請檢查 `SKILL.md` 中的中繼資料。宣告必要的環境變數、工具和權限，讓使用者在安裝前了解技能需要什麼。請參閱[技能格式](/zh-TW/clawhub/skill-format)。

對於包含多個技能的儲存庫，可重用的 GitHub 工作流程會為 `skills/` 下每個直接技能資料夾呼叫 `skill publish`：
...
公開清單會顯示最新的掃描狀態。被審核保留或封鎖的版本，在解決前可能會從搜尋和安裝介面中隱藏。
