...
OpenClaw 會記錄 Skills 的來源，因此後續更新可以繼續透過 ClawHub 解析。
...
無介面環境可以使用 ClawHub 網頁介面的 API 權杖：
...
對於包含多個 Skills 的儲存庫，可重複使用的 GitHub 工作流程會對 `skills/` 下每個直接 Skills 資料夾呼叫 `skill publish`：
...
先使用 `--dry-run` 預覽解析後的套件中繼資料、相容性欄位、來源歸屬和上傳計畫，而不進行發布。
...
公開列表會顯示最新的掃描狀態。遭審核保留或封鎖的版本，可能會在解決前從搜尋與安裝介面隱藏。
