...
當你要把項目安裝到 OpenClaw 中時，請使用 OpenClaw。當你要登入、發布、管理自己的清單，或使用登錄庫專屬工作流程時，請使用 `clawhub` CLI。
...
OpenClaw 會記錄 Skill 的來源，以便之後的更新可以繼續透過 ClawHub 解析。
...
無介面的環境可以使用 ClawHub 網頁 UI 中的 API token：
...
`sync` 會掃描 Skill 資料夾，並發布尚未同步的新 Skill 或已變更的 Skills。
...
安裝前，請使用 ClawHub 網頁或 CLI 詳細資訊指令來檢查中繼資料、來源連結、版本、變更日誌和掃描狀態：
...
公開清單會顯示最新掃描狀態。被審核保留或封鎖的發行版本，可能會在問題解決前從搜尋和安裝介面中隱藏。
