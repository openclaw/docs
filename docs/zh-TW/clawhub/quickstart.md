...
x-i18n:
    generated_at: "2026-07-03T13:15:27Z"
    model: gpt-5.5
...
將項目安裝到 OpenClaw 時，請使用 OpenClaw。當你要登入、發布、管理自己的清單，或使用登錄庫專屬工作流程時，請使用 `clawhub` 命令列介面。
...
安裝前，請使用 ClawHub 網頁或命令列介面的詳細資訊命令，檢查中繼資料、來源連結、版本、變更記錄與掃描狀態：
...
公開清單會顯示最新的掃描狀態。因審核而被保留或封鎖的版本，在問題解決前可能會從搜尋與安裝介面中隱藏。
