...
當你要將東西安裝到 OpenClaw 中時，請使用 OpenClaw。當你要登入、發布、管理自己的列表，或使用登錄檔專屬工作流程時，請使用 `clawhub` 命令列介面。
...
對於包含多個技能的儲存庫，可重複使用的 GitHub 工作流程會對 `skills/` 下每個直接的技能資料夾呼叫 `skill publish`：
...
公開列表會顯示最新掃描狀態。因審核而保留或封鎖的發布版本，可能會在問題解決前從搜尋與安裝介面中隱藏。
