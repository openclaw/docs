...
read_when:
    ...
    - 發布到 ClawHub
summary: 開始使用 ClawHub：尋找、安裝、更新及發布技能或外掛。
x-i18n:
...
當你要將內容安裝到 OpenClaw 時，請使用 OpenClaw。當你要登入、發布、管理自己的列表，或使用登錄檔專屬工作流程時，請使用 `clawhub` 命令列介面。
...
skill 是一個資料夾，其中包含必要的 `SKILL.md` 檔案和選用的支援檔案。
...
對於包含多個 skills 的儲存庫，可重複使用的 GitHub 工作流程會對 `skills/` 下每個直接 skill 資料夾呼叫 `skill publish`：
...
請先使用 `--dry-run` 預覽解析出的套件中繼資料、相容性欄位、來源署名和上傳計畫，而不進行發布。
...
公開列表會顯示最新的掃描狀態。因審核而保留或封鎖的版本，可能會在解決前從搜尋和安裝介面隱藏。
