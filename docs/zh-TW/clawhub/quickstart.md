...
read_when:
    - 首次使用 ClawHub
    - 從註冊庫安裝 Skills 或 Plugin
    - 發布至 ClawHub
summary: 開始使用 ClawHub：尋找、安裝、更新及發佈 Skills 或 Plugin。
x-i18n:
...
當你要將項目安裝到 OpenClaw 時，請使用 OpenClaw。當你要登入、發布、管理自己的列表，或使用註冊中心專屬工作流程時，請使用 `clawhub` CLI。
...
無頭環境可以使用 ClawHub 網頁 UI 中的 API token：
...
登入後，`sync` 也可能會傳送最小化的安裝快照，用於彙總安裝次數。請參閱 [遙測](/zh-TW/clawhub/telemetry)，了解會回報哪些內容以及如何選擇退出。
...
公開列表會顯示最新的掃描狀態。因審核而保留或封鎖的版本，在解決前可能會從搜尋和安裝介面中隱藏。
