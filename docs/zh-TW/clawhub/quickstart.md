...
當你要將項目安裝到 OpenClaw 中時，請使用 OpenClaw。當你要登入、發布、管理自己的列表，或使用登錄庫特定工作流程時，請使用 `clawhub` CLI。
...
當你已登入時，`sync` 也可能會傳送最小化的安裝快照，用於彙總安裝次數。請參閱 [遙測](/zh-TW/clawhub/telemetry)，了解回報內容以及如何選擇退出。
...
公開列表會顯示最新的掃描狀態。被審核保留或封鎖的版本，可能會在問題解決前從搜尋和安裝介面中隱藏。
