...
x-i18n:
    generated_at: "2026-07-03T02:41:52Z"
    model: gpt-5.5
...
描述、需求、環境變數與中繼資料。準確的中繼資料很重要，因為它能
協助使用者判斷是否要安裝該 skill，也能協助自動化掃描偵測宣告行為與觀察到的行為之間的不一致。
...
使用 dry run 在上傳前預覽解析後的酬載。公開頁面接著會
...
OpenClaw 會記錄安裝來源中繼資料，讓更新之後可以解析相同的
...
ClawHub 提供公開讀取 API，用於探索、搜尋、套件詳細資訊與
