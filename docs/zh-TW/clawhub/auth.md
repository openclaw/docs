...
x-i18n:
    generated_at: "2026-05-11T20:22:55Z"
    model: gpt-5.5
    provider: openai
...
已刪除、遭停權或停用的帳號無法完成一般 ClawHub 登入。如果登入後又回到登出狀態，你的帳號可能狀態不良。
...
如果因為防火牆、VPN 或 Proxy 規則導致你的瀏覽器無法連到本機回呼，請使用無頭權杖流程。

## 無頭登入

在 ClawHub 網頁使用者介面中建立權杖，然後將它傳給 CLI：
...
CLI 會印出一次性代碼，並在你於 `https://clawhub.ai/cli/device` 授權時等待。
