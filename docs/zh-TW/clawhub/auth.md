...
read_when:
    ...
    - 使用 ClawHub 命令列介面
    - 偵錯 401 錯誤
summary: ClawHub 登入、API 權杖、命令列介面登入、權杖儲存和撤銷。
x-i18n:
...
已刪除、遭封鎖或停用的帳戶無法完成一般 ClawHub 登入。如果登入後又回到登出狀態，你的帳戶可能狀態不佳。如果你的帳戶遭封鎖或停用，且你認為這是錯誤，請使用 [ClawHub 申訴表單](https://appeals.openclaw.ai/)。
...
如果你的瀏覽器因防火牆、VPN 或 Proxy 規則而無法連到本機回呼，請使用無周邊權杖流程。
...
此流程適用於伺服器、CI 工作或僅限終端機的環境。

對於你可在其他地方開啟瀏覽器的遠端 shell，請執行：
