...
已刪除、遭停權或遭停用的帳戶無法完成一般 ClawHub 登入。如果登入後回到已登出狀態，你的帳戶可能不是良好狀態。如果你的帳戶遭停權或停用，且你認為這是錯誤，請使用 [ClawHub 申訴表單](https://appeals.openclaw.ai/)。
...
如果你的瀏覽器因為防火牆、VPN 或 Proxy 規則而無法連到本機回呼，請使用無頭權杖流程。

## 無頭登入

在 ClawHub 網頁 UI 建立權杖，然後傳給命令列介面：
...
已刪除、遭停權或遭停用的帳戶無法繼續使用既有 API 權杖。如果你的帳戶遭停權或停用，且你認為這是錯誤，請使用 [ClawHub 申訴表單](https://appeals.openclaw.ai/)。
