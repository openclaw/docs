...
ClawHub 使用擁有者識別名稱、組織識別名稱、技能 slug、外掛套件名稱，以及
套件 scope 作為公開命名空間。如果某個命名空間看起來屬於
真實世界的專案、品牌、套件生態系或組織，但在 ClawHub 上已被
申請、保留、具誤導性或存在爭議，請要求工作人員透過
...
保留、轉移、重新命名、隱藏、隔離、建立別名，
或以其他方式變更時，請提出命名空間申請。

範例包括：

- 與你的 GitHub 組織、專案、公司或社群相符的組織識別名稱
- 例如 `@example-org/*` 這類應僅由相符 ClawHub 擁有者
  發布的套件 scope
- 看起來冒充某個專案的技能 slug 或外掛套件名稱
- 品牌、商標、專案重新命名或套件歷史爭議
- 已刪除、停用或無法聯絡的擁有者，阻擋了合法命名空間
  擁有者

如果該 listing 在所有權爭議之外還有不安全、惡意或誤導問題，
也請遵循相關的審核或安全指引。命名空間申請
表單用於所有權審查，而不是緊急漏洞揭露。

## 提交前

請先確認你是以與該命名空間相符的擁有者身分發布。
對於外掛套件，像 `@example-org/example-plugin` 這樣的 scoped 名稱必須以
...
請勿在公開 GitHub issue 中放入機密或私人證明。請勿包含：
