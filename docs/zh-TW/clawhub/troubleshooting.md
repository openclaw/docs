...
x-i18n:
    generated_at: "2026-07-02T13:58:10Z"
    model: gpt-5.5
...
- `RateLimit-Remaining`：當標頭存在時，你的確切剩餘配額。在 `429` 時，它是 `0`。
- `RateLimit-Reset` 或 `X-RateLimit-Reset`：重設時間。

如果許多使用者共用同一個出口 IP，即使每個人只傳送少量請求，也可能觸發匿名 IP 限制。請盡可能登入，並在回報的延遲後重試。
...
- 確認你是使用擁有該套件或可發布該套件的 GitHub 帳號登入。
- 檢查來源 URL 是公開的，或 ClawHub 可存取。
- 對於 GitHub 來源，請使用 `owner/repo`、`owner/repo@ref` 或完整的 GitHub URL。

## 發布失敗，因為命名空間已被宣告或保留

如果發布因擁有者帳號、組織命名空間、套件範圍、Skill slug 或套件名稱已被宣告或保留而失敗，請先確認你是使用符合該命名空間的擁有者進行發布。對於外掛套件，範圍名稱例如 `@example-org/example-plugin` 必須以相符的 `example-org` 擁有者發布。

如果你認為你的組織、專案或品牌是合法的命名空間擁有者，但無法管理目前的 ClawHub 擁有者，請提交
[組織／命名空間宣告議題](https://github.com/openclaw/clawhub/issues/new?template=org-namespace-claim.yml)
並附上公開、非敏感的證明。請參閱
[組織與命名空間宣告](/clawhub/namespace-claims) 以了解證明指引，以及哪些內容應避免放入公開議題。
