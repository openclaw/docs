---
read_when:
    - ClawHub CLI 或 OpenClaw registry 命令失敗
    - 套件無法安裝、發布或更新
...
對於 Plugin，請檢查 `package.json` 相容性中繼資料。程式碼 Plugin 發布需要 OpenClaw 相容性欄位，例如 `openclaw.compat.pluginApi` 和 `openclaw.build.openclawVersion`。
