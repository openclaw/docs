...
x-i18n:
    generated_at: "2026-07-12T21:23:13Z"
    model: gpt-5.6
...
## `clawhub login` 開啟瀏覽器，但始終無法完成

命令列介面會在瀏覽器登入期間啟動一個短暫運作的本機回呼伺服器。

- 確認你的瀏覽器可以連線至 `http://127.0.0.1:<port>/callback`。
...
- `RateLimit-Remaining`：標頭存在時，你確切的剩餘額度。發生 `429` 時，此值為 `0`。
- `RateLimit-Reset` 或 `X-RateLimit-Reset`：重設時間。

如果許多使用者共用同一個對外 IP，即使每個人只傳送少量要求，也可能
觸及匿名 IP 限制。請盡可能登入，並在回報的延遲時間過後重試。
...
- 如果你知道確切的 slug 或擁有者頁面，請直接查看。
- 確認該版本為公開狀態，且未因掃描或內容審核而遭到保留。
- 如果你擁有該 Skill，請登入並檢查：
...
僅擁有者可見的診斷資訊可能會說明掃描、上傳閘門或內容審核狀態。

## 發布因缺少必要中繼資料而失敗

若是 Skills，請檢查 `SKILL.md` frontmatter。應宣告必要的環境變數和
工具，讓使用者與掃描器可以瞭解套件。

若是外掛，請檢查 `package.json` 相容性中繼資料。程式碼外掛的發布
...
- 確認來源 URL 為公開，或 ClawHub 可存取該 URL。
- 若為 GitHub 來源，請使用 `owner/repo`、`owner/repo@ref` 或完整的 GitHub URL。

## 發布因命名空間已被宣告或保留而失敗

如果發布因擁有者代號、組織命名空間、套件範圍、Skill
slug 或套件名稱已被宣告或保留而失敗，請先確認你是使用與命名空間相符的
擁有者身分進行發布。若是外掛套件，
`@example-org/example-plugin` 等具有範圍的名稱，必須以相符的
`example-org` 擁有者身分發布。

如果你認為自己的組織、專案或品牌才是該命名空間的合法擁有者，但
無法管理目前的 ClawHub 擁有者，請使用公開且非敏感的證明，建立
[組織／命名空間宣告問題](https://github.com/openclaw/clawhub/issues/new?template=org-namespace-claim.yml)。
如需證明指引，以及哪些內容不應放入公開問題中，請參閱
...
本機檔案與 ClawHub 已知的任何版本都不相符。請選擇其中一種方式：
...
- 在套件詳細資料頁面查看掃描狀態和相容性中繼資料。
- 確認你的 OpenClaw 版本符合套件所標示的
  相容版本範圍。
- 如果套件被隱藏、保留或封鎖，在擁有者解決問題之前，
  可能無法安裝。

## 公開 API 要求失敗

- 遵循 `429` 重試標頭，並快取公開清單／搜尋回應。
- 將使用者連結回標準的 ClawHub 清單頁面。
- 請勿在公開 API 範圍之外鏡像隱藏、私人、保留或遭內容審核封鎖的
  內容。
