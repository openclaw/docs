---
read_when:
    - ClawHub CLI 或 OpenClaw 註冊庫命令失敗
    - 無法安裝、發布或更新套件
summary: 疑難排解 ClawHub 登入、安裝、發布、同步、更新與 API 問題。
x-i18n:
...
- 如果回呼一直沒有到達，請檢查本機防火牆、VPN 和 Proxy 規則。
...
如果許多使用者共用同一個對外連線 IP，即使每個人只傳送少量請求，也可能觸發匿名 IP 限制。請盡可能登入，並在回報的延遲時間後重試。
...
對於 skills，請檢查 `SKILL.md` frontmatter。應宣告必要的環境變數和工具，讓使用者與掃描器能了解套件。

對於 plugins，請檢查 `package.json` 相容性中繼資料。Code-plugin 發佈需要 OpenClaw 相容性欄位，例如 `openclaw.compat.pluginApi` 和
...
- 確認你的 OpenClaw 版本符合套件宣告的
  相容性範圍。
- 如果套件已隱藏、被保留或被封鎖，在擁有者解決問題之前可能無法安裝。

## 公開 API 請求失敗

- 遵守 `429` 重試標頭，並快取公開清單/搜尋回應。
- 將使用者連回 ClawHub 的標準列表。
- 不要在公開 API 介面之外鏡像隱藏、私人、被保留或被審核封鎖的內容。
