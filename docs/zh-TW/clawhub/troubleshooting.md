...
- 如果你使用自訂設定路徑，請確認 `CLAWHUB_CONFIG_PATH` 指向包含目前權杖的
  檔案。
- 如果你使用 API 權杖，請確認該權杖未在網頁 UI 中遭撤銷。
...
- `RateLimit-Remaining`：當標頭存在時，你確切的剩餘額度。在 `429` 時，其值為 `0`。
- `RateLimit-Reset` 或 `X-RateLimit-Reset`：重設時間。

如果許多使用者共用同一個對外 IP，即使每個人只傳送少量請求，也可能觸及
匿名 IP 限制。請盡可能登入，並在回報的延遲時間後重試。
...
對於外掛，請檢查 `package.json` 相容性中繼資料。程式碼外掛的發布
...
- 確認你已使用擁有該套件或可發布該套件的 GitHub 帳號
  登入。
- 檢查來源 URL 是否公開或可供 ClawHub 存取。
...
擁有者身分發布。對於外掛套件，`@example-org/example-plugin` 之類的
範圍名稱必須以相符的 `example-org` 擁有者身分發布。

如果你認為你的組織、專案或品牌才是該命名空間的合法擁有者，但
無法管理目前的 ClawHub 擁有者，請建立
[組織／命名空間認領議題](https://github.com/openclaw/clawhub/issues/new?template=org-namespace-claim.yml)，
並提供公開且不含敏感資訊的證明。關於證據指引以及哪些內容
不應放入公開議題，請參閱
...
本機檔案與 ClawHub 已知的任何版本都不相符。請選擇一種做法：
...
- 確認你的 OpenClaw 版本符合套件所公布的
  相容性範圍。
- 如果套件遭隱藏、擱置或封鎖，在擁有者解決問題之前可能
  無法安裝。

## 公開 API 請求失敗

- 遵循 `429` 重試標頭，並快取公開清單／搜尋回應。
- 將使用者連結回正式的 ClawHub 列表頁面。
- 請勿在公開 API 範圍之外鏡像隱藏、私人、擱置或遭審核封鎖的
  內容。
