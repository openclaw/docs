...
x-i18n:
    generated_at: "2026-07-05T01:55:09Z"
    model: gpt-5.5
...
- `RateLimit-Remaining`：標頭存在時，你的確切剩餘額度。在 `429` 上，它是 `0`。
- `RateLimit-Reset` 或 `X-RateLimit-Reset`：重設時間。

如果許多使用者共用同一個對外 IP，即使每個人只送出少量請求，也可能觸發匿名 IP 限制。請盡可能登入，並在回報的延遲後重試。
...
如果發布因擁有者 handle、組織命名空間、套件 scope、Skills slug 或套件名稱已被聲明或保留而失敗，請先確認你是使用與該命名空間相符的擁有者進行發布。對於外掛套件，像 `@example-org/example-plugin` 這類 scoped 名稱必須以相符的 `example-org` 擁有者發布。
...
- 將使用者連回標準 ClawHub 列表。
- 不要在公開 API 表面之外鏡像隱藏、私有、保留或被審核封鎖的內容。
