...
x-i18n:
    generated_at: "2026-07-05T17:41:13Z"
    model: gpt-5.5
...
- `RateLimit-Remaining`：當標頭存在時，你的精確剩餘額度。在 `429` 時為 `0`。
- `RateLimit-Reset` 或 `X-RateLimit-Reset`：重設時間。

如果許多使用者共用同一個對外 IP，即使每個人只送出少量請求，也可能會觸發匿名 IP 限制。可行時請登入，並在回報的延遲時間後重試。
...
對於 Skills，請檢查 `SKILL.md` frontmatter。應宣告必要的環境變數和工具，讓使用者和掃描器能理解該套件。
...
- 確認你已使用擁有該套件或可發布該套件的 GitHub 帳戶登入。
- 檢查來源 URL 是公開的，或 ClawHub 可存取。
- 對於 GitHub 來源，請使用 `owner/repo`、`owner/repo@ref` 或完整 GitHub URL。

## 發布因命名空間已被宣告或保留而失敗

如果發布因擁有者 handle、組織命名空間、套件 scope、Skill slug 或套件名稱已被宣告或保留而失敗，請先確認你是使用符合該命名空間的擁有者進行發布。對於外掛套件，像 `@example-org/example-plugin` 這類 scoped 名稱必須以對應的 `example-org` 擁有者發布。

如果你認為你的組織、專案或品牌是該命名空間的合法擁有者，但你無法管理目前的 ClawHub 擁有者，請開啟一個 [組織 / 命名空間宣告 issue](https://github.com/openclaw/clawhub/issues/new?template=org-namespace-claim.yml)，並提供公開、非敏感的證明。請參閱[組織與命名空間宣告](/clawhub/namespace-claims)，了解證據指南以及哪些內容不應放在公開 issue 中。
...
- 如果套件已隱藏、暫留或封鎖，可能必須等擁有者解決問題後才能安裝。

## 公開 API 請求失敗

- 遵守 `429` 重試標頭，並快取公開列表/搜尋回應。
- 將使用者連回標準 ClawHub 列表頁。
- 不要在公開 API surface 之外鏡像隱藏、私人、暫留或因審核而封鎖的內容。
