...
- 如果你使用 API token，請確認它沒有在 Web UI 中被撤銷。
...
如果許多使用者共用同一個出口 IP，即使每個人只傳送少量請求，也可能達到匿名 IP 限制。請盡可能登入，並在回報的延遲時間後重試。
...
對於技能，請檢查 `SKILL.md` frontmatter。應宣告必要的環境變數和工具，讓使用者與掃描器能了解套件。

對於 Plugin，請檢查 `package.json` 相容性中繼資料。發布 code-plugin 時需要 OpenClaw 相容性欄位，例如 `openclaw.compat.pluginApi` 和 `openclaw.build.openclawVersion`。
...
本機檔案與 ClawHub 已知的任何版本都不相符。請選擇其中一項：
...
- 將使用者連回正式的 ClawHub 清單頁面。
- 請勿在公開 API 介面之外鏡像隱藏、私人、保留或遭審核封鎖的內容。
