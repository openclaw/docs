...
x-i18n:
    generated_at: "2026-07-01T10:56:57Z"
    model: gpt-5.5
...
對於 Skills，請檢查 `SKILL.md` frontmatter。必要的環境變數和工具應宣告清楚，讓使用者和掃描器能理解此套件。
...
如果發布因擁有者代號、組織命名空間、套件範圍、skill slug 或套件名稱已被宣告或保留而失敗，請先確認你正使用符合該命名空間的擁有者身分發布。對於外掛套件，
`@example-org/example-plugin` 這類帶範圍名稱必須以相符的 `example-org` 擁有者發布。
