...
x-i18n:
    generated_at: "2026-07-04T10:27:00Z"
    model: gpt-5.5
...
- `RateLimit-Remaining`：標頭存在時，你的確切剩餘配額。在 `429` 時為 `0`。
...
如果發布因擁有者 handle、組織命名空間、套件範圍、技能 slug 或套件名稱已被宣告或保留而失敗，請先確認你是以符合該命名空間的擁有者身分發布。對於外掛套件，像 `@example-org/example-plugin` 這類 scoped 名稱必須以相符的 `example-org` 擁有者發布。
...
- 將使用者連回正式的 ClawHub 列表頁。
- 請勿在公開 API 表面之外鏡像隱藏、私人、保留或被審核封鎖的內容。
