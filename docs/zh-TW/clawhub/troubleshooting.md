...
x-i18n:
    generated_at: "2026-06-30T13:46:41Z"
    model: gpt-5.5
...
對於技能，請檢查 `SKILL.md` frontmatter。應宣告必要的環境變數和工具，讓使用者和掃描器能理解該套件。
...
ClawHub 會使用 GitHub 身分和來源歸因，將套件連結到其發布者。

- 請確認你使用擁有或可發布該套件的 GitHub 帳號登入。
- 檢查來源 URL 是公開的，或 ClawHub 可存取。
- 對於 GitHub 來源，請使用 `owner/repo`、`owner/repo@ref` 或完整的 GitHub URL。

## 發布因命名空間已被宣告或保留而失敗

如果發布失敗是因為擁有者 handle、組織命名空間、套件 scope、技能 slug 或套件名稱已被宣告或保留，請先確認你是使用符合該命名空間的擁有者來發布。對於外掛套件，像 `@example-org/example-plugin` 這樣的 scoped 名稱必須以相符的 `example-org` 擁有者發布。
...
- 將使用者連回標準 ClawHub 列表。
- 不要在公開 API 表面以外鏡像隱藏、私有、保留或因審核封鎖的內容。
