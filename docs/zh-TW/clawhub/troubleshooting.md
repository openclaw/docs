...
x-i18n:
    generated_at: "2026-07-05T11:06:16Z"
    model: gpt-5.5
...
- `RateLimit-Remaining`：當標頭存在時，你確切剩餘的配額。在 `429` 時，它是 `0`。
...
如果發布因擁有者 handle、組織命名空間、套件 scope、Skills slug 或套件名稱已被宣告或保留而失敗，請先確認你是使用符合該命名空間的擁有者進行發布。對於外掛套件，像 `@example-org/example-plugin` 這類 scoped 名稱必須以相符的 `example-org` 擁有者發布。

如果你認為你的組織、專案或品牌是該命名空間的正當擁有者，但無法管理目前的 ClawHub 擁有者，請開啟一個
[組織／命名空間宣告議題](https://github.com/openclaw/clawhub/issues/new?template=org-namespace-claim.yml)，並提供公開且非敏感的證明。請參閱
[組織與命名空間宣告](/clawhub/namespace-claims)，了解證據指引以及哪些內容不應放入公開議題。
...
- 如果該套件被隱藏、保留或封鎖，可能要等擁有者解決問題後才能安裝。

## 公開 API 請求失敗

- 遵守 `429` 重試標頭，並快取公開清單／搜尋回應。
- 將使用者連回正式的 ClawHub 列表。
- 不要在公開 API 表面之外鏡像隱藏、私人、保留或遭審核封鎖的內容。
