...
命令列介面會在瀏覽器登入期間啟動一個短生命週期的本機回呼伺服器。

- 確認你的瀏覽器可以連到 `http://127.0.0.1:<port>/callback`。
- 如果一直沒有收到回呼，請檢查本機防火牆、VPN 和代理規則。
- 在無介面環境中，請在 ClawHub 網頁 UI 中建立 API token，然後執行：
...
- `RateLimit-Remaining`：標頭存在時，你精確的剩餘配額。在 `429` 時，它是 `0`。
- `RateLimit-Reset` 或 `X-RateLimit-Reset`：重設時間。

如果多位使用者共用同一個出口 IP，即使每個人只送出少量請求，也可能觸發匿名 IP 限制。請盡可能登入，並在回報的延遲時間後重試。
...
對於 Skills，請檢查 `SKILL.md` frontmatter。應宣告必要的環境變數和工具，讓使用者和掃描器可以理解套件。
...
- 檢查來源 URL 是否公開或 ClawHub 可存取。
- 對於 GitHub 來源，請使用 `owner/repo`、`owner/repo@ref` 或完整 GitHub URL。

## 發布因命名空間已被宣告或保留而失敗

如果發布因擁有者 handle、組織命名空間、套件 scope、Skill slug 或套件名稱已被宣告或保留而失敗，請先確認你是使用與該命名空間相符的擁有者進行發布。對於外掛套件，像 `@example-org/example-plugin` 這類 scoped 名稱必須以相符的 `example-org` 擁有者發布。
...
- 不要在公開 API 介面以外鏡像隱藏、私有、保留或被審核封鎖的內容。
