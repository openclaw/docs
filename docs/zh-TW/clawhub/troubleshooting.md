...
read_when:
    ...
    - 無法安裝、發布或更新套件
summary: ClawHub 登入、安裝、發布、同步、更新和 API 問題的疑難排解。
x-i18n:
...
如果多位使用者共用同一個出口 IP，即使每個人只傳送少量請求，也可能觸及匿名 IP 限制。請盡可能登入，並在回報的延遲時間後重試。
...
對於 Skills，請檢查 `SKILL.md` frontmatter。必要的環境變數和工具應該宣告清楚，讓使用者和掃描器能理解該套件。

對於 plugins，請檢查 `package.json` 相容性中繼資料。程式碼 plugin 發布需要 OpenClaw 相容性欄位，例如 `openclaw.compat.pluginApi` 和 `openclaw.build.openclawVersion`。
...
- 請勿在公開 API 表面以外鏡像隱藏、私人、保留或被審核封鎖的內容。
