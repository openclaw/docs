...
- 如果一直收不到回呼，請檢查本機防火牆、VPN 和代理伺服器規則。
- 在無介面環境中，請在 ClawHub 網頁 UI 建立 API 權杖，然後執行：
...
如果許多使用者共用同一個出口 IP，即使每個人只傳送少量請求，也可能觸及匿名 IP 限制。可行時請登入，並在回報的延遲時間後重試。
...
對於 plugins，請檢查 `package.json` 相容性中繼資料。程式碼 Plugin 發布需要 OpenClaw 相容性欄位，例如 `openclaw.compat.pluginApi` 和
