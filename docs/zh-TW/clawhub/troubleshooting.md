...
read_when:
    ...
    - 套件無法安裝、發布或更新
summary: 排解 ClawHub 登入、安裝、發布、更新與 API 問題。
x-i18n:
...
- `RateLimit-Remaining`：標頭存在時，你確切剩餘的額度。在 `429` 時，它是 `0`。
- `RateLimit-Reset` 或 `X-RateLimit-Reset`：重設時間。

如果許多使用者共用同一個出站 IP，即使每個人只送出少量請求，也可能觸發匿名 IP 限制。可行時請登入，並在回報的延遲時間後重試。
...
如果發布失敗是因為擁有者 handle、組織命名空間、套件 scope、skill slug 或套件名稱已被宣告或保留，請先確認你是以符合該命名空間的擁有者身分發布。對於外掛套件，像 `@example-org/example-plugin` 這樣的 scoped 名稱必須由相符的 `example-org` 擁有者發布。

如果你認為自己的組織、專案或品牌是該命名空間的正當擁有者，但你無法管理目前的 ClawHub 擁有者，請使用公開且非敏感的證明開啟一個[組織／命名空間宣告 issue](https://github.com/openclaw/clawhub/issues/new?template=org-namespace-claim.yml)。請參閱[組織與命名空間宣告](/clawhub/namespace-claims)，了解證據指引以及哪些內容應避免放入公開 issue。
