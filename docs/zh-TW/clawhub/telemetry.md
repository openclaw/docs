...
x-i18n:
    generated_at: "2026-07-19T13:37:55Z"
    model: gpt-5.6
...
ClawHub 使用最低限度的命令列介面遙測資料，計算彙總安裝次數。
...
每次回報 `clawhub install` 時，命令列介面會以盡力而為的方式傳送一個安裝事件。
...
- `installsAllTime`：曾回報至少一次該 Skill 命令列介面安裝事件的不重複使用者人數。
- `installsCurrent`：曾回報安裝事件，且未刪除其
  遙測資料的不重複使用者人數。
