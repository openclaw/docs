...
x-i18n:
    generated_at: "2026-07-04T10:26:20Z"
    model: gpt-5.5
...
第三方目錄可以使用公開讀取端點來列出或搜尋 ClawHub Skills。請快取結果、遵守 `429`/`Retry-After`、將使用者連回標準 ClawHub 條目（`https://clawhub.ai/<owner>/skills/<slug>`），並避免暗示 ClawHub 背書第三方網站。請勿嘗試在公開 API 表面之外鏡像隱藏、私人或因審核而封鎖的內容。

網頁 slug 快捷方式會跨登錄檔系列解析，但 API 用戶端應使用讀取端點傳回的標準 URL，而不是自行重建路由優先順序。
...
- 如果 token 缺失或無效，行為會退回到 IP 強制執行。
- 已驗證的寫入端點在伺服器知道原因時，不應傳回空泛的 `Unauthorized`。缺失 token、無效/已撤銷 token，以及已刪除/封禁/停用帳戶，都應取得可行動的文字，讓命令列介面用戶端可以告知使用者是什麼阻擋了他們。

- 讀取：每個 IP 每分鐘 3000 次，每個 key 每分鐘 12000 次
- 寫入：每個 IP 每分鐘 300 次，每個 key 每分鐘 3000 次
- 下載：每個 IP 每分鐘 1200 次，每個 key 每分鐘 6000 次（下載端點）
...
- 使用帶有抖動的退避，以避免同步重試。
- 如果 `Retry-After` 缺失，請退回使用 `RateLimit-Reset`（或從 `X-RateLimit-Reset` 計算）。

IP 來源：

- 只有在部署明確啟用可信任的轉送標頭時，才會使用可信任的用戶端 IP 標頭，包括 `cf-connecting-ip`。
- ClawHub 使用可信任的轉送標頭在邊緣識別用戶端 IP。
- 如果沒有可用的可信任用戶端 IP，匿名請求會使用僅依速率限制種類設定範圍的備援 bucket。這些備援 bucket 不包含呼叫者提供的路徑、slug、套件名稱、版本、查詢字串或其他成品參數。

## 錯誤回應

公開 v1 錯誤回應是純文字，`content-type: text/plain; charset=utf-8`。
這包括驗證失敗（`400`）、缺失的公開資源（`404`）、驗證與權限失敗（`401`/`403`）、速率限制（`429`）以及遭封鎖的下載。用戶端應將回應本文讀取為人類可讀的字串。未知查詢參數會為了相容性而被忽略，但已識別的查詢參數若值無效則會傳回 `400`。
...
- 相關性比人氣更重要。精確的 slug 或顯示名稱 token 相符，可以排在互動更強但相符較鬆散的結果之前。
- ASCII 文字會依單字與標點邊界切分 token。例如，`personal-map` 包含獨立的 `map` token，而 `amap-jsapi-skill` 包含 `amap`、`jsapi` 與 `skill`；因此搜尋 `map` 時，`personal-map` 會比 `amap-jsapi-skill` 有更強的詞彙相符。
- 人氣會以對數縮放並設上限。高互動 Skills 在查詢文字相符較弱時可能排名較低。
- 可疑或隱藏的審核狀態可能會依呼叫者篩選器與目前審核狀態，將某個 Skill 從公開搜尋中移除。

發布者可探索性指引：

- 將使用者會實際搜尋的詞放在顯示名稱、摘要與標籤中。只有當獨立 slug token 也是你想保留的穩定身分時才使用。
- 不要只為了追逐單一查詢而重新命名 slug，除非新的 slug 是更好的長期標準名稱。舊 slug 會成為重新導向別名，但標準 URL、顯示的 slug 與未來搜尋摘要會使用新的 slug。
- 重新命名別名會保留舊 URL 與透過登錄檔解析之安裝的解析能力，但搜尋排名會以重新命名完成索引後的標準 Skill 中繼資料為準。現有統計資料會保留在該 Skill 上。
...
- `createdAt` 對新 Skill 爬取而言是穩定的；`updated` 會在現有 Skills 重新發布時變更。
- 當 `nonSuspiciousOnly=true` 時，基於游標的排序可能會在單頁傳回少於 `limit` 的項目，因為可疑 Skills 會在頁面擷取後被篩除。
- 存在 `nextCursor` 時，使用它繼續分頁。短頁面本身不代表結果已結束。
...
回報某個 Skill 以供審核者檢閱。回報屬於 Skill 層級，可選擇連結到某個版本，並會進入 Skill 回報佇列。
...
`confirmed` 與 `dismissed` 需要 `note`；將 `status` 設回 `open` 時可以省略。傳入 `finalAction: "hide"` 搭配已分流的回報，可在同一個可稽核工作流程中隱藏該 Skill。
...
- 可用時，`version.security` 會包含正規化的掃描驗證狀態與掃描器詳細資料
  （VirusTotal + LLM）。
...
- 掃描作業是非同步的。手動掃描請求會優先於一般發布/回填工作，但完成時間仍取決於工作程序可用性。
...
- 傳回已排隊/執行中/成功/失敗狀態。
- 排隊時傳回 `queue.queuedAhead` 和 `queue.position`，讓用戶端可顯示有多少優先手動掃描排在此請求前。非常大的佇列會受到上限限制，並以 `queuedAheadIsEstimate: true` 回報。
...
傳回 `clawhub skill verify` 使用的技能卡驗證信封。
...
- 只有在所選版本已有產生的技能卡、未被審核標記為惡意軟體封鎖，且 ClawScan 驗證為乾淨時，`ok` 才會是 `true`。
- 技能身分、發布者身分和所選版本中繼資料都是頂層信封欄位（`slug`、`displayName`、`publisherHandle`、`version`、`resolvedFrom`、`tag`、`createdAt`），讓 shell 自動化可在不解開巢狀包裝的情況下讀取它們。
...
- `security.signals.dependencyRegistry` 會保留以維持 v1 回應相容性，但相依套件登錄存在性掃描器已退役，且此鍵一律為 `null`。
...
傳回確切技能版本目前的精簡安全性判定。此集合端點適用於已知道需要顯示哪些已安裝 ClawHub 技能版本的用戶端，例如 OpenClaw Control UI。
...
- `channel=private` 只會傳回已驗證呼叫端可讀取的套件。

### `GET /api/v1/packages/search`

跨 Skills + 外掛套件的統一型錄搜尋。
...
- `channel`（選填）：`official`、`community` 或 `private`
- `isOfficial`（選填）：`true` 或 `false`
- `category`（選填）：外掛分類篩選器。僅在要求範圍限定為外掛套件時支援。受控分類與舊版 v1
...
在統一 `/api/v1/packages` 端點上，它僅限外掛；Skills 型錄請使用
...
- 目前由 GitHub 支援且掃描結果為 `clean` 或 `suspicious` 的 Skills 包含
  ...
  content hash 和 archive URL。它們不包含 ClawHub 託管的原始碼檔案。
...
傳回一個套件版本，包括檔案中繼資料、相容性、驗證、成品中繼資料和掃描資料。
...
- ClawPack 發行版本包含與 npm 相容的 `npmIntegrity`、`npmShasum` 和
  `npmTarballName` 欄位。
- `version.sha256hash` 是供舊用戶端使用的已棄用相容性中繼資料。它會對
  `/api/v1/packages/{name}/download` 傳回的精確 ZIP 位元組進行雜湊。
...
傳回供安裝用戶端使用的精確套件發行安全與信任摘要。這是公開的 OpenClaw 消費介面，
用於判斷已解析的發行版本是否可以安裝。
...
- `packageName` 會經過 npm 名稱正規化；規劃中的遷移可以缺少套件。
- 這只追蹤遷移就緒狀態。不會變更 OpenClaw 或產生 ClawPacks。
...
回報套件以供版主審查。回報是套件層級，可選擇連結到某個版本。它們會送入審核佇列，
但本身不會自動隱藏或封鎖下載；版主應使用發行審核來核准、隔離或撤銷成品。
...
`note` 在 `confirmed` 和 `dismissed` 時為必填；將 `status` 設回 `open` 時可以省略。針對已確認的檢舉傳入 `finalAction: "quarantine"` 或 `finalAction: "revoke"`，即可在同一個可稽核的工作流程中套用發布版本審核。
...
已隔離和已撤銷的發布版本會從成品下載路由回傳 `403`。每次變更都會寫入一筆稽核記錄。
...
- 二進位檔案回傳 `415`。
- 檔案大小限制：200KB。
- 待處理的 VirusTotal 掃描不會封鎖讀取；惡意發布版本仍可能在其他地方被扣留。
- 私有套件會回傳 `404`，除非呼叫者可以讀取所屬發布者。

### `GET /api/v1/packages/{name}/download`

下載套件發布版本的舊版確定性 ZIP 封存。
...
- 外掛/套件封存是具有 `package/` 根目錄的 zip 檔案，因此舊版 OpenClaw 用戶端可繼續運作。
- 此路由僅維持 ZIP。它不會串流 ClawPack `.tgz` 檔案。
- 回應包含 `ETag`、`Digest`、`X-ClawHub-Artifact-Type` 和 `X-ClawHub-Artifact-Sha256` 標頭，供解析器完整性檢查使用。
- 僅註冊表中存在的中繼資料不會注入到下載的封存中。
- 待處理的 VirusTotal 掃描不會封鎖下載；惡意發布版本會回傳 `403`。
...
- 舊版僅 ZIP 版本會刻意省略。
- `dist.tarball`、`dist.integrity` 和 `dist.shasum` 使用 npm 相容欄位，因此使用者可在選擇時將 npm 指向該鏡像。
- 具作用域的套件 packument 同時支援 `/api/npm/@scope/name` 和 npm 編碼的 `/api/npm/@scope%2Fname` 請求路徑。
...
下載託管的 skill 版本 ZIP，或針對目前由 GitHub 支援、掃描結果為 `clean` 或 `suspicious` 且沒有託管版本的 skill，回傳 GitHub 來源交接資訊。
...
- 軟刪除的版本會回傳 `410`。
- 由 GitHub 支援的 skill 交接不會代理或鏡像位元組。JSON 回應包含 `sourceRef: "public-github"`、`repo`、`commit`、`path`、`contentHash` 和 `archiveUrl`；掃描/目前狀態是閘門，不會作為成功酬載中繼資料包含。
- 下載統計會以每個 UTC 日的唯一身分計算（API 權杖有效時為 `userId`，否則為 IP）。
...
- 偏好：使用包含 `payload` JSON + `files[]` blob 的 `multipart/form-data`。
- 也接受包含 `files`（基於 storageId）的 JSON 主體。
- 選填酬載欄位：`ownerHandle`。存在時，API 會在伺服器端解析該發布者，並要求執行者具備發布者存取權。
- 選填酬載欄位：`migrateOwner`。當與 `ownerHandle` 一起為 `true` 時，如果執行者同時是目前和目標發布者的管理員/擁有者，既有 skill 可移至該擁有者。未明確選擇此選項時，會拒絕擁有者變更。
...
- 允許的表單欄位為 `payload`、重複的 `files` blob，或一個 `clawpack` tarball 參照。`clawpack` 可以是 `.tgz` blob，或上傳 URL 流程回傳的 storage id。使用暫存 storage-id 發布時，也必須包含該上傳 URL 一併回傳的 `clawpackUploadTicket`。
- 使用 `files` 或 `clawpack` 其中之一，同一個請求中絕不可同時使用兩者。
- 會拒絕 JSON 主體和呼叫者提供的 `payload.files` / `payload.artifact` 中繼資料。
- 直接 multipart 發布請求上限為 18MB。ClawPack tarball 可使用上傳 URL 流程，最高達 120MB tarball 上限。
...
- 代表他人發布時，仍會依目標擁有者帳號驗證 official 頻道資格。
...
由擁有者發起的軟刪除會保留該 slug 30 天，之後該 slug 可由另一個發布者宣告使用。此到期適用時，刪除回應會包含 `slugReservedUntil`。
...
已驗證使用者可自助建立組織發布者。建立新的組織發布者，並將呼叫者加入為擁有者。此端點不會遷移既有使用者/個人 handle，也不會將發布者標記為受信任/官方。
...
僅限管理員。為合法擁有者保留根 slug 和套件名稱，而不發布版本。套件名稱會成為沒有發布版本列的私有預留位置套件，因此同一位擁有者稍後可將真正的 code-plugin 或 bundle-plugin 發布版本發布到該名稱。
...
僅限管理員。為已驗證的替代 GitHub OAuth 主體復原個人發布者，而不編輯 Convex Auth 帳號列。請求必須命名兩個不可變的 GitHub provider account id；可變 handle 僅作為面向操作員的防護。

端點預設為試執行。套用復原需要在工作人員獨立驗證兩個
GitHub 主體之間的連續性後，設定 `dryRun: false` 與
`confirmIdentityVerified: true`。當目的地使用者目前的個人
發布者已有 skills、套件或 GitHub skill 來源時，復原會採取失敗即封閉。
復原也會遷移已復原發布者的 skills、skill slug 別名、套件、套件檢查器警告，
以及衍生的搜尋摘要資料列中的舊版 `ownerUserId` 欄位，讓直接擁有者路徑
與新的發布者權限一致。已復原 handle 的有效受保護 handle 保留也會重新指派給
替代使用者，使後續的設定檔同步無法還原前使用者的競爭權限。每個主要資料表在
每次套用交易中限制為 100 筆資料列；更大的復原必須先使用可續傳的擁有者遷移。
GitHub skill 來源屬於發布者範圍，會回報為已檢查，而不是被重寫。
...
發布必須將產生的儲存空間 id 以 `clawpack` 傳送，並將回傳的 ticket 以 `clawpackUploadTicket` 傳送。
