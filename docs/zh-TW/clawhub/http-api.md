...
第三方目錄可以使用公開讀取端點來列出或搜尋 ClawHub Skills。請快取結果、遵守 `429`/`Retry-After`、將使用者連回正式的 ClawHub 條目（`https://clawhub.ai/<owner>/skills/<slug>`），並避免暗示 ClawHub 為該第三方網站背書。請勿嘗試在公開 API 介面之外鏡像隱藏、私人或遭審核封鎖的內容。
...
- 已驗證的寫入端點在伺服器知道原因時，不應只傳回單純的 `Unauthorized`。缺少 token、無效/已撤銷的 token，以及已刪除/遭禁止/停用的帳號，都應取得可採取行動的文字，讓命令列介面用戶端可以告知使用者阻擋原因。
...
- 如果 `Retry-After` 存在，請等待指定秒數後再重試。
- 使用帶有 jitter 的 backoff，避免同步重試。
...
- 如果沒有可用的受信任用戶端 IP，匿名請求會使用只依速率限制種類限定範圍的後備 bucket。這些後備 bucket 不包含呼叫方提供的路徑、slug、套件名稱、版本、查詢字串或其他成品參數。

## 錯誤回應

公開 v1 錯誤回應是純文字，並帶有 `content-type: text/plain; charset=utf-8`。
這包括驗證失敗（`400`）、缺少公開資源（`404`）、驗證與權限失敗（`401`/`403`）、速率限制（`429`），以及遭封鎖的下載。用戶端應將回應本文讀取為人類可讀字串。未知查詢參數會為相容性而忽略，但已識別查詢參數若具有無效值，會傳回 `400`。
...
- 結果會依相關性順序傳回（embedding 相似度 + 精確 slug/名稱 token 加權 + 小幅熱門度先驗）。
- 相關性比熱門度更強。精確的 slug 或顯示名稱 token 符合，可以排在互動量高得多但符合較寬鬆的結果之前。
- ASCII 文字會依字詞與標點邊界切分成 token。例如，`personal-map` 包含獨立的 `map` token，而 `amap-jsapi-skill` 包含 `amap`、`jsapi` 和 `skill`；因此搜尋 `map` 會讓 `personal-map` 比 `amap-jsapi-skill` 取得更強的詞彙符合。
- 熱門度會以對數縮放並設有上限。高互動量的 Skills 可能在查詢文字符合較弱時排名較低。
- 可疑或隱藏的審核狀態，可能會依呼叫方篩選條件與目前審核狀態，將某個 Skills 從公開搜尋中移除。

發布者可發現性指引：

- 將使用者會實際搜尋的詞放在顯示名稱、摘要和標籤中。只有當獨立 slug token 也是你想保留的穩定識別時，才使用它。
- 不要只為了追逐某個查詢而重新命名 slug，除非新 slug 是更好的長期正式名稱。舊 slug 會成為重新導向別名，但正式 URL、顯示的 slug，以及未來搜尋摘要會使用新 slug。
- 重新命名別名會保留舊 URL，以及透過登錄解析的安裝解析能力，但搜尋排名會以重新命名完成索引後的正式 Skills 中繼資料為準。既有統計會留在該 Skills 上。
- 如果某個 Skills 意外不可見，登入後先使用 `clawhub inspect @owner/slug` 檢查審核狀態，再變更與排名相關的中繼資料。
...
- 公開呼叫方看到的證據會經過遮蔽，只有擁有者/審核員會取得原始片段。

### `POST /api/v1/skills/{slug}/report`

回報某個 Skills 以供審核員檢閱。回報是 Skills 層級，可選擇連結到版本，並會送入 Skills 回報佇列。
...
供審核員/admin 使用，用於解決或重新開啟 Skills 回報的端點。
...
`note` 對 `confirmed` 和 `dismissed` 是必填；將 `status` 設回 `open` 時可以省略。對已分流的回報傳入 `finalAction: "hide"`，可在同一個可稽核工作流程中隱藏該 Skills。
...
- 回傳已排隊/執行中/成功/失敗狀態。
- 排隊時回傳 `queue.queuedAhead` 和 `queue.position`，讓用戶端可以顯示該請求前方有多少個優先手動掃描。非常大的佇列會受限並以 `queuedAheadIsEstimate: true` 回報。
- 可用時，`report` 會包含 `clawscan`、`skillspector`、`staticAnalysis` 和 `virustotal` 區段。
- 失敗的掃描工作會回傳帶有 `lastError` 的 `status: "failed"`。
...
- 只有在所選版本有已產生的技能卡、未被審核判定為惡意軟體而封鎖，且 ClawScan 驗證為乾淨時，`ok` 才會是 `true`。
- 技能身分、發布者身分和所選版本中繼資料是頂層封套欄位（`slug`、`displayName`、`publisherHandle`、`version`、`resolvedFrom`、`tag`、`createdAt`），因此 shell 自動化可以在不解開巢狀包裝器的情況下讀取它們。
...
- `security.signals.dependencyRegistry` 會保留以維持 v1 回應相容性，但依賴項登錄存在性掃描器已退役，且此鍵一律為 `null`。
...
- `channel`（選用）：`official`、`community` 或 `private`
- `isOfficial`（選用）：`true` 或 `false`
- `category`（選用）：外掛分類篩選器。僅在請求範圍限定於外掛套件時支援。受控分類和舊版 v1
  篩選別名記載於 `GET /api/v1/plugins` 下。
...
- 目前由 GitHub 支援且掃描結果為 `clean` 或 `suspicious` 的 Skills，會包含
  ...
  content hash 和 archive URL。它們不包含 ClawHub 代管的來源檔案。
...
- `GET /api/v1/plugins` 下記錄的舊版 v1 篩選別名也會被接受。
- 類別篩選是真正的 API 篩選器，由外掛類別摘要列支援，
  而不是搜尋查詢改寫。
- 結果會依相關性順序傳回，且目前不支援分頁。
...
- ClawPack 發行版本包含 npm 相容的 `npmIntegrity`、`npmShasum` 與
  `npmTarballName` 欄位。
...
傳回供安裝用戶端使用的精確套件發行安全性與信任摘要。這是公開的 OpenClaw 消費介面，用於判斷已解析的發行版本是否可以安裝。
...
- `package.name`、`package.displayName` 與 `package.family` 識別已解析的
  登錄套件。
- `release.releaseId`、`release.version` 與 `release.createdAt` 識別已評估的
  精確發行版本。
- `release.artifactKind`、`release.artifactSha256`、`release.npmIntegrity`、
  `release.npmShasum` 與 `release.npmTarballName` 在發行成品已知時存在。
- `trust.scanStatus` 是從掃描器輸入與手動發行審核衍生出的有效信任狀態。
- `trust.moderationState` 可為 null。當不存在手動發行審核時，該值為 `null`。
- `trust.blockedFromDownload` 是安裝封鎖訊號。當此值為 `true` 時，OpenClaw
  與其他安裝用戶端應封鎖安裝，而不是從掃描器或審核欄位重新推導封鎖規則。
...
- 這只追蹤遷移就緒度。不會修改 OpenClaw 或產生 ClawPacks。
...
它們會進入審核佇列，但本身不會自動隱藏或封鎖下載；版主應使用發行審核來
核准、隔離或撤銷成品。
...
`note` 對 `confirmed` 和 `dismissed` 為必填；將 `status` 設回 `open` 時可以省略。對已確認的檢舉傳入 `finalAction: "quarantine"` 或 `finalAction: "revoke"`，即可在同一個可稽核工作流程中套用版本審核。
...
隔離和撤銷的版本會從成品下載路由傳回 `403`。每次變更都會寫入稽核記錄項目。
...
- 舊版僅 ZIP 的版本會刻意省略。
- `dist.tarball`、`dist.integrity` 和 `dist.shasum` 使用 npm 相容欄位，讓使用者可以在需要時將 npm 指向該鏡像。
...
下載託管的技能版本 ZIP；或針對目前以 GitHub 為後端、掃描狀態為 `clean` 或 `suspicious` 且沒有託管版本的技能，傳回 GitHub 原始碼交接資訊。
...
- 軟刪除版本會傳回 `410`。
- GitHub 支援的技能交接不會代理或鏡像位元組。JSON 回應包含 `sourceRef: "public-github"`、`repo`、`commit`、`path`、`contentHash` 和 `archiveUrl`；掃描/目前狀態是閘道，不會包含為成功酬載中繼資料。
- 下載統計會依 UTC 日計算唯一身分（API 權杖有效時為 `userId`，否則為 IP）。
...
- 選用酬載欄位：`migrateOwner`。當與 `ownerHandle` 一起為 `true` 時，如果執行者在目前與目標發布者上都是管理員/擁有者，既有技能可以移至該擁有者。未明確選用時，擁有者變更會被拒絕。
...
- 允許的表單欄位是 `payload`、重複的 `files` blob，或一個 `clawpack` tarball 參照。`clawpack` 可以是 `.tgz` blob，或由 upload-url 流程傳回的儲存 id。使用暫存 storage-id 發布時，也必須包含該上傳 URL 傳回的 `clawpackUploadTicket`。
- 使用 `files` 或 `clawpack` 其中之一，絕不可在同一個請求中同時使用兩者。
- JSON 內文和呼叫者提供的 `payload.files` / `payload.artifact` 中繼資料會被拒絕。
...
擁有者發起的軟刪除會保留 slug 30 天，之後該 slug 可由另一位發布者認領。刪除回應在此到期適用時包含 `slugReservedUntil`。
主持人/管理員隱藏和安全性移除不會以此方式到期。
...
已驗證的自助組織發布者建立。建立新的組織發布者，並將呼叫者新增為擁有者。此端點不會遷移既有使用者/個人 handle，也不會將發布者標記為受信任/官方。
...
僅限管理員。為合法擁有者保留根 slug 和套件名稱，而不發布版本。套件名稱會成為沒有版本資料列的私有預留位置套件，因此同一位擁有者稍後可以將真正的 code-plugin 或 bundle-plugin 版本發布到該名稱。
...
僅限管理員。為已驗證的替換 GitHub OAuth 主體恢復個人發布者，而不編輯 Convex Auth 帳戶資料列。請求必須指定兩個不可變的 GitHub provider 帳戶 id；可變 handle 僅用作面向操作者的防護。

端點預設為 dry-run。套用復原需要在工作人員獨立驗證兩個 GitHub 主體之間的連續性後，設定 `dryRun: false` 和
`confirmIdentityVerified: true`。當目的地使用者目前的個人發布者擁有 Skills、套件或 GitHub skill 來源時，復原會封閉失敗。
復原也會遷移已復原發布者的 Skills、skill slug 別名、套件、套件檢查器警告，以及衍生搜尋摘要列中的舊版 `ownerUserId` 欄位，使
直接擁有者路徑與新的發布者權限一致。已復原 handle 的有效受保護 handle
保留也會重新指派給替代使用者，因此後續的個人資料同步無法還原前使用者的競爭權限。每個主要資料表在每次套用交易中限制為
100 列；較大的復原必須先使用可續傳的擁有者遷移。
GitHub skill 來源是以發布者為範圍，並回報為已檢查，而非被重寫。
