...
x-i18n:
    generated_at: "2026-07-05T04:46:43Z"
    model: gpt-5.5
...
第三方目錄可以使用公開讀取端點來列出或搜尋 ClawHub skills。請快取結果、遵守 `429`/`Retry-After`、將使用者連回標準 ClawHub 列表（`https://clawhub.ai/<owner>/skills/<slug>`），並避免暗示 ClawHub 為第三方網站背書。請勿嘗試在公開 API 表面之外鏡像隱藏、私有或遭審核封鎖的內容。
...
- 已驗證的寫入端點在伺服器知道原因時，不應只回傳單純的 `Unauthorized`。遺失 token、無效/已撤銷 token，以及已刪除/停權/停用帳號都應取得可行動的文字，讓命令列介面用戶端能告訴使用者是什麼阻擋了他們。
...
- 於 `429`：`X-RateLimit-Remaining: 0` 和 `RateLimit-Remaining: 0`
- 於 `429`：`Retry-After`
...
- 使用加入抖動的退避來避免同步重試。
- 如果 `Retry-After` 遺失，請退回 `RateLimit-Reset`（或從 `X-RateLimit-Reset` 計算）。

IP 來源：

- 只有在部署明確啟用可信任轉送標頭時，才會使用可信任的用戶端 IP 標頭，包括 `cf-connecting-ip`。
- ClawHub 使用可信任的轉送標頭在邊緣識別用戶端 IP。
- 如果沒有可用的可信任用戶端 IP，匿名請求會使用只依速率限制種類作用域化的 fallback bucket。這些 fallback bucket 不包含呼叫者提供的路徑、slug、套件名稱、版本、查詢字串或其他成品參數。

## 錯誤回應

公開 v1 錯誤回應是純文字，並帶有 `content-type: text/plain; charset=utf-8`。
這包含驗證失敗（`400`）、缺少公開資源（`404`）、驗證與權限失敗（`401`/`403`）、速率限制（`429`）以及被封鎖的下載。用戶端應將回應本文讀取為人類可讀的字串。未知查詢參數會因相容性而被忽略，但已識別且值無效的查詢參數會回傳 `400`。
...
- 結果會依相關性順序傳回（嵌入相似度 + 精確 slug/名稱 token 加權 + 少量熱門度先驗）。
- 相關性比熱門度更強。精確的 slug 或顯示名稱 token 符合，可以排在互動度高很多但符合較鬆散的結果之前。
- ASCII 文字會依單字和標點邊界切分 token。例如，`personal-map` 包含獨立的 `map` token，而 `amap-jsapi-skill` 包含 `amap`、`jsapi` 和 `skill`；因此搜尋 `map` 時，`personal-map` 會比 `amap-jsapi-skill` 有更強的詞彙符合。
- 熱門度會以對數縮放並設上限。當查詢文字符合較弱時，高互動度 skills 可能排名較低。
- 可疑或隱藏的審核狀態可能會依呼叫者篩選條件與目前審核狀態，將某個 skill 從公開搜尋中移除。

發布者可發現性指引：

- 將使用者會實際搜尋的詞放在顯示名稱、摘要和標籤中。只有在獨立 slug token 也是你想保留的穩定識別時才使用。
- 不要只為了追逐單一查詢而重新命名 slug，除非新 slug 是更好的長期標準名稱。舊 slug 會成為重新導向別名，但標準 URL、顯示的 slug，以及未來搜尋摘要會使用新 slug。
- 重新命名別名會保留舊 URL 和透過登錄解析的安裝解析能力，但搜尋排名會基於重新命名完成索引後的標準 skill 中繼資料。既有統計會留在該 skill 上。
...
- `trending` 依最近 7 天內的安裝數排名（基於遙測）。
- `createdAt` 對新 skill 爬取保持穩定；`updated` 會在既有 skills 重新發布時變更。
- 當 `nonSuspiciousOnly=true` 時，基於游標的排序在單頁可能回傳少於 `limit` 個項目，因為可疑 skills 會在取回頁面後被篩除。
- 存在時使用 `nextCursor` 繼續分頁。短頁本身並不代表結果結束。
...
- `moderation` 只會在 skill 被標記，或擁有者正在檢視它時包含。
...
- 對公開呼叫者，證據會被遮蔽，且只有擁有者/審核員才會包含原始片段。

### `POST /api/v1/skills/{slug}/report`

回報 skill 供審核員審查。回報是 skill 層級，可選擇連結到版本，並會送入 skill 回報佇列。
...
`note` 對 `confirmed` 和 `dismissed` 為必填；將 `status` 設回 `open` 時可以省略。對已分類處理的回報傳入 `finalAction: "hide"`，即可在同一個可稽核工作流程中隱藏該 skill。
...
- `version.security` 會在可用時包含標準化掃描驗證狀態和掃描器詳細資料（VirusTotal + LLM）。
...
- 回傳已排隊／執行中／成功／失敗狀態。
- 排隊時回傳 `queue.queuedAhead` 和 `queue.position`，讓用戶端可以顯示該請求前方有多少個優先手動掃描。非常大的佇列會被設上界，並以 `queuedAheadIsEstimate: true` 回報。
...
- 回傳 ZIP，包含 `manifest.json`、`clawscan.json`、`skillspector.json`、`static-analysis.json`、`virustotal.json` 和 `README.md`。
...
- 只有在選取的版本具有已產生的技能卡、未被審核判定為惡意軟體封鎖，且 ClawScan 驗證為乾淨時，`ok` 才會是 `true`。
- 技能身分、發布者身分和選取版本中繼資料是頂層信封欄位（`slug`、`displayName`、`publisherHandle`、`version`、`resolvedFrom`、`tag`、`createdAt`），讓 shell 自動化可以不必解開巢狀包裝器就讀取它們。
...
- `security.signals.dependencyRegistry` 保留用於 v1 回應相容性，但相依性登錄存在性掃描器已淘汰，且此鍵一律為 `null`。
- 只有在 ClawHub 於發布或匯入期間解析並儲存 GitHub 儲存庫／ref／commit／path 時，`provenance` 才會是 `server-resolved-github-import`；否則為 `unavailable`。

### `POST /api/v1/skills/-/security-verdicts`

回傳精確技能版本目前的精簡安全性判定。此集合端點
適用於已知道需要顯示哪些已安裝 ClawHub 技能版本的用戶端，
...
- 回應僅包含安全性資訊。它不包含技能卡資料、已產生卡片狀態、成品檔案清單，或詳細掃描器酬載。
- `security.signals` 僅包含狀態層級的支援證據；若需要完整掃描器詳細資料，請使用 `/scan` 或 ClawHub 安全稽核頁面。
- `security.signals.dependencyRegistry` 保留用於 v1 回應相容性，但相依性登錄存在性掃描器已淘汰，且此鍵一律為 `null`。
- 技能卡不存在不會影響此端點的 `ok`、`decision` 或 `reasons`；用戶端需要卡片內容時，應在本機讀取已安裝的 `skill-card.md`。
- 當你需要單一技能的技能卡驗證信封時使用 `/verify`，需要已產生的卡片 Markdown 時使用 `/card`，需要詳細掃描器資料時使用 `/scan`。
...
- `GET /api/v1/code-plugins` 與 `GET /api/v1/bundle-plugins` 仍是固定系列別名。
- Skills 項目仍由 Skills 登錄支援，且仍只能透過 `POST /api/v1/skills` 發布。
- `POST /api/v1/packages` 仍僅適用於 code-plugin 與 bundle-plugin 發布。
- 匿名呼叫者只能看到公開套件通道。
...
在統一的 `/api/v1/packages` 端點上，它僅適用於外掛；若要使用 Skills 目錄，請使用
...
- `cursor`（選填）：上一個回應的分頁游標。

回應：

- 主體：ZIP 封存檔。
- 每個匯出的 Skills 都以 `{publisher}/{slug}/` 為根目錄。
- 託管的 Skills 包含最新已儲存版本檔案，並在
...
- 目前由 GitHub 支援且掃描結果為 `clean` 或 `suspicious` 的 Skills 包含
  ...
  內容雜湊與封存 URL。它們不包含由 ClawHub 託管的原始碼檔案。
...
- 舊世界套件封存檔的 `version.artifact.kind` 為 `legacy-zip`，
  或 ClawPack 支援的發行版本為 `npm-pack`。
- ClawPack 發行版本包含與 npm 相容的 `npmIntegrity`、`npmShasum` 和
  `npmTarballName` 欄位。
- `version.sha256hash` 是供舊客戶端使用的已棄用相容性中繼資料。它會雜湊
  `/api/v1/packages/{name}/download` 傳回的精確 ZIP 位元組。
  現代客戶端應使用 `version.artifact.sha256`，它識別標準發行成品。
...
傳回安裝客戶端使用的精確套件發行安全性與信任摘要。這是公開的
OpenClaw 消費介面，用於判斷已解析的發行版本是否可以安裝。
...
- `release.releaseId`、`release.version` 和 `release.createdAt` 識別已評估的
  精確發行版本。
- 當發行成品的資訊已知時，會出現 `release.artifactKind`、`release.artifactSha256`、`release.npmIntegrity`、
  `release.npmShasum` 和 `release.npmTarballName`。
- `trust.scanStatus` 是從掃描器輸入與手動發行審核衍生的有效信任狀態。
- `trust.moderationState` 可為 null。當沒有手動發行審核時，它為 `null`。
- `trust.blockedFromDownload` 是安裝封鎖訊號。當此值為 `true` 時，OpenClaw 和其他
  安裝客戶端應封鎖安裝，而不是從掃描器或審核欄位重新推導封鎖規則。
...
- 此端點精確到版本。客戶端應在解析出打算安裝的套件版本後呼叫它，
  而不只是讀取最新套件中繼資料後呼叫。
- 私有套件會傳回 `404`，除非呼叫者可以讀取所屬發布者。
- 此端點有意比擁有者/版主審核端點更窄。它公開安裝決策和公開說明，
  而不是回報者身分、回報內文、私有證據或內部審查時間線。
...
- 這是 OpenClaw 解析器介面；它避免從共享 URL 猜測封存格式。
...
- `packageName` 是 npm 名稱正規化；對於已規劃的遷移，套件可以不存在。
...
回報套件以供版主審查。回報是套件層級，可選擇連結到某個版本。
它們會進入審核佇列，但本身不會自動隱藏或封鎖下載；版主應使用發行版本審核來
核准、隔離或撤銷成品。
...
`confirmed` 和 `dismissed` 需要 `note`；將 `status` 設回 `open` 時可以省略。對已確認的回報傳入 `finalAction: "quarantine"` 或 `finalAction: "revoke"`，即可在同一個可稽核工作流程中套用版本審核。
...
- `revoked`：先前信任的版本在事後被封鎖。

隔離和撤銷的版本會在成品下載路由回傳 `403`。每次變更都會寫入稽核記錄項目。
...
- 外掛/套件封存檔是 zip 檔案，並具有 `package/` 根目錄，讓舊版 OpenClaw 用戶端能繼續運作。
...
- 舊版僅 ZIP 的版本會刻意省略。
- `dist.tarball`、`dist.integrity` 和 `dist.shasum` 使用 npm 相容欄位，因此使用者可以視需要將 npm 指向鏡像。
...
下載託管的技能版本 ZIP，或針對具有 `clean` 或 `suspicious` 掃描且沒有託管版本的目前 GitHub 支援技能，回傳 GitHub 原始碼交接。
...
- 下載統計會按 UTC 日計算唯一身分（API 權杖有效時使用 `userId`，否則使用 IP）。
...
- 建議：使用帶有 `payload` JSON + `files[]` blob 的 `multipart/form-data`。
- 也接受帶有 `files`（以 storageId 為基礎）的 JSON 主體。
- 選填酬載欄位：`ownerHandle`。存在時，API 會在伺服器端解析該發布者，並要求行動者具有發布者存取權。
- 選填酬載欄位：`migrateOwner`。當與 `ownerHandle` 一起為 `true` 時，如果行動者同時是目前和目標發布者的管理員/擁有者，現有技能可以移至該擁有者。若未明確選擇加入，擁有者變更會被拒絕。
...
- 允許的表單欄位為 `payload`、重複的 `files` blob，或一個 `clawpack` tarball 參照。`clawpack` 可以是 `.tgz` blob，或是 upload-url 流程回傳的 storage id。使用暫存 storage-id 發布時，也必須包含該上傳 URL 一併回傳的 `clawpackUploadTicket`。
...
- Code 外掛需要 `package.json`、原始碼 repo 中繼資料、原始碼 commit 中繼資料、config schema 中繼資料、`openclaw.compat.pluginApi` 和 `openclaw.build.openclawVersion`。
...
由擁有者發起的軟刪除會保留 slug 30 天，之後 slug 可由另一個發布者認領。刪除回應會在此到期適用時包含 `slugReservedUntil`。
...
僅限管理員。確保 handle 存在 org 發布者。如果該 handle 仍指向舊版共享使用者/個人發布者，端點會先將它遷移為 org 發布者。對於新建立的 org，請提供 `memberHandle`；執行操作的管理員不會被加入為成員。`memberRole` 預設為 `owner`。
...
經驗證的自助式 org 發布者建立。建立新的 org 發布者，並將呼叫者加入為擁有者。此端點不會遷移現有使用者/個人 handle，也不會將發布者標記為受信任/官方。
...
僅限管理員。為已驗證的替代 GitHub OAuth 主體復原個人發布者，而不編輯 Convex Auth 帳戶資料列。請求必須指定兩個不可變的 GitHub provider account id；可變 handle 只用作面向操作員的防護。
...
復原也會遷移已復原發布者的 Skills、Skill slug 別名、套件、套件檢查器警告，以及衍生搜尋摘要資料列中的舊版 `ownerUserId` 欄位，讓直接擁有者路徑與新的發布者權限一致。已復原 handle 的有效受保護 handle
保留項目也會重新指派給替代使用者，讓之後的
個人資料同步無法還原前使用者的競爭權限。每個主要資料表在每次套用交易中限制為
100 列；較大型的復原必須先使用可續傳的擁有者遷移。
GitHub Skill 來源是以發布者為範圍，並回報為已檢查而非重寫。
...
- 兩個端點都需要 API 權杖驗證，而且只適用於 Skill 擁有者。
