...
第三方目錄可以使用公開讀取端點列出或搜尋 ClawHub Skills。請快取結果、遵守 `429`/`Retry-After`、將使用者連回正式的 ClawHub 清單（`https://clawhub.ai/<owner>/skills/<slug>`），並避免暗示 ClawHub 為第三方網站背書。請勿嘗試在公開 API 範圍之外鏡像隱藏、私密或遭審核封鎖的內容。

網頁 slug 捷徑會跨登錄系列解析，但 API 用戶端應使用
讀取端點傳回的正式 URL，而不是重新建構路由
優先順序。
...
- 匿名請求：依 IP 執行。
- 已驗證請求（有效 Bearer 權杖）：依使用者桶執行。
- 如果權杖遺失或無效，行為會退回 IP 執行。
- 已驗證的寫入端點在伺服器知道原因時，不應只回傳裸露的 `Unauthorized`。遺失權杖、無效或已撤銷權杖，以及已刪除、遭停權或停用的帳戶，都應取得可操作的文字，讓命令列介面
  用戶端可以告訴使用者是什麼阻擋了他們。
...
- `X-RateLimit-Remaining` / `RateLimit-Remaining`：存在時為確切剩餘額度。
...
這包含驗證失敗（`400`）、遺失公開資源（`404`）、驗證與
權限失敗（`401`/`403`）、速率限制（`429`）以及遭封鎖的下載。用戶端
應將回應主體讀取為人類可讀字串。未知的查詢參數會
為了相容性而被忽略，但已識別且值無效的查詢參數會回傳
`400`。
...
- 相關性比熱門度更強。精確的 slug 或顯示名稱權杖符合，可以超越互動度高得多但符合較寬鬆的結果。
- ASCII 文字會依單字與標點邊界切分權杖。例如，`personal-map` 包含獨立的 `map` 權杖，而 `amap-jsapi-skill` 包含 `amap`、`jsapi` 與 `skill`；因此搜尋 `map` 會讓 `personal-map` 比 `amap-jsapi-skill` 取得更強的詞彙符合。
- 熱門度會採用對數縮放並設上限。當查詢文字符合較弱時，高互動度 Skills 可能排名較低。
- 可疑或隱藏的審核狀態可能會依呼叫者篩選條件與目前審核狀態，將 Skills 從公開搜尋中移除。

發布者可發現性指引：

- 將使用者會實際搜尋的詞放在顯示名稱、摘要與標籤中。只有當獨立 slug 權杖也是你想保留的穩定身分時才使用。
- 不要只為了追逐單一查詢而重新命名 slug，除非新 slug 是更好的長期正式名稱。舊 slug 會成為重新導向別名，但正式 URL、顯示的 slug 與未來搜尋摘要會使用新 slug。
- 重新命名別名會保留舊 URL 與透過登錄解析安裝的解析能力，但搜尋排名會基於重新命名已索引後的正式 Skill 中繼資料。既有統計會留在該 Skill 上。
- 如果某個 Skill 意外不可見，請在變更排名相關中繼資料之前，先於登入狀態使用 `clawhub inspect @owner/slug` 檢查審核狀態。
...
- 當 `nonSuspiciousOnly=true` 時，基於游標的排序可能會在一頁中回傳少於 `limit` 的項目，因為可疑 Skills 會在取回頁面後被篩除。
- 存在 `nextCursor` 時請用它繼續分頁。短頁本身不代表結果已結束。
...
- 公開呼叫者看到的證據會經過遮蔽，只有擁有者/審核員才會包含原始片段。

### `POST /api/v1/skills/{slug}/report`

回報 Skill 以供審核員審查。回報是 Skill 層級，可選擇連結
...
`note` 對 `confirmed` 與 `dismissed` 為必要；將
`status` 設回 `open` 時可以省略。對已分流的
...
- `version.security` 會在可用時包含正規化的掃描驗證狀態與掃描器詳細資訊
  （VirusTotal + LLM）。
...
- 回傳已排隊／執行中／成功／失敗狀態。
- 排隊時回傳 `queue.queuedAhead` 和 `queue.position`，讓用戶端可顯示請求前方有多少優先處理的手動掃描。非常大的佇列會受限並以 `queuedAheadIsEstimate: true` 回報。
...
- 技能身分、發布者身分與所選版本中繼資料是頂層封套欄位（`slug`、`displayName`、`publisherHandle`、`version`、`resolvedFrom`、`tag`、`createdAt`），因此 shell 自動化可在不拆開巢狀包裝的情況下讀取它們。
...
集合端點預期供已知道需要顯示哪些已安裝
ClawHub 技能版本的用戶端使用，例如 OpenClaw Control UI。
...
- `items` 必須包含 1 至 100 組唯一的 `{ slug, version }` 配對。
- 結果依項目回傳；一個缺少的技能或版本不會讓整個回應失敗。
- 回應僅限安全性資訊。它不包含技能卡資料、已產生卡片狀態、成品檔案清單或詳細掃描器酬載。
- `security.signals` 僅包含狀態層級的支援證據；如需完整掃描器詳細資料，請使用 `/scan` 或 ClawHub 安全稽核頁面。
- `security.signals.dependencyRegistry` 保留用於 v1 回應相容性，但依賴項登錄存在性掃描器已停用，且此鍵一律為 `null`。
...
- 匿名呼叫者只會看到公開套件通道。
- 已驗證的呼叫者可以在列表/搜尋結果中看到其所屬發布者的私有套件。
...
- 目前由 GitHub 支援且掃描結果為 `clean` 或 `suspicious` 的 skill 包含
  ...
  content hash 和封存 URL。它們不包含 ClawHub 託管的原始檔案。
- 每個 skill 都包含 `_export_skill_meta.json`。
- `_manifest.json` 一律包含在 ZIP 根目錄。
- 當個別 skill 或檔案無法匯出時，會包含 `_errors.json`。
...
- `family`（選填）：`code-plugin` 或 `bundle-plugin`。省略表示包含兩種
  外掛系列。
...
僅限外掛的搜尋，橫跨 code-plugin 與 bundle-plugin 套件。
...
- `version.sha256hash` 是供舊用戶端使用的已棄用相容性中繼資料。它會雜湊
  ...
  現代用戶端應使用 `version.artifact.sha256`，它識別標準發行成品。
...
傳回安裝用戶端使用的確切套件發行安全性與信任摘要。
這是 OpenClaw 用來判斷已解析的發行版本是否可安裝的公開取用介面。
...
- 當發行成品已知時，會包含 `release.artifactKind`、`release.artifactSha256`、`release.npmIntegrity`、
  `release.npmShasum` 和 `release.npmTarballName`。
- `trust.scanStatus` 是從掃描器輸入與手動發行審核推導出的有效信任狀態。
- `trust.moderationState` 可為空值。當沒有手動發行審核時，它是 `null`。
...
- 此端點精確到版本。用戶端應在解析欲安裝的套件版本後呼叫它，
  而不是只在讀取最新套件中繼資料後呼叫。
- 私有套件會傳回 `404`，除非呼叫端可以讀取擁有該套件的發布者。
- 此端點刻意比擁有者/版主審核端點更窄。它公開安裝決策與公開說明，
  而非回報者身分、回報內容、私有證據或內部審查時間線。
...
- `bundledPluginId` 會正規化為小寫，且是穩定的更新插入鍵。
...
回報套件以供版主審查。回報屬於套件層級，可選擇性連結至版本。
它們會送入審核佇列，但本身不會自動隱藏或封鎖下載；版主應使用發行審核來
核准、隔離或撤銷成品。
...
`confirmed` 和 `dismissed` 需要 `note`；將 `status` 設回 `open` 時可省略。對已確認的檢舉傳入 `finalAction: "quarantine"` 或 `finalAction: "revoke"`，即可在同一個可稽核的工作流程中套用發行版本審核。
...
- `revoked`：發行版本先前受信任後已封鎖。

隔離和撤銷的發行版本會從成品下載路由回傳 `403`。每次變更都會寫入稽核記錄項目。
...
- 二進位檔案會回傳 `415`。
- 檔案大小限制：200KB。
- 等待中的 VirusTotal 掃描不會阻擋讀取；惡意發行版本仍可能在其他地方被扣留。
...
- 外掛/套件封存檔是以 `package/` 為根目錄的 zip 檔案，讓舊版 OpenClaw 用戶端能持續運作。
...
- 等待中的 VirusTotal 掃描不會阻擋下載；惡意發行版本會回傳 `403`。
- 私有套件會回傳 `404`，除非呼叫者是擁有者。

### `GET /api/npm/{package}`

回傳與 npm 相容的 packument，適用於以 ClawPack 為後端的套件版本。

注意事項：

- 只列出已上傳 ClawPack npm-pack tarball 的版本。
- 舊版僅 ZIP 的版本會刻意省略。
- `dist.tarball`、`dist.integrity` 和 `dist.shasum` 使用與 npm 相容的欄位，讓使用者可選擇將 npm 指向此鏡像。
...
下載託管的 skill 版本 ZIP；或對目前由 GitHub 後端支援、掃描結果為 `clean` 或 `suspicious` 且沒有託管版本的 skill，回傳 GitHub 原始碼交接資訊。
...
- 軟刪除版本會回傳 `410`。
- GitHub 後端支援的 skill 交接不會代理或鏡像位元組。JSON 回應包含 `sourceRef: "public-github"`、`repo`、`commit`、`path`、`contentHash` 和 `archiveUrl`；掃描/目前狀態是閘門，不會作為成功承載的中繼資料包含在內。
- 下載統計會以每個 UTC 日的唯一身分計算（API 權杖有效時使用 `userId`，否則使用 IP）。
...
- 選填承載欄位：`ownerHandle`。存在時，API 會在伺服器端解析該發布者，並要求執行者具備發布者存取權。
- 選填承載欄位：`migrateOwner`。與 `ownerHandle` 一起為 `true` 時，如果執行者同時是目前發布者和目標發布者的管理員/擁有者，既有 skill 可移至該擁有者。若沒有此選擇加入，擁有者變更會被拒絕。
...
- 允許的表單欄位為 `payload`、重複的 `files` blob，或一個 `clawpack` tarball 參照。`clawpack` 可以是 `.tgz` blob，或上傳 URL 流程回傳的 storage id。分段 storage-id 發布也必須包含該上傳 URL 一併回傳的 `clawpackUploadTicket`。
...
- 只有 `openclaw` org 發布者和目前 `openclaw` org 成員的個人發布者，可發布到 `official` 頻道。
...
擁有者發起的軟刪除會保留 slug 30 天，之後該 slug 可由另一個發布者聲明。
刪除回應會在此到期適用時包含 `slugReservedUntil`。
審核員/管理員隱藏和安全移除不會以這種方式到期。
...
僅限管理員。確保 handle 的 org 發布者存在。若該 handle 仍指向舊版共享使用者/個人發布者，此端點會先將其遷移到 org 發布者。
若是新建立的 org，請提供 `memberHandle`；執行操作的管理員不會被加入為成員。
...
僅限管理員。為合法擁有者保留根 slug 和套件名稱，而不發布發行版本。套件名稱會成為沒有發行版本列的私有預留套件，讓同一擁有者稍後可將真正的 code-plugin 或 bundle-plugin 發行版本發布到該名稱。
...
僅限管理員。為已驗證的替代 GitHub OAuth principal 復原個人發布者，而不編輯 Convex Auth 帳戶列。請求必須指定兩個不可變的 GitHub provider account id；可變 handle 僅用作面向操作員的防護。

此端點預設為試執行。套用復原需要在工作人員獨立驗證兩個
GitHub 主體之間的連續性後，設定 `dryRun: false` 和
`confirmIdentityVerified: true`。當目的地使用者目前的個人發布者擁有技能、套件或 GitHub 技能來源時，
復原會以關閉方式失敗。
復原也會遷移已復原發布者技能、技能 slug 別名、套件、套件檢查器警告，以及衍生搜尋摘要資料列中的舊版 `ownerUserId` 欄位，讓直接擁有者路徑與新的發布者權限一致。已復原代稱的有效受保護代稱保留也會重新指派給替代使用者，讓後續
個人檔案同步無法還原前使用者的競爭權限。每個主要資料表在每次套用交易中限制為
100 筆資料列；較大的復原必須先使用可續傳的擁有者遷移。
