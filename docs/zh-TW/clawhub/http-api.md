...
x-i18n:
    generated_at: "2026-07-04T20:23:34Z"
    model: gpt-5.5
...
第三方目錄可以使用公開讀取端點來列出或搜尋 ClawHub Skills。請快取結果、遵守 `429`/`Retry-After`、將使用者連回標準 ClawHub 清單（`https://clawhub.ai/<owner>/skills/<slug>`），並避免暗示 ClawHub 為第三方網站背書。請勿嘗試在公開 API 介面之外鏡像隱藏、私人或被審核封鎖的內容。

網頁 slug 捷徑可跨登錄家族解析，但 API 用戶端應使用讀取端點傳回的標準 URL，而不是重建路由優先順序。
...
- 已驗證的寫入端點在伺服器知道原因時，不應傳回裸露的 `Unauthorized`。缺少權杖、無效/已撤銷權杖，以及已刪除/被封禁/停用的帳號，都應取得可採取行動的文字，讓命令列介面用戶端可以告知使用者被阻擋的原因。
...
- `X-RateLimit-Remaining` / `RateLimit-Remaining`：存在時表示確切剩餘配額。
  分片的成功請求會省略此標頭，而不是傳回近似的全域值。
- `Retry-After`：發生 `429` 時，重試前需等待的秒數（延遲）
...
- 使用加入抖動的退避，避免同步重試。
- 如果缺少 `Retry-After`，請退回到 `RateLimit-Reset`（或從 `X-RateLimit-Reset` 計算）。

IP 來源：

- 只有在部署明確啟用可信任轉送標頭時，才會使用可信任的用戶端 IP 標頭，包括 `cf-connecting-ip`。
- ClawHub 會使用可信任轉送標頭，在邊緣識別用戶端 IP。
- 如果沒有可用的可信任用戶端 IP，匿名請求會使用僅依速率限制類型限定範圍的備援配額桶。這些備援配額桶不包含呼叫者提供的路徑、slug、套件名稱、版本、查詢字串或其他成品參數。

## 錯誤回應

公開 v1 錯誤回應是純文字，並帶有 `content-type: text/plain; charset=utf-8`。
這包括驗證失敗（`400`）、缺少公開資源（`404`）、驗證和權限失敗（`401`/`403`）、速率限制（`429`），以及被封鎖的下載。用戶端應將回應主體讀取為人類可讀的字串。未知的查詢參數會為了相容性而被忽略，但已識別且值無效的查詢參數會傳回 `400`。
...
- 相關性比熱門度更強。精確的 slug 或顯示名稱權杖符合，可以勝過互動更強但符合較鬆散的結果。
- ASCII 文字會依單字和標點邊界進行權杖化。例如，`personal-map` 包含獨立的 `map` 權杖，而 `amap-jsapi-skill` 包含 `amap`、`jsapi` 和 `skill`；因此搜尋 `map` 會讓 `personal-map` 取得比 `amap-jsapi-skill` 更強的詞彙符合。
- 熱門度會經過對數縮放並設上限。當查詢文字符合較弱時，高互動 Skills 可能排名較低。
- 可疑或隱藏的審核狀態，可能會依呼叫者篩選條件和目前審核狀態，將某個 Skill 從公開搜尋中移除。

發布者可探索性指引：

- 將使用者會實際搜尋的詞放在顯示名稱、摘要和標籤中。只有在獨立 slug 權杖同時也是你想保留的穩定身分時，才使用它。
- 不要只為了追逐單一查詢而重新命名 slug，除非新的 slug 是更好的長期標準名稱。舊 slug 會變成重新導向別名，但標準 URL、顯示的 slug 和未來搜尋摘要會使用新的 slug。
- 重新命名別名會保留舊 URL 和透過登錄解析安裝的解析能力，但搜尋排名會基於重新命名建立索引後的標準 Skill 中繼資料。既有統計資料會留在該 Skill 上。
- 如果某個 Skill 意外不可見，請在變更排名相關中繼資料之前，先在登入狀態下使用 `clawhub inspect @owner/slug` 檢查審核狀態。
...
- 當 `nonSuspiciousOnly=true` 時，因為可疑 Skills 會在頁面擷取後被篩除，基於游標的排序在某一頁可能傳回少於 `limit` 的項目。
...
`note` 對 `confirmed` 和 `dismissed` 是必要的；將 `status` 設回 `open` 時可以省略。對已分流的回報傳入 `finalAction: "hide"`，可在同一個可稽核工作流程中隱藏該 Skill。
...
- 回應是 `202`，內容為 `{ "ok": true, "scanId": "...", "jobId": "...", "status": "queued", "sourceKind": "published", "update": false, "queue": { "queuedAhead": 0, "queuedAheadIsEstimate": false, "position": 1, "running": 0, "runningIsEstimate": false, "note": "Scans are asynchronous and may take time to complete." } }`。
...
- 回傳已排隊／執行中／成功／失敗狀態。
- 排隊時回傳 `queue.queuedAhead` 和 `queue.position`，讓用戶端可以顯示此請求前方有多少個已優先處理的手動掃描。非常大的佇列會受到上限限制，並以 `queuedAheadIsEstimate: true` 回報。
...
- 只有當所選版本有產生的技能卡、未被審核標示為惡意軟體封鎖，且 ClawScan 驗證為乾淨時，`ok` 才會是 `true`。
- 技能身分、發布者身分和所選版本中繼資料是頂層信封欄位（`slug`、`displayName`、`publisherHandle`、`version`、`resolvedFrom`、`tag`、`createdAt`），因此 Shell 自動化不必解開巢狀包裝即可讀取。
...
- 只有當 ClawHub 在發布或匯入期間解析並儲存 GitHub 儲存庫／ref／commit／path 時，`provenance` 才會是 `server-resolved-github-import`；否則為 `unavailable`。

### `POST /api/v1/skills/-/security-verdicts`

回傳確切技能版本目前的精簡安全性判定。此集合端點適用於已知需要顯示哪些已安裝 ClawHub 技能版本的用戶端，例如 OpenClaw Control UI。
...
- 結果按項目回傳；單一缺少的技能或版本不會導致整個回應失敗。
- 回應僅包含安全性資訊。不包含技能卡資料、已產生卡片狀態、成品檔案清單或詳細掃描器承載資料。
- `security.signals` 只包含狀態層級的支援證據；若需完整掃描器詳細資料，請使用 `/scan` 或 ClawHub 安全性稽核頁面。
- `security.signals.dependencyRegistry` 會保留以維持 v1 回應相容性，但相依性登錄存在性掃描器已停用，且此鍵一律為 `null`。
- 缺少技能卡不會影響此端點的 `ok`、`decision` 或 `reasons`；用戶端需要卡片內容時，應在本機讀取已安裝的 `skill-card.md`。
- 需要單一技能的技能卡驗證信封時，請使用 `/verify`；需要產生的卡片 Markdown 時，請使用 `/card`；需要詳細掃描器資料時，請使用 `/scan`。
...
- 目前由 GitHub 支援且掃描結果為 `clean` 或 `suspicious` 的 Skills 會包含
  ...
  內容雜湊和封存 URL。它們不包含 ClawHub 託管的原始檔案。
...
- 分類篩選是真正的 API 篩選器，由外掛分類摘要列支援，而不是搜尋查詢重寫。
- 結果會依相關性順序傳回，且目前不分頁。
...
- `version.artifact.kind` 對舊式套件封存檔為 `legacy-zip`，對 ClawPack 支援的發行版本為 `npm-pack`。
- ClawPack 發行版本包含與 npm 相容的 `npmIntegrity`、`npmShasum` 和
  `npmTarballName` 欄位。
- `version.sha256hash` 是供舊用戶端使用的已淘汰相容性中繼資料。它會雜湊
  `/api/v1/packages/{name}/download` 傳回的精確 ZIP 位元組。現代用戶端應使用
  `version.artifact.sha256`，它會識別標準發行成品。
...
傳回安裝用戶端所需的精確套件發行版本安全性與信任摘要。這是公開的 OpenClaw 消費介面，用於判定已解析的發行版本是否可安裝。
...
- `package.name`、`package.displayName` 和 `package.family` 會識別已解析的註冊表套件。
- `release.releaseId`、`release.version` 和 `release.createdAt` 會識別已評估的精確發行版本。
- `release.artifactKind`、`release.artifactSha256`、`release.npmIntegrity`、
  `release.npmShasum` 和 `release.npmTarballName` 在發行成品已知時會存在。
- `trust.scanStatus` 是由掃描器輸入和手動發行版本仲裁衍生出的有效信任狀態。
- `trust.moderationState` 可為 null。當沒有手動發行版本仲裁時，它為 `null`。
- `trust.blockedFromDownload` 是安裝封鎖訊號。OpenClaw 和其他安裝用戶端應在此值為 `true` 時封鎖安裝，而不是從掃描器或仲裁欄位重新推導封鎖規則。
- `trust.reasons` 是面向使用者與稽核用的說明清單。原因代碼是穩定、精簡的字串，例如 `manual:quarantined`、`scan:malicious` 和 `package:malicious`。
- `trust.pending` 表示一個或多個信任輸入仍在等待完成。
- `trust.stale` 表示信任摘要是由過期輸入計算而來，在做出高信心允許決策之前，應視為需要重新整理。

備註：

- 此端點精確到版本。用戶端應在解析打算安裝的套件版本後呼叫它，而不只是讀取最新套件中繼資料後呼叫。
- 私有套件會傳回 `404`，除非呼叫端可讀取擁有該套件的發布者。
- 此端點刻意比擁有者/仲裁者仲裁端點更窄。它公開安裝決策與公開說明，而不是檢舉者身分、檢舉內容、私有證據或內部審查時間軸。
...
- `packageName` 會以 npm 名稱正規化；對於規劃中的遷移，套件可以不存在。
...
檢舉套件以供仲裁者審查。檢舉屬於套件層級，可選擇連結到版本。它們會進入仲裁佇列，但本身不會自動隱藏或封鎖下載；仲裁者應使用發行版本仲裁來核准、隔離或撤銷成品。
...
- `revoked`：在先前信任某個版本後封鎖。

隔離和撤銷的版本會從成品下載路由回傳 `403`。每次變更都會寫入一筆稽核記錄。
...
下載託管的 skill 版本 ZIP，或針對具有 `clean` 或 `suspicious` 掃描結果且沒有託管版本的目前 GitHub 支援 skill，回傳 GitHub 來源交接資訊。
...
- 軟刪除版本會回傳 `410`。
- GitHub 支援的 skill 交接不會代理或鏡像位元組。JSON 回應包含 `sourceRef: "public-github"`、`repo`、`commit`、`path`、`contentHash` 和 `archiveUrl`；掃描/目前狀態是閘門，不會作為成功酬載中繼資料包含在內。
...
- 選用酬載欄位：`migrateOwner`。當搭配 `ownerHandle` 且為 `true` 時，如果執行者同時是目前和目標發布者的管理員/擁有者，既有 skill 可移至該擁有者。沒有此選擇加入時，擁有者變更會被拒絕。
...
- 使用 `files` 或 `clawpack` 其中一個，絕不可在同一請求中同時使用兩者。
...
- 程式碼外掛需要 `package.json`、來源 repo 中繼資料、來源 commit 中繼資料、設定結構描述中繼資料、`openclaw.compat.pluginApi` 和 `openclaw.build.openclawVersion`。
- `openclaw.hostTargets` 和 `openclaw.environment` 是選用中繼資料。
- 只有 `openclaw` 組織發布者和目前 `openclaw` 組織成員的個人發布者可發布至 `official` 頻道。
- 代為發布仍會依目標擁有者帳號驗證官方頻道資格。
...
僅限管理員。確保 handle 存在組織發布者。如果該 handle 仍指向舊版共用使用者/個人發布者，端點會先將它遷移成組織發布者。
...
已驗證使用者自助建立組織發布者。建立新的組織發布者並將呼叫者加入為擁有者。此端點不會遷移既有使用者/個人 handle，也不會將發布者標記為 trusted/official。
...
僅限管理員。為合法擁有者保留根 slug 和套件名稱，而不發布版本。套件名稱會成為沒有版本列的私有預留位置套件，因此同一擁有者稍後可將真正的 code-plugin 或 bundle-plugin 版本發布到該名稱。
...
僅限管理員。為已驗證的替代 GitHub OAuth 主體復原個人發布者，而不編輯 Convex Auth 帳號列。請求必須指定兩個不可變的 GitHub provider account id；可變的 handle 僅作為面向操作員的保護檢查。

端點預設為試跑。若要套用復原，必須在工作人員獨立驗證兩個
GitHub 主體之間的連續性後設定 `dryRun: false` 與
`confirmIdentityVerified: true`。當目的地使用者目前的個人發布者擁有技能、套件或 GitHub 技能來源時，復原會以關閉失敗處理。
復原也會遷移已復原發布者技能、技能 slug 別名、套件、套件檢查器警告，以及衍生搜尋摘要資料列中的舊版 `ownerUserId` 欄位，讓直接擁有者路徑與新的發布者權限一致。已復原 handle 的現行受保護 handle 保留也會重新指派給替換使用者，讓後續的個人檔案同步無法還原前使用者的競爭權限。每個主要資料表在每次套用交易中限制為
...
`POST /api/cli/upload-url` 會傳回 `uploadUrl` 與 `uploadTicket`。暫存 ClawPack tarball 的套件
發布必須將產生的儲存空間 ID 作為
