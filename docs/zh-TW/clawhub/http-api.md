...
第三方目錄可以使用公開讀取端點來列出或搜尋 ClawHub skills。請快取結果、遵守 `429`/`Retry-After`、將使用者連回標準 ClawHub 清單（`https://clawhub.ai/<owner>/skills/<slug>`），並避免暗示 ClawHub 為該第三方網站背書。不要嘗試在公開 API 表面之外鏡像隱藏、私有或遭審核封鎖的內容。

網頁 slug 捷徑可跨登錄檔家族解析，但 API 用戶端應使用
...
- 匿名請求：依 IP 強制執行。
- 已驗證請求（有效的 Bearer 權杖）：依使用者桶強制執行。
- 如果權杖遺失/無效，行為會退回 IP 強制執行。
- 已驗證寫入端點在伺服器知道原因時，不應只傳回裸露的 `Unauthorized`。
  權杖遺失、無效/已撤銷的權杖，以及已刪除/被封鎖/停用的帳戶都應取得可操作的文字，讓命令列介面
  用戶端能告訴使用者是什麼阻擋了他們。
...
- 於 `429`：`X-RateLimit-Remaining: 0` 與 `RateLimit-Remaining: 0`
- 於 `429`：`Retry-After`
...
- `X-RateLimit-Remaining` / `RateLimit-Remaining`：出現時為精確的剩餘預算。
...
- 只有在部署明確啟用受信任的轉發標頭時，才會使用受信任的用戶端 IP 標頭，包括 `cf-connecting-ip`。
- ClawHub 使用受信任的轉發標頭在邊緣識別用戶端 IP。
- 如果沒有可用的受信任用戶端 IP，匿名請求會使用僅依速率限制種類
  範圍界定的備用桶。這些備用桶不包含
...
這包括驗證失敗（`400`）、遺失公開資源（`404`）、驗證與
權限失敗（`401`/`403`）、速率限制（`429`）以及被封鎖的下載。用戶端
應將回應本文讀作人類可讀字串。未知查詢參數會
為相容性而忽略，但已辨識且值無效的查詢參數會傳回
`400`。
...
- 相關性比人氣更強。精確的 slug 或顯示名稱權杖相符，可以排在互動程度高得多但相符較鬆散的結果前面。
- ASCII 文字會依單字與標點邊界切成權杖。例如，`personal-map` 包含獨立的 `map` 權杖，而 `amap-jsapi-skill` 包含 `amap`、`jsapi` 與 `skill`；因此搜尋 `map` 時，`personal-map` 會比 `amap-jsapi-skill` 有更強的詞彙相符。
- 人氣會以對數縮放並設有上限。高互動 skills 在查詢文字相符較弱時可能排名較低。
- 依呼叫者篩選條件與目前審核狀態而定，可疑或隱藏的審核狀態可能會將 skill 從公開搜尋中移除。

發布者可發現性指引：

- 將使用者會實際搜尋的詞放在顯示名稱、摘要與標籤中。只有當獨立 slug 權杖也是你想保留的穩定身分時才使用。
- 不要只為追逐某個查詢而重新命名 slug，除非新 slug 是更好的長期標準名稱。舊 slug 會成為重新導向別名，但標準 URL、顯示的 slug 與未來搜尋摘要會使用新 slug。
- 重新命名別名會保留舊 URL 與透過登錄檔解析的安裝解析能力，但搜尋排名會以重新命名完成索引後的標準 skill 中繼資料為準。既有統計會保留在該 skill 上。
...
- 當 `nonSuspiciousOnly=true` 時，因為可疑 skills 會在頁面擷取後才被篩除，基於游標的排序可能在一頁中傳回少於 `limit` 的項目。
- 出現 `nextCursor` 時，使用它繼續分頁。頁面較短本身不代表結果結束。
...
- `moderation` 只有在 skill 被標記，或擁有者正在檢視時才包含。
...
- 擁有者與版主可以存取隱藏 skills 的審核詳細資料。
- 公開呼叫者只有對已標記且可見的 skills 才會取得 `200`。
- 對公開呼叫者，證據會被遮蔽，且只有擁有者/版主才包含原始片段。

### `POST /api/v1/skills/{slug}/report`

回報 skill 供版主審查。回報屬於 skill 層級，可選擇性連結
到版本，並進入 skill 回報佇列。
...
`note` 對 `confirmed` 與 `dismissed` 是必填；將
`status` 設回 `open` 時可省略。傳入 `finalAction: "hide"` 搭配已分類處理的
...
- `moderation` 是目前從最新版本衍生出的技能層級審核快照。
...
- 掃描工作是非同步的。手動掃描請求會優先於一般發布/回填工作，但完成時間仍取決於工作者可用性。
...
- 回傳已排隊/執行中/成功/失敗狀態。
- 排隊時回傳 `queue.queuedAhead` 和 `queue.position`，讓用戶端可以顯示在該請求前方有多少個優先手動掃描。非常大的佇列會受到上限限制，並以 `queuedAheadIsEstimate: true` 回報。
...
- 需要對技能或外掛具備擁有者/發布者管理存取權，或平台審核者/管理員權限。
...
- 只有在選取版本具備已產生的 Skill Card、未被審核標記為惡意軟體而封鎖，且 ClawScan 驗證為乾淨時，`ok` 才會是 `true`。
- 技能識別、發布者識別和選取版本中繼資料都是頂層封套欄位（`slug`、`displayName`、`publisherHandle`、`version`、`resolvedFrom`、`tag`、`createdAt`），因此 shell 自動化可以不拆開巢狀包裝器就讀取它們。
...
- `security.signals.dependencyRegistry` 會保留以維持 v1 回應相容性，但依賴項登錄檔存在性掃描器已停用，且此鍵一律為 `null`。
...
- 結果會逐項回傳；單一缺少的技能或版本不會讓整個回應失敗。
- 回應僅包含安全資訊。不包含 Skill Card 資料、已產生卡片狀態、成品檔案清單或詳細掃描器酬載。
- `security.signals` 僅包含狀態層級的支援證據；完整掃描器詳細資料請使用 `/scan` 或 ClawHub 安全稽核頁面。
- `security.signals.dependencyRegistry` 會保留以維持 v1 回應相容性，但依賴項登錄檔存在性掃描器已停用，且此鍵一律為 `null`。
- Skill Card 缺少與否不會影響此端點的 `ok`、`decision` 或 `reasons`；當用戶端需要卡片內容時，應在本機讀取已安裝的 `skill-card.md`。
- 需要單一技能的 Skill Card 驗證封套時使用 `/verify`，需要已產生的卡片 Markdown 時使用 `/card`，需要詳細掃描器資料時使用 `/scan`。
...
- `category`（選填）：外掛類別篩選器。僅在請求範圍限定為外掛套件（`/api/v1/plugins`、
...
- `POST /api/v1/packages` 仍僅適用於 code-plugin 和 bundle-plugin 發布版本。
- 匿名呼叫者只能看到公開套件 channel。
- 已驗證的呼叫者可在清單/搜尋結果中看到其所屬發布者的私有套件。
...
- 主體：ZIP 封存。
- 每個匯出的 Skill 都以 `{publisher}/{slug}/` 為根。
- 託管的 Skills 包含最新儲存版本檔案，並會以
  `sourceRef: "public-clawhub"` 列在 `_manifest.json` 中。
- 目前由 GitHub 支援且掃描結果為 `clean` 或 `suspicious` 的 Skills 會包含
  `_source_handoff.json`，其中含有 `sourceRef: "public-github"`、repo、commit、path、
  content hash 和 archive URL。它們不包含 ClawHub 託管的原始檔。
...
- Skills 也可以在統一型目錄中透過此路由解析。
- 私有套件會傳回 `404`，除非呼叫端可以讀取擁有該套件的發布者。
...
- 需要套件擁有者、組織發布者擁有者/管理員、平台審核員，或平台管理員的 API 權杖。
...
這是公開的 OpenClaw 消費介面，用於判斷已解析的版本是否可安裝。
...
- `trust.stale` 表示信任摘要是從過期輸入計算而來，且
...
- `packageName` 會以 npm 名稱正規化；對規劃中的
  遷移，套件可以不存在。
...
- `open`：可疑、惡意、待處理、已隔離、已撤銷，或已回報的版本。
- `blocked`：已隔離、已撤銷，或惡意版本。
- `manual`：任何具有手動審核覆寫的版本。
- `all`：任何具有手動覆寫、非乾淨掃描狀態，或套件回報的版本。
...
`note` 對 `confirmed` 和 `dismissed` 為必填；將 `status` 設回 `open` 時可省略。對已確認的檢舉傳入 `finalAction: "quarantine"` 或 `finalAction: "revoke"`，即可在同一個可稽核流程中套用發布版本審核處置。
...
- `revoked`：發布版本先前受信任後被封鎖。

遭隔離和撤銷的發布版本會從成品下載路由回傳 `403`。每次變更都會寫入一筆稽核記錄。
...
- 二進位檔案會回傳 `415`。
- 檔案大小限制：200KB。
- 待處理的 VirusTotal 掃描不會阻擋讀取；惡意發布版本仍可能在其他地方被保留不提供。
- 私有套件會回傳 `404`，除非呼叫者可讀取所屬發布者。

### `GET /api/v1/packages/{name}/download`

下載套件發布版本的舊版決定性 ZIP 封存檔。
...
- 外掛/套件封存檔是以 `package/` 為根目錄的 zip 檔，讓舊版 OpenClaw
  用戶端維持可用。
- 此路由維持僅支援 ZIP。不會串流 ClawPack `.tgz` 檔案。
- 回應包含 `ETag`、`Digest`、`X-ClawHub-Artifact-Type` 和
  `X-ClawHub-Artifact-Sha256` 標頭，以供解析器完整性檢查。
- 僅存在於 registry 的中繼資料不會注入下載的封存檔。
...
- 帶 scope 的套件 packument 同時支援 `/api/npm/@scope/name` 和 npm 的
...
- 軟刪除的版本會回傳 `410`。
- GitHub 支援的 skill 交接不會代理或鏡像位元組。JSON 回應包含 `sourceRef: "public-github"`、`repo`、`commit`、`path`、`contentHash`
  和 `archiveUrl`；掃描/目前狀態是閘門，不會作為成功 payload 中繼資料包含。
...
- 建議：`multipart/form-data`，包含 `payload` JSON + `files[]` blob。
- 也接受含 `files`（以 storageId 為基礎）的 JSON body。
- 選填 payload 欄位：`ownerHandle`。存在時，API 會在伺服器端解析該發布者，並要求行動者具有發布者存取權。
- 選填 payload 欄位：`migrateOwner`。與 `ownerHandle` 一起為 `true` 時，若行動者同時是目前與目標發布者的管理員/擁有者，既有 skill 可移至該擁有者。若未選擇此選項，擁有者變更會被拒絕。
...
- 允許的表單欄位為 `payload`、重複的 `files` blob，或一個 `clawpack`
  tarball 參照。`clawpack` 可以是 `.tgz` blob，或由 upload-url 流程回傳的 storage id。使用暫存 storage-id 發布時，也必須包含該上傳 URL 一併回傳的
...
- 選填 payload 欄位：`ownerHandle`。存在時，只有管理員可代表該擁有者發布。
...
- 只有 `openclaw` org 發布者，以及目前 `openclaw` org 成員的個人發布者，可以發布到 `official` 頻道。
...
其他發布者認領。當此到期機制適用時，刪除回應會包含 `slugReservedUntil`。
...
僅限管理員。確保某個 handle 存在 org 發布者。若該 handle 仍指向
舊版共用使用者/個人發布者，端點會先將其遷移為 org 發布者。
若是新建立的 org，請提供 `memberHandle`；執行操作的管理員不會被新增為成員。
...
已驗證使用者自助建立 org 發布者。建立新的 org 發布者並將
呼叫者加入為擁有者。此端點不會遷移既有使用者/個人 handle，也不會
將發布者標記為 trusted/official。
...
發布版本。套件名稱會成為沒有發布版本列的私有 placeholder 套件，因此同一
擁有者之後可將真正的 code-plugin 或 bundle-plugin 發布版本發布到該名稱。
...
僅限管理員。為已驗證的替代 GitHub OAuth principal 復原個人發布者，
且不編輯 Convex Auth 帳號列。請求必須指定兩個不可變的 GitHub
provider account id；可變 handle 僅作為面向操作者的防護。

端點預設為 dry-run。套用復原需要在工作人員獨立驗證兩個 GitHub 主體之間的連續性後，設定 `dryRun: false` 和
`confirmIdentityVerified: true`。當目標使用者目前的個人發布者擁有技能、套件或 GitHub 技能來源時，復原會以封閉失敗方式結束。
復原也會遷移已復原發布者的技能、技能 slug 別名、套件、套件檢查器警告，以及衍生搜尋摘要列中的舊版 `ownerUserId` 欄位，讓直接擁有者路徑與新的發布者權限一致。已復原 handle 的有效受保護 handle 保留項目也會重新指派給替代使用者，讓後續的
個人檔案同步無法還原前使用者的競爭權限。每個主要資料表在每個套用交易中限制為
100 列；較大的復原必須先使用可恢復的擁有者遷移。
GitHub 技能來源是發布者範圍，會回報為已檢查，而不是被重寫。
...
發布作業必須將產生的儲存空間 ID 以 `clawpack` 傳送，並將傳回的票證以 `clawpackUploadTicket` 傳送。
