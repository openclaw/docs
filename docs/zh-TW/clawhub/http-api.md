...
x-i18n:
    generated_at: "2026-06-28T05:03:08Z"
    model: gpt-5.5
...
第三方目錄可以使用公開讀取端點來列出或搜尋 ClawHub Skills。請快取結果、遵守 `429`/`Retry-After`、將使用者連回標準 ClawHub 清單（`https://clawhub.ai/<owner>/skills/<slug>`），並避免暗示 ClawHub 為該第三方網站背書。請勿嘗試在公開 API 表面之外鏡像隱藏、私人或被審核封鎖的內容。

網頁 slug 捷徑會跨登錄系列解析，但 API 用戶端應使用讀取端點傳回的標準 URL，而不是重新建構路由優先順序。
...
- 已驗證的寫入端點在伺服器知道原因時，不應傳回裸露的 `Unauthorized`。遺失 token、無效/撤銷的 token，以及已刪除/停權/停用的帳號，都應取得可採取行動的文字，讓命令列介面用戶端能告知使用者被阻擋的原因。
...
- 如果沒有可用的受信任用戶端 IP，匿名請求會使用僅依速率限制種類限定範圍的 fallback bucket。這些 fallback bucket 不包含呼叫者提供的路徑、slug、套件名稱、版本、查詢字串或其他成品參數。

## 錯誤回應

公開 v1 錯誤回應是純文字，`content-type: text/plain; charset=utf-8`。
這包含驗證失敗（`400`）、遺失公開資源（`404`）、驗證與權限失敗（`401`/`403`）、速率限制（`429`），以及被封鎖的下載。用戶端應將回應本文讀作人類可讀的字串。未知的查詢參數會因相容性而被忽略，但可辨識的查詢參數若值無效，會傳回 `400`。
...
- 結果會依相關性順序傳回（embedding 相似度 + 精確 slug/名稱 token boost + 少量熱門度先驗）。
- 相關性比熱門度更強。精確的 slug 或顯示名稱 token 符合項，可能排名高於互動強得多但符合較鬆散的項目。
- ASCII 文字會依單字與標點邊界 token 化。例如，`personal-map` 包含獨立的 `map` token，而 `amap-jsapi-skill` 包含 `amap`、`jsapi` 與 `skill`；因此搜尋 `map` 會讓 `personal-map` 比 `amap-jsapi-skill` 取得更強的詞彙符合。
- 熱門度會經過對數縮放並設上限。當查詢文字符合較弱時，高互動 Skills 可能排名較低。
- 可疑或隱藏的審核狀態可能會依呼叫者篩選器與目前審核狀態，從公開搜尋中移除 Skill。

發布者可探索性指引：

- 將使用者會實際搜尋的詞放在顯示名稱、摘要與標籤中。只有在獨立 slug token 也是你想保留的穩定身分時，才使用它。
- 不要只是為了追逐單一查詢而重新命名 slug，除非新 slug 是更好的長期標準名稱。舊 slug 會成為重新導向別名，但標準 URL、顯示的 slug，以及未來搜尋摘要會使用新 slug。
- 重新命名別名會保留舊 URL，以及透過登錄解析的安裝解析能力，但搜尋排名會依重新命名完成索引後的標準 Skill 中繼資料為準。既有統計資料會保留在該 Skill 上。
...
- 存在時，使用 `nextCursor` 繼續分頁。短頁面本身不代表結果結束。
...
- 公開呼叫者的證據會被遮蔽，且只有擁有者/審核者會包含原始片段。

### `POST /api/v1/skills/{slug}/report`

回報 Skill 以供審核者審查。回報屬於 Skill 層級，可選擇連結至版本，並會送入 Skill 回報佇列。
...
`confirmed` 與 `dismissed` 需要 `note`；將 `status` 設回 `open` 時可以省略。將 `finalAction: "hide"` 與已分流的回報一起傳入，可在同一個可稽核工作流程中隱藏該 Skill。
...
- 回傳已佇列/執行中/成功/失敗狀態。
- 佇列中時會回傳 `queue.queuedAhead` 和 `queue.position`，讓用戶端可顯示請求前方有多少個優先手動掃描。非常大的佇列會受限並以 `queuedAheadIsEstimate: true` 回報。
...
- 只有當所選版本已產生技能卡、未被審核判定為惡意軟體封鎖，且 ClawScan 驗證為乾淨時，`ok` 才會是 `true`。
- 技能身分、發布者身分與所選版本中繼資料是頂層信封欄位（`slug`、`displayName`、`publisherHandle`、`version`、`resolvedFrom`、`tag`、`createdAt`），讓 shell 自動化可以不拆開巢狀包裝器就讀取它們。
...
- `security.signals.dependencyRegistry` 會保留以維持 v1 回應相容性，但依賴項登錄檔存在性掃描器已停用，且此鍵永遠為 `null`。
...
回傳確切技能版本目前的精簡安全性判定。此集合端點適用於已知道需要顯示哪些已安裝 ClawHub 技能版本的用戶端，例如 OpenClaw Control UI。
...
- 回應僅包含安全性資訊。它不包含技能卡資料、已產生卡片狀態、成品檔案清單或詳細掃描器承載資料。
- `security.signals` 僅包含狀態層級的支援性證據；如需完整掃描器詳細資料，請使用 `/scan` 或 ClawHub 安全稽核頁面。
- `security.signals.dependencyRegistry` 會保留以維持 v1 回應相容性，但依賴項登錄檔存在性掃描器已停用，且此鍵永遠為 `null`。
- 技能卡不存在不會影響此端點的 `ok`、`decision` 或 `reasons`；用戶端需要卡片內容時，應在本機讀取已安裝的 `skill-card.md`。
- 需要單一技能的技能卡驗證信封時，請使用 `/verify`；需要已產生的卡片 Markdown 時，請使用 `/card`；需要詳細掃描器資料時，請使用 `/scan`。
...
在統一的 `/api/v1/packages` 端點上，它僅限外掛；若要使用 Skill 目錄，請用
...
- 主體：ZIP 封存檔。
- 每個匯出的 Skill 皆以 `{publisher}/{slug}/` 為根目錄。
- 託管的 Skills 包含最新儲存版本檔案，並列於
  `_manifest.json` 中，且 `sourceRef: "public-clawhub"`。
- 目前以 GitHub 為後端、且具有 `clean` 或 `suspicious` 掃描結果的 Skills，會包含
...
只針對 code-plugin 和 bundle-plugin 套件的外掛搜尋。
...
- 舊世界套件封存檔的 `version.artifact.kind` 是 `legacy-zip`，
  ClawPack 支援的發行版本則是 `npm-pack`。
...
- `version.sha256hash` 是供舊用戶端使用的已棄用相容性中繼資料。它會
  ...
  現代用戶端應使用 `version.artifact.sha256`，它會識別標準發行成品。
...
傳回安裝用戶端所需的精確套件發行安全性與信任摘要。這是公開的 OpenClaw
取用介面，用於判斷已解析的發行版本是否可以安裝。
...
- `packageName` 會依 npm 名稱正規化；計畫中的遷移可以缺少套件。
...
檢舉套件以供仲裁者審查。檢舉屬於套件層級，可選擇性連結到版本。
它們會進入仲裁佇列，但本身不會自動隱藏或封鎖下載；仲裁者應使用發行仲裁來
核准、隔離或撤銷成品。
...
`confirmed` 和 `dismissed` 需要 `note`；將 `status` 設回 `open` 時可以省略。對已確認的回報傳入 `finalAction: "quarantine"` 或 `finalAction: "revoke"`，即可在同一個可稽核工作流程中套用發布審核。
...
遭隔離和撤銷的發布會從成品下載路由傳回 `403`。每次變更都會寫入一筆稽核日誌項目。
...
- 二進位檔案會傳回 `415`。
- 檔案大小限制：200KB。
- 等待中的 VirusTotal 掃描不會封鎖讀取；惡意發布仍可能在其他地方被保留不提供。
...
- 僅限登錄檔的中繼資料不會注入下載的封存中。
- 等待中的 VirusTotal 掃描不會封鎖下載；惡意發布會傳回 `403`。
- 私有套件會傳回 `404`，除非呼叫者是擁有者。

### `GET /api/npm/{package}`

傳回由 ClawPack 支援的套件版本所用的 npm 相容 packument。

注意事項：

- 只會列出已上傳 ClawPack npm-pack tarball 的版本。
- 舊版僅 ZIP 版本會刻意省略。
- `dist.tarball`、`dist.integrity` 和 `dist.shasum` 使用 npm 相容欄位，因此使用者可視需要將 npm 指向該鏡像。
...
下載託管的技能版本 ZIP，或針對具 `clean` 或 `suspicious` 掃描且沒有託管版本的目前 GitHub 支援技能，傳回 GitHub 原始碼交接。
...
- 選用酬載欄位：`migrateOwner`。當它與 `ownerHandle` 一起為 `true` 時，如果執行者同時是目前和目標發布者的管理員/擁有者，現有技能可以移動到該擁有者。若未選擇啟用此項，擁有者變更會被拒絕。
...
- 允許的表單欄位為 `payload`、重複的 `files` blob，或一個 `clawpack`
  tarball 參照。`clawpack` 可以是 `.tgz` blob，也可以是上傳 URL 流程傳回的 storage id。分段 storage-id 發布也必須包含該上傳 URL 隨附傳回的 `clawpackUploadTicket`。
- 使用 `files` 或 `clawpack` 其一，不可在同一請求中同時使用。
...
- 代理發布仍會針對目標擁有者帳號驗證 official-channel 資格。
...
由擁有者啟動的軟刪除會保留 slug 30 天，之後該 slug 可由另一位發布者宣告。
...
已驗證使用者可自助建立組織發布者。建立新的組織發布者並將呼叫者新增為擁有者。此端點不會遷移現有使用者/個人 handle，也不會將發布者標記為受信任/官方。
...
僅限管理員。為合法擁有者保留根 slug 和套件名稱，而不發布發布。
套件名稱會成為沒有發布資料列的私有預留位置套件，因此同一位擁有者日後可將真正的 code-plugin 或 bundle-plugin 發布到該名稱。
...
僅限管理員。為已驗證的替換 GitHub OAuth principal 復原個人發布者，而不編輯 Convex Auth 帳號資料列。請求必須命名兩個不可變的 GitHub 提供者帳號 ID；可變 handle 僅作為面向操作員的保護檢查。

端點預設為 dry-run。套用復原需要在工作人員獨立驗證兩個
GitHub principal 之間的連續性後，設定 `dryRun: false` 和
`confirmIdentityVerified: true`。當目的地使用者目前的個人
publisher 有技能、套件或 GitHub 技能來源時，復原會以失敗關閉。
復原也會遷移已復原 publisher 的技能、技能 slug 別名、套件、套件檢查器警告，以及衍生搜尋 digest 資料列中的舊版 `ownerUserId` 欄位，讓直接 owner 路徑與新的 publisher 權限一致。已復原 handle 的有效受保護 handle 保留也會重新指派給替代使用者，讓後續的個人資料同步無法還原前使用者的競爭權限。每個主要資料表在每次套用交易中限制為
100 列；較大的復原必須先使用可恢復的 owner migration。
GitHub 技能來源是 publisher-scoped，並回報為已檢查，而不是重寫。

- 內容主體：`{ "handle": "gingiris", "nextUserHandle": "gingiris-1031", "previousGitHubProviderAccountId": "123", "nextGitHubProviderAccountId": "456", "reason": "Verified account continuity for issue #2555", "confirmIdentityVerified": true, "dryRun": false }`
...
- `POST /api/v1/skills/{slug}/transfer`
  - 內容主體：`{ "toUserHandle": "target_handle", "message": "optional" }`
