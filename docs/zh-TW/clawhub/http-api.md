...
- 已驗證的寫入端點在伺服器知道原因時，不應傳回單純的 `Unauthorized`。遺失 token、無效/已撤銷 token，以及已刪除/已停權/已停用帳號，都應取得可操作的文字，讓命令列介面用戶端可以告訴使用者被阻擋的原因。
...
- 只有在部署明確啟用可信任 forwarded headers 時，才會使用可信任的用戶端 IP 標頭，包括 `cf-connecting-ip`。
- ClawHub 使用可信任的轉送標頭，在邊緣識別用戶端 IP。
- 如果沒有可信任的用戶端 IP 可用，匿名請求會使用只依速率限制種類限定範圍的 fallback bucket。這些 fallback bucket 不包含呼叫者提供的路徑、slug、套件名稱、版本、查詢字串或其他 artifact 參數。

## 錯誤回應

公開 v1 錯誤回應是純文字，並帶有 `content-type: text/plain; charset=utf-8`。
這包括驗證失敗（`400`）、遺失公開資源（`404`）、驗證與權限失敗（`401`/`403`）、速率限制（`429`），以及被封鎖的下載。用戶端應將回應本文讀作人類可讀的字串。未知查詢參數會為了相容性而被忽略，但已識別查詢參數若帶有無效值會傳回 `400`。
...
- `q`（必填）：查詢字串
- `limit`（選填）：整數
- `highlightedOnly`（選填）：`true` 表示篩選為 highlighted Skills
...
- 結果會依相關性順序傳回（embedding 相似度 + 精確 slug/name token 加權 + 小幅 popularity prior）。
- 相關性比熱門度更重要。精確的 slug 或顯示名稱 token 符合，可能會超越互動量高得多但符合較鬆散的結果。
- ASCII 文字會依單字和標點邊界 token 化。例如，`personal-map` 包含獨立的 `map` token，而 `amap-jsapi-skill` 包含 `amap`、`jsapi` 和 `skill`；因此搜尋 `map` 時，`personal-map` 會比 `amap-jsapi-skill` 有更強的詞彙符合。
- 熱門度會經過對數縮放並設上限。高互動量的 Skills 在查詢文字符合較弱時，排名可能較低。
- 可疑或隱藏的審核狀態可能會依呼叫者篩選條件和目前審核狀態，將某個 Skill 從公開搜尋中移除。

發布者可發現性指引：

- 將使用者會實際搜尋的詞放在顯示名稱、摘要和標籤中。只有當獨立 slug token 也是你想保留的穩定身分識別時，才使用它。
- 除非新的 slug 是更好的長期標準名稱，否則不要只為了追逐某個查詢而重新命名 slug。舊 slug 會成為重新導向別名，但標準 URL、顯示的 slug，以及未來搜尋摘要會使用新的 slug。
- 重新命名別名會保留舊 URL 和透過 registry 解析的安裝解析能力，但搜尋排名會基於重新命名已索引後的標準 Skill metadata。既有統計資料會留在該 Skill 上。
- 如果某個 Skill 意外不可見，請先在登入狀態下使用 `clawhub inspect @owner/slug` 檢查審核狀態，再變更排名相關 metadata。
...
- `recommended` 使用互動量和近期性訊號。
- `trending` 依最近 7 天的安裝數排名（基於 telemetry）。
- `createdAt` 對新 Skill crawl 是穩定的；`updated` 會在既有 Skills 重新發布時變更。
- 當 `nonSuspiciousOnly=true` 時，基於 cursor 的排序在頁面擷取後會篩除可疑 Skills，因此一頁可能傳回少於 `limit` 的項目。
...
- 擁有者和 moderator 可以存取隱藏 Skills 的審核詳細資料。
- 公開呼叫者只會對已標記且可見的 Skills 取得 `200`。
- 證據會對公開呼叫者遮蔽，且只有擁有者/moderator 會包含原始片段。

### `POST /api/v1/skills/{slug}/report`

回報 Skill 以供 moderator 審查。回報是 Skill 層級，可選擇連結到版本，並會送入 Skill report queue。
...
用於 Skill report intake 的 moderator/admin 端點。
...
`note` 對 `confirmed` 和 `dismissed` 是必填；將 `status` 設回 `open` 時可以省略。對已 triage 的 report 傳入 `finalAction: "hide"`，可在同一個可稽核工作流程中隱藏該 Skill。
...
- 掃描工作是非同步的。手動掃描請求會優先於一般發布/回填工作，但完成時間仍取決於工作程序可用性。
...
- 回傳已佇列/執行中/成功/失敗狀態。
- 佇列中時會回傳 `queue.queuedAhead` 和 `queue.position`，讓用戶端可以顯示有多少個優先手動掃描排在該請求前面。非常大的佇列會受限並以 `queuedAheadIsEstimate: true` 回報。
...
- 只有在發布或匯入期間，ClawHub 解析並儲存 GitHub repo/ref/commit/path 時，`provenance` 才會是 `server-resolved-github-import`；否則為 `unavailable`。
...
- `category`（選填）：外掛分類篩選器。僅在請求限定為外掛套件（`/api/v1/plugins`、
...
- 匿名呼叫者只能看見公開套件 channel。
- 已驗證呼叫者可在清單/搜尋結果中看見其所屬發布者的私有套件。
...
- `channel`（選填）：`official`、`community` 或 `private`
- `isOfficial`（選填）：`true` 或 `false`
- `category`（選填）：外掛分類篩選器。僅在請求限定為外掛套件時支援。受控分類與舊版 v1
...
- 目前 GitHub 支援且掃描結果為 `clean` 或 `suspicious` 的 Skills 包含
  `_source_handoff.json`，帶有 `sourceRef: "public-github"`、repo、commit、path、
  content hash 和封存 URL。它們不包含 ClawHub 託管的來源檔案。
...
- `version.sha256hash` 是供舊用戶端使用的已棄用相容性中繼資料。它會
  ...
  現代用戶端應使用 `version.artifact.sha256`，它用來識別
  標準版本成品。
...
傳回安裝用戶端所需的確切套件版本安全性與信任摘要。
這是 OpenClaw 用於判定已解析版本是否可安裝的公開取用介面。
...
- `release.artifactKind`、`release.artifactSha256`、`release.npmIntegrity`、
  `release.npmShasum` 與 `release.npmTarballName` 會在版本成品已知時出現。
- `trust.scanStatus` 是由掃描器輸入與手動版本審核推導出的有效信任狀態。
- `trust.moderationState` 可為 null。當不存在手動版本審核時，它是 `null`。
- `trust.blockedFromDownload` 是安裝封鎖訊號。當此值為 `true` 時，
  OpenClaw 與其他安裝用戶端應封鎖安裝，而不是從掃描器或審核欄位
  重新推導封鎖規則。
- `trust.reasons` 是面向使用者與稽核用的說明清單。原因代碼是
  穩定、精簡的字串，例如 `manual:quarantined`、`scan:malicious`
  與 `package:malicious`。
...
- `packageName` 會正規化為 npm 名稱；規劃中的遷移可以缺少該套件。
...
檢舉套件以供版主審查。檢舉屬於套件層級，可選擇性地
連結至某個版本。它們會進入審核佇列，但不會自行自動隱藏或
封鎖下載；版主應使用版本審核來批准、隔離或撤銷成品。
...
`status` 設回 `open` 時可省略。針對已確認的檢舉傳入 `finalAction: "quarantine"` 或
`finalAction: "revoke"`，可在同一個可稽核工作流程中套用發布版本審核。
...
- `revoked`：先前信任某發布版本後將其封鎖。

已隔離和已撤銷的發布版本會從成品下載路由回傳 `403`。
每次變更都會寫入一筆稽核記錄。
...
下載託管的 skill 版本 ZIP，或針對
具有 `clean` 或 `suspicious` 掃描且沒有託管版本的目前 GitHub 支援 skill，回傳 GitHub 原始碼交接資訊。
...
- 選用承載欄位：`ownerHandle`。存在時，API 會在伺服器端解析該
  發布者，並要求行為者具備發布者存取權。
- 選用承載欄位：`migrateOwner`。當與 `ownerHandle` 一起為 `true` 時，
  若行為者同時是目前與目標發布者的管理員/擁有者，現有 skill 可移至該擁有者。
  若未明確選擇此選項，擁有者變更會被拒絕。
...
- 允許的表單欄位為 `payload`、重複的 `files` blob，或一個 `clawpack`
  ...
  upload-url 流程回傳的 storage id。使用暫存 storage-id 發布時，也必須包含
...
- 程式碼外掛需要 `package.json`、原始碼儲存庫中繼資料、來源 commit
...
其他發布者聲明。刪除回應在此到期適用時包含 `slugReservedUntil`。
...
已認證的自助式組織發布者建立。建立新的組織發布者並將
呼叫者加入為擁有者。此端點不會遷移現有使用者/個人 handle，也
不會將發布者標記為受信任/官方。
...
發布版本。套件名稱會變成沒有發布版本列的私有 placeholder 套件，因此同一
擁有者稍後可將真正的 code-plugin 或 bundle-plugin 發布版本發布到該名稱。
