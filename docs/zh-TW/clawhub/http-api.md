...
第三方目錄可以使用公開讀取端點來列出或搜尋 ClawHub 技能。請快取結果、遵守 `429`/`Retry-After`、將使用者連回標準 ClawHub 列表（`https://clawhub.ai/<owner>/skills/<slug>`），並避免暗示 ClawHub 為第三方網站背書。請勿嘗試在公開 API 表面之外鏡像隱藏、私人或因審核而封鎖的內容。

Web slug 快捷方式會跨登錄家族解析，但 API 用戶端應使用讀取端點傳回的標準 URL，而不是重新建構路由優先順序。
...
- 已驗證的寫入端點在伺服器知道原因時，不應只傳回單純的 `Unauthorized`。遺失 token、無效/已撤銷的 token，以及已刪除/封鎖/停用的帳號，都應取得可採取行動的文字，讓命令列介面用戶端可以告知使用者被阻擋的原因。
...
- 如果沒有可用的受信任用戶端 IP，匿名請求會使用只依速率限制種類界定範圍的後援 bucket。這些後援 bucket 不包含呼叫者提供的路徑、slug、套件名稱、版本、查詢字串或其他成品參數。

## 錯誤回應

公開 v1 錯誤回應是純文字，並帶有 `content-type: text/plain; charset=utf-8`。
這包括驗證失敗（`400`）、遺失的公開資源（`404`）、驗證與權限失敗（`401`/`403`）、速率限制（`429`），以及被封鎖的下載。用戶端應將回應主體讀為人類可讀的字串。未知查詢參數會為相容性而忽略，但已辨識的查詢參數若值無效會傳回 `400`。
...
- `q`（必填）：查詢字串
- `limit`（選填）：整數
- `highlightedOnly`（選填）：`true` 表示篩選為精選技能
- `nonSuspiciousOnly`（選填）：`true` 表示隱藏可疑（`flagged.suspicious`）技能
...
- 相關性比熱門度更強。精確的 slug 或顯示名稱 token 相符，可以排在參與度高得多但相符較寬鬆的結果之前。
- ASCII 文字會依字詞和標點邊界 token 化。例如，`personal-map` 包含獨立的 `map` token，而 `amap-jsapi-skill` 包含 `amap`、`jsapi` 和 `skill`；因此搜尋 `map` 時，`personal-map` 會比 `amap-jsapi-skill` 取得更強的詞彙相符。
- 熱門度會以對數縮放並設上限。高參與度技能在查詢文字相符較弱時，排名可能較低。
- 可疑或隱藏的審核狀態可能會根據呼叫者篩選器和目前審核狀態，將技能從公開搜尋中移除。

發布者可探索性指引：

- 將使用者實際會搜尋的詞放在顯示名稱、摘要和標籤中。只有當獨立 slug token 也是你想保留的穩定身分時才使用它。
- 不要只為了追逐某個查詢而重新命名 slug，除非新的 slug 是更好的長期標準名稱。舊 slug 會成為重新導向別名，但標準 URL、顯示的 slug 和未來搜尋摘要會使用新的 slug。
- 重新命名別名會保留舊 URL 和透過登錄解析的安裝解析能力，但搜尋排名會以重新命名已索引後的標準技能中繼資料為準。現有統計資料會留在該技能上。
- 如果技能意外不可見，請先在登入狀態下使用 `clawhub inspect @owner/slug` 檢查審核狀態，再變更排名相關中繼資料。
...
- `recommended` 使用參與度與新近度訊號。
- `trending` 依最近 7 天的安裝數排名（以遙測為基礎）。
- `createdAt` 對新技能爬取是穩定的；`updated` 會在現有技能重新發布時變更。
- 當 `nonSuspiciousOnly=true` 時，游標式排序可能會在某一頁傳回少於 `limit` 的項目，因為可疑技能是在頁面擷取後被篩除。
- 存在 `nextCursor` 時，請使用它繼續分頁。短頁本身不代表結果已結束。
...
- 由擁有者重新命名/合併流程建立的舊 slug 會解析到標準技能。
- `metadata.os`：技能 frontmatter 中宣告的 OS 限制（例如 `["macos"]`、`["linux"]`）。若未宣告則為 `null`。
- `metadata.systems`：Nix 系統目標（例如 `["aarch64-darwin", "x86_64-linux"]`）。若未宣告則為 `null`。
- 如果技能沒有平台中繼資料，`metadata` 為 `null`。
- `moderation` 只會在技能被標記或擁有者正在檢視時包含。
...
- 擁有者和審核員可以存取隱藏技能的審核詳細資料。
- 公開呼叫者只會對已標記且可見的技能取得 `200`。
- 公開呼叫者的證據會經過遮蔽，且只有擁有者/審核員會包含原始片段。

### `POST /api/v1/skills/{slug}/report`

回報技能以供審核員審查。回報屬於技能層級，可選擇連結到版本，並會進入技能回報佇列。
...
`note` 對 `confirmed` 和 `dismissed` 是必填；將 `status` 設回 `open` 時可以省略。傳入 `finalAction: "hide"` 搭配已分流的回報，可在同一個可稽核工作流程中隱藏技能。
...
- `version.security` 在可用時包含正規化掃描驗證狀態與掃描器詳細資料（VirusTotal + LLM）。
...
- 掃描請求酬載和可下載報告會在保留期限後從掃描請求儲存區到期移除。
...
- 掃描作業是非同步的。手動掃描請求會優先於一般發布/回填工作，但完成時間仍取決於 worker 可用性。
...
- 傳回佇列中/執行中/成功/失敗狀態。
- 佇列中時會傳回 `queue.queuedAhead` 和 `queue.position`，讓用戶端顯示請求前方有多少個優先手動掃描。非常大的佇列會受到限制，並以 `queuedAheadIsEstimate: true` 回報。
...
- 需要成功完成的掃描；非終止狀態的掃描會傳回 `409`。
- 傳回 ZIP，其中包含 `manifest.json`、`clawscan.json`、`skillspector.json`、`static-analysis.json`、`virustotal.json` 和 `README.md`。

### `GET /api/v1/skills/-/scan/download/{name}?version=<version>&kind=skill|plugin`

用於已提交版本的已驗證已儲存報告封存端點。

- 需要對該技能或外掛具備擁有者/發布者管理存取權，或具備平台版主/管理員權限。
- 傳回精確提交版本的已儲存掃描結果，包括已封鎖或隱藏的版本。
- `kind` 預設為 `skill`；外掛/套件掃描請使用 `kind=plugin`。
- 傳回與掃描請求下載相同的 ZIP 形狀。

### `POST /api/v1/skills/-/scan/batch`

僅限管理員的標準批次重新掃描路由。它接受與舊版 `POST /api/v1/skills/-/rescan-batch` 相同的酬載形狀。
...
- 只有在所選版本具有已產生的 Skill Card、未被審核標記為惡意軟體封鎖，且 ClawScan 驗證為乾淨時，`ok` 才會是 `true`。
- 技能身分、發布者身分和所選版本中繼資料是頂層封套欄位（`slug`、`displayName`、`publisherHandle`、`version`、`resolvedFrom`、`tag`、`createdAt`），因此 shell 自動化可以不需解開巢狀包裝器即可讀取它們。
...
- `security.signals.dependencyRegistry` 會保留以維持 v1 回應相容性，但相依性登錄存在性掃描器已退役，此鍵一律為 `null`。
...
- 結果依項目區分；單一缺少的技能或版本不會讓整個回應失敗。
- 回應僅包含安全性資訊。不包含 Skill Card 資料、已產生卡片狀態、成品檔案清單或詳細掃描器酬載。
...
- 需要單一技能的 Skill Card 驗證封套時請使用 `/verify`，需要已產生卡片 Markdown 時請使用 `/card`，需要詳細掃描器資料時請使用 `/scan`。
...
- `category`（選用）：外掛類別篩選器。僅在請求範圍限定為外掛套件（`/api/v1/plugins`、
...
- `POST /api/v1/packages` 仍僅用於 code-plugin 和 bundle-plugin 發行版。
- 匿名呼叫者只能看到公開套件頻道。
- 已驗證呼叫者可以在清單/搜尋結果中看到其所屬發布者的私人套件。
- `channel=private` 僅傳回已驗證呼叫者可讀取的套件。
...
- `channel`（選用）：`official`、`community` 或 `private`
- `isOfficial`（選用）：`true` 或 `false`
- `category`（選用）：外掛類別篩選器。僅在請求範圍限定為外掛套件時支援。受控類別與舊版 v1
...
- `cursor`（選用）：來自前一個回應的分頁游標。

回應：

- 主體：ZIP 封存。
- 每個匯出的技能根目錄為 `{publisher}/{slug}/`。
- 託管技能包含最新儲存版本的檔案，並列在
  `_manifest.json` 中，且帶有 `sourceRef: "public-clawhub"`。
- 目前由 GitHub 支援且掃描結果為 `clean` 或 `suspicious` 的技能，包含
  `_source_handoff.json`，其中帶有 `sourceRef: "public-github"`、repo、commit、path、
...
- `family`（選填）：`code-plugin` 或 `bundle-plugin`。省略表示兩種外掛系列都包含。
...
- 每個匯出的外掛都包含最新版本儲存的檔案。
- 個別外掛的匯出中繼資料儲存在 `__clawhub_export/{family}/{packageName}/plugin_meta.json`。
- `_manifest.json` 一律包含在 ZIP 根目錄。
...
- `GET /api/v1/plugins` 下記錄的舊版 v1 篩選器別名也可接受。
- 類別篩選是真正的 API 篩選器，由外掛類別摘要資料列支援，而不是搜尋查詢改寫。
- 結果會依相關性順序傳回，目前不分頁。
- 外掛搜尋的瀏覽器 UI 排序控制項會重新排序已載入的相關性結果，與目前 `/skills` 瀏覽行為一致。
...
- ClawPack 版本包含相容 npm 的 `npmIntegrity`、`npmShasum` 與 `npmTarballName` 欄位。
- `version.sha256hash` 是供舊用戶端使用的已棄用相容性中繼資料。它會雜湊 `/api/v1/packages/{name}/download` 傳回的確切 ZIP 位元組。現代用戶端應使用 `version.artifact.sha256`，它會識別標準版本成品。
- 存在掃描資料時，會包含 `version.vtAnalysis`、`version.llmAnalysis` 與 `version.staticScan`。
- 私有套件會傳回 `404`，除非呼叫者可以讀取擁有的發布者。

### `GET /api/v1/packages/{name}/versions/{version}/security`

傳回安裝用戶端所需的確切套件版本安全與信任摘要。這是公開的 OpenClaw 消費介面，用於判斷已解析版本是否可安裝。
...
- `release.releaseId`、`release.version` 與 `release.createdAt` 識別被評估的確切版本。
- 當版本成品已知時，會有 `release.artifactKind`、`release.artifactSha256`、`release.npmIntegrity`、`release.npmShasum` 與 `release.npmTarballName`。
- `trust.scanStatus` 是由掃描器輸入與手動版本仲裁衍生出的有效信任狀態。
- `trust.moderationState` 可為 null。沒有手動版本仲裁時為 `null`。
- `trust.blockedFromDownload` 是安裝封鎖訊號。OpenClaw 與其他安裝用戶端應在此值為 `true` 時封鎖安裝，而不是從掃描器或仲裁欄位重新推導封鎖規則。
- `trust.reasons` 是面向使用者與稽核的說明清單。原因代碼是穩定、精簡的字串，例如 `manual:quarantined`、`scan:malicious` 與 `package:malicious`。
- `trust.pending` 表示一個或多個信任輸入仍等待完成。
- `trust.stale` 表示信任摘要是根據過時輸入計算，應視為在做出高信心允許決策前需要重新整理。

注意事項：

- 此端點精確到版本。用戶端應在解析其打算安裝的套件版本後呼叫它，而不只是讀取最新套件中繼資料後呼叫。
- 私有套件會傳回 `404`，除非呼叫者可以讀取擁有的發布者。
- 此端點刻意比擁有者/仲裁者仲裁端點更窄。它公開安裝決策與公開說明，而不是回報者身分、回報本文、私人證據或內部審查時間軸。
...
- `packageName` 會以 npm 名稱正規化；對規劃中的遷移，套件可以不存在。
- 這只追蹤遷移就緒度。它不會變更 OpenClaw 或產生 ClawPack。
...
回報套件以供仲裁者審查。回報屬於套件層級，可選擇連結到版本。它們會進入仲裁佇列，但本身不會自動隱藏或封鎖下載；仲裁者應使用版本仲裁來核准、隔離或撤銷成品。
...
`confirmed` 和 `dismissed` 需要 `note`；將 `status` 設回 `open` 時可以省略。對已確認的報告傳入 `finalAction: "quarantine"` 或 `finalAction: "revoke"`，即可在同一個可稽核工作流程中套用發布版本審核。
...
- `revoked`：發布版本先前受信任後遭封鎖。

已隔離與已撤銷的發布版本會從成品下載路由回傳 `403`。每次變更都會寫入一筆稽核記錄。
...
- 二進位檔案會回傳 `415`。
- 檔案大小限制：200KB。
- 等待中的 VirusTotal 掃描不會阻擋讀取；惡意發布版本仍可能在其他位置被扣留。
- 私有套件會回傳 `404`，除非呼叫者可以讀取所屬發布者。

### `GET /api/v1/packages/{name}/download`

下載套件發布版本的舊版確定性 ZIP 封存檔。
...
- 外掛／套件封存檔是 ZIP 檔，並包含 `package/` 根目錄，讓舊版 OpenClaw 用戶端能繼續運作。
...
- 等待中的 VirusTotal 掃描不會阻擋下載；惡意發布版本會回傳 `403`。
- 私有套件會回傳 `404`，除非呼叫者是擁有者。

### `GET /api/npm/{package}`

回傳 ClawPack 支援的套件版本的 npm 相容 packument。

注意事項：

- 只會列出已上傳 ClawPack npm-pack tarball 的版本。
- 舊版僅 ZIP 的版本會刻意省略。
- `dist.tarball`、`dist.integrity` 和 `dist.shasum` 使用 npm 相容欄位，讓使用者可在需要時將 npm 指向該鏡像。
...
下載託管的 skill 版本 ZIP，或針對目前由 GitHub 支援、掃描結果為 `clean` 或 `suspicious` 且沒有託管版本的 skill，回傳 GitHub 原始碼交接資訊。
...
- 軟刪除的版本會回傳 `410`。
- GitHub 支援的 skill 交接不會代理或鏡像位元組。JSON 回應包含 `sourceRef: "public-github"`、`repo`、`commit`、`path`、`contentHash` 和 `archiveUrl`；掃描／目前狀態是閘門，不會作為成功酬載中繼資料納入。
...
- 偏好：使用 `multipart/form-data`，包含 `payload` JSON + `files[]` blobs。
- 也接受含有 `files`（基於 storageId）的 JSON body。
- 選填酬載欄位：`ownerHandle`。提供時，API 會在伺服器端解析該發布者，並要求行為者具有發布者存取權。
- 選填酬載欄位：`migrateOwner`。當搭配 `ownerHandle` 設為 `true` 時，如果行為者在目前與目標發布者上皆為管理員／擁有者，既有 skill 可移至該擁有者。若未明確選擇此項，擁有者變更會被拒絕。
...
- 允許的表單欄位為 `payload`、重複的 `files` blobs，或一個 `clawpack` tarball 參照。`clawpack` 可以是 `.tgz` blob，或是 upload-url 流程回傳的 storage id。使用暫存 storage-id 發布時，也必須包含該上傳 URL 一併回傳的 `clawpackUploadTicket`。
- 在同一個請求中只能使用 `files` 或 `clawpack`，不可同時使用。
- JSON bodies 與呼叫者提供的 `payload.files` / `payload.artifact` 中繼資料會被拒絕。
- 直接 multipart 發布請求上限為 18MB。ClawPack tarball 可使用 upload-url 流程，最高至 120MB tarball 上限。
- 選填酬載欄位：`ownerHandle`。提供時，只有管理員可以代表該擁有者發布。

驗證重點：

- `family` 必須是 `code-plugin` 或 `bundle-plugin`。
- 外掛套件需要 `openclaw.plugin.json`。ClawPack `.tgz` 上傳必須在 `package/openclaw.plugin.json` 包含它。
- 程式碼外掛需要 `package.json`、原始碼 repo 中繼資料、原始碼 commit 中繼資料、設定 schema 中繼資料、`openclaw.compat.pluginApi` 和 `openclaw.build.openclawVersion`。
- `openclaw.hostTargets` 和 `openclaw.environment` 是選填中繼資料。
- 只有 `openclaw` 組織發布者，以及目前 `openclaw` 組織成員的個人發布者，可以發布到 `official` 頻道。
- 代表發布仍會根據目標擁有者帳號驗證 official-channel 資格。
...
提供時，`reason` 會儲存為 skill 審核備註，並複製到稽核記錄中。
擁有者發起的軟刪除會保留該 slug 30 天，之後其他發布者可以宣告該 slug。
此到期適用時，刪除回應會包含 `slugReservedUntil`。
...
已驗證使用者自助建立組織發布者。建立新的組織發布者，並將呼叫者新增為擁有者。此端點不會遷移既有使用者／個人 handle，也不會將發布者標記為受信任／官方。
...
僅限管理員。為正當擁有者保留根 slug 和套件名稱，而不發布版本。套件名稱會成為沒有發布版本列的私有預留位置套件，因此同一擁有者之後可將真正的 code-plugin 或 bundle-plugin 發布版本發布到該名稱。
...
僅限管理員。為已驗證的替代 GitHub OAuth principal 復原個人發布者，而不編輯 Convex Auth 帳號列。請求必須指明兩個不可變的 GitHub provider account id；可變的 handle 僅作為面向操作員的防護使用。

端點預設為 dry-run。套用復原需要在工作人員獨立驗證兩個
GitHub 主體之間的連續性後，設定 `dryRun: false` 和
`confirmIdentityVerified: true`。當目的地使用者目前的個人
publisher 具有技能、套件或 GitHub 技能來源時，復原會失敗並採取關閉狀態。
復原也會遷移已復原 publisher 的技能、技能 slug 別名、套件、套件檢查器警告，以及衍生搜尋摘要資料列中的舊版 `ownerUserId` 欄位，讓直接擁有者路徑與新的 publisher 權限一致。已復原 handle 的有效受保護 handle 保留也會重新指派給替代使用者，讓後續
profile 同步無法還原前使用者的競爭權限。每個主要資料表在每次套用交易中限制為
100 列；較大的復原必須先使用可續行的擁有者遷移。
GitHub 技能來源以 publisher 為範圍，並回報為已檢查，而不是被重寫。
...
如果你自行代管，請提供此檔案（或明確設定 `CLAWHUB_REGISTRY`；舊版為 `CLAWDHUB_REGISTRY`）。
