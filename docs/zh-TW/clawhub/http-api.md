...
x-i18n:
    generated_at: "2026-07-02T07:57:52Z"
    model: gpt-5.5
...
第三方目錄可以使用公開讀取端點來列出或搜尋 ClawHub Skills。請快取結果、遵守 `429`/`Retry-After`、將使用者連回標準 ClawHub 清單（`https://clawhub.ai/<owner>/skills/<slug>`），並避免暗示 ClawHub 為該第三方網站背書。請勿嘗試在公開 API 表面之外鏡像隱藏、私人或因審核而封鎖的內容。

Web slug 捷徑會跨登錄檔系列解析，但 API 用戶端應使用讀取端點傳回的標準 URL，而不是自行重建路由優先順序。
...
- 如果 token 缺失/無效，行為會回退到 IP 強制執行。
- 已驗證的寫入端點在伺服器知道原因時，不應只傳回單純的 `Unauthorized`。缺失 token、無效/已撤銷 token，以及已刪除/被封禁/停用的帳號，都應取得可採取行動的文字，讓命令列介面用戶端能告知使用者被阻擋的原因。
...
- 如果沒有可用的受信任用戶端 IP，匿名請求會使用只依速率限制種類界定範圍的回退 bucket。這些回退 bucket 不包含呼叫端提供的路徑、slug、套件名稱、版本、查詢字串或其他成品參數。

## 錯誤回應

公開 v1 錯誤回應是純文字，並帶有 `content-type: text/plain; charset=utf-8`。
這包含驗證失敗（`400`）、缺失的公開資源（`404`）、驗證與權限失敗（`401`/`403`）、速率限制（`429`），以及被封鎖的下載。用戶端應將回應主體讀取為人類可讀的字串。未知的查詢參數會因相容性而被忽略，但已辨識的查詢參數若值無效，會傳回 `400`。
...
- 相關性比人氣更強。精確的 slug 或顯示名稱 token 符合，可以排在互動明顯更高但匹配較鬆散的結果之前。
- ASCII 文字會依單字和標點邊界斷詞。例如，`personal-map` 包含獨立的 `map` token，而 `amap-jsapi-skill` 包含 `amap`、`jsapi` 和 `skill`；因此搜尋 `map` 會讓 `personal-map` 比 `amap-jsapi-skill` 有更強的詞彙匹配。
- 人氣會以對數縮放並設上限。當查詢文字匹配較弱時，高互動的 Skills 仍可能排名較低。
- 可疑或隱藏的審核狀態，可能會依呼叫端篩選條件和目前審核狀態，將 Skill 從公開搜尋中移除。

發布者可探索性指引：

- 將使用者會實際搜尋的詞放在顯示名稱、摘要和標籤中。只有當獨立 slug token 也是你想保留的穩定身分時，才使用它。
- 不要只為了追逐單一查詢而重新命名 slug，除非新 slug 是更適合作為長期標準名稱的選擇。舊 slug 會成為重新導向別名，但標準 URL、顯示的 slug，以及未來的搜尋摘要會使用新 slug。
- 重新命名別名會保留舊 URL 和透過登錄檔解析的安裝解析能力，但搜尋排名會依重新命名完成索引後的標準 Skill 中繼資料計算。現有統計資料會保留在該 Skill 上。
...
- 當 `nonSuspiciousOnly=true` 時，基於游標的排序可能在單頁傳回少於 `limit` 個項目，因為可疑 Skills 會在頁面擷取後被篩除。
- 當 `nextCursor` 存在時，請用它繼續分頁。短頁本身並不表示結果已結束。
...
- 公開呼叫端只有對已標記且可見的 Skills 才會取得 `200`。
- 對公開呼叫端，證據會被遮蔽，且只有擁有者/審核者可取得原始片段。

### `POST /api/v1/skills/{slug}/report`

回報 Skill 供審核者審查。回報是 Skill 層級，可選擇連結到版本，並會送入 Skill 回報佇列。
...
`note` 對 `confirmed` 和 `dismissed` 是必要的；將 `status` 設回 `open` 時可以省略。傳入 `finalAction: "hide"` 搭配已分流的回報，可在同一個可稽核工作流程中隱藏該 Skill。
...
- 回傳已排隊/執行中/成功/失敗狀態。
- 排隊期間回傳 `queue.queuedAhead` 和 `queue.position`，讓用戶端可以顯示該請求前方有多少個已優先處理的手動掃描。非常大的佇列會受到上限限制，並以 `queuedAheadIsEstimate: true` 回報。
...
- 只有當選取的版本具有已產生的 Skill Card、未被審核封鎖為惡意軟體，且 ClawScan 驗證為乾淨時，`ok` 才會是 `true`。
- 技能身分、發布者身分與所選版本中繼資料是頂層信封欄位（`slug`、`displayName`、`publisherHandle`、`version`、`resolvedFrom`、`tag`、`createdAt`），讓 shell 自動化可在不解開巢狀包裝的情況下讀取它們。
...
回傳精確技能版本的目前精簡安全性判定。此集合端點適用於已知道需要顯示哪些已安裝
...
- `security.signals` 只包含狀態層級的支援證據；完整掃描器詳細資訊請使用 `/scan` 或 ClawHub security-audit 頁面。
- `security.signals.dependencyRegistry` 會保留以維持 v1 回應相容性，但依賴項登錄存在性掃描器已退役，此鍵一律為 `null`。
...
- `category`（選填）：外掛分類篩選器。僅在請求範圍限定為外掛套件時支援
  （`/api/v1/plugins`、
  `/api/v1/code-plugins`、`/api/v1/bundle-plugins`，或帶有
  `family=code-plugin`/`family=bundle-plugin` 的套件端點）。受控分類和
...
- `POST /api/v1/packages` 仍僅用於 code-plugin 和 bundle-plugin 發布版本。
- 匿名呼叫者只能看到公開套件通道。
- 已驗證呼叫者可在清單/搜尋結果中看到其所屬發布者的私人套件。
- `channel=private` 僅會傳回已驗證呼叫者可讀取的套件。
...
在統一型 `/api/v1/packages` 端點上，它僅適用於外掛；技能目錄請使用
`/api/v1/skills?sort=trending`。

舊版別名不會被接受作為已儲存或作者宣告的分類值。
...
- 託管技能包含最新儲存版本檔案，並列於
  `_manifest.json` 中，帶有 `sourceRef: "public-clawhub"`。
- 目前由 GitHub 支援且掃描結果為 `clean` 或 `suspicious` 的技能包含
...
- `version.sha256hash` 是供舊用戶端使用的已棄用相容性中繼資料。它會
  ...
  標準發行成品。
- `version.vtAnalysis`、`version.llmAnalysis` 和 `version.staticScan` 會在
  掃描資料存在時包含。
- 私有套件會傳回 `404`，除非呼叫端可以讀取擁有該套件的發布者。

### `GET /api/v1/packages/{name}/versions/{version}/security`

傳回安裝用戶端使用的確切套件發行安全性與信任摘要。這是公共 OpenClaw
消費介面，用於決定已解析的發行版本是否可以安裝。
...
- `trust.scanStatus` 是從掃描器輸入和手動發行審核推導出的有效信任狀態。
- `trust.moderationState` 可為 null。當不存在手動發行審核時為 `null`。
- `trust.blockedFromDownload` 是安裝封鎖訊號。OpenClaw 和其他安裝用戶端應在
  此值為 `true` 時封鎖安裝，而不是從掃描器或審核欄位重新推導封鎖規則。
...
- `trust.stale` 表示信任摘要是從過時輸入計算而來，並且在高信心允許決策前
  應視為需要重新整理。
...
`confirmed` 和 `dismissed` 需要 `note`；將 `status` 設回 `open` 時可省略。針對已確認的回報傳入 `finalAction: "quarantine"` 或 `finalAction: "revoke"`，即可在同一個可稽核工作流程中套用發布版本審核。
...
隔離和撤銷的發布版本會從成品下載路由傳回 `403`。每次變更都會寫入一筆稽核日誌項目。
...
- 二進位檔案會傳回 `415`。
- 檔案大小限制：200KB。
- 待處理的 VirusTotal 掃描不會封鎖讀取；惡意發布版本仍可能在其他地方被保留不提供。
...
- 具範圍套件的 packument 同時支援 `/api/npm/@scope/name` 和 npm 的編碼請求路徑 `/api/npm/@scope%2Fname`。
...
下載託管技能版本 ZIP，或針對目前由 GitHub 支援、掃描結果為 `clean` 或 `suspicious` 且沒有託管版本的技能，傳回 GitHub 來源交接。
...
- 軟刪除版本會傳回 `410`。
- GitHub 支援的技能交接不會代理或鏡像位元組。JSON 回應包含 `sourceRef: "public-github"`、`repo`、`commit`、`path`、`contentHash` 和 `archiveUrl`；掃描／目前狀態是門檻，不會作為成功酬載中繼資料包含在內。
- 下載統計會以每個 UTC 日的唯一身分計數（API 權杖有效時為 `userId`，否則為 IP）。
...
- 選用酬載欄位：`ownerHandle`。若存在，API 會在伺服器端解析該發布者，並要求執行者具備發布者存取權。
- 選用酬載欄位：`migrateOwner`。當它與 `ownerHandle` 一起為 `true` 時，如果執行者同時是目前與目標發布者的管理員／擁有者，現有技能可移動到該擁有者。若沒有此明確選擇，擁有者變更會被拒絕。
...
- 允許的表單欄位為 `payload`、重複的 `files` blob，或一個 `clawpack` tarball 參照。`clawpack` 可以是 `.tgz` blob，或上傳 URL 流程傳回的 storage id。使用暫存 storage-id 發布時，也必須包含該上傳 URL 一併傳回的 `clawpackUploadTicket`。
- 使用 `files` 或 `clawpack` 其中之一，絕不可在同一請求中同時使用兩者。
...
- 選用酬載欄位：`ownerHandle`。若存在，只有管理員可代表該擁有者發布。
...
- 代為發布仍會依目標擁有者帳戶驗證 official-channel 資格。
...
若存在，`reason` 會儲存為技能審核備註，並複製到稽核日誌。
擁有者發起的軟刪除會保留 slug 30 天，之後該 slug 可由另一位發布者宣告。
...
僅限管理員。確保某個 handle 存在組織發布者。如果該 handle 仍指向舊版共享使用者／個人發布者，端點會先將其遷移為組織發布者。
若是新建立的組織，請提供 `memberHandle`；執行動作的管理員不會被加入為成員。
...
已驗證的自助組織發布者建立。建立新的組織發布者，並將呼叫者加入為擁有者。此端點不會遷移現有使用者／個人 handle，也不會將發布者標記為受信任／官方。
...
僅限管理員。為合法擁有者保留根 slug 和套件名稱，而不發布任何發布版本。套件名稱會成為沒有發布版本資料列的私人預留位置套件，因此同一擁有者之後可將真正的 code-plugin 或 bundle-plugin 發布版本發布到該名稱。
...
僅限管理員。為已驗證的替代 GitHub OAuth 主體復原個人發布者，而不編輯 Convex Auth 帳戶資料列。請求必須命名兩個不可變的 GitHub provider account id；可變 handle 只作為面向操作員的防護。

端點預設為 dry-run。套用復原需要在工作人員獨立驗證兩個
GitHub 主體之間的連續性後，設定 `dryRun: false` 和
`confirmIdentityVerified: true`。當目的地使用者目前的個人
發布者擁有技能、套件或 GitHub 技能來源時，復原會以失敗關閉。
復原也會遷移已復原發布者的技能、技能 slug 別名、套件、套件檢查器警告，以及衍生搜尋摘要資料列中的舊版 `ownerUserId` 欄位，讓
直接擁有者路徑與新的發布者權限一致。已復原 handle 的作用中受保護 handle
保留項目也會重新指派給替代使用者，讓後續
