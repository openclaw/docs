...
x-i18n:
    generated_at: "2026-07-05T11:05:57Z"
    model: gpt-5.5
...
第三方目錄可以使用公開讀取端點來列出或搜尋 ClawHub Skills。請快取結果、遵守 `429`/`Retry-After`、將使用者連回正式的 ClawHub 列表（`https://clawhub.ai/<owner>/skills/<slug>`），並避免暗示 ClawHub 為第三方網站背書。請勿嘗試在公開 API 表面之外鏡像隱藏、私有或因審核而封鎖的內容。

網頁 slug 捷徑會跨登錄家族解析，但 API 用戶端應使用讀取端點回傳的正式 URL，而不是自行重建路由優先順序。
...
- 匿名請求：依 IP 強制執行。
- 已驗證請求（有效的 Bearer token）：依使用者配額桶強制執行。
- 如果 token 缺失/無效，行為會退回到依 IP 強制執行。
- 已驗證的寫入端點在伺服器知道原因時，不應只回傳裸露的 `Unauthorized`。缺失 token、無效/已撤銷 token，以及已刪除/已封禁/已停用帳號，都應取得可採取行動的文字，讓命令列介面用戶端能告知使用者被阻擋的原因。
...
- 如果沒有可用的受信任用戶端 IP，匿名請求會使用僅依速率限制種類限定範圍的後備配額桶。這些後備配額桶不包含呼叫者提供的路徑、slug、套件名稱、版本、查詢字串或其他成品參數。

## 錯誤回應

公開 v1 錯誤回應是純文字，並帶有 `content-type: text/plain; charset=utf-8`。
這包含驗證失敗（`400`）、缺失的公開資源（`404`）、驗證與權限失敗（`401`/`403`）、速率限制（`429`），以及被封鎖的下載。用戶端應將回應主體讀作人類可讀的字串。未知查詢參數會為相容性而被忽略，但已識別且值無效的查詢參數會回傳 `400`。
...
- 相關性比人氣更強。精確的 slug 或顯示名稱 token 符合，可以排在互動強得多但符合較鬆散的結果之前。
- ASCII 文字會依單字與標點邊界進行 token 化。例如，`personal-map` 包含獨立的 `map` token，而 `amap-jsapi-skill` 包含 `amap`、`jsapi` 與 `skill`；因此搜尋 `map` 會讓 `personal-map` 比 `amap-jsapi-skill` 有更強的詞彙符合。
- 人氣採用對數縮放並設有上限。當查詢文字符合較弱時，高互動 Skills 仍可能排名較低。
- 可疑或隱藏的審核狀態，可能會依呼叫者篩選條件與目前審核狀態，將某個 Skills 從公開搜尋中移除。

發布者可發現性指引：

- 將使用者實際會搜尋的詞放在顯示名稱、摘要與標籤中。只有在獨立 slug token 也是你想保留的穩定識別時才使用它。
- 除非新 slug 是更好的長期正式名稱，否則不要只為了追逐某個查詢而重新命名 slug。舊 slug 會成為重新導向別名，但正式 URL、顯示的 slug，以及未來搜尋摘要會使用新 slug。
- 重新命名別名會保留舊 URL 與透過登錄解析的安裝解析能力，但搜尋排名會以重新命名完成索引後的正式 Skills 中繼資料為基礎。既有統計資料會保留在該 Skills 上。
...
- `trending` 依最近 7 天安裝量排名（基於遙測）。
- `createdAt` 對新 Skills 抓取是穩定的；`updated` 會在既有 Skills 重新發布時變更。
- 當 `nonSuspiciousOnly=true` 時，基於游標的排序可能會在某頁回傳少於 `limit` 的項目，因為可疑 Skills 會在頁面擷取後被篩除。
...
- 由擁有者重新命名/合併流程建立的舊 slug，會解析到正式 Skills。
- `metadata.os`：Skills frontmatter 中宣告的作業系統限制（例如 `["macos"]`、`["linux"]`）。未宣告時為 `null`。
- `metadata.systems`：Nix 系統目標（例如 `["aarch64-darwin", "x86_64-linux"]`）。未宣告時為 `null`。
- 如果 Skills 沒有平台中繼資料，`metadata` 為 `null`。
- `moderation` 只會在 Skills 被標記，或擁有者正在檢視時包含。
...
- 擁有者與審核員可以存取隱藏 Skills 的審核詳細資訊。
- 公開呼叫者只會對已標記且可見的 Skills 取得 `200`。
- 證據會對公開呼叫者遮蔽，且只會對擁有者/審核員包含原始片段。

### `POST /api/v1/skills/{slug}/report`

回報 Skills 以供審核員檢閱。回報屬於 Skills 層級，可選擇連結到某個版本，並會送入 Skills 回報佇列。
...
供審核員/管理員使用，用於解決或重新開啟 Skills 回報的端點。
...
`note` 對 `confirmed` 與 `dismissed` 為必填；將 `status` 設回 `open` 時可以省略。傳入 `finalAction: "hide"` 與已分流的回報，可在同一個可稽核工作流程中隱藏該 Skills。
...
- `version.security` 會在可用時包含正規化掃描驗證狀態與掃描器詳細資訊（VirusTotal + LLM）。

### `GET /api/v1/skills/{slug}/scan`

回傳某個 Skills 版本的安全掃描驗證詳細資訊。

查詢參數：

- `version`（選填）：特定版本字串。
- `tag`（選填）：解析有標籤的版本（例如 `latest`）。

備註：

- 如果未提供 `version` 或 `tag`，則使用最新版本。
- 包含標準化驗證狀態，以及掃描器特定的詳細資料。
...
不再支援本機上傳掃描。使用
`multipart/form-data` 或 `{ "source": { "kind": "upload" } }` 的要求會回傳 `410`。
...
- 掃描工作是非同步的。手動掃描要求會優先於一般發布/回填工作，但完成時間仍取決於工作程式可用性。
...
- 佇列中時會回傳 `queue.queuedAhead` 和 `queue.position`，讓用戶端顯示要求前方還有多少個優先手動掃描。非常大的佇列會受到界限限制，並以 `queuedAheadIsEstimate: true` 回報。
- 可用時，`report` 會包含 `clawscan`、`skillspector`、`staticAnalysis` 和 `virustotal` 區段。
- 失敗的掃描工作會以 `lastError` 回傳 `status: "failed"`。

### `GET /api/v1/skills/-/scan/{scanId}/download`

已驗證的報告封存端點。

- 需要已成功的掃描；非終端狀態掃描會回傳 `409`。
...
- 只有在所選版本具有已產生的 Skill Card、未被審核標記為惡意軟體封鎖，且 ClawScan 驗證為 clean 時，`ok` 才會是 `true`。
...
- `security` 是頂層 ClawScan/安全性判定。自動化應依據 `ok`、`decision`、`reasons` 和 `security.status`。
- `security.signals` 包含支援用掃描器證據，例如 `staticScan`、`virusTotal` 和 `skillSpector`。
- `security.signals.dependencyRegistry` 會保留以相容 v1 回應，但依賴項登錄檔存在性掃描器已退役，且此鍵一律為 `null`。
- 只有在 ClawHub 於發布或匯入期間解析並儲存 GitHub 儲存庫/ref/commit/path 時，`provenance` 才會是 `server-resolved-github-import`；否則會是 `unavailable`。
...
- 需要單一技能的 Skill Card 驗證信封時請使用 `/verify`，需要已產生的卡片 Markdown 時請使用 `/card`，需要詳細掃描器資料時請使用 `/scan`。
...
- `category`（選填）：外掛分類篩選器。僅在請求範圍限定於外掛套件（`/api/v1/plugins`、`/api/v1/code-plugins`、`/api/v1/bundle-plugins`，或帶有 `family=code-plugin`/`family=bundle-plugin` 的套件端點）時支援。受控分類與舊版 v1 篩選器別名記錄於 `GET /api/v1/plugins` 下。

注意事項：

- `family`、`channel`、`isOfficial`、`featured`、`highlightedOnly` 或 `sort` 的無效值會傳回 `400`。未知的查詢參數會被忽略。
- `GET /api/v1/code-plugins` 和 `GET /api/v1/bundle-plugins` 仍為固定 family 的別名。
- Skill 項目仍由 skill registry 支援，且仍只能透過 `POST /api/v1/skills` 發布。
- `POST /api/v1/packages` 仍僅用於 code-plugin 和 bundle-plugin 發行版本。
- 匿名呼叫者只會看到公開套件 channel。
...
- `channel`（選填）：`official`、`community` 或 `private`
- `isOfficial`（選填）：`true` 或 `false`
- `category`（選填）：外掛分類篩選器。僅在請求範圍限定於外掛套件時支援。受控分類與舊版 v1 篩選器別名記錄於 `GET /api/v1/plugins` 下。
...
`trending` 是七日安裝/下載排行榜，不使用歷來總計。
在統一 `/api/v1/packages` 端點上，它僅限外掛；skill 目錄請使用
`/api/v1/skills?sort=trending`。

舊版別名不會作為已儲存或作者宣告的分類值被接受。

### `GET /api/v1/skills/export`

大量匯出最新公開 skills 以進行離線分析。
...
- 目前由 GitHub 支援且掃描結果為 `clean` 或 `suspicious` 的 skills 包含
  ...
  content hash 和 archive URL。它們不包含 ClawHub 託管的原始檔案。
...
- `family`（選填）：`code-plugin` 或 `bundle-plugin`。省略表示兩種外掛家族都包含。
...
僅限外掛搜尋，涵蓋 code-plugin 與 bundle-plugin 套件。
...
- 也接受 `GET /api/v1/plugins` 底下記載的舊版 v1 篩選器別名。
...
- ClawPack 版本包含 npm 相容的 `npmIntegrity`、`npmShasum` 和
  `npmTarballName` 欄位。
- `version.sha256hash` 是給舊用戶端使用的已淘汰相容性中繼資料。它會雜湊 `/api/v1/packages/{name}/download` 回傳的確切 ZIP 位元組。
...
回傳安裝用戶端所需的確切套件版本安全性與信任摘要。這是公開 OpenClaw 消費介面，用來判斷已解析版本是否可以安裝。
...
- `release.releaseId`、`release.version` 和 `release.createdAt` 識別經評估的確切版本。
- `release.artifactKind`、`release.artifactSha256`、`release.npmIntegrity`、
  `release.npmShasum` 和 `release.npmTarballName` 在版本成品已知時存在。
- `trust.scanStatus` 是由掃描器輸入與手動版本審核推導出的有效信任狀態。
- `trust.moderationState` 可為 null。當不存在手動版本審核時為 `null`。
- `trust.blockedFromDownload` 是安裝封鎖訊號。OpenClaw 和其他安裝用戶端應在此值為 `true` 時封鎖安裝，而不是從掃描器或審核欄位重新推導封鎖規則。
- `trust.reasons` 是面向使用者與稽核的說明清單。原因代碼是穩定、精簡的字串，例如 `manual:quarantined`、`scan:malicious` 和 `package:malicious`。
- `trust.pending` 表示一個或多個信任輸入仍在等待完成。
- `trust.stale` 表示信任摘要是從過期輸入計算而來，應視為需要重新整理後，才能做出高信心的允許決策。

注意事項：

- 此端點精確到版本。用戶端應在解析要安裝的套件版本後呼叫它，而不是只在讀取最新套件中繼資料後呼叫。
- 私有套件會回傳 `404`，除非呼叫者可以讀取所屬發布者。
- 此端點刻意比擁有者/審核員審核端點更窄。它公開安裝決策與公開說明，而不是回報者身分、報告本文、私有證據或內部審查時間軸。
...
- ClawPack 版本會回傳 `npm-pack` 成品、npm 完整性欄位、`tarballUrl` 和舊版 ZIP 相容性 URL。
...
- `packageName` 會以 npm 名稱正規化；對計畫中的遷移，套件可以不存在。
...
回報套件以供審核員審查。報告是套件層級，可選擇性連結至版本。它們會進入審核佇列，但不會自行自動隱藏或封鎖下載；審核員應使用版本審核來核准、隔離或撤銷成品。
...
`confirmed` 和 `dismissed` 需要 `note`；將 `status` 設回 `open` 時可省略。
傳入 `finalAction: "quarantine"` 或 `finalAction: "revoke"` 並搭配已確認的回報，即可在同一個可稽核工作流程中套用發行版審核處置。
...
- `revoked`：先前受信任的發行版現已封鎖。

隔離和撤銷的發行版會在成品下載路由傳回 `403`。
每次變更都會寫入一筆稽核日誌項目。
...
- 二進位檔案會傳回 `415`。
- 檔案大小限制：200KB。
- 等待中的 VirusTotal 掃描不會封鎖讀取；惡意發行版仍可能在其他地方被扣留。
...
- 外掛/套件封存檔是含有 `package/` 根目錄的 zip 檔案，讓舊版 OpenClaw 用戶端能繼續運作。
...
- 等待中的 VirusTotal 掃描不會封鎖下載；惡意發行版會傳回 `403`。
...
下載託管的 skill 版本 ZIP，或針對目前 GitHub 支援、掃描結果為 `clean` 或 `suspicious` 且沒有託管版本的 skill，傳回 GitHub 原始碼交接資訊。
...
- 軟刪除版本會傳回 `410`。
- GitHub 支援的 skill 交接不會代理或鏡像位元組。JSON 回應包含 `sourceRef: "public-github"`、`repo`、`commit`、`path`、`contentHash` 和 `archiveUrl`；掃描/目前狀態是門檻條件，不會作為成功酬載中繼資料包含在內。
- 下載統計會以 UTC 日為單位計算唯一身分（API 權杖有效時為 `userId`，否則為 IP）。
...
- 選填酬載欄位：`migrateOwner`。當它與 `ownerHandle` 一起為 `true` 時，若行動者同時是目前和目標發布者的管理員/擁有者，現有 skill 可移至該擁有者。若未明確選擇此選項，擁有者變更會被拒絕。
...
- 允許的表單欄位為 `payload`、重複的 `files` blob，或一個 `clawpack` tarball 參照。`clawpack` 可以是 `.tgz` blob，或由 upload-url 流程傳回的 storage id。使用暫存 storage-id 發布時，也必須包含該上傳 URL 傳回的 `clawpackUploadTicket`。
- 使用 `files` 或 `clawpack` 其中一種，同一請求中不可同時使用。
- JSON body 以及呼叫者提供的 `payload.files` / `payload.artifact` 中繼資料會被拒絕。
- 直接 multipart 發布請求上限為 18MB。ClawPack tarball 可透過 upload-url 流程使用最高 120MB 的 tarball 上限。
- 選填酬載欄位：`ownerHandle`。存在時，只有管理員可代表該擁有者發布。
...
存在時，`reason` 會儲存為 skill 審核備註，並複製到稽核日誌。
由擁有者發起的軟刪除會保留 slug 30 天，之後該 slug 可由另一個發布者取得。
...
已驗證使用者可自助建立組織發布者。建立新的組織發布者，並將呼叫者加入為擁有者。此端點不會遷移現有使用者/個人 handle，也不會將發布者標記為受信任/官方。

- Body：`{ "handle": "opik", "displayName": "Opik" }`
- Response：`{ "ok": true, "publisherId": "...", "handle": "opik", "created": true, "trusted": false }`
- 當 handle 已由發布者、使用者或個人發布者使用時，傳回 `409`。

### `POST /api/v1/users/reserve`

僅限管理員。為合法擁有者保留根 slug 和套件名稱，而不發布發行版。套件名稱會成為沒有發行列的私有預留位置套件，因此同一擁有者之後可將真正的 code-plugin 或 bundle-plugin 發行版發布到該名稱。
...
僅限管理員。為已驗證的替代 GitHub OAuth 主體復原個人發布者，而不編輯 Convex Auth 帳號列。請求必須命名兩個不可變的 GitHub 提供者帳號 id；可變 handle 僅作為面向操作員的防護。

此端點預設為 dry-run。套用復原需要在工作人員獨立驗證兩個
GitHub 主體之間的連續性後，設定 `dryRun: false` 和
`confirmIdentityVerified: true`。當目的地使用者目前的個人發布者擁有 skills、packages 或 GitHub skill sources 時，復原會失敗並關閉。
復原也會遷移復原發布者的 skills、skill slug aliases、packages、package inspector warnings，以及衍生 search digest rows 的舊版 `ownerUserId` 欄位，讓 direct-owner paths 與新的發布者權限一致。復原 handle 的有效 protected-handle
reservation 也會重新指派給替代使用者，讓後續
profile synchronization 無法還原前使用者的競爭權限。每個主要資料表在每次套用交易中限制為
100 列；較大的復原必須先使用可續傳的 owner migration。
GitHub skill sources 屬於發布者範圍，並會回報為已檢查，而不是被重寫。
...
- 兩個端點都需要 API token 驗證，且僅適用於 skill 擁有者。
