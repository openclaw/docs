...
x-i18n:
    generated_at: "2026-07-05T20:17:27Z"
    model: gpt-5.5
...
第三方目錄可以使用公開讀取端點來列出或搜尋 ClawHub skills。請快取結果、遵守 `429`/`Retry-After`、將使用者連回標準 ClawHub 列表（`https://clawhub.ai/<owner>/skills/<slug>`），並避免暗示 ClawHub 為第三方網站背書。不要嘗試在公開 API 範圍之外鏡像隱藏、私有或遭審核封鎖的內容。

網頁 slug 快捷方式會跨註冊表家族解析，但 API 用戶端應使用讀取端點傳回的標準 URL，而不是重建路由優先順序。
...
- 匿名請求：依 IP 強制執行。
- 已驗證請求（有效的 Bearer 權杖）：依使用者儲存桶強制執行。
- 如果權杖遺失/無效，行為會回退到 IP 強制執行。
- 已驗證的寫入端點在伺服器知道原因時，不應只傳回裸露的 `Unauthorized`。缺少權杖、無效/已撤銷權杖，以及已刪除/封鎖/停用的帳號，都應取得可操作的文字，讓命令列介面用戶端能告知使用者遭到阻擋的原因。
...
- 使用帶有抖動的退避，以避免同步重試。
- 如果 `Retry-After` 缺失，則回退到 `RateLimit-Reset`（或從 `X-RateLimit-Reset` 計算）。

IP 來源：

- 只有在部署明確啟用受信任的轉送標頭時，才會使用受信任的用戶端 IP 標頭，包括 `cf-connecting-ip`。
- ClawHub 使用受信任的轉送標頭，在邊緣識別用戶端 IP。
- 如果沒有可用的受信任用戶端 IP，匿名請求會使用僅依速率限制種類界定範圍的回退儲存桶。這些回退儲存桶不包含呼叫端提供的路徑、slug、套件名稱、版本、查詢字串或其他成品參數。

## 錯誤回應

公開 v1 錯誤回應是純文字，並帶有 `content-type: text/plain; charset=utf-8`。
這包括驗證失敗（`400`）、缺少公開資源（`404`）、驗證和權限失敗（`401`/`403`）、速率限制（`429`），以及被封鎖的下載。用戶端應將回應主體讀作可供人閱讀的字串。未知查詢參數會被忽略以維持相容性，但已識別且值無效的查詢參數會傳回 `400`。
...
- `q`（必要）：查詢字串
- `limit`（選用）：整數
- `highlightedOnly`（選用）：`true` 以篩選為精選 skills
- `nonSuspiciousOnly`（選用）：`true` 以隱藏可疑（`flagged.suspicious`）skills
...
- 相關性比人氣更強。精確的 slug 或顯示名稱權杖相符，可能會排在互動更強但相符較寬鬆的結果之前。
- ASCII 文字會依單字和標點邊界切分權杖。例如，`personal-map` 包含獨立的 `map` 權杖，而 `amap-jsapi-skill` 包含 `amap`、`jsapi` 和 `skill`；因此搜尋 `map` 會讓 `personal-map` 比 `amap-jsapi-skill` 取得更強的詞彙相符。
- 人氣會採對數縮放並設上限。當查詢文字相符較弱時，高互動 skills 可能排名較低。
- 依呼叫端篩選條件和目前審核狀態而定，可疑或隱藏的審核狀態可能會讓 skill 從公開搜尋中移除。

發布者可發現性指引：

- 將使用者實際會搜尋的詞放在顯示名稱、摘要和標籤中。只有在獨立 slug 權杖也是你想保留的穩定身分時，才使用它。
- 除非新 slug 是更好的長期標準名稱，否則不要只為追逐單一查詢而重新命名 slug。舊 slug 會成為重新導向別名，但標準 URL、顯示的 slug，以及未來的搜尋摘要都會使用新 slug。
- 重新命名別名會保留舊 URL 和透過註冊表解析安裝的解析能力，但搜尋排名會根據重新命名完成索引後的標準 skill 中繼資料。既有統計資料會保留在該 skill 上。
- 如果 skill 意外不可見，請先在登入狀態下使用 `clawhub inspect @owner/slug` 檢查審核狀態，再變更與排名相關的中繼資料。
...
- 當 `nonSuspiciousOnly=true` 時，基於游標的排序可能會在頁面上傳回少於 `limit` 的項目，因為可疑 skills 是在頁面擷取後才被篩除。
- 存在時請使用 `nextCursor` 繼續分頁。短頁面本身不代表結果已結束。
...
- `moderation` 只有在 skill 被標記或擁有者正在檢視時才會包含。
...
- 公開呼叫端的證據會被修訂，且只有擁有者/審核員才會包含原始片段。

### `POST /api/v1/skills/{slug}/report`

回報 skill 以供審核員審查。回報屬於 skill 層級，可選擇連結到版本，並會送入 skill 回報佇列。
...
`confirmed` 和 `dismissed` 需要 `note`；將 `status` 設回 `open` 時可以省略。對已分流處理的回報傳入 `finalAction: "hide"`，可在同一個可稽核工作流程中隱藏該 skill。
...
- 可用時，`version.security` 會包含正規化的掃描驗證狀態和掃描器詳細資料
  （VirusTotal + LLM）。
...
- 掃描作業是非同步的。手動掃描請求會優先於一般發布/回填工作，但完成時間仍取決於工作程式可用性。
...
- 回傳已排隊/執行中/成功/失敗狀態。
- 排隊時會回傳 `queue.queuedAhead` 和 `queue.position`，讓用戶端可以顯示請求前方有多少個優先處理的手動掃描。非常大的佇列會受限並以 `queuedAheadIsEstimate: true` 回報。
...
- 需要該技能或外掛的擁有者/發布者管理存取權，或平台審核者/管理員權限。
- 回傳確切已提交版本的已儲存掃描結果，包括遭封鎖或隱藏的版本。
...
- 只有在所選版本有已產生的技能卡、未被審核標記為惡意軟體封鎖，且 ClawScan 驗證為乾淨時，`ok` 才會是 `true`。
- 技能身分、發布者身分與所選版本中繼資料是頂層封套欄位（`slug`、`displayName`、`publisherHandle`、`version`、`resolvedFrom`、`tag`、`createdAt`），因此 Shell 自動化可以不拆開巢狀包裝器就讀取它們。
- `security` 是頂層 ClawScan/安全性判定。自動化應根據 `ok`、`decision`、`reasons` 和 `security.status` 判斷。
- `security.signals` 包含支援性的掃描器證據，例如 `staticScan`、`virusTotal` 和 `skillSpector`。
- `security.signals.dependencyRegistry` 保留用於 v1 回應相容性，但依賴項登錄存在性掃描器已停用，且此鍵一律為 `null`。
- 只有在 ClawHub 於發布或匯入期間解析並儲存 GitHub repo/ref/commit/path 時，`provenance` 才會是 `server-resolved-github-import`；否則會是 `unavailable`。

### `POST /api/v1/skills/-/security-verdicts`

回傳精確技能版本的目前精簡安全性判定。此集合端點適用於已知需要顯示哪些已安裝 ClawHub 技能版本的用戶端，例如 OpenClaw Control UI。
...
- 結果會依項目回傳；單一缺少的技能或版本不會讓整個回應失敗。
- 回應僅包含安全性資訊。不包含技能卡資料、已產生卡片狀態、成品檔案清單，或詳細掃描器承載資料。
...
- 需要單一技能的技能卡驗證封套時使用 `/verify`，需要已產生的卡片 Markdown 時使用 `/card`，需要詳細掃描器資料時使用 `/scan`。
...
- `category`（選填）：外掛分類篩選器。僅在請求限定於外掛套件時支援（`/api/v1/plugins`、`/api/v1/code-plugins`、`/api/v1/bundle-plugins`，或帶有 `family=code-plugin`/`family=bundle-plugin` 的套件端點）。受控分類與舊版 v1 篩選別名記錄於 `GET /api/v1/plugins` 下。
...
- Skills 項目仍由 Skills 登錄支援，且仍只能透過 `POST /api/v1/skills` 發布。
- `POST /api/v1/packages` 仍僅適用於 code-plugin 和 bundle-plugin 發行。
- 匿名呼叫者只能看到公開套件 channel。
- 已驗證呼叫者可以在列表/搜尋結果中看到其所屬發布者的 private 套件。
...
- `channel`（選填）：`official`、`community` 或 `private`
- `isOfficial`（選填）：`true` 或 `false`
- `category`（選填）：外掛分類篩選器。僅在請求限定於外掛套件時支援。受控分類與舊版 v1 篩選別名記錄於 `GET /api/v1/plugins` 下。
...
- 本文：ZIP 封存。
- 每個匯出的 Skills 以 `{publisher}/{slug}/` 為根目錄。
- 託管的 Skills 包含最新儲存版本檔案，並在 `_manifest.json` 中以 `sourceRef: "public-clawhub"` 列出。
- 目前由 GitHub 支援且具有 `clean` 或 `suspicious` 掃描結果的 Skills，包含 `_source_handoff.json`，其中有 `sourceRef: "public-github"`、repo、commit、path、content hash 和 archive URL。它們不包含 ClawHub 託管的原始碼檔案。
- 每個 Skills 都包含 `_export_skill_meta.json`。
...
- `family`（選用）：`code-plugin` 或 `bundle-plugin`。省略表示兩種
  外掛家族。
...
只針對 code-plugin 和 bundle-plugin 套件進行外掛搜尋。
...
- 私有套件會傳回 `404`，除非呼叫者可以讀取擁有該套件的發佈者。
...
- 需要套件擁有者、組織發佈者擁有者/管理員、平台審核員或平台管理員的 API 權杖。
...
- ClawPack 版本包含與 npm 相容的 `npmIntegrity`、`npmShasum` 和
  `npmTarballName` 欄位。
- `version.sha256hash` 是供舊用戶端使用的已棄用相容性中繼資料。它
  會雜湊 `/api/v1/packages/{name}/download` 傳回的精確 ZIP 位元組。
  現代用戶端應使用 `version.artifact.sha256`，它識別
  標準版本成品。
...
傳回供安裝用戶端使用的精確套件版本安全性與信任摘要。
這是公開的 OpenClaw 消費介面，用於決定已解析的版本是否可以安裝。

驗證：

- 公開讀取端點。不需要擁有者、發佈者、審核員或管理員權杖。
...
- `trust.blockedFromDownload` 是安裝封鎖訊號。OpenClaw 和其他
  安裝用戶端應在此值為 `true` 時封鎖安裝，而不是從掃描器或審核欄位
  重新推導封鎖規則。
- `trust.reasons` 是面向使用者和稽核用的說明清單。原因代碼是穩定、
  精簡的字串，例如 `manual:quarantined`、`scan:malicious` 和
  `package:malicious`。
...
- 此端點精確到版本。用戶端應在解析要安裝的套件版本後呼叫它，
  而不只是讀取最新套件中繼資料後呼叫。
- 私有套件會傳回 `404`，除非呼叫者可以讀取擁有該套件的發佈者。
- 此端點刻意比擁有者/審核員審核端點更窄。它公開安裝決策和公開說明，
...
回報套件以供審核員審查。回報屬於套件層級，可選擇性
連結到版本。它們會送入審核佇列，但不會自行自動隱藏或
封鎖下載；審核員應使用版本審核來
核准、隔離或撤銷成品。
...
`note` 對 `confirmed` 和 `dismissed` 是必填；將 `status` 設回 `open` 時可以省略。對已確認的檢舉傳入 `finalAction: "quarantine"` 或 `finalAction: "revoke"`，即可在同一個可稽核工作流程中套用發行版審核。
...
- `revoked`：發行版先前受信任後被封鎖。

遭隔離與撤銷的發行版會從成品下載路由回傳 `403`。每次變更都會寫入一筆稽核日誌項目。
...
- 二進位檔案回傳 `415`。
- 檔案大小限制：200KB。
- 待處理的 VirusTotal 掃描不會封鎖讀取；惡意發行版仍可能在其他地方被保留不提供。
...
- 外掛/套件封存檔是含有 `package/` 根目錄的 zip 檔案，讓舊版 OpenClaw
  用戶端能持續運作。
- 此路由維持僅限 ZIP。它不串流 ClawPack `.tgz` 檔案。
...
回傳由 ClawPack 支援的套件版本之 npm 相容 packument。
...
- 具作用域的套件 packument 同時支援 `/api/npm/@scope/name` 和 npm 的
...
- 軟刪除的版本回傳 `410`。
- 由 GitHub 支援的技能交接不會代理或鏡像位元組。JSON 回應包含
  `sourceRef: "public-github"`、`repo`、`commit`、`path`、`contentHash`
  和 `archiveUrl`；掃描/目前狀態是閘門，不會作為成功酬載中繼資料包含在內。
- 下載統計會依 UTC 日期以唯一身分計算（API token 有效時為 `userId`，否則為 IP）。
...
- 也接受含有 `files`（以 storageId 為基礎）的 JSON body。
- 選用酬載欄位：`ownerHandle`。存在時，API 會在伺服器端解析該發布者，並要求動作者擁有發布者存取權。
- 選用酬載欄位：`migrateOwner`。當它與 `ownerHandle` 同時為 `true` 時，若動作者在目前與目標發布者上都是管理員/擁有者，現有技能可以移至該擁有者。沒有這個選擇加入時，擁有者變更會被拒絕。
...
- 允許的表單欄位為 `payload`、重複的 `files` blob，或一個 `clawpack`
  tarball 參照。`clawpack` 可以是 `.tgz` blob，或由 upload-url 流程回傳的 storage id。分段 storage-id 發布也必須包含該上傳 URL 一併回傳的
  `clawpackUploadTicket`。
- 使用 `files` 或 `clawpack` 其中之一，絕不可在同一請求中同時使用。
- JSON body 和呼叫者提供的 `payload.files` / `payload.artifact`
  中繼資料會被拒絕。
- 直接 multipart 發布請求上限為 18MB。ClawPack tarball 可使用 upload-url 流程，最高可達 120MB tarball 上限。
- 選用酬載欄位：`ownerHandle`。存在時，只有管理員可以代表該擁有者發布。
...
- 只有 `openclaw` org 發布者和目前 `openclaw` org 成員的個人發布者可以發布到 `official` 頻道。
- 代理發布仍會針對目標擁有者帳戶驗證 official 頻道資格。
...
僅限管理員。確保某個 handle 存在 org 發布者。如果該 handle 仍指向舊版共用使用者/個人發布者，端點會先將其遷移為 org 發布者。
對新建立的 org，請提供 `memberHandle`；執行操作的管理員不會被新增為成員。
...
已驗證使用者自助建立 org 發布者。建立新的 org 發布者並將呼叫者新增為擁有者。此端點不會遷移現有使用者/個人 handle，也不會將發布者標記為受信任/官方。
...
僅限管理員。為正當擁有者保留根 slug 和套件名稱，而不發布發行版。
套件名稱會成為沒有發行版資料列的私有預留位置套件，因此同一擁有者之後可以將真正的 code-plugin 或 bundle-plugin 發行版發布到該名稱。
...
僅限管理員。為已驗證的替換 GitHub OAuth principal 復原個人發布者，而不編輯 Convex Auth 帳戶資料列。請求必須命名兩個不可變的 GitHub provider account id；可變的 handle 只作為面向操作員的防護。

端點預設為 dry-run。套用復原需要在工作人員獨立驗證兩個
GitHub 主體之間的連續性後，設定 `dryRun: false` 與
`confirmIdentityVerified: true`。當目的地使用者目前的個人發布者已有 skills、packages 或 GitHub skill sources 時，復原會以失敗關閉。
復原也會遷移已復原發布者的 skills、skill slug aliases、packages、package inspector warnings，以及衍生 search digest rows 的舊版 `ownerUserId` 欄位，讓直接擁有者路徑與新的發布者權限一致。已復原 handle 的有效受保護 handle reservation 也會重新指派給替代使用者，讓後續
profile synchronization 無法還原前使用者的競爭權限。每個主要表格在每次 apply transaction 中限制為
100 筆；更大的復原必須先使用可續傳的 owner migration。
GitHub skill sources 是以發布者為範圍，並回報為已檢查，而不是被重寫。
...
- 兩個端點都需要 API token auth，且只對技能擁有者有效。
...
封鎖使用者並硬刪除其擁有的技能（僅限 moderator/admin）。
...
新增/移除星號（highlight）。兩個端點都是 idempotent。
...
`POST /api/cli/upload-url` 會回傳 `uploadUrl` 與 `uploadTicket`。暫存 ClawPack tarball 的 Package
publishes 必須將產生的 storage id 作為
`clawpack` 傳送，並將回傳的 ticket 作為 `clawpackUploadTicket` 傳送。

## Registry discovery (`/.well-known/clawhub.json`)

命令列介面可以從網站探索 registry/auth settings：
...
如果你 self-host，請提供這個檔案（或明確設定 `CLAWHUB_REGISTRY`；舊版為 `CLAWDHUB_REGISTRY`）。
