...
x-i18n:
    generated_at: "2026-07-01T20:10:42Z"
    model: gpt-5.5
...
第三方目錄可以使用公開讀取端點來列出或搜尋 ClawHub Skills。請快取結果、遵守 `429`/`Retry-After`、將使用者連回標準 ClawHub 列表（`https://clawhub.ai/<owner>/skills/<slug>`），並避免暗示 ClawHub 為該第三方網站背書。請勿嘗試在公開 API 介面之外鏡像隱藏、私人或遭審核封鎖的內容。

Web slug 快捷方式可跨註冊表系列解析，但 API 用戶端應使用讀取端點回傳的標準 URL，而不是自行重建路由優先順序。
...
- 當伺服器知道原因時，已驗證的寫入端點不應回傳空泛的 `Unauthorized`。遺失權杖、無效/已撤銷權杖，以及已刪除/遭禁用/已停用帳號，都應各自取得可操作的文字，讓命令列介面用戶端能告知使用者遭阻擋的原因。
...
- `X-RateLimit-Remaining` / `RateLimit-Remaining`：存在時表示精確剩餘額度。
...
- 只有在部署明確啟用受信任轉送標頭時，才會使用受信任用戶端 IP 標頭，包括 `cf-connecting-ip`。
- ClawHub 會使用受信任的轉送標頭在邊緣識別用戶端 IP。
- 如果沒有可用的受信任用戶端 IP，匿名請求會使用只依速率限制種類界定範圍的備援 bucket。這些備援 bucket 不包含呼叫者提供的路徑、slug、套件名稱、版本、查詢字串或其他成品參數。

## 錯誤回應

公開 v1 錯誤回應是純文字，並帶有 `content-type: text/plain; charset=utf-8`。
這包括驗證失敗（`400`）、缺少公開資源（`404`）、驗證與權限失敗（`401`/`403`）、速率限制（`429`），以及遭封鎖的下載。用戶端應將回應主體讀取為人類可讀字串。未知查詢參數會因相容性而被忽略，但已識別且值無效的查詢參數會回傳 `400`。
...
- 相關性比人氣更強。精確的 slug 或顯示名稱 token 相符，可以排在參與度高得多但相符較鬆散的結果之前。
- ASCII 文字會依單字和標點邊界進行 token 化。例如，`personal-map` 包含獨立的 `map` token，而 `amap-jsapi-skill` 包含 `amap`、`jsapi` 和 `skill`；因此搜尋 `map` 時，`personal-map` 會比 `amap-jsapi-skill` 取得更強的詞彙相符。
- 人氣會以對數縮放並設上限。當查詢文字相符較弱時，高參與度 Skills 仍可能排名較低。
- 可疑或隱藏的審核狀態，可能會依呼叫者篩選器與目前審核狀態，將某個 Skill 從公開搜尋中移除。

發布者可發現性指引：

- 將使用者會實際搜尋的詞放在顯示名稱、摘要和標籤中。只有在獨立 slug token 也是你想保留的穩定身分時，才使用它。
- 不要只為追逐某個查詢而重新命名 slug，除非新的 slug 是更好的長期標準名稱。舊 slug 會成為重新導向別名，但標準 URL、顯示的 slug，以及未來搜尋摘要會使用新 slug。
- 重新命名別名會保留舊 URL，以及透過註冊表解析之安裝的解析能力，但搜尋排名會依重新命名已建立索引後的標準 Skill 中繼資料決定。既有統計資料會保留在該 Skill 上。
...
- `createdAt` 對新 Skill 抓取而言是穩定的；`updated` 會在既有 Skills 重新發布時變更。
- 當 `nonSuspiciousOnly=true` 時，基於游標的排序可能會在某頁回傳少於 `limit` 個項目，因為可疑 Skills 會在頁面擷取後被篩除。
...
- 如果 Skill 沒有平台中繼資料，`metadata` 會是 `null`。
...
回報 Skill 以供審核員審查。回報屬於 Skill 層級，可選擇連結至某個版本，並會進入 Skill 回報佇列。
...
`note` 對 `confirmed` 和 `dismissed` 是必填；將 `status` 設回 `open` 時可省略。對已分流的回報傳入 `finalAction: "hide"`，即可在同一個可稽核工作流程中隱藏該 Skill。
...
- `version.security` 會在可用時包含正規化的掃描驗證狀態和掃描器詳細資料（VirusTotal + LLM）。
...
- 回傳已排隊/執行中/成功/失敗狀態。
- 排隊時會回傳 `queue.queuedAhead` 和 `queue.position`，讓用戶端可以顯示該請求前方有多少優先手動掃描。非常大的佇列會受限並以 `queuedAheadIsEstimate: true` 回報。
...
- 只有當所選版本具有已產生的 Skill Card、未因審核被惡意軟體封鎖，且 ClawScan 驗證為乾淨時，`ok` 才會是 `true`。
- Skill 身分、發布者身分與所選版本中繼資料是頂層信封欄位（`slug`、`displayName`、`publisherHandle`、`version`、`resolvedFrom`、`tag`、`createdAt`），因此 shell 自動化可以在不解開巢狀包裝的情況下讀取它們。
- `security` 是頂層 ClawScan/security 判定。自動化應以 `ok`、`decision`、`reasons` 和 `security.status` 為依據。
- `security.signals` 包含支援性的掃描器證據，例如 `staticScan`、`virusTotal` 和 `skillSpector`。
- `security.signals.dependencyRegistry` 會保留以維持 v1 回應相容性，但相依性登錄存在性掃描器已退役，且此鍵一律為 `null`。
...
- 結果依項目區分；單一缺少的 skill 或版本不會讓整個回應失敗。
- 回應僅包含安全性資訊。不包含 Skill Card 資料、已產生卡片狀態、成品檔案清單或詳細掃描器酬載。
- `security.signals` 只包含狀態層級的支援證據；若需完整掃描器詳細資訊，請使用 `/scan` 或 ClawHub security-audit 頁面。
- `security.signals.dependencyRegistry` 會保留以維持 v1 回應相容性，但相依性登錄存在性掃描器已退役，且此鍵一律為 `null`。
- Skill Card 缺席不會影響此端點的 `ok`、`decision` 或 `reasons`；用戶端需要卡片內容時，應在本機讀取已安裝的 `skill-card.md`。
- 需要單一 skill 的 Skill Card 驗證信封時請使用 `/verify`，需要已產生的卡片 Markdown 時請使用 `/card`，需要詳細掃描器資料時請使用 `/scan`。
...
- `category`（選填）：外掛類別篩選器。僅在請求範圍限定為外掛套件時支援（`/api/v1/plugins`、
  `/api/v1/code-plugins`、`/api/v1/bundle-plugins`，或帶有
  `family=code-plugin`/`family=bundle-plugin` 的套件端點）。受控類別與
  舊版 v1 篩選器別名記載於 `GET /api/v1/plugins` 下。
...
- Skills 項目仍由 Skills 登錄檔支援，且仍只能透過 `POST /api/v1/skills` 發布。
- `POST /api/v1/packages` 仍僅用於 code-plugin 與 bundle-plugin 發行。
- 匿名呼叫者只會看到公開套件頻道。
- 已驗證的呼叫者可以在清單/搜尋結果中看到其所屬發布者的私人套件。
...
- `channel`（選填）：`official`、`community` 或 `private`
- `isOfficial`（選填）：`true` 或 `false`
- `category`（選填）：外掛類別篩選器。僅在請求範圍限定為外掛套件時支援。受控類別與舊版 v1
  篩選器別名記載於 `GET /api/v1/plugins` 下。
...
- 目前由 GitHub 支援且掃描結果為 `clean` 或 `suspicious` 的 Skills，包含
  ...
  內容雜湊與封存 URL。它們不包含 ClawHub 託管的原始碼檔案。
...
- 需要套件擁有者、組織發布者擁有者/管理員、平台仲裁者或平台管理員的 API token。
...
傳回一個套件版本，包括檔案中繼資料、相容性、驗證、
成品中繼資料和掃描資料。

注意事項：

- 對於舊世界套件封存檔，`version.artifact.kind` 是 `legacy-zip`；
  對於 ClawPack 支援的版本，則是 `npm-pack`。
- ClawPack 版本包含與 npm 相容的 `npmIntegrity`、`npmShasum` 和
  `npmTarballName` 欄位。
- `version.sha256hash` 是給舊用戶端使用的已棄用相容性中繼資料。它會雜湊
...
消費介面，用於判斷已解析的版本是否可以安裝。

驗證：

- 公開讀取端點。不需要擁有者、發布者、仲裁者或管理員 token。
...
- `package.name`、`package.displayName` 和 `package.family` 會識別已解析的
  registry 套件。
- `release.releaseId`、`release.version` 和 `release.createdAt` 會識別
  已評估的確切版本。
- 當版本成品已知時，會提供 `release.artifactKind`、`release.artifactSha256`、`release.npmIntegrity`、
  `release.npmShasum` 和 `release.npmTarballName`。
- `trust.scanStatus` 是從掃描器輸入和手動版本仲裁衍生出的有效信任狀態。
- `trust.moderationState` 可為 null。當不存在手動版本仲裁時，它是 `null`。
- `trust.blockedFromDownload` 是安裝封鎖訊號。當此值為 `true` 時，OpenClaw 和其他
  安裝用戶端應封鎖安裝，而不是從掃描器或仲裁欄位重新推導封鎖規則。
...
- 此端點精確到版本。用戶端應在解析打算安裝的套件版本後呼叫它，
  而不只是讀取最新套件中繼資料後呼叫。
- 私有套件會傳回 `404`，除非呼叫者可以讀取擁有該套件的發布者。
- 此端點刻意比擁有者/仲裁者仲裁端點更窄。它公開安裝決策與公開說明，
  不公開回報者身分、回報內容、私人證據或內部審查時間軸。
...
- `packageName` 會以 npm-name 正規化；對於計畫中的
  遷移，套件可以不存在。
...
- `all`：任何具有手動覆寫、非 clean 掃描狀態或套件回報的版本。
...
回報套件以供仲裁者審查。回報是套件層級，可選擇性連結到版本。
它們會進入仲裁佇列，但本身不會自動隱藏或封鎖下載；
仲裁者應使用版本仲裁來核准、隔離或撤銷成品。
...
`note` 對 `confirmed` 和 `dismissed` 為必填；將 `status` 設回 `open` 時可以省略。傳入 `finalAction: "quarantine"` 或 `finalAction: "revoke"` 並搭配已確認的檢舉，可在同一個可稽核工作流程中套用發布版本審核。
...
隔離與撤銷的發布版本會讓成品下載路由回傳 `403`。每次變更都會寫入稽核記錄項目。
...
- 二進位檔案會回傳 `415`。
- 檔案大小限制：200KB。
- 待處理的 VirusTotal 掃描不會阻擋讀取；惡意發布版本仍可能在其他地方被 withheld。
...
- 舊版僅 ZIP 版本會刻意省略。
- `dist.tarball`、`dist.integrity` 和 `dist.shasum` 使用 npm 相容欄位，讓使用者可以視需要將 npm 指向該鏡像。
- 具範圍的套件 packument 同時支援 `/api/npm/@scope/name` 與 npm 的編碼 `/api/npm/@scope%2Fname` 請求路徑。
...
- `slug`（必填）
- `hash`（必填）：組合包指紋的 64 字元十六進位 sha256
...
下載託管的技能版本 ZIP，或針對目前由 GitHub 支援、掃描狀態為 `clean` 或 `suspicious` 且沒有託管版本的技能，回傳 GitHub 原始碼交接資訊。
...
- 軟刪除版本會回傳 `410`。
- GitHub 支援的技能交接不會代理或鏡像位元組。JSON 回應包含 `sourceRef: "public-github"`、`repo`、`commit`、`path`、`contentHash` 和 `archiveUrl`；掃描/目前狀態是閘門，不會作為成功酬載中繼資料包含在內。
- 下載統計會按 UTC 日計算唯一身分（API token 有效時使用 `userId`，否則使用 IP）。
...
- 選填酬載欄位：`ownerHandle`。存在時，API 會在伺服器端解析該發布者，並要求操作者具有發布者存取權。
- 選填酬載欄位：`migrateOwner`。當搭配 `ownerHandle` 設為 `true` 時，若操作者同時是目前與目標發布者的管理員/擁有者，既有技能可移至該擁有者。若未選擇加入，擁有者變更會被拒絕。
...
- 允許的表單欄位為 `payload`、重複的 `files` blob，或一個 `clawpack` tarball 參照。`clawpack` 可以是 `.tgz` blob，或由 upload-url 流程回傳的 storage id。使用暫存 storage-id 發布時，也必須包含該上傳 URL 一併回傳的 `clawpackUploadTicket`。
- 使用 `files` 或 `clawpack` 其一，不可在同一個請求中同時使用。
- JSON body 與呼叫者提供的 `payload.files` / `payload.artifact` 中繼資料會被拒絕。
...
- 程式碼外掛需要 `package.json`、原始碼 repo 中繼資料、原始碼 commit 中繼資料、config schema 中繼資料、`openclaw.compat.pluginApi` 和 `openclaw.build.openclawVersion`。
- `openclaw.hostTargets` 和 `openclaw.environment` 是選填中繼資料。
- 只有 `openclaw` org 發布者與目前 `openclaw` org 成員的個人發布者可以發布到 `official` channel。
...
擁有者發起的軟刪除會保留 slug 30 天，之後該 slug 可由其他發布者宣告。
刪除回應會在適用此到期時包含 `slugReservedUntil`。
...
僅限管理員。確保某個 handle 存在 org 發布者。如果該 handle 仍指向舊版共用使用者/個人發布者，端點會先將其遷移為 org 發布者。對於新建立的 org，請提供 `memberHandle`；執行操作的管理員不會被新增為成員。`memberRole` 預設為 `owner`。
...
已驗證的自助式 org 發布者建立。建立新的 org 發布者，並將呼叫者新增為擁有者。此端點不會遷移既有使用者/個人 handle，也不會將發布者標記為 trusted/official。
...
僅限管理員。為合法擁有者保留根 slug 與套件名稱，而不發布 release。套件名稱會成為沒有 release rows 的私有預留套件，因此同一位擁有者稍後可以將真正的 code-plugin 或 bundle-plugin release 發布到該名稱。
...
僅限管理員。為已驗證的替代 GitHub OAuth 主體復原個人發布者，而不編輯 Convex Auth 帳戶資料列。請求必須指定兩個不可變的 GitHub provider account ids；可變 handle 只會作為面向操作員的防護條件使用。

端點預設為試執行。套用復原需要在工作人員獨立驗證兩個
GitHub 主體之間的連續性後，設定 `dryRun: false` 與
`confirmIdentityVerified: true`。當目標使用者目前的個人發布者已有技能、套件或 GitHub 技能來源時，復原會以關閉狀態失敗。
復原也會遷移已復原發布者技能、技能 slug 別名、套件、套件檢查器警告，以及衍生搜尋摘要列中的舊版 `ownerUserId` 欄位，讓直接擁有者路徑與新的發布者權威一致。已復原 handle 的有效受保護 handle 保留也會重新指派給替代使用者，讓後續的個人資料同步無法還原前使用者的競爭權威。每個主要資料表在每次套用交易中上限為
100 列；更大的復原必須先使用可續傳的擁有者遷移。
...
`POST /api/cli/upload-url` 會回傳 `uploadUrl` 與 `uploadTicket`。暫存 ClawPack tarball 的套件
發布，必須將產生的儲存空間 ID 作為
