...
第三方目錄可以使用公開讀取端點來列出或搜尋 ClawHub Skills。請快取結果、遵守 `429`/`Retry-After`、將使用者連回標準 ClawHub 列表（`https://clawhub.ai/<owner>/skills/<slug>`），並避免暗示 ClawHub 認可該第三方網站。請勿嘗試在公開 API 表面之外鏡像隱藏、私人或因審核而封鎖的內容。
...
- 匿名請求：依 IP 執行限制。
- 已驗證請求（有效的 Bearer token）：依使用者桶執行限制。
- 如果權杖缺漏或無效，行為會退回 IP 執行限制。
- 已驗證的寫入端點在伺服器知道原因時，不應只傳回單純的 `Unauthorized`。缺漏權杖、無效或已撤銷的權杖，以及已刪除、遭停權或停用的帳號，都應取得可採取行動的文字，讓命令列介面用戶端能告知使用者是什麼阻擋了他們。
...
- 使用加入抖動的退避，避免同步重試。
- 如果缺少 `Retry-After`，請退回使用 `RateLimit-Reset`（或由 `X-RateLimit-Reset` 計算）。

IP 來源：

- 只有在部署明確啟用受信任的轉送標頭時，才會使用受信任的用戶端 IP 標頭，包括 `cf-connecting-ip`。
- ClawHub 使用受信任的轉送標頭，在邊緣識別用戶端 IP。
- 如果沒有可用的受信任用戶端 IP，匿名請求會使用只依速率限制種類界定範圍的備援桶。這些備援桶不包含呼叫端提供的路徑、slug、套件名稱、版本、查詢字串或其他成品參數。

## 錯誤回應

公開 v1 錯誤回應是純文字，並帶有 `content-type: text/plain; charset=utf-8`。
這包括驗證失敗（`400`）、缺少公開資源（`404`）、驗證與權限失敗（`401`/`403`）、速率限制（`429`）以及被封鎖的下載。用戶端應將回應本文讀作人類可讀的字串。未知的查詢參數會為了相容性而被忽略，但已辨識且值無效的查詢參數會傳回 `400`。
...
- 相關性比熱門度更重要。精確的 slug 或顯示名稱權杖相符，可能會排在參與度高得多但相符較鬆散的結果之前。
- ASCII 文字會依單字與標點邊界切分為權杖。例如，`personal-map` 包含獨立的 `map` 權杖，而 `amap-jsapi-skill` 包含 `amap`、`jsapi` 與 `skill`；因此搜尋 `map` 會讓 `personal-map` 比 `amap-jsapi-skill` 取得更強的詞彙相符。
- 熱門度採用對數縮放並設有上限。當查詢文字相符較弱時，高參與度 Skills 仍可能排名較低。
- 可疑或隱藏的審核狀態，可能會依呼叫端篩選器與目前審核狀態，將 Skill 從公開搜尋中移除。

發布者可發現性指引：

- 將使用者實際會搜尋的詞放在顯示名稱、摘要與標籤中。只有在獨立 slug 權杖也是你想保留的穩定身分時才使用它。
- 不要只是為了追逐某個查詢而重新命名 slug，除非新的 slug 是更好的長期標準名稱。舊 slug 會成為重新導向別名，但標準 URL、顯示的 slug 與未來搜尋摘要會使用新的 slug。
- 重新命名別名會保留舊 URL 與透過登錄解析之安裝的解析能力，但搜尋排名會以重新命名完成索引後的標準 Skill 中繼資料為準。既有統計資料會留在該 Skill 上。
...
- `createdAt` 對新 Skill 爬取來說是穩定的；`updated` 會在既有 Skills 重新發布時變更。
- 當 `nonSuspiciousOnly=true` 時，游標式排序可能會在頁面上傳回少於 `limit` 的項目，因為可疑 Skills 會在擷取頁面後被篩除。
- 當 `nextCursor` 存在時，使用它繼續分頁。短頁面本身不代表結果已結束。
...
- 公開呼叫端的證據會被遮蔽，且只有擁有者或審核員會包含原始片段。
...
`note` 對 `confirmed` 與 `dismissed` 是必要的；將 `status` 設回 `open` 時可以省略。傳入 `finalAction: "hide"` 搭配已分流的回報，以在同一個可稽核工作流程中隱藏該 Skill。
...
- 掃描作業是非同步的。手動掃描請求會優先於一般發布／回填工作，但完成時間仍取決於工作站可用性。
...
- 排隊時回傳 `queue.queuedAhead` 和 `queue.position`，讓用戶端能顯示有多少優先手動掃描排在該請求之前。非常大的佇列會被限制，並以 `queuedAheadIsEstimate: true` 回報。
...
- 需要該技能或外掛的擁有者／發布者管理存取權，或平台版主／管理員權限。
...
- 技能身分、發布者身分和所選版本中繼資料是頂層信封欄位（`slug`、`displayName`、`publisherHandle`、`version`、`resolvedFrom`、`tag`、`createdAt`），因此 Shell 自動化可以在不拆開巢狀包裝的情況下讀取它們。
...
- `security.signals.dependencyRegistry` 會保留以維持 v1 回應相容性，但依賴項登錄存在性掃描器已停用，且此鍵一律為 `null`。
...
- `category`（選用）：外掛類別篩選器。僅在請求範圍限定於外掛套件（`/api/v1/plugins`、`/api/v1/code-plugins`、`/api/v1/bundle-plugins`，或帶有 `family=code-plugin`/`family=bundle-plugin` 的套件端點）時支援。受控類別與舊版 v1 篩選器別名記錄於 `GET /api/v1/plugins` 下。
...
- `channel`（選用）：`official`、`community` 或 `private`
- `isOfficial`（選用）：`true` 或 `false`
- `category`（選用）：外掛類別篩選器。僅在請求範圍限定於外掛套件時支援。受控類別與舊版 v1 篩選器別名記錄於 `GET /api/v1/plugins` 下。
...
在統一 `/api/v1/packages` 端點上，它僅適用於外掛；技能目錄請使用
...
- 代管技能包含最新儲存版本檔案，並會列在 `_manifest.json` 中，帶有 `sourceRef: "public-clawhub"`。
- 目前由 GitHub 支援且掃描結果為 `clean` 或 `suspicious` 的技能，會包含 `_source_handoff.json`，其中帶有 `sourceRef: "public-github"`、repo、commit、path、內容雜湊與封存 URL。它們不包含 ClawHub 代管的原始碼檔案。
...
- 也接受 `GET /api/v1/plugins` 底下記錄的舊版 v1 篩選器別名。
...
- Skills 也可以透過統一型錄中的這個路由解析。
- 除非呼叫者可以讀取擁有該套件的發布者，否則私有套件會傳回 `404`。
...
- 對於舊世界套件封存檔，`version.artifact.kind` 為 `legacy-zip`；對於
  ClawPack 支援的版本，則為 `npm-pack`。
- ClawPack 版本包含 npm 相容的 `npmIntegrity`、`npmShasum` 與
  `npmTarballName` 欄位。
- `version.sha256hash` 是提供舊用戶端使用的已棄用相容性中繼資料。它會
  雜湊 `/api/v1/packages/{name}/download` 傳回的確切 ZIP 位元組。
  現代用戶端應使用 `version.artifact.sha256`，該欄位會識別
  標準版本成品。
...
傳回安裝用戶端所需的確切套件版本安全性與信任摘要。這是公開的 OpenClaw 使用介面，用於判斷已解析版本是否可以安裝。
...
- `trust.scanStatus` 是由掃描器輸入與手動版本審核
  推導出的有效信任狀態。
...
- `trust.stale` 表示信任摘要是由過期輸入計算而來，
  在做出高信心允許決策之前，應視為需要重新整理。

注意事項：

- 此端點精確對應版本。用戶端應在解析要安裝的
  套件版本後呼叫它，而不是只在讀取最新
  套件中繼資料後呼叫。
- 除非呼叫者可以讀取擁有該套件的發布者，否則私有套件會傳回 `404`。
- 此端點刻意比擁有者/審核員審核
  端點更窄。它公開安裝決策與公開說明，而不是
  回報者身分、回報內文、私有證據或內部審查
  時間軸。
...
- `packageName` 會經過 npm 名稱正規化；規劃中的
  遷移可缺少該套件。
...
傳入 `finalAction: "quarantine"` 或 `finalAction: "revoke"` 搭配已確認的檢舉，即可在同一個可稽核工作流程中套用版本審核。
...
- 二進位檔案會回傳 `415`。
- 檔案大小限制：200KB。
- 待處理的 VirusTotal 掃描不會封鎖讀取；惡意版本仍可能在其他地方被扣留。
...
- 舊版僅 ZIP 版本會刻意省略。
- `dist.tarball`、`dist.integrity` 和 `dist.shasum` 使用 npm 相容欄位，因此使用者可依選擇將 npm 指向此鏡像。
- 具範圍的套件 packument 支援 `/api/npm/@scope/name` 和 npm 編碼的 `/api/npm/@scope%2Fname` 請求路徑。
...
下載託管的 skill 版本 ZIP，或針對目前 GitHub 支援、掃描狀態為 `clean` 或 `suspicious` 且沒有託管版本的 skill，回傳 GitHub 原始碼交接資訊。
...
- 選用酬載欄位：`migrateOwner`。與 `ownerHandle` 一起為 `true` 時，如果動作者同時是目前與目標發布者的管理員/擁有者，現有 skill 可移至該擁有者。沒有此選擇加入時，擁有者變更會被拒絕。
...
- 允許的表單欄位為 `payload`、重複的 `files` blob，或一個 `clawpack` tarball 參照。`clawpack` 可以是 `.tgz` blob，或上傳 URL 流程回傳的儲存 id。使用暫存 storage-id 發布時，也必須包含該上傳 URL 一併回傳的 `clawpackUploadTicket`。
- 使用 `files` 或 `clawpack` 其中之一，切勿在同一請求中同時使用兩者。
- JSON 主體和呼叫端提供的 `payload.files` / `payload.artifact` 中繼資料會被拒絕。
...
擁有者發起的軟刪除會保留 slug 30 天，之後該 slug 可由其他發布者認領。
...
針對新建立的組織，請提供 `memberHandle`；動作管理員不會被加入為成員。
...
經認證的自助式組織發布者建立。建立新的組織發布者，並將呼叫端加入為擁有者。此端點不會遷移現有使用者/個人代號，也不會將發布者標記為受信任/官方。
...
僅限管理員。為合法擁有者保留根 slug 和套件名稱，而不發布版本。套件名稱會成為沒有版本列的私有預留位置套件，因此同一擁有者之後可將真正的 code-plugin 或 bundle-plugin 版本發布到該名稱。
...
僅限管理員。為已驗證的替代 GitHub OAuth 主體復原個人發布者，而不編輯 Convex Auth 帳號列。請求必須指名兩個不可變的 GitHub 提供者帳號 id；可變代號僅作為面向操作員的防護。

端點預設為試執行。套用復原需要在工作人員獨立驗證兩個
GitHub 主體之間的連續性後，設定 `dryRun: false` 和
`confirmIdentityVerified: true`。
當目的地使用者目前的個人發布者已有 skills、packages 或 GitHub skill 來源時，復原會以關閉狀態失敗。
復原也會遷移已復原發布者 skills、skill slug aliases、packages、package inspector warnings，以及衍生 search digest rows 的舊版 `ownerUserId` 欄位，讓直接擁有者路徑與新的發布者權限一致。已復原 handle 的有效受保護 handle 保留也會重新指派給替代使用者，因此後續個人檔案同步無法還原前使用者的競爭權限。每個主要資料表在每次套用交易中限制為
...
封鎖使用者並硬刪除其擁有的 skills（僅限主持人/管理員）。
