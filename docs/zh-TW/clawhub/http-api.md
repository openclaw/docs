...
x-i18n:
    generated_at: "2026-07-19T13:37:26Z"
    model: gpt-5.6
...
第三方目錄可以使用公開唯讀端點列出或搜尋 ClawHub Skills。請快取結果、遵守 `429`/`Retry-After`、將使用者連結回標準的 ClawHub 項目頁面（`https://clawhub.ai/<owner>/skills/<slug>`），並避免暗示 ClawHub 為該第三方網站背書。請勿嘗試在公開 API 範圍之外鏡像隱藏、私人或遭內容審核封鎖的內容。
...
- 匿名請求：依 IP 強制執行。
- 已驗證請求（有效的 Bearer 權杖）：依使用者配額桶強制執行。
- 若權杖缺少或無效，行為會退回依 IP 強制執行。
- 當伺服器知道原因時，需要驗證的寫入端點不應只傳回 `Unauthorized`。
  缺少權杖、權杖無效或已撤銷，以及帳號已刪除、遭封禁或停用，
  都應分別收到可採取行動的文字，讓命令列介面用戶端能告知使用者遭到封鎖的原因。
...
- 若缺少 `Retry-After`，請退回使用 `RateLimit-Reset`（或透過 `X-RateLimit-Reset` 計算）。

IP 來源：

- 只有部署明確啟用受信任的轉送標頭時，才會使用受信任的用戶端 IP 標頭，包括 `cf-connecting-ip`。
- ClawHub 會使用受信任的轉送標頭，在邊緣識別用戶端 IP。
- 若沒有可用的受信任用戶端 IP，匿名請求會使用僅依速率限制類型劃分範圍的備用配額桶。
  這些備用配額桶不包含呼叫端提供的路徑、slug、套件名稱、版本、查詢字串或其他成品參數。

## 錯誤回應

公開 v1 錯誤回應是使用 `content-type: text/plain; charset=utf-8` 的純文字。
其中包括驗證失敗（`400`）、缺少公開資源（`404`）、身分驗證與
權限失敗（`401`/`403`）、速率限制（`429`），以及遭封鎖的下載。用戶端
應將回應本文讀取為人類可讀的字串。為了相容性，未知的查詢參數會被忽略，
但已識別且值無效的查詢參數會傳回
`400`。
...
- 結果會依相關性排序傳回（嵌入相似度 + 精確的 slug／名稱詞元加權 + 小幅熱門度先驗）。
- 相關性的影響高於熱門度。精確符合 slug 或顯示名稱詞元的結果，排序可能高於互動量強得多但符合程度較低的結果。
...
- 熱門度會按對數縮放並設定上限。查詢文字符合度較低時，高互動量的 Skills 仍可能排名較後。
- 依呼叫端篩選條件與目前的內容審核狀態，可疑或隱藏的內容審核狀態可能會使 Skills 從公開搜尋中移除。

發布者可探索性指引：

- 將使用者會直接搜尋的詞彙放在顯示名稱、摘要和標籤中。只有當獨立的 slug 詞元同時也是你想保留的穩定識別名稱時，才使用該詞元。
- 除非新的 slug 是更適合長期使用的標準名稱，否則不要只為了迎合單一查詢而重新命名 slug。舊 slug 會成為重新導向別名，但標準 URL、顯示的 slug，以及未來的搜尋摘要都會使用新 slug。
- 重新命名別名會保留舊 URL，以及透過登錄檔解析的安裝作業之解析能力，但重新命名完成索引後，搜尋排名會以標準 Skills 中繼資料為準。現有統計資料會保留在該 Skills 上。
- 若 Skills 意外無法顯示，請先登入並使用 `clawhub inspect @owner/slug` 檢查內容審核狀態，再變更排名相關的中繼資料。
...
- `sort`（選用）：`updated`（預設值）、`recommended`（別名：`default`）、`createdAt`（別名：`newest`）、`downloads`、`stars`（別名：`rating`）；舊版安裝別名 `installsCurrent`/`installs`/`installsAllTime` 對應至 `downloads`、`trending`
- `nonSuspiciousOnly`（選用）：使用 `true` 隱藏可疑（`flagged.suspicious`）Skills
- `nonSuspicious`（選用）：`nonSuspiciousOnly` 的舊版別名
...
- `recommended` 使用互動量和時效性訊號。
- `trending` 依過去 7 天的安裝次數排名（以遙測資料為準）。
- `createdAt` 對新 Skills 的檢索作業而言是穩定的；重新發布現有 Skills 時，`updated` 會發生變化。
- 當 `nonSuspiciousOnly=true` 時，游標式排序可能會在某頁傳回少於 `limit` 個項目，因為可疑的 Skills 會在擷取頁面後才被篩除。
- 若存在 `nextCursor`，請使用它繼續分頁。頁面項目較少本身不代表結果已結束。
...
- 透過擁有者重新命名／合併流程建立的舊 slug，會解析至標準 Skills。
...
- 只有 Skills 已遭標記，或由擁有者檢視時，才會包含 `moderation`。
...
- 擁有者和內容審核員可以存取隱藏 Skills 的內容審核詳細資料。
- 公開呼叫端只有在可見 Skills 已遭標記時，才會取得 `200`。
- 公開呼叫端取得的證據會經過遮蔽，只有擁有者／內容審核員能取得原始片段。
...
報告傳入 `finalAction: "hide"`，即可在同一個可稽核工作流程中隱藏該 Skill。
...
- 保留期限結束後，掃描請求承載資料與可下載報告會從掃描請求儲存區到期移除。
...
- 掃描工作為非同步。手動掃描請求的優先順序高於一般發布／回填工作，但完成時間仍取決於工作者是否可用。
...
- 傳回已排入佇列／執行中／成功／失敗狀態。
- 排入佇列時傳回 `queue.queuedAhead` 和 `queue.position`，讓用戶端能顯示此請求前方有多少個優先手動掃描。非常大的佇列會受到上限限制，並以 `queuedAheadIsEstimate: true` 回報。
...
- 只有當所選版本已有產生的 Skill Card、未遭管理機制以惡意軟體為由封鎖，且 ClawScan 驗證結果為無風險時，`ok` 才會是 `true`。
- Skill 身分、發布者身分和所選版本中繼資料是頂層封套欄位（`slug`、`displayName`、`publisherHandle`、`version`、`resolvedFrom`、`tag`、`createdAt`），因此 Shell 自動化不必解開巢狀包裝即可讀取。
...
- `security.signals.dependencyRegistry` 會保留以維持 v1 回應相容性，但相依套件登錄存在性掃描器已停用，因此此鍵一律為 `null`。
- 只有當 ClawHub 在發布或匯入期間解析並儲存 GitHub 儲存庫／參照／提交／路徑時，`provenance` 才會是 `server-resolved-github-import`；否則為 `unavailable`。
...
- 回應僅包含安全性資料，不包含 Skill Card 資料、產生的卡片狀態、成品檔案清單或詳細的掃描器承載資料。
...
- `category`（選用）：外掛類別篩選器。僅在
  請求範圍限定於外掛套件時支援（`/api/v1/plugins`、
  `/api/v1/code-plugins`、`/api/v1/bundle-plugins`，或含有
  `family=code-plugin`/`family=bundle-plugin` 的套件端點）。受控類別和
...
- `POST /api/v1/packages` 仍僅用於程式碼外掛和套組外掛版本。
- 匿名呼叫者只能看到公開套件頻道。
- 已驗證的呼叫者可在清單／搜尋結果中看到其所屬發布者的私人套件。
...
- 匿名呼叫端只能看到公開套件頻道。
- 已驗證身分的呼叫端可以搜尋其所屬發布者的私人套件。
- `channel=private` 只會傳回已驗證身分的呼叫端可讀取的套件。

### `GET /api/v1/plugins`

瀏覽僅限外掛的目錄，涵蓋程式碼外掛與套件組合外掛套件。
...
`trending` 是七天期的安裝／下載排行榜，不使用全期累計總數。
...
- 託管的 Skills 包含最新儲存版本的檔案，並以
  `sourceRef: "public-clawhub"` 列於 `_manifest.json`。
- 目前由 GitHub 支援且具有 `clean` 或 `suspicious` 掃描的 Skills，會包含
  `_source_handoff.json`，其中含 `sourceRef: "public-github"`、儲存庫、提交、路徑、
  內容雜湊及封存檔 URL。它們不包含由 ClawHub 託管的原始檔案。
...
- `limit`（選填）：整數（1-250），預設為 `250`。
- `cursor`（選填）：前一個回應中的分頁游標。
- `family`（選填）：`code-plugin` 或 `bundle-plugin`。省略時表示兩種
  外掛系列。
...
- 同樣接受記載於 `GET /api/v1/plugins` 的舊版 v1 篩選器別名。
- 類別篩選是由外掛類別摘要資料列支援的實際 API 篩選器，
  而非搜尋查詢的改寫。
- 結果會依相關性順序傳回，目前不支援分頁。
- 瀏覽器 UI 的外掛搜尋排序控制項會重新排列已載入的相關性結果，
  與目前 `/skills` 的瀏覽行為一致。
...
- ClawPack 版本包含與 npm 相容的 `npmIntegrity`、`npmShasum` 和
  `npmTarballName` 欄位。
- `version.sha256hash` 是提供舊版用戶端使用的已淘汰相容性中繼資料。它會
  對 `/api/v1/packages/{name}/download` 傳回的確切 ZIP 位元組進行雜湊。
  現代用戶端應使用 `version.artifact.sha256`，其可識別
  正規版本成品。
- 存在掃描資料時，會包含 `version.vtAnalysis`、`version.llmAnalysis` 和 `version.staticScan`。
- 除非呼叫端可讀取所屬發布者，否則私人套件會傳回 `404`。

### `GET /api/v1/packages/{name}/versions/{version}/security`

傳回安裝用戶端所需的確切套件版本安全性與信任摘要。這是公開的 OpenClaw 使用介面，
用於判斷已解析的版本是否可安裝。
...
- `release.releaseId`、`release.version` 和 `release.createdAt` 用於識別
  接受評估的確切版本。
- 版本成品的 `release.artifactKind`、`release.artifactSha256`、`release.npmIntegrity`、
  `release.npmShasum` 和 `release.npmTarballName` 已知時會提供這些欄位。
- `trust.scanStatus` 是從掃描器輸入與手動版本審核推導出的有效信任狀態。
- `trust.moderationState` 可為 null。沒有手動版本審核時，其值為 `null`。
- `trust.blockedFromDownload` 是安裝封鎖訊號。此值為 `true` 時，OpenClaw 與其他
  安裝用戶端應封鎖安裝，而非根據掃描器或審核欄位重新推導封鎖規則。
- `trust.reasons` 是面向使用者及供稽核使用的說明清單。原因代碼
  是穩定而精簡的字串，例如 `manual:quarantined`、`scan:malicious`
  和 `package:malicious`。
- `trust.pending` 表示一或多個信任輸入仍在等待完成。
- `trust.stale` 表示信任摘要是根據過時的輸入計算，
  在做出高可信度的允許決策前，應視為需要重新整理。

注意事項：

- 此端點精確對應版本。用戶端應在解析出
  預定安裝的套件版本後呼叫它，而非只在讀取最新
  套件中繼資料後呼叫。
- 除非呼叫端可讀取所屬發布者，否則私人套件會傳回 `404`。
- 此端點刻意比擁有者／版主審核端點的範圍更窄。
  它會公開安裝決策及公開說明，但不會公開
...
- ClawPack 版本會傳回 `npm-pack` 成品、npm 完整性欄位、
  `tarballUrl` 及舊版 ZIP 相容性 URL。
- 這是 OpenClaw 的解析器介面；它可避免從共用 URL
  猜測封存格式。
...
- ClawPack 版本會串流所上傳 npm-pack 的確切 `.tgz` 位元組。
...
- `open`：可疑、惡意、待處理、已隔離、已撤銷或遭檢舉的發行版本。
...
連結至某個版本。檢舉會進入審核佇列，但本身不會自動隱藏套件或
封鎖下載；版主應使用發行審核功能來核准、隔離或撤銷成品。
...
`note` 是 `confirmed` 和 `dismissed` 的必要項目；將
`status` 設回 `open` 時可省略。針對已確認的檢舉傳入
...
- 二進位檔案會傳回 `415`。
- 檔案大小限制：200KB。
- 待處理的 VirusTotal 掃描不會封鎖讀取；惡意發行版本仍可能在其他位置遭到扣留。
- 除非呼叫端可讀取所屬發佈者，否則私有套件會傳回 `404`。
...
- 外掛／套件封存檔是具有 `package/` 根目錄的 ZIP 檔案，以確保舊版 OpenClaw
  用戶端仍可運作。
- 此路由僅保留 ZIP 格式，不會串流 ClawPack `.tgz` 檔案。
- 回應包含 `ETag`、`Digest`、`X-ClawHub-Artifact-Type` 和
  `X-ClawHub-Artifact-Sha256` 標頭，供解析器進行完整性檢查。
- 僅限登錄檔的中繼資料不會注入下載的封存檔。
- 待處理的 VirusTotal 掃描不會封鎖下載；惡意發行版本會傳回 `403`。
- 除非呼叫端是擁有者，否則私有套件會傳回 `404`。
...
- `dist.tarball`、`dist.integrity` 和 `dist.shasum` 使用 npm 相容
  欄位，因此使用者可視需要將 npm 指向該鏡像。
- 具範圍的套件 packument 同時支援 `/api/npm/@scope/name` 和 npm
  編碼的 `/api/npm/@scope%2Fname` 要求路徑。

### `GET /api/npm/{package}/-/{tarball}.tgz`

為 npm 鏡像用戶端串流所上傳 ClawPack tarball 的確切位元組。
...
下載代管的 Skill 版本 ZIP；若目前由 GitHub 支援的 Skill 具有
`clean` 或 `suspicious` 掃描且沒有代管版本，則傳回 GitHub 來源交接資訊。
...
- 建議：使用含 `payload` JSON 與 `files[]` 二進位大型物件的 `multipart/form-data`。
- 也接受包含 `files`（以 storageId 為基礎）的 JSON 主體。
- 選用承載資料欄位：`ownerHandle`。若存在，API 會在伺服器端解析該
  發布者，並要求執行者具有發布者存取權。
- 選用承載資料欄位：`migrateOwner`。當 `true` 與 `ownerHandle` 搭配時，
...
- 允許的表單欄位為 `payload`、重複的 `files` 二進位大型物件，或一個 `clawpack`
  tarball 參照。`clawpack` 可以是 `.tgz` 二進位大型物件，或上傳 URL 流程傳回的儲存 ID。
  使用暫存儲存 ID 發布時，還必須包含該上傳 URL 一併傳回的
...
- 選用承載資料欄位：`ownerHandle`。若存在，只有管理員能代表該擁有者發布。
...
- 只有 `openclaw` 組織發布者，以及目前 `openclaw` 組織成員的
  個人發布者，能發布至 `official` 頻道。
- 代表發布仍會根據目標擁有者帳戶驗證官方頻道資格。
...
```json
{ "reason": "因等候法律審查而暫留以供審核。" }
```

若存在，`reason` 會儲存為 Skill 審核備註，並複製至稽核記錄。
擁有者發起的軟刪除會保留代稱 30 天，之後其他發布者即可
取得該代稱。適用此到期規則時，刪除回應會包含 `slugReservedUntil`。
...
僅限管理員。確保某個使用者代號已有組織發布者。若該代號仍指向
...
呼叫者新增為擁有者。此端點不會移轉現有使用者／個人代號，也不會
將發布者標示為受信任／官方。
...
僅限管理員。為合法擁有者保留根代稱與套件名稱，而不發布
版本。套件名稱會成為沒有版本資料列的私人預留位置套件，因此相同
擁有者稍後可將真正的程式碼外掛或套件外掛版本發布至該名稱。
...
而不編輯 Convex Auth 帳戶資料列。請求必須指明兩個不可變的 GitHub
提供者帳戶 ID；可變的使用者代號僅用作面向操作人員的防護條件。

此端點預設為試執行。套用復原需要 `dryRun: false` 和
`confirmIdentityVerified: true`，且工作人員須獨立驗證兩個
GitHub 主體之間的連續性。當目的地使用者目前的個人
發布者具有 Skill、套件或 GitHub Skill 來源時，復原會採取失敗即關閉機制。
復原也會移轉已復原發布者之 Skill、Skill 代稱別名、套件、套件檢查器警告，
以及衍生搜尋摘要資料列中的舊版 `ownerUserId` 欄位，讓
直接擁有者路徑與新的發布者權限一致。已復原代號的有效受保護代號
保留項目也會重新指派給替代使用者，避免日後的
個人資料同步重新建立前使用者的競爭權限。每個主要資料表在每次套用交易中上限為
100 列；規模更大的復原必須先使用可續傳的擁有者移轉。
GitHub Skill 來源的範圍以發布者為準，會回報為已檢查，而不會重寫。
...
發布作業必須將產生的儲存 ID 以 `clawpack` 傳送，並將傳回的票證以
