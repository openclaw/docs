...
第三方目錄可以使用公開讀取端點來列出或搜尋 ClawHub Skills。請快取結果、遵守 `429`/`Retry-After`、將使用者連回標準 ClawHub 清單（`https://clawhub.ai/<owner>/skills/<slug>`），並避免暗示 ClawHub 為第三方網站背書。請勿嘗試在公開 API 介面之外鏡像隱藏、私人或因審核而封鎖的內容。
...
- 匿名請求：依 IP 執行。
- 已驗證請求（有效的 Bearer 權杖）：依使用者配額桶執行。
- 如果權杖遺失或無效，行為會退回 IP 執行。
- 已驗證的寫入端點在伺服器知道原因時，不應只傳回裸露的 `Unauthorized`。缺少權杖、無效或已撤銷的權杖，以及已刪除、已封鎖或已停用的帳戶，都應各自取得可採取行動的文字，讓命令列介面用戶端能告知使用者被阻擋的原因。

- 讀取：每個 IP 3000/分鐘，每個金鑰 12000/分鐘
- 寫入：每個 IP 300/分鐘，每個金鑰 3000/分鐘
- 下載：每個 IP 1200/分鐘，每個金鑰 6000/分鐘（下載端點）
...
- 如果沒有可用的受信任用戶端 IP，匿名請求會使用僅依速率限制種類劃分的備援配額桶。這些備援配額桶不包含呼叫者提供的路徑、slug、套件名稱、版本、查詢字串或其他成品參數。

## 錯誤回應

公開 v1 錯誤回應是純文字，並使用 `content-type: text/plain; charset=utf-8`。
這包括驗證失敗（`400`）、缺少公開資源（`404`）、驗證與權限失敗（`401`/`403`）、速率限制（`429`），以及被封鎖的下載。用戶端應將回應本文讀作人類可讀的字串。未知查詢參數會因相容性而被忽略，但已辨識且值無效的查詢參數會傳回 `400`。
...
- 相關性比熱門度更重要。精確的 slug 或顯示名稱詞元符合，可能會排在互動高得多但符合較鬆散的結果之前。
- ASCII 文字會依單字與標點邊界進行詞元化。例如，`personal-map` 包含獨立的 `map` 詞元，而 `amap-jsapi-skill` 包含 `amap`、`jsapi` 與 `skill`；因此搜尋 `map` 會讓 `personal-map` 比 `amap-jsapi-skill` 取得更強的詞彙符合。
- 熱門度會以對數縮放並設上限。當查詢文字符合較弱時，高互動 Skills 仍可能排名較低。
- 可疑或隱藏的審核狀態可能會依呼叫者篩選器與目前審核狀態，從公開搜尋中移除某個 Skill。

發布者可發現性指引：

- 將使用者實際會搜尋的詞放在顯示名稱、摘要與標籤中。只有當獨立 slug 詞元也是你想保留的穩定識別時，才使用它。
- 不要只為了追逐某個查詢而重新命名 slug，除非新的 slug 是更好的長期標準名稱。舊 slug 會成為重新導向別名，但標準 URL、顯示的 slug，以及未來搜尋摘要都會使用新的 slug。
- 重新命名別名會保留舊 URL 以及透過註冊表解析之安裝的解析能力，但搜尋排名會以重新命名編入索引後的標準 Skill 中繼資料為基準。既有統計資料會留在該 Skill 上。
- 如果某個 Skill 意外不可見，請先在已登入狀態下使用 `clawhub inspect @owner/slug` 檢查審核狀態，再變更與排名相關的中繼資料。
...
- 當 `nonSuspiciousOnly=true` 時，游標式排序可能會在頁面上傳回少於 `limit` 的項目，因為可疑 Skills 會在頁面擷取後被篩除。
...
- 由擁有者重新命名或合併流程建立的舊 slug，會解析為標準 Skill。
- `metadata.os`：Skill frontmatter 中宣告的作業系統限制（例如 `["macos"]`、`["linux"]`）。未宣告時為 `null`。
- `metadata.systems`：Nix 系統目標（例如 `["aarch64-darwin", "x86_64-linux"]`）。未宣告時為 `null`。
- 如果 Skill 沒有平台中繼資料，`metadata` 為 `null`。
- 只有當 Skill 被標記，或擁有者正在檢視它時，才會包含 `moderation`。
...
- 對公開呼叫者，證據會經過遮蔽，並且只有擁有者或審核員才會包含原始片段。

### `POST /api/v1/skills/{slug}/report`

回報某個 Skill 以供審核員檢閱。回報是 Skill 層級，可選擇連結到某個版本，並會送入 Skill 回報佇列。
...
供審核員或管理員使用，用於解決或重新開啟 Skill 回報的端點。
...
`note` 在 `confirmed` 與 `dismissed` 時為必要；將 `status` 設回 `open` 時可省略。傳入 `finalAction: "hide"` 搭配已分流的回報，可在同一個可稽核工作流程中隱藏該 Skill。
...
- 掃描請求酬載和可下載報告會在保留期間結束後，從掃描請求儲存區過期。
- 已發布的掃描需要擁有者/發布者管理存取權，或平台審核員/管理員權限。
- 已發布的掃描只有在 `update: true` 且掃描成功完成時才會寫回。
...
- 傳回已排隊/執行中/成功/失敗狀態。
- 排隊期間會傳回 `queue.queuedAhead` 和 `queue.position`，讓用戶端顯示請求前方有多少個優先手動掃描。非常大的佇列會受限並以 `queuedAheadIsEstimate: true` 回報。
- 可用時，`report` 會包含 `clawscan`、`skillspector`、`staticAnalysis` 和 `virustotal` 區段。
- 失敗的掃描作業會傳回 `status: "failed"` 並包含 `lastError`。
...
- 需要該 Skill 或外掛的擁有者/發布者管理存取權，或平台審核員/管理員權限。
...
- 只有當所選版本有產生的 Skill Card、未被審核機制以惡意軟體封鎖，且 ClawScan 驗證為乾淨時，`ok` 才會是 `true`。
- Skill 身分、發布者身分和所選版本中繼資料是頂層封套欄位（`slug`、`displayName`、`publisherHandle`、`version`、`resolvedFrom`、`tag`、`createdAt`），因此 shell 自動化可以讀取它們，而不需要拆開巢狀包裝。
...
- `security.signals.dependencyRegistry` 會保留以維持 v1 回應相容性，但相依性登錄存在性掃描器已停用，且此鍵一律為 `null`。
...
傳回精確 Skill 版本目前的精簡安全性判定。此集合端點適用於已知道需要顯示哪些已安裝 ClawHub Skill 版本的用戶端，例如 OpenClaw Control UI。
...
- 需要單一 Skill 的 Skill Card 驗證封套時請使用 `/verify`，需要已產生的卡片 Markdown 時請使用 `/card`，需要詳細掃描器資料時請使用 `/scan`。
...
- `category`（選填）：外掛類別篩選器。僅在請求範圍限定為外掛套件（`/api/v1/plugins`、`/api/v1/code-plugins`、`/api/v1/bundle-plugins`，或帶有 `family=code-plugin`/`family=bundle-plugin` 的套件端點）時支援。受控類別與舊版 v1 篩選器別名記載於 `GET /api/v1/plugins` 下。

注意事項：

- `family`、`channel`、`isOfficial`、`featured`、`highlightedOnly` 或 `sort` 的值無效時會傳回 `400`。未知的查詢參數會被忽略。
- `GET /api/v1/code-plugins` 和 `GET /api/v1/bundle-plugins` 仍維持為固定 family 的別名。
- Skills 項目仍由 skill registry 支援，且仍只能透過 `POST /api/v1/skills` 發佈。
- `POST /api/v1/packages` 仍僅用於 code-plugin 和 bundle-plugin 發佈。
...
- `channel`（選填）：`official`、`community` 或 `private`
- `isOfficial`（選填）：`true` 或 `false`
- `category`（選填）：外掛類別篩選器。僅在請求範圍限定為外掛套件時支援。受控類別與舊版 v1 篩選器別名記載於 `GET /api/v1/plugins` 下。
...
- 代管的 Skills 會包含最新儲存版本檔案，並在 `_manifest.json` 中以 `sourceRef: "public-clawhub"` 列出。
- 目前由 GitHub 支援且掃描結果為 `clean` 或 `suspicious` 的 Skills 會包含 `_source_handoff.json`，其中有 `sourceRef: "public-github"`、repo、commit、path、content hash，以及 archive URL。它們不包含 ClawHub 代管的原始碼檔案。
...
跨 code-plugin 和 bundle-plugin 套件的外掛專用搜尋。
...
- 也接受 `GET /api/v1/plugins` 下記載的舊版 v1 篩選器別名。
- 分類篩選是由外掛分類摘要資料列支援的真實 API 篩選器，
  不是搜尋查詢重寫。
- 結果會依相關性順序傳回，目前不分頁。
...
- `version.artifact.kind` 對舊世界套件封存檔為 `legacy-zip`，對 ClawPack 支援的發行版為
  `npm-pack`。
- ClawPack 發行版包含與 npm 相容的 `npmIntegrity`、`npmShasum` 和
  `npmTarballName` 欄位。
- `version.sha256hash` 是舊用戶端的已棄用相容性中繼資料。它會雜湊 `/api/v1/packages/{name}/download`
  傳回的確切 ZIP 位元組。現代用戶端應使用 `version.artifact.sha256`，其識別標準發行成品。
- 當掃描資料存在時，會包含 `version.vtAnalysis`、`version.llmAnalysis` 和 `version.staticScan`。
- 私有套件會傳回 `404`，除非呼叫者可讀取擁有該套件的發布者。

### `GET /api/v1/packages/{name}/versions/{version}/security`

傳回安裝用戶端所需的確切套件發行版安全性與信任摘要。這是 OpenClaw 用於判斷已解析發行版是否可安裝的公開消費介面。
...
- `package.name`、`package.displayName` 和 `package.family` 識別已解析的註冊表套件。
- `release.releaseId`、`release.version` 和 `release.createdAt` 識別被評估的確切發行版。
- 當發行成品已知時，會提供 `release.artifactKind`、`release.artifactSha256`、`release.npmIntegrity`、
  `release.npmShasum` 和 `release.npmTarballName`。
- `trust.scanStatus` 是從掃描器輸入和手動發行版審核衍生出的有效信任狀態。
- `trust.moderationState` 可為 null。當沒有手動發行版審核時為 `null`。
- `trust.blockedFromDownload` 是安裝封鎖訊號。OpenClaw 和其他安裝用戶端應在此值為 `true` 時封鎖安裝，
  而不是從掃描器或審核欄位重新推導封鎖規則。
...
- `trust.stale` 表示信任摘要是根據過期輸入計算而得，在做出高信心允許決策前應視為需要重新整理。

注意事項：

- 此端點精確到版本。用戶端應在解析出欲安裝的套件版本後呼叫它，而不只是讀取最新套件中繼資料後呼叫。
- 私有套件會傳回 `404`，除非呼叫者可讀取擁有該套件的發布者。
- 此端點刻意比擁有者/版主審核端點更窄。它公開安裝決策和公開說明，
  而不是舉報者身分、報告內容、私有證據或內部審查時間軸。
...
- ClawPack 版本會傳回 `npm-pack` 成品、npm 完整性欄位、`tarballUrl` 和舊版 ZIP 相容性 URL。
...
- `packageName` 會經 npm 名稱正規化；規劃中的遷移可以缺少套件。
...
- `open`：可疑、惡意、待處理、隔離、撤銷或已舉報的發行版。
- `blocked`：已隔離、撤銷或惡意的發行版。
- `manual`：任何具有手動審核覆寫的發行版。
- `all`：任何具有手動覆寫、非乾淨掃描狀態或套件舉報的發行版。
...
舉報套件以供版主審查。舉報屬於套件層級，可選擇連結到版本。它們會進入審核佇列，但本身不會自動隱藏或封鎖下載；版主應使用發行版審核來核准、隔離或撤銷成品。
...
`confirmed` 和 `dismissed` 必須提供 `note`；將 `status` 設回 `open` 時可省略。對已確認的回報傳入 `finalAction: "quarantine"` 或 `finalAction: "revoke"`，即可在同一個可稽核工作流程中套用版本審核。
...
- 外掛/套件封存檔是以 `package/` 為根目錄的 zip 檔，因此舊版 OpenClaw 用戶端可持續運作。
- 此路由維持僅支援 ZIP。不會串流 ClawPack `.tgz` 檔案。
- 回應包含 `ETag`、`Digest`、`X-ClawHub-Artifact-Type` 和 `X-ClawHub-Artifact-Sha256` 標頭，供解析器進行完整性檢查。
- 僅限登錄檔的中繼資料不會注入下載的封存檔中。
...
- 舊版僅 ZIP 版本會刻意省略。
- `dist.tarball`、`dist.integrity` 和 `dist.shasum` 使用 npm 相容欄位，讓使用者可選擇將 npm 指向該鏡像。
- 具範圍的套件 packument 同時支援 `/api/npm/@scope/name` 和 npm 的編碼請求路徑 `/api/npm/@scope%2Fname`。

### `GET /api/npm/{package}/-/{tarball}.tgz`

為 npm 鏡像用戶端串流已上傳 ClawPack tarball 的確切位元組。
...
下載託管的 skill 版本 ZIP，或針對目前由 GitHub 支援、掃描結果為 `clean` 或 `suspicious` 且沒有託管版本的 skill，傳回 GitHub 原始碼交接資訊。
...
- 軟刪除版本會傳回 `410`。
- GitHub 支援的 skill 交接不會代理或鏡像位元組。JSON 回應包含 `sourceRef: "public-github"`、`repo`、`commit`、`path`、`contentHash` 和 `archiveUrl`；掃描/目前狀態是門檻條件，不會包含為成功酬載中繼資料。
...
- 建議：使用含 `payload` JSON 與 `files[]` blob 的 `multipart/form-data`。
- 也接受含 `files`（基於 storageId）的 JSON 主體。
- 選填酬載欄位：`ownerHandle`。提供時，API 會在伺服器端解析該發布者，並要求行為者具備發布者存取權。
- 選填酬載欄位：`migrateOwner`。當 `ownerHandle` 同時為 `true` 時，若行為者同時是目前發布者和目標發布者的管理員/擁有者，既有 skill 可移至該擁有者。未明確選擇此項時，擁有者變更會被拒絕。
...
- 允許的表單欄位為 `payload`、重複的 `files` blob，或一個 `clawpack` tarball 參照。`clawpack` 可以是 `.tgz` blob，或上傳 URL 流程傳回的儲存 id。使用已暫存 storage-id 發布時，也必須包含該上傳 URL 一併傳回的 `clawpackUploadTicket`。
- 在同一請求中只能使用 `files` 或 `clawpack`，不可同時使用。
- JSON 主體和呼叫者提供的 `payload.files` / `payload.artifact` 中繼資料會被拒絕。
- 直接 multipart 發布請求上限為 18MB。ClawPack tarball 可使用上傳 URL 流程，最高可達 120MB tarball 上限。
...
- 程式碼外掛需要 `package.json`、原始碼倉庫中繼資料、原始碼提交中繼資料、設定結構描述中繼資料、`openclaw.compat.pluginApi` 和 `openclaw.build.openclawVersion`。
- `openclaw.hostTargets` 和 `openclaw.environment` 是選填中繼資料。
- 只有 `openclaw` 組織發布者和目前 `openclaw` 組織成員的個人發布者可以發布到 `official` 頻道。
- 代表他人發布仍會針對目標擁有者帳戶驗證官方頻道資格。
...
提供時，`reason` 會儲存為 skill 審核備註，並複製到稽核記錄。
由擁有者發起的軟刪除會保留 slug 30 天，之後該 slug 可由其他發布者宣告。
...
僅限管理員。確保指定代號存在組織發布者。若該代號仍指向舊版共用使用者/個人發布者，此端點會先將其遷移為組織發布者。
若為新建立的組織，請提供 `memberHandle`；執行操作的管理員不會被新增為成員。
...
已驗證的自助式組織發布者建立。建立新的組織發布者，並將呼叫者新增為擁有者。此端點不會遷移既有使用者/個人代號，也不會將發布者標記為受信任/官方。

- 主體：`{ "handle": "opik", "displayName": "Opik" }`
- 回應：`{ "ok": true, "publisherId": "...", "handle": "opik", "created": true, "trusted": false }`
- 當代號已被發布者、使用者或個人發布者使用時，會傳回 `409`。

### `POST /api/v1/users/reserve`

僅限管理員。為合法擁有者保留根 slug 和套件名稱，而不發布版本。套件名稱會成為沒有版本資料列的私有預留位置套件，因此同一擁有者之後可以將真正的 code-plugin 或 bundle-plugin 版本發布到該名稱。
...
僅限管理員。為已驗證的替代 GitHub OAuth 主體復原個人發布者，而不編輯 Convex Auth 帳戶資料列。請求必須指定兩個不可變的 GitHub provider account id；可變代號僅作為面向操作員的保護檢查。

端點預設為 dry-run。套用復原需要在工作人員獨立驗證兩個
GitHub 主體之間的連續性後，設定 `dryRun: false` 和
`confirmIdentityVerified: true`。當目標使用者目前的個人發布者有 skills、packages 或 GitHub skill sources 時，復原會以失敗關閉。
復原也會遷移已復原發布者的 skills、skill slug aliases、packages、package inspector warnings，以及衍生 search digest rows 的舊版 `ownerUserId` 欄位，讓直接擁有者路徑與新的發布者權限一致。已復原 handle 的啟用中受保護 handle reservation 也會重新指派給替代使用者，讓後續
profile synchronization 無法還原前使用者的競爭權限。每個主要資料表在每次套用交易中限制為
...
- `rename` 會保留先前的 slug 作為 redirect alias。
...
`POST /api/cli/upload-url` 會回傳 `uploadUrl` 和 `uploadTicket`。暫存 ClawPack tarball 的 Package
發布，必須將產生的 storage id 作為
`clawpack` 傳送，並將回傳的 ticket 作為 `clawpackUploadTicket` 傳送。

## 登錄探索（`/.well-known/clawhub.json`）

命令列介面可從網站探索 registry/auth 設定：
