...
x-i18n:
    generated_at: "2026-07-02T17:32:50Z"
    model: gpt-5.5
...
第三方目錄可以使用公開讀取端點來列出或搜尋 ClawHub Skills。請快取結果、遵守 `429`/`Retry-After`、將使用者連回標準 ClawHub 清單（`https://clawhub.ai/<owner>/skills/<slug>`），並避免暗示 ClawHub 為第三方網站背書。請勿嘗試在公開 API 表面之外鏡像隱藏、私人或被審核封鎖的內容。

網頁 slug 快捷方式會跨註冊表系列解析，但 API 用戶端應使用讀取端點傳回的標準 URL，而不是自行重建路由優先順序。
...
- 當伺服器知道原因時，已驗證寫入端點不應傳回單純的 `Unauthorized`。缺失 token、無效/已撤銷 token，以及已刪除/封禁/停用帳號，都應各自取得可執行的文字，讓命令列介面用戶端能告知使用者被封鎖的原因。
...
- `Retry-After`：發生 `429` 時重試前需等待的秒數（延遲）
...
- 如果缺少 `Retry-After`，請回退使用 `RateLimit-Reset`（或從 `X-RateLimit-Reset` 計算）。

IP 來源：

- 只有在部署明確啟用可信任轉送標頭時，才會使用可信任的用戶端 IP 標頭，包括 `cf-connecting-ip`。
- ClawHub 使用可信任轉送標頭在邊緣識別用戶端 IP。
- 如果沒有可用的可信任用戶端 IP，匿名請求會使用僅依速率限制類型劃分範圍的回退 bucket。這些回退 bucket 不包含呼叫者提供的路徑、slug、套件名稱、版本、查詢字串或其他成品參數。

## 錯誤回應

公開 v1 錯誤回應是純文字，`content-type: text/plain; charset=utf-8`。
這包括驗證失敗（`400`）、缺少公開資源（`404`）、驗證與權限失敗（`401`/`403`）、速率限制（`429`）以及被封鎖的下載。用戶端應將回應本文讀取為人類可讀字串。未知查詢參數會被忽略以維持相容性，但已識別且值無效的查詢參數會傳回 `400`。
...
- `q`（必要）：查詢字串
- `limit`（選用）：整數
- `highlightedOnly`（選用）：`true` 以篩選至醒目標示的 Skills
...
- 相關性比熱門度更強。精確的 slug 或顯示名稱 token 符合，可以排在互動更強但符合較鬆散的結果之前。
- ASCII 文字會依單字與標點邊界進行 tokenization。例如，`personal-map` 包含獨立的 `map` token，而 `amap-jsapi-skill` 包含 `amap`、`jsapi` 與 `skill`；因此搜尋 `map` 會讓 `personal-map` 比 `amap-jsapi-skill` 有更強的詞彙符合。
- 熱門度會以 log 縮放並設定上限。高互動 Skills 在查詢文字符合較弱時可能排名較低。
- 可疑或隱藏的審核狀態，可能會依呼叫者篩選條件與目前審核狀態，將 Skill 從公開搜尋中移除。

發布者可發現性指引：

- 將使用者會實際搜尋的詞放在顯示名稱、摘要與標籤中。只有當獨立 slug token 也是你想保留的穩定身分時，才使用它。
- 不要只為了追逐單一查詢而重新命名 slug，除非新 slug 是更好的長期標準名稱。舊 slug 會成為重新導向別名，但標準 URL、顯示的 slug 與未來搜尋摘要會使用新 slug。
- 重新命名別名會保留舊 URL 與透過註冊表解析安裝的解析能力，但搜尋排名會以重新命名完成索引後的標準 Skill metadata 為基礎。既有統計資料會保留在該 Skill 上。
- 如果 Skill 意外不可見，請先在已登入狀態下使用 `clawhub inspect @owner/slug` 檢查審核狀態，再變更排名相關 metadata。
...
- 當 `nonSuspiciousOnly=true` 時，游標式排序可能在某頁傳回少於 `limit` 的項目，因為可疑 Skills 會在擷取頁面後被篩除。
...
- Owner 與審核者可以存取隱藏 Skills 的審核詳細資料。
- 公開呼叫者只會對已標記且可見的 Skills 取得 `200`。
- 證據會對公開呼叫者遮蔽，且只對 owner/審核者包含原始片段。

### `POST /api/v1/skills/{slug}/report`

回報 Skill 以供審核者審查。回報是 Skill 層級，可選擇連結到某個版本，並會送入 Skill 回報佇列。
...
`note` 對 `confirmed` 與 `dismissed` 是必要的；將 `status` 設回 `open` 時可以省略。對已分類的回報傳入 `finalAction: "hide"`，即可在同一個可稽核工作流程中隱藏該 Skill。
...
- 掃描作業是非同步的。手動掃描請求會優先於一般發布／回填工作，但完成時間仍取決於工作者可用性。
...
- 排隊時會回傳 `queue.queuedAhead` 和 `queue.position`，讓用戶端顯示此請求前方有多少優先處理的手動掃描。非常大的佇列會受限並以 `queuedAheadIsEstimate: true` 回報。
- 可用時，`report` 會包含 `clawscan`、`skillspector`、`staticAnalysis` 和 `virustotal` 區段。
- 失敗的掃描作業會回傳帶有 `lastError` 的 `status: "failed"`。
...
僅限管理員的標準批次重新掃描路由。它接受與舊版 `POST /api/v1/skills/-/rescan-batch` 相同的負載形狀。
...
- 只有在所選版本具有已產生的技能卡、未被審核標記為惡意軟體封鎖，且 ClawScan 驗證為乾淨時，`ok` 才會是 `true`。
- 技能身分、發布者身分與所選版本中繼資料是頂層信封欄位（`slug`、`displayName`、`publisherHandle`、`version`、`resolvedFrom`、`tag`、`createdAt`），因此 shell 自動化可以在不拆解巢狀包裝器的情況下讀取它們。
...
- `security.signals.dependencyRegistry` 會保留以相容 v1 回應，但依賴項登錄存在性掃描器已退役，且此鍵一律為 `null`。
...
- 結果按項目回傳；單一缺少的技能或版本不會讓整個回應失敗。
- 回應僅包含安全性資訊。它不包含技能卡資料、已產生卡片狀態、成品檔案清單或詳細掃描器負載。
- `security.signals` 僅包含狀態層級的支援性證據；如需完整掃描器詳細資料，請使用 `/scan` 或 ClawHub 安全性稽核頁面。
- `security.signals.dependencyRegistry` 會保留以相容 v1 回應，但依賴項登錄存在性掃描器已退役，且此鍵一律為 `null`。
...
- `category`（選用）：外掛類別篩選器。僅在請求限定於外掛套件（`/api/v1/plugins`、
  ...
  舊版 v1 篩選器別名記錄於 `GET /api/v1/plugins`。

注意事項：

- `family`、`channel`、`isOfficial`、`featured`、`highlightedOnly` 或 `sort`
  的值無效時會回傳 `400`。未知的查詢參數會被忽略。
...
- `category`（選用）：外掛類別篩選器。僅在請求限定於外掛套件時支援。受控類別和舊版 v1
  篩選器別名記錄於 `GET /api/v1/plugins`。

注意事項：

- `family`、`channel`、`isOfficial`、`featured` 或
  `highlightedOnly` 的值無效時會回傳 `400`。未知的查詢參數會被忽略。
- 匿名呼叫者只能看到公開套件頻道。
...
在統一 `/api/v1/packages` 端點上它僅適用於外掛；skill 目錄請使用
...
- 託管 skills 包含最新儲存版本的檔案，並列於
  `_manifest.json`，且 `sourceRef: "public-clawhub"`。
- 目前以 GitHub 為後端且掃描結果為 `clean` 或 `suspicious` 的 skills 包含
  `_source_handoff.json`，其中含有 `sourceRef: "public-github"`、repo、commit、path、
  content hash 和 archive URL。它們不包含由 ClawHub 託管的來源檔案。
- 每個 skill 都包含 `_export_skill_meta.json`。
- `_manifest.json` 一定會包含在 ZIP 根目錄。
- `_errors.json` 會在個別 skills 或檔案無法匯出時包含。
...
- 類別篩選是真正的 API 篩選器，由外掛類別摘要列支援，而不是搜尋查詢改寫。
- 結果會依相關性順序傳回，且目前不分頁。
...
- 需要套件擁有者、組織發布者擁有者/管理員、平台審核者或平台管理員的 API 權杖。
...
- `version.artifact.kind` 對舊世界套件封存檔為 `legacy-zip`，對 ClawPack 支援的發布版本為
  `npm-pack`。
- ClawPack 發布版本包含 npm 相容的 `npmIntegrity`、`npmShasum` 與
  `npmTarballName` 欄位。
- `version.sha256hash` 是舊用戶端的已淘汰相容性中繼資料。它會雜湊 `/api/v1/packages/{name}/download` 傳回的確切 ZIP 位元組。
  現代用戶端應使用 `version.artifact.sha256`，它會識別標準發布成品。
- 當掃描資料存在時，會包含 `version.vtAnalysis`、`version.llmAnalysis` 與 `version.staticScan`。
- 私有套件會傳回 `404`，除非呼叫者可以讀取擁有該套件的發布者。

### `GET /api/v1/packages/{name}/versions/{version}/security`

傳回安裝用戶端所需的確切套件發布安全性與信任摘要。這是公開的 OpenClaw 消費介面，用於判斷已解析的發布版本是否可以安裝。

驗證：

- 公開讀取端點。不需要擁有者、發布者、審核者或管理員權杖。
...
- `release.releaseId`、`release.version` 與 `release.createdAt` 會識別被評估的確切發布版本。
- `release.artifactKind`、`release.artifactSha256`、`release.npmIntegrity`、
  `release.npmShasum` 與 `release.npmTarballName` 在發布成品已知時存在。
- `trust.scanStatus` 是從掃描器輸入與手動發布審核推導出的有效信任狀態。
- `trust.moderationState` 可為 Null。沒有手動發布審核時，它是 `null`。
...
- `trust.stale` 表示信任摘要是從過時輸入計算而來，且在做出高信心允許決策前應視為需要重新整理。

注意事項：

- 此端點精確到版本。用戶端應在解析出打算安裝的套件版本後呼叫它，而不只是讀取最新套件中繼資料後呼叫。
- 私有套件會傳回 `404`，除非呼叫者可以讀取擁有該套件的發布者。
- 此端點刻意比擁有者/審核者審核端點更窄。它公開安裝決策與公開說明，而不是回報者身分、回報本文、私有證據或內部審查時間軸。
...
- `packageName` 會依 npm 名稱正規化；套件在規劃中的遷移可以不存在。
...
回報套件以供審核者審查。回報是套件層級，可選擇連結到版本。它們會送入審核佇列，但本身不會自動隱藏或封鎖下載；審核者應使用發布審核來核准、隔離或撤銷成品。
...
在已確認的檢舉中傳入 `finalAction: "quarantine"` 或
`finalAction: "revoke"`，即可在同一個可稽核工作流程中套用發行版審核。
...
回傳以 ClawPack 為後端的套件版本之 npm 相容 packument。
...
下載託管的 skill 版本 ZIP，或針對具有 `clean` 或 `suspicious` 掃描結果且沒有託管
版本的目前 GitHub-backed skill，回傳 GitHub 原始碼交接資訊。
...
- 軟刪除版本會回傳 `410`。
- GitHub-backed skill 交接不會代理或鏡像位元組。JSON 回應
  包含 `sourceRef: "public-github"`、`repo`、`commit`、`path`、`contentHash`
  和 `archiveUrl`；掃描/目前狀態是門檻，不會作為成功
  payload 中繼資料包含在內。
- 下載統計會以每個 UTC 日的唯一身分計算（API token 有效時使用 `userId`，否則使用 IP）。
...
- 建議：使用 `multipart/form-data`，包含 `payload` JSON + `files[]` blobs。
...
- 選用 payload 欄位：`migrateOwner`。當它與 `ownerHandle` 一起為 `true` 時，
  若操作者同時是目前和目標發布者的管理員/擁有者，現有 skill 可移至該擁有者。
  未明確選擇此選項時，擁有者變更會被拒絕。
...
- 允許的表單欄位為 `payload`、重複的 `files` blobs，或一個 `clawpack`
  tarball 參照。`clawpack` 可以是 `.tgz` blob，或由
  upload-url 流程回傳的 storage id。使用 staged storage-id 發布時也必須包含
  該上傳 URL 一併回傳的 `clawpackUploadTicket`。
- 在同一請求中只能使用 `files` 或 `clawpack`，不可同時使用。
- JSON bodies 和呼叫者提供的 `payload.files` / `payload.artifact`
  中繼資料會被拒絕。
- 直接 multipart 發布請求上限為 18MB。ClawPack tarball 可
  使用 upload-url 流程，最高可達 120MB tarball 上限。
...
- Code plugins 需要 `package.json`、原始碼 repo 中繼資料、原始碼 commit
...
另一個發布者宣告使用。若適用此到期時間，刪除回應會包含 `slugReservedUntil`。
...
呼叫者新增為擁有者。此端點不會遷移現有使用者/個人 handle，也不會
將發布者標記為受信任/官方。
...
發行版。套件名稱會成為沒有發行版列的私有佔位套件，因此同一
擁有者稍後可將實際 code-plugin 或 bundle-plugin 發行版發布到該名稱。
...
而不編輯 Convex Auth 帳號列。請求必須指定兩個不可變的 GitHub
provider account ids；可變 handle 僅作為面向操作者的防護。

端點預設為 dry-run。套用復原需要在工作人員獨立驗證兩個
GitHub 主體之間的連續性後，設定 `dryRun: false` 和
`confirmIdentityVerified: true`。當目的地使用者目前的個人發布者已有 Skills、套件或 GitHub Skills 來源時，復原會以封閉失敗方式中止。
復原也會遷移已復原發布者的 Skills、skill slug 別名、套件、套件檢查器警告，以及衍生搜尋摘要列中的舊版 `ownerUserId` 欄位，使直接擁有者路徑與新的發布者權限一致。已復原控制代碼的作用中受保護控制代碼保留也會重新指派給替代使用者，讓後續設定檔同步無法還原前使用者的競爭權限。每個主要資料表在每次套用交易中上限為
100 列；較大的復原必須先使用可續接的擁有者遷移。
GitHub Skills 來源以發布者為範圍，並回報為已檢查，而不是改寫。
...
變更現有封鎖所儲存的原因，而不解除封鎖或還原內容（僅限管理員）。除非 `dryRun` 為 `false`，否則預設為 dry-run。
...
`POST /api/cli/upload-url` 會傳回 `uploadUrl` 和 `uploadTicket`。暫存 ClawPack tarball 的套件發布，必須將產生的儲存空間 id 作為 `clawpack` 傳送，並將傳回的票證作為 `clawpackUploadTicket` 傳送。
