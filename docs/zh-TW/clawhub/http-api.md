...
x-i18n:
    generated_at: "2026-07-03T17:12:30Z"
    model: gpt-5.5
...
第三方目錄可以使用公開讀取端點來列出或搜尋 ClawHub skills。請快取結果、遵守 `429`/`Retry-After`、將使用者連回標準 ClawHub 清單（`https://clawhub.ai/<owner>/skills/<slug>`），並避免暗示 ClawHub 認可該第三方網站。請勿嘗試在公開 API 介面之外鏡像隱藏、私有或遭審核封鎖的內容。

網頁 slug 捷徑會跨登錄家族解析，但 API 用戶端應使用讀取端點傳回的標準 URL，而不是重新建構路由優先順序。
...
- 匿名請求：依 IP 執行。
- 已驗證請求（有效 Bearer 權杖）：依使用者儲值桶執行。
- 如果權杖遺失/無效，行為會退回 IP 執行。
- 已驗證的寫入端點在伺服器知道原因時，不應傳回裸露的 `Unauthorized`。遺失權杖、無效/已撤銷權杖，以及已刪除/停權/停用的帳戶，都應取得可操作的文字，讓命令列介面用戶端能告訴使用者是什麼封鎖了他們。
...
- `X-RateLimit-Remaining` / `RateLimit-Remaining`：存在時為確切剩餘配額。
  分片成功請求會省略此標頭，而不是傳回近似的全域值。
- `Retry-After`：`429` 時重試前要等待的秒數（延遲）
...
- 如果沒有可用的受信任用戶端 IP，匿名請求會使用僅依速率限制種類劃分範圍的後備儲值桶。這些後備儲值桶不包含呼叫者提供的路徑、slug、套件名稱、版本、查詢字串或其他成品參數。

## 錯誤回應

公開 v1 錯誤回應是純文字，且 `content-type: text/plain; charset=utf-8`。
這包含驗證失敗（`400`）、遺失的公開資源（`404`）、驗證和權限失敗（`401`/`403`）、速率限制（`429`），以及被封鎖的下載。用戶端應將回應主體讀取為人類可讀的字串。未知查詢參數會為了相容性而被忽略，但已識別且值無效的查詢參數會傳回 `400`。
...
- `q`（必要）：查詢字串
- `limit`（選用）：整數
- `highlightedOnly`（選用）：`true` 表示篩選為醒目標示的 skills
...
- 相關性比熱門度更強。精確的 slug 或顯示名稱權杖相符，可能會勝過互動度高得多但相符較鬆散的結果。
- ASCII 文字會依單字和標點邊界切分為權杖。例如，`personal-map` 包含獨立的 `map` 權杖，而 `amap-jsapi-skill` 包含 `amap`、`jsapi` 和 `skill`；因此搜尋 `map` 會讓 `personal-map` 比 `amap-jsapi-skill` 有更強的詞彙相符。
- 熱門度會以對數縮放並設定上限。當查詢文字相符較弱時，高互動度 skills 可能排名較低。
- 可疑或隱藏的審核狀態可能會依呼叫者篩選條件和目前審核狀態，將某個 skill 從公開搜尋中移除。

發布者可發現性指引：

- 將使用者實際會搜尋的詞放在顯示名稱、摘要和標籤中。只有在獨立 slug 權杖也是你想保留的穩定身分時，才使用它。
- 不要只為了追逐某個查詢而重新命名 slug，除非新 slug 是更好的長期標準名稱。舊 slug 會變成重新導向別名，但標準 URL、顯示的 slug 和未來搜尋摘要會使用新 slug。
- 重新命名別名會保留舊 URL 和透過登錄解析之安裝的解析能力，但搜尋排名是基於重新命名完成索引後的標準 skill 中繼資料。既有統計資料會保留在該 skill 上。
...
- 存在時使用 `nextCursor` 繼續分頁。短頁面本身不代表結果已結束。
...
- 公開呼叫者看到的證據會經過遮蔽，且只有擁有者/審核員會包含原始片段。

### `POST /api/v1/skills/{slug}/report`

回報 skill 以供審核員檢閱。回報是 skill 層級，可選擇性連結到版本，並會送入 skill 回報佇列。
...
`confirmed` 和 `dismissed` 需要 `note`；將 `status` 設回 `open` 時可省略它。將 `finalAction: "hide"` 與已分流的回報一起傳入，可在同一個可稽核工作流程中隱藏該 skill。
...
- `version.security` 會在可用時包含正規化掃描驗證狀態和掃描器詳細資訊（VirusTotal + LLM）。
...
- 已發布掃描只會在 `update: true` 且掃描成功完成時寫回。
...
- 掃描工作是非同步的。手動掃描請求的優先順序高於一般發布/回填工作，但完成時間仍取決於工作程式可用性。
...
- 回傳已排隊/執行中/成功/失敗狀態。
- 排隊時回傳 `queue.queuedAhead` 和 `queue.position`，讓用戶端可以顯示該請求前方有多少個優先手動掃描。非常大的佇列會受限，並以 `queuedAheadIsEstimate: true` 回報。
- 可用時，`report` 會包含 `clawscan`、`skillspector`、`staticAnalysis` 和 `virustotal` 區段。
- 失敗的掃描工作會回傳 `status: "failed"` 並包含 `lastError`。
...
- 需要技能或外掛的擁有者/發布者管理存取權，或平台審核員/管理員權限。
- 回傳精確提交版本的已儲存掃描結果，包括已封鎖或隱藏版本。
...
- 只有在選取版本已產生技能卡、未被審核判定為惡意軟體而封鎖，且 ClawScan 驗證為乾淨時，`ok` 才會是 `true`。
- 技能身分、發布者身分和選取版本中繼資料是頂層信封欄位（`slug`、`displayName`、`publisherHandle`、`version`、`resolvedFrom`、`tag`、`createdAt`），因此 shell 自動化可以不必解開巢狀包裝器就讀取它們。
...
- `security.signals.dependencyRegistry` 會保留以維持 v1 回應相容性，但依賴項登錄存在性掃描器已停用，此鍵一律為 `null`。
...
- 結果依項目回傳；單一遺失的技能或版本不會讓整個回應失敗。
- 回應僅包含安全性資訊。不包含技能卡資料、已產生卡片狀態、成品檔案清單或詳細掃描器酬載。
- `security.signals` 僅包含狀態層級的支援證據；完整掃描器詳細資料請使用 `/scan` 或 ClawHub security-audit 頁面。
- `security.signals.dependencyRegistry` 會保留以維持 v1 回應相容性，但依賴項登錄存在性掃描器已停用，此鍵一律為 `null`。
...
- `category`（選填）：外掛類別篩選器。僅在要求限定於外掛套件（`/api/v1/plugins`、
...
- `channel`（選填）：`official`、`community` 或 `private`
- `isOfficial`（選填）：`true` 或 `false`
- `category`（選填）：外掛類別篩選器。僅在要求限定於外掛套件時支援。受控類別和舊版 v1
...
- `family`（選用）：`code-plugin` 或 `bundle-plugin`。省略代表兩種
  外掛家族都包含。
...
- Skills 也可透過統一目錄中的這個路由解析。
- 除非呼叫者可讀取擁有的發布者，否則私人套件會傳回 `404`。
...
- 對於舊世界套件封存檔，`version.artifact.kind` 為 `legacy-zip`；對於 ClawPack 支援的版本，則為
  `npm-pack`。
- ClawPack 版本包含 npm 相容的 `npmIntegrity`、`npmShasum` 與
  `npmTarballName` 欄位。
- `version.sha256hash` 是舊客戶端的已棄用相容性中繼資料。它會雜湊 `/api/v1/packages/{name}/download` 傳回的精確 ZIP 位元組。
  現代客戶端應使用 `version.artifact.sha256`，它會識別標準版本成品。
- 當掃描資料存在時，會包含 `version.vtAnalysis`、`version.llmAnalysis` 與 `version.staticScan`。
- 除非呼叫者可讀取擁有的發布者，否則私人套件會傳回 `404`。

### `GET /api/v1/packages/{name}/versions/{version}/security`

傳回安裝客戶端所需的精確套件版本安全性與信任摘要。這是公開的 OpenClaw 消費介面，用於決定已解析版本是否可安裝。
...
- `package.name`、`package.displayName` 與 `package.family` 用於識別已解析的登錄套件。
- `release.releaseId`、`release.version` 與 `release.createdAt` 用於識別已評估的精確版本。
- 當版本成品已知時，會提供 `release.artifactKind`、`release.artifactSha256`、`release.npmIntegrity`、
  `release.npmShasum` 與 `release.npmTarballName`。
- `trust.scanStatus` 是從掃描器輸入與手動版本審核推導出的有效信任狀態。
- `trust.moderationState` 可為 null。當沒有手動版本審核時，它會是 `null`。
- `trust.blockedFromDownload` 是安裝封鎖訊號。OpenClaw 和其他安裝客戶端應在此值為 `true` 時封鎖安裝，而不是從掃描器或審核欄位重新推導封鎖規則。
- `trust.reasons` 是面向使用者與稽核的說明清單。原因代碼是穩定且精簡的字串，例如 `manual:quarantined`、`scan:malicious`
  與 `package:malicious`。
- `trust.pending` 表示一個或多個信任輸入仍在等待完成。
- `trust.stale` 表示信任摘要是從過期輸入計算而來，在做出高信心允許決策前，應視為需要重新整理。

備註：

- 此端點精確到版本。客戶端應在解析出打算安裝的套件版本後呼叫它，而不只是讀取最新套件中繼資料後呼叫。
- 除非呼叫者可讀取擁有的發布者，否則私人套件會傳回 `404`。
- 此端點刻意比擁有者/審核者審核端點更窄。它公開安裝決策與公開說明，而不是回報者身分、回報本文、私人證據或內部審查時間軸。
...
回報套件以供審核者審查。回報屬於套件層級，可選擇連結到某個版本。它們會進入審核佇列，但本身不會自動隱藏或封鎖下載；審核者應使用版本審核來核准、隔離或撤銷成品。
...
`note` 在 `confirmed` 和 `dismissed` 時為必填；將 `status` 設回 `open` 時可省略。對已確認的檢舉傳入 `finalAction: "quarantine"` 或 `finalAction: "revoke"`，即可在同一個可稽核工作流程中套用發行版本審核。
...
隔離與撤銷的發行版本會從成品下載路由回傳 `403`。每次變更都會寫入一筆稽核日誌項目。
...
- 二進位檔案回傳 `415`。
- 檔案大小限制：200KB。
- 擱置中的 VirusTotal 掃描不會封鎖讀取；惡意發行版本仍可能在其他地方被保留不提供。
...
- 此路由僅維持 ZIP。它不串流 ClawPack `.tgz` 檔案。
- 回應包含 `ETag`、`Digest`、`X-ClawHub-Artifact-Type` 和 `X-ClawHub-Artifact-Sha256` 標頭，供解析器完整性檢查使用。
- 僅供登錄檔使用的中繼資料不會注入下載的封存。
- 擱置中的 VirusTotal 掃描不會封鎖下載；惡意發行版本會回傳 `403`。
- 私有套件會回傳 `404`，除非呼叫者是擁有者。

### `GET /api/npm/{package}`

回傳 ClawPack 支援之套件版本的 npm 相容 packument。

注意事項：

- 只會列出已上傳 ClawPack npm-pack tarball 的版本。
- 僅有舊版 ZIP 的版本會刻意省略。
- `dist.tarball`、`dist.integrity` 和 `dist.shasum` 使用 npm 相容欄位，因此使用者若選擇可將 npm 指向該鏡像。
...
- 軟刪除版本會回傳 `410`。
- GitHub 支援的 skill 交接不會代理或鏡像位元組。JSON 回應包含 `sourceRef: "public-github"`、`repo`、`commit`、`path`、`contentHash` 和 `archiveUrl`；掃描／目前狀態是門檻，不會作為成功承載中繼資料納入。
- 下載統計會以每個 UTC 日的唯一身分計數（API token 有效時使用 `userId`，否則使用 IP）。
...
- 建議：使用包含 `payload` JSON + `files[]` blob 的 `multipart/form-data`。
- 也接受包含 `files`（基於 storageId）的 JSON 主體。
- 選填承載欄位：`ownerHandle`。存在時，API 會在伺服器端解析該發布者，並要求執行者具有發布者存取權。
- 選填承載欄位：`migrateOwner`。當它與 `ownerHandle` 一起為 `true` 時，如果執行者同時是目前與目標發布者的管理員／擁有者，現有 skill 可移至該擁有者。若沒有此選擇加入，擁有者變更會被拒絕。
...
- 允許的表單欄位為 `payload`、重複的 `files` blob，或一個 `clawpack` tarball 參照。`clawpack` 可以是 `.tgz` blob，或由 upload-url 流程回傳的 storage id。暫存 storage-id 發布也必須包含該上傳 URL 回傳的 `clawpackUploadTicket`。
- 使用 `files` 或 `clawpack` 其中之一，切勿在同一請求中同時使用兩者。
- JSON 主體與呼叫者提供的 `payload.files` / `payload.artifact` 中繼資料會被拒絕。
- 直接 multipart 發布請求上限為 18MB。ClawPack tarball 可使用 upload-url 流程，最高至 120MB tarball 上限。
- 選填承載欄位：`ownerHandle`。存在時，只有管理員可代表該擁有者發布。

驗證重點：

- `family` 必須是 `code-plugin` 或 `bundle-plugin`。
- 外掛套件需要 `openclaw.plugin.json`。ClawPack `.tgz` 上傳必須在 `package/openclaw.plugin.json` 包含它。
- Code plugins 需要 `package.json`、原始碼儲存庫中繼資料、原始碼提交中繼資料、設定結構描述中繼資料、`openclaw.compat.pluginApi` 和 `openclaw.build.openclawVersion`。
- `openclaw.hostTargets` 和 `openclaw.environment` 是選填中繼資料。
- 只有 `openclaw` 組織發布者和目前 `openclaw` 組織成員的個人發布者可發布到 `official` 頻道。
- 代理發布仍會依目標擁有者帳號驗證 official-channel 資格。
...
擁有者發起的軟刪除會保留 slug 30 天，之後該 slug 可由另一個發布者宣告使用。刪除回應在此到期適用時會包含 `slugReservedUntil`。
...
僅限管理員。確保某個 handle 存在組織發布者。若該 handle 仍指向舊版共用使用者／個人發布者，端點會先將其遷移為組織發布者。
...
已驗證使用者可自助建立組織發布者。建立新的組織發布者並將呼叫者加入為擁有者。此端點不會遷移現有使用者／個人 handle，也不會將發布者標記為受信任／官方。

- 主體：`{ "handle": "opik", "displayName": "Opik" }`
- 回應：`{ "ok": true, "publisherId": "...", "handle": "opik", "created": true, "trusted": false }`
- 當 handle 已由發布者、使用者或個人發布者使用時回傳 `409`。

### `POST /api/v1/users/reserve`

僅限管理員。為合法擁有者保留根 slug 和套件名稱，而不發布發行版本。套件名稱會成為沒有發行列的私有預留位置套件，因此同一擁有者稍後可將真正的 code-plugin 或 bundle-plugin 發行版本發布到該名稱。
...
僅限管理員。為已驗證的替代 GitHub OAuth 主體復原個人發布者，而不編輯 Convex Auth 帳號列。請求必須指明兩個不可變的 GitHub provider account id；可變 handle 僅作為面向操作員的防護。

端點預設為 dry-run。套用復原需要在工作人員獨立驗證兩個
GitHub 主體之間的連續性後，設定 `dryRun: false` 與
`confirmIdentityVerified: true`。當目的地使用者目前的個人
發布者已有技能、套件或 GitHub 技能來源時，復原會以拒絕方式失敗。
復原也會遷移已復原發布者的技能、技能 slug 別名、套件、套件檢查器警告，以及衍生搜尋摘要列中的舊版 `ownerUserId` 欄位，讓
直接擁有者路徑與新的發布者權限一致。已復原 handle 的有效受保護 handle
保留項目也會重新指派給替代使用者，讓之後的
個人資料同步無法還原前使用者的競爭權限。每個主要資料表在每個套用交易中限制為
100 列；較大的復原必須先使用可續傳的擁有者遷移。
GitHub 技能來源以發布者為範圍，並回報為已檢查，而非改寫。
