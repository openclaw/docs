...
x-i18n:
    generated_at: "2026-07-03T00:51:29Z"
    model: gpt-5.5
...
舊版 `/api/...` 和 `/api/cli/...` 仍保留作為相容性用途（請參閱 `DEPRECATIONS.md`）。
OpenAPI：`/api/v1/openapi.json`。

## 公開目錄重用

第三方目錄可以使用公開讀取端點來列出或搜尋 ClawHub Skills。請快取結果、遵守 `429`/`Retry-After`、將使用者連回正式的 ClawHub 清單（`https://clawhub.ai/<owner>/skills/<slug>`），並避免暗示 ClawHub 為第三方網站背書。請勿嘗試在公開 API 介面之外鏡像隱藏、私人或被審核封鎖的內容。

Web slug 捷徑會跨註冊表系列解析，但 API 用戶端應使用讀取端點傳回的正式 URL，而不是自行重建路由優先順序。
...
- 匿名請求：依 IP 執行。
- 已驗證請求（有效的 Bearer token）：依使用者配額執行。
- 如果 token 遺失/無效，行為會退回 IP 執行。
- 已驗證的寫入端點在伺服器知道原因時，不應只傳回單純的 `Unauthorized`。遺失 token、無效/已撤銷 token，以及已刪除/被封鎖/已停用帳號都應取得可採取行動的文字，讓命令列介面用戶端可以告知使用者阻擋原因。
...
- `X-RateLimit-Remaining` / `RateLimit-Remaining`：存在時表示精確的剩餘配額。
...
- 如果沒有可用的受信任用戶端 IP，匿名請求會使用只依速率限制種類限定範圍的備用配額。這些備用配額不包含呼叫者提供的路徑、slug、套件名稱、版本、查詢字串或其他成品參數。

## 錯誤回應

公開 v1 錯誤回應是純文字，並帶有 `content-type: text/plain; charset=utf-8`。
這包括驗證失敗（`400`）、缺少公開資源（`404`）、驗證與權限失敗（`401`/`403`）、速率限制（`429`），以及被封鎖的下載。用戶端應將回應本文讀作人類可讀字串。未知查詢參數會為了相容性而被忽略，但已辨識的查詢參數若值無效則會傳回 `400`。
...
- 結果會依相關性排序傳回（嵌入相似度 + 精確 slug/名稱 token 加權 + 小幅熱門度先驗）。
- 相關性比熱門度更強。精確的 slug 或顯示名稱 token 相符，可以排在互動量高很多但相符較鬆散的結果之前。
- ASCII 文字會依單字與標點邊界切分 token。例如，`personal-map` 包含獨立的 `map` token，而 `amap-jsapi-skill` 包含 `amap`、`jsapi` 和 `skill`；因此搜尋 `map` 時，`personal-map` 會比 `amap-jsapi-skill` 有更強的詞彙相符度。
- 熱門度會以對數縮放並設上限。當查詢文字相符較弱時，高互動量 Skills 仍可能排名較低。
- 可疑或隱藏的審核狀態可能會根據呼叫者篩選條件與目前審核狀態，將某個 Skill 從公開搜尋中移除。

發布者可發現性指引：

- 將使用者會實際搜尋的詞放入顯示名稱、摘要和標籤。只有在獨立的 slug token 同時也是你想保留的穩定身分時才使用它。
- 除非新的 slug 是更好的長期正式名稱，否則不要只為了追逐某個查詢而重新命名 slug。舊 slug 會變成重新導向別名，但正式 URL、顯示的 slug 和未來搜尋摘要會使用新的 slug。
- 重新命名別名會保留舊 URL，以及透過註冊表解析的安裝解析能力，但搜尋排名會根據重新命名已索引後的正式 Skill 中繼資料。既有統計資料會留在該 Skill 上。
...
- `createdAt` 對新 Skill 爬取是穩定的；`updated` 會在既有 Skills 重新發布時改變。
- 當 `nonSuspiciousOnly=true` 時，因為可疑 Skills 會在頁面擷取後被篩掉，基於游標的排序在某一頁可能傳回少於 `limit` 的項目。
- 存在 `nextCursor` 時，使用它繼續分頁。頁面較短本身不代表已到結果結尾。
...
- 對公開呼叫者，證據會被遮蔽，且只有擁有者/審核員會包含原始片段。
...
`note` 對 `confirmed` 和 `dismissed` 是必填；將 `status` 設回 `open` 時可省略。傳入 `finalAction: "hide"` 搭配已分流處理的回報，可在同一個可稽核工作流程中隱藏 Skill。
...
- 回傳已佇列/執行中/成功/失敗狀態。
- 佇列中時會回傳 `queue.queuedAhead` 與 `queue.position`，讓用戶端可以顯示此請求前方有多少個優先手動掃描。非常大的佇列會受到上限限制，並以 `queuedAheadIsEstimate: true` 回報。
...
- 技能身分、發布者身分與所選版本中繼資料是頂層封套欄位（`slug`、`displayName`、`publisherHandle`、`version`、`resolvedFrom`、`tag`、`createdAt`），因此 shell 自動化可以在不解開巢狀包裝的情況下讀取它們。
...
- `security.signals.dependencyRegistry` 保留供 v1 回應相容性使用，但依賴項登錄存在性掃描器已退役，且此鍵一律為 `null`。
...
- 結果以項目為單位；缺少一個技能或版本不會導致整個回應失敗。
- 回應僅包含安全性資訊。它不包含技能卡資料、已產生卡片狀態、成品檔案清單或詳細掃描器酬載。
- `security.signals` 只包含狀態層級的支援證據；若需完整掃描器詳細資訊，請使用 `/scan` 或 ClawHub 安全性稽核頁面。
- `security.signals.dependencyRegistry` 保留供 v1 回應相容性使用，但依賴項登錄存在性掃描器已退役，且此鍵一律為 `null`。
...
- 目前由 GitHub 支援且掃描結果為 `clean` 或 `suspicious` 的 Skills，會包含
  ...
  內容雜湊和封存 URL。它們不包含 ClawHub 託管的來源檔案。
...
傳回安裝用戶端的精確套件版本安全性與信任摘要。
這是公開的 OpenClaw 消費介面，用於判斷已解析的版本是否可以安裝。
...
- `trust.scanStatus` 是由掃描器輸入
  與手動版本審核衍生出的有效信任狀態。
- `trust.moderationState` 可為 null。當不存在手動版本
  審核時，它為 `null`。
...
`confirmed` 和 `dismissed` 需要 `note`；將 `status` 設回 `open` 時可以省略。對已確認的檢舉傳入 `finalAction: "quarantine"` 或 `finalAction: "revoke"`，可在同一個可稽核工作流程中套用版本發布審核。
...
- `revoked`：先前受信任的版本發布後來被封鎖。

被隔離和撤銷的版本發布會從成品下載路由回傳 `403`。
每次變更都會寫入一筆稽核日誌項目。
...
- 二進位檔案會回傳 `415`。
- 檔案大小限制：200KB。
- 待處理的 VirusTotal 掃描不會封鎖讀取；惡意版本發布仍可能在其他位置被保留不提供。
- 私有套件會回傳 `404`，除非呼叫者可讀取擁有該套件的發布者。

### `GET /api/v1/packages/{name}/download`

下載套件版本發布的舊版決定性 ZIP 封存檔。
...
- GitHub 支援的技能交接不會代理或鏡像位元組。JSON 回應包含
  ...
  和 `archiveUrl`；掃描/目前狀態是閘門，不會作為成功承載中繼資料包含在內。
...
- 選用承載欄位：`ownerHandle`。出現時，API 會在伺服器端解析該發布者，並要求執行者具有發布者存取權。
- 選用承載欄位：`migrateOwner`。當它與 `ownerHandle` 一起為 `true` 時，若執行者在目前與目標發布者上皆為管理員/擁有者，既有技能可移至該擁有者。沒有此選擇加入時，擁有者變更會遭拒。

### `POST /api/v1/packages`

發布 code-plugin 或 bundle-plugin 版本發布。

- 需要 Bearer 權杖驗證。
- 需要 `multipart/form-data`。
- 允許的表單欄位為 `payload`、重複的 `files` blob，或一個 `clawpack`
  tarball 參照。`clawpack` 可以是 `.tgz` blob，或是上傳 URL 流程回傳的儲存 id。使用暫存 storage-id 發布時，也必須包含該上傳 URL 一併回傳的
  `clawpackUploadTicket`。
- 使用 `files` 或 `clawpack` 其一，絕不可在同一個請求中同時使用。
- JSON 主體以及呼叫者提供的 `payload.files` / `payload.artifact`
  中繼資料會遭拒。
- 直接 multipart 發布請求上限為 18MB。ClawPack tarball 可使用上傳 URL 流程，最高達 120MB tarball 上限。
- 選用承載欄位：`ownerHandle`。出現時，只有管理員可代表該擁有者發布。
...
出現時，`reason` 會儲存為技能審核備註，並複製到稽核日誌。
由擁有者發起的軟刪除會保留 slug 30 天，之後該 slug 可由
其他發布者宣告使用。刪除回應會在適用此到期時包含 `slugReservedUntil`。
版主/管理員隱藏與安全性移除不會以此方式到期。
...
對於新建立的組織，提供 `memberHandle`；執行中的管理員不會被加入為成員。
...
已驗證使用者自助建立組織發布者。建立新的組織發布者，並將呼叫者加入為擁有者。此端點不會遷移既有使用者/個人 handle，也不會將發布者標示為受信任/官方。
...
僅限管理員。為正當擁有者保留根 slug 和套件名稱，而不發布版本發布。套件名稱會成為沒有版本發布列的私有佔位套件，因此同一擁有者稍後可將真正的 code-plugin 或 bundle-plugin 版本發布到該名稱。
...
而不編輯 Convex Auth 帳號列。請求必須指明兩個不可變的 GitHub
提供者帳號 id；可變 handle 只作為面向操作員的防護。

端點預設為 dry-run。套用復原需要在工作人員獨立驗證兩個
GitHub 主體之間的連續性後，設定 `dryRun: false` 和
`confirmIdentityVerified: true`。當目的地使用者目前的個人發布者擁有技能、套件或 GitHub 技能來源時，復原會以關閉狀態失敗。
復原也會遷移已復原發布者技能、技能 slug 別名、套件、套件檢查器警告，以及衍生搜尋摘要資料列的舊版 `ownerUserId` 欄位，讓直接擁有者路徑與新的發布者授權一致。已復原 handle 的有效受保護 handle
保留也會重新指派給替代使用者，讓後續的個人資料同步無法還原前使用者的競爭授權。每個主要資料表在每次套用交易中限制為
