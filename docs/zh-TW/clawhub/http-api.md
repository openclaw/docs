...
第三方目錄可以使用公開讀取端點列出或搜尋 ClawHub skills。請快取結果、遵守 `429`/`Retry-After`、將使用者連回標準 ClawHub 清單（`https://clawhub.ai/<owner>/skills/<slug>`），並避免暗示 ClawHub 認可該第三方網站。請勿嘗試在公開 API 介面之外鏡像隱藏、私人或被審核封鎖的內容。

Web slug 快捷方式會跨登錄檔家族解析，但 API 用戶端應使用讀取端點傳回的標準 URL，而不是自行重建路由優先順序。
...
- 已驗證的寫入端點在伺服器知道原因時，不應只傳回裸露的 `Unauthorized`。遺失 token、無效/已撤銷 token，以及已刪除/已封鎖/已停用帳號，都應取得可行動的文字，讓命令列介面用戶端能告訴使用者是什麼阻擋了他們。
...
- 使用帶抖動的退避，以避免同步重試。
- 如果 `Retry-After` 遺失，退回使用 `RateLimit-Reset`（或由 `X-RateLimit-Reset` 計算）。

IP 來源：

- 只有在部署明確啟用受信任的轉送標頭時，才會使用受信任的用戶端 IP 標頭，包括 `cf-connecting-ip`。
- ClawHub 會使用受信任的轉送標頭，在邊緣識別用戶端 IP。
- 如果沒有可用的受信任用戶端 IP，匿名請求會使用只按速率限制種類界定範圍的 fallback bucket。這些 fallback bucket 不包含呼叫者提供的路徑、slug、套件名稱、版本、查詢字串或其他成品參數。

## 錯誤回應

公開 v1 錯誤回應是純文字，並帶有 `content-type: text/plain; charset=utf-8`。
這包括驗證失敗（`400`）、遺失公開資源（`404`）、驗證與權限失敗（`401`/`403`）、速率限制（`429`），以及被封鎖的下載。用戶端應將回應主體讀作人類可讀字串。未知查詢參數會因相容性而被忽略，但可辨識的查詢參數若值無效，則會傳回 `400`。
...
- 結果會依相關性順序傳回（嵌入相似度 + 精確 slug/name token 加權 + 小幅熱門度先驗）。
- 相關性比熱門度更強。精確的 slug 或顯示名稱 token 符合，可能會勝過互動度高得多但匹配較寬鬆的結果。
- ASCII 文字會依單字與標點邊界斷詞。例如，`personal-map` 包含獨立的 `map` token，而 `amap-jsapi-skill` 包含 `amap`、`jsapi` 和 `skill`；因此搜尋 `map` 會讓 `personal-map` 取得比 `amap-jsapi-skill` 更強的詞彙匹配。
- 熱門度會以對數縮放並設上限。高互動度 skills 在查詢文字匹配較弱時，排名可能較低。
- 可疑或隱藏的審核狀態可能會依呼叫者篩選條件與目前審核狀態，將 skill 從公開搜尋中移除。

發布者可探索性指引：

- 將使用者會實際搜尋的詞彙放在顯示名稱、摘要與標籤中。只有當獨立 slug token 也是你想保留的穩定識別時，才使用它。
- 不要只為了追逐某個查詢而重新命名 slug，除非新 slug 是更好的長期標準名稱。舊 slug 會成為重新導向別名，但標準 URL、顯示的 slug 與未來搜尋摘要會使用新 slug。
- 重新命名別名會保留舊 URL 與透過登錄檔解析的安裝解析能力，但搜尋排名會以重新命名已建立索引後的標準 skill 中繼資料為準。既有統計會留在該 skill 上。
...
- 當 `nonSuspiciousOnly=true` 時，因為可疑 skills 會在頁面擷取後被篩除，以游標為基礎的排序可能在某頁傳回少於 `limit` 的項目。
- 存在 `nextCursor` 時，請使用它繼續分頁。短頁本身不代表結果結束。
...
- 公開呼叫者的證據會被遮蔽，且只對擁有者/審核員包含原始片段。

### `POST /api/v1/skills/{slug}/report`

回報 skill 供審核員審查。回報是 skill 層級，可選擇連結至版本，並會送入 skill 回報佇列。
...
`confirmed` 與 `dismissed` 需要 `note`；將 `status` 設回 `open` 時可省略。傳入 `finalAction: "hide"` 搭配已分流的回報，以在同一個可稽核工作流程中隱藏該 skill。
...
- 掃描請求承載資料和可下載報告會在保留期限後從掃描請求儲存中過期。
...
- 回傳已排入佇列/執行中/成功/失敗狀態。
- 排入佇列期間會回傳 `queue.queuedAhead` 和 `queue.position`，讓用戶端可以顯示請求前方有多少個優先手動掃描。非常大的佇列會受上限限制，並以 `queuedAheadIsEstimate: true` 回報。
- 可用時，`report` 會包含 `clawscan`、`skillspector`、`staticAnalysis` 和 `virustotal` 區段。
- 失敗的掃描作業會以 `lastError` 回傳 `status: "failed"`。
...
- 需要該 skill 或外掛的擁有者/發布者管理存取權，或平台版主/管理員權限。
...
- 只有當所選版本已有產生的 Skill Card、未被審核以惡意軟體封鎖，且 ClawScan 驗證為乾淨時，`ok` 才會是 `true`。
- Skill 身分、發布者身分和所選版本中繼資料是頂層信封欄位（`slug`、`displayName`、`publisherHandle`、`version`、`resolvedFrom`、`tag`、`createdAt`），因此 shell 自動化可以讀取它們，而不必解開巢狀包裝器。
- `security` 是頂層 ClawScan/安全性判定。自動化應以 `ok`、`decision`、`reasons` 和 `security.status` 作為依據。
- `security.signals` 包含輔助掃描器證據，例如 `staticScan`、`virusTotal` 和 `skillSpector`。
...
- 結果按項目回傳；缺少某個 skill 或版本不會讓整個回應失敗。
- 回應僅包含安全性資訊。不包含 Skill Card 資料、已產生卡片狀態、成品檔案清單，或詳細掃描器承載資料。
- `security.signals` 只包含狀態層級輔助證據；如需完整掃描器詳細資訊，請使用 `/scan` 或 ClawHub security-audit 頁面。
- `security.signals.dependencyRegistry` 保留用於 v1 回應相容性，但依賴項登錄存在性掃描器已退役，且此鍵一律為 `null`。
- 缺少 Skill Card 不會影響此端點的 `ok`、`decision` 或 `reasons`；當用戶端需要卡片內容時，應在本機讀取已安裝的 `skill-card.md`。
- 需要單一 skill Skill Card 驗證信封時請使用 `/verify`，需要產生的卡片 Markdown 時請使用 `/card`，需要詳細掃描器資料時請使用 `/scan`。
...
- `category`（選填）：外掛分類篩選器。僅在要求範圍限定為外掛套件（`/api/v1/plugins`、
...
- Skills 項目仍由 skill 登錄檔支援，且仍只能透過 `POST /api/v1/skills` 發布。
- `POST /api/v1/packages` 仍僅用於 code-plugin 和 bundle-plugin 發布。
- 匿名呼叫者只會看到公開套件渠道。
...
- `channel`（選填）：`official`、`community` 或 `private`
- `isOfficial`（選填）：`true` 或 `false`
- `category`（選填）：外掛分類篩選器。僅在要求範圍限定為外掛套件時支援。受控分類和舊版 v1
...
`trending` 是七天安裝/下載排行榜，不使用累計總數。
在統一的 `/api/v1/packages` 端點上，它僅限外掛；請使用
`/api/v1/skills?sort=trending` 查看 skill 目錄。

舊版別名不會被接受為儲存或作者宣告的分類值。

### `GET /api/v1/skills/export`

最新公開 Skills 的大量匯出，用於離線分析。
...
- 目前由 GitHub 支援且掃描結果為 `clean` 或 `suspicious` 的 Skills 包含
  ...
  content hash 和 archive URL。它們不包含 ClawHub 代管的原始檔案。
...
- ClawPack 版本包含與 npm 相容的 `npmIntegrity`、`npmShasum` 與
  `npmTarballName` 欄位。
- `version.sha256hash` 是給舊用戶端使用的已棄用相容性中繼資料。它會
...
傳回供安裝用戶端使用的確切套件版本安全性與信任摘要。這是公開的
OpenClaw 消費介面，用於判斷已解析的版本是否可以安裝。
...
- `trust.scanStatus` 是由掃描器輸入與手動版本審核衍生出的有效信任狀態。
...
- `trust.stale` 表示信任摘要是以過時輸入計算而成，
  在做出高信心允許決策前，應視為需要重新整理。

注意事項：

- 此端點精確對應版本。用戶端應在解析出想要安裝的
  套件版本後呼叫它，而不只是讀取最新
  套件中繼資料之後。
- 私有套件會傳回 `404`，除非呼叫者可以讀取擁有者發布者。
- 此端點刻意比擁有者/版主審核端點更窄。它公開的是安裝決策與公開說明，
  而不是回報者身分、回報本文、私有證據或內部審查
  時程。
...
- `packageName` 會正規化為 npm 名稱；規劃中的遷移可以沒有套件。
...
回報套件以供版主審查。回報屬於套件層級，可選擇
連結到某個版本。它們會送入審核佇列，但本身不會自動隱藏或
封鎖下載；版主應使用版本審核來
核准、隔離或撤銷成品。
...
`note` 在 `confirmed` 和 `dismissed` 時為必填；將 `status` 設回 `open` 時可以省略。對已確認的回報傳入 `finalAction: "quarantine"` 或 `finalAction: "revoke"`，即可在同一個可稽核工作流程中套用發行版審核。
...
- `revoked`：先前受信任的發行版之後遭到封鎖。

已隔離和已撤銷的發行版會在成品下載路由回傳 `403`。每次變更都會寫入稽核記錄項目。
...
- 二進位檔案回傳 `415`。
- 檔案大小限制：200KB。
- 待處理的 VirusTotal 掃描不會封鎖讀取；惡意發行版仍可能在其他地方被扣留。
...
- 外掛/套件封存檔是具有 `package/` 根目錄的 zip 檔案，因此舊版 OpenClaw 用戶端可持續運作。
...
- 舊版僅 ZIP 的版本會刻意省略。
- `dist.tarball`、`dist.integrity` 和 `dist.shasum` 使用 npm 相容欄位，因此使用者若選擇，可以將 npm 指向該鏡像。
- 具範圍的套件 packument 支援 `/api/npm/@scope/name` 以及 npm 的編碼請求路徑 `/api/npm/@scope%2Fname`。
...
下載託管的 skill 版本 ZIP，或針對目前由 GitHub 支援、掃描為 `clean` 或 `suspicious` 且沒有託管版本的 skill，回傳 GitHub 來源交接資訊。
...
- 軟刪除的版本會回傳 `410`。
- GitHub 支援的 skill 交接不會代理或鏡像位元組。JSON 回應包含 `sourceRef: "public-github"`、`repo`、`commit`、`path`、`contentHash` 和 `archiveUrl`；掃描/目前狀態是閘門，不會作為成功酬載中繼資料包含。
...
- 也接受含 `files`（基於 storageId）的 JSON body。
- 選用酬載欄位：`ownerHandle`。存在時，API 會在伺服器端解析該發布者，並要求執行者具有發布者存取權。
- 選用酬載欄位：`migrateOwner`。當與 `ownerHandle` 一起設為 `true` 時，如果執行者同時是目前與目標發布者的管理員/擁有者，既有 skill 可以移動到該擁有者。若沒有此選擇加入，擁有者變更會遭拒。
...
- 允許的表單欄位為 `payload`、重複的 `files` blob，或一個 `clawpack` tarball 參照。`clawpack` 可以是 `.tgz` blob，或由 upload-url 流程回傳的 storage id。暫存 storage-id 發布也必須包含該上傳 URL 一併回傳的 `clawpackUploadTicket`。
- 使用 `files` 或 `clawpack` 擇一，絕不可在同一請求中同時使用。
- JSON body 和呼叫者提供的 `payload.files` / `payload.artifact` 中繼資料會遭拒。
...
- 程式碼外掛需要 `package.json`、來源 repo 中繼資料、來源 commit 中繼資料、設定 schema 中繼資料、`openclaw.compat.pluginApi` 和 `openclaw.build.openclawVersion`。
...
擁有者發起的軟刪除會保留 slug 30 天，之後 slug 可由另一個發布者認領。刪除回應在此到期適用時會包含 `slugReservedUntil`。
...
僅限管理員。確保某個 handle 存在 org 發布者。如果該 handle 仍指向舊版共用使用者/個人發布者，此端點會先將其遷移為 org 發布者。
對於新建立的 org，請提供 `memberHandle`；執行操作的管理員不會被新增為成員。
...
已驗證的自助式 org 發布者建立。建立新的 org 發布者並將呼叫者新增為擁有者。此端點不會遷移既有使用者/個人 handle，也不會將發布者標記為受信任/official。
...
僅限管理員。為合法擁有者保留根 slug 和套件名稱，而不發布發行版。套件名稱會成為沒有發行版列的私有 placeholder 套件，因此同一擁有者稍後可以將真正的 code-plugin 或 bundle-plugin 發行版發布到該名稱。
...
僅限管理員。為已驗證的替代 GitHub OAuth 主體復原個人發布者，而不編輯 Convex Auth 帳戶列。請求必須命名兩個不可變的 GitHub provider account id；可變 handle 只會作為面向操作員的防護。

端點預設為試執行。套用復原需要在工作人員獨立驗證兩個
GitHub 身分主體之間的連續性後，設定 `dryRun: false` 和
`confirmIdentityVerified: true`。當目的地使用者目前的個人發布者已有 skills、packages 或 GitHub skill sources 時，復原會以失敗關閉方式處理。
復原也會遷移復原發布者的 skills、skill slug aliases、packages、package inspector warnings，以及衍生的 search digest rows 中的舊版 `ownerUserId` 欄位，讓直接擁有者路徑與新的發布者權限一致。復原 handle 的有效受保護 handle 保留也會重新指派給替代使用者，讓後續的 profile synchronization 無法還原前使用者的競爭權限。每個主要資料表在每次套用交易中限制為
100 列；較大的復原必須先使用可續行的擁有者遷移。
GitHub skill sources 以發布者為範圍，並回報為已檢查，而不是改寫。
...
- `merge` 會隱藏來源 listing，並將來源 slug 重新導向到目標 listing。
...
內容（僅限 admin）。除非 `dryRun` 為 `false`，否則預設為試執行。
...
`POST /api/cli/upload-url` 會回傳 `uploadUrl` 和 `uploadTicket`。Package
發布若會暫存 ClawPack tarball，必須將產生的 storage id 以
`clawpack` 傳送，並將回傳的 ticket 以 `clawpackUploadTicket` 傳送。
