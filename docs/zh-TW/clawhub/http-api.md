...
舊版 `/api/...` 和 `/api/cli/...` 仍保留以維持相容性（見 `DEPRECATIONS.md`）。
OpenAPI：`/api/v1/openapi.json`。

## 公開目錄重用

第三方目錄可以使用公開讀取端點來列出或搜尋 ClawHub Skills。請快取結果、遵守 `429`/`Retry-After`、將使用者連回標準 ClawHub 列表（`https://clawhub.ai/<owner>/skills/<slug>`），並避免暗示 ClawHub 認可第三方網站。不要嘗試在公開 API 表面之外鏡像隱藏、私人或因審核而封鎖的內容。

網頁 slug 捷徑會跨 registry 家族解析，但 API 用戶端應使用
讀取端點回傳的標準 URL，而不是重建路由
優先順序。
...
- 如果 token 遺失/無效，行為會退回到 IP 強制執行。
- 已驗證寫入端點在
  伺服器知道原因時，不應回傳裸露的 `Unauthorized`。遺失 token、無效/已撤銷 token，以及
  已刪除/封鎖/停用帳戶都應取得可操作文字，讓命令列介面
  用戶端能告訴使用者是什麼阻擋了他們。
...
- 僅在部署明確啟用受信任轉送標頭時，
  使用受信任的用戶端 IP 標頭，包括 `cf-connecting-ip`。
- ClawHub 使用受信任的轉送標頭在邊緣識別用戶端 IP。
- 如果沒有可用的受信任用戶端 IP，匿名請求會使用
  只依速率限制種類界定範圍的 fallback bucket。這些 fallback bucket 不包含
...
- `q`（必填）：查詢字串
- `limit`（選填）：整數
- `highlightedOnly`（選填）：`true` 以篩選到精選 Skills
...
- 相關性強於熱門度。精確的 slug 或顯示名稱 token 符合，可能勝過參與度強得多但匹配較鬆散的結果。
- ASCII 文字會依單字和標點邊界斷詞。例如，`personal-map` 包含獨立的 `map` token，而 `amap-jsapi-skill` 包含 `amap`、`jsapi` 和 `skill`；因此搜尋 `map` 會讓 `personal-map` 取得比 `amap-jsapi-skill` 更強的詞彙匹配。
- 熱門度會以對數縮放並設上限。查詢文字匹配較弱時，高參與度 Skills 可能排名較低。
- 可疑或隱藏的審核狀態可能會依呼叫者篩選條件和目前審核狀態，將 Skill 從公開搜尋中移除。
...
- 重新命名別名會保留舊 URL 和透過 registry 解析的安裝解析能力，但搜尋排名會以重新命名完成索引後的標準 Skill metadata 為基準。既有統計資料會留在該 Skill 上。
- 如果 Skill 意外不可見，請先在登入狀態下使用 `clawhub inspect @owner/slug` 檢查審核狀態，再變更與排名相關的 metadata。
...
- `recommended` 使用參與度和新近度訊號。
- `trending` 依最近 7 天的安裝數排名（以 telemetry 為基礎）。
- `createdAt` 對新 Skill 爬取是穩定的；`updated` 會在既有 Skills 重新發布時變更。
- 當 `nonSuspiciousOnly=true` 時，基於游標的排序可能會在一頁回傳少於 `limit` 的項目，因為可疑 Skills 會在取回頁面後被篩掉。
...
- Owner 和 moderator 可以存取隱藏 Skills 的審核詳細資訊。
- 公開呼叫者只會對已標記且可見的 Skills 取得 `200`。
- 公開呼叫者的證據會被遮蔽，且只有 owner/moderator 會包含原始片段。

### `POST /api/v1/skills/{slug}/report`

回報 Skill 以供 moderator 審查。回報是 Skill 層級，可選擇連結
...
Skill 回報收件的 moderator/admin 端點。
...
`status` 設回 `open` 時可以省略。對已 triage 的
...
- `version.security` 會在可用時包含正規化的掃描驗證狀態和掃描器詳細資訊
  （VirusTotal + LLM）。
...
- 掃描請求酬載和可下載報告會在保留期間過後，從掃描請求儲存區到期。
...
- 回傳已佇列/執行中/成功/失敗狀態。
- 佇列中時回傳 `queue.queuedAhead` 和 `queue.position`，讓用戶端可以顯示該請求前方有多少優先處理的手動掃描。非常大的佇列會受到限制，並以 `queuedAheadIsEstimate: true` 回報。
...
- 需要對該技能或外掛擁有擁有者/發布者管理存取權，或平台審核員/管理員權限。
- 回傳精確提交版本的已儲存掃描結果，包括已封鎖或隱藏的版本。
- `kind` 預設為 `skill`；針對外掛/套件掃描請使用 `kind=plugin`。
- 回傳與掃描請求下載相同的 ZIP 形狀。
...
- 只有在所選版本具有已產生的 Skill Card、未被審核封鎖為惡意軟體，且 ClawScan 驗證為乾淨時，`ok` 才會是 `true`。
- 技能身分、發布者身分和所選版本中繼資料是頂層封套欄位（`slug`、`displayName`、`publisherHandle`、`version`、`resolvedFrom`、`tag`、`createdAt`），因此 shell 自動化可以在不拆解巢狀包裝器的情況下讀取它們。
- `security` 是頂層 ClawScan/安全判定。自動化應以 `ok`、`decision`、`reasons` 和 `security.status` 作為判斷依據。
...
- `category`（選填）：外掛類別篩選器。僅在請求限定於外掛套件（`/api/v1/plugins`、
...
- `channel`（選填）：`official`、`community` 或 `private`
- `isOfficial`（選填）：`true` 或 `false`
- `category`（選填）：外掛類別篩選器。僅在請求限定於外掛套件時支援。受控類別與舊版 v1
...
- ClawPack 版本包含 npm 相容的 `npmIntegrity`、`npmShasum` 和
  `npmTarballName` 欄位。
- `version.sha256hash` 是供舊用戶端使用的已淘汰相容性中繼資料。它
...
- `trust.scanStatus` 是從掃描器輸入與手動版本審核
  推導出的有效信任狀態。
...
- `trust.blockedFromDownload` 是安裝封鎖訊號。OpenClaw 和其他
  ...
  從掃描器或審核欄位重新推導封鎖規則。
- `trust.reasons` 是面向使用者和稽核用的說明清單。原因代碼
...
`confirmed` 和 `dismissed` 需要 `note`；將 `status` 設回 `open` 時可以省略。傳入 `finalAction: "quarantine"` 或 `finalAction: "revoke"` 搭配已確認的回報，可在同一個可稽核的工作流程中套用版本審核。
...
- 二進位檔案會回傳 `415`。
- 檔案大小限制：200KB。
- 待處理的 VirusTotal 掃描不會封鎖讀取；惡意版本仍可能在其他地方被保留不公開。
...
- 外掛/套件封存檔是 zip 檔案，且具有 `package/` 根目錄，因此舊版 OpenClaw 用戶端可繼續運作。
...
回傳適用於 ClawPack 支援套件版本的 npm 相容 packument。

注意事項：

- 只會列出已上傳 ClawPack npm-pack tarball 的版本。
- 舊版僅 ZIP 版本會刻意省略。
- `dist.tarball`、`dist.integrity` 和 `dist.shasum` 使用 npm 相容欄位，因此使用者可在需要時將 npm 指向該鏡像。
...
下載託管的 Skills 版本 ZIP，或針對目前由 GitHub 支援、掃描結果為 `clean` 或 `suspicious` 且沒有託管版本的 Skills，回傳 GitHub 原始碼交接。
...
- 如果未提供 `version` 或 `tag`，則使用最新版本。
- 軟刪除版本會回傳 `410`。
- 由 GitHub 支援的 Skills 交接不會代理或鏡像位元組。JSON 回應包含 `sourceRef: "public-github"`、`repo`、`commit`、`path`、`contentHash` 和 `archiveUrl`；掃描/目前狀態是一個閘門，不會作為成功酬載中繼資料包含在內。
...
- 偏好：使用 `multipart/form-data`，其中包含 `payload` JSON + `files[]` blob。
- 也接受包含 `files`（以 storageId 為基礎）的 JSON 主體。
- 選填酬載欄位：`ownerHandle`。存在時，API 會在伺服器端解析該發布者，並要求執行者具有發布者存取權。
- 選填酬載欄位：`migrateOwner`。當它與 `ownerHandle` 一起為 `true` 時，如果執行者同時是目前和目標發布者的管理員/擁有者，既有 Skills 可以移至該擁有者。沒有這項選擇加入時，擁有者變更會被拒絕。
...
- 允許的表單欄位為 `payload`、重複的 `files` blob，或一個 `clawpack` tarball 參照。`clawpack` 可以是 `.tgz` blob，或上傳 URL 流程回傳的 storage id。使用暫存 storage-id 發布時，還必須包含該上傳 URL 一併回傳的 `clawpackUploadTicket`。
- 使用 `files` 或 `clawpack`，同一個請求中絕不可兩者並用。
...
- 程式碼外掛需要 `package.json`、原始碼儲存庫中繼資料、原始碼 commit 中繼資料、設定結構描述中繼資料、`openclaw.compat.pluginApi` 和 `openclaw.build.openclawVersion`。
- `openclaw.hostTargets` 和 `openclaw.environment` 是選填中繼資料。
- 只有 `openclaw` 組織發布者和目前 `openclaw` 組織成員的個人發布者可以發布到 `official` 頻道。
- 代表他人發布仍會根據目標擁有者帳戶驗證 official-channel 資格。
...
擁有者發起的軟刪除會保留 slug 30 天，之後其他發布者即可聲明該 slug。
...
已驗證的自助式組織發布者建立。建立新的組織發布者，並將呼叫者加入為擁有者。此端點不會遷移既有使用者/個人 handle，也不會將發布者標記為受信任/官方。
...
僅限管理員。為合法擁有者保留根 slug 和套件名稱，而不發布版本。套件名稱會變成沒有版本資料列的私有佔位套件，因此相同擁有者稍後可以將真正的 code-plugin 或 bundle-plugin 版本發布到該名稱。
...
僅限管理員。為已驗證的替代 GitHub OAuth principal 復原個人發布者，而不編輯 Convex Auth 帳戶資料列。請求必須命名兩個不可變的 GitHub provider account id；可變 handle 僅作為面向操作員的防護。

端點預設為試執行。套用復原需要在工作人員獨立驗證兩個
GitHub 主體之間的連續性後，設定 `dryRun: false` 和
`confirmIdentityVerified: true`。當目標使用者目前的個人發布者已有技能、套件或 GitHub 技能來源時，
復原會以關閉方式失敗。
復原也會遷移已復原發布者的技能、技能 slug 別名、套件、套件檢查器警告，以及衍生搜尋摘要資料列中的舊版 `ownerUserId` 欄位，讓
直接擁有者路徑與新的發布者權限一致。已復原 handle 的有效受保護 handle
保留也會重新指派給替代使用者，避免後續的
個人檔案同步還原前使用者的競爭權限。每個主要資料表在每筆套用交易中限制為
...
- `/.well-known/clawhub.json`（JSON，偏好使用）
