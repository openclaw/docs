...
第三方目錄可以使用公開讀取端點來列出或搜尋 ClawHub Skills。請快取結果、遵守 `429`/`Retry-After`、將使用者連回標準 ClawHub 條目（`https://clawhub.ai/<owner>/skills/<slug>`），並避免暗示 ClawHub 認可該第三方網站。請勿嘗試在公開 API 表面之外鏡像隱藏、私有或遭審核封鎖的內容。
...
- 已驗證的寫入端點在伺服器知道原因時，不應只傳回裸露的 `Unauthorized`。缺失權杖、無效/已撤銷權杖，以及已刪除/已封鎖/已停用帳號，都應取得可操作的文字，讓命令列介面用戶端能告訴使用者被阻擋的原因。
...
- 如果沒有可用的受信任用戶端 IP，匿名請求會使用僅按速率限制種類限定範圍的後備 bucket。這些後備 bucket 不包含呼叫者提供的路徑、slug、套件名稱、版本、查詢字串或其他成品參數。

## 錯誤回應

公開 v1 錯誤回應是純文字，且 `content-type: text/plain; charset=utf-8`。
這包括驗證失敗（`400`）、缺失公開資源（`404`）、驗證和權限失敗（`401`/`403`）、速率限制（`429`）以及遭封鎖的下載。用戶端應將回應本文讀作人類可讀字串。未知查詢參數會為了相容性而被忽略，但已辨識的查詢參數若值無效，則會傳回 `400`。
...
- 相關性比熱門度更強。精確的 slug 或顯示名稱 token 符合，可能會排在互動強得多但符合較寬鬆的結果前面。
- ASCII 文字會依單字和標點邊界切分 token。例如，`personal-map` 包含獨立的 `map` token，而 `amap-jsapi-skill` 包含 `amap`、`jsapi` 和 `skill`；因此搜尋 `map` 會讓 `personal-map` 取得比 `amap-jsapi-skill` 更強的詞彙符合。
- 熱門度會以對數縮放並設上限。當查詢文字符合較弱時，高互動 Skills 可能排名較低。
- 可疑或隱藏的審核狀態可能會根據呼叫者篩選器和目前審核狀態，將 Skill 從公開搜尋中移除。

發布者可探索性指引：

- 將使用者會逐字搜尋的詞放在顯示名稱、摘要和標籤中。只有在獨立 slug token 也是你想保留的穩定識別時才使用它。
- 除非新的 slug 是更好的長期標準名稱，否則不要只為了追逐某個查詢而重新命名 slug。舊 slug 會成為重新導向別名，但標準 URL、顯示的 slug 和未來搜尋摘要會使用新的 slug。
- 重新命名別名會保留舊 URL 以及透過登錄解析安裝項目的解析能力，但重新命名已建立索引後，搜尋排名會根據標準 Skill 中繼資料。既有統計資料會留在該 Skill 上。
...
- 當 `nonSuspiciousOnly=true` 時，基於游標的排序可能會在單頁傳回少於 `limit` 的項目，因為可疑 Skills 會在擷取頁面後被篩除。
- 存在時使用 `nextCursor` 繼續分頁。短頁本身不代表結果結束。
...
- 公開呼叫者的證據會經過遮蔽，且只有擁有者/審核者會包含原始片段。

### `POST /api/v1/skills/{slug}/report`

回報 Skill 以供審核者檢閱。回報是 Skill 層級，可選擇連結到版本，並會送入 Skill 回報佇列。
...
`confirmed` 和 `dismissed` 需要 `note`；將 `status` 設回 `open` 時可以省略。傳入 `finalAction: "hide"` 搭配已分類處理的回報，可在同一個可稽核工作流程中隱藏該 Skill。
...
- 在佇列中時回傳 `queue.queuedAhead` 和 `queue.position`，讓用戶端可以顯示該請求前方有多少個優先手動掃描。非常大的佇列會受到限制，並以 `queuedAheadIsEstimate: true` 回報。
- 可用時，`report` 會包含 `clawscan`、`skillspector`、`staticAnalysis` 和 `virustotal` 區段。
- 失敗的掃描作業會回傳 `status: "failed"` 和 `lastError`。
...
僅限管理員的正式批次重新掃描路由。它接受與舊版 `POST /api/v1/skills/-/rescan-batch` 相同的酬載形狀。

### `POST /api/v1/skills/-/scan/batch/status`

僅限管理員的正式批次狀態路由。它接受 `{ "jobIds": ["..."] }`，並回傳與舊版 `POST /api/v1/skills/-/rescan-batch/status` 相同的彙總計數器。
...
- 只有在所選版本有已產生的 Skill Card、未被審核標記為惡意軟體封鎖，且 ClawScan 驗證為乾淨時，`ok` 才會是 `true`。
- 技能身分、發布者身分和所選版本中繼資料是頂層信封欄位（`slug`、`displayName`、`publisherHandle`、`version`、`resolvedFrom`、`tag`、`createdAt`），因此 shell 自動化可以不需解開巢狀包裝器就讀取它們。
...
- `security.signals.dependencyRegistry` 保留用於 v1 回應相容性，但依賴項登錄檔存在性掃描器已退役，且此鍵一律為 `null`。
...
- 當你需要單一技能 Skill Card 驗證信封時使用 `/verify`，需要已產生的卡片 Markdown 時使用 `/card`，需要詳細掃描器資料時使用 `/scan`。
...
- `GET /api/v1/code-plugins` 和 `GET /api/v1/bundle-plugins` 仍然是固定系列別名。
...
在統一的 `/api/v1/packages` 端點上，它僅適用於外掛；請使用
...
- 目前由 GitHub 支援、且掃描結果為 `clean` 或 `suspicious` 的 Skills 包含
  `_source_handoff.json`，其中有 `sourceRef: "public-github"`、repo、commit、path、
  內容雜湊和封存 URL。它們不包含 ClawHub 託管的原始碼檔案。
...
- `family`（選用）：`code-plugin` 或 `bundle-plugin`。省略表示兩種外掛系列。
...
- 各外掛的匯出中繼資料儲存在 `__clawhub_export/{family}/{packageName}/plugin_meta.json`。
- `_manifest.json` 一律包含在 ZIP 根目錄。
...
只針對 `code-plugin` 與 `bundle-plugin` 套件進行外掛搜尋。
...
- 也接受記錄在 `GET /api/v1/plugins` 下的舊版 v1 篩選器別名。
- 分類篩選是真正的 API 篩選器，由外掛分類摘要資料列支援，而不是搜尋查詢重寫。
- 結果會依相關性順序傳回，且目前不分頁。
...
- 私人套件會傳回 `404`，除非呼叫者可以讀取擁有該套件的發布者。
...
傳回一個套件版本，包含檔案中繼資料、相容性、驗證、成品中繼資料與掃描資料。

備註：

- 對舊世界套件封存檔，`version.artifact.kind` 為 `legacy-zip`；對 ClawPack 支援的版本，則為 `npm-pack`。
- ClawPack 版本包含與 npm 相容的 `npmIntegrity`、`npmShasum` 與 `npmTarballName` 欄位。
- `version.sha256hash` 是供舊用戶端使用的已棄用相容性中繼資料。它會雜湊 `/api/v1/packages/{name}/download` 傳回的確切 ZIP 位元組。現代用戶端應使用 `version.artifact.sha256`，該值識別標準版本成品。
- 當掃描資料存在時，會包含 `version.vtAnalysis`、`version.llmAnalysis` 與 `version.staticScan`。
- 私人套件會傳回 `404`，除非呼叫者可以讀取擁有該套件的發布者。

### `GET /api/v1/packages/{name}/versions/{version}/security`

傳回供安裝用戶端使用的確切套件版本安全與信任摘要。這是公開的 OpenClaw 消費介面，用於判斷已解析版本是否可以安裝。
...
- `package.name`、`package.displayName` 與 `package.family` 會識別已解析的登錄套件。
- `release.releaseId`、`release.version` 與 `release.createdAt` 會識別已評估的確切版本。
- 當版本成品已知時，會出現 `release.artifactKind`、`release.artifactSha256`、`release.npmIntegrity`、`release.npmShasum` 與 `release.npmTarballName`。
- `trust.scanStatus` 是由掃描器輸入與手動版本審核衍生出的有效信任狀態。
- `trust.moderationState` 可為 null。當不存在手動版本審核時，其值為 `null`。
- `trust.blockedFromDownload` 是安裝封鎖訊號。當此值為 `true` 時，OpenClaw 與其他安裝用戶端應封鎖安裝，而不是從掃描器或審核欄位重新推導封鎖規則。
...
- `trust.stale` 表示信任摘要是根據過期輸入計算而成，且在做出高信心允許決策前應視為需要重新整理。

備註：

- 此端點精確對應版本。用戶端應在解析出要安裝的套件版本後呼叫它，而不只是讀取最新套件中繼資料後呼叫。
- 私人套件會傳回 `404`，除非呼叫者可以讀取擁有該套件的發布者。
- 此端點刻意比擁有者/版主審核端點更窄。它公開安裝決策與公開說明，而不是回報者身分、回報內容、私人證據或內部審查時間軸。
...
- `packageName` 會依 npm 名稱正規化；對於計畫中的遷移，套件可以不存在。
- 這只會追蹤遷移就緒狀態。它不會變更 OpenClaw，也不會產生 ClawPack。
...
回報套件以供版主審查。回報是套件層級，可選擇連結到某個版本。它們會送入審核佇列，但本身不會自動隱藏或封鎖下載；版主應使用版本審核來核准、隔離或撤銷成品。
...
`confirmed` 和 `dismissed` 需要 `note`；將 `status` 設回 `open` 時可以省略。傳入 `finalAction: "quarantine"` 或 `finalAction: "revoke"` 搭配已確認的回報，即可在同一個可稽核的工作流程中套用發布審核。
...
隔離和撤銷的發布會從成品下載路由傳回 `403`。每次變更都會寫入稽核記錄項目。
...
- 外掛/套件封存檔是帶有 `package/` 根目錄的 zip 檔案，讓舊版 OpenClaw 用戶端能持續運作。
...
- 具範圍套件 packument 同時支援 `/api/npm/@scope/name` 和 npm 的編碼 `/api/npm/@scope%2Fname` 請求路徑。
...
下載託管的技能版本 ZIP，或針對目前由 GitHub 支援且掃描為 `clean` 或 `suspicious`、且沒有託管版本的技能，傳回 GitHub 來源交接。
...
- 如果未提供 `version` 或 `tag`，則使用最新版本。
- 軟刪除版本會傳回 `410`。
- 由 GitHub 支援的技能交接不會代理或鏡像位元組。JSON 回應包含 `sourceRef: "public-github"`、`repo`、`commit`、`path`、`contentHash` 和 `archiveUrl`；掃描/目前狀態是門檻，不會作為成功酬載中繼資料包含。
- 下載統計會依 UTC 日以唯一身分計數（API 權杖有效時為 `userId`，否則為 IP）。
...
- 首選：使用帶有 `payload` JSON + `files[]` blob 的 `multipart/form-data`。
- 也接受帶有 `files`（基於 storageId）的 JSON 主體。
- 選填酬載欄位：`ownerHandle`。存在時，API 會在伺服器端解析該發布者，並要求執行者具有發布者存取權。
- 選填酬載欄位：`migrateOwner`。當與 `ownerHandle` 一起為 `true` 時，若執行者同時是目前和目標發布者的管理員/擁有者，既有技能可以移動到該擁有者。沒有此選擇加入時，擁有者變更會被拒絕。
...
- 使用 `files` 或 `clawpack` 其中之一，不得在同一個請求中同時使用。
- JSON 主體和呼叫者提供的 `payload.files` / `payload.artifact` 中繼資料會被拒絕。
- 直接 multipart 發布請求上限為 18MB。ClawPack tarball 可使用 upload-url 流程，最高達 120MB tarball 上限。
...
僅限管理員。確保 handle 存在 org 發布者。如果該 handle 仍指向舊版共用使用者/個人發布者，此端點會先將它遷移到 org 發布者。
...
已驗證的自助式 org 發布者建立。建立新的 org 發布者並將呼叫者加入為擁有者。此端點不會遷移既有使用者/個人 handle，也不會將發布者標記為受信任/官方。
...
僅限管理員。為合法擁有者保留根 slug 和套件名稱，而不發布版本。套件名稱會成為沒有發布列的私有佔位套件，因此同一擁有者之後可將真正的 code-plugin 或 bundle-plugin 發布到該名稱。
...
僅限管理員。為已驗證的替換 GitHub OAuth 主體復原個人發布者，而不編輯 Convex Auth 帳戶列。請求必須指定兩個不可變的 GitHub 提供者帳戶 ID；可變 handle 僅作為面向操作員的防護使用。

端點預設為試執行。套用復原需要在工作人員獨立確認兩個
GitHub 主體之間的連續性後，設定 `dryRun: false` 和
`confirmIdentityVerified: true`。當目的地使用者目前的個人
發布者擁有技能、套件或 GitHub 技能來源時，復原會以封閉方式失敗。
復原也會遷移已復原發布者之技能、技能 slug 別名、套件、套件檢查器警告，以及衍生搜尋摘要列的舊版 `ownerUserId` 欄位，使
直接擁有者路徑與新的發布者權限一致。已復原 handle 的有效受保護 handle
保留項目也會重新指派給替代使用者，使之後的
個人檔案同步無法還原前使用者的競爭權限。每個主要資料表在每個套用交易中限制為
100 列；較大的復原必須先使用可續傳的擁有者遷移。
GitHub 技能來源是發布者範圍，並會回報為已檢查，而不是被重寫。
...
`POST /api/cli/upload-url` 會回傳 `uploadUrl` 和 `uploadTicket`。暫存 ClawPack tar 封包的套件
發布必須將產生的儲存體 ID 作為
