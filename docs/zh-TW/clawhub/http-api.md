...
第三方目錄可以使用公開讀取端點來列出或搜尋 ClawHub Skills。請快取結果、遵守 `429`/`Retry-After`、將使用者連回標準 ClawHub 清單（`https://clawhub.ai/<owner>/skills/<slug>`），並避免暗示 ClawHub 認可該第三方網站。請勿嘗試在公開 API 表面之外鏡像隱藏、私人或被審核封鎖的內容。
...
- 已驗證的寫入端點在伺服器知道原因時，不應只傳回裸露的 `Unauthorized`。遺失權杖、無效/已撤銷的權杖，以及已刪除/停權/停用的帳號，都應取得可操作的文字，讓命令列介面用戶端能告訴使用者阻擋原因。
...
- 使用帶 jitter 的退避，以避免同步重試。
- 如果 `Retry-After` 缺少，退回使用 `RateLimit-Reset`（或從 `X-RateLimit-Reset` 計算）。

IP 來源：

- 只有在部署明確啟用受信任轉送標頭時，才使用受信任的用戶端 IP 標頭，包括 `cf-connecting-ip`。
- ClawHub 使用受信任的轉送標頭在邊緣識別用戶端 IP。
- 如果沒有可用的受信任用戶端 IP，匿名請求會使用僅由速率限制種類限定範圍的後備 bucket。這些後備 bucket 不包含呼叫者提供的路徑、slug、套件名稱、版本、查詢字串或其他成品參數。

## 錯誤回應

公開 v1 錯誤回應是純文字，並帶有 `content-type: text/plain; charset=utf-8`。
這包括驗證失敗（`400`）、缺少公開資源（`404`）、驗證與權限失敗（`401`/`403`）、速率限制（`429`）和被封鎖的下載。用戶端應將回應本文讀取為人類可讀字串。未知的查詢參數會為了相容性而被忽略，但具有無效值的已識別查詢參數會傳回 `400`。
...
- `q`（必要）：查詢字串
- `limit`（選用）：整數
- `highlightedOnly`（選用）：`true` 以篩選至精選 Skills
...
- 相關性比人氣更強。精確的 slug 或顯示名稱 token 相符，可能排名高於互動度高得多但相符較鬆散的結果。
- ASCII 文字會依單字和標點邊界分詞。例如，`personal-map` 包含獨立的 `map` token，而 `amap-jsapi-skill` 包含 `amap`、`jsapi` 和 `skill`；因此搜尋 `map` 會讓 `personal-map` 比 `amap-jsapi-skill` 有更強的詞彙相符。
- 人氣會以對數縮放並設上限。當查詢文字相符較弱時，高互動 Skills 可能排名較低。
- 可疑或隱藏的審核狀態可能會根據呼叫者篩選器與目前審核狀態，將 Skill 從公開搜尋中移除。

發布者可發現性指引：

- 將使用者會實際搜尋的詞放在顯示名稱、摘要和標籤中。只有在獨立 slug token 也是你想保留的穩定身分時才使用它。
- 不要只為了追逐某個查詢而重新命名 slug，除非新 slug 是更好的長期標準名稱。舊 slug 會成為重新導向別名，但標準 URL、顯示的 slug 和未來搜尋摘要會使用新 slug。
- 重新命名別名會保留舊 URL，以及透過登錄解析之安裝的解析能力，但搜尋排名會以重新命名索引後的標準 Skill 中繼資料為基礎。現有統計資料會保留在該 Skill 上。
- 如果 Skill 意外不可見，請在變更與排名相關的中繼資料前，先登入並用 `clawhub inspect @owner/slug` 檢查審核狀態。
...
- `sort`（選用）：`updated`（預設）、`recommended`（別名：`default`）、`createdAt`（別名：`newest`）、`downloads`、`stars`（別名：`rating`）、舊版安裝別名 `installsCurrent`/`installs`/`installsAllTime` 會映射到 `downloads`、`trending`
- `nonSuspiciousOnly`（選用）：`true` 以隱藏可疑（`flagged.suspicious`）Skills
- `nonSuspicious`（選用）：`nonSuspiciousOnly` 的舊版別名
...
- 當 `nonSuspiciousOnly=true` 時，因為可疑 Skills 會在擷取頁面後被篩除，游標式排序可能在一頁中傳回少於 `limit` 的項目。
...
`note` 對 `confirmed` 和 `dismissed` 是必要的；將 `status` 設回 `open` 時可以省略。對已分類的回報傳入 `finalAction: "hide"`，以在同一個可稽核工作流程中隱藏該 Skill。
...
- 佇列中時傳回 `queue.queuedAhead` 和 `queue.position`，讓用戶端可以顯示該請求前方有多少個優先手動掃描。非常大的佇列會受到限制，並以 `queuedAheadIsEstimate: true` 回報。
- 可用時，`report` 會包含 `clawscan`、`skillspector`、`staticAnalysis` 和 `virustotal` 區段。
- 失敗的掃描作業會傳回 `status: "failed"` 和 `lastError`。
...
- 需要該 skill 或外掛的擁有者/發布者管理存取權，或平台審核者/管理員權限。
- 傳回確切提交版本的已儲存掃描結果，包括被封鎖或隱藏的版本。
...
- 只有在選取版本已有產生的 Skill Card、未被審核判定為惡意軟體而封鎖，且 ClawScan 驗證為乾淨時，`ok` 才會是 `true`。
- Skill 身分、發布者身分和選取版本中繼資料是頂層封套欄位（`slug`、`displayName`、`publisherHandle`、`version`、`resolvedFrom`、`tag`、`createdAt`），因此 shell 自動化可以在不解開巢狀包裝的情況下讀取它們。
...
- `security.signals.dependencyRegistry` 保留用於 v1 回應相容性，但依賴項登錄存在性掃描器已退役，且此鍵一律為 `null`。
...
傳回精確 Skills 版本的目前精簡安全判定。此
集合端點是為已知道需要顯示哪些已安裝
ClawHub Skills 版本的用戶端而設計，例如 OpenClaw Control UI。
...
- 結果依項目回傳；缺少某個 skill 或版本不會使整個回應失敗。
- 回應僅包含安全資訊。它不包含 Skill Card 資料、產生的卡片狀態、成品檔案清單，或詳細掃描器酬載。
- `security.signals` 只包含狀態層級的支援證據；如需完整掃描器詳細資訊，請使用 `/scan` 或 ClawHub security-audit 頁面。
- `security.signals.dependencyRegistry` 保留用於 v1 回應相容性，但依賴項登錄存在性掃描器已退役，且此鍵一律為 `null`。
- Skill Card 缺少不會影響此端點的 `ok`、`decision` 或 `reasons`；用戶端需要卡片內容時，應在本機讀取已安裝的 `skill-card.md`。
- 當你需要單一 skill 的 Skill Card 驗證封套時使用 `/verify`，需要產生的卡片 Markdown 時使用 `/card`，需要詳細掃描器資料時使用 `/scan`。
...
- `category`（選填）：外掛類別篩選器。僅在要求範圍限定為外掛套件時支援（`/api/v1/plugins`、`/api/v1/code-plugins`、`/api/v1/bundle-plugins`，或帶有 `family=code-plugin`/`family=bundle-plugin` 的套件端點）。受控類別和舊版 v1 篩選別名記載於 `GET /api/v1/plugins` 下。

備註：

- `family`、`channel`、`isOfficial`、`featured`、`highlightedOnly` 或 `sort` 的無效值會傳回 `400`。未知查詢參數會被忽略。
...
- `channel`（選填）：`official`、`community` 或 `private`
- `isOfficial`（選填）：`true` 或 `false`
- `category`（選填）：外掛類別篩選器。僅在要求範圍限定為外掛套件時支援。受控類別和舊版 v1 篩選別名記載於 `GET /api/v1/plugins` 下。

備註：

- `family`、`channel`、`isOfficial`、`featured` 或 `highlightedOnly` 的無效值會傳回 `400`。未知查詢參數會被忽略。
- 匿名呼叫者只能看到公開套件頻道。
...
`trending` 是七天安裝/下載排行榜，並不使用歷來總計。
在統一的 `/api/v1/packages` 端點上，它僅適用於外掛；若要使用 Skills 目錄，請用
...
- 目前由 GitHub 支援且掃描結果為 `clean` 或 `suspicious` 的 Skills 會包含
  `_source_handoff.json`，其中包含 `sourceRef: "public-github"`、repo、commit、path、
  content hash，以及 archive URL。它們不包含 ClawHub 託管的原始檔。
...
- 分類篩選是由外掛分類摘要列支援的真實 API 篩選器，而不是搜尋查詢改寫。
- 結果會依相關性順序傳回，目前不分頁。
...
傳回單一套件版本，包括檔案中繼資料、相容性、驗證、成品中繼資料與掃描資料。
...
- ClawPack 版本包含與 npm 相容的 `npmIntegrity`、`npmShasum` 與
  `npmTarballName` 欄位。
- `version.sha256hash` 是供舊用戶端使用的已棄用相容性中繼資料。它會雜湊 `/api/v1/packages/{name}/download` 傳回的確切 ZIP 位元組。
...
傳回供安裝用戶端使用的確切套件版本安全性與信任摘要。這是 OpenClaw 用來判定已解析版本是否可安裝的公開取用介面。
...
- `package.name`、`package.displayName` 與 `package.family` 會識別已解析的註冊表套件。
- `release.releaseId`、`release.version` 與 `release.createdAt` 會識別經評估的確切版本。
...
- `trust.moderationState` 可為 null。當沒有手動版本審核時，它為 `null`。
- `trust.blockedFromDownload` 是安裝阻擋訊號。當此值為 `true` 時，OpenClaw 與其他安裝用戶端應阻擋安裝，而不是從掃描器或審核欄位重新推導阻擋規則。
- `trust.reasons` 是面向使用者與稽核用途的說明清單。原因代碼是穩定且精簡的字串，例如 `manual:quarantined`、`scan:malicious` 與 `package:malicious`。
- `trust.pending` 表示一個或多個信任輸入仍在等待完成。
- `trust.stale` 表示信任摘要是根據過時輸入計算而來，在做出高信心允許決策前，應視為需要重新整理。

備註：

- 此端點精確到版本。用戶端應在解析出預計安裝的套件版本後呼叫它，而不只是讀取最新套件中繼資料後呼叫。
- 私有套件會傳回 `404`，除非呼叫者可以讀取擁有該套件的發布者。
- 此端點刻意比擁有者/版主審核端點更窄。它公開安裝決策與公開說明，而不公開回報者身分、回報內容、私有證據或內部審查時間線。
...
回報套件以供版主審查。回報是套件層級，可選擇連結到版本。它們會送入審核佇列，但本身不會自動隱藏或阻擋下載；版主應使用版本審核來核准、隔離或撤銷成品。
...
`note` 在 `confirmed` 和 `dismissed` 時為必填；將 `status` 設回 `open` 時可省略。對已確認的回報傳入 `finalAction: "quarantine"` 或 `finalAction: "revoke"`，即可在同一個可稽核工作流程中套用版本審核。
...
- `revoked`：版本先前曾受信任，之後遭封鎖。

隔離和撤銷的版本會從成品下載路由傳回 `403`。每項變更都會寫入稽核記錄項目。
...
- 二進位檔案會傳回 `415`。
- 檔案大小限制：200KB。
- 待處理的 VirusTotal 掃描不會阻止讀取；惡意版本仍可能在其他地方被扣留。
...
- 外掛/套件封存是含有 `package/` 根目錄的 zip 檔案，讓舊版 OpenClaw 用戶端持續可用。
...
- `slug`（必填）
- `hash`（必填）：組合指紋的 64 字元十六進位 sha256
...
下載託管的 skill 版本 ZIP，或針對目前由 GitHub 支援、掃描結果為 `clean` 或 `suspicious` 且沒有託管版本的 skill，傳回 GitHub 原始碼交接。
...
- 如果未提供 `version` 或 `tag`，則使用最新版本。
- 軟刪除版本會傳回 `410`。
- 由 GitHub 支援的 skill 交接不會代理或鏡像位元組。JSON 回應包含 `sourceRef: "public-github"`、`repo`、`commit`、`path`、`contentHash` 和 `archiveUrl`；掃描/目前狀態是閘門，不會作為成功酬載中繼資料包含在內。
- 下載統計以每 UTC 日的不重複身分計算（API token 有效時為 `userId`，否則為 IP）。
...
- 偏好：含有 `payload` JSON + `files[]` blob 的 `multipart/form-data`。
- 也接受含有 `files`（以 storageId 為基礎）的 JSON body。
- 選填酬載欄位：`ownerHandle`。存在時，API 會在伺服器端解析該發布者，並要求執行者具有發布者存取權。
- 選填酬載欄位：`migrateOwner`。當搭配 `ownerHandle` 為 `true` 時，如果執行者同時是目前和目標發布者的管理員/擁有者，現有 skill 可移至該擁有者。沒有此明確選擇時，擁有者變更會被拒絕。
...
- 允許的表單欄位為 `payload`、重複的 `files` blob，或一個 `clawpack` tarball 參照。`clawpack` 可以是 `.tgz` blob，或由 upload-url 流程傳回的 storage id。暫存 storage-id 發布也必須包含該上傳 URL 一併傳回的 `clawpackUploadTicket`。
- 使用 `files` 或 `clawpack` 其中之一，不得在同一請求中同時使用。
- JSON body 和呼叫者提供的 `payload.files` / `payload.artifact` 中繼資料會被拒絕。
...
- 程式碼外掛需要 `package.json`、原始碼 repo 中繼資料、來源 commit 中繼資料、設定 schema 中繼資料、`openclaw.compat.pluginApi` 和 `openclaw.build.openclawVersion`。
- `openclaw.hostTargets` 和 `openclaw.environment` 是選填中繼資料。
- 只有 `openclaw` org 發布者和目前 `openclaw` org 成員的個人發布者可發布到 `official` 頻道。
...
擁有者發起的軟刪除會保留 slug 30 天，之後該 slug 可由另一個發布者取得。當此到期適用時，刪除回應會包含 `slugReservedUntil`。
版主/管理員隱藏與安全移除不會以此方式到期。
...
僅限管理員。確保 handle 存在 org 發布者。如果該 handle 仍指向舊版共享使用者/個人發布者，端點會先將其遷移成 org 發布者。
對於新建立的 org，請提供 `memberHandle`；執行的管理員不會被加入為成員。
...
已驗證的自助式 org 發布者建立。建立新的 org 發布者並將呼叫者加入為擁有者。此端點不會遷移現有的使用者/個人 handle，也不會將發布者標記為受信任/官方。
...
僅限管理員。為正當擁有者保留根 slug 和套件名稱，而不發布版本。套件名稱會成為沒有版本列的私有占位套件，因此同一擁有者稍後可將真正的 code-plugin 或 bundle-plugin 版本發布到該名稱。
...
僅限管理員。為已驗證的替代 GitHub OAuth 主體復原個人發布者，而不編輯 Convex Auth 帳號列。請求必須指定兩個不可變 GitHub provider account id；可變 handle 僅用作面向操作員的防護。

端點預設為試執行。套用復原需要在工作人員獨立驗證兩個 GitHub 主體之間的連續性後，設定 `dryRun: false` 和
`confirmIdentityVerified: true`。當目的地使用者目前的個人發布者擁有技能、套件或 GitHub 技能來源時，
復原會以預設拒絕方式失敗。
復原也會遷移已復原發布者技能、技能 slug 別名、套件、套件檢查器警告，以及衍生搜尋摘要列中的舊版 `ownerUserId` 欄位，讓
直接擁有者路徑與新的發布者權限一致。已復原 handle 的有效受保護 handle
保留也會重新指派給替代使用者，讓之後的個人檔案同步無法還原前使用者的競爭權限。每個主要表格在每次套用交易中限制為
100 列；更大的復原必須先使用可恢復的擁有者遷移。
GitHub 技能來源屬於發布者範圍，會回報為已檢查，而不是被重寫。
