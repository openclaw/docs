...
第三方目錄可以使用公開讀取端點來列出或搜尋 ClawHub Skills。請快取結果、遵守 `429`/`Retry-After`、將使用者連回標準 ClawHub 列表（`https://clawhub.ai/<owner>/skills/<slug>`），並避免暗示 ClawHub 認可該第三方網站。請勿嘗試在公開 API 介面之外鏡像隱藏、私人或遭審核封鎖的內容。

網頁 slug 捷徑會跨登錄檔家族解析，但 API 用戶端應使用
...
- 如果權杖缺失或無效，行為會退回 IP 強制執行。
- 已驗證寫入端點在伺服器知道原因時，不應只傳回空泛的 `Unauthorized`。
  缺失權杖、無效/已撤銷權杖，以及已刪除/封禁/停用的帳號，
  都應取得可行動的文字，讓命令列介面
  用戶端可以告訴使用者是什麼阻擋了他們。

- 讀取：每 IP 3000/min，每金鑰 12000/min
- 寫入：每 IP 300/min，每金鑰 3000/min
- 下載：每 IP 1200/min，每金鑰 6000/min（下載端點）
...
- 只有在部署明確啟用受信任的轉送標頭時，才會使用受信任的用戶端 IP 標頭，
  包括 `cf-connecting-ip`。
- ClawHub 使用受信任的轉送標頭在邊緣識別用戶端 IP。
- 如果沒有可用的受信任用戶端 IP，匿名請求會使用
  只依速率限制種類界定範圍的備援 bucket。這些備援 bucket 不包含
...
應將回應本文讀取為人類可讀字串。未知查詢參數會被
忽略以維持相容性，但已辨識且值無效的查詢參數會傳回
`400`。
...
- `q`（必要）：查詢字串
- `limit`（選用）：整數
- `highlightedOnly`（選用）：`true` 可篩選為精選 Skills
- `nonSuspiciousOnly`（選用）：`true` 可隱藏可疑（`flagged.suspicious`）Skills
...
- 結果會依相關性順序傳回（嵌入相似度 + 精確 slug/名稱權杖加權 + 小幅人氣先驗）。
- 相關性比人氣更強。精確的 slug 或顯示名稱權杖符合，可以勝過互動明顯更強但符合較寬鬆的結果。
- ASCII 文字會依單字和標點邊界權杖化。例如，`personal-map` 包含獨立的 `map` 權杖，而 `amap-jsapi-skill` 包含 `amap`、`jsapi` 和 `skill`；因此搜尋 `map` 會讓 `personal-map` 取得比 `amap-jsapi-skill` 更強的詞彙符合。
- 人氣會以對數縮放並設上限。當查詢文字符合較弱時，高互動 Skills 的排名可能較低。
- 視呼叫者篩選條件和目前審核狀態而定，可疑或隱藏的審核狀態可能會將某個 Skill 從公開搜尋中移除。

發布者可發現性指引：

- 將使用者會實際搜尋的詞放入顯示名稱、摘要和標籤。只有在獨立 slug 權杖也是你想保留的穩定身分時才使用它。
- 不要只為追逐某個查詢而重新命名 slug，除非新的 slug 是更好的長期標準名稱。舊 slug 會成為重新導向別名，但標準 URL、顯示的 slug 和未來搜尋摘要會使用新的 slug。
- 重新命名別名會保留舊 URL，以及透過登錄檔解析的安裝項目的解析能力，但搜尋排名會以重新命名已建立索引後的標準 Skill 中繼資料為依據。現有統計會保留在該 Skill 上。
...
- `sort`（選用）：`updated`（預設）、`recommended`（別名：`default`）、`createdAt`（別名：`newest`）、`downloads`、`stars`（別名：`rating`）、舊版安裝別名 `installsCurrent`/`installs`/`installsAllTime` 會對應到 `downloads`、`trending`
- `nonSuspiciousOnly`（選用）：`true` 可隱藏可疑（`flagged.suspicious`）Skills
- `nonSuspicious`（選用）：`nonSuspiciousOnly` 的舊版別名
...
- `recommended` 使用互動和近期性訊號。
- `trending` 依過去 7 天的安裝量排名（以遙測為基礎）。
- `createdAt` 對新 Skill 爬取是穩定的；`updated` 會在既有 Skills 重新發布時變更。
- 當 `nonSuspiciousOnly=true` 時，游標式排序可能在一頁中傳回少於 `limit` 個項目，因為可疑 Skills 會在頁面擷取後被篩除。
- 存在時使用 `nextCursor` 繼續分頁。頁面較短本身不表示結果已結束。
...
- 由擁有者重新命名/合併流程建立的舊 slug 會解析為標準 Skill。
...
- 只有在 Skill 已被標記，或擁有者正在檢視它時，才會包含 `moderation`。
...
- 擁有者和審核員可以存取隱藏 Skills 的審核詳細資訊。
- 公開呼叫者只會對已標記且可見的 Skills 取得 `200`。
- 證據會對公開呼叫者遮罩，且只會為擁有者/審核員包含原始片段。

### `POST /api/v1/skills/{slug}/report`

回報 Skill 以供審核員審查。回報屬於 Skill 層級，可選擇連結
...
`note` 對 `confirmed` 和 `dismissed` 為必要；將
`status` 設回 `open` 時可以省略。對已分類的
回報傳入 `finalAction: "hide"`，即可在同一個可稽核工作流程中隱藏該 Skill。
...
- `version.security` 會在可用時包含標準化掃描驗證狀態和掃描器詳細資訊
  （VirusTotal + LLM）。

### `GET /api/v1/skills/{slug}/scan`

傳回某個 Skill 版本的安全掃描驗證詳細資訊。
...
- 查詢歷史版本時，請先檢查 `moderation.matchesRequestedVersion` 和 `moderation.sourceVersion`，再將 `moderation` 和 `security` 視為相同版本情境。
...
- 佇列中時會回傳 `queue.queuedAhead` 和 `queue.position`，讓用戶端可以顯示有多少優先手動掃描排在該請求前方。非常大的佇列會受到限制，並以 `queuedAheadIsEstimate: true` 回報。
...
- 回傳確切提交版本的已儲存掃描結果，包括被封鎖或隱藏的版本。
- `kind` 預設為 `skill`；外掛/套件掃描請使用 `kind=plugin`。
- 回傳與掃描請求下載相同的 ZIP 形狀。

### `POST /api/v1/skills/-/scan/batch`

僅限管理員使用的標準批次重新掃描路由。它接受與舊版 `POST /api/v1/skills/-/rescan-batch` 相同的承載形狀。
...
- 只有在所選版本具有產生的 Skill Card、未被審核判定為惡意軟體而封鎖，且 ClawScan 驗證為 clean 時，`ok` 才會是 `true`。
- 技能身分、發布者身分和所選版本中繼資料是頂層信封欄位（`slug`、`displayName`、`publisherHandle`、`version`、`resolvedFrom`、`tag`、`createdAt`），因此 shell 自動化可以不拆開巢狀包裝器就讀取它們。
- `security` 是頂層 ClawScan/security 判定。自動化應以 `ok`、`decision`、`reasons` 和 `security.status` 作為依據。
- `security.signals` 包含支援性的掃描器證據，例如 `staticScan`、`virusTotal` 和 `skillSpector`。
- `security.signals.dependencyRegistry` 為了 v1 回應相容性而保留，但相依性登錄存在性掃描器已停用，且此鍵一律為 `null`。
...
- 結果以項目為單位；單一缺少的技能或版本不會使整個回應失敗。
- 回應僅包含安全資訊。它不包含 Skill Card 資料、產生的卡片狀態、成品檔案清單或詳細掃描器承載。
- `security.signals` 僅包含狀態層級的支援證據；如需完整掃描器詳細資料，請使用 `/scan` 或 ClawHub security-audit 頁面。
- `security.signals.dependencyRegistry` 為了 v1 回應相容性而保留，但相依性登錄存在性掃描器已停用，且此鍵一律為 `null`。
- Skill Card 缺失不會影響此端點的 `ok`、`decision` 或 `reasons`；當用戶端需要卡片內容時，應在本機讀取已安裝的 `skill-card.md`。
- 需要單一技能 Skill Card 驗證信封時使用 `/verify`，需要產生的卡片 Markdown 時使用 `/card`，需要詳細掃描器資料時使用 `/scan`。
...
- `category`（選填）：外掛類別篩選器。僅在請求限定於外掛套件（`/api/v1/plugins`、`/api/v1/code-plugins`、`/api/v1/bundle-plugins`，或帶有 `family=code-plugin`/`family=bundle-plugin` 的套件端點）時支援。受控類別與舊版 v1 篩選器別名記錄於 `GET /api/v1/plugins` 下。

注意事項：

- `family`、`channel`、`isOfficial`、`featured`、`highlightedOnly` 或 `sort` 的無效值會傳回 `400`。未知的查詢參數會被忽略。
- `GET /api/v1/code-plugins` 和 `GET /api/v1/bundle-plugins` 仍然是固定 family 的別名。
- Skill 項目仍由 Skill 登錄檔支援，且仍只能透過 `POST /api/v1/skills` 發布。
- `POST /api/v1/packages` 仍然僅適用於 code-plugin 和 bundle-plugin 發布。
- 匿名呼叫者只能看到公開套件通道。
...
- `channel`（選填）：`official`、`community` 或 `private`
- `isOfficial`（選填）：`true` 或 `false`
- `category`（選填）：外掛類別篩選器。僅在請求限定於外掛套件時支援。受控類別與舊版 v1 篩選器別名記錄於 `GET /api/v1/plugins` 下。
...
在統一的 `/api/v1/packages` 端點上，它僅限外掛；Skill 目錄請使用
...
- 主體：ZIP 封存檔。
- 每個匯出的 Skill 以 `{publisher}/{slug}/` 為根目錄。
- 託管的 Skills 包含最新儲存版本檔案，並列於 `_manifest.json` 中，且帶有 `sourceRef: "public-clawhub"`。
- 目前由 GitHub 支援且掃描結果為 `clean` 或 `suspicious` 的 Skills 會包含 `_source_handoff.json`，其中有 `sourceRef: "public-github"`、repo、commit、path、內容雜湊與封存 URL。它們不包含 ClawHub 託管的原始碼檔案。
...
- `limit`（選填）：整數（1-100）
- `isOfficial`（選填）：`true` 或 `false`
- `category`（選填）：外掛分類篩選器。目前值為：
  `channels`、`models`、`memory`、`context`、`voice`、`media`、`web`、
  `tools`、`runtime`、`gateway`、`security`、`other`。

注意事項：

- 也接受 `GET /api/v1/plugins` 文件中記載的舊版 v1 篩選別名。
- 分類篩選是真正的 API 篩選器，由外掛分類摘要列支援，
  而不是搜尋查詢改寫。
- 結果會依相關性順序傳回，且目前不分頁。
...
傳回一個套件版本，包含檔案中繼資料、相容性、驗證、
成品中繼資料和掃描資料。

注意事項：

- `version.artifact.kind` 對舊世界套件封存檔為 `legacy-zip`，
  對 ClawPack 支援的版本為 `npm-pack`。
...
- `version.sha256hash` 是供舊用戶端使用的已棄用相容性中繼資料。它會雜湊
  ...
  正規版本成品。
- 當掃描資料存在時，會包含 `version.vtAnalysis`、`version.llmAnalysis` 和
  `version.staticScan`。
- 私有套件會傳回 `404`，除非呼叫者可以讀取所屬發布者。

### `GET /api/v1/packages/{name}/versions/{version}/security`

傳回安裝用戶端所需的確切套件版本安全性與信任摘要。這是公開的 OpenClaw
取用介面，用於判斷已解析的版本是否可安裝。
...
- `package.name`、`package.displayName` 和 `package.family` 識別已解析的
  登錄套件。
- `release.releaseId`、`release.version` 和 `release.createdAt` 識別已評估的
  確切版本。
- 當版本成品已知時，會提供 `release.artifactKind`、`release.artifactSha256`、
  `release.npmIntegrity`、`release.npmShasum` 和 `release.npmTarballName`。
- `trust.scanStatus` 是從掃描器輸入和手動版本審核推導出的有效信任狀態。
- `trust.moderationState` 可為 null。當沒有手動版本審核時，其值為 `null`。
- `trust.blockedFromDownload` 是安裝封鎖訊號。OpenClaw 和其他安裝用戶端應在此值為
  `true` 時封鎖安裝，而不是從掃描器或審核欄位重新推導封鎖規則。
- `trust.reasons` 是面向使用者和稽核的說明清單。原因代碼是穩定、精簡的字串，
  例如 `manual:quarantined`、`scan:malicious` 和 `package:malicious`。
- `trust.pending` 表示一個或多個信任輸入仍在等待完成。
- `trust.stale` 表示信任摘要是從過時輸入計算而來，
  在做出高信心允許決策前，應視為需要重新整理。

注意事項：

- 此端點精確到版本。用戶端應在解析要安裝的套件版本後呼叫它，
  而不只是讀取最新套件中繼資料後呼叫。
- 私有套件會傳回 `404`，除非呼叫者可以讀取所屬發布者。
- 此端點刻意比擁有者/版主審核端點更窄。它公開的是安裝決策和公開說明，
...
- 這只追蹤遷移就緒度。它不會變更 OpenClaw 或產生 ClawPacks。
...
回報套件以供版主審查。回報屬於套件層級，可選擇連結到某個版本。
它們會進入審核佇列，但本身不會自動隱藏或封鎖下載；版主應使用版本審核來
核准、隔離或撤銷成品。
...
`note` 在 `confirmed` 和 `dismissed` 時為必填；將 `status` 設回 `open` 時可省略。對已確認的回報傳入 `finalAction: "quarantine"` 或 `finalAction: "revoke"`，即可在同一個可稽核工作流程中套用發布版本審核。
...
- `revoked`：在發布版本先前受信任後封鎖。

隔離和撤銷的發布版本會從成品下載路由傳回 `403`。每次變更都會寫入稽核記錄項目。
...
- 二進位檔案會傳回 `415`。
- 檔案大小限制：200KB。
- 擱置中的 VirusTotal 掃描不會阻擋讀取；惡意發布版本仍可能在其他位置被扣留。
- 私有套件會傳回 `404`，除非呼叫者可以讀取所屬發布者。
...
- 預設為最新發布版本。
- Skills 重新導向至 `GET /api/v1/download`。
- 外掛/套件封存檔是具有 `package/` 根目錄的 zip 檔案，讓舊版 OpenClaw 用戶端可繼續運作。
- 此路由維持僅 ZIP。它不會串流 ClawPack `.tgz` 檔案。
- 回應包含 `ETag`、`Digest`、`X-ClawHub-Artifact-Type` 和 `X-ClawHub-Artifact-Sha256` 標頭，用於解析器完整性檢查。
- 僅註冊表中繼資料不會注入下載的封存檔。
- 擱置中的 VirusTotal 掃描不會阻擋下載；惡意發布版本會傳回 `403`。
...
- 舊版僅 ZIP 的版本會刻意省略。
- `dist.tarball`、`dist.integrity` 和 `dist.shasum` 使用 npm 相容欄位，讓使用者可自行選擇將 npm 指向鏡像。
- 具範圍的套件 packument 同時支援 `/api/npm/@scope/name` 和 npm 的編碼 `/api/npm/@scope%2Fname` 請求路徑。
...
下載託管的 skill 版本 ZIP，或針對具有 `clean` 或 `suspicious` 掃描且沒有託管版本的目前 GitHub 支援 skill，傳回 GitHub 原始碼交接。
...
- 軟刪除版本會傳回 `410`。
- GitHub 支援的 skill 交接不會代理或鏡像位元組。JSON 回應包含 `sourceRef: "public-github"`、`repo`、`commit`、`path`、`contentHash` 和 `archiveUrl`；掃描/目前狀態是閘門，且不會作為成功承載中繼資料包含在內。
...
- 建議：使用含 `payload` JSON + `files[]` blob 的 `multipart/form-data`。
- 也接受含 `files`（基於 storageId）的 JSON 本文。
- 選用承載欄位：`ownerHandle`。存在時，API 會在伺服器端解析該發布者，並要求執行者具有發布者存取權。
- 選用承載欄位：`migrateOwner`。與 `ownerHandle` 搭配為 `true` 時，如果執行者在目前和目標發布者兩邊都是管理員/擁有者，現有 skill 可移至該擁有者。若沒有這個選擇加入，擁有者變更會被拒絕。
...
- 允許的表單欄位為 `payload`、重複的 `files` blob，或一個 `clawpack` tarball 參照。`clawpack` 可以是 `.tgz` blob，或由 upload-url 流程傳回的 storage id。分段 storage-id 發布也必須包含該上傳 URL 一併傳回的 `clawpackUploadTicket`。
- 使用 `files` 或 `clawpack` 其中之一，絕不可在同一請求中同時使用。
- JSON 本文和呼叫者提供的 `payload.files` / `payload.artifact` 中繼資料會被拒絕。
- 直接 multipart 發布請求上限為 18MB。ClawPack tarball 可使用 upload-url 流程，最高可達 120MB tarball 上限。
- 選用承載欄位：`ownerHandle`。存在時，只有管理員可以代表該擁有者發布。
...
- 只有 `openclaw` 組織發布者和目前 `openclaw` 組織成員的個人發布者，可以發布到 `official` 頻道。
- 代表他人發布仍會依目標擁有者帳戶驗證 official-channel 資格。
...
存在時，`reason` 會儲存為 skill 審核備註並複製到稽核記錄。
擁有者發起的軟刪除會保留 slug 30 天，之後該 slug 可由另一個發布者認領。刪除回應在此到期情況適用時包含 `slugReservedUntil`。
...
僅限管理員。確保 handle 存在一個組織發布者。如果 handle 仍指向舊版共享使用者/個人發布者，端點會先將其遷移為組織發布者。
對於新建立的組織，請提供 `memberHandle`；執行的管理員不會被加入為成員。
...
已驗證的自助式組織發布者建立。建立新的組織發布者，並將呼叫者加入為擁有者。此端點不會遷移現有使用者/個人 handle，也不會將發布者標記為受信任/official。
...
僅限管理員。為正當擁有者保留根 slug 和套件名稱，而不發布發布版本。套件名稱會成為沒有發布版本列的私有預留位置套件，因此同一擁有者稍後可將真正的 code-plugin 或 bundle-plugin 發布版本發布到該名稱。
...
僅限管理員。為已驗證的替代 GitHub OAuth 主體復原個人發布者，而不編輯 Convex Auth 帳戶列。請求必須命名兩個不可變的 GitHub 提供者帳戶 ID；可變 handle 只會用作面向操作員的保護檢查。

端點預設為試執行。套用復原需要在工作人員獨立驗證兩個
GitHub 主體之間的連續性後，設定 `dryRun: false` 和
`confirmIdentityVerified: true`。當目的地使用者目前的個人
發布者擁有 Skills、套件或 GitHub Skill 來源時，復原會以失敗關閉。
復原也會遷移已復原發布者的 Skills、Skill slug 別名、套件、套件檢查器警告，以及衍生搜尋摘要列的舊版 `ownerUserId` 欄位，讓
直接擁有者路徑與新的發布者權限一致。已復原 handle 的有效受保護 handle
保留也會重新指派給替代使用者，讓之後的個人資料同步無法還原前使用者的競爭權限。每個主要資料表在每個套用交易中限制為
100 列；較大的復原必須先使用可續傳的擁有者遷移。
GitHub Skill 來源的範圍限定於發布者，並回報為已檢查，而不是重寫。

- 主體：`{ "handle": "gingiris", "nextUserHandle": "gingiris-1031", "previousGitHubProviderAccountId": "123", "nextGitHubProviderAccountId": "456", "reason": "已驗證 issue #2555 的帳戶連續性", "confirmIdentityVerified": true, "dryRun": false }`
- 回應：`{ "ok": true, "dryRun": false, "recovered": true, "publisherId": "...", "handle": "gingiris", "previousUser": { "userId": "...", "handle": "gingiris", "nextHandle": "gingiris-recovered", "githubProviderAccountId": "123", "authAccountCount": 1 }, "nextUser": { "userId": "...", "handle": "gingiris-1031", "nextHandle": "gingiris", "githubProviderAccountId": "456", "authAccountCount": 1 }, "retiredPersonalPublisher": null, "resourceOwnerMigration": { "limitPerTable": 100, "skills": 1, "skillSlugAliases": 1, "packages": 0, "packageInspectorWarnings": 0, "githubSourcesChecked": 1, "handleReservations": 1 }, "identityVerified": true, "reason": "已驗證 issue #2555 的帳戶連續性" }`
