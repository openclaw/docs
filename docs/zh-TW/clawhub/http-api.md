...
x-i18n:
    generated_at: "2026-07-04T15:07:11Z"
    model: gpt-5.5
...
第三方目錄可以使用公開讀取端點來列出或搜尋 ClawHub Skills。請快取結果、遵守 `429`/`Retry-After`、將使用者連回正式 ClawHub 條目（`https://clawhub.ai/<owner>/skills/<slug>`），並避免暗示 ClawHub 認可該第三方網站。請勿嘗試在公開 API 介面之外鏡像隱藏、私人或因審核而封鎖的內容。

Web slug 捷徑會跨登錄檔家族解析，但 API 用戶端應使用讀取端點傳回的正式 URL，而不是自行重建路由優先順序。
...
- 如果權杖缺失/無效，行為會退回為依 IP 強制執行。
- 當伺服器知道原因時，已驗證的寫入端點不應傳回單純的 `Unauthorized`。缺少權杖、無效/已撤銷的權杖，以及已刪除/遭停權/已停用的帳號，都應取得可採取行動的文字，讓命令列介面用戶端可以告知使用者是什麼阻擋了他們。
...
- `X-RateLimit-Remaining` / `RateLimit-Remaining`：存在時表示精確剩餘配額。
...
- ClawHub 會使用受信任轉送標頭在邊緣識別用戶端 IP。
- 如果沒有可用的受信任用戶端 IP，匿名請求會使用只依速率限制種類劃分範圍的後備 bucket。這些後備 bucket 不包含呼叫者提供的路徑、slug、套件名稱、版本、查詢字串或其他成品參數。

## 錯誤回應

公開 v1 錯誤回應是純文字，`content-type: text/plain; charset=utf-8`。
這包括驗證失敗（`400`）、缺少公開資源（`404`）、驗證與權限失敗（`401`/`403`）、速率限制（`429`），以及被封鎖的下載。用戶端應將回應主體讀作人類可讀的字串。未知的查詢參數會為了相容性而被忽略，但已識別且值無效的查詢參數會傳回 `400`。
...
- 結果會依相關性順序傳回（嵌入相似度 + 精確 slug/名稱 token 加權 + 小幅人氣先驗）。
- 相關性比人氣更重要。精確的 slug 或顯示名稱 token 符合，可能會排在參與度高得多但符合較寬鬆的結果之前。
- ASCII 文字會依單字與標點邊界切分 token。例如，`personal-map` 包含獨立的 `map` token，而 `amap-jsapi-skill` 包含 `amap`、`jsapi` 和 `skill`；因此搜尋 `map` 時，`personal-map` 會比 `amap-jsapi-skill` 有更強的詞彙符合。
- 人氣會以對數縮放並設上限。當查詢文字符合較弱時，高參與度 Skills 可能排名較低。
- 可疑或隱藏的審核狀態可能會依呼叫者篩選器與目前審核狀態，將某項 Skill 從公開搜尋中移除。

發布者可發現性指引：

- 將使用者會實際搜尋的詞彙放在顯示名稱、摘要與標籤中。只有當獨立 slug token 也是你想保留的穩定身分時，才使用它。
- 除非新 slug 是更好的長期正式名稱，否則不要只為了追逐某個查詢而重新命名 slug。舊 slug 會變成重新導向別名，但正式 URL、顯示的 slug 與未來搜尋摘要會使用新 slug。
- 重新命名別名會為舊 URL 以及透過登錄檔解析的安裝保留解析能力，但搜尋排名會根據重新命名完成索引後的正式 Skill 中繼資料。現有統計資料會保留在該 Skill 上。
- 如果某項 Skill 意外不可見，請先在登入狀態下使用 `clawhub inspect @owner/slug` 檢查審核狀態，再變更排名相關中繼資料。
...
- `recommended` 使用參與度與近期性訊號。
- `trending` 依過去 7 天的安裝數排名（基於遙測）。
- `createdAt` 對新 Skill 爬取是穩定的；`updated` 會在現有 Skills 重新發布時變更。
- 當 `nonSuspiciousOnly=true` 時，基於游標的排序可能會在一頁中傳回少於 `limit` 的項目，因為可疑 Skills 會在頁面擷取後被篩掉。
- 存在時使用 `nextCursor` 繼續分頁。短頁本身不代表結果已結束。
...
- 由擁有者重新命名/合併流程建立的舊 slug 會解析到正式 Skill。
...
- 只有在 Skill 被標記或擁有者正在檢視時，才會包含 `moderation`。
...
- 對公開呼叫者，證據會經過遮蔽，且只有擁有者/審核人員才會包含原始片段。

### `POST /api/v1/skills/{slug}/report`

回報 Skill 以供審核人員審查。回報是 Skill 層級，可選擇連結到版本，並會送入 Skill 回報佇列。
...
`note` 對 `confirmed` 和 `dismissed` 為必填；將 `status` 設回 `open` 時可省略。傳入 `finalAction: "hide"` 搭配已分流處理的回報，即可在同一個可稽核工作流程中隱藏該 Skill。
...
- 包含正規化的驗證狀態，以及掃描器特定的詳細資料。
- 只有在掃描器產生明確判定（`clean`、`suspicious` 或 `malicious`）時，`security.hasScanResult` 才會是 `true`。
- `moderation` 是從最新版本衍生的目前 Skill 層級審核快照。
- 查詢歷史版本時，請先檢查 `moderation.matchesRequestedVersion` 和 `moderation.sourceVersion`，再將 `moderation` 和 `security` 視為相同的版本脈絡。

### `POST /api/v1/skills/-/scan`

用於新 ClawScan 作業的需驗證身份提交端點。
...
- 掃描請求酬載和可下載報告會在保留期間後從掃描請求儲存區過期。
- 已發佈掃描需要擁有者/發布者管理權限，或平台審核者/管理員權限。
- 已發佈掃描只有在 `update: true` 且掃描成功完成時才會寫回。
- 回應為 `202`，並帶有 `{ "ok": true, "scanId": "...", "jobId": "...", "status": "queued", "sourceKind": "published", "update": false, "queue": { "queuedAhead": 0, "queuedAheadIsEstimate": false, "position": 1, "running": 0, "runningIsEstimate": false, "note": "Scans are asynchronous and may take time to complete." } }`。
- 掃描作業是非同步的。手動掃描請求會優先於一般發佈/回填工作，但完成時間仍取決於工作程序可用性。
...
- 傳回排隊中/執行中/成功/失敗狀態。
- 排隊期間會傳回 `queue.queuedAhead` 和 `queue.position`，讓用戶端顯示該請求前方有多少個優先手動掃描。非常大的佇列會受到限制，並以 `queuedAheadIsEstimate: true` 回報。
- 可用時，`report` 會包含 `clawscan`、`skillspector`、`staticAnalysis` 和 `virustotal` 區段。
- 失敗的掃描作業會傳回帶有 `lastError` 的 `status: "failed"`。

### `GET /api/v1/skills/-/scan/{scanId}/download`

需驗證身份的報告封存端點。

- 需要成功的掃描；非終止狀態的掃描會傳回 `409`。
- 傳回包含 `manifest.json`、`clawscan.json`、`skillspector.json`、`static-analysis.json`、`virustotal.json` 和 `README.md` 的 ZIP。

### `GET /api/v1/skills/-/scan/download/{name}?version=<version>&kind=skill|plugin`

用於已提交版本的需驗證身份已儲存報告封存端點。

- 需要該 Skill 或外掛的擁有者/發布者管理權限，或平台審核者/管理員權限。
- 傳回確切已提交版本的已儲存掃描結果，包括被封鎖或隱藏的版本。
- `kind` 預設為 `skill`；外掛/套件掃描請使用 `kind=plugin`。
- 傳回與掃描請求下載相同形狀的 ZIP。

### `POST /api/v1/skills/-/scan/batch`

僅限管理員的標準批次重新掃描路由。它接受與舊版 `POST /api/v1/skills/-/rescan-batch` 相同形狀的酬載。

### `POST /api/v1/skills/-/scan/batch/status`

僅限管理員的標準批次狀態路由。它接受 `{ "jobIds": ["..."] }`，並傳回與舊版 `POST /api/v1/skills/-/rescan-batch/status` 相同的彙總計數器。
...
- 只有在所選版本具有已產生的 Skill Card、未被審核機制因惡意軟體封鎖，且 ClawScan 驗證為乾淨時，`ok` 才會是 `true`。
- Skill 身分、發布者身分和所選版本中繼資料是頂層封套欄位（`slug`、`displayName`、`publisherHandle`、`version`、`resolvedFrom`、`tag`、`createdAt`），因此 shell 自動化可以在不拆解巢狀包裝器的情況下讀取它們。
- `security` 是頂層 ClawScan/security 判定。自動化應依據 `ok`、`decision`、`reasons` 和 `security.status`。
- `security.signals` 包含支援性的掃描器證據，例如 `staticScan`、`virusTotal` 和 `skillSpector`。
- `security.signals.dependencyRegistry` 會保留以相容 v1 回應，但相依性登錄檔存在性掃描器已退役，且此鍵一律為 `null`。
- 只有在 ClawHub 於發佈或匯入期間解析並儲存 GitHub repo/ref/commit/path 時，`provenance` 才會是 `server-resolved-github-import`；否則為 `unavailable`。

### `POST /api/v1/skills/-/security-verdicts`

傳回精確 Skill 版本目前的精簡安全性判定。此
集合端點適用於已知道需要顯示哪些已安裝
...
- 結果以項目為單位；單一缺少的 Skill 或版本不會讓整個回應失敗。
- 回應僅限安全性資訊。不包含 Skill Card 資料、產生的卡片狀態、成品檔案清單或詳細掃描器酬載。
- `security.signals` 只包含狀態層級的支援證據；完整掃描器詳細資料請使用 `/scan` 或 ClawHub security-audit 頁面。
- `security.signals.dependencyRegistry` 會保留以相容 v1 回應，但相依性登錄檔存在性掃描器已退役，且此鍵一律為 `null`。
- Skill Card 缺失不會影響此端點的 `ok`、`decision` 或 `reasons`；用戶端需要卡片內容時，應在本機讀取已安裝的 `skill-card.md`。
- 需要單一 Skill 的 Skill Card 驗證封套時使用 `/verify`，需要產生的卡片 Markdown 時使用 `/card`，需要詳細掃描器資料時使用 `/scan`。
...
- `category`（選填）：外掛類別篩選器。僅在請求範圍限定為外掛套件（`/api/v1/plugins`、`/api/v1/code-plugins`、`/api/v1/bundle-plugins`，或帶有 `family=code-plugin`/`family=bundle-plugin` 的套件端點）時支援。受控類別和舊版 v1 篩選器別名記錄於 `GET /api/v1/plugins` 下。
...
- `channel`（選填）：`official`、`community` 或 `private`
- `isOfficial`（選填）：`true` 或 `false`
- `category`（選填）：外掛類別篩選器。僅在請求範圍限定為外掛套件時支援。受控類別和舊版 v1 篩選器別名記錄於 `GET /api/v1/plugins` 下。
...
在統一 `/api/v1/packages` 端點上，它僅限外掛；技能目錄請使用
`/api/v1/skills?sort=trending`。

舊版別名不得作為已儲存或作者宣告的類別值。

### `GET /api/v1/skills/export`

匯出最新公開 skills 的批次資料以供離線分析。
...
- Hosted skills 包含最新儲存版本檔案，並列於
  `_manifest.json` 中，且 `sourceRef: "public-clawhub"`。
- 目前由 GitHub 支援且掃描結果為 `clean` 或 `suspicious` 的 skills 包含
  `_source_handoff.json`，內含 `sourceRef: "public-github"`、repo、commit、path、
  content hash 和 archive URL。它們不包含 ClawHub 託管的來源檔案。
...
- 類別篩選是真正的 API 篩選器，由外掛類別摘要資料列支援，而不是搜尋查詢重寫。
- 結果會依相關性順序傳回，目前不分頁。
- 外掛搜尋的瀏覽器 UI 排序控制項會重新排序已載入的相關性結果，符合目前的 `/skills` 瀏覽行為。
...
- 需要套件擁有者、組織發布者擁有者/管理員、平台審核員或平台管理員的 API 權杖。
...
- `version.artifact.kind` 對舊世界套件封存檔為 `legacy-zip`，對 ClawPack 支援的版本為 `npm-pack`。
- ClawPack 版本包含與 npm 相容的 `npmIntegrity`、`npmShasum` 和 `npmTarballName` 欄位。
- `version.sha256hash` 是供舊用戶端使用的已棄用相容性中繼資料。它會雜湊 `/api/v1/packages/{name}/download` 傳回的精確 ZIP 位元組。現代用戶端應使用 `version.artifact.sha256`，它會識別標準版本成品。
- 當掃描資料存在時，會包含 `version.vtAnalysis`、`version.llmAnalysis` 和 `version.staticScan`。
- 私有套件會傳回 `404`，除非呼叫者可以讀取擁有該套件的發布者。

### `GET /api/v1/packages/{name}/versions/{version}/security`

傳回安裝用戶端所需的精確套件版本安全性與信任摘要。這是公開的 OpenClaw 使用介面，用於判斷已解析的版本是否可以安裝。
...
- `release.releaseId`、`release.version` 和 `release.createdAt` 識別已評估的精確版本。
- `release.artifactKind`、`release.artifactSha256`、`release.npmIntegrity`、`release.npmShasum` 和 `release.npmTarballName` 在版本成品已知時存在。
- `trust.scanStatus` 是從掃描器輸入和手動版本審核衍生的有效信任狀態。
- `trust.moderationState` 可為 null。當不存在手動版本審核時，它為 `null`。
- `trust.blockedFromDownload` 是安裝封鎖訊號。OpenClaw 和其他安裝用戶端應在此值為 `true` 時封鎖安裝，而不是從掃描器或審核欄位重新推導封鎖規則。
- `trust.reasons` 是面向使用者和稽核的說明清單。原因代碼是穩定、精簡的字串，例如 `manual:quarantined`、`scan:malicious` 和 `package:malicious`。
- `trust.pending` 表示一或多個信任輸入仍在等待完成。
- `trust.stale` 表示信任摘要是根據過期輸入計算而來，在做出高信心允許決策前應視為需要重新整理。

注意事項：

- 此端點精確到版本。用戶端應在解析出想要安裝的套件版本後呼叫它，而不是只在讀取最新套件中繼資料後呼叫。
- 私有套件會傳回 `404`，除非呼叫者可以讀取擁有該套件的發布者。
- 此端點刻意比擁有者/審核員審核端點更窄。它公開安裝決策和公開說明，而不公開回報者身分、回報內容、私有證據或內部審查時間軸。
...
- ClawPack 版本會傳回 `npm-pack` 成品、npm integrity 欄位、`tarballUrl` 和舊版 ZIP 相容性 URL。
...
- `packageName` 會依 npm 名稱正規化；對於已規劃的遷移，套件可以不存在。
- 這只追蹤遷移就緒狀態。它不會變更 OpenClaw 或產生 ClawPack。

### `GET /api/v1/packages/moderation/queue`

用於套件版本審查佇列的審核員/管理員端點。
...
回報套件以供審核員審查。回報屬於套件層級，可選擇連結到某個版本。它們會進入審核佇列，但本身不會自動隱藏或封鎖下載；審核員應使用版本審核來核准、隔離或撤銷成品。
...
`confirmed` 和 `dismissed` 需要 `note`；將 `status` 設回 `open` 時可以省略。對已確認的檢舉傳入 `finalAction: "quarantine"` 或 `finalAction: "revoke"`，即可在同一個可稽核工作流程中套用發行審核。
...
- `approved`：已手動審查並允許。
- `quarantined`：封鎖，等待後續處理。
- `revoked`：在發行版先前受信任後封鎖。

隔離和撤銷的發行版會從成品下載路由回傳 `403`。
每次變更都會寫入一筆稽核記錄。
...
- 二進位檔案會回傳 `415`。
- 檔案大小限制：200KB。
- 等待中的 VirusTotal 掃描不會封鎖讀取；惡意發行版仍可能在其他地方被保留不提供。
...
- 外掛/套件封存檔是具有 `package/` 根目錄的 zip 檔，因此舊版 OpenClaw 用戶端可以繼續運作。
- 此路由維持僅支援 ZIP。它不會串流 ClawPack `.tgz` 檔案。
- 回應包含 `ETag`、`Digest`、`X-ClawHub-Artifact-Type` 和 `X-ClawHub-Artifact-Sha256` 標頭，以供解析器完整性檢查使用。
- 僅註冊表使用的中繼資料不會注入下載的封存檔。
- 等待中的 VirusTotal 掃描不會封鎖下載；惡意發行版會回傳 `403`。
...
- 有意省略僅有舊版 ZIP 的版本。
- `dist.tarball`、`dist.integrity` 和 `dist.shasum` 使用 npm 相容欄位，因此使用者可以視需要將 npm 指向該鏡像。
- 具作用域套件的 packument 同時支援 `/api/npm/@scope/name` 和 npm 的編碼 `/api/npm/@scope%2Fname` 請求路徑。
...
下載託管的技能版本 ZIP，或針對目前由 GitHub 支援、掃描結果為 `clean` 或 `suspicious` 且沒有託管版本的技能，回傳 GitHub 原始碼交接資訊。
...
- 軟刪除的版本會回傳 `410`。
- 由 GitHub 支援的技能交接不會代理或鏡像位元組。JSON 回應包含 `sourceRef: "public-github"`、`repo`、`commit`、`path`、`contentHash` 和 `archiveUrl`；掃描/目前狀態是閘門，不會作為成功酬載中繼資料包含在內。
- 下載統計會依 UTC 日計算不重複身分（API 權杖有效時使用 `userId`，否則使用 IP）。
...
- 選填酬載欄位：`ownerHandle`。出現時，API 會在伺服器端解析該發布者，並要求執行者具有發布者存取權。
- 選填酬載欄位：`migrateOwner`。當與 `ownerHandle` 一起為 `true` 時，如果執行者同時是目前與目標發布者的管理員/擁有者，現有技能可以移至該擁有者。沒有此選擇加入時，擁有者變更會被拒絕。
...
- 允許的表單欄位是 `payload`、重複的 `files` blob，或一個 `clawpack` tarball 參照。`clawpack` 可以是 `.tgz` blob，或由 upload-url 流程回傳的 storage id。已暫存 storage-id 的發布也必須包含該上傳 URL 一併回傳的 `clawpackUploadTicket`。
- 在同一個請求中只能使用 `files` 或 `clawpack`，不可同時使用兩者。
- JSON 主體和呼叫者提供的 `payload.files` / `payload.artifact` 中繼資料會被拒絕。
- 直接 multipart 發布請求上限為 18MB。ClawPack tarball 可透過 upload-url 流程使用，最高到 120MB tarball 上限。
- 選填酬載欄位：`ownerHandle`。出現時，只有管理員可以代表該擁有者發布。

驗證重點：

- `family` 必須是 `code-plugin` 或 `bundle-plugin`。
- 外掛套件需要 `openclaw.plugin.json`。ClawPack `.tgz` 上傳必須在 `package/openclaw.plugin.json` 包含它。
- 程式碼外掛需要 `package.json`、原始碼 repo 中繼資料、原始碼 commit 中繼資料、設定架構中繼資料、`openclaw.compat.pluginApi` 和 `openclaw.build.openclawVersion`。
- `openclaw.hostTargets` 和 `openclaw.environment` 是選填中繼資料。
- 只有 `openclaw` 組織發布者和目前 `openclaw` 組織成員的個人發布者可以發布到 `official` 頻道。
- 代表他人發布仍會針對目標擁有者帳號驗證 official-channel 資格。
...
出現時，`reason` 會儲存為技能審核備註，並複製到稽核記錄。
擁有者發起的軟刪除會保留 slug 30 天，之後其他發布者即可宣告該 slug。
...
僅限管理員。確保 handle 存在一個組織發布者。如果該 handle 仍指向舊版共享使用者/個人發布者，端點會先將其遷移成組織發布者。
...
已驗證使用者自助建立組織發布者。建立新的組織發布者並將呼叫者新增為擁有者。此端點不會遷移現有使用者/個人 handle，也不會將發布者標記為受信任/官方。
...
僅限管理員。為正當擁有者保留根 slug 和套件名稱，而不發布發行版。套件名稱會變成沒有發行版資料列的私有佔位套件，因此同一個擁有者稍後可以將真正的 code-plugin 或 bundle-plugin 發行版發布到該名稱。
...
僅限管理員。為已驗證的替代 GitHub OAuth 主體復原個人發布者，而不編輯 Convex Auth 帳號資料列。請求必須指定兩個不可變的 GitHub provider account id；可變 handle 僅作為面向操作員的防護。

端點預設為 dry-run。套用復原需要在工作人員獨立驗證兩個
GitHub 主體之間的連續性後，設定 `dryRun: false` 和
`confirmIdentityVerified: true`。當目的使用者目前的個人發布者擁有技能、套件或 GitHub 技能來源時，復原會以失敗關閉。
復原也會遷移已復原發布者的技能、技能 slug 別名、套件、套件檢查器警告，以及衍生搜尋摘要列中的舊版 `ownerUserId` 欄位，讓直接擁有者路徑與新的發布者權限一致。已復原 handle 的有效受保護 handle 保留也會重新指派給替代使用者，讓後續個人檔案同步無法還原前使用者的競爭權限。每個主要資料表在每次套用交易中限制為
100 列；較大的復原必須先使用可續傳的擁有者遷移。
GitHub 技能來源屬於發布者範圍，並會回報為已檢查，而不是被重寫。
...
`POST /api/cli/upload-url` 會傳回 `uploadUrl` 和 `uploadTicket`。暫存 ClawPack tarball 的套件發布必須將產生的儲存體 ID 作為
