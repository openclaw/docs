...
舊版 `/api/...` 和 `/api/cli/...` 保留作為相容性用途（請參閱 `DEPRECATIONS.md`）。
OpenAPI：`/api/v1/openapi.json`。

## 公開目錄重用

第三方目錄可以使用公開讀取端點來列出或搜尋 ClawHub Skills。請快取結果、遵守 `429`/`Retry-After`、將使用者連回標準 ClawHub 清單（`https://clawhub.ai/<owner>/skills/<slug>`），並避免暗示 ClawHub 為第三方網站背書。請勿嘗試在公開 API 表面之外鏡像隱藏、私有或遭審核封鎖的內容。

Web slug 捷徑會跨註冊表系列解析，但 API 用戶端應使用讀取端點傳回的標準 URL，而不是重建路由優先順序。
...
- 如果 token 缺失/無效，行為會回退到 IP 執行。
- 已驗證的寫入端點在伺服器知道原因時，不應傳回裸露的 `Unauthorized`。缺少 token、無效/已撤銷的 token，以及已刪除/被封鎖/停用的帳號，都應取得可操作的文字，讓命令列介面用戶端可以告知使用者是什麼阻止了他們。
...
- 如果存在 `Retry-After`，請在重試前等待該秒數。
- 使用加入抖動的退避，以避免同步重試。
- 如果缺少 `Retry-After`，請回退到 `RateLimit-Reset`（或從 `X-RateLimit-Reset` 計算）。
...
這包括驗證失敗（`400`）、缺少公開資源（`404`）、驗證和權限失敗（`401`/`403`）、速率限制（`429`）以及遭封鎖的下載。用戶端應將回應本文讀取為人類可讀的字串。未知查詢參數會因相容性而被忽略，但值無效的已識別查詢參數會傳回 `400`。
...
- `q`（必填）：查詢字串
- `limit`（選填）：整數
- `highlightedOnly`（選填）：`true` 以篩選為醒目標示的 Skills
...
- 結果會依相關性順序傳回（嵌入相似度 + 精確 slug/name token 加權 + 小幅人氣先驗）。
- 相關性比人氣更強。精確的 slug 或顯示名稱 token 比對，可以勝過互動明顯更強但比對較寬鬆的結果。
- ASCII 文字會依單字和標點邊界進行 token 化。例如，`personal-map` 包含獨立的 `map` token，而 `amap-jsapi-skill` 包含 `amap`、`jsapi` 和 `skill`；因此搜尋 `map` 會讓 `personal-map` 比 `amap-jsapi-skill` 取得更強的詞彙比對。
- 人氣會以對數縮放並設上限。高互動的 Skills 在查詢文字比對較弱時可能排名較低。
- 可疑或隱藏的審核狀態可能會依呼叫端篩選器和目前審核狀態，將某個 Skill 從公開搜尋中移除。

發布者可發現性指引：

- 將使用者會實際搜尋的詞放入顯示名稱、摘要和標籤中。只有在它同時也是你想保留的穩定身分時，才使用獨立的 slug token。
- 不要只是為了追逐某個查詢而重新命名 slug，除非新的 slug 是更好的長期標準名稱。舊 slug 會成為重新導向別名，但標準 URL、顯示的 slug，以及未來搜尋摘要都會使用新的 slug。
- 重新命名別名會保留舊 URL 和透過註冊表解析的安裝解析能力，但搜尋排名會以重新命名完成索引後的標準 Skill 中繼資料為基準。既有統計資料會保留在該 Skill 上。
- 如果某個 Skill 意外不可見，請先在登入時使用 `clawhub inspect @owner/slug` 檢查審核狀態，再變更與排名相關的中繼資料。
...
- `createdAt` 對新 Skill 抓取是穩定的；`updated` 會在既有 Skills 重新發布時變更。
- 當 `nonSuspiciousOnly=true` 時，基於游標的排序可能會在某頁傳回少於 `limit` 的項目，因為可疑 Skills 會在頁面擷取後被篩除。
- 存在 `nextCursor` 時，使用它繼續分頁。短頁本身並不代表結果已結束。
...
`note` 在 `confirmed` 和 `dismissed` 時為必填；將 `status` 設回 `open` 時可以省略。將 `finalAction: "hide"` 與已分流的回報一起傳入，可在同一個可稽核工作流程中隱藏該 Skill。
...
- 傳回已排隊／執行中／成功／失敗狀態。
- 排隊期間會傳回 `queue.queuedAhead` 和 `queue.position`，讓用戶端可以顯示此請求前方有多少個優先手動掃描。非常大的佇列會受到限制，並以 `queuedAheadIsEstimate: true` 回報。
- 可用時，`report` 會包含 `clawscan`、`skillspector`、`staticAnalysis` 和 `virustotal` 區段。
- 失敗的掃描作業會以 `lastError` 傳回 `status: "failed"`。
...
傳回 `clawhub skill verify` 使用的技能卡驗證封套。
...
- 只有在所選版本有已產生的技能卡、未被審核機制封鎖為惡意軟體，且 ClawScan 驗證為乾淨時，`ok` 才會是 `true`。
- 技能身分、發布者身分和所選版本中繼資料是頂層封套欄位（`slug`、`displayName`、`publisherHandle`、`version`、`resolvedFrom`、`tag`、`createdAt`），因此 Shell 自動化可讀取它們，而不必拆解巢狀包裝。
...
- `security.signals.dependencyRegistry` 保留用於 v1 回應相容性，但相依性登錄存在性掃描器已退役，而且此鍵一律為 `null`。
- 只有在 ClawHub 於發布或匯入期間解析並儲存 GitHub 儲存庫／參照／提交／路徑時，`provenance` 才會是 `server-resolved-github-import`；否則會是 `unavailable`。

### `POST /api/v1/skills/-/security-verdicts`

傳回精確技能版本的目前精簡安全性判定。此
集合端點適用於已知道需要顯示哪些已安裝
...
- `items` 必須包含 1 到 100 個不重複的 `{ slug, version }` 配對。
- 結果以項目為單位；單一缺少的技能或版本不會讓整個回應失敗。
- 回應僅包含安全性資訊。它不包含技能卡資料、已產生卡片狀態、成品檔案清單或詳細掃描器酬載。
- `security.signals` 只包含狀態層級的支援性證據；若要完整掃描器詳細資料，請使用 `/scan` 或 ClawHub 安全稽核頁面。
- `security.signals.dependencyRegistry` 保留用於 v1 回應相容性，但相依性登錄存在性掃描器已退役，而且此鍵一律為 `null`。
- 技能卡不存在不會影響此端點的 `ok`、`decision` 或 `reasons`；用戶端需要卡片內容時，應在本機讀取已安裝的 `skill-card.md`。
- 需要單一技能的技能卡驗證封套時使用 `/verify`，需要已產生卡片 Markdown 時使用 `/card`，需要詳細掃描器資料時使用 `/scan`。
...
- `category`（選用）：外掛分類篩選器。僅在請求範圍限定為外掛套件（`/api/v1/plugins`、
  `/api/v1/code-plugins`、`/api/v1/bundle-plugins`，或帶有
  `family=code-plugin`/`family=bundle-plugin` 的套件端點）時支援。受控分類和
...
- `GET /api/v1/code-plugins` 和 `GET /api/v1/bundle-plugins` 仍維持為固定系列別名。
- Skill 項目仍由 Skill 登錄支援，且仍只能透過 `POST /api/v1/skills` 發佈。
- `POST /api/v1/packages` 仍僅適用於 code-plugin 和 bundle-plugin 發佈。
- 匿名呼叫者只能看見公開套件頻道。
- 已驗證的呼叫者可以在清單/搜尋結果中看見其所屬發佈者的私人套件。
...
- `channel`（選用）：`official`、`community` 或 `private`
- `isOfficial`（選用）：`true` 或 `false`
- `category`（選用）：外掛分類篩選器。僅在請求範圍限定為外掛套件時支援。受控分類和舊版 v1
...
在統一 `/api/v1/packages` 端點上，它僅限外掛；若要使用 Skill 目錄，請使用
`/api/v1/skills?sort=trending`。

舊版別名不接受作為已儲存或作者宣告的分類值。

### `GET /api/v1/skills/export`

匯出最新公開 Skills 的批次資料，以供離線分析。
...
- 主體：ZIP 封存檔。
- 每個匯出的 Skill 都以 `{publisher}/{slug}/` 作為根目錄。
- 託管 Skill 包含最新儲存版本檔案，並在
  `_manifest.json` 中以 `sourceRef: "public-clawhub"` 列出。
- 目前由 GitHub 支援且掃描結果為 `clean` 或 `suspicious` 的 Skill 包含
  `_source_handoff.json`，其中有 `sourceRef: "public-github"`、repo、commit、path、
  content hash 和封存 URL。它們不包含 ClawHub 託管的原始碼檔案。
...
回傳一個套件版本，包括檔案中繼資料、相容性、
驗證、成品中繼資料與掃描資料。

注意事項：

- 舊世界套件封存檔的 `version.artifact.kind` 為 `legacy-zip`，
  由 ClawPack 支援的版本則為 `npm-pack`。
- ClawPack 版本包含 npm 相容的 `npmIntegrity`、`npmShasum` 與
  `npmTarballName` 欄位。
- `version.sha256hash` 是舊用戶端的已淘汰相容性中繼資料。它會雜湊
  `/api/v1/packages/{name}/download` 回傳的精確 ZIP 位元組。
  現代用戶端應使用 `version.artifact.sha256`，它會識別標準版本成品。
- 當掃描資料存在時，會包含 `version.vtAnalysis`、`version.llmAnalysis`
  與 `version.staticScan`。
- 私有套件會回傳 `404`，除非呼叫者可以讀取擁有該套件的發布者。

### `GET /api/v1/packages/{name}/versions/{version}/security`

回傳安裝用戶端所需的精確套件版本安全性與信任摘要。這是公開的
OpenClaw 消費介面，用於判斷已解析版本是否可以安裝。
...
- `package.name`、`package.displayName` 與 `package.family` 會識別已解析的
  登錄套件。
...
- `trust.scanStatus` 是從掃描器輸入與手動版本審核衍生出的有效信任狀態。
...
- `trust.reasons` 是面向使用者與稽核用途的說明清單。原因代碼是穩定、
  精簡的字串，例如 `manual:quarantined`、`scan:malicious` 與
  `package:malicious`。
- `trust.pending` 表示一個或多個信任輸入仍在等待完成。
- `trust.stale` 表示信任摘要是根據過期輸入計算而來，
  在做出高信心允許決策前應視為需要重新整理。
...
- 舊版套件版本會回傳 `legacy-zip` 成品與舊版 ZIP `downloadUrl`。
- ClawPack 版本會回傳 `npm-pack` 成品、npm integrity 欄位、
  `tarballUrl` 與舊版 ZIP 相容性 URL。
...
- `packageName` 會依 npm 名稱正規化；對規劃中的遷移而言，套件可以不存在。
...
回報套件以供版主審查。回報屬於套件層級，可選擇連結到版本。它們會送入審核佇列，
但本身不會自動隱藏或封鎖下載；版主應使用版本審核來
核准、隔離或撤銷成品。
...
`confirmed` 和 `dismissed` 必須提供 `note`；將 `status` 設回 `open` 時可省略。對已確認的報告傳入 `finalAction: "quarantine"` 或 `finalAction: "revoke"`，即可在同一個可稽核工作流程中套用版本審核。
...
隔離和撤銷的版本會從成品下載路由回傳 `403`。每次變更都會寫入稽核日誌項目。
...
下載託管的技能版本 ZIP，或對目前由 GitHub 支援、掃描結果為 `clean` 或 `suspicious` 且沒有託管版本的技能，回傳 GitHub 原始碼交接資訊。
...
- 選用酬載欄位：`ownerHandle`。存在時，API 會在伺服器端解析該發布者，並要求執行者具備發布者存取權。
- 選用酬載欄位：`migrateOwner`。當其為 `true` 且搭配 `ownerHandle` 時，若執行者同時是目前和目標發布者的管理員/擁有者，現有技能可移至該擁有者。若未明確選擇此選項，擁有者變更會被拒絕。
...
- 允許的表單欄位是 `payload`、重複的 `files` blob，或一個 `clawpack` tarball 參照。`clawpack` 可以是 `.tgz` blob，或上傳 URL 流程回傳的 storage id。分段 storage-id 發布也必須包含該上傳 URL 一併回傳的 `clawpackUploadTicket`。
- 在同一個請求中只能使用 `files` 或 `clawpack` 其中之一，不能同時使用。
- JSON 主體以及呼叫者提供的 `payload.files` / `payload.artifact` 中繼資料會被拒絕。
- 直接 multipart 發布請求上限為 18MB。ClawPack tarball 可透過上傳 URL 流程使用最高 120MB 的 tarball 上限。
- 選用酬載欄位：`ownerHandle`。存在時，只有管理員可代表該擁有者發布。

驗證重點：

- `family` 必須是 `code-plugin` 或 `bundle-plugin`。
- 外掛套件需要 `openclaw.plugin.json`。ClawPack `.tgz` 上傳必須在 `package/openclaw.plugin.json` 中包含它。
- 程式碼外掛需要 `package.json`、原始碼 repo 中繼資料、原始碼提交中繼資料、設定結構描述中繼資料、`openclaw.compat.pluginApi` 和 `openclaw.build.openclawVersion`。
- `openclaw.hostTargets` 和 `openclaw.environment` 是選用中繼資料。
- 只有 `openclaw` 組織發布者和目前 `openclaw` 組織成員的個人發布者可發布到 `official` 頻道。
...
存在時，`reason` 會儲存為技能審核註記，並複製到稽核日誌。
擁有者發起的軟刪除會保留 slug 30 天，之後該 slug 可由另一個發布者宣告。
...
僅限管理員。確保某個代號存在組織發布者。若該代號仍指向舊版共用使用者/個人發布者，端點會先將其遷移為組織發布者。
對新建立的組織，請提供 `memberHandle`；執行的管理員不會被加入為成員。
...
已驗證的自助式組織發布者建立。建立新的組織發布者，並將呼叫者加入為擁有者。此端點不會遷移現有使用者/個人代號，也不會將發布者標示為受信任/官方。
...
僅限管理員。為合法擁有者保留根 slug 和套件名稱，而不發布版本。套件名稱會成為沒有版本列的私有占位套件，因此同一擁有者稍後可將實際 code-plugin 或 bundle-plugin 版本發布到該名稱。
...
僅限管理員。為已驗證的替代 GitHub OAuth 主體復原個人發布者，而不編輯 Convex Auth 帳號列。請求必須指名兩個不可變的 GitHub provider account id；可變代號僅作為面向操作員的防護。

端點預設為試跑。套用復原需要在工作人員獨立驗證兩個 GitHub 主體之間的連續性後，設定 `dryRun: false` 和
`confirmIdentityVerified: true`。
當目的地使用者目前的個人發布者擁有 skills、packages 或 GitHub skill sources 時，復原會以失敗關閉。
復原也會遷移已復原發布者的 Skills、skill slug aliases、packages、package inspector warnings，以及衍生 search digest rows 的舊版 `ownerUserId` 欄位，讓 direct-owner paths 與新的 publisher authority 一致。已復原 handle 的有效 protected-handle
reservation 也會重新指派給替代使用者，讓後續的
profile synchronization 無法還原前使用者的競爭 authority。每個主要資料表在每次套用交易中限制為
100 列；較大的復原必須先使用可續傳的 owner migration。
GitHub skill sources 以發布者為範圍，會回報為已檢查，而不是被改寫。
...
- 兩個端點都需要 API token 驗證，且只適用於 skill 擁有者。
