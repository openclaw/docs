...
第三方目錄可以使用公開讀取端點來列出或搜尋 ClawHub skills。請快取結果、遵守 `429`/`Retry-After`、將使用者連回標準 ClawHub 清單（`https://clawhub.ai/<owner>/skills/<slug>`），並避免暗示 ClawHub 認可第三方網站。不要嘗試在公開 API 介面之外鏡像隱藏、私人或因審核而封鎖的內容。

網頁 slug 捷徑會跨登錄檔家族解析，但 API 用戶端應使用讀取端點傳回的標準 URL，而不是重建路由優先順序。
...
- 匿名請求：依 IP 執行。
- 已驗證請求（有效的 Bearer 權杖）：依使用者儲存桶執行。
- 如果權杖遺失/無效，行為會退回到 IP 執行。
- 當伺服器知道原因時，已驗證的寫入端點不應傳回裸露的 `Unauthorized`。遺失權杖、無效/已撤銷權杖，以及已刪除/已封鎖/已停用帳號，都應各自取得可採取行動的文字，讓命令列介面用戶端可以告訴使用者是什麼阻擋了他們。
...
- 只有在部署明確啟用信任轉送標頭時，才會使用受信任的用戶端 IP 標頭，包括 `cf-connecting-ip`。
- ClawHub 使用受信任的轉送標頭在邊緣識別用戶端 IP。
- 如果沒有可用的受信任用戶端 IP，匿名請求會使用只依速率限制種類限定範圍的後援儲存桶。這些後援儲存桶不包含呼叫端提供的路徑、slug、套件名稱、版本、查詢字串或其他成品參數。

## 錯誤回應

公開 v1 錯誤回應是純文字，並帶有 `content-type: text/plain; charset=utf-8`。
這包括驗證失敗（`400`）、遺失的公開資源（`404`）、驗證與權限失敗（`401`/`403`）、速率限制（`429`）以及被封鎖的下載。用戶端應將回應主體讀取為人類可讀的字串。為了相容性，未知的查詢參數會被忽略，但已辨識的查詢參數若具有無效值，會傳回 `400`。
...
- 相關性強於熱門度。精確的 slug 或顯示名稱 token 符合項，可以排在互動明顯更強但符合較寬鬆的結果之前。
- ASCII 文字會依單字和標點邊界斷詞。例如，`personal-map` 包含獨立的 `map` token，而 `amap-jsapi-skill` 包含 `amap`、`jsapi` 和 `skill`；因此搜尋 `map` 會讓 `personal-map` 比 `amap-jsapi-skill` 有更強的詞彙符合。
- 熱門度會經過對數縮放並設有上限。高互動 skills 在查詢文字符合較弱時，排名可能較低。
- 可疑或隱藏的審核狀態，可能會依呼叫端篩選器和目前審核狀態，將某個 skill 從公開搜尋中移除。

發布者可發現性指引：

- 將使用者會直接搜尋的詞彙放在顯示名稱、摘要和標籤中。只有當獨立 slug token 也是你想保留的穩定身分時才使用。
- 不要只為了追逐單一查詢而重新命名 slug，除非新的 slug 是更好的長期標準名稱。舊 slug 會成為重新導向別名，但標準 URL、顯示的 slug 和未來搜尋摘要會使用新的 slug。
- 重新命名別名會保留舊 URL 與透過登錄檔解析安裝的解析能力，但搜尋排名會以重新命名已建立索引後的標準 skill 中繼資料為依據。現有統計資料會保留在該 skill 上。
...
- 當 `nonSuspiciousOnly=true` 時，基於游標的排序可能會在一頁中傳回少於 `limit` 個項目，因為可疑 skills 會在擷取頁面後被篩除。
- 存在時請使用 `nextCursor` 繼續分頁。短頁本身不表示結果已結束。
...
- 證據會針對公開呼叫端遮蔽，且只有擁有者/審核員會包含原始片段。

### `POST /api/v1/skills/{slug}/report`

回報 skill 供審核員檢閱。回報是 skill 層級，可選擇性連結到版本，並會送入 skill 回報佇列。
...
`note` 對 `confirmed` 和 `dismissed` 為必要；將 `status` 設回 `open` 時可以省略。對已分類的回報傳入 `finalAction: "hide"`，即可在同一個可稽核工作流程中隱藏該 skill。
...
- `moderation` 是衍生自最新版本的目前技能層級審核快照。
...
- 回傳已排隊/執行中/成功/失敗狀態。
- 排隊期間會回傳 `queue.queuedAhead` 和 `queue.position`，讓用戶端能顯示此請求前方還有多少個優先手動掃描。非常大的佇列會受上限限制，並以 `queuedAheadIsEstimate: true` 回報。
...
僅限管理員使用的正式批次重新掃描路由。它接受與舊版 `POST /api/v1/skills/-/rescan-batch` 相同的承載資料形狀。

### `POST /api/v1/skills/-/scan/batch/status`

僅限管理員使用的正式批次狀態路由。它接受 `{ "jobIds": ["..."] }`，並回傳與舊版 `POST /api/v1/skills/-/rescan-batch/status` 相同的彙總計數器。
...
- 只有在所選版本有已產生的技能卡、未被審核判定為惡意軟體封鎖，且 ClawScan 驗證為乾淨時，`ok` 才會是 `true`。
- 技能身分、發布者身分和所選版本中繼資料是頂層信封欄位（`slug`、`displayName`、`publisherHandle`、`version`、`resolvedFrom`、`tag`、`createdAt`），因此 Shell 自動化可以不解開巢狀包裝就讀取它們。
...
- `security.signals.dependencyRegistry` 保留用於 v1 回應相容性，但相依性登錄存在性掃描器已停用，且此鍵一律為 `null`。
...
集合端點是為已知需要顯示哪些已安裝
ClawHub 技能版本的用戶端而設計，例如 OpenClaw Control UI。
...
- 結果以項目為單位；缺少某一個技能或版本不會使整個回應失敗。
- 回應僅包含安全性資訊。不包含技能卡資料、已產生卡片狀態、成品檔案清單或詳細掃描器承載資料。
- `security.signals` 僅包含狀態層級的支援性證據；完整掃描器詳細資訊請使用 `/scan` 或 ClawHub security-audit 頁面。
- `security.signals.dependencyRegistry` 保留用於 v1 回應相容性，但相依性登錄存在性掃描器已停用，且此鍵一律為 `null`。
...
- `GET /api/v1/code-plugins` 與 `GET /api/v1/bundle-plugins` 仍為固定 family 別名。
- Skills 項目仍由 Skills 登錄檔支援，且仍只能透過 `POST /api/v1/skills` 發布。
- `POST /api/v1/packages` 仍僅用於 code-plugin 與 bundle-plugin 發行版本。
- 匿名呼叫者只能看見公開套件頻道。
- 已驗證的呼叫者可以在清單/搜尋結果中看見其所屬發布者的私有套件。
- `channel=private` 只會傳回已驗證呼叫者可讀取的套件。

### `GET /api/v1/packages/search`

跨 Skills 與外掛套件的統一型錄搜尋。
...
在統一 `/api/v1/packages` 端點上，它僅適用於外掛；Skills 型錄請使用
...
在 code-plugin 與 bundle-plugin 套件中進行外掛專用搜尋。
...
- `version.artifact.kind` 對舊式套件封存檔為 `legacy-zip`，對
  ClawPack 支援的版本為 `npm-pack`。
- ClawPack 版本包含與 npm 相容的 `npmIntegrity`、`npmShasum` 和
  `npmTarballName` 欄位。
- `version.sha256hash` 是給舊用戶端使用的已棄用相容性中繼資料。它會
  雜湊 `/api/v1/packages/{name}/download` 傳回的精確 ZIP 位元組。
...
- `package.name`、`package.displayName` 和 `package.family` 會識別
  已解析的登錄套件。
- `release.releaseId`、`release.version` 和 `release.createdAt` 會識別
  已評估的精確版本。
- 當版本成品已知時，會包含 `release.artifactKind`、`release.artifactSha256`、`release.npmIntegrity`、
...
- `trust.blockedFromDownload` 是安裝封鎖訊號。OpenClaw 和其他
  ...
  從掃描器或審核欄位重新推導封鎖規則。
- `trust.reasons` 是面向使用者與稽核用途的說明清單。原因代碼
...
- 此端點精確到版本。用戶端應在解析打算安裝的
  套件版本後呼叫它，而不只是讀取最新
  套件中繼資料後呼叫。
- 私有套件會傳回 `404`，除非呼叫端可以讀取擁有該套件的發布者。
- 此端點刻意比擁有者/版主審核端點更窄。
  它公開安裝決策與公開說明，而不公開
  回報者身分、回報本文、私有證據或內部審查
  時間軸。
...
`confirmed` 和 `dismissed` 需要 `note`；將 `status` 設回 `open` 時可省略。對已確認的檢舉傳入 `finalAction: "quarantine"` 或 `finalAction: "revoke"`，即可在同一個可稽核工作流程中套用發布審核。
...
- 二進位檔案傳回 `415`。
- 檔案大小限制：200KB。
- 待處理的 VirusTotal 掃描不會阻止讀取；惡意發布仍可能在其他地方被 withheld。
...
- 外掛/套件封存檔是具有 `package/` 根目錄的 zip 檔案，讓舊版 OpenClaw 用戶端維持可用。
- 此路由維持僅 ZIP。它不串流 ClawPack `.tgz` 檔案。
- 回應包含 `ETag`、`Digest`、`X-ClawHub-Artifact-Type` 和 `X-ClawHub-Artifact-Sha256` 標頭，用於解析器完整性檢查。
- 僅登錄檔的中繼資料不會注入下載的封存檔。
- 待處理的 VirusTotal 掃描不會阻止下載；惡意發布會傳回 `403`。
...
- 舊版僅 ZIP 的版本會刻意省略。
- `dist.tarball`、`dist.integrity` 和 `dist.shasum` 使用 npm 相容欄位，讓使用者可選擇將 npm 指向鏡像。
...
- 下載統計會按 UTC 日期計算唯一身分（API 權杖有效時為 `userId`，否則為 IP）。
...
- 選填酬載欄位：`migrateOwner`。當與 `ownerHandle` 一起為 `true` 時，如果行為者在目前和目標發布者上都是管理員/擁有者，現有 skill 可以移至該擁有者。沒有此選擇加入時，擁有者變更會遭拒。
...
- 允許的表單欄位是 `payload`、重複的 `files` blob，或一個 `clawpack` tarball 參照。`clawpack` 可以是 `.tgz` blob，或 upload-url 流程傳回的 storage id。以 staged storage-id 發布時，也必須包含該上傳 URL 一併傳回的 `clawpackUploadTicket`。
- 使用 `files` 或 `clawpack`，同一個請求中絕不可同時使用兩者。
...
擁有者發起的軟刪除會保留 slug 30 天，之後 slug 可由另一位發布者宣告。
...
僅限管理員。確保某個 handle 存在組織發布者。如果該 handle 仍指向舊版共用使用者/個人發布者，端點會先將它遷移為組織發布者。
...
僅限管理員。為正當擁有者保留根 slug 和套件名稱，而不發布 release。套件名稱會成為沒有 release 列的私有佔位套件，因此同一擁有者之後可以將真正的 code-plugin 或 bundle-plugin 發布到該名稱。
...
僅限管理員。為已驗證的替代 GitHub OAuth 主體復原個人發布者，而不編輯 Convex Auth 帳戶列。請求必須指定兩個不可變的 GitHub provider account id；可變 handle 僅用作面向操作員的防護。

端點預設為乾執行。套用復原需要在工作人員獨立驗證兩個
GitHub 主體之間的連續性後，設定 `dryRun: false` 和
`confirmIdentityVerified: true`。當目的地使用者目前的個人發布者擁有技能、套件或 GitHub 技能來源時，復原會以封閉方式失敗。
復原也會遷移已復原發布者技能、技能 slug 別名、套件、套件檢查器警告，以及衍生搜尋摘要列中的舊版 `ownerUserId` 欄位，使直接擁有者路徑與新的發布者權限一致。已復原控制代碼的作用中受保護控制代碼保留項目，也會重新指派給替代使用者，讓之後的
個人資料同步無法還原前使用者的競爭權限。每個主要資料表在每次套用交易中上限為
...
變更現有封鎖已儲存的原因，不解除封鎖或還原內容（僅限管理員）。除非 `dryRun` 為 `false`，否則預設為乾執行。
