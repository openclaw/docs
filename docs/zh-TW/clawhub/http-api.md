...
第三方目錄可以使用公開讀取端點來列出或搜尋 ClawHub skills。請快取結果、遵守 `429`/`Retry-After`、將使用者連回正式 ClawHub 清單（`https://clawhub.ai/<owner>/skills/<slug>`），並避免暗示 ClawHub 為第三方網站背書。請勿嘗試在公開 API 介面之外鏡像隱藏、私人或遭審核封鎖的內容。

網頁 slug 捷徑會跨登錄家族解析，但 API 用戶端應使用讀取端點傳回的正式 URL，而不是自行重建路由優先順序。
...
- 若權杖缺失/無效，行為會退回 IP 強制執行。
- 當伺服器知道原因時，已驗證的寫入端點不應只傳回裸露的 `Unauthorized`。缺少權杖、無效/已撤銷權杖，以及已刪除/封禁/停用帳號，都應取得可採取行動的文字，讓命令列介面用戶端能告知使用者被阻擋的原因。
...
- 僅在部署明確啟用受信任轉送標頭時，才使用受信任的用戶端 IP 標頭，包括 `cf-connecting-ip`。
- ClawHub 使用受信任的轉送標頭在邊緣識別用戶端 IP。
- 如果沒有可用的受信任用戶端 IP，匿名請求會使用僅依速率限制種類界定的退回 bucket。這些退回 bucket 不包含呼叫端提供的路徑、slug、套件名稱、版本、查詢字串或其他成品參數。

## 錯誤回應

公開 v1 錯誤回應是純文字，並帶有 `content-type: text/plain; charset=utf-8`。
這包括驗證失敗（`400`）、缺少公開資源（`404`）、驗證與權限失敗（`401`/`403`）、速率限制（`429`），以及被封鎖的下載。用戶端應將回應本文讀取為人類可讀的字串。未知查詢參數會為了相容性而被忽略，但已識別且值無效的查詢參數會傳回 `400`。
...
- 相關性比熱門度更強。精確的 slug 或顯示名稱 token 相符，可能排名高於互動強很多但相符較鬆散的結果。
- ASCII 文字會依單字與標點邊界斷詞。例如，`personal-map` 包含獨立的 `map` token，而 `amap-jsapi-skill` 包含 `amap`、`jsapi` 與 `skill`；因此搜尋 `map` 時，`personal-map` 會比 `amap-jsapi-skill` 有更強的詞彙相符。
- 熱門度會以對數縮放並設上限。高互動 skills 在查詢文字相符較弱時可能排名較低。
- 可疑或隱藏的審核狀態可能會依呼叫端篩選器與目前審核狀態，將 skill 從公開搜尋移除。

發布者可探索性指引：

- 將使用者會實際搜尋的字詞放入顯示名稱、摘要與標籤。只有當獨立 slug token 也是你想保留的穩定身分時，才使用它。
- 不要只為追逐單一查詢而重新命名 slug，除非新 slug 是更好的長期正式名稱。舊 slug 會成為重新導向別名，但正式 URL、顯示的 slug，以及未來搜尋摘要都會使用新 slug。
- 重新命名別名會保留舊 URL，以及透過登錄解析的安裝之解析能力，但搜尋排名會依重新命名完成索引後的正式 skill 中繼資料為準。既有統計資料會保留在該 skill 上。
- 如果 skill 意外不可見，請在變更排名相關中繼資料之前，先在登入狀態下使用 `clawhub inspect @owner/slug` 檢查審核狀態。
...
- 當 `nonSuspiciousOnly=true` 時，基於游標的排序可能在某頁傳回少於 `limit` 的項目，因為可疑 skills 會在頁面擷取後被篩除。
...
- 只有當 skill 已被標記或擁有者正在檢視時，才會包含 `moderation`。
...
- 公開呼叫端的證據會經過遮罩，且只有擁有者/審核者才會包含原始片段。

### `POST /api/v1/skills/{slug}/report`

回報 skill 以供審核者檢閱。回報屬於 skill 層級，可選擇連結到版本，並會進入 skill 回報佇列。
...
`note` 對 `confirmed` 與 `dismissed` 為必填；將 `status` 設回 `open` 時可以省略。搭配已分流的回報傳入 `finalAction: "hide"`，可在同一個可稽核工作流程中隱藏該 skill。
...
- 回傳已排入佇列/執行中/成功/失敗狀態。
- 排入佇列時回傳 `queue.queuedAhead` 和 `queue.position`，讓用戶端可以顯示有多少個優先手動掃描排在此請求之前。非常大的佇列會受到限制，並以 `queuedAheadIsEstimate: true` 回報。
...
- 只有當選取版本已有產生的 Skill Card、未被審核判定為惡意軟體封鎖，且 ClawScan 驗證為乾淨時，`ok` 才會是 `true`。
- 技能身分、發布者身分和選取版本中繼資料是頂層信封欄位（`slug`、`displayName`、`publisherHandle`、`version`、`resolvedFrom`、`tag`、`createdAt`），讓 shell 自動化可在不解開巢狀包裝的情況下讀取。
...
- 結果以項目為單位；缺少某個技能或版本不會讓整個回應失敗。
- 回應僅限安全性資訊。它不包含 Skill Card 資料、已產生卡片狀態、成品檔案清單，或詳細掃描器酬載。
- `security.signals` 只包含狀態層級的支援證據；如需完整掃描器詳細資料，請使用 `/scan` 或 ClawHub 安全性稽核頁面。
- `security.signals.dependencyRegistry` 會保留以維持 v1 回應相容性，但相依性登錄存在性掃描器已停用，且此鍵一律為 `null`。
...
- `POST /api/v1/packages` 仍僅適用於 code-plugin 和 bundle-plugin 版本發布。
- 匿名呼叫者只能看見公開套件頻道。
- 已驗證呼叫者可以在清單/搜尋結果中看見其所屬發布者的私人套件。
...
- `channel`（選填）：`official`、`community` 或 `private`
- `isOfficial`（選填）：`true` 或 `false`
- `category`（選填）：外掛類別篩選器。僅在要求限定為外掛套件時支援。受控類別與舊版 v1
  篩選別名記錄於 `GET /api/v1/plugins` 下。
...
- 目前由 GitHub 支援且掃描結果為 `clean` 或 `suspicious` 的 Skills 包含
  ...
  內容雜湊，以及封存 URL。它們不包含 ClawHub 代管的原始碼檔案。
...
- ClawPack 版本包含與 npm 相容的 `npmIntegrity`、`npmShasum` 和
  `npmTarballName` 欄位。
- `version.sha256hash` 是供舊用戶端使用的已淘汰相容性中繼資料。它會
  雜湊 `/api/v1/packages/{name}/download` 傳回的確切 ZIP 位元組。
  現代用戶端應使用 `version.artifact.sha256`，其識別標準版本成品。
...
- `trust.reasons` 是面向使用者與稽核用途的說明清單。原因代碼是穩定、
  精簡的字串，例如 `manual:quarantined`、`scan:malicious`
  和 `package:malicious`。
...
- `open`：可疑、惡意、待處理、已隔離、已撤銷或已被回報的版本。
...
`note` 在 `confirmed` 和 `dismissed` 時為必填；將 `status` 設回 `open` 時可以省略。傳入 `finalAction: "quarantine"` 或 `finalAction: "revoke"` 搭配已確認的回報，可在同一個可稽核工作流程中套用版本審核。
...
被隔離和撤銷的版本會讓成品下載路由回傳 `403`。每次變更都會寫入稽核記錄項目。
...
- 二進位檔案會回傳 `415`。
- 檔案大小限制：200KB。
- 待處理的 VirusTotal 掃描不會封鎖讀取；惡意版本仍可能在其他位置被扣留。
...
- 外掛／套件封存檔是 zip 檔案，並具有 `package/` 根目錄，讓舊版 OpenClaw 用戶端能持續運作。
...
- Scoped package packument 同時支援 `/api/npm/@scope/name` 和 npm 的編碼請求路徑 `/api/npm/@scope%2Fname`。
...
下載託管的技能版本 ZIP，或針對目前由 GitHub 支援且掃描結果為 `clean` 或 `suspicious`、且沒有託管版本的技能，回傳 GitHub 原始碼交接資訊。
...
- 選填酬載欄位：`migrateOwner`。當與 `ownerHandle` 一起為 `true` 時，如果執行者同時是目前與目標發布者的管理員／擁有者，既有技能可以移至該擁有者。若沒有此選擇加入，擁有者變更會被拒絕。
...
- 允許的表單欄位為 `payload`、重複的 `files` blob，或一個 `clawpack` tarball 參照。`clawpack` 可以是 `.tgz` blob，或上傳 URL 流程回傳的儲存 id。已暫存的 storage-id 發布也必須包含該上傳 URL 一併回傳的 `clawpackUploadTicket`。
- 使用 `files` 或 `clawpack`，同一請求中不可兩者並用。
...
- Code plugins 需要 `package.json`、原始碼 repo 中繼資料、來源 commit 中繼資料、設定 schema 中繼資料、`openclaw.compat.pluginApi` 和 `openclaw.build.openclawVersion`。
...
由擁有者發起的軟刪除會保留 slug 30 天，之後該 slug 可由另一個發布者認領。
...
僅限管理員。確保 handle 有對應的 org 發布者存在。如果該 handle 仍指向舊版共用使用者／個人發布者，端點會先將其遷移為 org 發布者。
對於新建立的 org，請提供 `memberHandle`；執行操作的管理員不會被加入為成員。
...
僅限管理員。為合法擁有者保留根 slug 和套件名稱，而不發布版本。套件名稱會成為沒有版本列的私有佔位套件，因此同一擁有者日後可以將真正的 code-plugin 或 bundle-plugin 版本發布到該名稱。
...
僅限管理員。為已驗證的替代 GitHub OAuth 主體復原個人發布者，而不編輯 Convex Auth 帳戶列。請求必須指定兩個不可變的 GitHub provider 帳戶 ID；可變 handle 僅作為面向操作者的保護措施。

端點預設為試執行。套用復原需要在工作人員獨立驗證兩個
GitHub 主體之間的連續性後，設定 `dryRun: false` 和
`confirmIdentityVerified: true`。當目標使用者目前的個人
發布者擁有 Skills、套件或 GitHub 技能來源時，復原會以拒絕方式失敗。
復原也會遷移已復原發布者的 Skills、技能 slug 別名、套件、套件檢查器警告，以及衍生搜尋摘要列中的舊版 `ownerUserId` 欄位，使
...
GitHub 技能來源是以發布者為範圍，並回報為已檢查，而不是被重寫。
...
在不解除封鎖或還原內容的情況下，變更現有封鎖的已儲存原因（僅限管理員）。除非 `dryRun` 為 `false`，否則預設為試執行。
