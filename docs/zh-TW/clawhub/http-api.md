...
第三方目錄可以使用公開讀取端點來列出或搜尋 ClawHub Skills。請快取結果、遵守 `429`/`Retry-After`、將使用者連回標準 ClawHub 清單（`https://clawhub.ai/<owner>/skills/<slug>`），並避免暗示 ClawHub 為第三方網站背書。請勿嘗試在公開 API 介面之外鏡像隱藏、私人或遭審核封鎖的內容。
...
- 如果 token 遺失或無效，行為會退回為依 IP 強制執行。
- 已驗證的寫入端點在伺服器知道原因時，不應只傳回裸露的 `Unauthorized`。缺少 token、無效或已撤銷的 token，以及已刪除、封鎖或停用的帳號，都應取得可操作的文字，讓命令列介面用戶端能告訴使用者是什麼阻擋了他們。
...
- 在 `429` 上：`X-RateLimit-Remaining: 0` 與 `RateLimit-Remaining: 0`
- 在 `429` 上：`Retry-After`
...
- 如果沒有可用的受信任用戶端 IP，匿名請求會使用僅依速率限制類型界定範圍的後備 bucket。這些後備 bucket 不包含呼叫端提供的路徑、slug、套件名稱、版本、查詢字串或其他成品參數。

## 錯誤回應

公開 v1 錯誤回應是純文字，並帶有 `content-type: text/plain; charset=utf-8`。
這包括驗證失敗（`400`）、缺少公開資源（`404`）、驗證與權限失敗（`401`/`403`）、速率限制（`429`）以及遭封鎖的下載。用戶端應將回應本文讀取為人類可讀的字串。未知的查詢參數會為了相容性而忽略，但已辨識且值無效的查詢參數會傳回 `400`。
...
- 相關性比熱門度更強。精確的 slug 或顯示名稱 token 相符，可能排名高於互動更強但相符較寬鬆的結果。
- ASCII 文字會依單字與標點邊界切分 token。例如，`personal-map` 包含獨立的 `map` token，而 `amap-jsapi-skill` 包含 `amap`、`jsapi` 與 `skill`；因此搜尋 `map` 時，`personal-map` 會比 `amap-jsapi-skill` 有更強的詞彙相符度。
- 熱門度採用對數縮放並設有上限。高互動 Skills 在查詢文字相符較弱時，排名可能較低。
- 可疑或隱藏的審核狀態，可能會依呼叫端篩選器與目前審核狀態，將某個 Skill 從公開搜尋中移除。
...
- 重新命名別名會保留舊 URL 以及透過登錄解析的安裝解析能力，但搜尋排名會基於重新命名索引後的標準 Skill 中繼資料。現有統計資料會保留在該 Skill 上。
- 如果某個 Skill 意外不可見，請在變更排名相關中繼資料前，登入後先使用 `clawhub inspect @owner/slug` 檢查審核狀態。
...
- `metadata.os`：在 Skill frontmatter 中宣告的 OS 限制（例如 `["macos"]`、`["linux"]`）。未宣告時為 `null`。
- `metadata.systems`：Nix 系統目標（例如 `["aarch64-darwin", "x86_64-linux"]`）。未宣告時為 `null`。
- 如果 Skill 沒有平台中繼資料，`metadata` 為 `null`。
- `moderation` 只有在 Skill 被標記或擁有者正在檢視它時才會包含。
...
回報 Skill 以供審核員審查。回報是 Skill 層級，可選擇連結到某個版本，並會送入 Skill 回報佇列。
...
`confirmed` 與 `dismissed` 需要 `note`；將 `status` 設回 `open` 時可省略。對已分流的回報傳入 `finalAction: "hide"`，可在同一個可稽核工作流程中隱藏該 Skill。
...
- 掃描請求酬載和可下載報告會在保留期間過後從掃描請求儲存區過期。
...
- 回傳已排隊/執行中/成功/失敗狀態。
- 排隊時回傳 `queue.queuedAhead` 和 `queue.position`，讓用戶端可以顯示此請求前方有多少個優先手動掃描。非常大的佇列會被限制，並以 `queuedAheadIsEstimate: true` 回報。
...
- 需要對該技能或外掛具備擁有者/發布者管理存取權，或平台審核者/管理員權限。
...
- 只有在所選版本已產生 Skill Card、未被審核封鎖為惡意軟體，且 ClawScan 驗證為乾淨時，`ok` 才會是 `true`。
- 技能身分、發布者身分和所選版本中繼資料是頂層信封欄位（`slug`、`displayName`、`publisherHandle`、`version`、`resolvedFrom`、`tag`、`createdAt`），因此 shell 自動化可以讀取它們，而不必拆解巢狀包裝。
- `security` 是頂層 ClawScan/安全判定。自動化應以 `ok`、`decision`、`reasons` 和 `security.status` 作為依據。
- `security.signals` 包含支援性的掃描器證據，例如 `staticScan`、`virusTotal` 和 `skillSpector`。
- `security.signals.dependencyRegistry` 會保留以維持 v1 回應相容性，但依賴項登錄存在性掃描器已退役，此鍵一律為 `null`。
...
- 結果依項目而定；單一缺少的技能或版本不會導致整個回應失敗。
- 回應僅包含安全資訊。它不包含 Skill Card 資料、已產生卡片狀態、成品檔案清單或詳細掃描器酬載。
- `security.signals` 僅包含狀態層級的支援證據；若需要完整掃描器詳細資料，請使用 `/scan` 或 ClawHub security-audit 頁面。
- `security.signals.dependencyRegistry` 會保留以維持 v1 回應相容性，但依賴項登錄存在性掃描器已退役，此鍵一律為 `null`。
...
- `category`（選填）：外掛分類篩選器。僅在請求範圍限定為外掛套件時支援（`/api/v1/plugins`、`/api/v1/code-plugins`、`/api/v1/bundle-plugins`，或帶有 `family=code-plugin`/`family=bundle-plugin` 的套件端點）。受控分類與舊版 v1 篩選器別名記載於 `GET /api/v1/plugins` 下。
...
- 託管 Skills 包含最新儲存版本檔案，並列於 `_manifest.json`，帶有 `sourceRef: "public-clawhub"`。
- 目前由 GitHub 支援且掃描結果為 `clean` 或 `suspicious` 的 Skills 會包含 `_source_handoff.json`，其中含有 `sourceRef: "public-github"`、repo、commit、path、content hash 與封存 URL。它們不包含由 ClawHub 託管的來源檔案。
...
傳回安裝用戶端所需的精確套件版本安全性與信任摘要。這是公開的 OpenClaw 消費介面，用於決定已解析的版本是否可以安裝。
...
- `trust.moderationState` 可為空值。當不存在手動版本仲裁時為 `null`。
- `trust.blockedFromDownload` 是安裝阻擋訊號。OpenClaw 與其他
  安裝用戶端應在此值為 `true` 時阻擋安裝，而不是
  從掃描器或仲裁欄位重新推導阻擋規則。
...
回報套件以供仲裁者審查。回報是套件層級，可選擇
連結到某個版本。它們會送入仲裁佇列，但本身不會自動隱藏或
阻擋下載；仲裁者應使用版本仲裁來
核准、隔離或撤銷成品。
...
`note` 在 `confirmed` 和 `dismissed` 時為必填；將 `status` 設回 `open` 時可以省略。對已確認的回報傳入 `finalAction: "quarantine"` 或
`finalAction: "revoke"`，可在同一個可稽核工作流程中套用發布審核。
...
- 舊版僅 ZIP 的版本會刻意省略。
- `dist.tarball`、`dist.integrity` 和 `dist.shasum` 使用 npm 相容欄位，讓使用者可視需要將 npm 指向鏡像。
- 具範圍的套件 packument 同時支援 `/api/npm/@scope/name` 和 npm 的已編碼 `/api/npm/@scope%2Fname` 請求路徑。
...
下載託管技能版本 ZIP，或對於目前由 GitHub 支援、掃描結果為 `clean` 或 `suspicious` 且沒有託管版本的技能，回傳 GitHub 原始碼交接資訊。
...
- 軟刪除版本會回傳 `410`。
- GitHub 支援的技能交接不會代理或鏡像位元組。JSON 回應包含 `sourceRef: "public-github"`、`repo`、`commit`、`path`、`contentHash`
  和 `archiveUrl`；掃描/目前狀態是閘門，不會作為成功酬載中繼資料包含在內。
- 下載統計會按每個 UTC 日的唯一身分計算（API 權杖有效時使用 `userId`，否則使用 IP）。
...
- 選用酬載欄位：`migrateOwner`。當它與 `ownerHandle` 一起為 `true` 時，如果執行者同時是目前與目標發布者的管理員/擁有者，既有技能可移至該擁有者。若未明確選擇此選項，擁有者變更會被拒絕。
...
- 允許的表單欄位為 `payload`、重複的 `files` blob，或一個 `clawpack`
  tarball 參考。`clawpack` 可以是 `.tgz` blob，或是 upload-url 流程回傳的 storage id。使用暫存 storage-id 發布時，也必須包含該上傳 URL 一併回傳的
  `clawpackUploadTicket`。
- 使用 `files` 或 `clawpack` 其中之一，絕不可在同一個請求中同時使用。
...
- 程式碼外掛需要 `package.json`、原始碼儲存庫中繼資料、原始碼 commit
  中繼資料、設定 schema 中繼資料、`openclaw.compat.pluginApi` 和
  `openclaw.build.openclawVersion`。
- `openclaw.hostTargets` 和 `openclaw.environment` 是選用中繼資料。
- 只有 `openclaw` 組織發布者與目前 `openclaw` 組織成員的個人發布者可發布到 `official` 頻道。
- 代表他人發布仍會依目標擁有者帳戶驗證官方頻道資格。
...
存在時，`reason` 會儲存為技能審核註記，並複製到稽核日誌中。
由擁有者發起的軟刪除會保留 slug 30 天，之後該 slug 可由
另一位發布者宣告。刪除回應會在此到期適用時包含 `slugReservedUntil`。
...
僅限管理員。確保某個 handle 存在組織發布者。若該 handle 仍指向舊版共用使用者/個人發布者，此端點會先將它遷移為組織發布者。
對於新建立的組織，提供 `memberHandle`；執行操作的管理員不會被加入為成員。
...
呼叫者新增為擁有者。此端點不會遷移既有使用者/個人 handle，也不會
將發布者標記為受信任/官方。
...
發布版本。套件名稱會成為沒有發布列的私有預留位置套件，因此同一個
擁有者稍後可以將真正的 code-plugin 或 bundle-plugin 發布版本發布到該名稱。
...
而不編輯 Convex Auth 帳戶列。請求必須同時指定兩個不可變的 GitHub
provider account id；可變 handle 僅作為面向操作員的防護。

端點預設為試跑。套用復原需要在工作人員獨立驗證兩個
GitHub 主體之間的連續性後，設定 `dryRun: false` 和
`confirmIdentityVerified: true`。當目的地使用者目前的個人
發布者擁有 Skills、套件或 GitHub Skills 來源時，復原會以關閉方式失敗。
復原也會遷移已復原發布者的 Skills、Skills slug 別名、套件、套件檢查器警告，以及衍生搜尋摘要列的舊版 `ownerUserId` 欄位，讓直接擁有者路徑與新的發布者權限一致。已復原控制代碼的有效受保護控制代碼保留也會重新指派給替代使用者，使後續
個人檔案同步無法還原前使用者的競爭權限。每個主要資料表在每個套用交易中限制為
100 列；較大的復原必須先使用可續傳的擁有者遷移。
GitHub Skills 來源是以發布者為範圍，並回報為已檢查，而不是重寫。

- 請求本文：`{ "handle": "gingiris", "nextUserHandle": "gingiris-1031", "previousGitHubProviderAccountId": "123", "nextGitHubProviderAccountId": "456", "reason": "Verified account continuity for issue #2555", "confirmIdentityVerified": true, "dryRun": false }`
...
- `POST /api/v1/skills/{slug}/rename`
  - 請求本文：`{ "newSlug": "new-canonical-slug" }`
  - 回應：`{ "ok": true, "slug": "new-canonical-slug", "previousSlug": "old-slug" }`
- `POST /api/v1/skills/{slug}/merge`
  - 請求本文：`{ "targetSlug": "canonical-target-slug" }`
...
- `POST /api/v1/skills/{slug}/transfer`
  - 請求本文：`{ "toUserHandle": "target_handle", "message": "optional" }`
...
發布必須將產生的儲存 ID 以 `clawpack` 傳送，並將回傳的票證以 `clawpackUploadTicket` 傳送。
