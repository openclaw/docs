...
x-i18n:
    generated_at: "2026-07-02T00:42:21Z"
    model: gpt-5.5
...
第三方目錄可以使用公開讀取端點來列出或搜尋 ClawHub Skills。請快取結果、遵守 `429`/`Retry-After`、將使用者連回正式的 ClawHub 列表（`https://clawhub.ai/<owner>/skills/<slug>`），並避免暗示 ClawHub 背書第三方網站。請勿嘗試在公開 API 介面之外鏡像隱藏、私有或因審核而封鎖的內容。

網頁 slug 捷徑會跨註冊表家族解析，但 API 用戶端應使用讀取端點傳回的正式 URL，而不是自行重建路由優先順序。
...
- 如果權杖缺失/無效，行為會退回到 IP 執行。
- 當伺服器知道原因時，已驗證的寫入端點不應回傳裸露的 `Unauthorized`。缺失權杖、無效/已撤銷權杖，以及已刪除/遭封鎖/已停用的帳號，都應取得可採取行動的文字，讓命令列介面用戶端能告訴使用者是什麼阻擋了他們。
...
- 只有在部署明確啟用可信任轉送標頭時，才會使用可信任的用戶端 IP 標頭，包括 `cf-connecting-ip`。
- ClawHub 使用可信任的轉送標頭，在邊緣識別用戶端 IP。
- 如果沒有可用的可信任用戶端 IP，匿名請求會使用僅依速率限制種類界定範圍的備援 bucket。這些備援 bucket 不包含呼叫者提供的路徑、slug、套件名稱、版本、查詢字串或其他成品參數。

## 錯誤回應

公開 v1 錯誤回應是純文字，並帶有 `content-type: text/plain; charset=utf-8`。
這包括驗證失敗（`400`）、缺少公開資源（`404`）、驗證與權限失敗（`401`/`403`）、速率限制（`429`）以及遭封鎖的下載。用戶端應將回應本文讀取為人類可讀字串。未知查詢參數會為了相容性而被忽略，但已識別且值無效的查詢參數會回傳 `400`。
...
- 相關性比人氣更強。精確的 slug 或顯示名稱 token 相符，可以排在互動強很多但匹配較鬆散的結果之前。
- ASCII 文字會依單字與標點邊界進行 token 化。例如，`personal-map` 包含獨立的 `map` token，而 `amap-jsapi-skill` 包含 `amap`、`jsapi` 和 `skill`；因此搜尋 `map` 會讓 `personal-map` 比 `amap-jsapi-skill` 取得更強的詞彙匹配。
- 人氣會以對數縮放並設有上限。當查詢文字的匹配較弱時，高互動 Skills 的排名可能較低。
- 可疑或隱藏的審核狀態，可能會根據呼叫者篩選器與目前審核狀態，將某個 Skill 從公開搜尋中移除。

發布者可發現性指引：

- 將使用者會實際搜尋的詞放入顯示名稱、摘要和標籤中。只有當獨立 slug token 也是你想保留的穩定身分時才使用它。
- 不要只為了追逐某個查詢而重新命名 slug，除非新的 slug 是更好的長期正式名稱。舊 slug 會成為重新導向別名，但正式 URL、顯示的 slug，以及未來搜尋摘要都會使用新的 slug。
- 重新命名別名會保留舊 URL 以及透過註冊表解析的安裝解析能力，但搜尋排名會依重新命名完成索引後的正式 Skill 中繼資料計算。既有統計資料會留在該 Skill 上。
- 如果某個 Skill 意外不可見，請在登入狀態下先用 `clawhub inspect @owner/slug` 檢查審核狀態，再變更與排名相關的中繼資料。
...
- `trending` 依最近 7 天的安裝量排名（基於遙測）。
- `createdAt` 對新 Skill 爬取是穩定的；`updated` 會在既有 Skills 重新發布時變更。
- 當 `nonSuspiciousOnly=true` 時，基於游標的排序可能會在單頁回傳少於 `limit` 個項目，因為可疑 Skills 會在頁面擷取後被篩除。
- 存在 `nextCursor` 時，請用它繼續分頁。短頁本身不代表已到結果結尾。
...
- `metadata.os`：Skill frontmatter 中宣告的 OS 限制（例如 `["macos"]`、`["linux"]`）。如果未宣告則為 `null`。
...
- 公開呼叫者看到的證據會經過遮蔽，而且只有擁有者/審核員會包含原始片段。

### `POST /api/v1/skills/{slug}/report`

回報 Skill 以供審核員檢閱。回報屬於 Skill 層級，可選擇性連結到某個版本，並會送入 Skill 回報佇列。
...
`note` 對 `confirmed` 和 `dismissed` 是必要的；將 `status` 設回 `open` 時可以省略。對已分流的回報傳入 `finalAction: "hide"`，即可在同一個可稽核工作流程中隱藏該 Skill。
...
- `version.security` 在可用時會包含正規化的掃描驗證狀態與掃描器詳細資料（VirusTotal + LLM）。
...
- 掃描請求承載資料和可下載報告會在保留期間結束後從掃描請求儲存區過期。
...
- 傳回 queued/running/succeeded/failed 狀態。
- 排隊時會傳回 `queue.queuedAhead` 和 `queue.position`，讓用戶端可以顯示有多少個優先手動掃描排在該請求前面。非常大的佇列會受限並以 `queuedAheadIsEstimate: true` 回報。
- 可用時，`report` 會包含 `clawscan`、`skillspector`、`staticAnalysis` 和 `virustotal` 區段。
- 失敗的掃描作業會傳回 `status: "failed"` 以及 `lastError`。

### `GET /api/v1/skills/-/scan/{scanId}/download`

已驗證的報告封存端點。

- 需要已成功的掃描；非終止狀態的掃描會傳回 `409`。
...
- 只有在選取的版本具有已產生的 Skill Card、未被審核判定為惡意軟體而封鎖，且 ClawScan 驗證為乾淨時，`ok` 才會是 `true`。
- Skill 身分、發布者身分和所選版本中繼資料是最上層封套欄位（`slug`、`displayName`、`publisherHandle`、`version`、`resolvedFrom`、`tag`、`createdAt`），因此 shell 自動化可以在不解開巢狀包裝的情況下讀取它們。
- `security` 是最上層 ClawScan/安全性判定。自動化應以 `ok`、`decision`、`reasons` 和 `security.status` 作為判斷依據。
- `security.signals` 包含支援性的掃描器證據，例如 `staticScan`、`virusTotal` 和 `skillSpector`。
- `security.signals.dependencyRegistry` 為了 v1 回應相容性而保留，但依賴項登錄存在性掃描器已停用，且此鍵一律為 `null`。
...
- 結果以各項目為單位；單一缺少的 skill 或版本不會導致整個回應失敗。
...
- `security.signals.dependencyRegistry` 為了 v1 回應相容性而保留，但依賴項登錄存在性掃描器已停用，且此鍵一律為 `null`。
- Skill Card 缺失不會影響此端點的 `ok`、`decision` 或 `reasons`；用戶端在需要卡片內容時，應在本機讀取已安裝的 `skill-card.md`。
- 當你需要單一 skill 的 Skill Card 驗證封套時，請使用 `/verify`；需要已產生卡片 Markdown 時，請使用 `/card`；需要詳細掃描器資料時，請使用 `/scan`。
...
- `category`（選用）：外掛分類篩選器。僅在請求範圍限定為外掛套件時支援（`/api/v1/plugins`、`/api/v1/code-plugins`、`/api/v1/bundle-plugins`，或帶有 `family=code-plugin`/`family=bundle-plugin` 的套件端點）。受控分類與舊版 v1 篩選器別名記錄於 `GET /api/v1/plugins` 下。
...
- `channel`（選用）：`official`、`community` 或 `private`
- `isOfficial`（選用）：`true` 或 `false`
- `category`（選用）：外掛分類篩選器。僅在請求範圍限定為外掛套件時支援。受控分類與舊版 v1 篩選器別名記錄於 `GET /api/v1/plugins` 下。
...
- `startDate`（必要）：Skills `updatedAt` 的 Unix 毫秒下限。
- `endDate`（必要）：Skills `updatedAt` 的 Unix 毫秒上限。
...
- 託管 Skills 包含最新儲存版本檔案，並列於 `_manifest.json` 中，帶有 `sourceRef: "public-clawhub"`。
- 目前由 GitHub 支援且掃描結果為 `clean` 或 `suspicious` 的 Skills 會包含 `_source_handoff.json`，其中帶有 `sourceRef: "public-github"`、repo、commit、path、content hash 和 archive URL。它們不包含 ClawHub 託管的原始檔案。
...
- `family`（選用）：`code-plugin` 或 `bundle-plugin`。省略表示兩種外掛系列皆包含。

回應：

- 主體：ZIP 封存檔。
- 每個匯出的外掛都以 `{family}/{packageName}/` 為根目錄。
- 每個匯出的外掛都包含最新發布版本已儲存的檔案。
- 每個外掛的匯出中繼資料儲存在
...
只針對外掛，在 code-plugin 與 bundle-plugin 套件之間搜尋。
...
- 需要套件擁有者、組織發布者擁有者/管理員、平台審核員或平台管理員的 API token。
...
- `version.artifact.kind` 對舊世界套件封存檔為 `legacy-zip`，
  對 ClawPack 支援的發布版本為 `npm-pack`。
- ClawPack 發布版本包含 npm 相容的 `npmIntegrity`、`npmShasum` 與
  `npmTarballName` 欄位。
- `version.sha256hash` 是供舊用戶端使用的已淘汰相容性中繼資料。它會雜湊
  `/api/v1/packages/{name}/download` 傳回的精確 ZIP 位元組。
  現代用戶端應使用 `version.artifact.sha256`，其可識別標準發布成品。
...
傳回安裝用戶端使用的精確套件發布安全與信任摘要。這是公開的 OpenClaw
取用介面，用於判斷已解析的發布版本是否可安裝。

驗證：

- 公開讀取端點。不需要擁有者、發布者、審核員或管理員 token。
...
- `package.name`、`package.displayName` 與 `package.family` 用於識別已解析的
  登錄套件。
- `release.releaseId`、`release.version` 與 `release.createdAt` 用於識別已評估的
  精確發布版本。
- 當發布成品有已知資訊時，會提供 `release.artifactKind`、`release.artifactSha256`、
  `release.npmIntegrity`、`release.npmShasum` 與 `release.npmTarballName`。
- `trust.scanStatus` 是由掃描器輸入與手動發布審核推導出的有效信任狀態。
- `trust.moderationState` 可為 null。沒有手動發布審核時為 `null`。
- `trust.blockedFromDownload` 是安裝封鎖訊號。當此值為 `true` 時，
  OpenClaw 與其他安裝用戶端應封鎖安裝，而不是從掃描器或審核欄位重新推導封鎖規則。
- `trust.reasons` 是面向使用者與稽核的說明清單。原因代碼是穩定、精簡的字串，
  例如 `manual:quarantined`、`scan:malicious` 與 `package:malicious`。
- `trust.pending` 表示一個或多個信任輸入仍在等待完成。
- `trust.stale` 表示信任摘要是從過期輸入計算而來，在做出高信心允許決策前，
  應視為需要重新整理。

注意事項：

- 此端點精確到版本。用戶端應在解析其打算安裝的套件版本後呼叫它，
  而不是只在讀取最新套件中繼資料後呼叫。
- 私有套件會傳回 `404`，除非呼叫端可讀取所屬發布者。
- 此端點刻意比擁有者/審核員審核端點更窄。它公開的是安裝決策與公開說明，
  而不是回報者身分、回報本文、私有證據或內部審查時間軸。
...
- `packageName` 會依 npm 名稱正規化；規劃中的遷移可以沒有套件。
- 這只追蹤遷移就緒狀態。它不會變更 OpenClaw 或產生 ClawPack。

### `GET /api/v1/packages/moderation/queue`

用於套件發布審查佇列的審核員/管理員端點。
...
- `open`：可疑、惡意、待處理、已隔離、已撤銷或已被回報的發布版本。
- `blocked`：已隔離、已撤銷或惡意發布版本。
- `manual`：任何具有手動審核覆寫的發布版本。
- `all`：任何具有手動覆寫、非乾淨掃描狀態或套件回報的發布版本。
...
回報套件以供審核員審查。回報屬於套件層級，可選擇連結至版本。
它們會送入審核佇列，但本身不會自動隱藏或封鎖下載；審核員應使用發布審核來
核准、隔離或撤銷成品。
...
- 外掛/套件封存是 zip 檔案，且有 `package/` 根目錄，讓舊版 OpenClaw
  用戶端保持可用。
...
回傳以 ClawPack 為基礎的套件版本之 npm 相容 packument。
...
下載託管的 Skill 版本 ZIP，或針對目前由 GitHub 支援、掃描為 `clean` 或 `suspicious`
且沒有託管版本的 Skill，回傳 GitHub 原始碼交接。
...
- 軟刪除版本回傳 `410`。
- GitHub 支援的 Skill 交接不會代理或鏡像位元組。JSON 回應
  包含 `sourceRef: "public-github"`、`repo`、`commit`、`path`、`contentHash`
  和 `archiveUrl`；掃描/目前狀態是一道閘門，不會作為成功
  酬載中繼資料包含在內。
- 下載統計依 UTC 日計算唯一身分（API 權杖有效時為 `userId`，否則為 IP）。
...
- 首選：包含 `payload` JSON + `files[]` blob 的 `multipart/form-data`。
- 也接受包含 `files`（以 storageId 為基礎）的 JSON 主體。
- 選用酬載欄位：`ownerHandle`。存在時，API 會在伺服器端解析該
  發布者，並要求行為者具備發布者存取權。
- 選用酬載欄位：`migrateOwner`。當它與 `ownerHandle` 同時為 `true` 時，
  如果行為者在目前和目標發布者中都是管理員/擁有者，
  既有 Skill 可移到該擁有者。未明確選擇此選項時，擁有者變更會被
  拒絕。
...
- 允許的表單欄位為 `payload`、重複的 `files` blob，或一個 `clawpack`
  tarball 參照。`clawpack` 可以是 `.tgz` blob，或上傳 URL 流程回傳的
  storage id。使用 staged storage-id 發布時，也必須包含該上傳 URL 一併回傳的
  `clawpackUploadTicket`。
- 使用 `files` 或 `clawpack` 其一，不得在同一請求中同時使用。
...
存在時，`reason` 會儲存為 Skill 審核備註，並複製到稽核記錄中。
由擁有者發起的軟刪除會保留 slug 30 天，之後該 slug 可由
另一個發布者宣告。刪除回應會在此到期適用時包含 `slugReservedUntil`。
主持人/管理員隱藏和安全性移除不會以這種方式到期。
...
僅限管理員。確保某個代號存在組織發布者。如果該代號仍指向
舊版共用使用者/個人發布者，端點會先將它遷移成組織發布者。
...
呼叫者加入為擁有者。此端點不會遷移既有使用者/個人代號，也不會
將發布者標記為受信任/官方。
...
版本。套件名稱會成為沒有版本資料列的私有佔位套件，因此同一
擁有者日後可以將真正的 code-plugin 或 bundle-plugin 版本發布到該名稱。
...
而不編輯 Convex Auth 帳戶資料列。請求必須指定兩個不可變的 GitHub
提供者帳戶 ID；可變的代號僅作為面向操作員的防護。

端點預設為 dry-run。套用復原需要在工作人員獨立驗證兩個
GitHub 主體之間的連續性後，設定 `dryRun: false` 和
`confirmIdentityVerified: true`。當目的地使用者目前的個人發布者已有 Skills、套件或 GitHub Skill 來源時，復原會採取失敗關閉。
復原也會遷移已復原發布者的 Skills、Skill slug 別名、套件、套件檢查器警告，以及衍生的搜尋摘要列中的舊版 `ownerUserId` 欄位，讓直接擁有者路徑與新的發布者權限一致。已復原控制代碼的有效受保護控制代碼保留也會重新指派給替代使用者，讓後續的個人檔案同步無法還原前使用者的競爭權限。每個主要資料表在每次套用交易中限制為
100 列；較大的復原必須先使用可續傳的擁有者遷移。
GitHub Skill 來源是以發布者為範圍，並回報為已檢查，而不是重寫。
...
封鎖使用者並硬刪除其擁有的 Skills（僅限仲裁者/管理員）。
...
發布必須將產生的儲存空間 ID 以
`clawpack` 傳送，並將回傳的票證以 `clawpackUploadTicket` 傳送。
