...
Web slug 捷徑會跨註冊表系列解析，但 API 用戶端應使用讀取端點傳回的標準 URL，而不是自行重建路由優先順序。
...
- 如果 token 遺失/無效，行為會退回為依 IP 強制執行。
- 已驗證的寫入端點在伺服器知道原因時，不應只傳回裸露的 `Unauthorized`。遺失 token、無效/已撤銷的 token，以及已刪除/遭停權/停用的帳號，都應取得可操作的文字，讓命令列介面用戶端能告知使用者被阻擋的原因。
...
- 如果沒有可用的受信任用戶端 IP，匿名請求會使用僅依速率限制種類設定範圍的 fallback bucket。這些 fallback bucket 不包含呼叫端提供的路徑、slug、套件名稱、版本、查詢字串或其他成品參數。

## 錯誤回應

公開 v1 錯誤回應是純文字，並帶有 `content-type: text/plain; charset=utf-8`。
這包括驗證失敗（`400`）、缺少公開資源（`404`）、驗證與權限失敗（`401`/`403`）、速率限制（`429`），以及遭封鎖的下載。用戶端應將回應本文讀作人類可讀字串。未知查詢參數會為了相容性而被忽略，但已識別且值無效的查詢參數會傳回 `400`。
...
- 結果會依相關性順序傳回（embedding 相似度 + 精確 slug/名稱 token 加權 + 小幅人氣先驗）。
- 相關性比人氣更強。精確的 slug 或顯示名稱 token 相符，可能會排在互動更強但相符較寬鬆的結果之前。
- ASCII 文字會依單字與標點邊界 token 化。例如，`personal-map` 包含獨立的 `map` token，而 `amap-jsapi-skill` 包含 `amap`、`jsapi` 與 `skill`；因此搜尋 `map` 會讓 `personal-map` 取得比 `amap-jsapi-skill` 更強的詞彙相符。
- 人氣採用對數縮放並設有上限。當查詢文字相符較弱時，高互動 skills 可能排名較低。
- 可疑或隱藏的審核狀態可能會依呼叫端篩選器與目前審核狀態，將 skill 從公開搜尋移除。

發布者可發現性指引：

- 將使用者會實際搜尋的詞放入顯示名稱、摘要和標籤。只有在獨立 slug token 也是你想保留的穩定身分時才使用它。
- 不要只為了追逐單一查詢而重新命名 slug，除非新的 slug 是更好的長期標準名稱。舊 slug 會成為重新導向別名，但標準 URL、顯示的 slug，以及未來搜尋摘要會使用新的 slug。
- 重新命名別名會保留舊 URL 與透過註冊表解析的安裝解析能力，但搜尋排名會基於重新命名已建立索引後的標準 skill 中繼資料。既有統計資料會保留在該 skill 上。
- 如果 skill 意外不可見，請在變更排名相關中繼資料之前，先於登入狀態使用 `clawhub inspect @owner/slug` 檢查審核狀態。
...
- 當 `nonSuspiciousOnly=true` 時，基於游標的排序可能會在一頁中傳回少於 `limit` 的項目，因為可疑 skills 會在擷取頁面後被篩除。
- 存在 `nextCursor` 時，請用它繼續分頁。短頁本身不代表結果已結束。
...
- `moderation` 僅在 skill 被標記或擁有者正在檢視時包含。
...
- 擁有者和版主可以存取隱藏 skills 的審核詳細資料。
- 公開呼叫端只會對已標記且可見的 skills 取得 `200`。
- 公開呼叫端的證據會經過遮蔽，且只有擁有者/版主會包含原始片段。

### `POST /api/v1/skills/{slug}/report`

回報 skill 以供版主審查。回報屬於 skill 層級，可選擇連結至某個版本，並會送入 skill 回報佇列。
...
`note` 對 `confirmed` 與 `dismissed` 為必填；將 `status` 設回 `open` 時可以省略。對已分流的回報傳入 `finalAction: "hide"`，即可在同一個可稽核工作流程中隱藏該 skill。
...
- 已發布掃描只有在 `update: true` 且掃描成功完成時才會回寫。
- 回應為 `202`，內容為 `{ "ok": true, "scanId": "...", "jobId": "...", "status": "queued", "sourceKind": "published", "update": false, "queue": { "queuedAhead": 0, "queuedAheadIsEstimate": false, "position": 1, "running": 0, "runningIsEstimate": false, "note": "掃描為非同步作業，可能需要一些時間才能完成。" } }`。
...
- 傳回佇列中/執行中/成功/失敗狀態。
- 佇列中時會傳回 `queue.queuedAhead` 和 `queue.position`，讓用戶端可顯示此請求前方有多少個優先手動掃描。非常大的佇列會受到限制，並以 `queuedAheadIsEstimate: true` 回報。
...
- 只有在選取版本已產生技能卡、未被審核以惡意軟體封鎖，且 ClawScan 驗證為乾淨時，`ok` 才會是 `true`。
- 技能身分、發布者身分和選取版本中繼資料是頂層信封欄位（`slug`、`displayName`、`publisherHandle`、`version`、`resolvedFrom`、`tag`、`createdAt`），因此 shell 自動化可以在不解開巢狀包裝器的情況下讀取它們。
...
- `security.signals.dependencyRegistry` 會保留以相容 v1 回應，但依賴項登錄存在性掃描器已停用，且此鍵一律為 `null`。
...
傳回確切技能版本的目前精簡安全性判定。此集合端點適用於已知道需要顯示哪些已安裝 ClawHub 技能版本的用戶端，例如 OpenClaw Control UI。
...
- 結果按項目提供；單一缺少的技能或版本不會讓整個回應失敗。
- 回應僅包含安全性資料。不包含技能卡資料、已產生卡片狀態、成品檔案清單或詳細掃描器酬載。
- `security.signals` 只包含狀態層級的支援證據；完整掃描器詳細資料請使用 `/scan` 或 ClawHub 安全稽核頁面。
- `security.signals.dependencyRegistry` 會保留以相容 v1 回應，但依賴項登錄存在性掃描器已停用，且此鍵一律為 `null`。
...
- `category`（選用）：外掛分類篩選器。僅在請求範圍限定為外掛套件（`/api/v1/plugins`、`/api/v1/code-plugins`、`/api/v1/bundle-plugins`，或含有 `family=code-plugin`/`family=bundle-plugin` 的套件端點）時支援。受控分類與舊版 v1 篩選器別名記錄於 `GET /api/v1/plugins` 下。
...
- 託管的 Skills 包含最新儲存版本檔案，並列於 `_manifest.json` 中，且 `sourceRef: "public-clawhub"`。
- 目前由 GitHub 支援且掃描結果為 `clean` 或 `suspicious` 的 Skills 會包含 `_source_handoff.json`，其中含有 `sourceRef: "public-github"`、repo、commit、path、content hash 和 archive URL。它們不包含 ClawHub 託管的原始檔案。
...
- `version.artifact.kind` 對舊世界套件封存檔為 `legacy-zip`，對
...
- `version.sha256hash` 是給舊用戶端使用的已棄用相容性中繼資料。它會
  ...
  標準發行成品。
- 有掃描資料時，會包含 `version.vtAnalysis`、`version.llmAnalysis` 和 `version.staticScan`。
- 私有套件會傳回 `404`，除非呼叫者可以讀取所屬發布者。

### `GET /api/v1/packages/{name}/versions/{version}/security`

傳回供安裝用戶端使用的精確套件發行版本安全性與信任摘要。
這是公開的 OpenClaw 消費介面，用於判斷已解析的發行版本是否可安裝。
...
- `trust.moderationState` 可為 null。沒有手動發行版本審核時為 `null`。
- `trust.blockedFromDownload` 是安裝封鎖訊號。OpenClaw 和其他安裝用戶端
  應在此值為 `true` 時封鎖安裝，而不是從掃描器或審核欄位重新推導封鎖規則。
- `trust.reasons` 是面向使用者與稽核的說明清單。原因代碼是穩定、
...
- 此端點精確到版本。用戶端應在解析出要安裝的套件版本後呼叫它，
  而不只是讀取最新套件中繼資料後呼叫。
- 私有套件會傳回 `404`，除非呼叫者可以讀取所屬發布者。
- 此端點刻意比擁有者/審核員審核端點更窄。它公開安裝決策與公開說明，
...
- `packageName` 會依 npm 名稱正規化；規劃中的遷移可以沒有該套件。
- 這只追蹤遷移就緒狀態。它不會變更 OpenClaw 或產生 ClawPack。

### `GET /api/v1/packages/moderation/queue`

用於套件發行版本審查佇列的審核員/管理員端點。
...
檢舉套件以供審核員審查。檢舉是套件層級，可選擇連結到版本。
它們會送入審核佇列，但本身不會自動隱藏或封鎖下載；審核員應使用
發行版本審核來核准、隔離或撤銷成品。
...
`note` 在 `confirmed` 和 `dismissed` 時為必填；將 `status` 設回 `open` 時可以省略。傳入 `finalAction: "quarantine"` 或 `finalAction: "revoke"` 搭配已確認的檢舉，可在同一個可稽核工作流程中套用版本審核處置。
...
- 外掛/套件封存檔是含有 `package/` 根目錄的 zip 檔，讓舊版 OpenClaw 用戶端能繼續運作。
...
- 舊版僅 ZIP 的版本會有意省略。
- `dist.tarball`、`dist.integrity` 和 `dist.shasum` 使用 npm 相容欄位，讓使用者可選擇將 npm 指向此鏡像。
...
下載託管的 skill 版本 ZIP，或對目前 GitHub 支援且具有 `clean` 或 `suspicious` 掃描、但沒有託管版本的 skill，回傳 GitHub 原始碼交接資訊。
...
- 下載統計會依每個 UTC 日的唯一身分計算（API 權杖有效時使用 `userId`，否則使用 IP）。
...
- 選填酬載欄位：`migrateOwner`。當它與 `ownerHandle` 一起為 `true` 時，若執行者同時是目前與目標發布者的管理員/擁有者，現有 skill 可移至該擁有者。沒有此選擇加入時，擁有者變更會被拒絕。
...
- 允許的表單欄位為 `payload`、重複的 `files` blob，或一個 `clawpack` tarball 參照。`clawpack` 可以是 `.tgz` blob，或由 upload-url 流程回傳的 storage id。以分段 storage-id 發布時，也必須包含該上傳 URL 回傳的 `clawpackUploadTicket`。
- 使用 `files` 或 `clawpack` 其中之一，絕不要在同一請求中同時使用兩者。
...
- Code 外掛需要 `package.json`、原始碼 repo 中繼資料、原始碼 commit 中繼資料、設定 schema 中繼資料、`openclaw.compat.pluginApi` 和 `openclaw.build.openclawVersion`。
...
僅限管理員。確保某個 handle 存在 org 發布者。如果該 handle 仍指向舊版共用使用者/個人發布者，此端點會先將它遷移成 org 發布者。
...
已驗證的自助式 org 發布者建立。建立新的 org 發布者，並將呼叫者加入為擁有者。此端點不會遷移現有使用者/個人 handle，也不會將發布者標記為受信任/official。
...
僅限管理員。為正當擁有者保留根 slug 和套件名稱，而不發布版本。套件名稱會成為沒有版本列的私有佔位套件，因此同一擁有者之後可將真正的 code-plugin 或 bundle-plugin 版本發布到該名稱。
...
僅限管理員。為已驗證的替代 GitHub OAuth 主體復原個人發布者，而不編輯 Convex Auth 帳號列。請求必須指定兩個不可變的 GitHub provider account id；可變 handle 只作為面向操作員的防護。

端點預設為試執行。套用復原需要在工作人員獨立驗證兩個
GitHub 主體之間的連續性後，設定 `dryRun: false` 和
`confirmIdentityVerified: true`。當目標使用者目前的個人發布者有技能、套件或 GitHub 技能來源時，
復原會以封閉方式失敗。
復原也會遷移已復原發布者技能、技能 slug 別名、套件、套件檢查器警告和衍生搜尋摘要列的舊版 `ownerUserId` 欄位，讓
