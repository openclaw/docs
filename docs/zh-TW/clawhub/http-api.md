...
x-i18n:
    generated_at: "2026-07-03T13:15:29Z"
    model: gpt-5.5
...
第三方目錄可以使用公開讀取端點來列出或搜尋 ClawHub Skills。請快取結果、遵守 `429`/`Retry-After`、將使用者連回標準 ClawHub 列表（`https://clawhub.ai/<owner>/skills/<slug>`），並避免暗示 ClawHub 認可第三方網站。請勿嘗試在公開 API 介面以外鏡像隱藏、私人或被審核封鎖的內容。
...
- 已驗證寫入端點在伺服器知道原因時，不應只回傳裸露的 `Unauthorized`。缺少 token、無效/已撤銷 token，以及已刪除/封鎖/停用帳號，都應取得可採取行動的文字，讓命令列介面用戶端能告知使用者阻擋原因。
...
- `Retry-After`：在 `429` 時重試前等待的秒數（延遲）
...
- 如果沒有可用的受信任用戶端 IP，匿名請求會使用僅依速率限制種類限定範圍的後備 bucket。這些後備 bucket 不包含呼叫端提供的路徑、slug、套件名稱、版本、查詢字串或其他 artifact 參數。

## 錯誤回應

公開 v1 錯誤回應是純文字，並帶有 `content-type: text/plain; charset=utf-8`。
這包括驗證失敗（`400`）、缺少公開資源（`404`）、驗證與權限失敗（`401`/`403`）、速率限制（`429`）以及被封鎖的下載。用戶端應將回應本文讀作人類可讀字串。未知查詢參數會為了相容性而被忽略，但已識別的查詢參數若值無效則會回傳 `400`。
...
- 結果會依相關性順序回傳（embedding 相似度 + 精確 slug/名稱 token 加權 + 小幅熱門度先驗）。
- 相關性比熱門度更強。精確 slug 或顯示名稱 token 符合可以排在互動更強但符合較寬鬆的結果之前。
- ASCII 文字會在單字與標點邊界進行 token 化。例如，`personal-map` 包含獨立的 `map` token，而 `amap-jsapi-skill` 包含 `amap`、`jsapi` 和 `skill`；因此搜尋 `map` 會讓 `personal-map` 比 `amap-jsapi-skill` 有更強的詞彙符合。
- 熱門度採對數縮放並設有上限。當查詢文字符合較弱時，高互動 Skills 可能排名較低。
- 可疑或隱藏的審核狀態可能會根據呼叫端篩選條件與目前審核狀態，將某個 Skill 從公開搜尋中移除。

發布者可探索性指引：

- 將使用者實際會搜尋的詞放在顯示名稱、摘要和標籤中。只有在獨立 slug token 也是你想保留的穩定身分時才使用它。
- 不要只為了追逐某個查詢而重新命名 slug，除非新的 slug 是更好的長期標準名稱。舊 slug 會成為重新導向別名，但標準 URL、顯示的 slug，以及未來搜尋摘要都會使用新 slug。
- 重新命名別名會保留舊 URL 與透過登錄解析的安裝解析能力，但搜尋排名會以重新命名已索引後的標準 Skill metadata 為準。既有統計資料會留在該 Skill 上。
- 如果某個 Skill 意外不可見，請在變更排名相關 metadata 前，先在登入狀態下使用 `clawhub inspect @owner/slug` 檢查審核狀態。
...
- 當 `nonSuspiciousOnly=true` 時，游標式排序在某頁可能回傳少於 `limit` 的項目，因為可疑 Skills 會在頁面擷取後被篩除。
...
- `moderation` 只會在 Skill 被標記，或擁有者檢視它時包含。
...
- 擁有者與審核者可以存取隱藏 Skills 的審核詳細資訊。
- 公開呼叫端只會對已標記且可見的 Skills 取得 `200`。
- 公開呼叫端看到的證據會被遮蔽，只有擁有者/審核者會包含原始片段。
...
`note` 在 `confirmed` 和 `dismissed` 時為必要；將 `status` 設回 `open` 時可以省略。對已分流的回報傳入 `finalAction: "hide"`，可在同一個可稽核工作流程中隱藏該 Skill。
...
- `version.security` 在可用時包含正規化掃描驗證狀態與掃描器詳細資訊
  （VirusTotal + LLM）。

### `GET /api/v1/skills/{slug}/scan`

回傳 Skill 版本的安全掃描驗證詳細資訊。
...
- 掃描請求酬載和可下載報告會在保留期限過後從掃描請求儲存區中過期。
...
- 排入佇列時回傳 `queue.queuedAhead` 和 `queue.position`，讓用戶端可以顯示目前請求前方有多少個優先處理的手動掃描。非常大的佇列會受到限制，並以 `queuedAheadIsEstimate: true` 回報。
...
- 只有當所選版本已有產生的技能卡、未被審核判定為惡意軟體而封鎖，且 ClawScan 驗證為 clean 時，`ok` 才會是 `true`。
- 技能身分、發布者身分和所選版本中繼資料都是頂層信封欄位（`slug`、`displayName`、`publisherHandle`、`version`、`resolvedFrom`、`tag`、`createdAt`），因此 shell 自動化可以在不解開巢狀包裝器的情況下讀取它們。
...
- 回應僅包含安全性資訊。它不包含技能卡資料、產生的卡片狀態、成品檔案清單或詳細掃描器酬載。
...
- 需要單一技能的技能卡驗證信封時使用 `/verify`，需要產生的卡片 Markdown 時使用 `/card`，需要詳細掃描器資料時使用 `/scan`。
...
- `category`（選填）：外掛分類篩選器。僅在請求範圍限定為外掛套件時支援
  ...
  `/api/v1/bundle-plugins`，或帶有
  `family=code-plugin`/`family=bundle-plugin` 的套件端點）。受控分類與
  舊版 v1 篩選器別名記錄於 `GET /api/v1/plugins` 下。

注意事項：

- `family`、`channel`、`isOfficial`、`featured`、
  `highlightedOnly` 或 `sort` 的無效值會回傳 `400`。未知查詢參數會被忽略。
- `GET /api/v1/code-plugins` 和 `GET /api/v1/bundle-plugins` 仍是固定 family 的別名。
- Skill 項目仍由 Skill 登錄檔支援，且仍只能透過 `POST /api/v1/skills` 發布。
- `POST /api/v1/packages` 仍僅適用於 code-plugin 與 bundle-plugin 發行版本。
- 匿名呼叫者只能看到公開套件頻道。
...
- `channel`（選填）：`official`、`community` 或 `private`
- `isOfficial`（選填）：`true` 或 `false`
- `category`（選填）：外掛分類篩選器。僅在請求範圍限定為外掛套件時支援。受控分類與舊版 v1
  篩選器別名記錄於 `GET /api/v1/plugins` 下。

注意事項：

- `family`、`channel`、`isOfficial`、`featured` 或
  `highlightedOnly` 的無效值會回傳 `400`。未知查詢參數會被忽略。
- 匿名呼叫者只能看到公開套件頻道。
...
`trending` 是七天的安裝/下載排行榜，不使用總計數。
在統一的 `/api/v1/packages` 端點上，它僅適用於外掛；Skill 型錄請使用
`/api/v1/skills?sort=trending`。

舊版別名不會被接受作為儲存或作者宣告的分類值。

### `GET /api/v1/skills/export`

大量匯出最新公開 Skills，用於離線分析。
...
- 託管的 Skills 包含最新儲存版本檔案，並會列在
  `_manifest.json` 中，帶有 `sourceRef: "public-clawhub"`。
- 目前由 GitHub 支援且掃描結果為 `clean` 或 `suspicious` 的 Skills 會包含
  `_source_handoff.json`，其中有 `sourceRef: "public-github"`、儲存庫、提交、路徑、
  內容雜湊與封存 URL。它們不包含由 ClawHub 託管的原始檔案。
...
僅限外掛的搜尋，跨越 code-plugin 與 bundle-plugin 套件。
...
- `version.sha256hash` 是給舊用戶端使用的已棄用相容性中繼資料。它會雜湊
  ...
  現代用戶端應使用 `version.artifact.sha256`，此欄位可識別
  標準發行成品。
...
傳回安裝用戶端所需的確切套件發行版本安全性與信任摘要。
這是公開的 OpenClaw 消費介面，用於決定已解析的發行版本是否可安裝。
...
- `package.name`、`package.displayName` 和 `package.family` 可識別已解析的
  登錄套件。
...
- `trust.scanStatus` 是由掃描器輸入與手動發行版本審核推導出的有效信任狀態。
- `trust.moderationState` 可為 null。當沒有手動發行版本審核時，其值為 `null`。
...
- `packageName` 會依 npm 名稱正規化；規劃中的
  遷移可以缺少套件。
- 這只追蹤遷移就緒狀態。它不會變更 OpenClaw，也不會產生
  ClawPacks。
...
`confirmed` 和 `dismissed` 需要 `note`；將 `status` 設回 `open` 時可省略。傳入 `finalAction: "quarantine"` 或 `finalAction: "revoke"`，搭配已確認的檢舉，即可在同一個可稽核工作流程中套用發布版本審核。
...
- `revoked`：先前受信任的發布版本，之後已封鎖。

遭隔離和撤銷的發布版本會在成品下載路由回傳 `403`。
每次變更都會寫入稽核記錄項目。
...
- 外掛/套件封存檔是具有 `package/` 根目錄的 zip 檔，讓舊版 OpenClaw 用戶端能持續運作。
...
- `slug`（必填）
- `hash`（必填）：套件包指紋的 64 字元十六進位 sha256
...
- 軟刪除的版本會回傳 `410`。
- 由 GitHub 支援的 skill 交接不會代理或鏡像位元組。JSON 回應包含 `sourceRef: "public-github"`、`repo`、`commit`、`path`、`contentHash` 和 `archiveUrl`；掃描/目前狀態是門檻，不會作為成功承載的中繼資料包含在內。
- 下載統計會依 UTC 日以唯一身分計算（API token 有效時為 `userId`，否則為 IP）。
...
- 偏好：使用 `multipart/form-data`，包含 `payload` JSON + `files[]` blob。
- 也接受含 `files`（基於 storageId）的 JSON 主體。
- 選用承載欄位：`ownerHandle`。存在時，API 會在伺服器端解析該發布者，並要求行動者具有發布者存取權。
- 選用承載欄位：`migrateOwner`。當與 `ownerHandle` 一起為 `true` 時，如果行動者同時是目前和目標發布者的管理員/擁有者，既有 skill 可移至該擁有者。未選擇加入時，擁有者變更會被拒絕。
...
- 允許的表單欄位為 `payload`、重複的 `files` blob，或一個 `clawpack` tarball 參照。`clawpack` 可為 `.tgz` blob，或上傳 URL 流程回傳的儲存 id。使用預備儲存 id 發布時，也必須包含該上傳 URL 一併回傳的 `clawpackUploadTicket`。
- 在同一個請求中只能使用 `files` 或 `clawpack`，不能同時使用。
- JSON 主體和呼叫者提供的 `payload.files` / `payload.artifact` 中繼資料會被拒絕。
- 直接 multipart 發布請求上限為 18MB。ClawPack tarball 可使用上傳 URL 流程，最高到 120MB tarball 上限。
...
- 程式碼外掛需要 `package.json`、原始碼儲存庫中繼資料、原始碼 commit 中繼資料、設定 schema 中繼資料、`openclaw.compat.pluginApi` 和 `openclaw.build.openclawVersion`。
...
僅限管理員。為合法擁有者保留根 slug 和套件名稱，而不發布版本。套件名稱會變成沒有發布版本資料列的私有佔位套件，因此同一位擁有者稍後可將真正的 code-plugin 或 bundle-plugin 發布版本發布到該名稱。
...
僅限管理員。為已驗證的替代 GitHub OAuth 主體復原個人發布者，而不編輯 Convex Auth 帳號資料列。請求必須指定兩個不可變的 GitHub provider account id；可變 handle 只會作為面向操作員的防護。

端點預設為 dry-run。套用復原需要在工作人員獨立驗證兩個 GitHub 主體之間的連續性後，設定 `dryRun: false` 和
`confirmIdentityVerified: true`。
當目的地使用者目前的個人發布者擁有 Skills、套件或 GitHub Skill 來源時，復原會以關閉狀態失敗。
復原也會遷移已復原發布者的 Skills、Skill slug 別名、套件、套件檢查器警告，以及衍生搜尋摘要列中的舊版 `ownerUserId` 欄位，讓直接擁有者路徑與新的發布者權限一致。已復原 handle 的有效受保護 handle 保留也會重新指派給替代使用者，讓後續個人檔案同步無法還原前使用者的競爭權限。每個主要資料表在每次套用交易中限制為
100 列；較大的復原必須先使用可續傳的擁有者遷移。
GitHub Skill 來源以發布者為範圍，並回報為已檢查，而不是重寫。
