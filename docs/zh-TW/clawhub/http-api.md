...
第三方目錄可以使用公開讀取端點來列出或搜尋 ClawHub Skills。請快取結果、遵守 `429`/`Retry-After`、將使用者連回標準 ClawHub 條目（`https://clawhub.ai/<owner>/skills/<slug>`），並避免暗示 ClawHub 為第三方網站背書。不要嘗試在公開 API 介面之外鏡像隱藏、私人或因審核而封鎖的內容。
...
- 已驗證的寫入端點在伺服器知道原因時，不應只傳回空泛的 `Unauthorized`。缺少權杖、無效/已撤銷的權杖，以及已刪除/封鎖/停用的帳號，都應取得可採取行動的文字，讓命令列介面用戶端能告知使用者是什麼阻擋了他們。
...
- `Retry-After`：在 `429` 上重試前要等待的秒數（延遲）
...
- 如果沒有可用的受信任用戶端 IP，匿名請求會使用僅依速率限制類型作用域化的退回儲存桶。這些退回儲存桶不包含呼叫端提供的路徑、slug、套件名稱、版本、查詢字串或其他成品參數。

## 錯誤回應

公開 v1 錯誤回應是純文字，並帶有 `content-type: text/plain; charset=utf-8`。
這包括驗證失敗（`400`）、缺少公開資源（`404`）、驗證與權限失敗（`401`/`403`）、速率限制（`429`），以及被封鎖的下載。用戶端應將回應主體讀取為可供人類閱讀的字串。未知查詢參數會因相容性而被忽略，但已識別且值無效的查詢參數會傳回 `400`。
...
- `q`（必要）：查詢字串
- `limit`（選用）：整數
- `highlightedOnly`（選用）：`true` 表示篩選為醒目顯示的 Skills
- `nonSuspiciousOnly`（選用）：`true` 表示隱藏可疑的（`flagged.suspicious`）Skills
...
- 相關性比人氣更強。精確的 slug 或顯示名稱權杖符合，可能會高於互動量強得多但符合較寬鬆的結果。
- ASCII 文字會依單字和標點邊界切分權杖。例如，`personal-map` 包含獨立的 `map` 權杖，而 `amap-jsapi-skill` 包含 `amap`、`jsapi` 和 `skill`；因此搜尋 `map` 時，`personal-map` 會比 `amap-jsapi-skill` 有更強的詞彙符合。
- 人氣會經過對數縮放並設上限。當查詢文字的符合較弱時，高互動 Skills 仍可能排名較低。
- 視呼叫端篩選條件和目前審核狀態而定，可疑或隱藏的審核狀態可能會使某個 Skill 從公開搜尋中移除。

發布者可發現性指引：

- 將使用者會實際搜尋的字詞放在顯示名稱、摘要和標籤中。只有在獨立 slug 權杖也是你想保留的穩定身分時，才使用它。
- 不要只為了追逐某個查詢而重新命名 slug，除非新的 slug 是更好的長期標準名稱。舊 slug 會成為重新導向別名，但標準 URL、顯示的 slug，以及未來搜尋摘要會使用新的 slug。
- 重新命名別名會為舊 URL 和透過登錄解析的安裝保留解析能力，但搜尋排名會以重新命名已建立索引後的標準 Skill 中繼資料為準。既有統計資料會保留在該 Skill 上。
- 如果某個 Skill 意外不可見，請在變更排名相關中繼資料之前，先以登入狀態使用 `clawhub inspect @owner/slug` 檢查審核狀態。
...
- `recommended` 會使用互動和近期性訊號。
- `trending` 依過去 7 天的安裝數排名（以遙測為基礎）。
- `createdAt` 對新 Skill 爬取是穩定的；`updated` 會在既有 Skills 重新發布時變更。
- 當 `nonSuspiciousOnly=true` 時，基於游標的排序可能會在某一頁傳回少於 `limit` 的項目，因為可疑 Skills 會在頁面擷取後被篩除。
- 存在 `nextCursor` 時，使用它繼續分頁。短頁面本身不代表結果結束。
...
- 證據會對公開呼叫端遮蔽，且只有擁有者/審核員會取得原始片段。

### `POST /api/v1/skills/{slug}/report`

回報某個 Skill 供審核員檢閱。回報以 Skill 為層級，可選擇連結到某個版本，並會送入 Skill 回報佇列。
...
`note` 對 `confirmed` 和 `dismissed` 為必要；將 `status` 設回 `open` 時可省略。傳入 `finalAction: "hide"` 搭配已分流處理的回報，可在同一個可稽核工作流程中隱藏該 Skill。
...
- 回傳已佇列/執行中/成功/失敗狀態。
- 在佇列中時回傳 `queue.queuedAhead` 和 `queue.position`，讓用戶端可以顯示有多少優先手動掃描排在該請求前方。非常大的佇列會被限制，並以 `queuedAheadIsEstimate: true` 回報。
...
- `kind` 預設為 `skill`；對外掛/套件掃描使用 `kind=plugin`。
- 回傳與掃描請求下載相同的 ZIP 形狀。

### `POST /api/v1/skills/-/scan/batch`

僅限管理員的標準批次重新掃描路由。它接受與舊版 `POST /api/v1/skills/-/rescan-batch` 相同的承載形狀。
...
- 技能識別、發布者識別和所選版本中繼資料都是頂層信封欄位（`slug`、`displayName`、`publisherHandle`、`version`、`resolvedFrom`、`tag`、`createdAt`），讓 shell 自動化可以不需解開巢狀包裝器即可讀取。
...
- `security.signals.dependencyRegistry` 會保留以維持 v1 回應相容性，但依賴登錄存在性掃描器已停用，且此鍵一律為 `null`。
...
- 結果依項目分列；單一遺失的技能或版本不會讓整個回應失敗。
- 回應僅包含安全性資訊。它不包含 Skill Card 資料、已產生卡片狀態、成品檔案清單或詳細掃描器承載。
- `security.signals` 僅包含狀態層級的支援證據；完整掃描器詳細資料請使用 `/scan` 或 ClawHub 安全稽核頁面。
- `security.signals.dependencyRegistry` 會保留以維持 v1 回應相容性，但依賴登錄存在性掃描器已停用，且此鍵一律為 `null`。
- Skill Card 不存在不會影響此端點的 `ok`、`decision` 或 `reasons`；用戶端需要卡片內容時，應在本機讀取已安裝的 `skill-card.md`。
...
- `category`（選填）：外掛分類篩選器。僅在請求範圍限定為外掛套件（`/api/v1/plugins`、
...
- `channel`（選填）：`official`、`community` 或 `private`
- `isOfficial`（選填）：`true` 或 `false`
- `category`（選填）：外掛分類篩選器。僅在請求範圍限定為外掛套件時支援。受控分類和舊版 v1
...
- `version.sha256hash` 是供舊用戶端使用的已棄用相容性中繼資料。它
  ...
  現代用戶端應使用 `version.artifact.sha256`，其識別
  標準版本成品。
...
- `trust.moderationState` 可為 null。當沒有手動版本
  審核時為 `null`。
...
- `trust.stale` 表示信任摘要是由過期輸入計算而成，
  在做出高信心允許決策前應視為需要重新整理。

注意事項：

- 此端點精確到版本。用戶端應在解析出打算安裝的
  套件版本後呼叫它，而不只是讀取最新
  套件中繼資料後呼叫。
- 私人套件會傳回 `404`，除非呼叫者可讀取所屬發布者。
- 此端點刻意比擁有者/版主審核端點更窄。
  它公開安裝決策與公開說明，而不是
  回報者身分、回報內容、私人證據或內部審查
  時間線。
...
- `bundledPluginId` 會標準化為小寫，並且是穩定的 upsert 鍵。
- `packageName` 會以 npm 名稱標準化；對於計畫中的
  遷移，套件可以不存在。
- 這只追蹤遷移就緒度。它不會變更 OpenClaw，也不會產生
  ClawPack。
...
供擁有者/審核者查看套件審核可見性的端點。

驗證：

- 需要套件擁有者、發布者成員、審核者或管理員使用者的 API 權杖。
...
`confirmed` 和 `dismissed` 需要 `note`；將 `status` 設回 `open` 時可省略。對已確認的回報傳入 `finalAction: "quarantine"` 或
`finalAction: "revoke"`，即可在同一個可稽核工作流程中套用發布版本審核。
...
- `revoked`：先前受信任的發布版本之後遭封鎖。

遭隔離和撤銷的發布版本會從成品下載路由傳回 `403`。
每次變更都會寫入稽核記錄項目。
...
- 外掛/套件封存檔是含有 `package/` 根目錄的 zip 檔案，讓舊版 OpenClaw 用戶端保持可用。
...
- 待處理的 VirusTotal 掃描不會封鎖下載；惡意發布版本會傳回 `403`。
...
下載託管的 skill 版本 ZIP，或對目前由 GitHub 支援、掃描結果為 `clean` 或 `suspicious` 且沒有託管版本的 skill，傳回 GitHub 原始碼交接資訊。
...
- 下載統計以每 UTC 日的唯一身分計算（API 權杖有效時為 `userId`，否則為 IP）。
...
- 選用酬載欄位：`ownerHandle`。存在時，API 會在伺服器端解析該發布者，並要求行動者具有發布者存取權。
- 選用酬載欄位：`migrateOwner`。與 `ownerHandle` 一起為 `true` 時，如果行動者同時是目前和目標發布者的管理員/擁有者，既有 skill 可移至該擁有者。若未明確選擇此選項，擁有者變更會被拒絕。
...
- 允許的表單欄位為 `payload`、重複的 `files` blob，或一個 `clawpack` tarball 參照。`clawpack` 可為 `.tgz` blob，或由 upload-url 流程傳回的儲存 ID。使用暫存 storage-id 發布時，也必須包含該上傳 URL 傳回的 `clawpackUploadTicket`。
- 使用 `files` 或 `clawpack` 其中之一，不可在同一請求中同時使用。
...
- 程式碼外掛需要 `package.json`、原始碼儲存庫中繼資料、原始碼提交中繼資料、設定結構描述中繼資料、`openclaw.compat.pluginApi` 和
  `openclaw.build.openclawVersion`。
- `openclaw.hostTargets` 和 `openclaw.environment` 是選用中繼資料。
- 只有 `openclaw` 組織發布者和目前 `openclaw` 組織成員的個人發布者可發布到 `official` 頻道。
- 代為發布仍會根據目標擁有者帳戶驗證 official-channel 資格。
...
由擁有者發起的軟刪除會保留 slug 30 天，之後該 slug 可由另一位發布者宣告。
刪除回應會在此到期情況適用時包含 `slugReservedUntil`。
審核者/管理員隱藏和安全性移除不會以這種方式到期。
...
僅限管理員。確保 handle 有組織發布者存在。如果該 handle 仍指向舊版共用使用者/個人發布者，端點會先將它遷移為組織發布者。
...
已驗證的自助式組織發布者建立。建立新的組織發布者，並將呼叫者新增為擁有者。此端點不會遷移既有使用者/個人 handle，也不會將發布者標記為受信任/官方。
...
僅限管理員。為正當擁有者保留根 slug 和套件名稱，而不發布版本。套件名稱會成為沒有發布版本列的私有佔位套件，因此同一擁有者稍後可將真正的 code-plugin 或 bundle-plugin 發布版本發布到該名稱。
...
僅限管理員。為已驗證的替代 GitHub OAuth 主體復原個人發布者，而不編輯 Convex Auth 帳戶列。請求必須指定兩個不可變的 GitHub provider account id；可變 handle 僅作為面向操作員的保護檢查。

端點預設為 dry-run。套用復原需要在工作人員獨立驗證兩個
GitHub 主體之間的連續性後，設定 `dryRun: false` 與
`confirmIdentityVerified: true`。當目的地使用者目前的個人
publisher 擁有技能、套件或 GitHub 技能來源時，復原會封閉失敗。
復原也會遷移已復原 publisher 的技能、技能 slug 別名、套件、套件檢查器警告，以及衍生搜尋摘要列中的舊版 `ownerUserId` 欄位，讓
直接擁有者路徑與新的 publisher 授權一致。已復原 handle 的有效受保護 handle
保留也會重新指派給替代使用者，因此後續
個人資料同步無法還原前使用者的競爭授權。每個主要資料表在每次套用交易中限制為
100 列；較大的復原必須先使用可續傳的擁有者遷移。
GitHub 技能來源以 publisher 為範圍，並回報為已檢查，而不是重寫。
...
`POST /api/cli/upload-url` 會傳回 `uploadUrl` 和 `uploadTicket`。將 ClawPack tarball 暫存的套件
發布，必須將產生的儲存空間 ID 作為
