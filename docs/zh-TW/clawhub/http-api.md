...
舊版 `/api/...` 與 `/api/cli/...` 會保留以維持相容性（請見 `DEPRECATIONS.md`）。
OpenAPI：`/api/v1/openapi.json`。

## 公開目錄重用

第三方目錄可以使用公開讀取端點來列出或搜尋 ClawHub Skills。請快取結果、遵守 `429`/`Retry-After`、將使用者連回標準 ClawHub 清單頁（`https://clawhub.ai/<owner>/<slug>`），並避免暗示 ClawHub 為該第三方網站背書。請勿嘗試在公開 API 介面之外鏡像隱藏、私人或遭審核封鎖的內容。
...
- 已驗證的寫入端點在伺服器知道原因時，不應只回傳空泛的 `Unauthorized`。遺失 token、無效/已撤銷 token，以及已刪除/停權/停用帳戶，都應取得可採取行動的文字，讓 CLI 用戶端能告知使用者被阻擋的原因。

- 讀取：每 IP 每分鐘 600 次，每 key 每分鐘 2400 次
- 寫入：每 IP 每分鐘 45 次，每 key 每分鐘 180 次
- 下載：每 IP 每分鐘 30 次，每 key 每分鐘 180 次（`/api/v1/download`）
...
- 如果沒有可用的受信任用戶端 IP，匿名下載請求會使用端點範圍的備用 bucket，而不是單一全域 `ip:unknown` bucket。匿名讀取/寫入請求仍使用共享 unknown bucket，讓缺失 IP 的路由保持可見且保守。
...
- `q`（必填）：查詢字串
- `limit`（選填）：整數
- `highlightedOnly`（選填）：`true` 表示只篩選精選 Skills
...
- 結果會依相關性順序傳回（embedding 相似度 + 精確 slug/名稱 token 加權 + 來自下載數的熱門度先驗）。
- 相關性比熱門度更強。精確的 slug 或顯示名稱 token 相符，可能會排在下載數多很多但相符較鬆散的結果之前。
- ASCII 文字會依單字與標點邊界切分 token。例如，`personal-map` 包含獨立的 `map` token，而 `amap-jsapi-skill` 包含 `amap`、`jsapi` 與 `skill`；因此搜尋 `map` 會讓 `personal-map` 比 `amap-jsapi-skill` 取得更強的詞彙相符。
- 下載數會作為小幅對數縮放的先驗與平手決勝因素，而不是主要排名訊號。當查詢文字相符較弱時，高下載量 Skills 仍可能排名較低。
- 可疑或隱藏的審核狀態，可能會依呼叫端篩選條件與目前審核狀態，將 Skill 從公開搜尋中移除。

發布者可探索性指引：

- 將使用者會實際搜尋的詞放入顯示名稱、摘要與標籤中。只有當獨立 slug token 也是你想保留的穩定身分時，才使用它。
- 不要只是為了追逐某個查詢而重新命名 slug，除非新 slug 是更好的長期標準名稱。舊 slug 會成為重新導向別名，但標準 URL、顯示的 slug 與未來搜尋摘要會使用新 slug。
- 重新命名別名會為透過登錄解析的舊 URL 與安裝保留解析能力，但搜尋排名會依重新命名索引完成後的標準 Skill 中繼資料為準。既有統計資料會保留在該 Skill 上。
- 如果某個 Skill 意外不可見，請先在登入狀態下使用 `clawhub inspect <slug>` 檢查審核狀態，再變更排名相關中繼資料。
...
- 當 `nonSuspiciousOnly=true` 時，游標式排序在某一頁傳回的項目可能少於 `limit`，因為可疑 Skills 會在擷取頁面後被篩除。
- 若存在 `nextCursor`，請用它繼續分頁。短頁本身不代表結果已結束。
...
- 證據會對公開呼叫端遮蔽，且只有擁有者/審核者會看到原始片段。

### `POST /api/v1/skills/{slug}/report`

回報 Skill 以供審核者檢閱。回報屬於 Skill 層級，可選擇連結到某個版本，並會進入 Skill 回報佇列。
...
供 Skill 擁有者/發布者對 Skill 審核提出申訴的端點。

驗證：

- 需要 Skill 擁有者或發布者成員的 API token。
...
隱藏、移除、可疑、惡意或掃描器標記的 Skill 結果，都可接受申訴。ClawHub 對每個 Skill 保留一個開放中的申訴。
...
要求對最新發布的 Skill 版本進行安全重新掃描。

驗證：

- 需要 Skill 擁有者、發布者管理員、平台審核者或平台管理員的 API token。
- 擁有者與發布者管理員受每版本擁有者復原限制約束。平台審核者與管理員不受此限制，但 ClawHub 仍只允許每個版本有一個作用中的重新掃描。
...
`confirmed` 與 `dismissed` 需要 `note`；將 `status` 設回 `open` 時可省略。傳入 `finalAction: "hide"` 搭配已分流的回報，可在同一個可稽核工作流程中隱藏該 Skill。

### `GET /api/v1/skills/-/appeals`

供審核者/管理員接收 Skill 申訴的端點。

查詢參數：

- `status`（選填）：`open`（預設）、`accepted`、`rejected` 或 `all`
- `limit`（選填）：整數（1-200）
- `cursor`（選填）：分頁游標

### `POST /api/v1/skills/-/appeals/{appealId}/resolve`

供審核者/管理員接受、拒絕或重新開啟 Skill 申訴的端點。
`accepted` 與 `rejected` 需要 `note`；將 `status` 設回 `open` 時可省略。傳入 `finalAction: "restore"` 搭配已接受的申訴，可讓該 Skill 再次可用。
...
- 偵測到時，`security.capabilityTags` 會包含確定性的能力/風險標籤，例如
...
- `moderation` 是由最新版本衍生出的目前 skill 層級審核快照。
...
- `npmMirror`（選填）：`true`/`1`，用來顯示可透過 npm 鏡像取得、
  由 ClawPack 支援的套件版本
...
- `artifactKind`（選填）：`legacy-zip` 或 `npm-pack`
- `npmMirror`（選填）：`true`/`1`，用來搜尋可透過 npm 鏡像取得、
  由 ClawPack 支援的套件版本
...
- Skills 也可透過統一目錄中的此路由解析。
- 除非呼叫者可讀取擁有方發布者，否則私有套件會傳回 `404`。
...
用於列出官方 OpenClaw plugin 遷移列的審核員端點。
...
- `packageName` 會經過 npm-name 正規化；規劃中的
  遷移可以缺少套件。
...
- `open`：可疑、惡意、待處理、已隔離、已撤銷或已回報的發行。
- `blocked`：已隔離、已撤銷或惡意的發行。
- `manual`：任何具有手動審核覆寫的發行。
- `all`：任何具有手動覆寫、非 clean 掃描狀態或套件回報的發行。
...
向審核員回報套件以供審查。回報屬於套件層級，可選擇性地
連結至版本。它們會進入審核佇列，但本身不會自動隱藏或
封鎖下載；審核員應使用發行審核來
核准、隔離或撤銷成品。
...
套件擁有者/發布者端點，用於對發行的審核提出申訴。

驗證：

- 需要套件擁有者或發布者成員的 API token。
...
只有已隔離、已撤銷、
可疑或惡意的發行才會接受申訴。ClawHub 會為每個發行保留一個未結申訴。
...
- 需要套件擁有者、發布者管理員、平台審核員或平台管理員的 API 權杖。
- 擁有者和發布者管理員會受每個發布版本的擁有者復原限制約束。平台審核員和管理員不受此限制，但 ClawHub 仍然只允許每個發布版本有一個作用中的重新掃描。
...
`note` 在 `accepted` 和 `rejected` 時為必填；將 `status` 設回 `open` 時可以省略。對已接受的申訴傳入 `finalAction: "approve"`，可在同一個可稽核工作流程中核准受影響的發布版本。
...
`note` 在 `confirmed` 和 `dismissed` 時為必填；將 `status` 設回 `open` 時可以省略。對已確認的檢舉傳入 `finalAction: "quarantine"` 或 `finalAction: "revoke"`，可在同一個可稽核工作流程中套用發布版本審核。
...
- `revoked`：先前受信任的發布版本被封鎖。

已隔離和已撤銷的發布版本會從成品下載路由回傳 `403`。每次變更都會寫入稽核記錄項目。

### `POST /api/v1/packages/backfill/artifacts`

僅限管理員的維護端點，用於為較舊的套件發布版本標記明確的成品種類中繼資料。
...
- 預設為試執行。
- 沒有 ClawPack 儲存空間的發布版本會標記為 `legacy-zip`。
- 缺少 `artifactKind` 的既有 ClawPack 後援資料列會修復為 `npm-pack`。
...
- 二進位檔案會回傳 `415`。
- 檔案大小限制：200KB。
- 等待中的 VirusTotal 掃描不會封鎖讀取；惡意發布版本仍可能在其他地方被保留。
...
- Plugin/套件封存檔是具有 `package/` 根目錄的 zip 檔案，讓舊版 OpenClaw 用戶端保持正常運作。
- 此路由維持僅限 ZIP。它不會串流 ClawPack `.tgz` 檔案。
- 回應會包含 `ETag`、`Digest`、`X-ClawHub-Artifact-Type` 和 `X-ClawHub-Artifact-Sha256` 標頭，以供解析器完整性檢查使用。
- 僅限登錄檔的中繼資料不會注入下載的封存檔。
- 等待中的 VirusTotal 掃描不會封鎖下載；惡意發布版本會回傳 `403`。
- 私有套件會回傳 `404`，除非呼叫者是擁有者。

### `GET /api/npm/{package}`

回傳 ClawPack 後援套件版本的 npm 相容 packument。
...
- 作用域套件 packument 同時支援 `/api/npm/@scope/name` 和 npm 的編碼請求路徑 `/api/npm/@scope%2Fname`。
...
- 軟刪除版本會回傳 `410`。
- 下載統計會以每小時唯一身分計算（API 權杖有效時為 `userId`，否則為 IP）。
...
- 選填酬載欄位：`migrateOwner`。當它與 `ownerHandle` 一起為 `true` 時，如果執行者同時是目前和目標發布者的管理員/擁有者，既有技能可移至該擁有者。沒有此選擇加入時，擁有者變更會被拒絕。
...
- 程式碼 Plugin 需要 `package.json`、原始碼儲存庫中繼資料、原始碼提交中繼資料、設定架構中繼資料、`openclaw.compat.pluginApi` 和 `openclaw.build.openclawVersion`。
- `openclaw.hostTargets` 和 `openclaw.environment` 是選填中繼資料。
- 只有受信任的發布者可以發布到 `official` 頻道。
- 代表發布仍會依目標擁有者帳戶驗證官方頻道資格。
...
由擁有者發起的軟刪除會保留 slug 30 天，之後 slug 可由另一個發布者宣告。
...
僅限管理員。確保某個控制代碼存在組織發布者。如果該控制代碼仍指向舊版共享使用者/個人發布者，此端點會先將其遷移為組織發布者。
...
版本。套件名稱會變成沒有版本列的私人預留位置套件，因此同一位
