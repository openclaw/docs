...
x-i18n:
    generated_at: "2026-07-01T15:18:51Z"
    model: gpt-5.5
...
第三方目錄可以使用公開讀取端點列出或搜尋 ClawHub Skills。請快取結果、遵守 `429`/`Retry-After`、將使用者連回標準 ClawHub 清單（`https://clawhub.ai/<owner>/skills/<slug>`），並避免暗示 ClawHub 背書該第三方網站。不要嘗試在公開 API 表面之外鏡像隱藏、私人或遭審核封鎖的內容。
...
- 已驗證的寫入端點在伺服器知道原因時，不應只回傳裸露的 `Unauthorized`。缺少 token、無效或已撤銷的 token，以及已刪除、已停權或已停用的帳號，都應取得可操作的文字，讓命令列介面用戶端能告知使用者被阻擋的原因。

- 讀取：每 IP 每分鐘 3000 次，每 key 每分鐘 12000 次
- 寫入：每 IP 每分鐘 300 次，每 key 每分鐘 3000 次
- 下載：每 IP 每分鐘 1200 次，每 key 每分鐘 6000 次（下載端點）
...
- `X-RateLimit-Remaining` / `RateLimit-Remaining`：存在時代表精確剩餘額度。
  分片成功請求會省略此標頭，而不是回傳近似的全域值。
...
- 如果沒有可用的受信任用戶端 IP，匿名請求會使用僅依速率限制種類劃分的 fallback bucket。這些 fallback bucket 不包含呼叫者提供的路徑、slug、套件名稱、版本、查詢字串或其他成品參數。

## 錯誤回應

公開 v1 錯誤回應是純文字，`content-type: text/plain; charset=utf-8`。
這包括驗證失敗（`400`）、缺少公開資源（`404`）、驗證與權限失敗（`401`/`403`）、速率限制（`429`）以及遭封鎖的下載。用戶端應將回應本文讀取為人類可讀字串。未知查詢參數會為相容性而忽略，但可辨識查詢參數若有無效值會回傳 `400`。
...
- 結果會依相關性順序回傳（embedding 相似度 + 精確 slug/名稱 token 加權 + 小幅人氣先驗）。
- 相關性比人氣更強。精確的 slug 或顯示名稱 token 相符，可以排在互動更強但相符較鬆散的結果之前。
- ASCII 文字會依單字與標點邊界分詞。例如，`personal-map` 包含獨立的 `map` token，而 `amap-jsapi-skill` 包含 `amap`、`jsapi` 和 `skill`；因此搜尋 `map` 會讓 `personal-map` 比 `amap-jsapi-skill` 有更強的詞彙相符。
- 人氣會做 log 縮放並設上限。當查詢文字相符較弱時，高互動 Skills 可能排名較低。
- 可疑或隱藏的審核狀態可能會依呼叫者篩選條件與目前審核狀態，從公開搜尋中移除 Skills。

發布者可發現性指引：

- 將使用者會實際搜尋的詞放入顯示名稱、摘要和標籤中。只有在獨立 slug token 也是你想保留的穩定身分時才使用。
- 不要只為追逐某個查詢而重新命名 slug，除非新 slug 是更好的長期標準名稱。舊 slug 會變成重新導向別名，但標準 URL、顯示的 slug，以及未來搜尋摘要會使用新 slug。
- 重新命名別名會保留舊 URL，以及透過登錄解析的安裝解析能力，但搜尋排名會基於重新命名完成索引後的標準 Skill metadata。現有統計會留在該 Skill 上。
- 如果某個 Skill 意外不可見，請在登入後先用 `clawhub inspect @owner/slug` 檢查審核狀態，再變更與排名相關的 metadata。
...
- `limit`（選用）：整數（1–200）
- `cursor`（選用）：任何非 `trending` 排序的分頁 cursor
...
- 當 `nonSuspiciousOnly=true` 時，基於 cursor 的排序可能在一頁中回傳少於 `limit` 的項目，因為可疑 Skills 會在頁面擷取後被篩除。
- 存在 `nextCursor` 時，用它繼續分頁。短頁本身不代表結果已結束。
...
- 由擁有者重新命名或合併流程建立的舊 slug 會解析到標準 Skill。
- `metadata.os`：Skill frontmatter 中宣告的 OS 限制（例如 `["macos"]`、`["linux"]`）。若未宣告則為 `null`。
- `metadata.systems`：Nix system targets（例如 `["aarch64-darwin", "x86_64-linux"]`）。若未宣告則為 `null`。
- 如果該 Skill 沒有平台 metadata，`metadata` 會是 `null`。
- `moderation` 只會在該 Skill 被標記，或擁有者正在檢視它時包含。
...
- 證據會對公開呼叫者遮蔽，且只有擁有者或審核員可看到原始片段。

### `POST /api/v1/skills/{slug}/report`

回報 Skill 以供審核員審查。回報屬於 Skill 層級，可選擇連結到某個版本，並會進入 Skill 回報佇列。
...
`note` 對 `confirmed` 和 `dismissed` 是必要的；將 `status` 設回 `open` 時可以省略。將 `finalAction: "hide"` 搭配已 triage 的回報傳入，可在同一個可稽核工作流程中隱藏該 Skill。
...
- 掃描請求酬載和可下載報告會在保留期間結束後從掃描請求儲存區過期。
...
- 掃描作業是非同步的。手動掃描請求會優先於一般發布/回填工作，但完成時間仍取決於工作處理器可用性。
...
- 回傳已排隊/執行中/成功/失敗狀態。
- 排隊時回傳 `queue.queuedAhead` 和 `queue.position`，讓用戶端可以顯示此請求前方有多少優先處理的手動掃描。非常大的佇列會受到上限限制，並以 `queuedAheadIsEstimate: true` 回報。
- 可用時，`report` 會包含 `clawscan`、`skillspector`、`staticAnalysis` 和 `virustotal` 區段。
- 失敗的掃描作業會回傳 `status: "failed"` 以及 `lastError`。
...
- 回傳一個 ZIP，其中包含 `manifest.json`、`clawscan.json`、`skillspector.json`、`static-analysis.json`、`virustotal.json` 和 `README.md`。

### `GET /api/v1/skills/-/scan/download/{name}?version=<version>&kind=skill|plugin`

用於已提交版本的已驗證儲存報告封存端點。

- 需要該技能或外掛的擁有者/發布者管理存取權，或平台審核員/管理員權限。
...
回傳 `clawhub skill verify` 使用的技能卡驗證封套。
...
- 只有當所選版本有已產生的技能卡、未被審核標記為惡意軟體封鎖，且 ClawScan 驗證為乾淨時，`ok` 才會是 `true`。
- 技能身分、發布者身分和所選版本中繼資料是頂層封套欄位（`slug`、`displayName`、`publisherHandle`、`version`、`resolvedFrom`、`tag`、`createdAt`），讓 shell 自動化無需拆開巢狀包裝即可讀取它們。
- `security` 是頂層 ClawScan/安全性判定。自動化應依據 `ok`、`decision`、`reasons` 和 `security.status` 判斷。
- `security.signals` 包含支援性的掃描器證據，例如 `staticScan`、`virusTotal` 和 `skillSpector`。
- `security.signals.dependencyRegistry` 為 v1 回應相容性而保留，但依賴項登錄存在性掃描器已停用，且此鍵一律為 `null`。
- 只有當 ClawHub 在發布或匯入期間解析並儲存 GitHub 儲存庫/ref/提交/path 時，`provenance` 才會是 `server-resolved-github-import`；否則為 `unavailable`。

### `POST /api/v1/skills/-/security-verdicts`

回傳精確技能版本目前的精簡安全性判定。此集合端點
適用於已知道需要顯示哪些已安裝
...
- 結果依項目回傳；單一技能或版本缺失不會導致整個回應失敗。
- 回應僅包含安全性資訊。它不包含技能卡資料、已產生卡片狀態、成品檔案清單或詳細掃描器酬載。
- `security.signals` 只包含狀態層級的支援證據；如需完整掃描器詳細資料，請使用 `/scan` 或 ClawHub security-audit 頁面。
- `security.signals.dependencyRegistry` 為 v1 回應相容性而保留，但依賴項登錄存在性掃描器已停用，且此鍵一律為 `null`。
- 技能卡缺失不會影響此端點的 `ok`、`decision` 或 `reasons`；用戶端需要卡片內容時，應在本機讀取已安裝的 `skill-card.md`。
- 當你需要單一技能的技能卡驗證封套時使用 `/verify`，需要已產生卡片 Markdown 時使用 `/card`，需要詳細掃描器資料時使用 `/scan`。
...
- `category`（選填）：外掛分類篩選器。僅在請求範圍限定為外掛套件（`/api/v1/plugins`、`/api/v1/code-plugins`、`/api/v1/bundle-plugins`，或帶有 `family=code-plugin`/`family=bundle-plugin` 的套件端點）時支援。受控分類與舊版 v1 篩選器別名記載於 `GET /api/v1/plugins` 下。
...
- `channel`（選填）：`official`、`community` 或 `private`
- `isOfficial`（選填）：`true` 或 `false`
- `category`（選填）：外掛分類篩選器。僅在請求範圍限定為外掛套件時支援。受控分類與舊版 v1 篩選器別名記載於 `GET /api/v1/plugins` 下。
...
僅限外掛的型錄瀏覽，涵蓋 code-plugin 與 bundle-plugin 套件。
...
在統一的 `/api/v1/packages` 端點上，它僅適用於外掛；技能型錄請使用
`/api/v1/skills?sort=trending`。

舊版別名不可作為儲存或作者宣告的分類值接受。
...
- 目前由 GitHub 支援且掃描結果為 `clean` 或 `suspicious` 的技能包含 `_source_handoff.json`，其中含有 `sourceRef: "public-github"`、repo、commit、path、content hash 與 archive URL。它們不包含 ClawHub 託管的原始檔案。
...
跨 code-plugin 與 bundle-plugin 套件的外掛專用搜尋。
...
- 也接受 `GET /api/v1/plugins` 下記錄的舊版 v1 篩選別名。
- 類別篩選是由外掛類別摘要資料列支援的真實 API 篩選器，
  不是搜尋查詢重寫。
- 結果會依相關性順序傳回，目前不分頁。
...
- 對於舊世界套件封存檔，`version.artifact.kind` 為 `legacy-zip`；
  對於由 ClawPack 支援的發行版，則為 `npm-pack`。
- ClawPack 發行版包含 npm 相容的 `npmIntegrity`、`npmShasum` 與
  `npmTarballName` 欄位。
- `version.sha256hash` 是舊用戶端的已棄用相容性中繼資料。它會雜湊
  `/api/v1/packages/{name}/download` 傳回的確切 ZIP 位元組。
...
傳回安裝用戶端所需的確切套件發行安全性與信任摘要。這是用於判斷已解析發行版是否可安裝的公開 OpenClaw 取用介面。
...
- `package.name`、`package.displayName` 與 `package.family` 會識別
  已解析的登錄套件。
- `release.releaseId`、`release.version` 與 `release.createdAt` 會識別
  已評估的確切發行版。
- 當發行成品已知時，會提供 `release.artifactKind`、`release.artifactSha256`、`release.npmIntegrity`、
  `release.npmShasum` 與 `release.npmTarballName`。
- `trust.scanStatus` 是由掃描器輸入與手動發行審核衍生出的有效信任狀態。
- `trust.moderationState` 可為 null。當不存在手動發行審核時，其值為 `null`。
- `trust.blockedFromDownload` 是安裝封鎖訊號。OpenClaw 與其他安裝用戶端應在此值為 `true` 時封鎖安裝，而不是從掃描器或審核欄位重新推導封鎖規則。
...
- `trust.stale` 表示信任摘要是由過時輸入計算而得，在做出高信心允許決策前，應視為需要重新整理。

備註：

- 此端點精確到版本。用戶端應在解析出打算安裝的套件版本後呼叫它，而不是只在讀取最新套件中繼資料後呼叫。
- 私有套件會傳回 `404`，除非呼叫端可以讀取擁有的發布者。
- 此端點刻意比擁有者/版主審核端點更窄。它公開安裝決策與公開說明，而不是回報者身分、回報內容、私有證據或內部審查時間軸。
...
回報套件以供版主審查。回報屬於套件層級，可選擇連結到版本。它們會進入審核佇列，但本身不會自動隱藏或封鎖下載；版主應使用發行版審核來核准、隔離或撤銷成品。
...
`note` 對 `confirmed` 和 `dismissed` 為必填；將 `status` 設回 `open` 時可以省略。傳入 `finalAction: "quarantine"` 或 `finalAction: "revoke"` 搭配已確認的檢舉，即可在同一個可稽核工作流程中套用版本審核。
...
- `revoked`：先前受信任的版本發行後遭封鎖。

遭隔離和撤銷的版本會從成品下載路由回傳 `403`。
每次變更都會寫入稽核記錄項目。
...
- 二進位檔案回傳 `415`。
- 檔案大小限制：200KB。
- 等待中的 VirusTotal 掃描不會封鎖讀取；惡意版本仍可能在其他地方被扣留。
...
- 外掛/套件封存檔是含有 `package/` 根目錄的 zip 檔，因此舊版 OpenClaw 用戶端可繼續運作。
- 此路由維持僅支援 ZIP。它不串流 ClawPack `.tgz` 檔案。
- 回應會包含 `ETag`、`Digest`、`X-ClawHub-Artifact-Type` 和 `X-ClawHub-Artifact-Sha256` 標頭，供解析器完整性檢查使用。
- 僅存在於登錄檔的中繼資料不會被注入下載的封存檔。
- 等待中的 VirusTotal 掃描不會封鎖下載；惡意版本會回傳 `403`。
- 私有套件會回傳 `404`，除非呼叫者是擁有者。

### `GET /api/npm/{package}`

回傳由 ClawPack 支援的套件版本所適用的 npm 相容 packument。

注意事項：

- 只會列出已上傳 ClawPack npm-pack tarball 的版本。
- 舊版僅 ZIP 的版本會刻意省略。
- `dist.tarball`、`dist.integrity` 和 `dist.shasum` 使用 npm 相容欄位，因此使用者可依需要將 npm 指向鏡像。
- 具命名空間的套件 packument 同時支援 `/api/npm/@scope/name` 和 npm 編碼的 `/api/npm/@scope%2Fname` 請求路徑。
...
- 軟刪除的版本會回傳 `410`。
- 由 GitHub 支援的技能交接不會代理或鏡像位元組。JSON 回應包含 `sourceRef: "public-github"`、`repo`、`commit`、`path`、`contentHash` 和 `archiveUrl`；掃描/目前狀態是門檻條件，不會作為成功酬載中繼資料包含在內。
- 下載統計會按 UTC 日計算唯一身分（API 權杖有效時為 `userId`，否則為 IP）。
...
- 選填酬載欄位：`ownerHandle`。存在時，API 會在伺服器端解析該發布者，並要求行為者具有發布者存取權。
- 選填酬載欄位：`migrateOwner`。當它與 `ownerHandle` 一起為 `true` 時，如果行為者同時是目前和目標發布者的管理員/擁有者，現有技能可以移動到該擁有者。沒有此選擇加入時，擁有者變更會遭拒。
...
- 允許的表單欄位為 `payload`、重複的 `files` blob，或一個 `clawpack` tarball 參照。`clawpack` 可以是 `.tgz` blob，或由 upload-url 流程回傳的儲存 ID。使用已暫存 storage-id 發布時，也必須包含該上傳 URL 一併回傳的 `clawpackUploadTicket`。
- 使用 `files` 或 `clawpack`，同一個請求中絕不能兩者並用。
- JSON 主體和呼叫者提供的 `payload.files` / `payload.artifact` 中繼資料會遭拒。
...
- 程式碼外掛需要 `package.json`、原始碼 repo 中繼資料、原始碼 commit 中繼資料、設定結構描述中繼資料、`openclaw.compat.pluginApi` 和 `openclaw.build.openclawVersion`。
- `openclaw.hostTargets` 和 `openclaw.environment` 是選填中繼資料。
- 只有 `openclaw` 組織發布者和目前 `openclaw` 組織成員的個人發布者可以發布到 `official` 頻道。
- 代表他人發布仍會依目標擁有者帳號驗證官方頻道資格。
...
存在時，`reason` 會儲存為技能審核備註，並複製到稽核記錄中。
由擁有者發起的軟刪除會保留該 slug 30 天，之後另一個發布者可宣告該 slug。
刪除回應會在此到期規則適用時包含 `slugReservedUntil`。
版主/管理員隱藏和安全移除不會以這種方式到期。
...
僅限管理員。確保某個代稱存在組織發布者。如果該代稱仍指向舊版共用使用者/個人發布者，端點會先將其遷移為組織發布者。
若為新建立的組織，請提供 `memberHandle`；執行操作的管理員不會被加入為成員。
...
已驗證的自助式組織發布者建立。建立新的組織發布者，並將呼叫者加入為擁有者。此端點不會遷移現有使用者/個人代稱，也不會將發布者標記為受信任/官方。

- 主體：`{ "handle": "opik", "displayName": "Opik" }`
- 回應：`{ "ok": true, "publisherId": "...", "handle": "opik", "created": true, "trusted": false }`
- 當代稱已被發布者、使用者或個人發布者使用時，回傳 `409`。

### `POST /api/v1/users/reserve`

僅限管理員。為正當擁有者保留根 slug 和套件名稱，而不發布版本。套件名稱會成為沒有版本列的私有預留位置套件，因此同一位擁有者稍後可以將真正的 code-plugin 或 bundle-plugin 版本發布到該名稱。
...
僅限管理員。為已驗證的替代 GitHub OAuth 主體復原個人發布者，而不編輯 Convex Auth 帳號列。請求必須指定兩個不可變的 GitHub 提供者帳號 ID；可變代稱僅作為面向操作員的防護檢查。

端點預設為 dry-run。套用復原需要在工作人員獨立驗證兩個
GitHub 主體之間的連續性後，設定 `dryRun: false` 與
`confirmIdentityVerified: true`。當目的地使用者目前的個人
發布者已有 Skills、套件或 GitHub Skills 來源時，復原會以封閉方式失敗。
復原也會遷移已復原發布者的 Skills、Skills slug 別名、套件、套件檢查器警告，以及衍生搜尋摘要列中的舊版 `ownerUserId` 欄位，讓直接擁有者路徑與新的發布者權限一致。已復原帳號代稱若有有效的受保護帳號代稱保留，也會重新指派給替代使用者，讓之後的
個人檔案同步無法還原前使用者的競爭權限。每個主要資料表在每次套用交易中限制為
100 列；較大的復原必須先使用可續傳的擁有者遷移。
GitHub Skills 來源是以發布者為範圍，並回報為已檢查，而非改寫。
...
- `merge` 會隱藏來源列表，並將來源 slug 重新導向到目標列表。
...
- `POST /api/v1/skills/{slug}/transfer/cancel`
  - Response（接受/拒絕/取消）：`{ "ok": true, "skillSlug": "demo-skill?" }`
