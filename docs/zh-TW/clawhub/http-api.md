...
網頁 slug 捷徑可跨登錄檔系列解析，但 API 用戶端應使用讀取端點傳回的標準 URL，而不是自行重建路由優先順序。
...
- 當伺服器知道原因時，已驗證寫入端點不應傳回單純的 `Unauthorized`。遺失權杖、無效/撤銷的權杖，以及已刪除/封禁/停用的帳戶，都應取得可採取行動的文字，讓命令列介面用戶端可以告知使用者被阻擋的原因。
...
- 如果沒有可用的受信任用戶端 IP，匿名請求會使用僅依速率限制種類限定範圍的備用儲存桶。這些備用儲存桶不包含呼叫者提供的路徑、slug、套件名稱、版本、查詢字串或其他成品參數。

## 錯誤回應

公開 v1 錯誤回應是 `content-type: text/plain; charset=utf-8` 的純文字。
這包括驗證失敗（`400`）、缺少公開資源（`404`）、驗證與權限失敗（`401`/`403`）、速率限制（`429`）以及遭封鎖的下載。用戶端應將回應本文讀取為人類可讀字串。未知查詢參數會為了相容性而被忽略，但已辨識查詢參數若具有無效值，會傳回 `400`。
...
- `q`（必填）：查詢字串
- `limit`（選用）：整數
- `highlightedOnly`（選用）：`true` 表示篩選至醒目標示的 Skills
...
- 結果會依相關性順序傳回（嵌入相似度 + 精確 slug/名稱詞元加權 + 小幅人氣先驗）。
- 相關性強於人氣。精確的 slug 或顯示名稱詞元符合，可能會排在互動量高很多但符合較寬鬆的結果之前。
- ASCII 文字會依字詞和標點邊界切分為詞元。例如，`personal-map` 包含獨立的 `map` 詞元，而 `amap-jsapi-skill` 包含 `amap`、`jsapi` 和 `skill`；因此搜尋 `map` 會讓 `personal-map` 比 `amap-jsapi-skill` 取得更強的詞彙符合。
- 人氣採對數縮放並設有上限。高互動量 Skills 在查詢文字符合較弱時，可能排名較低。
- 可疑或隱藏的審核狀態可能會依呼叫者篩選器和目前審核狀態，將某個 Skill 從公開搜尋中移除。

發布者可探索性指引：

- 將使用者會實際搜尋的詞放在顯示名稱、摘要和標籤中。只有在獨立 slug 詞元也是你想保留的穩定身分時才使用。
- 不要只為追逐某個查詢而重新命名 slug，除非新 slug 是更好的長期標準名稱。舊 slug 會成為重新導向別名，但標準 URL、顯示的 slug 和未來搜尋摘要會使用新 slug。
- 重新命名別名會保留舊 URL 和透過登錄檔解析的安裝解析能力，但搜尋排名會以重新命名完成索引後的標準 Skill 中繼資料為準。既有統計資料會保留在該 Skill 上。
...
- `sort`（選用）：`updated`（預設）、`recommended`（別名：`default`）、`createdAt`（別名：`newest`）、`downloads`、`stars`（別名：`rating`）、舊版安裝別名 `installsCurrent`/`installs`/`installsAllTime` 對應至 `downloads`、`trending`
- `nonSuspiciousOnly`（選用）：`true` 表示隱藏可疑（`flagged.suspicious`）Skills
- `nonSuspicious`（選用）：`nonSuspiciousOnly` 的舊版別名
...
- 當 `nonSuspiciousOnly=true` 時，基於游標的排序可能在某一頁傳回少於 `limit` 的項目，因為可疑 Skills 會在頁面擷取後被篩除。
- 當 `nextCursor` 存在時，請用它繼續分頁。短頁面本身不表示結果已結束。
...
- 證據會針對公開呼叫者遮蔽，且只有擁有者/審核員會包含原始片段。

### `POST /api/v1/skills/{slug}/report`

回報 Skill 以供審核員審查。回報是 Skill 層級，可選擇連結至版本，並會送入 Skill 回報佇列。
...
`note` 對 `confirmed` 和 `dismissed` 為必填；將 `status` 設回 `open` 時可以省略。對已分流的回報傳入 `finalAction: "hide"`，可在同一個可稽核工作流程中隱藏該 Skill。
...
- 只有在 `update: true` 且掃描成功完成時，已發布掃描才會回寫。
...
- 傳回已排隊/執行中/成功/失敗狀態。
- 排隊期間會傳回 `queue.queuedAhead` 和 `queue.position`，讓用戶端可顯示請求前方有多少優先處理的手動掃描。非常大的佇列會被設限，並以 `queuedAheadIsEstimate: true` 回報。
- 可用時，`report` 會包含 `clawscan`、`skillspector`、`staticAnalysis` 和 `virustotal` 區段。
- 失敗的掃描工作會傳回 `status: "failed"` 與 `lastError`。
...
傳回 `clawhub skill verify` 使用的 Skill Card 驗證信封。
...
- 只有在選取的版本有產生的 Skill Card、未因審核被惡意軟體封鎖，且 ClawScan 驗證為 clean 時，`ok` 才會是 `true`。
- Skill 身分、發布者身分與所選版本中繼資料都是最上層信封欄位（`slug`、`displayName`、`publisherHandle`、`version`、`resolvedFrom`、`tag`、`createdAt`），讓 shell 自動化可以讀取它們，而不必解開巢狀包裝。
- `security` 是最上層 ClawScan/security 判定。自動化應以 `ok`、`decision`、`reasons` 和 `security.status` 為依據。
...
- 結果依項目提供；單一缺少的 skill 或版本不會使整個回應失敗。
...
- 需要單一 skill Skill Card 驗證信封時，請使用 `/verify`；需要產生的卡片 Markdown 時，請使用 `/card`；需要詳細掃描器資料時，請使用 `/scan`。
...
- 每個技能包含 `_export_skill_meta.json`。
...
- 也接受 `GET /api/v1/plugins` 底下記錄的舊版 v1 篩選別名。
...
傳回供安裝用戶端使用的精確套件版本安全性與信任摘要。這是公開的 OpenClaw 消費介面，用來判斷
已解析的版本是否可安裝。
...
- `trust.stale` 表示信任摘要是根據過期輸入計算而來，且
  在做出高信心允許決策前，應視為需要重新整理。

備註：

- 此端點精確對應版本。用戶端應在解析它們打算安裝的
  套件版本後呼叫它，而不只是讀取最新
  套件中繼資料後呼叫。
- 除非呼叫端可以讀取擁有的發布者，否則私有套件會傳回 `404`。
- 此端點刻意比擁有者/版主審核
...
- `packageName` 會進行 npm-name 正規化；計畫中的
  遷移可以沒有套件。
- 這只會追蹤遷移就緒度。它不會變更 OpenClaw 或產生
  ClawPacks。
...
`note` 對 `confirmed` 和 `dismissed` 是必填；將 `status` 設回 `open`
時可省略。對已確認的報告傳入 `finalAction: "quarantine"` 或
`finalAction: "revoke"`，即可在同一個可稽核工作流程中套用發布審核。
...
- `dist.tarball`、`dist.integrity` 和 `dist.shasum` 使用 npm 相容
  欄位，因此使用者可以在選擇時將 npm 指向鏡像。
- 作用域套件 packument 同時支援 `/api/npm/@scope/name` 和 npm 的
...
下載託管技能版本 ZIP，或針對具備 `clean` 或 `suspicious` 掃描且沒有託管
版本的目前 GitHub 支援技能，回傳 GitHub 原始碼交接資訊。
...
- 也接受含有 `files`（storageId 型）的 JSON 主體。
- 選用酬載欄位：`ownerHandle`。存在時，API 會在伺服器端解析該
  發布者，並要求行動者具備發布者存取權。
- 選用酬載欄位：`migrateOwner`。當與 `ownerHandle` 一起設為 `true` 時，
  如果行動者同時是目前與目標發布者的管理員/擁有者，現有技能可移至該擁有者。
  若未選擇加入，擁有者變更會被拒絕。
...
- 允許的表單欄位為 `payload`、重複的 `files` blob，或一個 `clawpack`
  tarball 參照。`clawpack` 可以是 `.tgz` blob，或上傳 URL 流程回傳的儲存 ID。
  分段 storage-id 發布也必須包含該上傳 URL 一併回傳的
  `clawpackUploadTicket`。
- 使用 `files` 或 `clawpack` 其一，絕不要在同一請求中同時使用。
...
另一位發布者認領。刪除回應會在此到期情況適用時包含 `slugReservedUntil`。
...
僅限管理員。確保控制代碼有一個組織發布者。如果該控制代碼仍指向
...
呼叫者加入為擁有者。此端點不會遷移現有使用者/個人控制代碼，也不會
將發布者標記為受信任/官方。

- 主體：`{ "handle": "opik", "displayName": "Opik" }`
- 回應：`{ "ok": true, "publisherId": "...", "handle": "opik", "created": true, "trusted": false }`
- 當控制代碼已由發布者、使用者或個人發布者使用時，回傳 `409`。

### `POST /api/v1/users/reserve`

僅限管理員。為合法擁有者保留根 slug 和套件名稱，而不發布
發布。套件名稱會變成沒有發布列的私有預留位置套件，因此同一
擁有者稍後可以將真正的 code-plugin 或 bundle-plugin 發布發佈到該名稱。
...
且不編輯 Convex Auth 帳戶列。請求必須指定兩個不可變的 GitHub
提供者帳戶 ID；可變控制代碼只作為面向操作者的防護。

端點預設為 dry-run。套用復原需要在工作人員獨立驗證兩個
GitHub 主體之間的連續性後，設定 `dryRun: false` 和
`confirmIdentityVerified: true`。當目的地使用者目前的個人
發布者擁有 skills、packages 或 GitHub skill 來源時，復原會以失敗關閉。
復原也會遷移已復原發布者的 skills、skill slug 別名、packages、package inspector 警告，以及衍生的搜尋摘要列中的舊版 `ownerUserId` 欄位，讓直接擁有者路徑與新的發布者權限一致。已復原 handle 的有效受保護 handle
保留也會重新指派給替代使用者，讓後續的個人檔案同步無法還原前使用者的競爭權限。每個主要資料表在每次套用交易中限制為
100 列；較大的復原必須先使用可續傳的擁有者遷移。
GitHub skill 來源以發布者為範圍，並回報為已檢查，而不是被重寫。
...
- `/.well-known/clawhub.json`（JSON，首選）
