---
read_when:
    - 你想要讓 Prometheus、Grafana、VictoriaMetrics 或其他擷取器收集 OpenClaw 閘道指標
    - 你需要用於儀表板或警示的 Prometheus 指標名稱與標籤政策
    - 你想在不執行 OpenTelemetry 收集器的情況下取得指標資料
sidebarTitle: Prometheus
summary: 透過 diagnostics-prometheus 外掛將 OpenClaw 診斷資訊公開為 Prometheus 文字格式指標
title: Prometheus 指標
x-i18n:
    generated_at: "2026-07-19T13:45:03Z"
    model: gpt-5.6
...
<Warning>
此路由使用閘道驗證（操作員範圍、受信任的操作員介面）。請勿將其公開為不需驗證的 `/metrics` 端點。請透過其他操作員 API 所使用的相同驗證路徑擷取指標。
</Warning>

如需追蹤、記錄、OTLP 推送和 OpenTelemetry GenAI 語意屬性，請參閱 [OpenTelemetry 匯出](/zh-TW/gateway/opentelemetry)。
...
<Note>
`diagnostics.enabled` 預設為 `true`；僅限在嚴格受控的環境中將其設為 `false`。若其為 `false`，外掛仍會註冊 HTTP 路由，但不會有任何診斷事件流入匯出器，因此回應為空。
</Note>
...
對於模型呼叫指標，`observation_unit="request"` 衡量一次可觀測的
供應商請求。`observation_unit="turn"` 衡量一次合成的 Claude Code
...
<AccordionGroup>
  <Accordion title="有界且低基數的標籤">
    Prometheus 標籤維持有界且低基數。匯出器不會發出原始診斷識別碼，例如 `runId`、`sessionKey`、`sessionId`、`callId`、`toolCallId`、訊息 ID、聊天 ID 或供應商請求 ID。

    標籤值會經過遮蔽處理，且必須符合 OpenClaw 的低基數字元政策。不符合政策的值會依指標替換為 `unknown`、`other` 或 `none`。看似具範圍限定之代理程式工作階段金鑰的標籤，也會替換為 `unknown`。

  </Accordion>
  <Accordion title="時間序列上限與溢位計數">
    匯出器在記憶體中保留的時間序列上限為 **2048** 個，此上限涵蓋所有計數器、儀表和直方圖。超出此上限的新時間序列會遭捨棄，而每次捨棄時，`openclaw_prometheus_series_dropped_total` 都會加一。

    請監控此計數器，將其視為上游某個屬性正在洩漏高基數值的明確訊號。匯出器絕不會自動提高上限；如果計數持續上升，請修正來源，而不是停用上限。

  </Accordion>
  <Accordion title="絕不會出現在 Prometheus 輸出中的內容">
    - 提示文字、回應文字、工具輸入、工具輸出、系統提示
    - Talk 逐字稿、音訊承載資料、通話 ID、聊天室 ID、交接權杖、回合 ID，以及原始工作階段 ID
    - 原始供應商請求 ID（僅在適用時於追蹤範圍中使用有界雜湊，絕不會用於指標）
    - 工作階段金鑰和工作階段 ID
    - 主機名稱、檔案路徑、機密值
...
# 遭捨棄的 Prometheus 時間序列（基數警示）
increase(openclaw_prometheus_series_dropped_total[15m]) > 0
```

<Tip>
跨供應商儀表板應優先使用 `gen_ai_client_token_usage`：它遵循 OpenTelemetry GenAI 語意慣例，並與非 OpenClaw GenAI 服務的指標一致。
</Tip>

## 在 Prometheus 與 OpenTelemetry 匯出之間選擇

OpenClaw 分別支援這兩種介面。你可以執行任一種、同時執行兩種，或兩種都不執行。

<Tabs>
  <Tab title="diagnostics-prometheus">
    - **提取**模型：Prometheus 會抓取 `/api/diagnostics/prometheus`。
    - 不需要外部收集器。
    - 透過一般的閘道驗證進行身分驗證。
    - 此介面僅提供指標（不含追蹤或記錄）。
    - 最適合已將 Prometheus + Grafana 標準化的技術堆疊。
  ...
  <Tab title="diagnostics-otel">
    ...
    - 如需完整目錄，請參閱 [OpenTelemetry 匯出](/zh-TW/gateway/opentelemetry)。
...
<AccordionGroup>
  <Accordion title="回應內文為空">
    - 檢查設定中的 `diagnostics.enabled` 是否未設為 `false`（預設值為 `true`）。
    - 使用 `openclaw plugins list --enabled` 確認外掛已啟用並載入。
    - 產生一些流量；計數器和直方圖只有在至少發生一個事件後才會輸出資料列。

  </Accordion>
  <Accordion title="401 / 未授權">
    此端點需要閘道操作員範圍（`auth: "gateway"` 搭配 `gatewayRuntimeScopeSurface: "trusted-operator"`）。請使用 Prometheus 存取任何其他閘道操作員路由時所使用的相同權杖或密碼。此處不提供公開的未經身分驗證模式。
  </Accordion>
  <Accordion title="`openclaw_prometheus_series_dropped_total` 持續上升">
    有新的屬性超過 **2048** 個時間序列的上限。檢查近期指標中是否有基數異常偏高的標籤，並從來源修正。匯出器會刻意捨棄新的時間序列，而不會在未提示的情況下重寫標籤。
  </Accordion>
  <Accordion title="重新啟動後，Prometheus 顯示過時的時間序列">
    外掛僅將狀態保留在記憶體中。閘道重新啟動後，計數器會重設為零，而儀表會從下次回報的值重新開始。使用 PromQL 的 `rate()` 和 `increase()`，即可妥善處理重設。
  </Accordion>
</AccordionGroup>
...
- [記錄](/zh-TW/logging) — 檔案式記錄
- [OpenTelemetry 匯出](/zh-TW/gateway/opentelemetry) — 透過 OTLP 推送追蹤、指標和記錄
