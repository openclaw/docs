...
read_when:
    ...
    - 你正在 Gateway RPC 與外掛 SDK 之間做選擇
    - 您正在整合閘道代理執行、工作階段、事件、核准、模型或工具
...
外部應用程式透過閘道協定與 OpenClaw 通訊：WebSocket
傳輸加上 RPC 方法。當指令碼、儀表板、CI 作業、IDE
擴充功能，或其他程序想要啟動代理執行、串流事件、等待
結果、取消工作，或檢查閘道資源時，請使用它。

<Warning>
  目前尚無公開的 npm 用戶端套件。在發行說明宣布已發布
  套件且本頁包含安裝說明之前，請勿將 OpenClaw 用戶端套件
  名稱加入應用程式相依性。
</Warning>

<Note>
  本頁適用於 OpenClaw 程序外部的程式碼。在 OpenClaw
  內部執行的外掛程式碼，應改用已記錄的 `openclaw/plugin-sdk/*` 子路徑。
</Note>
...
| 介面                                    | 狀態 | 用途                                                                                          |
| --------------------------------------- | ------ | --------------------------------------------------------------------------------------------- |
| [閘道協定](/zh-TW/gateway/protocol)           | 就緒  | WebSocket 傳輸、連線交握、驗證範圍、協定版本控管，以及事件。                                  |
| [閘道 RPC 參考](/zh-TW/reference/rpc)         | 就緒  | 目前用於代理、工作階段、任務、模型、工具、成品和核准的閘道方法。                              |
| [`openclaw agent`](/zh-TW/cli/agent)          | 就緒  | 當呼叫命令列介面已足夠時，用於一次性指令碼整合。                                              |
| [`openclaw message`](/zh-TW/cli/message)      | 就緒  | 從指令碼傳送訊息或通道動作。                                                                  |

未來的用戶端函式庫套件正在內部開發中，但目前尚不是
公開安裝介面。在發行版本宣布已發布且有版本的套件之前，
請將其視為預覽實作細節。
...
3. 呼叫[閘道 RPC 參考](/zh-TW/reference/rpc)中記錄的 RPC 方法。
4. 釘選你測試所依據的 OpenClaw 版本。
5. 升級 OpenClaw 時重新檢查 RPC 參考。

對於代理執行，請從 `agent` RPC 開始，並搭配 `agent.wait` 取得
終端結果。對於持久的對話狀態，請使用 `sessions.*` 方法。
對於 UI 整合，請訂閱閘道事件，並只呈現你的應用程式理解的事件
家族。
