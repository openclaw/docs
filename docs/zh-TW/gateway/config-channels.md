...
read_when:
    ...
    - 各通道設定鍵疑難排解
    - 稽核私訊政策、群組政策或提及控管
summary: 通道設定：Slack、Discord、Telegram、WhatsApp、Matrix、iMessage 等通道的存取控制、配對與個別通道金鑰
title: 設定 — 通道
x-i18n:
...
每個頻道在 `channels.*` 底下的設定鍵。涵蓋 DM 與群組存取、多帳號設定、提及閘控，以及 Slack、Discord、Telegram、WhatsApp、Matrix、iMessage 和其他捆綁頻道 Plugin 的每頻道鍵。

若要了解代理、工具、Gateway 執行階段與其他頂層鍵，請參閱
...
如果提供者區塊完全缺失（沒有 `channels.<provider>`），執行階段群組政策會回退到 `allowlist`（失敗時關閉），並顯示啟動警告。
</Note>

### 頻道模型覆寫

使用 `channels.modelByChannel` 將特定頻道 ID 固定到模型。值可接受 `provider/model` 或已設定的模型別名。當工作階段尚未有模型覆寫時（例如透過 `/model` 設定），會套用頻道對應。
...
- `channels.defaults.contextVisibility`：所有頻道的預設補充情境可見性模式。值：`all`（預設，包含所有引用/討論串/歷史情境）、`allowlist`（僅包含來自允許清單寄件者的情境）、`allowlist_quote`（與 allowlist 相同，但保留明確引用/回覆情境）。每頻道覆寫：`channels.<channel>.contextVisibility`。
...
WhatsApp 透過 Gateway 的 Web 頻道（Baileys Web）執行。當已連結的工作階段存在時，會自動啟動。
...
- 如果存在帳號 `default`，傳出命令預設使用該帳號；否則使用第一個已設定的帳號 ID（已排序）。
...
- Bot token：`channels.telegram.botToken` 或 `channels.telegram.tokenFile`（僅限一般檔案；拒絕符號連結），預設帳號會以 `TELEGRAM_BOT_TOKEN` 作為後備。
- `apiRoot` 只限 Telegram Bot API 根目錄。請使用 `https://api.telegram.org` 或你的自架/代理根目錄，而不是 `https://api.telegram.org/bot<TOKEN>`；`openclaw doctor --fix` 會移除意外尾隨的 `/bot<TOKEN>` 後綴。
- 選用的 `channels.telegram.defaultAccount` 會在符合已設定帳號 ID 時覆寫預設帳號選擇。
- 在多帳號設定（2 個以上帳號 ID）中，請設定明確的預設值（`channels.telegram.defaultAccount` 或 `channels.telegram.accounts.default`）以避免後備路由；缺失或無效時，`openclaw doctor` 會警告。
- `configWrites: false` 會封鎖由 Telegram 發起的設定寫入（超級群組 ID 遷移、`/config set|unset`）。
- 具備 `type: "acp"` 的頂層 `bindings[]` 項目會為論壇主題設定持久 ACP 綁定（在 `match.peer.id` 中使用標準 `chatId:topic:topicId`）。欄位語義在 [ACP 代理](/zh-TW/tools/acp-agents#channel-specific-settings) 中共用。
...
- 提供明確 Discord `token` 的直接對外呼叫會使用該權杖執行呼叫；帳號重試/政策設定仍來自作用中執行階段快照中的所選帳號。
- 選用的 `channels.discord.defaultAccount` 會在符合已設定帳號 ID 時覆寫預設帳號選取。
- 使用 `user:<id>`（DM）或 `channel:<id>`（伺服器頻道）作為傳送目標；裸數字 ID 會遭拒。
- 伺服器 slug 皆為小寫，空格會替換為 `-`；頻道鍵使用經 slug 化的名稱（不含 `#`）。建議使用伺服器 ID。
- 預設會忽略機器人撰寫的訊息。`allowBots: true` 會啟用這些訊息；使用 `allowBots: "mentions"` 僅接受提及該機器人的機器人訊息（仍會過濾自己的訊息）。
- `channels.discord.guilds.<id>.ignoreOtherMentions`（以及頻道覆寫）會丟棄提及其他使用者或角色但未提及該機器人的訊息（不含 @everyone/@here）。
- `channels.discord.mentionAliases` 會在傳送前將穩定的對外 `@handle` 文字對應到 Discord 使用者 ID，因此即使暫時性目錄快取為空，也能以可預期方式提及已知隊友。各帳號覆寫位於 `channels.discord.accounts.<accountId>.mentionAliases`。
- `maxLinesPerMessage`（預設 17）即使在少於 2000 個字元時，也會分割行數過高的訊息。
- `channels.discord.threadBindings` 控制 Discord 執行緒繫結路由：
  ...
  - `idleHours`：Discord 對非活動自動取消聚焦時數的覆寫（`0` 會停用）
  - `maxAgeHours`：Discord 對硬性最長時數的覆寫（`0` 會停用）
  - `spawnSessions`：`sessions_spawn({ thread: true })` 與 ACP 執行緒生成自動建立/繫結執行緒的開關（預設：`true`）
  - `defaultSpawnContext`：執行緒繫結生成的原生子代理內容（預設為 `"fork"`）
- 帶有 `type: "acp"` 的最上層 `bindings[]` 項目會為頻道和執行緒設定持久 ACP 繫結（在 `match.peer.id` 中使用頻道/執行緒 ID）。欄位語意在 [ACP 代理](/zh-TW/tools/acp-agents#channel-specific-settings) 中共用。
- `channels.discord.ui.components.accentColor` 會設定 Discord 元件 v2 容器的強調色。
...
- `channels.discord.voice.reconnectGraceMs` 控制已中斷連線的語音工作階段可花多久進入重新連線訊號狀態，之後 OpenClaw 會銷毀它（預設為 `15000`）。
...
- `channels.discord.execApprovals`：Discord 原生 exec 核准傳送與核准者授權。
  ...
  - `approvers`：允許核准 exec 要求的 Discord 使用者 ID。省略時會回退到 `commands.ownerAllowFrom`。
  - `agentFilter`：選用的代理 ID 允許清單。省略則轉送所有代理的核准。
  - `sessionFilter`：選用的工作階段鍵模式（子字串或 regex）。
  - `target`：傳送核准提示的位置。`"dm"`（預設）會傳送至核准者 DM，`"channel"` 會傳送至來源頻道，`"both"` 會兩者都傳送。當目標包含 `"channel"` 時，按鈕只有已解析的核准者可使用。
...
- **HTTP 模式** 需要 `botToken` 加上 `signingSecret`（位於根層級或各帳號）。
- `socketMode` 會將 Slack SDK Socket Mode 傳輸微調傳遞至公開 Bolt 接收器 API。僅在調查 ping/pong 逾時或陳舊 websocket 行為時使用。
...
- Slack 帳號快照會公開每個憑證的來源/狀態欄位，例如
  ...
  `signingSecretStatus`。`configured_unavailable` 表示帳號是透過 SecretRef
  設定，但目前的命令/執行階段路徑無法解析祕密值。
- `configWrites: false` 會封鎖由 Slack 啟動的設定寫入。
...
**執行緒工作階段隔離：** `thread.historyScope` 是各執行緒（預設）或跨頻道共用。`thread.inheritParent` 會將父頻道逐字稿複製到新執行緒。

- Slack 原生串流加上 Slack 助理風格的「is typing...」執行緒狀態需要回覆執行緒目標。最上層 DM 預設保持不在執行緒中，因此會使用 `typingReaction` 或一般傳送，而非執行緒風格預覽。
- `typingReaction` 會在回覆執行時，向傳入的 Slack 訊息新增暫時反應，並在完成時移除。使用 Slack emoji shortcode，例如 `"hourglass_flowing_sand"`。
- `channels.slack.execApprovals`：Slack 原生 exec 核准傳送與核准者授權。與 Discord 相同的 schema：`enabled`（`true`/`false`/`"auto"`）、`approvers`（Slack 使用者 ID）、`agentFilter`、`sessionFilter` 和 `target`（`"dm"`、`"channel"` 或 `"both"`）。
...
Mattermost 在目前的 OpenClaw 發行版中隨附為 bundled Plugin。較舊或
自訂建置可以透過
`openclaw plugins install @openclaw/mattermost` 安裝目前的 npm 套件；如果 npm 回報
OpenClaw 擁有的套件已棄用，請使用 bundled Plugin 或本機 checkout，
直到較新的 npm 套件發布為止。
...
- `commands.callbackUrl` 必須解析到 OpenClaw Gateway 端點，且可從 Mattermost 伺服器連線。
- 原生斜線回呼會使用 Mattermost 在斜線命令註冊期間傳回的每個命令權杖進行驗證。如果註冊失敗或沒有啟用任何命令，OpenClaw 會以 `Unauthorized: invalid command token.` 拒絕回呼。
...
- `channels.mattermost.requireMention`：要求在頻道中回覆前必須先有 `@mention`。
- `channels.mattermost.groups.<channelId>.requireMention`：每個頻道的提及閘控覆寫（`"*"` 代表預設值）。
...
BlueBubbles 是建議使用的 iMessage 路徑（由 Plugin 支援，設定於 `channels.bluebubbles` 下）。
...
- 具有 `type: "acp"` 的頂層 `bindings[]` 項目可將 BlueBubbles 對話繫結到持久 ACP 工作階段。請在 `match.peer.id` 中使用 BlueBubbles 控制代碼或目標字串（`chat_id:*`、`chat_guid:*`、`chat_identifier:*`）。共用欄位語意：[ACP Agents](/zh-TW/tools/acp-agents#channel-specific-settings)。
...
- 具有 `type: "acp"` 的頂層 `bindings[]` 項目可將 iMessage 對話繫結到持久 ACP 工作階段。請在 `match.peer.id` 中使用正規化控制代碼或明確聊天目標（`chat_id:*`、`chat_guid:*`、`chat_identifier:*`）。共用欄位語意：[ACP Agents](/zh-TW/tools/acp-agents#channel-specific-settings)。
...
- `channels.matrix.proxy` 會透過明確的 HTTP(S) proxy 路由 Matrix HTTP 流量。具名帳號可用 `channels.matrix.accounts.<id>.proxy` 覆寫它。
- `channels.matrix.network.dangerouslyAllowPrivateNetwork` 允許私人／內部 homeserver。`proxy` 與這個網路選擇加入是獨立控制項。
...
- `channels.matrix.execApprovals`：Matrix 原生 exec 核准傳送與核准者授權。
  ...
  - `agentFilter`：可選的 Agent ID 允許清單。省略則轉送所有 Agent 的核准。
...
- 完整 IRC 頻道設定（主機／連接埠／TLS／頻道／允許清單／提及閘控）記錄於 [IRC](/zh-TW/channels/irc)。
...
- 如果你透過 `openclaw channels add`（或頻道 onboarding）新增非預設帳號，而仍使用單帳號頂層頻道設定，OpenClaw 會先將帳號範圍的頂層單帳號值提升到頻道帳號對映中，讓原始帳號繼續運作。大多數頻道會將它們移到 `channels.<channel>.accounts.default`；Matrix 則可改為保留既有相符的具名／預設目標。
- 既有僅限頻道的繫結（沒有 `accountId`）會繼續符合預設帳號；帳號範圍繫結仍為可選。
- `openclaw doctor --fix` 也會透過將帳號範圍的頂層單帳號值移到該頻道所選的已提升帳號，來修復混合形狀。大多數頻道使用 `accounts.default`；Matrix 則可改為保留既有相符的具名／預設目標。

### 其他 Plugin 頻道

許多 Plugin 頻道設定為 `channels.<id>`，並記錄於其專屬頻道頁面（例如 Feishu、Matrix、LINE、Nostr、Zalo、Nextcloud Talk、Synology Chat 和 Twitch）。
...
可見回覆另行控制。群組／頻道房間預設為 `messages.groupChat.visibleReplies: "message_tool"`：OpenClaw 仍會處理該輪對話，但一般最終回覆會保持私密，而可見房間輸出需要 `message(action=send)`。只有在你想要一般回覆也張貼回房間的舊版行為時，才設定 `"automatic"`。若要將相同的僅工具可見回覆行為也套用到直接聊天，請設定 `messages.visibleReplies: "message_tool"`；Codex harness 也會將該僅工具行為作為未設定直接聊天的預設值。

如果在作用中的工具政策下無法使用 message tool，OpenClaw 會退回到自動可見回覆，而不是默默抑制回應。`openclaw doctor` 會警告此不相符情況。

Gateway 會在檔案儲存後熱重新載入 `messages` 設定。只有在部署中停用檔案監看或設定重新載入時才需要重新啟動。

**提及類型：**

- **中繼資料提及**：原生平台 @-mentions。在 WhatsApp 自我聊天模式中會忽略。
- **文字模式**：`agents.list[].groupChat.mentionPatterns` 中的安全 regex 模式。無效模式與不安全的巢狀重複會被忽略。
- 只有在可以偵測時（原生提及或至少一個模式），才會強制執行提及閘控。
...
`messages.groupChat.historyLimit` 會設定全域預設值。Channels 可用 `channels.<channel>.historyLimit` 覆寫（或依帳戶覆寫）。設定為 `0` 可停用。
...
解析順序：依 DM 覆寫 → provider 預設值 → 無限制（全部保留）。
...
- 此區塊會設定命令介面。如需目前內建 + bundled 命令目錄，請參閱 [Slash Commands](/zh-TW/tools/slash-commands)。
- 本頁是**設定鍵參考**，不是完整命令目錄。頻道/Plugin 擁有的命令，例如 QQ Bot `/bot-ping` `/bot-help` `/bot-logs`、LINE `/card`、裝置配對 `/pair`、記憶體 `/dreaming`、電話控制 `/phone`，以及 Talk `/voice`，會記錄在其頻道/Plugin 頁面與 [Slash Commands](/zh-TW/tools/slash-commands)。
- 文字命令必須是帶有前導 `/` 的**獨立**訊息。
...
- `config: true` 會啟用 `/config`（讀取/寫入 `openclaw.json`）。對 Gateway `chat.send` 用戶端而言，持久性 `/config set|unset` 寫入也需要 `operator.admin`；唯讀 `/config show` 仍可供一般具寫入範圍的 operator 用戶端使用。
...
- 對多帳戶頻道，`channels.<provider>.accounts.<id>.configWrites` 也會控管以該帳戶為目標的寫入（例如 `/allowlist --config --account <id>` 或 `/config set channels.<provider>.accounts.<id>...`）。
...
- [Configuration reference](/zh-TW/gateway/configuration-reference) — 頂層鍵
- [Configuration — agents](/zh-TW/gateway/config-agents)
- [Channels overview](/zh-TW/channels)
