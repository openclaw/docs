...
read_when:
    ...
    - 你正在將追蹤資料、指標或日誌接入 Grafana、Datadog、Honeycomb、New Relic、Tempo 或其他 OTLP 後端
    - 你需要精確的指標名稱、span 名稱或屬性結構來建立儀表板或警示
...
OpenClaw 透過官方 `diagnostics-otel` Plugin，使用 **OTLP/HTTP (protobuf)** 匯出診斷資料。任何接受 OTLP/HTTP 的收集器或後端都可在不修改程式碼的情況下運作。如需本機檔案日誌以及如何讀取它們，請參閱
[Logging](/zh-TW/logging)。

## 運作方式

- **診斷事件** 是由 Gateway 和隨附 Plugin 針對模型執行、訊息流程、工作階段、佇列和 exec 發出的結構化處理程序內記錄。
- **`diagnostics-otel` Plugin** 訂閱這些事件，並透過 OTLP/HTTP 將它們匯出為 OpenTelemetry **指標**、**追蹤**和**日誌**。
- **提供者呼叫** 會在提供者傳輸接受自訂標頭時，從 OpenClaw 受信任的模型呼叫跨度內容接收 W3C `traceparent` 標頭。不會傳播由 Plugin 發出的追蹤內容。
- 只有在診斷介面和 Plugin 都啟用時，匯出器才會附加，因此預設的處理程序內成本會維持接近於零。
...
| **追蹤**    | 模型用量、模型呼叫、測試框架生命週期、工具執行、exec、webhook/訊息處理、內容組裝和工具迴圈的跨度。                                       |
| **日誌**    | 當 `diagnostics.otel.logs` 啟用時，透過 OTLP 匯出的結構化 `logging.file` 記錄。                                                            |

可以獨立切換 `traces`、`metrics` 和 `logs`。當 `diagnostics.otel.enabled` 為 true 時，三者預設皆為開啟。
...
| `OTEL_SEMCONV_STABILITY_OPT_IN`                                                                                   | 設為 `gen_ai_latest_experimental`，以發出最新的實驗性 GenAI 跨度屬性 (`gen_ai.provider.name`)，而不是舊版 `gen_ai.system`。無論如何，GenAI 指標一律使用有界、低基數的語意屬性。 |
| `OPENCLAW_OTEL_PRELOADED`                                                                                         | 當另一個預載或主機程序已註冊全域 OpenTelemetry SDK 時設為 `1`。Plugin 接著會略過自己的 NodeSDK 生命週期，但仍會接上診斷監聽器，並遵循 `traces`/`metrics`/`logs`。         |

## 隱私權與內容擷取

原始模型/工具內容預設**不會**匯出。跨度會攜帶有界識別碼（通道、提供者、模型、錯誤類別、僅雜湊的請求 ID），且絕不包含提示文字、回應文字、工具輸入、工具輸出或工作階段金鑰。

傳出的模型請求可能包含 W3C `traceparent` 標頭。該標頭只會由作用中模型呼叫的 OpenClaw 所有診斷追蹤內容產生。現有呼叫端提供的 `traceparent` 標頭會被取代，因此 Plugin 或自訂提供者選項無法偽造跨服務追蹤祖先關係。

只有在你的收集器和保留政策已核准可處理提示、回應、工具或系統提示文字時，才將 `diagnostics.otel.captureContent.*` 設為 `true`。每個子鍵都可獨立選擇加入：
...
當任何子鍵啟用時，模型和工具跨度只會針對該類別取得有界且已遮蔽的 `openclaw.content.*` 屬性。

## 取樣與清送

- **追蹤：** `diagnostics.otel.sampleRate`（僅根跨度，`0.0` 會捨棄全部，`1.0` 會保留全部）。
- **指標：** `diagnostics.otel.flushIntervalMs`（最小值 `1000`）。
- **日誌：** OTLP 日誌遵循 `logging.level`（檔案日誌層級）。它們使用診斷日誌記錄遮蔽路徑，而非主控台格式化。高流量安裝應優先使用 OTLP 收集器取樣/篩選，而不是本機取樣。
- **檔案日誌關聯：** 當日誌呼叫攜帶有效的診斷追蹤內容時，JSONL 檔案日誌會包含頂層 `traceId`、`spanId`、`parentSpanId` 和 `traceFlags`，讓日誌處理器可將本機日誌行與匯出的跨度合併。
- **請求關聯：** Gateway HTTP 請求和 WebSocket 框架會建立內部請求追蹤範圍。該範圍內的日誌和診斷事件預設會繼承請求追蹤，而代理執行和模型呼叫跨度會建立為子項，讓提供者 `traceparent` 標頭維持在同一個追蹤上。
...
`diagnostics.stuckSessionWarnMs` 是工作階段存活性診斷的無進度年齡閾值。當 OpenClaw 觀察到回覆、工具、狀態、區塊或 ACP 執行階段進度時，`processing` 工作階段不會朝此閾值累積年齡。輸入狀態 keepalive 不會被計為進度，因此仍可偵測到沉默的模型或測試框架。

OpenClaw 依其仍可觀察到的工作對工作階段進行分類：

- `session.long_running`：作用中的嵌入式工作、模型呼叫或工具呼叫仍在進行。
- `session.stalled`：存在作用中的工作，但作用中的執行近期未回報進度。停滯的嵌入式執行一開始會維持僅觀察，接著在 `diagnostics.stuckSessionAbortMs` 無進度後中止並排空，讓該通道後方排隊的回合可以恢復。未設定時，中止門檻預設為較安全的延長視窗，也就是至少 10 分鐘且為 `diagnostics.stuckSessionWarnMs` 的 5 倍。
- `session.stuck`：沒有作用中工作的過期 session 簿記。這會立即釋放受影響的 session 通道。

復原會發出結構化的 `session.recovery.requested` 和 `session.recovery.completed` 事件。診斷 session 狀態只會在會變更狀態的復原結果（`aborted` 或 `released`）之後標記為閒置，而且只有在同一個處理世代仍為目前世代時才會標記。

只有 `session.stuck` 會發出 `openclaw.session.stuck` 計數器、`openclaw.session.stuck_age_ms` 直方圖，以及 `openclaw.session.stuck` span。重複的 `session.stuck` 診斷會在 session 維持不變時退避，因此儀表板應該對持續增加發出警示，而不是對每一次 Heartbeat tick 發出警示。如需設定旋鈕與預設值，請參閱[設定參考](/zh-TW/gateway/configuration-reference#diagnostics)。
...
- `openclaw.model.call`
  ...
  - `openclaw.provider.request_id_hash`（以上游供應商請求 id 為基礎的有界 SHA 雜湊；不會匯出原始 id）
...
明確啟用內容擷取時，模型與工具 spans 也可以針對你選用的特定內容類別，包含有界且已遮蔽的 `openclaw.content.*` 屬性。

## 診斷事件目錄

下列事件支援上方的指標與 spans。Plugin 也可以不經由 OTLP 匯出，直接訂閱這些事件。

**模型用量**

- `model.usage` — tokens、成本、持續時間、情境、供應商/模型/頻道、session ids。`usage` 是供應商/回合層級的成本與遙測計算；`context.used` 是目前提示/情境快照，當涉及快取輸入或工具迴圈呼叫時，可能低於供應商的 `usage.total`。
...
- `harness.run.started` / `harness.run.completed` / `harness.run.error` — 代理 harness 的每次執行生命週期。包含 `harnessId`、選用的 `pluginId`、供應商/模型/頻道，以及執行 id。完成時會新增 `durationMs`、`outcome`、選用的 `resultClassification`、`yieldDetected`，以及 `itemLifecycle` 計數。錯誤時會新增 `phase`（`prepare`/`start`/`send`/`resolve`/`cleanup`）、`errorCategory`，以及選用的 `cleanupFailed`。

**Exec**

- `exec.process.completed` — 終端結果、持續時間、目標、模式、結束代碼，以及失敗種類。不包含命令文字與工作目錄。

## 沒有匯出器時

你可以在不執行 `diagnostics-otel` 的情況下，讓診斷事件可供 Plugin 或自訂 sink 使用：
...
若要在不提高 `logging.level` 的情況下輸出目標式除錯資訊，請使用診斷旗標。旗標不區分大小寫，且支援萬用字元（例如 `telegram.*` 或 `*`）：
...
- [記錄](/zh-TW/logging) — 檔案記錄、主控台輸出、CLI tailing，以及 Control UI 的記錄分頁
- [Gateway 記錄內部](/zh-TW/gateway/logging) — WS 記錄樣式、子系統前綴，以及主控台擷取
