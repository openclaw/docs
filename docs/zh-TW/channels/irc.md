...
IRC 以 bundled Plugin 形式提供，但會在主要設定的 `channels.irc` 底下設定。
...
建議使用私有 IRC 伺服器進行機器人協調。如果你有意使用公開 IRC 網路，常見選項包括 Libera.Chat、OFTC 和 Snoonet。請避免將可預測的公開頻道用於機器人或群集後向通道流量。
...
- IRC 使用 OpenClaw 操作者管理的轉送 Proxy 路由之外的原始 TCP/TLS socket。在要求所有輸出流量都必須通過該轉送 Proxy 的部署中，除非已明確核准直接 IRC 輸出，否則請設定 `channels.irc.enabled=false`。
...
- 每頻道控制項（頻道 + 傳送者 + 提及規則）：`channels.irc.groups["#channel"]`
- `channels.irc.groupPolicy="open"` 允許未設定的頻道（**預設仍受提及閘控限制**）
...
即使某個頻道已被允許（透過 `groupPolicy` + `groups`），且傳送者也被允許，OpenClaw 在群組情境中仍預設使用**提及閘控**。

這表示除非訊息包含符合機器人的提及模式，否則你可能會看到像 `drop channel … (missing-mention)` 這樣的記錄。

若要讓機器人在 IRC 頻道中回覆且**不需要提及**，請停用該頻道的提及閘控：
...
或者，若要允許**所有** IRC 頻道（不使用每頻道允許清單），且仍在沒有提及時回覆：
...
- `toolsBySender` 鍵應使用 `id:` 表示 IRC 傳送者身分值：
  `id:eigen`，或使用 `id:eigen!~eigen@174.127.248.171` 以取得更強的比對。
- 舊版未加前綴的鍵仍會被接受，並只會以 `id:` 進行比對。
...
暱稱註冊完成後，請停用 `register`，避免重複嘗試 REGISTER。
...
- 如果機器人已連線但從不在頻道中回覆，請確認 `channels.irc.groups`，**以及**提及閘控是否正在丟棄訊息（`missing-mention`）。如果你希望它不需要 ping 就回覆，請為該頻道設定 `requireMention:false`。
