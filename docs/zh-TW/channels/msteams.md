...
Microsoft Teams 在目前的 OpenClaw 發行版本中作為隨附 Plugin 提供，因此一般封裝建置不需要
另外安裝。
...
<Note>
Teams CLI 目前處於 preview。命令和旗標可能會在不同發行版本之間變更。
</Note>
...
<Note>
需要 `--allow-anonymous`，因為 Teams 無法使用 devtunnels 驗證。每個傳入的 bot request 仍會由 Teams SDK 自動驗證。
</Note>

替代方案：`ngrok http 3978` 或 `tailscale funnel 3978`（但這些方案可能每個 session 都會變更 URL）。
...
- 建立 Entra ID (Azure AD) application
- 產生 client secret
- 建置並上傳 Teams app manifest（含 icons）
- 註冊 bot（預設由 Teams 管理，不需要 Azure subscription）

輸出會顯示 `CLIENT_ID`、`CLIENT_SECRET`、`TENANT_ID` 和 **Teams App ID**，請記下這些資訊供後續步驟使用。它也會提供直接在 Teams 中安裝 app 的選項。
...
`teams app create` 會提示你安裝 app，請選擇「Install in Teams」。如果你略過了，可以稍後取得連結：
...
這會針對 bot 註冊、AAD app config、manifest 有效性和 SSO 設定執行診斷。

對於 production 部署，請考慮使用[聯合驗證](/zh-TW/channels/msteams#federated-authentication-certificate-plus-managed-identity)（certificate 或 managed identity）取代 client secrets。

<Note>
群組聊天預設會被封鎖（`channels.msteams.groupPolicy: "allowlist"`）。若要允許群組回覆，請設定 `channels.msteams.groupAllowFrom`，或使用 `groupPolicy: "open"` 允許任何成員（mention-gated）。
</Note>

## 目標

- 透過 Teams DM、群組聊天或頻道與 OpenClaw 對話。
- 保持路由具決定性：回覆一律回到訊息來源的頻道。
...
- 預設：`channels.msteams.dmPolicy = "pairing"`。未知傳送者會被忽略，直到獲得核准。
- `channels.msteams.allowFrom` 應使用穩定的 AAD object ID，或靜態傳送者存取群組，例如 `accessGroup:core-team`。
- 不要依賴 UPN/顯示名稱比對作為 allowlists，因為它們可能會變更。OpenClaw 預設停用直接名稱比對；請使用 `channels.msteams.dangerouslyAllowNameMatching: true` 明確選擇啟用。
...
- `channels.msteams.groupAllowFrom` 控制哪些傳送者或靜態傳送者存取群組可以在群組聊天/頻道中觸發（fallback 至 `channels.msteams.allowFrom`）。
- 設定 `groupPolicy: "open"` 允許任何成員（預設仍然 mention-gated）。
...
- 在 `channels.msteams.teams` 下列出 teams 和 channels，以限定群組/頻道回覆範圍。
- Keys 應使用來自 Teams links 的穩定 Teams conversation ID，而不是可變的顯示名稱。
- 當 `groupPolicy="allowlist"` 且存在 teams allowlist 時，只接受列出的 teams/channels（mention-gated）。
- configure wizard 接受 `Team/Channel` entries，並為你儲存。
- 啟動時，OpenClaw 會將 team/channel 和 user allowlist 名稱解析為 ID（當 Graph 權限允許時）
  並記錄 mapping；未解析的 team/channel 名稱會按輸入保留，但預設會被路由忽略，除非啟用 `channels.msteams.dangerouslyAllowNameMatching: true`。
...
1. 確認 Microsoft Teams Plugin 可用（目前發行版本已隨附）。
...
6. Gateway 預設在 `/api/messages` 監聽 Bot Framework Webhook traffic。
...
2. 填寫 **Basics** 分頁：
   ...
   | **Subscription**   | 選擇你的 Azure subscription                           |
   | **Resource group** | 建立新的或使用既有項目                               |
   | **Pricing tier**   | 開發/測試使用 **Free**                                 |
   | **Type of App**    | **Single Tenant**（建議，請參閱下方 note）         |
   | **Creation type**  | **Create new Microsoft App ID**                          |

<Warning>
建立新的 multi-tenant bots 已於 2025-07-31 後棄用。新的 bots 請使用 **Single Tenant**。
</Warning>
...
1. 前往你的 Azure Bot resource → **Configuration**
...
- 包含一個 `bot` entry，且 `botId = <App ID>`。
- Scopes：`personal`、`team`、`groupChat`。
- `supportsFiles: true`（personal scope 檔案處理所需）。
- 新增 RSC 權限（請參閱 [RSC 權限](#current-teams-rsc-permissions-manifest)）。
- 建立 icons：`outline.png` (32x32) 和 `color.png` (192x192)。
- 將三個檔案一起 zip：`manifest.json`、`outline.png`、`color.png`。
...
當 Plugin 可用且 `msteams` config 存在並含有憑證時，Teams channel 會自動啟動。

</details>

## 聯合驗證（certificate plus managed identity）

> 新增於 2026.4.11

對於 production 部署，OpenClaw 支援 **federated authentication**，作為比 client secrets 更安全的替代方案。有兩種方法可用：

### 選項 A：Certificate-based authentication

使用已註冊到你的 Entra ID app registration 的 PEM certificate。

**設定：**

1. 產生或取得 certificate（含 private key 的 PEM format）。
2. 在 Entra ID → App Registration → **Certificates & secrets** → **Certificates** → 上傳 public certificate。
...
使用 Azure Managed Identity 進行無密碼驗證。這非常適合部署在 Azure infrastructure（AKS、App Service、Azure VMs）且有 managed identity 可用的情境。

**運作方式：**

1. bot pod/VM 具有 managed identity（system-assigned 或 user-assigned）。
2. **federated identity credential** 將 managed identity 連結到 Entra ID app registration。
3. 在 runtime，OpenClaw 使用 `@azure/identity` 從 Azure IMDS endpoint (`169.254.169.254`) 取得 tokens。
4. token 會傳遞給 Teams SDK 進行 bot authentication。

**先決條件：**

- 已啟用 managed identity 的 Azure infrastructure（AKS workload identity、App Service、VM）
- 在 Entra ID app registration 上建立 federated identity credential
- pod/VM 可存取 IMDS (`169.254.169.254:80`) 的網路

**設定檔（system-assigned managed identity）：**
...
5. **確保可存取 IMDS**（`169.254.169.254`）- 如果使用 NetworkPolicy，請新增輸出規則，允許連到連接埠 80 上 `169.254.169.254/32` 的流量。
...
| **憑證**             | `authType: "federated"` + `certificatePath`    | 網路上沒有共用密碼       | 憑證管理成本               |
| **受控身分**         | `authType: "federated"` + `useManagedIdentity` | 無密碼，無需管理密鑰     | 需要 Azure 基礎架構        |

**預設行為：** 未設定 `authType` 時，OpenClaw 預設使用用戶端密碼驗證。既有設定可繼續運作，無需變更。
...
一次檢查 Bot 註冊、AAD 應用程式、資訊清單與 SSO 設定。
...
OpenClaw 為 Microsoft Teams 公開由 Graph 支援的 `member-info` 動作，讓代理程式與自動化可直接從 Microsoft Graph 解析頻道成員詳細資料（顯示名稱、電子郵件、角色）。

需求：

- `Member.Read.Group` RSC 權限（已在建議資訊清單中）
- 跨團隊查詢：具備管理員同意的 `User.Read.All` Graph 應用程式權限

此動作由 `channels.msteams.actions.memberInfo` 控制（預設：有 Graph 認證時啟用）。

## 歷史內容脈絡

- `channels.msteams.historyLimit` 控制要將多少最近的頻道/群組訊息包進提示。
- 會退回使用 `messages.groupChat.historyLimit`。設為 `0` 可停用（預設 50）。
- 擷取到的對話串歷史會依傳送者允許清單（`allowFrom` / `groupAllowFrom`）過濾，因此對話串脈絡植入只包含允許傳送者的訊息。
- 引用附件脈絡（從 Teams 回覆 HTML 衍生的 `ReplyTo*`）目前會以收到的內容傳遞。
- 換句話說，允許清單會控管誰可以觸發代理程式；目前只有特定補充脈絡路徑會被過濾。
- DM 歷史可使用 `channels.msteams.dmHistoryLimit`（使用者回合）限制。每位使用者覆寫：`channels.msteams.dms["<user_id>"].historyLimit`。

## 目前的 Teams RSC 權限（資訊清單）

以下是我們 Teams 應用程式資訊清單中的**既有 resourceSpecific 權限**。它們只適用於安裝該應用程式的團隊/聊天內。

**頻道（團隊範圍）：**

- `ChannelMessage.Read.Group`（Application）- 不需 @提及即可接收所有頻道訊息
...
- `ChatMessage.Read.Chat`（Application）- 不需 @提及即可接收所有群組聊天訊息
...
- `bots[].supportsFiles: true` 是在個人範圍處理檔案的必要設定。
...
| **離線可用**           | 否（必須正在執行）   | 是（可隨時查詢）                  |

**重點：** RSC 用於即時監聽；Graph API 用於歷史存取。若要在離線時補抓錯過的訊息，你需要具備 `ChannelMessage.Read.All` 的 Graph API（需要管理員同意）。

## 啟用 Graph 的媒體 + 歷史（頻道必要）

如果你需要**頻道**中的圖片/檔案，或想擷取**訊息歷史**，必須啟用 Microsoft Graph 權限並授與管理員同意。
...
**使用者提及的額外權限：** 對話中的使用者 @提及可直接使用。不過，如果你想動態搜尋並提及**不在目前對話中**的使用者，請新增 `User.Read.All`（Application）權限並授與管理員同意。
...
OpenClaw 透過快速返回並主動傳送回覆來處理這個情況，但非常慢的回應仍可能造成問題。
...
- 支援用於投票和語意呈現傳送的 Adaptive Cards（見下方）

## 設定

關鍵設定（共享頻道模式請見 `/gateway/configuration`）：
...
- `channels.msteams.dangerouslyAllowNameMatching`：緊急開關，用於重新啟用可變的 UPN/顯示名稱比對，以及直接團隊/頻道名稱路由。
- `channels.msteams.textChunkLimit`：對外文字區塊大小。
- `channels.msteams.chunkMode`：`length`（預設）或 `newline`，先依空白行（段落邊界）分割，再依長度分塊。
- `channels.msteams.mediaAllowHosts`：輸入附件主機允許清單（預設為 Microsoft/Teams 網域）。
- `channels.msteams.mediaAuthAllowHosts`：媒體重試時可附加 Authorization 標頭的主機允許清單（預設為 Graph + Bot Framework 主機）。
- `channels.msteams.requireMention`：在頻道/群組中要求 @mention（預設 true）。
...
- `channels.msteams.teams.<teamId>.tools`：每個團隊的預設工具政策覆寫（`allow`/`deny`/`alsoAllow`），在缺少頻道覆寫時使用。
...
- `channels.msteams.actions.memberInfo`：啟用或停用由 Graph 支援的成員資訊動作（預設：當 Graph 憑證可用時啟用）。
...
- `channels.msteams.sharePointSiteId`：群組聊天/頻道中檔案上傳所用的 SharePoint 網站 ID（請見[在群組聊天中傳送檔案](#sending-files-in-group-chats)）。
...
Teams 最近在相同的底層資料模型上引入了兩種頻道 UI 樣式：
...
**問題：** Teams API 不會公開頻道使用哪一種 UI 樣式。如果使用錯誤的 `replyStyle`：

- 在執行緒樣式頻道中使用 `thread` → 回覆會彆扭地顯示為巢狀
- 在貼文樣式頻道中使用 `top-level` → 回覆會顯示為獨立的頂層貼文，而不是執行緒內回覆

**解決方案：** 依據頻道設定方式，為每個頻道設定 `replyStyle`：
...
當 bot 將回覆傳送到頻道時，`replyStyle` 會從最具體的覆寫一路解析到預設值。第一個非 `undefined` 值勝出：
...
4. **隱含預設** — 由 `requireMention` 衍生：
   - `requireMention: true` → `thread`
   - `requireMention: false` → `top-level`

如果你在全域設定 `requireMention: false`，但沒有明確設定 `replyStyle`，貼文樣式頻道中的提及會以頂層貼文顯示，即使傳入訊息原本是執行緒回覆。請在全域、團隊或頻道層級釘選 `replyStyle: "thread"`，以避免意外情況。

### 執行緒內容保留

當 `replyStyle: "thread"` 生效，且 bot 是在頻道執行緒內被 @mentioned 時，OpenClaw 會將原始執行緒根重新附加到對外 conversation reference（`19:…@thread.tacv2;messageid=<root>`），讓回覆落在同一個執行緒內。這同時適用於即時（同回合）傳送，以及 Bot Framework 回合內容過期後進行的主動傳送（例如長時間執行的 agents、透過 `mcp__openclaw__message` 排入佇列的工具呼叫回覆）。

執行緒根取自 conversation reference 上儲存的 `threadId`。早於 `threadId` 的舊版已儲存 reference 會退回使用 `activityId`（最後為 conversation 播種的任何傳入 activity），因此既有部署不需要重新播種即可繼續運作。

當 `replyStyle: "top-level"` 生效時，頻道執行緒傳入訊息會刻意以新的頂層貼文回答，不會附加執行緒後綴。這是執行緒樣式頻道的正確行為；如果你看到頂層貼文，但預期是執行緒回覆，代表該頻道的 `replyStyle` 設定不正確。
...
- **頻道/群組：** 附件位於 M365 儲存空間（SharePoint/OneDrive）。Webhook 承載內容只包含 HTML stub，不包含實際檔案位元組。**需要 Graph API 權限**才能下載頻道附件。
- 對於明確以檔案優先的傳送，請使用 `action=upload-file` 搭配 `media` / `filePath` / `path`；選用的 `message` 會成為隨附文字/註解，`filename` 會覆寫上傳名稱。

沒有 Graph 權限時，含有圖片的頻道訊息會以純文字接收（bot 無法存取圖片內容）。
OpenClaw 預設只會從 Microsoft/Teams 主機名稱下載媒體。可使用 `channels.msteams.mediaAllowHosts` 覆寫（使用 `["*"]` 允許任何主機）。
Authorization 標頭只會附加到 `channels.msteams.mediaAuthAllowHosts` 中的主機（預設為 Graph + Bot Framework 主機）。請保持此清單嚴格（避免多租用戶後綴）。

## 在群組聊天中傳送檔案

Bot 可使用 FileConsentCard 流程在 DM 中傳送檔案（內建）。然而，**在群組聊天/頻道中傳送檔案**需要額外設定：

| 內容                     | 檔案傳送方式                              | 所需設定                                        |
| ------------------------ | ----------------------------------------- | ----------------------------------------------- |
| **DM**                   | FileConsentCard → 使用者接受 → bot 上傳   | 可直接運作                                      |
| **群組聊天/頻道**        | 上傳到 SharePoint → 分享連結              | 需要 `sharePointSiteId` + Graph 權限            |
| **圖片（任何內容）**     | Base64 編碼內嵌                           | 可直接運作                                      |

### 為什麼群組聊天需要 SharePoint

Bot 沒有個人 OneDrive drive（`/me/drive` Graph API 端點不適用於應用程式身分）。若要在群組聊天/頻道中傳送檔案，bot 會上傳到 **SharePoint 網站**並建立分享連結。
...
| 權限                                    | 分享行為                                           |
| --------------------------------------- | -------------------------------------------------- |
| 僅 `Sites.ReadWrite.All`                | 組織範圍分享連結（組織內任何人皆可存取）           |
...
| 任何內容 + 圖片                                   | Base64 編碼內嵌（不需 SharePoint 即可運作）      |
...
使用 `message` 工具或 CLI，將語意化簡報承載內容傳送給 Teams 使用者或交談。OpenClaw 會依據通用簡報合約，將其呈現為 Teams Adaptive Cards。
...
- 如需 `dmPolicy` 和允許清單閘門，請參閱 `/gateway/configuration`。

## 團隊與頻道 ID（常見陷阱）

Teams URL 中的 `groupId` 查詢參數**不是**設定使用的團隊 ID。請改從 URL 路徑擷取 ID：
...
- 團隊鍵 = `/team/` 後方的路徑片段（URL 解碼後，例如 `19:Bk4j...@thread.tacv2`；較舊租用戶可能顯示 `@thread.skype`，這也有效）
- 頻道鍵 = `/channel/` 後方的路徑片段（URL 解碼後）
- **忽略** OpenClaw 路由中的 `groupId` 查詢參數。它是 Microsoft Entra 群組 ID，不是傳入 Teams 活動使用的 Bot Framework 交談 ID。
...
- **頻道中沒有回應：** 預設需要提及；請設定 `channels.msteams.requireMention=false`，或依團隊/頻道設定。
- **版本不符（Teams 仍顯示舊資訊清單）：** 移除並重新新增應用程式，然後完全結束 Teams 以重新整理。
- **Webhook 傳回 401 Unauthorized：** 手動測試時若未使用 Azure JWT，這是預期結果，表示端點可連線但驗證失敗。請使用 Azure Web Chat 正確測試。

### 資訊清單上傳錯誤

- **"Icon file cannot be empty"：** 資訊清單參照了 0 位元組的圖示檔。請建立有效的 PNG 圖示（`outline.png` 為 32x32，`color.png` 為 192x192）。
...
- **側載失敗：** 請嘗試「Upload an app to your org's app catalog」，而不是「Upload a custom app」- 這通常能繞過側載限制。
