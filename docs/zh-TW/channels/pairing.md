...
read_when:
    ...
    - 檢視 OpenClaw 安全態勢
summary: 配對概覽：核准誰可以傳送私訊給你 + 哪些節點可以加入
title: 配對
x-i18n:
    generated_at: "2026-05-06T02:44:20Z"
    model: gpt-5.5
    provider: openai
...
- 待處理的 DM 配對請求預設每個訊息通道上限為 **3 個**；額外請求會被忽略，直到其中一個過期或被核准。
...
這會為首次設定提供一個明確的擁有者，用於特權命令與 exec
核准提示。擁有者存在後，後續配對核准只授予 DM
存取；它們不會新增更多擁有者。

支援的訊息通道：`bluebubbles`、`discord`、`feishu`、`googlechat`、`imessage`、`irc`、`line`、`matrix`、`mattermost`、`msteams`、`nextcloud-talk`、`nostr`、`openclaw-weixin`、`signal`、`slack`、`synology-chat`、`telegram`、`twitch`、`whatsapp`、`zalo`、`zalouser`。

### 可重複使用的寄件者群組

當同一組受信任寄件者應套用到多個訊息通道，或同時套用到 DM 與群組允許清單時，請使用頂層 `accessGroups`。

靜態群組使用 `type: "message.senders"`，並從訊息通道允許清單以
`accessGroup:<name>` 參照：
...
- 非預設帳號只讀寫其範圍內的允許清單檔案。
- 預設帳號使用訊息通道範圍、未加範圍的允許清單檔案。

請將這些視為敏感內容（它們會管控對你助理的存取）。

<Note>
配對允許清單儲存用於 DM 存取。群組授權是分開的。
核准 DM 配對代碼不會自動允許該寄件者執行群組
命令，或在群組中控制機器人。首次擁有者啟動設定是
`commands.ownerAllowFrom` 中獨立的設定狀態，而群組聊天傳遞仍遵循
訊息通道的群組允許清單（例如 `groupAllowFrom`、`groups`，或依訊息通道而定的個別群組
或個別主題覆寫）。
</Note>

## 2) Node 裝置配對（iOS/Android/macOS/無頭節點）

節點會以 `role: node` 的**裝置**身分連線到 Gateway。Gateway
會建立必須被核准的裝置配對請求。
...
2. 機器人會回覆兩則訊息：一則說明訊息，以及一則獨立的**設定代碼**訊息（在 Telegram 中易於複製/貼上）。
...
- 任何移交的 `operator` token 都會限制在啟動允許清單：
  `operator.approvals`、`operator.read`、`operator.talk.secrets`、`operator.write`
- 啟動範圍檢查是以角色作為前綴，而不是單一扁平範圍池：
  operator 範圍項目只滿足 operator 請求，而非 operator 角色
  仍必須在自己的角色前綴下請求範圍
- 後續 token 輪換/撤銷仍受裝置已核准的
  角色合約與呼叫端工作階段的 operator 範圍共同限制

設定代碼有效時，請像密碼一樣保護它。

對於 Tailscale、公開或其他遠端行動配對，請使用 Tailscale Serve/Funnel
或另一個 `wss://` Gateway URL。明文 `ws://` 設定代碼只接受
...
當明確核准因為正在核准的已配對裝置工作階段
以僅配對範圍開啟而遭拒時，CLI 會使用
`operator.admin` 重試同一個請求。這讓現有具備 admin 能力的已配對裝置，可以在不手動編輯 `devices/paired.json` 的情況下恢復新的
Control UI/瀏覽器配對。Gateway 仍會驗證重試的連線；無法以
...
<Note>
已配對的裝置不會被靜默授予更廣泛的存取權。如果它重新連線並請求更多範圍或更廣泛的角色，OpenClaw 會保持現有核准不變，並建立新的待處理升級請求。核准前，請使用 `openclaw devices list` 比較目前已核准的存取權與新請求的存取權。
</Note>
...
- 舊版 `node.pair.*` API（CLI：`openclaw nodes pending|approve|reject|remove|rename`）是
  獨立的 Gateway 擁有配對儲存。WS 節點仍需要裝置配對。
- 配對記錄是已核准角色的持久事實來源。作用中
  裝置 token 仍限制在該已核准角色集合內；已核准角色以外的零散 token 項目
  不會建立新的存取權。
