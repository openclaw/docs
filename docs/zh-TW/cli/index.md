...
`openclaw` 是主要的 CLI 進入點。每個核心命令都有專屬參考頁面，或會在其別名對應的命令中記錄；此索引列出命令、全域旗標，以及套用於整個 CLI 的輸出樣式規則。

依目的使用設定命令：

- `openclaw setup` 會建立基準設定與工作區，而不會走完整的引導式上線流程。
- `openclaw onboard` 是完整的引導式首次執行路徑，涵蓋 Gateway、模型驗證、工作區、通道、Skills 與健康狀態。
- `openclaw configure` 會變更既有設定的特定部分，例如模型驗證、Gateway、通道、Plugin 或 Skills。
- `openclaw channels add` 會在基準存在後設定通道帳號；不加旗標執行可進入引導式通道設定，或搭配通道專用旗標用於腳本。
...
| 網路與 Node          | [`directory`](/zh-TW/cli/directory) · [`nodes`](/zh-TW/cli/nodes) · [`devices`](/zh-TW/cli/devices) · [`node`](/zh-TW/cli/node)                                                                                                                                   |
...
- OSC-8 超連結會在支援處呈現為可點擊連結；否則 CLI 會退回純 URL。
- `--json`（以及支援處的 `--plain`）會停用樣式，以提供乾淨輸出。
- 長時間執行的命令會顯示進度指示器（支援時使用 OSC 9;4）。

調色盤唯一真實來源：`src/terminal/palette.ts`。
...
Plugin 可以加入其他頂層命令（例如 `openclaw voicecall`）。
...
`openclaw status --usage` 與 Control UI 會在 OAuth/API 憑證可用時顯示供應商用量/配額。資料會直接來自供應商用量端點，並正規化為 `X% left`。目前具有用量時窗的供應商：Anthropic、GitHub Copilot、Gemini CLI、OpenAI Codex、MiniMax、Xiaomi 和 z.ai。
