...
summary: 外寄回覆的文字轉語音 — 供應商、角色、斜線命令與各頻道輸出
title: 文字轉語音
x-i18n:
    generated_at: "2026-06-27T20:11:30Z"
    model: gpt-5.5
...
OpenClaw 可以將傳出的回覆轉換成音訊，支援 **14 個語音提供者**，並在 Feishu、Matrix、Telegram 和 WhatsApp 上傳送原生語音訊息，在其他地方傳送音訊附件，以及為電話語音與 Talk 提供 PCM/Ulaw 串流。

TTS 是 Talk 的 `stt-tts` 模式中負責語音輸出的半部。提供者原生的 `realtime` Talk 工作階段會在即時提供者內合成語音，而不是呼叫此 TTS 路徑；`transcription` 工作階段則不會合成助理語音回覆。

## 快速開始

<Steps>
  <Step title="Pick a provider">
    OpenAI 和 ElevenLabs 是最可靠的託管選項。Microsoft 和本機命令列介面不需要 API 金鑰即可運作。完整清單請參閱[提供者矩陣](#supported-providers)。
  </Step>
  <Step title="Set the API key">
    匯出你的提供者所需的環境變數（例如 `OPENAI_API_KEY`、`ELEVENLABS_API_KEY`）。Microsoft 和本機命令列介面不需要金鑰。
  </Step>
...
<Note>
Auto-TTS 預設為**關閉**。當未設定 `messages.tts.provider` 時，OpenClaw 會依照登錄自動選取順序挑選第一個已設定的提供者。內建的 `tts` 代理工具僅用於明確意圖：一般聊天會維持文字，除非使用者要求音訊、使用 `/tts`，或啟用 Auto-TTS/指令式語音。
</Note>
...
| 提供者            | 驗證                                                                                                             | 備註                                                                                        |
| ----------------- | ---------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------- |
| **Azure Speech**  | `AZURE_SPEECH_KEY` + `AZURE_SPEECH_REGION`（也支援 `AZURE_SPEECH_API_KEY`、`SPEECH_KEY`、`SPEECH_REGION`）        | 原生 Ogg/Opus 語音便條輸出與電話語音。                                                      |
| **DeepInfra**     | `DEEPINFRA_API_KEY`                                                                                              | OpenAI 相容的 TTS。預設為 `hexgrad/Kokoro-82M`。                                            |
| **ElevenLabs**    | `ELEVENLABS_API_KEY` 或 `XI_API_KEY`                                                                             | 語音複製、多語言，透過 `seed` 決定性輸出；串流用於 Discord 語音播放。                       |
| **Google Gemini** | `GEMINI_API_KEY` 或 `GOOGLE_API_KEY`                                                                             | Gemini API 批次 TTS；透過 `promptTemplate: "audio-profile-v1"` 支援角色風格。               |
| **Gradium**       | `GRADIUM_API_KEY`                                                                                                | 語音便條與電話語音輸出。                                                                    |
| **Inworld**       | `INWORLD_API_KEY`                                                                                                | 串流 TTS API。原生 Opus 語音便條與 PCM 電話語音。                                           |
| **本機命令列介面** | 無                                                                                                               | 執行已設定的本機 TTS 命令。                                                                 |
| **Microsoft**     | 無                                                                                                               | 透過 `node-edge-tts` 使用公開 Edge 神經網路 TTS。盡力提供，無 SLA。                         |
...
| **xAI**           | `XAI_API_KEY`                                                                                                    | xAI 批次 TTS。**不**支援原生 Opus 語音便條。                                                |
| **Xiaomi MiMo**   | `XIAOMI_API_KEY`                                                                                                 | 透過 Xiaomi chat completions 使用 MiMo TTS。                                                |

如果設定了多個提供者，所選的提供者會優先使用，其他提供者則作為備援選項。自動摘要會使用 `summaryModel`（或 `agents.defaults.model.primary`），因此如果你保留摘要功能啟用，該提供者也必須完成驗證。

<Warning>
內建的 **Microsoft** 提供者會透過 `node-edge-tts` 使用 Microsoft Edge 的線上神經網路 TTS 服務。這是沒有已發布 SLA 或配額的公開 Web 服務，請視為盡力提供。舊版提供者 ID `edge` 會正規化為 `microsoft`，且 `openclaw doctor --fix` 會重寫已保存的設定；新設定應一律使用 `microsoft`。
</Warning>

## 設定

TTS 設定位於 `~/.openclaw/openclaw.json` 的 `messages.tts` 下。選擇一個預設範本並調整提供者區塊：
...
若使用 Xiaomi `mimo-v2.5-tts-voicedesign`，請省略 `speakerVoice`，並將 `style` 設為語音設計提示。OpenClaw 會將該提示作為 TTS `user` 訊息送出，且不會為 voicedesign 模型送出 `audio.voice`。

### 每個代理的語音覆寫

使用 `agents.list[].tts` 可讓某個 agent 以不同的提供者、語音、模型、persona 或自動 TTS 模式發聲。agent 區塊會深度合併到 `messages.tts` 之上，因此提供者憑證可以保留在全域提供者設定中：
...
若要釘選每個 agent 的 persona，請在提供者設定旁設定 `agents.list[].tts.persona`，它只會覆寫該 agent 的全域 `messages.tts.persona`。

自動回覆、`/tts audio`、`/tts status` 與 `tts` agent 工具的優先順序：
...
頻道與帳號覆寫使用與 `messages.tts` 相同的形狀，並會深度合併到先前的層級之上，因此共享提供者憑證可以保留在 `messages.tts`，而頻道或機器人帳號只變更說話語音、模型、persona 或自動模式：
...
**persona** 是穩定的口語身分，可跨提供者以確定性方式套用。它可以偏好某個提供者、定義提供者中立的提示意圖，並攜帶語音、模型、提示範本、種子與語音設定的提供者專屬繫結。
...
Persona 提示欄位（`profile`、`scene`、`sampleContext`、`style`、`accent`、`pacing`、`constraints`）是**提供者中立**的。每個提供者會自行決定如何使用：

<AccordionGroup>
  <Accordion title="Google Gemini">
    只有當有效的 Google 提供者設定設為 `promptTemplate: "audio-profile-v1"` 或 `personaPrompt` 時，才會將 persona 提示欄位包裝成 Gemini TTS 提示結構。較舊的 `audioProfile` 與 `speakerName` 欄位仍會作為 Google 專屬提示文字前置加入。`[[tts:text]]` 區塊內的行內音訊標籤，例如 `[whispers]` 或 `[laughs]`，會保留在 Gemini 逐字稿內；OpenClaw 不會產生這些標籤。
  </Accordion>
  <Accordion title="OpenAI">
    只有在未設定明確 OpenAI `instructions` 時，才會將 persona 提示欄位映射到請求的 `instructions` 欄位。明確的 `instructions` 一律優先。
  </Accordion>
  <Accordion title="其他提供者">
    只使用 `personas.<id>.providers.<provider>` 底下的提供者專屬 persona 繫結。除非提供者實作自己的 persona 提示映射，否則會忽略 persona 提示欄位。
  </Accordion>
</AccordionGroup>

### 備援政策

`fallbackPolicy` 控制 persona 對嘗試的提供者**沒有繫結**時的行為：

| 政策                | 行為                                                                                                                                       |
| ------------------- | ------------------------------------------------------------------------------------------------------------------------------------------ |
| `preserve-persona`  | **預設。** 提供者中立提示欄位仍可使用；提供者可以使用或忽略它們。                                                                         |
| `provider-defaults` | 該次嘗試的提示準備會省略 persona；提供者會使用其中立預設值，同時繼續備援到其他提供者。                                                     |
| `fail`              | 以 `reasonCode: "not_configured"` 與 `personaBinding: "missing"` 跳過該提供者嘗試。仍會嘗試備援提供者。                                     |

整個 TTS 請求只會在**每個**嘗試的提供者都被跳過或失敗時才會失敗。

Talk 工作階段提供者選擇具有工作階段範圍。Talk 用戶端應從 `talk.catalog` 選擇提供者 ID、模型 ID、語音 ID 與語言環境，並透過 Talk 工作階段或 handoff 請求傳遞它們。開啟語音工作階段不應修改 `messages.tts` 或全域 Talk 提供者預設值。

## 模型驅動指令

預設情況下，assistant **可以**輸出 `[[tts:...]]` 指令，為單一回覆覆寫語音、模型或速度，並可附帶選用的 `[[tts:text]]...[[/tts:text]]` 區塊，用於只應出現在音訊中的表達提示：
...
當 `messages.tts.auto` 為 `"tagged"` 時，**必須有指令**才會觸發音訊。串流區塊傳遞會在頻道看見之前，從可見文字中移除指令，即使它們分散在相鄰區塊中也一樣。

除非 `modelOverrides.allowProvider: true`，否則會忽略 `provider=...`。當回覆宣告 `provider=...` 時，該指令中的其他鍵只會由該提供者剖析；不支援的鍵會被移除，並以 TTS 指令警告回報。
...
單一命令 `/tts`。在 Discord 上，OpenClaw 也會註冊 `/voice`，因為 `/tts` 是內建 Discord 命令，文字 `/tts ...` 仍然可用。
...
<Note>
命令需要授權的傳送者（套用 allowlist/owner 規則），且必須啟用 `commands.text` 或原生命令註冊。
</Note>

行為備註：

- `/tts on` 會將本機 TTS 偏好寫入 `always`；`/tts off` 會寫入 `off`。
- `/tts chat on|off|default` 會為目前聊天寫入工作階段範圍的自動 TTS 覆寫。
- `/tts persona <id>` 會寫入本機 persona 偏好；`/tts persona off` 會清除它。
- `/tts latest` 會從目前工作階段逐字稿讀取最新的 assistant 回覆，並將其傳送為一次音訊。它只會在工作階段項目上儲存該回覆的雜湊，以抑制重複語音傳送。
- `/tts audio` 會產生一次性音訊回覆（**不會**開啟 TTS）。
- `limit` 與 `summary` 會儲存在**本機偏好**中，而不是主要設定。
- `/tts status` 包含最新嘗試的備援診斷，`Fallback: <primary> -> <used>`、`Attempts: ...`，以及每次嘗試的詳細資料（`provider:outcome(reasonCode) latency`）。
- `/status` 會在 TTS 啟用時顯示啟用中的 TTS 模式，以及已設定的提供者、模型、語音與已消毒的自訂端點中繼資料。
...
這些會覆寫 `messages.tts` 加上該主機啟用中的 `agents.list[].tts` 區塊所形成的有效設定。

## 輸出格式（固定）

TTS 語音傳遞由頻道能力驅動。頻道外掛會宣告語音樣式 TTS 是否應要求提供者產生原生 `voice-note` 目標，或保留一般 `audio-file` 合成，並只將相容輸出標記為語音傳遞。

- **支援語音訊息的通道**：語音訊息回覆優先使用 Opus（ElevenLabs 的 `opus_48000_64`，OpenAI 的 `opus`）。
  - 48kHz / 64kbps 是語音訊息的良好取捨。
- **Feishu / WhatsApp**：當語音訊息回覆產生為 MP3/WebM/WAV/M4A
  或其他可能的音訊檔時，通道外掛會在傳送原生語音訊息前，
  使用 `ffmpeg` 將其轉碼為 48kHz Ogg/Opus。WhatsApp 會透過 Baileys
  `audio` payload 傳送結果，並使用 `ptt: true` 和
  `audio/ogg; codecs=opus`。如果轉換失敗，Feishu 會收到原始檔案作為附件；
  WhatsApp 傳送會失敗，而不是發布不相容的 PTT payload。
- **其他通道**：MP3（ElevenLabs 的 `mp3_44100_128`，OpenAI 的 `mp3`）。
  - 44.1kHz / 128kbps 是語音清晰度的預設平衡。
- **MiniMax**：一般音訊附件使用 MP3（`speech-2.8-hd` 模型，32kHz 取樣率）。對於通道宣告的語音訊息目標，當通道宣告支援轉碼時，OpenClaw 會在交付前使用 `ffmpeg` 將 MiniMax MP3 轉碼為 48kHz Opus。
- **Xiaomi MiMo**：預設使用 MP3，或在設定時使用 WAV。對於通道宣告的語音訊息目標，當通道宣告支援轉碼時，OpenClaw 會在交付前使用 `ffmpeg` 將 Xiaomi 輸出轉碼為 48kHz Opus。
- **本機命令列介面**：使用已設定的 `outputFormat`。語音訊息目標會
  轉換為 Ogg/Opus，電話語音輸出會使用 `ffmpeg` 轉換為原始 16 kHz 單聲道 PCM。
- **Google Gemini**：Gemini API TTS 會回傳原始 24kHz PCM。OpenClaw 會將其包裝為 WAV 以供音訊附件使用、轉碼為 48kHz Opus 以供語音訊息目標使用，並針對 Talk/電話語音直接回傳 PCM。
- **Gradium**：音訊附件使用 WAV，語音訊息目標使用 Opus，電話語音使用 8 kHz 的 `ulaw_8000`。
- **Inworld**：一般音訊附件使用 MP3，語音訊息目標使用原生 `OGG_OPUS`，Talk/電話語音使用 22050 Hz 的原始 `PCM`。
- **xAI**：預設使用 MP3；`responseFormat` 可以是 `mp3`、`wav`、`pcm`、`mulaw` 或 `alaw`。OpenClaw 使用 xAI 的批次 REST TTS 端點並回傳完整的音訊附件；此提供者路徑不使用 xAI 的串流 TTS WebSocket。此路徑不支援原生 Opus 語音訊息格式。
...
- 略過非常短的回覆（少於 10 個字元）。
- 啟用摘要時，使用 `summaryModel`（或 `agents.defaults.model.primary`）
  摘要較長的回覆。
- 將產生的音訊附加到回覆。
- 在 `mode: "final"` 中，對於串流的最終回覆，仍會在文字串流完成後
...
## 按通道劃分的輸出格式
  ...
  | 目標                                  | 格式                                                                                                                                  |
  | ------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------- |
  | Feishu / Matrix / Telegram / WhatsApp | 語音訊息回覆偏好使用 **Opus**（ElevenLabs 的 `opus_48000_64`、OpenAI 的 `opus`）。48 kHz / 64 kbps 可在清晰度與大小之間取得平衡。 |
  | 其他頻道                              | **MP3**（ElevenLabs 的 `mp3_44100_128`、OpenAI 的 `mp3`）。44.1 kHz / 128 kbps 是語音的預設值。                                 |
  | 通話 / 電話語音                       | 提供者原生 **PCM**（Inworld 22050 Hz、Google 24 kHz），或使用 Gradium 的 `ulaw_8000` 進行電話語音。                                 |

  各提供者注意事項：

  - **Feishu / WhatsApp 轉碼：** 當語音訊息回覆以 MP3/WebM/WAV/M4A 傳入時，頻道外掛會使用 `ffmpeg` 轉碼為 48 kHz Ogg/Opus。WhatsApp 會透過 Baileys 以 `ptt: true` 和 `audio/ogg; codecs=opus` 傳送。如果轉換失敗：Feishu 會改為附加原始檔案；WhatsApp 則會傳送失敗，而不是發布不相容的 PTT 酬載。
  - **MiniMax / Xiaomi MiMo：** 預設為 MP3（MiniMax `speech-2.8-hd` 為 32 kHz）；透過 `ffmpeg` 為語音訊息目標轉碼為 48 kHz Opus。
  - **本機命令列介面：** 使用設定的 `outputFormat`。語音訊息目標會轉換為 Ogg/Opus，電話語音輸出則轉換為原始 16 kHz 單聲道 PCM。
  - **Google Gemini：** 傳回原始 24 kHz PCM。OpenClaw 會包裝為 WAV 供附件使用，為語音訊息目標轉碼為 48 kHz Opus，並直接為通話/電話語音傳回 PCM。
  - **Inworld：** MP3 附件、原生 `OGG_OPUS` 語音訊息、供通話/電話語音使用的原始 `PCM` 22050 Hz。
  - **xAI：** 預設為 MP3；`responseFormat` 可為 `mp3|wav|pcm|mulaw|alaw`。使用 xAI 的批次 REST 端點 — **不** 使用串流 WebSocket TTS。不支援原生 Opus 語音訊息格式。
  - **Microsoft：** 使用 `microsoft.outputFormat`（預設 `audio-24khz-48kbitrate-mono-mp3`）。Telegram `sendVoice` 接受 OGG/MP3/M4A；如果需要保證使用 Opus 語音訊息，請使用 OpenAI/ElevenLabs。如果設定的 Microsoft 格式失敗，OpenClaw 會以 MP3 重試。

  OpenAI 和 ElevenLabs 的輸出格式會依照上方所列的各頻道固定。
  ...
  <Accordion title="Top-level messages.tts.*">
    <ParamField path="auto" type='"off" | "always" | "inbound" | "tagged"'>
      自動 TTS 模式。`inbound` 只會在傳入語音訊息後傳送音訊；`tagged` 只會在回覆包含 `[[tts:...]]` 指令或 `[[tts:text]]` 區塊時傳送音訊。
    </ParamField>
    <ParamField path="enabled" type="boolean" deprecated>
      舊版切換開關。`openclaw doctor --fix` 會將其遷移到 `auto`。
    </ParamField>
    <ParamField path="mode" type='"final" | "all"' default="final">
      `"all"` 除了最終回覆之外，也包含工具/區塊回覆。
    </ParamField>
    ...
    <ParamField path="providers.<id>" type="object">
      由提供者擁有、以語音提供者 id 作為鍵的設定。舊版直接區塊（`messages.tts.openai`、`.elevenlabs`、`.microsoft`、`.edge`）會由 `openclaw doctor --fix` 重寫；只提交 `messages.tts.providers.<id>`。
    </ParamField>
    <ParamField path="maxTextLength" type="number">
      TTS 輸入字元的硬性上限。若超過上限，`/tts audio` 會失敗。
    </ParamField>
  ...
  <Accordion title="Azure Speech">
    ...
    <ParamField path="speakerVoice" type="string">Azure 語音 ShortName。預設 `en-US-JennyNeural`。舊版別名：`voice`。</ParamField>
  ...
  <Accordion title="ElevenLabs">
    <ParamField path="apiKey" type="string">會回退到 `ELEVENLABS_API_KEY` 或 `XI_API_KEY`。</ParamField>
    <ParamField path="model" type="string">模型 id（例如 `eleven_multilingual_v2`、`eleven_v3`）。</ParamField>
    <ParamField path="speakerVoiceId" type="string">ElevenLabs 語音 id。舊版別名：`voiceId`。</ParamField>
    ...
    <ParamField path="seed" type="number">整數 `0..4294967295`，用於盡力達成決定性。</ParamField>
    <ParamField path="baseUrl" type="string">覆寫 ElevenLabs API 基底 URL。</ParamField>
  </Accordion>

  <Accordion title="Google Gemini">
    <ParamField path="apiKey" type="string">會回退到 `GEMINI_API_KEY` / `GOOGLE_API_KEY`。若省略，TTS 可在回退到環境變數之前重用 `models.providers.google.apiKey`。</ParamField>
    <ParamField path="model" type="string">Gemini TTS 模型。預設 `gemini-3.1-flash-tts-preview`。</ParamField>
    <ParamField path="speakerVoice" type="string">Gemini 預建語音名稱。預設 `Kore`。舊版別名：`voiceName`、`voice`。</ParamField>
  ...
  <Accordion title="Gradium">
    ...
    <ParamField path="baseUrl" type="string">預設 `https://api.gradium.ai`。</ParamField>
  ...
  <Accordion title="Inworld">
    ...
    <ParamField path="modelId" type="string">預設 `inworld-tts-1.5-max`。另有：`inworld-tts-1.5-mini`、`inworld-tts-1-max`、`inworld-tts-1`。</ParamField>
  ...
  <Accordion title="Local CLI (tts-local-cli)">
    ...
    <ParamField path="timeoutMs" type="number">命令逾時毫秒數。預設 `120000`。</ParamField>
  ...
  <Accordion title="Microsoft (no API key)">
    ...
    <ParamField path="speakerVoice" type="string">Microsoft 神經語音名稱（例如 `en-US-MichelleNeural`）。舊版別名：`voice`。</ParamField>
    <ParamField path="lang" type="string">語言代碼（例如 `en-US`）。</ParamField>
    <ParamField path="outputFormat" type="string">Microsoft 輸出格式。預設 `audio-24khz-48kbitrate-mono-mp3`。並非所有格式都受內建 Edge 後援傳輸支援。</ParamField>
  ...
  <Accordion title="MiniMax">
    ...
    <ParamField path="pitch" type="number">整數 `-12..12`。預設 `0`。小數值會在請求前截斷。</ParamField>
  </Accordion>
  ...
  <Accordion title="OpenAI">
    ...
    <ParamField path="speakerVoice" type="string">語音名稱（例如 `alloy`、`cedar`）。舊版別名：`voice`。</ParamField>
    <ParamField path="instructions" type="string">明確的 OpenAI `instructions` 欄位。設定後，角色提示欄位**不會**自動映射。</ParamField>
    <ParamField path="extraBody / extra_body" type="Record<string, unknown>">在產生的 OpenAI TTS 欄位之後，合併到 `/audio/speech` 請求主體的額外 JSON 欄位。可用於 Kokoro 等 OpenAI 相容端點，這類端點需要 `lang` 等供應商專用鍵；不安全的 prototype 鍵會被忽略。</ParamField>
  ...
  <Accordion title="Xiaomi MiMo">
    ...
    <ParamField path="model" type="string">預設 `mimo-v2.5-tts`。環境變數：`XIAOMI_TTS_MODEL`。也支援 `mimo-v2-tts` 與 `mimo-v2.5-tts-voicedesign`。</ParamField>
    <ParamField path="speakerVoice" type="string">預設語音模型的預設值為 `mimo_default`。環境變數：`XIAOMI_TTS_VOICE`。舊版別名：`voice`。不會針對 `mimo-v2.5-tts-voicedesign` 傳送。</ParamField>
    <ParamField path="format" type='"mp3" | "wav"'>預設 `mp3`。環境變數：`XIAOMI_TTS_FORMAT`。</ParamField>
    <ParamField path="style" type="string">以使用者訊息送出的選用自然語言風格指示；不會被朗讀。對於 `mimo-v2.5-tts-voicedesign`，這是語音設計提示；省略時 OpenClaw 會提供預設值。</ParamField>
  </Accordion>
</AccordionGroup>

## Agent 工具

`tts` 工具會將文字轉換為語音，並回傳音訊附件以供回覆傳送。在 Feishu、Matrix、Telegram 和 WhatsApp 上，音訊會以語音訊息而非檔案附件傳送。當 `ffmpeg` 可用時，Feishu 和 WhatsApp 可在此路徑上將非 Opus TTS 輸出轉碼。

WhatsApp 會透過 Baileys 將音訊作為 PTT 語音備註傳送（`audio` 搭配 `ptt: true`），並且會將可見文字與 PTT 音訊**分開**傳送，因為用戶端不一定會一致地在語音備註上呈現說明文字。

此工具接受選用的 `channel` 和 `timeoutMs` 欄位；`timeoutMs` 是單次呼叫的供應商請求逾時毫秒數。單次呼叫值會覆寫 `messages.tts.timeoutMs`；已設定的 TTS 逾時會覆寫任何外掛撰寫的供應商預設值。

## 閘道 RPC

| 方法              | 用途                               |
| ----------------- | ---------------------------------- |
| `tts.status`      | 讀取目前 TTS 狀態與最後一次嘗試。 |
