...
summary: 所有可用的斜線命令、指令與內嵌捷徑 — 設定、路由與各介面的行為。
title: 斜線命令
x-i18n:
    generated_at: "2026-07-01T20:13:08Z"
    model: gpt-5.5
...
閘道會處理以 `/` 開頭、作為獨立訊息送出的命令。
僅限主機的 bash 命令使用 `! <cmd>`（`/bash <cmd>` 作為別名）。

當對話綁定到 ACP 工作階段時，一般文字會路由到 ACP
harness。閘道管理命令仍維持本機處理：`/acp ...` 一律會送達
OpenClaw 命令處理器，而只要該介面已啟用命令處理，`/status` 加上 `/unfocus` 就會維持本機處理。
...
<CardGroup cols={3}>
  ...
  <Card title="指令" icon="sliders">
    ...
    `/exec`、`/model`、`/queue` — 會在模型看到訊息之前從訊息中移除。單獨送出時會保留工作階段設定；與其他文字一併送出時則作為行內提示。
  </Card>
  <Card title="行內捷徑" icon="bolt">
    `/help`、`/commands`、`/status`、`/whoami` — 立即執行，並在模型看到剩餘文字之前移除。僅限已授權的傳送者。
  </Card>
</CardGroup>
...
<AccordionGroup>
  <Accordion title="指令行為詳細資料">
    - 指令會在模型看到訊息之前從訊息中移除。
    - 在**僅含指令**的訊息中（訊息只有指令），它們會保留到工作階段，並回覆確認。
    - 在含有其他文字的**一般聊天**訊息中，它們會作為行內提示，且**不會**保留工作階段設定。
    - 指令只適用於**已授權的傳送者**。如果已設定 `commands.allowFrom`，它就是唯一使用的允許清單；否則授權會來自頻道允許清單/配對加上 `commands.useAccessGroups`。未授權的傳送者會看到指令被視為純文字。
  </Accordion>
</AccordionGroup>
...
<ParamField path="commands.text" type="boolean" default="true">
  ...
  （WhatsApp、WebChat、Signal、iMessage、Google Chat、Microsoft Teams）上，即使設為 `false`，文字
  命令仍會運作。
</ParamField>
...
<ParamField path="commands.native" type='boolean | "auto"' default='"auto"'>
  ...
  註冊；先前註冊的命令可能會在移除前仍保持可見。
</ParamField>

<ParamField path="commands.nativeSkills" type='boolean | "auto"' default='"auto"'>
  支援時以原生方式註冊 skill 命令。自動：Discord/Telegram 開啟；
  Slack 關閉。可用
...
<ParamField path="commands.bashForegroundMs" type="number" default="2000">
  bash 在切換到背景模式之前等待的時間（`0` 會立即進入背景）。
</ParamField>
...
<ParamField path="commands.mcp" type="boolean" default="false">
  啟用 `/mcp`（讀取/寫入 `mcp.servers` 底下由 OpenClaw 管理的 MCP 設定）。僅限擁有者。
</ParamField>
...
<AccordionGroup>
  <Accordion title="工作階段與執行">
    ...
    | `/reset [soft [message]]` | 就地重設目前工作階段。`soft` 會保留 transcript、移除重複使用的命令列介面後端工作階段 id，並重新執行啟動 |
    | `/name <title>` | 命名或重新命名目前工作階段。省略標題可查看目前名稱與建議 |
    | `/compact [instructions]` | 壓縮工作階段情境。請參閱[壓縮](/zh-TW/concepts/compaction) |
    | `/stop` | 中止目前執行 |
    | `/session idle <duration\|off>` | 管理 thread 綁定的閒置到期時間 |
    | `/session max-age <duration\|off>` | 管理 thread 綁定的最長存續時間到期 |
    ...
    <Note>
      ...
      且目前父項是 agent 的主要工作階段 — 在此情況下，`/new`
  ...
  <Accordion title="模型與執行控制">
    ...
    | `/queue <mode>` | 管理作用中執行佇列行為。請參閱[佇列](/zh-TW/concepts/queue)與[佇列導向](/zh-TW/concepts/queue-steering) |
    | `/steer <message>` | 將指引注入作用中執行。別名：`/tell`。請參閱[導向](/zh-TW/tools/steer) |
    ...
    <AccordionGroup>
      <Accordion title="verbose / trace / fast / reasoning 安全性">
        ...
        - `/fast` 依提供者而定：OpenAI/Codex 會將其對應到 `service_tier=priority`；直接 Anthropic 請求會將其對應到 `service_tier=auto` 或 `standard_only`。
        - `/reasoning`、`/verbose` 與 `/trace` 在群組設定中有風險 — 它們可能揭露內部推理或外掛診斷資訊。請在群組聊天中保持關閉。

      </Accordion>
      <Accordion title="模型切換詳細資料">
        - `/model` 會立即將新模型保留到工作階段。
        - 如果 agent 閒置中，下一次執行會立即使用它。
        - 如果執行正在進行，切換會標記為待處理，並在下一個乾淨的重試點套用。
  ...
  <Accordion title="探索與狀態">
    ...
    | `/status` | 顯示執行/執行階段狀態、閘道與系統 uptime、外掛健康狀態，以及提供者使用量/配額 |
    ...
    | `/diagnostics [note]` | 僅限擁有者的支援報告流程。每次都要求 exec 核准 |
    | `/crestodian <request>` | 從擁有者 DM 執行 Crestodian 設定與修復輔助程式 |
    | `/tasks` | 列出目前工作階段的作用中/近期背景工作 |
    | `/context [list\|detail\|map\|json]` | 說明情境如何組裝 |
  ...
  <Accordion title="子代理與 ACP">
    ...
    | `/focus <target>` | 將目前的 Discord 串或 Telegram 主題繫結到工作階段目標 |
    | `/unfocus` | 移除目前的串繫結 |
    | `/agents` | 列出目前工作階段的串繫結代理 |
  </Accordion>
...
| `/phone status\|arm ...\|disarm`                                                             | 暫時啟用高風險手機節點命令                                                         |
...
<AccordionGroup>
  ...
  <Accordion title="原生命令引數">
    Discord 會在省略必要引數時，對動態選項與按鈕選單使用自動完成。
    Telegram 與 Slack 會針對含有選項的命令顯示按鈕選單。
    動態選項會依目標工作階段模型解析，因此模型特定選項（例如 `/think` 層級）
    會遵循工作階段的 `/model` 覆寫。
  </Accordion>
</AccordionGroup>
...
`/tools` 回答的是執行階段問題：**此代理在這段
對話中現在可以使用什麼** — 不是靜態設定目錄。
...
結果以工作階段為範圍。變更代理、頻道、串、傳送者
授權或模型都可能改變輸出。如需編輯設定檔與覆寫，
請使用控制介面的 Tools 面板或設定介面。
...
在 Discord 上，`/model` 與 `/models` 會開啟含供應商與
模型下拉選單的互動式選擇器。選擇器會遵循 `agents.defaults.models`，
包括 `provider/*` 項目。
...
`/trace` 會顯示工作階段範圍的外掛追蹤/除錯行，而不需啟用完整詳細
...
- 使用目前工作階段作為背景情境。
- 在 Codex harness 工作階段中，會以暫時性的 Codex 側邊串執行。
- **不會**變更未來的工作階段情境。
- 不會寫入逐字記錄歷史。
...
<AccordionGroup>
  ...
  <Accordion title="引數注意事項">
    ...
    - `/new <model>` 接受模型別名、`provider/model` 或供應商名稱（模糊比對）；若沒有相符項目，文字會被視為訊息正文。
...
- **權杖/快取行**在 `/status` 中，當即時工作階段快照稀疏時，可退回使用最新的逐字記錄使用量項目。
- **執行與執行階段：**`/status` 會針對有效 sandbox 路徑回報 `Execution`，並針對執行工作階段者回報 `Runtime`：`OpenClaw Default`、`OpenAI Codex`、命令列介面後端或 ACP 後端。
- **每次回應權杖/成本：**由 `/usage off|tokens|full` 控制。
- `/model status` 與模型/驗證/端點相關，而非使用量。
