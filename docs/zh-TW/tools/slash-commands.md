...
summary: 所有可用的斜線命令、指令與行內捷徑 — 設定、路由，以及各介面的行為。
title: 斜線命令
x-i18n:
    generated_at: "2026-06-30T13:47:45Z"
    model: gpt-5.5
...
Gateway 會處理以 `/` 開頭、作為獨立訊息傳送的命令。
僅限主機的 bash 命令使用 `! <cmd>`（`/bash <cmd>` 作為別名）。

當對話繫結至 ACP 工作階段時，一般文字會路由到 ACP
harness。Gateway 管理命令仍保留在本機：`/acp ...` 一律會到達
OpenClaw 命令處理器，而只要該介面的命令處理已啟用，`/status` 加上 `/unfocus`
就會留在本機。
...
<CardGroup cols={3}>
  <Card title="命令" icon="terminal">
    由 Gateway 處理的獨立 `/...` 訊息。必須作為訊息中的
    唯一內容傳送。
  </Card>
  <Card title="指令" icon="sliders">
    `/think`、`/fast`、`/verbose`、`/trace`、`/reasoning`、`/elevated`、
    `/exec`、`/model`、`/queue` — 會在模型看到訊息前從訊息中移除。
    單獨傳送時會保存工作階段設定；與其他文字一起傳送時則作為行內提示。
  </Card>
  <Card title="行內捷徑" icon="bolt">
    `/help`、`/commands`、`/status`、`/whoami` — 會立即執行，並在模型看到剩餘文字前
    從訊息中移除。僅限已授權的傳送者。
  </Card>
</CardGroup>

<AccordionGroup>
  <Accordion title="指令行為詳細資料">
    - 指令會在模型看到訊息前從訊息中移除。
    - 在**僅含指令**的訊息中（訊息只有指令），它們會
      保存到工作階段，並回覆確認訊息。
    - 在包含其他文字的**一般聊天**訊息中，它們會作為行內提示，且
      **不會**保存工作階段設定。
    - 指令只會套用於**已授權的傳送者**。如果已設定 `commands.allowFrom`，
      它就是唯一使用的允許清單；否則授權會來自
      頻道允許清單/配對，加上 `commands.useAccessGroups`。未授權的
      傳送者會看到指令被視為純文字。
  </Accordion>
</AccordionGroup>
...
<ParamField path="commands.native" type='boolean | "auto"' default='"auto"'>
  ...
  註冊；先前已註冊的命令可能會維持可見，直到被移除。
</ParamField>

<ParamField path="commands.nativeSkills" type='boolean | "auto"' default='"auto"'>
  在支援時以原生方式註冊 skill 命令。自動：Discord/Telegram 開啟；
  Slack 關閉。可用
...
<ParamField path="commands.plugins" type="boolean" default="false">
  啟用 `/plugins`（外掛探索/狀態，加上安裝與啟用/停用）。寫入僅限擁有者。
</ParamField>
...
<ParamField path="commands.ownerDisplaySecret" type="string">
  `commands.ownerDisplay: "hash"` 時使用的 HMAC 密鑰。
</ParamField>

<ParamField path="commands.allowFrom" type="object">
  依提供者設定的命令授權允許清單。設定後，它就是
  命令與指令的**唯一**授權來源。使用 `"*"` 作為
  全域預設值；提供者專屬鍵會覆寫它。
</ParamField>
...
<AccordionGroup>
  <Accordion title="工作階段與執行">
    ...
    | `/reset [soft [message]]` | 就地重設目前工作階段。`soft` 會保留逐字稿、丟棄重複使用的命令列介面後端工作階段 id，並重新執行啟動流程 |
    | `/name <title>` | 命名或重新命名目前工作階段。省略標題可查看目前名稱和建議 |
    | `/compact [instructions]` | 壓縮工作階段情境。請參閱[壓縮](/zh-TW/concepts/compaction) |
    | `/stop` | 中止目前執行 |
    | `/session idle <duration\|off>` | 管理對話串繫結的閒置到期 |
    | `/session max-age <duration\|off>` | 管理對話串繫結的最大存活時間到期 |
    | `/export-session [path]` | 將目前工作階段匯出為 HTML。別名：`/export` |
    | `/export-trajectory [path]` | 匯出目前工作階段的 JSONL 軌跡套件組。別名：`/trajectory` |

    <Note>
      Control UI 會攔截輸入的 `/new`，以建立並切換到新的
      儀表板工作階段，除非已設定 `session.dmScope: "main"`，
      且目前父項是 agent 的主要工作階段 — 在該情況下 `/new`
      會就地重設主要工作階段。輸入的 `/reset` 仍會執行 Gateway 的
      就地重設。當你想清除已釘選的工作階段模型選擇時，請使用 `/model default`。
    </Note>
  ...
  <Accordion title="模型與執行控制">
    | 命令 | 說明 |
    | --- | --- |
    | `/think <level\|default>` | 設定思考層級，或清除工作階段覆寫。別名：`/thinking`、`/t` |
    ...
    | `/models [provider] [page] [limit=<n>\|all]` | 列出已設定/有可用驗證的提供者或模型 |
    | `/queue <mode>` | 管理作用中執行佇列行為。請參閱[佇列](/zh-TW/concepts/queue)和[佇列導向](/zh-TW/concepts/queue-steering) |
    | `/steer <message>` | 將指引注入作用中的執行。別名：`/tell`。請參閱[導向](/zh-TW/tools/steer) |

    <AccordionGroup>
      <Accordion title="verbose / trace / fast / reasoning 安全性">
        - `/verbose` 用於偵錯 — 一般使用時請保持**關閉**。
        - `/trace` 只會揭露外掛擁有的追蹤/偵錯行；一般詳細閒聊會保持關閉。
        - `/fast auto|on|off` 會保存工作階段覆寫；使用 Sessions UI 的 `inherit` 選項可清除它。
        - `/fast` 是提供者專屬的：OpenAI/Codex 會將它對應到 `service_tier=priority`；直接 Anthropic 請求則會對應到 `service_tier=auto` 或 `standard_only`。
        - `/reasoning`、`/verbose` 和 `/trace` 在群組設定中有風險 — 它們可能揭露內部推理或外掛診斷。請在群組聊天中保持關閉。

      </Accordion>
      <Accordion title="模型切換詳細資料">
        - `/model` 會立即將新模型保存到工作階段。
        - 如果 agent 閒置，下一次執行會立即使用它。
        - 如果有執行正在進行，切換會標記為待處理，並在下一個乾淨的重試點套用。
  ...
  <Accordion title="探索與狀態">
    ...
    | `/status` | 顯示執行/執行階段狀態、Gateway 與系統運作時間、外掛健康狀態，以及提供者使用量/配額 |
    | `/status plugins` | 顯示詳細外掛健康狀態：載入錯誤、隔離、頻道失敗、相依性問題、相容性通知 |
    ...
    | `/crestodian <request>` | 從擁有者私訊執行 Crestodian 設定與修復協助工具 |
    | `/tasks` | 列出目前工作階段的作用中/近期背景工作 |
    | `/context [list\|detail\|map\|json]` | 說明情境如何組合 |
    | `/whoami` | 顯示你的傳送者 id。別名：`/id` |
    | `/usage off\|tokens\|full\|reset\|cost` | 控制每則回應的使用量頁尾（`reset`/`inherit`/`clear`/`default` 會清除工作階段覆寫，以重新繼承已設定的預設值），或列印本機成本摘要 |
  </Accordion>
  ...
  <Accordion title="Skills、允許清單、核准">
    ...
    | `/btw <question>` | 在不變更工作階段情境的情況下詢問旁支問題。別名：`/side`。請參閱[BTW](/zh-TW/tools/btw) |
  </Accordion>
  ...
  <Accordion title="子代理與 ACP">
    ...
    | `/focus <target>` | 將目前的 Discord 討論串或 Telegram 主題繫結到工作階段目標 |
    | `/unfocus` | 移除目前的討論串繫結 |
    | `/agents` | 列出目前工作階段中繫結至討論串的代理 |
  </Accordion>

  <Accordion title="擁有者限定寫入與管理">
    | 命令 | 需求 | 說明 |
    | --- | --- | --- |
    | `/config show\|get\|set\|unset` | `commands.config: true` | 讀取或寫入 `openclaw.json`。擁有者限定 |
    | `/mcp show\|get\|set\|unset` | `commands.mcp: true` | 讀取或寫入 OpenClaw 管理的 MCP 伺服器設定。擁有者限定 |
    | `/plugins list\|inspect\|show\|get\|install\|enable\|disable` | `commands.plugins: true` | 檢查或變更外掛狀態。寫入為擁有者限定。別名：`/plugin` |
    | `/debug show\|set\|unset\|reset` | `commands.debug: true` | 僅限執行階段的設定覆寫。擁有者限定 |
...
停駐命令會將作用中工作階段的回覆路由切換到另一個已連結通道。
設定與疑難排解請參閱[通道停駐](/zh-TW/concepts/channel-docking)。
...
| `/codex status\|models\|threads\|resume\|compact\|review\|diagnostics\|account\|mcp\|skills` | 控制 Codex 應用程式伺服器 harness。請參閱 [Codex harness](/zh-TW/plugins/codex-harness)    |
...
<AccordionGroup>
  ...
  <Accordion title="原生命令引數">
    Discord 會在需要動態選項時使用自動完成，並在省略必要
    引數時使用按鈕選單。Telegram 和 Slack 會針對具有
    選項的命令顯示按鈕選單。動態選項會根據目標工作階段模型解析，因此像 `/think` 層級這類模型
...
`/tools` 會回答執行階段問題：**這個代理在目前這段
對話中現在可以使用什麼** — 而不是靜態設定目錄。
...
結果限於工作階段範圍。變更代理、通道、討論串、傳送者
授權或模型都可能改變輸出。若要編輯設定檔與覆寫，
請使用 Control UI Tools 面板或設定介面。
...
<Note>
  擁有者限定。預設停用 — 使用 `commands.config: true` 啟用。
</Note>
...
<Note>
  擁有者限定。預設停用 — 使用 `commands.mcp: true` 啟用。
</Note>
...
<Note>
  擁有者限定。預設停用 — 使用 `commands.debug: true` 啟用。
...
<Note>
  寫入為擁有者限定。預設停用 — 使用 `commands.plugins: true` 啟用。
</Note>
...
外掛執行階段以供新的代理回合使用。`/plugins install` 會自動重新啟動受管理的
閘道，因為外掛原始碼模組已變更。
...
`/trace` 會顯示工作階段範圍的外掛追蹤／偵錯行，而不啟用完整詳細
模式。它不會取代 `/debug`（執行階段覆寫）或 `/verbose`（一般
工具輸出）。

## `/btw` — 順帶問題

`/btw` 是關於目前工作階段情境的快速順帶問題。別名：`/side`。
...
- 使用目前工作階段作為背景情境。
- 在 Codex harness 工作階段中，會作為暫時性 Codex 側邊討論串執行。
- **不會**改變未來的工作階段情境。
- 不會寫入逐字稿歷史。

完整行為請參閱 [BTW 順帶問題](/zh-TW/tools/btw)。
...
<AccordionGroup>
  <Accordion title="各介面的工作階段範圍">
    ...
    - **`/stop`** 目標是作用中聊天工作階段，用於中止目前執行。

  </Accordion>
  <Accordion title="Slack 特定事項">
    `channels.slack.slashCommand` 支援單一 `/openclaw` 風格命令。
    使用 `commands.native: true` 時，為每個內建
    命令建立一個 Slack 斜線命令。請註冊 `/agentstatus`（而非 `/status`），因為 Slack 保留了
    `/status`。文字 `/status` 在 Slack 訊息中仍然可用。
  </Accordion>
  <Accordion title="快速路徑與行內捷徑">
    - 來自允許清單傳送者的純命令訊息會立即處理（繞過佇列 + 模型）。
    - 行內捷徑（`/help`、`/commands`、`/status`、`/whoami`）也可嵌入一般訊息中使用，且會在模型看到剩餘文字前被移除。
    - 未授權的純命令訊息會被靜默忽略；行內 `/...` 權杖會視為純文字。

  </Accordion>
  <Accordion title="引數注意事項">
    - 命令與引數之間可接受選用的 `:`（`/think: high`、`/send: on`）。
    - `/new <model>` 接受模型別名、`provider/model` 或供應商名稱（模糊比對）；若無符合項目，文字會視為訊息本文。
    - `/allowlist add|remove` 需要 `commands.config: true`，並遵循通道 `configWrites`。
...
- **Token／快取行** 在即時工作階段快照稀疏時，可在 `/status` 中退回使用最新的逐字稿用量項目。
- **執行與執行階段：**`/status` 會針對有效沙箱路徑回報 `Execution`，並針對執行工作階段的主體回報 `Runtime`：`OpenClaw Default`、`OpenAI Codex`、命令列介面後端或 ACP 後端。
- **每次回應的 token／成本：**由 `/usage off|tokens|full` 控制。
- `/model status` 是關於模型／驗證／端點，而非用量。
