...
summary: 所有可用的斜線命令、指示詞與行內快捷方式——包括設定、路由及各介面的行為。
title: 斜線命令
x-i18n:
    generated_at: "2026-07-16T12:01:59Z"
    model: gpt-5.6
...
當對話綁定至 ACP 工作階段時，一般文字會路由至 ACP
框架。閘道管理命令仍在本機處理：`/acp ...` 一律會傳至
OpenClaw 命令處理常式，而只要該介面已啟用命令處理，
...
<AccordionGroup>
  <Accordion title="指令行為詳情">
    ...
    - 指令僅適用於**已授權的傳送者**。若已設定 `commands.allowFrom`，
      ...
      頻道允許清單／配對，加上 `commands.useAccessGroups`。未經授權的
      傳送者所傳送的指令會被視為純文字。
  </Accordion>
</AccordionGroup>
...
<ParamField path="commands.text" type="boolean" default="true">
  啟用剖析聊天訊息中的 `/...`。在沒有原生命令的介面上
  （WhatsApp、WebChat、Signal、iMessage、Google Chat、Microsoft Teams），即使設為
  `false`，文字命令仍可運作。
</ParamField>

<ParamField path="commands.native" type='boolean | "auto"' default='"auto"'>
  註冊原生命令。自動模式：Discord／Telegram 啟用；Slack 停用；
  不支援原生命令的提供者會忽略此設定。可透過
  `channels.<provider>.commands.native` 針對個別頻道覆寫。在 Discord 上，`false` 會略過斜線命令
  註冊；先前註冊的命令可能會持續顯示，直到遭到移除。
</ParamField>
...
<ParamField path="commands.bash" type="boolean" default="false">
  啟用 `! <cmd>` 以執行主機殼層命令（`/bash <cmd>` 為別名）。需要
  `tools.elevated` 允許清單。
</ParamField>
...
<ParamField path="channels.<channel>.commands.enforceOwnerForCommands" type="boolean" default="false">
  針對個別頻道：要求擁有者專用命令必須使用擁有者身分。當 `true` 時，
  傳送者必須符合 `commands.ownerAllowFrom`，或具有內部 `operator.admin`
  範圍。萬用字元 `allowFrom` 項目**不足以**滿足要求。
</ParamField>
...
<ParamField path="commands.allowFrom" type="object">
  針對個別提供者的命令授權允許清單。設定後，它會成為命令與指令的
  **唯一**授權來源。使用 `"*"` 設定全域預設值；
  提供者專用鍵會覆寫它。
</ParamField>

<ParamField path="commands.useAccessGroups" type="boolean" default="true">
  未設定 `commands.allowFrom` 時，對命令強制執行允許清單／原則。
</ParamField>
...
- **產生的停駐區命令：** `src/auto-reply/commands-registry.data.ts`
...
<AccordionGroup>
  <Accordion title="工作階段與執行">
    ...
    | `/reset [soft [message]]` | 就地重設目前的工作階段。`soft` 會保留文字記錄、捨棄重複使用的命令列介面後端工作階段 ID，並重新執行啟動程序 |
    | `/name <title>` | 命名或重新命名目前的工作階段。省略標題即可查看目前名稱與建議 |
    | `/compact [instructions]` | 壓縮工作階段內容。請參閱[壓縮](/zh-TW/concepts/compaction) |
    | `/stop` | 中止目前的執行 |
    | `/session idle <duration\|off>` | 管理執行緒綁定的閒置到期時間 |
    | `/session max-age <duration\|off>` | 管理執行緒綁定的最長存續時間 |
  ...
  <Accordion title="模型與執行控制">
    ...
    | `/login [codex\|openai\|openai-codex]` | 從私人聊天或網頁 UI 工作階段配對 Codex／OpenAI 登入。僅限擁有者／管理員 |
    ...
    <AccordionGroup>
      <Accordion title="詳細輸出／追蹤／快速／推理安全性">
        - `/verbose` 用於偵錯 — 一般使用時請保持**關閉**。
        - `/trace` 只會顯示外掛所擁有的追蹤／偵錯行；一般的詳細訊息仍會保持關閉。
        - `/fast auto|on|off` 會保存工作階段覆寫；請使用工作階段 UI 的 `inherit` 選項來清除。
        - `/fast` 依提供者而異：OpenAI／Codex 會將其對應至 `service_tier=priority`；直接 Anthropic 要求則會將其對應至 `service_tier=auto` 或 `standard_only`。
  ...
  <Accordion title="探索與狀態">
    ...
    | `/status plugins` | 顯示詳細的外掛健康狀態：載入錯誤、隔離、頻道外掛失敗、相依性問題、相容性通知。需要 `commands.plugins: true` |
    ...
    | `/usage off\|tokens\|full\|reset\|cost` | 控制每次回應的用量頁尾（`reset`／`inherit`／`clear`／`default` 會清除工作階段覆寫，以重新繼承已設定的預設值），或列印本機費用摘要 |
  </Accordion>
  ...
  <Accordion title="Skills、允許清單與核准">
    ...
    | `/btw <question>` | 提出附帶問題而不變更工作階段脈絡。別名：`/side`。請參閱 [BTW](/zh-TW/tools/btw) |
  </Accordion>

  <Accordion title="子代理與 ACP">
    | 命令 | 說明 |
    | --- | --- |
    | `/subagents list\|log\|info` | 檢查目前工作階段的子代理執行情況 |
...
停駐命令會將作用中工作階段的回覆路由切換至另一個已連結的頻道。
如需設定與疑難排解資訊，請參閱[頻道停駐](/zh-TW/concepts/channel-docking)。
...
| `/codex <action> ...`                                   | 繫結、引導並檢查 Codex app-server 測試框架（狀態、討論串、繼續、模型、快速模式、權限、壓縮、審查、MCP、Skills 等）。請參閱 [Codex 測試框架](/zh-TW/plugins/codex-harness) |
...
<AccordionGroup>
  ...
  <Accordion title="原生命令引數">
    略過必要引數時，Discord 會對動態選項使用自動完成與按鈕選單。
    Telegram 與 Slack 會為含有選項的命令顯示按鈕選單。
    動態選項會依目標工作階段模型解析，因此 `/think` 層級等模型
...
在 Discord 上，`/model` 與 `/models` 會開啟含有提供者與
模型下拉式選單的互動式選擇器。此選擇器會遵循 `agents.defaults.models`，包括
`provider/*` 項目。

## `/config`：寫入磁碟設定

<Note>
  僅限擁有者。預設停用 — 請使用 `commands.config: true` 啟用。
</Note>
...
設定會在寫入前驗證。無效的變更會遭拒絕。`/config`
更新會跨重新啟動保留。

## `/mcp`：MCP 伺服器設定

<Note>
  僅限擁有者。預設停用 — 請使用 `commands.mcp: true` 啟用。
</Note>
...
`/mcp` 會將設定儲存在 OpenClaw 設定中，而非嵌入式代理的專案設定。
`/mcp show` 會遮蔽含有認證資訊的欄位、可辨識的認證資訊旗標
值，以及已知具有機密格式的引數。從群組執行時，
設定會私下傳送給擁有者；若沒有可用的擁有者私人路由，
命令會以封閉方式失敗，並要求擁有者從直接
聊天重試。

## `/debug`：僅限執行階段的覆寫

<Note>
  僅限擁有者。預設停用 — 請使用 `commands.debug: true` 啟用。
...
<Note>
  寫入僅限擁有者。預設停用 — 請使用 `commands.plugins: true` 啟用。
</Note>
...
外掛執行階段，以供新的代理回合使用。由於外掛原始碼模組已變更，`/plugins install` 會自動重新啟動受管理的
閘道。受信任的 ClawHub
與官方目錄安裝不需要額外確認。任意 npm、
git、封存檔、`npm-pack:` 與本機路徑來源會顯示來源警告，且
在你審查來源後，必須於末尾加上 `--force`。此旗標表示確認
該來源，並允許取代現有安裝；它不會略過
`security.installPolicy` 或安裝程式的安全性檢查。帶有
風險警告的 ClawHub 版本仍需要另一個僅限 shell 的
`--acknowledge-clawhub-risk` 旗標。市集、連結與固定版本安裝也
仍僅限 shell。
...
`/trace` 可顯示以工作階段為範圍的外掛追蹤／偵錯行，而不需要完整詳細
模式。它不會取代 `/debug`（執行階段覆寫）或 `/verbose`（一般
工具輸出）。

## `/btw`：附帶問題

`/btw` 是針對目前工作階段脈絡的快速附帶問題。別名：`/side`。

```text
/btw 我們目前正在做什麼？
/side 主要執行繼續進行期間發生了哪些變更？
```

與一般訊息不同：

- 使用目前工作階段作為背景脈絡。
- 在 Codex 測試框架工作階段中，會作為暫時性的 Codex 附帶討論串執行。
- **不會**變更未來的工作階段脈絡。
- 不會寫入逐字稿歷程。
...
<AccordionGroup>
  ...
  <Accordion title="Slack 特定事項">
    ...
    請註冊 `/agentstatus`（而非 `/status`），因為 Slack 保留
    `/status`。文字 `/status` 在 Slack 訊息中仍然有效。
  </Accordion>
  <Accordion title="快速路徑與行內捷徑">
    - 來自允許清單中傳送者的純命令訊息會立即處理（略過佇列和模型）。
    - 行內捷徑（`/help`、`/commands`、`/status`、`/whoami`）也可嵌入一般訊息中使用，並會在模型看到其餘文字前遭到移除。
    - 未經授權的純命令訊息會被無聲忽略；行內 `/...` 權杖會視為純文字。
...
- 當即時工作階段快照的資料不足時，`/status` 中的**權杖／快取行**可退回使用最新的逐字稿用量項目。
- **執行與執行階段：**`/status` 會以 `Execution` 回報有效的沙箱路徑，並以 `Runtime` 回報工作階段的執行者：`OpenClaw Default`、`OpenAI Codex`、命令列介面後端或 ACP 後端。
...
<CardGroup cols={2}>
  ...
  <Card title="順帶一問" href="/zh-TW/tools/btw" icon="comments">
    在不變更工作階段內容的情況下提出旁支問題。
  </Card>
  <Card title="引導" href="/zh-TW/tools/steer" icon="compass">
    使用 `/steer` 在代理程式執行期間加以引導。
  </Card>
</CardGroup>
