...
title: 文件地圖
x-i18n:
    generated_at: "2026-06-30T22:05:48Z"
    model: gpt-5.5
...
此檔案是由 `docs/**/*.md` 和 `docs/**/*.mdx` 的標題產生，用來協助代理程式瀏覽文件樹。
...
- 標題：
  ...
  - H3: 步驟 2：取得憑證
  - H3: 步驟 3：設定 Messaging Endpoint
  - H3: 步驟 4：啟用 Teams 頻道
  - H3: 步驟 5：建立 Teams App Manifest
  - H3: 步驟 6：設定 OpenClaw
  - H3: 步驟 7：執行閘道
  - H2: 聯合驗證（憑證加受控身分識別）
  - H3: 選項 A：憑證式驗證
  ...
  - H2: 成員資訊動作
  - H2: 歷史上下文
  - H2: 目前的 Teams RSC 權限（manifest）
  - H2: Teams manifest 範例（已修訂）
  - H3: Manifest 注意事項（必備欄位）
  - H3: 更新現有應用程式
  - H2: 能力：僅 RSC 與 Graph
...
- 標題：
  ...
  - H2: 這是什麼
  - H2: 設定（快速路徑）
  - H3: 1) 建立機器人 Token（Zalo Bot Platform）
...
- 標題：
  ...
  - H2: Paperclip / openclawgateway 初次執行核准
...
- 標題：
  ...
  - H3: 角色
  - H3: 範圍（操作者）
  - H3: Caps/commands/permissions（節點）
  - H2: 存在狀態
  - H3: 節點背景存活事件
  - H2: 廣播事件範圍限定
  - H2: 常見 RPC 方法家族
...
- 標題：
  ...
  - H2：Live：Codex app-server 測試框架煙霧測試
  - H3：建議的 Live 配方
  - H2：Live：模型矩陣（我們涵蓋的範圍）
  - H3：現代煙霧測試集（工具呼叫 + 圖片）
...
- 標題：
  ...
  - H2：我該執行哪個套件？
  - H2：Live（觸及網路）測試
  - H2：Docker 執行器（選用的「在 Linux 可運作」檢查）
  - H2：文件健全性
  - H2：離線回歸（CI 安全）
  - H2：代理可靠性評估（Skills）
  - H2：合約測試（外掛與頻道形狀）
...
- 標題：
  ...
  - H2：自動偵測（預設）
  - H2：設定範例
  - H3：供應商 + 命令列介面 fallback（OpenAI + Whisper 命令列介面）
  - H3：僅供應商並使用範圍控管
  - H3：僅供應商（Deepgram）
...
- 標題：
  ...
  - H3：執行位置
  - H3：啟動節點主機（前景）
  - H3：透過 SSH 通道的遠端閘道（loopback bind）
  - H3：啟動節點主機（服務）
  - H3：配對 + 命名
  - H3：將命令加入允許清單
  - H3：將 exec 指向節點
...
- 標題：
  ...
  - H2：期望行為
  - H2：設計限制
  - H3：Codex app-server 仍是原生 thread 狀態的權威來源
  - H3：Context engine 組裝必須投射到 Codex 輸入
  - H3：Prompt-cache 穩定性很重要
  - H3：Runtime 選擇語義不變
  - H2：實作計畫
  - H3：1. 匯出或搬移可重用的 context-engine attempt helpers
  - H3：2. 新增 Codex context projection helper
  - H3：3. 在 Codex thread 啟動前接入 bootstrap
...
- 標題：
  ...
  - H2：需求
  - H2：快速開始（配對 + 連線）
  - H2：官方 build 的 relay-backed push
  - H2：背景 alive beacons
  - H2：驗證與信任流程
  - H2：探索路徑
  - H3：Bonjour（LAN）
...
- 標題：
  ...
  - H2：發佈
  - H2：疑難排解
  - H3：找不到 plugin entry：./dist/index.js
  - H3：plugin entry 未公開 defineToolPlugin 中繼資料
...
- 標題：
  ...
  - H3：第 0 階段：凍結邊界
  - H3：第 1 階段：完成全域控制平面
  - H3：第 2 階段：導入每個代理的資料庫
  - H3：第 3 階段：替換工作階段儲存 API
  - H3：第 4 階段：移動文字記錄、ACP 串流、軌跡和 VFS
...
- 標題：
  - H2：主要旋鈕
  - H3：cacheRetention（全域預設、模型和每個代理）
...
- 標題：
  ...
  - H3：第 2 層：驗證 WSL2 是否能連到該 Windows 端點
