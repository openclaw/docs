...
title: 文件地圖
x-i18n:
    generated_at: "2026-07-01T07:51:13Z"
    model: gpt-5.5
...
此檔案是由 `docs/**/*.md` 和 `docs/**/*.mdx` 標題產生，用來幫助代理導覽文件樹。
...
- 標題：
  ...
  - H2：設定鍵（概觀）
  - H3：安全的允許清單編輯
  - H2：「Model is not allowed」（以及回覆停止的原因）
  - H2：在聊天中切換模型（/model）
  - H2：命令列介面命令
  - H3：models list
  - H3：models status
...
- 標題：
  - H2: 建議預設值（大多數使用者）
  - H2: macOS VM 選項
  - H3: 你 Apple Silicon Mac 上的本機 VM（Lume）
  - H3: 託管 Mac 供應者（雲端）
...
- 標題：
  ...
  - H2: 自動偵測（預設）
  - H2: 設定範例
  - H3: Provider + 命令列介面備援（OpenAI + Whisper 命令列介面）
  - H3: 僅 Provider，搭配 scope gating
...
- 標題：
  ...
  - H3: 權限
  - H3: Android 前景需求
  - H3: Android 指令（透過閘道 node.invoke）
  - H3: Payload 保護
  - H2: macOS app
  - H3: 使用者設定（預設關閉）
...
- 標題：
  ...
  - H2: 期望行為
  - H2: 設計限制
  - H3: Codex app-server 仍是原生 thread 狀態的標準來源
  - H3: Context engine 組裝必須投射到 Codex 輸入
  - H3: Prompt-cache 穩定性很重要
  - H3: Runtime 選擇語意不變
  - H2: 實作計畫
  - H3: 1. 匯出或移動可重用的 context-engine attempt 輔助工具
...
- 標題：
  ...
  - H3: 1) 啟動閘道
  - H3: 2) 驗證探索（選用）
  - H4: 透過 unicast DNS-SD 的 Tailnet（維也納 ⇄ 倫敦）探索
  - H3: 3) 從 Android 連線
...
- 標題：
  ...
  - H2：卡片執行與任務
  - H2：Agent 協調
  - H3：Dispatch worker 選擇
  - H3：Worker prompt 與生命週期
  - H3：Dispatch 進入點
  - H2：命令列介面與 slash command
  - H2：Session 生命週期同步
  - H2：Dashboard 工作流程
  - H2：權限
  - H2：設定
  - H2：疑難排解
  - H3：索引標籤顯示 Workboard 無法使用
  - H3：卡片未儲存
  - H3：啟動卡片未開啟預期的 session
  - H3：Dispatch 未啟動 worker
  - H2：相關
...
- 標題：
  ...
  - H3: 第 2 層：驗證 WSL2 能連到該 Windows 端點
