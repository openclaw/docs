...
此檔案是由 `docs/**/*.md` 和 `docs/**/*.mdx` 標題產生，用來協助代理瀏覽文件樹。
...
- 標題：
  ...
  - H3：可重複使用的寄件者群組
  - H3：狀態所在位置
  - H2：2) 節點裝置配對（iOS/Android/macOS/無周邊節點）
...
- 標題：
  ...
  - H3: 其他隨附提供者外掛
  - H4: 值得了解的特性
  - H2: 透過 models.providers 使用提供者（自訂/base URL）
  - H3: Moonshot AI（Kimi）
  - H3: Kimi Coding
...
- 標題:
  ...
  - H3: 多平台設定
  - H3: 可信節點網路自動核准
  - H3: 安全 DM 模式（共享收件匣 / 多使用者 DM）
  - H3: Anthropic API 金鑰 + MiniMax 後援
  - H3: 工作機器人（限制存取）
  - H3: 僅限本機模型
  - H2: 提示
  - H2: 相關
...
- 標題:
  ...
  - H2: 工具政策：哪些工具存在/可呼叫
  - H3: 工具群組（速記）
  - H2: Elevated：僅 exec 的「在主機上執行」
  - H2: 常見「sandbox jail」修正
  - H3: 「工具 X 被 sandbox 工具政策封鎖」
  - H3: 「我以為這是 main，為什麼被 sandboxed？」
  - H2: 相關
...
- 標題:
  ...
  - H3: 記錄與逐字稿
  - H2: 安全基準（複製/貼上）
  - H3: 獨立號碼（WhatsApp、Signal、Telegram）
  - H2: 事件回應
  - H3: 圍堵
  - H3: 輪換（若密鑰外洩，假設已遭入侵）
  - H3: 稽核
...
- 標題:
  ...
  - H3: 驗證詳細代碼快速對照
  - H2: 閘道服務未執行
  - H2: macOS 閘道靜默停止回應，接著在你觸碰儀表板時恢復
  - H2: 閘道在高記憶體使用期間結束
  - H2: 閘道拒絕無效設定
  - H2: 閘道探測警告
  - H2: 頻道已連線，訊息未流動
...
- 標題:
  ...
  - H2: 即時：ACP 繫結煙霧測試（/acp spawn ... --bind here）
...
- 標題：
  - H2: 建議預設值（多數使用者）
  - H2: macOS VM 選項
  - H3: 在你的 Apple Silicon Mac 上執行本機 VM（Lume）
  - H3: 託管 Mac 提供者（雲端）
  - H2: 快速路徑（Lume，熟練使用者）
  - H2: 你需要什麼（Lume）
  - H2: 1) 安裝 Lume
...
- 標題：
  ...
  - H2: 自動偵測（預設）
  - H2: 設定範例
  - H3: 提供者 + 命令列介面備援（OpenAI + Whisper CLI）
  - H3: 僅提供者且使用範圍門控
  - H3: 僅提供者（Deepgram）
...
- 標題：
  ...
  - H3：Runtime 選擇語意不變
  - H2：實作計畫
  - H3：1. 匯出或遷移可重用的 context-engine 嘗試輔助工具
  - H3：2. 新增 Codex context projection 輔助工具
  - H3：3. 在 Codex 對話串啟動前接入 bootstrap
  - H3：4. 在 thread/start / thread/resume 和 turn/start 前接入 assemble
  - H3：5. 保留 prompt-cache 穩定格式
  - H3：6. 在 transcript mirroring 後接入 post-turn
  - H3：7. 正規化 usage 和 prompt-cache runtime context
  - H3：8. 壓縮政策
  - H4：/compact 和明確 OpenClaw 壓縮
...
- 標題：
  - H1：記錄（macOS）
  - H2：輪替診斷檔案記錄（偵錯窗格）
  - H2：macOS 上的 unified logging 私人資料
...
- 標題：
  - H2：穩定權限需求
  - H2：Node 和命令列介面 runtime 的輔助使用授權
  - H2：提示消失時的復原檢查清單
  - H2：檔案和資料夾權限（Desktop/Documents/Downloads）
  - H2：相關
...
## 外掛/sdk-channel-turn.md

- 路由：/plugins/sdk-channel-turn
- 標題：無

## 外掛/sdk-entrypoints.md
...
- 標題:
  ...
  - H3: 命令列介面工具框架提供者（Claude Code、Gemini CLI）
  - H3: 其他提供者
  - H2: 系統提示快取邊界
  - H2: OpenClaw 快取穩定性防護
  - H2: 調校模式
...
- 標題:
  ...
  - H3: macOS IPC 流程
  - H2: 常見問題
  - H3: accountId 和 threadId 何時會用於核准目標？
