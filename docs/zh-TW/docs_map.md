...
summary: OpenClaw 文件頁面的已產生標題對應表
title: 文件對照表
x-i18n:
    generated_at: "2026-07-04T06:21:39Z"
    model: gpt-5.5
...
此檔案是從 `docs/**/*.md` 與 `docs/**/*.mdx` 標題產生，用來協助代理程式瀏覽文件樹。
...
- 標題：
  ...
  - H2: 這是什麼
  - H2: 設定（快速路徑）
  - H3: 1) 建立 Bot 權杖（Zalo Bot Platform）
...
- 標題：
  ...
  - H3：/context map
  - H2：哪些內容會計入脈絡視窗
  - H2：OpenClaw 如何建構系統提示詞
  - H2：注入的工作區檔案（專案脈絡）
  - H2：Skills：注入與按需載入
  - H2：工具：有兩種成本
  - H2：命令、指令與「行內捷徑」
  - H2：工作階段、壓縮與修剪（會保留的內容）
  - H2：/context 實際回報的內容
  - H2：相關
...
- 標題：
  - H2: 訊息信封（預設使用本地）
  - H3: 範例
  - H2: 系統 prompt：目前日期與時間
  - H2: 系統事件行（預設使用本地）
  - H3: 設定使用者時區 + 格式
  - H2: 時間格式偵測（自動）
  - H2: 工具 payload + 連接器（原始提供者時間 + 正規化欄位）
  - H2: 相關文件
...
- 標題：
  ...
  - H3: 每個代理的 bootstrap 設定檔覆寫
  - H3: agents.defaults.bootstrapPromptTruncationWarning
  - H3: Context budget 擁有權對照表
...
- 標題:
  ...
  - H2: Rescue-Bot 快速開始
  - H2: 這為何可行
  - H2: `--profile rescue onboard` 變更了什麼
  - H2: 一般多閘道設定
  - H2: 隔離檢查清單
  - H2: 連接埠對應（衍生）
  - H2: 瀏覽器/CDP 備註（常見陷阱）
...
- 標題:
  ...
  - H2: 範例
  - H3: 最小遠端設定
  - H3: 使用 GPU 的 mirror 模式
  - H3: 搭配自訂閘道的每 Agent OpenShell
  - H2: 生命週期管理
  - H3: 何時重新建立
  - H2: 安全強化
  - H2: 目前限制
...
- 標題：
  ...
  - H2：工具政策：哪些工具存在/可被呼叫
  - H3：工具群組（簡寫）
  - H2：提權：僅限 exec 的「在主機上執行」
  - H2：常見「沙箱限制」修正
  - H3：「工具 X 被沙箱工具政策封鎖」
  - H3：「我以為這是 main，為什麼它被沙箱化？」
  - H2：相關
...
- 標題：
  ...
  - H3：範例：完整存取（無沙箱）
  - H3：範例：唯讀工具 + 唯讀工作區
  - H3：範例：無檔案系統/Shell 存取（允許提供者訊息傳遞）
  - H2：事件回應
  - H3：圍堵
  - H3：輪替（若秘密外洩，假設已遭入侵）
  - H3：稽核
...
- 標題：
  ...
  - H3：驗證詳細代碼快速對照
  - H2：閘道服務未執行
  - H2：macOS 閘道無聲停止回應，接著在你觸碰儀表板時恢復
  - H2：閘道在高記憶體使用期間結束
  - H2：閘道拒絕無效設定
  - H2：閘道探測警告
  - H2：頻道已連線，但訊息未流動
...
- 標題：
  ...
  - H2: 即時：模型煙霧測試（profile keys）
  - H3: 第 1 層：直接模型完成（無閘道）
  - H3: 第 2 層：閘道 + dev agent 煙霧測試（「@openclaw」實際執行的內容）
  - H2: 即時：命令列介面後端煙霧測試（Claude、Gemini 或其他本機命令列介面）
  - H2: 即時：APNs HTTP/2 proxy 可連線性
  - H2: 即時：ACP bind 煙霧測試（/acp spawn ... --bind here）
  - H2: 即時：Codex app-server harness 煙霧測試
  - H3: 建議的即時 recipes
  - H2: 即時：模型矩陣（我們涵蓋的範圍）
  - H3: 現代煙霧測試集（工具呼叫 + 圖片）
  - H3: 基準：工具呼叫（Read + 可選 Exec）
  - H3: 視覺：圖片傳送（附件 → 多模態訊息）
  - H3: Aggregators / alternate gateways
  - H2: 憑證（絕不提交）
  - H2: Deepgram 即時（音訊轉錄）
  - H2: BytePlus coding plan 即時
  - H2: ComfyUI workflow media 即時
  - H2: 圖片生成即時
  - H2: 音樂生成即時
  - H2: 影片生成即時
  - H2: 媒體即時 harness
...
- 標題：
  ...
  - H2: 測試暫存目錄
  - H2: QA 專用 runners
  - H3: 透過 Convex 共享 Telegram 憑證（v1）
  - H3: 將 channel 加入 QA
  - H2: 測試套件（哪裡會跑什麼）
  - H3: 單元 / 整合（預設）
  - H3: 穩定性（閘道）
  ...
  - H2: 我應該執行哪個套件？
  - H2: 即時（會接觸網路）測試
  - H2: Docker runners（可選的「可在 Linux 運作」檢查）
  - H2: 文件健全性
  - H2: 離線迴歸（CI 安全）
  - H2: 代理可靠性評估（Skills）
  - H2: 契約測試（外掛和 channel shape）
  - H3: 命令
...
- 標題：
  ...
  - H2: 2) 安裝先決條件（在 VM 上）
  - H2: 3) 安裝 OpenClaw
  - H2: 4) 設定 nginx，將 OpenClaw proxy 到 port 8000
...
- 標題：
  ...
  - H2: 建議：安裝程式 script
  - H2: 替代安裝方法
  - H3: 本機 prefix installer（install-cli.sh）
  - H3: npm、pnpm 或 bun
  - H3: 從原始碼
  - H3: 從 GitHub main checkout 安裝
  - H3: 容器與 package managers
  - H2: 驗證安裝
  - H2: 託管與部署
  - H2: 更新、遷移或解除安裝
...
- 標題：
  ...
  - H2: 6) 安裝 OpenClaw
  - H2: 7) 設定 channels
  - H2: 8) 以 headless 方式執行 VM
  - H2: Bonus：iMessage 整合
  - H2: 儲存 golden image
  - H2: 24/7 執行
  - H2: 疑難排解
  - H2: 相關文件
...
- 標題：
  ...
  - H2: Systemd（Quadlet，可選）
  - H2: Config、env 與 storage
  - H2: 實用命令
  - H2: 疑難排解
  - H2: 相關
...
- 標題：
  ...
  - H2：自動偵測（預設）
  - H2：設定範例
  - H3：提供者 + 命令列介面 fallback（OpenAI + Whisper CLI）
  - H3：僅提供者且使用 scope gating
  - H3：僅提供者（Deepgram）
...
- 標題：
  ...
  - H2：期望行為
  - H2：設計限制
  - H3：Codex app-server 仍是原生 thread state 的 canonical 來源
  - H3：Context engine assembly 必須投射到 Codex inputs
  - H3：Prompt-cache 穩定性很重要
  - H3：Runtime selection semantics 不變
  - H2：實作計畫
  - H3：1. 匯出或重新定位可重用的 context-engine attempt helpers
  - H3：2. 新增 Codex context projection helper
  - H3：3. 在 Codex thread startup 前串接 bootstrap
  - H3：4. 在 thread/start / thread/resume 與 turn/start 前串接 assemble
  - H3：5. 保留 prompt-cache stable formatting
...
- 標題：
  ...
  - H3：6) 聊天 + 歷史記錄
  - H3：7) Canvas + 相機
  - H4：閘道 Canvas Host（建議用於 web content）
...
- 標題:
  ...
  - H2: 成本估算（顯示時）
  - H2: 快取 TTL 與修剪影響
  - H3: 範例：透過心跳偵測讓 1 小時快取保持暖機
  - H3: 範例：採用每代理快取策略的混合流量
  - H3: Anthropic 1M 情境
  - H2: 降低 token 壓力的提示
  - H2: 相關
...
- 標題：
  ...
  - H3: 解法 2：搭配 Attach-Only Mode 使用 Snap Chromium
  - H3: 驗證瀏覽器可運作
  - H3: 設定參考
...
- 標題：
  ...
  - H3: 第 2 層：驗證 WSL2 可以連到該 Windows 端點
...
- 標題：
  ...
  - H2: 工作階段覆寫（/exec）
  - H2: 授權模型
  - H2: Exec 核准（companion app / node host）
  - H2: 允許清單 + 安全執行檔
  - H2: 範例
  - H2: applypatch
  - H2: 相關
