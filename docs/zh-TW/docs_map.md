...
- 標題：
  ...
  - H2: 安全性預設值
  - H2: 存取控制
  - H3: 常見陷阱：allowFrom 用於私訊，而不是頻道
  - H2: 回覆觸發（提及）
  - H2: 安全性注意事項（建議用於公開頻道）
  - H3: 頻道中每個人使用相同工具
  - H3: 每位寄件者使用不同工具（擁有者獲得更多權限）
  - H2: NickServ
  - H2: 環境變數
  - H2: 疑難排解
  - H2: 相關
...
- 標題：
  ...
  - H2：元件與流程
  - H3：閘道（daemon）
  - H3：用戶端（mac app / 命令列介面 / web admin）
...
- 標題：
  ...
  - H2：標籤政策
  - H2：PromQL 配方
  - H2：在 Prometheus 和 OpenTelemetry 匯出之間選擇
  - H2：疑難排解
  - H2：相關內容
...
- 標題：
  ...
  - H3：範例：完整存取（無沙箱）
  - H3：範例：唯讀工具 + 唯讀工作區
  - H3：範例：無檔案系統/shell 存取（允許供應者訊息傳遞）
  - H2：事件回應
  - H3：圍堵
  - H3：輪換（若秘密外洩，假設已遭入侵）
  - H3：稽核
...
- 標題：
  ...
  - H2：我應該執行哪個套件？
  - H2：即時（觸及網路）測試
  - H2：Docker 執行器（可選的「可在 Linux 運作」檢查）
  - H2：文件健全性
  - H2：離線迴歸（CI 安全）
  - H2：代理可靠性評估（Skills）
  - H2：合約測試（外掛與通道形狀）
...
- 標題：
  ...
  - H2：自動偵測（預設）
  - H2：設定範例
  - H3：提供者 + 命令列介面後援（OpenAI + Whisper 命令列介面）
  - H3：僅提供者且使用範圍控管
  - H3：僅提供者（Deepgram）
...
- 標題：
  - H2: 主要旋鈕
  - H3: cacheRetention（全域預設值、模型與每個代理）
...
- 標題：
  - H2: 全域規則：執行階段脈絡不是使用者 transcript
  - H2: 這在哪裡執行
  - H2: 全域規則：圖片清理
  - H2: 全域規則：格式錯誤的工具呼叫
  - H2: 全域規則：未完成的僅推理回合
...
- 標題：
  ...
  - H2: 雙手機設定（建議）
  - H2: 5 分鐘快速開始
  - H2: 給 agent 一個工作區（AGENTS）
  - H2: 將它變成「assistant」的設定
  - H2: 工作階段與記憶
  - H2: 心跳偵測（主動模式）
  - H2: 媒體輸入與輸出
  - H2: 操作檢查清單
