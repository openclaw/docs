...
此檔案是從 `docs/**/*.md` 和 `docs/**/*.mdx` 標題產生，用來協助代理程式瀏覽文件樹。
...
- 標題：
  ...
  - H2: 私有 API 動作
  - H2: 設定寫入
  - H2: 合併分割傳送的私訊（命令 + URL 在同一組合中）
  - H3: 情境與代理程式看到的內容
  - H2: Bridge 或閘道重新啟動後的傳入復原
  - H3: 操作者可見訊號
  - H3: 遷移
  - H2: 疑難排解
  - H2: 設定參考指標
...
- 標題：
  ...
  - H2: 使用案例
  - H3: 更新 Docker image 之後
  - H3: 變更沙箱設定之後
  - H3: 變更 SSH 目標或 SSH 驗證素材之後
  - H3: 變更 OpenShell 來源、政策或模式之後
  - H3: 變更 setupCommand 之後
  - H3: 僅限特定 Agent
  - H2: 為何需要這個
  - H2: Registry 遷移
  - H2: 設定
...
- 標題：
  ...
  - H3：其他隨附提供者外掛
  - H4：值得知道的特殊情況
  - H2：透過 models.providers 的提供者（自訂/基礎 URL）
  - H3：Moonshot AI（Kimi）
  - H3：Kimi coding
...
- 路由: /gateway/bonjour
- 標題:
  - H2: 透過 Tailscale 使用廣域 Bonjour（單點傳播 DNS-SD）
  - H3: 閘道設定（建議）
...
- 標題：
  ...
  - H2: Rescue-Bot 快速入門
  - H2: 為何這樣可行
  - H2: `--profile rescue onboard` 會變更的內容
  - H2: 一般多閘道設定
  - H2: 隔離檢查清單
  - H2: 連接埠對應（衍生）
...
- 標題：
  ...
  - H2：macOS app 遠端模式
  - H2：安全規則（遠端/VPN）
  - H3：macOS：透過 LaunchAgent 持久化 SSH 通道
  - H4：步驟 1：新增 SSH 設定
...
- 標題：
  ...
  - H2：上游 403 封鎖回應
  - H2：本機 OpenAI 相容後端直接探測通過但代理執行失敗
  - H2：沒有回覆
  - H2：儀表板控制 UI 連線能力
  - H3：驗證詳細代碼快速對照
  - H2：閘道服務未執行
  - H2：macOS 閘道默默停止回應，接著在你碰觸儀表板時恢復
  - H2：閘道在高記憶體使用期間退出
  - H2：閘道拒絕無效設定
  - H2：閘道探測警告
  - H2：頻道已連線，訊息未流動
...
- 標題：
  ...
  - H2：自動偵測（預設）
  - H2：設定範例
  - H3：供應商 + 命令列介面後備（OpenAI + Whisper CLI）
  - H3：僅供應商並使用範圍管控
  - H3：僅供應商（Deepgram）
...
- 標題：
  ...
  - H3：3. 在 Codex 執行緒啟動前接線 bootstrap
  - H3：4. 在 thread/start / thread/resume 與 turn/start 前接線 assemble
  - H3：5. 保留提示快取穩定格式
  - H3：6. 在逐字稿鏡像後接線 post-turn
  - H3：7. 正規化使用量與提示快取執行階段 context
  - H3：8. 壓縮政策
...
- 標題：
  ...
  - H3：1) 啟動閘道
  - H3：2) 驗證探索（選用）
  - H4：透過單播 DNS-SD 進行 Tailnet（Vienna ⇄ London）探索
  - H3：3) 從 Android 連線
  - H3：線上狀態存活信標
  - H3：4) 核准配對（命令列介面）
  - H3：5) 驗證節點已連線
...
- 標題：
  ...
  - H3：解決方案 2：搭配僅附加模式使用 Snap Chromium
  - H3：驗證瀏覽器可正常運作
  - H3：設定參考
