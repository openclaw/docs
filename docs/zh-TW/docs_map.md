...
title: 文件地圖
x-i18n:
    generated_at: "2026-07-03T17:13:01Z"
    model: gpt-5.5
...
此檔案由 `docs/**/*.md` 與 `docs/**/*.mdx` 的標題產生，用來協助代理程式導覽文件樹。
...
- 標題：
  ...
  - H2：傳送 SMS
  - H2：驗證設定
  - H3：來自 macOS iMessage/SMS 的端對端測試
  - H2：網路鉤子安全性
  - H2：多帳號設定
  - H2：疑難排解
...
- 標題：
  ...
  - H2: 提示組裝 + 系統提示
  - H2: Hook 點（你可以攔截的位置）
  - H3: 內部 hooks（Gateway hooks）
  - H3: 外掛 hooks（代理 + gateway 生命週期）
  - H2: 串流 + 部分回覆
  - H2: 工具執行 + 訊息工具
  - H2: 回覆塑形 + 抑制
  - H2: 壓縮 + 重試
...
- 標題：
  ...
  - H3：其他內建提供者外掛
  - H4：值得知道的特性
  - H2：透過 models.providers 使用提供者（自訂/基底 URL）
  - H3：Moonshot AI（Kimi）
  - H3：Kimi coding
...
- 路由：/gateway/bonjour
- 標題：
  - H2：透過 Tailscale 的廣域 Bonjour（單播 DNS-SD）
  - H3：閘道設定（建議）
...
- 標題：
  ...
  - H2：macOS app 遠端模式
  - H2：安全規則（遠端/VPN）
  - H3：macOS：透過 LaunchAgent 的持久 SSH 通道
  - H4：步驟 1：加入 SSH 設定
...
- 標題：
  ...
  - H2: 即時：模型冒煙測試（設定檔鍵）
  - H3: 第 1 層：直接模型補全（無閘道）
  - H3: 第 2 層：閘道 + 開發代理冒煙測試（「@openclaw」實際做的事）
...
- 標題：
  ...
  - H3：執行階段選擇語意不變
  - H2：實作計畫
  - H3：1. 匯出或重新放置可重用的 context-engine attempt helper
  - H3：2. 新增 Codex context projection helper
  - H3：3. 在 Codex thread 啟動前接線 bootstrap
  - H3：4. 在 thread/start / thread/resume 與 turn/start 前接線 assemble
  - H3：5. 保留 prompt-cache 穩定格式
  - H3：6. 在 transcript mirroring 後接線 post-turn
...
- 標題：
  ...
  - H3：1) 啟動閘道
  - H3：2) 驗證探索（選用）
  - H4：Tailnet（Vienna ⇄ London）透過 unicast DNS-SD 探索
  - H3：3) 從 Android 連線
...
- 標題:
  ...
  - H3: 第 0 階段：凍結邊界
  - H3: 第 1 階段：完成全域控制平面
  - H3: 第 2 階段：引入每個代理程式專屬資料庫
  - H3: 第 3 階段：取代工作階段儲存 API
  - H3: 第 4 階段：移動轉錄、ACP 串流、軌跡與 VFS
...
- 標題:
  - H2: 主要控制項
  - H3: cacheRetention（全域預設值、模型與每個代理程式）
...
- 標題：
  ...
  - H2：成本估算（顯示時）
  - H2：快取 TTL 與修剪影響
  - H3：範例：以心跳偵測維持 1h 快取溫熱
  - H3：範例：混合流量搭配每個代理的快取策略
  - H3：Anthropic 1M 脈絡
  - H2：降低 Token 壓力的提示
  - H2：相關
...
- 標題：
  ...
  - H3: 解法 2：搭配僅附加模式使用 Snap Chromium
  - H3: 驗證瀏覽器可運作
  - H3: 設定參考
...
- 標題：
  ...
  - H3: 第 2 層：驗證 WSL2 可連到該 Windows 端點
