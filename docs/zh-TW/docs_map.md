...
此檔案由 `docs/**/*.md` 和 `docs/**/*.mdx` 標題產生，用來協助代理導覽文件樹。
...
- 標題：
  ...
  - H3：其他 bundled 供應者外掛
  - H4：值得知道的特殊情況
  - H2：透過 models.providers 的供應者（自訂/base URL）
  - H3：Moonshot AI（Kimi）
  - H3：Kimi coding
...
- 標題：
  ...
  - H2：組態鍵（概觀）
  - H3：安全編輯允許清單
  - H2：「Model is not allowed」（以及為何回覆停止）
  - H2：在聊天中切換模型（/model）
  - H2：命令列介面命令
  - H3：models list
  - H3：models status
...
- 標題:
  ...
  - H2: 運作方式
  - H2: 工作階段
  - H2: 來自 claude-cli 工作階段的 fallback 前置內容
  - H2: 圖片（直通）
  - H2: 輸入 / 輸出
  - H2: 預設值（由外掛擁有）
  - H2: 外掛擁有的預設值
...
- 標題：
  ...
  - H2：Live：ACP 繫結煙霧測試（/acp spawn ... --bind here）
  - H2：Live：Codex 應用程式伺服器測試框架煙霧測試
  - H3：建議的 live 配方
  - H2：Live：模型矩陣（涵蓋範圍）
  - H3：現代煙霧測試集（工具呼叫 + 影像）
...
- 標題：
  ...
  - H3: 上下文引擎組裝必須投射到 Codex 輸入
  - H3: 提示快取穩定性很重要
  - H3: 執行階段選擇語意不變
  - H2: 實作計畫
  - H3: 1. 匯出或重新定位可重用的上下文引擎嘗試輔助工具
  - H3: 2. 新增 Codex 上下文投射輔助工具
  - H3: 3. 在 Codex 執行緒啟動前接上 bootstrap
  - H3: 4. 在 thread/start / thread/resume 與 turn/start 前接上 assemble
  - H3: 5. 保留提示快取穩定格式
  - H3: 6. 在轉錄鏡像後接上 post-turn
  - H3: 7. 正規化用量與提示快取執行階段上下文
  - H3: 8. 壓縮政策
...
- 標題：
  ...
  - H3: 1) 啟動閘道
  - H3: 2) 驗證探索（選用）
  - H4: 透過單播 DNS-SD 在 Tailnet（Vienna ⇄ London）探索
  - H3: 3) 從 Android 連線
  - H3: 存活狀態信標
  - H3: 4) 核准配對（命令列介面）
  - H3: 5) 驗證節點已連線
...
- 標題：
  ...
  - H2: Persona
  - H3: 最小 Persona
  - H3: 完整 Persona（供應商中立提示）
  - H3: Persona 解析
  - H3: 供應商如何使用 Persona 提示
  - H3: 備援政策
  - H2: 模型驅動指令
  - H2: 斜線命令
  - H2: 每位使用者偏好設定
