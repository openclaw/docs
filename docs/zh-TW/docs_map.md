...
summary: OpenClaw 文件頁面的已產生標題對照表
title: 文件地圖
x-i18n:
...
此檔案根據 `docs/**/*.md` 和 `docs/**/*.mdx` 的標題產生，用於協助代理程式瀏覽文件樹狀結構。
...
- 標題：
  ...
  - H3：使用 Teams RSC 與 Microsoft Graph 應用程式權限
  - H3：RSC 與 Graph API 的比較
  - H2：啟用 Graph 的媒體與歷史記錄
...
- 標題：
  ...
  - H2：元件與流程
  - H3：閘道（常駐程式）
  - H3：用戶端（Mac App／命令列介面／網頁管理介面）
  - H3：節點（macOS／iOS／Android／無頭模式）
  - H3：WebChat
  - H2：連線生命週期（單一用戶端）
  - H2：線路通訊協定（摘要）
  - H2：配對與本機信任
...
- 標題：
  ...
  - H2：注入的工作區檔案（專案上下文）
  - H2：Skills：注入與隨需載入的比較
  - H2：工具：有兩種成本
  - H2：命令、指令與「行內捷徑」
  - H2：工作階段、壓縮與修剪（哪些內容會保留）
  - H2：/context 實際回報的內容
  - H2：相關內容
...
- 標題：
  ...
  - H3：其他隨附的供應者外掛
  - H4：值得注意的特殊行為
  - H2：透過 models.providers 使用供應者（自訂／基礎 URL）
  - H3：Moonshot AI（Kimi）
  - H3：Kimi Coding
  ...
  - H3：vLLM
  - H3：SGLang
  - H3：本機 Proxy（LM Studio、vLLM、LiteLLM 等）
  - H2：命令列介面範例
  - H2：相關內容
...
- 標題：
  ...
  - H2：快速開始
  - H2：多個代理程式、多種角色設定
  - H2：各代理程式的 Memory Wiki 儲存庫
  - H2：跨代理程式 QMD 記憶搜尋
  - H2：一個 WhatsApp 號碼，多位使用者（私訊分流）
  - H2：路由規則
  - H2：多個帳號／電話號碼
  - H2：概念
  - H2：平台範例
...
- 路由：/gateway/bonjour
- 標題：
  - H2：透過 Tailscale 的廣域 Bonjour（單點傳播 DNS-SD）
  - H3：閘道設定
...
- 標題：
  ...
  - H3：建議的入門設定
  - H2：擴充範例（主要選項）
  - H3：以符號連結連接的同層 Skill 儲存庫
  - H2：常見模式
  - H3：共用 Skill 基準搭配單一覆寫
  - H3：多平台設定
  - H3：受信任節點網路自動核准
...
- 標題：
  ...
  - H2：macOS 應用程式遠端模式
  - H2：安全性規則（遠端／VPN）
  - H3：macOS：透過 LaunchAgent 建立持續性 SSH 通道
  - H4：步驟 1：新增 SSH 設定
  - H4：步驟 2：複製 SSH 金鑰（僅一次）
  - H4：步驟 3：設定閘道權杖
...
- 標題：
  ...
  - H3：完整存取權（無沙箱）
  - H3：唯讀工具與唯讀工作區
  - H3：無檔案系統／Shell 存取權（允許提供者傳訊）
  - H2：瀏覽器控制風險
  - H3：瀏覽器 SSRF 政策（預設為嚴格）
  - H2：網路公開
  - H3：繫結、連接埠、防火牆
...
- 標題：
  ...
  - H3：驗證詳細代碼快速對照
  - H2：閘道服務未執行
  - H2：macOS 閘道無聲停止回應，操作儀表板後又恢復
  - H2：重複的閘道／節點 LaunchAgent 導致 macOS launchd 監督程式迴圈
  - H2：閘道在高記憶體使用量期間結束
  - H2：閘道拒絕無效設定
  - H2：閘道探測警告
  - H2：頻道已連線，但訊息未流通
...
- 標題：
  ...
  - H3：第 1 層：直接完成模型請求（不使用閘道）
  - H3：第 2 層：閘道 + 開發代理程式冒煙測試（「@openclaw」實際執行的內容）
  - H2：即時測試：命令列介面後端冒煙測試（Claude、Gemini 或其他本機命令列介面）
  - H2：即時測試：APNs HTTP/2 Proxy 連線能力
...
- 標題：
  ...
  - H2：疑難排解
  - H3：openclaw：找不到指令
  - H3：執行 npm install -g 時發生權限錯誤（Linux）
  - H2：相關內容
...
- 標題：
  ...
  - H2：自動偵測（預設）
  - H2：設定範例
  - H3：供應商與命令列介面備援（OpenAI 與 Whisper 命令列介面）
  - H3：僅使用供應商並進行範圍限制
  - H3：僅使用供應商（Deepgram）
...
- 標題：
  ...
  - H2：非目標（v1）
  - H2：先前技術（我們沿用與反轉的部分）
  - H2：架構決策：在工作節點上執行迴圈，透過閘道進行推論
  - H2：元件
  - H3：1. 環境狀態機與供應商合約
  - H3：2. 工作節點啟動程序：在機器上安裝 OpenClaw
  - H3：3. 傳輸：所有內容皆透過 SSH
  - H3：4. 工作節點通訊協定（專用；非節點通訊協定）
  - H3：5. 工作階段後端 RPC
  - H3：6. 工作區同步
...
- 標題：
  - H2：穩定權限的需求
  - H2：Node 與命令列介面執行階段的輔助使用權限
  - H2：提示消失時的復原檢查清單
  - H2：檔案與檔案夾權限（桌面／文件／下載）
  - H2：相關內容
...
- 標題：
  ...
  - H2：思考功能預設值（Claude Sonnet 5、Mythos 5、Fable 5、4.8 與 4.6）
  - H2：安全性拒絕備援（Claude Fable 5）
  - H3：為何需要此功能
  - H3：運作方式
  - H3：可觀測性與計費
  - H3：範圍
...
- 標題：
  ...
  - H3：命令列介面測試框架提供者（Claude Code、Gemini CLI）
  - H3：其他提供者
  - H2：系統提示詞快取邊界
  - H2：OpenClaw 快取穩定性防護
  - H2：調校模式
...
- 標題：
  ...
  - H2：核心命令
  - H2：共用測試狀態與處理程序輔助工具
  - H2：控制介面、終端介面與擴充功能執行路徑
  - H2：閘道與端對端測試
  - H2：完整 Docker 測試套件（pnpm test:docker:all）
  - H3：值得注意的 Docker 執行路徑
  - H2：本機 PR 檢查關卡
  - H2：測試效能工具
  - H2：效能基準測試
  - H2：初始設定端對端測試（Docker）
...
- 標題：
  ...
  - H4：T-PERSIST-002：Skill 更新下毒
  - H4：T-PERSIST-003：竄改代理程式設定
  ...
  - H4：T-EXFIL-003：蒐集認證資訊
  - H3：3.8 影響（AML.TA0011）
  - H4：T-IMPACT-001：未授權執行命令
...
- 標題：
  ...
  - H2：客製化策略（避免更新造成影響）
  - H2：從此儲存庫執行閘道
  - H2：穩定工作流程（先使用 macOS 應用程式）
  - H2：前沿工作流程（在終端機中執行閘道）
  - H3：0）（選用）也從原始碼執行 macOS 應用程式
  - H3：1）啟動開發版閘道
  - H3：2）將 macOS 應用程式指向執行中的閘道
  - H3：3）驗證
  - H3：常見陷阱
  - H2：認證資訊儲存對照表
  - H2：更新（且不破壞你的設定）
...
- 標題：
  ...
  - H3：解決方案 2：以僅連接模式使用 snap Chromium
  - H3：確認瀏覽器正常運作
  - H3：設定參考資料
...
- 標題：
  - H2：先選擇正確的瀏覽器模式
  - H3：選項 1：從 WSL2 直接遠端連線至 Windows CDP
  ...
  - H3：第 2 層：確認 WSL2 可連線至該 Windows 端點
  - H3：第 3 層：設定正確的瀏覽器設定檔
  - H3：第 4 層：個別驗證 Control UI 層
  - H3：第 5 層：驗證端對端瀏覽器控制
  - H2：常見的誤導性錯誤
  - H2：快速分流檢查清單
  - H2：相關內容
...
- 標題：
  ...
  - H2：設定
  - H3：螢幕截圖視覺辨識（支援純文字模型）
  - H2：使用 Brave 或其他以 Chromium 為基礎的瀏覽器
  - H2：本機與遠端控制
  - H2：節點瀏覽器代理（零設定預設值）
  - H2：Browserless（託管的遠端 CDP）
...
- 標題：
  ...
  - H3：將核准要求傳送至工作階段時，該工作階段中的任何人都可以核准嗎？
  - H2：相關內容
...
- 標題：
  ...
  - H3：重要限制：內嵌 Lobster 與 openclaw.invoke
  - H2：工作流程檔案（.lobster）
  - H2：工具參數
  - H3：run
  - H3：resume
...
- 標題：
  ...
  - H2：操作員安裝原則（security.installPolicy）
  - H2：內建 Skill 允許清單
  - H2：各 Skill 的項目（skills.entries）
  - H2：代理程式允許清單（agents）
  - H2：工作坊（skills.workshop）
  - H2：以符號連結連接的 Skill 根目錄
  - H2：沙箱化的 Skills 與環境變數
  - H2：載入順序提醒
  - H2：相關內容
...
- 標題：
  - H2：載入順序
  - H2：由 Node 託管的 Skills
  - H2：各代理程式專用與共用的 Skills
  - H2：代理程式允許清單
  - H2：外掛與 Skills
  - H2：Skill 工作坊
  - H2：從 ClawHub 安裝
