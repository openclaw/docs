...
title: 文件地圖
x-i18n:
    generated_at: "2026-07-05T20:18:07Z"
    model: gpt-5.5
...
- 標題：
  ...
  - H2：多帳號設定
  - H2：疑難排解
  - H3：Twilio 回傳 403 或 OpenClaw 拒絕網路鉤子
  - H3：沒有配對要求出現
  - H3：外送傳送失敗
  - H3：訊息抵達但代理未回答
...
- 標題：
  ...
  - H2：快速模型政策
  - H2：入門設定
  - H2：「Model is not allowed」（以及回覆為何停止）
  - H2：聊天中的 /model
  - H2：命令列介面
...
- 標題：
  ...
  - H2：產生者（Presence 來自何處）
  - H3：1) 閘道自我項目
  - H3：2) WebSocket 連線
  - H4：為何一次性命令列介面命令不會顯示
  - H3：3) system-event Beacon
  - H3：4) 節點連線（role: node）
  - H2：合併 + 去重規則（instanceId 為何重要）
  - H2：TTL 與有界大小
...
- 標題：
  ...
  - H3: 建議的起始設定
  - H2: 展開範例（主要選項）
  - H3: 以符號連結連到同層級 Skills 儲存庫
  - H2: 常見模式
  - H3: 共用 Skills 基準並設定一項覆寫
  - H3: 多平台設定
  - H3: 受信任節點網路自動核准
...
- 標題：
  ...
  - H2: macOS 應用程式遠端模式
  - H2: 安全規則（遠端/VPN）
  - H3: macOS：透過 LaunchAgent 的持續 SSH 通道
  - H4: 步驟 1：新增 SSH 設定
...
- 標題：
  ...
  - H2：分裂腦安裝與新版設定保護
  - H2：復原後通訊協定不相符
  - H2：Skills 符號連結因路徑逸出而略過
  - H2：Anthropic 429 長脈絡需要額外用量
  - H2：上游 403 封鎖回應
  - H2：本機 OpenAI 相容後端通過直接探測但代理執行失敗
  - H2：沒有回覆
  - H2：Dashboard control UI 連線能力
  - H3：驗證詳細代碼快速對照
  - H2：閘道服務未執行
  - H2：macOS 閘道靜默停止回應，然後在你觸碰儀表板時恢復
  - H2：閘道在高記憶體使用期間結束
  - H2：閘道拒絕無效設定
  - H2：閘道探測警告
  - H2：頻道已連線，訊息未流動
...
- 標題：
  ...
  - H2：Live：模型冒煙測試（設定檔鍵）
  - H3：第 1 層：直接模型完成（無閘道）
  - H3：第 2 層：閘道 + 開發代理冒煙測試（「@openclaw」實際上做的事）
  - H2：Live：命令列介面後端冒煙測試（Claude、Gemini 或其他本機命令列介面）
  - H2：Live：APNs HTTP/2 代理可達性
  - H2：Live：ACP 繫結冒煙測試（/acp spawn ... --bind here）
  - H2：Live：Codex app-server harness 冒煙測試
  - H3：建議的 Live 配方
  - H2：Live：模型矩陣（涵蓋內容）
  - H3：聚合器 / 替代閘道
  - H2：憑證（切勿提交）
...
- 標題：
  - H2：快速開始
  - H2：測試暫存目錄
  - H2：Live 與 Docker/Parallels 工作流程
  - H2：QA 專用執行器
  ...
  - H2：我應該執行哪個套件？
  - H2：Live（觸及網路）測試
  - H2：Docker 執行器（選用「可在 Linux 運作」檢查）
  - H2：文件健全性
  - H2：離線迴歸（CI 安全）
  - H2：代理可靠性評估（Skills）
  - H2：合約測試（外掛與頻道形狀）
...
- 標題：
  - H2：前 60 秒
  - H2：助理感覺受限或缺少工具
  - H2：Anthropic 長脈絡 429
  - H2：本機 OpenAI 相容後端可直接運作但在 OpenClaw 中失敗
...
- 標題：
  ...
  - H2：自動偵測（預設）
  - H2：設定範例
  - H3：提供者 + 命令列介面備用方案（OpenAI + Whisper 命令列介面）
...
- 標題:
  ...
  - H3: 6. Chat 與記錄
  - H3: 7. Canvas 與相機
  - H4: Gateway Canvas Host（建議用於網頁內容）
...
- 標題：
  ...
  - H2: 設定形態
  - H2: 進階後端掛鉤
  - H3: ownsNativeCompaction：退出 OpenClaw 壓縮
  - H2: MCP 工具橋接
  - H2: 使用者設定
  - H2: 驗證
  - H2: 檢查清單
...
- 標題：
  - H2: 需求
  - H2: 快速開始
  - H2: 與 Codex Desktop 和命令列介面分享執行緒
  - H2: 設定
  - H3: 壓縮
  - H2: 驗證 Codex 執行階段
  - H2: 路由與模型選擇
...
- 標題：
  ...
  - H2：安全拒絕 fallback（Claude Fable 5）
  - H3：存在原因
  - H3：運作方式
  - H3：可觀測性與計費
  - H3：範圍
...
- Headings:
  ...
  - H3: 階段 4：移動逐字稿、ACP 串流、軌跡和 VFS
  - H3: 階段 5：備份、還原、Vacuum 和驗證
  - H3: 階段 6：工作程式執行階段
  - H3: 階段 7：刪除舊世界
  - H2: 備份與還原
  - H2: 執行階段重構計畫
...
- Headings:
  ...
  - H3: 命令列介面測試架提供者（Claude Code、Gemini CLI）
  - H3: 其他提供者
  - H2: 系統提示快取邊界
  - H2: OpenClaw 快取穩定性防護
  - H2: 調整模式
...
- 標題：
  ...
  - H2：先決條件（從原始碼）
  - H2：客製策略（避免更新造成問題）
  - H2：從此 repo 執行 Gateway
  - H2：穩定工作流程（macOS 應用程式優先）
  - H2：前沿工作流程（在終端機中執行 Gateway）
  - H3：0)（選用）也從原始碼執行 macOS 應用程式
  - H3：1) 啟動開發 Gateway
  - H3：2) 將 macOS 應用程式指向正在執行的 Gateway
  - H3：3) 驗證
  - H3：常見陷阱
  - H2：憑證儲存對照表
  - H2：更新（不破壞你的設定）
...
- 標題：
  ...
  - H2：必要設定
  - H2：acpx backend 的外掛設定
  - H3：acpx runtime startup probe
  - H3：自動下載 adapter
  - H3：外掛工具 MCP bridge
  - H3：OpenClaw 工具 MCP bridge
  - H3：執行階段操作逾時設定
  - H3：健康狀態探測代理設定
  - H2：權限設定
  - H3：permissionMode
...
- 標題：
  ...
  - H3: 解決方案 2：以僅附加模式使用 snap Chromium
  - H3: 驗證瀏覽器可正常運作
  - H3: 設定參考
...
- 標題：
  ...
  - H2: 角色
  - H3: 最小角色
  - H3: 完整角色（Provider 中立提示）
  - H3: 角色解析
  - H3: Provider 如何使用角色提示
  - H3: 備援政策
  - H2: 模型驅動指令
  - H2: 斜線命令
  - H2: 個別使用者偏好設定
