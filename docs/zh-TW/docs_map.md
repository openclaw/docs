...
此檔案是由 `docs/**/*.md` 與 `docs/**/*.mdx` 的標題產生，用來協助代理瀏覽文件樹。
...
- 標題:
  ...
  - H2: 分裂腦安裝與較新設定防護
  - H2: 復原後的協定不相符
  - H2: Skill 符號連結因路徑逃逸而跳過
  - H2: Anthropic 429 長上下文需要額外用量
  - H2: 上游 403 封鎖回應
  - H2: 本機 OpenAI 相容後端可通過直接探測但代理執行失敗
  - H2: 沒有回覆
  - H2: Dashboard 控制 UI 連線能力
  - H3: 驗證詳細代碼快速對照
  - H2: 閘道服務未執行
  - H2: macOS 閘道默默停止回應，接著在你觸碰 Dashboard 時恢復
  - H2: 閘道在高記憶體使用期間結束
  - H2: 閘道拒絕無效設定
  - H2: 閘道探測警告
  - H2: 頻道已連線，訊息未流動
...
- 標題：
  ...
  - H2: Live：模型煙霧測試（設定檔鍵）
  - H3: 第 1 層：直接模型補全（不經閘道）
  - H3: 第 2 層：閘道 + 開發代理煙霧測試（「@openclaw」實際做的事）
...
- 標題：
  ...
  - H3：哪些東西在哪裡執行
  - H3：啟動節點主機（前景）
  - H3：透過 SSH tunnel 的遠端閘道（loopback 綁定）
  - H3：啟動節點主機（服務）
  - H3：配對 + 命名
  - H3：將命令加入允許清單
  - H3：將 exec 指向節點
...
- 標題：
  ...
  - H3：1) 啟動閘道
  - H3：2) 驗證探索（可選）
  - H4：透過 unicast DNS-SD 進行 Tailnet（維也納 ⇄ 倫敦）探索
  - H3：3) 從 Android 連線
...
- 標題:
  - H2: 需求
  - H2: 快速入門
  - H2: 與 Codex Desktop 和命令列介面共用 threads
  - H2: 設定
  - H2: 驗證 Codex 執行階段
  - H2: 路由與模型選擇
  - H2: 部署模式
...
- 標題:
  ...
  - H2: 內建 Skills 允許清單
  - H2: 每個 Skills 項目 (skills.entries)
  - H2: 代理允許清單 (agents)
