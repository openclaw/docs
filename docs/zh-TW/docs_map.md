...
此檔案是由 `docs/**/*.md` 和 `docs/**/*.mdx` 的標題產生，用於協助代理導覽文件樹。
...
- 標題：
  ...
  - H3: 外掛 hooks（agent + gateway 生命週期）
  - H2: 串流 + 部分回覆
  - H2: 工具執行 + 訊息工具
  - H2: 回覆塑形 + 抑制
  - H2: 壓縮 + 重試
...
- 標題:
  ...
  - H2: 即時：ACP 繫結冒煙測試（/acp spawn ... --bind here）
  - H2: 即時：Codex app-server 測試工具冒煙測試
  - H3: 建議的即時配方
  - H2: 即時：模型矩陣（我們涵蓋的內容）
  - H3: 現代冒煙測試組（工具呼叫 + 圖片）
...
- 標題:
  ...
  - H2: 自動偵測（預設）
  - H2: 設定範例
  - H3: Provider + 命令列介面 fallback（OpenAI + Whisper 命令列介面）
  - H3: 僅 Provider 並使用 scope gating
  - H3: 僅 Provider (Deepgram)
  - H3: 僅 Provider (Mistral Voxtral)
  - H3: 僅 Provider (SenseAudio)
  - H3: 將轉錄 echo 到聊天（選擇啟用）
  - H2: 注意事項與限制
  - H3: Proxy 環境支援
  - H2: 群組中的 mention 偵測
  - H2: 注意事項
...
- 標題:
  ...
  - H2: 設定概覽
  - H3: 模型項目
  - H3: Provider 憑證 (apiKey)
  - H2: 預設值與限制
  - H3: 自動偵測媒體理解（預設）
  - H3: Proxy 環境支援（Provider 模型）
  - H2: 能力（選用）
  - H2: Provider 支援矩陣（OpenClaw 整合）
  - H2: 模型選擇指南
  - H2: 附件政策
  - H2: 設定範例
  - H2: 狀態輸出
...
- 標題:
  ...
  - H3: Context engine assembly 必須投影到 Codex 輸入
  - H3: Prompt-cache 穩定性很重要
  - H3: Runtime 選擇語義不會改變
  - H2: 實作計畫
  - H3: 1. 匯出或移動可重用的 context-engine attempt helpers
  - H3: 2. 新增 Codex context projection helper
  - H3: 3. 在 Codex thread startup 前接上 bootstrap
  ...
  - H4: Turn 內 Codex 原生 contextCompaction 事件
  - H3: 9. Session reset 與 binding 行為
  - H3: 10. 錯誤處理
  - H2: 測試計畫
  - H3: 單元測試
  - H3: 要更新的既有測試
...
- 標題:
  ...
  - H3: 1) 啟動閘道
  - H3: 2) 驗證探索（選用）
  - H4: 透過 unicast DNS-SD 的 Tailnet（Vienna ⇄ London）探索
  - H3: 3) 從 Android 連線
  - H3: 存活狀態 beacon
  - H3: 4) 核准配對（命令列介面）
  - H3: 5) 驗證節點已連線
...
- 標題:
  ...
  - H2: 設定進入點
  - H3: 狹窄的設定輔助工具匯入
  - H3: Channel 擁有的單一帳號提升
  - H2: 設定結構描述
  - H3: 建置 channel 設定結構描述
  - H2: 設定精靈
  - H2: 發佈和安裝
  - H2: 相關
...
- 標題:
  ...
  - H3: 測試註冊合約
  - H3: 測試執行階段設定存取
  - H3: 對 channel 外掛進行單元測試
  - H3: 對 provider 外掛進行單元測試
  - H3: 模擬外掛執行階段
  - H3: 使用個別執行個體 stub 進行測試
  - H2: 合約測試（儲存庫內外掛）
  - H3: 執行限定範圍測試
  - H2: Lint 強制執行（儲存庫內外掛）
  - H2: 測試設定
...
- 標題:
  ...
  - H3: openclaw.plugin.json 產生的中繼資料已過時
  - H3: package.json openclaw.extensions 必須包含 ./dist/index.js
  - H3: 找不到 package 'typebox'
  - H3: 安裝後未顯示工具
  - H2: 另請參閱
...
- 標題:
  - H2: 開始使用
  - H2: Thinking 預設值（Claude Fable 5、4.8 和 4.6）
...
- 標題:
  ...
  - H3: 第 2 層：驗證 WSL2 能否連到該 Windows 端點
...
- 標題:
  ...
  - H3: macOS IPC 流程
  - H2: 常見問題
  - H3: 何時會在核准目標上使用 accountId 與 threadId？
...
- 標題:
  ...
  - H2: 工作階段覆寫（/exec）
  - H2: 授權模型
  - H2: Exec 核准（companion app / 節點主機）
  - H2: Allowlist + 安全 bins
  - H2: 範例
  - H2: applypatch
  - H2: 相關
...
- 標題:
  ...
  - H2: 設定 Firecrawl search
  - H2: 設定 Firecrawl webfetch fallback
  - H3: 自行託管的 Firecrawl
  - H2: Firecrawl 外掛工具
  - H3: firecrawlsearch
  - H3: firecrawlscrape
