...
title: LM Studio
x-i18n:
    generated_at: "2026-07-21T09:02:45Z"
    model: gpt-5.6
...
<Steps>
  ...
  <Step title="執行初始設定">
    ...
    在全新的引導式設定中，OpenClaw 會先查詢預設或已設定的 LM Studio 主機上的
    `/api/v1/models`。只有當 LM Studio 回報模型已接受工具訓練，且有效
    上下文至少為 16K 時，才會自動提供現有的 LLM。對於已載入的模型，載入執行個體的上下文優先於
    所公告的較大上限。相同的命令列介面/macOS 設定流程會先使用實際補全驗證
    路由，再儲存設定。自動檢查絕不會
    下載模型，並會忽略僅供嵌入使用的目錄項目。
...
LM Studio 模型鍵使用 `author/model-name` 格式（例如 `qwen/qwen3.5-9b`）；OpenClaw 模型參照會
在前方加上提供者：`lmstudio/qwen/qwen3.5-9b`。若要尋找模型的確切鍵值，請執行
下列命令並查看 `key` 欄位：
...
`--custom-model-id` 接受 LM Studio 傳回的模型鍵（例如 `qwen/qwen3.5-9b`），不含
`lmstudio/` 提供者前綴。對於需要驗證的
伺服器，請傳入 `--lmstudio-api-key`（或設定 `LM_API_TOKEN`）；對於不需要驗證的伺服器則省略此項，OpenClaw 會改為儲存本機非機密標記。
...
"on"]`），但在 `/v1/chat/completions` 上拒絕這些字面值；OpenClaw 會先將該
二元格式正規化為六級尺度，再傳送請求；這也適用於仍含有
...
LM Studio 支援即時（JIT）模型載入，可在第一次請求時載入模型。OpenClaw
預設會透過 LM Studio 的原生載入端點預先載入模型，這在停用 JIT 時很有幫助。
若要改由 LM Studio 的 JIT、閒置 TTL 與自動逐出行為管理模型生命週期，
...
`lmstudio` 會自動信任其已設定的模型請求端點，包括回送、區域網路
與 tailnet 主機（中繼資料／鏈路本機來源除外）。任何自訂／本機 OpenAI 相容
提供者項目都會獲得相同的精確來源信任。傳送至不同私人主機或連接埠的請求仍
需要 `models.providers.<id>.request.allowPrivateNetwork: true`；將其設為 `false` 即可選擇停用
預設信任。
...
若已啟用驗證，也請設定 `LM_API_TOKEN`。確認 API 可連線：
