...
read_when:
    ...
    - 你正在內建 Copilot 供應商、Copilot SDK 控制框架與 Copilot Proxy 之間進行選擇
summary: 使用裝置流程或非互動式權杖匯入，從 OpenClaw 登入 GitHub Copilot
title: GitHub Copilot
x-i18n:
    generated_at: "2026-07-12T14:45:37Z"
    model: gpt-5.6
...
GitHub Copilot 是 GitHub 的 AI 程式設計助理。它可讓你的 GitHub 帳戶與方案使用 Copilot
模型。OpenClaw 能以三種不同方式使用 Copilot 作為模型
提供者或代理程式執行階段。
...
<Tabs>
  ...
  <Tab title="Copilot SDK 控制框架外掛 (copilot)">
    ...
    如果你希望這些代理程式回合使用原生 Copilot 命令列介面工作階段、由 SDK 管理的執行緒
    狀態，以及由 Copilot 負責的壓縮，請選擇此方式。若未明確選擇使用
    `agentRuntime`，`github-copilot/*` 模型會繼續使用
    內建提供者。完整的執行階段契約請參閱 [Copilot SDK 控制框架](/zh-TW/plugins/copilot)。
  ...
  <Tab title="Copilot Proxy 外掛 (copilot-proxy)">
    ...
    `copilot-proxy` 外掛隨 OpenClaw 一起提供，且預設啟用。
    使用以下命令設定基礎 URL 與模型 ID：
...
租用戶標記且不含 Proxy 提示，因此補全基礎 URL 會退回使用
租用戶的 Copilot 主機，而非公用端點。

<Note>
切換網域一律會重新執行裝置登入。如果你已儲存
Copilot 權杖並選擇不同的網域（公用 `github.com` ↔ `*.ghe.com`
租用戶，或從一個租用戶切換至另一個），OpenClaw 不會重複使用現有權杖，
而會強制重新登入，以確保權杖的範圍符合要寫入
設定的網域。針對*相同*網域重新執行登入時，仍會提供重複使用目前
權杖的選項。切換回公用 `github.com` 時，會清除已保存的
`githubDomain`，使設定恢復為預設值。
</Note>

<Note>
`COPILOT_GITHUB_DOMAIN` 環境變數會覆寫所有解析 Copilot 網域之路徑的
解析結果，包括 Enterprise 裝置登入
（`--method device-enterprise`）、獨立的
`openclaw models auth login-github-copilot` 捷徑、權杖重新整理、嵌入，
以及補全。若要進行完全無人值守或 CI
設定，請將其設為你的 `*.ghe.com` 主機。若要使用公用 `github.com`，
請不要設定此變數（且不要設定設定參數）。
登入時會保存用於核發權杖的網域（透過公用 `github.com` 登入時則會清除），
因此即使取消設定環境變數，路由仍會保持正確。
</Note>
...
| `openclaw models auth login --provider github-copilot --method device` | `--set-default` | 同時套用提供者建議的預設模型               |
...
GitHub Copilot 提供者驗證選項。如果省略該旗標，初始設定會依序改用
`COPILOT_GITHUB_TOKEN`、`GH_TOKEN`，再到 `GITHUB_TOKEN`。若已設定
`COPILOT_GITHUB_TOKEN`，可使用 `--secret-input-mode ref`，在
`auth-profiles.json` 中儲存由環境變數支援的 `tokenRef`，而非純文字。

<AccordionGroup>
  <Accordion title="需要互動式終端介面">
    裝置登入流程需要互動式終端介面。請直接在
    終端機中執行，不要在非互動式指令碼或 CI 管線中執行。
  </Accordion>

  <Accordion title="模型可用性取決於你的方案">
    Copilot 模型可用性取決於你的 GitHub 方案。如果模型遭到
    拒絕，請嘗試其他 ID（例如 `github-copilot/gpt-5.5`）。目前的模型清單請參閱
  ...
  <Accordion title="從 Copilot API 即時重新整理目錄">
    ...
    （與 VS Code Copilot 使用的端點相同）重新整理模型目錄，讓執行階段能掌握
    各帳戶的使用資格與準確的上下文視窗，而不必頻繁變更資訊清單。
    新發布的 Copilot 模型無須升級 OpenClaw 即可顯示，
    而且上下文視窗會反映實際的各模型限制
    （例如 gpt-5.x 系列為 400k，內部
    `claude-opus-*-1m` 變體為 1M）。

    當探索功能停用、使用者沒有 GitHub 驗證設定檔、權杖交換
    失敗，或 `/models` HTTPS 呼叫發生錯誤時，隨附的靜態目錄仍會作為可見的備援。
    若要選擇停用並完全依賴
    靜態資訊清單目錄（離線／實體隔離情境）：
  ...
  <Accordion title="傳輸選擇">
    ...
    Gemini 模型使用 OpenAI Chat Completions 傳輸；GPT 與 o 系列
    模型則繼續使用 OpenAI Responses 傳輸。OpenClaw 會根據模型參照
    選擇正確的傳輸方式。
  </Accordion>

  <Accordion title="請求相容性">
    OpenClaw 會在 Copilot 傳輸中傳送 Copilot IDE 樣式的請求標頭
    （VS Code 編輯器／外掛版本與 `vscode-chat` 整合 ID），
    將工具結果的後續回合標記為由代理程式發起，並在回合包含影像輸入時設定 Copilot
    視覺標頭。
  </Accordion>
  ...
  <Accordion title="權杖儲存">
    登入會將 GitHub 權杖儲存在驗證設定檔存放區（設定檔 ID
...
[記憶搜尋](/zh-TW/concepts/memory-search)的嵌入提供者。如果你訂閱了 Copilot 且
已登入，OpenClaw 不需要另外的 API 金鑰即可將其用於嵌入。

### 設定

若要使用 GitHub Copilot 嵌入，請明確將 `memorySearch.provider` 設為相應值。如果
GitHub 權杖可用，OpenClaw 會從
Copilot API 探索可用的嵌入模型，並自動選擇最佳模型。
