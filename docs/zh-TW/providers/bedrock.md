...
<Tabs>
  <Tab title="Access keys / env vars">
    **最適合：** 開發者機器、CI，或你直接管理 AWS 憑證的主機。
    ...
    <Tip>
    使用環境標記驗證（`AWS_ACCESS_KEY_ID`、`AWS_PROFILE` 或 `AWS_BEARER_TOKEN_BEDROCK`）時，OpenClaw 會自動啟用隱含的 Bedrock 提供者，以便在不需額外設定的情況下探索模型。
    </Tip>
    ...
    **最適合：** 已附加 IAM 角色、並使用執行個體中繼資料服務進行驗證的 EC2 執行個體。
    ...
    <Steps>
      <Step title="Enable discovery explicitly">
        使用 IMDS 時，OpenClaw 無法只靠環境標記偵測 AWS 驗證，因此你必須明確選擇啟用：
      ...
      <Step title="Optionally add an env marker for auto mode">
        如果你也希望環境標記自動偵測路徑能運作（例如用於 `openclaw status` 介面）：
    ...
    <Note>
    只有在你特別想要為自動模式或狀態介面提供環境標記時，才需要 `AWS_PROFILE=default`。實際的 Bedrock 執行階段驗證路徑使用 AWS SDK 預設鏈，因此即使沒有環境標記，IMDS 執行個體角色驗證也能運作。
    </Note>
...
OpenClaw 可以自動探索支援**串流**和**文字輸出**的 Bedrock 模型。探索使用 `bedrock:ListFoundationModels` 和 `bedrock:ListInferenceProfiles`，結果會被快取（預設：1 小時）。
...
- 如果未設定 `plugins.entries.amazon-bedrock.config.discovery.enabled`，
  OpenClaw 只有在看到下列 AWS 驗證標記之一時，才會自動加入
  隱含的 Bedrock 提供者：
  `AWS_BEARER_TOKEN_BEDROCK`、`AWS_ACCESS_KEY_ID` +
  `AWS_SECRET_ACCESS_KEY`，或 `AWS_PROFILE`。
- 實際的 Bedrock 執行階段驗證路徑仍使用 AWS SDK 預設鏈，因此
  共用設定、SSO 和 IMDS 執行個體角色驗證可以運作，即使探索
  需要 `enabled: true` 才能選擇啟用。

<Note>
對於明確的 `models.providers["amazon-bedrock"]` 項目，OpenClaw 仍可從 `AWS_BEARER_TOKEN_BEDROCK` 等 AWS 環境標記提早解析 Bedrock 環境標記驗證，而不強制載入完整執行階段驗證。實際的模型呼叫驗證路徑仍使用 AWS SDK 預設鏈。
</Note>
...
<AccordionGroup>
  <Accordion title="Discovery config options">
    ...
    | 選項 | 預設 | 說明 |
    | ------ | ------- | ----------- |
    | `enabled` | auto | 在自動模式下，OpenClaw 只有在看到支援的 AWS 環境標記時，才會啟用隱含的 Bedrock 提供者。設為 `true` 可強制探索。 |
    ...
    | `defaultContextWindow` | `32000` | 用於探索到的模型的上下文視窗（如果你知道模型限制，可覆寫）。 |
    | `defaultMaxTokens` | `4096` | 用於探索到的模型的最大輸出 token 數（如果你知道模型限制，可覆寫）。 |
...
<AccordionGroup>
  <Accordion title="Inference profiles">
    OpenClaw 會在基礎模型旁一併探索**區域與全域推論設定檔**。當設定檔對應到已知基礎模型時，該設定檔會繼承該模型的能力（上下文視窗、最大 token 數、推理、視覺），並自動注入正確的 Bedrock 請求區域。這表示跨區域 Claude 設定檔可在不手動覆寫提供者的情況下運作。

    推論設定檔 ID 看起來像 `us.anthropic.claude-opus-4-6-v1:0`（區域）
    或 `anthropic.claude-opus-4-6-v1:0`（全域）。如果後端模型已在
    探索結果中，設定檔會繼承其完整能力集；否則會套用安全預設值。

    不需要額外設定。只要已啟用探索，且 IAM 主體具備 `bedrock:ListInferenceProfiles`，設定檔就會與基礎模型一同出現在 `openclaw models list` 中。

  </Accordion>

  <Accordion title="Service tier">
    某些 Bedrock 模型支援 `service_tier` 參數，可針對成本或延遲最佳化。可用層級如下：

    | 層級 | 說明 |
    |------|-------------|
    | `default` | 標準 Bedrock 層級 |
    | `flex` | 適用於可容忍較長延遲工作負載的折扣處理 |
    | `priority` | 適用於延遲敏感工作負載的優先處理 |
    | `reserved` | 適用於穩定狀態工作負載的預留容量 |

    可透過 `agents.defaults.params` 為 Bedrock 模型請求設定 `serviceTier`（或 `service_tier`），或在
    `agents.defaults.models["<model-key>"].params` 中依模型設定：
    ...
    有效值為 `default`、`flex`、`priority` 和 `reserved`。並非所有
    模型都支援所有層級；如果請求了不支援的層級，Bedrock 會
    回傳驗證錯誤。注意：錯誤訊息可能有些誤導；
    它可能會說「The provided model identifier is invalid」，而不是指出
    不支援的服務層級。如果看到此錯誤，請檢查模型是否
    支援所請求的層級。
  ...
  <Accordion title="Claude Opus 4.7 temperature">
    ...
    會自動省略任何 Opus 4.7 Bedrock 參照的 `temperature`，包括
    基礎模型 ID、具名推論設定檔、其底層模型透過
    `bedrock:GetInferenceProfile` 解析為 Opus 4.7 的應用程式推論
    設定檔，以及帶有選用區域前綴（`us.`、`eu.`、`ap.`、`apac.`、`au.`、`jp.`、
    `global.`）的點號 `opus-4.7` 變體。不需要設定旋鈕，且省略會同時套用於
    請求選項物件和 `inferenceConfig` 承載欄位。
  </Accordion>

  <Accordion title="Claude Fable 5">
    在 `us-east-1` 使用 `amazon-bedrock/anthropic.claude-fable-5`，或使用
    `us.anthropic.claude-fable-5` 等區域推論 ID。
    OpenClaw 會套用 Fable 的 1M 情境視窗、128K 輸出上限、常時啟用的
    自適應思考，以及支援的 effort 對應。`/think off` 和
    `/think minimal` 會對應到 `low`；不支援的 temperature 和強制工具
    選擇控制項會被省略。串流輸出會保留到 Bedrock
    回傳終止狀態，因此串流中途的拒絕不會暴露部分文字。
    Fable 僅支援標準服務層級；OpenClaw 會忽略此模型已設定的
    `flex`、`priority` 和 `reserved` 層級。

    AWS 要求先明確選擇加入 `provider_data_share` 資料保留設定，
    Fable 才可使用。提示和完成內容會與 Anthropic 分享，並為了信任與安全保留最多 30 天。
    啟用模型前，請先檢閱並設定
  ...
  <Accordion title="防護欄">
    你可以透過在 `amazon-bedrock` 外掛設定中加入 `guardrail` 物件，
    將 [Amazon Bedrock 防護欄](https://docs.aws.amazon.com/bedrock/latest/userguide/guardrails.html)
    套用到所有 Bedrock 模型呼叫。防護欄可讓你強制執行內容篩選、
    ...
    | `guardrailVersion` | 是 | 已發布的版本號，或工作草稿使用的 `"DRAFT"`。 |
    | `streamProcessingMode` | 否 | 串流期間進行防護欄評估時使用的 `"sync"` 或 `"async"`。若省略，Bedrock 會使用其預設值。 |
    | `trace` | 否 | 偵錯時使用 `"enabled"` 或 `"enabled_full"`；正式環境請省略或設為 `"disabled"`。 |
  ...
  <Accordion title="用於記憶搜尋的嵌入">
    Bedrock 也可以作為
    [記憶搜尋](/zh-TW/concepts/memory-search)的嵌入提供者。這是與推論提供者分開設定的 --
    ...
    Bedrock 嵌入會使用與推論相同的 AWS SDK 憑證鏈
    （執行個體角色、SSO、存取金鑰、共用設定和 Web 身分）。
    不需要 API 金鑰。請明確設定 `memorySearch.provider: "bedrock"` 以使用 Bedrock
    嵌入。
  ...
  <Accordion title="注意事項與限制">
    ...
    - 如果你依賴自動模式，請在閘道主機上設定其中一個支援的 AWS 驗證環境標記。
      如果你偏好在沒有環境標記的情況下使用 IMDS/共用設定驗證，請設定
      `plugins.entries.amazon-bedrock.config.discovery.enabled: true`。
    - OpenClaw 會依此順序呈現憑證來源：`AWS_BEARER_TOKEN_BEDROCK`，
      接著是 `AWS_ACCESS_KEY_ID` + `AWS_SECRET_ACCESS_KEY`，再來是 `AWS_PROFILE`，
      最後是預設 AWS SDK 鏈。
    - 推理支援取決於模型；請查看 Bedrock 模型卡以了解目前功能。
    - 如果你偏好受管理的金鑰流程，也可以在 Bedrock 前方放置 OpenAI 相容代理，
      並改將其設定為 OpenAI 提供者。
  </Accordion>
</AccordionGroup>
