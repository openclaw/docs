...
[Featherless AI](https://featherless.ai) 透過相容 OpenAI 的 API 提供開放模型服務。OpenClaw 會將 Featherless 安裝為官方外部供應商外掛，在維持內建目錄精簡的同時，於執行階段接受 Featherless 的精確模型 ID。
...
此外掛使用 `Qwen/Qwen3-32B` 作為設定時的預設模型，因為 Featherless 文件指出 Qwen 3 系列原生支援工具呼叫。OpenClaw 會設定其 32,768 個權杖的上下文視窗、保守的 4,096 個權杖輸出上限，以及 Qwen 聊天範本的思考控制項。

目錄中的成本欄位皆為零，因為 Featherless 支援多種計費模式，而 OpenClaw 不會內嵌帳戶專屬方案或按請求計價的費率。

## 其他 Featherless 模型

在 `featherless/` 供應商前綴之後使用精確的 Featherless 模型 ID：
...
OpenClaw 刻意不將 Featherless 的完整公開模型索引複製到選擇器中。該索引規模龐大，且未提供足夠的結構化能力中繼資料，無法安全地為每個文字、視覺、嵌入和推理模型分類。因此，未知 ID 會採用保守的純文字、非推理預設值：4,096 個權杖的上下文視窗，以及 1,024 個權杖的輸出上限。
...
新增自訂中繼資料前，請查看 Featherless 的模型目錄，以確認目前的模型可用性與能力標籤。

## 疑難排解

- `401` 或 `403`：確認閘道程序可存取 `FEATHERLESS_API_KEY`，或再次執行初始設定。
- 未知模型：在 `featherless/` 前綴之後，使用 Featherless 提供且區分大小寫的精確 ID。
- 工具呼叫以文字形式傳回：選擇 Featherless 文件中標示原生支援函式呼叫的模型系列，例如 Qwen 3。
- 受管理的閘道無法存取金鑰：將其放入 `~/.openclaw/.env` 或服務會載入的其他環境來源，然後重新啟動閘道。
