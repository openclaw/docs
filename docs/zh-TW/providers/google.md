...
<Tabs>
  <Tab title="API 金鑰">
    **最適合：** 透過 Google AI Studio 使用標準 Gemini API 存取。
    ...
    <Tip>
    環境變數 `GEMINI_API_KEY` 和 `GOOGLE_API_KEY` 都可接受。使用你已設定好的任一個即可。
    </Tip>
...
[Gemini 搜尋](/zh-TW/tools/gemini-search) 了解供應商特定工具行為。

<Tip>
Gemini 3 模型使用 `thinkingLevel`，而不是 `thinkingBudget`。OpenClaw 會將
Gemini 3、Gemini 3.1 和 `gemini-*-latest` 別名推理控制項對應到
...
Google 可選擇層級；Gemini 2.5 會傳送 Google 的動態 sentinel
`thinkingBudget: -1`。
...
<Note>
請參閱[音樂生成](/zh-TW/tools/music-generation)，了解共用工具參數、供應商選擇與容錯移轉行為。
</Note>
...
供應商。這不是個別的 Cloud Text-to-Speech API 路徑。
</Note>
...
OpenClaw 會將電話/Meet 橋接音訊適配到 Gemini 的 PCM Live API 串流，並
讓工具呼叫維持在共享的即時語音合約上。除非需要變更取樣，否則請讓 `temperature`
保持未設定；OpenClaw 會省略非正值，因為 Google Live 在 `temperature: 0` 時
可能傳回沒有音訊的逐字稿。
Gemini API 轉錄在未設定 `languageCodes` 的情況下啟用；目前的 Google
SDK 會拒絕此 API 路徑上的語言代碼提示。
</Note>

<Note>
Control UI Talk 支援使用受限一次性
權杖的 Google Live 瀏覽器工作階段。僅後端的即時語音提供者也可以透過一般
Gateway 轉送傳輸執行，這會將提供者憑證保留在 Gateway 上。
</Note>
