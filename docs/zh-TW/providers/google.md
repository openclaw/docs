...
title: Google（Gemini）
x-i18n:
    generated_at: "2026-07-19T14:00:17Z"
    model: gpt-5.6
...
<Tabs>
  <Tab title="API 金鑰">
    **最適合：**透過 Google AI Studio 進行標準 Gemini API 存取。
    ...
    <Tip>
    `GEMINI_API_KEY` 和 `GOOGLE_API_KEY` 皆可接受。使用已設定的任一項即可。
    </Tip>

  </Tab>

  <Tab title="Gemini CLI (OAuth)">
    **最適合：**透過 Gemini CLI OAuth 登入 Google 帳戶，而非使用獨立的 API 金鑰。

    <Warning>
    `google-gemini-cli` 提供者是非官方整合。部分使用者回報，以此方式使用 OAuth 時
    帳戶會受到限制。請自行承擔風險。
    </Warning>
    ...
    Gemini 3.1 Pro 的 Gemini API 模型 ID 為 `gemini-3.1-pro-preview`。OpenClaw 為方便使用，也接受較短的 `google/gemini-3.1-pro` 別名，並會在呼叫提供者前將其標準化。
    ...
    **環境變數：**
    ...
    初始設定的自動偵測會列出既有的 Gemini CLI 登入，但絕不會
    自動測試，因為 Gemini CLI 沒有不使用工具的探測方式。請選擇 Gemini CLI
    OAuth 或 Gemini API 金鑰以繼續。

    `google-gemini-cli/*` 模型參照是舊版相容別名。新
...
<Note>
`google/gemini-3-pro-preview` 已於 2026-03-09 停用；請改用 `google/gemini-3.1-pro-preview`。重新執行 Gemini API 金鑰設定（`openclaw onboard --auth-choice gemini-api-key` 或 `openclaw models auth login --provider google`）會將過時的已設定預設值改寫為目前模型。
</Note>
      ...
      google: {
        config: {
          webSearch: {
            ...
            baseUrl: "https://generativelanguage.googleapis.com/v1beta", // 回退至 models.providers.google.baseUrl
...
再來是 `models.providers.google.apiKey`。`webSearch.baseUrl` 為選用，
供營運者代理伺服器或相容的 Gemini API 端點使用；省略時，
Gemini 網路搜尋會重複使用 `models.providers.google.baseUrl`。請參閱
[Gemini 搜尋](/zh-TW/tools/gemini-search)，瞭解提供者特定的工具行為。
...
<Note>
請參閱[圖片生成](/zh-TW/tools/image-generation)，瞭解共用工具參數、提供者選擇和容錯移轉行為。
</Note>
...
<Note>
請參閱[影片生成](/zh-TW/tools/video-generation)，瞭解共用工具參數、提供者選擇和容錯移轉行為。
</Note>
...
- 輸出格式：預設為 `mp3`，另可在 `google/lyria-3-pro-preview` 上使用 `wav`
- 參照輸入：最多 10 張圖片
...
<Note>
請參閱[音樂生成](/zh-TW/tools/music-generation)，瞭解共用工具參數、提供者選擇和容錯移轉行為。
</Note>

## 文字轉語音

內建的 `google` 語音提供者會使用 Gemini API TTS 路徑搭配
...
`generateContent` 回應中傳回生成的音訊。若要取得延遲最低的語音對話，請使用
...
`audioProfile`，可在語音文字前加上可重複使用的風格提示詞。若
提示詞文字提及具名說話者，請設定 `speakerName`。

Gemini API TTS 也接受文字中的表達性方括號音訊標籤，
例如 `[whispers]` 或 `[laughs]`。若要避免標籤出現在可見的聊天回覆中，
同時將其傳送至 TTS，請將標籤放在 `[[tts:text]]...[[/tts:text]]`
區塊內：
...
OpenClaw 會調整電話／Meet 橋接音訊以符合 Gemini 的 PCM Live API 串流，
並讓工具呼叫沿用共用的即時語音合約。除非需要變更取樣設定，否則請勿設定 `temperature`；
OpenClaw 會略過非正值，因為使用 `temperature: 0` 時，
Google Live 可能只傳回轉錄文字而沒有音訊。
Gemini API 轉錄無須 `languageCodes` 即會啟用；目前的 Google
SDK 會拒絕此 API 路徑上的語言代碼提示。
</Note>

<Note>
Gemini 3.1 Live 可透過即時輸入接收對話文字，並使用
循序函式呼叫。OpenClaw 會針對此模型略過舊版 `NON_BLOCKING`、函式
回應排程與情感對話欄位。建議使用
`thinkingLevel`；已設定的正值 `thinkingBudget` 會對應至最接近的
支援層級，而 `-1` 則會保留 Google 的預設值。請參閱
[Gemini Live 功能比較](https://ai.google.dev/gemini-api/docs/live-api/capabilities)。
</Note>

<Note>
控制介面的 Talk 支援使用受限單次使用權杖的 Google Live 瀏覽器工作階段。
在 Video Talk 中，瀏覽器會直接將有界限的 JPEG 影格傳送至
Google Live，最高採用供應商規定的每秒一個影格。
`describe_view` 函式會回報該攝影機串流是否處於作用中。
...
此煙霧測試也涵蓋 OpenAI 後端／WebRTC 路徑；Google 測試環節會鑄造與
控制介面 Talk 相同形態的受限 Live API 權杖、開啟瀏覽器
WebSocket 端點、傳送初始設定承載資料與一個 JPEG 影格，並
驗證文字回應及 `describe_view` 函式往返。
...
<AccordionGroup>
  <Accordion title="直接重複使用 Gemini 快取">
    ...
    - 使用 `cachedContent` 或舊版 `cached_content`
      設定各模型或全域參數
    - 較明確範圍的參數（模型層級優先於全域）一律優先。
      在同一範圍內，若同時設定兩個鍵，則 `cached_content` 優先。
      每個範圍僅使用一個鍵，以避免非預期結果。
  ...
  <Accordion title="環境與常駐程式設定">
    若閘道以常駐程式（launchd/systemd）執行，請確保該程序可存取 `GEMINI_API_KEY`
    （例如位於 `~/.openclaw/.env` 中，或透過
    `env.shellEnv`）。
  </Accordion>
</AccordionGroup>
