...
title: Google (Gemini)
x-i18n:
    generated_at: "2026-06-27T19:54:34Z"
    model: gpt-5.5
...
Google 外掛透過 Google AI Studio 提供 Gemini 模型存取，並支援
...
- 執行階段選項：provider/model `agentRuntime.id: "google-gemini-cli"`
  會重用 Gemini CLI OAuth，同時讓模型參照保持標準的 `google/*`。
...
<Tabs>
  <Tab title="API 金鑰">
    **最適合：** 透過 Google AI Studio 進行標準 Gemini API 存取。
    ...
    <Tip>
    環境變數 `GEMINI_API_KEY` 和 `GOOGLE_API_KEY` 皆可使用。使用你已經設定好的那一個即可。
    </Tip>

  </Tab>

  <Tab title="Gemini CLI (OAuth)">
    **最適合：** 透過 PKCE OAuth 重用既有的 Gemini CLI 登入，而非使用獨立 API 金鑰。

    <Warning>
    `google-gemini-cli` 提供者是非官方整合。有些使用者
    回報以這種方式使用 OAuth 時會遇到帳戶限制。請自行承擔風險。
    </Warning>
    ...
    <Steps>
      <Step title="安裝 Gemini CLI">
        ...
        OpenClaw 同時支援 Homebrew 安裝與全域 npm 安裝，包含
...
[Gemini 搜尋](/zh-TW/tools/gemini-search)了解提供者特定工具行為。
...
- 輸出格式：預設為 `mp3`，在 `google/lyria-3-pro-preview` 上也支援 `wav`
- 參照輸入：最多 10 張圖片
- 由工作階段支援的執行會透過共用工作/狀態流程分離，包含 `action: "status"`
...
`generateContent` 回應中回傳生成的音訊。若要獲得最低延遲的語音對話，請使用
...
`audioProfile` 可在朗讀文字前加入可重用的風格提示。當提示文字提到具名講者時，請設定
`speakerName`。

Gemini API TTS 也接受文字中的表現性方括號音訊標籤，
例如 `[whispers]` 或 `[laughs]`。若要在將標籤傳送至 TTS 的同時，
避免它們出現在可見聊天回覆中，請將它們放在 `[[tts:text]]...[[/tts:text]]`
區塊內：
...
在共用的即時語音契約上保留工具呼叫。除非你需要變更取樣，否則請讓 `temperature`
保持未設定；OpenClaw 會省略非正值，因為 Google Live 對於 `temperature: 0`
可能會回傳沒有音訊的轉錄文字。
Gemini API 轉錄功能會在沒有 `languageCodes` 的情況下啟用；目前的 Google
SDK 會拒絕此 API 路徑上的語言代碼提示。
</Note>

<Note>
Control UI Talk 支援使用受限一次性權杖的 Google Live 瀏覽器會話。
後端專用的即時語音提供者也可以透過通用
閘道轉送傳輸執行，這會將提供者認證保留在閘道上。
</Note>

若要進行維護者即時驗證，請執行
`OPENAI_API_KEY=... GEMINI_API_KEY=... node --import tsx scripts/dev/realtime-talk-live-smoke.ts`。
此煙霧測試也涵蓋 OpenAI 後端/WebRTC 路徑；Google 部分會鑄造與
Control UI Talk 使用相同受限 Live API 權杖形狀，開啟瀏覽器
...
<AccordionGroup>
  ...
  <Accordion title="Gemini CLI usage notes">
    ...
    命令列介面 `stream-json` 輸出，並從最終
    `stats` 承載正規化用量。舊版 `--output-format json` 覆寫仍會使用
    JSON 剖析器。

    - 串流回覆文字來自助理 `message` 事件。
    - 對於舊版 JSON 輸出，回覆文字來自命令列介面 JSON `response` 欄位。
    - 當命令列介面將 `usage` 留空時，用量會退回使用 `stats`。
  ...
  <Accordion title="Environment and daemon setup">
    如果閘道以守護程式執行（launchd/systemd），請確認 `GEMINI_API_KEY`
