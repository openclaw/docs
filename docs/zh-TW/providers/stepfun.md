...
title: 階躍星辰
x-i18n:
    generated_at: "2026-07-11T21:43:58Z"
    model: gpt-5.6
...
<Warning>
標準供應商與 Step Plan 是使用不同端點和模型參照前綴（`stepfun/...` 與 `stepfun-plan/...`）的**獨立供應商**。中國金鑰應搭配 `.com` 端點使用，全球金鑰則應搭配 `.ai` 端點使用。
</Warning>
...
| `stepfun/step-3.7-flash` | 262,144 | 262,144    | 支援多模態圖片輸入           |
...
| `stepfun-plan/step-3.7-flash`      | 262,144 | 262,144    | 支援多模態圖片輸入           |
...
單次驗證流程會同時為 `stepfun` 和 `stepfun-plan` 寫入與區域相符的設定檔，因此執行一次新手設定後，便可同時探索這兩個介面。
...
<AccordionGroup>
  ...
  <Accordion title="備註">
    - `step-3.7-flash` 可透過 OpenClaw 接受文字和圖片輸入。StepFun 的 API 也支援影片，但 OpenClaw 尚未將影片納入模型輸入模態。
    - Step 3.7 支援 `low`、`medium` 和 `high` 推理強度。由於此模型沒有非推理模式，`/think off` 會對應至 `low`。
    - `step-3.5-flash-2603` 目前僅透過 `stepfun-plan` 提供。
    - 使用 `openclaw models list` 和 `openclaw models set <provider/model>` 檢視或切換模型。
