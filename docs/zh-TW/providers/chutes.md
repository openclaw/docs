...
[Chutes](https://chutes.ai) 透過與 OpenAI 相容的 API 公開開源模型目錄。OpenClaw 支援 `chutes` 提供者的瀏覽器 OAuth 與直接 API 金鑰驗證。
...
<Tabs>
  <Tab title="OAuth">
    <Steps>
      ...
      <Step title="驗證預設模型">
        入門完成後，預設模型會設為
        `chutes/zai-org/GLM-4.7-TEE`，並註冊 Chutes 靜態目錄。
      </Step>
    </Steps>
  </Tab>
  <Tab title="API 金鑰">
...
<Note>
兩種驗證路徑都會註冊 Chutes 靜態目錄，並將預設模型設為
...
當 Chutes 驗證可用時，OpenClaw 會使用該憑證查詢 Chutes 目錄，並使用探索到的模型。如果探索失敗，OpenClaw 會退回使用靜態目錄，讓入門與啟動仍能運作。

## 預設別名

OpenClaw 會為 Chutes 靜態目錄註冊三個便利別名：
