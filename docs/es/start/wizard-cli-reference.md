...
summary: Referencia completa para el flujo de configuración de la CLI, la configuración de autenticación/modelo, las salidas y los componentes internos
title: Referencia de configuración de CLI
x-i18n:
    generated_at: "2026-06-27T12:59:05Z"
    model: gpt-5.5
...
- Configuración de modelo y autenticación (OAuth de suscripción OpenAI Code, CLI de Anthropic Claude o clave de API, además de opciones de MiniMax, GLM, Ollama, Moonshot, StepFun y AI Gateway)
...
- Instalación del daemon (LaunchAgent, unidad de usuario systemd o tarea programada nativa de Windows con respaldo en carpeta de inicio)
- Comprobación de estado
- Configuración de Skills

El modo remoto configura esta máquina para conectarse a un gateway ubicado en otro lugar.
...
<Steps>
  ...
  <Step title="Gateway">
    ...
    - Recomendado: mantén la autenticación por token activada incluso para loopback, para que los clientes WS locales deban autenticarse.
  ...
  <Step title="Canales">
    ...
    - [iMessage](/es/channels/imessage): ruta de CLI `imsg` + acceso a la base de datos de Messages; usa un wrapper SSH cuando el Gateway se ejecute fuera de un Mac
  ...
  <Step title="Instalación del daemon">
    ...
    - Windows nativo: primero tarea programada
      - Si se deniega la creación de la tarea, OpenClaw recurre a un elemento de inicio de sesión por usuario en la carpeta de inicio y arranca el gateway inmediatamente.
      - Las tareas programadas siguen siendo preferibles porque ofrecen un mejor estado de supervisor.
  ...
  <Step title="Skills">
    ...
    - Te permite elegir gestor de Node: npm, pnpm o bun.
...
Si faltan los activos de Control UI, el asistente intenta compilarlos; el respaldo es `pnpm ui:build` (instala automáticamente las dependencias de UI).
</Note>
...
<AccordionGroup>
  ...
  <Accordion title="Suscripción OpenAI Code (OAuth)">
    ...
    Establece `agents.defaults.model` en `openai/gpt-5.5` mediante el runtime de Codex cuando el modelo no está establecido o ya pertenece a la familia OpenAI.
  ...
  <Accordion title="Clave de API de OpenAI">
    ...
    Establece `agents.defaults.model` en `openai/gpt-5.5` cuando el modelo no está establecido, es `openai/*` o son refs de modelo Codex heredadas.
  ...
  <Accordion title="Código de dispositivo de xAI (Grok)">
    ...
    devolución de llamada localhost. Úsalo desde hosts SSH, Docker o VPS.
  </Accordion>
  ...
  <Accordion title="MiniMax">
    La configuración se escribe automáticamente. El valor predeterminado hospedado es `MiniMax-M3`; la configuración con clave de API usa
...
- El onboarding de proveedor personalizado infiere la compatibilidad con imágenes para IDs de modelo comunes y solo pregunta cuando el nombre del modelo es desconocido.
- Cuando el onboarding empieza desde una opción de autenticación de proveedor, el selector de modelo prefiere
...
- El comportamiento predeterminado de incorporación conserva las claves de API como valores de texto sin formato en los perfiles de autenticación.
...
- En modo no interactivo, `--secret-input-mode ref` solo está respaldado por env.
  - Define la variable de entorno del proveedor en el entorno del proceso de incorporación.
  - Las marcas de clave en línea (por ejemplo `--openai-api-key`) requieren que esa variable de entorno esté definida; de lo contrario, la incorporación falla rápido.
  ...
  - En ese caso de proveedor personalizado, `--custom-api-key` requiere que `CUSTOM_API_KEY` esté definido; de lo contrario, la incorporación falla rápido.
...
- `session.dmScope` (la incorporación local establece esto de forma predeterminada en `per-channel-peer` cuando no está definido; se conservan los valores explícitos existentes)
