...
read_when:
    ...
    - Necesitas los nombres exactos de las métricas, los nombres de los tramos o las formas de los atributos para crear paneles o alertas
...
- Las **llamadas a proveedores** reciben un encabezado W3C `traceparent` desde el
  contexto de span de llamada a modelo de confianza de OpenClaw cuando el transporte del proveedor acepta encabezados
...
- Los exportadores solo se adjuntan cuando tanto la superficie de diagnósticos como el Plugin están
  habilitados, por lo que el coste en proceso se mantiene cerca de cero por defecto.
...
| **Registros** | Registros estructurados `logging.file` exportados por OTLP cuando `diagnostics.otel.logs` está habilitado.                              |

Activa o desactiva `traces`, `metrics` y `logs` de forma independiente. Los tres están activados por defecto
...
| `OTEL_SEMCONV_STABILITY_OPT_IN`                                                                                   | Establécelo en `gen_ai_latest_experimental` para emitir el atributo de span experimental más reciente de GenAI (`gen_ai.provider.name`) en lugar del heredado `gen_ai.system`. Las métricas de GenAI siempre usan atributos semánticos acotados y de baja cardinalidad de todos modos. |
| `OPENCLAW_OTEL_PRELOADED`                                                                                         | Establécelo en `1` cuando otra precarga o proceso anfitrión ya haya registrado el SDK global de OpenTelemetry. Entonces el Plugin omite su propio ciclo de vida de NodeSDK, pero sigue conectando escuchadores de diagnóstico y respeta `traces`/`metrics`/`logs`. |

## Privacidad y captura de contenido

El contenido sin procesar de modelos/herramientas **no** se exporta por defecto. Los spans llevan
...
genera solo a partir del contexto de traza de diagnóstico propiedad de OpenClaw para la llamada a modelo
activa. Los encabezados `traceparent` existentes suministrados por el llamador se reemplazan, por lo que los Plugins o
las opciones de proveedor personalizadas no pueden suplantar la ascendencia de trazas entre servicios.
...
Cada subclave es de inclusión voluntaria de forma independiente:
...
- **Correlación de solicitudes:** las solicitudes HTTP del Gateway y los marcos WebSocket crean un
  ...
  heredan la traza de solicitud por defecto, mientras que los spans de ejecución de agente y llamada a modelo se
...
- `openclaw.model_call.request_bytes` (histograma, tamaño en bytes UTF-8 de la carga útil final de solicitud al modelo; sin contenido de carga útil sin procesar)
- `openclaw.model_call.response_bytes` (histograma, tamaño en bytes UTF-8 de eventos de respuesta del modelo transmitidos; sin contenido de respuesta sin procesar)
...
mientras OpenClaw observe progreso de respuesta, herramienta, estado, bloque o tiempo de ejecución ACP.
Los mantenimientos de escritura no cuentan como progreso, por lo que un modelo o arnés silencioso
aún puede detectarse.

OpenClaw clasifica las sesiones según el trabajo que todavía puede observar:

- `session.long_running`: trabajo insertado activo, llamadas al modelo o llamadas a herramientas
  aún avanzan.
- `session.stalled`: existe trabajo activo, pero la ejecución activa no ha informado
  progreso reciente. Las ejecuciones insertadas estancadas permanecen primero en modo solo observación y luego
  abortan y drenan después de al menos 10 minutos y 5 veces `diagnostics.stuckSessionWarnMs`
...
Los diagnósticos repetidos de `session.stuck` aplican retroceso mientras la sesión permanezca
...
contenido específicas que hayas habilitado.
...
- `exec.process.completed` — resultado de terminal, duración, destino, modo, código
  de salida y tipo de fallo. El texto de comando y los directorios de trabajo no se
  incluyen.
...
Las marcas no distinguen mayúsculas y minúsculas y admiten comodines (por ejemplo, `telegram.*` o
`*`):
...
O como una sobrescritura de entorno de uso único:

```bash
OPENCLAW_DIAGNOSTICS=telegram.http,telegram.payload openclaw gateway
```

La salida de marcas va al archivo de registro estándar (`logging.file`) y sigue estando
...
- [Registro](/es/logging) — registros de archivo, salida de consola, seguimiento por CLI y la pestaña Logs de la interfaz de Control
