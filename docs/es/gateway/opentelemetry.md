---
read_when:
    - Quieres enviar el uso del modelo de OpenClaw, el flujo de mensajes o las métricas de sesión a un colector de OpenTelemetry
...
summary: Exporta los diagnósticos de OpenClaw a cualquier colector OpenTelemetry mediante el Plugin diagnostics-otel (OTLP/HTTP)
title: Exportación de OpenTelemetry
x-i18n:
    generated_at: "2026-05-05T05:23:02Z"
    model: gpt-5.5
    provider: openai
...
funciona sin cambios de código. Para registros en archivos locales y cómo leerlos, consulta
[Registro](/es/logging).
...
- Las **llamadas al proveedor** reciben un encabezado W3C `traceparent` desde el contexto
  del tramo de llamada al modelo de confianza de OpenClaw cuando el transporte del proveedor acepta encabezados
  personalizados. El contexto de traza emitido por el Plugin no se propaga.
- Los exportadores solo se conectan cuando tanto la superficie de diagnósticos como el Plugin están
  habilitados, por lo que el costo en proceso se mantiene cerca de cero de forma predeterminada.
...
| Señal       | Qué contiene                                                                                                                                         |
| ----------- | ---------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Métricas** | Contadores e histogramas para uso de tokens, costo, duración de ejecución, flujo de mensajes, carriles de cola, estado de sesión, ejecución y presión de memoria. |
| **Trazas**  | Tramos para uso de modelo, llamadas a modelo, ciclo de vida del arnés, ejecución de herramientas, ejecución, procesamiento de webhook/mensajes, ensamblado de contexto y bucles de herramientas. |
...
| `OTEL_SEMCONV_STABILITY_OPT_IN`                                                                                   | Establécela en `gen_ai_latest_experimental` para emitir el atributo de tramo GenAI experimental más reciente (`gen_ai.provider.name`) en lugar del `gen_ai.system` heredado. Las métricas GenAI siempre usan atributos semánticos acotados y de baja cardinalidad de todos modos. |
| `OPENCLAW_OTEL_PRELOADED`                                                                                         | Establécela en `1` cuando otra precarga o proceso anfitrión ya haya registrado el SDK global de OpenTelemetry. El Plugin entonces omite su propio ciclo de vida de NodeSDK, pero sigue conectando los escuchadores de diagnóstico y respeta `traces`/`metrics`/`logs`. |
...
genera solo a partir del contexto de traza de diagnóstico propiedad de OpenClaw para la llamada al modelo
...
Establece `diagnostics.otel.captureContent.*` en `true` solo cuando tu colector y
...
- `toolInputs` — cargas útiles de argumentos de herramienta.
- `toolOutputs` — cargas útiles de resultados de herramienta.
- `systemPrompt` — prompt de sistema/desarrollador ensamblado.

Cuando cualquier subclave está habilitada, los tramos de modelo y herramienta reciben atributos
...
- **Registros:** los registros OTLP respetan `logging.level` (nivel de registro de archivo). Usan la
  ...
  deberían preferir el muestreo/filtrado del colector OTLP sobre el muestreo local.
...
- `openclaw.model_call.request_bytes` (histograma, tamaño en bytes UTF-8 de la carga útil final de solicitud al modelo; sin contenido bruto de la carga útil)
- `openclaw.model_call.response_bytes` (histograma, tamaño en bytes UTF-8 de eventos de respuesta del modelo transmitidos; sin contenido bruto de la respuesta)
...
actividad de sesión. Una sesión `processing` no envejece hacia este umbral
mientras OpenClaw observa progreso de respuesta, herramienta, estado, bloque o runtime ACP.
Los keepalives de escritura no cuentan como progreso, por lo que un modelo o arnés silencioso aún puede
detectarse.

OpenClaw clasifica las sesiones según el trabajo que todavía puede observar:

- `session.long_running`: trabajo integrado activo, llamadas al modelo o llamadas a herramientas
  siguen progresando.
- `session.stalled`: existe trabajo activo, pero la ejecución activa no ha informado
  progreso reciente. Las ejecuciones integradas detenidas quedan inicialmente en modo de solo observación y luego
  se abortan y drenan después de `diagnostics.stuckSessionAbortMs` sin progreso para que los turnos
  en cola detrás del carril puedan reanudarse. Cuando no está configurado, el umbral de aborto usa de forma predeterminada
  la ventana extendida más segura de al menos 10 minutos y 5 veces
  `diagnostics.stuckSessionWarnMs`.
- `session.stuck`: contabilidad obsoleta de sesión sin trabajo activo. Esto libera
...
sin cambios, por lo que los paneles deben alertar sobre aumentos sostenidos en lugar de en cada
...
- `openclaw.model.usage`
  ...
  - `gen_ai.system` de forma predeterminada, o `gen_ai.provider.name` cuando se opta por las convenciones semánticas más recientes de GenAI
...
- `harness.run.started` / `harness.run.completed` / `harness.run.error` —
  ciclo de vida por ejecución para el arnés de agente. Incluye `harnessId`, `pluginId`
...
Para obtener salida de depuración dirigida sin elevar `logging.level`, usa indicadores de diagnóstico.
...
O como una anulación de entorno de una sola vez:
...
- [Registro](/es/logging) — registros de archivo, salida de consola, seguimiento desde la CLI y la pestaña Registros de la interfaz de control
- [Componentes internos de registro del Gateway](/es/gateway/logging) — estilos de registro WS, prefijos de subsistema y captura de consola
