...
Para agentes, herramientas, tiempo de ejecución del Gateway y otras claves de nivel superior, consulta
...
Si falta por completo un bloque de proveedor (`channels.<provider>` ausente), la política de grupo en tiempo de ejecución recurre a `allowlist` (cerrado por defecto) con una advertencia al inicio.
</Note>
...
- `channels.defaults.heartbeat.useIndicator`: representa la salida compacta de Heartbeat con estilo de indicador.
...
- El valor opcional `channels.whatsapp.defaultAccount` sobrescribe esa selección predeterminada de cuenta de respaldo cuando coincide con un id de cuenta configurado.
...
- `apiRoot` es solo la raíz de la API de bots de Telegram. Usa `https://api.telegram.org` o tu raíz autoalojada/de proxy, no `https://api.telegram.org/bot<TOKEN>`; `openclaw doctor --fix` elimina un sufijo accidental `/bot<TOKEN>` al final.
...
- En configuraciones de múltiples cuentas (2 o más ids de cuenta), establece un valor predeterminado explícito (`channels.telegram.defaultAccount` o `channels.telegram.accounts.default`) para evitar el enrutamiento de respaldo; `openclaw doctor` advierte cuando falta o no es válido.
...
- Las entradas `bindings[]` de nivel superior con `type: "acp"` configuran enlaces ACP persistentes para temas de foro (usa el formato canónico `chatId:topic:topicId` en `match.peer.id`). La semántica de los campos se comparte en [Agentes ACP](/es/tools/acp-agents#persistent-channel-bindings).
...
- Token: `channels.discord.token`, con `DISCORD_BOT_TOKEN` como valor alternativo para la cuenta predeterminada.
...
- Usa `user:<id>` (DM) o `channel:<id>` (canal del servidor) para los destinos de entrega; los ID numéricos sin prefijo se rechazan.
- Los slugs de servidor están en minúsculas con los espacios reemplazados por `-`; las claves de canal usan el nombre convertido a slug (sin `#`). Prefiere los ID de servidor.
...
- `channels.discord.guilds.<id>.ignoreOtherMentions` (y las sobrescrituras de canal) descarta mensajes que mencionen a otro usuario o rol, pero no al bot (excluyendo @everyone/@here).
- `channels.discord.mentionAliases` asigna texto `@handle` saliente estable a ID de usuario de Discord antes de enviar, para que se pueda mencionar de forma determinista a compañeros conocidos aunque la caché transitoria del directorio esté vacía. Las sobrescrituras por cuenta viven en `channels.discord.accounts.<accountId>.mentionAliases`.
...
- `channels.discord.threadBindings` controla el enrutamiento vinculado a hilos de Discord:
  ...
  - `maxAgeHours`: sobrescritura de Discord para edad máxima rígida en horas (`0` lo deshabilita)
  - `spawnSessions`: interruptor para `sessions_spawn({ thread: true })` y creación/vinculación automática de hilos con generación de hilos de ACP (predeterminado: `true`)
...
- `channels.discord.ui.components.accentColor` establece el color de énfasis para los contenedores v2 de componentes de Discord.
- `channels.discord.voice` habilita conversaciones en canales de voz de Discord y sobrescrituras opcionales de auto-unión + LLM + TTS. Las configuraciones de Discord solo de texto dejan la voz desactivada de forma predeterminada; establece `channels.discord.voice.enabled=true` para activarla.
...
- `channels.discord.voice.connectTimeoutMs` controla la espera inicial Ready de `@discordjs/voice` para intentos de `/vc join` y auto-unión (`30000` de forma predeterminada).
...
- `channels.discord.autoPresence` asigna la disponibilidad del runtime a la presencia del bot (healthy => online, degraded => idle, exhausted => dnd) y permite sobrescrituras opcionales de texto de estado.
...
- `channels.discord.execApprovals`: entrega nativa de Discord de aprobaciones de ejecución y autorización de aprobadores.
  ...
  - `agentFilter`: allowlist opcional de ID de agente. Omítelo para reenviar aprobaciones de todos los agentes.
  - `sessionFilter`: patrones opcionales de clave de sesión (subcadena o regex).
  - `target`: dónde enviar los avisos de aprobación. `"dm"` (predeterminado) envía a los DM de aprobadores, `"channel"` envía al canal de origen, `"both"` envía a ambos. Cuando el destino incluye `"channel"`, los botones solo pueden usarlos los aprobadores resueltos.
...
- Valores alternativos de entorno: `GOOGLE_CHAT_SERVICE_ACCOUNT` o `GOOGLE_CHAT_SERVICE_ACCOUNT_FILE`.
...
- **Modo Socket** requiere tanto `botToken` como `appToken` (`SLACK_BOT_TOKEN` + `SLACK_APP_TOKEN` para el valor alternativo de entorno de la cuenta predeterminada).
- **Modo HTTP** requiere `botToken` además de `signingSecret` (en la raíz o por cuenta).
- `socketMode` pasa el ajuste de transporte Socket Mode del SDK de Slack a la API pública del receptor Bolt. Úsalo solo al investigar timeouts de ping/pong o comportamiento de websocket obsoleto.
...
- `channels.slack.streaming.mode` es la clave canónica del modo de transmisión de Slack. `channels.slack.streaming.nativeTransport` controla el transporte de streaming nativo de Slack. Los valores heredados `streamMode`, booleanos `streaming` y `nativeStreaming` se migran automáticamente.
...
- `channels.slack.execApprovals`: entrega nativa de Slack de aprobaciones de ejecución y autorización de aprobadores. El mismo esquema que Discord: `enabled` (`true`/`false`/`"auto"`), `approvers` (ID de usuario de Slack), `agentFilter`, `sessionFilter` y `target` (`"dm"`, `"channel"` o `"both"`).
...
Mattermost se distribuye como Plugin incluido en las versiones actuales de OpenClaw. Las compilaciones antiguas o
...
- Las entradas `bindings[]` de nivel superior con `type: "acp"` pueden vincular conversaciones de BlueBubbles a sesiones ACP persistentes. Use un identificador de BlueBubbles o una cadena de destino (`chat_id:*`, `chat_guid:*`, `chat_identifier:*`) en `match.peer.id`. Semántica de campos compartidos: [Agentes ACP](/es/tools/acp-agents#persistent-channel-bindings).
...
- `cliPath` puede apuntar a un wrapper SSH; configure `remoteHost` (`host` o `user@host`) para la obtención de adjuntos por SCP.
...
- SCP usa comprobación estricta de claves de host, así que asegúrese de que la clave del host de retransmisión ya exista en `~/.ssh/known_hosts`.
...
- `channels.matrix.autoJoin` tiene `off` como valor predeterminado, así que las salas con invitación y las invitaciones nuevas de estilo DM se ignoran hasta que configure `autoJoin: "allowlist"` con `autoJoinAllowlist` o `autoJoin: "always"`.
- `channels.matrix.execApprovals`: entrega de aprobaciones de ejecución nativa de Matrix y autorización de aprobadores.
  ...
  - `agentFilter`: lista de permitidos opcional de ids de agente. Omítala para reenviar aprobaciones de todos los agentes.
...
- `channels.matrix.dm.sessionScope` controla cómo los DM de Matrix se agrupan en sesiones: `per-user` (predeterminado) comparte por par enrutado, mientras que `per-room` aísla cada sala DM.
...
- Si agrega una cuenta no predeterminada mediante `openclaw channels add` (o el onboarding del canal) mientras aún usa una configuración de canal de nivel superior de una sola cuenta, OpenClaw primero promueve los valores de una sola cuenta de nivel superior con alcance de cuenta al mapa de cuentas del canal para que la cuenta original siga funcionando. La mayoría de los canales los mueven a `channels.<channel>.accounts.default`; Matrix puede conservar en su lugar un destino existente con nombre/predeterminado que coincida.
...
- `openclaw doctor --fix` también repara formas mixtas moviendo los valores de una sola cuenta de nivel superior con alcance de cuenta a la cuenta promovida elegida para ese canal. La mayoría de los canales usan `accounts.default`; Matrix puede conservar en su lugar un destino existente con nombre/predeterminado que coincida.
...
Las respuestas visibles se controlan por separado. Las salas de grupo/canal tienen `messages.groupChat.visibleReplies: "message_tool"` como valor predeterminado: OpenClaw sigue procesando el turno, pero las respuestas finales normales se mantienen privadas y la salida visible en la sala requiere `message(action=send)`. Configure `"automatic"` solo cuando quiera el comportamiento heredado donde las respuestas normales se publican de vuelta en la sala. Para aplicar el mismo comportamiento de respuesta visible solo mediante herramienta también a los chats directos, configure `messages.visibleReplies: "message_tool"`; el arnés de Codex también usa ese comportamiento solo mediante herramienta como valor predeterminado sin configurar para chats directos.

Las respuestas visibles solo mediante herramienta requieren un modelo/tiempo de ejecución que llame a herramientas de forma fiable. Si
el registro de sesión muestra texto del asistente con `didSendViaMessagingTool: false`, el
modelo produjo una respuesta final privada en lugar de llamar a la herramienta de mensajes.
Cambie a un modelo más sólido en llamadas a herramientas para ese canal, o configure
`messages.groupChat.visibleReplies: "automatic"` para restaurar las respuestas finales
visibles heredadas.

Si la herramienta de mensajes no está disponible bajo la política de herramientas activa, OpenClaw recurre a respuestas visibles automáticas en lugar de suprimir la respuesta silenciosamente. `openclaw doctor` advierte sobre esta incompatibilidad.

El Gateway recarga en caliente la configuración de `messages` después de guardar el archivo. Reinicie solo cuando la observación de archivos o la recarga de configuración esté deshabilitada en el despliegue.

**Tipos de mención:**

- **Menciones de metadatos**: @-menciones nativas de la plataforma. Se ignoran en el modo de chat consigo mismo de WhatsApp.
- **Patrones de texto**: Patrones regex seguros en `agents.list[].groupChat.mentionPatterns`. Se ignoran los patrones no válidos y la repetición anidada no segura.
...
`messages.visibleReplies` es el valor predeterminado global de turnos de origen; `messages.groupChat.visibleReplies` lo sobrescribe para turnos de origen de grupo/canal. Cuando `messages.visibleReplies` no está definido, un arnés puede proporcionar su propio valor predeterminado directo/de origen; el arnés de Codex usa `message_tool` de forma predeterminada. Las listas de permisos de canales y la compuerta de menciones siguen decidiendo si se procesa un turno.
...
Incluye tu propio número en `allowFrom` para habilitar el modo de chat consigo mismo (ignora las @-menciones nativas, solo responde a patrones de texto):
...
- Sobrescritura por canal: `channels.discord.commands.native` (booleano o `"auto"`). En Discord, `false` omite el registro y la limpieza de comandos nativos durante el inicio.
...
- `config: true` habilita `/config` (lee/escribe `openclaw.json`). Para clientes `chat.send` de Gateway, las escrituras persistentes de `/config set|unset` también requieren `operator.admin`; `/config show` de solo lectura sigue disponible para clientes operadores normales con ámbito de escritura.
...
- Para canales con varias cuentas, `channels.<provider>.accounts.<id>.configWrites` también controla escrituras dirigidas a esa cuenta (por ejemplo, `/allowlist --config --account <id>` o `/config set channels.<provider>.accounts.<id>...`).
- `restart: false` deshabilita `/restart` y las acciones de la herramienta de reinicio de Gateway. Valor predeterminado: `true`.
- `ownerAllowFrom` es la lista de permisos explícita del propietario para comandos/herramientas solo para propietario. Es independiente de `allowFrom`.
- `ownerDisplay: "hash"` aplica hash a los id de propietario en el prompt del sistema. Establece `ownerDisplaySecret` para controlar el hash.
- `allowFrom` es por proveedor. Cuando está definido, es la **única** fuente de autorización (se ignoran las listas de permisos/emparejamiento de canales y `useAccessGroups`).
...
- Mapa de documentación de comandos:
  - catálogo incorporado y empaquetado: [Comandos de barra](/es/tools/slash-commands)
