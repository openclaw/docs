...
read_when:
    ...
    - Resolución de problemas de claves de configuración por canal
...
Si falta por completo un bloque de proveedor (`channels.<provider>` ausente), la política de grupo en tiempo de ejecución vuelve a `allowlist` (fallo cerrado) con una advertencia de inicio.
</Note>
...
- Token de bot: `channels.telegram.botToken` o `channels.telegram.tokenFile` (solo archivo normal; se rechazan los enlaces simbólicos), con `TELEGRAM_BOT_TOKEN` como reserva para la cuenta predeterminada.
...
- En configuraciones multicuenta (2 o más ids de cuenta), establece un valor predeterminado explícito (`channels.telegram.defaultAccount` o `channels.telegram.accounts.default`) para evitar el enrutamiento de reserva; `openclaw doctor` advierte cuando falta o no es válido.
...
- El valor opcional `channels.discord.defaultAccount` anula la selección de cuenta predeterminada cuando coincide con un id de cuenta configurada.
- Usa `user:<id>` (DM) o `channel:<id>` (canal de servidor) para los destinos de entrega; los ID numéricos sin prefijo se rechazan.
- Los slugs de servidor están en minúsculas y sustituyen los espacios por `-`; las claves de canal usan el nombre convertido a slug (sin `#`). Prefiere los ID de servidor.
...
- `channels.discord.threadBindings` controla el enrutamiento de Discord vinculado a hilos:
  ...
  - `spawnSessions`: interruptor para `sessions_spawn({ thread: true })` y la creación/vinculación automática de hilos al generar hilos de ACP (predeterminado: `true`)
  - `defaultSpawnContext`: contexto nativo de subagente para generaciones vinculadas a hilos (`"fork"` de forma predeterminada)
- Las entradas de nivel superior `bindings[]` con `type: "acp"` configuran vinculaciones persistentes de ACP para canales e hilos (usa el id de canal/hilo en `match.peer.id`). La semántica de los campos se comparte en [Agentes ACP](/es/tools/acp-agents#channel-specific-settings).
- `channels.discord.ui.components.accentColor` define el color de énfasis para los contenedores de componentes v2 de Discord.
- `channels.discord.voice` habilita conversaciones en canales de voz de Discord y anulaciones opcionales de unión automática + LLM + TTS. Las configuraciones de Discord solo de texto dejan la voz desactivada de forma predeterminada; define `channels.discord.voice.enabled=true` para activarla.
...
- Además, OpenClaw intenta recuperar la recepción de voz saliendo y volviendo a unirse a una sesión de voz tras fallos repetidos de descifrado.
...
- `channels.discord.execApprovals`: entrega de aprobación de exec nativa de Discord y autorización de aprobadores.
...
**Modos de notificación de reacción:** `off` (ninguna), `own` (mensajes del bot, predeterminado), `all` (todos los mensajes), `allowlist` (de `guilds.<id>.users` en todos los mensajes).
...
- `socketMode` pasa el ajuste de transporte Socket Mode del SDK de Slack a la API pública del receptor Bolt. Úsalo solo al investigar tiempos de espera de ping/pong o comportamiento de websocket obsoleto.
...
- La transmisión nativa de Slack más el estado de hilo de estilo asistente de Slack "is typing..." requieren un destino de hilo de respuesta. Los DM de nivel superior permanecen fuera de hilo de forma predeterminada, así que usan `typingReaction` o entrega normal en lugar de la vista previa de estilo hilo.
...
- `channels.slack.execApprovals`: entrega de aprobación de exec nativa de Slack y autorización de aprobadores. El mismo esquema que Discord: `enabled` (`true`/`false`/`"auto"`), `approvers` (ID de usuario de Slack), `agentFilter`, `sessionFilter` y `target` (`"dm"`, `"channel"` o `"both"`).
...
Mattermost se distribuye como plugin incluido en las versiones actuales de OpenClaw. Las compilaciones antiguas o personalizadas pueden instalar un paquete npm actual con `openclaw plugins install @openclaw/mattermost`; si npm informa que el paquete propiedad de OpenClaw está obsoleto, usa el plugin incluido o una copia local hasta que se publique un paquete npm más reciente.
...
Modos de chat: `oncall` (responde al mencionar con @, predeterminado), `onmessage` (cada mensaje), `onchar` (mensajes que empiezan con el prefijo disparador).
...
- `channels.mattermost.requireMention`: exige `@mention` antes de responder en canales.
...
OpenClaw inicia `imsg rpc` (JSON-RPC por stdio). No requiere daemon ni puerto.
...
- `cliPath` puede apuntar a un wrapper SSH; define `remoteHost` (`host` o `user@host`) para obtener adjuntos por SCP.
...
- SCP usa verificación estricta de claves de host, así que asegúrate de que la clave del host relé ya exista en `~/.ssh/known_hosts`.
...
Matrix está respaldado por plugin y configurado en `channels.matrix`.
...
- `channels.matrix.autoJoin` tiene `off` como valor predeterminado, así que las salas invitadas y las invitaciones nuevas de estilo DM se ignoran hasta que definas `autoJoin: "allowlist"` con `autoJoinAllowlist` o `autoJoin: "always"`.
...
Microsoft Teams está respaldado por plugin y configurado en `channels.msteams`.
...
IRC está respaldado por plugin y configurado en `channels.irc`.
...
- Si agregas una cuenta no predeterminada mediante `openclaw channels add` (u onboarding de canal) mientras sigues usando una configuración de canal de nivel superior de cuenta única, OpenClaw primero promueve los valores de cuenta única de nivel superior con alcance de cuenta al mapa de cuentas del canal para que la cuenta original siga funcionando. La mayoría de los canales los mueven a `channels.<channel>.accounts.default`; Matrix puede conservar un destino existente coincidente con nombre/predeterminado en su lugar.
...
- `openclaw doctor --fix` también repara formas mixtas moviendo los valores de cuenta única de nivel superior con alcance de cuenta a la cuenta promovida elegida para ese canal. La mayoría de los canales usan `accounts.default`; Matrix puede conservar un destino existente coincidente con nombre/predeterminado en su lugar.
...
Los mensajes de grupo por defecto **requieren mención** (mención de metadatos o patrones regex seguros). Se aplica a chats de grupo de WhatsApp, Telegram, Discord, Google Chat e iMessage.

Las respuestas visibles se controlan por separado. Las salas de grupo/canal usan `messages.groupChat.visibleReplies: "message_tool"` como valor predeterminado: OpenClaw sigue procesando el turno, pero las respuestas finales normales permanecen privadas y la salida visible en la sala requiere `message(action=send)`. Define `"automatic"` solo cuando quieras el comportamiento heredado en el que las respuestas normales se publican de vuelta en la sala. Para aplicar el mismo comportamiento de respuesta visible solo con herramienta también a los chats directos, define `messages.visibleReplies: "message_tool"`; el arnés de Codex también usa ese comportamiento solo con herramienta como valor predeterminado sin definir para chats directos.
...
- **Menciones de metadatos**: @-mentions nativas de la plataforma. Se ignoran en el modo de self-chat de WhatsApp.
...
- `nativeSkills: "auto"` activa comandos de Skills nativos para Discord/Telegram y deja Slack desactivado.
- Sobrescribe por canal: `channels.discord.commands.native` (booleano o `"auto"`). `false` borra los comandos registrados anteriormente.
...
- `config: true` habilita `/config` (lee/escribe `openclaw.json`). Para clientes de Gateway `chat.send`, las escrituras persistentes de `/config set|unset` también requieren `operator.admin`; `/config show` de solo lectura sigue disponible para clientes de operador normales con alcance de escritura.
...
- Mapa de documentación de comandos:
  - catálogo integrado y empaquetado: [Comandos de barra](/es/tools/slash-commands)
