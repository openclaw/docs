...
summary: Consideraciones de seguridad y modelo de amenazas para ejecutar un Gateway de IA con acceso al intérprete de comandos
title: Seguridad
x-i18n:
    generated_at: "2026-05-06T05:36:16Z"
    model: gpt-5.5
    provider: openai
...
<Warning>
  **Modelo de confianza de asistente personal.** Esta guía asume un límite de
  ...
  usuarios adversariales que comparten un agente o Gateway. Si necesitas operar con
  confianza mixta o usuarios adversariales, separa los límites de confianza (Gateway +
...
- Postura de seguridad admitida: un usuario/límite de confianza por Gateway (preferiblemente un usuario de SO/host/VPS por límite).
- No es un límite de seguridad admitido: un Gateway/agente compartido usado por usuarios mutuamente no confiables o adversariales.
- Si se requiere aislamiento de usuarios adversariales, separa por límite de confianza (Gateway + credenciales separados, e idealmente usuarios/hosts de SO separados).
...
`security audit --fix` se mantiene intencionadamente acotado: cambia políticas de grupo abiertas comunes a listas de permitidos, restaura `logging.redactSensitive: "tools"`, endurece los permisos de estado/configuración/archivos incluidos y usa restablecimientos de ACL de Windows en lugar de `chmod` POSIX cuando se ejecuta en Windows.
...
- Valor recomendado por defecto: un usuario por máquina/host (o VPS), un Gateway para ese usuario y uno o más agentes en ese Gateway.
...
OpenClaw usa `@openclaw/fs-safe` para acceso a archivos limitado por raíz, escrituras atómicas, extracción de archivos comprimidos, espacios de trabajo temporales y helpers de archivos secretos. OpenClaw desactiva por defecto el helper POSIX Python opcional de fs-safe; configura `OPENCLAW_FS_SAFE_PYTHON_MODE=auto` o `require` solo cuando quieras el endurecimiento adicional de mutación relativa a fd y puedas admitir un runtime de Python.

Detalles: [Operaciones de archivo seguras](/es/gateway/security/secure-file-operations).
...
- la inyección de prompt/contenido de un remitente puede causar acciones que afecten estado, dispositivos o salidas compartidos;
...
Esto es aceptable cuando todas las personas que usan ese agente están en el mismo límite de confianza (por ejemplo, un equipo de una empresa) y el agente está estrictamente acotado al negocio.
...
- Un llamador autenticado ante el Gateway es de confianza en el alcance del Gateway. Después del emparejamiento, las acciones de Node son acciones de operador de confianza en ese Node.
- Los niveles de alcance del operador y las comprobaciones en tiempo de aprobación se resumen en
  [Alcances de operador](/es/gateway/operator-scopes).
- Los clientes backend de loopback directos autenticados con el token/contraseña compartido del Gateway pueden hacer RPC internos del plano de control sin presentar una identidad de dispositivo de usuario. Esto no es una omisión del emparejamiento remoto o de navegador: los clientes de red, clientes de Node, clientes con token de dispositivo e identidades de dispositivo explícitas siguen pasando por el emparejamiento y la aplicación de mejoras de alcance.
...
- El valor por defecto del producto OpenClaw para configuraciones confiables de un solo operador es que la ejecución de host en `gateway`/`node` está permitida sin solicitudes de aprobación (`security="full"`, `ask="off"` salvo que lo endurezcas). Ese valor por defecto es UX intencionada, no una vulnerabilidad por sí mismo.
- Las aprobaciones de exec vinculan el contexto exacto de la solicitud y los operandos de archivos locales directos con el mejor esfuerzo; no modelan semánticamente cada ruta de carga de runtime/intérprete. Usa sandboxing y aislamiento del host para límites fuertes.
...
| Shell `!` de TUI local                                    | Ejecución local explícitamente activada por el operador | "El comando práctico de shell local es inyección remota"                     |
| Emparejamiento de Node y comandos de Node                 | Ejecución remota a nivel de operador en dispositivos emparejados | "El control de dispositivos remotos debe tratarse como acceso de usuario no confiable por defecto" |
...
- Reportes que tratan `gateway.nodes.pairing.autoApproveCidrs` configurado como una vulnerabilidad por sí mismo. Esta opción está deshabilitada por defecto, requiere entradas CIDR/IP explícitas, solo se aplica al emparejamiento inicial con `role: node` sin alcances solicitados y no aprueba automáticamente operador/navegador/Control UI, WebChat, mejoras de rol, mejoras de alcance, cambios de metadatos, cambios de clave pública ni rutas de encabezado de proxy de confianza por loopback en el mismo host salvo que la autenticación de proxy de confianza por loopback se haya habilitado explícitamente.
...
Esto mantiene el Gateway solo local, aísla los DM y deshabilita por defecto las herramientas del plano de control/runtime.

## Regla rápida para bandeja de entrada compartida

Si más de una persona puede enviar un DM a tu bot:
...
- Esto endurece bandejas de entrada cooperativas/compartidas, pero no está diseñado como aislamiento hostil entre coinquilinos cuando los usuarios comparten acceso de escritura al host/configuración.
...
Las listas de permitidos controlan activaciones y autorización de comandos. La opción `contextVisibility` controla cómo se filtra el contexto complementario (respuestas citadas, raíces de hilo, historial obtenido):

- `contextVisibility: "all"` (predeterminado) conserva el contexto suplementario tal como se recibió.
- `contextVisibility: "allowlist"` filtra el contexto suplementario para enviarlo a remitentes permitidos por las comprobaciones activas de la allowlist.
...
- Las afirmaciones que solo muestran que "el modelo puede ver texto citado o histórico de remitentes no incluidos en la allowlist" son hallazgos de endurecimiento que se pueden abordar con `contextVisibility`, no omisiones de límites de autenticación o sandbox por sí solas.
...
- **Desviación de aprobación de ejecución** (`security=full`, `autoAllowSkills`, allowlists de intérpretes sin `strictInlineEval`): ¿las barreras de ejecución en host siguen haciendo lo que crees que hacen?
  - `security="full"` es una advertencia amplia de postura, no una prueba de un bug. Es el valor predeterminado elegido para configuraciones de asistente personal de confianza; endurécelo solo cuando tu modelo de amenazas necesite aprobación o barreras de allowlist.
...
- **Desviación de políticas/configuración incorrecta** (opciones de sandbox docker configuradas pero modo sandbox desactivado; patrones `gateway.nodes.denyCommands` ineficaces porque la coincidencia es solo por nombre exacto de comando (por ejemplo `system.run`) y no inspecciona el texto de shell; entradas `gateway.nodes.allowCommands` peligrosas; `tools.profile="minimal"` global anulado por perfiles por agente; herramientas propiedad de plugins alcanzables bajo una política de herramientas permisiva).
- **Desviación de expectativas de runtime** (por ejemplo, asumir que la ejecución implícita todavía significa `sandbox` cuando `tools.exec.host` ahora usa `auto` por defecto, o establecer explícitamente `tools.exec.host="sandbox"` mientras el modo sandbox está desactivado).
- **Higiene del modelo** (advierte cuando los modelos configurados parecen antiguos; no es un bloqueo estricto).

Si ejecutas `--deep`, OpenClaw también intenta una prueba en vivo del Gateway de mejor esfuerzo.
...
1. **Cualquier cosa "abierta" + herramientas habilitadas**: bloquea primero los DM/grupos (emparejamiento/allowlists), luego endurece la política de herramientas/sandboxing.
...
desactiva por completo las comprobaciones de identidad de dispositivo. Esto es una degradación severa de seguridad;
mantenlo desactivado salvo que estés depurando activamente y puedas revertirlo rápido.

Por separado de esas flags peligrosas, `gateway.auth.mode: "trusted-proxy"` exitoso
...
Las cabeceras de proxy de confianza no hacen que el emparejamiento de dispositivos de nodo sea automáticamente de confianza.
`gateway.nodes.pairing.autoApproveCidrs` es una política de operador separada y desactivada por defecto.
Incluso cuando está habilitada, las rutas de cabecera de trusted-proxy con origen loopback
se excluyen de la aprobación automática de nodos porque los llamantes locales pueden falsificar esas
cabeceras, incluso cuando la autenticación trusted-proxy loopback está habilitada explícitamente.
...
- Para despliegues de Control UI que no sean loopback, `gateway.controlUi.allowedOrigins` es obligatorio por defecto.
...
- Los fallos de autenticación de origen de navegador en loopback siguen teniendo limitación de tasa incluso cuando la
  exención general de loopback está habilitada, pero la clave de bloqueo tiene alcance por
  valor normalizado de `Origin` en lugar de un bucket localhost compartido.
...
- Trata el DNS rebinding y el comportamiento de cabeceras de host de proxy como preocupaciones de endurecimiento de despliegue; mantén `trustedProxies` estricto y evita exponer el gateway directamente a internet público.
...
- El Gateway aplica una política global básica de comandos de Node mediante `gateway.nodes.allowCommands` / `denyCommands`.
...
- Que un Node emparejado que se reconecta anuncie una lista de comandos distinta no es, por sí solo, una vulnerabilidad si la política global del Gateway y las aprobaciones locales de ejecución del Node siguen imponiendo el límite real de ejecución.
...
- **Nodes remotos**: conectar un Node de macOS puede hacer que Skills exclusivas de macOS sean elegibles (según la comprobación de binarios).
...
- Intentar engañar a tu IA para que haga cosas perjudiciales
- Usar ingeniería social para obtener acceso a tus datos
...
Los comandos de barra diagonal y las directivas solo se respetan para **remitentes autorizados**. La autorización se deriva de
...
y [Comandos de barra diagonal](/es/tools/slash-commands)). Si una lista de permitidos de canal está vacía o incluye `"*"`,
...
Las ediciones impulsadas por agentes de `gateway config.apply` y `gateway config.patch`
fallan cerradas por defecto: solo un conjunto reducido de rutas de prompt, modelo y activación por mención
son ajustables por agentes. Por tanto, los nuevos árboles de configuración sensibles quedan protegidos
...
- Si instalas o actualizas Plugins (`openclaw plugins install <package>`, `openclaw plugins update <id>`), trátalo como ejecutar código no confiable:
  ...
  - Las instalaciones de Plugins con npm y git ejecutan la convergencia de dependencias del gestor de paquetes solo durante el flujo explícito de instalación/actualización. Las rutas locales y los archivos se tratan como paquetes de Plugin autónomos; OpenClaw los copia/referencia sin ejecutar `npm install`.
  ...
  - `--dangerously-force-unsafe-install` es solo para casos de emergencia ante falsos positivos del análisis integrado en flujos de instalación/actualización de Plugins. No omite bloqueos de política del hook `before_install` del Plugin ni omite fallos de análisis.
...
Por defecto, OpenClaw enruta **todos los DM a la sesión principal** para que tu asistente tenga continuidad entre dispositivos y canales. Si **varias personas** pueden enviar DM al bot (DM abiertos o una lista de permitidos con varias personas), considera aislar las sesiones de DM:
...
- Aislamiento de pares entre canales: `session.dmScope: "per-peer"` (cada remitente obtiene una sesión entre todos los canales del mismo tipo).
...
- Nota: el sandboxing es optativo. Si el modo sandbox está desactivado, `host=auto` implícito se resuelve al host del Gateway. `host=sandbox` explícito aún falla de forma cerrada porque no hay runtime de sandbox disponible. Define `host=gateway` si quieres que ese comportamiento sea explícito en la configuración.
...
- Si incluyes intérpretes en listas de permitidos (`python`, `node`, `ruby`, `perl`, `php`, `lua`, `osascript`), habilita `tools.exec.strictInlineEval` para que las formas de evaluación en línea sigan requiriendo aprobación explícita.
- El análisis de aprobación de shell también rechaza formas de expansión de parámetros POSIX (`$VAR`, `$?`, `$$`, `$1`, `$@`, `${…}`) dentro de **heredocs sin comillas**, por lo que un cuerpo de heredoc permitido no puede colar expansión de shell como texto sin formato tras la revisión de la lista de permitidos. Pon entre comillas el terminador del heredoc (por ejemplo `<<'EOF'`) para optar por semánticas de cuerpo literal; los heredocs sin comillas que habrían expandido variables se rechazan.
- **La elección del modelo importa:** los modelos antiguos/pequeños/heredados son significativamente menos robustos frente a la inyección de prompts y el uso indebido de herramientas. Para agentes con herramientas habilitadas, usa el modelo más potente disponible de última generación y reforzado para seguir instrucciones.
...
OpenClaw elimina de contenido externo envuelto y metadatos los literales comunes de tokens especiales de plantillas de chat de LLM autoalojados antes de que lleguen al modelo. Las familias de marcadores cubiertas incluyen Qwen/ChatML, Llama, Gemma, Mistral, Phi y tokens de rol/turno GPT-OSS.

Por qué:

- Los backends compatibles con OpenAI que sirven modelos autoalojados a veces preservan tokens especiales que aparecen en texto de usuario, en lugar de enmascararlos. Un atacante que pueda escribir en contenido externo entrante (una página obtenida, el cuerpo de un correo, la salida de una herramienta de contenido de archivo) podría de otro modo inyectar un límite sintético de rol `assistant` o `system` y escapar de las barreras de protección del contenido envuelto.
- La sanitización ocurre en la capa de envoltura de contenido externo, por lo que se aplica de forma uniforme en herramientas de obtención/lectura y en contenido de canales entrantes, en lugar de hacerlo por proveedor.
- Las respuestas salientes del modelo ya tienen un sanitizador separado que elimina `<tool_call>`, `<function_calls>`, `<system-reminder>`, `<previous_response>` filtrados y andamiaje similar del runtime interno de las respuestas visibles para el usuario en el límite final de entrega del canal. El sanitizador de contenido externo es la contraparte entrante.

Esto no reemplaza el resto del endurecimiento en esta página: `dmPolicy`, listas de permitidos, aprobaciones de exec, sandboxing y `contextVisibility` siguen haciendo el trabajo principal. Cierra una omisión específica de la capa del tokenizador contra stacks autoalojados que reenvían texto de usuario con tokens especiales intactos.
...
- Mantén esto sin definir/en false en producción.
- Habilítalo solo temporalmente para depuración con alcance muy limitado.
- Si está habilitado, aísla ese agente (sandbox + herramientas mínimas + espacio de nombres de sesión dedicado).

Nota de riesgo de hooks:

- Las cargas de hooks son contenido no confiable, incluso cuando la entrega proviene de sistemas que controlas (correo/docs/contenido web puede transportar inyección de prompts).
- Los niveles de modelo débiles aumentan este riesgo. Para automatización impulsada por hooks, prefiere niveles de modelos modernos y potentes y mantén estricta la política de herramientas (`tools.profile: "messaging"` o más estricta), además de sandboxing cuando sea posible.
...
- Para entradas de archivo de OpenResponses, el texto `input_file` decodificado aún se inyecta como
...
o stacks personalizados de tokenizadores de Hugging Face, pueden diferir de proveedores alojados en cómo
...
La resistencia a la inyección de prompts **no** es uniforme entre niveles de modelos. Los modelos más pequeños/baratos suelen ser más susceptibles al uso indebido de herramientas y al secuestro de instrucciones, especialmente bajo prompts adversariales.
...
no estaban pensados para un canal público. En contextos grupales, trátalos como **solo depuración**
...
- Si los habilitas, hazlo solo en MD confiables o salas estrictamente controladas.
- Recuerda: la salida detallada y de traza puede incluir argumentos de herramientas, URLs, diagnósticos de Plugin y datos que el modelo vio.
...
- IU de Control (activos SPA) (ruta base predeterminada `/`)
...
- Si debes hacer bind a LAN, restringe el puerto en el firewall a una lista de permitidos estricta de IPs de origen; no lo reenvíes ampliamente.
...
de Docker, no solo las reglas `INPUT` del host.
...
varían entre imágenes VPS (`ens3`, `enp*`, etc.) y las discrepancias pueden hacer que accidentalmente
...
1. **Mantén Bonjour deshabilitado salvo que se necesite descubrimiento en LAN.** Bonjour se inicia automáticamente en hosts macOS y es opcional en otros entornos; las URL directas de Gateway, Tailnet, SSH o DNS-SD de área amplia evitan la multidifusión local.
...
La autenticación de Gateway es **obligatoria de forma predeterminada**. Si no hay configurada una ruta de autenticación de gateway válida,
...
<Note>
`gateway.remote.token` y `gateway.remote.password` son fuentes de credenciales de cliente. Por sí mismas **no** protegen el acceso WS local. Las rutas de llamada locales pueden usar `gateway.remote.*` como alternativa solo cuando `gateway.auth.*` no está establecido. Si `gateway.auth.token` o `gateway.auth.password` se configuran explícitamente mediante SecretRef y no se resuelven, la resolución falla cerrada (sin enmascaramiento por alternativa remota).
</Note>
Opcional: fija TLS remoto con `gateway.remote.tlsFingerprint` al usar `wss://`.
Texto sin cifrar `ws://` es solo para local loopback de forma predeterminada. Para rutas de red privada
...
- Las conexiones de Tailnet y LAN, incluidas las vinculaciones de tailnet del mismo host, se tratan como
...
UI de control/WebSocket. OpenClaw verifica la identidad resolviendo la dirección
...
**no** usan autenticación por encabezado de identidad de Tailscale. Siguen respetando el
modo de autenticación HTTP configurado del gateway.

Nota importante de límite:

- La autenticación bearer HTTP de Gateway es, en la práctica, acceso de operador de todo o nada.
...
- En la superficie HTTP compatible con OpenAI, la autenticación bearer con secreto compartido restaura los alcances de operador predeterminados completos (`operator.admin`, `operator.approvals`, `operator.pairing`, `operator.read`, `operator.talk.secrets`, `operator.write`) y la semántica de propietario para turnos de agente; valores más estrechos de `x-openclaw-scopes` no reducen esa ruta de secreto compartido.
...
OpenClaw carga archivos `.env` locales del workspace para agentes y herramientas, pero nunca permite que esos archivos sobrescriban silenciosamente los controles de runtime del gateway.

- Cualquier clave que empiece con `OPENCLAW_*` se bloquea en archivos `.env` de workspace no confiables.
- Los ajustes de endpoints de canal para Matrix, Mattermost, IRC y Synology Chat también se bloquean frente a sobrescrituras desde `.env` de workspace, de modo que los workspaces clonados no puedan redirigir el tráfico de conectores incluidos mediante configuración de endpoints local. Las claves env de endpoints (como `MATRIX_HOMESERVER`, `MATTERMOST_URL`, `IRC_HOST`, `SYNOLOGY_CHAT_INCOMING_URL`) deben provenir del entorno del proceso gateway o de `env.shellEnv`, no de un `.env` cargado desde el workspace.
...
- Las variables de entorno confiables del proceso/SO (la shell propia del gateway, unidad launchd/systemd, paquete de app) siguen aplicándose - esto solo restringe la carga de archivos `.env`.
...
- Al compartir diagnósticos, prefiere `openclaw status --all` (pegable, secretos redactados) frente a registros sin procesar.
...
- `tools.exec.applyPatch.workspaceOnly: true` (predeterminado): garantiza que `apply_patch` no pueda escribir/eliminar fuera del directorio del espacio de trabajo, incluso cuando el sandboxing está desactivado. Configúralo como `false` solo si quieres intencionalmente que `apply_patch` modifique archivos fuera del espacio de trabajo.
...
Si también quieres una ejecución de herramientas "más segura por defecto", añade un sandbox y deniega herramientas peligrosas para cualquier agente que no sea el propietario (ejemplo más abajo en "Perfiles de acceso por agente").
...
<Note>
Para evitar el acceso entre agentes, mantén `agents.defaults.sandbox.scope` en `"agent"` (predeterminado) o `"session"` para un aislamiento más estricto por sesión. `scope: "shared"` usa un único contenedor o espacio de trabajo.
</Note>
...
- Los `sandbox.docker.binds` adicionales se validan contra rutas de origen normalizadas y canonizadas. Los trucos con enlaces simbólicos de directorios padre y los alias canónicos del directorio personal siguen fallando en modo cerrado si se resuelven dentro de raíces bloqueadas como `/etc`, `/var/run` o directorios de credenciales bajo el directorio personal del sistema operativo.
...
- Mantén `agents.defaults.subagents.allowAgents` y cualquier sobrescritura por agente de `agents.list[].subagents.allowAgents` restringidas a agentes de destino que se sepa que son seguros.
...
- La API independiente de control de navegador loopback solo respeta la autenticación por secreto compartido
...
- Mantén el Gateway y los hosts de nodos solo en la tailnet; evita exponer puertos de control del navegador a la LAN o a Internet público.
...
- Modo de activación explícita: configura `browser.ssrfPolicy.dangerouslyAllowPrivateNetwork: true` para permitir destinos privados/internos/de uso especial.
...
3. **Congela el acceso:** cambia los DMs/grupos riesgosos a `dmPolicy: "disabled"` / exige menciones, y elimina las entradas de permitir todo `"*"` si las tenías.
...
3. Revisa los cambios recientes de configuración (cualquier cosa que pudiera haber ampliado el acceso: `gateway.bind`, `gateway.auth`, políticas de DM/grupo, `tools.elevated`, cambios de Plugin).
...
3. Te daremos crédito (salvo que prefieras permanecer en el anonimato)
