...
summary: Consideraciones de seguridad y modelo de amenazas para ejecutar un Gateway de IA con acceso de shell
title: Seguridad
x-i18n:
...
<Warning>
  **Modelo de confianza de asistente personal.** Esta guía presupone un límite
  de operador de confianza por gateway (modelo de asistente personal de un solo usuario).
...
`security audit --fix` se mantiene intencionalmente limitado: cambia políticas comunes de grupos abiertos a listas de permitidos, restaura `logging.redactSensitive: "tools"`, ajusta permisos de estado/configuración/archivos incluidos y usa restablecimientos de ACL de Windows en lugar de `chmod` de POSIX cuando se ejecuta en Windows.

Marca errores comunes (exposición de autenticación del Gateway, exposición de control del navegador, listas de permitidos elevadas, permisos del sistema de archivos, aprobaciones de exec permisivas y exposición de herramientas en canales abiertos).

OpenClaw es tanto un producto como un experimento: estás conectando comportamiento de modelos frontera a superficies de mensajería reales y herramientas reales. **No existe una configuración “perfectamente segura”.** El objetivo es actuar de forma deliberada sobre:

- quién puede hablar con tu bot
- dónde puede actuar el bot
- qué puede tocar el bot

Empieza con el acceso más pequeño que todavía funcione y luego amplíalo a medida que ganes confianza.
...
- la inyección de prompts/contenido de un remitente puede provocar acciones que afecten estado, dispositivos o salidas compartidos;
- si un agente compartido tiene credenciales/archivos sensibles, cualquier remitente permitido puede potencialmente impulsar exfiltración mediante el uso de herramientas.
...
- Los clientes backend directos de local loopback autenticados con el token/contraseña
  compartidos del gateway pueden realizar RPC internas del plano de control sin presentar una identidad de
  dispositivo de usuario. Esto no es una omisión del emparejamiento remoto o del navegador: los clientes de red,
...
- Las aprobaciones de exec (lista de permitidos + solicitud) son barreras de protección para la intención del operador, no aislamiento multiinquilino hostil.
- El valor predeterminado de producto de OpenClaw para configuraciones confiables de un solo operador es que exec de host en `gateway`/`node` está permitido sin solicitudes de aprobación (`security="full"`, `ask="off"` a menos que lo endurezcas). Ese valor predeterminado es UX intencional, no una vulnerabilidad por sí mismo.
- Las aprobaciones de exec vinculan el contexto exacto de la solicitud y los operandos de archivos locales directos de mejor esfuerzo; no modelan semánticamente todas las rutas de carga de runtime/intérprete. Usa sandboxing y aislamiento de host para límites fuertes.

Si necesitas aislamiento de usuario hostil, separa los límites de confianza por usuario/host de SO y ejecuta gateways separados.
...
| --------------------------------------------------------- | ------------------------------------------------- | ----------------------------------------------------------------------------- |
| `gateway.auth` (token/password/trusted-proxy/device auth) | Autentica llamadores ante APIs del gateway        | "Necesita firmas por mensaje en cada trama para ser seguro"                   |
...
| Barreras de protección de prompts/contenido               | Reducen el riesgo de abuso del modelo             | "La inyección de prompt por sí sola demuestra una omisión de autenticación"    |
...
- Afirmaciones que clasifican el acceso normal de operador en rutas de lectura (por ejemplo
...
- Hallazgos de implementación solo en localhost (por ejemplo, HSTS en un
  gateway solo local loopback).
- Hallazgos de firmas de webhook entrante de Discord para rutas entrantes que no
...
- Reportes que tratan `gateway.nodes.pairing.autoApproveCidrs` configurado como una
  ...
  ni rutas de encabezado trusted-proxy de local loopback en el mismo host salvo que la autenticación trusted-proxy de local loopback se haya habilitado explícitamente.
...
Esto mantiene el Gateway solo local, aísla los mensajes directos y deshabilita por defecto las herramientas de plano de control/runtime.
...
- Esto endurece buzones cooperativos/compartidos, pero no está diseñado como aislamiento hostil entre coinquilinos cuando los usuarios comparten acceso de escritura al host/configuración.
...
Las listas de permitidos controlan activaciones y autorización de comandos. El ajuste `contextVisibility` controla cómo se filtra el contexto complementario (respuestas citadas, raíces de hilos, historial obtenido):

- `contextVisibility: "all"` (predeterminado) mantiene el contexto complementario tal como se recibe.
- `contextVisibility: "allowlist"` filtra el contexto complementario a remitentes permitidos por las comprobaciones activas de lista de permitidos.
...
- Las afirmaciones que solo muestran que "el modelo puede ver texto citado o histórico de remitentes no incluidos en la lista de permitidos" son hallazgos de endurecimiento abordables con `contextVisibility`, no omisiones de autenticación o de límites de sandbox por sí mismas.
- Para tener impacto en la seguridad, los informes todavía necesitan una omisión demostrada de un límite de confianza (autenticación, política, sandbox, aprobación u otro límite documentado).
...
- **Deriva de aprobación de exec** (`security=full`, `autoAllowSkills`, listas de permitidos del intérprete sin `strictInlineEval`): ¿las protecciones de ejecución en el host siguen haciendo lo que crees que hacen?
  - `security="full"` es una advertencia de postura amplia, no prueba de un error. Es el valor predeterminado elegido para configuraciones de asistente personal de confianza; ajústalo solo cuando tu modelo de amenazas necesite protecciones de aprobación o lista de permitidos.
...
Si ejecutas `--deep`, OpenClaw también intenta una sonda de Gateway en vivo con el mejor esfuerzo.
...
1. **Cualquier cosa "abierta" + herramientas habilitadas**: primero bloquea MD/grupos (emparejamiento/listas de permitidos), luego endurece la política de herramientas/sandboxing.
2. **Exposición de red pública** (vinculación LAN, Funnel, autenticación ausente): corrígelo inmediatamente.
...
5. **Plugins**: carga solo aquello en lo que confías explícitamente.
...
deshabilita por completo las comprobaciones de identidad de dispositivo. Esto es una degradación de seguridad grave;
mantenlo desactivado salvo que estés depurando activamente y puedas revertirlo rápidamente.

Separado de esas marcas peligrosas, `gateway.auth.mode: "trusted-proxy"` exitoso
...
interruptores de depuración inseguros/peligrosos conocidos están habilitados. Déjalos sin configurar en
producción.
...
<AccordionGroup>
  ...
  <Accordion title="All `dangerous*` / `dangerously*` keys in the config schema">
    ...
    Coincidencia de nombres de canal (canales incluidos y de plugin; también disponible por
...
- los proxies inversos de loopback en el mismo host pueden usar `gateway.trustedProxies` para la detección de cliente local y la gestión de IP reenviada
- los proxies inversos de loopback en el mismo host pueden satisfacer `gateway.auth.mode: "trusted-proxy"` solo cuando `gateway.auth.trustedProxy.allowLoopback = true`; de lo contrario, usa autenticación con token/contraseña
...
Incluso cuando está habilitada, las rutas de encabezado de trusted-proxy con origen loopback
...
- OpenClaw gateway prioriza local/loopback. Si terminas TLS en un proxy inverso, establece HSTS allí en el dominio HTTPS orientado al proxy.
...
- Para despliegues de Control UI que no son loopback, `gateway.controlUi.allowedOrigins` es obligatorio de forma predeterminada.
...
- Los fallos de autenticación de origen de navegador en loopback siguen estando limitados por tasa incluso cuando la
...
- `gateway.controlUi.dangerouslyAllowHostHeaderOriginFallback=true` habilita el modo de fallback de origen de encabezado Host; trátalo como una política peligrosa seleccionada por el operador.
- Trata el DNS rebinding y el comportamiento de encabezado de host de proxy como preocupaciones de endurecimiento del despliegue; mantén `trustedProxies` estrictos y evita exponer el gateway directamente a internet público.
...
- Se controla en la Mac mediante **Configuración → Aprobaciones de exec** (seguridad + preguntar + lista de permitidos).
- La política `system.run` por Node es el archivo propio de aprobaciones de exec del Node (`exec.approvals.node.*`), que puede ser más estricta o más permisiva que la política global de ID de comandos del Gateway.
- Un Node que se ejecuta con `security="full"` y `ask="off"` sigue el modelo predeterminado de operador de confianza. Trátalo como comportamiento esperado a menos que tu despliegue requiera explícitamente una postura de aprobación o lista de permitidos más estricta.
...
- Para `host=node`, las ejecuciones respaldadas por aprobación también almacenan un
  ...
  validación del Gateway rechaza las ediciones del llamador al contexto de comando/cwd/sesión después de que se
...
- Los informes que tratan los metadatos de emparejamiento de Node como una segunda capa oculta de aprobación por comando suelen ser confusión de política/UX, no una elusión de un límite de seguridad.

## Skills dinámicas (observador / Nodes remotos)

OpenClaw puede actualizar la lista de Skills en mitad de una sesión:

- **Observador de Skills**: los cambios en `SKILL.md` pueden actualizar la instantánea de Skills en el siguiente turno del agente.
- **Nodes remotos**: conectar un Node de macOS puede hacer que Skills solo para macOS sean elegibles (según sondeos de binarios).
...
- **Primero identidad:** decide quién puede hablar con el bot (emparejamiento de DM / listas de permitidos / “open” explícito).
- **Después alcance:** decide dónde puede actuar el bot (listas de permitidos de grupos + activación por mención, herramientas, sandboxing, permisos de dispositivo).
...
La herramienta de runtime `gateway`, solo para el propietario, sigue rechazando reescribir
...
Las ediciones impulsadas por agente `gateway config.apply` y `gateway config.patch`
fallan cerradas de forma predeterminada: solo un conjunto reducido de rutas de prompt, modelo y activación por mención
...
- Si instalas o actualizas Plugins (`openclaw plugins install <package>`, `openclaw plugins update <id>`), trátalo como ejecutar código no confiable:
  ...
  - `--dangerously-force-unsafe-install` es solo una vía de emergencia para falsos positivos del escaneo integrado en flujos de instalación/actualización de Plugins. No omite bloqueos de política del hook `before_install` del Plugin y no omite fallos de escaneo.
  - Las instalaciones de dependencias de Skills respaldadas por Gateway siguen la misma división peligroso/sospechoso: los hallazgos integrados `critical` bloquean a menos que el llamador establezca explícitamente `dangerouslyForceUnsafeInstall`, mientras que los hallazgos sospechosos siguen solo advirtiendo. `openclaw skills install` sigue siendo el flujo separado de descarga/instalación de Skills de ClawHub.
...
- `pairing` (predeterminado): los remitentes desconocidos reciben un código corto de emparejamiento y el bot ignora su mensaje hasta que se apruebe. Los códigos expiran después de 1 hora; los DM repetidos no reenviarán un código hasta que se cree una nueva solicitud. Las solicitudes pendientes tienen un límite predeterminado de **3 por canal**.
...
- **Lista de permitidos de grupo** (específica del canal): de qué grupos/canales/gremios aceptará mensajes el bot.
  - Patrones comunes:
    - `channels.whatsapp.groups`, `channels.telegram.groups`, `channels.imessage.groups`: valores predeterminados por grupo como `requireMention`; cuando se establece, también actúa como una lista de permitidos de grupo (incluye `"*"` para conservar el comportamiento de permitir todo).
...
La inyección de prompt ocurre cuando un atacante crea un mensaje que manipula el modelo para hacer algo inseguro (“ignora tus instrucciones”, “vuelca tu sistema de archivos”, “sigue este enlace y ejecuta comandos”, etc.).

Incluso con prompts de sistema sólidos, **la inyección de prompt no está resuelta**. Las barreras de seguridad del prompt de sistema son solo orientación blanda; la aplicación estricta proviene de la política de herramientas, aprobaciones de exec, sandboxing y listas de permitidos de canales (y los operadores pueden desactivarlas por diseño). Lo que ayuda en la práctica:

- Mantén bloqueados los mensajes directos entrantes (emparejamiento/listas de permitidos).
...
- Ejecuta herramientas sensibles en una sandbox; mantén los secretos fuera del sistema de archivos alcanzable por el agente.
- Nota: el uso de sandbox es optativo. Si el modo sandbox está desactivado, `host=auto` implícito se resuelve al host del Gateway. `host=sandbox` explícito sigue fallando de forma cerrada porque no hay ningún runtime de sandbox disponible. Configura `host=gateway` si quieres que ese comportamiento sea explícito en la configuración.
...
- Si incluyes intérpretes en la lista de permitidos (`python`, `node`, `ruby`, `perl`, `php`, `lua`, `osascript`), activa `tools.exec.strictInlineEval` para que las formas de evaluación inline sigan necesitando aprobación explícita.
- El análisis de aprobación de shell también rechaza formas de expansión de parámetros POSIX (`$VAR`, `$?`, `$$`, `$1`, `$@`, `${…}`) dentro de **heredocs sin comillas**, de modo que un cuerpo de heredoc en la lista de permitidos no pueda colar expansión de shell más allá de la revisión de la lista de permitidos como texto plano. Pon entre comillas el terminador del heredoc (por ejemplo, `<<'EOF'`) para optar por la semántica de cuerpo literal; los heredocs sin comillas que habrían expandido variables se rechazan.
- **La elección del modelo importa:** los modelos antiguos, más pequeños o legacy son significativamente menos robustos frente a la inyección de prompts y el uso indebido de herramientas. Para agentes con herramientas habilitadas, usa el modelo de última generación, reforzado para instrucciones, más potente disponible.
...
- El saneamiento ocurre en la capa de envoltura de contenido externo, por lo que se aplica de forma uniforme entre herramientas de fetch/read y contenido entrante de canales, en lugar de ser por proveedor.
...
Esto no reemplaza el resto del endurecimiento en esta página: `dmPolicy`, las listas de permitidos, las aprobaciones de exec, el uso de sandbox y `contextVisibility` siguen haciendo el trabajo principal. Cierra una derivación específica de la capa de tokenización contra pilas autohospedadas que reenvían texto de usuario con tokens especiales intactos.
...
- Mantén estas opciones sin configurar/en `false` en producción.
- Actívalas solo temporalmente para depuración de alcance muy limitado.
...
- Los payloads de hooks son contenido no confiable, incluso cuando la entrega viene de sistemas que controlas (el contenido de correo/docs/web puede llevar inyección de prompts).
- Los niveles de modelo débiles aumentan este riesgo. Para automatización impulsada por hooks, prefiere niveles de modelo modernos y fuertes, y mantén una política de herramientas estricta (`tools.profile: "messaging"` o más estricta), además de sandbox cuando sea posible.
...
- Para entradas de URL de OpenResponses (`input_file` / `input_image`), configura listas de permitidos estrictas en
  ...
  si quieres desactivar por completo la obtención de URLs.
- Para entradas de archivo de OpenResponses, el texto decodificado de `input_file` se sigue inyectando como
...
- Activar sandbox y listas de permitidos estrictas de herramientas para cualquier agente que toque entrada no confiable.
...
o pilas personalizadas de tokenizadores de Hugging Face pueden diferir de los proveedores hospedados en cómo
...
y prefiere opciones del backend que separen o escapen tokens especiales
en contenido proporcionado por usuarios cuando estén disponibles. Los proveedores hospedados como OpenAI
...
<Warning>
Para agentes con herramientas habilitadas o agentes que leen contenido no confiable, el riesgo de inyección de prompts con modelos antiguos/más pequeños suele ser demasiado alto. No ejecutes esas cargas de trabajo en niveles de modelo débiles.
</Warning>
...
- Al ejecutar modelos pequeños, **activa sandbox para todas las sesiones** y **desactiva web_search/web_fetch/browser** salvo que las entradas estén estrictamente controladas.
...
- Si los activas, hazlo solo en mensajes directos de confianza o salas estrechamente controladas.
- Recuerda: la salida detallada y de traza puede incluir argumentos de herramientas, URLs, diagnósticos de plugins y datos que vio el modelo.
...
- Si debes vincular a LAN, limita el puerto con firewall a una lista de permitidos estricta de IPs de origen; no hagas port-forward amplio.
...
(`-p HOST:CONTAINER` o Compose `ports:`) se enrutan a través de las cadenas de reenvío de Docker,
...
`DOCKER-USER` (esta cadena se evalúa antes de las propias reglas accept de Docker).
...
varían entre imágenes de VPS (`ens3`, `enp*`, etc.) y las discrepancias pueden accidentalmente
omitir tu regla de denegación.
...
1. **Mantén Bonjour deshabilitado salvo que se necesite descubrimiento LAN.** Bonjour se inicia automáticamente en hosts macOS y es opcional en otros sistemas; las URL directas del Gateway, Tailnet, SSH o DNS-SD de área amplia evitan la multidifusión local.

2. **Modo mínimo** (predeterminado cuando Bonjour está habilitado, recomendado para gateways expuestos): omite campos confidenciales de las difusiones mDNS:
...
5. **Variable de entorno** (alternativa): define `OPENCLAW_DISABLE_BONJOUR=1` para deshabilitar mDNS sin cambios de configuración.
...
La autenticación del Gateway es **obligatoria de forma predeterminada**. Si no se configura una ruta de autenticación válida para el gateway,
...
El texto claro `ws://` es solo para loopback de forma predeterminada. Para rutas de
red privada de confianza, define `OPENCLAW_ALLOW_INSECURE_PRIVATE_WS=1` en el proceso
del cliente como medida de emergencia. Esto es intencionalmente solo entorno de
proceso, no una clave de configuración de `openclaw.json`.
...
- Las conexiones Tailnet y LAN, incluidas las vinculaciones tailnet del mismo host, se tratan como remotas para el emparejamiento y siguen necesitando aprobación.
- La evidencia de encabezados reenviados en una solicitud loopback descalifica la localidad loopback. La aprobación automática por actualización de metadatos tiene un alcance estrecho. Consulta [Emparejamiento del Gateway](/es/gateway/pairing) para ambas reglas.

Modos de autenticación:

- `gateway.auth.mode: "token"`: token bearer compartido (recomendado para la mayoría de configuraciones).
- `gateway.auth.mode: "password"`: autenticación con contraseña (preferible definirla mediante env: `OPENCLAW_GATEWAY_PASSWORD`).
- `gateway.auth.mode: "trusted-proxy"`: confía en que un proxy inverso con identidad autentique a los usuarios y pase la identidad mediante encabezados (consulta [Autenticación de proxy de confianza](/es/gateway/trusted-proxy-auth)).

Lista de verificación de rotación (token/contraseña):

1. Genera/define un secreto nuevo (`gateway.auth.token` o `OPENCLAW_GATEWAY_PASSWORD`).
...
acepta encabezados de identidad de Tailscale Serve (`tailscale-user-login`) para la
autenticación de la UI de control/WebSocket. OpenClaw verifica la identidad resolviendo la
...
se serializan antes de que el limitador registre el fallo. Por lo tanto, los reintentos incorrectos
concurrentes de un cliente Serve pueden bloquear de inmediato el segundo intento
...
- En la superficie HTTP compatible con OpenAI, la autenticación bearer con secreto compartido restaura todos los alcances de operador predeterminados (`operator.admin`, `operator.approvals`, `operator.pairing`, `operator.read`, `operator.talk.secrets`, `operator.write`) y la semántica de propietario para turnos de agente; valores `x-openclaw-scopes` más estrechos no reducen esa ruta de secreto compartido.
...
- En esos modos con identidad, omitir `x-openclaw-scopes` vuelve al conjunto normal de alcances predeterminados del operador; envía el encabezado explícitamente cuando quieras un conjunto de alcances más estrecho.
...
- No compartas estas credenciales con llamadores que no sean de confianza; prefiere gateways separados por cada límite de confianza.

**Supuesto de confianza:** la autenticación Serve sin token asume que el host del gateway es de confianza.
...
Consulta [Tailscale](/es/gateway/tailscale) y [Vista general web](/es/web).
...
en la máquina del navegador y deja que el Gateway proxy las acciones del navegador (consulta [Herramienta de navegador](/es/tools/browser)).
...
- Empareja el node deliberadamente; deshabilita el enrutamiento proxy del navegador si no lo necesitas.
...
- `openclaw.json`: la configuración puede incluir tokens (gateway, gateway remoto), ajustes de proveedores y allowlists.
- `credentials/**`: credenciales de canales (ejemplo: credenciales de WhatsApp), allowlists de emparejamiento, importaciones OAuth heredadas.
...
- `secrets.json` (opcional): carga de secreto respaldada por archivo usada por proveedores SecretRef de `file` (`secrets.providers`).
...
- Mantén permisos estrictos (`700` en dirs, `600` en archivos).
...
- Los ajustes de endpoint de canal para Matrix, Mattermost, IRC y Synology Chat también se bloquean frente a sobrescrituras desde `.env` del espacio de trabajo, de modo que los espacios de trabajo clonados no puedan redirigir el tráfico de conectores incluidos mediante configuración de endpoint local. Las claves env de endpoint (como `MATRIX_HOMESERVER`, `MATTERMOST_URL`, `IRC_HOST`, `SYNOLOGY_CHAT_INCOMING_URL`) deben venir del entorno del proceso del gateway o de `env.shellEnv`, no de un `.env` cargado desde el espacio de trabajo.
- El bloqueo falla de forma cerrada: una variable nueva de control de runtime añadida en una versión futura no puede heredarse desde un `.env` registrado o suministrado por un atacante; la clave se ignora y el gateway conserva su propio valor.
- Las variables de entorno confiables del proceso/SO (el shell propio del gateway, unidad launchd/systemd, paquete de app) siguen aplicándose — esto solo restringe la carga de archivos `.env`.

Motivo: los archivos `.env` del espacio de trabajo suelen vivir junto al código del agente, se confirman por accidente o los escriben las herramientas. Bloquear todo el prefijo `OPENCLAW_*` significa que añadir más adelante una bandera `OPENCLAW_*` nueva nunca puede derivar en herencia silenciosa desde el estado del espacio de trabajo.
...
### Modo de solo lectura (mediante entorno aislado y herramientas)

Puedes crear un perfil de solo lectura combinando:

- `agents.defaults.sandbox.workspaceAccess: "ro"` (o `"none"` para no permitir acceso al espacio de trabajo)
...
- `tools.exec.applyPatch.workspaceOnly: true` (predeterminado): garantiza que `apply_patch` no pueda escribir/eliminar fuera del directorio del espacio de trabajo aunque el aislamiento esté desactivado. Establécelo en `false` solo si quieres intencionadamente que `apply_patch` toque archivos fuera del espacio de trabajo.
...
- Mantén estrechas las raíces del sistema de archivos: evita raíces amplias como tu directorio personal para espacios de trabajo de agentes/espacios de trabajo aislados. Las raíces amplias pueden exponer archivos locales sensibles (por ejemplo, estado/configuración bajo `~/.openclaw`) a herramientas del sistema de archivos.

### Línea base segura (copiar/pegar)

Una configuración de “valor seguro predeterminado” que mantiene el Gateway privado, requiere emparejamiento por DM y evita bots grupales siempre activos:
...
Si también quieres una ejecución de herramientas “más segura de forma predeterminada”, añade un entorno aislado y deniega herramientas peligrosas para cualquier agente que no sea propietario (ejemplo más abajo en “Perfiles de acceso por agente”).
...
## Aislamiento en entorno aislado (recomendado)

Documento dedicado: [Aislamiento en entorno aislado](/es/gateway/sandboxing)

Dos enfoques complementarios:

- **Ejecutar el Gateway completo en Docker** (límite de contenedor): [Docker](/es/install/docker)
- **Entorno aislado de herramientas** (`agents.defaults.sandbox`, gateway del host + herramientas aisladas por sandbox; Docker es el backend predeterminado): [Aislamiento en entorno aislado](/es/gateway/sandboxing)

<Note>
Para evitar acceso entre agentes, mantén `agents.defaults.sandbox.scope` en `"agent"` (predeterminado) o `"session"` para un aislamiento por sesión más estricto. `scope: "shared"` usa un único contenedor o espacio de trabajo.
</Note>

Considera también el acceso al espacio de trabajo del agente dentro del entorno aislado:
...
- Los `sandbox.docker.binds` adicionales se validan contra rutas de origen normalizadas y canonicalizadas. Los trucos con enlaces simbólicos de directorios padre y los alias canónicos del directorio personal siguen fallando de forma cerrada si se resuelven a raíces bloqueadas como `/etc`, `/var/run` o directorios de credenciales bajo el directorio personal del sistema operativo.

<Warning>
`tools.elevated` es la vía de escape de línea base global que ejecuta exec fuera del entorno aislado. El host efectivo es `gateway` de forma predeterminada, o `node` cuando el destino de exec está configurado como `node`. Mantén `tools.elevated.allowFrom` estrictamente limitado y no lo habilites para desconocidos. Puedes restringir aún más el modo elevado por agente mediante `agents.list[].tools.elevated`. Consulta [Modo elevado](/es/tools/elevated).
</Warning>

### Barrera de protección para delegación en subagentes

Si permites herramientas de sesión, trata las ejecuciones de subagentes delegados como otra decisión de límites:

- Deniega `sessions_spawn` salvo que el agente necesite realmente delegación.
- Mantén `agents.defaults.subagents.allowAgents` y cualquier anulación por agente en `agents.list[].subagents.allowAgents` restringidas a agentes de destino conocidos como seguros.
- Para cualquier flujo de trabajo que deba permanecer en entorno aislado, llama a `sessions_spawn` con `sandbox: "require"` (el valor predeterminado es `inherit`).
- `sandbox: "require"` falla rápido cuando el runtime secundario de destino no está en entorno aislado.
...
- Mantén deshabilitado el control del navegador del host para agentes en entorno aislado salvo que confíes en ellos.
- La API independiente de control de navegador por local loopback solo respeta autenticación por secreto compartido
...
- Mantén el Gateway y los hosts Node accesibles solo por tailnet; evita exponer puertos de control del navegador a la LAN o a Internet pública.
...
- Modo de inclusión explícita: establece `browser.ssrfPolicy.dangerouslyAllowPrivateNetwork: true` para permitir destinos privados/internos/de uso especial.
...
- La navegación se comprueba antes de la solicitud y se vuelve a comprobar, con el mayor esfuerzo posible, en la URL `http(s)` final después de la navegación para reducir pivotes basados en redirecciones.
...
Con enrutamiento multiagente, cada agente puede tener su propia política de entorno aislado + herramientas:
usa esto para dar **acceso completo**, **solo lectura** o **sin acceso** por agente.
Consulta [Entorno aislado y herramientas multiagente](/es/tools/multi-agent-sandbox-tools) para ver todos los detalles
y las reglas de precedencia.

Casos de uso comunes:

- Agente personal: acceso completo, sin entorno aislado
- Agente familiar/laboral: entorno aislado + herramientas de solo lectura
- Agente público: entorno aislado + sin herramientas de sistema de archivos/shell

### Ejemplo: acceso completo (sin entorno aislado)
...
3. **Congela el acceso:** cambia los DM/grupos riesgosos a `dmPolicy: "disabled"` / requiere menciones, y elimina entradas de permitir todo `"*"` si las tenías.
...
falla, elimina o rota el material de clave confirmado en commits y luego reprodúcelo localmente:
