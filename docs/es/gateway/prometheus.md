...
x-i18n:
    generated_at: "2026-04-30T05:43:46Z"
    model: gpt-5.5
    provider: openai
...
OpenClaw puede exponer métricas de diagnóstico mediante el Plugin incluido `diagnostics-prometheus`. Escucha diagnósticos internos de confianza y renderiza un endpoint de texto de Prometheus en:
...
<Warning>
La ruta usa autenticación del Gateway (alcance de operador). No la expongas como un endpoint `/metrics` público sin autenticar. Haz el scraping a través de la misma ruta de autenticación que usas para otras API de operador.
</Warning>
...
<AccordionGroup>
  <Accordion title="Etiquetas acotadas y de baja cardinalidad">
    Las etiquetas de Prometheus se mantienen acotadas y con baja cardinalidad. El exportador no emite identificadores de diagnóstico sin procesar como `runId`, `sessionKey`, `sessionId`, `callId`, `toolCallId`, ID de mensajes, ID de chats o ID de solicitudes del proveedor.
  ...
  <Accordion title="Límite de series y contabilización de desbordamiento">
    El exportador limita las series temporales retenidas en memoria a **2048** series entre contadores, indicadores e histogramas combinados. Las series nuevas que superan ese límite se descartan, y `openclaw_prometheus_series_dropped_total` se incrementa en uno cada vez.

    Vigila este contador como una señal firme de que un atributo upstream está filtrando valores de alta cardinalidad. El exportador nunca eleva el límite automáticamente; si aumenta, corrige el origen en lugar de deshabilitar el límite.
  ...
  <Accordion title="Qué nunca aparece en la salida de Prometheus">
    ...
    - ID de solicitudes del proveedor sin procesar (solo hashes acotados, cuando corresponda, en spans; nunca en métricas)
...
```promql
# Tokens por minuto, separados por proveedor
sum by (provider) (rate(openclaw_model_tokens_total[1m]))

# Gasto (USD) durante la última hora, por modelo
sum by (model) (increase(openclaw_model_cost_usd_total[1h]))

# Duración del percentil 95 de ejecución del modelo
histogram_quantile(
  0.95,
...
# SLO de tiempo de espera de cola (95p por debajo de 2s)
histogram_quantile(
  0.95,
...
<Tip>
Prefiere `gen_ai_client_token_usage` para paneles entre proveedores: sigue las convenciones semánticas GenAI de OpenTelemetry y es coherente con las métricas de servicios GenAI que no son OpenClaw.
</Tip>

## Elegir entre Prometheus y la exportación de OpenTelemetry

OpenClaw admite ambas superficies de forma independiente. Puedes ejecutar cualquiera, ambas o ninguna.
...
<AccordionGroup>
  ...
  <Accordion title="401 / no autorizado">
    El endpoint requiere el alcance de operador del Gateway (`auth: "gateway"` con `gatewayRuntimeScopeSurface: "trusted-operator"`). Usa el mismo token o contraseña que Prometheus usa para cualquier otra ruta de operador del Gateway. No hay modo público sin autenticar.
  </Accordion>
  <Accordion title="`openclaw_prometheus_series_dropped_total` está aumentando">
    Un atributo nuevo está superando el límite de **2048** series. Inspecciona las métricas recientes para encontrar una etiqueta con cardinalidad inesperadamente alta y corrígela en el origen. El exportador descarta intencionalmente las series nuevas en lugar de reescribir etiquetas silenciosamente.
  </Accordion>
  <Accordion title="Prometheus muestra series obsoletas después de un reinicio">
    El Plugin mantiene el estado solo en memoria. Después de reiniciar el Gateway, los contadores se restablecen a cero y los indicadores se reinician con su siguiente valor informado. Usa PromQL `rate()` e `increase()` para gestionar los reinicios limpiamente.
  </Accordion>
</AccordionGroup>
...
- [Salud y preparación](/es/gateway/health) — comprobaciones `/healthz` y `/readyz`
