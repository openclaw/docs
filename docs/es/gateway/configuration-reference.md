...
Referencia de configuración principal para `~/.openclaw/openclaw.json`. Para una vista general orientada a tareas, consulte [Configuration](/es/gateway/configuration).

Esta página cubre las principales superficies de configuración de OpenClaw y enlaza hacia afuera cuando un subsistema tiene su propia referencia más profunda. **No** intenta incluir en una sola página cada catálogo de comandos perteneciente a canales/plugins ni cada ajuste profundo de memoria/QMD.

Fuente de verdad del código:

- `openclaw config schema` imprime el JSON Schema activo usado para la validación y la interfaz de Control, con metadatos agrupados de plugins/canales cuando están disponibles
- `config.schema.lookup` devuelve un nodo de esquema con alcance a una ruta para herramientas de inspección detallada
...
- [Comandos de barra](/es/tools/slash-commands) para el catálogo actual de comandos integrados + agrupados
...
| `open`                | Omite las listas de permitidos de grupo (el filtrado por mención sigue aplicándose) |
...
Los códigos de vinculación vencen después de 1 hora. Las solicitudes pendientes de vinculación por MD están limitadas a **3 por canal**.
...
Use `channels.modelByChannel` para fijar IDs de canal específicos a un modelo. Los valores aceptan `provider/model` o alias de modelo configurados. La asignación del canal se aplica cuando una sesión aún no tiene una sobrescritura de modelo (por ejemplo, establecida mediante `/model`).
...
Use `channels.defaults` para el comportamiento compartido de política de grupo y Heartbeat entre proveedores:
...
- `channels.defaults.heartbeat.useIndicator`: renderiza una salida compacta de Heartbeat con estilo de indicador.
...
```json5
{
  channels: {
    whatsapp: {
      ...
      mediaMaxMb: 50,
      sendReadReceipts: true, // double blue check (false in self-chat mode)
      groups: {
        "*": { requireMention: true },
      },
...
- En configuraciones con múltiples cuentas (2+ ids de cuenta), establezca un valor predeterminado explícito (`channels.telegram.defaultAccount` o `channels.telegram.accounts.default`) para evitar el enrutamiento de respaldo; `openclaw doctor` advierte cuando falta o no es válido.
...
- Las entradas de nivel superior `bindings[]` con `type: "acp"` configuran enlaces persistentes de ACP para temas de foros (use el `chatId:topic:topicId` canónico en `match.peer.id`). La semántica de los campos se comparte en [ACP Agents](/es/tools/acp-agents#channel-specific-settings).
...
- Use `user:<id>` (MD) o `channel:<id>` (canal de guild) para destinos de entrega; los IDs numéricos sin prefijo se rechazan.
- Los slugs de guild están en minúsculas y con los espacios reemplazados por `-`; las claves de canal usan el nombre en formato slug (sin `#`). Prefiera los IDs de guild.
- Los mensajes creados por bots se ignoran de forma predeterminada. `allowBots: true` los habilita; use `allowBots: "mentions"` para aceptar solo mensajes de bots que mencionen al bot (los propios mensajes siguen filtrados).
...
- `maxLinesPerMessage` (17 de forma predeterminada) divide los mensajes altos incluso cuando están por debajo de 2000 caracteres.
- `channels.discord.threadBindings` controla el enrutamiento vinculado a hilos de Discord:
  ...
  - `idleHours`: sobrescritura de Discord para desfocalización automática por inactividad en horas (`0` la desactiva)
  - `maxAgeHours`: sobrescritura de Discord para antigüedad máxima estricta en horas (`0` la desactiva)
...
- Las entradas de nivel superior `bindings[]` con `type: "acp"` configuran enlaces persistentes de ACP para canales e hilos (use el id del canal/hilo en `match.peer.id`). La semántica de los campos se comparte en [ACP Agents](/es/tools/acp-agents#channel-specific-settings).
...
- `channels.discord.execApprovals`: entrega de aprobaciones exec nativa de Discord y autorización de aprobadores.
  ...
  - `agentFilter`: lista opcional de permitidos de IDs de agente. Omítala para reenviar aprobaciones para todos los agentes.
  ...
  - `cleanupAfterResolve`: cuando es `true`, elimina los MD de aprobación después de aprobar, denegar o agotar el tiempo de espera.
...
- Use `spaces/<spaceId>` o `users/<userId>` para destinos de entrega.
...
- **Modo socket** requiere `botToken` y `appToken` (`SLACK_BOT_TOKEN` + `SLACK_APP_TOKEN` como respaldo por variable de entorno para la cuenta predeterminada).
...
- Use `user:<id>` (MD) o `channel:<id>` para destinos de entrega.
...
- La transmisión nativa de Slack más el estado de hilo estilo asistente de Slack “is typing...” requieren un destino de respuesta en hilo. Los MD de nivel superior permanecen fuera de hilo de forma predeterminada, por lo que usan `typingReaction` o entrega normal en lugar de la vista previa estilo hilo.
- `typingReaction` añade una reacción temporal al mensaje entrante de Slack mientras se ejecuta una respuesta y luego la elimina al completarse. Use un shortcode de emoji de Slack como `"hourglass_flowing_sand"`.
...
- `commands.callbackUrl` debe resolverse al endpoint Gateway de OpenClaw y debe ser accesible desde el servidor Mattermost.
- Las devoluciones de llamada nativas de slash se autentican con los tokens por comando devueltos por Mattermost durante el registro del slash command. Si el registro falla o no se activa ningún comando, OpenClaw rechaza las devoluciones de llamada con `Unauthorized: invalid command token.`
...
- `channels.mattermost.groups.<channelId>.requireMention`: sobrescritura de filtrado por mención por canal (`"*"` como predeterminado).
...
- Las entradas de nivel superior `bindings[]` con `type: "acp"` pueden vincular conversaciones de BlueBubbles a sesiones persistentes de ACP. Use un identificador o cadena de destino de BlueBubbles (`chat_id:*`, `chat_guid:*`, `chat_identifier:*`) en `match.peer.id`. Semántica compartida de campos: [ACP Agents](/es/tools/acp-agents#channel-specific-settings).
...
- `cliPath` puede apuntar a un wrapper SSH; establezca `remoteHost` (`host` o `user@host`) para obtener adjuntos mediante SCP.
...
- Las entradas de nivel superior `bindings[]` con `type: "acp"` pueden vincular conversaciones de iMessage a sesiones persistentes de ACP. Use un identificador normalizado o un destino de chat explícito (`chat_id:*`, `chat_guid:*`, `chat_identifier:*`) en `match.peer.id`. Semántica compartida de campos: [ACP Agents](/es/tools/acp-agents#channel-specific-settings).
...
- `channels.matrix.autoJoin` tiene como valor predeterminado `off`, por lo que las salas invitadas y las nuevas invitaciones tipo MD se ignoran hasta que establezca `autoJoin: "allowlist"` con `autoJoinAllowlist` o `autoJoin: "always"`.
...
- La configuración completa del canal IRC (host/puerto/TLS/canales/listas de permitidos/filtrado por mención) está documentada en [IRC](/es/channels/irc).
...
### Filtrado por mención en chats de grupo

Los mensajes de grupo requieren mención de forma predeterminada (**require mention**) (mención de metadatos o patrones regex seguros). Se aplica a los chats grupales de WhatsApp, Telegram, Discord, Google Chat e iMessage.
...
- El filtrado por mención solo se aplica cuando la detección es posible (menciones nativas o al menos un patrón).
...
- Este bloque configura las superficies de comandos. Para el catálogo actual de comandos integrados + agrupados, consulte [Slash Commands](/es/tools/slash-commands).
...
- `config: true` habilita `/config` (lee/escribe `openclaw.json`). Para clientes `chat.send` de Gateway, las escrituras persistentes de `/config set|unset` también requieren `operator.admin`; el `/config show` de solo lectura sigue disponible para clientes operador normales con alcance de escritura.
...
- `restart: false` desactiva `/restart` y las acciones de la herramienta de reinicio de Gateway. Predeterminado: `true`.
- `ownerAllowFrom` es la lista explícita de permitidos del propietario para comandos/herramientas solo para propietario. Es independiente de `allowFrom`.
- `ownerDisplay: "hash"` aplica hash a los ids de propietario en el system prompt. Establezca `ownerDisplaySecret` para controlar el hash.
- `allowFrom` es por proveedor. Cuando está definido, es la **única** fuente de autorización (las listas de permitidos/vinculación del canal y `useAccessGroups` se ignoran).
...
- Mapa de documentación de comandos:
  ...
  - superficies de comandos específicas por canal: [Channels](/es/channels)
...
Raíz opcional del repositorio mostrada en la línea Runtime del system prompt. Si no se define, OpenClaw la detecta automáticamente recorriendo hacia arriba desde el espacio de trabajo.
...
- Omita `agents.defaults.skills` para permitir Skills sin restricciones de forma predeterminada.
- Omita `agents.list[].skills` para heredar los valores predeterminados.
- Establezca `agents.list[].skills: []` para no usar Skills.
...
- `"continuation-skip"`: los turnos seguros de continuación (después de una respuesta completada del asistente) omiten la reinyección bootstrap del espacio de trabajo, lo que reduce el tamaño del prompt. Las ejecuciones de Heartbeat y los reintentos posteriores a Compaction siguen reconstruyendo el contexto.
...
divididos intencionalmente por subsistema en lugar de fluir todos a través de un
solo ajuste genérico.
...
- `agents.defaults.contextLimits.*`:
  extractos delimitados en tiempo de ejecución y bloques inyectados propiedad del entorno de ejecución.
- `memory.qmd.limits.*`:
  dimensionamiento de fragmentos de búsqueda de memoria indexada e inyección.
...
Controla el preludio de inicio del primer turno inyectado en ejecuciones vacías de `/new` y `/reset`.
...
Valores predeterminados compartidos para superficies de contexto delimitadas en tiempo de ejecución.
...
- `memoryGetMaxChars`: límite predeterminado del extracto de `memory_get` antes de que se añadan metadatos de truncamiento y aviso de continuación.
...
Sobrescritura por agente para los ajustes compartidos de `contextLimits`. Los campos omitidos heredan de `agents.defaults.contextLimits`.
...
- `imageGenerationModel`: acepta una cadena (`"provider/model"`) o un objeto (`{ primary, fallbacks }`).
  ...
  - Si selecciona directamente un proveedor/modelo, configure también la autenticación/clave API del proveedor correspondiente (por ejemplo `GEMINI_API_KEY` o `GOOGLE_API_KEY` para `google/*`, `OPENAI_API_KEY` para `openai/*`, `FAL_KEY` para `fal/*`).
  - Si se omite, `image_generate` aún puede inferir un proveedor predeterminado respaldado por autenticación. Primero prueba el proveedor predeterminado actual y luego los proveedores de generación de imágenes registrados restantes en orden de id de proveedor.
- `musicGenerationModel`: acepta una cadena (`"provider/model"`) o un objeto (`{ primary, fallbacks }`).
  ...
  - Si se omite, `music_generate` aún puede inferir un proveedor predeterminado respaldado por autenticación. Primero prueba el proveedor predeterminado actual y luego los proveedores de generación de música registrados restantes en orden de id de proveedor.
  - Si selecciona directamente un proveedor/modelo, configure también la autenticación/clave API del proveedor correspondiente.
- `videoGenerationModel`: acepta una cadena (`"provider/model"`) o un objeto (`{ primary, fallbacks }`).
  ...
  - Si se omite, `video_generate` aún puede inferir un proveedor predeterminado respaldado por autenticación. Primero prueba el proveedor predeterminado actual y luego los proveedores de generación de video registrados restantes en orden de id de proveedor.
  - Si selecciona directamente un proveedor/modelo, configure también la autenticación/clave API del proveedor correspondiente.
...
- `model.primary`: formato `provider/model` (p. ej. `openai/gpt-5.4`). Si omite el proveedor, OpenClaw primero intenta un alias, luego una coincidencia única de proveedor configurado para ese id de modelo exacto, y solo entonces usa como respaldo el proveedor predeterminado configurado (comportamiento heredado en desuso por compatibilidad, así que prefiera `provider/model` explícito). Si ese proveedor ya no expone el modelo predeterminado configurado, OpenClaw usa como respaldo el primer proveedor/modelo configurado en lugar de mostrar un valor predeterminado obsoleto de un proveedor eliminado.
...
- `params`: parámetros predeterminados globales del proveedor aplicados a todos los modelos. Se establecen en `agents.defaults.params` (p. ej. `{ cacheRetention: "long" }`).
...
- `embeddedHarness`: política predeterminada de entorno de ejecución integrado de bajo nivel para agentes. Use `runtime: "auto"` para dejar que los harness de plugins registrados reclamen modelos compatibles, `runtime: "pi"` para forzar el harness integrado de Pi o un id de harness registrado como `runtime: "codex"`. Establezca `fallback: "none"` para desactivar el respaldo automático de Pi.
- Los escritores de configuración que mutan estos campos (por ejemplo `/models set`, `/models set-image` y comandos add/remove de respaldo) guardan la forma canónica de objeto y conservan las listas de respaldo existentes cuando es posible.
...
Úselo cuando un plugin de confianza proporcione un harness nativo, como el harness
agrupado del servidor de aplicaciones Codex.
...
- `runtime`: `"auto"`, `"pi"` o un id de harness de plugin registrado. El Plugin agrupado Codex registra `codex`.
- `fallback`: `"pi"` o `"none"`. `"pi"` mantiene el harness integrado de Pi como respaldo de compatibilidad. `"none"` hace que una selección de harness de plugin ausente o no compatible falle en lugar de usar Pi silenciosamente.
...
- Para implementaciones solo de Codex, establezca `model: "codex/gpt-5.4"`, `embeddedHarness.runtime: "codex"` y `embeddedHarness.fallback: "none"`.
...
Los modelos GLM-4.x de Z.AI habilitan automáticamente el modo thinking a menos que establezca `--thinking off` o defina usted mismo `agents.defaults.models["zai/<model>"].params.thinking`.
Los modelos de Z.AI habilitan `tool_stream` de forma predeterminada para la transmisión de llamadas de herramientas. Establezca `agents.defaults.models["zai/<model>"].params.tool_stream` en `false` para desactivarlo.
Los modelos Claude 4.6 de Anthropic usan `adaptive` thinking de forma predeterminada cuando no se establece un nivel de thinking explícito.
...
- Los backends CLI están orientados a texto; las herramientas siempre están desactivadas.
...
- `every`: cadena de duración (ms/s/m/h). Predeterminado: `30m` (autenticación con clave API) o `1h` (autenticación OAuth). Establezca `0m` para desactivarlo.
...
- `timeoutSeconds`: tiempo máximo en segundos permitido para un turno de agente de Heartbeat antes de abortarlo. Déjelo sin definir para usar `agents.defaults.timeoutSeconds`.
- `directPolicy`: política de entrega directa/MD. `allow` (predeterminado) permite entrega con destino directo. `block` suprime la entrega con destino directo y emite `reason=dm-blocked`.
...
- `isolatedSession`: cuando es true, cada ejecución de Heartbeat se ejecuta en una sesión nueva sin historial de conversación previo. Mismo patrón de aislamiento que Cron `sessionTarget: "isolated"`. Reduce el costo de tokens por Heartbeat de ~100K a ~2-5K tokens.
...
- Heartbeat ejecuta turnos completos de agente: intervalos más cortos consumen más tokens.
...
- `provider`: id de un Plugin proveedor de Compaction registrado. Cuando se establece, se llama a `summarize()` del proveedor en lugar del resumen LLM integrado. En caso de fallo, vuelve al integrado. Establecer un proveedor fuerza `mode: "safeguard"`. Consulte [Compaction](/es/concepts/compaction).
...
- `postCompactionSections`: nombres opcionales de secciones H2/H3 de AGENTS.md para reinyectar después de Compaction. Predeterminado: `["Session Startup", "Red Lines"]`; establezca `[]` para desactivar la reinyección. Cuando no está definido o se establece explícitamente en ese par predeterminado, también se aceptan los encabezados heredados `Every Session`/`Safety` como respaldo heredado.
- `model`: sobrescritura opcional `provider/model-id` solo para el resumen de Compaction. Úselo cuando la sesión principal deba conservar un modelo, pero los resúmenes de Compaction deban ejecutarse con otro; cuando no se define, Compaction usa el modelo principal de la sesión.
- `notifyUser`: cuando es `true`, envía avisos breves al usuario cuando comienza y cuando termina Compaction (por ejemplo, "Compacting context..." y "Compaction complete"). Está desactivado de forma predeterminada para mantener Compaction en silencio.
- `memoryFlush`: turno silencioso y agéntico antes de la auto-Compaction para almacenar memorias duraderas. Se omite cuando el espacio de trabajo es de solo lectura.
...
- La poda primero aplica recorte suave a resultados de herramientas sobredimensionados y luego vaciado completo a resultados antiguos si es necesario.
...
Consulte [Session Pruning](/es/concepts/session-pruning) para detalles del comportamiento.
...
Consulte [Streaming](/es/concepts/streaming) para detalles del comportamiento y de la fragmentación.
...
**Los contenedores usan por defecto `network: "none"`**; establézcalo en `"bridge"` (o una red bridge personalizada) si el agente necesita acceso saliente.
...
**`docker.binds`** monta directorios adicionales del host; los mounts globales y por agente se fusionan.
...
- `network` usa por defecto `openclaw-sandbox-browser` (red bridge dedicada). Establézcalo en `bridge` solo cuando quiera explícitamente conectividad global de bridge.
...
- Los valores predeterminados de lanzamiento se definen en `scripts/sandbox-browser-entrypoint.sh` y están ajustados para hosts con contenedores:
  ...
  - Los valores predeterminados son la línea base de la imagen del contenedor; use una imagen de navegador personalizada con un entrypoint personalizado para cambiar los valores predeterminados del contenedor.
...
- `default`: cuando hay varios definidos, gana el primero (se registra una advertencia). Si no hay ninguno definido, la primera entrada de la lista es la predeterminada.
- `model`: la forma de cadena sobrescribe solo `primary`; la forma de objeto `{ primary, fallbacks }` sobrescribe ambos (`[]` desactiva los respaldos globales). Los trabajos de Cron que solo sobrescriben `primary` siguen heredando los respaldos predeterminados, a menos que establezca `fallbacks: []`.
- `params`: parámetros de flujo por agente combinados sobre la entrada de modelo seleccionada en `agents.defaults.models`. Úselo para sobrescrituras específicas del agente como `cacheRetention`, `temperature` o `maxTokens` sin duplicar todo el catálogo de modelos.
...
- `thinkingDefault`: valor predeterminado opcional por agente para el nivel de thinking (`off | minimal | low | medium | high | xhigh | adaptive | max`). Sobrescribe `agents.defaults.thinkingDefault` para este agente cuando no hay sobrescritura por mensaje o sesión.
- `reasoningDefault`: valor predeterminado opcional por agente para la visibilidad del razonamiento (`on | off | stream`). Se aplica cuando no hay sobrescritura de razonamiento por mensaje o sesión.
- `fastModeDefault`: valor predeterminado opcional por agente para fast mode (`true | false`). Se aplica cuando no hay sobrescritura de fast mode por mensaje o sesión.
- `embeddedHarness`: sobrescritura opcional por agente para la política de harness de bajo nivel. Use `{ runtime: "codex", fallback: "none" }` para hacer que un agente sea solo Codex mientras otros conservan el respaldo predeterminado de Pi.
- `runtime`: descriptor opcional de entorno de ejecución por agente. Use `type: "acp"` con valores predeterminados `runtime.acp` (`agent`, `backend`, `mode`, `cwd`) cuando el agente deba usar por defecto sesiones de harness ACP.
...
- `subagents.requireAgentId`: cuando es true, bloquea llamadas a `sessions_spawn` que omiten `agentId` (fuerza la selección explícita del perfil; predeterminado: false).

---

## Enrutamiento multiagente

Ejecute múltiples agentes aislados dentro de un Gateway. Consulte [Multi-Agent](/es/concepts/multi-agent).
...
Para entradas `type: "acp"`, OpenClaw resuelve por identidad exacta de conversación (`match.channel` + cuenta + `match.peer.id`) y no usa el orden de niveles de binding de route anterior.
...
Consulte [Multi-Agent Sandbox & Tools](/es/tools/multi-agent-sandbox-tools) para detalles de precedencia.
...
- **`scope`**: estrategia base de agrupación de sesiones para contextos de chat grupal.
  ...
  - `global`: todos los participantes de un contexto de canal comparten una sola sesión (úselo solo cuando se pretenda compartir contexto).
...
- **`reset`**: política principal de reinicio. `daily` reinicia a la hora local `atHour`; `idle` reinicia después de `idleMinutes`. Cuando ambas están configuradas, se aplica la que expire primero.
...
- **`parentForkMaxTokens`**: máximo de `totalTokens` de la sesión padre permitido al crear una sesión de hilo bifurcada (predeterminado `100000`).
  ...
  - Establezca `0` para desactivar esta protección y permitir siempre la bifurcación desde el padre.
...
- **`sendPolicy`**: coincide por `channel`, `chatType` (`direct|group|channel`, con alias heredado `dm`), `keyPrefix` o `rawKeyPrefix`. La primera denegación tiene prioridad.
- **`maintenance`**: controles de limpieza + retención del almacén de sesiones.
  ...
  - `resetArchiveRetention`: retención para archivos de transcripción `*.reset.<timestamp>`. Usa `pruneAfter` por defecto; establezca `false` para desactivarlo.
...
- **`threadBindings`**: valores predeterminados globales para funciones de sesión vinculadas a hilos.
  - `enabled`: interruptor predeterminado maestro (los proveedores pueden sobrescribirlo; Discord usa `channels.discord.threadBindings.enabled`)
  - `idleHours`: desfocalización automática predeterminada por inactividad en horas (`0` la desactiva; los proveedores pueden sobrescribirlo)
  - `maxAgeHours`: antigüedad máxima estricta predeterminada en horas (`0` la desactiva; los proveedores pueden sobrescribirlo)
...
- Usa por defecto `identity.emoji` del agente activo; en caso contrario `"👀"`. Establezca `""` para desactivarla.
...
- `removeAckAfterReply`: elimina el acuse después de la respuesta en Slack, Discord y Telegram.
- `messages.statusReactions.enabled`: habilita reacciones de estado de ciclo de vida en Slack, Discord y Telegram.
  En Slack y Discord, si no se define, mantiene las reacciones de estado habilitadas cuando las reacciones de acuse están activas.
  En Telegram, establézcalo explícitamente en `true` para habilitar las reacciones de estado de ciclo de vida.
...
- `silenceTimeoutMs` controla cuánto espera el modo Talk después del silencio del usuario antes de enviar la transcripción. Si no se define, se conserva la ventana de pausa predeterminada de la plataforma (`700 ms en macOS y Android, 900 ms en iOS`).
...
Política global de permitir/denegar herramientas (deny tiene prioridad). No distingue mayúsculas de minúsculas y admite comodines `*`. Se aplica incluso cuando el sandbox de Docker está desactivado.
...
- `historySize`: historial máximo de llamadas de herramientas retenido para el análisis de bucles.
...
- `detectors.pingPong`: advierte/bloquea en patrones alternantes de pares sin progreso.
...
- `agent`: cualquier sesión que pertenezca al id del agente actual (puede incluir otros usuarios si ejecuta sesiones por remitente con el mismo id de agente).
...
Indicadores experimentales de herramientas integradas. Predeterminado: desactivado, a menos que se aplique una regla de autoactivación estricta-agentic de GPT-5.
...
- Predeterminado: `false`, a menos que `agents.defaults.embeddedPi.executionContract` (o una sobrescritura por agente) esté establecido en `"strict-agentic"` para una ejecución de la familia GPT-5 de OpenAI o OpenAI Codex. Establezca `true` para forzar la herramienta fuera de ese ámbito, o `false` para mantenerla desactivada incluso en ejecuciones GPT-5 strict-agentic.
- Cuando está habilitada, el system prompt también añade guía de uso para que el modelo solo la use en trabajo sustancial y mantenga como máximo un paso `in_progress`.
...
- `model`: modelo predeterminado para los subagentes iniciados. Si se omite, los subagentes heredan el modelo del llamador.
- `allowAgents`: lista predeterminada de permitidos de ids de agentes de destino para `sessions_spawn` cuando el agente solicitante no define su propio `subagents.allowAgents` (`["*"]` = cualquiera; predeterminado: solo el mismo agente).
...
OpenClaw usa el catálogo de modelos integrado. Añada proveedores personalizados mediante `models.providers` en la configuración o `~/.openclaw/agents/<agentId>/agent/models.json`.
...
- Precedencia de combinación para ids de proveedor coincidentes:
  ...
  - Los valores `apiKey` del proveedor administrados por SecretRef se actualizan a partir de marcadores de origen (`ENV_VAR_NAME` para referencias de entorno, `secretref-managed` para referencias de archivo/exec) en lugar de persistir secretos resueltos.
  - Los valores de encabezado del proveedor administrados por SecretRef se actualizan a partir de marcadores de origen (`secretref-env:ENV_VAR_NAME` para referencias de entorno, `secretref-managed` para referencias de archivo/exec).
  - Los valores vacíos o ausentes de `apiKey`/`baseUrl` del agente usan como respaldo `models.providers` en la configuración.
  ...
  - Los valores coincidentes de modelo `contextTokens` conservan un límite explícito del entorno de ejecución cuando está presente; úselo para limitar el contexto efectivo sin cambiar los metadatos nativos del modelo.
...
- `models.providers.*.apiKey`: credencial del proveedor (prefiera SecretRef/sustitución por entorno).
...
- `models.providers.*.request`: sobrescrituras de transporte para solicitudes HTTP del proveedor de modelos.
  ...
  - `request.auth`: sobrescritura de estrategia de autenticación. Modos: `"provider-default"` (usa la autenticación integrada del proveedor), `"authorization-bearer"` (con `token`), `"header"` (con `headerName`, `value` y `prefix` opcional).
  ...
  - `request.allowPrivateNetwork`: cuando es `true`, permite HTTPS a `baseUrl` cuando el DNS resuelve a rangos privados, CGNAT o similares, mediante la protección SSRF de fetch HTTP del proveedor (activación opcional del operador para endpoints compatibles con OpenAI autohospedados de confianza). WebSocket usa el mismo `request` para encabezados/TLS, pero no esa protección SSRF de fetch. Predeterminado: `false`.
...
- `models.providers.*.models.*.contextTokens`: límite opcional de contexto en tiempo de ejecución. Úselo cuando quiera un presupuesto de contexto efectivo menor que `contextWindow` nativo del modelo.
- `models.providers.*.models.*.compat.supportsDeveloperRole`: sugerencia opcional de compatibilidad. Para `api: "openai-completions"` con `baseUrl` no nativo no vacío (host distinto de `api.openai.com`), OpenClaw fuerza esto a `false` en tiempo de ejecución. Un `baseUrl` vacío u omitido mantiene el comportamiento predeterminado de OpenAI.
- `models.providers.*.models.*.compat.requiresStringContent`: sugerencia opcional de compatibilidad para endpoints de chat compatibles con OpenAI que solo aceptan cadenas. Cuando es `true`, OpenClaw aplana los arrays `messages[].content` de texto puro a cadenas simples antes de enviar la solicitud.
...
- Para el endpoint general, defina un proveedor personalizado con la sobrescritura de base URL.
...
Los endpoints nativos de Moonshot anuncian compatibilidad de uso de transmisión en el transporte compartido
...
La base URL debe omitir `/v1` (el cliente Anthropic lo añade). Atajo: `openclaw onboard --auth-choice synthetic-api-key`.
...
En la ruta de transmisión compatible con Anthropic, OpenClaw desactiva el thinking de MiniMax
de forma predeterminada a menos que usted establezca explícitamente `thinking`. `/fast on` o
...
Consulte [Local Models](/es/gateway/local-models). En resumen: ejecute un modelo local grande mediante la API Responses de LM Studio en hardware potente; mantenga los modelos alojados combinados para respaldo.
...
- `allowBundled`: lista opcional de permitidos solo para Skills agrupadas (las Skills administradas/del espacio de trabajo no se ven afectadas).
...
- `install.preferBrew`: cuando es true, prefiere instaladores de Homebrew cuando `brew` está
  disponible antes de usar otros tipos de instalador.
- `install.nodeManager`: preferencia del gestor de Node para especificaciones
...
- **Los cambios de configuración requieren reiniciar Gateway.**
...
- `plugins.entries.<id>.hooks.allowPromptInjection`: cuando es `false`, el núcleo bloquea `before_prompt_build` e ignora campos que mutan el prompt de `before_agent_start` heredado, mientras conserva `modelOverride` y `providerOverride` heredados. Se aplica a hooks de plugins nativos y a directorios de hooks proporcionados por bundles compatibles.
...
- `plugins.entries.<id>.subagent.allowedModels`: lista opcional de permitidos de destinos canónicos `provider/model` para sobrescrituras confiables de subagentes. Use `"*"` solo cuando quiera intencionalmente permitir cualquier modelo.
...
- `plugins.entries.memory-core.config.dreaming`: configuración de Dreaming de memoria. Consulte [Dreaming](/es/concepts/dreaming) para fases y umbrales.
...
- Los plugins de bundles Claude habilitados también pueden aportar valores predeterminados integrados de Pi desde `settings.json`; OpenClaw los aplica como ajustes de agente saneados, no como parches de configuración en bruto de OpenClaw.
- `plugins.slots.memory`: elija el id del plugin de memoria activo, o `"none"` para desactivar los plugins de memoria.
- `plugins.slots.contextEngine`: elija el id del plugin del motor de contexto activo; usa `"legacy"` de forma predeterminada a menos que instale y seleccione otro motor.
- `plugins.installs`: metadatos de instalación administrados por CLI usados por `openclaw plugins update`.
  ...
  - Trate `plugins.installs.*` como estado administrado; prefiera los comandos CLI antes que las ediciones manuales.
...
- `ssrfPolicy.dangerouslyAllowPrivateNetwork` está desactivado cuando no se define, por lo que la navegación del navegador sigue siendo estricta de forma predeterminada.
...
- Los perfiles remotos son solo de conexión adjunta (inicio/detención/reinicio desactivados).
...
- Los perfiles `existing-session` mantienen los límites actuales de la ruta Chrome MCP:
  acciones guiadas por snapshot/ref en lugar de direccionamiento por selector CSS, hooks de carga
  de un solo archivo, sin sobrescrituras de tiempo de espera de diálogos, sin `wait --load networkidle` y sin
...
- Los perfiles `openclaw` administrados localmente asignan automáticamente `cdpPort` y `cdpUrl`; solo
  defina `cdpUrl` explícitamente para CDP remoto.
...
- `extraArgs` añade flags extra de inicio al arranque local de Chromium (por ejemplo
...
- `seamColor`: color de acento para el chrome de la UI de la aplicación nativa (tinte de burbuja de modo Talk, etc.).
- `assistant`: sobrescritura de identidad de la interfaz de Control. Usa como respaldo la identidad del agente activo.
...
- Si se configuran tanto `gateway.auth.token` como `gateway.auth.password` (incluidos SecretRefs), establezca `gateway.auth.mode` explícitamente en `token` o `password`. Los flujos de inicio e instalación/reparación del servicio fallan cuando ambos están configurados y el modo no está definido.
- `gateway.auth.mode: "none"`: modo explícito sin autenticación. Úselo solo para configuraciones locales de local loopback de confianza; intencionalmente no se ofrece en los prompts de incorporación.
- `gateway.auth.mode: "trusted-proxy"`: delega la autenticación a un proxy inverso con reconocimiento de identidad y confía en los encabezados de identidad de `gateway.trustedProxies` (consulte [Trusted Proxy Auth](/es/gateway/trusted-proxy-auth)). Este modo espera un origen de proxy **no loopback**; los proxies inversos en loopback del mismo host no cumplen los requisitos de autenticación trusted-proxy.
- `gateway.auth.allowTailscale`: cuando es `true`, los encabezados de identidad de Tailscale Serve pueden satisfacer la autenticación de la interfaz de Control/WebSocket (verificada mediante `tailscale whois`). Los endpoints HTTP API **no** usan esa autenticación por encabezado de Tailscale; siguen en su lugar el modo normal de autenticación HTTP de Gateway. Este flujo sin token asume que el host Gateway es de confianza. Usa `true` por defecto cuando `tailscale.mode = "serve"`.
- `gateway.auth.rateLimit`: limitador opcional de autenticación fallida. Se aplica por IP cliente y por alcance de autenticación (secreto compartido y token de dispositivo se rastrean de forma independiente). Los intentos bloqueados devuelven `429` + `Retry-After`.
  - En la ruta asíncrona de la interfaz de Control de Tailscale Serve, los intentos fallidos del mismo `{scope, clientIp}` se serializan antes de escribir el fallo. Por tanto, intentos incorrectos concurrentes del mismo cliente pueden activar el limitador en la segunda solicitud en lugar de dejar que ambas pasen como simples discrepancias.
  - `gateway.auth.rateLimit.exemptLoopback` usa `true` por defecto; establézcalo en `false` cuando quiera intencionalmente que el tráfico localhost también tenga limitación de tasa (para configuraciones de prueba o implementaciones estrictas con proxy).
...
- `controlUi.allowedOrigins`: lista explícita de permitidos de orígenes de navegador para conexiones WebSocket de Gateway. Obligatoria cuando se esperan clientes de navegador desde orígenes que no son loopback.
- `controlUi.dangerouslyAllowHostHeaderOriginFallback`: modo peligroso que habilita el respaldo de origen por encabezado Host para implementaciones que dependen intencionalmente de una política de origen basada en encabezado Host.
- `remote.transport`: `ssh` (predeterminado) o `direct` (ws/wss). Para `direct`, `remote.url` debe ser `ws://` o `wss://`.
- `OPENCLAW_ALLOW_INSECURE_PRIVATE_WS=1`: sobrescritura de emergencia del lado cliente que permite `ws://` en texto plano hacia IPs de red privada de confianza; el valor predeterminado sigue siendo solo loopback para texto plano.
- `gateway.remote.token` / `.password` son campos de credenciales del cliente remoto. No configuran por sí mismos la autenticación de Gateway.
- `gateway.push.apns.relay.baseUrl`: base HTTPS URL del relay APNs externo usado por las compilaciones oficiales/TestFlight de iOS después de que publiquen registros respaldados por relay en Gateway. Esta URL debe coincidir con la URL del relay compilada en la compilación de iOS.
- `gateway.push.apns.relay.timeoutMs`: tiempo de espera en milisegundos para envíos de Gateway al relay. Predeterminado: `10000`.
- Los registros respaldados por relay se delegan a una identidad específica de Gateway. La app iOS emparejada obtiene `gateway.identity.get`, incluye esa identidad en el registro del relay y reenvía a Gateway una concesión de envío con alcance al registro. Otro Gateway no puede reutilizar ese registro almacenado.
...
- `gateway.channelHealthCheckMinutes`: intervalo del monitor de estado de canales en minutos. Establezca `0` para desactivar globalmente los reinicios del monitor de estado. Predeterminado: `5`.
...
- Las rutas de llamadas de Gateway local pueden usar `gateway.remote.*` como respaldo solo cuando `gateway.auth.*` no está definido.
- Si `gateway.auth.token` / `gateway.auth.password` se configuran explícitamente mediante SecretRef y no se resuelven, la resolución falla de forma cerrada (sin enmascaramiento de respaldo remoto).
- `trustedProxies`: IPs de proxy inverso que terminan TLS o inyectan encabezados de cliente reenviado. Liste solo proxies que usted controle. Las entradas loopback siguen siendo válidas para configuraciones de proxy local/detección local del mismo host (por ejemplo Tailscale Serve o un proxy inverso local), pero **no** hacen que las solicitudes loopback sean aptas para `gateway.auth.mode: "trusted-proxy"`.
- `allowRealIpFallback`: cuando es `true`, Gateway acepta `X-Real-IP` si falta `X-Forwarded-For`. Predeterminado `false` para comportamiento fail-closed.
...
- Fortalecimiento de entradas URL para Responses:
...
- Encabezado opcional de fortalecimiento de respuesta:
  - `gateway.http.securityHeaders.strictTransportSecurity` (defínalo solo para orígenes HTTPS que usted controle; consulte [Trusted Proxy Auth](/es/gateway/trusted-proxy-auth#tls-termination-and-hsts))

### Aislamiento de múltiples instancias

Ejecute múltiples Gateways en un mismo host con puertos y directorios de estado únicos:
...
- `keyPath`: ruta del sistema de archivos a la clave privada TLS; mantenga los permisos restringidos.
...
- `mode`: controla cómo se aplican las ediciones de configuración en tiempo de ejecución.
  ...
  - `"restart"`: siempre reinicia el proceso Gateway ante un cambio de configuración.
  - `"hot"`: aplica cambios dentro del proceso sin reiniciar.
  - `"hybrid"` (predeterminado): primero intenta recarga hot; si hace falta, usa como respaldo reinicio.
...
- `hooks.enabled=true` requiere `hooks.token` no vacío.
...
- Si una mapping o preset usa un `sessionKey` con plantilla, establezca `hooks.allowedSessionKeyPrefixes` y `hooks.allowRequestSessionKey=true`. Las claves estáticas de mapping no requieren esa activación opcional.

**Endpoints:**

- `POST /hooks/wake` → `{ text, mode?: "now"|"next-heartbeat" }`
- `POST /hooks/agent` → `{ message, name?, agentId?, sessionKey?, wakeMode?, deliver?, channel?, to?, model?, thinking?, timeoutSeconds? }`
  - `sessionKey` del payload de solicitud solo se acepta cuando `hooks.allowRequestSessionKey=true` (predeterminado: `false`).
- `POST /hooks/<name>` → resuelto mediante `hooks.mappings`
  - Los valores `sessionKey` de mapping renderizados mediante plantilla se tratan como suministrados externamente y también requieren `hooks.allowRequestSessionKey=true`.
...
- `allowedSessionKeyPrefixes`: lista opcional de prefijos permitidos para valores explícitos de `sessionKey` (solicitud + mapping), por ejemplo `["hook:"]`. Se vuelve obligatoria cuando algún mapping o preset usa un `sessionKey` con plantilla.
- `deliver: true` envía la respuesta final a un canal; `channel` usa `last` por defecto.
- `model` sobrescribe el LLM para esta ejecución del hook (debe estar permitido si el catálogo de modelos está definido).
...
- Si conserva ese enrutamiento por mensaje, establezca `hooks.allowRequestSessionKey: true` y restrinja `hooks.allowedSessionKeyPrefixes` para que coincidan con el espacio de nombres de Gmail, por ejemplo `["hook:", "hook:gmail:"]`.
- Si necesita `hooks.allowRequestSessionKey: false`, sobrescriba el preset con un `sessionKey` estático en lugar del valor predeterminado con plantilla.
...
- No ejecute un `gog gmail watch serve` independiente junto con Gateway.
...
- Sirve HTML/CSS/JS editable por agentes y A2UI sobre HTTP bajo el puerto de Gateway:
  - `http://<gateway-host>:<gateway.port>/__openclaw__/canvas/`
  - `http://<gateway-host>:<gateway.port>/__openclaw__/a2ui/`
- Solo local: mantenga `gateway.bind: "loopback"` (predeterminado).
- Binds que no son loopback: las rutas de canvas requieren autenticación de Gateway (token/contraseña/trusted-proxy), igual que otras superficies HTTP de Gateway.
- Los Node WebViews normalmente no envían encabezados de autenticación; después de que un node se empareja y conecta, Gateway anuncia URLs de capacidad con alcance al node para acceso a canvas/A2UI.
- Las URLs de capacidad están vinculadas a la sesión WS activa del node y expiran rápidamente. No se usa respaldo basado en IP.
- Inyecta cliente de recarga en vivo en el HTML servido.
...
- El hostname usa `openclaw` por defecto. Sobrescríbalo con `OPENCLAW_MDNS_HOSTNAME`.

### De área amplia (DNS-SD)
...
- Escápelo con `$${VAR}` para un literal `${VAR}`.
...
Las referencias SecretRef son aditivas: los valores de texto sin formato siguen funcionando.
...
- De forma predeterminada, se rechazan rutas de comando con symlink. Establezca `allowSymlinkCommand: true` para permitir rutas con symlink mientras se valida la ruta del destino resuelto.
- Si `trustedDirs` está configurado, la comprobación de directorios de confianza se aplica a la ruta del destino resuelto.
- El entorno hijo de `exec` es mínimo de forma predeterminada; pase explícitamente las variables requeridas con `passEnv`.
- Las referencias SecretRef se resuelven en el momento de la activación en una instantánea en memoria; después, las rutas de solicitud solo leen esa instantánea.
...
- Las credenciales estáticas en tiempo de ejecución provienen de instantáneas resueltas en memoria; las entradas heredadas estáticas de `auth.json` se limpian cuando se detectan.
- Importaciones heredadas de OAuth desde `~/.openclaw/credentials/oauth.json`.
- Consulte [OAuth](/es/concepts/oauth).
- Comportamiento del entorno de secretos y herramientas `audit/configure/apply`: [Secrets Management](/es/gateway/secrets).
...
- `billingBackoffHours`: backoff base en horas cuando un perfil falla por errores reales
  ...
  específicos del proveedor siguen limitados al proveedor al que pertenecen (por ejemplo OpenRouter
  `Key limit exceeded`). Los mensajes de límite de gasto de ventana de uso
  o de organización/espacio de trabajo con HTTP `402` reintentables se mantienen en cambio en la ruta
  `rate_limit`.
...
- `overloadedProfileRotations`: máximo de rotaciones de auth-profile del mismo proveedor para errores de sobrecarga antes de cambiar al respaldo de modelo (predeterminado: `1`). Las formas de proveedor ocupado como `ModelNotReadyException` llegan aquí.
...
- `consoleLevel` aumenta a `debug` con `--verbose`.
...
- `flags`: array de cadenas de indicadores que habilitan salida de registro dirigida (admite comodines como `"telegram.*"` o `"*"`).
...
- `cacheTrace.includeMessages` / `includePrompt` / `includeSystem`: controlan qué se incluye en la salida del trazado de caché (todos con predeterminado: `true`).
...
- `checkOnStart`: comprueba actualizaciones npm cuando Gateway se inicia (predeterminado: `true`).
- `auto.enabled`: habilita la autoactualización en segundo plano para instalaciones de paquetes (predeterminado: `false`).
...
- `auto.betaCheckIntervalHours`: frecuencia en horas con que se ejecutan las comprobaciones del canal beta (predeterminado: `1`; máximo: `24`).
...
- `dispatch.enabled`: puerta independiente para el despacho de turnos de sesión ACP (predeterminado: `true`). Establézcalo en `false` para mantener disponibles los comandos ACP mientras se bloquea la ejecución.
...
- `allowedAgents`: lista de permitidos de ids de agentes para sesiones del entorno de ejecución ACP; vacío significa sin restricción adicional.
- `maxConcurrentSessions`: máximo de sesiones ACP activas concurrentes.
...
- `stream.tagVisibility`: registro de nombres de etiquetas con sobrescrituras booleanas de visibilidad para eventos transmitidos.
- `runtime.ttlMinutes`: TTL inactivo en minutos para workers de sesión ACP antes de ser elegibles para limpieza.
...
Metadatos escritos por los flujos guiados de configuración de CLI (`onboard`, `configure`, `doctor`):
...
Las compilaciones actuales ya no incluyen el bridge TCP. Los nodes se conectan mediante Gateway WebSocket. Las claves `bridge.*` ya no forman parte del esquema de configuración (la validación falla hasta que se eliminen; `openclaw doctor --fix` puede quitar claves desconocidas).
...
- `sessionRetention`: cuánto tiempo se conservan las sesiones completadas de ejecuciones aisladas de Cron antes de podarlas de `sessions.json`. También controla la limpieza de transcripciones archivadas de Cron eliminadas. Predeterminado: `24h`; establezca `false` para desactivarlo.
...
- `webhook`: fallback heredado obsoleto de URL de Webhook (http/https) usado solo para trabajos almacenados que aún tienen `notify: true`.
...
Se aplica solo a trabajos de Cron de una sola ejecución. Los trabajos recurrentes usan un manejo de fallos independiente.
...
- `mode`: `"announce"` o `"webhook"`; usa `"announce"` por defecto cuando existe suficiente información de destino.
...
- Cuando no se define ni destino global ni por trabajo, los trabajos que ya entregan mediante `announce` usan como respaldo ese destino announce principal en caso de fallo.
...
- Array de archivos: se combinan en profundidad en orden (los posteriores sobrescriben a los anteriores).
...
- Rutas: se resuelven en relación con el archivo que incluye, pero deben permanecer dentro del directorio de configuración de nivel superior (`dirname` de `openclaw.json`). Se permiten formas absolutas/`../` solo cuando aún resuelven dentro de ese límite.
