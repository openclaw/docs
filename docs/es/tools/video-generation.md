...
summary: Generar videos a partir de texto, imágenes o videos existentes usando 14 backends de proveedores
title: Generación de video
x-i18n:
    generated_at: "2026-04-24T05:56:38Z"
    model: gpt-5.4
    provider: openai
...
Los agentes de OpenClaw pueden generar videos a partir de prompts de texto, imágenes de referencia o videos existentes. Se admiten catorce backends de proveedores, cada uno con distintas opciones de modelos, modos de entrada y conjuntos de funciones. El agente elige automáticamente el proveedor adecuado según tu configuración y las claves de API disponibles.

<Note>
La herramienta `video_generate` solo aparece cuando hay al menos un proveedor de generación de video disponible. Si no la ves en las herramientas de tu agente, establece una clave de API del proveedor o configura `agents.defaults.videoGenerationModel`.
</Note>
...
Fuera de las ejecuciones de agentes respaldadas por sesión (por ejemplo, invocaciones directas de herramientas), la herramienta recurre a la generación inline y devuelve la ruta final del medio en el mismo turno.
...
Algunos proveedores aceptan variables de entorno de clave de API adicionales o alternativas. Consulta las [páginas de proveedores](#related) individuales para ver más detalles.
...
y la batería compartida en vivo.

| Proveedor | `generate` | `imageToVideo` | `videoToVideo` | Líneas compartidas en vivo actualmente                                                                                                    |
| --------- | ---------- | -------------- | -------------- | ------------------------------------------------------------------------------------------------------------------------------------------ |
...
| ComfyUI   | Sí         | Sí             | No             | No está en la batería compartida; la cobertura específica del workflow vive con las pruebas de Comfy                                      |
| fal       | Sí         | Sí             | No             | `generate`, `imageToVideo`                                                                                                                 |
| Google    | Sí         | Sí             | Sí             | `generate`, `imageToVideo`; el `videoToVideo` compartido se omite porque la batería actual de Gemini/Veo respaldada por búfer no acepta esa entrada |
| MiniMax   | Sí         | Sí             | No             | `generate`, `imageToVideo`                                                                                                                 |
| OpenAI    | Sí         | Sí             | Sí             | `generate`, `imageToVideo`; el `videoToVideo` compartido se omite porque esta ruta de organización/entrada actualmente necesita acceso del proveedor a inpaint/remix |
...
| Vydra     | Sí         | Sí             | No             | `generate`; el `imageToVideo` compartido se omite porque `veo3` incluido es solo de texto y `kling` incluido requiere una URL de imagen remota |
...
| --------- | ------ | ------------------------------------------------------------------------------ |
| `prompt`  | string | Descripción de texto del video a generar (obligatoria para `action: "generate"`) |
...
Las sugerencias de rol se reenvían al proveedor tal como están. Los valores canónicos provienen de
la unión `VideoGenerationAssetRole`, pero los proveedores pueden aceptar
cadenas de rol adicionales. Los arrays `*Roles` no deben tener más entradas que la
lista de referencias correspondiente; los errores de desfase se rechazan con un error claro.
...
| `audio`           | boolean | Habilita audio generado en la salida cuando sea compatible. Distinto de `audioRef*` (entradas) |
| `watermark`       | boolean | Activa o desactiva la marca de agua del proveedor cuando sea compatible                  |
...
| `timeoutMs`       | number | Timeout opcional de la solicitud al proveedor en milisegundos                                                                                                                                                                                                                                                                                          |
| `providerOptions` | object | Opciones específicas del proveedor como objeto JSON (por ejemplo `{"seed": 42, "draft": true}`). Los proveedores que declaran un esquema tipado validan las claves y los tipos; las claves desconocidas o incompatibles hacen que se omita el candidato durante el fallback. Los proveedores sin esquema declarado reciben las opciones tal cual. Ejecuta `video_generate action=list` para ver qué acepta cada proveedor |

No todos los proveedores admiten todos los parámetros. OpenClaw ya normaliza la duración al valor compatible más cercano del proveedor y también reasigna sugerencias de geometría traducidas, como tamaño a proporción de aspecto, cuando un proveedor de fallback expone una superficie de control distinta. Las sobrescrituras realmente no compatibles se ignoran en la medida de lo posible y se informan como advertencias en el resultado de la herramienta. Los límites estrictos de capacidad (como demasiadas entradas de referencia) fallan antes del envío.
...
Las referencias mixtas de imágenes y videos no constituyen una superficie de capacidad compartida estable.
...
límite de la herramienta, de modo que una solicitud que excede los límites del proveedor principal
...
- Si la solicitud contiene `providerOptions` y el candidato activo
  ...
  opciones tal cual (paso compatible hacia atrás). Un proveedor puede
  optar explícitamente por no admitir ninguna provider option declarando un esquema vacío
  (`capabilities.providerOptions: {}`), lo que provoca la misma omisión que una
  incompatibilidad de tipos.

La primera razón de omisión en una solicitud se registra con nivel `warn` para que los operadores vean
...
<AccordionGroup>
  <Accordion title="Alibaba">
    Usa el endpoint asíncrono de DashScope / Model Studio. Las imágenes y videos de referencia deben ser URLs remotas `http(s)`.
  </Accordion>
  ...
  <Accordion title="BytePlus (1.0)">
    ...
    Los modelos T2V (`*-t2v-*`) no aceptan entradas de imagen; los modelos I2V y los modelos generales `*-pro-*` admiten una única imagen de referencia (primer fotograma). Pasa la imagen por posición o establece `role: "first_frame"`. Los IDs de modelo T2V cambian automáticamente a la variante I2V correspondiente cuando se proporciona una imagen.
  ...
  <Accordion title="BytePlus Seedance 1.5">
    ...
    `aspectRatio: "adaptive"` detecta automáticamente la proporción a partir de la imagen de entrada. `audio: true` se asigna a `generate_audio`. `providerOptions.seed` (number) se reenvía.
  ...
  <Accordion title="BytePlus Seedance 2.0">
    ...
    Usa la API unificada `content[]`. Admite hasta 9 imágenes de referencia, 3 videos de referencia y 3 audios de referencia. Todas las entradas deben ser URLs remotas `https://`. Establece `role` en cada recurso; valores compatibles: `"first_frame"`, `"last_frame"`, `"reference_image"`, `"reference_video"`, `"reference_audio"`.
  ...
  <Accordion title="ComfyUI">
    Ejecución local o en la nube basada en workflow. Admite texto a video e imagen a video mediante el grafo configurado.
  </Accordion>

  <Accordion title="fal">
    Usa un flujo respaldado por cola para trabajos de larga duración. Solo admite una imagen de referencia.
  </Accordion>
  ...
  <Accordion title="MiniMax">
    Solo admite una imagen de referencia.
  </Accordion>

  <Accordion title="OpenAI">
    Solo se reenvía la sobrescritura `size`. Las demás sobrescrituras de estilo (`aspectRatio`, `resolution`, `audio`, `watermark`) se ignoran con una advertencia.
  </Accordion>

  <Accordion title="Qwen">
    Usa el mismo backend DashScope que Alibaba. Las entradas de referencia deben ser URLs remotas `http(s)`; los archivos locales se rechazan desde el principio.
  </Accordion>

  <Accordion title="Runway">
    Admite archivos locales mediante data URIs. `videoToVideo` requiere `runway/gen4_aleph`. Las ejecuciones solo de texto exponen las proporciones `16:9` y `9:16`.
  </Accordion>
  ...
  <Accordion title="xAI">
    Admite flujos de texto a video, imagen a video y edición/extensión de video remoto.
  </Accordion>
</AccordionGroup>

## Modos de capacidad del proveedor

El contrato compartido de generación de video ahora permite que los proveedores declaren capacidades
específicas por modo en lugar de solo límites agregados planos. Las nuevas implementaciones de
proveedores deberían preferir bloques de modo explícitos:
...
Cobertura en vivo opt-in para los proveedores compartidos incluidos:
...
Este archivo en vivo carga variables de entorno de proveedor faltantes desde `~/.profile`, da prioridad
a las claves de API en vivo/del entorno por encima de los perfiles de autenticación almacenados por defecto,
y ejecuta por defecto una prueba rápida segura para lanzamientos:

- `generate` para cada proveedor no FAL de la batería
...
FAL es opt-in porque la latencia de cola del lado del proveedor puede dominar el tiempo de lanzamiento:
...
que la batería compartida puede probar con seguridad usando medios locales:
...
- `videoToVideo` cuando `capabilities.videoToVideo.enabled` y el proveedor/modelo
  acepta entrada de video local respaldada por búfer en la batería compartida

Hoy, la línea en vivo compartida de `videoToVideo` cubre:
