...
title: Texto a voz
x-i18n:
    generated_at: "2026-07-19T02:09:21Z"
    model: gpt-5.6
...
TTS es la mitad de salida de voz del modo `stt-tts` de Talk (`talk.speak` usa esta
...
<Steps>
  <Step title="Elegir un proveedor">
    ...
    la CLI local funcionan sin una clave de API. Consulte la [matriz de proveedores](#supported-providers)
...
<Note>
TTS automático está **desactivado** de forma predeterminada. Cuando `messages.tts.provider` no está configurado,
OpenClaw elige el primer proveedor configurado según el orden de selección automática del registro.
La herramienta integrada del agente `tts` solo se activa mediante una intención explícita: el chat normal permanece
en texto, salvo que el usuario solicite audio, use `/tts` o habilite TTS automático/voz
mediante directivas.
</Note>
...
| **Google Gemini** | `GEMINI_API_KEY` o `GOOGLE_API_KEY`                                                                          | TTS por lotes de la API de Gemini; adaptado a la personalidad mediante `promptTemplate: "audio-profile-v1"`. |
...
| **Xiaomi MiMo**   | `XIAOMI_API_KEY`                                                                                                | TTS de MiMo mediante las finalizaciones de chat de Xiaomi.                                  |

Si se configuran varios proveedores, se utiliza primero el seleccionado y los
demás quedan como opciones alternativas. El resumen automático utiliza `summaryModel` (o
...
SLA ni una cuota publicados; considérelo un servicio sin garantías. El id de proveedor heredado `edge` se
...
que se muestran a continuación son canónicos; los nombres de campo `voice`/`voiceId`/
...
Use `agents.list[].tts` cuando un agente deba hablar con un proveedor, una voz,
un modelo, una persona o un modo de TTS automático diferentes. El bloque del agente se combina
en profundidad sobre `messages.tts`, por lo que las credenciales del proveedor pueden permanecer en la configuración global del proveedor:
...
Para fijar una persona por agente, establezca `agents.list[].tts.persona` junto con la configuración
del proveedor; esta sustituye el valor global de `messages.tts.persona` únicamente para ese agente.
...
4. sustitución de la cuenta, cuando el canal pasa `channels.<channel>.accounts.<id>.tts`
5. preferencias locales de `/tts` para este host
6. directivas `[[tts:...]]` en línea cuando están habilitadas las [sustituciones controladas por el modelo](#model-driven-directives)

Las sustituciones de canal y cuenta usan la misma estructura que `messages.tts` y
se combinan en profundidad sobre las capas anteriores, por lo que las credenciales compartidas del proveedor pueden permanecer en
`messages.tts` mientras un canal o una cuenta de bot cambia únicamente la voz del hablante, el modelo, la persona
...
prompt, semillas y ajustes de voz.
  ...
  messages: {
    tts: {
      ...
      personas: {
        alfred: {
          label: "Alfred",
          description: "Narrador mayordomo británico, seco y cordial.",
          provider: "google",
          fallbackPolicy: "preserve-persona",
          prompt: {
            profile: "Un brillante mayordomo británico. Seco, ingenioso, cordial, encantador, emocionalmente expresivo, nunca genérico.",
            scene: "Un estudio tranquilo a altas horas de la noche. Narración con micrófono cercano para un operador de confianza.",
            sampleContext: "El hablante responde a una solicitud técnica privada con confianza concisa y una cordialidad seca.",
            style: "Refinado, discreto, ligeramente divertido.",
            accent: "Inglés británico.",
            pacing: "Pausado, con breves pausas dramáticas.",
            constraints: ["No leer en voz alta los valores de configuración.", "No explicar la persona."],
          },
          providers: {
            google: {
              model: "gemini-3.1-flash-tts-preview",
...
En cada intento con un proveedor, OpenClaw combina las configuraciones en este orden:
...
<AccordionGroup>
  <Accordion title="Google Gemini">
    Envuelve los campos del prompt de persona en una estructura de prompt de TTS de Gemini **solo cuando**
    la configuración efectiva del proveedor de Google establece `promptTemplate: "audio-profile-v1"`
    o `personaPrompt`. Los campos anteriores `audioProfile` y `speakerName`
  ...
  <Accordion title="OpenAI">
    ...
    no se ha configurado ningún `instructions` explícito de OpenAI. El `instructions` explícito
    siempre prevalece.
  </Accordion>
  <Accordion title="Otros proveedores">
    Usan únicamente las vinculaciones de persona específicas del proveedor en
    `personas.<id>.providers.<provider>`. Los campos del prompt de persona se ignoran
    salvo que el proveedor implemente su propia asignación de prompts de persona.
  </Accordion>
</AccordionGroup>
...
proveedor con el que se intenta operar:
...
| `provider-defaults` | La persona se omite de la preparación del prompt para ese intento; el proveedor usa sus valores predeterminados neutros mientras continúa la reserva con otros proveedores.                 |
| `fail`              | Omite ese intento con el proveedor mediante `reasonCode: "not_configured"` y `personaBinding: "missing"`. Se siguen probando los proveedores de reserva.                                    |

La solicitud TTS completa solo falla cuando **todos** los proveedores con los que se intenta operar se omiten
o fallan.

La selección del proveedor de una sesión de Talk se limita a la sesión. Un cliente de Talk debe elegir
los identificadores de proveedor, modelo y voz, así como las configuraciones regionales, desde `talk.catalog` y pasarlos
mediante la solicitud de sesión o transferencia de Talk. Abrir una sesión de voz no debe
modificar `messages.tts` ni los valores globales predeterminados del proveedor de Talk.
...
`[[tts:text]]...[[/tts:text]]` opcional para indicaciones expresivas que deben aparecer
únicamente en el audio:
...
respuesta declara `provider=...`, las demás claves de esa directiva se analizan
únicamente mediante ese proveedor; las claves no admitidas se eliminan y se notifican como advertencias de
directivas TTS.
...
- `/tts latest` lee la respuesta más reciente del asistente en la transcripción de la sesión actual y la envía una vez como audio. Almacena únicamente un hash de esa respuesta en la entrada de la sesión para impedir envíos de voz duplicados.
...
- `/status` muestra el modo TTS activo, además del proveedor, el modelo y la voz configurados, y los metadatos saneados del punto de conexión personalizado cuando TTS está habilitado.

## Preferencias por usuario

Los comandos de barra diagonal escriben las sustituciones locales en `prefsPath`. El valor predeterminado es
`~/.openclaw/settings/tts.json`; se puede sustituir mediante la variable de entorno `OPENCLAW_TTS_PREFS`
...
| `summarize`  | Activación del resumen (valor predeterminado: `true`)                                                  |
...
La entrega de voz TTS depende de las capacidades del canal. Los plugins de canal indican
si el TTS de estilo de voz debe solicitar a los proveedores un destino `voice-note` nativo o
...
- **Transcodificación de Feishu / WhatsApp:** cuando una respuesta de nota de voz llega como MP3/WebM/WAV/M4A u otro archivo de audio probable, el plugin de canal la transcodifica a Ogg/Opus de 48 kHz con `ffmpeg` (`libopus`, 64 kbps) antes de enviar el mensaje de voz nativo. WhatsApp envía el resultado mediante la carga útil `audio` de Baileys con `ptt: true` y `audio/ogg; codecs=opus`. Si falla la transcodificación: Feishu captura el error y recurre al envío del archivo original como un archivo adjunto normal; WhatsApp no tiene alternativa, por lo que el propio envío falla en lugar de publicar una carga útil PTT incompatible.
- **MiniMax:** MP3 (modelo `speech-2.8-hd`, frecuencia de muestreo de 32 kHz) para archivos adjuntos de audio normales; se transcodifica a Opus de 48 kHz con `ffmpeg` para los destinos de notas de voz anunciados por el canal.
- **Xiaomi MiMo:** MP3 de forma predeterminada, o WAV cuando está configurado; se transcodifica a Opus de 48 kHz con `ffmpeg` para los destinos de notas de voz anunciados por el canal.
- **CLI local:** usa el `outputFormat` configurado. Los destinos de notas de voz se convierten a Ogg/Opus y la salida de telefonía se convierte a PCM mono sin procesar de 16 kHz con `ffmpeg`.
- **Google Gemini:** devuelve PCM sin procesar de 24 kHz. OpenClaw lo encapsula como WAV para los archivos adjuntos de audio, lo transcodifica a Opus de 48 kHz para los destinos de notas de voz y devuelve PCM directamente para conversación/telefonía.
...
- **xAI:** MP3 de forma predeterminada; la síntesis de archivos de audio puede usar `mp3`, `wav`, `pcm`, `mulaw` o `alaw` tanto para la salida almacenada en búfer como para la salida en streaming. Los destinos de notas de voz usan MP3 para el streaming y la alternativa almacenada en búfer porque las salidas `pcm`, `mulaw` y `alaw` de xAI son audio sin procesar y sin encabezado. La síntesis almacenada en búfer usa el endpoint REST por lotes `/v1/tts` de xAI; `textToSpeechStream` usa `wss://api.x.ai/v1/tts` nativo. Este no es el contrato de voz en tiempo real. No se admite el formato de notas de voz Opus nativo.
- **Microsoft:** usa `microsoft.outputFormat` (valor predeterminado: `audio-24khz-48kbitrate-mono-mp3`).
  ...
  - El `sendVoice` de Telegram acepta OGG/MP3/M4A; use OpenAI/ElevenLabs si necesita mensajes de voz Opus garantizados.
  - Si falla el formato de salida de Microsoft configurado, OpenClaw vuelve a intentarlo con MP3.
  - Cuando no se establece una anulación explícita de la voz y se usa la voz inglesa predeterminada, OpenClaw cambia automáticamente a una voz neuronal china (`zh-CN-XiaoxiaoNeural`, configuración regional `zh-CN`) si el texto de la respuesta contiene predominantemente caracteres CJK.
...
- Omite el TTS si la respuesta ya contiene contenido multimedia estructurado.
...
- En `mode: "final"`, sigue enviando TTS solo de audio para las respuestas finales transmitidas
  ...
  normalización de contenido multimedia del canal que los archivos adjuntos de respuestas normales.
...
<AccordionGroup>
  <Accordion title="messages.tts.* de nivel superior">
    ...
    <ParamField path="enabled" type="boolean" deprecated>
      Activación heredada. `openclaw doctor --fix` la migra a `auto`.
    </ParamField>
    <ParamField path="mode" type='"final" | "all"' default="final">
      `"all"` incluye las respuestas de herramientas/bloques además de las respuestas finales.
    </ParamField>
    <ParamField path="provider" type="string">
      Id. del proveedor de voz. Si no se establece, OpenClaw usa el primer proveedor configurado según el orden de selección automática del registro. `provider: "edge"` heredado se reescribe como `"microsoft"` mediante `openclaw doctor --fix`.
    </ParamField>
    <ParamField path="persona" type="string">
      Id. de la persona activa de `personas`. Se normaliza a minúsculas.
    </ParamField>
    <ParamField path="personas.<id>" type="object">
      Identidad oral estable. Campos: `label`, `description`, `provider`, `fallbackPolicy`, `prompt`, `providers.<provider>`. Consulte [Personas](#personas).
    </ParamField>
    ...
    <ParamField path="providers.<id>" type="object">
      Configuración propiedad del proveedor, indexada por el id. del proveedor de voz. `openclaw doctor --fix` reescribe los bloques directos heredados (`messages.tts.openai`, `.elevenlabs`, `.microsoft`, `.edge`); confirme únicamente `messages.tts.providers.<id>`.
    </ParamField>
    <ParamField path="maxTextLength" type="number" default="4096">
      Límite estricto de caracteres para la entrada de TTS. `/tts audio`, `tts.convert` y `tts.speak` fallan si se supera.
    </ParamField>
    <ParamField path="timeoutMs" type="number" default="30000">
      Tiempo de espera de la solicitud en milisegundos. Un `timeoutMs` por llamada (herramienta del agente, Gateway) tiene prioridad cuando se establece; de lo contrario, un `messages.tts.timeoutMs` configurado explícitamente tiene prioridad sobre cualquier valor predeterminado del proveedor definido por el plugin.
    </ParamField>
...
una SecretRef de TTS no disponible marca la capacidad de TTS integrada
como configurada pero no disponible, en lugar de detener el Gateway. A continuación, `tts.speak` devuelve
`UNAVAILABLE` con el motivo `SECRET_SURFACE_UNAVAILABLE` y no se envía ninguna solicitud al
proveedor. El estado y doctor muestran el propietario de TTS degradado y sus rutas de configuración. Las
referencias explícitas permanecen en la instantánea del entorno de ejecución, por lo que las credenciales
del entorno o del perfil no pueden seleccionar silenciosamente una cuenta diferente. Las recargas y la comprobación previa
de escritura de la configuración aplican la política de degradación que tiene en cuenta al propietario: un propietario de TTS apto
que no haya cambiado puede conservar como obsoletas sus últimas credenciales válidas conocidas, mientras que un fallo
nuevo o modificado pasa a estar en frío sin bloquear a los propietarios en buen estado. Las referencias
estructuralmente no válidas y los valores resueltos siguen provocando un fallo de arranque o el rechazo de la actualización.
  ...
  <Accordion title="ElevenLabs">
    ...
    <ParamField path="model" type="string">Id. del modelo. Valor predeterminado: `eleven_multilingual_v2`. Los identificadores heredados `eleven_turbo_v2_5`/`eleven_turbo_v2` se normalizan al modelo `flash` correspondiente.</ParamField>
    ...
    <ParamField path="baseUrl" type="string">Reemplaza la URL base de la API de ElevenLabs.</ParamField>
  </Accordion>
  ...
  <Accordion title="Google Gemini">
    ...
    <ParamField path="promptTemplate" type='"audio-profile-v1"'>Establézcalo en `audio-profile-v1` para envolver los campos de instrucción de la persona activa en una estructura determinista de instrucciones TTS de Gemini.</ParamField>
    <ParamField path="personaPrompt" type="string">Texto adicional de instrucciones de persona específico de Google que se añade a las notas del director de la plantilla.</ParamField>
  ...
  <Accordion title="CLI local (tts-local-cli)">
    ...
    <ParamField path="env" type="Record<string, string>">Reemplazos opcionales del entorno para el comando.</ParamField>

    La salida estándar del comando y el audio generado o convertido están limitados a 50 MiB. La salida de error de diagnóstico está limitada a 1 MiB. OpenClaw finaliza el comando y hace que la síntesis falle cuando se supera cualquiera de los límites.

  </Accordion>

  <Accordion title="Microsoft (sin clave de API)">
    <ParamField path="enabled" type="boolean" default="true">Permite el uso de la síntesis de voz de Microsoft.</ParamField>
    ...
    <ParamField path="timeoutMs" type="number">Reemplazo del tiempo de espera de las solicitudes (ms).</ParamField>
  ...
  <Accordion title="OpenAI">
    ...
    <ParamField path="instructions" type="string">Campo `instructions` explícito de OpenAI. Cuando se establece, los campos de instrucciones de persona **no** se asignan automáticamente.</ParamField>
    <ParamField path="extraBody / extra_body" type="Record<string, unknown>">Campos JSON adicionales que se combinan con los cuerpos de las solicitudes `/audio/speech` después de los campos TTS de OpenAI generados. Utilice esta opción para endpoints compatibles con OpenAI, como Kokoro, que requieren claves específicas del proveedor, como `lang`; se ignoran las claves de prototipo no seguras.</ParamField>
    <ParamField path="baseUrl" type="string">
      Reemplaza el endpoint TTS de OpenAI. Orden de resolución: configuración → `OPENAI_TTS_BASE_URL` → `https://api.openai.com/v1`. Los valores no predeterminados se tratan como endpoints TTS compatibles con OpenAI, por lo que se aceptan nombres personalizados de modelos y voces, y `speed` pierde su comprobación del intervalo `0.25..4.0`.
    </ParamField>
  </Accordion>
  ...
  <Accordion title="OpenRouter">
    ...
    <ParamField path="speed" type="number">Reemplazo de velocidad nativo del proveedor.</ParamField>
  </Accordion>

  <Accordion title="Volcengine (BytePlus Seed Speech)">
    <ParamField path="apiKey" type="string">Entorno: `VOLCENGINE_TTS_API_KEY` o `BYTEPLUS_SEED_SPEECH_API_KEY`.</ParamField>
    <ParamField path="resourceId" type="string">Valor predeterminado: `seed-tts-1.0`. Entorno: `VOLCENGINE_TTS_RESOURCE_ID`. Utilice `seed-tts-2.0` cuando el proyecto tenga derecho de uso de TTS 2.0.</ParamField>
    <ParamField path="appKey" type="string">Encabezado de clave de la aplicación. Valor predeterminado: `aGjiRDfUWi`. Entorno: `VOLCENGINE_TTS_APP_KEY`.</ParamField>
    <ParamField path="baseUrl" type="string">Reemplaza el endpoint HTTP de TTS de Seed Speech. Entorno: `VOLCENGINE_TTS_BASE_URL`.</ParamField>
  ...
  <Accordion title="xAI">
    ...
    <ParamField path="speakerVoiceId" type="string">Valor predeterminado: `eve`. Con autenticación, `openclaw infer tts voices --provider xai` obtiene el catálogo integrado actual; sin autenticación, enumera las alternativas sin conexión `ara`, `eve`, `leo`, `rex` y `sal`. Los identificadores de voces personalizadas de la cuenta se reenvían aunque no estén presentes en la lista integrada. Alias heredado: `voiceId`.</ParamField>
    ...
    <ParamField path="speed" type="number">Reemplazo de velocidad nativo del proveedor, `0.7..1.5`.</ParamField>
  </Accordion>
  ...
  <Accordion title="Xiaomi MiMo">
    ...
    <ParamField path="speakerVoice" type="string">Valor predeterminado: `mimo_default` para los modelos de voces predefinidas. Variable de entorno: `XIAOMI_TTS_VOICE`. Alias heredado: `voice`. No se envía para `mimo-v2.5-tts-voicedesign`.</ParamField>
    <ParamField path="format" type='"mp3" | "wav"'>Valor predeterminado: `mp3`. Variable de entorno: `XIAOMI_TTS_FORMAT`.</ParamField>
    <ParamField path="style" type="string">Instrucción opcional de estilo en lenguaje natural que se envía como mensaje del usuario; no se pronuncia. Para `mimo-v2.5-tts-voicedesign`, es el prompt de diseño de voz; OpenClaw proporciona un valor predeterminado si se omite.</ParamField>
  </Accordion>
</AccordionGroup>
...
enviarlo en la respuesta. En Feishu, Matrix, Telegram y WhatsApp, el audio se
envía como mensaje de voz en lugar de como archivo adjunto. Feishu y
WhatsApp pueden transcodificar en esta ruta la salida TTS que no esté en formato Opus cuando
...
tiempo de espera por llamada para la solicitud al proveedor, en milisegundos. Los valores por llamada prevalecen sobre
`messages.tts.timeoutMs`; los tiempos de espera de TTS configurados prevalecen sobre cualquier valor
predeterminado del proveedor definido por un plugin.
