...
summary: Genera y edita imágenes mediante image_generate en OpenAI, Google, fal, MiniMax, ComfyUI, DeepInfra, OpenRouter, LiteLLM, xAI y Vydra
title: Generación de imágenes
x-i18n:
    generated_at: "2026-04-30T06:04:33Z"
    model: gpt-5.5
    provider: openai
...
configura `agents.defaults.imageGenerationModel`, configura una clave de API de proveedor
...
<Steps>
  <Step title="Configure auth">
    Configura una clave de API para al menos un proveedor (por ejemplo `OPENAI_API_KEY`,
  ...
  <Step title="Pick a default model (optional)">
    ...
    imágenes a través de ese perfil OAuth en lugar de probar primero
    `OPENAI_API_KEY`. La configuración explícita `models.providers.openai` (clave de API,
    URL base personalizada/Azure) vuelve a optar por la ruta directa de la API de
    imágenes de OpenAI.
...
La misma herramienta `image_generate` gestiona texto a imagen y edición con imagen de
referencia. Usa `image` para una referencia o `images` para varias referencias.
Las pistas de salida admitidas por el proveedor, como `quality`, `outputFormat` y
`background`, se reenvían cuando están disponibles y se informan como ignoradas cuando un
proveedor no las admite. La compatibilidad incluida con fondo transparente es
específica de OpenAI; otros proveedores aún pueden preservar el alfa de PNG si su
backend lo emite.
...
| --------------------- | ------------------ | --------- | ----------------- | -------------- | --------------------- | -------------- | ----- | -------------- |
| Generar (conteo máximo) | Definido por workflow | 4         | 4                 | 4              | 9                     | 4              | 1     | 4              |
...
| Relación de aspecto   | —                  | —         | ✓ (solo generación) | ✓              | ✓                     | —              | —     | ✓              |
...
<ParamField path="size" type="string">
  Pista de tamaño: `1024x1024`, `1536x1024`, `1024x1536`, `2048x2048`, `3840x2160`.
</ParamField>
<ParamField path="aspectRatio" type="string">
  Relación de aspecto: `1:1`, `2:3`, `3:2`, `3:4`, `4:3`, `4:5`, `5:4`, `9:16`, `16:9`, `21:9`.
</ParamField>
<ParamField path="resolution" type='"1K" | "2K" | "4K"'>Pista de resolución.</ParamField>
<ParamField path="quality" type='"low" | "medium" | "high" | "auto"'>
  Pista de calidad cuando el proveedor la admite.
</ParamField>
<ParamField path="outputFormat" type='"png" | "jpeg" | "webp"'>
  Pista de formato de salida cuando el proveedor la admite.
</ParamField>
<ParamField path="background" type='"transparent" | "opaque" | "auto"'>
  Pista de fondo cuando el proveedor la admite. Usa `transparent` con
  `outputFormat: "png"` o `"webp"` para proveedores compatibles con transparencia.
</ParamField>
<ParamField path="count" type="number">Número de imágenes para generar (1–4).</ParamField>
<ParamField path="timeoutMs" type="number">Tiempo de espera opcional de solicitud al proveedor en milisegundos.</ParamField>
<ParamField path="filename" type="string">Pista de nombre de archivo de salida.</ParamField>
<ParamField path="openai" type="object">
  Pistas exclusivas de OpenAI: `background`, `moderation`, `outputCompression` y `user`.
</ParamField>

<Note>
No todos los proveedores admiten todos los parámetros. Cuando un proveedor de reserva admite una
opción de geometría cercana en lugar de la solicitada exacta, OpenClaw reasigna al
tamaño, relación de aspecto o resolución compatible más cercano antes del envío.
Las pistas de salida no admitidas se descartan para los proveedores que no declaran
compatibilidad y se informan en el resultado de la herramienta. Los resultados de la herramienta informan la
configuración aplicada; `details.normalization` captura cualquier
...
Si un proveedor falla (error de autenticación, límite de tasa, etc.), se prueba automáticamente
...
<AccordionGroup>
  <Accordion title="Per-call model overrides are exact">
    Una anulación de `model` por llamada prueba solo ese proveedor/modelo y no
    continúa con el proveedor principal/reserva configurado ni con proveedores detectados automáticamente.
  </Accordion>
  <Accordion title="Auto-detection is auth-aware">
    ...
    autenticar realmente ese proveedor. Configura
  ...
  <Accordion title="Timeouts">
    Configura `agents.defaults.imageGenerationModel.timeoutMs` para backends de imágenes
  ...
  <Accordion title="Inspect at runtime">
    ...
    sus modelos predeterminados y pistas de variables de entorno de autenticación.
  </Accordion>
</AccordionGroup>
...
<AccordionGroup>
  <Accordion title="OpenAI gpt-image-2 (y gpt-image-1.5)">
    ...
    **no** recurre silenciosamente a `OPENAI_API_KEY` para esa solicitud;
    para forzar el enrutamiento directo a la API OpenAI Images, configura
    ...
    edición con imagen de referencia mediante la misma herramienta `image_generate`.
    ...
    esos valores a un `size` compatible; de lo contrario, la herramienta los informa como
  ...
  <Accordion title="Modelos de imagen de OpenRouter">
    ...
    se enruta mediante la API de imágenes de completions de chat de OpenRouter. Selecciona
    ...
    Los accesos directos actuales integrados de modelos de imagen de OpenRouter incluyen
    `google/gemini-3.1-flash-image-preview`,
  ...
  <Accordion title="xAI grok-imagine-image">
    ...
    - Resoluciones: `1K`, `2K`
    - Salidas: se devuelven como adjuntos de imagen administrados por OpenClaw

    OpenClaw no expone intencionalmente `quality`, `mask`,
    `user` ni relaciones de aspecto adicionales nativas de xAI hasta que esos controles existan
...
`openclaw infer image edit`; `--openai-background` se mantiene como un alias
específico de OpenAI. Los proveedores incluidos distintos de OpenAI no declaran
hoy control explícito de fondo, por lo que `background: "transparent"` se informa
