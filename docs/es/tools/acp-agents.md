---
read_when:
    - Ejecutar entornos de codificación mediante ACP
    ...
    - Resolución de problemas del backend ACP, el cableado del Plugin o la entrega de finalizaciones
    - Ejecutar comandos /acp desde el chat
sidebarTitle: ACP agents
summary: Ejecuta arneses de codificación externos (Claude Code, Cursor, Gemini CLI, ACP explícito de Codex, OpenClaw ACP, OpenCode) mediante el backend de ACP
title: Agentes ACP
x-i18n:
    generated_at: "2026-05-02T05:36:56Z"
    model: gpt-5.5
    provider: openai
...
[Agent Client Protocol (ACP)](https://agentclientprotocol.com/) sesiones
...
runtime integrado `agentRuntime.id: "codex"`; ACP posee los
controles `/acp ...` y las sesiones `sessions_spawn({ runtime: "acp" })`.
...
Normalmente sí. Las instalaciones nuevas incluyen el Plugin de runtime `acpx` empaquetado y habilitado
de forma predeterminada, con un binario `acpx` fijado local al Plugin que OpenClaw sondea
y repara automáticamente en cuanto el listener HTTP de Gateway está activo. Ejecuta
`/acp doctor` para una comprobación de disponibilidad.
...
actual no debe estar bloqueada por sandbox y debe haberse cargado un backend de runtime.
...
<AccordionGroup>
  <Accordion title="Puntos problemáticos del primer uso">
    - Si `plugins.allow` está configurado, es un inventario restrictivo de plugins y **debe** incluir `acpx`; de lo contrario, el valor predeterminado empaquetado se bloquea intencionadamente y `/acp doctor` informa de la entrada faltante en la lista de permitidos.
    - El adaptador Codex ACP empaquetado se prepara con el Plugin `acpx` y se inicia localmente cuando es posible.
    - Otros adaptadores de arnés de destino todavía pueden descargarse bajo demanda con `npx` la primera vez que los uses.
    - La autenticación del proveedor aún debe existir en el host para ese arnés.
    - Si el host no tiene npm o acceso de red, las descargas de adaptadores del primer uso fallan hasta que las cachés se precalientan o el adaptador se instala de otra forma.

  </Accordion>
  <Accordion title="Prerrequisitos de runtime">
    ...
    - El modo de permisos coincide con el trabajo. Las sesiones no interactivas no pueden hacer clic en solicitudes de permisos nativas, por lo que las ejecuciones de codificación con mucha escritura/exec normalmente necesitan un perfil de permisos ACPX que pueda avanzar sin interacción.
...
Con el backend `acpx` empaquetado, usa estos ids de arnés como destinos de `/acp spawn <id>`
...
| `cursor`   | ACP de Cursor CLI (`cursor-agent acp`)            | Sobrescribe el comando acpx si una instalación local expone un punto de entrada ACP distinto.    |
...
| `pi`       | Runtime Pi/OpenClaw integrado                   | Usado para experimentos de arnés nativos de OpenClaw.                                       |
...
<AccordionGroup>
  <Accordion title="Detalles del ciclo de vida">
    ...
    - El mantenimiento de tareas cierra sesiones ACP one-shot terminales o huérfanas pertenecientes al padre. Las sesiones ACP persistentes se preservan mientras permanezca una vinculación de conversación activa; las sesiones persistentes obsoletas sin una vinculación activa se cierran para que no puedan reanudarse silenciosamente después de que la tarea propietaria termine o su registro de tarea desaparezca.
    - Los mensajes de seguimiento vinculados van directamente a la sesión ACP hasta que la vinculación se cierra, pierde el foco, se restablece o expira.
  ...
  <Accordion title="Reglas de enrutamiento nativo de Codex">
    ...
    Para eventos de herramientas nativas de Codex, OpenClaw inyecta un relevo de hooks nativo
    ...
    mediante aprobaciones de OpenClaw. Los hooks `Stop` de Codex se reenvían a
    `before_agent_finalize` de OpenClaw, donde los plugins pueden solicitar una pasada adicional
    del modelo antes de que Codex finalice su respuesta. El relevo permanece
    ...
    cuando quieras el modelo de runtime/sesión de ACP. El límite de compatibilidad integrado de Codex
    está documentado en el
    [contrato de compatibilidad v1 del arnés Codex](/es/plugins/codex-harness#v1-support-contract).

  </Accordion>
  <Accordion title="Model / provider / runtime selection cheat sheet">
    - `openai-codex/*` — ruta de OAuth/suscripción de PI Codex.
    - `openai/*` más `agentRuntime.id: "codex"` — runtime integrado nativo del servidor de aplicación de Codex.
  ...
  <Accordion title="ACP-routing natural-language triggers">
    Desencadenadores que deben enrutar al runtime ACP:
    ...
    OpenClaw elige `runtime: "acp"`, resuelve el arnés `agentId`,
    se enlaza con la conversación o el hilo actual cuando está disponible, y
    enruta los seguimientos a esa sesión hasta el cierre o vencimiento. Codex solo
    ...
    se configura con `runtime.type="acp"`, OpenClaw usa
...
nativo de Codex** para el enlace/control de conversaciones de Codex cuando el Plugin `codex`
...
- **¿Quieres respaldo local de texto simple mediante la CLI sin procesar?** Usa backends CLI.

## Sesiones enlazadas

### Modelo mental

- **Superficie de chat** — donde la gente sigue conversando (canal de Discord, tema de Telegram, chat de iMessage).
...
sesión ACP creada: sin hilo secundario, misma superficie de chat. OpenClaw sigue
controlando transporte, autenticación, seguridad y entrega. Los mensajes de seguimiento en esa
...
<AccordionGroup>
  <Accordion title="Binding rules and exclusivity">
    ...
    - `--bind here` solo funciona en canales que anuncian enlace de conversación actual; de lo contrario, OpenClaw devuelve un mensaje claro de no admitido. Los enlaces persisten entre reinicios del Gateway.
  ...
  <Accordion title="Thread-bound sessions">
    ...
    - Desenfocar/cerrar/archivar/tiempo de espera por inactividad o vencimiento por edad máxima elimina el enlace.
    - `/acp close`, `/acp cancel`, `/acp status`, `/status` y `/unfocus` son comandos del Gateway, no prompts para el arnés ACP.

    Indicadores de función requeridos para ACP enlazado a hilos:
    ...
    La compatibilidad con enlaces de hilo es específica del adaptador. Si el adaptador
    de canal activo no admite enlaces de hilo, OpenClaw devuelve un mensaje claro
  ...
  <Accordion title="Thread-supporting channels">
    ...
    - Los canales de Plugin pueden añadir compatibilidad mediante la misma interfaz de enlace.
...
- **DM/grupo de BlueBubbles:** `match.channel="bluebubbles"` + `match.peer.id="<handle|chat_id:*|chat_guid:*|chat_identifier:*>"`. Prefiere `chat_id:*` o `chat_identifier:*` para enlaces de grupo estables.
...
**Precedencia de anulación para sesiones enlazadas ACP:**
...
- Los enlaces de runtime temporales (por ejemplo, creados por flujos de enfoque de hilo) siguen aplicándose donde existan.
...
- Las rutas heredadas de espacio de trabajo faltantes vuelven al cwd predeterminado del backend; los fallos de acceso no relacionados con rutas faltantes aparecen como errores de creación.
...
<ParamField path="agentId" type="string">
  Id. del arnés de destino ACP. Recurre a `acp.defaultAgent` si está definido.
</ParamField>
...
<ParamField path="cwd" type="string">
  Directorio de trabajo de runtime solicitado (validado por la política
  de backend/runtime). Si se omite, la creación ACP hereda el espacio de trabajo
  del agente de destino cuando está configurado; las rutas heredadas ausentes recurren a los valores
...
<ParamField path="runTimeoutSeconds" type="number">
  ...
  ruta sin tiempo de espera del gateway. El mismo valor se aplica a la ejecución de Gateway
  y al runtime ACP para que los arneses bloqueados o con cuota agotada no
  ocupen indefinidamente el carril del agente padre.
</ParamField>
<ParamField path="model" type="string">
  Sustitución explícita del modelo para la sesión hija ACP. Las creaciones ACP de Codex
  normalizan referencias Codex de OpenClaw como `openai-codex/gpt-5.4` a la configuración
  de inicio ACP de Codex antes de `session/new`; las formas de barra como
  `openai-codex/gpt-5.4/high` también establecen el esfuerzo de razonamiento ACP de Codex.
  Otros arneses deben anunciar `models` ACP y admitir
...
<ParamField path="thinking" type="string">
  ...
  omite la sustitución de inicio del esfuerzo de razonamiento.
</ParamField>

## Modos de vinculación y hilo de creación
...
<Tabs>
  ...
  <Tab title="--thread auto|here|off">
    ...
    - En superficies sin vinculación de hilos, el comportamiento predeterminado es, en la práctica, `off`.
...
<AccordionGroup>
  <Accordion title="Sesiones ACP interactivas">
    Las sesiones interactivas están pensadas para seguir hablando en una superficie
    de chat visible:
    ...
    Qué envía OpenClaw al arnés:

    - Los seguimientos vinculados normales se envían como texto de prompt, además de adjuntos solo cuando el arnés/backend los admite.
    - Los comandos de gestión `/acp` y los comandos locales de Gateway se interceptan antes del envío ACP.
    - Los eventos de finalización generados por el runtime se materializan por destino. Los agentes de OpenClaw reciben el sobre de contexto de runtime interno de OpenClaw; los arneses ACP externos reciben un prompt simple con el resultado hijo y la instrucción. El sobre sin procesar `<<<BEGIN_OPENCLAW_INTERNAL_CONTEXT>>>` nunca debe enviarse a arneses externos ni persistirse como texto de transcripción de usuario ACP.
    - Las entradas de transcripción ACP usan el texto activador visible para el usuario o el prompt de finalización simple. Los metadatos de eventos internos permanecen estructurados en OpenClaw cuando es posible y no se tratan como contenido de chat escrito por el usuario.
  ...
  <Accordion title="Sesiones ACP de una sola ejecución propiedad del padre">
    ...
    - Los turnos hijos se ejecutan en el mismo carril en segundo plano que usan las creaciones nativas de subagentes, de modo que un arnés ACP lento no bloquea trabajo no relacionado de la sesión principal.
    - Los informes de finalización vuelven por la ruta de anuncio de finalización de tareas. OpenClaw convierte los metadatos de finalización internos en un prompt ACP simple antes de enviarlo a un arnés externo, por lo que los arneses no ven marcadores de contexto de runtime exclusivos de OpenClaw.
    - El padre reescribe el resultado del hijo con voz normal de asistente cuando una respuesta visible para el usuario es útil.
  ...
  <Accordion title="sessions_send y entrega A2A">
    ...
    ejecutar A2A encima de la finalización de tareas puede despertar al padre con el
  ...
  <Accordion title="Reanudar una sesión existente">
    ...
    - `resumeSessionId` es un id. de reanudación ACP/arnés local del host, no una clave de sesión de canal de OpenClaw; OpenClaw sigue comprobando la política de creación ACP y la política del agente de destino antes del envío, mientras que el backend o arnés ACP es responsable de la autorización para cargar ese id. ascendente.
  ...
  <Accordion title="Prueba de humo posterior al despliegue">
    ...
    2. Abre una sesión puente ACPX temporal a un agente en vivo.
    ...
    las rutas de `mode: "session"` vinculadas a hilos y de retransmisión de stream son pasos
    de integración más completos y separados.
...
Las sesiones ACP se ejecutan actualmente en el runtime del host, **no** dentro del
sandbox de OpenClaw.
...
- OpenClaw sigue aplicando las puertas de funciones de ACP, los agentes permitidos, la propiedad de sesión, las vinculaciones de canal y la política de entrega del Gateway.
...
- Si la sesión solicitante está en sandbox, las creaciones de ACP se bloquean tanto para `sessions_spawn({ runtime: "acp" })` como para `/acp spawn`.
...
| `/acp steer`         | Envía una instrucción de conducción a la sesión en ejecución. | `/acp steer --session support inbox prioritize failing tests` |
| `/acp close`         | Cierra la sesión y desvincula los destinos de hilo.         | `/acp close`                                                  |
| `/acp status`        | Muestra backend, modo, estado, opciones de tiempo de ejecución, capacidades. | `/acp status`                                                 |
| `/acp set-mode`      | Define el modo de tiempo de ejecución de la sesión de destino. | `/acp set-mode plan`                                          |
| `/acp set`           | Escribe una opción genérica de configuración de tiempo de ejecución. | `/acp set model openai/gpt-5.4`                               |
| `/acp cwd`           | Define la anulación del directorio de trabajo de tiempo de ejecución. | `/acp cwd /Users/user/Projects/repo`                          |
...
| `/acp reset-options` | Elimina las anulaciones de opciones de tiempo de ejecución de la sesión. | `/acp reset-options`                                          |
| `/acp sessions`      | Lista las sesiones ACP recientes del almacén.               | `/acp sessions`                                               |
...
`/acp status` muestra las opciones de tiempo de ejecución efectivas más los identificadores de sesión de nivel de tiempo de ejecución y
...
almacén de la sesión vinculada actual o de la sesión solicitante; los tokens de destino
...
los puentes MCP de plugin-tools y OpenClaw-tools, y los modos de permiso de ACP,
consulta
...
| `ACP dispatch is disabled by policy (acp.dispatch.enabled=false)`           | El despacho automático desde mensajes normales de hilos está deshabilitado.                                            | Configura `acp.dispatch.enabled=true` para reanudar el enrutamiento automático de hilos; las llamadas explícitas `sessions_spawn({ runtime: "acp" })` siguen funcionando. |
...
| `/acp doctor` reports backend not ready right after startup                 | Falta el Plugin de backend, está deshabilitado, está bloqueado por la política de permitir/denegar, o su ejecutable configurado no está disponible. | Instala/habilita el Plugin de backend, vuelve a ejecutar `/acp doctor` e inspecciona el error de instalación o de política del backend si sigue sin estar en buen estado. |
| No se encuentra el comando del arnés                                         | La CLI del adaptador no está instalada, falta el Plugin externo o la obtención inicial de `npx` falló para un adaptador que no es Codex. | Ejecuta `/acp doctor`, instala/precalienta el adaptador en el host del Gateway o configura explícitamente el comando del agente acpx.                                      |
...
| `--bind here requires running /acp spawn inside an active ... conversation` | Se usó `--bind here` sin una conversación vinculable activa.                                                           | Muévete al chat/canal de destino y vuelve a intentarlo, o usa una generación sin vincular.                                                                                |
| `Conversation bindings are unavailable for <channel>.`                      | Al adaptador le falta la capacidad de vinculación ACP de la conversación actual.                                      | Usa `/acp spawn ... --thread ...` donde sea compatible, configura `bindings[]` de nivel superior o muévete a un canal compatible.                                         |
...
| `Only <user-id> can rebind this channel/conversation/thread.`               | Otro usuario posee el destino de vinculación activo.                                                                   | Vuelve a vincular como propietario o usa otra conversación u otro hilo.                                                                                                   |
| `Thread bindings are unavailable for <channel>.`                            | Al adaptador le falta la capacidad de vinculación de hilos.                                                           | Usa `--thread off` o muévete a un adaptador/canal compatible.                                                                                                             |
| `Sandboxed sessions cannot spawn ACP sessions ...`                          | El tiempo de ejecución ACP está del lado del host; la sesión solicitante está en sandbox.                             | Usa `runtime="subagent"` desde sesiones en sandbox, o ejecuta ACP spawn desde una sesión sin sandbox.                                                                     |
...
| `Cannot apply --model ... did not advertise model support`                  | El arnés de destino no expone el cambio genérico de modelo ACP.                                                       | Usa un arnés que anuncie ACP `models`/`session/set_model`, usa referencias de modelo ACP de Codex o configura el modelo directamente en el arnés si tiene su propia marca de inicio. |
...
| La sesión ACP falla temprano con poca salida                                | Las solicitudes de permisos están bloqueadas por `permissionMode`/`nonInteractivePermissions`.                        | Revisa los registros del gateway en busca de `AcpRuntimeError`. Para permisos completos, configura `permissionMode=approve-all`; para degradación controlada, configura `nonInteractivePermissions=deny`. |
| La sesión ACP se queda detenida indefinidamente después de completar el trabajo | El proceso del arnés terminó, pero la sesión ACP no informó la finalización.                                          | Supervisa con `ps aux \| grep acpx`; termina manualmente los procesos obsoletos.                                                                                         |
