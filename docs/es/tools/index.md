...
read_when:
    - Quieres saber qué herramientas ofrece OpenClaw
    - Estás eligiendo entre herramientas integradas, Skills y plugins
    - Necesitas el punto de entrada adecuado de la documentación para la política de herramientas, la automatización o la coordinación de agentes.
...
x-i18n:
    generated_at: "2026-07-11T23:35:00Z"
    model: gpt-5.6
...
Esta es una página de descripción general y orientación. Para consultar exhaustivamente la política de herramientas, los valores predeterminados,
...
Para la mayoría de los agentes, empieza con las categorías de herramientas integradas y después ajusta la política
...
<Steps>
  <Step title="Usa una herramienta cuando el agente necesite actuar">
    ...
    permitir/denegar, las restricciones del proveedor, el estado del sandbox, los permisos
    del canal y la disponibilidad de los plugins.
  ...
  <Step title="Usa un plugin cuando OpenClaw necesite una capacidad nueva">
...
permitir/denegar, usa [Herramientas y proveedores personalizados](/es/gateway/config-tools).

| Categoría                    | Úsala cuando el agente necesite...                                                | Herramientas representativas                                                                         | Consulta después                                                                            |
| ---------------------------- | --------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------- |
| Ejecución                    | Ejecutar comandos, gestionar procesos o usar análisis de Python respaldado por un proveedor | `exec`, `process`, `code_execution`                                                                  | [Exec](/es/tools/exec), [Ejecución de código](/es/tools/code-execution)                           |
...
| Multimedia                   | Analizar, generar o reproducir mediante voz contenido multimedia                   | `image`, `image_generate`, `music_generate`, `video_generate`, `tts`                                 | [Descripción general de multimedia](/es/tools/media-overview)                                 |
| Catálogos grandes de OpenClaw | Buscar e invocar muchas herramientas aptas sin enviar todos los esquemas al modelo | `tool_search_code`, `tool_search`, `tool_describe`                                                   | [Búsqueda de herramientas](/es/tools/tool-search)                                              |

<Note>
La Búsqueda de herramientas es una superficie experimental para agentes de OpenClaw. Las ejecuciones del arnés de Codex usan
el modo de código nativo de Codex, la búsqueda de herramientas nativa, herramientas dinámicas diferidas y
...
para obtener detalles de los contratos.
...
- [Mostrar widget](/tools/show-widget) para SVG y HTML autocontenidos e integrados en el chat web
- [Tarea de LLM](/es/tools/llm-task) para pasos de flujos de trabajo exclusivamente en JSON
...
- [Canvas](/es/plugins/reference/canvas) para controlar Canvas en nodos y
  representar A2UI
...
debido a la configuración global, la configuración específica del agente, la política del canal, las restricciones
del proveedor, las reglas del sandbox, la política del canal o del entorno de ejecución, o la disponibilidad de los plugins.

- [Herramientas y proveedores personalizados](/es/gateway/config-tools) documenta los perfiles de herramientas,
  las listas de permitir/denegar, las restricciones específicas de cada proveedor, la detección de bucles y
  la configuración de herramientas respaldadas por proveedores.
- [Aprobaciones de Exec](/es/tools/exec-approvals) documenta la política de aprobación
  de comandos del host.
- [Exec elevado](/es/tools/elevated) documenta la ejecución controlada fuera del
  sandbox.
- [Sandbox frente a política de herramientas frente a acceso elevado](/es/gateway/sandbox-vs-tool-policy-vs-elevated)
...
Elige la vía de ampliación según la tarea que necesites que OpenClaw realice:
...
- Añade o ajusta instrucciones reutilizables para agentes con [Skills](/es/tools/skills) y
  [Crear Skills](/es/tools/creating-skills).
- Usa [SDK de Plugin](/es/plugins/sdk-overview) y
  [Manifiesto de Plugin](/es/plugins/manifest) cuando necesites contratos
  de implementación.

## Soluciona problemas de herramientas ausentes

Si el modelo no puede ver o invocar una herramienta, empieza por la política efectiva del
turno actual:
...
2. Comprueba las restricciones específicas del proveedor en
   ...
   [proveedor de modelos](/es/concepts/model-providers) seleccionado admite la estructura de la
   herramienta.
...
5. Para ejecuciones delegadas, comprueba las restricciones por agente en
   [Sandbox y restricciones de herramientas por agente](/es/tools/multi-agent-sandbox-tools).
6. Para catálogos grandes de OpenClaw, confirma si la ejecución usa la exposición directa de
   herramientas o la [Búsqueda de herramientas](/es/tools/tool-search).

## Contenido relacionado

- [Automatización](/es/automation) para Cron, tareas, Heartbeat, compromisos, hooks,
  órdenes permanentes y TaskFlow
- [Agentes](/es/concepts/agent) para el modelo de agente, las sesiones, la memoria y
  la coordinación multiagente
- [Herramientas y proveedores personalizados](/es/gateway/config-tools) como referencia canónica
  de políticas de herramientas
- [Plugins](/es/tools/plugin) para instalar y gestionar plugins
...
- [Taller de Skills](/es/tools/skill-workshop) para crear Skills generadas
  y revisadas
- [Búsqueda de herramientas](/es/tools/tool-search) para descubrir el catálogo compacto
  de herramientas de OpenClaw
