...
x-i18n:
    generated_at: "2026-05-03T21:39:05Z"
    model: gpt-5.5
    provider: openai
...
Todo lo que el agente hace más allá de generar texto sucede mediante **herramientas**.
...
| ------------------------------------------ | --------------------------------------------------------------------- | ------------------------------------------------------------ |
| `exec` / `process`                         | Ejecuta comandos de shell, gestiona procesos en segundo plano         | [Exec](/es/tools/exec), [Aprobaciones de Exec](/es/tools/exec-approvals) |
| `code_execution`                           | Ejecuta análisis remoto de Python en sandbox                          | [Ejecución de código](/es/tools/code-execution)                 |
| `browser`                                  | Controla un navegador Chromium (navegar, hacer clic, captura)         | [Navegador](/es/tools/browser)                                  |
| `web_search` / `x_search` / `web_fetch`    | Busca en la web, busca publicaciones de X, obtiene contenido de páginas | [Web](/es/tools/web), [Obtención web](/es/tools/web-fetch)         |
| `read` / `write` / `edit`                  | E/S de archivos en el espacio de trabajo                              |                                                              |
| `apply_patch`                              | Parches de archivo con múltiples hunks                                | [Apply Patch](/es/tools/apply-patch)                            |
| `message`                                  | Envía mensajes por todos los canales                                  | [Envío del agente](/es/tools/agent-send)                        |
| `canvas`                                   | Controla Canvas de Node (presentar, evaluar, instantánea)             |                                                              |
| `nodes`                                    | Descubre y dirige dispositivos emparejados                            |                                                              |
| `cron` / `gateway`                         | Gestiona trabajos programados; inspecciona, parchea, reinicia o actualiza el Gateway |                                                              |
| `image` / `image_generate`                 | Analiza o genera imágenes                                             | [Generación de imágenes](/es/tools/image-generation)            |
| `music_generate`                           | Genera pistas musicales                                               | [Generación de música](/es/tools/music-generation)              |
| `video_generate`                           | Genera videos                                                         | [Generación de videos](/es/tools/video-generation)              |
...
la lista `contracts.tools` del manifiesto del plugin. OpenClaw captura el
descriptor de herramienta validado durante el descubrimiento y lo almacena en caché por origen y contrato
del plugin, para que la planificación posterior de herramientas pueda omitir la carga del runtime del plugin.
La ejecución de herramientas todavía carga el plugin propietario y llama a la implementación registrada en vivo.
...
pero no la herramienta completa de control de navegador. La automatización del navegador puede controlar
...
etiquetas de razonamiento, andamiaje de `<relevant-memories>`, cargas XML de
llamadas a herramientas en texto plano (incluidos `<tool_call>...</tool_call>`,
`<function_call>...</function_call>`, `<tool_calls>...</tool_calls>`,
`<function_calls>...</function_calls>` y bloques truncados de llamadas a
herramientas), andamiaje degradado de llamadas a herramientas, tokens de control
del modelo filtrados en ASCII/ancho completo y XML malformado de llamadas a
herramientas de MiniMax del texto del asistente; luego aplica redacción/truncamiento
y posibles marcadores de posición para filas sobredimensionadas en lugar de actuar
como un volcado de transcripción sin procesar.

### Restricciones específicas de proveedor
