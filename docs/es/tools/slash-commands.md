...
x-i18n:
    generated_at: "2026-07-22T10:52:57Z"
    model: gpt-5.6
...
Los comandos de bash exclusivos del host usan `! <cmd>` (con `/bash <cmd>` como alias).
...
<AccordionGroup>
  <Accordion title="Detalles del comportamiento de las directivas">
    ...
    - Las directivas solo se aplican a **remitentes autorizados**. Si se establece `commands.allowFrom`,
      ...
      las listas de permitidos de los canales, el emparejamiento y la aplicación permanente de grupos de acceso. Para los remitentes
...
<ParamField path="commands.native" type='boolean | "auto"' default='"auto"'>
  ...
  se ignora para proveedores sin compatibilidad nativa. Se puede anular por canal mediante
  `channels.<provider>.commands.native`. En Discord, `false` omite el registro de comandos
  de barra diagonal; los comandos registrados anteriormente pueden permanecer visibles hasta que se eliminen.
</ParamField>

<ParamField path="commands.nativeSkills" type='boolean | "auto"' default='"auto"'>
  Registra de forma nativa los comandos de Skills cuando se admite. Automático: activado para
  Discord/Telegram; desactivado para Slack. Se puede anular mediante
  `channels.<provider>.commands.nativeSkills`.
</ParamField>

<ParamField path="commands.bash" type="boolean" default="false">
  Habilita `! <cmd>` para ejecutar comandos del shell del host (alias `/bash <cmd>`). Requiere
  las listas de permitidos `tools.elevated`.
</ParamField>

<ParamField path="commands.bashForegroundMs" type="number" default="2000">
  Tiempo que espera bash antes de pasar al modo en segundo plano (`0` pasa
...
<ParamField path="commands.debug" type="boolean" default="false">
  Habilita `/debug` (anulaciones de configuración exclusivas del entorno de ejecución). Solo para el propietario.
</ParamField>
...
<ParamField path="commands.ownerAllowFrom" type="string[]">
  Lista explícita de propietarios permitidos para superficies de comandos exclusivas del propietario. Es independiente de
  `commands.allowFrom` y del acceso por emparejamiento de mensajes directos.
</ParamField>
...
<AccordionGroup>
  <Accordion title="Sesiones y ejecuciones">
    ...
    | `/reset [soft [message]]` | Restablece la sesión actual sin reemplazarla. `soft` conserva la transcripción, descarta los identificadores de sesión reutilizados del backend de la CLI y vuelve a ejecutar el inicio |
    ...
    Las rutas explícitas `/export-session` reemplazan los archivos existentes dentro del
    espacio de trabajo. Omita la ruta para generar un nombre de archivo que evite colisiones.

    <Note>
      Control UI intercepta el texto `/new` para crear una nueva sesión del
      panel y cambiar a ella, excepto cuando `session.dmScope: "main"` está configurado
      y el elemento superior actual es la sesión principal del agente; en ese caso, `/new`
      restablece la sesión principal sin reemplazarla. El texto `/reset` sigue ejecutando el
      restablecimiento sin reemplazo del Gateway. Use `/model default` cuando desee borrar
      la selección de modelo fijada de una sesión.
    </Note>
  ...
  <Accordion title="Controles del modelo y de la ejecución">
    | Comando | Descripción |
    | --- | --- |
    | `/think <level\|default>` | Establece el nivel de pensamiento o borra la anulación de la sesión. Alias: `/thinking`, `/t` |
    ...
    | `/login [codex\|openai\|openai-codex]` | Vincula el inicio de sesión de Codex/OpenAI desde un chat privado o una sesión de la interfaz web. Solo para propietarios/administradores |
    ...
    | `/queue <mode>` | Gestiona el comportamiento de la cola de ejecuciones activas. Consulte [Cola](/es/concepts/queue) y [Control de la cola](/es/concepts/queue-steering) |
    | `/steer <message>` | Inserta instrucciones en la ejecución activa. Alias: `/tell`. Consulte [Orientar](/es/tools/steer) |

    <AccordionGroup>
      <Accordion title="Seguridad de los modos detallado, seguimiento, rápido y razonamiento">
        - `/verbose` sirve para la depuración; manténgalo **desactivado** durante el uso normal.
        - `/trace` solo muestra las líneas de seguimiento/depuración propias de los plugins; los mensajes detallados normales permanecen desactivados.
        - `/fast auto|on|off` conserva una anulación de sesión; use la opción `inherit` de la interfaz de sesiones para borrarla.
  ...
  <Accordion title="Detección y estado">
    ...
    | `/diagnostics [note]` | Flujo de informe de soporte solo para el propietario. Solicita aprobación de ejecución cada vez |
    ...
    | `/usage off\|tokens\|full\|reset\|cost` | Controla el pie de uso por respuesta (`reset`/`inherit`/`clear`/`default` borra la anulación de la sesión para volver a heredar el valor predeterminado configurado) o muestra un resumen local de costes |
  </Accordion>
  ...
  <Accordion title="Skills, listas de permitidos, aprobaciones">
    ...
    | `/learn [request]` | Redactar una skill revisable a partir de la conversación actual o de fuentes indicadas mediante [Skill Workshop](/es/tools/skill-workshop) |
  ...
  <Accordion title="Subagentes y ACP">
    ...
    | `/acp spawn\|cancel\|steer\|close\|sessions\|status\|set-mode\|set\|cwd\|permissions\|timeout\|model\|reset-options\|doctor\|install\|help` | Gestionar sesiones de ACP y opciones del entorno de ejecución. Los controles del entorno de ejecución requieren la identidad del propietario externo o del administrador interno del Gateway |
    | `/focus <target>` | Vincular el hilo actual de Discord o el tema de Telegram con un destino de sesión |
  ...
  <Accordion title="Escrituras y administración exclusivas del propietario">
    ...
    | `/mcp show\|get\|set\|unset` | `commands.mcp: true` | Leer o escribir la configuración del servidor MCP gestionada por OpenClaw. Solo para el propietario |
...
| Comando                                                 | Descripción                                                                                                                                                                                    |
| ------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `/dreaming [on\|off\|status\|help]`                     | Activar o desactivar Dreaming de la memoria (propietario o administrador del Gateway). Véase [Dreaming](/es/concepts/dreaming)                                                                                                            |
...
| `/voice status\|list\|set <voiceId>`                    | Gestionar la configuración de voz de Talk. Nombre nativo de Discord: `/talkvoice`                                                                                                                                    |
| `/card ...`                                             | Enviar preajustes de tarjetas enriquecidas de LINE. Véase [LINE](/es/channels/line)                                                                                                                                        |
...
utilice el panel Tools de la Control UI o las superficies de configuración.
...
`/mcp show` oculta los campos que contienen credenciales, los valores reconocidos de opciones de credenciales
y los argumentos con formas conocidas de secretos. Cuando se ejecuta desde un grupo, la
configuración se envía de forma privada al propietario; si no hay disponible una ruta privada hacia el propietario,
el comando se cierra de forma segura y solicita al propietario que vuelva a intentarlo desde un
chat directo.
...
`/plugins enable|disable` actualiza la configuración de los plugins y recarga en caliente el entorno de ejecución de plugins del Gateway
...
requieren un `--force` al final después de revisar el código fuente. Esta opción confirma
el origen y permite sustituir una instalación existente; no omite
`security.installPolicy` ni las comprobaciones de seguridad del instalador. Las versiones de ClawHub con
advertencias de riesgo siguen requiriendo la opción independiente, disponible solo en el shell,
`--acknowledge-clawhub-risk`. Las instalaciones del marketplace, vinculadas y fijadas también
siguen estando disponibles solo en el shell.
...
<AccordionGroup>
  ...
  <Accordion title="Vía rápida y atajos insertados">
    - Los mensajes que contienen solo comandos de remitentes incluidos en la lista de permitidos se procesan inmediatamente (omiten la cola y el modelo).
    - Los atajos insertados (`/help`, `/commands`, `/status`, `/whoami`) también funcionan dentro de mensajes normales y se eliminan antes de que el modelo vea el texto restante.
    - Los mensajes no autorizados que contienen solo comandos se ignoran silenciosamente; los tokens `/...` insertados se tratan como texto sin formato.
...
- El **uso o la cuota del proveedor** (por ejemplo, «Claude: 80 % restante») se muestra en `/status` para el proveedor del modelo actual cuando está habilitado el seguimiento del uso.
