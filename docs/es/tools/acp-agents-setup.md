---
read_when:
    - Instalación o configuración del arnés de acpx para Claude Code / Codex / Gemini CLI
...
Para la descripción general, la guía operativa y los conceptos, consulta [agentes ACP](/es/tools/acp-agents).
...
Usa esta página solo cuando estés configurando la ruta ACP/acpx. Para la
configuración del runtime app-server nativo de Codex, usa [arnés de Codex](/es/plugins/codex-harness). Para
claves de API de OpenAI o configuración del proveedor de modelos OAuth de Codex, usa
...
El control de modelos depende de las capacidades del adaptador. Las referencias
de modelo ACP de Codex son normalizadas por OpenClaw antes del inicio. Otros arneses necesitan `models` de ACP más
...
Si desactivaste `acpx`, lo denegaste mediante `plugins.allow` / `plugins.deny`, o quieres
...
De forma predeterminada, el Plugin `acpx` registra el backend ACP integrado sin
generar un agente ACP durante el inicio del Gateway. Ejecuta `/acp doctor` para una
prueba en vivo explícita. Configura `OPENCLAW_ACPX_RUNTIME_STARTUP_PROBE=1` solo cuando necesites que el
Gateway pruebe el agente configurado al inicio.
...
mediante un hook de postinstalación. Si la instalación automática falla, el gateway sigue iniciándose
con normalidad e informa la dependencia faltante mediante `openclaw acp doctor`.
...
Si quieres que agentes ACP como Codex o Claude Code llamen a herramientas de Plugins
de OpenClaw instaladas, como recuperar/almacenar memoria, activa el puente dedicado:
...
MCP. Activa el puente separado de herramientas del núcleo cuando un agente ACP necesite herramientas
...
El Plugin `acpx` establece de forma predeterminada un tiempo de espera de 120 segundos
para los turnos de runtime integrados. Esto da a arneses más lentos, como Gemini CLI, tiempo suficiente para completar
el inicio y la inicialización de ACP. Sobrescríbelo si tu host necesita un límite de
runtime diferente:
...
Cuando `/acp doctor` o la prueba de inicio opcional comprueba el backend, el Plugin
`acpx` incluido prueba un agente de arnés. Si `acp.allowedAgents` está configurado, el valor
predeterminado es el primer agente permitido; de lo contrario, el valor predeterminado es `codex`. Si tu
despliegue necesita un agente ACP diferente para comprobaciones de estado, configura el agente de prueba
explícitamente:
...
Si necesitas restringir permisos, configura `nonInteractivePermissions` como `deny` para que las sesiones se degraden de forma gradual en lugar de fallar.
</Warning>
