...
<AccordionGroup>
  ...
  <Accordion title="¿Qué modelo se recomienda?">
    ...
    especialmente para agentes con herramientas habilitadas o que reciben entradas no confiables; los modelos
    más débiles o excesivamente cuantizados son más vulnerables a la inyección de prompts y a comportamientos
    inseguros (consulte [Seguridad](/es/gateway/security)). Dirija los chats rutinarios o de bajo
    riesgo a modelos más económicos según el rol del agente.
  ...
  <Accordion title="¿Se pueden utilizar modelos autoalojados (llama.cpp, vLLM, Ollama)?">
    ...
    Utilice modelos grandes para cualquier bot con acceso a herramientas; si aun así utiliza modelos
  ...
  <Accordion title="Si dos proveedores ofrecen el mismo identificador de modelo, ¿cuál utiliza /model?">
  ...
  <Accordion title="¿Se puede utilizar GPT 5.5 para tareas diarias y Codex 5.5 para programación?">
    ...
    - **Autenticación del agente de OpenAI mediante clave de API:** `/model openai/gpt-5.5` con un perfil
  ...
  <Accordion title="¿Cómo se configura el modo rápido para GPT 5.5?">
    ...
    - **Límite automático:** `/fast auto` o `params.fastMode: "auto"` ejecuta rápidamente las nuevas
      llamadas al modelo hasta alcanzar el límite y, después, ejecuta las llamadas posteriores de reintento, conmutación por error,
      resultado de herramienta o continuación sin el modo rápido. El límite predeterminado es de
      60 segundos; anúlelo con `params.fastAutoOnSeconds` en el modelo.
  ...
  <Accordion title='¿Por qué aparece "El modelo ... no está permitido" y después no hay respuesta?'>
  ...
  <Accordion title='¿Por qué aparece "Modelo desconocido: minimax/MiniMax-M3"?'>
  ...
  <Accordion title="¿Se puede utilizar MiniMax como predeterminado y OpenAI para tareas complejas?">
  ...
  <Accordion title="¿Cómo se definen o anulan los accesos directos de modelos (alias)?">
  ...
  <Accordion title="¿Cómo se añaden modelos de otros proveedores como OpenRouter o Z.AI?">
    ...
    Si falta la clave del proveedor correspondiente a un proveedor/modelo referenciado, se produce un error de
...
<AccordionGroup>
  <Accordion title="¿Cómo funciona la conmutación por error?">
    ...
    Se aplican periodos de espera a los perfiles que fallan (retroceso exponencial), por lo que OpenClaw
    sigue respondiendo cuando un proveedor limita la frecuencia o falla temporalmente.

    El grupo de límites de frecuencia abarca más que simples `429`: `Too many concurrent
    ...
    proveedor. El texto genérico interno de alternativa como `LLM request failed
    with an unknown error.` se interpreta de forma conservadora y no activa una alternativa
    por sí solo.
  ...
  <Accordion title='¿Qué significa "No credentials found for profile anthropic:default"?'>
    ...
    **Lista de comprobación para solucionarlo:**
    ...
    - Confirma que el Gateway carga la variable de entorno. Si `ANTHROPIC_API_KEY` solo está definida en
      el shell, no llegará a un Gateway ejecutado mediante systemd/launchd; añádela a
    ...
    **Para "No credentials found for profile anthropic" (sin sufijo de correo electrónico):**
    ...
    - Si se prefiere una clave de API: añade `ANTHROPIC_API_KEY` a
      `~/.openclaw/.env` en el host del Gateway y, a continuación, borra cualquier orden fijado
    ...
    - Modo remoto: los perfiles de autenticación se almacenan en la máquina del Gateway, no en el
      portátil; confirma que los comandos se ejecutan allí.
  ...
  <Accordion title="¿Por qué también probó Google Gemini y falló?">
    ...
    hay credenciales de Google configuradas, se obtiene `No API key found for provider
    "google"`. Solución: añade la autenticación de Google o elimina los modelos de Google de
...
<AccordionGroup>
  <Accordion title="¿Qué es un perfil de autenticación?">
    Un registro de credenciales con nombre (OAuth o clave de API) vinculado a un proveedor, almacenado
    en:
  ...
  <Accordion title="¿Puedo controlar qué perfil de autenticación se prueba primero?">
    ...
    --json` y comprueba `auth.unusableProfiles`. Los periodos de espera por límites de frecuencia pueden
    ...
    # Limita la rotación a un solo perfil
    ...
    Verifica qué se probará realmente: `openclaw models status --probe`. Un
  ...
  <Accordion title="OAuth frente a clave de API: ¿cuál es la diferencia?">
    - El **inicio de sesión con OAuth o mediante la CLI** suele utilizar el acceso por suscripción cuando el
      ...
      usa `claude -p` de Claude Code, que Anthropic considera actualmente
      un uso programático o mediante el Agent SDK que consume los límites de uso de la suscripción;
      consulta [Anthropic](/es/providers/anthropic) para conocer el estado actual de la pausa de facturación
