...
read_when:
    ...
    - No se puede acceder a docs.openclaw.ai, no se puede abrir el panel de control, la instalación se queda atascada
sidebarTitle: First-run FAQ
summary: 'Preguntas frecuentes: inicio rápido y configuración del primer uso — instalación, incorporación, autenticación, suscripciones, fallos iniciales'
...
---
  ...
  Inicio rápido y preguntas y respuestas de primera ejecución. Para operaciones cotidianas, modelos, autenticación, sesiones
  y resolución de problemas, consulta las [Preguntas frecuentes](/es/help/faq) principales.
  ...
  <Accordion title="Estoy atascado; la forma más rápida de desbloquearme">
    ...
    Si descubres un error real o una corrección, abre una issue de GitHub o envía un PR:
  ...
  <Accordion title="Heartbeat sigue omitiéndose. ¿Qué significan los motivos de omisión?">
    Motivos comunes por los que se omite Heartbeat:

    - `quiet-hours`: fuera de la ventana de horas activas configurada
    - `empty-heartbeat-file`: `HEARTBEAT.md` existe, pero solo contiene una estructura vacía o solo con encabezados
    - `no-tasks-due`: el modo de tareas de `HEARTBEAT.md` está activo, pero ninguno de los intervalos de tareas ha vencido aún
    - `alerts-disabled`: toda la visibilidad de Heartbeat está desactivada (`showOk`, `showAlerts` y `useIndicator` están desactivados)

    En el modo de tareas, las marcas de tiempo vencidas solo avanzan después de que se complete
  ...
  <Accordion title="¿Cómo autentico el panel en localhost frente a remoto?">
    **Localhost (misma máquina):**
    ...
    - Si solicita autenticación con secreto compartido, pega el token o la contraseña configurados en la configuración de la interfaz de control.
    ...
    **No en localhost:**
    ...
    - **Tailscale Serve** (recomendado): mantén el enlace en local loopback, ejecuta `openclaw gateway --tailscale serve`, abre `https://<magicdns>/`. Si `gateway.auth.allowTailscale` es `true`, los encabezados de identidad satisfacen la autenticación de la interfaz de control/WebSocket (sin secreto compartido pegado, asume un host de gateway de confianza); las API HTTP siguen requiriendo autenticación con secreto compartido salvo que uses deliberadamente private-ingress `none` o autenticación HTTP con proxy de confianza.
      Los intentos concurrentes incorrectos de autenticación de Serve desde el mismo cliente se serializan antes de que el limitador de autenticación fallida los registre, por lo que el segundo reintento incorrecto ya puede mostrar `retry later`.
  ...
  <Accordion title="¿Por qué hay dos configuraciones de aprobación de exec para aprobaciones por chat?">
    ...
    - Si un canal nativo compatible puede inferir aprobadores de forma segura, OpenClaw ahora habilita automáticamente aprobaciones nativas primero por DM cuando `channels.<channel>.execApprovals.enabled` no está definido o es `"auto"`.
    ...
    - Las aprobaciones de Plugin vuelven a ser independientes: usan `/approve` en el mismo chat de forma predeterminada, reenvío opcional con `approvals.plugin`, y solo algunos canales nativos mantienen manejo nativo de aprobación de Plugin además de eso.

    Versión breve: el reenvío es para el enrutamiento; la configuración de cliente nativo es para una UX más rica y específica del canal.
  ...
  <Accordion title="¿Algún consejo para instalaciones en Raspberry Pi?">
    Versión breve: funciona, pero espera algunas asperezas.
  ...
  <Accordion title="Está atascado en wake up my friend / onboarding will not hatch. ¿Y ahora qué?">
    Esa pantalla depende de que el Gateway sea accesible y esté autenticado. La TUI también envía
    "Wake up, my friend!" automáticamente en el primer hatch. Si ves esa línea con **sin respuesta**
  ...
  <Accordion title="¿Puedo migrar mi configuración a una máquina nueva (Mac mini) sin rehacer el onboarding?">
    ...
    mantiene tu bot "exactamente igual" (memoria, historial de sesiones, autenticación y estado del canal)
    ...
    **Importante:** si solo haces commit/push de tu workspace a GitHub, estás haciendo copia de seguridad
    ...
    Relacionado: [Migrar](/es/install/migrating), [Dónde viven las cosas en disco](/es/help/faq#where-things-live-on-disk),
  ...
  <Accordion title="¿Dónde veo qué hay de nuevo en la versión más reciente?">
    ...
    **Fixes** (más secciones de documentación/otras cuando se necesitan).
  ...
  <Accordion title="¿Cómo pruebo los cambios más recientes?">
    ...
    Si prefieres hacer un clon limpio manualmente, usa:
  ...
  <Accordion title="La instalación de Windows dice que no se encontró git u openclaw no se reconoce">
    ...
    **2) openclaw no se reconoce después de instalar**
    ...
    - Añade ese directorio al PATH de tu usuario (no se necesita el sufijo `\bin` en Windows; en la mayoría de los sistemas es `%AppData%\npm`).
  ...
  <Accordion title="¿Dónde están las guías de instalación en la nube/VPS?">
    ...
    desde tu portátil/teléfono mediante la UI de control (o Tailscale/SSH). Tu estado + espacio de trabajo
  ...
  <Accordion title="¿Qué hace realmente la incorporación?">
    ...
    - **Configuración de modelo/autenticación** (OAuth del proveedor, claves de API, setup-token de Anthropic, además de opciones de modelo local como LM Studio)
  ...
  <Accordion title="¿Necesito una suscripción a Claude u OpenAI para ejecutar esto?">
    ...
    **modelos solo locales** para que tus datos permanezcan en tu dispositivo. Las suscripciones (Claude
    ...
    más predecible. OAuth de OpenAI Codex es compatible explícitamente con herramientas externas
    como OpenClaw.
  ...
  <Accordion title="¿Admiten la autenticación de suscripción de Claude (Claude Pro o Max)?">
    Sí.
    ...
    El setup-token de Anthropic sigue disponible como una ruta de token compatible en OpenClaw, pero OpenClaw ahora prefiere la reutilización de Claude CLI y `claude -p` cuando estén disponibles.
...
<AccordionGroup>
  <Accordion title="¿Por qué veo HTTP 429 rate_limit_error de Anthropic?">
    ...
    credencial cumple los requisitos para la facturación de contexto largo (facturación con clave de API o la
    ruta de inicio de sesión con Claude de OpenClaw con Extra Usage habilitado).

    Consejo: configura un **modelo de respaldo** para que OpenClaw pueda seguir respondiendo mientras un proveedor tenga límites de tasa.
  ...
  <Accordion title="¿Cómo funciona la autenticación de Codex?">
    ...
    `openai/gpt-5.5` para la configuración común: autenticación de suscripción de ChatGPT/Codex más
    ejecución nativa de Codex en el servidor de aplicación. Las referencias de modelo `openai-codex/gpt-*` son
    configuración heredada reparada por `openclaw doctor --fix`. El acceso directo con clave de API de OpenAI
    sigue disponible para superficies de la API de OpenAI que no son de agente y para modelos de agente
  ...
  <Accordion title="¿Por qué OpenClaw sigue mencionando openai-codex?">
    ...
    - `openai/gpt-5.5` = autenticación de suscripción de ChatGPT/Codex con runtime nativo de Codex para turnos de agente
  ...
  <Accordion title="¿OpenClaw admite autenticación por suscripción de OpenAI (OAuth de Codex)?">
  ...
  <Accordion title="Telegram: ¿qué va en allowFrom?">
    ...
    La configuración solo solicita IDs de usuario numéricos. Si ya tienes entradas heredadas `@username` en la configuración, `openclaw doctor --fix` puede intentar resolverlas.

    Más seguro (sin bot de terceros):

    - Envía un mensaje directo a tu bot, luego ejecuta `openclaw logs --follow` y lee `from.id`.

    Bot API oficial:

    - Envía un mensaje directo a tu bot, luego llama a `https://api.telegram.org/bot<bot_token>/getUpdates` y lee `message.from.id`.
  ...
  <Accordion title="¿Varias personas pueden usar un número de WhatsApp con distintas instancias de OpenClaw?">
    Sí, mediante **enrutamiento multiagente**. Vincula el **mensaje directo** de WhatsApp de cada remitente (par `kind: "direct"`, remitente E.164 como `+15551234567`) a un `agentId` distinto, para que cada persona tenga su propio espacio de trabajo y almacén de sesiones. Las respuestas siguen saliendo de la **misma cuenta de WhatsApp**, y el control de acceso de mensajes directos (`channels.whatsapp.dmPolicy` / `channels.whatsapp.allowFrom`) es global por cuenta de WhatsApp. Consulta [Enrutamiento multiagente](/es/concepts/multi-agent) y [WhatsApp](/es/channels/whatsapp).
  </Accordion>

  <Accordion title='¿Puedo ejecutar un agente de "chat rápido" y un agente "Opus para programación"?'>
  ...
  <Accordion title="Diferencia entre la instalación modificable con git y la instalación con npm">
    - **Instalación modificable (git):** copia completa del código fuente, editable, ideal para colaboradores.
      Ejecutas compilaciones localmente y puedes parchear código/documentación.
    - **Instalación con npm:** instalación global de CLI, sin repositorio, ideal para "solo ejecutarlo".
      Las actualizaciones vienen de las dist-tags de npm.

    Documentación: [Primeros pasos](/es/start/getting-started), [Actualización](/es/install/updating).

  </Accordion>

  <Accordion title="¿Puedo cambiar más adelante entre instalaciones con npm y git?">
    ...
    seguimientos de Doctor, actualiza las fuentes de Plugins para el canal objetivo y
    ...
    Consejos de copia de seguridad: consulta [Estrategia de copias de seguridad](/es/help/faq#where-things-live-on-disk).

  </Accordion>

  <Accordion title="¿Debo ejecutar el Gateway en mi laptop o en un VPS?">
    Respuesta breve: **si quieres fiabilidad 24/7, usa un VPS**. Si quieres la
    menor fricción y te parecen aceptables la suspensión y los reinicios, ejecútalo localmente.

    **Laptop (Gateway local)**

    - **Ventajas:** sin costo de servidor, acceso directo a archivos locales, ventana del navegador en vivo.
    - **Desventajas:** suspensión/caídas de red = desconexiones, las actualizaciones/reinicios del SO interrumpen, debe permanecer activa.

    **VPS / nube**

    - **Ventajas:** siempre activo, red estable, sin problemas de suspensión de la laptop, más fácil de mantener en ejecución.
    ...
    **Nota específica de OpenClaw:** WhatsApp/Telegram/Slack/Mattermost/Discord funcionan bien desde un VPS. La única concesión real es **navegador sin interfaz gráfica** frente a una ventana visible. Consulta [Navegador](/es/tools/browser).

    **Valor predeterminado recomendado:** VPS si ya tuviste desconexiones del gateway. Local es ideal cuando estás usando activamente la Mac y quieres acceso a archivos locales o automatización de UI con un navegador visible.
  ...
  <Accordion title="¿Qué tan importante es ejecutar OpenClaw en una máquina dedicada?">
    ...
    - **Laptop/escritorio compartido:** totalmente válido para pruebas y uso activo, pero espera pausas cuando la máquina se suspenda o se actualice.

    Si quieres lo mejor de ambos mundos, mantén el Gateway en un host dedicado y empareja tu laptop como un **nodo** para herramientas locales de pantalla/cámara/exec. Consulta [Nodos](/es/nodes).
  ...
  <Accordion title="¿Cuáles son los requisitos mínimos de VPS y el SO recomendado?">
    ...
    - **Recomendado:** 1-2 vCPU, 2 GB de RAM o más para margen (registros, medios, varios canales). Las herramientas de Node y la automatización del navegador pueden consumir muchos recursos.

    SO: usa **Ubuntu LTS** (o cualquier Debian/Ubuntu moderno). La ruta de instalación de Linux es la mejor probada allí.
  ...
  <Accordion title="¿Puedo ejecutar OpenClaw en una VM y cuáles son los requisitos?">
    ...
    compatibilidad con herramientas. Consulta [Windows](/es/platforms/windows), [Alojamiento VPS](/es/vps).
