---
read_when:
    - Instalación nueva, configuración inicial bloqueada o errores de primera ejecución
    - Elegir autenticación y suscripciones de proveedor
    - No se puede acceder a docs.openclaw.ai, no se puede abrir el panel, la instalación se queda atascada
...
---
  ...
  <Accordion title="Estoy bloqueado, forma más rápida de desbloquearme">
    ...
    en Discord, porque la mayoría de los casos de "estoy bloqueado" son **problemas de configuración local o de entorno** que
  ...
  <Accordion title="Heartbeat sigue omitiéndose. ¿Qué significan los motivos de omisión?">
    ...
    - `empty-heartbeat-file`: `HEARTBEAT.md` existe, pero solo contiene andamiaje en blanco o solo con encabezado
  ...
  <Accordion title="¿Cómo autentico el panel en localhost frente a remoto?">
    ...
    **No en localhost:**
    ...
    - **Tailscale Serve** (recomendado): mantén el bind en local loopback, ejecuta `openclaw gateway --tailscale serve`, abre `https://<magicdns>/`. Si `gateway.auth.allowTailscale` es `true`, los encabezados de identidad satisfacen la autenticación de Control UI/WebSocket (sin pegar un secreto compartido, asume un host Gateway confiable); las API HTTP siguen requiriendo autenticación con secreto compartido, salvo que uses deliberadamente `none` para ingreso privado o autenticación HTTP de proxy confiable.
      Los intentos de autenticación incorrectos concurrentes de Serve desde el mismo cliente se serializan antes de que el limitador de autenticación fallida los registre, por lo que el segundo reintento incorrecto ya puede mostrar `retry later`.
    - **Bind tailnet**: ejecuta `openclaw gateway --bind tailnet --token "<token>"` (o configura autenticación por contraseña), abre `http://<tailscale-ip>:18789/` y luego pega el secreto compartido correspondiente en la configuración del panel.
    - **Proxy inverso con identidad**: mantén el Gateway detrás de un proxy confiable, configura `gateway.auth.mode: "trusted-proxy"` y luego abre la URL del proxy. Los proxies de local loopback en el mismo host requieren `gateway.auth.trustedProxy.allowLoopback = true` explícito.
  ...
  <Accordion title="¿Por qué hay dos configuraciones de aprobación exec para aprobaciones por chat?">
    ...
    - `channels.<channel>.execApprovals`: hace que ese canal actúe como cliente de aprobación nativo para aprobaciones exec

    La política exec del host sigue siendo la verdadera puerta de aprobación. La configuración de chat solo controla dónde
    ...
    - Si un canal nativo compatible puede inferir aprobadores de forma segura, OpenClaw ahora habilita automáticamente aprobaciones nativas con DM primero cuando `channels.<channel>.execApprovals.enabled` no está definido o es `"auto"`.
    - Cuando hay disponibles tarjetas/botones de aprobación nativos, esa UI nativa es la ruta principal; el agente solo debe incluir un comando manual `/approve` si el resultado de la herramienta indica que las aprobaciones por chat no están disponibles o que la aprobación manual es la única ruta.
    ...
    - Las aprobaciones de Plugin son otra cosa separada: usan `/approve` en el mismo chat de forma predeterminada, reenvío opcional con `approvals.plugin`, y solo algunos canales nativos mantienen manejo nativo de aprobaciones de Plugin encima.

    Versión corta: el reenvío sirve para enrutar; la configuración de cliente nativo sirve para una UX más rica y específica del canal.
  ...
  <Accordion title="¿Se ejecuta en Raspberry Pi?">
    ...
    Consejo: una Pi/VPS pequeña puede hospedar el Gateway, y puedes emparejar **nodos** en tu portátil/teléfono para
  ...
  <Accordion title="Se queda bloqueado en wake up my friend / el onboarding no eclosiona. ¿Y ahora qué?">
    Esa pantalla depende de que el Gateway sea accesible y esté autenticado. La TUI también envía
    "Wake up, my friend!" automáticamente en la primera eclosión. Si ves esa línea sin **ninguna respuesta**
    ...
    Si el Gateway es remoto, asegúrate de que el túnel/conexión Tailscale esté activo y de que la UI
    apunte al Gateway correcto. Consulta [Acceso remoto](/es/gateway/remote).

  </Accordion>

  <Accordion title="¿Puedo migrar mi configuración a una máquina nueva (Mac mini) sin repetir el onboarding?">
    ...
    modo remoto, recuerda que el host del gateway es dueño del almacén de sesiones y el workspace.
  ...
  <Accordion title="¿Dónde están las guías de instalación en la nube/VPS?">
    ...
    viven en el servidor, así que trata el host como la fuente de la verdad y haz copias de seguridad.
  ...
  <Accordion title="¿Necesito una suscripción a Claude u OpenAI para ejecutar esto?">
    ...
    **modelos solo locales** para que tus datos permanezcan en tu dispositivo. Las suscripciones (Claude
    ...
    predecible. OAuth de OpenAI Codex está admitido explícitamente para herramientas externas
    como OpenClaw.
  ...
  <Accordion title="¿Admiten autenticación de suscripción a Claude (Claude Pro o Max)?">
    Sí.
...
<AccordionGroup>
  <Accordion title="¿Por qué veo HTTP 429 rate_limit_error de Anthropic?">
    Eso significa que tu **cuota/límite de frecuencia de Anthropic** se agotó para la ventana actual. Si
  ...
  <Accordion title="¿Cómo funciona la autenticación de Codex?">
    ...
    Consulta [Proveedores de modelos](/es/concepts/model-providers) y [Configuración inicial (CLI)](/es/start/wizard).
  </Accordion>
  ...
  <Accordion title="¿Por qué los límites de OAuth de Codex pueden diferir de ChatGPT web?">
    ...
    ambas están vinculadas a la misma cuenta.
  ...
  <Accordion title="¿Tengo que comprar un Mac Mini para instalar esto?">
    ...
    compran uno como host siempre activo, pero un VPS pequeño, un servidor doméstico o un equipo de clase Raspberry Pi también funcionan.

    Solo necesitas un Mac **para herramientas exclusivas de macOS**. Para iMessage, usa [BlueBubbles](/es/channels/bluebubbles) (recomendado): el servidor BlueBubbles se ejecuta en cualquier Mac, y el Gateway puede ejecutarse en Linux o en otro lugar. Si quieres otras herramientas exclusivas de macOS, ejecuta el Gateway en un Mac o empareja un nodo macOS.
  ...
  <Accordion title="¿Debería ejecutar el Gateway en mi portátil o en un VPS?">
    ...
    menor fricción y no te importan la suspensión/reinicios, ejecútalo localmente.
