...
x-i18n:
    generated_at: "2026-05-07T13:19:02Z"
    model: gpt-5.5
    provider: openai
...
3. **Estado del daemon + puerto**
   ...
   Muestra el tiempo de ejecución del supervisor frente a la accesibilidad RPC, la URL objetivo de sondeo y qué configuración probablemente usó el servicio.
...
4. **Sondeos profundos**
   ...
   Ejecuta un sondeo de salud en vivo del gateway, incluidos sondeos de canales cuando son compatibles
...
<AccordionGroup>
  ...
  <Accordion title="Propuesta de valor">
    ...
    - **Código abierto y modificable:** inspecciona, amplía y autoaloja sin quedar atado a un proveedor.
  ...
  <Accordion title="¿Cuáles son los cinco principales casos de uso cotidianos de OpenClaw?">
    Los logros cotidianos suelen verse así:
  ...
  <Accordion title="¿Puede OpenClaw ayudar con generación de leads, contacto, anuncios y blogs para un SaaS?">
    ...
    resumir prospectos y escribir borradores de contacto o textos publicitarios.

    Para **campañas de contacto o anuncios**, mantén a una persona en el circuito. Evita el spam, cumple las leyes locales y
    las políticas de las plataformas, y revisa todo antes de que se envíe. El patrón más seguro es dejar que
  ...
  <Accordion title="¿Cuáles son las ventajas frente a Claude Code para desarrollo web?">
    ...
    Claude Code o Codex para el ciclo de programación directa más rápido dentro de un repo. Usa OpenClaw cuando
...
<AccordionGroup>
  <Accordion title="¿Cómo personalizo Skills sin mantener el repo con cambios sucios?">
    Usa sobrescrituras gestionadas en lugar de editar la copia del repo. Coloca tus cambios en `~/.openclaw/skills/<name>/SKILL.md` (o añade una carpeta mediante `skills.load.extraDirs` en `~/.openclaw/openclaw.json`). La precedencia es `<workspace>/skills` → `<workspace>/.agents/skills` → `~/.agents/skills` → `~/.openclaw/skills` → incluidas → `skills.load.extraDirs`, así que las sobrescrituras gestionadas siguen ganando sobre las skills incluidas sin tocar git. Si necesitas que la skill se instale globalmente pero solo sea visible para algunos agentes, mantén la copia compartida en `~/.openclaw/skills` y controla la visibilidad con `agents.defaults.skills` y `agents.list[].skills`. Solo las ediciones dignas de upstream deberían vivir en el repo y salir como PR.
  </Accordion>
  ...
  <Accordion title="Un subagente terminó, pero la actualización de finalización fue al lugar equivocado o nunca se publicó. ¿Qué debería comprobar?">
    ...
    - La entrega de subagente en modo finalización prefiere cualquier hilo vinculado o ruta de conversación cuando existe.
    ...
    - Los destinos inválidos o obsoletos aún pueden forzar el recurso a cola o el fallo final de entrega.
    ...
    - Si el hijo agotó el tiempo después de solo llamadas a herramientas, el anuncio puede condensarlo en un breve resumen de progreso parcial en lugar de reproducir la salida sin procesar de las herramientas.
  ...
  <Accordion title="Cron se ejecutó, pero no se envió nada al canal. ¿Por qué?">
    ...
    - Un resultado aislado silencioso (solo `NO_REPLY` / `no_reply`) se trata como intencionalmente no entregable, así que el ejecutor también suprime la entrega de respaldo en cola.
  ...
  <Accordion title="¿Cómo instalo Skills en Linux?">
    Usa comandos nativos `openclaw skills` o coloca Skills en tu espacio de trabajo. La interfaz de Skills de macOS no está disponible en Linux.
  ...
  <Accordion title="¿Puedo ejecutar Skills exclusivas de Apple macOS desde Linux?">
    ...
    **Opción B - usa un nodo macOS (sin SSH).**
    Ejecuta el Gateway en Linux, empareja un nodo macOS (app de la barra de menús) y configura **Comandos de ejecución de Node** como "Preguntar siempre" o "Permitir siempre" en la Mac. OpenClaw puede tratar las Skills exclusivas de macOS como elegibles cuando los binarios requeridos existen en el nodo. El agente ejecuta esas Skills mediante la herramienta `nodes`. Si eliges "Preguntar siempre", aprobar "Permitir siempre" en el prompt añade ese comando a la lista de permitidos.

    **Opción C - proxy de binarios de macOS por SSH (avanzado).**
    Mantén el Gateway en Linux, pero haz que los binarios de CLI requeridos se resuelvan a wrappers SSH que se ejecutan en una Mac. Luego anula la Skill para permitir Linux, de modo que siga siendo elegible.
  ...
  <Accordion title="¿Tienen una integración con Notion o HeyGen?">
    ...
    Las instalaciones nativas terminan en el directorio `skills/` del espacio de trabajo activo. Para Skills compartidas entre agentes, colócalas en `~/.openclaw/skills/<name>/SKILL.md`. Si solo algunos agentes deberían ver una instalación compartida, configura `agents.defaults.skills` o `agents.list[].skills`. Algunas Skills esperan binarios instalados mediante Homebrew; en Linux eso significa Linuxbrew (consulta la entrada de preguntas frecuentes de Homebrew Linux arriba). Consulta [Skills](/es/tools/skills), [Configuración de Skills](/es/tools/skills-config) y [ClawHub](/es/tools/clawhub).
  ...
  <Accordion title="¿Cómo uso mi Chrome ya autenticado con OpenClaw?">
    ...
    - las acciones están guiadas por referencias, no por selectores CSS
    - las cargas requieren `ref` / `inputRef` y actualmente admiten un archivo a la vez
    - `responsebody`, exportación PDF, interceptación de descargas y acciones por lotes todavía necesitan un navegador administrado o un perfil CDP sin procesar
...
<AccordionGroup>
  ...
  <Accordion title="Docker parece limitado - ¿cómo habilito todas las funciones?">
    ...
    - Configura `PLAYWRIGHT_BROWSERS_PATH` y asegúrate de que la ruta se conserve.
  ...
  <Accordion title="¿Cómo enlazo una carpeta del host en el aislamiento?">
    Configura `agents.defaults.sandbox.docker.binds` como `["host:path:mode"]` (p. ej., `"/home/user/src:/src:ro"`). Los enlaces globales y por agente se fusionan; los enlaces por agente se ignoran cuando `scope: "shared"`. Usa `:ro` para cualquier cosa sensible y recuerda que los enlaces eluden las paredes del sistema de archivos del aislamiento.

    OpenClaw valida las fuentes de enlace contra la ruta normalizada y la ruta canónica resuelta a través del ancestro existente más profundo. Eso significa que los escapes por padre con symlink siguen fallando de forma cerrada incluso cuando el último segmento de ruta todavía no existe, y las comprobaciones de raíz permitida siguen aplicándose después de la resolución de symlinks.
  ...
  <Accordion title="La memoria sigue olvidando cosas. ¿Cómo hago que se conserve?">
  ...
  <Accordion title="¿La memoria persiste para siempre? ¿Cuáles son los límites?">
    ...
    del modelo, así que las conversaciones largas pueden compactarse o truncarse. Por eso existe
    la búsqueda en memoria: trae de vuelta al contexto solo las partes relevantes.
  ...
  <Accordion title="¿La búsqueda semántica de memoria requiere una clave de API de OpenAI?">
    ...
    **no** concede acceso a embeddings, así que **iniciar sesión con Codex (OAuth o el
    ...
    resuelve una clave de Gemini, luego Voyage y después Mistral. Si no hay ninguna clave remota disponible, la búsqueda de
...
<AccordionGroup>
  ...
  <Accordion title="¿Dónde deberían vivir AGENTS.md / SOUL.md / USER.md / MEMORY.md?">
    ...
    Si el bot "olvida" después de reiniciar, confirma que el Gateway esté usando el mismo
  ...
  <Accordion title="Estrategia de copia de seguridad recomendada">
    ...
    **No** hagas commit de nada bajo `~/.openclaw` (credenciales, sesiones, tokens o cargas de secretos cifradas).
...
<AccordionGroup>
  ...
  <Accordion title='Configuré gateway.bind: "lan" (o "tailnet") y ahora nada escucha / la UI dice no autorizado'>
    ...
    - `gateway.auth.mode: "trusted-proxy"` detrás de un proxy inverso consciente de identidad configurado correctamente
    ...
    - Si `gateway.auth.token` / `gateway.auth.password` está configurado explícitamente mediante SecretRef y no se resuelve, la resolución falla cerrada (sin enmascaramiento por respaldo remoto).
  ...
  <Accordion title="¿Por qué necesito un token en localhost ahora?">
    OpenClaw aplica autenticación del gateway de forma predeterminada, incluido loopback. En la ruta predeterminada normal, eso significa autenticación por token: si no se configura una ruta de autenticación explícita, el inicio del gateway se resuelve en modo token y genera un token solo en tiempo de ejecución para ese inicio, así que **los clientes WS locales deben autenticarse**. Configura `gateway.auth.token`, `gateway.auth.password`, `OPENCLAW_GATEWAY_TOKEN` o `OPENCLAW_GATEWAY_PASSWORD` explícitamente cuando los clientes necesiten un secreto estable entre reinicios. Esto bloquea que otros procesos locales llamen al Gateway.

    Si prefieres una ruta de autenticación diferente, puedes elegir explícitamente el modo contraseña (o, para proxies inversos conscientes de identidad, `trusted-proxy`). Si **realmente** quieres loopback abierto, configura `gateway.auth.mode: "none"` explícitamente en tu configuración. Doctor puede generarte un token en cualquier momento: `openclaw doctor --generate-gateway-token`.
  ...
  <Accordion title="config.apply borró mi configuración. ¿Cómo la recupero y evito que ocurra?">
    ...
    - Si usas la herramienta `gateway` solo para propietarios desde una ejecución de agente, seguirá rechazando escrituras en `tools.exec.ask` / `tools.exec.security` (incluidos los alias heredados `tools.bash.*` que se normalizan a las mismas rutas exec protegidas).
  ...
  <Accordion title="¿Puede ejecutarse sin interfaz el navegador de OpenClaw?">
    ...
    El valor predeterminado es `false` (con interfaz). El modo sin interfaz tiene más probabilidades de activar comprobaciones antibot en algunos sitios. Consulta [Navegador](/es/tools/browser).

    El modo sin interfaz usa el **mismo motor Chromium** y funciona para la mayoría de la automatización (formularios, clics, extracción, inicios de sesión). Las diferencias principales:

    - No hay ventana de navegador visible (usa capturas de pantalla si necesitas elementos visuales).
    - Algunos sitios son más estrictos con la automatización en modo sin interfaz (CAPTCHA, antibot).
...
<AccordionGroup>
  <Accordion title="¿Cómo se propagan los comandos entre Telegram, el gateway y los Nodes?">
    ...
    solo después llama a los Nodes mediante el **Gateway WebSocket** cuando se necesita una herramienta de Node:
  ...
  <Accordion title="¿Cómo puede mi agente acceder a mi computadora si el Gateway está alojado de forma remota?">
    Respuesta breve: **empareja tu computadora como un Node**. El Gateway se ejecuta en otro lugar, pero puede
    ...
    4. Abre la app de macOS localmente y conéctate en modo **Remoto mediante SSH** (o tailnet directa)
    ...
    Documentación: [Nodes](/es/nodes), [Protocolo de Gateway](/es/gateway/protocol), [Modo remoto de macOS](/es/platforms/mac/remote), [Seguridad](/es/gateway/security).
  ...
  <Accordion title="¿Pueden comunicarse dos instancias de OpenClaw entre sí (local + VPS)?">
    ...
    **Puente CLI (genérico):** ejecuta un script que llame al otro Gateway con
    ...
    Consejo: agrega una medida de seguridad para que los dos bots no entren en bucle interminablemente (solo menciones, listas
  ...
  <Accordion title="¿Hay algún beneficio en usar un Node en mi portátil personal en lugar de SSH desde un VPS?">
    Sí: los Nodes son la forma principal de llegar a tu portátil desde un Gateway remoto y
    ...
    común es un host siempre encendido más tu portátil como Node.
    ...
    - **Automatización de navegador local.** Mantén el Gateway en un VPS, pero ejecuta Chrome localmente mediante un host de Node en el portátil, o adjunta Chrome local en el host mediante Chrome MCP.

    SSH está bien para acceso de shell ad hoc, pero los Nodes son más simples para flujos de trabajo continuos de agentes y
  ...
  <Accordion title="¿Los Nodes ejecutan un servicio de gateway?">
    ...
    al gateway (Nodes de iOS/Android, o "modo Node" de macOS en la app de la barra de menú). Para hosts de Node sin interfaz
    y control CLI, consulta [CLI de host de Node](/es/cli/node).

    Se requiere un reinicio completo para cambios en la superficie de `gateway`, `discovery` y Plugin alojado.
  ...
  <Accordion title="¿Hay una forma API / RPC de aplicar la configuración?">
    ...
    - `config.schema.lookup`: inspecciona un subárbol de configuración con su nodo de esquema superficial, la sugerencia de UI coincidente y resúmenes inmediatos de hijos antes de escribir
    ...
    - La herramienta de runtime `gateway`, solo para propietarios, sigue negándose a reescribir `tools.exec.ask` / `tools.exec.security`; los alias heredados `tools.bash.*` se normalizan a las mismas rutas de exec protegidas
  ...
  <Accordion title="¿Cómo conecto un Node de Mac a un Gateway remoto (Tailscale Serve)?">
    Serve expone la **UI de Control del Gateway + WS**. Los Nodes se conectan mediante el mismo endpoint WS del Gateway.
  ...
  <Accordion title="¿Debería instalar en una segunda laptop o solo añadir un Node?">
    Si solo necesitas **herramientas locales** (pantalla/cámara/exec) en la segunda laptop, añádela como un
    **Node**. Eso mantiene un único Gateway y evita configuración duplicada. Las herramientas de Node local son
...
<AccordionGroup>
  ...
  <Accordion title='Configuré COPILOT_GITHUB_TOKEN, pero el estado de modelos muestra "Shell env: off." ¿Por qué?'>
    ...
    **no** significa que falten tus variables de entorno; solo significa que OpenClaw no cargará
    ...
    Luego reinicia el gateway y comprueba de nuevo:
...
<AccordionGroup>
  ...
  <Accordion title="¿Las sesiones se restablecen automáticamente si nunca envío /new?">
    ...
    Configúralo con un valor positivo para habilitar la caducidad por inactividad. Cuando está habilitada, el **siguiente**
  ...
  <Accordion title="¿Por qué se truncó el contexto en mitad de la tarea? ¿Cómo lo evito?">
  ...
  <Accordion title='¿Por qué veo "LLM request rejected: messages.content.tool_use.input field required"?'>
    ...
    Corrección: inicia una sesión nueva con `/new` (mensaje independiente).
  ...
  <Accordion title="¿Por qué recibo mensajes de Heartbeat cada 30 minutos?">
    ...
    Si falta el archivo, Heartbeat aún se ejecuta y el modelo decide qué hacer.
  ...
  <Accordion title="¿Por qué OpenClaw no responde en un grupo?">
    ...
    - El control por mención está activado (predeterminado). Debes @mencionar al bot (o coincidir con `mentionPatterns`).
  ...
  <Accordion title="¿Cuántos workspaces y agentes puedo crear?">
    ...
    - Usa `openclaw doctor` para detectar workspaces sueltos e incompatibilidades de perfil.
  ...
  <Accordion title="¿Puedo ejecutar varios bots o chats al mismo tiempo (Slack), y cómo debería configurarlo?">
    ...
    El acceso al navegador es potente, pero no puede "hacer cualquier cosa que pueda hacer una persona": los sistemas anti-bot, los CAPTCHAs y MFA
    aún pueden bloquear la automatización. Para el control del navegador más fiable, usa Chrome MCP local en el host,
...
<AccordionGroup>
  ...
  <Accordion title='¿Por qué openclaw gateway status muestra "Config (cli)" y "Config (service)" diferentes?'>
    Estás editando un archivo de configuración mientras el servicio ejecuta otro (a menudo una incompatibilidad de `--profile` / `OPENCLAW_STATE_DIR`).
  ...
  <Accordion title='¿Qué significa "another gateway instance is already listening"?'>
    OpenClaw aplica un bloqueo de runtime enlazando el listener WebSocket inmediatamente al inicio (predeterminado `ws://127.0.0.1:18789`). Si el enlace falla con `EADDRINUSE`, lanza `GatewayLockError`, lo que indica que otra instancia ya está escuchando.
  ...
  <Accordion title='Control UI dice "unauthorized" (o sigue reconectándose). ¿Qué hago ahora?'>
    ...
    - Control UI mantiene el token en `sessionStorage` para la sesión actual de la pestaña del navegador y la URL de gateway seleccionada, por lo que las actualizaciones en la misma pestaña siguen funcionando sin restaurar la persistencia de tokens de larga duración en localStorage.
    ...
    - Ese reintento con token en caché ahora reutiliza los ámbitos aprobados en caché almacenados con el token de dispositivo. Los llamadores con `deviceToken` explícito / `scopes` explícitos siguen conservando su conjunto de ámbitos solicitados en lugar de heredar ámbitos en caché.
    ...
    - Las comprobaciones de ámbito del token de bootstrap tienen prefijo de rol. La allowlist de operador de bootstrap integrada solo satisface solicitudes de operador; los nodos u otros roles no operadores siguen necesitando ámbitos bajo su propio prefijo de rol.

    Solución:

    - Más rápido: `openclaw dashboard` (imprime + copia la URL del dashboard, intenta abrirla; muestra una pista de SSH si no hay pantalla).
    ...
    - Modo proxy de confianza: asegúrate de entrar mediante el proxy configurado con reconocimiento de identidad, no con una URL de gateway sin procesar. Los proxies local loopback del mismo host también necesitan `gateway.auth.trustedProxy.allowLoopback = true`.
    - Si la incompatibilidad persiste después del único reintento, rota/vuelve a aprobar el token de dispositivo emparejado:
      - `openclaw devices list`
  ...
  <Accordion title="Configuré gateway.bind en tailnet, pero no puede enlazar y nada escucha">
    El enlace `tailnet` elige una IP de Tailscale de tus interfaces de red (100.64.0.0/10). Si la máquina no está en Tailscale (o la interfaz está inactiva), no hay nada a lo que enlazarse.
  ...
  <Accordion title="¿Puedo ejecutar varios Gateways en el mismo host?">
    Normalmente no: un Gateway puede ejecutar varios canales de mensajería y agentes. Usa varios Gateways solo cuando necesites redundancia (por ejemplo, un bot de rescate) o aislamiento estricto.
...
<AccordionGroup>
  ...
  <Accordion title="El Gateway está activo pero las respuestas nunca llegan. ¿Qué debería revisar?">
    ...
    - Autenticación del modelo no cargada en el **host del gateway** (comprueba `models status`).
  ...
  <Accordion title='"Desconectado del gateway: sin motivo" - ¿y ahora qué?'>
    ...
    4. Si es remoto, ¿el enlace del túnel/Tailscale está activo?
  ...
  <Accordion title="Telegram setMyCommands falla. ¿Qué debo comprobar?">
    ...
    - `BOT_COMMANDS_TOO_MUCH`: el menú de Telegram tiene demasiadas entradas. OpenClaw ya recorta hasta el límite de Telegram y vuelve a intentar con menos comandos, pero aún hay que eliminar algunas entradas del menú. Reduce comandos de plugin/skill/personalizados, o desactiva `channels.telegram.commands.native` si no necesitas el menú.
  ...
  <Accordion title="Explícamelo de forma sencilla: openclaw gateway restart vs openclaw gateway">
  ...
  <Accordion title="La forma más rápida de obtener más detalles cuando algo falla">
    Inicia el Gateway con `--verbose` para obtener más detalle en la consola. Luego inspecciona el archivo de registro para ver errores de autenticación del canal, enrutamiento de modelos y RPC.
  </Accordion>
</AccordionGroup>
...
<AccordionGroup>
  <Accordion title="Mi skill generó una imagen/PDF, pero no se envió nada">
    ...
    - `tools.fs.workspaceOnly=true` mantiene los envíos con rutas locales limitados al workspace, temp/media-store y archivos validados por sandbox.
...
<AccordionGroup>
  <Accordion title="¿Es seguro exponer OpenClaw a DM entrantes?">
    ...
    - Abrir los DM públicamente requiere una activación explícita (`dmPolicy: "open"` y lista de permitidos `"*"`).
  ...
  <Accordion title="¿La inyección de prompts solo preocupa en bots públicos?">
    No. La inyección de prompts trata de **contenido no confiable**, no solo de quién puede enviar DM al bot.
    ...
    El mayor riesgo aparece cuando las herramientas están activadas: se puede engañar al modelo para
    ...
    - tratar también el texto decodificado de archivos/documentos como no confiable: OpenResponses
      ...
      marcadores explícitos de límite de contenido externo en vez de pasar el texto sin procesar del archivo
  ...
  <Accordion title="¿Mi bot debería tener su propio correo electrónico, cuenta de GitHub o número de teléfono?">
    ...
    Empieza pequeño. Da acceso solo a las herramientas y cuentas que realmente necesitas, y amplía
  ...
  <Accordion title="¿Puedo darle autonomía sobre mis mensajes de texto y es seguro?">
    **No** recomendamos autonomía completa sobre tus mensajes personales. El patrón más seguro es:
    ...
    - Mantener los DM en **modo de emparejamiento** o una lista de permitidos estricta.
  ...
  <Accordion title="¿Puedo usar modelos más baratos para tareas de asistente personal?">
    ...
    o cuando lean contenido no confiable. Si debes usar un modelo más pequeño, bloquea
  ...
  <Accordion title="WhatsApp: ¿enviará mensajes a mis contactos? ¿Cómo funciona el emparejamiento?">
    ...
    Prompt de número de teléfono del asistente: se usa para configurar tu **lista de permitidos/propietario** para que tus propios DM estén permitidos. No se usa para envíos automáticos. Si lo ejecutas en tu número personal de WhatsApp, usa ese número y activa `channels.whatsapp.selfChatMode`.
...
<AccordionGroup>
  <Accordion title="¿Cómo evito que los mensajes internos del sistema aparezcan en el chat?">
    ...
    Si sigue habiendo demasiado ruido, comprueba la configuración de sesión en la interfaz de control y establece verbose
    en **heredar**. Confirma también que no estás usando un perfil de bot con `verboseDefault` configurado
  ...
  <Accordion title="¿Cómo detengo/cancelo una tarea en ejecución?">
    ...
    Estos son disparadores de cancelación (no comandos de barra).
  ...
  <Accordion title='¿Por qué parece que el bot "ignora" mensajes enviados muy rápido?'>
    ...
    - `followup` - ejecuta mensajes uno por uno
...
- [Solución de problemas](/es/help/troubleshooting) — diagnóstico por síntoma
