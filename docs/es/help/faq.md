---
read_when:
    - Responder preguntas comunes de configuración, instalación, incorporación o soporte en tiempo de ejecución
...
1. **Estado rápido (primera comprobación)**
   ...
   Resumen local rápido: SO + actualización, accesibilidad del gateway/servicio, agentes/sesiones, configuración de proveedores + problemas de runtime (cuando el Gateway es accesible).
...
4. **Sondas profundas**
   ...
   Ejecuta una sonda de salud del Gateway en vivo, incluidas sondas de canales cuando son compatibles
...
<AccordionGroup>
  ...
  <Accordion title="Propuesta de valor">
    ...
    - **Código abierto y modificable:** inspecciona, extiende y autoaloja sin dependencia de un proveedor.
  ...
  <Accordion title="Acabo de configurarlo: ¿qué debería hacer primero?">
    ...
    - Crear un prototipo de una aplicación móvil (esquema, pantallas, plan de API).
  ...
  <Accordion title="¿Puede OpenClaw ayudar con generación de leads, contacto comercial, anuncios y blogs para un SaaS?">
    Sí para **investigación, cualificación y redacción**. Puede escanear sitios, crear listas cortas,
    resumir prospectos y escribir borradores de mensajes de contacto o textos publicitarios.
...
<AccordionGroup>
  <Accordion title="¿Cómo personalizo Skills sin mantener el repositorio sucio?">
    Usa sobrescrituras gestionadas en lugar de editar la copia del repositorio. Coloca tus cambios en `~/.openclaw/skills/<name>/SKILL.md` (o agrega una carpeta mediante `skills.load.extraDirs` en `~/.openclaw/openclaw.json`). La precedencia es `<workspace>/skills` → `<workspace>/.agents/skills` → `~/.agents/skills` → `~/.openclaw/skills` → incluidas → `skills.load.extraDirs`, por lo que las sobrescrituras gestionadas siguen teniendo prioridad sobre las skills incluidas sin tocar git. Si necesitas que la skill esté instalada globalmente pero solo sea visible para algunos agentes, mantén la copia compartida en `~/.openclaw/skills` y controla la visibilidad con `agents.defaults.skills` y `agents.list[].skills`. Solo las ediciones aptas para upstream deberían vivir en el repositorio y salir como PRs.
  </Accordion>
  ...
  <Accordion title="¿Cómo puedo usar modelos diferentes para tareas diferentes?">
  ...
  <Accordion title="El bot se congela mientras hace trabajo pesado. ¿Cómo lo descargo?">
    ...
    Consejo sobre tokens: las tareas largas y los subagentes consumen tokens. Si te preocupa el coste, establece un
  ...
  <Accordion title="¿Cómo funcionan las sesiones de subagente vinculadas a hilos en Discord?">
    Usa vinculaciones de hilos. Puedes vincular un hilo de Discord a un objetivo de subagente o sesión para que los mensajes de seguimiento en ese hilo permanezcan en esa sesión vinculada.
  ...
  <Accordion title="Un subagente terminó, pero la actualización de finalización fue al lugar equivocado o nunca se publicó. ¿Qué debería comprobar?">
    ...
    - Si el origen de finalización solo incluye un canal, OpenClaw recurre a la ruta almacenada de la sesión solicitante (`lastChannel` / `lastTo` / `lastAccountId`) para que la entrega directa todavía pueda funcionar.
    - Si no existe una ruta vinculada ni una ruta almacenada utilizable, la entrega directa puede fallar y el resultado vuelve a entrega de sesión en cola en lugar de publicarse inmediatamente en el chat.
    - Los objetivos inválidos u obsoletos aún pueden forzar la alternativa de cola o un fallo final de entrega.
    ...
    - Si el hijo agotó el tiempo después de solo llamadas a herramientas, el anuncio puede contraerlo en un breve resumen de progreso parcial en lugar de reproducir la salida sin procesar de la herramienta.
  ...
  <Accordion title="Cron se ejecutó, pero no se envió nada al canal. ¿Por qué?">
    ...
    - Los errores de autenticación del canal (`unauthorized`, `Forbidden`) significan que el ejecutor intentó entregar el mensaje, pero las credenciales lo bloquearon.
    ...
    del ejecutor para el texto final que el agente no haya enviado ya.
  ...
  <Accordion title="¿Por qué una ejecución Cron aislada cambió de modelos o reintentó una vez?">
    ...
    proveedor/modelo cambiado y, si el cambio traía una nueva anulación de perfil de autenticación, Cron
  ...
  <Accordion title="¿Puede OpenClaw ejecutar tareas de forma programada o continuamente en segundo plano?">
    ...
    - **Trabajos aislados** para agentes autónomos que publican resúmenes o entregan mensajes a chats.
  ...
  <Accordion title="¿Puedo ejecutar Skills solo para Apple macOS desde Linux?">
    ...
    **Opción B - usar un nodo macOS (sin SSH).**
    Ejecuta el Gateway en Linux, empareja un nodo macOS (app de barra de menús) y establece **Comandos de ejecución de Node** en "Preguntar siempre" o "Permitir siempre" en la Mac. OpenClaw puede tratar las Skills solo para macOS como elegibles cuando los binarios requeridos existen en el nodo. El agente ejecuta esas Skills mediante la herramienta `nodes`. Si eliges "Preguntar siempre", aprobar "Permitir siempre" en el prompt añade ese comando a la lista de permitidos.

    **Opción C - usar un proxy para binarios de macOS mediante SSH (avanzado).**
    Mantén el Gateway en Linux, pero haz que los binarios de CLI requeridos se resuelvan como wrappers SSH que se ejecutan en una Mac. Luego anula la Skill para permitir Linux, de modo que siga siendo elegible.
  ...
  <Accordion title="¿Tienen una integración con Notion o HeyGen?">
    ...
    Las instalaciones nativas se colocan en el directorio `skills/` del espacio de trabajo activo. Para Skills compartidas entre agentes, colócalas en `~/.openclaw/skills/<name>/SKILL.md`. Si solo algunos agentes deben ver una instalación compartida, configura `agents.defaults.skills` o `agents.list[].skills`. Algunas Skills esperan binarios instalados mediante Homebrew; en Linux eso significa Linuxbrew (consulta la entrada de las preguntas frecuentes de Homebrew para Linux anterior). Consulta [Skills](/es/tools/skills), [Configuración de Skills](/es/tools/skills-config) y [ClawHub](/es/tools/clawhub).
  ...
  <Accordion title="¿Cómo uso mi Chrome con sesión iniciada existente con OpenClaw?">
    ...
    Esta ruta puede usar el navegador del host local o un nodo de navegador conectado. Si el Gateway se ejecuta en otro lugar, ejecuta un host de nodo en la máquina del navegador o usa CDP remoto en su lugar.
...
<AccordionGroup>
  ...
  <Accordion title="¿Cómo vinculo una carpeta del host al aislamiento?">
    Establece `agents.defaults.sandbox.docker.binds` en `["host:path:mode"]` (por ejemplo, `"/home/user/src:/src:ro"`). Los enlaces globales y por agente se fusionan; los enlaces por agente se ignoran cuando `scope: "shared"`. Usa `:ro` para cualquier cosa sensible y recuerda que los enlaces omiten los muros del sistema de archivos del aislamiento.

    OpenClaw valida las fuentes de enlace contra la ruta normalizada y la ruta canónica resuelta mediante el ancestro existente más profundo. Eso significa que los escapes por padres con enlaces simbólicos siguen fallando de forma cerrada incluso cuando el último segmento de la ruta aún no existe, y las comprobaciones de raíz permitida siguen aplicándose después de resolver enlaces simbólicos.
  ...
  <Accordion title="La memoria sigue olvidando cosas. ¿Cómo hago que se conserven?">
    Pide al bot que **escriba el hecho en memoria**. Las notas de largo plazo van en `MEMORY.md`,
    el contexto de corto plazo va en `memory/YYYY-MM-DD.md`.

    Esta sigue siendo un área que estamos mejorando. Ayuda recordar al modelo que guarde memorias;
  ...
  <Accordion title="¿La memoria persiste para siempre? ¿Cuáles son los límites?">
    ...
    existe la búsqueda en memoria: vuelve a incorporar al contexto solo las partes relevantes.
  ...
  <Accordion title="¿La búsqueda semántica en memoria requiere una clave de API de OpenAI?">
    ...
    **no** concede acceso a embeddings, así que **iniciar sesión con Codex (OAuth o el
    inicio de sesión de Codex CLI)** no ayuda con la búsqueda semántica en memoria. Los embeddings de OpenAI
    ...
    resuelve una clave de Gemini, luego Voyage y luego Mistral. Si no hay ninguna clave remota disponible, la búsqueda en memoria
...
<AccordionGroup>
  ...
  <Accordion title="¿Dónde deberían vivir AGENTS.md / SOUL.md / USER.md / MEMORY.md?">
    ...
    Si el bot "olvida" después de reiniciar, confirma que el Gateway está usando el mismo
  ...
  <Accordion title="Estrategia de copia de seguridad recomendada">
    Pon tu **espacio de trabajo del agente** en un repositorio git **privado** y hazle una copia de seguridad en algún lugar
...
<AccordionGroup>
  ...
  <Accordion title='Definí gateway.bind: "lan" (o "tailnet") y ahora nada escucha / la UI dice no autorizado'>
    ...
    - `gateway.auth.mode: "trusted-proxy"` detrás de un proxy inverso correctamente configurado y consciente de identidad
    ...
    - Para autenticación con contraseña, define `gateway.auth.mode: "password"` junto con `gateway.auth.password` (o `OPENCLAW_GATEWAY_PASSWORD`) en su lugar.
  ...
  <Accordion title="¿Cómo habilito la búsqueda web (y la obtención web)?">
    ...
    **Recomendado:** ejecuta `openclaw configure --section web` y elige un proveedor.
    ...
    La configuración de respaldo de captura web de Firecrawl vive en `plugins.entries.firecrawl.config.webFetch.*`.
    ...
    - Si se omite `tools.web.fetch.provider`, OpenClaw detecta automáticamente el primer proveedor de respaldo de captura listo a partir de las credenciales disponibles. Hoy el proveedor incluido es Firecrawl.
  ...
  <Accordion title="config.apply borró mi configuración. ¿Cómo la recupero y evito que ocurra?">
    ...
    - Usa `config.schema.lookup` primero cuando no estés seguro de una ruta exacta o de la forma de un campo; devuelve un nodo de esquema superficial más resúmenes de hijos inmediatos para profundizar.
    - Usa `config.patch` para ediciones RPC parciales; reserva `config.apply` solo para reemplazos de configuración completa.
    - Si estás usando la herramienta `gateway` solo para propietarios desde una ejecución de agente, aún rechazará escrituras a `tools.exec.ask` / `tools.exec.security` (incluidos los alias heredados `tools.bash.*` que se normalizan a las mismas rutas exec protegidas).
  ...
  <Accordion title="¿Puede el navegador de OpenClaw ejecutarse sin interfaz gráfica?">
    ...
    El valor predeterminado es `false` (con interfaz). El modo sin interfaz gráfica tiene más probabilidades de activar comprobaciones antibot en algunos sitios. Consulta [Navegador](/es/tools/browser).
...
<AccordionGroup>
  <Accordion title="¿Cómo se propagan los comandos entre Telegram, el gateway y los nodos?">
    ...
    solo entonces llama a los nodos por el **WebSocket del Gateway** cuando se necesita una herramienta de nodo:
  ...
  <Accordion title="¿Cómo puede mi agente acceder a mi computadora si el Gateway está alojado remotamente?">
    ...
    2. Coloca el host del Gateway + tu computadora en la misma tailnet.
    ...
    No se requiere un puente TCP separado; los nodos se conectan por el WebSocket del Gateway.
  ...
  <Accordion title="¿Pueden dos instancias de OpenClaw hablar entre sí (local + VPS)?">
    ...
    **Puente CLI (genérico):** ejecuta un script que llame al otro Gateway con
    ...
    Consejo: añade una protección para que los dos bots no entren en un bucle infinito (solo con mención, listas
  ...
  <Accordion title="¿Necesito VPS separados para varios agentes?">
    ...
    distintas que no quieras compartir. De lo contrario, mantén un Gateway y
  ...
  <Accordion title="¿Hay alguna ventaja en usar un nodo en mi portátil personal en lugar de SSH desde un VPS?">
    Sí: los nodos son la forma de primera clase de alcanzar tu portátil desde un Gateway remoto, y
    desbloquean más que acceso de shell. El Gateway se ejecuta en macOS/Linux (Windows mediante WSL2) y es
    ligero (un VPS pequeño o una máquina de clase Raspberry Pi está bien; 4 GB de RAM son suficientes), por lo que una configuración
    común es un host siempre activo más tu portátil como nodo.

    - **No requiere SSH entrante.** Los nodos se conectan hacia el WebSocket del Gateway y usan emparejamiento de dispositivos.
    - **Controles de ejecución más seguros.** `system.run` está protegido por listas de permitidos/aprobaciones de nodo en ese portátil.
    - **Más herramientas de dispositivo.** Los nodos exponen `canvas`, `camera` y `screen` además de `system.run`.
    - **Automatización local del navegador.** Mantén el Gateway en un VPS, pero ejecuta Chrome localmente mediante un host de nodo en el portátil, o conéctate a Chrome local en el host mediante Chrome MCP.
  ...
  <Accordion title="¿Los nodos ejecutan un servicio de gateway?">
    ...
    Se requiere un reinicio completo para cambios de `gateway`, `discovery` y `canvasHost`.
  ...
  <Accordion title="¿Hay una forma API / RPC de aplicar configuración?">
    ...
    - La herramienta de runtime `gateway` solo para propietarios todavía se niega a reescribir `tools.exec.ask` / `tools.exec.security`; los alias heredados `tools.bash.*` se normalizan a las mismas rutas exec protegidas
  ...
  <Accordion title="¿Cómo conecto un nodo Mac a un Gateway remoto (Tailscale Serve)?">
    ...
    2. **Usa la aplicación macOS en modo remoto** (el destino SSH puede ser el nombre de host de la tailnet).
       La aplicación tunelizará el puerto del Gateway y se conectará como nodo.
  ...
  <Accordion title="¿Debo instalarlo en un segundo portátil o solo agregar un nodo?">
    Si solo necesitas **herramientas locales** (pantalla/cámara/ejecución) en el segundo portátil, agrégalo como
...
<AccordionGroup>
  <Accordion title="¿Cómo carga OpenClaw las variables de entorno?">
    ...
    También puedes definir variables de entorno en línea en la configuración (aplicadas solo si faltan en el entorno del proceso):
  ...
  <Accordion title="Inicié el Gateway mediante el servicio y mis variables de entorno desaparecieron. ¿Qué hago ahora?">
    ...
    1. Pon las claves faltantes en `~/.openclaw/.env` para que se recojan incluso cuando el servicio no herede tu entorno de shell.
  ...
  <Accordion title='Configuré COPILOT_GITHUB_TOKEN, pero el estado de modelos muestra "Shell env: off." ¿Por qué?'>
    ...
    **no** significa que falten tus variables de entorno; solo significa que OpenClaw no cargará
    ...
    de shell. Arréglalo haciendo una de estas cosas:
...
<AccordionGroup>
  ...
  <Accordion title="¿Por qué se truncó el contexto a mitad de tarea? ¿Cómo lo evito?">
    ...
    archivos pueden activar compactación o truncamiento.
  ...
  <Accordion title='Recibo errores de "context too large"; ¿cómo restablezco o compacto?'>
    ...
    - **Compactar** (conserva la conversación pero resume turnos anteriores):
    ...
    - Habilita o ajusta la **poda de sesiones** (`agents.defaults.contextPruning`) para recortar salidas antiguas de herramientas.
  ...
  <Accordion title="¿Por qué recibo mensajes de heartbeat cada 30 minutos?">
    Los Heartbeats se ejecutan cada **30m** de forma predeterminada (**1h** al usar autenticación OAuth). Ajústalos o deshabilítalos:
  ...
  <Accordion title='¿Necesito agregar una "cuenta de bot" a un grupo de WhatsApp?'>
    ...
    De forma predeterminada, las respuestas en grupo están bloqueadas hasta que permites remitentes (`groupPolicy: "allowlist"`).
  ...
  <Accordion title="¿Los grupos/hilos comparten contexto con los MD?">
    Los chats directos se fusionan con la sesión principal de forma predeterminada. Los grupos/canales tienen sus propias claves de sesión, y los temas de Telegram / hilos de Discord son sesiones separadas. Consulta [Grupos](/es/channels/groups) y [Mensajes de grupo](/es/channels/group-messages).
  </Accordion>
  ...
  <Accordion title="¿Cuántos espacios de trabajo y agentes puedo crear?">
    ...
    - **Costo de tokens:** más agentes implica más uso concurrente del modelo.
  ...
  <Accordion title="¿Puedo ejecutar varios bots o chats al mismo tiempo (Slack), y cómo debería configurarlo?">
    ...
    El acceso al navegador es potente, pero no permite "hacer cualquier cosa que pueda hacer un humano": las medidas anti-bot, los CAPTCHA y MFA
    ...
    [Navegador](/es/tools/browser), [Nodes](/es/nodes).
...
<AccordionGroup>
  ...
  <Accordion title='¿Qué significa "another gateway instance is already listening"?'>
    OpenClaw aplica un bloqueo de runtime vinculando el listener de WebSocket inmediatamente al inicio (predeterminado `ws://127.0.0.1:18789`). Si el enlace falla con `EADDRINUSE`, lanza `GatewayLockError`, lo que indica que otra instancia ya está escuchando.
  ...
  <Accordion title="¿Cómo ejecuto OpenClaw en modo remoto (el cliente se conecta a un Gateway en otro lugar)?">
    Define `gateway.mode: "remote"` y apunta a una URL remota de WebSocket, opcionalmente con credenciales remotas de secreto compartido:
    ...
    - `openclaw gateway` solo inicia cuando `gateway.mode` es `local` (o pasas la bandera de anulación).
  ...
  <Accordion title='Control UI dice "unauthorized" (o sigue reconectándose). ¿Qué hago ahora?'>
    La ruta de autenticación de tu Gateway y el método de autenticación de la UI no coinciden.

    Hechos (desde el código):

    - Control UI mantiene el token en `sessionStorage` para la sesión actual de pestaña del navegador y la URL de Gateway seleccionada, por lo que las recargas en la misma pestaña siguen funcionando sin restaurar persistencia de tokens de larga duración en localStorage.
    ...
    - Ese reintento con token en caché ahora reutiliza los alcances aprobados en caché almacenados con el token de dispositivo. Los llamadores con `deviceToken` explícito / `scopes` explícitos siguen conservando su conjunto de alcances solicitado en lugar de heredar los alcances en caché.
    - Fuera de esa ruta de reintento, la prioridad de autenticación de conexión es primero token/contraseña compartidos explícitos, luego `deviceToken` explícito, luego token de dispositivo almacenado y luego token de arranque.
    - Las comprobaciones de alcance del token de arranque tienen prefijo de rol. La lista integrada de permitidos para operador de arranque solo satisface solicitudes de operador; los roles de Node u otros no operadores aún necesitan alcances bajo su propio prefijo de rol.

    Solución:

    - Más rápido: `openclaw dashboard` (imprime + copia la URL del panel, intenta abrirlo; muestra una pista de SSH si no hay pantalla).
    ...
    - Modo Tailscale Serve: asegúrate de que `gateway.auth.allowTailscale` esté habilitado y de estar abriendo la URL de Serve, no una URL sin procesar de loopback/tailnet que omita los encabezados de identidad de Tailscale.
    - Modo de proxy de confianza: asegúrate de entrar a través del proxy configurado con conocimiento de identidad, no una URL sin procesar del Gateway. Los proxies de loopback en el mismo host también necesitan `gateway.auth.trustedProxy.allowLoopback = true`.
    ...
    - Si esa llamada de rotación dice que fue denegada, revisa dos cosas:
      ...
      - los valores explícitos de `--scope` no pueden exceder los alcances de operador actuales del llamador
  ...
  <Accordion title="Definí gateway.bind tailnet, pero no puede vincularse y nada escucha">
  ...
  <Accordion title="¿Puedo ejecutar varios Gateways en el mismo host?">
    ...
    - `agents.defaults.workspace` (aislamiento de workspace)
    - `gateway.port` (puertos únicos)

    Configuración rápida (recomendada):

    - Usa `openclaw --profile <name> ...` por instancia (crea automáticamente `~/.openclaw-<name>`).
    - Define un `gateway.port` único en cada configuración de perfil (o pasa `--port` para ejecuciones manuales).
    - Instala un servicio por perfil: `openclaw --profile <name> gateway install`.

    Los perfiles también agregan sufijos a los nombres de servicio (`ai.openclaw.<profile>`; heredados `com.openclaw.*`, `openclaw-gateway-<profile>.service`, `OpenClaw Gateway (<profile>)`).
...
<AccordionGroup>
  ...
  <Accordion title="Cerré mi terminal en Windows: ¿cómo reinicio OpenClaw?">
    ...
    **2) Windows nativo (no recomendado):** el Gateway se ejecuta directamente en Windows.
    ...
    Docs: [Windows (WSL2)](/es/platforms/windows), [Runbook del servicio Gateway](/es/gateway).
  ...
  <Accordion title="El Gateway está activo, pero las respuestas nunca llegan. ¿Qué debo comprobar?">
    ...
    - La autenticación del modelo no está cargada en el **host del gateway** (consulta `models status`).
    - El emparejamiento/lista de permitidos del canal bloquea las respuestas (revisa la configuración del canal + registros).
  ...
  <Accordion title='"Disconnected from gateway: no reason" - ¿qué hago ahora?'>
    ...
    4. Si es remoto, ¿está activo el enlace de túnel/Tailscale?
  ...
  <Accordion title="Telegram setMyCommands falla. ¿Qué debo revisar?">
    ...
    - `BOT_COMMANDS_TOO_MUCH`: el menú de Telegram tiene demasiadas entradas. OpenClaw ya lo recorta al límite de Telegram y reintenta con menos comandos, pero aún hay que eliminar algunas entradas del menú. Reduce los comandos de plugins/Skills/personalizados, o desactiva `channels.telegram.commands.native` si no necesitas el menú.
  ...
  <Accordion title="La TUI no muestra salida. ¿Qué debo revisar?">
...
<AccordionGroup>
  <Accordion title="Mi skill generó una imagen/PDF, pero no se envió nada">
    ...
    - `tools.fs.workspaceOnly=true` limita los envíos de rutas locales al workspace, temp/media-store y archivos validados por el sandbox.
    - `tools.fs.workspaceOnly=false` permite que `MEDIA:` envíe archivos locales del host que el agente ya puede leer, pero solo para medios y tipos de documentos seguros (imágenes, audio, video, PDF y documentos de Office). Los archivos de texto plano y similares a secretos siguen bloqueados.
...
<AccordionGroup>
  <Accordion title="¿Es seguro exponer OpenClaw a DM entrantes?">
    ...
    - El comportamiento predeterminado en canales con DM es el **emparejamiento**:
    ...
    - Abrir los DM públicamente requiere opt-in explícito (`dmPolicy: "open"` y lista de permitidos `"*"`).
  ...
  <Accordion title="¿La inyección de prompts solo preocupa en bots públicos?">
    No. La inyección de prompts se refiere al **contenido no confiable**, no solo a quién puede enviar DM al bot.
    ...
    exfiltre contexto o invoque herramientas en tu nombre. Reduce el radio de impacto mediante:
    ...
    - tratar también el texto decodificado de archivos/documentos como no confiable: tanto
      `input_file` de OpenResponses como la extracción de adjuntos multimedia envuelven el texto extraído en
      marcadores explícitos de límite de contenido externo, en lugar de pasar el texto sin procesar del archivo
    - aplicar sandboxing y listas estrictas de herramientas permitidas
  ...
  <Accordion title="Ejecuté /start en Telegram pero no recibí un código de emparejamiento">
    ...
    Si quieres acceso inmediato, agrega tu id de remitente a la lista de permitidos o define `dmPolicy: "open"`
    para esa cuenta.

  </Accordion>

  <Accordion title="WhatsApp: ¿enviará mensajes a mis contactos? ¿Cómo funciona el emparejamiento?">
    No. La política de DM predeterminada de WhatsApp es **emparejamiento**. Los remitentes desconocidos solo reciben un código de emparejamiento y su mensaje **no se procesa**. OpenClaw solo responde a chats que recibe o a envíos explícitos que tú activas.
    ...
    Prompt del número de teléfono del asistente de configuración: se usa para establecer tu **lista de permitidos/propietario**, de modo que tus propios DM estén permitidos. No se usa para envíos automáticos. Si lo ejecutas en tu número personal de WhatsApp, usa ese número y habilita `channels.whatsapp.selfChatMode`.
...
<AccordionGroup>
  <Accordion title="¿Cómo evito que los mensajes internos del sistema aparezcan en el chat?">
    ...
    Si sigue siendo ruidoso, revisa la configuración de la sesión en la UI de control y define verbose
    como **heredar**. Confirma también que no estés usando un perfil de bot con `verboseDefault` establecido
  ...
  <Accordion title="¿Cómo detengo/cancelo una tarea en ejecución?">
    ...
    La mayoría de comandos deben enviarse como un mensaje **independiente** que empieza con `/`, pero algunos accesos directos (como `/status`) también funcionan en línea para remitentes en la lista de permitidos.
  ...
  <Accordion title='¿Por qué parece que el bot "ignora" los mensajes rápidos consecutivos?'>
    El modo de cola controla cómo interactúan los mensajes nuevos con una ejecución en curso. Usa `/queue` para cambiar de modo:

    - `steer` - pone en cola toda la guía pendiente para el siguiente límite de modelo en la ejecución actual
    - `queue` - guía heredada de una en una
    - `followup` - ejecuta los mensajes uno por uno
    - `collect` - agrupa mensajes y responde una vez
    - `steer-backlog` - guía ahora y luego procesa el backlog
    - `interrupt` - cancela la ejecución actual y empieza de nuevo

    El modo predeterminado es `steer`. Puedes agregar opciones como `debounce:0.5s cap:25 drop:summarize` para modos de seguimiento. Consulta [Cola de comandos](/es/concepts/queue) y [Cola de guía](/es/concepts/queue-steering).
...
- [Preguntas frecuentes del primer inicio](/es/help/faq-first-run) — instalación, incorporación, autenticación, suscripciones, errores iniciales
