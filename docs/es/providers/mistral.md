...
read_when:
    ...
    - Quieres transcripción en tiempo real de Voxtral para llamada de voz
    - Necesitas la configuración inicial de la clave de API de Mistral y referencias de modelos
...
OpenClaw incluye actualmente este catálogo de Mistral integrado:
...
Usa Voxtral para la transcripción de audio por lotes mediante la canalización
...
## STT en streaming para llamadas de voz
...
<Note>
OpenClaw configura de forma predeterminada el STT en tiempo real de Mistral en `pcm_mulaw` a 8 kHz para que Voice Call
pueda reenviar directamente los fotogramas de medios de Twilio. Usa `encoding: "pcm_s16le"` y una
`sampleRate` coincidente solo si tu flujo ascendente ya es PCM sin procesar.
</Note>
...
<AccordionGroup>
  <Accordion title="Razonamiento ajustable (mistral-small-latest)">
    ...
    <Note>
    Otros modelos del catálogo de Mistral integrado no usan este parámetro. Sigue usando modelos `magistral-*` cuando quieras el comportamiento nativo de Mistral centrado primero en el razonamiento.
    </Note>
