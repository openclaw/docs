...
read_when:
    ...
    - Quiere la transcripción en tiempo real de Voxtral para llamadas de voz
    - Necesitas configurar una clave de API de Mistral y referencias de modelos
summary: Usa los modelos Mistral y la transcripción de Voxtral con OpenClaw
title: Mistral
x-i18n:
...
El plugin `mistral` incluido registra cuatro contratos: finalizaciones de chat, comprensión multimedia (transcripción por lotes de Voxtral), STT en tiempo real para Voice Call (Voxtral Realtime) e incrustaciones de memoria (`mistral-embed`).
...
| `mistral/mistral-small-latest`    | texto, imagen | 262,144  | 16,384        | Versión más reciente de Mistral Small 4; `reasoning_effort` ajustable |
...
Consulte la fila correspondiente del catálogo incluido antes de cambiar la configuración:

```bash
openclaw models list --all --provider mistral --plain
```

Realice una prueba rápida de un modelo sin iniciar el Gateway:

```bash
openclaw infer model run --local \
  --model mistral/mistral-medium-3-5 \
  --prompt "Responde exactamente: mistral-ok" \
  --json
```

## Transcripción de audio (Voxtral)

Use Voxtral para la transcripción de audio por lotes mediante el flujo de comprensión multimedia:
...
<Tip>
La ruta de transcripción multimedia usa `/v1/audio/transcriptions`. El modelo de audio predeterminado de Mistral es `voxtral-mini-latest`.
</Tip>

## STT de transmisión de Voice Call

El plugin `mistral` incluido registra Voxtral Realtime como proveedor de STT de transmisión para Voice Call.
...
<Note>
OpenClaw configura de forma predeterminada el STT en tiempo real de Mistral como `pcm_mulaw` a 8 kHz para que Voice Call pueda reenviar directamente las tramas multimedia de Twilio. Use `encoding: "pcm_s16le"` y un `sampleRate` correspondiente solo si el flujo ascendente ya contiene PCM sin procesar.
</Note>
...
<AccordionGroup>
  <Accordion title="Razonamiento ajustable">
    ...
    <Warning>
    Evite combinar el modo de razonamiento de Medium 3.5 con `temperature: 0`; se ha informado que la API HTTP de Mistral rechaza `reasoning_effort="high"` junto con `temperature: 0` mediante una respuesta 400. No establezca la temperatura, o desactive el razonamiento o configúrelo como mínimo para que OpenClaw envíe `reasoning_effort: "none"` antes de establecer una temperatura baja.
    </Warning>

    Ejemplo de configuración específica del modelo para el razonamiento de Medium 3.5:
    ...
    <Note>
    Los demás modelos incluidos en el catálogo de Mistral no usan este parámetro. Siga usando los modelos `magistral-*` cuando desee el comportamiento nativo de Mistral centrado primero en el razonamiento.
    </Note>
  ...
  <Accordion title="Autenticación y URL base">
    ...
    - La URL base del proveedor es `https://api.mistral.ai/v1` de forma predeterminada y acepta el formato estándar de solicitudes de finalizaciones de chat compatible con OpenAI.
    - El modelo predeterminado durante la incorporación es `mistral/mistral-large-latest`.
    - Sobrescriba la URL base en `models.providers.mistral.baseUrl` únicamente cuando Mistral publique explícitamente un punto de conexión regional que necesite.
