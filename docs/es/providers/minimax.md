...
title: MiniMax
x-i18n:
    generated_at: "2026-07-22T10:44:48Z"
    model: gpt-5.6
...
| `image-01`               | Generación de imágenes | Conversión de texto a imagen y edición de imagen a imagen |
...
<Tabs>
  <Tab title="OAuth (Coding Plan)">
    **Ideal para:** una configuración rápida con MiniMax Coding Plan mediante OAuth, sin necesidad de una clave de API.
    ...
    <Note>
    Las configuraciones con OAuth usan el identificador de proveedor `minimax-portal`. Las referencias de modelos siguen el formato `minimax-portal/MiniMax-M3`.
    </Note>

  </Tab>

  <Tab title="Clave de API">
    **Ideal para:** MiniMax alojado con una API compatible con Anthropic.
    ...
    <Warning>
    El punto de conexión de transmisión compatible con Anthropic de MiniMax-M2.x emite `reasoning_content` en fragmentos delta al estilo de OpenAI en lugar de bloques de pensamiento nativos de Anthropic, lo que expone el razonamiento interno en la salida visible si el pensamiento se deja habilitado implícitamente. OpenClaw deshabilita de forma predeterminada el pensamiento de M2.x, salvo que se establezca explícitamente `thinking`. MiniMax-M3 (y las versiones M3.x compatibles con versiones posteriores) está exento: M3 emite bloques de pensamiento de Anthropic adecuados y requiere que el pensamiento esté activo para producir contenido visible, por lo que OpenClaw mantiene M3 en la ruta de pensamiento adaptativo del proveedor. Consulte la sección Valores predeterminados de pensamiento en Configuración avanzada más adelante.
    </Warning>

    <Note>
    Las configuraciones con clave de API usan el identificador de proveedor `minimax`. Las referencias de modelos siguen el formato `minimax/MiniMax-M3`.
    </Note>
...
El plugin MiniMax registra el modelo `image-01` para la herramienta `image_generate` tanto en `minimax` como en `minimax-portal`, reutilizando la misma `MINIMAX_API_KEY` o autenticación OAuth que los modelos de texto.

- Generación de texto a imagen y edición de imagen a imagen (referencia del sujeto), ambas con control de la relación de aspecto
...
El plugin `minimax` incluido registra MiniMax T2A v2 como proveedor de voz para `tts`.

- Modelo TTS predeterminado: `speech-2.8-hd`
- Voz predeterminada: `English_expressive_narrator`
- Identificadores de modelos incluidos: `speech-2.8-hd`, `speech-2.8-turbo`, `speech-2.6-hd`, `speech-2.6-turbo`, `speech-02-hd`, `speech-02-turbo`, `speech-01-hd`, `speech-01-turbo`
- Orden de resolución de la autenticación: `tts.providers.minimax.apiKey`, después los perfiles de autenticación OAuth/token de `minimax-portal`, después las claves de entorno de Token Plan (`MINIMAX_OAUTH_TOKEN`, `MINIMAX_CODE_PLAN_KEY`, `MINIMAX_CODING_API_KEY`) y, por último, `MINIMAX_API_KEY`
- Si no se configura ningún host TTS, OpenClaw reutiliza el host OAuth configurado en `minimax-portal` y elimina los sufijos de ruta compatibles con Anthropic, como `/anthropic`
- Los archivos adjuntos de audio normales permanecen en MP3. Los destinos de notas de voz (Feishu, Telegram y otros canales que solicitan un archivo adjunto compatible con notas de voz) se transcodifican de MP3 de MiniMax a Opus de 48 kHz con `ffmpeg`, porque, por ejemplo, la API de archivos de Feishu/Lark solo acepta `file_type: "opus"` para mensajes de audio nativos
- MiniMax T2A acepta valores fraccionarios para `speed` y `vol`, pero `pitch` se envía como un entero; OpenClaw trunca los valores fraccionarios de `pitch` antes de la solicitud a la API

| Ajuste                          | Variable de entorno     | Valor predeterminado             | Descripción                      |
| ------------------------------- | ---------------------- | ----------------------------- | -------------------------------- |
| `tts.providers.minimax.baseUrl` | `MINIMAX_API_HOST`     | `https://api.minimax.io`      | Host de la API T2A de MiniMax.            |
| `tts.providers.minimax.model`   | `MINIMAX_TTS_MODEL`    | `speech-2.8-hd`               | Identificador del modelo TTS.                    |
| `tts.providers.minimax.voiceId` | `MINIMAX_TTS_VOICE_ID` | `English_expressive_narrator` | Identificador de voz utilizado para la salida de voz. |
| `tts.providers.minimax.speed`   |                        | `1.0`                         | Velocidad de reproducción, `0.5..2.0`.      |
| `tts.providers.minimax.vol`     |                        | `1.0`                         | Volumen, `(0, 10]`.               |
| `tts.providers.minimax.pitch`   |                        | `0`                           | Desplazamiento entero del tono, `-12..12`.  |
...
- Las ejecuciones respaldadas por sesiones se desacoplan mediante el flujo compartido de tarea/estado, incluido `action: "status"`
...
Por eso, el enrutamiento automático de contenido multimedia puede utilizar la comprensión de imágenes de MiniMax incluso cuando el catálogo incluido de proveedores de texto también contiene referencias de chat M3 compatibles con imágenes. La comprensión de PDF utiliza `MiniMax-M2.7` únicamente para extraer texto; MiniMax no registra una ruta de conversión de PDF a imagen.
...
- La búsqueda permanece en el ID de proveedor `minimax`; la configuración OAuth para China/global puede dirigir indirectamente la región mediante `models.providers.minimax-portal.baseUrl` y proporcionar autenticación de portador mediante `MINIMAX_OAUTH_TOKEN`

La configuración se encuentra en `plugins.entries.minimax.config.webSearch.*`.

<Note>
Consulte [Búsqueda de MiniMax](/es/tools/minimax-search) para obtener información completa sobre la configuración y el uso de la búsqueda web.
</Note>
...
<AccordionGroup>
  <Accordion title="Opciones de configuración">
    ...
    | `agents.defaults.modelPolicy.allow` | Lista de modelos permitidos explícita opcional |
  ...
  <Accordion title="Valores predeterminados de razonamiento">
    En `api: "anthropic-messages"`, OpenClaw inserta `thinking: { type: "disabled" }` para los modelos MiniMax M2.x, salvo que un envoltorio anterior ya haya establecido el campo `thinking` en la carga útil. Esto impide que el punto de conexión de transmisión de M2.x emita `reasoning_content` en fragmentos delta con formato de OpenAI, lo que expondría el razonamiento interno en la salida visible.

    MiniMax-M3 (y M3.x) queda exento: M3 devuelve una matriz `content` vacía con `stop_reason: "end_turn"` cuando el razonamiento está desactivado, por lo que OpenClaw elimina el valor predeterminado implícito de desactivación para M3 y, cuando se establece un nivel de razonamiento, fuerza `thinking: { type: "adaptive" }`.
  ...
  <Accordion title="Modo rápido">
    `/fast on` o `params.fastMode: true` reescribe `MiniMax-M2.7` como `MiniMax-M2.7-highspeed` en la ruta de transmisión compatible con Anthropic (`api: "anthropic-messages"`, proveedor `minimax` o `minimax-portal`).
  </Accordion>

  <Accordion title="Ejemplo de alternativa">
    **Ideal para:** mantener como principal el modelo más potente de última generación y usar MiniMax M2.7 como alternativa. El ejemplo siguiente utiliza Opus como modelo principal concreto; sustitúyalo por el modelo principal de última generación que prefiera.
  ...
  <Accordion title="Detalles de uso de Coding Plan">
    ...
    - El sondeo de uso obtiene el host de `models.providers.minimax-portal.baseUrl` o `models.providers.minimax.baseUrl` cuando están configurados, por lo que las configuraciones globales que utilizan `https://api.minimax.io/anthropic` sondean `api.minimax.io`. Las URL base ausentes o con formato incorrecto mantienen la alternativa para China por compatibilidad.
    - OpenClaw normaliza el uso del Coding Plan de MiniMax con la misma visualización `% left` utilizada por otros proveedores. Los campos sin procesar `usage_percent` / `usagePercent` de MiniMax representan la cuota restante, no la cuota consumida, por lo que OpenClaw los invierte. Los campos basados en recuentos tienen prioridad cuando están presentes.
    - Cuando la API devuelve `model_remains`, OpenClaw da preferencia a la entrada del modelo de chat, obtiene la etiqueta de la ventana de `start_time` / `end_time` cuando es necesario e incluye el nombre del modelo seleccionado en la etiqueta del plan para facilitar la distinción entre las ventanas de Coding Plan.
    - Las instantáneas de uso tratan `minimax`, `minimax-cn`, `minimax-portal` y `minimax-portal-cn` como la misma superficie de cuota de MiniMax, y prefieren el OAuth de MiniMax almacenado antes de recurrir a las variables de entorno de la clave de Coding Plan.
...
- La comprensión de imágenes utiliza el proveedor multimedia `MiniMax-VL-01`, propiedad del plugin
- Actualice los valores de precios en `models.json` si necesita un seguimiento exacto de los costes
- Utilice `openclaw models list` para confirmar el ID de proveedor actual y, después, cambie con `openclaw models set minimax/MiniMax-M3` o `openclaw models set minimax-portal/MiniMax-M3`
...
<AccordionGroup>
  <Accordion title='"Modelo desconocido: minimax/MiniMax-M3"'>
    Esto suele significar que el **proveedor MiniMax no está configurado** (no se encontró ninguna entrada de proveedor coincidente ni ningún perfil de autenticación o clave de entorno de MiniMax). Para solucionarlo:
    ...
    - Configure `MINIMAX_API_KEY`, `MINIMAX_OAUTH_TOKEN` o un perfil de autenticación de MiniMax para que se pueda insertar el proveedor correspondiente.

    Asegúrese de que el ID del modelo **distinga entre mayúsculas y minúsculas**:

    - Ruta de la clave de API: `minimax/MiniMax-M3`, `minimax/MiniMax-M2.7` o `minimax/MiniMax-M2.7-highspeed`
    - Ruta de OAuth: `minimax-portal/MiniMax-M3`, `minimax-portal/MiniMax-M2.7` o `minimax-portal/MiniMax-M2.7-highspeed`

    Después, vuelva a comprobarlo con:
