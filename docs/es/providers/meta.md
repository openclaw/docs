...
read_when:
    ...
    - Necesitas la variable de entorno MODEL_API_KEY o la opción de autenticación de la CLI
...
La **API de Meta** usa la **API Responses** compatible con OpenAI (`POST /v1/responses`)
...
```
  ...
  <Step title="Comprobar que los modelos estén disponibles">
    ...
    Muestra la entrada estática del catálogo de `muse-spark-1.1`. Si `MODEL_API_KEY` no se puede resolver,
...
- Esfuerzo de razonamiento: `minimal`, `low`, `medium`, `high`, `xhigh` (predeterminado: `high`)
- Reproducción cifrada y sin estado del razonamiento (`store: false`, `include: ["reasoning.encrypted_content"]`)

<Warning>
`muse-spark-1.1` no acepta `reasoning.effort: "none"`. OpenClaw asigna
`--thinking off` a `minimal` para este proveedor.
</Warning>
...
Las pruebas en vivo usan `muse-spark-1.1` con `POST /v1/responses`.
...
<CardGroup cols={2}>
  ...
  <Card title="Modos de pensamiento" href="/es/tools/thinking" icon="brain">
    Niveles de esfuerzo de razonamiento para muse-spark-1.1.
  </Card>
