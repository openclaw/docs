...
read_when:
    ...
    - Necesitas un comportamiento más estricto para la ejecución de agentes GPT-5
...
OpenClaw admite tres rutas de la familia OpenAI. El prefijo del modelo selecciona la ruta de proveedor/autenticación; una configuración de runtime separada selecciona quién ejecuta el bucle de agente embebido:

- **Clave de API** — acceso directo a OpenAI Platform con facturación basada en uso (modelos `openai/*`)
- **Suscripción Codex a través de PI** — inicio de sesión ChatGPT/Codex con acceso por suscripción (modelos `openai-codex/*`)
- **Arnés de servidor de aplicaciones Codex** — ejecución nativa del servidor de aplicaciones Codex (modelos `openai/*` más `agents.defaults.agentRuntime.id: "codex"`)
...
| GPT-5.5 con comportamiento nativo del servidor de aplicaciones Codex | `openai/gpt-5.5` más `agentRuntime.id: "codex"` | Fuerza el arnés del servidor de aplicaciones Codex para esa referencia de modelo. |
...
| `openai-codex`                     | Prefijo de proveedor | Ruta de OAuth/suscripción de OpenAI Codex a través del runner PI normal de OpenClaw.              |
| Plugin `codex`                     | Plugin            | Plugin incluido de OpenClaw que proporciona runtime nativo del servidor de aplicaciones Codex y controles de chat `/codex`. |
| `agentRuntime.id: codex`           | Runtime de agente | Fuerza el arnés nativo del servidor de aplicaciones Codex para turnos embebidos.                 |
...
Esto significa que una configuración puede contener intencionalmente tanto `openai-codex/*` como el Plugin `codex`. Eso es válido cuando quieres OAuth de Codex a través de PI y también quieres tener disponibles controles de chat nativos `/codex`. `openclaw doctor` advierte sobre esa combinación para que puedas confirmar que es intencional; no la reescribe.

<Note>
GPT-5.5 está disponible tanto mediante acceso directo con clave de API de OpenAI Platform como mediante rutas de suscripción/OAuth. Usa `openai/gpt-5.5` para tráfico directo con `OPENAI_API_KEY`, `openai-codex/gpt-5.5` para OAuth de Codex a través de PI, o `openai/gpt-5.5` con `agentRuntime.id: "codex"` para el arnés nativo del servidor de aplicaciones Codex.
</Note>

<Note>
Habilitar el Plugin de OpenAI, o seleccionar un modelo `openai-codex/*`, no habilita el Plugin de servidor de aplicaciones Codex incluido. OpenClaw habilita ese Plugin solo cuando seleccionas explícitamente el arnés nativo de Codex con `agentRuntime.id: "codex"` o usas una referencia de modelo heredada `codex/*`.
...
OpenClaw puede usar OpenAI, o un endpoint compatible con OpenAI para embeddings, para la indexación de `memory_search` y los embeddings de consulta:
...
Para endpoints compatibles con OpenAI que requieren etiquetas de embeddings asimétricas, define `queryInputType` y `documentInputType` bajo `memorySearch`. OpenClaw los reenvía como campos de solicitud `input_type` específicos del proveedor: los embeddings de consulta usan `queryInputType`; los fragmentos de memoria indexados y la indexación por lotes usan `documentInputType`. Consulta la [referencia de configuración de memoria](/es/reference/memory-config#provider-specific-config) para ver el ejemplo completo.
...
<Tabs>
  ...
  <Tab title="Suscripción Codex">
    **Ideal para:** usar tu suscripción ChatGPT/Codex en lugar de una clave de API separada. Codex cloud requiere iniciar sesión en ChatGPT.
    ...
    <Steps>
      <Step title="Ejecuta OAuth de Codex">
        ...
        Para configuraciones sin interfaz o hostiles a callbacks, añade `--device-code` para iniciar sesión con un flujo de código de dispositivo de ChatGPT en lugar del callback de navegador localhost:
    ...
    | Referencia de modelo | Configuración de runtime | Ruta | Autenticación |
    |-----------|----------------|-------|------|
    | `openai-codex/gpt-5.5` | omitido / `runtime: "pi"` | OAuth ChatGPT/Codex a través de PI | Inicio de sesión de Codex |
    | `openai-codex/gpt-5.4-mini` | omitido / `runtime: "pi"` | OAuth ChatGPT/Codex a través de PI | Inicio de sesión de Codex |
    ...
    <Note>
    Sigue usando el id de proveedor `openai-codex` para comandos de autenticación/perfil. El prefijo de modelo `openai-codex/*` también es la ruta PI explícita para OAuth de Codex. No selecciona ni habilita automáticamente el arnés del servidor de aplicaciones Codex incluido.
    </Note>
    ...
    selecciona el arnés de app-server de Codex incluido, `/status` muestra
    ...
    todavía se resuelve a través de PI. Mantén la configuración sin cambios cuando esa sea la
...
El arnés nativo del app-server de Codex usa refs de modelo `openai/*` más
`agentRuntime.id: "codex"`, pero su autenticación sigue basada en la cuenta. OpenClaw
...
2. La cuenta existente del app-server, como un inicio de sesión local de ChatGPT en Codex CLI.
...
Eso significa que un inicio de sesión local por suscripción de ChatGPT/Codex no se reemplaza solo
porque el proceso del Gateway también tenga `OPENAI_API_KEY` para modelos directos de OpenAI
o embeddings. La alternativa de clave de API por env solo es la ruta local de stdio sin cuenta; no
se envía a conexiones WebSocket del app-server. Cuando se selecciona un perfil de Codex
de estilo suscripción, OpenClaw también mantiene `CODEX_API_KEY` y `OPENAI_API_KEY`
fuera del proceso hijo de app-server por stdio generado y envía las credenciales seleccionadas
...
| Relación de aspecto / resolución | No se reenvía a la API Images de OpenAI | Se mapea a un tamaño admitido cuando es seguro |
...
El mismo ajuste se expone para ejecuciones headless de CLI:
...
almacenado y envía solicitudes de imagen mediante el backend Responses de Codex. No
...
bloqueados los endpoints de imagen privados/internos compatibles con OpenAI salvo que este opt-in esté
presente.
...
<Note>
Consulta [Generación de vídeo](/es/tools/video-generation) para ver los parámetros compartidos de la herramienta, la selección de proveedor y el comportamiento de conmutación por error.
</Note>

## Contribución de prompt de GPT-5

OpenClaw añade una contribución compartida de prompt de GPT-5 para ejecuciones de la familia GPT-5 entre proveedores. Se aplica por id de modelo, así que `openai-codex/gpt-5.5`, `openai/gpt-5.5`, `openrouter/openai/gpt-5.5`, `opencode/gpt-5.5` y otras refs compatibles con GPT-5 reciben la misma superposición. Los modelos GPT-4.x anteriores no.

El arnés nativo de Codex incluido usa el mismo comportamiento de GPT-5 y la superposición de Heartbeat mediante instrucciones de desarrollador del app-server de Codex, así que las sesiones `openai/gpt-5.x` forzadas mediante `agentRuntime.id: "codex"` conservan la misma guía de seguimiento y Heartbeat proactivo aunque Codex sea propietario del resto del prompt del arnés.

La contribución de GPT-5 añade un contrato de comportamiento etiquetado para persistencia de persona, seguridad de ejecución, disciplina de herramientas, forma de salida, comprobaciones de finalización y verificación. El comportamiento específico del canal para respuestas y mensajes silenciosos permanece en el prompt de sistema compartido de OpenClaw y la política de entrega saliente. La guía de GPT-5 siempre está habilitada para los modelos coincidentes. La capa de estilo de interacción amigable está separada y es configurable.
...
<Tip>
Los valores no distinguen entre mayúsculas y minúsculas en runtime, así que `"Off"` y `"off"` deshabilitan ambos la capa de estilo amigable.
</Tip>
...
<AccordionGroup>
  ...
  <Accordion title="Voz en tiempo real">
    ...
    <Note>
    Admite Azure OpenAI mediante las claves de configuración `azureEndpoint` y `azureDeployment` para puentes en tiempo real de backend. Admite llamadas a herramientas bidireccionales. Usa el formato de audio G.711 u-law.
    </Note>

    <Note>
    Control UI Talk usa sesiones en tiempo real de OpenAI en el navegador con un secreto
    de cliente efímero acuñado por Gateway y un intercambio SDP WebRTC directo del
    navegador contra la API OpenAI Realtime. La verificación en vivo para mantenedores
    está disponible con
    `OPENAI_API_KEY=... GEMINI_API_KEY=... node --import tsx scripts/dev/realtime-talk-live-smoke.ts`;
    la etapa de OpenAI acuña un secreto de cliente en Node, genera una oferta SDP del
    navegador con medios de micrófono simulados, la publica en OpenAI y aplica la
...
en tiempo real** en [Voz y habla](#voice-and-speech) para ver su configuración de
Azure.
</Note>
...
flujo de onboarding o una configuración dedicada de proveedor de Azure; `openai.baseUrl` por sí solo
...
<AccordionGroup>
  ...
  <Accordion title="Preparación de WebSocket">
    OpenClaw habilita de forma predeterminada la preparación de WebSocket para `openai/*` y `openai-codex/*` a fin de reducir la latencia del primer turno.
  ...
  <Accordion title="Procesamiento prioritario (service_tier)">
    La API de OpenAI expone procesamiento prioritario mediante `service_tier`. Configúralo por modelo en OpenClaw:
    ...
    <Warning>
    `serviceTier` solo se reenvía a puntos de conexión nativos de OpenAI (`api.openai.com`) y puntos de conexión nativos de Codex (`chatgpt.com/backend-api`). Si enrutas cualquiera de los proveedores mediante un proxy, OpenClaw deja `service_tier` sin modificar.
    </Warning>

  </Accordion>

  <Accordion title="Compaction del lado del servidor (API Responses)">
    Para modelos directos de OpenAI Responses (`openai/*` en `api.openai.com`), el contenedor de streaming Pi-harness del Plugin OpenAI habilita automáticamente la Compaction del lado del servidor:
    ...
    Esto se aplica a la ruta Pi harness integrada y a los hooks del proveedor OpenAI usados por ejecuciones embebidas. El harness nativo del servidor de aplicaciones Codex administra su propio contexto mediante Codex y se configura por separado con `agents.defaults.agentRuntime.id`.
  ...
  <Accordion title="Modo GPT estrictamente agéntico">
    ...
    - Ya no trata un turno solo con plan como progreso correcto cuando hay una acción de herramienta disponible
    ...
    <Note>
    Limitado solo a ejecuciones de la familia OpenAI y Codex GPT-5. Otros proveedores y familias de modelos anteriores mantienen el comportamiento predeterminado.
    </Note>
  ...
  <Accordion title="Rutas nativas frente a rutas compatibles con OpenAI">
    ...
    **Rutas de proxy/compatibles:**
    ...
    - Eliminan Completions `store` de las cargas útiles `openai-completions` no nativas
