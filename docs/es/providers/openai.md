...
read_when:
    ...
    - Quieres usar la autenticación por suscripción de Codex en lugar de claves de API
    - Necesitas un comportamiento de ejecución de agentes GPT-5 más estricto
summary: Usar OpenAI con claves de API o una suscripción a Codex en OpenClaw
title: OpenAI
x-i18n:
    generated_at: "2026-05-02T21:03:45Z"
    model: gpt-5.5
    provider: openai
...
OpenAI proporciona APIs para desarrolladores para modelos GPT, y Codex también está disponible como agente de codificación del plan
ChatGPT a través de los clientes Codex de OpenAI. OpenClaw mantiene esas
superficies separadas para que la configuración siga siendo predecible.

OpenClaw admite tres rutas de la familia OpenAI. La mayoría de los suscriptores de ChatGPT/Codex
que quieren el comportamiento de Codex deberían usar el runtime nativo del servidor de aplicaciones de Codex. El
prefijo del modelo selecciona el proveedor/nombre del modelo; un ajuste de runtime separado selecciona
quién ejecuta el bucle de agente incrustado:

- **Clave de API** - acceso directo a OpenAI Platform con facturación basada en uso (modelos `openai/*`)
- **Suscripción de Codex con runtime nativo de Codex** - inicio de sesión de ChatGPT/Codex más ejecución del servidor de aplicaciones de Codex (modelos `openai/*` más `agents.defaults.agentRuntime.id: "codex"`)
- **Suscripción de Codex a través de PI** - inicio de sesión de ChatGPT/Codex con el ejecutor PI normal de OpenClaw (modelos `openai-codex/*`)
...
| Objetivo                                             | Uso                                              | Notas                                                                     |
| ---------------------------------------------------- | ------------------------------------------------ | ------------------------------------------------------------------------- |
| Suscripción de ChatGPT/Codex con runtime nativo de Codex | `openai/gpt-5.5` más `agentRuntime.id: "codex"` | Configuración de Codex recomendada para la mayoría de los usuarios. Inicia sesión con autenticación `openai-codex`. |
...
| Autenticación de suscripción de ChatGPT/Codex a través de PI | `openai-codex/gpt-5.5`                           | Úsalo solo cuando quieras intencionadamente el ejecutor PI normal. |
...
| Plugin `codex`                     | Plugin            | Plugin integrado de OpenClaw que proporciona el runtime nativo del servidor de aplicaciones de Codex y controles de chat `/codex`. |
| `agentRuntime.id: codex`           | Runtime de agente | Fuerza el arnés nativo del servidor de aplicaciones de Codex para turnos incrustados.             |
...
Esto significa que una configuración puede contener intencionadamente tanto `openai-codex/*` como el
Plugin `codex`. Eso es válido cuando quieres Codex OAuth a través de PI y también quieres
que los controles de chat nativos `/codex` estén disponibles. `openclaw doctor` advierte sobre esa
combinación para que puedas confirmar que es intencionada; no la reescribe.
...
usa `openai/gpt-5.5` con `agentRuntime.id: "codex"`. Usa
`openai-codex/gpt-5.5` solo para Codex OAuth a través de PI, o `openai/gpt-5.5`
sin una sobrescritura de runtime de Codex para tráfico directo de `OPENAI_API_KEY`.
</Note>

<Note>
Habilitar el Plugin de OpenAI, o seleccionar un modelo `openai-codex/*`, no
habilita el Plugin integrado del servidor de aplicaciones de Codex. OpenClaw habilita ese Plugin solo
cuando seleccionas explícitamente el arnés nativo de Codex con
`agentRuntime.id: "codex"` o usas una referencia de modelo heredada `codex/*`.
Si el Plugin integrado `codex` está habilitado pero `openai-codex/*` aún se resuelve
...
| Arnés del servidor de aplicaciones de Codex | `openai/<model>` con `agentRuntime.id: codex` | Sí                                                     |
...
Para endpoints compatibles con OpenAI que requieren etiquetas de embeddings asimétricas, define
...
<Tabs>
  <Tab title="Clave de API (OpenAI Platform)">
    **Ideal para:** acceso directo a la API y facturación basada en uso.
    ...
    | `openai/gpt-5.5`       | `agentRuntime.id: "codex"`           | Arnés del servidor de aplicaciones de Codex | Servidor de aplicaciones de Codex |

    <Note>
    `openai/*` es la ruta directa con clave de API de OpenAI, salvo que fuerces explícitamente
    el arnés del servidor de aplicaciones de Codex. Usa `openai-codex/*` para Codex OAuth a través de
    el ejecutor PI predeterminado, o usa `openai/gpt-5.5` con
    `agentRuntime.id: "codex"` para la ejecución nativa del servidor de aplicaciones de Codex.
    </Note>
  ...
  <Tab title="Suscripción de Codex">
    **Ideal para:** usar tu suscripción de ChatGPT/Codex con ejecución nativa del servidor de aplicaciones de Codex en lugar de una clave de API separada. La nube de Codex requiere inicio de sesión de ChatGPT.
    ...
    <Steps>
      <Step title="Ejecuta Codex OAuth">
        ...
        Para configuraciones sin interfaz gráfica o hostiles a callbacks, añade `--device-code` para iniciar sesión con un flujo de código de dispositivo de ChatGPT en lugar del callback del navegador localhost:
    ...
    | Ref. de modelo | Configuración de runtime | Ruta | Autenticación |
    |-----------|----------------|-------|------|
    | `openai/gpt-5.5` | `agentRuntime.id: "codex"` | Arnés nativo del servidor de aplicaciones de Codex | Inicio de sesión de Codex o perfil `openai-codex` seleccionado |
    | `openai-codex/gpt-5.5` | omitido / `runtime: "pi"` | ChatGPT/Codex OAuth a través de PI | Inicio de sesión de Codex |
    | `openai-codex/gpt-5.4-mini` | omitido / `runtime: "pi"` | ChatGPT/Codex OAuth a través de PI | Inicio de sesión de Codex |
    ...
    No selecciona ni habilita automáticamente el arnés de servidor de aplicaciones Codex integrado. Para
    ...
    Para mantener OAuth de Codex en el ejecutor PI normal en su lugar, usa
    `openai-codex/gpt-5.5` y omite la anulación de runtime de Codex.
    ...
    selecciona el arnés de servidor de aplicaciones Codex integrado, `/status` muestra
    ...
    Si el plugin `codex` integrado está habilitado mientras se selecciona una ruta
    ...
    El límite predeterminado más pequeño ofrece en la práctica mejores características de latencia y calidad. Anúlalo con `contextTokens`:
...
`agentRuntime.id: "codex"`, pero su autenticación sigue estando basada en cuentas. OpenClaw
...
2. La cuenta existente del servidor de aplicaciones, como un inicio de sesión local de ChatGPT en Codex CLI.
...
o embeddings. La alternativa de clave de API de entorno es solo la ruta local stdio sin cuenta; no
...
El plugin `openai` integrado registra la generación de imágenes mediante la herramienta `image_generate`.
Admite tanto la generación de imágenes con clave de API de OpenAI como la generación de imágenes con
OAuth de Codex mediante la misma referencia de modelo `openai/gpt-image-2`.
...
Para una solicitud de fondo transparente, los agentes deben llamar a `image_generate` con
`model: "openai/gpt-image-1.5"`, `outputFormat: "png"` o `"webp"`, y
`background: "transparent"`; la opción anterior de proveedor `openai.background` todavía
se acepta. OpenClaw también protege las rutas públicas de OpenAI y
...
Usa los mismos flags `--output-format` y `--background` con
...
almacenado y envía solicitudes de imagen mediante el backend de Responses de Codex. No
...
en su lugar.
Si ese endpoint de imagen personalizado está en una LAN/dirección privada de confianza, configura también
`browser.ssrfPolicy.dangerouslyAllowPrivateNetwork: true`; OpenClaw mantiene
bloqueados los endpoints de imagen privados/internos compatibles con OpenAI salvo que esta opción de aceptación esté
presente.
...
El plugin `openai` integrado registra la generación de video mediante la herramienta `video_generate`.
...
OpenClaw agrega una contribución de prompt GPT-5 compartida para ejecuciones de la familia GPT-5 entre proveedores. Se aplica por id de modelo, por lo que `openai-codex/gpt-5.5`, `openai/gpt-5.5`, `openrouter/openai/gpt-5.5`, `opencode/gpt-5.5` y otras referencias compatibles con GPT-5 reciben la misma superposición. Los modelos GPT-4.x anteriores no.

El arnés nativo Codex integrado usa el mismo comportamiento GPT-5 y la misma superposición de heartbeat mediante instrucciones de desarrollador del servidor de aplicaciones Codex, por lo que las sesiones `openai/gpt-5.x` forzadas mediante `agentRuntime.id: "codex"` conservan la misma orientación de seguimiento y heartbeat proactivo aunque Codex sea propietario del resto del prompt del arnés.

La contribución GPT-5 agrega un contrato de comportamiento etiquetado para persistencia de persona, seguridad de ejecución, disciplina de herramientas, forma de salida, comprobaciones de finalización y verificación. El comportamiento de respuesta específico del canal y de mensajes silenciosos permanece en el prompt de sistema compartido de OpenClaw y la política de entrega saliente. La guía GPT-5 siempre está habilitada para modelos coincidentes. La capa de estilo de interacción amistoso es independiente y configurable.
...
<Tip>
Los valores no distinguen mayúsculas de minúsculas en runtime, así que `"Off"` y `"off"` deshabilitan ambos la capa de estilo amistoso.
</Tip>

<Note>
El ajuste heredado `plugins.entries.openai.config.personality` todavía se lee como alternativa de compatibilidad cuando el ajuste compartido `agents.defaults.promptOverlays.gpt5.personality` no está establecido.
</Note>
...
<AccordionGroup>
  <Accordion title="Síntesis de voz (TTS)">
    El plugin `openai` integrado registra síntesis de voz para la superficie `messages.tts`.
    ...
    `extraBody` se combina en el JSON de la solicitud `/audio/speech` después de los campos generados por OpenClaw, así que úsalo para endpoints compatibles con OpenAI que requieren claves adicionales como `lang`. Las claves de prototipo se ignoran.
  ...
  <Accordion title="Voz en tiempo real">
    ...
    <Note>
    Admite Azure OpenAI mediante las claves de configuración `azureEndpoint` y `azureDeployment` para puentes en tiempo real de backend. Admite llamadas bidireccionales a herramientas. Usa formato de audio G.711 u-law.
    </Note>

    <Note>
    Control UI Talk usa sesiones en tiempo real de OpenAI en el navegador con un secreto
    de cliente efímero emitido por el Gateway y un intercambio SDP WebRTC directo
    desde el navegador contra la API OpenAI Realtime. La verificación en vivo para mantenedores
    está disponible con
    `OPENAI_API_KEY=... GEMINI_API_KEY=... node --import tsx scripts/dev/realtime-talk-live-smoke.ts`;
    el tramo de OpenAI emite un secreto de cliente en Node, genera una oferta SDP
    del navegador con medios de micrófono falsos, la publica en OpenAI y aplica la respuesta SDP
...
de imágenes al anular la URL base. En la ruta de generación de imágenes, OpenClaw
...
en tiempo real** en [Voz y habla](#voice-and-speech) para ver sus ajustes de Azure.
</Note>
...
`openai.baseUrl` personalizado como el endpoint público de OpenAI y fallarán contra
implementaciones de imágenes de Azure.
</Note>

### Versión de API

Establece `AZURE_OPENAI_API_VERSION` para fijar una versión preliminar o GA específica
...
los encabezados de atribución ocultos de OpenClaw; consulta el acordeón **Rutas
nativas frente a compatibles con OpenAI** en [Configuración avanzada](#advanced-configuration).
...
<AccordionGroup>
  ...
  <Accordion title="Precalentamiento de WebSocket">
    OpenClaw habilita el precalentamiento de WebSocket de forma predeterminada para `openai/*` y `openai-codex/*` a fin de reducir la latencia del primer turno.
  ...
  <Accordion title="Modo rápido">
    ...
    <Note>
    Las anulaciones de sesión prevalecen sobre la configuración. Borrar la anulación de sesión en la UI de sesiones devuelve la sesión al valor predeterminado configurado.
    </Note>
  ...
  <Accordion title="Compaction del lado del servidor (Responses API)">
    Para modelos directos de OpenAI Responses (`openai/*` en `api.openai.com`), el wrapper de transmisión del arnés Pi del Plugin de OpenAI habilita automáticamente Compaction del lado del servidor:
    ...
    Esto se aplica a la ruta integrada del arnés Pi y a los hooks del proveedor OpenAI usados por ejecuciones integradas. El arnés nativo del servidor de la app Codex gestiona su propio contexto mediante Codex y se configura por separado con `agents.defaults.agentRuntime.id`.
  ...
  <Accordion title="Modo GPT agente estricto">
    ...
    - Ya no trata un turno solo de planificación como progreso correcto cuando hay una acción de herramienta disponible
    - Reintenta el turno con una orientación para actuar ahora
    - Habilita automáticamente `update_plan` para trabajo sustancial
    - Expone un estado bloqueado explícito si el modelo sigue planificando sin actuar
  ...
  <Accordion title="Rutas nativas frente a rutas compatibles con OpenAI">
    ...
    **Rutas nativas** (`openai/*`, Azure OpenAI):
    ...
    - Mantienen el moldeado de solicitudes exclusivo de OpenAI (`service_tier`, `store`, compatibilidad de razonamiento, pistas de caché de prompts)
