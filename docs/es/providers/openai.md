...
read_when:
    ...
    - Quieres la autenticación por suscripción de Codex en lugar de claves de API
...
OpenAI proporciona API para desarrolladores para modelos GPT, y Codex también está disponible como agente de programación con plan de ChatGPT a través de los clientes Codex de OpenAI. OpenClaw mantiene esas superficies separadas para que la configuración siga siendo predecible.

OpenClaw admite tres rutas de la familia OpenAI. La mayoría de los suscriptores de ChatGPT/Codex que quieren el comportamiento de Codex deberían usar el runtime nativo del servidor de aplicación de Codex. El prefijo del modelo selecciona el nombre del proveedor/modelo; una configuración de runtime separada selecciona quién ejecuta el bucle de agente integrado:
...
- **Suscripción a Codex a través de PI** - inicio de sesión de ChatGPT/Codex con el ejecutor normal de OpenClaw PI (modelos `openai-codex/*`)

OpenAI admite explícitamente el uso de OAuth de suscripción en herramientas y flujos de trabajo externos como OpenClaw.

El proveedor, el modelo, el runtime y el canal son capas separadas. Si esas etiquetas se están mezclando, lee [Runtimes de agente](/es/concepts/agent-runtimes) antes de cambiar la configuración.

## Elección rápida

| Objetivo                                             | Usar                                             | Notas                                                                     |
| ---------------------------------------------------- | ------------------------------------------------ | ------------------------------------------------------------------------- |
| Suscripción a ChatGPT/Codex con runtime nativo de Codex | `openai/gpt-5.5` más `agentRuntime.id: "codex"` | Configuración de Codex recomendada para la mayoría de los usuarios. Inicia sesión con la autenticación `openai-codex`. |
| Facturación directa con clave de API                 | `openai/gpt-5.5`                                 | Define `OPENAI_API_KEY` o ejecuta el proceso de incorporación con clave de API de OpenAI. |
| Autenticación de suscripción a ChatGPT/Codex a través de PI | `openai-codex/gpt-5.5`                           | Úsalo solo cuando quieras intencionadamente el ejecutor normal de PI.      |
...
| `openai-codex`                     | Prefijo de proveedor | Ruta de OAuth/suscripción de OpenAI Codex a través del ejecutor normal de OpenClaw PI.             |
| Plugin `codex`                     | Plugin            | Plugin incluido de OpenClaw que proporciona el runtime nativo del servidor de aplicación de Codex y controles de chat `/codex`. |
...
| `/codex ...`                       | Conjunto de comandos de chat | Enlaza/controla hilos del servidor de aplicación de Codex desde una conversación.                  |
| `runtime: "acp", agentId: "codex"` | Ruta de sesión ACP | Ruta de reserva explícita que ejecuta Codex a través de ACP/acpx.                                  |

Esto significa que una configuración puede contener intencionadamente tanto `openai-codex/*` como el Plugin `codex`. Eso es válido cuando quieres OAuth de Codex a través de PI y también quieres que los controles de chat nativos `/codex` estén disponibles. `openclaw doctor` advierte sobre esa combinación para que puedas confirmar que es intencionada; no la reescribe.

<Note>
GPT-5.5 está disponible tanto mediante acceso directo con clave de API de OpenAI Platform como mediante rutas de suscripción/OAuth. Para suscripción a ChatGPT/Codex más ejecución nativa de Codex, usa `openai/gpt-5.5` con `agentRuntime.id: "codex"`. Usa `openai-codex/gpt-5.5` solo para OAuth de Codex a través de PI, o `openai/gpt-5.5` sin una sobrescritura de runtime de Codex para tráfico directo de `OPENAI_API_KEY`.
</Note>

<Note>
Habilitar el Plugin de OpenAI, o seleccionar un modelo `openai-codex/*`, no habilita el Plugin incluido del servidor de aplicación de Codex. OpenClaw habilita ese Plugin solo cuando seleccionas explícitamente el arnés nativo de Codex con `agentRuntime.id: "codex"` o usas una referencia de modelo heredada `codex/*`.
...
| Arnés del servidor de aplicación de Codex | `openai/<model>` con `agentRuntime.id: codex` | Sí                                                     |
...
OpenClaw puede usar OpenAI, o un endpoint de embeddings compatible con OpenAI, para la indexación de `memory_search` y embeddings de consulta:
...
<Tabs>
  <Tab title="Clave de API (OpenAI Platform)">
    **Ideal para:** acceso directo a la API y facturación basada en uso.
    ...
    | `openai/gpt-5.5`       | `agentRuntime.id: "codex"`           | Arnés del servidor de aplicación de Codex | Servidor de aplicación de Codex |

    <Note>
    `openai/*` es la ruta directa con clave de API de OpenAI, a menos que fuerces explícitamente el arnés del servidor de aplicación de Codex. Usa `openai-codex/*` para OAuth de Codex a través del ejecutor PI predeterminado, o usa `openai/gpt-5.5` con `agentRuntime.id: "codex"` para ejecución nativa del servidor de aplicación de Codex.
    </Note>
  ...
  <Tab title="Suscripción a Codex">
    **Ideal para:** usar tu suscripción de ChatGPT/Codex con ejecución nativa del servidor de aplicación de Codex en lugar de una clave de API separada. La nube de Codex requiere inicio de sesión de ChatGPT.
    ...
    <Steps>
      <Step title="Ejecuta OAuth de Codex">
        ...
        Para configuraciones sin interfaz gráfica o problemáticas con callbacks, añade `--device-code` para iniciar sesión con un flujo de código de dispositivo de ChatGPT en lugar del callback del navegador localhost:
    ...
    | Ref. de modelo | Configuración de runtime | Ruta | Autenticación |
    |-----------|----------------|-------|------|
    | `openai/gpt-5.5` | `agentRuntime.id: "codex"` | Arnés nativo del servidor de aplicación de Codex | Inicio de sesión de Codex o perfil `openai-codex` seleccionado |
    ...
    <Warning>
    No configures referencias de modelo antiguas `openai-codex/gpt-5.1*`, `openai-codex/gpt-5.2*` ni `openai-codex/gpt-5.3*`. Las cuentas OAuth de ChatGPT/Codex ahora rechazan esos modelos. Usa `openai-codex/gpt-5.5` para la ruta OAuth de PI, o `openai/gpt-5.5` con `agentRuntime.id: "codex"` para la ejecución del runtime nativo de Codex.
    </Warning>

    <Note>
    Sigue usando el id de proveedor `openai-codex` para los comandos de autenticación/perfil. El
    prefijo de modelo `openai-codex/*` también es la ruta de PI explícita para OAuth de Codex.
    No selecciona ni habilita automáticamente el arnés de servidor de aplicaciones Codex incluido. Para
    la configuración común con suscripción más runtime nativo, inicia sesión con
    `openai-codex`, pero mantén la referencia del modelo como `openai/gpt-5.5` y establece
    ...
    Para conservar OAuth de Codex en el ejecutor de PI normal en su lugar, usa
    `openai-codex/gpt-5.5` y omite la anulación del runtime de Codex.

    <Note>
    La incorporación ya no importa material de OAuth desde `~/.codex`. Inicia sesión con OAuth en el navegador (predeterminado) o con el flujo de código de dispositivo anterior: OpenClaw administra las credenciales resultantes en su propio almacén de autenticación de agentes.
    </Note>

    ### Comprobar y recuperar el enrutamiento de OAuth de Codex
    ...
    Si una ejecución de `doctor --fix` de 2026.5.5 cambió una configuración de suscripción GPT-5.5 de
    `openai-codex/gpt-5.5` a `openai/gpt-5.5`, vuelve a cambiar el agente predeterminado
    a la ruta de PI con OAuth de Codex:
    ...
    `openai-codex/*` significa OAuth de ChatGPT/Codex mediante PI. `openai/*` con
    `agentRuntime.id: "codex"` significa ejecución nativa del servidor de aplicaciones Codex.

    ### Indicador de estado

    Chat `/status` muestra qué runtime de modelo está activo para la sesión actual.
    El arnés de PI predeterminado aparece como `Runtime: OpenClaw Pi Default`. Cuando se
    selecciona el arnés de servidor de aplicaciones Codex incluido, `/status` muestra
    ...
    Si el Plugin `codex` incluido está habilitado mientras se selecciona una ruta `openai-codex/*`,
    `openclaw doctor` advierte que el modelo todavía se resuelve mediante PI.
    Mantén la configuración sin cambios solo cuando esa ruta de autenticación por suscripción de PI sea
    intencional. Cambia a `openai/<model>` más `agentRuntime.id: "codex"` cuando
    quieras la ejecución nativa del servidor de aplicaciones Codex.
...
El arnés nativo de servidor de aplicaciones Codex usa referencias de modelo `openai/*` más
...
2. La cuenta existente del servidor de aplicaciones, como un inicio de sesión local de ChatGPT en la CLI de Codex.
3. Solo para lanzamientos locales de servidor de aplicaciones por stdio, `CODEX_API_KEY`, luego
   `OPENAI_API_KEY`, cuando el servidor de aplicaciones informa que no hay cuenta y todavía requiere
...
o embeddings. La alternativa de clave de API por entorno es solo la ruta local stdio sin cuenta; no
se envía a conexiones WebSocket de servidor de aplicaciones. Cuando se selecciona un perfil Codex
...
Admite tanto la generación de imágenes con clave de API de OpenAI como la generación de imágenes
con OAuth de Codex mediante la misma referencia de modelo `openai/gpt-image-2`.
...
| Relación de aspecto / resolución | No se reenvía a la API de OpenAI Images      | Se asigna a un tamaño admitido cuando es seguro     |
...
acepta. OpenClaw también protege las rutas públicas de OpenAI y de OAuth de
OpenAI Codex reescribiendo las solicitudes transparentes predeterminadas de `openai/gpt-image-2`
a `gpt-image-1.5`; Azure y los endpoints personalizados compatibles con OpenAI conservan
sus nombres configurados de implementación/modelo.
...
almacenado y envía solicitudes de imágenes mediante el backend Codex Responses. No
...
URL base personalizada o endpoint de Azure cuando quieras la ruta directa de la API
OpenAI Images.
Si ese endpoint de imagen personalizado está en una dirección LAN/privada de confianza, establece también
`browser.ssrfPolicy.dangerouslyAllowPrivateNetwork: true`; OpenClaw mantiene bloqueados
los endpoints privados/internos de imágenes compatibles con OpenAI salvo que exista esta adhesión explícita.
...
OpenClaw agrega una contribución de prompt compartida de GPT-5 para ejecuciones de la familia GPT-5 entre proveedores. Se aplica por id de modelo, así que `openai-codex/gpt-5.5`, `openai/gpt-5.5`, `openrouter/openai/gpt-5.5`, `opencode/gpt-5.5` y otras referencias compatibles con GPT-5 reciben la misma superposición. Los modelos GPT-4.x anteriores no.

El arnés nativo de Codex incluido usa el mismo comportamiento de GPT-5 y la misma superposición de heartbeat mediante instrucciones de desarrollador del servidor de aplicaciones Codex, así que las sesiones `openai/gpt-5.x` forzadas mediante `agentRuntime.id: "codex"` conservan la misma guía de seguimiento y heartbeat proactivo aunque Codex controle el resto del prompt del arnés.
...
<Note>
La opción heredada `plugins.entries.openai.config.personality` todavía se lee como respaldo de compatibilidad cuando la configuración compartida `agents.defaults.promptOverlays.gpt5.personality` no está establecida.
</Note>
...
<AccordionGroup>
  <Accordion title="Síntesis de voz (TTS)">
    ...
    `extraBody` se fusiona en el JSON de solicitud de `/audio/speech` después de los campos generados por OpenClaw, así que úsalo para puntos de conexión compatibles con OpenAI que requieren claves adicionales como `lang`. Las claves de prototipo se ignoran.
    ...
    <Note>
    Establece `OPENAI_TTS_BASE_URL` para anular la URL base de TTS sin afectar al punto de conexión de la API de chat.
    </Note>
  ...
  <Accordion title="Voz a texto">
    ...
    - Compatible con OpenClaw dondequiera que la transcripción de audio entrante use
      `tools.media.audio`, incluidos segmentos de canales de voz de Discord y archivos
    ...
    Las sugerencias de idioma e instrucción se reenvían a OpenAI cuando las proporciona
  ...
  <Accordion title="Transcripción en tiempo real">
    ...
    | Instrucción | `...openai.prompt` | (sin establecer) |
  ...
  <Accordion title="Voz en tiempo real">
    ...
    <Note>
    Admite Azure OpenAI mediante las claves de configuración `azureEndpoint` y `azureDeployment` para puentes de tiempo real del backend. Admite llamadas bidireccionales a herramientas. Usa formato de audio G.711 u-law.
    </Note>

    <Note>
    Control UI Talk usa sesiones en tiempo real de OpenAI en el navegador con un secreto
    de cliente efímero generado por Gateway y un intercambio SDP de WebRTC directo del
    navegador contra la API OpenAI Realtime. La verificación en vivo para mantenedores
    está disponible con
    `OPENAI_API_KEY=... GEMINI_API_KEY=... node --import tsx scripts/dev/realtime-talk-live-smoke.ts`;
    el tramo de OpenAI emite un secreto de cliente en Node, genera una oferta SDP del
    navegador con medios de micrófono simulados, la publica en OpenAI y aplica la respuesta
    SDP sin registrar secretos.
    </Note>
...
generación de imágenes al anular la URL base. En la ruta de generación de imágenes,
OpenClaw detecta nombres de host de Azure en `models.providers.openai.baseUrl` y cambia
automáticamente a la estructura de solicitud de Azure.
...
**Voz en tiempo real** en [Voz y habla](#voice-and-speech) para ver su configuración
de Azure.
</Note>
...
Otras URL base (OpenAI público, proxies compatibles con OpenAI) mantienen la estructura
...
`openai.baseUrl` personalizada como el punto de conexión público de OpenAI y fallarán
contra implementaciones de imágenes de Azure.
</Note>

### Versión de API

Establece `AZURE_OPENAI_API_VERSION` para fijar una versión específica preliminar o GA
...
Azure OpenAI vincula modelos a implementaciones. Para solicitudes de generación de
imágenes de Azure enrutadas mediante el proveedor `openai` incluido, el campo `model`
en OpenClaw debe ser el **nombre de implementación de Azure** que configuraste en el
portal de Azure, no el identificador de modelo público de OpenAI.
...
La misma regla de nombre de implementación se aplica a las llamadas de generación de
imágenes enrutadas mediante el proveedor `openai` incluido.

### Disponibilidad regional

La generación de imágenes de Azure actualmente solo está disponible en un subconjunto de
regiones (por ejemplo `eastus2`, `swedencentral`, `polandcentral`, `westus3`,
...
Azure puede rechazar opciones que OpenAI público permite (por ejemplo, ciertos valores
de `background` en `gpt-image-2`) o exponerlas solo en versiones de modelo específicas.
Estas diferencias provienen de Azure y del modelo subyacente, no de OpenClaw. Si una
solicitud de Azure falla con un error de validación, revisa el conjunto de parámetros
compatible con tu implementación y versión de API específicas en el portal de Azure.
...
<AccordionGroup>
  <Accordion title="Transporte (WebSocket frente a SSE)">
    OpenClaw prioriza WebSocket con respaldo de SSE (`"auto"`) tanto para `openai/*` como para `openai-codex/*`.

    En modo `"auto"`, OpenClaw:
    - Reintenta un fallo temprano de WebSocket antes de recurrir a SSE
    - Después de un fallo, marca WebSocket como degradado durante ~60 segundos y usa SSE durante el periodo de espera
    ...
    |-------|----------|
    | `"auto"` (predeterminado) | WebSocket primero, respaldo de SSE |
    | `"sse"` | Forzar solo SSE |
    ...
    - [Respuestas de API por transmisión (SSE)](https://platform.openai.com/docs/guides/streaming-responses)
  ...
  <Accordion title="Modo rápido">
    ...
    <Note>
    Las anulaciones de sesión prevalecen sobre la configuración. Al borrar la anulación de sesión en la IU de Sesiones, la sesión vuelve al valor predeterminado configurado.
    </Note>
  ...
  <Accordion title="Compaction del lado del servidor (Responses API)">
    ...
    Esto se aplica a la ruta integrada de Pi harness y a los hooks del proveedor OpenAI usados por ejecuciones incrustadas. El harness nativo del servidor de aplicaciones de Codex gestiona su propio contexto mediante Codex y se configura por separado con `agents.defaults.agentRuntime.id`.
    ...
    <Note>
    `responsesServerCompaction` solo controla la inyección de `context_management`. Los modelos directos de OpenAI Responses todavía fuerzan `store: true`, salvo que la compatibilidad establezca `supportsStore: false`.
    </Note>

  </Accordion>

  <Accordion title="Modo GPT estricto agentic">
    ...
    - Ya no considera un turno solo de planificación como progreso correcto cuando hay una acción de herramienta disponible
  ...
  <Accordion title="Rutas nativas frente a rutas compatibles con OpenAI">
    ...
    **Rutas nativas** (`openai/*`, Azure OpenAI):
    ...
    - Mantienen la conformación de solicitudes exclusiva de OpenAI (`service_tier`, `store`, compatibilidad de razonamiento, indicaciones de caché de prompts)
