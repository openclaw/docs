...
title: Google (Gemini)
x-i18n:
    generated_at: "2026-07-19T02:22:48Z"
    model: gpt-5.6
...
<Tabs>
  ...
  <Tab title="Gemini CLI (OAuth)">
    **Recomendado para:** iniciar sesión con una cuenta de Google mediante OAuth de Gemini CLI en lugar de utilizar una clave de API independiente.
    ...
    <Steps>
      <Step title="Instalar Gemini CLI">
        ...
        OpenClaw admite tanto las instalaciones de Homebrew como las instalaciones globales de npm, incluidos
    ...
    El identificador de modelo de Gemini API para Gemini 3.1 Pro es `gemini-3.1-pro-preview`. OpenClaw acepta la forma abreviada `google/gemini-3.1-pro` como alias práctico y la normaliza antes de llamar al proveedor.
    ...
    **Variables de entorno:**
    ...
    La detección automática durante la incorporación muestra un inicio de sesión existente de Gemini CLI, pero nunca
    lo prueba automáticamente porque Gemini CLI no dispone de una comprobación sin herramientas. Elija OAuth de Gemini CLI
...
<Note>
`google/gemini-3-pro-preview` se retiró el 2026-03-09; utilice `google/gemini-3.1-pro-preview` en su lugar. Si se vuelve a ejecutar la configuración de la clave de API de Gemini (`openclaw onboard --auth-choice gemini-api-key` o `openclaw models auth login --provider google`), se sustituirá un valor predeterminado configurado obsoleto por el modelo actual.
</Note>
...
La precedencia de credenciales es `webSearch.apiKey` específica, seguida de `GEMINI_API_KEY`
y, después, `models.providers.google.apiKey`. `webSearch.baseUrl` es opcional y
existe para proxies de operadores o endpoints compatibles con la API de Gemini; si se omite,
...
`thinkingLevel`, de modo que las ejecuciones predeterminadas o de baja latencia no envíen valores
...
reescribe `thinkingBudget` como un valor de Google `thinkingLevel` compatible con Gemma 4.
...
<Note>
Consulte [Generación de imágenes](/es/tools/image-generation) para obtener información sobre los parámetros compartidos de la herramienta, la selección del proveedor y el comportamiento de conmutación por error.
</Note>
...
<Note>
Consulte [Generación de vídeo](/es/tools/video-generation) para obtener información sobre los parámetros compartidos de la herramienta, la selección del proveedor y el comportamiento de conmutación por error.
</Note>
...
<Note>
Consulte [Generación de música](/es/tools/music-generation) para obtener información sobre los parámetros compartidos de la herramienta, la selección del proveedor y el comportamiento de conmutación por error.
</Note>
...
proveedor de voz en tiempo real de Google basado en Gemini Live API en lugar de TTS
por lotes.
    ...
    tts: {
      ...
      providers: {
        google: {
          ...
          speakerVoice: "Kore",
          audioProfile: "Hable profesionalmente con un tono sereno.",
        },
      },
    },
  },
...
`audioProfile` para anteponer un prompt de estilo reutilizable al texto hablado. Establezca
...
pero se envíen al TTS, colóquelas dentro de un bloque `[[tts:text]]...[[/tts:text]]`:
...
<Note>
La API Live de Google utiliza audio bidireccional y llamadas a funciones mediante un WebSocket.
...
<Note>
Talk de la interfaz de control admite sesiones de navegador de Google Live con tokens restringidos
de un solo uso. En Video Talk, el navegador envía fotogramas JPEG limitados directamente a
...
exclusivos del backend también pueden ejecutarse mediante el transporte de retransmisión genérico del Gateway,
que mantiene las credenciales del proveedor en el Gateway.
</Note>
...
formato restringido de token de la API Live que utiliza Talk de la interfaz de control, abre el punto de conexión
WebSocket del navegador, envía la carga útil de configuración inicial junto con un fotograma JPEG y
verifica una respuesta de texto y un ciclo completo de la función `describe_view`.
...
<AccordionGroup>
  <Accordion title="Reutilización directa de la caché de Gemini">
    ...
    - Configure parámetros globales o por modelo con
      `cachedContent` o el valor heredado `cached_content`
    - Los parámetros del ámbito más específico (nivel de modelo antes que global) siempre prevalecen.
    ...
    - El uso por aciertos de caché de Gemini se normaliza como `cacheRead` de OpenClaw a partir de
  ...
  <Accordion title="Notas de uso de la CLI de Gemini">
    Al utilizar el proveedor OAuth `google-gemini-cli`, OpenClaw usa de forma predeterminada
    la salida `stream-json` de la CLI de Gemini y normaliza el uso a partir de la carga útil
    final `stats`. Las anulaciones heredadas de `--output-format json` siguen utilizando el
    analizador JSON.

    - El texto de la respuesta transmitida proviene de los eventos `message` del asistente.
    - En la salida JSON heredada, el texto de la respuesta proviene del campo `response` del JSON de la CLI.
...
<CardGroup cols={2}>
  ...
  <Card title="Generación de vídeo" href="/es/tools/video-generation" icon="video">
    Parámetros compartidos de herramientas de vídeo y selección de proveedores.
  </Card>
