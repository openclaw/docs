...
x-i18n:
    generated_at: "2026-07-19T02:04:00Z"
    model: gpt-5.6
...
Los controladores exclusivamente de observación se ejecutan en paralelo, y los envíos de observación
sin espera de resultado pueden solaparse con eventos posteriores. No use la prioridad para ordenar
...
Los operadores pueden configurar los tiempos máximos de los hooks sin modificar el código del plugin:
...
`hooks.timeouts.<hookName>` prevalece sobre `hooks.timeoutMs`, que a su vez prevalece sobre el
valor `api.on(..., { timeoutMs })` definido por el plugin. Cada valor debe ser un
entero positivo de hasta 600000 ms. Se recomienda usar valores específicos por hook para los
hooks que se sabe que son lentos, de modo que un plugin no disponga de más tiempo en todos los casos.

La promesa de un controlador que supera el tiempo máximo continúa ejecutándose porque las devoluciones de llamada de los hooks no
reciben una señal de cancelación. El envío del hook puede liberar su admisión en el Gateway
mientras el trabajo de ese plugin sigue en curso. Los plugins que gestionan
...
valor predeterminado de 15 segundos por controlador. Si uno supera el tiempo máximo, OpenClaw registra el error del plugin
y continúa con la carga útil más reciente para que el canal de entrega serializado pueda
finalizar. Configure un tiempo máximo mayor por hook para los plugins que realizan intencionadamente
trabajos más lentos antes de la entrega.

Del mismo modo, los plugins de canal que usan `createReplyDispatcher` pueden declarar un
tiempo máximo positivo mayor por etapa con `beforeDeliverOptions: { timeoutMs }`, o al
añadir trabajo con `dispatcher.appendBeforeDeliver(handler, { timeoutMs })`.
Sin un tiempo máximo declarado por el propietario, esas devoluciones de llamada usan el mismo valor predeterminado de
15 segundos para que una devolución de llamada bloqueada no pueda retener el canal de entrega serializado.
...
exclusivamente de observación.

**Turno del agente**

| Hook                            | Finalidad                                                                                  |
| ------------------------------- | ---------------------------------------------------------------------------------------- |
| `before_model_resolve`          | Sustituir el proveedor o modelo antes de cargar los mensajes de la sesión                                  |
...
| `before_agent_start`            | Fase combinada solo por compatibilidad; se recomienda usar los dos hooks anteriores                            |
...
| **`before_agent_reply`**        | Interrumpir anticipadamente el turno del modelo con una respuesta sintética o silencio                           |
...
| `after_tool_call`          | Observar los resultados y errores de las herramientas, así como la duración                |
...
| **`channel_pairing_requested`** | Observar las solicitudes de vinculación por mensaje directo recién creadas                         |
...
Para las llamadas a `sessions.create` con `parentSessionKey` y `emitCommandHooks: true`, cada hijo distinto siempre recibe `session_start`. Los invocadores declaran si el padre también recibe el evento terminal `session_end` mediante `succeedsParent`: `true` significa sucesor y `false` significa hijo paralelo. Si se omite, se conserva el comportamiento heredado de relevo del padre. Los hooks `command:new` y `before_reset` siguen describiendo la acción `/new` solicitada en ambos casos.

**Subagentes**

- `subagent_spawned` / `subagent_ended` - observan el inicio y la finalización del subagente.
- `subagent_delivery_target` - enlace de compatibilidad para entregar la finalización cuando ningún enlace de sesión del núcleo puede proyectar una ruta.
- `subagent_spawning` - enlace de compatibilidad obsoleto. Ahora el núcleo prepara los enlaces de subagente `thread: true` mediante adaptadores de enlace de sesión del canal antes de que se active `subagent_spawned`.
...
- `subagent_ended` contiene `targetSessionKey` (identidad; coincide con `subagent_spawned.childSessionKey`), `targetKind` (`"subagent"` o `"acp"`), `reason`, `outcome` opcional (`"ok"`, `"error"`, `"timeout"`, `"killed"`, `"reset"` o `"deleted"`), `error` opcional, `runId`, `endedAt`, `accountId` y `sendFarewell`. **No** incluye `agentId` ni `childSessionKey`; use `targetSessionKey` para correlacionarlo con el evento `subagent_spawned` correspondiente.

**Ciclo de vida**

| Enlace                             | Propósito                                                                                              |
| -------------------------------- | ---------------------------------------------------------------------------------------------------- |
| `gateway_start` / `gateway_stop` | Iniciar o detener con el Gateway los servicios propiedad del plugin                                  |
| `deactivate`                     | Alias de compatibilidad obsoleto de `gateway_stop`; use `gateway_stop` en plugins nuevos             |
| `cron_reconciled`                | Conciliar con el estado completo de Cron del Gateway tras el inicio o la recarga                      |
| `cron_changed`                   | Observar los cambios del ciclo de vida de Cron gestionados por el Gateway (añadido, actualizado, eliminado, iniciado, finalizado, programado) |
| **`before_install`**             | Inspeccionar el material preparado para instalar una Skill o un plugin desde un entorno de ejecución de plugin cargado |
...
pendiente. El enlace se ejecuta cuando se crea la solicitud; la entrega mediante el canal de
la respuesta de vinculación no se retrasa por controladores de enlaces lentos o con errores.
...
El enlace es solo de observación. No aprueba, rechaza, suprime ni reescribe
la respuesta de vinculación. La carga incluye el canal, `accountId` opcional,
`senderId` específico del canal, el `code` de vinculación y los metadatos del canal. Trate el
código de vinculación como una credencial activa de aprobación de un solo uso y entréguelo únicamente a un
destino de operador de confianza. Trate `metadata` como texto de identidad no confiable
proporcionado por el remitente. El enlace no incluye el cuerpo ni los medios del mensaje entrante.

## Enlaces de depuración del entorno de ejecución

Use `before_model_resolve` para cambiar el proveedor o el modelo durante un turno del agente; se
...
Para demostrar el modelo efectivo de la sesión, inspeccione los registros del entorno de ejecución y, después,
use `openclaw sessions` o las superficies de sesión/estado del Gateway. Para depurar
las cargas del proveedor, inicie el Gateway con `--raw-stream` y
...
- `event.toolName`
- `event.params`
- `event.toolKind` y `event.toolInputKind` opcionales, discriminadores definidos de forma autoritativa por el host
  para herramientas que comparten nombres intencionadamente; por ejemplo, las llamadas externas
  `exec` en modo de código usan `toolKind: "code_mode_exec"` e incluyen
  `toolInputKind: "javascript" | "typescript"` cuando se conoce el lenguaje de
  entrada
- `event.derivedPaths` opcional, indicaciones de rutas de destino derivadas por el host con el mejor esfuerzo
  para envoltorios de herramientas conocidos como `apply_patch`; estas rutas pueden estar
  incompletas o sobreaproximar lo que la herramienta modificará realmente (por
...
type BeforeToolCallResult = {
  ...
  requireApproval?: {
    ...
    timeoutMs?: number;
    /** @deprecated Las aprobaciones sin resolver siempre se deniegan. */
...
Comportamiento de las protecciones para los enlaces de ciclo de vida con tipos:
...
- `requireApproval` pausa la ejecución del agente y solicita confirmación al usuario mediante las
  aprobaciones del plugin. `/approve` puede aprobar tanto las ejecuciones como las aprobaciones del plugin. En los
  retransmisores nativos `PreToolUse` del modo de informe del servidor de aplicaciones de Codex, esto se delega a la
...
Los plugins que necesitan una política a nivel de host pueden registrar políticas de herramientas de confianza con
`api.registerTrustedToolPolicy(...)`. Estas se ejecutan antes que los enlaces
`before_tool_call` ordinarios y antes que las decisiones normales de los enlaces. Las políticas de confianza
incluidas se ejecutan primero; las políticas de confianza de plugins instalados se ejecutan después, según el orden de carga
...
se rechazan antes del registro. Los ids de políticas pertenecen al ámbito del plugin que los registra,
...
### Enlace del entorno de ejecución

`resolve_exec_env` permite que los plugins aporten variables de entorno a las invocaciones de la herramienta
`exec` antes de ejecutar el comando. Recibe:
...
se ejecutan por orden de prioridad; los resultados posteriores sustituyen a los anteriores para la misma
clave.

La salida del enlace se filtra mediante la política de claves del entorno de ejecución del host antes
de combinarse. `PATH` siempre se descarta (la resolución de comandos y las comprobaciones
de binarios seguros dependen de ella). Se descartan las claves no válidas y las claves peligrosas que sustituyen valores del host, como `LD_*`,
`DYLD_*`, `NODE_OPTIONS`, las variables de proxy (`HTTP_PROXY`, `HTTPS_PROXY`,
`ALL_PROXY`, `NO_PROXY`) y las variables que sustituyen valores de TLS (`NODE_TLS_REJECT_UNAUTHORIZED`,
`SSL_CERT_FILE` y similares). El entorno filtrado del plugin se incluye
en los metadatos de aprobación/auditoría del Gateway y se reenvía a las solicitudes
de ejecución del host de Node.

### Persistencia de los resultados de herramientas

Los resultados de herramientas pueden incluir `details` estructurados para la representación en la interfaz, los diagnósticos,
el enrutamiento de medios o los metadatos propiedad del plugin. Trate `details` como metadatos del entorno de ejecución,
no como contenido del prompt:

- OpenClaw elimina `toolResult.details` antes de la reproducción para el proveedor y de la entrada de
  Compaction, para que los metadatos no se conviertan en contexto del modelo.
- Las entradas de sesión persistentes solo conservan `details` acotados. Los detalles de tamaño excesivo se
...
- `tool_result_persist` y `before_message_write` se ejecutan antes del límite
  final de persistencia. Mantenga pequeño el valor `details` devuelto y evite colocar
...
Use los enlaces específicos de cada fase para los plugins nuevos:
...
- `agent_turn_prepare`: recibe el prompt actual, los mensajes preparados de la sesión
  y cualquier inyección en cola de ejecución única extraída para esta sesión.
...
- `heartbeat_prompt_contribution`: se ejecuta únicamente para turnos de Heartbeat y devuelve
...
`before_agent_start` se mantiene por compatibilidad. Se recomienda usar los enlaces explícitos
...
`message` es el sustituto orientado al usuario. Solo se admiten los resultados `pass` y `block`;
las formas de decisión no compatibles se cierran de forma restrictiva.

Cuando se bloquea una ejecución, OpenClaw almacena únicamente el texto de sustitución en
`message.content`, junto con metadatos de bloqueo no sensibles, como el id del
plugin que realiza el bloqueo y la marca de tiempo. El texto original del usuario no se conserva en la transcripción
...
Para la observabilidad deben usarse campos saneados, como el id del bloqueador, el resultado,
la marca de tiempo o una categoría segura.

`before_agent_start` y `agent_end` incluyen `event.runId` cuando OpenClaw puede
identificar la ejecución activa; el mismo valor también figura en `ctx.runId`. Las ejecuciones
iniciadas por Cron también exponen `ctx.jobId` (el id del trabajo de Cron de origen) en el contexto
del turno del agente, para que los enlaces puedan limitar las métricas, los efectos secundarios o el estado a un
trabajo programado específico. `ctx.jobId` no forma parte del contexto de herramienta `before_tool_call`.

Para las ejecuciones originadas en canales, `ctx.channel` y `ctx.messageProvider` identifican
...
- `ctx.senderId` - ID del remitente con ámbito de canal (p. ej., `open_id` de Feishu, ID de
  usuario de Discord). Se rellena cuando la ejecución se origina en un mensaje de usuario con
  metadatos conocidos del remitente.
- `ctx.chatId` - identificador de conversación nativo del transporte (p. ej.,
  `chat_id` de Feishu, `chat_id` de Telegram). Se rellena cuando el canal de origen
  proporciona un ID de conversación nativo.
- `ctx.channelContext.sender.id` - el mismo ID de remitente que `ctx.senderId`, dentro de
...
- `ctx.channelContext.chat.id` - el mismo ID de conversación que `ctx.chatId`,
  dentro de un objeto propiedad del canal que los plugins pueden ampliar con campos
  específicos del canal.

El núcleo solo define los campos `id` anidados. Los plugins de canal que pasan
metadatos más detallados del remitente o del chat mediante el asistente de entrada pueden ampliar
...
Los plugins de canal pasan esos campos mediante el asistente del SDK de entrada:
...
específicas del canal deben residir en `ctx.channelContext.sender` mediante la
ampliación de módulos.

`agent_end` es un hook de observación. Las rutas del Gateway y del arnés persistente lo ejecutan
sin esperar su resultado después del turno, mientras que las rutas efímeras de un solo uso de la CLI esperan
la promesa del hook antes de limpiar el proceso, para que los plugins de confianza puedan volcar
...
cancela el trabajo de red propiedad del plugin, a menos que este también utilice su propia señal
de cancelación.
...
solicitud ni ID de solicitudes del proveedor. Estos hooks incluyen metadatos estables como
...
hash acotado del ID de solicitud del proveedor. Cuando el entorno de ejecución ha resuelto
...
Los controladores tienen un presupuesto predeterminado de 15s; si se agota el tiempo, OpenClaw registra el fallo y
...
hacer que la pasada adicional del modelo sea acotada y segura frente a repeticiones:
...
Los hooks que modifican prompts y las inyecciones duraderas para el siguiente turno pueden deshabilitarse por
...
Los plugins de flujo de trabajo pueden conservar un pequeño estado de sesión compatible con JSON mediante
`api.session.state.registerSessionExtension(...)` y actualizarlo mediante el método
`sessions.pluginPatch` del Gateway. Las filas de sesión proyectan el estado de las
extensiones registradas mediante `pluginExtensions`, lo que permite que la interfaz de control y otros
...
comportamiento). OpenClaw consume las inyecciones en cola antes de los hooks de prompts, descarta
las inyecciones caducadas y elimina duplicados por `idempotencyKey` para cada plugin. Este es
el punto de integración adecuado para reanudar aprobaciones, resúmenes de políticas, cambios de monitores
...
La semántica de limpieza forma parte del contrato. Las devoluciones de llamada de limpieza de extensiones
de sesión y del ciclo de vida del entorno de ejecución reciben `reset`, `delete`, `disable` o
`restart`. El host elimina el estado persistente de la extensión de sesión
y las inyecciones pendientes para el siguiente turno del plugin propietario al restablecer, eliminar o deshabilitar; un reinicio
conserva el estado duradero de la sesión, mientras que las devoluciones de llamada de limpieza permiten a los plugins liberar
tareas del programador, contexto de ejecución y otros recursos externos de la generación
anterior del entorno de ejecución.

## Hooks de mensajes

Utilice hooks de mensajes para el enrutamiento y la política de entrega en el nivel del canal:
...
Para las respuestas TTS solo de audio, `content` puede contener la transcripción hablada
oculta, incluso cuando la carga útil del canal no tiene texto o subtítulo visible.
...
`replyToBody`, `replyToSender` y `replyToIsQuote`. Priorice estos
campos de primera clase antes de leer metadatos heredados.

Priorice los campos tipados `threadId` y `replyToId` antes de utilizar metadatos
específicos del canal.
...
- El valor `content` reescrito continúa hacia hooks de menor prioridad, salvo que un hook posterior
  cancele la entrega.
- `reply_payload_sending` se ejecuta después de normalizar la carga útil y antes de la entrega
  al canal, incluidas las respuestas redirigidas al canal de origen.
  Los controladores se ejecutan secuencialmente y cada uno ve la carga útil más reciente generada
  por los controladores de mayor prioridad.
- Las cargas útiles `reply_payload_sending` no exponen marcadores de confianza del entorno de ejecución como
  `trustedLocalMedia`; los plugins pueden editar la forma de la carga útil, pero no pueden otorgar confianza
  a los elementos multimedia locales.
- `message_sending` puede devolver `cancelReason` y un valor `metadata` acotado junto con una
  cancelación. Las nuevas API del ciclo de vida de los mensajes exponen esto como un resultado de
  entrega suprimida con el motivo `cancelled_by_message_sending_hook`; la entrega directa
...
Utilice `security.installPolicy` para las decisiones de permitir o bloquear que pertenecen al operador. Esa
política se ejecuta desde la configuración de OpenClaw, abarca las rutas de instalación y actualización de la CLI y
deniega de forma segura cuando está habilitada pero no está disponible.

`before_install` es un hook del ciclo de vida del entorno de ejecución de plugins. Se ejecuta después de
`security.installPolicy` solo en el proceso de OpenClaw donde los hooks de plugins ya se han
cargado, como en los flujos de instalación respaldados por el Gateway. Resulta útil para
observaciones, advertencias y comprobaciones de compatibilidad propiedad de los plugins, pero no es
...
`block: true` es terminal. `block: false` se trata como si no hubiera decisión. Los fallos de los
controladores bloquean la instalación de forma segura.

## Ciclo de vida del Gateway

Utilice `gateway_start` para iniciar servicios generales de plugins y `gateway_stop` para
limpiar recursos de larga duración. El programador de Cron aún puede estar cargándose cuando
se ejecuta `gateway_start`, por lo que no debe utilizarse como señal de referencia para una proyección
externa de Cron.
...
hayan conciliado su estado duradero. Se activa tanto durante el inicio
inicial como al reemplazar el programador durante una recarga de configuración. El evento informa de
`reason` (`startup` o `reload`) y del estado efectivo de `enabled`. Aunque Cron esté
deshabilitado, se emite con `enabled: false`, lo que permite a una proyección externa
eliminar activaciones obsoletas. Utilice `ctx.getCron?.()` para la instancia exacta del programador que
completó la conciliación; una recarga posterior no redirige esa devolución de llamada.
`ctx.abortSignal` pertenece a esa misma instantánea del programador. El Gateway la cancela en
cuanto se activa un programador más reciente o comienza el apagado. Pásela a todos los
efectos secundarios duraderos y no acepte la instantánea después de su cancelación.
Esta es una señal del ciclo de vida del programador, no una señal de activación de plugins: una
recarga en caliente exclusiva de plugins no vuelve a emitirla. Un consumidor recién habilitado recibe
su primera referencia en el siguiente reemplazo del programador o inicio del Gateway.

Al igual que otros hooks de observación, las devoluciones de llamada `gateway_start` y `cron_reconciled`
pueden solaparse. Si ambos controladores comparten la inicialización del plugin, coordínelos
con una promesa de disponibilidad local del plugin en lugar de depender del orden de las devoluciones de llamada.

`cron_changed` se activa para eventos del ciclo de vida de Cron gestionados por el Gateway con una carga útil de evento tipada
que abarca los motivos `added`, `updated`, `removed`, `started`, `finished`
...
son posteriores a la confirmación: solo se activan después de que la eliminación duradera se complete correctamente y siguen incluyendo
la instantánea de la tarea eliminada para que los planificadores externos puedan conciliar el estado.

Un evento `scheduled` es posterior a la confirmación: solo se activa después de que una escritura duradera correcta
cambie el `nextRunAtMs` efectivo de una tarea existente, sin incluir el evento explícito del ciclo de vida
`added`, `updated` o `removed` de esa tarea. El
`event.nextRunAtMs` de nivel superior es la próxima activación confirmada; cuando no está presente, la tarea
no tiene próxima activación. Trate estos eventos como indicaciones de conciliación, no como un registro
ordenado de cambios incrementales. Úselos como indicaciones combinables para volver a leer el planificador capturado por última vez por
`cron_reconciled`; no adopte el planificador de un contexto `cron_changed`.
...
Proyecte una instantánea completa de activaciones en lugar de reenviar los cambios incrementales de eventos de Cron. La
operación `replaceAll` del adaptador externo debe ser atómica e idempotente, y
solo debe resolverse después de que el host haya aceptado de forma duradera la instantánea. También debe
respetar la señal de cancelación proporcionada: si la señal se cancela antes de la aceptación
duradera, el adaptador no debe aceptar esa instantánea.

Este patrón mantiene en ejecución un solo proceso de trabajo con el estado más reciente. Solo `cron_reconciled`
adopta una instancia del planificador; `cron_changed` únicamente solicita que ese proceso de trabajo vuelva a leer
...
`replaceAll([])` y elimina las activaciones externas obsoletas. Los reintentos y la espera exponencial de este ejemplo
son locales al proceso y tratan los fallos del adaptador en tiempo de ejecución como transitorios; valide
la configuración que no admite reintentos antes del registro. OpenClaw no proporciona un
buzón de salida para los efectos de los hooks de los plugins. Si el proceso termina antes de la aceptación duradera,
el siguiente inicio del Gateway emite una nueva instantánea autoritativa `cron_reconciled`.
`gateway_stop` cancela el trabajo del host en curso, espera a que el proceso de trabajo finalice y después
cierra el adaptador.

## Próximas obsolescencias

Algunas superficies relacionadas con hooks están obsoletas, pero siguen siendo compatibles. Migre
...
- **`before_agent_start`** se mantiene por compatibilidad. Los plugins nuevos deben usar
  ...
  combinada.
- **`subagent_spawning`** se mantiene por compatibilidad con plugins antiguos, pero
  los plugins nuevos no deben devolver el enrutamiento de hilos desde él. El núcleo prepara
...
- **`deactivate`** se mantiene como alias de compatibilidad de limpieza obsoleto hasta
