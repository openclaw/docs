...
x-i18n:
    generated_at: "2026-07-19T02:03:03Z"
    model: gpt-5.6
...
canal de mensajería, un proveedor de modelos, un backend de CLI local, una herramienta de agente, un hook, un proveedor multimedia
...
Las especificaciones de paquetes sin prefijo se siguen instalando desde npm durante la transición del lanzamiento. Use el
prefijo `clawhub:` cuando desee la resolución mediante ClawHub.
...
- Para trabajar en plugins incluidos en el repositorio, clone el repositorio y ejecute `pnpm install`.
  El desarrollo de plugins desde una copia del código fuente solo admite pnpm porque OpenClaw detecta
...
<CardGroup cols={2}>
  ...
  <Card title="Plugin de backend de CLI" icon="terminal" href="/es/plugins/cli-backend-plugins">
    Ejecute una CLI de IA local mediante el mecanismo de respaldo de modelos de OpenClaw.
  </Card>
...
```
    ...
    compilados. Consulte [Puntos de entrada del SDK](/es/plugins/sdk-entrypoints) para conocer el contrato completo de los puntos
    de entrada.
    ...
    cargar anticipadamente la ejecución de cada plugin. Establezca `activation.onStartup`
    ...
    y `api.registerTrustedToolPolicy(...)` necesita que cada identificador de política figure en
  ...
  <Step title="Probar la instalación del paquete">
    ...
    `openclaw.extensions` a archivos JavaScript compilados como `./dist/index.js` y asegúrese
    de que `npm pack` incluya esa salida `dist/`. Las entradas de código fuente TypeScript
    son solo para copias del código fuente y rutas de desarrollo local.
    ...
    errores en las dependencias de ejecución que las pruebas desde una copia del código fuente pueden ocultar. Verifica
    ...
    No use una instalación directa desde un archivo o una ruta como verificación final del comportamiento
    oficial o privilegiado de un plugin. El código fuente directo es útil para la depuración local, pero
    no verifica la misma ruta de dependencias que las instalaciones desde npm o ClawHub. Si
    el plugin depende del estado de confianza de un plugin oficial, añada una segunda verificación
...
[Modo de código](/tools/code-mode) y [Búsqueda de herramientas](/es/tools/tool-search). Las llamadas al catálogo
rechazan los esquemas no válidos antes de la ejecución y validan el valor final después de
los hooks de la herramienta. Omítalo en herramientas que no tengan un resultado JSON estable. Consulte
...
Los usuarios conceden su aceptación mediante `tools.allow`:
...
del núcleo; los conflictos se omiten y se notifican en los diagnósticos del plugin. Los registros
con formato incorrecto se omiten y se notifican del mismo modo: un `name` no vacío ausente,
un `execute` que no sea una función o un descriptor de herramienta sin un objeto `parameters`.
...
metadatos informativos de ejecución, no como un límite de seguridad frente al operador
local, el código de plugins instalado o un entorno de ejecución de OpenClaw modificado. Las herramientas
locales sensibles deben seguir exigiendo una aceptación explícita del plugin o del operador y
cerrarse de forma segura cuando los metadatos del modelo activo falten o no sean adecuados.

El manifiesto declara la propiedad y la detección; la ejecución sigue llamando a la implementación
...
`openclaw/plugin-sdk/gateway-method-runtime` está reservado para las rutas HTTP de plugins
...
1. Siga las versiones de [openclaw/openclaw](https://github.com/openclaw/openclaw/releases) (`Watch` > `Releases`). Las etiquetas beta tienen un aspecto similar a `v2026.3.N-beta.1`. También puede seguir a [@openclaw](https://x.com/openclaw) en X para conocer los anuncios de versiones.
...
3. Después de probarlo, publique en el hilo de su plugin en el canal `plugin-forum` de Discord ([discord.gg/clawd](https://discord.gg/clawd)), ya sea `all good` o lo que haya dejado de funcionar. Cree un hilo si aún no tiene uno.
4. Si algo deja de funcionar, abra o actualice un issue titulado `Beta blocker: <plugin-name> - <summary>` y aplique la etiqueta `beta-blocker`. Enlace el issue en su hilo.
5. Abra un PR en `main` titulado `fix(<plugin-id>): beta blocker - <summary>` y enlace el issue tanto en el PR como en su hilo de Discord. Los colaboradores no pueden etiquetar PR, por lo que el título es la señal del PR para los mantenedores y la automatización. Los bloqueadores con un PR se fusionan; los que no lo tienen podrían publicarse de todos modos.
6. El silencio significa que todo está correcto. Si no se cumple el plazo, normalmente la corrección se incorpora en el siguiente ciclo.
