---
read_when:
    - Desea que un agente de OpenClaw se una a una llamada de Google Meet
...
- `bidi` sigue disponible como el modo alternativo directo de modelo de voz en tiempo real.
- Los agentes eligen el comportamiento de unión con `mode`: usa `agent` para escucha/respuesta hablada
  en vivo, `bidi` para el modo alternativo directo de voz en tiempo real, o `transcribe`
  para unirse/controlar el navegador sin el puente de respuesta hablada.
- La autenticación comienza como OAuth personal de Google o un perfil de Chrome que ya tenga sesión iniciada.
...
- Twilio acepta un número de marcado más un PIN o una secuencia DTMF opcionales; no
...
Instala las dependencias de audio locales y configura un proveedor de transcripción
en tiempo real más el TTS normal de OpenClaw. OpenAI es el proveedor de transcripción
predeterminado; Google Gemini Live también funciona como alternativa de voz `bidi`
separada con `realtime.voiceProvider: "google"`:
...
La salida de configuración está pensada para ser legible por agentes y consciente del modo. Informa el perfil de Chrome,
la fijación de nodo y, para uniones de Chrome en tiempo real, el puente de audio
BlackHole/SoX y las comprobaciones de introducción en tiempo real retrasadas. Para uniones solo de observación, comprueba el mismo
...
`--transport chrome-node` o `--transport twilio` para comprobar previamente un
...
de Webhook inaccesible antes de que el agente intente marcar la reunión.
...
actualmente depende de `BlackHole 2ch` en macOS. En Linux, usa `mode: "transcribe"`,
marcado de Twilio o un host `chrome-node` de macOS para la participación con respuesta hablada
de Chrome.
...
que la política sin llamada previa de la sala sea explícita en lugar de heredada de los
valores predeterminados de la cuenta de Google:

```bash
openclaw googlemeet create --access-type OPEN --transport chrome-node --mode agent
```

`OPEN` permite que cualquiera con la URL de Meet se una sin llamar. `TRUSTED` permite que los
usuarios de confianza de la organización anfitriona, los usuarios externos invitados y los usuarios de marcado
se unan sin llamar. `RESTRICTED` limita la entrada sin llamada previa a las personas invitadas. Estos
...
`meetings.space.settings` a la pantalla de consentimiento OAuth de Google.
...
- Alternativa del navegador: se usa cuando no hay credenciales OAuth. OpenClaw usa el
  ...
  nueva. La coincidencia ignora cadenas de consulta inocuas de URL como `authuser`, por lo que un
...
Para una unión solo de observación/control de navegador, establece `"mode": "transcribe"`. Eso no
...
la ruta sin micrófono y, en caso contrario, informa una acción manual en lugar de abrir
...
y una cola breve de `recentTranscript` para que los operadores puedan saber si el navegador
...
necesites una prueba de sí/no: se une en modo transcripción, espera movimiento fresco de subtítulos o
transcripción, y devuelve `listenVerified`, `listenTimedOut`, campos de acción
manual y el estado más reciente de subtítulos.
...
de entrada/salida, contadores de bytes y el estado cerrado del puente. Si aparece un aviso seguro
de una página de Meet, la automatización del navegador lo gestiona cuando puede. El inicio de sesión, la admisión del anfitrión y
...
solo dispositivo BlackHole basta para una primera prueba de humo, pero puede hacer eco.
...
host de nodo en la VM. Activa el Plugin incluido en la VM una vez para que el nodo
anuncie el comando de Chrome:

Qué se ejecuta dónde:

- Host de Gateway: OpenClaw Gateway, espacio de trabajo del agente, claves de modelo/API, proveedor en tiempo real
...
- No se necesita en la VM: servicio Gateway, configuración de agente, clave de OpenAI/GPT ni configuración
...
Instala o actualiza OpenClaw en la VM y luego activa allí el Plugin incluido:
...
WebSocket de texto claro a menos que aceptes explícitamente esa red privada de confianza:
...
`OPENCLAW_ALLOW_INSECURE_PRIVATE_WS=1` es entorno de proceso, no un ajuste de
...
Para una prueba de humo de un solo comando que crea o reutiliza una sesión, dice una
frase conocida e imprime la salud de la sesión:

```bash
openclaw googlemeet test-speech https://meet.google.com/abc-defg-hij
```

Durante la unión en tiempo real, la automatización del navegador de OpenClaw completa el nombre del invitado, hace clic en Unirse/Solicitar unirse y acepta la opción inicial de Meet "Usar micrófono" cuando aparece ese aviso. Durante la unión solo de observación o la creación de reuniones solo con navegador, continúa más allá del mismo aviso sin micrófono cuando esa opción está disponible. Si el perfil del navegador no tiene la sesión iniciada, Meet está esperando la admisión del anfitrión, Chrome necesita permiso de micrófono/cámara para una unión en tiempo real, o Meet está bloqueado en un aviso que la automatización no pudo resolver, el resultado de join/test-speech informa `manualActionRequired: true` con `manualActionReason` y `manualActionMessage`. Los agentes deben dejar de reintentar la unión, informar ese mensaje exacto junto con el `browserUrl`/`browserTitle` actual, y reintentar solo después de que se complete la acción manual en el navegador.

Si se omite `chromeNode.node`, OpenClaw selecciona automáticamente solo cuando exactamente un nodo conectado anuncia tanto `googlemeet.chrome` como control del navegador. Si hay varios nodos compatibles conectados, establece `chromeNode.node` en el id del nodo, el nombre para mostrar o la IP remota.

Comprobaciones comunes de fallos:

- `Configured Google Meet node ... is not usable: offline`: el nodo fijado es conocido por el Gateway pero no está disponible. Los agentes deben tratar ese nodo como estado de diagnóstico, no como un host de Chrome utilizable, e informar el bloqueo de configuración en lugar de recurrir a otro transporte salvo que el usuario lo haya pedido.
- `No connected Google Meet-capable node`: inicia `openclaw node run` en la VM, aprueba el emparejamiento y asegúrate de que `openclaw plugins enable google-meet` y `openclaw plugins enable browser` se ejecutaron en la VM. Confirma también que el host del Gateway permite ambos comandos de nodo con `gateway.nodes.allowCommands: ["googlemeet.chrome", "browser.proxy"]`.
- `BlackHole 2ch audio device not found`: instala `blackhole-2ch` en el host que se está comprobando y reinicia antes de usar el audio local de Chrome.
...
- Sin audio: en Meet, enruta el audio del micrófono/altavoz a través de la ruta del dispositivo de audio virtual usada por OpenClaw; usa dispositivos virtuales separados o enrutamiento al estilo Loopback para audio dúplex limpio.

## Notas de instalación

El valor predeterminado de respuesta de voz de Chrome usa dos herramientas externas:

- `sox`: utilidad de audio de línea de comandos. El Plugin usa comandos explícitos de dispositivos CoreAudio para el puente de audio PCM16 predeterminado de 24 kHz.
...
OpenClaw no incluye ni redistribuye ninguno de los paquetes. La documentación pide a los usuarios que los instalen como dependencias del host mediante Homebrew. SoX tiene licencia `LGPL-2.0-only AND GPL-2.0-only`; BlackHole es GPL-3.0. Si creas un instalador o appliance que incluye BlackHole con OpenClaw, revisa los términos de licencia originales de BlackHole o consigue una licencia separada de Existential Audio.

## Transportes

### Chrome

El transporte Chrome abre la URL de Meet mediante el control del navegador de OpenClaw y se une como el perfil de navegador de OpenClaw con sesión iniciada. En macOS, el Plugin comprueba `BlackHole 2ch` antes de iniciar. Si está configurado, también ejecuta un comando de salud del puente de audio y un comando de inicio antes de abrir Chrome. Usa `chrome` cuando Chrome/audio están en el host del Gateway; usa `chrome-node` cuando Chrome/audio están en un nodo emparejado como una VM de macOS en Parallels. Para Chrome local, elige el perfil con `browser.defaultProfile`; `chrome.browserProfile` se pasa a hosts `chrome-node`.
...
El transporte Twilio es un plan de marcado estricto delegado al Plugin Voice Call. No analiza páginas de Meet para buscar números de teléfono.

Úsalo cuando la participación con Chrome no esté disponible o quieras una alternativa de acceso telefónico. Google Meet debe exponer un número de acceso telefónico y un PIN para la reunión; OpenClaw no los descubre desde la página de Meet.
...
Reinicia o recarga el Gateway después de habilitar `voice-call`; los cambios de configuración del Plugin no aparecen en un proceso de Gateway que ya está en ejecución hasta que se recarga.
...
OAuth es opcional para crear un enlace de Meet porque `googlemeet create` puede recurrir a la automatización del navegador. Configura OAuth cuando quieras creación mediante API oficial, resolución de espacios o comprobaciones previas de la API de Meet Media.
...
OAuth no reemplaza la ruta de unión con Chrome. Los transportes Chrome y Chrome-node siguen uniéndose a través de un perfil de Chrome con sesión iniciada, BlackHole/SoX y un nodo conectado cuando usas participación con navegador. OAuth es solo para la ruta oficial de la API de Google Meet: crear espacios de reunión, resolver espacios y ejecutar comprobaciones previas de la API de Meet Media.
...
`meetings.space.created` es requerido por Google Meet `spaces.create`. `meetings.space.readonly` permite a OpenClaw resolver URLs/códigos de Meet en espacios. `meetings.space.settings` permite a OpenClaw pasar ajustes de `SpaceConfig` como `accessType` durante la creación de salas mediante API. `meetings.conference.media.readonly` es para comprobaciones previas de la API de Meet Media y trabajo multimedia; Google puede requerir inscripción en Developer Preview para el uso real de la Media API. Si solo necesitas uniones con Chrome basadas en navegador, omite OAuth por completo.
...
El comando imprime un bloque de configuración `oauth` con un token de actualización. Usa PKCE, callback de localhost en `http://localhost:8085/oauth2callback` y un flujo manual de copiar/pegar con `--manual`.
...
Prefiere variables de entorno cuando no quieras el token de actualización en la configuración. Si hay valores tanto de configuración como de entorno, el Plugin resuelve primero la configuración y luego usa el entorno como alternativa.
...
Ejecuta el doctor de OAuth cuando quieras una comprobación rápida de salud sin secretos:
...
`doctor --oauth --meeting` y `resolve-space` comprueban el acceso de lectura a un espacio existente
al que puede acceder la cuenta de Google autorizada. Un `403` de estas comprobaciones
...
No se necesitan credenciales OAuth para el modo alternativo del navegador. En ese modo, la autenticación de Google
proviene del perfil de Chrome con sesión iniciada en el Node seleccionado, no de la configuración de
OpenClaw.
...
Está separado de `googlemeet leave`: `leave` detiene la participación local/de sesión de
OpenClaw, mientras que `end-active-conference` le pide a Google Meet que finalice la conferencia activa
del espacio.
...
transcripciones, entradas de transcripción estructuradas y notas inteligentes cuando
...
filas de sesiones de participante con horas de primera/última vista, duración total de sesión,
indicadores de llegada tarde/salida anticipada y recursos de participante duplicados combinados por usuario
con sesión iniciada o nombre visible. Pasa `--no-merge-duplicates` para mantener separados los recursos
de participante sin procesar, `--late-after-minutes` para ajustar la detección de llegada tarde, y
...
a la carpeta. Pasa `--include-doc-bodies` para exportar texto de Google Docs de transcripciones y
...
Configura `"dryRun": true` para devolver solo el manifiesto de exportación y omitir escrituras de archivos.
...
Para la validación de escuchar primero, los agentes deben usar `test_listen` antes de afirmar que la
reunión es útil:
...
Ejecuta la prueba en vivo de escuchar primero en el navegador contra una reunión en la que alguien vaya a
...
- `OPENCLAW_GOOGLE_MEET_REFRESH_TOKEN` or `GOOGLE_MEET_REFRESH_TOKEN` proporciona
...
necesita `https://www.googleapis.com/auth/calendar.events.readonly`. La exportación del cuerpo de documentos de Drive necesita
...
El comando imprime el nuevo `meeting uri`, la fuente y la sesión de unión. Con credenciales OAuth
usa la API oficial de Google Meet. Sin credenciales OAuth, usa como alternativa
el perfil de navegador con sesión iniciada del Node Chrome fijado. Los agentes pueden
usar la herramienta `google_meet` con `action: "create"` para crear y unirse en un solo
paso. Para crear solo la URL, pasa `"join": false`.

Ejemplo de salida JSON del modo alternativo del navegador:
...
Si el modo alternativo del navegador encuentra un bloqueo de inicio de sesión de Google o de permisos de Meet antes de
poder crear la URL, el método Gateway devuelve una respuesta fallida y la herramienta
...
`manualActionMessage` más el contexto de Node/pestaña del navegador y dejar de abrir nuevas
...
Crear un Meet se une de forma predeterminada. El transporte de Chrome o Chrome-node aún
...
- `defaultMode: "agent"` (`"realtime"` solo se acepta como alias de compatibilidad
...
- `chrome.guestName: "OpenClaw Agent"`: nombre usado en la pantalla de invitado de Meet con sesión
  cerrada
- `chrome.autoJoin: true`: relleno del nombre de invitado y clic en Unirse ahora con el mayor esfuerzo
...
- `chrome.reuseExistingTab: true`: activa una pestaña existente de Meet en lugar de
  abrir duplicados
- `chrome.waitForInCallMs: 20000`: espera a que la pestaña de Meet informe que está en la llamada
...
- `chrome.audioBufferBytes: 4096`: búfer de procesamiento de SoX para comandos de audio
  ...
  Los valores inferiores al mínimo de SoX se limitan a 17 bytes.
...
- `mode: "agent"`: modo predeterminado de respuesta hablada. El habla de los participantes se transcribe con
  ...
  tiempo de ejecución normal de TTS de OpenClaw.
- `mode: "bidi"`: modo de modelo en tiempo real bidireccional directo de reserva. El
...
- `realtime.provider: "openai"`: reserva de compatibilidad usada cuando no están establecidos los campos
  de proveedor con ámbito de abajo.
...
- `realtime.voiceProvider`: id de proveedor usado por el modo `bidi` para voz en tiempo real
  directa. Establécelo en `"google"` para usar Gemini Live manteniendo la transcripción
  en modo de agente en OpenAI.
...
directivas por respuesta `[[tts:voiceId=... model=eleven_v3]]` cuando las sustituciones del modelo de TTS
están habilitadas, pero la configuración es el valor predeterminado determinista para las reuniones.
Al unirse, los registros deben mostrar `transcriptionProvider=elevenlabs` y cada
respuesta hablada debe registrar `provider=elevenlabs model=eleven_v3 voice=<voiceId>`.
...
llamada PSTN real, DTMF y el saludo de introducción al Plugin Voice Call. Voice Call
...
configurado produce la respuesta y el TTS normal de OpenClaw la pronuncia en Meet. Usa
`mode: "bidi"` cuando quieras que el modelo de voz en tiempo real responda directamente.
El `mode: "realtime"` sin procesar sigue aceptándose como alias de compatibilidad heredado para
`mode: "agent"`, pero ya no se anuncia en el esquema de herramientas del agente.
Los registros del modo de agente incluyen el proveedor/modelo de transcripción resuelto al iniciar
el puente y el proveedor, modelo, voz, formato de salida y frecuencia de muestreo de TTS después
...
activar una frase conocida y devolver la salud `inCall` cuando el host de Chrome pueda
informarla. `test_speech` siempre fuerza `mode: "agent"` y falla si se le pide
ejecutarse en `mode: "transcribe"` porque las sesiones solo observación no pueden emitir
voz intencionalmente. Su resultado `speechOutputVerified` se basa en el aumento de bytes de salida de audio
en tiempo real durante esta llamada de prueba, por lo que una sesión reutilizada con audio anterior
no cuenta como una comprobación de voz correcta reciente. Usa `action: "leave"` para marcar
...
- `micMuted`: estado del micrófono de Meet con el mayor esfuerzo
- `manualActionRequired` / `manualActionReason` / `manualActionMessage`: el
  perfil del navegador necesita inicio de sesión manual, admisión del host de Meet, permisos o
...
- `speechReady` / `speechBlockedReason` / `speechBlockedMessage`: si
  ...
  envió la introducción/frase de prueba al puente de audio.
...
pronuncia mediante el tiempo de ejecución normal de TTS de OpenClaw. Establece `mode: "bidi"` cuando quieras
...
suprime mientras el audio del asistente en cola sigue reproduciéndose,
y los ecos recientes de transcripciones similares al asistente se ignoran antes de la consulta al agente
...
| ------- | -------------------------------- | ---------------------------------------- | ----------------------------------------------------- |
| `agent` | El agente de OpenClaw configurado | Tiempo de ejecución normal de TTS de OpenClaw | Quieres el comportamiento de "mi agente está en la reunión" |
| `bidi`  | El modelo de voz en tiempo real   | Respuesta de audio del proveedor de voz en tiempo real | Quieres el bucle de voz conversacional con la menor latencia |
...
La herramienta de consulta ejecuta el agente normal de OpenClaw en segundo plano con contexto reciente de la transcripción de la reunión y devuelve una respuesta hablada concisa. En modo `agent`, OpenClaw envía esa respuesta directamente al runtime de TTS; en modo `bidi`, el modelo de voz en tiempo real puede decir el resultado de la consulta de vuelta en la reunión. Usa la misma maquinaria compartida de consulta que Voice Call.

De forma predeterminada, las consultas se ejecutan contra el agente `main`. Configura `realtime.agentId` cuando una vía de Meet deba consultar un espacio de trabajo de agente de OpenClaw dedicado, valores predeterminados de modelo, política de herramientas, memoria e historial de sesión.

Las consultas en modo agente usan una clave de sesión `agent:<id>:subagent:google-meet:<session>` por reunión, para que las preguntas de seguimiento conserven el contexto de la reunión mientras heredan la política normal de agente del agente configurado.
...
La clave de sesión de consulta está limitada a cada sesión de Meet, por lo que las llamadas de consulta de seguimiento pueden reutilizar el contexto previo de consulta durante la misma reunión.
...
- `googlemeet setup` aparece completamente en verde.
- `googlemeet setup` incluye `chrome-node-connected` cuando Chrome-node es el transporte predeterminado o se fija un nodo.
...
Para un host remoto de Chrome, como una VM de macOS en Parallels, esta es la comprobación segura más corta después de actualizar el Gateway o la VM:
...
Para una prueba de humo de Twilio, usa una reunión que exponga detalles de llamada telefónica:
...
- `openclaw logs --follow` muestra el DTMF TwiML servido antes del TwiML en tiempo real y luego un puente en tiempo real con el saludo inicial en cola.
...
Si acabas de editar `plugins.entries.google-meet`, reinicia o recarga el Gateway. El agente en ejecución solo ve herramientas de Plugin registradas por el proceso actual del Gateway.

En hosts de Gateway que no son macOS, la herramienta `google_meet` orientada al agente permanece visible, pero las acciones locales de respuesta por voz de Chrome se bloquean antes de llegar al puente de audio. El audio local de respuesta por voz de Chrome actualmente depende de `BlackHole 2ch` de macOS, por lo que los agentes Linux deben usar `mode: "transcribe"`, llamada telefónica de Twilio o un host `chrome-node` de macOS en lugar de la ruta predeterminada de agente con Chrome local.
...
Si `googlemeet setup` falla en `chrome-node-connected` o el registro del Gateway informa `gateway token mismatch`, reinstala o reinicia el nodo con el token actual del Gateway. Para un Gateway en LAN, esto normalmente significa:
...
### La creación de reunión falla

`googlemeet create` usa primero el endpoint `spaces.create` de la API de Google Meet cuando hay credenciales OAuth configuradas. Sin credenciales OAuth, recurre al navegador del nodo de Chrome fijado. Confirma:

- Para la creación por API: `oauth.clientId` y `oauth.refreshToken` están configurados, o existen variables de entorno `OPENCLAW_GOOGLE_MEET_*` coincidentes.
- Para la creación por API: el token de actualización se emitió después de que se añadiera el soporte de creación. Los tokens anteriores pueden no tener el alcance `meetings.space.created`; vuelve a ejecutar `openclaw googlemeet auth login --json` y actualiza la configuración del Plugin.
- Para el respaldo del navegador: `defaultTransport: "chrome-node"` y `chromeNode.node` apuntan a un nodo conectado con `browser.proxy` y `googlemeet.chrome`.
- Para el respaldo del navegador: el perfil de Chrome de OpenClaw en ese nodo ha iniciado sesión en Google y puede abrir `https://meet.google.com/new`.
- Para el respaldo del navegador: los reintentos reutilizan una pestaña existente de `https://meet.google.com/new` o de solicitud de cuenta de Google antes de abrir una pestaña nueva. Si un agente agota el tiempo, reintenta la llamada a la herramienta en lugar de abrir manualmente otra pestaña de Meet.
- Para el respaldo del navegador: si la herramienta devuelve `manualActionRequired: true`, usa los valores devueltos `browser.nodeId`, `browser.targetId`, `browserUrl` y `manualActionMessage` para guiar al operador. No reintentes en bucle hasta que esa acción esté completa.
- Para el respaldo del navegador: si Meet muestra "Do you want people to hear you in the meeting?", deja la pestaña abierta. OpenClaw debería hacer clic en **Use microphone** o, para el respaldo solo de creación, **Continue without microphone** mediante automatización del navegador y seguir esperando la URL de Meet generada. Si no puede hacerlo, el error debe mencionar `meet-audio-choice-required`, no `google-login-required`.
...
Usa `mode: "agent"` para la ruta normal STT -> agente de OpenClaw -> respuesta por voz TTS, o `mode: "bidi"` para el respaldo directo de voz en tiempo real. `mode: "transcribe"` intencionalmente no inicia el puente de respuesta por voz. Para depuración solo de observación, ejecuta `openclaw googlemeet status --json <session-id>` después de que hablen los participantes y comprueba `captioning`, `transcriptLines` y `lastCaptionText`. Si `inCall` es true pero `transcriptLines` permanece en `0`, es posible que los subtítulos de Meet estén deshabilitados, que nadie haya hablado desde que se instaló el observador, que la interfaz de Meet haya cambiado o que los subtítulos en vivo no estén disponibles para el idioma o la cuenta de la reunión.
...
- Hay disponible una clave de proveedor en tiempo real en el host del Gateway, como `OPENAI_API_KEY` o `GEMINI_API_KEY`.
...
Si un agente agotó el tiempo y puedes ver una pestaña de Meet ya abierta, inspecciona esa pestaña sin abrir otra:
...
Las URL de loopback y privadas no son válidas para callbacks de operadores telefónicos. No uses `localhost`, `127.0.0.1`, `0.0.0.0`, `10.x`, `172.16.x`-`172.31.x`, `192.168.x`, `169.254.x`, `fc00::/7` ni `fd00::/8` como `publicUrl`.
...
Añade `--yes` solo cuando quieras realizar intencionalmente una llamada de notificación
saliente real:
...
Confirma que el evento de Meet expone los detalles de llamada telefónica. Pasa el número
de acceso telefónico y el PIN exactos, o una secuencia DTMF personalizada:
...
- Ejecuta `openclaw googlemeet doctor <session-id>` para confirmar el ID de llamada
  delegado de Twilio, si DTMF se puso en cola y si se solicitó el saludo introductorio.
- Ejecuta `openclaw voicecall status --call-id <id>` y confirma que la llamada sigue
  activa.
- Ejecuta `openclaw voicecall tail` y comprueba que los Webhooks de Twilio estén llegando
  al Gateway.
- Ejecuta `openclaw logs --follow` y busca la secuencia de Twilio Meet: Google
  Meet delega la entrada, Voice Call inicia el tramo telefónico, Google Meet espera
  `voiceCall.dtmfDelayMs`, envía DTMF con `voicecall.dtmf`, espera
  `voiceCall.postDtmfSpeechDelayMs` y luego solicita la voz introductoria con
  `voicecall.speak`.
- Vuelve a ejecutar `openclaw googlemeet setup --transport twilio`; se requiere una
  comprobación de configuración correcta, pero eso no demuestra que la secuencia del
  PIN de la reunión sea correcta.
- Confirma que el número de acceso telefónico pertenezca a la misma invitación y región
  de Meet que el PIN.
- Aumenta `voiceCall.dtmfDelayMs` si Meet responde lentamente o si la transcripción de
  la llamada todavía muestra la indicación que solicita un PIN después de enviar DTMF.
- Si el participante se une, pero no oyes el saludo, revisa
  ...
  la reproducción TTS mediante flujo multimedia o la alternativa `<Say>` de Twilio. Si la
  transcripción de la llamada todavía contiene "introduzca el PIN de la reunión", el tramo
  telefónico aún no se ha unido a la sala de Meet, por lo que los participantes de la
  reunión no oirán la voz.
...
La API multimedia oficial de Google Meet está orientada a la recepción, por lo que hablar en una llamada
de Meet aún necesita una ruta de participante. Este Plugin mantiene visible ese límite:
Chrome gestiona la participación en el navegador y el enrutamiento de audio local; Twilio gestiona
la participación mediante acceso telefónico.

Los modos de respuesta de voz de Chrome necesitan `BlackHole 2ch` más uno de estos elementos:
...
- `chrome.audioInputCommand` más `chrome.audioOutputCommand`: OpenClaw controla el
  ...
  de 24 kHz con `chrome.audioBufferBytes: 4096`; G.711 mu-law de 8 kHz sigue
...
Cuando un agente llama a la herramienta `google_meet` en modo agente, la sesión consultora
de la reunión bifurca la transcripción actual del llamador antes de responder al habla de los
participantes. La sesión de Meet sigue estando separada (`agent:<agentId>:subagent:google-meet:<sessionId>`)
...
hablar. Esto mantiene el habla humana por delante de la salida del asistente incluso cuando la
entrada loopback compartida de BlackHole se suprime temporalmente durante la reproducción del asistente.
...
de confianza explícita y no la apuntes a scripts de ubicaciones no confiables.

`googlemeet speak` activa el puente de audio de respuesta de voz activo para una sesión de
Chrome. `googlemeet leave` detiene ese puente. En sesiones de Twilio delegadas
