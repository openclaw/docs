---
read_when:
    - Se desea crear un Plugin sencillo de OpenClaw que solo añada herramientas de agente
    - Se quiere usar defineToolPlugin en lugar de escribir manualmente los metadatos del manifiesto del plugin
...
x-i18n:
    generated_at: "2026-07-22T20:05:36Z"
    model: gpt-5.6
...
metadatos del manifiesto que OpenClaw necesita para descubrir herramientas sin cargar
el código de ejecución del plugin.

Para plugins de proveedor, canal, hook, servicio o capacidades mixtas, comience en
...
- `typebox` en `dependencies` (no solo `devDependencies`; el plugin
  generado lo importa durante la ejecución).
...
| Indicador            | Predeterminado      | Efecto                                      |
| -------------------- | ------------------- | ------------------------------------------- |
| `--directory <path>` | `<id>`             | Directorio de salida                        |
| `--name <name>`      | `<id>` con mayúsculas iniciales | Nombre para mostrar                         |
...
lo bastante específicos como para evitar colisiones con herramientas del núcleo u otros plugins.
...
`toolMetadata.<tool>.optional` correspondiente, para que OpenClaw pueda detectar que la
...
Use `factory` cuando una herramienta necesite el contexto de herramientas de ejecución antes de poder
...
Las fábricas siguen declarando por adelantado un nombre fijo de herramienta. Use `definePluginEntry`
directamente cuando el plugin calcule nombres de herramientas dinámicamente o combine herramientas
con hooks, servicios, proveedores o comandos.

## Valores devueltos

`defineToolPlugin` envuelve los valores devueltos sin formato en el formato de resultado
de herramientas de OpenClaw:

- Devuelva una cadena cuando el modelo deba ver ese texto exacto.
...
esquema en una sugerencia de salida acotada con estilo de TypeScript. Esto permite que un modelo invoque y
transforme un resultado conocido en un solo programa, en lugar de gastar otro turno del modelo
observando su forma.
...
Un esquema no válido impide ejecutar la herramienta; una discrepancia en el resultado hace que falle la
llamada completada. Incluya todas las variantes de resultado que no lancen excepciones, incluidas las variantes
de error estructuradas, u omita el esquema cuando el resultado no sea estable. No incluya secretos
ni valores confidenciales en las descripciones del esquema, porque los metadatos de salida de confianza pueden
quedar visibles para el modelo.
Use `{ additionalProperties: false }` en las capas de objetos cuando quiera una sugerencia de salida
compacta y completa; los esquemas abiertos o truncados siguen disponibles mediante
...
independiente porque podría dejar de coincidir con la herramienta de ejecución.
...
codifique secretos directamente en el código fuente ni en ejemplos de documentación; use la configuración, variables de
...
cambiar el identificador, el nombre, la descripción, el esquema de configuración, la activación o los nombres de herramientas
del plugin:
...
manifiesto obsoleto puede hacer que una herramienta no aparezca durante el descubrimiento o que un error de registro
...
que los editores muestran como advertencias de migración. Para aplicarlas en CI, habilite una
regla con información de tipos como
[`@typescript-eslint/no-deprecated`](https://typescript-eslint.io/rules/no-deprecated/).
Oxlint no tiene información de tipos, por lo que no puede aplicar estas anotaciones. Por tanto, la estructura
`plugins init` generada no añade una configuración de lint para obsolescencias.

`plugins validate` comprueba que:

- `openclaw.plugin.json` existe y supera el cargador normal de manifiestos.
...
- `package.json` dirige `openclaw.extensions` a la entrada de entorno de ejecución seleccionada.
...
Las especificaciones simples de paquetes npm se siguen instalando desde npm durante la transición del lanzamiento, pero
ClawHub es la superficie preferida para descubrir y distribuir plugins de OpenClaw.
...
los metadatos del paquete con la entrada que se pretende distribuir.
