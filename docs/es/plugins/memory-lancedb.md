...
read_when:
    ...
    - Estás utilizando embeddings locales compatibles con OpenAI, como Ollama
...
x-i18n:
    generated_at: "2026-07-22T10:40:16Z"
    model: gpt-5.6
...
la ranura de memoria, ese plugin se deshabilita con una advertencia.
...
<Note>
`memory_recall` de LanceDB no recibe la autorización protegida para transcripciones privadas
que usa `memory.search.rememberAcrossConversations`. Use `autoRecall` de LanceDB
o su herramienta `memory_recall` mediante
[Active Memory avanzada](/es/concepts/active-memory#lancedb-memory).
`openclaw doctor` informa cuando Remember across conversations no está disponible
...
- **Ruta directa del cliente compatible con OpenAI**: deje `embedding.provider` sin establecer
  ...
  para un endpoint de embeddings compatible con OpenAI sin un adaptador de proveedor
  incluido.
...
proveedor compatible con embeddings, como `github-copilot` o `ollama`.
...
omite `encoding_format` en las solicitudes y acepta respuestas tanto de matrices de números de punto flotante como
de float32 codificado en base64, por lo que ambos formatos de respuesta funcionan sin configuración.
...
| Configuración           | Valor predeterminado | Intervalo                        | Se aplica a                                                 |
| ----------------- | ------- | ---------------------------- | ---------------------------------------------------------- |
...
integrados abarcan frases comunes sobre memoria en inglés, checo, chino, japonés y coreano
...
el listado, las consultas sin procesar y la eliminación aplican ese propietario antes de devolver
o modificar filas. Un agente con `memory.search.enabled: false` en su entrada `agents.entries.*`,
o que herede una búsqueda de nivel superior deshabilitada, tampoco recibe ninguna de las herramientas `memory_recall`, `memory_store`
o `memory_forget`, ni participa en la recuperación o captura automáticas,
incluso cuando están activadas las opciones `autoRecall`/`autoCapture` del plugin.
...
- `memory_forget`: elimina por `memoryId` o por `query` (elimina automáticamente una única
  coincidencia con una puntuación superior al 90%; de lo contrario, muestra los ID candidatos para resolver la ambigüedad).

## Almacenamiento

Los datos de LanceDB se almacenan de forma predeterminada en `~/.openclaw/memory/lancedb`. Anule este valor con `dbPath`:
...
El plugin mantiene una tabla de LanceDB y almacena en cada fila un propietario de agente normalizado.
Este es un límite de almacenamiento, no un filtro posterior a la búsqueda: la propiedad del agente se
aplica antes de la clasificación vectorial y se incluye en los predicados de listado, consulta, recuento
y eliminación. `ltm query --filter` acepta una comparación validada sobre las
columnas de salida públicas. El almacén construye esa comparación por separado del
predicado obligatorio del propietario, de modo que un filtro no puede ampliar la consulta a otro
agente.

Las bases de datos creadas antes de la propiedad por agente no tienen una procedencia de filas fiable.
Durante la actualización, `openclaw doctor --fix` asigna esas filas heredadas una sola vez al
agente predeterminado configurado. El acceso durante el entorno de ejecución se deniega de forma segura hasta que esa migración
...
paquete del plugin (no de la distribución principal de OpenClaw). El inicio del Gateway no repara
...
con Intel). En esa plataforma, el plugin registra durante la carga que LanceDB no está disponible;
