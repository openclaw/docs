...
x-i18n:
    generated_at: "2026-07-06T10:51:36Z"
    model: gpt-5.5
...
y las exclusiones de `files` del paquete npm raíz. Regénerala con:
...
Durante la transición de lanzamiento, las especificaciones ordinarias de paquetes sin prefijo siguen instalándose desde npm.
...
[Discord](/es/channels/discord), para añadir credenciales y configuración del canal. Consulta
[Gestionar plugins](/es/plugins/manage-plugins) para ver comandos de actualización, desinstalación y publicación.
...
- **[anthropic](/es/plugins/reference/anthropic)** (`@openclaw/anthropic-provider`) - incluido en OpenClaw. Añade compatibilidad con el proveedor de modelos Anthropic a OpenClaw.

- **[azure-speech](/es/plugins/reference/azure-speech)** (`@openclaw/azure-speech`) - incluido en OpenClaw. Conversión de texto a voz de Azure AI Speech (MP3, notas de voz Ogg/Opus nativas, telefonía PCM).
...
- **[codex-supervisor](/es/plugins/reference/codex-supervisor)** (`@openclaw/codex-supervisor`) - incluido en OpenClaw. Supervisa sesiones del servidor de aplicación de Codex desde OpenClaw.
...
- **[document-extract](/es/plugins/reference/document-extract)** (`@openclaw/document-extract-plugin`) - incluido en OpenClaw. Extrae texto e imágenes de página de respaldo desde adjuntos de documentos locales.
...
- **[llm-task](/es/plugins/reference/llm-task)** (`@openclaw/llm-task`) - incluido en OpenClaw. Herramienta de LLM genérica solo JSON para tareas estructuradas invocable desde flujos de trabajo.

- **[lmstudio](/es/plugins/reference/lmstudio)** (`@openclaw/lmstudio-provider`) - incluido en OpenClaw. Añade compatibilidad con el proveedor de modelos LM Studio a OpenClaw.
...
- **[memory-wiki](/es/plugins/reference/memory-wiki)** (`@openclaw/memory-wiki`) - incluido en OpenClaw. Compilador de wiki persistente y bóveda de conocimiento compatible con Obsidian para OpenClaw.
...
- **[microsoft-foundry](/es/plugins/reference/microsoft-foundry)** (`@openclaw/microsoft-foundry`) - incluido en OpenClaw. Añade compatibilidad con el proveedor de modelos Microsoft Foundry a OpenClaw.
...
- **[migrate-hermes](/es/plugins/reference/migrate-hermes)** (`@openclaw/migrate-hermes`) - incluido en OpenClaw. Importa configuración, memorias, skills y credenciales compatibles de Hermes a OpenClaw.
...
- **[mistral](/es/plugins/reference/mistral)** (`@openclaw/mistral-provider`) - incluido en OpenClaw. Añade compatibilidad con el proveedor de modelos Mistral a OpenClaw.
...
- **[nvidia](/es/plugins/reference/nvidia)** (`@openclaw/nvidia-provider`) - incluido en OpenClaw. Añade compatibilidad con el proveedor de modelos NVIDIA a OpenClaw.

- **[oc-path](/es/plugins/reference/oc-path)** (`@openclaw/oc-path`) - incluido en OpenClaw. Añade la CLI de rutas de openclaw para direccionamiento de archivos del espacio de trabajo `oc://`.
...
- **[open-prose](/es/plugins/reference/open-prose)** (`@openclaw/open-prose`) - incluido en OpenClaw. Paquete de Skills de OpenProse VM con un comando slash /prose.

- **[openai](/es/plugins/reference/openai)** (`@openclaw/openai-provider`) - incluido en OpenClaw. Añade compatibilidad con el proveedor de modelos OpenAI a OpenClaw.

- **[opencode](/es/plugins/reference/opencode)** (`@openclaw/opencode-provider`) - incluido en OpenClaw. Añade compatibilidad con el proveedor de modelos OpenCode a OpenClaw.

- **[opencode-go](/es/plugins/reference/opencode-go)** (`@openclaw/opencode-go-provider`) - incluido en OpenClaw. Añade compatibilidad con el proveedor de modelos OpenCode Go a OpenClaw.

- **[openrouter](/es/plugins/reference/openrouter)** (`@openclaw/openrouter-provider`) - incluido en OpenClaw. Añade compatibilidad con el proveedor de modelos OpenRouter a OpenClaw.

- **[policy](/es/plugins/reference/policy)** (`@openclaw/policy`) - incluido en OpenClaw. Añade comprobaciones de doctor respaldadas por políticas para la conformidad del espacio de trabajo.
...
- **[sglang](/es/plugins/reference/sglang)** (`@openclaw/sglang-provider`) - incluido en OpenClaw. Añade compatibilidad con el proveedor de modelos SGLang a OpenClaw.

- **[synthetic](/es/plugins/reference/synthetic)** (`@openclaw/synthetic-provider`) - incluido en OpenClaw. Añade compatibilidad con el proveedor de modelos Synthetic a OpenClaw.
...
- **[together](/es/plugins/reference/together)** (`@openclaw/together-provider`) - incluido en OpenClaw. Añade compatibilidad con el proveedor de modelos Together a OpenClaw.
...
- **[workboard](/es/plugins/reference/workboard)** (`@openclaw/workboard`) - incluido en OpenClaw. Panel de tablero de trabajo para incidencias y sesiones propiedad de agentes.
...
- **[codex](/es/plugins/reference/codex)** (`@openclaw/codex`) - npm; ClawHub. Plugin proveedor de modelos y arnés de servidor de aplicaciones Codex de OpenClaw con un catálogo GPT administrado por Codex.
...
- **[firecrawl](/es/plugins/reference/firecrawl)** (`@openclaw/firecrawl-plugin`) - npm; ClawHub: `clawhub:@openclaw/firecrawl-plugin`. Añade herramientas invocables por agentes. Añade compatibilidad con proveedor de obtención web. Añade compatibilidad con proveedor de búsqueda web.
...
- **[inworld](/es/plugins/reference/inworld)** (`@openclaw/inworld-speech`) - npm; ClawHub: `clawhub:@openclaw/inworld-speech`. Texto a voz en streaming de Inworld (MP3, OGG_OPUS, PCM telefónico).
...
- **[kimi](/es/plugins/reference/kimi)** (`@openclaw/kimi-provider`) - npm; ClawHub: `clawhub:@openclaw/kimi-provider`. Añade compatibilidad con el proveedor de modelos Kimi y Kimi Coding a OpenClaw.
...
- **[msteams](/es/plugins/reference/msteams)** (`@openclaw/msteams`) - npm; ClawHub. Plugin de canal Microsoft Teams de OpenClaw para conversaciones de bots.
...
- **[openshell](/es/plugins/reference/openshell)** (`@openclaw/openshell-sandbox`) - npm; ClawHub. Backend de sandbox de OpenClaw para la CLI NVIDIA OpenShell con espacios de trabajo locales replicados y ejecución de comandos SSH.
...
- **[qwen](/es/plugins/reference/qwen)** (`@openclaw/qwen-provider`) - npm; ClawHub: `clawhub:@openclaw/qwen-provider`. Añade compatibilidad con el proveedor de modelos Qwen, Qwen Cloud, Model Studio, DashScope, Qwen Oauth, Qwen Portal y Qwen CLI a OpenClaw.
...
- **[sms](/es/plugins/reference/sms)** (`@openclaw/sms`) - npm; ClawHub: `clawhub:@openclaw/sms`. Plugin de canal Twilio SMS para mensajes de texto de OpenClaw.

- **[stepfun](/es/plugins/reference/stepfun)** (`@openclaw/stepfun-provider`) - npm; ClawHub: `clawhub:@openclaw/stepfun-provider`. Añade compatibilidad con el proveedor de modelos StepFun y StepFun Plan a OpenClaw.
...
- **[tencent](/es/plugins/reference/tencent)** (`@openclaw/tencent-provider`) - npm; ClawHub: `clawhub:@openclaw/tencent-provider`. Añade compatibilidad con el proveedor de modelos Tencent TokenHub y TokenPlan a OpenClaw.
...
- **[voice-call](/es/plugins/reference/voice-call)** (`@openclaw/voice-call`) - npm; ClawHub. Plugin voice-call de OpenClaw para llamadas telefónicas de Twilio, Telnyx y Plivo.
...
- **[qa-channel](/es/plugins/reference/qa-channel)** (`@openclaw/qa-channel`) - solo checkout de código fuente. Añade la superficie QA Channel para enviar y recibir mensajes de OpenClaw.

- **[qa-lab](/es/plugins/reference/qa-lab)** (`@openclaw/qa-lab`) - solo checkout de código fuente. Plugin QA lab de OpenClaw con interfaz de depurador privada y ejecutor de escenarios.

- **[qa-matrix](/es/plugins/reference/qa-matrix)** (`@openclaw/qa-matrix`) - solo checkout de código fuente. Ejecutor y sustrato de transporte de QA de Matrix.
