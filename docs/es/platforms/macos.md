---
read_when:
    - Instalación de la app de macOS
    - Decidir entre el modo Gateway local y remoto en macOS
    - Buscando descargas de versiones de la app para macOS
summary: Instalar y usar la aplicación de barra de menú de macOS de OpenClaw
...
La app de macOS es el **complemento de la barra de menús** de OpenClaw: interfaz de bandeja nativa, solicitudes de permisos de macOS, notificaciones, WebChat, entrada de voz, Canvas y herramientas de Node alojadas en Mac, como `system.run`.

¿Solo necesitas la CLI y el Gateway? Empieza con [Primeros pasos](/es/start/getting-started).

## Descarga

Obtén las compilaciones de la app de macOS desde las [versiones de OpenClaw en GitHub](https://github.com/openclaw/openclaw/releases).
Cuando una versión incluya recursos de la app de macOS, busca:
...
no tiene ningún recurso de la app de macOS, usa la más reciente que sí lo tenga, o compila desde el código fuente con
[configuración de desarrollo de macOS](/es/platforms/mac/dev-setup).

## Primera ejecución

1. Instala e inicia **OpenClaw.app**.
2. Elige **This Mac** para un Gateway local, o conéctate a un Gateway remoto.
3. Modo local: espera mientras la app instala su entorno de ejecución en espacio de usuario y el Gateway.
4. Completa la configuración del proveedor y la lista de verificación de permisos de macOS.
...
| Remoto | Otro host ejecuta el Gateway; este Mac lo controla por SSH, LAN o Tailnet.      | [Control remoto](/es/platforms/mac/remote)            |

El modo local necesita una CLI `openclaw` instalada. En un Mac nuevo, la app instala
automáticamente la CLI y el entorno de ejecución correspondientes antes de iniciar el asistente de Gateway.
Consulta [Gateway en macOS](/es/platforms/mac/bundled-gateway) para la recuperación manual.
...
- Herramientas locales de Node: Canvas, captura de cámara/pantalla, notificaciones y `system.run`.
...
La app **no** sustituye al Gateway ni a la documentación general de la CLI. La
configuración del Gateway, proveedores, plugins, canales, herramientas y seguridad tienen su
...
| Mantener el estado fuera de carpetas sincronizadas en la nube | [Gateway en macOS](/es/platforms/mac/bundled-gateway#state-directory-on-macos) |
...
| Alojar PeekabooBridge para automatización de UI | [Puente Peekaboo](/es/platforms/mac/peekaboo)                                         |
...
| Inspeccionar comandos de Node de Mac e IPC de la app | [IPC de macOS](/es/platforms/mac/xpc)                                                  |
