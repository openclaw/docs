---
read_when:
    - Implementando OpenClaw en Northflank
    - Quieres una implementación en la nube de un clic con Control UI basada en navegador
summary: Implementar OpenClaw en Northflank con una plantilla de un clic
title: Northflank
x-i18n:
    generated_at: "2026-04-23T14:04:23Z"
    model: gpt-5.4
    provider: openai
...
Implementa OpenClaw en Northflank con una plantilla de un clic y accede a él a través de la Control UI web.
Esta es la ruta más fácil de "sin terminal en el servidor": Northflank ejecuta el Gateway por ti.
...
- Almacenamiento persistente mediante Northflank Volume (`/data`) para que `openclaw.json`,
  `auth-profiles.json` por agente, el estado de canales/proveedores, las sesiones y
  el espacio de trabajo sobrevivan a las reimplementaciones
