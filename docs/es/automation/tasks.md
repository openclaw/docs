...
read_when:
    ...
    - Depuración de errores de entrega en ejecuciones de agente desacopladas
...
- Las tareas cron permanecen activas mientras el runtime de cron aún posee el trabajo; si el
  estado del runtime en memoria ya no existe, el mantenimiento de tareas primero revisa el historial duradero de
...
- La finalización se impulsa por notificaciones push: el trabajo desacoplado puede notificar directamente o despertar la
  ...
  suelen tener una forma inadecuada.
...
<AccordionGroup>
  ...
  <Accordion title="Protección de video_generate concurrente">
...
- Tareas Cron: el runtime de cron ya no registra el trabajo como activo y el historial duradero de
...
- Tareas de la CLI: las tareas con un id de ejecución/id de origen usan el contexto de ejecución en vivo, por lo que
  las filas persistentes de sesión hija o sesión de chat no las mantienen activas después de que desaparece la
  ejecución propiedad del Gateway. Las tareas heredadas de la CLI sin identidad de ejecución aún recurren
  a la sesión hija. Las ejecuciones `openclaw agent` respaldadas por Gateway también finalizan
  a partir de su resultado de ejecución, por lo que las ejecuciones completadas no permanecen activas hasta que el proceso de barrido
  las marca como `lost`.
...
El Gateway mantiene acotado el registro de escritura adelantada de SQLite usando el umbral predeterminado de autocheckpoint de SQLite, además de checkpoints `TRUNCATE` periódicos y durante el apagado.
