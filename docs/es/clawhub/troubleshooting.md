...
La CLI inicia un servidor de devolución de llamada local de corta duración durante el inicio de sesión mediante el navegador.

- Asegúrese de que el navegador pueda acceder a `http://127.0.0.1:<port>/callback`.
- Compruebe las reglas locales del cortafuegos, la VPN y el proxy si la devolución de llamada nunca llega.
...
Si muchos usuarios comparten una dirección IP de salida, se pueden alcanzar los límites de IP anónima incluso cuando cada
persona solo envía unas pocas solicitudes. Inicie sesión cuando sea posible y vuelva a intentarlo después del
retraso indicado.
...
- Confirme que la versión sea pública y que no esté retenida por un análisis o por moderación.
...
Para los plugins, compruebe los metadatos de compatibilidad de `package.json`. Las publicaciones de plugins de código
...
publicando con el propietario que corresponde al espacio de nombres. En el caso de los paquetes de plugins,
...
no puede gestionar al propietario actual en ClawHub, abra una
...
Previsualice primero si no tiene claro qué se publicará:
...
- Publicar la copia modificada como un nuevo slug o una bifurcación.
...
- Dirija a los usuarios de vuelta al listado canónico de ClawHub.
- No replique contenido oculto, privado, retenido o bloqueado por moderación fuera de la
  superficie de la API pública.

Consulte [API HTTP](/clawhub/http-api) para obtener detalles de los endpoints.
