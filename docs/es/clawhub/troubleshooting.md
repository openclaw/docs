...
x-i18n:
    generated_at: "2026-07-03T09:22:12Z"
    model: gpt-5.5
...
- Revisa las reglas del firewall local, VPN y proxy si la devolución de llamada nunca llega.
...
## Buscar o instalar devuelve `Rate limit exceeded` (429)
...
Si muchos usuarios comparten una sola IP de salida, los límites de IP anónima pueden alcanzarse incluso cuando cada
persona solo envía unas pocas solicitudes. Inicia sesión cuando sea posible y reintenta después del
retraso indicado.

## Buscar o instalar falla detrás de un proxy
...
Si una publicación falla porque el handle del propietario, el namespace de la organización, el scope del paquete, el slug de la skill
o el nombre del paquete ya está reclamado o reservado, primero confirma que estás
publicando con el propietario que coincide con el namespace. Para paquetes de plugin,
los nombres con scope como `@example-org/example-plugin` deben publicarse como el
...
[issue de Reclamo de organización / namespace](https://github.com/openclaw/clawhub/issues/new?template=org-namespace-claim.yml)
con pruebas públicas y no sensibles. Consulta
[Reclamos de organización y namespace](/clawhub/namespace-claims) para obtener orientación sobre las pruebas y qué
...
- Si el paquete está oculto, retenido o bloqueado, quizá no pueda instalarse hasta que
  el propietario resuelva el problema.

## Las solicitudes de API pública fallan

- Respeta los encabezados de reintento de `429` y almacena en caché las respuestas públicas de lista/búsqueda.
