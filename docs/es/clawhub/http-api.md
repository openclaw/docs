...
x-i18n:
    generated_at: "2026-07-03T17:12:39Z"
    model: gpt-5.5
...
- Si no hay una IP de cliente confiable disponible, las solicitudes anónimas usan buckets de reserva
  ...
  rutas, slugs, nombres de paquete, versiones, cadenas de consulta u otros parámetros de
  artefacto suministrados por el llamador.
...
- La popularidad se escala logarítmicamente y se limita. Las Skills con alto engagement pueden clasificarse más abajo cuando el texto de consulta coincide peor.
- El estado de moderación sospechoso u oculto puede retirar una Skill de la búsqueda pública según los filtros del llamador y el estado actual de moderación.
...
- Los alias de renombrado preservan la resolución para URL antiguas e instalaciones que se resuelven mediante el registro, pero el ranking de búsqueda se basa en los metadatos canónicos de la Skill después de que el renombrado se haya indexado. Las estadísticas existentes permanecen con la Skill.
- Si una Skill está inesperadamente invisible, revisa primero el estado de moderación con `clawhub inspect @owner/slug` mientras has iniciado sesión antes de cambiar metadatos relacionados con el ranking.
...
`note` es obligatorio para `confirmed` y `dismissed`; se puede omitir cuando
se vuelve a establecer `status` en `open`. Pasa `finalAction: "hide"` con un reporte
...
Devuelve los detalles de verificación de escaneo de seguridad para una versión de Skill.
...
- Los escaneos publicados solo escriben los resultados cuando `update: true` y el escaneo se completa correctamente.
...
- Los trabajos de escaneo son asíncronos. Las solicitudes de escaneo manual se priorizan por delante del trabajo normal de publicación/rellenado, pero su finalización sigue dependiendo de la disponibilidad de los workers.
...
- `security.signals` contiene solo evidencia de respaldo a nivel de estado; usa `/scan` o la página de auditoría de seguridad de ClawHub para ver los detalles completos de los escáneres.
- `security.signals.dependencyRegistry` se conserva por compatibilidad con respuestas v1, pero el escáner de existencia del registro de dependencias está retirado y esta clave siempre es `null`.
...
- Usa `/verify` cuando necesites el sobre de verificación de Skill Card de un solo skill, `/card` cuando necesites Markdown de tarjeta generada y `/scan` cuando necesites datos detallados de escáneres.
...
- `category` (opcional): filtro de categoría de plugin. Solo se admite cuando la
  ...
  alias de filtro heredados de v1 se documentan en `GET /api/v1/plugins`.
...
Descarga el artefacto de versión mediante la ruta explícita del resolver.
...
Reporta un paquete para revisión de moderador. Los informes son de nivel de paquete, opcionalmente
...
- `revoked`: bloqueada después de que una versión se hubiera considerado de confianza previamente.
...
- Límite de tamaño de archivo: 200 KB.
- Los análisis de VirusTotal pendientes no bloquean las lecturas; las versiones maliciosas aún pueden retenerse en otro lugar.
...
Transmite los bytes exactos del tarball de ClawPack cargado para clientes del espejo npm.
...
- Las entregas de skills respaldadas por GitHub no actúan como proxy ni reflejan bytes. La respuesta JSON
...
- Los Plugins de código requieren `package.json`, metadatos del repositorio de origen, metadatos de commit
...
versión. Los nombres de paquetes se convierten en paquetes privados de marcador de posición sin filas de versión, para que el mismo
...
los alias de slug de skills, paquetes, advertencias del inspector de paquetes y filas derivadas del resumen de búsqueda para que
