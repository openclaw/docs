...
- Los endpoints de escritura autenticados no deben devolver un simple `Unauthorized` cuando
  el servidor conoce el motivo. Los tokens ausentes, tokens inválidos/revocados y
  cuentas eliminadas/prohibidas/deshabilitadas deben recibir texto accionable para que los clientes
...
- Si no hay una IP de cliente confiable disponible, las solicitudes anónimas usan buckets de respaldo
  ...
  rutas suministradas por el llamador, slugs, nombres de paquete, versiones, cadenas de consulta u otros
...
- Los resultados se devuelven en orden de relevancia (similitud de embeddings + refuerzos por coincidencia exacta de tokens de slug/nombre + un pequeño factor previo de popularidad).
...
- La popularidad se escala logarítmicamente y tiene un tope. Los Skills con mucha interacción pueden clasificarse más bajo cuando el texto de la consulta es una coincidencia más débil.
...
- Los slugs antiguos creados por flujos de cambio de nombre/fusión de propietarios se resuelven al Skill canónico.
...
establecer `status` de vuelta en `open`. Pasa `finalAction: "hide"` con un reporte
triaged para ocultar el Skill en el mismo flujo auditable.
...
- Los trabajos de escaneo son asíncronos. Las solicitudes de escaneo manual tienen prioridad sobre el trabajo normal de publicación/reposición, pero la finalización todavía depende de la disponibilidad de los workers.
...
- Requiere acceso de administración de propietario/publicador a la skill o al plugin, o autoridad de moderador/administrador de la plataforma.
...
- `kind` tiene el valor predeterminado `skill`; usa `kind=plugin` para escaneos de plugin/paquete.
...
- `security.signals.dependencyRegistry` se conserva por compatibilidad con respuestas v1, pero el escáner de existencia del registro de dependencias está retirado y esta clave siempre es `null`.
- La ausencia de Skill Card no afecta el `ok`, `decision` ni `reasons` de este endpoint; los clientes deben leer localmente el `skill-card.md` instalado cuando necesiten contenido de tarjeta.
- Usa `/verify` cuando necesites el sobre de verificación de Skill Card de una sola skill, `/card` cuando necesites Markdown de tarjeta generado y `/scan` cuando necesites datos detallados de escáner.
...
- Los llamadores autenticados pueden ver paquetes privados de los publicadores a los que pertenecen en los resultados de listas/búsqueda.
- `channel=private` solo devuelve paquetes que el llamador autenticado puede leer.

### `GET /api/v1/packages/search`

Búsqueda en el catálogo unificado entre Skills y paquetes de plugins.
...
- Las Skills alojadas incluyen los archivos de la última versión almacenada y se listan en
...
- Los controles de ordenación de la interfaz del navegador para la búsqueda de Plugins reordenan los resultados de relevancia cargados,
  de forma equivalente al comportamiento actual de exploración en `/skills`.
...
- Requiere un token de API del propietario del paquete, un propietario/administrador de la organización editora,
...
- Los lanzamientos de ClawPack incluyen los campos compatibles con npm `npmIntegrity`, `npmShasum` y
  `npmTarballName`.
- `version.sha256hash` son metadatos de compatibilidad obsoletos para clientes antiguos. Genera el hash
...
Devuelve el resumen exacto de seguridad y confianza del lanzamiento del paquete para clientes
de instalación. Esta es la superficie pública de consumo de OpenClaw para decidir si un
...
- Este endpoint es exacto por versión. Los clientes deben llamarlo después de resolver la
  ...
  del paquete.
- Los paquetes privados devuelven `404` salvo que el llamador pueda leer el editor propietario.
- Este endpoint es intencionalmente más limitado que los endpoints de moderación
  de propietarios/moderadores. Expone la decisión de instalación y la explicación pública, no
  identidades de denunciantes, cuerpos de informes, evidencias privadas ni cronologías internas
  de revisión.
...
- Las versiones de paquetes heredados devuelven un artefacto `legacy-zip` y una URL
...
- Las versiones de ClawPack transmiten los bytes `.tgz` exactos subidos de npm-pack.
...
- `open`: lanzamientos sospechosos, maliciosos, pendientes, en cuarentena, revocados o denunciados.
...
- `all`: cualquier lanzamiento con una anulación manual, estado de análisis no limpio o informe de paquete.
...
bloquean automáticamente las descargas; los moderadores deben usar la moderación de lanzamientos para
...
- Las comprobaciones de moderación y acceso a paquetes privados aún se aplican.
...
- Campo de carga útil opcional: `migrateOwner`. Cuando es `true` con `ownerHandle`, una
  ...
  los publicadores actuales como de destino. Sin esta adhesión explícita, los cambios de propietario se
  rechazan.
...
- Los campos de formulario permitidos son `payload`, blobs `files` repetidos o una referencia de tarball
  ...
  el flujo de upload-url. Las publicaciones con id de almacenamiento preparadas también deben incluir el
...
Para una org recién creada, proporciona `memberHandle`; el administrador que actúa no se agrega como miembro.
...
llamador como propietario. Este endpoint no migra identificadores de usuarios/personales existentes y no
...
versión. Los nombres de paquetes se convierten en paquetes de marcador privado sin filas de versión, para que el mismo
...
Bloquea a un usuario y elimina de forma definitiva las Skills de su propiedad (solo moderador/admin).
...
Desbloquea a un usuario y restaura las Skills aptas (solo admin).
...
Si alojas el servicio por tu cuenta, sirve este archivo (o establece `CLAWHUB_REGISTRY` explícitamente; `CLAWDHUB_REGISTRY` heredado).
