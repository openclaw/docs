...
x-i18n:
    generated_at: "2026-07-04T03:34:47Z"
    model: gpt-5.5
...
- Solicitudes autenticadas (token Bearer válido): se aplican por cubo de usuario.
...
- Los endpoints de escritura autenticados no deberían devolver un `Unauthorized` sin más cuando
  ...
  las cuentas eliminadas, bloqueadas o deshabilitadas deberían recibir texto accionable para que los clientes
...
- Si falta `Retry-After`, usa como alternativa `RateLimit-Reset` (o calcula a partir de `X-RateLimit-Reset`).
...
- ClawHub usa encabezados de reenvío de confianza para identificar las IP de clientes en el borde.
- Si no hay una IP de cliente de confianza disponible, las solicitudes anónimas usan cubos alternativos
  delimitados solo por el tipo de límite de tasa. Estos cubos alternativos no incluyen
...
- El texto ASCII se tokeniza en límites de palabras y puntuación. Por ejemplo, `personal-map` contiene un token independiente `map`, mientras que `amap-jsapi-skill` contiene `amap`, `jsapi` y `skill`; por tanto, buscar `map` da a `personal-map` una coincidencia léxica más fuerte que a `amap-jsapi-skill`.
- La popularidad se escala logarítmicamente y se limita. Los Skills con alta interacción pueden clasificarse más abajo cuando el texto de la consulta coincide peor.
...
- Si un Skill está inesperadamente invisible, revisa primero el estado de moderación con `clawhub inspect @owner/slug` mientras tienes sesión iniciada antes de cambiar metadatos relacionados con la clasificación.
...
- Los slugs antiguos creados por flujos de cambio de nombre o fusión de propietario se resuelven al Skill canónico.
...
- `moderation` se incluye solo cuando el Skill está marcado o lo está viendo el propietario.
...
Reporta un Skill para revisión por moderadores. Los reportes son a nivel de Skill, opcionalmente enlazados
...
- Las cargas útiles de solicitud de escaneo y los informes descargables caducan en el almacén de solicitudes de escaneo después de la ventana de retención.
...
- Los escaneos publicados escriben los resultados de vuelta solo cuando `update: true` y el escaneo se completa correctamente.
...
- Los trabajos de escaneo son asíncronos. Las solicitudes de escaneo manual tienen prioridad sobre el trabajo normal de publicación/rellenado, pero la finalización sigue dependiendo de la disponibilidad de los workers.

### `GET /api/v1/skills/-/scan/{scanId}`

Endpoint de sondeo autenticado para un escaneo enviado.
...
- `ok` es `true` solo cuando la versión seleccionada tiene una Skill Card generada, no está bloqueada por malware mediante moderación y la verificación de ClawScan es limpia.
...
- `security.signals` contiene evidencia de respaldo de escáneres, como `staticScan`, `virusTotal` y `skillSpector`.
...
- Los resultados son por elemento; un skill o versión ausente no hace fallar toda la respuesta.
...
- `security.signals` contiene solo evidencia de respaldo a nivel de estado; usa `/scan` o la página de auditoría de seguridad de ClawHub para ver todos los detalles de los escáneres.
- `security.signals.dependencyRegistry` se conserva por compatibilidad con respuestas v1, pero el escáner de existencia del registro de dependencias está retirado y esta clave siempre es `null`.
- La ausencia de Skill Card no afecta a `ok`, `decision` ni `reasons` de este endpoint; los clientes deben leer el `skill-card.md` instalado localmente cuando necesiten contenido de tarjeta.
- Usa `/verify` cuando necesites el sobre de verificación de Skill Card de un solo skill, `/card` cuando necesites Markdown de tarjeta generado y `/scan` cuando necesites datos detallados de escáneres.
...
- Los clientes anónimos solo ven canales de paquetes públicos.
- Los clientes autenticados pueden ver paquetes privados de publicadores a los que pertenecen en los resultados de listado/búsqueda.
- `channel=private` solo devuelve paquetes que el cliente autenticado puede leer.

### `GET /api/v1/packages/search`

Búsqueda unificada de catálogo en Skills + paquetes de plugins.

Parámetros de consulta:

- `q` (requerido): cadena de consulta
- `limit` (opcional): entero (1–100)
- `family` (opcional): `skill`, `code-plugin` o `bundle-plugin`
- `channel` (opcional): `official`, `community` o `private`
- `isOfficial` (opcional): `true` o `false`
...
- Los clientes anónimos solo ven canales de paquetes públicos.
- Los clientes autenticados pueden buscar paquetes privados de publicadores a los que pertenecen.
- `channel=private` solo devuelve paquetes que el cliente autenticado puede leer.

### `GET /api/v1/plugins`

Exploración del catálogo solo de plugins entre paquetes code-plugin y bundle-plugin.
...
- `startDate` (requerido): límite inferior en milisegundos Unix para `updatedAt` de la Skill.
- `endDate` (requerido): límite superior en milisegundos Unix para `updatedAt` de la Skill.
...
- El filtrado por categoría es un filtro de API real respaldado por filas de
  resumen de categoría de Plugin, no una reescritura de consulta de búsqueda.
- Los resultados se devuelven en orden de relevancia y actualmente no se paginan.
- Los controles de ordenación de la interfaz de navegador para la búsqueda de Plugin reordenan los resultados de relevancia cargados,
...
- Requiere un token de API del propietario del paquete, un propietario/administrador del publicador de organización,
...
- Las versiones de ClawPack incluyen campos compatibles con npm `npmIntegrity`, `npmShasum` y
  `npmTarballName`.
- `version.sha256hash` es metadato de compatibilidad obsoleto para clientes antiguos. Este
  hashea los bytes ZIP exactos devueltos por `/api/v1/packages/{name}/download`.
...
- `trust.reasons` es la lista de explicación orientada al usuario y de auditoría. Los códigos de razón
...
- `trust.stale` significa que el resumen de confianza se calculó a partir de entradas obsoletas y
  debe tratarse como si requiriera actualización antes de una decisión de permitir de alta confianza.
...
- Este endpoint es intencionalmente más estrecho que los endpoints de moderación de propietario/moderador.
  ...
  identidades de reporteros, cuerpos de reporte, evidencia privada ni cronologías de revisión
  internas.

### `GET /api/v1/packages/{name}/versions/{version}/artifact`

Devuelve los metadatos explícitos del resolvedor de artefacto para una versión de paquete.
...
- Las versiones de ClawPack devuelven un artefacto `npm-pack`, campos de integridad npm, una
  `tarballUrl` y la URL de compatibilidad con ZIP heredado.
...
Endpoint de moderador para listar filas de migración de Plugin oficial de OpenClaw.
...
- `all`: cualquier versión con anulación manual, estado de análisis no limpio o reporte de paquete.
...
- Las respuestas incluyen los encabezados `ETag`, `Digest`, `X-ClawHub-Artifact-Type` y
  `X-ClawHub-Artifact-Sha256` para las comprobaciones de integridad del resolver.
...
Transmite los bytes exactos del tarball ClawPack subido para clientes de espejo npm.

Notas:

- Usa el bucket de tasa de descarga.
- Los encabezados de descarga incluyen el SHA-256 de ClawHub más metadatos integrity/shasum de npm.
...
Descarga un ZIP de versión de Skill alojado, o devuelve una entrega de origen de GitHub para una
Skill actual respaldada por GitHub con un escaneo `clean` o `suspicious` y sin versión
alojada.
...
- Las entregas de Skill respaldadas por GitHub no hacen proxy ni reflejan bytes. La respuesta JSON
  incluye `sourceRef: "public-github"`, `repo`, `commit`, `path`, `contentHash`
  y `archiveUrl`; el estado de escaneo/actual es una compuerta y no se incluye como metadatos de
  carga útil de éxito.
...
- Las publicaciones en nombre de otro siguen validando la elegibilidad del canal oficial contra la cuenta de propietario de destino.
...
versión. Los nombres de paquete se convierten en paquetes marcadores de posición privados sin filas de versión, para que el mismo
propietario pueda publicar más tarde la versión code-plugin o bundle-plugin real en ese nombre.
...
proveedor GitHub; los identificadores mutables solo se usan como una protección orientada al operador.
...
La recuperación también migra los campos heredados `ownerUserId` para las Skills,
los alias de slug de Skills, los paquetes, las advertencias del inspector de paquetes y las filas derivadas del resumen de búsqueda del publicador recuperado, de modo que
...
Quita el baneo a un usuario y restaura las Skills elegibles (solo administrador).
