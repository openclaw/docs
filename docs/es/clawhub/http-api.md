...
summary: Referencia de la API HTTP (puntos de conexión públicos + de CLI + autenticación).
x-i18n:
...
- Los resultados se devuelven por orden de relevancia (similitud de embeddings + boosts por coincidencia exacta de token de slug/nombre + una pequeña prioridad de popularidad).
...
- La popularidad se escala logarítmicamente y se limita. Los Skills con mucha interacción pueden clasificarse más bajo cuando el texto de la consulta tiene una coincidencia más débil.
...
- No cambies el nombre de un slug solo para perseguir una consulta, salvo que el nuevo slug sea un mejor nombre canónico a largo plazo. Los slugs antiguos se convierten en alias de redirección, pero la URL canónica, el slug mostrado y los resúmenes de búsqueda futuros usan el slug nuevo.
- Los alias de cambio de nombre preservan la resolución de URL antiguas e instalaciones que se resuelven a través del registro, pero la clasificación de búsqueda se basa en los metadatos canónicos del Skill después de que el cambio de nombre se haya indexado. Las estadísticas existentes permanecen con el Skill.
- Si un Skill está inesperadamente invisible, primero revisa el estado de moderación con `clawhub inspect @owner/slug` mientras tienes la sesión iniciada antes de cambiar metadatos relacionados con la clasificación.
...
- `metadata.systems`: objetivos de sistema de Nix (por ejemplo, `["aarch64-darwin", "x86_64-linux"]`). `null` si no se declaró.
...
- Los propietarios y moderadores pueden acceder a los detalles de moderación para Skills ocultos.
...
- Los trabajos de escaneo son asincrónicos. Las solicitudes de escaneo manual tienen prioridad sobre el trabajo normal de publicación/rellenado, pero la finalización sigue dependiendo de la disponibilidad de los workers.
...
- `isOfficial` (opcional): `true` o `false`
- `category` (opcional): filtro de categoría de plugin. Compatible solo cuando la
  solicitud está acotada a paquetes de plugin. Las categorías controladas y los alias de filtro heredados de v1
...
- `mcp-tooling`, `data` y `automation` se resuelven a `tools`.
- `observability` y `deployment` se resuelven a `gateway`.
- `dev-tools` se resuelve a `runtime`.
...
- El filtrado por categoría es un filtro de API real respaldado por filas de resumen
  de categorías de Plugin, no una reescritura de consulta de búsqueda.
...
- Skills también pueden resolverse a través de esta ruta en el catálogo unificado.
...
- Las versiones de ClawPack incluyen campos compatibles con npm: `npmIntegrity`, `npmShasum` y
  `npmTarballName`.
- `version.sha256hash` es metadato de compatibilidad obsoleto para clientes antiguos. Aplica hash
  a los bytes ZIP exactos devueltos por `/api/v1/packages/{name}/download`.
...
- Endpoint de lectura pública. No se requiere token de propietario, publicador,
...
- `trust.reasons` es la lista de explicaciones orientada al usuario y de auditoría. Los códigos de razón
...
- Este endpoint es intencionalmente más estrecho que los endpoints de moderación de propietario/moderador.
  ...
  informantes, cuerpos de informes, evidencia privada ni cronologías internas de revisión.
...
Descarga el artefacto de versión a través de la ruta explícita del resolvedor.
...
- Usa el contenedor de límite de tasa de descarga.
...
- `packageName` se normaliza como nombre npm; el paquete puede faltar para migraciones
  planificadas.
- Esto solo registra la preparación de la migración. No muta OpenClaw ni genera
  ClawPacks.
...
`finalAction: "revoke"` con un reporte confirmado para aplicar la moderación del lanzamiento en el
...
- `revoked`: bloqueado después de que un lanzamiento se consideró confiable anteriormente.
...
- Usa de forma predeterminada el lanzamiento más reciente.
...
- Los análisis pendientes de VirusTotal no bloquean las lecturas; los lanzamientos maliciosos aún pueden retenerse en otros lugares.
- Los paquetes privados devuelven `404` salvo que el llamador pueda leer el publicador propietario.

### `GET /api/v1/packages/{name}/download`

Descarga el archivo ZIP determinista heredado para un lanzamiento de paquete.
...
- Siguen aplicándose las comprobaciones de moderación y acceso a paquetes privados.
...
Descarga un ZIP de versión de skill alojada, o devuelve una entrega de origen de GitHub para una
...
- Las entregas de skills respaldadas por GitHub no hacen proxy ni reflejan bytes. La respuesta JSON
  incluye `sourceRef: "public-github"`, `repo`, `commit`, `path`, `contentHash`
  y `archiveUrl`; el estado de análisis/actual es una barrera y no se incluye como metadatos
...
Publica un lanzamiento de code-plugin o bundle-plugin.

- Requiere autenticación con token Bearer.
- Requiere `multipart/form-data`.
- Los campos de formulario permitidos son `payload`, blobs repetidos `files` o una referencia de tarball
  `clawpack`. `clawpack` puede ser un blob `.tgz` o un id de almacenamiento devuelto por
  el flujo upload-url. Las publicaciones con storage-id preparados también deben incluir el
...
Solo administradores. Reserva slugs raíz y nombres de paquete para un propietario legítimo sin publicar un
lanzamiento. Los nombres de paquete se convierten en paquetes de marcador privados sin filas de lanzamiento, para que el mismo
propietario pueda publicar más tarde el lanzamiento real code-plugin o bundle-plugin en ese nombre.
...
de proveedor GitHub; los handles mutables solo se usan como una guarda orientada al operador.
...
Prohibir a un usuario y eliminar de forma permanente las skills de su propiedad (solo moderador/admin).
...
Quitar la prohibición a un usuario y restaurar las skills elegibles (solo admin).
...
Cambiar el motivo almacenado para una prohibición existente sin quitar la prohibición ni restaurar
...
Añadir/eliminar una estrella (destacados). Ambos endpoints son idempotentes.
...
Siguen siendo compatibles con versiones antiguas de CLI:
...
Si lo autoalojas, sirve este archivo (o define `CLAWHUB_REGISTRY` explícitamente; `CLAWDHUB_REGISTRY` heredado).
