...
Los directorios de terceros pueden usar los endpoints públicos de lectura para listar o buscar Skills de ClawHub. Almacena los resultados en caché, respeta `429`/`Retry-After`, enlaza a los usuarios de vuelta al listado canónico de ClawHub (`https://clawhub.ai/<owner>/skills/<slug>`) y evita sugerir que ClawHub respalda el sitio de terceros. No intentes replicar contenido oculto, privado o bloqueado por moderación fuera de la superficie de la API pública.

Los atajos de slugs web se resuelven entre familias de registro, pero los clientes de API deben usar
...
- ClawHub usa encabezados de reenvío confiables para identificar IPs de clientes en el borde.
...
- La relevancia pesa más que la popularidad. Una coincidencia precisa de token de slug o nombre visible puede superar a una coincidencia menos estricta con mucha más interacción.
...
- La popularidad se escala logarítmicamente y se limita. Las Skills con alta interacción pueden posicionarse más abajo cuando el texto de la consulta coincide peor.
...
- Los alias de renombre preservan la resolución para URL antiguas e instalaciones que se resuelven a través del registro, pero la clasificación de búsqueda se basa en los metadatos canónicos de la Skill después de que el renombre se haya indexado. Las estadísticas existentes permanecen con la Skill.
- Si una Skill es inesperadamente invisible, revisa primero el estado de moderación con `clawhub inspect @owner/slug` con sesión iniciada antes de cambiar metadatos relacionados con la clasificación.
...
- Los slugs antiguos creados por flujos de renombre/fusión de propietario se resuelven a la Skill canónica.
...
- Los propietarios y moderadores pueden acceder a detalles de moderación de Skills ocultas.
...
Informa una Skill para revisión de moderadores. Los informes son a nivel de Skill, opcionalmente vinculados
...
- `version.security` incluye estado normalizado de verificación de escaneo y detalles del escáner
...
- Incluye el estado de verificación normalizado más detalles específicos del analizador.
...
- Los escaneos publicados requieren acceso de administración del propietario/editor, o autoridad de moderador/administrador de la plataforma.
...
- Los trabajos de escaneo son asíncronos. Las solicitudes de escaneo manual se priorizan por delante del trabajo normal de publicación/rellenado, pero la finalización sigue dependiendo de la disponibilidad de trabajadores.
...
- Devuelve `queue.queuedAhead` y `queue.position` mientras está en cola para que los clientes puedan mostrar cuántos escaneos manuales priorizados están por delante de la solicitud. Las colas muy grandes se acotan y se notifican con `queuedAheadIsEstimate: true`.
...
- Requiere acceso de administración del propietario/editor al Skill o Plugin, o autoridad de moderador/administrador de la plataforma.
...
Ruta canónica solo para administradores de reescaneo por lotes. Acepta la misma forma de carga útil que la ruta heredada `POST /api/v1/skills/-/rescan-batch`.
...
Devuelve el sobre de verificación de la tarjeta de Skill usado por `clawhub skill verify`.
...
- `ok` es `true` solo cuando la versión seleccionada tiene una tarjeta de Skill generada, no está bloqueada como malware por moderación y la verificación de ClawScan está limpia.
...
- `security.signals` contiene evidencia de analizadores de apoyo, como `staticScan`, `virusTotal` y `skillSpector`.
- `security.signals.dependencyRegistry` se conserva por compatibilidad con respuestas v1, pero el analizador de existencia del registro de dependencias está retirado y esta clave siempre es `null`.
...
endpoint de colección está pensado para clientes que ya saben qué versiones de Skills
...
- La respuesta es solo de seguridad. No incluye datos de la tarjeta de Skill, estado de tarjeta generada, listas de archivos de artefactos ni cargas útiles detalladas de analizadores.
- `security.signals` contiene solo evidencia de apoyo a nivel de estado; usa `/scan` o la página de auditoría de seguridad de ClawHub para obtener detalles completos de los analizadores.
- `security.signals.dependencyRegistry` se conserva por compatibilidad con respuestas v1, pero el analizador de existencia del registro de dependencias está retirado y esta clave siempre es `null`.
- La ausencia de la tarjeta de Skill no afecta a `ok`, `decision` ni `reasons` de este endpoint; los clientes deben leer el `skill-card.md` instalado localmente cuando necesiten contenido de la tarjeta.
- Usa `/verify` cuando necesites el sobre de verificación de tarjeta de Skill para un único Skill, `/card` cuando necesites markdown de tarjeta generada y `/scan` cuando necesites datos detallados de analizadores.
...
Búsqueda de catálogo unificada entre Skills y paquetes de plugins.
...
- `isOfficial` (opcional): `true` o `false`
- `category` (opcional): filtro de categoría de plugin. Compatible solo cuando la
  solicitud está acotada a paquetes de plugins. Las categorías controladas y los alias
...
- Los controles de ordenación de la interfaz del navegador para la búsqueda de Plugin reordenan los resultados de relevancia cargados,
  de forma coincidente con el comportamiento actual de exploración de `/skills`.
...
Elimina de forma reversible un paquete y todos sus lanzamientos.
...
Devuelve el resumen exacto de seguridad y confianza del lanzamiento del paquete para clientes de instalación. Esta es la superficie pública de consumo de OpenClaw para decidir si un
...
- `trust.scanStatus` es el estado de confianza efectivo derivado de entradas de escáner
  y de la moderación manual del lanzamiento.
- `trust.moderationState` puede ser nulo. Es `null` cuando no existe moderación manual
  del lanzamiento.
...
- `trust.stale` significa que el resumen de confianza se calculó a partir de entradas desactualizadas y
  debe tratarse como que requiere actualización antes de una decisión de permitir con alta confianza.
...
- Este endpoint es exacto por versión. Los clientes deben llamarlo después de resolver la
  ...
  del paquete.
- Los paquetes privados devuelven `404` salvo que el llamador pueda leer el publicador propietario.
- Este endpoint es intencionalmente más estrecho que los endpoints de moderación de propietarios/moderadores.
  Expone la decisión de instalación y la explicación pública, no
  identidades de reportantes, cuerpos de reportes, evidencia privada ni cronologías internas
  de revisión.
...
- Las versiones de paquete heredadas devuelven un artefacto `legacy-zip` y una URL ZIP heredada
  `downloadUrl`.
...
- `packageName` se normaliza como nombre npm; el paquete puede faltar en migraciones
  planificadas.
- Esto solo rastrea la preparación de migración. No modifica OpenClaw ni genera
  ClawPacks.
...
- `quarantined`: bloqueada en espera de seguimiento.
...
- Usa de forma predeterminada la versión más reciente.
- Usa el bucket de límite de tasa de lectura, no el bucket de descarga.
...
Transmite los bytes exactos del tarball de ClawPack subido para clientes de mirror npm.
...
- Las transferencias de skills respaldadas por GitHub no hacen proxy ni mirror de bytes. La respuesta JSON
...
- Preferido: `multipart/form-data` con JSON de `payload` + blobs `files[]`.
...
- Campo opcional de carga útil: `migrateOwner`. Cuando es `true` con `ownerHandle`, una
  ...
  los publicadores actual como de destino. Sin esta aceptación explícita, se
  rechazan los cambios de propietario.
...
- Se rechazan cuerpos JSON y metadatos `payload.files` / `payload.artifact`
...
- Las publicaciones en nombre de otro propietario aún validan la elegibilidad del canal oficial contra la cuenta del propietario de destino.
...
Las eliminaciones reversibles iniciadas por el propietario reservan el slug durante 30 días; luego el slug puede ser reclamado por
otro publicador. La respuesta de eliminación incluye `slugReservedUntil` cuando aplica este vencimiento.
Las ocultaciones de moderador/administrador y las eliminaciones por seguridad no vencen de esta manera.
...
usuario heredado compartido/publicador personal, el endpoint lo migra primero a un publicador de organización.
Para una organización recién creada, proporciona `memberHandle`; el administrador actuante no se agrega como miembro.
`memberRole` usa de forma predeterminada `owner`.
...
Creación autoservicio autenticada de publicador de organización. Crea un nuevo publicador de organización y agrega al
llamador como propietario. Este endpoint no migra handles existentes de usuario/personales y no
...
versión. Los nombres de paquete se convierten en paquetes placeholder privados sin filas de versión, para que el mismo
...
inmutables; los handles mutables solo se usan como una salvaguarda orientada al operador.

El endpoint usa ejecución de prueba de forma predeterminada. Para aplicar la recuperación se requiere `dryRun: false` y
...
los alias de slug de Skill, paquetes, advertencias del inspector de paquetes y filas de resumen de búsqueda derivadas para que
...
Las fuentes de Skills de GitHub tienen alcance de publicador y se informan como verificadas en lugar de reescribirse.
...
Aún compatibles con versiones anteriores de la CLI:
...
que preparan un archivo tar de ClawPack deben enviar el id de almacenamiento resultante como
