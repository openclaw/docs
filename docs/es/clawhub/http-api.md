...
x-i18n:
    generated_at: "2026-07-01T02:56:57Z"
    model: gpt-5.5
...
- Usa cabeceras de IP de cliente de confianza, incluida `cf-connecting-ip`, solo cuando el
  despliegue habilita explícitamente cabeceras reenviadas de confianza.
- ClawHub usa cabeceras de reenvío de confianza para identificar las IP de cliente en el borde.
- Si no hay una IP de cliente de confianza disponible, las solicitudes anónimas usan buckets alternativos
  limitados solo por tipo de límite de frecuencia. Estos buckets alternativos no incluyen
...
- Los resultados se devuelven en orden de relevancia (similitud de embeddings + mejoras por coincidencia exacta de tokens de slug/nombre + un pequeño prior de popularidad).
...
- La popularidad se escala logarítmicamente y se limita. Los Skills con alta interacción pueden clasificarse más bajo cuando el texto de la consulta es una coincidencia más débil.
...
- Si un Skill está inesperadamente invisible, comprueba primero el estado de moderación con `clawhub inspect @owner/slug` con sesión iniciada antes de cambiar metadatos relacionados con la clasificación.
...
Informa sobre un Skill para revisión de moderadores. Los informes son de nivel Skill, opcionalmente vinculados
...
evaluado para ocultar el Skill en el mismo flujo de trabajo auditable.
...
- Los trabajos de escaneo son asíncronos. Las solicitudes de escaneo manual tienen prioridad sobre el trabajo normal de publicación/relleno, pero la finalización sigue dependiendo de la disponibilidad de los workers.
...
- Devuelve `queue.queuedAhead` y `queue.position` mientras está en cola para que los clientes puedan mostrar cuántos escaneos manuales priorizados están delante de la solicitud. Las colas muy grandes se acotan y se informan con `queuedAheadIsEstimate: true`.
...
- Requiere acceso de administración del propietario/publicador a la habilidad o al plugin, o autoridad de moderador/administrador de la plataforma.
- Devuelve resultados de escaneo almacenados para la versión enviada exacta, incluidas versiones bloqueadas u ocultas.
- `kind` toma `skill` como valor predeterminado; usa `kind=plugin` para escaneos de plugins/paquetes.
...
- La identidad de la habilidad, la identidad del publicador y los metadatos de la versión seleccionada son campos de nivel superior del sobre (`slug`, `displayName`, `publisherHandle`, `version`, `resolvedFrom`, `tag`, `createdAt`) para que la automatización de shell pueda leerlos sin desempaquetar envoltorios anidados.
...
- `security.signals.dependencyRegistry` se conserva por compatibilidad de respuesta v1, pero el escáner de existencia del registro de dependencias está retirado y esta clave siempre es `null`.
- La ausencia de Skill Card no afecta a `ok`, `decision` ni `reasons` de este endpoint; los clientes deberían leer el `skill-card.md` instalado localmente cuando necesiten contenido de la tarjeta.
- Usa `/verify` cuando necesites el sobre de verificación de Skill Card de una sola habilidad, `/card` cuando necesites Markdown de tarjeta generado y `/scan` cuando necesites datos detallados del escáner.
...
- `trust.moderationState` admite nulos. Es `null` cuando no existe moderación manual de
  la versión.
...
- `trust.stale` significa que el resumen de confianza se calculó a partir de entradas desactualizadas y
  debe tratarse como si requiriera actualización antes de una decisión de permiso de alta confianza.
...
- Este endpoint es intencionalmente más estrecho que los endpoints de moderación de propietario/moderador.
  ...
  identidades de denunciantes, cuerpos de informes, evidencia privada ni cronogramas internos de revisión.
...
- Usa el contenedor de límite de tasa de descargas.
...
- `packageName` se normaliza como nombre npm; el paquete puede faltar para migraciones
  planificadas.
- Esto solo realiza seguimiento de la preparación de la migración. No muta OpenClaw ni genera
  ClawPacks.
...
- `manual`: cualquier versión con anulación manual de moderación.
- `all`: cualquier versión con anulación manual, estado de análisis no limpio o informe de paquete.
...
Denuncia un paquete para revisión de moderador. Los informes son de nivel de paquete y, opcionalmente,
...
- `revoked`: bloqueado después de que una versión se hubiera considerado confiable anteriormente.
...
- Límite de tamaño de archivo: 200 KB.
- Los escaneos de VirusTotal pendientes no bloquean las lecturas; las versiones maliciosas aún pueden retenerse en otro lugar.
- Los paquetes privados devuelven `404` a menos que el llamador pueda leer el publicador propietario.
...
- Los archivos Plugin/paquete son archivos zip con una raíz `package/` para que los clientes antiguos de OpenClaw
  sigan funcionando.
...
- Los escaneos de VirusTotal pendientes no bloquean las descargas; las versiones maliciosas devuelven `403`.
...
Descarga un ZIP de versión de habilidad alojada, o devuelve una transferencia a fuente de GitHub para una
...
- Las transferencias de habilidades respaldadas por GitHub no hacen proxy ni espejo de bytes. La respuesta JSON
  incluye `sourceRef: "public-github"`, `repo`, `commit`, `path`, `contentHash`
  y `archiveUrl`; el estado de escaneo/actual es una compuerta y no se incluye como metadatos de carga útil
  de éxito.
...
- Campo opcional de carga útil: `migrateOwner`. Cuando es `true` con `ownerHandle`, una
  habilidad existente puede moverse a ese propietario si el actor es administrador/propietario tanto en los publicadores
  actual como de destino. Sin esta adhesión explícita, los cambios de propietario se
  rechazan.
...
- Los paquetes Plugin requieren `openclaw.plugin.json`. Las subidas `.tgz` de ClawPack deben
  contenerlo en `package/openclaw.plugin.json`.
- Los Plugin de código requieren `package.json`, metadatos de repositorio fuente, metadatos de commit
...
- Las publicaciones en nombre de otro todavía validan la elegibilidad del canal oficial contra la cuenta del propietario de destino.
...
Las ocultaciones por moderador/administrador y las eliminaciones de seguridad no expiran de esta manera.
...
Creación autenticada autoservicio de publicador de organización. Crea un nuevo publicador de organización y agrega al
...
versión. Los nombres de paquetes se convierten en paquetes marcadores privados sin filas de versiones, para que el mismo
...
Solo administradores. Recupera un publicador personal para un principal sustituto verificado de GitHub OAuth
sin editar filas de cuentas de Convex Auth. La solicitud debe nombrar ambos ids inmutables de cuenta de
proveedor GitHub; los identificadores mutables solo se usan como una protección visible para el operador.

El endpoint usa ejecución en seco de forma predeterminada. Aplicar la recuperación requiere `dryRun: false` y
`confirmIdentityVerified: true` después de que el personal verifique de forma independiente la continuidad entre ambas
identidades de GitHub. La recuperación se deniega de forma predeterminada cuando el publicador personal actual del usuario de destino
...
para el identificador recuperado también se reasigna al usuario de reemplazo para que la sincronización de perfiles posterior
...
Bloquea a un usuario y elimina de forma permanente las Skills que posee (solo moderador/admin).
...
Cambia el motivo almacenado para un bloqueo existente sin desbloquear ni restaurar
contenido (solo admin). Usa ejecución en seco de forma predeterminada salvo que `dryRun` sea `false`.
...
Si autohospedas, sirve este archivo (o define `CLAWHUB_REGISTRY` explícitamente; heredado `CLAWDHUB_REGISTRY`).
