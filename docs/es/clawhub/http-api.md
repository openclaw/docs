...
x-i18n:
    generated_at: "2026-07-05T05:07:40Z"
    model: gpt-5.5
...
- Los endpoints autenticados de escritura no deberían devolver un simple `Unauthorized` cuando
  el servidor conoce el motivo. Los tokens ausentes, inválidos/revocados y las
...
- Si no hay una IP de cliente de confianza disponible, las solicitudes anónimas usan contenedores de reserva
...
- Los resultados se devuelven en orden de relevancia (similitud de incrustaciones + mejoras por coincidencia exacta de tokens de slug/nombre + un pequeño prior de popularidad).
- La relevancia pesa más que la popularidad. Una coincidencia precisa de slug o de token de nombre mostrado puede superar a una coincidencia más laxa con mucha más interacción.
...
- La popularidad se escala logarítmicamente y se limita. Las Skills con alta interacción pueden clasificarse más abajo cuando el texto de consulta es una coincidencia más débil.
...
- Si una Skill está inesperadamente invisible, revisa primero el estado de moderación con `clawhub inspect @owner/slug` mientras tienes sesión iniciada antes de cambiar metadatos relacionados con ranking.
...
- Las cargas útiles de solicitudes de escaneo y los informes descargables expiran del almacén de solicitudes de escaneo tras el periodo de retención.
...
- Los trabajos de escaneo son asíncronos. Las solicitudes manuales de escaneo tienen prioridad sobre el trabajo normal de publicación/rellenado, pero la finalización sigue dependiendo de la disponibilidad de workers.
...
- `security.signals` contiene pruebas de apoyo del escáner, como `staticScan`, `virusTotal` y `skillSpector`.
- `security.signals.dependencyRegistry` se conserva por compatibilidad de la respuesta v1, pero el escáner de existencia del registro de dependencias está retirado y esta clave siempre es `null`.
...
- `security.signals` contiene solo pruebas de apoyo a nivel de estado; usa `/scan` o la página de auditoría de seguridad de ClawHub para ver todos los detalles del escáner.
- `security.signals.dependencyRegistry` se conserva por compatibilidad de la respuesta v1, pero el escáner de existencia del registro de dependencias está retirado y esta clave siempre es `null`.
- La ausencia de Skill Card no afecta a `ok`, `decision` ni `reasons` de este endpoint; los clientes deben leer localmente el `skill-card.md` instalado cuando necesiten el contenido de la tarjeta.
...
- `category` (opcional): filtro de categoría de plugin. Solo se admite cuando la
  ...
  alias heredados del filtro v1 se documentan en `GET /api/v1/plugins`.
...
Búsqueda de catálogo unificada entre Skills + paquetes de plugin.
...
- `category` (opcional): filtro de categoría de plugin. Solo se admite cuando la
  ...
  heredados del filtro v1 se documentan en `GET /api/v1/plugins`.
...
- Requiere un token de API del propietario del paquete, un miembro del publicador,
...
- `revoked`: bloqueada después de que una versión se hubiera considerado de confianza anteriormente.
...
Transmite los bytes exactos del tarball de ClawPack subido para clientes del espejo npm.
...
- Las transferencias de skills respaldadas por GitHub no actúan como proxy ni reflejan bytes. La respuesta JSON
  ...
  y `archiveUrl`; el estado de análisis/actual es una compuerta y no se incluye como metadatos de
  carga útil de éxito.
...
- Los campos de formulario permitidos son `payload`, blobs `files` repetidos o una referencia de tarball
  ...
  el flujo de URL de subida. Las publicaciones preparadas con id de almacenamiento también deben incluir el
...
- Las solicitudes de publicación multipart directas tienen un límite de 18 MB. Los tarballs de ClawPack pueden
  usar el flujo de URL de subida hasta el límite de tarball de 120 MB.
...
publicador puede reclamarlo. La respuesta de eliminación incluye `slugReservedUntil` cuando se aplica esta caducidad.
Los ocultamientos por moderador/administrador y las eliminaciones de seguridad no caducan de esta forma.
...
no marca al publicador como de confianza/oficial.
...
proveedor de GitHub; los identificadores mutables solo se usan como protección visible para el operador.

El extremo usa dry-run de forma predeterminada. Para aplicar la recuperación se requiere `dryRun: false` y
...
Desbloquea a un usuario y restaura las Skills aptas (solo administrador).
...
## Extremos heredados de la CLI (obsoletos)
...
que preparen un tarball de ClawPack deben enviar el id de almacenamiento resultante como
