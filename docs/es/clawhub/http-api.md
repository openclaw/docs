...
read_when:
    ...
    - Depurar solicitudes CLI ↔ registry
...
- Solicitudes autenticadas (token Bearer válido): se aplica por contenedor de usuario.
...
- Los endpoints de escritura autenticados no deberían devolver un `Unauthorized` sin más cuando
  ...
  las cuentas eliminadas/baneadas/deshabilitadas deberían recibir texto accionable para que los
...
- Si no hay una IP de cliente confiable disponible, las solicitudes anónimas usan contenedores de reserva
  delimitados solo por el tipo de límite de tasa. Estos contenedores de reserva no incluyen
...
- La relevancia es más fuerte que la popularidad. Una coincidencia precisa de slug o token de nombre mostrado puede superar a una coincidencia más amplia con mucha más interacción.
...
- La popularidad se escala logarítmicamente y se limita. Las Skills con alta interacción pueden posicionarse más abajo cuando el texto de consulta es una coincidencia más débil.
...
- Los alias de renombrado preservan la resolución para URL antiguas e instalaciones que se resuelven mediante el registro, pero el posicionamiento de búsqueda se basa en los metadatos canónicos de la Skill después de que el renombrado se haya indexado. Las estadísticas existentes permanecen con la Skill.
- Si una Skill es inesperadamente invisible, revisa primero el estado de moderación con `clawhub inspect @owner/slug` con la sesión iniciada antes de cambiar metadatos relacionados con el posicionamiento.
...
triado para ocultar la Skill en el mismo flujo auditable.
...
- `ok` es `true` solo cuando la versión seleccionada tiene una Skill Card generada, la moderación no la bloquea como malware y la verificación de ClawScan está limpia.
...
endpoint de colección está destinado a clientes que ya saben qué versiones instaladas de Skills de
ClawHub necesitan mostrar, como OpenClaw Control UI.
...
- Las entradas de habilidades siguen respaldadas por el registro de habilidades y todavía solo pueden publicarse mediante `POST /api/v1/skills`.
...
Búsqueda de catálogo unificada en habilidades y paquetes de plugins.
...
- Las habilidades alojadas incluyen los archivos de la versión almacenada más reciente y aparecen en
  `_manifest.json` con `sourceRef: "public-clawhub"`.
- Las habilidades actuales respaldadas por GitHub con un análisis `clean` o `suspicious` incluyen
...
Devuelve el resumen exacto de seguridad y confianza de la versión del paquete para clientes de instalación. Esta es la superficie pública de consumo de OpenClaw para decidir si se puede instalar una
versión resuelta.
...
Devuelve los metadatos explícitos del solucionador de artefactos para una versión de paquete.
...
- Usa el bloque de tasa de descarga.
...
Reporta un paquete para revisión de moderadores. Los informes son a nivel de paquete, opcionalmente
...
- Requiere un token de API para el propietario del paquete, un miembro del publicador,
...
volver a establecer `status` como `open`. Pasa `finalAction: "quarantine"` o
...
Descarga un ZIP de versión de skill alojada, o devuelve una transferencia a la fuente de GitHub para una
...
- Las transferencias de skills respaldadas por GitHub no hacen proxy ni espejo de bytes. La respuesta JSON
  ...
  y `archiveUrl`; el estado de análisis/actual es una puerta de acceso y no se incluye como metadatos
...
- Las solicitudes de publicación multipart directa tienen un límite de 18 MB. Los tarballs de ClawPack pueden
...
- Las publicaciones en nombre de otro usuario siguen validando la elegibilidad para el canal oficial contra la cuenta propietaria de destino.
...
Para una organización recién creada, proporciona `memberHandle`; el administrador actor no se agrega como miembro.
`memberRole` toma de forma predeterminada `owner`.
...
Creación autenticada de publicadores de organización de autoservicio. Crea un publicador de organización nuevo y agrega al
...
versión. Los nombres de paquete se convierten en paquetes de marcador de posición privados sin filas de versión, para que el mismo
propietario pueda publicar más adelante la versión real de code-plugin o bundle-plugin en ese nombre.
...
sin editar filas de cuentas de Convex Auth. La solicitud debe nombrar ambos ids inmutables de cuenta
...
Bloquea a un usuario y elimina permanentemente las skills que posee (solo moderador/administrador).
...
Si haces self-hosting, sirve este archivo (o establece `CLAWHUB_REGISTRY` explícitamente; `CLAWDHUB_REGISTRY` heredado).
