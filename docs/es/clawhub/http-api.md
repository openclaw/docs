...
- Los endpoints de escritura autenticados no deben devolver un simple `Unauthorized` cuando
  ...
  las cuentas eliminadas/baneadas/deshabilitadas deben recibir texto procesable para que los clientes
...
- `highlightedOnly` (opcional): `true` para filtrar Skills destacadas
...
- Los resultados se devuelven por orden de relevancia (similitud de embeddings + refuerzos por token exacto de slug/nombre + una pequeña prioridad de popularidad).
...
- La popularidad se escala logarítmicamente y se limita. Las Skills con alta interacción pueden clasificarse más bajo cuando el texto de la consulta coincide peor.
...
Endpoint de moderador/administrador para la ingesta de reportes de Skills.
...
- Los trabajos de análisis son asíncronos. Las solicitudes de análisis manual se priorizan por delante del trabajo normal de publicación/relleno, pero la finalización sigue dependiendo de la disponibilidad de los workers.
...
- Requiere acceso de administración de propietario/publicador a la skill o el plugin, o autoridad de moderador/administrador de la plataforma.
...
- Los clientes autenticados pueden ver paquetes privados de los publicadores a los que pertenecen en los resultados de lista/búsqueda.
...
- Los clientes anónimos solo ven canales de paquetes públicos.
- Los clientes autenticados pueden buscar paquetes privados de los publicadores a los que pertenecen.
- `channel=private` solo devuelve paquetes que el cliente autenticado puede leer.
...
- Este endpoint es exacto por versión. Los clientes deben llamarlo después de resolver la
  versión de paquete que pretenden instalar, no solo después de leer los metadatos más recientes
  del paquete.
- Los paquetes privados devuelven `404` salvo que el llamador pueda leer el publicador propietario.
...
- Usa el depósito de tasa de lectura, no el depósito de descargas.
...
- Los archivos de Plugin/paquete son archivos zip con una raíz `package/` para que los clientes antiguos de OpenClaw
  sigan funcionando.
- Esta ruta permanece solo ZIP. No transmite archivos `.tgz` de ClawPack.
...
Descarga un ZIP de versión de Skill alojado, o devuelve una entrega de código fuente de GitHub para una
...
- Campo de carga útil opcional: `migrateOwner`. Cuando es `true` con `ownerHandle`, una
  ...
  el publicador actual como en el de destino. Sin esta aceptación explícita, se rechazan los
...
Las eliminaciones reversibles iniciadas por el propietario reservan el slug durante 30 días; después, otro publicador puede reclamar
el slug. La respuesta de eliminación incluye `slugReservedUntil` cuando esta expiración aplica.
...
versión. Los nombres de paquetes se convierten en paquetes de marcador de posición privados sin filas de versión, para que el mismo
...
Las fuentes de Skills de GitHub tienen alcance de publicador y se informan como comprobadas, no se reescriben.
...
Bloquea a un usuario y elimina de forma irreversible los Skills que posee (solo moderador/administrador).
...
Agrega o elimina una estrella (destacados). Ambos endpoints son idempotentes.
