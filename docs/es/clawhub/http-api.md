...
- Si no hay una IP de cliente confiable disponible, las solicitudes de descarga anónimas usan un bucket de reserva con ámbito de endpoint en lugar de un único bucket global `ip:unknown`. Las solicitudes anónimas de lectura/escritura siguen usando el bucket desconocido compartido para que el enrutamiento sin IP siga siendo visible y conservador.
...
- Los resultados se devuelven en orden de relevancia (similitud de embeddings + refuerzos de token exacto de slug/nombre + prioridad de popularidad de descargas).
- La relevancia pesa más que la popularidad. Una coincidencia precisa de slug o token de nombre visible puede superar a una coincidencia menos exacta con muchas más descargas.
...
- Las descargas se usan como una pequeña prioridad con escala logarítmica y como desempate, no como la señal principal de clasificación. Los Skills con muchas descargas pueden clasificarse más abajo cuando el texto de la consulta coincide peor.
...
- Si un Skill está inesperadamente invisible, primero comprueba el estado de moderación con `clawhub inspect <slug>` mientras has iniciado sesión antes de cambiar metadatos relacionados con la clasificación.
...
- `metadata.systems`: destinos de sistema Nix (por ejemplo, `["aarch64-darwin", "x86_64-linux"]`). `null` si no se declaró.
...
Informa de un Skill para revisión de moderadores. Los informes son a nivel de Skill, se pueden vincular
...
- `os`, `arch`, `libc` (opcional): forma abreviada para filtros de capacidades del host
...
Búsqueda unificada en el catálogo entre Skills + paquetes de plugin.
...
- `channel=private` solo devuelve paquetes que el llamador autenticado puede leer.
- Los filtros de artefactos están respaldados por etiquetas de capacidades indexadas:
...
- Esta es la superficie del solucionador de OpenClaw; evita adivinar el formato del archivo a partir
...
- repositorio de código fuente y procedencia del commit
...
Reporta un paquete para revisión del moderador. Los reportes son a nivel de paquete y, opcionalmente,
...
Endpoint de mantenimiento solo para administradores para etiquetar versiones de paquetes anteriores con
...
- Usa modo de simulación de forma predeterminada.
...
- Usa la última versión de forma predeterminada.
...
- Campo opcional de carga útil: `migrateOwner`. Cuando es `true` con `ownerHandle`, un
...
Elimina lógicamente / restaura un skill (propietario, moderador o admin).
...
publicador puede reclamar el slug. La respuesta de eliminación incluye `slugReservedUntil` cuando aplica esta caducidad.
Las ocultaciones de moderador/admin y las eliminaciones de seguridad no caducan de esta manera.
...
publicador compartido heredado de usuario/personal, el endpoint lo migra primero a un publicador de organización.
...
propietario puede publicar posteriormente la versión real de code-plugin o bundle-plugin en ese nombre.
...
Desbloquea a un usuario y restaura skills elegibles (solo admin).
