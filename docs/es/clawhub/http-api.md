...
read_when:
    ...
    - Depuración de solicitudes del CLI ↔ registro
...
- Estandarizadas: `RateLimit-Limit`, `RateLimit-Remaining`, `RateLimit-Reset`
- En `429`: `Retry-After`
...
- ClawHub usa cabeceras de reenvío confiables para identificar las IP de clientes en el edge.
- Si no hay una IP de cliente confiable disponible, las solicitudes de descarga anónimas usan un bucket de reserva con ámbito de endpoint en lugar de un bucket global `ip:unknown`. Las solicitudes anónimas de lectura/escritura siguen usando el bucket desconocido compartido para que el enrutamiento sin IP siga siendo visible y conservador.
...
- `highlightedOnly` (opcional): `true` para filtrar solo Skills destacados
...
- Los resultados se devuelven en orden de relevancia (similitud de embeddings + aumentos por coincidencias exactas de token de slug/nombre + prioridad de popularidad por descargas).
- La relevancia pesa más que la popularidad. Una coincidencia precisa de token de slug o nombre visible puede superar a una coincidencia más laxa con muchas más descargas.
...
- Las descargas se usan como una pequeña prioridad con escala logarítmica y como criterio de desempate, no como señal principal de ranking. Los Skills con muchas descargas pueden posicionarse más abajo cuando el texto de la consulta es una coincidencia más débil.
...
- Si un Skill es inesperadamente invisible, revisa primero el estado de moderación con `clawhub inspect <slug>` mientras tienes sesión iniciada antes de cambiar metadatos relacionados con el ranking.
...
- `metadata.systems`: destinos de sistema Nix (por ejemplo, `["aarch64-darwin", "x86_64-linux"]`). `null` si no están declarados.
...
Informa de un Skill para revisión por moderadores. Los informes son a nivel de Skill, opcionalmente vinculados
...
Endpoint de moderador/administrador para recepción de informes de Skills.
...
Búsqueda de catálogo unificada entre Skills + paquetes de plugins.
...
- Las versiones de paquetes heredados devuelven un artefacto `legacy-zip` y una URL ZIP heredada
  `downloadUrl`.
...
- Esta es la superficie del resolvedor de OpenClaw; evita adivinar el formato de archivo desde
  una URL compartida.
...
`finalAction: "revoke"` con un reporte confirmado para aplicar la moderación de lanzamiento en el
...
Punto de conexión de moderador/administrador para la revisión de lanzamientos de paquetes.
...
- `quarantined`: bloqueado hasta seguimiento.
...
Punto de conexión de mantenimiento solo para administradores para etiquetar lanzamientos de paquetes antiguos con
...
- De forma predeterminada, ejecuta un simulacro.
...
- Usa de forma predeterminada el último lanzamiento.
...
- Los paquetes privados devuelven `404` a menos que quien llama pueda leer el editor propietario.
...
- Campo opcional de payload: `ownerHandle`. Cuando está presente, la API resuelve ese
  editor en el servidor y requiere que el actor tenga acceso al editor.
...
Publica un lanzamiento de Plugin de código o Plugin de paquete.
...
- Solo los editores confiables pueden publicar en el canal `official`.
...
editor puede reclamarlo. La respuesta de eliminación incluye `slugReservedUntil` cuando aplica esta expiración.
...
Solo administradores. Garantiza que exista un editor de organización para un identificador. Si el identificador aún apunta a un
editor heredado compartido de usuario/personal, el punto de conexión lo migra primero a un editor de organización.
...
lanzamiento. Los nombres de paquete se convierten en paquetes marcadores de posición privados sin filas de lanzamiento, para que el mismo
propietario pueda publicar más adelante el lanzamiento real de Plugin de código o Plugin de paquete en ese nombre.
...
### Puntos de conexión de administración de slugs de propietario
...
Revoca el baneo de un usuario y restaura las Skills elegibles (solo administrador).
...
## Puntos de conexión CLI heredados (obsoletos)
