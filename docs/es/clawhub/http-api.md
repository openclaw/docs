...
- Solicitudes anónimas: aplicadas por IP.
- Solicitudes autenticadas (token Bearer válido): aplicadas por bucket de usuario.
...
- Los endpoints de escritura autenticados no deben devolver un simple `Unauthorized` cuando
  el servidor conoce la razón. Los tokens ausentes, tokens no válidos/revocados y
  cuentas eliminadas/baneadas/deshabilitadas deben recibir cada uno texto accionable para que los clientes
  CLI puedan decir a los usuarios qué los bloqueó.
...
- Usa retroceso con fluctuación para evitar reintentos sincronizados.
...
- ClawHub usa encabezados de reenvío confiables para identificar IPs de cliente en el borde.
- Si no hay IP de cliente confiable disponible, las solicitudes anónimas usan buckets de respaldo
  limitados solo por tipo de límite de tasa. Estos buckets de respaldo no incluyen
...
- Los resultados se devuelven en orden de relevancia (similitud de embeddings + refuerzos por coincidencia exacta de token de slug/nombre + una pequeña prioridad de popularidad).
- La relevancia es más fuerte que la popularidad. Una coincidencia precisa de token de slug o nombre visible puede superar a una coincidencia más laxa con una interacción mucho mayor.
- El texto ASCII se tokeniza en límites de palabra y puntuación. Por ejemplo, `personal-map` contiene un token independiente `map`, mientras que `amap-jsapi-skill` contiene `amap`, `jsapi` y `skill`; por lo tanto, buscar `map` le da a `personal-map` una coincidencia léxica más fuerte que a `amap-jsapi-skill`.
- La popularidad se escala logarítmicamente y tiene límite máximo. Skills con alta interacción pueden clasificarse más abajo cuando el texto de la consulta coincide de forma más débil.
...
- Los alias de cambio de nombre conservan la resolución para URL antiguas e instalaciones que resuelven mediante el registro, pero la clasificación de búsqueda se basa en los metadatos canónicos de la skill después de que el cambio de nombre se haya indexado. Las estadísticas existentes permanecen con la skill.
- Si una skill está inesperadamente invisible, revisa primero el estado de moderación con `clawhub inspect @owner/slug` con la sesión iniciada antes de cambiar metadatos relacionados con la clasificación.
...
- `createdAt` es estable para rastreos de nuevas skills; `updated` cambia cuando se republican Skills existentes.
...
Endpoint de moderador/admin para entrada de reportes de skills.
...
establecer `status` de vuelta a `open`. Pasa `finalAction: "hide"` con un reporte triado
...
- Las cargas útiles de solicitudes de escaneo y los informes descargables caducan del almacén de solicitudes de escaneo después del periodo de retención.
...
- Los trabajos de escaneo son asíncronos. Las solicitudes de escaneo manual se priorizan por delante del trabajo normal de publicación/relleno, pero la finalización aún depende de la disponibilidad de los trabajadores.

### `GET /api/v1/skills/-/scan/{scanId}`

Endpoint autenticado de sondeo para un escaneo enviado.

- Devuelve el estado en cola/en ejecución/completado/con error.
- Devuelve `queue.queuedAhead` y `queue.position` mientras está en cola para que los clientes puedan mostrar cuántos escaneos manuales priorizados están por delante de la solicitud. Las colas muy grandes se limitan y se informan con `queuedAheadIsEstimate: true`.
- Cuando está disponible, `report` contiene secciones `clawscan`, `skillspector`, `staticAnalysis` y `virustotal`.
- Los trabajos de escaneo con error devuelven `status: "failed"` con `lastError`.
...
- Devuelve resultados de escaneo almacenados para la versión enviada exacta, incluidas versiones bloqueadas u ocultas.
...
- `items` debe contener de 1 a 100 pares únicos `{ slug, version }`.
...
- `security.signals.dependencyRegistry` se conserva por compatibilidad de respuesta v1, pero el escáner de existencia del registro de dependencias está retirado y esta clave siempre es `null`.
- La ausencia de Skill Card no afecta los valores `ok`, `decision` ni `reasons` de este endpoint; los clientes deben leer el `skill-card.md` instalado localmente cuando necesiten contenido de la tarjeta.
...
- `category` (opcional): filtro de categoría de Plugin. Solo se admite cuando la
  solicitud tiene como ámbito paquetes de Plugin (`/api/v1/plugins`,
...
- Las entradas de habilidades siguen respaldadas por el registro de habilidades y aún solo se pueden publicar mediante `POST /api/v1/skills`.
- `POST /api/v1/packages` sigue siendo solo para lanzamientos de code-plugin y bundle-plugin.
- Los autores de llamadas anónimos solo ven canales de paquetes públicos.
- Los autores de llamadas autenticados pueden ver paquetes privados de publicadores a los que pertenecen en los resultados de lista/búsqueda.
- `channel=private` solo devuelve paquetes que el autor de la llamada autenticado puede leer.

### `GET /api/v1/packages/search`

Búsqueda de catálogo unificada en habilidades + paquetes de Plugin.
...
- `isOfficial` (opcional): `true` o `false`
- `category` (opcional): filtro de categoría de Plugin. Solo se admite cuando la
  solicitud tiene como ámbito paquetes de Plugin. Las categorías controladas y
  los alias de filtro heredados de v1 están documentados en `GET /api/v1/plugins`.
...
- Los autores de llamadas anónimos solo ven canales de paquetes públicos.
- Los autores de llamadas autenticados pueden buscar paquetes privados de publicadores a los que pertenecen.
- `channel=private` solo devuelve paquetes que el autor de la llamada autenticado puede leer.
...
- Cuerpo: archivo ZIP.
- Cada habilidad exportada tiene como raíz `{publisher}/{slug}/`.
- Las habilidades alojadas incluyen los archivos de la última versión almacenada y se listan en
...
- `version.sha256hash` son metadatos de compatibilidad obsoletos para clientes antiguos. Genera
  un hash de los bytes ZIP exactos devueltos por `/api/v1/packages/{name}/download`.
...
- `trust.scanStatus` es el estado de confianza efectivo derivado de entradas del analizador
  y de la moderación manual de la versión.
- `trust.moderationState` admite nulos. Es `null` cuando no existe moderación manual
  de la versión.
- `trust.blockedFromDownload` es la señal de bloqueo de instalación. OpenClaw y otros
  ...
  volver a derivar reglas de bloqueo a partir de campos del analizador o de moderación.
...
- `trust.stale` significa que el resumen de confianza se calculó a partir de entradas obsoletas y
  debería tratarse como que requiere actualización antes de una decisión de autorización de alta confianza.
...
- Este endpoint es intencionalmente más limitado que los endpoints de moderación de propietario/moderador.
  ...
  identidades de reporteros, cuerpos de reportes, evidencia privada ni cronologías de revisión
  interna.
...
- Esta es la superficie de resolución de OpenClaw; evita adivinar el formato del archivo a partir
  de una URL compartida.

### `GET /api/v1/packages/{name}/versions/{version}/artifact/download`

Descarga el artefacto de versión mediante la ruta explícita del resolutor.

Notas:

- Las versiones ClawPack transmiten los bytes `.tgz` exactos del npm-pack subido.
...
- Usa el depósito de límite de lectura, no el depósito de descargas.
...
La CLI lo usa para asignar una huella local a una versión conocida.
...
- Las transferencias de skills respaldadas por GitHub no hacen proxy ni reflejan bytes. La respuesta JSON
...
Valida el token y devuelve el identificador de usuario.
...
- Campo opcional de carga útil: `migrateOwner`. Cuando es `true` con `ownerHandle`, una
  skill existente puede moverse a ese propietario si el actor es administrador/propietario tanto en los publicadores
  actual como destino. Sin esta opción explícita, se rechazan los cambios de propietario.
...
- Solo el publicador de la organización `openclaw` y los publicadores personales de miembros actuales
  de la organización `openclaw` pueden publicar en el canal `official`.
- Las publicaciones en nombre de terceros siguen validando la elegibilidad del canal oficial contra la cuenta propietaria destino.
...
puede reclamarlo. La respuesta de eliminación incluye `slugReservedUntil` cuando aplica esta expiración.
...
llamador como propietario. Este endpoint no migra identificadores de usuario/personales existentes y no
...
sin editar filas de cuentas de Convex Auth. La solicitud debe nombrar ambos ids de cuenta
inmutables del proveedor GitHub; los identificadores mutables se usan solo como una protección orientada al operador.

De forma predeterminada, el endpoint usa simulación. Aplicar la recuperación requiere `dryRun: false` y
...
los alias de slug de Skills, paquetes, advertencias del inspector de paquetes y filas de resumen de búsqueda derivadas para que
...
contenido (solo administrador). De forma predeterminada usa simulación, salvo que `dryRun` sea `false`.
...
Si lo autoalojas, sirve este archivo (o establece `CLAWHUB_REGISTRY` explícitamente; `CLAWDHUB_REGISTRY` heredado).
