...
summary: Referencia de la API HTTP (endpoints públicos y de CLI + autenticación).
x-i18n:
...
Los directorios de terceros pueden usar los endpoints públicos de lectura para listar o buscar Skills de ClawHub. Almacena los resultados en caché, respeta `429`/`Retry-After`, enlaza a los usuarios con la ficha canónica de ClawHub (`https://clawhub.ai/<owner>/skills/<slug>`) y evita insinuar que ClawHub respalda el sitio de terceros. No intentes replicar contenido oculto, privado o bloqueado por moderación fuera de la superficie pública de la API.

Los accesos directos de slug web se resuelven entre familias del registro, pero los clientes de API deben usar
...
- Los endpoints de escritura autenticados no deberían devolver un simple `Unauthorized` cuando
  ...
  cuentas eliminadas/prohibidas/deshabilitadas deberían recibir texto accionable para que los clientes
...
- `RateLimit-Reset`: segundos hasta el restablecimiento (retardo)
...
- `Retry-After`: segundos que esperar antes de reintentar (retardo) en `429`
...
- Usa encabezados de IP de cliente de confianza, incluido `cf-connecting-ip`, solo cuando la
  implementación habilita explícitamente los encabezados reenviados de confianza.
- ClawHub usa encabezados de reenvío de confianza para identificar las IP de cliente en el borde.
- Si no hay ninguna IP de cliente de confianza disponible, las solicitudes anónimas usan buckets de reserva
  acotados solo por tipo de límite de tasa. Estos buckets de reserva no incluyen
...
- Los resultados se devuelven por orden de relevancia (similitud de embeddings + aumentos por coincidencia exacta de tokens de slug/nombre + un pequeño factor previo de popularidad).
...
- La popularidad se escala logarítmicamente y se limita. Skills con alta interacción pueden clasificarse más abajo cuando el texto de la consulta coincide peor.
...
- Los alias de cambio de nombre conservan la resolución para URL antiguas e instalaciones que se resuelven mediante el registro, pero el ranking de búsqueda se basa en los metadatos canónicos de la Skill después de que el cambio de nombre se haya indexado. Las estadísticas existentes permanecen con la Skill.
- Si una Skill está inesperadamente invisible, comprueba primero el estado de moderación con `clawhub inspect @owner/slug` mientras tienes sesión iniciada antes de cambiar metadatos relacionados con el ranking.
...
Informa de una Skill para revisión por moderadores. Los informes son a nivel de Skill, opcionalmente vinculados
...
volver a establecer `status` en `open`. Pasa `finalAction: "hide"` con un informe
evaluado para ocultar la Skill en el mismo flujo de trabajo auditable.
...
- `version.security` incluye el estado de verificación de análisis normalizado y detalles del escáner
  (VirusTotal + LLM), cuando están disponibles.

### `GET /api/v1/skills/{slug}/scan`

Devuelve detalles de verificación del análisis de seguridad para una versión de Skill.
...
- Las cargas útiles de solicitudes de escaneo y los informes descargables caducan del almacén de solicitudes de escaneo después de la ventana de retención.
- Los escaneos publicados requieren acceso de gestión del propietario/editor, o autoridad de moderador/administrador de la plataforma.
- Los escaneos publicados escriben los resultados solo cuando `update: true` y el escaneo se completa correctamente.
...
- Los trabajos de escaneo son asíncronos. Las solicitudes de escaneo manual tienen prioridad sobre el trabajo normal de publicación/rellenado, pero su finalización sigue dependiendo de la disponibilidad de workers.
...
- Devuelve `queue.queuedAhead` y `queue.position` mientras está en cola para que los clientes puedan mostrar cuántos escaneos manuales priorizados hay por delante de la solicitud. Las colas muy grandes están acotadas y se informan con `queuedAheadIsEstimate: true`.
...
- Requiere un escaneo completado correctamente; los escaneos no terminales devuelven `409`.
...
- Requiere acceso de gestión del propietario/editor al Skill o plugin, o autoridad de moderador/administrador de la plataforma.
...
- `kind` usa `skill` de forma predeterminada; usa `kind=plugin` para escaneos de plugins/paquetes.
...
- La identidad del Skill, la identidad del editor y los metadatos de la versión seleccionada son campos de sobre de nivel superior (`slug`, `displayName`, `publisherHandle`, `version`, `resolvedFrom`, `tag`, `createdAt`) para que la automatización de shell pueda leerlos sin desempaquetar contenedores anidados.
...
- `security.signals` contiene solo evidencia de apoyo a nivel de estado; usa `/scan` o la página de auditoría de seguridad de ClawHub para ver los detalles completos de los escáneres.
- `security.signals.dependencyRegistry` se conserva por compatibilidad de respuesta v1, pero el escáner de existencia del registro de dependencias está retirado y esta clave siempre es `null`.
- La ausencia de Skill Card no afecta a `ok`, `decision` ni `reasons` de este endpoint; los clientes deberían leer el `skill-card.md` instalado localmente cuando necesiten el contenido de la tarjeta.
- Usa `/verify` cuando necesites el sobre de verificación de Skill Card para un solo Skill, `/card` cuando necesites el Markdown de tarjeta generado y `/scan` cuando necesites datos detallados del escáner.
...
Punto de conexión de catálogo unificado para:
...
- `category` (opcional): filtro de categoría de plugin. Solo se admite cuando la
  ...
  `/api/v1/code-plugins`, `/api/v1/bundle-plugins` o puntos de conexión de paquetes con
  `family=code-plugin`/`family=bundle-plugin`). Las categorías controladas y
  los aliases de filtro heredados de v1 se documentan en `GET /api/v1/plugins`.
...
- Las entradas de Skills siguen respaldadas por el registro de Skills y aún pueden publicarse solo mediante `POST /api/v1/skills`.
...
- `isOfficial` (opcional): `true` o `false`
- `category` (opcional): filtro de categoría de plugin. Solo se admite cuando la
  solicitud está limitada a paquetes de plugins. Las categorías controladas y los aliases
...
Los aliases de filtro heredados de v1 siguen aceptándose en puntos de conexión de lectura:
...
En el punto de conexión unificado `/api/v1/packages` es solo para plugins; usa
...
Exportación masiva de los Skills públicos más recientes para análisis sin conexión.
...
- `startDate` (obligatorio): límite inferior en milisegundos Unix para `updatedAt` del Skill.
- `endDate` (obligatorio): límite superior en milisegundos Unix para `updatedAt` del Skill.
...
- Cuerpo: archivo ZIP.
- Cada Skill exportado tiene como raíz `{publisher}/{slug}/`.
- Los Skills alojados incluyen los archivos de la versión almacenada más reciente y se listan en
  `_manifest.json` con `sourceRef: "public-clawhub"`.
- Los Skills actuales respaldados por GitHub con un escaneo `clean` o `suspicious` incluyen
...
- Las versiones ClawPack incluyen campos compatibles con npm: `npmIntegrity`, `npmShasum` y
  `npmTarballName`.
...
- `trust.blockedFromDownload` es la señal de bloqueo de instalación. OpenClaw y otros
  ...
  volver a derivar reglas de bloqueo de campos de escáner o moderación.
- `trust.reasons` es la lista de explicaciones orientada al usuario y de auditoría. Los códigos de motivo
...
- Los paquetes privados devuelven `404` salvo que el llamador pueda leer el publicador propietario.
- Este endpoint es intencionalmente más limitado que los endpoints de moderación para propietarios/moderadores.
  Expone la decisión de instalación y la explicación pública, no identidades de
  informantes, cuerpos de informes, evidencia privada ni cronogramas internos de revisión.
...
- Las versiones ClawPack devuelven un artefacto `npm-pack`, campos de integridad npm, una
...
- Las versiones ClawPack transmiten los bytes exactos `.tgz` npm-pack cargados.
...
Endpoint de moderador para enumerar filas de migración de plugins oficiales de OpenClaw.
...
Reporta un paquete para revisión del moderador. Los informes son a nivel de paquete y, opcionalmente,
...
Endpoint de moderador/admin para la recepción de reportes de paquetes.

Autenticación:

- Requiere un token de API para un usuario moderador o admin.
...
Endpoint de moderador/admin para resolver o reabrir reportes de paquetes.
...
`note` es obligatorio para `confirmed` y `dismissed`; se puede omitir al
...
Endpoint de moderador/admin para la revisión de versiones de paquetes.
...
- Límite de tamaño de archivo: 200 KB.
- Los análisis pendientes de VirusTotal no bloquean lecturas; las versiones maliciosas aún pueden retenerse en otro lugar.
- Los paquetes privados devuelven `404` salvo que el llamador pueda leer el publicador propietario.
...
- Los packuments de paquetes con scope admiten tanto `/api/npm/@scope/name` como la ruta de solicitud
...
- Las versiones eliminadas lógicamente devuelven `410`.
...
- Campo opcional de carga útil: `migrateOwner`. Cuando es `true` con `ownerHandle`, una
  skill existente puede moverse a ese propietario si el actor es admin/propietario tanto en
...
- Los campos de formulario permitidos son `payload`, blobs `files` repetidos o una referencia de tarball
  ...
  el flujo upload-url. Las publicaciones de storage-id en staging también deben incluir el
...
- Campo opcional de carga útil: `ownerHandle`. Cuando está presente, solo los admins pueden publicar en nombre de ese propietario.
...
- Solo el publicador de la org `openclaw` y los publicadores personales de miembros actuales de la org `openclaw`
  pueden publicar en el canal `official`.
- Las publicaciones en nombre de alguien aún validan la elegibilidad para el canal oficial contra la cuenta propietaria de destino.

### `DELETE /api/v1/skills/{slug}` / `POST /api/v1/skills/{slug}/undelete`

Elimina lógicamente / restaura una skill (propietario, moderador o admin).
...
Las eliminaciones lógicas iniciadas por propietarios reservan el slug durante 30 días; luego otro
publicador puede reclamar el slug. La respuesta de eliminación incluye `slugReservedUntil` cuando aplica esta expiración.
Las ocultaciones de moderador/admin y las eliminaciones de seguridad no expiran de esta manera.
...
Solo admin. Garantiza que exista un publicador de org para un identificador. Si el identificador aún apunta a un
usuario compartido heredado/publicador personal, el endpoint lo migra primero a un publicador de org.
Para una org recién creada, proporciona `memberHandle`; el admin actuante no se agrega como miembro.
`memberRole` toma `owner` de forma predeterminada.
...
Creación autenticada de publicador de org de autoservicio. Crea un nuevo publicador de org y agrega al
...
Solo admin. Reserva slugs raíz y nombres de paquetes para un propietario legítimo sin publicar una
versión. Los nombres de paquetes se convierten en paquetes marcadores privados sin filas de versión, para que el mismo
propietario pueda publicar más adelante la versión real de code-plugin o bundle-plugin con ese nombre.
...
Solo admin. Recupera un publicador personal para un principal de GitHub OAuth de reemplazo verificado
sin editar filas de cuenta de Convex Auth. La solicitud debe nombrar ambos ids de cuenta de proveedor de GitHub
inmutables; los identificadores mutables solo se usan como una protección orientada al operador.
...
del perfil no pueda restaurar la autoridad en competencia del usuario anterior. Cada tabla principal está limitada a
...
Las fuentes de Skills de GitHub están acotadas al publicador y se reportan como comprobadas en lugar de reescribirse.
...
Banea a un usuario y elimina definitivamente sus skills propias (solo moderador/admin).
...
Quita el baneo a un usuario y restaura las skills elegibles (solo admin).
...
La CLI puede descubrir ajustes de registro/autenticación desde el sitio:
...
Si alojas tu propia instancia, sirve este archivo (o configura `CLAWHUB_REGISTRY` explícitamente; heredado `CLAWDHUB_REGISTRY`).
