...
Los directorios de terceros pueden usar los endpoints públicos de lectura para listar o buscar Skills de ClawHub. Almacena los resultados en caché, respeta `429`/`Retry-After`, enlaza a los usuarios de vuelta al listado canónico de ClawHub (`https://clawhub.ai/<owner>/skills/<slug>`) y evita dar a entender que ClawHub avala el sitio de terceros. No intentes replicar contenido oculto, privado o bloqueado por moderación fuera de la superficie pública de la API.

Los accesos directos de slugs web se resuelven entre familias de registro, pero los clientes de API deberían usar
...
- Solicitudes autenticadas (token Bearer válido): se aplica por bucket de usuario.
- Si el token falta o no es válido, el comportamiento vuelve a la aplicación por IP.
- Los endpoints de escritura autenticados no deberían devolver solo `Unauthorized` cuando
  el servidor conoce el motivo. Los tokens faltantes, tokens no válidos o revocados, y
  cuentas eliminadas, bloqueadas o deshabilitadas deberían recibir texto accionable para que los clientes
...
- `X-RateLimit-Remaining` / `RateLimit-Remaining`: presupuesto restante exacto cuando está presente.
  Las solicitudes exitosas con particionamiento omiten este encabezado en lugar de devolver un valor global aproximado.
...
- Los resultados se devuelven en orden de relevancia (similitud de embeddings + aumentos por token exacto de slug/nombre + una pequeña prioridad de popularidad).
...
- El texto ASCII se tokeniza por límites de palabra y puntuación. Por ejemplo, `personal-map` contiene un token independiente `map`, mientras que `amap-jsapi-skill` contiene `amap`, `jsapi` y `skill`; por lo tanto, buscar `map` da a `personal-map` una coincidencia léxica más fuerte que a `amap-jsapi-skill`.
- La popularidad se escala logarítmicamente y se limita. Los Skills con alta interacción pueden clasificarse más bajo cuando el texto de la consulta coincide peor.
...
- No renombres un slug solo para perseguir una consulta, salvo que el slug nuevo sea un mejor nombre canónico a largo plazo. Los slugs antiguos se convierten en alias de redirección, pero la URL canónica, el slug mostrado y los resúmenes de búsqueda futuros usan el slug nuevo.
- Los alias de cambio de nombre preservan la resolución para URL antiguas e instalaciones que resuelven mediante el registro, pero la clasificación de búsqueda se basa en los metadatos canónicos del Skill después de que el cambio de nombre se haya indexado. Las estadísticas existentes permanecen con el Skill.
- Si un Skill está inesperadamente invisible, revisa primero el estado de moderación con `clawhub inspect @owner/slug` mientras tienes la sesión iniciada antes de cambiar metadatos relacionados con la clasificación.
...
Reporta un Skill para revisión por moderadores. Los reportes son a nivel de Skill, opcionalmente vinculados
...
- Las cargas útiles de solicitud de análisis y los informes descargables expiran del almacén de solicitudes de análisis después de la ventana de retención.
...
- Los trabajos de análisis son asíncronos. Las solicitudes de análisis manuales tienen prioridad sobre el trabajo normal de publicación/rellenado, pero la finalización sigue dependiendo de la disponibilidad de workers.
...
- Devuelve `queue.queuedAhead` y `queue.position` mientras está en cola para que los clientes puedan mostrar cuántos análisis manuales priorizados van por delante de la solicitud. Las colas muy grandes se acotan y se informan con `queuedAheadIsEstimate: true`.
...
- Requiere acceso de administración de propietario/editor a la skill o al plugin, o autoridad de moderador/administrador de la plataforma.
- Devuelve resultados de análisis almacenados para la versión exacta enviada, incluidas versiones bloqueadas u ocultas.
...
Ruta canónica de reanálisis por lotes solo para administradores. Acepta la misma forma de carga útil que la ruta heredada `POST /api/v1/skills/-/rescan-batch`.
...
- La identidad de la skill, la identidad del editor y los metadatos de la versión seleccionada son campos de sobre de nivel superior (`slug`, `displayName`, `publisherHandle`, `version`, `resolvedFrom`, `tag`, `createdAt`) para que la automatización de shell pueda leerlos sin desempaquetar envoltorios anidados.
...
Devuelve veredictos compactos de seguridad actuales para versiones exactas de skills. Este
...
- `security.signals.dependencyRegistry` se conserva por compatibilidad con respuestas v1, pero el escáner de existencia del registro de dependencias está retirado y esta clave siempre es `null`.
- La ausencia de Skill Card no afecta el `ok`, `decision` ni `reasons` de este endpoint; los clientes deben leer el `skill-card.md` instalado localmente cuando necesiten el contenido de la tarjeta.
- Usa `/verify` cuando necesites el sobre de verificación de Skill Card de una sola skill, `/card` cuando necesites Markdown de tarjeta generada y `/scan` cuando necesites datos detallados del escáner.
...
- `isOfficial` (opcional): `true` o `false`
- `category` (opcional): filtro de categoría de plugin. Solo se admite cuando la
  solicitud está limitada a paquetes de plugin. Las categorías controladas y los alias de
  filtro heredados de v1 están documentados en `GET /api/v1/plugins`.
...
- `cursor` (opcional): cursor de paginación de la respuesta anterior.
- `family` (opcional): `code-plugin` o `bundle-plugin`. Omitirlo significa ambas
  familias de plugins.
...
- Los resultados se devuelven en orden de relevancia y actualmente no están paginados.
...
- Endpoint de lectura pública. No se requiere token de propietario, editor, moderador ni administrador.
...
- `trust.moderationState` admite valores nulos. Es `null` cuando no existe moderación manual de versión.
...
- `trust.reasons` es la lista de explicación orientada al usuario y de auditoría. Los códigos de motivo
  ...
  y `package:malicious`.
- `trust.pending` significa que una o más entradas de confianza siguen esperando completarse.
- `trust.stale` significa que el resumen de confianza se calculó a partir de entradas desactualizadas y
  debería tratarse como que requiere actualización antes de una decisión de permiso de alta confianza.
...
- Esta es la superficie de resolvedor de OpenClaw; evita inferir el formato de archivo a partir
...
- `packageName` se normaliza como nombre npm; el paquete puede faltar en migraciones
  planificadas.
- Esto solo hace seguimiento de la preparación de migración. No muta OpenClaw ni genera
  ClawPacks.
...
Reporta un paquete para revisión de moderador. Los reportes son a nivel de paquete, opcionalmente
vinculados a una versión. Alimentan la cola de moderación, pero por sí mismos no ocultan automáticamente ni
bloquean descargas; los moderadores deberían usar la moderación de versiones para
...
- Usa de forma predeterminada la versión más reciente.
- Skills redirigen a `GET /api/v1/download`.
...
- Las respuestas incluyen los encabezados `ETag`, `Digest`, `X-ClawHub-Artifact-Type` y
  `X-ClawHub-Artifact-Sha256` para comprobaciones de integridad del solucionador.
...
- Las transferencias de Skills respaldadas por GitHub no hacen proxy ni espejan bytes. La respuesta JSON
...
- Campo opcional de carga útil: `migrateOwner`. Cuando es `true` con `ownerHandle`, una
  ...
  los publicadores actuales como de destino. Sin esta aceptación explícita, los cambios de propietario se
  rechazan.
...
- Las publicaciones en nombre de otro siguen validando la elegibilidad del canal oficial contra la cuenta propietaria de destino.
...
publicador puede reclamarlo. La respuesta de eliminación incluye `slugReservedUntil` cuando aplica esta expiración.
...
Solo admin. Garantiza que exista un publicador de org para un handle. Si el handle aún apunta a un
usuario/publicador personal compartido heredado, el endpoint lo migra primero a un publicador de org.
Para una org recién creada, proporciona `memberHandle`; el admin actuante no se añade como miembro.
...
Creación autenticada de publicador de org de autoservicio. Crea un nuevo publicador de org y añade al
...
- Devuelve `409` cuando el handle ya está usado por un publicador, usuario o publicador personal.

### `POST /api/v1/users/reserve`

Solo admin. Reserva slugs raíz y nombres de paquetes para un propietario legítimo sin publicar una
versión. Los nombres de paquetes se convierten en paquetes privados de marcador de posición sin filas de versión, de modo que el mismo
...
Solo admin. Recupera un publicador personal para un principal de OAuth de GitHub de reemplazo verificado
...
los alias de slug de Skills, los paquetes, las advertencias del inspector de paquetes y las filas derivadas de resumen de búsqueda para que
...
- Cuerpo: `{ "handle": "gingiris", "nextUserHandle": "gingiris-1031", "previousGitHubProviderAccountId": "123", "nextGitHubProviderAccountId": "456", "reason": "Continuidad de cuenta verificada para el issue #2555", "confirmIdentityVerified": true, "dryRun": false }`
- Respuesta: `{ "ok": true, "dryRun": false, "recovered": true, "publisherId": "...", "handle": "gingiris", "previousUser": { "userId": "...", "handle": "gingiris", "nextHandle": "gingiris-recovered", "githubProviderAccountId": "123", "authAccountCount": 1 }, "nextUser": { "userId": "...", "handle": "gingiris-1031", "nextHandle": "gingiris", "githubProviderAccountId": "456", "authAccountCount": 1 }, "retiredPersonalPublisher": null, "resourceOwnerMigration": { "limitPerTable": 100, "skills": 1, "skillSlugAliases": 1, "packages": 0, "packageInspectorWarnings": 0, "githubSourcesChecked": 1, "handleReservations": 1 }, "identityVerified": true, "reason": "Continuidad de cuenta verificada para el issue #2555" }`
...
- `POST /api/v1/skills/{slug}/transfer`
  - Cuerpo: `{ "toUserHandle": "target_handle", "message": "opcional" }`
...
```json
{ "handle": "user_handle", "reason": "motivo de baneo opcional" }
```

o

```json
{ "userId": "users_...", "reason": "motivo de baneo opcional" }
```
...
```json
{ "handle": "user_handle", "reason": "motivo opcional para quitar el baneo" }
```

o

```json
{ "userId": "users_...", "reason": "motivo opcional para quitar el baneo" }
```
...
contenido (solo administrador). Usa ejecución de prueba de forma predeterminada salvo que `dryRun` sea `false`.
...
```json
{
  ...
  "userId": "users_...",
  "handle": "user_handle",
  "previousReason": "baneo automático por malware",
...
Añade/quita una estrella (destacados). Ambos endpoints son idempotentes.
...
que preparen un tarball ClawPack deben enviar el id de almacenamiento resultante como
