...
x-i18n:
    generated_at: "2026-07-02T22:21:33Z"
    model: gpt-5.5
...
- Los endpoints de escritura autenticados no deberían devolver un simple `Unauthorized` cuando
  el servidor conoce el motivo. Los tokens faltantes, no válidos o revocados, y las cuentas
  eliminadas, prohibidas o deshabilitadas deberían recibir texto accionable para que los clientes
...
- ClawHub usa encabezados de reenvío confiables para identificar las IP de los clientes en el borde.
- Si no hay una IP de cliente confiable disponible, las solicitudes anónimas usan buckets de reserva
  con alcance solo por tipo de límite de tasa. Estos buckets de reserva no incluyen
  rutas, slugs, nombres de paquete, versiones, cadenas de consulta u otros
  parámetros de artefactos proporcionados por el llamador.
...
- Los resultados se devuelven en orden de relevancia (similitud de embeddings + refuerzos por coincidencia exacta de tokens de slug/nombre + un pequeño ajuste previo de popularidad).
- La relevancia pesa más que la popularidad. Una coincidencia precisa de slug o token de nombre visible puede superar a una coincidencia más imprecisa con mucha más interacción.
...
Endpoint de moderador/admin para la admisión de reportes de Skills.
...
triajeado para ocultar el Skill en el mismo flujo auditable.
...
- Las cargas útiles de solicitud de escaneo y los informes descargables caducan en el almacén de solicitudes de escaneo después del periodo de retención.
...
- Los trabajos de escaneo son asíncronos. Las solicitudes de escaneo manual tienen prioridad sobre el trabajo normal de publicación/relleno, pero la finalización aún depende de la disponibilidad de los trabajadores.
...
- `kind` tiene `skill` como valor predeterminado; usa `kind=plugin` para escaneos de plugins/paquetes.
...
- `security.signals.dependencyRegistry` se conserva por compatibilidad con la respuesta v1, pero el escáner de existencia del registro de dependencias está retirado y esta clave siempre es `null`.
- La ausencia de Skill Card no afecta `ok`, `decision` ni `reasons` de este endpoint; los clientes deben leer `skill-card.md` instalado localmente cuando necesiten el contenido de la tarjeta.
- Usa `/verify` cuando necesites el sobre de verificación de Skill Card para una sola Skill, `/card` cuando necesites Markdown de tarjeta generado y `/scan` cuando necesites datos detallados de escáner.
...
Exploración de catálogo solo de Plugin en paquetes de code-plugin y bundle-plugin.
...
- Los paquetes privados devuelven `404` a menos que quien llama pueda leer el publicador propietario.
- Este endpoint es intencionadamente más limitado que los endpoints de moderación de propietario/moderador.
...
- `packageName` se normaliza como nombre npm; el paquete puede faltar para migraciones
  planificadas.
- Esto rastrea solo la preparación de migración. No muta OpenClaw ni genera
  ClawPacks.
...
Endpoint de moderador/admin para revisión de versiones de paquetes.
...
- Campo opcional de carga útil: `migrateOwner`. Cuando es `true` con `ownerHandle`, una
  skill existente puede moverse a ese propietario si el actor es admin/propietario tanto en los
...
Las ocultaciones por moderador/admin y las eliminaciones de seguridad no expiran de esta forma.
...
Solo admins. Garantiza que exista un publicador de organización para un identificador. Si el identificador todavía apunta a un
usuario compartido heredado/publicador personal, el endpoint lo migra primero a un publicador de organización.
Para una organización recién creada, proporciona `memberHandle`; el admin actuante no se añade como miembro.
...
Solo admins. Reserva slugs raíz y nombres de paquete para un propietario legítimo sin publicar una
...
propietario pueda publicar después la versión real de code-plugin o bundle-plugin en ese nombre.
...
Solo admins. Recupera un publicador personal para un principal de OAuth de GitHub de reemplazo verificado
sin editar filas de cuenta de Convex Auth. La solicitud debe nombrar ambos ids inmutables de cuenta de proveedor
de GitHub; los identificadores mutables solo se usan como una protección orientada al operador.
...
La recuperación también migra los campos heredados `ownerUserId` para los Skills del publicador recuperado,
...
Bloquea a un usuario y elimina de forma permanente los Skills de su propiedad (solo moderador/administrador).
...
Desbloquea a un usuario y restaura los Skills elegibles (solo administrador).
...
Si lo alojas por tu cuenta, sirve este archivo (o establece `CLAWHUB_REGISTRY` explícitamente; `CLAWDHUB_REGISTRY` heredado).
