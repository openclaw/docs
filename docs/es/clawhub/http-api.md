...
x-i18n:
    generated_at: "2026-05-11T22:19:23Z"
    model: gpt-5.5
    provider: openai
...
- Los resultados se devuelven en orden de relevancia (similitud de embeddings + refuerzos por coincidencia exacta de tokens de slug/nombre + prioridad de popularidad a partir de descargas).
- La relevancia pesa más que la popularidad. Una coincidencia precisa de slug o token de nombre visible puede superar a una coincidencia más imprecisa con muchas más descargas.
...
- Las descargas se usan como una pequeña prioridad escalada logarítmicamente y como desempate, no como señal principal de ranking. Las Skills con muchas descargas pueden clasificarse más abajo cuando el texto de consulta tiene una coincidencia más débil.
...
- Si una Skill está inesperadamente invisible, revisa primero el estado de moderación con `clawhub inspect <slug>` mientras tienes sesión iniciada antes de cambiar metadatos relacionados con el ranking.
...
- `npmMirror` (opcional): `true`/`1` para mostrar versiones de paquetes respaldadas
  por ClawPack disponibles mediante el espejo npm

Notas:

- `GET /api/v1/code-plugins` y `GET /api/v1/bundle-plugins` siguen siendo alias de familia fija.
- Las entradas de Skills siguen respaldadas por el registro de Skills y solo se pueden seguir publicando mediante `POST /api/v1/skills`.
...
- Las versiones de paquetes heredadas devuelven un artefacto `legacy-zip` y una URL de descarga
  ZIP heredada `downloadUrl`.
...
- Esta es la superficie del resolvedor de OpenClaw; evita inferir el formato del archivo a partir de
  una URL compartida.
...
- `packageName` se normaliza como nombre npm; el paquete puede faltar para migraciones
  planificadas.
- Esto solo realiza el seguimiento de la preparación de la migración. No modifica OpenClaw ni genera
  ClawPacks.
...
Endpoint de moderador/administrador para recepción de reportes de paquetes.
...
- El valor predeterminado es ejecución de prueba.
...
- Los escaneos pendientes de VirusTotal no bloquean las lecturas; las versiones maliciosas aún pueden retenerse en otro lugar.
...
Las ocultaciones y eliminaciones de seguridad de moderador/administrador no expiran de esta manera.
...
versión. Los nombres de paquetes se convierten en paquetes marcador privados sin filas de versión, por lo que el mismo
...
Desbanea a un usuario y restaura las skills elegibles (solo administrador).
