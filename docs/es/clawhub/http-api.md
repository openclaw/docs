...
Los directorios de terceros pueden usar los endpoints públicos de lectura para listar o buscar Skills de ClawHub. Almacena los resultados en caché, respeta `429`/`Retry-After`, dirige a los usuarios de vuelta al listado canónico de ClawHub (`https://clawhub.ai/<owner>/skills/<slug>`) y evita sugerir que ClawHub respalda el sitio de terceros. No intentes reflejar contenido oculto, privado o bloqueado por moderación fuera de la superficie de la API pública.

Los atajos de slug web se resuelven entre familias del registro, pero los clientes de API deben usar
...
- Solicitudes autenticadas (token Bearer válido): se aplican por contenedor de usuario.
- Si el token falta o no es válido, el comportamiento recurre a la aplicación por IP.
- Los endpoints autenticados de escritura no deben devolver solo `Unauthorized` cuando
  el servidor conoce el motivo. Los tokens ausentes, tokens no válidos/revocados y
  cuentas eliminadas/baneadas/deshabilitadas deben recibir texto procesable para que los clientes
...
- Si no hay una IP de cliente confiable disponible, las solicitudes anónimas usan contenedores de respaldo
  delimitados solo por tipo de límite de tasa. Estos contenedores de respaldo no incluyen
  rutas, slugs, nombres de paquete, versiones, cadenas de consulta ni otros parámetros
  de artefacto suministrados por el llamador.
...
- Los resultados se devuelven en orden de relevancia (similitud de incrustación + impulsos por coincidencia exacta de tokens de slug/nombre + un pequeño prior de popularidad).
- La relevancia tiene más peso que la popularidad. Una coincidencia precisa de slug o token de nombre mostrado puede superar una coincidencia más imprecisa con mucha más interacción.
...
- La popularidad se escala logarítmicamente y se limita. Los Skills con alta interacción pueden clasificarse por debajo cuando el texto de la consulta coincide peor.
...
- Incluye los términos que los usuarios buscarán literalmente en el nombre mostrado, el resumen y las etiquetas. Usa un token de slug independiente solo cuando también sea una identidad estable que quieras conservar.
...
- Si un Skill es inesperadamente invisible, comprueba primero el estado de moderación con `clawhub inspect @owner/slug` mientras tienes la sesión iniciada antes de cambiar metadatos relacionados con la clasificación.
...
Informa de un Skill para revisión de moderadores. Los informes son de nivel de Skill, opcionalmente vinculados
...
- `version.security` incluye el estado normalizado de verificación de análisis y detalles del escáner
...
Los análisis de subidas locales ya no son compatibles. Las solicitudes que usan
...
- Las cargas de solicitud de análisis y los informes descargables expiran del almacén de solicitudes de análisis después de la ventana de retención.
...
- Los trabajos de análisis son asincrónicos. Las solicitudes de análisis manuales tienen prioridad sobre el trabajo normal de publicación/rellenado, pero la finalización sigue dependiendo de la disponibilidad de workers.
...
- `kind` tiene el valor predeterminado `skill`; usa `kind=plugin` para análisis de Plugin/paquete.
- Devuelve la misma forma de ZIP que las descargas de solicitudes de análisis.

### `POST /api/v1/skills/-/scan/batch`

Ruta canónica de reanálisis por lotes solo para administradores. Acepta la misma forma de carga que el `POST /api/v1/skills/-/rescan-batch` heredado.
...
- `security.signals.dependencyRegistry` se conserva por compatibilidad de respuesta v1, pero el escáner de existencia del registro de dependencias está retirado y esta clave siempre es `null`.
- La ausencia de Skill Card no afecta el `ok`, `decision` ni `reasons` de este endpoint; los clientes deberían leer el `skill-card.md` instalado localmente cuando necesiten el contenido de la tarjeta.
- Usa `/verify` cuando necesites el sobre de verificación de Skill Card de un solo Skill, `/card` cuando necesites el Markdown de la tarjeta generada y `/scan` cuando necesites datos detallados del escáner.
...
- Los solicitantes autenticados pueden ver paquetes privados de los publicadores a los que pertenecen en los resultados de lista/búsqueda.
...
- `category` (opcional): filtro de categoría de plugin. Solo se admite cuando la
  ...
  heredados del filtro v1 están documentados en `GET /api/v1/plugins`.
...
Los alias heredados no se aceptan como valores de categoría almacenados ni declarados por autores.
...
- Los Skills alojados incluyen los archivos de la versión almacenada más reciente y aparecen en
...
- `version.artifact.kind` es `legacy-zip` para archivos de paquetes del modelo anterior o
...
- Esta es la superficie del resolvedor de OpenClaw; evita inferir el formato del archivo desde
  una URL compartida.
...
Reporta un paquete para revisión por moderador. Los informes son a nivel de paquete, opcionalmente
...
Punto de conexión de moderador/administrador para la ingesta de informes de paquetes.
...
- `revoked`: bloqueada después de que una versión se considerara confiable anteriormente.
...
- Las transferencias de skills respaldadas por GitHub no hacen proxy ni reflejan bytes. La respuesta JSON
  ...
  y `archiveUrl`; el estado de análisis/actual es una puerta de control y no se incluye como metadatos de carga útil
  de éxito.
...
- Los campos de formulario permitidos son `payload`, blobs `files` repetidos o una referencia de tarball
  ...
  el flujo upload-url. Las publicaciones de id de almacenamiento preparado también deben incluir el
...
- Las solicitudes directas de publicación multipart tienen un límite de 18 MB. Los tarballs ClawPack pueden
...
- Las publicaciones en nombre de otro aún validan la elegibilidad del canal oficial contra la cuenta propietaria de destino.
...
otro publicador. La respuesta de eliminación incluye `slugReservedUntil` cuando se aplica esta expiración.
...
Creación autoservicio autenticada de publicador de organización. Crea un nuevo publicador de organización y añade al
...
versión. Los nombres de paquete se convierten en paquetes de marcador privados sin filas de versión, para que el mismo
...
proveedor de GitHub; los handles mutables solo se usan como una protección visible para el operador.

El punto de conexión usa simulación de forma predeterminada. Aplicar la recuperación requiere `dryRun: false` y
...
los alias de slug de Skill, paquetes, advertencias del inspector de paquetes y filas derivadas de resumen de búsqueda para que
...
### Puntos de conexión de gestión de slugs de propietario
...
- Ambos puntos de conexión requieren autenticación con token de API y solo funcionan para el propietario de la Skill.
...
### Puntos de conexión de transferencia de propiedad
...
## Puntos de conexión heredados de CLI (obsoletos)
...
Si alojas tu propia instancia, sirve este archivo (o establece `CLAWHUB_REGISTRY` explícitamente; `CLAWDHUB_REGISTRY` heredado).
