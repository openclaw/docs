...
- Los endpoints de escritura autenticados no deben devolver un `Unauthorized` sin más cuando
...
- Si falta `Retry-After`, usa `RateLimit-Reset` (o calcula a partir de `X-RateLimit-Reset`).
...
- Los resultados se devuelven en orden de relevancia (similitud de embedding + impulsos por tokens exactos de slug/nombre + una pequeña prioridad de popularidad).
- La relevancia pesa más que la popularidad. Una coincidencia precisa de slug o token de nombre para mostrar puede superar a una coincidencia más laxa con mucha más interacción.
...
- Los alias de cambio de nombre preservan la resolución para URL antiguas e instalaciones que se resuelven mediante el registro, pero el ranking de búsqueda se basa en los metadatos canónicos de la Skill después de que el cambio de nombre se haya indexado. Las estadísticas existentes permanecen con la Skill.
...
clasificado para ocultar la Skill en el mismo flujo auditable.
...
- `version.security` incluye estado de verificación de escaneo normalizado y detalles del escáner
...
- Usa `/verify` cuando necesites el sobre de verificación de Skill Card para una sola skill, `/card` cuando necesites markdown de tarjeta generada y `/scan` cuando necesites datos detallados del escáner.
...
- `category` (opcional): filtro de categoría de plugin. Solo se admite cuando la
  solicitud tiene como ámbito paquetes de plugins (`/api/v1/plugins`,
...
Búsqueda de catálogo unificado entre Skills + paquetes de plugins.
...
- `isOfficial` (opcional): `true` o `false`
- `category` (opcional): filtro de categoría de plugin. Solo se admite cuando la
  solicitud tiene como ámbito paquetes de plugins. Las categorías controladas y los alias de filtros
  heredados de v1 están documentados en `GET /api/v1/plugins`.
...
`trending` es una tabla de clasificación de instalaciones/descargas de siete días y no usa totales históricos.
...
- También se aceptan los alias de filtros legacy de v1 documentados en `GET /api/v1/plugins`.
...
- Requiere un token de API para el propietario del paquete, un propietario/administrador del editor de la organización,
...
Devuelve el resumen exacto de seguridad y confianza de la versión del paquete para los clientes de instalación. Esta es la superficie pública de consumo de OpenClaw para decidir si una
...
- `trust.moderationState` admite nulo. Es `null` cuando no existe moderación manual de
  la versión.
...
- Las versiones de paquetes legacy devuelven un artefacto `legacy-zip` y una URL legacy de ZIP
  `downloadUrl`.
- Las versiones de ClawPack devuelven un artefacto `npm-pack`, campos de integridad npm, una
  `tarballUrl` y la URL legacy de compatibilidad ZIP.
- Esta es la superficie de resolución de OpenClaw; evita inferir el formato del archivo a partir de
  una URL compartida.
...
- Las versiones legacy ZIP redirigen a `/api/v1/packages/{name}/download?version=`.
...
Reporta un paquete para revisión por moderadores. Los informes son a nivel de paquete, opcionalmente
...
Punto de conexión de moderador/administrador para la recepción de reportes de paquetes.
...
Punto de conexión de moderador/administrador para resolver o reabrir reportes de paquetes.
...
- Usa de forma predeterminada la versión más reciente.
- Usa el depósito de límite de lectura, no el depósito de descarga.
...
- Usa el depósito de límite de descarga.
...
Descarga un ZIP de versión de Skill alojado, o devuelve una transferencia a fuente de GitHub para un
Skill actual respaldado por GitHub con un análisis `clean` o `suspicious` y sin versión
alojada.
...
- Las transferencias de Skills respaldados por GitHub no hacen proxy ni reflejan bytes. La respuesta JSON
  incluye `sourceRef: "public-github"`, `repo`, `commit`, `path`, `contentHash`
  y `archiveUrl`; el análisis/estado actual es una barrera y no se incluye como metadatos de carga útil
  de éxito.
...
- Campo de carga útil opcional: `migrateOwner`. Cuando es `true` con `ownerHandle`, un
  Skill existente puede moverse a ese propietario si el actor es administrador/propietario tanto en
  el publicador actual como en el de destino. Sin esta participación explícita, los cambios de propietario se
  rechazan.
...
- Los campos de formulario permitidos son `payload`, blobs `files` repetidos o una referencia de tarball
  ...
  el flujo de URL de carga. Las publicaciones preparadas con id de almacenamiento también deben incluir el
...
- Las solicitudes directas de publicación multipart tienen un límite de 18MB. Los tarballs de ClawPack pueden
  usar el flujo de URL de carga hasta el límite de tarball de 120MB.
...
Elimina de forma reversible / restaura un Skill (propietario, moderador o administrador).
...
Cuando está presente, `reason` se almacena como la nota de moderación del Skill y se copia en el registro de auditoría.
...
otro publicador. La respuesta de eliminación incluye `slugReservedUntil` cuando esta caducidad aplica.
Los ocultamientos de moderador/administrador y las eliminaciones de seguridad no caducan de esta manera.
...
publicador compartido heredado de usuario/personal, el punto de conexión primero lo migra a un publicador de organización.
Para una organización recién creada, proporciona `memberHandle`; el administrador actuante no se agrega como miembro.
`memberRole` se establece de forma predeterminada en `owner`.
...
versión. Los nombres de paquetes se convierten en paquetes de marcador privados sin filas de versión, de modo que el mismo
...
los identificadores mutables solo se usan como una barrera orientada al operador.
...
Las fuentes de Skills de GitHub están delimitadas por publicador y se informan como comprobadas en lugar de reescribirse.
...
contenido (solo admin). Usa dry-run de forma predeterminada a menos que `dryRun` sea `false`.
...
## Endpoints de CLI heredada (obsoletos)
