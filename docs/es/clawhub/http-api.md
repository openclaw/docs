...
x-i18n:
    generated_at: "2026-07-05T04:48:32Z"
    model: gpt-5.5
...
Los accesos directos de slugs web se resuelven entre familias de registro, pero los clientes de la API deben usar
...
- `X-RateLimit-Remaining` / `RateLimit-Remaining`: presupuesto restante exacto cuando está presente.
  Las solicitudes fragmentadas correctas omiten esta cabecera en lugar de devolver un valor global aproximado.
...
- ClawHub usa cabeceras de reenvío de confianza para identificar las IP de cliente en el edge.
- Si no hay una IP de cliente de confianza disponible, las solicitudes anónimas usan buckets de respaldo
  acotados solo por tipo de límite de tasa. Estos buckets de respaldo no incluyen
...
- Los resultados se devuelven en orden de relevancia (similitud de embedding + aumentos por coincidencia exacta de tokens de slug/nombre + una pequeña prioridad de popularidad).
...
- La popularidad se escala logarítmicamente y se limita. Las Skills con alto engagement pueden posicionarse más abajo cuando el texto de consulta tiene una coincidencia más débil.
- El estado de moderación sospechoso u oculto puede eliminar una Skill de la búsqueda pública según los filtros del llamador y el estado actual de moderación.
...
- Los alias de renombrado preservan la resolución para URL antiguas e instalaciones que se resuelven a través del registro, pero la clasificación de búsqueda se basa en los metadatos canónicos de la Skill después de que el renombrado se haya indexado. Las estadísticas existentes permanecen con la Skill.
- Si una Skill es inesperadamente invisible, comprueba primero el estado de moderación con `clawhub inspect @owner/slug` mientras tienes sesión iniciada antes de cambiar metadatos relacionados con el ranking.
...
- Los trabajos de escaneo son asíncronos. Las solicitudes de escaneo manual tienen prioridad sobre el trabajo normal de publicación/rellenado, pero la finalización sigue dependiendo de la disponibilidad de workers.
...
- Devuelve la misma forma de ZIP que las descargas de solicitud de escaneo.
...
- `security.signals` contiene evidencia de escáner de soporte, como `staticScan`, `virusTotal` y `skillSpector`.
...
- `security.signals` contiene solo evidencia de soporte a nivel de estado; usa `/scan` o la página de auditoría de seguridad de ClawHub para ver todos los detalles del escáner.
- `security.signals.dependencyRegistry` se conserva por compatibilidad con respuestas v1, pero el escáner de existencia del registro de dependencias está retirado y esta clave siempre es `null`.
...
Búsqueda en catálogo unificado entre skills y paquetes de plugin.
...
- El filtrado por categoría es un filtro real de API respaldado por filas de resumen de
  categorías de Plugin, no una reescritura de la consulta de búsqueda.
...
Realiza una eliminación lógica de un paquete y todas sus versiones.
...
- `trust.scanStatus` es el estado de confianza efectivo derivado de entradas de escáner
  y la moderación manual de versiones.
- `trust.moderationState` admite nulo. Es `null` cuando no existe moderación manual de la
  versión.
...
- Este punto de conexión es exacto por versión. Los clientes deben llamarlo después de resolver la
  ...
  del paquete.
- Los paquetes privados devuelven `404` a menos que el llamador pueda leer el publicador propietario.
- Este punto de conexión es intencionalmente más estrecho que los puntos de conexión de moderación de propietario/moderador.
...
- Las versiones de paquetes heredadas devuelven un artefacto `legacy-zip` y una `downloadUrl`
  ZIP heredada.
...
Devuelve la preparación calculada para el consumo futuro por OpenClaw.
...
- repositorio de origen y procedencia de commit
...
- Requiere un token de API para el propietario del paquete, miembro del publicador,
...
- Los packuments de paquetes con ámbito admiten tanto `/api/npm/@scope/name` como la ruta
  de solicitud codificada de npm `/api/npm/@scope%2Fname`.
...
- Las versiones eliminadas de forma recuperable devuelven `410`.
- Las transferencias de Skills respaldadas por GitHub no actúan como proxy ni reflejan bytes. La respuesta JSON
  incluye `sourceRef: "public-github"`, `repo`, `commit`, `path`, `contentHash`
  y `archiveUrl`; el estado de análisis/actual es una puerta de validación y no se incluye como metadatos
...
- Campo opcional de carga útil: `migrateOwner`. Cuando es `true` con `ownerHandle`, una
  ...
  el publicador actual como en el de destino. Sin esta adhesión explícita, los cambios de propietario se
  rechazan.
...
- Los campos de formulario permitidos son `payload`, blobs `files` repetidos o una referencia de tarball
  ...
  el flujo upload-url. Las publicaciones preparadas con id de almacenamiento también deben incluir el
...
- Los plugins de código requieren `package.json`, metadatos del repositorio de código fuente, metadatos de commit
  de código fuente, metadatos de esquema de configuración, `openclaw.compat.pluginApi` y
  `openclaw.build.openclawVersion`.
- `openclaw.hostTargets` y `openclaw.environment` son metadatos opcionales.
- Solo el publicador de la organización `openclaw` y los publicadores personales de miembros actuales de la organización
  `openclaw` pueden publicar en el canal `official`.
- Las publicaciones en nombre de otro usuario aún validan la elegibilidad del canal oficial contra la cuenta de propietario de destino.

### `DELETE /api/v1/skills/{slug}` / `POST /api/v1/skills/{slug}/undelete`

Elimina de forma recuperable / restaura una skill (propietario, moderador o administrador).
...
Las eliminaciones recuperables iniciadas por el propietario reservan el slug durante 30 días; luego otro
...
Creación autenticada de autoservicio de publicadores de organización. Crea un nuevo publicador de organización y añade al
...
propietario pueda publicar más tarde la versión code-plugin o bundle-plugin real con ese nombre.
...
proveedor de GitHub; los handles mutables solo se usan como protección orientada al operador.

El endpoint usa de forma predeterminada una ejecución de prueba. Aplicar la recuperación requiere `dryRun: false` y
...
los alias de slugs de Skills, los paquetes, las advertencias del inspector de paquetes y las filas derivadas de resumen de búsqueda para que
...
para el identificador recuperado también se reasigna al usuario sustituto para que la sincronización posterior del perfil
...
Las fuentes de Skills de GitHub están acotadas al editor y se informan como comprobadas en lugar de reescribirse.
...
Bloquea a un usuario y elimina de forma definitiva las Skills que posee (solo moderador/administrador).
...
Si lo alojas tú mismo, sirve este archivo (o define `CLAWHUB_REGISTRY` explícitamente; `CLAWDHUB_REGISTRY` heredado).
