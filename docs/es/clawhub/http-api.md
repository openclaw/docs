...
- Los endpoints de escritura autenticados no deben devolver un simple `Unauthorized` cuando
  ...
  de CLI puedan indicar a los usuarios qué los bloqueó.
...
- `Retry-After`: segundos a esperar antes de reintentar (retraso) en `429`
...
- Usa `cf-connecting-ip` (Cloudflare) como IP del cliente de forma predeterminada.
- ClawHub usa encabezados de reenvío de confianza para identificar IPs de clientes en el borde.
- Si no hay una IP de cliente de confianza disponible, las solicitudes de descarga anónimas usan un bucket de respaldo con alcance de endpoint en lugar de un bucket global `ip:unknown`. Las solicitudes anónimas de lectura/escritura aún usan el bucket desconocido compartido para que el enrutamiento sin IP siga siendo visible y conservador.
...
- Los resultados se devuelven en orden de relevancia (similitud de embeddings + mejoras por coincidencias exactas de tokens de slug/nombre + prioridad de popularidad por descargas).
- La relevancia pesa más que la popularidad. Una coincidencia precisa de slug o token de nombre visible puede superar a una coincidencia más laxa con muchas más descargas.
...
- Las descargas se usan como una pequeña prioridad con escala logarítmica y como desempate, no como señal principal de ranking. Los Skills con muchas descargas pueden posicionarse más abajo cuando el texto de la consulta coincide peor.
- Un estado de moderación sospechoso u oculto puede retirar un Skill de la búsqueda pública según los filtros del llamador y el estado de moderación actual.
...
- Los alias de cambio de nombre preservan la resolución para URL antiguas e instalaciones que se resuelven a través del registro, pero el ranking de búsqueda se basa en los metadatos canónicos del Skill después de que el cambio de nombre se haya indexado. Las estadísticas existentes permanecen con el Skill.
- Si un Skill está inesperadamente invisible, comprueba primero el estado de moderación con `clawhub inspect <slug>` tras iniciar sesión antes de cambiar metadatos relacionados con el ranking.
...
Reporta un Skill para revisión de moderadores. Los reportes tienen alcance de Skill, están vinculados opcionalmente
...
evaluado para ocultar el Skill en el mismo flujo auditable.
...
- Incluye estado de verificación normalizado más detalles específicos del escáner.
...
- `os`, `arch`, `libc` (opcional): abreviatura para filtros de capacidad del host
- `requiresBrowser`, `requiresDesktop`, `requiresNativeDeps`,
  `requiresExternalService`, `requiresBinary`, `requiresOsPermission`
  (opcional): abreviatura `true`/`1` para etiquetas de requisitos de entorno
- `externalService`, `binary`, `osPermission` (opcional): abreviatura para etiquetas de requisitos de entorno con nombre
- `artifactKind` (opcional): `legacy-zip` o `npm-pack`
- `npmMirror` (opcional): `true`/`1` para mostrar versiones de paquete respaldadas por ClawPack
...
- Los llamadores autenticados pueden ver paquetes privados de los publicadores a los que pertenecen en los resultados de listas/búsquedas.
...
- `artifactKind` (opcional): `legacy-zip` o `npm-pack`
- `npmMirror` (opcional): `true`/`1` para buscar versiones de paquete respaldadas por ClawPack
  disponibles a través del espejo de npm
...
- `channel=private` solo devuelve paquetes que el llamador autenticado puede leer.
- Los filtros de artefacto están respaldados por etiquetas de capacidad indexadas:
...
Elimina de forma reversible un paquete y todos los lanzamientos.
...
- Esta es la superficie del resolutor de OpenClaw; evita adivinar el formato de archivo a partir
...
- Usa el bucket de límite de descargas.
...
bloquean descargas por sí mismos; los moderadores deben usar la moderación de lanzamientos para
...
Endpoint de moderador/administrador para entrada de reportes de paquetes.
...
Endpoint de mantenimiento solo para administradores para etiquetar versiones antiguas de paquetes con
...
- Las filas existentes respaldadas por ClawPack a las que les falta `artifactKind` se reparan como
  `npm-pack`.
- Esto no genera ClawPacks ni muta bytes de artefactos.
...
- El valor predeterminado es la última versión.
...
Descarga un zip de una versión de una skill.
...
- Campo opcional de carga útil: `migrateOwner`. Cuando es `true` con `ownerHandle`, una
  ...
  los publicadores actuales como en los de destino. Sin esta suscripción explícita, los cambios de propietario se
  rechazan.
...
- Los plugins de código requieren `package.json`, metadatos del repositorio de código fuente, metadatos de commit de código fuente,
...
Elimina/restaura de forma reversible una skill (propietario, moderador o administrador).
...
La respuesta de eliminación incluye `slugReservedUntil` cuando aplica esta caducidad.
Las ocultaciones de moderador/administrador y las eliminaciones de seguridad no caducan de esta manera.
...
Solo para administradores. Reserva slugs raíz y nombres de paquetes para un propietario legítimo sin publicar una
...
Prohíbe a un usuario y elimina permanentemente las skills de su propiedad (solo moderador/administrador).
...
Todavía se admiten para versiones antiguas de la CLI:
