...
- Si no hay disponible una IP de cliente de confianza, las solicitudes anónimas usan buckets de reserva
  delimitados solo por tipo de límite de frecuencia. Estos buckets de reserva no incluyen
...
- Los resultados se devuelven en orden de relevancia (similitud de incrustación + aumentos por coincidencia exacta de token de slug/nombre + un pequeño prior de popularidad).
- La relevancia tiene más peso que la popularidad. Una coincidencia precisa de slug o token de nombre visible puede superar a una coincidencia más laxa con mucha más interacción.
...
- La popularidad se escala logarítmicamente y se limita. Las Skills con alta interacción pueden clasificarse más bajo cuando el texto de la consulta es una coincidencia más débil.
...
- `security.signals.dependencyRegistry` se conserva por compatibilidad de respuesta v1, pero el escáner de existencia del registro de dependencias está retirado y esta clave siempre es `null`.
- La ausencia de Skill Card no afecta a `ok`, `decision` ni `reasons` de este endpoint; los clientes deben leer `skill-card.md` instalado localmente cuando necesiten contenido de la tarjeta.
- Usa `/verify` cuando necesites el sobre de verificación de Skill Card para una sola skill, `/card` cuando necesites Markdown de tarjeta generado, y `/scan` cuando necesites datos detallados del escáner.
...
- Los controles de ordenación de la interfaz del navegador para la búsqueda de plugins reordenan los resultados de relevancia cargados,
  coincidiendo con el comportamiento de exploración actual de `/skills`.
...
- `trust.blockedFromDownload` es la señal de bloqueo de instalación. OpenClaw y otros
  ...
  volver a derivar reglas de bloqueo de los campos del escáner o de moderación.
...
- Este endpoint es intencionalmente más estrecho que los endpoints de moderación de propietarios/moderadores.
  ...
  identidades de denunciantes, cuerpos de reportes, evidencia privada ni cronogramas internos
  de revisión.
...
- Usa de forma predeterminada la versión más reciente.
- Usa el cupo de frecuencia de lectura, no el cupo de descarga.
...
- Los paquetes privados devuelven `404` salvo que el invocador pueda leer el publicador propietario.
...
- Los paquetes privados devuelven `404` salvo que el invocador sea el propietario.
...
Transmite los bytes exactos del tarball de ClawPack subido para clientes del espejo de npm.

Notas:

- Usa el cupo de frecuencia de descarga.
...
- Los cuerpos JSON y los metadatos `payload.files` / `payload.artifact`
  suministrados por el invocador se rechazan.
...
- Los paquetes de Plugin requieren `openclaw.plugin.json`. Las subidas de `.tgz` de ClawPack deben
...
- Los plugins de código requieren `package.json`, metadatos de repositorio fuente, metadatos de commit
  de fuente, metadatos de esquema de configuración, `openclaw.compat.pluginApi` y
  `openclaw.build.openclawVersion`.
- `openclaw.hostTargets` y `openclaw.environment` son metadatos opcionales.
- Solo el publicador de la organización `openclaw` y los publicadores personales de miembros
...
Las eliminaciones reversibles iniciadas por el propietario reservan el slug durante 30 días; luego el slug puede reclamarlo
...
invocador como propietario. Este endpoint no migra identificadores existentes de usuario/personales y no
...
versión. Los nombres de paquetes se convierten en paquetes privados de marcador de posición sin filas de versiones, para que el mismo
...
Las fuentes de skill de GitHub están delimitadas al publicador y se informan como comprobadas en lugar de reescribirse.
...
Bloquea a un usuario y elimina de forma irreversible las skills de su propiedad (solo moderador/admin).
...
Cambia la razón almacenada de un bloqueo existente sin desbloquear ni restaurar
