...
x-i18n:
    generated_at: "2026-07-02T07:54:52Z"
    model: gpt-5.5
...
Los directorios de terceros pueden usar los endpoints públicos de lectura para enumerar o buscar Skills de ClawHub. Almacena los resultados en caché, respeta `429`/`Retry-After`, enlaza a los usuarios con el listado canónico de ClawHub (`https://clawhub.ai/<owner>/skills/<slug>`) y evita insinuar que ClawHub respalda el sitio de terceros. No intentes reflejar contenido oculto, privado o bloqueado por moderación fuera de la superficie pública de la API.

Los accesos directos de slug web se resuelven entre familias de registro, pero los clientes de API deberían usar
...
- Si falta el token o no es válido, el comportamiento recae en la aplicación por IP.
...
- ClawHub usa encabezados de reenvío de confianza para identificar IPs de cliente en el borde.
- Si no hay una IP de cliente de confianza disponible, las solicitudes anónimas usan depósitos alternativos
  acotados solo por tipo de límite de tasa. Estos depósitos alternativos no incluyen
...
- Los resultados se devuelven en orden de relevancia (similitud de embeddings + impulsos por token exacto de slug/nombre + un pequeño previo de popularidad).
- La relevancia pesa más que la popularidad. Una coincidencia precisa de slug o token de nombre visible puede superar a una coincidencia más laxa con una interacción mucho mayor.
- El texto ASCII se tokeniza en límites de palabra y puntuación. Por ejemplo, `personal-map` contiene un token independiente `map`, mientras que `amap-jsapi-skill` contiene `amap`, `jsapi` y `skill`; buscar `map` por tanto da a `personal-map` una coincidencia léxica más fuerte que a `amap-jsapi-skill`.
- La popularidad se escala logarítmicamente y se limita. Los Skills con mucha interacción pueden clasificarse más bajo cuando el texto de consulta coincide peor.
...
- Si un Skill es inesperadamente invisible, comprueba primero el estado de moderación con `clawhub inspect @owner/slug` mientras tengas sesión iniciada antes de cambiar metadatos relacionados con la clasificación.
...
Informa de un Skill para revisión de moderador. Los informes son a nivel de Skill, opcionalmente vinculados
...
Endpoint de moderador/admin para entrada de informes de Skills.
...
Endpoint de moderador/admin para resolver o reabrir informes de Skills.
...
establecer `status` de nuevo en `open`. Pasa `finalAction: "hide"` con un informe triado
...
- Los escaneos publicados escriben cambios de vuelta solo cuando `update: true` y el escaneo se completa correctamente.
...
- Los trabajos de escaneo son asíncronos. Las solicitudes de escaneo manual tienen prioridad sobre el trabajo normal de publicación/rellenado, pero la finalización aún depende de la disponibilidad de los workers.
...
- Requiere acceso de administración del propietario/editor a la skill o Plugin, o autoridad de moderador/administrador de la plataforma.
...
- `kind` toma `skill` como valor predeterminado; usa `kind=plugin` para escaneos de Plugin/paquete.
...
- La identidad de la skill, la identidad del editor y los metadatos de la versión seleccionada son campos de nivel superior del sobre (`slug`, `displayName`, `publisherHandle`, `version`, `resolvedFrom`, `tag`, `createdAt`) para que la automatización de shell pueda leerlos sin desempaquetar envoltorios anidados.
- `security` es el veredicto ClawScan/seguridad de nivel superior. La automatización debe basarse en `ok`, `decision`, `reasons` y `security.status`.
- `security.signals` contiene evidencia de escáner de apoyo, como `staticScan`, `virusTotal` y `skillSpector`.
- `security.signals.dependencyRegistry` se conserva por compatibilidad de respuesta v1, pero el escáner de existencia del registro de dependencias se retiró y esta clave siempre es `null`.
...
- Los resultados son por elemento; una skill o versión ausente no hace fallar toda la respuesta.
...
- `security.signals` contiene solo evidencia de apoyo a nivel de estado; usa `/scan` o la página de auditoría de seguridad de ClawHub para los detalles completos del escáner.
- `security.signals.dependencyRegistry` se conserva por compatibilidad de respuesta v1, pero el escáner de existencia del registro de dependencias se retiró y esta clave siempre es `null`.
- La ausencia de Skill Card no afecta los valores `ok`, `decision` ni `reasons` de este endpoint; los clientes deben leer el `skill-card.md` instalado localmente cuando necesiten el contenido de la tarjeta.
- Usa `/verify` cuando necesites el sobre de verificación de Skill Card de una sola skill, `/card` cuando necesites el Markdown de la tarjeta generada y `/scan` cuando necesites datos detallados del escáner.
...
- `category` (opcional): filtro de categoría de plugin. Admitido solo cuando la
  ...
  los alias heredados de filtro v1 están documentados en `GET /api/v1/plugins`.
...
- Los llamadores autenticados pueden ver paquetes privados de publicadores a los que pertenecen en resultados de lista/búsqueda.
- `channel=private` solo devuelve paquetes que el llamador autenticado puede leer.

### `GET /api/v1/packages/search`

Búsqueda unificada de catálogo entre Skills + paquetes de plugins.
...
- `isOfficial` (opcional): `true` o `false`
- `category` (opcional): filtro de categoría de plugin. Admitido solo cuando la
  solicitud está delimitada a paquetes de plugins. Las categorías controladas y los alias
  heredados de filtro v1 están documentados en `GET /api/v1/plugins`.
...
Los alias heredados de filtro v1 siguen aceptándose en endpoints de lectura:
...
- Las skills alojadas incluyen los archivos de la versión almacenada más reciente y figuran en
...
- El filtrado por categoría es un filtro real de API respaldado por filas de
  resumen de categorías de plugin, no una reescritura de la consulta de búsqueda.
...
- Las versiones de ClawPack incluyen campos compatibles con npm: `npmIntegrity`, `npmShasum` y
  `npmTarballName`.
- `version.sha256hash` es metadato de compatibilidad obsoleto para clientes antiguos. Calcula el
...
- `version.vtAnalysis`, `version.llmAnalysis` y `version.staticScan` se incluyen
  cuando existen datos de escaneo.
- Los paquetes privados devuelven `404` a menos que el llamador pueda leer el publicador propietario.
...
- `trust.scanStatus` es el estado de confianza efectivo derivado de entradas de escáner
  y moderación manual de versiones.
- `trust.moderationState` puede ser nulo. Es `null` cuando no existe moderación manual de versión.
...
- `trust.stale` significa que el resumen de confianza se calculó a partir de entradas obsoletas y
  debe tratarse como si requiriera actualización antes de una decisión de permitir con alta confianza.
...
- Esta es la superficie del resolvedor de OpenClaw; evita adivinar el formato del archivo a partir
...
- Las versiones de ClawPack transmiten los bytes `.tgz` exactos de npm-pack cargados.
...
bloquean descargas automáticamente por sí solos; los moderadores deben usar la moderación de versiones para
...
- `revoked`: bloqueado después de que una versión fuera considerada confiable previamente.
...
- Usa de forma predeterminada la versión más reciente.
- Usa el cupo de tasa de lectura, no el cupo de descarga.
...
- Los archivos de Plugin/paquete son archivos zip con una raíz `package/` para que los clientes antiguos de OpenClaw
  sigan funcionando.
- Esta ruta se mantiene solo para ZIP. No transmite archivos `.tgz` de ClawPack.
...
Transmite los bytes exactos del archivo tar de ClawPack subido para clientes de espejo npm.

Notas:

- Usa el cupo de tasa de descarga.
...
Descarga un ZIP de versión de skill hospedada, o devuelve una transferencia de origen de GitHub para una
...
- Las transferencias de skills respaldadas por GitHub no hacen proxy ni replican bytes. La respuesta JSON
  incluye `sourceRef: "public-github"`, `repo`, `commit`, `path`, `contentHash`
  y `archiveUrl`; el estado actual/de análisis es una compuerta y no se incluye como metadatos de carga útil
  de éxito.
...
- Campo opcional de carga útil: `migrateOwner`. Cuando es `true` con `ownerHandle`, una
  skill existente puede moverse a ese propietario si el actor es administrador/propietario en ambos
  publicadores, el actual y el de destino. Sin esta aceptación explícita, los cambios de propietario se
  rechazan.
...
- Los campos de formulario permitidos son `payload`, blobs `files` repetidos o una referencia de archivo tar
...
- Las solicitudes de publicación multipart directas tienen un límite de 18 MB. Los archivos tar de ClawPack pueden
  usar el flujo upload-url hasta el límite de 120 MB para archivos tar.
...
Las eliminaciones lógicas iniciadas por el propietario reservan el slug durante 30 días; luego el slug puede ser reclamado por
otro publicador. La respuesta de eliminación incluye `slugReservedUntil` cuando aplica esta expiración.
Los ocultamientos de moderador/administrador y las retiradas de seguridad no expiran de esta forma.
...
Creación autenticada de autoservicio de publicador de organización. Crea un nuevo publicador de organización y agrega al
...
sin editar filas de cuentas de Convex Auth. La solicitud debe nombrar ambos ids inmutables de cuenta de proveedor de GitHub;
los identificadores mutables solo se usan como una protección visible para el operador.
...
los alias de slug de Skill, los paquetes, las advertencias del inspector de paquetes y las filas de resumen de búsqueda derivadas para que
...
Banea a un usuario y elimina de forma definitiva las Skills que posee (solo moderador/admin).
...
Desbanea a un usuario y restaura las Skills elegibles (solo admin).
...
Cambia el motivo almacenado de un baneo existente sin desbanear ni restaurar
...
Agrega/quita una estrella (destacados). Ambos endpoints son idempotentes.
...
Si alojas tu propia instancia, sirve este archivo (o define `CLAWHUB_REGISTRY` explícitamente; `CLAWDHUB_REGISTRY` heredado).
