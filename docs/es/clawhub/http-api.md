...
- Los endpoints de escritura autenticados no deben devolver un simple `Unauthorized` cuando
  el servidor conoce el motivo. Los tokens faltantes, los tokens no válidos/revocados y
  las cuentas eliminadas/prohibidas/deshabilitadas deben recibir texto accionable para que los clientes
...
- Estandarizado: `RateLimit-Limit`, `RateLimit-Reset`
...
- Los resultados se devuelven en orden de relevancia (similitud de embeddings + aumentos por coincidencia exacta de token de slug/nombre + un pequeño prior de popularidad).
- La relevancia pesa más que la popularidad. Una coincidencia precisa de slug o token de nombre mostrado puede superar a una coincidencia más imprecisa con una interacción mucho más alta.
...
- La popularidad se escala logarítmicamente y se limita. Skills con alta interacción pueden clasificarse más abajo cuando el texto de consulta tiene una coincidencia más débil.
...
- Los alias de cambio de nombre conservan la resolución para URL antiguas e instalaciones que se resuelven a través del registro, pero el ranking de búsqueda se basa en los metadatos canónicos del Skill después de que el cambio de nombre se haya indexado. Las estadísticas existentes permanecen con el Skill.
- Si un Skill está inesperadamente invisible, comprueba primero el estado de moderación con `clawhub inspect @owner/slug` con sesión iniciada antes de cambiar metadatos relacionados con el ranking.
...
- Las cargas útiles de las solicitudes de escaneo y los informes descargables caducan del almacén de solicitudes de escaneo después del período de retención.
...
- `security.signals.dependencyRegistry` se conserva por compatibilidad con la respuesta v1, pero el escáner de existencia del registro de dependencias está retirado y esta clave siempre es `null`.
- La ausencia de Skill Card no afecta el `ok`, `decision` ni `reasons` de este endpoint; los clientes deberían leer el `skill-card.md` instalado localmente cuando necesiten contenido de la tarjeta.
...
Búsqueda unificada en el catálogo entre Skills y paquetes de plugins.
...
- El filtrado por categoría es un filtro real de API respaldado por filas de
  resumen de categorías de Plugin, no una reescritura de consulta de búsqueda.
...
- Las versiones de ClawPack incluyen los campos compatibles con npm `npmIntegrity`, `npmShasum` y
  `npmTarballName`.
- `version.sha256hash` son metadatos de compatibilidad obsoletos para clientes antiguos. Aplica hash
...
- `trust.moderationState` puede ser nulo. Es `null` cuando no existe
...
- Este endpoint es intencionalmente más limitado que los endpoints de moderación de
  propietario/moderador. Expone la decisión de instalación y la explicación pública, no
...
- Esta es la superficie de resolvedor de OpenClaw; evita inferir el formato de archivo desde
  una URL compartida.
...
- `revoked`: bloqueado después de que una versión se hubiera considerado de confianza anteriormente.
...
- Las transferencias de habilidades respaldadas por GitHub no actúan como proxy ni espejean bytes. La respuesta JSON
...
- Campo de carga útil opcional: `migrateOwner`. Cuando es `true` con `ownerHandle`, una
  habilidad existente puede moverse a ese propietario si el actor es administrador/propietario tanto en los
...
- Las solicitudes de publicación multipart directas están limitadas a 18 MB. Los tarballs ClawPack pueden
...
Cuando está presente, `reason` se almacena como nota de moderación de la habilidad y se copia en el registro de auditoría.
...
los alias de slug de habilidades, los paquetes, las advertencias del inspector de paquetes y las filas derivadas de resumen de búsqueda para que
...
Las fuentes de habilidades de GitHub tienen alcance de publicador y se informan como revisadas en lugar de reescribirse.
...
Bloquea a un usuario y elimina de forma permanente las habilidades de su propiedad (solo moderador/administrador).
...
Si haces autohospedaje, sirve este archivo (o configura `CLAWHUB_REGISTRY` explícitamente; `CLAWDHUB_REGISTRY` heredado).
