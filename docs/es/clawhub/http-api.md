...
read_when:
    ...
    - Depuración de solicitudes de CLI ↔ registro
summary: Referencia de la API HTTP (endpoints públicos + de CLI + autenticación).
x-i18n:
    generated_at: "2026-07-03T02:42:07Z"
    model: gpt-5.5
...
- `X-RateLimit-Remaining` / `RateLimit-Remaining`: presupuesto restante exacto cuando está presente.
  Las solicitudes exitosas particionadas omiten este encabezado en lugar de devolver un valor global aproximado.
...
- Los resultados se devuelven en orden de relevancia (similitud de incrustación + refuerzos por coincidencia exacta de tokens de slug/nombre + un pequeño prior de popularidad).
...
- La popularidad se escala logarítmicamente y se limita. Los Skills con alta interacción pueden clasificarse más abajo cuando el texto de la consulta coincide con menor fuerza.
...
- Si un Skill está inesperadamente invisible, revisa primero el estado de moderación con `clawhub inspect @owner/slug` con sesión iniciada antes de cambiar metadatos relacionados con la clasificación.
...
- `metadata.os`: restricciones de SO declaradas en el frontmatter del Skill (p. ej., `["macos"]`, `["linux"]`). `null` si no se declaró.
- `metadata.systems`: destinos de sistema Nix (p. ej., `["aarch64-darwin", "x86_64-linux"]`). `null` si no se declaró.
...
- Las cargas útiles de solicitudes de análisis y los informes descargables expiran del almacén de solicitudes de análisis después de la ventana de retención.
...
- Los trabajos de análisis son asíncronos. Las solicitudes de análisis manual tienen prioridad sobre el trabajo normal de publicación/relleno, pero la finalización sigue dependiendo de la disponibilidad de trabajadores.
...
- `kind` usa `skill` de forma predeterminada; usa `kind=plugin` para análisis de plugin/paquete.
...
- `security` es el veredicto de seguridad/ClawScan de nivel superior. La automatización debería basarse en `ok`, `decision`, `reasons` y `security.status`.
...
- `security.signals` contiene solo evidencia de apoyo a nivel de estado; usa `/scan` o la página de auditoría de seguridad de ClawHub para ver detalles completos del escáner.
- `security.signals.dependencyRegistry` se conserva por compatibilidad con respuestas v1, pero el escáner de existencia de registro de dependencias está retirado y esta clave siempre es `null`.
...
Exploración de catálogo solo de Plugin entre paquetes code-plugin y bundle-plugin.
...
- El filtrado por categoría es un filtro de API real respaldado por filas de resumen
  de categoría de plugin, no una reescritura de la consulta de búsqueda.
...
- Los controles de ordenación de la interfaz del navegador para la búsqueda de plugins reordenan los resultados de relevancia cargados,
  igual que el comportamiento actual de navegación de `/skills`.
...
- `trust.scanStatus` es el estado de confianza efectivo derivado de las entradas de los escáneres
...
- `trust.reasons` es la lista de explicación para usuarios y auditoría. Los códigos de motivo
  ...
  y `package:malicious`.
- `trust.pending` significa que una o más entradas de confianza todavía esperan finalización.
...
- Este endpoint es intencionalmente más limitado que los endpoints de moderación de propietario/moderador.
  Expone la decisión de instalación y la explicación pública, no las identidades de los denunciantes,
  cuerpos de reportes, evidencia privada ni cronogramas internos de revisión.
...
- Las versiones de paquetes heredados devuelven un artefacto `legacy-zip` y un
  `downloadUrl` ZIP heredado.
- Las versiones de ClawPack devuelven un artefacto `npm-pack`, campos de
  integridad de npm, un `tarballUrl` y la URL de compatibilidad con ZIP heredado.
- Esta es la superficie del resolvedor de OpenClaw; evita inferir el formato del
  archivo desde una URL compartida.
...
- `packageName` se normaliza como nombre npm; el paquete puede faltar en
...
Endpoint de moderador/administrador para colas de revisión de publicaciones de paquetes.
...
- `open`: publicaciones sospechosas, maliciosas, pendientes, en cuarentena, revocadas o reportadas.
- `blocked`: publicaciones en cuarentena, revocadas o maliciosas.
- `manual`: cualquier publicación con una anulación manual de moderación.
- `all`: cualquier publicación con una anulación manual, estado de escaneo no limpio o reporte de paquete.
...
Reporta un paquete para revisión por moderadores. Los reportes son de nivel de paquete,
opcionalmente vinculados a una versión. Alimentan la cola de moderación, pero no ocultan
automáticamente ni bloquean descargas por sí solos; los moderadores deben usar la moderación
de publicaciones para aprobar, poner en cuarentena o revocar artefactos.
...
- Requiere un token de API del propietario del paquete, un miembro publicador,
...
- Las transferencias de Skills respaldadas por GitHub no actúan como proxy ni espejan bytes. La respuesta JSON
...
- Campo de carga útil opcional: `migrateOwner`. Cuando es `true` con `ownerHandle`, una
  ...
  los publicadores actual como destino. Sin esta opción explícita, los cambios de propietario se
  rechazan.
...
- Las publicaciones en nombre de otro usuario aún validan la elegibilidad para el canal oficial contra la cuenta del propietario destino.
...
Las eliminaciones lógicas iniciadas por el propietario reservan el slug durante 30 días; luego, otro publicador
puede reclamar el slug. La respuesta de eliminación incluye `slugReservedUntil` cuando aplica este vencimiento.
...
para el identificador recuperado también se reasigna al usuario de reemplazo para que la sincronización posterior del
perfil no pueda restaurar la autoridad competidora del usuario anterior. Cada tabla principal está limitada a
...
Bloquea a un usuario y elimina de forma permanente las Skills de su propiedad (solo moderador/administrador).
