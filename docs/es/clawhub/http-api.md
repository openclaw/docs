...
x-i18n:
    generated_at: "2026-07-03T09:21:26Z"
    model: gpt-5.5
...
- Los endpoints autenticados de escritura no deberían devolver un simple `Unauthorized` cuando
  el servidor conoce el motivo. Los tokens ausentes, no válidos o revocados, y las cuentas
  eliminadas, bloqueadas o deshabilitadas deberían recibir texto accionable para que los clientes
...
- Si `Retry-After` existe, espera esa cantidad de segundos antes de reintentar.
...
- Si no hay IP de cliente de confianza disponible, las solicitudes anónimas usan buckets de reserva
...
- Los resultados se devuelven por orden de relevancia (similitud de embeddings + refuerzos por tokens exactos de slug/nombre + un pequeño prior de popularidad).
- La relevancia pesa más que la popularidad. Una coincidencia precisa de slug o de token del nombre mostrado puede superar a una coincidencia más laxa con mucha más interacción.
...
- La popularidad se escala logarítmicamente y se limita. Los Skills con alta interacción pueden clasificarse más abajo cuando el texto de consulta coincide peor.
- El estado de moderación sospechoso u oculto puede retirar una skill de la búsqueda pública según los filtros del llamador y el estado de moderación actual.
...
- Los alias de cambio de nombre preservan la resolución para URL antiguas e instalaciones que se resuelven mediante el registro, pero la clasificación de búsqueda se basa en los metadatos canónicos de la skill después de que se indexe el cambio de nombre. Las estadísticas existentes permanecen con la skill.
...
establecer `status` de vuelta a `open`. Pasa `finalAction: "hide"` con un reporte clasificado
...
- Los escaneos publicados requieren acceso de administración de propietario/publicador, o autoridad de moderador/administrador de la plataforma.
...
- Requiere acceso de administración de propietario/publicador a la skill o Plugin, o autoridad de moderador/administrador de la plataforma.
...
- `ok` es `true` solo cuando la versión seleccionada tiene una Skill Card generada, no está bloqueada como malware por la moderación y la verificación de ClawScan está limpia.
...
- Las Skills actuales respaldadas por GitHub con un escaneo `clean` o `suspicious` incluyen
  ...
  hash de contenido y URL del archivo. No incluyen archivos de origen alojados en ClawHub.
...
- También se aceptan los alias de filtros v1 heredados documentados en `GET /api/v1/plugins`.
...
- Las versiones de ClawPack incluyen los campos compatibles con npm `npmIntegrity`, `npmShasum` y
  `npmTarballName`.
- `version.sha256hash` es metadato de compatibilidad obsoleto para clientes antiguos. Calcula el hash
...
- `trust.blockedFromDownload` es la señal de bloqueo de instalación. OpenClaw y otros
  clientes de instalación deben bloquear la instalación cuando este valor sea `true` en vez de
...
- `trust.reasons` es la lista de explicaciones para usuarios y auditoría. Los códigos de motivo
  ...
  y `package:malicious`.
- `trust.pending` significa que una o más entradas de confianza aún esperan finalización.
...
- Esta es la superficie de resolución de OpenClaw; evita inferir el formato del archivo a partir
...
Denuncia un paquete para revisión de moderadores. Los informes son a nivel de paquete, opcionalmente
...
Punto de conexión de moderador/admin para la recepción de informes de paquetes.
...
Punto de conexión de moderador/admin para resolver o reabrir informes de paquetes.
...
Punto de conexión de moderador/admin para revisión de versiones de paquetes.
...
- `revoked`: bloqueado después de que una versión fuera previamente confiable.
...
- `dist.tarball`, `dist.integrity` y `dist.shasum` usan campos compatibles con npm
  para que los usuarios puedan apuntar npm al espejo si así lo deciden.
...
Descarga un ZIP de versión de skill alojada, o devuelve una transferencia a fuente GitHub para una
...
- Campo opcional de carga útil: `migrateOwner`. Cuando es `true` con `ownerHandle`, una
  skill existente puede moverse a ese propietario si el actor es admin/propietario tanto en el
...
Los ocultamientos por moderador/admin y las eliminaciones de seguridad no vencen de esta manera.
...
usuario compartido heredado/publicador personal, el punto de conexión primero lo migra a un publicador de organización.
...
Creación autenticada de publicador de organización autoservicio. Crea un nuevo publicador de organización y agrega al
...
versión. Los nombres de paquetes se convierten en paquetes privados de marcador de posición sin filas de versión, por lo que el mismo
propietario puede publicar más adelante la versión real de code-plugin o bundle-plugin en ese nombre.
...
de proveedor GitHub; los identificadores mutables solo se usan como una protección orientada al operador.
...
los alias de slug de habilidades, los paquetes, las advertencias del inspector de paquetes y las filas derivadas del resumen de búsqueda para que
...
Las fuentes de habilidades de GitHub tienen alcance de publicador y se informan como comprobadas en lugar de reescribirse.
...
Banea a un usuario y elimina permanentemente las habilidades de su propiedad (solo moderador/administrador).
...
contenido (solo administrador). Usa dry-run de forma predeterminada a menos que `dryRun` sea `false`.
...
Si haces self-hosting, sirve este archivo (o establece `CLAWHUB_REGISTRY` explícitamente; heredado `CLAWDHUB_REGISTRY`).
