...
- `Retry-After`: segundos que se deben esperar antes de reintentar (retraso) en `429`
...
- Use espera exponencial con variación aleatoria para evitar reintentos sincronizados.
...
- Si no hay disponible una IP de cliente de confianza, las solicitudes anónimas usan grupos alternativos
  limitados únicamente por el tipo de límite de frecuencia. Estos grupos alternativos no incluyen
  rutas proporcionadas por el llamador, slugs, nombres de paquetes, versiones, cadenas de consulta ni otros
  parámetros de artefactos.
...
- Los resultados se devuelven en orden de relevancia (similitud de incrustaciones + refuerzos por coincidencia exacta de tokens del slug/nombre + una pequeña ponderación previa de popularidad).
- La relevancia tiene más peso que la popularidad. Una coincidencia precisa con un slug o un token del nombre para mostrar puede superar una coincidencia menos precisa con una interacción mucho mayor.
- El texto ASCII se tokeniza según los límites de palabras y puntuación. Por ejemplo, `personal-map` contiene un token `map` independiente, mientras que `amap-jsapi-skill` contiene `amap`, `jsapi` y `skill`; por lo tanto, buscar `map` otorga a `personal-map` una coincidencia léxica más fuerte que a `amap-jsapi-skill`.
- La popularidad se escala logarítmicamente y tiene un límite máximo. Las Skills con mucha interacción pueden obtener una posición inferior cuando el texto de la consulta coincide de forma menos precisa.
- Un estado de moderación sospechoso u oculto puede eliminar una Skill de la búsqueda pública en función de los filtros del llamador y del estado actual de moderación.

Orientación sobre la visibilidad para los editores:

- Incluya los términos que los usuarios buscarán literalmente en el nombre para mostrar, el resumen y las etiquetas. Use un token independiente en el slug solo cuando también sea una identidad estable que desee conservar.
- No cambie el nombre de un slug únicamente para ajustarse a una consulta, salvo que el nuevo slug sea un nombre canónico mejor a largo plazo. Los slugs antiguos se convierten en alias de redirección, pero la URL canónica, el slug mostrado y los futuros resúmenes de búsqueda usan el nuevo slug.
...
- `trending` clasifica por instalaciones durante los últimos 7 días (basándose en telemetría).
...
- Use `nextCursor` para continuar la paginación cuando esté presente. Una página corta no significa por sí sola que se hayan agotado los resultados.
...
- `moderation` se incluye solo cuando la Skill está marcada o cuando la consulta su propietario.
...
- Los llamadores públicos solo obtienen `200` para las Skills visibles que ya están marcadas.
- Las pruebas se ocultan para los llamadores públicos y solo incluyen fragmentos sin procesar para propietarios y moderadores.

### `POST /api/v1/skills/{slug}/report`

Denuncia una Skill para que la revisen los moderadores. Las denuncias corresponden a la Skill completa, pueden vincularse
opcionalmente a una versión y alimentan la cola de denuncias de Skills.
...
Endpoint de moderador/administrador para recibir denuncias de Skills.
...
Endpoint de moderador/administrador para resolver o reabrir denuncias de Skills.
...
- `moderation` es una instantánea actual de la moderación a nivel de skill derivada de la versión más reciente.
...
- Los análisis publicados requieren acceso de gestión de propietario o publicador, o autoridad de moderador o administrador de la plataforma.
- Los análisis publicados solo vuelven a escribir los datos cuando `update: true` y el análisis finaliza correctamente.
...
- Los trabajos de análisis son asíncronos. Las solicitudes de análisis manual tienen prioridad sobre el trabajo normal de publicación o procesamiento de elementos pendientes, pero su finalización sigue dependiendo de la disponibilidad de los procesos de trabajo.

### `GET /api/v1/skills/-/scan/{scanId}`

Endpoint de consulta autenticado para un análisis enviado.

- Devuelve el estado en cola/en ejecución/completado/con errores.
- Devuelve `queue.queuedAhead` y `queue.position` mientras está en cola para que los clientes puedan mostrar cuántos análisis manuales prioritarios hay por delante de la solicitud. Las colas muy grandes se limitan y se notifican con `queuedAheadIsEstimate: true`.
- Cuando está disponible, `report` contiene las secciones `clawscan`, `skillspector`, `staticAnalysis` y `virustotal`.
- Los trabajos de análisis con errores devuelven `status: "failed"` con `lastError`.

### `GET /api/v1/skills/-/scan/{scanId}/download`

Endpoint autenticado para el archivo de informes.

- Requiere que el análisis se haya completado correctamente; los análisis que no estén en un estado terminal devuelven `409`.
...
- Requiere acceso de gestión como propietario/editor de la skill o del plugin, o autoridad de moderador/administrador de la plataforma.
- Devuelve los resultados de análisis almacenados de la versión exacta enviada, incluidas las versiones bloqueadas u ocultas.
- El valor predeterminado de `kind` es `skill`; use `kind=plugin` para los análisis de plugins/paquetes.
- Devuelve la misma estructura ZIP que las descargas de solicitudes de análisis.

### `POST /api/v1/skills/-/scan/batch`

Ruta canónica de reescaneo por lotes exclusiva para administradores. Acepta la misma estructura de carga útil que la ruta heredada `POST /api/v1/skills/-/rescan-batch`.
...
- `ok` es `true` únicamente cuando la versión seleccionada tiene una tarjeta de Skill generada, no está bloqueada por moderación debido a software malicioso y la verificación de ClawScan no detecta problemas.
- La identidad de la Skill, la identidad del publicador y los metadatos de la versión seleccionada son campos de nivel superior del contenedor (`slug`, `displayName`, `publisherHandle`, `version`, `resolvedFrom`, `tag`, `createdAt`), de modo que la automatización de shell pueda leerlos sin desempaquetar contenedores anidados.
...
- `security.signals` contiene evidencia complementaria de los analizadores, como `staticScan`, `virusTotal` y `skillSpector`.
- `security.signals.dependencyRegistry` se conserva por compatibilidad con la respuesta v1, pero el analizador de existencia en el registro de dependencias se ha retirado y esta clave siempre es `null`.
- `provenance` es `server-resolved-github-import` únicamente cuando ClawHub resolvió y almacenó un repositorio, una referencia, una confirmación y una ruta de GitHub durante la publicación o importación; de lo contrario, es `unavailable`.
...
de ClawHub instaladas necesitan mostrar, como la interfaz de control de OpenClaw.
...
- Los resultados se proporcionan por elemento; la ausencia de una Skill o versión no provoca el fallo de toda la respuesta.
- La respuesta contiene únicamente información de seguridad. No incluye datos de la tarjeta de Skill, el estado de generación de la tarjeta, listas de archivos del artefacto ni cargas útiles detalladas de los analizadores.
- `security.signals` contiene únicamente evidencia complementaria a nivel de estado; utilice `/scan` o la página de auditoría de seguridad de ClawHub para consultar todos los detalles de los analizadores.
- `security.signals.dependencyRegistry` se conserva por compatibilidad con la respuesta v1, pero el analizador de existencia en el registro de dependencias se ha retirado y esta clave siempre es `null`.
- La ausencia de una tarjeta de Skill no afecta a `ok`, `decision` ni `reasons` de este endpoint; los clientes deben leer localmente la tarjeta `skill-card.md` instalada cuando necesiten su contenido.
- Utilice `/verify` cuando necesite el contenedor de verificación de la tarjeta de Skill de una sola Skill, `/card` cuando necesite el Markdown de la tarjeta generada y `/scan` cuando necesite datos detallados de los analizadores.
...
- Las Skills actuales respaldadas por GitHub con un análisis `clean` o `suspicious` incluyen
  ...
  el hash del contenido y la URL del archivo. No incluyen archivos de origen alojados en ClawHub.
...
- Los controles de ordenación de la interfaz del navegador para la búsqueda de plugins reordenan los resultados de relevancia cargados,
  de acuerdo con el comportamiento actual de exploración de `/skills`.
...
- Los paquetes privados devuelven `404`, salvo que el solicitante pueda leer al publicador propietario.
...
- Requiere un token de API del propietario del paquete, de un propietario o administrador de la organización publicadora,
...
- `version.artifact.kind` es `legacy-zip` para los archivos de paquetes del sistema antiguo o
...
- `trust.moderationState` admite valores nulos. Es `null` cuando no existe ninguna
...
- `trust.reasons` es la lista de explicaciones para el usuario y la auditoría. Los códigos de motivo
...
- Este endpoint es específico de la versión. Los clientes deben llamarlo después de resolver la
  ...
  del paquete.
- Los paquetes privados devuelven `404`, salvo que el solicitante pueda leer al publicador propietario.
- Este endpoint es intencionadamente más limitado que los endpoints de moderación para
  propietarios o moderadores. Expone la decisión de instalación y la explicación pública, no
...
- Las versiones de ClawPack devuelven un artefacto `npm-pack`, campos de integridad de npm, una
  `tarballUrl` y la URL de compatibilidad con el ZIP heredado.
...
- Utiliza el grupo de límites de descarga.
...
Endpoint para que los moderadores enumeren las filas de migración de plugins oficiales de OpenClaw.
...
Endpoint para que los administradores creen o actualicen una fila de migración de un plugin oficial.
...
```json
{
  ...
  "moderationApproved": false,
  "runtimeBundlesReady": false,
  "notes": "a la espera de que el publicador cargue el archivo"
}
```
...
- `packageName` se normaliza como nombre de npm; el paquete puede no existir para migraciones
  planificadas.
...
Endpoint para las colas de revisión de versiones de paquetes por moderadores y administradores.
...
Endpoint para propietarios/moderadores destinado a consultar el estado de moderación de un paquete.

Autenticación:

- Requiere un token de API para el propietario del paquete, un miembro del publicador, un moderador o un
...
- `revoked`: bloqueada después de que anteriormente se hubiera confiado en la versión.

Las versiones en cuarentena y revocadas devuelven `403` desde las rutas de descarga de artefactos.
Cada cambio genera una entrada en el registro de auditoría.
...
- Límite de tamaño del archivo: 200KB.
- Los análisis pendientes de VirusTotal no bloquean las lecturas; es posible que las versiones maliciosas sigan reteniéndose en otros lugares.
...
- Las versiones heredadas que solo disponen de ZIP se omiten intencionadamente.
- `dist.tarball`, `dist.integrity` y `dist.shasum` usan campos compatibles con npm
  para que los usuarios puedan configurar npm para usar el espejo si así lo desean.
...
- Usa el depósito de tasa de descargas.
...
- Las transferencias de skills respaldadas por GitHub no actúan como proxy ni replican bytes. La respuesta JSON
  ...
  y `archiveUrl`; el análisis/estado actual funciona como condición de acceso y no se incluye como metadatos
...
- Los campos de formulario permitidos son `payload`, blobs `files` repetidos o una referencia a un
  archivo tar `clawpack`. `clawpack` puede ser un blob `.tgz` o un identificador de almacenamiento
  devuelto por el flujo de URL de carga. Las publicaciones preparadas mediante un identificador de
  almacenamiento también deben incluir el `clawpackUploadTicket` devuelto con esa URL de carga.
...
- Las solicitudes directas de publicación multipart tienen un límite de 18MB. Los archivos tar
  ClawPack pueden usar el flujo de URL de carga hasta el límite de 120MB por archivo tar.
...
- Solo el publicador de la organización `openclaw` y los publicadores personales de los miembros
  actuales de la organización `openclaw` pueden publicar en el canal `official`.
...
Elimine de forma reversible o restaure una skill (propietario, moderador o administrador).
...
Solo para administradores. Garantiza que exista un publicador de organización para un identificador. Si el identificador todavía apunta a un usuario compartido heredado/publicador personal, el endpoint lo migra primero a un publicador de organización.
...
Creación autenticada de publicadores de organización mediante autoservicio. Crea un nuevo publicador de organización y añade al solicitante como propietario. Este endpoint no migra identificadores de usuario/personales existentes ni marca al publicador como de confianza/oficial.
...
Solo para administradores. Reserva slugs raíz y nombres de paquetes para su propietario legítimo sin publicar una versión. Los nombres de paquetes se convierten en paquetes marcadores privados sin filas de versiones, para que el mismo propietario pueda publicar posteriormente en ese nombre la versión real del Plugin de código o del Plugin de paquete.
...
Solo para administradores. Recupera un publicador personal para una entidad principal de OAuth de GitHub de reemplazo verificada sin editar las filas de cuentas de Convex Auth. La solicitud debe indicar ambos identificadores inmutables de cuenta del proveedor de GitHub; los identificadores mutables solo se usan como comprobación para el operador.

El endpoint usa el modo de ejecución de prueba de forma predeterminada. Para aplicar la recuperación, se requieren `dryRun: false` y
`confirmIdentityVerified: true` después de que el personal verifique de forma independiente la continuidad entre ambas
identidades principales de GitHub. La recuperación se cierra de forma segura si el publicador personal actual
...
los alias de slugs de Skills, los paquetes, las advertencias del inspector de paquetes y las filas derivadas del resumen de búsqueda, de modo que
las rutas de propietario directo concuerden con la nueva autoridad del publicador. También se reasigna al usuario sustituto
una reserva activa del identificador protegido correspondiente al identificador recuperado, para que una sincronización posterior
...
Las fuentes de Skills de GitHub están asociadas al publicador y se indican como comprobadas en lugar de reescribirse.
...
el contenido (solo administradores). Usa el modo de ejecución de prueba de forma predeterminada, salvo que `dryRun` sea `false`.
...
Si se aloja en infraestructura propia, sirva este archivo (o establezca `CLAWHUB_REGISTRY` explícitamente; `CLAWDHUB_REGISTRY` es heredado).
