...
x-i18n:
    generated_at: "2026-07-01T15:18:53Z"
    model: gpt-5.5
...
- `X-RateLimit-Reset`: segundos absolutos de la época Unix
...
- Los resultados se devuelven en orden de relevancia (similitud de embeddings + refuerzos de tokens exactos de slug/nombre + un pequeño prior de popularidad).
...
- La popularidad se escala logarítmicamente y se limita. Las Skills con alta interacción pueden clasificarse más abajo cuando el texto de la consulta tiene una coincidencia más débil.
- Un estado de moderación sospechoso u oculto puede eliminar una Skill de la búsqueda pública según los filtros del llamador y el estado de moderación actual.
...
- Si una Skill está inesperadamente invisible, revisa primero el estado de moderación con `clawhub inspect @owner/slug` mientras tienes la sesión iniciada antes de cambiar metadatos relacionados con la clasificación.
...
- `metadata.os`: restricciones de sistema operativo declaradas en el frontmatter de la Skill (por ejemplo, `["macos"]`, `["linux"]`). `null` si no se declara.
...
- Los trabajos de escaneo son asíncronos. Las solicitudes de escaneo manual tienen prioridad sobre el trabajo normal de publicación/rellenado, pero su finalización aún depende de la disponibilidad de workers.
...
- `security.signals` contiene solo evidencia de apoyo a nivel de estado; usa `/scan` o la página de auditoría de seguridad de ClawHub para ver todos los detalles del escáner.
- `security.signals.dependencyRegistry` se conserva por compatibilidad con la respuesta v1, pero el escáner de existencia del registro de dependencias está retirado y esta clave siempre es `null`.
...
Búsqueda de catálogo unificada entre Skills y paquetes de plugin.
...
- `trust.blockedFromDownload` es la señal de bloqueo de instalación. OpenClaw y otros
  ...
  volver a derivar las reglas de bloqueo a partir de los campos de escáner o moderación.
- `trust.reasons` es la lista de explicación de cara al usuario y de auditoría. Los códigos de motivo
...
- Usa el segmento de límite de descargas.
...
`finalAction: "revoke"` con un informe confirmado para aplicar moderación de versiones en el
...
- Usa de forma predeterminada la última versión.
...
- Los análisis de VirusTotal pendientes no bloquean las lecturas; las versiones maliciosas aún pueden retenerse en otra parte.
...
- Si no se proporciona ni `version` ni `tag`, se usa la última versión.
...
- Campo de carga útil opcional: `ownerHandle`. Cuando está presente, la API resuelve ese
  editor del lado del servidor y exige que el actor tenga acceso al editor.
- Campo de carga útil opcional: `migrateOwner`. Cuando es `true` con `ownerHandle`, una
  ...
  los editores actual como de destino. Sin esta adhesión explícita, los cambios de propietario se
  rechazan.
...
- Los cuerpos JSON y los metadatos `payload.files` / `payload.artifact` suministrados por el llamador
  se rechazan.
...
- Los code plugins requieren `package.json`, metadatos de repositorio fuente, metadatos de commit fuente,
...
- Las publicaciones en nombre de otro usuario aún validan la elegibilidad del canal oficial contra la cuenta del propietario de destino.
...
otro editor. La respuesta de eliminación incluye `slugReservedUntil` cuando se aplica esta caducidad.
...
usuario/editor personal compartido heredado, el punto de conexión lo migra primero a un editor de organización.
...
Creación autenticada de editor de organización en autoservicio. Crea un nuevo editor de organización y añade al
...
Solo administrador. Recupera un editor personal para un principal de GitHub OAuth de reemplazo verificado
...
El endpoint usa modo de simulación de forma predeterminada. Aplicar la recuperación requiere `dryRun: false` y
...
Las fuentes de skill de GitHub tienen alcance de publicador y se informan como comprobadas en lugar de reescribirse.
...
contenido (solo administrador). Usa modo de simulación de forma predeterminada salvo que `dryRun` sea `false`.
...
Si autoalojas, sirve este archivo (o configura `CLAWHUB_REGISTRY` explícitamente; `CLAWDHUB_REGISTRY` heredado).
