...
summary: Referencia de API HTTP (pública + endpoints de CLI + autenticación).
x-i18n:
...
- La popularidad se escala logarítmicamente y tiene un límite. Los Skills con alta interacción pueden posicionarse más abajo cuando el texto de la consulta coincide peor.
- Un estado de moderación sospechoso u oculto puede eliminar un Skill de la búsqueda pública según los filtros del llamador y el estado de moderación actual.
...
- No cambies el nombre de un slug solo para perseguir una consulta, a menos que el nuevo slug sea un mejor nombre canónico a largo plazo. Los slugs antiguos se convierten en alias de redirección, pero la URL canónica, el slug mostrado y los futuros resúmenes de búsqueda usan el slug nuevo.
- Los alias de cambio de nombre conservan la resolución para URL antiguas e instalaciones que se resuelven mediante el registro, pero el ranking de búsqueda se basa en los metadatos canónicos del Skill después de que el cambio de nombre se haya indexado. Las estadísticas existentes permanecen con el Skill.
- Si un Skill es inesperadamente invisible, revisa primero el estado de moderación con `clawhub inspect @owner/slug` mientras tienes la sesión iniciada antes de cambiar metadatos relacionados con el ranking.
...
- `createdAt` es estable para rastreos de Skills nuevos; `updated` cambia cuando los Skills existentes se republican.
...
- Usa `nextCursor` para continuar la paginación cuando esté presente. Una página corta no significa por sí sola que se hayan agotado los resultados.
...
- Los llamadores públicos solo reciben `200` para Skills visibles que ya están marcados.
...
Reporta un Skill para revisión de moderación. Los reportes son a nivel de Skill, opcionalmente vinculados
...
- Las cargas útiles de solicitudes de análisis y los informes descargables caducan en el almacén de solicitudes de análisis después de la ventana de retención.
- Los análisis publicados requieren acceso de administración del propietario/publicador, o autoridad de moderador/administrador de la plataforma.
- Los análisis publicados escriben los resultados de vuelta solo cuando `update: true` y el análisis se completa correctamente.
...
- Los trabajos de análisis son asíncronos. Las solicitudes de análisis manual se priorizan por delante del trabajo normal de publicación/rellenado, pero su finalización aún depende de la disponibilidad de los workers.
...
- Devuelve resultados de análisis almacenados para la versión exacta enviada, incluidas las versiones bloqueadas u ocultas.
- `kind` tiene como valor predeterminado `skill`; usa `kind=plugin` para análisis de plugins/paquetes.
...
- `security.signals.dependencyRegistry` se conserva por compatibilidad de respuesta v1, pero el escáner de existencia del registro de dependencias está retirado y esta clave siempre es `null`.
- La ausencia de Skill Card no afecta `ok`, `decision` ni `reasons` de este endpoint; los clientes deben leer el `skill-card.md` instalado localmente cuando necesiten el contenido de la tarjeta.
...
- `category` (opcional): filtro de categoría de plugin. Solo se admite cuando la
  ...
  alias heredados de filtro v1 están documentados en `GET /api/v1/plugins`.
...
- Los llamadores autenticados pueden ver paquetes privados para los publicadores a los que pertenecen en los resultados de lista/búsqueda.
- `channel=private` solo devuelve paquetes que el llamador autenticado puede leer.

### `GET /api/v1/packages/search`

Búsqueda de catálogo unificada en Skills + paquetes de plugins.
...
- `isOfficial` (opcional): `true` o `false`
- `category` (opcional): filtro de categoría de plugin. Solo se admite cuando la
  solicitud está acotada a paquetes de plugins. Las categorías controladas y los alias heredados de filtro v1
  están documentados en `GET /api/v1/plugins`.
...
- Los llamadores anónimos solo ven canales de paquetes públicos.
- Los llamadores autenticados pueden buscar paquetes privados para los publicadores a los que pertenecen.
- `channel=private` solo devuelve paquetes que el llamador autenticado puede leer.
...
- `_errors.json` se incluye cuando no se pudieron exportar
  Skills o archivos individuales.
...
- `cursor` (opcional): cursor de paginación de la respuesta anterior.
- `family` (opcional): `code-plugin` o `bundle-plugin`. Omitido significa ambas
  familias de Plugins.
...
- También se aceptan los alias de filtros legados de v1 documentados en `GET /api/v1/plugins`.
...
- Los controles de ordenación de la IU del navegador para la búsqueda de Plugins reordenan
  los resultados de relevancia cargados, coincidiendo con el comportamiento actual de navegación de `/skills`.
...
- Requiere un token de API para el propietario del paquete, un propietario/administrador
  del publicador de la organización, moderador de la plataforma o administrador de la plataforma.
...
verificación, metadatos de artefacto y datos de escaneo.
...
- Las versiones de ClawPack incluyen campos compatibles con npm `npmIntegrity`, `npmShasum` y
  `npmTarballName`.
- `version.sha256hash` son metadatos de compatibilidad obsoletos para clientes antiguos. Hashea
  los bytes ZIP exactos devueltos por `/api/v1/packages/{name}/download`.
...
- `trust.stale` significa que el resumen de confianza se calculó con entradas desactualizadas y
  debe tratarse como que requiere actualización antes de una decisión de autorización de alta confianza.

Notas:

- Este punto de conexión es exacto por versión. Los clientes deben llamarlo después de resolver la
  versión del paquete que pretenden instalar, no solo después de leer los metadatos más recientes
  del paquete.
- Los paquetes privados devuelven `404` a menos que el llamador pueda leer el publicador propietario.
- Este punto de conexión es intencionalmente más estrecho que los puntos de conexión de moderación
  de propietario/moderador. Expone la decisión de instalación y la explicación pública, no
  identidades de denunciantes, cuerpos de reportes, evidencia privada ni cronologías internas
  de revisión.
...
- Las versiones de paquete legadas devuelven un artefacto `legacy-zip` y una URL de descarga ZIP legada
  `downloadUrl`.
...
- Esta es la superficie de resolución de OpenClaw; evita adivinar el formato de archivo desde
  una URL compartida.
...
Punto de conexión de moderador/administrador para revisión de versiones de paquetes.
...
- `revoked`: bloqueada después de que una versión fuera considerada confiable anteriormente.
...
- Usa de forma predeterminada la versión más reciente.
- Usa el depósito de límites de lectura, no el depósito de descargas.
...
- Usa el depósito de límites de descarga.
...
Descarga un ZIP de versión de Skill alojado, o devuelve una transferencia de origen de GitHub para una
...
- Las transferencias de Skills respaldadas por GitHub no hacen proxy ni espejan bytes. La respuesta JSON
  ...
  y `archiveUrl`; el análisis/estado actual es una barrera y no se incluye como metadatos
...
- Campo opcional de carga útil: `migrateOwner`. Cuando es `true` con `ownerHandle`, una
  ...
  publicadores actuales como de destino. Sin esta aceptación explícita, los cambios de propietario se
  rechazan.
...
- Las publicaciones en nombre de terceros siguen validando la elegibilidad del canal oficial contra la cuenta del propietario de destino.
...
Las eliminaciones reversibles iniciadas por el propietario reservan el slug durante 30 días; luego otro publicador
puede reclamar el slug. La respuesta de eliminación incluye `slugReservedUntil` cuando se aplica esta expiración.
...
Creación autoservicio autenticada de publicadores de organización. Crea un nuevo publicador de organización y agrega al
...
versión. Los nombres de paquetes se convierten en paquetes marcadores privados sin filas de versión, por lo que el mismo
propietario puede publicar más tarde la versión real code-plugin o bundle-plugin con ese nombre.
...
Solo administrador. Recupera un publicador personal para un principal de OAuth de GitHub de reemplazo verificado
...
Todavía son compatibles con versiones anteriores de la CLI:
