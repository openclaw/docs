...
summary: Referencia de la API HTTP (endpoints públicos + CLI + autenticación).
x-i18n:
    generated_at: "2026-06-30T22:04:48Z"
    model: gpt-5.5
...
- Los endpoints de escritura autenticados no deberían devolver un simple `Unauthorized` cuando
  el servidor conoce el motivo. Los tokens ausentes, no válidos/revocados y las cuentas
  eliminadas/baneadas/deshabilitadas deberían recibir texto accionable para que los clientes
...
- `X-RateLimit-Remaining` / `RateLimit-Remaining`: presupuesto restante exacto cuando está presente.
  Las solicitudes correctas fragmentadas omiten esta cabecera en lugar de devolver un valor global aproximado.
...
- ClawHub usa cabeceras de reenvío de confianza para identificar IPs de clientes en el borde.
- Si no hay ninguna IP de cliente de confianza disponible, las solicitudes anónimas usan buckets de reserva
  delimitados solo por el tipo de límite de frecuencia. Estos buckets de reserva no incluyen
  rutas, slugs, nombres de paquetes, versiones, cadenas de consulta u otros
...
- Los resultados se devuelven por orden de relevancia (similitud de embeddings + refuerzos por coincidencia exacta de tokens de slug/nombre + un pequeño prior de popularidad).
- La relevancia es más fuerte que la popularidad. Una coincidencia precisa de slug o de token de nombre visible puede superar a una coincidencia más laxa con mucha más interacción.
...
- La popularidad se escala logarítmicamente y se limita. Las Skills con mucha interacción pueden clasificarse más abajo cuando el texto de consulta coincide peor.
...
- Los alias de cambio de nombre preservan la resolución de URL antiguas e instalaciones que se resuelven mediante el registro, pero la clasificación de búsqueda se basa en los metadatos canónicos de la Skill después de que el cambio de nombre se haya indexado. Las estadísticas existentes permanecen con la Skill.
- Si una Skill es inesperadamente invisible, comprueba primero el estado de moderación con `clawhub inspect @owner/slug` mientras tienes la sesión iniciada antes de cambiar metadatos relacionados con la clasificación.
...
- `metadata.os`: restricciones de sistema operativo declaradas en el frontmatter de la Skill (por ejemplo, `["macos"]`, `["linux"]`). `null` si no se declararon.
...
- `moderation` se incluye solo cuando la Skill está marcada o la está viendo el propietario.
...
Informa sobre una Skill para revisión por moderadores. Los informes son a nivel de Skill, opcionalmente vinculados
...
establecer `status` de nuevo en `open`. Pasa `finalAction: "hide"` con un informe
...
Devuelve detalles de verificación de análisis de seguridad para una versión de Skill.
...
Los análisis de carga local ya no son compatibles. Las solicitudes que usan
...
- Las cargas útiles de solicitud de análisis y los informes descargables caducan en el almacén de solicitudes de análisis después del periodo de retención.
- Los análisis publicados requieren acceso de administración de propietario/publicador, o autoridad de moderador/administrador de la plataforma.
...
- Los trabajos de análisis son asíncronos. Las solicitudes de análisis manuales se priorizan por delante del trabajo normal de publicación/rellenado, pero la finalización sigue dependiendo de la disponibilidad de workers.

### `GET /api/v1/skills/-/scan/{scanId}`

Endpoint de sondeo autenticado para un análisis enviado.
...
- Requiere acceso de administración de propietario/publicador al skill o Plugin, o autoridad de moderador/administrador de la plataforma.
- Devuelve resultados de análisis almacenados para la versión exacta enviada, incluidas versiones bloqueadas u ocultas.
- `kind` tiene `skill` como valor predeterminado; usa `kind=plugin` para análisis de plugin/paquete.
- Devuelve la misma forma ZIP que las descargas de solicitudes de análisis.
...
- La identidad del skill, la identidad del publicador y los metadatos de la versión seleccionada son campos de sobre de nivel superior (`slug`, `displayName`, `publisherHandle`, `version`, `resolvedFrom`, `tag`, `createdAt`) para que la automatización de shell pueda leerlos sin desempaquetar envoltorios anidados.
...
- Los resultados son por elemento; que falte un skill o una versión no hace fallar toda la respuesta.
...
- `security.signals` contiene solo evidencia de apoyo a nivel de estado; usa `/scan` o la página de auditoría de seguridad de ClawHub para obtener detalles completos de los escáneres.
- `security.signals.dependencyRegistry` se conserva por compatibilidad de respuesta v1, pero el escáner de existencia del registro de dependencias está retirado y esta clave siempre es `null`.
- La ausencia de Skill Card no afecta a `ok`, `decision` ni `reasons` de este endpoint; los clientes deberían leer `skill-card.md` instalado localmente cuando necesiten el contenido de la tarjeta.
- Usa `/verify` cuando necesites el sobre de verificación de Skill Card de un solo skill, `/card` cuando necesites Markdown de tarjeta generada y `/scan` cuando necesites datos detallados de escáner.
...
- `POST /api/v1/packages` sigue siendo solo para lanzamientos de Plugins de código y Plugins de paquete.
- Los llamadores anónimos solo ven canales de paquetes públicos.
- Los llamadores autenticados pueden ver paquetes privados de los editores a los que pertenecen en los resultados de lista/búsqueda.
...
Exploración de catálogo solo de Plugins en paquetes de Plugin de código y Plugin de paquete.
...
- `_errors.json` se incluye cuando no se pudieron exportar Skills o archivos individuales.
...
- `trust.stale` significa que el resumen de confianza se calculó a partir de entradas obsoletas y
  debe tratarse como si requiriera actualización antes de una decisión de autorización de alta confianza.
...
- Este endpoint es intencionalmente más limitado que los endpoints de moderación de propietarios/moderadores.
  ...
  identidades de reporteros, cuerpos de reportes, evidencia privada ni cronogramas internos de revisión.

### `GET /api/v1/packages/{name}/versions/{version}/artifact`

Devuelve los metadatos explícitos del resolver de artefactos para una versión de paquete.
...
- Esta es la superficie de resolver de OpenClaw; evita adivinar el formato de archivo a partir
  de una URL compartida.

### `GET /api/v1/packages/{name}/versions/{version}/artifact/download`

Descarga el artefacto de la versión mediante la ruta explícita del resolver.
...
Endpoint de moderador/admin para la recepción de informes de paquetes.

Autenticación:

- Requiere un token de API de un usuario moderador o admin.
...
Endpoint de moderador/admin para resolver o reabrir informes de paquetes.
...
- Los encabezados de descarga incluyen SHA-256 de ClawHub más metadatos integrity/shasum de npm.
...
- Campo opcional de la carga útil: `migrateOwner`. Cuando es `true` con `ownerHandle`, una
  skill existente puede moverse a ese propietario si el actor es admin/propietario tanto en los publicadores
  actuales como de destino. Sin esta opción explícita, los cambios de propietario se
  rechazan.
...
- Los campos de formulario permitidos son `payload`, blobs `files` repetidos o una referencia de tarball
  ...
  el flujo upload-url. Las publicaciones con storage-id en etapa también deben incluir el
...
- Las solicitudes de publicación multipart directas están limitadas a 18MB. Los tarballs de ClawPack pueden
  usar el flujo upload-url hasta el límite de tarball de 120MB.
- Campo opcional de la carga útil: `ownerHandle`. Cuando está presente, solo los admins pueden publicar en nombre de ese propietario.
...
Eliminación reversible / restauración de una skill (propietario, moderador o admin).
...
Las eliminaciones reversibles iniciadas por el propietario reservan el slug durante 30 días; después el slug puede reclamarlo
otro publicador. La respuesta de eliminación incluye `slugReservedUntil` cuando aplica este vencimiento.
Las ocultaciones de moderador/admin y las eliminaciones de seguridad no vencen de esta manera.
...
Solo admin. Garantiza que exista un publicador de organización para un identificador. Si el identificador aún apunta a un
...
sin editar filas de cuenta de Convex Auth. La solicitud debe nombrar ambos ids inmutables de cuenta
de proveedor GitHub; los identificadores mutables solo se usan como una guarda orientada al operador.
...
identidades principales de GitHub. La recuperación falla de forma cerrada cuando el publicador personal actual del usuario de destino
...
los alias de slug de habilidad, los paquetes, las advertencias del inspector de paquetes y las filas derivadas del resumen de búsqueda para que
...
para el identificador recuperado también se reasigna al usuario de reemplazo para que una sincronización de perfil posterior
...
Las fuentes de habilidades de GitHub tienen alcance de publicador y se informan como verificadas en lugar de reescribirse.
...
Bloquea a un usuario y elimina permanentemente las habilidades de su propiedad (solo moderador/administrador).
...
Si hospedas por cuenta propia, sirve este archivo (o establece `CLAWHUB_REGISTRY` explícitamente; heredado `CLAWDHUB_REGISTRY`).
