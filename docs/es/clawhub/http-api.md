...
- Si no hay disponible una IP de cliente de confianza, las solicitudes anónimas usan buckets de respaldo
...
- Los resultados se devuelven en orden de relevancia (similitud de embeddings + refuerzos por coincidencias exactas de tokens de slug/nombre + un pequeño factor previo de popularidad).
- La relevancia tiene más peso que la popularidad. Una coincidencia precisa de slug o token de nombre visible puede superar a una coincidencia más amplia con mucha más interacción.
...
- La popularidad se escala logarítmicamente y se limita. Los Skills con alta interacción pueden clasificarse más abajo cuando el texto de la consulta es una coincidencia más débil.
...
- No cambies el nombre de un slug solo para perseguir una consulta, salvo que el nuevo slug sea un mejor nombre canónico a largo plazo. Los slugs antiguos se convierten en alias de redirección, pero la URL canónica, el slug mostrado y los futuros resúmenes de búsqueda usan el slug nuevo.
...
Informa sobre un Skill para revisión de moderadores. Los informes son a nivel de Skill, opcionalmente vinculados
...
- Las cargas útiles de solicitudes de escaneo y los informes descargables caducan en el almacén de solicitudes de escaneo después de la ventana de retención.
...
Ruta canónica de reescaneo por lotes solo para administradores. Acepta la misma forma de carga útil que la heredada `POST /api/v1/skills/-/rescan-batch`.
...
- La identidad del Skill, la identidad del editor y los metadatos de la versión seleccionada son campos de nivel superior del envoltorio (`slug`, `displayName`, `publisherHandle`, `version`, `resolvedFrom`, `tag`, `createdAt`) para que la automatización de shell pueda leerlos sin desempaquetar envoltorios anidados.
...
- `security.signals.dependencyRegistry` se conserva por compatibilidad de respuesta v1, pero el escáner de existencia del registro de dependencias está retirado y esta clave siempre es `null`.
- La ausencia de Skill Card no afecta a `ok`, `decision` ni `reasons` de este endpoint; los clientes deberían leer localmente el `skill-card.md` instalado cuando necesiten el contenido de la tarjeta.
- Usa `/verify` cuando necesites el envoltorio de verificación de Skill Card para un solo Skill, `/card` cuando necesites Markdown de tarjeta generado y `/scan` cuando necesites datos detallados del escáner.
...
En el endpoint unificado `/api/v1/packages`, solo aplica a plugins; usa
...
- Las Skills hospedadas incluyen los archivos de la versión almacenada más reciente y se enumeran en
...
- Las Skills actuales respaldadas por GitHub con un análisis `clean` o `suspicious` incluyen
  ...
  hash de contenido y URL del archivo. No incluyen archivos fuente hospedados en ClawHub.
...
- `trust.moderationState` admite null. Es `null` cuando no existe moderación manual de la versión.
...
- `trust.reasons` es la lista de explicaciones orientada al usuario y a auditoría. Los códigos de motivo
...
- Este endpoint es intencionalmente más limitado que los endpoints de moderación
  ...
  identidades de reportadores, cuerpos de reportes, evidencia privada ni cronologías internas de revisión.
...
- Esta es la superficie de resolución de OpenClaw; evita adivinar el formato de archivo a partir de
  una URL compartida.
...
- Las versiones de ClawPack transmiten los bytes `.tgz` exactos del npm-pack cargado.
...
`finalAction: "revoke"` con un reporte confirmado para aplicar moderación de la versión en el
...
- Los campos de formulario permitidos son `payload`, blobs repetidos `files` o una referencia a tarball
...
- Los Plugins de código requieren `package.json`, metadatos de repositorio de código fuente, metadatos de commit
...
- Las publicaciones en nombre de otro también validan la elegibilidad del canal oficial contra la cuenta de propietario de destino.
...
El extremo usa dry-run de forma predeterminada. Aplicar la recuperación requiere `dryRun: false` y
...
La recuperación también migra campos heredados `ownerUserId` para las skills del publicador recuperado,
...
Las fuentes de Skills de GitHub tienen alcance de publicador y se reportan como comprobadas en lugar de reescribirse.
...
### Extremos de gestión de slugs de propietario
...
- Ambos extremos requieren autenticación con token de API y solo funcionan para el propietario de la skill.
...
### Extremos de transferencia de propiedad
...
Bloquea a un usuario y elimina definitivamente las skills de su propiedad (solo moderador/admin).
...
Añade/elimina una estrella (destacados). Ambos extremos son idempotentes.
...
## Extremos de CLI heredados (obsoletos)
