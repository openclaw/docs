...
x-i18n:
    generated_at: "2026-07-06T10:46:46Z"
    model: gpt-5.5
...
- Los endpoints de escritura autenticados no deben devolver un simple `Unauthorized` cuando
  el servidor conoce el motivo. Los tokens faltantes, los tokens inválidos/revocados y
  las cuentas eliminadas/baneadas/deshabilitadas deben recibir texto accionable para que los
  clientes CLI puedan indicar a los usuarios qué los bloqueó.
...
- Si no hay una IP de cliente confiable disponible, las solicitudes anónimas usan buckets de reserva
  delimitados solo por tipo de límite de tasa. Estos buckets de reserva no incluyen
...
- Los resultados se devuelven en orden de relevancia (similitud de embedding + impulsos por coincidencias exactas de tokens de slug/nombre + un pequeño prior de popularidad).
...
- La popularidad se escala logarítmicamente y tiene un límite. Los Skills con mucha interacción pueden posicionarse más abajo cuando el texto de la consulta coincide peor.
...
- Los alias de renombre preservan la resolución para URL antiguas e instalaciones que se resuelven a través del registro, pero el ranking de búsqueda se basa en los metadatos canónicos del Skill después de que el renombre se haya indexado. Las estadísticas existentes permanecen con el Skill.
- Si un Skill está inesperadamente invisible, revisa primero el estado de moderación con `clawhub inspect @owner/slug` mientras tienes sesión iniciada antes de cambiar metadatos relacionados con el ranking.
...
- Los slugs antiguos creados por flujos de renombre/fusión del propietario se resuelven al Skill canónico.
- `metadata.os`: restricciones de SO declaradas en el frontmatter del Skill (p. ej., `["macos"]`, `["linux"]`). `null` si no se declaran.
...
- Las cargas de solicitud de análisis y los informes descargables caducan del almacén de solicitudes de análisis después de la ventana de retención.
...
- La respuesta es `202` con `{ "ok": true, "scanId": "...", "jobId": "...", "status": "queued", "sourceKind": "published", "update": false, "queue": { "queuedAhead": 0, "queuedAheadIsEstimate": false, "position": 1, "running": 0, "runningIsEstimate": false, "note": "Los análisis son asincrónicos y pueden tardar en completarse." } }`.
- Los trabajos de análisis son asincrónicos. Las solicitudes de análisis manual se priorizan por delante del trabajo normal de publicación/rellenado, pero la finalización sigue dependiendo de la disponibilidad de los trabajadores.
...
- Devuelve `queue.queuedAhead` y `queue.position` mientras está en cola para que los clientes puedan mostrar cuántos análisis manuales priorizados están por delante de la solicitud. Las colas muy grandes se limitan y se informan con `queuedAheadIsEstimate: true`.
...
- Requiere acceso de administración de propietario/editor a la skill o Plugin, o autoridad de moderador/administrador de la plataforma.
- Devuelve resultados de análisis almacenados para la versión exacta enviada, incluidas versiones bloqueadas u ocultas.
- `kind` tiene como valor predeterminado `skill`; usa `kind=plugin` para análisis de plugin/paquete.
- Devuelve la misma forma ZIP que las descargas de solicitudes de análisis.

### `POST /api/v1/skills/-/scan/batch`

Ruta canónica de reanálisis por lotes solo para administradores. Acepta la misma forma de carga que la ruta heredada `POST /api/v1/skills/-/rescan-batch`.
...
- `ok` es `true` solo cuando la versión seleccionada tiene una Skill Card generada, no está bloqueada por malware en la moderación y la verificación de ClawScan está limpia.
...
- `security.signals.dependencyRegistry` se conserva por compatibilidad de respuesta v1, pero el escáner de existencia de registro de dependencias está retirado y esta clave siempre es `null`.
- La ausencia de Skill Card no afecta el `ok`, `decision` ni `reasons` de este endpoint; los clientes deben leer `skill-card.md` instalado localmente cuando necesiten contenido de la tarjeta.
...
- Las Skills actuales respaldadas por GitHub con un escaneo `clean` o `suspicious` incluyen
  ...
  hash de contenido y URL de archivo. No incluyen archivos de origen alojados en ClawHub.
- Cada Skill incluye `_export_skill_meta.json`.
- `_manifest.json` siempre se incluye en la raíz del ZIP.
- `_errors.json` se incluye cuando Skills o archivos individuales no pudieron
  exportarse.
...
- Los controles de ordenación de la interfaz de navegador para la búsqueda de plugins reordenan los resultados de relevancia cargados,
  coincidiendo con el comportamiento de navegación actual de `/skills`.
...
Devuelve una versión de paquete, incluidos los metadatos de archivos, compatibilidad,
verificación, metadatos de artefacto y datos de análisis.

Notas:

- `version.artifact.kind` es `legacy-zip` para archivos de paquete del sistema antiguo o
...
- `trust.moderationState` es anulable. Es `null` cuando no existe moderación manual
  de la versión.
...
- Esta es la superficie del resolvedor de OpenClaw; evita adivinar el formato de archivo a partir
...
Lo usa la CLI para mapear una huella digital local a una versión conocida.
...
Descarga un ZIP de versión de skill alojado, o devuelve una entrega a fuente de GitHub para una
...
- Las entregas de skills respaldadas por GitHub no actúan como proxy ni mirror de bytes. La respuesta JSON
...
- Campo opcional del payload: `ownerHandle`. Cuando está presente, la API resuelve ese
  publicador del lado del servidor y requiere que el actor tenga acceso al publicador.
- Campo opcional del payload: `migrateOwner`. Cuando es `true` con `ownerHandle`, una
  skill existente puede moverse a ese propietario si el actor es administrador/propietario tanto en el
  publicador actual como en el de destino. Sin esta aceptación explícita, los cambios de propietario se
  rechazan.
...
- Los campos de formulario permitidos son `payload`, blobs `files` repetidos o una referencia de tarball
  ...
  el flujo upload-url. Las publicaciones por storage-id preparado también deben incluir el
...
publicador puede reclamar el slug. La respuesta de eliminación incluye `slugReservedUntil` cuando se aplica esta expiración.
...
Solo administradores. Garantiza que exista un publicador de organización para un handle. Si el handle todavía apunta a un
...
versión. Los nombres de paquetes se convierten en paquetes de marcador privado sin filas de versión, de modo que el mismo
propietario pueda publicar luego la versión real de code-plugin o bundle-plugin en ese nombre.
...
Si usas alojamiento propio, sirve este archivo (o establece `CLAWHUB_REGISTRY` explícitamente; heredado `CLAWDHUB_REGISTRY`).
