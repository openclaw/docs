...
x-i18n:
    generated_at: "2026-07-03T13:15:36Z"
    model: gpt-5.5
...
Los directorios de terceros pueden usar los endpoints públicos de lectura para listar o buscar Skills de ClawHub. Almacena los resultados en caché, respeta `429`/`Retry-After`, enlaza a los usuarios a la ficha canónica de ClawHub (`https://clawhub.ai/<owner>/skills/<slug>`) y evita sugerir respaldo de ClawHub al sitio de terceros. No intentes replicar contenido oculto, privado o bloqueado por moderación fuera de la superficie de la API pública.

Los atajos de slugs web se resuelven entre familias del registro, pero los clientes de API deben usar
...
- Los endpoints de escritura autenticados no deberían devolver un `Unauthorized` sin más cuando
  el servidor conoce la razón. Los tokens ausentes, tokens inválidos/revocados y
...
- Usa retroceso con variación aleatoria para evitar reintentos sincronizados.
- Si falta `Retry-After`, recurre a `RateLimit-Reset` (o calcula a partir de `X-RateLimit-Reset`).

Origen de IP:

- Usa cabeceras de IP de cliente de confianza, incluida `cf-connecting-ip`, solo cuando el
  despliegue habilita explícitamente las cabeceras reenviadas de confianza.
- ClawHub usa cabeceras de reenvío de confianza para identificar las IP de clientes en el borde.
- Si no hay disponible una IP de cliente de confianza, las solicitudes anónimas usan buckets de reserva
  ...
  parámetros de artefactos suministrados por el llamador.
...
- Los resultados se devuelven en orden de relevancia (similitud de embeddings + refuerzos por coincidencia exacta de slug/token de nombre + una pequeña prioridad de popularidad).
- La relevancia pesa más que la popularidad. Una coincidencia precisa de slug o token de nombre mostrado puede superar a una coincidencia más laxa con mucha más interacción.
...
- La popularidad se escala logarítmicamente y se limita. Las Skills con alta interacción pueden posicionarse más abajo cuando el texto de la consulta es una coincidencia más débil.
...
- Coloca los términos que los usuarios buscarán literalmente en el nombre mostrado, el resumen y las etiquetas. Usa un token de slug independiente solo cuando también sea una identidad estable que quieras conservar.
- No renombres un slug solo para perseguir una consulta, salvo que el nuevo slug sea un mejor nombre canónico a largo plazo. Los slugs antiguos se convierten en alias de redirección, pero la URL canónica, el slug mostrado y los futuros resúmenes de búsqueda usan el nuevo slug.
- Los alias de renombrado preservan la resolución para URL antiguas e instalaciones que se resuelven mediante el registro, pero la clasificación de búsqueda se basa en los metadatos canónicos de la Skill después de que el renombrado se haya indexado. Las estadísticas existentes permanecen con la Skill.
- Si una Skill está inesperadamente invisible, primero revisa el estado de moderación con `clawhub inspect @owner/slug` mientras tienes sesión iniciada antes de cambiar metadatos relacionados con la clasificación.
...
- `sort` (opcional): `updated` (predeterminado), `recommended` (alias: `default`), `createdAt` (alias: `newest`), `downloads`, `stars` (alias: `rating`), los alias heredados de instalaciones `installsCurrent`/`installs`/`installsAllTime` se asignan a `downloads`, `trending`
- `nonSuspiciousOnly` (opcional): `true` para ocultar Skills sospechosas (`flagged.suspicious`)
- `nonSuspicious` (opcional): alias heredado de `nonSuspiciousOnly`
...
- `recommended` usa señales de interacción y recencia.
...
- Usa `nextCursor` para continuar la paginación cuando esté presente. Una página corta no significa por sí sola que se acabaron los resultados.
...
- Los slugs antiguos creados por flujos de renombrado/fusión de propietario se resuelven a la Skill canónica.
...
- `metadata.systems`: destinos de sistema Nix (por ejemplo, `["aarch64-darwin", "x86_64-linux"]`). `null` si no se declararon.
...
- Incluye el estado de verificación normalizado más los detalles específicos del escáner.
...
- Las cargas útiles de solicitud de escaneo y los informes descargables expiran del almacén de solicitudes de escaneo después de la ventana de retención.
...
- Los escaneos publicados solo escriben de vuelta cuando `update: true` y el escaneo se completa correctamente.
...
- Los trabajos de escaneo son asíncronos. Las solicitudes de escaneo manual se priorizan por delante del trabajo normal de publicación/rellenado, pero la finalización sigue dependiendo de la disponibilidad de los workers.
...
- `provenance` es `server-resolved-github-import` solo cuando ClawHub resolvió y almacenó un repo/ref/commit/ruta de GitHub durante la publicación o importación; de lo contrario, es `unavailable`.
...
- Los resultados son por elemento; un skill o una versión ausente no hacen fallar toda la respuesta.
...
- `category` (opcional): filtro de categoría de plugin. Solo se admite cuando la
  solicitud está acotada a paquetes de plugin (`/api/v1/plugins`,
...
- Las entradas de habilidades siguen respaldadas por el registro de habilidades y aún solo pueden publicarse mediante `POST /api/v1/skills`.
...
Búsqueda de catálogo unificado entre habilidades y paquetes de plugin.
...
- `isOfficial` (opcional): `true` o `false`
- `category` (opcional): filtro de categoría de plugin. Solo se admite cuando la
  solicitud está acotada a paquetes de plugin. Las categorías controladas y los
  alias heredados del filtro v1 están documentados en `GET /api/v1/plugins`.
...
`/api/v1/skills?sort=trending` para el catálogo de habilidades.

Los alias heredados no se aceptan como valores de categoría almacenados o declarados por autores.

### `GET /api/v1/skills/export`

Exportación masiva de las habilidades públicas más recientes para análisis sin conexión.
...
- `startDate` (obligatorio): límite inferior en milisegundos Unix para `updatedAt` de la habilidad.
- `endDate` (obligatorio): límite superior en milisegundos Unix para `updatedAt` de la habilidad.
...
- Cuerpo: archivo ZIP.
- Cada habilidad exportada tiene raíz en `{publisher}/{slug}/`.
- Las habilidades alojadas incluyen los archivos de la versión almacenada más reciente y se listan en
  `_manifest.json` con `sourceRef: "public-clawhub"`.
- Las habilidades actuales respaldadas por GitHub con un escaneo `clean` o `suspicious` incluyen
...
- Cada habilidad incluye `_export_skill_meta.json`.
- `_manifest.json` siempre se incluye en la raíz del ZIP.
- `_errors.json` se incluye cuando no se pudieron exportar habilidades o archivos
  individuales.
...
- También se aceptan los alias heredados de filtros v1 documentados en `GET /api/v1/plugins`.
- El filtrado por categoría es un filtro real de API respaldado por filas de resumen
  de categorías de Plugin, no una reescritura de la consulta de búsqueda.
...
Esta es la superficie pública de consumo de OpenClaw para decidir si se puede instalar
...
- `trust.stale` significa que el resumen de confianza se calculó a partir de entradas desactualizadas y
  debe tratarse como si requiriera una actualización antes de una decisión de permiso de alta confianza.
...
- Este endpoint es intencionalmente más limitado que los endpoints de moderación para propietarios/moderadores.
  Expone la decisión de instalación y la explicación pública, no identidades de denunciantes,
  cuerpos de reportes, evidencia privada ni cronologías internas de revisión.
...
- Esta es la superficie de resolvedor de OpenClaw; evita adivinar el formato del archivo desde
  una URL compartida.
...
- `revoked`: bloqueado después de que una versión se considerara previamente confiable.
...
- Usa el grupo de límite de lectura, no el grupo de límite de descarga.
...
Transmite los bytes exactos del tarball ClawPack subido para clientes del espejo npm.

Notas:

- Usa el grupo de límite de descarga.
- Los encabezados de descarga incluyen SHA-256 de ClawHub más metadatos npm de integridad/shasum.
...
skill actual respaldada por GitHub con un análisis `clean` o `suspicious` y sin
versión alojada.
...
- Las transferencias de skills respaldadas por GitHub no actúan como proxy ni espejo de bytes. La respuesta JSON
  ...
  y `archiveUrl`; el análisis/estado actual es una compuerta y no se incluye como metadatos de carga útil
  de éxito.
...
- Campo opcional de carga útil: `migrateOwner`. Cuando es `true` con `ownerHandle`, una
  ...
  los publicadores actual como de destino. Sin esta aceptación explícita, los cambios de propietario
  se rechazan.
...
- Las publicaciones en nombre de otra cuenta siguen validando la elegibilidad del canal oficial contra la cuenta propietaria de destino.
...
Las eliminaciones reversibles iniciadas por el propietario reservan el slug durante 30 días; después, otro publicador
puede reclamar el slug. La respuesta de eliminación incluye `slugReservedUntil` cuando aplica esta caducidad.
Las ocultaciones por moderador/administrador y las eliminaciones de seguridad no caducan de esta forma.
...
usuario/publicador personal compartido heredado, el endpoint lo migra primero a un publicador de organización.
...
Creación autenticada y autoservicio de publicador de organización. Crea un nuevo publicador de organización y añade al
llamador como propietario. Este endpoint no migra identificadores existentes de usuario/personales y
...
versión. Los nombres de paquetes se convierten en paquetes privados de marcador sin filas de versión, para que el mismo
propietario pueda publicar más tarde la versión real code-plugin o bundle-plugin en ese nombre.
...
sin editar las filas de cuenta de Convex Auth. La solicitud debe nombrar ambos ids de cuenta de proveedor GitHub
...
principales de GitHub. La recuperación falla cerrada cuando el publicador personal actual del usuario de destino
...
las rutas de propietario directo coincidan con la nueva autoridad del publicador. Una reserva activa de handle protegido
para el handle recuperado también se reasigna al usuario de reemplazo para que la sincronización posterior del perfil no pueda restaurar la autoridad competidora del usuario anterior. Cada tabla principal está limitada a
...
Bloquea a un usuario y elimina definitivamente las Skills que posee (solo moderador/admin).
...
Desbloquea a un usuario y restaura las Skills elegibles (solo admin).
...
contenido (solo admin). Usa dry-run de forma predeterminada salvo que `dryRun` sea `false`.
...
Si usas alojamiento propio, sirve este archivo (o define `CLAWHUB_REGISTRY` explícitamente; `CLAWDHUB_REGISTRY` heredado).
