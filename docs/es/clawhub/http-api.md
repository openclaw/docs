---
read_when:
    - Añadir/modificar puntos de conexión
...
Los directorios de terceros pueden usar los endpoints públicos de lectura para listar o buscar Skills de ClawHub. Almacena los resultados en caché, respeta `429`/`Retry-After`, enlaza a los usuarios de vuelta al listado canónico de ClawHub (`https://clawhub.ai/<owner>/<slug>`) y evita insinuar que ClawHub avala el sitio de terceros. No intentes replicar contenido oculto, privado o bloqueado por moderación fuera de la superficie de la API pública.

Los atajos de slug web se resuelven entre familias de registro, pero los clientes de API deberían usar
...
- Los endpoints autenticados de escritura no deberían devolver un `Unauthorized` aislado cuando
  el servidor conoce el motivo. Los tokens ausentes, tokens no válidos o revocados, y las
  cuentas eliminadas, bloqueadas o deshabilitadas deberían recibir texto accionable para que los
  clientes CLI puedan decirles a los usuarios qué los bloqueó.
...
- Los resultados se devuelven en orden de relevancia (similitud de embeddings + refuerzos por coincidencia exacta de tokens de slug/nombre + prioridad de popularidad basada en descargas).
...
- Las descargas se usan como una pequeña prioridad con escala logarítmica y como desempate, no como la señal principal de clasificación. Las Skills con muchas descargas pueden posicionarse más abajo cuando el texto de la consulta es una coincidencia más débil.
...
- Incluye los términos que los usuarios buscarán literalmente en el nombre visible, el resumen y las etiquetas. Usa un token de slug independiente solo cuando también sea una identidad estable que quieras mantener.
...
- `metadata.os`: restricciones de sistema operativo declaradas en el frontmatter de la Skill (por ejemplo, `["macos"]`, `["linux"]`). `null` si no se declaró.
...
Reporta una Skill para revisión de moderación. Los reportes son a nivel de Skill, opcionalmente vinculados
...
volver a establecer `status` en `open`. Pasa `finalAction: "hide"` con un reporte evaluado
...
- Los llamadores autenticados pueden ver paquetes privados de publicadores a los que pertenecen en los resultados de listado/búsqueda.
...
- `all`: cualquier lanzamiento con anulación manual, estado de análisis no limpio o reporte de paquete.
...
Endpoint de propietario/moderador para visibilidad de moderación del paquete.
...
- De forma predeterminada, se ejecuta en modo de simulación.
...
Transmite los bytes exactos del tarball de ClawPack subido para clientes de mirror de npm.
...
- Siguen aplicándose las comprobaciones de moderación y de acceso a paquetes privados.
...
- Los paquetes Plugin requieren `openclaw.plugin.json`. Las subidas ClawPack `.tgz` deben
...
Quita el baneo de un usuario y restaura las skills elegibles (solo administradores).
...
Si alojas tu propia instancia, sirve este archivo (o establece `CLAWHUB_REGISTRY` explícitamente; heredado `CLAWDHUB_REGISTRY`).
