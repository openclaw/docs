...
Los directorios de terceros pueden usar los endpoints públicos de lectura para listar o buscar Skills de ClawHub. Guarda los resultados en caché, respeta `429`/`Retry-After`, enlaza a los usuarios de vuelta al listado canónico de ClawHub (`https://clawhub.ai/<owner>/skills/<slug>`) y evita insinuar que ClawHub avala el sitio de terceros. No intentes replicar contenido oculto, privado o bloqueado por moderación fuera de la superficie de la API pública.
...
- Si no hay una IP de cliente confiable disponible, las solicitudes anónimas usan buckets de respaldo
  delimitados solo por el tipo de límite de tasa. Estos buckets de respaldo no incluyen
...
- Los resultados se devuelven en orden de relevancia (similitud de embeddings + impulsos por coincidencia exacta de tokens de slug/nombre + un pequeño prior de popularidad).
- La relevancia pesa más que la popularidad. Una coincidencia precisa de token de slug o nombre visible puede superar a una coincidencia menos exacta con mucha más interacción.
...
- La popularidad se escala logarítmicamente y se limita. Los Skills con alta interacción pueden posicionarse más abajo cuando el texto de consulta coincide peor.
...
- Los alias de renombre preservan la resolución para URL antiguas e instalaciones que se resuelven mediante el registro, pero el ranking de búsqueda se basa en los metadatos canónicos del Skill después de que el cambio de nombre se haya indexado. Las estadísticas existentes permanecen con el Skill.
- Si un Skill está inesperadamente invisible, revisa primero el estado de moderación con `clawhub inspect @owner/slug` con sesión iniciada antes de cambiar metadatos relacionados con el ranking.
...
- Usa `nextCursor` para continuar la paginación cuando esté presente. Una página corta no significa por sí sola que se alcanzó el final de los resultados.
...
- Los slugs antiguos creados por flujos de renombre/fusión de propietario se resuelven al Skill canónico.
- `metadata.os`: restricciones de SO declaradas en el frontmatter del Skill (por ejemplo, `["macos"]`, `["linux"]`). `null` si no están declaradas.
- `metadata.systems`: objetivos de sistema Nix (por ejemplo, `["aarch64-darwin", "x86_64-linux"]`). `null` si no están declarados.
...
Ruta canónica solo para administradores de reescaneo por lotes. Acepta la misma forma de carga útil que la heredada `POST /api/v1/skills/-/rescan-batch`.

### `POST /api/v1/skills/-/scan/batch/status`

Ruta canónica solo para administradores de estado de lotes. Acepta `{ "jobIds": ["..."] }` y devuelve los mismos contadores agregados que la heredada `POST /api/v1/skills/-/rescan-batch/status`.
...
- `ok` es `true` solo cuando la versión seleccionada tiene una Skill Card generada, no está bloqueada como malware por la moderación y la verificación de ClawScan es limpia.
...
- `security` es el veredicto de nivel superior de ClawScan/seguridad. La automatización debería basarse en `ok`, `decision`, `reasons` y `security.status`.
...
- Usa `/verify` cuando necesites el sobre de verificación de Skill Card de un solo skill, `/card` cuando necesites Markdown de tarjeta generado y `/scan` cuando necesites datos detallados del escáner.
...
Exploración del catálogo solo para Plugins entre paquetes code-plugin y bundle-plugin.
...
Devuelve el resumen exacto de seguridad y confianza de la versión del paquete para clientes de instalación. Esta es la superficie pública de consumo de OpenClaw para decidir si una versión resuelta puede instalarse.
...
- Usa el grupo de límite de tasa de descargas.
...
- repositorio fuente y procedencia de commit
...
Reporta un paquete para revisión de moderador. Los reportes son de nivel de paquete, opcionalmente
...
- Requiere un token de API para el propietario del paquete, miembro editor, moderador o
...
- Los archivos de Plugin/paquete son archivos zip con una raíz `package/` para que los clientes antiguos de OpenClaw
  sigan funcionando.
- Esta ruta permanece solo ZIP. No transmite archivos ClawPack `.tgz`.
...
Usado por la CLI para mapear una huella digital local a una versión conocida.
...
- Las transferencias de Skills respaldadas por GitHub no intermedian ni espejan bytes. La respuesta JSON
  incluye `sourceRef: "public-github"`, `repo`, `commit`, `path`, `contentHash`
  y `archiveUrl`; el estado de análisis/actual es una puerta y no se incluye como metadatos de
  payload de éxito.
...
- Campo opcional de payload: `ownerHandle`. Cuando está presente, la API resuelve ese
  editor del lado del servidor y requiere que el actor tenga acceso al editor.
- Campo opcional de payload: `migrateOwner`. Cuando es `true` con `ownerHandle`, una
  ...
  los editores actual como de destino. Sin esta aceptación explícita, los cambios de propietario se
  rechazan.
...
- Las solicitudes de publicación multipart directa tienen un límite de 18MB. Los tarballs de ClawPack pueden
...
Las eliminaciones reversibles iniciadas por el propietario reservan el slug durante 30 días; después, otro editor
puede reclamar el slug. La respuesta de eliminación incluye `slugReservedUntil` cuando esta expiración aplica.
...
Solo administrador. Garantiza que exista un editor de organización para un identificador. Si el identificador aún apunta a un
usuario compartido/editor personal heredado, el punto de conexión primero lo migra a un editor de organización.
...
Creación autenticada de editor de organización en autoservicio. Crea un nuevo editor de organización y agrega al
llamador como propietario. Este punto de conexión no migra identificadores de usuario/personales existentes y
no marca al editor como confiable/oficial.

- Cuerpo: `{ "handle": "opik", "displayName": "Opik" }`
- Respuesta: `{ "ok": true, "publisherId": "...", "handle": "opik", "created": true, "trusted": false }`
- Devuelve `409` cuando el identificador ya está usado por un editor, usuario o editor personal.
...
Solo administrador. Recupera un editor personal para un principal de OAuth de GitHub de reemplazo verificado
sin editar filas de cuenta de Convex Auth. La solicitud debe nombrar ambos ids inmutables de cuenta de
proveedor de GitHub; los identificadores mutables solo se usan como una protección orientada al operador.
...
Prohíbe a un usuario y elimina definitivamente las skills de su propiedad (solo moderador/administrador).
...
Quita la prohibición de un usuario y restaura las skills elegibles (solo administrador).
...
Cambia el motivo almacenado de una prohibición existente sin quitar la prohibición ni restaurar
...
Si usas alojamiento propio, sirve este archivo (o define `CLAWHUB_REGISTRY` explícitamente; heredado `CLAWDHUB_REGISTRY`).
