...
- Los endpoints de escritura autenticados no deberían devolver un simple `Unauthorized` cuando
  ...
  cuentas eliminadas, baneadas o deshabilitadas deberían recibir texto accionable para que los clientes
...
- Si no hay una IP de cliente confiable disponible, las solicitudes anónimas usan segmentos de respaldo
  delimitados solo por el tipo de límite de tasa. Estos segmentos de respaldo no incluyen
...
- Los resultados se devuelven en orden de relevancia (similitud de embeddings + impulsos por coincidencia exacta de token de slug/nombre + un pequeño valor previo de popularidad).
- La relevancia pesa más que la popularidad. Una coincidencia precisa de slug o token de nombre para mostrar puede superar a una coincidencia más amplia con mucha más interacción.
...
- La popularidad se escala logarítmicamente y tiene un tope. Los Skills con alta interacción pueden posicionarse más abajo cuando el texto de la consulta coincide peor.
...
- Pon los términos que los usuarios buscarán literalmente en el nombre para mostrar, el resumen y las etiquetas. Usa un token de slug independiente solo cuando también sea una identidad estable que quieras conservar.
...
- Los alias de cambio de nombre preservan la resolución para URL antiguas e instalaciones que resuelven mediante el registro, pero el posicionamiento de búsqueda se basa en los metadatos canónicos del Skill después de que el cambio de nombre se haya indexado. Las estadísticas existentes permanecen con el Skill.
- Si un Skill está inesperadamente invisible, revisa primero el estado de moderación con `clawhub inspect @owner/slug` mientras tienes sesión iniciada antes de cambiar metadatos relacionados con el posicionamiento.
...
Informa sobre un Skill para revisión de moderador. Los informes son a nivel de Skill, opcionalmente vinculados
...
- Las cargas útiles de solicitudes de análisis y los informes descargables caducan del almacén de solicitudes de análisis después de la ventana de retención.
...
- Los análisis publicados solo escriben de vuelta cuando `update: true` y el análisis finaliza correctamente.
...
- Los trabajos de análisis son asíncronos. Las solicitudes de análisis manual se priorizan por delante del trabajo normal de publicación/relleno, pero la finalización sigue dependiendo de la disponibilidad de workers.
...
Skills de ClawHub instaladas necesitan mostrar, como OpenClaw Control UI.
...
- La respuesta es solo de seguridad. No incluye datos de Skill Card, estado de tarjeta generada, listas de archivos de artefactos ni cargas útiles detalladas del escáner.
...
- Cuerpo: archivo ZIP.
- Cada Skill exportado tiene raíz en `{publisher}/{slug}/`.
- Los Skills alojados incluyen los archivos de la última versión almacenada y se listan en
  `_manifest.json` con `sourceRef: "public-clawhub"`.
- Los Skills actuales respaldados por GitHub con un análisis `clean` o `suspicious` incluyen
...
- Skills también se pueden resolver a través de esta ruta en el catálogo unificado.
...
- Esta es la superficie de resolución de OpenClaw; evita adivinar el formato del archivo desde
  una URL compartida.
...
- Usa el grupo de límite de tasa de descarga.
...
- Requiere un token de API del propietario del paquete, miembro publicador,
...
Descarga un ZIP de versión de Skills alojada o devuelve una entrega de origen de GitHub para una
...
- Las entregas de Skills respaldadas por GitHub no proxifican ni replican bytes. La respuesta JSON
...
- Campo de carga útil opcional: `migrateOwner`. Cuando es `true` con `ownerHandle`, una
  Skills existente puede moverse a ese propietario si el actor es administrador/propietario en ambos
...
Cuando está presente, `reason` se almacena como la nota de moderación de la Skills y se copia en el registro de auditoría.
...
`memberRole` toma `owner` como valor predeterminado.
...
inmutables; los identificadores mutables solo se usan como protección visible para el operador.

El punto de conexión usa simulación por defecto. Aplicar la recuperación requiere `dryRun: false` y
...
los alias de slugs de habilidades, los paquetes, las advertencias del inspector de paquetes y las filas derivadas del resumen de búsqueda para que
...
### Puntos de conexión de gestión de slug de propietario
...
- Ambos puntos de conexión requieren autenticación con token de API y solo funcionan para el propietario de la habilidad.
...
Bloquea a un usuario y elimina de forma definitiva las habilidades de su propiedad (solo moderador/administrador).
...
contenido (solo administrador). Usa simulación por defecto salvo que `dryRun` sea `false`.
...
Agrega/elimina una estrella (destacados). Ambos puntos de conexión son idempotentes.
...
Si alojas el servicio tú mismo, sirve este archivo (o establece `CLAWHUB_REGISTRY` explícitamente; `CLAWDHUB_REGISTRY` heredado).
