...
x-i18n:
    generated_at: "2026-07-04T15:07:07Z"
    model: gpt-5.5
...
- Si no hay una IP de cliente confiable disponible, las solicitudes anónimas usan buckets de reserva
  ...
  rutas, slugs, nombres de paquetes, versiones, cadenas de consulta u otros parámetros
  de artefactos proporcionados por el llamador.
...
- Los resultados se devuelven en orden de relevancia (similitud de embeddings + refuerzos por coincidencia exacta de tokens de slug/nombre + una pequeña prioridad de popularidad).
...
- La popularidad se escala logarítmicamente y se limita. Las skills con alta interacción pueden clasificarse más abajo cuando el texto de consulta coincide peor.
...
- Los alias de cambio de nombre conservan la resolución para URL antiguas e instalaciones que se resuelven a través del registro, pero el ranking de búsqueda se basa en los metadatos canónicos de la skill después de que se indexe el cambio de nombre. Las estadísticas existentes permanecen con la skill.
...
- Usa `nextCursor` para continuar la paginación cuando esté presente. Una página corta no significa por sí sola que se hayan acabado los resultados.
...
- Los slugs antiguos creados por flujos de cambio de nombre o fusión del propietario se resuelven a la skill canónica.
- `metadata.os`: restricciones de sistema operativo declaradas en el frontmatter de la skill (p. ej., `["macos"]`, `["linux"]`). `null` si no se declara.
...
- Devuelve `queue.queuedAhead` y `queue.position` mientras está en cola para que los clientes puedan mostrar cuántos escaneos manuales priorizados están por delante de la solicitud. Las colas muy grandes están acotadas y se informan con `queuedAheadIsEstimate: true`.
...
Ruta canónica solo para administradores de reescaneo por lotes. Acepta la misma forma de carga útil que el `POST /api/v1/skills/-/rescan-batch` heredado.

### `POST /api/v1/skills/-/scan/batch/status`

Ruta canónica solo para administradores de estado de lote. Acepta `{ "jobIds": ["..."] }` y devuelve los mismos contadores agregados que el `POST /api/v1/skills/-/rescan-batch/status` heredado.
...
- `security.signals.dependencyRegistry` se conserva por compatibilidad con respuestas v1, pero el escáner de existencia de registro de dependencias está retirado y esta clave siempre es `null`.
- La ausencia de Skill Card no afecta a `ok`, `decision` ni `reasons` de este endpoint; los clientes deberían leer `skill-card.md` instalado localmente cuando necesiten contenido de la tarjeta.
...
- Los llamadores autenticados pueden ver paquetes privados de los editores a los que pertenecen en los resultados de listado/búsqueda.
- `channel=private` solo devuelve paquetes que el llamador autenticado puede leer.

### `GET /api/v1/packages/search`

Búsqueda de catálogo unificado entre Skills + paquetes de plugin.
...
- También se aceptan los alias de filtros legados de v1 documentados en
  `GET /api/v1/plugins`.
...
- Skills también se pueden resolver mediante esta ruta en el catálogo unificado.
...
- Las versiones de ClawPack incluyen campos compatibles con npm `npmIntegrity`, `npmShasum` y
  `npmTarballName`.
- `version.sha256hash` es metadato de compatibilidad obsoleto para clientes antiguos. Calcula
...
- `trust.moderationState` puede ser nulo. Es `null` cuando no existe moderación manual de
  la versión.
- `trust.blockedFromDownload` es la señal de bloqueo de instalación. OpenClaw y otros
  ...
  volver a derivar reglas de bloqueo a partir de campos de análisis o moderación.
...
- Las versiones de paquetes legadas devuelven un artefacto `legacy-zip` y una
...
- Esta es la superficie de resolución de OpenClaw; evita inferir el formato del archivo
...
- Las versiones de ClawPack transmiten los bytes exactos `.tgz` npm-pack cargados.
...
`finalAction: "revoke"` con un informe confirmado para aplicar moderación de la versión en el
...
- `revoked`: bloqueado después de que una versión hubiera sido considerada confiable anteriormente.
...
Descarga un ZIP de una versión alojada de una habilidad, o devuelve una transferencia a fuente de GitHub para una
...
- Las transferencias de habilidades respaldadas por GitHub no actúan como proxy ni reflejan bytes. La respuesta JSON
  incluye `sourceRef: "public-github"`, `repo`, `commit`, `path`, `contentHash`,
  y `archiveUrl`; el estado de escaneo/actual es una puerta de control y no se incluye como metadatos
  de payload de éxito.
...
- Campo de payload opcional: `ownerHandle`. Cuando está presente, la API resuelve ese
  editor en el servidor y exige que el actor tenga acceso de editor.
- Campo de payload opcional: `migrateOwner`. Cuando es `true` con `ownerHandle`, una
  habilidad existente puede moverse a ese propietario si el actor es administrador/propietario tanto en
  el editor actual como en el de destino. Sin esta adhesión explícita, se rechazan los cambios de
  propietario.
...
- Los campos de formulario permitidos son `payload`, blobs `files` repetidos, o una referencia de tarball `clawpack`.
  ...
  el flujo upload-url. Las publicaciones con storage-id en staging también deben incluir el
...
- Campo de payload opcional: `ownerHandle`. Cuando está presente, solo los administradores pueden publicar en nombre de ese propietario.
...
- Solo el editor de la organización `openclaw` y los editores personales de los miembros actuales de la organización `openclaw`
...
Las eliminaciones reversibles iniciadas por el propietario reservan el slug durante 30 días; después, otro editor puede reclamar el slug.
...
Solo administradores. Garantiza que exista un editor de organización para un identificador. Si el identificador todavía apunta a un
usuario compartido heredado/editor personal, el endpoint lo migra primero a un editor de organización.
...
Creación autenticada y autoservicio de editores de organización. Crea un nuevo editor de organización y añade al
llamador como propietario. Este endpoint no migra identificadores existentes de usuario/personales y no
marca al editor como confiable/oficial.

- Cuerpo: `{ "handle": "opik", "displayName": "Opik" }`
- Respuesta: `{ "ok": true, "publisherId": "...", "handle": "opik", "created": true, "trusted": false }`
- Devuelve `409` cuando el identificador ya está en uso por un editor, usuario o editor personal.

### `POST /api/v1/users/reserve`

Solo administradores. Reserva slugs raíz y nombres de paquetes para un propietario legítimo sin publicar una
versión. Los nombres de paquetes se convierten en paquetes marcadores privados sin filas de versión, de modo que el mismo
...
Solo administradores. Recupera un editor personal para un principal de GitHub OAuth de reemplazo verificado
sin editar filas de cuenta de Convex Auth. La solicitud debe nombrar ambos ids de cuenta de proveedor de GitHub
inmutables; los identificadores mutables solo se usan como una protección visible para el operador.

El endpoint usa la simulación de forma predeterminada. Aplicar la recuperación requiere `dryRun: false` y
...
tiene skills, packages o fuentes de Skills de GitHub.
La recuperación también migra los campos heredados `ownerUserId` para las skills del publicador recuperado,
los alias de slug de skill, los packages, las advertencias del inspector de packages y las filas derivadas del resumen de búsqueda para que
...
Bloquea a un usuario y elimina permanentemente las skills de su propiedad (solo moderador/administrador).
...
contenido (solo administrador). Usa simulación de forma predeterminada a menos que `dryRun` sea `false`.
...
`POST /api/cli/upload-url` devuelve `uploadUrl` y `uploadTicket`. Las publicaciones de packages
