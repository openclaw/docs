...
summary: Referencia de la API HTTP (pública + puntos de conexión de CLI + autenticación).
x-i18n:
...
- Los endpoints de escritura autenticados no deben devolver un `Unauthorized` desnudo cuando
  el servidor conoce el motivo. Los tokens ausentes, tokens no válidos/revocados y
  cuentas eliminadas/baneadas/deshabilitadas deben recibir texto accionable para que los
...
- Los resultados se devuelven en orden de relevancia (similitud de embeddings + mejoras por coincidencia exacta de token de slug/nombre + un pequeño prior de popularidad).
...
- Si una Skill está inesperadamente invisible, comprueba primero el estado de moderación con `clawhub inspect @owner/slug` mientras tienes sesión iniciada antes de cambiar metadatos relacionados con la clasificación.
...
- `moderation` se incluye solo cuando la Skill está marcada o la ve su propietario.
...
Reporta una Skill para revisión por moderadores. Los reportes son a nivel de Skill, opcionalmente enlazados
...
establecer `status` de vuelta a `open`. Pasa `finalAction: "hide"` con un reporte
...
- `version.security` incluye el estado normalizado de verificación de escaneo y detalles del escáner
  (VirusTotal + LLM), cuando está disponible.
...
- Las cargas útiles de solicitud de análisis y los informes descargables caducan del almacén de solicitudes de análisis después de la ventana de retención.
...
- Los análisis publicados solo escriben los cambios cuando `update: true` y el análisis se completa correctamente.
...
- Los trabajos de análisis son asíncronos. Las solicitudes de análisis manuales tienen prioridad sobre el trabajo normal de publicación/rellenado, pero la finalización sigue dependiendo de la disponibilidad de los workers.
...
- Devuelve `queue.queuedAhead` y `queue.position` mientras está en cola para que los clientes puedan mostrar cuántos análisis manuales priorizados están por delante de la solicitud. Las colas muy grandes se acotan y se notifican con `queuedAheadIsEstimate: true`.
...
- `kind` tiene `skill` como valor predeterminado; usa `kind=plugin` para análisis de plugins/paquetes.
- Devuelve la misma forma de ZIP que las descargas de solicitudes de análisis.

### `POST /api/v1/skills/-/scan/batch`

Ruta canónica de reanálisis por lotes solo para administradores. Acepta la misma forma de carga útil que el `POST /api/v1/skills/-/rescan-batch` heredado.
...
endpoint de colección está pensado para clientes que ya saben qué versiones de
...
- Los resultados son por elemento; un skill o una versión ausente no hace fallar toda la respuesta.
...
- `trust.scanStatus` es el estado efectivo de confianza derivado de las entradas del escáner
  y la moderación manual de la versión.
- `trust.moderationState` admite valor nulo. Es `null` cuando no existe moderación manual
  de la versión.
...
- `trust.reasons` es la lista de explicación orientada al usuario y para auditoría. Los códigos de motivo
...
Reporta un paquete para revisión por moderador. Los reportes son a nivel de paquete, opcionalmente
...
`finalAction: "revoke"` con un reporte confirmado para aplicar moderación de lanzamiento en el
...
Punto de conexión de moderador/administrador para revisión de lanzamientos de paquetes.
...
- `revoked`: bloqueado después de que un lanzamiento fuera considerado confiable previamente.
...
propietario pueda publicar posteriormente el lanzamiento real code-plugin o bundle-plugin en ese nombre.
...
Las fuentes de Skills de GitHub están delimitadas al editor y se informan como comprobadas en lugar de reescribirse.
...
paquetes que preparan un tarball ClawPack deben enviar el id de almacenamiento resultante como
