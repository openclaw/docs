...
x-i18n:
    generated_at: "2026-07-01T05:27:33Z"
    model: gpt-5.5
...
Los directorios de terceros pueden usar los endpoints públicos de lectura para listar o buscar Skills de ClawHub. Almacena los resultados en caché, respeta `429`/`Retry-After`, enlaza a los usuarios de vuelta al listado canónico de ClawHub (`https://clawhub.ai/<owner>/skills/<slug>`) y evita insinuar que ClawHub avala el sitio de terceros. No intentes replicar contenido oculto, privado o bloqueado por moderación fuera de la superficie de la API pública.
...
- Si falta `Retry-After`, usa `RateLimit-Reset` (o calcúlalo a partir de `X-RateLimit-Reset`).
...
- Si no hay una IP de cliente confiable disponible, las solicitudes anónimas usan buckets de reserva
  limitados solo por tipo de límite de tasa. Estos buckets de reserva no incluyen
  rutas suministradas por el llamador, slugs, nombres de paquete, versiones, cadenas de consulta ni otros
...
- Los resultados se devuelven en orden de relevancia (similitud de embeddings + aumentos por coincidencias exactas de token de slug/nombre + un pequeño prior de popularidad).
...
- La popularidad se escala logarítmicamente y se limita. Las Skills con alta interacción pueden clasificarse más abajo cuando el texto de la consulta es una coincidencia más débil.
...
- `cursor` (opcional): cursor de paginación para cualquier ordenamiento que no sea `trending`
...
Endpoint de moderador/administrador para la admisión de reportes de Skills.
...
triageado para ocultar la Skill en el mismo flujo auditable.
...
- `version.security` incluye el estado de verificación de análisis normalizado y los detalles del escáner
...
- Las cargas útiles de solicitudes de escaneo y los informes descargables expiran del almacén de solicitudes de escaneo después del período de retención.
...
- Los trabajos de escaneo son asíncronos. Las solicitudes de escaneo manual se priorizan por delante del trabajo normal de publicación/rellenado, pero la finalización aún depende de la disponibilidad de trabajadores.
...
- Requiere acceso de administración del propietario/publicador a la skill o al plugin, o autoridad de moderador/administrador de la plataforma.
...
- La identidad de la skill, la identidad del publicador y los metadatos de la versión seleccionada son campos de sobre de nivel superior (`slug`, `displayName`, `publisherHandle`, `version`, `resolvedFrom`, `tag`, `createdAt`) para que la automatización de shell pueda leerlos sin desempaquetar envoltorios anidados.
...
- Usa `/verify` cuando necesites el sobre de verificación de Skill Card de una sola skill, `/card` cuando necesites markdown de tarjeta generada y `/scan` cuando necesites datos detallados de escáner.
...
- `isOfficial` (opcional): `true` o `false`
- `category` (opcional): filtro de categoría de Plugin. Solo se admite cuando la
  solicitud está acotada a paquetes de Plugin. Las categorías controladas y los alias
...
`trending` es una clasificación de instalaciones/descargas de siete días y no usa los totales históricos.
...
- También se aceptan los alias de filtro legados de v1 documentados en `GET /api/v1/plugins`.
...
- Los controles de ordenación de la interfaz de navegador para la búsqueda de Plugins reordenan los resultados de relevancia cargados,
  igual que el comportamiento de exploración actual de `/skills`.
...
Devuelve una versión del paquete, incluidos metadatos de archivo, compatibilidad,
...
Devuelve el resumen exacto de seguridad y confianza de la versión del paquete para clientes
...
- `trust.moderationState` puede ser nulo. Es `null` cuando no existe moderación
...
- `trust.blockedFromDownload` es la señal de bloqueo de instalación. OpenClaw y otros
  ...
  volver a derivar reglas de bloqueo a partir de los campos de escáner o moderación.
- `trust.reasons` es la lista de explicación orientada al usuario y para auditoría. Los códigos de razón
  son cadenas estables y compactas, como `manual:quarantined`, `scan:malicious`
  y `package:malicious`.
- `trust.pending` significa que una o más entradas de confianza todavía esperan completarse.
- `trust.stale` significa que el resumen de confianza se calculó a partir de entradas obsoletas y
  debe tratarse como pendiente de actualización antes de una decisión de permitir con alta confianza.
...
Devuelve los metadatos explícitos del resolutor de artefactos para una versión de paquete.

Notas:

- Las versiones de paquetes legados devuelven un artefacto `legacy-zip` y una URL ZIP legada
  `downloadUrl`.
- Las versiones de ClawPack devuelven un artefacto `npm-pack`, campos de integridad npm, una
  `tarballUrl` y la URL de compatibilidad ZIP legada.
- Esta es la superficie de resolución de OpenClaw; evita inferir el formato de archivo a partir
...
- Las versiones ZIP legadas redirigen a `/api/v1/packages/{name}/download?version=`.
- Usa el segmento de tasa de descarga.
...
- `packageName` se normaliza como nombre npm; el paquete puede faltar para migraciones
  planificadas.
- Esto solo hace seguimiento de la preparación de la migración. No modifica OpenClaw ni genera
  ClawPacks.
...
Reporta un paquete para revisión de moderadores. Los reportes son de nivel de paquete, opcionalmente
...
`note` es obligatorio para `confirmed` y `dismissed`; puede omitirse al
establecer `status` de vuelta en `open`. Pasa `finalAction: "quarantine"` o
`finalAction: "revoke"` con un reporte confirmado para aplicar moderación de release en el
...
Endpoint de moderador/admin para la revisión de releases de paquetes.
...
- `revoked`: bloqueado después de que una release se hubiera considerado confiable previamente.

Las releases en cuarentena y revocadas devuelven `403` desde las rutas de descarga de artefactos.
...
- Usa de forma predeterminada la release más reciente.
- Usa el bucket de límite de lectura, no el bucket de descarga.
- Los archivos binarios devuelven `415`.
- Límite de tamaño de archivo: 200 KB.
- Los análisis pendientes de VirusTotal no bloquean las lecturas; las releases maliciosas pueden seguir reteniéndose en otros lugares.
- Los paquetes privados devuelven `404` salvo que el llamador pueda leer el publicador propietario.

### `GET /api/v1/packages/{name}/download`

Descarga el archivo ZIP determinista heredado para una release de paquete.
...
- Los análisis pendientes de VirusTotal no bloquean las descargas; las releases maliciosas devuelven `403`.
...
- `dist.tarball`, `dist.integrity` y `dist.shasum` usan campos compatibles con npm
  para que los usuarios puedan apuntar npm al mirror si lo desean.
...
Transmite los bytes exactos del tarball de ClawPack cargado para clientes de mirror npm.
...
- Las transferencias de skill respaldadas por GitHub no hacen proxy ni mirror de bytes. La respuesta JSON
  incluye `sourceRef: "public-github"`, `repo`, `commit`, `path`, `contentHash`
  y `archiveUrl`; el estado de análisis/actual es una puerta y no se incluye como metadatos de payload de éxito.
...
- Campo de payload opcional: `ownerHandle`. Cuando está presente, la API resuelve ese
  publicador del lado del servidor y requiere que el actor tenga acceso al publicador.
- Campo de payload opcional: `migrateOwner`. Cuando es `true` con `ownerHandle`, una
  skill existente puede moverse a ese propietario si el actor es admin/propietario tanto en
  el publicador actual como en el destino. Sin esta aceptación explícita, los cambios de propietario se
  rechazan.

### `POST /api/v1/packages`

Publica una release de code-plugin o bundle-plugin.

- Requiere autenticación con token Bearer.
- Requiere `multipart/form-data`.
- Los campos de formulario permitidos son `payload`, blobs `files` repetidos o una referencia de tarball `clawpack`.
  `clawpack` puede ser un blob `.tgz` o un id de almacenamiento devuelto por
...
- Campo de payload opcional: `ownerHandle`. Cuando está presente, solo los admins pueden publicar en nombre de ese propietario.
...
- Las publicaciones en nombre de otra persona siguen validando la elegibilidad del canal oficial contra la cuenta del propietario de destino.

### `DELETE /api/v1/skills/{slug}` / `POST /api/v1/skills/{slug}/undelete`

Elimina de forma reversible / restaura una skill (propietario, moderador o admin).
...
Las eliminaciones reversibles iniciadas por el propietario reservan el slug durante 30 días; después otro publicador puede
reclamar el slug. La respuesta de eliminación incluye `slugReservedUntil` cuando aplica este vencimiento.
Las ocultaciones por moderador/admin y las eliminaciones de seguridad no vencen de esta forma.
...
Solo admin. Garantiza que exista un publicador de organización para un identificador. Si el identificador todavía apunta a un
usuario compartido heredado/publicador personal, el endpoint lo migra primero a un publicador de organización.
Para una organización recién creada, proporciona `memberHandle`; el admin actuante no se agrega como miembro.
...
Creación autenticada de publicador de organización por autoservicio. Crea un nuevo publicador de organización y agrega al
...
release. Los nombres de paquetes se convierten en paquetes privados de marcador de posición sin filas de release, por lo que el mismo
propietario puede publicar más tarde la release real de code-plugin o bundle-plugin en ese nombre.
...
Solo admin. Recupera un publicador personal para un principal verificado de reemplazo de OAuth de GitHub
...
los identificadores mutables solo se usan como una salvaguarda visible para el operador.

El endpoint usa de forma predeterminada el modo de ensayo. Aplicar la recuperación requiere `dryRun: false` y
`confirmIdentityVerified: true` después de que el personal verifique independientemente la continuidad entre ambos
principales de GitHub. La recuperación falla cerrada cuando el editor personal actual del usuario de destino
...
los alias de slug de Skills, los paquetes, las advertencias del inspector de paquetes y las filas derivadas de resúmenes de búsqueda para que
...
Las fuentes de Skills de GitHub tienen alcance de editor y se informan como comprobadas en lugar de reescribirse.
...
Banea a un usuario y elimina de forma definitiva las Skills de su propiedad (solo moderador/admin).
...
Revoca el baneo de un usuario y restaura las Skills elegibles (solo admin).
...
Cambia el motivo almacenado de un baneo existente sin revocar el baneo ni restaurar
contenido (solo admin). Usa el modo de ensayo de forma predeterminada a menos que `dryRun` sea `false`.
...
Aún son compatibles con versiones anteriores de la CLI:
