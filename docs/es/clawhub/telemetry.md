...
summary: Telemetría de instalación recopilada mediante `clawhub sync` + opción de exclusión voluntaria.
x-i18n:
...
ClawHub utiliza **telemetría mínima** para calcular los **recuentos de instalaciones** (lo que realmente está en uso) y para ofrecer una mejor ordenación/filtrado.
...
- Has **iniciado sesión** en la CLI (ya requerimos autenticación para los flujos de sync/publish).
...
- `label`: una etiqueta legible por humanos derivada de los dos últimos segmentos de la ruta (las rutas de inicio se muestran con `~`).
...
- Ninguna ruta absoluta de carpeta sin procesar (solo `rootId` con hash + una etiqueta de visualización corta).
- Ningún contenido de archivo.
- Ningún registro por ejecución, prompt ni otra salida de la CLI.
- Ningún rastreo para skills que no se hayan subido al registro (los slugs desconocidos se ignoran).
...
- `installsAllTime`: usuarios únicos que alguna vez informaron que tenían el skill instalado.
...
ClawHub proporciona una pestaña privada “Instalados” en tu propio perfil:

- Muestra las raíces exactas + los skills instalados que almacenamos.
- Incluye una vista de **exportación JSON**.
- Incluye una acción **Eliminar telemetría** para quitar toda la telemetría almacenada de tu cuenta.
