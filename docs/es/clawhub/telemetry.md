...
Esto se basa en el comando de CLI `clawhub sync`.
...
- Ningún registro por ejecución, prompts ni otra salida de CLI.
- Ningún rastreo para skills que no estén subidas al registro (los slugs desconocidos se ignoran).
...
- `installsAllTime`: usuarios únicos que alguna vez han informado que tienen la skill instalada.

### Varias raíces

Si sincronizas desde varias carpetas, tratamos cada raíz de escaneo de forma independiente. Una skill está “actualmente instalada” si existe en **cualquier** raíz activa.
...
Todos los demás solo ven **contadores agregados de instalaciones**; nadie más puede ver tus raíces/carpetas.
