...
ClawHub usa **telemetría mínima** para calcular **recuentos de instalaciones** (lo que realmente está en uso) y permitir una mejor ordenación/filtrado.
...
En cada `clawhub sync`, la CLI informa una **instantánea completa** de lo que encontró, agrupada por raíz de análisis (“carpeta/raíz”).
...
- Ninguna ruta de carpeta absoluta sin procesar (solo `rootId` con hash + una etiqueta de visualización corta).
- Ningún contenido de archivos.
- Ningún registro por ejecución, prompts u otra salida de la CLI.
- Ningún seguimiento de skills que no estén subidas al registro (los slugs desconocidos se ignoran).
...
Si sincronizas desde varias carpetas, tratamos cada raíz de análisis de forma independiente. Una skill está “actualmente instalada” si existe en **cualquier** raíz activa.
...
Los demás solo ven **contadores de instalaciones agregados**; nadie más puede ver tus raíces/carpetas.
