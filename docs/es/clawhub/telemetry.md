...
ClawHub usa **telemetría mínima** para calcular **conteos de instalaciones** (lo que realmente está en uso) y para impulsar una mejor ordenación/filtrado.
...
- `label`: una etiqueta legible por humanos derivada de los dos últimos segmentos de ruta (las rutas de inicio se muestran con `~`).
...
- No rutas de carpetas absolutas sin procesar (solo `rootId` con hash + una etiqueta breve de visualización).
- No contenidos de archivos.
- No registros por ejecución, prompts ni otra salida de la CLI.
- No seguimiento de skills que no se hayan subido al registro (los slugs desconocidos se ignoran).
...
- `installsAllTime`: usuarios únicos que alguna vez han informado que la skill está instalada.
