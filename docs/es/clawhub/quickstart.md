---
read_when:
    - Primer uso de ClawHub
    - Instalación de una habilidad o Plugin desde el registro
    - Publicar en ClawHub
summary: 'Comienza a usar ClawHub: busca, instala, actualiza y publica skills o plugins.'
x-i18n:
    generated_at: "2026-07-02T22:22:01Z"
    model: gpt-5.5
...
Los entornos sin interfaz pueden usar un token de API desde la interfaz web de
ClawHub:
...
El comando omite el contenido sin cambios. Las Skills nuevas empiezan en
...
Para repositorios que contienen varias Skills, el flujo de trabajo reutilizable
de GitHub llama a `skill publish` para cada carpeta de Skill inmediata bajo
`skills/`:
...
Usa primero `--dry-run` para previsualizar los metadatos del paquete resuelto,
...
Los listados públicos muestran el estado de escaneo más reciente. Las versiones
retenidas o bloqueadas por moderación pueden permanecer ocultas de las
