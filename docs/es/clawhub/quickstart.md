...
OpenClaw registra de dónde provino la Skills para que las actualizaciones posteriores puedan seguir
...
Una Skills es una carpeta con un archivo `SKILL.md` obligatorio y archivos de apoyo
opcionales.
...
Skills antes de instalarla. Consulta [Formato de Skills](/es/clawhub/skill-format).

Para repositorios que contienen varias Skills, el flujo de trabajo reutilizable de GitHub llama a
`skill publish` para cada carpeta inmediata de Skills dentro de `skills/`:
...
la atribución de la fuente y el plan de carga sin publicar.
