...
ClawHub es un registro para habilidades y plugins de OpenClaw.

Usa OpenClaw cuando estés instalando cosas en OpenClaw. Usa la CLI `clawhub`
cuando estés iniciando sesión, publicando, gestionando tus propios listados o usando
...
OpenClaw registra de dónde provino la habilidad para que las actualizaciones posteriores puedan seguir
...
Una habilidad es una carpeta con un archivo `SKILL.md` obligatorio y archivos de apoyo
opcionales.
...
El comando omite el contenido sin cambios. Las habilidades nuevas empiezan en `1.0.0`; los cambios posteriores
...
entorno, herramientas y permisos requeridos para que los usuarios puedan entender qué
necesita la habilidad antes de instalarla. Consulta [Formato de habilidad](/es/clawhub/skill-format).

Para repositorios que contienen varias habilidades, el flujo de trabajo reutilizable de GitHub llama a
`skill publish` para cada carpeta de habilidad inmediata bajo `skills/`:
...
Usa `--dry-run` primero para previsualizar los metadatos del paquete resuelto, los campos de compatibilidad,
...
moderación pueden quedar ocultas de las superficies de búsqueda e instalación hasta que se resuelvan.
