...
x-i18n:
    generated_at: "2026-07-05T01:54:20Z"
    model: gpt-5.5
...
Una skill es una carpeta con un archivo `SKILL.md` requerido y archivos de apoyo
opcionales.
...
Usa `--dry-run` para previsualizar o `--version` para elegir una versión
explícita.
...
de GitHub llama a `skill publish` para cada carpeta inmediata de skill dentro de `skills/`:
...
`package.json`, incluidos `openclaw.compat.pluginApi` y `openclaw.build.openclawVersion`.
...
Los listados públicos muestran el estado del análisis más reciente. Las versiones
