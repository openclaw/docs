...
para HTTP sin formato. `NO_PROXY` / `no_proxy` se respeta para omitir el proxy en
...
- Añade o elimina una Skill de tus destacados.
...
- La búsqueda favorece coincidencias exactas de tokens de slug/nombre antes que la popularidad por descargas. Un token de slug independiente como `map` coincide con `personal-map` con más fuerza que la subcadena dentro de `amap`.
- Las descargas son una pequeña señal previa de popularidad, no una garantía de posición superior.
...
- Marca una Skill instalada como anclada en el lockfile.
...
- Las Skills ancladas también rechazan `install --force`, para que los bytes locales no puedan sustituirse por accidente.

### `unpin <slug>`

- Elimina del lockfile el anclaje de una Skill instalada para que futuras actualizaciones puedan modificarla.

### `update [slug]` / `update --all`

- Calcula la huella a partir de los archivos locales.
- Si la huella coincide con una versión conocida: no muestra ningún prompt.
...
- `update <slug>` falla rápido para los slugs anclados y te indica que ejecutes primero `clawhub unpin <slug>`.
...
- Cambia el nombre de una Skill de tu propiedad y conserva el slug anterior como alias de redirección.
- Llama a `POST /api/v1/skills/{slug}/rename`.
- `--yes` omite la confirmación.

### `skill merge <source-slug> <target-slug>`

- Fusiona una Skill de tu propiedad con otra Skill de tu propiedad.
...
- Las transferencias a identificadores de org/publicador se aplican inmediatamente solo cuando el actor tiene
...
- Restaura un paquete y sus versiones eliminados de forma reversible.
- Requiere el propietario del paquete, un propietario/administrador de publicador de organización, un moderador de la plataforma
  o un administrador de la plataforma.
...
- Los reportes se aplican a nivel de paquete, opcionalmente vinculados a una versión, y pasan a ser visibles
...
- Reporta bloqueos para el estado oficial, la disponibilidad de ClawPack, el resumen del artefacto,
