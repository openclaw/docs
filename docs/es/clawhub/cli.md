...
x-i18n:
    generated_at: "2026-07-02T17:29:55Z"
    model: gpt-5.5
...
- Reserva heredada: si `clawhub/config.json` aún no existe pero `clawdhub/config.json` sí existe, la CLI reutiliza la ruta heredada
- sobrescritura: `CLAWHUB_CONFIG_PATH` (`CLAWDHUB_CONFIG_PATH` heredado)
...
- La popularidad es un pequeño factor previo de clasificación, no una garantía de aparecer en primer lugar.
- Si una skill debería aparecer pero no aparece, ejecuta `clawhub inspect @owner/slug` con la sesión iniciada para revisar los diagnósticos de moderación visibles para el propietario antes de renombrar metadatos.

### `explore`

- Lista las skills más nuevas mediante `/api/v1/skills?limit=...&sort=createdAt` (ordenadas por `createdAt` desc).
- Marcas:
...
- Envía telemetría de mejor esfuerzo con la sesión iniciada para que los conteos actuales de instalación puedan
  desactivarse.
...
- Las skills fijadas se omiten con `update --all` y se rechazan con `update <skill>` directo.
...
- Las skills nuevas usan `1.0.0` de forma predeterminada; las skills modificadas usan de forma predeterminada la siguiente versión
  de parche.
...
- Escanea el workdir actual, el directorio de skills configurado y cualquier
  ...
  `skill.md`.
- Compara la huella digital de cada skill local con ClawHub y publica solo skills nuevas o
  modificadas.
- Las skills nuevas se publican como `1.0.0`; las skills modificadas publican la siguiente versión de parche
  de forma predeterminada. Usa `--bump minor|major` para lotes de actualización que deban avanzar por un
  paso semver mayor.
...
- Ejecuta ClawHub ClawScan mediante `POST /api/v1/skills/-/scan`, luego consulta hasta que el escaneo sea terminal.
- Los escaneos son asíncronos y pueden tardar en completarse. Mientras están en cola, el indicador de la terminal muestra la posición actual priorizada del escaneo y cuántos escaneos hay por delante.
- Los escaneos publicados requieren propiedad o acceso de administración de publicador. Los moderadores/administradores pueden usar el mismo backend mediante `clawhub-admin`.
...
- Los escaneos de rutas locales ya no son compatibles. Sube una versión nueva y luego usa `scan download` para recuperar los resultados de escaneo almacenados para esa versión enviada.
...
- Las descargas de skills usan el slug de la skill y tienen como predeterminado `--kind skill`.
...
- La publicación de skills V1 usa `clawhub_token`; la publicación confiable con OIDC de GitHub es solo para paquetes por ahora.
...
- `--version <version>` elimina permanentemente una versión propia que no sea la más reciente mediante una ruta de cierre seguro ante fallos
...
- `--reason <text>` registra una nota de moderación en una eliminación reversible de todo el skill y en el registro de auditoría.
...
- No existe recuperación de versiones; las versiones eliminadas permanentemente no se pueden restaurar.
...
- Muestra de nuevo un skill (propietario, moderador o administrador).
- Alias de `undelete`.
...
- Descarga el artefacto desde el `downloadUrl` del resolver.
...
- Con `--package`, resuelve los metadatos esperados desde ClawHub y compara el
  archivo local con los metadatos publicados del artefacto.
- Con opciones directas de resumen, verifica sin una consulta de red.
- Opciones:
...
- Opciones:
  ...
  - `--runtime`: habilita la captura en runtime; importa código del Plugin.
  - `--allow-execute`: permite la captura en runtime en un espacio de trabajo aislado.
  - `--no-mock-sdk`: deshabilita el SDK simulado de OpenClaw durante la captura en runtime.
  - `--json`: salida legible por máquina.
...
- `--version <version>` elimina permanentemente una versión propia que no sea la más reciente mediante una ruta de cierre seguro ante fallos
  específica de la versión.
  Las versiones eliminadas no se pueden restaurar ni volver a publicar. Publica un reemplazo antes de eliminar la
  versión actual más reciente. Este flujo solo de versión requiere el propietario del paquete o un administrador
  publicador de la organización; el personal de la plataforma no omite la propiedad del paquete.
- La eliminación reversible de todo el paquete requiere el propietario del paquete, un propietario/administrador publicador de la organización, un moderador
...
- No existe recuperación de versiones; las versiones eliminadas permanentemente no se pueden restaurar.
- Requiere el propietario del paquete, un propietario/administrador publicador de la organización, un moderador de la plataforma
...
- Requiere acceso de administrador tanto al propietario actual del paquete como al publicador de destino,
  a menos que lo realice un administrador de la plataforma.
- Los nombres de paquetes con scope deben transferirse al propietario del scope correspondiente.
...
- Los reportes son a nivel de paquete, opcionalmente vinculados a una versión, y se vuelven visibles
  para que los moderadores los revisen.
- Los reportes no ocultan automáticamente paquetes ni bloquean descargas por sí mismos.
- Opciones:
...
- Llama al mismo endpoint de readiness calculado que `package readiness`, pero imprime
...
- Falla si el identificador ya está usado por un publicador, usuario o ruta reservada existente.
...
- Los metadatos se detectan automáticamente desde `package.json`, `openclaw.plugin.json` y
  marcadores reales de paquetes OpenClaw como `.codex-plugin/plugin.json`,
  `.claude-plugin/plugin.json` y `.cursor-plugin/plugin.json`.
- Las fuentes `.tgz` se tratan como ClawPack. La CLI carga exactamente los bytes
...
- Los Plugins de código externos deben declarar `openclaw.compat.pluginApi` y
  ...
  La `package.json.version` de nivel superior no se usa como alternativa para la validación de publicación.
...
Para Plugins de código, la publicación de carpeta construye y carga un artefacto ClawPack desde
...
- Si la validación informa de un código de remediación, consulta
...
- La publicación de confianza sin secreto solo funciona en `workflow_dispatch`; los pushes de etiquetas aún necesitan `clawhub_token`.
...
- Úsalo después de configurar para confirmar el repositorio, el nombre del archivo de flujo de trabajo
  y el pin opcional del entorno.
- Opciones:
  - `--json`: salida legible por máquina.
...
- Adjunta o reemplaza la configuración de publicador de confianza de GitHub Actions para un
  paquete existente.
- El paquete debe crearse primero mediante el flujo normal manual o autenticado con token de
  `clawhub package publish`.
- Después de establecer la configuración, las futuras publicaciones compatibles desde GitHub Actions pueden usar
  publicación OIDC/de confianza sin un token de ClawHub de larga duración.
...
- `--workflow-filename <file>` debe coincidir con el nombre del archivo de flujo de trabajo en
  `.github/workflows/`.
- `--environment <name>` es opcional. Cuando se configure, el entorno de GitHub Actions
...
- ClawHub verifica el repositorio de GitHub configurado cuando se ejecuta este comando.
  ...
  ejemplo mediante una futura instalación de ClawHub GitHub App u otra integración de
  GitHub autorizada.
- Opciones:
...
- Úsalo como reversión si el pin del flujo de trabajo, repositorio o entorno debe
...
- El reporte se realiza en modo de mejor esfuerzo. Los comandos de instalación no fallan si la telemetría no está
  disponible.
