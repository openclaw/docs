...
read_when:
    ...
    - Solución de problemas de instalación, actualización, publicación o sincronización
...
- Las descargas son un pequeño indicador previo de popularidad, no una garantía de aparecer primero.
- Si una skill debería aparecer pero no lo hace, ejecuta `clawhub inspect <slug>` con la sesión iniciada para revisar los diagnósticos de moderación visibles para el propietario antes de cambiar el nombre de los metadatos.
...
- Lee `<workdir>/.clawhub/lock.json` (legacy `.clawdhub`).
- Muestra `pinned` junto a las Skills congeladas con `clawhub pin`, incluido el motivo opcional.

### `pin <slug>`

- Marca una Skill instalada como anclada en el archivo de bloqueo.
- `--reason <text>` registra por qué la Skill está congelada.
- Las Skills ancladas se omiten con `update --all` y se rechazan con `update <slug>` directo.
- Las Skills ancladas también rechazan `install --force` para que los bytes locales no puedan reemplazarse accidentalmente.

### `unpin <slug>`

- Elimina del archivo de bloqueo el anclaje de una Skill instalada para que las actualizaciones futuras puedan modificarla.

### `update [slug]` / `update --all`

- Calcula la huella digital a partir de los archivos locales.
- Si la huella digital coincide con una versión conocida: no hay solicitud.
- Si la huella digital no coincide:
  - se rechaza de forma predeterminada
  - se sobrescribe con `--force` (o con solicitud, si es interactivo)
- Las Skills ancladas nunca se actualizan con `--force`.
- `update <slug>` falla rápidamente para slugs anclados y te indica que ejecutes primero `clawhub unpin <slug>`.
- `update --all` omite los slugs anclados e imprime un resumen de lo que permaneció congelado.
...
- Elimina una Skill de forma reversible (propietario, moderador o administrador).
...
- Usa esto para metadatos, compatibilidad, verificación, origen e inspección de versión/archivo de plugins.
- `--version <version>`: inspecciona una versión específica (predeterminado: la más reciente).
...
- Las denuncias son a nivel de paquete, pueden vincularse opcionalmente a una versión y quedan visibles
...
- Muestra el estado actual del escaneo del paquete, el número de denuncias abiertas, el estado de moderación manual de la versión más reciente,
...
- Informa de bloqueadores para el estado oficial, disponibilidad de ClawPack, resumen del artefacto,
  procedencia del código fuente, compatibilidad con OpenClaw, destinos de host, metadatos del entorno
  y estado de escaneo.
- Indicadores:
  - `--json`: salida legible por máquina.
...
- Las carpetas de plugins de código se empaquetan en un tarball npm de ClawPack antes de subirse para que
...
- Para fuentes de GitHub, la atribución de código fuente se completa automáticamente a partir del repositorio, commit resuelto, ref y subruta.
...
- `--clawscan-note <text>` añade una nota de ClawScan. Esta nota proporciona a ClawScan
  contexto para comportamientos que de otro modo podrían parecer inusuales, como acceso a la red,
  acceso nativo al host o credenciales específicas del proveedor. La nota se almacena en
  la versión publicada.
- Los nombres de paquetes con ámbito deben coincidir con el propietario seleccionado. Consulta `docs/publishing.md`.
- Los indicadores existentes (`--family`, `--name`, `--version`, `--source-repo`, `--source-commit`, `--source-ref`, `--source-path`) siguen funcionando como sobrescrituras.
...
- El flujo de trabajo reutilizable usa de forma predeterminada `source` con el repositorio llamador.
...
- La publicación de confianza sin secreto solo funciona en `workflow_dispatch`; los envíos de etiquetas aún necesitan `clawhub_token`.
...
- Se envía durante `sync` cuando la sesión está iniciada, salvo que `CLAWHUB_DISABLE_TELEMETRY=1` (`CLAWDHUB_DISABLE_TELEMETRY=1` heredado).
