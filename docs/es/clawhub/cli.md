...
Cuando cualquiera de estas variables está definida, la CLI enruta las solicitudes salientes a través de
...
- Predeterminado: abre el navegador en `<site>/cli/auth` y completa mediante devolución de llamada local loopback.
...
- Las descargas son un pequeño indicador previo de popularidad, no una garantía de aparecer en las primeras posiciones.
...
- Obtiene metadatos de Skill y archivos de versión sin instalar.
...
- Si la huella coincide con una versión conocida: no hay aviso.
...
- Fusiona una skill propia en otra skill propia.
...
- Las denuncias se aplican a nivel de paquete, pueden vincularse opcionalmente a una versión y pasan a ser visibles
...
- Muestra el estado actual de escaneo del paquete, el número de denuncias abiertas, el estado de moderación manual
...
- Informa de bloqueos relacionados con el estado oficial, la disponibilidad de ClawPack, el resumen del artefacto,
  la procedencia del código fuente, la compatibilidad con OpenClaw, los objetivos de host, los metadatos del entorno
...
- Las fuentes `.tgz` se tratan como ClawPack. La CLI sube los bytes exactos de npm-pack
  ...
  relleno previo de metadatos.
- Las carpetas de Plugin de código se empaquetan en un tarball npm de ClawPack antes de la subida para que
  las instalaciones de OpenClaw puedan verificar el artefacto exacto. Las carpetas de Plugin de bundle todavía
  usan la ruta de publicación de archivos extraídos.
- Para fuentes de GitHub, la atribución de código fuente se rellena automáticamente desde el repositorio, el commit resuelto, la ref y la subruta.
...
- Los Plugins de código externos deben declarar `openclaw.compat.pluginApi` y
  ...
  `package.json.version` de nivel superior no se usa como valor de reserva para la validación de publicación.
...
Usa primero `--dry-run` para poder confirmar los metadatos del paquete resueltos y
la atribución de código fuente antes de crear una versión activa:
...
- `package.json.version` es la versión de publicación de tu paquete, pero no se usa como
  valor de reserva para la validación de compatibilidad/compilación de OpenClaw.
...
- El workflow reutilizable usa el repositorio llamador como valor predeterminado de `source`.
- Para monorepos, pasa `source_path` para que el workflow publique la carpeta del paquete
  de Plugin, por ejemplo `source_path: extensions/codex`.
...
- Indicadores:
  ...
  - `--bump patch|minor|major` (valor predeterminado: patch)
  - `--changelog <text>` (no interactivo)
  - `--tags a,b,c` (valor predeterminado: latest)
  - `--concurrency <n>` (valor predeterminado: 4)
