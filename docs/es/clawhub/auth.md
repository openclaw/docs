...
x-i18n:
    generated_at: "2026-07-19T01:48:38Z"
    model: gpt-5.6
...
Si al iniciar sesión se vuelve a un estado sin sesión iniciada, es posible que la cuenta no esté en
regla. Si la cuenta fue bloqueada o deshabilitada, use el
[formulario de apelación de ClawHub](https://appeals.openclaw.ai/) si considera que se trata de un
error.
...
Si el navegador no puede acceder a la devolución de llamada local debido a reglas del firewall, la VPN o
...
Para shells remotos en los que se pueda abrir un navegador en otro lugar, ejecute:
...
Sobrescriba la ruta con:

```bash
export CLAWHUB_CONFIG_PATH=/path/to/config.json
```

Muestre el token almacenado para configurar la CI con:
...
Si la cuenta fue bloqueada o deshabilitada, use el
[formulario de apelación de ClawHub](https://appeals.openclaw.ai/) si considera que se trata de un
error.
