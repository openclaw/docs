...
summary: Inicio de sesión de ClawHub, tokens de API, inicio de sesión en la CLI, almacenamiento de tokens y revocación.
x-i18n:
...
Si el inicio de sesión te devuelve a un estado de sesión cerrada, es posible que tu cuenta no esté en buen
...
3. Tras iniciar sesión con GitHub, ClawHub crea un token de API.
...
Si tu navegador no puede llegar a la devolución de llamada local por reglas de firewall, VPN o
...
Usa este flujo para servidores, trabajos de CI o entornos solo con terminal.
...
Los tokens revocados, no válidos o ausentes devuelven `401 Unauthorized`. Vuelve a iniciar sesión
