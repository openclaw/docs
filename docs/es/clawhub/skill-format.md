...
- cualquier archivo auxiliar _basado en texto_ (consulte «Archivos permitidos»)
- `.clawhubignore` (patrones de exclusión para la publicación; formato heredado `.clawdhubignore`)
...
- `description` se usa como resumen de la skill en la interfaz y en las búsquedas.

Para que las Agent Skills sean portátiles, `name` debe coincidir con el directorio principal y usar
...
siendo publicables y no se reescriben de forma silenciosa. Las listas del catálogo pueden acortar visualmente los nombres largos
...
Use `requires.env` para las variables de entorno que deben estar presentes antes de que la skill pueda ejecutarse. Use `envVars` cuando necesite metadatos para cada variable, incluidas variables opcionales con `required: false`.
...
| `primaryEnv`       | `string`   | Variable de entorno principal que contiene la credencial de la skill.                                                                        |
...
| `always`           | `boolean`  | Si es `true`, la skill está siempre activa (no es necesaria una instalación explícita).                                                       |
  ...
  openclaw:
    ...
    envVars:
      ...
      - name: TODOIST_PROJECT_ID
        required: false
        description: ID opcional del proyecto predeterminado cuando el usuario no especifica uno.
```

### Por qué es importante

El análisis de seguridad de ClawHub comprueba que lo declarado por la skill coincida con lo que realmente hace. Si el código hace referencia a `TODOIST_API_KEY`, pero el frontmatter no la declara en `requires.env`, `primaryEnv` o `envVars`, el análisis marcará una discrepancia en los metadatos. Mantener las declaraciones precisas ayuda a que la skill supere la revisión y a que los usuarios comprendan qué están instalando.
...
metadata:
  openclaw:
    ...
    envVars:
      ...
      - name: TODOIST_PROJECT_ID
        required: false
        description: ID opcional del proyecto predeterminado.
    emoji: "\u2705"
...
- Los archivos de scripts se siguen analizando después de cargarlos; los archivos de PowerShell `.ps1`, `.psm1` y `.psd1` se aceptan como texto.
- Los tipos de contenido que comienzan por `text/` se tratan como texto, junto con una pequeña lista de tipos permitidos (JSON/YAML/TOML/JS/TS/Markdown/SVG).

Límites (del lado del servidor):

- Tamaño total del paquete: 50MB.
- El texto de las incrustaciones incluye `SKILL.md` y hasta aproximadamente 40 archivos que no sean `.md` (límite sujeto al mejor esfuerzo).

## Slugs

- De forma predeterminada, se derivan del nombre de la carpeta.
- Los ámbitos de los paquetes deben coincidir exactamente con el identificador del editor de ClawHub. Los identificadores de los editores pueden usar letras minúsculas, números, guiones, puntos y guiones bajos; deben comenzar y terminar con una letra minúscula o un número.
...
- Todas las skills publicadas en ClawHub tienen la licencia `MIT-0`.
...
- No añada condiciones de licencia incompatibles en `SKILL.md`; ClawHub no admite sobrescrituras de licencia para cada skill.

## Skills de pago

- ClawHub no admite skills de pago, precios por skill, muros de pago ni reparto de ingresos.
- No añada metadatos de precios a `SKILL.md`; no forman parte del formato de la skill y no harán que una skill publicada sea de pago.
- Si la skill se integra con un servicio de terceros de pago, documente claramente el coste externo y la cuenta necesaria en las instrucciones de la skill y en las declaraciones de entorno (`requires.env` para las variables obligatorias o `envVars` con `required: false` para las variables opcionales).
