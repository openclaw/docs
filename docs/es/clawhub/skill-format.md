...
x-i18n:
    generated_at: "2026-07-03T00:51:52Z"
    model: gpt-5.5
...
archivos `SKILL.md` o el heredado `skills.md` en repositorios públicos, no bifurcados, propiedad de
...
Declara variables de entorno opcionales en `metadata.openclaw.envVars` y define `required: false`. No agregues entradas opcionales a `requires.env`, porque `requires.env` significa que la Skill no puede ejecutarse sin ellas.
...
El análisis de seguridad de ClawHub comprueba que lo que declara tu Skill coincida con lo que realmente hace. Si tu código hace referencia a `TODOIST_API_KEY` pero tu frontmatter no lo declara en `requires.env`, `primaryEnv` o `envVars`, el análisis marcará una discrepancia de metadatos. Mantener las declaraciones precisas ayuda a que tu Skill pase la revisión y ayuda a los usuarios a entender qué están instalando.
...
- Los ámbitos de paquete deben coincidir exactamente con el identificador de editor de ClawHub. Los identificadores de editor pueden usar letras minúsculas, números, guiones, puntos y guiones bajos; deben empezar y terminar con una letra minúscula o un número.
...
- Cualquier persona puede usar, modificar y redistribuir Skills publicadas, incluso con fines comerciales.
...
- Si tu Skill se integra con un servicio de terceros de pago, documenta claramente el costo externo y la cuenta requerida en las instrucciones de la Skill y en las declaraciones de env (`requires.env` para variables obligatorias, o `envVars` con `required: false` para variables opcionales).
