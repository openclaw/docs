...
x-i18n:
    generated_at: "2026-07-01T05:28:13Z"
    model: gpt-5.5
...
- `.clawhubignore` (patrones de exclusión para la publicación, legado `.clawdhubignore`)
...
archivos `SKILL.md` o el legado `skills.md` en repositorios públicos que no sean forks y pertenezcan a
...
| `requires.anyBins` | `string[]` | Binarios de CLI de los que debe existir al menos uno.                                                                                                    |
...
| `always`           | `boolean`  | Si es `true`, la skill está siempre activa (no se necesita instalación explícita).                                                                        |
...
- Los archivos de script se analizan de todos modos después de la carga; los archivos PowerShell `.ps1`, `.psm1` y `.psd1` se aceptan como texto.
...
- Las etiquetas son punteros de cadena a una versión; se usa comúnmente `latest`.
