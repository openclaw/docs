...
x-i18n:
    generated_at: "2026-07-02T13:57:55Z"
    model: gpt-5.5
...
Declara las variables de entorno opcionales en `metadata.openclaw.envVars` y define `required: false`. No agregues entradas opcionales a `requires.env`, porque `requires.env` significa que la Skill no puede ejecutarse sin ellas.
...
El análisis de seguridad de ClawHub comprueba que lo que declara tu Skill coincida con lo que realmente hace. Si tu código hace referencia a `TODOIST_API_KEY` pero tu frontmatter no la declara en `requires.env`, `primaryEnv` ni `envVars`, el análisis marcará una discrepancia de metadatos. Mantener declaraciones precisas ayuda a que tu Skill pase la revisión y ayuda a los usuarios a entender qué están instalando.
...
- Los archivos de script se siguen analizando después de la carga; los archivos de PowerShell `.ps1`, `.psm1` y `.psd1` se aceptan como texto.
