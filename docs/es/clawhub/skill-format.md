...
- `.clawhubignore` (patrones de exclusión para publicación, legado `.clawdhubignore`)
...
| `homepage`         | `string`   | URL a la página principal o documentación de la habilidad.                                                                                   |
...
Declara variables de entorno opcionales bajo `metadata.openclaw.envVars` y define `required: false`. No agregues entradas opcionales a `requires.env`, porque `requires.env` significa que la habilidad no puede ejecutarse sin ellas.
...
El análisis de seguridad de ClawHub comprueba que lo que tu habilidad declara coincida con lo que realmente hace. Si tu código referencia `TODOIST_API_KEY` pero tu frontmatter no lo declara bajo `requires.env`, `primaryEnv` o `envVars`, el análisis marcará una discrepancia de metadatos. Mantener declaraciones precisas ayuda a que tu habilidad pase la revisión y ayuda a los usuarios a entender qué están instalando.
...
- Los archivos de script aún se escanean después de la carga; los archivos PowerShell `.ps1`, `.psm1` y `.psd1` se aceptan como texto.
...
- No agregues términos de licencia contradictorios en `SKILL.md`; ClawHub no admite anulaciones de licencia por habilidad.
