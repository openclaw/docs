...
x-i18n:
    generated_at: "2026-06-30T22:05:10Z"
    model: gpt-5.5
...
archivos `SKILL.md` o el heredado `skills.md` en repositorios públicos, que no sean bifurcaciones, propiedad de
...
Los metadatos de la habilidad se declaran en el frontmatter YAML en la parte superior de tu `SKILL.md`. Esto indica al registro (y al análisis de seguridad) qué necesita tu habilidad para ejecutarse.
...
| `always`           | `boolean`  | Si es `true`, la habilidad siempre está activa (no hace falta instalación explícita).                                                        |
| `skillKey`         | `string`   | Anula la clave de invocación de la habilidad.                                                                                                |
...
- El texto de incrustación incluye `SKILL.md` + hasta ~40 archivos que no sean `.md` (límite de máximo esfuerzo).
...
- No agregues términos de licencia en conflicto en `SKILL.md`; ClawHub no admite anulaciones de licencia por habilidad.
