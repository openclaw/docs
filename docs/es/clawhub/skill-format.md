...
- `.clawhubignore` (patrones de omisión para publicar/sincronizar, legado `.clawdhubignore`)
...
Estado de instalación de workdir (escrito por la CLI):
...
| `requires.anyBins` | `string[]` | Binarios de CLI de los que al menos uno debe existir.                                                                                                           |
...
El análisis de seguridad de ClawHub comprueba que lo que declara tu Skill coincida con lo que realmente hace. Si tu código hace referencia a `TODOIST_API_KEY` pero tu frontmatter no lo declara bajo `requires.env`, `primaryEnv` o `envVars`, el análisis marcará una discrepancia de metadatos. Mantener las declaraciones precisas ayuda a que tu Skill pase la revisión y ayuda a los usuarios a entender qué están instalando.
...
Publicar solo acepta archivos “basados en texto”.
...
- Los tipos de contenido que empiezan con `text/` se tratan como texto; además, una pequeña lista permitida (JSON/YAML/TOML/JS/TS/Markdown/SVG).
...
- No agregues términos de licencia contradictorios en `SKILL.md`; ClawHub no admite anulaciones de licencia por Skill.
...
- Si tu Skill se integra con un servicio externo de pago, documenta claramente el costo externo y la cuenta requerida en las instrucciones de la Skill y en las declaraciones de entorno (`requires.env` para variables requeridas, o `envVars` con `required: false` para variables opcionales).
