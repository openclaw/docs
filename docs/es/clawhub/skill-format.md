...
x-i18n:
    generated_at: "2026-07-16T11:30:25Z"
    model: gpt-5.6
...
archivos `SKILL.md` o archivos heredados `skills.md` en repositorios públicos que no sean bifurcaciones y que pertenezcan a
...
Para las Agent Skills portátiles, `name` debe coincidir con el directorio superior y utilizar
...
pudiéndose publicar y no se reescriben silenciosamente. Las listas del catálogo pueden abreviar visualmente los nombres largos
...
| `primaryEnv`       | `string`   | Variable de entorno principal para las credenciales de la Skill.                                                                                                  |
...
| `homepage`         | `string`   | URL de la página principal o de la documentación de la Skill.                                                                                                         |
| `os`               | `string[]` | Restricciones del sistema operativo (p. ej., `["macos"]`, `["linux"]`).                                                                                             |
| `install`          | `array`    | Especificaciones de instalación de las dependencias (véase más adelante).                                                                                                  |
...
El análisis de seguridad de ClawHub comprueba que lo declarado por la Skill coincida con lo que realmente hace. Si el código hace referencia a `TODOIST_API_KEY`, pero el frontmatter no lo declara en `requires.env`, `primaryEnv` o `envVars`, el análisis marcará una discrepancia en los metadatos. Mantener las declaraciones exactas ayuda a que la Skill supere la revisión y permite que los usuarios comprendan lo que están instalando.
...
- Los tipos de contenido que comienzan por `text/` se consideran texto, además de una pequeña lista permitida (JSON/YAML/TOML/JS/TS/Markdown/SVG).

Límites (del servidor):

- Tamaño total del paquete: 50MB.
- El texto de incrustación incluye `SKILL.md` y hasta unos 40 archivos que no sean `.md` (límite aproximado sujeto a disponibilidad).

## Slugs

- De forma predeterminada, se derivan del nombre de la carpeta.
- Los ámbitos del paquete deben coincidir exactamente con el identificador del publicador de ClawHub. Los identificadores de publicador pueden utilizar letras minúsculas, números, guiones, puntos y guiones bajos; deben comenzar y terminar con una letra minúscula o un número.
...
- Todas las Skills publicadas en ClawHub se ofrecen bajo la licencia `MIT-0`.
...
- No añada términos de licencia incompatibles en `SKILL.md`; ClawHub no admite anulaciones de licencia para cada Skill.
...
- Si la Skill se integra con un servicio de terceros de pago, documente claramente el coste externo y la cuenta necesaria en las instrucciones de la Skill y las declaraciones de entorno (`requires.env` para las variables obligatorias, o `envVars` con `required: false` para las variables opcionales).
