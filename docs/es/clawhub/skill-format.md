...
x-i18n:
    generated_at: "2026-07-05T11:08:36Z"
    model: gpt-5.5
...
- `.clawhubignore` (patrones de exclusión para la publicación, `.clawdhubignore` heredado)
...
archivos `SKILL.md` o `skills.md` heredados en repositorios públicos, que no sean bifurcaciones, propiedad de
la cuenta de GitHub con sesión iniciada. No importa repositorios privados, bifurcaciones,
...
| `install`          | `array`    | Especificaciones de instalación para dependencias (ver más abajo).                                                                           |
...
Declara las variables de entorno opcionales bajo `metadata.openclaw.envVars` y define `required: false`. No agregues entradas opcionales a `requires.env`, porque `requires.env` significa que la habilidad no puede ejecutarse sin ellas.
...
El análisis de seguridad de ClawHub comprueba que lo que declara tu habilidad coincida con lo que realmente hace. Si tu código hace referencia a `TODOIST_API_KEY` pero tu frontmatter no lo declara bajo `requires.env`, `primaryEnv` o `envVars`, el análisis marcará una discrepancia de metadatos. Mantener declaraciones precisas ayuda a que tu habilidad supere la revisión y ayuda a los usuarios a entender qué están instalando.
...
- Los archivos de script se analizan igualmente después de la carga; los archivos PowerShell `.ps1`, `.psm1` y `.psd1` se aceptan como texto.
...
- ClawHub no admite habilidades pagas, precios por habilidad, muros de pago ni reparto de ingresos.
- No agregues metadatos de precios a `SKILL.md`; no forman parte del formato de habilidad y no harán que una habilidad publicada sea paga.
- Si tu habilidad se integra con un servicio de terceros pago, documenta claramente el costo externo y la cuenta requerida en las instrucciones de la habilidad y en las declaraciones de entorno (`requires.env` para variables obligatorias, o `envVars` con `required: false` para variables opcionales).
