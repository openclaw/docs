...
- `.clawhubignore` (patrones de ignorados para publicación/sincronización, `.clawdhubignore` heredado)
...
| `homepage`         | `string`   | URL de la página principal o la documentación del Skill.                                                                                            |
...
Si tu Skill necesita que se instalen dependencias, decláralas en el array `install`:
...
Declara las variables de entorno opcionales en `metadata.openclaw.envVars` y establece `required: false`. No agregues entradas opcionales a `requires.env`, porque `requires.env` significa que el Skill no puede ejecutarse sin ellas.
...
Publicación solo acepta archivos “basados en texto”.

- La lista de extensiones permitidas está en `packages/schema/src/textFiles.ts` (`TEXT_FILE_EXTENSIONS`).
- Los archivos de scripts se siguen escaneando después de la carga; los archivos de PowerShell `.ps1`, `.psm1` y `.psd1` se aceptan como texto.
