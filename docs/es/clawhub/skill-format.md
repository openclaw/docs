...
`SKILL.md` o heredados `skills.md` en repositorios públicos, que no sean forks, propiedad de
...
| `always`           | `boolean`  | Si es `true`, el Skill siempre está activo (no necesita instalación explícita).                                                               |
...
Declara variables de entorno opcionales bajo `metadata.openclaw.envVars` y establece `required: false`. No agregues entradas opcionales a `requires.env`, porque `requires.env` significa que el Skill no puede ejecutarse sin ellas.
...
- Los tipos de contenido que empiezan por `text/` se tratan como texto, más una pequeña lista permitida (JSON/YAML/TOML/JS/TS/Markdown/SVG).
