...
Los metadatos de la Skill se declaran en el frontmatter YAML al principio de tu `SKILL.md`. Esto le indica al registro (y al análisis de seguridad) qué necesita tu Skill para ejecutarse.
...
- Los archivos de script se escanean igualmente después de la carga; los archivos PowerShell `.ps1`, `.psm1` y `.psd1` se aceptan como texto.
...
- Si tu Skill se integra con un servicio externo de pago, documenta claramente el costo externo y la cuenta requerida en las instrucciones de la Skill y las declaraciones de entorno (`requires.env` para variables requeridas, o `envVars` con `required: false` para variables opcionales).
