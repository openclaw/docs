...
x-i18n:
    generated_at: "2026-07-04T20:24:04Z"
    model: gpt-5.5
...
- `.clawhubignore` (patrones de ignorados para la publicación, legado `.clawdhubignore`)
...
Declara las variables de entorno opcionales bajo `metadata.openclaw.envVars` y establece `required: false`. No agregues entradas opcionales a `requires.env`, porque `requires.env` significa que la habilidad no puede ejecutarse sin ellas.
...
El análisis de seguridad de ClawHub comprueba que lo que tu habilidad declara coincida con lo que realmente hace. Si tu código hace referencia a `TODOIST_API_KEY` pero tu frontmatter no lo declara bajo `requires.env`, `primaryEnv` o `envVars`, el análisis marcará una discrepancia de metadatos. Mantener declaraciones precisas ayuda a que tu habilidad pase la revisión y ayuda a los usuarios a entender qué están instalando.
...
- Los archivos de script se siguen escaneando después de subirlos; los archivos de PowerShell `.ps1`, `.psm1` y `.psd1` se aceptan como texto.
...
- El texto de embeddings incluye `SKILL.md` + hasta aproximadamente 40 archivos que no sean `.md` (límite de mejor esfuerzo).
