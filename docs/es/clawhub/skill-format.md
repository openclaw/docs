...
- `SKILL.md` (o `skill.md`; también se acepta `skills.md` heredado)

Opcional:

- cualquier archivo _basado en texto_ de apoyo (consulta “Archivos permitidos”)
- `.clawhubignore` (patrones de ignorar para la publicación, `.clawdhubignore` heredado)
...
`SKILL.md` o `skills.md` heredado en repositorios públicos, que no sean bifurcaciones y que pertenezcan a
...
Declara las variables de entorno opcionales en `metadata.openclaw.envVars` y define `required: false`. No añadas entradas opcionales a `requires.env`, porque `requires.env` significa que la Skill no puede ejecutarse sin ellas.
...
- El texto de incrustación incluye `SKILL.md` + hasta unos 40 archivos que no sean `.md` (límite de mejor esfuerzo).

## Slugs

- Se derivan del nombre de la carpeta por defecto.
...
- Cualquiera puede usar, modificar y redistribuir las Skills publicadas, incluso comercialmente.
