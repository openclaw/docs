...
summary: Formato de la carpeta de Skill, archivos requeridos, tipos de archivo permitidos, límites.
x-i18n:
...
- `description` se usa como resumen de la habilidad en la IU/búsqueda.
...
Declara los requisitos de tiempo de ejecución de tu habilidad en `metadata.openclaw` (alias: `metadata.clawdbot`, `metadata.clawdis`).
...
| `always`           | `boolean`  | Si es `true`, la habilidad siempre está activa (no se requiere instalación explícita).                                                          |
| `skillKey`         | `string`   | Sobrescribe la clave de invocación de la habilidad.                                                                                             |
| `emoji`            | `string`   | Emoji mostrado para la habilidad.                                                                                                               |
...
Declara variables de entorno opcionales en `metadata.openclaw.envVars` y establece `required: false`. No agregues entradas opcionales a `requires.env`, porque `requires.env` significa que la habilidad no puede ejecutarse sin ellas.
...
El análisis de seguridad de ClawHub comprueba que lo que declara tu habilidad coincida con lo que realmente hace. Si tu código hace referencia a `TODOIST_API_KEY` pero tu frontmatter no lo declara en `requires.env`, `primaryEnv` o `envVars`, el análisis marcará una discrepancia de metadatos. Mantener las declaraciones precisas ayuda a que tu habilidad supere la revisión y ayuda a los usuarios a entender qué están instalando.
...
- Los archivos de scripts siguen analizándose después de la carga; los archivos PowerShell `.ps1`, `.psm1` y `.psd1` se aceptan como texto.
...
- Todas las habilidades publicadas en ClawHub están licenciadas bajo `MIT-0`.
...
- Si tu habilidad se integra con un servicio de terceros de pago, documenta claramente el coste externo y la cuenta requerida en las instrucciones de la habilidad y en las declaraciones de entorno (`requires.env` para variables obligatorias, o `envVars` con `required: false` para variables opcionales).
