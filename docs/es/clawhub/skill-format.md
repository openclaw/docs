...
x-i18n:
    generated_at: "2026-06-28T20:42:43Z"
    model: gpt-5.5
...
- `.clawhubignore` (patrones de omisión para la publicación, `.clawdhubignore` heredado)
...
`SKILL.md` o el `skills.md` heredado en repositorios públicos, que no sean forks, propiedad de
...
- `description` se usa como resumen de la Skill en la IU/búsqueda.
...
Solo se aceptan archivos “basados en texto” en la publicación.
...
- Si tu Skill se integra con un servicio de terceros de pago, documenta claramente el coste externo y la cuenta necesaria en las instrucciones de la Skill y en las declaraciones de entorno (`requires.env` para variables obligatorias, o `envVars` con `required: false` para variables opcionales).
