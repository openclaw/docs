...
ClawHub es la capa de registro para Skills y plugins de OpenClaw. Da a los usuarios un
lugar para descubrir paquetes, da a los editores un lugar para publicar versiones y
...
la descripción, los requisitos, las variables de entorno y los metadatos. Los metadatos precisos
...
anunciados antes de instalarlo. Los registros de paquete pueden incluir compatibilidad de API,
versión mínima de Gateway, objetivos de host, requisitos de entorno y resúmenes criptográficos
de artefactos.

Usa un origen de instalación explícito de ClawHub cuando quieras que el registro sea la
fuente de verdad:
...
Publicar crea un nuevo registro de versión inmutable. Los editores usan la CLI `clawhub`
...
Los comandos de instalación de OpenClaw usan ClawHub como origen de paquetes:
...
OpenClaw registra los metadatos del origen de instalación para que las actualizaciones puedan resolver el mismo
paquete del registro más adelante. La CLI de ClawHub también admite flujos de trabajo directos de instalación y
actualización de skills para usuarios que quieren carpetas de skills administradas por el registro fuera de un
...
listado canónico de ClawHub, respetan los límites de tasa y evitan implicar respaldo.
