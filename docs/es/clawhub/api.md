...
x-i18n:
    generated_at: "2026-07-14T13:28:55Z"
    model: gpt-5.6
...
- Almacene en caché las respuestas y respete `429`, `Retry-After` y los encabezados de límite de frecuencia en lugar de realizar consultas frecuentes.
- Incluya un enlace a la URL canónica del Skill de ClawHub al mostrar los listados para que los usuarios puedan consultar el registro de origen.
...
- `X-RateLimit-Remaining` / `RateLimit-Remaining`: presupuesto restante exacto cuando
  está presente; las solicitudes distribuidas que se completan correctamente lo omiten en lugar de devolver un valor
  global aproximado
...
- `GET /api/v1/download?slug=&version=&tag=`
  ...
  - Los Skills actuales respaldados por GitHub con un análisis `clean` o `suspicious` devuelven un
    descriptor JSON `public-github` de transferencia en lugar de bytes de ClawHub.
...
Los elementos heredados `/api/*` y `/api/cli/*` siguen disponibles. Consulte `DEPRECATIONS.md`.
