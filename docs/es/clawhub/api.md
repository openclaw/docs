...
- Usa endpoints públicos de lectura como `GET /api/v1/skills`, `GET /api/v1/search` y `GET /api/v1/skills/{slug}` para listados de catálogos.
...
- No repliques contenido oculto, privado o bloqueado por moderación omitiendo filtros de la API pública o límites de autenticación.
...
Aplicación con reconocimiento de autenticación:

- Solicitudes anónimas: por IP.
- Solicitudes autenticadas (token Bearer válido): por segmento de usuario.
- Un token faltante/no válido recurre a la aplicación por IP.
...
- `POST /api/v1/users/reserve` reserva slugs raíz y marcadores de posición privados de paquetes sin lanzamiento para el identificador de un propietario.
