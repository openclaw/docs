...
x-i18n:
    generated_at: "2026-07-16T11:24:25Z"
    model: gpt-5.6
...
- Almacene en caché las respuestas y respete `429`, `Retry-After` y las cabeceras de límite de solicitudes, en lugar de realizar consultas frecuentes de forma agresiva.
- Incluya un enlace a la URL canónica de la Skill de ClawHub al mostrar listados, para que los usuarios puedan consultar el registro de origen.
...
Aplicación basada en la autenticación:
...
- `X-RateLimit-Remaining` / `RateLimit-Remaining`: presupuesto restante exacto cuando
  está presente; las solicitudes distribuidas correctas lo omiten en lugar de devolver un valor
  global aproximado
- `Retry-After`: segundos de espera ante `429`
...
- `POST /api/v1/users/reserve` reserva slugs raíz y marcadores de posición de paquetes privados sin versión para el identificador de un propietario.

## Heredado

Los `/api/*` y `/api/cli/*` heredados siguen disponibles. Consulte `DEPRECATIONS.md`.
