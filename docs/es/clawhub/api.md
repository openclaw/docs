...
- Guarda las respuestas en caché y respeta `429`, `Retry-After` y los encabezados de límite de tasa en lugar de sondear agresivamente.
- Enlaza de vuelta a la URL canónica del Skill en ClawHub al mostrar listados para que los usuarios puedan inspeccionar el registro de origen en el registro.
- Usa URL de páginas canónicas con la forma `https://clawhub.ai/<owner>/skills/<slug>`.
- No des a entender que ClawHub respalda, verifica u opera el sitio de terceros.
- No repliques contenido oculto, privado o bloqueado por moderación evitando los filtros de la API pública o los límites de autenticación.
...
Aplicación con conocimiento de autenticación:

- Solicitudes anónimas: por IP.
- Solicitudes autenticadas (token Bearer válido): por cupo de usuario.
