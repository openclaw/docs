...
x-i18n:
    generated_at: "2026-07-04T17:47:48Z"
    model: gpt-5.5
...
- Almacena respuestas en caché y respeta `429`, `Retry-After` y los encabezados de límite de frecuencia en lugar de hacer sondeos agresivos.
- Enlaza de vuelta a la URL canónica de la Skill en ClawHub al mostrar listados para que los usuarios puedan inspeccionar el registro de origen en el registro.
- Usa URLs de página canónicas con la forma `https://clawhub.ai/<owner>/skills/<slug>`.
- No des a entender que ClawHub respalda, verifica u opera el sitio de terceros.
- No reflejes contenido oculto, privado o bloqueado por moderación eludiendo los filtros de la API pública o los límites de autenticación.
...
- Un token ausente/no válido recae en la aplicación por IP.
...
- `RateLimit-Reset`: segundos de retraso hasta el restablecimiento
- `X-RateLimit-Remaining` / `RateLimit-Remaining`: presupuesto restante exacto cuando
  ...
  global aproximado
- `Retry-After`: segundos de retraso para esperar en `429`
...
- De lo contrario, usa `RateLimit-Reset` o deriva el retraso a partir de `X-RateLimit-Reset`.
...
- `POST /api/v1/users/reserve` reserva slugs raíz y marcadores privados de paquete sin lanzamiento para un identificador de propietario.
