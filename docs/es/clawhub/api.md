...
x-i18n:
    generated_at: "2026-07-11T22:57:26Z"
    model: gpt-5.6
...
Puedes crear un catálogo, directorio o interfaz de búsqueda de terceros sobre las API públicas de lectura de ClawHub. Los metadatos y archivos públicos de Skills se publican conforme a las reglas de licencia de Skills de ClawHub, mientras que la propia API está sujeta a límites de uso y debe utilizarse de manera responsable.
...
- Almacena las respuestas en caché y respeta `429`, `Retry-After` y los encabezados de límite de uso en lugar de realizar consultas frecuentes.
- Al mostrar listados, incluye un enlace a la URL canónica de la Skill en ClawHub para que los usuarios puedan consultar el registro de origen.
- Usa URLs de página canónicas con el formato `https://clawhub.ai/<owner>/skills/<slug>`.
- No insinúes que ClawHub respalda, verifica u opera el sitio de terceros.
- No repliques contenido oculto, privado o bloqueado por moderación eludiendo los filtros de la API pública o los límites de autenticación.
...
- `X-RateLimit-Remaining` / `RateLimit-Remaining`: cuota restante exacta cuando
  está presente; las solicitudes fragmentadas correctas lo omiten en lugar de devolver un valor
  global aproximado
- `Retry-After`: segundos que se debe esperar tras recibir un `429`
...
- Da preferencia a `Retry-After` cuando esté presente.
- De lo contrario, usa `RateLimit-Reset` o calcula la espera a partir de `X-RateLimit-Reset`.
- Añade una variación aleatoria a los reintentos.
...
- `GET /api/v1/skills?limit=&cursor=&sort=`
  ...
  - Alias heredado: `nonSuspicious=true`
  - Con `nonSuspiciousOnly=true`, las páginas basadas en cursor pueden contener menos elementos que `limit`; usa `nextCursor` para continuar.
...
- `GET /api/v1/download?slug=&version=&tag=`
  ...
  - Las Skills actuales respaldadas por GitHub cuyo análisis sea `clean` o `suspicious` devuelven un
    descriptor JSON de transferencia `public-github` en lugar de bytes de ClawHub.
- `GET /api/v1/skills/export?startDate=&endDate=&limit=&cursor=`
  - Las Skills alojadas se exportan como archivos almacenados.
  - Las Skills actuales respaldadas por GitHub cuyo análisis sea `clean` o `suspicious` se exportan
...
- `POST /api/v1/users/reserve` reserva slugs raíz y marcadores privados de paquetes sin versiones para un identificador de propietario.

## Heredado

Las rutas heredadas `/api/*` y `/api/cli/*` siguen disponibles. Consulta `DEPRECATIONS.md`.
