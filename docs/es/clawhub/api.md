...
- Almacena respuestas en caché y respeta `429`, `Retry-After` y los encabezados de límite de frecuencia en lugar de sondear agresivamente.
- Enlaza a la URL canónica del Skill de ClawHub al mostrar listados para que los usuarios puedan inspeccionar el registro fuente del registro.
- Usa URL de página canónicas con la forma `https://clawhub.ai/<owner>/skills/<slug>`.
- No sugieras que ClawHub avala, verifica u opera el sitio de terceros.
...
- Los errores v1 son texto plano (`text/plain; charset=utf-8`), incluidos `400`,
...
- `GET /api/v1/download?slug=&version=&tag=`
  ...
  - Los Skills actuales respaldados por GitHub con un escaneo `clean` o `suspicious` devuelven un
...
- `GET /api/v1/skills/export?startDate=&endDate=&limit=&cursor=`
  ...
  - Los Skills actuales respaldados por GitHub con un escaneo `clean` o `suspicious` se exportan
...
- `POST /api/v1/users/reserve` reserva slugs raíz y marcadores de posición de paquetes privados sin publicación para un identificador de propietario.
