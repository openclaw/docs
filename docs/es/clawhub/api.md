...
- Almacena respuestas en caché y respeta `429`, `Retry-After` y los encabezados de límite de tasa en lugar de hacer sondeos agresivos.
- Enlaza a la URL canónica de la skill de ClawHub al mostrar listados para que los usuarios puedan inspeccionar el registro de origen del registro.
...
- Un token ausente o inválido recurre a la aplicación por IP.
...
- En caso contrario, usa `RateLimit-Reset` o deriva la demora a partir de `X-RateLimit-Reset`.
- Añade jitter a los reintentos.

## Errores

- Los errores de v1 son texto plano (`text/plain; charset=utf-8`), incluidos `400`,
...
- Los parámetros de consulta conocidos con valores inválidos devuelven `400`.
...
- `GET /api/v1/skills?limit=&cursor=&sort=`
  ...
  - Los valores de `sort` inválidos devuelven `400`
...
- `GET /api/v1/download?slug=&version=&tag=`
  ...
  - Las Skills actuales respaldadas por GitHub con un análisis `clean` o `suspicious` devuelven un
    descriptor JSON de entrega `public-github` en lugar de bytes de ClawHub.
- `GET /api/v1/skills/export?startDate=&endDate=&limit=&cursor=`
  ...
  - Las Skills actuales respaldadas por GitHub con un análisis `clean` o `suspicious` se exportan
    como descriptores de entrega `public-github`.
...
- `POST /api/v1/users/reserve` reserva slugs raíz y marcadores de posición privados de paquetes sin publicación para el identificador de un propietario.
