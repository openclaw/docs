...
- Almacena en caché las respuestas y respeta `429`, `Retry-After` y los encabezados de límite de frecuencia en lugar de sondear de forma agresiva.
...
- No espejes contenido oculto, privado o bloqueado por moderación omitiendo los filtros de la API pública o los límites de autenticación.
...
- Si falta el token o no es válido, se recurre a la aplicación por IP.
...
- `X-RateLimit-Reset`: segundos de época Unix (hora de reinicio absoluta)
- `RateLimit-Reset`: segundos de demora hasta el reinicio
- `X-RateLimit-Remaining` / `RateLimit-Remaining`: presupuesto restante exacto cuando
  está presente; las solicitudes exitosas particionadas lo omiten en lugar de devolver un valor
  global aproximado
...
- Agrega variación aleatoria a los reintentos.
...
- `GET /api/v1/download?slug=&version=&tag=`
  ...
  - Las Skills actuales respaldadas por GitHub con un análisis `clean` o `suspicious` devuelven un
    descriptor de entrega JSON `public-github` en lugar de bytes de ClawHub.
