...
x-i18n:
    generated_at: "2026-07-01T18:06:50Z"
    model: gpt-5.5
...
ClawHub valida paquetes de Plugin antes de publicarlos y también puede mostrar hallazgos de
...
contiene códigos de mantenimiento del escáner sin guía de corrección para el autor, esos
corresponden a los mantenedores de OpenClaw y no a los autores de Plugin.
...
| Código                                  | Empieza aquí                                                                                                               |
| --------------------------------------- | --------------------------------------------------------------------------------------------------------------------------- |
| `package-json-missing`                  | [Agrega metadatos de paquete](/es/clawhub/plugin-validation-fixes#package-json-missing)                                                   |
...
| `package-openclaw-entry-missing`        | [Declara puntos de entrada de paquete de OpenClaw](/es/clawhub/plugin-validation-fixes#package-openclaw-entry-missing)                         |
...
| `package-npm-pack-unavailable`          | [Haz que el artefacto npm sea empaquetable](/es/clawhub/plugin-validation-fixes#package-npm-pack-unavailable)                                 |
...
- Consulta [Puntos de entrada de Plugin](/es/plugins/sdk-entrypoints) y
  [campos de package.json que afectan la detección](/es/plugins/manifest#packagejson-fields-that-affect-discovery).
- Vuelve a ejecutar `clawhub package validate <path-to-plugin>`.
...
- Completa `openclaw.install` con la fuente de instalación compatible, como
  `clawhubSpec`, `npmSpec` o `localPath`.
- Configura `openclaw.install.defaultChoice` cuando haya más de una fuente de instalación
  disponible.
...
- Mantén los puntos de entrada del paquete, compatibilidad, instalación, configuración y metadatos de catálogo
...
- Mueve los metadatos de paquete o instalación a los campos admitidos de `package.json#openclaw`
...
- Mueve el comportamiento de tiempo de ejecución al código de registro del plugin y limita `contracts`
...
- Reemplaza las importaciones del barril raíz por importaciones de subrutas públicas específicas.
...
- Usa [Convenciones de importación](/es/plugins/building-plugins#import-conventions) y
  [Subrutas del SDK de plugin](/es/plugins/sdk-subpaths) para encontrar la importación más limitada.
- Vuelve a ejecutar `clawhub package validate <path-to-plugin>`.
...
- Reemplaza las importaciones reservadas del SDK interno de OpenClaw por subrutas públicas documentadas de
  `openclaw/plugin-sdk/*`.
...
- Usa `appendSessionTranscriptMessageByIdentity(...)` para agregar mensajes a la transcripción.
