...
summary: Corrige los hallazgos de validación del paquete de Plugin de ClawHub antes de publicarlo
title: Correcciones de validación de Plugin
x-i18n:
    generated_at: "2026-07-05T17:40:34Z"
    model: gpt-5.5
...
No cubre los hallazgos de cobertura internos de Plugin Inspector. Si un informe completo
contiene códigos de mantenimiento del escáner sin orientación de remediación para el autor, esos
son para los mantenedores de OpenClaw, no para los autores de Plugin.
...
| `package-openclaw-entry-missing`        | [Declara los puntos de entrada del paquete OpenClaw](/es/clawhub/plugin-validation-fixes#package-openclaw-entry-missing)                         |
...
| `package-plugin-api-compat-missing`     | [Declara la compatibilidad con la API de Plugin](/es/clawhub/plugin-validation-fixes#package-plugin-api-compat-missing)                          |
...
| `sdk-load-session-store`                | [Reemplaza el acceso al almacén completo de sesión](/es/clawhub/plugin-validation-fixes#sdk-load-session-store)                                   |
| `sdk-session-store-write`               | [Reemplaza escrituras del almacén completo de sesión](/es/clawhub/plugin-validation-fixes#sdk-session-store-write)                                  |
...
El paquete tiene `package.json`, pero no declara metadatos de paquete
OpenClaw.
...
- Usa la versión de la API de Plugin de OpenClaw o el mínimo semver contra el que compilaste
  y probaste.
- Mantén esto separado de la versión del paquete. La versión del paquete describe la
  versión publicada del Plugin; `openclaw.compat.pluginApi` describe el contrato de API del host.
- Consulta [Campos de package.json que afectan el descubrimiento](/es/plugins/manifest#packagejson-fields-that-affect-discovery).
- Vuelve a ejecutar `clawhub package validate <path-to-plugin>`.
...
- Prefiere `package.json#version` como la versión de publicación del paquete.
...
- Mantén `name` legible para personas y mantén `id` como el identificador estable de máquina.
...
El Plugin importa una ruta del SDK reservada para Plugins empaquetados o compatibilidad
interna.
...
El Plugin aún usa ayudantes obsoletos de ruta de archivo de sesión, como
...
- Usa la identidad de transcripción o los ayudantes de destino cuando el código prepara una
...
- Usa `resolveSessionTranscriptIdentity(...)` cuando el código solo necesita la identidad pública
  de la sesión.
...
El Plugin aún usa ayudantes obsoletos de transcripción de bajo nivel, como
...
- Prefiere la superficie estructurada de tiempo de ejecución de transcripción para que OpenClaw pueda aplicar los
...
- Mantén los metadatos de variables de entorno de canal declarativos para que OpenClaw pueda inspeccionar el estado
  de setup sin cargar el tiempo de ejecución del canal.
- Refleja la configuración de canal impulsada por variables de entorno en los metadatos actuales de setup, configuración de canal o
