...
read_when:
    ...
    - Estás actualizando los metadatos del paquete Plugin antes del lanzamiento
summary: Corregir los hallazgos de validación de paquetes de Plugin de ClawHub antes de publicar
title: Correcciones de validación de Plugin
x-i18n:
    generated_at: "2026-07-04T10:26:53Z"
    model: gpt-5.5
...
análisis automatizados de paquetes. Esta página cubre los hallazgos orientados a autores, es decir,
...
contiene códigos de mantenimiento del analizador sin guía de corrección para autores, esos
...
## Hallazgos orientados a autores
...
| `package-openclaw-unsupported-metadata` | [Eliminar metadatos de paquete OpenClaw no compatibles](/es/clawhub/plugin-validation-fixes#package-openclaw-unsupported-metadata)          |
...
| `manifest-name-missing`                 | [Añadir un nombre visible al manifiesto](/es/clawhub/plugin-validation-fixes#manifest-name-missing)                                           |
...
| `legacy-root-sdk-import`                | [Sustituir importaciones raíz del SDK](/es/clawhub/plugin-validation-fixes#legacy-root-sdk-import)                                             |
| `reserved-sdk-import`                   | [Eliminar importaciones reservadas del SDK](/es/clawhub/plugin-validation-fixes#reserved-sdk-import)                                             |
| `sdk-load-session-store`                | [Sustituir el acceso al almacén de sesión completo](/es/clawhub/plugin-validation-fixes#sdk-load-session-store)                                   |
| `sdk-session-store-write`               | [Sustituir escrituras en el almacén de sesión completo](/es/clawhub/plugin-validation-fixes#sdk-session-store-write)                                  |
| `sdk-session-file-helper`               | [Sustituir ayudantes de ruta de archivo de sesión](/es/clawhub/plugin-validation-fixes#sdk-session-file-helper)                                   |
| `sdk-session-transcript-file-target`    | [Sustituir destinos de archivo de transcripción heredados](/es/clawhub/plugin-validation-fixes#sdk-session-transcript-file-target)                   |
| `sdk-session-transcript-low-level`      | [Sustituir ayudantes de transcripción de bajo nivel](/es/clawhub/plugin-validation-fixes#sdk-session-transcript-low-level)                       |
| `legacy-before-agent-start`             | [Sustituir before_agent_start](/es/clawhub/plugin-validation-fixes#legacy-before-agent-start)                                        |
| `provider-auth-env-vars`                | [Mover variables de entorno del proveedor a metadatos de configuración](/es/clawhub/plugin-validation-fixes#provider-auth-env-vars)                             |
...
- Añade `openclaw.extensions` para puntos de entrada de Plugin nativos.
...
- Rellena `openclaw.install` con la fuente de instalación compatible, como
...
- Si `openclaw.plugin.json` también tiene `version`, actualízala para que coincida o elimina
  los metadatos de versión de manifiesto obsoletos cuando los metadatos del paquete sean la fuente de autoridad.
...
- Elimina campos no compatibles como `openclaw.bundle`.
- Mantén los metadatos de Plugin nativos en `openclaw.plugin.json`.
- Mantén los puntos de entrada del paquete, la compatibilidad, la instalación, la configuración y los metadatos de catálogo
...
- Asegúrate de que `openclaw.plugin.json` esté incluido cuando el paquete sea un Plugin nativo de
  OpenClaw.
...
### falta-nombre-manifest

El manifiesto nativo del Plugin no incluye un nombre para mostrar.

- Añade un campo `name` no vacío a `openclaw.plugin.json`.
- Mantén `name` legible para humanos y conserva `id` como el identificador estable para máquina.
- Consulta [Manifiesto de Plugin](/es/plugins/manifest).
- Vuelve a ejecutar `clawhub package validate <path-to-plugin>`.

### campos-desconocidos-manifest
...
- Mueve los metadatos de paquete o instalación a campos admitidos de `package.json#openclaw`
  en lugar de ponerlos en el manifiesto.
- Vuelve a ejecutar `clawhub package validate <path-to-plugin>`.

### contratos-desconocidos-manifest
...
- Mueve el comportamiento de runtime al código de registro del Plugin y limita `contracts`
  a metadatos estáticos de propiedad de capacidades.
- Vuelve a ejecutar `clawhub package validate <path-to-plugin>`.

## Migración de SDK y compatibilidad

### import-sdk-root-legado

El Plugin importa desde el barrel raíz obsoleto del SDK:
`openclaw/plugin-sdk`.

- Sustituye las importaciones desde el barrel raíz por importaciones de subrutas públicas específicas.
...
- Usa [Convenciones de importación](/es/plugins/building-plugins#import-conventions) y
  [subrutas del SDK de Plugin](/es/plugins/sdk-subpaths) para encontrar la importación más precisa.
- Vuelve a ejecutar `clawhub package validate <path-to-plugin>`.

### import-sdk-reservado

El Plugin importa una ruta del SDK reservada para Plugins incluidos o compatibilidad
interna.

- Sustituye las importaciones reservadas del SDK interno de OpenClaw por subrutas públicas
  documentadas de `openclaw/plugin-sdk/*`.
- Si el comportamiento no tiene un SDK público, conserva el helper dentro de tu paquete o
  solicita una API pública de OpenClaw.
- Usa [subrutas del SDK de Plugin](/es/plugins/sdk-subpaths) y
  [Migración de SDK](/es/plugins/sdk-migration) para elegir una importación admitida.
- Vuelve a ejecutar `clawhub package validate <path-to-plugin>`.
...
- Usa `getSessionEntry(...)` o `listSessionEntries(...)` al leer estado de sesión.
- Usa `patchSessionEntry(...)` o `upsertSessionEntry(...)` al escribir estado de sesión.
...
- Conserva `loadSessionStore(...)` solo mientras tu rango de compatibilidad declarado
  todavía admita versiones antiguas de OpenClaw que lo requieren.
...
- Evita cargar, mutar y guardar el objeto completo del almacén de sesión.
- Conserva los helpers de escritura de almacén completo solo mientras tu rango de compatibilidad declarado
  todavía admita versiones antiguas de OpenClaw que los requieren.
- Consulta [API de runtime](/es/plugins/sdk-runtime#agent-session-state) y
  [subrutas del SDK de Plugin](/es/plugins/sdk-subpaths).
- Vuelve a ejecutar `clawhub package validate <path-to-plugin>`.
...
- Usa helpers de identidad de transcripción o de destino cuando el código prepare una
  operación de transcripción.
- No persistas ni dependas de rutas de archivo de transcripción legadas.
- Consulta [API de runtime](/es/plugins/sdk-runtime#agent-session-state) y
  [subrutas del SDK de Plugin](/es/plugins/sdk-subpaths).
- Vuelve a ejecutar `clawhub package validate <path-to-plugin>`.
...
- Evita leer o construir destinos de archivo de transcripción legados directamente.
- Conserva el helper legado solo mientras tu rango de compatibilidad declarado todavía
  admita versiones antiguas de OpenClaw que lo requieren.
- Consulta [API de runtime](/es/plugins/sdk-runtime#agent-session-state) y
  [subrutas del SDK de Plugin](/es/plugins/sdk-subpaths).
- Vuelve a ejecutar `clawhub package validate <path-to-plugin>`.
...
El Plugin todavía usa helpers obsoletos de transcripción de bajo nivel, como
`appendSessionTranscriptMessage` o `emitSessionTranscriptUpdate`.

- Usa `appendSessionTranscriptMessageByIdentity(...)` para añadir contenido a transcripciones.
...
- Conserva los helpers de transcripción de bajo nivel solo mientras tu rango de compatibilidad declarado
  todavía admita versiones antiguas de OpenClaw que los requieren.
- Consulta [API de runtime](/es/plugins/sdk-runtime#agent-session-state) y
  [subrutas del SDK de Plugin](/es/plugins/sdk-subpaths).
- Vuelve a ejecutar `clawhub package validate <path-to-plugin>`.
...
El Plugin todavía usa el hook legado `before_agent_start`.
...
El manifiesto todavía usa metadatos legados de autenticación de proveedor `providerAuthEnvVars`.

- Refleja los metadatos de variables de entorno del proveedor en `setup.providers[].envVars`.
- Conserva `providerAuthEnvVars` solo como metadatos de compatibilidad mientras tu rango admitido
  de OpenClaw todavía los necesite.
- Consulta [referencia de setup](/es/plugins/manifest#setup-reference) y
  [Migración de SDK](/es/plugins/sdk-migration).
- Vuelve a ejecutar `clawhub package validate <path-to-plugin>`.

### channel-env-vars

El manifiesto usa metadatos legados o antiguos de variables de entorno de canal sin los metadatos
actuales de setup o configuración que ClawHub espera.

- Mantén los metadatos de variables de entorno de canal declarativos para que OpenClaw pueda inspeccionar
  el estado de setup sin cargar el runtime del canal.
- Refleja el setup de canal basado en variables de entorno en los metadatos actuales de setup, configuración de canal o
  canal de paquete que use la forma de tu Plugin.
- Conserva `channelEnvVars` solo como metadatos de compatibilidad mientras versiones antiguas admitidas
  de OpenClaw todavía los requieran.
...
- Elimina la URL del esquema si es solo informativa.
...
- Mantén el comportamiento sensible a seguridad documentado en la documentación pública de tu paquete o
