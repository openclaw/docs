...
x-i18n:
    generated_at: "2026-07-03T00:51:49Z"
    model: gpt-5.5
...
escaneos automatizados de paquetes. Esta página cubre los hallazgos orientados a autores, es decir,
...
No cubre los hallazgos internos de cobertura del Plugin Inspector. Si un informe completo
contiene códigos de mantenimiento del escáner sin guía de remediación para autores, esos
...
| `sdk-session-store-write`               | [Reemplaza las escrituras del almacén de sesión completo](/es/clawhub/plugin-validation-fixes#sdk-session-store-write)                                  |
| `sdk-session-file-helper`               | [Reemplaza los ayudantes de ruta de archivo de sesión](/es/clawhub/plugin-validation-fixes#sdk-session-file-helper)                                   |
...
- Rellena `openclaw.install` con el origen de instalación compatible, como
...
- Usa la versión de la API de Plugin de OpenClaw o el límite inferior semver con el que
...
- Si `openclaw.plugin.json` también tiene `version`, actualízala para que coincida o elimina
  metadatos obsoletos de versión del manifiesto cuando los metadatos del paquete sean la fuente autorizada.
...
- Mantén los metadatos nativos del Plugin en `openclaw.plugin.json`.
- Mantén los puntos de entrada del paquete, la compatibilidad, la instalación, la configuración y los metadatos
  de catálogo en campos compatibles de `package.json#openclaw`.
- Consulta [campos de package.json que afectan al descubrimiento](/es/plugins/manifest#packagejson-fields-that-affect-discovery).
- Vuelve a ejecutar `clawhub package validate <path-to-plugin>`.
...
- Mantén `name` legible para personas y mantén `id` como el identificador de máquina estable.
- Consulta [Manifiesto de Plugin](/es/plugins/manifest).
- Vuelve a ejecutar `clawhub package validate <path-to-plugin>`.
...
- Sustituye las importaciones del barrel raíz por importaciones de subrutas públicas específicas.
...
- Usa [Convenciones de importación](/es/plugins/building-plugins#import-conventions) y
  [Subrutas del Plugin SDK](/es/plugins/sdk-subpaths) para encontrar la importación más acotada.
- Vuelve a ejecutar `clawhub package validate <path-to-plugin>`.
...
- Sustituye las importaciones reservadas del SDK interno de OpenClaw por subrutas públicas documentadas
  `openclaw/plugin-sdk/*`.
...
- Mantén `loadSessionStore(...)` solo mientras tu rango de compatibilidad declarado
  siga admitiendo versiones antiguas de OpenClaw que lo requieran.
...
- Usa `upsertSessionEntry(...)` al sustituir o crear una entrada de sesión.
- Evita cargar, mutar y guardar el objeto completo del almacén de sesión.
- Mantén los helpers de escritura del almacén completo solo mientras tu rango de compatibilidad declarado
  siga admitiendo versiones antiguas de OpenClaw que los requieran.
- Consulta [API en tiempo de ejecución](/es/plugins/sdk-runtime#agent-session-state) y
  [Subrutas del Plugin SDK](/es/plugins/sdk-subpaths).
- Vuelve a ejecutar `clawhub package validate <path-to-plugin>`.
...
- Mantén el helper heredado solo mientras tu rango de compatibilidad declarado siga
  admitiendo versiones antiguas de OpenClaw que lo requieran.
- Consulta [API en tiempo de ejecución](/es/plugins/sdk-runtime#agent-session-state) y
  [Subrutas del Plugin SDK](/es/plugins/sdk-subpaths).
- Vuelve a ejecutar `clawhub package validate <path-to-plugin>`.
...
- Elimina la URL del esquema si solo es consultiva.
