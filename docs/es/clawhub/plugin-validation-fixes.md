...
read_when:
    ...
    - ClawHub rechazó una publicación de paquete de Plugin o emitió una advertencia sobre ella.
    - Estás actualizando los metadatos del paquete del Plugin antes del lanzamiento
summary: Corrige los hallazgos de validación del paquete de Plugin de ClawHub antes de publicar
...
análisis automatizados de paquetes. Esta página cubre los hallazgos dirigidos al autor, es decir,
...
contiene códigos de mantenimiento del analizador sin orientación de corrección para el autor, esos
...
- Agrega metadatos de compatibilidad e instalación cuando el paquete vaya a publicarse o
...
La versión del paquete y la versión del manifiesto de Plugin no coinciden.
...
- Si `openclaw.plugin.json` también tiene `version`, actualízala para que coincida o elimina
  metadatos de versión de manifiesto obsoletos cuando los metadatos del paquete sean la fuente de autoridad.
...
- Mantén los metadatos de puntos de entrada del paquete, compatibilidad, instalación, configuración y catálogo
...
El manifiesto nativo de Plugin no incluye un nombre visible.
...
El manifiesto de Plugin tiene campos de nivel superior que OpenClaw no admite.
...
- Usa [convenciones de importación](/es/plugins/building-plugins#import-conventions) y
  [subrutas del SDK de plugins](/es/plugins/sdk-subpaths) para encontrar la importación estrecha.
- Vuelve a ejecutar `clawhub package validate <path-to-plugin>`.
...
- Evita cargar, modificar y guardar todo el objeto de almacén de sesión.
