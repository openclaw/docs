---
read_when:
    - Ejecutaste clawhub package validate y debes corregir los hallazgos del Plugin
...
summary: Corregir los hallazgos de validación del paquete del Plugin ClawHub antes de publicar
title: Correcciones de validación de Plugin
x-i18n:
    generated_at: "2026-07-02T07:55:18Z"
    model: gpt-5.5
...
ClawHub valida los paquetes de Plugin antes de publicarlos y también puede mostrar hallazgos de análisis automatizados de paquetes. Esta página cubre los hallazgos orientados al autor, es decir, hallazgos que el autor del Plugin puede corregir en los metadatos del paquete, el manifiesto, las importaciones del SDK o el artefacto publicado.

No cubre los hallazgos internos de cobertura de Plugin Inspector. Si un informe completo contiene códigos de mantenimiento del escáner sin orientación de remediación para el autor, esos códigos son para los mantenedores de OpenClaw, no para los autores de plugins.
...
| Código                                  | Empieza aquí                                                                                                                |
| --------------------------------------- | --------------------------------------------------------------------------------------------------------------------------- |
| `package-json-missing`                  | [Agregar metadatos del paquete](/es/clawhub/plugin-validation-fixes#package-json-missing)                                      |
| `package-openclaw-metadata-missing`     | [Agregar el bloque openclaw del paquete](/es/clawhub/plugin-validation-fixes#package-openclaw-metadata-missing)                |
...
| `package-plugin-api-compat-missing`     | [Declarar compatibilidad con la API de plugins](/es/clawhub/plugin-validation-fixes#package-plugin-api-compat-missing)         |
...
| `manifest-name-missing`                 | [Agregar un nombre visible al manifiesto](/es/clawhub/plugin-validation-fixes#manifest-name-missing)                           |
...
| `legacy-root-sdk-import`                | [Reemplazar importaciones del SDK raíz](/es/clawhub/plugin-validation-fixes#legacy-root-sdk-import)                            |
| `reserved-sdk-import`                   | [Eliminar importaciones reservadas del SDK](/es/clawhub/plugin-validation-fixes#reserved-sdk-import)                           |
| `sdk-load-session-store`                | [Reemplazar el acceso a todo el almacén de sesión](/es/clawhub/plugin-validation-fixes#sdk-load-session-store)                 |
| `sdk-session-store-write`               | [Reemplazar escrituras en todo el almacén de sesión](/es/clawhub/plugin-validation-fixes#sdk-session-store-write)              |
| `sdk-session-file-helper`               | [Reemplazar ayudantes de rutas de archivos de sesión](/es/clawhub/plugin-validation-fixes#sdk-session-file-helper)             |
| `sdk-session-transcript-file-target`    | [Reemplazar destinos de archivo de transcripción heredados](/es/clawhub/plugin-validation-fixes#sdk-session-transcript-file-target) |
...
| `provider-auth-env-vars`                | [Mover variables de entorno del proveedor a los metadatos de configuración](/es/clawhub/plugin-validation-fixes#provider-auth-env-vars) |
| `channel-env-vars`                      | [Reflejar variables de entorno del canal en los metadatos actuales](/es/clawhub/plugin-validation-fixes#channel-env-vars)      |
...
- Usa [Crear plugins](/es/plugins/building-plugins) para ver un ejemplo mínimo de paquete y [Manifiesto de Plugin](/es/plugins/manifest#manifest-versus-packagejson) para la separación entre paquete y manifiesto.
- Vuelve a ejecutar `clawhub package validate <path-to-plugin>`.

### package-openclaw-metadata-missing

El paquete tiene `package.json`, pero no declara metadatos de paquete OpenClaw.

- Agrega `package.json#openclaw`.
- Incluye metadatos de punto de entrada como `openclaw.extensions` u `openclaw.runtimeExtensions`.
- Agrega metadatos de compatibilidad e instalación cuando el paquete se vaya a publicar o instalar mediante ClawHub.
- Consulta [campos de package.json que afectan la detección](/es/plugins/manifest#packagejson-fields-that-affect-discovery).
- Vuelve a ejecutar `clawhub package validate <path-to-plugin>`.
...
- Consulta [Puntos de entrada de Plugin](/es/plugins/sdk-entrypoints) y [campos de package.json que afectan la detección](/es/plugins/manifest#packagejson-fields-that-affect-discovery).
- Vuelve a ejecutar `clawhub package validate <path-to-plugin>`.

### package-entrypoint-missing

El paquete declara un punto de entrada de OpenClaw, pero el archivo referenciado no existe en el paquete que se está validando.
...
- Mantén esto separado de la versión del paquete. La versión del paquete describe la publicación del Plugin; `openclaw.compat.pluginApi` describe el contrato de API del host.
- Consulta [campos de package.json que afectan la detección](/es/plugins/manifest#packagejson-fields-that-affect-discovery).
- Vuelve a ejecutar `clawhub package validate <path-to-plugin>`.
...
- Revisa cualquier metadato de compilación de OpenClaw en el paquete, como la versión de OpenClaw usada durante la publicación.
...
- Si `openclaw.plugin.json` también tiene `version`, actualízala para que coincida o elimina los metadatos obsoletos de versión del manifiesto cuando los metadatos del paquete sean la fuente de autoridad.
...
El bloque `package.json#openclaw` contiene campos que no son metadatos de paquete OpenClaw admitidos.
...
- Corrige metadatos de paquete no válidos, scripts de ciclo de vida rotos o entradas de archivos que hacen fallar el empaquetado.
- Elimina `private: true` si este paquete está pensado para publicación pública.
- Vuelve a ejecutar `clawhub package validate <path-to-plugin>`.
...
- Mantén `name` legible para personas y conserva `id` como el identificador estable de máquina.
...
- Usa `openclaw/plugin-sdk/channel-core` para los ayudantes de entrada de canal.
...
- Si el comportamiento no tiene un SDK público, mantén el ayudante dentro de tu paquete o
...
El Plugin aún usa el ayudante obsoleto para todo el almacén de sesiones
`loadSessionStore`.
...
- Evita cargar, mutar y guardar todo el objeto del almacén de sesiones.
...
El Plugin aún usa un ayudante obsoleto de escritura para todo el almacén de sesiones, como
...
- Evita cargar, mutar y guardar todo el objeto del almacén de sesiones.
- Mantén los ayudantes de escritura para todo el almacén solo mientras tu rango de compatibilidad declarado
...
El Plugin aún usa ayudantes obsoletos de rutas de archivo de sesión, como
...
- Usa la identidad de transcripción o ayudantes de destino cuando el código esté preparando una
...
El Plugin aún usa el ayudante obsoleto de destino de archivo de transcripción
...
- Mantén el ayudante heredado solo mientras tu rango de compatibilidad declarado aún
...
El Plugin aún usa ayudantes obsoletos de bajo nivel para transcripciones, como
...
- Usa `publishSessionTranscriptUpdateByIdentity(...)` para notificaciones de actualización
  de transcripción.
- Prefiere la superficie estructurada de runtime de transcripción para que OpenClaw pueda aplicar
  los límites de transacción y el manejo de identidad correctos.
- Mantén los ayudantes de transcripción de bajo nivel solo mientras tu rango de compatibilidad declarado
  aún admita versiones anteriores de OpenClaw que los requieran.
- Consulta [API de runtime](/es/plugins/sdk-runtime#agent-session-state) y
  [Subrutas del SDK de Plugin](/es/plugins/sdk-subpaths).
- Vuelve a ejecutar `clawhub package validate <path-to-plugin>`.
...
- Mantén los metadatos de variables de entorno de canal declarativos para que OpenClaw pueda inspeccionar el estado
  de setup sin cargar el runtime del canal.
...
- Mantén `channelEnvVars` solo como metadatos de compatibilidad mientras versiones anteriores compatibles
