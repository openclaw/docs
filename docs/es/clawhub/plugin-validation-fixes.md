...
summary: Corregir los hallazgos de validación del paquete del plugin ClawHub antes de publicarlo
...
ClawHub valida paquetes de plugins antes de la publicación y también puede mostrar hallazgos de
análisis automatizados de paquetes. Esta página cubre hallazgos dirigidos a autores, es decir,
...
contiene códigos de mantenimiento del escáner sin orientación de remediación para autores, esos
son para mantenedores de OpenClaw en lugar de autores de plugins.
...
| `package-plugin-api-compat-missing`     | [Declara compatibilidad de la API del plugin](/es/clawhub/plugin-validation-fixes#package-plugin-api-compat-missing)                          |
...
| `sdk-session-file-helper`               | [Reemplaza ayudantes de rutas de archivo de sesión](/es/clawhub/plugin-validation-fixes#sdk-session-file-helper)                                   |
...
| `channel-env-vars`                      | [Refleja variables de entorno del canal en los metadatos actuales](/es/clawhub/plugin-validation-fixes#channel-env-vars)                                |
...
- Completa `openclaw.install` con el origen de instalación compatible, como
  `clawhubSpec`, `npmSpec` o `localPath`.
- Establece `openclaw.install.defaultChoice` cuando haya más de un origen de instalación
  disponible.
...
- Usa la versión de la API de plugins de OpenClaw o el piso semver contra el que
...
- Mantén los puntos de entrada del paquete y los metadatos de compatibilidad, instalación, configuración y catálogo
...
- Usa [Convenciones de importación](/es/plugins/building-plugins#import-conventions) y
  [Subrutas del SDK de plugins](/es/plugins/sdk-subpaths) para encontrar la importación más acotada.
- Vuelve a ejecutar `clawhub package validate <path-to-plugin>`.
...
- Reemplaza las importaciones internas reservadas del SDK de OpenClaw por subrutas públicas documentadas
  `openclaw/plugin-sdk/*`.
- Si el comportamiento no tiene un SDK público, mantén el helper dentro de tu paquete o
  solicita una API pública de OpenClaw.
- Usa [Subrutas del SDK de plugins](/es/plugins/sdk-subpaths) y
...
- Usa `getSessionEntry(...)` para leer metadatos de sesión por agente e identidad
  de sesión.
- Usa `patchSessionEntry(...)` o `upsertSessionEntry(...)` para persistir metadatos
  de sesión.
- Usa identidad de transcripción o helpers de destino cuando el código esté preparando una
...
- Usa `resolveSessionTranscriptIdentity(...)` cuando el código solo necesite identidad
  pública de sesión.
...
- Mantén el helper heredado solo mientras tu rango de compatibilidad declarado aún
...
El plugin aún usa helpers de transcripción de bajo nivel obsoletos, como
...
- Mantén `providerAuthEnvVars` solo como metadatos de compatibilidad mientras tu rango
  de OpenClaw admitido aún lo necesite.
...
- Replica el setup de canal basado en variables de entorno en los metadatos actuales de setup, configuración de canal o
  paquete de canal usados por la forma de tu plugin.
- Mantén `channelEnvVars` solo como metadatos de compatibilidad mientras versiones anteriores admitidas de
