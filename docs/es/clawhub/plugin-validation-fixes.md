...
| `provider-auth-env-vars`                | [Mueve las vars de entorno del proveedor a los metadatos de configuración](/es/clawhub/plugin-validation-fixes#provider-auth-env-vars) |
| `channel-env-vars`                      | [Replica las vars de entorno del canal en los metadatos actuales](/es/clawhub/plugin-validation-fixes#channel-env-vars)        |
...
- Usa la versión de API de plugins de OpenClaw o el mínimo semver contra el que creaste
  y probaste.
...
- Si `openclaw.plugin.json` también tiene `version`, actualízalo para que coincida o elimina
  los metadatos de versión de manifiesto obsoletos cuando los metadatos del paquete sean autoritativos.
...
- Corrige metadatos de paquete no válidos, scripts de ciclo de vida rotos o entradas de archivos que
  hacen fallar el empaquetado.
- Elimina `private: true` si este paquete está destinado a publicación pública.
- Vuelve a ejecutar `clawhub package validate <path-to-plugin>`.

### package-npm-pack-entrypoint-missing

El paquete puede empaquetarse, pero el artefacto empaquetado no incluye los archivos de
punto de entrada declarados en `package.json#openclaw`.
...
- Mantén `name` legible para humanos y conserva `id` como id estable de máquina.
- Consulta [Manifiesto de Plugin](/es/plugins/manifest).
- Vuelve a ejecutar `clawhub package validate <path-to-plugin>`.
...
- Usa [Convenciones de importación](/es/plugins/building-plugins#import-conventions) y
  [Subrutas del SDK de Plugin](/es/plugins/sdk-subpaths) para encontrar la importación precisa.
- Vuelve a ejecutar `clawhub package validate <path-to-plugin>`.
...
- Replica el setup de canal impulsado por variables de entorno en los metadatos actuales de setup, configuración de canal o
