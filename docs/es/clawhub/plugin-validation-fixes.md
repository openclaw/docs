...
x-i18n:
    generated_at: "2026-07-03T23:26:11Z"
    model: gpt-5.5
...
ClawHub valida los paquetes de Plugin antes de publicarlos y también puede mostrar hallazgos de análisis automatizados de paquetes. Esta página cubre los hallazgos dirigidos a autores, es decir, hallazgos que el autor del Plugin puede corregir en los metadatos de su paquete, el manifiesto, las importaciones del SDK o el artefacto publicado.

No cubre hallazgos internos de cobertura de Plugin Inspector. Si un informe completo contiene códigos de mantenimiento del analizador sin guía de remediación para autores, esos códigos son para mantenedores de OpenClaw, no para autores de Plugins.
...
| `sdk-session-file-helper`               | [Sustituye los ayudantes de rutas de archivo de sesión](/es/clawhub/plugin-validation-fixes#sdk-session-file-helper)                                   |
| `sdk-session-transcript-file-target`    | [Sustituye los destinos heredados de archivo de transcripción](/es/clawhub/plugin-validation-fixes#sdk-session-transcript-file-target)                   |
| `sdk-session-transcript-low-level`      | [Sustituye los ayudantes de transcripción de bajo nivel](/es/clawhub/plugin-validation-fixes#sdk-session-transcript-low-level)                       |
...
- Añade un bloque `openclaw` cuando el paquete envíe un Plugin de OpenClaw.
...
- Añade metadatos de compatibilidad e instalación cuando el paquete vaya a publicarse o instalarse mediante ClawHub.
...
- Mantén esto separado de la versión del paquete. La versión del paquete describe la versión publicada del Plugin; `openclaw.compat.pluginApi` describe el contrato de la API del host.
- Consulta [campos de package.json que afectan al descubrimiento](/es/plugins/manifest#packagejson-fields-that-affect-discovery).
- Vuelve a ejecutar `clawhub package validate <path-to-plugin>`.
...
- Si `openclaw.plugin.json` también tiene `version`, actualízala para que coincida o elimina metadatos obsoletos de versión del manifiesto cuando los metadatos del paquete sean la fuente de autoridad.
...
- Mantén los puntos de entrada, compatibilidad, instalación, configuración y metadatos de catálogo del paquete en campos compatibles de `package.json#openclaw`.
- Consulta [campos de package.json que afectan al descubrimiento](/es/plugins/manifest#packagejson-fields-that-affect-discovery).
- Vuelve a ejecutar `clawhub package validate <path-to-plugin>`.
...
- Reemplaza las importaciones del barril raíz por importaciones enfocadas de subrutas públicas.
...
El plugin todavía usa el helper obsoleto de almacén de sesiones completo
`loadSessionStore`.
...
El plugin todavía usa un helper obsoleto de escritura del almacén de sesiones completo, como
...
- Usa helpers de identidad de transcripción o de destino cuando el código esté preparando una
...
- Usa `resolveSessionTranscriptIdentity(...)` cuando el código solo necesite la identidad pública
  de sesión.
- Usa `resolveSessionTranscriptTarget(...)` cuando el código necesite un destino estructurado de
  operación de transcripción.
- Evita leer o construir directamente destinos de archivo de transcripción heredados.
- Mantén el helper heredado solo mientras tu rango de compatibilidad declarado todavía
...
- Usa `appendSessionTranscriptMessageByIdentity(...)` para agregar entradas a la transcripción.
...
- Mantén `providerAuthEnvVars` solo como metadatos de compatibilidad mientras tu rango admitido
...
- Replica la configuración de canal basada en variables de entorno en los metadatos actuales de setup, configuración de canal o
