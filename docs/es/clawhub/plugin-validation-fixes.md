...
read_when:
    ...
    - ClawHub rechazó o emitió una advertencia en la publicación de un paquete de plugin
...
x-i18n:
    generated_at: "2026-07-04T15:07:41Z"
    model: gpt-5.5
...
No cubre los hallazgos internos de cobertura del Inspector de Plugin. Si un informe completo
...
| `manifest-unknown-fields`               | [Elimina campos del manifiesto no compatibles](/es/clawhub/plugin-validation-fixes#manifest-unknown-fields)                                  |
...
| `sdk-session-store-write`               | [Reemplaza las escrituras al almacén de sesión completo](/es/clawhub/plugin-validation-fixes#sdk-session-store-write)                                  |
...
| `sdk-session-transcript-low-level`      | [Reemplaza los helpers de bajo nivel de transcripción](/es/clawhub/plugin-validation-fixes#sdk-session-transcript-low-level)                       |
...
- Agrega un bloque `openclaw` cuando el paquete distribuye un Plugin de OpenClaw.
- Usa [Creación de plugins](/es/plugins/building-plugins) para ver un ejemplo mínimo de paquete
...
- Usa la versión de la API de Plugin de OpenClaw o el piso semver contra el que compilaste
  y probaste.
...
- Mantén `name` legible para humanos y mantén `id` como el identificador de máquina estable.
...
- Usa [Convenciones de importación](/es/plugins/building-plugins#import-conventions) y
  [subrutas del SDK de Plugin](/es/plugins/sdk-subpaths) para encontrar la importación acotada.
- Vuelve a ejecutar `clawhub package validate <path-to-plugin>`.
...
El plugin todavía usa el helper obsoleto para todo el almacén de sesiones
`loadSessionStore`.

- Usa `getSessionEntry(...)` o `listSessionEntries(...)` al leer el estado de la sesión.
- Usa `patchSessionEntry(...)` o `upsertSessionEntry(...)` al escribir el estado de la sesión.
- Evita cargar, modificar y guardar todo el objeto del almacén de sesiones.
...
El plugin todavía usa un helper obsoleto de escritura de todo el almacén de sesiones, como
...
- Evita cargar, modificar y guardar todo el objeto del almacén de sesiones.
- Mantén los helpers de escritura de todo el almacén solo mientras tu rango de compatibilidad declarado
...
- Usa `appendSessionTranscriptMessageByIdentity(...)` para añadir transcripciones.
- Usa `publishSessionTranscriptUpdateByIdentity(...)` para notificaciones de actualización de transcripción.
- Prefiere la superficie estructurada del runtime de transcripción para que OpenClaw pueda aplicar los
  límites de transacción correctos y el manejo de identidad.
...
- Mantén `providerAuthEnvVars` solo como metadatos de compatibilidad mientras tu rango compatible de
  OpenClaw aún lo necesite.
...
- Mantén `channelEnvVars` solo como metadatos de compatibilidad mientras las versiones anteriores compatibles de
