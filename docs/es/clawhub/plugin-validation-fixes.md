...
read_when:
    ...
    - ClawHub rechazó o emitió una advertencia sobre la publicación de un paquete de plugin
    - Estás actualizando los metadatos del paquete de Plugin antes del lanzamiento
summary: Corrige los hallazgos de validación del paquete Plugin de ClawHub antes de publicar
title: Correcciones de validación de Plugin
x-i18n:
    generated_at: "2026-07-03T13:15:37Z"
    model: gpt-5.5
...
No cubre los hallazgos internos de cobertura del Plugin Inspector. Si un informe completo contiene códigos de mantenimiento del escáner sin orientación de remediación para el autor, esos son para los mantenedores de OpenClaw, no para los autores de Plugin.
...
| `sdk-session-transcript-file-target`    | [Reemplaza los destinos heredados de archivo de transcripción](/es/clawhub/plugin-validation-fixes#sdk-session-transcript-file-target)                   |
...
- Añade `openclaw.extensions` para puntos de entrada de Plugin nativo.
...
- Mantén esto separado de la versión del paquete. La versión del paquete describe la publicación del Plugin; `openclaw.compat.pluginApi` describe el contrato de la API del host.
- Consulta [campos de package.json que afectan al descubrimiento](/es/plugins/manifest#packagejson-fields-that-affect-discovery).
- Vuelve a ejecutar `clawhub package validate <path-to-plugin>`.
...
- Si `openclaw.plugin.json` también tiene `version`, actualízalo para que coincida o elimina los metadatos de versión de manifiesto obsoletos cuando los metadatos del paquete sean la fuente autorizada.
...
- Mantén `name` legible para personas y mantén `id` como el identificador estable para máquinas.
- Consulta [Manifiesto de Plugin](/es/plugins/manifest).
- Vuelve a ejecutar `clawhub package validate <path-to-plugin>`.
...
- Usa `openclaw/plugin-sdk/channel-core` para helpers de punto de entrada de canal.
...
- Reemplaza las importaciones internas reservadas del SDK de OpenClaw por subrutas públicas documentadas de
  `openclaw/plugin-sdk/*`.
...
- Evita cargar, mutar y guardar todo el objeto de almacén de sesión.
...
El plugin aún usa un helper obsoleto de escritura de almacén de sesión completo, como
...
- Usa helpers de identidad de transcripción o destino cuando el código esté preparando una
...
- Conserva el helper heredado solo mientras tu rango de compatibilidad declarado siga
...
- Conserva `before_agent_start` solo mientras tu rango de compatibilidad declarado siga
  admitiendo versiones anteriores de OpenClaw que lo requieran.
...
- Conserva `providerAuthEnvVars` solo como metadatos de compatibilidad mientras tu rango compatible
...
- Refleja el setup de canal impulsado por variables de entorno en los metadatos actuales de setup, configuración de canal o
...
- Elimina la URL del esquema si es solo consultiva.
