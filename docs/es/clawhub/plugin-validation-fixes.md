...
ClawHub valida los paquetes de Plugin antes de publicar y también puede mostrar hallazgos de escaneos automatizados de paquetes. Esta página cubre los hallazgos orientados al autor, es decir, hallazgos que el autor del Plugin puede corregir en los metadatos del paquete, el manifiesto, las importaciones del SDK o el artefacto publicado.

No cubre los hallazgos internos de cobertura de Plugin Inspector. Si un informe completo contiene códigos de mantenimiento del escáner sin orientación de corrección para el autor, esos están destinados a mantenedores de OpenClaw en lugar de autores de Plugin.
...
| `package-openclaw-unsupported-metadata` | [Elimina los metadatos de paquete OpenClaw no compatibles](/es/clawhub/plugin-validation-fixes#package-openclaw-unsupported-metadata) |
...
| `manifest-name-missing`                 | [Añade un nombre para mostrar en el manifiesto](/es/clawhub/plugin-validation-fixes#manifest-name-missing)                    |
...
| `sdk-session-file-helper`               | [Reemplaza helpers de ruta de archivo de sesión](/es/clawhub/plugin-validation-fixes#sdk-session-file-helper)                 |
...
| `provider-auth-env-vars`                | [Mueve las variables de entorno del proveedor a metadatos de configuración](/es/clawhub/plugin-validation-fixes#provider-auth-env-vars) |
...
Los metadatos del paquete existen, pero no declaran un punto de entrada de tiempo de ejecución de OpenClaw.
...
- Consulta [Puntos de entrada de Plugin](/es/plugins/sdk-entrypoints) y [campos de package.json que afectan al descubrimiento](/es/plugins/manifest#packagejson-fields-that-affect-discovery).
- Vuelve a ejecutar `clawhub package validate <path-to-plugin>`.
...
- Usa la versión de API del Plugin de OpenClaw o el mínimo semver contra el que compilaste y probaste.
...
- Revisa cualquier metadato de compilación de OpenClaw en el paquete, como la versión de OpenClaw utilizada durante la publicación.
...
- Si `openclaw.plugin.json` también tiene `version`, actualízala para que coincida o elimina los metadatos de versión del manifiesto obsoletos cuando los metadatos del paquete sean la autoridad.
...
El bloque `package.json#openclaw` contiene campos que no son metadatos de paquete OpenClaw compatibles.

- Elimina campos no compatibles como `openclaw.bundle`.
- Mantén los metadatos de Plugin nativos en `openclaw.plugin.json`.
- Mantén los puntos de entrada del paquete, la compatibilidad, la instalación, la configuración y los metadatos de catálogo en campos compatibles de `package.json#openclaw`.
- Consulta [campos de package.json que afectan al descubrimiento](/es/plugins/manifest#packagejson-fields-that-affect-discovery).
- Vuelve a ejecutar `clawhub package validate <path-to-plugin>`.
...
- Corrige metadatos de paquete inválidos, scripts de ciclo de vida rotos o entradas de archivos que hagan fallar el empaquetado.
...
Al artefacto empaquetado le faltan metadatos de OpenClaw que existen en tu paquete de origen.
...
- Sustituye las importaciones del barril raíz por importaciones enfocadas de subrutas públicas.
...
- Sustituye las importaciones internas reservadas del SDK de OpenClaw por subrutas públicas documentadas
  `openclaw/plugin-sdk/*`.
...
El plugin aún usa el ayudante obsoleto de almacén de sesión completo
`loadSessionStore`.
...
- Evita cargar, modificar y guardar todo el objeto del almacén de sesión.
...
El plugin aún usa un ayudante obsoleto de escritura de almacén de sesión completo, como
...
- Evita cargar, modificar y guardar todo el objeto del almacén de sesión.
- Mantén los ayudantes de escritura de almacén completo solo mientras tu rango de compatibilidad declarado
...
- Usa la identidad de transcripción o ayudantes de destino cuando el código prepare una
...
- Mantén el ayudante heredado solo mientras tu rango de compatibilidad declarado todavía
...
- Mantén `before_agent_start` solo mientras tu rango de compatibilidad declarado todavía
  admita versiones anteriores de OpenClaw que lo requieran.
...
- Duplica el setup del canal impulsado por variables de entorno en el setup actual, la configuración de canal o
