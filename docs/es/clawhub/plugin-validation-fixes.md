...
read_when:
    ...
    - Estás actualizando los metadatos del paquete del plugin antes de la versión
...
x-i18n:
    generated_at: "2026-07-01T15:19:15Z"
    model: gpt-5.5
...
| `sdk-session-store-write`               | [Reemplazar escrituras del almacén de sesión completo](/es/clawhub/plugin-validation-fixes#sdk-session-store-write)                                  |
...
- Usa [Crear Plugins](/es/plugins/building-plugins) para un ejemplo mínimo de paquete
...
- Añade metadatos de compatibilidad e instalación cuando el paquete vaya a publicarse o
  instalarse a través de ClawHub.
...
- Si `openclaw.plugin.json` también tiene `version`, actualízala para que coincida o elimina
  los metadatos de versión obsoletos del manifiesto cuando los metadatos del paquete sean autoritativos.
...
- Mueve los metadatos de paquete o instalación a los campos compatibles de `package.json#openclaw`
...
- Usa `appendSessionTranscriptMessageByIdentity(...)` para añadir entradas a la transcripción.
...
- Conserva `providerAuthEnvVars` solo como metadatos de compatibilidad mientras tu rango
...
- Conserva `channelEnvVars` solo como metadatos de compatibilidad mientras las versiones antiguas compatibles
...
- Elimina la URL del esquema si solo es informativa.
