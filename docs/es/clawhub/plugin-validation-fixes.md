...
x-i18n:
    generated_at: "2026-07-06T21:47:02Z"
    model: gpt-5.5
...
ClawHub valida los paquetes de Plugin antes de publicarlos y también puede mostrar hallazgos de escaneos automatizados de paquetes. Esta página cubre los hallazgos dirigidos al autor, es decir, hallazgos que el autor del Plugin puede corregir en los metadatos del paquete, el manifiesto, las importaciones del SDK o el artefacto publicado.

No cubre los hallazgos internos de cobertura de Plugin Inspector. Si un informe completo contiene códigos de mantenimiento del escáner sin guía de corrección para el autor, esos son para los mantenedores de OpenClaw, no para los autores de Plugins.
...
- Añade un bloque `openclaw` cuando el paquete incluye un Plugin de OpenClaw.
- Usa [Creación de Plugins](/es/plugins/building-plugins) para ver un ejemplo de paquete mínimo y [Manifiesto de Plugin](/es/plugins/manifest#manifest-versus-packagejson) para la separación entre paquete y manifiesto.
...
- Añade metadatos de compatibilidad e instalación cuando el paquete se vaya a publicar o instalar a través de ClawHub.
...
- Rellena `openclaw.install` con la fuente de instalación admitida, como `clawhubSpec`, `npmSpec` o `localPath`.
...
- Comprueba cualquier metadato de compilación de OpenClaw en el paquete, como la versión de OpenClaw usada durante el lanzamiento.
...
- Mantén los puntos de entrada, compatibilidad, instalación, configuración y metadatos de catálogo del paquete en campos admitidos de `package.json#openclaw`.
- Consulta [campos de package.json que afectan al descubrimiento](/es/plugins/manifest#packagejson-fields-that-affect-discovery).
- Vuelve a ejecutar `clawhub package validate <path-to-plugin>`.
...
- Mantén `name` legible para humanos y conserva `id` como el id estable para máquina.
...
El Plugin todavía usa el helper obsoleto de almacén completo de sesión
`loadSessionStore`.
...
El Plugin todavía usa un helper de escritura obsoleto de almacén completo de sesión, como
...
- Mantén los metadatos de variables de entorno de canal como declarativos para que OpenClaw pueda inspeccionar el estado de setup
...
- Mantén `channelEnvVars` solo como metadatos de compatibilidad mientras las versiones antiguas admitidas
