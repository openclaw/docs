---
read_when:
    - Ejecutaste clawhub package validate y necesitas corregir hallazgos del plugin
...
x-i18n:
    generated_at: "2026-07-05T01:54:26Z"
    model: gpt-5.5
...
escaneos automatizados de paquetes. Esta página cubre los hallazgos dirigidos al autor, es decir,
hallazgos que el autor del Plugin puede corregir en los metadatos del paquete, el manifiesto, las importaciones del SDK
o el artefacto publicado.

No cubre hallazgos internos de cobertura de Plugin Inspector. Si un informe completo
contiene códigos de mantenimiento del escáner sin guía de remediación para el autor, esos
son para los mantenedores de OpenClaw y no para los autores de Plugin.
...
| `sdk-session-store-write`               | [Reemplaza las escrituras en el almacén de sesión completo](/es/clawhub/plugin-validation-fixes#sdk-session-store-write)                                  |
| `sdk-session-file-helper`               | [Reemplaza los helpers de ruta de archivo de sesión](/es/clawhub/plugin-validation-fixes#sdk-session-file-helper)                                   |
...
| `sdk-session-transcript-low-level`      | [Reemplaza los helpers de transcripción de bajo nivel](/es/clawhub/plugin-validation-fixes#sdk-session-transcript-low-level)                       |
...
- Añade `openclaw.runtimeExtensions` cuando el paquete publicado deba cargar JavaScript
  compilado.
- Mantén todas las rutas de punto de entrada dentro del directorio del paquete.
...
- Usa la versión de API de Plugin de OpenClaw o el mínimo semver contra el que compilaste
  y probaste.
- Mantén esto separado de la versión del paquete. La versión del paquete describe la
  publicación del Plugin; `openclaw.compat.pluginApi` describe el contrato de API del host.
- Consulta [campos de package.json que afectan al descubrimiento](/es/plugins/manifest#packagejson-fields-that-affect-discovery).
- Vuelve a ejecutar `clawhub package validate <path-to-plugin>`.
...
- Si `openclaw.plugin.json` también tiene `version`, actualízala para que coincida o elimina
  metadatos de versión del manifiesto obsoletos cuando los metadatos del paquete sean autoritativos.
...
- Mantén los puntos de entrada, compatibilidad, instalación, configuración y metadatos de catálogo
...
- Mantén `name` legible para humanos y mantén `id` como el identificador estable de máquina.
- Consulta [Manifiesto de Plugin](/es/plugins/manifest).
- Vuelve a ejecutar `clawhub package validate <path-to-plugin>`.
...
- Mueve los metadatos de paquete o instalación a campos `package.json#openclaw`
  admitidos en lugar de al manifiesto.
- Vuelve a ejecutar `clawhub package validate <path-to-plugin>`.
...
- Reemplaza las importaciones del barrel raíz con importaciones enfocadas de subrutas públicas.
- Usa `openclaw/plugin-sdk/plugin-entry` para `definePluginEntry`.
- Usa `openclaw/plugin-sdk/channel-core` para los ayudantes de punto de entrada de canal.
...
- Si el comportamiento no tiene SDK público, mantén el ayudante dentro de tu paquete o
...
- Evita cargar, mutar y guardar el objeto completo del almacén de sesiones.
- Mantén los ayudantes de escritura del almacén completo solo mientras tu rango de compatibilidad declarado
...
El Plugin todavía usa ayudantes obsoletos de rutas de archivo de sesión, como
...
- Usa la identidad de transcripción o los ayudantes de destino cuando el código esté preparando una
  operación de transcripción.
- No conserves ni dependas de rutas de archivo de transcripción heredadas.
- Consulta [API en tiempo de ejecución](/es/plugins/sdk-runtime#agent-session-state) y
  [subrutas del SDK de Plugin](/es/plugins/sdk-subpaths).
- Vuelve a ejecutar `clawhub package validate <path-to-plugin>`.
...
El Plugin todavía usa ayudantes obsoletos de bajo nivel para transcripciones, como
...
- Usa `publishSessionTranscriptUpdateByIdentity(...)` para notificaciones de actualización
  de transcripción.
- Prefiere la superficie estructurada de transcripción en tiempo de ejecución para que OpenClaw pueda aplicar los
...
El manifiesto todavía usa metadatos de autenticación de proveedor heredados `providerAuthEnvVars`.
...
- Mantén `providerAuthEnvVars` solo como metadatos de compatibilidad mientras tu rango de
  OpenClaw admitido aún los necesite.
...
- Refleja el setup de canal impulsado por variables de entorno en el setup actual, la configuración de canal o
  los metadatos de canal de paquete usados por la forma de tu Plugin.
- Mantén `channelEnvVars` solo como metadatos de compatibilidad mientras versiones anteriores de
  OpenClaw admitidas aún lo requieran.
