...
x-i18n:
    generated_at: "2026-07-02T17:30:44Z"
    model: gpt-5.5
...
No cubre los hallazgos internos de cobertura de Plugin Inspector. Si un informe completo contiene códigos de mantenimiento del escáner sin guía de remediación para el autor, esos son para los mantenedores de OpenClaw, no para los autores de Plugin.
...
| `package-openclaw-unsupported-metadata` | [Elimina metadatos de paquete de OpenClaw no compatibles](/es/clawhub/plugin-validation-fixes#package-openclaw-unsupported-metadata)          |
...
| `manifest-unknown-fields`               | [Elimina campos de manifiesto no compatibles](/es/clawhub/plugin-validation-fixes#manifest-unknown-fields)                                  |
| `manifest-unknown-contracts`            | [Elimina claves de contrato no compatibles](/es/clawhub/plugin-validation-fixes#manifest-unknown-contracts)                                 |
...
| `sdk-session-store-write`               | [Reemplaza escrituras en el almacén de sesión completo](/es/clawhub/plugin-validation-fixes#sdk-session-store-write)                                  |
| `sdk-session-file-helper`               | [Reemplaza ayudantes de rutas de archivos de sesión](/es/clawhub/plugin-validation-fixes#sdk-session-file-helper)                                   |
...
| `provider-auth-env-vars`                | [Mueve variables de entorno del proveedor a los metadatos de configuración](/es/clawhub/plugin-validation-fixes#provider-auth-env-vars)                             |
| `channel-env-vars`                      | [Replica variables de entorno del canal en los metadatos actuales](/es/clawhub/plugin-validation-fixes#channel-env-vars)                                |
...
| `unrecognized-security-manifest`        | [Elimina archivos de manifiesto de seguridad no compatibles](/es/clawhub/plugin-validation-fixes#unrecognized-security-manifest)                   |
...
- Usa [Crear Plugins](/es/plugins/building-plugins) para ver un ejemplo de paquete mínimo y [Manifiesto de Plugin](/es/plugins/manifest#manifest-versus-packagejson) para la separación entre paquete y manifiesto.
...
- Consulta [Puntos de entrada de Plugin](/es/plugins/sdk-entrypoints) y [campos de package.json que afectan el descubrimiento](/es/plugins/manifest#packagejson-fields-that-affect-discovery).
- Vuelve a ejecutar `clawhub package validate <path-to-plugin>`.
...
- Rellena `openclaw.install` con el origen de instalación compatible, como `clawhubSpec`, `npmSpec` o `localPath`.
...
- Usa la versión de la API de Plugin de OpenClaw o el límite inferior de semver contra los que compilaste y probaste.
- Mantén esto separado de la versión del paquete. La versión del paquete describe la versión del Plugin; `openclaw.compat.pluginApi` describe el contrato de API del host.
- Consulta [campos de package.json que afectan el descubrimiento](/es/plugins/manifest#packagejson-fields-that-affect-discovery).
- Vuelve a ejecutar `clawhub package validate <path-to-plugin>`.
...
- Si `openclaw.plugin.json` también tiene `version`, actualízalo para que coincida o elimina metadatos obsoletos de versión del manifiesto cuando los metadatos del paquete sean la fuente de referencia.
...
El bloque `package.json#openclaw` contiene campos que no son metadatos de paquete de OpenClaw compatibles.

- Elimina campos no compatibles como `openclaw.bundle`.
- Mantén los metadatos de Plugin nativo en `openclaw.plugin.json`.
- Mantén los puntos de entrada, la compatibilidad, la instalación, la configuración y los metadatos de catálogo del paquete en campos compatibles de `package.json#openclaw`.
- Consulta [campos de package.json que afectan el descubrimiento](/es/plugins/manifest#packagejson-fields-that-affect-discovery).
- Vuelve a ejecutar `clawhub package validate <path-to-plugin>`.
...
El paquete puede empaquetarse, pero el artefacto empaquetado no incluye los archivos de punto de entrada declarados en `package.json#openclaw`.
...
- Mantén `name` legible para humanos y conserva `id` como el id estable para máquinas.
- Consulta [Manifiesto de Plugin](/es/plugins/manifest).
- Vuelve a ejecutar `clawhub package validate <path-to-plugin>`.
...
- Usa `openclaw/plugin-sdk/channel-core` para ayudantes de entrada de canal.
- Usa [Convenciones de importación](/es/plugins/building-plugins#import-conventions) y
  [Subrutas del SDK de Plugin](/es/plugins/sdk-subpaths) para encontrar la importación estrecha.
- Vuelve a ejecutar `clawhub package validate <path-to-plugin>`.
...
- Sustituye las importaciones internas reservadas del SDK de OpenClaw por subrutas públicas
  documentadas de `openclaw/plugin-sdk/*`.
- Si el comportamiento no tiene SDK público, conserva el ayudante dentro de tu paquete o
...
El Plugin todavía usa el ayudante obsoleto de almacén de sesión completo
`loadSessionStore`.
...
El Plugin todavía usa un ayudante obsoleto de escritura de almacén de sesión completo, como
...
- Evita cargar, mutar y guardar todo el objeto del almacén de sesión.
- Conserva los ayudantes de escritura de almacén completo solo mientras tu rango de compatibilidad declarado
...
El Plugin todavía usa ayudantes obsoletos de ruta de archivo de sesión, como
...
El Plugin todavía usa el ayudante obsoleto de destino de archivo de transcripción
`resolveSessionTranscriptLegacyFileTarget`.

- Usa `resolveSessionTranscriptIdentity(...)` cuando el código solo necesite la identidad
...
- Conserva el ayudante heredado solo mientras tu rango de compatibilidad declarado todavía
...
El Plugin todavía usa ayudantes de transcripción de bajo nivel obsoletos, como
`appendSessionTranscriptMessage` o `emitSessionTranscriptUpdate`.

- Usa `appendSessionTranscriptMessageByIdentity(...)` para anexar a transcripciones.
...
- Conserva los ayudantes de transcripción de bajo nivel solo mientras tu rango de compatibilidad declarado
  todavía admita versiones anteriores de OpenClaw que los requieran.
- Consulta [API de runtime](/es/plugins/sdk-runtime#agent-session-state) y
  [Subrutas del SDK de Plugin](/es/plugins/sdk-subpaths).
- Vuelve a ejecutar `clawhub package validate <path-to-plugin>`.
...
- Conserva `providerAuthEnvVars` solo como metadatos de compatibilidad mientras tu rango admitido
  de OpenClaw todavía lo necesite.
...
- Refleja el setup de canal basado en variables de entorno en los metadatos actuales de setup, configuración de canal o
  metadatos de canal del paquete que usa la forma de tu Plugin.
