...
read_when:
    ...
    - Decidir si instalar un skill o Plugin
...
Las auditorías de seguridad de ClawHub te ayudan a decidir si una habilidad o plugin es lo bastante seguro para instalarlo. Muestran qué hace una versión, qué autoridad solicita y si algo merece atención adicional antes de que pueda acceder a archivos, cuentas, credenciales, código o servicios externos.
...
Un `Pass` es tranquilizador, pero no reemplaza tu propio criterio. Esto importa especialmente para herramientas que pueden publicar contenido, editar datos, ejecutar comandos, leer archivos o acceder a sistemas de producción.
...
Por ejemplo, una habilidad de publicación puede mostrar `Review` con riesgo `Medium`. Eso no significa que sea maliciosa. Significa que la habilidad parece alineada con su propósito, pero puede actuar con autoridad significativa sobre la cuenta.
...
Los hallazgos pueden estar etiquetados como `Info`, `Low`, `Medium`, `High` o `Critical`. Los hallazgos de mayor gravedad contribuyen con más fuerza al nivel de riesgo y al estado de auditoría.
...
ClawHub usa VirusTotal como telemetría de malware en la pila de auditoría. VirusTotal es un estándar de confianza en la industria para la reputación de archivos y el escaneo de malware, y nuestra colaboración permite que ClawHub agregue inteligencia de seguridad más amplia a la revisión de habilidades y plugins.

VirusTotal es especialmente útil para artefactos maliciosos conocidos, coincidencias de motores y señales de reputación que complementan la revisión consciente de agentes de ClawHub. Cuando hay recuentos de motores de proveedores disponibles, la auditoría los resume en lenguaje claro, como:
