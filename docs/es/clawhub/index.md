...
read_when:
    ...
    - Elección entre los flujos de CLI de OpenClaw y ClawHub
...
title: ClawHub
x-i18n:
    generated_at: "2026-07-19T01:51:37Z"
    model: gpt-5.6
...
| Plugins empaquetados | Paquetes de plugins preparados para la distribución de OpenClaw | `clawhub package publish <source>`           |
...
para que se pueda examinar una Skill o un plugin antes de instalarlo.
...
Las especificaciones simples de plugins compatibles con npm pueden resolverse mediante npm durante las transiciones de lanzamiento, y
`npm:<package>` continúa siendo exclusivo de npm cuando el origen debe indicarse explícitamente.

Las instalaciones de plugins validan la compatibilidad anunciada de `pluginApi` y `minGatewayVersion`
antes de instalar el archivo. Cuando una versión de un paquete publica un artefacto
...
La CLI de ClawHub se utiliza para las operaciones autenticadas en el registro:
...
Publique plugins desde una carpeta local, `owner/repo`, `owner/repo@ref` o una URL de GitHub:

```bash
clawhub package publish <source>
```

Utilice `--dry-run` para generar el plan de publicación exacto sin subir nada, y `--json`
...
`openclaw.build.openclawVersion`. Consulte [CLI](/es/clawhub/cli) para obtener la referencia completa de los comandos
y [Formato de las Skills](/es/clawhub/skill-format) para conocer los metadatos de las Skills.

## Seguridad y moderación

ClawHub es abierto de forma predeterminada: cualquiera puede subir contenido, pero para publicar se requiere una cuenta de GitHub
con la antigüedad suficiente para superar el control de subida. Las páginas públicas de detalles resumen el
estado del análisis más reciente antes de la instalación o descarga.

ClawHub ejecuta comprobaciones automatizadas sobre las Skills y las versiones de plugins publicadas. Las versiones
retenidas por el análisis o bloqueadas pueden desaparecer del catálogo público y de las superficies de instalación, pero
...
[Uso aceptable](/clawhub/acceptable-usage) para obtener información detallada sobre las políticas y su aplicación.

## Telemetría y entorno

Cuando se ejecuta `clawhub install` tras iniciar sesión, la CLI puede enviar un evento de instalación
sin garantías para que ClawHub pueda calcular el número agregado de instalaciones. Desactive esta función con:
...
[Solución de problemas](/clawhub/troubleshooting) para obtener material de referencia más detallado.
