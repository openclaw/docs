...
read_when:
    ...
    - Elegir entre flujos de CLI de openclaw y clawhub
sidebarTitle: ClawHub
summary: Resumen público de ClawHub sobre descubrimiento, instalación, publicación, seguridad y la CLI de clawhub.
title: ClawHub
x-i18n:
...
- Usa la CLI `clawhub` separada para los flujos de autenticación del registro, publicación y eliminación/restauración.
...
| -------------- | ------------------------------------------------------------ | -------------------------------------------- |
| Skills         | Paquetes de texto versionados con `SKILL.md` y archivos de soporte | `openclaw skills install @openclaw/demo`     |
...
descargas, estrellas y resúmenes de escaneos de seguridad. Las páginas públicas muestran el estado actual del registro
...
los metadatos de origen para que los comandos de actualización posteriores puedan mantenerse en ClawHub.

Usa `clawhub:<package>` cuando una instalación de plugin deba resolverse a través de ClawHub.
Las especificaciones de plugin simples y compatibles con npm pueden resolverse a través de npm durante los cambios de lanzamiento, y
`npm:<package>` se mantiene solo para npm cuando una fuente debe ser explícita.
...
Opciones de publicación comunes:
...
Los plugins de código deben incluir los metadatos de compatibilidad de OpenClaw requeridos en
...
ClawHub es abierto de forma predeterminada: cualquiera puede subir contenido, pero publicar requiere una cuenta de GitHub
...
ClawHub ejecuta comprobaciones automatizadas sobre Skills publicadas y lanzamientos de plugins. Los lanzamientos retenidos por escaneo
...
[Uso aceptable](/es/clawhub/acceptable-usage) para obtener detalles sobre políticas y aplicación.
