...
- Usa la CLI `clawhub` separada para la autenticación del registro, la publicación y los flujos de trabajo de eliminación/restauración.
...
Las especificaciones de plugin compatibles con npm sin prefijo pueden resolverse mediante npm durante los cambios de lanzamiento, y
`npm:<package>` permanece solo en npm cuando una fuente debe ser explícita.
...
ClawPack, OpenClaw prefiere el `.tgz` exacto subido con npm-pack, verifica
...
con la antigüedad suficiente para superar la puerta de subida. Las páginas públicas de detalles resumen el
estado del escaneo más reciente antes de la instalación o descarga.

ClawHub ejecuta comprobaciones automatizadas en Skills publicadas y versiones de plugins. Las versiones retenidas por escaneo
...
[Uso aceptable](/clawhub/acceptable-usage) para obtener detalles de políticas y aplicación.
...
| Variable                      | Efecto                                            |
| ----------------------------- | ------------------------------------------------- |
| `CLAWHUB_SITE`                | Anula la URL del sitio usada para iniciar sesión en el navegador. |
...
[Solución de problemas](/es/clawhub/troubleshooting) para material de referencia más profundo.
