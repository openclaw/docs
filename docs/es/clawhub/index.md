...
los metadatos de origen para que los comandos de actualización posteriores puedan permanecer en ClawHub.

Usa `clawhub:<package>` cuando la instalación de un plugin deba resolverse a través de ClawHub.
Las especificaciones de plugins sin prefijo y válidas para npm pueden resolverse a través de npm durante las transiciones de lanzamiento, y
...
artefacto ClawPack, OpenClaw prefiere el `.tgz` de npm-pack exacto subido, verifica
la cabecera de resumen de ClawHub y los bytes descargados, y registra metadatos del artefacto para
...
Usa `--dry-run` para construir el plan de publicación exacto sin subirlo, y `--json`
...
y [Formato de Skill](/es/clawhub/skill-format) para los metadatos de Skill.
...
con antigüedad suficiente para superar la puerta de subida. Las páginas de detalle públicas resumen el
estado de análisis más reciente antes de instalar o descargar.

ClawHub ejecuta comprobaciones automatizadas en Skills publicadas y versiones de plugins. Las versiones retenidas por análisis
o bloqueadas pueden desaparecer del catálogo público y de las superficies de instalación mientras
...
[Seguridad y moderación](/es/clawhub/security) para ver detalles de políticas y aplicación.
