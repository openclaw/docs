...
Las especificaciones de plugins sin prefijo y compatibles con npm pueden resolverse mediante npm durante transiciones de lanzamiento, y
...
Usa `--dry-run` para generar el plan de publicación exacto sin subirlo, y `--json`
...
[Seguridad + moderación](/es/clawhub/security) para detalles de políticas y aplicación.

## Telemetría y entorno

Cuando ejecutas `clawhub sync` con sesión iniciada, la CLI envía una instantánea mínima para que
ClawHub pueda calcular conteos de instalaciones. Desactiva esto con:
...
| `CLAWHUB_CONFIG_PATH`         | Sobrescribe dónde almacena la CLI el estado de tokens/configuración. |
