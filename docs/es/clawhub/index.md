...
title: ClawHub
x-i18n:
    generated_at: "2026-07-05T04:48:37Z"
    model: gpt-5.5
...
- Usa la CLI `clawhub` independiente para flujos de autenticación del registro, publicación y eliminación/restauración.
...
| Plugins en paquetes | Paquetes de plugins empaquetados para la distribución de OpenClaw | `clawhub package publish <source>`          |

ClawHub registra versiones semver, etiquetas como `latest`, changelogs, archivos,
...
- `--changelog <text>`: texto del changelog.
...
Usa `--dry-run` para construir el plan de publicación exacto sin subir nada, y `--json`
para obtener salida adecuada para CI.
...
con antigüedad suficiente para pasar la puerta de subida. Las páginas públicas de detalle resumen el
...
[Uso aceptable](/es/clawhub/acceptable-usage) para ver detalles sobre políticas y aplicación.
