...
title: ClawHub
x-i18n:
    generated_at: "2026-05-13T02:51:32Z"
    model: gpt-5.5
    provider: openai
...
- Usa la CLI `clawhub` separada para flujos de trabajo de autenticación del registro, publicación, eliminación/restauración y sincronización.
...
Instala la CLI de ClawHub cuando necesites flujos de trabajo autenticados con el registro, como
...
| Plugins de código | Paquetes de Plugin de OpenClaw con metadatos de compatibilidad | `openclaw plugins install clawhub:<package>` |
| Plugins de paquete | Paquetes de Plugin empaquetados para la distribución de OpenClaw | `clawhub package publish <source>`           |
...
actual del registro para que los usuarios puedan inspeccionar un Skill o Plugin antes de instalarlo.
...
`npm:<package>` permanece solo para npm cuando un origen debe ser explícito.
...
ClawPack, OpenClaw prefiere el `.tgz` exacto de npm-pack cargado, verifica
...
Usa `--dry-run` para construir el plan de publicación exacto sin cargar nada, y `--json`
...
ClawHub está abierto de forma predeterminada: cualquiera puede cargar contenido, pero publicar requiere una cuenta de GitHub
...
ClawHub ejecuta comprobaciones automatizadas en Skills publicados y versiones de Plugin. Las versiones
retenidas por análisis o bloqueadas pueden desaparecer del catálogo público y de las superficies de instalación mientras
...
[Seguridad y moderación](/es/clawhub/security) para detalles sobre políticas y aplicación.
