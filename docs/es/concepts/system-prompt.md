...
Usa contribuciones propiedad del proveedor para ajustes específicos por familia de modelos. Mantén la
mutación de prompt heredada `before_prompt_build` para compatibilidad o cambios de prompt
realmente globales, no para el comportamiento normal del proveedor.
...
- **Sesgo de ejecución**: orientación compacta de seguimiento: actuar en el turno ante
  solicitudes accionables, continuar hasta terminar o quedar bloqueado, recuperarse de resultados débiles de herramientas,
  comprobar estado mutable en vivo y verificar antes de finalizar.
- **Seguridad**: recordatorio breve de límites para evitar comportamiento de búsqueda de poder o eludir supervisión.
...
- **Autoactualización de OpenClaw**: cómo inspeccionar la configuración de forma segura con
  ...
  completa con `config.apply` y ejecutar `update.run` solo ante solicitud explícita del usuario.
...
- **Sandbox** (cuando está habilitado): indica runtime aislado, rutas de sandbox y si está disponible la ejecución elevada.
...
- **Heartbeat**: prompt de Heartbeat y comportamiento de acuse, cuando los Heartbeat están habilitados para el agente predeterminado.
...
**Reacciones**, **Heartbeat** y **Runtime**, se agregan por debajo de ese límite
...
- usar `exec` / `process` solo para comandos que empiezan ahora y siguen ejecutándose
  en segundo plano
- cuando la activación automática al completar está habilitada, iniciar el comando una sola vez y confiar en
  la ruta de activación basada en envío cuando emita salida o falle
- usar `process` para registros, estado, entrada o intervención cuando necesites
  inspeccionar un comando en ejecución
- si la tarea es mayor, preferir `sessions_spawn`; la finalización del subagente se basa en envío
  y se anuncia automáticamente al solicitante
...
Los límites de seguridad en el prompt del sistema son orientativos. Guían el comportamiento del modelo, pero no hacen cumplir políticas. Usa política de herramientas, aprobaciones de ejecución, sandboxing y listas de canales permitidos para la aplicación estricta; los operadores pueden deshabilitarlos por diseño.
...
- `minimal`: usado para subagentes; omite **Skills**, **Recuperación de memoria**, **Autoactualización de OpenClaw
  Self-Update**, **Alias de modelo**, **Identidad del usuario**, **Etiquetas de respuesta**,
  **Mensajería**, **Respuestas silenciosas** y **Heartbeat**. Herramientas, **Seguridad**,
...
específico de la conversación resuelto. Esto evita repetir mecánicas de tokens
...
vinculadas al modelo para turnos directos de Telegram, grupos de Discord y Heartbeat. Esa pila
incluye una fixture fijada de prompt del modelo Codex `gpt-5.5` generada a partir de la
forma del catálogo/caché de modelos de Codex, el texto de desarrollador de permisos de la ruta feliz de Codex,
...
Actualiza la fixture fijada del prompt del modelo Codex con
...
memorias, instrucciones de app/plugin e instrucciones integradas predeterminadas de modo de colaboración
...
de límite para que los cambios de prompt y las actualizaciones de instantáneas permanezcan vinculados al mismo
PR.

## Inyección de arranque del espacio de trabajo

Los archivos de arranque se recortan y se agregan bajo **Contexto del proyecto** para que el modelo vea identidad y contexto de perfil sin necesitar lecturas explícitas:
...
se aplique una compuerta específica de archivo. `HEARTBEAT.md` se omite en ejecuciones normales cuando
los Heartbeat están deshabilitados para el agente predeterminado o
...
los archivos de arranque inyectados (por ejemplo, intercambiar `SOUL.md` por una persona alternativa).
...
Consulta [Fecha y hora](/es/date-time) para conocer todos los detalles de comportamiento.
...
Las Skills incluidas por plugins solo son elegibles cuando su plugin propietario está habilitado.
