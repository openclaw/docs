...
x-i18n:
    generated_at: "2026-05-02T05:25:06Z"
    model: gpt-5.5
    provider: openai
...
- Extrae el uso/cuota de los proveedores directamente de sus endpoints de uso.
...
- La salida de estado legible por humanos se normaliza a `X% left`, incluso cuando una
  API de origen informa cuota consumida, cuota restante o solo conteos sin procesar.
- `/status` a nivel de sesión y `session_status` pueden recurrir a la entrada de uso de
  transcripción más reciente cuando la instantánea de la sesión en vivo es escasa. Ese
  respaldo completa los contadores faltantes de tokens/caché, puede recuperar la etiqueta
  del modelo de runtime activo y prefiere el total más grande orientado al prompt cuando
  faltan los metadatos de sesión o son menores. Los valores en vivo distintos de cero existentes siguen teniendo prioridad.

## Dónde aparece

- `/status` en chats: tarjeta de estado con abundantes emojis con tokens de sesión + costo estimado (solo clave de API). El uso del proveedor se muestra para el **proveedor del modelo actual** cuando está disponible como una ventana normalizada `X% left`.
...
- CLI: `openclaw channels list` imprime la misma instantánea de uso junto a la configuración del proveedor (usa `--no-usage` para omitirla).
...
- **MiniMax**: clave de API o perfil de autenticación OAuth de MiniMax. OpenClaw trata
  ...
  de MiniMax, prefiere OAuth de MiniMax almacenado cuando está presente y, si no,
  recurre a `MINIMAX_CODE_PLAN_KEY`, `MINIMAX_CODING_API_KEY` o `MINIMAX_API_KEY`.
  El sondeo de uso deriva el host de Coding Plan desde `models.providers.minimax-portal.baseUrl`
  o `models.providers.minimax.baseUrl` cuando están configurados y, si no, usa el
  host CN de MiniMax.
  Los campos sin procesar `usage_percent` / `usagePercent` de MiniMax significan cuota **restante**,
  por lo que OpenClaw los invierte antes de mostrarlos; los campos basados en conteos tienen prioridad cuando
  están presentes.
  ...
  - Si el endpoint del plan de codificación devuelve `model_remains`, OpenClaw prefiere la
    entrada del modelo de chat, deriva la etiqueta de ventana a partir de marcas de tiempo cuando faltan los campos explícitos
