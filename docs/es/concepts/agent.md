...
x-i18n:
    generated_at: "2026-07-12T14:27:48Z"
    model: gpt-5.6
...
Si `agents.defaults.sandbox` está habilitado, las sesiones no principales pueden sustituirlo por
...
| `USER.md`      | Perfil del usuario y tratamiento preferido            |
| `HEARTBEAT.md` | Instrucciones específicas de Heartbeat                |
| `BOOTSTRAP.md` | Ritual único de primera ejecución (se elimina al completarlo) |
...
`BOOTSTRAP.md` solo se crea para un **espacio de trabajo completamente nuevo** (sin ningún otro archivo de arranque). Mientras esté pendiente, OpenClaw lo mantiene en el contexto del proyecto y añade al prompt del sistema instrucciones de arranque para el ritual inicial, en lugar de copiarlo en el mensaje del usuario. Si lo elimina después de completar el ritual, no se vuelve a crear en reinicios posteriores.

Después de observar un espacio de trabajo, OpenClaw también conserva en el directorio de estado un marcador de certificación para la ruta del espacio de trabajo. Si un espacio de trabajo certificado recientemente desaparece o se borra, el inicio se niega a volver a generar silenciosamente `BOOTSTRAP.md`; restaure el espacio de trabajo o realice un restablecimiento completo de la incorporación para que el espacio de trabajo y el marcador se eliminen juntos.

Para deshabilitar por completo la creación de archivos de arranque (para espacios de trabajo preconfigurados), establezca:
...
Las Skills pueden condicionarse mediante la configuración o las variables de entorno (consulte `skills` en [Configuración del Gateway](/es/gateway/configuration)).

## Límites del entorno de ejecución

El entorno de ejecución de agente integrado pertenece a OpenClaw: el descubrimiento de modelos, la conexión de herramientas, la composición de prompts, la gestión de sesiones y la entrega a canales comparten una única superficie de ejecución integrada.
...
El direccionamiento se entrega **después de que el turno actual del asistente termina de ejecutar sus
...
`/queue collect` hacen que los mensajes esperen a un turno posterior en lugar de incorporarlos.
`/queue interrupt` cancela la ejecución activa. Consulte [Cola](/es/concepts/queue)
y [Cola de direccionamiento](/es/concepts/queue-steering) para conocer el comportamiento de la cola y sus límites.
...
Controle la división flexible de bloques con `agents.defaults.blockStreamingChunk` (el valor predeterminado es de
800-1200 caracteres; prioriza los saltos de párrafo, después los saltos de línea y, por último, las oraciones).
Agrupe los fragmentos transmitidos mediante `agents.defaults.blockStreamingCoalesce` para reducir
...
Las referencias de modelos de la configuración (por ejemplo, `agents.defaults.model` y `agents.defaults.models`) se analizan dividiéndolas por la **primera** `/`.
...
- Si omite el proveedor, OpenClaw prueba primero un alias, después una coincidencia única
