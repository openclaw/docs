---
read_when:
    - Cambio del entorno de ejecución del agente, la inicialización del espacio de trabajo o el comportamiento de la sesión
...
OpenClaw incluye un **entorno de ejecución de agente integrado**: un bucle de agente incorporado, la
conexión de herramientas y el ensamblado del prompt, distinto de delegar turnos a un proceso
de arnés externo. Cada agente configurado (consulte [Enrutamiento multiagente](/es/concepts/multi-agent)
para ejecutar varios) tiene su propio espacio de trabajo, archivos de arranque y almacén
de sesiones. Esta página abarca el contrato de ese entorno de ejecución: qué debe
contener el espacio de trabajo, qué archivos se inyectan y cómo se inicializan las sesiones con él.
...
Diseño completo del espacio de trabajo y guía de copia de seguridad: [Espacio de trabajo del agente](/es/concepts/agent-workspace)
...
| Archivo        | Finalidad                                             |
| -------------- | ---------------------------------------------------- |
| `AGENTS.md`    | Instrucciones operativas y «memoria»                 |
...
| `USER.md`      | Perfil del usuario y forma de tratamiento preferida   |
...
Los archivos vacíos se omiten. Los archivos grandes se recortan y truncan con un marcador para mantener los prompts ligeros (lea el archivo para consultar el contenido completo). Si falta un archivo (excepto `MEMORY.md`), se inyecta en su lugar una única línea de marcador de «archivo ausente»; `openclaw setup` crea una plantilla predeterminada segura para él.
...
Después de observar un espacio de trabajo, OpenClaw también conserva en el directorio de estado un marcador de atestación para la ruta del espacio de trabajo. Si un espacio de trabajo atestiguado recientemente desaparece o se borra, el inicio se niega a volver a generar silenciosamente `BOOTSTRAP.md`; restaure el espacio de trabajo o realice un restablecimiento completo de la incorporación para que el espacio de trabajo y el marcador se borren juntos.

Para deshabilitar por completo la creación de archivos de arranque (para espacios de trabajo preparados previamente), establezca:
...
sujetas a la política de herramientas. `apply_patch` está habilitado de forma predeterminada para los modelos de OpenAI y está condicionado por
`tools.exec.applyPatch` (`enabled`, `workspaceOnly`, `allowModels`). `TOOLS.md` **no** controla qué herramientas existen; ofrece
orientación sobre cómo se desea que se utilicen.

## Skills

OpenClaw carga Skills desde estas ubicaciones (en orden de mayor a menor precedencia):
...
nombre plano del frontmatter, por ejemplo `foo`.
...
el ensamblado del prompt, la gestión de sesiones y la entrega por canales comparten una única
superficie de ejecución integrada.

## Sesiones

Las filas de sesión se almacenan en la base de datos SQLite de cada agente:

- `~/.openclaw/agents/<agentId>/agent/openclaw-agent.sqlite`

Los archivos JSONL de transcripciones aún pueden residir en
...
Los prompts entrantes que llegan durante una ejecución se incorporan de forma predeterminada a la ejecución actual.
...
`/queue collect` hacen que los mensajes esperen a un turno posterior en lugar de dirigir la ejecución.
`/queue interrupt` cancela la ejecución activa. Consulte [Cola](/es/concepts/queue)
y [Cola de dirección](/es/concepts/queue-steering) para conocer el comportamiento de la cola y sus límites.

La transmisión por bloques envía los bloques completados del asistente en cuanto finalizan; está
...
Controle la fragmentación flexible de bloques con `agents.defaults.blockStreamingChunk` (valor predeterminado:
...
la saturación de líneas individuales (combinación basada en inactividad antes del envío). Los canales distintos de Telegram requieren
`*.streaming.block.enabled: true` explícito para habilitar las respuestas por bloques (QQ Bot,
en cambio, transmite respuestas por bloques salvo que `channels.qqbot.streaming.mode` sea `"off"`).
...
Más información: [Transmisión y fragmentación](/es/concepts/streaming).

## Referencias de modelos

Las referencias de modelos de la configuración (por ejemplo, `agents.defaults.model` y `agents.defaults.models`) se analizan dividiéndolas en el **primer** `/`.
...
- Si se omite el proveedor, OpenClaw prueba primero un alias, después una coincidencia única
