---
read_when:
    - Trabajando en funcionalidades de canales de Discord
...
<Steps>
  ...
  <Step title="Habilitar el modo de desarrollador y recopilar tus ID">
    De vuelta en la aplicación Discord, debes habilitar el modo de desarrollador para poder copiar ID internos.
  ...
  <Step title="Permitir MD de miembros del servidor">
    Para que el emparejamiento funcione, Discord debe permitir que tu bot te envíe MD. Haz clic derecho en el **icono de tu servidor** → **Privacy Settings** → activa **Direct Messages**.
  ...
  <Step title="Configurar el token de tu bot de forma segura (no lo envíes por chat)">
...
```
    ...
    Para instalaciones de servicio administrado, ejecuta `openclaw gateway install` desde una shell donde `DISCORD_BOT_TOKEN` esté presente, o almacena la variable en `~/.openclaw/.env`, para que el servicio pueda resolver el SecretRef de entorno después de reiniciarse.
  ...
  <Step title="Configurar OpenClaw y emparejar">
    ...
    <Tabs>
      ...
      <Tab title="CLI / configuración">
        Si prefieres configuración basada en archivos, define:
...
Para llamadas salientes avanzadas (herramienta de mensajes/acciones de canal), se usa un `token` explícito por llamada para esa llamada. Esto se aplica a acciones de envío y de lectura/sondeo (por ejemplo, leer/buscar/obtener/hilo/mensajes fijados/permisos). La política de cuenta y la configuración de reintentos siguen viniendo de la cuenta seleccionada en la instantánea de tiempo de ejecución activa.
</Note>

## Recomendado: Configurar un espacio de trabajo de servidor

Una vez que los MD funcionen, puedes configurar tu servidor de Discord como un espacio de trabajo completo donde cada canal obtiene su propia sesión de agente con su propio contexto. Esto se recomienda para servidores privados donde solo estás tú y tu bot.

<Steps>
  <Step title="Agregar tu servidor a la lista de servidores permitidos">
    Esto permite que tu agente responda en cualquier canal de tu servidor, no solo en MD.

    <Tabs>
      <Tab title="Pregúntale a tu agente">
        > "Agrega mi Discord Server ID `<server_id>` a la lista de servidores permitidos"
      </Tab>
...
```
  ...
  <Step title="Permitir respuestas sin @mención">
    ...
    En canales de servidor, las respuestas finales normales del asistente permanecen privadas de forma predeterminada. La salida visible en Discord debe enviarse explícitamente con la herramienta `message`, para que el agente pueda permanecer al acecho de forma predeterminada y solo publicar cuando decida que una respuesta en el canal es útil.

    Esto significa que el modelo seleccionado debe llamar herramientas de forma fiable. Si Discord muestra que está escribiendo y los registros muestran uso de tokens pero no aparece ningún mensaje publicado, revisa el registro de sesión para ver texto del asistente con `didSendViaMessagingTool: false`. Eso significa que el modelo produjo una respuesta final privada en lugar de llamar a `message(action=send)`. Cambia a un modelo más sólido para llamadas a herramientas, o usa la configuración siguiente para restaurar las respuestas finales automáticas heredadas.
...
```
        ...
        Para restaurar las respuestas finales automáticas heredadas en salas de grupo/canal, define `messages.groupChat.visibleReplies: "automatic"`.
...
Para restringir quién puede hacer clic en un botón, establece `allowedUsers` en ese botón (ID de usuario de Discord, etiquetas o `*`). Cuando está configurado, los usuarios que no coinciden reciben una denegación efímera.

Los comandos slash `/model` y `/models` abren un selector interactivo de modelo con desplegables de proveedor, modelo y runtime compatible, además de un paso Enviar. `/models add` está obsoleto y ahora devuelve un mensaje de obsolescencia en lugar de registrar modelos desde el chat. La respuesta del selector es efímera y solo puede usarla el usuario que la invoca.
...
<Tabs>
  <Tab title="Política de DM">
    `channels.discord.dmPolicy` controla el acceso por DM. `channels.discord.allowFrom` es la allowlist canónica de DM.
    ...
    - Las cuentas con nombre heredan `channels.discord.allowFrom` cuando sus propios `allowFrom` y `dm.allowFrom` heredado no están establecidos.
    ...
    Los ID numéricos sin prefijo normalmente se resuelven como ID de canal cuando un canal predeterminado está activo, pero los ID enumerados en el `allowFrom` efectivo de DM de la cuenta se tratan como destinos de DM de usuario por compatibilidad.
...
```
    ...
    Un canal de texto de Discord no tiene una lista de miembros separada. `type: "discord.channelAudience"` modela la membresía así: el remitente de DM es miembro del guild configurado y actualmente tiene permiso efectivo `ViewChannel` en el canal configurado después de aplicar roles y sobrescrituras de canal.
...
```
    ...
    Las búsquedas fallan de forma cerrada. Si Discord devuelve `Missing Access`, la búsqueda de miembro falla o el canal pertenece a un guild diferente, el remitente de DM se trata como no autorizado.

    Activa **Server Members Intent** en el Portal para desarrolladores de Discord para el bot al usar grupos de acceso por audiencia de canal. Los DM no incluyen el estado de miembro de guild, por lo que OpenClaw resuelve el miembro mediante Discord REST en el momento de la autorización.
  ...
  <Tab title="Política de guild">
    ...
    - el guild debe coincidir con `channels.discord.guilds` (se prefiere `id`, se acepta slug)
    - allowlists opcionales de remitentes: `users` (se recomiendan ID estables) y `roles` (solo ID de rol); si cualquiera está configurada, los remitentes se permiten cuando coinciden con `users` O `roles`
    ...
    - si un guild tiene `channels` configurado, los canales no listados se deniegan
    - si un guild no tiene bloque `channels`, todos los canales de ese guild incluido en la allowlist se permiten
...
```
    ...
    Si solo estableces `DISCORD_BOT_TOKEN` y no creas un bloque `channels.discord`, la reserva en runtime es `groupPolicy="allowlist"` (con una advertencia en los registros), incluso si `channels.defaults.groupPolicy` es `open`.
  ...
  <Tab title="Menciones y DM de grupo">
    ...
    - allowlist opcional mediante `dm.groupChannels` (ID de canal o slugs)
...
Usa `bindings[].match.roles` para enrutar miembros de guild de Discord a diferentes agentes por ID de rol. Las vinculaciones basadas en roles solo aceptan ID de rol y se evalúan después de las vinculaciones peer o parent-peer y antes de las vinculaciones solo de guild. Si una vinculación también establece otros campos de coincidencia (por ejemplo `peer` + `guildId` + `roles`), todos los campos configurados deben coincidir.
...
- Es posible que los comandos sigan visibles en la interfaz de Discord para usuarios que no están autorizados; la ejecución sigue aplicando la autenticación de OpenClaw y devuelve "not authorized".
...
<AccordionGroup>
  <Accordion title="Etiquetas de respuesta y respuestas nativas">
    ...
    Nota: `off` deshabilita el hilado implícito de respuestas. Las etiquetas explícitas `[[reply_to_*]]` se siguen respetando.
    ...
    turno entrante fue un lote con debounce de varios mensajes. Esto resulta útil
...
```
    ...
    Oculta el texto sin procesar de comando/ejecución y conserva las líneas de progreso compactas:
  ...
  <Accordion title="Historial, contexto y comportamiento de hilos">
    ...
    - Las sesiones de hilo heredan la selección `/model` de nivel de sesión del canal principal solo como alternativa de modelo; las selecciones `/model` locales del hilo siguen teniendo prioridad y el historial de transcripción principal no se copia salvo que la herencia de transcripción esté habilitada.
    - `channels.discord.thread.inheritParent` (predeterminado `false`) opta a los nuevos hilos automáticos para inicializarse desde la transcripción principal. Las sobrescrituras por cuenta se encuentran en `channels.discord.accounts.<id>.thread.inheritParent`.
...
```
    ...
    - `defaultSpawnContext` controla el contexto de subagente nativo para generaciones vinculadas a hilos. Predeterminado: `"fork"`.
  ...
  <Accordion title="Vinculaciones persistentes de canales ACP">
    Para espacios de trabajo ACP estables y "siempre activos", configura vinculaciones ACP tipadas de nivel superior orientadas a conversaciones de Discord.
...
```
    ...
    - En un canal o hilo vinculado, `/new` y `/reset` reinician la misma sesión ACP en el lugar. Las vinculaciones temporales de hilos pueden sobrescribir la resolución de destino mientras estén activas.
...
```
  ...
  <Accordion title="Proxy de Gateway">
    Enruta el tráfico WebSocket del Gateway de Discord y las consultas REST de inicio (ID de aplicación + resolución de lista de permitidos) mediante un proxy HTTP(S) con `channels.discord.proxy`.
...
```
    ...
    - las consultas usan el ID de mensaje original y están restringidas por ventana temporal
...
```
  ...
  <Accordion title="Approvals in Discord">
    Discord admite la gestión de aprobaciones basada en botones en DM y, de forma opcional, puede publicar solicitudes de aprobación en el canal de origen.
    ...
    Discord habilita automáticamente las aprobaciones de ejecución nativas cuando `enabled` no está definido o es `"auto"` y se puede resolver al menos un aprobador, ya sea desde `execApprovals.approvers` o desde `commands.ownerAllowFrom`. Discord no infiere aprobadores de ejecución desde el `allowFrom` del canal, el `dm.allowFrom` heredado ni el `defaultTo` de mensajes directos. Define `enabled: false` para deshabilitar Discord explícitamente como cliente de aprobación nativo.

    Para comandos de grupo sensibles solo para propietarios, como `/diagnostics` y `/export-trajectory`, OpenClaw envía las solicitudes de aprobación y los resultados finales de forma privada. Primero intenta el DM de Discord cuando el propietario que invoca tiene una ruta de propietario de Discord; si no está disponible, recurre a la primera ruta de propietario disponible de `commands.ownerAllowFrom`, como Telegram.

    Cuando `target` es `channel` o `both`, la solicitud de aprobación es visible en el canal. Solo los aprobadores resueltos pueden usar los botones; otros usuarios reciben una denegación efímera. Las solicitudes de aprobación incluyen el texto del comando, así que habilita la entrega en canal solo en canales de confianza. Si el ID de canal no se puede derivar de la clave de sesión, OpenClaw recurre a la entrega por DM.

    Discord también renderiza los botones de aprobación compartidos que usan otros canales de chat. El adaptador nativo de Discord añade principalmente enrutamiento de DM para aprobadores y distribución a canales.
    ...
    tiempo de ejecución está activo pero no se puede entregar una tarjeta nativa a ningún destino,
...
Las puertas de acción viven en `channels.discord.actions.*`.
...
- `channels.discord.ui.components.accentColor` define el color de énfasis usado por los contenedores de componentes de Discord (hex).
...
Discord tiene dos superficies de voz distintas: **canales de voz** en tiempo real (conversaciones continuas) y **archivos adjuntos de mensajes de voz** (el formato de vista previa con forma de onda). El gateway admite ambas.
...
- `voice.model` sobrescribe el LLM usado solo para las respuestas de canal de voz de Discord. Déjalo sin definir para heredar el modelo del agente enrutado.
...
- La voz de Discord es opcional para configuraciones solo de texto; define `channels.discord.voice.enabled=true` (o conserva un bloque `channels.discord.voice` existente) para habilitar los comandos `/vc`, el tiempo de ejecución de voz y la intención de gateway `GuildVoiceStates`.
- `channels.discord.intents.voiceStates` puede sobrescribir explícitamente la suscripción a la intención de estado de voz. Déjalo sin definir para que la intención siga la habilitación efectiva de voz.
...
- OpenClaw también observa los fallos de descifrado de recepción y se recupera automáticamente saliendo y volviendo a entrar al canal de voz tras fallos repetidos en una ventana corta.
- Si los registros de recepción muestran repetidamente `DecryptionFailed(UnencryptedWhenPassthroughDisabled)` después de actualizar, recopila un informe de dependencias y registros. La línea incluida de `@discordjs/voice` incluye la corrección de relleno upstream de la PR #11449 de discord.js, que cerró el issue #11419 de discord.js.
...
- La transcripción se envía a través de la entrada y el enrutamiento de Discord mientras el LLM de respuesta se ejecuta con una política de salida de voz que oculta la herramienta `tts` del agente y pide texto devuelto, porque la voz de Discord posee la reproducción final de TTS.
...
<AccordionGroup>
  <Accordion title="Used disallowed intents or bot sees no guild messages">
    ...
    - reinicia el gateway después de cambiar intenciones
  ...
  <Accordion title="Guild messages blocked unexpectedly">
    ...
    - verifica la lista de permitidos del servidor bajo `channels.discord.guilds`
...
```
  ...
  <Accordion title="Require mention false but still blocked">
    ...
    - `requireMention` configurado en el lugar equivocado (debe estar bajo `channels.discord.guilds` o la entrada de canal)
  ...
  <Accordion title="Long-running Discord turns or duplicate replies">
    ...
    Discord no aplica un tiempo de espera propiedad del canal a los turnos de agente en cola. Los listeners de mensajes transfieren el trabajo de inmediato, y las ejecuciones en cola de Discord conservan el orden por sesión hasta que el ciclo de vida de sesión/herramienta/tiempo de ejecución se completa o aborta el trabajo.
...
```
  ...
  <Accordion title="Advertencias de tiempo de espera en la búsqueda de metadatos del Gateway">
    OpenClaw obtiene los metadatos `/gateway/bot` de Discord antes de conectarse. Los fallos transitorios recurren a la URL predeterminada del gateway de Discord y tienen limitación de frecuencia en los registros.
  ...
  <Accordion title="Reinicios por tiempo de espera de READY del Gateway">
    ...
    - alternativa de env de tiempo de ejecución cuando la configuración no está definida: `OPENCLAW_DISCORD_RUNTIME_READY_TIMEOUT_MS`
  ...
  <Accordion title="Discordancias en la auditoría de permisos">
    ...
    Si usas claves slug, la coincidencia en tiempo de ejecución aún puede funcionar, pero probe no puede verificar completamente los permisos.
...
```
  ...
  <Accordion title="Pérdidas de STT de voz con DecryptionFailed(...)">
    ...
    - empieza con `channels.discord.voice.decryptionFailureTolerance=24` (valor predeterminado del upstream) y ajusta solo si es necesario
