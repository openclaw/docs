...
<Steps>
  <Step title="Crear una aplicación y un bot de Discord">
    ...
    Haz clic en **Bot** en la barra lateral. Configura el **Username** como quieras llamar a tu agente de OpenClaw.
  ...
  <Step title="Copiar el token de tu bot">
    ...
    Copia el token y guárdalo en algún sitio. Este es tu **Bot Token** y lo necesitarás en breve.
  ...
  <Step title="Generar una URL de invitación y añadir el bot a tu servidor">
    ...
    **Permisos de texto**
    ...
    Este es el conjunto base para canales de texto normales. Si planeas publicar en hilos de Discord, incluidos flujos de trabajo de canales de foro o medios que crean o continúan un hilo, activa también **Send Messages in Threads**.
  ...
  <Step title="Activar el modo de desarrollador y recopilar tus ID">
    De vuelta en la app de Discord, necesitas activar el modo de desarrollador para poder copiar los ID internos.
  ...
  <Step title="Permitir MD de miembros del servidor">
    ...
    Esto permite que los miembros del servidor (incluidos los bots) te envíen MD. Mantén esto activado si quieres usar MD de Discord con OpenClaw. Si solo planeas usar canales de servidor, puedes desactivar los MD después de la vinculación.
...
```
    ...
    Para instalaciones como servicio administrado, ejecuta `openclaw gateway install` desde una shell donde `DISCORD_BOT_TOKEN` esté presente, o guarda la variable en `~/.openclaw/.env`, para que el servicio pueda resolver el SecretRef de entorno después del reinicio.
...
<Note>
La resolución de tokens reconoce la cuenta. Los valores de token de la configuración tienen prioridad sobre el respaldo de entorno. `DISCORD_BOT_TOKEN` solo se usa para la cuenta predeterminada.
Si dos cuentas de Discord activadas se resuelven al mismo token de bot, OpenClaw inicia solo un monitor de Gateway para ese token. Un token procedente de la configuración tiene prioridad sobre el respaldo de entorno predeterminado; de lo contrario, gana la primera cuenta activada y la cuenta duplicada se informa como desactivada.
Para llamadas salientes avanzadas (acciones de herramienta/canal de mensajes), se usa un `token` explícito por llamada para esa llamada. Esto se aplica a acciones de envío y de tipo lectura/sondeo (por ejemplo read/search/fetch/thread/pins/permissions). La configuración de política/reintentos de la cuenta sigue viniendo de la cuenta seleccionada en la instantánea del runtime activo.
</Note>
...
<Steps>
  <Step title="Añadir tu servidor a la lista de permitidos de servidores">
    ...
    <Tabs>
      <Tab title="Pregúntale a tu agente">
        > "Añade mi Discord Server ID `<server_id>` a la lista de permitidos de servidores"
      </Tab>
      <Tab title="Config">
...
```
  ...
  <Step title="Permitir respuestas sin @mención">
    ...
    <Tabs>
      ...
      <Tab title="Config">
        Configura `requireMention: false` en la configuración de tu servidor:
...
- Los metadatos de servidor/canal de Discord se agregan al prompt del modelo como contexto no confiable, no como un prefijo de respuesta visible para el usuario. Si un modelo copia ese sobre de vuelta, OpenClaw elimina los metadatos copiados de las respuestas salientes y del contexto de reproducción futuro.
...
- La entrega de anuncios de cron/heartbeat solo de texto a Discord usa una vez la respuesta final visible para el asistente. Las cargas útiles de medios y componentes estructurados siguen siendo de varios mensajes cuando el agente emite varias cargas útiles entregables.
...
De forma predeterminada, los componentes son de un solo uso. Configura `components.reusable=true` para permitir que botones, selecciones y formularios se usen varias veces hasta que expiren.

Para restringir quién puede hacer clic en un botón, configura `allowedUsers` en ese botón (IDs de usuario de Discord, etiquetas o `*`). Cuando está configurado, los usuarios que no coinciden reciben una denegación efímera.

Los comandos slash `/model` y `/models` abren un selector interactivo de modelo con desplegables de proveedor, modelo y runtime compatible, además de un paso de envío. `/models add` está obsoleto y ahora devuelve un mensaje de obsolescencia en lugar de registrar modelos desde el chat. La respuesta del selector es efímera y solo el usuario que lo invocó puede usarla.
...
<Tabs>
  <Tab title="Política de DM">
    ...
    Los IDs numéricos sin prefijo normalmente se resuelven como IDs de canal cuando un valor predeterminado de canal está activo, pero los IDs enumerados en el `allowFrom` de DM efectivo de la cuenta se tratan como destinos de DM de usuario por compatibilidad.
...
```
    ...
    Las búsquedas fallan de forma cerrada. Si Discord devuelve `Missing Access`, falla la búsqueda del miembro o el canal pertenece a otro servidor, el remitente del DM se trata como no autorizado.

    Habilita **Server Members Intent** del Discord Developer Portal para el bot al usar grupos de acceso de audiencia de canal. Los DM no incluyen el estado de miembro del servidor, por lo que OpenClaw resuelve el miembro mediante Discord REST en el momento de la autorización.
  ...
  <Tab title="Política de servidor">
    ...
    - listas de permitidos opcionales de remitentes: `users` (se recomiendan IDs estables) y `roles` (solo IDs de rol); si cualquiera está configurada, los remitentes se permiten cuando coinciden con `users` O `roles`
...
```
    ...
    Si solo configuras `DISCORD_BOT_TOKEN` y no creas un bloque `channels.discord`, la alternativa de runtime es `groupPolicy="allowlist"` (con una advertencia en los registros), incluso si `channels.defaults.groupPolicy` es `open`.

  </Tab>

  <Tab title="Menciones y DM grupales">
...
- Los comandos aún pueden estar visibles en la interfaz de Discord para usuarios que no están autorizados; la ejecución sigue aplicando la autenticación de OpenClaw y devuelve "not authorized".
...
```
    ...
    La transmisión de vista previa es solo texto; las respuestas con medios recurren a la entrega normal. Cuando la transmisión `block` está habilitada explícitamente, OpenClaw omite la transmisión de vista previa para evitar una transmisión doble.
  ...
  <Accordion title="Historial, contexto y comportamiento de hilos">
    ...
    - Las sesiones de hilo heredan la selección `/model` a nivel de sesión del canal principal como alternativa solo de modelo; las selecciones `/model` locales del hilo siguen teniendo prioridad y el historial de transcripción del padre no se copia salvo que la herencia de transcripción esté habilitada.
    - `channels.discord.thread.inheritParent` (predeterminado `false`) permite que los nuevos hilos automáticos se inicialicen desde la transcripción del padre. Las sobrescrituras por cuenta están en `channels.discord.accounts.<id>.thread.inheritParent`.
    ...
    Los temas de canal se inyectan como contexto **no confiable**. Las listas de permitidos controlan quién puede activar al agente, no son un límite completo de censura de contexto suplementario.
...
```
  ...
  <Accordion title="Reacciones de confirmación">
    ...
    - Usa `""` para desactivar la reacción de un canal o una cuenta.

  </Accordion>

  <Accordion title="Escrituras de configuración">
    Las escrituras de configuración iniciadas por canales están habilitadas de forma predeterminada.
...
```
  ...
  <Accordion title="Aprobaciones en Discord">
    ...
    Discord también renderiza los botones de aprobación compartidos que usan otros canales de chat. El adaptador nativo de Discord añade principalmente enrutamiento de DM para aprobadores y difusión al canal.
    ...
    la solicitud local determinista `/approve <id> <decision>`. Si el
    runtime está activo pero no se puede entregar una tarjeta nativa a ningún destino,
    OpenClaw envía un aviso de respaldo en el mismo chat con el comando exacto `/approve`
    de la aprobación pendiente.

    La autenticación de Gateway y la resolución de aprobaciones siguen el contrato compartido del cliente de Gateway (`plugin:` IDs se resuelven mediante `plugin.approval.resolve`; otros IDs mediante `exec.approval.resolve`). Las aprobaciones caducan después de 30 minutos de forma predeterminada.
...
- La voz de Discord es opcional para configuraciones solo de texto; establece `channels.discord.voice.enabled=true` (o conserva un bloque `channels.discord.voice` existente) para habilitar comandos `/vc`, el runtime de voz y el intent de gateway `GuildVoiceStates`.
...
- OpenClaw también vigila fallos de descifrado de recepción y se recupera automáticamente saliendo y volviendo a entrar al canal de voz tras fallos repetidos en una ventana breve.
...
- La transcripción se envía por la entrada y el enrutamiento de Discord mientras el LLM de respuesta se ejecuta con una política de salida de voz que oculta la herramienta `tts` del agente y solicita texto devuelto, porque la voz de Discord controla la reproducción TTS final.
...
```
  ...
  <Accordion title="Long-running Discord turns or duplicate replies">
    ...
    Discord no aplica un timeout propiedad del canal a los turnos de agente en cola. Los listeners de mensajes transfieren inmediatamente, y las ejecuciones de Discord en cola conservan el orden por sesión hasta que el ciclo de vida de sesión/herramienta/runtime completa o aborta el trabajo.
...
```
  ...
  <Accordion title="Gateway metadata lookup timeout warnings">
    OpenClaw obtiene los metadatos `/gateway/bot` de Discord antes de conectarse. Los fallos transitorios recurren a la URL de gateway predeterminada de Discord y están limitados por frecuencia en los registros.
  ...
  <Accordion title="Gateway READY timeout restarts">
    OpenClaw espera el evento `READY` del gateway de Discord durante el inicio y después de reconexiones de runtime. Las configuraciones de varias cuentas con escalonamiento de inicio pueden necesitar una ventana READY de inicio más larga que la predeterminada.

    Controles de timeout de READY:

    - startup de cuenta única: `channels.discord.gatewayReadyTimeoutMs`
    - startup multicuenta: `channels.discord.accounts.<accountId>.gatewayReadyTimeoutMs`
    - fallback de env de startup cuando la configuración no está definida: `OPENCLAW_DISCORD_READY_TIMEOUT_MS`
    - valor predeterminado de startup: `15000` (15 segundos), máximo: `120000`
    - runtime de cuenta única: `channels.discord.gatewayRuntimeReadyTimeoutMs`
  ...
  <Accordion title="DM and pairing issues">
    ...
    - MD desactivados: `channels.discord.dm.enabled=false`
...
- startup/autenticación: `enabled`, `token`, `accounts.*`, `allowBots`
