...
OpenClaw se conecta a Discord como bot a través del Gateway oficial de Discord. Se admiten mensajes directos y canales de servidor.
...
Crea una aplicación de Discord con un bot, añade el bot a tu servidor y emparéjalo con OpenClaw. Usa un servidor privado si puedes; [crea uno primero](https://support.discord.com/hc/en-us/articles/204849977-How-do-I-create-a-server) (**Create My Own > For me and my friends**) si es necesario.

<Steps>
  <Step title="Crear una aplicación y un bot de Discord">
    En el [Portal para desarrolladores de Discord](https://discord.com/developers/applications), haz clic en **New Application** y ponle un nombre (por ejemplo, "OpenClaw").

    Abre **Bot** en la barra lateral y establece **Username** como el nombre de tu agente.
  ...
  <Step title="Habilitar intents privilegiados">
    ...
    - **Server Members Intent** (recomendado; obligatorio para listas de permitidos por rol, coincidencia de nombre a ID y grupos de acceso por audiencia de canal)
  ...
  <Step title="Generar una URL de invitación y añadir el bot a tu servidor">
    ...
    En la sección **Bot Permissions** que aparece, habilita al menos:
    ...
    **Text Permissions**
    ...
    Esa es la base para canales de texto normales. Si el bot publicará en hilos, incluidos flujos de trabajo de canales de foro o multimedia que crean o continúan un hilo, habilita también **Send Messages in Threads**.
  ...
  <Step title="Habilitar el modo de desarrollador y recopilar tus ID">
    En la aplicación de Discord, habilita el modo de desarrollador para poder copiar ID:

    1. **User Settings** (icono de engranaje) → **Developer** → activa **Developer Mode**
       *(en móvil: **App Settings** → **Advanced**)*
    2. Haz clic derecho en el **icono de tu servidor** → **Copy Server ID**
    3. Haz clic derecho en tu **propio avatar** → **Copy User ID**

    Conserva el ID de servidor y el ID de usuario junto con tu token de bot; necesitarás los tres a continuación.

  </Step>

  <Step title="Permitir mensajes directos de miembros del servidor">
    Para que el emparejamiento funcione, Discord debe permitir que el bot te envíe mensajes directos. Haz clic derecho en el **icono de tu servidor** → **Privacy Settings** → activa **Direct Messages**.

    Mantén esto activado si usas mensajes directos de Discord con OpenClaw. Si solo usas canales de servidor, puedes desactivarlo después del emparejamiento.
...
```
    ...
    Si OpenClaw ya se ejecuta como servicio en segundo plano, reinícialo mediante la app de OpenClaw para Mac o deteniendo y reiniciando el proceso `openclaw gateway run`.
    Para instalaciones de servicio administradas, ejecuta `openclaw gateway install` desde un shell donde `DISCORD_BOT_TOKEN` esté configurado, o almacena la variable en `~/.openclaw/.env` para que el servicio pueda resolver el SecretRef de env después del reinicio.
    Si Discord bloquea o limita por frecuencia tu host durante la búsqueda de la aplicación al inicio, configura el ID de aplicación/cliente desde el Portal para desarrolladores para que el inicio pueda omitir esa llamada REST: `channels.discord.applicationId` para la cuenta predeterminada, o `channels.discord.accounts.<accountId>.applicationId` por bot.
  ...
  <Step title="Configurar OpenClaw y emparejar">
    ...
    <Tabs>
      <Tab title="Preguntar a tu agente">
        Chatea con tu agente de OpenClaw en un canal existente (por ejemplo, Telegram) y díselo. Si Discord es tu primer canal, usa en su lugar la pestaña CLI / configuración.
...
```
        ...
        Para configuración con scripts o remota, escribe el mismo bloque JSON5 con `openclaw config patch --file ./discord.patch.json5 --dry-run` y luego vuelve a ejecutarlo sin `--dry-run`. Las cadenas `token` en texto plano también funcionan, y se admiten valores SecretRef para `channels.discord.token` en proveedores env/file/exec. Consulta [Gestión de secretos](/es/gateway/secrets).

        Para varios bots de Discord, mantén cada token de bot e ID de aplicación bajo su cuenta. Un `channels.discord.applicationId` de nivel superior se hereda por las cuentas, así que configúralo allí solo cuando cada cuenta use el mismo ID de aplicación.
...
```
  ...
  <Step title="Aprobar el primer emparejamiento por mensaje directo">
    Una vez que el Gateway esté en ejecución, envía un mensaje directo a tu bot en Discord. Responderá con un código de emparejamiento.
...
```
    ...
    Los códigos de emparejamiento caducan después de 1 hora. Tras la aprobación, chatea con tu agente en un mensaje directo de Discord.

  </Step>
</Steps>

<Note>
La resolución de tokens tiene en cuenta la cuenta. Los valores de token de configuración tienen prioridad sobre el fallback de env, y `DISCORD_BOT_TOKEN` solo se usa para la cuenta predeterminada.
Si dos cuentas de Discord habilitadas se resuelven al mismo token de bot, OpenClaw inicia solo un monitor de Gateway para ese token: un token proveniente de la configuración tiene prioridad sobre el fallback de env; de lo contrario, la primera cuenta habilitada gana y la cuenta duplicada se informa como deshabilitada con el motivo `duplicate bot token`.
Para llamadas salientes avanzadas (herramienta de mensajes/acciones de canal), se usa un `token` explícito por llamada para esa llamada. Esto se aplica a acciones de envío y de estilo lectura/sondeo (read/search/fetch/thread/pins/permissions). La política de cuenta y la configuración de reintentos siguen proviniendo de la cuenta seleccionada en la instantánea activa del runtime.
</Note>

## Recomendado: Configurar un espacio de trabajo de servidor

Una vez que los mensajes directos funcionen, puedes convertir tu servidor en un espacio de trabajo completo donde cada canal tenga su propia sesión de agente con su propio contexto. Recomendado para servidores privados donde solo estáis tú y tu bot.
...
```
  ...
  <Step title="Permitir respuestas sin @mención">
    De forma predeterminada, el agente solo responde en canales de servidor cuando se lo @menciona. En un servidor privado, probablemente quieras que responda a todos los mensajes.

    En canales de servidor, las respuestas normales se publican automáticamente de forma predeterminada. Para salas compartidas siempre activas, opta por `messages.groupChat.visibleReplies: "message_tool"` para que el agente pueda observar de forma discreta y solo publicar cuando decida que una respuesta en el canal es útil. Esto funciona mejor con modelos de última generación y fiables con herramientas, como GPT 5.5. Los eventos ambientales de sala permanecen en silencio salvo que la herramienta envíe. Consulta [Eventos ambientales de sala](/es/channels/ambient-room-events) para ver la configuración completa del modo de observación discreta.

    Si Discord muestra que se está escribiendo y los registros muestran uso de tokens pero no hay mensaje publicado, comprueba si el turno se configuró como evento ambiental de sala o si se optó por respuestas visibles mediante herramienta de mensajes.
...
```
        ...
        Para exigir envíos mediante herramienta de mensajes para respuestas visibles de grupo/canal, configura `messages.groupChat.visibleReplies: "message_tool"`.
  ...
  <Step title="Planificar la memoria en canales de servidor">
    La memoria a largo plazo (MEMORY.md) solo se carga automáticamente en sesiones de mensajes directos; los canales de servidor no la cargan.
    ...
    <Tabs>
      ...
      <Tab title="Manual">
        Para contexto compartido en todos los canales, coloca instrucciones estables en `AGENTS.md` o `USER.md` (inyectadas en cada sesión). Mantén las notas a largo plazo en `MEMORY.md` y accede a ellas bajo demanda con herramientas de memoria.
      </Tab>
    </Tabs>
...
Ahora crea canales y empieza a chatear. El agente ve el nombre del canal, y cada canal es una sesión aislada: configura `#coding`, `#home`, `#research` o lo que encaje con tu flujo de trabajo.

## Modelo de runtime

- Gateway es propietario de la conexión de Discord.
- El enrutamiento de respuestas es determinista: las entradas de Discord responden a Discord.
...
- La entrega de anuncios de Cron/Heartbeat solo de texto a Discord se reduce a la respuesta final visible para el asistente, enviada una vez. Los payloads multimedia y de componentes estructurados siguen siendo de varios mensajes cuando el agente emite varios payloads entregables.

## Canales de foro

Los canales de foro y multimedia de Discord solo aceptan publicaciones de hilo. OpenClaw admite dos formas de crearlas:

- Envía un mensaje al padre del foro (`channel:<forumId>`) para crear automáticamente un hilo. El título del hilo es la primera línea no vacía del mensaje (truncada al límite de Discord de 100 caracteres para nombres de hilo).
...
Para restringir quién puede hacer clic en un botón, establece `allowedUsers` en ese botón (IDs de usuario de Discord, etiquetas o `*`). Los usuarios no coincidentes reciben una denegación efímera.

Las devoluciones de llamada de componentes caducan después de 30 minutos de forma predeterminada. Establece `channels.discord.agentComponents.ttlMs` para cambiar la vida útil del registro de devoluciones de llamada de la cuenta predeterminada, o `channels.discord.accounts.<accountId>.agentComponents.ttlMs` por cuenta. El valor está en milisegundos, debe ser un entero positivo y tiene un límite de `86400000` (24 horas). TTL más largos son adecuados para flujos de revisión/aprobación que necesitan que los botones sigan siendo utilizables, pero amplían la ventana en la que un mensaje antiguo de Discord todavía puede activar una acción. Prefiere el TTL más corto que encaje y conserva el valor predeterminado cuando las devoluciones de llamada obsoletas serían sorprendentes.

Los comandos slash `/model` y `/models` abren un selector interactivo de modelos con menús desplegables de proveedor, modelo y runtime compatible, además de un paso de Enviar. `/models add` está obsoleto y devuelve un mensaje de obsolescencia en lugar de registrar modelos desde el chat. La respuesta del selector es efímera y solo puede usarla el usuario que la invocó. Los menús de selección de Discord están limitados a 25 opciones, así que agrega entradas `provider/*` a `agents.defaults.models` cuando quieras que el selector muestre modelos descubiertos dinámicamente solo para proveedores seleccionados como `openai` o `vllm`.
...
<Tabs>
  <Tab title="DM policy">
    ...
    `channels.discord.dm.policy` y `channels.discord.dm.allowFrom` heredados todavía se leen por compatibilidad. `openclaw doctor --fix` los migra a `dmPolicy` y `allowFrom` cuando puede hacerlo sin cambiar el acceso.
    ...
    Los IDs numéricos sin prefijo normalmente se resuelven como IDs de canal cuando hay un canal predeterminado activo, pero los IDs enumerados en el `allowFrom` efectivo de DM de la cuenta se tratan como destinos de DM de usuario por compatibilidad.
...
```
    ...
    Un canal de texto de Discord no tiene una lista de miembros separada. `type: "discord.channelAudience"` modela la pertenencia así: el remitente de DM es miembro del servidor configurado y actualmente tiene permiso efectivo `ViewChannel` sobre el canal configurado después de aplicar las sobrescrituras de rol y canal.

    Ejemplo: permite que cualquiera que pueda ver `#maintainers` envíe DM al bot, manteniendo los DM cerrados para todos los demás.
...
```
    ...
    Las búsquedas fallan cerradas. Si Discord devuelve `Missing Access`, falla la búsqueda de miembros o el canal pertenece a otro servidor, el remitente de DM se trata como no autorizado.

    Habilita **Server Members Intent** en el Portal de Desarrolladores de Discord al usar grupos de acceso de audiencia de canal. Los DM no incluyen estado de miembro del servidor, así que OpenClaw resuelve el miembro mediante Discord REST en el momento de la autorización.
  ...
  <Tab title="Guild policy">
    ...
    - listas de remitentes permitidos opcionales: `users` (se recomiendan IDs estables) y `roles` (solo IDs de rol); si cualquiera está configurada, los remitentes se permiten cuando coinciden con `users` O `roles`
...
- `commands.native` usa de forma predeterminada `"auto"` y está habilitado para Discord.
- Anulación por canal: `channels.discord.commands.native`.
- `commands.native=false` omite el registro y la limpieza de comandos slash de Discord durante el inicio. Los comandos registrados anteriormente pueden seguir visibles en Discord hasta que los elimines de la app de Discord.
...
- Los comandos aún pueden ser visibles en la interfaz de Discord para usuarios no autorizados; la ejecución aplica la autenticación de OpenClaw y responde "no autorizado".
- Configuración predeterminada de comandos slash: `ephemeral: true` (`channels.discord.slashCommand.ephemeral`).
...
<AccordionGroup>
  <Accordion title="Etiquetas de respuesta y respuestas nativas">
    ...
    - `off` (predeterminado): sin hilado de respuestas implícito; las etiquetas explícitas `[[reply_to_*]]` se siguen respetando
    ...
    Los ID de mensajes se exponen en el contexto/historial para que los agentes puedan apuntar a mensajes específicos.

  </Accordion>

  <Accordion title="Vistas previas de enlaces">
    Discord genera embeds enriquecidos de enlaces para las URL de forma predeterminada. OpenClaw suprime esos embeds generados en los mensajes salientes de Discord de forma predeterminada, por lo que las URL enviadas por el agente permanecen como enlaces simples salvo que lo habilites:
...
```
    ...
    Configura `channels.discord.accounts.<id>.suppressEmbeds` para anular una cuenta. Los envíos con la herramienta de mensajes del agente también pueden pasar `suppressEmbeds: false` para un solo mensaje. Las cargas útiles explícitas `embeds` de Discord no se suprimen por la configuración predeterminada de vista previa de enlaces.

  </Accordion>

  <Accordion title="Vista previa de transmisión en vivo">
    OpenClaw puede transmitir borradores de respuestas enviando un mensaje temporal y editándolo a medida que llega el texto. `channels.discord.streaming.mode` acepta `off` | `partial` | `block` | `progress` (predeterminado cuando no se establece la clave `streaming` ni la clave heredada `streamMode`). `streamMode` es un alias heredado; ejecuta `openclaw doctor --fix` para reescribir la configuración persistida a la forma anidada canónica `streaming`.
...
```
    ...
    - `block` emite fragmentos del tamaño de un borrador; ajusta el tamaño y los puntos de corte con `streaming.preview.chunk` (`minChars`, `maxChars`, `breakPreference`), limitado a `textChunkLimit`. Cuando la transmisión por bloques está habilitada explícitamente, OpenClaw omite la transmisión de vista previa para evitar la doble transmisión.
    - `progress` mantiene un borrador de estado editable y lo actualiza con el progreso de las herramientas hasta la entrega final; la etiqueta inicial compartida es una línea móvil, por lo que se desplaza fuera de la vista como el resto cuando aparece suficiente trabajo.
    ...
    - `streaming.progress.commentary` (predeterminado `false`) habilita el texto de comentario/preámbulo del asistente en el borrador temporal de progreso. El comentario se limpia antes de mostrarse, permanece transitorio y no cambia la entrega de la respuesta final.
  ...
  <Accordion title="Historial, contexto y comportamiento de hilos">
    ...
    - Las sesiones de hilo heredan la selección `/model` de nivel de sesión del canal principal como alternativa solo de modelo; las selecciones `/model` locales del hilo tienen prioridad, y el historial de transcripción principal no se copia salvo que la herencia de transcripción esté habilitada.
    - `channels.discord.thread.inheritParent` (predeterminado `false`) hace que los nuevos hilos automáticos se inicialicen desde la transcripción principal. Anulación por cuenta: `channels.discord.accounts.<id>.thread.inheritParent`.
    ...
    Los temas de canal se inyectan como contexto **no confiable**. Las listas de permitidos restringen quién puede activar el agente, no son un límite completo de censura de contexto suplementario.
...
```
    ...
    - `session.threadBindings.*` establece valores predeterminados globales; `channels.discord.threadBindings.*` anula el comportamiento de Discord.
...
```
  ...
  <Accordion title="Escrituras de configuración">
    Las escrituras de configuración iniciadas por canal están habilitadas de forma predeterminada. Esto afecta los flujos `/config set|unset` (cuando las características de comandos están habilitadas).
...
```
  ...
  <Accordion title="Proxy de Gateway">
    Enruta el tráfico WebSocket del Gateway de Discord y las búsquedas REST de inicio (ID de aplicación + resolución de lista de permitidos) mediante un proxy HTTP(S) con `channels.discord.proxy`.
...
```
  ...
  <Accordion title="Compatibilidad con PluralKit">
    Habilita la resolución de PluralKit para mapear mensajes enviados por proxy a la identidad de miembro del sistema:
...
```
    ...
    - si la búsqueda falla, los mensajes enviados por proxy se tratan como mensajes de bot y se descartan salvo que `allowBots` los deje pasar
...
```
    ...
    Presencia automática (señal de estado del runtime):
...
```
    ...
    La presencia automática asigna la disponibilidad del runtime al estado de Discord: correcto => online, degradado o desconocido => idle, agotado o no disponible => dnd. Valores predeterminados: `intervalMs` 30000, `minUpdateIntervalMs` 15000 (debe ser menor o igual que `intervalMs`). Sustituciones de texto opcionales:
  ...
  <Accordion title="Aprobaciones en Discord">
    Discord admite la gestión de aprobaciones basada en botones en DMs y, opcionalmente, puede publicar solicitudes de aprobación en el canal de origen.
    ...
    Discord habilita automáticamente las aprobaciones nativas de exec cuando `enabled` no está definido o es `"auto"` y se puede resolver al menos un aprobador, ya sea desde `execApprovals.approvers` o desde `commands.ownerAllowFrom`. Discord no infiere aprobadores de exec desde el canal `allowFrom`, el `dm.allowFrom` heredado ni el `defaultTo` de mensajes directos. Establece `enabled: false` para deshabilitar explícitamente Discord como cliente de aprobación nativo.

    Para comandos de grupo sensibles solo para propietarios, como `/diagnostics` y `/export-trajectory`, OpenClaw envía las solicitudes de aprobación y los resultados finales de forma privada. Primero intenta DM de Discord cuando el propietario que invoca tiene una ruta de propietario de Discord; de lo contrario, recurre a la primera ruta de propietario disponible desde `commands.ownerAllowFrom`, como Telegram.

    Cuando `target` es `channel` o `both`, la solicitud de aprobación es visible en el canal. Solo los aprobadores resueltos pueden usar los botones; otros usuarios reciben una denegación efímera. Las solicitudes de aprobación incluyen el texto del comando, por lo que solo debes habilitar la entrega en canales de confianza. Si el ID del canal no se puede derivar de la clave de sesión, OpenClaw recurre a la entrega por DM.

    Discord renderiza los botones de aprobación compartidos que usan otros canales de chat; el adaptador nativo de Discord principalmente agrega enrutamiento de DMs de aprobadores y distribución al canal. Cuando esos botones están presentes, son la UX de aprobación principal; OpenClaw solo debe incluir un comando manual `/approve` cuando el resultado de la herramienta indique que las aprobaciones por chat no están disponibles o que la aprobación manual es la única ruta. Si el runtime de aprobación nativo de Discord no está activo, OpenClaw mantiene visible la solicitud local determinista `/approve <id> <decision>`. Si el runtime está activo pero no se puede entregar una tarjeta nativa a ningún destino, OpenClaw envía un aviso de respaldo en el mismo chat con el comando `/approve` exacto de la aprobación pendiente.

    La autenticación de Gateway y la resolución de aprobaciones siguen el contrato compartido del cliente de Gateway (los IDs `plugin:` se resuelven mediante `plugin.approval.resolve`; otros IDs mediante `exec.approval.resolve`). Las aprobaciones caducan después de 30 minutos de forma predeterminada.
...
Las acciones de mensajes de Discord cubren mensajería, administración de canales, moderación, presencia y metadatos.
...
- `channels.discord.ui.components.accentColor` establece el color de énfasis usado por los contenedores de componentes de Discord (hex). Por cuenta: `channels.discord.accounts.<id>.ui.components.accentColor`.
- `channels.discord.agentComponents.ttlMs` controla cuánto tiempo permanecen registrados los callbacks de componentes de Discord enviados (valor predeterminado `1800000`, máximo `86400000`). Por cuenta: `channels.discord.accounts.<id>.agentComponents.ttlMs`.
- `embeds` se ignoran cuando hay componentes v2 presentes.
- Las vistas previas de URL simples se suprimen de forma predeterminada. Establece `suppressEmbeds: false` en una acción de mensaje cuando un único enlace saliente debe expandirse.
...
2. Habilita Server Members Intent cuando se usen listas de permisos de roles/usuarios.
...
Usa `/vc join|leave|status` para controlar sesiones. El comando usa el agente predeterminado de la cuenta y sigue las mismas reglas de lista de permisos y política de grupo que otros comandos de Discord.
...
- `voice.agentSession` controla qué conversación de OpenClaw recibe los turnos de voz. Déjalo sin establecer para la sesión propia del canal de voz, o establece `{ mode: "target", target: "channel:<text-channel-id>" }` para hacer que el canal de voz actúe como la extensión de micrófono/altavoz de una sesión existente de canal de texto de Discord, como `#maintainers`.
...
- `agent-proxy` enruta el habla mediante `discord-voice`, que conserva la autorización normal de propietario/herramienta para el hablante y la sesión de destino, pero oculta la herramienta `tts` del agente porque la voz de Discord es dueña de la reproducción. De forma predeterminada, `agent-proxy` da a la consulta acceso completo a herramientas equivalente al propietario para hablantes propietarios (`voice.realtime.toolPolicy: "owner"`) y prefiere firmemente consultar al agente OpenClaw antes de respuestas sustantivas (`voice.realtime.consultPolicy: "always"`). En ese modo predeterminado `always`, la capa en tiempo real no pronuncia automáticamente relleno antes de la respuesta de la consulta; captura y transcribe el habla, y luego pronuncia la respuesta de OpenClaw enrutada. Si varias respuestas de consulta forzada terminan mientras Discord todavía reproduce la primera respuesta, las respuestas posteriores de habla exacta se ponen en cola hasta que la reproducción quede inactiva, en lugar de reemplazar el habla a mitad de frase.
...
- Los modos de voz en tiempo real incluyen de forma predeterminada pequeños archivos de perfil `IDENTITY.md`, `USER.md` y `SOUL.md` en las instrucciones del proveedor en tiempo real para que los turnos directos rápidos conserven la misma identidad, fundamentación de usuario y persona que el agente OpenClaw enrutado. Establece `voice.realtime.bootstrapContextFiles` en un subconjunto para personalizar esto, o `[]` para deshabilitarlo. Solo se admiten esos archivos de perfil; `AGENTS.md` permanece en el contexto normal del agente. El contexto de perfil inyectado no reemplaza a `openclaw_agent_consult` para trabajo en el workspace, datos actuales, búsqueda en memoria o acciones respaldadas por herramientas.
- En el modo en tiempo real `agent-proxy` de OpenAI, establece `voice.realtime.requireWakeName: true` para mantener la voz en tiempo real de Discord en silencio hasta que una transcripción empiece o termine con un nombre de activación. Los nombres de activación configurados deben tener una o dos palabras. Si `voice.realtime.wakeNames` no está establecido, OpenClaw usa el `name` del agente enrutado más `OpenClaw`, con fallback al id del agente más `OpenClaw`. La compuerta por nombre de activación deshabilita la respuesta automática del proveedor en tiempo real, enruta los turnos aceptados por la ruta de consulta del agente OpenClaw y da un breve acuse hablado cuando se reconoce un nombre de activación inicial a partir de la transcripción parcial antes de que llegue la transcripción final.
- El proveedor en tiempo real de OpenAI acepta los nombres de eventos actuales de Realtime 2 y alias heredados compatibles con Codex para eventos de audio de salida y transcripción, por lo que las instantáneas de proveedores compatibles pueden desviarse sin perder el audio del asistente.
- `voice.realtime.bargeIn` controla si los eventos de inicio de habla de Discord interrumpen la reproducción en tiempo real activa. Si no se establece, sigue la configuración de interrupción de audio de entrada del proveedor en tiempo real.
...
- `voice.allowedChannels` es una allowlist opcional de residencia. Déjalo sin establecer para permitir `/vc join` en cualquier canal de voz autorizado de Discord. Cuando se establece, `/vc join`, la unión automática al iniciar y los movimientos de estado de voz del bot se restringen a las entradas `{ guildId, channelId }` listadas. Establécelo en un arreglo vacío para denegar todas las uniones a voz de Discord. Si Discord mueve el bot fuera de la allowlist, OpenClaw sale de ese canal y vuelve a unirse al destino de unión automática configurado cuando hay uno disponible.
- `voice.daveEncryption` y `voice.decryptionFailureTolerance` se pasan a las opciones de unión de `@discordjs/voice`; los valores predeterminados del upstream son `daveEncryption=true` y `decryptionFailureTolerance=24`.
- OpenClaw usa el códec incluido `libopus-wasm` para la recepción de voz de Discord y la reproducción PCM sin procesar en tiempo real. Incluye una compilación WebAssembly de libopus fijada y no requiere complementos opus nativos.
...
- En modo `stt-tts`, la reproducción de voz no se detiene simplemente porque otro usuario empiece a hablar. Para evitar bucles de retroalimentación, OpenClaw ignora nuevas capturas de voz mientras TTS se reproduce; habla después de que termine la reproducción para el siguiente turno. Los modos en tiempo real reenvían los inicios de hablante como señales de interrupción al proveedor en tiempo real.
...
- Cuando ElevenLabs es el proveedor TTS seleccionado, la reproducción de voz de Discord usa TTS en streaming y comienza desde el flujo de respuesta del proveedor. Los proveedores sin compatibilidad de streaming vuelven a la ruta de archivo temporal sintetizado.
- OpenClaw vigila los fallos de descifrado de recepción y se recupera automáticamente saliendo y volviendo a unirse al canal de voz tras fallos repetidos en una ventana corta.
- Si los registros de recepción muestran repetidamente `DecryptionFailed(UnencryptedWhenPassthroughDisabled)` después de actualizar, recopila un informe de dependencias y registros. La línea incluida de `@discordjs/voice` incluye la corrección de relleno upstream del PR #11449 de discord.js, que cerró el issue #11419 de discord.js.
...
- Los registros detallados de voz de Discord incluyen una vista previa acotada de una línea de la transcripción STT para cada segmento de hablante aceptado, de modo que la depuración muestre tanto el lado del usuario como el lado de la respuesta del agente sin volcar texto de transcripción ilimitado.
- En modo `agent-proxy`, el fallback de consulta forzada omite fragmentos de transcripción probablemente incompletos, como texto que termina en `...` o un conector final como "and", además de cierres obviamente no accionables como "be right back" o "bye". Los registros muestran `forced agent consult skipped reason=...` cuando esto evita una respuesta en cola obsoleta.

### Seguir usuarios en voz

Usa `voice.followUsers` cuando quieras que el bot de voz de Discord permanezca con uno o más usuarios conocidos de Discord en lugar de unirse a un canal fijo al iniciar o esperar `/vc join`.
...
- `followUsersEnabled` tiene como valor predeterminado `true` cuando `followUsers` está configurado. Establécelo en `false` para conservar la lista guardada pero detener el seguimiento automático de voz.
...
- Si varios usuarios seguidos están en el mismo servidor y el usuario seguido activo se va, OpenClaw se mueve al canal de otro usuario seguido rastreado antes de salir del servidor. Si varios usuarios seguidos se mueven a la vez, gana el último evento de estado de voz observado.
...
- OpenClaw reconcilia eventos de estado de voz omitidos al iniciar y en un intervalo acotado. La reconciliación muestrea servidores configurados y limita las consultas REST por ejecución, por lo que listas `followUsers` muy grandes pueden tardar más de un intervalo en converger.
...
- La recuperación de recepción DAVE puede salir y volver a unirse al mismo canal tras fallos de descifrado repetidos. Las sesiones propiedad de seguimiento conservan su propiedad de seguimiento durante esa ruta de recuperación, por lo que una desconexión posterior del usuario seguido aún hace que se salga del canal.

Elige entre los modos de unión:

- Usa `followUsers` para configuraciones personales o de operador en las que el bot debe estar automáticamente en voz cuando tú lo estás.
- Usa `autoJoin` para bots de sala fija que deben estar presentes incluso cuando ningún usuario rastreado está en voz.
- Usa `/vc join` para uniones puntuales o salas donde la presencia automática en voz sería inesperada.

Códec de voz de Discord:

- Los registros de recepción de voz muestran `discord voice: opus decoder: libopus-wasm`.
- La reproducción en tiempo real codifica PCM estéreo sin procesar a 48 kHz en Opus con el mismo paquete incluido `libopus-wasm` antes de entregar paquetes a `@discordjs/voice`.
- La reproducción de archivos y flujos de proveedor transcodifica a PCM estéreo sin procesar a 48 kHz con ffmpeg, y luego usa `libopus-wasm` para el flujo de paquetes Opus enviado a Discord.
...
- La transcripción se envía mediante la entrada y el enrutamiento de Discord mientras el LLM de respuesta se ejecuta con una política de salida de voz que oculta la herramienta `tts` del agente y solicita texto devuelto, porque la voz de Discord posee la reproducción TTS final.
...
Sin un bloque `voice.agentSession`, cada canal de voz obtiene su propia sesión enrutada de OpenClaw. Por ejemplo, `/vc join channel:234567890123456789` habla con la sesión de ese canal de voz de Discord. El modelo realtime es solo la interfaz de voz; las solicitudes sustantivas se entregan al agente de OpenClaw configurado. Si el modelo realtime produce una transcripción final sin llamar a la herramienta de consulta, OpenClaw fuerza la consulta como fallback para que el comportamiento predeterminado siga siendo como hablar con el agente.
...
En modo `agent-proxy`, el bot se une al canal de voz configurado, pero los turnos del agente de OpenClaw usan la sesión enrutada normal y el agente del canal objetivo. La sesión de voz realtime dice el resultado devuelto de vuelta en el canal de voz. El agente supervisor todavía puede usar herramientas de mensaje normales según su política de herramientas, incluido enviar un mensaje separado de Discord si esa es la acción correcta.

Mientras una ejecución delegada de OpenClaw está activa, las nuevas transcripciones de voz de Discord se tratan como control de ejecución en vivo antes de iniciar otro turno de agente. Frases como "status", "cancel that", "use the smaller fix" o "when you're done also check tests" se clasifican como estado, cancelación, orientación o entrada de seguimiento para la sesión activa. Los resultados de estado, cancelación, orientación aceptada y seguimiento se dicen de vuelta en el canal de voz para que quien llama sepa si OpenClaw gestionó la solicitud.
...
Use esto cuando el modelo oiga su propia reproducción de Discord a través de un micrófono abierto, pero usted todavía quiera interrumpirlo hablando. OpenClaw evita que OpenAI interrumpa automáticamente con audio de entrada sin procesar, mientras que `bargeIn: true` permite que los eventos de inicio de hablante de Discord y el audio de hablante ya activo cancelen respuestas realtime activas antes de que el siguiente turno capturado llegue a OpenAI. Las señales de barge-in muy tempranas con `audioEndMs` por debajo de `minBargeInAudioEndMs` se tratan como probable eco/ruido y se ignoran para que el modelo no se corte en el primer fotograma de reproducción.
...
- Al iniciar realtime: `discord voice: realtime bridge starting ... autoRespond=false interruptResponse=false bargeIn=false minBargeInAudioEndMs=...`
...
- En voz obsoleta omitida: `discord voice: realtime forced agent consult skipped reason=incomplete-transcript ...` o `reason=non-actionable-closing ...`
- Al completarse la respuesta realtime: `discord voice: realtime audio playback finishing reason=response.done ... audioMs=... chunks=...`
- Al detener/restablecer la reproducción: `discord voice: realtime audio playback stopped reason=... audioMs=... elapsedMs=... chunks=...`
- En consulta realtime: `discord voice: realtime consult requested ... voiceSession=... supervisorSession=... question=...`
...
- En detección de barge-in: `discord voice: realtime barge-in detected source=speaker-start ...` o `discord voice: realtime barge-in detected source=active-speaker-audio ...`, seguido de `discord voice: realtime barge-in requested reason=... outputAudioMs=... outputActive=...`
- En interrupción realtime: `discord voice: realtime model interrupt requested client:response.cancel reason=barge-in`, seguido de `discord voice: realtime model audio truncated client:conversation.item.truncate reason=barge-in audioEndMs=...` o `discord voice: realtime model interrupt confirmed server:response.done status=cancelled ...`
...
- Con barge-in deshabilitado: `discord voice: realtime capture ignored during playback (barge-in disabled) ...`
- En reproducción inactiva: `discord voice: realtime barge-in ignored reason=... outputActive=false ... playbackChunks=0`

Para depurar audio cortado, lea los registros de voz realtime como una línea de tiempo:

1. `realtime audio playback started` significa que Discord ha empezado a reproducir audio del asistente. El puente empieza a contar fragmentos de salida del asistente, bytes PCM de Discord, bytes realtime del proveedor y duración de audio sintetizado desde este punto.
...
5. `barge-in requested reason=...` significa que OpenClaw pidió al proveedor realtime cancelar o truncar la respuesta activa. Incluye `outputAudioMs`, `outputActive` y `playbackChunks` para que pueda ver cuánto audio del asistente se había reproducido realmente antes de la interrupción.
6. `realtime audio playback stopped reason=...` es el punto local de restablecimiento de reproducción de Discord. La razón indica quién detuvo la reproducción: `barge-in`, `player-idle`, `provider-clear-audio`, `forced-agent-consult`, `stream-close` o `session-close`.
7. `realtime speaker turn closed` resume el turno de entrada capturado. `chunks=0` o `hasAudio=false` significa que el turno de hablante se abrió, pero ningún audio utilizable llegó al puente realtime. `interruptedPlayback=true` significa que ese turno de entrada se solapó con la salida del asistente y activó la lógica de barge-in.

Campos útiles:

- `outputAudioMs`: duración del audio del asistente generada por el proveedor realtime antes de la línea de registro.
- `audioMs`: duración del audio del asistente que OpenClaw contó antes de que se detuviera la reproducción.
- `elapsedMs`: tiempo de reloj entre abrir y cerrar el stream de reproducción o el turno de hablante.
- `discordBytes`: bytes PCM estéreo de 48 kHz enviados a o recibidos desde la voz de Discord.
- `realtimeBytes`: bytes PCM en formato del proveedor enviados a o recibidos desde el proveedor realtime.
...
- Un corte inmediato con `source=active-speaker-audio`, `outputAudioMs` pequeño y el mismo usuario cerca suele apuntar a eco del altavoz entrando en el micrófono. Aumente `voice.realtime.minBargeInAudioEndMs`, baje el volumen del altavoz, use auriculares o establezca `voice.realtime.providers.openai.interruptResponseOnInputAudio: false`.
- `source=speaker-start` seguido de `speaker turn closed ... hasAudio=false` significa que Discord informó de un inicio de hablante, pero ningún audio llegó a OpenClaw. Eso puede ser un evento transitorio de voz de Discord, comportamiento de puerta de ruido o que un cliente active brevemente el micrófono.
- `audio playback stopped reason=stream-close` sin un barge-in cercano ni `provider-clear-audio` significa que el stream local de reproducción de Discord terminó inesperadamente. Revise los registros anteriores del proveedor y del reproductor de Discord.
- `capture ignored during playback (barge-in disabled)` significa que OpenClaw descartó intencionalmente la entrada mientras el audio del asistente estaba activo. Habilite `voice.realtime.bargeIn` si quiere que la voz interrumpa la reproducción.
- `barge-in ignored ... outputActive=false` significa que Discord o el VAD del proveedor informó voz, pero OpenClaw no tenía reproducción activa que interrumpir. Esto no debería cortar el audio.

Las credenciales se resuelven por componente: autenticación de ruta LLM para `voice.model`, autenticación STT para `tools.media.audio`, autenticación TTS para `messages.tts`/`voice.tts` y autenticación del proveedor realtime para `voice.realtime.providers` o la configuración de autenticación normal del proveedor.
...
```
  ...
  <Accordion title="Require mention false pero sigue bloqueado">
    ...
    - `requireMention` configurado en el lugar incorrecto (debe estar bajo `channels.discord.guilds` o una entrada de canal)
  ...
  <Accordion title="Turnos largos de Discord o respuestas duplicadas">
    ...
    Discord no aplica un timeout propiedad del canal a los turnos de agente en cola. Los listeners de mensajes transfieren el trabajo de inmediato, y las ejecuciones de Discord en cola preservan el orden por sesión hasta que el ciclo de vida de la sesión/herramienta/runtime completa o aborta el trabajo.
...
```
  ...
  <Accordion title="Advertencias de timeout en la búsqueda de metadatos del Gateway">
    ...
    Opciones de timeout de metadatos:
  ...
  <Accordion title="Reinicios por timeout de READY del Gateway">
    OpenClaw espera el evento `READY` del gateway de Discord durante el arranque y después de reconexiones del runtime. Las configuraciones con varias cuentas y arranque escalonado pueden necesitar una ventana READY de arranque más larga que la predeterminada.

    Opciones de timeout de READY:

    - arranque con una sola cuenta: `channels.discord.gatewayReadyTimeoutMs`
    - arranque con varias cuentas: `channels.discord.accounts.<accountId>.gatewayReadyTimeoutMs`
    - fallback de env de arranque cuando la configuración no está definida: `OPENCLAW_DISCORD_READY_TIMEOUT_MS`
    - valor predeterminado de arranque: `15000` (15 segundos), máximo: `120000`
    - runtime con una sola cuenta: `channels.discord.gatewayRuntimeReadyTimeoutMs`
  ...
  <Accordion title="Discrepancias en la auditoría de permisos">
    ...
    Si usas claves slug, la coincidencia en runtime aún puede funcionar, pero la sonda no puede verificar completamente los permisos.
  ...
  <Accordion title="Bucles de bot a bot">
    ...
    Si estableces `channels.discord.allowBots=true`, usa reglas estrictas de mención y listas de permitidos para evitar comportamientos de bucle.
    Prefiere `channels.discord.allowBots="mentions"` para aceptar solo mensajes de bots que mencionen al bot.

    OpenClaw también incluye [protección contra bucles de bots](/es/channels/bot-loop-protection) compartida. Siempre que `allowBots` permite que mensajes escritos por bots lleguen al despacho, Discord asigna el evento entrante a hechos de `(cuenta, canal, par de bots)` y la guarda genérica de pares suprime el par después de que cruza el presupuesto de eventos configurado. La guarda evita bucles descontrolados entre dos bots que antes tenían que ser detenidos por los límites de frecuencia de Discord; no afecta a despliegues de un solo bot ni a respuestas puntuales de bots que permanecen por debajo del presupuesto.
    ...
    - `cooldownSeconds: 60` -- una vez que se supera el presupuesto, cada mensaje adicional de bot a bot en cualquier dirección se descarta durante un minuto

    Configura el valor predeterminado compartido una vez bajo `channels.defaults.botLoopProtection`, luego sobrescribe Discord cuando un flujo de trabajo legítimo necesite más margen. La precedencia es:
...
```
  ...
  <Accordion title="Caídas de STT de voz con DecryptionFailed(...)">
    ...
    - observa los registros para detectar:
...
- medios/reintento: `mediaMaxMb` (limita las cargas salientes de Discord, predeterminado `100`), `retry`
- acciones: `actions.*`
...
- Concede los permisos mínimos necesarios de Discord.
...
<CardGroup cols={2}>
  ...
  <Card title="Seguridad" icon="shield" href="/es/gateway/security">
    Modelo de amenazas y hardening.
  </Card>
