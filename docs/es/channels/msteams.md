...
Estado: se admiten texto + archivos adjuntos de DM; el envío de archivos en canales/grupos requiere `sharePointSiteId` + permisos de Graph (consulta [Enviar archivos en chats de grupo](#sending-files-in-group-chats)). Las encuestas se envían mediante Adaptive Cards. Las acciones de mensaje exponen `upload-file` explícito para envíos donde el archivo va primero.

## Plugin incluido

Microsoft Teams se distribuye como Plugin incluido en las versiones actuales de OpenClaw, por lo que no se requiere una instalación
independiente en la compilación empaquetada normal.

Si usas una compilación anterior o una instalación personalizada que excluye Teams incluido,
instala directamente el paquete npm:
...
<Note>
Teams CLI está actualmente en vista previa. Los comandos y las marcas pueden cambiar entre versiones.
</Note>

**2. Inicia un túnel** (Teams no puede alcanzar localhost)

Instala y autentica devtunnel CLI si aún no lo has hecho ([guía de introducción](https://learn.microsoft.com/en-us/azure/developer/dev-tunnels/get-started)).
...
<Note>
`--allow-anonymous` es obligatorio porque Teams no puede autenticarse con devtunnels. Cada solicitud de bot entrante sigue siendo validada automáticamente por Teams SDK.
</Note>
...
- Registra el bot (administrado por Teams de forma predeterminada; no se necesita una suscripción de Azure)
...
O usa directamente variables de entorno: `MSTEAMS_APP_ID`, `MSTEAMS_APP_PASSWORD`, `MSTEAMS_TENANT_ID`.

**5. Instala la aplicación en Teams**

`teams app create` te pedirá instalar la aplicación; selecciona "Instalar en Teams". Si lo omitiste, puedes obtener el enlace más adelante:
...
<Note>
Los chats de grupo están bloqueados de forma predeterminada (`channels.msteams.groupPolicy: "allowlist"`). Para permitir respuestas de grupo, establece `channels.msteams.groupAllowFrom` o usa `groupPolicy: "open"` para permitir a cualquier miembro (controlado por mención).
</Note>
...
- Usar de forma predeterminada un comportamiento seguro de canal (menciones requeridas salvo que se configure lo contrario).
...
- No dependas de la coincidencia por UPN/nombre para mostrar para las listas de permitidos, ya que pueden cambiar. OpenClaw desactiva de forma predeterminada la coincidencia directa de nombres; actívala explícitamente con `channels.msteams.dangerouslyAllowNameMatching: true`.
...
- Establece `groupPolicy: "open"` para permitir a cualquier miembro (aun así, controlado por mención de forma predeterminada).
...
- Delimita las respuestas de grupo/canal enumerando equipos y canales en `channels.msteams.teams`.
- Las claves deben usar IDs de conversación estables de Teams desde enlaces de Teams, no nombres para mostrar mutables.
- Cuando `groupPolicy="allowlist"` y hay una lista de permitidos de equipos, solo se aceptan los equipos/canales enumerados (controlado por mención).
- El asistente de configuración acepta entradas `Team/Channel` y las almacena por ti.
- Al iniciar, OpenClaw resuelve nombres de equipo/canal y de lista de permitidos de usuarios a IDs (cuando los permisos de Graph lo permiten)
...
<details>
<summary><strong>Configuración manual (sin Teams CLI)</strong></summary>

Si no puedes usar Teams CLI, puedes configurar el bot manualmente mediante Azure Portal.
...
3. Crea un **paquete de aplicación de Teams** que haga referencia al bot e incluya los permisos RSC siguientes.
...
<Warning>
La creación de nuevos bots multiinquilino quedó obsoleta después del 2025-07-31. Usa **Single Tenant** para bots nuevos.
</Warning>
...
3. Haz clic en **Manage Password** → ve al registro de aplicación
...
2. En Entra ID → Registro de aplicación → **Certificates & secrets** → **Certificates** → carga el certificado público.
...
Usa Azure Managed Identity para autenticación sin contraseña. Esto es ideal para despliegues en infraestructura de Azure (AKS, App Service, VM de Azure) donde hay una identidad administrada disponible.
...
4. El token se pasa a Teams SDK para la autenticación del bot.

**Requisitos previos:**

- Infraestructura de Azure con identidad administrada activada (identidad de carga de trabajo de AKS, App Service, VM)
...
| **Certificado**        | `authType: "federated"` + `certificatePath`    | Sin secreto compartido por la red     | Sobrecarga de administración de certificados    |
| **Managed Identity**   | `authType: "federated"` + `useManagedIdentity` | Sin contraseña, sin secretos que administrar | Requiere infraestructura de Azure          |

**Comportamiento predeterminado:** Cuando `authType` no está definido, OpenClaw usa autenticación con secreto de cliente de forma predeterminada. Las configuraciones existentes siguen funcionando sin cambios.
...
- Recurre a `messages.groupChat.historyLimit`. Define `0` para deshabilitarlo (valor predeterminado 50).
- El historial de hilos recuperado se filtra mediante listas de remitentes permitidos (`allowFrom` / `groupAllowFrom`), por lo que la inicialización del contexto del hilo solo incluye mensajes de remitentes permitidos.
...
- `ChannelMessage.Read.Group` (Aplicación) - recibir todos los mensajes de canal sin @mención
...
- `ChatMessage.Read.Chat` (Aplicación) - recibir todos los mensajes de chat grupal sin @mención
...
- **Contenido de imágenes o archivos** de canal/grupo (la carga útil solo incluye un marcador HTML).
...
**Conclusión:** RSC es para escuchar en tiempo real; Graph API es para acceso histórico. Para ponerse al día con mensajes perdidos mientras estás sin conexión, necesitas Graph API con `ChannelMessage.Read.All` (requiere consentimiento de administrador).
...
**Permiso adicional para menciones de usuarios:** Las @menciones de usuarios funcionan de forma predeterminada para usuarios en la conversación. Sin embargo, si quieres buscar y mencionar dinámicamente a usuarios que **no están en la conversación actual**, agrega el permiso `User.Read.All` (Aplicación) y concede consentimiento de administrador.
...
- `channels.msteams.allowFrom`: lista de permitidos de mensajes directos (se recomiendan ID de objeto de AAD). El asistente resuelve nombres a ID durante la configuración cuando el acceso a Graph está disponible.
...
- `channels.msteams.teams.<teamId>.tools`: anulaciones predeterminadas por equipo de la política de herramientas (`allow`/`deny`/`alsoAllow`) utilizadas cuando falta una anulación de canal.
...
- `top-level` en un canal con estilo de publicaciones → las respuestas aparecen como publicaciones separadas de nivel superior en lugar de dentro del hilo
...
- **Mensajes directos:** Las imágenes y los adjuntos de archivos funcionan mediante las API de archivos de bots de Teams.
- **Canales/grupos:** Los adjuntos residen en almacenamiento de M365 (SharePoint/OneDrive). La carga útil del Webhook solo incluye un fragmento HTML, no los bytes reales del archivo. **Se requieren permisos de Graph API** para descargar adjuntos de canal.
- Para envíos explícitos centrados primero en archivos, usa `action=upload-file` con `media` / `filePath` / `path`; el `message` opcional se convierte en el texto/comentario acompañante, y `filename` anula el nombre subido.
...
| ------------------------ | ---------------------------------------------- | ----------------------------------------------- |
| **Mensajes directos**    | FileConsentCard → el usuario acepta → el bot sube | Funciona de inmediato                           |
| **Chats grupales/canales** | Subir a SharePoint → compartir enlace          | Requiere `sharePointSiteId` + permisos de Graph |
| **Imágenes (cualquier contexto)** | En línea codificadas en Base64                 | Funciona de inmediato                           |
...
Para detalles sobre el formato de destino, consulta [Formatos de destino](#target-formats) abajo.
...
<Note>
Sin el prefijo `user:`, los nombres se resuelven de forma predeterminada como grupo o equipo. Usa siempre `user:` al dirigirte a personas por nombre visible.
</Note>
...
- Consulta `/gateway/configuration` para `dmPolicy` y la compuerta de la lista de permitidos.
...
https://teams.microsoft.com/l/team/19%3ABk4j...%40thread.tacv2/conversations?groupId=...
                                    └────────────────────────────┘
                                    Id. de conversación de equipo (decodifica esto de la URL)
```
...
- Clave de equipo = segmento de ruta después de `/team/` (decodificado de URL, por ejemplo, `19:Bk4j...@thread.tacv2`; los inquilinos antiguos pueden mostrar `@thread.skype`, que también es válido)
...
- **Las imágenes no aparecen en los canales:** faltan permisos de Graph o consentimiento de administrador. Reinstala la aplicación de Teams y cierra por completo/vuelve a abrir Teams.
...
- **401 No autorizado desde Webhook:** esperado al probar manualmente sin JWT de Azure; significa que el endpoint es accesible, pero falló la autenticación. Usa Azure Web Chat para probar correctamente.

### Errores de carga del manifiesto

- **"El archivo de icono no puede estar vacío":** el manifiesto referencia archivos de icono que tienen 0 bytes. Crea iconos PNG válidos (32x32 para `outline.png`, 192x192 para `color.png`).
- **"webApplicationInfo.Id ya está en uso":** la aplicación sigue instalada en otro equipo/chat. Búscala y desinstálala primero, o espera entre 5 y 10 minutos para la propagación.
- **"Algo salió mal" al cargar:** carga mediante [https://admin.teams.microsoft.com](https://admin.teams.microsoft.com) en su lugar, abre DevTools del navegador (F12) → pestaña Network y revisa el cuerpo de la respuesta para ver el error real.
- **Error en la carga lateral:** prueba "Cargar una aplicación en el catálogo de aplicaciones de tu organización" en lugar de "Cargar una aplicación personalizada"; esto suele evitar restricciones de carga lateral.
...
- [Grupos](/es/channels/groups) — comportamiento de chats de grupo y compuerta por menciones
