...
title: Matriz
x-i18n:
    generated_at: "2026-05-10T19:22:12Z"
    model: gpt-5.5
    provider: openai
...
Las especificaciones de Plugin sin prefijo prueban primero ClawHub y luego recurren a npm. Para forzar el origen del registro, usa `openclaw plugins install clawhub:@openclaw/matrix` o `openclaw plugins install npm:@openclaw/matrix`.
...
4. Inicia un MD con el bot o invítalo a una sala (consulta [unión automática](#auto-join): las invitaciones nuevas solo llegan cuando `autoJoin` las permite).
...
El asistente solicita: URL del homeserver, método de autenticación (token de acceso o contraseña), ID de usuario (solo autenticación con contraseña), nombre de dispositivo opcional, si se debe habilitar E2EE y si se deben configurar el acceso a salas y la unión automática.

Si ya existen variables de entorno `MATRIX_*` coincidentes y la cuenta seleccionada no tiene autenticación guardada, el asistente ofrece un atajo mediante variables de entorno. Para resolver nombres de salas antes de guardar una lista de permitidos, ejecuta `openclaw channels resolve --channel matrix "Project Room"`. Cuando E2EE está habilitado, el asistente escribe la configuración y ejecuta el mismo arranque que [`openclaw matrix encryption setup`](#encryption-and-verification).
...
`channels.matrix.autoJoin` tiene como valor predeterminado `off`. Con el valor predeterminado, el bot no aparecerá en salas nuevas ni en MD de invitaciones recientes hasta que te unas manualmente.

OpenClaw no puede saber en el momento de la invitación si una sala invitada es un MD o un grupo, por lo que todas las invitaciones, incluidas las de estilo MD, pasan primero por `autoJoin`. `dm.policy` solo se aplica después, cuando el bot se ha unido y la sala se ha clasificado.

<Warning>
Establece `autoJoin: "allowlist"` junto con `autoJoinAllowlist` para restringir qué invitaciones acepta el bot, o `autoJoin: "always"` para aceptar todas las invitaciones.
...
Las listas de permitidos de MD y salas se completan mejor con ID estables:

- MD (`dm.allowFrom`, `groupAllowFrom`, `groups.<room>.users`): usa `@user:server`. Los nombres visibles solo se resuelven cuando el directorio del homeserver devuelve exactamente una coincidencia.
...
Para la cuenta `ops`, los nombres pasan a ser `MATRIX_OPS_HOMESERVER`, `MATRIX_OPS_ACCESS_TOKEN`, etc. Las variables de entorno de clave de recuperación las leen los flujos de CLI compatibles con recuperación (`verify backup restore`, `verify device`, `verify bootstrap`) cuando canalizas la clave mediante `--recovery-key-stdin`.

`MATRIX_HOMESERVER` no puede establecerse desde un `.env` de espacio de trabajo; consulta [Archivos `.env` de espacio de trabajo](/es/gateway/security).
...
Para conservar las vistas previas de respuestas en vivo pero ocultar las líneas intermedias de herramientas/progreso, usa la forma de objeto:
...
Las solicitudes de aprobación nativas de Matrix son eventos `m.room.message` normales con contenido de evento personalizado específico de OpenClaw bajo `com.openclaw.approval`. Matrix permite claves de contenido de evento personalizadas, por lo que los clientes estándar siguen renderizando el cuerpo del texto, mientras que los clientes compatibles con OpenClaw pueden leer el ID de aprobación estructurado, el tipo, el estado, las decisiones disponibles y los detalles de exec/Plugin.

Cuando una solicitud de aprobación es demasiado larga para un evento de Matrix, OpenClaw divide el texto visible en fragmentos y adjunta `com.openclaw.approval` solo al primer fragmento. Las reacciones para decisiones de permitir/denegar se vinculan a ese primer evento, por lo que las solicitudes largas mantienen el mismo destino de aprobación que las solicitudes de un solo evento.
...
De forma predeterminada, se ignoran los mensajes de Matrix de otras cuentas de Matrix configuradas en OpenClaw.
...
Usa listas de permitidos de salas estrictas y requisitos de mención al habilitar el tráfico bot a bot en salas compartidas.

## Cifrado y verificación

En salas cifradas (E2EE), los eventos de imagen salientes usan `thumbnail_file` para que las vistas previas de imagen se cifren junto con el adjunto completo. Las salas sin cifrar siguen usando `thumbnail_url` sin formato. No se necesita configuración: el Plugin detecta automáticamente el estado de E2EE.
...
Inicializa el almacenamiento secreto y la firma cruzada, crea una copia de seguridad de claves de sala si es necesario y luego imprime el estado y los próximos pasos. Marcas útiles:

- `--recovery-key <key>` aplica una clave de recuperación antes de inicializar (prefiere la forma por stdin documentada abajo)
- `--force-reset-cross-signing` descarta la identidad de firma cruzada actual y crea una nueva (úsala solo de forma intencional)

Para una cuenta nueva, habilita E2EE durante la creación:
...
- `Locally trusted`: confianza otorgada solo por este cliente
- `Cross-signing verified`: el SDK informa verificación mediante firma cruzada
- `Signed by owner`: firmada por tu propia clave de autofirma (solo diagnóstico)

`Verified by owner` pasa a `yes` solo cuando `Cross-signing verified` es `yes`. La confianza local o una firma del propietario por sí solas no son suficientes.
...
Si el homeserver requiere UIA para cargar claves de firma cruzada, OpenClaw intenta primero sin autenticación, luego `m.login.dummy` y después `m.login.password` (requiere `channels.matrix.password`).
...
Para reemplazar una copia de seguridad rota con una línea base nueva (acepta perder historial antiguo irrecuperable; también puede recrear el almacenamiento secreto si el secreto de la copia de seguridad actual no se puede cargar):

```bash
openclaw matrix verify backup reset --yes
```

Añade `--rotate-recovery-key` solo cuando quieras intencionalmente que la clave de recuperación anterior deje de desbloquear la línea base nueva de la copia de seguridad.
...
Envía una solicitud de verificación desde esta cuenta de OpenClaw. `--own-user` solicita autoverificación (aceptas el aviso en otro cliente de Matrix del mismo usuario); `--user-id`/`--device-id`/`--room-id` apuntan a otra persona. `--own-user` no se puede combinar con las demás marcas de destino.

Para gestionar el ciclo de vida a nivel más bajo, normalmente al seguir solicitudes entrantes desde otro cliente, estos comandos actúan sobre una solicitud específica `<id>` (impresa por `verify list` y `verify request`):
...
`accept`, `start`, `sas`, `confirm-sas`, `mismatch-sas` y `cancel` aceptan todos `--user-id` y `--room-id` como pistas de seguimiento de DM cuando la verificación está anclada a una sala de mensaje directo específica.
...
<AccordionGroup>
  ...
  <Accordion title="Verification notices">
    Matrix publica avisos del ciclo de vida de verificación en la sala estricta de verificación por DM como mensajes `m.notice`: solicitud, listo (con guía de "Verify by emoji"), inicio/finalización y detalles SAS (emoji/decimal) cuando estén disponibles.

    Las solicitudes entrantes desde otro cliente de Matrix se rastrean y aceptan automáticamente. Para la autoverificación, OpenClaw inicia el flujo SAS automáticamente y confirma su propio lado una vez que la verificación por emoji está disponible; aun así, debes comparar y confirmar "They match" en tu cliente de Matrix.
...
```
  ...
  <Accordion title="Device hygiene">
    Los dispositivos antiguos gestionados por OpenClaw pueden acumularse. Lista y depura:
...
Puedes pasar ambas opciones en una sola llamada. Matrix acepta directamente URL de avatar `mxc://`; cuando pasas `http://` o `https://`, OpenClaw carga primero el archivo y guarda la URL `mxc://` resuelta en `channels.matrix.avatarUrl` (o en la sobrescritura por cuenta).

## Hilos

Matrix admite hilos nativos de Matrix tanto para respuestas automáticas como para envíos con la herramienta de mensajes. Dos controles independientes gobiernan el comportamiento:
...
Los enlaces de conversación explícitos siempre prevalecen sobre `sessionScope`, por lo que las salas y los hilos enlazados conservan la sesión de destino elegida.

### Hilos de respuesta (`threadReplies`)

`threadReplies` decide dónde publica el bot su respuesta:

- `"off"`: las respuestas son de nivel superior. Los mensajes entrantes con hilo permanecen en la sesión principal.
- `"inbound"`: responde dentro de un hilo solo cuando el mensaje entrante ya estaba en ese hilo.
- `"always"`: responde dentro de un hilo enraizado en el mensaje que lo desencadena; esa conversación se enruta mediante una sesión con ámbito de hilo coincidente desde el primer desencadenante en adelante.
...
- Los envíos con la herramienta de mensajes heredan automáticamente el hilo de Matrix actual cuando apuntan a la misma sala (o al mismo destino de usuario de DM), salvo que se proporcione un `threadId` explícito.
- La reutilización de destinos de usuario de DM solo se activa cuando los metadatos de la sesión actual demuestran que es el mismo par de DM en la misma cuenta de Matrix; de lo contrario, OpenClaw vuelve al enrutamiento normal con alcance de usuario.
- `/focus`, `/unfocus`, `/agents`, `/session idle`, `/session max-age` y `/acp spawn` ligado a un hilo funcionan en salas de Matrix y en DM.
- `/focus` de nivel superior crea un nuevo hilo de Matrix y lo vincula con la sesión de destino cuando `threadBindings.spawnSessions` está habilitado.
- Ejecutar `/focus` o `/acp spawn --thread here` dentro de un hilo de Matrix existente vincula ese hilo en el lugar.

Cuando OpenClaw detecta una sala de DM de Matrix que colisiona con otra sala de DM en la misma sesión compartida, publica un `m.notice` único en esa sala que apunta a la salida de emergencia `/focus` y sugiere un cambio de `dm.sessionScope`. El aviso solo aparece cuando las vinculaciones de hilos están habilitadas.

## Vinculaciones de conversaciones ACP

Las salas de Matrix, los DM y los hilos de Matrix existentes pueden convertirse en espacios de trabajo ACP duraderos sin cambiar la superficie de chat.

Flujo rápido para operadores:

- Ejecuta `/acp spawn codex --bind here` dentro del DM, la sala o el hilo existente de Matrix que quieres seguir usando.
- En un DM o una sala de Matrix de nivel superior, el DM/la sala actual permanece como la superficie de chat y los mensajes futuros se enrutan a la sesión ACP generada.
...
Las generaciones de sesiones ligadas a hilos de Matrix están habilitadas de forma predeterminada:

- Establece `threadBindings.spawnSessions: false` para impedir que `/focus` de nivel superior y `/acp spawn --thread auto|here` creen/vinculen hilos de Matrix.
- Establece `threadBindings.defaultSpawnContext: "isolated"` cuando las generaciones de hilos de subagentes nativos no deban bifurcar la transcripción principal.
...
- `reactions` lista el resumen de reacciones actual para un evento de Matrix.
...
- `channels.matrix.historyLimit` controla cuántos mensajes recientes de la sala se incluyen como `InboundHistory` cuando un mensaje de sala de Matrix activa el agente. Recurre a `messages.groupChat.historyLimit`; si ambos no están establecidos, el valor predeterminado efectivo es `0`. Establece `0` para deshabilitarlo.
- El historial de salas de Matrix solo pertenece a la sala. Los DM siguen usando el historial normal de sesión.
- El historial de salas de Matrix solo incluye pendientes: OpenClaw almacena en búfer mensajes de sala que todavía no activaron una respuesta y luego captura esa ventana cuando llega una mención u otro activador.
- El mensaje activador actual no se incluye en `InboundHistory`; permanece en el cuerpo entrante principal de ese turno.
- Los reintentos del mismo evento de Matrix reutilizan la instantánea de historial original en lugar de avanzar hacia mensajes de sala más nuevos.
...
- `contextVisibility: "all"` es el valor predeterminado. El contexto complementario se conserva como se recibió.
- `contextVisibility: "allowlist"` filtra el contexto complementario para enviadores permitidos por las comprobaciones activas de lista de permitidos de sala/usuario.
- `contextVisibility: "allowlist_quote"` se comporta como `allowlist`, pero aun así conserva una respuesta citada explícita.

Este ajuste afecta la visibilidad del contexto complementario, no si el mensaje entrante en sí puede activar una respuesta.
...
Para silenciar los DM por completo y mantener las salas funcionando, establece `dm.enabled: false`:
...
Consulta [Grupos](/es/channels/groups) para conocer el comportamiento de control por mención y lista de permitidos.
...
Si un usuario de Matrix no aprobado sigue enviándote mensajes antes de la aprobación, OpenClaw reutiliza el mismo código de emparejamiento pendiente y puede enviar una respuesta recordatoria después de un breve tiempo de espera en lugar de acuñar un código nuevo.

Consulta [Emparejamiento](/es/channels/pairing) para conocer el flujo compartido de emparejamiento de DM y el diseño de almacenamiento.
...
- recurre a cualquier DM 1:1 estricto unido actualmente con ese usuario
...
- `enabled`: entrega aprobaciones mediante avisos nativos de Matrix. Cuando no está establecido o es `"auto"`, Matrix se habilita automáticamente una vez que se pueda resolver al menos un aprobador. Establece `false` para deshabilitarlo explícitamente.
...
- `target`: adónde van los avisos. `"dm"` (predeterminado) envía a los DM de los aprobadores; `"channel"` envía a la sala o el DM de Matrix de origen; `"both"` envía a ambos.
- `agentFilter` / `sessionFilter`: listas de permitidos opcionales para determinar qué agentes/sesiones activan la entrega por Matrix.

La autorización difiere ligeramente entre tipos de aprobación:

- **Aprobaciones de exec** usan `execApprovals.approvers` y recurren a `dm.allowFrom`.
...
- `✅` permitir una vez
- `❌` denegar
- `♾️` permitir siempre (cuando la política de exec efectiva lo permite)

Comandos slash de reserva: `/approve <id> allow-once`, `/approve <id> allow-always`, `/approve <id> deny`.

Solo los aprobadores resueltos pueden aprobar o denegar. La entrega al canal para aprobaciones de exec incluye el texto del comando; habilita `channel` o `both` solo en salas de confianza.

Relacionado: [Aprobaciones de exec](/es/tools/exec-approvals).

## Comandos slash

Los comandos slash (`/new`, `/reset`, `/model`, `/focus`, `/unfocus`, `/agents`, `/session`, `/acp`, `/approve`, etc.) funcionan directamente en DM. En salas, OpenClaw también reconoce comandos que tienen como prefijo la mención de Matrix propia del bot, por lo que `@bot:server /new` activa la ruta de comandos sin una expresión regular de mención personalizada. Esto mantiene al bot receptivo a las publicaciones de sala de estilo `@mention /command` que Element y clientes similares emiten cuando un usuario autocompleta con tabulación el bot antes de escribir el comando.

Las reglas de autorización siguen aplicándose: los remitentes de comandos deben satisfacer las mismas políticas de propietario/lista de permitidos de DM o sala que los mensajes normales.
...
- Los valores de nivel superior de `channels.matrix` actúan como valores predeterminados para cuentas con nombre salvo que una cuenta los sobrescriba.
- Delimita el alcance de una entrada de sala heredada a una cuenta específica con `groups.<room>.account`. Las entradas sin `account` se comparten entre cuentas; `account: "default"` sigue funcionando cuando la cuenta predeterminada está configurada en el nivel superior.

**Selección de cuenta predeterminada:**

- Establece `defaultAccount` para elegir la cuenta con nombre que prefieren el enrutamiento implícito, las sondas y los comandos CLI.
- Si tienes varias cuentas y una se llama literalmente `default`, OpenClaw la usa implícitamente incluso cuando `defaultAccount` no está establecido.
- Si tienes varias cuentas con nombre y no se selecciona ninguna predeterminada, los comandos CLI se niegan a adivinar: establece `defaultAccount` o pasa `--account <id>`.
...
- Cuando OpenClaw promociona una configuración de una sola cuenta a varias cuentas durante reparación o configuración inicial, conserva la cuenta con nombre existente si existe alguna o si `defaultAccount` ya apunta a una. Solo las claves de autenticación/arranque de Matrix se mueven a la cuenta promocionada; las claves compartidas de política de entrega permanecen en el nivel superior.

Consulta [Referencia de configuración](/es/gateway/config-channels#multi-account-all-channels) para conocer el patrón compartido de varias cuentas.

## Servidores domésticos privados/LAN

De forma predeterminada, OpenClaw bloquea servidores domésticos de Matrix privados/internos para protección contra SSRF, salvo que
...
Esta opción explícita solo permite destinos privados/internos de confianza. Los servidores de origen públicos sin cifrar como
...
Los ID de sala de Matrix distinguen mayúsculas y minúsculas. Usa exactamente las mismas mayúsculas y minúsculas del ID de sala de Matrix
...
- Las búsquedas de usuario consultan el directorio de usuarios de Matrix en ese servidor de origen.
- Las búsquedas de sala aceptan directamente ID de sala y alias explícitos, y luego recurren a buscar nombres de salas unidas para esa cuenta.
- La búsqueda de nombres de salas unidas es de mejor esfuerzo. Si un nombre de sala no puede resolverse a un ID o alias, la resolución de listas de permitidos en tiempo de ejecución lo ignora.

## Referencia de configuración

Los campos de estilo lista de permitidos (`groupAllowFrom`, `dm.allowFrom`, `groups.<room>.users`) aceptan ID de usuario completos de Matrix (lo más seguro). Las coincidencias exactas de directorio se resuelven al inicio y cada vez que cambia la lista de permitidos mientras el monitor está en ejecución; las entradas que no puedan resolverse se ignoran en tiempo de ejecución. Las listas de permitidos de salas prefieren ID de sala o alias por el mismo motivo.
...
- `proxy`: URL opcional de proxy HTTP(S) para el tráfico de Matrix. Se admite anulación por cuenta.
- `userId`: ID de usuario completo de Matrix (`@bot:example.org`).
- `accessToken`: token de acceso para autenticación basada en token. Se admiten valores de texto sin formato y SecretRef en proveedores de entorno/archivo/exec ([Gestión de secretos](/es/gateway/secrets)).
...
- `dm.threadReplies`: anulación solo para DM del enlazado de respuestas (`"off"`, `"inbound"`, `"always"`).
...
- `autoJoin`: `"always"`, `"allowlist"` o `"off"`. Valor predeterminado: `"off"`. Se aplica a todas las invitaciones de Matrix, incluidas las invitaciones de estilo DM.
...
- `ackReactionScope`: anulación de alcance (`"group-mentions"` predeterminado, `"group-all"`, `"direct"`, `"all"`, `"none"`, `"off"`).
...
- `groups`: mapa de políticas por sala. La identidad de sesión usa el ID de sala estable después de la resolución. (`rooms` es un alias heredado).
  ...
  - `groups.<room>.autoReply`: anulación por sala del requisito de mención. `true` deshabilita los requisitos de mención para esa sala; `false` los vuelve a forzar.
  - `groups.<room>.skills`: filtro de skill por sala.
  - `groups.<room>.systemPrompt`: fragmento de indicación del sistema por sala.

### Configuración de aprobación de exec

- `execApprovals.enabled`: entrega aprobaciones de exec mediante solicitudes nativas de Matrix.
...
- [Grupos](/es/channels/groups) - comportamiento del chat grupal y requisito de mención
