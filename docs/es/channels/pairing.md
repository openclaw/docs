...
contiene `open` con entradas concretas de `allowFrom`, en tiempo de ejecución aún se admiten
...
- La cuenta predeterminada usa el archivo de lista de permitidos sin ámbito del canal.

Trátalos como confidenciales (controlan el acceso a tu asistente).

<Note>
El almacén de lista de permitidos de emparejamiento es para el acceso de DM. La autorización de grupos es independiente.
Aprobar un código de emparejamiento de DM no permite automáticamente a ese remitente ejecutar comandos de grupo
ni controlar el bot en grupos. La inicialización del primer propietario es un estado de configuración separado
en `commands.ownerAllowFrom`, y la entrega de chat grupal sigue las listas de permitidos de grupo
...
- `bootstrapToken`: un token de arranque de dispositivo único y corta duración usado para el handshake inicial de emparejamiento
...
- las comprobaciones de ámbito de arranque tienen prefijo de rol, no son un único conjunto plano de ámbitos:
  las entradas de ámbito de operator solo satisfacen solicitudes de operator, y los roles que no son operator
...
- la rotación/revocación posterior de tokens permanece limitada tanto por el contrato de rol aprobado del dispositivo
  como por los ámbitos de operator de la sesión que llama
...
fallan de forma cerrada antes de emitir el QR/código de configuración.
...
Gateway todavía valida la conexión reintentada; los tokens que no pueden autenticarse
con `operator.admin` siguen bloqueados.

Si el mismo dispositivo vuelve a intentarlo con detalles de autenticación diferentes (por ejemplo, diferente
rol/ámbitos/clave pública), la solicitud pendiente anterior se sustituye y se crea un nuevo
`requestId`.

<Note>
Un dispositivo ya emparejado no obtiene acceso más amplio silenciosamente. Si se reconecta solicitando más ámbitos o un rol más amplio, OpenClaw mantiene la aprobación existente tal cual y crea una nueva solicitud pendiente de actualización. Usa `openclaw devices list` para comparar el acceso aprobado actualmente con el acceso recién solicitado antes de aprobar.
</Note>
...
ámbitos solicitados. Los clientes de operator, navegador, Control UI y WebChat aún requieren aprobación
...
- La API heredada `node.pair.*` (CLI: `openclaw nodes pending|approve|reject|remove|rename`) es un
  almacén de emparejamiento separado propiedad del gateway. Los nodos WS aún requieren emparejamiento de dispositivos.
