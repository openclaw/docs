...
pasa al proceso guiado de incorporación. Use `-m`/`--message` para una solicitud o
...
En el modo guiado, `--workspace <dir>` es el espacio de trabajo que se propone a OpenClaw;
solo se conserva después de aprobar la propuesta. Las configuraciones de referencia, clásica y
...
modelos configurados, los inicios de sesión compatibles de la CLI, las variables de entorno de
claves de API y los modelos de Ollama o LM Studio ya instalados. Este proceso
automático nunca descarga modelos locales. Los entornos de ejecución locales detectados se prueban
automáticamente después de los candidatos de la CLI y de claves de API; cuando hay varios modelos
locales disponibles, OpenClaw prefiere la familia de instrucciones más potente que admita llamadas
a herramientas. El candidato seleccionado debe responder a una finalización real antes de que se
guarden la configuración de su proveedor y la del modelo. También se informa de las CLI instaladas
de Gemini, Antigravity, Pi y OpenCode cuando no pueden utilizarse como ruta de inferencia reutilizable
para la configuración guiada.

`setup` acepta las mismas opciones de incorporación que `openclaw onboard`, incluidas las de
autenticación (`--auth-choice`, `--token`, opciones de claves del proveedor), Gateway
...
ejemplos no interactivos. `openclaw onboard --modern` se mantiene como una entrada de
compatibilidad para el mismo asistente de OpenClaw sujeto a la comprobación de inferencia.

<Note>
`openclaw setup` está destinado a instalaciones con configuración mutable. En el modo Nix (`OPENCLAW_NIX_MODE=1`), OpenClaw rechaza las escrituras de configuración porque Nix gestiona el archivo de configuración. Use la [Guía de inicio rápido de nix-openclaw](https://github.com/openclaw/nix-openclaw#quick-start) oficial o la configuración de origen equivalente para otro paquete de Nix.
</Note>
...
| `--workspace <dir>`        | Propuesta de espacio de trabajo en el modo guiado; se conserva directamente en la configuración de referencia, clásica y no interactiva. |
| `--baseline`               | Crea las carpetas de configuración, espacio de trabajo y sesión de referencia sin incorporación.                |
...
asistente con indicaciones, mientras que la configuración no interactiva utiliza la ruta de automatización.

### Modo de referencia

`openclaw setup --baseline` conserva el comportamiento anterior limitado a la configuración de referencia:
crea los directorios de configuración, espacio de trabajo y sesiones y, a continuación, finaliza sin
...
- Después de la configuración de referencia, ejecute `openclaw onboard` para realizar todo el recorrido guiado, `openclaw configure` para efectuar cambios específicos o `openclaw channels add` para añadir cuentas de canales.
- Si se detecta el estado de Hermes, la incorporación interactiva puede ofrecer la migración automáticamente. La incorporación con importación requiere una configuración nueva; use [Migración](/es/cli/migrate) para obtener planes de simulación, copias de seguridad y el modo de sobrescritura fuera de la incorporación.
