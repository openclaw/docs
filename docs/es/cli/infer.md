...
summary: CLI que prioriza la inferencia para flujos de trabajo de modelos, imágenes, audio, TTS, video, web e incrustaciones respaldados por proveedores
title: CLI de inferencia
x-i18n:
    generated_at: "2026-05-02T05:22:42Z"
    model: gpt-5.5
    provider: openai
...
| Generar un video             | `openclaw infer video generate --prompt "..." --json`                                         | Admite pistas de proveedor como `--resolution`        |
...
- `model run` local es una finalización de proveedor puntual y ligera. Resuelve el modelo de agente configurado y la autenticación, pero no inicia un turno de agente de chat, carga herramientas ni abre servidores MCP incluidos.
...
- `model run --gateway` ejercita el enrutamiento de Gateway, la autenticación guardada, la selección de proveedor y el runtime integrado, pero aun así se ejecuta como una prueba de modelo sin procesar: envía el prompt suministrado y cualquier adjunto de imagen sin transcripción previa de sesión, contexto de arranque/AGENTS, ensamblaje del motor de contexto, herramientas ni servidores MCP incluidos.
...
- `model run` local es la prueba smoke de CLI más estrecha para la salud de proveedor/modelo/autenticación porque solo envía el prompt suministrado al modelo seleccionado.
- `model run --file` local conserva esa ruta ligera y adjunta contenido de imagen directamente al único mensaje de usuario. Los archivos de imagen comunes como PNG, JPEG y WebP funcionan cuando su tipo MIME se detecta como `image/*`; los archivos no compatibles o no reconocidos fallan antes de llamar al proveedor.
...
- `model run` local sale con código distinto de cero cuando el proveedor no devuelve salida de texto, por lo que los proveedores locales inaccesibles y las finalizaciones vacías no parecen pruebas exitosas.
...
- Usa `--size`, `--aspect-ratio` o `--resolution` con `image edit` para proveedores/modelos que admiten pistas de geometría en ediciones con imágenes de referencia.
- Usa `--output-format png --background transparent` con `--model openai/gpt-image-1.5` para salida PNG de OpenAI con fondo transparente; `--openai-background` sigue disponible como alias específico de OpenAI. Los proveedores que no declaran compatibilidad con fondo informan la pista como una anulación ignorada.
- Usa `image providers --json` para verificar qué proveedores de imagen incluidos son detectables, están configurados, están seleccionados y qué capacidades de generación/edición expone cada proveedor.
- Usa `image generate --model <provider/model> --json` como la prueba smoke de CLI en vivo más estrecha para cambios de generación de imágenes. Ejemplo:
  ...
  salida escritas. Cuando se define `--output`, la extensión final puede seguir el
  tipo MIME devuelto por el proveedor.

- Para `image describe` e `image describe-many`, usa `--prompt` para dar al modelo de visión una instrucción específica de la tarea, como OCR, comparación, inspección de la interfaz de usuario o descripción breve.
...
Usa `video` para la generación y la descripción.
...
Usa `embedding` para la creación de vectores y la inspección de proveedores de embedding.
...
Los comandos infer normalizan la salida JSON bajo una envoltura compartida:
