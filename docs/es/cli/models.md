...
summary: Referencia de CLI para `openclaw models` (status/list/set/scan, alias, alternativas de respaldo, autenticación)
title: Modelos
x-i18n:
...
Detección, escaneo y configuración de modelos (modelo predeterminado, alternativas, perfiles de autenticación).
...
`openclaw models status` muestra el valor predeterminado resuelto y las alternativas, además de una descripción general de la autenticación.
...
En la salida `--json`, `auth.providers` es la descripción general de proveedores con conocimiento de env/config/store,
mientras que `auth.oauth` es solo el estado de los perfiles del almacén de autenticación.
...
el comando usa `OPENCLAW_AGENT_DIR`/`PI_CODING_AGENT_DIR` si están configurados; de lo contrario, usa el
agente predeterminado configurado.
Las filas de prueba pueden provenir de perfiles de autenticación, credenciales env o `models.json`.
...
- La columna `Auth` está a nivel de proveedor y es de solo lectura. Se calcula a partir de metadatos
  ...
  de proveedor local, marcadores env/perfil de AWS Bedrock y metadatos de autenticación sintética de plugins;
  no carga el runtime del proveedor, lee secretos del llavero, llama a API del proveedor
...
- `models list --all --provider <id>` puede incluir filas de catálogo estático propiedad del proveedor
  desde manifiestos de plugins o metadatos de catálogo de proveedor incluido incluso cuando todavía
  no te has autenticado con ese proveedor. Esas filas aún se muestran como
...
- `models list` mantiene el plano de control receptivo mientras el descubrimiento del catálogo del proveedor
  ...
  sintéticas tras una breve espera y dejan que el descubrimiento termine en
...
uso como alternativa. El catálogo en sí es público, por lo que los escaneos de solo metadatos no necesitan
una clave de OpenRouter.

De forma predeterminada, OpenClaw intenta probar la compatibilidad con herramientas e imágenes mediante llamadas de modelo en vivo.
Si no hay una clave de OpenRouter configurada, el comando recurre a una salida de solo metadatos
...
de solo metadatos son informativos y no se aplican a la configuración.
...
`--json` mantiene stdout reservado para la carga JSON. Los diagnósticos de perfiles
de autenticación, proveedor e inicio se envían a stderr para que los scripts puedan canalizar stdout directamente
...
- `excluded_by_auth_order`: existe un perfil almacenado, pero `auth.order.<provider>`
  explícito lo omitió, así que la prueba informa la exclusión en lugar de
  intentarlo.
...
- `no_model`: existe autenticación del proveedor, pero OpenClaw no pudo resolver un candidato
  de modelo que se pueda probar para ese proveedor.
...
Usa `openclaw models auth --agent <id> <subcommand>` para escribir los resultados de autenticación en un
almacén de agente configurado específico. La opción principal `--agent` es respetada por
...
- Nota de Anthropic: el personal de Anthropic nos dijo que el uso de Claude CLI al estilo de OpenClaw vuelve a estar permitido, por lo que OpenClaw trata la reutilización de Claude CLI y el uso de `claude -p` como sancionados para esta integración, a menos que Anthropic publique una nueva política.
- Anthropic `setup-token` / `paste-token` siguen disponibles como una ruta de token de OpenClaw compatible, pero OpenClaw ahora prefiere la reutilización de Claude CLI y `claude -p` cuando están disponibles.
