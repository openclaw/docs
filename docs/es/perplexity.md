---
redirect: /tools/perplexity-search
summary: Redirigir a /tools/perplexity-search
title: Búsqueda de Perplexity
x-i18n:
    generated_at: "2026-05-06T05:41:03Z"
    model: gpt-5.5
    provider: openai
    source_hash: 1c41e7de5f183075e494182024214e3bbd4a93706de42a4ee6933a0c079d8cd0
    source_path: perplexity.md
    workflow: 16
    postprocess_version: locale-links-v1
---

Esta página se ha movido a [búsqueda de Perplexity](/es/tools/perplexity-search).

## Relacionado

- [Herramientas web](/es/tools/web)
