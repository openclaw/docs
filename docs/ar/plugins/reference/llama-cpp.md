---
read_when:
    - أنت تثبّت Plugin llama-cpp أو تضبطه أو تراجعه
summary: تضمينات GGUF المحلية عبر node-llama-cpp.
title: Plugin Llama Cpp
x-i18n:
    generated_at: "2026-06-27T18:13:22Z"
    model: gpt-5.5
    postprocess_version: locale-links-v1
    provider: openai
    source_hash: be9fd199e252fd3f905e8064316e45b9b2816945a2e5e16a56a6a88fdd55ef8e
    source_path: plugins/reference/llama-cpp.md
    workflow: 16
---

# Plugin Llama Cpp

تضمينات GGUF المحلية عبر node-llama-cpp.

## التوزيع

- الحزمة: `@openclaw/llama-cpp-provider`
- مسار التثبيت: npm؛ ClawHub

## السطح

العقود: embeddingProviders

## الوثائق ذات الصلة

- [llama-cpp](/ar/plugins/llama-cpp)
