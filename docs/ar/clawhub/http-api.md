...
- يجب ألا تعيد نقاط نهاية الكتابة المصادَق عليها رسالة `Unauthorized` مجردة عندما
...
- `X-RateLimit-Remaining` / `RateLimit-Remaining`: الميزانية المتبقية الدقيقة عند وجودها.
  تحذف الطلبات الناجحة المقسمة إلى أجزاء هذه الترويسة بدلا من إرجاع قيمة عامة تقريبية.
- `Retry-After`: عدد الثواني الواجب انتظارها قبل إعادة المحاولة (تأخير) عند `429`
...
- استخدم تراجعا مع تذبذب لتجنب إعادة المحاولات المتزامنة.
...
- يستخدم ترويسات IP العميل الموثوقة، بما في ذلك `cf-connecting-ip`، فقط عندما
  يفعّل النشر صراحة الترويسات المعاد توجيهها الموثوقة.
- يستخدم ClawHub ترويسات التوجيه الموثوقة لتحديد عناوين IP للعملاء عند الحافة.
- إذا لم يتوفر IP عميل موثوق، تستخدم الطلبات المجهولة حاويات احتياطية
  محصورة فقط حسب نوع حد المعدل. لا تتضمن هذه الحاويات الاحتياطية
  المسارات أو slugs أو أسماء الحزم أو الإصدارات أو سلاسل الاستعلام أو غيرها
  من معاملات الأثر التي يوفرها المستدعي.
...
قراءة جسم الاستجابة كسلسلة نصية مفهومة للبشر. تُتجاهل معاملات الاستعلام غير المعروفة
...
- تُعاد النتائج بترتيب الصلة (تشابه التضمين + تعزيزات مطابقة رمز slug/الاسم الدقيقة + عامل شعبية صغير سابق).
- الصلة أقوى من الشعبية. يمكن لمطابقة دقيقة لرمز slug أو اسم العرض أن تتقدم على مطابقة أوسع ذات تفاعل أقوى بكثير.
- يُجزأ نص ASCII عند حدود الكلمات وعلامات الترقيم. على سبيل المثال، يحتوي `personal-map` على الرمز المستقل `map`، بينما يحتوي `amap-jsapi-skill` على `amap` و`jsapi` و`skill`؛ لذلك فإن البحث عن `map` يمنح `personal-map` مطابقة لفظية أقوى من `amap-jsapi-skill`.
- تُقاس الشعبية لوغاريتميا وتُحد بسقف. قد تحتل Skills ذات التفاعل المرتفع مرتبة أدنى عندما يكون نص الاستعلام أضعف مطابقة.
...
- لا تُعد تسمية slug فقط لملاحقة استعلام واحد إلا إذا كان slug الجديد اسما رسميا أفضل على المدى الطويل. تصبح slugs القديمة أسماء مستعارة لإعادة التوجيه، لكن عنوان URL الرسمي وslug المعروض وملخصات البحث المستقبلية تستخدم slug الجديد.
- تحافظ أسماء إعادة التسمية المستعارة على الحل لعناوين URL القديمة وعمليات التثبيت التي تُحل عبر السجل، لكن ترتيب البحث يعتمد على بيانات Skill الوصفية الرسمية بعد فهرسة إعادة التسمية. تبقى الإحصاءات الحالية مع Skill.
...
- `sort` (اختياري): `updated` (افتراضي)، `recommended` (اسم مستعار: `default`)، `createdAt` (اسم مستعار: `newest`)، `downloads`، `stars` (اسم مستعار: `rating`)، أسماء التثبيت القديمة المستعارة `installsCurrent`/`installs`/`installsAllTime` تُربط بـ `downloads`، `trending`
- `nonSuspiciousOnly` (اختياري): `true` لإخفاء Skills المشبوهة (`flagged.suspicious`)
- `nonSuspicious` (اختياري): اسم مستعار قديم لـ `nonSuspiciousOnly`
...
- `createdAt` مستقر لعمليات زحف Skills الجديدة؛ ويتغير `updated` عند إعادة نشر Skills الحالية.
- عند `nonSuspiciousOnly=true`، قد تعيد الترتيبات المعتمدة على المؤشر عناصر أقل من `limit` في الصفحة لأن Skills المشبوهة تُصفى بعد جلب الصفحة.
- استخدم `nextCursor` لمتابعة ترقيم الصفحات عند وجوده. لا تعني الصفحة القصيرة وحدها نهاية النتائج.
...
- يحصل المستدعون العامون فقط على `200` للـ Skills المرئية المعلّمة مسبقا.
- تُنقح الأدلة للمستدعين العامين ولا تتضمن المقاطع الخام إلا للمالكين/المشرفين.

### `POST /api/v1/skills/{slug}/report`

أبلغ عن Skill لمراجعة المشرف. التقارير على مستوى Skill، ومرتبطة اختياريا
...
- `tag` (اختياري): يحل إصدارا موسوما (على سبيل المثال `latest`).
...
تستخدم عمليات فحص المنشورات JSON:
...
- تتطلب عمليات فحص المنشورات وصول إدارة المالك/الناشر، أو صلاحية مشرف/مدير المنصة.
- تكتب عمليات فحص المنشورات التغييرات مرة أخرى فقط عندما تكون `update: true` ويكتمل الفحص بنجاح.
...
- مهام الفحص غير متزامنة. تُعطى طلبات الفحص اليدوية أولوية قبل أعمال النشر/الملء الخلفي العادية، لكن الاكتمال لا يزال يعتمد على توفر العامل.

### `GET /api/v1/skills/-/scan/{scanId}`

نقطة نهاية استطلاع مصادق عليها لفحص مُرسَل.

- تُرجع حالة في الانتظار/قيد التشغيل/ناجحة/فاشلة.
- تُرجع `queue.queuedAhead` و`queue.position` أثناء الانتظار حتى يتمكن العملاء من إظهار عدد الفحوصات اليدوية ذات الأولوية التي تسبق الطلب. الطوابير الكبيرة جدًا تكون محدودة ويتم الإبلاغ عنها مع `queuedAheadIsEstimate: true`.
...
نقطة نهاية أرشيف التقرير المصادق عليها.
...
- تُرجع نتائج الفحص المخزنة للإصدار المُرسَل الدقيق، بما في ذلك الإصدارات المحظورة أو المخفية.
...
مسار إعادة فحص دفعي قانوني للمديرين فقط. يقبل شكل الحمولة نفسه مثل `POST /api/v1/skills/-/rescan-batch` القديم.

### `POST /api/v1/skills/-/scan/batch/status`

مسار حالة الدفعة القانوني للمديرين فقط. يقبل `{ "jobIds": ["..."] }` ويُرجع عدادات التجميع نفسها مثل `POST /api/v1/skills/-/rescan-batch/status` القديم.
...
- تكون `ok` بقيمة `true` فقط عندما يكون للإصدار المحدد بطاقة Skill مولدة، ولا يكون محظورًا كبرمجية خبيثة بواسطة الإشراف، ويكون تحقق ClawScan نظيفًا.
...
- يحتوي `security.signals` على أدلة الماسحات الداعمة مثل `staticScan` و`virusTotal` و`skillSpector`.
...
- تكون `provenance` بقيمة `server-resolved-github-import` فقط عندما يحل ClawHub ويخزن مستودع/مرجع/التزام/مسار GitHub أثناء النشر أو الاستيراد؛ وإلا تكون `unavailable`.

### `POST /api/v1/skills/-/security-verdicts`

يُرجع أحكام الأمان المدمجة الحالية لإصدارات المهارات الدقيقة. نقطة نهاية
المجموعة هذه مخصصة للعملاء الذين يعرفون بالفعل إصدارات Skills من ClawHub المثبتة
التي يحتاجون إلى عرضها، مثل واجهة تحكم OpenClaw.
...
- الاستجابة خاصة بالأمان فقط. لا تتضمن بيانات بطاقة Skill أو حالة البطاقة المولدة أو قوائم ملفات الأثر أو حمولات الماسحات التفصيلية.
...
- يُحتفظ بـ`security.signals.dependencyRegistry` لتوافق استجابة v1، لكن ماسح وجود سجل التبعيات متقاعد وهذا المفتاح دائمًا `null`.
- لا يؤثر غياب بطاقة Skill على `ok` أو `decision` أو `reasons` في نقطة النهاية هذه؛ يجب أن يقرأ العملاء `skill-card.md` المثبت محليًا عندما يحتاجون إلى محتوى البطاقة.
- استخدم `/verify` عندما تحتاج إلى غلاف تحقق بطاقة Skill لمهارة واحدة، و`/card` عندما تحتاج إلى Markdown البطاقة المولدة، و`/scan` عندما تحتاج إلى بيانات ماسح مفصلة.
...
- تبقى إدخالات Skill مدعومة بسجل Skills ولا يزال نشرها ممكنًا فقط من خلال `POST /api/v1/skills`.
...
- يعيد `channel=private` الحزم التي يستطيع المتصل المصادق عليه قراءتها فقط.
...
استعراض فهرس مخصص لـ Plugin فقط عبر حزم code-plugin وbundle-plugin.
...
- `mcp-tooling` و`data` و`automation` تُحل إلى `tools`.
- `observability` و`deployment` تُحل إلى `gateway`.
- `dev-tools` يُحل إلى `runtime`.

`trending` هي لوحة صدارة للتثبيتات/التنزيلات خلال سبعة أيام ولا تستخدم الإجماليات على مدى كل الوقت.
...
- `startDate` (مطلوب): الحد الأدنى بميلي ثانية Unix لـ `updatedAt` الخاص بـ Skill.
- `endDate` (مطلوب): الحد الأعلى بميلي ثانية Unix لـ `updatedAt` الخاص بـ Skill.
...
- تتضمن Skills الحالية المدعومة من GitHub ذات فحص `clean` أو `suspicious`
  ملف `_source_handoff.json` مع `sourceRef: "public-github"`، والمستودع، والتثبيتة، والمسار،
...
- `startDate` (مطلوب): الحد الأدنى بمللي ثانية Unix لقيمة `updatedAt` الخاصة بـ Plugin.
- `endDate` (مطلوب): الحد الأعلى بمللي ثانية Unix لقيمة `updatedAt` الخاصة بـ Plugin.
- `limit` (اختياري): عدد صحيح (1-250)، الافتراضي `250`.
- `cursor` (اختياري): مؤشر ترقيم الصفحات من الاستجابة السابقة.
- `family` (اختياري): `code-plugin` أو `bundle-plugin`. يعني الحذف عائلتَي
  Plugin كلتيهما.
...
- ترشيح الفئة هو مرشح API حقيقي مدعوم بصفوف ملخص فئة Plugin،
  وليس إعادة كتابة لاستعلام البحث.
- تُعاد النتائج بترتيب الصلة ولا تدعم حاليًا ترقيم الصفحات.
- تعيد عناصر التحكم في الفرز في واجهة المتصفح لبحث Plugin ترتيب نتائج الصلة المحمَّلة،
...
يحذف حزمة وكل إصداراتها حذفًا مبدئيًا.
...
- تكون `version.artifact.kind` مساوية لـ `legacy-zip` لأرشيفات حزم العالم القديم أو
...
- تُعد `version.sha256hash` بيانات تعريف توافق مهملة للعملاء القدامى. فهي
  تجزّئ بايتات ZIP الدقيقة المعادة من `/api/v1/packages/{name}/download`.
...
يعيد ملخص الأمان والثقة الدقيق لإصدار الحزمة لعملاء التثبيت. هذا هو سطح الاستهلاك العام في OpenClaw لتقرير ما إذا كان
يمكن تثبيت إصدار محلول.
...
- `trust.blockedFromDownload` هي إشارة حظر التثبيت. يجب على OpenClaw وغيره من
  عملاء التثبيت حظر التثبيت عندما تكون هذه القيمة `true` بدلًا من
...
- تعني `trust.stale` أن ملخص الثقة حُسب من مدخلات قديمة ويجب
  التعامل معه على أنه يتطلب تحديثًا قبل قرار سماح عالي الثقة.

ملاحظات:

- نقطة النهاية هذه دقيقة على مستوى الإصدار. يجب على العملاء استدعاؤها بعد حل
  إصدار الحزمة الذي ينوون تثبيته، وليس فقط بعد قراءة أحدث
  بيانات تعريف للحزمة.
- تعيد الحزم الخاصة `404` ما لم يكن بإمكان المستدعي قراءة الناشر المالك.
- نقطة النهاية هذه أضيق عمدًا من نقاط نهاية إشراف المالك/المشرف. فهي تعرض قرار التثبيت والشرح العام، لا
  هويات المبلّغين، أو نصوص البلاغات، أو الأدلة الخاصة، أو الجداول الزمنية للمراجعة الداخلية.
...
- تعيد إصدارات الحزم القديمة أثر `legacy-zip` و
  `downloadUrl` قديمًا لملف ZIP.
...
- تبث إصدارات ClawPack بايتات `.tgz` المرفوعة الدقيقة لـ npm-pack.
...
- يُطبَّع `packageName` كاسم npm؛ يمكن أن تكون الحزمة مفقودة للترحيلات المخطط لها.
...
- `open`: إصدارات مشبوهة أو ضارة أو معلقة أو معزولة أو ملغاة أو مُبلَّغ عنها.
...
الإبلاغ عن حزمة لمراجعة المشرف. البلاغات على مستوى الحزمة، ويمكن ربطها اختياريًا
بإصدار. تغذي قائمة انتظار الإشراف لكنها لا تخفي التنزيلات أو
تحظرها تلقائيًا بذاتها؛ يجب على المشرفين استخدام إشراف الإصدار
...
نقطة نهاية للمالك/المشرف لعرض حالة إشراف الحزمة.
...
- أرشيفات Plugin/الحزم هي ملفات zip لها جذر `package/` حتى تستمر عملاء OpenClaw
  القديمة في العمل.
- يبقى هذا المسار مقتصرا على ZIP فقط. لا يبث ملفات ClawPack `.tgz`.
...
- لا تعرض إلا الإصدارات التي رُفعت لها tarballs من نوع ClawPack npm-pack.
- يتم إغفال الإصدارات القديمة التي تقتصر على ZIP عمدا.
...
- يستخدم حاوية معدل التنزيل.
- تتضمن ترويسات التنزيل ClawHub SHA-256 إضافة إلى بيانات تكامل/shasum الوصفية الخاصة بـ npm.
...
ينزل إصدار ZIP مستضافا من Skills، أو يعيد تسليما إلى مصدر GitHub لمهارة
حالية مدعومة من GitHub ذات فحص `clean` أو `suspicious` ولا تملك
إصدارا مستضافا.
...
- تعيد الإصدارات المحذوفة حذفا لينا `410`.
- لا تقوم تسليمات Skills المدعومة من GitHub بتمرير البايتات عبر وكيل أو عكسها. تتضمن استجابة JSON
  ...
  و`archiveUrl`؛ حالة الفحص/الحالة الحالية بوابة ولا يتم تضمينها كبيانات وصفية لحمولة النجاح.
...
- يتم أيضا قبول جسم JSON مع `files` (استنادا إلى storageId).
- حقل حمولة اختياري: `ownerHandle`. عند وجوده، تحل API ذلك
  الناشر من جهة الخادم وتتطلب أن يملك الفاعل وصولا إلى الناشر.
- حقل حمولة اختياري: `migrateOwner`. عندما يكون `true` مع `ownerHandle`، يمكن
  ...
  الناشرين الحالي والهدف. دون هذا الاشتراك الصريح، تُرفض تغييرات
  المالك.
...
- حقول النموذج المسموح بها هي `payload`، أو كتل `files` متكررة، أو مرجع tarball
  ...
  `clawpackUploadTicket` المعاد مع عنوان URL ذلك الرفع.
...
- تُرفض أجسام JSON والبيانات الوصفية `payload.files` / `payload.artifact`
  المقدمة من المستدعي.
- تحد طلبات النشر المباشر متعددة الأجزاء بـ 18MB. يمكن لملفات tarball من ClawPack
...
- تتطلب code plugins ملف `package.json`، وبيانات وصفية لمستودع المصدر، وبيانات وصفية
...
حذف لين / استعادة مهارة (المالك، أو المشرف، أو المسؤول).
...
عند وجود `reason`، يتم تخزينه كملاحظة إشراف المهارة ونسخه إلى سجل التدقيق.
تحجز عمليات الحذف اللين التي يبدأها المالك slug لمدة 30 يوما، ثم يمكن لناشر
...
مستخدم/ناشر شخصي مشترك قديم، تنقله نقطة النهاية إلى ناشر مؤسسة أولا.
بالنسبة إلى مؤسسة منشأة حديثا، وفر `memberHandle`؛ لا تتم إضافة المسؤول الفاعل كعضو.
تكون `memberRole` افتراضيا `owner`.
...
إنشاء ناشر مؤسسة بالخدمة الذاتية بعد المصادقة. ينشئ ناشر مؤسسة جديدا ويضيف
...
- يعيد `409` عندما يكون المقبض مستخدما بالفعل بواسطة ناشر أو مستخدم أو ناشر شخصي.

### `POST /api/v1/users/reserve`

للمسؤولين فقط. يحجز slugs الجذرية وأسماء الحزم للمالك الشرعي دون نشر
إصدار. تصبح أسماء الحزم حزما نائبة خاصة بلا صفوف إصدارات، حتى يتمكن المالك نفسه
لاحقا من نشر إصدار code-plugin أو bundle-plugin الحقيقي بذلك الاسم.
...
موفر GitHub غير القابلين للتغيير؛ ولا تستخدم المقابض القابلة للتغيير إلا كحارس موجه للمشغل.
...
هويتي GitHub. يفشل الاسترداد بإغلاق آمن عندما يكون لدى الناشر الشخصي الحالي للمستخدم الوجهة
skills أو packages أو مصادر GitHub skill.
يرحّل الاسترداد أيضاً حقول `ownerUserId` القديمة الخاصة بمهارات الناشر المسترد،
والأسماء المستعارة لمسارات skill، والحزم، وتحذيرات فاحص الحزم، وصفوف ملخص البحث المشتقة، بحيث
تتفق مسارات المالك المباشر مع سلطة الناشر الجديدة. كما يُعاد تعيين حجز protected-handle
نشط للمعرّف المسترد إلى المستخدم البديل حتى لا تتمكن مزامنة الملف الشخصي لاحقاً من استعادة
سلطة المستخدم السابق المتنافسة. كل جدول أساسي محدود بـ
100 صف لكل معاملة تطبيق؛ يجب أن تستخدم عمليات الاسترداد الأكبر أولاً ترحيلاً قابلاً للاستئناف للمالك.
مصادر GitHub skill تكون ضمن نطاق الناشر ويُبلّغ عنها باعتبارها مفحوصة بدلاً من إعادة كتابتها.
...
- يحافظ `rename` على المسار السابق كاسم مستعار لإعادة التوجيه.
- يخفي `merge` إدراج المصدر ويعيد توجيه مسار المصدر إلى إدراج الهدف.
...
حظر مستخدم وحذف skills المملوكة حذفاً نهائياً (للمشرف/المسؤول فقط).
...
المحتوى (للمسؤول فقط). يكون الوضع افتراضياً تشغيلاً تجريبياً ما لم تكن `dryRun` هي `false`.
...
التي تهيئ أرشيف ClawPack tarball معرّف التخزين الناتج باعتباره
`clawpack` والتذكرة المعادة باعتبارها `clawpackUploadTicket`.
