...
- ينبغي ألا تعيد نقاط نهاية الكتابة المصادق عليها `Unauthorized` مجردة عندما
  ...
  والحسابات المحذوفة/المحظورة/المعطلة، كل على نص قابل للتنفيذ حتى يتمكن عملاء
  CLI من إخبار المستخدمين بما منعهم.
...
- `X-RateLimit-Remaining` / `RateLimit-Remaining`: الميزانية المتبقية الدقيقة عند وجودها.
  تحذف الطلبات المجزأة الناجحة هذا الرأس بدلا من إرجاع قيمة عالمية تقريبية.
...
- استخدم تراجعا عشوائي التذبذب لتجنب إعادة المحاولة المتزامنة.
...
- إذا لم يتوفر IP عميل موثوق، تستخدم الطلبات المجهولة حاويات احتياطية
  ...
  معاملات الأثرية التي يوردها المستدعي.
...
قراءة جسم الاستجابة كسلسلة قابلة للقراءة من قبل البشر. يتم تجاهل معاملات الاستعلام
غير المعروفة لأغراض التوافق، لكن معاملات الاستعلام المعروفة ذات القيم غير الصالحة تعيد
`400`.
...
- تُعاد النتائج بترتيب الصلة (تشابه التضمين + تعزيزات مطابقة رمز slug/الاسم الدقيقة + أفضلية شعبية صغيرة).
- الصلة أقوى من الشعبية. يمكن لمطابقة دقيقة لرمز slug أو اسم العرض أن تتفوق في الترتيب على مطابقة أوسع ذات تفاعل أعلى بكثير.
- يُجزأ نص ASCII عند حدود الكلمات وعلامات الترقيم. على سبيل المثال، يحتوي `personal-map` على رمز `map` مستقل، بينما يحتوي `amap-jsapi-skill` على `amap` و`jsapi` و`skill`؛ لذلك فإن البحث عن `map` يمنح `personal-map` مطابقة معجمية أقوى من `amap-jsapi-skill`.
- تُقاس الشعبية لوغاريتميا وتُحدد بسقف. يمكن أن تكون Skills عالية التفاعل أدنى ترتيبا عندما يكون نص الاستعلام أضعف مطابقة.
...
- ضع المصطلحات التي سيبحث عنها المستخدمون حرفيا في اسم العرض والملخص والوسوم. استخدم رمز slug مستقلا فقط عندما يكون أيضا هوية ثابتة تريد الاحتفاظ بها.
- لا تعيد تسمية slug لملاحقة استعلام واحد فقط إلا إذا كان slug الجديد اسما أساسيا أفضل على المدى الطويل. تصبح slugs القديمة أسماء مستعارة لإعادة التوجيه، لكن عنوان URL الأساسي وslug المعروض وملخصات البحث المستقبلية تستخدم slug الجديد.
- تحفظ الأسماء المستعارة لإعادة التسمية حل العناوين القديمة والتثبيتات التي تُحل عبر السجل، لكن ترتيب البحث يعتمد على بيانات Skill الوصفية الأساسية بعد فهرسة إعادة التسمية. تبقى الإحصاءات الحالية مع Skill.
- إذا أصبحت Skill غير مرئية بشكل غير متوقع، فتحقق أولا من حالة الإشراف باستخدام `clawhub inspect @owner/slug` أثناء تسجيل الدخول قبل تغيير البيانات الوصفية المتعلقة بالترتيب.
...
- يرتب `trending` حسب التثبيتات في آخر 7 أيام (مستند إلى القياس عن بعد).
- `createdAt` ثابت لزحوف Skills الجديدة؛ ويتغير `updated` عندما يُعاد نشر Skills موجودة.
- عندما يكون `nonSuspiciousOnly=true`، قد تعيد عمليات الفرز المستندة إلى المؤشر عناصر أقل من `limit` في الصفحة لأن Skills المشبوهة تُرشح بعد استرجاع الصفحة.
...
- تُحل slugs القديمة التي تنشئها تدفقات إعادة تسمية/دمج المالك إلى Skill الأساسية.
...
- تُضمّن `moderation` فقط عندما تكون Skill معلمة أو عندما يعرضها المالك.
...
- يحصل المستدعون العامون على `200` فقط لـ Skills المرئية المعلمة مسبقا.
- تُنقح الأدلة للمستدعين العامين ولا تتضمن مقتطفات خاما إلا للمالكين/المشرفين.

### `POST /api/v1/skills/{slug}/report`

الإبلاغ عن Skill لمراجعة المشرف. التقارير على مستوى Skill، وترتبط اختياريا
...
لإخفاء Skill ضمن سير العمل القابل للتدقيق نفسه.
...
تستخدم فحوصات الإصدارات المنشورة JSON:
...
- تتطلب فحوصات الإصدارات المنشورة صلاحية إدارة المالك/الناشر، أو سلطة مشرف/مدير المنصة.
- لا تكتب فحوصات الإصدارات المنشورة النتائج عكسيًا إلا عندما تكون `update: true` ويكتمل الفحص بنجاح.
...
- مهام الفحص غير متزامنة. تُمنح طلبات الفحص اليدوي أولوية قبل أعمال النشر/الملء الخلفي العادية، لكن الاكتمال لا يزال يعتمد على توفر العاملين.
...
- تُرجع `queue.queuedAhead` و`queue.position` أثناء وجود الطلب في قائمة الانتظار حتى يتمكن العملاء من عرض عدد الفحوصات اليدوية ذات الأولوية التي تسبق الطلب. تُقيَّد قوائم الانتظار الكبيرة جدًا ويُبلَّغ عنها باستخدام `queuedAheadIsEstimate: true`.
- عند توفر `report`، يحتوي على أقسام `clawscan` و`skillspector` و`staticAnalysis` و`virustotal`.
...
- تتطلب صلاحية إدارة المالك/الناشر للمهارة أو Plugin، أو سلطة مشرف/مدير المنصة.
...
مسار إعادة فحص دُفعي معياري للمديرين فقط. يقبل شكل الحمولة نفسه مثل المسار القديم `POST /api/v1/skills/-/rescan-batch`.

### `POST /api/v1/skills/-/scan/batch/status`

مسار حالة دُفعي معياري للمديرين فقط. يقبل `{ "jobIds": ["..."] }` ويُرجع عدادات التجميع نفسها مثل المسار القديم `POST /api/v1/skills/-/rescan-batch/status`.
...
- تكون هوية المهارة، وهوية الناشر، وبيانات الإصدار المحدد حقولًا علوية في الغلاف (`slug` و`displayName` و`publisherHandle` و`version` و`resolvedFrom` و`tag` و`createdAt`) حتى تتمكن أتمتة الصدفة من قراءتها من دون فك أغلفة متداخلة.
- `security` هو حكم ClawScan/الأمان في المستوى العلوي. ينبغي للأتمتة الاعتماد على `ok` و`decision` و`reasons` و`security.status`.
- يحتوي `security.signals` على أدلة داعمة من الماسحات مثل `staticScan` و`virusTotal` و`skillSpector`.
- يُحتفظ بـ `security.signals.dependencyRegistry` للتوافق مع استجابة v1، لكن ماسح وجود سجل الاعتماديات متقاعد وهذا المفتاح يكون دائمًا `null`.
- تكون `provenance` بقيمة `server-resolved-github-import` فقط عندما يحل ClawHub مستودع GitHub/المرجع/الالتزام/المسار ويخزنه أثناء النشر أو الاستيراد؛ وإلا فهي `unavailable`.

### `POST /api/v1/skills/-/security-verdicts`

يُرجع أحكام الأمان المضغوطة الحالية لإصدارات مهارات محددة. نقطة نهاية
...
- النتائج لكل عنصر؛ عدم العثور على مهارة أو إصدار واحد لا يُفشل الاستجابة كلها.
- الاستجابة للأمان فقط. لا تتضمن بيانات بطاقة المهارة، أو حالة البطاقة المولدة، أو قوائم ملفات الأثر، أو حمولات ماسح مفصلة.
...
- يُحتفظ بـ `security.signals.dependencyRegistry` للتوافق مع استجابة v1، لكن ماسح وجود سجل الاعتماديات متقاعد وهذا المفتاح يكون دائمًا `null`.
- لا يؤثر غياب بطاقة المهارة في `ok` أو `decision` أو `reasons` لنقطة النهاية هذه؛ ينبغي للعملاء قراءة `skill-card.md` المثبتة محليًا عندما يحتاجون إلى محتوى البطاقة.
- استخدم `/verify` عندما تحتاج إلى غلاف تحقق بطاقة المهارة لمهارة واحدة، و`/card` عندما تحتاج إلى Markdown البطاقة المولدة، و`/scan` عندما تحتاج إلى بيانات ماسح مفصلة.
...
- `category` (اختياري): عامل تصفية فئة Plugin. مدعوم فقط عندما يكون
  الطلب محدود النطاق إلى حزم Plugin (`/api/v1/plugins`،
  ...
  عوامل تصفية v1 القديمة المستعارة موثقة ضمن `GET /api/v1/plugins`.
...
- `isOfficial` (اختياري): `true` أو `false`
- `category` (اختياري): عامل تصفية فئة Plugin. مدعوم فقط عندما يكون
  الطلب محدود النطاق إلى حزم Plugin. الفئات المضبوطة وأسماء عوامل تصفية v1
...
تظل أسماء عوامل تصفية v1 القديمة المستعارة مقبولة على نقاط نهاية القراءة:
...
`trending` هي لوحة صدارة تثبيت/تنزيل لمدة سبعة أيام، ولا تستخدم الإجماليات عبر كل الوقت.
...
تصدير دفعي لأحدث إصدارات Plugin العامة للتحليل دون اتصال.
...
- تُخزّن بيانات التصدير الوصفية لكل Plugin في
  `__clawhub_export/{family}/{packageName}/plugin_meta.json`.
- يُضمّن `_manifest.json` دائمًا في جذر ZIP.
- يُضمّن `_errors.json` عندما يتعذر تصدير Plugins أو ملفات فردية.
...
- تُقبل أيضًا الأسماء البديلة لعوامل تصفية v1 القديمة الموثقة ضمن `GET /api/v1/plugins`.
- تصفية الفئة هي عامل تصفية API حقيقي مدعوم بصفوف ملخص فئات Plugin،
...
يعيد إصدار حزمة واحدًا، بما في ذلك بيانات الملفات الوصفية، والتوافق،
والتحقق، وبيانات الأثر الوصفية، وبيانات الفحص.
...
- `version.sha256hash` هي بيانات وصفية توافقية مهملة للعملاء القدامى. وهي
  ...
  الإصدار القانوني.
- تُضمّن `version.vtAnalysis` و`version.llmAnalysis` و`version.staticScan`
  عندما توجد بيانات فحص.
- تعيد الحزم الخاصة `404` ما لم يكن بإمكان المستدعي قراءة الناشر المالك.

### `GET /api/v1/packages/{name}/versions/{version}/security`

يعيد ملخص الأمان والثقة الدقيق لإصدار الحزمة لعملاء التثبيت. هذه هي واجهة
الاستهلاك العامة في OpenClaw لتحديد ما إذا كان يمكن تثبيت إصدار محلول.
...
- `trust.moderationState` قابلة للقيمة الفارغة. تكون `null` عندما لا يوجد
...
- نقطة النهاية هذه دقيقة على مستوى الإصدار. ينبغي للعملاء استدعاؤها بعد حل
  إصدار الحزمة الذي ينوون تثبيته، وليس فقط بعد قراءة أحدث بيانات وصفية للحزمة.
- تعيد الحزم الخاصة `404` ما لم يكن بإمكان المستدعي قراءة الناشر المالك.
...
يعيد بيانات محلل الأثر الصريحة لإصدار حزمة.
...
- تبث إصدارات ClawPack بايتات `.tgz` الخاصة بـ npm-pack التي رُفعت بدقة.
...
- يُطبّع `packageName` كاسم npm؛ يمكن أن تكون الحزمة مفقودة في الترحيلات
  المخطط لها.
...
تحظرها تلقائيًا بذاتها؛ ينبغي للمشرفين استخدام إشراف الإصدار للموافقة على
الأثار أو حجرها أو إلغائها.
...
- حد حجم الملف: 200KB.
- لا تمنع فحوصات VirusTotal المعلقة القراءة؛ قد تظل الإصدارات الضارة محجوبة في مكان آخر.
- تعيد الحزم الخاصة `404` ما لم يكن بإمكان المستدعي قراءة الناشر المالك.
...
- أرشيفات Plugin/الحزم هي ملفات zip بجذر `package/` حتى يواصل عملاء OpenClaw
  القدامى العمل.
- يبقى هذا المسار مخصصًا لـ ZIP فقط. ولا يبث ملفات ClawPack بصيغة `.tgz`.
...
- تُدرج فقط الإصدارات التي تحتوي على حزم tarball من نوع ClawPack npm-pack مرفوعة.
...
ينزّل إصدار ZIP مستضافًا من skill، أو يعيد تسليم مصدر GitHub ل skill
حالية مدعومة بـ GitHub مع فحص `clean` أو `suspicious` وبدون
إصدار مستضاف.
...
- تعيد الإصدارات المحذوفة حذفًا لينًا `410`.
- لا تقوم عمليات تسليم skill المدعومة بـ GitHub بتمرير البايتات عبر وكيل أو عكسها. تتضمن استجابة JSON
  `sourceRef: "public-github"`، و`repo`، و`commit`، و`path`، و`contentHash`،
  و`archiveUrl`؛ حالة الفحص/الحالة الحالية هي بوابة ولا تُدرج كبيانات تعريف
  لحمولة نجاح.
...
- يُقبل أيضًا جسم JSON يحتوي على `files` (بناءً على storageId).
- حقل payload اختياري: `ownerHandle`. عند وجوده، تحل API ذلك
  الناشر من جهة الخادم وتتطلب أن يكون للفاعل وصول إلى الناشر.
- حقل payload اختياري: `migrateOwner`. عند ضبطه على `true` مع `ownerHandle`، يمكن أن تنتقل
  skill موجودة إلى ذلك المالك إذا كان الفاعل مديرًا/مالكًا لدى كل من
...
- حقول النموذج المسموح بها هي `payload`، أو كتل `files` مكررة، أو مرجع tarball واحد باسم `clawpack`.
  ...
  تدفق upload-url. يجب أن تتضمن عمليات النشر المرحلية بمعرّف تخزين أيضًا
  `clawpackUploadTicket` المعاد مع عنوان URL للرفع.
- استخدم إما `files` أو `clawpack`، وليس كليهما مطلقًا في الطلب نفسه.
- تُرفض أجسام JSON وبيانات تعريف `payload.files` / `payload.artifact`
  المقدمة من المستدعي.
- تُحدد طلبات النشر المباشرة متعددة الأجزاء بحد 18MB. يمكن لحزم ClawPack tarballs
...
- تتطلب code plugins وجود `package.json`، وبيانات تعريف مستودع المصدر، وبيانات تعريف commit المصدر،
  وبيانات تعريف مخطط الإعدادات، و`openclaw.compat.pluginApi`، و
  `openclaw.build.openclawVersion`.
- تُعد `openclaw.hostTargets` و`openclaw.environment` بيانات تعريف اختيارية.
- لا يجوز النشر إلى قناة `official` إلا لناشر مؤسسة `openclaw` وأعضاء مؤسسة `openclaw`
  الحاليين لدى ناشريهم الشخصيين.
- لا تزال عمليات النشر نيابة عن الغير تتحقق من أهلية قناة official مقابل حساب المالك الهدف.
...
تحجز عمليات الحذف اللين التي يبدأها المالك slug لمدة 30 يومًا، ثم يمكن
لناشر آخر المطالبة بـ slug. تتضمن استجابة الحذف `slugReservedUntil` عند انطباق هذا الانتهاء.
...
إنشاء ناشر مؤسسة ذاتي الخدمة لمستخدم مصادق عليه. ينشئ ناشر مؤسسة جديدًا ويضيف
المستدعي كمالك. لا تنقل نقطة النهاية هذه مقابض المستخدم/الشخصية الموجودة ولا
تعلّم الناشر كموثوق/official.

- الجسم: `{ "handle": "opik", "displayName": "Opik" }`
- الاستجابة: `{ "ok": true, "publisherId": "...", "handle": "opik", "created": true, "trusted": false }`
- يعيد `409` عندما يكون المقبض مستخدمًا بالفعل بواسطة ناشر أو مستخدم أو ناشر شخصي.
...
المالك نفسه لاحقًا من نشر إصدار code-plugin أو bundle-plugin الحقيقي بذلك الاسم.
...
للمديرين فقط. يستعيد ناشرًا شخصيًا لهوية GitHub OAuth بديلة متحقق منها
دون تعديل صفوف حساب Convex Auth. يجب أن يذكر الطلب كلا معرفي حساب مزود GitHub
الثابتين؛ ولا تُستخدم المقابض القابلة للتغيير إلا كحارس موجّه للمشغّل.
...
هويتي GitHub الأساسيتين. يفشل الاسترداد بإغلاق آمن عندما يكون للناشر الشخصي الحالي للمستخدم الوجهة
مهارات أو حزم أو مصادر مهارات GitHub.
ينقل الاسترداد أيضاً حقول `ownerUserId` القديمة لمهارات الناشر المسترد،
والأسماء البديلة لـ slug المهارة، والحزم، وتحذيرات فاحص الحزم، وصفوف ملخص البحث المشتقة بحيث
تتوافق مسارات المالك المباشر مع سلطة الناشر الجديدة. كما يعاد تعيين حجز نشط لمقبض محمي
للمقبض المسترد إلى المستخدم البديل بحيث لا يمكن لمزامنة الملف الشخصي لاحقاً استعادة سلطة المستخدم السابق المتنافسة. يقتصر كل جدول أساسي على
100 صف لكل معاملة تطبيق؛ ويجب أن تستخدم عمليات الاسترداد الأكبر أولاً ترحيل مالك قابل للاستئناف.
مصادر مهارات GitHub مرتبطة بالناشر ويبلغ عنها على أنها فُحصت بدلاً من إعادة كتابتها.
...
احظر مستخدماً واحذف المهارات المملوكة نهائياً (للمشرف/المسؤول فقط).
...
ألغِ حظر مستخدم واستعد المهارات المؤهلة (للمسؤول فقط).
...
غيّر السبب المخزن لحظر موجود دون إلغاء الحظر أو استعادة
المحتوى (للمسؤول فقط). يكون افتراضياً في وضع التشغيل التجريبي ما لم يكن `dryRun` هو `false`.
...
التي تجهز أرشيف ClawPack tarball معرّف التخزين الناتج كـ
