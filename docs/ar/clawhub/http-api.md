...
x-i18n:
    generated_at: "2026-06-30T14:02:58Z"
    model: gpt-5.5
...
- `X-RateLimit-Remaining` / `RateLimit-Remaining`: الميزانية المتبقية الدقيقة عند وجودها.
  الطلبات الناجحة المجزأة تتجاهل هذا الرأس بدلًا من إعادة قيمة عالمية تقريبية.
...
- يستخدم رؤوس IP العميل الموثوقة، بما في ذلك `cf-connecting-ip`، فقط عندما يفعّل
  النشر صراحةً الرؤوس الممررة الموثوقة.
...
- إذا لم يتوفر IP عميل موثوق، تستخدم الطلبات المجهولة حاويات احتياطية
  ...
  من معاملات الأثر التي يزودها المستدعي.

## استجابات الخطأ

استجابات الخطأ العامة في v1 هي نص عادي مع `content-type: text/plain; charset=utf-8`.
...
- تُعاد النتائج بترتيب الصلة (تشابه embedding + تعزيزات مطابقة رمز slug/الاسم الدقيقة + عامل شعبية صغير مسبق).
- الصلة أقوى من الشعبية. يمكن لمطابقة رمز slug دقيق أو اسم عرض أن تتقدم على مطابقة أوسع ذات تفاعل أقوى بكثير.
...
- الشعبية مقاسة لوغاريتميًا ومحدودة بسقف. يمكن أن تحتل Skills ذات التفاعل العالي مرتبة أدنى عندما يكون نص الاستعلام أضعف مطابقة.
- قد تزيل حالة إشراف مشبوهة أو مخفية إحدى Skills من البحث العام بحسب مرشحات المستدعي وحالة الإشراف الحالية.
...
- لا تعِد تسمية slug لمجرد ملاحقة استعلام واحد إلا إذا كان slug الجديد اسمًا قانونيًا أفضل على المدى الطويل. تصبح slugs القديمة أسماء مستعارة لإعادة التوجيه، لكن عنوان URL القانوني وslug المعروض وملخصات البحث المستقبلية تستخدم slug الجديد.
- تحافظ الأسماء المستعارة لإعادة التسمية على الحل لعناوين URL القديمة وعمليات التثبيت التي تُحل عبر السجل، لكن ترتيب البحث يستند إلى بيانات تعريف Skill القانونية بعد فهرسة إعادة التسمية. تبقى الإحصاءات الحالية مع Skill.
...
- `sort` (اختياري): `updated` (افتراضي)، `recommended` (اسم مستعار: `default`)، `createdAt` (اسم مستعار: `newest`)، `downloads`، `stars` (اسم مستعار: `rating`)، أسماء تثبيت قديمة مستعارة `installsCurrent`/`installs`/`installsAllTime` تُطابق إلى `downloads`، `trending`
- `nonSuspiciousOnly` (اختياري): `true` لإخفاء Skills المشبوهة (`flagged.suspicious`)
- `nonSuspicious` (اختياري): اسم مستعار قديم لـ `nonSuspiciousOnly`
...
- `createdAt` مستقر لزحوف Skills الجديدة؛ يتغير `updated` عندما يُعاد نشر Skills الحالية.
- عندما يكون `nonSuspiciousOnly=true`، قد تعيد الفرزات المعتمدة على المؤشر عناصر أقل من `limit` في الصفحة لأن Skills المشبوهة تُصفى بعد جلب الصفحة.
...
- تُحل slugs القديمة المنشأة بواسطة تدفقات إعادة تسمية/دمج المالك إلى Skill القانونية.
...
الإبلاغ عن Skill لمراجعة المشرف. التقارير على مستوى Skill، ويمكن ربطها اختياريًا
...
إعادة تعيين `status` إلى `open`. مرر `finalAction: "hide"` مع تقرير مُفرز
...
- تتضمن `version.security` حالة تحقق الفحص المعيارية وتفاصيل الماسح
...
- إذا لم يُقدَّم أيٌّ من `version` أو `tag`، فيُستخدم أحدث إصدار.
- يتضمن حالة تحقق مُطبَّعة بالإضافة إلى تفاصيل خاصة بالماسح.
- تكون `security.hasScanResult` مساوية لـ `true` فقط عندما ينتج ماسح حكمًا حاسمًا (`clean` أو `suspicious` أو `malicious`).
...
نقطة نهاية إرسال موثَّقة لمهام ClawScan الجديدة.
...
- تتطلب فحوصات المنشورات وصول إدارة المالك/الناشر، أو صلاحية مشرف/مدير المنصة.
...
- مهام الفحص غير متزامنة. تُعطى طلبات الفحص اليدوية أولوية على أعمال النشر/الملء الخلفي العادية، لكن الإكمال لا يزال يعتمد على توفر العاملين.

### `GET /api/v1/skills/-/scan/{scanId}`

نقطة نهاية استطلاع موثَّقة لفحص مُرسل.

- تُرجع حالة في الانتظار/قيد التشغيل/ناجحة/فاشلة.
- تُرجع `queue.queuedAhead` و`queue.position` أثناء الانتظار حتى يتمكن العملاء من عرض عدد الفحوصات اليدوية ذات الأولوية التي تسبق الطلب. تُحدَّد الطوابير الكبيرة جدًا وتُبلَّغ مع `queuedAheadIsEstimate: true`.
...
نقطة نهاية موثَّقة لأرشيف تقرير مخزَّن للإصدارات المُرسلة.
...
مسار إعادة فحص دفعي قياسي للمشرفين فقط. يقبل بنية الحمولة نفسها مثل `POST /api/v1/skills/-/rescan-batch` القديم.

### `POST /api/v1/skills/-/scan/batch/status`

مسار حالة دفعي قياسي للمشرفين فقط. يقبل `{ "jobIds": ["..."] }` ويُرجع العدادات الإجمالية نفسها مثل `POST /api/v1/skills/-/rescan-batch/status` القديم.
...
- تكون `ok` مساوية لـ `true` فقط عندما يكون للإصدار المحدد بطاقة مهارة مُولَّدة، ولا يكون محظورًا كبرمجية خبيثة بواسطة الإشراف، ويكون تحقق ClawScan نظيفًا.
- هوية المهارة، وهوية الناشر، وبيانات وصف الإصدار المحدد هي حقول على المستوى الأعلى في الغلاف (`slug` و`displayName` و`publisherHandle` و`version` و`resolvedFrom` و`tag` و`createdAt`) حتى تتمكن أتمتة الصدفة من قراءتها دون فك أغلفة متداخلة.
...
- تكون `provenance` مساوية لـ `server-resolved-github-import` فقط عندما يحل ClawHub ويخزن مستودع/مرجع/التزام/مسار GitHub أثناء النشر أو الاستيراد؛ وإلا فهي `unavailable`.
...
ClawHub المثبتة التي يحتاجون إلى عرضها، مثل واجهة تحكم OpenClaw.
...
- الاستجابة خاصة بالأمان فقط. لا تتضمن بيانات بطاقة المهارة، أو حالة البطاقة المُولَّدة، أو قوائم ملفات القطع الأثرية، أو حمولات الماسح التفصيلية.
...
- استخدم `/verify` عندما تحتاج إلى غلاف تحقق بطاقة المهارة لمهارة واحدة، و`/card` عندما تحتاج إلى Markdown البطاقة المُولَّدة، و`/scan` عندما تحتاج إلى بيانات ماسح تفصيلية.
...
- يتم الضبط افتراضيًا على أحدث إصدار.
- حد حجم الملف: 200KB.
...
استعراض كتالوج خاص بـ Plugin فقط عبر حزم code-plugin وbundle-plugin.
...
تظل أسماء عوامل التصفية المستعارة القديمة في v1 مقبولة في نقاط نهاية القراءة:
...
`trending` هي لوحة صدارة للتثبيت/التنزيل لمدة سبعة أيام ولا تستخدم الإجماليات التاريخية.
...
- `startDate` (مطلوب): الحد الأدنى بالمللي ثانية وفق Unix لـ `updatedAt` الخاص بـ Skill.
- `endDate` (مطلوب): الحد الأعلى بالمللي ثانية وفق Unix لـ `updatedAt` الخاص بـ Skill.
...
بحث مخصص لـ Plugin عبر حزم code-plugin و bundle-plugin.
...
- يتم أيضًا قبول الأسماء البديلة القديمة لمرشحات v1 الموثقة ضمن `GET /api/v1/plugins`.
- ترشيح الفئات هو مرشح API حقيقي مدعوم بصفوف ملخص فئة Plugin،
  وليس إعادة كتابة لاستعلام البحث.
- تُرجع النتائج حسب ترتيب الصلة ولا تدعم ترقيم الصفحات حاليًا.
...
يعيد إصدارًا واحدًا من الحزمة، بما في ذلك بيانات الملفات الوصفية، والتوافق،
...
- `version.sha256hash` هي بيانات وصفية توافقية مهملة للعملاء القدامى. وهي
  ...
  يجب على العملاء الحديثين استخدام `version.artifact.sha256`، الذي يعرّف
...
- `trust.scanStatus` هي حالة الثقة الفعلية المشتقة من مدخلات الفاحص
...
- `trust.blockedFromDownload` هي إشارة حظر التثبيت. يجب على OpenClaw وعملاء
  ...
  اشتقاق قواعد الحظر من حقول الفاحص أو الإشراف.
- `trust.reasons` هي قائمة الشرح الموجهة للمستخدم والتدقيق. رموز الأسباب
  هي سلاسل مستقرة وموجزة مثل `manual:quarantined` و `scan:malicious`
  و `package:malicious`.
...
- نقطة النهاية هذه أضيق عمدًا من نقاط نهاية الإشراف الخاصة بالمالك/المشرف.
  فهي تعرض قرار التثبيت والشرح العام، وليس هويات المبلغين أو نصوص البلاغات
...
- هذا هو سطح المحلل في OpenClaw؛ ويتجنب تخمين تنسيق الأرشيف من رابط مشترك.
...
- يتم تطبيع `packageName` كاسم npm؛ يمكن أن تكون الحزمة مفقودة للترحيلات المخططة.
- يتتبع هذا جاهزية الترحيل فقط. ولا يعدّل OpenClaw أو ينشئ ClawPacks.
...
اختياريًا بإصدار. وهي تغذي قائمة انتظار الإشراف لكنها لا تخفي أو تحظر
التنزيلات تلقائيًا بذاتها؛ يجب على المشرفين استخدام إشراف الإصدار
...
- أرشيفات Plugin/الحزم هي ملفات zip بجذر `package/` بحيث يستمر عمل عملاء OpenClaw
  القدامى.
...
- لا تُدرج إلا الإصدارات التي رُفعت لها tarballs npm-pack من ClawPack.
...
- يستخدم حاوية معدل التنزيل.
- تتضمن ترويسات التنزيل SHA-256 الخاص بـ ClawHub بالإضافة إلى بيانات npm الوصفية integrity/shasum.
...
- `slug` (مطلوبة)
- `hash` (مطلوبة): sha256 سداسي بطول 64 حرفًا لبصمة الحزمة
...
ينزّل ZIP لإصدار skill مستضاف، أو يعيد تسليم مصدر GitHub لـ skill حالية مدعومة من GitHub
ذات فحص `clean` أو `suspicious` ومن دون إصدار مستضاف.
...
- إذا لم يُقدَّم أي من `version` أو `tag`، يُستخدم أحدث إصدار.
- تعيد الإصدارات المحذوفة حذفًا مبدئيًا `410`.
- لا تمرر تسليمات skills المدعومة من GitHub البايتات عبر وكيل ولا تنسخها إلى مرآة. تتضمن استجابة JSON
...
- حقل حمولة اختياري: `ownerHandle`. عند وجوده، تحلّ API ذلك
  الناشر على الخادم وتتطلب أن يكون للفاعل وصول إلى الناشر.
- حقل حمولة اختياري: `migrateOwner`. عند `true` مع `ownerHandle`، قد تنتقل
...
- حقول النموذج المسموح بها هي `payload`، أو كتل `files` متكررة، أو مرجع tarball واحد باسم `clawpack`.
  يمكن أن يكون `clawpack` كتلة `.tgz` أو معرّف تخزين أرجعته
...
- تُحد طلبات النشر المباشر بـ multipart عند 18MB. يمكن أن تستخدم tarballs ClawPack
...
- لا يجوز النشر إلى القناة `official` إلا لناشر مؤسسة `openclaw` وأعضاء مؤسسة `openclaw` الحاليين
...
عند وجود `reason`، يُخزَّن كملاحظة إشراف skill ويُنسخ إلى سجل التدقيق.
تحجز عمليات الحذف المبدئي التي يبدأها المالك slug لمدة 30 يومًا، ثم يمكن
...
إنشاء ناشر مؤسسة ذاتي الخدمة للمستخدم المصادق عليه. ينشئ ناشر مؤسسة جديدًا ويضيف
...
إصدار. تصبح أسماء الحزم حزمًا نائبة خاصة من دون صفوف إصدارات، بحيث يمكن للمالك نفسه
نشر إصدار code-plugin أو bundle-plugin الحقيقي لاحقًا إلى ذلك الاسم.
...
للمدير فقط. يستعيد ناشرًا شخصيًا لجهة GitHub OAuth بديلة ومتحقق منها
دون تعديل صفوف حساب Convex Auth. يجب أن يذكر الطلب معرّفي حساب GitHub
لدى الموفر غير القابلين للتغيير؛ ولا تُستخدم المعرّفات القابلة للتغيير إلا كحاجز موجه للمشغل.
...
تتفق مسارات المالك المباشر مع سلطة الناشر الجديدة. كما يعاد تعيين حجز المعرّف المحمي النشط
للمعرّف المسترد إلى المستخدم البديل كي لا تتمكن مزامنة الملف الشخصي لاحقا من استعادة السلطة المنافسة للمستخدم السابق. يقتصر كل جدول أساسي على
100 صف لكل معاملة تطبيق؛ يجب أن تستخدم عمليات الاسترداد الأكبر أولا ترحيل مالك قابل للاستئناف.
مصادر Skills من GitHub تكون ضمن نطاق الناشر ويتم الإبلاغ عنها كمفحوصة بدلا من إعادة كتابتها.
...
المحتوى (للمدير فقط). يكون افتراضيا في وضع التشغيل التجريبي ما لم تكن `dryRun` هي `false`.
