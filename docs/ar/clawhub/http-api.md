...
- الطلبات مجهولة الهوية: تُفرض لكل عنوان IP.
...
- يجب ألا تُرجع نقاط نهاية الكتابة المصادق عليها `Unauthorized` مجردة عندما
  ...
  والحسابات المحذوفة/المحظورة/المعطلة على نص قابل للتنفيذ لكل حالة، حتى يتمكن
...
- استخدم تراجعًا عشوائي التفاوت لتجنب إعادة المحاولات المتزامنة.
...
- إذا لم يتوفر عنوان IP موثوق للعميل، تستخدم طلبات التنزيل مجهولة الهوية حاوية احتياطية محددة بنقطة النهاية بدلًا من حاوية `ip:unknown` عامة واحدة. تستمر طلبات القراءة/الكتابة مجهولة الهوية في استخدام حاوية المجهول المشتركة حتى يبقى توجيه IP المفقود ظاهرًا ومحافظًا.
...
- تُرجع النتائج بترتيب الصلة (تشابه التضمين + تعزيزات مطابقة رمز slug/الاسم الدقيقة + أولوية الشعبية من التنزيلات).
- الصلة أقوى من الشعبية. يمكن لمطابقة دقيقة لرمز slug أو اسم العرض أن تتفوق على مطابقة أوسع لديها تنزيلات أكثر بكثير.
- يُجزأ نص ASCII عند حدود الكلمات وعلامات الترقيم. على سبيل المثال، يحتوي `personal-map` على رمز `map` مستقل، بينما يحتوي `amap-jsapi-skill` على `amap` و`jsapi` و`skill`؛ لذلك يعطي البحث عن `map` مطابقة معجمية أقوى لـ `personal-map` مقارنة بـ `amap-jsapi-skill`.
- تُستخدم التنزيلات كأولوية صغيرة بمقياس لوغاريتمي وكعامل حسم، وليس كإشارة الترتيب الأساسية. قد تأتي Skills كثيرة التنزيلات في ترتيب أدنى عندما يكون نص الاستعلام أضعف مطابقة.
- يمكن لحالة الإشراف المشبوهة أو المخفية إزالة Skill من البحث العام بحسب فلاتر المستدعي وحالة الإشراف الحالية.
...
- لا تعِد تسمية slug لمجرد ملاحقة استعلام واحد، إلا إذا كان slug الجديد اسمًا أساسيًا أفضل على المدى الطويل. تصبح slugs القديمة أسماء مستعارة لإعادة التوجيه، لكن عنوان URL الأساسي وslug المعروض وملخصات البحث المستقبلية تستخدم slug الجديد.
- تحافظ أسماء إعادة التسمية المستعارة على الحل لعناوين URL القديمة وعمليات التثبيت التي تُحل عبر السجل، لكن ترتيب البحث يستند إلى بيانات Skill الوصفية الأساسية بعد فهرسة إعادة التسمية. تبقى الإحصاءات الموجودة مع Skill.
...
- يكون `createdAt` ثابتًا لزحف Skills الجديدة؛ ويتغير `updated` عند إعادة نشر Skills الموجودة.
- عند `nonSuspiciousOnly=true`، قد تُرجع عمليات الفرز المعتمدة على المؤشر عناصر أقل من `limit` في الصفحة لأن Skills المشبوهة تُصفى بعد جلب الصفحة.
...
- `metadata.os`: قيود نظام التشغيل المعلنة في frontmatter الخاصة بـ Skill (مثل `["macos"]`، `["linux"]`). `null` إذا لم تُعلن.
- `metadata.systems`: أهداف أنظمة Nix (مثل `["aarch64-darwin", "x86_64-linux"]`). `null` إذا لم تُعلن.
- تكون `metadata` هي `null` إذا لم تكن لدى Skill بيانات وصفية للمنصة.
- تُضمَّن `moderation` فقط عندما تكون Skill موسومة أو عندما يعرضها المالك.
...
إعادة تعيين `status` إلى `open`. مرر `finalAction: "hide"` مع تقرير فُرز
...
- إذا لم يُقدَّم لا `version` ولا `tag`، يستخدم الإصدار الأحدث.
- يتضمن حالة التحقق الموحدة بالإضافة إلى تفاصيل خاصة بكل ماسح.
...
- تكون `security.hasScanResult` هي `true` فقط عندما ينتج ماسح حكمًا نهائيًا (`clean` أو `suspicious` أو `malicious`).
- `moderation` هي لقطة إشراف حالية على مستوى Skill مشتقة من الإصدار الأحدث.
...
- يتم قبول `target` / `hostTarget`، و`os`، و`arch`، و`libc`، و`requiresBrowser`،
  ...
  و`requiresBinary`، و`requiresOsPermission`، و`externalService`، و`binary`،
  و`osPermission` كاختصارات لوسوم الإمكانات الشائعة
- `artifactKind` (اختياري): `legacy-zip` أو `npm-pack`
...
- لا يعيد `channel=private` إلا الحزم التي يمكن للمستدعي المصادق عليه قراءتها.
- تستند مرشحات الأثر إلى وسوم إمكانات مفهرسة:
...
- يمكن حل Skills أيضا عبر هذا المسار في الفهرس الموحد.
- تعيد الحزم الخاصة `404` ما لم يكن المستدعي قادرا على قراءة الناشر المالك.

### `DELETE /api/v1/packages/{name}`

يحذف الحزمة وجميع إصداراتها حذفا مرنا.
...
والإمكانات، والتحقق، وبيانات تعريف الأثر، وبيانات الفحص.
...
يعيد بيانات تعريف محلل الأثر الصريح لإصدار الحزمة.

ملاحظات:

- تعيد إصدارات الحزم القديمة أثرا من نوع `legacy-zip` و`downloadUrl` قديم
  بصيغة ZIP.
- تعيد إصدارات ClawPack أثرا من نوع `npm-pack`، وحقول سلامة npm، و`tarballUrl`،
...
- تبث إصدارات ClawPack بايتات `.tgz` المطابقة تماما لحزمة npm-pack المرفوعة.
...
- ملخص الأثر
- مصدر مستودع المصدر وبيانات منشأ الالتزام
...
يبلغ عن حزمة لمراجعة المشرفين. تكون البلاغات على مستوى الحزمة، ويمكن ربطها اختياريا
بإصدار. تغذي هذه البلاغات طابور الإشراف لكنها لا تخفي التنزيلات أو
تحظرها تلقائيا بحد ذاتها؛ ينبغي للمشرفين استخدام إشراف الإصدارات
...
نقطة نهاية صيانة للمسؤولين فقط لوسم إصدارات الحزم الأقدم
...
- يكون الوضع الافتراضي تشغيلًا تجريبيًا.
- تُوسم الإصدارات التي لا تحتوي على تخزين ClawPack باسم `legacy-zip`.
- تُصلح الصفوف الحالية المدعومة من ClawPack والتي ينقصها `artifactKind` لتصبح
  `npm-pack`.
...
- حد حجم الملف: 200KB.
- لا تمنع فحوصات VirusTotal المعلّقة القراءة؛ وقد تظل الإصدارات الخبيثة محجوبة في مواضع أخرى.
...
- أرشيفات Plugin/الحزم هي ملفات zip لها جذر `package/` حتى تظل عملاء OpenClaw
  القديمة تعمل.
- يبقى هذا المسار مقتصرًا على ZIP. ولا يبث ملفات ClawPack ذات اللاحقة `.tgz`.
...
- تُحذف عمدًا الإصدارات القديمة التي تحتوي على ZIP فقط.
...
- تدعم packuments للحزم ذات النطاق كلاً من مسار الطلب `/api/npm/@scope/name` ومسار
...
- يستخدم حاوية معدل التنزيل.
- تتضمن ترويسات التنزيل SHA-256 الخاص بـ ClawHub إضافة إلى بيانات سلامة/مجموع shasum الوصفية الخاصة بـ npm.
...
- تُحتسب إحصاءات التنزيل كهويات فريدة لكل ساعة (`userId` عندما يكون رمز API صالحًا، وإلا فالعنوان IP).
...
- المفضّل: `multipart/form-data` مع JSON في `payload` + كتل `files[]`.
- يُقبل أيضًا جسم JSON مع `files` (مستند إلى storageId).
- حقل حمولة اختياري: `ownerHandle`. عند وجوده، يحل API ذلك
  الناشر على جانب الخادم ويتطلب أن يكون لدى الفاعل وصول إلى الناشر.
...
يحذف skill حذفًا مرنًا / يستعيدها (المالك أو المشرف أو المسؤول).
...
عند وجوده، يُخزّن `reason` كملاحظة إشراف skill ويُنسخ إلى سجل التدقيق.
عمليات الحذف المرن التي يبدؤها المالك تحجز slug لمدة 30 يومًا، ثم يمكن أن يطالب
ناشر آخر بـ slug. تتضمن استجابة الحذف `slugReservedUntil` عند انطباق هذه المهلة.
لا تنتهي صلاحية إخفاءات المشرف/المسؤول وعمليات الإزالة الأمنية بهذه الطريقة.
...
إصدار. تصبح أسماء الحزم حزمًا نائبة خاصة بلا صفوف إصدار، حتى يستطيع
المالك نفسه لاحقًا نشر إصدار code-plugin أو bundle-plugin الحقيقي بهذا الاسم.
...
- تتطلب نقطتا النهاية كلتاهما مصادقة رمز API، وتعملان فقط لمالك skill.
- يحافظ `rename` على slug السابق كاسم مستعار لإعادة التوجيه.
- يخفي `merge` قائمة المصدر ويعيد توجيه slug المصدر إلى القائمة المستهدفة.
...
يحظر مستخدمًا ويحذف نهائيًا Skills المملوكة (للمشرفين/المسؤولين فقط).
...
يلغي حظر مستخدم ويستعيد Skills المؤهلة (للمسؤولين فقط).
...
يغيّر دور مستخدم (للمسؤولين فقط).
...
يعرض المستخدمين أو يبحث عنهم (للمسؤولين فقط).

معلمات الاستعلام:

- `q` (اختياري): استعلام بحث
...
يضيف/يزيل نجمة (تمييزات). نقطتا النهاية كلتاهما غير متأثرتين بالتكرار.
...
إذا كنت تستضيف ذاتيًا، فقدّم هذا الملف (أو عيّن `CLAWHUB_REGISTRY` صراحةً؛ أو `CLAWDHUB_REGISTRY` القديم).
