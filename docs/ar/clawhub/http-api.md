...
x-i18n:
    generated_at: "2026-07-04T18:01:32Z"
    model: gpt-5.5
...
- الطلبات المصدق عليها (رمز Bearer صالح): تُفرض لكل حاوية مستخدم.
- إذا كان الرمز مفقودا/غير صالح، يعود السلوك إلى الفرض حسب IP.
- يجب ألا تُرجع نقاط نهاية الكتابة المصدق عليها `Unauthorized` مجردة عندما
  يعرف الخادم السبب. يجب أن يحصل كل من الرموز المفقودة، والرموز غير الصالحة/الملغاة،
...
- `X-RateLimit-Remaining` / `RateLimit-Remaining`: الميزانية المتبقية الدقيقة عند وجودها.
  الطلبات الناجحة المجزأة تتجاهل هذه الترويسة بدلا من إرجاع قيمة عامة تقريبية.
...
- استخدم تراجعا مع jitter لتجنب إعادة المحاولات المتزامنة.
...
- يستخدم ترويسات IP العميل الموثوقة، بما في ذلك `cf-connecting-ip`، فقط عندما
  يفعّل النشر صراحة ترويسات التمرير الموثوقة.
- يستخدم ClawHub ترويسات التمرير الموثوقة لتحديد عناوين IP للعملاء عند الحافة.
- إذا لم يتوفر IP عميل موثوق، تستخدم الطلبات المجهولة حاويات احتياطية
  ...
  مسارات أو slugs أو أسماء حزم أو إصدارات أو سلاسل استعلام أو أي
  معاملات artifacts أخرى يزودها المستدعي.
...
قراءة جسم الاستجابة كسلسلة نصية قابلة للقراءة من البشر. يتم تجاهل معاملات الاستعلام
غير المعروفة للتوافق، لكن معاملات الاستعلام المعروفة ذات القيم غير الصالحة تُرجع
`400`.
...
- تُرجع النتائج بترتيب الصلة (تشابه embedding + تعزيزات مطابقة رمز slug/الاسم الدقيقة + أسبقية شعبية صغيرة).
- الصلة أقوى من الشعبية. يمكن لمطابقة دقيقة لرمز slug أو اسم عرض أن تتقدم على مطابقة أوسع ذات تفاعل أقوى بكثير.
- يُجزأ نص ASCII عند حدود الكلمات وعلامات الترقيم. على سبيل المثال، يحتوي `personal-map` على رمز `map` مستقل، بينما يحتوي `amap-jsapi-skill` على `amap` و`jsapi` و`skill`؛ لذلك يعطي البحث عن `map` لـ `personal-map` مطابقة معجمية أقوى من `amap-jsapi-skill`.
- الشعبية مقيسة لوغاريتميا ومحددة بسقف. يمكن أن تأتي Skills عالية التفاعل بترتيب أدنى عندما يكون نص الاستعلام أضعف مطابقة.
- يمكن أن تزيل حالة الإشراف المشبوهة أو المخفية مهارة من البحث العام حسب مرشحات المستدعي وحالة الإشراف الحالية.
...
- لا تعد تسمية slug فقط لملاحقة استعلام واحد ما لم يكن slug الجديد اسما رسميا أفضل على المدى الطويل. تصبح slugs القديمة أسماء مستعارة لإعادة التوجيه، لكن عنوان URL الرسمي وslug المعروض وملخصات البحث المستقبلية تستخدم slug الجديد.
- تحافظ أسماء إعادة التسمية المستعارة على الحل لعناوين URL القديمة وعمليات التثبيت التي تُحل عبر السجل، لكن ترتيب البحث يعتمد على بيانات المهارة الرسمية بعد فهرسة إعادة التسمية. تبقى الإحصاءات الحالية مع المهارة.
- إذا كانت مهارة غير مرئية بشكل غير متوقع، فتحقق أولا من حالة الإشراف باستخدام `clawhub inspect @owner/slug` أثناء تسجيل الدخول قبل تغيير البيانات الوصفية المتعلقة بالترتيب.
...
- `cursor` (اختياري): مؤشر صفحات لأي ترتيب غير `trending`
...
- يرتب `trending` حسب عمليات التثبيت في آخر 7 أيام (استنادا إلى telemetry).
- `createdAt` مستقر لعمليات زحف المهارات الجديدة؛ يتغير `updated` عندما تُعاد نشر Skills الحالية.
- عند `nonSuspiciousOnly=true`، قد تُرجع الترتيبات المعتمدة على المؤشر عناصر أقل من `limit` في الصفحة لأن Skills المشبوهة تُصفى بعد استرجاع الصفحة.
- استخدم `nextCursor` لمتابعة الصفحات عند وجوده. الصفحة القصيرة لا تعني وحدها نهاية النتائج.
...
- slugs القديمة التي تنشئها تدفقات إعادة تسمية/دمج المالك تُحل إلى المهارة الرسمية.
- `metadata.os`: قيود OS المعلنة في frontmatter المهارة (مثل `["macos"]`، `["linux"]`). `null` إذا لم تُعلن.
...
- يتم تضمين `moderation` فقط عندما تكون المهارة معلما عليها أو عندما يعرضها المالك.
...
- يحصل المستدعون العامون فقط على `200` لـ Skills المرئية التي عُلم عليها مسبقا.
- تُنقح الأدلة للمستدعين العامين ولا تتضمن المقاطع الخام إلا للمالكين/المشرفين.

### `POST /api/v1/skills/{slug}/report`

أبلغ عن مهارة لمراجعة المشرفين. التقارير على مستوى المهارة، ويمكن ربطها اختياريا
...
تم فرزه لإخفاء المهارة ضمن سير العمل نفسه القابل للتدقيق.
...
يُرجع بيانات الإصدار الوصفية + قائمة الملفات.
...
يُرجع تفاصيل تحقق فحص الأمان لإصدار مهارة.
...
- مهام الفحص غير متزامنة. تُمنح طلبات الفحص اليدوية أولوية قبل أعمال النشر/إعادة الملء العادية، لكن الإكمال لا يزال يعتمد على توفر العاملين.
...
- تُرجع `queue.queuedAhead` و`queue.position` أثناء وجود الطلب في قائمة الانتظار حتى يتمكن العملاء من عرض عدد الفحوصات اليدوية ذات الأولوية الموجودة قبل الطلب. تُحدَّد قوائم الانتظار الكبيرة جدًا وتُبلّغ باستخدام `queuedAheadIsEstimate: true`.
...
نقطة نهاية أرشيف تقرير مخزّن ومصادَق عليها للإصدارات المُرسلة.
...
- تُرجع شكل ZIP نفسه كما في تنزيلات طلبات الفحص.
...
مسار حالة دفعي أساسي للمسؤولين فقط. يقبل `{ "jobIds": ["..."] }` ويُرجع العدادات المجمعة نفسها مثل `POST /api/v1/skills/-/rescan-batch/status` القديم.
...
- هوية المهارة، وهوية الناشر، وبيانات الإصدار المحدد الوصفية هي حقول غلاف على المستوى الأعلى (`slug` و`displayName` و`publisherHandle` و`version` و`resolvedFrom` و`tag` و`createdAt`) حتى تتمكن أتمتة الصدفة من قراءتها دون فك تغليف أغلفة متداخلة.
...
- يُحتفظ بـ `security.signals.dependencyRegistry` للتوافق مع استجابة v1، لكن ماسح وجود سجل الاعتماديات أُحيل إلى التقاعد وهذا المفتاح يكون دائمًا `null`.
- تكون `provenance` بقيمة `server-resolved-github-import` فقط عندما يحل ClawHub مستودع/مرجع/التزام/مسار GitHub ويخزنه أثناء النشر أو الاستيراد؛ وإلا فتكون `unavailable`.

### `POST /api/v1/skills/-/security-verdicts`

يُرجع أحكام الأمان المدمجة الحالية لإصدارات مهارات دقيقة. نقطة نهاية
...
- النتائج تكون لكل عنصر؛ عدم وجود مهارة أو إصدار واحد لا يؤدي إلى فشل الاستجابة كلها.
- الاستجابة مخصصة للأمان فقط. لا تتضمن بيانات بطاقة المهارة، أو حالة البطاقة المُولَّدة، أو قوائم ملفات الأثر، أو حمولات الماسحات التفصيلية.
...
- يُحتفظ بـ `security.signals.dependencyRegistry` للتوافق مع استجابة v1، لكن ماسح وجود سجل الاعتماديات أُحيل إلى التقاعد وهذا المفتاح يكون دائمًا `null`.
- لا يؤثر غياب بطاقة المهارة في `ok` أو `decision` أو `reasons` لنقطة النهاية هذه؛ يجب على العملاء قراءة `skill-card.md` المثبت محليًا عندما يحتاجون إلى محتوى البطاقة.
- استخدم `/verify` عندما تحتاج إلى غلاف تحقق بطاقة المهارة لمهارة واحدة، و`/card` عندما تحتاج إلى Markdown البطاقة المُولَّدة، و`/scan` عندما تحتاج إلى بيانات ماسحات تفصيلية.
...
- يبقى `GET /api/v1/code-plugins` و`GET /api/v1/bundle-plugins` اسمين مستعارين لعائلة ثابتة.
...
- يستطيع المتصلون المصادق عليهم رؤية الحزم الخاصة للناشرين الذين ينتمون إليهم في نتائج القائمة/البحث.
...
- `isOfficial` (اختياري): `true` أو `false`
- `category` (اختياري): عامل تصفية فئة Plugin. مدعوم فقط عندما يكون
  الطلب مقيدًا بحزم Plugin. الفئات المضبوطة وأسماء عوامل التصفية
...
- يرى المتصلون المجهولون قنوات الحزم العامة فقط.
- يستطيع المتصلون المصادق عليهم البحث في الحزم الخاصة للناشرين الذين ينتمون إليهم.
- لا يعيد `channel=private` إلا الحزم التي يمكن للمتصل المصادق عليه قراءتها.

### `GET /api/v1/plugins`

تصفح كتالوج Plugin فقط عبر حزم code-plugin وbundle-plugin.
...
- `startDate` (مطلوب): حد أدنى بوحدة ميلي ثانية Unix لـ `updatedAt` في Skills.
- `endDate` (مطلوب): حد أعلى بوحدة ميلي ثانية Unix لـ `updatedAt` في Skills.
...
- المتن: أرشيف ZIP.
- يتم تجذير كل Skill مصدرة عند `{publisher}/{slug}/`.
- تتضمن Skills المستضافة ملفات أحدث إصدار مخزن وتُدرج في
  `_manifest.json` مع `sourceRef: "public-clawhub"`.
- تتضمن Skills الحالية المدعومة من GitHub مع فحص `clean` أو `suspicious`
...
بحث خاص بـ Plugin عبر حزم code-plugin و bundle-plugin.
...
- تُقبَل أيضًا الأسماء المستعارة القديمة لمرشحات v1 الموثقة ضمن `GET /api/v1/plugins`.
- تصفية الفئات مرشح API حقيقي مدعوم بصفوف ملخص فئات Plugin،
...
- تكون `release.artifactKind` و `release.artifactSha256` و `release.npmIntegrity`
  ...
  لأثر الإصدار.
- `trust.scanStatus` هي حالة الثقة الفعلية المستمدة من مدخلات الفاحص
  والمراجعة اليدوية للإصدار.
- `trust.moderationState` قابلة للقيمة الفارغة. تكون `null` عند عدم وجود
...
- نقطة النهاية هذه أضيق عمدًا من نقاط نهاية المراجعة الخاصة بالمالك/المشرف.
  فهي تكشف قرار التثبيت والشرح العام، لا هويات المبلّغين، أو متون البلاغات،
...
- هذا هو سطح محلل OpenClaw؛ فهو يتجنب تخمين صيغة الأرشيف من رابط مشترك.
...
- يُطبَّع `packageName` كاسم npm؛ ويمكن أن تكون الحزمة مفقودة للترحيلات المخططة.
...
تلقائيًا بذاتها؛ ينبغي للمشرفين استخدام مراجعة الإصدار للموافقة على الآثار
أو عزلها أو إلغائها.
...
- أرشيفات Plugin/الحزم هي ملفات zip ذات جذر `package/` بحيث تواصل عملاء OpenClaw
  القديمة العمل.
- يبقى هذا المسار خاصا بملفات ZIP فقط. ولا يبث ملفات ClawPack `.tgz`.
...
- تُدرج فقط الإصدارات التي تحتوي على كرات tarball مرفوعة من نوع ClawPack npm-pack.
- تُحذف عمدا الإصدارات القديمة الخاصة بملفات ZIP فقط.
...
- تدعم packuments الحزم ذات النطاق كلا من مساري الطلب `/api/npm/@scope/name` و
  المسار المرمز في npm وهو `/api/npm/@scope%2Fname`.
...
ينزل ملف ZIP لإصدار Skill مستضاف، أو يعيد تسليما لمصدر GitHub من أجل Skill
حالية مدعومة من GitHub ذات فحص `clean` أو `suspicious` ودون إصدار
مستضاف.
...
- لا تقوم تسليمات Skill المدعومة من GitHub بتمرير البايتات أو عكسها. تتضمن استجابة JSON
  ...
  و`archiveUrl`؛ وحالة الفحص/الحالة الحالية بوابة ولا تُضمَّن كبيانات تعريف
  لحمولة النجاح.
...
- حقول النموذج المسموح بها هي `payload`، أو كتل `files` مكررة، أو مرجع tarball واحد من `clawpack`.
  قد يكون `clawpack` كتلة `.tgz` أو معرّف تخزين أعاده
  سير upload-url. يجب أن تتضمن عمليات النشر المرحلية بمعرّف التخزين أيضا
...
- تُحد طلبات النشر المباشر متعددة الأجزاء عند 18MB. يمكن لكرات tarball من ClawPack
...
- تتطلب حزم Plugin وجود `openclaw.plugin.json`. ويجب أن تحتوي عمليات رفع ClawPack `.tgz`
  عليه في `package/openclaw.plugin.json`.
- تتطلب code plugins وجود `package.json`، وبيانات تعريف مستودع المصدر، وبيانات تعريف تثبيت المصدر،
...
تحجز عمليات الحذف اللين التي يبدأها المالك slug لمدة 30 يوما، ثم يمكن
لناشر آخر المطالبة به. تتضمن استجابة الحذف `slugReservedUntil` عند انطباق هذا الانتهاء.
لا تنتهي عمليات الإخفاء من المشرف/المدير وعمليات الإزالة الأمنية بهذه الطريقة.
...
مستخدم مشترك قديم/ناشر شخصي قديم، فتنقله نقطة النهاية أولا إلى ناشر مؤسسة.
للمؤسسة المنشأة حديثا، قدم `memberHandle`؛ ولا يُضاف المدير الفاعل كعضو.
...
- يعيد `409` عندما يكون المعرّف مستخدما بالفعل من قبل ناشر أو مستخدم أو ناشر شخصي.

### `POST /api/v1/users/reserve`

للمديرين فقط. يحجز root slugs وأسماء الحزم للمالك الشرعي من دون نشر
إصدار. تصبح أسماء الحزم حزما نائبة خاصة بلا صفوف إصدار، بحيث يمكن للمالك نفسه
نشر إصدار code-plugin أو bundle-plugin الحقيقي لاحقا في ذلك الاسم.
...
للمديرين فقط. يستعيد ناشرا شخصيا لمعرّف أساسي بديل ومتحقق منه في GitHub OAuth
من دون تعديل صفوف حساب Convex Auth. يجب أن يسمي الطلب كلا معرّفي حساب
مزود GitHub غير القابلين للتغيير؛ ولا تُستخدم المعرّفات القابلة للتغيير إلا كحماية موجهة للمشغل.
...
هويتي GitHub. يفشل الاسترداد بشكل مغلق عندما يكون للناشر الشخصي الحالي للمستخدم الوجهة
...
للمقبض المسترد إلى المستخدم البديل كي لا تتمكن مزامنة الملف الشخصي لاحقاً من استعادة
...
مصادر Skills من GitHub ذات نطاق ناشر، ويُبلّغ عنها كعناصر جرى فحصها بدلاً من إعادة كتابتها.
...
تغيير السبب المخزن لحظر قائم من دون إلغاء الحظر أو استعادة
المحتوى (للمدير فقط). يكون افتراضياً في وضع التشغيل التجريبي ما لم تكن `dryRun` بقيمة `false`.
...
يعيد `POST /api/cli/upload-url` كلاً من `uploadUrl` و`uploadTicket`. يجب على عمليات نشر الحزم
التي تجهز أرشيف ClawPack tarball إرسال معرّف التخزين الناتج باسم
