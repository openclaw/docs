...
summary: مرجع واجهة برمجة تطبيقات HTTP (النقاط الطرفية العامة + نقاط CLI الطرفية + المصادقة).
x-i18n:
...
جميع مسارات v1 موجودة ضمن `/api/v1/...`.
تبقى `/api/...` و`/api/cli/...` القديمة للتوافق (راجع `DEPRECATIONS.md`).
OpenAPI: `/api/v1/openapi.json`.

## إعادة استخدام الفهرس العام

يمكن للأدلة التابعة لجهات خارجية استخدام نقاط النهاية العامة للقراءة لسرد Skills في ClawHub أو البحث عنها. يُرجى تخزين النتائج مؤقتًا، واحترام `429`/`Retry-After`، وربط المستخدمين مرة أخرى بقائمة ClawHub الأصلية (`https://clawhub.ai/<owner>/skills/<slug>`)، وتجنب الإيحاء بأن ClawHub يؤيد موقع الجهة الخارجية. لا تحاول عكس المحتوى المخفي أو الخاص أو المحظور بسبب الإشراف خارج سطح API العام.

تُحل اختصارات معرفات URL على الويب عبر عائلات السجل، لكن على عملاء API استخدام
...
- ينبغي ألا تعيد نقاط نهاية الكتابة المصادَق عليها `Unauthorized` مجردًا عندما
  يعرف الخادم السبب. ينبغي أن يحصل كل من الرموز المفقودة، والرموز غير الصالحة/الملغاة،
  والحسابات المحذوفة/المحظورة/المعطلة على نص قابل للتصرف كي يتمكن عملاء CLI
...
- `X-RateLimit-Remaining` / `RateLimit-Remaining`: الميزانية المتبقية الدقيقة عند وجودها.
  تحذف الطلبات الناجحة المقسمة إلى شظايا هذا الرأس بدلًا من إعادة قيمة عامة تقريبية.
...
- استخدم تراجعًا عشوائيًا لتجنب عمليات إعادة المحاولة المتزامنة.
...
- يستخدم ClawHub رؤوس التمرير الموثوقة لتحديد عناوين IP للعملاء عند الطرف.
- إذا لم يتوفر IP عميل موثوق، تستخدم الطلبات المجهولة حاويات بديلة
  محددة فقط حسب نوع حد المعدل. لا تتضمن هذه الحاويات البديلة
  المسارات أو المعرفات المختصرة أو أسماء الحزم أو الإصدارات أو سلاسل الاستعلام أو غيرها من
...
قراءة جسم الاستجابة كسلسلة قابلة للقراءة بشريًا. تُتجاهل معلمات الاستعلام غير المعروفة
...
- تُعاد النتائج بترتيب الصلة (تشابه التضمين + تعزيزات تطابق رمز المعرف المختصر/الاسم الدقيقة + عامل شعبية سابق صغير).
- الصلة أقوى من الشعبية. يمكن لتطابق دقيق في المعرف المختصر أو رمز اسم العرض أن يتقدم على تطابق أوسع ذي تفاعل أقوى بكثير.
- يُجزأ نص ASCII عند حدود الكلمات وعلامات الترقيم. على سبيل المثال، يحتوي `personal-map` على رمز `map` مستقل، بينما يحتوي `amap-jsapi-skill` على `amap` و`jsapi` و`skill`؛ لذلك يعطي البحث عن `map` تطابقًا معجميًا أقوى لـ `personal-map` من `amap-jsapi-skill`.
- تُقاس الشعبية بمقياس لوغاريتمي وتُحدد بسقف. يمكن أن تحصل Skills ذات التفاعل العالي على ترتيب أدنى عندما يكون نص الاستعلام تطابقًا أضعف.
...
- ضع المصطلحات التي سيبحث عنها المستخدمون حرفيًا في اسم العرض والملخص والوسوم. استخدم رمز معرف مختصر مستقلًا فقط عندما يكون أيضًا هوية مستقرة تريد الاحتفاظ بها.
- لا تعيد تسمية معرف مختصر لملاحقة استعلام واحد فقط ما لم يكن المعرف المختصر الجديد اسمًا أصليًا أفضل على المدى الطويل. تصبح المعرفات المختصرة القديمة أسماء إعادة توجيه بديلة، لكن عنوان URL الأصلي والمعرف المختصر المعروض وملخصات البحث المستقبلية تستخدم المعرف المختصر الجديد.
- تحافظ أسماء إعادة التسمية البديلة على الحل لعناوين URL القديمة وعمليات التثبيت التي تُحل عبر السجل، لكن ترتيب البحث يعتمد على بيانات Skill الوصفية الأصلية بعد فهرسة إعادة التسمية. تبقى الإحصاءات الحالية مع Skill.
...
- `createdAt` مستقر لزحف Skills الجديدة؛ يتغير `updated` عند إعادة نشر Skills الحالية.
- عندما تكون `nonSuspiciousOnly=true`، قد تعيد عمليات الترتيب المستندة إلى المؤشر عدد عناصر أقل من `limit` في الصفحة لأن Skills المشبوهة تُصفى بعد جلب الصفحة.
...
- تُحل المعرفات المختصرة القديمة التي أنشأتها تدفقات إعادة تسمية/دمج المالك إلى Skill الأصلية.
...
- تُدرج `moderation` فقط عندما تكون Skill مُعلَّمة أو عندما يعرضها المالك.
...
- يحصل المستدعون العامون على `200` فقط لـ Skills المرئية المُعلَّمة مسبقًا.
...
- يتضمن حالة تحقق موحدة بالإضافة إلى تفاصيل خاصة بكل ماسح.
...
- تتطلب فحوصات المنشور وصول إدارة المالك/الناشر، أو صلاحية مشرف/مسؤول المنصة.
...
- مهام الفحص غير متزامنة. تعطى طلبات الفحص اليدوية أولوية على أعمال النشر/التعبئة الراجعة العادية، لكن الإكمال لا يزال يعتمد على توفر العامل.

### `GET /api/v1/skills/-/scan/{scanId}`

نقطة استطلاع مصادق عليها لفحص تم إرساله.

- ترجع حالة الانتظار/التشغيل/النجاح/الفشل.
- ترجع `queue.queuedAhead` و`queue.position` أثناء الانتظار حتى يتمكن العملاء من عرض عدد الفحوصات اليدوية ذات الأولوية الموجودة قبل الطلب. يتم تقييد قوائم الانتظار الكبيرة جدا والإبلاغ عنها باستخدام `queuedAheadIsEstimate: true`.
...
- ترجع شكل ZIP نفسه مثل تنزيلات طلبات الفحص.
...
مسار حالة دفعة أساسي للمسؤولين فقط. يقبل `{ "jobIds": ["..."] }` ويرجع العدادات الإجمالية نفسها مثل `POST /api/v1/skills/-/rescan-batch/status` القديم.

### `GET /api/v1/skills/{slug}/verify`

يرجع غلاف التحقق من بطاقة المهارة المستخدم بواسطة `clawhub skill verify`.

معلمات الاستعلام:

- `version` (اختياري): سلسلة إصدار محددة.
- `tag` (اختياري): حل إصدار ذي وسم (مثلا `latest`).

ملاحظات:

- تكون `ok` بقيمة `true` فقط عندما يحتوي الإصدار المحدد على بطاقة مهارة مولدة، ولا يكون محظورا كبرمجية خبيثة بواسطة الإشراف، ويكون تحقق ClawScan نظيفا.
...
- يتم الاحتفاظ بـ `security.signals.dependencyRegistry` لتوافق استجابة v1، لكن ماسح وجود سجل الاعتمادات متقاعد وهذا المفتاح يكون دائما `null`.
- تكون `provenance` بقيمة `server-resolved-github-import` فقط عندما يحل ClawHub ويخزن مستودع GitHub/مرجعا/التزاما/مسارا أثناء النشر أو الاستيراد؛ وإلا تكون `unavailable`.

### `POST /api/v1/skills/-/security-verdicts`

يرجع أحكام الأمان المدمجة الحالية لإصدارات المهارات الدقيقة. نقطة نهاية
المجموعة هذه مخصصة للعملاء الذين يعرفون مسبقا أي إصدارات مهارات ClawHub المثبتة
يحتاجون إلى عرضها، مثل OpenClaw Control UI.
...
- يحتوي `security.signals` على أدلة داعمة على مستوى الحالة فقط؛ استخدم `/scan` أو صفحة تدقيق أمان ClawHub للحصول على التفاصيل الكاملة للماسح.
- يتم الاحتفاظ بـ `security.signals.dependencyRegistry` لتوافق استجابة v1، لكن ماسح وجود سجل الاعتمادات متقاعد وهذا المفتاح يكون دائما `null`.
- لا يؤثر غياب بطاقة المهارة في `ok` أو `decision` أو `reasons` الخاصة بنقطة النهاية هذه؛ يجب على العملاء قراءة `skill-card.md` المثبت محليا عندما يحتاجون إلى محتوى البطاقة.
- استخدم `/verify` عندما تحتاج إلى غلاف التحقق من بطاقة المهارة لمهارة واحدة، و`/card` عندما تحتاج إلى Markdown البطاقة المولدة، و`/scan` عندما تحتاج إلى بيانات ماسح تفصيلية.
...
- الإعداد الافتراضي هو أحدث إصدار.
- حد حجم الملف: 200KB.
...
- `sort` (اختياري): `updated` (الافتراضي)، أو `recommended`، أو `trending`، أو `downloads`، أو الاسم المستعار القديم `installs`
- `category` (اختياري): مرشح فئة Plugin. مدعوم فقط عندما يكون
  الطلب محصورًا في حزم Plugin (`/api/v1/plugins` أو
  ...
  مرشحات v1 القديمة المستعارة موثقة ضمن `GET /api/v1/plugins`.
...
- يظل `GET /api/v1/code-plugins` و`GET /api/v1/bundle-plugins` اسمين مستعارين لعائلتين ثابتتين.
...
- `category` (اختياري): مرشح فئة Plugin. مدعوم فقط عندما يكون
  ...
  القديمة المستعارة موثقة ضمن `GET /api/v1/plugins`.
...
استعراض فهرس مخصص لـ Plugin فقط عبر حزم `code-plugin` و`bundle-plugin`.
...
- `cursor` (اختياري): مؤشر ترقيم الصفحات
- `isOfficial` (اختياري): `true` أو `false`
- `sort` (اختياري): `recommended` (الافتراضي)، أو `trending`، أو `downloads`، أو `updated`، أو الاسم المستعار القديم `installs`
- `category` (اختياري): مرشح فئة Plugin. القيم الحالية:
  `channels`، `models`، `memory`، `context`، `voice`، `media`، `web`،
  `tools`، `runtime`، `gateway`، `security`، `other`.

تظل أسماء مرشحات v1 القديمة المستعارة مقبولة في نقاط نهاية القراءة:
...
في نقطة النهاية الموحدة `/api/v1/packages` يكون مخصصًا لـ Plugin فقط؛ استخدم
...
- `startDate` (مطلوب): الحد الأدنى بميلي ثواني Unix لقيمة `updatedAt` في Skill.
- `endDate` (مطلوب): الحد الأعلى بميلي ثواني Unix لقيمة `updatedAt` في Skill.
...
- تتضمن Skills الحالية المدعومة من GitHub ذات فحص `clean` أو `suspicious`
  `_source_handoff.json` مع `sourceRef: "public-github"` والمستودع وcommit والمسار
...
تصدير مجمّع لأحدث إصدارات Plugin العامة للتحليل دون اتصال.
...
- تُخزَّن بيانات وصفية للتصدير لكل Plugin عند
  `__clawhub_export/{family}/{packageName}/plugin_meta.json`.
- يُدرج `_manifest.json` دائماً في جذر ملف ZIP.
- يُدرج `_errors.json` عندما لا يمكن تصدير Plugins أو ملفات فردية.
...
يعيد إصدار حزمة واحداً، بما في ذلك بيانات وصفية للملفات، والتوافق،
والتحقق، وبيانات وصفية للأثر، وبيانات الفحص.

ملاحظات:

- تكون `version.artifact.kind` بالقيمة `legacy-zip` لأرشيفات حزم العالم القديم أو
...
- `version.sha256hash` هي بيانات وصفية توافقية مهملة للعملاء القدامى. وهي
...
- تُدرج `version.vtAnalysis` و`version.llmAnalysis` و`version.staticScan` عندما
  توجد بيانات فحص.
- تعيد الحزم الخاصة `404` ما لم يتمكن المستدعي من قراءة الناشر المالك.
...
- تكون `release.artifactKind` و`release.artifactSha256` و`release.npmIntegrity`
  ...
  لأثر الإصدار.
- `trust.scanStatus` هي حالة الثقة الفعالة المشتقة من مدخلات الفاحص
  ومن الإشراف اليدوي على الإصدار.
...
- `trust.blockedFromDownload` هي إشارة حظر التثبيت. ينبغي لـ OpenClaw وعملاء
  التثبيت الآخرين حظر التثبيت عندما تكون هذه القيمة `true` بدلاً من
  إعادة اشتقاق قواعد الحظر من حقول الفاحص أو الإشراف.
- `trust.reasons` هي قائمة الشرح الموجهة للمستخدم والتدقيق. رموز الأسباب
  ...
  و`package:malicious`.
- `trust.pending` تعني أن واحداً أو أكثر من مدخلات الثقة لا يزال ينتظر الاكتمال.
- `trust.stale` تعني أن ملخص الثقة حُسب من مدخلات قديمة وينبغي التعامل معه
  على أنه يتطلب تحديثاً قبل قرار سماح عالي الثقة.

ملاحظات:

- نقطة النهاية هذه دقيقة للإصدار. ينبغي للعملاء استدعاؤها بعد حل إصدار
  الحزمة الذي ينوون تثبيته، وليس فقط بعد قراءة أحدث بيانات وصفية للحزمة.
- تعيد الحزم الخاصة `404` ما لم يتمكن المستدعي من قراءة الناشر المالك.
- نقطة النهاية هذه أضيق عمداً من نقاط نهاية إشراف المالك/المشرف. فهي تعرض
...
يعيد بيانات وصفية صريحة لمحلل الأثر لإصدار حزمة.

ملاحظات:

- تعيد إصدارات الحزم القديمة أثراً من نوع `legacy-zip` و
  `downloadUrl` لملف ZIP قديم.
- تعيد إصدارات ClawPack أثراً من نوع `npm-pack`، وحقول سلامة npm،
...
- تبث إصدارات ClawPack بايتات `.tgz` الدقيقة من npm-pack المرفوع.
- تعيد إصدارات ZIP القديمة التوجيه إلى `/api/v1/packages/{name}/download?version=`.
- تستخدم حاوية معدل التنزيل.

### `GET /api/v1/packages/{name}/readiness`

يعيد الجاهزية المحسوبة لاستهلاك OpenClaw مستقبلاً.
...
- ملخص الأثر
- مستودع المصدر ومصدرية الالتزام
...
- `open`: إصدارات مشبوهة أو خبيثة أو معلقة أو محجورة أو مسحوبة أو مُبلّغ عنها.
- `blocked`: إصدارات محجورة أو مسحوبة أو خبيثة.
...
اختيارياً بإصدار. وهي تغذي قائمة انتظار الإشراف لكنها لا تخفي التنزيلات
أو تحظرها تلقائياً بحد ذاتها؛ ينبغي للمشرفين استخدام إشراف الإصدارات
لاعتماد الآثار أو حجرها أو سحبها.
...
نقطة نهاية للمالك/المشرف لإتاحة رؤية الإشراف على الحزمة.
...
- يتم التعيين افتراضيًا إلى أحدث إصدار.
...
- أرشيفات Plugin/الحزم هي ملفات zip بجذر `package/` حتى تستمر عملاء OpenClaw
  القديمة في العمل.
...
- لا تُدرج إلا الإصدارات التي تحتوي على أرشيفات tarball بصيغة npm-pack من ClawPack مرفوعة.
- تُحذف الإصدارات القديمة المقتصرة على ZIP عمدًا.
...
- تدعم packuments للحزم ذات النطاق كلًا من مسار الطلب `/api/npm/@scope/name` ومسار npm
  المرمّز `/api/npm/@scope%2Fname`.

### `GET /api/npm/{package}/-/{tarball}.tgz`

يبث بايتات أرشيف tarball المرفوع من ClawPack بالضبط لعملاء مرآة npm.
...
- `slug` (مطلوب)
- `hash` (مطلوب): sha256 سداسي عشري مكون من 64 حرفًا لبصمة الحزمة
...
ينزّل ZIP لإصدار Skill مستضافة، أو يعيد تسليمًا إلى مصدر GitHub لـ Skill
حالية مدعومة من GitHub لديها فحص `clean` أو `suspicious` ولا تملك إصدارًا
مستضافًا.
...
- لا تقوم عمليات تسليم Skills المدعومة من GitHub بتمرير البايتات عبر وكيل أو عكسها. تتضمن استجابة JSON
  ...
  و`archiveUrl`؛ حالة الفحص/الحالة الحالية هي بوابة ولا تُضمّن كبيانات وصفية لحمولة نجاح.
- تُحتسب إحصاءات التنزيل كهويات فريدة لكل يوم UTC (`userId` عندما يكون رمز API صالحًا، وإلا فعنوان IP).
...
- يُقبل أيضًا جسم JSON يحتوي على `files` (مستندة إلى storageId).
...
- حقول النموذج المسموح بها هي `payload`، أو كائنات blob مكررة في `files`، أو مرجع tarball واحد
  في `clawpack`. قد يكون `clawpack` كائن blob بصيغة `.tgz` أو معرّف تخزين أعاده
...
- تُحد طلبات النشر المباشرة متعددة الأجزاء عند 18 ميغابايت. يمكن لأرشيفات ClawPack tarballs
  استخدام تدفق upload-url حتى حد 120 ميغابايت لأرشيف tarball.
- حقل حمولة اختياري: `ownerHandle`. عند وجوده، لا يجوز إلا للمديرين النشر نيابةً عن ذلك المالك.
...
تحجز عمليات الحذف المرن التي يبدأها المالك الـ slug لمدة 30 يومًا، ثم يمكن أن يطالب
ناشر آخر بالـ slug. تتضمن استجابة الحذف `slugReservedUntil` عندما ينطبق هذا الانتهاء.
ولا تنتهي بهذه الطريقة عمليات الإخفاء من المشرف/المدير وإزالات الأمان.
...
بالنسبة إلى مؤسسة منشأة حديثًا، قدّم `memberHandle`؛ ولا يُضاف المدير الفاعل كعضو.
...
إنشاء ناشر مؤسسة بالخدمة الذاتية مع المصادقة. ينشئ ناشر مؤسسة جديدًا ويضيف
المستدعي كمالك. لا تنقل نقطة النهاية هذه المعرّفات الموجودة للمستخدم/الشخصية، ولا
...
للمدير فقط. يحجز slugs الجذرية وأسماء الحزم لمالك مستحق من دون نشر
إصدار. تصبح أسماء الحزم حزمًا نائبة خاصة من دون صفوف إصدار، بحيث يستطيع
المالك نفسه نشر إصدار code-plugin أو bundle-plugin الحقيقي إلى ذلك الاسم لاحقًا.
...
للمدير فقط. يستعيد ناشرًا شخصيًا لجهة GitHub OAuth بديلة متحقق منها
من دون تحرير صفوف حساب Convex Auth. يجب أن يذكر الطلب معرّفي حساب موفر GitHub
الثابتين؛ ولا تُستخدم المعرّفات القابلة للتغيير إلا كحماية موجهة للمشغّل.
...
حسابَي GitHub الأساسيين. يفشل الاسترداد بإغلاق آمن عندما يكون للناشر الشخصي الحالي للمستخدم الوجهة
Skills أو حزم أو مصادر Skills على GitHub.
يرحّل الاسترداد أيضًا حقول `ownerUserId` القديمة الخاصة بـ Skills الناشر المسترَد،
والأسماء المستعارة لارتباطات Skills، والحزم، وتحذيرات مفتش الحزم، وصفوف ملخصات البحث المشتقة بحيث
تتفق مسارات المالك المباشر مع سلطة الناشر الجديدة. كما يُعاد تعيين حجز المعرّف المحمي النشط
للمعرّف المسترَد إلى المستخدم البديل حتى لا تتمكن مزامنة الملف الشخصي لاحقًا من استعادة سلطة المستخدم السابق المنافسة. كل جدول أساسي مقيّد بـ
100 صف لكل معاملة تطبيق؛ يجب أن تستخدم عمليات الاسترداد الأكبر أولًا ترحيل مالك قابلًا للاستئناف.
تكون مصادر Skills على GitHub ضمن نطاق الناشر ويُبلّغ عنها كمفحوصة بدلًا من إعادة كتابتها.
...
### نقاط نهاية إدارة ارتباط المالك
...
- يحافظ `rename` على الارتباط السابق كاسم مستعار لإعادة التوجيه.
- يخفي `merge` إدراج المصدر ويعيد توجيه ارتباط المصدر إلى إدراج الهدف.
...
المحتوى (للمدير فقط). يكون الوضع الافتراضي تشغيلًا تجريبيًا ما لم تكن `dryRun` هي `false`.
...
إضافة/إزالة نجمة (تمييزات). كلتا نقطتَي النهاية متكافئتا التكرار.
...
يعيد `POST /api/cli/upload-url` القيمتَين `uploadUrl` و`uploadTicket`. يجب على عمليات نشر الحزم
التي تجهّز أرشيف ClawPack بصيغة tarball إرسال معرّف التخزين الناتج باسم
...
إذا كنت تستضيف ذاتيًا، فوفّر هذا الملف (أو اضبط `CLAWHUB_REGISTRY` صراحةً؛ القديم `CLAWDHUB_REGISTRY`).
