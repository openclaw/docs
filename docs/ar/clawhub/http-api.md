...
- يجب ألا تُرجع نقاط نهاية الكتابة المصادق عليها مجرد `Unauthorized` عندما
  يعرف الخادم السبب. يجب أن يحصل كل من الرموز المفقودة، والرموز غير الصالحة/الملغاة،
  والحسابات المحذوفة/المحظورة/المعطلة على نص قابل للتنفيذ بحيث يستطيع عملاء CLI
...
- يستخدم ترويسات IP العميل الموثوقة، بما في ذلك `cf-connecting-ip`، فقط عندما
  يفعّل النشر صراحةً الترويسات المعاد توجيهها الموثوقة.
...
- إذا لم يكن IP عميل موثوق متاحًا، تستخدم الطلبات المجهولة حاويات احتياطية
  ...
  معلمات العناصر التي يوفّرها المستدعي.
...
قراءة جسم الاستجابة كسلسلة نصية قابلة للقراءة من البشر. تُتجاهل معلمات الاستعلام
غير المعروفة حفاظًا على التوافق، لكن معلمات الاستعلام المعروفة ذات القيم غير الصالحة تُرجع
`400`.
...
- تُعاد النتائج بترتيب الصلة (تشابه التضمين + تعزيزات المطابقة الدقيقة لرموز slug/الاسم + عامل شائع صغير).
- الصلة أقوى من الشيوع. يمكن لمطابقة دقيقة لرمز slug أو اسم العرض أن تتفوق على مطابقة أوسع ذات تفاعل أقوى بكثير.
- يُجزّأ نص ASCII على حدود الكلمات وعلامات الترقيم. على سبيل المثال، يحتوي `personal-map` على رمز `map` مستقل، بينما يحتوي `amap-jsapi-skill` على `amap` و`jsapi` و`skill`؛ لذلك فإن البحث عن `map` يعطي `personal-map` مطابقة معجمية أقوى من `amap-jsapi-skill`.
- الشيوع مُقاس لوغاريتميًا ومحدود بسقف. يمكن أن تأتي Skills ذات التفاعل العالي في مرتبة أدنى عندما يكون نص الاستعلام أضعف مطابقة.
...
- لا تعيد تسمية slug لمجرد ملاحقة استعلام واحد ما لم يكن slug الجديد اسمًا أساسيًا أفضل على المدى الطويل. تصبح slugs القديمة أسماء مستعارة لإعادة التوجيه، لكن عنوان URL الأساسي، وslug المعروض، وملخصات البحث المستقبلية تستخدم slug الجديد.
- تحافظ أسماء إعادة التسمية المستعارة على الحل لعناوين URL القديمة وعمليات التثبيت التي تُحل عبر السجل، لكن ترتيب البحث يعتمد على بيانات Skill الوصفية الأساسية بعد فهرسة إعادة التسمية. تبقى الإحصاءات الحالية مع Skill.
- إذا كانت Skill غير مرئية بشكل غير متوقع، فتحقق من حالة الإشراف أولًا باستخدام `clawhub inspect @owner/slug` أثناء تسجيل الدخول قبل تغيير البيانات الوصفية المرتبطة بالترتيب.
...
- `sort` (اختياري): `updated` (الافتراضي)، `recommended` (اسم مستعار: `default`)، `createdAt` (اسم مستعار: `newest`)، `downloads`، `stars` (اسم مستعار: `rating`)، أسماء تثبيت قديمة مستعارة `installsCurrent`/`installs`/`installsAllTime` تُعيّن إلى `downloads`، `trending`
- `nonSuspiciousOnly` (اختياري): `true` لإخفاء Skills المشبوهة (`flagged.suspicious`)
- `nonSuspicious` (اختياري): اسم مستعار قديم لـ `nonSuspiciousOnly`
...
- يرتّب `trending` حسب عمليات التثبيت في آخر 7 أيام (مبني على القياسات).
- يكون `createdAt` مستقرًا لزحف Skills الجديدة؛ يتغير `updated` عندما يُعاد نشر Skills الحالية.
- عندما تكون `nonSuspiciousOnly=true`، قد تُرجع عمليات الفرز المعتمدة على المؤشر عناصر أقل من `limit` في الصفحة لأن Skills المشبوهة تُصفّى بعد استرجاع الصفحة.
...
- تحل slugs القديمة التي تنشئها تدفقات إعادة تسمية/دمج المالك إلى Skill الأساسية.
...
بإصدار اختياريًا، وتغذي قائمة انتظار بلاغات Skill.
...
تعيين `status` مرة أخرى إلى `open`. مرّر `finalAction: "hide"` مع بلاغ مُفرَز
...
- إذا لم يُوفَّر لا `version` ولا `tag`، فسيستخدم أحدث إصدار.
...
- لا تكتب فحوصات المنشور النتائج رجوعًا إلا عندما تكون `update: true` ويكتمل الفحص بنجاح.
...
- مهام الفحص غير متزامنة. تُمنح طلبات الفحص اليدوية أولوية قبل أعمال النشر/الملء الخلفي العادية، لكن الإكمال ما زال يعتمد على توفر العمال.
...
نقطة نهاية أرشيف التقرير المصادَق عليها.
...
نقطة نهاية أرشيف التقرير المخزَّن المصادَق عليها للإصدارات المُرسَلة.

- تتطلب وصول إدارة المالك/الناشر إلى المهارة أو Plugin، أو صلاحية مشرف/مسؤول المنصة.
- تُرجع نتائج الفحص المخزَّنة للإصدار المُرسَل المحدد، بما في ذلك الإصدارات المحظورة أو المخفية.
...
مسار حالة الدفعة القانوني للمسؤولين فقط. يقبل `{ "jobIds": ["..."] }` ويُرجع عدادات التجميع نفسها مثل `POST /api/v1/skills/-/rescan-batch/status` القديم.
...
- هوية المهارة وهوية الناشر وبيانات تعريف الإصدار المحدد هي حقول في المستوى الأعلى من الغلاف (`slug` و`displayName` و`publisherHandle` و`version` و`resolvedFrom` و`tag` و`createdAt`) حتى تتمكن أتمتة الصدفة من قراءتها دون فك أغلفة متداخلة.
- `security` هو حكم ClawScan/الأمان في المستوى الأعلى. ينبغي للأتمتة الاعتماد على `ok` و`decision` و`reasons` و`security.status`.
- تحتوي `security.signals` على أدلة ماسحات داعمة مثل `staticScan` و`virusTotal` و`skillSpector`.
- يُحتفَظ بـ `security.signals.dependencyRegistry` للتوافق مع استجابة v1، لكن ماسح وجود سجل التبعيات متقاعد وهذا المفتاح دائمًا `null`.
- تكون `provenance` بقيمة `server-resolved-github-import` فقط عندما يحل ClawHub ويخزّن مستودع/مرجع/التزام/مسار GitHub أثناء النشر أو الاستيراد؛ وإلا فهي `unavailable`.
...
- يجب أن تحتوي `items` على 1-100 زوجًا فريدًا من `{ slug, version }`.
- النتائج لكل عنصر؛ عدم وجود مهارة أو إصدار واحد لا يفشل الاستجابة كلها.
- الاستجابة للأمان فقط. لا تتضمن بيانات بطاقة المهارة، أو حالة البطاقة المُنشأة، أو قوائم ملفات الأثر، أو حمولات الماسح التفصيلية.
- تحتوي `security.signals` على أدلة داعمة على مستوى الحالة فقط؛ استخدم `/scan` أو صفحة تدقيق أمان ClawHub للحصول على تفاصيل الماسح الكاملة.
- يُحتفَظ بـ `security.signals.dependencyRegistry` للتوافق مع استجابة v1، لكن ماسح وجود سجل التبعيات متقاعد وهذا المفتاح دائمًا `null`.
- عدم وجود بطاقة المهارة لا يؤثر في `ok` أو `decision` أو `reasons` لنقطة النهاية هذه؛ ينبغي للعملاء قراءة `skill-card.md` المثبت محليًا عندما يحتاجون إلى محتوى البطاقة.
- استخدم `/verify` عندما تحتاج إلى غلاف تحقق بطاقة مهارة واحدة، و`/card` عندما تحتاج إلى Markdown البطاقة المُنشأة، و`/scan` عندما تحتاج إلى بيانات ماسح تفصيلية.
...
- `category` (اختياري): مرشح فئة Plugin. مدعوم فقط عندما يكون
  الطلب محدد النطاق بحِزم Plugin (`/api/v1/plugins`،
  ...
  مرشحات v1 البديلة القديمة موثقة تحت `GET /api/v1/plugins`.
...
- لا يرى المستدعون المجهولون إلا قنوات الحِزم العامة.
- يمكن للمستدعين المصادق عليهم رؤية الحِزم الخاصة للناشرين الذين ينتمون إليهم في نتائج القوائم/البحث.
- يعيد `channel=private` فقط الحِزم التي يستطيع المستدعي المصادق عليه قراءتها.

### `GET /api/v1/packages/search`

بحث موحد في الفهرس عبر Skills + حِزم Plugin.
...
- `isOfficial` (اختياري): `true` أو `false`
- `category` (اختياري): مرشح فئة Plugin. مدعوم فقط عندما يكون
  الطلب محدد النطاق بحِزم Plugin. الفئات المضبوطة وأسماء مرشحات v1
  البديلة القديمة موثقة تحت `GET /api/v1/plugins`.
...
تصفح فهرس مخصص لـ Plugin فقط عبر حِزم code-plugin وbundle-plugin.
...
`trending` هو ترتيب تثبيت/تنزيل لسبعة أيام ولا يستخدم الإجماليات على مر الزمن.
...
- `startDate` (مطلوب): الحد الأدنى بالميلي ثانية في Unix لـ `updatedAt` في Skill.
- `endDate` (مطلوب): الحد الأعلى بالميلي ثانية في Unix لـ `updatedAt` في Skill.
...
- الجسم: أرشيف ZIP.
- يتم تجذير كل Skill مصدّرة عند `{publisher}/{slug}/`.
...
- يتم تضمين `_errors.json` عندما يتعذر تصدير Skills أو ملفات منفردة.
...
- يكون `version.artifact.kind` بالقيمة `legacy-zip` لأرشيفات الحزم القديمة أو
...
- `version.sha256hash` هو بيانات تعريف توافق مهملة للعملاء القدامى. وهو
  يجزّئ بايتات ZIP الدقيقة التي يعيدها `/api/v1/packages/{name}/download`.
...
- نقطة النهاية هذه دقيقة على مستوى الإصدار. يجب على العملاء استدعاؤها بعد
  حل إصدار الحزمة الذي ينوون تثبيته، وليس فقط بعد قراءة أحدث بيانات تعريف
  للحزمة.
- تعيد الحزم الخاصة `404` ما لم يكن بإمكان المستدعي قراءة الناشر المالك.
- نقطة النهاية هذه أضيق عمدًا من نقاط نهاية إشراف المالك/المشرف. فهي تعرض
  قرار التثبيت والشرح العام، لا هويات المبلّغين، أو نصوص البلاغات، أو
...
- تعيد إصدارات الحزم القديمة أثر `legacy-zip` و`downloadUrl` لملف ZIP قديم.
...
- `open`: إصدارات مشبوهة أو خبيثة أو معلقة أو محجورة أو ملغاة أو مُبلَّغ عنها.
...
التنزيلات أو تحظرها تلقائيًا بذاتها؛ يجب على المشرفين استخدام إشراف الإصدار
...
- أرشيفات Plugin/الحزم هي ملفات zip بجذر `package/` حتى يظل عملاء OpenClaw
  القدامى يعملون.
- يبقى هذا المسار مقتصرًا على ZIP فقط. ولا يبث ملفات ClawPack بصيغة `.tgz`.
...
يبث بايتات tarball المرفوع بالضبط من ClawPack لعملاء مرآة npm.
...
- `slug` (مطلوب)
- `hash` (مطلوب): sha256 سداسي مكون من 64 حرفًا لبصمة الحزمة
...
ينزل ملف ZIP لإصدار skill مستضاف، أو يعيد تسليمًا إلى مصدر GitHub لـ skill
...
- إذا لم يُقدّم `version` ولا `tag`، فيُستخدم أحدث إصدار.
...
- لا تقوم تسليمات skills المدعومة من GitHub بتمرير البايتات عبر وكيل أو عكسها. تتضمن استجابة JSON
  ...
  و`archiveUrl`؛ حالة الفحص/الحالة الحالية بوابة ولا تُدرج كبيانات تعريف
  لحمولة نجاح.
...
- مفضل: `multipart/form-data` مع JSON في `payload` + كائنات `files[]`.
- يُقبل أيضًا جسم JSON مع `files` (المستندة إلى storageId).
- حقل حمولة اختياري: `ownerHandle`. عند وجوده، تحل واجهة API ذلك
  الناشر من جانب الخادم وتتطلب أن يكون لدى الفاعل صلاحية وصول إلى الناشر.
- حقل حمولة اختياري: `migrateOwner`. عندما يكون `true` مع `ownerHandle`، يمكن
  نقل skill موجودة إلى ذلك المالك إذا كان الفاعل مسؤولًا/مالكًا لدى كل من
...
- حقول النموذج المسموح بها هي `payload`، أو كائنات `files` المتكررة، أو مرجع tarball
  واحد باسم `clawpack`. قد يكون `clawpack` كائن `.tgz` أو معرّف تخزين أعاده
...
- استخدم إما `files` أو `clawpack`، وليس كليهما أبدًا في الطلب نفسه.
- تُرفض أجسام JSON وبيانات التعريف `payload.files` / `payload.artifact`
  المقدمة من المستدعي.
- تُحد طلبات النشر multipart المباشرة عند 18MB. يمكن لملفات tarball من ClawPack
  استخدام مسار upload-url حتى حد tarball البالغ 120MB.
- حقل حمولة اختياري: `ownerHandle`. عند وجوده، يجوز للمسؤولين فقط النشر نيابةً عن ذلك المالك.
...
- تتطلب code plugins وجود `package.json`، وبيانات تعريف مستودع المصدر، وبيانات تعريف
  تثبيت المصدر، وبيانات تعريف مخطط التكوين، و`openclaw.compat.pluginApi`، و
  `openclaw.build.openclawVersion`.
- `openclaw.hostTargets` و`openclaw.environment` بيانات تعريف اختيارية.
- يمكن فقط لناشر مؤسسة `openclaw` والأعضاء الحاليين في مؤسسة `openclaw` الذين يملكون
  ناشرين شخصيين النشر إلى قناة `official`.
- تظل عمليات النشر بالنيابة تتحقق من أهلية قناة official مقابل حساب المالك الهدف.
...
تحجز عمليات الحذف الناعم التي يبدأها المالك slug لمدة 30 يومًا، ثم يمكن
...
مستخدم مشترك قديم/ناشر شخصي، فتنقله نقطة النهاية إلى ناشر مؤسسة أولًا.
بالنسبة إلى مؤسسة منشأة حديثًا، قدم `memberHandle`؛ لا يُضاف المسؤول الفاعل كعضو.
...
المستدعي كمالك. لا تنقل نقطة النهاية هذه معرّفات المستخدمين/الشخصية الموجودة ولا
...
للمسؤولين فقط. يستعيد ناشرًا شخصيًا لمعرّف GitHub OAuth بديل ومتحقق منه
دون تعديل صفوف حساب Convex Auth. يجب أن يسمي الطلب كلا معرّفي حساب
المزود GitHub غير القابلين للتغيير؛ ولا تُستخدم المعرّفات القابلة للتغيير إلا كحاجز مواجه للمشغل.

يفترض الطرفية التشغيل التجريبي افتراضياً. يتطلب تطبيق الاسترداد `dryRun: false` و
...
يرحّل الاسترداد أيضاً حقول `ownerUserId` القديمة لمهارات الناشر المسترد،
...
للمقبض المسترد إلى المستخدم البديل حتى لا تتمكن مزامنة الملف الشخصي لاحقاً من استعادة سلطة المستخدم السابق المنافسة. يقتصر كل جدول أساسي على
100 صف لكل معاملة تطبيق؛ يجب أن تستخدم عمليات الاسترداد الأكبر أولاً ترحيلاً قابلاً للاستئناف للمالك.
مصادر مهارات GitHub ذات نطاق ناشر ويُبلغ عنها كمدققة بدلاً من إعادة كتابتها.
...
- تتطلب كلتا النقطتين مصادقة رمز API ولا تعملان إلا لمالك المهارة.
...
المحتوى (للمسؤول فقط). يكون التشغيل التجريبي هو الافتراضي ما لم تكن `dryRun` هي `false`.
...
إضافة/إزالة نجمة (تمييزات). كلتا النقطتين متكافئتا التكرار.
...
يعيد `POST /api/cli/upload-url` كلاً من `uploadUrl` و`uploadTicket`. يجب أن ترسل
عمليات نشر الحزم التي تهيئ أرشيف ClawPack الناتج بصيغة tar معرّف التخزين الناتج باسم
