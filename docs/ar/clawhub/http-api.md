...
x-i18n:
    generated_at: "2026-06-27T17:17:42Z"
    model: gpt-5.5
...
- ينبغي ألا تُرجع نقاط نهاية الكتابة الموثقة `Unauthorized` مجردة عندما
...
قراءة نص الاستجابة كسلسلة قابلة للقراءة البشرية. تُتجاهل معاملات الاستعلام غير المعروفة
...
- تُرجع النتائج بترتيب الصلة (تشابه التضمين + تعزيزات مطابقة رمز slug/الاسم الدقيقة + أولوية شعبية صغيرة).
- الصلة أقوى من الشعبية. يمكن لمطابقة دقيقة لرمز slug أو اسم عرض أن تتفوق على مطابقة أوسع ذات تفاعل أقوى بكثير.
- يُجزّأ نص ASCII عند حدود الكلمات وعلامات الترقيم. على سبيل المثال، يحتوي `personal-map` على رمز `map` مستقل، بينما يحتوي `amap-jsapi-skill` على `amap` و`jsapi` و`skill`؛ لذلك يمنح البحث عن `map` مطابقة معجمية أقوى لـ `personal-map` مقارنةً بـ `amap-jsapi-skill`.
- تُقاس الشعبية لوغاريتميًا وتُحدد بسقف. يمكن أن تحصل Skills ذات التفاعل العالي على ترتيب أدنى عندما يكون نص الاستعلام مطابقة أضعف.
- يمكن أن تزيل حالة الإشراف المشبوهة أو المخفية إحدى Skills من البحث العام بناءً على فلاتر المستدعي وحالة الإشراف الحالية.
...
- لا تعيد تسمية slug لملاحقة استعلام واحد فقط إلا إذا كان slug الجديد اسمًا رسميًا أفضل على المدى الطويل. تصبح slugs القديمة أسماء مستعارة لإعادة التوجيه، لكن عنوان URL الرسمي وslug المعروض وملخصات البحث المستقبلية تستخدم slug الجديد.
- تحافظ أسماء إعادة التسمية المستعارة على حل عناوين URL القديمة وعمليات التثبيت التي تُحل عبر السجل، لكن ترتيب البحث يستند إلى بيانات تعريف Skill الرسمية بعد فهرسة إعادة التسمية. تبقى الإحصاءات الحالية مع Skill.
- إذا كانت Skill غير مرئية بشكل غير متوقع، فتحقق أولًا من حالة الإشراف باستخدام `clawhub inspect @owner/slug` أثناء تسجيل الدخول قبل تغيير بيانات التعريف المتعلقة بالترتيب.
...
- `sort` (اختياري): `updated` (افتراضي)، `recommended` (اسم مستعار: `default`)، `createdAt` (اسم مستعار: `newest`)، `downloads`، `stars` (اسم مستعار: `rating`)، أسماء التثبيت القديمة المستعارة `installsCurrent`/`installs`/`installsAllTime` تُطابق `downloads`، `trending`
- `nonSuspiciousOnly` (اختياري): `true` لإخفاء Skills المشبوهة (`flagged.suspicious`)
- `nonSuspicious` (اختياري): اسم مستعار قديم لـ `nonSuspiciousOnly`
...
- `createdAt` ثابت لعمليات زحف Skills الجديدة؛ يتغير `updated` عند إعادة نشر Skills الموجودة.
- عندما يكون `nonSuspiciousOnly=true`، قد تُرجع عمليات الفرز المعتمدة على المؤشر عناصر أقل من `limit` في الصفحة لأن Skills المشبوهة تُصفّى بعد جلب الصفحة.
...
الإبلاغ عن Skill لمراجعة المشرف. البلاغات على مستوى Skill، ويمكن ربطها
اختياريًا بإصدار، وتغذي قائمة انتظار بلاغات Skill.
...
تم فرزه لإخفاء Skill ضمن نفس سير العمل القابل للتدقيق.
...
- إذا لم يتم توفير `version` أو `tag`، فسيستخدم أحدث إصدار.
- يتضمن حالة تحقق موحدة بالإضافة إلى تفاصيل خاصة بالماسح.
- تكون `security.hasScanResult` بالقيمة `true` فقط عندما ينتج ماسح حكماً نهائياً (`clean` أو `suspicious` أو `malicious`).
...
نقطة إرسال مصادقة لمهام ClawScan الجديدة.
...
- لا تكتب الفحوصات المنشورة النتائج عائداً إلا عندما تكون `update: true` ويكتمل الفحص بنجاح.
...
- مهام الفحص غير متزامنة. تُعطى طلبات الفحص اليدوية أولوية قبل أعمال النشر/الردم العادية، لكن الإكمال لا يزال يعتمد على توفر العمال.

### `GET /api/v1/skills/-/scan/{scanId}`

نقطة استطلاع مصادقة لفحص مُرسل.

- تُرجع حالة في الطابور/قيد التشغيل/نجح/فشل.
- تُرجع `queue.queuedAhead` و`queue.position` أثناء وجود الطلب في الطابور حتى تتمكن العملاء من عرض عدد الفحوصات اليدوية ذات الأولوية الموجودة قبل الطلب. تُحدَّد الطوابير الكبيرة جداً وتُبلَّغ مع `queuedAheadIsEstimate: true`.
...
نقطة أرشيف تقرير مصادقة.

- تتطلب فحصاً ناجحاً؛ الفحوصات غير النهائية تُرجع `409`.
...
نقطة أرشيف تقرير مخزن ومصادقة للإصدارات المرسلة.

- تتطلب وصول إدارة المالك/الناشر إلى المهارة أو الإضافة، أو صلاحية مشرف/مسؤول المنصة.
- تُرجع نتائج الفحص المخزنة للإصدار المرسل المحدد، بما في ذلك الإصدارات المحظورة أو المخفية.
- القيمة الافتراضية لـ `kind` هي `skill`؛ استخدم `kind=plugin` لفحوصات الإضافات/الحزم.
- تُرجع شكل ZIP نفسه لتنزيلات طلبات الفحص.
...
مسار حالة دفعي قياسي للمسؤولين فقط. يقبل `{ "jobIds": ["..."] }` ويُرجع العدادات التجميعية نفسها مثل المسار القديم `POST /api/v1/skills/-/rescan-batch/status`.
...
- تكون `ok` بالقيمة `true` فقط عندما يكون للإصدار المحدد بطاقة مهارة مولدة، ولا يكون محظوراً كبرمجية خبيثة بواسطة الإشراف، ويكون تحقق ClawScan نظيفاً.
- هوية المهارة، وهوية الناشر، وبيانات الإصدار المحدد الوصفية هي حقول غلاف علوية (`slug` و`displayName` و`publisherHandle` و`version` و`resolvedFrom` و`tag` و`createdAt`) حتى تتمكن أتمتة shell من قراءتها دون فك أغلفة متداخلة.
...
- يتم الاحتفاظ بـ `security.signals.dependencyRegistry` لتوافق استجابة v1، لكن ماسح وجود سجل التبعيات متقاعد وهذا المفتاح يكون دائماً `null`.
...
المجموعة هذه مخصصة للعملاء الذين يعرفون بالفعل أي إصدارات مهارات ClawHub
...
- الاستجابة خاصة بالأمان فقط. لا تتضمن بيانات بطاقة المهارة، أو حالة البطاقة المولدة، أو قوائم ملفات الأثر، أو حمولات الماسحات التفصيلية.
...
- يتم الاحتفاظ بـ `security.signals.dependencyRegistry` لتوافق استجابة v1، لكن ماسح وجود سجل التبعيات متقاعد وهذا المفتاح يكون دائماً `null`.
- لا يؤثر غياب بطاقة المهارة في `ok` أو `decision` أو `reasons` الخاصة بنقطة النهاية هذه؛ يجب أن يقرأ العملاء `skill-card.md` المثبت محلياً عندما يحتاجون إلى محتوى البطاقة.
...
`trending` هو ترتيب تثبيت/تنزيل لسبعة أيام ولا يستخدم الإجماليات منذ البداية.
على نقطة النهاية الموحدة `/api/v1/packages`، هو مخصص لـ Plugin فقط؛ استخدم
...
- `startDate` (مطلوب): حد أدنى بمللي ثانية Unix لـ `updatedAt` في Skill.
- `endDate` (مطلوب): حد أعلى بمللي ثانية Unix لـ `updatedAt` في Skill.
...
- المتن: أرشيف ZIP.
- كل Skill مصدّرة جذرها عند `{publisher}/{slug}/`.
...
- تتضمن Skills الحالية المدعومة من GitHub التي لديها فحص `clean` أو `suspicious`
  ...
  وبصمة المحتوى، ورابط URL للأرشيف. ولا تتضمن ملفات المصدر المستضافة على ClawHub.
...
- تُقبل أيضًا أسماء مرشحات v1 القديمة المستعارة الموثقة ضمن `GET /api/v1/plugins`.
...
- تعيد عناصر تحكم فرز واجهة المتصفح لبحث Plugin ترتيب نتائج الصلة المحمّلة،
...
يعيد الملخص الأمني وملخص الثقة الدقيقين لإصدار الحزمة من أجل عملاء التثبيت.
هذا هو سطح الاستهلاك العام في OpenClaw لتحديد ما إذا كان يمكن تثبيت إصدار
محلول.
...
- تُعيد الحزم الخاصة `404` ما لم يكن المتصل قادرًا على قراءة الناشر المالك.
- نقطة النهاية هذه أضيق عمدًا من نقاط نهاية المراجعة الخاصة بالمالك/المشرف.
  فهي تكشف قرار التثبيت والشرح العام، وليس هويات المبلغين، أو نصوص البلاغات،
...
- هذا هو سطح محلل OpenClaw؛ ويتجنب تخمين تنسيق الأرشيف من رابط مشترك.
...
- تبث إصدارات ClawPack بايتات `.tgz` الدقيقة المحمّلة كـ npm-pack.
...
- تتم تسوية `packageName` كاسم npm؛ ويمكن أن تكون الحزمة مفقودة للترحيلات
  المخطط لها.
- يتتبع هذا جاهزية الترحيل فقط. ولا يغيّر OpenClaw ولا ينشئ ClawPacks.
...
- `open`: إصدارات مريبة أو خبيثة أو معلقة أو معزولة أو ملغاة أو مُبلّغ عنها.
...
أبلِغ عن حزمة لمراجعة المشرف. البلاغات على مستوى الحزمة، وترتبط اختياريًا
بإصدار. وهي تغذي قائمة انتظار المراجعة لكنها لا تخفي التنزيلات أو تحظرها
...
- حد حجم الملف: 200KB.
- لا تمنع فحوصات VirusTotal المعلقة القراءات؛ قد تظل الإصدارات الخبيثة محجوبة في مكان آخر.
...
- أرشيفات Plugin/الحزم هي ملفات zip ذات جذر `package/` حتى تظل عملاء OpenClaw
  القديمة تعمل.
- يبقى هذا المسار مقتصرًا على ZIP. لا يبث ملفات ClawPack `.tgz`.
...
- تُدرج فقط الإصدارات التي تحتوي على كرات tar بصيغة ClawPack npm-pack مرفوعة.
...
- يستخدم حاوية معدل التنزيل.
- تتضمن ترويسات التنزيل SHA-256 الخاص بـ ClawHub إضافة إلى بيانات تعريف integrity/shasum الخاصة بـ npm.
...
ينزل ZIP لإصدار Skill مستضاف، أو يعيد تسليمًا إلى مصدر GitHub لـ Skill
حالي مدعوم من GitHub ذي فحص `clean` أو `suspicious` ومن دون إصدار
مستضاف.
...
- لا تقوم تسليمات Skill المدعومة من GitHub بتمرير البايتات عبر وكيل أو عكسها. تتضمن استجابة JSON
  ...
  و`archiveUrl`؛ حالة الفحص/الحالة الحالية هي بوابة ولا تُضمن كبيانات تعريف حمولة نجاح.
...
- يُقبل أيضًا جسم JSON مع `files` (مستند إلى storageId).
- حقل حمولة اختياري: `ownerHandle`. عند وجوده، تحل API ذلك
  الناشر من جهة الخادم وتتطلب أن يكون لدى الفاعل وصول إلى الناشر.
- حقل حمولة اختياري: `migrateOwner`. عند `true` مع `ownerHandle`، قد
  ...
  الناشرين الحالي والهدف. ومن دون هذا الاشتراك، تُرفض تغييرات المالك.
...
- حقول النموذج المسموح بها هي `payload`، أو كائنات blob مكررة في `files`، أو مرجع كرة tar واحد في `clawpack`.
  قد يكون `clawpack` كائن blob بامتداد `.tgz` أو معرّف تخزين تعيده
  عملية upload-url. يجب أن تتضمن عمليات النشر المرحلية عبر معرّف التخزين أيضًا
...
- تُحدد طلبات النشر المباشر عبر multipart بسقف 18MB. يمكن لكرات tar من ClawPack
  استخدام عملية upload-url حتى سقف كرة tar البالغ 120MB.
...
- تتطلب Plugins البرمجية `package.json`، وبيانات تعريف مستودع المصدر، وبيانات تعريف تثبيت المصدر،
  وبيانات تعريف مخطط الإعداد، و`openclaw.compat.pluginApi`، و
...
- لا تزال عمليات النشر بالنيابة تتحقق من أهلية القناة الرسمية وفق حساب المالك الهدف.

### `DELETE /api/v1/skills/{slug}` / `POST /api/v1/skills/{slug}/undelete`

حذف مبدئي / استعادة Skill (المالك أو المشرف أو المدير).
...
تحجز عمليات الحذف المبدئي التي يبدأها المالك slug لمدة 30 يومًا، ثم يمكن لناشر
آخر المطالبة بالـ slug. تتضمن استجابة الحذف `slugReservedUntil` عندما تنطبق هذه المهلة.
...
ناشر مستخدم/شخصي مشترك قديم، فتنقله نقطة النهاية أولًا إلى ناشر مؤسسة.
بالنسبة إلى مؤسسة منشأة حديثًا، قدّم `memberHandle`؛ لا يُضاف المدير المنفذ كعضو.
يكون `memberRole` افتراضيًا `owner`.
...
إنشاء ناشر مؤسسة بالخدمة الذاتية بعد المصادقة. ينشئ ناشر مؤسسة جديدًا ويضيف
المستدعي كمالك. لا تنقل نقطة النهاية هذه المعرّفات الشخصية/المستخدمين الموجودة ولا
...
- يعيد `409` عندما يكون المعرّف مستخدمًا بالفعل من ناشر أو مستخدم أو ناشر شخصي.

### `POST /api/v1/users/reserve`

للمدير فقط. يحجز slugs الجذرية وأسماء الحزم للمالك الشرعي من دون نشر
إصدار. تصبح أسماء الحزم حزم عناصر نائبة خاصة بلا صفوف إصدار، حتى يتمكن
المالك نفسه لاحقًا من نشر إصدار code-plugin أو bundle-plugin الحقيقي تحت ذلك الاسم.
...
للمدير فقط. يستعيد ناشرًا شخصيًا لأساس GitHub OAuth بديل متحقق منه
من دون تعديل صفوف حساب Convex Auth. يجب أن يذكر الطلب كلا معرّفي حساب
موفر GitHub غير القابلين للتغيير؛ لا تُستخدم المعرّفات القابلة للتغيير إلا كحاجز مواجه للمشغل.

الإعداد الافتراضي لنقطة النهاية هو التشغيل التجريبي. يتطلب تطبيق الاسترداد `dryRun: false` و
...
يرحّل الاسترداد أيضًا حقول `ownerUserId` القديمة لمهارات الناشر المسترد،
وأسماء slug المستعارة للمهارات، والحزم، وتحذيرات مفتش الحزم، وصفوف ملخص البحث المشتقة بحيث
تتوافق مسارات المالك المباشر مع سلطة الناشر الجديدة. ويُعاد أيضًا تعيين حجز المقبض المحمي النشط
للمقبض المسترد إلى المستخدم البديل حتى لا تتمكن مزامنة الملف الشخصي اللاحقة من استعادة السلطة المنافسة للمستخدم السابق. يقتصر كل جدول أساسي على
100 صف لكل معاملة تطبيق؛ ويجب أن تستخدم عمليات الاسترداد الأكبر أولًا ترحيل مالك قابلًا للاستئناف.
مصادر مهارات GitHub محددة النطاق بالناشر ويُبلغ عنها على أنها مفحوصة بدلًا من إعادة كتابتها.
...
رفع حظر مستخدم واستعادة المهارات المؤهلة (للمسؤول فقط).
...
تغيير السبب المخزن لحظر قائم دون رفع الحظر أو استعادة
المحتوى (للمسؤول فقط). يكون الإعداد الافتراضي تشغيلًا تجريبيًا ما لم تكن `dryRun` هي `false`.
...
إضافة نجمة أو إزالتها (تمييزات). كلتا نقطتي النهاية متطابقتا النتيجة عند التكرار.
...
التي تجهز أرشيف ClawPack بصيغة tarball معرّف التخزين الناتج باعتباره
...
إذا كنت تستضيف ذاتيًا، فقدم هذا الملف (أو عيّن `CLAWHUB_REGISTRY` صراحةً؛ والقيمة القديمة `CLAWDHUB_REGISTRY`).
