...
x-i18n:
    generated_at: "2026-07-02T00:55:19Z"
    model: gpt-5.5
...
يمكن للأدلة التابعة لجهات خارجية استخدام نقاط النهاية العامة للقراءة لسرد Skills في ClawHub أو البحث فيها. يُرجى تخزين النتائج مؤقتًا، واحترام `429`/`Retry-After`، وإعادة توجيه المستخدمين إلى قائمة ClawHub الأساسية (`https://clawhub.ai/<owner>/skills/<slug>`)، وتجنب الإيحاء بتأييد ClawHub للموقع التابع لجهة خارجية. لا تحاول عكس المحتوى المخفي أو الخاص أو المحظور بسبب الإشراف خارج سطح API العام.
...
- ينبغي ألا تعيد نقاط نهاية الكتابة المصادق عليها `Unauthorized` مجردة عندما
  ...
  الحسابات المحذوفة/المحظورة/المعطلة على نص قابل للتنفيذ لكل منها كي يتمكن عملاء CLI
...
- استخدم تراجعًا عشوائي التذبذب لتجنب إعادة المحاولات المتزامنة.
- إذا كان `Retry-After` مفقودًا، فارجع إلى `RateLimit-Reset` (أو احسب من `X-RateLimit-Reset`).

مصدر عنوان IP:

- يستخدم رؤوس عنوان IP للعميل الموثوق، بما في ذلك `cf-connecting-ip`، فقط عندما
...
- إذا لم يتوفر عنوان IP موثوق للعميل، تستخدم الطلبات المجهولة حاويات احتياطية
  يقتصر نطاقها على نوع حد المعدل فقط. لا تتضمن هذه الحاويات الاحتياطية
  المسارات أو slugs أو أسماء الحزم أو الإصدارات أو سلاسل الاستعلام أو غيرها من
  معلمات القطع الأثرية التي يقدّمها المستدعي.
...
قراءة جسم الاستجابة كسلسلة نصية قابلة للقراءة من البشر. يتم تجاهل معلمات الاستعلام
غير المعروفة للتوافق، لكن معلمات الاستعلام المعروفة ذات القيم غير الصالحة تعيد
`400`.
...
- تُعاد النتائج بترتيب الصلة (تشابه التضمين + تعزيزات رمز slug/الاسم المطابقة تمامًا + أسبقية شعبية صغيرة).
- الصلة أقوى من الشعبية. يمكن لمطابقة دقيقة لرمز slug أو اسم العرض أن تتقدم على مطابقة أوسع ذات تفاعل أقوى بكثير.
- يُجزأ نص ASCII على حدود الكلمات وعلامات الترقيم. على سبيل المثال، يحتوي `personal-map` على رمز `map` مستقل، بينما يحتوي `amap-jsapi-skill` على `amap` و`jsapi` و`skill`؛ لذلك يمنح البحث عن `map` تطابقًا معجميًا أقوى لـ `personal-map` مقارنةً بـ `amap-jsapi-skill`.
- تُقاس الشعبية بمقياس لوغاريتمي وتُحدد بسقف. يمكن أن تأتي Skills ذات التفاعل العالي في ترتيب أدنى عندما يكون نص الاستعلام أضعف تطابقًا.
...
- تحافظ أسماء إعادة التسمية المستعارة على الحل لعناوين URL القديمة والتثبيتات التي تُحل عبر السجل، لكن ترتيب البحث يعتمد على بيانات Skill الوصفية الأساسية بعد فهرسة إعادة التسمية. تبقى الإحصاءات الحالية مع Skill.
...
- عندما تكون `nonSuspiciousOnly=true`، قد تعيد عمليات الترتيب المعتمدة على المؤشر عناصر أقل من `limit` في الصفحة لأن Skills المشبوهة تُصفى بعد استرجاع الصفحة.
...
- تكون `metadata` هي `null` إذا لم تتضمن Skill بيانات وصفية للمنصة.
- تُضمّن `moderation` فقط عندما تكون Skill مُعلّمة أو عندما يعرضها المالك.
...
- يحصل المستدعون العامون على `200` فقط لـ Skills المرئية والمُعلّمة مسبقًا.
...
إعادة ضبط `status` إلى `open`. مرّر `finalAction: "hide"` مع بلاغ تمت فرزه
...
- تكون `security.hasScanResult` بالقيمة `true` فقط عندما ينتج ماسح حُكمًا نهائيًا (`clean` أو `suspicious` أو `malicious`).
- `moderation` هي لقطة إشراف حالية على مستوى المهارة مشتقة من أحدث إصدار.
- عند الاستعلام عن إصدار تاريخي، تحقّق من `moderation.matchesRequestedVersion` و`moderation.sourceVersion` قبل التعامل مع `moderation` و`security` كسياق الإصدار نفسه.
...
- مهام الفحص غير متزامنة. تُمنح طلبات الفحص اليدوية أولوية على أعمال النشر/الردم العادية، لكن اكتمالها لا يزال يعتمد على توفر العمال.
...
- تُرجع `queue.queuedAhead` و`queue.position` أثناء الانتظار حتى يتمكن العملاء من عرض عدد الفحوصات اليدوية ذات الأولوية المتقدمة على الطلب. تُقيَّد الصفوف الكبيرة جدًا ويُبلَّغ عنها مع `queuedAheadIsEstimate: true`.
...
نقطة أرشيف تقارير مخزنة مصادَق عليها للإصدارات المُرسلة.
...
مسار حالة دفعة قانوني للمسؤولين فقط. يقبل `{ "jobIds": ["..."] }` ويُرجع عدادات التجميع نفسها مثل `POST /api/v1/skills/-/rescan-batch/status` القديم.
...
- تكون `ok` بالقيمة `true` فقط عندما يحتوي الإصدار المحدد على بطاقة مهارة مولَّدة، ولا يكون محظورًا كبرمجية خبيثة عبر الإشراف، ويكون تحقق ClawScan نظيفًا.
...
- `security` هو حُكم ClawScan/الأمان على المستوى الأعلى. يجب أن تعتمد الأتمتة على `ok` و`decision` و`reasons` و`security.status`.
- يحتوي `security.signals` على أدلة ماسحات داعمة مثل `staticScan` و`virusTotal` و`skillSpector`.
- يُحتفظ بـ `security.signals.dependencyRegistry` لتوافق استجابة v1، لكن ماسح وجود سجل الاعتماديات أُحيل إلى التقاعد وهذا المفتاح يكون دائمًا `null`.
- تكون `provenance` بالقيمة `server-resolved-github-import` فقط عندما يحل ClawHub ويخزن مستودع GitHub/مرجعًا/التزامًا/مسارًا أثناء النشر أو الاستيراد؛ وإلا فهي `unavailable`.
...
- الاستجابة خاصة بالأمان فقط. لا تتضمن بيانات بطاقة المهارة، أو حالة البطاقة المولَّدة، أو قوائم ملفات الأثر، أو حمولات الماسح التفصيلية.
...
- استخدم `/verify` عندما تحتاج إلى غلاف تحقق بطاقة المهارة لمهارة واحدة، و`/card` عندما تحتاج إلى Markdown البطاقة المولَّدة، و`/scan` عندما تحتاج إلى بيانات ماسح تفصيلية.
...
نقطة نهاية كتالوج موحدة من أجل:
...
- `category` (اختياري): مرشح فئة الإضافة. مدعوم فقط عندما يكون
  الطلب محدد النطاق إلى حزم الإضافات (`/api/v1/plugins`،
...
- القيم غير الصالحة لـ `family` أو `channel` أو `isOfficial` أو `featured` أو
  `highlightedOnly` أو `sort` ترجع `400`. تُتجاهل معلمات الاستعلام غير المعروفة.
- يبقى `GET /api/v1/code-plugins` و`GET /api/v1/bundle-plugins` اسمين مستعارين ثابتَي العائلة.
- تظل إدخالات المهارات مدعومة بسجل المهارات، ولا يزال نشرها ممكنًا فقط عبر `POST /api/v1/skills`.
...
- لا يرجع `channel=private` إلا الحزم التي يمكن للمتصل المصادق عليه قراءتها.

### `GET /api/v1/packages/search`

بحث موحد في الكتالوج عبر المهارات + حزم الإضافات.
...
- `isOfficial` (اختياري): `true` أو `false`
- `category` (اختياري): مرشح فئة الإضافة. مدعوم فقط عندما يكون
  الطلب محدد النطاق إلى حزم الإضافات. الفئات المضبوطة والأسماء المستعارة القديمة لمرشحات v1
  موثقة ضمن `GET /api/v1/plugins`.

ملاحظات:

- القيم غير الصالحة لـ `family` أو `channel` أو `isOfficial` أو `featured` أو
  `highlightedOnly` ترجع `400`. تُتجاهل معلمات الاستعلام غير المعروفة.
- لا يرى المتصلون المجهولون إلا قنوات الحزم العامة.
...
استعراض كتالوج مخصص للإضافات فقط عبر حزم code-plugin وbundle-plugin.
...
- `category` (اختياري): مرشح فئة الإضافة. القيم الحالية:
...
في نقطة النهاية الموحدة `/api/v1/packages` يكون مخصصًا للإضافات فقط؛ استخدم
`/api/v1/skills?sort=trending` لكتالوج المهارات.

لا تُقبل الأسماء المستعارة القديمة كقيم فئة مخزنة أو معلنة من المؤلف.
...
- `startDate` (مطلوب): الحد الأدنى بمللي ثانية Unix لـ `updatedAt` الخاص بالمهارة.
- `endDate` (مطلوب): الحد الأعلى بمللي ثانية Unix لـ `updatedAt` الخاص بالمهارة.
...
- تتضمن المهارات المستضافة أحدث ملفات الإصدار المخزنة، وتُدرج في
  `_manifest.json` مع `sourceRef: "public-clawhub"`.
- تتضمن المهارات الحالية المدعومة من GitHub ذات فحص `clean` أو `suspicious`
  الملف `_source_handoff.json` مع `sourceRef: "public-github"`، والمستودع، والالتزام، والمسار،
...
- `startDate` (مطلوب): الحد الأدنى بالميلي ثانية وفق Unix لقيمة `updatedAt` في Plugin.
- `endDate` (مطلوب): الحد الأعلى بالميلي ثانية وفق Unix لقيمة `updatedAt` في Plugin.
- `limit` (اختياري): عدد صحيح (1-250)، الافتراضي `250`.
- `cursor` (اختياري): مؤشر ترقيم الصفحات من الاستجابة السابقة.
...
- يتضمن كل Plugin مُصدَّر ملفات أحدث إصدار مخزنة.
...
- يتطلب رمز API لمالك الحزمة، أو مالك/مسؤول ناشر مؤسسة، أو مشرف منصة، أو مسؤول منصة.
...
والتحقق، وبيانات تعريف القطعة الأثرية، وبيانات الفحص.
...
- `version.sha256hash` هي بيانات تعريف توافق مهجورة للعملاء القدامى. وهي
  تجزئ بايتات ZIP الدقيقة المُرجَعة من `/api/v1/packages/{name}/download`.
  ينبغي للعملاء الحديثين استخدام `version.artifact.sha256`، الذي يحدد
  قطعة الإصدار الأثرية القانونية.
...
- تكون `release.artifactKind` و`release.artifactSha256` و`release.npmIntegrity`
  ...
  لقطعة الإصدار الأثرية.
- `trust.scanStatus` هي حالة الثقة الفعلية المستمدة من مُدخلات الماسح
...
- `trust.reasons` هي قائمة الشرح الموجَّهة للمستخدم والتدقيق. رموز الأسباب
  سلاسل مستقرة وموجزة مثل `manual:quarantined` و`scan:malicious`
  و`package:malicious`.
- تعني `trust.pending` أن واحدًا أو أكثر من مُدخلات الثقة لا يزال بانتظار
  الاكتمال.
- تعني `trust.stale` أن ملخص الثقة حُسب من مُدخلات قديمة وينبغي التعامل معه
...
- نقطة النهاية هذه دقيقة على مستوى الإصدار. ينبغي للعملاء استدعاؤها بعد حل
  ...
  للحزمة.
- ترجع الحزم الخاصة `404` ما لم يكن المتصل قادرًا على قراءة الناشر المالك.
- نقطة النهاية هذه أضيق عمدًا من نقاط نهاية الإشراف الخاصة بالمالك/المشرف.
  فهي تكشف قرار التثبيت والشرح العام، وليس هويات المبلّغين، أو نصوص
  البلاغات، أو الأدلة الخاصة، أو الجداول الزمنية الداخلية للمراجعة.

### `GET /api/v1/packages/{name}/versions/{version}/artifact`

يرجع بيانات تعريف محلل القطعة الأثرية الصريح لإصدار حزمة.

ملاحظات:

- ترجع إصدارات الحزم القديمة قطعة أثرية `legacy-zip` و`downloadUrl` قديمًا
  لملف ZIP.
- ترجع إصدارات ClawPack قطعة أثرية `npm-pack`، وحقول تكامل npm، و`tarballUrl`،
  ورابط توافق ZIP القديم.
- هذا هو سطح المحلل في OpenClaw؛ وهو يتجنب تخمين تنسيق الأرشيف من رابط مشترك.
...
يرجع الجاهزية المحسوبة لاستهلاك OpenClaw المستقبلي.
...
- يُطبَّع `packageName` وفق اسم npm؛ يمكن أن تكون الحزمة مفقودة للترحيلات
  المخططة.
...
- `open`: إصدارات مشتبه بها، أو خبيثة، أو معلقة، أو معزولة، أو ملغاة، أو مبلّغ عنها.
...
تحظرها تلقائيًا بحد ذاتها؛ ينبغي للمشرفين استخدام إشراف الإصدارات للموافقة
على القطع الأثرية أو عزلها أو إلغائها.
...
- تُرجع الملفات الثنائية `415`.
- حد حجم الملف: 200KB.
- لا تمنع عمليات فحص VirusTotal المعلقة القراءة؛ وقد تظل الإصدارات الخبيثة محجوبة في أماكن أخرى.
...
- أرشيفات Plugin/الحزمة هي ملفات zip بجذر `package/` حتى يظل عملاء OpenClaw
  القدامى يعملون.
...
- لا تمنع عمليات فحص VirusTotal المعلقة التنزيلات؛ وتُرجع الإصدارات الخبيثة `403`.
- تُرجع الحزم الخاصة `404` ما لم يكن المستدعي هو المالك.
...
- تدعم packuments الحزم ذات النطاق كلا من مساري الطلب `/api/npm/@scope/name` و
  المسار المشفر الخاص بـ npm وهو `/api/npm/@scope%2Fname`.
...
- يستخدم حاوية معدل التنزيل.
- تتضمن ترويسات التنزيل SHA-256 من ClawHub بالإضافة إلى بيانات integrity/shasum الوصفية الخاصة بـ npm.
...
ينزل ZIP لإصدار مهارة مستضافة، أو يعيد تسليما إلى مصدر GitHub
لمهارة حالية مدعومة من GitHub ذات فحص `clean` أو `suspicious` ومن دون
إصدار مستضاف.
...
- تُرجع الإصدارات المحذوفة حذفًا مبدئيا `410`.
- لا تعمل تسليمات المهارات المدعومة من GitHub كوكيل للبايتات ولا تنشئ لها مرآة. تتضمن استجابة JSON
  `sourceRef: "public-github"` و`repo` و`commit` و`path` و`contentHash`
  و`archiveUrl`؛ وتكون حالة الفحص/الحالية بوابة ولا تُضمَّن كبيانات وصفية لحمولة نجاح.
- تُحسب إحصاءات التنزيل كهويات فريدة لكل يوم UTC (`userId` عندما يكون رمز API صالحا، وإلا فعنوان IP).
...
- يُقبل أيضا جسم JSON يحتوي على `files` (مستندة إلى storageId).
- حقل حمولة اختياري: `ownerHandle`. عند وجوده، يحل API ذلك
  الناشر من جانب الخادم ويتطلب أن يمتلك الفاعل صلاحية وصول إلى الناشر.
- حقل حمولة اختياري: `migrateOwner`. عند كونه `true` مع `ownerHandle`، يمكن
  نقل مهارة موجودة إلى ذلك المالك إذا كان الفاعل مسؤولا/مالكا لدى كل من
...
- حقول النموذج المسموح بها هي `payload`، أو كتل `files` مكررة، أو مرجع tarball واحد باسم `clawpack`.
  يمكن أن يكون `clawpack` كتلة `.tgz` أو معرف تخزين أعاده
  سير upload-url. يجب أن تتضمن عمليات النشر بمعرفات التخزين المرحلية أيضا
...
- تُرفض أجسام JSON وبيانات `payload.files` / `payload.artifact`
  الوصفية التي يزودها المستدعي.
- تُحدد طلبات النشر multipart المباشرة بسقف 18MB. ويمكن أن تستخدم tarballs من ClawPack
  سير upload-url حتى سقف tarball البالغ 120MB.
...
- يمكن فقط لناشر مؤسسة `openclaw` وأعضاء مؤسسة `openclaw` الحاليين الذين لديهم
  ناشرون شخصيون النشر إلى قناة `official`.
- لا تزال عمليات النشر بالنيابة تتحقق من أهلية القناة الرسمية مقابل حساب المالك الهدف.

### `DELETE /api/v1/skills/{slug}` / `POST /api/v1/skills/{slug}/undelete`

حذف مبدئي / استعادة مهارة (المالك أو المشرف أو المسؤول).
...
عند وجود `reason`، تُخزَّن كملاحظة إشراف للمهارة وتُنسخ إلى سجل التدقيق.
تحجز عمليات الحذف المبدئي التي يبدأها المالك slug لمدة 30 يوما، ثم يمكن
لناشر آخر المطالبة به. تتضمن استجابة الحذف `slugReservedUntil` عندما ينطبق هذا الانتهاء.
لا تنتهي بهذه الطريقة عمليات الإخفاء بواسطة المشرف/المسؤول وعمليات الإزالة الأمنية.
...
للمسؤولين فقط. يضمن وجود ناشر مؤسسة لاسم handle. إذا كان handle لا يزال يشير إلى
مستخدم مشترك قديم/ناشر شخصي، تنقله نقطة النهاية إلى ناشر مؤسسة أولا.
بالنسبة إلى مؤسسة منشأة حديثا، قدم `memberHandle`؛ ولا يُضاف المسؤول الفاعل كعضو.
...
إنشاء ذاتي الخدمة لناشر مؤسسة مع مصادقة. ينشئ ناشر مؤسسة جديدا ويضيف
المستدعي كمالك. لا تنقل نقطة النهاية هذه handles المستخدمين/الشخصية الحالية ولا
...
- تُرجع `409` عندما يكون handle مستخدما مسبقا بواسطة ناشر أو مستخدم أو ناشر شخصي.

### `POST /api/v1/users/reserve`

للمسؤولين فقط. يحجز slugs الجذرية وأسماء الحزم للمالك الشرعي من دون نشر
إصدار. تصبح أسماء الحزم حزما نائبة خاصة بلا صفوف إصدارات، حتى يستطيع
...
للمسؤولين فقط. يستعيد ناشرا شخصيا لصاحب GitHub OAuth بديل متحقق منه
من دون تحرير صفوف حساب Convex Auth. يجب أن يسمي الطلب كلا معرفي حساب
مزود GitHub غير القابلين للتغيير؛ ولا تُستخدم handles القابلة للتغيير إلا كحاجز يواجه المشغل.
...
كياني GitHub الرئيسيين. يفشل الاسترداد بحالة مغلقة عندما يكون لدى الناشر الشخصي الحالي للمستخدم الوجهة
Skills أو حزم أو مصادر Skills من GitHub.
يرحّل الاسترداد أيضا حقول `ownerUserId` القديمة الخاصة بـ Skills للناشر المسترد،
وأسماء slug المستعارة لـ Skills، والحزم، وتحذيرات فاحص الحزم، وصفوف ملخص البحث المشتقة بحيث
تتوافق مسارات المالك المباشر مع سلطة الناشر الجديدة. كما يعاد تعيين حجز protected-handle
النشط للمعرّف المسترد إلى المستخدم البديل، بحيث لا يمكن لمزامنة الملف الشخصي لاحقا
استعادة سلطة المستخدم السابق المنافسة. يقتصر كل جدول أساسي على
100 صف لكل معاملة تطبيق؛ ويجب أن تستخدم عمليات الاسترداد الأكبر أولا ترحيلا قابلا للاستئناف للمالك.
مصادر Skills من GitHub محددة النطاق بالناشر ويتم الإبلاغ عنها كمفحوصة بدلا من إعادة كتابتها.

- نص الطلب: `{ "handle": "gingiris", "nextUserHandle": "gingiris-1031", "previousGitHubProviderAccountId": "123", "nextGitHubProviderAccountId": "456", "reason": "Verified account continuity for issue #2555", "confirmIdentityVerified": true, "dryRun": false }`
...
- `POST /api/v1/skills/{slug}/rename`
  - نص الطلب: `{ "newSlug": "new-canonical-slug" }`
  - الاستجابة: `{ "ok": true, "slug": "new-canonical-slug", "previousSlug": "old-slug" }`
- `POST /api/v1/skills/{slug}/merge`
  - نص الطلب: `{ "targetSlug": "canonical-target-slug" }`
...
- `POST /api/v1/skills/{slug}/transfer`
  - نص الطلب: `{ "toUserHandle": "target_handle", "message": "optional" }`
...
- `POST /api/v1/skills/{slug}/transfer/cancel`
  - الاستجابة (accept/reject/cancel): `{ "ok": true, "skillSlug": "demo-skill?" }`
...
حظر مستخدم وحذف Skills المملوكة نهائيا (للمشرف/المسؤول فقط).
...
المحتوى (للمسؤول فقط). يكون افتراضيا في وضع التشغيل التجريبي ما لم تكن `dryRun` هي `false`.
...
التي تجهز أرشيف ClawPack أن ترسل معرّف التخزين الناتج كـ
`clawpack` والتذكرة المرجعة كـ `clawpackUploadTicket`.
...
إذا كنت تستضيف ذاتيا، فقدم هذا الملف (أو عيّن `CLAWHUB_REGISTRY` صراحة؛ القيمة القديمة `CLAWDHUB_REGISTRY`).
