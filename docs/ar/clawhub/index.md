...
read_when:
    ...
    - نشر المهارات أو الإضافات إلى السجل
...
title: ClawHub
x-i18n:
    generated_at: "2026-07-03T13:32:12Z"
    model: gpt-5.5
...
| Plugins حِزم   | حِزم Plugin معبأة لتوزيع OpenClaw                            | `clawhub package publish <source>`           |
...
الوصفية للمصدر حتى تتمكن أوامر التحديث اللاحقة من البقاء على ClawHub.

استخدم `clawhub:<package>` عندما يجب أن يتم حل تثبيت Plugin عبر ClawHub.
قد تُحل مواصفات Plugins الآمنة لـ npm عبر npm أثناء مراحل التحويل عند الإطلاق، ويبقى
...
يفضّل OpenClaw ملف `.tgz` المطابق تمامًا والمرفوع بصيغة npm-pack، ويتحقق من
ترويسة ملخص ClawHub والبايتات المنزّلة، ويسجل البيانات الوصفية للأثر
للتحديثات اللاحقة.
...
يجب أن تتضمن Plugins البرمجية بيانات التوافق الوصفية المطلوبة من OpenClaw في
...
و[تنسيق Skill](/clawhub/skill-format) للبيانات الوصفية للـ Skill.
...
يشغّل ClawHub فحوصات آلية على Skills المنشورة وإصدارات Plugin. قد تختفي الإصدارات
المعلقة للفحص أو المحظورة من الكتالوج العام وأسطح التثبيت، مع بقائها
...
[الاستخدام المقبول](/clawhub/acceptable-usage) لتفاصيل السياسة والتنفيذ.

## القياسات عن بُعد والبيئة

عند تشغيل `clawhub install` وأنت مسجل الدخول، قد يرسل CLI حدث تثبيت بأفضل جهد
حتى يتمكن ClawHub من حساب إجمالي عدد عمليات التثبيت. عطّل ذلك باستخدام:
...
| `CLAWHUB_REGISTRY`            | يتجاوز URL واجهة API للسجل.                      |
...
راجع [القياسات عن بُعد](/clawhub/telemetry)، و[واجهة HTTP API](/clawhub/http-api)، و
