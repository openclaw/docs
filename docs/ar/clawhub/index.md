...
summary: نظرة عامة موجهة للجمهور حول ClawHub للاكتشاف والتثبيت والنشر والأمان وواجهة CLI الخاصة بـ clawhub.
title: ClawHub
x-i18n:
...
ثبّت ClawHub CLI عندما تريد سير عمل موثّقة في السجل مثل
...
استخدم `clawhub:<package>` عندما ينبغي أن يُحلّ تثبيت Plugin عبر ClawHub.
...
المعلَنَين قبل تشغيل تثبيت الأرشيف. عندما ينشر إصدار حزمة أداة ClawPack، يفضّل
OpenClaw ملف `.tgz` المطابق المرفوع من npm-pack، ويتحقق من رأس بصمة ClawHub
والبايتات التي تم تنزيلها، ويسجل بيانات تعريف الأداة للتحديثات اللاحقة.

## ClawHub CLI

ClawHub CLI مخصص للأعمال الموثّقة في السجل:
...
الإصدارات الموقوفة بسبب الفحص أو المحظورة من أسطح الفهرس العام والتثبيت، مع
...
عند تشغيل `clawhub sync` وأنت مسجل الدخول، يرسل CLI لقطة بسيطة حتى
...
| `CLAWHUB_REGISTRY`            | تجاوز عنوان URL لـ API السجل.                    |
| `CLAWHUB_CONFIG_PATH`         | تجاوز المكان الذي يخزّن فيه CLI حالة الرمز/الإعدادات. |
