...
summary: 'مرجع CLI: الأوامر، والرايات، والإعدادات، وملف القفل، وسلوك المزامنة.'
x-i18n:
...
يخزن رمز API المميز الخاص بك + عنوان URL المخزن مؤقتًا للسجل.
...
- الافتراضي: يفتح المتصفح على `<site>/cli/auth` ويُكمل عبر رد اتصال الحلقة المحلية.
- بدون واجهة رسومية: `clawhub login --token clh_...`
- تفاعلي عن بُعد/بدون واجهة رسومية: يطبع `clawhub login --device` رمزًا وينتظر بينما تصرّح له عند `<site>/cli/device`.

### `whoami`

- يتحقق من الرمز المميز المخزن عبر `/api/v1/whoami`.
...
- التنزيلات عامل شعبية أولي صغير، وليست ضمانًا للترتيب الأعلى.
...
- يرفض الكتابة فوق المهارات المثبتة بدبوس؛ شغّل `clawhub unpin <slug>` أولًا.
- يكتب:
...
- يضع علامة مثبّتة على Skill مثبّتة في ملف القفل.
...
- يفشل `update <slug>` بسرعة مع slugs المثبّتة ويطلب منك تشغيل `clawhub unpin <slug>` أولًا.
...
- Skills المنشورة مجانية للاستخدام والتعديل وإعادة التوزيع بدون نسب.
...
- يضيف `--clawscan-note <text>` ملاحظة ClawScan. تمنح هذه الملاحظة ClawScan
  ...
  أو الوصول إلى المضيف الأصلي، أو بيانات الاعتماد الخاصة بمزوّد معين. تُخزّن الملاحظة على
  الإصدار المنشور.
...
- يحذف Skill حذفًا ناعمًا (مالك، مشرف، أو مسؤول).
- يستدعي `DELETE /api/v1/skills/{slug}`.
- تحتفظ عمليات الحذف الناعم التي يبدأها المالك بـ slug لمدة 30 يومًا؛ يطبع الأمر وقت انتهاء الصلاحية.
...
- يستعيد Skill مخفية (مالك، مشرف، أو مسؤول).
...
- يخفي Skill (مالك، مشرف، أو مسؤول).
- اسم مستعار لـ `delete`.

### `unhide <slug>`

- يلغي إخفاء Skill (مالك، مشرف، أو مسؤول).
...
- يدمج Skill مملوكة واحدة في Skill مملوكة أخرى.
...
- استخدم هذا لـ plugins وإدخالات عائلات الحزم الأخرى؛ يظل `search` في المستوى الأعلى سطح البحث في Skills.
- العلامات:
...
- بالنسبة لآثار ClawPack npm-pack، يتحقق أيضًا من سلامة npm `sha512`،
...
- مع علامات البصمة المباشرة، يتحقق بدون بحث شبكي.
- العلامات:
...
- يتطلب وصول مسؤول إلى كلٍ من مالك الحزمة الحالي والناشر الوجهة،
  ما لم يُنفَّذ بواسطة مسؤول المنصة.
...
- أمر موثَّق للإبلاغ عن حزمة إلى المشرفين.
...
- يعرض حالة فحص الحزمة الحالية، وعدد البلاغات المفتوحة، وحالة الإشراف اليدوي
  لآخر إصدار، وحالة حظر التنزيل، وأسباب الإشراف.
- العلامات:
  - `--json`: مخرجات قابلة للقراءة آليًا.
...
- يبلّغ عن العوائق المتعلقة بالحالة الرسمية، وتوافر ClawPack، وبصمة الأثر،
...
- يعرض حالة الترحيل الموجهة للمشغلين لحزمة قد تستبدل Plugin مضمنًا
  في OpenClaw.
...
- يقبل `<source>`:
  ...
  - URL لـ GitHub: `https://github.com/owner/repo`
...
- تُعامل مصادر `.tgz` بصفتها ClawPack. يرفع CLI بايتات npm-pack المطابقة
  ويستخدم محتويات `package/` المستخرجة فقط للتحقق وملء بيانات التعريف مسبقًا.
- تُحزم مجلدات Plugin البرمجية في أرشيف ClawPack npm tarball قبل الرفع كي
  تتمكن تثبيتات OpenClaw من التحقق من الأثر المطابق. لا تزال مجلدات Plugin
  الحزم تستخدم مسار النشر بالملفات المستخرجة.
- بالنسبة إلى مصادر GitHub، تُملأ نسبة المصدر تلقائيًا من المستودع، والالتزام المحلول، والمرجع، والمسار الفرعي.
- بالنسبة إلى المجلدات المحلية، تُكتشف نسبة المصدر تلقائيًا من git المحلي عندما يشير أصل المصدر البعيد إلى GitHub.
- يجب أن تعلن إضافات Plugin البرمجية الخارجية `openclaw.compat.pluginApi` و
  `openclaw.build.openclawVersion` صراحةً.
  لا يُستخدم `package.json.version` في المستوى الأعلى كبديل احتياطي للتحقق من النشر.
- يعرض `--dry-run` حمولة النشر المحلولة دون رفع.
- يُصدر `--json` مخرجات قابلة للقراءة آليًا من أجل CI.
- ينشر `--owner <handle>` تحت معرّف ناشر مستخدم أو مؤسسة عندما يكون لدى الفاعل وصول ناشر.
- يضيف `--clawscan-note <text>` ملاحظة ClawScan. تمنح هذه الملاحظة ClawScan
  سياقًا لسلوك قد يبدو غير معتاد بخلاف ذلك، مثل الوصول إلى الشبكة،
  أو الوصول الأصلي إلى المضيف، أو بيانات اعتماد خاصة بالمزوّد. تُخزن الملاحظة في
  الإصدار المنشور.
- يجب أن تطابق أسماء الحزم ذات النطاق المالك المحدد. راجع `docs/publishing.md`.
- لا تزال العلامات الموجودة (`--family` و`--name` و`--version` و`--source-repo` و`--source-commit` و`--source-ref` و`--source-path`) تعمل كتجاوزات.
...
بالنسبة إلى إضافات Plugin البرمجية، يبني نشر المجلد أثر ClawPack من
مجلد الحزمة ويرفعه:
...
تحتاج إضافات Plugin البرمجية الخارجية إلى قدر صغير من بيانات تعريف OpenClaw في
...
- إذا كنت تستخدم إصدارًا أقدم من CLI `clawhub`، فقم بالترقية قبل النشر حتى
  تُشغَّل فحوصات ما قبل الرفع المحلية قبل الرفع.
...
- يجب أن يستخدم `pull_request` القيمة `dry_run: true` حتى يظل CI غير ملوِّث.
...
- يضيف جذور Skills الخاصة بـ Clawdbot تلقائيًا عند وجود `~/.clawdbot/clawdbot.json`:
...
- يُرسل أثناء `sync` عند تسجيل الدخول، ما لم تكن `CLAWHUB_DISABLE_TELEMETRY=1` (القديمة `CLAWDHUB_DISABLE_TELEMETRY=1`).
