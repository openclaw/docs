...
x-i18n:
    generated_at: "2026-07-04T10:39:45Z"
    model: gpt-5.5
...
يمكنك بناء فهرس أو دليل أو واجهة بحث تابعة لجهة خارجية فوق واجهات API العامة للقراءة في ClawHub. تُنشر بيانات Skills الوصفية العامة وملفات Skills بموجب قواعد ترخيص Skills في ClawHub، بينما تخضع API نفسها لحدود معدل ويجب استخدامها بمسؤولية.

الإرشادات:

- استخدم نقاط نهاية القراءة العامة مثل `GET /api/v1/skills` و`GET /api/v1/search` و`GET /api/v1/skills/{slug}` لقوائم الفهرس.
- خزّن الاستجابات مؤقتا واحترم ترويسات `429` و`Retry-After` وحدود المعدل بدلا من الاستطلاع المكثف.
...
- لا توح بأن ClawHub يؤيد أو يتحقق من أو يشغل موقع الجهة الخارجية.
- لا تنسخ محتوى مخفيا أو خاصا أو محظورا بالإشراف عبر تجاوز مرشحات API العامة أو حدود المصادقة.
...
- الرمز المفقود/غير الصالح يعود إلى إنفاذ حسب عنوان IP.

- قراءة: 3000/دقيقة لكل عنوان IP، 12000/دقيقة لكل مفتاح
- كتابة: 300/دقيقة لكل عنوان IP، 3000/دقيقة لكل مفتاح
- تنزيل: 1200/دقيقة لكل عنوان IP، 6000/دقيقة لكل مفتاح
...
- `X-RateLimit-Remaining` / `RateLimit-Remaining`: الميزانية المتبقية الدقيقة عند
  وجودها؛ الطلبات الناجحة الموزعة على الشظايا تحذفها بدلا من إرجاع قيمة
  عالمية تقريبية
...
- `GET /api/v1/skills?limit=&cursor=&sort=`
  - `sort`: `updated` (افتراضي)، `recommended` (`default`)، `createdAt` (`newest`)، `downloads`، `stars` (`rating`)، أسماء التثبيت البديلة القديمة `installsCurrent`/`installs`/`installsAllTime` تُطابق `downloads`، `trending`
...
- `GET /api/v1/download?slug=&version=&tag=`
  - ترجع Skills المستضافة بايتات ZIP حتمية.
  - ترجع Skills الحالية المدعومة من GitHub التي لديها فحص `clean` أو `suspicious`
    واصف تسليم JSON باسم `public-github` بدلا من بايتات ClawHub.
- `GET /api/v1/skills/export?startDate=&endDate=&limit=&cursor=`
  - يتم تصدير Skills المستضافة كملفات مخزنة.
  - يتم تصدير Skills الحالية المدعومة من GitHub التي لديها فحص `clean` أو `suspicious`
    كواصفات تسليم `public-github`.
- `GET /api/v1/packages?limit=&cursor=&sort=`
  - `sort`: `updated` (افتراضي)، `recommended`، `downloads`، الاسم البديل القديم `installs`
  - قيم `sort` غير الصالحة ترجع `400`
- `GET /api/v1/plugins?limit=&cursor=&sort=`
  - `sort`: `recommended` (افتراضي)، `downloads`، `updated`، الاسم البديل القديم `installs`
...
- `POST /api/v1/skills` (نشر، يفضّل multipart)
