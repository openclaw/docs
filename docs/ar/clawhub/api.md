...
يمكنك بناء فهرس أو دليل أو واجهة بحث تابعة لجهة خارجية فوق واجهات API العامة للقراءة في ClawHub. تُنشر بيانات تعريف Skills العامة وملفات Skills بموجب قواعد ترخيص Skills الخاصة بـ ClawHub، بينما تخضع واجهة API نفسها لحدود معدل ويجب استهلاكها بمسؤولية.

الإرشادات:

- استخدم نقاط نهاية القراءة العامة مثل `GET /api/v1/skills` و`GET /api/v1/search` و`GET /api/v1/skills/{slug}` لقوائم الفهرس.
- خزّن الاستجابات مؤقتًا واحترم رؤوس `429` و`Retry-After` وحدود المعدل بدلًا من الاستقصاء المكثف.
...
- لا تنسخ محتوى مخفيًا أو خاصًا أو محظورًا بالاعتدال عبر تجاوز مرشحات واجهة API العامة أو حدود المصادقة.

## المصادقة

- القراءة العامة: لا يتطلب رمزًا مميزًا.
...
الرؤوس: `X-RateLimit-Limit`، `X-RateLimit-Reset`، `RateLimit-Limit`، `RateLimit-Reset`؛
تُضمَّن `X-RateLimit-Remaining` و`RateLimit-Remaining` و`Retry-After` عند `429`.

الدلالات:

- `X-RateLimit-Reset`: ثواني Unix epoch (وقت إعادة التعيين المطلق)
...
- يتم تجاهل معاملات الاستعلام غير المعروفة من أجل التوافق.
...
- `GET /api/v1/download?slug=&version=&tag=`
  ...
  - Skills الحالية المدعومة من GitHub ذات فحص `clean` أو `suspicious` تُرجع
    واصف تسليم JSON باسم `public-github` بدلًا من بايتات ClawHub.
- `GET /api/v1/skills/export?startDate=&endDate=&limit=&cursor=`
  - تُصدَّر Skills المستضافة كملفات مخزنة.
  - Skills الحالية المدعومة من GitHub ذات فحص `clean` أو `suspicious` تُصدَّر
...
- `POST /api/v1/users/reserve` يحجز slugs جذرية وعناصر نائبة خاصة لحزم بلا إصدارات لمعرّف مالك.
