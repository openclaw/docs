...
x-i18n:
    generated_at: "2026-07-03T13:31:36Z"
    model: gpt-5.5
...
يمكنك بناء فهرس أو دليل أو واجهة بحث تابعة لجهة خارجية فوق واجهات API العامة للقراءة في ClawHub. تُنشر بيانات Skills الوصفية العامة وملفات Skills بموجب قواعد ترخيص Skills في ClawHub، بينما تكون API نفسها محدودة المعدل ويجب استهلاكها بمسؤولية.
...
- لا تعكس محتوى مخفيًا أو خاصًا أو محظورًا بالاعتدال عبر تجاوز مرشحات API العامة أو حدود المصادقة.
...
- `X-RateLimit-Remaining` / `RateLimit-Remaining`: الميزانية المتبقية الدقيقة عند
  وجودها؛ تحذف الطلبات الناجحة المقسّمة ذلك بدلًا من إرجاع قيمة عالمية تقريبية
...
- أضف تباينًا عشوائيًا صغيرًا إلى إعادة المحاولة.
...
- `GET /api/v1/skills?limit=&cursor=&sort=`
  - `sort`: `updated` (الافتراضي)، `recommended` (`default`)، `createdAt` (`newest`)، `downloads`، `stars` (`rating`)، أسماء التثبيت المستعارة القديمة `installsCurrent`/`installs`/`installsAllTime` تُطابق `downloads`، `trending`
  - قيم `sort` غير الصالحة تُرجع `400`
  - ينطبق `cursor` على الفرز غير `trending`
...
- `GET /api/v1/download?slug=&version=&tag=`
  ...
  - Skills الحالية المدعومة من GitHub مع فحص `clean` أو `suspicious` تُرجع واصف تسليم
    JSON `public-github` بدلًا من بايتات ClawHub.
- `GET /api/v1/skills/export?startDate=&endDate=&limit=&cursor=`
  - تُصدَّر Skills المستضافة كملفات مخزنة.
  - Skills الحالية المدعومة من GitHub مع فحص `clean` أو `suspicious` تُصدَّر
    كواصفات تسليم `public-github`.
- `GET /api/v1/packages?limit=&cursor=&sort=`
  - `sort`: `updated` (الافتراضي)، `recommended`، `downloads`، اسم مستعار قديم `installs`
  - قيم `sort` غير الصالحة تُرجع `400`
- `GET /api/v1/plugins?limit=&cursor=&sort=`
  - `sort`: `recommended` (الافتراضي)، `downloads`، `updated`، اسم مستعار قديم `installs`
...
- `POST /api/v1/users/reserve` يحجز مقاطع الجذر النمطية وعناصر نائبة خاصة لحزم بلا إصدار لمعرّف مالك.
