...
summary: نظرة عامة على واجهة REST API العامة (v1) والاصطلاحات المتبعة.
x-i18n:
...
يمكنك بناء فهرس أو دليل أو واجهة بحث تابعة لجهة خارجية فوق واجهات API العامة للقراءة في ClawHub. تُنشر بيانات المهارات الوصفية العامة وملفات المهارات بموجب قواعد ترخيص المهارات في ClawHub، بينما تخضع API نفسها لحدود معدل ويجب استهلاكها بمسؤولية.
...
- أضف رابطًا إلى عنوان URL الأصلي لمهارة ClawHub عند عرض القوائم حتى يتمكن المستخدمون من فحص سجل السجل المصدر.
- استخدم عناوين URL الأصلية للصفحات بالصيغة `https://clawhub.ai/<owner>/skills/<slug>`.
- لا توحِ بأن ClawHub يؤيد الموقع الخارجي أو يتحقق منه أو يشغله.
...
- `GET /api/v1/skills?limit=&cursor=&sort=`
  - `sort`: `updated` (الافتراضي)، `recommended` (`default`)، `createdAt` (`newest`)، `downloads`، `stars` (`rating`)، أسماء التثبيت البديلة القديمة `installsCurrent`/`installs`/`installsAllTime` تُعيَّن إلى `downloads`، `trending`
