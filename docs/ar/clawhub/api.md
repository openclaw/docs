...
x-i18n:
    generated_at: "2026-07-01T15:24:43Z"
    model: gpt-5.5
...
يمكنك بناء كتالوج أو دليل أو واجهة بحث تابعة لجهة خارجية فوق واجهات API العامة للقراءة في ClawHub. تُنشر بيانات skill الوصفية العامة وملفات skill بموجب قواعد ترخيص skill في ClawHub، بينما تخضع API نفسها لحدود معدل الطلبات ويجب استهلاكها بمسؤولية.
...
- اربط بعنوان URL الأساسي لـ skill في ClawHub عند عرض القوائم حتى يتمكن المستخدمون من فحص سجل السجل المصدري.
...
- يعود الرمز المفقود/غير الصالح إلى إنفاذ عنوان IP.
...
- `X-RateLimit-Remaining` / `RateLimit-Remaining`: الميزانية المتبقية الدقيقة عند
  وجودها؛ وتحذف الطلبات الناجحة الموزعة على أجزاء هذه القيمة بدلًا من إرجاع قيمة
  عامة تقريبية
...
- يحجز `POST /api/v1/users/reserve` معرفات slug الجذرية والعناصر النائبة لحزم خاصة بلا إصدار لمعرّف مالك.
