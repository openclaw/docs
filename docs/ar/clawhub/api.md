...
x-i18n:
    generated_at: "2026-07-01T08:03:35Z"
    model: gpt-5.5
...
يمكنك بناء فهرس أو دليل أو سطح بحث تابع لجهة خارجية فوق واجهات API العامة للقراءة في ClawHub. تُنشر بيانات تعريف Skills العامة وملفات Skills بموجب قواعد ترخيص Skills في ClawHub، بينما تخضع API نفسها لحدود المعدل ويجب استهلاكها بمسؤولية.
...
- اربط مرة أخرى بعنوان URL القانوني لـ Skill في ClawHub عند عرض القوائم حتى يتمكن المستخدمون من فحص سجل السجل المصدر.
...
- يعود الرمز المفقود/غير الصالح إلى الفرض حسب IP.
...
- `GET /api/v1/skills?limit=&cursor=&sort=`
  - `sort`: `updated` (الافتراضي)، و`recommended` (`default`)، و`createdAt` (`newest`)، و`downloads`، و`stars` (`rating`)، وأسماء التثبيت المستعارة القديمة `installsCurrent`/`installs`/`installsAllTime` تُعيَّن إلى `downloads`، و`trending`
...
- يحجز `POST /api/v1/users/reserve` slugs الجذرية والعناصر النائبة الخاصة لحزم no-release لمعرّف المالك.
