...
x-i18n:
    generated_at: "2026-07-04T03:48:23Z"
    model: gpt-5.5
...
يمكنك بناء فهرس أو دليل أو واجهة بحث تابعة لجهة خارجية فوق واجهات API العامة للقراءة في ClawHub. تُنشر بيانات Skills الوصفية العامة وملفات Skills بموجب قواعد ترخيص Skills في ClawHub، بينما تخضع واجهة API نفسها لحدود معدل ويجب استهلاكها بمسؤولية.
...
- أضف رابطًا عائدًا إلى عنوان URL الأساسي لـ Skills في ClawHub عند عرض القوائم حتى يتمكن المستخدمون من فحص سجل السجل المصدري.
...
- القراءة العامة: لا يلزم أي رمز مميز.
...
- أضف jitter إلى إعادة المحاولات.
...
- `GET /api/v1/skills?limit=&cursor=&sort=`
  - `sort`: `updated` (الافتراضي)، `recommended` (`default`)، `createdAt` (`newest`)، `downloads`، `stars` (`rating`)، الأسماء البديلة القديمة للتثبيت `installsCurrent`/`installs`/`installsAllTime` تُطابَق مع `downloads`، و`trending`
...
- `POST /api/v1/users/reserve` يحجز slugs الجذرية وعناصر نائبة خاصة للحزم بلا إصدار لمعرّف مالك.
