...
x-i18n:
    generated_at: "2026-07-02T22:33:16Z"
    model: gpt-5.5
...
يمكنك بناء فهرس أو دليل أو واجهة بحث تابعة لجهة خارجية فوق واجهات API العامة للقراءة في ClawHub. تُنشر بيانات Skills الوصفية العامة وملفات Skills بموجب قواعد ترخيص Skills في ClawHub، بينما تخضع API نفسها لحدود المعدل وينبغي استهلاكها بمسؤولية.
...
- التنزيل: 1200/دقيقة لكل IP، 6000/دقيقة لكل مفتاح
...
- `X-RateLimit-Remaining` / `RateLimit-Remaining`: الميزانية المتبقية الدقيقة عند
  وجودها؛ الطلبات الناجحة المجزأة تتجنب تضمينها بدلًا من إرجاع قيمة عالمية
  تقريبية
...
- `GET /api/v1/skills?limit=&cursor=&sort=`
  - `sort`: `updated` (الافتراضي)، `recommended` (`default`)، `createdAt` (`newest`)، `downloads`، `stars` (`rating`)، أسماء التثبيت القديمة البديلة `installsCurrent`/`installs`/`installsAllTime` تُطابق `downloads`، `trending`
...
- `GET /api/v1/download?slug=&version=&tag=`
  ...
  - Skills الحالية المدعومة من GitHub والتي لديها فحص `clean` أو `suspicious` تُرجع
    واصف تسليم JSON `public-github` بدلًا من بايتات ClawHub.
- `GET /api/v1/skills/export?startDate=&endDate=&limit=&cursor=`
  - تُصدّر Skills المستضافة كملفات مخزنة.
  - Skills الحالية المدعومة من GitHub والتي لديها فحص `clean` أو `suspicious` تُصدّر
...
- `POST /api/v1/users/reserve` يحجز slugs الجذرية والعناصر النائبة الخاصة لحزم بلا إصدار لمعرّف المالك.
