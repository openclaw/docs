...
يمكنك بناء فهرس أو دليل أو واجهة بحث تابعة لجهة خارجية فوق واجهات API العامة للقراءة في ClawHub. تُنشر بيانات تعريف المهارات العامة وملفات المهارات بموجب قواعد ترخيص المهارات في ClawHub، بينما تخضع API نفسها لحدود معدلات الطلبات ويجب استهلاكها بمسؤولية.
...
- اربط بعنوان URL القانوني للمهارة في ClawHub عند عرض القوائم حتى يتمكن المستخدمون من فحص سجل السجلّ المصدر.
- استخدم عناوين URL القانونية للصفحات بالشكل `https://clawhub.ai/<owner>/skills/<slug>`.
- لا توحِ بأن ClawHub يصادق على موقع الجهة الخارجية أو يتحقق منه أو يشغّله.
- لا تعكس المحتوى المخفي أو الخاص أو المحظور إشرافيًا عبر تجاوز مرشحات API العامة أو حدود المصادقة.
...
- `X-RateLimit-Remaining` / `RateLimit-Remaining`: الميزانية المتبقية الدقيقة عند
  وجودها؛ تحذف الطلبات الناجحة الموزعة على أجزاء هذه القيمة بدلًا من إرجاع قيمة
  عالمية تقريبية
...
- تعيد معاملات الاستعلام المعروفة ذات القيم غير الصالحة `400`.
...
- `GET /api/v1/skills?limit=&cursor=&sort=`
  - `sort`: `updated` (افتراضي)، `recommended` (`default`)، `createdAt` (`newest`)، `downloads`، `stars` (`rating`)، أسماء تثبيت بديلة قديمة `installsCurrent`/`installs`/`installsAllTime` تُطابق إلى `downloads`، `trending`
  - قيم `sort` غير الصالحة تعيد `400`
  - يطبَّق `cursor` على أنواع الفرز غير `trending`
...
- `GET /api/v1/download?slug=&version=&tag=`
  - تعيد المهارات المستضافة بايتات ZIP حتمية.
  - المهارات الحالية المدعومة من GitHub ذات فحص `clean` أو `suspicious` تعيد
...
- `POST /api/v1/skills` (نشر، ويفضَّل متعدد الأجزاء)
...
- `POST /api/v1/users/reserve` يحجز المقاطع الجذرية النمطية وعناصر نائب الحزم الخاصة بلا إصدار لمعرّف مالك.
