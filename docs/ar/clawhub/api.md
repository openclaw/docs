...
يمكنك بناء فهرس أو دليل أو واجهة بحث تابعة لجهة خارجية فوق واجهات API العامة للقراءة في ClawHub. تُنشر بيانات Skills الوصفية العامة وملفات Skills بموجب قواعد ترخيص Skills في ClawHub، بينما تخضع واجهة API نفسها لحدود معدل الاستخدام ويجب استهلاكها بمسؤولية.

الإرشادات:

- استخدم نقاط نهاية القراءة العامة مثل `GET /api/v1/skills` و`GET /api/v1/search` و`GET /api/v1/skills/{slug}` لقوائم الفهرس.
- خزّن الاستجابات مؤقتًا واحترم رؤوس `429` و`Retry-After` وحدود معدل الاستخدام بدلًا من الاستطلاع المكثف.
...
- `X-RateLimit-Remaining` / `RateLimit-Remaining`: الميزانية المتبقية الدقيقة عند
  وجودها؛ الطلبات الناجحة المجزأة تتجاهلها بدلًا من إرجاع قيمة عالمية
  تقريبية
...
- أضف عشوائية بسيطة إلى إعادات المحاولة.
...
- `GET /api/v1/skills?limit=&cursor=&sort=`
  - `sort`: `updated` (الافتراضي)، `recommended` (`default`)، `createdAt` (`newest`)، `downloads`، `stars` (`rating`)، أسماء التثبيت المستعارة القديمة `installsCurrent`/`installs`/`installsAllTime` تُطابق `downloads`، و`trending`
...
- `POST /api/v1/users/reserve` يحجز معرفات slug الجذرية وعناصر نائبة لحزم خاصة بلا إصدارات لمعرّف المالك.
