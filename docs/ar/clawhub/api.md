...
يمكنك بناء فهرس أو دليل أو واجهة بحث تابعة لجهة خارجية فوق واجهات API العامة للقراءة في ClawHub. تُنشر بيانات المهارات الوصفية العامة وملفات المهارات وفق قواعد ترخيص المهارات في ClawHub، بينما تخضع واجهة API نفسها لحدود معدل ويجب استخدامها بمسؤولية.
...
- لا توحِ بأن ClawHub يزكي أو يتحقق من أو يشغل موقع الجهة الخارجية.
- لا تنسخ المحتوى المخفي أو الخاص أو المحظور بالرقابة عبر تجاوز مرشحات API العامة أو حدود المصادقة.
...
- فضّل `Retry-After` عند وجودها.
- وإلا فاستخدم `RateLimit-Reset` أو استنتج التأخير من `X-RateLimit-Reset`.
- أضف تذبذبًا عشوائيًا إلى إعادة المحاولات.
...
- `GET /api/v1/skills?limit=&cursor=&sort=`
  - `sort`: `updated` (الافتراضي)، `recommended` (`default`)، `createdAt` (`newest`)، `downloads`، `stars` (`rating`)، أسماء التثبيت القديمة المستعارة `installsCurrent`/`installs`/`installsAllTime` تُعيَّن إلى `downloads`، و`trending`
