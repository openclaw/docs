...
يمكنك بناء كتالوج أو دليل أو واجهة بحث تابعة لجهة خارجية فوق واجهات API العامة للقراءة في ClawHub. تُنشر بيانات تعريف Skills العامة وملفات Skills بموجب قواعد ترخيص Skills في ClawHub، بينما تكون واجهة API نفسها محدودة المعدل ويجب استخدامها بمسؤولية.

الإرشادات:

- استخدم نقاط نهاية القراءة العامة مثل `GET /api/v1/skills` و`GET /api/v1/search` و`GET /api/v1/skills/{slug}` لقوائم الكتالوج.
- خزّن الاستجابات مؤقتًا واحترم رؤوس `429` و`Retry-After` وحدود المعدل بدلًا من الاستقصاء بكثافة.
...
- لا تعكس محتوى مخفيًا أو خاصًا أو محظورًا بالإشراف من خلال تجاوز مرشحات API العامة أو حدود المصادقة.
...
- `X-RateLimit-Reset`: ثواني حقبة Unix (وقت إعادة تعيين مطلق)
- `RateLimit-Reset`: ثواني التأخير حتى إعادة التعيين
- `X-RateLimit-Remaining` / `RateLimit-Remaining`: الميزانية المتبقية الدقيقة عند
  وجودها؛ أما الطلبات الناجحة الموزعة فتُغفلها بدلًا من إرجاع قيمة عالمية
  تقريبية
...
- `GET /api/v1/skills?limit=&cursor=&sort=`
  - `sort`: `updated` (الافتراضي)، `recommended` (`default`)، `createdAt` (`newest`)، `downloads`، `stars` (`rating`)، أسماء مستعارة قديمة للتثبيت `installsCurrent`/`installs`/`installsAllTime` تُطابق `downloads`، `trending`
...
- `POST /api/v1/skills` (نشر، ويفضّل متعدد الأجزاء)
...
- `POST /api/v1/users/reserve` يحجز slugs الجذرية وعناصر نائبة لحزم خاصة بلا إصدار لمعرّف المالك.
