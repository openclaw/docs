...
x-i18n:
    generated_at: "2026-06-30T14:01:01Z"
    model: gpt-5.5
...
يمكنك بناء كتالوج أو دليل أو واجهة بحث تابعة لجهة خارجية فوق واجهات API العامة للقراءة في ClawHub. تُنشر بيانات Skills الوصفية العامة وملفات Skills بموجب قواعد ترخيص Skills في ClawHub، بينما تخضع واجهة API نفسها لحدود معدل ويجب استخدامها بمسؤولية.
...
- اربط بعنوان URL القانوني للـ Skill في ClawHub عند عرض القوائم حتى يتمكن المستخدمون من فحص سجل السجل المصدري.
- استخدم عناوين URL القانونية للصفحات بصيغة `https://clawhub.ai/<owner>/skills/<slug>`.
...
- `X-RateLimit-Remaining` / `RateLimit-Remaining`: الميزانية المتبقية الدقيقة عند
  وجودها؛ تحذف الطلبات الناجحة الموزعة على شرائح هذه القيمة بدلًا من إرجاع قيمة
  عامة تقريبية
...
- يحجز `POST /api/v1/users/reserve` الشرائح الجذرية والعناصر النائبة الخاصة لحزم بلا إصدار لمعرّف مالك.
