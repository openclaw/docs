...
x-i18n:
    generated_at: "2026-07-04T18:01:11Z"
    model: gpt-5.5
...
يمكنك بناء فهرس أو دليل أو واجهة بحث تابعة لجهة خارجية فوق واجهات API العامة للقراءة في ClawHub. تُنشر بيانات Skills الوصفية العامة وملفات Skills بموجب قواعد ترخيص Skills في ClawHub، بينما تكون واجهة API نفسها محدودة المعدل ويجب استهلاكها بمسؤولية.

الإرشادات:

- استخدم نقاط نهاية القراءة العامة مثل `GET /api/v1/skills` و`GET /api/v1/search` و`GET /api/v1/skills/{slug}` لقوائم الفهرس.
- خزّن الاستجابات مؤقتًا واحترم `429` و`Retry-After` وترويسات حدود المعدل بدلاً من الاستقصاء المكثف.
- اربط مرة أخرى بعنوان URL الأساسي لـ Skills في ClawHub عند عرض القوائم كي يتمكن المستخدمون من فحص سجل السجل المصدر.
- استخدم عناوين URL الأساسية للصفحات بصيغة `https://clawhub.ai/<owner>/skills/<slug>`.
...
- `X-RateLimit-Remaining` / `RateLimit-Remaining`: الميزانية المتبقية الدقيقة عند
  وجودها؛ تحذف الطلبات الناجحة المجزأة هذه القيمة بدلاً من إرجاع قيمة عامة تقريبية
...
- `GET /api/v1/skills?limit=&cursor=&sort=`
  - `sort`: `updated` (الافتراضي)، `recommended` (`default`)، `createdAt` (`newest`)، `downloads`، `stars` (`rating`)، أسماء التثبيت القديمة المستعارة `installsCurrent`/`installs`/`installsAllTime` تُطابَق إلى `downloads`، `trending`
...
- `GET /api/v1/download?slug=&version=&tag=`
  ...
  - تُرجع Skills الحالية المدعومة من GitHub ذات فحص `clean` أو `suspicious`
    واصف تسليم JSON باسم `public-github` بدلاً من بايتات ClawHub.
...
- `POST /api/v1/users/reserve` يحجز slugs الجذرية وعناصر نائبة خاصة لحزم بلا إصدار لمعرّف المالك.
