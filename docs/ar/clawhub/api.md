...
summary: نظرة عامة واصطلاحات واجهة برمجة تطبيقات REST العامة (v1).
x-i18n:
...
يمكنك بناء فهرس أو دليل أو واجهة بحث تابعة لجهة خارجية فوق واجهات API العامة للقراءة في ClawHub. تُنشر بيانات skill الوصفية العامة وملفات skill بموجب قواعد ترخيص skills في ClawHub، بينما تكون API نفسها محدودة المعدل ويجب استخدامها بمسؤولية.
...
- أضف رابطًا إلى عنوان URL الأساسي لـ skill في ClawHub عند عرض القوائم حتى يتمكن المستخدمون من فحص سجل السجل المصدر.
...
- لا تعكس المحتوى المخفي أو الخاص أو المحظور بواسطة الإشراف عبر تجاوز مرشحات API العامة أو حدود المصادقة.

## المصادقة

- القراءة العامة: لا يلزم token.
...
- الطلبات المصادق عليها (token Bearer صالح): لكل حاوية مستخدم.
- يعود token المفقود/غير الصالح إلى إنفاذ IP.
...
- `POST /api/v1/users/reserve` يحجز root slugs وعناصر نائبة خاصة لحزم بلا إصدار لمعرّف مالك.
