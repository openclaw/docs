...
يمكنك بناء كتالوج أو دليل أو واجهة بحث من طرف ثالث فوق واجهات API العامة للقراءة في ClawHub. تُنشر بيانات تعريف Skills العامة وملفات Skills بموجب قواعد ترخيص Skills في ClawHub، بينما تكون واجهة API نفسها محدودة المعدل وينبغي استخدامها بمسؤولية.
...
- لا توح بأن ClawHub يؤيد موقع الطرف الثالث أو يتحقق منه أو يشغله.
- لا تنسخ محتوى مخفيا أو خاصا أو محظورا بسبب الإشراف من خلال تجاوز مرشحات API العامة أو حدود المصادقة.
...
- `X-RateLimit-Remaining` / `RateLimit-Remaining`: الميزانية المتبقية الدقيقة عند
  وجودها؛ تحذف الطلبات الناجحة المجزأة هذه القيمة بدلا من إرجاع قيمة
  عالمية تقريبية
- `Retry-After`: ثواني التأخير الواجب انتظارها عند `429`
...
- أضف تفاوتا عشوائيا إلى محاولات إعادة المحاولة.
...
- `GET /api/v1/skills?limit=&cursor=&sort=`
  - `sort`: `updated` (افتراضي)، `recommended` (`default`)، `createdAt` (`newest`)، `downloads`، `stars` (`rating`)، أسماء التثبيت المستعارة القديمة `installsCurrent`/`installs`/`installsAllTime` تُعيّن إلى `downloads`، `trending`
...
- `GET /api/v1/download?slug=&version=&tag=`
  ...
  - تُرجع Skills الحالية المدعومة من GitHub ذات فحص `clean` أو `suspicious`
    واصف تسليم JSON `public-github` بدلا من بايتات ClawHub.
