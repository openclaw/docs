...
summary: نظرة عامة واصطلاحات واجهة API REST العامة (v1).
x-i18n:
...
يمكنك بناء كتالوج أو دليل أو واجهة بحث تابعة لجهة خارجية فوق واجهات API العامة للقراءة في ClawHub. تُنشر بيانات تعريف المهارات العامة وملفات المهارات بموجب قواعد ترخيص المهارات في ClawHub، بينما تكون واجهة API نفسها محدودة المعدل ويجب استهلاكها بمسؤولية.
...
- أضف رابطًا إلى عنوان URL الأساسي لمهارة ClawHub عند عرض القوائم حتى يتمكن المستخدمون من فحص سجل السجل المصدري.
...
- أضف عشوائية خفيفة إلى إعادة المحاولات.
...
- تُرجع معاملات الاستعلام المعروفة ذات القيم غير الصالحة `400`.
...
- `GET /api/v1/skills?limit=&cursor=&sort=`
  - `sort`: `updated` (الافتراضي)، `recommended` (`default`)، `createdAt` (`newest`)، `downloads`، `stars` (`rating`)، أسماء التثبيت القديمة المستعارة `installsCurrent`/`installs`/`installsAllTime` تُعيّن إلى `downloads`، `trending`
...
- `GET /api/v1/download?slug=&version=&tag=`
  ...
  - المهارات الحالية المدعومة من GitHub ذات فحص `clean` أو `suspicious` تُرجع
    واصف تسليم JSON من نوع `public-github` بدلًا من بايتات ClawHub.
...
- `POST /api/v1/skills` (نشر، ويُفضّل multipart)
