...
يمكنك بناء فهرس أو دليل أو واجهة بحث تابعة لجهة خارجية فوق واجهات القراءة العامة الخاصة بـ ClawHub. تُنشر بيانات المهارات الوصفية العامة وملفات المهارات بموجب قواعد ترخيص المهارات في ClawHub، بينما تخضع واجهة API نفسها لحدود معدل ويجب استهلاكها بمسؤولية.

الإرشادات:

- استخدم نقاط نهاية القراءة العامة مثل `GET /api/v1/skills` و`GET /api/v1/search` و`GET /api/v1/skills/{slug}` لقوائم الفهارس.
...
- `RateLimit-Reset`: عدد الثواني المتبقية حتى إعادة التعيين
...
- `GET /api/v1/skills?limit=&cursor=&sort=`
  - `sort`: `updated` (الافتراضي)، `recommended` (`default`)، `createdAt` (`newest`)، `downloads`، `stars` (`rating`)، أسماء التثبيت القديمة البديلة `installsCurrent`/`installs`/`installsAllTime` تُطابَق إلى `downloads`، `trending`
  - قيم `sort` غير الصالحة تُرجع `400`
  - ينطبق `cursor` على أنواع الترتيب غير `trending`
...
- `POST /api/v1/skills` (النشر، يُفضَّل multipart)
