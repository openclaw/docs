...
x-i18n:
    generated_at: "2026-07-02T14:02:34Z"
    model: gpt-5.5
...
- `GET /api/v1/skills?limit=&cursor=&sort=`
  - `sort`: `updated` (الافتراضي)، `recommended` (`default`)، `createdAt` (`newest`)، `downloads`، `stars` (`rating`)، أسماء التثبيت المستعارة القديمة `installsCurrent`/`installs`/`installsAllTime` تُعيَّن إلى `downloads`، `trending`
