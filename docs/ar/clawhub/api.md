...
x-i18n:
    generated_at: "2026-07-04T06:33:32Z"
    model: gpt-5.5
...
يمكنك بناء فهرس أو دليل أو واجهة بحث تابعة لطرف ثالث فوق واجهات API العامة للقراءة في ClawHub. تُنشر بيانات تعريف المهارات العامة وملفات المهارات بموجب قواعد ترخيص المهارات في ClawHub، بينما تخضع واجهة API نفسها لحدود معدل ويجب استهلاكها بمسؤولية.
...
- لا تنسخ المحتوى المخفي أو الخاص أو المحظور رقابيًا عبر تجاوز مرشحات API العامة أو حدود المصادقة.
...
- أضف عشوائية طفيفة إلى إعادة المحاولة.

## الأخطاء

- أخطاء v1 نص عادي (`text/plain; charset=utf-8`)، بما في ذلك `400` و
  `401` و`403` و`404` و`429` واستجابات التنزيل المحظور.
- تُتجاهل معاملات الاستعلام غير المعروفة من أجل التوافق.
...
- `GET /api/v1/skills/export?startDate=&endDate=&limit=&cursor=`
  ...
  - المهارات الحالية المدعومة من GitHub التي لديها فحص `clean` أو `suspicious` تُصدَّر
...
- `POST /api/v1/users/reserve` يحجز مقاطع الجذر النسبية وعناصر نائبة خاصة لحزم بلا إصدار لمعرّف مالك.
