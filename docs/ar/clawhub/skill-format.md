...
x-i18n:
    generated_at: "2026-07-04T15:20:11Z"
    model: gpt-5.5
...
لحساب GitHub الذي سجل الدخول. لا يستورد المستودعات الخاصة، أو التفريعات،
...
يتم تعريف بيانات تعريف المهارة في frontmatter بتنسيق YAML أعلى ملف `SKILL.md` الخاص بك. هذا يخبر السجل (وتحليل الأمان) بما تحتاجه مهارتك للتشغيل.
...
---
name: my-skill
description: ملخص قصير لما تفعله هذه المهارة.
version: 1.0.0
---
```
...
| `requires.anyBins` | `string[]` | ثنائيات CLI التي يجب أن يكون واحد منها على الأقل موجودًا.                                                                                                  |
...
| `envVars`          | `array`    | تعريفات متغيرات البيئة مع `name` و`required` اختياري و`description` اختياري. اضبط `required: false` لمتغيرات البيئة الاختيارية. |
...
أعلن متغيرات البيئة الاختيارية ضمن `metadata.openclaw.envVars` واضبط `required: false`. لا تضف إدخالات اختيارية إلى `requires.env`، لأن `requires.env` يعني أن المهارة لا يمكنها العمل بدونها.
  ...
  openclaw:
    ...
    envVars:
      - name: TODOIST_API_KEY
        required: true
        description: رمز Todoist API المستخدم للطلبات الموثقة.
      - name: TODOIST_PROJECT_ID
        required: false
        description: معرف المشروع الافتراضي الاختياري عندما لا يحدد المستخدم واحدًا.
```

### لماذا هذا مهم

يتحقق تحليل الأمان في ClawHub من أن ما تعلنه مهارتك يطابق ما تفعله فعليًا. إذا كان كودك يشير إلى `TODOIST_API_KEY` لكن frontmatter الخاص بك لا يعلنه ضمن `requires.env` أو `primaryEnv` أو `envVars`، فسيشير التحليل إلى عدم تطابق في بيانات التعريف. يساعد الحفاظ على دقة التصريحات مهارتك على اجتياز المراجعة ويساعد المستخدمين على فهم ما يقومون بتثبيته.
...
metadata:
  openclaw:
    ...
    envVars:
      ...
      - name: TODOIST_PROJECT_ID
        required: false
        description: معرف المشروع الافتراضي الاختياري.
    emoji: "\u2705"
...
- قائمة امتدادات السماح موجودة في `packages/schema/src/textFiles.ts` (`TEXT_FILE_EXTENSIONS`).
- لا تزال ملفات السكربت تُفحص بعد الرفع؛ يتم قبول ملفات PowerShell ذات الامتدادات `.ps1` و`.psm1` و`.psd1` كنص.
- يتم التعامل مع أنواع المحتوى التي تبدأ بـ `text/` كنص؛ بالإضافة إلى قائمة سماح صغيرة (JSON/YAML/TOML/JS/TS/Markdown/SVG).

الحدود (من جهة الخادم):

- إجمالي حجم الحزمة: 50MB.
- تضمين النص يشمل `SKILL.md` + ما يصل إلى نحو 40 ملفًا غير `.md` (حد بأفضل جهد).
