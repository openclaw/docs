...
summary: تنسيق مجلد Skills، الملفات المطلوبة، أنواع الملفات المسموح بها، الحدود.
x-i18n:
    generated_at: "2026-07-02T22:33:46Z"
    model: gpt-5.5
...
حساب GitHub المسجل الدخول. لا يستورد المستودعات الخاصة، أو التفرعات،
...
استخدم `requires.env` لمتغيرات البيئة التي يجب أن تكون موجودة قبل أن تعمل المهارة. استخدم `envVars` عندما تحتاج إلى بيانات وصفية لكل متغير، بما في ذلك المتغيرات الاختيارية مع `required: false`.
...
| `primaryEnv`       | `string`   | متغير البيئة الرئيسي للاعتماد السري لمهارتك.                                                                                                  |
...
| `emoji`            | `string`   | رمز emoji معروض للمهارة.                                                                                                                 |
...
أعلن متغيرات البيئة الاختيارية تحت `metadata.openclaw.envVars` وعيّن `required: false`. لا تُضف إدخالات اختيارية إلى `requires.env`، لأن `requires.env` يعني أن المهارة لا يمكن أن تعمل بدونها.
...
يتحقق تحليل الأمان في ClawHub من أن ما تعلنه مهارتك يطابق ما تفعله فعليًا. إذا أشار كودك إلى `TODOIST_API_KEY` لكن frontmatter لا يعلنه تحت `requires.env` أو `primaryEnv` أو `envVars`، فسيُبلغ التحليل عن عدم تطابق في البيانات الوصفية. يساعد إبقاء التصريحات دقيقة مهارتك على اجتياز المراجعة، ويساعد المستخدمين على فهم ما يثبتونه.
...
- لا تزال ملفات السكربت تُفحص بعد الرفع؛ تُقبل ملفات PowerShell `.ps1` و`.psm1` و`.psd1` كنصوص.
...
- إذا كانت مهارتك تتكامل مع خدمة طرف ثالث مدفوعة، فوثّق التكلفة الخارجية والحساب المطلوب بوضوح في تعليمات المهارة وتصريحات البيئة (`requires.env` للمتغيرات المطلوبة، أو `envVars` مع `required: false` للمتغيرات الاختيارية).
