...
summary: تنسيق مجلد Skills، الملفات المطلوبة، أنواع الملفات المسموح بها، والحدود.
x-i18n:
    generated_at: "2026-07-02T14:03:21Z"
    model: gpt-5.5
...
- `SKILL.md` (أو `skill.md`؛ ويُقبل أيضًا الاسم القديم `skills.md`)

اختياري:

- أي ملفات داعمة _نصية_ (راجع "الملفات المسموح بها")
- `.clawhubignore` (أنماط تجاهل للنشر، الاسم القديم `.clawdhubignore`)
- `.gitignore` (يُحترم أيضًا)

## استيراد GitHub

مستورِد GitHub عبر الويب أكثر صرامة من النشر/المزامنة المحليين. فهو يكتشف فقط
ملفات `SKILL.md` أو الملفات القديمة `skills.md` في المستودعات العامة غير المتفرعة المملوكة
لحساب GitHub المسجّل دخوله. ولا يستورد المستودعات الخاصة، أو التفريعات،
...
- Markdown مع بيانات YAML أمامية اختيارية.
- يستخرج الخادم بيانات التعريف من البيانات الأمامية أثناء النشر.
- يُستخدم `description` كملخص للمهارة في واجهة المستخدم/البحث.

## بيانات التعريف الأمامية

تُعلن بيانات تعريف المهارة في بيانات YAML الأمامية أعلى ملف `SKILL.md`. وهذا يخبر السجل (وتحليل الأمان) بما تحتاجه مهارتك لتعمل.

### البيانات الأمامية الأساسية
...
استخدم `requires.env` لمتغيرات البيئة التي يجب أن تكون موجودة قبل أن تتمكن المهارة من العمل. واستخدم `envVars` عندما تحتاج إلى بيانات تعريف لكل متغير، بما في ذلك المتغيرات الاختيارية مع `required: false`.
...
| `emoji`            | `string`   | رمز تعبيري يُعرض للمهارة.                                                                                                                 |
| `homepage`         | `string`   | عنوان URL للصفحة الرئيسية للمهارة أو وثائقها.                                                                                                         |
...
أعلن متغيرات البيئة الاختيارية ضمن `metadata.openclaw.envVars` واضبط `required: false`. لا تضف إدخالات اختيارية إلى `requires.env`، لأن `requires.env` يعني أن المهارة لا يمكن أن تعمل بدونها.
...
يتحقق تحليل الأمان في ClawHub من أن ما تعلنه مهارتك يطابق ما تفعله فعليًا. إذا كان كودك يشير إلى `TODOIST_API_KEY` لكن بياناتك الأمامية لا تعلنه ضمن `requires.env` أو `primaryEnv` أو `envVars`، فسيعلّم التحليل ذلك كعدم تطابق في بيانات التعريف. يساعد الحفاظ على دقة التصريحات مهارتك على اجتياز المراجعة، ويساعد المستخدمين على فهم ما يثبتونه.

### مثال: بيانات أمامية كاملة
...
- إذا كانت مهارتك تتكامل مع خدمة خارجية مدفوعة، فوثّق التكلفة الخارجية والحساب المطلوب بوضوح في تعليمات المهارة وتصريحات البيئة (`requires.env` للمتغيرات المطلوبة، أو `envVars` مع `required: false` للمتغيرات الاختيارية).
