...
x-i18n:
    generated_at: "2026-07-01T15:25:16Z"
    model: gpt-5.5
...
- `.clawhubignore` (أنماط تجاهل للنشر، أو `.clawdhubignore` القديم)
...
تُعلن بيانات تعريف المهارة في frontmatter بصيغة YAML في أعلى ملف `SKILL.md`. يخبر هذا السجل (وتحليل الأمان) بما تحتاجه مهارتك للتشغيل.
...
أعلن متغيرات البيئة الاختيارية ضمن `metadata.openclaw.envVars` واضبط `required: false`. لا تضف إدخالات اختيارية إلى `requires.env`، لأن `requires.env` يعني أن المهارة لا يمكنها العمل من دونها.
...
يتحقق تحليل الأمان في ClawHub من أن ما تعلنه مهارتك يطابق ما تفعله فعليًا. إذا كان كودك يشير إلى `TODOIST_API_KEY` لكن frontmatter لا يعلنه ضمن `requires.env` أو `primaryEnv` أو `envVars`، فسيرفع التحليل علامة على عدم تطابق بيانات التعريف. الحفاظ على دقة الإعلانات يساعد مهارتك على اجتياز المراجعة ويساعد المستخدمين على فهم ما يثبتونه.
...
- يجب أن تطابق نطاقات الحزم معرّف ناشر ClawHub بالضبط. يمكن لمعرّفات الناشرين استخدام الأحرف الصغيرة، والأرقام، والواصلات، والنقاط، والشرطات السفلية؛ ويجب أن تبدأ وتنتهي بحرف صغير أو رقم.
- يجب أن تكون slugs الحزم بحروف صغيرة وآمنة لـ npm، مثل `@example.tools/demo-plugin` أو `demo-plugin`.
