...
- `.clawhubignore` (أنماط التجاهل للنشر/المزامنة، والاسم القديم `.clawdhubignore`)
...
تُعلَن بيانات تعريف المهارة في frontmatter بصيغة YAML أعلى ملف `SKILL.md`. يوضح هذا للسجل (ولتحليل الأمان) ما تحتاجه مهارتك للتشغيل.
...
أعلِن متطلبات وقت تشغيل مهارتك تحت `metadata.openclaw` (الأسماء البديلة: `metadata.clawdbot`، `metadata.clawdis`).
...
| `always`           | `boolean`  | إذا كانت `true`، فالمهارة نشطة دائمًا (لا حاجة إلى تثبيت صريح).                                                                              |
...
إذا كانت مهارتك تحتاج إلى تثبيت تبعيات، فأعلِنها في مصفوفة `install`:
...
أعلِن متغيرات البيئة الاختيارية تحت `metadata.openclaw.envVars` واضبط `required: false`. لا تضف إدخالات اختيارية إلى `requires.env`، لأن `requires.env` يعني أن المهارة لا يمكن تشغيلها بدونها.
...
يتحقق تحليل الأمان في ClawHub من أن ما تعلنه مهارتك يطابق ما تفعله فعليًا. إذا كان كودك يشير إلى `TODOIST_API_KEY` لكن frontmatter لا يعلنه تحت `requires.env` أو `primaryEnv` أو `envVars`، فسيرفع التحليل إشارة إلى عدم تطابق في بيانات التعريف. يساعد الحفاظ على دقة الإعلانات مهارتك على اجتياز المراجعة ويساعد المستخدمين على فهم ما يثبتونه.
...
- تُعامَل أنواع المحتوى التي تبدأ بـ `text/` كنص؛ بالإضافة إلى قائمة صغيرة مسموح بها (JSON/YAML/TOML/JS/TS/Markdown/SVG).

الحدود (من جهة الخادم):

- حجم الحزمة الإجمالي: 50MB.
- يتضمن نص التضمين `SKILL.md` + حتى نحو 40 ملفًا غير `.md` (حد بأفضل جهد).
...
- إذا كانت مهارتك تتكامل مع خدمة خارجية مدفوعة، فوَثِّق التكلفة الخارجية والحساب المطلوب بوضوح في تعليمات المهارة وإعلانات البيئة (`requires.env` للمتغيرات المطلوبة، أو `envVars` مع `required: false` للمتغيرات الاختيارية).
