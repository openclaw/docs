...
read_when:
    - نشر Skills
    - استكشاف أخطاء إخفاقات النشر/المزامنة وإصلاحها
...
- `.clawhubignore` (أنماط تجاهل للنشر/المزامنة، الصيغة القديمة `.clawdhubignore`)
...
يتحقق تحليل الأمان في ClawHub من أن ما تعلنه مهارتك يطابق ما تفعله فعليًا. إذا كان كودك يشير إلى `TODOIST_API_KEY` لكن frontmatter لا يعلنه تحت `requires.env` أو `primaryEnv` أو `envVars`، فسيُعلّم التحليل ذلك كتعارض في بيانات التعريف. يساعد الحفاظ على دقة التصريحات مهارتك على اجتياز المراجعة، ويساعد المستخدمين على فهم ما يثبتونه.
