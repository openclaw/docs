...
مستورد GitHub عبر الويب أكثر صرامة من النشر/المزامنة المحليين. فهو لا يكتشف إلا
ملفات `SKILL.md` أو `skills.md` القديمة في مستودعات عامة غير متفرعة ومملوكة
لحساب GitHub الذي سجّل الدخول. ولا يستورد المستودعات الخاصة، أو التفريعات،
...
تُعلن بيانات تعريف المهارة في YAML frontmatter أعلى ملف `SKILL.md`. يخبر هذا السجل (وتحليل الأمان) بما تحتاجه مهارتك للتشغيل.
...
أعلن متطلبات وقت تشغيل مهارتك ضمن `metadata.openclaw` (الأسماء البديلة: `metadata.clawdbot` و`metadata.clawdis`).
...
استخدم `requires.env` لمتغيرات البيئة التي يجب أن تكون موجودة قبل أن تتمكن المهارة من التشغيل. استخدم `envVars` عندما تحتاج إلى بيانات تعريف لكل متغير، بما في ذلك المتغيرات الاختيارية ذات `required: false`.
...
| `requires.config`  | `string[]` | مسارات ملفات الإعدادات التي تقرأها مهارتك.                                                                                                          |
...
يتحقق تحليل الأمان في ClawHub من أن ما تعلنه مهارتك يطابق ما تفعله فعليًا. إذا كان كودك يشير إلى `TODOIST_API_KEY` لكن frontmatter لا يعلنه ضمن `requires.env` أو `primaryEnv` أو `envVars`، فسيضع التحليل علامة على عدم تطابق في بيانات التعريف. يساعد الحفاظ على دقة الإعلانات مهارتك على اجتياز المراجعة، ويساعد المستخدمين على فهم ما يثبتونه.
...
- يمكن لأي شخص استخدام Skills المنشورة وتعديلها وإعادة توزيعها، بما في ذلك تجاريًا.
- لا يلزم الإسناد.
