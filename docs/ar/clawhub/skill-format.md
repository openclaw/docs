...
x-i18n:
    generated_at: "2026-07-03T17:25:55Z"
    model: gpt-5.5
...
مستورِد GitHub على الويب أكثر صرامة من النشر/المزامنة المحليين. فهو يكتشف فقط ملفات
...
حالة تثبيت مجلد العمل (تكتبها CLI):
...
تُعلَن بيانات تعريف المهارة في YAML frontmatter أعلى ملف `SKILL.md`. يخبر هذا السجلّ (وتحليل الأمان) بما تحتاجه مهارتك للتشغيل.
...
| `envVars`          | `array`    | إعلانات متغيرات البيئة مع `name`، و`required` اختياري، و`description` اختياري. اضبط `required: false` لمتغيرات البيئة الاختيارية. |
...
| `homepage`         | `string`   | عنوان URL لصفحة المهارة الرئيسية أو وثائقها.                                                                                                         |
...
أعلن متغيرات البيئة الاختيارية تحت `metadata.openclaw.envVars` واضبط `required: false`. لا تضف إدخالات اختيارية إلى `requires.env`، لأن `requires.env` يعني أن المهارة لا يمكنها التشغيل من دونها.
...
يتحقق تحليل أمان ClawHub من أن ما تعلنه مهارتك يطابق ما تفعله فعليًا. إذا كان كودك يشير إلى `TODOIST_API_KEY` لكن frontmatter لا يعلنه تحت `requires.env` أو `primaryEnv` أو `envVars`، فسيضع التحليل علامة على عدم تطابق بيانات التعريف. يساعد الحفاظ على دقة الإعلانات مهارتك على اجتياز المراجعة، ويساعد المستخدمين على فهم ما يثبتونه.
...
- إذا كانت مهارتك تتكامل مع خدمة مدفوعة تابعة لجهة خارجية، فوثّق بوضوح التكلفة الخارجية والحساب المطلوب في تعليمات المهارة وإعلانات البيئة (`requires.env` للمتغيرات المطلوبة، أو `envVars` مع `required: false` للمتغيرات الاختيارية).
