...
read_when:
    - نشر Skills
    - تصحيح أخطاء فشل النشر/المزامنة
...
تُعلن بيانات المهارة الوصفية في YAML frontmatter أعلى ملف `SKILL.md`. يوضح هذا للسجل (ولتحليل الأمان) ما تحتاجه مهارتك للعمل.
...
| `homepage`         | `string`   | عنوان URL للصفحة الرئيسية للمهارة أو مستنداتها.                                                                                                         |
...
إذا كانت مهارتك تحتاج إلى تثبيت اعتماديات، فأعلن عنها في مصفوفة `install`:
...
يتحقق تحليل الأمان في ClawHub من أن ما تعلنه مهارتك يطابق ما تفعله فعليًا. إذا كان كودك يشير إلى `TODOIST_API_KEY` لكن frontmatter لديك لا يعلنه ضمن `requires.env` أو `primaryEnv` أو `envVars`، فسيضع التحليل علامة على عدم تطابق في البيانات الوصفية. يساعد الحفاظ على دقة الإعلانات مهارتك على اجتياز المراجعة، ويساعد المستخدمين على فهم ما يثبتونه.
