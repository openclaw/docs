...
تُعلن بيانات تعريف المهارة في frontmatter بصيغة YAML أعلى ملف `SKILL.md`. يوضح هذا للسجل (وتحليل الأمان) ما تحتاج إليه مهارتك للتشغيل.
...
أعلن متطلبات وقت تشغيل مهارتك ضمن `metadata.openclaw` (أسماء بديلة: `metadata.clawdbot`, `metadata.clawdis`).
...
يتحقق تحليل الأمان في ClawHub من أن ما تعلنه مهارتك يطابق ما تفعله فعليًا. إذا أشار كودك إلى `TODOIST_API_KEY` لكن frontmatter لا يعلنه ضمن `requires.env` أو `primaryEnv` أو `envVars`، فسيعلّم التحليل ذلك على أنه عدم تطابق في بيانات التعريف. يساعد الحفاظ على دقة الإعلانات مهارتك على اجتياز المراجعة ويساعد المستخدمين على فهم ما يثبتونه.
...
- إذا كانت مهارتك تتكامل مع خدمة طرف ثالث مدفوعة، فوثّق التكلفة الخارجية والحساب المطلوب بوضوح في تعليمات المهارة وإعلانات البيئة (`requires.env` للمتغيرات المطلوبة، أو `envVars` مع `required: false` للمتغيرات الاختيارية).
