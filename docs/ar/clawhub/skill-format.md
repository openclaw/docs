...
ملفات `SKILL.md` أو ملفات `skills.md` القديمة في المستودعات العامة غير المتفرعة التي يملكها
...
بيانات التعريف للتثبيت المحلي (يكتبها CLI):
...
تُعرّف بيانات تعريف المهارة في frontmatter بصيغة YAML أعلى ملف `SKILL.md`. يوضح ذلك للسجل (ولتحليل الأمان) ما تحتاجه مهارتك للتشغيل.
...
| `requires.bins`    | `string[]` | ملفات CLI الثنائية التي يجب أن تكون كلها مثبتة.                                                                                                     |
...
صرّح بمتغيرات البيئة الاختيارية ضمن `metadata.openclaw.envVars` وعيّن `required: false`. لا تضف إدخالات اختيارية إلى `requires.env`، لأن `requires.env` يعني أن المهارة لا يمكن تشغيلها من دونها.
...
يتحقق تحليل الأمان في ClawHub من أن ما تصرّح به مهارتك يطابق ما تفعله فعليًا. إذا كان كودك يشير إلى `TODOIST_API_KEY` لكن frontmatter لديك لا يصرّح به ضمن `requires.env` أو `primaryEnv` أو `envVars`، فسيُبلغ التحليل عن عدم تطابق في بيانات التعريف. يساعد الحفاظ على دقة التصريحات مهارتك على اجتياز المراجعة، ويساعد المستخدمين على فهم ما يثبتونه.
...
- ما تزال ملفات السكربتات تُفحص بعد الرفع؛ وتُقبل ملفات PowerShell ذات الامتدادات `.ps1` و`.psm1` و`.psd1` كنصوص.
- تُعامل أنواع المحتوى التي تبدأ بـ `text/` كنصوص؛ إضافة إلى قائمة صغيرة مسموح بها (JSON/YAML/TOML/JS/TS/Markdown/SVG).
