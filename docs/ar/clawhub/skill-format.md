...
summary: تنسيق مجلد Skill، الملفات المطلوبة، أنواع الملفات المسموح بها، الحدود.
x-i18n:
    generated_at: "2026-07-04T06:34:08Z"
    model: gpt-5.5
...
لحساب GitHub الذي سجّل الدخول. ولا يستورد المستودعات الخاصة، أو التفرعات،
...
تُعلن بيانات تعريف المهارة في YAML frontmatter أعلى ملف `SKILL.md`. يوضّح هذا للسجل (وتحليل الأمان) ما تحتاجه مهارتك للتشغيل.
...
أعلن متغيرات البيئة الاختيارية ضمن `metadata.openclaw.envVars` وعيّن `required: false`. لا تضف إدخالات اختيارية إلى `requires.env`، لأن `requires.env` يعني أن المهارة لا يمكن تشغيلها من دونها.
...
يتحقق تحليل الأمان في ClawHub من أن ما تعلنه مهارتك يطابق ما تفعله فعليًا. إذا كان كودك يشير إلى `TODOIST_API_KEY` لكن frontmatter لا يعلنه ضمن `requires.env` أو `primaryEnv` أو `envVars`، فسيعلّم التحليل ذلك كتعارض في بيانات التعريف. يساعد إبقاء التصريحات دقيقة مهارتك على اجتياز المراجعة، ويساعد المستخدمين على فهم ما يثبتونه.
...
- يجب أن تطابق نطاقات الحزم معرّف ناشر ClawHub تمامًا. يمكن أن تستخدم معرّفات الناشرين أحرفًا صغيرة، وأرقامًا، وواصلات، ونقاطًا، وشرطات سفلية؛ ويجب أن تبدأ وتنتهي بحرف صغير أو رقم.
...
- إذا كانت مهارتك تتكامل مع خدمة خارجية مدفوعة، فوّثق التكلفة الخارجية والحساب المطلوب بوضوح في تعليمات المهارة وتصريحات البيئة (`requires.env` للمتغيرات المطلوبة، أو `envVars` مع `required: false` للمتغيرات الاختيارية).
