...
x-i18n:
    generated_at: "2026-07-05T06:25:32Z"
    model: gpt-5.5
...
`SKILL.md` أو ملفات `skills.md` القديمة في المستودعات العامة غير المتفرعة المملوكة
...
تُعلن بيانات تعريف المهارة في frontmatter بصيغة YAML أعلى ملف `SKILL.md`. يخبر هذا السجل (وتحليل الأمان) بما تحتاجه مهارتك للعمل.
...
يتحقق تحليل الأمان في ClawHub من أن ما تعلنه مهارتك يطابق ما تفعله فعليًا. إذا كان كودك يشير إلى `TODOIST_API_KEY` لكن frontmatter لا يعلنه ضمن `requires.env` أو `primaryEnv` أو `envVars`، فسيضع التحليل علامة على عدم تطابق بيانات التعريف. يساعد الحفاظ على دقة الإعلانات مهارتك على اجتياز المراجعة، ويساعد المستخدمين على فهم ما يقومون بتثبيته.
...
- لا تزال ملفات السكربت تُفحص بعد الرفع؛ وتُقبل ملفات PowerShell `.ps1` و`.psm1` و`.psd1` كنص.
...
- يجب أن تطابق نطاقات الحزمة مقبض ناشر ClawHub بالضبط. يمكن أن تستخدم مقابض الناشرين أحرفًا صغيرة، وأرقامًا، وواصلات، ونقاطًا، وشرطات سفلية؛ ويجب أن تبدأ وتنتهي بحرف صغير أو رقم.
