---
read_when:
    - شغّلتَ clawhub package validate وتحتاج إلى إصلاح ملاحظات Plugin
...
x-i18n:
    generated_at: "2026-07-01T12:59:36Z"
    model: gpt-5.5
...
| `provider-auth-env-vars`                | [انقل متغيرات بيئة المزود إلى بيانات تعريف الإعداد](/ar/clawhub/plugin-validation-fixes#provider-auth-env-vars)                             |
...
توجد بيانات تعريف الحزمة، لكنها لا تصرح بنقطة دخول وقت تشغيل
OpenClaw.
...
- ابنِ الحزمة إذا كانت نقطة الدخول تُنشأ في `dist`.
...
لا تصرح الحزمة بنطاق واجهة برمجة تطبيقات Plugin في OpenClaw الذي تدعمه.
...
لا يتطابق الحد الأدنى لإصدار المضيف في الحزمة مع بيانات تعريف إصدار OpenClaw
...
- أصلح بيانات تعريف الحزمة غير الصالحة، أو سكربتات دورة الحياة المعطلة، أو مدخلات الملفات التي
  تجعل الحزم يفشل.
...
- أبقِ `name` قابلا للقراءة البشرية وأبقِ `id` معرّف الجهاز الثابت.
- راجع [بيان Plugin](/ar/plugins/manifest).
- أعد تشغيل `clawhub package validate <path-to-plugin>`.
...
ما زال Plugin يستخدم مساعد كتابة مخزن الجلسة الكامل المهمل مثل
...
ما زال Plugin يستخدم مساعدات النصوص المنسوخة منخفضة المستوى المهملة مثل
`appendSessionTranscriptMessage` أو `emitSessionTranscriptUpdate`.

- استخدم `appendSessionTranscriptMessageByIdentity(...)` لإضافات النصوص المنسوخة.
- استخدم `publishSessionTranscriptUpdateByIdentity(...)` لإشعارات تحديث النصوص
  المنسوخة.
- فضّل واجهة وقت تشغيل النصوص المنسوخة المنظمة حتى يتمكن OpenClaw من تطبيق
...
ما زال البيان يستخدم بيانات تعريف مصادقة المزود القديمة `providerAuthEnvVars`.

- انسخ بيانات تعريف متغيرات بيئة المزود إلى `setup.providers[].envVars`.
...
يستخدم البيان بيانات تعريف متغيرات بيئة قناة قديمة أو أقدم دون بيانات تعريف
...
- انسخ إعداد القناة المدفوع بالبيئة إلى بيانات تعريف الإعداد أو تكوين القناة أو
  حزمة القناة الحالية التي يستخدمها شكل Plugin لديك.
