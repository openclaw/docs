...
x-i18n:
    generated_at: "2026-07-04T18:01:49Z"
    model: gpt-5.5
...
فحوصات الحزم الآلية. تغطي هذه الصفحة النتائج الموجهة للمؤلفين، أي
...
على رموز صيانة للماسح من دون إرشادات معالجة للمؤلف، فهي مخصصة لمشرفي
OpenClaw لا لمؤلفي Plugin.
...
| `package-plugin-api-compat-missing`     | [صرح بتوافق واجهة برمجة تطبيقات Plugin](/ar/clawhub/plugin-validation-fixes#package-plugin-api-compat-missing)                          |
...
بيانات الحزمة الوصفية موجودة، لكنها لا تصرح بنقطة دخول تشغيل
OpenClaw.

- أضف `openclaw.extensions` لنقاط دخول Plugin الأصلية.
- أضف `openclaw.runtimeExtensions` عندما ينبغي أن تحمّل الحزمة المنشورة
  JavaScript مبنيًا.
...
لا تصرح الحزمة بنطاق واجهة برمجة تطبيقات Plugin لـ OpenClaw الذي تدعمه.

- أضف `openclaw.compat.pluginApi` إلى `package.json`.
- استخدم إصدار واجهة برمجة تطبيقات Plugin لـ OpenClaw أو الحد الأدنى وفق semver الذي بنيت واختبرت
  عليه.
...
لا يطابق الحد الأدنى لإصدار مضيف الحزمة بيانات إصدار OpenClaw الوصفية
...
- انقل سلوك وقت التشغيل إلى كود تسجيل Plugin، وأبقِ `contracts`
  محصوراً في بيانات ملكية القدرات الوصفية الثابتة.
- أعد تشغيل `clawhub package validate <path-to-plugin>`.
...
- استبدل واردات المجمع الجذري بواردات مسارات فرعية عامة ومركزة.
...
- استبدل واردات SDK الداخلية المحجوزة في OpenClaw بمسارات فرعية عامة وموثقة
  ضمن `openclaw/plugin-sdk/*`.
...
- استخدم `patchSessionEntry(...)` أو `upsertSessionEntry(...)` لحفظ بيانات
  الجلسة الوصفية.
- استخدم هوية النص التفريغي أو مساعدات الهدف عندما يكون الكود يحضّر عملية
  نص تفريغي.
- لا تحفظ مسارات ملفات النصوص التفريغية القديمة ولا تعتمد عليها.
- راجع [API وقت التشغيل](/ar/plugins/sdk-runtime#agent-session-state) و
  [المسارات الفرعية لـ Plugin SDK](/ar/plugins/sdk-subpaths).
- أعد تشغيل `clawhub package validate <path-to-plugin>`.
...
لا يزال Plugin يستخدم مساعد هدف ملف النص التفريغي المهمل
...
- استخدم `resolveSessionTranscriptTarget(...)` عندما يحتاج الكود إلى هدف عملية
  نص تفريغي منظّم.
- تجنب قراءة أهداف ملفات النصوص التفريغية القديمة أو إنشائها مباشرة.
...
لا يزال Plugin يستخدم مساعدات نص تفريغي منخفضة المستوى مهملة مثل
`appendSessionTranscriptMessage` أو `emitSessionTranscriptUpdate`.

- استخدم `appendSessionTranscriptMessageByIdentity(...)` لإلحاقات النص التفريغي.
- استخدم `publishSessionTranscriptUpdateByIdentity(...)` لإشعارات تحديث النص التفريغي.
- فضّل سطح وقت تشغيل النص التفريغي المنظّم حتى يتمكن OpenClaw من تطبيق
...
لا يزال البيان يستخدم بيانات تعريف مصادقة المزود القديمة `providerAuthEnvVars`.

- اعكس بيانات تعريف متغيرات بيئة المزود في `setup.providers[].envVars`.
- أبقِ `providerAuthEnvVars` فقط كبيانات تعريف توافق ما دام نطاق
...
يستخدم البيان بيانات تعريف متغيرات بيئة قناة قديمة أو أقدم من دون بيانات
...
- أبقِ بيانات تعريف متغيرات بيئة القناة تصريحية حتى يتمكن OpenClaw من فحص حالة
  setup من دون تحميل وقت تشغيل القناة.
- اعكس إعداد القناة المدفوع بالبيئة في setup الحالي، أو config القناة، أو
  بيانات تعريف قناة الحزمة التي يستخدمها شكل Plugin لديك.
- أبقِ `channelEnvVars` فقط كبيانات تعريف توافق ما دامت إصدارات
...
- أزل عنوان URL للمخطط إذا كان إرشادياً فقط.
...
- أزل `openclaw.security.json` حتى يوثق OpenClaw مخطط بيان أمان
