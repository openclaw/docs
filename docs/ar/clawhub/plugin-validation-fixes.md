...
x-i18n:
    generated_at: "2026-07-01T20:21:02Z"
    model: gpt-5.5
...
يحتوي على رموز صيانة للماسح دون إرشادات معالجة للمؤلف، فهذه مخصصة لمشرفي
...
- عيّن `openclaw.install.defaultChoice` عندما يتوفر أكثر من مصدر تثبيت
  واحد.
...
لا تصرّح الحزمة بنطاق API الخاص بـ Plugin في OpenClaw الذي تدعمه.

- أضف `openclaw.compat.pluginApi` إلى `package.json`.
- استخدم إصدار API الخاص بـ Plugin في OpenClaw أو حد semver الأدنى الذي بنيت
  واختبرت عليه.
...
- واءم الحد الأدنى لإصدار المضيف مع نطاق إصدار المضيف الذي تدعمه الحزمة
  فعليًا.
- راجع [حقول package.json التي تؤثر في الاكتشاف](/ar/plugins/manifest#packagejson-fields-that-affect-discovery).
- أعد تشغيل `clawhub package validate <path-to-plugin>`.
...
تحتوي كتلة `package.json#openclaw` على حقول لا تُدعم بوصفها
...
- أبقِ `name` مقروءًا للبشر، وأبقِ `id` كمعرّف آلة مستقر.
- راجع [بيان Plugin](/ar/plugins/manifest).
- أعد تشغيل `clawhub package validate <path-to-plugin>`.
...
- انقل سلوك وقت التشغيل إلى كود تسجيل Plugin، وأبقِ `contracts`
  مقتصرة على بيانات ملكية القدرات الثابتة.
- أعد تشغيل `clawhub package validate <path-to-plugin>`.
...
يستورد Plugin من مدخل SDK الجذري المهمل:
`openclaw/plugin-sdk`.

- استبدل استيرادات المدخل الجذري باستيرادات مسارات فرعية عامة ومركزة.
...
يستورد Plugin مسار SDK مخصصًا للـ Plugins المضمنة أو للتوافق
الداخلي.
...
ما زال Plugin يستخدم مساعدات مسارات ملفات الجلسة المهملة، مثل
...
- استخدم `getSessionEntry(...)` لقراءة بيانات الجلسة الوصفية حسب هوية الوكيل
  والجلسة.
- استخدم `patchSessionEntry(...)` أو `upsertSessionEntry(...)` للاستمرار في حفظ بيانات الجلسة
  الوصفية.
- استخدم هوية النص المنسوخ أو مساعدات الهدف عندما يكون الكود يحضّر
  عملية نص منسوخ.
...
- استخدم `resolveSessionTranscriptIdentity(...)` عندما يحتاج الكود فقط إلى هوية الجلسة
  العامة.
- استخدم `resolveSessionTranscriptTarget(...)` عندما يحتاج الكود إلى هدف عملية نص منسوخ
  منظم.
...
ما زال Plugin يستخدم مساعدات نصوص منسوخة منخفضة المستوى مهملة، مثل
...
ما زال البيان يستخدم بيانات مصادقة المزوّد الوصفية القديمة `providerAuthEnvVars`.
...
يستخدم البيان بيانات متغيرات بيئة القناة القديمة أو الأقدم من دون بيانات
...
- انسخ setup القناة المدفوع بالبيئة إلى بيانات setup الحالية أو config القناة أو
