...
x-i18n:
    generated_at: "2026-07-02T22:33:38Z"
    model: gpt-5.5
...
فحوصات الحزم الآلية. تغطي هذه الصفحة النتائج الموجّهة للمؤلفين، أي
...
على رموز صيانة للماسح من دون إرشادات معالجة للمؤلف، فهذه مخصصة لمشرفي
...
| `package-plugin-api-compat-missing`     | [أعلن توافق واجهة API الخاصة بـ Plugin](/ar/clawhub/plugin-validation-fixes#package-plugin-api-compat-missing)                          |
...
بيانات تعريف الحزمة موجودة، لكنها لا تعلن نقطة دخول تشغيل
OpenClaw.
...
لا تعلن الحزمة عن نطاق واجهة برمجة تطبيقات Plugin في OpenClaw الذي تدعمه.
...
- راجع [حقول package.json التي تؤثر في الاكتشاف](/ar/plugins/manifest#packagejson-fields-that-affect-discovery).
- شغّل `clawhub package validate <path-to-plugin>` مرة أخرى.
...
- وائم الحد الأدنى لإصدار المضيف مع نطاق إصدار المضيف الذي
  تدعمه الحزمة فعليًا.
- راجع [حقول package.json التي تؤثر في الاكتشاف](/ar/plugins/manifest#packagejson-fields-that-affect-discovery).
- شغّل `clawhub package validate <path-to-plugin>` مرة أخرى.
...
يوجد تعارض بين إصدار الحزمة وإصدار بيان Plugin.

- فضّل `package.json#version` باعتباره إصدار نشر الحزمة.
- إذا كان `openclaw.plugin.json` يحتوي أيضًا على `version`، فحدّثه ليطابقه أو أزِل
...
- أصلح بيانات تعريف الحزمة غير الصالحة، أو نصوص دورة الحياة المعطلة، أو إدخالات الملفات التي
  تجعل الحزم يفشل.
- أزِل `private: true` إذا كانت هذه الحزمة مخصّصة للنشر العام.
- شغّل `clawhub package validate <path-to-plugin>` مرة أخرى.

### package-npm-pack-entrypoint-missing

يمكن حزم الحزمة، لكن الأثر المحزوم لا يتضمن
ملفات نقطة الدخول المعلنة في `package.json#openclaw`.

- شغّل `npm pack --dry-run` وافحص الملفات التي ستُضمَّن.
- ابنِ نقاط الدخول المولّدة قبل الحزم.
- حدّث `files` أو `.npmignore` أو مخرجات البناء بحيث تُضمَّن
  نقاط الدخول المعلنة.
- راجع [نقاط دخول Plugin](/ar/plugins/sdk-entrypoints).
- شغّل `clawhub package validate <path-to-plugin>` مرة أخرى.
...
- تأكد من تضمين `openclaw.plugin.json` عندما تكون الحزمة Plugin أصليًا
  لـ OpenClaw.
- حدّث `files` أو `.npmignore` كي لا تُستبعد بيانات تعريف الحزمة.
...
- أبقِ `name` قابلاً للقراءة البشرية، وأبقِ `id` معرّف الآلة المستقر.
- راجع [بيان Plugin](/ar/plugins/manifest).
- أعد تشغيل `clawhub package validate <path-to-plugin>`.
...
- انقل بيانات تعريف الحزمة أو التثبيت إلى حقول `package.json#openclaw`
...
- انقل سلوك وقت التشغيل إلى كود تسجيل Plugin، وأبقِ `contracts`
  مقتصراً على بيانات تعريف ملكية القدرات الثابتة.
- أعد تشغيل `clawhub package validate <path-to-plugin>`.
...
- استبدل واردات الحزمة الجذرية بواردات مسارات فرعية عامة ومحددة.
...
- استخدم [اصطلاحات الاستيراد](/ar/plugins/building-plugins#import-conventions) و
  [المسارات الفرعية لـ SDK الخاص بـ Plugin](/ar/plugins/sdk-subpaths) للعثور على الاستيراد الضيق.
- أعد تشغيل `clawhub package validate <path-to-plugin>`.
...
- استخدم [المسارات الفرعية لـ SDK الخاص بـ Plugin](/ar/plugins/sdk-subpaths) و
...
- استخدم `patchSessionEntry(...)` عند تحديث حقول في إدخال جلسة قائم.
...
لا يزال Plugin يستخدم مساعدات مسارات ملفات الجلسة المهملة مثل
`resolveSessionFilePath` أو `resolveAndPersistSessionFile`.

- استخدم `getSessionEntry(...)` لقراءة بيانات تعريف الجلسة حسب هوية الوكيل
  والجلسة.
- استخدم `patchSessionEntry(...)` أو `upsertSessionEntry(...)` لحفظ بيانات تعريف
  الجلسة.
- استخدم هوية النص أو مساعدات الهدف عندما يجهز الكود عملية
  نص.
...
- استخدم `resolveSessionTranscriptIdentity(...)` عندما يحتاج الكود إلى هوية
  الجلسة العامة فقط.
...
يستخدم البيان بيانات تعريف متغيرات بيئة قناة قديمة أو أقدم بدون بيانات تعريف
...
- انسخ إعداد القناة المدفوع بمتغيرات البيئة إلى بيانات تعريف الإعداد الحالي أو تكوين القناة أو
  بيانات تعريف قناة الحزمة المستخدمة في شكل Plugin لديك.
