...
x-i18n:
    generated_at: "2026-07-05T06:34:35Z"
    model: gpt-5.5
...
يتضمن رموز صيانة للماسح من دون إرشادات معالجة للمؤلف، فهذه
مخصصة لمشرفي OpenClaw وليست لمؤلفي Plugin.
...
| `package-manifest-version-drift`        | [وائم إصدارات الحزمة والبيان](/ar/clawhub/plugin-validation-fixes#package-manifest-version-drift)                          |
...
- استخدم [بناء Plugin](/ar/plugins/building-plugins) للحصول على مثال حزمة
  بالحد الأدنى و[بيان Plugin](/ar/plugins/manifest#manifest-versus-packagejson)
...
- أضف بيانات تعريف التوافق والتثبيت عندما تُنشر الحزمة أو
  تُثبّت عبر ClawHub.
...
- أصلح بيانات تعريف الحزمة غير الصالحة، أو سكربتات دورة الحياة المعطّلة، أو إدخالات الملفات التي
  تجعل الحزم يفشل.
...
- اجعل `name` مقروءا للبشر، وأبق `id` كمعرف آلي ثابت.
- راجع [بيان Plugin](/ar/plugins/manifest).
- أعد تشغيل `clawhub package validate <path-to-plugin>`.
...
- انقل سلوك وقت التشغيل إلى كود تسجيل Plugin، وأبق `contracts`
  مقتصرا على بيانات وصفية ثابتة لملكية الإمكانات.
- أعد تشغيل `clawhub package validate <path-to-plugin>`.
...
يستورد Plugin مسارا من SDK محجوزا للـ Plugins المضمنة أو للتوافق
الداخلي.
...
- أبق `loadSessionStore(...)` فقط ما دام نطاق التوافق المصرح به لديك
...
- تجنب تحميل كائن مخزن الجلسة الكامل وتعديله وحفظه.
- أبق مساعدات كتابة المخزن الكامل فقط ما دام نطاق التوافق المصرح به لديك
...
- استخدم `getSessionEntry(...)` لقراءة البيانات الوصفية للجلسة حسب هوية الوكيل
  والجلسة.
- استخدم `patchSessionEntry(...)` أو `upsertSessionEntry(...)` لاستبقاء البيانات
  الوصفية للجلسة.
- استخدم هوية النص المنسوخ أو مساعدات الهدف عندما يكون الكود يحضر
  عملية نص منسوخ.
- لا تستبق مسارات ملفات النصوص المنسوخة القديمة ولا تعتمد عليها.
- راجع [Runtime API](/ar/plugins/sdk-runtime#agent-session-state) و
  [المسارات الفرعية لـ Plugin SDK](/ar/plugins/sdk-subpaths).
- أعد تشغيل `clawhub package validate <path-to-plugin>`.
...
- استخدم `resolveSessionTranscriptIdentity(...)` عندما لا يحتاج الكود إلا إلى هوية
  الجلسة العامة.
...
- أبق المساعد القديم فقط ما دام نطاق التوافق المصرح به لديك ما زال
...
ما زال Plugin يستخدم مساعدات نص منسوخ منخفضة المستوى ومهملة مثل
...
- استخدم `publishSessionTranscriptUpdateByIdentity(...)` لإشعارات تحديث النص
  المنسوخ.
- فضل سطح وقت تشغيل النص المنسوخ المنظم حتى يتمكن OpenClaw من تطبيق
  حدود المعاملات ومعالجة الهوية الصحيحة.
- أبق مساعدات النص المنسوخ منخفضة المستوى فقط ما دام نطاق التوافق المصرح به لديك
  ما زال يدعم إصدارات OpenClaw الأقدم التي تتطلبها.
- راجع [Runtime API](/ar/plugins/sdk-runtime#agent-session-state) و
  [المسارات الفرعية لـ Plugin SDK](/ar/plugins/sdk-subpaths).
- أعد تشغيل `clawhub package validate <path-to-plugin>`.
...
- أبق `before_agent_start` فقط ما دام نطاق التوافق المصرح به لديك ما زال
  يدعم إصدارات OpenClaw الأقدم التي تتطلبه.
...
- اعكس بيانات env-var الوصفية للمزود إلى `setup.providers[].envVars`.
- أبق `providerAuthEnvVars` كبيانات وصفية للتوافق فقط ما دام نطاق
  OpenClaw المدعوم لديك ما زال يحتاج إليه.
...
يستخدم البيان بيانات وصفية قديمة أو أقدم لـ env-var الخاصة بالقناة دون بيانات
setup أو config الوصفية الحالية التي يتوقعها ClawHub.

- أبق بيانات env-var الوصفية للقناة تصريحية حتى يتمكن OpenClaw من فحص حالة
  setup دون تحميل وقت تشغيل القناة.
- اعكس إعداد القناة المعتمد على env إلى بيانات setup أو config الخاصة بالقناة أو
  بيانات الحزمة الوصفية للقناة الحالية التي يستخدمها شكل Plugin لديك.
- أبق `channelEnvVars` كبيانات وصفية للتوافق فقط ما دامت إصدارات
  OpenClaw الأقدم المدعومة ما زالت تتطلبه.
...
- أزل عنوان URL الخاص بالمخطط إذا كان إرشاديا فقط.
- استخدم مخططا موثقا ومحدد الإصدار فقط بعد أن ينشر OpenClaw واحدا.
- أعد تشغيل `clawhub package validate <path-to-plugin>`.
