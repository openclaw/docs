...
| `package-plugin-api-compat-missing`     | [صرّح بتوافق واجهة Plugin API](/ar/clawhub/plugin-validation-fixes#package-plugin-api-compat-missing)                          |
...
- استخدم إصدار OpenClaw plugin API أو حد semver الأدنى الذي بنيت واختبرت
  بناءً عليه.
- أبقِ هذا منفصلًا عن إصدار الحزمة. يصف إصدار الحزمة
  إصدار Plugin؛ ويصف `openclaw.compat.pluginApi` عقد API الخاص بالمضيف.
- راجع [حقول package.json التي تؤثر في الاكتشاف](/ar/plugins/manifest#packagejson-fields-that-affect-discovery).
- أعد تشغيل `clawhub package validate <path-to-plugin>`.
...
- اجعل `name` قابلا للقراءة البشرية، وأبق `id` معرّف الآلة الثابت.
- راجع [بيان Plugin](/ar/plugins/manifest).
- أعد تشغيل `clawhub package validate <path-to-plugin>`.
...
- انقل سلوك وقت التشغيل إلى كود تسجيل Plugin، وأبق `contracts`
  مقتصرا على بيانات ملكية القدرات الثابتة.
- أعد تشغيل `clawhub package validate <path-to-plugin>`.
...
- استبدل استيرادات المجمع الجذري باستيرادات مسارات فرعية عامة ومحددة.
...
يستورد Plugin مسار SDK محجوزا للـ Plugins المضمنة أو للتوافق الداخلي.

- استبدل استيرادات SDK الداخلية المحجوزة في OpenClaw بمسارات فرعية عامة موثقة من
  `openclaw/plugin-sdk/*`.
...
- استخدم `getSessionEntry(...)` لقراءة بيانات الجلسة الوصفية حسب هوية الوكيل والجلسة.
...
- استخدم `resolveSessionTranscriptTarget(...)` عندما يحتاج الكود إلى هدف عملية نص منسوخ منظم.
...
لا يزال Plugin يستخدم مساعدات نصوص منسوخة منخفضة المستوى ومهملة مثل
...
- فضّل سطح وقت تشغيل النصوص المنسوخة المنظم حتى يتمكن OpenClaw من تطبيق
  حدود المعاملات ومعالجة الهوية الصحيحة.
- أبق مساعدات النصوص المنسوخة منخفضة المستوى فقط ما دام نطاق التوافق المعلن لديك
  لا يزال يدعم إصدارات OpenClaw الأقدم التي تتطلبها.
- راجع [واجهة API لوقت التشغيل](/ar/plugins/sdk-runtime#agent-session-state) و
  [المسارات الفرعية لـ Plugin SDK](/ar/plugins/sdk-subpaths).
- أعد تشغيل `clawhub package validate <path-to-plugin>`.
...
- أبق `providerAuthEnvVars` كبيانات توافق وصفية فقط ما دام نطاق
...
- أبق بيانات متغيرات بيئة القناة الوصفية تصريحية حتى يتمكن OpenClaw من فحص حالة الإعداد
  دون تحميل وقت تشغيل القناة.
- انسخ إعداد القناة المعتمد على متغيرات البيئة إلى بيانات الإعداد الحالية أو تكوين القناة أو
  بيانات القناة الوصفية في الحزمة المستخدمة حسب شكل Plugin لديك.
- أبق `channelEnvVars` كبيانات توافق وصفية فقط ما دامت إصدارات
...
- أبق السلوك الحساس أمنيا موثقا في مستندات حزمتك العامة أو
