---
read_when:
    - شغّلت clawhub package validate وتحتاج إلى إصلاح نتائج الإضافة
...
x-i18n:
    generated_at: "2026-07-04T15:19:34Z"
    model: gpt-5.5
...
فحوصات الحزم الآلية. تغطي هذه الصفحة النتائج الموجهة للمؤلف، أي
...
يحتوي على رموز صيانة للماسح من دون إرشادات معالجة للمؤلف، فهذه
...
| `package-plugin-api-compat-missing`     | [صرح بتوافق واجهة API للـ Plugin](/ar/clawhub/plugin-validation-fixes#package-plugin-api-compat-missing)                          |
...
| `package-npm-pack-metadata-missing`     | [ضمّن بيانات التعريف في مخرجات npm pack](/ar/clawhub/plugin-validation-fixes#package-npm-pack-metadata-missing)                       |
...
لا يستطيع ClawHub تحديد طريقة تثبيت الحزمة أو تحديثها.
...
لا تصرح الحزمة بنطاق واجهة API للـ Plugin في OpenClaw الذي تدعمه.

- أضف `openclaw.compat.pluginApi` إلى `package.json`.
- استخدم إصدار واجهة API للـ Plugin في OpenClaw أو حد semver الأدنى الذي بنيت واختبرت
  وفقًا له.
...
- انشر إصدار حزمة جديدًا بعد تغيير بيانات تعريف منشورة.
...
- شغّل `npm pack --dry-run` وافحص الملفات التي ستُضمن.
- ابنِ نقاط الدخول المولدة قبل الحزم.
- حدّث `files` أو `.npmignore` أو مخرجات البناء حتى تُضمن نقاط الدخول
  المصرح بها.
- راجع [نقاط دخول Plugin](/ar/plugins/sdk-entrypoints).
- أعد تشغيل `clawhub package validate <path-to-plugin>`.
...
- اجعل `name` مقروءًا للبشر، وأبقِ `id` معرّف الآلة المستقر.
- راجع [بيان Plugin](/ar/plugins/manifest).
- أعد تشغيل `clawhub package validate <path-to-plugin>`.
...
- انقل سلوك وقت التشغيل إلى كود تسجيل Plugin، وأبقِ `contracts`
  مقتصرًا على بيانات وصفية ثابتة لملكية القدرات.
- أعد تشغيل `clawhub package validate <path-to-plugin>`.
...
يستورد Plugin مسار SDK محجوزًا لـ Plugins المضمّنة أو للتوافق
الداخلي.
...
- استخدم `resolveSessionTranscriptIdentity(...)` عندما يحتاج الكود إلى هوية الجلسة
  العامة فقط.
...
يستخدم البيان بيانات وصفية قديمة أو أقدم لمتغيرات بيئة القناة بدون بيانات
...
- انسخ إعداد القناة المعتمد على البيئة إلى بيانات الإعداد الحالية أو تكوين القناة أو
  بيانات القناة الوصفية في الحزمة التي يستخدمها شكل Plugin لديك.
