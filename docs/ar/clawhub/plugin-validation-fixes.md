...
summary: أصلح نتائج التحقق من صحة حزمة Plugin في ClawHub قبل النشر
title: إصلاحات التحقق من صحة Plugin
x-i18n:
    generated_at: "2026-07-02T00:56:23Z"
    model: gpt-5.5
...
عمليات فحص الحزم الآلية. تغطي هذه الصفحة النتائج الموجهة إلى المؤلفين، أي
...
على رموز صيانة للماسح من دون إرشادات معالجة للمؤلف، فهذه الرموز مخصصة
...
| `package-plugin-api-compat-missing`     | [صرح بتوافق واجهة API الخاصة بـ Plugin](/ar/clawhub/plugin-validation-fixes#package-plugin-api-compat-missing)                          |
...
تحتوي الحزمة على `package.json`، لكنها لا تصرح ببيانات تعريف حزمة
OpenClaw.
...
بيانات تعريف الحزمة موجودة، لكنها لا تصرح بنقطة دخول تشغيلية لـ
OpenClaw.
...
تصرح الحزمة بنقطة دخول لـ OpenClaw، لكن الملف المشار إليه مفقود
...
لا تصرح الحزمة بنطاق واجهة API الخاصة بـ Plugin في OpenClaw الذي تدعمه.

- أضف `openclaw.compat.pluginApi` إلى `package.json`.
- استخدم إصدار واجهة API الخاصة بـ Plugin في OpenClaw أو الحد الأدنى وفق semver الذي بنيت واختبرت
  عليه.
- أبقِ هذا منفصلًا عن إصدار الحزمة. يصف إصدار الحزمة
  إصدار Plugin؛ أما `openclaw.compat.pluginApi` فيصف عقد واجهة API للمضيف.
- راجع [حقول package.json التي تؤثر في الاكتشاف](/ar/plugins/manifest#packagejson-fields-that-affect-discovery).
- أعد تشغيل `clawhub package validate <path-to-plugin>`.
...
تحتوي كتلة `package.json#openclaw` على حقول ليست بيانات تعريف حزمة
OpenClaw مدعومة.
...
- أصلح بيانات تعريف الحزمة غير الصالحة، أو سكربتات دورة الحياة المعطلة، أو إدخالات files التي
  تجعل الحزم يفشل.
...
- أبقِ `name` مقروءًا للبشر وأبقِ `id` معرّف الآلة الثابت.
- راجع [بيان Plugin](/ar/plugins/manifest).
- أعد تشغيل `clawhub package validate <path-to-plugin>`.
...
- انقل سلوك وقت التشغيل إلى كود تسجيل Plugin، وأبقِ `contracts`
  مقتصرة على بيانات ملكية الإمكانات الثابتة.
- أعد تشغيل `clawhub package validate <path-to-plugin>`.
...
لا يزال Plugin يستخدم مساعد كتابة مهملًا لمخزن الجلسة الكامل مثل
...
- استخدم `patchSessionEntry(...)` أو `upsertSessionEntry(...)` لاستمرار بيانات الجلسة
  الوصفية.
- استخدم هوية السجل النصي أو مساعدات الهدف عندما يكون الكود يحضّر
  عملية سجل نصي.
...
- استخدم `resolveSessionTranscriptTarget(...)` عندما يحتاج الكود إلى هدف عملية
  سجل نصي منظّم.
- تجنب قراءة أهداف ملفات السجل النصي القديمة أو بنائها مباشرة.
...
لا يزال Plugin يستخدم مساعدات سجل نصي منخفضة المستوى ومهملة مثل
...
- فضّل سطح وقت تشغيل السجل النصي المنظّم كي يتمكن OpenClaw من تطبيق
  حدود المعاملات الصحيحة ومعالجة الهوية.
- أبقِ مساعدات السجل النصي منخفضة المستوى فقط ما دام نطاق التوافق المعلن لديك
  لا يزال يدعم إصدارات OpenClaw الأقدم التي تتطلبها.
- راجع [API وقت التشغيل](/ar/plugins/sdk-runtime#agent-session-state) و
  [المسارات الفرعية لـ Plugin SDK](/ar/plugins/sdk-subpaths).
- أعد تشغيل `clawhub package validate <path-to-plugin>`.
...
- انسخ إعداد القناة المدفوع بالبيئة إلى بيانات الإعداد أو تهيئة القناة أو
  بيانات القناة الوصفية للحزمة الحالية المستخدمة بواسطة شكل Plugin لديك.
...
تشحن الحزمة `openclaw.security.json` بمرجع مخطط لا يتعرف عليه ClawHub
على أنه متاح.

- أزِل عنوان URL للمخطط إذا كان للإرشاد فقط.
