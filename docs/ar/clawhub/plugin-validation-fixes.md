...
x-i18n:
    generated_at: "2026-06-30T14:01:41Z"
    model: gpt-5.5
...
عمليات فحص الحزم المؤتمتة. تغطي هذه الصفحة النتائج الموجّهة إلى المؤلفين، أي
...
توجد بيانات تعريف الحزمة، لكنها لا تصرّح بنقطة دخول تشغيل OpenClaw.
...
- افحص أي بيانات تعريف بناء OpenClaw في الحزمة، مثل إصدار OpenClaw
...
يختلف إصدار الحزمة وإصدار بيان Plugin.
...
- اجعل `name` مقروءًا للبشر، وأبقِ `id` بوصفه معرّف الآلة الثابت.
- راجع [بيان Plugin](/ar/plugins/manifest).
- أعد تشغيل `clawhub package validate <path-to-plugin>`.
...
- انقل البيانات الوصفية للحزمة أو التثبيت إلى حقول `package.json#openclaw`
...
يستورد Plugin مسار SDK محجوزًا للـ Plugins المضمنة أو للتوافق
الداخلي.
...
- استخدم `getSessionEntry(...)` لقراءة البيانات الوصفية للجلسة حسب هوية الوكيل
  والجلسة.
- استخدم `patchSessionEntry(...)` أو `upsertSessionEntry(...)` لاستدامة البيانات
  الوصفية للجلسة.
...
- فضّل سطح وقت تشغيل السجل النصي المنظّم حتى يتمكن OpenClaw من تطبيق
...
- اعكس البيانات الوصفية لمتغيرات بيئة المزوّد في `setup.providers[].envVars`.
...
- أبقِ البيانات الوصفية لمتغيرات بيئة القناة تصريحية حتى يتمكن OpenClaw من فحص حالة الإعداد
  دون تحميل وقت تشغيل القناة.
- اعكس إعداد القناة المعتمد على البيئة في بيانات الإعداد أو تهيئة القناة أو
...
- استخدم مخططًا موثقًا ومحدد الإصدار فقط بعد أن ينشر OpenClaw واحدًا.
- أعد تشغيل `clawhub package validate <path-to-plugin>`.
