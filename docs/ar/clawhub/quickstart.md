...
x-i18n:
    generated_at: "2026-05-11T20:26:00Z"
    model: gpt-5.5
    provider: openai
...
يسجّل OpenClaw مصدر Skill لكي تتمكن التحديثات اللاحقة من الاستمرار في
الحل عبر ClawHub.
...
يمكن للبيئات بلا واجهة تفاعلية استخدام رمز API من واجهة الويب في ClawHub:
...
Skill هي مجلد يحتوي على ملف `SKILL.md` مطلوب وملفات داعمة اختيارية.
...
قبل النشر، تحقق من البيانات الوصفية في `SKILL.md`. صرّح عن متغيرات
...
استخدم `--dry-run` أولاً لمعاينة بيانات الحزمة الوصفية التي تم حلها، وحقول
التوافق، وإسناد المصدر، وخطة الرفع من دون النشر.

يجب أن تتضمن code plugins بيانات وصفية للتوافق مع OpenClaw في `package.json`،
بما في ذلك `openclaw.compat.pluginApi` و`openclaw.build.openclawVersion`.

## مزامنة Skills التي تصونها

يفحص `sync` مجلدات Skills وينشر Skills الجديدة أو المعدلة التي لم تتم
مزامنتها بالفعل.
...
عندما تكون مسجّل الدخول، قد يرسل `sync` أيضًا لقطة تثبيت مصغّرة لأعداد
التثبيت الإجمالية. راجع [القياس عن بُعد](/ar/clawhub/telemetry) لمعرفة ما يتم
الإبلاغ عنه وكيفية إلغاء الاشتراك.
...
بسبب الإشراف من أسطح البحث والتثبيت إلى أن يتم حلها.
