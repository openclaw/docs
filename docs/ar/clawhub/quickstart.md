...
x-i18n:
    generated_at: "2026-07-05T06:25:12Z"
    model: gpt-5.5
...
ClawHub هو سجلّ لـ Skills وPlugins في OpenClaw.
...
يتجاوز الأمر المحتوى غير المتغيّر. تبدأ Skills الجديدة عند `1.0.0`؛ أما التغييرات اللاحقة
فتنشر تلقائيًا إصدار التصحيح التالي. استخدم `--dry-run` للمعاينة أو
`--version` لاختيار إصدار صريح.

قبل النشر، تحقّق من البيانات الوصفية في `SKILL.md`. صرّح بمتغيرات
البيئة والأدوات والأذونات المطلوبة حتى يتمكّن المستخدمون من فهم ما يحتاجه
Skill قبل تثبيته. راجع [تنسيق Skill](/ar/clawhub/skill-format).

بالنسبة إلى المستودعات التي تحتوي على عدة Skills، يستدعي مسار عمل GitHub القابل لإعادة الاستخدام
