...
- `tools.exec.host=auto` يختار **أين** يعمل exec: داخل sandbox عند توفرها، وإلا فعلى gateway.
- يختار YOLO **كيف** تُعتمد exec على المضيف: `security=full` مع `ask=off`.
- في وضع YOLO، لا يضيف OpenClaw **بوابة موافقة استدلالية منفصلة** لإخفاء الأوامر أو طبقة رفض تمهيدي للسكربتات فوق سياسة exec على المضيف المهيأة.
- لا يجعل `auto` توجيه gateway تجاوزًا مجانيًا من جلسة داخل sandbox. ويُسمح بطلب `host=node` لكل استدعاء من `auto`؛ أما `host=gateway` فلا يُسمح به من `auto` إلا عندما لا يكون هناك وقت تشغيل sandbox نشط. وللحصول على افتراضي ثابت غير `auto`، اضبط `tools.exec.host` أو استخدم `/exec host=...` صراحةً.
  </Warning>
