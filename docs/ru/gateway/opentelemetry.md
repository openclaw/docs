---
read_when:
    - Вы хотите отправлять метрики использования моделей OpenClaw, потока сообщений или сессий в коллектор OpenTelemetry
    - Вы подключаете трассы, метрики или журналы к Grafana, Datadog, Honeycomb, New Relic, Tempo или другому backend OTLP
    - Вам нужны точные имена метрик, имена span или формы атрибутов для создания дашбордов или оповещений
summary: Экспорт диагностики OpenClaw в коллекторы OpenTelemetry или JSONL в stdout через Plugin diagnostics-otel
...
конвейеров логов контейнеров и песочниц. Любой коллектор или бэкенд, принимающий
OTLP/HTTP, работает без изменений кода. О локальных файловых логах и том, как их читать,
см. [Ведение логов](/ru/logging).

## Как это устроено

- **События диагностики** — это структурированные внутрипроцессные записи, которые
  Gateway и встроенные Plugin создают для запусков моделей, потока сообщений, сессий, очередей,
  и exec.
...
- **Вызовы провайдеров** получают заголовок W3C `traceparent` из принадлежащего OpenClaw
  контекста доверенного span вызова модели, когда транспорт провайдера принимает пользовательские
  заголовки. Контекст трейса, созданный Plugin, не распространяется.
- Экспортеры подключаются только когда включены и диагностическая поверхность, и Plugin,
...
| Сигнал      | Что в него попадает                                                                                                                                                                                                    |
| ----------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| **Метрики** | Счетчики и гистограммы для использования токенов, стоимости, длительности запуска, переключения при отказе, использования skill, потока сообщений, событий Talk, полос очередей, состояния/восстановления сессии, выполнения инструментов, слишком больших полезных нагрузок, exec и давления на память. |
...
| **Логи**    | Структурированные записи `logging.file`, экспортируемые через OTLP или stdout JSONL, когда включено `diagnostics.otel.logs`; тела логов скрываются, если захват содержимого не включен явно.                                |

Переключайте `traces`, `metrics` и `logs` независимо. Трейсы и метрики
по умолчанию включены, когда `diagnostics.otel.enabled` имеет значение true. Логи по умолчанию выключены и
...
stdout или `both`, чтобы отправлять каждую диагностическую запись лога в OTLP и stdout.
...
| `OTEL_EXPORTER_OTLP_TRACES_ENDPOINT` / `OTEL_EXPORTER_OTLP_METRICS_ENDPOINT` / `OTEL_EXPORTER_OTLP_LOGS_ENDPOINT` | Переопределения конечных точек для отдельных сигналов, используемые когда соответствующий ключ конфигурации `diagnostics.otel.*Endpoint` не задан. Конфигурация для конкретного сигнала имеет приоритет над env для конкретного сигнала, а та имеет приоритет над общей конечной точкой.                                                                                                                                         |
| `OTEL_SERVICE_NAME`                                                                                               | Переопределяет `diagnostics.otel.serviceName`.                                                                                                                                                                                                                                                                                                       |
| `OTEL_EXPORTER_OTLP_PROTOCOL`                                                                                     | Переопределяет сетевой протокол (сегодня учитывается только `http/protobuf`).                                                                                                                                                                                                                                                                            |
| `OTEL_SEMCONV_STABILITY_OPT_IN`                                                                                   | Установите `gen_ai_latest_experimental`, чтобы создавать новейшую экспериментальную форму span вывода GenAI, включая имена span `{gen_ai.operation.name} {gen_ai.request.model}`, вид span `CLIENT` и `gen_ai.provider.name` вместо устаревшего `gen_ai.system`. Метрики GenAI всегда используют ограниченные семантические атрибуты с низкой кардинальностью независимо от этого. |
| `OPENCLAW_OTEL_PRELOADED`                                                                                         | Установите `1`, когда другой preload или процесс хоста уже зарегистрировал глобальный OpenTelemetry SDK. Тогда Plugin пропускает собственный жизненный цикл NodeSDK, но все равно подключает диагностические слушатели и соблюдает `traces`/`metrics`/`logs`.                                                                                                                    |

## Конфиденциальность и захват содержимого

Сырые данные модели/инструментов **не** экспортируются по умолчанию. Spans несут ограниченные
идентификаторы (канал, провайдер, модель, категорию ошибки, идентификаторы запросов только в виде хэша,
источник инструмента, владельца инструмента и имя/источник skill) и никогда не включают текст prompt,
текст ответа, входные данные инструмента, выходные данные инструмента, пути к файлам skill или ключи сессии.
...
только когда `diagnostics.otel.captureContent` задан как boolean `true`. Детальные
...
провайдер и тип события. Они не включают транскрипты, аудио-полезные нагрузки,
идентификаторы сессий, turn ids, call ids, room ids или токены передачи.

Исходящие запросы к модели могут включать заголовок W3C `traceparent`. Этот заголовок
создается только из принадлежащего OpenClaw диагностического контекста трейса для активного вызова модели.
Существующие предоставленные вызывающей стороной заголовки `traceparent` заменяются, поэтому Plugin или
...
- `systemPrompt` - собранный системный/developer prompt.
...
Содержимое `toolInputs`/`toolOutputs` захватывается для выполнений инструментов встроенной среды выполнения агента
(`openclaw.content.tool_input` на spans завершения/ошибки,
`openclaw.content.tool_output` на spans завершения). Вызовы инструментов внешних harness
...
- **Логи:** логи OTLP учитывают `logging.level` (уровень файлового лога). Они используют
  путь редактирования диагностических log-record, а не форматирование консоли. Установки с
  высокой нагрузкой должны предпочитать выборку/фильтрацию в коллекторе OTLP локальной выборке.
  Установите `diagnostics.otel.logsExporter: "stdout"`, если ваша платформа уже
  отправляет stdout/stderr в обработчик логов и у вас нет коллектора логов OTLP.
  Записи stdout — это один объект JSON на строку с `ts`, `signal`,
  `service.name`, уровнем серьезности, телом, отредактированными атрибутами и доверенными полями трассировки,
  когда они доступны.
- **Корреляция файловых логов:** файловые логи JSONL включают поля верхнего уровня `traceId`,
  `spanId`, `parentSpanId` и `traceFlags`, когда вызов логирования несет валидный
  диагностический контекст трассировки, что позволяет обработчикам логов связывать локальные строки логов с
  экспортированными span.
- **Корреляция запросов:** HTTP-запросы Gateway и кадры WebSocket создают
  ...
  по умолчанию наследуют трассировку запроса, а span запуска агента и вызова модели
  создаются как дочерние, чтобы заголовки `traceparent` провайдера оставались в той же трассировке.
- **Корреляция вызовов модели:** span `openclaw.model.call` по умолчанию включают безопасные размеры
  компонентов prompt и включают атрибуты токенов для каждого вызова, когда
  результат провайдера раскрывает usage. `openclaw.model.usage` остается span учета на уровне запуска
  для совокупной стоимости, контекста и панелей каналов; он остается
  в той же диагностической трассировке, когда испускающая runtime-среда имеет доверенный
...
- `openclaw.model_call.response_bytes` (гистограмма, размер в байтах UTF-8 полезных нагрузок фрагментов потокового ответа; высокочастотный текст, thinking и дельты вызовов инструментов учитывают только добавочные байты `delta`; без сырого содержимого ответа)
...
- `openclaw.session.stuck` (счетчик, атрибуты: `openclaw.state`; испускается для восстанавливаемого устаревшего учета сеансов)
- `openclaw.session.stuck_age_ms` (гистограмма, атрибуты: `openclaw.state`; испускается для восстанавливаемого устаревшего учета сеансов)
...
активности сеанса. Сеанс `processing` не стареет относительно этого порога,
пока OpenClaw наблюдает прогресс ответа, инструмента, статуса, блока или runtime ACP.
Keepalive-события набора текста не считаются прогрессом, поэтому тихая модель или harness
все равно могут быть обнаружены.

OpenClaw классифицирует сеансы по работе, которую все еще может наблюдать:

- `session.long_running`: активная встроенная работа, вызовы модели или вызовы инструментов
  все еще прогрессируют. Принадлежащие вызовы модели, которые остаются тихими дольше
  `diagnostics.stuckSessionWarnMs`, также сообщаются как долгие перед
  `diagnostics.stuckSessionAbortMs`, чтобы медленные или непотоковые провайдеры моделей
  не выглядели как зависшие сеансы Gateway, пока их по-прежнему можно наблюдаемо прервать.
- `session.stalled`: активная работа существует, но активный запуск не сообщал
  недавний прогресс. Принадлежащие вызовы модели переключаются с `session.long_running` на
  `session.stalled` на уровне или после `diagnostics.stuckSessionAbortMs`; бесхозная
  устаревшая активность модели/инструмента не считается безвредной долгой работой.
  Зависшие встроенные запуски сначала остаются только наблюдаемыми, затем выполняют abort-drain после
  `diagnostics.stuckSessionAbortMs` без прогресса, чтобы ожидающие в очереди за этой lane повороты
  могли возобновиться. Если значение не задано, порог прерывания по умолчанию равен более безопасному
  расширенному окну не менее 5 минут и 3x
...
- `session.stuck`: устаревший учет сеанса без активной работы или простаивающий
  ...
  затронутую lane сеанса сразу после прохождения gate восстановления.

Восстановление испускает структурированные события `session.recovery.requested` и
`session.recovery.completed`. Диагностическое состояние сеанса помечается как idle
только после мутирующего результата восстановления (`aborted` или `released`) и только если
то же поколение processing все еще актуально.
...
Повторяющиеся диагностики `session.stuck` выполняют backoff, пока сеанс остается
неизменным, поэтому панели должны предупреждать о устойчивых увеличениях, а не о каждом
тике Heartbeat. Ручку конфигурации и значения по умолчанию см. в
[справочнике конфигурации](/ru/gateway/configuration-reference#diagnostics).
...
- `openclaw.tool.execution.duration_ms` (гистограмма, атрибуты: `gen_ai.tool.name`, `openclaw.toolName`, `openclaw.tool.source`, `openclaw.tool.owner`, `openclaw.tool.params.kind`, плюс `openclaw.errorCategory` при ошибках)
...
### Внутренние диагностические метрики (память и цикл инструментов)
...
- `openclaw.model.call`
  ...
  - `openclaw.model_call.prompt.input_messages_count`, `openclaw.model_call.prompt.input_messages_chars`, `openclaw.model_call.prompt.system_prompt_chars`, `openclaw.model_call.prompt.tool_definitions_count`, `openclaw.model_call.prompt.tool_definitions_chars`, `openclaw.model_call.prompt.total_chars` (только безопасные размеры компонентов, без текста промпта)
  - `openclaw.model_call.usage.*` и `gen_ai.usage.*`, когда результат вызова модели содержит данные использования провайдера для этого отдельного вызова
  - `openclaw.provider.request_id_hash` (ограниченный SHA-хеш идентификатора запроса вышестоящего провайдера; исходные идентификаторы не экспортируются)
  - С `OTEL_SEMCONV_STABILITY_OPT_IN=gen_ai_latest_experimental` спаны вызова модели используют новейшее имя спана инференса GenAI `{gen_ai.operation.name} {gen_ai.request.model}` и вид спана `CLIENT` вместо `openclaw.model.call`.
...
- `openclaw.context.assembled`
  - `openclaw.prompt.size`, `openclaw.history.size`, `openclaw.context.tokens`, `openclaw.errorCategory` (без промпта, истории, ответа или содержимого ключа сессии)
- `openclaw.tool.loop`
...
События ниже поддерживают метрики и спаны выше. Plugins также могут подписываться
...
- `model.usage` - токены, стоимость, длительность, контекст, провайдер/модель/канал,
  ...
  `context.used` — это текущий снимок промпта/контекста, и он может быть ниже, чем
...
- `exec.process.completed` - терминальный результат, длительность, цель, режим, код
  выхода и вид сбоя. Текст команды и рабочие каталоги не
  включаются.
- `exec.approval.followup_suppressed` - устаревшее продолжение подтверждения отброшено после
  восстановления сессии. Включает `approvalId`, `reason` (`session_rebound`),
  `phase` (`direct_delivery` или `gateway_preflight`) и отметку времени диспетчера.
  Ключи сессий, маршруты и текст команды не включаются.

## Без экспортера

Вы можете сделать диагностические события доступными для Plugins или пользовательских приемников без
...
флаги. Флаги нечувствительны к регистру и поддерживают подстановочные знаки (например, `telegram.*` или
`*`):
...
- [Журналирование](/ru/logging) - файловые журналы, вывод в консоль, отслеживание через CLI и вкладка журналов Control UI
...
- [Экспорт диагностики](/ru/gateway/diagnostics) - инструмент пакета поддержки оператора (отдельно от экспорта OTEL)
