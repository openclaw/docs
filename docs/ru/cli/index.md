...
x-i18n:
    generated_at: "2026-07-13T19:37:34Z"
    model: gpt-5.6
    postprocess_version: locale-links-v1
    prompt_version: 24
    provider: openai
    source_hash: 607f8936bc221f1c9a3001b0fec3ee2cf490444886169a6302ce4fe21e0bc04b
    source_path: cli/index.md
    workflow: 16
---

`openclaw` — основная точка входа CLI. Каждой основной команде посвящена отдельная
страница справочника либо она документирована вместе с командой, псевдонимом которой является; в этом указателе перечислены
команды, глобальные флаги и правила оформления вывода, применяемые во всём CLI.

Команды настройки по назначению:

- `openclaw setup` и `openclaw onboard` сначала проверяют инференс, затем запускают Crestodian для настройки Gateway, рабочего пространства, каналов, навыков и проверки работоспособности.
...
- `openclaw configure` изменяет отдельные части существующей конфигурации: аутентификацию модели, Gateway, каналы, плагины или навыки.
- `openclaw channels add` настраивает учётные записи каналов после создания базовой конфигурации; запускайте без флагов для пошаговой настройки или с флагами конкретных каналов для использования в скриптах.
...
| Модели и инференс            | [`models`](/ru/cli/models) · [`promos`](/ru/cli/promos) · [`infer`](/ru/cli/infer) · `capability` (псевдоним для [`infer`](/ru/cli/infer)) · [`memory`](/ru/cli/memory) · [`commitments`](/ru/cli/commitments) · [`wiki`](/ru/cli/wiki)                            |
...
| `--update`              | Сокращение для [`openclaw update`](/ru/cli/update); работает как для рабочих копий исходного кода, так и для установок пакета |
| `-V`, `--version`, `-v` | Вывести версию и завершить работу                                                                       |

## Режимы вывода

- Цвета ANSI и индикаторы прогресса отображаются только в сеансах TTY.
- Гиперссылки OSC-8 отображаются как кликабельные ссылки там, где это поддерживается; в противном случае
  CLI использует обычные URL-адреса.
- `--json` (и `--plain`, где поддерживается) отключает оформление для чистого вывода.
- Длительные команды отображают индикатор прогресса (OSC 9;4, если поддерживается).
...
| `accentBright` | `#FF7A3D` | Имена команд, акцентирование         |
...
`openclaw status --usage` и интерфейс Control UI отображают использование и квоты провайдера, когда
