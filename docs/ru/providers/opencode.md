...
`OPENCODE_ZEN_API_KEY`). OpenClaw разделяет идентификаторы провайдеров среды выполнения,
чтобы маршрутизация отдельных моделей на стороне вышестоящего сервиса оставалась корректной,
однако при первоначальной настройке и в документации они рассматриваются как единая конфигурация OpenCode.
...
который также включает позиции бесплатного уровня, например `opencode/big-pickle` и
...
<AccordionGroup>
  <Accordion title="Псевдонимы ключа API">
    `OPENCODE_ZEN_API_KEY` также принимается в качестве псевдонима для `OPENCODE_API_KEY`.
  </Accordion>

  <Accordion title="Общие учётные данные">
    При вводе одного ключа OpenCode во время настройки сохраняются учётные данные для обоих
    провайдеров среды выполнения. Выполнять первоначальную настройку каждого каталога отдельно не требуется.
  </Accordion>

  <Accordion title="Получение ключа API">
    Создайте учётную запись OpenCode и сгенерируйте ключ API на странице
    [opencode.ai/auth](https://opencode.ai/auth). Оплата и доступность каталогов
    управляются в панели OpenCode.
  </Accordion>

  <Accordion title="Поведение воспроизведения Gemini">
    Ссылки OpenCode на базе Gemini остаются на прокси-пути Gemini, поэтому OpenClaw продолжает
    очищать сигнатуры рассуждений Gemini, не включая собственную проверку воспроизведения Gemini
    или перезапись начальной загрузки.
  </Accordion>

  <Accordion title="Поведение воспроизведения моделей, отличных от Gemini">
    Ссылки OpenCode, не относящиеся к Gemini, сохраняют минимальную политику воспроизведения,
...
<CardGroup cols={2}>
  <Card title="OpenCode Go" href="/ru/providers/opencode-go" icon="server">
    Полный справочник по каталогу Go.
  </Card>
