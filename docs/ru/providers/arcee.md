...
read_when:
    ...
    - Вам нужна переменная окружения с ключом API или вариант аутентификации через CLI.
summary: Настройка Arcee AI (аутентификация и выбор модели)
title: Arcee AI
x-i18n:
    generated_at: "2026-07-13T20:11:18Z"
    model: gpt-5.6
...
[Arcee AI](https://arcee.ai) предоставляет семейство моделей Trinity на основе смеси экспертов через API, совместимый с OpenAI. Все модели Trinity распространяются по лицензии Apache 2.0. Arcee — официальный плагин OpenClaw, который не входит в состав ядра, поэтому перед первоначальной настройкой его необходимо установить.

Получить доступ к моделям Arcee можно напрямую через платформу Arcee или через [OpenRouter](/ru/providers/openrouter).
  ...
  <Tab title="Через OpenRouter">
    <Steps>
      ...
      <Step title="Задайте модель по умолчанию">
        ...
        Одни и те же ссылки на модели работают как при прямой настройке, так и при настройке через OpenRouter.
      </Step>
    </Steps>
...
| Ссылка на модель               | Название               | Входные данные | Контекст | Макс. объём вывода | Стоимость (вход/выход за 1 млн) | Инструменты | Примечания                                |
| ------------------------------ | ---------------------- | ------------- | -------- | ------------------ | -------------------------------- | ----------- | ----------------------------------------- |
| `arcee/trinity-large-thinking` | Trinity Large Thinking | текст         | 256K     | 80K                | $0.25 / $0.90                    | Нет         | Модель по умолчанию; расширенное мышление |
| `arcee/trinity-large-preview`  | Trinity Large Preview  | текст         | 128K     | 16K                | $0.25 / $1.00                    | Да          | Универсальная; 400B параметров, 13B активных |
| `arcee/trinity-mini`           | Trinity Mini 26B       | текст         | 128K     | 80K                | $0.045 / $0.15                   | Да          | Быстрая и экономичная; вызов функций      |

<Tip>
Предустановка первоначальной настройки задаёт `arcee/trinity-large-thinking` в качестве модели по умолчанию.
</Tip>
...
<AccordionGroup>
  <Accordion title="Примечание об окружении">
    ...
    (или `OPENROUTER_API_KEY`) доступна этому процессу, например в
  ...
  <Accordion title="Маршрутизация OpenRouter">
    ...
    OpenClaw прозрачно выполняет маршрутизацию в соответствии с выбранным способом аутентификации. Подробные
    сведения о настройке OpenRouter см. в [документации провайдера OpenRouter](/ru/providers/openrouter).
  </Accordion>
</AccordionGroup>
