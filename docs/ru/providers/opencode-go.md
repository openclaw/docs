...
title: OpenCode Go
x-i18n:
    generated_at: "2026-07-13T20:13:43Z"
    model: gpt-5.6
...
общие с каталогом Zen учётные данные `OPENCODE_API_KEY`, но сохраняет собственный
идентификатор провайдера среды выполнения (`opencode-go`), чтобы вышестоящая маршрутизация
для каждой модели оставалась корректной.
...
<AccordionGroup>
  <Accordion title="Поведение маршрутизации">
    OpenClaw автоматически маршрутизирует любую ссылку на модель `opencode-go/...`. Дополнительная
    конфигурация провайдера не требуется.
  </Accordion>

  <Accordion title="Соглашение о ссылках среды выполнения">
    Ссылки среды выполнения остаются явными: `opencode/...` для Zen, `opencode-go/...` для
    Go. Это обеспечивает корректную вышестоящую маршрутизацию для каждой модели в обоих каталогах.
  </Accordion>

  <Accordion title="Общие учётные данные">
    Один `OPENCODE_API_KEY` охватывает каталоги Zen и Go. При вводе
    ключа во время настройки учётные данные сохраняются для обоих провайдеров среды выполнения.
  </Accordion>
</AccordionGroup>

<Tip>
Общие сведения о первоначальной настройке и полный справочник каталогов
Zen и Go см. в разделе [OpenCode](/ru/providers/opencode).
</Tip>
