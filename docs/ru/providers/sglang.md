...
title: SGLang
x-i18n:
    generated_at: "2026-07-13T18:31:17Z"
    model: gpt-5.6
...
| Переменная окружения для аутентификации              | `SGLANG_API_KEY` (любое непустое значение, если сервер не использует аутентификацию) |
...
| Использование потоковой передачи           | Да (`supportsStreamingUsage: true`)                         |
| Тарификация                   | Помечено как бесплатное внешнее использование (`modelPricing.external: false`)        |

OpenClaw также **автоматически обнаруживает** доступные модели в SGLang, когда вы включаете эту возможность с помощью `SGLANG_API_KEY`. Используйте `sglang/*` в `agents.defaults.models`, чтобы обнаружение оставалось динамическим, если вы также настраиваете пользовательский базовый URL SGLang. См. раздел [Обнаружение моделей (неявный провайдер)](#model-discovery-implicit-provider) ниже.
...
Если задано `SGLANG_API_KEY` (или существует профиль аутентификации) и вы **не**
определили `models.providers.sglang`, OpenClaw выполняет запрос к:
...
<Note>
Если вы явно зададите `models.providers.sglang`, OpenClaw по умолчанию будет использовать объявленные
вами модели. Добавьте `"sglang/*": {}` в `agents.defaults.models`, если вы
хотите, чтобы OpenClaw запрашивал конечную точку `/models` настроенного провайдера и включал
...
- SGLang работает на другом узле или порте.
...
<AccordionGroup>
  <Accordion title="Поведение в режиме прокси">
    SGLang рассматривается как прокси-серверная часть `/v1`, совместимая с OpenAI, а не как
    нативная конечная точка OpenAI.

    | Поведение | SGLang |
    |----------|--------|
    | Формирование запросов только для OpenAI | Не применяется |
    | `service_tier`, Responses `store`, подсказки для кэша промптов | Не отправляются |
    | Формирование полезной нагрузки для совместимости с рассуждениями | Не применяется |
    | Скрытые заголовки атрибуции (`originator`, `version`, `User-Agent`) | Не добавляются для пользовательских базовых URL SGLang |
  ...
  <Accordion title="Устранение неполадок">
    ...
    **Ошибки аутентификации**
    ...
    Если запросы завершаются ошибками аутентификации, задайте настоящий `SGLANG_API_KEY`, соответствующий
    конфигурации вашего сервера, либо явно настройте провайдера в
    `models.providers.sglang`.

    <Tip>
    Если вы запускаете SGLang без аутентификации, любого непустого значения
