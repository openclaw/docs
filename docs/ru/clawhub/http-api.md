...
x-i18n:
    generated_at: "2026-07-03T23:38:42Z"
    model: gpt-5.5
...
Сторонние каталоги могут использовать публичные конечные точки чтения, чтобы перечислять или искать навыки ClawHub. Кэшируйте результаты, соблюдайте `429`/`Retry-After`, направляйте пользователей обратно к канонической странице ClawHub (`https://clawhub.ai/<owner>/skills/<slug>`) и не создавайте впечатление, что ClawHub одобряет сторонний сайт. Не пытайтесь зеркалировать скрытый, приватный или заблокированный модерацией контент за пределами публичной поверхности API.

Веб-сокращения slug разрешаются по семействам реестра, но API-клиентам следует использовать
...
- Анонимные запросы: ограничение применяется по IP.
- Аутентифицированные запросы (действительный Bearer-токен): ограничение применяется по пользовательскому бакету.
...
- Аутентифицированные конечные точки записи не должны возвращать голое `Unauthorized`, когда
  ...
  удаленные, заблокированные или отключенные учетные записи должны получать применимый текст, чтобы CLI
  мог объяснить пользователям, что их заблокировало.
...
- `highlightedOnly` (необязательный): `true`, чтобы отфильтровать только выделенные навыки
...
- Результаты возвращаются в порядке релевантности (сходство embedding + усиления для точного токена slug/имени + небольшой предварительный фактор популярности).
- Релевантность сильнее популярности. Точное совпадение slug или токена отображаемого имени может обойти более свободное совпадение с гораздо более высоким вовлечением.
...
- Популярность логарифмически масштабируется и ограничивается сверху. Навыки с высоким вовлечением могут ранжироваться ниже, когда текст запроса совпадает слабее.
...
- Помещайте термины, которые пользователи будут буквально искать, в отображаемое имя, сводку и теги. Используйте самостоятельный токен slug только тогда, когда он также является стабильной идентичностью, которую вы хотите сохранить.
...
- Псевдонимы переименования сохраняют разрешение для старых URL и установок, которые разрешаются через реестр, но поисковый рейтинг основан на канонических метаданных навыка после индексации переименования. Существующая статистика остается у навыка.
...
- `recommended` использует сигналы вовлечения и новизны.
...
- Когда `nonSuspiciousOnly=true`, сортировки на основе курсора могут возвращать на странице меньше элементов, чем `limit`, потому что подозрительные навыки фильтруются после получения страницы.
...
- Старые slug, созданные потоками переименования или слияния владельца, разрешаются в канонический навык.
...
Сообщить о навыке для проверки модератором. Отчеты относятся к уровню навыка, необязательно привязаны
к версии и попадают в очередь отчетов по навыкам.
...
Конечная точка модератора/администратора для приема отчетов по навыкам.
...
Конечная точка модератора/администратора для разрешения или повторного открытия отчетов по навыкам.
...
отчетом, чтобы скрыть навык в том же проверяемом workflow.
...
Возвращает подробности проверки сканирования безопасности для версии навыка.
...
- Возвращает статус в очереди/выполняется/успешно/неуспешно.
...
- `kind` по умолчанию равно `skill`; используйте `kind=plugin` для сканирований плагинов/пакетов.
...
- `provenance` равно `server-resolved-github-import` только когда ClawHub разрешил и сохранил репозиторий/ref/commit/path GitHub во время публикации или импорта; иначе значение — `unavailable`.
...
- `security.signals.dependencyRegistry` сохранен для совместимости ответа v1, но сканер существования реестра зависимостей выведен из эксплуатации, и этот ключ всегда равен `null`.
- Отсутствие Skill Card не влияет на `ok`, `decision` или `reasons` этой конечной точки; клиенты должны локально читать установленный `skill-card.md`, когда им нужно содержимое карточки.
- Используйте `/verify`, когда вам нужен конверт проверки Skill Card для одного навыка, `/card`, когда нужен сгенерированный Markdown карточки, и `/scan`, когда нужны подробные данные сканеров.
...
`trending` — это рейтинг установок/загрузок за семь дней, который не использует суммарные показатели за всё время.
В единой конечной точке `/api/v1/packages` он доступен только для plugin; используйте
...
- Тело: ZIP-архив.
- Каждый экспортированный Skills имеет корень в `{publisher}/{slug}/`.
...
- Тело: ZIP-архив.
- Каждый экспортированный Plugin имеет корень в `{family}/{packageName}/`.
...
- Устаревшие псевдонимы фильтров v1, документированные в `GET /api/v1/plugins`, также
  принимаются.
- Фильтрация по категории является настоящим фильтром API, основанным на строках
...
- Приватные пакеты возвращают `404`, если вызывающий пользователь не может читать владеющего издателя.
...
- `trust.reasons` — пользовательский и аудиторский список объяснений. Коды причин
  ...
  и `package:malicious`.
- `trust.pending` означает, что один или несколько входных сигналов доверия еще ожидают завершения.
...
- `quarantined`: заблокирован до дальнейшей проверки.
...
- Packument для scoped-пакетов поддерживает и `/api/npm/@scope/name`, и npm-путь запроса
  в закодированном виде `/api/npm/@scope%2Fname`.
...
Скачивает ZIP размещенной версии навыка или возвращает передачу исходного кода GitHub для
...
- Необязательное поле payload: `migrateOwner`. Когда `true` с `ownerHandle`,
  существующий навык может перейти к этому владельцу, если действующий пользователь является администратором/владельцем и у текущего,
...
- Разрешенные поля формы: `payload`, повторяющиеся blob-объекты `files` или одна ссылка на tarball
  ...
  flow upload-url. Публикации staged storage-id также должны включать
...
- Прямые multipart-запросы публикации ограничены 18MB. Tarball ClawPack могут
  использовать flow upload-url до ограничения tarball 120MB.
...
- Публикации от имени другого владельца все равно проверяют право целевой учетной записи владельца на канал official.
...
При наличии `reason` сохраняется как заметка модерации навыка и копируется в журнал аудита.
...
Только для администратора. Обеспечивает существование org-издателя для handle. Если handle все еще указывает на
устаревшего общего пользователя/личного издателя, конечная точка сначала мигрирует его в org-издателя.
Для новосозданной org укажите `memberHandle`; действующий администратор не добавляется как участник.
...
Аутентифицированное self-serve создание org-издателя. Создает нового org-издателя и добавляет
...
релиза. Имена пакетов становятся приватными пакетами-заглушками без строк релиза, поэтому тот же
...
идентификатора учетных записей провайдера GitHub; изменяемые handles используются только как guard, видимый оператору.
...
GitHub principals. Восстановление завершается отказом по умолчанию, если текущий личный
publisher пользователя назначения имеет skills, packages или GitHub skill sources.
Восстановление также переносит устаревшие поля `ownerUserId` для skills восстановленного publisher,
псевдонимов skill slug, packages, предупреждений package inspector и производных строк search digest, чтобы
пути direct-owner согласовывались с новым полномочием publisher. Активное резервирование protected-handle
...
GitHub skill sources имеют область действия publisher и сообщаются как проверенные, а не переписываются.
...
### Endpoint для управления слагами владельца
...
- Оба endpoint требуют аутентификацию по API-токену и работают только для владельца skill.
- `rename` сохраняет предыдущий слаг как псевдоним перенаправления.
- `merge` скрывает исходный листинг и перенаправляет исходный слаг на целевой листинг.

### Endpoint для передачи владения
...
Добавить/удалить звезду (выделение). Оба endpoint идемпотентны.
...
## Устаревшие endpoint CLI (deprecated)
