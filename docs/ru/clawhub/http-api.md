...
x-i18n:
    generated_at: "2026-07-04T18:12:17Z"
    model: gpt-5.5
...
- Аутентифицированные запросы (действительный Bearer-токен): применяются на пользовательский бакет.
...
- Аутентифицированные конечные точки записи не должны возвращать простой `Unauthorized`, когда
  ...
  удаленные/забаненные/отключенные аккаунты должны получать действенный текст, чтобы CLI
  могли объяснить пользователям, что их заблокировало.
...
- `X-RateLimit-Remaining` / `RateLimit-Remaining`: точный оставшийся бюджет, если присутствует.
  Успешные шардированные запросы не возвращают этот заголовок вместо приблизительного глобального значения.
...
должны читать тело ответа как человекочитаемую строку. Неизвестные query parameters
игнорируются для совместимости, но распознанные query parameters с недопустимыми значениями возвращают
`400`.
...
- `highlightedOnly` (необязательно): `true` для фильтрации только выделенных Skills
- `nonSuspiciousOnly` (необязательно): `true` для скрытия подозрительных (`flagged.suspicious`) Skills
...
- Результаты возвращаются в порядке релевантности (сходство embedding + усиления точного совпадения slug/name token + небольшой приоритет популярности).
- Релевантность сильнее популярности. Точное совпадение slug или display-name token может ранжироваться выше более свободного совпадения с гораздо более сильной вовлеченностью.
- ASCII-текст токенизируется по границам слов и знаков пунктуации. Например, `personal-map` содержит отдельный token `map`, тогда как `amap-jsapi-skill` содержит `amap`, `jsapi` и `skill`; поэтому поиск `map` дает `personal-map` более сильное лексическое совпадение, чем `amap-jsapi-skill`.
...
- Поместите термины, которые пользователи будут буквально искать, в display name, summary и tags. Используйте отдельный slug token только тогда, когда это также стабильная идентичность, которую вы хотите сохранить.
...
- Rename aliases сохраняют разрешение для старых URL и установок, которые разрешаются через реестр, но ранжирование поиска основано на канонических метаданных Skill после индексации переименования. Существующая статистика остается со Skill.
...
- `sort` (необязательно): `updated` (по умолчанию), `recommended` (alias: `default`), `createdAt` (alias: `newest`), `downloads`, `stars` (alias: `rating`), устаревшие aliases установки `installsCurrent`/`installs`/`installsAllTime` сопоставляются с `downloads`, `trending`
- `nonSuspiciousOnly` (необязательно): `true` для скрытия подозрительных (`flagged.suspicious`) Skills
- `nonSuspicious` (необязательно): устаревший alias для `nonSuspiciousOnly`
...
- Когда `nonSuspiciousOnly=true`, сортировки на основе cursor могут возвращать меньше элементов, чем `limit`, на странице, потому что подозрительные Skills фильтруются после извлечения страницы.
...
- Evidence редактируется для публичных вызывающих сторон и включает необработанные фрагменты только для владельцев/модераторов.
...
- Полезные нагрузки запросов сканирования и загружаемые отчеты удаляются из хранилища запросов сканирования после окончания срока хранения.
...
- Задания сканирования асинхронны. Ручные запросы сканирования получают приоритет перед обычной работой публикации/дозаполнения, но завершение по-прежнему зависит от доступности рабочих процессов.
...
- `kind` по умолчанию равно `skill`; используйте `kind=plugin` для сканирования plugin/пакета.
- Возвращает тот же формат ZIP, что и загрузки запросов сканирования.
...
- Используйте `/verify`, когда нужен проверочный конверт Skill Card для одного навыка, `/card`, когда нужен сгенерированный markdown карточки, и `/scan`, когда нужны подробные данные сканеров.
...
- Записи Skills по-прежнему основаны на реестре skills и всё ещё могут публиковаться только через `POST /api/v1/skills`.
...
`trending` — это таблица лидеров установок/скачиваний за семь дней, и она не использует суммарные показатели за всё время.
...
- Метаданные экспорта для каждого плагина хранятся в
...
- Устаревшие псевдонимы фильтров v1, задокументированные в `GET /api/v1/plugins`, также
  принимаются.
- Фильтрация по категории является настоящим API-фильтром на основе строк дайджеста категорий
  плагинов, а не переписыванием поискового запроса.
...
- Приватные пакеты возвращают `404`, если вызывающий пользователь не может читать владельца-издателя.
...
- Требуется API-токен владельца пакета, владельца/администратора организации-издателя,
...
устанавливать разрешенный выпуск.
...
- Эта конечная точка намеренно уже, чем конечные точки модерации для владельцев/модераторов.
  ...
  личности отправителей жалоб, тексты жалоб, приватные доказательства или внутренние
...
- Версии ClawPack передают поток точных загруженных байтов `.tgz` npm-pack.
- Устаревшие ZIP-версии перенаправляют на `/api/v1/packages/{name}/download?version=`.
- Использует корзину ограничения частоты скачивания.
...
Конечная точка модератора для вывода строк миграции официальных плагинов OpenClaw.
...
Конечная точка администратора для создания или обновления строки миграции официального плагина.
...
Пожаловаться на пакет для проверки модератором. Жалобы относятся к уровню пакета и необязательно
связаны с версией. Они попадают в очередь модерации, но сами по себе не скрывают автоматически и не
блокируют скачивания; модераторам следует использовать модерацию выпусков, чтобы
...
- Ограничение размера файла: 200 КБ.
- Ожидающие проверки VirusTotal не блокируют чтение; вредоносные релизы все равно могут быть скрыты в других местах.
...
- Ответы включают заголовки `ETag`, `Digest`, `X-ClawHub-Artifact-Type` и
  `X-ClawHub-Artifact-Sha256` для проверок целостности в резолвере.
...
- Packument для scoped-пакетов поддерживает как `/api/npm/@scope/name`, так и закодированный
...
## Эндпоинты авторизации (Bearer-токен)
...
- Предпочтительно: `multipart/form-data` с JSON `payload` + blob `files[]`.
...
- Необязательное поле payload: `migrateOwner`. При `true` с `ownerHandle`
  существующий skill может быть перенесен к этому владельцу, если действующее лицо является администратором/владельцем у обоих
...
- Требует авторизацию Bearer-токеном.
- Требует `multipart/form-data`.
- Разрешенные поля формы: `payload`, повторяющиеся blob `files` или одна ссылка на tarball `clawpack`.
...
- Code plugins требуют `package.json`, метаданные репозитория исходников, метаданные commit исходников,
...
- Публикации от имени другого лица все равно проверяют право на official-channel относительно целевой учетной записи владельца.
...
Только для администраторов. Гарантирует, что org-издатель существует для handle. Если handle все еще указывает на
устаревшего общего пользователя/персонального издателя, эндпоинт сначала мигрирует его в org-издателя.
...
вызывающего как владельца. Этот эндпоинт не мигрирует существующие пользовательские/персональные handle и
...
владелец может позже опубликовать настоящий релиз code-plugin или bundle-plugin с этим именем.
...
Только для администраторов. Восстанавливает персонального издателя для проверенного заменяющего principal GitHub OAuth
...
По умолчанию конечная точка работает в режиме dry-run. Для применения восстановления требуются `dryRun: false` и
...
пути прямого владельца согласовывались с новым полномочием издателя. Активное резервирование защищенного handle
...
которые подготавливают tarball ClawPack, должны отправлять итоговый идентификатор хранилища как
