...
x-i18n:
    generated_at: "2026-07-05T05:24:05Z"
    model: gpt-5.5
...
## Публичное повторное использование каталога

Сторонние каталоги могут использовать публичные эндпоинты чтения, чтобы выводить список Skills ClawHub или искать по ним. Кэшируйте результаты, соблюдайте `429`/`Retry-After`, направляйте пользователей обратно к канонической странице ClawHub (`https://clawhub.ai/<owner>/skills/<slug>`) и не создавайте впечатление, что ClawHub одобряет сторонний сайт. Не пытайтесь зеркалировать скрытый, приватный или заблокированный модерацией контент за пределами публичной поверхности API.

Короткие web slug разрешаются между семействами реестра, но клиенты API должны использовать
канонические URL, возвращаемые эндпоинтами чтения, вместо реконструирования приоритета
маршрутов.
...
- Скачивание: 1200/мин на IP, 6000/мин на ключ (эндпоинты скачивания)
...
- Если доверенный IP клиента недоступен, анонимные запросы используют fallback buckets,
  ограниченные только типом rate limit. Эти fallback buckets не включают
  предоставленные вызывающим пути, slug, имена пакетов, версии, строки запроса или другие
...
прав (`401`/`403`), ограничения частоты (`429`) и заблокированные скачивания. Клиенты
...
- `highlightedOnly` (необязательный): `true`, чтобы фильтровать до выделенных Skills
- `nonSuspiciousOnly` (необязательный): `true`, чтобы скрывать подозрительные (`flagged.suspicious`) Skills
...
- Результаты возвращаются в порядке релевантности (сходство embedding + boosts для точных токенов slug/имени + небольшой prior популярности).
- Релевантность сильнее популярности. Точное совпадение токена slug или отображаемого имени может ранжироваться выше, чем более свободное совпадение с гораздо более высокой вовлеченностью.
...
- Помещайте термины, которые пользователи будут буквально искать, в отображаемое имя, summary и tags. Используйте отдельный токен slug только когда он также является стабильной идентичностью, которую вы хотите сохранить.
...
- Если Skill неожиданно невидим, сначала проверьте состояние модерации с помощью `clawhub inspect @owner/slug`, будучи залогиненным, прежде чем менять метаданные, связанные с ранжированием.
...
- `metadata.systems`: целевые Nix-системы (например, `["aarch64-darwin", "x86_64-linux"]`). `null`, если не объявлены.
...
Пожаловаться на Skill для проверки модератором. Жалобы относятся к уровню Skill, опционально связываются
...
Эндпоинт модератора/администратора для приема жалоб на Skills.
...
Эндпоинт модератора/администратора для разрешения или повторного открытия жалоб на Skills.
...
установке `status` обратно в `open`. Передайте `finalAction: "hide"` с рассмотренной
...
- Опубликованные сканирования требуют управленческого доступа владельца/публикатора либо полномочий модератора/администратора платформы.
...
- Задания сканирования выполняются асинхронно. Ручные запросы сканирования имеют приоритет над обычной работой publish/backfill, но завершение все равно зависит от доступности воркеров.
...
- Возвращает `queue.queuedAhead` и `queue.position`, пока запрос находится в очереди, чтобы клиенты могли показать, сколько приоритетных ручных сканирований находится перед запросом. Очень большие очереди ограничиваются и сообщаются с `queuedAheadIsEstimate: true`.
...
Аутентифицированная конечная точка архива отчетов.
...
Аутентифицированная конечная точка архива сохраненных отчетов для отправленных версий.

- Требует управленческого доступа владельца/публикатора к Skills или plugin либо полномочий модератора/администратора платформы.
- Возвращает сохраненные результаты сканирования для точной отправленной версии, включая заблокированные или скрытые версии.
- `kind` по умолчанию равен `skill`; используйте `kind=plugin` для сканирований plugin/package.
- Возвращает ту же ZIP-структуру, что и загрузки scan-request.
...
Возвращает проверочный конверт Skill Card, используемый `clawhub skill verify`.
...
- `ok` равно `true` только когда выбранная версия имеет сгенерированный Skill Card, не заблокирована модерацией как вредоносная и проверка ClawScan чистая.
- Идентификатор Skills, идентификатор публикатора и метаданные выбранной версии являются полями верхнего уровня конверта (`slug`, `displayName`, `publisherHandle`, `version`, `resolvedFrom`, `tag`, `createdAt`), чтобы shell-автоматизация могла читать их без распаковки вложенных оберток.
- `security` — это вердикт ClawScan/security верхнего уровня. Автоматизация должна опираться на `ok`, `decision`, `reasons` и `security.status`.
- `security.signals` содержит подтверждающие свидетельства сканеров, такие как `staticScan`, `virusTotal` и `skillSpector`.
- `security.signals.dependencyRegistry` сохраняется для совместимости ответа v1, но сканер существования реестра зависимостей выведен из эксплуатации, и этот ключ всегда равен `null`.
- `provenance` равно `server-resolved-github-import` только когда ClawHub разрешил и сохранил repo/ref/commit/path GitHub во время публикации или импорта; иначе оно равно `unavailable`.
...
версии ClawHub Skills им нужно отображать, например OpenClaw Control UI.
...
- Результаты возвращаются по каждому элементу; один отсутствующий Skills или версия не приводят к сбою всего ответа.
...
- `security.signals` содержит только подтверждающие свидетельства уровня статуса; используйте `/scan` или страницу security-audit ClawHub для полных сведений сканера.
- `security.signals.dependencyRegistry` сохраняется для совместимости ответа v1, но сканер существования реестра зависимостей выведен из эксплуатации, и этот ключ всегда равен `null`.
...
- Используйте `/verify`, когда нужен проверочный конверт Skill Card для одного Skills, `/card`, когда нужен сгенерированный Markdown карточки, и `/scan`, когда нужны подробные данные сканера.
...
- `category` (необязательный): фильтр категории плагинов. Поддерживается только когда
  ...
  `/api/v1/code-plugins`, `/api/v1/bundle-plugins` или конечные точки пакетов с
...
- `isOfficial` (необязательный): `true` или `false`
- `category` (необязательный): фильтр категории плагинов. Поддерживается только когда
  запрос ограничен пакетами плагинов. Контролируемые категории и устаревшие псевдонимы
...
Каталог только плагинов для просмотра пакетов code-plugin и bundle-plugin.
...
`trending` — это семидневная таблица лидеров по установкам/загрузкам, она не использует суммарные показатели за все время.
На единой конечной точке `/api/v1/packages` она применяется только к плагинам; используйте
...
- Текущие навыки на базе GitHub со сканированием `clean` или `suspicious` включают
...
- `family` (необязательно): `code-plugin` или `bundle-plugin`. Если опущено, подразумеваются оба семейства Plugin.
...
- Устаревшие псевдонимы фильтров v1, задокументированные в `GET /api/v1/plugins`, также
  принимаются.
- Фильтрация по категории — это настоящий API-фильтр, основанный на строках
...
Возвращает точную сводку безопасности и доверия для релиза пакета, предназначенную для
клиентов установки. Это публичная поверхность потребления OpenClaw для принятия решения о том, можно ли
...
- `trust.reasons` — список объяснений для пользователя и аудита. Коды причин
  являются стабильными компактными строками, например `manual:quarantined`, `scan:malicious`
  и `package:malicious`.
...
- `trust.stale` означает, что сводка доверия была вычислена из устаревших входных данных и
  перед решением о разрешении с высокой уверенностью ее следует считать требующей обновления.
...
- Эта конечная точка намеренно уже, чем конечные точки модерации для владельцев/модераторов.
  ...
  репортеров, тела отчетов, приватные доказательства или внутренние временные шкалы
  проверки.
...
- Это поверхность резолвера OpenClaw; она позволяет не угадывать формат архива по
  общему URL.
...
Администраторская конечная точка для создания или обновления строки миграции официального Plugin.
...
- `open`: подозрительные, вредоносные, ожидающие, помещенные в карантин, отозванные или зарепорченные релизы.
...
Сообщает о пакете для проверки модератором. Отчеты относятся к уровню пакета и могут
необязательно быть связаны с версией. Они попадают в очередь модерации, но сами по себе
...
- Ограничение размера файла: 200 КБ.
- Ожидающие сканирования VirusTotal не блокируют чтение; вредоносные релизы все еще могут быть удержаны в других местах.
- Приватные пакеты возвращают `404`, если вызывающий пользователь не может читать издателя-владельца.
...
- Приватные пакеты возвращают `404`, если вызывающий пользователь не является владельцем.
...
- Проверки модерации и доступа к приватным пакетам все еще применяются.
...
Скачивает размещенную ZIP-версию Skills или возвращает передачу на исходный код GitHub для
...
- Передачи Skills на базе GitHub не проксируют и не зеркалируют байты. JSON-ответ
  включает `sourceRef: "public-github"`, `repo`, `commit`, `path`, `contentHash`
  и `archiveUrl`; сканирование/текущее состояние является шлюзом и не включается как метаданные успешной
  полезной нагрузки.
- Статистика скачиваний учитывается как уникальные идентичности за UTC-день (`userId`, когда API-токен действителен, иначе IP).
...
- Необязательное поле payload: `migrateOwner`. Когда `true` с `ownerHandle`,
  существующий Skills может перейти к этому владельцу, если действующий пользователь является администратором/владельцем как у
...
- JSON-тела и предоставленные вызывающим пользователем метаданные `payload.files` / `payload.artifact`
  отклоняются.
...
- Публикации от имени другого пользователя все равно проверяют право на официальный канал относительно учетной записи целевого владельца.
...
вызывающего пользователя как владельца. Эта конечная точка не мигрирует существующие пользовательские/личные handle и не
помечает издателя как доверенного/официального.
...
владелец может позже опубликовать реальный релиз code-plugin или bundle-plugin под этим именем.
...
субъектами GitHub. Восстановление завершается отказом в закрытом режиме, если у текущего личного
издателя пользователя назначения есть skills, packages или источники GitHub skill.
Восстановление также переносит устаревшие поля `ownerUserId` для skills восстановленного издателя,
алиасов slug skill, пакетов, предупреждений инспектора пакетов и производных строк поискового дайджеста, чтобы
...
100 строками на применяющую транзакцию; для более крупных восстановлений сначала нужно использовать возобновляемую миграцию владельца.
Источники GitHub skill имеют область действия издателя и помечаются как проверенные, а не перезаписываются.
...
- `POST /api/v1/skills/{slug}/transfer/cancel`
  - Ответ (принять/отклонить/отменить): `{ "ok": true, "skillSlug": "demo-skill?" }`
...
- `limit` (необязательно): максимум результатов (по умолчанию 20, максимум 200)
