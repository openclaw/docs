...
summary: Справочник HTTP API (публичные эндпоинты, эндпоинты CLI и аутентификация).
x-i18n:
    generated_at: "2026-07-04T06:43:05Z"
    model: gpt-5.5
...
Сторонние каталоги могут использовать публичные конечные точки чтения, чтобы выводить список или искать Skills ClawHub. Кэшируйте результаты, соблюдайте `429`/`Retry-After`, направляйте пользователей обратно к канонической странице ClawHub (`https://clawhub.ai/<owner>/skills/<slug>`) и не создавайте впечатления, что ClawHub одобряет сторонний сайт. Не пытайтесь зеркалировать скрытый, приватный или заблокированный модерацией контент за пределами публичной поверхности API.
...
- Аутентифицированные запросы (действительный Bearer-токен): применяются на корзину пользователя.
...
- ClawHub использует доверенные заголовки пересылки, чтобы определять IP клиентов на edge.
...
- Результаты возвращаются в порядке релевантности (сходство embedding + усиления по точному токену slug/имени + небольшой prior популярности).
- Релевантность сильнее популярности. Точное совпадение токена slug или отображаемого имени может ранжироваться выше, чем менее точное совпадение с намного более сильной вовлеченностью.
...
- Популярность логарифмически масштабируется и ограничивается верхней границей. Skills с высокой вовлеченностью могут ранжироваться ниже, когда текст запроса совпадает слабее.
- Подозрительное или скрытое состояние модерации может удалить навык из публичного поиска в зависимости от фильтров вызывающей стороны и текущего статуса модерации.

Рекомендации по обнаружению для издателей:

- Помещайте термины, которые пользователи будут буквально искать, в отображаемое имя, сводку и теги. Используйте отдельный токен slug только когда это также стабильная идентичность, которую вы хотите сохранить.
...
- Псевдонимы переименования сохраняют разрешение для старых URL и установок, которые разрешаются через реестр, но поисковое ранжирование основано на канонических метаданных навыка после индексации переименования. Существующая статистика остается с навыком.
- Если навык неожиданно невидим, сначала проверьте состояние модерации через `clawhub inspect @owner/slug` после входа в систему, прежде чем менять метаданные, связанные с ранжированием.
...
Сообщает о навыке для проверки модератором. Жалобы относятся к уровню навыка, необязательно связаны
...
возврате `status` обратно в `open`. Передайте `finalAction: "hide"` с разобранной
жалобой, чтобы скрыть навык в том же аудируемом рабочем процессе.
...
- `security.hasScanResult` имеет значение `true` только когда сканер выдал окончательный вердикт (`clean`, `suspicious` или `malicious`).
...
- Задания сканирования асинхронны. Ручные запросы сканирования имеют приоритет над обычной публикацией/обратным заполнением, но завершение все равно зависит от доступности workers.
...
- Возвращает `queue.queuedAhead` и `queue.position`, пока запрос стоит в очереди, чтобы клиенты могли показать, сколько приоритетных ручных сканирований находится перед этим запросом. Очень большие очереди ограничиваются и сообщаются с `queuedAheadIsEstimate: true`.
...
- По умолчанию `kind` равен `skill`; используйте `kind=plugin` для сканирований plugin/пакета.
...
- `ok` имеет значение `true` только когда выбранная версия имеет сгенерированную карточку Skill, не заблокирована модерацией как вредоносная и проверка ClawScan чистая.
- Идентификатор skill, идентификатор издателя и метаданные выбранной версии являются полями верхнего уровня конверта (`slug`, `displayName`, `publisherHandle`, `version`, `resolvedFrom`, `tag`, `createdAt`), чтобы автоматизация оболочки могла читать их без распаковки вложенных оберток.
...
Возвращает текущие компактные вердикты безопасности для точных версий skill. Этот
endpoint коллекции предназначен для клиентов, которые уже знают, какие установленные
версии skill ClawHub им нужно отобразить, например OpenClaw Control UI.
...
- Результаты возвращаются по каждому элементу; один отсутствующий skill или версия не приводит к сбою всего ответа.
...
- `security.signals` содержит только подтверждающие данные уровня статуса; используйте `/scan` или страницу аудита безопасности ClawHub для полных сведений сканера.
- `security.signals.dependencyRegistry` сохранен для совместимости ответа v1, но сканер наличия реестра зависимостей выведен из эксплуатации, и этот ключ всегда равен `null`.
- Отсутствие карточки Skill не влияет на `ok`, `decision` или `reasons` этого endpoint; клиенты должны читать установленный `skill-card.md` локально, когда им нужно содержимое карточки.
- Используйте `/verify`, когда нужен конверт проверки карточки Skill для одного skill, `/card`, когда нужен сгенерированный Markdown карточки, и `/scan`, когда нужны подробные данные сканера.
...
Единый поиск по каталогу среди Skills и пакетов plugins.
...
Просмотр каталога только plugins среди пакетов code-plugin и bundle-plugin.
...
`trending` — это рейтинг установок/загрузок за семь дней, он не использует суммарные показатели за всё время.
На единой конечной точке `/api/v1/packages` он применяется только к plugins; используйте
`/api/v1/skills?sort=trending` для каталога Skills.

Устаревшие псевдонимы не принимаются как сохранённые или объявленные авторами значения категорий.
...
- `startDate` (обязательный): нижняя граница в миллисекундах Unix для `updatedAt` Skill.
- `endDate` (обязательный): верхняя граница в миллисекундах Unix для `updatedAt` Skill.
...
- Тело: ZIP-архив.
- Каждый экспортированный Skill имеет корень в `{publisher}/{slug}/`.
...
- `_manifest.json` всегда включается в корень ZIP.
- `_errors.json` включается, если отдельные Plugin или файлы не удалось
  экспортировать.
...
- Устаревшие псевдонимы фильтров v1, документированные в `GET /api/v1/plugins`, также
  принимаются.
- Фильтрация по категории — реальный фильтр API, основанный на строках дайджеста категорий Plugin,
...
- `version.artifact.kind` равен `legacy-zip` для архивов пакетов старого мира или
...
- Эта конечная точка привязана к точной версии. Клиентам следует вызывать ее после разрешения
  версии пакета, которую они собираются установить, а не просто после чтения последних
  метаданных пакета.
- Приватные пакеты возвращают `404`, если вызывающий не может читать владеющего издателя.
- Эта конечная точка намеренно уже, чем конечные точки модерации владельца/модератора.
  ...
  личности авторов отчетов, тела отчетов, приватные доказательства или внутренние
  сроки проверки.
...
- `open`: подозрительные, вредоносные, ожидающие, помещенные в карантин, отозванные или получившие жалобы выпуски.
...
Сообщить о пакете для проверки модератором. Отчеты имеют уровень пакета и необязательно
связаны с версией. Они попадают в очередь модерации, но сами по себе не скрывают автоматически и
не блокируют загрузки; модераторам следует использовать модерацию выпусков, чтобы
...
- Ограничение размера файла: 200 КБ.
- Ожидающие сканирования VirusTotal не блокируют чтение; вредоносные выпуски все равно могут быть скрыты в другом месте.
- Приватные пакеты возвращают `404`, если вызывающий не может читать владеющего издателя.
...
- Архивы Plugin/пакетов являются ZIP-файлами с корнем `package/`, чтобы старые клиенты OpenClaw
  продолжали работать.
- Этот маршрут остается только ZIP. Он не транслирует файлы ClawPack `.tgz`.
...
Транслирует точные байты загруженного tarball-архива ClawPack для клиентов npm-зеркала.
...
- `slug` (обязательно)
- `hash` (обязательно): 64-символьный hex sha256 fingerprint бандла
...
Скачивает ZIP размещенной версии skill или возвращает передачу на исходники GitHub для
...
- Передачи skill на базе GitHub не проксируют и не зеркалируют байты. JSON-ответ
  ...
  и `archiveUrl`; состояние сканирования/актуальности является ограничителем и не включается как метаданные
  успешной полезной нагрузки.
- Статистика скачиваний считается как уникальные идентификаторы за день UTC (`userId`, когда API-токен действителен, иначе IP).
...
- Необязательное поле payload: `ownerHandle`. Если оно присутствует, API разрешает этого
  издателя на стороне сервера и требует, чтобы у актера был доступ к издателю.
- Необязательное поле payload: `migrateOwner`. Если `true` с `ownerHandle`, существующий
  skill может перейти к этому владельцу, если актер является администратором/владельцем у обоих
...
- Требуется аутентификация Bearer-токеном.
...
- Разрешенные поля формы: `payload`, повторяющиеся blob-объекты `files` или одна ссылка на tarball
  ...
  потоком upload-url. Публикации из промежуточного storage-id также должны включать
...
- Необязательное поле payload: `ownerHandle`. Если оно присутствует, публиковать от имени этого владельца могут только администраторы.
...
- Code plugins требуют `package.json`, метаданные репозитория исходников, метаданные коммита исходников,
...
- Публикации от имени другого владельца все равно проверяют право на official-channel относительно целевой учетной записи владельца.
...
Если `reason` присутствует, он сохраняется как заметка модерации skill и копируется в журнал аудита.
...
владелец может позже опубликовать настоящий выпуск code-plugin или bundle-plugin под этим именем.
...
без редактирования строк учетных записей Convex Auth. Запрос должен указать оба неизменяемых идентификатора учетных записей
провайдера GitHub; изменяемые handle используются только как видимая оператору защита.

Эндпоинт по умолчанию работает в режиме dry-run. Для применения восстановления требуются `dryRun: false` и
`confirmIdentityVerified: true` после того, как сотрудники независимо проверят непрерывность между обеими
учетными записями GitHub. Восстановление завершается отказом в закрытом режиме, если у текущего персонального
издателя пользователя назначения есть skills, packages или GitHub skill sources.
Восстановление также переносит устаревшие поля `ownerUserId` для навыков восстановленного издателя,
псевдонимов slug навыков, пакетов, предупреждений инспектора пакетов и производных строк поискового дайджеста, чтобы
пути прямого владельца соответствовали новым полномочиям издателя. Активная резервация защищенного handle
...
100 строками на транзакцию применения; для более крупных восстановлений сначала требуется возобновляемая миграция владельца.
GitHub skill sources имеют область действия издателя и сообщаются как проверенные, а не перезаписываются.
...
- Оба эндпоинта требуют аутентификации по API-токену и работают только для владельца навыка.
...
Заблокировать пользователя и безвозвратно удалить принадлежащие ему навыки (только moderator/admin).
...
Разблокировать пользователя и восстановить подходящие навыки (только admin).
...
контента (только admin). По умолчанию используется dry-run, если `dryRun` не равно `false`.
...
## Устаревшие эндпоинты CLI (deprecated)
