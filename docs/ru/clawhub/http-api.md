...
Сторонние каталоги могут использовать публичные конечные точки чтения для перечисления или поиска Skills ClawHub. Кэшируйте результаты, соблюдайте `429`/`Retry-After`, направляйте пользователей обратно к канонической странице ClawHub (`https://clawhub.ai/<owner>/skills/<slug>`) и не создавайте впечатление, что ClawHub одобряет сторонний сайт. Не пытайтесь зеркалировать скрытый, приватный или заблокированный модерацией контент за пределами публичной поверхности API.
...
- Анонимные запросы: применяются для каждого IP.
- Аутентифицированные запросы (действительный Bearer-токен): применяются для корзины пользователя.
- Если токен отсутствует или недействителен, поведение возвращается к применению ограничений по IP.
...
- Если `Retry-After` отсутствует, используйте запасной вариант `RateLimit-Reset` (или вычислите из `X-RateLimit-Reset`).
...
- Если доверенный IP клиента недоступен, анонимные запросы используют резервные корзины,
  ограниченные только типом rate limit. Эти резервные корзины не включают
  предоставленные вызывающим пути, slugs, имена пакетов, версии, строки запроса или другие
  параметры артефактов.

## Ответы об ошибках

Публичные ответы об ошибках v1 являются plain text с `content-type: text/plain; charset=utf-8`.
Это включает сбои валидации (`400`), отсутствующие публичные ресурсы (`404`), сбои аутентификации и
...
- `highlightedOnly` (необязательно): `true`, чтобы фильтровать только highlighted skills
- `nonSuspiciousOnly` (необязательно): `true`, чтобы скрывать подозрительные (`flagged.suspicious`) skills
...
- Результаты возвращаются в порядке релевантности (сходство embedding + усиления точного совпадения токенов slug/имени + небольшой prior популярности).
- Релевантность важнее популярности. Точное совпадение токена slug или отображаемого имени может ранжироваться выше, чем более свободное совпадение с гораздо более сильным вовлечением.
- ASCII-текст токенизируется по границам слов и знаков пунктуации. Например, `personal-map` содержит самостоятельный токен `map`, а `amap-jsapi-skill` содержит `amap`, `jsapi` и `skill`; поэтому поиск `map` дает `personal-map` более сильное лексическое совпадение, чем `amap-jsapi-skill`.
...
- Помещайте термины, которые пользователи буквально будут искать, в отображаемое имя, сводку и теги. Используйте самостоятельный токен slug только когда он также является стабильной идентичностью, которую вы хотите сохранить.
...
- `metadata.systems`: системные цели Nix (например, `["aarch64-darwin", "x86_64-linux"]`). `null`, если не объявлены.
...
- Доказательства редактируются для публичных вызывающих и включают необработанные фрагменты только для владельцев/модераторов.
...
возврате `status` к `open`. Передайте `finalAction: "hide"` с обработанным
...
- Полезные нагрузки запросов сканирования и загружаемые отчеты удаляются из хранилища scan-request после окончания окна хранения.
- Для сканирования опубликованных версий требуется доступ владельца/издателя к управлению либо полномочия модератора/администратора платформы.
...
- Требуется доступ владельца/издателя к управлению навыком или Plugin либо полномочия модератора/администратора платформы.
...
- Возвращает тот же формат ZIP, что и загрузки scan-request.
...
- `ok` равно `true` только когда для выбранной версии сгенерирована карточка навыка, она не заблокирована модерацией как вредоносная, и проверка ClawScan чистая.
...
- `provenance` равно `server-resolved-github-import` только когда ClawHub разрешил и сохранил репозиторий/ref/коммит/путь GitHub во время публикации или импорта; в противном случае значение равно `unavailable`.
...
- Аутентифицированные вызывающие могут видеть приватные пакеты издателей, к которым они принадлежат, в результатах списков и поиска.
...
- `isOfficial` (необязательный): `true` или `false`
- `category` (необязательный): фильтр категории Plugin. Поддерживается только когда
  запрос ограничен пакетами Plugin. Контролируемые категории и устаревшие
  псевдонимы фильтров v1 задокументированы в разделе `GET /api/v1/plugins`.
...
`trending` — это рейтинг установок/загрузок за семь дней; он не использует общие итоги за все время.
...
- Устаревшие псевдонимы фильтров v1, задокументированные в `GET /api/v1/plugins`, также
  принимаются.
- Фильтрация по категории является реальным API-фильтром, основанным на строках дайджеста категорий Plugin,
  а не переписыванием поискового запроса.
- Результаты возвращаются в порядке релевантности и сейчас не поддерживают пагинацию.
- Элементы управления сортировкой в браузерном интерфейсе для поиска Plugin переупорядочивают загруженные результаты релевантности,
...
- `trust.moderationState` допускает null. Значение равно `null`, когда ручной модерации релиза
  нет.
...
- Эта конечная точка намеренно уже, чем конечные точки модерации для владельцев/модераторов.
  ...
  личности сообщивших, тексты отчетов, приватные доказательства или внутренние временные линии
  проверки.
...
Сообщить о пакете для проверки модератором. Отчеты относятся к уровню пакета и необязательно
связаны с версией. Они попадают в очередь модерации, но сами по себе автоматически не скрывают и не
...
- Устаревшие версии только в ZIP намеренно пропущены.
...
Транслирует точные байты загруженного tarball-файла ClawPack для клиентов npm-зеркала.
...
- `slug` (обязательно)
- `version` (необязательно): semver-строка
...
- Необязательное поле полезной нагрузки: `migrateOwner`. Когда `true` вместе с `ownerHandle`,
  существующий skill может перейти к этому владельцу, если действующее лицо является администратором/владельцем у обоих
...
- Разрешенные поля формы: `payload`, повторяющиеся blob-объекты `files` или одна ссылка на tarball `clawpack`.
  ...
  потоком upload-url. Публикации из staged storage-id также должны включать
...
- Публикации от имени другого лица все равно проверяют право на official-channel по учетной записи целевого владельца.
...
При наличии `reason` сохраняется как заметка модерации skill и копируется в журнал аудита.
Мягкие удаления, инициированные владельцем, резервируют slug на 30 дней, после чего slug может быть заявлен
...
Только для администратора. Гарантирует, что для handle существует org-издатель. Если handle все еще указывает на
...
владелец позднее может опубликовать настоящий релиз code-plugin или bundle-plugin под этим именем.
...
Только для администратора. Восстанавливает персонального издателя для проверенного заменяющего GitHub OAuth principal
без редактирования строк учетной записи Convex Auth. Запрос должен назвать оба неизменяемых GitHub
provider account id; изменяемые handle используются только как guard, видимый оператору.
...
субъектами GitHub. Восстановление завершается безопасным отказом, если у текущего личного
издателя целевого пользователя есть skills, packages или источники GitHub skill.
Восстановление также переносит устаревшие поля `ownerUserId` для skills восстановленного издателя,
псевдонимов slug skill, packages, предупреждений инспектора package и производных строк поискового дайджеста, чтобы
пути прямого владельца согласовывались с новой властью издателя. Активное резервирование защищенного handle
для восстановленного handle также переназначается заменяющему пользователю, чтобы последующая
синхронизация профиля не могла восстановить конкурирующие права прежнего пользователя. Каждая основная таблица ограничена
100 строками на транзакцию применения; для более крупных восстановлений сначала необходимо использовать возобновляемую миграцию владельца.
Источники GitHub skill имеют область действия издателя и отображаются как проверенные, а не переписываются.
...
которые размещают tarball ClawPack на промежуточном этапе, должны отправить получившийся идентификатор хранилища как
