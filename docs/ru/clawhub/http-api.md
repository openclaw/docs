...
Сторонние каталоги могут использовать публичные конечные точки чтения, чтобы выводить список Skills ClawHub или искать их. Кэшируйте результаты, соблюдайте `429`/`Retry-After`, направляйте пользователей обратно к канонической странице ClawHub (`https://clawhub.ai/<owner>/skills/<slug>`) и не создавайте впечатление, что ClawHub одобряет сторонний сайт. Не пытайтесь зеркалировать скрытый, приватный или заблокированный модерацией контент за пределами публичной поверхности API.

Веб-сокращения slug разрешаются между семействами реестра, но клиентам API следует использовать
...
- Аутентифицированные запросы (действительный Bearer-токен): применяются по пользовательской корзине.
- Если токен отсутствует или недействителен, поведение возвращается к применению по IP.
- Аутентифицированные конечные точки записи не должны возвращать голое `Unauthorized`, когда
  ...
  удаленные/заблокированные/отключенные учетные записи должны получать понятный текст с действием, чтобы CLI
...
- `X-RateLimit-Remaining` / `RateLimit-Remaining`: точный оставшийся бюджет при наличии.
  Успешные шардированные запросы опускают этот заголовок вместо возврата приблизительного глобального значения.
- `Retry-After`: секунд ожидания перед повторной попыткой (задержка) при `429`
...
- Используйте backoff с джиттером, чтобы избежать синхронизированных повторных попыток.
- Если `Retry-After` отсутствует, используйте запасной вариант `RateLimit-Reset` (или вычислите по `X-RateLimit-Reset`).
...
- Если доверенный IP клиента недоступен, анонимные запросы используют резервные корзины,
  ограниченные только типом ограничения частоты. Эти резервные корзины не включают
  переданные вызывающей стороной пути, slug, имена пакетов, версии, строки запросов или другие
  параметры артефактов.

## Ответы об ошибках

Публичные ответы об ошибках v1 представляют собой обычный текст с `content-type: text/plain; charset=utf-8`.
Сюда входят ошибки валидации (`400`), отсутствующие публичные ресурсы (`404`), ошибки аутентификации и
разрешений (`401`/`403`), ограничения частоты (`429`) и заблокированные скачивания. Клиентам
следует читать тело ответа как человекочитаемую строку. Неизвестные параметры запроса
...
- `highlightedOnly` (необязательно): `true`, чтобы отфильтровать только выделенные Skills
...
- Результаты возвращаются в порядке релевантности (сходство embedding + усиления по точному токену slug/имени + небольшой априорный вес популярности).
- Релевантность сильнее популярности. Точное совпадение токена slug или отображаемого имени может ранжироваться выше более свободного совпадения с гораздо более сильной вовлеченностью.
- ASCII-текст токенизируется по границам слов и знаков пунктуации. Например, `personal-map` содержит отдельный токен `map`, тогда как `amap-jsapi-skill` содержит `amap`, `jsapi` и `skill`; поэтому поиск `map` дает `personal-map` более сильное лексическое совпадение, чем `amap-jsapi-skill`.
- Популярность масштабируется логарифмически и ограничивается сверху. Skills с высокой вовлеченностью могут ранжироваться ниже, когда текст запроса совпадает слабее.
...
- Псевдонимы переименования сохраняют разрешение для старых URL и установок, которые разрешаются через реестр, но ранжирование поиска основано на канонических метаданных Skill после индексации переименования. Существующая статистика остается со Skill.
...
- Когда `nonSuspiciousOnly=true`, сортировки на основе курсора могут вернуть меньше элементов, чем `limit`, на странице, потому что подозрительные Skills фильтруются после получения страницы.
- Используйте `nextCursor`, чтобы продолжить пагинацию при наличии. Короткая страница сама по себе не означает конец результатов.
...
Сообщить о Skill для проверки модератором. Жалобы относятся к уровню Skill, опционально привязаны
к версии и попадают в очередь жалоб на Skill.
...
Конечная точка модератора/администратора для приема жалоб на Skill.
...
Конечная точка модератора/администратора для разрешения или повторного открытия жалоб на Skill.
...
возврате `status` к `open`. Передайте `finalAction: "hide"` с триажированной
...
- При запросе исторической версии проверяйте `moderation.matchesRequestedVersion` и `moderation.sourceVersion`, прежде чем рассматривать `moderation` и `security` как относящиеся к одному контексту версии.
...
- Для опубликованных сканирований требуется управленческий доступ владельца/публикатора или полномочия модератора/администратора платформы.
- Опубликованные сканирования записывают результаты обратно только когда `update: true` и сканирование успешно завершается.
- Ответ имеет статус `202` с `{ "ok": true, "scanId": "...", "jobId": "...", "status": "queued", "sourceKind": "published", "update": false, "queue": { "queuedAhead": 0, "queuedAheadIsEstimate": false, "position": 1, "running": 0, "runningIsEstimate": false, "note": "Scans are asynchronous and may take time to complete." } }`.
- Задания сканирования выполняются асинхронно. Ручные запросы сканирования имеют приоритет перед обычной публикацией/обратным заполнением, но завершение все равно зависит от доступности рабочих процессов.
...
- Возвращает `queue.queuedAhead` и `queue.position`, пока запрос находится в очереди, чтобы клиенты могли показывать, сколько приоритетных ручных сканирований находится перед этим запросом. Очень большие очереди ограничиваются и сообщаются с `queuedAheadIsEstimate: true`.
...
- Требует управленческий доступ владельца/публикатора к навыку или plugin либо полномочия модератора/администратора платформы.
- Возвращает сохраненные результаты сканирования для точной отправленной версии, включая заблокированные или скрытые версии.
- По умолчанию `kind` имеет значение `skill`; используйте `kind=plugin` для сканирований plugin/пакета.
...
- Идентификатор навыка, идентификатор публикатора и метаданные выбранной версии являются полями верхнего уровня конверта (`slug`, `displayName`, `publisherHandle`, `version`, `resolvedFrom`, `tag`, `createdAt`), чтобы автоматизация shell могла читать их без распаковки вложенных оберток.
...
- `provenance` имеет значение `server-resolved-github-import` только когда ClawHub разрешил и сохранил репозиторий/ref/commit/path GitHub во время публикации или импорта; в противном случае оно равно `unavailable`.
...
- `security.signals` содержит только подтверждающие свидетельства на уровне статуса; используйте `/scan` или страницу аудита безопасности ClawHub для полных сведений сканера.
- `security.signals.dependencyRegistry` сохранен для совместимости ответа v1, но сканер существования реестра зависимостей выведен из эксплуатации, и этот ключ всегда равен `null`.
- Отсутствие карточки навыка не влияет на `ok`, `decision` или `reasons` этой конечной точки; клиентам следует читать установленный `skill-card.md` локально, когда им нужно содержимое карточки.
- Используйте `/verify`, когда нужен конверт проверки карточки одного навыка, `/card`, когда нужен Markdown сгенерированной карточки, и `/scan`, когда нужны подробные данные сканера.
...
- Записи Skills по-прежнему поддерживаются реестром Skills и всё ещё могут публиковаться только через `POST /api/v1/skills`.
...
`trending` — это семидневная таблица лидеров по установкам/загрузкам, она не использует совокупные итоги за всё время.
В единой конечной точке `/api/v1/packages` она применяется только к plugin; используйте
`/api/v1/skills?sort=trending` для каталога Skills.

Устаревшие псевдонимы не принимаются как сохранённые или заявленные автором значения категорий.
...
- Размещённые Skills включают последние сохранённые файлы версии и перечислены в
...
- `limit` (необязательный): целое число (1-250), по умолчанию `250`.
- `cursor` (необязательный): курсор пагинации из предыдущего ответа.
- `family` (необязательный): `code-plugin` или `bundle-plugin`. Если опущен,
  используются оба семейства Plugin.
...
- Устаревшие псевдонимы фильтров v1, документированные в `GET /api/v1/plugins`,
  также принимаются.
- Фильтрация по категории является настоящим фильтром API на основе строк
  дайджеста категорий Plugin, а не переписыванием поискового запроса.
- Результаты возвращаются в порядке релевантности и сейчас не поддерживают
  пагинацию.
- Элементы управления сортировкой в браузерном UI для поиска Plugin
  переупорядочивают загруженные результаты релевантности, соответствуя текущему
  поведению просмотра `/skills`.
...
- Приватные пакеты возвращают `404`, если вызывающий не может читать
  владельца-издателя.

### `DELETE /api/v1/packages/{name}`

Помечает пакет и все выпуски как удаленные без физического удаления.

Примечания:

- Требуется токен API владельца пакета, владельца/администратора
  организации-издателя, модератора платформы или администратора платформы.
...
- `version.artifact.kind` имеет значение `legacy-zip` для архивов пакетов
  старого формата или `npm-pack` для выпусков на основе ClawPack.
- Выпуски ClawPack включают npm-совместимые поля `npmIntegrity`, `npmShasum` и
  `npmTarballName`.
- `version.sha256hash` — устаревшие метаданные совместимости для старых
  клиентов. Они хешируют точные байты ZIP, возвращаемые
  `/api/v1/packages/{name}/download`. Современным клиентам следует использовать
  `version.artifact.sha256`, который идентифицирует канонический артефакт
  выпуска.
...
Возвращает точную сводку безопасности и доверия для выпуска пакета для
клиентов установки. Это публичная поверхность потребления OpenClaw для решения,
можно ли установить разрешенный выпуск.

Аутентификация:

- Публичная конечная точка чтения. Токен владельца, издателя, модератора или
...
- `release.releaseId`, `release.version` и `release.createdAt`
  идентифицируют точный выпуск, который был оценен.
- `release.artifactKind`, `release.artifactSha256`, `release.npmIntegrity`,
  `release.npmShasum` и `release.npmTarballName` присутствуют, когда известны
  для артефакта выпуска.
- `trust.scanStatus` — эффективный статус доверия, полученный из входных данных
  сканеров и ручной модерации выпуска.
- `trust.moderationState` может быть nullable. Он равен `null`, когда ручная
  модерация выпуска отсутствует.
- `trust.blockedFromDownload` — сигнал блокировки установки. OpenClaw и другим
  клиентам установки следует блокировать установку, когда это значение равно
  `true`, вместо повторного вывода правил блокировки из полей сканера или
  модерации.
- `trust.reasons` — список пользовательских и аудиторских объяснений. Коды
  причин являются стабильными компактными строками, такими как
  `manual:quarantined`, `scan:malicious` и `package:malicious`.
- `trust.pending` означает, что один или несколько входных сигналов доверия все
  еще ожидают завершения.
- `trust.stale` означает, что сводка доверия была вычислена из устаревших
  входных данных и должна рассматриваться как требующая обновления перед
  решением о разрешении с высокой уверенностью.

Примечания:

- Эта конечная точка привязана к точной версии. Клиентам следует вызывать ее
  после разрешения версии пакета, которую они намерены установить, а не только
  после чтения метаданных последнего пакета.
- Приватные пакеты возвращают `404`, если вызывающий не может читать
  владельца-издателя.
- Эта конечная точка намеренно уже, чем конечные точки модерации для
  владельцев/модераторов. Она раскрывает решение об установке и публичное
  объяснение, но не личности отправителей жалоб, тела отчетов, приватные
  доказательства или внутренние сроки проверки.
...
Конечная точка модератора для перечисления строк миграции официальных Plugin
OpenClaw.

Аутентификация:

- Требуется токен API пользователя-модератора или администратора.
...
Конечная точка администратора для создания или обновления строки миграции
официального Plugin.

Аутентификация:

- Требуется токен API пользователя-администратора.
...
- `bundledPluginId` нормализуется к нижнему регистру и является стабильным
  ключом upsert.
- `packageName` нормализуется как имя npm; пакет может отсутствовать для
  запланированных миграций.
- Это отслеживает только готовность миграции. Это не изменяет OpenClaw и не
  генерирует ClawPacks.

### `GET /api/v1/packages/moderation/queue`

Конечная точка модератора/администратора для очередей проверки выпусков
пакетов.
...
- `open`: подозрительные, вредоносные, ожидающие, помещенные в карантин,
  отозванные выпуски или выпуски с жалобами.
- `blocked`: помещенные в карантин, отозванные или вредоносные выпуски.
- `manual`: любой выпуск с ручным переопределением модерации.
- `all`: любой выпуск с ручным переопределением, не чистым состоянием
  сканирования или жалобой на пакет.
...
Сообщает о пакете для проверки модератором. Жалобы относятся к уровню пакета и
могут быть необязательно связаны с версией. Они попадают в очередь модерации, но
сами по себе не скрывают и не блокируют загрузки автоматически; модераторам
следует использовать модерацию выпуска, чтобы одобрять, помещать в карантин или
отзывать артефакты.
...
Конечная точка владельца/модератора для просмотра статуса модерации пакета.
...
Карантинные и отозванные релизы возвращают `403` из маршрутов скачивания артефактов.
...
- Ограничение размера файла: 200 КБ.
- Ожидающие проверки VirusTotal не блокируют чтение; вредоносные релизы все еще могут удерживаться в других местах.
- Приватные пакеты возвращают `404`, если вызывающий не может читать данные владельца-издателя.
...
- Ожидающие проверки VirusTotal не блокируют скачивания; вредоносные релизы возвращают `403`.
- Приватные пакеты возвращают `404`, если вызывающий не является владельцем.

### `GET /api/npm/{package}`

Возвращает совместимый с npm packument для версий пакета на основе ClawPack.
...
- Проверки модерации и доступа к приватным пакетам все равно применяются.
...
Скачивает ZIP размещенной версии навыка или возвращает передачу управления к исходному коду GitHub для
текущего навыка на GitHub с проверкой `clean` или `suspicious` и без размещенной
версии.
...
- Передачи управления для навыков на GitHub не проксируют и не зеркалируют байты. JSON-ответ
  включает `sourceRef: "public-github"`, `repo`, `commit`, `path`, `contentHash`
  и `archiveUrl`; состояние проверки/актуальности является условием допуска и не включается как метаданные
  полезной нагрузки успешного ответа.
- Статистика скачиваний считается по уникальным идентификаторам за день UTC (`userId`, когда API-токен действителен, иначе IP).
...
- Необязательное поле payload: `ownerHandle`. При наличии API разрешает этого
...
- Необязательное поле payload: `migrateOwner`. Когда `true` с `ownerHandle`,
  существующий навык может быть перемещен к этому владельцу, если действующее лицо является администратором/владельцем у обоих
...
- Разрешенные поля формы: `payload`, повторяющиеся blob-объекты `files` или одна ссылка tarball
  `clawpack`. `clawpack` может быть blob `.tgz` или storage id, возвращенный
  процессом upload-url. Публикации staged storage-id также должны включать
...
- Прямые multipart-запросы публикации ограничены 18 МБ. Tarball ClawPack могут
  использовать процесс upload-url до лимита tarball 120 МБ.
- Необязательное поле payload: `ownerHandle`. При наличии только администраторы могут публиковать от имени этого владельца.
...
- Публикации от имени другого владельца все равно проверяют право на канал official относительно учетной записи целевого владельца.

### `DELETE /api/v1/skills/{slug}` / `POST /api/v1/skills/{slug}/undelete`

Мягко удалить / восстановить навык (владелец, модератор или администратор).
...
При наличии `reason` сохраняется как примечание модерации навыка и копируется в журнал аудита.
Мягкие удаления, инициированные владельцем, резервируют slug на 30 дней, после чего slug может быть занят
другим издателем. Ответ на удаление включает `slugReservedUntil`, когда применяется этот срок.
Скрытие модератором/администратором и удаления по безопасности таким образом не истекают.
...
Только для администратора. Восстанавливает личного издателя для подтвержденного замещающего принципала GitHub OAuth
без редактирования строк учетных записей Convex Auth. В запросе должны быть указаны оба неизменяемых идентификатора учетных записей
провайдера GitHub; изменяемые handle используются только как ориентированный на оператора предохранитель.
...
GitHub принципалами. Восстановление завершается отказом в закрытом режиме, если у текущего личного
издателя целевого пользователя есть навыки, пакеты или источники навыков GitHub.
Восстановление также переносит устаревшие поля `ownerUserId` для навыков восстановленного издателя,
алиасов slug навыков, пакетов, предупреждений инспектора пакетов и производных строк поискового дайджеста, чтобы
пути прямого владельца соответствовали новому полномочию издателя. Активное резервирование защищенного handle
для восстановленного handle также переназначается пользователю-замене, чтобы последующая
синхронизация профиля не могла восстановить конкурирующее полномочие прежнего пользователя. Каждая основная таблица ограничена
...
Источники навыков GitHub имеют область действия издателя и помечаются как проверенные, а не переписываются.
...
Если вы размещаете самостоятельно, отдавайте этот файл (или явно задайте `CLAWHUB_REGISTRY`; устаревшее `CLAWDHUB_REGISTRY`).
