...
x-i18n:
    generated_at: "2026-07-03T09:49:48Z"
    model: gpt-5.5
...
- Аутентифицированные запросы (действительный Bearer-токен): применяются по корзине пользователя.
- Если токен отсутствует или недействителен, поведение возвращается к применению по IP.
- Аутентифицированные конечные точки записи не должны возвращать голый `Unauthorized`, когда
...
- `X-RateLimit-Reset`: абсолютные секунды эпохи Unix
...
- `X-RateLimit-Remaining` / `RateLimit-Remaining`: точный оставшийся бюджет, если присутствует.
  Сегментированные успешные запросы опускают этот заголовок вместо возврата приблизительного глобального значения.
...
- Если доверенный IP клиента недоступен, анонимные запросы используют fallback-корзины,
  ограниченные только типом rate-limit. Эти fallback-корзины не включают
...
- Результаты возвращаются в порядке релевантности (сходство embedding + точные повышения для токенов slug/имени + небольшой prior популярности).
- Релевантность сильнее популярности. Точное совпадение токена slug или отображаемого имени может ранжироваться выше более свободного совпадения с намного более сильной вовлеченностью.
...
- Помещайте термины, которые пользователи будут буквально искать, в отображаемое имя, сводку и теги. Используйте самостоятельный токен slug только когда он также является стабильной идентичностью, которую вы хотите сохранить.
- Не переименовывайте slug только ради одного запроса, если новый slug не является лучшим долгосрочным каноническим именем. Старые slugs становятся псевдонимами перенаправления, но канонический URL, отображаемый slug и будущие поисковые дайджесты используют новый slug.
...
- Если Skill неожиданно невидим, сначала проверьте состояние модерации с помощью `clawhub inspect @owner/slug` после входа в систему, прежде чем изменять метаданные, связанные с ранжированием.
...
- `sort` (необязательно): `updated` (по умолчанию), `recommended` (псевдоним: `default`), `createdAt` (псевдоним: `newest`), `downloads`, `stars` (псевдоним: `rating`), устаревшие псевдонимы установки `installsCurrent`/`installs`/`installsAllTime` сопоставляются с `downloads`, `trending`
- `nonSuspiciousOnly` (необязательно): `true`, чтобы скрыть подозрительные (`flagged.suspicious`) Skills
- `nonSuspicious` (необязательно): устаревший псевдоним для `nonSuspiciousOnly`
...
- Владельцы и модераторы могут получить доступ к деталям модерации скрытых Skills.
...
возврате `status` в `open`. Передайте `finalAction: "hide"` с триажированной
...
Сканирования локальных загрузок больше не поддерживаются. Запросы с
...
- Возвращает `queue.queuedAhead` и `queue.position`, пока запрос находится в очереди, чтобы клиенты могли показать, сколько приоритетных ручных сканирований находится перед этим запросом. Очень большие очереди ограничиваются и возвращаются с `queuedAheadIsEstimate: true`.
...
- Требует доступа к управлению владельца/издателя для Skills или Plugin либо полномочий модератора/администратора платформы.
...
- `ok` имеет значение `true` только когда выбранная версия имеет сгенерированную Skill Card, не заблокирована модерацией как вредоносная и проверка ClawScan чистая.
...
- `security.signals` содержит вспомогательные доказательства сканера, такие как `staticScan`, `virusTotal` и `skillSpector`.
- `security.signals.dependencyRegistry` сохраняется для совместимости ответа v1, но сканер наличия реестра зависимостей выведен из эксплуатации, и этот ключ всегда равен `null`.
- `provenance` имеет значение `server-resolved-github-import` только когда ClawHub разрешил и сохранил репозиторий/ref/commit/path GitHub во время публикации или импорта; в остальных случаях значение равно `unavailable`.
...
- Результаты возвращаются по каждому элементу; отсутствие одного Skills или версии не приводит к ошибке всего ответа.
...
`trending` — это семидневная таблица лидеров по установкам/загрузкам, которая не использует суммарные показатели за всё время.
...
- Также принимаются устаревшие псевдонимы фильтров v1, документированные в разделе `GET /api/v1/plugins`.
...
- `trust.reasons` — список пользовательских и аудиторских объяснений. Коды причин
...
- Эта конечная точка точна для версии. Клиентам следует вызывать ее после разрешения
  ...
  метаданных пакета.
- Приватные пакеты возвращают `404`, если вызывающий клиент не может читать владеющего издателя.
- Эта конечная точка намеренно уже, чем конечные точки модерации для владельца/модератора.
  Она раскрывает решение об установке и публичное объяснение, а не
  личности отправителей отчетов, тела отчетов, приватные доказательства или внутренние сроки
  проверки.
...
- Версии ClawPack передают поток с точными загруженными байтами npm-pack `.tgz`.
...
- `open`: подозрительные, вредоносные, ожидающие, помещенные в карантин, отозванные или заявленные в отчетах релизы.
...
Сообщает о пакете для проверки модератором. Отчеты относятся к пакету в целом и необязательно
...
- Ограничение размера файла: 200 КБ.
- Ожидающие сканирования VirusTotal не блокируют чтение; вредоносные релизы все равно могут удерживаться в других местах.
...
- Устаревшие версии только в ZIP намеренно опускаются.
...
- `slug` (обязательно)
- `hash` (обязательно): 64-символьный шестнадцатеричный sha256 отпечатка бандла
...
Скачивает ZIP размещенной версии навыка или возвращает передачу на источник GitHub для
текущего навыка на базе GitHub со сканированием `clean` или `suspicious` и без размещенной
версии.
...
- Передачи навыков на базе GitHub не проксируют и не зеркалируют байты. JSON-ответ
...
- Статистика скачиваний считается по уникальным идентичностям за день UTC (`userId`, когда API-токен действителен, иначе IP).
...
- Необязательное поле полезной нагрузки: `migrateOwner`. При `true` вместе с `ownerHandle`
  существующий навык может перейти к этому владельцу, если действующее лицо является администратором/владельцем у обоих
...
- Прямые multipart-запросы публикации ограничены 18 МБ. Tarball-файлы ClawPack могут
  использовать поток upload-url до лимита tarball в 120 МБ.
...
- Публикации от имени другого владельца все равно проверяют право на канал official по учетной записи целевого владельца.

### `DELETE /api/v1/skills/{slug}` / `POST /api/v1/skills/{slug}/undelete`

Мягкое удаление / восстановление навыка (владелец, модератор или администратор).
...
Если `reason` присутствует, оно сохраняется как заметка модерации навыка и копируется в журнал аудита.
...
- `401`: не авторизован
- `403`: запрещено
- `404`: навык/пользователь не найден
- `500`: внутренняя ошибка сервера

### `POST /api/v1/users/publisher`

Только для администраторов. Гарантирует, что издатель организации существует для handle. Если handle все еще указывает на
...
релиза. Имена пакетов становятся приватными пакетами-заполнителями без строк релиза, поэтому тот же
...
без редактирования строк учетной записи Convex Auth. Запрос должен указывать оба неизменяемых
идентификатора учетной записи провайдера GitHub; изменяемые handle используются только как защита для оператора.

Конечная точка по умолчанию работает в режиме dry-run. Для применения восстановления требуются `dryRun: false` и
`confirmIdentityVerified: true` после того, как сотрудники независимо проверят непрерывность между обеими
учетными записями GitHub. Восстановление завершается отказом, если у текущего личного
...
пути прямого владельца соответствовали новым полномочиям издателя. Активное защищенное
резервирование handle для восстановленного handle также переназначается пользователю-замене, чтобы последующая
...
- Для обеих конечных точек требуется аутентификация с API-токеном, и они работают только для владельца навыка.
...
контента (только администратор). По умолчанию используется режим dry-run, если `dryRun` не равно `false`.
...
которые подготавливают tarball ClawPack, нужно отправлять полученный идентификатор хранилища как
