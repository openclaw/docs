...
x-i18n:
    generated_at: "2026-06-28T22:39:33Z"
    model: gpt-5.5
...
Сторонние каталоги могут использовать публичные конечные точки чтения, чтобы выводить список или искать Skills ClawHub. Кэшируйте результаты, соблюдайте `429`/`Retry-After`, направляйте пользователей обратно к канонической странице ClawHub (`https://clawhub.ai/<owner>/skills/<slug>`) и не создавайте впечатление, что ClawHub одобряет сторонний сайт. Не пытайтесь зеркалировать скрытый, приватный или заблокированный модерацией контент за пределами поверхности публичного API.
...
- Аутентифицированные запросы (действительный токен Bearer): ограничиваются по пользовательскому бакету.
...
- Аутентифицированные конечные точки записи не должны возвращать голый `Unauthorized`, когда
  ...
  удаленные, заблокированные или отключенные учетные записи должны получать действенный текст, чтобы CLI
...
- `nonSuspicious` (необязательно): устаревший alias для `nonSuspiciousOnly`
...
- Результаты возвращаются в порядке релевантности (embedding-сходство + усиления для точного совпадения токенов slug/имени + небольшой априорный вес популярности).
- Релевантность сильнее популярности. Точное совпадение slug или токена отображаемого имени может ранжироваться выше более свободного совпадения с гораздо более сильной вовлеченностью.
...
- `sort` (необязательно): `updated` (по умолчанию), `recommended` (alias: `default`), `createdAt` (alias: `newest`), `downloads`, `stars` (alias: `rating`), устаревшие alias установок `installsCurrent`/`installs`/`installsAllTime` сопоставляются с `downloads`, `trending`
- `nonSuspiciousOnly` (необязательно): `true`, чтобы скрыть подозрительные (`flagged.suspicious`) Skills
- `nonSuspicious` (необязательно): устаревший alias для `nonSuspiciousOnly`
...
- `recommended` использует сигналы вовлеченности и актуальности.
...
- Когда `nonSuspiciousOnly=true`, сортировки на основе курсора могут возвращать на странице меньше элементов, чем `limit`, потому что подозрительные Skills фильтруются после получения страницы.
...
- Старые slug, созданные потоками переименования или слияния владельца, разрешаются в канонический Skill.
...
Конечная точка модератора/администратора для приема отчетов по Skills.
...
Конечная точка модератора/администратора для разрешения или повторного открытия отчетов по Skills.
...
- `version.security` включает нормализованный статус проверки сканированием и сведения о сканере
...
- Полезные нагрузки запросов сканирования и загружаемые отчеты удаляются из хранилища scan-request после окончания периода хранения.
- Для опубликованных сканирований требуется доступ на управление для владельца/издателя либо полномочия модератора/администратора платформы.
...
- Задания сканирования выполняются асинхронно. Ручные запросы сканирования получают приоритет перед обычной работой публикации/обратного заполнения, но завершение все равно зависит от доступности воркеров.
...
- Требуется успешно завершенное сканирование; нетерминальные сканирования возвращают `409`.
...
Аутентифицированная конечная точка сохраненного архива отчетов для отправленных версий.

- Требуется доступ на управление для владельца/издателя к навыку или plugin либо полномочия модератора/администратора платформы.
...
Возвращает оболочку проверки карточки навыка, используемую `clawhub skill verify`.
...
- `ok` равно `true` только когда для выбранной версии есть сгенерированная карточка навыка, она не заблокирована модерацией как вредоносная, и проверка ClawScan чистая.
- Идентичность навыка, идентичность издателя и метаданные выбранной версии являются полями верхнего уровня оболочки (`slug`, `displayName`, `publisherHandle`, `version`, `resolvedFrom`, `tag`, `createdAt`), чтобы shell-автоматизация могла читать их без распаковки вложенных оберток.
...
- `security.signals` содержит только подтверждающие свидетельства уровня статуса; используйте `/scan` или страницу аудита безопасности ClawHub для полных сведений сканера.
- `security.signals.dependencyRegistry` сохранен для совместимости ответа v1, но сканер наличия реестра зависимостей выведен из эксплуатации, и этот ключ всегда равен `null`.
...
- Используйте `/verify`, когда нужна оболочка проверки карточки для одного навыка, `/card`, когда нужен Markdown сгенерированной карточки, и `/scan`, когда нужны подробные данные сканеров.
...
`trending` — это рейтинг установок/загрузок за семь дней, который не использует общие показатели за все время.
...
- Устаревшие псевдонимы фильтров v1, задокументированные в `GET /api/v1/plugins`, также
  принимаются.
- Фильтрация по категориям — это настоящий фильтр API на основе строк дайджеста категорий Plugin,
  а не переписывание поискового запроса.
- Результаты возвращаются в порядке релевантности и сейчас не имеют пагинации.
- Элементы управления сортировкой в браузерном интерфейсе для поиска Plugin переупорядочивают загруженные результаты по релевантности,
  в соответствии с текущим поведением просмотра `/skills`.
...
- Требует токен API владельца пакета, владельца/администратора издателя-организации,
...
установки. Это публичная поверхность потребления OpenClaw для принятия решения о том, можно ли
...
- Эта конечная точка намеренно уже, чем конечные точки модерации для владельцев/модераторов.
  ...
  личности репортеров, тела отчетов, приватные доказательства или внутренние
  графики проверки.
...
- Версии ClawPack передают потоком точные загруженные байты npm-pack `.tgz`.
...
Сообщает о пакете для проверки модератором. Отчеты создаются на уровне пакета, при необходимости
связываются с версией. Они попадают в очередь модерации, но сами по себе не скрывают автоматически и не
...
`finalAction: "revoke"` с подтвержденным отчетом, чтобы применить модерацию релиза в том
же проверяемом рабочем процессе.
...
- Использует лимит скорости чтения, а не лимит скачивания.
...
Передает точные байты загруженного tarball-файла ClawPack для клиентов npm-зеркала.

Примечания:

- Использует лимит скорости скачивания.
...
Скачивает ZIP размещенной версии skill или возвращает передачу управления источнику GitHub для
...
- Передачи управления для skill на основе GitHub не проксируют и не зеркалируют байты. JSON-ответ
  включает `sourceRef: "public-github"`, `repo`, `commit`, `path`, `contentHash`
  и `archiveUrl`; состояние сканирования/актуальности является условием допуска и не включается как метаданные
...
- Необязательное поле payload: `migrateOwner`. При `true` с `ownerHandle`
  существующий skill может перейти к этому владельцу, если действующее лицо является администратором/владельцем как у
...
- Разрешенные поля формы: `payload`, повторяющиеся blob-объекты `files` или одна ссылка на tarball
  `clawpack`. `clawpack` может быть blob-объектом `.tgz` или id хранилища, возвращенным
...
- Code plugins требуют `package.json`, метаданные исходного репозитория, метаданные исходного commit,
  метаданные config schema, `openclaw.compat.pluginApi` и
...
Скрытия модератором/администратором и удаления по соображениям безопасности так не истекают.
...
без редактирования строк учетной записи Convex Auth. Запрос должен указать оба неизменяемых id учетных записей
...
субъектами GitHub. Восстановление завершается отказом в закрытом режиме, если у текущего персонального
издателя целевого пользователя есть Skills, пакеты или источники GitHub для Skills.
Восстановление также переносит устаревшие поля `ownerUserId` для Skills восстановленного издателя,
псевдонимов slug Skills, пакетов, предупреждений инспектора пакетов и производных строк дайджеста поиска, чтобы
...
Источники GitHub для Skills привязаны к издателю и помечаются как проверенные, а не переписываются.
...
- Оба эндпоинта требуют аутентификации с API-токеном и работают только для владельца Skills.
...
Забанить пользователя и необратимо удалить принадлежащие ему Skills (только moderator/admin).
...
Разбанить пользователя и восстановить подходящие Skills (только admin).
...
Изменить сохраненную причину существующего бана без разбана или восстановления
