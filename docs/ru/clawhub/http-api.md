...
x-i18n:
    generated_at: "2026-07-03T02:52:17Z"
    model: gpt-5.5
...
Сторонние каталоги могут использовать публичные конечные точки чтения, чтобы выводить список Skills ClawHub или искать по ним. Кэшируйте результаты, соблюдайте `429`/`Retry-After`, направляйте пользователей обратно на каноническую страницу ClawHub (`https://clawhub.ai/<owner>/skills/<slug>`) и не создавайте впечатление, что ClawHub одобряет сторонний сайт. Не пытайтесь зеркалировать скрытый, приватный или заблокированный модерацией контент за пределами публичной поверхности API.
...
- Аутентифицированные запросы (действительный Bearer-токен): ограничиваются по пользовательской корзине.
...
Публичные ответы с ошибками v1 являются обычным текстом с `content-type: text/plain; charset=utf-8`.
...
- Результаты возвращаются в порядке релевантности (сходство embeddings + усиления точных токенов slug/имени + небольшой априорный вес популярности).
- Релевантность сильнее популярности. Точное совпадение токена slug или отображаемого имени может ранжироваться выше, чем менее точное совпадение с гораздо более сильным вовлечением.
...
- `createdAt` стабилен для обхода новых Skills; `updated` меняется, когда существующие Skills публикуются повторно.
...
Сообщить о Skill для проверки модератором. Жалобы действуют на уровне Skill, необязательно привязаны
...
возврате `status` в `open`. Передайте `finalAction: "hide"` с проверенной
...
- Для сканирования опубликованных версий требуется доступ к управлению владельца/издателя либо полномочия модератора/администратора платформы.
- Сканирования опубликованных версий записывают результаты обратно только когда `update: true` и сканирование успешно завершено.
...
- Задания сканирования асинхронны. Ручные запросы сканирования имеют приоритет над обычной работой публикации/обратного заполнения, но завершение по-прежнему зависит от доступности воркеров.
...
Канонический маршрут статуса пакетной операции только для администраторов. Он принимает `{ "jobIds": ["..."] }` и возвращает те же агрегированные счетчики, что и устаревший `POST /api/v1/skills/-/rescan-batch/status`.
...
- `ok` равно `true` только когда для выбранной версии есть сгенерированная карточка навыка, она не заблокирована модерацией как вредоносная, а проверка ClawScan чистая.
- Идентичность навыка, идентичность издателя и метаданные выбранной версии находятся в полях верхнего уровня конверта (`slug`, `displayName`, `publisherHandle`, `version`, `resolvedFrom`, `tag`, `createdAt`), чтобы shell-автоматизация могла читать их без распаковки вложенных оберток.
...
- `provenance` равно `server-resolved-github-import` только когда ClawHub разрешил и сохранил репозиторий GitHub/ref/commit/path во время публикации или импорта; в остальных случаях оно равно `unavailable`.

### `POST /api/v1/skills/-/security-verdicts`

Возвращает текущие компактные вердикты безопасности для точных версий навыков. Эта
конечная точка коллекции предназначена для клиентов, которые уже знают, какие версии навыков
ClawHub им нужно отображать, например OpenClaw Control UI.
...
- `security.signals` содержит только вспомогательные доказательства на уровне статуса; используйте `/scan` или страницу аудита безопасности ClawHub для полных сведений сканеров.
- `security.signals.dependencyRegistry` сохранен для совместимости ответа v1, но сканер существования реестра зависимостей выведен из эксплуатации, и этот ключ всегда равен `null`.
...
`trending` — это семидневная таблица лидеров по установкам/загрузкам, и она не использует общие показатели за всё время.
...
- Устаревшие псевдонимы фильтров v1, задокументированные в `GET /api/v1/plugins`, также
  принимаются.
- Фильтрация по категории является настоящим API-фильтром на основе строк дайджеста
...
Мягко удаляет пакет и все выпуски.
...
- `version.artifact.kind` равен `legacy-zip` для архивов пакетов старого формата или
...
установки. Это публичная поверхность потребления OpenClaw для принятия решения о том, может ли
разрешенный выпуск быть установлен.
...
- `trust.blockedFromDownload` — сигнал блокировки установки. OpenClaw и другим
  ...
  повторного выведения правил блокировки из полей сканера или модерации.
...
- Эта конечная точка намеренно уже, чем конечные точки модерации владельца/модератора.
  ...
  личности отправителей жалоб, тексты жалоб, приватные доказательства или внутренние графики
  проверки.
...
- Версии ClawPack передают поток точных байтов загруженного npm-pack `.tgz`.
...
Сообщить о пакете для проверки модератором. Жалобы относятся к уровню пакета и могут быть
...
`finalAction: "revoke"` с подтвержденным отчетом, чтобы применить модерацию выпуска в том же
...
- Использует лимитный бакет чтения, а не бакет скачивания.
...
- Приватные пакеты возвращают `404`, если вызывающий субъект не может читать владеющего издателя.
...
- Приватные пакеты возвращают `404`, если вызывающий субъект не является владельцем.
...
- Перечисляются только версии с загруженными tarball npm-pack ClawPack.
- Устаревшие версии только ZIP намеренно пропущены.
...
- Packument пакетов с областью видимости поддерживают как `/api/npm/@scope/name`, так и путь запроса npm
  с кодированием `/api/npm/@scope%2Fname`.
...
Скачивает ZIP размещенной версии Skill или возвращает передачу GitHub-источника для
текущей Skill на базе GitHub со сканированием `clean` или `suspicious` и без размещенной
версии.
...
- Необязательное поле полезной нагрузки: `migrateOwner`. При `true` вместе с `ownerHandle`
  существующая Skill может перейти к этому владельцу, если актер является администратором/владельцем как у
...
- JSON-тела и предоставленные вызывающим субъектом метаданные `payload.files` / `payload.artifact`
  отклоняются.
...
- Публикации от имени другого субъекта все равно проверяют право на official-канал относительно учетной записи целевого владельца.
...
вызывающего субъекта как владельца. Эта конечная точка не мигрирует существующие пользовательские/личные handles и
...
владелец позднее может опубликовать настоящий выпуск code-plugin или bundle-plugin под этим именем.
...
без редактирования строк учетных записей Convex Auth. В запросе должны быть указаны оба неизменяемых GitHub
provider account id; изменяемые handles используются только как ориентированный на оператора предохранитель.
...
субъектами GitHub. Восстановление завершается закрыто, если у текущего личного издателя целевого пользователя
есть skills, packages или источники GitHub skill.
Восстановление также мигрирует устаревшие поля `ownerUserId` для skills восстановленного издателя,
псевдонимов slug skill, packages, предупреждений инспектора пакетов и производных строк поискового дайджеста, чтобы
пути прямого владельца согласовывались с новой полномочностью издателя. Активная резервация защищенного handle
...
Источники GitHub skill имеют область действия издателя и указываются как проверенные, а не переписываются.
...
контента (только администратор). По умолчанию работает в режиме dry-run, если `dryRun` не равен `false`.
...
## Устаревшие конечные точки CLI (не рекомендуется)
