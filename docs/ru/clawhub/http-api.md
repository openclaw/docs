...
x-i18n:
    generated_at: "2026-07-02T17:42:38Z"
    model: gpt-5.5
...
Сторонние каталоги могут использовать публичные конечные точки чтения, чтобы перечислять или искать Skills ClawHub. Кэшируйте результаты, соблюдайте `429`/`Retry-After`, направляйте пользователей обратно к канонической странице ClawHub (`https://clawhub.ai/<owner>/skills/<slug>`) и не создавайте впечатление, что ClawHub одобряет сторонний сайт. Не пытайтесь зеркалировать скрытый, приватный или заблокированный модерацией контент за пределами публичной поверхности API.

Веб-сокращения slug разрешаются между семействами реестра, но клиенты API должны использовать
канонические URL, возвращаемые конечными точками чтения, вместо самостоятельного восстановления приоритета
маршрутов.
...
- Конечные точки записи с аутентификацией не должны возвращать простой `Unauthorized`, когда
...
- ClawHub использует доверенные заголовки пересылки для идентификации клиентских IP на edge.
- Если доверенный клиентский IP недоступен, анонимные запросы используют резервные бакеты,
  ограниченные только типом ограничения частоты. Эти резервные бакеты не включают
...
- Результаты возвращаются в порядке релевантности (сходство embedding + повышения за точное совпадение токена slug/имени + небольшой приоритет популярности).
- Релевантность сильнее популярности. Точное совпадение токена slug или отображаемого имени может ранжироваться выше, чем более свободное совпадение с гораздо большей вовлеченностью.
...
Рекомендации для обнаруживаемости издателями:

- Помещайте термины, которые пользователи будут буквально искать, в отображаемое имя, краткое описание и теги. Используйте отдельный токен slug только тогда, когда это также стабильная идентичность, которую вы хотите сохранить.
...
- Старые slug, созданные потоками переименования/объединения владельца, разрешаются в канонический Skill.
...
Пожаловаться на Skill для проверки модератором. Жалобы относятся к уровню Skill, необязательно связаны
...
Возвращает подробности проверки сканирования безопасности для версии Skill.
...
- Сканирование опубликованных версий требует управленческого доступа владельца/публикатора либо полномочий модератора/администратора платформы.
- Сканирование опубликованных версий записывает результаты обратно только когда `update: true` и сканирование успешно завершено.
...
- Задания сканирования асинхронны. Ручные запросы сканирования получают приоритет перед обычной публикацией/обратным заполнением, но завершение все равно зависит от доступности воркеров.

### `GET /api/v1/skills/-/scan/{scanId}`

Аутентифицированная конечная точка опроса отправленного сканирования.

- Возвращает статус в очереди/выполняется/успешно/ошибка.
...
- `kind` по умолчанию равен `skill`; используйте `kind=plugin` для сканирования plugins/пакетов.
...
- Идентификатор навыка, идентификатор публикатора и метаданные выбранной версии являются полями верхнего уровня конверта (`slug`, `displayName`, `publisherHandle`, `version`, `resolvedFrom`, `tag`, `createdAt`), чтобы shell-автоматизация могла читать их без распаковки вложенных оберток.
...
- Результаты возвращаются по каждому элементу; один отсутствующий навык или версия не приводят к сбою всего ответа.
...
- `security.signals` содержит только вспомогательные доказательства на уровне статуса; используйте `/scan` или страницу аудита безопасности ClawHub для полных данных сканеров.
- `security.signals.dependencyRegistry` сохранен для совместимости ответа v1, но сканер существования реестра зависимостей выведен из эксплуатации, и этот ключ всегда равен `null`.
...
- Записи Skills по-прежнему поддерживаются реестром Skills и всё еще могут публиковаться только через `POST /api/v1/skills`.
...
Единый поиск по каталогу в Skills и пакетах Plugin.
...
Обзор каталога только Plugin по пакетам code-plugin и bundle-plugin.
...
`trending` — это лидерборд установок/загрузок за семь дней, который не использует совокупные итоги за всё время.
...
Устаревшие псевдонимы не принимаются как сохраненные или объявленные авторами значения категорий.
...
- `trust.moderationState` допускает null. Значение `null` используется, когда ручная
...
- Эта конечная точка привязана к точной версии. Клиенты должны вызывать ее после разрешения
  версии пакета, которую они собираются установить, а не только после чтения последних
  метаданных пакета.
- Приватные пакеты возвращают `404`, если вызывающий не может читать владельца-издателя.
- Эта конечная точка намеренно уже, чем конечные точки модерации для владельцев/модераторов.
  Она раскрывает решение об установке и публичное объяснение, но не
...
- Использует бакет ограничения частоты скачивания.
...
- Ожидающие проверки VirusTotal не блокируют чтение; вредоносные релизы все еще могут удерживаться в другом месте.
- Приватные пакеты возвращают `404`, если вызывающий не может читать владельца-издателя.
...
- Необязательное поле payload: `migrateOwner`. Когда `true` с `ownerHandle`, существующий
  skill может быть перемещен к этому владельцу, если действующее лицо является администратором/владельцем у обоих
...
- Разрешенные поля формы: `payload`, повторяющиеся blob-объекты `files` или одна ссылка на tarball
  ...
  потоком upload-url. Публикации через staged storage-id также должны включать
...
- Публикации от имени другого владельца все равно проверяют право на официальный канал по целевой учетной записи владельца.
...
без редактирования строк учетных записей Convex Auth. Запрос должен указать оба неизменяемых id учетной записи
провайдера GitHub; изменяемые handle используются только как ориентированная на оператора защита.

Эндпоинт по умолчанию работает в режиме пробного запуска. Применение восстановления требует `dryRun: false` и
`confirmIdentityVerified: true` после того, как сотрудники независимо подтвердят непрерывность между обеими
учетными записями GitHub. Восстановление безопасно завершается отказом, если у текущего личного
издателя пользователя назначения есть навыки, пакеты или источники навыков GitHub.
...
пути прямого владельца согласовывались с новой полномочной записью издателя. Активное резервирование защищенного handle
...
Источники навыков GitHub имеют область видимости издателя и сообщаются как проверенные, а не переписываются.
...
Добавить/убрать звездочку (избранное). Оба эндпоинта идемпотентны.
