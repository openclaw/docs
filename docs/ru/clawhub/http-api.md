...
Сторонние каталоги могут использовать публичные конечные точки чтения, чтобы выводить список Skills ClawHub или выполнять поиск по ним. Кэшируйте результаты, соблюдайте `429`/`Retry-After`, отправляйте пользователей обратно к канонической странице ClawHub (`https://clawhub.ai/<owner>/skills/<slug>`) и не создавайте впечатления, что ClawHub одобряет сторонний сайт. Не пытайтесь зеркалировать скрытый, приватный или заблокированный модерацией контент за пределами публичной поверхности API.
...
- Аутентифицированные запросы (действительный Bearer token): применяются по пользовательскому bucket.
- Если токен отсутствует или недействителен, поведение откатывается к ограничениям по IP.
- Аутентифицированные конечные точки записи не должны возвращать голое `Unauthorized`, когда
  ...
  удаленные/забаненные/отключенные учетные записи должны получать полезный текст, чтобы CLI
...
- Использует доверенные заголовки IP клиента, включая `cf-connecting-ip`, только когда
  развертывание явно включает доверенные forwarded headers.
- ClawHub использует доверенные forwarding headers, чтобы определять IP клиентов на edge.
- Если доверенный IP клиента недоступен, анонимные запросы используют fallback buckets,
  ограниченные только видом rate limit. Эти fallback buckets не включают
  переданные вызывающей стороной пути, slugs, имена пакетов, версии, query strings или другие
  параметры артефактов.

## Ответы с ошибками

Публичные ответы об ошибках v1 являются plain text с `content-type: text/plain; charset=utf-8`.
Сюда входят ошибки валидации (`400`), отсутствующие публичные ресурсы (`404`), ошибки auth и
прав доступа (`401`/`403`), rate limits (`429`) и заблокированные скачивания. Клиенты
...
- `highlightedOnly` (необязательно): `true`, чтобы фильтровать только выделенные Skills
- `nonSuspiciousOnly` (необязательно): `true`, чтобы скрыть подозрительные (`flagged.suspicious`) Skills
- `nonSuspicious` (необязательно): устаревший алиас для `nonSuspiciousOnly`
...
- Результаты возвращаются в порядке релевантности (embedding similarity + усиления точного совпадения slug/name token + небольшой popularity prior).
- Релевантность сильнее популярности. Точное совпадение slug или display-name token может обойти более свободное совпадение с гораздо более высокой вовлеченностью.
- ASCII-текст токенизируется по границам слов и пунктуации. Например, `personal-map` содержит самостоятельный token `map`, а `amap-jsapi-skill` содержит `amap`, `jsapi` и `skill`; поэтому поиск `map` дает `personal-map` более сильное лексическое совпадение, чем `amap-jsapi-skill`.
...
- Поместите термины, которые пользователи буквально будут искать, в display name, summary и tags. Используйте самостоятельный slug token только когда он также является стабильной идентичностью, которую вы хотите сохранить.
- Не переименовывайте slug только ради одного запроса, если новый slug не является лучшим долгосрочным каноническим именем. Старые slugs становятся redirect aliases, но канонический URL, отображаемый slug и будущие search digests используют новый slug.
- Rename aliases сохраняют разрешение для старых URL и установок, которые разрешаются через реестр, но поисковое ранжирование основано на канонических метаданных Skill после того, как переименование проиндексировано. Существующая статистика остается со Skill.
...
- `sort` (необязательно): `updated` (по умолчанию), `recommended` (алиас: `default`), `createdAt` (алиас: `newest`), `downloads`, `stars` (алиас: `rating`), устаревшие алиасы установок `installsCurrent`/`installs`/`installsAllTime` сопоставляются с `downloads`, `trending`
- `nonSuspiciousOnly` (необязательно): `true`, чтобы скрыть подозрительные (`flagged.suspicious`) Skills
- `nonSuspicious` (необязательно): устаревший алиас для `nonSuspiciousOnly`
...
- Когда `nonSuspiciousOnly=true`, сортировки на основе cursor могут возвращать меньше элементов, чем `limit`, на странице, потому что подозрительные Skills фильтруются после получения страницы.
...
- `metadata.os`: ограничения OS, объявленные во frontmatter Skill (например, `["macos"]`, `["linux"]`). `null`, если не объявлено.
...
- Публичные вызывающие стороны получают `200` только для уже отмеченных видимых Skills.
- Evidence редактируется для публичных вызывающих сторон и включает raw snippets только для владельцев/модераторов.

### `POST /api/v1/skills/{slug}/report`

Сообщить о Skill для проверки модератором. Отчеты относятся к уровню Skill, необязательно связаны
с версией и попадают в очередь отчетов по Skill.
...
Конечная точка модератора/администратора для приема отчетов по Skill.
...
Конечная точка модератора/администратора для разрешения или повторного открытия отчетов по Skill.
...
возврате `status` к `open`. Передайте `finalAction: "hide"` с triaged
report, чтобы скрыть Skill в том же аудируемом workflow.
...
- `tag` (необязательно): разрешить tagged version (например, `latest`).
...
- При запросе исторической версии проверяйте `moderation.matchesRequestedVersion` и `moderation.sourceVersion`, прежде чем считать `moderation` и `security` относящимися к одному и тому же контексту версии.

### `POST /api/v1/skills/-/scan`

Аутентифицированный эндпоинт отправки для новых заданий ClawScan.
...
- Полезные нагрузки запросов сканирования и скачиваемые отчеты удаляются из хранилища запросов сканирования после истечения окна хранения.
- Для сканирования опубликованных версий требуется управленческий доступ владельца/издателя либо права модератора/администратора платформы.
- Сканирование опубликованных версий выполняет обратную запись только когда `update: true` и сканирование успешно завершается.
...
- Задания сканирования асинхронны. Ручные запросы сканирования имеют приоритет перед обычной работой публикации/обратного заполнения, но завершение все равно зависит от доступности рабочих процессов.

### `GET /api/v1/skills/-/scan/{scanId}`

Аутентифицированный эндпоинт опроса для отправленного сканирования.

- Возвращает статус в очереди/выполняется/успешно/с ошибкой.
- Возвращает `queue.queuedAhead` и `queue.position`, пока запрос находится в очереди, чтобы клиенты могли показывать, сколько приоритетных ручных сканирований находится перед запросом. Очень большие очереди ограничиваются и возвращаются с `queuedAheadIsEstimate: true`.
...
Аутентифицированный эндпоинт архива отчета.
...
Аутентифицированный эндпоинт архива сохраненного отчета для отправленных версий.

- Требует управленческий доступ владельца/издателя к навыку или plugin либо права модератора/администратора платформы.
- Возвращает сохраненные результаты сканирования для точной отправленной версии, включая заблокированные или скрытые версии.
- `kind` по умолчанию равно `skill`; используйте `kind=plugin` для сканирования plugin/пакетов.
- Возвращает ту же форму ZIP, что и скачивания запросов сканирования.
...
- `version` (необязательно): строка конкретной версии.
- `tag` (необязательно): разрешает версию с тегом (например, `latest`).
...
- `provenance` равно `server-resolved-github-import` только когда ClawHub разрешил и сохранил репозиторий/ref/commit/path GitHub во время публикации или импорта; в противном случае значение равно `unavailable`.

### `POST /api/v1/skills/-/security-verdicts`

Возвращает текущие компактные вердикты безопасности для точных версий навыков. Этот
эндпоинт коллекции предназначен для клиентов, которые уже знают, какие установленные
...
- Ответ относится только к безопасности. Он не включает данные карточки навыка, статус сгенерированной карточки, списки файлов артефактов или подробные полезные нагрузки сканеров.
...
- `security.signals.dependencyRegistry` сохранен для совместимости ответа v1, но сканер существования реестра зависимостей выведен из эксплуатации, и этот ключ всегда равен `null`.
- Отсутствие карточки навыка не влияет на `ok`, `decision` или `reasons` этого эндпоинта; клиенты должны читать установленный `skill-card.md` локально, когда им нужно содержимое карточки.
...
- `category` (необязательный): фильтр категории плагинов. Поддерживается только когда
...
- `category` (необязательный): фильтр категории плагинов. Текущие значения:
...
`trending` — это рейтинг установок/загрузок за семь дней, и он не использует итоговые значения за все время.
В единой конечной точке `/api/v1/packages` он применяется только к плагинам; используйте
...
Массовый экспорт последних общедоступных Skills для офлайн-анализа.
...
Массовый экспорт последних публичных выпусков Plugin для автономного анализа.
...
- `startDate` (обязательно): нижняя граница Unix в миллисекундах для `updatedAt` Plugin.
- `endDate` (обязательно): верхняя граница Unix в миллисекундах для `updatedAt` Plugin.
- `limit` (необязательно): целое число (1-250), по умолчанию `250`.
- `cursor` (необязательно): курсор пагинации из предыдущего ответа.
- `family` (необязательно): `code-plugin` или `bundle-plugin`. Если опущено, используются оба
  семейства Plugin.
...
- Устаревшие псевдонимы фильтров v1, задокументированные в `GET /api/v1/plugins`, также
  принимаются.
- Фильтрация по категории является настоящим API-фильтром, основанным на строках
...
Помечает пакет и все выпуски как удаленные.

Примечания:

- Требуется токен API владельца пакета, владельца/администратора издателя-организации,
...
- `version.sha256hash` — устаревшие метаданные совместимости для старых клиентов. Они
  хэшируют точные байты ZIP, возвращаемые `/api/v1/packages/{name}/download`.
...
Возвращает точную сводку безопасности и доверия выпуска пакета для клиентов
...
- `trust.moderationState` допускает null. Оно равно `null`, когда ручная модерация выпуска
  отсутствует.
...
- Версии ClawPack возвращают артефакт `npm-pack`, поля целостности npm,
  `tarballUrl` и URL совместимости для устаревшего ZIP.
...
- Версии ClawPack передают потоково точные загруженные байты npm-pack `.tgz`.
...
- дайджест артефакта
- происхождение репозитория исходного кода и коммита
...
- `all`: любой выпуск с ручным переопределением, не-чистым состоянием сканирования или жалобой на пакет.
...
Сообщить о пакете для проверки модератором. Жалобы относятся к уровню пакета и могут
опционально связываться с версией. Они попадают в очередь модерации, но сами по себе
не скрывают и не блокируют загрузки автоматически; модераторам следует использовать модерацию выпусков, чтобы
...
Moderator/admin endpoint для приема отчетов о пакетах.

Аутентификация:

- Требуется API-токен пользователя с ролью moderator или admin.
...
Owner/moderator endpoint для видимости модерации пакета.

Аутентификация:

- Требуется API-токен владельца пакета, участника издателя, moderator или
  пользователя admin.
...
Moderator/admin endpoint для разрешения или повторного открытия отчетов о пакетах.
...
Moderator/admin endpoint для проверки релиза пакета.
...
Релизы в состоянии quarantine и revoked возвращают `403` из маршрутов скачивания артефактов.
...
- Ограничение размера файла: 200 КБ.
- Ожидающие сканы VirusTotal не блокируют чтение; вредоносные релизы все еще могут удерживаться в других местах.
...
- Архивы Plugin/пакетов являются zip-файлами с корнем `package/`, чтобы старые клиенты OpenClaw
  продолжали работать.
- Этот маршрут остается только ZIP. Он не транслирует ClawPack-файлы `.tgz`.
- Ответы включают заголовки `ETag`, `Digest`, `X-ClawHub-Artifact-Type` и
  `X-ClawHub-Artifact-Sha256` для проверок целостности resolver.
- Метаданные только реестра не внедряются в скачанный архив.
- Ожидающие сканы VirusTotal не блокируют скачивания; вредоносные релизы возвращают `403`.
...
- Устаревшие версии только в ZIP намеренно опущены.
- `dist.tarball`, `dist.integrity` и `dist.shasum` используют npm-совместимые
  поля, чтобы пользователи могли указать npm на зеркало, если выберут это.
...
Транслирует точные байты загруженного tarball ClawPack для клиентов npm-зеркала.

Примечания:

- Использует квоту частоты скачивания.
- Заголовки скачивания включают SHA-256 ClawHub плюс метаданные integrity/shasum npm.
...
Скачивает ZIP размещенной версии skill или возвращает передачу к исходникам GitHub для
...
- Передачи для skill на основе GitHub не проксируют и не зеркалируют байты. JSON-ответ
  ...
  и `archiveUrl`; состояние scan/current является gate и не включается как метаданные
  успешной полезной нагрузки.
- Статистика скачиваний считается как уникальные идентичности за UTC-день (`userId`, когда API-токен действителен, иначе IP).

## Auth endpoints (Bearer token)

Все endpoint требуют:
...
- Необязательное поле payload: `ownerHandle`. Когда оно присутствует, API разрешает этого
  издателя на стороне сервера и требует, чтобы actor имел доступ к издателю.
- Необязательное поле payload: `migrateOwner`. Когда `true` вместе с `ownerHandle`, существующий
  skill может перейти к этому владельцу, если actor является admin/owner у обоих
...
- Необязательное поле payload: `ownerHandle`. Когда оно присутствует, только admins могут публиковать от имени этого владельца.
...
- Code plugins требуют `package.json`, метаданные исходного репозитория, метаданные исходного commit,
...
- Публикации от имени другого владельца по-прежнему проверяют право на official-channel относительно целевой учетной записи владельца.

### `DELETE /api/v1/skills/{slug}` / `POST /api/v1/skills/{slug}/undelete`

Soft-delete / восстановление skill (owner, moderator или admin).
...
Когда `reason` присутствует, он сохраняется как примечание модерации skill и копируется в журнал аудита.
Soft delete, инициированный владельцем, резервирует slug на 30 дней, затем slug может быть занят
другим издателем. Ответ удаления включает `slugReservedUntil`, когда применим этот срок истечения.
Скрытия moderator/admin и удаления по безопасности таким образом не истекают.
...
Только admin. Гарантирует существование издателя организации для handle. Если handle все еще указывает на
устаревшего общего пользователя/личного издателя, endpoint сначала мигрирует его в издателя организации.
Для вновь созданной организации укажите `memberHandle`; действующий admin не добавляется как участник.
...
вызывающего как владельца. Этот endpoint не мигрирует существующие пользовательские/личные handle и
...
Только admin. Резервирует корневые slug и имена пакетов для законного владельца без публикации
...
Только admin. Восстанавливает личного издателя для проверенного заменяющего principal GitHub OAuth
без редактирования строк учетных записей Convex Auth. Запрос должен называть оба неизменяемых id учетных записей
провайдера GitHub; изменяемые handle используются только как guard, видимый оператору.

Конечная точка по умолчанию работает в режиме пробного запуска. Применение восстановления требует `dryRun: false` и
`confirmIdentityVerified: true` после того, как сотрудники независимо проверят непрерывность между обеими
учетными записями GitHub. Восстановление завершается закрытым отказом, если у текущего персонального
издателя целевого пользователя есть Skills, пакеты или источники GitHub Skills.
Восстановление также переносит устаревшие поля `ownerUserId` для Skills восстановленного издателя,
псевдонимов slug Skill, пакетов, предупреждений инспектора пакетов и производных строк поискового дайджеста, чтобы
...
Источники GitHub Skills имеют область действия издателя и отображаются как проверенные, а не перезаписываются.
...
контента (только администратор). По умолчанию выполняется пробный запуск, если `dryRun` не равно `false`.
