...
summary: Справочник HTTP API (публичные эндпоинты + эндпоинты CLI + аутентификация).
x-i18n:
    generated_at: "2026-07-02T22:40:41Z"
    model: gpt-5.5
...
Сторонние каталоги могут использовать публичные конечные точки чтения, чтобы выводить список или выполнять поиск skills ClawHub. Кэшируйте результаты, учитывайте `429`/`Retry-After`, направляйте пользователей обратно к канонической странице ClawHub (`https://clawhub.ai/<owner>/skills/<slug>`) и не создавайте впечатление, что ClawHub одобряет сторонний сайт. Не пытайтесь зеркалировать скрытый, приватный или заблокированный модерацией контент вне публичной поверхности API.

Веб-сокращения slug разрешаются между семействами реестра, но API-клиенты должны использовать
...
- Если token отсутствует или недействителен, поведение возвращается к применению по IP.
- Аутентифицированные конечные точки записи не должны возвращать голое `Unauthorized`, когда
  ...
  удаленные/заблокированные/отключенные учетные записи должны получать полезный текст, чтобы CLI
  клиенты могли сообщить пользователям, что их заблокировало.

- Чтение: 3000/мин на IP, 12000/мин на key
- Запись: 300/мин на IP, 3000/мин на key
- Загрузка: 1200/мин на IP, 6000/мин на key (конечные точки загрузки)
...
- Если доверенный IP клиента недоступен, анонимные запросы используют fallback-корзины,
  ограниченные только типом ограничения частоты. Эти fallback-корзины не включают
...
- Результаты возвращаются в порядке релевантности (сходство embedding + boosts за точные токены slug/имени + небольшой приоритет популярности).
- Релевантность сильнее популярности. Точное совпадение токена slug или отображаемого имени может обойти более свободное совпадение с намного более высокой вовлеченностью.
- ASCII-текст токенизируется по границам слов и знаков пунктуации. Например, `personal-map` содержит отдельный токен `map`, а `amap-jsapi-skill` содержит `amap`, `jsapi` и `skill`; поэтому поиск `map` дает `personal-map` более сильное лексическое совпадение, чем `amap-jsapi-skill`.
...
- Размещайте термины, которые пользователи буквально будут искать, в отображаемом имени, summary и tags. Используйте отдельный токен slug только тогда, когда он также является стабильной идентичностью, которую вы хотите сохранить.
- Не переименовывайте slug только ради одного запроса, если новый slug не является лучшим долгосрочным каноническим именем. Старые slugs становятся redirect aliases, но канонический URL, отображаемый slug и будущие поисковые digests используют новый slug.
- Rename aliases сохраняют разрешение для старых URL и установок, которые разрешаются через реестр, но поисковое ранжирование основывается на канонических метаданных skill после индексации переименования. Существующая статистика остается со skill.
...
- `sort` (необязательный): `updated` (по умолчанию), `recommended` (псевдоним: `default`), `createdAt` (псевдоним: `newest`), `downloads`, `stars` (псевдоним: `rating`), устаревшие install-псевдонимы `installsCurrent`/`installs`/`installsAllTime` сопоставляются с `downloads`, `trending`
- `nonSuspiciousOnly` (необязательный): `true`, чтобы скрыть подозрительные (`flagged.suspicious`) skills
- `nonSuspicious` (необязательный): устаревший псевдоним для `nonSuspiciousOnly`
...
- `metadata.systems`: целевые Nix-системы (например, `["aarch64-darwin", "x86_64-linux"]`). `null`, если не объявлено.
...
- Evidence редактируется для публичных вызывающих сторон и включает необработанные snippets только для владельцев/модераторов.

### `POST /api/v1/skills/{slug}/report`

Сообщить о skill для проверки модератором. Отчеты относятся к уровню skill, необязательно связываются
...
возврате `status` обратно в `open`. Передайте `finalAction: "hide"` с triaged
отчетом, чтобы скрыть skill в том же auditable workflow.
...
- `tag` (необязательный): разрешить помеченную версию (например, `latest`).
...
- `moderation` — текущий снимок модерации на уровне Skill, полученный из последней версии.
...
- Полезные нагрузки запросов на сканирование и загружаемые отчеты удаляются из хранилища запросов на сканирование после истечения окна хранения.
...
- Опубликованные сканирования записывают результаты обратно только когда `update: true` и сканирование успешно завершено.
...
- Задания сканирования асинхронные. Ручные запросы на сканирование имеют приоритет перед обычной работой публикации/обратного заполнения, но завершение все равно зависит от доступности воркеров.
...
- Возвращает `queue.queuedAhead` и `queue.position` во время нахождения в очереди, чтобы клиенты могли показать, сколько приоритетных ручных сканирований находится перед этим запросом. Очень большие очереди ограничиваются и сообщаются с `queuedAheadIsEstimate: true`.
...
- Требует доступа владельца/издателя к управлению Skill или plugin либо полномочий модератора/администратора платформы.
...
- Возвращает такую же структуру ZIP, как загрузки запросов на сканирование.

### `POST /api/v1/skills/-/scan/batch`

Канонический маршрут пакетного повторного сканирования только для администраторов. Он принимает ту же структуру полезной нагрузки, что и устаревший `POST /api/v1/skills/-/rescan-batch`.
...
- Идентичность Skill, идентичность издателя и метаданные выбранной версии являются полями верхнего уровня конверта (`slug`, `displayName`, `publisherHandle`, `version`, `resolvedFrom`, `tag`, `createdAt`), чтобы shell-автоматизация могла читать их без распаковки вложенных оберток.
...
версии Skills ClawHub им нужно отобразить, например OpenClaw Control UI.
...
- Результаты возвращаются по каждому элементу; один отсутствующий Skill или версия не приводит к ошибке всего ответа.
...
- Используйте `/verify`, когда нужен конверт проверки Skill Card для одного Skill, `/card`, когда нужен сгенерированный Markdown карточки, и `/scan`, когда нужны подробные данные сканеров.
...
- Записи Skills остаются основанными на реестре Skills и по-прежнему могут публиковаться только через `POST /api/v1/skills`.
...
`trending` — это семидневный рейтинг установок/загрузок, который не использует суммарные показатели за все время.
...
- Устаревшие псевдонимы фильтров v1, описанные в `GET /api/v1/plugins`, также
  принимаются.
- Фильтрация по категориям — это настоящий API-фильтр, основанный на строках
  дайджеста категорий плагинов, а не переписывание поискового запроса.
...
- `version.artifact.kind` имеет значение `legacy-zip` для архивов пакетов старого типа или
...
- `trust.moderationState` допускает null. Значение равно `null`, когда ручная модерация релиза
  отсутствует.
- `trust.blockedFromDownload` — сигнал блокировки установки. OpenClaw и другим
  ...
  повторного вывода правил блокировки из полей сканеров или модерации.
- `trust.reasons` — список объяснений для пользователей и аудита. Коды причин
...
- `trust.stale` означает, что сводка доверия была вычислена из устаревших входных данных и
  должна считаться требующей обновления перед разрешающим решением с высокой уверенностью.
...
- Эта конечная точка намеренно уже, чем конечные точки модерации для владельцев/модераторов.
  ...
  личности заявителей, тексты отчетов, приватные доказательства или внутренние сроки
  проверки.
...
- Устаревшие версии пакетов возвращают артефакт `legacy-zip` и устаревший ZIP
  `downloadUrl`.
- Версии ClawPack возвращают артефакт `npm-pack`, поля npm integrity,
...
Сообщает о пакете для проверки модератором. Отчеты создаются на уровне пакета и могут
быть связаны с версией. Они попадают в очередь модерации, но сами по себе не скрывают и
не блокируют скачивания автоматически; модераторам следует использовать модерацию релизов, чтобы
...
Эндпоинт модератора/администратора для закрытия или повторного открытия отчетов о пакетах.
...
Релизы в карантине и отозванные релизы возвращают `403` из маршрутов загрузки артефактов.
...
- Использует корзину лимита скорости чтения, а не корзину загрузок.
...
- Частные пакеты возвращают `404`, если вызывающий не может читать владельца-издателя.
...
- Метаданные только реестра не внедряются в загружаемый архив.
...
- В списке присутствуют только версии с загруженными tarball-файлами ClawPack npm-pack.
...
Передает потоково точные байты загруженного tarball ClawPack для клиентов npm-зеркала.

Примечания:

- Использует корзину лимита скорости загрузок.
- Заголовки загрузки включают SHA-256 ClawHub плюс метаданные integrity/shasum npm.
- Проверки модерации и доступа к частным пакетам по-прежнему применяются.
...
Загружает размещенную ZIP-версию skill или возвращает передачу на исходный код GitHub для
...
- Передачи skill на базе GitHub не проксируют и не зеркалируют байты. JSON-ответ
  ...
  и `archiveUrl`; состояние сканирования/актуальности является пропускным условием и не включается как метаданные
  полезной нагрузки успешного ответа.
- Статистика загрузок считается как уникальные идентификаторы за день UTC (`userId`, когда API-токен действителен, иначе IP).
...
- Необязательное поле payload: `migrateOwner`. Когда `true` с `ownerHandle`, существующий
  skill может перейти к этому владельцу, если действующее лицо является администратором/владельцем как у
...
- Разрешенные поля формы: `payload`, повторяющиеся blob-объекты `files` или одна ссылка на tarball `clawpack`.
  `clawpack` может быть blob-объектом `.tgz` или идентификатором хранилища, возвращенным
  потоком URL загрузки. Публикации с подготовленным идентификатором хранилища также должны включать
...
- Прямые multipart-запросы публикации ограничены 18 МБ. Tarball-файлы ClawPack могут
  использовать поток URL загрузки до лимита tarball 120 МБ.
- Необязательное поле payload: `ownerHandle`. Когда оно присутствует, публиковать от имени этого владельца могут только администраторы.
...
- Публикации от имени другого владельца все равно проверяют право на official-канал по учетной записи целевого владельца.
...
Только для администраторов. Гарантирует существование издателя организации для handle. Если handle все еще указывает на
...
релиза. Имена пакетов становятся частными пакетами-заполнителями без строк релизов, поэтому тот же
...
Только для администраторов. Восстанавливает личного издателя для проверенного заменяющего принципала GitHub OAuth
...
провайдера GitHub; изменяемые handle используются только как защита, видимая оператору.
...
учетными записями GitHub. Восстановление безопасно завершается отказом, если у текущего персонального
...
псевдонимов слагов Skills, пакетов, предупреждений инспектора пакетов и производных строк поискового дайджеста, чтобы
...
Источники Skills GitHub имеют область действия издателя и помечаются как проверенные, а не перезаписываются.
...
- Обе конечные точки требуют аутентификацию API-токеном и работают только для владельца Skill.
- `rename` сохраняет предыдущий слаг как псевдоним для перенаправления.
...
При самостоятельном хостинге отдавайте этот файл (или явно задайте `CLAWHUB_REGISTRY`; устаревший `CLAWDHUB_REGISTRY`).
