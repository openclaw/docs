...
x-i18n:
    generated_at: "2026-07-04T15:26:50Z"
    model: gpt-5.5
...
- Аутентифицированные конечные точки записи не должны возвращать простой `Unauthorized`, когда
  ...
  удаленные, заблокированные или отключенные учетные записи должны получать полезный текст, чтобы CLI
...
- ClawHub использует доверенные forwarding-заголовки для идентификации клиентских IP на edge.
...
- Результаты возвращаются в порядке релевантности (сходство embedding + усиление точных токенов slug/имени + небольшой приоритет популярности).
- Релевантность важнее популярности. Точное совпадение токена slug или отображаемого имени может быть выше менее точного совпадения с гораздо более сильной вовлеченностью.
...
- Популярность масштабируется логарифмически и ограничивается сверху. Skills с высокой вовлеченностью могут ранжироваться ниже, если текст запроса совпадает слабее.
...
- Помещайте термины, которые пользователи буквально будут искать, в отображаемое имя, summary и теги. Используйте отдельный slug-токен только когда он также является стабильной идентичностью, которую вы хотите сохранить.
...
- Если skill неожиданно невидим, сначала проверьте состояние модерации с помощью `clawhub inspect @owner/slug` в состоянии входа в систему, прежде чем менять метаданные, связанные с ранжированием.
...
- Старые slugs, созданные потоками переименования или объединения владельца, разрешаются в канонический skill.
...
- Владельцы и модераторы могут получать доступ к подробностям модерации для скрытых skills.
- Публичные вызывающие стороны получают `200` только для уже помеченных видимых skills.
- Доказательства редактируются для публичных вызывающих сторон и включают необработанные фрагменты только для владельцев и модераторов.

### `POST /api/v1/skills/{slug}/report`

Сообщить о skill для проверки модератором. Отчеты относятся к уровню skill, опционально связаны
...
отчетом, чтобы скрыть skill в рамках того же аудируемого workflow.
...
- Полезные нагрузки запросов сканирования и загружаемые отчеты удаляются из хранилища запросов сканирования после окончания окна хранения.
...
- Задания сканирования асинхронны. Ручные запросы сканирования получают приоритет перед обычной работой публикации/дозаполнения, но завершение все равно зависит от доступности воркеров.

### `GET /api/v1/skills/-/scan/{scanId}`

Аутентифицированный эндпоинт опроса отправленного сканирования.

- Возвращает статус queued/running/succeeded/failed.
- Возвращает `queue.queuedAhead` и `queue.position`, пока задание находится в очереди, чтобы клиенты могли показать, сколько приоритетных ручных сканирований находится перед запросом. Очень большие очереди ограничиваются и возвращаются с `queuedAheadIsEstimate: true`.
- Когда доступно, `report` содержит разделы `clawscan`, `skillspector`, `staticAnalysis` и `virustotal`.
- Неуспешные задания сканирования возвращают `status: "failed"` с `lastError`.
...
- `ok` равно `true` только когда выбранная версия имеет сгенерированную Skill Card, не заблокирована модерацией как вредоносная, а проверка ClawScan чистая.
- Идентичность навыка, идентичность публикатора и метаданные выбранной версии являются полями верхнего уровня конверта (`slug`, `displayName`, `publisherHandle`, `version`, `resolvedFrom`, `tag`, `createdAt`), чтобы shell-автоматизация могла читать их без распаковки вложенных оболочек.
...
- `provenance` имеет значение `server-resolved-github-import` только когда ClawHub разрешил и сохранил repo/ref/commit/path GitHub во время публикации или импорта; в противном случае значение — `unavailable`.
...
- `security.signals.dependencyRegistry` сохраняется для совместимости ответа v1, но сканер существования реестра зависимостей выведен из эксплуатации, и этот ключ всегда равен `null`.
- Отсутствие Skill Card не влияет на `ok`, `decision` или `reasons` этого эндпоинта; клиенты должны читать установленный `skill-card.md` локально, когда им нужно содержимое карточки.
...
- `family` (необязательный): `code-plugin` или `bundle-plugin`. Если опущено, означает оба
  семейства plugin.
...
Поиск только по plugins среди пакетов code-plugin и bundle-plugin.
...
- Устаревшие псевдонимы фильтров v1, задокументированные в `GET /api/v1/plugins`, также
  принимаются.
- Фильтрация по категории является настоящим фильтром API, основанным на строках сводки категорий plugin,
  а не переписыванием поискового запроса.
- Результаты возвращаются в порядке релевантности и сейчас не поддерживают пагинацию.
- Элементы управления сортировкой в браузерном UI для поиска plugins переупорядочивают загруженные результаты по релевантности,
...
- Приватные пакеты возвращают `404`, если вызывающий не может читать издателя-владельца.
...
- `version.sha256hash` — устаревшие метаданные совместимости для старых клиентов. Они
  ...
  Современные клиенты должны использовать `version.artifact.sha256`, который идентифицирует
...
- `trust.moderationState` может быть null. Он равен `null`, когда ручная модерация выпуска
  отсутствует.
...
- Эта конечная точка намеренно уже, чем конечные точки модерации для владельцев/модераторов.
  ...
  личности отправителей отчетов, содержимое отчетов, приватные доказательства или внутренние
...
- Версии ClawPack возвращают артефакт `npm-pack`, поля целостности npm,
  `tarballUrl` и устаревший ZIP URL совместимости.
...
- Использует корзину ограничения частоты скачиваний.
...
Конечная точка модератора для перечисления строк миграции официальных OpenClaw plugins.
...
- `open`: подозрительные, вредоносные, ожидающие, помещенные в карантин, отозванные или отмеченные отчетами выпуски.
...
Отправляет отчет о пакете на проверку модератором. Отчеты создаются на уровне пакета, с необязательной
связью с версией. Они попадают в очередь модерации, но сами по себе не скрывают автоматически и не
блокируют скачивания; модераторы должны использовать модерацию выпусков, чтобы
...
Конечная точка модератора/администратора для закрытия или повторного открытия отчетов о пакетах.
...
- Использует квоту скорости чтения, а не квоту загрузки.
- Двоичные файлы возвращают `415`.
- Ограничение размера файла: 200 КБ.
- Ожидающие сканирования VirusTotal не блокируют чтение; вредоносные релизы все еще могут быть скрыты в других местах.
...
Передает потоково точные байты загруженного tarball-файла ClawPack для клиентов зеркала npm.

Примечания:

- Использует квоту скорости загрузки.
...
Загружает ZIP размещенной версии Skills или возвращает передачу управления на источник GitHub для
...
- Необязательное поле payload: `migrateOwner`. При `true` вместе с `ownerHandle`
  существующий Skills может быть перемещен к этому владельцу, если действующее лицо является администратором/владельцем у обоих
...
- Разрешенные поля формы: `payload`, повторяющиеся blob-объекты `files` или одна ссылка на tarball
  ...
  потоком upload-url. Публикации со staged storage-id также должны включать
...
- Публикации от имени другого лица все равно проверяют право на official-channel относительно учетной записи целевого владельца.
...
Если `reason` присутствует, он сохраняется как примечание модерации Skills и копируется в журнал аудита.
...
релиза. Имена пакетов становятся приватными пакетами-заполнителями без строк релизов, чтобы тот же
владелец мог позже опубликовать настоящий релиз code-plugin или bundle-plugin с этим именем.
...
без редактирования строк аккаунтов Convex Auth. Запрос должен указывать оба неизменяемых идентификатора аккаунтов
провайдера GitHub; изменяемые handle используются только как операторский защитный контроль.

Конечная точка по умолчанию выполняет пробный запуск. Для применения восстановления требуется `dryRun: false` и
`confirmIdentityVerified: true` после того, как сотрудники независимо проверят непрерывность между обоими
GitHub-принципалами. Восстановление завершается безопасным отказом, если у текущего персонального
публикатора целевого пользователя есть навыки, пакеты или источники навыков GitHub.
Восстановление также переносит устаревшие поля `ownerUserId` для навыков восстановленного публикатора,
псевдонимов слагов навыков, пакетов, предупреждений инспектора пакетов и производных строк поискового дайджеста, чтобы
пути прямого владельца согласовывались с новым полномочием публикатора. Активное резервирование защищенного handle
...
100 строками на транзакцию применения; более крупные восстановления должны сначала использовать возобновляемую миграцию владельца.
Источники навыков GitHub имеют область действия публикатора и сообщаются как проверенные, а не перезаписываются.
...
- Обе конечные точки требуют аутентификацию с API-токеном и работают только для владельца навыка.
...
контента (только администратор). По умолчанию выполняется пробный запуск, если `dryRun` не равен `false`.
...
## Устаревшие конечные точки CLI (deprecated)
...
которые подготавливают tarball ClawPack, должны отправлять получившийся идентификатор хранилища как
...
Если вы размещаете самостоятельно, отдавайте этот файл (или задайте `CLAWHUB_REGISTRY` явно; устаревшее `CLAWDHUB_REGISTRY`).
