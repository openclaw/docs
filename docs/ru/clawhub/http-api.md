...
x-i18n:
    generated_at: "2026-07-01T20:30:12Z"
    model: gpt-5.5
...
Сторонние каталоги могут использовать публичные read-эндпоинты для перечисления или поиска Skills ClawHub. Кэшируйте результаты, соблюдайте `429`/`Retry-After`, направляйте пользователей обратно к канонической странице ClawHub (`https://clawhub.ai/<owner>/skills/<slug>`) и не создавайте впечатление, что ClawHub одобряет сторонний сайт. Не пытайтесь зеркалировать скрытый, приватный или заблокированный модерацией контент вне поверхности публичного API.

Web-сокращения slug разрешаются между семействами реестра, но API-клиентам следует использовать
канонические URL, возвращаемые read-эндпоинтами, вместо самостоятельного восстановления приоритета
маршрутов.
...
- Аутентифицированные write-эндпоинты не должны возвращать простой `Unauthorized`, когда
...
- Если `Retry-After` отсутствует, используйте fallback на `RateLimit-Reset` (или вычислите из `X-RateLimit-Reset`).

Источник IP:

- Использует доверенные заголовки клиентского IP, включая `cf-connecting-ip`, только когда
  развертывание явно включает доверенные пересылаемые заголовки.
- ClawHub использует доверенные пересылаемые заголовки для идентификации клиентских IP на edge.
- Если доверенный клиентский IP недоступен, анонимные запросы используют fallback-бакеты,
  ограниченные только видом ограничения частоты. Эти fallback-бакеты не включают
...
Это включает ошибки валидации (`400`), отсутствующие публичные ресурсы (`404`), ошибки auth и
разрешений (`401`/`403`), ограничения частоты (`429`) и заблокированные загрузки. Клиенты
должны читать тело ответа как человекочитаемую строку. Неизвестные query-параметры
игнорируются для совместимости, но распознанные query-параметры с недействительными значениями возвращают
`400`.
...
- `highlightedOnly` (необязательный): `true` для фильтрации только до выделенных skills
...
- Результаты возвращаются в порядке релевантности (embedding-сходство + boost точного токена slug/имени + небольшой prior популярности).
- Релевантность сильнее популярности. Точное совпадение slug или токена отображаемого имени может ранжироваться выше, чем более свободное совпадение с гораздо более сильной вовлеченностью.
- ASCII-текст токенизируется по границам слов и пунктуации. Например, `personal-map` содержит самостоятельный токен `map`, а `amap-jsapi-skill` содержит `amap`, `jsapi` и `skill`; поэтому поиск `map` дает `personal-map` более сильное лексическое совпадение, чем `amap-jsapi-skill`.
- Популярность логарифмически масштабируется и ограничивается сверху. Skills с высокой вовлеченностью могут ранжироваться ниже, если текст запроса совпадает слабее.
...
- Помещайте термины, которые пользователи буквально будут искать, в отображаемое имя, summary и теги. Используйте самостоятельный токен slug только когда он также является стабильной идентичностью, которую вы хотите сохранить.
- Не переименовывайте slug только ради одного запроса, если новый slug не является лучшим долгосрочным каноническим именем. Старые slug становятся redirect-alias, но канонический URL, отображаемый slug и будущие поисковые digest используют новый slug.
- Rename-alias сохраняют разрешение для старых URL и установок, которые разрешаются через реестр, но поисковое ранжирование основано на канонических метаданных skill после индексации переименования. Существующая статистика остается со skill.
...
- `sort` (необязательный): `updated` (по умолчанию), `recommended` (alias: `default`), `createdAt` (alias: `newest`), `downloads`, `stars` (alias: `rating`), устаревшие install-alias `installsCurrent`/`installs`/`installsAllTime` отображаются в `downloads`, `trending`
- `nonSuspiciousOnly` (необязательный): `true` для скрытия подозрительных (`flagged.suspicious`) skills
- `nonSuspicious` (необязательный): устаревший alias для `nonSuspiciousOnly`
...
- `createdAt` стабилен для обхода новых skills; `updated` меняется при повторной публикации существующих skills.
...
Сообщить о skill на проверку модератором. Отчеты относятся к уровню skill, необязательно связаны
...
Эндпоинт модератора/администратора для приема отчетов по skills.
...
Эндпоинт модератора/администратора для разрешения или повторного открытия отчетов по skills.
...
установке `status` обратно в `open`. Передайте `finalAction: "hide"` с обработанным
...
Возвращает детали проверки security-сканирования для версии skill.
...
- Полезные нагрузки запросов сканирования и скачиваемые отчеты удаляются из хранилища запросов сканирования после окна хранения.
- Для опубликованных сканирований требуется доступ владельца/издателя к управлению либо полномочия модератора/администратора платформы.
- Опубликованные сканирования выполняют обратную запись только когда `update: true` и сканирование успешно завершилось.
- Ответ имеет код `202` с `{ "ok": true, "scanId": "...", "jobId": "...", "status": "queued", "sourceKind": "published", "update": false, "queue": { "queuedAhead": 0, "queuedAheadIsEstimate": false, "position": 1, "running": 0, "runningIsEstimate": false, "note": "Scans are asynchronous and may take time to complete." } }`.
- Задания сканирования асинхронны. Ручные запросы сканирования имеют приоритет перед обычной публикацией/обратным заполнением, но завершение все равно зависит от доступности воркеров.
...
- Требует успешного сканирования; незавершенные сканирования возвращают `409`.
...
- `tag` (необязательно): разрешить помеченную версию (например, `latest`).

Примечания:

- `ok` равно `true` только когда для выбранной версии сгенерирован Skill Card, она не заблокирована модерацией как вредоносная, а проверка ClawScan чистая.
- Идентификатор навыка, идентификатор издателя и метаданные выбранной версии являются полями верхнего уровня конверта (`slug`, `displayName`, `publisherHandle`, `version`, `resolvedFrom`, `tag`, `createdAt`), чтобы автоматизация оболочки могла читать их без распаковки вложенных оберток.
...
- Используйте `/verify`, когда нужен конверт проверки Skill Card для одного навыка, `/card`, когда нужен сгенерированный Markdown карточки, и `/scan`, когда нужны подробные данные сканера.
...
- `category` (необязательный): фильтр категории plugin. Поддерживается только когда
  запрос ограничен plugin-пакетами (`/api/v1/plugins`,
...
- Аутентифицированные вызывающие стороны могут видеть приватные пакеты издателей, к которым они принадлежат, в результатах списка и поиска.
- `channel=private` возвращает только пакеты, которые аутентифицированная вызывающая сторона может читать.

### `GET /api/v1/packages/search`

Единый поиск по каталогу среди Skills и plugin-пакетов.
...
- `isOfficial` (необязательный): `true` или `false`
- `category` (необязательный): фильтр категории plugin. Поддерживается только когда
  запрос ограничен plugin-пакетами. Управляемые категории и устаревшие псевдонимы
...
`trending` — это рейтинг установок/загрузок за семь дней, он не использует совокупные итоги за все время.
В единой конечной точке `/api/v1/packages` он применяется только к plugins; используйте
...
- Текущие Skills на основе GitHub со статусом сканирования `clean` или `suspicious` включают
...
- Устаревшие псевдонимы фильтров v1, задокументированные в `GET /api/v1/plugins`, также
  принимаются.
- Фильтрация по категориям является настоящим API-фильтром на основе строк дайджеста категорий Plugin,
...
- Приватные пакеты возвращают `404`, если вызывающий клиент не может читать владельца-публикатора.
...
- `release.artifactKind`, `release.artifactSha256`, `release.npmIntegrity`,
  ...
  артефакта выпуска.
- `trust.scanStatus` — эффективный статус доверия, выведенный из входных данных сканера
  и ручной модерации выпуска.
- `trust.moderationState` допускает значение null. Оно равно `null`, когда ручная модерация выпуска
  отсутствует.
...
- `trust.stale` означает, что сводка доверия была вычислена из устаревших входных данных и
  должна рассматриваться как требующая обновления перед высоконадежным разрешающим решением.
...
- Эта конечная точка намеренно уже, чем конечные точки модерации для владельцев/модераторов.
  ...
  личности репортеров, тела отчетов, приватные доказательства или внутренние временные шкалы
  проверки.
...
блокируют загрузки; модераторам следует использовать модерацию выпусков, чтобы
...
Конечная точка moderator/admin для приема отчетов о пакетах.

Аутентификация:

- Требует API-токен пользователя moderator или admin.
...
Конечная точка owner/moderator для видимости модерации пакета.

Аутентификация:

- Требует API-токен владельца пакета, участника издателя, пользователя moderator или
  admin.
...
Конечная точка moderator/admin для разрешения или повторного открытия отчетов о пакетах.
...
том же проверяемом рабочем процессе.
...
Конечная точка moderator/admin для проверки релиза пакета.
...
Помещенные в карантин и отозванные релизы возвращают `403` из маршрутов загрузки артефактов.
...
- Использует лимит чтения, а не лимит загрузок.
- Двоичные файлы возвращают `415`.
- Ограничение размера файла: 200 КБ.
- Ожидающие проверки VirusTotal не блокируют чтение; вредоносные релизы все еще могут быть скрыты в другом месте.
...
- Метаданные только реестра не внедряются в загруженный архив.
- Ожидающие проверки VirusTotal не блокируют загрузки; вредоносные релизы возвращают `403`.
- Приватные пакеты возвращают `404`, если вызывающий не является владельцем.

### `GET /api/npm/{package}`

Возвращает npm-совместимый packument для версий пакетов на базе ClawPack.

Примечания:

- Указываются только версии с загруженными tarball npm-pack ClawPack.
- Устаревшие версии только ZIP намеренно опущены.
...
- Packument пакетов со scope поддерживают и `/api/npm/@scope/name`, и закодированный
  путь запроса npm `/api/npm/@scope%2Fname`.

### `GET /api/npm/{package}/-/{tarball}.tgz`

Передает точные байты загруженного tarball ClawPack для клиентов зеркала npm.

Примечания:

- Использует лимит загрузок.
- Заголовки загрузки включают SHA-256 ClawHub плюс метаданные npm integrity/shasum.
...
Загружает ZIP размещенной версии Skill или возвращает передачу исходного кода GitHub для
текущего Skill на базе GitHub со сканом `clean` или `suspicious` и без размещенной
версии.
...
- Передачи Skill на базе GitHub не проксируют и не зеркалируют байты. JSON-ответ
  ...
  payload успешного ответа.
- Статистика загрузок считается как уникальные идентичности за день UTC (`userId`, когда API-токен действителен, иначе IP).
...
- Необязательное поле payload: `migrateOwner`. Если `true` вместе с `ownerHandle`, существующий
  Skill может перейти к этому владельцу, если субъект является admin/owner у обоих
...
- Необязательное поле payload: `ownerHandle`. Если оно присутствует, только admins могут публиковать от имени этого владельца.
...
- Публикации от имени другого владельца по-прежнему проверяют право на official-channel по целевой учетной записи владельца.

### `DELETE /api/v1/skills/{slug}` / `POST /api/v1/skills/{slug}/undelete`

Мягко удаляет / восстанавливает Skill (owner, moderator или admin).
...
Если `reason` присутствует, он сохраняется как примечание модерации Skill и копируется в журнал аудита.
Мягкие удаления, инициированные владельцем, резервируют slug на 30 дней, затем slug может быть занят
другим издателем. Ответ удаления включает `slugReservedUntil`, когда применяется этот срок.
...
Только admin. Гарантирует существование издателя org для handle. Если handle все еще указывает на
устаревшего общего пользователя/личного издателя, конечная точка сначала мигрирует его в издателя org.
Для вновь созданной org укажите `memberHandle`; действующий admin не добавляется как участник.
...
Аутентифицированное самостоятельное создание издателя org. Создает нового издателя org и добавляет
вызывающего как owner. Эта конечная точка не мигрирует существующие пользовательские/личные handle и
...
Только admin. Резервирует корневые slugs и имена пакетов для законного владельца без публикации
релиза. Имена пакетов становятся приватными placeholder-пакетами без строк релизов, поэтому тот же
...
без редактирования строк учетных записей Convex Auth. Запрос должен указывать оба неизменяемых id учетных записей
провайдера GitHub; изменяемые handle используются только как защита для оператора.
...
издателя пользователя назначения есть Skills, пакеты или источники Skills GitHub.
...
Источники Skills GitHub имеют область действия издателя и помечаются как проверенные, а не переписываются.
...
контента (только admin). По умолчанию работает в режиме пробного запуска, если `dryRun` не равно `false`.
...
Вывести список пользователей или выполнить поиск по ним (только admin).
