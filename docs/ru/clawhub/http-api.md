...
summary: Справочник HTTP API (публичные + CLI-эндпоинты + аутентификация).
x-i18n:
    generated_at: "2026-07-04T10:52:06Z"
    model: gpt-5.5
...
Сторонние каталоги могут использовать публичные эндпоинты чтения, чтобы выводить список или искать Skills ClawHub. Кэшируйте результаты, соблюдайте `429`/`Retry-After`, направляйте пользователей обратно к канонической странице ClawHub (`https://clawhub.ai/<owner>/skills/<slug>`) и не создавайте впечатление, что ClawHub одобряет сторонний сайт. Не пытайтесь зеркалировать скрытый, приватный или заблокированный модерацией контент за пределами публичной поверхности API.

Веб-сокращения слагов разрешаются по семействам реестра, но клиенты API должны использовать
...
- Аутентифицированные запросы (действительный токен Bearer): применяются по пользовательскому бакету.
- Если токен отсутствует или недействителен, поведение возвращается к применению по IP.
- Аутентифицированные эндпоинты записи не должны возвращать простой `Unauthorized`, когда
  сервер знает причину. Отсутствующие токены, недействительные/отозванные токены и
  удаленные/забаненные/отключенные учетные записи должны получать пригодный к действию текст, чтобы CLI
...
- `X-RateLimit-Reset`: абсолютное время Unix в секундах
...
- Используйте откат с джиттером, чтобы избежать синхронизированных повторных попыток.
...
- ClawHub использует доверенные заголовки пересылки для идентификации IP клиентов на краю сети.
- Если доверенный IP клиента недоступен, анонимные запросы используют запасные бакеты,
  ограниченные только типом ограничения скорости. Эти запасные бакеты не включают
  пути, слаги, имена пакетов, версии, строки запроса или другие
  параметры артефактов, предоставленные вызывающей стороной.
...
- `highlightedOnly` (необязательно): `true` для фильтрации до подсвеченных Skills
...
- Результаты возвращаются в порядке релевантности (сходство эмбеддингов + усиления точных токенов слага/имени + небольшой предварительный коэффициент популярности).
- Релевантность сильнее популярности. Точное совпадение токена слага или отображаемого имени может ранжироваться выше, чем более свободное совпадение с намного более сильной вовлеченностью.
...
- Помещайте термины, которые пользователи будут буквально искать, в отображаемое имя, сводку и теги. Используйте самостоятельный токен слага только тогда, когда это также стабильная идентичность, которую вы хотите сохранить.
- Не переименовывайте слаг только ради одного запроса, если новый слаг не является лучшим долгосрочным каноническим именем. Старые слаги становятся псевдонимами перенаправления, но канонический URL, отображаемый слаг и будущие поисковые сводки используют новый слаг.
...
- Старые слаги, созданные потоками переименования/слияния владельцев, разрешаются в канонический Skill.
...
Сообщить о Skill для проверки модератором. Жалобы относятся к уровню Skill, опционально связываются
...
- Полезные нагрузки запросов сканирования и отчеты, доступные для скачивания, удаляются из хранилища запросов сканирования после истечения окна хранения.
...
- Опубликованные сканирования записывают результат обратно только когда `update: true` и сканирование успешно завершено.
...
- `kind` по умолчанию имеет значение `skill`; используйте `kind=plugin` для сканирований Plugin/пакета.
...
- `ok` равно `true` только когда для выбранной версии создана карточка навыка, версия не заблокирована модерацией как вредоносная и проверка ClawScan чистая.
...
- `provenance` равно `server-resolved-github-import` только когда ClawHub разрешил и сохранил GitHub repo/ref/commit/path во время публикации или импорта; в остальных случаях значение — `unavailable`.
...
- Ответ содержит только данные безопасности. Он не включает данные карточки навыка, статус созданной карточки, списки файлов артефактов или подробные полезные нагрузки сканеров.
...
На единой конечной точке `/api/v1/packages` он доступен только для плагинов; используйте
...
Массовый экспорт последних публичных выпусков плагинов для офлайн-анализа.
...
- Устаревшие псевдонимы фильтров v1, документированные в `GET /api/v1/plugins`, также
  принимаются.
- Фильтрация по категории — это настоящий API-фильтр, основанный на строках дайджеста
  категорий плагинов, а не переписывание поискового запроса.
- Результаты возвращаются в порядке релевантности и сейчас не поддерживают пагинацию.
- Элементы управления сортировкой в браузерном интерфейсе для поиска плагинов переупорядочивают загруженные релевантные результаты,
...
- `trust.moderationState` допускает значение null. Он равен `null`, когда ручная модерация
  выпуска отсутствует.
...
- `trust.stale` означает, что сводка доверия была вычислена по устаревшим входным данным и
  должна рассматриваться как требующая обновления перед решением о разрешении с высокой степенью уверенности.
...
- Эта конечная точка намеренно уже, чем конечные точки модерации для владельцев/модераторов.
  ...
  репортеров, тексты отчетов, приватные доказательства или внутренние временные линии
  проверки.
...
Загружает артефакт версии через явный путь разрешителя.
...
Сообщить о пакете для проверки модератором. Отчеты относятся к уровню пакета и могут быть
опционально связаны с версией. Они попадают в очередь модерации, но сами по себе не скрывают
и не блокируют загрузки автоматически; модераторы должны использовать модерацию выпусков, чтобы
...
Конечная точка владельца/модератора для просмотра состояния модерации пакета.
...
- `quarantined`: заблокирован до последующего рассмотрения.
...
- Ожидающие сканирования VirusTotal не блокируют чтение; вредоносные релизы все равно могут быть скрыты в другом месте.
...
- Отображаются только версии с загруженными tarball-файлами ClawPack npm-pack.
...
Передает потоком точные байты загруженного tarball-файла ClawPack для клиентов npm-зеркала.
...
- `slug` (обязательно)
- `hash` (обязательно): 64-символьный hex sha256 fingerprint пакета
...
Скачивает ZIP размещенной версии навыка или возвращает передачу к исходному коду GitHub для
...
- Передачи навыков на базе GitHub не проксируют и не зеркалируют байты. JSON-ответ
  ...
  и `archiveUrl`; состояние сканирования/актуальности является условием доступа и не включается как метаданные
  успешной полезной нагрузки.
- Статистика скачиваний считается по уникальным идентичностям за день UTC (`userId`, если API-токен действителен, иначе IP).
...
- Необязательное поле payload: `migrateOwner`. Если `true` вместе с `ownerHandle`, существующий
  навык может быть перенесен к этому владельцу, если действующий пользователь является администратором/владельцем как у
...
- Публикации от имени другого пользователя все равно проверяют право на канал official для целевой учетной записи владельца.
...
Если `reason` присутствует, он сохраняется как примечание модерации навыка и копируется в журнал аудита.
...
вызывающего пользователя как владельца. Эта конечная точка не мигрирует существующие пользовательские/персональные handle и
...
Только для администратора. Восстанавливает персонального издателя для проверенного заменяющего принципала GitHub OAuth
без редактирования строк учетных записей Convex Auth. Запрос должен указывать оба неизменяемых GitHub
provider account id; изменяемые handle используются только как операторская защита.

По умолчанию конечная точка работает в режиме пробного запуска. Для применения восстановления требуется `dryRun: false` и
`confirmIdentityVerified: true` после того, как сотрудники независимо подтвердят непрерывность между обеими
учетными записями GitHub. Восстановление завершается безопасным отказом, если у текущего личного
...
пути прямого владельца согласовывались с новым полномочием публикатора. Активная резервация защищенного handle
...
Источники навыков GitHub привязаны к публикатору и сообщаются как проверенные, а не перезаписываются.
...
- Обе конечные точки требуют аутентификации по API-токену и работают только для владельца навыка.
...
`clawpack`, а возвращенный тикет как `clawpackUploadTicket`.
...
При самостоятельном размещении отдавайте этот файл (или явно задайте `CLAWHUB_REGISTRY`; устаревшее `CLAWDHUB_REGISTRY`).
