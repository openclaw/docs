...
x-i18n:
    generated_at: "2026-07-05T07:36:35Z"
    model: gpt-5.5
...
Сторонние каталоги могут использовать публичные эндпоинты чтения, чтобы выводить список или выполнять поиск Skills ClawHub. Кэшируйте результаты, учитывайте `429`/`Retry-After`, направляйте пользователей обратно к канонической странице ClawHub (`https://clawhub.ai/<owner>/skills/<slug>`) и не создавайте впечатление, что ClawHub одобряет сторонний сайт. Не пытайтесь зеркалировать скрытый, приватный или заблокированный модерацией контент за пределами публичной поверхности API.

Сокращенные веб-slug разрешаются между семействами реестра, но клиенты API должны использовать
канонические URL, возвращаемые эндпоинтами чтения, вместо реконструкции приоритета
маршрутов.
...
- Аутентифицированные эндпоинты записи не должны возвращать голый `Unauthorized`, когда
...
- Если доверенный IP клиента недоступен, анонимные запросы используют резервные бакеты,
  ограниченные только видом ограничения частоты. Эти резервные бакеты не включают
  предоставленные вызывающей стороной пути, slug, имена пакетов, версии, строки запроса или другие
  параметры артефактов.

## Ответы с ошибками

Публичные ответы об ошибках v1 передаются простым текстом с `content-type: text/plain; charset=utf-8`.
...
- Результаты возвращаются в порядке релевантности (сходство embedding + точные повышения для токенов slug/имени + небольшой априорный вес популярности).
- Релевантность важнее популярности. Точное совпадение slug или токена отображаемого имени может опередить более свободное совпадение с гораздо более сильным вовлечением.
...
- Размещайте термины, которые пользователи будут буквально искать, в отображаемом имени, сводке и тегах. Используйте отдельный токен slug только тогда, когда он также является стабильной идентичностью, которую вы хотите сохранить.
...
- `recommended` использует сигналы вовлеченности и недавности.
...
возврате `status` в `open`. Передайте `finalAction: "hide"` с обработанной
...
- Возвращает `queue.queuedAhead` и `queue.position`, пока задание находится в очереди, чтобы клиенты могли показать, сколько приоритетных ручных сканирований находится перед запросом. Очень большие очереди ограничиваются и сообщаются с `queuedAheadIsEstimate: true`.
...
- Возвращает сохраненные результаты сканирования для точно отправленной версии, включая заблокированные или скрытые версии.
...
- `provenance` равно `server-resolved-github-import` только когда ClawHub разрешил и сохранил репозиторий GitHub/ref/commit/path во время публикации или импорта; в противном случае оно равно `unavailable`.
...
- Результаты возвращаются по каждому элементу; отсутствие одного skill или версии не приводит к сбою всего ответа.
- Ответ содержит только сведения безопасности. Он не включает данные Skill Card, статус сгенерированной карточки, списки файлов артефактов или подробные полезные нагрузки сканеров.
...
- Используйте `/verify`, когда нужен конверт проверки Skill Card для одного skill, `/card`, когда нужен сгенерированный markdown карточки, и `/scan`, когда нужны подробные данные сканера.
...
- Записи Skills по-прежнему опираются на реестр Skills и могут публиковаться только через `POST /api/v1/skills`.
...
- `mcp-tooling`, `data` и `automation` сопоставляются с `tools`.
- `observability` и `deployment` сопоставляются с `gateway`.
- `dev-tools` сопоставляется с `runtime`.

`trending` — это семидневная таблица лидеров по установкам/загрузкам, которая не использует итоговые значения за все время.
На единой конечной точке `/api/v1/packages` она применяется только к plugins; используйте
...
- Тело: ZIP-архив.
- Каждый экспортированный Skill располагается в корне `{publisher}/{slug}/`.
...
- `category` (необязательный): фильтр по категории Plugin. Текущие значения:
  `channels`, `models`, `memory`, `context`, `voice`, `media`, `web`,
  `tools`, `runtime`, `gateway`, `security`, `other`.
...
- Эта конечная точка намеренно уже, чем конечные точки модерации владельца/модератора.
  ...
  личности репортеров, тела отчетов, приватные доказательства или внутренние сроки
  проверки.
...
- Версии ClawPack потоково передают точные загруженные байты npm-pack `.tgz`.
...
- дайджест артефакта
- репозиторий исходного кода и происхождение коммита
...
- Требуется API-токен модератора или администратора.
...
- `open`: подозрительные, вредоносные, ожидающие, помещенные в карантин, отозванные или отмеченные отчетами релизы.
...
Отправляет отчет о пакете на проверку модератором. Отчеты относятся к уровню пакета и могут
необязательно связываться с версией. Они попадают в очередь модерации, но сами по себе не скрывают автоматически и не
...
- Использует квоту скорости чтения, а не квоту скачивания.
...
- Метаданные только из registry не внедряются в скачанный архив.
...
- Использует квоту скорости скачивания.
...
- `slug` (обязательно)
- `hash` (обязательно): 64-символьный шестнадцатеричный sha256 fingerprint бандла
...
Скачивает ZIP размещенной версии навыка или возвращает передачу GitHub-источника для
...
- Передачи навыков на базе GitHub не проксируют и не зеркалируют байты. JSON-ответ
  ...
  и `archiveUrl`; состояние сканирования/актуальности является gate и не включается как метаданные
...
- Необязательное поле payload: `ownerHandle`. Если оно присутствует, API разрешает этого
  издателя на стороне сервера и требует, чтобы актор имел доступ к издателю.
- Необязательное поле payload: `migrateOwner`. Когда `true` вместе с `ownerHandle`,
  существующий навык может перейти к этому владельцу, если актор является администратором/владельцем у обоих
  издателей: текущего и целевого. Без этой явной настройки изменения владельца
  отклоняются.
...
- Разрешенные поля формы: `payload`, повторяющиеся blob-объекты `files` или одна ссылка на tarball
  ...
  workflow upload-url. Публикации staged storage-id также должны включать
  `clawpackUploadTicket`, возвращенный с этим upload URL.
...
- Прямые multipart-запросы публикации ограничены 18 МБ. Tarball-файлы ClawPack могут
  использовать workflow upload-url до лимита tarball 120 МБ.
...
- Пакетам Plugin требуется `openclaw.plugin.json`. Загрузки ClawPack `.tgz` должны
  содержать его в `package/openclaw.plugin.json`.
- Code-плагинам требуются `package.json`, метаданные исходного репозитория, метаданные исходного commit,
...
- Публикации от имени другого владельца все равно проверяют право на официальный канал по целевому аккаунту владельца.
...
Если присутствует, `reason` сохраняется как заметка модерации навыка и копируется в журнал аудита.
...
Аутентифицированное самообслуживаемое создание org-издателя. Создает нового org-издателя и добавляет
вызывающего как владельца. Эта конечная точка не мигрирует существующие user/personal handles и не
...
владелец позднее может опубликовать реальный релиз code-plugin или bundle-plugin под этим именем.
...
Только для администраторов. Восстанавливает личного издателя для проверенного заменяющего GitHub OAuth principal
без редактирования строк аккаунтов Convex Auth. Запрос должен называть оба неизменяемых GitHub
provider account id; изменяемые handles используются только как guard, видимый оператору.
...
Восстановление также мигрирует устаревшие поля `ownerUserId` для skills восстановленного издателя,
...
Источники GitHub skills имеют область действия издателя и сообщаются как проверенные, а не переписываются.
...
Показать список пользователей или выполнить поиск пользователей (только администратор).
...
При самостоятельном размещении отдавайте этот файл (или задайте `CLAWHUB_REGISTRY` явно; устаревшее `CLAWDHUB_REGISTRY`).
