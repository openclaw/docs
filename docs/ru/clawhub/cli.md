...
(например, Docker-контейнеры, Hetzner VPS с интернетом только через прокси,
...
- Устаревший откат: если `clawhub/config.json` еще не существует, но существует `clawdhub/config.json`, CLI повторно использует устаревший путь
...
- Удаленный/без графического интерфейса интерактивный режим: `clawhub login --device` печатает код и ожидает, пока вы авторизуете его на `<site>/cli/device`.
...
- Полезно для передачи локального токена входа в команды настройки секретов CI через конвейер.
...
- Поиск отдает предпочтение точным совпадениям токенов slug/имени перед популярностью скачиваний. Отдельный токен slug, такой как `map`, совпадает с `personal-map` сильнее, чем подстрока внутри `amap`.
- Популярность — небольшой предварительный фактор ранжирования, а не гарантия верхней позиции.
- Если навык должен появиться, но не появляется, выполните `clawhub inspect @owner/slug` после входа, чтобы проверить видимую владельцу модерационную диагностику перед переименованием метаданных.

### `explore`

- Перечисляет новейшие Skills через `/api/v1/skills?limit=...&sort=createdAt` (отсортировано по `createdAt` по убыванию).
- Флаги:
  - `--limit <n>` (1-200, по умолчанию: 25)
  - `--sort newest|updated|rating|downloads|trending` (по умолчанию: newest). Устаревшие алиасы сортировки установки все еще работают для совместимости.
...
- `--limit <n>`: максимальное количество версий для вывода (1-200).
- `--files`: перечислить файлы выбранной версии.
- `--file <path>`: получить сырой контент файла (только текстовые файлы; лимит 200 КБ).
- `--json`: машиночитаемый вывод.
...
- Отправляет best-effort телеметрию, пока выполнен вход, чтобы текущие счетчики установок можно было
  деактивировать.
...
- Удаляет закрепление lockfile у установленного навыка, чтобы будущие обновления могли его изменять.
...
- `update <skill>` быстро завершается ошибкой для закрепленных навыков и сообщает сначала выполнить `clawhub unpin <skill>`.
- `update --all` пропускает закрепленные slugs и печатает сводку о том, что осталось замороженным.

### `skill publish <path>`

- Сравнивает fingerprint локального бандла с ClawHub и успешно завершает работу, когда
  контент уже опубликован.
...
- `--version <version>` явно выбирает версию и публикует даже тогда, когда
  контент совпадает с существующей версией.
...
- `--owner <handle>` публикует под handle издателя организации/пользователя, когда
  у действующего пользователя есть доступ издателя.
- `--migrate-owner` перемещает существующий навык к `--owner` при публикации новой
  версии. Требует доступа admin/owner у обоих издателей.
- Поведение владельца и проверки объяснено в `docs/publishing.md`.
...
- ClawHub не поддерживает платные Skills или ценообразование для отдельных навыков.
...
автоматическое поведение patch-версии.

Задайте `dry_run: true`, чтобы предварительно просмотреть без токена. Для реальных публикаций требуется
...
- Сканирования асинхронны и могут занять время. Пока задача в очереди, терминальный spinner показывает текущую приоритетную позицию сканирования и сколько сканирований впереди.
- Опубликованные сканирования требуют права владения или доступа к управлению издателем. Модераторы/admin могут использовать тот же backend через `clawhub-admin`.
...
- Скачивания навыков используют slug навыка и по умолчанию используют `--kind skill`.
...
для репозиториев навыков и каталогов.
...
- Мягкие удаления по инициативе владельца резервируют slug на 30 дней; команда выводит время истечения срока.
- `--version <version>` безвозвратно удаляет одну принадлежащую владельцу не последнюю версию через отказоустойчивый,
  специализированный для версии маршрут.
...
- `--reason <text>` записывает примечание модерации при мягком удалении всего навыка и в журнал аудита.
...
- Восстановления версий нет; безвозвратно удаленные версии нельзя восстановить.
- Вызывает `POST /api/v1/skills/{slug}/undelete`.
- `--reason <text>` записывает примечание модерации для навыка и в журнал аудита.
- `--note <text>` является псевдонимом для `--reason`.
- `--yes` пропускает подтверждение.
...
- Передачи на идентификаторы пользователей создают ожидающий запрос, который получатель принимает.
- Передачи на идентификаторы организаций/издателей применяются немедленно только тогда, когда у действующего пользователя есть
...
- Используйте это для плагинов и других записей семейств пакетов; `search` верхнего уровня остается поверхностью поиска навыков.
- Флаги:
...
- `--tag <tag>`: проверить версию с тегом (например, `latest`).
- `--versions`: вывести историю версий (первая страница).
- `--limit <n>`: максимальное количество версий для вывода (1-100).
...
- Для артефактов ClawPack npm-pack также проверяет целостность npm `sha512`,
  npm shasum и имя/версию `package.json` tarball-архива.
...
- По умолчанию выполняет автономную/статическую проверку, без поиска или импорта локального
  checkout OpenClaw.
- Критические ошибки совместимости завершаются с ненулевым кодом. Предупреждения без ошибок
  выводятся, но завершаются с нулевым кодом.
- Флаги:
...
- `--version <version>` безвозвратно удаляет один принадлежащий владельцу не последний выпуск через отказоустойчивый,
  специализированный для версии маршрут.
  Удаленные версии нельзя восстановить или опубликовать повторно. Опубликуйте замену перед удалением
...
- Требует административный доступ и к текущему владельцу пакета, и к целевому
...
- Сами по себе жалобы не скрывают пакеты автоматически и не блокируют загрузки.
- Флаги:
...
- Показывает текущее состояние сканирования пакета, количество открытых жалоб, состояние ручной
  модерации последнего выпуска, состояние блокировки загрузок и причины модерации.
- Флаги:
  - `--json`: машиночитаемый вывод.
...
- Вызывает ту же вычисляемую конечную точку готовности, что и `package readiness`, но выводит
  статус с фокусом на миграции, последнюю версию, состояние официального пакета, проверки и
  блокеры.
- Флаги:
  - `--json`: машиночитаемый вывод.
...
- Идентификатор нормализуется к нижнему регистру и может передаваться с `@` или без него.
- Новые созданные издатели организаций по умолчанию не являются доверенными/официальными.
...
- Публикует Plugin с кодом или пакетный Plugin через `POST /api/v1/packages`.
- `<source>` принимает:
...
- Папки Plugin с кодом перед загрузкой упаковываются в npm tarball ClawPack,
  чтобы установки OpenClaw могли проверить точный артефакт. Папки пакетных Plugin по-прежнему
...
- Для локальных папок атрибуция источника автоматически определяется из локального git, когда remote origin указывает на GitHub.
- Внешние Plugin с кодом должны явно объявлять `openclaw.compat.pluginApi` и
  `openclaw.build.openclawVersion`.
  Верхнеуровневое `package.json.version` не используется как запасной вариант для валидации публикации.
...
- `--owner <handle>` публикует от имени пользователя или организации-издателя, если у действующего лица есть доступ издателя.
...
Для Plugin с кодом публикация папки собирает и загружает артефакт ClawPack из
папки пакета:
...
Внешним Plugin с кодом требуется небольшой объем метаданных OpenClaw в
...
- Для монорепозиториев передайте `source_path`, чтобы workflow публиковал папку
  пакета Plugin, например `source_path: extensions/codex`.
- Закрепляйте переиспользуемый workflow на стабильном теге или полном SHA коммита. Не запускайте публикацию релиза из `@main`.
- `pull_request` должен использовать `dry_run: true`, чтобы CI не оставлял загрязняющих следов.
- Реальные публикации должны быть ограничены доверенными событиями, такими как `workflow_dispatch` или push тегов.
- Доверенная публикация без секрета работает только для `workflow_dispatch`; push тегов все еще требует `clawhub_token`.
...
- Прикрепляет или заменяет конфигурацию доверенного издателя GitHub Actions для существующего
  пакета.
- Сначала пакет должен быть создан через обычную ручную или token-authenticated
...
- `--environment <name>` необязателен. Если он настроен, окружение GitHub Actions
  в утверждении OIDC должно совпадать точно.
- ClawHub проверяет настроенный репозиторий GitHub при выполнении этой команды.
  ...
  репозитории требуют, чтобы у ClawHub был доступ GitHub к этому репозиторию, например
...
- Отправляется после `clawhub install <slug>` при входе в систему, если только
