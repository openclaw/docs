...
read_when:
    ...
    - Публикация Skills или Plugin в реестр
    - Выбор между потоками CLI openclaw и clawhub
sidebarTitle: ClawHub
summary: Общий обзор ClawHub для обнаружения, установки, публикации, безопасности и clawhub CLI.
title: ClawHub
x-i18n:
    generated_at: "2026-07-04T10:55:37Z"
    model: gpt-5.5
...
текущее состояние реестра, чтобы пользователи могли проверить Skill или плагин
перед установкой.
...
во время переходов при запуске, а `npm:<package>` остается только npm-источником,
когда источник должен быть явным.
...
CLI также имеет команды установки/обновления Skills для прямых рабочих процессов
с реестром:
...
- `--slug <slug>`: имя Skill в опубликованном URL.
...
ClawHub по умолчанию открыт: загружать может кто угодно, но для публикации нужна
учетная запись GitHub, достаточно старая, чтобы пройти шлюз загрузки. Публичные
страницы сведений показывают сводку последнего состояния проверки перед
установкой или скачиванием.

ClawHub выполняет автоматические проверки опубликованных Skills и релизов
плагинов. Релизы, удержанные проверкой или заблокированные, могут исчезать из
публичного каталога и поверхностей установки, оставаясь видимыми владельцу в
`/dashboard`.

Вошедшие пользователи могут жаловаться на Skills и пакеты. Модераторы могут
рассматривать жалобы, скрывать или восстанавливать содержимое и блокировать
злоупотребляющие аккаунты. См.
...
политиках и правоприменении.

## Телеметрия и среда

Когда вы запускаете `clawhub install` после входа в систему, CLI может отправить
событие установки по принципу best-effort, чтобы ClawHub мог рассчитывать
