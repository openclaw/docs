...
Установите CLI ClawHub, когда вам нужны рабочие процессы с аутентификацией в реестре, например
...
| Поверхность        | Что она хранит                                               | Типичная команда                              |
| -------------- | ------------------------------------------------------------ | -------------------------------------------- |
...
ClawHub отслеживает semver-версии, теги вроде `latest`, списки изменений, файлы,
загрузки, звезды и сводки проверок безопасности. Публичные страницы показывают текущее состояние
реестра, чтобы пользователи могли изучить Skill или плагин перед установкой.
...
Простые npm-безопасные спецификации плагинов могут разрешаться через npm во время переходов при запуске, а
...
В CLI также есть команды установки/обновления Skills для прямых рабочих процессов реестра:
...
- `--changelog <text>`: текст списка изменений.
...
или заблокированные выпуски могут исчезать из публичного каталога и поверхностей установки, оставаясь
...
скрывать или восстанавливать содержимое и блокировать нарушающие правила учетные записи. См.
...
Когда вы запускаете `clawhub install`, войдя в систему, CLI может отправить по мере возможности
