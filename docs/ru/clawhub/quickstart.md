...
Команда пропускает неизмененное содержимое. Новые Skills начинаются с `1.0.0`; последующие изменения
...
переменные среды, инструменты и разрешения, чтобы пользователи могли понять, что
требуется skill, до его установки. См. [Формат skill](/ru/clawhub/skill-format).
...
Code plugins должны включать метаданные совместимости с OpenClaw в `package.json`,
