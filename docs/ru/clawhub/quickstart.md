...
Используйте OpenClaw, когда устанавливаете что-либо в OpenClaw. Используйте CLI `clawhub`, когда выполняете вход, публикуете, управляете собственными карточками или используете рабочие процессы, специфичные для реестра.
...
Перед публикацией проверьте метаданные в `SKILL.md`. Объявите необходимые переменные окружения, инструменты и разрешения, чтобы пользователи могли понять, что требуется навыку, до его установки. См. [Формат навыка](/ru/clawhub/skill-format).

Для репозиториев с несколькими навыками переиспользуемый рабочий процесс GitHub вызывает `skill publish` для каждой непосредственной папки навыка в `skills/`:
...
Сначала используйте `--dry-run`, чтобы предварительно просмотреть разрешенные метаданные пакета, поля совместимости, указание источника и план загрузки без публикации.
...
Перед установкой используйте веб-страницу ClawHub или команды подробной информации в CLI, чтобы проверить метаданные, ссылки на источники, версии, списки изменений и статус проверки:
...
Публичные карточки показывают последнее состояние проверки. Релизы, удерживаемые или заблокированные модерацией, могут быть скрыты из поиска и поверхностей установки до разрешения ситуации.
