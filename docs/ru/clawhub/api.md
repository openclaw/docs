...
x-i18n:
    generated_at: "2026-07-03T09:50:13Z"
    model: gpt-5.5
...
Вы можете построить сторонний каталог, справочник или поисковую поверхность поверх публичных API чтения ClawHub. Публичные метаданные навыков и файлы навыков публикуются по правилам лицензирования навыков ClawHub, а сам API ограничен по частоте запросов и должен использоваться ответственно.
...
- При отображении списков давайте ссылку на канонический URL навыка в ClawHub, чтобы пользователи могли проверить исходную запись реестра.
...
- Не зеркалируйте скрытое, приватное или заблокированное модерацией содержимое путем обхода фильтров публичного API или границ аутентификации.
...
- Аутентифицированные запросы (действительный Bearer-токен): по пользовательскому лимиту.
- Отсутствующий или недействительный токен откатывается к применению лимита по IP.
...
- `X-RateLimit-Remaining` / `RateLimit-Remaining`: точный оставшийся бюджет, когда
  присутствует; шардированные успешные запросы опускают его, вместо того чтобы возвращать приблизительное
  глобальное значение
...
- Ошибки v1 представляют собой обычный текст (`text/plain; charset=utf-8`), включая `400`,
  `401`, `403`, `404`, `429` и ответы для заблокированных загрузок.
...
- `POST /api/v1/users/reserve` резервирует корневые slug и приватные заглушки пакетов без выпусков для owner handle.
