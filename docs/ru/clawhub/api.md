...
summary: Обзор публичного REST API (v1) и соглашения.
x-i18n:
    generated_at: "2026-07-04T06:42:05Z"
    model: gpt-5.5
...
Вы можете построить сторонний каталог, справочник или поисковую поверхность поверх публичных API чтения ClawHub. Публичные метаданные Skills и файлы Skills публикуются согласно правилам лицензии Skills ClawHub, а сам API имеет ограничения частоты запросов и должен использоваться ответственно.
...
- Авторизованные запросы (действительный Bearer-токен): по пользовательской квоте.
- Отсутствующий/недействительный токен возвращает применение ограничений по IP.
