...
x-i18n:
    generated_at: "2026-07-05T05:25:08Z"
    model: gpt-5.5
...
Вы можете создать сторонний каталог, справочник или поисковую поверхность поверх публичных API чтения ClawHub. Публичные метаданные навыков и файлы навыков публикуются по правилам лицензии навыков ClawHub, а сам API ограничен по частоте запросов и должен использоваться ответственно.
...
- Не зеркалируйте скрытый, приватный или заблокированный модерацией контент, обходя публичные фильтры API или границы аутентификации.
...
Принудительное применение с учетом аутентификации:

- Анонимные запросы: по IP.
- Аутентифицированные запросы (действительный токен Bearer): по корзине пользователя.
- Отсутствующий/недействительный токен переключается на ограничение по IP.
...
- `POST /api/v1/users/reserve` резервирует корневые slugs и приватные плейсхолдеры пакетов без релиза для идентификатора владельца.
