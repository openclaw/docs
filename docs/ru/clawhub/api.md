...
x-i18n:
    generated_at: "2026-07-01T13:14:47Z"
    model: gpt-5.5
...
Вы можете создать сторонний каталог, справочник или поисковую поверхность поверх публичных API чтения ClawHub. Публичные метаданные навыков и файлы навыков публикуются по правилам лицензирования навыков ClawHub, а сам API имеет ограничения частоты запросов и должен использоваться ответственно.
...
- Не зеркалируйте скрытый, приватный или заблокированный модерацией контент, обходя фильтры публичного API или границы авторизации.
...
- Аутентифицированные запросы (действительный Bearer-токен): по корзине пользователя.
- Отсутствующий или недействительный токен приводит к применению ограничения по IP.
...
- `X-RateLimit-Remaining` / `RateLimit-Remaining`: точный оставшийся бюджет, когда
  присутствует; сегментированные успешные запросы опускают его, а не возвращают приблизительное
  глобальное значение
...
- Ошибки v1 представляют собой простой текст (`text/plain; charset=utf-8`), включая `400`,
  `401`, `403`, `404`, `429` и ответы о заблокированной загрузке.
...
- `POST /api/v1/users/reserve` резервирует корневые slug и приватные заполнители пакетов без релизов для дескриптора владельца.
