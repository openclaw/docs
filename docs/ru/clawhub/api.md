...
Вы можете построить сторонний каталог, справочник или поисковую поверхность поверх публичных API чтения ClawHub. Публичные метаданные навыков и файлы навыков публикуются по правилам лицензирования навыков ClawHub, а сам API имеет ограничения частоты запросов и должен использоваться ответственно.
...
- При отображении списков ссылайтесь на канонический URL навыка в ClawHub, чтобы пользователи могли проверить исходную запись реестра.
...
- `POST /api/v1/users/reserve` резервирует корневые слаги и приватные заполнители пакетов без релиза для дескриптора владельца.
