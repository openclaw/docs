...
read_when:
    ...
    - Добавление эндпоинтов или схем
summary: Обзор и соглашения общедоступного REST API (v1).
x-i18n:
    generated_at: "2026-07-04T03:58:33Z"
    model: gpt-5.5
...
Вы можете построить сторонний каталог, директорию или поисковую поверхность поверх публичных API чтения ClawHub. Публичные метаданные Skills и файлы Skills публикуются по лицензионным правилам ClawHub для Skills, а сам API имеет ограничения частоты запросов и должен использоваться ответственно.
...
- При отображении списков давайте ссылку обратно на канонический URL Skills в ClawHub, чтобы пользователи могли проверить исходную запись реестра.
...
- Не зеркалируйте скрытый, приватный или заблокированный модерацией контент в обход фильтров публичного API или границ аутентификации.
...
- Добавляйте jitter к повторным попыткам.

## Ошибки

- Ошибки v1 представлены обычным текстом (`text/plain; charset=utf-8`), включая `400`,
...
- `POST /api/v1/users/reserve` резервирует корневые slug и приватные заполнители пакетов без релиза для handle владельца.
