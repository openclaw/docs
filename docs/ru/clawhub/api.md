...
x-i18n:
    generated_at: "2026-07-03T01:01:38Z"
    model: gpt-5.5
...
Вы можете создать сторонний каталог, справочник или поисковую поверхность на основе публичных API чтения ClawHub. Публичные метаданные Skills и файлы Skills публикуются согласно правилам лицензии Skills ClawHub, а сам API имеет ограничения частоты запросов и должен использоваться ответственно.
...
- Авторизованные запросы (действительный Bearer-токен): по пользовательскому лимиту.
- Отсутствующий/недействительный токен использует применение по IP.
...
- `POST /api/v1/users/reserve` резервирует корневые slugs и приватные заполнители пакетов без выпусков для дескриптора владельца.
