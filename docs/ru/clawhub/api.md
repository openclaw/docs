...
x-i18n:
    generated_at: "2026-07-03T02:52:01Z"
    model: gpt-5.5
...
Вы можете построить сторонний каталог, справочник или поисковую поверхность поверх публичных API чтения ClawHub. Публичные метаданные Skills и файлы Skills публикуются по правилам лицензии Skills ClawHub, а сам API ограничен по частоте запросов и должен использоваться ответственно.
...
- Кэшируйте ответы и учитывайте `429`, `Retry-After` и заголовки ограничения частоты вместо агрессивного опроса.
- При отображении списков ссылайтесь на канонический URL Skills в ClawHub, чтобы пользователи могли проверить исходную запись реестра.
...
- Не зеркалируйте скрытый, приватный или заблокированный модерацией контент, обходя фильтры публичного API или границы аутентификации.
...
- Аутентифицированные запросы (действительный Bearer-токен): по пользовательской корзине.
- Отсутствующий/недействительный токен возвращается к применению ограничений по IP.
...
- `X-RateLimit-Remaining` / `RateLimit-Remaining`: точный оставшийся бюджет, если
  присутствует; сегментированные успешные запросы опускают его вместо возврата приблизительного
...
- `GET /api/v1/download?slug=&version=&tag=`
  ...
  - Текущие Skills на основе GitHub со сканированием `clean` или `suspicious` возвращают
...
- `POST /api/v1/users/reserve` резервирует корневые slugs и приватные заполнители пакетов без выпусков для идентификатора владельца.
