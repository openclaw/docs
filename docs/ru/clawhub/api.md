...
x-i18n:
    generated_at: "2026-07-04T18:11:43Z"
    model: gpt-5.5
...
Вы можете построить сторонний каталог, директорию или поисковую поверхность поверх публичных API чтения ClawHub. Публичные метаданные Skills и файлы Skills публикуются по правилам лицензирования Skills ClawHub, а сам API ограничен по частоте запросов и должен использоваться ответственно.
...
- Кэшируйте ответы и соблюдайте `429`, `Retry-After` и заголовки ограничения частоты вместо агрессивного опроса.
- Давайте ссылку на канонический URL Skills в ClawHub при отображении списков, чтобы пользователи могли проверить исходную запись реестра.
...
- Предпочитайте `Retry-After`, когда присутствует.
...
- `POST /api/v1/users/reserve` резервирует корневые slug и приватные заполнители пакетов без релиза для дескриптора владельца.
