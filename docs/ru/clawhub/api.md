...
x-i18n:
    generated_at: "2026-07-02T22:42:08Z"
    model: gpt-5.5
...
Вы можете создать сторонний каталог, справочник или поисковую поверхность поверх публичных API чтения ClawHub. Публичные метаданные Skills и файлы Skills публикуются по правилам лицензирования Skills ClawHub, а сам API имеет ограничения частоты запросов и должен использоваться ответственно.
...
- При отображении списков ссылайтесь на канонический URL Skills ClawHub, чтобы пользователи могли проверить исходную запись реестра.
...
- `GET /api/v1/skills?limit=&cursor=&sort=`
  ...
  - `cursor` применяется к сортировкам, не являющимся `trending`
  - Необязательный фильтр: `nonSuspiciousOnly=true`
  - Устаревший псевдоним: `nonSuspicious=true`
  - С `nonSuspiciousOnly=true` страницы на основе курсора могут содержать меньше чем `limit` элементов; используйте `nextCursor`, чтобы продолжить.
...
- `POST /api/v1/users/reserve` резервирует корневые slug и приватные заполнители пакетов без релиза для идентификатора владельца.
