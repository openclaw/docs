...
Вы можете построить сторонний каталог, директорию или поисковую поверхность поверх публичных API чтения ClawHub. Публичные метаданные навыков и файлы навыков публикуются согласно лицензионным правилам ClawHub для навыков, а сам API имеет ограничения частоты запросов и должен использоваться ответственно.
...
- При отображении списков ссылайтесь на канонический URL навыка ClawHub, чтобы пользователи могли изучить исходную запись реестра.
...
- Аутентифицированные запросы (действительный Bearer-токен): по пользовательской квоте.
...
- `X-RateLimit-Remaining` / `RateLimit-Remaining`: точный оставшийся бюджет, когда
  ...
  глобальное значение
- `Retry-After`: задержка в секундах перед ожиданием при `429`
...
- Ошибки v1 являются простым текстом (`text/plain; charset=utf-8`), включая `400`,
...
- `POST /api/v1/users/reserve` резервирует корневые slugs и приватные заполнители пакетов без релиза для owner handle.
