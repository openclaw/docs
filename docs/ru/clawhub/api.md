...
x-i18n:
    generated_at: "2026-07-05T07:45:59Z"
    model: gpt-5.5
...
Вы можете построить сторонний каталог, директорию или поисковую поверхность поверх публичных API чтения ClawHub. Публичные метаданные Skills и файлы Skills публикуются по правилам лицензирования Skills ClawHub, а сам API имеет ограничения частоты запросов и должен использоваться ответственно.
...
- При отображении списков добавляйте ссылку на канонический URL Skill в ClawHub, чтобы пользователи могли проверить исходную запись реестра.
- Используйте канонические URL страниц в форме `https://clawhub.ai/<owner>/skills/<slug>`.
- Не подразумевайте, что ClawHub одобряет, проверяет или обслуживает сторонний сайт.
...
- Отсутствующий/недействительный токен использует ограничение по IP.
...
- `X-RateLimit-Remaining` / `RateLimit-Remaining`: точный оставшийся бюджет, когда
  присутствует; шардированные успешные запросы опускают его, а не возвращают приблизительное
  глобальное значение
...
- Ошибки v1 возвращаются обычным текстом (`text/plain; charset=utf-8`), включая `400`,
  `401`, `403`, `404`, `429` и ответы для заблокированных скачиваний.
