...
x-i18n:
    generated_at: "2026-07-04T20:38:11Z"
    model: gpt-5.5
...
Вы можете создать сторонний каталог, справочник или поверхность поиска поверх публичных API чтения ClawHub. Публичные метаданные Skills и файлы Skills публикуются согласно правилам лицензирования Skills в ClawHub, а сам API имеет ограничения частоты запросов и должен использоваться ответственно.
...
- Не подразумевайте, что ClawHub одобряет, проверяет или эксплуатирует сторонний сайт.
- Не зеркалируйте скрытый, приватный или заблокированный модерацией контент в обход фильтров публичного API или границ авторизации.
...
- Отсутствующий или недействительный токен приводит к применению ограничений по IP.
...
- `X-RateLimit-Remaining` / `RateLimit-Remaining`: точный оставшийся бюджет, когда
  присутствует; шардированные успешные запросы не включают его, вместо того чтобы возвращать приблизительное
  глобальное значение
...
- `POST /api/v1/users/reserve` резервирует корневые слаги и приватные заполнители пакетов без релиза для идентификатора владельца.
