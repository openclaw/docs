...
x-i18n:
    generated_at: "2026-07-02T01:05:07Z"
    model: gpt-5.5
...
Вы можете создать сторонний каталог, директорию или поисковую поверхность поверх публичных API чтения ClawHub. Публичные метаданные Skills и файлы Skills публикуются по лицензионным правилам Skills ClawHub, при этом сам API имеет ограничения частоты запросов и должен использоваться ответственно.
...
- `POST /api/v1/users/reserve` резервирует корневые slugs и приватные заполнители пакетов без релизов для дескриптора владельца.
