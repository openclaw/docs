...
x-i18n:
    generated_at: "2026-07-01T08:20:59Z"
    model: gpt-5.5
...
Вы можете создать сторонний каталог, директорию или поисковую поверхность поверх публичных API чтения ClawHub. Публичные метаданные Skills и файлы Skills публикуются по лицензионным правилам ClawHub для Skills, а сам API ограничен по частоте запросов и должен использоваться ответственно.
...
- При отображении списков давайте ссылку на канонический URL Skills в ClawHub, чтобы пользователи могли проверить исходную запись реестра.
...
- Не зеркалируйте скрытое, приватное или заблокированное модерацией содержимое, обходя фильтры публичного API или границы аутентификации.
...
- Аутентифицированные запросы (действительный токен Bearer): по пользовательскому бакету.
- Отсутствующий или недействительный токен откатывается к ограничению по IP-адресу.
...
- `POST /api/v1/users/reserve` резервирует корневые slugs и приватные заполнители пакетов без выпусков для handle владельца.
