...
Вы можете создать сторонний каталог, директорию или поисковую поверхность поверх публичных API ClawHub для чтения. Публичные метаданные Skills и файлы Skills публикуются по правилам лицензирования Skills ClawHub, а сам API имеет ограничения частоты запросов и должен использоваться ответственно.
...
- Кэшируйте ответы и соблюдайте `429`, `Retry-After` и заголовки ограничения частоты запросов вместо агрессивного опроса.
- При отображении списков давайте ссылку на канонический URL Skills ClawHub, чтобы пользователи могли изучить исходную запись реестра.
...
- Авторизованные запросы (действительный Bearer-токен): по пользовательскому бакету.
...
- Добавляйте случайный разброс к повторным попыткам.
...
- `POST /api/v1/users/reserve` резервирует корневые слаги и приватные placeholder-пакеты без релиза для дескриптора владельца.
