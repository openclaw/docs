...
x-i18n:
    generated_at: "2026-07-03T17:33:23Z"
    model: gpt-5.5
...
Вы можете создать сторонний каталог, справочник или поисковую поверхность поверх публичных API чтения ClawHub. Публичные метаданные Skills и файлы Skills публикуются по правилам лицензии Skills ClawHub, а сам API имеет ограничения частоты запросов и должен использоваться ответственно.
...
- При отображении списков добавляйте ссылку на канонический URL Skills в ClawHub, чтобы пользователи могли проверить исходную запись реестра.
...
- Отсутствующий/недействительный токен приводит к применению ограничений по IP.
...
- `X-RateLimit-Remaining` / `RateLimit-Remaining`: точный оставшийся бюджет, когда
  присутствует; сегментированные успешные запросы опускают его, вместо того чтобы возвращать приблизительное
  глобальное значение
...
- Добавляйте случайное отклонение к повторным попыткам.

## Ошибки

- Ошибки v1 представлены простым текстом (`text/plain; charset=utf-8`), включая `400`,
  `401`, `403`, `404`, `429` и ответы заблокированного скачивания.
...
- `GET /api/v1/download?slug=&version=&tag=`
  ...
  - Текущие Skills, поддерживаемые GitHub, со сканированием `clean` или `suspicious` возвращают
...
- `GET /api/v1/skills/export?startDate=&endDate=&limit=&cursor=`
  ...
  - Текущие Skills, поддерживаемые GitHub, со сканированием `clean` или `suspicious` экспортируются
...
- `POST /api/v1/users/reserve` резервирует корневые slugs и приватные заполнители пакетов без релиза для идентификатора владельца.
