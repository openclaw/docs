...
Вы можете создать сторонний каталог, справочник или поисковую поверхность поверх публичных API чтения ClawHub. Публичные метаданные навыков и файлы навыков публикуются в соответствии с правилами лицензии ClawHub для навыков, а сам API имеет ограничения частоты запросов и должен использоваться ответственно.
...
- Кэшируйте ответы и учитывайте `429`, `Retry-After` и заголовки ограничений частоты запросов вместо агрессивного опроса.
- При отображении списков ссылайтесь на канонический URL навыка ClawHub, чтобы пользователи могли проверить исходную запись реестра.
...
- Не зеркальте скрытый, приватный или заблокированный модерацией контент, обходя фильтры публичного API или границы аутентификации.
...
- Аутентифицированные запросы (действительный токен Bearer): по пользовательскому лимиту.
- Отсутствующий/недействительный токен откатывается к применению лимитов по IP.
...
- `X-RateLimit-Remaining` / `RateLimit-Remaining`: точный оставшийся бюджет, когда
  присутствует; сегментированные успешные запросы пропускают его вместо возврата приблизительного
...
- В противном случае используйте `RateLimit-Reset` или вычисляйте задержку из `X-RateLimit-Reset`.
- Добавляйте джиттер к повторным попыткам.

## Ошибки

- Ошибки v1 представляют собой обычный текст (`text/plain; charset=utf-8`), включая `400`,
  `401`, `403`, `404`, `429` и ответы заблокированных загрузок.
...
- `GET /api/v1/skills?limit=&cursor=&sort=`
  ...
  - Устаревший псевдоним: `nonSuspicious=true`
  - При `nonSuspiciousOnly=true` страницы на основе курсора могут содержать меньше элементов, чем `limit`; используйте `nextCursor`, чтобы продолжить.
...
- `GET /api/v1/download?slug=&version=&tag=`
  - Размещенные навыки возвращают детерминированные байты ZIP.
...
- `POST /api/v1/users/reserve` резервирует корневые slugs и приватные заполнители пакетов без релизов для handle владельца.
