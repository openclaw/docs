...
x-i18n:
    generated_at: "2026-07-04T15:27:43Z"
    model: gpt-5.5
...
Вы можете создать сторонний каталог, справочник или поисковую поверхность поверх публичных API чтения ClawHub. Публичные метаданные Skills и файлы Skills публикуются по правилам лицензии Skills в ClawHub, а сам API имеет ограничения частоты запросов и должен использоваться ответственно.
...
- Кэшируйте ответы и соблюдайте `429`, `Retry-After` и заголовки ограничений частоты запросов вместо агрессивного опроса.
...
- `POST /api/v1/users/reserve` резервирует корневые слаги и приватные заполнители пакетов без релизов для дескриптора владельца.
