...
x-i18n:
    generated_at: "2026-06-30T22:26:26Z"
    model: gpt-5.5
...
Используйте `requires.env` для переменных окружения, которые должны присутствовать до запуска навыка. Используйте `envVars`, когда нужны метаданные по отдельным переменным, включая необязательные переменные с `required: false`.
...
Объявите необязательные переменные окружения в `metadata.openclaw.envVars` и установите `required: false`. Не добавляйте необязательные записи в `requires.env`, потому что `requires.env` означает, что навык не может работать без них.
...
- Текст для эмбеддингов включает `SKILL.md` + до примерно 40 файлов, не являющихся `.md` (ограничение по возможности).
...
- ClawHub не поддерживает платные навыки, ценообразование для отдельных навыков, paywall или распределение доходов.
