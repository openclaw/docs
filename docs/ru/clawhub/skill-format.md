...
`SKILL.md` или устаревшие файлы `skills.md` в публичных репозиториях, не являющихся форками, принадлежащих
вошедшему в систему аккаунту GitHub. Он не импортирует приватные репозитории, форки,
...
Используйте `requires.env` для переменных окружения, которые должны быть заданы до запуска навыка. Используйте `envVars`, когда нужны метаданные для отдельных переменных, включая необязательные переменные с `required: false`.
...
- Текст для embeddings включает `SKILL.md` + до примерно 40 файлов не `.md` (лимит по мере возможности).
...
- Слаги пакетов должны быть в нижнем регистре и безопасны для npm, например `@example.tools/demo-plugin` или `demo-plugin`.
...
- ClawHub не поддерживает платные Skills, цены для отдельных навыков, paywall или распределение доходов.
