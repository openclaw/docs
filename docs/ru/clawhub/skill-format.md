...
x-i18n:
    generated_at: "2026-07-04T18:12:24Z"
    model: gpt-5.5
...
- `.clawhubignore` (шаблоны исключений для публикации, устаревший `.clawdhubignore`)
...
`SKILL.md` или устаревшие файлы `skills.md` в публичных, не являющихся форками репозиториях, принадлежащих
...
- Файлы сценариев все равно сканируются после загрузки; файлы PowerShell `.ps1`, `.psm1` и `.psd1` принимаются как текст.
- Типы содержимого, начинающиеся с `text/`, считаются текстом; плюс небольшой список разрешений (JSON/YAML/TOML/JS/TS/Markdown/SVG).

Ограничения (на стороне сервера):

- Общий размер пакета: 50 МБ.
- Текст для эмбеддингов включает `SKILL.md` + до ~40 файлов не `.md` (ограничение по возможности).
