...
Веб-импортер GitHub строже локальной публикации/синхронизации. Он обнаруживает только
...
Для публикации принимаются только «текстовые» файлы.
...
- Текст для эмбеддингов включает `SKILL.md` + до примерно 40 файлов не `.md` (ограничение по принципу best-effort).
...
- Если ваш навык интегрируется с платным сторонним сервисом, четко задокументируйте внешнюю стоимость и требуемый аккаунт в инструкциях навыка и объявлениях переменных окружения (`requires.env` для обязательных переменных или `envVars` с `required: false` для необязательных переменных).
