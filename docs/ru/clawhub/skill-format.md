...
x-i18n:
    generated_at: "2026-07-03T01:02:05Z"
    model: gpt-5.5
...
файлы `SKILL.md` или устаревшие `skills.md` в публичных, не являющихся форками репозиториях, принадлежащих
...
- Файлы сценариев все равно сканируются после загрузки; файлы PowerShell `.ps1`, `.psm1` и `.psd1` принимаются как текстовые.
...
- Текст для эмбеддингов включает `SKILL.md` + до примерно 40 файлов не `.md` (ограничение по возможности).
