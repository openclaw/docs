...
x-i18n:
    generated_at: "2026-07-03T23:39:35Z"
    model: gpt-5.5
...
Веб-импортер GitHub строже, чем локальная публикация/синхронизация. Он обнаруживает
только файлы `SKILL.md` или устаревшие `skills.md` в публичных репозиториях, не являющихся форками,
которые принадлежат вошедшему в систему аккаунту GitHub. Он не импортирует приватные репозитории,
форки, архивные/отключенные репозитории или сторонние публичные репозитории.
...
Объявляйте требования среды выполнения навыка в `metadata.openclaw` (псевдонимы: `metadata.clawdbot`, `metadata.clawdis`).
...
- Текст для embedding включает `SKILL.md` + до примерно 40 файлов, не являющихся `.md` (ограничение по мере возможности).

## Slugs

- По умолчанию формируются из имени папки.
- Области пакетов должны точно совпадать с ClawHub publisher handle. Publisher handles могут использовать строчные буквы, цифры, дефисы, точки и подчеркивания; они должны начинаться и заканчиваться строчной буквой или цифрой.
...
- ClawHub не поддерживает платные навыки, ценообразование для отдельных навыков, paywalls или распределение доходов.
- Не добавляйте метаданные ценообразования в `SKILL.md`; они не являются частью формата навыка и не сделают опубликованный навык платным.
- Если ваш навык интегрируется с платным сторонним сервисом, четко документируйте внешнюю стоимость и требуемую учетную запись в инструкциях навыка и объявлениях env (`requires.env` для обязательных переменных или `envVars` с `required: false` для необязательных переменных).
