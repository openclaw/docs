...
`SKILL.md` или устаревшие файлы `skills.md` в публичных репозиториях, не являющихся форками, которые принадлежат
...
Используйте `requires.env` для переменных окружения, которые должны присутствовать перед запуском навыка. Используйте `envVars`, когда нужны метаданные для отдельных переменных, включая необязательные переменные с `required: false`.
...
- Типы содержимого, начинающиеся с `text/`, считаются текстовыми; плюс небольшой список разрешенных форматов (JSON/YAML/TOML/JS/TS/Markdown/SVG).

Ограничения (на стороне сервера):

- Общий размер пакета: 50MB.
- Текст для эмбеддингов включает `SKILL.md` + до примерно 40 файлов, не являющихся `.md` (ограничение применяется по возможности).
