...
x-i18n:
    generated_at: "2026-07-01T18:17:28Z"
    model: gpt-5.5
...
### Метаданные выполнения (`metadata.openclaw`)

Объявляйте требования выполнения вашего навыка в `metadata.openclaw` (псевдонимы: `metadata.clawdbot`, `metadata.clawdis`).
...
| `envVars`          | `array`    | Объявления переменных окружения с `name`, необязательным `required` и необязательным `description`. Укажите `required: false` для необязательных переменных окружения. |
...
| `emoji`            | `string`   | Эмодзи для отображения навыка.                                                                                                               |
...
Объявляйте необязательные переменные окружения в `metadata.openclaw.envVars` и указывайте `required: false`. Не добавляйте необязательные записи в `requires.env`, потому что `requires.env` означает, что навык не может работать без них.
...
- Файлы скриптов по-прежнему сканируются после загрузки; файлы PowerShell `.ps1`, `.psm1` и `.psd1` принимаются как текст.
...
- Текст для embedding включает `SKILL.md` + до примерно 40 файлов, не являющихся `.md` (ограничение по принципу best-effort).

## Slugs

- По умолчанию выводятся из имени папки.
- Области пакетов должны точно совпадать с handle издателя ClawHub. Handle издателей могут использовать строчные буквы, цифры, дефисы, точки и подчеркивания; они должны начинаться и заканчиваться строчной буквой или цифрой.
- Slug пакетов должны быть в нижнем регистре и совместимы с npm, например `@example.tools/demo-plugin` или `demo-plugin`.
...
- Если ваш навык интегрируется с платным сторонним сервисом, четко задокументируйте внешнюю стоимость и требуемую учетную запись в инструкциях навыка и объявлениях переменных окружения (`requires.env` для обязательных переменных или `envVars` с `required: false` для необязательных переменных).
