...
summary: Формат папки Skill, обязательные файлы, допустимые типы файлов, ограничения.
x-i18n:
    generated_at: "2026-07-04T06:42:34Z"
    model: gpt-5.5
...
`SKILL.md` или устаревшие `skills.md` в публичных репозиториях, не являющихся форками, которые принадлежат
...
- `description` используется как краткое описание навыка в интерфейсе/поиске.
...
Объявляйте требования runtime вашего навыка в `metadata.openclaw` (псевдонимы: `metadata.clawdbot`, `metadata.clawdis`).
...
| `requires.bins`    | `string[]` | CLI-бинарные файлы, которые должны быть установлены все без исключения.                                                                              |
...
| `envVars`          | `array`    | Объявления переменных окружения с `name`, необязательным `required` и необязательным `description`. Укажите `required: false` для необязательных env vars. |
...
- Текст для эмбеддинга включает `SKILL.md` + до ~40 файлов не с расширением `.md` (ограничение best-effort).
...
- Все навыки, опубликованные на ClawHub, лицензируются на условиях `MIT-0`.
...
- ClawHub не поддерживает платные навыки, ценообразование для отдельных навыков, paywall или распределение дохода.
- Не добавляйте метаданные цены в `SKILL.md`; они не являются частью формата навыка и не сделают опубликованный навык платным.
- Если ваш навык интегрируется с платным сторонним сервисом, четко опишите внешнюю стоимость и требуемую учетную запись в инструкциях навыка и объявлениях env (`requires.env` для обязательных переменных или `envVars` с `required: false` для необязательных переменных).
