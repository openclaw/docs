...
summary: Исправить замечания валидации пакета Plugin ClawHub перед публикацией
title: Исправления валидации Plugin
x-i18n:
    generated_at: "2026-07-02T08:35:35Z"
    model: gpt-5.5
...
ClawHub проверяет пакеты Plugin перед публикацией и также может показывать результаты автоматического сканирования пакетов. Эта страница описывает результаты для авторов, то есть результаты, которые автор Plugin может исправить в метаданных пакета, манифесте, импортах SDK или опубликованном артефакте.

Она не описывает внутренние результаты покрытия Plugin Inspector. Если полный отчет содержит коды обслуживания сканера без рекомендаций по исправлению для автора, они предназначены для сопровождающих OpenClaw, а не для авторов Plugin.
...
| `sdk-session-transcript-file-target`    | [Замените устаревшие файловые цели транскриптов](/ru/clawhub/plugin-validation-fixes#sdk-session-transcript-file-target)     |
| `sdk-session-transcript-low-level`      | [Замените низкоуровневые вспомогательные функции транскриптов](/ru/clawhub/plugin-validation-fixes#sdk-session-transcript-low-level) |
...
Пакет содержит `package.json`, но не объявляет метаданные пакета OpenClaw.
...
- Установите `openclaw.install.defaultChoice`, если доступно больше одного источника установки.
...
- Используйте версию API Plugin OpenClaw или минимальную версию semver, относительно которой вы выполняли разработку и тестирование.
...
- Проверьте любые метаданные сборки OpenClaw в пакете, например версию OpenClaw, использованную при выпуске.
...
- Исправьте недействительные метаданные пакета, сломанные lifecycle-скрипты или записи файлов, из-за которых упаковка завершается с ошибкой.
...
Нативный манифест Plugin не включает отображаемое имя.

- Добавьте непустое поле `name` в `openclaw.plugin.json`.
- Оставьте `name` удобочитаемым для человека, а `id` — стабильным машинным идентификатором.
- См. [манифест Plugin](/ru/plugins/manifest).
- Повторно запустите `clawhub package validate <path-to-plugin>`.
...
- Переместите поведение времени выполнения в код регистрации Plugin, а `contracts`
...
- Если для поведения нет публичного SDK, оставьте вспомогательное средство внутри своего пакета или
...
Plugin все еще использует устаревшее вспомогательное средство всего хранилища сеансов
`loadSessionStore`.
...
Plugin все еще использует устаревшее вспомогательное средство записи всего хранилища, такое как
...
Plugin все еще использует устаревшие вспомогательные средства путей к файлам сеансов, такие как
...
- Используйте идентичность transcript или вспомогательные средства target, когда код подготавливает
  операцию transcript.
...
Plugin все еще использует устаревшее вспомогательное средство target файла transcript
...
- Избегайте прямого чтения или построения устаревших target файлов transcript.
...
Plugin все еще использует устаревшие низкоуровневые вспомогательные средства transcript, такие как
...
- Предпочитайте структурированную поверхность runtime transcript, чтобы OpenClaw мог применять
...
- Переместите работу с переопределением модели или провайдера в `before_model_resolve`.
- Переместите работу с изменением prompt или context в `before_prompt_build`.
...
метаданных setup или config, которых ожидает ClawHub.
...
- Отразите управляемую env настройку канала в текущих метаданных setup, channel config или
  package channel metadata, используемых формой вашего Plugin.
