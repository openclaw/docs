...
summary: Исправьте замечания по проверке пакета Plugin ClawHub перед публикацией
title: Исправления проверки Plugin
x-i18n:
    generated_at: "2026-07-04T06:43:40Z"
    model: gpt-5.5
...
| `sdk-session-store-write`               | [Замените записи во все хранилище сеансов](/ru/clawhub/plugin-validation-fixes#sdk-session-store-write)                           |
...
- Предпочитайте `package.json#version` в качестве версии выпуска пакета.
...
Блок `package.json#openclaw` содержит поля, которые не поддерживаются метаданными пакета OpenClaw.
...
Пакет невозможно упаковать в артефакт, который ClawHub будет проверять или публиковать.

- Запустите `npm pack --dry-run` из корня пакета.
- Исправьте недействительные метаданные пакета, сломанные lifecycle-скрипты или записи files, из-за которых упаковка завершается ошибкой.
...
В нативном манифесте Plugin не указано отображаемое имя.
...
- Перенесите поведение времени выполнения в код регистрации Plugin и ограничьте `contracts`
...
Plugin всё еще использует устаревшее вспомогательное средство всего хранилища сеансов
`loadSessionStore`.
...
- Оставляйте `loadSessionStore(...)` только пока объявленный диапазон совместимости
  всё еще поддерживает старые версии OpenClaw, которым оно требуется.
...
Plugin всё еще использует устаревшее вспомогательное средство записи всего хранилища сеансов, такое как
...
Plugin всё еще использует устаревшие вспомогательные средства путей к файлам сеанса, такие как
...
Plugin всё еще использует устаревшее вспомогательное средство цели файла транскрипта
...
- Оставляйте устаревшее вспомогательное средство только пока объявленный диапазон совместимости всё еще
...
Plugin всё еще использует устаревшие низкоуровневые вспомогательные средства транскриптов, такие как
...
- Отразите настройку канала, управляемую env, в текущих метаданных setup, config канала или
...
- Описывайте поведение, чувствительное к безопасности, в публичной документации пакета или
