...
x-i18n:
    generated_at: "2026-07-03T01:02:18Z"
    model: gpt-5.5
...
- Если в `openclaw.plugin.json` также есть `version`, обновите ее для совпадения или удалите устаревшие метаданные версии манифеста, когда метаданные пакета являются авторитетными.
...
Пакет невозможно упаковать в артефакт, который ClawHub проверял бы или публиковал.

- Запустите `npm pack --dry-run` из корня пакета.
- Исправьте некорректные метаданные пакета, сломанные скрипты жизненного цикла или записи files, из-за которых упаковка завершается ошибкой.
...
- Используйте `publishSessionTranscriptUpdateByIdentity(...)` для уведомлений об обновлении
  transcript.
- Предпочитайте структурированную поверхность runtime для transcript, чтобы OpenClaw мог применять
...
- Держите чувствительное к безопасности поведение задокументированным в публичной документации вашего пакета или
