---
read_when:
    - Вы запустили clawhub package validate и вам нужно исправить выявленные проблемы Plugin
...
x-i18n:
    generated_at: "2026-07-04T10:55:58Z"
    model: gpt-5.5
...
содержит служебные коды сканера без указаний по исправлению для автора, они предназначены
для сопровождающих OpenClaw, а не для авторов Plugin.
...
- Установите `openclaw.install.defaultChoice`, когда доступно несколько источников
  установки.
...
- Используйте версию API Plugin OpenClaw или минимальную semver-версию, против которой вы выполняли сборку и тестирование.
...
- Исправьте недопустимые метаданные пакета, сломанные lifecycle-скрипты или записи files,
...
Пакет можно упаковать, но упакованный артефакт не содержит файлы
...
- Перенесите поведение runtime в код регистрации Plugin и ограничьте `contracts`
  статическими метаданными владения возможностями.
- Повторно запустите `clawhub package validate <path-to-plugin>`.
...
- Оставляйте `loadSessionStore(...)` только пока объявленный диапазон совместимости
  всё еще поддерживает старые версии OpenClaw, которым она нужна.
...
Plugin всё еще использует устаревшую вспомогательную функцию записи всего хранилища сессий, например
...
- Оставляйте вспомогательные функции записи всего хранилища только пока объявленный диапазон совместимости
  всё еще поддерживает старые версии OpenClaw, которым они нужны.
- См. [Runtime API](/ru/plugins/sdk-runtime#agent-session-state) и
  [подпути Plugin SDK](/ru/plugins/sdk-subpaths).
- Повторно запустите `clawhub package validate <path-to-plugin>`.
...
Plugin всё еще использует устаревшие вспомогательные функции путей к файлам сессий, например
`resolveSessionFilePath` или `resolveAndPersistSessionFile`.

- Используйте `getSessionEntry(...)`, чтобы читать метаданные сессии по агенту и
...
- Используйте идентификатор transcript или вспомогательные функции target, когда код подготавливает
  операцию transcript.
- Не сохраняйте и не используйте устаревшие пути к файлам transcript.
- См. [Runtime API](/ru/plugins/sdk-runtime#agent-session-state) и
  [подпути Plugin SDK](/ru/plugins/sdk-subpaths).
- Повторно запустите `clawhub package validate <path-to-plugin>`.
...
- Избегайте прямого чтения или построения устаревших целей файлов transcript.
- Оставляйте устаревшую вспомогательную функцию только пока объявленный диапазон совместимости всё еще
  поддерживает старые версии OpenClaw, которым она нужна.
- См. [Runtime API](/ru/plugins/sdk-runtime#agent-session-state) и
  [подпути Plugin SDK](/ru/plugins/sdk-subpaths).
- Повторно запустите `clawhub package validate <path-to-plugin>`.
...
- Оставляйте низкоуровневые вспомогательные функции transcript только пока объявленный диапазон совместимости
  всё еще поддерживает старые версии OpenClaw, которым они нужны.
- См. [Runtime API](/ru/plugins/sdk-runtime#agent-session-state) и
  [подпути Plugin SDK](/ru/plugins/sdk-subpaths).
- Повторно запустите `clawhub package validate <path-to-plugin>`.
...
- Оставляйте `before_agent_start` только пока объявленный диапазон совместимости всё еще
  поддерживает старые версии OpenClaw, которым он нужен.
