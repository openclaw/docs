...
автоматических сканирований пакетов. Эта страница описывает результаты, предназначенные для авторов,
то есть те, которые автор Plugin может исправить в метаданных пакета, манифесте, импортах SDK
...
| `sdk-session-transcript-file-target`    | [Замените устаревшие файловые цели transcript](/ru/clawhub/plugin-validation-fixes#sdk-session-transcript-file-target)                   |
| `sdk-session-transcript-low-level`      | [Замените низкоуровневые вспомогательные функции transcript](/ru/clawhub/plugin-validation-fixes#sdk-session-transcript-low-level)                       |
...
- Задайте `openclaw.install.defaultChoice`, когда доступно несколько источников
  установки.
...
- Используйте версию API Plugin OpenClaw или минимальную semver-версию, под которую вы
...
- Если в `openclaw.plugin.json` также есть `version`, обновите ее для совпадения или удалите
...
Пакет не может быть упакован в артефакт, который ClawHub будет проверять или
публиковать.
...
- Убедитесь, что `openclaw.plugin.json` включен, когда пакет является собственным
  Plugin OpenClaw.
...
- Сделайте `name` понятным для человека, а `id` оставьте стабильным машинным идентификатором.
- См. [манифест Plugin](/ru/plugins/manifest).
- Повторно выполните `clawhub package validate <path-to-plugin>`.
...
- Используйте [правила импортов](/ru/plugins/building-plugins#import-conventions) и
...
- Используйте идентичность транскрипта или вспомогательные функции target, когда код подготавливает
  операцию с транскриптом.
- Не сохраняйте и не полагайтесь на устаревшие пути к файлам транскриптов.
- См. [Runtime API](/ru/plugins/sdk-runtime#agent-session-state) и
  [подпути Plugin SDK](/ru/plugins/sdk-subpaths).
- Повторно выполните `clawhub package validate <path-to-plugin>`.
...
Plugin все еще использует устаревшую вспомогательную функцию target файла транскрипта
...
- Избегайте прямого чтения или создания устаревших файловых targets транскриптов.
- Сохраняйте устаревшую вспомогательную функцию только пока объявленный диапазон совместимости все еще
...
- Используйте `appendSessionTranscriptMessageByIdentity(...)` для добавлений в транскрипт.
...
- Сохраняйте `before_agent_start` только пока объявленный диапазон совместимости все еще
...
- Оставляйте метаданные переменных окружения канала декларативными, чтобы OpenClaw мог проверять статус setup
  без загрузки runtime канала.
- Отразите настройку канала, управляемую переменными окружения, в текущих метаданных setup, channel config или
...
- Удалите URL схемы, если он предназначен только для справки.
