...
summary: Исправить замечания проверки пакета плагина ClawHub перед публикацией
title: Исправления валидации Plugin
x-i18n:
    generated_at: "2026-07-05T07:47:44Z"
    model: gpt-5.5
...
автоматизированного сканирования пакетов. Эта страница описывает находки, предназначенные для авторов, то есть
находки, которые автор Plugin может исправить в метаданных пакета, манифесте, импортах SDK
или опубликованном артефакте.

Она не описывает внутренние находки покрытия Plugin Inspector. Если полный отчет
...
относительно которой пакет был собран.
...
- Исправьте недействительные метаданные пакета, сломанные lifecycle-скрипты или записи files, из-за которых
...
- Используйте `getSessionEntry(...)`, чтобы читать метаданные сессии по идентификаторам агента и
  сессии.
...
- Избегайте прямого чтения или создания устаревших файловых целей транскрипта.
...
Plugin все еще использует устаревшие низкоуровневые вспомогательные функции транскрипта, например
`appendSessionTranscriptMessage` или `emitSessionTranscriptUpdate`.

- Используйте `appendSessionTranscriptMessageByIdentity(...)` для добавления записей в транскрипт.
...
- Продублируйте метаданные env-переменных провайдера в `setup.providers[].envVars`.
...
Манифест использует устаревшие или старые метаданные env-переменных канала без текущих
метаданных setup или конфигурации, которых ожидает ClawHub.

- Держите метаданные env-переменных канала декларативными, чтобы OpenClaw мог проверять статус setup
  без загрузки runtime канала.
- Продублируйте setup канала на основе env в текущие метаданные setup, конфигурации канала или
  пакета канала, используемые вашей формой Plugin.
