---
read_when:
    - Вы запустили clawhub package validate и должны исправить замечания к Plugin
...
x-i18n:
    generated_at: "2026-07-04T20:38:45Z"
    model: gpt-5.5
...
# Исправления проверок Plugin

ClawHub проверяет пакеты Plugin перед публикацией, а также может показывать находки из
автоматизированных сканирований пакетов. На этой странице описаны находки для авторов,
то есть находки, которые автор Plugin может исправить в метаданных пакета, манифесте,
импортах SDK или опубликованном артефакте.

Она не описывает внутренние находки покрытия Plugin Inspector. Если полный отчет
содержит коды обслуживания сканера без рекомендаций по исправлению для автора,
они предназначены для сопровождающих OpenClaw, а не для авторов Plugin.

После применения любого исправления снова выполните:
...
| `provider-auth-env-vars`                | [Переместите env vars провайдера в метаданные настройки](/ru/clawhub/plugin-validation-fixes#provider-auth-env-vars)                             |
...
- Снова выполните `clawhub package validate <path-to-plugin>`.
...
- Заполните `openclaw.install` поддерживаемым источником установки, таким как
  `clawhubSpec`, `npmSpec` или `localPath`.
- Задайте `openclaw.install.defaultChoice`, когда доступно больше одного источника
  установки.
...
- Используйте версию API Plugin OpenClaw или минимальную semver-версию, против которой вы
  собирали и тестировали пакет.
...
- Если `openclaw.plugin.json` также содержит `version`, обновите его, чтобы он совпадал,
  или удалите устаревшие метаданные версии манифеста, когда метаданные пакета являются авторитетными.
...
Plugin все еще использует устаревшую вспомогательную функцию для всего хранилища сессий
`loadSessionStore`.
...
Plugin все еще использует устаревшую вспомогательную функцию записи всего хранилища сессий, такую как
...
- Оставляйте низкоуровневые вспомогательные функции транскрипта только пока объявленный диапазон совместимости
  все еще поддерживает старые версии OpenClaw, которым они требуются.
- См. [API среды выполнения](/ru/plugins/sdk-runtime#agent-session-state) и
  [подпути Plugin SDK](/ru/plugins/sdk-subpaths).
- Повторно выполните `clawhub package validate <path-to-plugin>`.
...
- Оставляйте метаданные env-var канала декларативными, чтобы OpenClaw мог проверять статус настройки
