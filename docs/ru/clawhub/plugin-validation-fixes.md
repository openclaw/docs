...
read_when:
    ...
    - ClawHub отклонил или выдал предупреждение при публикации пакета Plugin
...
x-i18n:
    generated_at: "2026-07-03T17:34:02Z"
    model: gpt-5.5
...
ClawHub проверяет пакеты Plugin перед публикацией, а также может показывать результаты автоматических сканирований пакета. На этой странице описаны результаты для авторов, то есть результаты, которые автор Plugin может исправить в метаданных пакета, манифесте, импортах SDK или опубликованном артефакте.

Она не охватывает внутренние результаты покрытия Plugin Inspector. Если полный отчет содержит коды обслуживания сканера без указаний по исправлению для автора, они предназначены для сопровождающих OpenClaw, а не для авторов Plugin.
...
| `sdk-session-store-write`               | [Замените записи во все хранилище сессии](/ru/clawhub/plugin-validation-fixes#sdk-session-store-write)                             |
| `sdk-session-file-helper`               | [Замените вспомогательные функции путей к файлам сессии](/ru/clawhub/plugin-validation-fixes#sdk-session-file-helper)              |
...
В корне пакета нет `package.json`, поэтому ClawHub не может определить npm-пакет, версию, точки входа или метаданные OpenClaw.

- Добавьте `package.json` с `name`, `version` и `type`.
- Добавьте блок `openclaw`, если пакет поставляет Plugin OpenClaw.
- Используйте [Создание Plugin](/ru/plugins/building-plugins) как минимальный пример пакета и [Манифест Plugin](/ru/plugins/manifest#manifest-versus-packagejson), чтобы понять разделение пакета и манифеста.
...
Метаданные пакета существуют, но в них не объявлена точка входа runtime OpenClaw.
...
- Проверьте все метаданные сборки OpenClaw в пакете, например версию OpenClaw, использованную во время выпуска.
...
- Если в `openclaw.plugin.json` тоже есть `version`, обновите ее, чтобы она совпадала, или удалите устаревшие метаданные версии манифеста, когда метаданные пакета являются авторитетными.
...
- Держите собственные метаданные Plugin в `openclaw.plugin.json`.
...
- Вместо манифеста переместите метаданные пакета или установки в поддерживаемые поля
...
- Замените импорты из корневого барреля точечными импортами из публичных подпутей.
...
- Замените зарезервированные внутренние импорты SDK OpenClaw документированными публичными
...
Plugin все еще использует устаревшую вспомогательную функцию для всего хранилища сеансов
`loadSessionStore`.
...
Plugin все еще использует устаревшую вспомогательную функцию записи всего хранилища сеансов, например
`saveSessionStore` или `updateSessionStore`.

- Используйте `patchSessionEntry(...)` при обновлении полей существующей записи сеанса.
...
Plugin все еще использует устаревшие вспомогательные функции путей к файлам сеансов, такие как
`resolveSessionFilePath` или `resolveAndPersistSessionFile`.

- Используйте `getSessionEntry(...)`, чтобы читать метаданные сеанса по идентификаторам агента и сеанса.
...
- Переместите работу по изменению промпта или контекста в `before_prompt_build`.
