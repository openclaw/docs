...
x-i18n:
    generated_at: "2026-06-30T22:27:33Z"
    model: gpt-5.5
...
- Добавьте блок `openclaw`, если пакет поставляет OpenClaw Plugin.
...
- Запустите `clawhub package validate <path-to-plugin>` повторно.
...
- Если в `openclaw.plugin.json` также есть `version`, обновите ее, чтобы она совпадала, или удалите
  устаревшие метаданные версии манифеста, когда метаданные пакета являются авторитетным источником.
...
Нативный манифест плагина не содержит отображаемого имени.

- Добавьте непустое поле `name` в `openclaw.plugin.json`.
- Оставьте `name` понятным для человека, а `id` используйте как стабильный машинный идентификатор.
- См. [манифест Plugin](/ru/plugins/manifest).
- Повторно выполните `clawhub package validate <path-to-plugin>`.

### manifest-unknown-fields

В манифесте плагина есть поля верхнего уровня, которые OpenClaw не поддерживает.
...
Плагин импортирует из устаревшего корневого barrel SDK:
`openclaw/plugin-sdk`.
...
Плагин все еще использует устаревшую вспомогательную функцию всего хранилища сессий
`loadSessionStore`.
...
Плагин все еще использует устаревшую вспомогательную функцию записи всего хранилища сессий, например
...
Плагин все еще использует устаревшие вспомогательные функции путей к файлам сессий, например
`resolveSessionFilePath` или `resolveAndPersistSessionFile`.

- Используйте `getSessionEntry(...)`, чтобы читать метаданные сессии по идентификаторам
  агента и сессии.
- Используйте `patchSessionEntry(...)` или `upsertSessionEntry(...)`, чтобы сохранять метаданные
  сессии.
- Используйте идентификатор transcript или вспомогательные функции target, когда код готовит
  операцию transcript.
...
Плагин все еще использует устаревшую вспомогательную функцию target файла transcript
...
- Избегайте прямого чтения или построения устаревших targets файлов transcript.
...
Плагин все еще использует устаревшие низкоуровневые вспомогательные функции transcript, например
...
- Используйте `publishSessionTranscriptUpdateByIdentity(...)` для уведомлений об обновлениях
  transcript.
- Предпочитайте структурированную поверхность времени выполнения transcript, чтобы OpenClaw мог применять
...
Плагин все еще использует устаревший hook `before_agent_start`.
...
- Отразите настройку канала через env в текущих метаданных setup, config канала или
  пакета канала, используемых формой вашего плагина.
