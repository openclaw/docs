---
read_when:
    - Вы выполнили clawhub package validate, и вам нужно исправить замечания Plugin
...
x-i18n:
    generated_at: "2026-07-03T23:39:06Z"
    model: gpt-5.5
...
ClawHub проверяет пакеты Plugin перед публикацией, а также может показывать результаты автоматического сканирования пакетов. На этой странице описаны результаты для авторов, то есть те, которые автор Plugin может исправить в метаданных пакета, манифесте, импортах SDK или опубликованном артефакте.
...
- Используйте версию API Plugin OpenClaw или нижнюю границу semver, с которой вы собирали и тестировали.
...
Минимальная версия хоста в пакете не совпадает с метаданными версии OpenClaw, с которыми пакет был собран.
...
Пакет не может быть упакован в артефакт, который ClawHub проверял бы или публиковал.

- Выполните `npm pack --dry-run` из корня пакета.
- Исправьте некорректные метаданные пакета, неработающие lifecycle-скрипты или записи files, из-за которых упаковка завершается ошибкой.
...
- Убедитесь, что `openclaw.plugin.json` включен, если пакет является собственным Plugin OpenClaw.
...
Plugin все еще использует устаревшее вспомогательное средство для всего хранилища сеансов
`loadSessionStore`.
...
- Оставляйте `loadSessionStore(...)` только пока объявленный диапазон совместимости
  все еще поддерживает более старые версии OpenClaw, которым это требуется.
...
Plugin все еще использует устаревшее вспомогательное средство записи всего хранилища сеансов, например
...
Plugin все еще использует устаревшие вспомогательные средства путей к файлам сеансов, например
...
- Используйте `patchSessionEntry(...)` или `upsertSessionEntry(...)`, чтобы сохранять метаданные
  сеанса.
- Используйте идентичность транскрипта или вспомогательные средства цели, когда код готовит
...
- Избегайте прямого чтения или построения устаревших файловых целей транскриптов.
...
- Отразите метаданные переменных среды провайдера в `setup.providers[].envVars`.
...
Манифест использует устаревшие или более старые метаданные переменных среды канала без текущих
метаданных setup или config, которые ожидает ClawHub.

- Держите метаданные переменных среды канала декларативными, чтобы OpenClaw мог проверять состояние setup
  без загрузки runtime канала.
- Отразите настройку канала на основе переменных среды в текущих метаданных setup, config канала или
