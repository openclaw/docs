...
Pronto per DM e canali di guild tramite il gateway ufficiale di Discord.
...
<Steps>
  <Step title="Create a Discord application and bot">
    ...
    Fai clic su **Bot** nella barra laterale. Imposta **Username** su qualsiasi nome tu dia al tuo agente OpenClaw.
  ...
  <Step title="Generate an invite URL and add the bot to your server">
    ...
    **Text Permissions**
    ...
    Questo è il set di base per i normali canali di testo. Se prevedi di pubblicare nei thread di Discord, inclusi flussi di lavoro di canali forum o media che creano o continuano un thread, abilita anche **Send Messages in Threads**.
    Copia l'URL generato in basso, incollalo nel browser, seleziona il tuo server e fai clic su **Continue** per connettere. Ora dovresti vedere il tuo bot nel server Discord.
  ...
  <Step title="Allow DMs from server members">
    Perché l'abbinamento funzioni, Discord deve consentire al tuo bot di inviarti DM. Fai clic con il tasto destro sulla tua **icona del server** → **Privacy Settings** → attiva **Direct Messages**.

    Questo consente ai membri del server (inclusi i bot) di inviarti DM. Mantienilo abilitato se vuoi usare i DM di Discord con OpenClaw. Se prevedi di usare solo canali di guild, puoi disabilitare i DM dopo l'abbinamento.
...
```
    ...
    Per installazioni come servizio gestito, esegui `openclaw gateway install` da una shell in cui `DISCORD_BOT_TOKEN` è presente, oppure memorizza la variabile in `~/.openclaw/.env`, così il servizio può risolvere SecretRef env dopo il riavvio.
    Se il tuo host è bloccato o limitato dal rate limiting della ricerca dell'applicazione di avvio di Discord, imposta l'ID applicazione/client Discord dal Developer Portal così l'avvio può saltare quella chiamata REST. Usa `channels.discord.applicationId` per l'account predefinito, oppure `channels.discord.accounts.<accountId>.applicationId` quando esegui più bot Discord.
  ...
  <Step title="Configure OpenClaw and pair">
    ...
    <Tabs>
      <Tab title="Ask your agent">
        Chatta con il tuo agente OpenClaw su qualsiasi canale esistente (ad es. Telegram) e diglielo. Se Discord è il tuo primo canale, usa invece la scheda CLI / config.
...
```
        ...
        Per più bot Discord, mantieni ogni token del bot e ID applicazione sotto il relativo account. Un `channels.discord.applicationId` di livello superiore viene ereditato dagli account, quindi impostalo lì solo quando ogni account deve usare lo stesso ID applicazione.
...
Per chiamate in uscita avanzate (azioni dello strumento messaggio/canale), per quella chiamata viene usato un `token` esplicito per chiamata. Questo si applica ad azioni di invio e di tipo lettura/sondaggio (per esempio read/search/fetch/thread/pins/permissions). Le impostazioni di policy/retry dell'account provengono comunque dall'account selezionato nello snapshot del runtime attivo.
</Note>

## Consigliato: configura un workspace di guild

Quando i DM funzionano, puoi configurare il tuo server Discord come workspace completo in cui ogni canale ottiene la propria sessione agente con il proprio contesto. Questo è consigliato per server privati in cui ci siete solo tu e il tuo bot.
...
<Steps>
  <Step title="Add your server to the guild allowlist">
    ...
    <Tabs>
      <Tab title="Ask your agent">
        > "Aggiungi il mio Discord Server ID `<server_id>` alla allowlist della guild"
      </Tab>
      <Tab title="Config">
...
```
  ...
  <Step title="Allow responses without @mention">
    Per impostazione predefinita, il tuo agente risponde nei canali di guild solo quando viene @menzionato. Per un server privato, probabilmente vuoi che risponda a ogni messaggio.

    Nei canali di guild, le normali risposte finali dell'assistente restano private per impostazione predefinita. L'output visibile su Discord deve essere inviato esplicitamente con lo strumento `message`, così l'agente può restare in ascolto per impostazione predefinita e pubblicare solo quando decide che una risposta nel canale è utile.

    Questo significa che il modello selezionato deve chiamare gli strumenti in modo affidabile. Se Discord mostra che sta scrivendo e i log mostrano uso di token ma nessun messaggio pubblicato, controlla nel log della sessione la presenza di testo dell'assistente con `didSendViaMessagingTool: false`. Questo significa che il modello ha prodotto una risposta finale privata invece di chiamare `message(action=send)`. Passa a un modello più forte nella chiamata degli strumenti, oppure usa la configurazione sotto per ripristinare le risposte finali automatiche legacy.
...
```
  ...
  <Step title="Plan for memory in guild channels">
    Per impostazione predefinita, la memoria a lungo termine (MEMORY.md) viene caricata solo nelle sessioni DM. I canali di guild non caricano automaticamente MEMORY.md.
    ...
    <Tabs>
      ...
      <Tab title="Manual">
        Se hai bisogno di contesto condiviso in ogni canale, metti le istruzioni stabili in `AGENTS.md` o `USER.md` (vengono iniettate in ogni sessione). Mantieni le note a lungo termine in `MEMORY.md` e accedivi su richiesta con gli strumenti di memoria.
      </Tab>
    </Tabs>
...
- I metadati di guild/canale Discord vengono aggiunti al prompt del modello come contesto non attendibile, non come prefisso della risposta visibile all'utente. Se un modello ricopia quell'involucro, OpenClaw rimuove i metadati copiati dalle risposte in uscita e dal contesto di replay futuro.
...
- La consegna degli annunci testuali Cron/Heartbeat a Discord usa una sola volta la risposta finale visibile all'assistente. I payload multimediali e dei componenti strutturati rimangono multi-messaggio quando l'agente emette più payload consegnabili.
...
OpenClaw supporta i contenitori dei componenti Discord v2 per i messaggi degli agenti. Usa lo strumento messaggio con un payload `components`. I risultati delle interazioni vengono instradati all'agente come normali messaggi in ingresso e seguono le impostazioni Discord `replyToMode` esistenti.
...
<Tabs>
  <Tab title="Criterio DM">
    ...
    Se il criterio DM non è aperto, gli utenti sconosciuti vengono bloccati (o invitati all'abbinamento in modalità `pairing`).
    ...
    Gli ID numerici nudi normalmente si risolvono come ID canale quando è attivo un canale predefinito, ma gli ID elencati nel DM effettivo dell'account `allowFrom` vengono trattati come target DM utente per compatibilità.
...
```
    ...
    Un canale testuale Discord non ha un elenco membri separato. `type: "discord.channelAudience"` modella l'appartenenza così: il mittente DM è membro della guild configurata e al momento dispone dell'autorizzazione effettiva `ViewChannel` sul canale configurato dopo l'applicazione dei ruoli e delle sovrascritture del canale.
...
```
    ...
    Le ricerche falliscono in modo chiuso. Se Discord restituisce `Missing Access`, la ricerca del membro fallisce, o il canale appartiene a una guild diversa, il mittente DM viene trattato come non autorizzato.

    Abilita **Server Members Intent** nel Portale sviluppatori Discord per il bot quando usi gruppi di accesso basati sul pubblico del canale. I DM non includono lo stato del membro della guild, quindi OpenClaw risolve il membro tramite Discord REST al momento dell'autorizzazione.
...
```
  ...
  <Tab title="Menzioni e DM di gruppo">
    I messaggi delle guild sono soggetti per impostazione predefinita a gate basato su menzione.
    ...
    Quando scrivi messaggi Discord in uscita, usa la sintassi canonica delle menzioni: `<@USER_ID>` per gli utenti, `<#CHANNEL_ID>` per i canali e `<@&ROLE_ID>` per i ruoli. Non usare il formato di menzione nickname legacy `<@!USER_ID>`.
...
- `commands.native` usa come predefinito `"auto"` ed è abilitato per Discord.
- Override per canale: `channels.discord.commands.native`.
- `commands.native=false` salta la registrazione e la pulizia dei comandi slash Discord durante l'avvio. I comandi registrati in precedenza possono rimanere visibili in Discord finché non li rimuovi dall'app Discord.
- L'autenticazione dei comandi nativi usa le stesse allowlist/policy Discord della normale gestione dei messaggi.
- I comandi possono comunque essere visibili nell'interfaccia utente di Discord per gli utenti non autorizzati; l'esecuzione applica comunque l'autenticazione di OpenClaw e restituisce "not authorized".

Consulta [Comandi slash](/it/tools/slash-commands) per catalogo e comportamento dei comandi.
...
<AccordionGroup>
  <Accordion title="Reply tags and native replies">
    ...
    `first` allega sempre il riferimento implicito alla risposta nativa al primo messaggio Discord in uscita per il turno.
  ...
  <Accordion title="Live stream preview">
    OpenClaw può trasmettere bozze di risposta inviando un messaggio temporaneo e modificandolo man mano che arriva il testo. `channels.discord.streaming` accetta `off` | `partial` | `block` | `progress` (predefinito). `progress` mantiene una bozza di stato modificabile e la aggiorna con l'avanzamento degli strumenti fino alla consegna finale; l'etichetta iniziale condivisa è una riga mobile, quindi scorre via come il resto quando compare abbastanza lavoro. `streamMode` è un alias runtime legacy. Esegui `openclaw doctor --fix` per riscrivere la configurazione persistente nella chiave canonica.
...
```
    ...
    - `block` emette blocchi delle dimensioni di una bozza (usa `draftChunk` per regolare dimensione e punti di interruzione, con limite massimo a `textChunkLimit`).
    - I finali multimediali, di errore e con risposta esplicita annullano le modifiche di anteprima in sospeso.
    ...
    - Le righe di strumenti/avanzamento vengono rese come emoji compatta + titolo + dettaglio quando disponibile, per esempio `🛠️ Bash: run tests` o `🔎 Web Search: for "query"`.
  ...
  <Accordion title="History, context, and thread behavior">
    ...
    - Le sessioni thread ereditano la selezione `/model` a livello di sessione del canale padre come fallback solo modello; le selezioni `/model` locali al thread hanno comunque la precedenza e la cronologia della trascrizione padre non viene copiata salvo che sia abilitata l'ereditarietà della trascrizione.
    ...
    Gli argomenti dei canali vengono iniettati come contesto **non attendibile**. Le allowlist limitano chi può attivare l'agente, non sono un vero confine di redazione del contesto supplementare.

  </Accordion>

  <Accordion title="Thread-bound sessions for subagents">
    Discord può associare un thread a un target di sessione, così i messaggi successivi in quel thread continuano a essere instradati alla stessa sessione (incluse le sessioni dei sottoagenti).
    ...
    - `/agents` mostra le esecuzioni attive e lo stato di associazione
    - `/session idle <duration|off>` ispeziona/aggiorna l'auto-unfocus per inattività delle associazioni focalizzate
    - `/session max-age <duration|off>` ispeziona/aggiorna l'età massima rigida delle associazioni focalizzate
...
```
    ...
    - `defaultSpawnContext` controlla il contesto nativo del sottoagente per gli spawn associati a thread. Predefinito: `"fork"`.
    ...
    Consulta [Sottoagenti](/it/tools/subagents), [Agenti ACP](/it/tools/acp-agents) e [Riferimento di configurazione](/it/gateway/configuration-reference).

  </Accordion>

  <Accordion title="Persistent ACP channel bindings">
    Per workspace ACP stabili "always-on", configura associazioni ACP tipizzate di livello superiore destinate a conversazioni Discord.
...
```
    ...
    Consulta [Agenti ACP](/it/tools/acp-agents) per i dettagli sul comportamento di associazione.
...
```
  ...
  <Accordion title="PluralKit support">
    Abilita la risoluzione PluralKit per mappare i messaggi proxied all'identità del membro di sistema:
...
```
    ...
    - le ricerche usano l'ID messaggio originale e sono limitate da una finestra temporale
    - se la ricerca non riesce, i messaggi proxied vengono trattati come messaggi bot e scartati, salvo `allowBots=true`
...
```
    ...
    - 2: In ascolto
    - 3: Guardando
    - 4: Personalizzato (usa il testo dell'attivita come stato; l'emoji e facoltativa)
    - 5: In competizione
...
```
    ...
    La presenza automatica mappa la disponibilita del runtime allo stato Discord: sano => online, degradato o sconosciuto => inattivo, esaurito o non disponibile => dnd. Override di testo facoltativi:
  ...
  <Accordion title="Approvazioni in Discord">
    Discord supporta la gestione delle approvazioni basata su pulsanti nei DM e puo facoltativamente pubblicare le richieste di approvazione nel canale di origine.
    ...
    Discord abilita automaticamente le approvazioni exec native quando `enabled` non e impostato o e `"auto"` e almeno un approvatore puo essere risolto, da `execApprovals.approvers` o da `commands.ownerAllowFrom`. Discord non deduce gli approvatori exec dal canale `allowFrom`, dal legacy `dm.allowFrom` o dal messaggio diretto `defaultTo`. Imposta `enabled: false` per disabilitare esplicitamente Discord come client di approvazione nativo.

    Per comandi di gruppo sensibili riservati al proprietario, come `/diagnostics` e `/export-trajectory`, OpenClaw invia privatamente le richieste di approvazione e i risultati finali. Prova prima il DM Discord quando il proprietario che invoca il comando ha una route proprietario Discord; se non e disponibile, ripiega sulla prima route proprietario disponibile da `commands.ownerAllowFrom`, come Telegram.

    Quando `target` e `channel` o `both`, la richiesta di approvazione e visibile nel canale. Solo gli approvatori risolti possono usare i pulsanti; gli altri utenti ricevono un rifiuto effimero. Le richieste di approvazione includono il testo del comando, quindi abilita la consegna nel canale solo in canali attendibili. Se l'ID del canale non puo essere derivato dalla chiave di sessione, OpenClaw ripiega sulla consegna via DM.

    Discord esegue anche il rendering dei pulsanti di approvazione condivisi usati da altri canali chat. L'adapter Discord nativo aggiunge principalmente il routing DM degli approvatori e il fanout del canale.
    ...
    le approvazioni chat non sono disponibili o l'approvazione manuale e l'unico percorso.
    Se il runtime di approvazione nativo Discord non e attivo, OpenClaw mantiene visibile il
    prompt deterministico locale `/approve <id> <decision>`. Se il
    runtime e attivo ma non e possibile consegnare una scheda nativa ad alcun target,
...
- `voice.tts` sostituisce `messages.tts` solo per la riproduzione vocale `stt-tts`. Le modalità in tempo reale usano `voice.realtime.voice`.
- `voice.mode` controlla il percorso della conversazione. Il valore predefinito è `agent-proxy`: un front end vocale in tempo reale gestisce tempistica dei turni, interruzione e riproduzione, delega il lavoro sostanziale all'agente OpenClaw instradato tramite `openclaw_agent_consult` e tratta il risultato come un prompt Discord digitato da quell'interlocutore. `stt-tts` mantiene il flusso batch precedente STT più TTS. `bidi` consente al modello in tempo reale di conversare direttamente esponendo al contempo `openclaw_agent_consult` per il cervello OpenClaw.
- `voice.agentSession` controlla quale conversazione OpenClaw riceve i turni vocali. Lascialo non impostato per la sessione propria del canale vocale, oppure imposta `{ mode: "target", target: "channel:<text-channel-id>" }` per fare in modo che il canale vocale agisca come estensione microfono/altoparlante di una sessione di canale testuale Discord esistente, come `#maintainers`.
- `voice.model` sostituisce il cervello dell'agente OpenClaw per le risposte vocali Discord e le consultazioni in tempo reale. Lascialo non impostato per ereditare il modello dell'agente instradato. È separato da `voice.realtime.model`.
- `agent-proxy` instrada il parlato tramite `discord-voice`, che preserva la normale autorizzazione del proprietario e degli strumenti per l'interlocutore e la sessione di destinazione, ma nasconde lo strumento `tts` dell'agente perché la voce Discord possiede la riproduzione. Per impostazione predefinita, `agent-proxy` concede alla consultazione l'accesso completo agli strumenti equivalente al proprietario per gli interlocutori proprietari (`voice.realtime.toolPolicy: "owner"`) e preferisce fortemente consultare l'agente OpenClaw prima di risposte sostanziali (`voice.realtime.consultPolicy: "always"`). In quella modalità predefinita `always`, il livello in tempo reale non pronuncia automaticamente riempitivi prima della risposta della consultazione; cattura e trascrive il parlato, quindi pronuncia la risposta OpenClaw instradata. Se più risposte di consultazione forzata terminano mentre Discord sta ancora riproducendo la prima risposta, le risposte successive con parlato esatto vengono accodate finché la riproduzione non è inattiva, invece di sostituire il parlato a metà frase.
- In modalità `stt-tts`, STT usa `tools.media.audio`; `voice.model` non influisce sulla trascrizione.
- Nelle modalità in tempo reale, `voice.realtime.provider`, `voice.realtime.model` e `voice.realtime.voice` configurano la sessione audio in tempo reale. Per OpenAI Realtime 2 più il cervello Codex, usa `voice.realtime.model: "gpt-realtime-2"` e `voice.model: "openai-codex/gpt-5.5"`.
- Il provider in tempo reale OpenAI accetta i nomi evento correnti di Realtime 2 e gli alias legacy compatibili con Codex per gli eventi di audio in uscita e trascrizione, così gli snapshot di provider compatibili possono divergere senza perdere l'audio dell'assistente.
- `voice.realtime.bargeIn` controlla se gli eventi di inizio parlato di un interlocutore Discord interrompono la riproduzione attiva in tempo reale. Se non è impostato, segue l'impostazione di interruzione audio in input del provider in tempo reale.
- `voice.realtime.minBargeInAudioEndMs` controlla la durata minima di riproduzione dell'assistente prima che un'interruzione OpenAI in tempo reale tronchi l'audio. Predefinito: `250`. Imposta `0` per l'interruzione immediata in stanze con poco eco, oppure aumentalo per configurazioni con altoparlanti molto soggette a eco.
- Per una voce OpenAI sulla riproduzione Discord, imposta `voice.tts.provider: "openai"` e scegli una voce Text-to-speech in `voice.tts.openai.voice` o `voice.tts.providers.openai.voice`. `cedar` è una buona scelta dal suono maschile sul modello TTS OpenAI corrente.
- Le sostituzioni `systemPrompt` Discord per canale si applicano ai turni di trascrizione vocale per quel canale vocale.
- I turni di trascrizione vocale derivano lo stato di proprietario da `allowFrom` di Discord (o `dm.allowFrom`); gli interlocutori non proprietari non possono accedere agli strumenti riservati ai proprietari (per esempio `gateway` e `cron`).
- La voce Discord è opt-in per le configurazioni solo testuali; imposta `channels.discord.voice.enabled=true` (oppure mantieni un blocco `channels.discord.voice` esistente) per abilitare i comandi `/vc`, il runtime vocale e l'intent Gateway `GuildVoiceStates`.
...
- Se `voice.autoJoin` ha più voci per la stessa gilda, OpenClaw entra nell'ultimo canale configurato per quella gilda.
- `voice.daveEncryption` e `voice.decryptionFailureTolerance` passano alle opzioni di join di `@discordjs/voice`.
- I valori predefiniti di `@discordjs/voice` sono `daveEncryption=true` e `decryptionFailureTolerance=24` se non impostati.
- OpenClaw usa per impostazione predefinita il decoder `opusscript` pure-JS per la ricezione vocale Discord. Il pacchetto nativo opzionale `@discordjs/opus` è ignorato dalla policy di installazione pnpm del repository, così le installazioni normali, le corsie Docker e i test non correlati non compilano un addon nativo. Gli host dedicati alle prestazioni vocali possono optare per l'uso con `OPENCLAW_DISCORD_OPUS_DECODER=native` dopo aver installato l'addon nativo.
...
- `voice.reconnectGraceMs` controlla per quanto tempo OpenClaw attende che una sessione vocale disconnessa inizi la riconnessione prima di distruggerla. Predefinito: `15000`.
- In modalità `stt-tts`, la riproduzione vocale non si interrompe solo perché un altro utente inizia a parlare. Per evitare cicli di feedback, OpenClaw ignora la nuova cattura vocale mentre TTS è in riproduzione; parla dopo il termine della riproduzione per il turno successivo. Le modalità in tempo reale inoltrano gli inizi parlato degli interlocutori come segnali di interruzione al provider in tempo reale.
- Nelle modalità in tempo reale, l'eco dagli altoparlanti verso un microfono aperto può sembrare un'interruzione e interrompere la riproduzione. Per stanze Discord molto soggette a eco, imposta `voice.realtime.providers.openai.interruptResponseOnInputAudio: false` per impedire a OpenAI di interrompere automaticamente sull'audio in input. Aggiungi `voice.realtime.bargeIn: true` se vuoi comunque che gli eventi di inizio parlato Discord interrompano la riproduzione attiva. Il bridge in tempo reale OpenAI ignora i troncamenti di riproduzione più brevi di `voice.realtime.minBargeInAudioEndMs` come probabile eco/rumore e li registra come saltati invece di cancellare la riproduzione Discord.
- `voice.captureSilenceGraceMs` controlla per quanto tempo OpenClaw attende dopo che Discord segnala che un interlocutore ha smesso di parlare prima di finalizzare quel segmento audio per STT. Predefinito: `2500`; aumentalo se Discord divide le pause normali in trascrizioni parziali frammentate.
- Quando ElevenLabs è il provider TTS selezionato, la riproduzione vocale Discord usa TTS in streaming e parte dal flusso di risposta del provider. I provider senza supporto allo streaming ripiegano sul percorso del file temporaneo sintetizzato.
- OpenClaw osserva anche gli errori di decrittazione in ricezione e recupera automaticamente uscendo e rientrando nel canale vocale dopo errori ripetuti in una breve finestra.
- Se i log di ricezione mostrano ripetutamente `DecryptionFailed(UnencryptedWhenPassthroughDisabled)` dopo l'aggiornamento, raccogli un report delle dipendenze e i log. La linea `@discordjs/voice` inclusa comprende la correzione upstream del padding dalla PR discord.js #11449, che ha chiuso la issue discord.js #11419.
- Gli eventi di ricezione `The operation was aborted` sono previsti quando OpenClaw finalizza un segmento interlocutore catturato; sono diagnostica dettagliata, non avvisi.
- I log vocali dettagliati Discord includono un'anteprima limitata su una riga della trascrizione STT per ogni segmento interlocutore accettato, così il debug mostra sia il lato utente sia il lato risposta dell'agente senza scaricare testo di trascrizione non limitato.
- In modalità `agent-proxy`, il fallback della consultazione forzata salta frammenti di trascrizione probabilmente incompleti, come testo che termina in `...` o un connettore finale come `and`, più chiusure ovvie non azionabili come “be right back” o “bye”. I log mostrano `forced agent consult skipped reason=...` quando questo impedisce una risposta accodata obsoleta.

Configurazione di opus nativo per checkout del sorgente:
...
Usa Node 22 per il Gateway quando vuoi l'addon nativo precompilato upstream macOS arm64. Se usi un altro runtime Node, l'installer opt-in potrebbe richiedere una toolchain locale di build da sorgente `node-gyp`.
...
I log vocali dettagliati dovrebbero mostrare `discord voice: opus decoder: @discordjs/opus`. Senza l'opt-in tramite variabile d'ambiente, oppure se l'addon nativo è mancante o non può essere caricato sull'host, OpenClaw registra `discord voice: opus decoder: opusscript` e continua a ricevere voce tramite il fallback pure-JS.
...
- La trascrizione viene inviata tramite ingresso e instradamento Discord mentre l'LLM di risposta gira con una policy di output vocale che nasconde lo strumento `tts` dell'agente e richiede testo restituito, perché la voce Discord possiede la riproduzione TTS finale.
- `voice.model`, quando impostato, sostituisce solo l'LLM di risposta per questo turno del canale vocale.
- `voice.tts` viene unito sopra `messages.tts`; i provider con supporto allo streaming alimentano direttamente il lettore, altrimenti il file audio risultante viene riprodotto nel canale a cui si è entrati.

Esempio di sessione predefinita del canale vocale agent-proxy:
...
Voce come estensione di una sessione canale Discord esistente:
...
Usalo quando il modello sente la propria riproduzione Discord attraverso un microfono aperto, ma vuoi comunque interromperlo parlando. OpenClaw impedisce a OpenAI di interrompere automaticamente su audio di input grezzo, mentre `bargeIn: true` consente agli eventi di avvio dell'altoparlante Discord e all'audio di un altoparlante già attivo di annullare le risposte realtime attive prima che il turno successivo catturato raggiunga OpenAI. I segnali di interruzione molto precoci con `audioEndMs` inferiore a `minBargeInAudioEndMs` vengono trattati come probabile eco/rumore e ignorati, così il modello non si interrompe al primo frame di riproduzione.
...
- Sull'audio dell'altoparlante: `discord voice: realtime speaker turn opened ...`, `discord voice: realtime input audio started ... outputAudioMs=... outputActive=...`, e `discord voice: realtime speaker turn closed ... chunks=... discordBytes=... realtimeBytes=... interruptedPlayback=...`
...
- All'arresto/reimpostazione della riproduzione: `discord voice: realtime audio playback stopped reason=... audioMs=... elapsedMs=... chunks=...`
...
- Su riproduzione inattiva: `discord voice: realtime barge-in ignored reason=... outputActive=false ... playbackChunks=0`

Per eseguire il debug dell'audio interrotto, leggi i log vocali realtime come una timeline:
...
2. `realtime speaker turn opened` indica che un altoparlante Discord è diventato attivo. Se la riproduzione è già attiva e `bargeIn` è abilitato, può essere seguito da `barge-in detected source=speaker-start`.
3. `realtime input audio started` indica il primo frame audio effettivo ricevuto per quel turno dell'altoparlante. `outputActive=true` o un valore `outputAudioMs` diverso da zero qui significa che il microfono sta inviando input mentre la riproduzione dell'assistente è ancora attiva.
4. `barge-in detected source=active-speaker-audio` significa che OpenClaw ha rilevato audio live dell'altoparlante mentre la riproduzione dell'assistente era attiva. Questo è utile per distinguere una vera interruzione da un evento di avvio dell'altoparlante Discord senza audio utile.
...
6. `realtime audio playback stopped reason=...` è il punto di reimpostazione della riproduzione Discord locale. Il motivo indica chi ha arrestato la riproduzione: `barge-in`, `player-idle`, `provider-clear-audio`, `forced-agent-consult`, `stream-close` o `session-close`.
7. `realtime speaker turn closed` riepiloga il turno di input catturato. `chunks=0` o `hasAudio=false` significa che il turno dell'altoparlante si è aperto ma nessun audio utilizzabile ha raggiunto il bridge realtime. `interruptedPlayback=true` significa che quel turno di input si è sovrapposto all'output dell'assistente e ha attivato la logica di interruzione.
...
- `elapsedMs`: tempo wall-clock tra l'apertura e la chiusura dello stream di riproduzione o del turno dell'altoparlante.
...
- `sinceLastAudioMs`: intervallo tra l'ultimo frame audio dell'altoparlante catturato e la chiusura del turno dell'altoparlante.

Schemi comuni:

- Interruzione immediata con `source=active-speaker-audio`, `outputAudioMs` ridotto e lo stesso utente nelle vicinanze di solito indica che l'eco dell'altoparlante entra nel microfono. Aumenta `voice.realtime.minBargeInAudioEndMs`, abbassa il volume dell'altoparlante, usa le cuffie o imposta `voice.realtime.providers.openai.interruptResponseOnInputAudio: false`.
- `source=speaker-start` seguito da `speaker turn closed ... hasAudio=false` significa che Discord ha segnalato l'avvio di un altoparlante, ma nessun audio ha raggiunto OpenClaw. Può trattarsi di un evento vocale Discord transitorio, del comportamento del noise gate o di un client che attiva brevemente il microfono.
- `audio playback stopped reason=stream-close` senza un'interruzione vicina o `provider-clear-audio` significa che lo stream di riproduzione Discord locale è terminato in modo imprevisto. Controlla i log precedenti del provider e del player Discord.
...
<AccordionGroup>
  ...
  <Accordion title="Messaggi del server bloccati in modo imprevisto">
    ...
    - verifica l'allowlist del server sotto `channels.discord.guilds`
...
```
  ...
  <Accordion title="Require mention false ma ancora bloccato">
    ...
    - `groupPolicy="allowlist"` senza un'allowlist del server/canale corrispondente
  ...
  <Accordion title="Turni Discord a lunga durata o risposte duplicate">
    ...
    Discord non applica un timeout gestito dal canale ai turni dell'agente in coda. I listener dei messaggi passano immediatamente il controllo, e le esecuzioni Discord in coda preservano l'ordinamento per sessione finché il ciclo di vita di sessione/strumento/runtime non completa o interrompe il lavoro.
...
```
  ...
  <Accordion title="Avvisi di timeout della ricerca dei metadati del Gateway">
    OpenClaw recupera i metadati Discord `/gateway/bot` prima della connessione. Gli errori transitori ricadono sull'URL del Gateway predefinito di Discord e sono sottoposti a limitazione di frequenza nei log.
  ...
  <Accordion title="Riavvii per timeout READY del Gateway">
    OpenClaw attende l'evento `READY` del Gateway Discord durante l'avvio e dopo le riconnessioni runtime. Le configurazioni multi-account con scaglionamento dell'avvio possono richiedere una finestra READY di avvio più lunga di quella predefinita.
  ...
  <Accordion title="Disallineamenti nell'audit delle autorizzazioni">
    ...
    Se usi chiavi slug, il matching runtime può comunque funzionare, ma probe non può verificare completamente le autorizzazioni.
  ...
  <Accordion title="Loop da bot a bot">
    ...
    Se imposti `channels.discord.allowBots=true`, usa regole rigorose per menzioni e allowlist per evitare comportamenti a ciclo.
...
```
  ...
  <Accordion title="Interruzioni dello STT vocale con DecryptionFailed(...)">

    - mantieni OpenClaw aggiornato (`openclaw update`) così è presente la logica di ripristino della ricezione vocale di Discord
    - conferma `channels.discord.voice.daveEncryption=true` (predefinito)
    - parti da `channels.discord.voice.decryptionFailureTolerance=24` (predefinito upstream) e modifica solo se necessario
    - controlla nei log:
...
<Accordion title="Campi Discord ad alta rilevanza">
...
- media/nuovi tentativi: `mediaMaxMb` (limita i caricamenti Discord in uscita, predefinito `100MB`), `retry`
- azioni: `actions.*`
...
- Concedi le autorizzazioni Discord con privilegi minimi.
- Se lo stato/la distribuzione dei comandi è obsoleto, riavvia il Gateway e ricontrolla con `openclaw channels status --probe`.
