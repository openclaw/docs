...
<Steps>
  <Step title="Crea un'applicazione Discord e un bot">
    ...
    Fai clic su **Bot** nella barra laterale. Imposta lo **Username** su qualunque nome usi per il tuo agente OpenClaw.
  ...
  <Step title="Consenti i DM dai membri del server">
    Per far funzionare l'associazione, Discord deve consentire al bot di inviarti DM. Fai clic con il tasto destro sull'**icona del server** → **Privacy Settings** → attiva **Direct Messages**.
...
```
    ...
    Per le installazioni di servizio gestito, esegui `openclaw gateway install` da una shell in cui `DISCORD_BOT_TOKEN` è presente, oppure memorizza la variabile in `~/.openclaw/.env`, così il servizio può risolvere la SecretRef env dopo il riavvio.
    Se il tuo host è bloccato o limitato da rate limit dalla ricerca dell'applicazione all'avvio di Discord, imposta l'ID applicazione/client Discord dal Developer Portal così l'avvio può saltare quella chiamata REST. Usa `channels.discord.applicationId` per l'account predefinito, oppure `channels.discord.accounts.<accountId>.applicationId` quando esegui più bot Discord.
...
```
        ...
        Per configurazioni scriptate o remote, scrivi lo stesso blocco JSON5 con `openclaw config patch --file ./discord.patch.json5 --dry-run` e poi riesegui senza `--dry-run`. I valori `token` in testo semplice sono supportati. I valori SecretRef sono supportati anche per `channels.discord.token` tra provider env/file/exec. Vedi [Gestione dei segreti](/it/gateway/secrets).

        Per più bot Discord, mantieni ogni token bot e ID applicazione sotto il relativo account. Un `channels.discord.applicationId` di livello superiore viene ereditato dagli account, quindi impostalo lì solo quando ogni account deve usare lo stesso ID applicazione.
...
<Note>
La risoluzione del token tiene conto dell'account. I valori del token nella configurazione prevalgono sul fallback env. `DISCORD_BOT_TOKEN` viene usato solo per l'account predefinito.
Se due account Discord abilitati risolvono allo stesso token bot, OpenClaw avvia un solo monitor Gateway per quel token. Un token proveniente dalla configurazione prevale sul fallback env predefinito; altrimenti prevale il primo account abilitato e l'account duplicato viene segnalato come disabilitato.
Per chiamate outbound avanzate (strumento message/azioni canale), un `token` esplicito per chiamata viene usato per quella chiamata. Questo si applica alle azioni di invio e di tipo lettura/probe (ad esempio read/search/fetch/thread/pins/permissions). Le impostazioni di policy/retry dell'account provengono comunque dall'account selezionato nello snapshot di runtime attivo.
</Note>

## Consigliato: configura uno spazio di lavoro guild

Una volta che i DM funzionano, puoi configurare il tuo server Discord come spazio di lavoro completo in cui ogni canale ottiene la propria sessione agente con il proprio contesto. È consigliato per server privati in cui ci siete solo tu e il tuo bot.
...
- I metadati di guild/canale Discord vengono aggiunti al prompt del modello come contesto non attendibile, non come prefisso di risposta visibile all'utente. Se un modello copia nuovamente quell'involucro, OpenClaw rimuove i metadati copiati dalle risposte in uscita e dal contesto di riproduzione futuro.
...
- La consegna degli annunci di cron/heartbeat solo testo a Discord usa una sola volta la risposta finale visibile all'assistente. I payload multimediali e dei componenti strutturati restano multi-messaggio quando l'agente emette più payload consegnabili.
...
OpenClaw supporta i contenitori componenti v2 di Discord per i messaggi degli agenti. Usa lo strumento messaggi con un payload `components`. I risultati delle interazioni vengono reinstradati all'agente come normali messaggi in ingresso e seguono le impostazioni Discord `replyToMode` esistenti.
...
<Tabs>
  <Tab title="Criterio DM">
    ...
    - Gli account con nome ereditano `channels.discord.allowFrom` quando il proprio `allowFrom` e il legacy `dm.allowFrom` non sono impostati.
    ...
    Gli ID numerici semplici normalmente vengono risolti come ID canale quando è attivo un valore predefinito di canale, ma gli ID elencati nell'`allowFrom` DM effettivo dell'account sono trattati come target DM utente per compatibilità.
...
```
    ...
    Un canale di testo Discord non ha un elenco membri separato. `type: "discord.channelAudience"` modella l'appartenenza così: il mittente del DM è membro della guild configurata e al momento dispone dell'autorizzazione effettiva `ViewChannel` sul canale configurato dopo l'applicazione dei ruoli e delle sovrascritture di canale.
...
```
    ...
    Abilita **Server Members Intent** nel Discord Developer Portal per il bot quando usi gruppi di accesso basati sull'audience del canale. I DM non includono lo stato dei membri della guild, quindi OpenClaw risolve il membro tramite REST Discord al momento dell'autorizzazione.
  ...
  <Tab title="Criterio guild">
    ...
    Il baseline sicuro quando `channels.discord` esiste è `allowlist`.

    Comportamento di `allowlist`:

    - la guild deve corrispondere a `channels.discord.guilds` (`id` preferito, slug accettato)
    - allowlist mittenti opzionali: `users` (ID stabili consigliati) e `roles` (solo ID ruolo); se una delle due è configurata, i mittenti sono consentiti quando corrispondono a `users` O `roles`
...
```
  ...
  <Tab title="Menzioni e DM di gruppo">
    ...
    Quando scrivi messaggi Discord in uscita, usa la sintassi di menzione canonica: `<@USER_ID>` per gli utenti, `<#CHANNEL_ID>` per i canali e `<@&ROLE_ID>` per i ruoli. Non usare la forma legacy di menzione nickname `<@!USER_ID>`.
...
Usa `bindings[].match.roles` per instradare i membri delle guild Discord a diversi agenti in base all'ID ruolo. I binding basati sui ruoli accettano solo ID ruolo e vengono valutati dopo i binding peer o parent-peer e prima dei binding solo guild. Se un binding imposta anche altri campi di corrispondenza (per esempio `peer` + `guildId` + `roles`), tutti i campi configurati devono corrispondere.
...
- I comandi possono comunque essere visibili nell'interfaccia Discord per utenti non autorizzati; l'esecuzione applica comunque l'autenticazione OpenClaw e restituisce "not authorized".

Vedi [Slash command](/it/tools/slash-commands) per il catalogo dei comandi e il comportamento.
...
<AccordionGroup>
  <Accordion title="Reply tags and native replies">
    ...
    `batched` collega il riferimento di risposta nativa implicita di Discord solo quando il
  ...
  <Accordion title="Live stream preview">
    OpenClaw può trasmettere risposte bozza inviando un messaggio temporaneo e modificandolo man mano che arriva il testo. `channels.discord.streaming` accetta `off` (predefinito) | `partial` | `block` | `progress`. `progress` mantiene una bozza di stato modificabile e la aggiorna con l'avanzamento degli strumenti fino alla consegna finale; `streamMode` è un alias runtime legacy. Esegui `openclaw doctor --fix` per riscrivere la configurazione persistita sulla chiave canonica.

    Il valore predefinito resta `off` perché le modifiche all'anteprima Discord raggiungono rapidamente i limiti di frequenza quando più bot o gateway condividono un account.
...
```
    ...
    - `block` emette blocchi delle dimensioni di una bozza (usa `draftChunk` per regolare dimensioni e punti di interruzione, limitati a `textChunkLimit`).
  ...
  <Accordion title="History, context, and thread behavior">
    ...
    - Le sessioni thread ereditano la selezione `/model` a livello di sessione del canale padre come fallback solo per il modello; le selezioni `/model` locali al thread hanno comunque precedenza e la cronologia della trascrizione del padre non viene copiata salvo che l'ereditarietà della trascrizione sia abilitata.
    - `channels.discord.thread.inheritParent` (predefinito `false`) abilita per i nuovi auto-thread il seed dalla trascrizione del padre. Gli override per account si trovano in `channels.discord.accounts.<id>.thread.inheritParent`.
    ...
    Gli argomenti dei canali vengono inseriti come contesto **non attendibile**. Le allowlist controllano chi può attivare l'agente, non sono un confine completo di redazione del contesto supplementare.
  ...
  <Accordion title="Thread-bound sessions for subagents">
    ...
    - `/session idle <duration|off>` ispeziona/aggiorna l'auto-unfocus per inattività delle associazioni con focus
    - `/session max-age <duration|off>` ispeziona/aggiorna l'età massima rigida delle associazioni con focus
...
```
    ...
    - `defaultSpawnContext` controlla il contesto nativo del subagente per gli spawn associati al thread. Predefinito: `"fork"`.
    ...
    Vedi [Subagenti](/it/tools/subagents), [Agenti ACP](/it/tools/acp-agents) e [Riferimento configurazione](/it/gateway/configuration-reference).

  </Accordion>

  <Accordion title="Persistent ACP channel bindings">
    Per workspace ACP stabili "sempre attivi", configura associazioni ACP tipizzate di primo livello indirizzate a conversazioni Discord.
...
```
    ...
    - `spawnSessions` controlla la creazione/associazione dei thread figli tramite `--thread auto|here`.
...
```
    ...
    - se la ricerca non riesce, i messaggi proxied vengono trattati come messaggi di bot e scartati salvo che `allowBots=true`

  </Accordion>

  <Accordion title="Outbound mention aliases">
    Usa `mentionAliases` quando gli agenti devono produrre menzioni in uscita deterministiche per utenti Discord noti. Le chiavi sono handle senza `@` iniziale; i valori sono ID utente Discord. Gli handle sconosciuti, `@everyone`, `@here` e le menzioni all'interno di span di codice Markdown restano invariati.
...
```
    ...
    La presenza automatica mappa la disponibilità in runtime allo stato Discord: integro => online, degradato o sconosciuto => inattivo, esaurito o non disponibile => dnd. Override di testo opzionali:
  ...
  <Accordion title="Approvazioni in Discord">
    Discord supporta la gestione delle approvazioni basata su pulsanti nei DM e può opzionalmente pubblicare richieste di approvazione nel canale di origine.
    ...
    Discord renderizza anche i pulsanti di approvazione condivisi usati da altri canali chat. L'adapter Discord nativo aggiunge principalmente routing DM degli approvatori e fanout sul canale.
    ...
    runtime è attivo ma una scheda nativa non può essere consegnata a nessuna destinazione,
    OpenClaw invia un avviso di fallback nella stessa chat con il comando `/approve`
    esatto dall'approvazione in sospeso.

    L'autenticazione Gateway e la risoluzione delle approvazioni seguono il contratto condiviso del client Gateway (gli ID `plugin:` vengono risolti tramite `plugin.approval.resolve`; gli altri ID tramite `exec.approval.resolve`). Le approvazioni scadono dopo 30 minuti per impostazione predefinita.
...
L'azione `event-create` accetta un parametro opzionale `image` (URL o percorso file locale) per impostare l'immagine di copertina dell'evento programmato.
...
- I turni della trascrizione vocale derivano lo stato proprietario da `allowFrom` di Discord (o `dm.allowFrom`); i parlanti non proprietari non possono accedere agli strumenti riservati al proprietario (per esempio `gateway` e `cron`).
...
- `channels.discord.intents.voiceStates` può sovrascrivere esplicitamente la sottoscrizione all'intent voice-state. Lascialo non impostato perché l'intent segua l'abilitazione vocale effettiva.
- `voice.daveEncryption` e `voice.decryptionFailureTolerance` vengono passati alle opzioni join di `@discordjs/voice`.
...
- OpenClaw osserva anche gli errori di decifrazione in ricezione e si ripristina automaticamente uscendo e rientrando nel canale vocale dopo errori ripetuti in una finestra breve.
- Se i log di ricezione mostrano ripetutamente `DecryptionFailed(UnencryptedWhenPassthroughDisabled)` dopo l'aggiornamento, raccogli un report delle dipendenze e i log. La linea `@discordjs/voice` in bundle include la correzione upstream del padding dalla PR discord.js #11449, che ha chiuso l'issue discord.js #11419.
...
```
  ...
  <Accordion title="Turni Discord a lunga esecuzione o risposte duplicate">
    ...
    Discord non applica un timeout di proprietà del canale ai turni agente in coda. I listener dei messaggi passano il controllo immediatamente, e le esecuzioni Discord in coda preservano l'ordinamento per sessione finché il ciclo di vita di sessione/strumento/runtime completa o interrompe il lavoro.
...
```
  ...
  <Accordion title="Avvisi di timeout per la ricerca dei metadati del Gateway">
    OpenClaw recupera i metadati `/gateway/bot` di Discord prima della connessione. Gli errori transitori ripiegano sull'URL del Gateway predefinito di Discord e sono limitati nella frequenza nei log.
  ...
  <Accordion title="Riavvii per timeout READY del Gateway">
    OpenClaw attende l'evento `READY` del Gateway di Discord durante l'avvio e dopo le riconnessioni a runtime. Le configurazioni con più account e avvio scaglionato possono richiedere una finestra READY di avvio più lunga di quella predefinita.

    Parametri di timeout READY:

    - avvio con account singolo: `channels.discord.gatewayReadyTimeoutMs`
    - avvio con più account: `channels.discord.accounts.<accountId>.gatewayReadyTimeoutMs`
    ...
    - runtime con più account: `channels.discord.accounts.<accountId>.gatewayRuntimeReadyTimeoutMs`
  ...
  <Accordion title="Mancate corrispondenze nell'audit delle autorizzazioni">
    ...
    Se usi chiavi slug, la corrispondenza a runtime può comunque funzionare, ma probe non può verificare completamente le autorizzazioni.
  ...
  <Accordion title="Loop da bot a bot">
    ...
    Se imposti `channels.discord.allowBots=true`, usa regole rigorose per menzioni e allowlist per evitare comportamenti a loop.
...
```
  ...
  <Accordion title="Cadute dello STT vocale con DecryptionFailed(...)">

    - mantieni OpenClaw aggiornato (`openclaw update`) in modo che la logica di ripristino della ricezione vocale di Discord sia presente
...
- Concedi le autorizzazioni Discord con privilegio minimo.
- Se lo stato o la distribuzione dei comandi è obsoleto, riavvia il Gateway e ricontrolla con `openclaw channels status --probe`.
...
<CardGroup cols={2}>
  ...
  <Card title="Gruppi" icon="users" href="/it/channels/groups">
    Comportamento delle chat di gruppo e dell'allowlist.
  </Card>
