...
title: Feishu
x-i18n:
    generated_at: "2026-05-06T08:39:39Z"
    model: gpt-5.5
    provider: openai
...
- `"allowlist"` - solo gli utenti elencati in `allowFrom` possono chattare (impostazione predefinita: solo il proprietario del bot)
...
- `@all` e `@_all` solo broadcast non sono trattati come menzioni del bot. Un messaggio che menziona sia `@all` sia direttamente il bot viene comunque conteggiato come menzione del bot.
...
In modalità `allowlist`, puoi anche ammettere un gruppo aggiungendo una voce esplicita `groups.<chat_id>`. Le voci esplicite non sovrascrivono `groupPolicy: "disabled"`. Le impostazioni predefinite con carattere jolly in `groups.*` configurano i gruppi corrispondenti, ma da sole non ammettono gruppi.
...
## Ottieni gli ID di gruppi/utenti
...
`accounts.<id>.tts` usa la stessa forma di `messages.tts` ed esegue un deep merge sulla
configurazione TTS globale, quindi le configurazioni Feishu multi-bot possono mantenere le credenziali
...
Imposta `streaming: false` per inviare la risposta completa in un unico messaggio. `blockStreaming` è disattivato per impostazione predefinita; abilitalo solo quando vuoi inviare i blocchi completati dell'assistente prima della risposta finale.
...
Feishu/Lark supporta ACP per DM e messaggi di thread di gruppo. ACP di Feishu/Lark è basato su comandi testuali: non ci sono menu nativi per comandi slash, quindi usa i messaggi `/acp ...` direttamente nella conversazione.
...
`--thread here` funziona per DM e messaggi di thread Feishu/Lark. I messaggi successivi nella conversazione associata vengono instradati direttamente a quella sessione ACP.
...
Vedi [Ottieni gli ID di gruppi/utenti](#get-groupuser-ids) per suggerimenti sulla ricerca.
...
- ⚠️ Rich text (formattazione in stile post; non supporta tutte le funzionalità di authoring Feishu/Lark)

I fumetti audio nativi Feishu/Lark usano il tipo di messaggio Feishu `audio` e richiedono
...
vengono transcodificati in Ogg/Opus a 48 kHz con `ffmpeg` solo quando la risposta richiede
la consegna vocale (`audioAsVoice` / strumento messaggi `asVoice`, incluse le risposte
con nota vocale TTS). Gli allegati MP3 ordinari restano file normali. Se `ffmpeg` manca o
...
Feishu/Lark usano l'evento `thread_id` (`omt_*`) come chiave canonica della sessione
topic. Se un evento starter di topic nativo omette `thread_id`, OpenClaw lo idrata da
Feishu prima di effettuare il routing del turno. Le normali risposte di gruppo che
OpenClaw trasforma in thread continuano a usare l'ID del messaggio radice della risposta
(`om_*`), così il primo turno e il turno successivo restano nella stessa sessione.
