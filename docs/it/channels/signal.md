...
summary: Supporto a Signal tramite signal-cli (JSON-RPC + SSE), percorsi di configurazione e modello dei numeri
title: Signal
x-i18n:
    generated_at: "2026-05-06T08:41:18Z"
    model: gpt-5.5
    provider: openai
...
- OpenClaw installato sul tuo server (il flusso Linux seguente è stato testato su Ubuntu 24).
...
- Canale Signal tramite `signal-cli` (non libsignal integrato).
...
- Per "mando un messaggio al bot e lui risponde", usa un **numero bot separato**.
...
Supporto multi-account: usa `channels.signal.accounts` con configurazione per account e `name` opzionale. Consulta [`gateway/configuration`](/it/gateway/config-channels#multi-account-all-channels) per il modello condiviso.
...
Mantieni `signal-cli` aggiornato; upstream segnala che le versioni vecchie possono smettere di funzionare quando cambiano le API server di Signal.
...
3. Esegui dalla stessa IP esterna della sessione browser quando possibile.
...
<Warning>
Registrare un account con numero di telefono tramite `signal-cli` può deautenticare la sessione principale dell'app Signal per quel numero. Preferisci un numero bot dedicato, oppure usa la modalità collegamento QR se devi mantenere la configurazione dell'app telefonica esistente.
</Warning>
...
Questo evita l'avvio automatico e l'attesa di startup all'interno di OpenClaw. Per avvii lenti con avvio automatico, imposta `channels.signal.startupTimeoutMs`.
...
- L'inserimento di un gruppo Signal in allowlist tramite `groupAllowFrom` non disattiva di per sé il gating delle menzioni. Una voce `channels.signal.groups["<group-id>"]` configurata in modo specifico elabora ogni messaggio del gruppo a meno che `requireMention=true` sia impostato.
- Nota runtime: se `channels.signal` manca del tutto, il runtime ripiega su `groupPolicy="allowlist"` per i controlli dei gruppi (anche se `channels.defaults.groupPolicy` è impostato).

## Come funziona (comportamento)

- `signal-cli` viene eseguito come daemon; il gateway legge gli eventi tramite SSE.
- I messaggi in ingresso vengono normalizzati nella busta canale condivisa.
...
- Suddivisione opzionale sulle nuove righe: imposta `channels.signal.chunkMode="newline"` per suddividere sulle righe vuote (confini di paragrafo) prima della suddivisione per lunghezza.
- Allegati supportati (base64 recuperato da `signal-cli`).
- Gli allegati di note vocali usano il nome file `signal-cli` come fallback MIME quando `contentType` manca, così la trascrizione audio può comunque classificare i memo vocali AAC.
...
- Il contesto della cronologia dei gruppi usa `channels.signal.historyLimit` (o `channels.signal.accounts.*.historyLimit`), con fallback a `messages.groupChat.historyLimit`. Imposta `0` per disattivare (predefinito 50).
...
- Destinazioni: E.164 o UUID del mittente (usa `uuid:<id>` dall'output dell'associazione; funziona anche l'UUID semplice).
...
- `channels.signal.reactionLevel`: `off | ack | minimal | extensive`.
  - `off`/`ack` disattiva le reazioni dell'agente (lo strumento messaggio `react` genererà errore).
...
Poi conferma lo stato di associazione DM se necessario:
...
- DM ignorati: il mittente è in attesa dell'approvazione dell'associazione.
- Messaggi di gruppo ignorati: il gating del mittente gruppo/menzione blocca la consegna.
...
- La verifica SMS è necessaria solo per flussi di registrazione o recupero, ma perdere il controllo del numero/account può complicare una nuova registrazione.
...
- `channels.signal.allowFrom`: elenco consentiti per DM (E.164 o `uuid:<id>`). `open` richiede `"*"`. Signal non ha nomi utente; usa ID telefono/UUID.
- `channels.signal.groupPolicy`: `open | allowlist | disabled` (predefinito: allowlist).
- `channels.signal.groupAllowFrom`: elenco consentiti per gruppi; accetta ID gruppo Signal (raw, `group:<id>` o `signal:group:<id>`), numeri E.164 del mittente o valori `uuid:<id>`.
