...
summary: Configurazione e procedura di avvio iniziale opzionali basate su Docker per OpenClaw
title: Docker
x-i18n:
    generated_at: "2026-05-06T08:56:09Z"
    model: gpt-5.5
    provider: openai
...
e scritture della configurazione durante la fase di setup tramite `openclaw-gateway` con
...
| `OTEL_EXPORTER_OTLP_ENDPOINT`              | Endpoint condiviso del collector OTLP/HTTP per l'esportazione OpenTelemetry |
...
localmente e vuoi rendere disponibile nell'immagine l'exporter OpenTelemetry in bundle,
includi le sue dipendenze di runtime:
...
Per insidie e risoluzione dei problemi, vedi [Rilevamento Bonjour](/it/gateway/bonjour).
...
- `.env` per segreti di runtime basati su env come `OPENCLAW_GATEWAY_TOKEN`

I Plugin scaricabili installati salvano il loro stato pacchetto sotto la home
OpenClaw montata, quindi i record di installazione Plugin e le root dei pacchetti sopravvivono alla
sostituzione del container. L'avvio del Gateway non genera alberi di dipendenze per i Plugin in bundle.
...
Consulta [Hetzner (VPS Docker)](/it/install/hetzner) e
[Runtime VM Docker](/it/install/docker-vm-runtime) per i passaggi di distribuzione su VM condivisa,
inclusi incorporazione dei binari, persistenza e aggiornamenti.
...
- [Sandboxing](/it/gateway/sandboxing) -- riferimento completo per la sandbox
...
Per installazioni npm senza checkout dei sorgenti, consulta [Sandboxing § Immagini e configurazione](/it/gateway/sandboxing#images-and-setup) per i comandi `docker build` inline.
...
<AccordionGroup>
  <Accordion title="Immagine mancante o container sandbox che non si avvia">
    ...
    (checkout dei sorgenti) o il comando `docker build` inline da [Sandboxing § Immagini e configurazione](/it/gateway/sandboxing#images-and-setup) (installazione npm),
  ...
  <Accordion title="Il target del Gateway mostra ws://172.x.x.x o errori di abbinamento dalla CLI Docker">
