...
read_when:
    ...
    - Vuoi configurare il componente server QMD
...
x-i18n:
    generated_at: "2026-05-02T08:33:52Z"
    model: gpt-5.5
    provider: openai
...
Questa pagina elenca ogni impostazione di configurazione per la ricerca nella memoria di OpenClaw. Per panoramiche concettuali, vedi:
...
Tutte le impostazioni di ricerca nella memoria si trovano sotto `agents.defaults.memorySearch` in `openclaw.json`, salvo diversa indicazione.

<Note>
Se cerchi l'interruttore della funzionalità **Active Memory** e la configurazione del sotto-agente, si trovano invece sotto `plugins.entries.active-memory`, non sotto `memorySearch`.

Active Memory usa un modello a due gate:

1. il Plugin deve essere abilitato e deve avere come destinazione l'id dell'agente corrente
2. la richiesta deve essere una sessione di chat persistente interattiva idonea

Vedi [Active Memory](/it/concepts/active-memory) per il modello di attivazione, la configurazione di proprietà del Plugin, la persistenza della trascrizione e il pattern di rollout sicuro.
</Note>
...
<Steps>
  ...
  <Step title="bedrock">
    Selezionato se la catena di credenziali dell'AWS SDK viene risolta (ruolo dell'istanza, chiavi di accesso, profilo, SSO, identità web o configurazione condivisa).
  </Step>
</Steps>
...
`memorySearch.provider` può puntare a una voce `models.providers.<id>` personalizzata. OpenClaw risolve il proprietario `api` di quel provider per l'adapter di embedding, preservando al tempo stesso l'id provider personalizzato per gestione di endpoint, autenticazione e prefisso del modello. Questo permette a configurazioni multi-GPU o multi-host di dedicare gli embedding di memoria a uno specifico endpoint locale:
...
<AccordionGroup>
  ...
  <Accordion title="OpenAI-compatible input types">
    Gli endpoint di embedding compatibili con OpenAI possono aderire ai campi di richiesta `input_type` specifici del provider. Questo è utile per modelli di embedding asimmetrici che richiedono etichette diverse per embedding di query e documenti.
  ...
  <Accordion title="Bedrock">
    ...
    **Autenticazione:** l'autenticazione Bedrock usa l'ordine standard di risoluzione delle credenziali dell'AWS SDK:
    ...
    2. Cache token SSO
    3. Credenziali token identità web
    4. File condivisi di credenziali e configurazione
    5. Credenziali metadati ECS o EC2

    La regione viene risolta da `AWS_REGION`, `AWS_DEFAULT_REGION`, dal `baseUrl` del provider `amazon-bedrock` oppure, come predefinito, da `us-east-1`.

    **Permessi IAM:** il ruolo o utente IAM necessita di:
  ...
  <Accordion title="Local (GGUF + node-llama-cpp)">
    ...
    | `local.contextSize`   | `number \| "auto"` | `4096`                 | Dimensione della finestra di contesto per il contesto di embedding. 4096 copre i chunk tipici (128-512 token) limitando la VRAM non usata dai pesi. Riduci a 1024-2048 sugli host vincolati. `"auto"` usa il massimo addestrato del modello, non consigliato per modelli 8B+ (Qwen3-Embedding-8B: 40 960 token -> ~32 GB di VRAM contro ~8,8 GB a 4096). |

    Modello predefinito: `embeddinggemma-300m-qat-Q8_0.gguf` (~0,6 GB, scaricato automaticamente). I checkout del sorgente richiedono comunque l'approvazione della build nativa: `pnpm approve-builds` poi `pnpm rebuild node-llama-cpp`.
...
Se non impostato, usa il valore predefinito del provider: 600 secondi per provider locali/self-hosted come `local`, `ollama` e `lmstudio`, e 120 secondi per provider in hosting. Aumenta questo valore quando i batch di embedding locali vincolati dalla CPU funzionano correttamente ma sono lenti.
</ParamField>
...
<Tabs>
  ...
  <Tab title="Temporal decay (recency)">
    ...
    I file sempreverdi (`MEMORY.md`, file senza data in `memory/`) non subiscono mai decadimento.
...
Per la ricerca nelle trascrizioni tra agenti con ambito agente, usa `agents.list[].memorySearch.qmd.extraCollections` invece di `memory.qmd.paths`. Queste raccolte extra seguono la stessa forma `{ path, name, pattern? }`, ma vengono unite per agente e possono preservare nomi condivisi espliciti quando il percorso punta fuori dal workspace corrente. Se lo stesso percorso risolto appare sia in `memory.qmd.paths` sia in `memorySearch.qmd.extraCollections`, QMD mantiene la prima voce e salta il duplicato.
...
È separato da `sync.embeddingBatchTimeoutSeconds`, che controlla il timeout per le chiamate di embedding inline.
...
| `store.vector.extensionPath` | `string`  | bundled     | Sostituisci il percorso di sqlite-vec |
...
`searchMode: "search"` è solo lessicale/BM25. OpenClaw non esegue sonde di disponibilità dei vettori semantici né manutenzione degli embedding QMD per quella modalità, anche durante `memory status --deep`; `vsearch` e `query` continuano a richiedere disponibilità dei vettori QMD ed embedding.

OpenClaw preferisce la raccolta QMD corrente e le forme di query MCP attuali, ma mantiene funzionanti le versioni QMD precedenti provando flag di pattern di raccolta compatibili e nomi di strumenti MCP precedenti quando necessario. Quando QMD dichiara il supporto per più filtri di raccolta, le raccolte della stessa origine vengono cercate con un unico processo QMD; le build QMD precedenti mantengono il percorso di compatibilità per raccolta. Stessa origine significa che le raccolte di memoria durevole vengono raggruppate insieme, mentre le raccolte delle trascrizioni di sessione restano un gruppo separato, così la diversificazione delle fonti continua ad avere entrambi gli input.

<Note>
Le sostituzioni dei modelli QMD restano sul lato QMD, non nella configurazione di OpenClaw. Se devi sostituire globalmente i modelli di QMD, imposta variabili d'ambiente come `QMD_EMBED_MODEL`, `QMD_RERANK_MODEL` e `QMD_GENERATE_MODEL` nell'ambiente di runtime del gateway.
</Note>
...
<AccordionGroup>
  <Accordion title="Pianificazione degli aggiornamenti">
    ...
    | `update.onBoot`           | `boolean` | `true`      | Aggiorna quando si apre il gestore QMD a lunga durata; controlla anche l'aggiornamento all'avvio con consenso esplicito |
    ...
    | `update.waitForBootSync`  | `boolean` | `false`     | Blocca l'apertura del gestore finché l'aggiornamento iniziale non viene completato |
    | `update.embedInterval`    | `string`  | --          | Cadenza separata degli embedding      |
  ...
  <Accordion title="Ambito">
    ...
    Il predefinito distribuito consente sessioni dirette e di canale, pur continuando a negare i gruppi.

    Il predefinito è solo DM. `match.keyPrefix` corrisponde alla chiave di sessione normalizzata; `match.rawKeyPrefix` corrisponde alla chiave grezza, incluso `agent:<id>:`.
  ...
  <Accordion title="Citazioni">
    ...
    | ---------------- | ------------------------------------------------- |
    | `auto` (default) | Include il piè di pagina `Source: <path#line>` negli snippet |
...
Gli aggiornamenti di avvio QMD usano un percorso subprocess monouso durante l'avvio del gateway. Il gestore QMD a lunga durata mantiene comunque la proprietà del normale file watcher e dei timer di intervallo quando la ricerca in memoria viene aperta per uso interattivo.
...
Dreaming viene eseguito come un'unica scansione pianificata e usa fasi interne light/deep/REM come dettaglio di implementazione.
...
| `model`     | `string`  | modello predefinito | Sostituzione opzionale del modello del subagente Dream Diary |
...
- `dreaming.model` usa il gate di attendibilità esistente del subagente del plugin; imposta `plugins.entries.memory-core.subagent.allowModelOverride: true` prima di abilitarlo.
...
- La policy e le soglie delle fasi light/deep/REM sono comportamento interno, non configurazione destinata all'utente.
