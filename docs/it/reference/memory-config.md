...
summary: Tutte le opzioni di configurazione per la ricerca nella memoria, i fornitori di incorporamenti, QMD, la ricerca ibrida e l'indicizzazione multimodale
title: Riferimento per la configurazione della memoria
x-i18n:
    generated_at: "2026-04-30T16:30:07Z"
    model: gpt-5.5
    provider: openai
...
Questa pagina elenca ogni opzione di configurazione per la ricerca della memoria di OpenClaw. Per panoramiche concettuali, vedi:
...
<CardGroup cols={2}>
  ...
  <Card title="Ricerca della memoria" href="/it/concepts/memory-search">
...
Tutte le impostazioni di ricerca della memoria si trovano sotto `agents.defaults.memorySearch` in `openclaw.json`, salvo diversa indicazione.

<Note>
Se stai cercando l'interruttore della funzionalità **Active Memory** e la configurazione del sotto-agente, si trova invece sotto `plugins.entries.active-memory`, non sotto `memorySearch`.
...
Vedi [Active Memory](/it/concepts/active-memory) per il modello di attivazione, la configurazione gestita dal plugin, la persistenza della trascrizione e il modello di distribuzione sicura.
</Note>
...
| ---------- | --------- | ----------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `provider` | `string`  | rilevato automaticamente | ID adattatore di embedding come `bedrock`, `deepinfra`, `gemini`, `github-copilot`, `local`, `mistral`, `ollama`, `openai` o `voyage`; può anche essere un `models.providers.<id>` configurato il cui `api` punta a uno di questi adattatori |
| `model`    | `string`  | predefinito del provider | Nome del modello di embedding                                                                                                                                                                                                                |
| `fallback` | `string`  | `"none"`                | ID adattatore di fallback quando il primario non riesce                                                                                                                                                                                      |
| `enabled`  | `boolean` | `true`                  | Abilita o disabilita la ricerca della memoria                                                                                                                                                                                                |
...
<Steps>
  ...
  <Step title="bedrock">
    Selezionato se la catena di credenziali dell'AWS SDK viene risolta (ruolo di istanza, chiavi di accesso, profilo, SSO, identità web o configurazione condivisa).
  </Step>
</Steps>

`ollama` è supportato ma non rilevato automaticamente (impostalo esplicitamente).

### ID provider personalizzati

`memorySearch.provider` può puntare a una voce `models.providers.<id>` personalizzata. OpenClaw risolve il proprietario `api` di quel provider per l'adattatore di embedding, preservando al contempo l'id provider personalizzato per la gestione di endpoint, autenticazione e prefisso del modello. Questo consente a configurazioni multi-GPU o multi-host di dedicare gli embedding della memoria a uno specifico endpoint locale:
...
Gli embedding remoti richiedono una chiave API. Bedrock usa invece la catena di credenziali predefinita dell'AWS SDK (ruoli di istanza, SSO, chiavi di accesso).
...
| GitHub Copilot | `COPILOT_GITHUB_TOKEN`, `GH_TOKEN`, `GITHUB_TOKEN` | Profilo di autenticazione tramite accesso dispositivo |
...
<AccordionGroup>
  ...
  <Accordion title="Tipi di input compatibili con OpenAI">
    Gli endpoint di embedding compatibili con OpenAI possono usare campi di richiesta `input_type` specifici del provider. È utile per modelli di embedding asimmetrici che richiedono etichette diverse per gli embedding di query e documenti.
  ...
  <Accordion title="Bedrock">
    Bedrock usa la catena di credenziali predefinita dell'AWS SDK: non servono chiavi API. Se OpenClaw viene eseguito su EC2 con un ruolo di istanza abilitato per Bedrock, imposta semplicemente il provider e il modello:
    ...
    **Autorizzazioni IAM:** il ruolo o l'utente IAM necessita di:
    ...
    Per il privilegio minimo, limita `InvokeModel` al modello specifico:
  ...
  <Accordion title="Locale (GGUF + node-llama-cpp)">
    ...
    | `local.contextSize`   | `number \| "auto"` | `4096`                 | Dimensione della finestra di contesto per il contesto di embedding. 4096 copre i chunk tipici (128-512 token) limitando al contempo la VRAM non relativa ai pesi. Riduci a 1024-2048 su host con risorse limitate. `"auto"` usa il massimo addestrato del modello: non consigliato per modelli 8B+ (Qwen3-Embedding-8B: 40 960 token -> ~32 GB di VRAM rispetto a ~8,8 GB a 4096). |

    Modello predefinito: `embeddinggemma-300m-qat-Q8_0.gguf` (~0,6 GB, scaricato automaticamente). Le installazioni pacchettizzate riparano il runtime nativo `node-llama-cpp` tramite le dipendenze di runtime gestite del Plugin quando `provider: "local"` è configurato. I checkout sorgente richiedono comunque l'approvazione della build nativa: `pnpm approve-builds` quindi `pnpm rebuild node-llama-cpp`.
    ...
    Se `provider` è `auto`, `local` viene selezionato solo quando `local.modelPath` punta a un file locale esistente. I riferimenti a modelli `hf:` e HTTP(S) possono comunque essere usati esplicitamente con `provider: "local"`, ma non fanno sì che `auto` selezioni il locale prima che il modello sia disponibile su disco.
...
<Tabs>
  ...
  <Tab title="Decadimento temporale (recenza)">
    ...
    I file sempreverdi (`MEMORY.md`, file senza data in `memory/`) non vengono mai decaduti.
...
I percorsi possono essere assoluti o relativi al workspace. Le directory vengono analizzate ricorsivamente per file `.md`. La gestione dei symlink dipende dal backend attivo: il motore integrato ignora i symlink, mentre QMD segue il comportamento dello scanner QMD sottostante.

Per la ricerca di trascrizioni tra agenti con ambito agente, usa `agents.list[].memorySearch.qmd.extraCollections` invece di `memory.qmd.paths`. Queste raccolte aggiuntive seguono la stessa forma `{ path, name, pattern? }`, ma vengono unite per agente e possono preservare nomi condivisi espliciti quando il percorso punta fuori dal workspace corrente. Se lo stesso percorso risolto appare sia in `memory.qmd.paths` sia in `memorySearch.qmd.extraCollections`, QMD mantiene la prima voce e salta il duplicato.
...
Evita di ricalcolare gli embedding del testo invariato durante la reindicizzazione o gli aggiornamenti delle trascrizioni.
...
`remote.nonBatchConcurrency` controlla le chiamate di embedding inline usate dai provider locali/self-hosted e dai provider hosted quando le API batch del provider non sono attive. Ollama usa per impostazione predefinita `1` per l'indicizzazione non batch per evitare di sovraccaricare gli host locali più piccoli; imposta un valore più alto su macchine più grandi.
...
Indicizza le trascrizioni di sessione e le espone tramite `memory_search`:
...
`searchMode: "search"` è solo lessicale/BM25. OpenClaw non esegue probe di prontezza vettoriale semantica né manutenzione degli embedding QMD per quella modalità, anche durante `memory status --deep`; `vsearch` e `query` continuano a richiedere la prontezza vettoriale QMD e gli embedding.

OpenClaw preferisce la raccolta QMD corrente e le forme di query MCP correnti, ma mantiene funzionanti le versioni QMD precedenti provando flag di pattern di raccolta compatibili e nomi di strumenti MCP precedenti quando necessario. Quando QMD dichiara il supporto per più filtri di raccolta, le raccolte della stessa origine vengono cercate con un unico processo QMD; le build QMD precedenti mantengono il percorso di compatibilità per raccolta. Stessa origine significa che le raccolte di memoria durevole sono raggruppate insieme, mentre le raccolte di trascrizioni delle sessioni restano un gruppo separato, così la diversificazione delle fonti continua ad avere entrambi gli input.
...
<AccordionGroup>
  <Accordion title="Pianificazione degli aggiornamenti">
    ...
    | `update.onBoot`           | `boolean` | `true`      | Aggiorna quando il gestore QMD a lunga durata si apre; controlla anche l'aggiornamento all'avvio con opt-in |
  ...
  <Accordion title="Ambito">
    ...
    Il valore predefinito distribuito consente sessioni dirette e di canale, pur negando ancora i gruppi.
...
Gli aggiornamenti di avvio QMD usano un percorso di sottoprocesso una tantum durante l'avvio del gateway. Il gestore QMD a lunga durata continua a possedere il normale watcher dei file e i timer a intervallo quando la ricerca in memoria viene aperta per l'uso interattivo.
...
- `dreaming.model` usa il trust gate del subagente Plugin esistente; imposta `plugins.entries.memory-core.subagent.allowModelOverride: true` prima di abilitarlo.
- Dream Diary riprova una volta con il modello predefinito della sessione quando il modello configurato non è disponibile. Gli errori di trust o allowlist vengono registrati e non vengono riprovati silenziosamente.
