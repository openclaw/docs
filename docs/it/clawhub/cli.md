...
il proxy specificato. `HTTPS_PROXY` è usato per le richieste HTTPS, `HTTP_PROXY`
per HTTP semplice. `NO_PROXY` / `no_proxy` è rispettato per bypassare il proxy per
host o domini specifici.

Questo è necessario sui sistemi in cui le connessioni dirette in uscita sono bloccate
(ad es. container Docker, VPS Hetzner con accesso a Internet solo tramite proxy, firewall
aziendali).
...
- I download sono un piccolo indicatore preliminare di popolarità, non una garanzia di posizionamento in alto.
...
- `--owner <handle>` pubblica con l'handle publisher di un'organizzazione/utente quando
  l'attore dispone dell'accesso publisher.
- `--migrate-owner` sposta una skill esistente su `--owner` durante la pubblicazione di una nuova
  versione. Richiede accesso amministratore/proprietario su entrambi i publisher.
...
- Lo slug di origine non viene più elencato pubblicamente e diventa un alias di reindirizzamento alla destinazione.
- Chiama `POST /api/v1/skills/{sourceSlug}/merge`.
- `--yes` salta la conferma.

### `transfer`

- Workflow di trasferimento della proprietà.
- I trasferimenti verso handle utente creano una richiesta in sospeso che il destinatario accetta.
- I trasferimenti verso handle di organizzazione/publisher vengono applicati immediatamente solo quando l'attore dispone
  dell'accesso amministratore sia al proprietario corrente sia al publisher di destinazione.
- Sottocomandi:
...
- Per gli artefatti ClawPack npm-pack, verifica anche l'integrità npm `sha512`,
  lo shasum npm e il nome/versione di `package.json` del tarball.
...
- Richiede l'accesso admin sia al proprietario attuale del pacchetto sia al publisher
...
- Verifica se un pacchetto è pronto per il futuro utilizzo da parte di OpenClaw.
...
- Segnala i blocchi per lo stato ufficiale, la disponibilità di ClawPack, il digest dell'artefatto,
  la provenienza del sorgente, la compatibilità OpenClaw, i target host, i metadati dell'ambiente
...
- Chiama lo stesso endpoint di readiness calcolata usato da `package readiness`, ma stampa
...
- I metadati vengono rilevati automaticamente da `package.json`, `openclaw.plugin.json` e
  marker reali di bundle OpenClaw come `.codex-plugin/plugin.json`,
...
- Le sorgenti `.tgz` vengono trattate come ClawPack. La CLI carica i byte npm-pack esatti
  e usa i contenuti estratti di `package/` solo per la convalida e il precompilamento
  dei metadati.
...
- I plugin di codice esterni devono dichiarare esplicitamente `openclaw.compat.pluginApi` e
  ...
  `package.json.version` di primo livello non viene usato come fallback per la convalida di pubblicazione.
...
- `--owner <handle>` pubblica sotto l'handle di publisher di un utente o di un'organizzazione quando l'attore ha accesso al publisher.
- `--clawscan-note <text>` aggiunge una nota ClawScan. Questa nota fornisce a ClawScan
  contesto per comportamenti che altrimenti potrebbero sembrare insoliti, come accesso alla rete,
  accesso all'host nativo o credenziali specifiche del provider. La nota viene salvata sulla
  release pubblicata.
...
- `pull_request` deve usare `dry_run: true` così la CI non produce effetti persistenti.
- Le pubblicazioni reali devono essere limitate a eventi attendibili come `workflow_dispatch` o push di tag.
- La pubblicazione attendibile senza secret funziona solo su `workflow_dispatch`; i push di tag richiedono comunque `clawhub_token`.
...
- Analizza le cartelle Skills locali e pubblica quelle nuove/modificate.
- Le root possono essere qualsiasi cartella: una directory Skills o una singola cartella Skill con `SKILL.md`.
- Aggiunge automaticamente le root Skills di Clawdbot quando `~/.clawdbot/clawdbot.json` è presente:
...
- Flag:
  - `--root <dir...>` root di scansione aggiuntive
...
- Inviata durante `sync` quando si è effettuato l'accesso, a meno che `CLAWHUB_DISABLE_TELEMETRY=1` (legacy `CLAWDHUB_DISABLE_TELEMETRY=1`).
