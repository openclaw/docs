...
summary: 'Riferimento della CLI: comandi, opzioni, configurazione, file di lock, comportamento di sincronizzazione.'
x-i18n:
...
La CLI rispetta le variabili di ambiente standard per proxy HTTP per i sistemi dietro
...
Quando una qualsiasi di queste variabili è impostata, la CLI instrada le richieste in uscita tramite
...
- I download sono un piccolo criterio preliminare di popolarità, non una garanzia di posizionamento in alto.
...
- Rifiuta di sovrascrivere le skill bloccate a una versione; esegui prima `clawhub unpin <slug>`.
- Scrive:
...
- `--migrate-owner` sposta una skill esistente a `--owner` mentre pubblica una nuova
...
- Le eliminazioni temporanee avviate dall'owner riservano lo slug per 30 giorni; il comando stampa l'orario di scadenza.
...
- Rende di nuovo visibile una skill (owner, moderatore o admin).
- Alias per `undelete`.
...
- Usalo per metadati del Plugin, compatibilità, verifica, sorgente e ispezione di versione/file.
...
- Verifica SHA-256 ClawHub per tutti gli artefatti.
...
- Richiede l'accesso di amministratore sia al proprietario attuale del pacchetto sia all'editore
  di destinazione, salvo che l'operazione sia eseguita da un amministratore della piattaforma.
...
- Le segnalazioni sono a livello di pacchetto, possono essere associate facoltativamente a una versione e diventano visibili
...
- Riporta i blocchi relativi allo stato ufficiale, alla disponibilità di ClawPack, al digest dell'artefatto,
  alla provenienza del sorgente, alla compatibilità con OpenClaw, ai target host, ai metadati dell'ambiente
  e allo stato della scansione.
- Flag:
  - `--json`: output leggibile dalla macchina.
...
- I metadati vengono rilevati automaticamente da `package.json`, `openclaw.plugin.json` e
  dai marcatori reali dei bundle OpenClaw come `.codex-plugin/plugin.json`,
...
- Le sorgenti `.tgz` sono trattate come ClawPack. La CLI carica i byte npm-pack esatti
  e usa i contenuti estratti di `package/` solo per la convalida e la precompilazione
  dei metadati.
- Le cartelle dei plugin di codice vengono pacchettizzate in un tarball npm ClawPack prima del caricamento, così
...
- `--clawscan-note <text>` aggiunge una nota ClawScan. Questa nota fornisce a ClawScan
  il contesto per comportamenti che altrimenti potrebbero apparire insoliti, come accesso alla rete,
...
- Se stai usando una release più vecchia della CLI `clawhub`, aggiorna prima di pubblicare in modo che
  i controlli di preflight locali vengano eseguiti prima del caricamento.
...
- Per i monorepo, passa `source_path` in modo che il workflow pubblichi la cartella del pacchetto
  del plugin, ad esempio `source_path: extensions/codex`.
...
- `pull_request` dovrebbe usare `dry_run: true` in modo che la CI resti non inquinante.
...
- Mantieni `clawhub_token` disponibile per la prima pubblicazione, pacchetti non attendibili o pubblicazioni break-glass.
...
- Le radici possono essere qualunque cartella: una directory di skills o una singola cartella di skill con `SKILL.md`.
- Aggiunge automaticamente le radici skill di Clawdbot quando `~/.clawdbot/clawdbot.json` è presente:
