...
x-i18n:
    generated_at: "2026-06-27T17:15:42Z"
    model: gpt-5.5
...
- `--workdir <dir>`: directory di lavoro (predefinita: cwd; ripiega sull'area di lavoro di Clawdbot se configurata)
...
La CLI rispetta le variabili di ambiente standard del proxy HTTP per i sistemi dietro
...
- Utile per convogliare un token di login locale nei comandi di configurazione dei secret CI.
...
- Se una skill dovrebbe comparire ma non compare, esegui `clawhub inspect @owner/slug` mentre sei autenticato per controllare la diagnostica di moderazione visibile al proprietario prima di rinominare i metadati.
...
- Risolve la versione più recente per il proprietario e la skill nominati.
- Scarica lo zip tramite `/api/v1/download`.
- Estrae in `<workdir>/<dir>/<slug>`.
- Rifiuta di sovrascrivere le skill fissate; esegui prima `clawhub unpin <skill>`.
- Scrive:
...
- Invia telemetria best-effort mentre sei autenticato, così i conteggi di installazione correnti possono essere
  disattivati.
...
- Mostra `pinned` accanto alle skill congelate con `clawhub pin`, inclusa la motivazione facoltativa.
...
- `update --all` salta gli slug fissati e stampa un riepilogo di ciò che è rimasto congelato.

### `skill publish <path>`

- Confronta l'impronta del bundle locale con ClawHub ed esce con successo quando
  il contenuto è già pubblicato.
- Le nuove skill usano per impostazione predefinita `1.0.0`; le skill modificate usano per impostazione predefinita la versione patch
  successiva.
...
sotto `root` (predefinito: `skills`). Salta le skill invariate e usa lo
...
- Analizza l'attuale workdir, la directory delle skill configurata e qualsiasi
  cartella `--root <dir>` alla ricerca di cartelle skill locali contenenti `SKILL.md` o
  `skill.md`.
- Confronta ogni impronta di skill locale con ClawHub e pubblica solo skill nuove o
  modificate.
...
- `sync` è solo pubblicazione unidirezionale. Non installa, aggiorna, scarica né
  riporta telemetria di installazione/download.
...
- Le scansioni sono asincrone e possono richiedere tempo per completarsi. Durante la coda, lo spinner del terminale mostra l'attuale posizione di scansione prioritizzata e quante scansioni sono davanti.
- Le scansioni pubblicate richiedono proprietà o accesso di gestione dell'editore. Moderatori/admin possono usare lo stesso backend tramite `clawhub-admin`.
- `--update` è valido solo con `--slug`; scrive i risultati delle scansioni pubblicate riuscite nella versione selezionata.
...
- Richiede `clawhub login`.
- Scarica lo ZIP del report di scansione archiviato per una versione di skill o plugin inviata, incluse le versioni bloccate o nascoste dai controlli di sicurezza di ClawHub.
...
per repo di skill e repo catalogo.
...
- `root` usa per impostazione predefinita `skills` per i repo catalogo.
...
- La pubblicazione skill V1 usa `clawhub_token`; la pubblicazione affidabile tramite GitHub OIDC per ora è solo per pacchetti.

### `delete <skill>`

- Senza `--version`, elimina temporaneamente una skill (proprietario, moderatore o amministratore).
...
- `--version <version>` elimina definitivamente una versione posseduta non più recente tramite una route fail-closed,
  ...
  versione attuale più recente. Il personale della piattaforma non aggira la proprietà per questo flusso limitato alla versione.
- `--reason <text>` registra una nota di moderazione su un'eliminazione temporanea dell'intera skill e nel log di audit.
...
- Non esiste un annullamento dell'eliminazione per versione; le versioni eliminate definitivamente non possono essere ripristinate.
...
- Lo slug di origine non viene più elencato pubblicamente e diventa un alias di reindirizzamento verso la destinazione.
...
- I trasferimenti verso handle di organizzazione/publisher vengono applicati immediatamente solo quando l'attore ha
  accesso amministratore sia al proprietario attuale sia al publisher di destinazione.
- Sottocomandi:
...
- Usalo per Plugin e altre voci di famiglie di pacchetti; `search` di primo livello resta la superficie di ricerca delle skill.
- Flag:
...
- Esegue il Plugin Inspector incluso nella CLI di ClawHub su una cartella di pacchetto Plugin
  locale.
- Il valore predefinito è la validazione offline/statica, senza localizzare o importare un checkout
  OpenClaw locale.
- Gli errori di compatibilità critici terminano con codice diverso da zero. I risultati con sole avvertenze vengono stampati ma
  terminano con zero.
- Flag:
  - `--out <dir>`: scrivi i report di Plugin Inspector in questa directory.
  ...
  - `--allow-execute`: consenti l'acquisizione runtime in un'area di lavoro isolata.
...
Se la validazione segnala un problema relativo a pacchetto, manifesto, importazione SDK o artefatto, consulta
[Correzioni per la validazione dei Plugin](/it/clawhub/plugin-validation-fixes), quindi esegui di nuovo il comando.

### `package delete <name>`

- Senza `--version`, elimina temporaneamente un pacchetto e tutte le release.
- `--version <version>` elimina definitivamente una release posseduta non più recente tramite una route fail-closed,
  specifica per versione.
  Le versioni eliminate non possono essere ripristinate o ripubblicate. Pubblica una sostituzione prima di eliminare la
  versione attuale più recente. Questo flusso limitato alla versione richiede il proprietario del pacchetto o un amministratore
  del publisher dell'organizzazione; il personale della piattaforma non aggira la proprietà del pacchetto.
- L'eliminazione temporanea dell'intero pacchetto richiede il proprietario del pacchetto, un proprietario/amministratore del publisher dell'organizzazione, un moderatore della piattaforma o un amministratore della piattaforma.
- Flag:
...
- Richiede accesso amministratore sia al proprietario attuale del pacchetto sia al publisher di destinazione,
...
- Le segnalazioni sono a livello di pacchetto, facoltativamente legate a una versione, e diventano visibili
...
- Mostra lo stato attuale di scansione del pacchetto, il conteggio delle segnalazioni aperte, lo stato di moderazione manuale
...
- Mostra lo stato di migrazione orientato all'operatore per un pacchetto che potrebbe sostituire un
...
- I publisher di organizzazione appena creati non sono trusted/ufficiali per impostazione predefinita.
...
- Le sorgenti `.tgz` sono trattate come ClawPack. La CLI carica i byte npm-pack
  esatti e usa i contenuti estratti di `package/` solo per la convalida e
...
- `--owner <handle>` pubblica sotto un handle editore utente o organizzazione quando l'attore dispone dell'accesso editore.
...
Per i Plugin di codice, la pubblicazione della cartella crea e carica un artefatto ClawPack dalla
...
`package.json`. Questo manifesto minimo è sufficiente per una pubblicazione riuscita:
...
- `openclaw.hostTargets` e `openclaw.environment` sono metadati facoltativi.
  ClawHub può mostrarli quando sono presenti, ma non sono obbligatori per la pubblicazione.
...
- Se stai usando una release meno recente della CLI `clawhub`, aggiorna prima della pubblicazione così
...
- Collega o sostituisce la configurazione dell'editore attendibile GitHub Actions per un pacchetto
  esistente.
- Il pacchetto deve essere creato prima tramite la normale pubblicazione manuale o autenticata da token
  `clawhub package publish`.
- Dopo che la configurazione è stata impostata, le future pubblicazioni GitHub Actions supportate possono usare
  la pubblicazione OIDC/attendibile senza un token ClawHub a lunga durata.
...
- ClawHub verifica il repository GitHub configurato quando questo comando viene eseguito.
  ...
  esempio tramite una futura installazione GitHub App di ClawHub o un'altra integrazione
  GitHub autorizzata.
- Flag:
...
- Usalo come rollback se il workflow, il repository o il pin dell'ambiente devono essere
  disabilitati o ricreati.
- Le future pubblicazioni reali devono usare la normale pubblicazione autenticata fino a quando la configurazione
  non viene impostata di nuovo.
- Flag:
  - `--json`: output leggibile dalle macchine.
