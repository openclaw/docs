...
- La ricerca favorisce le corrispondenze esatte dei token di slug/nome prima della popolarità dei download. Un token slug autonomo come `map` corrisponde a `personal-map` in modo più forte della sottostringa dentro `amap`.
- I download sono un piccolo precedente di popolarità, non una garanzia di posizionamento in cima.
...
- Rifiuta di sovrascrivere Skill con pin; esegui prima `clawhub unpin <slug>`.
- Scrive:
...
- Se l'impronta digitale corrisponde a una versione nota: nessuna richiesta.
...
- `update <slug>` non procede per gli slug bloccati e indica di eseguire prima `clawhub unpin <slug>`.
...
- Usa questo per i plugins e altre voci della famiglia di pacchetti; `search` di primo livello resta la superficie di ricerca delle skill.
- Flag:
...
- Usa questo per metadati, compatibilità, verifica, sorgente e ispezione di versioni/file dei plugin.
- `--version <version>`: ispeziona una versione specifica (predefinita: latest).
...
- Richiede accesso amministrativo sia al proprietario attuale del pacchetto sia all'editore
  di destinazione, salvo se eseguito da un amministratore della piattaforma.
...
- Segnala blocchi per lo stato ufficiale, la disponibilità di ClawPack, il digest dell'artefatto,
...
- Chiama lo stesso endpoint di prontezza calcolata di `package readiness`, ma stampa
  stato incentrato sulla migrazione, versione più recente, stato di pacchetto ufficiale, controlli e
  blocchi.
- Flag:
  - `--json`: output leggibile da macchina.
...
- Per le cartelle locali, l'attribuzione del sorgente viene rilevata automaticamente da git locale quando il remote origin punta a GitHub.
...
- `--owner <handle>` pubblica sotto un handle di editore utente o organizzazione quando l'attore ha accesso da editore.
- `--clawscan-note <text>` aggiunge una nota ClawScan. Questa nota fornisce a ClawScan
  contesto per comportamenti che potrebbero altrimenti apparire insoliti, come accesso alla rete,
...
- Se stai usando una release più vecchia della CLI `clawhub`, esegui l'upgrade prima della pubblicazione, così
...
- Il workflow riutilizzabile imposta `source` sul repo chiamante per impostazione predefinita.
...
- Blocca il workflow riutilizzabile a un tag stabile o a uno SHA completo del commit. Non eseguire pubblicazioni di release da `@main`.
...
- Cerca cartelle di skill locali e pubblica quelle nuove/modificate.
