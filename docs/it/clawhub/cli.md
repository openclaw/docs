...
- I download sono un piccolo indicatore preventivo di popolarità, non una garanzia di posizionamento in cima.
...
- Rifiuta di sovrascrivere le skill bloccate; esegui prima `clawhub unpin <slug>`.
- Scrive:
...
- `--owner <handle>` pubblica sotto un handle editore di organizzazione/utente quando
...
- Lo slug sorgente non viene più elencato pubblicamente e diventa un alias di reindirizzamento al target.
...
- I trasferimenti a handle di organizzazione/editore si applicano immediatamente solo quando l'attore ha
...
- Usalo per Plugin e altre voci di famiglie di pacchetti; `search` di primo livello rimane la superficie di ricerca delle skill.
- Flag:
...
- Usalo per metadati, compatibilità, verifica, origine e ispezione di versione/file dei Plugin.
...
- Richiede l'accesso admin sia al proprietario attuale del pacchetto sia al publisher
  di destinazione, a meno che l'operazione non venga eseguita da un admin della piattaforma.
...
- Segnala i blocchi relativi a stato ufficiale, disponibilità ClawPack, digest dell'artefatto,
  provenienza del sorgente, compatibilità con OpenClaw, target host, metadati di ambiente
...
- Per le sorgenti GitHub, l'attribuzione della sorgente viene popolata automaticamente dal repo, dal commit risolto, dal ref e dal sottopercorso.
...
- `--clawscan-note <text>` aggiunge una nota ClawScan. Questa nota fornisce a ClawScan
  contesto per comportamenti che altrimenti potrebbero sembrare insoliti, come accesso alla rete,
  accesso host nativo o credenziali specifiche del provider. La nota viene memorizzata
  sulla release pubblicata.
...
- `openclaw.hostTargets` e `openclaw.environment` sono metadati facoltativi.
  ClawHub potrebbe mostrarli quando presenti, ma non sono richiesti per la pubblicazione.
...
- Il workflow riutilizzabile imposta per default `source` sul repo chiamante.
...
- Esegue la scansione delle cartelle Skills locali e pubblica quelle nuove/modificate.
- Le radici possono essere qualsiasi cartella: una directory Skills o una singola cartella Skill con `SKILL.md`.
