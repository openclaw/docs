...
- `--dir <dir>`: directory di installazione sotto la directory di lavoro (predefinita: `skills`)
- `--site <url>`: URL di base per l'accesso dal browser (predefinito: `https://clawhub.ai`)
- `--registry <url>`: URL di base dell'API (predefinito: rilevato automaticamente, altrimenti `https://clawhub.ai`)
- `--no-input`: disabilita i prompt

Equivalenti nelle variabili d'ambiente:
...
La CLI rispetta le variabili d'ambiente standard per proxy HTTP per i sistemi dietro
...
- Fallback precedente: se `clawhub/config.json` non esiste ancora ma `clawdhub/config.json` esiste, la CLI riusa il percorso precedente
...
- La ricerca privilegia le corrispondenze esatte dei token slug/nome prima della popolarità dei download. Un token slug autonomo come `map` corrisponde a `personal-map` in modo più forte della sottostringa dentro `amap`.
- I download sono un piccolo indicatore preliminare di popolarità, non una garanzia di posizionamento in cima.
- Se una skill dovrebbe comparire ma non compare, esegui `clawhub inspect <slug>` dopo aver effettuato l'accesso per controllare la diagnostica di moderazione visibile al proprietario prima di rinominare i metadati.
...
- Rifiuta di sovrascrivere le Skills bloccate; esegui prima `clawhub unpin <slug>`.
- Scrive:
...
- `--reason <text>` registra il motivo per cui la skill è bloccata.
...
- `--owner <handle>` pubblica sotto un handle editore di org/utente quando
...
- Rende visibile una skill (proprietario, moderatore o admin).
- Alias di `undelete`.
...
- I trasferimenti verso handle org/editore si applicano immediatamente solo quando l'attore ha
...
- Usa questo per i plugin e altre voci di famiglie di pacchetti; `search` di primo livello rimane la superficie di ricerca delle skill.
- Flag:
...
- Usa questo per metadati del plugin, compatibilità, verifica, sorgente e ispezione di versione/file.
- `--version <version>`: ispeziona una versione specifica (predefinito: latest).
...
- Per gli artefatti npm-pack ClawPack, verifica anche l'integrità npm `sha512`,
...
- Calcola SHA-256 ClawHub, integrità npm `sha512` e shasum npm per un
  artefatto locale.
...
- Esegue la soft-delete di un pacchetto e di tutte le release.
- Richiede il proprietario del pacchetto, un proprietario/admin del publisher dell'organizzazione, un moderatore della piattaforma
...
- Ripristina un pacchetto e le release eliminati tramite soft-delete.
- Richiede il proprietario del pacchetto, un proprietario/admin del publisher dell'organizzazione, un moderatore della piattaforma
  o un admin della piattaforma.
...
- Richiede l'accesso da admin sia al proprietario attuale del pacchetto sia al publisher
  di destinazione, salvo che venga eseguito da un admin della piattaforma.
...
- Comando per i proprietari per controllare la visibilità di moderazione del pacchetto.
- Chiama `GET /api/v1/packages/{name}/moderation`.
- Mostra lo stato corrente della scansione del pacchetto, il numero di segnalazioni aperte, lo stato di moderazione manuale
  della release più recente, lo stato del blocco dei download e i motivi della moderazione.
- Flag:
  - `--json`: output leggibile da macchina.
...
- Segnala blocchi per stato ufficiale, disponibilità di ClawPack, digest dell'artefatto,
...
- Le sorgenti `.tgz` vengono trattate come ClawPack. La CLI carica i byte npm-pack esatti
  e usa i contenuti `package/` estratti solo per la validazione e
...
- `--owner <handle>` pubblica sotto un handle di publisher utente o organizzazione quando l'attore dispone dell'accesso da publisher.
- `--clawscan-note <text>` aggiunge una nota ClawScan. Questa nota fornisce a ClawScan
  contesto per comportamenti che altrimenti potrebbero sembrare insoliti, come accesso alla rete,
...
- Il workflow riutilizzabile imposta per impostazione predefinita `source` sul repo chiamante.
...
- Le root possono essere qualsiasi cartella: una directory di skill o una singola cartella di skill con `SKILL.md`.
- Aggiunge automaticamente le root delle skill Clawdbot quando `~/.clawdbot/clawdbot.json` è presente:
...
- Inviata durante `sync` quando si è effettuato l'accesso, salvo `CLAWHUB_DISABLE_TELEMETRY=1` (legacy `CLAWDHUB_DISABLE_TELEMETRY=1`).
