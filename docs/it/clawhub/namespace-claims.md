...
rivendicato, riservato, fuorviante o contestato su ClawHub, chiedi al personale di esaminarlo
con il
[modulo per issue di rivendicazione organizzazione / namespace](https://github.com/openclaw/clawhub/issues/new?template=org-namespace-claim.yml).

Usa questo percorso per revisioni pubbliche e non sensibili della proprietà. Non usare le segnalazioni
nel prodotto o il modulo di appello dell'account per le rivendicazioni di namespace.

## Quando aprire una rivendicazione

Apri una rivendicazione di namespace quando ritieni che il personale di ClawHub debba verificare se un
namespace debba essere riservato, trasferito, rinominato, nascosto, messo in quarantena, associato a un alias
o modificato in altro modo a causa di proprietà nel mondo reale.
...
segui anche le linee guida di moderazione o sicurezza pertinenti. Il modulo di rivendicazione del namespace
serve per la revisione della proprietà, non per la divulgazione di emergenza di vulnerabilità.

## Prima di inviare

Conferma prima che stai pubblicando con il proprietario che corrisponde al namespace.
...
quando non puoi gestire il proprietario attuale o quando il personale deve risolvere una
disputa.
...
Spiega cosa dimostra ogni link. Il personale dovrebbe poter comprendere la
...
Il modulo di rivendicazione chiede se le prove sensibili richiedono un canale privato con il personale.
Usa questa opzione invece di pubblicare materiale sensibile pubblicamente.

## Esiti possibili

A seconda delle prove e del rischio, il personale di ClawHub può riservare un namespace,
...
Il personale valuta prove pubbliche, uso esistente, rischio per la sicurezza e impatto sugli utenti.
