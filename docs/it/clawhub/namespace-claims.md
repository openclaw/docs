---
read_when:
    - Rivendicare un'organizzazione, un marchio, uno scope di pacchetto, un handle del proprietario, uno slug di skill o un namespace di pacchetto
...
x-i18n:
    generated_at: "2026-06-28T07:41:49Z"
    model: gpt-5.5
...
con il
[modulo issue per rivendicazioni di organizzazioni / namespace](https://github.com/openclaw/clawhub/issues/new?template=org-namespace-claim.yml).
...
o modificato in altro modo per motivi di proprietà reale.
...
Se la voce è non sicura, malevola o fuorviante oltre alla controversia sulla proprietà,
segui anche le indicazioni pertinenti per moderazione o sicurezza. Il modulo di rivendicazione
del namespace serve per la revisione della proprietà, non per la divulgazione urgente di vulnerabilità.

## Prima di inviare

Conferma prima di tutto che stai pubblicando con il proprietario che corrisponde al namespace.
Per i pacchetti plugin, nomi con ambito come `@example-org/example-plugin` devono essere
...
- prove di marchio, brand o proprietà del progetto che siano sicure da discutere
  pubblicamente
...
Non inserire segreti o prove private in una issue pubblica di GitHub. Non includere:
...
- [Moderazione e sicurezza degli account](/it/clawhub/moderation)
