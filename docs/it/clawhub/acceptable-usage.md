...
x-i18n:
    generated_at: "2026-07-02T14:04:54Z"
    model: gpt-5.5
...
consulta [Richieste relative ai diritti sui contenuti](/it/clawhub/content-rights).
...
| Sicurezza difensiva, moderazione e revisione degli abusi | Lo strumento è presentato per una revisione autorizzata, preserva le prove e mantiene chiari i confini di approvazione umana.     |
...
basato sul consenso e inaccettabile quando viene confezionato come flusso di lavoro per abusi.
...
| ----------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Accesso non autorizzato o aggiramento della sicurezza       | Aggiramento dell'autenticazione, appropriazione di account, abuso dei limiti di frequenza, appropriazione di chiamate live o agent, furto di sessioni riutilizzabili o approvazione automatica di flussi di pairing per utenti non approvati.                                                                 |
| Abuso della piattaforma ed elusione dei ban                 | Account nascosti dopo ban, riscaldamento o coltivazione di account, engagement falso, automazione multi-account, pubblicazione di massa, bot spam o automazione costruita per evitare il rilevamento.                                                                                                        |
...
| Arricchimento invasivo della privacy o sorveglianza         | Raccolta di contatti per spam, doxxing, stalking, estrazione di lead associata a contatti non richiesti, monitoraggio occulto, matching biometrico non consensuale o uso di dati trapelati o dump di violazioni.                                                                                              |
...
| Requisiti di esecuzione nascosti, non sicuri o fuorvianti   | Comandi di installazione offuscati, installer pipe-to-shell come contenuti scaricati eseguiti con `sh` o `bash` senza chiara verificabilità, requisiti non dichiarati di segreti o chiavi private, esecuzione remota di `npx @latest` senza chiara verificabilità o metadati che nascondono ciò di cui la scheda ha davvero bisogno per funzionare. |
| Materiale che viola copyright o diritti                     | Ripubblicare Skills, Plugin, documentazione, risorse di brand o codice proprietario di qualcun altro senza autorizzazione; violare i termini di licenza; o impersonare l'autore o l'editore originale.                                                                                                       |
...
non prova di per sé un abuso; aiuta ClawHub a decidere cosa richiede revisione.

Possiamo:

- nascondere, sospendere, rimuovere, eliminare temporaneamente o, dove supportato per il tipo di risorsa,
  eliminare definitivamente le schede in violazione
- bloccare download o installazioni per release non sicure
- revocare token API
- eliminare temporaneamente contenuti associati
- limitare l'accesso alla pubblicazione
- bannare recidivi o autori di violazioni gravi
