...
I plugin di codice devono includere metadati di compatibilità con OpenClaw in
`package.json`, inclusi `openclaw.compat.pluginApi` e `openclaw.build.openclawVersion`.

## Sincronizzare le Skills che mantieni

`sync` analizza le cartelle delle Skills e pubblica Skills nuove o modificate che non sono
già sincronizzate.
