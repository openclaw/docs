...
quando esegui l'accesso, pubblichi, gestisci le tue inserzioni o usi
...
Il comando salta il contenuto invariato. Le nuove skill iniziano da `1.0.0`; le modifiche successive
...
`skill publish` per ogni cartella di skill immediata sotto `skills/`:
...
Usa prima `--dry-run` per visualizzare un'anteprima dei metadati del pacchetto risolti, dei campi di compatibilità, dell'attribuzione della sorgente e del piano di caricamento senza pubblicare.
...
Prima di installare, usa la pagina web di ClawHub o i comandi di dettaglio della CLI per esaminare
...
moderazione potrebbero essere nascoste dalle superfici di ricerca e installazione finché non vengono risolte.
