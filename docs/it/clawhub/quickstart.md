...
quando accedi, pubblichi, gestisci le tue inserzioni o usi workflow
...
modifiche successive pubblicano automaticamente la versione patch seguente. Usa
`--dry-run` per un'anteprima o `--version` per scegliere una versione esplicita.
...
[Formato delle skill](/it/clawhub/skill-format).
...
I code plugin devono includere in `package.json` i metadati di compatibilità con
OpenClaw, inclusi `openclaw.compat.pluginApi` e
...
Le inserzioni pubbliche mostrano lo stato della scansione più recente. Le release
