...
read_when:
    ...
    - Installare uno skill o un plugin dal registro
...
Il comando ignora i contenuti invariati. Le nuove skill partono da `1.0.0`; le
modifiche successive pubblicano automaticamente la versione patch successiva.
Usa `--dry-run` per vedere un'anteprima oppure `--version` per scegliere una
versione esplicita.
...
Usa prima `--dry-run` per vedere un'anteprima dei metadati del pacchetto risolti,
dei campi di compatibilità, dell'attribuzione della sorgente e del piano di
...
Le inserzioni pubbliche mostrano lo stato più recente della scansione. Le release
