...
pubblicano automaticamente la versione patch successiva. Usa `--dry-run` per vedere un'anteprima o
...
serve alla Skill prima di installarla. Consulta [Formato Skill](/it/clawhub/skill-format).
...
Usa prima `--dry-run` per vedere un'anteprima dei metadati del pacchetto risolti, dei campi di compatibilità,
