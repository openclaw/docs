...
x-i18n:
    generated_at: "2026-07-01T13:04:01Z"
    model: gpt-5.5
...
pubblicano automaticamente la versione patch successiva. Usa `--dry-run` per l'anteprima o
`--version` per scegliere una versione esplicita.

Prima della pubblicazione, controlla i metadati in `SKILL.md`. Dichiara le
variabili d'ambiente, gli strumenti e le autorizzazioni richiesti, così gli utenti possono capire di cosa ha
bisogno la skill prima di installarla. Vedi [Formato delle skill](/it/clawhub/skill-format).

Per i repository contenenti più skill, il workflow GitHub riutilizzabile chiama
...
Usa prima `--dry-run` per visualizzare in anteprima i metadati del pacchetto risolto, i campi di compatibilità,
