...
x-i18n:
    generated_at: "2026-07-01T15:25:51Z"
    model: gpt-5.5
...
quando accedi, pubblichi, gestisci le tue inserzioni o usi workflow specifici
del registro.
...
OpenClaw registra l'origine della skill, così gli aggiornamenti successivi
...
ClawHub anziché tramite npm o un'altra sorgente.
...
Il comando salta il contenuto invariato. Le nuove skill partono da `1.0.0`; le
...
capire di cosa ha bisogno la skill prima di installarla. Consulta [Formato skill](/it/clawhub/skill-format).

Per i repository che contengono più skill, il workflow GitHub riutilizzabile
...
Pubblica un plugin da una cartella locale, un repository GitHub, un riferimento
GitHub o un archivio esistente:
...
Usa prima `--dry-run` per vedere in anteprima i metadati del pacchetto risolti, i
campi di compatibilità, l'attribuzione della sorgente e il piano di caricamento
senza pubblicare.

I plugin di codice devono includere i metadati di compatibilità OpenClaw in
`package.json`, inclusi `openclaw.compat.pluginApi` e
`openclaw.build.openclawVersion`.

## Ispezionare prima dell'installazione

Prima di installare, usa la pagina web di ClawHub o i comandi di dettaglio della
CLI per ispezionare metadati, link sorgente, versioni, changelog e stato delle
scansioni:
...
Le inserzioni pubbliche mostrano lo stato della scansione più recente. Le
release trattenute o bloccate dalla moderazione possono essere nascoste dalle
superfici di ricerca e installazione finché non vengono risolte.
