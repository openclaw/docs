...
x-i18n:
    generated_at: "2026-07-04T15:21:03Z"
    model: gpt-5.5
...
Guida alla rilevabilità per i publisher:
...
- Gli alias di rinomina preservano la risoluzione per i vecchi URL e le installazioni che si risolvono tramite il registro, ma il ranking di ricerca si basa sui metadati canonici della skill dopo che la rinomina è stata indicizzata. Le statistiche esistenti rimangono con la skill.
...
- `metadata.os`: restrizioni OS dichiarate nel frontmatter della skill (ad esempio `["macos"]`, `["linux"]`). `null` se non dichiarato.
- `metadata.systems`: target di sistema Nix (ad esempio `["aarch64-darwin", "x86_64-linux"]`). `null` se non dichiarato.
...
imposta di nuovo `status` su `open`. Passa `finalAction: "hide"` con una segnalazione valutata
...
Endpoint autenticato dell'archivio dei report memorizzati per le versioni inviate.
...
- Restituisce i risultati di scansione memorizzati per l'esatta versione inviata, incluse versioni bloccate o nascoste.
- `kind` è impostato in modo predefinito su `skill`; usa `kind=plugin` per scansioni di plugin/pacchetti.
- Restituisce la stessa struttura ZIP dei download delle richieste di scansione.

### `POST /api/v1/skills/-/scan/batch`

Route canonica di riesecuzione batch della scansione, solo per amministratori. Accetta la stessa forma di payload della route legacy `POST /api/v1/skills/-/rescan-batch`.
...
- `security.signals.dependencyRegistry` viene mantenuto per compatibilità con le risposte v1, ma lo scanner di esistenza del registro delle dipendenze è stato ritirato e questa chiave è sempre `null`.
...
- `items` deve contenere da 1 a 100 coppie univoche `{ slug, version }`.
...
- `category` (facoltativo): filtro categoria plugin. Supportato solo quando la
  ...
  `/api/v1/code-plugins`, `/api/v1/bundle-plugins` o endpoint pacchetto con
...
- Le voci Skill rimangono basate sul registro delle Skill e possono ancora essere pubblicate solo tramite `POST /api/v1/skills`.
...
- `version.artifact.kind` è `legacy-zip` per gli archivi dei pacchetti del vecchio mondo o
...
- `trust.stale` indica che il riepilogo di attendibilità è stato calcolato da input obsoleti e
  deve essere trattato come richiedente un aggiornamento prima di una decisione di autorizzazione ad alta confidenza.
...
- Questo endpoint è intenzionalmente più ristretto degli endpoint di moderazione
  per proprietario/moderatore. Espone la decisione di installazione e la spiegazione pubblica, non
...
- `open`: release sospette, dannose, in attesa, in quarantena, revocate o segnalate.
- `blocked`: release in quarantena, revocate o dannose.
- `manual`: qualsiasi release con un override di moderazione manuale.
- `all`: qualsiasi release con un override manuale, stato di scansione non pulito o segnalazione di pacchetto.
...
bloccano i download da sole; i moderatori devono usare la moderazione della release per
...
Endpoint per moderatori/admin per l'acquisizione dei report sui pacchetti.
...
Endpoint per proprietari/moderatori per la visibilità sulla moderazione dei pacchetti.
...
Endpoint per moderatori/admin per risolvere o riaprire i report sui pacchetti.
...
Endpoint per moderatori/admin per la revisione delle release dei pacchetti.
...
Scarica uno ZIP di una versione di Skill ospitata, oppure restituisce un passaggio di consegne a una sorgente GitHub per una
...
Valida il token e restituisce l'handle utente.
...
- Campo payload opzionale: `migrateOwner`. Quando `true` con `ownerHandle`, una
  Skill esistente può essere spostata a quel proprietario se l'attore è un admin/proprietario sia sul
  publisher corrente sia su quello di destinazione. Senza questo consenso esplicito, le modifiche di proprietario vengono
  rifiutate.
...
- I campi modulo consentiti sono `payload`, blob `files` ripetuti oppure un riferimento a tarball `clawpack`.
  ...
  flusso upload-url. Le pubblicazioni con ID di storage preparato devono includere anche il
...
utente condiviso/publisher personale legacy, l'endpoint lo migra prima in un publisher org.
...
non marca il publisher come attendibile/ufficiale.
...
immutabili; gli handle mutabili sono usati solo come protezione visibile all'operatore.
...
i percorsi direct-owner concordano con la nuova autorità dell'editore. Anche una prenotazione active protected-handle
...
Le sorgenti di skill GitHub sono con ambito editore e vengono segnalate come controllate invece che riscritte.
...
contenuti (solo amministratore). Usa per impostazione predefinita dry-run, a meno che `dryRun` sia `false`.
