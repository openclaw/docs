---
read_when:
    - Aggiunta/modifica dei punti di accesso
...
- Gli endpoint di scrittura autenticati non dovrebbero restituire un semplice `Unauthorized` quando
  ...
  account eliminati/bannati/disabilitati dovrebbero ricevere ciascuno un testo operativo, così i client
...
- I risultati sono restituiti in ordine di rilevanza (similarità degli embedding + incrementi per corrispondenza esatta dei token di slug/nome + priorità di popolarità dai download).
...
- I download sono usati come piccola priorità in scala logaritmica e come discriminante in caso di parità, non come segnale di classificazione principale. Le Skills con molti download possono posizionarsi più in basso quando il testo della query è una corrispondenza più debole.
...
- Gli alias di rinomina preservano la risoluzione per i vecchi URL e per le installazioni che si risolvono tramite il registro, ma il ranking di ricerca si basa sui metadati canonici della Skill dopo che la rinomina è stata indicizzata. Le statistiche esistenti rimangono associate alla Skill.
...
- Predefinito alla versione più recente.
...
- `os`, `arch`, `libc` (facoltativo): abbreviazione per filtri di capacità host
- `requiresBrowser`, `requiresDesktop`, `requiresNativeDeps`,
  `requiresExternalService`, `requiresBinary`, `requiresOsPermission`
  (facoltativo): abbreviazione `true`/`1` per i tag dei requisiti di ambiente
- `externalService`, `binary`, `osPermission` (facoltativo): abbreviazione per tag
...
Ricerca catalogo unificata tra Skills e pacchetti Plugin.
...
Restituisce i metadati del resolver esplicito dell'artefatto per una versione del pacchetto.
...
Scarica l'artefatto della versione tramite il percorso del resolver esplicito.
...
Segnala un pacchetto per la revisione da parte del moderatore. Le segnalazioni sono a livello di pacchetto, facoltativamente
...
Endpoint moderatore/admin per la ricezione delle segnalazioni dei pacchetti.
...
Endpoint di manutenzione riservato agli amministratori per etichettare le release di pacchetti più vecchie con
...
- L'impostazione predefinita è l'esecuzione di prova.
...
- L'impostazione predefinita è la release più recente.
...
- Gli archivi di Plugin/pacchetto sono file zip con una radice `package/` così i vecchi client OpenClaw
...
Solo amministratori. Garantisce che esista un publisher organizzazione per un handle. Se l'handle punta ancora a un
...
proprietario può pubblicare in seguito la release code-plugin o bundle-plugin reale con quel nome.
...
- `merge` nasconde la voce di elenco sorgente e reindirizza lo slug sorgente alla voce di elenco di destinazione.
