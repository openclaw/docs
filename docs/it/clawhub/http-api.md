...
x-i18n:
    generated_at: "2026-07-04T10:43:29Z"
    model: gpt-5.5
...
Le scorciatoie slug web si risolvono tra famiglie di registry, ma i client API dovrebbero usare
...
- Gli endpoint di scrittura autenticati non dovrebbero restituire un semplice `Unauthorized` quando
  ...
  account eliminati/bannati/disabilitati dovrebbero ricevere ciascuno un testo utilizzabile affinché i client
...
- I risultati sono restituiti in ordine di pertinenza (somiglianza embedding + boost per token esatti di slug/nome + un piccolo prior di popolarità).
- La pertinenza è più forte della popolarità. Una corrispondenza precisa di slug o token del nome visualizzato può superare una corrispondenza più vaga con coinvolgimento molto maggiore.
- Il testo ASCII è tokenizzato sui confini di parole e punteggiatura. Per esempio, `personal-map` contiene un token autonomo `map`, mentre `amap-jsapi-skill` contiene `amap`, `jsapi` e `skill`; cercare `map` quindi assegna a `personal-map` una corrispondenza lessicale più forte rispetto a `amap-jsapi-skill`.
- La popolarità è scalata logaritmicamente e limitata. Skills ad alto coinvolgimento possono posizionarsi più in basso quando il testo della query è una corrispondenza più debole.
...
- Gli alias di rinomina preservano la risoluzione per i vecchi URL e le installazioni che si risolvono tramite il registry, ma il ranking di ricerca si basa sui metadati canonici della Skill dopo che la rinomina è stata indicizzata. Le statistiche esistenti restano con la Skill.
...
- `sort` (opzionale): `updated` (predefinito), `recommended` (alias: `default`), `createdAt` (alias: `newest`), `downloads`, `stars` (alias: `rating`), alias legacy di installazione `installsCurrent`/`installs`/`installsAllTime` mappano a `downloads`, `trending`
- `nonSuspiciousOnly` (opzionale): `true` per nascondere Skills sospette (`flagged.suspicious`)
- `nonSuspicious` (opzionale): alias legacy per `nonSuspiciousOnly`
...
- `createdAt` è stabile per i crawl di nuove Skill; `updated` cambia quando Skills esistenti vengono ripubblicate.
...
gestita per nascondere la Skill nello stesso flusso di lavoro verificabile.
...
- `category` (facoltativo): filtro per categoria di Plugin. Supportato solo quando la
  ...
  alias legacy v1 dei filtri sono documentati in `GET /api/v1/plugins`.
...
- `category` (facoltativo): filtro per categoria di Plugin. Supportato solo quando la
  ...
  legacy v1 dei filtri sono documentati in `GET /api/v1/plugins`.
...
Navigazione del catalogo solo Plugin tra pacchetti Plugin di codice e Plugin bundle.
...
Gli alias legacy v1 dei filtri restano accettati sugli endpoint di lettura:
...
- Le Skill attuali basate su GitHub con scansione `clean` o `suspicious` includono
...
Esportazione massiva degli ultimi rilasci pubblici dei plugin per analisi offline.
...
- Richiede un token API per il proprietario del pacchetto, un proprietario/amministratore dell'editore organizzazione,
...
- I rilasci ClawPack includono i campi compatibili con npm `npmIntegrity`, `npmShasum` e
  `npmTarballName`.
- `version.sha256hash` è un metadato di compatibilità deprecato per i vecchi client. Esegue l'hash dei byte ZIP esatti restituiti da `/api/v1/packages/{name}/download`.
...
Restituisce il riepilogo esatto di sicurezza e affidabilità del rilascio del pacchetto per i client
...
- `package.name`, `package.displayName` e `package.family` identificano il
  pacchetto del registry risolto.
...
- Le versioni ClawPack trasmettono i byte `.tgz` esatti dell'npm-pack caricato.
...
Restituisce il contenuto di testo non elaborato per un file di pacchetto.
...
- I metadati solo registro non vengono iniettati nell'archivio scaricato.
...
Scarica uno ZIP di una versione di skill ospitata, oppure restituisce un passaggio di consegne al sorgente GitHub per una
...
- Campo payload opzionale: `migrateOwner`. Quando `true` con `ownerHandle`, una
  skill esistente può passare a quel proprietario se l'attore è un admin/proprietario sia dell'
  editore corrente sia di quello di destinazione. Senza questa adesione esplicita, le modifiche del proprietario vengono
  rifiutate.
...
Elimina logicamente / ripristina una skill (proprietario, moderatore o admin).
...
Le eliminazioni logiche avviate dal proprietario riservano lo slug per 30 giorni, poi lo slug può essere rivendicato da
...
Solo admin. Garantisce che esista un editore organizzazione per un handle. Se l'handle punta ancora a un
editore legacy condiviso utente/personale, l'endpoint lo migra prima in un editore organizzazione.
...
Creazione self-service autenticata di editori organizzazione. Crea un nuovo editore organizzazione e aggiunge il
...
immutabili; gli handle mutabili vengono usati solo come guardia rivolta all'operatore.
...
i principali GitHub. Il recupero fallisce in modo chiuso quando il publisher personale attuale dell'utente di destinazione
ha skills, packages o GitHub skill sources.
...
i percorsi con proprietario diretto concordino con la nuova autorità del publisher. Anche una prenotazione attiva di protected-handle
...
Le GitHub skill sources hanno ambito di publisher e sono segnalate come controllate anziché riscritte.
