...
x-i18n:
    generated_at: "2026-07-02T08:23:49Z"
    model: gpt-5.5
...
Le scorciatoie slug web vengono risolte tra le famiglie del registro, ma i client API dovrebbero usare
...
- ClawHub usa header di forwarding attendibili per identificare gli IP client al margine.
...
dovrebbero leggere il corpo della risposta come stringa leggibile da persone. I parametri di query sconosciuti sono
...
- I risultati vengono restituiti in ordine di rilevanza (somiglianza degli embedding + boost per token esatti di slug/nome + un piccolo prior di popolarità).
- La rilevanza è più forte della popolarità. Una corrispondenza precisa con uno slug o un token del nome visualizzato può superare una corrispondenza più debole con coinvolgimento molto più alto.
- Il testo ASCII viene tokenizzato su confini di parola e punteggiatura. Per esempio, `personal-map` contiene un token autonomo `map`, mentre `amap-jsapi-skill` contiene `amap`, `jsapi` e `skill`; cercare `map` quindi dà a `personal-map` una corrispondenza lessicale più forte rispetto a `amap-jsapi-skill`.
...
- Gli alias di rinomina preservano la risoluzione per vecchi URL e installazioni che vengono risolti tramite il registro, ma il ranking di ricerca si basa sui metadati canonici della Skill dopo che la rinomina è stata indicizzata. Le statistiche esistenti restano associate alla Skill.
- Se una Skill è inaspettatamente invisibile, controlla prima lo stato di moderazione con `clawhub inspect @owner/slug` dopo aver effettuato l'accesso, prima di modificare i metadati relativi al ranking.
...
- `trending` ordina in base alle installazioni degli ultimi 7 giorni (basato sulla telemetria).
...
valutata per nascondere la Skill nello stesso flusso di lavoro verificabile.
...
Route canonica di nuova scansione batch riservata agli amministratori. Accetta la stessa forma di payload del legacy `POST /api/v1/skills/-/rescan-batch`.

### `POST /api/v1/skills/-/scan/batch/status`

Route canonica di stato batch riservata agli amministratori. Accetta `{ "jobIds": ["..."] }` e restituisce gli stessi contatori aggregati del legacy `POST /api/v1/skills/-/rescan-batch/status`.
...
- `security.signals.dependencyRegistry` viene mantenuto per compatibilità della risposta v1, ma lo scanner di esistenza del registro delle dipendenze è ritirato e questa chiave è sempre `null`.
...
- `category` (facoltativo): filtro categoria plugin. Supportato solo quando la
  ...
  alias legacy del filtro v1 sono documentati sotto `GET /api/v1/plugins`.
...
- `POST /api/v1/packages` resta ancora solo per le release di code-plugin e bundle-plugin.
...
- `category` (facoltativo): filtro categoria plugin. Supportato solo quando la
  ...
  legacy del filtro v1 sono documentati sotto `GET /api/v1/plugins`.
...
`trending` è una classifica installazioni/download su sette giorni e non usa i totali complessivi.
Sull'endpoint unificato `/api/v1/packages` è solo per plugin; usa
`/api/v1/skills?sort=trending` per il catalogo skill.

Gli alias legacy non sono accettati come valori categoria archiviati o dichiarati dall'autore.
...
- `_errors.json` è incluso quando singole skill o singoli file non possono essere
  esportati.
...
- Il filtro per categoria è un vero filtro API supportato da righe di digest
...
- Le Skills possono anche risolversi tramite questa rotta nel catalogo unificato.
...
- `version.artifact.kind` è `legacy-zip` per archivi di pacchetti del vecchio mondo oppure
  `npm-pack` per release supportate da ClawPack.
- Le release ClawPack includono i campi compatibili con npm `npmIntegrity`, `npmShasum` e
  `npmTarballName`.
- `version.sha256hash` è un metadato di compatibilità deprecato per client obsoleti. Calcola l'hash
...
Restituisce il riepilogo esatto di sicurezza e attendibilità della release del pacchetto per i client di installazione. Questa è la superficie pubblica di consumo di OpenClaw per decidere se una
...
- `trust.reasons` è l'elenco di spiegazioni rivolte all'utente e di audit. I codici motivo
  ...
  e `package:malicious`.
- `trust.pending` indica che uno o più input di attendibilità sono ancora in attesa di completamento.
- `trust.stale` indica che il riepilogo di attendibilità è stato calcolato da input obsoleti e
  dovrebbe essere considerato come richiedente un aggiornamento prima di una decisione di autorizzazione ad alta confidenza.
...
- Le versioni legacy del pacchetto restituiscono un artefatto `legacy-zip` e un
...
- Usa il bucket del limite di frequenza dei download.

### `GET /api/v1/packages/{name}/readiness`

Restituisce l'idoneità calcolata per il futuro consumo da parte di OpenClaw.
...
- `packageName` è normalizzato come nome npm; il pacchetto può mancare per migrazioni
  pianificate.
- Questo traccia solo l'idoneità alla migrazione. Non modifica OpenClaw né genera
  ClawPack.
...
Endpoint moderatore/amministratore per risolvere o riaprire segnalazioni sui pacchetti.
...
Restituisce un packument compatibile con npm per versioni di pacchetti supportate da ClawPack.
...
- Le intestazioni di download includono lo SHA-256 di ClawHub più i metadati npm integrity/shasum.
...
skill corrente supportata da GitHub con una scansione `clean` o `suspicious` e nessuna versione
ospitata.
...
- I passaggi di skill supportate da GitHub non fanno proxy né mirror dei byte. La risposta JSON
...
- Campo payload facoltativo: `ownerHandle`. Quando presente, l'API risolve quell'
...
- Campo payload facoltativo: `migrateOwner`. Quando `true` con `ownerHandle`, una
  ...
  editori corrente sia su quelli di destinazione. Senza questo consenso esplicito, le modifiche di proprietario sono
  rifiutate.
...
- I campi del modulo consentiti sono `payload`, blob `files` ripetuti o un riferimento tarball
  `clawpack`. `clawpack` può essere un blob `.tgz` o un id di storage restituito dal
...
- Solo l'editore dell'organizzazione `openclaw` e gli editori personali degli attuali membri dell'organizzazione `openclaw`
...
Gli occultamenti di moderatore/amministratore e le rimozioni di sicurezza non scadono in questo modo.
...
Solo amministratori. Assicura che esista un editore di organizzazione per un handle. Se l'handle punta ancora a un
utente legacy condiviso/editore personale, l'endpoint lo migra prima in un editore di organizzazione.
...
Creazione autenticata self-service di un editore di organizzazione. Crea un nuovo editore di organizzazione e aggiunge il
...
i principali GitHub. Il recupero fallisce in modalità chiusa quando l'attuale pubblicatore personale dell'utente di destinazione
ha skill, pacchetti o sorgenti di skill GitHub.
Il recupero migra anche i campi legacy `ownerUserId` per le skill del pubblicatore recuperato,
...
i percorsi del proprietario diretto concordino con la nuova autorità del pubblicatore. Anche una prenotazione attiva di handle protetto
...
Le sorgenti di skill GitHub hanno ambito del pubblicatore e sono segnalate come controllate anziché riscritte.
...
Blocca un utente ed elimina definitivamente le skill possedute (solo moderatore/amministratore).
...
Sblocca un utente e ripristina le skill idonee (solo amministratore).
...
Modifica il motivo memorizzato per un blocco esistente senza sbloccare né ripristinare
contenuti (solo amministratore). Usa per impostazione predefinita l'esecuzione di prova a meno che `dryRun` sia `false`.
...
Se esegui il self-hosting, servi questo file (oppure imposta `CLAWHUB_REGISTRY` esplicitamente; legacy `CLAWDHUB_REGISTRY`).
