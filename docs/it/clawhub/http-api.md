...
x-i18n:
    generated_at: "2026-06-27T17:15:54Z"
    model: gpt-5.5
...
- `X-RateLimit-Remaining` / `RateLimit-Remaining`: budget residuo esatto quando presente.
  Le richieste con sharding riuscite omettono questo header invece di restituire un valore globale approssimativo.
...
Ciò include errori di validazione (`400`), risorse pubbliche mancanti (`404`), errori di autenticazione e
autorizzazione (`401`/`403`), limiti di frequenza (`429`) e download bloccati. I client
devono leggere il corpo della risposta come stringa leggibile. I parametri di query sconosciuti sono
...
- I risultati sono restituiti in ordine di pertinenza (similarità degli embedding + potenziamenti per token slug/nome esatti + un piccolo prior di popolarità).
- La pertinenza conta più della popolarità. Una corrispondenza precisa di slug o di token del nome visualizzato può superare una corrispondenza più vaga con coinvolgimento molto più alto.
...
- Non rinominare uno slug solo per inseguire una query, a meno che il nuovo slug sia un nome canonico migliore a lungo termine. I vecchi slug diventano alias di reindirizzamento, ma l'URL canonico, lo slug mostrato e i futuri digest di ricerca usano il nuovo slug.
...
- Usa `nextCursor` per continuare la paginazione quando presente. Una pagina corta non significa di per sé che i risultati siano terminati.
...
- `category` (opzionale): filtro categoria Plugin. Supportato solo quando la
  ...
  alias di filtro legacy v1 sono documentati sotto `GET /api/v1/plugins`.
...
- `category` (opzionale): filtro categoria Plugin. Supportato solo quando la
  ...
  di filtro legacy v1 sono documentati sotto `GET /api/v1/plugins`.
...
Consultazione del catalogo solo Plugin tra pacchetti code-plugin e bundle-plugin.
...
- Ogni Plugin esportato include i file memorizzati dell'ultima release.
- I metadati di esportazione per Plugin sono memorizzati in
...
- I controlli di ordinamento dell'interfaccia browser per la ricerca dei Plugin riordinano i risultati
  di pertinenza caricati, allineandosi al comportamento di esplorazione attuale di `/skills`.
...
- `version.artifact.kind` è `legacy-zip` per gli archivi di pacchetto del mondo precedente o
...
Restituisce il riepilogo esatto di sicurezza e fiducia della release del pacchetto per i client di installazione.
Questa è la superficie pubblica di consumo di OpenClaw per decidere se una
...
- Questo endpoint è intenzionalmente più ristretto degli endpoint di moderazione
  ...
  le identità dei segnalatori, i contenuti delle segnalazioni, le prove private o le cronologie interne
  di revisione.
...
- `revoked`: bloccata dopo che una release era stata precedentemente considerata affidabile.
...
Restituisce contenuto di testo non elaborato per un file del pacchetto.
...
- Questa route resta solo ZIP. Non esegue lo streaming di file ClawPack `.tgz`.
...
skill corrente basata su GitHub con scansione `clean` o `suspicious` e senza una
versione ospitata.
...
- Gli handoff delle skill basate su GitHub non fanno proxy né mirror dei byte. La risposta JSON
  ...
  e `archiveUrl`; lo stato della scansione/corrente è un gate e non è incluso come metadato del payload
  di successo.
...
Le rimozioni di sicurezza e gli occultamenti di moderatore/admin non scadono in questo modo.
...
Per un'organizzazione appena creata, fornisci `memberHandle`; l'admin attivo non viene aggiunto come membro.
...
non contrassegna il publisher come trusted/official.
...
L'endpoint usa per impostazione predefinita la dry-run. Applicare il recupero richiede `dryRun: false` e
...
gli alias degli slug delle Skills, i pacchetti, gli avvisi dell'ispettore dei pacchetti e le righe derivate del digest di ricerca, così
i percorsi con proprietario diretto concordano con la nuova autorità dell'editore. Una prenotazione attiva di handle protetto
...
Le sorgenti skill GitHub sono con ambito editore e vengono segnalate come controllate anziché riscritte.
...
contenuti (solo admin). Per impostazione predefinita usa la dry-run, a meno che `dryRun` sia `false`.
