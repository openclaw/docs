...
Directory di terze parti possono usare gli endpoint pubblici di lettura per elencare o cercare le skill di ClawHub. Memorizza i risultati nella cache, rispetta `429`/`Retry-After`, collega gli utenti alla scheda canonica di ClawHub (`https://clawhub.ai/<owner>/<slug>`) ed evita di implicare un'approvazione di ClawHub per il sito di terze parti. Non tentare di replicare contenuti nascosti, privati o bloccati dalla moderazione al di fuori della superficie dell'API pubblica.
...
- Se `Retry-After` manca, ripiega su `RateLimit-Reset` (oppure calcola da `X-RateLimit-Reset`).
...
- I risultati vengono restituiti in ordine di rilevanza (similarità dell'embedding + boost per token esatti di slug/nome + prior di popolarità dai download).
- La rilevanza è più forte della popolarità. Una corrispondenza precisa di slug o token del nome visualizzato può superare una corrispondenza più debole con molti più download.
...
- I download vengono usati come piccolo prior con scala logaritmica e criterio di spareggio, non come segnale primario di ranking. Le skill con molti download possono posizionarsi più in basso quando il testo della query è una corrispondenza più debole.
...
- Non rinominare uno slug solo per inseguire una query, a meno che il nuovo slug sia un nome canonico migliore a lungo termine. Gli slug vecchi diventano alias di reindirizzamento, ma l'URL canonico, lo slug visualizzato e i futuri digest di ricerca usano il nuovo slug.
- Gli alias di rinomina preservano la risoluzione per vecchi URL e installazioni che si risolvono tramite il registro, ma il ranking della ricerca si basa sui metadati canonici della skill dopo che la rinomina è stata indicizzata. Le statistiche esistenti restano con la skill.
- Se una skill è inaspettatamente invisibile, controlla prima lo stato di moderazione con `clawhub inspect <slug>` dopo aver effettuato l'accesso, prima di modificare metadati legati al ranking.
...
- `createdAt` è stabile per le scansioni di nuove skill; `updated` cambia quando skill esistenti vengono ripubblicate.
...
- Proprietari e moderatori possono accedere ai dettagli di moderazione per skill nascoste.
...
Segnala una skill per revisione da parte dei moderatori. Le segnalazioni sono a livello di skill, collegate facoltativamente
...
si imposta `status` di nuovo su `open`. Passa `finalAction: "hide"` con una segnalazione
...
- Include lo stato normalizzato di verifica più dettagli specifici dello scanner.
...
collegate a una versione. Alimentano la coda di moderazione, ma da sole non nascondono automaticamente né
bloccano i download; i moderatori devono usare la moderazione delle release per
...
`note` è obbligatorio per `confirmed` e `dismissed`; può essere omesso quando
si reimposta `status` su `open`. Passa `finalAction: "quarantine"` oppure
...
Endpoint di manutenzione riservato agli amministratori per etichettare le release di pacchetti precedenti con
...
- Per impostazione predefinita esegue una simulazione.
...
- Usa il bucket di limite per le letture, non quello per i download.
...
- Campo facoltativo del payload: `ownerHandle`. Quando presente, l'API risolve tale
...
- Campo facoltativo del payload: `migrateOwner`. Quando `true` con `ownerHandle`, una
  skill esistente può essere spostata a tale proprietario se l'attore è amministratore/proprietario sia
  sull'editore corrente sia su quello di destinazione. Senza questa adesione esplicita, le modifiche del proprietario vengono
  rifiutate.
...
- Solo gli editori attendibili possono pubblicare nel canale `official`.
...
Quando presente, `reason` viene salvato come nota di moderazione della skill e copiato nel registro di audit.
...
Le rimozioni di moderatori/amministratori e di sicurezza non scadono in questo modo.
...
Solo amministratori. Garantisce l'esistenza di un editore organizzazione per un handle. Se l'handle punta ancora a un
...
Solo amministratori. Riserva slug radice e nomi di pacchetto per il proprietario legittimo senza pubblicare una
