...
x-i18n:
    generated_at: "2026-07-04T20:32:33Z"
    model: gpt-5.5
...
dovrebbero leggere il corpo della risposta come stringa leggibile da un essere umano. I parametri di query sconosciuti sono
...
- `highlightedOnly` (opzionale): `true` per filtrare sulle skill evidenziate
...
- I risultati vengono restituiti in ordine di pertinenza (similarità degli embedding + boost esatti su token di slug/nome + un piccolo fattore di popolarità).
- La pertinenza pesa più della popolarità. Una corrispondenza precisa su slug o token del nome visualizzato può superare una corrispondenza più vaga con un coinvolgimento molto più forte.
...
- Gli alias di rinomina preservano la risoluzione per i vecchi URL e le installazioni che si risolvono tramite il registro, ma il ranking di ricerca si basa sui metadati canonici della skill dopo l’indicizzazione della rinomina. Le statistiche esistenti restano con la skill.
...
- `recommended` usa segnali di coinvolgimento e attualità.
...
Endpoint per moderatori/admin per risolvere o riaprire segnalazioni di skill.
...
si riporta `status` a `open`. Passa `finalAction: "hide"` con una segnalazione esaminata
...
- Restituisce `queue.queuedAhead` e `queue.position` durante l'accodamento, così i client possono mostrare quante scansioni manuali prioritarie precedono la richiesta. Le code molto grandi sono limitate e segnalate con `queuedAheadIsEstimate: true`.
...
- `kind` ha come valore predefinito `skill`; usa `kind=plugin` per le scansioni di plugin/package.
- Restituisce la stessa forma ZIP dei download delle richieste di scansione.

### `POST /api/v1/skills/-/scan/batch`

Route canonica solo admin per la nuova scansione batch. Accetta la stessa forma di payload della route legacy `POST /api/v1/skills/-/rescan-batch`.

### `POST /api/v1/skills/-/scan/batch/status`

Route canonica solo admin per lo stato batch. Accetta `{ "jobIds": ["..."] }` e restituisce gli stessi contatori aggregati della route legacy `POST /api/v1/skills/-/rescan-batch/status`.
...
- `security.signals.dependencyRegistry` è mantenuto per compatibilità con la risposta v1, ma lo scanner di esistenza del registro delle dipendenze è ritirato e questa chiave è sempre `null`.
...
- La risposta riguarda solo la sicurezza. Non include dati Skill Card, stato della scheda generata, elenchi di file degli artefatti o payload dettagliati degli scanner.
...
- `security.signals.dependencyRegistry` è mantenuto per compatibilità con la risposta v1, ma lo scanner di esistenza del registro delle dipendenze è ritirato e questa chiave è sempre `null`.
- L'assenza di una Skill Card non influisce su `ok`, `decision` o `reasons` di questo endpoint; i client dovrebbero leggere localmente `skill-card.md` installato quando hanno bisogno del contenuto della scheda.
- Usa `/verify` quando ti serve l'envelope di verifica Skill Card per una singola skill, `/card` quando ti serve il markdown della scheda generata e `/scan` quando ti servono dati dettagliati degli scanner.
...
Navigazione del catalogo riservata ai Plugin tra pacchetti code-plugin e bundle-plugin.
...
Gli alias dei filtri legacy v1 continuano a essere accettati sugli endpoint di lettura:
...
Esportazione massiva delle ultime release pubbliche dei Plugin per l'analisi offline.
...
Ricerca limitata ai Plugin nei pacchetti code-plugin e bundle-plugin.
...
- Il filtro per categoria è un vero filtro API basato sulle righe del digest delle categorie dei Plugin,
...
- I controlli di ordinamento dell'interfaccia browser per la ricerca di Plugin riordinano i risultati di rilevanza caricati,
...
- Richiede un token API per il proprietario del pacchetto, un proprietario/amministratore del publisher dell'organizzazione,
  moderatore della piattaforma o amministratore della piattaforma.
...
Restituisce il riepilogo esatto di sicurezza e attendibilità della release del pacchetto per i client di installazione. Questa è la superficie di consumo pubblica di OpenClaw per decidere se una release
risolta può essere installata.
...
- `trust.stale` significa che il riepilogo di attendibilità è stato calcolato da input obsoleti e
  dovrebbe essere trattato come se richiedesse un aggiornamento prima di una decisione di autorizzazione ad alta confidenza.
...
- Questo endpoint è intenzionalmente più ristretto degli endpoint di moderazione per proprietari/moderatori.
  ...
  identità dei segnalatori, corpi delle segnalazioni, evidenze private o timeline interne di revisione.
...
- Le versioni di pacchetti legacy restituiscono un artefatto `legacy-zip` e un `downloadUrl`
  ZIP legacy.
...
- Usa il bucket di limite di velocità per i download.
...
Endpoint per amministratori per creare o aggiornare una riga di migrazione di Plugin ufficiale.
...
approvare, mettere in quarantena o revocare artefatti.
...
Scarica uno ZIP di una versione di skill ospitata oppure restituisce un handoff sorgente GitHub per uno
skill corrente basato su GitHub con una scansione `clean` o `suspicious` e senza versione
ospitata.
...
- Gli handoff degli skill basati su GitHub non fanno proxy né mirror dei byte. La risposta JSON
  ...
  e `archiveUrl`; la scansione/stato corrente è un gate e non è inclusa come metadato
...
- Campo payload opzionale: `migrateOwner`. Quando `true` con `ownerHandle`, uno
  skill esistente può essere spostato a quel proprietario se l'attore è admin/proprietario su entrambi
  i publisher corrente e di destinazione. Senza questa adesione esplicita, le modifiche del proprietario sono
  rifiutate.
...
Punti salienti della validazione:
...
- Le pubblicazioni per conto terzi validano comunque l'idoneità al canale ufficiale rispetto all'account proprietario di destinazione.
...
utente condiviso legacy/publisher personale, l'endpoint lo migra prima in un publisher org.
...
senza modificare le righe dell'account Convex Auth. La richiesta deve indicare entrambi gli ID immutabili
degli account provider GitHub; gli handle mutabili sono usati solo come protezione rivolta all'operatore.

Per impostazione predefinita, l'endpoint è in modalità dry-run. Per applicare il ripristino servono `dryRun: false` e
`confirmIdentityVerified: true` dopo che lo staff ha verificato in modo indipendente la continuità tra entrambe
le entità GitHub principali. Il ripristino fallisce in modo chiuso quando il publisher personale corrente dell'utente di destinazione
ha Skills, pacchetti o fonti di Skills GitHub.
Il ripristino migra anche i campi legacy `ownerUserId` per le Skills del publisher recuperato,
...
Le fonti di Skills GitHub sono nell'ambito del publisher e vengono riportate come controllate anziché riscritte.
...
Rimuove il ban da un utente e ripristina le Skills idonee (solo amministratore).
...
contenuti (solo amministratore). Per impostazione predefinita è in modalità dry-run, a meno che `dryRun` sia `false`.
...
Se ospiti autonomamente, servi questo file (o imposta esplicitamente `CLAWHUB_REGISTRY`; legacy `CLAWDHUB_REGISTRY`).
