...
Le scorciatoie web per gli slug vengono risolte tra le famiglie del registro, ma i client API devono usare
...
- Se il token manca o non è valido, il comportamento ripiega sull’applicazione per IP.
- Gli endpoint di scrittura autenticati non devono restituire un semplice `Unauthorized` quando
  ...
  account eliminati, bannati o disabilitati devono ricevere ciascuno un testo utilizzabile, così i client
...
- I risultati vengono restituiti in ordine di rilevanza (similarità degli embedding + boost sui token esatti di slug/nome + un piccolo prior di popolarità).
- La rilevanza è più forte della popolarità. Una corrispondenza precisa di slug o token del nome visualizzato può superare una corrispondenza più vaga con molto più engagement.
...
- La popolarità è scalata logaritmicamente e limitata. Le Skills con alto engagement possono classificarsi più in basso quando il testo della query è una corrispondenza più debole.
...
- Gli alias di rinomina preservano la risoluzione per i vecchi URL e le installazioni che vengono risolte tramite il registro, ma il ranking di ricerca si basa sui metadati canonici della Skill dopo che la rinomina è stata indicizzata. Le statistiche esistenti restano associate alla Skill.
...
- `sort` (facoltativo): `updated` (predefinito), `recommended` (alias: `default`), `createdAt` (alias: `newest`), `downloads`, `stars` (alias: `rating`), gli alias di installazione legacy `installsCurrent`/`installs`/`installsAllTime` mappano a `downloads`, `trending`
- `nonSuspiciousOnly` (facoltativo): `true` per nascondere le Skills sospette (`flagged.suspicious`)
- `nonSuspicious` (facoltativo): alias legacy per `nonSuspiciousOnly`
...
- `metadata.os`: restrizioni OS dichiarate nel frontmatter della Skill (es. `["macos"]`, `["linux"]`). `null` se non dichiarate.
- `metadata.systems`: target di sistema Nix (es. `["aarch64-darwin", "x86_64-linux"]`). `null` se non dichiarati.
...
- Le prove vengono oscurate per i chiamanti pubblici e includono snippet grezzi solo per proprietari/moderatori.
...
- Restituisce `queue.queuedAhead` e `queue.position` durante l'accodamento, così i client possono mostrare quante scansioni manuali prioritarie precedono la richiesta. Le code molto grandi sono limitate e riportate con `queuedAheadIsEstimate: true`.
...
Endpoint autenticato per l'archivio dei report memorizzati per versioni inviate.
...
Route canonica di nuova scansione batch riservata agli amministratori. Accetta la stessa struttura di payload della route legacy `POST /api/v1/skills/-/rescan-batch`.
...
- `ok` è `true` solo quando la versione selezionata ha una Skill Card generata, non è bloccata come malware dalla moderazione e la verifica ClawScan è clean.
- L'identità della skill, l'identità dell'editore e i metadati della versione selezionata sono campi di primo livello dell'envelope (`slug`, `displayName`, `publisherHandle`, `version`, `resolvedFrom`, `tag`, `createdAt`), così l'automazione shell può leggerli senza estrarre wrapper annidati.
...
- `security.signals.dependencyRegistry` è mantenuto per la compatibilità della risposta v1, ma lo scanner di esistenza del registro delle dipendenze è stato ritirato e questa chiave è sempre `null`.
...
- `security.signals` contiene solo prove di supporto a livello di stato; usa `/scan` o la pagina di security-audit di ClawHub per i dettagli completi degli scanner.
- `security.signals.dependencyRegistry` è mantenuto per la compatibilità della risposta v1, ma lo scanner di esistenza del registro delle dipendenze è stato ritirato e questa chiave è sempre `null`.
...
- `category` (facoltativo): filtro per categoria Plugin. Supportato solo quando la
...
- `category` (facoltativo): filtro per categoria Plugin. Valori attuali:
...
- `_errors.json` è incluso quando singole Skill o file non possono essere
  esportati.
...
- Sono accettati anche gli alias legacy dei filtri v1 documentati sotto
  `GET /api/v1/plugins`.
- Il filtro per categoria è un vero filtro API basato su righe del digest delle
  categorie di plugin, non una riscrittura della query di ricerca.
- I risultati sono restituiti in ordine di rilevanza e al momento non sono paginati.
- I controlli di ordinamento dell'interfaccia browser per la ricerca dei plugin riordinano i risultati di rilevanza caricati,
  corrispondendo al comportamento attuale di navigazione in `/skills`.
...
- `trust.reasons` è l'elenco di spiegazioni per l'utente e per l'audit. I codici motivo
  ...
  e `package:malicious`.
- `trust.pending` significa che uno o più input di fiducia sono ancora in attesa di completamento.
- `trust.stale` significa che il riepilogo di fiducia è stato calcolato da input obsoleti e
  dovrebbe essere trattato come da aggiornare prima di una decisione di consenso ad alta affidabilità.
...
- I pacchetti privati restituiscono `404` a meno che il chiamante possa leggere il publisher proprietario.
- Questo endpoint è intenzionalmente più ristretto degli endpoint di moderazione per proprietario/moderatore.
  Espone la decisione di installazione e la spiegazione pubblica, non
  identità dei segnalatori, corpi delle segnalazioni, prove private o timeline di revisione interne.
...
Endpoint moderatore/admin per l'acquisizione di segnalazioni sui pacchetti.
...
Restituisce il contenuto testuale grezzo per un file del pacchetto.
...
- Usa il bucket di limite di frequenza in lettura, non il bucket di download.
...
- Usa il bucket di limite di frequenza per il download.
...
Scarica uno ZIP di una versione Skill ospitata, oppure restituisce un handoff del sorgente GitHub per una
...
- È accettato anche un body JSON con `files` (basati su storageId).
...
- Campo payload facoltativo: `migrateOwner`. Quando `true` con `ownerHandle`, una
  Skill esistente può passare a quel proprietario se l'attore è un admin/proprietario sia sul
  publisher corrente sia su quello di destinazione. Senza questo consenso esplicito, le modifiche del proprietario vengono
  rifiutate.
...
- I campi form consentiti sono `payload`, blob `files` ripetuti oppure un riferimento tarball `clawpack`.
...
- I body JSON e i metadati `payload.files` / `payload.artifact` forniti dal chiamante
  vengono rifiutati.
...
Body JSON facoltativo:

```json
{ "reason": "Held for moderation pending legal review." }
```

Quando presente, `reason` viene archiviato come nota di moderazione della Skill e copiato nel registro di audit.
...
Le rimozioni operate da moderatore/admin e le rimozioni di sicurezza non scadono in questo modo.
...
Creazione self-service autenticata di un publisher org. Crea un nuovo publisher org e aggiunge il
...
Solo admin. Riserva slug radice e nomi pacchetto per un legittimo proprietario senza pubblicare una
release. I nomi pacchetto diventano pacchetti placeholder privati senza righe di release, così lo stesso
...
immutabili; gli handle mutabili vengono usati solo come protezione rivolta all'operatore.

L'endpoint usa per impostazione predefinita l'esecuzione di prova. L'applicazione del recupero richiede `dryRun: false` e
`confirmIdentityVerified: true` dopo che lo staff ha verificato in modo indipendente la continuità tra entrambi
i principali GitHub. Il recupero fallisce in modo chiuso quando l'attuale publisher personale dell'utente di destinazione
...
per l'handle recuperato viene riassegnata all'utente sostitutivo, così la successiva sincronizzazione del profilo
...
Le sorgenti di skill GitHub hanno ambito publisher e vengono riportate come controllate anziché riscritte.
...
- `merge` nasconde l'inserzione sorgente e reindirizza lo slug sorgente all'inserzione di destinazione.
...
Cambia il motivo memorizzato per un ban esistente senza rimuovere il ban né ripristinare
contenuti (solo amministratore). Per impostazione predefinita usa l'esecuzione di prova, a meno che `dryRun` non sia `false`.
...
che predispongono un tarball ClawPack devono inviare l'id di archiviazione risultante come
`clawpack` e il ticket restituito come `clawpackUploadTicket`.

## Rilevamento del registro (`/.well-known/clawhub.json`)

La CLI può rilevare le impostazioni del registro/autenticazione dal sito:
