...
read_when:
    ...
    - Risoluzione dei problemi delle richieste CLI ↔ registro
...
Le directory di terze parti possono usare gli endpoint pubblici in lettura per elencare o cercare le skill di ClawHub. Memorizza i risultati nella cache, rispetta `429`/`Retry-After`, rimanda gli utenti alla scheda canonica di ClawHub (`https://clawhub.ai/<owner>/<slug>`) ed evita di suggerire un'approvazione di ClawHub per il sito di terze parti. Non tentare di replicare contenuti nascosti, privati o bloccati dalla moderazione al di fuori della superficie dell'API pubblica.

Le scorciatoie degli slug web vengono risolte tra famiglie di registry, ma i client API dovrebbero usare
...
- Gli endpoint autenticati in scrittura non dovrebbero restituire un semplice `Unauthorized` quando
  il server conosce il motivo. Token mancanti, token non validi/revocati e
  account eliminati/bannati/disabilitati dovrebbero ciascuno ricevere testo azionabile, così i client
...
- Se non è disponibile alcun IP client attendibile, le richieste anonime di download usano un bucket di fallback con ambito sull'endpoint invece di un unico bucket globale `ip:unknown`. Le richieste anonime in lettura/scrittura continuano a usare il bucket sconosciuto condiviso, così il routing con IP mancante resta visibile e conservativo.
...
- La pertinenza è più forte della popolarità. Una corrispondenza precisa dello slug o di un token del nome visualizzato può superare una corrispondenza meno stretta con molti più download.
...
- I download vengono usati come piccolo prior scalato logaritmicamente e come criterio di spareggio, non come segnale di ranking principale. Le skill con molti download possono classificarsi più in basso quando il testo della query è una corrispondenza più debole.
...
- Non rinominare uno slug solo per inseguire una query, a meno che il nuovo slug non sia un nome canonico migliore nel lungo periodo. I vecchi slug diventano alias di reindirizzamento, ma l'URL canonico, lo slug visualizzato e i futuri digest di ricerca usano il nuovo slug.
- Gli alias di rinomina preservano la risoluzione per i vecchi URL e le installazioni che risolvono tramite il registry, ma il ranking di ricerca si basa sui metadati canonici della skill dopo che la rinomina è stata indicizzata. Le statistiche esistenti restano associate alla skill.
- Se una skill è inaspettatamente invisibile, controlla prima lo stato di moderazione con `clawhub inspect <slug>` dopo aver effettuato l'accesso, prima di cambiare metadati legati al ranking.
...
si imposta `status` di nuovo su `open`. Passa `finalAction: "hide"` con una segnalazione sottoposta a triage
...
- Include lo stato normalizzato di verifica più i dettagli specifici dello scanner.
...
- `capabilityTag` (facoltativo): filtro delle capacità per i pacchetti Plugin
...
- `npmMirror` (facoltativo): `true`/`1` per mostrare le versioni dei pacchetti supportate da ClawPack
...
- `artifactKind` (facoltativo): `legacy-zip` o `npm-pack`
- `npmMirror` (facoltativo): `true`/`1` per cercare versioni di pacchetti supportate da ClawPack
  disponibili tramite il mirror npm
...
capacità, verifica, metadati degli artefatti e dati di scansione.

Note:

- `version.artifact.kind` è `legacy-zip` per gli archivi di pacchetti del vecchio modello o
  `npm-pack` per le release supportate da ClawPack.
...
Endpoint per proprietari/moderatori per la visibilità sulla moderazione del pacchetto.
...
- Le release senza storage ClawPack vengono etichettate `legacy-zip`.
...
- `tag` (opzionale): nome del tag (es. `latest`)
...
- Campo payload opzionale: `migrateOwner`. Quando è `true` con `ownerHandle`, una
  ...
  publisher corrente sia su quello di destinazione. Senza questo consenso esplicito, le modifiche di proprietario
  vengono rifiutate.
...
Le eliminazioni reversibili avviate dal proprietario riservano lo slug per 30 giorni, poi lo slug può essere richiesto da
un altro publisher. La risposta di eliminazione include `slugReservedUntil` quando questa scadenza si applica.
I nascondimenti moderator/admin e le rimozioni di sicurezza non scadono in questo modo.
...
### Endpoint di trasferimento proprietà
