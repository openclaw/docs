...
x-i18n:
    generated_at: "2026-07-03T00:57:34Z"
    model: gpt-5.5
...
- Se non è disponibile alcun IP client attendibile, le richieste anonime usano bucket di fallback
  limitati solo per tipo di limite di frequenza. Questi bucket di fallback non includono
...
- I risultati sono restituiti in ordine di rilevanza (somiglianza degli embedding + boost per token esatti di slug/nome + un piccolo prior di popolarità).
- La rilevanza pesa più della popolarità. Una corrispondenza precisa con uno slug o un token del nome visualizzato può superare una corrispondenza più ampia con coinvolgimento molto più forte.
- Il testo ASCII viene tokenizzato sui confini di parole e punteggiatura. Ad esempio, `personal-map` contiene un token autonomo `map`, mentre `amap-jsapi-skill` contiene `amap`, `jsapi` e `skill`; cercare `map` quindi assegna a `personal-map` una corrispondenza lessicale più forte rispetto a `amap-jsapi-skill`.
- La popolarità è scalata logaritmicamente e limitata. Le Skills con alto coinvolgimento possono posizionarsi più in basso quando il testo della query corrisponde meno.
...
- Quando `nonSuspiciousOnly=true`, gli ordinamenti basati su cursore possono restituire meno di `limit` elementi su una pagina perché le skill sospette vengono filtrate dopo il recupero della pagina.
...
- Le evidenze sono redatte per i chiamanti pubblici e includono frammenti grezzi solo per proprietari/moderatori.
...
Endpoint moderatore/amministratore per risolvere o riaprire le segnalazioni delle skill.
...
si reimposta `status` a `open`. Passa `finalAction: "hide"` con una segnalazione triaged
...
Le scansioni di caricamento locale non sono più supportate. Le richieste che usano
...
- Le scansioni pubblicate richiedono accesso di gestione del proprietario/editore, oppure autorità di moderatore/amministratore della piattaforma.
...
Endpoint autenticato per l’archivio del report memorizzato per le versioni inviate.

- Richiede accesso di gestione del proprietario/editore alla skill o al plugin, oppure autorità di moderatore/amministratore della piattaforma.
- Restituisce i risultati di scansione memorizzati per la versione esatta inviata, incluse le versioni bloccate o nascoste.
- `kind` usa come predefinito `skill`; usa `kind=plugin` per scansioni di plugin/pacchetto.
- Restituisce la stessa struttura ZIP dei download delle richieste di scansione.

### `POST /api/v1/skills/-/scan/batch`

Route canonica solo per amministratori per la nuova scansione batch. Accetta la stessa struttura di payload della route legacy `POST /api/v1/skills/-/rescan-batch`.
...
- `security.signals` contiene evidenze di supporto degli scanner, come `staticScan`, `virusTotal` e `skillSpector`.
...
- La risposta riguarda solo la sicurezza. Non include dati della Skill Card, stato della scheda generata, elenchi dei file degli artefatti o payload dettagliati degli scanner.
- `security.signals` contiene solo evidenze di supporto a livello di stato; usa `/scan` o la pagina di audit di sicurezza ClawHub per i dettagli completi degli scanner.
- `security.signals.dependencyRegistry` è mantenuto per compatibilità con la risposta v1, ma lo scanner di esistenza del registro delle dipendenze è stato ritirato e questa chiave è sempre `null`.
...
- Usa `/verify` quando hai bisogno dell’envelope di verifica della Skill Card per una singola skill, `/card` quando hai bisogno del Markdown della scheda generata e `/scan` quando hai bisogno di dati dettagliati degli scanner.
...
- `category` (facoltativo): filtro per categoria di Plugin. Supportato solo quando la
  ...
  gli alias legacy dei filtri v1 sono documentati sotto `GET /api/v1/plugins`.
...
- `POST /api/v1/packages` resta riservato solo alle release di code-plugin e bundle-plugin.
...
- `category` (facoltativo): filtro per categoria di Plugin. Supportato solo quando la
  ...
  legacy dei filtri v1 sono documentati sotto `GET /api/v1/plugins`.
...
Sfoglia il catalogo dei soli Plugin tra pacchetti code-plugin e bundle-plugin.
...
Sull'endpoint unificato `/api/v1/packages` è riservato ai Plugin; usa
...
Esportazione in blocco delle ultime Skills pubbliche per analisi offline.
...
- Le Skills attuali supportate da GitHub con una scansione `clean` o `suspicious` includono
  `_source_handoff.json` con `sourceRef: "public-github"`, repo, commit, path,
...
- Il filtro per categoria è un vero filtro API basato su righe di digest della
  categoria del Plugin, non una riscrittura della query di ricerca.
...
- I controlli di ordinamento dell'interfaccia browser per la ricerca Plugin riordinano i risultati di rilevanza caricati,
  corrispondendo al comportamento di esplorazione attuale di `/skills`.
...
- Anche le Skills possono essere risolte tramite questa route nel catalogo unificato.
- I pacchetti privati restituiscono `404` a meno che il chiamante non possa leggere il publisher proprietario.

### `DELETE /api/v1/packages/{name}`

Esegue una soft delete di un pacchetto e di tutti i rilasci.
...
- I rilasci ClawPack includono i campi compatibili con npm `npmIntegrity`, `npmShasum` e
  `npmTarballName`.
- `version.sha256hash` è un metadato di compatibilità deprecato per i vecchi client. Calcola
...
Restituisce il riepilogo esatto di sicurezza e fiducia del rilascio del pacchetto per i client
di installazione. Questa è la superficie pubblica di consumo OpenClaw per decidere se un
...
- `trust.moderationState` è nullable. È `null` quando non esiste alcuna moderazione manuale del rilascio.
...
- `trust.stale` indica che il riepilogo di fiducia è stato calcolato da input obsoleti e
  dovrebbe essere trattato come richiedente un aggiornamento prima di una decisione di autorizzazione ad alta confidenza.
...
- Usa il bucket di limite di velocità dei download.
...
- `packageName` viene normalizzato come nome npm; il pacchetto può essere assente per le migrazioni
  pianificate.
- Questo traccia solo la readiness della migrazione. Non muta OpenClaw né genera
  ClawPack.
...
`finalAction: "revoke"` con un report confermato per applicare la moderazione del rilascio nello
...
Endpoint moderatore/amministratore per la revisione dei rilasci dei pacchetti.
...
- `revoked`: bloccato dopo che un rilascio era stato considerato attendibile in precedenza.
...
- Usa come valore predefinito il rilascio più recente.
- Usa il bucket del limite di lettura, non il bucket di download.
...
Scarica l'archivio ZIP deterministico legacy per un rilascio del pacchetto.
...
- Usa il bucket del limite di download.
...
Scarica uno ZIP di versione Skills ospitata oppure restituisce un passaggio a una sorgente GitHub per una
...
- I passaggi a Skills basate su GitHub non fanno proxy né mirror dei byte. La risposta JSON
  include `sourceRef: "public-github"`, `repo`, `commit`, `path`, `contentHash`
  e `archiveUrl`; lo stato di scansione/corrente è un gate e non viene incluso come metadato
...
- Campo facoltativo del payload: `ownerHandle`. Quando presente, l'API risolve quell'
...
- Campo facoltativo del payload: `migrateOwner`. Quando `true` con `ownerHandle`, una
  Skills esistente può essere spostata a quel proprietario se l'attore è amministratore/proprietario sia sull'
  editore corrente sia su quello di destinazione. Senza questa adesione esplicita, le modifiche di proprietario vengono
  rifiutate.
...
- I campi form consentiti sono `payload`, blob `files` ripetuti oppure un riferimento a tarball `clawpack`.
  `clawpack` può essere un blob `.tgz` o un ID storage restituito dal
  flusso upload-url. Le pubblicazioni con ID storage preparato devono includere anche il
...
- Solo l'editore dell'organizzazione `openclaw` e gli editori personali degli attuali membri
  dell'organizzazione `openclaw` possono pubblicare nel canale `official`.
- Le pubblicazioni per conto terzi convalidano comunque l'idoneità al canale ufficiale rispetto all'account proprietario di destinazione.
...
Solo amministratori. Garantisce che esista un editore organizzazione per un handle. Se l'handle punta ancora a un
utente condiviso legacy/editore personale, l'endpoint lo migra prima in un editore organizzazione.
Per un'organizzazione appena creata, fornisci `memberHandle`; l'amministratore che agisce non viene aggiunto come membro.
`memberRole` è predefinito su `owner`.
...
Creazione autenticata self-service di un editore organizzazione. Crea un nuovo editore organizzazione e aggiunge il
...
Solo amministratori. Riserva slug radice e nomi di pacchetto per un legittimo proprietario senza pubblicare un
rilascio. I nomi dei pacchetti diventano pacchetti segnaposto privati senza righe di rilascio, così lo stesso
...
senza modificare le righe degli account Convex Auth. La richiesta deve indicare entrambi gli ID account provider GitHub
immutabili; gli handle mutabili sono usati solo come protezione rivolta all'operatore.

L'endpoint usa per impostazione predefinita l'esecuzione simulata. Per applicare il ripristino sono richiesti `dryRun: false` e
`confirmIdentityVerified: true` dopo che lo staff ha verificato indipendentemente la continuità tra entrambe
le entità principali GitHub. Il ripristino fallisce in modo chiuso quando l'attuale
publisher personale dell'utente di destinazione ha Skills, pacchetti o fonti Skills GitHub.
Il ripristino migra anche i campi legacy `ownerUserId` per le Skills del publisher ripristinato,
gli alias degli slug delle Skills, i pacchetti, gli avvisi dell'ispettore dei pacchetti e le righe derivate del digest di ricerca, in modo che
...
Le fonti Skills GitHub sono con ambito publisher e vengono segnalate come controllate anziché riscritte.
...
Blocca un utente ed elimina definitivamente le Skills possedute (solo moderatore/amministratore).
...
Sblocca un utente e ripristina le Skills idonee (solo amministratore).
...
Modifica il motivo memorizzato per un blocco esistente senza sbloccare o ripristinare
i contenuti (solo amministratore). Usa per impostazione predefinita l'esecuzione simulata, a meno che `dryRun` sia `false`.
...
Se usi self-hosting, servi questo file (oppure imposta esplicitamente `CLAWHUB_REGISTRY`; legacy `CLAWDHUB_REGISTRY`).
