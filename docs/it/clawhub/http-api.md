...
x-i18n:
    generated_at: "2026-07-05T06:44:30Z"
    model: gpt-5.5
...
Directory di terze parti possono usare gli endpoint pubblici di lettura per elencare o cercare le Skills di ClawHub. Memorizza i risultati nella cache, rispetta `429`/`Retry-After`, rimanda gli utenti alla scheda ClawHub canonica (`https://clawhub.ai/<owner>/skills/<slug>`) ed evita di implicare un'approvazione di ClawHub per il sito di terze parti. Non tentare di replicare contenuti nascosti, privati o bloccati dalla moderazione fuori dalla superficie dell'API pubblica.
...
- Gli endpoint di scrittura autenticati non dovrebbero restituire un semplice `Unauthorized` quando
  il server ne conosce il motivo. Token mancanti, token non validi/revocati e
...
- Se non è disponibile alcun IP client attendibile, le richieste anonime usano bucket di fallback
  con ambito limitato solo al tipo di limite di frequenza. Questi bucket di fallback non includono
...
- I risultati sono restituiti in ordine di rilevanza (similarità degli embedding + boost per token esatti di slug/nome + un piccolo prior di popolarità).
- La rilevanza è più forte della popolarità. Una corrispondenza precisa di slug o token del nome visualizzato può superare una corrispondenza più vaga con coinvolgimento molto maggiore.
...
- La popolarità è scalata logaritmicamente e limitata. Skills con alto coinvolgimento possono posizionarsi più in basso quando il testo della query ha una corrispondenza più debole.
...
Indicazioni di scopribilità per publisher:
...
- Le evidenze sono redatte per i chiamanti pubblici e includono snippet grezzi solo per proprietari/moderatori.
...
- I processi di scansione sono asincroni. Le richieste di scansione manuali hanno priorità rispetto al normale lavoro di pubblicazione/backfill, ma il completamento dipende comunque dalla disponibilità dei worker.
...
- Restituisce `queue.queuedAhead` e `queue.position` mentre è in coda, così i client possono mostrare quante scansioni manuali con priorità precedono la richiesta. Le code molto grandi sono limitate e riportate con `queuedAheadIsEstimate: true`.
...
Route canonica, solo amministratori, per la nuova scansione batch. Accetta la stessa struttura di payload del legacy `POST /api/v1/skills/-/rescan-batch`.

### `POST /api/v1/skills/-/scan/batch/status`

Route canonica, solo amministratori, per lo stato batch. Accetta `{ "jobIds": ["..."] }` e restituisce gli stessi contatori aggregati del legacy `POST /api/v1/skills/-/rescan-batch/status`.
...
- `version` (opzionale): stringa di versione specifica.
- `tag` (opzionale): risolve una versione etichettata (per esempio `latest`).
...
- L'identità della skill, l'identità dell'editore e i metadati della versione selezionata sono campi dell'involucro di primo livello (`slug`, `displayName`, `publisherHandle`, `version`, `resolvedFrom`, `tag`, `createdAt`), così l'automazione shell può leggerli senza decomprimere wrapper annidati.
...
- `security.signals` contiene solo prove di supporto a livello di stato; usa `/scan` o la pagina di audit di sicurezza ClawHub per i dettagli completi dello scanner.
- `security.signals.dependencyRegistry` viene mantenuto per compatibilità con la risposta v1, ma lo scanner di esistenza del registro delle dipendenze è ritirato e questa chiave è sempre `null`.
...
- Usa `/verify` quando ti serve l'involucro di verifica della Skill Card per una singola skill, `/card` quando ti serve il markdown della scheda generata e `/scan` quando ti servono dati dettagliati dello scanner.
...
- Le voci Skill continuano a basarsi sul registro delle Skill e possono ancora essere pubblicate solo tramite `POST /api/v1/skills`.
- `POST /api/v1/packages` è ancora riservato alle release code-plugin e bundle-plugin.
- I chiamanti anonimi vedono solo i canali dei pacchetti pubblici.
- I chiamanti autenticati possono vedere nei risultati di elenco/ricerca i pacchetti privati degli editori di cui fanno parte.
...
- I chiamanti anonimi vedono solo i canali dei pacchetti pubblici.
- I chiamanti autenticati possono cercare i pacchetti privati degli editori di cui fanno parte.
- `channel=private` restituisce solo i pacchetti che il chiamante autenticato può leggere.
...
Esportazione in blocco delle release pubbliche più recenti dei Plugin per analisi offline.
...
- `category` (opzionale): filtro di categoria del Plugin. Valori attuali:
  `channels`, `models`, `memory`, `context`, `voice`, `media`, `web`,
  `tools`, `runtime`, `gateway`, `security`, `other`.
...
- I controlli di ordinamento dell'interfaccia browser per la ricerca Plugin riordinano i risultati di pertinenza caricati,
  corrispondendo al comportamento di esplorazione corrente di `/skills`.
...
- `version.artifact.kind` è `legacy-zip` per gli archivi pacchetto del vecchio formato o
...
- Le release ClawPack includono campi compatibili con npm `npmIntegrity`, `npmShasum` e
  `npmTarballName`.
- `version.sha256hash` è un metadato di compatibilità deprecato per i client vecchi. Calcola
...
Restituisce il riepilogo esatto di sicurezza e attendibilità della release del pacchetto per i
client di installazione. Questa è la superficie pubblica di consumo OpenClaw per decidere se una
release risolta possa essere installata.
...
- `trust.reasons` è l'elenco di spiegazioni rivolto all'utente e di audit. I codici motivo
...
- Usa il bucket del limite di lettura, non il bucket del limite di download.
...
- Continuano ad applicarsi i controlli di moderazione e di accesso ai pacchetti privati.
...
Scarica uno ZIP di una versione di skill ospitata, oppure restituisce un passaggio alla fonte GitHub per una
...
- I passaggi alle skill basate su GitHub non fungono da proxy né da mirror dei byte. La risposta JSON
...
- Campo payload opzionale: `migrateOwner`. Quando `true` con `ownerHandle`, una
  skill esistente può spostarsi a quel proprietario se l'attore è un amministratore/proprietario su entrambi
  i publisher corrente e di destinazione. Senza questo opt-in, le modifiche del proprietario vengono
  rifiutate.
...
- I campi modulo consentiti sono `payload`, blob `files` ripetuti, oppure un riferimento a tarball
...
Gli oscuramenti da parte di moderatore/amministratore e le rimozioni di sicurezza non scadono in questo modo.
...
Solo amministratori. Garantisce che esista un publisher org per un handle. Se l'handle punta ancora a un
publisher legacy condiviso utente/personale, l'endpoint lo migra prima in un publisher org.
Per un'org appena creata, fornisci `memberHandle`; l'amministratore attore non viene aggiunto come membro.
...
immutabili; gli handle mutabili sono usati solo come controllo rivolto all'operatore.
...
i percorsi con proprietario diretto concordino con la nuova autorità del publisher. Una prenotazione attiva di handle protetto
per l'handle recuperato viene inoltre riassegnata all'utente sostitutivo, così la successiva
...
Le sorgenti di skill GitHub hanno ambito di publisher e vengono segnalate come controllate anziché riscritte.
...
Modifica il motivo salvato per un ban esistente senza revocare il ban o ripristinare
i contenuti (solo amministratore). Usa per impostazione predefinita l'esecuzione di prova, a meno che `dryRun` sia `false`.
