...
x-i18n:
    generated_at: "2026-07-02T00:56:36Z"
    model: gpt-5.5
...
Le directory di terze parti possono usare gli endpoint pubblici di lettura per elencare o cercare skill ClawHub. Memorizzate i risultati nella cache, rispettate `429`/`Retry-After`, rimandate gli utenti alla voce ClawHub canonica (`https://clawhub.ai/<owner>/skills/<slug>`) ed evitate di implicare un'approvazione di ClawHub per il sito di terze parti. Non tentate di replicare contenuti nascosti, privati o bloccati dalla moderazione al di fuori della superficie dell'API pubblica.

Le scorciatoie slug web si risolvono tra le famiglie del registro, ma i client API dovrebbero usare
...
- `X-RateLimit-Remaining` / `RateLimit-Remaining`: budget rimanente esatto quando presente.
  Le richieste riuscite in shard omettono questo header invece di restituire un valore globale approssimativo.
...
- I risultati vengono restituiti in ordine di rilevanza (similarità degli embedding + boost per token esatti di slug/nome + un piccolo prior di popolarità).
- La rilevanza è più forte della popolarità. Una corrispondenza precisa di slug o token del nome visualizzato può superare una corrispondenza più generica con molto più engagement.
...
- `createdAt` è stabile per i crawl di nuove skill; `updated` cambia quando skill esistenti vengono ripubblicate.
...
- Owner e moderatori possono accedere ai dettagli di moderazione per skill nascoste.
...
si reimposta `status` a `open`. Passa `finalAction: "hide"` con una segnalazione valutata
...
- `provenance` è `server-resolved-github-import` solo quando ClawHub ha risolto e memorizzato repo/ref/commit/path GitHub durante la pubblicazione o l'importazione; altrimenti è `unavailable`.
...
- La risposta è solo di sicurezza. Non include dati Skill Card, stato della scheda generata, elenchi di file artefatto o payload dettagliati degli scanner.
...
- `security.signals.dependencyRegistry` viene mantenuto per compatibilità della risposta v1, ma lo scanner di esistenza del registro delle dipendenze è ritirato e questa chiave è sempre `null`.
- L'assenza della Skill Card non influisce su `ok`, `decision` o `reasons` di questo endpoint; i client dovrebbero leggere `skill-card.md` installato localmente quando hanno bisogno del contenuto della scheda.
...
- Imposta per default la versione più recente.
...
- `limit` (opzionale): numero intero (1-250), predefinito `250`.
...
Ricerca solo dei Plugin tra i pacchetti code-plugin e bundle-plugin.
...
- I controlli di ordinamento dell'interfaccia browser per la ricerca dei Plugin riordinano i risultati di rilevanza caricati,
  corrispondendo all'attuale comportamento di navigazione di `/skills`.
...
- `version.artifact.kind` è `legacy-zip` per gli archivi di pacchetti del mondo precedente oppure
...
- `trust.reasons` è l'elenco di spiegazioni rivolte all'utente e per l'audit. I codici motivo
...
- `trust.stale` indica che il riepilogo di attendibilità è stato calcolato da input obsoleti e
  dovrebbe essere trattato come richiedente un aggiornamento prima di una decisione di autorizzazione ad alta affidabilità.
...
- Questo endpoint è intenzionalmente più ristretto degli endpoint di moderazione per proprietari/moderatori.
  ...
  identità dei segnalanti, corpi delle segnalazioni, prove private o timeline interne di revisione.
...
- Usa il bucket del limite di frequenza di lettura, non il bucket di download.
...
- Usa il bucket del limite di frequenza di download.
- Gli header di download includono SHA-256 ClawHub più metadati di integrità/shasum npm.
...
- Campo payload opzionale: `migrateOwner`. Quando `true` con `ownerHandle`, una
  skill esistente può passare a quel proprietario se l'attore è admin/proprietario sia sui
  publisher corrente sia su quello di destinazione. Senza questo opt-in, le modifiche al proprietario vengono
  rifiutate.
...
- I campi form consentiti sono `payload`, blob `files` ripetuti o un riferimento a tarball
...
Per un'organizzazione appena creata, fornisci `memberHandle`; l'admin che esegue l'azione non viene aggiunto come membro.
...
Per impostazione predefinita, l'endpoint usa l'esecuzione di prova. Applicare il ripristino richiede `dryRun: false` e
`confirmIdentityVerified: true` dopo che lo staff ha verificato in modo indipendente la continuità tra entrambi
i principal GitHub. Il ripristino fallisce in modo chiuso quando il publisher personale corrente dell'utente di destinazione
ha Skills, pacchetti o fonti GitHub di Skills.
...
sincronizzazione del profilo non potrà ripristinare l'autorità concorrente dell'utente precedente. Ogni tabella primaria è limitata a
100 righe per transazione di applicazione; ripristini più grandi devono prima usare una migrazione del proprietario riprendibile.
Le fonti GitHub delle Skills hanno ambito del publisher e vengono segnalate come controllate anziché riscritte.
...
Banna un utente ed elimina definitivamente le Skills di proprietà (solo moderatore/amministratore).
...
Rimuovi il ban da un utente e ripristina le Skills idonee (solo amministratore).
...
Cambia il motivo memorizzato per un ban esistente senza rimuovere il ban o ripristinare
contenuti (solo amministratore). Per impostazione predefinita usa l'esecuzione di prova, a meno che `dryRun` sia `false`.
