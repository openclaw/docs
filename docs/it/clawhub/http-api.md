...
x-i18n:
    generated_at: "2026-07-01T20:23:01Z"
    model: gpt-5.5
...
Le scorciatoie degli slug web vengono risolte tra famiglie di registri, ma i client API devono usare
...
- Se non è disponibile alcun IP client attendibile, le richieste anonime usano bucket di fallback
  ...
  parametri di artefatti forniti dal chiamante.
...
- `highlightedOnly` (opzionale): `true` per filtrare solo le Skills in evidenza
...
- I risultati sono restituiti in ordine di pertinenza (similarità degli embedding + boost per token esatti di slug/nome + un piccolo prior di popolarità).
- La pertinenza è più forte della popolarità. Una corrispondenza precisa di slug o token del nome visualizzato può superare una corrispondenza più vaga con coinvolgimento molto più elevato.
...
- La popolarità è scalata logaritmicamente e limitata. Skills ad alto coinvolgimento possono classificarsi più in basso quando il testo della query è una corrispondenza più debole.
...
- Gli alias di rinomina preservano la risoluzione per vecchi URL e installazioni che passano dal registro, ma il ranking di ricerca si basa sui metadati canonici della Skill dopo che la rinomina è stata indicizzata. Le statistiche esistenti restano con la Skill.
...
- `createdAt` è stabile per crawl di nuove Skill; `updated` cambia quando Skills esistenti vengono ripubblicate.
- Quando `nonSuspiciousOnly=true`, gli ordinamenti basati su cursore possono restituire meno di `limit` elementi su una pagina perché le Skills sospette vengono filtrate dopo il recupero della pagina.
...
- I vecchi slug creati dai flussi di rinomina/unione del proprietario vengono risolti alla Skill canonica.
...
Endpoint moderatore/amministratore per l'acquisizione delle segnalazioni di Skills.
...
Endpoint moderatore/amministratore per risolvere o riaprire segnalazioni di Skills.
...
- Le scansioni pubblicate richiedono accesso di gestione owner/publisher, oppure autorità di moderatore/amministratore della piattaforma.
- Le scansioni pubblicate riscrivono i risultati solo quando `update: true` e la scansione si completa correttamente.
...
Endpoint autenticato dell'archivio del report.

- Richiede una scansione completata con successo; le scansioni non terminali restituiscono `409`.
...
Endpoint autenticato dell'archivio dei report archiviati per le versioni inviate.

- Richiede accesso di gestione owner/publisher alla skill o al plugin, oppure autorità di moderatore/amministratore della piattaforma.
- Restituisce i risultati di scansione archiviati per la versione esatta inviata, incluse le versioni bloccate o nascoste.
- `kind` ha valore predefinito `skill`; usa `kind=plugin` per le scansioni di plugin/package.
- Restituisce la stessa forma ZIP dei download delle richieste di scansione.

### `POST /api/v1/skills/-/scan/batch`

Route canonica solo per amministratori per la nuova scansione batch. Accetta la stessa forma di payload della route legacy `POST /api/v1/skills/-/rescan-batch`.

### `POST /api/v1/skills/-/scan/batch/status`

Route canonica solo per amministratori per lo stato batch. Accetta `{ "jobIds": ["..."] }` e restituisce gli stessi contatori aggregati della route legacy `POST /api/v1/skills/-/rescan-batch/status`.
...
- `version` (opzionale): stringa della versione specifica.
...
- La risposta è solo di sicurezza. Non include dati della Skill Card, stato della card generata, elenchi dei file artefatto o payload dettagliati degli scanner.
...
- `category` (facoltativo): filtro categoria Plugin. Supportato solo quando la
  ...
  alias dei filtri legacy v1 sono documentati in `GET /api/v1/plugins`.

Note:

- Valori non validi per `family`, `channel`, `isOfficial`, `featured`,
  `highlightedOnly` o `sort` restituiscono `400`. I parametri query sconosciuti vengono ignorati.
...
- Valori non validi per `family`, `channel`, `isOfficial`, `featured` o
  `highlightedOnly` restituiscono `400`. I parametri query sconosciuti vengono ignorati.
- I chiamanti anonimi vedono solo i canali di pacchetti pubblici.
...
- Le skill attuali basate su GitHub con una scansione `clean` o `suspicious` includono
...
Esportazione in blocco degli ultimi rilasci pubblici dei Plugin per l'analisi offline.
...
- Ogni Plugin esportato include i file archiviati dell'ultimo rilascio.
...
- `limit` (facoltativo): intero (1-100)
- `isOfficial` (facoltativo): `true` o `false`
- `category` (facoltativo): filtro della categoria del Plugin. Valori attuali:
  `channels`, `models`, `memory`, `context`, `voice`, `media`, `web`,
  `tools`, `runtime`, `gateway`, `security`, `other`.

Note:

- Sono accettati anche gli alias dei filtri legacy v1 documentati in
  `GET /api/v1/plugins`.
- Il filtro per categoria è un vero filtro API basato su righe digest delle
  categorie dei Plugin, non una riscrittura della query di ricerca.
- I risultati vengono restituiti in ordine di pertinenza e attualmente non sono
  paginati.
- I controlli di ordinamento dell'interfaccia browser per la ricerca dei Plugin
  riordinano i risultati di pertinenza caricati, in linea con il comportamento
  attuale di navigazione di `/skills`.
...
Esegue la soft delete di un pacchetto e di tutti i rilasci.

Note:

- Richiede un token API per il proprietario del pacchetto, un proprietario/amministratore
  del publisher dell'organizzazione, un moderatore della piattaforma o un amministratore
  della piattaforma.
...
- `version.artifact.kind` è `legacy-zip` per gli archivi di pacchetti del vecchio
  mondo o `npm-pack` per i rilasci basati su ClawPack.
- I rilasci ClawPack includono i campi compatibili con npm `npmIntegrity`,
  `npmShasum` e `npmTarballName`.
- `version.sha256hash` è un metadato di compatibilità deprecato per i vecchi client.
  Esegue l'hash dei byte ZIP esatti restituiti da `/api/v1/packages/{name}/download`.
  I client moderni dovrebbero usare `version.artifact.sha256`, che identifica
  l'artefatto canonico del rilascio.
- `version.vtAnalysis`, `version.llmAnalysis` e `version.staticScan` sono inclusi
  quando esistono dati di scansione.
- I pacchetti privati restituiscono `404` a meno che il chiamante possa leggere il publisher proprietario.

### `GET /api/v1/packages/{name}/versions/{version}/security`

Restituisce il riepilogo esatto di sicurezza e attendibilità del rilascio del
pacchetto per i client di installazione. Questa è la superficie pubblica di
consumo di OpenClaw per decidere se un rilascio risolto può essere installato.

Autenticazione:

- Endpoint di lettura pubblico. Non è richiesto alcun token di proprietario,
  publisher, moderatore o amministratore.
...
- `release.releaseId`, `release.version` e `release.createdAt` identificano
  il rilascio esatto che è stato valutato.
- `release.artifactKind`, `release.artifactSha256`, `release.npmIntegrity`,
  `release.npmShasum` e `release.npmTarballName` sono presenti quando noti per
  l'artefatto del rilascio.
- `trust.scanStatus` è lo stato di attendibilità effettivo derivato dagli input
  dello scanner e dalla moderazione manuale del rilascio.
- `trust.moderationState` è nullable. È `null` quando non esiste alcuna
  moderazione manuale del rilascio.
- `trust.blockedFromDownload` è il segnale di blocco dell'installazione. OpenClaw
  e altri client di installazione dovrebbero bloccare l'installazione quando
  questo valore è `true` invece di ricalcolare le regole di blocco dai campi
  dello scanner o della moderazione.
- `trust.reasons` è l'elenco di spiegazioni rivolto all'utente e per l'audit.
  I codici motivo sono stringhe stabili e compatte, come `manual:quarantined`,
  `scan:malicious` e `package:malicious`.
- `trust.pending` indica che uno o più input di attendibilità sono ancora in
  attesa di completamento.
- `trust.stale` indica che il riepilogo di attendibilità è stato calcolato da
  input obsoleti e dovrebbe essere trattato come da aggiornare prima di una
  decisione di autorizzazione ad alta confidenza.

Note:

- Questo endpoint è esatto per versione. I client dovrebbero chiamarlo dopo aver
  risolto la versione del pacchetto che intendono installare, non solo dopo aver
  letto gli ultimi metadati del pacchetto.
- I pacchetti privati restituiscono `404` a meno che il chiamante possa leggere il publisher proprietario.
- Questo endpoint è intenzionalmente più ristretto degli endpoint di moderazione
  per proprietari/moderatori. Espone la decisione di installazione e la spiegazione
  pubblica, non le identità dei segnalatori, i corpi delle segnalazioni, prove
  private o cronologie di revisione interne.
...
- Le versioni ClawPack restituiscono un artefatto `npm-pack`, campi di integrità
  npm, un `tarballUrl` e l'URL di compatibilità ZIP legacy.
- Questa è la superficie del resolver di OpenClaw; evita di indovinare il formato
  dell'archivio da un URL condiviso.
...
Endpoint amministratore per creare o aggiornare una riga di migrazione di Plugin ufficiale.
...
- `packageName` è normalizzato come nome npm; il pacchetto può essere assente per
  migrazioni pianificate.
- Questo traccia solo la readiness della migrazione. Non modifica OpenClaw né
  genera ClawPack.
...
Segnala un pacchetto per la revisione da parte dei moderatori. Le segnalazioni
sono a livello di pacchetto, facoltativamente collegate a una versione. Alimentano
la coda di moderazione ma non nascondono automaticamente né bloccano i download
da sole; i moderatori dovrebbero usare la moderazione dei rilasci per approvare,
mettere in quarantena o revocare gli artefatti.
...
- Gli header di download includono SHA-256 ClawHub più i metadati integrity/shasum di npm.
...
Scarica uno ZIP di una versione di skill ospitata, oppure restituisce un handoff del sorgente GitHub per una
...
- Gli handoff delle skill basate su GitHub non fanno da proxy né eseguono il mirror dei byte. La risposta JSON
  include `sourceRef: "public-github"`, `repo`, `commit`, `path`, `contentHash`
  e `archiveUrl`; lo stato della scansione/corrente è un gate e non viene incluso come metadato del payload
  di successo.
...
- Campo payload facoltativo: `ownerHandle`. Quando presente, l'API risolve
  quell'editore lato server e richiede che l'attore abbia accesso all'editore.
- Campo payload facoltativo: `migrateOwner`. Quando `true` con `ownerHandle`, una
  skill esistente può essere spostata a quel proprietario se l'attore è admin/proprietario sia negli editori
  correnti sia in quelli di destinazione. Senza questa adesione esplicita, le modifiche del proprietario sono
  rifiutate.
...
Eliminazione reversibile / ripristino di una skill (proprietario, moderatore o admin).
...
un altro editore. La risposta di eliminazione include `slugReservedUntil` quando questa scadenza si applica.
Le rimozioni per nascondimento da moderatore/admin e per sicurezza non scadono in questo modo.
...
Solo admin. Garantisce che esista un editore dell'organizzazione per un handle. Se l'handle punta ancora a un
utente condiviso legacy/editore personale, l'endpoint lo migra prima in un editore dell'organizzazione.
Per un'organizzazione appena creata, fornisci `memberHandle`; l'admin attore non viene aggiunto come membro.
...
non contrassegna l'editore come attendibile/ufficiale.
...
senza modificare le righe degli account Convex Auth. La richiesta deve nominare entrambi gli id account provider GitHub
immutabili; gli handle mutabili sono usati solo come guard rivolto all'operatore.
...
principal GitHub. Il ripristino fallisce in modo chiuso quando il publisher personale corrente dell'utente di destinazione
ha skills, pacchetti o origini Skills GitHub.
...
i percorsi con proprietario diretto concordino con la nuova autorità del publisher. Anche una prenotazione active protected-handle
per l'handle ripristinato viene riassegnata all'utente sostitutivo, così la sincronizzazione successiva del profilo
...
Le origini Skills GitHub sono a livello di publisher e vengono segnalate come controllate anziché riscritte.

- Corpo: `{ "handle": "gingiris", "nextUserHandle": "gingiris-1031", "previousGitHubProviderAccountId": "123", "nextGitHubProviderAccountId": "456", "reason": "Continuità dell'account verificata per l'issue #2555", "confirmIdentityVerified": true, "dryRun": false }`
- Risposta: `{ "ok": true, "dryRun": false, "recovered": true, "publisherId": "...", "handle": "gingiris", "previousUser": { "userId": "...", "handle": "gingiris", "nextHandle": "gingiris-recovered", "githubProviderAccountId": "123", "authAccountCount": 1 }, "nextUser": { "userId": "...", "handle": "gingiris-1031", "nextHandle": "gingiris", "githubProviderAccountId": "456", "authAccountCount": 1 }, "retiredPersonalPublisher": null, "resourceOwnerMigration": { "limitPerTable": 100, "skills": 1, "skillSlugAliases": 1, "packages": 0, "packageInspectorWarnings": 0, "githubSourcesChecked": 1, "handleReservations": 1 }, "identityVerified": true, "reason": "Continuità dell'account verificata per l'issue #2555" }`
...
- `POST /api/v1/skills/{slug}/transfer`
  - Corpo: `{ "toUserHandle": "target_handle", "message": "facoltativo" }`
...
```json
{ "handle": "user_handle", "reason": "motivo facoltativo del ban" }
```

oppure

```json
{ "userId": "users_...", "reason": "motivo facoltativo del ban" }
```
...
Rimuovi il ban di un utente e ripristina le skill idonee (solo admin).

Corpo:

```json
{ "handle": "user_handle", "reason": "motivo facoltativo della rimozione del ban" }
```

oppure

```json
{ "userId": "users_...", "reason": "motivo facoltativo della rimozione del ban" }
```
...
contenuti (solo admin). Usa per impostazione predefinita la modalità dry-run, salvo che `dryRun` sia `false`.

Corpo:

```json
{ "handle": "user_handle", "reason": "spam di pubblicazione in blocco", "dryRun": true }
```

oppure

```json
{ "userId": "users_...", "reason": "spam di pubblicazione in blocco", "dryRun": false }
```
...
Aggiungi/rimuovi una star (in evidenza). Entrambi gli endpoint sono idempotenti.
...
Se esegui self-hosting, servi questo file (o imposta esplicitamente `CLAWHUB_REGISTRY`; legacy `CLAWDHUB_REGISTRY`).
