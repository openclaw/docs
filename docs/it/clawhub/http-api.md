...
x-i18n:
    generated_at: "2026-07-02T22:34:30Z"
    model: gpt-5.5
...
- Gli endpoint di scrittura autenticati non dovrebbero restituire un semplice `Unauthorized` quando
  ...
  account eliminati/bannati/disabilitati dovrebbero ciascuno ricevere un testo utilizzabile, così che i client
...
- I risultati vengono restituiti in ordine di pertinenza (similarità embedding + boost sui token esatti di slug/nome + un piccolo prior di popolarità).
- La pertinenza pesa più della popolarità. Una corrispondenza precisa di slug o token del nome visualizzato può superare una corrispondenza più debole con molto più coinvolgimento.
- Il testo ASCII viene tokenizzato sui confini di parola e punteggiatura. Per esempio, `personal-map` contiene un token autonomo `map`, mentre `amap-jsapi-skill` contiene `amap`, `jsapi` e `skill`; cercare `map` assegna quindi a `personal-map` una corrispondenza lessicale più forte rispetto a `amap-jsapi-skill`.
- La popolarità è scalata logaritmicamente e limitata. Le Skills con alto coinvolgimento possono classificarsi più in basso quando il testo della query ha una corrispondenza più debole.
...
Indicazioni per la rilevabilità da parte dei publisher:

- Inserisci nel nome visualizzato, nel riepilogo e nei tag i termini che gli utenti cercheranno letteralmente. Usa un token slug autonomo solo quando è anche un'identità stabile che vuoi mantenere.
- Non rinominare uno slug solo per inseguire una query, a meno che il nuovo slug non sia un nome canonico migliore a lungo termine. I vecchi slug diventano alias di reindirizzamento, ma l'URL canonico, lo slug mostrato e i futuri digest di ricerca usano il nuovo slug.
...
- Se una Skill è inaspettatamente invisibile, controlla prima lo stato di moderazione con `clawhub inspect @owner/slug` dopo aver effettuato l'accesso, prima di modificare metadati legati al ranking.
...
Endpoint moderatore/admin per risolvere o riaprire le segnalazioni Skill.
...
sottoposta a triage per nascondere la Skill nello stesso workflow verificabile.
...
- Le scansioni pubblicate richiedono accesso di gestione come proprietario/publisher, oppure autorità di moderatore/admin della piattaforma.
...
- Richiede accesso di gestione come proprietario/publisher alla skill o al plugin, oppure autorità di moderatore/admin della piattaforma.
...
Route batch canonica solo per admin per rieseguire scansioni. Accetta la stessa struttura di payload della route legacy `POST /api/v1/skills/-/rescan-batch`.
...
- Identità della skill, identità del publisher e metadati della versione selezionata sono campi di primo livello dell'envelope (`slug`, `displayName`, `publisherHandle`, `version`, `resolvedFrom`, `tag`, `createdAt`), così l'automazione shell può leggerli senza decomprimere wrapper annidati.
...
endpoint di raccolta è pensato per client che sanno già quali versioni delle skill
...
- La risposta riguarda solo la sicurezza. Non include dati Skill Card, stato della card generata, elenchi di file degli artifact o payload dettagliati degli scanner.
...
- `security.signals.dependencyRegistry` viene mantenuto per compatibilità con la risposta v1, ma lo scanner di esistenza del registro delle dipendenze è stato ritirato e questa chiave è sempre `null`.
- L'assenza di Skill Card non influisce su `ok`, `decision` o `reasons` di questo endpoint; i client dovrebbero leggere localmente `skill-card.md` installato quando hanno bisogno del contenuto della card.
- Usa `/verify` quando ti serve l'envelope di verifica Skill Card per una singola skill, `/card` quando ti serve il markdown della card generata e `/scan` quando ti servono dati dettagliati degli scanner.
...
- `POST /api/v1/packages` resta disponibile solo per release di code-plugin e bundle-plugin.
...
`trending` è una classifica di installazioni/download su sette giorni e non usa i totali di tutti i tempi.
...
- Il filtro per categoria è un filtro API reale basato su righe di digest delle categorie
  dei plugin, non una riscrittura della query di ricerca.
- I risultati sono restituiti in ordine di rilevanza e attualmente non prevedono paginazione.
- I controlli di ordinamento dell'interfaccia browser per la ricerca dei plugin riordinano i risultati di rilevanza caricati,
  allineandosi al comportamento corrente di navigazione di `/skills`.
...
- I rilasci ClawPack includono i campi compatibili con npm `npmIntegrity`, `npmShasum` e
  `npmTarballName`.
- `version.sha256hash` è metadato di compatibilità deprecato per i vecchi client. Calcola l'hash dei byte ZIP esatti restituiti da `/api/v1/packages/{name}/download`.
...
Restituisce il riepilogo esatto di sicurezza e affidabilità del rilascio del pacchetto per i client di installazione. Questa è la superficie pubblica di consumo di OpenClaw per decidere se un rilascio risolto può essere installato.
...
- Questo endpoint è intenzionalmente più ristretto degli endpoint di moderazione per proprietari/moderatori. Espone la decisione di installazione e la spiegazione pubblica, non
  le identità dei segnalanti, i corpi delle segnalazioni, le prove private o le timeline interne di revisione.
...
Endpoint amministrativo per creare o aggiornare una riga di migrazione di plugin ufficiale.
...
- Richiede un token API per il proprietario del pacchetto, un membro del publisher,
  un moderatore o un utente admin.
...
Endpoint moderatore/admin per la revisione della release del pacchetto.
...
- Gli archivi Plugin/pacchetto sono file zip con una radice `package/` affinché i vecchi
  client OpenClaw continuino a funzionare.
...
Scarica uno ZIP della versione di una skill ospitata, oppure restituisce un handoff sorgente GitHub per una
...
- Gli handoff delle skill basate su GitHub non fanno proxy né mirroring dei byte. La risposta JSON
  include `sourceRef: "public-github"`, `repo`, `commit`, `path`, `contentHash`
  e `archiveUrl`; lo stato della scansione/corrente è un gate e non è incluso come metadati
  del payload di successo.
- Le statistiche di download sono conteggiate come identità univoche per giorno UTC (`userId` quando il token API è valido, altrimenti IP).
...
- Campo payload facoltativo: `migrateOwner`. Quando `true` con `ownerHandle`, una
  ...
  publisher corrente sia su quello di destinazione. Senza questo opt-in, le modifiche al proprietario sono
  rifiutate.
...
- Le richieste di pubblicazione multipart dirette hanno un limite di 18MB. I tarball ClawPack possono
  usare il flusso upload-url fino al limite tarball di 120MB.
...
I nascondimenti moderatore/admin e le rimozioni di sicurezza non scadono in questo modo.
...
Solo admin. Assicura che esista un publisher di organizzazione per un handle. Se l'handle punta ancora a un
...
Creazione self-service autenticata di publisher di organizzazione. Crea un nuovo publisher di organizzazione e aggiunge il
...
proprietario può in seguito pubblicare la release code-plugin o bundle-plugin reale in quel nome.
...
Solo admin. Recupera un publisher personale per un principale GitHub OAuth sostitutivo verificato
senza modificare le righe account Convex Auth. La richiesta deve nominare entrambi gli id immutabili
dell'account provider GitHub; gli handle mutabili vengono usati solo come guardia rivolta all'operatore.

L'endpoint usa per impostazione predefinita il dry-run. Per applicare il ripristino sono richiesti `dryRun: false` e
`confirmIdentityVerified: true` dopo che lo staff ha verificato in modo indipendente la continuità tra entrambi i
principali GitHub. Il ripristino fallisce in modo chiuso quando l'utente di destinazione ha un publisher personale corrente
con skills, pacchetti o sorgenti di skill GitHub.
Il ripristino migra anche i campi legacy `ownerUserId` per le skills del publisher ripristinato,
gli alias degli slug delle skill, i pacchetti, gli avvisi dell'ispettore dei pacchetti e le righe di digest di ricerca derivate, in modo che
i percorsi con proprietario diretto siano coerenti con la nuova autorità del publisher. Una prenotazione attiva di handle protetto
per l'handle ripristinato viene inoltre riassegnata all'utente sostitutivo, così la successiva
...
Le sorgenti di skill GitHub hanno ambito publisher e vengono segnalate come controllate anziché riscritte.
...
Banna un utente ed elimina definitivamente le skills di proprietà (solo moderator/admin).
...
contenuti (solo admin). Usa per impostazione predefinita il dry-run a meno che `dryRun` sia `false`.
...
Aggiungi/rimuovi una star (evidenziazioni). Entrambi gli endpoint sono idempotenti.
