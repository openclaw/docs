...
- Se `Retry-After` è assente, usa come fallback `RateLimit-Reset` (oppure calcola da `X-RateLimit-Reset`).
...
- Se non è disponibile alcun IP client attendibile, le richieste anonime di download usano un bucket di fallback con ambito endpoint invece di un singolo bucket globale `ip:unknown`. Le richieste anonime di lettura/scrittura usano ancora il bucket unknown condiviso, così il routing senza IP rimane visibile e conservativo.
...
- I risultati vengono restituiti in ordine di pertinenza (similarità embedding + boost per token esatti di slug/nome + prior di popolarità dai download).
...
- I download sono usati come piccolo prior scalato logaritmicamente e criterio di spareggio, non come segnale di ranking primario. Le Skills con molti download possono classificarsi più in basso quando il testo della query è una corrispondenza più debole.
...
Indicazioni per la reperibilità del publisher:
...
- Gli alias di rinomina preservano la risoluzione per i vecchi URL e per le installazioni che risolvono tramite il registry, ma il ranking di ricerca si basa sui metadati canonici della Skill dopo che la rinomina è stata indicizzata. Le statistiche esistenti rimangono associate alla Skill.
- Se una Skill è inaspettatamente invisibile, controlla prima lo stato di moderazione con `clawhub inspect <slug>` mentre hai effettuato l'accesso, prima di modificare i metadati relativi al ranking.
...
- `tag` (opzionale): risolve una versione taggata (ad esempio `latest`).
...
- Impostazione predefinita alla versione più recente.
...
- `capabilityTag` (facoltativo): filtro delle funzionalità per i pacchetti Plugin
- `target` / `hostTarget` (facoltativo): abbreviazione per `host:<target>`
- `os`, `arch`, `libc` (facoltativo): abbreviazione per i filtri delle funzionalità dell'host
...
- Le voci Skills restano basate sul registro delle skill e possono ancora essere pubblicate solo tramite `POST /api/v1/skills`.
...
- `target` / `hostTarget`, `os`, `arch`, `libc`, `requiresBrowser`,
  ...
  `osPermission` sono accettati come abbreviazioni per tag di funzionalità comuni
- `artifactKind` (facoltativo): `legacy-zip` o `npm-pack`
...
- `channel=private` restituisce solo i pacchetti che il chiamante autenticato può leggere.
- I filtri degli artefatti sono basati su tag di funzionalità indicizzati:
...
- Richiede un token API per il proprietario del pacchetto, un proprietario/amministratore dell'editore org,
...
funzionalità, verifica, metadati degli artefatti e dati di scansione.

Note:

- `version.artifact.kind` è `legacy-zip` per gli archivi di pacchetti del vecchio modello o
  ...
  "checks": [
    {
      ...
      "status": "fail",
      "message": "La versione più recente è solo ZIP legacy."
    }
  ],
  "blockers": ["clawpack"]
}
...
Endpoint moderatore/amministratore per l'accettazione delle segnalazioni dei pacchetti.
...
Endpoint moderatore/amministratore per risolvere o riaprire segnalazioni dei pacchetti.
...
Endpoint di manutenzione riservato agli admin per etichettare le release di pacchetti più vecchie con
...
- Gli header di download includono lo SHA-256 di ClawHub più i metadati npm integrity/shasum.
...
- Campo payload opzionale: `migrateOwner`. Quando `true` con `ownerHandle`, una
  ...
  publisher corrente sia su quello di destinazione. Senza questo consenso esplicito, i cambi di proprietario vengono
  rifiutati.
...
Elimina in modo reversibile / ripristina una skill (proprietario, moderator o admin).
...
Le eliminazioni reversibili avviate dal proprietario riservano lo slug per 30 giorni, poi lo slug può essere reclamato da
un altro publisher. La risposta di eliminazione include `slugReservedUntil` quando questa scadenza si applica.
Le rimozioni di sicurezza e i nascondimenti da parte di moderator/admin non scadono in questo modo.
...
- `merge` nasconde il listing sorgente e reindirizza lo slug sorgente al listing di destinazione.
