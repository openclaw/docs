...
- ClawHub usa header di inoltro attendibili per identificare gli IP dei client al margine.
- Se non è disponibile alcun IP client attendibile, le richieste di download anonime usano un bucket di fallback con ambito endpoint invece di un singolo bucket globale `ip:unknown`. Le richieste anonime di lettura/scrittura continuano a usare il bucket sconosciuto condiviso, così il routing senza IP resta visibile e conservativo.
...
- I risultati vengono restituiti in ordine di pertinenza (similarità degli embedding + boost per token esatti di slug/nome + prior di popolarità dai download).
- La pertinenza è più forte della popolarità. Una corrispondenza precisa del token dello slug o del nome visualizzato può superare una corrispondenza più vaga con molti più download.
...
- I download vengono usati come piccolo prior scalato logaritmicamente e come criterio di spareggio, non come segnale principale di ordinamento. Le Skills con molti download possono avere un posizionamento inferiore quando il testo della query è una corrispondenza più debole.
...
- Gli alias di rinomina preservano la risoluzione per i vecchi URL e le installazioni che risolvono tramite il registro, ma il ranking di ricerca si basa sui metadati canonici della Skill dopo che la rinomina è stata indicizzata. Le statistiche esistenti restano associate alla Skill.
- Se una Skill è inaspettatamente invisibile, controlla prima lo stato di moderazione con `clawhub inspect <slug>` mentre hai effettuato l'accesso, prima di cambiare metadati legati al ranking.
...
Endpoint moderatore/amministratore per l'accettazione delle segnalazioni delle Skill.
...
- `capabilityTag` (facoltativo): filtro di capability per i pacchetti plugin
- `target` / `hostTarget` (facoltativo): abbreviazione per `host:<target>`
- `os`, `arch`, `libc` (facoltativo): abbreviazione per i filtri di capability dell'host
- `requiresBrowser`, `requiresDesktop`, `requiresNativeDeps`,
  `requiresExternalService`, `requiresBinary`, `requiresOsPermission`
  (facoltativo): abbreviazione `true`/`1` per i tag dei requisiti di ambiente
- `externalService`, `binary`, `osPermission` (facoltativo): abbreviazione per i tag dei requisiti di ambiente denominati
- `artifactKind` (facoltativo): `legacy-zip` o `npm-pack`
- `npmMirror` (facoltativo): `true`/`1` per mostrare le versioni dei pacchetti basati su ClawPack
...
- `artifactKind` (facoltativo): `legacy-zip` o `npm-pack`
- `npmMirror` (facoltativo): `true`/`1` per cercare le versioni dei pacchetti basati su ClawPack
  disponibili tramite il mirror npm
...
- Le Skills possono anche essere risolte tramite questa rotta nel catalogo unificato.
...
capability, verifica, metadati degli artefatti e dati di scansione.

Note:

- `version.artifact.kind` è `legacy-zip` per gli archivi di pacchetti legacy oppure
...
Restituisce il riepilogo esatto di sicurezza e attendibilità della release del pacchetto per i client di installazione. Questa è la superficie pubblica di consumo di OpenClaw per decidere se una release risolta può essere installata.

Autenticazione:

- Endpoint di lettura pubblico. Non è richiesto alcun token di proprietario, editore, moderatore o admin.
...
- `trust.stale` significa che il riepilogo di attendibilità è stato calcolato da input obsoleti e
  deve essere trattato come da aggiornare prima di una decisione di autorizzazione ad alta affidabilità.

Note:

- Questo endpoint è esatto per versione. I client devono chiamarlo dopo aver risolto la
  versione del pacchetto che intendono installare, non solo dopo aver letto i metadati più recenti del
  pacchetto.
- I pacchetti privati restituiscono `404` a meno che il chiamante non possa leggere l'editore proprietario.
...
Endpoint per moderatore/admin per l'acquisizione delle segnalazioni dei pacchetti.
...
Endpoint per proprietario/moderatore per la visibilità della moderazione dei pacchetti.
...
Endpoint per moderatore/admin per risolvere o riaprire le segnalazioni dei pacchetti.
...
Endpoint per moderatore/admin per la revisione dei rilasci dei pacchetti.
...
Endpoint di manutenzione solo per admin per etichettare i rilasci di pacchetti meno recenti con
...
- I rilasci senza storage ClawPack sono etichettati `legacy-zip`.
- Le righe esistenti supportate da ClawPack senza `artifactKind` vengono riparate come
  `npm-pack`.
...
- Usa per impostazione predefinita il rilascio più recente.
...
- Le risposte includono gli header `ETag`, `Digest`, `X-ClawHub-Artifact-Type` e
  `X-ClawHub-Artifact-Sha256` per i controlli di integrità del risolutore.
- I metadati solo di registro non vengono inseriti nell'archivio scaricato.
...
- Campo payload facoltativo: `migrateOwner`. Quando è `true` con `ownerHandle`, una
  ...
  publisher corrente sia su quello di destinazione. Senza questa adesione esplicita, i cambi di proprietario vengono
  rifiutati.
...
Le operazioni di nascondimento e rimozione per sicurezza eseguite da moderatore/admin non scadono in questo modo.
...
Solo admin. Riserva slug root e nomi di pacchetto per il legittimo proprietario senza pubblicare un
rilascio. I nomi dei pacchetti diventano pacchetti placeholder privati senza righe di rilascio, così lo stesso
...
Banna un utente ed elimina definitivamente le Skills di sua proprietà (solo moderator/admin).
