...
Le directory di terze parti possono usare gli endpoint pubblici di lettura per elencare o cercare Skills ClawHub. Memorizza i risultati nella cache, rispetta `429`/`Retry-After`, collega gli utenti alla scheda ClawHub canonica (`https://clawhub.ai/<owner>/skills/<slug>`) ed evita di suggerire un'approvazione di ClawHub per il sito di terze parti. Non tentare di rispecchiare contenuti nascosti, privati o bloccati dalla moderazione al di fuori della superficie dell'API pubblica.
...
- Gli endpoint di scrittura autenticati non dovrebbero restituire un semplice `Unauthorized` quando
  ...
  CLI possono dire agli utenti che cosa li ha bloccati.
...
- `X-RateLimit-Remaining` / `RateLimit-Remaining`: budget rimanente esatto quando presente.
  Le richieste shardate riuscite omettono questo header invece di restituire un valore globale approssimativo.
...
Le risposte di errore v1 pubbliche sono testo semplice con `content-type: text/plain; charset=utf-8`.
...
- I risultati sono restituiti in ordine di pertinenza (similarità degli embedding + boost esatti per token slug/nome + un piccolo prior di popolarità).
- La pertinenza è più forte della popolarità. Una corrispondenza precisa di slug o token del nome visualizzato può superare una corrispondenza più vaga con coinvolgimento molto più alto.
...
- La popolarità è scalata logaritmicamente e limitata. Le Skills con alto coinvolgimento possono classificarsi più in basso quando il testo della query è una corrispondenza più debole.
...
Indicazioni per la reperibilità degli editori:
...
- Gli alias di ridenominazione preservano la risoluzione per vecchi URL e installazioni che si risolvono tramite il registro, ma il ranking di ricerca si basa sui metadati canonici della Skill dopo che la ridenominazione è stata indicizzata. Le statistiche esistenti restano con la Skill.
...
- I vecchi slug creati da flussi di ridenominazione/unione del proprietario si risolvono alla Skill canonica.
...
- I chiamanti pubblici ricevono `200` solo per le Skills visibili già segnalate.
- Le prove sono redatte per i chiamanti pubblici e includono frammenti grezzi solo per proprietari/moderatori.

### `POST /api/v1/skills/{slug}/report`

Segnala una Skill per la revisione da parte dei moderatori. Le segnalazioni sono a livello di Skill, opzionalmente collegate
a una versione, e alimentano la coda di segnalazioni delle Skill.
...
Endpoint moderatore/amministratore per risolvere o riaprire le segnalazioni Skill.
...
- I job di scansione sono asincroni. Le richieste di scansione manuali hanno priorità sul normale lavoro di pubblicazione/backfill, ma il completamento dipende comunque dalla disponibilità dei worker.
...
Endpoint autenticato per archivio di report.
...
Endpoint autenticato per archivio di report archiviati per versioni inviate.

- Richiede accesso di gestione da proprietario/editore allo skill o al plugin, oppure autorità di moderatore/amministratore della piattaforma.
- Restituisce i risultati di scansione archiviati per l'esatta versione inviata, incluse versioni bloccate o nascoste.
- `kind` usa `skill` come predefinito; usare `kind=plugin` per scansioni di plugin/pacchetti.
- Restituisce la stessa forma ZIP dei download di richieste di scansione.

### `POST /api/v1/skills/-/scan/batch`

Route canonica solo per amministratori per la nuova scansione batch. Accetta la stessa forma di payload della legacy `POST /api/v1/skills/-/rescan-batch`.

### `POST /api/v1/skills/-/scan/batch/status`

Route canonica solo per amministratori per lo stato batch. Accetta `{ "jobIds": ["..."] }` e restituisce gli stessi contatori aggregati della legacy `POST /api/v1/skills/-/rescan-batch/status`.
...
- Identità dello skill, identità dell'editore e metadati della versione selezionata sono campi di primo livello dell'envelope (`slug`, `displayName`, `publisherHandle`, `version`, `resolvedFrom`, `tag`, `createdAt`) così l'automazione shell può leggerli senza decomprimere wrapper annidati.
...
- `security.signals.dependencyRegistry` è mantenuto per compatibilità della risposta v1, ma lo scanner di esistenza del registro delle dipendenze è stato ritirato e questa chiave è sempre `null`.
...
Restituisce verdetti di sicurezza compatti attuali per versioni esatte degli skill. Questo
...
- I risultati sono per elemento; uno skill o una versione mancante non fa fallire l'intera risposta.
- La risposta è solo di sicurezza. Non include dati Skill Card, stato della card generata, elenchi di file artefatto o payload dettagliati degli scanner.
- `security.signals` contiene solo prove di supporto a livello di stato; usare `/scan` o la pagina di audit di sicurezza ClawHub per i dettagli completi degli scanner.
- `security.signals.dependencyRegistry` è mantenuto per compatibilità della risposta v1, ma lo scanner di esistenza del registro delle dipendenze è stato ritirato e questa chiave è sempre `null`.
- L'assenza di Skill Card non influisce su `ok`, `decision` o `reasons` di questo endpoint; i client dovrebbero leggere localmente lo `skill-card.md` installato quando hanno bisogno del contenuto della card.
- Usare `/verify` quando serve l'envelope di verifica Skill Card per un singolo skill, `/card` quando serve il Markdown della card generata, e `/scan` quando servono dati dettagliati dello scanner.
...
- `category` (facoltativo): filtro categoria plugin. Supportato solo quando la
  richiesta è circoscritta ai pacchetti plugin (`/api/v1/plugins`,
...
- Le voci skill restano basate sul registro skill e possono ancora essere pubblicate solo tramite `POST /api/v1/skills`.
...
- `isOfficial` (facoltativo): `true` o `false`
- `category` (facoltativo): filtro categoria plugin. Supportato solo quando la
  richiesta è circoscritta ai pacchetti plugin. Le categorie controllate e gli alias
...
Gli alias legacy non sono accettati come valori categoria memorizzati o dichiarati dall'autore.
...
- Le skill correnti basate su GitHub con scansione `clean` o `suspicious` includono
...
- `trust.moderationState` è annullabile. È `null` quando non esiste alcuna
...
- Questa è la superficie resolver di OpenClaw; evita di indovinare il formato dell'archivio da
  un URL condiviso.
...
- `packageName` è normalizzato come nome npm; il pacchetto può mancare per le migrazioni
  pianificate.
- Tiene traccia solo della readiness della migrazione. Non modifica OpenClaw né genera
  ClawPack.
...
- I packument dei pacchetti con scope supportano sia `/api/npm/@scope/name` sia il percorso
  di richiesta codificato di npm `/api/npm/@scope%2Fname`.
...
Scarica uno ZIP di versione Skills ospitata, oppure restituisce un passaggio di consegna a sorgente GitHub per una
Skills attuale basata su GitHub con una scansione `clean` o `suspicious` e nessuna versione
ospitata.
...
- I passaggi di consegna per Skills basate su GitHub non effettuano proxy né mirror dei byte. La risposta JSON
...
- Campo payload facoltativo: `migrateOwner`. Quando `true` con `ownerHandle`, una
  Skills esistente può essere spostata a quel proprietario se l'attore è amministratore/proprietario su entrambi
...
- I campi modulo consentiti sono `payload`, blob `files` ripetuti, o un riferimento a tarball `clawpack`.
  `clawpack` può essere un blob `.tgz` o un id di storage restituito dal
  flusso upload-url. Le pubblicazioni da storage-id preparato devono includere anche il
...
- Le richieste di pubblicazione multipart diretta sono limitate a 18MB. I tarball ClawPack possono
...
Le eliminazioni reversibili avviate dal proprietario riservano lo slug per 30 giorni, quindi lo slug può essere rivendicato da
un altro publisher. La risposta di eliminazione include `slugReservedUntil` quando si applica questa scadenza.
I nascondimenti di moderatore/amministratore e le rimozioni di sicurezza non scadono in questo modo.
...
publisher legacy condiviso utente/personale, l'endpoint lo migra prima in un publisher dell'organizzazione.
...
Creazione autenticata self-service di un publisher dell'organizzazione. Crea un nuovo publisher dell'organizzazione e aggiunge il
...
Solo amministratori. Riserva slug radice e nomi di pacchetto per un legittimo proprietario senza pubblicare una
...
senza modificare le righe account di Convex Auth. La richiesta deve indicare entrambi gli ID account provider GitHub
immutabili; gli handle modificabili sono usati solo come protezione rivolta all'operatore.

L'endpoint usa per impostazione predefinita la modalità dry-run. L'applicazione del recupero richiede `dryRun: false` e
`confirmIdentityVerified: true` dopo che lo staff ha verificato in modo indipendente la continuità tra entrambi i
principal GitHub. Il recupero fallisce in modalità chiusa quando l'attuale publisher personale dell'utente di destinazione
ha skill, pacchetti o fonti di skill GitHub.
Il recupero migra anche i campi legacy `ownerUserId` per le skill del publisher recuperato,
...
per l'handle recuperato viene riassegnata all'utente sostitutivo, così che una successiva
...
Le fonti di skill GitHub hanno ambito del publisher e sono riportate come controllate anziché riscritte.
...
contenuti (solo amministratore). Usa per impostazione predefinita la modalità dry-run, a meno che `dryRun` sia `false`.
...
Aggiunge/rimuove una star (evidenziazioni). Entrambi gli endpoint sono idempotenti.
...
che preparano una tarball ClawPack devono inviare l'id di storage risultante come
`clawpack` e il ticket restituito come `clawpackUploadTicket`.

## Discovery del registro (`/.well-known/clawhub.json`)
...
Se ospiti autonomamente, servi questo file (o imposta `CLAWHUB_REGISTRY` esplicitamente; legacy `CLAWDHUB_REGISTRY`).
