...
x-i18n:
    generated_at: "2026-06-28T06:00:55Z"
    model: gpt-5.5
...
Le directory di terze parti possono usare gli endpoint pubblici di lettura per elencare o cercare le Skills di ClawHub. Memorizza nella cache i risultati, rispetta `429`/`Retry-After`, rimanda gli utenti alla scheda canonica di ClawHub (`https://clawhub.ai/<owner>/skills/<slug>`) ed evita di implicare un'approvazione di ClawHub per il sito di terze parti. Non tentare di rispecchiare contenuti nascosti, privati o bloccati dalla moderazione al di fuori della superficie dell'API pubblica.
...
- `highlightedOnly` (opzionale): `true` per filtrare solo le Skills evidenziate
...
- Il testo ASCII viene tokenizzato sui confini di parole e punteggiatura. Per esempio, `personal-map` contiene un token autonomo `map`, mentre `amap-jsapi-skill` contiene `amap`, `jsapi` e `skill`; cercare `map` quindi dà a `personal-map` una corrispondenza lessicale più forte rispetto ad `amap-jsapi-skill`.
- La popolarità è scalata logaritmicamente e limitata. Le Skills ad alto coinvolgimento possono posizionarsi più in basso quando il testo della query è una corrispondenza più debole.
...
Indicazioni sulla scopribilità per i publisher:
...
- `moderation` è inclusa solo quando la Skill è contrassegnata o il proprietario la sta visualizzando.
...
- I chiamanti pubblici ricevono `200` solo per le Skill visibili già contrassegnate.
...
si imposta di nuovo `status` su `open`. Passa `finalAction: "hide"` con una segnalazione triagiata
...
- Le scansioni pubblicate scrivono modifiche solo quando `update: true` e la scansione viene completata correttamente.
...
- Restituisce lo stato in coda/in esecuzione/completato con successo/non riuscito.
...
Endpoint autenticato dell'archivio report.
...
Endpoint autenticato dell'archivio report memorizzato per le versioni inviate.
...
- `kind` ha come valore predefinito `skill`; usa `kind=plugin` per le scansioni di plugin/pacchetti.
- Restituisce la stessa struttura ZIP dei download delle richieste di scansione.

### `POST /api/v1/skills/-/scan/batch`

Route canonica di nuova scansione batch riservata agli amministratori. Accetta la stessa struttura di payload della legacy `POST /api/v1/skills/-/rescan-batch`.
...
- `security.signals` contiene evidenze di supporto dagli scanner, come `staticScan`, `virusTotal` e `skillSpector`.
...
endpoint di raccolta è destinato ai client che sanno già quali versioni di skill ClawHub installate devono visualizzare, come OpenClaw Control UI.
...
- `security.signals` contiene solo evidenze di supporto a livello di stato; usa `/scan` o la pagina security-audit di ClawHub per i dettagli completi degli scanner.
- `security.signals.dependencyRegistry` viene mantenuto per compatibilità della risposta v1, ma lo scanner di esistenza del registro delle dipendenze è ritirato e questa chiave è sempre `null`.
...
- Usa `/verify` quando hai bisogno dell'envelope di verifica della Skill Card per una singola skill, `/card` quando hai bisogno del markdown della card generata e `/scan` quando hai bisogno di dati dettagliati degli scanner.
...
- `category` (facoltativo): filtro della categoria del plugin. Supportato solo quando la
  ...
  alias legacy dei filtri v1 sono documentati sotto `GET /api/v1/plugins`.
...
- Le voci delle skill continuano a essere basate sul registro delle skill e possono ancora essere pubblicate solo tramite `POST /api/v1/skills`.
...
Ricerca unificata nel catalogo tra skill e pacchetti plugin.
...
- `isOfficial` (facoltativo): `true` o `false`
- `category` (facoltativo): filtro della categoria del plugin. Supportato solo quando la
  richiesta è limitata ai pacchetti plugin. Le categorie controllate e gli alias legacy
  dei filtri v1 sono documentati sotto `GET /api/v1/plugins`.
...
Sull'endpoint unificato `/api/v1/packages` è disponibile solo per i plugin; usa
...
- `_manifest.json` è sempre incluso nella radice dello ZIP.
- `_errors.json` è incluso quando singoli plugin o file non sono stati
  esportati.
...
- I risultati vengono restituiti in ordine di pertinenza e attualmente non usano la paginazione.
- I controlli di ordinamento dell'interfaccia browser per la ricerca plugin riordinano i risultati di pertinenza caricati,
...
- `version.artifact.kind` è `legacy-zip` per gli archivi di pacchetti del vecchio modello oppure
...
- `trust.moderationState` può essere nullo. È `null` quando non esiste alcuna moderazione
  manuale della release.
- `trust.blockedFromDownload` è il segnale di blocco dell'installazione. OpenClaw e altri
  ...
  ridefinire le regole di blocco dai campi dello scanner o di moderazione.
...
- `trust.stale` indica che il riepilogo di attendibilità è stato calcolato da input obsoleti e
  deve essere trattato come da aggiornare prima di una decisione di autorizzazione ad alta confidenza.

Note:

- Questo endpoint è specifico per la versione. I client devono chiamarlo dopo aver risolto la
...
- Questa route rimane solo ZIP. Non esegue lo streaming dei file ClawPack `.tgz`.
...
Scarica uno ZIP di una versione ospitata di uno skill, oppure restituisce un handoff sorgente GitHub per uno
skill corrente basato su GitHub con una scansione `clean` o `suspicious` e nessuna versione
ospitata.
...
- Gli handoff degli skill basati su GitHub non fanno proxy né mirror dei byte. La risposta JSON
...
- Campo payload facoltativo: `migrateOwner`. Quando `true` con `ownerHandle`, uno
  skill esistente può spostarsi a quel proprietario se l’attore è admin/proprietario sia sul
  publisher corrente sia su quello di destinazione. Senza questo opt-in, i cambi di proprietario vengono
  rifiutati.
...
- Le pubblicazioni per conto di altri continuano a validare l’idoneità al canale ufficiale rispetto all’account proprietario di destinazione.

### `DELETE /api/v1/skills/{slug}` / `POST /api/v1/skills/{slug}/undelete`

Elimina in modo reversibile / ripristina uno skill (proprietario, moderatore o admin).
...
Quando presente, `reason` viene memorizzato come nota di moderazione dello skill e copiato nel registro di audit.
...
Per un’org appena creata, fornisci `memberHandle`; l’admin che agisce non viene aggiunto come membro.
...
proprietario può in seguito pubblicare la reale release code-plugin o bundle-plugin con quel nome.
...
principali GitHub. Il ripristino fallisce in sicurezza quando l'attuale publisher personale dell'utente di destinazione
...
i percorsi con proprietario diretto concordino con la nuova autorità del publisher. Anche una prenotazione attiva
protetta dell'handle per l'handle ripristinato viene riassegnata all'utente sostitutivo, così la sincronizzazione successiva
...
Le sorgenti skill GitHub sono nell'ambito del publisher e sono segnalate come controllate anziché riscritte.
...
### Endpoint di gestione degli slug dei proprietari
...
- `merge` nasconde l'inserzione di origine e reindirizza lo slug di origine all'inserzione di destinazione.
...
Banna un utente ed elimina definitivamente le skill possedute (solo moderator/admin).
...
Cambia il motivo salvato per un ban esistente senza rimuovere il ban o ripristinare
...
Ancora supportati per versioni CLI meno recenti:
