...
Le scorciatoie degli slug web si risolvono tra famiglie di registro, ma i client API dovrebbero usare
...
- Gli endpoint di scrittura autenticati non dovrebbero restituire un semplice `Unauthorized` quando
  ...
  CLI possano dire agli utenti che cosa li ha bloccati.
...
- `X-RateLimit-Remaining` / `RateLimit-Remaining`: budget residuo esatto quando presente.
  Le richieste riuscite su shard omettono questa intestazione invece di restituire un valore globale approssimativo.
...
- Usa intestazioni IP client attendibili, incluso `cf-connecting-ip`, solo quando il
  deployment abilita esplicitamente le intestazioni inoltrate attendibili.
- ClawHub usa intestazioni di inoltro attendibili per identificare gli IP client all'edge.
...
dovrebbero leggere il corpo della risposta come stringa leggibile. I parametri di query sconosciuti sono
...
- I risultati sono restituiti in ordine di pertinenza (similarità embedding + boost esatti per token di slug/nome + un piccolo prior di popolarità).
- La pertinenza è più forte della popolarità. Una corrispondenza precisa di slug o token del nome visualizzato può superare una corrispondenza più ampia con coinvolgimento molto più forte.
- Il testo ASCII viene tokenizzato su confini di parole e punteggiatura. Ad esempio, `personal-map` contiene un token autonomo `map`, mentre `amap-jsapi-skill` contiene `amap`, `jsapi` e `skill`; cercare `map` quindi dà a `personal-map` una corrispondenza lessicale più forte rispetto a `amap-jsapi-skill`.
- La popolarità è scalata logaritmicamente e limitata. Le Skills ad alto coinvolgimento possono classificarsi più in basso quando il testo della query è una corrispondenza più debole.
...
- Gli alias di rinomina preservano la risoluzione per vecchi URL e installazioni che si risolvono tramite il registro, ma il ranking di ricerca si basa sui metadati canonici della Skill dopo che la rinomina è stata indicizzata. Le statistiche esistenti restano con la Skill.
...
- `createdAt` è stabile per i crawl delle nuove Skill; `updated` cambia quando le Skill esistenti vengono ripubblicate.
...
- I vecchi slug creati dai flussi di rinomina/unione del proprietario si risolvono nella Skill canonica.
...
Segnala una Skill per la revisione dei moderatori. Le segnalazioni sono a livello di Skill, eventualmente collegate
...
reimposta `status` su `open`. Passa `finalAction: "hide"` con una segnalazione analizzata
...
- Restituisce lo stato in coda/in esecuzione/riuscita/non riuscita.
...
- `kind` è impostato per impostazione predefinita su `skill`; usa `kind=plugin` per le scansioni di plugin/pacchetti.
- Restituisce la stessa forma ZIP dei download delle richieste di scansione.

### `POST /api/v1/skills/-/scan/batch`

Route canonica di nuova scansione batch solo per amministratori. Accetta la stessa forma di payload della route legacy `POST /api/v1/skills/-/rescan-batch`.

### `POST /api/v1/skills/-/scan/batch/status`

Route canonica dello stato batch solo per amministratori. Accetta `{ "jobIds": ["..."] }` e restituisce gli stessi contatori aggregati della route legacy `POST /api/v1/skills/-/rescan-batch/status`.
...
- L'identità della skill, l'identità dell'editore e i metadati della versione selezionata sono campi di primo livello dell'involucro (`slug`, `displayName`, `publisherHandle`, `version`, `resolvedFrom`, `tag`, `createdAt`), così l'automazione shell può leggerli senza estrarre wrapper annidati.
...
- `provenance` è `server-resolved-github-import` solo quando ClawHub ha risolto e memorizzato un repo/ref/commit/percorso GitHub durante la pubblicazione o l'importazione; altrimenti è `unavailable`.

### `POST /api/v1/skills/-/security-verdicts`

Restituisce i verdetti di sicurezza compatti correnti per versioni esatte delle skill. Questo
endpoint di raccolta è pensato per i client che sanno già quali versioni di skill
...
- `POST /api/v1/packages` è ancora riservato alle release di code-plugin e bundle-plugin.
...
Esplorazione del catalogo dei soli Plugin tra pacchetti code-plugin e bundle-plugin.
...
Gli alias legacy del filtro v1 rimangono accettati sugli endpoint di lettura:
...
- `_errors.json` è incluso quando singole Skill o singoli file non hanno potuto essere
  esportati.
...
- Sono accettati anche gli alias legacy dei filtri v1 documentati sotto `GET /api/v1/plugins`.
- Il filtro per categoria è un filtro API reale basato su righe di digest delle
...
- Le Skills possono essere risolte anche tramite questa rotta nel catalogo unificato.
...
- `version.artifact.kind` è `legacy-zip` per archivi di pacchetti del vecchio mondo o
...
- `trust.reasons` è l'elenco di spiegazioni per l'utente e per audit. I codici motivo
...
- Questo endpoint è intenzionalmente più ristretto degli endpoint di moderazione per proprietari/moderatori.
  ...
  identità dei segnalatori, corpi delle segnalazioni, prove private o timeline interne di revisione.
...
- Usa il bucket di limite frequenza per i download.
...
Endpoint moderatore/admin per la ricezione dei report sui pacchetti.
...
Endpoint moderatore/admin per risolvere o riaprire i report sui pacchetti.
...
Restituisce un packument compatibile con npm per le versioni dei pacchetti supportate da ClawPack.
...
- Le intestazioni di download includono SHA-256 ClawHub più metadati npm integrity/shasum.
...
Scarica uno ZIP di versione skill ospitata, oppure restituisce un handoff sorgente GitHub per una
skill corrente supportata da GitHub con scansione `clean` o `suspicious` e senza versione
ospitata.
...
- Gli handoff delle skill supportate da GitHub non fanno proxy né mirror dei byte. La risposta JSON
...
- Campo payload opzionale: `migrateOwner`. Quando `true` con `ownerHandle`, una
  ...
  i publisher corrente e di destinazione. Senza questo consenso esplicito, le modifiche del proprietario vengono
  rifiutate.
...
Solo admin. Garantisce l'esistenza di un publisher organizzazione per un handle. Se l'handle punta ancora a un
...
`memberRole` è per impostazione predefinita `owner`.
...
Creazione self-service autenticata di un publisher organizzazione. Crea un nuovo publisher organizzazione e aggiunge il
chiamante come proprietario. Questo endpoint non migra handle utente/personali esistenti e non
contrassegna il publisher come trusted/official.
...
L'endpoint usa dry-run come impostazione predefinita. Applicare il recupero richiede `dryRun: false` e
`confirmIdentityVerified: true` dopo che lo staff ha verificato in modo indipendente la continuità tra entrambi
i principal GitHub. Il recupero fallisce in modo chiuso quando l'editore personale corrente dell'utente di destinazione
ha skills, pacchetti o sorgenti di skill GitHub.
Il recupero migra anche i campi legacy `ownerUserId` per le skills dell'editore recuperato,
...
Le sorgenti di skill GitHub sono nell'ambito dell'editore e vengono segnalate come controllate anziché riscritte.
...
### Endpoint di gestione dello slug del proprietario
...
contenuti (solo admin). Usa dry-run come impostazione predefinita a meno che `dryRun` sia `false`.
