...
x-i18n:
    generated_at: "2026-07-01T18:12:57Z"
    model: gpt-5.5
...
di permesso (`401`/`403`), limiti di frequenza (`429`) e download bloccati. I client
dovrebbero leggere il corpo della risposta come stringa leggibile. I parametri di query sconosciuti vengono
...
- `highlightedOnly` (opzionale): `true` per filtrare solo le skill in evidenza
...
- I risultati sono restituiti in ordine di rilevanza (similarità degli embedding + boost dei token esatti di slug/nome + un piccolo prior di popolarità).
- La rilevanza è più forte della popolarità. Una corrispondenza precisa di slug o token del nome visualizzato può superare una corrispondenza più debole con coinvolgimento molto più alto.
...
Indicazioni per la scopribilità degli autori:
...
- Non rinominare uno slug solo per inseguire una query, a meno che il nuovo slug non sia un nome canonico migliore a lungo termine. Gli slug vecchi diventano alias di reindirizzamento, ma l'URL canonico, lo slug visualizzato e i futuri digest di ricerca usano il nuovo slug.
- Gli alias di rinomina preservano la risoluzione per vecchi URL e installazioni che si risolvono tramite il registro, ma il ranking della ricerca si basa sui metadati canonici della skill dopo che la rinomina è stata indicizzata. Le statistiche esistenti restano associate alla skill.
- Se una skill è inaspettatamente invisibile, controlla prima lo stato di moderazione con `clawhub inspect @owner/slug` dopo aver effettuato l'accesso, prima di modificare metadati relativi al ranking.
...
- Gli slug vecchi creati dai flussi di rinomina/unione del proprietario si risolvono alla skill canonica.
- `metadata.os`: restrizioni del sistema operativo dichiarate nel frontmatter della skill (per esempio `["macos"]`, `["linux"]`). `null` se non dichiarate.
...
- Restituisce `queue.queuedAhead` e `queue.position` durante l'attesa in coda, così i client possono mostrare quante scansioni manuali prioritarie precedono la richiesta. Le code molto grandi sono limitate e riportate con `queuedAheadIsEstimate: true`.
...
- Restituisce i risultati di scansione memorizzati per la versione inviata esatta, incluse le versioni bloccate o nascoste.
- `kind` usa `skill` come predefinito; usa `kind=plugin` per le scansioni di plugin/pacchetti.
...
- L'identità della skill, l'identità del publisher e i metadati della versione selezionata sono campi di primo livello dell'envelope (`slug`, `displayName`, `publisherHandle`, `version`, `resolvedFrom`, `tag`, `createdAt`) così l'automazione shell può leggerli senza estrarre wrapper annidati.
...
- `security.signals.dependencyRegistry` viene mantenuto per compatibilità con la risposta v1, ma lo scanner di esistenza del registro delle dipendenze è ritirato e questa chiave è sempre `null`.
- L'assenza della Skill Card non influisce su `ok`, `decision` o `reasons` di questo endpoint; i client dovrebbero leggere `skill-card.md` installato localmente quando hanno bisogno del contenuto della card.
...
- `POST /api/v1/packages` resta solo per i rilasci di code-plugin e bundle-plugin.
...
Ricerca unificata nel catalogo tra skills + pacchetti di plugin.
...
- `isOfficial` (facoltativo): `true` o `false`
- `category` (facoltativo): filtro per categoria di plugin. Supportato solo quando la
  richiesta è limitata ai pacchetti di plugin. Le categorie controllate e gli alias legacy v1
...
- I controlli di ordinamento dell'interfaccia browser per la ricerca dei plugin riordinano i risultati di rilevanza caricati,
  in linea con il comportamento attuale di navigazione di `/skills`.
...
Restituisce il riepilogo esatto di sicurezza e attendibilità della release del pacchetto per i client di installazione. Questa è la superficie di consumo pubblica di OpenClaw per decidere se una
...
- Questo endpoint è intenzionalmente più ristretto degli endpoint di moderazione per proprietari/moderatori.
  ...
  identità dei segnalatori, contenuti delle segnalazioni, prove private o timeline di revisione
  interne.
...
Endpoint moderatore/amministratore per l'acquisizione di segnalazioni sui pacchetti.
...
- Richiede un token API per il proprietario del pacchetto, un membro dell'editore, un moderatore o un
...
- Limite di dimensione del file: 200 KB.
- Le scansioni VirusTotal in sospeso non bloccano le letture; le release dannose potrebbero comunque essere trattenute altrove.
...
Scarica uno ZIP di versione skill ospitata, oppure restituisce un passaggio di consegne alla sorgente GitHub per una
...
- I passaggi di consegne delle skill basate su GitHub non effettuano proxy né mirroring dei byte. La risposta JSON
  include `sourceRef: "public-github"`, `repo`, `commit`, `path`, `contentHash`
  e `archiveUrl`; lo stato scansione/corrente è un gate e non è incluso come metadati
...
- Campo payload opzionale: `migrateOwner`. Quando `true` con `ownerHandle`, una
  skill esistente può essere spostata a quel proprietario se l'attore è amministratore/proprietario sia sugli editori
  correnti sia su quelli di destinazione. Senza questo opt-in, le modifiche al proprietario vengono
  rifiutate.
...
Le rimozioni di sicurezza e i nascondimenti da moderatore/amministratore non scadono in questo modo.
...
Solo amministratori. Garantisce che esista un editore org per un handle. Se l'handle punta ancora a un
utente/editore personale condiviso legacy, l'endpoint lo migra prima in un editore org.
Per una org appena creata, fornisci `memberHandle`; l'amministratore agente non viene aggiunto come membro.
`memberRole` usa come impostazione predefinita `owner`.
...
Creazione self-service autenticata di editori org. Crea un nuovo editore org e aggiunge il
...
senza modificare le righe dell'account Convex Auth. La richiesta deve indicare entrambi gli id immutabili degli account
provider GitHub; gli handle mutabili sono usati solo come guard rivolto all'operatore.
...
i principali GitHub. Il ripristino fallisce in modo chiuso quando l'attuale publisher personale dell'utente di destinazione
...
Le sorgenti di skill GitHub hanno ambito publisher e vengono segnalate come controllate invece di essere riscritte.
...
Rimuovi il ban di un utente e ripristina le Skills idonee (solo admin).
...
il contenuto (solo admin). Usa per impostazione predefinita la modalità dry-run, a meno che `dryRun` sia `false`.
...
Aggiungi/rimuovi una stella (evidenziazioni). Entrambi gli endpoint sono idempotenti.
...
Se usi self-hosting, servi questo file (o imposta esplicitamente `CLAWHUB_REGISTRY`; legacy `CLAWDHUB_REGISTRY`).
