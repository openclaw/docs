...
Le directory di terze parti possono usare gli endpoint pubblici di lettura per elencare o cercare Skills di ClawHub. Memorizza i risultati nella cache, rispetta `429`/`Retry-After`, rimanda gli utenti alla scheda canonica di ClawHub (`https://clawhub.ai/<owner>/skills/<slug>`) ed evita di insinuare un'approvazione di ClawHub per il sito di terze parti. Non tentare di rispecchiare contenuti nascosti, privati o bloccati dalla moderazione al di fuori della superficie dell'API pubblica.
...
- Gli endpoint di scrittura autenticati non devono restituire un semplice `Unauthorized` quando
  ...
  account eliminati/bannati/disabilitati devono ciascuno ricevere un testo utile, così i client
...
- `X-RateLimit-Remaining` / `RateLimit-Remaining`: budget rimanente esatto quando presente.
  Le richieste distribuite su shard riuscite omettono questo header invece di restituire un valore globale approssimato.
...
- Se non è disponibile alcun IP client attendibile, le richieste anonime usano bucket di fallback
  ...
  percorsi, slug, nomi pacchetto, versioni, stringhe di query o altri
...
- La pertinenza è più forte della popolarità. Una corrispondenza precisa dello slug o di un token del nome visualizzato può superare una corrispondenza più vaga con un coinvolgimento molto più alto.
...
- Gli alias di rinomina preservano la risoluzione per i vecchi URL e le installazioni che si risolvono tramite il registro, ma il ranking di ricerca si basa sui metadati canonici della Skill dopo l'indicizzazione della rinomina. Le statistiche esistenti restano associate alla Skill.
...
a una versione, e alimentano la coda di segnalazione delle Skill.
...
si reimposta `status` a `open`. Passa `finalAction: "hide"` con una segnalazione
classificata per nascondere la Skill nello stesso flusso di lavoro verificabile.
...
- `provenance` è `server-resolved-github-import` solo quando ClawHub ha risolto e salvato un repository/ref/commit/percorso GitHub durante la pubblicazione o l'importazione; altrimenti è `unavailable`.
...
- `security.signals` contiene solo prove di supporto a livello di stato; usa `/scan` o la pagina di audit della sicurezza di ClawHub per i dettagli completi degli scanner.
- `security.signals.dependencyRegistry` viene mantenuto per compatibilità con la risposta v1, ma lo scanner di esistenza del registro delle dipendenze è ritirato e questa chiave è sempre `null`.
- L'assenza della Skill Card non influisce su `ok`, `decision` o `reasons` di questo endpoint; i client dovrebbero leggere localmente il file `skill-card.md` installato quando hanno bisogno del contenuto della scheda.
- Usa `/verify` quando ti serve l'envelope di verifica della Skill Card per una singola skill, `/card` quando ti serve il markdown della scheda generata e `/scan` quando ti servono dati dettagliati degli scanner.
...
- Le voci Skill continuano a essere basate sul registro delle Skill e possono ancora essere pubblicate solo tramite `POST /api/v1/skills`.
- `POST /api/v1/packages` rimane riservato solo alle release code-plugin e bundle-plugin.
...
Gli alias di filtro legacy v1 rimangono accettati sugli endpoint di lettura:
...
Esportazione massiva degli ultimi rilasci pubblici dei plugin per l'analisi offline.
...
- Sono accettati anche gli alias dei filtri v1 legacy documentati sotto `GET /api/v1/plugins`.
...
- I controlli di ordinamento dell'interfaccia browser per la ricerca dei plugin riordinano i risultati di pertinenza caricati,
  in linea con l'attuale comportamento di navigazione di `/skills`.
...
Elimina in modo reversibile un pacchetto e tutti i rilasci.
...
Restituisce il riepilogo esatto di sicurezza e attendibilità del rilascio del pacchetto per i client di installazione.
Questa è la superficie pubblica di consumo di OpenClaw per decidere se un
...
- `trust.stale` significa che il riepilogo di attendibilità è stato calcolato da input obsoleti e
  dovrebbe essere trattato come bisognoso di aggiornamento prima di una decisione di autorizzazione ad alta affidabilità.
...
- Questo endpoint è intenzionalmente più ristretto degli endpoint di moderazione per proprietari/moderatori.
  ...
  identità dei segnalanti, corpi delle segnalazioni, prove private o timeline di revisione
  interne.
...
- `manual`: qualsiasi rilascio con una sovrascrittura di moderazione manuale.
- `all`: qualsiasi rilascio con una sovrascrittura manuale, uno stato di scansione non pulito o una segnalazione del pacchetto.
...
- Usa il bucket dei limiti di lettura, non il bucket dei download.
...
- I metadati solo del registry non vengono iniettati nell'archivio scaricato.
...
- Usa il bucket dei limiti di download.
...
Scarica uno ZIP di una versione di skill ospitata, oppure restituisce un passaggio di consegne alla sorgente GitHub per una
...
- I passaggi di consegne delle skill basate su GitHub non fanno proxy né mirror dei byte. La risposta JSON
...
- Campo payload facoltativo: `migrateOwner`. Quando `true` con `ownerHandle`, una
  skill esistente può passare a quel proprietario se l'attore è admin/proprietario sia sugli editori
  corrente sia di destinazione. Senza questo opt-in, le modifiche al proprietario vengono
  rifiutate.
...
Soft-delete / ripristina una skill (proprietario, moderatore o admin).
...
I soft-delete avviati dal proprietario riservano lo slug per 30 giorni, poi lo slug può essere rivendicato da
...
Solo admin. Assicura che esista un editore org per un handle. Se l'handle punta ancora a un
...
`memberRole` ha valore predefinito `owner`.
...
non contrassegna l'editore come attendibile/official.
...
Solo admin. Riserva slug radice e nomi di pacchetto per il proprietario legittimo senza pubblicare una
...
Per impostazione predefinita, l'endpoint esegue un dry-run. L'applicazione del ripristino richiede `dryRun: false` e
...
gli alias degli slug delle skill, i pacchetti, gli avvisi dell'inspector dei pacchetti e le righe derivate del digest di ricerca, così che
...
Le sorgenti di skill GitHub sono nell'ambito del publisher e vengono riportate come controllate anziché riscritte.
...
Modifica il motivo memorizzato per un'esclusione esistente senza rimuovere l'esclusione né ripristinare
contenuti (solo amministratore). Per impostazione predefinita usa un dry-run, a meno che `dryRun` sia `false`.
...
Aggiungi/rimuovi una star (elementi in evidenza). Entrambi gli endpoint sono idempotenti.
