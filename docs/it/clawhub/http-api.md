...
summary: Riferimento API HTTP (endpoint pubblici e CLI + autenticazione).
x-i18n:
    generated_at: "2026-07-01T13:03:51Z"
    model: gpt-5.5
...
- Gli endpoint di scrittura autenticati non dovrebbero restituire un semplice `Unauthorized` quando
  ...
  account eliminati/bannati/disabilitati dovrebbero ricevere ciascuno un testo utilizzabile, in modo che i client
...
- La rilevanza è più forte della popolarità. Una corrispondenza precisa di slug o token del nome visualizzato può superare una corrispondenza più vaga con molto più coinvolgimento.
...
- Non rinominare uno slug solo per inseguire una query, a meno che il nuovo slug non sia un nome canonico migliore a lungo termine. Gli slug precedenti diventano alias di reindirizzamento, ma l'URL canonico, lo slug visualizzato e i futuri riepiloghi di ricerca usano il nuovo slug.
- Gli alias di rinomina preservano la risoluzione per vecchi URL e installazioni che si risolvono tramite il registro, ma il posizionamento nella ricerca si basa sui metadati canonici della Skill dopo l'indicizzazione della rinomina. Le statistiche esistenti restano con la Skill.
- Se una Skill è inaspettatamente invisibile, controlla prima lo stato di moderazione con `clawhub inspect @owner/slug` dopo aver effettuato l'accesso, prima di modificare metadati relativi al posizionamento.
...
si imposta nuovamente `status` su `open`. Passa `finalAction: "hide"` con una segnalazione
triage per nascondere la Skill nello stesso flusso di lavoro verificabile.
...
- I job di scansione sono asincroni. Le richieste di scansione manuali hanno priorità rispetto al normale lavoro di pubblicazione/backfill, ma il completamento dipende comunque dalla disponibilità dei worker.
...
- Identità della skill, identità del publisher e metadati della versione selezionata sono campi di envelope di primo livello (`slug`, `displayName`, `publisherHandle`, `version`, `resolvedFrom`, `tag`, `createdAt`), così l'automazione shell può leggerli senza spacchettare wrapper annidati.
...
- `provenance` è `server-resolved-github-import` solo quando ClawHub ha risolto e memorizzato un repository/ref/commit/percorso GitHub durante pubblicazione o importazione; altrimenti è `unavailable`.
...
devono visualizzare, come OpenClaw Control UI.
...
- `category` (opzionale): filtro della categoria del plugin. Supportato solo quando la
  ...
  gli alias legacy del filtro v1 sono documentati sotto `GET /api/v1/plugins`.
...
- Le voci delle Skills restano supportate dal registro delle skill e possono ancora essere pubblicate solo tramite `POST /api/v1/skills`.
- `POST /api/v1/packages` rimane solo per i rilasci code-plugin e bundle-plugin.
...
- `category` (opzionale): filtro della categoria del plugin. Supportato solo quando la
  ...
  del filtro sono documentati sotto `GET /api/v1/plugins`.
...
- `category` (opzionale): filtro della categoria del plugin. Valori attuali:
...
- `limit` (opzionale): intero (1-100)
- `isOfficial` (opzionale): `true` o `false`
- `category` (opzionale): filtro per categoria Plugin. Valori attuali:
  `channels`, `models`, `memory`, `context`, `voice`, `media`, `web`,
  `tools`, `runtime`, `gateway`, `security`, `other`.
...
- Il filtro per categoria è un vero filtro API basato su righe di digest delle
  categorie Plugin, non una riscrittura della query di ricerca.
...
- I controlli di ordinamento dell'interfaccia browser per la ricerca Plugin riordinano i risultati di rilevanza caricati,
  in linea con il comportamento attuale di esplorazione di `/skills`.
...
- `version.artifact.kind` è `legacy-zip` per archivi di pacchetti old-world oppure
...
- Le release ClawPack includono i campi compatibili con npm `npmIntegrity`, `npmShasum` e
  `npmTarballName`.
- `version.sha256hash` è un metadato di compatibilità deprecato per vecchi client. Calcola
...
- Questo endpoint è intenzionalmente più ristretto degli endpoint di moderazione per proprietari/moderatori.
  ...
  identità dei segnalatori, corpi delle segnalazioni, prove private o cronologie di revisione interne.
...
- Le versioni ClawPack trasmettono in streaming i byte `.tgz` esatti del npm-pack caricato.
...
- `packageName` è normalizzato come nome npm; il pacchetto può mancare per migrazioni
  pianificate.
- Questo tiene traccia solo della readiness della migrazione. Non modifica OpenClaw né genera
  ClawPack.
...
Segnala un pacchetto per revisione da parte dei moderatori. Le segnalazioni sono a livello di pacchetto, opzionalmente
...
Endpoint moderatore/admin per la revisione della release di un pacchetto.
...
- Gli archivi di Plugin/pacchetto sono file zip con una radice `package/` in modo che i vecchi client OpenClaw
...
- `slug` (obbligatorio)
- `hash` (obbligatorio): sha256 esadecimale a 64 caratteri dell’impronta del bundle
...
Scarica uno ZIP di una versione di skill ospitata, oppure restituisce un handoff sorgente GitHub per una
...
- Campo payload opzionale: `migrateOwner`. Quando `true` con `ownerHandle`, una
  skill esistente può essere spostata a quel proprietario se l’attore è admin/proprietario sia sugli editori
  corrente sia di destinazione. Senza questo opt-in, i cambi di proprietario vengono
  rifiutati.
...
- I campi del modulo consentiti sono `payload`, blob `files` ripetuti, oppure un riferimento a tarball
...
- Le richieste di pubblicazione multipart dirette sono limitate a 18 MB. I tarball ClawPack possono
  usare il flusso upload-url fino al limite di tarball di 120 MB.
...
Le rimozioni nascoste da moderatore/admin e le rimozioni di sicurezza non scadono in questo modo.
...
Per una org appena creata, fornisci `memberHandle`; l’admin agente non viene aggiunto come membro.
...
Creazione self-service autenticata di un editore org. Crea un nuovo editore org e aggiunge il
chiamante come proprietario. Questo endpoint non migra handle utente/personali esistenti e non
contrassegna l’editore come attendibile/official.
...
Solo admin. Riserva slug root e nomi di pacchetto per il legittimo proprietario senza pubblicare una
release. I nomi di pacchetto diventano pacchetti placeholder privati senza righe di release, così lo stesso
proprietario può pubblicare in seguito la vera release code-plugin o bundle-plugin con quel nome.
...
senza modificare le righe dell’account Convex Auth. La richiesta deve indicare entrambi gli id account immutabili del provider
GitHub; gli handle mutabili sono usati solo come protezione rivolta all’operatore.

Per impostazione predefinita, l'endpoint esegue una prova senza applicare modifiche. Per applicare il recupero sono richiesti `dryRun: false` e
`confirmIdentityVerified: true` dopo che lo staff ha verificato in modo indipendente la continuità tra entrambi
i principali GitHub. Il recupero fallisce in modo restrittivo quando il publisher personale corrente dell'utente di destinazione
ha Skills, pacchetti o sorgenti Skills GitHub.
Il recupero migra anche i campi legacy `ownerUserId` per le Skills del publisher recuperato,
gli alias degli slug delle Skills, i pacchetti, gli avvisi dell'ispettore dei pacchetti e le righe derivate dei digest di ricerca, così
i percorsi con proprietario diretto concordano con la nuova autorità del publisher. Anche una prenotazione attiva con handle protetto
...
100 righe per transazione di applicazione; i recuperi più grandi devono prima usare una migrazione proprietario riprendibile.
Le sorgenti Skills GitHub sono nell'ambito del publisher e vengono riportate come controllate anziché riscritte.
...
contenuti (solo amministratore). Per impostazione predefinita esegue una prova senza applicare modifiche, a meno che `dryRun` sia `false`.
...
Aggiungi/rimuovi una stella (elementi in evidenza). Entrambi gli endpoint sono idempotenti.
...
Se ospiti autonomamente, servi questo file (oppure imposta `CLAWHUB_REGISTRY` esplicitamente; legacy `CLAWDHUB_REGISTRY`).
