...
x-i18n:
    generated_at: "2026-07-03T02:48:51Z"
    model: gpt-5.5
...
- I risultati vengono restituiti in ordine di rilevanza (similarità dell'embedding + boost per token esatti di slug/nome + un piccolo prior di popolarità).
- La rilevanza è più forte della popolarità. Una corrispondenza precisa di slug o token del nome visualizzato può superare una corrispondenza più lasca con coinvolgimento molto più alto.
...
Linee guida per la reperibilità dei publisher:
...
- `createdAt` è stabile per i crawl delle nuove Skill; `updated` cambia quando Skills esistenti vengono ripubblicate.
...
Endpoint autenticato per l’archivio dei report archiviati per versioni inviate.
...
Route canonica, riservata agli amministratori, per rieseguire scansioni batch. Accetta la stessa forma di payload della route legacy `POST /api/v1/skills/-/rescan-batch`.
...
- `provenance` è `server-resolved-github-import` solo quando ClawHub ha risolto e archiviato repo/ref/commit/path GitHub durante la pubblicazione o l’importazione; altrimenti è `unavailable`.
...
- Le Skill correnti supportate da GitHub con una scansione `clean` o `suspicious` includono
  ...
  hash del contenuto e URL archivio. Non includono file sorgente ospitati su ClawHub.
...
- `category` (opzionale): filtro per categoria di Plugin. Valori attuali:
  `channels`, `models`, `memory`, `context`, `voice`, `media`, `web`,
  `tools`, `runtime`, `gateway`, `security`, `other`.
...
- Il filtro per categoria è un vero filtro API supportato da righe digest delle
...
- I controlli di ordinamento dell'interfaccia browser per la ricerca dei Plugin riordinano i risultati di rilevanza caricati,
  in modo coerente con il comportamento di esplorazione attuale di `/skills`.
...
- `version.artifact.kind` è `legacy-zip` per gli archivi pacchetto del vecchio mondo o
...
- Questa è la superficie resolver di OpenClaw; evita di dedurre il formato dell'archivio da
  un URL condiviso.

### `GET /api/v1/packages/{name}/versions/{version}/artifact/download`

Scarica l'artefatto della versione tramite il percorso resolver esplicito.

Note:

- Le versioni ClawPack inviano in streaming i byte `.tgz` esatti di npm-pack caricati.
...
- `packageName` è normalizzato come nome npm; il pacchetto può mancare per migrazioni
  pianificate.
- Traccia solo la readiness della migrazione. Non modifica OpenClaw né genera
  ClawPack.
...
- Richiede un token API per un utente moderator o admin.
...
Endpoint owner/moderator per la visibilità della moderazione del pacchetto.

Autenticazione:

- Richiede un token API per il proprietario del pacchetto, un membro publisher, un moderator o
  un utente admin.
...
Endpoint moderator/admin per la revisione della release di un pacchetto.
...
- `quarantined`: bloccata in attesa di seguito.
...
Scarica uno ZIP di una versione di competenza ospitata, oppure restituisce un passaggio di consegne al sorgente GitHub per una
competenza corrente basata su GitHub con scansione `clean` o `suspicious` e nessuna versione
ospitata.
...
- I passaggi di consegne delle competenze basate su GitHub non fanno proxy né mirror dei byte. La risposta JSON
...
- Campo payload facoltativo: `migrateOwner`. Quando `true` con `ownerHandle`, una
  competenza esistente può essere spostata a quel proprietario se l'attore è admin/proprietario sia sul
...
Elimina in modo reversibile / ripristina una competenza (proprietario, moderator o admin).
...
Quando presente, `reason` viene memorizzato come nota di moderazione della competenza e copiato nel registro di audit.
...
Gli occultamenti moderator/admin e le rimozioni di sicurezza non scadono in questo modo.
...
- `401`: non autorizzato
- `403`: vietato
- `404`: competenza/utente non trovato
- `500`: errore interno del server

### `POST /api/v1/users/publisher`

Solo admin. Garantisce l'esistenza di un publisher organizzazione per un handle. Se l'handle punta ancora a un
utente/publisher personale condiviso legacy, l'endpoint lo migra prima in un publisher organizzazione.
...
senza modificare le righe account Convex Auth. La richiesta deve nominare entrambi gli ID account provider GitHub
...
principal GitHub. Il recupero fallisce in modo chiuso quando l'utente di destinazione ha attualmente un
publisher personale con skills, pacchetti o sorgenti di skill GitHub.
...
per l'handle recuperato viene riassegnata all'utente sostitutivo, così che la successiva
...
Le sorgenti di skill GitHub sono nell'ambito del publisher e vengono segnalate come controllate invece che riscritte.
...
i contenuti (solo admin). Usa per impostazione predefinita la modalità dry-run, a meno che `dryRun` sia `false`.
