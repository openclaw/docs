...
- ClawHub usa header di inoltro attendibili per identificare gli IP dei client all'edge.
...
- `highlightedOnly` (opzionale): `true` per filtrare sulle Skills evidenziate
...
- La rilevanza è più forte della popolarità. Una corrispondenza precisa di slug o token del nome visualizzato può superare una corrispondenza più vaga con un coinvolgimento molto più alto.
...
- Gli alias di rinomina preservano la risoluzione per i vecchi URL e per le installazioni che si risolvono tramite il registro, ma il ranking di ricerca si basa sui metadati canonici della skill dopo che la rinomina è stata indicizzata. Le statistiche esistenti rimangono con la skill.
...
- `status` (opzionale): `open` (predefinito), `confirmed`, `dismissed` oppure `all`
- `limit` (opzionale): intero (1-200)
...
si reimposta `status` su `open`. Passa `finalAction: "hide"` con una segnalazione esaminata
...
- Le scansioni pubblicate richiedono l'accesso di gestione del proprietario/pubblicatore oppure l'autorità di moderatore/amministratore della piattaforma.
...
- Richiede l'accesso di gestione del proprietario/pubblicatore alla skill o al plugin, oppure l'autorità di moderatore/amministratore della piattaforma.
- Restituisce i risultati di scansione memorizzati per la versione esatta inviata, incluse le versioni bloccate o nascoste.
- `kind` usa come predefinito `skill`; usa `kind=plugin` per le scansioni di plugin/pacchetti.
- Restituisce la stessa forma ZIP dei download delle richieste di scansione.

### `POST /api/v1/skills/-/scan/batch`

Route canonica solo per amministratori per la nuova scansione in batch. Accetta la stessa forma di payload della route legacy `POST /api/v1/skills/-/rescan-batch`.
...
- Identità della skill, identità del pubblicatore e metadati della versione selezionata sono campi di primo livello dell'envelope (`slug`, `displayName`, `publisherHandle`, `version`, `resolvedFrom`, `tag`, `createdAt`) così l'automazione shell può leggerli senza decomprimere wrapper annidati.
...
- Usa `/verify` quando hai bisogno dell'envelope di verifica della Skill Card per una singola skill, `/card` quando hai bisogno del markdown della scheda generata e `/scan` quando hai bisogno dei dati dettagliati degli scanner.
...
- `POST /api/v1/packages` è ancora solo per release di Plugin di codice e Plugin bundle.
...
- I chiamanti anonimi vedono solo canali di pacchetti pubblici.
- I chiamanti autenticati possono cercare pacchetti privati dei publisher a cui appartengono.
- `channel=private` restituisce solo i pacchetti che il chiamante autenticato può leggere.

### `GET /api/v1/plugins`

Esplorazione del catalogo solo Plugin tra pacchetti Plugin di codice e Plugin bundle.
...
Gli alias legacy dei filtri v1 rimangono accettati sugli endpoint di lettura:
...
Esportazione in blocco degli ultimi rilasci pubblici di Plugin per analisi offline.
...
Ricerca limitata ai Plugin tra pacchetti code-plugin e bundle-plugin.
...
- `limit` (facoltativo): intero (1-100)
- `isOfficial` (facoltativo): `true` o `false`
- `category` (facoltativo): filtro di categoria del Plugin. Valori correnti:
  `channels`, `models`, `memory`, `context`, `voice`, `media`, `web`,
  `tools`, `runtime`, `gateway`, `security`, `other`.
...
- I risultati vengono restituiti in ordine di rilevanza e attualmente non usano paginazione.
- I controlli di ordinamento dell'interfaccia browser per la ricerca Plugin riordinano i risultati di rilevanza caricati,
  corrispondendo al comportamento attuale di esplorazione di `/skills`.
...
- Richiede un token API per il proprietario del pacchetto, un proprietario/amministratore dell'organizzazione publisher,
...
- `version.artifact.kind` è `legacy-zip` per gli archivi di pacchetto vecchio stile oppure
...
Restituisce il riepilogo esatto di sicurezza e attendibilità del rilascio del pacchetto per i client
di installazione. Questa è la superficie pubblica di consumo di OpenClaw per decidere se un
...
- Questo endpoint è intenzionalmente più ristretto degli endpoint di moderazione per proprietari/moderatori.
  ...
  identità dei segnalanti, corpi delle segnalazioni, prove private o cronologie interne
  di revisione.
...
- Usa il bucket di limite di frequenza per i download.
...
Endpoint moderatore/amministratore per le code di revisione dei rilasci di pacchetti.
...
Endpoint moderator/admin per l'acquisizione delle segnalazioni dei pacchetti.
...
Endpoint moderator/admin per risolvere o riaprire le segnalazioni dei pacchetti.
...
- Le intestazioni di download includono lo SHA-256 ClawHub più i metadati integrity/shasum di npm.
...
Scarica uno ZIP di una versione Skills ospitata, oppure restituisce un passaggio di consegna al sorgente GitHub per una
...
- I passaggi di consegna delle Skills basate su GitHub non fanno proxy né mirror dei byte. La risposta JSON
  include `sourceRef: "public-github"`, `repo`, `commit`, `path`, `contentHash`
  e `archiveUrl`; lo stato della scansione/corrente è un gate e non è incluso come metadati del payload
  di successo.
...
- Campo payload opzionale: `migrateOwner`. Quando `true` con `ownerHandle`, una
  Skills esistente può spostarsi a quel proprietario se l'attore è admin/proprietario sia sul
...
- I campi del modulo consentiti sono `payload`, blob `files` ripetuti o un riferimento tarball `clawpack`.
...
Eliminazione reversibile / ripristino di una Skills (proprietario, moderatore o admin).
...
Quando presente, `reason` viene salvato come nota di moderazione della Skills e copiato nel registro di audit.
...
Le rimozioni nascoste da moderatore/admin e per sicurezza non scadono in questo modo.
...
Solo admin. Garantisce l'esistenza di un publisher dell'organizzazione per un handle. Se l'handle punta ancora a un
...
Creazione autenticata self-service di publisher dell'organizzazione. Crea un nuovo publisher dell'organizzazione e aggiunge il
...
`confirmIdentityVerified: true` dopo che lo staff ha verificato in modo indipendente la continuità tra entrambe le
identità GitHub. Il ripristino fallisce in modo chiuso quando l'attuale publisher personale dell'utente di destinazione
...
Le origini skill GitHub sono con ambito publisher e vengono segnalate come controllate anziché riscritte.
