...
x-i18n:
    generated_at: "2026-07-01T05:45:15Z"
    model: gpt-5.5
...
Directory di terze parti possono usare gli endpoint pubblici di lettura per elencare o cercare Skills di ClawHub. Memorizza i risultati nella cache, rispetta `429`/`Retry-After`, rimanda gli utenti alla scheda canonica di ClawHub (`https://clawhub.ai/<owner>/skills/<slug>`) ed evita di implicare l'approvazione di ClawHub per il sito di terze parti. Non tentare di replicare contenuti nascosti, privati o bloccati dalla moderazione al di fuori della superficie API pubblica.

Le scorciatoie degli slug Web si risolvono tra famiglie di registri, ma i client API devono usare
...
- Se non è disponibile alcun IP client attendibile, le richieste anonime usano bucket di fallback
  con ambito solo per tipo di limite di frequenza. Questi bucket di fallback non includono
...
- `highlightedOnly` (opzionale): `true` per filtrare solo Skills evidenziate
...
- La rilevanza è più forte della popolarità. Una corrispondenza precisa dello slug o di un token del nome visualizzato può superare una corrispondenza più vaga con molto più engagement.
...
- La popolarità è scalata logaritmicamente e limitata. Skills con engagement elevato possono posizionarsi più in basso quando il testo della query è una corrispondenza più debole.
...
- Se una Skill è inaspettatamente invisibile, controlla prima lo stato di moderazione con `clawhub inspect @owner/slug` dopo l'accesso, prima di modificare metadati correlati al ranking.
...
- Le evidenze sono oscurate per i chiamanti pubblici e includono snippet grezzi solo per proprietari/moderatori.
...
Endpoint moderatore/admin per l'acquisizione delle segnalazioni sulle Skill.
...
Endpoint moderatore/admin per risolvere o riaprire segnalazioni sulle Skill.
...
si reimposta `status` a `open`. Passa `finalAction: "hide"` con una segnalazione analizzata
...
- `kind` ha valore predefinito `skill`; usa `kind=plugin` per le scansioni di plugin/pacchetti.
...
- L’identità della skill, l’identità del publisher e i metadati della versione selezionata sono campi di primo livello dell’envelope (`slug`, `displayName`, `publisherHandle`, `version`, `resolvedFrom`, `tag`, `createdAt`) così l’automazione shell può leggerli senza spacchettare wrapper annidati.
...
endpoint di raccolta è pensato per client che sanno già quali versioni installate di
skill ClawHub devono visualizzare, come OpenClaw Control UI.
...
- Valore predefinito: versione più recente.
...
- `category` (facoltativo): filtro categoria plugin. Supportato solo quando la
  richiesta ha ambito sui pacchetti plugin (`/api/v1/plugins`,
...
- Le voci delle skill restano supportate dal registro delle skill e possono ancora essere pubblicate solo tramite `POST /api/v1/skills`.
- `POST /api/v1/packages` resta solo per release di code-plugin e bundle-plugin.
...
- `isOfficial` (facoltativo): `true` o `false`
- `category` (facoltativo): filtro categoria plugin. Supportato solo quando la
  richiesta ha ambito sui pacchetti plugin. Le categorie controllate e gli alias
...
Esportazione bulk delle skill pubbliche più recenti per analisi offline.
...
- Le skill ospitate includono i file della versione memorizzata più recente e sono elencate in
  `_manifest.json` con `sourceRef: "public-clawhub"`.
- Le skill attuali supportate da GitHub con scansione `clean` o `suspicious` includono
...
Esportazione in blocco degli ultimi rilasci pubblici dei plugin per analisi offline.
...
- `version.artifact.kind` è `legacy-zip` per gli archivi di pacchetti del vecchio formato o
...
di installazione. Questa è la superficie di consumo pubblica di OpenClaw per decidere se un
...
Endpoint moderatore/amministratore per le code di revisione dei rilasci dei pacchetti.
...
Endpoint moderator/admin per l'acquisizione delle segnalazioni sui pacchetti.
...
Endpoint moderator/admin per risolvere o riaprire segnalazioni sui pacchetti.
...
- Gli archivi Plugin/pacchetto sono file zip con una root `package/` così i vecchi client OpenClaw
...
Restituisce un packument compatibile con npm per versioni di pacchetto basate su ClawPack.
...
Scarica uno ZIP di una versione Skills ospitata, oppure restituisce un passaggio alla sorgente GitHub per una
Skills corrente basata su GitHub con una scansione `clean` o `suspicious` e senza versione
ospitata.
...
- Le versioni soft-deleted restituiscono `410`.
- I passaggi delle Skills basate su GitHub non fanno proxy né mirror dei byte. La risposta JSON
...
- Campo payload facoltativo: `migrateOwner`. Quando `true` con `ownerHandle`, una
  Skills esistente può essere spostata a quel proprietario se l'attore è admin/proprietario sia sul
  publisher corrente sia su quello di destinazione. Senza questo opt-in, le modifiche del proprietario sono
  rifiutate.
...
Elimina reversibilmente / ripristina una Skills (proprietario, moderatore o admin).
...
Quando presente, `reason` viene archiviato come nota di moderazione della Skills e copiato nel registro di audit.
...
Per una org appena creata, fornisci `memberHandle`; l'admin che agisce non viene aggiunto come membro.
`memberRole` usa per impostazione predefinita `owner`.
...
senza modificare le righe account di Convex Auth. La richiesta deve nominare entrambi gli id account
provider GitHub immutabili; gli handle mutabili sono usati solo come guardrail rivolto all'operatore.
...
i percorsi del proprietario diretto concordino con la nuova autorità dell'editore. Anche una prenotazione active protected-handle
per l'handle recuperato viene riassegnata all'utente sostitutivo, così la sincronizzazione successiva del profilo
...
Le sorgenti Skills GitHub hanno ambito editore e vengono riportate come controllate invece che riscritte.
...
Cambia il motivo archiviato per un ban esistente senza rimuovere il ban né ripristinare
i contenuti (solo admin). Usa per impostazione predefinita la simulazione, a meno che `dryRun` sia `false`.
...
Se effettui il self-hosting, servi questo file (oppure imposta `CLAWHUB_REGISTRY` esplicitamente; legacy `CLAWDHUB_REGISTRY`).
