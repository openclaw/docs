...
x-i18n:
    generated_at: "2026-06-30T14:06:39Z"
    model: gpt-5.5
...
Le directory di terze parti possono usare gli endpoint pubblici di lettura per elencare o cercare le skill di ClawHub. Metti in cache i risultati, rispetta `429`/`Retry-After`, collega gli utenti alla scheda canonica di ClawHub (`https://clawhub.ai/<owner>/skills/<slug>`) ed evita di suggerire un'approvazione di ClawHub per il sito di terze parti. Non tentare di replicare contenuti nascosti, privati o bloccati dalla moderazione al di fuori della superficie API pubblica.

Le scorciatoie degli slug web si risolvono tra le famiglie del registro, ma i client API devono usare
gli URL canonici restituiti dagli endpoint di lettura invece di ricostruire la
...
Le risposte di errore pubbliche v1 sono testo normale con `content-type: text/plain; charset=utf-8`.
...
- I risultati vengono restituiti in ordine di pertinenza (similarità embedding + boost per token esatti di slug/nome + una piccola priorità di popolarità).
- La pertinenza è più forte della popolarità. Una corrispondenza precisa di slug o token del nome visualizzato può superare una corrispondenza più debole con un coinvolgimento molto maggiore.
...
- `security.signals.dependencyRegistry` è mantenuto per compatibilità con la risposta v1, ma lo scanner di esistenza nel registro delle dipendenze è stato ritirato e questa chiave è sempre `null`.
...
- Le voci Skills restano basate sul registro delle Skills e possono ancora essere pubblicate solo tramite `POST /api/v1/skills`.
...
- `version.sha256hash` è un metadato di compatibilità deprecato per i vecchi client. Esegue l'hash
  degli esatti byte ZIP restituiti da `/api/v1/packages/{name}/download`.
...
Restituisce il riepilogo esatto di sicurezza e attendibilità della release del pacchetto per i client di installazione.
Questa è la superficie pubblica di consumo OpenClaw per decidere se una
...
- Questo endpoint è intenzionalmente più ristretto degli endpoint di moderazione per proprietari/moderatori.
  ...
  identità dei segnalatori, corpi delle segnalazioni, prove private o tempistiche di revisione
  interne.
...
- Le versioni ClawPack trasmettono in streaming gli esatti byte `.tgz` npm-pack caricati.
...
Restituisce la readiness calcolata per il futuro consumo OpenClaw.
...
- Gli archivi Plugin/pacchetto sono file zip con una radice `package/` così i vecchi
  client OpenClaw continuano a funzionare.
...
- Sono elencate solo le versioni con tarball ClawPack npm-pack caricati.
...
Scarica uno ZIP di versione Skill ospitata, oppure restituisce un passaggio di consegne del sorgente GitHub per una
...
- I passaggi di consegne delle Skill basate su GitHub non eseguono proxy né mirroring dei byte. La risposta JSON
...
- È accettato anche il corpo JSON con `files` (basato su storageId).
...
- Campo payload opzionale: `migrateOwner`. Quando `true` con `ownerHandle`, una
  Skill esistente può passare a quel proprietario se l'attore è admin/proprietario sia sul
...
- I campi form consentiti sono `payload`, blob `files` ripetuti, oppure un riferimento a tarball
...
Solo admin. Garantisce che esista un publisher dell'organizzazione per un handle. Se l'handle punta ancora a un
publisher condiviso legacy utente/personale, l'endpoint lo migra prima in un publisher dell'organizzazione.
...
Solo admin. Riserva slug radice e nomi di pacchetto per un legittimo proprietario senza pubblicare una
...
principali GitHub. Il ripristino fallisce in modo chiuso quando l'utente di destinazione ha un editore personale
corrente con Skills, pacchetti o sorgenti di Skills GitHub.
...
i percorsi del proprietario diretto concordano con la nuova autorità dell'editore. Anche una prenotazione attiva di handle protetto
...
Le sorgenti di Skills GitHub sono nell'ambito dell'editore e vengono segnalate come controllate invece di essere riscritte.
...
Modifica il motivo archiviato per un ban esistente senza rimuovere il ban o ripristinare
