...
- Usa un backoff con jitter per evitare retry sincronizzati.
...
- I risultati vengono restituiti in ordine di rilevanza (similarità embedding + boost per token slug/nome esatti + un piccolo prior di popolarità).
- La rilevanza è più forte della popolarità. Una corrispondenza precisa di slug o token del nome visualizzato può superare una corrispondenza più vaga con un coinvolgimento molto più forte.
- Il testo ASCII viene tokenizzato su confini di parole e punteggiatura. Per esempio, `personal-map` contiene un token autonomo `map`, mentre `amap-jsapi-skill` contiene `amap`, `jsapi` e `skill`; cercare `map` quindi dà a `personal-map` una corrispondenza lessicale più forte rispetto a `amap-jsapi-skill`.
...
- Gli alias di rinomina preservano la risoluzione per i vecchi URL e per le installazioni che vengono risolte tramite il registro, ma il ranking di ricerca si basa sui metadati canonici della Skill dopo l'indicizzazione della rinomina. Le statistiche esistenti restano associate alla Skill.
- Se una Skill è inaspettatamente invisibile, controlla prima lo stato di moderazione con `clawhub inspect @owner/slug` dopo aver effettuato l'accesso, prima di modificare metadati correlati al ranking.
...
si imposta `status` di nuovo a `open`. Passa `finalAction: "hide"` con una segnalazione
triaged per nascondere la Skill nello stesso flusso di lavoro verificabile.
...
- Restituisce `queue.queuedAhead` e `queue.position` mentre la richiesta è in coda, così i client possono mostrare quante scansioni manuali prioritarie precedono la richiesta. Le code molto grandi sono limitate e segnalate con `queuedAheadIsEstimate: true`.
...
- Richiede accesso di gestione come proprietario/editore della skill o del plugin, oppure autorità di moderatore/amministratore della piattaforma.
...
- I risultati sono per elemento; una skill o una versione mancante non causa il fallimento dell'intera risposta.
- La risposta riguarda solo la sicurezza. Non include dati della scheda Skill, stato della scheda generata, elenchi di file artefatto o payload dettagliati degli scanner.
...
- `security.signals.dependencyRegistry` è mantenuto per compatibilità con la risposta v1, ma lo scanner di esistenza del registro delle dipendenze è stato ritirato e questa chiave è sempre `null`.
- L'assenza della scheda Skill non influisce su `ok`, `decision` o `reasons` di questo endpoint; i client dovrebbero leggere localmente `skill-card.md` installato quando hanno bisogno del contenuto della scheda.
...
- `category` (facoltativo): filtro categoria Plugin. Supportato solo quando la
  ...
  `/api/v1/code-plugins`, `/api/v1/bundle-plugins` o endpoint di pacchetto con
  `family=code-plugin`/`family=bundle-plugin`). Le categorie controllate e gli
  alias dei filtri v1 legacy sono documentati in `GET /api/v1/plugins`.
...
- Le voci Skill continuano a essere basate sul registro Skills e possono ancora essere pubblicate solo tramite `POST /api/v1/skills`.
...
- `category` (facoltativo): filtro categoria Plugin. Supportato solo quando la
  ...
  dei filtri v1 legacy sono documentati in `GET /api/v1/plugins`.
...
Gli alias dei filtri v1 legacy rimangono accettati sugli endpoint di lettura:
...
- Le Skills ospitate includono i file della versione memorizzata più recente e sono elencate in
...
- Il filtro per categoria è un vero filtro API supportato dalle righe del digest
...
- I controlli di ordinamento dell'interfaccia browser per la ricerca dei Plugin riordinano i risultati di rilevanza caricati,
  in linea con l'attuale comportamento di esplorazione di `/skills`.
...
- Le release ClawPack includono i campi compatibili con npm `npmIntegrity`, `npmShasum` e
  `npmTarballName`.
- `version.sha256hash` è un metadato di compatibilità deprecato per i vecchi client. Esegue
...
- Questo endpoint è intenzionalmente più ristretto degli endpoint di moderazione per proprietari/moderatori.
  ...
  le identità dei segnalanti, i corpi delle segnalazioni, le prove private o le tempistiche di revisione
  interne.
...
- Questa è la superficie del resolver di OpenClaw; evita di indovinare il formato dell'archivio da
  un URL condiviso.
...
Restituisce la preparazione calcolata per il futuro consumo di OpenClaw.

I controlli di preparazione coprono:
...
- `packageName` è normalizzato come nome npm; il pacchetto può mancare per le migrazioni
  pianificate.
- Questo tiene traccia solo della preparazione della migrazione. Non modifica OpenClaw né genera
  ClawPack.
...
- `manual`: qualsiasi release con una sostituzione manuale di moderazione.
- `all`: qualsiasi release con una sostituzione manuale, stato di scansione non pulito o segnalazione del pacchetto.
...
- Usa il bucket di limitazione delle letture, non il bucket dei download.
...
- Usa il bucket di limitazione dei download.
...
Scarica lo ZIP di una versione di skill ospitata, oppure restituisce un passaggio di consegne al sorgente GitHub per una
...
- I campi form consentiti sono `payload`, blob `files` ripetuti o un riferimento tarball `clawpack`.
...
Per un'organizzazione appena creata, fornisci `memberHandle`; l'admin agente non viene aggiunto come membro.
`memberRole` ha come impostazione predefinita `owner`.
...
principal GitHub. Il recupero fallisce in modo chiuso quando il publisher personale corrente dell’utente di destinazione
...
gli alias di slug delle skill, i pacchetti, gli avvisi dell’ispettore pacchetti e le righe digest di ricerca derivate, in modo che
...
Le sorgenti skill GitHub sono nell’ambito del publisher e vengono segnalate come controllate anziché riscritte.
...
Banna un utente ed elimina definitivamente le skill di proprietà (solo moderatore/admin).
...
Aggiunge/rimuove una star (highlight). Entrambi gli endpoint sono idempotenti.
...
che preparano un tarball ClawPack devono inviare l’id storage risultante come
`clawpack` e il ticket restituito come `clawpackUploadTicket`.

## Rilevamento del registry (`/.well-known/clawhub.json`)

La CLI può rilevare le impostazioni di registry/auth dal sito:
