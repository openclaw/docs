...
x-i18n:
    generated_at: "2026-07-03T23:34:37Z"
    model: gpt-5.5
...
Puoi creare un catalogo, una directory o una superficie di ricerca di terze parti basati sulle API pubbliche di lettura di ClawHub. I metadati pubblici degli Skills e i file degli Skills sono pubblicati secondo le regole di licenza degli Skills di ClawHub, mentre l'API stessa è soggetta a limiti di frequenza e deve essere usata responsabilmente.
...
- Inserisci un link all'URL canonico dello Skill su ClawHub quando mostri gli elenchi, così gli utenti possono ispezionare il record del registro di origine.
...
- `GET /api/v1/download?slug=&version=&tag=`
  - Gli Skills ospitati restituiscono byte ZIP deterministici.
  - Gli Skills correnti basati su GitHub con una scansione `clean` o `suspicious` restituiscono un
    descrittore di passaggio JSON `public-github` invece dei byte di ClawHub.
- `GET /api/v1/skills/export?startDate=&endDate=&limit=&cursor=`
  - Gli Skills ospitati vengono esportati come file archiviati.
  - Gli Skills correnti basati su GitHub con una scansione `clean` o `suspicious` vengono esportati
...
I percorsi legacy `/api/*` e `/api/cli/*` sono ancora disponibili. Vedi `DEPRECATIONS.md`.
