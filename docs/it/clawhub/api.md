...
x-i18n:
    generated_at: "2026-07-03T00:57:59Z"
    model: gpt-5.5
...
Puoi creare un catalogo, una directory o una superficie di ricerca di terze parti basata sulle API di lettura pubbliche di ClawHub. I metadati pubblici delle skill e i file delle skill sono pubblicati secondo le regole di licenza delle skill di ClawHub, mentre l'API stessa è soggetta a limiti di frequenza e deve essere usata in modo responsabile.
...
- Non eseguire il mirror di contenuti nascosti, privati o bloccati dalla moderazione aggirando i filtri dell'API pubblica o i confini di autenticazione.
...
- Token mancante/non valido: ritorna all'applicazione per IP.
...
- `RateLimit-Reset`: secondi di attesa fino al reset
...
- `Retry-After`: secondi di attesa su `429`
...
- `GET /api/v1/skills/export?startDate=&endDate=&limit=&cursor=`
  - Le Skills ospitate vengono esportate come file memorizzati.
...
Gli endpoint legacy `/api/*` e `/api/cli/*` sono ancora disponibili. Vedi `DEPRECATIONS.md`.
