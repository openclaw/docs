...
x-i18n:
    generated_at: "2026-07-05T05:11:09Z"
    model: gpt-5.5
...
Puoi creare un catalogo, una directory o una superficie di ricerca di terze parti sopra le API pubbliche di lettura di ClawHub. I metadati pubblici delle Skills e i file delle Skills sono pubblicati secondo le regole di licenza delle Skills di ClawHub, mentre l'API stessa è soggetta a limiti di frequenza e deve essere consumata responsabilmente.
...
- Memorizza nella cache le risposte e rispetta `429`, `Retry-After` e le intestazioni dei limiti di frequenza invece di eseguire polling aggressivo.
- Inserisci un link all'URL canonico della Skill su ClawHub quando mostri gli elenchi, così gli utenti possono ispezionare il record del registro sorgente.
...
- `X-RateLimit-Reset`: secondi dell'epoca Unix (tempo assoluto di reset)
- `RateLimit-Reset`: secondi di ritardo fino al reset
- `X-RateLimit-Remaining` / `RateLimit-Remaining`: budget rimanente esatto quando
  presente; le richieste riuscite distribuite su shard lo omettono invece di restituire un valore
...
- `GET /api/v1/download?slug=&version=&tag=`
  ...
  - Le Skills correnti basate su GitHub con una scansione `clean` o `suspicious` restituiscono un
    descrittore di handoff JSON `public-github` invece dei byte di ClawHub.
- `GET /api/v1/skills/export?startDate=&endDate=&limit=&cursor=`
  - Le Skills ospitate sono esportate come file archiviati.
  - Le Skills correnti basate su GitHub con una scansione `clean` o `suspicious` sono esportate
