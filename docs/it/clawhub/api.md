...
x-i18n:
    generated_at: "2026-07-04T20:32:47Z"
    model: gpt-5.5
...
Puoi creare un catalogo, una directory o una superficie di ricerca di terze parti basati sulle API pubbliche di lettura di ClawHub. I metadati pubblici delle Skills e i file delle Skills sono pubblicati secondo le regole di licenza delle Skills di ClawHub, mentre l'API stessa è soggetta a limiti di frequenza e deve essere usata in modo responsabile.
...
- Memorizza le risposte nella cache e rispetta gli header `429`, `Retry-After` e di limite di frequenza invece di effettuare polling aggressivo.
...
- Token mancante/non valido: ripiego sull'applicazione per IP.
...
I percorsi legacy `/api/*` e `/api/cli/*` sono ancora disponibili. Consulta `DEPRECATIONS.md`.
