...
summary: Panoramica e convenzioni della REST API pubblica (v1).
x-i18n:
...
Puoi creare un catalogo, una directory o una superficie di ricerca di terze parti sopra le API di lettura pubbliche di ClawHub. I metadati pubblici delle skill e i file delle skill sono pubblicati in base alle regole di licenza per le skill di ClawHub, mentre l'API stessa è soggetta a limiti di frequenza e deve essere usata in modo responsabile.
...
Gli endpoint `/api/*` e `/api/cli/*` delle versioni precedenti sono ancora disponibili. Vedi `DEPRECATIONS.md`.
