...
x-i18n:
    generated_at: "2026-07-01T15:25:15Z"
    model: gpt-5.5
...
Puoi creare un catalogo, una directory o una superficie di ricerca di terze parti basata sulle API pubbliche di lettura di ClawHub. I metadati pubblici delle Skills e i file delle Skills sono pubblicati secondo le regole di licenza delle Skills di ClawHub, mentre l'API stessa è soggetta a limiti di frequenza e deve essere consumata responsabilmente.
...
- Inserisci un link all'URL canonico della skill ClawHub quando mostri gli elenchi, così gli utenti possono esaminare il record del registro di origine.
...
- Non effettuare il mirror di contenuti nascosti, privati o bloccati dalla moderazione aggirando i filtri delle API pubbliche o i confini di autenticazione.
...
- `GET /api/v1/download?slug=&version=&tag=`
  ...
  - Le Skills correnti supportate da GitHub con una scansione `clean` o `suspicious` restituiscono un
...
- `GET /api/v1/skills/export?startDate=&endDate=&limit=&cursor=`
  ...
  - Le Skills correnti supportate da GitHub con una scansione `clean` o `suspicious` vengono esportate
...
- `POST /api/v1/users/reserve` riserva slug radice e segnaposto privati per pacchetti senza rilascio per un handle proprietario.
