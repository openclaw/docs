...
x-i18n:
    generated_at: "2026-05-12T00:55:57Z"
    model: gpt-5.5
    provider: openai
...
Puoi creare un catalogo, una directory o una superficie di ricerca di terze parti basati sulle API di lettura pubbliche di ClawHub. I metadati pubblici degli skill e i file degli skill sono pubblicati secondo le regole di licenza degli skill di ClawHub, mentre l'API stessa è soggetta a limiti di frequenza e deve essere usata in modo responsabile.
...
- Inserisci un link all'URL canonico dello skill ClawHub quando mostri gli elenchi, così gli utenti possono ispezionare il record del registro di origine.
...
- `POST /api/v1/users/reserve` riserva slug radice e placeholder di pacchetti privati senza release per un handle proprietario.
