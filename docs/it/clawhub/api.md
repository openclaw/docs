...
x-i18n:
    generated_at: "2026-07-01T08:06:05Z"
    model: gpt-5.5
...
Puoi creare un catalogo, una directory o una superficie di ricerca di terze parti basata sulle API pubbliche di lettura di ClawHub. I metadati pubblici delle skill e i file delle skill sono pubblicati secondo le regole della licenza per le skill di ClawHub, mentre l'API stessa è soggetta a limiti di frequenza e deve essere usata in modo responsabile.
...
- `X-RateLimit-Reset`: secondi dell'epoca Unix (ora assoluta di reset)
...
- `GET /api/v1/download?slug=&version=&tag=`
  ...
  - Le skill correnti basate su GitHub con una scansione `clean` o `suspicious` restituiscono un
    descrittore JSON di passaggio `public-github` invece dei byte di ClawHub.
