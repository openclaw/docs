...
x-i18n:
    generated_at: "2026-07-04T10:43:17Z"
    model: gpt-5.5
...
Puoi creare un catalogo, una directory o una superficie di ricerca di terze parti basati sulle API pubbliche di lettura di ClawHub. I metadati pubblici delle skill e i file delle skill sono pubblicati secondo le regole di licenza delle skill di ClawHub, mentre l'API stessa è soggetta a limiti di frequenza e deve essere consumata in modo responsabile.
...
- Non effettuare il mirroring di contenuti nascosti, privati o bloccati dalla moderazione aggirando i filtri dell'API pubblica o i confini di autenticazione.
...
- Token mancante/non valido: fallback all'applicazione per IP.
...
- `X-RateLimit-Remaining` / `RateLimit-Remaining`: budget rimanente esatto quando
  presente; le richieste riuscite shardate lo omettono invece di restituire un valore
...
- Gli errori v1 sono testo semplice (`text/plain; charset=utf-8`), inclusi `400`,
  `401`, `403`, `404`, `429` e le risposte per download bloccato.
...
- `POST /api/v1/users/reserve` riserva slug radice e segnaposto privati di pacchetti senza rilascio per un handle proprietario.
