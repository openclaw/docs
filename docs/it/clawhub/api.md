...
Puoi creare un catalogo, una directory o una superficie di ricerca di terze parti basandoti sulle API di lettura pubbliche di ClawHub. I metadati delle skill pubbliche e i file delle skill sono pubblicati secondo le regole di licenza delle skill di ClawHub, mentre l'API stessa è soggetta a limiti di frequenza e deve essere usata responsabilmente.
...
- `X-RateLimit-Reset`: secondi dall'epoca Unix (orario assoluto di reset)
...
- In caso contrario usa `RateLimit-Reset` o deriva il ritardo da `X-RateLimit-Reset`.
...
- `POST /api/v1/users/reserve` riserva gli slug root e i segnaposto privati di pacchetti senza release per un handle proprietario.
