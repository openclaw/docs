...
- Collega all'URL canonico della skill ClawHub quando mostri gli elenchi, così gli utenti possono ispezionare il record del registro di origine.
- Usa URL di pagina canonici nel formato `https://clawhub.ai/<owner>/<slug>`.
- Non suggerire che ClawHub approvi, verifichi o gestisca il sito di terze parti.
...
Intestazioni: `X-RateLimit-Limit`, `X-RateLimit-Remaining`, `X-RateLimit-Reset`, `RateLimit-Limit`, `RateLimit-Remaining`, `RateLimit-Reset`, `Retry-After` (su 429).

Semantica:

- `X-RateLimit-Reset`: secondi Unix epoch (tempo assoluto di reimpostazione)
...
Le versioni precedenti di `/api/*` e `/api/cli/*` sono ancora disponibili. Vedi `DEPRECATIONS.md`.
