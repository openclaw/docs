...
- Usa endpoint pubblici di lettura come `GET /api/v1/skills`, `GET /api/v1/search` e `GET /api/v1/skills/{slug}` per le inserzioni del catalogo.
...
- Collega all'URL canonico dello skill ClawHub quando mostri le inserzioni, così gli utenti possono ispezionare il record del registro di origine.
...
- Non fare mirroring di contenuti nascosti, privati o bloccati dalla moderazione aggirando i filtri dell'API pubblica o i confini di autenticazione.
...
- `X-RateLimit-Reset`: secondi Unix epoch (ora assoluta di reimpostazione)
...
- `X-RateLimit-Remaining` / `RateLimit-Remaining`: budget rimanente esatto quando
  presente; le richieste riuscite suddivise in shard lo omettono invece di restituire un valore
...
- In alternativa usa `RateLimit-Reset` o deriva il ritardo da `X-RateLimit-Reset`.
- Aggiungi jitter ai tentativi.

## Errori

- Gli errori v1 sono testo semplice (`text/plain; charset=utf-8`), incluse le risposte `400`,
  `401`, `403`, `404`, `429` e di download bloccato.
...
- `GET /api/v1/download?slug=&version=&tag=`
  - Gli skill ospitati restituiscono byte ZIP deterministici.
  - Gli skill correnti supportati da GitHub con una scansione `clean` o `suspicious` restituiscono un
    descrittore di passaggio JSON `public-github` invece dei byte ClawHub.
- `GET /api/v1/skills/export?startDate=&endDate=&limit=&cursor=`
  - Gli skill ospitati vengono esportati come file archiviati.
  - Gli skill correnti supportati da GitHub con una scansione `clean` o `suspicious` vengono esportati
