...
x-i18n:
    generated_at: "2026-06-27T17:15:14Z"
    model: gpt-5.5
...
- Usa endpoint pubblici di lettura come `GET /api/v1/skills`, `GET /api/v1/search` e `GET /api/v1/skills/{slug}` per gli elenchi di catalogo.
- Memorizza le risposte nella cache e rispetta `429`, `Retry-After` e le intestazioni dei limiti di frequenza invece di effettuare polling aggressivo.
- Aggiungi un link all'URL canonico dello skill su ClawHub quando visualizzi gli elenchi, così gli utenti possono ispezionare il record del registro di origine.
...
- `X-RateLimit-Reset`: secondi Unix epoch (tempo assoluto di reset)
- `RateLimit-Reset`: secondi di ritardo fino al reset
- `X-RateLimit-Remaining` / `RateLimit-Remaining`: budget residuo esatto quando
  presente; le richieste riuscite frammentate lo omettono invece di restituire un valore
...
- `GET /api/v1/download?slug=&version=&tag=`
  ...
  - Gli skill correnti supportati da GitHub con una scansione `clean` o `suspicious` restituiscono un
    descrittore di passaggio JSON `public-github` invece di byte ClawHub.
