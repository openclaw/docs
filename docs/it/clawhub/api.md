...
x-i18n:
    generated_at: "2026-07-02T14:04:22Z"
    model: gpt-5.5
...
- Memorizza le risposte nella cache e rispetta `429`, `Retry-After` e gli header di limite di frequenza invece di effettuare polling aggressivo.
- Rimanda all'URL canonico della skill ClawHub quando mostri gli elenchi, così gli utenti possono ispezionare il record del registro di origine.
...
- `X-RateLimit-Remaining` / `RateLimit-Remaining`: budget residuo esatto quando
  presente; le richieste riuscite con sharding lo omettono invece di restituire un valore globale
  approssimativo
...
- `GET /api/v1/download?slug=&version=&tag=`
  ...
  - Le skill attuali basate su GitHub con una scansione `clean` o `suspicious` restituiscono un
...
- `GET /api/v1/skills/export?startDate=&endDate=&limit=&cursor=`
  ...
  - Le skill attuali basate su GitHub con una scansione `clean` o `suspicious` vengono esportate
