...
x-i18n:
    generated_at: "2026-07-02T17:38:10Z"
    model: gpt-5.5
...
Puoi creare un catalogo, una directory o una superficie di ricerca di terze parti sopra le API pubbliche in lettura di ClawHub. I metadata pubblici delle skill e i file delle skill sono pubblicati secondo le regole di licenza delle skill di ClawHub, mentre l'API stessa è soggetta a limiti di frequenza e deve essere usata in modo responsabile.

Linee guida:

- Usa endpoint pubblici in lettura come `GET /api/v1/skills`, `GET /api/v1/search` e `GET /api/v1/skills/{slug}` per gli elenchi del catalogo.
...
- Non creare mirror di contenuti nascosti, privati o bloccati dalla moderazione aggirando i filtri dell'API pubblica o i confini di autenticazione.
...
- `X-RateLimit-Reset`: secondi dall'epoch Unix (tempo di reimpostazione assoluto)
...
- `GET /api/v1/download?slug=&version=&tag=`
  ...
  - Le skill correnti basate su GitHub con una scansione `clean` o `suspicious` restituiscono un
    descrittore JSON di passaggio `public-github` invece di byte ClawHub.
