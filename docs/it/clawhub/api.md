...
read_when:
    ...
    - Aggiungere endpoint o schemi
summary: Panoramica e convenzioni dell’API REST pubblica (v1).
x-i18n:
    generated_at: "2026-07-03T13:34:27Z"
    model: gpt-5.5
...
Puoi creare un catalogo, una directory o una superficie di ricerca di terze parti sopra le API pubbliche di lettura di ClawHub. I metadati pubblici delle skill e i file delle skill sono pubblicati secondo le regole di licenza delle skill di ClawHub, mentre l'API stessa è soggetta a limiti di frequenza e deve essere consumata responsabilmente.
...
- Memorizza nella cache le risposte e rispetta `429`, `Retry-After` e le intestazioni dei limiti di frequenza invece di effettuare polling aggressivo.
- Inserisci un link all'URL canonico della skill su ClawHub quando visualizzi gli elenchi, in modo che gli utenti possano esaminare il record del registro di origine.
...
- Non duplicare contenuti nascosti, privati o bloccati dalla moderazione aggirando i filtri dell'API pubblica o i confini di autenticazione.
...
- `X-RateLimit-Reset`: secondi dell'epoca Unix (tempo assoluto di reimpostazione)
...
- `X-RateLimit-Remaining` / `RateLimit-Remaining`: budget rimanente esatto quando
  presente; le richieste riuscite con sharding lo omettono invece di restituire un valore
...
- `GET /api/v1/download?slug=&version=&tag=`
  ...
  - Le skill attuali supportate da GitHub con una scansione `clean` o `suspicious` restituiscono un
...
- `GET /api/v1/skills/export?startDate=&endDate=&limit=&cursor=`
  ...
  - Le skill attuali supportate da GitHub con una scansione `clean` o `suspicious` vengono esportate
...
- `POST /api/v1/users/reserve` riserva slug radice e segnaposto privati per pacchetti senza release per un handle proprietario.

## Legacy

Legacy `/api/*` e `/api/cli/*` ancora disponibili. Consulta `DEPRECATIONS.md`.
