...
x-i18n:
    generated_at: "2026-07-02T00:57:38Z"
    model: gpt-5.5
...
- Collega all'URL canonico dello skill ClawHub quando mostri gli elenchi, così gli utenti possono ispezionare il record del registro sorgente.
...
- Non rispecchiare contenuti nascosti, privati o bloccati dalla moderazione aggirando i filtri dell'API pubblica o i confini di autenticazione.
...
- `X-RateLimit-Reset`: secondi Unix epoch (ora di reimpostazione assoluta)
- `RateLimit-Reset`: secondi di ritardo fino alla reimpostazione
- `X-RateLimit-Remaining` / `RateLimit-Remaining`: budget restante esatto quando
...
- `GET /api/v1/download?slug=&version=&tag=`
  ...
  - Gli skill attuali basati su GitHub con una scansione `clean` o `suspicious` restituiscono un
...
- `GET /api/v1/skills/export?startDate=&endDate=&limit=&cursor=`
  ...
  - Gli skill attuali basati su GitHub con una scansione `clean` o `suspicious` vengono esportati
