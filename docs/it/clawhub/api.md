...
x-i18n:
    generated_at: "2026-07-04T03:50:41Z"
    model: gpt-5.5
...
Puoi creare un catalogo, una directory o un'interfaccia di ricerca di terze parti sopra le API di lettura pubbliche di ClawHub. I metadati pubblici delle Skills e i file delle Skills sono pubblicati secondo le regole di licenza delle Skills di ClawHub, mentre l'API stessa è soggetta a limiti di frequenza e deve essere usata in modo responsabile.
...
- Inserisci un link all'URL canonico della Skill ClawHub quando mostri gli elenchi, così gli utenti possono ispezionare il record del registro sorgente.
...
- `X-RateLimit-Reset`: secondi dell'epoca Unix (tempo di reset assoluto)
...
- `GET /api/v1/skills?limit=&cursor=&sort=`
  ...
  - Alias legacy: `nonSuspicious=true`
  - Con `nonSuspiciousOnly=true`, le pagine basate su cursor possono contenere meno di `limit` elementi; usa `nextCursor` per continuare.
...
- `GET /api/v1/download?slug=&version=&tag=`
  ...
  - Le Skills correnti supportate da GitHub con una scansione `clean` o `suspicious` restituiscono un
    descrittore di handoff JSON `public-github` invece dei byte ClawHub.
- `GET /api/v1/skills/export?startDate=&endDate=&limit=&cursor=`
  ...
  - Le Skills correnti supportate da GitHub con una scansione `clean` o `suspicious` vengono esportate
    come descrittori di handoff `public-github`.
...
- `POST /api/v1/users/reserve` riserva gli slug radice e i segnaposto privati di pacchetti senza release per l'handle di un proprietario.
