...
summary: Risoluzione dei problemi relativi all'accesso, all'installazione, alla pubblicazione, alla sincronizzazione, agli aggiornamenti e alle API di ClawHub.
x-i18n:
...
gli strumenti obbligatori devono essere dichiarati affinché utenti e scanner possano comprendere il pacchetto.
...
## La pubblicazione fallisce con un errore di proprietario GitHub o di origine
...
## `sync` segnala che non sono state trovate skill
...
- Rispetta gli header di riprova `429` e memorizza nella cache le risposte pubbliche di elenco/ricerca.
- Reindirizza gli utenti alla scheda ClawHub canonica.
- Non replicare contenuti nascosti, privati, bloccati in attesa o bloccati dalla moderazione al di fuori della
  superficie API pubblica.

Vedi [API HTTP](/it/clawhub/http-api) per i dettagli sugli endpoint.
