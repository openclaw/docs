...
x-i18n:
    generated_at: "2026-07-02T08:21:55Z"
    model: gpt-5.5
...
- Controlla firewall locale, VPN e regole del proxy se la callback non arriva mai.
...
- `RateLimit-Remaining`: il budget rimanente esatto quando l'header è presente. Su `429`, è `0`.
...
- Controlla lo slug esatto o la pagina dell'owner, se li conosci.
...
La diagnostica visibile all'owner può spiegare lo stato di scansione, gate di caricamento o moderazione.

## La pubblicazione non riesce perché mancano metadati obbligatori

Per le skill, controlla il frontmatter di `SKILL.md`. Le variabili d'ambiente e
gli strumenti richiesti devono essere dichiarati in modo che utenti e scanner possano comprendere il pacchetto.
...
## La pubblicazione non riesce con un errore di owner o sorgente GitHub
...
- Per sorgenti GitHub, usa `owner/repo`, `owner/repo@ref` o un URL GitHub completo.

## La pubblicazione non riesce perché un namespace è rivendicato o riservato

Se una pubblicazione non riesce perché l'handle dell'owner, il namespace dell'organizzazione, l'ambito del pacchetto, lo
slug della skill o il nome del pacchetto è già rivendicato o riservato, conferma prima che tu stia
pubblicando con l'owner che corrisponde al namespace. Per i pacchetti plugin,
nomi con ambito come `@example-org/example-plugin` devono essere pubblicati come
owner `example-org` corrispondente.

Se ritieni che la tua organizzazione, il tuo progetto o brand sia il legittimo owner del namespace ma
non puoi gestire l'owner ClawHub attuale, apri una
...
Indirizzalo alle radici che vuoi scansionare:
...
I file locali non corrispondono ad alcuna versione nota a ClawHub. Scegline una:
...
- Se il pacchetto è nascosto, trattenuto o bloccato, potrebbe non essere installabile finché
  l'owner non risolve il problema.
...
- Rimanda gli utenti alla scheda canonica di ClawHub.
