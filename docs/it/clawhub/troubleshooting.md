...
x-i18n:
    generated_at: "2026-07-03T09:40:19Z"
    model: gpt-5.5
...
- `RateLimit-Remaining`: il budget residuo esatto quando l'intestazione è presente. Con `429`, è `0`.
...
Per le Skills, controlla il frontmatter di `SKILL.md`. Le variabili d'ambiente obbligatorie e
gli strumenti devono essere dichiarati in modo che utenti e scanner possano comprendere il pacchetto.
...
## La pubblicazione non riesce con un errore relativo al proprietario GitHub o alla fonte

ClawHub usa l'identità GitHub e l'attribuzione della fonte per collegare i pacchetti ai rispettivi
publisher.
...
nomi con scope come `@example-org/example-plugin` devono essere pubblicati come proprietario
