...
x-i18n:
    generated_at: "2026-07-02T00:58:50Z"
    model: gpt-5.5
...
- Se usi un percorso di configurazione personalizzato, conferma che `CLAWHUB_CONFIG_PATH` punti al
  file che contiene il token corrente.
...
- `RateLimit-Remaining`: il budget residuo esatto quando l'intestazione è presente. Su `429`, è `0`.
- `RateLimit-Reset` o `X-RateLimit-Reset`: tempistica del ripristino.

Se molti utenti condividono un solo IP di uscita, i limiti IP anonimi possono essere raggiunti anche quando ogni
...
- Conferma che il rilascio sia pubblico e non bloccato da scansione o moderazione.
...
Per i plugin, controlla i metadati di compatibilità in `package.json`. Le pubblicazioni code-plugin
...
## La pubblicazione non riesce con un errore relativo a proprietario GitHub o sorgente
...
Se una pubblicazione non riesce perché handle del proprietario, namespace dell'organizzazione, scope del pacchetto, slug della skill
o nome del pacchetto è già rivendicato o riservato, conferma prima che tu stia
...
[issue di rivendicazione organizzazione / namespace](https://github.com/openclaw/clawhub/issues/new?template=org-namespace-claim.yml)
...
I file locali non corrispondono a nessuna versione conosciuta da ClawHub. Scegline una:
