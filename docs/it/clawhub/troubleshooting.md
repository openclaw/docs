...
summary: Risoluzione dei problemi relativi ad accesso, installazione, pubblicazione, aggiornamento e API di ClawHub.
x-i18n:
...
- `RateLimit-Remaining`: il tuo budget rimanente esatto quando l'intestazione è presente. Su `429`, è `0`.
- `RateLimit-Reset` o `X-RateLimit-Reset`: tempistica del ripristino.
...
- Controlla lo slug esatto o la pagina del proprietario se lo conosci.
...
La diagnostica visibile al proprietario può spiegare lo stato di scansione, blocco al caricamento o moderazione.

## La pubblicazione non riesce perché mancano metadati obbligatori

Per le Skills, controlla il frontmatter di `SKILL.md`. Le variabili d'ambiente e gli
strumenti obbligatori dovrebbero essere dichiarati in modo che utenti e scanner possano comprendere il pacchetto.

Per i plugin, controlla i metadati di compatibilità di `package.json`. Le pubblicazioni di code-plugin
...
ClawHub usa l'identità GitHub e l'attribuzione della fonte per collegare i pacchetti ai loro
publisher.

- Assicurati di aver effettuato l'accesso con l'account GitHub che possiede o può pubblicare
  il pacchetto.
- Controlla che l'URL della fonte sia pubblico o accessibile a ClawHub.
- Per le fonti GitHub, usa `owner/repo`, `owner/repo@ref` o un URL GitHub completo.

## La pubblicazione non riesce perché un namespace è rivendicato o riservato

Se una pubblicazione non riesce perché l'handle del proprietario, il namespace dell'organizzazione, l'ambito del pacchetto, lo
slug della skill o il nome del pacchetto è già rivendicato o riservato, verifica prima di stare
...
[segnalazione di rivendicazione di organizzazione / namespace](https://github.com/openclaw/clawhub/issues/new?template=org-namespace-claim.yml)
...
## `sync` dice che non sono state trovate Skills
...
Visualizza prima in anteprima se non sei sicuro di cosa verrà pubblicato:
...
- Usa una fonte ClawHub esplicita:
...
- Reindirizza gli utenti alla scheda canonica di ClawHub.
- Non eseguire il mirroring di contenuti nascosti, privati, trattenuti o bloccati dalla moderazione al di fuori della
