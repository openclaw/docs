...
title: ClawHub
x-i18n:
    generated_at: "2026-06-30T22:20:22Z"
    model: gpt-5.5
...
Installa la CLI ClawHub quando vuoi flussi autenticati dal registro, come la
...
download, stelle e riepiloghi delle scansioni di sicurezza. Le pagine pubbliche
mostrano lo stato corrente del registro, così gli utenti possono esaminare una Skill
o un Plugin prima di installarlo.

## Flussi nativi di OpenClaw

I comandi nativi di OpenClaw installano nello spazio di lavoro OpenClaw attivo e
mantengono i metadati della sorgente, così i comandi di aggiornamento successivi
possono restare su ClawHub.

Usa `clawhub:<package>` quando l'installazione di un Plugin deve essere risolta tramite ClawHub.
Le specifiche Plugin bare sicure per npm possono essere risolte tramite npm durante i cutover
di lancio, e `npm:<package>` resta solo npm quando una sorgente deve essere esplicita.

Le installazioni dei Plugin convalidano la compatibilità dichiarata di `pluginApi` e
`minGatewayVersion` prima dell'installazione dell'archivio. Quando una versione del pacchetto pubblica un
...
La CLI ClawHub serve per attività autenticate dal registro:
...
ClawHub esegue controlli automatizzati sulle Skills pubblicate e sulle release dei Plugin. Le release trattenute
dalla scansione o bloccate possono scomparire dal catalogo pubblico e dalle superfici di installazione, pur
...
nascondere o ripristinare contenuti e bloccare account abusivi. Consulta
...
[Uso accettabile](/clawhub/acceptable-usage) per i dettagli sulle policy e sull'applicazione.
...
| `CLAWHUB_REGISTRY`            | Sostituisce l'URL API del registro.                    |
| `CLAWHUB_CONFIG_PATH`         | Sostituisce la posizione in cui la CLI archivia lo stato di token/configurazione. |
