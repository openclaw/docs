...
I comandi OpenClaw nativi installano nello spazio di lavoro OpenClaw attivo e mantengono
...
Le specifiche plugin compatibili con npm senza prefisso possono risolversi tramite npm durante i cutover di lancio, e
...
prima dell’installazione dell’archivio. Quando una versione del pacchetto pubblica un
...
La CLI dispone anche di comandi di installazione/aggiornamento delle Skill per flussi diretti con il registro:
...
Quando esegui `clawhub sync` mentre hai effettuato l’accesso, la CLI invia un’istantanea minima così
ClawHub può calcolare i conteggi di installazione. Disattivala con:
...
| `CLAWHUB_CONFIG_PATH`         | Sostituisce la posizione in cui la CLI archivia stato token/configurazione. |
