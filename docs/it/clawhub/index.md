...
Le specifiche plugin semplici e compatibili con npm possono risolversi tramite npm durante le transizioni di lancio, e
...
I plugin di codice devono includere in `package.json` i metadati di compatibilità OpenClaw richiesti,
inclusi `openclaw.compat.pluginApi` e
`openclaw.build.openclawVersion`. Consulta [CLI](/it/clawhub/cli) per il riferimento completo dei comandi
e [Formato Skills](/it/clawhub/skill-format) per i metadati delle skill.

## Sicurezza e moderazione

ClawHub è aperto per impostazione predefinita: chiunque può caricare, ma la pubblicazione richiede un account GitHub
abbastanza datato da superare il controllo di caricamento. Le pagine di dettaglio pubbliche riepilogano
...
| Variabile                     | Effetto                                           |
| ----------------------------- | ------------------------------------------------- |
| `CLAWHUB_SITE`                | Sostituisce l'URL del sito usato per l'accesso via browser. |
| `CLAWHUB_REGISTRY`            | Sostituisce l'URL dell'API del registro.          |
| `CLAWHUB_CONFIG_PATH`         | Sostituisce il percorso in cui la CLI archivia token e stato di configurazione. |
