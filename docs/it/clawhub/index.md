...
ClawHub è il registro pubblico per le skill e i plugin di OpenClaw.

- Usa i comandi nativi `openclaw` per cercare, installare e aggiornare le skill e per installare plugin da ClawHub.
...
Installa la CLI di ClawHub quando ti servono flussi di lavoro autenticati dal registro, come
...
| Plugin bundle | Bundle plugin confezionati per la distribuzione OpenClaw | `clawhub package publish <source>`           |
...
Le specifiche plugin bare compatibili con npm possono risolversi tramite npm durante i passaggi di lancio, e
...
La CLI ha anche comandi di installazione/aggiornamento delle skill per flussi di lavoro diretti con il registro:
...
Questi comandi installano skill in `./skills` sotto la directory di lavoro corrente
...
abbastanza vecchio da superare il controllo di caricamento. Le pagine pubbliche di dettaglio riassumono
lo stato più recente della scansione prima dell'installazione o del download.

ClawHub esegue controlli automatici sulle skill pubblicate e sui rilasci dei plugin. I rilasci
...
Gli utenti autenticati possono segnalare skill e pacchetti. I moderatori possono esaminare le segnalazioni,
...
Quando esegui `clawhub sync` mentre hai effettuato l'accesso, la CLI invia uno snapshot minimale così
