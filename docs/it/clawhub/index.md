...
title: ClawHub
x-i18n:
    generated_at: "2026-07-01T13:03:05Z"
    model: gpt-5.5
...
- Usa la CLI separata `clawhub` per i flussi di autenticazione nel registro, pubblicazione ed eliminazione/ripristino.
...
Installa la CLI ClawHub quando vuoi flussi autenticati nel registro, come
...
La CLI include anche comandi di installazione/aggiornamento delle skill per flussi diretti con il registro:
...
abbastanza vecchio da superare il gate di caricamento. Le pagine pubbliche di dettaglio riassumono lo
stato dell’ultima scansione prima dell’installazione o del download.

ClawHub esegue controlli automatizzati su Skills pubblicate e release di plugin. Le release trattenute
...
| `CLAWHUB_CONFIG_PATH`         | Sovrascrive dove la CLI archivia stato di token/config. |
