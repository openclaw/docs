...
title: ClawHub
x-i18n:
    generated_at: "2026-07-03T00:57:55Z"
    model: gpt-5.5
...
Installa la CLI di ClawHub quando vuoi flussi di lavoro autenticati dal registro, come
...
Le specifiche Plugin semplici sicure per npm possono risolversi tramite npm durante le transizioni di lancio, e
...
La CLI include anche comandi di installazione/aggiornamento delle Skill per flussi di lavoro diretti del registro:
...
Usa `--dry-run` per creare il piano esatto di pubblicazione senza caricare, e `--json`
...
sufficientemente vecchio da superare il gate di caricamento. Le pagine di dettaglio pubbliche riepilogano lo
...
Gli utenti con accesso possono segnalare Skills e pacchetti. I moderatori possono esaminare le segnalazioni,
...
Quando esegui `clawhub install` mentre hai effettuato l'accesso, la CLI può inviare, al meglio delle possibilità,
un evento di installazione così ClawHub può calcolare conteggi aggregati delle installazioni. Disabilitalo con:
...
| Variabile                     | Effetto                                           |
| ----------------------------- | ------------------------------------------------- |
| `CLAWHUB_SITE`                | Sovrascrive l'URL del sito usato per l'accesso via browser. |
