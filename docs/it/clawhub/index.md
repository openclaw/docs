...
Specifiche plugin bare compatibili con npm possono risolversi tramite npm durante le transizioni di lancio, e
...
La CLI ClawHub è per il lavoro autenticato con il registro:
...
GitHub abbastanza vecchio da superare il gate di caricamento. Le pagine di dettaglio pubbliche riepilogano lo
stato più recente della scansione prima dell'installazione o del download.

ClawHub esegue controlli automatizzati su Skills pubblicate e release di plugin. Le release trattenute dalla scansione
...
| `CLAWHUB_CONFIG_PATH`         | Sovrascrive dove la CLI archivia stato di token/configurazione. |
