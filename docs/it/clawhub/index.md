...
title: ClawHub
x-i18n:
    generated_at: "2026-07-03T23:33:41Z"
    model: gpt-5.5
...
Installa la CLI di ClawHub quando vuoi flussi di lavoro autenticati sul registro, come
...
I comandi nativi OpenClaw installano nello workspace OpenClaw attivo e salvano
...
Le specifiche di plugin compatibili con npm senza prefisso possono risolversi tramite npm durante i passaggi di transizione del lancio, e
...
La CLI di ClawHub serve per il lavoro autenticato sul registro:
...
La CLI ha anche comandi di installazione/aggiornamento Skills per flussi di lavoro diretti sul registro:
...
e [Formato Skill](/clawhub/skill-format) per i metadati delle Skills.
...
restando visibili al rispettivo proprietario in `/dashboard`.
...
[Uso accettabile](/it/clawhub/acceptable-usage) per dettagli su criteri e applicazione.

## Telemetria e ambiente

Quando esegui `clawhub install` mentre sei autenticato, la CLI può inviare un evento di installazione
best-effort così ClawHub può calcolare conteggi aggregati di installazione. Disabilitalo con:
...
| Variabile                     | Effetto                                           |
| ----------------------------- | ------------------------------------------------- |
| `CLAWHUB_SITE`                | Sovrascrive l'URL del sito usato per il login dal browser. |
| `CLAWHUB_REGISTRY`            | Sovrascrive l'URL API del registro.              |
| `CLAWHUB_CONFIG_PATH`         | Sovrascrive dove la CLI archivia stato token/configurazione. |
