...
read_when:
    ...
    - Cercare, installare o aggiornare skill o plugin
...
| -------------- | ------------------------------------------------------------ | ------------------------------------------ |
| Skills         | Bundle di testo con versioni con `SKILL.md` più file di supporto | `openclaw skills install @openclaw/demo` |
...
I comandi nativi OpenClaw installano nello spazio di lavoro OpenClaw attivo e rendono persistenti
i metadati della fonte, così i comandi di aggiornamento successivi possono restare su ClawHub.

Usa `clawhub:<package>` quando l'installazione di un Plugin deve risolversi tramite ClawHub.
Le specifiche Plugin compatibili con npm senza prefisso possono risolversi tramite npm durante i passaggi di transizione al lancio, e
...
La CLI ClawHub è destinata al lavoro autenticato con il registro:
...
abbastanza vecchio da superare il gate di caricamento. Le pagine di dettaglio pubbliche riepilogano lo stato
più recente della scansione prima dell'installazione o del download.

ClawHub esegue controlli automatici su Skills pubblicate e release di Plugin. Le release
...
[Moderazione e sicurezza degli account](/it/clawhub/moderation) e
