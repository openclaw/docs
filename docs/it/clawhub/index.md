...
title: ClawHub
x-i18n:
    generated_at: "2026-07-02T14:04:59Z"
    model: gpt-5.5
...
- Usa la CLI `clawhub` separata per i flussi di autenticazione al registro, pubblicazione ed eliminazione/ripristino.
...
Installa la CLI ClawHub quando vuoi usare flussi autenticati al registro come
...
I comandi nativi OpenClaw installano nello spazio di lavoro OpenClaw attivo e salvano
...
Le specifiche di plugin bare compatibili con npm possono risolversi tramite npm durante le transizioni di lancio, e
...
La CLI ClawHub è destinata al lavoro autenticato al registro:
...
sufficientemente vecchio da superare il gate di caricamento. Le pagine pubbliche di dettaglio riassumono lo
...
Quando esegui `clawhub install` mentre sei connesso, la CLI può inviare un evento di installazione best-effort
