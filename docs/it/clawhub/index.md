...
Le specifiche di plugin bare compatibili con npm possono risolversi tramite npm durante i passaggi di transizione al lancio, e
...
dichiarata prima dell'installazione dell'archivio. Quando una versione di pacchetto pubblica un
...
La CLI include anche comandi di installazione/aggiornamento delle Skills per flussi diretti sul registro:
...
ClawHub esegue controlli automatizzati sulle Skills pubblicate e sui rilasci dei plugin. I rilasci
...
[Uso accettabile](/clawhub/acceptable-usage) per i dettagli su criteri e applicazione.

## Telemetria e ambiente

Quando esegui `clawhub install` mentre hai effettuato l'accesso, la CLI può inviare un evento
di installazione best-effort, così ClawHub può calcolare i conteggi aggregati delle installazioni. Disabilitalo con:
