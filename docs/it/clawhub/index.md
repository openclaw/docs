...
title: ClawHub
x-i18n:
    generated_at: "2026-07-03T02:49:22Z"
    model: gpt-5.5
...
I comandi nativi OpenClaw installano nell'area di lavoro OpenClaw attiva e mantengono
...
Le installazioni dei plugin verificano la compatibilità dichiarata di `pluginApi` e `minGatewayVersion`
prima dell'esecuzione dell'installazione dell'archivio. Quando una versione del pacchetto pubblica un artefatto
...
La CLI include anche comandi di installazione/aggiornamento delle Skills per flussi diretti con il registro:
...
GitHub abbastanza vecchio da superare il controllo di caricamento. Le pagine pubbliche di dettaglio riepilogano
...
best-effort in modo che ClawHub possa calcolare conteggi aggregati delle installazioni. Disabilitalo con:

```bash
export CLAWHUB_DISABLE_TELEMETRY=1
```

Override d'ambiente utili:
