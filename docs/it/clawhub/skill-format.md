...
L’importatore GitHub web è più rigoroso della pubblicazione/sincronizzazione locale. Scopre solo
...
| `primaryEnv`       | `string`   | La variabile d’ambiente delle credenziali principale per la tua skill.                                                                                |
...
| `homepage`         | `string`   | URL alla homepage o alla documentazione della skill.                                                                                                 |
...
- Gli scope dei pacchetti devono corrispondere esattamente all’handle publisher ClawHub. Gli handle publisher possono usare lettere minuscole, numeri, trattini, punti e trattini bassi; devono iniziare e terminare con una lettera minuscola o un numero.
- Gli slug dei pacchetti devono essere minuscoli e npm-safe, per esempio `@example.tools/demo-plugin` o `demo-plugin`.
