...
x-i18n:
    generated_at: "2026-07-03T23:35:03Z"
    model: gpt-5.5
...
| ------------------ | ---------- | -------------------------------------------------------------------------------------------------------------------------------------------- |
| `requires.env`     | `string[]` | Variabili d'ambiente obbligatorie previste dal tuo skill.                                                                                           |
...
| `primaryEnv`       | `string`   | La variabile d'ambiente principale per le credenziali del tuo skill.                                                                                                  |
| `envVars`          | `array`    | Dichiarazioni di variabili d'ambiente con `name`, `required` facoltativo e `description` facoltativa. Imposta `required: false` per le variabili d'ambiente facoltative. |
| `always`           | `boolean`  | Se `true`, lo skill è sempre attivo (non è necessaria alcuna installazione esplicita).                                                                              |
| `skillKey`         | `string`   | Sovrascrive la chiave di invocazione dello skill.                                                                                                         |
| `emoji`            | `string`   | Emoji da visualizzare per lo skill.                                                                                                                 |
...
- Gli ambiti dei pacchetti devono corrispondere esattamente all'handle dell'editore ClawHub. Gli handle degli editori possono usare lettere minuscole, numeri, trattini, punti e trattini bassi; devono iniziare e terminare con una lettera minuscola o un numero.
...
- Tutti gli skill pubblicati su ClawHub sono concessi in licenza sotto `MIT-0`.
...
- Se il tuo skill si integra con un servizio di terze parti a pagamento, documenta chiaramente il costo esterno e l'account richiesto nelle istruzioni dello skill e nelle dichiarazioni env (`requires.env` per le variabili obbligatorie, o `envVars` con `required: false` per le variabili facoltative).
