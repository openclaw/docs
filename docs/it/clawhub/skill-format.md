...
- `description` è usato come riepilogo della skill nell'interfaccia utente/ricerca.

## Metadati del frontmatter

I metadati della skill sono dichiarati nel frontmatter YAML all'inizio del tuo `SKILL.md`. Questo comunica al registry (e all'analisi di sicurezza) ciò di cui la tua skill ha bisogno per essere eseguita.
...
- I tipi di contenuto che iniziano con `text/` sono trattati come testo; in più c'è un piccolo elenco consentito (JSON/YAML/TOML/JS/TS/Markdown/SVG).
...
- Deve essere minuscolo e sicuro per gli URL: `^[a-z0-9][a-z0-9-]*$`.
...
- Se la tua skill si integra con un servizio di terze parti a pagamento, documenta chiaramente il costo esterno e l'account richiesto nelle istruzioni della skill e nelle dichiarazioni di ambiente (`requires.env` per le variabili obbligatorie, oppure `envVars` con `required: false` per le variabili facoltative).
