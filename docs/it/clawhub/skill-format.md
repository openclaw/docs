...
| `primaryEnv`       | `string`   | La principale variabile d’ambiente delle credenziali per la tua skill.                                                                        |
...
- Se la tua skill si integra con un servizio di terze parti a pagamento, documenta chiaramente il costo esterno e l’account richiesto nelle istruzioni della skill e nelle dichiarazioni d’ambiente (`requires.env` per variabili obbligatorie, oppure `envVars` con `required: false` per variabili facoltative).
