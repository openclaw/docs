...
| `envVars`          | `array`    | Dichiarazioni di variabili d'ambiente con `name`, `required` facoltativo e `description` facoltativa. Imposta `required: false` per env var opzionali. |
| `always`           | `boolean`  | Se `true`, la skill è sempre attiva (non è richiesta alcuna installazione esplicita).                                                                 |
...
- Se la tua skill si integra con un servizio di terze parti a pagamento, documenta chiaramente nelle istruzioni della skill e nelle dichiarazioni env il costo esterno e l'account richiesto (`requires.env` per le variabili obbligatorie, oppure `envVars` con `required: false` per le variabili facoltative).
