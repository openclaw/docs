...
x-i18n:
    generated_at: "2026-06-28T20:42:40Z"
    model: gpt-5.5
...
- eventuali file _basati su testo_ di supporto (vedi “File consentiti”)
...
L'importatore GitHub web è più restrittivo rispetto a publish/sync locale. Rileva solo
...
Usa `requires.env` per le variabili d'ambiente che devono essere presenti prima che la skill possa essere eseguita. Usa `envVars` quando ti servono metadati per variabile, incluse variabili facoltative con `required: false`.
