...
summary: Formato della cartella Skill, file richiesti, tipi di file consentiti, limiti.
x-i18n:
    generated_at: "2026-06-28T06:03:08Z"
    model: gpt-5.5
...
L’importatore web di GitHub è più rigoroso della pubblicazione/sincronizzazione locale. Scopre solo
...
I metadati della skill sono dichiarati nel frontmatter YAML all’inizio del tuo `SKILL.md`. Questo indica al registro (e all’analisi di sicurezza) ciò di cui la tua skill ha bisogno per essere eseguita.
...
- La allowlist delle estensioni si trova in `packages/schema/src/textFiles.ts` (`TEXT_FILE_EXTENSIONS`).
...
- Gli scope dei pacchetti devono corrispondere esattamente all’handle del publisher ClawHub. Gli handle dei publisher possono usare lettere minuscole, numeri, trattini, punti e trattini bassi; devono iniziare e finire con una lettera minuscola o un numero.
