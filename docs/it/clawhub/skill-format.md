...
- `description` viene usato come riepilogo della skill nell'UI/ricerca.
...
Usa `requires.env` per le variabili d'ambiente che devono essere presenti prima che la skill possa essere eseguita. Usa `envVars` quando hai bisogno di metadati per variabile, incluse variabili opzionali con `required: false`.
