...
L’importatore web da GitHub è più rigoroso rispetto a publish/sync locali. Rileva solo file
...
| `envVars`          | `array`    | Dichiarazioni di variabili d’ambiente con `name`, `required` facoltativo e `description` facoltativa. Imposta `required: false` per variabili env facoltative. |
...
- Chiunque può usare, modificare e redistribuire le skill pubblicate, anche commercialmente.
