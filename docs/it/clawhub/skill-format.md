...
L'importatore web di GitHub è più restrittivo rispetto alla pubblicazione/sincronizzazione locale. Rileva solo
...
- Se la tua skill si integra con un servizio di terze parti a pagamento, documenta chiaramente nelle istruzioni della skill e nelle dichiarazioni env il costo esterno e l'account richiesto (`requires.env` per variabili obbligatorie, oppure `envVars` con `required: false` per variabili facoltative).
