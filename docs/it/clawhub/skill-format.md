...
- `.clawhubignore` (pattern di esclusione per publish/sync, legacy `.clawdhubignore`)
...
Stato dell’installazione nella workdir (scritto dalla CLI):
...
| `emoji`            | `string`   | Emoji mostrata per la skill.                                                                                                                                 |
...
Publish accetta solo file “basati su testo”.
...
- I tipi di contenuto che iniziano con `text/` sono trattati come testo; in più, viene applicato un piccolo elenco di elementi consentiti (JSON/YAML/TOML/JS/TS/Markdown/SVG).
...
- Devono essere minuscoli e sicuri per URL: `^[a-z0-9][a-z0-9-]*$`.

## Versionamento + tag

- Ogni publish crea una nuova versione (semver).
