...
x-i18n:
    generated_at: "2026-06-28T07:41:53Z"
    model: gpt-5.5
...
Gli audit di sicurezza di ClawHub ti aiutano a decidere se uno skill o un Plugin è abbastanza sicuro
da installare. Mostrano cosa fa una release, quale autorità richiede e
se qualcosa merita attenzione aggiuntiva prima che possa accedere a file, account,
...
Vedi anche [Sicurezza](/it/clawhub/security), [Uso accettabile](/it/clawhub/acceptable-usage)
...
Lo stato dell'audit ti dice come reagire al risultato dell'audit:
...
| `Medium`   | La release ha un'autorità significativa, come accesso ad account o modifiche ai dati. |
...
Per esempio, uno skill di pubblicazione può mostrare `Review` con rischio `Medium`. Questo
non significa che sia malevolo. Significa che lo skill sembra allineato allo scopo, ma può
...
- il contenuto dello skill o del Plugin pertinente
- una raccomandazione

I risultati possono essere etichettati come `Info`, `Low`, `Medium`, `High` o `Critical`. I risultati con
gravità più alta contribuiscono più fortemente al livello di rischio e allo stato dell'audit.

I risultati con bassa confidenza sono nascosti dal riepilogo pubblico dell'audit, così la pagina
...
- istruzioni dello skill o metadati del Plugin
...
comandi locali, API di provider o installazioni di pacchetti. L'audit controlla se tale
...
segnali di reputazione che completano la revisione consapevole degli agenti di ClawHub. Quando sono
disponibili i conteggi dei motori dei fornitori, l'audit li riassume in linguaggio semplice, ad
esempio:
...
VirusTotal rimane telemetria. Non sostituisce l'analisi del rischio consapevole degli artefatti
di ClawHub.
...
risultati SkillSpector, telemetria VirusTotal e contesto fornito dall'editore.
...
come lente per rischi come prompt injection, uso improprio degli strumenti, esposizione delle credenziali,
