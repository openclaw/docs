...
x-i18n:
    generated_at: "2026-06-28T20:42:19Z"
    model: gpt-5.5
...
| `Warn`      | Usa maggiore cautela. ClawHub ha trovato una preoccupazione ad alto impatto o un segnale di avviso. |
...
Il livello di rischio descrive il raggio d'azione: quanto potere sembra avere la release se
...
VirusTotal è particolarmente utile per artefatti malevoli noti, riscontri dei motori e
segnali di reputazione che completano la revisione agent-aware di ClawHub. Quando i conteggi dei
motori dei vendor sono disponibili, l'audit li riassume in linguaggio semplice, ad esempio:
...
Quando ClawHub non ha telemetria dei conteggi dei vendor da riassumere, l'audit dice:

```text
No VirusTotal findings
```

VirusTotal resta telemetria. Non sostituisce l'analisi del rischio di ClawHub basata sugli artefatti.

## Analisi del rischio

L'analisi del rischio è alimentata internamente da ClawScan, il sistema di audit di sicurezza
proprio di ClawHub. Esamina ogni release come artefatto destinato agli agenti: istruzioni,
metadati, permessi dichiarati, file, segnali di capacità, segnali di scansione statica,
risultati di SkillSpector, telemetria VirusTotal e contesto fornito dall'editore.
