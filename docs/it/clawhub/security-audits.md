...
summary: Come comprendere i risultati degli audit di sicurezza di ClawHub prima di installare una skill o un Plugin.
title: Audit di sicurezza
x-i18n:
    generated_at: "2026-07-04T18:05:12Z"
    model: gpt-5.5
...
Gli audit di sicurezza di ClawHub ti aiutano a decidere se una skill o un Plugin sono abbastanza sicuri
...
| `Review`    | Leggi le risultanze prima dell'installazione. La release può comunque essere legittima. |
...
| `High`     | La release ha autorità ad alto impatto, risultanze gravi o segnali malevoli.  |
...
Le risultanze spiegano perché è stato mostrato un risultato di audit. Ogni risultanza di solito include:
...
Le risultanze possono essere etichettate come `Info`, `Low`, `Medium`, `High` o `Critical`. Le risultanze di
gravità maggiore contribuiscono più fortemente al livello di rischio e allo stato dell'audit.

Le risultanze a bassa confidenza sono nascoste dal riepilogo pubblico dell'audit, così la pagina
...
segnali di reputazione che integrano la revisione agent-aware di ClawHub. Quando sono disponibili
conteggi dei motori dei vendor, l'audit li riassume in linguaggio semplice, ad esempio:
...
Quando ClawHub non ha telemetria sui conteggi dei vendor da riassumere, l'audit dice:
...
risultanze di SkillSpector, telemetria di VirusTotal e contesto fornito dall'editore.
...
come lente per rischi quali prompt injection, uso improprio degli strumenti, esposizione di credenziali,
