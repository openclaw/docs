...
x-i18n:
    generated_at: "2026-07-04T15:21:41Z"
    model: gpt-5.5
...
| `Warn`      | Usa particolare cautela. ClawHub ha trovato una preoccupazione ad alto impatto o un segnale di avviso. |
| `Malicious` | Non installare.                                                              |
| `Pending`   | Gli audit non sono ancora terminati.                                         |
| `Error`     | L'audit non ha potuto essere completato.                                     |
...
- il contenuto della skill o del plugin rilevante
- una raccomandazione

I risultati possono essere etichettati `Info`, `Low`, `Medium`, `High` o `Critical`. I risultati con
...
ClawHub esegue audit degli artefatti di release inviati, inclusi:
...
segnali di reputazione che completano la revisione agent-aware di ClawHub. Quando sono
disponibili conteggi dei motori dei fornitori, l'audit li riassume in linguaggio semplice, ad
esempio:
...
Chiede se la capacità è dichiarata, allineata allo scopo e supportata dal caso d'uso dichiarato
della release.
