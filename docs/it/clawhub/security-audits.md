...
Gli audit di sicurezza di ClawHub ti aiutano a decidere se una competenza o un plugin è abbastanza sicuro
...
| `High`     | La release ha autorità ad alto impatto, risultati gravi o segnali dannosi.    |
...
Ad esempio, una competenza di pubblicazione può mostrare `Review` con rischio `Medium`. Questo
non significa che sia dannosa. Significa che la competenza sembra allineata allo scopo, ma può
...
- il contenuto pertinente della competenza o del plugin
- una raccomandazione

I risultati possono essere etichettati `Info`, `Low`, `Medium`, `High` o `Critical`. I risultati di
...
rimane concentrata su evidenze utili.

## Cosa controlla ClawHub

ClawHub esegue audit sugli artefatti di release inviati, inclusi:

- istruzioni della competenza o metadati del plugin
...
e contenuto effettivo sono allineati con ciò che gli utenti potrebbero ragionevolmente aspettarsi?

Un comportamento potente non è automaticamente negativo. Molti strumenti utili richiedono credenziali,
comandi locali, API dei provider o installazioni di pacchetti. L'audit verifica se quel
...
standard di settore affidabile per reputazione dei file e scansione malware, e la nostra
partnership consente a ClawHub di aggiungere intelligence di sicurezza più ampia alla revisione di competenze e plugin.
...
VirusTotal rimane telemetria. Non sostituisce l'analisi del rischio artifact-aware
di ClawHub.
