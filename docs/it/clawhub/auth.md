...
x-i18n:
    generated_at: "2026-07-01T13:03:57Z"
    model: gpt-5.5
...
Se l'accesso ti riporta a uno stato di disconnessione, il tuo account potrebbe non essere in regola.
...
2. Il browser apre la pagina di accesso ClawHub.
3. Dopo l'accesso con GitHub, ClawHub crea un token API.
4. Il browser reindirizza alla callback locale.
5. La CLI salva il token nel file di configurazione ClawHub.

Se il browser non riesce a raggiungere la callback locale a causa di firewall, VPN o
regole proxy, usa il flusso con token headless.
...
Stampa il token salvato per la configurazione CI con:
