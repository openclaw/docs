...
read_when:
    ...
    - Debug dei 401
summary: Accesso a ClawHub, token API, accesso da CLI, archiviazione dei token e revoca.
x-i18n:
    generated_at: "2026-07-01T20:23:04Z"
    model: gpt-5.5
...
tramite quell'account con accesso effettuato.

## Accesso web

Usa GitHub per accedere su [clawhub.ai](https://clawhub.ai).

Gli account eliminati, bloccati o disabilitati non possono completare il normale accesso a ClawHub.
Se l'accesso ti riporta a uno stato disconnesso, il tuo account potrebbe non essere in regola.
Se il tuo account è stato bloccato o disabilitato, usa il
...
proxy, usa il flusso di token headless.
...
Gli account eliminati, bloccati o disabilitati non possono continuare a usare i token API esistenti.
Se il tuo account è stato bloccato o disabilitato, usa il
[modulo di ricorso ClawHub](https://appeals.openclaw.ai/) se ritieni che si tratti di un
errore.
