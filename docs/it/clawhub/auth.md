...
summary: Accesso a ClawHub, token API, accesso dalla CLI, archiviazione dei token e revoca.
x-i18n:
...
Se il browser non riesce a raggiungere la callback locale a causa di regole del firewall, della VPN o
del proxy, usa il flusso di token senza interfaccia grafica.
