---
read_when:
    - Segnalazione di uno skill, plugin o pacchetto
    - Ripristino da una listing sospesa, nascosta o bloccata
    - Comprendere la moderazione, i ban o lo stato dell’account di ClawHub
sidebarTitle: Moderation and Account Safety
summary: Come funzionano segnalazioni ClawHub, sospensioni di moderazione, inserzioni nascoste, ban e reputazione dell'account.
title: Moderazione e sicurezza dell'account
x-i18n:
    generated_at: "2026-07-05T06:51:10Z"
    model: gpt-5.5
...
richiedono comunque misure di controllo. Segnalazioni, sospensioni di moderazione, inserzioni nascoste e azioni sugli account
...
[Uso accettabile](/clawhub/acceptable-usage). Per questioni di copyright o altri diritti sui contenuti,
...
ClawHub stesso. Alcuni esempi includono bug nel sito web, nell'API, nella CLI, nel registro, nell'autenticazione,
nella scansione, nella moderazione o nei confini di attendibilità di download/installazione. Non usare gli avvisi di
...
o comunque revisionato. Non includere segreti, documenti privati, documenti legali privati,
documenti di identità personale, token API o token di challenge DNS in una
issue pubblica.

## Sospensioni di moderazione

Alcuni rilievi gravi o problemi di policy possono sottoporre un editore o un'inserzione a una
sospensione di moderazione. Quando ciò accade, il contenuto interessato può essere nascosto dalla scoperta
pubblica oppure le pubblicazioni future possono iniziare come nascoste finché il problema non viene esaminato.

Le sospensioni di moderazione hanno lo scopo di proteggere gli utenti mentre ClawHub risolve casi ad alto rischio.
...
I segnali di pressione da abuso degli editori vengono controllati ogni giorno. I segnali che raggiungono
la soglia di potenziale ban di ClawHub possono attivare un avviso automatico. Se la scansione
idonea successiva dopo la scadenza dell'avviso colloca ancora l'editore nella
soglia di potenziale ban, ClawHub può applicare automaticamente l'azione sull'account.
I segnali di revisione con minore confidenza e limitati temporalmente restano esclusi dall'applicazione
automatica.
...
`--kind plugin`. Esamina l'output della scansione, correggi l'inserzione, incrementa il numero
di versione e carica la versione corretta.
