...
risultati che l'autore del Plugin può correggere nei metadati del pacchetto, nel manifesto, negli import dell'SDK
o nell'artefatto pubblicato.

Non tratta i risultati di copertura interni di Plugin Inspector. Se un report completo
contiene codici di manutenzione dello scanner senza indicazioni di correzione per l'autore, quei
codici sono destinati ai manutentori di OpenClaw anziché agli autori di Plugin.
...
- Usa [Creare Plugin](/it/plugins/building-plugins) per un esempio minimale di pacchetto
...
- Vedi [Campi di package.json che influenzano il discovery](/it/plugins/manifest#packagejson-fields-that-affect-discovery).
- Esegui di nuovo `clawhub package validate <path-to-plugin>`.
...
- Vedi [Punti di ingresso dei Plugin](/it/plugins/sdk-entrypoints) e
  [Campi di package.json che influenzano il discovery](/it/plugins/manifest#packagejson-fields-that-affect-discovery).
- Esegui di nuovo `clawhub package validate <path-to-plugin>`.
...
- Usa `openclaw.install.minHostVersion` per la versione host OpenClaw minima.
- Vedi [Campi di package.json che influenzano il discovery](/it/plugins/manifest#packagejson-fields-that-affect-discovery).
- Esegui di nuovo `clawhub package validate <path-to-plugin>`.
...
usati per compilare il pacchetto.
...
- Allinea la versione host minima con l'intervallo di versioni host che il pacchetto
...
- Se anche `openclaw.plugin.json` ha `version`, aggiornalo in modo che corrisponda o rimuovi
...
- Mantieni `name` leggibile da una persona e mantieni `id` come identificatore macchina stabile.
...
- Usa [Convenzioni di importazione](/it/plugins/building-plugins#import-conventions) e
  [Sottopercorsi dell'SDK Plugin](/it/plugins/sdk-subpaths) per trovare l'import più ristretto.
- Esegui di nuovo `clawhub package validate <path-to-plugin>`.
...
- Evita di caricare, modificare e salvare l'intero oggetto archivio di sessione.
...
Il plugin usa ancora un helper di scrittura deprecato per l'intero archivio di sessione, come
...
Il plugin usa ancora helper deprecati per percorsi file di sessione, come
...
- Non rendere persistenti né dipendere da percorsi file di trascrizione legacy.
- Vedi [API runtime](/it/plugins/sdk-runtime#agent-session-state) e
  [Sottopercorsi dell'SDK Plugin](/it/plugins/sdk-subpaths).
- Esegui di nuovo `clawhub package validate <path-to-plugin>`.
...
Il plugin usa ancora l'helper deprecato della destinazione file di trascrizione
`resolveSessionTranscriptLegacyFileTarget`.

- Usa `resolveSessionTranscriptIdentity(...)` quando il codice ha bisogno solo dell'identità
  pubblica della sessione.
- Usa `resolveSessionTranscriptTarget(...)` quando il codice ha bisogno di una destinazione
  strutturata per un'operazione sulla trascrizione.
...
Il manifest usa metadati legacy o precedenti per variabili d'ambiente dei canali senza gli attuali
