...
x-i18n:
    generated_at: "2026-07-05T05:10:21Z"
    model: gpt-5.5
...
| `package-openclaw-unsupported-metadata` | [Rimuovi metadati di pacchetto OpenClaw non supportati](/it/clawhub/plugin-validation-fixes#package-openclaw-unsupported-metadata)          |
...
| `manifest-unknown-fields`               | [Rimuovi campi del manifest non supportati](/it/clawhub/plugin-validation-fixes#manifest-unknown-fields)                                  |
| `manifest-unknown-contracts`            | [Rimuovi chiavi di contratto non supportate](/it/clawhub/plugin-validation-fixes#manifest-unknown-contracts)                                 |
...
| `unrecognized-security-manifest`        | [Rimuovi file manifest di sicurezza non supportati](/it/clawhub/plugin-validation-fixes#unrecognized-security-manifest)                   |
...
Il pacchetto ha `package.json`, ma non dichiara metadati di pacchetto
OpenClaw.
...
- Vedi [Entry point del Plugin](/it/plugins/sdk-entrypoints) e
...
- Vedi [Entry point del Plugin](/it/plugins/sdk-entrypoints).
- Riesegui `clawhub package validate <path-to-plugin>`.
...
- Usa la versione dell'API Plugin OpenClaw o il limite minimo semver su cui hai compilato e testato.
...
- Allinea la versione host minima all'intervallo di versioni host effettivamente
...
- Evita di caricare, modificare e salvare l'intero oggetto dell'archivio delle sessioni.
- Mantieni gli helper di scrittura per l'intero archivio solo mentre l'intervallo di compatibilità dichiarato
...
- Usa `resolveSessionTranscriptIdentity(...)` quando il codice richiede solo l'identità
  pubblica della sessione.
- Usa `resolveSessionTranscriptTarget(...)` quando il codice richiede una destinazione strutturata
...
- Mantieni `providerAuthEnvVars` solo come metadati di compatibilità mentre l'intervallo di OpenClaw
...
- Duplica il setup del canale guidato dall'ambiente negli attuali metadati di setup, configurazione del canale o
...
- Mantieni documentato il comportamento sensibile alla sicurezza nella documentazione pubblica del tuo pacchetto o
