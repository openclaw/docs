...
x-i18n:
    generated_at: "2026-07-04T06:37:44Z"
    model: gpt-5.5
...
ClawHub valida i pacchetti Plugin prima della pubblicazione e può anche mostrare risultati da scansioni automatizzate dei pacchetti. Questa pagina copre i risultati rivolti agli autori, ovvero risultati che l'autore del Plugin può correggere nei metadati del pacchetto, nel manifesto, negli import SDK o nell'artefatto pubblicato.

Non copre i risultati interni di copertura di Plugin Inspector. Se un report completo contiene codici di manutenzione dello scanner senza indicazioni di correzione per l'autore, questi sono destinati ai manutentori di OpenClaw anziché agli autori di Plugin.
...
| `package-plugin-api-compat-missing`     | [Dichiara la compatibilità API del Plugin](/it/clawhub/plugin-validation-fixes#package-plugin-api-compat-missing)                          |
...
| `sdk-session-transcript-file-target`    | [Sostituisci le destinazioni file di trascrizione legacy](/it/clawhub/plugin-validation-fixes#sdk-session-transcript-file-target)                   |
...
| `unrecognized-security-manifest`        | [Rimuovi i file manifesto di sicurezza non supportati](/it/clawhub/plugin-validation-fixes#unrecognized-security-manifest)                   |
...
- Consulta [campi package.json che influiscono sul discovery](/it/plugins/manifest#packagejson-fields-that-affect-discovery).
- Esegui di nuovo `clawhub package validate <path-to-plugin>`.
...
- Consulta [Punti di ingresso dei Plugin](/it/plugins/sdk-entrypoints) e [campi package.json che influiscono sul discovery](/it/plugins/manifest#packagejson-fields-that-affect-discovery).
- Esegui di nuovo `clawhub package validate <path-to-plugin>`.
...
- Usa la versione dell'API Plugin di OpenClaw o il limite inferiore semver rispetto a cui hai sviluppato e testato.
- Tienilo separato dalla versione del pacchetto. La versione del pacchetto descrive il rilascio del Plugin; `openclaw.compat.pluginApi` descrive il contratto API dell'host.
- Consulta [campi package.json che influiscono sul discovery](/it/plugins/manifest#packagejson-fields-that-affect-discovery).
- Esegui di nuovo `clawhub package validate <path-to-plugin>`.
...
- Controlla eventuali metadati di build OpenClaw nel pacchetto, come la versione di OpenClaw usata durante il rilascio.
...
La versione del pacchetto e la versione del manifesto del Plugin non concordano.

- Preferisci `package.json#version` come versione di rilascio del pacchetto.
- Se anche `openclaw.plugin.json` ha `version`, aggiornalo per farlo corrispondere o rimuovi i metadati obsoleti della versione del manifesto quando i metadati del pacchetto sono autorevoli.
...
- Mantieni entrypoint del pacchetto, compatibilità, installazione, setup e metadati del catalogo nei campi `package.json#openclaw` supportati.
- Consulta [campi package.json che influiscono sul discovery](/it/plugins/manifest#packagejson-fields-that-affect-discovery).
- Esegui di nuovo `clawhub package validate <path-to-plugin>`.
...
- Correggi metadati del pacchetto non validi, script del ciclo di vita interrotti o voci di file che fanno fallire il packaging.
...
All'artefatto impacchettato mancano metadati OpenClaw presenti nel pacchetto sorgente.
...
- Usa [Convenzioni di importazione](/it/plugins/building-plugins#import-conventions) e
  [sottopercorsi dell'SDK dei Plugin](/it/plugins/sdk-subpaths) per trovare l'import più ristretto.
- Esegui di nuovo `clawhub package validate <path-to-plugin>`.

### reserved-sdk-import

Il plugin importa un percorso dell'SDK riservato ai plugin in bundle o alla
...
- Usa [sottopercorsi dell'SDK dei Plugin](/it/plugins/sdk-subpaths) e
...
- Usa `getSessionEntry(...)` o `listSessionEntries(...)` quando leggi lo
  stato della sessione.
- Usa `patchSessionEntry(...)` o `upsertSessionEntry(...)` quando scrivi lo
  stato della sessione.
...
- Usa `patchSessionEntry(...)` o `upsertSessionEntry(...)` per rendere persistenti i metadati
  di sessione.
- Usa l'identità della trascrizione o gli helper di destinazione quando il codice sta preparando
  un'operazione di trascrizione.
- Non rendere persistenti i percorsi dei file di trascrizione legacy né dipendere da essi.
- Vedi [API runtime](/it/plugins/sdk-runtime#agent-session-state) e
  [sottopercorsi dell'SDK dei Plugin](/it/plugins/sdk-subpaths).
- Esegui di nuovo `clawhub package validate <path-to-plugin>`.
...
- Usa `publishSessionTranscriptUpdateByIdentity(...)` per le notifiche di aggiornamento
  della trascrizione.
- Preferisci l'interfaccia runtime strutturata per le trascrizioni, così OpenClaw può applicare
...
attuali di setup o configurazione che ClawHub si aspetta.
...
- Rispecchia il setup del canale guidato da variabili d'ambiente nel setup corrente, nella configurazione del canale o nei
