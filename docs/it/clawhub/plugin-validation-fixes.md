...
summary: Correggi i riscontri di validazione dei pacchetti Plugin di ClawHub prima della pubblicazione
title: Correzioni di convalida dei Plugin
x-i18n:
    generated_at: "2026-07-03T23:33:54Z"
    model: gpt-5.5
...
sono destinati ai manutentori di OpenClaw invece che agli autori di plugin.
...
- Usa [Creare plugin](/it/plugins/building-plugins) per un esempio di pacchetto
  minimale e [Manifest del Plugin](/it/plugins/manifest#manifest-versus-packagejson)
...
- Aggiungi i metadati di compatibilità e installazione quando il pacchetto sarà pubblicato o
...
- Aggiungi `openclaw.runtimeExtensions` quando il pacchetto pubblicato deve caricare JavaScript
  compilato.
- Mantieni tutti i percorsi dei punti di ingresso dentro la directory del pacchetto.
...
- Usa la versione dell'API dei plugin OpenClaw o il valore minimo semver con cui hai compilato e testato
  il pacchetto.
- Tieni questo dato separato dalla versione del pacchetto. La versione del pacchetto descrive il
  rilascio del plugin; `openclaw.compat.pluginApi` descrive il contratto dell'API dell'host.
- Vedi [campi package.json che influiscono sulla discovery](/it/plugins/manifest#packagejson-fields-that-affect-discovery).
- Riesegui `clawhub package validate <path-to-plugin>`.
...
- Pubblica una nuova versione del pacchetto dopo avere modificato metadati pubblicati.
...
- Mantieni i punti di ingresso del pacchetto e i metadati di compatibilità, installazione, configurazione e catalogo
...
- Usa [Convenzioni di importazione](/it/plugins/building-plugins#import-conventions) e
  [Sottopercorsi dell'SDK del Plugin](/it/plugins/sdk-subpaths) per trovare l'import più ristretto.
- Esegui di nuovo `clawhub package validate <path-to-plugin>`.
...
- Sostituisci gli import interni riservati dell'SDK di OpenClaw con sottopercorsi pubblici
...
- Usa `resolveSessionTranscriptTarget(...)` quando il codice ha bisogno di una destinazione
  strutturata per l'operazione di trascrizione.
