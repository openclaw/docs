...
read_when:
    ...
    - ClawHub ha rifiutato o emesso un avviso su una pubblicazione di un pacchetto Plugin
...
# Correzioni della convalida Plugin
...
- Usa [Creare Plugin](/it/plugins/building-plugins) per un esempio di pacchetto
  minimo e [Manifest Plugin](/it/plugins/manifest#manifest-versus-packagejson)
...
- Vedi [Entry point Plugin](/it/plugins/sdk-entrypoints) e
...
- Vedi [Entry point Plugin](/it/plugins/sdk-entrypoints).
- Esegui di nuovo `clawhub package validate <path-to-plugin>`.
...
- Usa la versione dell'API Plugin OpenClaw o il limite minimo semver rispetto a cui hai sviluppato e testato.
...
La versione del pacchetto e la versione del manifest Plugin non coincidono.

- Preferisci `package.json#version` come versione di release del pacchetto.
- Se anche `openclaw.plugin.json` ha `version`, aggiornalo perché corrisponda o rimuovi
...
- Correggi metadati del pacchetto non validi, script del ciclo di vita non funzionanti o voci di file che
  causano l'errore di impacchettamento.
...
- Mantieni `name` leggibile per le persone e mantieni `id` come identificatore macchina stabile.
- Vedi [Manifest Plugin](/it/plugins/manifest).
- Esegui di nuovo `clawhub package validate <path-to-plugin>`.
...
- Usa `openclaw/plugin-sdk/channel-core` per gli helper di entrypoint dei canali.
...
Il manifest usa metadati legacy o più vecchi delle variabili d'ambiente del canale senza gli attuali
...
- Replica il setup del canale basato su variabili d'ambiente nel setup attuale, nella configurazione del canale o nei
