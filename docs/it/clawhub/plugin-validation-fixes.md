...
x-i18n:
    generated_at: "2026-06-30T22:20:40Z"
    model: gpt-5.5
...
ClawHub valida i pacchetti Plugin prima della pubblicazione e può anche mostrare rilievi da
scansioni automatiche dei pacchetti. Questa pagina copre i rilievi rivolti agli autori, cioè
rilievi che l'autore del Plugin può correggere nei metadati del pacchetto, nel manifest, negli
import dell'SDK o nell'artefatto pubblicato.

Non copre i rilievi interni sulla copertura di Plugin Inspector. Se un report completo
contiene codici di manutenzione dello scanner senza indicazioni di correzione per l'autore,
questi sono destinati ai manutentori di OpenClaw e non agli autori di Plugin.
...
- Se anche `openclaw.plugin.json` ha `version`, aggiornala perché corrisponda oppure rimuovi
...
- Correggi metadati del pacchetto non validi, script del ciclo di vita non funzionanti o voci files che
  causano il fallimento del packing.
...
- Sposta il comportamento di runtime nel codice di registrazione del plugin e limita `contracts`
  ai metadati statici di proprietà delle capacità.
- Esegui di nuovo `clawhub package validate <path-to-plugin>`.
...
- Usa [Convenzioni di import](/it/plugins/building-plugins#import-conventions) e
  [Sottopercorsi dell'SDK dei plugin](/it/plugins/sdk-subpaths) per trovare l'import ristretto.
- Esegui di nuovo `clawhub package validate <path-to-plugin>`.

### reserved-sdk-import

Il plugin importa un percorso SDK riservato ai plugin inclusi nel bundle o alla
compatibilità interna.

- Sostituisci gli import SDK interni riservati di OpenClaw con sottopercorsi pubblici documentati
  `openclaw/plugin-sdk/*`.
- Se il comportamento non ha un SDK pubblico, mantieni l'helper nel tuo pacchetto oppure
...
Il plugin usa ancora l'helper deprecato per l'intero archivio di sessione
`loadSessionStore`.
...
Il plugin usa ancora un helper deprecato di scrittura dell'intero archivio di sessione, come
...
- Usa `getSessionEntry(...)` per leggere i metadati di sessione in base all'identità dell'agente e della sessione.
...
Il plugin usa ancora l'helper deprecato di destinazione del file di trascrizione
...
- Usa `appendSessionTranscriptMessageByIdentity(...)` per aggiungere contenuti alla trascrizione.
...
- Consulta [riferimento di configurazione](/it/plugins/manifest#setup-reference) e
  [Migrazione dell'SDK](/it/plugins/sdk-migration).
- Esegui di nuovo `clawhub package validate <path-to-plugin>`.

### channel-env-vars

Il manifest usa metadati legacy o precedenti per le variabili d'ambiente del canale senza gli attuali
