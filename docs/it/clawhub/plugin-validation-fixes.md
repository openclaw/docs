...
x-i18n:
    generated_at: "2026-07-02T00:58:17Z"
    model: gpt-5.5
...
Non copre i risultati interni di copertura di Plugin Inspector. Se un report completo
contiene codici di manutenzione dello scanner senza indicazioni di rimedio per l'autore, questi
sono destinati ai manutentori di OpenClaw invece che agli autori dei Plugin.
...
| `sdk-session-transcript-file-target`    | [Sostituisci le destinazioni file transcript legacy](/it/clawhub/plugin-validation-fixes#sdk-session-transcript-file-target)                   |
...
- Usa la versione dell'API dei Plugin OpenClaw o il limite inferiore semver rispetto a cui hai compilato e testato.
- Mantieni questo valore separato dalla versione del pacchetto. La versione del pacchetto descrive il
...
Il manifest Plugin nativo non include un nome visualizzato.

- Aggiungi un campo `name` non vuoto a `openclaw.plugin.json`.
- Mantieni `name` leggibile dalle persone e mantieni `id` come id macchina stabile.
- Consulta [Manifest Plugin](/it/plugins/manifest).
- Esegui di nuovo `clawhub package validate <path-to-plugin>`.

### manifest-unknown-fields

Il manifest Plugin contiene campi di primo livello che OpenClaw non supporta.
...
- Sposta invece i metadati del pacchetto o di installazione nei campi supportati
  `package.json#openclaw`, anziché nel manifest.
- Esegui di nuovo `clawhub package validate <path-to-plugin>`.
...
- Sposta il comportamento runtime nel codice di registrazione del Plugin e mantieni
  `contracts` limitato ai metadati statici di proprietà delle capacità.
- Esegui di nuovo `clawhub package validate <path-to-plugin>`.
...
- Sostituisci gli import dal root barrel con import mirati da sottopercorsi pubblici.
...
- Usa [Convenzioni di import](/it/plugins/building-plugins#import-conventions) e
  [Sottopercorsi Plugin SDK](/it/plugins/sdk-subpaths) per trovare l'import più specifico.
- Esegui di nuovo `clawhub package validate <path-to-plugin>`.
...
- Sostituisci gli import SDK interni riservati di OpenClaw con i sottopercorsi pubblici
...
- Usa [Sottopercorsi Plugin SDK](/it/plugins/sdk-subpaths) e
...
- Usa `getSessionEntry(...)` per leggere i metadati di sessione in base ad agent e identità
  della sessione.
- Usa `patchSessionEntry(...)` o `upsertSessionEntry(...)` per persistere i metadati di sessione.
- Usa gli helper di identità della trascrizione o di destinazione quando il codice prepara
...
Il Plugin usa ancora l'helper deprecato della destinazione del file di trascrizione
...
- Rispecchia i metadati delle env var del provider in `setup.providers[].envVars`.
...
Il manifest usa metadati legacy o più vecchi per le env var del canale senza i metadati
di setup o configurazione correnti previsti da ClawHub.

- Mantieni dichiarativi i metadati delle env var del canale, così OpenClaw può ispezionare lo stato
  di setup senza caricare il runtime del canale.
...
- Rimuovi `openclaw.security.json` finché OpenClaw non documenta uno schema versionato del manifest
  di sicurezza e il comportamento di ClawHub.
...
- [Punti di ingresso Plugin](/it/plugins/sdk-entrypoints)
- [Compatibilità Plugin](/it/plugins/compatibility)
