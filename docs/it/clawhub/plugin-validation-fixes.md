...
x-i18n:
    generated_at: "2026-06-30T14:04:18Z"
    model: gpt-5.5
...
scansioni automatizzate dei pacchetti. Questa pagina tratta i risultati rivolti agli autori, ovvero
risultati che l'autore del Plugin può correggere nei metadati del pacchetto, nel manifest, nelle
importazioni SDK o nell'artefatto pubblicato.

Non tratta i risultati interni della copertura del Plugin Inspector. Se un report completo
...
Dopo aver applicato qualsiasi correzione, eseguire di nuovo:
...
| `legacy-root-sdk-import`                | [Sostituisci le importazioni SDK root](/it/clawhub/plugin-validation-fixes#legacy-root-sdk-import)                                             |
...
| `channel-env-vars`                      | [Replica le variabili env del canale nei metadati attuali](/it/clawhub/plugin-validation-fixes#channel-env-vars)                                |
...
- Usa [Creazione di Plugin](/it/plugins/building-plugins) per un esempio di pacchetto
...
- Vedi [campi package.json che influiscono sul discovery](/it/plugins/manifest#packagejson-fields-that-affect-discovery).
- Esegui di nuovo `clawhub package validate <path-to-plugin>`.
...
- Aggiungi `openclaw.runtimeExtensions` quando il pacchetto pubblicato deve caricare JavaScript
  compilato.
- Mantieni tutti i percorsi degli entrypoint dentro la directory del pacchetto.
- Vedi [Punti di ingresso dei Plugin](/it/plugins/sdk-entrypoints) e
  [campi package.json che influiscono sul discovery](/it/plugins/manifest#packagejson-fields-that-affect-discovery).
- Esegui di nuovo `clawhub package validate <path-to-plugin>`.
...
- Imposta `openclaw.install.defaultChoice` quando è disponibile più di una
...
- Usa la versione dell'API Plugin OpenClaw o il floor semver rispetto a cui hai compilato e testato.
...
- Se anche `openclaw.plugin.json` contiene `version`, aggiornalo perché corrisponda o rimuovi
...
Nell'artefatto compresso mancano metadati OpenClaw presenti nel pacchetto
sorgente.
...
### contratti-manifest-sconosciuti
...
### import-sdk-root-legacy

Il plugin importa dal barrel root dell'SDK deprecato:
`openclaw/plugin-sdk`.
...
- Usa [Convenzioni di importazione](/it/plugins/building-plugins#import-conventions) e
  [Sottopercorsi dell'SDK per Plugin](/it/plugins/sdk-subpaths) per trovare l'importazione più specifica.
- Esegui di nuovo `clawhub package validate <path-to-plugin>`.

### import-sdk-riservato
...
- Usa [Sottopercorsi dell'SDK per Plugin](/it/plugins/sdk-subpaths) e
  [Migrazione dell'SDK](/it/plugins/sdk-migration) per scegliere un'importazione supportata.
- Esegui di nuovo `clawhub package validate <path-to-plugin>`.

### sdk-caricamento-archivio-sessioni
...
### sdk-scrittura-archivio-sessioni
...
### sdk-destinazione-file-trascrizione-sessione
...
### sdk-trascrizione-sessione-basso-livello
...
- Mantieni `providerAuthEnvVars` solo come metadati di compatibilità finché l'intervallo di
...
- Duplica il setup del canale guidato da variabili d'ambiente nei metadati correnti di setup, configurazione del canale o
...
### schema-manifest-sicurezza-non-disponibile

Il pacchetto include `openclaw.security.json` con un riferimento di schema che ClawHub
non riconosce come disponibile.

- Rimuovi l'URL dello schema se ha solo valore informativo.
- Usa uno schema versionato documentato solo dopo che OpenClaw ne avrà pubblicato uno.
- Esegui di nuovo `clawhub package validate <path-to-plugin>`.

### manifest-sicurezza-non-riconosciuto

Il pacchetto include un file manifest di sicurezza non supportato.
