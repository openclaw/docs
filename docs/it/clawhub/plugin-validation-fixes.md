...
x-i18n:
    generated_at: "2026-07-04T03:50:42Z"
    model: gpt-5.5
...
ClawHub valida i pacchetti Plugin prima della pubblicazione e può anche mostrare risultati provenienti da scansioni automatizzate dei pacchetti. Questa pagina copre i risultati rivolti agli autori, cioè risultati che l'autore del Plugin può correggere nei metadati del pacchetto, nel manifest, negli import dell'SDK o nell'artefatto pubblicato.

Non copre i risultati interni di copertura del Plugin Inspector. Se un report completo contiene codici di manutenzione dello scanner senza indicazioni di correzione per l'autore, questi sono destinati ai manutentori di OpenClaw anziché agli autori dei Plugin.
...
- Usa [Creare Plugin](/it/plugins/building-plugins) per un esempio di pacchetto minimale e [Manifest del Plugin](/it/plugins/manifest#manifest-versus-packagejson) per la distinzione tra pacchetto e manifest.
...
- Vedi [campi package.json che influenzano la scoperta](/it/plugins/manifest#packagejson-fields-that-affect-discovery).
- Esegui di nuovo `clawhub package validate <path-to-plugin>`.
...
- Vedi [Entry point dei Plugin](/it/plugins/sdk-entrypoints) e [campi package.json che influenzano la scoperta](/it/plugins/manifest#packagejson-fields-that-affect-discovery).
- Esegui di nuovo `clawhub package validate <path-to-plugin>`.

### package-entrypoint-missing

Il pacchetto dichiara un entrypoint OpenClaw, ma il file a cui fa riferimento manca dal pacchetto in fase di validazione.
...
- Usa la versione dell'API Plugin OpenClaw o il limite semver minimo rispetto a cui hai sviluppato e testato.
- Mantieni questo valore separato dalla versione del pacchetto. La versione del pacchetto descrive la release del Plugin; `openclaw.compat.pluginApi` descrive il contratto dell'API host.
- Vedi [campi package.json che influenzano la scoperta](/it/plugins/manifest#packagejson-fields-that-affect-discovery).
- Esegui di nuovo `clawhub package validate <path-to-plugin>`.
...
- Allinea la versione host minima con l'intervallo di versioni host che il pacchetto supporta effettivamente.
- Vedi [campi package.json che influenzano la scoperta](/it/plugins/manifest#packagejson-fields-that-affect-discovery).
- Esegui di nuovo `clawhub package validate <path-to-plugin>`.
...
- Se anche `openclaw.plugin.json` ha `version`, aggiornalo perché corrisponda o rimuovi i metadati di versione del manifest obsoleti quando i metadati del pacchetto sono autorevoli.
...
- Mantieni entrypoint del pacchetto, compatibilità, installazione, setup e metadati del catalogo nei campi supportati di `package.json#openclaw`.
- Vedi [campi package.json che influenzano la scoperta](/it/plugins/manifest#packagejson-fields-that-affect-discovery).
- Esegui di nuovo `clawhub package validate <path-to-plugin>`.
...
Il pacchetto può essere impacchettato, ma l'artefatto impacchettato non include i file di entrypoint dichiarati in `package.json#openclaw`.
...
- Usa `getSessionEntry(...)` o `listSessionEntries(...)` quando leggi lo stato della
  sessione.
- Usa `patchSessionEntry(...)` o `upsertSessionEntry(...)` quando scrivi lo stato della
  sessione.
...
Il plugin usa ancora un helper di scrittura deprecato per l'intero archivio sessione, come
`saveSessionStore` o `updateSessionStore`.

- Usa `patchSessionEntry(...)` quando aggiorni campi su una voce di sessione
  esistente.
- Usa `upsertSessionEntry(...)` quando sostituisci o crei una voce di sessione.
- Evita di caricare, modificare e salvare l'intero oggetto dell'archivio sessione.
- Mantieni gli helper di scrittura per l'intero archivio solo finché l'intervallo di compatibilità dichiarato
...
- Usa `getSessionEntry(...)` per leggere i metadati di sessione per identità di agente e
  sessione.
- Usa `patchSessionEntry(...)` o `upsertSessionEntry(...)` per persistere i metadati di
  sessione.
- Usa l'identità della trascrizione o gli helper target quando il codice prepara
...
Il plugin usa ancora l'helper deprecato per il target file di trascrizione
`resolveSessionTranscriptLegacyFileTarget`.

- Usa `resolveSessionTranscriptIdentity(...)` quando al codice serve solo l'identità pubblica
  della sessione.
- Usa `resolveSessionTranscriptTarget(...)` quando al codice serve un target strutturato per
...
- Usa `appendSessionTranscriptMessageByIdentity(...)` per gli append alla trascrizione.
- Usa `publishSessionTranscriptUpdateByIdentity(...)` per le notifiche di aggiornamento della
  trascrizione.
- Preferisci la superficie di runtime strutturata per le trascrizioni, così OpenClaw può applicare i
  corretti confini di transazione e la gestione dell'identità.
...
Il manifest usa metadati legacy o meno recenti delle variabili d'ambiente del canale senza i metadati
di setup o configurazione attuali previsti da ClawHub.
...
- Duplica il setup del canale basato su env nei metadati attuali di setup, configurazione del canale o
