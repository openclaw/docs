...
summary: Correggere i rilievi di validazione del pacchetto Plugin ClawHub prima della pubblicazione
title: Correzioni alla validazione dei Plugin
x-i18n:
...
ClawHub valida i pacchetti Plugin prima della pubblicazione e può anche mostrare risultati provenienti da scansioni automatizzate dei pacchetti. Questa pagina riguarda i risultati rivolti agli autori, cioè i risultati che l'autore del Plugin può correggere nei metadati del pacchetto, nel manifesto, negli import dell'SDK o nell'artefatto pubblicato.

Non riguarda i risultati di copertura interni di Plugin Inspector. Se un report completo contiene codici di manutenzione dello scanner senza indicazioni di rimedio per l'autore, questi sono destinati ai manutentori di OpenClaw anziché agli autori dei Plugin.
...
- Usa [Creazione di Plugin](/it/plugins/building-plugins) per un esempio minimo di pacchetto e [Manifesto del Plugin](/it/plugins/manifest#manifest-versus-packagejson) per la distinzione tra pacchetto e manifesto.
...
- Consulta [Entry point dei Plugin](/it/plugins/sdk-entrypoints) e [campi package.json che influiscono sul rilevamento](/it/plugins/manifest#packagejson-fields-that-affect-discovery).
- Riesegui `clawhub package validate <path-to-plugin>`.
...
Il pacchetto non dichiara l'intervallo dell'API del Plugin OpenClaw che supporta.

- Aggiungi `openclaw.compat.pluginApi` a `package.json`.
- Usa la versione dell'API dei Plugin OpenClaw o il limite minimo semver rispetto a cui hai compilato e testato.
...
- Se anche `openclaw.plugin.json` contiene `version`, aggiornalo perché corrisponda o rimuovi i metadati obsoleti della versione del manifesto quando i metadati del pacchetto sono autorevoli.
...
- Mantieni entrypoint del pacchetto, compatibilità, installazione, configurazione e metadati di catalogo nei campi `package.json#openclaw` supportati.
- Consulta [campi package.json che influiscono sul rilevamento](/it/plugins/manifest#packagejson-fields-that-affect-discovery).
- Riesegui `clawhub package validate <path-to-plugin>`.
...
- Correggi metadati del pacchetto non validi, script del ciclo di vita non funzionanti o voci `files` che fanno fallire l'impacchettamento.
...
- Sostituisci gli import SDK interni riservati di OpenClaw con i sottopercorsi pubblici documentati
  `openclaw/plugin-sdk/*`.
- Se il comportamento non ha un SDK pubblico, mantieni l'helper dentro il tuo pacchetto o
  richiedi un'API OpenClaw pubblica.
...
- Usa `patchSessionEntry(...)` o `upsertSessionEntry(...)` quando scrivi lo
  stato della sessione.
- Evita di caricare, modificare e salvare l'intero oggetto archivio sessione.
- Mantieni `loadSessionStore(...)` solo finché il tuo intervallo di compatibilità dichiarato
...
- Sposta il lavoro di override del modello o del fornitore in `before_model_resolve`.
- Sposta il lavoro di modifica del prompt o del contesto in `before_prompt_build`.
- Mantieni `before_agent_start` solo finché il tuo intervallo di compatibilità dichiarato
  supporta ancora versioni precedenti di OpenClaw che lo richiedono.
...
Il manifest usa ancora i metadati legacy di autenticazione del fornitore `providerAuthEnvVars`.

- Rispecchia i metadati delle variabili d'ambiente del fornitore in `setup.providers[].envVars`.
...
- Rispecchia il setup del canale guidato dalle variabili d'ambiente nel setup corrente, nella configurazione del canale o
