...
x-i18n:
    generated_at: "2026-06-28T00:11:31Z"
    model: gpt-5.5
...
ClawHub valida i pacchetti dei Plugin prima della pubblicazione e può anche mostrare risultati da scansioni automatizzate dei pacchetti. Questa pagina riguarda i risultati rivolti agli autori, cioè risultati che l'autore del Plugin può correggere nei metadati del pacchetto, nel manifest, negli import dell'SDK o nell'artefatto pubblicato.

Non riguarda i risultati di copertura interni del Plugin Inspector. Se un report completo contiene codici di manutenzione dello scanner senza indicazioni di correzione per l'autore, questi sono destinati ai manutentori di OpenClaw anziché agli autori dei Plugin.
...
Il pacchetto dichiara un entrypoint OpenClaw, ma il file referenziato manca dal pacchetto in validazione.
...
- Usa la versione dell'API dei Plugin OpenClaw o il minimo semver rispetto a cui hai sviluppato e testato.
- Tienilo separato dalla versione del pacchetto. La versione del pacchetto descrive il rilascio del Plugin; `openclaw.compat.pluginApi` descrive il contratto dell'API dell'host.
- Vedi [campi di package.json che influenzano il rilevamento](/it/plugins/manifest#packagejson-fields-that-affect-discovery).
- Riesegui `clawhub package validate <path-to-plugin>`.
...
- Allinea la versione minima dell'host con l'intervallo di versioni dell'host che il pacchetto supporta effettivamente.
- Vedi [campi di package.json che influenzano il rilevamento](/it/plugins/manifest#packagejson-fields-that-affect-discovery).
- Riesegui `clawhub package validate <path-to-plugin>`.
...
- Se anche `openclaw.plugin.json` contiene `version`, aggiornalo perché corrisponda oppure rimuovi i metadati obsoleti della versione del manifest quando i metadati del pacchetto sono autorevoli.
...
Il pacchetto non può essere pacchettizzato nell'artefatto che ClawHub ispezionerebbe o pubblicherebbe.
...
Il pacchetto può essere pacchettizzato, ma l'artefatto pacchettizzato non include i file entrypoint dichiarati in `package.json#openclaw`.
...
Nell'artefatto pacchettizzato mancano metadati OpenClaw presenti nel pacchetto sorgente.

- Esegui `npm pack --dry-run` e ispeziona i file di metadati inclusi.
- Assicurati che `package.json` includa il blocco `openclaw` nell'artefatto pacchettizzato.
...
- Mantieni `name` leggibile per gli esseri umani e `id` come id macchina stabile.
- Vedi [Manifest del Plugin](/it/plugins/manifest).
- Riesegui `clawhub package validate <path-to-plugin>`.

### manifest-unknown-fields

Il manifest del Plugin ha campi di livello superiore che OpenClaw non supporta.
...
Il manifest usa metadati legacy o più vecchi delle variabili d'ambiente del canale senza i metadati correnti
...
- Mantieni il comportamento sensibile alla sicurezza documentato nella documentazione pubblica del tuo pacchetto o nel
  README finché il contract del manifest non esiste.
- Esegui di nuovo `clawhub package validate <path-to-plugin>`.
