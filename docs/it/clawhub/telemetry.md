...
x-i18n:
    generated_at: "2026-07-01T05:46:41Z"
    model: gpt-5.5
...
ClawHub utilizza una telemetria CLI minima per calcolare conteggi aggregati delle installazioni.
...
A ogni `clawhub install` segnalato, la CLI invia un evento di installazione al meglio possibile.
...
- `installsAllTime`: utenti univoci che hanno segnalato almeno un’installazione CLI per la skill.
...
Con questa impostazione, la CLI non invierà telemetria delle installazioni.
