...
ClawHub usa una **telemetria minima** per calcolare i **conteggi di installazione** (ciò che è effettivamente in uso) e per abilitare un ordinamento/filtro migliore.
...
- `skillId` (risolto tramite slug; vengono tracciate solo le skills esistenti nel registro).
...
- Se la skill viene rimossa da tutte le tue radici, non conta più per `installsCurrent`.
- `installsAllTime` non diminuisce mai, a meno che tu non elimini la telemetria (vedi sotto).

### Obsolescenza (120 giorni)

Le radici che non segnalano telemetria per **120 giorni** vengono contrassegnate come obsolete e le loro installazioni smettono di contare per `installsCurrent`.
...
Eliminando il tuo account vengono eliminati anche i tuoi dati di telemetria.
