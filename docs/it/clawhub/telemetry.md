---
read_when:
    - Lavoro sui controlli di telemetria e privacy
...
x-i18n:
    generated_at: "2026-05-11T22:20:21Z"
    model: gpt-5.5
    provider: openai
...
ClawHub usa una **telemetria minima** per calcolare i **conteggi di installazione** (ciò che è effettivamente in uso) e per alimentare ordinamento e filtri migliori.
...
- `skillId` (risolta tramite slug; vengono tracciate solo le skill presenti nel registro).
...
- `installsCurrent`: utenti unici che attualmente hanno l'abilità installata in almeno una radice attiva.
- `installsAllTime`: utenti unici che hanno mai segnalato l'abilità come installata.

### Radici multiple

Se sincronizzi da più cartelle, trattiamo ogni radice di scansione in modo indipendente. Un'abilità è “attualmente installata” se esiste in **qualsiasi** radice attiva.

### Rilevamento della disinstallazione

Poiché `sync` segnala l'intero insieme per radice:

- Se un'abilità scompare da una radice alla sincronizzazione successiva, la contrassegniamo come rimossa per quella radice.
- Se l'abilità viene rimossa da tutte le tue radici, non viene più conteggiata in `installsCurrent`.
...
Questa valutazione avviene in modo differito (al report di telemetria successivo) per evitare processi in background.

## Trasparenza + controlli utente

ClawHub fornisce una scheda privata “Installate” nel tuo profilo:

- Mostra le radici esatte + le abilità installate che archiviamo.
