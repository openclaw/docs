...
x-i18n:
    generated_at: "2026-05-12T15:43:07Z"
    model: gpt-5.5
    provider: openai
...
ClawHub usa una **telemetria minima** per calcolare i **conteggi delle installazioni** (ciò che è effettivamente in uso) e per supportare ordinamenti/filtri migliori.
...
- `lastVersion` (nel limite del possibile; attualmente la versione corrispondente nel registro, se nota).
...
- Nessun tracciamento per skill che non sono caricate nel registro (gli slug sconosciuti vengono ignorati).
...
Se esegui la sincronizzazione da più cartelle, trattiamo ogni radice di scansione in modo indipendente. Una skill è “attualmente installata” se esiste in **qualsiasi** radice attiva.
...
Questo viene valutato in modo differito (alla segnalazione di telemetria successiva) per evitare job in background.

## Trasparenza + controlli utente

ClawHub offre una scheda privata “Installate” sul tuo profilo:

- Mostra le radici esatte + le skill installate che archiviamo.
...
Tutti gli altri vedono solo i **contatori aggregati delle installazioni**; nessun altro può vedere le tue radici/cartelle.

Eliminando il tuo account elimini anche i dati di telemetria.
