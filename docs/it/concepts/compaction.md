...
title: Compaction
x-i18n:
    generated_at: "2026-04-24T08:35:56Z"
    model: gpt-5.4
    provider: openai
...
Ogni modello ha una finestra di contesto, cioè il numero massimo di token che può elaborare.
...
Quando OpenClaw divide la cronologia in blocchi di Compaction, mantiene le chiamate
agli strumenti dell'assistente abbinate alle rispettive voci `toolResult`. Se un punto
di divisione cade all'interno di un blocco di strumenti, OpenClaw sposta il confine così la coppia rimane unita e
viene preservata la coda corrente non riassunta.

La cronologia completa della conversazione rimane su disco. La Compaction cambia solo ciò che il
modello vede al turno successivo.

## Compaction automatica

La Compaction automatica è attiva per impostazione predefinita. Viene eseguita quando la sessione si avvicina al limite
di contesto, oppure quando il modello restituisce un errore di overflow del contesto (in tal caso
OpenClaw esegue la Compaction e riprova). Le tipiche firme di overflow includono
...
nei file di [memoria](/it/concepts/memory). Questo previene la perdita di contesto.
</Info>

Usa l'impostazione `agents.defaults.compaction` nel tuo `openclaw.json` per configurare il comportamento della Compaction (modalità, token target, ecc.).
La Compaction preserva per impostazione predefinita gli identificatori opachi (`identifierPolicy: "strict"`). Puoi sostituire questo comportamento con `identifierPolicy: "off"` oppure fornire testo personalizzato con `identifierPolicy: "custom"` e `identifierInstructions`.

Puoi facoltativamente specificare un modello diverso per il riepilogo della Compaction tramite `agents.defaults.compaction.model`. Questo è utile quando il tuo modello principale è locale o piccolo e vuoi che i riepiloghi di Compaction siano prodotti da un modello più capace. L'override accetta qualsiasi stringa `provider/model-id`:
...
Funziona anche con modelli locali, per esempio un secondo modello Ollama dedicato al riepilogo o uno specialista di Compaction fine-tuned:
...
I Plugin possono registrare un provider di Compaction personalizzato tramite `registerCompactionProvider()` sull'API del plugin. Quando un provider è registrato e configurato, OpenClaw delega il riepilogo a esso invece che alla pipeline LLM integrata.

Per usare un provider registrato, imposta l'id del provider nella configurazione:
...
Impostare `provider` forza automaticamente `mode: "safeguard"`. I provider ricevono le stesse istruzioni di Compaction e la stessa policy di preservazione degli identificatori del percorso integrato, e OpenClaw continua a preservare il contesto di suffisso dei turni recenti e dei turni divisi dopo l'output del provider. Se il provider fallisce o restituisce un risultato vuoto, OpenClaw usa come fallback il riepilogo LLM integrato.
...
Prima della Compaction, OpenClaw può eseguire un turno di **flush silenzioso della memoria** per archiviare
note durevoli su disco. Vedi [Memoria](/it/concepts/memory) per dettagli e configurazione.
...
Per impostazione predefinita, la Compaction viene eseguita silenziosamente. Per mostrare brevi notifiche quando la Compaction
inizia e quando termina, abilita `notifyUser`:
...
Quando abilitato, l'utente vede brevi messaggi di stato attorno a ogni esecuzione della Compaction
(per esempio, "Compacting context..." e "Compaction complete").
...
Il [Session pruning](/it/concepts/session-pruning) è un complemento più leggero che
riduce l'output degli strumenti senza riassumere.

## Risoluzione dei problemi

**La Compaction avviene troppo spesso?** La finestra di contesto del modello potrebbe essere piccola, oppure gli output
...
guidare il riepilogo, oppure abilita il [flush della memoria](/it/concepts/memory) in modo che le note
sopravvivano.
...
contesto personalizzati, Compaction lato server OpenAI), vedi il
