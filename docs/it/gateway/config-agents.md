...
read_when:
    ...
    - Regolazione della sessione, della consegna dei messaggi e del comportamento della modalità conversazione
summary: Impostazioni predefinite degli agenti, routing multi-agente, sessione, messaggi e configurazione di talk
...
Salta la creazione dei file opzionali selezionati della workspace continuando a scrivere i file di bootstrap richiesti. Valori validi: `SOUL.md`, `USER.md`, `HEARTBEAT.md` e `IDENTITY.md`.
...
- `"never"`: disabilita il bootstrap della workspace e l'iniezione di file di contesto a ogni turno. Usalo solo per agenti che gestiscono completamente il ciclo di vita del proprio prompt (motori di contesto personalizzati, runtime nativi che costruiscono il proprio contesto o workflow specializzati senza bootstrap). Anche i turni Heartbeat e di recupero da Compaction saltano l'iniezione.
...
### Mappa di ownership del budget di contesto
...
- `memoryGetDefaultLines`: finestra di righe predefinita di `memory_get` quando `lines` viene
  omesso.
- `toolResultMaxChars`: limite dei risultati live degli strumenti usato per i risultati persistiti e
  il recupero dall'overflow.
...
Valori più bassi di solito riducono l'uso di token per visione e la dimensione del payload della richiesta nelle esecuzioni con molti screenshot.
...
- `reasoningDefault`: visibilità del ragionamento predefinita per gli agenti. Valori: `"off"`, `"on"`, `"stream"`. `agents.list[].reasoningDefault` per agente sovrascrive questo valore predefinito. I valori predefiniti di ragionamento configurati vengono applicati solo per proprietari, mittenti autorizzati o contesti Gateway di amministratore-operatore quando non è impostato alcun override del ragionamento per messaggio o sessione.
- `elevatedDefault`: livello predefinito di output elevato per gli agenti. Valori: `"off"`, `"on"`, `"ask"`, `"full"`. Predefinito: `"on"`.
- `model.primary`: formato `provider/model` (ad es. `openai/gpt-5.5` per accesso tramite chiave API o `openai-codex/gpt-5.5` per Codex OAuth). Se ometti il provider, OpenClaw prova prima un alias, poi una corrispondenza univoca di provider configurato per quell’esatto ID modello, e solo dopo usa come fallback il provider predefinito configurato (comportamento di compatibilità deprecato, quindi preferisci `provider/model` esplicito). Se quel provider non espone più il modello predefinito configurato, OpenClaw usa come fallback il primo provider/modello configurato invece di mostrare un valore predefinito obsoleto di un provider rimosso.
...
- `params.extra_body`/`params.extraBody`: JSON avanzato pass-through unito nei corpi richiesta `api: "openai-completions"` per proxy compatibili con OpenAI. Se entra in conflitto con chiavi richiesta generate, il corpo extra ha la precedenza; le route di completamento non native rimuovono comunque `store` solo OpenAI in seguito.
- `params.chat_template_kwargs`: argomenti di modello chat compatibili con vLLM/OpenAI uniti nei corpi richiesta di livello superiore `api: "openai-completions"`. Per `vllm/nemotron-3-*` con thinking disattivato, il Plugin vLLM incluso invia automaticamente `enable_thinking: false` e `force_nonempty_content: true`; `chat_template_kwargs` espliciti sovrascrivono i valori predefiniti generati, e `extra_body.chat_template_kwargs` ha comunque la precedenza finale. Per i controlli thinking di vLLM Qwen, imposta `params.qwenThinkingFormat` su `"chat-template"` o `"top-level"` in quella voce modello.
- `compat.supportedReasoningEfforts`: elenco per modello degli sforzi di ragionamento compatibili con OpenAI. Includi `"xhigh"` per endpoint personalizzati che lo accettano davvero; OpenClaw espone quindi `/think xhigh` nei menu comando, nelle righe di sessione Gateway, nella validazione delle patch di sessione, nella validazione CLI degli agenti e nella validazione `llm-task` per quel provider/modello configurato. Usa `compat.reasoningEffortMap` quando il backend richiede un valore specifico del provider per un livello canonico.
- `params.preserveThinking`: opt-in solo Z.AI per il thinking preservato. Quando abilitato e il thinking è attivo, OpenClaw invia `thinking.clear_thinking: false` e riproduce il `reasoning_content` precedente; vedi [Thinking Z.AI e thinking preservato](/it/providers/zai#thinking-and-preserved-thinking).
- `agentRuntime`: criterio predefinito di runtime agente a basso livello. Un ID omesso usa come valore predefinito OpenClaw Pi. Usa `id: "pi"` per forzare l’harness PI integrato, `id: "auto"` per consentire agli harness dei Plugin registrati di rivendicare modelli supportati e usare PI quando nessuno corrisponde, un ID harness registrato come `id: "codex"` per richiedere quell’harness, oppure un alias di backend CLI supportato come `id: "claude-cli"`. I runtime Plugin espliciti falliscono in modo chiuso quando l’harness non è disponibile o fallisce. Mantieni i riferimenti modello canonici come `provider/model`; seleziona Codex, Claude CLI, Gemini CLI e altri backend di esecuzione tramite la configurazione runtime invece dei prefissi provider runtime legacy. Vedi [Runtime agenti](/it/concepts/agent-runtimes) per come questo differisce dalla selezione provider/modello.
- Gli autori di configurazione che modificano questi campi (per esempio `/models set`, `/models set-image` e i comandi di aggiunta/rimozione fallback) salvano la forma oggetto canonica e preservano gli elenchi fallback esistenti quando possibile.
...
`agentRuntime` controlla quale esecutore a basso livello esegue i turni agente. La maggior parte
...
vedi [Runtime agenti](/it/concepts/agent-runtimes).
...
- `id: "auto"` consente agli harness dei Plugin registrati di rivendicare i turni supportati e usa PI quando nessun harness corrisponde. Un runtime Plugin esplicito come `id: "codex"` richiede quell’harness e fallisce in modo chiuso se non è disponibile o fallisce.
- Override di ambiente: `OPENCLAW_AGENT_RUNTIME=<id|auto|pi>` sovrascrive `id` per quel processo.
...
- Le chiavi dei criteri runtime precedenti vengono riscritte in `agentRuntime` da `openclaw doctor --fix`.
- La scelta dell’harness viene fissata per ID sessione dopo la prima esecuzione incorporata. Le modifiche a configurazione/env interessano sessioni nuove o reimpostate, non una trascrizione esistente. Le sessioni legacy con cronologia trascrizione ma senza pin registrato vengono trattate come fissate a PI. `/status` riporta il runtime effettivo, per esempio `Runtime: OpenClaw Pi Default` o `Runtime: OpenAI Codex`.
- Questo controlla solo l’esecuzione testuale dei turni agente. Generazione media, visione, PDF, musica, video e TTS usano comunque le rispettive impostazioni provider/modello.
...
Backend CLI facoltativi per esecuzioni di fallback solo testuali (senza chiamate agli strumenti). Utili come backup quando i provider API non funzionano.
...
- `directPolicy`: criterio di recapito diretto/DM. `allow` (predefinito) consente il recapito a target diretto. `block` sopprime il recapito a target diretto ed emette `reason=dm-blocked`.
- `lightContext`: quando è true, le esecuzioni Heartbeat usano un contesto di bootstrap leggero e conservano solo `HEARTBEAT.md` dai file di bootstrap dell'area di lavoro.
- `isolatedSession`: quando è true, ogni Heartbeat viene eseguito in una nuova sessione senza cronologia di conversazione precedente. Stesso schema di isolamento del cron `sessionTarget: "isolated"`. Riduce il costo in token per Heartbeat da circa 100K a circa 2-5K token.
- `skipWhenBusy`: quando è true, le esecuzioni Heartbeat vengono rinviate in caso di corsie occupate aggiuntive: lavoro di subagenti o comandi annidati. Le corsie Cron rinviano sempre gli Heartbeat, anche senza questo flag.
...
- `provider`: ID di un provider Plugin di compaction registrato. Quando è impostato, viene chiamato `summarize()` del provider invece del riepilogo LLM integrato. In caso di errore, esegue il fallback all'integrato. Impostare un provider forza `mode: "safeguard"`. Vedi [Compaction](/it/concepts/compaction).
...
- `identifierPolicy`: `strict` (predefinito), `off` o `custom`. `strict` antepone linee guida integrate per la conservazione degli identificatori opachi durante il riepilogo della Compaction.
...
- `midTurnPrecheck`: controllo facoltativo Pi della pressione del ciclo strumenti. Quando `enabled: true`, OpenClaw controlla la pressione del contesto dopo l'aggiunta dei risultati degli strumenti e prima della chiamata successiva al modello. Se il contesto non rientra più, interrompe il tentativo corrente prima di inviare il prompt e riusa il percorso di recupero precheck esistente per troncare i risultati degli strumenti o eseguire Compaction e riprovare. Funziona con entrambe le modalità di Compaction `default` e `safeguard`. Predefinito: disabilitato.
- `postCompactionSections`: nomi facoltativi di sezioni H2/H3 di AGENTS.md da reiniettare dopo la Compaction. Il valore predefinito è `["Session Startup", "Red Lines"]`; imposta `[]` per disabilitare la reiniezione. Quando non è impostato o è impostato esplicitamente su quella coppia predefinita, anche le intestazioni precedenti `Every Session`/`Safety` sono accettate come fallback legacy.
...
- `maxActiveTranscriptBytes`: soglia di byte facoltativa (`number` o stringhe come `"20mb"`) che attiva la Compaction locale normale prima di un'esecuzione quando il JSONL attivo supera la soglia. Richiede `truncateAfterCompaction` affinché una Compaction riuscita possa ruotare a una trascrizione successiva più piccola. Disabilitato quando non è impostato o è `0`.
...
Isolamento sandbox facoltativo per l'agente incorporato. Vedi [Sandboxing](/it/gateway/sandboxing) per la guida completa.
...
- `identityData` / `certificateData` / `knownHostsData`: contenuti inline o SecretRefs che OpenClaw materializza in file temporanei durante il runtime
...
- `ro`: spazio di lavoro sandbox in `/workspace`, spazio di lavoro dell'agente montato in sola lettura su `/agent`
- `rw`: spazio di lavoro dell'agente montato in lettura/scrittura su `/workspace`
...
**`setupCommand`** viene eseguito una volta dopo la creazione del container (tramite `sh -lc`). Richiede accesso di rete in uscita, root scrivibile, utente root.

**I container usano `network: "none"` per impostazione predefinita**: impostalo su `"bridge"` (o su una rete bridge personalizzata) se l'agente necessita di accesso in uscita.
...
- `default`: quando ne sono impostati più di uno, vince il primo (avviso registrato). Se non ne è impostato nessuno, la prima voce dell'elenco è quella predefinita.
...
- `params`: parametri di stream per agente uniti alla voce del modello selezionata in `agents.defaults.models`. Usalo per override specifici dell'agente come `cacheRetention`, `temperature` o `maxTokens` senza duplicare l'intero catalogo dei modelli.
- `tts`: override text-to-speech facoltativi per agente. Il blocco esegue un'unione profonda sopra `messages.tts`, quindi mantieni credenziali del provider condivise e policy di fallback in `messages.tts` e imposta qui solo valori specifici della persona, come provider, voce, modello, stile o modalità automatica.
- `skills`: allowlist facoltativa di skill per agente. Se omessa, l'agente eredita `agents.defaults.skills` quando è impostato; un elenco esplicito sostituisce i valori predefiniti invece di unirsi a essi, e `[]` significa nessuna skill.
...
- `reasoningDefault`: visibilità del ragionamento predefinita facoltativa per agente (`on | off | stream`). Sovrascrive `agents.defaults.reasoningDefault` per questo agente quando non è impostato alcun override di ragionamento per messaggio o sessione.
- `fastModeDefault`: valore predefinito facoltativo per agente per la modalità veloce (`true | false`). Si applica quando non è impostato alcun override di modalità veloce per messaggio o sessione.
...
- `subagents.allowAgents`: allowlist di ID agente per target espliciti `sessions_spawn.agentId` (`["*"]` = qualsiasi; predefinito: solo lo stesso agente). Includi l'ID del richiedente quando devono essere consentite chiamate `agentId` verso sé stesso.
...
Esegui più agenti isolati all'interno di un unico Gateway. Consulta [Multi-agente](/it/concepts/multi-agent).
...
Consulta [Sandbox multi-agente e strumenti](/it/tools/multi-agent-sandbox-tools) per i dettagli sulla precedenza.
...
- **`identityLinks`**: mappa gli ID canonici ai peer con prefisso del provider per condividere sessioni tra canali. I comandi di dock come `/dock_discord` usano la stessa mappa per spostare il percorso di risposta della sessione attiva verso un altro peer di canale collegato; vedi [Docking dei canali](/it/concepts/channel-docking).
- **`reset`**: criterio di reimpostazione principale. `daily` reimposta all'ora locale `atHour`; `idle` reimposta dopo `idleMinutes`. Quando sono configurati entrambi, vince quello che scade per primo. La freschezza della reimpostazione giornaliera usa `sessionStartedAt` della riga di sessione; la freschezza della reimpostazione per inattività usa `lastInteractionAt`. Scritture in background/eventi di sistema come Heartbeat, riattivazioni Cron, notifiche exec e contabilità del Gateway possono aggiornare `updatedAt`, ma non mantengono fresche le sessioni giornaliere/per inattività.
- **`resetByType`**: override per tipo (`direct`, `group`, `thread`). Il valore legacy `dm` è accettato come alias di `direct`.
- **`mainKey`**: campo legacy. Il runtime usa sempre `"main"` per il bucket principale di chat diretta.
...
- **`maintenance`**: controlli di pulizia e conservazione dell'archivio sessioni.
  ...
  - `maxEntries`: numero massimo di voci in `sessions.json` (predefinito `500`). Il runtime scrive la pulizia in batch con un piccolo buffer di soglia alta per limiti di dimensione produzione; `openclaw sessions cleanup --enforce` applica subito il limite.
  ...
  - `highWaterBytes`: target opzionale dopo la pulizia per budget. Il valore predefinito è `80%` di `maxDiskBytes`.
- **`threadBindings`**: impostazioni predefinite globali per le funzionalità di sessione legate ai thread.
  ...
  - `spawnSessions`: gate predefinito per creare sessioni di lavoro legate ai thread da `sessions_spawn` e spawn di thread ACP. Il valore predefinito è `true` quando i binding di thread sono abilitati; provider/account possono fare override.
  - `defaultSpawnContext`: contesto subagent nativo predefinito per spawn legati ai thread (`"fork"` o `"isolated"`). Il valore predefinito è `"fork"`.
...
- Per impostazione predefinita usa `identity.emoji` dell'agente attivo, altrimenti `"👀"`. Imposta `""` per disabilitarla.
...
Raggruppa i messaggi rapidi solo testuali dello stesso mittente in un unico turno dell'agente. Media/allegati svuotano subito il batch. I comandi di controllo ignorano il debounce.
...
- `auto` controlla la modalità automatica TTS predefinita: `off`, `always`, `inbound` o `tagged`. `/tts on|off` può sovrascrivere le preferenze locali e `/tts status` mostra lo stato effettivo.
- `summaryModel` sovrascrive `agents.defaults.model.primary` per il riepilogo automatico.
- `modelOverrides` è abilitato per impostazione predefinita; `modelOverrides.allowProvider` ha valore predefinito `false` (adesione esplicita).
...
- Le chiavi Talk piatte legacy (`talk.voiceId`, `talk.voiceAliases`, `talk.modelId`, `talk.outputFormat`, `talk.apiKey`) esistono solo per compatibilità e vengono migrate automaticamente in `talk.providers.<provider>`.
...
- La riproduzione MLX su macOS passa attraverso l'helper incluso `openclaw-mlx-tts` quando presente, oppure tramite un eseguibile su `PATH`; `OPENCLAW_MLX_TTS_BIN` sovrascrive il percorso dell'helper per lo sviluppo.
