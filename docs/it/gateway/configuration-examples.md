...
```json5
{
    ...
    media: {
      audio: {
        ...
        models: [
          ...
          // Fallback CLI facoltativo (binario Whisper):
  ...
  session: {
    scope: "per-sender",
    dmScope: "per-channel-peer", // consigliato per inbox multiutente
    reset: {
      mode: "daily",
      atHour: 4,
      idleMinutes: 60,
  ...
  agents: {
    defaults: {
      ...
      skills: ["github", "weather"], // ereditate dagli agenti che omettono list[].skills
      thinkingDefault: "low",
      verboseDefault: "off",
      elevatedDefault: "on",
...
```json5
{
  ...
  channels: {
    // Esempio: inbox WhatsApp multiutente
    whatsapp: {
      dmPolicy: "allowlist",
      allowFrom: ["+15555550123", "+15555550124"],
    },

    // Esempio: inbox Discord multiutente
    discord: {
      enabled: true,
      token: "YOUR_DISCORD_BOT_TOKEN",
