...
Chiavi di configurazione per canale sotto `channels.*`. Copre l'accesso DM e di gruppo,
...
| `allowlist`         | Solo i mittenti in `allowFrom` (o nell'archivio degli autorizzati tramite pairing) |
...
| `open`                | Ignora le allowlist di gruppo (il gating delle menzioni resta applicato) |
...
Se un blocco provider manca del tutto (`channels.<provider>` assente), la policy di gruppo runtime ripiega su `allowlist` (fail-closed) con un avviso all'avvio.
</Note>

### Override dei modelli per canale

Usa `channels.modelByChannel` per associare ID di canale specifici a un modello. I valori accettano `provider/model` o alias di modello configurati. La mappatura del canale si applica quando una sessione non ha già un override del modello (per esempio, impostato tramite `/model`).
...
- `channels.defaults.groupPolicy`: policy di gruppo di fallback quando `groupPolicy` a livello di provider non è impostata.
- `channels.defaults.contextVisibility`: modalità predefinita di visibilità del contesto supplementare per tutti i canali. Valori: `all` (predefinito, include tutto il contesto di citazioni/thread/cronologia), `allowlist` (include solo il contesto proveniente da mittenti in allowlist), `allowlist_quote` (come allowlist ma mantiene il contesto esplicito di citazione/risposta). Override per canale: `channels.<channel>.contextVisibility`.
...
- `channels.defaults.heartbeat.useIndicator`: renderizza output Heartbeat compatto in stile indicatore.
...
- `channels.whatsapp.defaultAccount` facoltativo sostituisce quella selezione predefinita dell'account di fallback quando corrisponde a un ID account configurato.
...
- Nelle configurazioni multi-account (2+ ID account), imposta un valore predefinito esplicito (`channels.telegram.defaultAccount` o `channels.telegram.accounts.default`) per evitare routing di fallback; `openclaw doctor` avvisa quando questo manca o non è valido.
...
- Le voci di primo livello `bindings[]` con `type: "acp"` configurano associazioni ACP persistenti per gli argomenti dei forum (usa il formato canonico `chatId:topic:topicId` in `match.peer.id`). La semantica dei campi è condivisa in [Agenti ACP](/it/tools/acp-agents#persistent-channel-bindings).
...
- Le chiamate dirette in uscita che forniscono un Discord `token` esplicito usano quel token per la chiamata; le impostazioni di tentativo/policy dell'account provengono comunque dall'account selezionato nello snapshot del runtime attivo.
...
- Usa `user:<id>` (DM) o `channel:<id>` (canale guild) per i destinatari di consegna; gli ID numerici nudi vengono rifiutati.
- Gli slug delle guild sono in minuscolo con gli spazi sostituiti da `-`; le chiavi dei canali usano il nome con slug (senza `#`). Preferisci gli ID delle guild.
- I messaggi creati dai bot sono ignorati per impostazione predefinita. `allowBots: true` li abilita; usa `allowBots: "mentions"` per accettare solo messaggi di bot che menzionano il bot (i propri messaggi restano filtrati).
- `channels.discord.guilds.<id>.ignoreOtherMentions` (e gli override dei canali) elimina i messaggi che menzionano un altro utente o ruolo ma non il bot (esclusi @everyone/@here).
- `channels.discord.mentionAliases` mappa il testo stabile in uscita `@handle` agli ID utente Discord prima dell'invio, così i compagni di squadra noti possono essere menzionati in modo deterministico anche quando la cache transitoria della directory è vuota. Gli override per account stanno sotto `channels.discord.accounts.<accountId>.mentionAliases`.
...
- `channels.discord.threadBindings` controlla il routing Discord vincolato ai thread:
  ...
  - `spawnSessions`: interruttore per `sessions_spawn({ thread: true })` e creazione/binding automatici di thread per thread-spawn ACP (predefinito: `true`)
  - `defaultSpawnContext`: contesto subagent nativo per spawn vincolati ai thread (`"fork"` per impostazione predefinita)
- Le voci `bindings[]` di livello superiore con `type: "acp"` configurano binding ACP persistenti per canali e thread (usa l'id del canale/thread in `match.peer.id`). La semantica dei campi è condivisa in [Agenti ACP](/it/tools/acp-agents#persistent-channel-bindings).
- `channels.discord.ui.components.accentColor` imposta il colore di accento per i contenitori dei componenti Discord v2.
- `channels.discord.voice` abilita conversazioni nei canali vocali Discord e override opzionali per auto-join + LLM + TTS. Le configurazioni Discord solo testo lasciano la voce disattivata per impostazione predefinita; imposta `channels.discord.voice.enabled=true` per aderire.
...
- La riproduzione vocale Discord non viene interrotta dall'evento speaking-start di un altro utente. Per evitare loop di feedback, OpenClaw ignora la nuova cattura vocale mentre il TTS è in riproduzione.
- OpenClaw tenta inoltre il ripristino della ricezione vocale uscendo e rientrando in una sessione vocale dopo ripetuti errori di decrittazione.
...
- `socketMode` passa la configurazione del trasporto Socket Mode dello Slack SDK all'API pubblica del receiver Bolt. Usalo solo quando indaghi timeout ping/pong o comportamento websocket obsoleto.
...
- Gli snapshot degli account Slack espongono campi sorgente/stato per credenziale come
  ...
  configurato tramite SecretRef ma il percorso di comando/runtime corrente non è riuscito a
  risolvere il valore del segreto.
- `configWrites: false` blocca le scritture di configurazione iniziate da Slack.
...
- `channels.slack.streaming.mode` è la chiave canonica della modalità stream Slack. `channels.slack.streaming.nativeTransport` controlla il trasporto di streaming nativo di Slack. I valori legacy `streamMode`, booleani `streaming` e `nativeStreaming` restano alias di runtime; esegui `openclaw doctor --fix` per riscrivere la configurazione persistita.
...
- Lo streaming nativo Slack più lo stato thread in stile assistente Slack "is typing..." richiedono un target thread di risposta. I DM di livello superiore restano fuori thread per impostazione predefinita, quindi possono comunque trasmettere in streaming tramite anteprime bozza Slack pubblica-e-modifica invece di mostrare l'anteprima di stream/stato nativa in stile thread.
...
Modalità chat: `oncall` (risponde alla @-menzione, predefinito), `onmessage` (ogni messaggio), `onchar` (messaggi che iniziano con il prefisso di attivazione).
...
- Le callback slash native sono autenticate con i token per comando restituiti da Mattermost durante la registrazione dei comandi slash. Se la registrazione non riesce o non viene attivato alcun comando, OpenClaw rifiuta le callback con `Unauthorized: invalid command token.`
- Per host di callback privati/tailnet/interni, Mattermost può richiedere che `ServiceSettings.AllowedUntrustedInternalConnections` includa l'host/il dominio di callback. Usa valori host/dominio, non URL completi.
...
- `channels.mattermost.groups.<channelId>.requireMention`: override per canale del controllo tramite menzione (`"*"` per il valore predefinito).
...
BlueBubbles è il bridge iMessage legacy (supportato da Plugin, configurato in `channels.bluebubbles`). Le configurazioni esistenti restano supportate, ma le nuove distribuzioni iMessage di OpenClaw dovrebbero preferire `channels.imessage` quando `imsg` può essere eseguito sull'host di Messages.
...
OpenClaw avvia `imsg rpc` (JSON-RPC su stdio). Non sono richiesti daemon o porte. Questo è il percorso preferito per le nuove configurazioni iMessage di OpenClaw quando l'host può concedere i permessi per il database di Messages e Automation.
...
- `channels.matrix.proxy` instrada il traffico HTTP Matrix attraverso un proxy HTTP(S) esplicito. Gli account denominati possono sovrascriverlo con `channels.matrix.accounts.<id>.proxy`.
...
- `channels.matrix.autoJoin` è predefinito a `off`, quindi le stanze a cui si è invitati e i nuovi inviti in stile DM vengono ignorati finché non imposti `autoJoin: "allowlist"` con `autoJoinAllowlist` o `autoJoin: "always"`.
- `channels.matrix.execApprovals`: consegna delle approvazioni exec native Matrix e autorizzazione degli approvatori.
  - `enabled`: `true`, `false` o `"auto"` (predefinito). In modalità auto, le approvazioni exec si attivano quando gli approvatori possono essere risolti da `approvers` o `commands.ownerAllowFrom`.
  - `approvers`: ID utente Matrix (per es. `@owner:example.org`) autorizzati ad approvare richieste exec.
...
- `channels.matrix.dm.sessionScope` controlla il modo in cui i DM Matrix vengono raggruppati in sessioni: `per-user` (predefinito) condivide in base al peer instradato, mentre `per-room` isola ogni stanza DM.
...
IRC è supportato da Plugin e configurato in `channels.irc`.
...
- La configurazione completa del canale IRC (host/porta/TLS/canali/allowlist/controllo tramite menzione) è documentata in [IRC](/it/channels/irc).
...
- Le associazioni esistenti solo canale (senza `accountId`) continuano a corrispondere all'account predefinito; le associazioni con ambito account restano opzionali.
...
### Controllo tramite menzione nelle chat di gruppo
...
Le risposte visibili sono controllate separatamente. Le stanze di gruppo/canale usano per impostazione predefinita `messages.groupChat.visibleReplies: "message_tool"`: OpenClaw elabora comunque il turno, ma le normali risposte finali restano private e l’output visibile della stanza richiede `message(action=send)`. Imposta `"automatic"` solo quando vuoi il comportamento legacy in cui le normali risposte vengono pubblicate nella stanza. Per applicare lo stesso comportamento di risposta visibile solo tramite strumento anche alle chat dirette, imposta `messages.visibleReplies: "message_tool"`; anche l’harness Codex usa quel comportamento solo tramite strumento come impostazione predefinita non configurata per le chat dirette.
...
- **Menzioni da metadati**: @-menzioni native della piattaforma. Ignorate in modalità chat con se stessi di WhatsApp.
...
{
  messages: {
    visibleReplies: "automatic", // impostazione predefinita globale per chat dirette/sorgente; l'harness Codex usa per impostazione predefinita message_tool per le chat dirette non configurate
    groupChat: {
      historyLimit: 50,
      visibleReplies: "message_tool", // impostazione predefinita; usa "automatic" per le risposte finali legacy
    },
  },
  agents: {
...
- Questo blocco configura le superfici dei comandi. Per il catalogo attuale dei comandi integrati + in bundle, vedi [Comandi Slash](/it/tools/slash-commands).
...
- Per canali multi-account, anche `channels.<provider>.accounts.<id>.configWrites` controlla le scritture che hanno come destinazione quell’account (ad esempio `/allowlist --config --account <id>` o `/config set channels.<provider>.accounts.<id>...`).
- `restart: false` disabilita `/restart` e le azioni dello strumento di riavvio Gateway. Predefinito: `true`.
- `ownerAllowFrom` è la allowlist esplicita dei proprietari per comandi/strumenti riservati al proprietario. È separata da `allowFrom`.
...
- `allowFrom` è per provider. Quando impostato, è l’**unica** sorgente di autorizzazione (le allowlist/associazioni dei canali e `useAccessGroups` vengono ignorate).
