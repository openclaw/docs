---
read_when:
    - Vuoi inviare l'utilizzo del modello OpenClaw, il flusso dei messaggi o le metriche di sessione a un collettore OpenTelemetry
    - Stai collegando tracce, metriche o log a Grafana, Datadog, Honeycomb, New Relic, Tempo o un altro backend OTLP
    - Servono i nomi esatti delle metriche, i nomi degli span o le strutture degli attributi per creare dashboard o avvisi
...
x-i18n:
    generated_at: "2026-05-03T21:33:44Z"
    model: gpt-5.5
    provider: openai
...
- Le **chiamate ai provider** ricevono un header W3C `traceparent` dal contesto
  dello span fidato di chiamata al modello di OpenClaw quando il trasporto del provider accetta header
...
| ----------- | --------------------------------------------------------------------------------------------------------------------------------------------- |
| **Metriche** | Counter e istogrammi per utilizzo dei token, costo, durata dell'esecuzione, flusso dei messaggi, lane delle code, stato della sessione, exec e pressione di memoria. |
| **Tracce**  | Span per utilizzo dei modelli, chiamate ai modelli, ciclo di vita dell'harness, esecuzione dei tool, exec, elaborazione webhook/messaggi, assemblaggio del contesto e loop dei tool. |
...
| `OTEL_EXPORTER_OTLP_TRACES_ENDPOINT` / `OTEL_EXPORTER_OTLP_METRICS_ENDPOINT` / `OTEL_EXPORTER_OTLP_LOGS_ENDPOINT` | Sovrascritture endpoint specifiche del segnale, usate quando la chiave di configurazione `diagnostics.otel.*Endpoint` corrispondente non è impostata. La configurazione specifica del segnale prevale sulla variabile d'ambiente specifica del segnale, che prevale sull'endpoint condiviso. |
...
Il contenuto grezzo di modelli/tool **non** viene esportato per impostazione predefinita. Gli span contengono identificatori
limitati (canale, provider, modello, categoria di errore, ID richiesta solo hash)
e non includono mai testo del prompt, testo della risposta, input dei tool, output dei tool o
chiavi di sessione.

Le richieste in uscita ai modelli possono includere un header W3C `traceparent`. Questo header viene
...
la policy di conservazione sono approvati per testo di prompt, risposta, tool o prompt di sistema.
...
- `toolInputs` — payload degli argomenti del tool.
- `toolOutputs` — payload dei risultati del tool.
- `systemPrompt` — prompt di sistema/sviluppatore assemblato.

Quando una sottochiave è abilitata, gli span di modelli e tool ricevono attributi
...
- **Correlazione dei log su file:** i log su file JSONL includono `traceId`,
  `spanId`, `parentSpanId` e `traceFlags` al livello superiore quando la chiamata di log porta un
  contesto di traccia diagnostico valido, consentendo ai processori di log di unire le righe di log locali con
  gli span esportati.
- **Correlazione delle richieste:** le richieste HTTP del Gateway e i frame WebSocket creano uno
  scope di traccia richiesta interno. Log ed eventi diagnostici dentro quello scope
...
- `openclaw.tokens` (counter, attributi: `openclaw.token`, `openclaw.channel`, `openclaw.provider`, `openclaw.model`, `openclaw.agent`)
- `openclaw.cost.usd` (counter, attributi: `openclaw.channel`, `openclaw.provider`, `openclaw.model`)
...
- `gen_ai.client.operation.duration` (istogramma, secondi, metrica delle convenzioni semantiche GenAI, attributi: `gen_ai.provider.name`, `gen_ai.operation.name`, `gen_ai.request.model`, `error.type` opzionale)
...
- `openclaw.webhook.received` (counter, attributi: `openclaw.channel`, `openclaw.webhook`)
- `openclaw.webhook.error` (counter, attributi: `openclaw.channel`, `openclaw.webhook`)
- `openclaw.webhook.duration_ms` (istogramma, attributi: `openclaw.channel`, `openclaw.webhook`)
- `openclaw.message.queued` (counter, attributi: `openclaw.channel`, `openclaw.source`)
- `openclaw.message.processed` (counter, attributi: `openclaw.channel`, `openclaw.outcome`)
- `openclaw.message.duration_ms` (istogramma, attributi: `openclaw.channel`, `openclaw.outcome`)
- `openclaw.message.delivery.started` (counter, attributi: `openclaw.channel`, `openclaw.delivery.kind`)
...
- `openclaw.session.stuck` (counter, attributi: `openclaw.state`; emesso solo per la contabilità di sessioni obsolete senza lavoro attivo)
- `openclaw.session.stuck_age_ms` (istogramma, attributi: `openclaw.state`; emesso solo per la contabilità di sessioni obsolete senza lavoro attivo)
...
di vitalità della sessione. Una sessione `processing` non invecchia verso questa soglia
mentre OpenClaw osserva avanzamento runtime di risposta, tool, stato, blocco o ACP.
...
- `session.stalled`: esiste lavoro attivo, ma l'esecuzione attiva non ha segnalato
  ...
  eseguono abort-drain dopo almeno 10 minuti e 5 volte `diagnostics.stuckSessionWarnMs`
...
Le diagnostiche `session.stuck` ripetute applicano un backoff finché la sessione resta
invariata, quindi i dashboard dovrebbero generare avvisi sugli aumenti sostenuti invece che a ogni
...
- `openclaw.context.assembled`
  - `openclaw.prompt.size`, `openclaw.history.size`, `openclaw.context.tokens`, `openclaw.errorCategory` (nessun contenuto di prompt, cronologia, risposta o chiave sessione)
- `openclaw.tool.loop`
  - `openclaw.toolName`, `openclaw.outcome`, `openclaw.iterations`, `openclaw.errorCategory` (nessun messaggio del ciclo, parametro o output degli strumenti)
- `openclaw.memory.pressure`
  - `openclaw.memory.level`, `openclaw.memory.heap_used_bytes`, `openclaw.memory.rss_bytes`

Quando l'acquisizione del contenuto è abilitata esplicitamente, gli span del modello e degli strumenti possono anche
...
Puoi mantenere gli eventi di diagnostica disponibili per Plugin o destinazioni personalizzate senza
...
- [Flag di diagnostica](/it/diagnostics/flags) — flag di log di debug mirati
- [Esportazione diagnostica](/it/gateway/diagnostics) — strumento di bundle di supporto per operatori (separato dall'esportazione OTEL)
