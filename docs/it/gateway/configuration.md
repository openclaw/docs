...
x-i18n:
    generated_at: "2026-05-03T21:32:45Z"
    model: gpt-5.5
    provider: openai
...
[Configuration reference](/it/gateway/configuration-reference) per la mappa dei campi più ampia
e le impostazioni predefinite.

<Tip>
**Non conosci ancora la configurazione?** Inizia con `openclaw onboard` per la configurazione interattiva, oppure consulta la guida [Esempi di configurazione](/it/gateway/configuration-examples) per configurazioni complete da copiare e incollare.
</Tip>
...
<Tabs>
  ...
  <Tab title="Interfaccia di controllo">
    ...
    L'interfaccia di controllo genera un modulo dallo schema di configurazione live, includendo i metadati
    di documentazione `title` / `description` dei campi, oltre agli schemi di plugin e canali quando
    disponibili, con un editor **Raw JSON** come via d'uscita. Per UI di approfondimento
    e altri strumenti, il Gateway espone anche `config.schema.lookup` per
    recuperare un nodo schema limitato a un percorso, più riepiloghi immediati dei figli.
  </Tab>
...
<Warning>
OpenClaw accetta solo configurazioni che corrispondono pienamente allo schema. Chiavi sconosciute, tipi non validi o valori non validi fanno sì che il Gateway **rifiuti di avviarsi**. L'unica eccezione a livello radice è `$schema` (stringa), così gli editor possono collegare metadati JSON Schema.
</Warning>

`openclaw config schema` stampa il JSON Schema canonico usato dall'interfaccia di controllo
e dalla validazione. `config.schema.lookup` recupera un singolo nodo limitato a un percorso, più
riepiloghi dei figli per strumenti di approfondimento. I metadati di documentazione `title`/`description`
dei campi si propagano attraverso oggetti annidati, wildcard (`*`), elementi di array (`[]`) e rami `anyOf`/
...
non supera la validazione (inclusa la validazione locale del plugin), l'avvio del Gateway fallisce oppure
...
ripristinare la copia dell'ultima configurazione valida. La promozione a ultima configurazione valida viene saltata quando un
...
<AccordionGroup>
  ...
  <Accordion title="Scegli e configura i modelli">
    ...
    - Usa `openclaw config set agents.defaults.models '<json>' --strict-json --merge` per aggiungere voci allowlist senza rimuovere modelli esistenti. Le sostituzioni semplici che rimuoverebbero voci vengono rifiutate a meno che tu non passi `--replace`.
    ...
    - Consulta [CLI dei modelli](/it/concepts/models) per cambiare modelli in chat e [Failover del modello](/it/concepts/model-failover) per la rotazione dell'autenticazione e il comportamento di fallback.
  ...
  <Accordion title="Controlla chi può inviare messaggi al bot">
    ...
    - `"allowlist"`: solo i mittenti in `allowFrom` (o nello store consentito associato)
  ...
  <Accordion title="Configura il gating delle menzioni nelle chat di gruppo">
    ...
    - **Menzioni nei metadati**: @-mentions native (menzione tramite tocco in WhatsApp, Telegram @bot, ecc.)
  ...
  <Accordion title="Regola il timeout dell'handshake WebSocket del gateway">
    Concedi ai client locali più tempo per completare l'handshake WebSocket pre-autenticazione su
    host sotto carico o a bassa potenza:
    ...
    - `OPENCLAW_HANDSHAKE_TIMEOUT_MS` ha comunque precedenza per override una tantum di servizio o shell.
    - Preferisci prima correggere gli stalli di startup/event-loop; questa manopola è per host sani ma lenti durante il riscaldamento.
  ...
  <Accordion title="Abilita il sandboxing">
    ...
    Crea prima l'immagine: da un checkout del sorgente esegui `scripts/sandbox-setup.sh`, oppure da un'installazione npm consulta il comando inline `docker build` in [Sandboxing § Immagini e configurazione](/it/gateway/sandboxing#images-and-setup).
  ...
  <Accordion title="Abilita il push supportato da relay per le build iOS ufficiali">
    ...
    - Usa una concessione di invio con ambito di registrazione inoltrata dall'app iOS abbinata. Il gateway non richiede un token relay valido per tutto il deployment.
    - Associa ogni registrazione supportata da relay all'identità del gateway con cui l'app iOS è stata abbinata, quindi un altro gateway non può riutilizzare la registrazione archiviata.
    ...
    - Deve corrispondere all'URL di base del relay incorporato nella build iOS ufficiale/TestFlight, così il traffico di registrazione e invio raggiunge lo stesso deployment relay.
    ...
    3. Abbina l'app iOS al gateway e consenti la connessione sia alle sessioni node sia a quelle operatore.
    4. L'app iOS recupera l'identità del gateway, si registra con il relay usando App Attest più la ricevuta dell'app, quindi pubblica il payload `push.apns.register` supportato da relay sul gateway abbinato.
    5. Il gateway archivia l'handle relay e la concessione di invio, quindi li usa per `push.test`, solleciti di risveglio e risvegli di riconnessione.
    ...
    - Se distribuisci una nuova build iOS che punta a un deployment relay diverso, l'app aggiorna la propria registrazione relay memorizzata nella cache invece di riutilizzare la vecchia origine relay.

    Nota di compatibilità:

    - `OPENCLAW_APNS_RELAY_BASE_URL` e `OPENCLAW_APNS_RELAY_TIMEOUT_MS` funzionano ancora come override temporanei tramite env.
    - `OPENCLAW_APNS_RELAY_ALLOW_HTTP=true` rimane una via di fuga di sviluppo solo per loopback; non salvare URL relay HTTP nella configurazione.
  ...
  <Accordion title="Configura job Cron">
    ...
    - `sessionRetention`: rimuove le sessioni di esecuzione isolate completate da `sessions.json` (predefinito `24h`; imposta `false` per disabilitare).
    - `runLog`: rimuove `cron/runs/<jobId>.jsonl` in base alle dimensioni e alle righe conservate.
  ...
  <Accordion title="Configura i Webhook (hook)">
    ...
    - Mantieni disabilitati i flag di bypass dei contenuti non sicuri (`hooks.gmail.allowUnsafeExternalContent`, `hooks.mappings[].allowUnsafeExternalContent`) salvo per debugging con ambito strettamente definito.
    ...
    - Per agenti guidati da hook, preferisci livelli di modello moderni e solidi e criteri degli strumenti rigorosi (ad esempio solo messaggistica più sandboxing dove possibile).
  ...
  <Accordion title="Suddividi la configurazione in più file ($include)">
    ...
    - **Array di file**: uniti in profondità nell'ordine indicato (l'ultimo prevale)
    ...
    - **Write-through non supportato**: include root, array di include e include
      con override adiacenti falliscono in modo chiuso per le scritture di proprietà di OpenClaw invece di
...
che il rumore di scrittura temporanea/rinomina dell'editor si stabilizzi, legge il file finale e rifiuta
le modifiche esterne non valide senza riscrivere `openclaw.json`. Le scritture di configurazione
di proprietà di OpenClaw usano lo stesso gate di schema prima della scrittura; clobber distruttivi come
...
| ---------------------- | --------------------------------------------------------------------------------------- |
| **`hybrid`** (predefinita) | Applica subito a caldo le modifiche sicure. Riavvia automaticamente per quelle critiche. |
...
Questo mantiene prevedibili le decisioni di ricaricamento a caldo (applicazione a caldo rispetto al riavvio) anche quando una
...
- `config.schema.lookup` per ispezionare un sottoalbero (nodo dello schema superficiale + riepiloghi dei figli)
...
- `update.run` per un auto-aggiornamento esplicito più riavvio; includi `continuationMessage` quando la sessione post-riavvio deve eseguire un turno di follow-up
...
Gli agenti devono trattare `config.schema.lookup` come il primo punto di riferimento per documentazione e vincoli esatti
a livello di campo. Usa [Riferimento di configurazione](/it/gateway/configuration-reference)
quando serve la mappa di configurazione più ampia, i valori predefiniti o i link ai riferimenti dedicati dei sottosistemi.
...
`update.status` è in sola lettura ma con ambito amministrativo perché il sentinella di riavvio può
...
Consulta [Ambiente](/it/help/environment) per precedenza e fonti complete.
