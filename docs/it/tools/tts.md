...
x-i18n:
    generated_at: "2026-05-06T09:13:59Z"
    model: gpt-5.5
    provider: openai
...
`realtime` native del provider sintetizzano il parlato all'interno del provider realtime
...
testuale a meno che l'utente non chieda l'audio, usi `/tts` o abiliti Auto-TTS/direttiva
vocale.
</Note>
...
| **MiniMax**       | `MINIMAX_API_KEY` (o Piano Token: `MINIMAX_OAUTH_TOKEN`, `MINIMAX_CODE_PLAN_KEY`, `MINIMAX_CODING_API_KEY`)      | API T2A v2. Predefinito su `speech-2.8-hd`.                             |
...
le nuove configurazioni dovrebbero usare sempre `microsoft`.
</Warning>
...
una voce, un modello, una persona o una modalità Auto-TTS diversi. Il blocco agente esegue un deep merge sopra
...
sovrascrive la `messages.tts.persona` globale solo per quell'agente.
...
si uniscono in profondità ai livelli precedenti, quindi le credenziali condivise
del provider possono restare in `messages.tts` mentre un canale o un account bot
modifica solo voce, modello, persona o modalità automatica:
...
tra provider. Può preferire un provider, definire l'intento del prompt
indipendente dal provider e portare binding specifici del provider per voci, modelli, template di prompt,
...
La selezione del provider usa prima i valori espliciti:
...
4. Override da direttive TTS emesse dal modello e consentite
...
<AccordionGroup>
  <Accordion title="Google Gemini">
    Racchiude i campi prompt della persona in una struttura di prompt TTS Gemini **solo quando**
  ...
  <Accordion title="Altri provider">
    ...
    a meno che il provider implementi la propria mappatura dei prompt della persona.
  </Accordion>
</AccordionGroup>
...
| `provider-defaults` | La persona viene omessa dalla preparazione del prompt per quel tentativo; il provider usa i propri valori predefiniti neutri mentre il fallback ad altri provider continua. |
...
gli ID provider, gli ID modello, gli ID voce e le impostazioni locali da `talk.catalog` e passarli
...
`[[tts:text]]...[[/tts:text]]` per segnali espressivi che dovrebbero apparire
solo nell'audio:
...
<Note>
I comandi richiedono un mittente autorizzato (si applicano le regole allowlist/owner) e
...
- `/tts latest` legge l'ultima risposta dell'assistente dalla trascrizione della sessione corrente e la invia una volta come audio. Memorizza solo un hash di quella risposta nella voce della sessione per sopprimere invii vocali duplicati.
...
| `summarize`      | Attiva/disattiva riepilogo (predefinito `true`) |
...
La consegna vocale TTS è guidata dalle capacità del canale. I Plugin di canale pubblicizzano
se il TTS in stile messaggio vocale deve chiedere ai provider un target nativo `voice-note` o
mantenere la normale sintesi `audio-file` e marcare solo l'output compatibile per la consegna
vocale.

- **Canali compatibili con le note vocali**: le risposte come note vocali preferiscono Opus (`opus_48000_64` da ElevenLabs, `opus` da OpenAI).
...
- **Feishu / WhatsApp**: quando una risposta come nota vocale viene prodotta come MP3/WebM/WAV/M4A
  o un altro file probabilmente audio, il plugin del canale la transcodifica a Ogg/Opus
...
- **MiniMax**: MP3 (modello `speech-2.8-hd`, frequenza di campionamento 32 kHz) per i normali allegati audio. Per le destinazioni per note vocali dichiarate dal canale, OpenClaw transcodifica l'MP3 MiniMax a Opus 48 kHz con `ffmpeg` prima della consegna quando il canale dichiara la transcodifica.
- **Xiaomi MiMo**: MP3 per impostazione predefinita, oppure WAV quando configurato. Per le destinazioni per note vocali dichiarate dal canale, OpenClaw transcodifica l'output Xiaomi a Opus 48 kHz con `ffmpeg` prima della consegna quando il canale dichiara la transcodifica.
- **CLI locale**: usa l'`outputFormat` configurato. Le destinazioni per note vocali sono
  convertite in Ogg/Opus e l'output di telefonia viene convertito in PCM mono raw a 16 kHz
  con `ffmpeg`.
- **Google Gemini**: Gemini API TTS restituisce PCM raw a 24 kHz. OpenClaw lo incapsula come WAV per gli allegati audio, lo transcodifica a Opus 48 kHz per le destinazioni per note vocali e restituisce direttamente PCM per Talk/telefonia.
...
- **xAI**: MP3 per impostazione predefinita; `responseFormat` può essere `mp3`, `wav`, `pcm`, `mulaw` o `alaw`. OpenClaw usa l'endpoint TTS REST batch di xAI e restituisce un allegato audio completo; il WebSocket TTS streaming di xAI non viene usato da questo percorso del provider. Il formato nativo Opus per note vocali non è supportato da questo percorso.
...
| ------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------- |
| Feishu / Matrix / Telegram / WhatsApp | Le risposte come note vocali preferiscono **Opus** (`opus_48000_64` da ElevenLabs, `opus` da OpenAI). 48 kHz / 64 kbps bilancia chiarezza e dimensione. |
...
- **Transcodifica Feishu / WhatsApp:** Quando una risposta come nota vocale arriva come MP3/WebM/WAV/M4A, il plugin del canale la transcodifica a Ogg/Opus 48 kHz con `ffmpeg`. WhatsApp invia tramite Baileys con `ptt: true` e `audio/ogg; codecs=opus`. Se la conversione non riesce: Feishu ripiega sull'allegare il file originale; l'invio di WhatsApp non riesce invece di pubblicare un payload PTT incompatibile.
- **MiniMax / Xiaomi MiMo:** MP3 predefinito (32 kHz per MiniMax `speech-2.8-hd`); transcodificato a Opus 48 kHz per le destinazioni per note vocali tramite `ffmpeg`.
- **CLI locale:** Usa l'`outputFormat` configurato. Le destinazioni per note vocali sono convertite in Ogg/Opus e l'output di telefonia in PCM mono raw a 16 kHz.
- **Google Gemini:** Restituisce PCM raw a 24 kHz. OpenClaw lo incapsula come WAV per gli allegati, lo transcodifica a Opus 48 kHz per le destinazioni per note vocali, restituisce direttamente PCM per Talk/telefonia.
- **Inworld:** Allegati MP3, nota vocale `OGG_OPUS` nativa, `PCM` raw a 22050 Hz per Talk/telefonia.
- **xAI:** MP3 per impostazione predefinita; `responseFormat` può essere `mp3|wav|pcm|mulaw|alaw`. Usa l'endpoint REST batch di xAI: il WebSocket TTS streaming **non** viene usato. Il formato nativo Opus per note vocali **non** è supportato.
...
<AccordionGroup>
  <Accordion title="messages.tts.* di primo livello">
    <ParamField path="auto" type='"off" | "always" | "inbound" | "tagged"'>
      Modalità Auto-TTS. `inbound` invia l'audio solo dopo un messaggio vocale in ingresso; `tagged` invia l'audio solo quando la risposta include direttive `[[tts:...]]` o un blocco `[[tts:text]]`.
    </ParamField>
    ...
    <ParamField path="summaryModel" type="string">
      Modello economico per il riassunto automatico; il valore predefinito è `agents.defaults.model.primary`. Accetta `provider/model` o un alias di modello configurato.
    </ParamField>
    <ParamField path="modelOverrides" type="object">
      Consente al modello di emettere direttive TTS. `enabled` è `true` per impostazione predefinita; `allowProvider` è `false` per impostazione predefinita.
    </ParamField>
  ...
  <Accordion title="Azure Speech">
    ...
    <ParamField path="voiceNoteOutputFormat" type="string">`X-Microsoft-OutputFormat` di Azure per l'output delle note vocali. Predefinito `ogg-24khz-16bit-mono-opus`.</ParamField>
  </Accordion>
  ...
  <Accordion title="Google Gemini">
    ...
    <ParamField path="model" type="string">Modello Gemini TTS. Predefinito `gemini-3.1-flash-tts-preview`.</ParamField>
    ...
    <ParamField path="promptTemplate" type='"audio-profile-v1"'>Imposta su `audio-profile-v1` per incapsulare i campi del prompt della persona attiva in una struttura di prompt Gemini TTS deterministica.</ParamField>
    <ParamField path="personaPrompt" type="string">Testo aggiuntivo del prompt della persona specifico di Google, aggiunto alle note del regista del template.</ParamField>
  ...
  <Accordion title="Inworld">
    ...
    <ParamField path="modelId" type="string">Predefinito `inworld-tts-1.5-max`. Inoltre: `inworld-tts-1.5-mini`, `inworld-tts-1-max`, `inworld-tts-1`.</ParamField>
  ...
  <Accordion title="CLI locale (tts-local-cli)">
    ...
    <ParamField path="cwd" type="string">Directory di lavoro del comando facoltativa.</ParamField>
    <ParamField path="env" type="Record<string, string>">Override di ambiente facoltativi per il comando.</ParamField>
  </Accordion>
  ...
  <Accordion title="Microsoft (nessuna chiave API)">
    ...
    <ParamField path="saveSubtitles" type="boolean">Scrivi i sottotitoli JSON insieme al file audio.</ParamField>
  ...
  <Accordion title="OpenAI">
    ...
    <ParamField path="instructions" type="string">Campo `instructions` OpenAI esplicito. Quando impostato, i campi del prompt persona **non** vengono mappati automaticamente.</ParamField>
    ...
    <ParamField path="baseUrl" type="string">
      Override dell'endpoint TTS OpenAI. Ordine di risoluzione: configurazione → `OPENAI_TTS_BASE_URL` → `https://api.openai.com/v1`. I valori non predefiniti vengono trattati come endpoint TTS compatibili con OpenAI, quindi sono accettati nomi di modelli e voci personalizzati.
    </ParamField>
  </Accordion>
  ...
  <Accordion title="Volcengine (BytePlus Seed Speech)">
    ...
    <ParamField path="resourceId" type="string">Predefinito `seed-tts-1.0`. Env: `VOLCENGINE_TTS_RESOURCE_ID`. Usa `seed-tts-2.0` quando il tuo progetto ha l'autorizzazione TTS 2.0.</ParamField>
    <ParamField path="appKey" type="string">Header della chiave app. Predefinito `aGjiRDfUWi`. Env: `VOLCENGINE_TTS_APP_KEY`.</ParamField>
    <ParamField path="baseUrl" type="string">Override dell'endpoint HTTP TTS Seed Speech. Env: `VOLCENGINE_TTS_BASE_URL`.</ParamField>
...
timeout di richiesta del provider per chiamata in millisecondi.

## RPC Gateway

| Metodo            | Scopo                                      |
| ----------------- | ------------------------------------------ |
| `tts.status`      | Legge lo stato TTS attuale e l'ultimo tentativo. |
