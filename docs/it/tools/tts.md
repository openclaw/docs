...
read_when:
    ...
    - Configurare un provider TTS, una catena di riserva o una persona
    - Uso dei comandi o delle direttive /tts
sidebarTitle: Text to speech (TTS)
summary: Sintesi vocale per le risposte in uscita — provider, profili persona, comandi slash e output per canale
title: Sintesi vocale
x-i18n:
    generated_at: "2026-05-02T08:37:07Z"
    model: gpt-5.5
    provider: openai
...
OpenClaw può convertire le risposte in uscita in audio usando **14 provider vocali**
...
<Steps>
  ...
  <Step title="Imposta la chiave API">
    ...
    `ELEVENLABS_API_KEY`). Microsoft e Local CLI non richiedono chiavi.
  </Step>
  ...
  <Step title="Provalo in chat">
    ...
    invia una risposta audio singola.
  </Step>
</Steps>
...
Lo strumento agent integrato `tts` è solo per intenti espliciti: la chat ordinaria resta
testuale a meno che l'utente non chieda audio, usi `/tts` o abiliti il parlato Auto-TTS/direttiva.
</Note>
...
| **OpenAI**        | `OPENAI_API_KEY`                                                                                                 | Usato anche per il riepilogo automatico; supporta la persona `instructions`. |
...
altri sono opzioni di fallback. Il riepilogo automatico usa `summaryModel` (oppure
...
### Override vocali per agent

Usa `agents.list[].tts` quando un agent deve parlare con un provider,
una voce, un modello, una persona o una modalità Auto-TTS diversi. Il blocco dell'agent viene unito in profondità sopra
...
Per fissare una persona per singolo agent, imposta `agents.list[].tts.persona` insieme alla configurazione del provider:
sovrascrive la `messages.tts.persona` globale solo per quell'agent.
...
Gli override di canale e account usano la stessa forma di `messages.tts` ed
eseguono un deep merge sui livelli precedenti, quindi le credenziali del provider
condivise possono restare in `messages.tts` mentre un canale o un account bot
...
tra i provider. Può preferire un provider, definire un intento di prompt
neutrale rispetto al provider e contenere associazioni specifiche del provider per voci, modelli, template di prompt,
...
2. `messages.tts.persona`, se impostato.
3. Nessuna persona.
...
Per ogni tentativo con un provider, OpenClaw unisce le configurazioni in questo ordine:
...
### Come i provider usano i prompt delle persona
...
<AccordionGroup>
  <Accordion title="Google Gemini">
    Racchiude i campi del prompt della persona in una struttura di prompt TTS Gemini **solo quando**
  ...
  <Accordion title="Altri provider">
    ...
    a meno che il provider implementi la propria mappatura del prompt della persona.
  </Accordion>
</AccordionGroup>
...
| `provider-defaults` | La persona viene omessa dalla preparazione del prompt per quel tentativo; il provider usa i propri valori predefiniti neutri mentre il fallback verso altri provider continua. |
| `fail`              | Salta quel tentativo con il provider con `reasonCode: "not_configured"` e `personaBinding: "missing"`. I provider di fallback vengono comunque provati. |
...
`[[tts:text]]...[[/tts:text]]` per indicazioni espressive che devono apparire
solo nell'audio:
...
l'audio. La consegna in streaming dei blocchi rimuove le direttive dal testo visibile prima che il
...
**Consenti il cambio di provider mantenendo configurabili gli altri parametri:**
...
Un solo comando `/tts`. Su Discord, OpenClaw registra anche `/voice` perché
...
<Note>
I comandi richiedono un mittente autorizzato (si applicano le regole allowlist/proprietario) e devono essere
abilitati `commands.text` o la registrazione nativa dei comandi.
</Note>
...
se il TTS in stile vocale debba chiedere ai provider un target nativo `voice-note` oppure
mantenere la normale sintesi `audio-file` e contrassegnare solo l'output compatibile per la consegna
vocale.

- **Canali compatibili con le note vocali**: le risposte con nota vocale preferiscono Opus (`opus_48000_64` da ElevenLabs, `opus` da OpenAI).
...
- **Feishu / WhatsApp**: quando una risposta con nota vocale viene prodotta come MP3/WebM/WAV/M4A
  ...
  come allegato; l'invio WhatsApp fallisce invece di pubblicare un payload PTT incompatibile.
...
- **MiniMax**: MP3 (modello `speech-2.8-hd`, frequenza di campionamento 32kHz) per i normali allegati audio. Per le destinazioni con nota vocale dichiarate dal canale, OpenClaw transcodifica l'MP3 MiniMax in Opus a 48kHz con `ffmpeg` prima della consegna quando il canale dichiara la transcodifica.
- **Xiaomi MiMo**: MP3 per impostazione predefinita, oppure WAV quando configurato. Per le destinazioni con nota vocale dichiarate dal canale, OpenClaw transcodifica l'output Xiaomi in Opus a 48kHz con `ffmpeg` prima della consegna quando il canale dichiara la transcodifica.
- **CLI locale**: usa l'`outputFormat` configurato. Le destinazioni con nota vocale vengono
  convertite in Ogg/Opus e l'output per telefonia viene convertito in PCM mono raw a 16 kHz
  con `ffmpeg`.
- **Google Gemini**: il TTS dell'API Gemini restituisce PCM raw a 24kHz. OpenClaw lo incapsula come WAV per gli allegati audio, lo transcodifica in Opus a 48kHz per le destinazioni con nota vocale e restituisce direttamente PCM per Talk/telefonia.
- **Gradium**: WAV per gli allegati audio, Opus per le destinazioni con nota vocale e `ulaw_8000` a 8 kHz per la telefonia.
- **Inworld**: MP3 per i normali allegati audio, `OGG_OPUS` nativo per le destinazioni con nota vocale e `PCM` raw a 22050 Hz per Talk/telefonia.
...
- In `mode: "final"`, invia comunque TTS solo audio per le risposte finali in streaming
  dopo il completamento dello stream di testo; i contenuti multimediali generati passano attraverso la stessa
  normalizzazione dei media del canale dei normali allegati di risposta.
...
## Formati di output per canale
  ...
  | ------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------- |
  | Feishu / Matrix / Telegram / WhatsApp | Le risposte con nota vocale preferiscono **Opus** (`opus_48000_64` da ElevenLabs, `opus` da OpenAI). 48 kHz / 64 kbps bilancia chiarezza e dimensioni. |
  ...
  - **CLI locale:** usa il `outputFormat` configurato. Le destinazioni con nota vocale vengono convertite in Ogg/Opus e l'output telefonico in PCM mono grezzo a 16 kHz.
  ...
  I formati di output di OpenAI ed ElevenLabs sono fissati per canale come elencato sopra.
  ...
  <Accordion title="Top-level messages.tts.*">
    ...
    <ParamField path="persona" type="string">
      ID persona attiva da `personas`. Normalizzato in minuscolo.
    </ParamField>
  ...
  <Accordion title="Azure Speech">
    ...
    <ParamField path="voiceNoteOutputFormat" type="string">`X-Microsoft-OutputFormat` di Azure per l'output con nota vocale. Predefinito `ogg-24khz-16bit-mono-opus`.</ParamField>
  </Accordion>
  ...
  <Accordion title="Google Gemini">
    ...
    <ParamField path="personaPrompt" type="string">Testo aggiuntivo del prompt persona specifico di Google aggiunto alle Note del direttore del template.</ParamField>
  ...
  <Accordion title="CLI locale (tts-local-cli)">
    <ParamField path="command" type="string">Eseguibile locale o stringa di comando per TTS da CLI.</ParamField>
  ...
  <Accordion title="Microsoft (nessuna chiave API)">
    ...
    <ParamField path="outputFormat" type="string">Formato di output Microsoft. Predefinito `audio-24khz-48kbitrate-mono-mp3`. Non tutti i formati sono supportati dal trasporto in bundle basato su Edge.</ParamField>
  ...
  <Accordion title="OpenAI">
    ...
    <ParamField path="baseUrl" type="string">
      Sovrascrive l'endpoint TTS OpenAI. Ordine di risoluzione: configurazione → `OPENAI_TTS_BASE_URL` → `https://api.openai.com/v1`. I valori non predefiniti vengono trattati come endpoint TTS compatibili con OpenAI, quindi sono accettati nomi di modello e voce personalizzati.
    </ParamField>
  </Accordion>
  ...
  <Accordion title="Volcengine (BytePlus Seed Speech)">
    ...
    <ParamField path="resourceId" type="string">Predefinito `seed-tts-1.0`. Env: `VOLCENGINE_TTS_RESOURCE_ID`. Usa `seed-tts-2.0` quando il tuo progetto ha il diritto TTS 2.0.</ParamField>
...
i client non mostrano in modo coerente le didascalie sulle note vocali.

Lo strumento accetta i campi opzionali `channel` e `timeoutMs`; `timeoutMs` è un
timeout della richiesta del provider per chiamata in millisecondi.
