...
title: Aragosta
x-i18n:
    generated_at: "2026-05-06T09:12:13Z"
    model: gpt-5.5
    provider: openai
...
Lobster è un livello di authoring sopra il lavoro in background distaccato. Per l'orchestrazione dei flussi sopra i singoli task, vedi [Task Flow](/it/automation/taskflow) (`openclaw tasks flow`). Per il registro delle attività dei task, vedi [`openclaw tasks`](/it/automation/tasks).
...
- **Approve/resume è integrato**: un programma normale può chiedere conferma a una persona, ma non può _mettersi in pausa e riprendere_ con un token duraturo senza che tu inventi quel runtime.
...
<Note>
Le allowlist sono opt-in per i Plugin opzionali. `alsoAllow` abilita solo gli strumenti Plugin opzionali indicati, preservando il normale set di strumenti core. Per limitare gli strumenti core, usa `tools.allow` con gli strumenti o i gruppi core che vuoi.
</Note>
...
- `cwd`: directory di lavoro relativa per la pipeline (deve restare dentro la directory di lavoro del Gateway).
...
OpenProse si abbina bene a Lobster: usa `/prose` per orchestrare la preparazione multi-agente, poi esegui una pipeline Lobster per approvazioni deterministiche. Se un programma Prose richiede Lobster, consenti lo strumento `lobster` per i sub-agenti tramite `tools.subagents.tools`. Vedi [OpenProse](/it/prose).
...
- **`lobster timed out`** → aumenta `timeoutMs` oppure suddividi una pipeline lunga.
