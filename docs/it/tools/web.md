...
summary: web_search, x_search e web_fetch -- cerca sul web, cerca post su X o recupera il contenuto della pagina
title: Ricerca web
x-i18n:
    generated_at: "2026-05-03T21:45:38Z"
    model: gpt-5.5
    provider: openai
...
`web_fetch` per recuperare URL in modo leggero. In questa fase, `web_fetch` resta
...
<Steps>
  ...
  <Step title="Use it">
    L'agente può ora chiamare `web_search`:
...
<CardGroup cols={2}>
  ...
  <Card title="Firecrawl" icon="flame" href="/it/tools/firecrawl">
    Risultati strutturati. Ideale insieme a `firecrawl_search` e `firecrawl_scrape` per estrazioni approfondite.
  </Card>
  ...
  <Card title="Kimi" icon="moon" href="/it/tools/kimi-search">
    Risposte sintetizzate dall'AI con citazioni tramite la ricerca web Moonshot; i fallback di chat senza grounding falliscono esplicitamente.
  </Card>
...
| [Kimi](/it/tools/kimi-search)                | Sintetizzati dall'AI + citazioni; fallisce sui fallback di chat senza grounding | --                          | `KIMI_API_KEY` / `MOONSHOT_API_KEY`                                                     |
...
- Configurala in `tools.web.search.openaiCodex`
...
Se la ricerca Codex nativa è abilitata ma il modello corrente non è compatibile con Codex, OpenClaw mantiene il normale comportamento di `web_search` gestito.

## Sicurezza di rete

Le chiamate ai provider `web_search` gestite usano il percorso fetch protetto di OpenClaw. Per
gli host API di provider attendibili, OpenClaw consente risposte DNS fake-IP di Surge, Clash e sing-box
in `198.18.0.0/15` e `fc00::/7` solo per quel nome host del provider.
Altre destinazioni private, di local loopback, link-local e di metadata restano bloccate.

Questa concessione automatica non si applica agli URL `web_fetch` arbitrari. Per
...
11. **Ollama Web Search** -- fallback senza chiave tramite il tuo host Ollama locale configurato quando è raggiungibile e con accesso effettuato tramite `ollama signin`; può riusare l'autenticazione bearer del provider Ollama quando l'host la richiede, e può chiamare la ricerca diretta su `https://ollama.com` quando è configurato con `OLLAMA_API_KEY` (ordine 110)
12. **SearXNG** -- `SEARXNG_BASE_URL` o `plugins.entries.searxng.config.webSearch.baseUrl` (ordine 200)

Se non viene rilevato alcun provider, torna a Brave (riceverai un errore di chiave mancante
...
`models.providers.google.apiKey` e `models.providers.google.baseUrl` come fallback con priorità inferiore
dopo la sua configurazione dedicata di ricerca web e `GEMINI_API_KEY`. Consulta le
...
provider configurato ha solo evidenze di Plugin obsolete, come un blocco
`plugins.entries.<plugin>` residuo dopo la disinstallazione di un Plugin di terze parti,
...
- oppure ometti quel campo e lascia che OpenClaw rilevi automaticamente il primo provider web-fetch pronto
  dalle credenziali disponibili
- `web_fetch` non sandboxed può usare provider di Plugin installati che dichiarano
  `contracts.webFetchProviders`; i fetch sandboxed rimangono limitati a quelli inclusi
...
- il modello Kimi predefinito per la ricerca web (predefinito: `kimi-k2.6`)
...
<Warning>
  ...
  freshness personalizzati di Brave richiedono sia date di inizio sia date di fine.
  Gemini, Grok e Kimi restituiscono una risposta sintetizzata con citazioni. Accettano
  `count` per compatibilità con lo strumento condiviso, ma non cambia la forma
  della risposta fondata. Gemini supporta `freshness`, `date_after` e
...
`x_search` interroga post di X (in precedenza Twitter) usando xAI e restituisce
risposte sintetizzate dall'IA con citazioni. Accetta query in linguaggio naturale e
filtri strutturati facoltativi. OpenClaw abilita lo strumento `x_search` integrato di xAI
...
<Note>
  ...
  e recupero thread. Per statistiche di engagement per singolo post come repost,
  risposte, segnalibri o visualizzazioni, preferisci una ricerca mirata per l'URL esatto del post
  o l'ID dello status. Le ricerche ampie per parole chiave possono trovare il post giusto ma restituire metadati
  per singolo post meno completi. Un buon modello è: individua prima il post, poi
  esegui una seconda query `x_search` concentrata su quel post esatto.
</Note>
...
- [Web Browser](/it/tools/browser) -- automazione completa del browser per siti con uso intenso di JS
