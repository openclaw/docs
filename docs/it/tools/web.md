---
read_when:
    - Si desidera abilitare o configurare web_search
...
Lo strumento `web_search` cerca sul web usando il provider configurato e
...
<Info>
  ...
  siti ricchi di JS o accessi, usa [Browser web](/it/tools/browser). Per
...
<Steps>
  <Step title="Choose a provider">
    Scegli un provider e completa l'eventuale configurazione richiesta. Alcuni provider non
    richiedono chiavi, mentre altri usano chiavi API. Consulta le pagine dei provider qui sotto per
    i dettagli.
  </Step>
  <Step title="Configure">
    ...
    ```
    Questo memorizza il provider e l'eventuale credenziale necessaria. Puoi anche impostare una variabile
    d'ambiente (per esempio `BRAVE_API_KEY`) e saltare questo passaggio per i provider
    basati su API.
  </Step>
  <Step title="Use it">
    Ora l'agente può chiamare `web_search`:
...
<CardGroup cols={2}>
  ...
  <Card title="Firecrawl" icon="flame" href="/it/tools/firecrawl">
    Risultati strutturati. Ideale in combinazione con `firecrawl_search` e `firecrawl_scrape` per estrazione approfondita.
  </Card>
  ...
  <Card title="Kimi" icon="moon" href="/it/tools/kimi-search">
    Risposte sintetizzate dall'IA con citazioni tramite ricerca web Moonshot; i fallback di chat senza grounding falliscono esplicitamente.
  </Card>
  ...
  <Card title="Tavily" icon="globe" href="/it/tools/tavily">
    Risultati strutturati con profondità di ricerca, filtro per argomento e `tavily_extract` per estrazione da URL.
  </Card>
</CardGroup>
...
| [Ollama Web Search](/it/tools/ollama-search) | Snippet strutturati                                          | --                                               | Nessuna per host locali con accesso effettuato; `OLLAMA_API_KEY` per ricerca diretta su `https://ollama.com` |
...
- Il `web_search` gestito continua ad applicarsi ai modelli non Codex
...
Se la ricerca nativa Codex è abilitata ma il modello corrente non è compatibile con Codex, OpenClaw mantiene il normale comportamento `web_search` gestito.

## Sicurezza della rete

Le chiamate al provider `web_search` gestito usano il percorso fetch protetto di OpenClaw. Per
host API provider attendibili, OpenClaw consente risposte DNS fake-IP di Surge,
Clash e sing-box in `198.18.0.0/15` e `fc00::/7` solo per quel nome host del provider.
Altre destinazioni private, local loopback, link-local e metadati restano bloccate.
...
8. **Exa** -- `EXA_API_KEY` o `plugins.entries.exa.config.webSearch.apiKey`; l'opzione `plugins.entries.exa.config.webSearch.baseUrl` sovrascrive l'endpoint Exa (ordine 65)
...
11. **Ollama Web Search** -- fallback senza chiave tramite il tuo host Ollama locale configurato quando è raggiungibile e ha l'accesso effettuato con `ollama signin`; può riutilizzare l'autenticazione bearer del provider Ollama quando l'host la richiede, e può chiamare la ricerca diretta `https://ollama.com` quando configurato con `OLLAMA_API_KEY` (ordine 110)
12. **SearXNG** -- `SEARXNG_BASE_URL` o `plugins.entries.searxng.config.webSearch.baseUrl` (ordine 200)

Se non viene rilevato alcun provider, usa Brave come fallback (riceverai un errore di chiave
mancante che ti chiederà di configurarne una).
...
<Note>
  ...
  venga selezionato tramite rilevamento automatico. In modalità di rilevamento automatico, OpenClaw risolve solo la
...
a priorità inferiore dopo la sua configurazione dedicata per la ricerca web e `GEMINI_API_KEY`. Consulta le
pagine dei provider per gli esempi.

`tools.web.search.provider` viene validato rispetto agli id dei provider di ricerca web
dichiarati dai manifest dei plugin in bundle e installati. Un errore di battitura come `"brvae"`
fa fallire la validazione della configurazione invece di eseguire silenziosamente il fallback al rilevamento automatico. Se un
provider configurato ha solo prove di plugin obsolete, come un blocco
...
- `web_fetch` non in sandbox può usare provider di plugin installati che dichiarano
  `contracts.webFetchProviders`; i recuperi in sandbox restano solo in bundle
- oggi il provider web-fetch in bundle è Firecrawl, configurato in
...
<Warning>
  ...
  rifiuta `ui_lang`; anche `date_before` richiede `date_after` perché gli intervalli
  di freshness personalizzati di Brave richiedono sia la data di inizio sia quella di fine.
  Gemini, Grok e Kimi restituiscono una singola risposta sintetizzata con citazioni. Accettano
  `count` per compatibilità con lo strumento condiviso, ma non modifica la forma della
  risposta fondata. Gemini supporta `freshness`, `date_after` e
  ...
  SearXNG accetta `http://` solo per host privati affidabili o local loopback;
...
filtri strutturati facoltativi. OpenClaw abilita lo strumento integrato `x_search` di xAI
...
<Note>
  ...
  e recupero dei thread. Per statistiche di engagement per singolo post, come ripubblicazioni,
  risposte, segnalibri o visualizzazioni, preferisci una ricerca mirata dell'URL esatto del post
  o dell'ID di stato. Le ricerche ampie per parole chiave possono trovare il post corretto ma restituire metadati
...
`x_search` invia i post a `<baseUrl>/responses` quando
`plugins.entries.xai.config.xSearch.baseUrl` è impostato. Se quel campo viene omesso,
usa come fallback `plugins.entries.xai.config.webSearch.baseUrl`, poi il
legacy `tools.web.search.grok.baseUrl` e infine l'endpoint pubblico di xAI.
...
- [Web Browser](/it/tools/browser) -- automazione completa del browser per siti con molto JS
