...
read_when:
    ...
    - Vuoi i modelli di visione Ollama per la comprensione delle immagini
...
l'uso diretto di Ollama Cloud. Usa riferimenti come `ollama-cloud/kimi-k2.5:cloud` quando
vuoi routing solo cloud senza condividere l'id provider `ollama` locale.

Per la pagina di configurazione dedicata solo cloud, consulta [Ollama Cloud](/it/providers/ollama-cloud).

<Warning>
**Utenti Ollama remoti**: non usare l'URL compatibile con OpenAI `/v1` (`http://host:11434/v1`) con OpenClaw. Questo interrompe le chiamate agli strumenti e i modelli potrebbero emettere JSON grezzo degli strumenti come testo normale. Usa invece l'URL dell'API nativa di Ollama: `baseUrl: "http://host:11434"` (senza `/v1`).
</Warning>

La configurazione del provider Ollama usa `baseUrl` come chiave canonica. OpenClaw accetta anche `baseURL` per compatibilità con esempi in stile SDK OpenAI, ma le nuove configurazioni dovrebbero preferire `baseUrl`.

## Regole di autenticazione

<AccordionGroup>
  <Accordion title="Host locali e LAN">
    Gli host Ollama locali e LAN non richiedono un vero token bearer. OpenClaw usa il marcatore locale `ollama-local` solo per URL base Ollama di loopback, reti private, `.local` e hostname semplici.
  </Accordion>
  ...
  <Accordion title="Id provider personalizzati">
    Gli id provider personalizzati che impostano `api: "ollama"` seguono le stesse regole. Ad esempio, un provider `ollama-remote` che punta a un host Ollama su LAN privata può usare `apiKey: "ollama-local"` e i sotto-agenti risolveranno quel marcatore tramite l'hook del provider Ollama invece di trattarlo come una credenziale mancante. La ricerca in memoria può anche impostare `agents.defaults.memorySearch.provider` su quell'id provider personalizzato, così gli embedding usano l'endpoint Ollama corrispondente.
  </Accordion>
  <Accordion title="Profili di autenticazione">
    `auth-profiles.json` archivia la credenziale per un id provider. Inserisci le impostazioni dell'endpoint (`baseUrl`, `api`, id modello, header, timeout) in `models.providers.<id>`. I vecchi file di profilo di autenticazione piatti come `{ "ollama-windows": { "apiKey": "ollama-local" } }` non sono un formato runtime; esegui `openclaw doctor --fix` per riscriverli nel profilo di chiave API canonico `ollama-windows:default` con un backup. `baseUrl` in quel file è rumore di compatibilità e dovrebbe essere spostato nella configurazione del provider.
  </Accordion>
  <Accordion title="Ambito degli embedding di memoria">
    ...
    - `agents.*.memorySearch.remote.apiKey` viene inviato solo al suo host remoto di embedding.
    ...
    **Ideale per:** il percorso più rapido verso una configurazione Ollama cloud o locale funzionante.
    ...
    <Steps>
      ...
      <Step title="Scegli la modalità">
        - **Cloud + locale** — host Ollama locale più modelli cloud instradati tramite quell'host
        - **Solo cloud** — modelli Ollama ospitati tramite `https://ollama.com`
        - **Solo locale** — solo modelli locali

      </Step>
      <Step title="Seleziona un modello">
        `Cloud only` richiede `OLLAMA_API_KEY` e suggerisce valori predefiniti cloud ospitati. `Cloud + Local` e `Local only` richiedono un URL base Ollama, rilevano i modelli disponibili e scaricano automaticamente il modello locale selezionato se non è ancora disponibile. Quando Ollama segnala un tag `:latest` installato, come `gemma4:latest`, la configurazione mostra quel modello installato una sola volta invece di mostrare sia `gemma4` sia `gemma4:latest` o di scaricare di nuovo l'alias semplice. `Cloud + Local` controlla anche se quell'host Ollama ha effettuato l'accesso per l'accesso cloud.
      </Step>
    ...
    **Ideale per:** pieno controllo sulla configurazione cloud o locale.
    ...
    <Steps>
      <Step title="Scegli cloud o locale">
        - **Cloud + locale**: installa Ollama, accedi con `ollama signin` e instrada le richieste cloud tramite quell'host
        - **Solo cloud**: usa `https://ollama.com` con una `OLLAMA_API_KEY`
        - **Solo locale**: installa Ollama da [ollama.com/download](https://ollama.com/download)
      ...
      <Step title="Abilita Ollama per OpenClaw">
        Per `Cloud only`, usa la tua vera `OLLAMA_API_KEY`. Per configurazioni basate su host, va bene qualsiasi valore segnaposto:
...
<Tabs>
  ...
  <Tab title="Solo cloud">
    `Cloud only` viene eseguito verso l'API ospitata di Ollama su `https://ollama.com`.

    Usa **Solo cloud** durante la configurazione. OpenClaw richiede `OLLAMA_API_KEY`, imposta `baseUrl: "https://ollama.com"` e inizializza l'elenco dei modelli cloud ospitati. Questo percorso **non** richiede un server Ollama locale né `ollama signin`.
  ...
  <Tab title="Solo locale">
    ...
    Attualmente OpenClaw suggerisce `gemma4` come valore predefinito locale.
...
| Rilevamento capacità | Usa lookup `/api/show` best-effort per leggere `contextWindow`, parametri Modelfile `num_ctx` espansi e capacità incluse visione/strumenti                          |
| Modelli vision       | I modelli con una capacità `vision` riportata da `/api/show` sono marcati come capaci di gestire immagini (`input: ["text", "image"]`), quindi OpenClaw inietta automaticamente le immagini nel prompt |
| Rilevamento reasoning | Usa le capacità di `/api/show` quando disponibili, inclusa `thinking`; ripiega su un'euristica del nome modello (`r1`, `reasoning`, `think`) quando Ollama omette le capacità |
...
riferimento completo `ollama/<model>:cloud`, OpenClaw valida quel preciso modello mancante con
...
Per uno smoke test ristretto di generazione testo che evita l'intera superficie degli strumenti dell'agente,
...
Quel percorso usa comunque il provider, l'autenticazione e il trasporto nativo Ollama
configurati di OpenClaw, ma non avvia un turno chat-agent né carica il contesto MCP/strumenti. Se
questo riesce mentre le risposte normali dell'agente falliscono, verifica poi la capacità del modello
per prompt/strumenti dell'agente.

Per uno smoke test ristretto di un modello vision sullo stesso percorso snello, aggiungi uno o più
...
JPEG e WebP comuni. I file non immagine vengono rifiutati prima della chiamata a Ollama.
Per il riconoscimento vocale, usa invece `openclaw infer audio transcribe`.

Quando passi una conversazione a `/model ollama/<model>`, OpenClaw lo considera
una selezione utente esatta. Se l'`baseUrl` Ollama configurato non è
...
I processi cron isolati eseguono un controllo di sicurezza locale aggiuntivo prima
di avviare il turno dell'agente. Se il modello selezionato si risolve in un
provider Ollama locale, su rete privata o `.local` e `/api/tags` non è
...
stesso daemon Ollama arrestato non avviano tutti richieste modello destinate a fallire.

Verifica dal vivo il percorso di testo locale, il percorso di stream nativo e gli
embedding rispetto a Ollama locale con:
...
Il nuovo modello verrà scoperto automaticamente e sarà disponibile per l'uso.

<Note>
Se imposti `models.providers.ollama` esplicitamente, o configuri un provider remoto personalizzato come `models.providers.ollama-cloud` con `api: "ollama"`, la scoperta automatica viene saltata e devi definire i modelli manualmente. I provider personalizzati loopback come `http://127.0.0.2:11434` vengono comunque trattati come locali. Vedi la sezione di configurazione esplicita qui sotto.
</Note>

## Visione e descrizione delle immagini

Il plugin Ollama incluso registra Ollama come provider di comprensione multimediale con supporto per immagini. Questo consente a OpenClaw di instradare richieste esplicite di descrizione immagini e impostazioni predefinite configurate per modelli immagine tramite modelli di visione Ollama locali o ospitati.
...
Usa `infer image describe` quando vuoi il flusso del provider di comprensione immagini di OpenClaw, `agents.defaults.imageModel` configurato e la forma dell'output di descrizione immagini. Usa `infer model run --file` quando vuoi una verifica grezza di un modello multimodale con un prompt personalizzato e una o più immagini.

Per rendere Ollama il modello di comprensione immagini predefinito per i media in ingresso, configura `agents.defaults.imageModel`:
...
Preferisci il riferimento completo `ollama/<model>`. Se lo stesso modello è elencato sotto `models.providers.ollama.models` con `input: ["text", "image"]` e nessun altro provider di immagini configurato espone quell'ID modello semplice, OpenClaw normalizza anche un riferimento `imageModel` semplice come `qwen2.5vl:7b` in `ollama/qwen2.5vl:7b`. Se più di un provider di immagini configurato ha lo stesso ID semplice, usa esplicitamente il prefisso del provider.

I modelli di visione locali lenti possono richiedere un timeout di comprensione immagini più lungo rispetto ai modelli cloud. Possono anche andare in crash o arrestarsi quando Ollama tenta di allocare l'intero contesto di visione pubblicizzato su hardware limitato. Imposta un timeout di capacità e limita `num_ctx` sulla voce del modello quando ti serve solo un normale turno di descrizione immagine:
...
Questo timeout si applica alla comprensione immagini in ingresso e allo strumento esplicito `image` che l'agente può chiamare durante un turno. `models.providers.ollama.timeoutSeconds` a livello di provider controlla comunque la protezione della richiesta HTTP Ollama sottostante per le normali chiamate modello.
...
OpenClaw rifiuta le richieste di descrizione immagini per modelli che non sono contrassegnati come compatibili con le immagini. Con la scoperta implicita, OpenClaw legge queste informazioni da Ollama quando `/api/show` segnala una capacità di visione.
...
<Tabs>
  <Tab title="Basic (implicit discovery)">
    ...
    <Tip>
    Se `OLLAMA_API_KEY` è impostata, puoi omettere `apiKey` nella voce del provider e OpenClaw la inserirà per i controlli di disponibilità.
    </Tip>

  </Tab>

  <Tab title="Explicit (manual models)">
    Usa la configurazione esplicita quando vuoi configurare il cloud ospitato, Ollama gira su un altro host/porta, vuoi forzare finestre di contesto o liste di modelli specifiche, oppure vuoi definizioni di modello completamente manuali.
  ...
  <Tab title="Custom base URL">
    Se Ollama è in esecuzione su un host o una porta diversi (la configurazione esplicita disabilita la scoperta automatica, quindi definisci i modelli manualmente):
    ...
    <Warning>
    Non aggiungere `/v1` all'URL. Il percorso `/v1` usa la modalità compatibile con OpenAI, in cui la chiamata agli strumenti non è affidabile. Usa l'URL base di Ollama senza suffisso di percorso.
    </Warning>
...
Usale come punti di partenza e sostituisci gli ID dei modelli con i nomi esatti da `ollama list` o `openclaw models list --provider ollama`.

<AccordionGroup>
  <Accordion title="Local model with auto-discovery">
    Usa questo quando Ollama gira sulla stessa macchina del Gateway e vuoi che OpenClaw scopra automaticamente i modelli installati.
  ...
  <Accordion title="LAN Ollama host with manual models">
    Usa URL Ollama nativi per host LAN. Non aggiungere `/v1`.
  ...
  <Accordion title="Ollama Cloud only">
    Usa questo quando non esegui un daemon locale e vuoi usare direttamente modelli Ollama ospitati.
  ...
  <Accordion title="Cloud plus local through a signed-in daemon">
    Usa questo quando un daemon Ollama locale o LAN ha eseguito l'accesso con `ollama signin` e deve servire sia modelli locali sia modelli `:cloud`.
  ...
  <Accordion title="Lean local model profile">
    ...
    `localModelLean` rimuove gli strumenti browser, cron e messaggi dalla superficie diretta dell'agente e colloca per impostazione predefinita i cataloghi più grandi dietro controlli strutturati di Tool Search, salvo quando un'esecuzione deve mantenere la semantica di consegna diretta dei messaggi, ma non modifica il contesto di runtime o la modalità di pensiero di Ollama. Abbinalo a `params.num_ctx` esplicito e `params.thinking: false` per piccoli modelli di pensiero in stile Qwen che entrano in loop o consumano il budget di risposta nel ragionamento nascosto.
...
Sono supportati anche ID provider Ollama personalizzati. Quando un riferimento di modello usa il prefisso del
provider attivo, ad esempio `ollama-spark/qwen3:32b`, OpenClaw rimuove solo quel
prefisso prima di chiamare Ollama, così il server riceve `qwen3:32b`.

Per modelli locali lenti, preferisci la regolazione delle richieste con ambito provider prima di aumentare il
...
header, streaming del body e abort totale del fetch protetto. `params.keep_alive`
...
OpenClaw supporta **Ollama Web Search** come provider `web_search` in bundle.

| Proprietà   | Dettaglio                                                                                                                                                            |
| ----------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Host        | Usa il tuo host Ollama configurato (`models.providers.ollama.baseUrl` quando impostato, altrimenti `http://127.0.0.1:11434`); `https://ollama.com` usa direttamente l'API in hosting |
| Autenticazione | Senza chiave per host Ollama locali con accesso effettuato; `OLLAMA_API_KEY` o autenticazione provider configurata per la ricerca diretta su `https://ollama.com` o host protetti da autenticazione |
| Requisito   | Gli host locali/self-hosted devono essere in esecuzione e con accesso effettuato tramite `ollama signin`; la ricerca diretta in hosting richiede `baseUrl: "https://ollama.com"` più una vera chiave API Ollama |
...
Per la ricerca diretta in hosting tramite Ollama Cloud:
...
Per un daemon locale con accesso effettuato, OpenClaw usa il proxy `/api/experimental/web_search` del daemon. Per `https://ollama.com`, chiama direttamente l'endpoint `/api/web_search` in hosting.

  <Note>
  Per la configurazione completa e i dettagli sul comportamento, consulta [Ricerca web di Ollama](/it/tools/ollama-search).
  </Note>

  ## Configurazione avanzata

  <AccordionGroup>
  <Accordion title="Modalità precedente compatibile con OpenAI">
    <Warning>
  ...
  <Accordion title="Finestre di contesto">
    Per i modelli rilevati automaticamente, OpenClaw usa la finestra di contesto riportata da Ollama quando disponibile, inclusi i valori `PARAMETER num_ctx` più grandi provenienti da Modelfile personalizzati. In caso contrario, torna alla finestra di contesto predefinita di Ollama usata da OpenClaw.

    Puoi impostare valori predefiniti a livello di provider per `contextWindow`, `contextTokens` e `maxTokens` per ogni modello sotto quel provider Ollama, quindi sovrascriverli per singolo modello quando necessario. `contextWindow` è il budget di prompt e Compaction di OpenClaw. Le richieste native Ollama lasciano `options.num_ctx` non impostato a meno che tu non configuri esplicitamente `params.num_ctx`, così Ollama può applicare il proprio valore predefinito basato su modello, `OLLAMA_CONTEXT_LENGTH` o VRAM. Per limitare o forzare il contesto di runtime per richiesta di Ollama senza ricostruire un Modelfile, imposta `params.num_ctx`; i valori non validi, zero, negativi e non finiti vengono ignorati. Se hai aggiornato una configurazione precedente che usava solo `contextWindow` o `maxTokens` per forzare un contesto di richiesta Ollama nativo, esegui `openclaw doctor --fix` per copiare quei budget espliciti del provider o del modello in `params.num_ctx`. L'adattatore Ollama compatibile con OpenAI inserisce ancora `options.num_ctx` per impostazione predefinita da `params.num_ctx` o `contextWindow` configurati; disabilitalo con `injectNumCtxForOpenAICompat: false` se il tuo upstream rifiuta `options`.

    Le voci dei modelli Ollama nativi accettano anche le opzioni di runtime comuni di Ollama sotto `params`, incluse `temperature`, `top_p`, `top_k`, `min_p`, `num_predict`, `stop`, `repeat_penalty`, `num_batch`, `num_thread` e `use_mmap`. OpenClaw inoltra solo le chiavi di richiesta Ollama, quindi i parametri di runtime OpenClaw come `streaming` non vengono passati a Ollama. Usa `params.think` o `params.thinking` per inviare `think` Ollama di primo livello; `false` disabilita il ragionamento a livello API per i modelli di ragionamento in stile Qwen.
    ...
    Anche `agents.defaults.models["ollama/<model>"].params.num_ctx` funziona. Se entrambi sono configurati, la voce esplicita del modello del provider prevale sul valore predefinito dell'agente.

  </Accordion>

  <Accordion title="Controllo del ragionamento">
    Per i modelli Ollama nativi, OpenClaw inoltra il controllo del ragionamento come previsto da Ollama: `think` di primo livello, non `options.think`. I modelli rilevati automaticamente la cui risposta `/api/show` include la capacità `thinking` espongono `/think low`, `/think medium`, `/think high` e `/think max`; i modelli senza ragionamento espongono solo `/think off`.
    ...
    `params.think` o `params.thinking` per singolo modello possono disabilitare o forzare il ragionamento dell'API Ollama per uno specifico modello configurato. OpenClaw preserva quei parametri espliciti del modello quando l'esecuzione attiva ha solo il valore predefinito implicito `off`; i comandi runtime diversi da off, come `/think medium`, sovrascrivono comunque l'esecuzione attiva.
  ...
  <Accordion title="Embedding della memoria">
    ...
    e la chiave API, chiama l'endpoint `/api/embed` corrente di Ollama e raggruppa
    più blocchi di memoria in una richiesta `input` quando possibile.

    Quando `proxy.enabled=true`, le richieste di embedding della memoria Ollama verso l'origine
    host-local loopback esatta derivata dal `baseUrl` configurato usano
    il percorso diretto protetto di OpenClaw invece del proxy inoltrato gestito. Il
    nome host configurato deve essere esso stesso `localhost` o un literal IP di loopback;
    i nomi DNS che si limitano a risolversi in loopback usano comunque il percorso del proxy gestito.
    Anche gli host Ollama su LAN, tailnet, rete privata e pubblici restano sul
    percorso del proxy gestito. I reindirizzamenti verso un altro host o un'altra porta non ereditano la fiducia.
    Gli operatori possono ancora impostare l'opzione globale `proxy.loopbackMode: "proxy"` per
    inviare il traffico di loopback tramite il proxy, oppure `proxy.loopbackMode: "block"`
    per negare le connessioni di loopback prima di aprire una connessione; vedi
    ...
    Gli embedding al momento della query usano prefissi di recupero per i modelli che li richiedono o li consigliano, inclusi `nomic-embed-text`, `qwen3-embedding` e `mxbai-embed-large`. I batch dei documenti di memoria restano grezzi, così gli indici esistenti non richiedono una migrazione di formato.
  ...
  <Accordion title="Configurazione dello streaming">
    L'integrazione Ollama di OpenClaw usa per impostazione predefinita la **API Ollama nativa** (`/api/chat`), che supporta pienamente streaming e chiamata di strumenti contemporaneamente. Non è necessaria alcuna configurazione speciale.

    Per le richieste native `/api/chat`, OpenClaw inoltra anche il controllo del ragionamento direttamente a Ollama: `/think off` e `openclaw agent --thinking off` inviano `think: false` al livello superiore, a meno che non sia configurato un valore esplicito del modello `params.think`/`params.thinking`, mentre `/think low|medium|high` inviano la stringa di impegno `think` corrispondente al livello superiore. `/think max` corrisponde all'impegno nativo massimo di Ollama, `think: "high"`.
...
<AccordionGroup>
  <Accordion title="Ciclo di arresti anomali WSL2 (riavvii ripetuti)">
    Su WSL2 con NVIDIA/CUDA, l'installer Linux ufficiale di Ollama crea un'unità systemd `ollama.service` con `Restart=always`. Se quel servizio si avvia automaticamente e carica un modello basato su GPU durante l'avvio di WSL2, Ollama può bloccare la memoria dell'host mentre il modello viene caricato. Il recupero della memoria di Hyper-V non riesce sempre a recuperare quelle pagine bloccate, quindi Windows può terminare la VM WSL2, systemd avvia di nuovo Ollama e il ciclo si ripete.
  ...
  <Accordion title="L'host remoto funziona con curl ma non con OpenClaw">
    ...
    - Il modello è presente sul demone del tuo laptop ma non sul demone remoto.

  </Accordion>

  <Accordion title="Il modello restituisce JSON degli strumenti come testo">
    Questo di solito significa che il provider sta usando la modalità compatibile con OpenAI o che il modello non è in grado di gestire gli schemi degli strumenti.
  ...
  <Accordion title="Kimi o GLM restituisce simboli illeggibili">
    Le risposte Kimi/GLM ospitate che consistono in lunghe sequenze di simboli non linguistici vengono trattate come output del provider non riuscito invece che come risposta riuscita dell'assistente. Questo consente ai normali meccanismi di nuovo tentativo, fallback o gestione degli errori di prendere il controllo senza salvare il testo corrotto nella sessione.
  ...
  <Accordion title="Il modello locale freddo va in timeout">
    I modelli locali di grandi dimensioni possono richiedere un primo caricamento lungo prima che inizi lo streaming. Mantieni il timeout limitato al provider Ollama e, facoltativamente, chiedi a Ollama di mantenere il modello caricato tra un turno e l'altro:
  ...
  <Accordion title="Il modello a contesto ampio è troppo lento o esaurisce la memoria">
    Molti modelli Ollama dichiarano contesti più grandi di quanto il tuo hardware possa eseguire comodamente. Ollama nativo usa il contesto di runtime predefinito di Ollama a meno che tu non imposti `params.num_ctx`. Limita sia il budget di OpenClaw sia il contesto di richiesta di Ollama quando vuoi una latenza prevedibile fino al primo token:
    ...
    Riduci prima `contextWindow` se OpenClaw sta inviando troppo prompt. Riduci `params.num_ctx` se Ollama sta caricando un contesto di runtime troppo grande per la macchina. Riduci `maxTokens` se la generazione dura troppo a lungo.
...
<CardGroup cols={2}>
  ...
  <Card title="Ricerca web Ollama" href="/it/tools/ollama-search" icon="magnifying-glass">
    Configurazione completa e dettagli sul comportamento per la ricerca web basata su Ollama.
  </Card>
