...
title: OpenRouter
x-i18n:
    generated_at: "2026-06-27T18:09:11Z"
    model: gpt-5.5
  ...
  <Tab title="OAuth">
    <Steps>
      <Step title="Esegui l'onboarding OAuth">
        ...
        OpenClaw apre il flusso di accesso nel browser di OpenRouter, scambia il codice
        PKCE con una chiave API OpenRouter e archivia quella chiave nel profilo di autenticazione
        OpenRouter predefinito. Sugli host remoti/headless, OpenClaw stampa l'URL di
        accesso e chiede di incollare l'URL di reindirizzamento dopo l'accesso.
      </Step>
      <Step title="(Facoltativo) Passa a un modello specifico">
        L'onboarding usa come impostazione predefinita `openrouter/auto`. Scegli in seguito un modello concreto:
  ...
  <Tab title="Chiave API">
    <Steps>
      <Step title="Ottieni la tua chiave API">
        Crea una chiave API in [openrouter.ai/keys](https://openrouter.ai/keys).
      </Step>
...
<Note>
I riferimenti dei modelli seguono lo schema `openrouter/<provider>/<model>`. Per l'elenco completo dei
...
Per impostazione predefinita, le immagini di riferimento vengono inviate come immagini del primo/ultimo fotogramma; le immagini
...
di generazione video attualmente accetta riferimenti testuali e immagini.

## Generazione musicale

OpenRouter può anche supportare lo strumento `music_generate` tramite l'output
audio dei completamenti chat. Usa un modello audio OpenRouter in
...
Il provider musicale OpenRouter incluso usa come impostazione predefinita
`google/lyria-3-pro-preview` ed espone anche
`google/lyria-3-clip-preview`. OpenClaw invia `modalities: ["text",
"audio"]`, abilita lo streaming, raccoglie i blocchi audio in streaming e salva
il risultato come contenuto multimediale generato per la consegna al canale. Le immagini di riferimento sono
...
## Speech-to-text (audio in ingresso)
...
Questo si applica a qualsiasi Plugin di canale che inoltri voce/audio in ingresso nel
...
Usa OpenRouter Fusion quando vuoi che un unico riferimento modello OpenClaw interroghi diversi
modelli OpenRouter in parallelo, faccia valutare a OpenRouter le loro risposte e restituisca una
...
sia il prefisso provider OpenClaw sia lo spazio dei nomi upstream OpenRouter:
...
sanificata al prompt di sistema con i modelli di analisi configurati e il modello giudice, così
...
di solito più alta rispetto a una richiesta diretta a singolo modello. Usa Fusion per risposte
ponderate e di alta qualità o per percorsi di escalation, non come impostazione predefinita per
...
vuoi incollare una chiave che hai creato manualmente su OpenRouter.
...
<Warning>
Se punti il provider OpenRouter a un altro proxy o URL base, OpenClaw
...
<AccordionGroup>
  <Accordion title="Cache delle risposte">
    La cache delle risposte OpenRouter è opt-in. Abilitala per ogni modello OpenRouter con
    ...
    `openrouter.ai` verificate, non su URL base di proxy personalizzati.
  ...
  <Accordion title="Prefill del ragionamento Anthropic">
    ...
    rispettando il requisito di Anthropic secondo cui le conversazioni con ragionamento terminano con un turno
    dell'utente.
  </Accordion>
  ...
  <Accordion title="Iniezione di pensiero / ragionamento">
    ...
    `openrouter/auto` saltano questa iniezione di ragionamento. Hunter Alpha salta anche il
    ragionamento proxy per riferimenti a modelli configurati obsoleti perché OpenRouter potrebbe
    restituire il testo della risposta finale nei campi di ragionamento per quella route ritirata.
  </Accordion>
  ...
  <Accordion title="Replay del ragionamento DeepSeek V4">
    ...
    turni assistente riprodotti, così le conversazioni di pensiero/strumenti mantengono la forma
    di follow-up richiesta da DeepSeek V4. OpenClaw invia i valori
    `reasoning_effort` supportati da OpenRouter per queste route; `xhigh` è il livello più alto
    pubblicizzato e gli override `max` obsoleti vengono mappati a `xhigh`.
  </Accordion>
  ...
  <Accordion title="Metadati di routing del provider">
    OpenRouter supporta un oggetto di richiesta `provider` per il routing del provider
    sottostante. Configura una policy predefinita per tutte le richieste di modelli di testo OpenRouter
    ...
    OpenClaw inoltra quell'oggetto a OpenRouter come payload `provider`
    della richiesta. Usa i campi snake_case documentati da OpenRouter, inclusi `sort`,
    `only`, `ignore`, `order`, `allow_fallbacks`, `require_parameters`,
    `data_collection`, `quantizations`, `max_price`, `preferred_max_latency`,
    `preferred_min_throughput`, `zdr` ed `enforce_distillable_text`.

    I parametri per modello continuano a sovrascrivere l'oggetto di routing valido per tutto il provider:
    ...
    Questo si applica solo sulle route chat-completions di OpenRouter. Le route dirette Anthropic,
    Google, OpenAI o di provider personalizzati ignorano i parametri di routing OpenRouter.
