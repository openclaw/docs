...
x-i18n:
    generated_at: "2026-07-24T03:59:34Z"
    model: gpt-5.6
...
Fable 5 verwendet immer adaptives Denken und standardmäßig die Aufwandsstufe `high`. `/think off` und
`/think minimal` verwenden die Aufwandsstufe `low`, da das Modell das Deaktivieren des Denkens nicht unterstützt.

## Claude Sonnet 5

Verwenden Sie `anthropic-vertex/claude-sonnet-5` mit dem `global`-, `us`- oder `eu`-
Endpunkt von Vertex. Sonnet 5 verwendet standardmäßig adaptives Denken mit der Aufwandsstufe `high` und unterstützt
`/think off` oder die nativen `/think xhigh|max`-Stufen. OpenClaw veröffentlicht sein
...
Million Eingabe-/Ausgabe-Token und ab dem
1\. September `$3/$15`. Für die regionsübergreifenden Endpunkte `us` und `eu` gilt der von Vertex dokumentierte
Aufschlag von 10 %.
