---
read_when:
    - Sie installieren, konfigurieren oder prüfen das lmstudio-Plugin.
summary: Fügt Unterstützung für den LM-Studio-Modell-Provider zu OpenClaw hinzu.
title: LM-Studio-Plugin
x-i18n:
    generated_at: "2026-07-16T13:22:31Z"
    model: gpt-5.6
    postprocess_version: locale-links-v1
    prompt_version: 32
    provider: openai
    source_hash: 8fc13cb8579875fab911aaf456f1b632426761b173cc06a8a7f60698e96cfb16
    source_path: plugins/reference/lmstudio.md
    workflow: 16
---

# LM Studio-Plugin

Fügt OpenClaw Unterstützung für den LM Studio-Modell-Provider hinzu.

## Distribution

- Paket: `@openclaw/lmstudio-provider`
- Installationsweg: in OpenClaw enthalten

## Oberfläche

Provider: `lmstudio`; Verträge: `memoryEmbeddingProviders`

## Zugehörige Dokumentation

- [lmstudio](/de/providers/lmstudio)
