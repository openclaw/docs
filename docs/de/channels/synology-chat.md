...
title: Synology Chat
x-i18n:
    generated_at: "2026-07-12T15:01:04Z"
    model: gpt-5.6
...
- Nutzdaten können `application/x-www-form-urlencoded` oder `application/json` verwenden; `token`, `user_id` und `text` sind erforderlich.
...
`SYNOLOGY_CHAT_INCOMING_URL` und `SYNOLOGY_NAS_HOST` können nicht über eine `.env`-Datei des Workspace festgelegt werden; siehe [Workspace-`.env`-Dateien](/de/gateway/security#workspace-env-files).

## Richtlinie und Zugriffskontrolle für Direktnachrichten

- Unterstützte `dmPolicy`-Werte: `allowlist` (Standard), `open` und `disabled`. Synology Chat verfügt über keinen Kopplungsablauf; genehmigen Sie Absender, indem Sie deren numerische Synology-Benutzer-IDs zu `allowedUserIds` hinzufügen.
...
- `dmPolicy: "open"` erlaubt öffentliche Direktnachrichten nur, wenn `allowedUserIds` `"*"` enthält; bei einschränkenden Einträgen können nur übereinstimmende Benutzer chatten. Bei `open` mit einer leeren `allowedUserIds`-Liste wird der Start der Route ebenfalls verweigert.
- `dmPolicy: "disabled"` blockiert Direktnachrichten.
- Die Bindung des Antwortempfängers erfolgt standardmäßig anhand der stabilen numerischen `user_id`. `channels.synology-chat.dangerouslyAllowNameMatching: true` ist ein Notfall-Kompatibilitätsmodus, der die Suche anhand veränderlicher Benutzernamen/Spitznamen für die Zustellung von Antworten wieder aktiviert.
...
Ausgehender Text wird bei 2000 Zeichen aufgeteilt. Das Senden von Medien wird über eine URL-basierte Dateizustellung unterstützt: Das NAS lädt die Datei herunter und hängt sie an (max. 32 MB). URLs ausgehender Dateien müssen `http` oder `https` verwenden, und private oder anderweitig blockierte Netzwerkziele werden abgelehnt, bevor OpenClaw die URL an den NAS-Webhook weiterleitet.
...
Direktnachrichtensitzungen werden nach Konto und Benutzer isoliert, sodass dieselbe numerische `user_id`
bei zwei verschiedenen Synology-Konten keinen gemeinsamen Transkriptstatus verwendet.
...
Wenn Sie für ein benanntes Konto absichtlich die veraltete Vererbung benötigen, legen Sie
`dangerouslyAllowInheritedWebhookPath: true` für dieses Konto oder unter `channels.synology-chat` fest;
exakt identische Pfade werden jedoch weiterhin sicher abgelehnt. Bevorzugen Sie explizite Pfade pro Konto.
...
- Halten Sie `token` geheim und wechseln Sie es, falls es offengelegt wurde.
...
- Eingehende Webhook-Anfragen werden anhand des Tokens verifiziert und pro Absender ratenbegrenzt (`rateLimitPerMinute`, Standardwert 30).
- Prüfungen auf ungültige Token verwenden einen zeitkonstanten Vergleich von Geheimnissen und lehnen sicher ab; wiederholte Versuche mit ungültigen Token sperren die Quell-IP vorübergehend.
- Der Text eingehender Nachrichten wird gegen bekannte Prompt-Injection-Muster bereinigt und bei 4000 Zeichen abgeschnitten.
- Bevorzugen Sie für den Produktionseinsatz `dmPolicy: "allowlist"`.
- Lassen Sie `dangerouslyAllowNameMatching` deaktiviert, sofern Sie nicht ausdrücklich die veraltete, benutzernamenbasierte Zustellung von Antworten benötigen.
- Lassen Sie `dangerouslyAllowInheritedWebhookPath` deaktiviert, sofern Sie nicht ausdrücklich das Risiko einer Weiterleitung über gemeinsam genutzte Pfade in einer Konfiguration mit mehreren Konten akzeptieren.
...
- `Invalid token`:
  ...
  - die Anfrage trifft auf das falsche Konto oder den falschen Webhook-Pfad
