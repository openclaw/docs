...
x-i18n:
    generated_at: "2026-05-07T13:13:06Z"
    model: gpt-5.5
    provider: openai
...
Verwenden Sie das unveränderte Paket, um dem aktuellen offiziellen Release-Tag zu folgen. Pinnen Sie eine exakte Version nur, wenn Sie eine reproduzierbare Installation benötigen.
...
Die [`@microsoft/teams.cli`](https://www.npmjs.com/package/@microsoft/teams.cli) übernimmt Bot-Registrierung, Manifest-Erstellung und Generierung von Zugangsdaten in einem einzigen Befehl.
...
`teams app create` fordert Sie auf, die App zu installieren - wählen Sie „In Teams installieren“. Wenn Sie dies übersprungen haben, können Sie den Link später abrufen:
...
<Note>
Gruppenchats sind standardmäßig blockiert (`channels.msteams.groupPolicy: "allowlist"`). Um Gruppenantworten zu erlauben, setzen Sie `channels.msteams.groupAllowFrom` oder verwenden Sie `groupPolicy: "open"`, um jedes Mitglied zu erlauben (durch Erwähnungen gesteuert).
</Note>
...
- Verlassen Sie sich bei Allowlists nicht auf UPN-/Anzeigenamen-Abgleich - diese können sich ändern. OpenClaw deaktiviert direkten Namensabgleich standardmäßig; aktivieren Sie ihn ausdrücklich mit `channels.msteams.dangerouslyAllowNameMatching: true`.
- Der Assistent kann Namen über Microsoft Graph in IDs auflösen, wenn die Zugangsdaten dies erlauben.

**Gruppenzugriff**

- Standard: `channels.msteams.groupPolicy = "allowlist"` (blockiert, sofern Sie `groupAllowFrom` nicht hinzufügen). Verwenden Sie `channels.defaults.groupPolicy`, um den Standard zu überschreiben, wenn nichts gesetzt ist.
...
- Schlüssel sollten stabile Teams-Konversations-IDs aus Teams-Links verwenden, keine veränderlichen Anzeigenamen.
- Wenn `groupPolicy="allowlist"` gesetzt ist und eine Teams-Allowlist vorhanden ist, werden nur aufgelistete Teams/Kanäle akzeptiert (durch Erwähnungen gesteuert).
...
6. Der Gateway lauscht standardmäßig auf Bot-Framework-Webhook-Traffic unter `/api/messages`.
...
2. Füllen Sie den Tab **Grundlagen** aus:
   ...
   | **Preisstufe**     | **Kostenlos** für Entwicklung/Tests                      |
   | **App-Typ**        | **Einzelner Mandant** (empfohlen - siehe Hinweis unten)  |
   | **Erstellungstyp** | **Neue Microsoft App ID erstellen**                      |

<Warning>
Die Erstellung neuer Multi-Tenant-Bots wurde nach dem 31.07.2025 eingestellt. Verwenden Sie **Einzelner Mandant** für neue Bots.
</Warning>
...
1. Öffnen Sie Ihre Azure-Bot-Ressource → **Konfiguration**
2. Kopieren Sie die **Microsoft App ID** → dies ist Ihre `appId`
3. Klicken Sie auf **Kennwort verwalten** → wechseln Sie zur App-Registrierung
4. Unter **Zertifikate & Secrets** → **Neues Client-Secret** → kopieren Sie den **Wert** → dies ist Ihr `appPassword`
5. Öffnen Sie **Übersicht** → kopieren Sie die **Verzeichnis-ID (Mandant)** → dies ist Ihre `tenantId`
...
Verwenden Sie ein PEM-Zertifikat, das mit Ihrer Entra ID-App-Registrierung registriert ist.
...
1. Der Bot-Pod/die VM hat eine verwaltete Identität (systemseitig zugewiesen oder benutzerseitig zugewiesen).
...
4. Das Token wird für die Bot-Authentifizierung an das Teams SDK übergeben.
...
**Konfiguration (benutzerseitig zugewiesene Managed Identity):**
...
2. **Erstellen Sie eine Verbundidentitäts-Anmeldeinformation** in der Entra ID-App-Registrierung:
...
4. **Kennzeichnen Sie den Pod** für Workload-Identity-Injektion:
...
| **Zertifikat**       | `authType: "federated"` + `certificatePath`    | Kein gemeinsames Geheimnis über Netzwerk | Verwaltungsaufwand für Zertifikate             |
| **Managed Identity** | `authType: "federated"` + `useManagedIdentity` | Passwortlos, keine Geheimnisse zu verwalten | Azure-Infrastruktur erforderlich            |
...
Teams kann `localhost` nicht erreichen. Verwenden Sie einen dauerhaften Dev-Tunnel, damit Ihre URL über Sitzungen hinweg gleich bleibt:
...
Alternativen: `ngrok http 3978` oder `tailscale funnel 3978` (URLs können sich je Sitzung ändern).
...
- `MSTEAMS_CERTIFICATE_THUMBPRINT` (optional, für Auth nicht erforderlich)
...
- Für teamübergreifende Abfragen: `User.Read.All`-Graph-Anwendungsberechtigung mit Administratorzustimmung
...
- Abgerufener Thread-Verlauf wird nach Sender-Zulassungslisten (`allowFrom` / `groupAllowFrom`) gefiltert, sodass die Initialisierung des Thread-Kontexts nur Nachrichten von erlaubten Sendern enthält.
- Kontext aus zitierten Anhängen (`ReplyTo*`, aus Teams-Antwort-HTML abgeleitet) wird derzeit unverändert weitergegeben.
- Anders gesagt: Zulassungslisten steuern, wer den Agenten auslösen kann; nur bestimmte ergänzende Kontextpfade werden derzeit gefiltert.
...
- `bots[].supportsFiles: true` ist für Dateiverarbeitung im persönlichen Scope erforderlich.
...
3. **Zippen Sie** das Manifest mit Symbolen erneut (`manifest.json`, `outline.png`, `color.png`)
...
**Kurz gesagt:** RSC ist für Echtzeit-Listening; Graph API ist für historischen Zugriff. Um verpasste Nachrichten nachzuholen, während Sie offline waren, benötigen Sie Graph API mit `ChannelMessage.Read.All` (erfordert Administratorzustimmung).
...
**Zusätzliche Berechtigung für Benutzererwähnungen:** Benutzer-@mentions funktionieren für Benutzer in der Unterhaltung sofort. Wenn Sie jedoch Benutzer dynamisch suchen und erwähnen möchten, die **nicht in der aktuellen Unterhaltung** sind, fügen Sie die Berechtigung `User.Read.All` (Application) hinzu und erteilen Sie Administratorzustimmung.

## Bekannte Einschränkungen

### Webhook-Timeouts

Teams liefert Nachrichten per HTTP-Webhook aus. Wenn die Verarbeitung zu lange dauert (z. B. langsame LLM-Antworten), können Sie Folgendes sehen:
...
- Grundlegende Formatierung funktioniert: **Fettdruck**, _Kursivschrift_, `code`, Links
...
- `channels.msteams.dangerouslyAllowNameMatching`: Notfall-Umschalter, um veränderliches Matching von UPN/Anzeigenamen und direkte Team-/Kanalnamen-Routing wieder zu aktivieren.
- `channels.msteams.textChunkLimit`: Chunk-Größe für ausgehenden Text.
- `channels.msteams.chunkMode`: `length` (Standard) oder `newline`, um vor dem Aufteilen nach Länge an Leerzeilen (Absatzgrenzen) zu teilen.
- `channels.msteams.mediaAllowHosts`: Zulassungsliste für Hosts eingehender Anhänge (standardmäßig Microsoft-/Teams-Domains).
- `channels.msteams.mediaAuthAllowHosts`: Zulassungsliste für das Anhängen von Authorization-Headern bei Medien-Wiederholungen (standardmäßig Graph- und Bot-Framework-Hosts).
...
- `channels.msteams.actions.memberInfo`: die Graph-gestützte Aktion für Mitgliederinformationen aktivieren oder deaktivieren (Standard: aktiviert, wenn Graph-Anmeldeinformationen verfügbar sind).
...
- `channels.msteams.certificateThumbprint`: Zertifikatfingerabdruck (optional, für die Authentifizierung nicht erforderlich).
- `channels.msteams.useManagedIdentity`: Authentifizierung mit verwalteter Identität aktivieren (föderierter Modus).
- `channels.msteams.managedIdentityClientId`: Client-ID für benutzerseitig zugewiesene verwaltete Identität.
...
Wenn Sie `requireMention: false` global ohne explizites `replyStyle` festlegen, erscheinen Erwähnungen in Kanälen im Beiträge-Stil als Beiträge auf oberster Ebene, auch wenn die eingehende Nachricht eine Thread-Antwort war. Setzen Sie `replyStyle: "thread"` auf globaler, Team- oder Kanalebene fest, um Überraschungen zu vermeiden.

### Erhaltung des Thread-Kontexts

Wenn `replyStyle: "thread"` wirksam ist und der Bot aus einem Kanal-Thread heraus @mentioned wurde, hängt OpenClaw die ursprüngliche Thread-Wurzel wieder an die ausgehende Konversationsreferenz an (`19:…@thread.tacv2;messageid=<root>`), sodass die Antwort im selben Thread landet. Dies gilt sowohl für Live-Sendungen (innerhalb desselben Turns) als auch für proaktive Sendungen, die nach Ablauf des Bot-Framework-Turn-Kontexts erfolgen (z. B. lang laufende Agenten, in die Warteschlange gestellte Tool-Call-Antworten über `mcp__openclaw__message`).

Die Thread-Wurzel wird aus der gespeicherten `threadId` in der Konversationsreferenz entnommen. Ältere gespeicherte Referenzen, die vor `threadId` liegen, fallen auf `activityId` zurück (die eingehende Aktivität, die die Konversation zuletzt initialisiert hat), sodass bestehende Deployments ohne erneutes Initialisieren weiter funktionieren.

Wenn `replyStyle: "top-level"` wirksam ist, werden eingehende Kanal-Thread-Nachrichten absichtlich als neue Beiträge auf oberster Ebene beantwortet — es wird kein Thread-Suffix angehängt. Dies ist das korrekte Verhalten für Kanäle im Threads-Stil; wenn Sie Beiträge auf oberster Ebene sehen, obwohl Sie Thread-Antworten erwartet haben, ist Ihr `replyStyle` für diesen Kanal falsch gesetzt.

## Anhänge und Bilder

**Aktuelle Einschränkungen:**

- **Direktnachrichten:** Bilder und Dateianhänge funktionieren über Teams-Bot-Datei-APIs.
- **Kanäle/Gruppen:** Anhänge liegen in M365-Speicher (SharePoint/OneDrive). Die Webhook-Nutzlast enthält nur einen HTML-Stub, nicht die eigentlichen Dateibytes. **Graph-API-Berechtigungen sind erforderlich**, um Kanal-Anhänge herunterzuladen.
- Für explizite dateizentrierte Sendungen verwenden Sie `action=upload-file` mit `media` / `filePath` / `path`; optionales `message` wird zum begleitenden Text/Kommentar, und `filename` überschreibt den hochgeladenen Namen.
...
| **Gruppenchats/Kanäle**  | Upload nach SharePoint → Freigabelink        | Erfordert `sharePointSiteId` + Graph-Berechtigungen |
...
2. **Administratorzustimmung erteilen** für den Tenant.
...
- Umfragen veröffentlichen noch nicht automatisch Ergebniszusammenfassungen (prüfen Sie bei Bedarf die Speicherdatei).
...
<Note>
Ohne das Präfix `user:` werden Namen standardmäßig als Gruppe oder Team aufgelöst. Verwenden Sie immer `user:`, wenn Sie Personen anhand des Anzeigenamens adressieren.
</Note>

## Proaktives Messaging

- Proaktive Nachrichten sind nur möglich, **nachdem** ein Benutzer interagiert hat, da wir an diesem Punkt Unterhaltungsreferenzen speichern.
- Siehe `/gateway/configuration` für `dmPolicy` und Allowlist-Steuerung.

## Team- und Kanal-IDs (häufige Fehlerquelle)

Der Abfrageparameter `groupId` in Teams-URLs ist **NICHT** die Team-ID, die für die Konfiguration verwendet wird. Extrahieren Sie stattdessen IDs aus dem URL-Pfad:
...
- Team-Schlüssel = Pfadsegment nach `/team/` (URL-dekodiert, z. B. `19:Bk4j...@thread.tacv2`; ältere Tenants zeigen möglicherweise `@thread.skype`, was ebenfalls gültig ist)
- Kanal-Schlüssel = Pfadsegment nach `/channel/` (URL-dekodiert)
- **Ignorieren** Sie den Abfrageparameter `groupId` für das Routing in OpenClaw. Er ist die Microsoft Entra-Gruppen-ID, nicht die Bot Framework-Unterhaltungs-ID, die in eingehenden Teams-Aktivitäten verwendet wird.
...
- **401 Unauthorized vom Webhook:** Erwartet bei manuellen Tests ohne Azure JWT - bedeutet, dass der Endpunkt erreichbar ist, die Authentifizierung jedoch fehlgeschlagen ist. Verwenden Sie Azure Web Chat, um korrekt zu testen.
...
- **"webApplicationInfo.Id already in use":** Die App ist noch in einem anderen Team/Chat installiert. Suchen und deinstallieren Sie sie zuerst, oder warten Sie 5 bis 10 Minuten auf die Weitergabe.
- **"Something went wrong" beim Hochladen:** Laden Sie stattdessen über [https://admin.teams.microsoft.com](https://admin.teams.microsoft.com) hoch, öffnen Sie die Browser-DevTools (F12) → Registerkarte Netzwerk, und prüfen Sie den Antworttext auf den tatsächlichen Fehler.
- **Sideload schlägt fehl:** Versuchen Sie stattdessen "Upload an app to your org's app catalog" statt "Upload a custom app" - dies umgeht häufig Sideload-Einschränkungen.
