...
Die [`@microsoft/teams.cli`](https://www.npmjs.com/package/@microsoft/teams.cli) übernimmt Bot-Registrierung, Manifest-Erstellung und Anmeldedatengenerierung in einem einzigen Befehl.
...
Installieren und authentifizieren Sie die devtunnel CLI, falls Sie dies noch nicht getan haben ([Einstiegsanleitung](https://learn.microsoft.com/en-us/azure/developer/dev-tunnels/get-started)).
...
<Note>
`--allow-anonymous` ist erforderlich, da Teams sich nicht bei devtunnels authentifizieren kann. Jede eingehende Bot-Anfrage wird dennoch automatisch durch das Teams SDK validiert.
</Note>
...
Für Produktionsbereitstellungen sollten Sie [föderierte Authentifizierung](/de/channels/msteams#federated-authentication-certificate-plus-managed-identity) (Zertifikat oder Managed Identity) anstelle von Client-Geheimnissen in Betracht ziehen.

<Note>
Gruppenchats sind standardmäßig blockiert (`channels.msteams.groupPolicy: "allowlist"`). Um Gruppenantworten zu erlauben, legen Sie `channels.msteams.groupAllowFrom` fest, oder verwenden Sie `groupPolicy: "open"`, um jedes Mitglied zu erlauben (durch Erwähnungen gesteuert).
</Note>
...
<Note>
Ohne das Präfix `user:` werden Namen standardmäßig über Gruppen- oder Team-Auflösung aufgelöst. Verwenden Sie immer `user:`, wenn Sie Personen nach Anzeigename adressieren.
</Note>
