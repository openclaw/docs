...
title: Chat-Kanäle
x-i18n:
    generated_at: "2026-07-16T12:25:33Z"
    model: gpt-5.6
...
OpenClaw kann über jede Chat-App, die Sie bereits verwenden, mit Ihnen kommunizieren. Jeder Kanal stellt die Verbindung über das Gateway her.
Text wird überall unterstützt; Medien und Reaktionen unterscheiden sich je nach Kanal.
...
- [iMessage](/de/channels/imessage) – Im Kern enthalten. Native macOS-Integration über die `imsg`-Bridge auf einem angemeldeten Mac (oder einen SSH-Wrapper, wenn das Gateway an anderer Stelle ausgeführt wird), einschließlich privater API-Aktionen für Antworten, Tapbacks, Effekte, Anhänge und Gruppenverwaltung.
- [IRC](/de/channels/irc) – Klassische IRC-Server; Kanäle und Direktnachrichten mit Kopplungs- und Zulassungslistensteuerung (offizielles Plugin).
...
- [Raft](/de/channels/raft) – Raft-CLI-Wake-Bridge für die Zusammenarbeit zwischen Menschen und Agenten (offizielles Plugin).
...
- [Zalo ClawBot](/de/channels/zaloclawbot) – Persönlicher Zalo-Assistent mit QR-Anmeldung; an den Eigentümer gebunden (externes Plugin).
...
- Telegram-Antworten, die Markdown-Bildsyntax wie `![alt](url)` enthalten,
  werden im abschließenden ausgehenden Pfad nach Möglichkeit in Medienantworten umgewandelt.
...
- Unterstützte dauerhaft aktive Räume können [Ereignisse für Umgebungsräume](/de/channels/ambient-room-events) verwenden,
  sodass nicht an den Agenten gerichtete Raumunterhaltungen zu stillem Kontext werden, sofern der Agent nicht mit
  dem Werkzeug `message` sendet.

## Hinweise

- Kanäle können gleichzeitig ausgeführt werden; konfigurieren Sie mehrere, und OpenClaw führt das Routing je Chat durch.
...
- Das Gruppenverhalten unterscheidet sich je nach Kanal; siehe [Gruppen](/de/channels/groups).
- Die Kopplung von Direktnachrichten und Zulassungslisten werden zur Sicherheit durchgesetzt; siehe [Sicherheit](/de/gateway/security).
