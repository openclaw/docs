...
x-i18n:
    generated_at: "2026-07-16T13:04:17Z"
    model: gpt-5.6
...
- **Xcode 26.2+** (Swift-6.2-Toolchain) auf der neuesten macOS-Version, die unter
  Software Update verfügbar ist.
- **Node.js 24.15+ und pnpm** für Gateway, CLI und Paketierungsskripte. Node
...
Die Ausgabe erfolgt unter `dist/OpenClaw.app`. Ohne ein Apple-Developer-ID-Zertifikat greift das
Skript auf eine Ad-hoc-Signierung zurück.

Informationen zu Entwicklungsmodi, Signierungsoptionen und zur Fehlerbehebung bei der Team-ID finden Sie unter
...
<Note>
Ad-hoc-signierte Anwendungen können Sicherheitsabfragen auslösen. Wenn die Anwendung
...
passende CLI und Laufzeit im Benutzerbereich, bevor sie den Gateway-Assistenten startet.
...
`pnpm add -g openclaw@<version>` und `bun add -g openclaw@<version>`
funktionieren ebenfalls. Node bleibt die empfohlene Laufzeit für das Gateway selbst.
...
### Anwendung stürzt beim Gewähren einer Berechtigung ab

Wenn die Anwendung abstürzt, während Sie versuchen, den Zugriff auf **Speech Recognition** oder
**Microphone** zu erlauben, kann die Ursache ein beschädigter TCC-Cache oder eine abweichende Signatur sein.
...
2. Wenn dies fehlschlägt, ändern Sie vorübergehend `BUNDLE_ID` in
   ...
   um einen sauberen Ausgangszustand von macOS zu erzwingen.
...
letzten Ausweg die oben ermittelte PID.
