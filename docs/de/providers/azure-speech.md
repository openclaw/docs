...
x-i18n:
    generated_at: "2026-07-16T13:19:06Z"
    model: gpt-5.6
...
Telefoniekanäle wie Voice Call. Die Anfrage sendet das vom Provider festgelegte
...
<Steps>
  <Step title="Eine Azure-Speech-Ressource erstellen">
    ...
    unter Resource Management > Keys and Endpoint und kopieren Sie den
...
| Option                  | Beschreibung                                                                                              |
| ----------------------- | --------------------------------------------------------------------------------------------------------- |
| `apiKey`      | Ressourcenschlüssel für Azure Speech. Fällt auf `AZURE_SPEECH_KEY`, `AZURE_SPEECH_API_KEY` oder `SPEECH_KEY` zurück. |
| `region`      | Ressourcenregion für Azure Speech. Fällt auf `AZURE_SPEECH_REGION` oder `SPEECH_REGION` zurück.          |
...
| `timeoutMs`      | Überschreibung des Anfrage-Timeouts in Millisekunden. Fällt auf das globale `messages.tts.timeoutMs` zurück.    |

Der Provider gilt als konfiguriert, sobald `apiKey` sowie einer der Werte
`region`, `endpoint` oder `baseUrl` festgelegt ist. Umgebungsvariablen werden nur als
Fallback für nicht gesetzte Konfigurationsschlüssel geprüft. `.env`-Dateien im Workspace können
`AZURE_SPEECH_ENDPOINT` nicht festlegen; verwenden Sie für das Endpunkt-Routing die Prozessumgebung,
die globale Laufzeit-dotenv oder eine explizite Konfiguration.
...
<AccordionGroup>
  ...
  <Accordion title="Stimmennamen">
    Verwenden Sie den Wert `ShortName` der Azure-Speech-Stimme, zum Beispiel
