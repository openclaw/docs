...
read_when:
    ...
    - Sie möchten Codex-Abonnement-Authentifizierung statt API-Schlüsseln verwenden
...
title: OpenAI
x-i18n:
    generated_at: "2026-04-30T07:11:34Z"
    model: gpt-5.5
    provider: openai
...
OpenAI stellt Entwickler-APIs für GPT-Modelle bereit, und Codex ist über die Codex-Clients von OpenAI auch als Coding-Agent eines ChatGPT-Tarifs verfügbar. OpenClaw hält diese Oberflächen getrennt, damit die Konfiguration vorhersehbar bleibt.

OpenClaw unterstützt drei Routen der OpenAI-Familie. Das Modellpräfix wählt die Provider-/Authentifizierungsroute aus; eine separate Runtime-Einstellung legt fest, wer die eingebettete Agent-Schleife ausführt:
...
Provider, Modell, Runtime und Kanal sind separate Ebenen. Wenn diese Bezeichnungen vermischt werden, lesen Sie [Agent-Runtimes](/de/concepts/agent-runtimes), bevor Sie die Konfiguration ändern.

## Schnellauswahl

| Ziel                                          | Verwenden                                        | Hinweise                                                                     |
| --------------------------------------------- | ------------------------------------------------ | ---------------------------------------------------------------------------- |
| Direkte Abrechnung per API-Schlüssel          | `openai/gpt-5.5`                                 | Setzen Sie `OPENAI_API_KEY` oder führen Sie das Onboarding für den OpenAI-API-Schlüssel aus. |
| GPT-5.5 mit ChatGPT-/Codex-Abonnement-Auth    | `openai-codex/gpt-5.5`                           | Standard-PI-Route für Codex OAuth. Beste erste Wahl für Abonnement-Setups. |
...
| `agentRuntime.id: codex`             | Agent-Runtime     | Erzwingt den nativen Codex-App-Server-Harness für eingebettete Durchläufe.                        |
...
<Note>
GPT-5.5 ist sowohl über direkten OpenAI Platform-Zugriff per API-Schlüssel als auch über Abonnement-/OAuth-Routen verfügbar. Verwenden Sie `openai/gpt-5.5` für direkten `OPENAI_API_KEY`-Traffic, `openai-codex/gpt-5.5` für Codex OAuth über PI oder `openai/gpt-5.5` mit `agentRuntime.id: "codex"` für den nativen Codex-App-Server-Harness.
</Note>

<Note>
Das Aktivieren des OpenAI-Plugins oder die Auswahl eines `openai-codex/*`-Modells aktiviert nicht das gebündelte Codex-App-Server-Plugin. OpenClaw aktiviert dieses Plugin nur, wenn Sie den nativen Codex-Harness ausdrücklich mit `agentRuntime.id: "codex"` auswählen oder eine ältere `codex/*`-Modellreferenz verwenden.
...
| OpenAI-Fähigkeit         | OpenClaw-Schnittstelle                                    | Status                                                 |
| ------------------------ | ---------------------------------------------------------- | ------------------------------------------------------ |
...
OpenClaw kann OpenAI oder einen OpenAI-kompatiblen Embedding-Endpunkt für die `memory_search`-Indexierung und Abfrage-Embeddings verwenden:
...
Für OpenAI-kompatible Endpunkte, die asymmetrische Embedding-Labels erfordern, setzen Sie `queryInputType` und `documentInputType` unter `memorySearch`. OpenClaw leitet diese als providerspezifische `input_type`-Anfragefelder weiter: Abfrage-Embeddings verwenden `queryInputType`; indexierte Memory-Blöcke und Batch-Indexierung verwenden `documentInputType`. Das vollständige Beispiel finden Sie in der [Memory-Konfigurationsreferenz](/de/reference/memory-config#provider-specific-config).
...
<Tabs>
  <Tab title="API-Schlüssel (OpenAI Platform)">
    **Am besten für:** direkten API-Zugriff und nutzungsbasierte Abrechnung.
    ...
    <Note>
    `openai/*` ist die direkte OpenAI-API-Schlüsselroute, sofern Sie nicht ausdrücklich den Codex-App-Server-Harness erzwingen. Verwenden Sie `openai-codex/*` für Codex OAuth über den Standard-PI-Runner oder `openai/gpt-5.5` mit `agentRuntime.id: "codex"` für native Codex-App-Server-Ausführung.
    </Note>
  ...
  <Tab title="Codex-Abonnement">
    **Am besten für:** die Nutzung Ihres ChatGPT-/Codex-Abonnements anstelle eines separaten API-Schlüssels. Codex Cloud erfordert die ChatGPT-Anmeldung.
    ...
    <Steps>
      <Step title="Codex OAuth ausführen">
        ...
        Fügen Sie für Headless- oder callback-feindliche Setups `--device-code` hinzu, um sich mit einem ChatGPT-Gerätecode-Flow anstelle des localhost-Browser-Callbacks anzumelden:
    ...
    | `openai-codex/gpt-5.5` | `runtime: "auto"` | Weiterhin PI, sofern ein Plugin nicht ausdrücklich `openai-codex` beansprucht | Codex-Anmeldung |
    | `openai/gpt-5.5` | `agentRuntime.id: "codex"` | Codex-App-Server-Harness | Codex-App-Server-Auth |

    <Note>
    Verwenden Sie weiterhin die Provider-ID `openai-codex` für Auth-/Profilbefehle. Das Modellpräfix `openai-codex/*` ist außerdem die explizite PI-Route für Codex OAuth. Es wählt den gebündelten Codex-App-Server-Harness nicht aus und aktiviert ihn nicht automatisch.
    </Note>

    <Warning>
    `openai-codex/gpt-5.4-mini` ist keine unterstützte Codex OAuth-Route. Verwenden Sie `openai/gpt-5.4-mini` mit einem OpenAI-API-Schlüssel oder `openai-codex/gpt-5.5` mit Codex OAuth.
    </Warning>
    ...
    <Note>
    Das Onboarding importiert kein OAuth-Material mehr aus `~/.codex`. Melden Sie sich mit Browser-OAuth (Standard) oder dem oben genannten Gerätecode-Flow an — OpenClaw verwaltet die resultierenden Zugangsdaten in seinem eigenen Agent-Auth-Speicher.
    </Note>

    ### Statusanzeige

    Chat `/status` zeigt, welche Modell-Laufzeitumgebung für die aktuelle Sitzung aktiv ist.
    ...
    beabsichtigte Route für Abonnement-Authentifizierung ist. Wechseln Sie nur dann zu
    `openai/<model>` plus `agentRuntime.id: "codex"`, wenn Sie native Codex-
    ...
    - Standardmäßiges Laufzeit-Limit `contextTokens`: `272000`

    Das kleinere Standardlimit bietet in der Praxis bessere Latenz- und Qualitätsmerkmale. Überschreiben Sie es mit `contextTokens`:
    ...
    <Note>
    Verwenden Sie `contextWindow`, um native Modell-Metadaten zu deklarieren. Verwenden Sie `contextTokens`, um das Laufzeit-Kontextbudget zu begrenzen.
    </Note>

    ### Katalog-Wiederherstellung

    OpenClaw verwendet Upstream-Codex-Katalog-Metadaten für `gpt-5.5`, wenn sie
...
2. Das vorhandene Konto des App-Servers, beispielsweise eine lokale ChatGPT-Anmeldung der Codex CLI.
...
oder Einbettungen hat. Der Env-API-Schlüssel-Fallback ist nur der lokale stdio-Pfad ohne Konto; er
...
aus dem gestarteten stdio-App-Server-Kindprozess heraus und sendet die ausgewählten Anmeldeinformationen
...
| Seitenverhältnis / Auflösung | Nicht an OpenAI Images API weitergeleitet | Wird bei sicherer Verwendung einer unterstützten Größe zugeordnet |
...
explizite Modellüberschreibungen nutzbar. Verwenden Sie `openai/gpt-image-1.5` für Ausgabe mit transparentem Hintergrund
als PNG/WebP; die aktuelle API `gpt-image-2` lehnt
...
Bei Codex-OAuth-Installationen behalten Sie dieselbe Referenz `openai/gpt-image-2` bei. Wenn ein
...
versucht für diese Anfrage nicht zuerst `OPENAI_API_KEY` und fällt auch nicht stillschweigend auf einen API-Schlüssel zurück.
Konfigurieren Sie `models.providers.openai` explizit mit einem API-Schlüssel,
einer benutzerdefinierten Basis-URL oder einem Azure-Endpunkt, wenn Sie stattdessen die direkte Route über die OpenAI Images API
verwenden möchten.
...
private/interne OpenAI-kompatible Bildendpunkte blockiert, sofern diese Opt-in-Einstellung nicht
vorhanden ist.
...
OpenClaw fügt einen gemeinsamen GPT-5-Prompt-Beitrag für Läufe der GPT-5-Familie über Provider hinweg hinzu. Er wird anhand der Modell-ID angewendet, daher erhalten `openai-codex/gpt-5.5`, `openai/gpt-5.5`, `openrouter/openai/gpt-5.5`, `opencode/gpt-5.5` und andere kompatible GPT-5-Referenzen dasselbe Overlay. Ältere GPT-4.x-Modelle nicht.

Das gebündelte native Codex-Harness verwendet dasselbe GPT-5-Verhalten und Heartbeat-Overlay über Codex-App-Server-Developer-Instructions, sodass `openai/gpt-5.x`-Sitzungen, die über `agentRuntime.id: "codex"` erzwungen werden, dieselbe Follow-through- und proaktive Heartbeat-Anleitung behalten, obwohl Codex den Rest des Harness-Prompts besitzt.
...
<Tip>
Werte sind zur Laufzeit unabhängig von Groß-/Kleinschreibung, daher deaktivieren sowohl `"Off"` als auch `"off"` die freundliche Stil-Ebene.
</Tip>

<Note>
Das Legacy-Element `plugins.entries.openai.config.personality` wird weiterhin als Kompatibilitäts-Fallback gelesen, wenn die gemeinsame Einstellung `agents.defaults.promptOverlays.gpt5.personality` nicht gesetzt ist.
</Note>
...
<AccordionGroup>
  ...
  <Accordion title="Speech-to-text">
    ...
    die Medienverständnis-Transkriptionsoberfläche von OpenClaw.
    ...
    So erzwingen Sie OpenAI für eingehende Audio-Transkription:
    ...
    gemeinsamen Audio-Medienkonfiguration oder der Transkriptionsanfrage pro Aufruf bereitgestellt werden.
  ...
  <Accordion title="Echtzeit-Transkription">
    ...
    <Note>
    Verwendet eine WebSocket-Verbindung zu `wss://api.openai.com/v1/realtime` mit G.711 u-law (`g711_ulaw` / `audio/pcmu`) Audio. Dieser Streaming-Provider ist für den Echtzeit-Transkriptionspfad von Voice Call vorgesehen; Discord Voice zeichnet derzeit kurze Segmente auf und verwendet stattdessen den Batch-Transkriptionspfad `tools.media.audio`.
    </Note>
  ...
  <Accordion title="Echtzeit-Stimme">
    ...
    <Note>
    Control UI Talk verwendet OpenAI-Browser-Echtzeitsitzungen mit einem vom Gateway erzeugten
    ...
    der OpenAI-Teil erzeugt ein Client-Secret in Node, generiert ein Browser-SDP-Angebot
...
- Sie bereits über ein Azure OpenAI-Abonnement, Kontingent oder eine Enterprise-Vereinbarung verfügen
- Sie regionale Datenresidenz oder Compliance-Steuerungen benötigen, die Azure bereitstellt
- Sie den Traffic innerhalb einer vorhandenen Azure-Tenancy halten möchten
...
- Verwendet ein Standard-Request-Timeout von 600 Sekunden für Azure-Bildgenerierungsaufrufe.
...
OpenClaw 2026.4.22 oder höher. Frühere Versionen behandeln jedes benutzerdefinierte
...
`uaenorth`). Prüfen Sie die aktuelle Regionsliste von Microsoft, bevor Sie eine
Bereitstellung erstellen, und bestätigen Sie, dass das spezifische Modell in Ihrer Region angeboten wird.
...
im Azure-Portal den Parametersatz, der von Ihrer spezifischen Bereitstellung und API-Version unterstützt wird.

<Note>
Azure OpenAI verwendet natives Transport- und Kompatibilitätsverhalten, erhält jedoch nicht
die verborgenen Attribution-Header von OpenClaw — siehe das Accordion **Native vs. OpenAI-kompatible
...
<AccordionGroup>
  <Accordion title="Transport (WebSocket vs. SSE)">
    OpenClaw verwendet für `openai/*` und `openai-codex/*` WebSocket zuerst mit SSE-Fallback (`"auto"`).
  ...
  <Accordion title="Schnellmodus">
    ...
    Wenn aktiviert, ordnet OpenClaw den Schnellmodus der priorisierten Verarbeitung von OpenAI zu (`service_tier = "priority"`). Vorhandene `service_tier`-Werte bleiben erhalten, und der Schnellmodus überschreibt weder `reasoning` noch `text.verbosity`.
  ...
  <Accordion title="Priorisierte Verarbeitung (service_tier)">
    Die API von OpenAI stellt priorisierte Verarbeitung über `service_tier` bereit. Legen Sie sie in OpenClaw pro Modell fest:
  ...
  <Accordion title="Serverseitige Compaction (Responses API)">
    ...
    <Note>
    `responsesServerCompaction` steuert nur die `context_management`-Injection. Direkte OpenAI-Responses-Modelle erzwingen weiterhin `store: true`, sofern Compat nicht `supportsStore: false` festlegt.
    </Note>
  ...
  <Accordion title="Strenger agentischer GPT-Modus">
    ...
    - Ein reiner Planungsturn gilt nicht mehr als erfolgreicher Fortschritt, wenn eine Tool-Aktion verfügbar ist
  ...
  <Accordion title="Native vs. OpenAI-kompatible Routen">
    ...
    **Native Routen** (`openai/*`, Azure OpenAI):
    ...
    - Setzen Tool-Schemas standardmäßig in den Strict-Modus
    - Fügen versteckte Zuordnungs-Header nur auf verifizierten nativen Hosts an
    - Behalten OpenAI-spezifische Anfrageformung bei (`service_tier`, `store`, Reasoning-Compat, Prompt-Cache-Hinweise)
    ...
    **Proxy-/kompatible Routen:**
    ...
    - Erzwingen weder strikte Tool-Schemas noch nur native Header

    Azure OpenAI verwendet nativen Transport und Compat-Verhalten, erhält aber keine versteckten Zuordnungs-Header.
