...
title: Inworld
x-i18n:
    generated_at: "2026-07-24T04:52:57Z"
    model: gpt-5.6
...
Inworld ist ein Provider für Text-to-Speech-Streaming (TTS). In OpenClaw synthetisiert er Audiodaten für ausgehende Antworten (standardmäßig MP3, OGG_OPUS für Sprachnachrichten) sowie rohe PCM-Audiodaten für Telefoniekanäle wie Voice Call.

OpenClaw sendet Anfragen an den Streaming-TTS-Endpunkt von Inworld, fügt die zurückgegebenen Base64-Audiosegmente zu einem einzelnen Puffer zusammen und übergibt das Ergebnis an die standardmäßige Pipeline für Antwortaudiodaten.
...
<Steps>
  ...
  <Step title="Nachricht senden">
    Senden Sie eine Antwort über einen beliebigen verbundenen Kanal. OpenClaw synthetisiert die Audiodaten mit Inworld und stellt sie als MP3 bereit (oder als OGG_OPUS, wenn der Kanal eine Sprachnachricht erwartet).
  </Step>
</Steps>
...
| ------------- | ----------------------------------- | ------------------------------------------------------------------- |
| `apiKey`      | `tts.providers.inworld.apiKey`      | Base64-Dashboard-Anmeldedaten. Fällt auf `INWORLD_API_KEY` zurück.  |
...
<AccordionGroup>
  <Accordion title="Authentifizierung">
    Inworld verwendet HTTP-Basic-Authentifizierung mit einer einzelnen Base64-codierten Zeichenfolge für die Anmeldedaten. Kopieren Sie sie unverändert aus dem Inworld-Dashboard. Der Provider sendet sie ohne weitere Codierung als `Authorization: Basic <apiKey>`. Codieren Sie sie daher nicht selbst mit Base64 und übergeben Sie kein Token im Bearer-Stil. Siehe [Hinweise zur TTS-Authentifizierung](/de/tools/tts#inworld-primary) für denselben Hinweis.
  </Accordion>
  ...
  <Accordion title="Audioausgaben">
    Antworten verwenden standardmäßig MP3. Wenn das Kanalziel `voice-note` ist, fordert OpenClaw bei Inworld `OGG_OPUS` an, damit die Audiodaten als native Sprachblase wiedergegeben werden. Die Telefoniesynthese verwendet rohe `PCM` mit 22050 Hz, um die Telefonie-Bridge zu speisen.
  </Accordion>
...
<CardGroup cols={2}>
  <Card title="Text-to-Speech" href="/de/tools/tts" icon="waveform-lines">
    TTS-Übersicht, Provider und Konfiguration von `tts`.
  </Card>
