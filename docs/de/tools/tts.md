...
x-i18n:
    generated_at: "2026-05-07T13:27:06Z"
    model: gpt-5.5
    provider: openai
...
OpenClaw kann ausgehende Antworten über **14 Sprach-Provider**
in Audio umwandeln und native Sprachnachrichten auf Feishu, Matrix, Telegram und WhatsApp,
...
wählt OpenClaw den ersten konfigurierten Provider in der Auto-Auswahlreihenfolge der Registry aus.
Das integrierte `tts`-Agententool ist nur für explizite Absichten gedacht: Normale Chats bleiben
Text, sofern der Benutzer nicht Audio anfordert, `/tts` verwendet oder Auto-TTS/direktivenbasiertes
Sprechen aktiviert.
</Note>
...
| **DeepInfra**     | `DEEPINFRA_API_KEY`                                                                                              | OpenAI-kompatibles TTS. Standard ist `hexgrad/Kokoro-82M`.                                    |
...
| **Google Gemini** | `GEMINI_API_KEY` oder `GOOGLE_API_KEY`                                                                             | Gemini-API-Batch-TTS; persona-aware über `promptTemplate: "audio-profile-v1"`.               |
...
| **MiniMax**       | `MINIMAX_API_KEY` (oder Token-Plan: `MINIMAX_OAUTH_TOKEN`, `MINIMAX_CODE_PLAN_KEY`, `MINIMAX_CODING_API_KEY`)      | T2A-v2-API. Standard ist `speech-2.8-hd`.                                                    |
| **OpenAI**        | `OPENAI_API_KEY`                                                                                                 | Wird auch für Auto-Zusammenfassung verwendet; unterstützt Persona-`instructions`.                                |
...
<Warning>
Der gebündelte **Microsoft**-Provider verwendet Microsoft Edges Online-Dienst für neuronales TTS
...
einer anderen Stimme, einem anderen Modell, einer anderen Persona oder einem anderen Auto-TTS-Modus sprechen soll. Der Agentenblock wird tief über
`messages.tts` zusammengeführt, sodass Provider-Zugangsdaten in der globalen Provider-Konfiguration bleiben können:
...
Um eine agentenspezifische Persona festzulegen, setzen Sie `agents.list[].tts.persona` neben der Provider-Konfiguration. Sie überschreibt `messages.tts.persona` global nur für diesen Agenten.
...
Kanal- und Konto-Overrides verwenden dieselbe Struktur wie `messages.tts` und werden per Deep-Merge über frühere Ebenen gelegt. So können gemeinsame Provider-Zugangsdaten in `messages.tts` bleiben, während ein Kanal oder Bot-Konto nur Stimme, Modell, Persona oder Auto-Modus ändert:
...
Die Provider-Auswahl erfolgt nach dem Explicit-first-Prinzip:
...
<AccordionGroup>
  ...
  <Accordion title="Andere Provider">
    Verwenden nur die Provider-spezifischen Persona-Bindings unter `personas.<id>.providers.<provider>`. Persona-Prompt-Felder werden ignoriert, sofern der Provider kein eigenes Persona-Prompt-Mapping implementiert.
  </Accordion>
</AccordionGroup>
...
Die Provider-Auswahl für Talk-Sitzungen ist sitzungsbezogen. Ein Talk-Client sollte Provider-IDs, Modell-IDs, Stimmen-IDs und Gebietsschemas aus `talk.catalog` auswählen und sie über die Talk-Sitzung oder den Handoff-Request übergeben. Das Öffnen einer Sprachsitzung sollte `messages.tts` oder globale Talk-Provider-Standardwerte nicht verändern.

## Modellgesteuerte Direktiven

Standardmäßig **kann** der Assistent `[[tts:...]]`-Direktiven ausgeben, um Stimme, Modell oder Geschwindigkeit für eine einzelne Antwort zu überschreiben, plus optionalem `[[tts:text]]...[[/tts:text]]`-Block für expressive Hinweise, die nur im Audio erscheinen sollen:
...
Wenn `messages.tts.auto` auf `"tagged"` gesetzt ist, sind **Direktiven erforderlich**, um Audio auszulösen. Die Streaming-Blockzustellung entfernt Direktiven aus dem sichtbaren Text, bevor der Kanal sie sieht, selbst wenn sie über benachbarte Blöcke verteilt sind.
...
- `pitch` (ganzzahliger MiniMax-Pitch, -12 bis 12; Bruchwerte werden abgeschnitten)
...
Einzelner Befehl `/tts`. Auf Discord registriert OpenClaw außerdem `/voice`, weil `/tts` ein integrierter Discord-Befehl ist — Textbefehle `/tts ...` funktionieren weiterhin.
...
- `/tts latest` liest die neueste Assistentenantwort aus dem aktuellen Sitzungstranskript und sendet sie einmalig als Audio. Es speichert nur einen Hash dieser Antwort im Sitzungseintrag, um doppelte Sprachsendungen zu unterdrücken.
...
- `/status` zeigt den aktiven TTS-Modus sowie konfigurierten Provider, Modell, Stimme und bereinigte Metadaten für benutzerdefinierte Endpunkte an, wenn TTS aktiviert ist.

## Benutzerspezifische Einstellungen

Slash-Befehle schreiben lokale Overrides nach `prefsPath`. Standard ist `~/.openclaw/settings/tts.json`; überschreiben Sie dies mit der Env-Var `OPENCLAW_TTS_PREFS` oder `messages.tts.prefsPath`.
...
Die TTS-Sprachausgabe wird durch Kanalfähigkeiten gesteuert. Kanal-Plugins geben an, ob TTS im Sprachstil Provider nach einem nativen `voice-note`-Ziel fragen oder normale `audio-file`-Synthese beibehalten und kompatible Ausgabe nur für Sprachzustellung markieren soll.

- **Kanäle mit Sprachnotiz-Unterstützung**: Sprachnotiz-Antworten bevorzugen Opus (`opus_48000_64` von ElevenLabs, `opus` von OpenAI).
  - 48 kHz / 64 kbps sind ein guter Kompromiss für Sprachnachrichten.
- **Feishu / WhatsApp**: Wenn eine Sprachnotiz-Antwort als MP3/WebM/WAV/M4A
  oder als andere wahrscheinliche Audiodatei erzeugt wird, transkodiert das Kanal-Plugin
  sie mit `ffmpeg` zu 48 kHz Ogg/Opus, bevor die native Sprachnachricht gesendet wird. WhatsApp sendet
...
- **BlueBubbles**: belässt die Provider-Synthese auf dem normalen Audiodatei-Pfad; MP3-
...
- **MiniMax**: MP3 (`speech-2.8-hd`-Modell, 32-kHz-Abtastrate) für normale Audioanhänge. Für vom Kanal deklarierte Sprachnotiz-Ziele transkodiert OpenClaw das MiniMax-MP3 vor der Zustellung mit `ffmpeg` zu 48 kHz Opus, wenn der Kanal Transkodierung deklariert.
- **Xiaomi MiMo**: Standardmäßig MP3 oder WAV, wenn konfiguriert. Für vom Kanal deklarierte Sprachnotiz-Ziele transkodiert OpenClaw die Xiaomi-Ausgabe vor der Zustellung mit `ffmpeg` zu 48 kHz Opus, wenn der Kanal Transkodierung deklariert.
...
- Fasst lange Antworten zusammen, wenn Zusammenfassungen aktiviert sind, mit
...
| Ziel                                  | Format                                                                                                                                        |
| ------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------- |
| Feishu / Matrix / Telegram / WhatsApp | Sprachnotiz-Antworten bevorzugen **Opus** (`opus_48000_64` von ElevenLabs, `opus` von OpenAI). 48 kHz / 64 kbps gleichen Klarheit und Größe aus. |
...
- **Feishu / WhatsApp-Transkodierung:** Wenn eine Sprachnotiz-Antwort als MP3/WebM/WAV/M4A ankommt, transkodiert das Kanal-Plugin sie mit `ffmpeg` zu 48 kHz Ogg/Opus. WhatsApp sendet über Baileys mit `ptt: true` und `audio/ogg; codecs=opus`. Wenn die Konvertierung fehlschlägt: Feishu fällt auf das Anhängen der Originaldatei zurück; der WhatsApp-Versand schlägt fehl, statt eine inkompatible PTT-Payload zu posten.
- **MiniMax / Xiaomi MiMo:** Standardmäßig MP3 (32 kHz für MiniMax `speech-2.8-hd`); wird für Sprachnotiz-Ziele über `ffmpeg` zu 48 kHz Opus transkodiert.
...
- **xAI:** Standardmäßig MP3; `responseFormat` kann `mp3|wav|pcm|mulaw|alaw` sein. Verwendet den Batch-REST-Endpunkt von xAI — Streaming-WebSocket-TTS wird **nicht** verwendet. Das native Opus-Format für Sprachnotizen wird **nicht** unterstützt.
...
<AccordionGroup>
  <Accordion title="Top-level messages.tts.*">
    ...
    <ParamField path="enabled" type="boolean" deprecated>
      Veralteter Schalter. `openclaw doctor --fix` migriert dies zu `auto`.
    </ParamField>
    <ParamField path="mode" type='"final" | "all"' default="final">
      `"all"` bezieht zusätzlich zu finalen Antworten Tool-/Block-Antworten ein.
    </ParamField>
    <ParamField path="provider" type="string">
      Speech-Provider-ID. Wenn nicht festgelegt, verwendet OpenClaw den ersten konfigurierten Provider in der Auto-Auswahlreihenfolge der Registry. Legacy-`provider: "edge"` wird durch `openclaw doctor --fix` zu `"microsoft"` umgeschrieben.
    </ParamField>
    ...
    <ParamField path="summaryModel" type="string">
      Kostengünstiges Modell für Auto-Zusammenfassungen; Standard ist `agents.defaults.model.primary`. Akzeptiert `provider/model` oder einen konfigurierten Modellalias.
    </ParamField>
  ...
  <Accordion title="Azure Speech">
    <ParamField path="apiKey" type="string">Fällt auf `AZURE_SPEECH_KEY`, `AZURE_SPEECH_API_KEY` oder `SPEECH_KEY` zurück.</ParamField>
    ...
    <ParamField path="voice" type="string">Azure-Stimmen-ShortName. Standard `en-US-JennyNeural`.</ParamField>
  ...
  <Accordion title="ElevenLabs">
    ...
    <ParamField path="voiceId" type="string">ElevenLabs-Stimmen-ID.</ParamField>
    ...
    <ParamField path="languageCode" type="string">Zweibuchstabiger ISO-639-1-Code (z. B. `en`, `de`).</ParamField>
    <ParamField path="seed" type="number">Ganzzahl `0..4294967295` für möglichst deterministische Ergebnisse.</ParamField>
    <ParamField path="baseUrl" type="string">Überschreibt die ElevenLabs-API-Basis-URL.</ParamField>
  </Accordion>

  <Accordion title="Google Gemini">
    <ParamField path="apiKey" type="string">Fällt auf `GEMINI_API_KEY` / `GOOGLE_API_KEY` zurück. Wenn ausgelassen, kann TTS vor dem Fallback auf Umgebungsvariablen `models.providers.google.apiKey` wiederverwenden.</ParamField>
    <ParamField path="model" type="string">Gemini-TTS-Modell. Standard `gemini-3.1-flash-tts-preview`.</ParamField>
    <ParamField path="voiceName" type="string">Vorgefertigter Gemini-Stimmenname. Standard `Kore`. Alias: `voice`.</ParamField>
    <ParamField path="audioProfile" type="string">Natürlichsprachlicher Stil-Prompt, der dem gesprochenen Text vorangestellt wird.</ParamField>
    ...
    <ParamField path="personaPrompt" type="string">Google-spezifischer zusätzlicher Persona-Prompt-Text, der an die Regieanmerkungen der Vorlage angehängt wird.</ParamField>
  ...
  <Accordion title="Microsoft (no API key)">
    ...
    <ParamField path="edge.*" type="object" deprecated>Legacy-Alias. Führen Sie `openclaw doctor --fix` aus, um persistierte Konfiguration zu `providers.microsoft` umzuschreiben.</ParamField>
  </Accordion>
  ...
  <Accordion title="OpenAI">
    ...
    <ParamField path="extraBody / extra_body" type="Record<string, unknown>">Zusätzliche JSON-Felder, die nach generierten OpenAI-TTS-Feldern in Anfragebodies für `/audio/speech` zusammengeführt werden. Verwenden Sie dies für OpenAI-kompatible Endpunkte wie Kokoro, die Provider-spezifische Schlüssel wie `lang` erfordern; unsichere Prototypschlüssel werden ignoriert.</ParamField>
    <ParamField path="baseUrl" type="string">
      OpenAI-TTS-Endpunkt überschreiben. Auflösungsreihenfolge: Konfiguration → `OPENAI_TTS_BASE_URL` → `https://api.openai.com/v1`. Nicht standardmäßige Werte werden als OpenAI-kompatible TTS-Endpunkte behandelt, sodass benutzerdefinierte Modell- und Stimmennamen akzeptiert werden.
    </ParamField>
  </Accordion>
  ...
  <Accordion title="Volcengine (BytePlus Seed Speech)">
    ...
    <ParamField path="resourceId" type="string">Standard `seed-tts-1.0`. Env: `VOLCENGINE_TTS_RESOURCE_ID`. Verwenden Sie `seed-tts-2.0`, wenn Ihr Projekt über eine TTS-2.0-Berechtigung verfügt.</ParamField>
    <ParamField path="appKey" type="string">App-Key-Header. Standard `aGjiRDfUWi`. Env: `VOLCENGINE_TTS_APP_KEY`.</ParamField>
    <ParamField path="baseUrl" type="string">Seed-Speech-TTS-HTTP-Endpunkt überschreiben. Env: `VOLCENGINE_TTS_BASE_URL`.</ParamField>
  ...
  <Accordion title="Xiaomi MiMo">
    ...
    <ParamField path="style" type="string">Optionale Anweisung zum Stil in natürlicher Sprache, die als Benutzernachricht gesendet wird; wird nicht gesprochen.</ParamField>
  </Accordion>
</AccordionGroup>
...
Clients Untertitel bei Sprachnotizen nicht zuverlässig darstellen.
...
| `tts.setProvider` | Lokale Provider-Präferenz festlegen.       |
| `tts.setPersona`  | Lokale Persona-Präferenz festlegen.        |
