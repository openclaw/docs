...
title: Sprachausgabe
x-i18n:
    generated_at: "2026-05-06T07:07:45Z"
    model: gpt-5.5
    provider: openai
...
wählt OpenClaw den ersten konfigurierten Provider in der Auto-Select-Reihenfolge
der Registry aus. Das integrierte `tts`-Agent-Tool ist nur für explizite Absichten:
Gewöhnliche Chats bleiben Text, außer der Benutzer fragt nach Audio, verwendet
`/tts` oder aktiviert Auto-TTS/Direktivsprache.
</Note>
...
<Warning>
Der gebündelte **Microsoft**-Provider verwendet den Online-Dienst für neuronales
TTS von Microsoft Edge über `node-edge-tts`. Es handelt sich um einen öffentlichen
Webdienst ohne veröffentlichtes SLA oder Kontingent. Behandeln Sie ihn als
Best-Effort. Die Legacy-Provider-ID `edge` wird zu `microsoft` normalisiert und
`openclaw doctor --fix` schreibt persistierte Konfiguration um; neue
Konfigurationen sollten immer `microsoft` verwenden.
</Warning>
...
über Provider hinweg angewendet werden kann. Sie kann einen Provider bevorzugen, providerneutrale Prompt-
Absichten definieren und providerspezifische Bindungen für Stimmen, Modelle, Prompt-
...
- **Feishu / WhatsApp**: Wenn eine Sprachnotizantwort als MP3/WebM/WAV/M4A
  ...
  Datei als Anhang; das Senden über WhatsApp schlägt fehl, statt eine inkompatible
...
- **Lokale CLI**: verwendet das konfigurierte `outputFormat`. Sprachnotizziele werden
...
- In `mode: "final"` wird weiterhin reines Audio-TTS für gestreamte finale Antworten gesendet,
...
<AccordionGroup>
  ...
  <Accordion title="Azure Speech">
    ...
    <ParamField path="endpoint" type="string">Optionaler Azure Speech-Endpunkt-Override (Alias `baseUrl`).</ParamField>
    ...
    <ParamField path="voiceNoteOutputFormat" type="string">Azure `X-Microsoft-OutputFormat` für Sprachnotizausgabe. Standard `ogg-24khz-16bit-mono-opus`.</ParamField>
  </Accordion>
  ...
  <Accordion title="Google Gemini">
    ...
    <ParamField path="voiceName" type="string">Name der vorgefertigten Gemini-Stimme. Standard `Kore`. Alias: `voice`.</ParamField>
    ...
    <ParamField path="promptTemplate" type='"audio-profile-v1"'>Setzen Sie dies auf `audio-profile-v1`, um aktive Persona-Prompt-Felder in eine deterministische Gemini TTS-Prompt-Struktur einzubetten.</ParamField>
  ...
  <Accordion title="MiniMax">
    ...
    <ParamField path="voiceId" type="string">Standard `English_expressive_narrator`. Umgebung: `MINIMAX_TTS_VOICE_ID`.</ParamField>
    ...
    <ParamField path="pitch" type="number">Ganzzahl `-12..12`. Standard `0`. Dezimalwerte werden vor der Anfrage abgeschnitten.</ParamField>
  </Accordion>
  ...
  <Accordion title="OpenAI">
    ...
    <ParamField path="extraBody / extra_body" type="Record<string, unknown>">Zusätzliche JSON-Felder, die nach den generierten OpenAI-TTS-Feldern in `/audio/speech`-Anfragetexte zusammengeführt werden. Verwenden Sie dies für OpenAI-kompatible Endpunkte wie Kokoro, die Provider-spezifische Schlüssel wie `lang` erfordern; unsichere Prototypschlüssel werden ignoriert.</ParamField>
  ...
  <Accordion title="Volcengine (BytePlus Seed Speech)">
    ...
    <ParamField path="voice" type="string">Stimmentyp. Standard `en_female_anna_mars_bigtts`. Umgebung: `VOLCENGINE_TTS_VOICE`.</ParamField>
  ...
  <Accordion title="Xiaomi MiMo">
    ...
    <ParamField path="model" type="string">Standard `mimo-v2.5-tts`. Umgebung: `XIAOMI_TTS_MODEL`. Unterstützt auch `mimo-v2-tts`.</ParamField>
    <ParamField path="voice" type="string">Standard `mimo_default`. Umgebung: `XIAOMI_TTS_VOICE`.</ParamField>
