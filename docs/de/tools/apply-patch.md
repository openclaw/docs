...
x-i18n:
    generated_at: "2026-07-12T15:56:20Z"
    model: gpt-5.6
...
Das Tool akzeptiert eine einzelne `input`-Zeichenfolge, die einen oder mehrere Dateivorgänge umfasst:
...
- `tools.exec.applyPatch.workspaceOnly` ist standardmäßig auf `true` gesetzt (auf den Workspace beschränkt). Setzen Sie den Wert nur dann auf `false`, wenn `apply_patch` absichtlich außerhalb des Workspace-Verzeichnisses schreiben oder Dateien löschen soll.
...
- Standardmäßig für jedes Modell aktiviert. Legen Sie `tools.exec.applyPatch.enabled: false`
  fest, um das Tool zu deaktivieren, oder beschränken Sie es mit
  `tools.exec.applyPatch.allowModels` auf bestimmte Modelle (akzeptiert reine IDs wie `gpt-5.4` oder vollständige
...
<CardGroup cols={2}>
  ...
  <Card title="Ausführungs-Tool" href="/de/tools/exec" icon="terminal">
