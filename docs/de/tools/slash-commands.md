...
<AccordionGroup>
  <Accordion title="Sitzungen und Läufe">
    ...
    - `/export-trajectory [path]` fordert eine Exec-Genehmigung an und exportiert dann ein JSONL-[Trajektorienbündel](/de/tools/trajectory) für die aktuelle Sitzung. Verwenden Sie es, wenn Sie Prompt-, Tool- und Transkript-Zeitleiste für eine OpenClaw-Sitzung benötigen. In Gruppenchats gehen die Genehmigungsaufforderung und das Exportergebnis privat an den Owner. Alias: `/trajectory`.
  ...
  <Accordion title="Modell- und Laufsteuerung">
    ...
    - `/queue <mode>` verwaltet das Warteschlangenverhalten (`steer`, Legacy-`queue`, `followup`, `collect`, `steer-backlog`, `interrupt`) plus Optionen wie `debounce:0.5s cap:25 drop:summarize`; `/queue default` oder `/queue reset` löscht den Sitzungs-Override. Siehe [Befehlswarteschlange](/de/concepts/queue) und [Steuerungswarteschlange](/de/concepts/queue-steering).
    - `/steer <message>` fügt unabhängig vom `/queue`-Modus Anleitung in den aktiven Lauf der aktuellen Sitzung ein. Es startet keinen neuen Lauf, wenn die Sitzung inaktiv ist. Alias: `/tell`. Siehe [Steer](/de/tools/steer).
  ...
  <Accordion title="Schreibzugriffe nur für Owner und Administration">
  ...
  <Accordion title="Voice, TTS, Channel-Steuerung">
...
Channel um. Siehe [Channel-Docking](/de/concepts/channel-docking) für Einrichtung,
...
- `/pair [qr|status|pending|approve|cleanup|notify]` verwaltet den Ablauf für Gerätekopplung/-einrichtung. Siehe [Pairing](/de/channels/pairing).
...
- `/codex status|models|threads|resume|compact|review|diagnostics|account|mcp|skills` inspiziert und steuert den gebündelten Codex-App-Server-Harness. Siehe [Codex-Harness](/de/plugins/codex-harness).
- Nur QQBot-Befehle:
  - `/bot-ping`
  - `/bot-version`
  - `/bot-help`
...
<AccordionGroup>
  ...
  <Accordion title="Channel-spezifisches Verhalten">
  ...
  <Accordion title="Ausführlichkeit / Trace / Fast / Reasoning-Sicherheit">
...
<Note>
Überschreibungen gelten sofort für neue Konfigurationslesevorgänge, schreiben aber **nicht** in `openclaw.json`. Verwenden Sie `/debug reset`, um alle Überschreibungen zu löschen und zur Konfiguration auf Datenträger zurückzukehren.
</Note>
...
<Note>
- `/plugins list` und `/plugins show` verwenden echte Plugin-Erkennung für den aktuellen Arbeitsbereich plus Konfiguration auf Datenträger.
...
- Aktivierungs- und Deaktivierungsänderungen laden Gateway-Plugin-Laufzeitoberflächen für neue Agent-Runden per Hot-Reload neu; Installationen fordern einen Gateway-Neustart an, da sich Plugin-Quellmodule geändert haben.
...
Weitere Informationen zum vollständigen Verhalten und zu den Client-UX-Details finden Sie unter [BTW-Nebenfragen](/de/tools/btw).
