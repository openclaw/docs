...
title: CLI anbinden
x-i18n:
    generated_at: "2026-07-12T01:30:37Z"
    model: gpt-5.6
...
`openclaw attach` startet Claude Code mit einer strikt isolierten temporären MCP-Konfiguration, die an eine einzelne Gateway-Sitzung gebunden ist.
...
Das Bearer-Token wird über Umgebungsvariablen und nicht über argv übergeben. OpenClaw startet Claude Code mit `--strict-mcp-config --mcp-config <path>`, sodass MCP-Server aus der Claude-Umgebung nicht der angehängten Sitzung beitreten. Bei normalen Starts (ohne `--print-config`) wird die Berechtigung widerrufen, sobald der Claude-Code-Prozess beendet wird.
