...
x-i18n:
    generated_at: "2026-07-24T03:50:15Z"
    model: gpt-5.6
...
OpenClaw kann Bonjour (mDNS/DNS-SD) verwenden, um ein aktives Gateway (WebSocket-Endpunkt) zu erkennen. Das Durchsuchen per Multicast `local.` ist eine **reine LAN-Komfortfunktion**: Das mitgelieferte `bonjour`-Plugin übernimmt die LAN-Ankündigung, startet automatisch auf macOS-Hosts und muss unter Linux, Windows und bei containerisierten Gateway-Bereitstellungen explizit aktiviert werden. Derselbe Beacon kann zur netzwerkübergreifenden Erkennung auch über eine konfigurierte Wide-Area-DNS-SD-Domain veröffentlicht werden. Die Erkennung erfolgt nach bestem Bemühen und ersetzt **nicht** die Konnektivität über SSH oder Tailnet.

## Wide-Area Bonjour (Unicast DNS-SD) über Tailscale

Wenn sich Node und Gateway in verschiedenen Netzwerken befinden, kann Multicast-mDNS die Netzwerkgrenze nicht überschreiten. Behalten Sie dieselbe Benutzerführung für die Erkennung bei, indem Sie über Tailscale zu **Unicast DNS-SD** („Wide-Area Bonjour“) wechseln:
...
`openclaw.internal.` oben ist nur ein Beispiel — OpenClaw unterstützt jede beliebige Erkennungsdomain. iOS-/Android-Nodes durchsuchen sowohl `local.` als auch Ihre konfigurierte Wide-Area-Domain.
...
`discovery.wideArea.domain` akzeptiert außerdem als Fallback die Umgebungsvariable `OPENCLAW_WIDE_AREA_DOMAIN`, wenn kein Wert festgelegt ist.
...
Führen Sie den Befehl zunächst ohne `--apply` aus, um den Plan (Domain, Pfad der Zonendatei, erkannte Tailnet-IP, empfohlene Konfiguration) in einer Vorschau anzuzeigen, ohne etwas zu installieren.
...
| `sshPort=<port>`             | Nur im vollständigen Modus; im minimalen und deaktivierten Modus weggelassen.   |
| `cliPath=<path>`             | Nur im vollständigen Modus; im minimalen und deaktivierten Modus weggelassen.   |
...
- iOS-/Android-Nodes sollten direkte Verbindungen auf Grundlage der Erkennung als **ausschließlich TLS-basiert** behandeln und eine ausdrückliche Bestätigung durch den Benutzer verlangen, bevor sie einem erstmalig angezeigten Fingerabdruck vertrauen.

## Fehlerdiagnose unter macOS
...
Wenn das Durchsuchen funktioniert, die Auflösung jedoch fehlschlägt, liegt üblicherweise ein Problem mit einer LAN-Richtlinie oder dem mDNS-Resolver vor.

## Fehlerdiagnose in Gateway-Protokollen
...
Wenn mehrere OpenClaw-Gateways vom selben Host aus Ankündigungen senden, kann Bonjour Suffixe wie `(2)` oder `(3)` anhängen, um eindeutige Namen für Dienstinstanzen sicherzustellen. Diese Suffixe sind eine normale Konfliktauflösung und weisen nicht auf eine doppelte OCM-Überwachung hin.

Bonjour verwendet den Systemhostnamen für den angekündigten `.local`-Host, wenn er eine gültige DNS-Bezeichnung ist. Wenn der Systemhostname Leerzeichen, Unterstriche oder ein anderes für DNS-Bezeichnungen ungültiges Zeichen enthält, greift OpenClaw auf `openclaw.local` zurück. Legen Sie `OPENCLAW_MDNS_HOSTNAME=<name>` vor dem Start des Gateways fest, wenn Sie eine explizite Hostbezeichnung benötigen.

## Fehlerdiagnose auf dem iOS-Node
...
| Modus                       | Verhalten                                                                                                                               |
| --------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------- |
| `minimal` (Standard) | Nur zentrale TXT-Schlüssel; lässt `sshPort`, `cliPath`, `tailnetDns` weg.                                      |
| `full`          | Fügt `sshPort`, `cliPath`, `tailnetDns` hinzu — verwenden Sie diesen Modus, wenn Clients diese Hinweise benötigen. |
| `off`          | Unterdrückt LAN-Multicast, ohne die Aktivierung des Plugins zu ändern; Wide-Area-DNS-SD kann weiterhin veröffentlichen, wenn `discovery.wideArea.domain` festgelegt ist. |
...
Verwenden Sie die Umgebungsvariable für bereitstellungsspezifische Probleme (sicher für Docker-Images, Dienstdateien, Startskripte und einmalige Fehlerdiagnosen — die Einstellung verschwindet zusammen mit der Umgebung):
...
- Bonjour startet auf macOS-Hosts automatisch und muss andernorts explizit aktiviert werden. Wenn es deaktiviert bleibt, wird das Gateway nicht angehalten — lediglich die LAN-Multicast-Ankündigung wird übersprungen.
...
- Das Deaktivieren von Bonjour deaktiviert Wide-Area-DNS-SD nicht. Verwenden Sie die Wide-Area-Erkennung oder Tailnet, wenn sich Gateway und Node nicht im selben LAN befinden.
- Die Wiederverwendung desselben `OPENCLAW_CONFIG_DIR` außerhalb von Docker übernimmt die Richtlinie zur automatischen Deaktivierung im Container nicht dauerhaft.
- Legen Sie `OPENCLAW_DISABLE_BONJOUR=0` nur für Hostnetzwerke, macvlan oder ein anderes Netzwerk fest, in dem mDNS-Multicast nachweislich weitergeleitet wird; setzen Sie den Wert auf `1`, um die Deaktivierung zu erzwingen.
...
4. Wenn Sie das Bonjour-Plugin in Docker bewusst aktiviert und die Ankündigung mit `OPENCLAW_DISABLE_BONJOUR=0` erzwungen haben, testen Sie Multicast vom Host aus:
   ...
   Wenn beim Durchsuchen keine Ergebnisse erscheinen oder die Gateway-Protokolle wiederholte ciao-Prüffehler anzeigen, stellen Sie `OPENCLAW_DISABLE_BONJOUR=1` wieder her und verwenden Sie eine direkte Route oder eine Tailnet-Route.
...
- **Advertiser bleibt beim Prüfen/Ankündigen hängen**: Hosts mit blockiertem Multicast, Container-Bridges, WSL oder häufigen Schnittstellenwechseln können dazu führen, dass der Responder in einem nicht angekündigten Zustand verbleibt. Das Gateway bleibt über direkte, SSH-, Tailnet- oder Wide-Area-DNS-SD-Routen verfügbar; deaktivieren Sie LAN-Bonjour mit `discovery.mdns.mode: "off"` oder `OPENCLAW_DISABLE_BONJOUR=1`, wenn Multicast nicht verfügbar ist.
- **Docker-Bridge-Netzwerk**: Bonjour wird in erkannten Containern automatisch deaktiviert. Setzen Sie `OPENCLAW_DISABLE_BONJOUR=0` nur für ein Host-, macvlan- oder anderes mDNS-fähiges Netzwerk.
- **Ruhezustand/Schnittstellenwechsel**: macOS kann mDNS-Ergebnisse vorübergehend verlieren; versuchen Sie es erneut.
- **Durchsuchen funktioniert, aber die Auflösung schlägt fehl**: Verwenden Sie einfache Rechnernamen (vermeiden Sie Emojis oder Satzzeichen) und starten Sie anschließend das Gateway neu. Der Name der Dienstinstanz wird vom Hostnamen abgeleitet, sodass übermäßig komplexe Namen einige Resolver verwirren können.
...
| `openclaw plugins disable bonjour`                   | Deaktiviert die LAN-Multicast-Ankündigung durch Deaktivieren des mitgelieferten Plugins. |
| `OPENCLAW_DISABLE_BONJOUR=1` (oder `true`/`yes`/`on`)  | Deaktiviert die LAN-Multicast-Ankündigung, ohne die Plugin-Konfiguration zu ändern. |
| `OPENCLAW_DISABLE_BONJOUR=0` (oder `false`/`no`/`off`) | Erzwingt die LAN-Multicast-Ankündigung, auch innerhalb erkannter Container.        |
...
macOS-Hosts starten das mitgelieferte LAN-Erkennungs-Plugin standardmäßig automatisch. Wenn das Bonjour-Plugin aktiviert und `OPENCLAW_DISABLE_BONJOUR` nicht gesetzt ist, kündigt Bonjour auf normalen Hosts Dienste an und wird innerhalb erkannter Container (Docker, Fly.io-Maschinen und gängige Container-Runtimes) automatisch deaktiviert.
