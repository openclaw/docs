---
read_when:
    - Sie möchten eine zuverlässige Fallback-Lösung, wenn API-Provider ausfallen
    - Sie verwenden Codex CLI oder andere lokale KI-CLIs und möchten sie wiederverwenden
    - Sie möchten die MCP-Loopback-Brücke für den Zugriff auf Backend-Werkzeuge der CLI verstehen
summary: 'CLI-Backends: lokaler KI-CLI-Fallback mit optionaler MCP-Tool-Bridge'
title: CLI-Backends
x-i18n:
    generated_at: "2026-05-06T06:47:08Z"
    model: gpt-5.5
    provider: openai
...
ratenbegrenzt sind oder sich vorübergehend fehlerhaft verhalten. Dies ist bewusst konservativ:
...
Thread-/Konversationsbindung und persistenten externen Codierungssitzungen möchten, verwenden Sie stattdessen
[ACP Agents](/de/tools/acp-agents). CLI-Backends sind kein ACP.

## Einsteigerfreundlicher Schnellstart

Sie können die Codex CLI **ohne jegliche Konfiguration** verwenden (das gebündelte OpenAI-Plugin
...
- Wenn der primäre Provider fehlschlägt (Auth, Ratenlimits, Timeouts), versucht OpenClaw
...
1. **Wählt ein Backend** anhand des Provider-Präfixes (`codex-cli/...`) aus.
...
3. **Führt die CLI** mit einer Sitzungs-ID aus (falls unterstützt), damit der Verlauf konsistent bleibt.
   Das gebündelte Backend `claude-cli` hält pro OpenClaw-Sitzung einen Claude-stdio-Prozess aktiv
...
haben uns mitgeteilt, dass OpenClaw-artige Claude-CLI-Nutzung wieder erlaubt ist, daher behandelt OpenClaw
...
Flag `--append-system-prompt` bereit, daher schreibt OpenClaw den zusammengesetzten Prompt für jede
...
nur die berechtigten Skills für diesen Agenten/diese Sitzung, sodass der native Skill-Resolver von Claude Code
dieselbe gefilterte Menge sieht, die OpenClaw sonst im
Prompt bewerben würde. Skill-Env-/API-Schlüssel-Overrides werden weiterhin von OpenClaw auf die
Child-Process-Umgebung für den Lauf angewendet.

Claude CLI hat außerdem einen eigenen nichtinteraktiven Berechtigungsmodus. OpenClaw ordnet diesen
der bestehenden Exec-Richtlinie zu, statt Claude-spezifische Konfiguration hinzuzufügen: Wenn die
...
für nicht-deaktivierte Stufen dem nativen Claude-Code-Flag `--effort` zu. `minimal` und
`low` werden `low` zugeordnet, `adaptive` und `medium` werden `medium` zugeordnet, und `high`,
`xhigh` und `max` werden direkt zugeordnet. Andere CLI-Backends benötigen ihr zugehöriges Plugin, um
...
- `claude-cli` verwendet standardmäßig `liveSession: "claude-stdio"`, `output: "jsonl"` und
  `input: "stdin"`, sodass Folgeturns den laufenden Claude-Prozess wiederverwenden, solange
  ...
  beendet wird, setzt OpenClaw von der gespeicherten Claude-Sitzungs-ID fort. Gespeicherte Sitzungs-IDs
  werden vor dem Fortsetzen gegen ein vorhandenes lesbares Projekttranskript geprüft,
...
- Claude-Live-Sitzungen behalten begrenzte JSONL-Ausgabeschutzmechanismen. Standardwerte erlauben bis zu
  ...
  Zeilen.
- Gespeicherte CLI-Sitzungen sind Provider-eigene Kontinuität. Der implizite tägliche Sitzungsreset
  trennt sie nicht; `/reset` und explizite `session.reset`-Richtlinien tun dies weiterhin.
...
- OpenClaw verwirft die Wiederverwendung gespeicherter CLI-Sitzungen, wenn sich die ausgewählte Auth-Identität ändert,
  ...
  Kontoidentität, wenn die CLI eine offenlegt. Rotation von OAuth-Zugriffs- und Refresh-Tokens
  trennt die gespeicherte CLI-Sitzung nicht. Wenn eine CLI keine
  stabile OAuth-Konto-ID offenlegt, lässt OpenClaw diese CLI die Resume-Berechtigungen durchsetzen.

## Fallback-Präludium aus claude-cli-Sitzungen

Wenn ein `claude-cli`-Versuch auf einen Nicht-CLI-Kandidaten in
[`agents.defaults.model.fallbacks`](/de/concepts/model-failover) ausweicht, versieht OpenClaw
den nächsten Versuch mit einem Kontext-Präludium, das aus dem lokalen
JSONL-Transkript von Claude Code unter `~/.claude/projects/` gewonnen wird. Ohne diesen Seed würde der Fallback-
...
- Das Präludium bevorzugt die neueste `/compact`-Zusammenfassung oder den Marker `compact_boundary`
  und hängt dann die jüngsten post-boundary Turns bis zu einem Zeichenbudget an.
...
- Der Seed verwendet die vorhandene Validierung des Claude-Sitzungsdateipfads wieder, sodass
...
Dateipfade an den Prompt an (Pfadinjektion), was für CLIs genügt, die lokale Dateien aus
Klartextpfaden automatisch laden.
...
Für CLIs, die Claude-Code-`stream-json`-kompatibles JSONL ausgeben, setzen Sie
...
- `codex-cli`: Inline-Konfigurationsüberschreibungen für `mcp_servers`; der generierte
  OpenClaw-Loopback-Server wird mit Codexs Tool-Genehmigungsmodus pro Server markiert,
  damit MCP-Aufrufe nicht durch lokale Genehmigungs-Prompts blockieren können
...
- beschränkt den Toolzugriff auf die aktuelle Sitzung, das Konto und den Kanalkontext
- lädt aktivierte Bundle-MCP-Server für den aktuellen Workspace
- führt sie mit vorhandenen Backend-MCP-Konfigurations-/Einstellungsstrukturen zusammen
- schreibt die Startkonfiguration mit dem Backend-eigenen Integrationsmodus aus der zuständigen Erweiterung um
...
Minuten; auf `0` setzen, um dies zu deaktivieren). Einmalige eingebettete Läufe wie Authentifizierungsprüfungen,
Slug-Generierung und Active-Memory-Abruf fordern die Bereinigung am Laufende an, damit stdio-
...
- **Codex-CLI-Sitzungen** werden über Textausgabe fortgesetzt (kein JSONL), was weniger
  strukturiert ist als der anfängliche `--json`-Lauf. OpenClaw-Sitzungen funktionieren trotzdem
  normal.
