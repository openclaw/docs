---
read_when:
    - Sie möchten die Modellnutzung, den Nachrichtenfluss oder Sitzungsmetriken von OpenClaw an einen OpenTelemetry Collector senden
...
summary: OpenClaw-Diagnosedaten über das diagnostics-otel-Plugin (OTLP/HTTP) in einen beliebigen OpenTelemetry-Collector exportieren
title: OpenTelemetry-Export
x-i18n:
    generated_at: "2026-05-06T06:49:27Z"
    model: gpt-5.5
    provider: openai
...
akzeptiert, funktioniert ohne Codeänderungen. Informationen zu lokalen Datei-Logs
...
- **Diagnoseereignisse** sind strukturierte, prozessinterne Datensätze, die vom
  ...
  Warteschlangen und Ausführung ausgegeben werden.
...
- Exporter werden nur eingebunden, wenn sowohl die Diagnoseoberfläche als auch
...
| Signal      | Inhalt                                                                                                                                                                      |
| ----------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Metriken** | Zähler und Histogramme für Token-Nutzung, Kosten, Laufdauer, Nachrichtenfluss, Warteschlangen-Lanes, Sitzungszustand, Ausführung und Speicherdruck.                       |
| **Traces**  | Spans für Modellnutzung, Modellaufrufe, Harness-Lebenszyklus, Tool-Ausführung, Ausführung, Webhook-/Nachrichtenverarbeitung, Kontextaufbau und Tool-Schleifen.            |
...
sind standardmäßig aktiviert, wenn `diagnostics.otel.enabled` auf `true` gesetzt
ist.
...
| `OTEL_SEMCONV_STABILITY_OPT_IN`                                                                                   | Auf `gen_ai_latest_experimental` setzen, um das neueste experimentelle GenAI-Span-Attribut (`gen_ai.provider.name`) statt des veralteten `gen_ai.system` auszugeben. GenAI-Metriken verwenden unabhängig davon immer begrenzte semantische Attribute mit niedriger Kardinalität. |
| `OPENCLAW_OTEL_PRELOADED`                                                                                         | Auf `1` setzen, wenn ein anderer Preload oder Host-Prozess bereits das globale OpenTelemetry-SDK registriert hat. Das Plugin überspringt dann seinen eigenen NodeSDK-Lebenszyklus, verbindet aber weiterhin Diagnose-Listener und berücksichtigt `traces`/`metrics`/`logs`. |

## Datenschutz und Inhaltserfassung

Rohinhalte von Modellen und Tools werden standardmäßig **nicht** exportiert.
Spans enthalten begrenzte Bezeichner (Kanal, Provider, Modell, Fehlerkategorie,
...
Modellaufruf generiert. Vorhandene vom Aufrufer bereitgestellte
...
System-Prompt-Text freigegeben sind. Jeder Unterschlüssel ist separat optional:
...
diese Klasse begrenzte, redigierte Attribute unter `openclaw.content.*`.
...
- **Logs:** OTLP-Logs berücksichtigen `logging.level` (Datei-Log-Level). Sie
...
- **Datei-Log-Korrelation:** JSONL-Datei-Logs enthalten auf oberster Ebene
  `traceId`, `spanId`, `parentSpanId` und `traceFlags`, wenn der Log-Aufruf einen
  gültigen Diagnose-Trace-Kontext mitführt. Dadurch können Log-Prozessoren
  lokale Log-Zeilen mit exportierten Spans verknüpfen.
- **Anfragekorrelation:** Gateway-HTTP-Anfragen und WebSocket-Frames erstellen
  einen internen Anfrage-Trace-Bereich. Logs und Diagnoseereignisse innerhalb
  dieses Bereichs erben standardmäßig den Anfrage-Trace, während Agentenlauf- und
...
- `openclaw.model_call.request_bytes` (Histogramm, UTF-8-Bytegröße der endgültigen Modellanfrage-Payload; kein Rohinhalt der Payload)
- `openclaw.model_call.response_bytes` (Histogramm, UTF-8-Bytegröße gestreamter Modellantwortereignisse; kein Rohinhalt der Antwort)
...
Diagnosen zur Sitzungslebendigkeit. Eine `processing`-Sitzung altert nicht in
Richtung dieses Grenzwerts, während OpenClaw Antwort-, Tool-, Status-, Block-
...
- `session.stalled`: aktive Arbeit ist vorhanden, aber der aktive Lauf hat keinen
  aktuellen Fortschritt gemeldet. Angehaltene eingebettete Läufe bleiben zunächst
  rein beobachtbar und werden dann nach `diagnostics.stuckSessionAbortMs` ohne Fortschritt
  abgebrochen und geleert, damit eingereihte
  Turns hinter der Lane fortgesetzt werden können. Wenn nicht gesetzt, verwendet der Abbruchschwellenwert standardmäßig
  das sicherere erweiterte Fenster von mindestens 10 Minuten und 5x
  `diagnostics.stuckSessionWarnMs`.
- `session.stuck`: veraltete Sitzungsbuchführung ohne aktive Arbeit. Dies gibt
...
`session.recovery.completed` aus. Der diagnostische Sitzungszustand wird erst nach
einem verändernden Wiederherstellungsergebnis (`aborted` oder `released`) und nur dann als inaktiv
...
aus. Wiederholte `session.stuck`-Diagnosen werden zurückgestellt, solange die Sitzung
unverändert bleibt. Dashboards sollten daher auf anhaltende Anstiege statt auf jeden
Heartbeat-Tick alarmieren. Den Konfigurationsregler und die Standardwerte finden Sie in der
...
- `openclaw.context.assembled`
  - `openclaw.prompt.size`, `openclaw.history.size`, `openclaw.context.tokens`, `openclaw.errorCategory` (kein Prompt-, Verlauf-, Antwort- oder Sitzungsschlüsselinhalt)
- `openclaw.tool.loop`
...
Wenn Inhaltserfassung explizit aktiviert ist, können Modell- und Tool-Spans außerdem
begrenzte, redigierte Attribute `openclaw.content.*` für die spezifischen
Inhaltsklassen enthalten, für die Sie sich entschieden haben.

## Diagnoseereigniskatalog

Die folgenden Ereignisse bilden die Grundlage für die obigen Metriken und Spans. Plugins können diese auch direkt abonnieren,
ohne OTLP-Export.
...
Flags. Flags sind unabhängig von Groß-/Kleinschreibung und unterstützen Platzhalter (z. B. `telegram.*` oder
`*`):
...
- [Protokollierung](/de/logging) - Dateilogs, Konsolenausgabe, CLI-Tailing und der Logs-Tab der Control UI
- [Gateway-Protokollierungsinterna](/de/gateway/logging) - WS-Logstile, Subsystem-Präfixe und Konsolenerfassung
...
- [Konfigurationsreferenz](/de/gateway/configuration-reference#diagnostics) - vollständige Referenz der Felder `diagnostics.*`
