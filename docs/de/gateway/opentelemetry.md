---
read_when:
    - Sie möchten die OpenClaw-Modellnutzung, den Nachrichtenfluss oder Sitzungsmetriken an einen OpenTelemetry-Collector senden
...
akzeptiert, funktioniert ohne Codeänderungen. Informationen zu lokalen
Datei-Logs und dazu, wie Sie sie lesen, finden Sie unter
[Logging](/de/logging).
...
- **Provider-Aufrufe** erhalten von OpenClaw einen W3C-`traceparent`-Header aus
  dem vertrauenswürdigen Span-Kontext des Modellaufrufs, wenn der
...
- Exporter werden nur angehängt, wenn sowohl die Diagnoseoberfläche als auch das
  Plugin aktiviert sind, sodass die prozessinternen Kosten standardmäßig nahezu
  null bleiben.
...
| ----------- | ------------------------------------------------------------------------------------------------------------------------------------------ |
| **Metriken** | Zähler und Histogramme für Token-Nutzung, Kosten, Laufdauer, Nachrichtenfluss, Warteschlangen-Lanes, Sitzungszustand, Exec und Speicherdruck.          |
...
| `OPENCLAW_OTEL_PRELOADED`                                                                                         | Auf `1` setzen, wenn ein anderes Preload oder ein Hostprozess bereits das globale OpenTelemetry-SDK registriert hat. Das Plugin überspringt dann seinen eigenen NodeSDK-Lebenszyklus, verdrahtet aber weiterhin Diagnose-Listener und berücksichtigt `traces`/`metrics`/`logs`.                |

## Datenschutz und Inhaltserfassung

Rohe Modell-/Tool-Inhalte werden standardmäßig **nicht** exportiert. Spans
enthalten begrenzte Kennungen (Kanal, Provider, Modell, Fehlerkategorie,
...
- `toolInputs` — Tool-Argument-Nutzdaten.
- `toolOutputs` — Tool-Ergebnis-Nutzdaten.
...
- **Logs:** OTLP-Logs beachten `logging.level` (Datei-Log-Level). Sie verwenden
...
- **Anfragekorrelation:** Gateway-HTTP-Anfragen und WebSocket-Frames erzeugen
...
- `openclaw.model_call.request_bytes` (Histogramm, UTF-8-Bytegröße der finalen Modellanfrage-Nutzdaten; keine rohen Nutzdateninhalte)
- `openclaw.model_call.response_bytes` (Histogramm, UTF-8-Bytegröße gestreamter Modellantwortereignisse; keine rohen Antwortinhalte)
...
Fortschritt, sodass ein stilles Modell oder Harness weiterhin erkannt werden
kann.
...
- `session.stalled`: aktive Arbeit ist vorhanden, aber der aktive Lauf hat
  kürzlich keinen Fortschritt gemeldet. Angehaltene eingebettete Läufe bleiben
  zunächst im reinen Beobachtungsmodus und wechseln dann nach mindestens 10 Minuten und 5x `diagnostics.stuckSessionWarnMs`
  ohne Fortschritt zu abort-drain, damit wartende Turns hinter der Lane fortgesetzt werden können.
- `session.stuck`: veraltete Sitzungsbuchführung ohne aktive Arbeit. Dadurch wird
...
unverändert bleibt; Dashboards sollten daher auf anhaltende Anstiege und nicht auf jeden
...
Für gezielte Debug-Ausgaben ohne Anheben von `logging.level` verwenden Sie Diagnose-
...
Flag-Ausgaben gehen in die Standardprotokolldatei (`logging.file`) und werden weiterhin
...
- [Protokollierung](/de/logging) — Dateiprotokolle, Konsolenausgabe, CLI-Tailing und der Logs-Tab der Control UI
- [Gateway-Protokollierungsinternals](/de/gateway/logging) — WS-Protokollstile, Subsystem-Präfixe und Konsolenerfassung
