...
- Verwenden Sie die separate `clawhub` CLI für Registry-Authentifizierung, Veröffentlichung und Lösch-/Wiederherstellungs-Workflows.
...
| Bereich        | Was gespeichert wird                                         | Typischer Befehl                             |
| -------------- | ------------------------------------------------------------ | -------------------------------------------- |
| Skills         | Versionierte Text-Bundles mit `SKILL.md` plus unterstützenden Dateien | `openclaw skills install @openclaw/demo`     |
...
Downloads, Sterne und Zusammenfassungen von Sicherheits-Scans. Öffentliche Seiten zeigen den aktuellen Registry-
...
Plugin-Installationen validieren die beworbene `pluginApi`- und `minGatewayVersion`-
...
Die CLI verfügt außerdem über Skill-Installations-/Aktualisierungsbefehle für direkte Registry-Workflows:
...
ClawHub führt automatisierte Prüfungen für veröffentlichte Skills und Plugin-Releases aus. Scan-zurückgehaltene
...
Wenn Sie `clawhub install` ausführen, während Sie angemeldet sind, kann die CLI nach bestem Bemühen ein
...
| Variable                      | Wirkung                                           |
| ----------------------------- | ------------------------------------------------- |
| `CLAWHUB_SITE`                | Überschreibt die Website-URL, die für die Browser-Anmeldung verwendet wird. |
