...
- Verwenden Sie die separate `clawhub`-CLI für Registry-Authentifizierung, Veröffentlichung und Lösch-/Wiederherstellungsworkflows.
...
| -------------- | ------------------------------------------------------------ | -------------------------------------------- |
| Skills         | Versionierte Text-Bundles mit `SKILL.md` und unterstützenden Dateien | `openclaw skills install @openclaw/demo`     |
...
Native OpenClaw-Befehle installieren in den aktiven OpenClaw-Workspace und speichern
...
Plugin-Spezifikationen ohne Präfix, die npm-sicher sind, können während Launch-Umstellungen über npm aufgelöst werden, und
...
den ClawHub-Digest-Header und die heruntergeladenen Bytes und speichert Artefaktmetadaten für
...
und erfassen installierte Versionen in `.clawhub/lock.json`.
...
ClawHub führt automatisierte Prüfungen für veröffentlichte Skills und Plugin-Releases aus. Wegen Scans zurückgehaltene
oder blockierte Releases können aus dem öffentlichen Katalog und den Installationsoberflächen verschwinden, bleiben
aber für ihre Besitzer in `/dashboard` sichtbar.
...
[Sicherheitsprüfungen](/de/clawhub/security-audits),
[Moderation und Kontosicherheit](/de/clawhub/moderation) und
[Zulässige Nutzung](/de/clawhub/acceptable-usage) für Details zu Richtlinien und Durchsetzung.

## Telemetrie und Umgebung

Wenn Sie `clawhub install` ausführen, während Sie angemeldet sind, kann die CLI nach bestem Aufwand ein
...
| Variable                      | Wirkung                                           |
| ----------------------------- | ------------------------------------------------- |
| `CLAWHUB_SITE`                | Überschreibt die für den Browser-Login verwendete Website-URL. |
