...
read_when:
    ...
    - ClawHub hat die Veröffentlichung eines Plugin-Pakets abgelehnt oder mit einer Warnung versehen
...
x-i18n:
    generated_at: "2026-06-30T22:12:35Z"
    model: gpt-5.5
...
automatisierten Paketscans anzeigen. Diese Seite behandelt autorenseitige Befunde,
also Befunde, die der Plugin-Autor in den Paketmetadaten, im Manifest, in SDK-
Imports oder im veröffentlichten Artefakt seines Pakets beheben kann.

Sie behandelt keine internen Abdeckungsbefunde des Plugin Inspector. Wenn ein
vollständiger Bericht Scanner-Wartungscodes ohne Hinweise zur Behebung durch
...
| `sdk-load-session-store`                | [Zugriff auf den gesamten Sitzungs-Store ersetzen](/de/clawhub/plugin-validation-fixes#sdk-load-session-store)                                   |
| `sdk-session-store-write`               | [Schreibzugriffe auf den gesamten Sitzungs-Store ersetzen](/de/clawhub/plugin-validation-fixes#sdk-session-store-write)                                  |
...
Der Paket-Root enthält keine `package.json`, daher kann ClawHub das npm-Paket,
die Version, Einstiegspunkte oder OpenClaw-Metadaten nicht identifizieren.
...
- Verwenden Sie [Plugins erstellen](/de/plugins/building-plugins) für ein minimales
...
Das Paket hat eine `package.json`, deklariert aber keine OpenClaw-
Paketmetadaten.
...
fehlt in dem validierten Paket.
...
- Verwenden Sie die OpenClaw-Plugin-API-Version oder die semver-Untergrenze, gegen
  die Sie gebaut und getestet haben.
...
Die minimale Hostversion des Pakets stimmt nicht mit den OpenClaw-
Versionsmetadaten überein, gegen die das Paket gebaut wurde.

- Prüfen Sie `openclaw.install.minHostVersion`.
- Prüfen Sie alle OpenClaw-Build-Metadaten im Paket, etwa die während des
  Releases verwendete OpenClaw-Version.
- Gleichen Sie die minimale Hostversion an den Hostversionsbereich an, den das
...
- Wenn `openclaw.plugin.json` ebenfalls `version` enthält, aktualisieren Sie sie
  entsprechend oder entfernen Sie veraltete Manifestversionsmetadaten, wenn die
...
Der Block `package.json#openclaw` enthält Felder, die nicht als OpenClaw-
Paketmetadaten unterstützt werden.
...
- Bewahren Sie Paketeinstiegspunkte, Kompatibilität, Installation, Setup und
  Katalogmetadaten in unterstützten `package.json#openclaw`-Feldern auf.
- Siehe [package.json-Felder, die die Erkennung beeinflussen](/de/plugins/manifest#packagejson-fields-that-affect-discovery).
- Führen Sie `clawhub package validate <path-to-plugin>` erneut aus.
...
- Beheben Sie ungültige Paketmetadaten, defekte Lifecycle-Skripte oder
  Dateieinträge, durch die das Packen fehlschlägt.
- Entfernen Sie `private: true`, wenn dieses Paket für die öffentliche
...
- Aktualisieren Sie `files`, `.npmignore` oder die Build-Ausgabe, damit
...
- Halten Sie `name` menschenlesbar und behalten Sie `id` als stabile maschinenlesbare ID bei.
- Siehe [Plugin-Manifest](/de/plugins/manifest).
- Führen Sie `clawhub package validate <path-to-plugin>` erneut aus.
...
Das Plugin verwendet noch einen veralteten Schreibhelfer für den gesamten Sitzungsspeicher, etwa
`saveSessionStore` oder `updateSessionStore`.

- Verwenden Sie `patchSessionEntry(...)`, wenn Sie Felder in einem vorhandenen Sitzungseintrag
  aktualisieren.
...
Das Plugin verwendet noch veraltete Helfer für Sitzungdateipfade, etwa
...
- Verwenden Sie `patchSessionEntry(...)` oder `upsertSessionEntry(...)`, um Sitzungsmetadaten
  zu speichern.
- Verwenden Sie Transkriptidentität oder Ziel-Helfer, wenn der Code eine
...
- Verwenden Sie `resolveSessionTranscriptIdentity(...)`, wenn der Code nur eine öffentliche
...
Das Plugin verwendet noch veraltete Low-Level-Transkripthelfer, etwa
...
Das Manifest verwendet noch veraltete Provider-Authentifizierungsmetadaten `providerAuthEnvVars`.
...
- Spiegeln Sie env-gesteuertes Channel-Setup in die aktuelle Setup-, Channel-Konfigurations- oder
  Paket-Channel-Metadatenstruktur, die Ihre Plugin-Form verwendet.
...
- Dokumentieren Sie sicherheitssensibles Verhalten in Ihrer öffentlichen Paketdokumentation oder
  README, bis der Manifest-Contract existiert.
- Führen Sie `clawhub package validate <path-to-plugin>` erneut aus.
