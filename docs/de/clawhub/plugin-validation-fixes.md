...
read_when:
    ...
    - ClawHub hat beim Veröffentlichen eines Plugin-Pakets abgelehnt oder gewarnt
...
x-i18n:
    generated_at: "2026-06-30T13:52:55Z"
    model: gpt-5.5
...
Sie behandelt keine internen Abdeckungsbefunde des Plugin Inspectors. Wenn ein vollständiger Bericht
...
| `package-openclaw-metadata-missing`     | [Den package-Block `openclaw` hinzufügen](/de/clawhub/plugin-validation-fixes#package-openclaw-metadata-missing)                            |
...
| `sdk-session-file-helper`               | [Session-Dateipfad-Hilfsfunktionen ersetzen](/de/clawhub/plugin-validation-fixes#sdk-session-file-helper)                                   |
...
- Füllen Sie `openclaw.install` mit der unterstützten Installationsquelle, z. B.
...
- Prüfen Sie alle OpenClaw-Build-Metadaten im Paket, z. B. die während der Veröffentlichung
...
- Beheben Sie ungültige Paketmetadaten, defekte Lifecycle-Skripte oder `files`-Einträge, durch die
...
- Verschieben Sie Paket- oder Installationsmetadaten stattdessen in unterstützte Felder unter `package.json#openclaw`
...
- Verschieben Sie Laufzeitverhalten in den Plugin-Registrierungscode, und beschränken Sie `contracts`
  auf statische Metadaten zur Zuständigkeit für Capabilities.
- Führen Sie `clawhub package validate <path-to-plugin>` erneut aus.
...
- Verwenden Sie Transcript-Identität oder Target-Hilfsfunktionen, wenn der Code eine
...
Das Plugin verwendet weiterhin die veraltete Hilfsfunktion für Transcript-Datei-Targets
...
- Verwenden Sie `resolveSessionTranscriptTarget(...)`, wenn der Code ein strukturiertes
  Target für Transcript-Operationen benötigt.
- Vermeiden Sie es, veraltete Transcript-Datei-Targets direkt zu lesen oder zu konstruieren.
...
- Verwenden Sie `appendSessionTranscriptMessageByIdentity(...)` für Transcript-Appends.
...
- Bevorzugen Sie die strukturierte Transcript-Laufzeitoberfläche, damit OpenClaw die
  korrekten Transaktionsgrenzen und das korrekte Identitäts-Handling anwenden kann.
...
Das Plugin verwendet weiterhin den alten Hook `before_agent_start`.

- Verschieben Sie Arbeiten an Modell- oder Provider-Overrides nach `before_model_resolve`.
- Verschieben Sie Arbeiten an Prompt- oder Context-Mutationen nach `before_prompt_build`.
...
Das Manifest verwendet weiterhin alte `providerAuthEnvVars`-Metadaten für Provider-Authentifizierung.
...
- Spiegeln Sie env-gesteuertes Channel-Setup in das aktuelle Setup, die Channel-Config oder
  die Paket-Channel-Metadaten, die von Ihrer Plugin-Form verwendet werden.
...
- Halten Sie sicherheitsrelevantes Verhalten in Ihren öffentlichen Paketdokumenten oder
  im README dokumentiert, bis der Manifest-Contract existiert.
- Führen Sie `clawhub package validate <path-to-plugin>` erneut aus.
