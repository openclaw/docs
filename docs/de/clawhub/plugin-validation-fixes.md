...
title: Korrekturen der Plugin-Validierung
x-i18n:
    generated_at: "2026-07-01T18:10:19Z"
    model: gpt-5.5
...
automatisierten Paketscans anzeigen. Diese Seite behandelt autorbezogene Ergebnisse, also
Ergebnisse, die Plugin-Autoren in ihren Paketmetadaten, ihrem Manifest, ihren SDK-
Importen oder ihrem veröffentlichten Artefakt beheben können.

Sie behandelt keine internen Abdeckungsergebnisse des Plugin Inspector. Wenn ein vollständiger Bericht
...
| `sdk-session-store-write`               | [Schreibvorgänge auf den gesamten Session Store ersetzen](/de/clawhub/plugin-validation-fixes#sdk-session-store-write)                                  |
...
Das Paket enthält `package.json`, deklariert jedoch keine OpenClaw-Paketmetadaten.
...
- Fügen Sie `openclaw.runtimeExtensions` hinzu, wenn das veröffentlichte Paket gebaute
  JavaScript-Dateien laden soll.
...
- Verwenden Sie die OpenClaw-Plugin-API-Version oder die Semver-Untergrenze, gegen die Sie entwickelt und getestet haben.
- Halten Sie dies getrennt von der Paketversion. Die Paketversion beschreibt die
  Plugin-Veröffentlichung; `openclaw.compat.pluginApi` beschreibt den Host-API-Vertrag.
- Siehe [package.json-Felder, die die Erkennung beeinflussen](/de/plugins/manifest#packagejson-fields-that-affect-discovery).
- Führen Sie `clawhub package validate <path-to-plugin>` erneut aus.
...
- Prüfen Sie alle OpenClaw-Build-Metadaten im Paket, beispielsweise die während der Veröffentlichung verwendete OpenClaw-Version.
...
- Wenn `openclaw.plugin.json` ebenfalls `version` enthält, aktualisieren Sie sie entsprechend oder entfernen Sie
  veraltete Manifestversions-Metadaten, wenn Paketmetadaten maßgeblich sind.
...
- Halten Sie Paket-Einstiegspunkte, Kompatibilität, Installation, Setup und Katalogmetadaten
...
- Führen Sie `npm pack --dry-run` vom Paket-Root aus.
...
- Verschieben Sie Paket- oder Installationsmetadaten stattdessen in unterstützte Felder
...
- Vergleichen Sie jeden Schlüssel unter `contracts` mit der
  [Referenz zu Contracts](/de/plugins/manifest#contracts-reference).
...
- Verschieben Sie Laufzeitverhalten in den Plugin-Registrierungscode und beschränken Sie `contracts`
  auf statische Metadaten zum Besitz von Capabilities.
- Führen Sie `clawhub package validate <path-to-plugin>` erneut aus.
...
Das Plugin verwendet weiterhin den veralteten Helper für den gesamten Session Store
`loadSessionStore`.
...
- Verwenden Sie `patchSessionEntry(...)` oder `upsertSessionEntry(...)`, wenn Sie Session-
  Status schreiben.
- Vermeiden Sie das Laden, Ändern und Speichern des gesamten Session-Store-Objekts.
- Behalten Sie `loadSessionStore(...)` nur bei, solange Ihr deklarierter Kompatibilitätsbereich
  weiterhin ältere OpenClaw-Versionen unterstützt, die dies erfordern.
...
Das Plugin verwendet weiterhin einen veralteten Schreib-Helper für den gesamten Session Store, z. B.
`saveSessionStore` oder `updateSessionStore`.

- Verwenden Sie `patchSessionEntry(...)`, wenn Sie Felder in einem vorhandenen Session-
  Eintrag aktualisieren.
- Verwenden Sie `upsertSessionEntry(...)`, wenn Sie einen Session-Eintrag ersetzen oder erstellen.
- Vermeiden Sie das Laden, Ändern und Speichern des gesamten Session-Store-Objekts.
- Behalten Sie Schreib-Helper für den gesamten Store nur bei, solange Ihr deklarierter Kompatibilitätsbereich
  weiterhin ältere OpenClaw-Versionen unterstützt, die sie erfordern.
- Siehe [Runtime API](/de/plugins/sdk-runtime#agent-session-state) und
  [Plugin-SDK-Subpaths](/de/plugins/sdk-subpaths).
- Führen Sie `clawhub package validate <path-to-plugin>` erneut aus.
...
Das Plugin verwendet weiterhin veraltete Session-Dateipfad-Helper wie
...
- Verwenden Sie Transcript-Identität oder Target-Helper, wenn der Code einen
  Transcript-Vorgang vorbereitet.
- Speichern Sie keine veralteten Transcript-Dateipfade und verlassen Sie sich nicht darauf.
- Siehe [Runtime API](/de/plugins/sdk-runtime#agent-session-state) und
  [Plugin-SDK-Subpaths](/de/plugins/sdk-subpaths).
- Führen Sie `clawhub package validate <path-to-plugin>` erneut aus.
...
Das Plugin verwendet weiterhin den veralteten Transcript-Dateiziel-Helper
...
- Behalten Sie den Legacy-Helper nur bei, solange Ihr deklarierter Kompatibilitätsbereich weiterhin
...
Das Plugin verwendet weiterhin veraltete Low-Level-Transcript-Helper wie
...
- Verwenden Sie `publishSessionTranscriptUpdateByIdentity(...)` für Benachrichtigungen zu Transcript-Updates.
...
- Behalten Sie Low-Level-Transcript-Helper nur bei, solange Ihr deklarierter Kompatibilitätsbereich
  weiterhin ältere OpenClaw-Versionen unterstützt, die sie erfordern.
- Siehe [Runtime API](/de/plugins/sdk-runtime#agent-session-state) und
  [Plugin-SDK-Subpaths](/de/plugins/sdk-subpaths).
- Führen Sie `clawhub package validate <path-to-plugin>` erneut aus.
...
- Behalten Sie `before_agent_start` nur bei, solange Ihr deklarierter Kompatibilitätsbereich weiterhin
  ältere OpenClaw-Versionen unterstützt, die ihn erfordern.
