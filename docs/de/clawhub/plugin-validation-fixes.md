...
x-i18n:
    generated_at: "2026-07-01T05:36:45Z"
    model: gpt-5.5
...
automatisierten Paket-Scans anzeigen. Diese Seite behandelt Befunde für Autorinnen
und Autoren, also Befunde, die Plugin-Autorinnen und -Autoren in ihren
Paketmetadaten, ihrem Manifest, ihren SDK-Imports oder ihrem veröffentlichten
Artefakt beheben können.
...
Autorinnen und Autoren enthält, sind diese für OpenClaw-Maintainer und nicht für
Plugin-Autorinnen und -Autoren bestimmt.
...
- Fügen Sie einen `openclaw`-Block hinzu, wenn das Paket ein OpenClaw-Plugin
  ausliefert.
...
fehlt in dem Paket, das validiert wird.
...
- Füllen Sie `openclaw.install` mit der unterstützten Installationsquelle, zum
  Beispiel `clawhubSpec`, `npmSpec` oder `localPath`.
...
Die Mindestversion des Paket-Hosts stimmt nicht mit den
OpenClaw-Versionsmetadaten überein, gegen die das Paket gebaut wurde.

- Prüfen Sie `openclaw.install.minHostVersion`.
- Prüfen Sie alle OpenClaw-Buildmetadaten im Paket, zum Beispiel die während des
  Releases verwendete OpenClaw-Version.
- Gleichen Sie die Mindestversion des Hosts an den Hostversionsbereich an, den
  das Paket tatsächlich unterstützt.
- Siehe [package.json-Felder, die die Erkennung beeinflussen](/de/plugins/manifest#packagejson-fields-that-affect-discovery).
- Führen Sie `clawhub package validate <path-to-plugin>` erneut aus.
...
- Bewahren Sie Paket-Einstiegspunkte, Kompatibilitäts-, Installations-, Setup-
  und Katalogmetadaten in unterstützten `package.json#openclaw`-Feldern auf.
- Siehe [package.json-Felder, die die Erkennung beeinflussen](/de/plugins/manifest#packagejson-fields-that-affect-discovery).
- Führen Sie `clawhub package validate <path-to-plugin>` erneut aus.
...
- Aktualisieren Sie `files`, `.npmignore` oder die Build-Ausgabe, sodass
...
- Verschieben Sie Runtime-Verhalten in den Plugin-Registrierungscode und beschränken Sie `contracts`
...
- Verwenden Sie Transkriptidentität oder Target-Hilfsfunktionen, wenn der Code eine
  Transkriptoperation vorbereitet.
- Persistieren Sie keine veralteten Transkript-Dateipfade und verlassen Sie sich nicht darauf.
- Siehe [Runtime-API](/de/plugins/sdk-runtime#agent-session-state) und
  [Plugin-SDK-Subpaths](/de/plugins/sdk-subpaths).
- Führen Sie `clawhub package validate <path-to-plugin>` erneut aus.
...
Das Plugin verwendet noch die veraltete Hilfsfunktion für Transkript-Datei-Targets
...
- Verwenden Sie `resolveSessionTranscriptTarget(...)`, wenn der Code ein strukturiertes
  Target für eine Transkriptoperation benötigt.
- Vermeiden Sie es, veraltete Transkript-Datei-Targets direkt zu lesen oder zu konstruieren.
...
- Verschieben Sie Arbeiten zur Mutation von Prompt oder Kontext nach `before_prompt_build`.
...
Das Manifest verwendet noch veraltete `providerAuthEnvVars`-Provider-Auth-Metadaten.
...
- Halten Sie Channel-Env-Var-Metadaten deklarativ, damit OpenClaw den Setup-Status
  prüfen kann, ohne die Channel-Runtime zu laden.
