...
x-i18n:
    generated_at: "2026-07-04T15:11:29Z"
    model: gpt-5.5
...
Der Paket-Root enthält kein `package.json`, daher kann ClawHub das
...
- Prüfen Sie alle OpenClaw-Buildmetadaten im Paket, etwa die während des Releases
...
- Wenn `openclaw.plugin.json` ebenfalls `version` enthält, aktualisieren Sie sie entsprechend
...
- Beheben Sie ungültige Paketmetadaten, defekte Lifecycle-Skripte oder Dateieinträge, durch die
...
- Verwenden Sie `openclaw/plugin-sdk/channel-core` für Channel-Einstiegshelfer.
...
- Verwenden Sie `resolveSessionTranscriptTarget(...)`, wenn der Code ein strukturiertes
  Transkriptoperationsziel benötigt.
