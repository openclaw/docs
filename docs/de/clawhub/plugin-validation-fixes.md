...
summary: Befunde der Validierung des ClawHub-Plugin-Pakets vor der Veröffentlichung beheben
title: Plugin-Validierungskorrekturen
x-i18n:
    generated_at: "2026-07-02T22:26:23Z"
    model: gpt-5.5
...
oder im veröffentlichten Artefakt seines Pakets beheben kann.
...
| `sdk-session-store-write`               | [Schreibvorgänge auf den vollständigen Sitzungsspeicher ersetzen](/de/clawhub/plugin-validation-fixes#sdk-session-store-write)                                  |
...
| `sdk-session-transcript-file-target`    | [Alte Transkript-Dateiziele ersetzen](/de/clawhub/plugin-validation-fixes#sdk-session-transcript-file-target)                   |
...
- Wenn `openclaw.plugin.json` ebenfalls `version` enthält, aktualisieren Sie diese,
  damit sie übereinstimmt, oder entfernen Sie veraltete Manifestversionsmetadaten,
  wenn die Paketmetadaten maßgeblich sind.
...
- Bewahren Sie Paket-Einstiegspunkte, Kompatibilität, Installation, Setup und Katalogmetadaten
...
- Beheben Sie ungültige Paketmetadaten, defekte Lifecycle-Skripte oder `files`-Einträge,
...
- Halten Sie `name` menschenlesbar und `id` als stabile maschinenlesbare ID.
- Siehe [Plugin-Manifest](/de/plugins/manifest).
- Führen Sie `clawhub package validate <path-to-plugin>` erneut aus.
...
- Verschieben Sie Paket- oder Installationsmetadaten stattdessen in unterstützte
  Felder unter `package.json#openclaw`.
- Führen Sie `clawhub package validate <path-to-plugin>` erneut aus.
...
- Verwenden Sie `patchSessionEntry(...)`, wenn Sie Felder in einem vorhandenen
...
Das Plugin verwendet noch die veraltete Hilfsfunktion für Transcript-Datei-Targets
...
- Vermeiden Sie es, Legacy-Transcript-Datei-Targets direkt zu lesen oder zu konstruieren.
