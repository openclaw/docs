...
x-i18n:
    generated_at: "2026-07-04T03:42:12Z"
    model: gpt-5.5
...
automatisierten Paket-Scans anzeigen. Diese Seite behandelt autorbezogene Befunde, also
Befunde, die der Plugin-Autor in Paketmetadaten, Manifest, SDK-Importen
oder veröffentlichtem Artefakt beheben kann.
...
| `sdk-load-session-store`                | [Zugriff auf den vollständigen Session Store ersetzen](/de/clawhub/plugin-validation-fixes#sdk-load-session-store)          |
| `sdk-session-store-write`               | [Schreibvorgänge auf den vollständigen Session Store ersetzen](/de/clawhub/plugin-validation-fixes#sdk-session-store-write) |
...
Die Paketmetadaten existieren, deklarieren aber keinen OpenClaw-Runtime-Einstiegspunkt.
...
- Prüfen Sie alle OpenClaw-Build-Metadaten im Paket, z. B. die OpenClaw-Version,
...
Der Block `package.json#openclaw` enthält Felder, die nicht als OpenClaw-Paketmetadaten
unterstützt werden.
...
- Bewahren Sie Paket-Einstiegspunkte, Kompatibilitäts-, Installations-, Setup- und Katalogmetadaten
...
- Entfernen Sie `private: true`, wenn dieses Paket für die öffentliche Veröffentlichung bestimmt ist.
- Führen Sie `clawhub package validate <path-to-plugin>` erneut aus.
...
- Verschieben Sie Runtime-Verhalten in den Plugin-Registrierungscode, und
...
- Wenn das Verhalten kein öffentliches SDK hat, behalten Sie die Hilfsfunktion
  in Ihrem Paket oder fordern Sie eine öffentliche OpenClaw-API an.
...
- Vermeiden Sie es, das gesamte Session-Store-Objekt zu laden, zu verändern und
  zu speichern.
- Behalten Sie `loadSessionStore(...)` nur, solange Ihr deklarierter
...
Das Plugin verwendet noch eine veraltete Schreib-Hilfsfunktion für den gesamten
Session Store, etwa `saveSessionStore` oder `updateSessionStore`.

- Verwenden Sie `patchSessionEntry(...)`, wenn Sie Felder eines bestehenden
  Sitzungseintrags aktualisieren.
- Verwenden Sie `upsertSessionEntry(...)`, wenn Sie einen Sitzungseintrag ersetzen
  oder erstellen.
- Vermeiden Sie es, das gesamte Session-Store-Objekt zu laden, zu verändern und
  zu speichern.
- Behalten Sie Schreib-Hilfsfunktionen für den gesamten Store nur, solange Ihr
  deklarierter Kompatibilitätsbereich noch ältere OpenClaw-Versionen unterstützt,
  die sie erfordern.
- Siehe [Runtime-API](/de/plugins/sdk-runtime#agent-session-state) und
  [Plugin-SDK-Subpaths](/de/plugins/sdk-subpaths).
- Führen Sie `clawhub package validate <path-to-plugin>` erneut aus.
...
Das Plugin verwendet noch veraltete Hilfsfunktionen für Sitzungspfade, etwa
`resolveSessionFilePath` oder `resolveAndPersistSessionFile`.

- Verwenden Sie `getSessionEntry(...)`, um Sitzungsmetadaten nach Agent- und
  Sitzungsidentität zu lesen.
- Verwenden Sie `patchSessionEntry(...)` oder `upsertSessionEntry(...)`, um
  Sitzungsmetadaten zu persistieren.
- Verwenden Sie Transkriptidentitäts- oder Ziel-Hilfsfunktionen, wenn der Code
  eine Transkriptoperation vorbereitet.
- Persistieren Sie keine veralteten Transkriptdateipfade und verlassen Sie sich
  nicht auf diese.
- Siehe [Runtime-API](/de/plugins/sdk-runtime#agent-session-state) und
  [Plugin-SDK-Subpaths](/de/plugins/sdk-subpaths).
- Führen Sie `clawhub package validate <path-to-plugin>` erneut aus.
...
- Verwenden Sie `resolveSessionTranscriptIdentity(...)`, wenn der Code nur die
  öffentliche Sitzungsidentität benötigt.
- Verwenden Sie `resolveSessionTranscriptTarget(...)`, wenn der Code ein
  strukturiertes Ziel für Transkriptoperationen benötigt.
- Vermeiden Sie es, veraltete Transkriptdateiziele direkt zu lesen oder zu
  konstruieren.
- Behalten Sie die veraltete Hilfsfunktion nur, solange Ihr deklarierter
  Kompatibilitätsbereich noch ältere OpenClaw-Versionen unterstützt, die sie
  erfordern.
- Siehe [Runtime-API](/de/plugins/sdk-runtime#agent-session-state) und
  [Plugin-SDK-Subpaths](/de/plugins/sdk-subpaths).
- Führen Sie `clawhub package validate <path-to-plugin>` erneut aus.
...
- Verwenden Sie `appendSessionTranscriptMessageByIdentity(...)` für das Anhängen
  an Transkripte.
- Verwenden Sie `publishSessionTranscriptUpdateByIdentity(...)` für
  Benachrichtigungen zu Transkriptaktualisierungen.
- Bevorzugen Sie die strukturierte Transkript-Runtime-Oberfläche, damit OpenClaw
  die korrekten Transaktionsgrenzen und die korrekte Identitätsbehandlung
  anwenden kann.
- Behalten Sie Low-Level-Transkript-Hilfsfunktionen nur, solange Ihr deklarierter
  Kompatibilitätsbereich noch ältere OpenClaw-Versionen unterstützt, die sie
  erfordern.
- Siehe [Runtime-API](/de/plugins/sdk-runtime#agent-session-state) und
  [Plugin-SDK-Subpaths](/de/plugins/sdk-subpaths).
...
- Verschieben Sie Arbeiten an Modell- oder Provider-Überschreibungen nach
  `before_model_resolve`.
- Verschieben Sie Arbeiten an Prompt- oder Kontextänderungen nach
  `before_prompt_build`.
- Behalten Sie `before_agent_start` nur, solange Ihr deklarierter
  Kompatibilitätsbereich noch ältere OpenClaw-Versionen unterstützt, die dies
  erfordern.
...
Das Manifest verwendet noch veraltete Provider-Auth-Metadaten
`providerAuthEnvVars`.

- Spiegeln Sie Provider-Env-Var-Metadaten in `setup.providers[].envVars`.
- Behalten Sie `providerAuthEnvVars` nur als Kompatibilitätsmetadaten, solange
  Ihr unterstützter OpenClaw-Bereich dies noch benötigt.
...
Das Manifest verwendet veraltete oder ältere Channel-Env-Var-Metadaten ohne die
...
- Spiegeln Sie env-gesteuertes Channel-Setup in die aktuellen Setup-,
  Channel-Config- oder Paket-Channel-Metadaten, die von Ihrer Plugin-Form
  verwendet werden.
- Behalten Sie `channelEnvVars` nur als Kompatibilitätsmetadaten, solange ältere
  unterstützte OpenClaw-Versionen dies noch erfordern.
...
- Dokumentieren Sie sicherheitsrelevantes Verhalten in Ihren öffentlichen
  Paketdokumenten oder in der README, bis der Manifest-Contract existiert.
- Führen Sie `clawhub package validate <path-to-plugin>` erneut aus.
