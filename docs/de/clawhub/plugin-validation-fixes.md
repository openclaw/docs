...
read_when:
    ...
    - ClawHub hat eine Plugin-Paketveröffentlichung abgelehnt oder davor gewarnt
...
x-i18n:
    generated_at: "2026-07-02T17:34:39Z"
    model: gpt-5.5
...
Imports oder ihrem veröffentlichten Artefakt beheben können.
...
| `sdk-session-transcript-file-target`    | [Veraltete Transkriptdatei-Ziele ersetzen](/de/clawhub/plugin-validation-fixes#sdk-session-transcript-file-target)                   |
...
- Prüfen Sie alle OpenClaw-Build-Metadaten im Paket, z. B. die während des Releases
...
- Halten Sie `name` menschenlesbar und verwenden Sie `id` als stabile maschinenlesbare ID.
- Siehe [Plugin-Manifest](/de/plugins/manifest).
- Führen Sie `clawhub package validate <path-to-plugin>` erneut aus.
...
Das Manifest deklariert nicht unterstützte Schlüssel in `contracts`.
...
- Verschieben Sie Laufzeitverhalten in den Plugin-Registrierungscode und
  beschränken Sie `contracts` auf Metadaten zur statischen Zuständigkeit für Fähigkeiten.
- Führen Sie `clawhub package validate <path-to-plugin>` erneut aus.
...
Das Plugin verwendet noch eine veraltete Schreib-Hilfsfunktion für den gesamten Session-Store, etwa
...
Das Plugin verwendet noch veraltete Hilfsfunktionen für Session-Dateipfade, etwa
...
- Persistieren Sie keine Legacy-Transkriptdateipfade und hängen Sie nicht von ihnen ab.
- Siehe [Laufzeit-API](/de/plugins/sdk-runtime#agent-session-state) und
  [Plugin-SDK-Subpaths](/de/plugins/sdk-subpaths).
- Führen Sie `clawhub package validate <path-to-plugin>` erneut aus.
...
Das Plugin verwendet noch die veraltete Hilfsfunktion für Transkriptdatei-Ziele
...
- Vermeiden Sie es, Legacy-Transkriptdatei-Ziele direkt zu lesen oder zu konstruieren.
...
Das Manifest verwendet noch die Legacy-Provider-Auth-Metadaten `providerAuthEnvVars`.
