...
x-i18n:
    generated_at: "2026-07-04T06:28:41Z"
    model: gpt-5.5
...
Paketmetadaten, im Manifest, in SDK-Imports oder im veröffentlichten Artefakt
beheben können.
...
Autorinnen und Autoren enthält, sind diese für OpenClaw-Maintainer statt für
...
| `sdk-session-store-write`               | [Schreibzugriffe auf den vollständigen Session-Store ersetzen](/de/clawhub/plugin-validation-fixes#sdk-session-store-write)                                  |
...
| `sdk-session-transcript-file-target`    | [Veraltete Transcript-Dateiziele ersetzen](/de/clawhub/plugin-validation-fixes#sdk-session-transcript-file-target)                   |
...
Das Paket-Root enthält keine `package.json`, daher kann ClawHub das npm-Paket,
...
- Prüfen Sie OpenClaw-Buildmetadaten im Paket, etwa die OpenClaw-Version, die
  während des Releases verwendet wurde.
...
- Beheben Sie ungültige Paketmetadaten, defekte Lifecycle-Skripte oder Dateien-Einträge,
  die das Packen fehlschlagen lassen.
...
- Verwenden Sie [Import-Konventionen](/de/plugins/building-plugins#import-conventions) und
...
Das Plugin verwendet noch die veraltete Hilfsfunktion für Transkriptdatei-Targets
...
- Vermeiden Sie es, veraltete Transkriptdatei-Targets direkt zu lesen oder zu konstruieren.
...
- Verschieben Sie Arbeiten an Prompt- oder Kontextmutationen nach `before_prompt_build`.
...
- Dokumentieren Sie sicherheitsrelevantes Verhalten in Ihren öffentlichen Paketdokumenten oder
  im README, bis der Manifest-Contract existiert.
- Führen Sie `clawhub package validate <path-to-plugin>` erneut aus.
