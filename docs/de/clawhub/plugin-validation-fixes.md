...
x-i18n:
    generated_at: "2026-07-01T15:22:14Z"
    model: gpt-5.5
...
ClawHub validiert Plugin-Pakete vor der Veröffentlichung und kann außerdem Befunde aus automatisierten Paket-Scans anzeigen. Diese Seite behandelt autorseitige Befunde, also Befunde, die Plugin-Autoren in ihren Paketmetadaten, ihrem Manifest, ihren SDK-Importen oder ihrem veröffentlichten Artefakt beheben können.

Sie behandelt keine internen Abdeckungsbefunde des Plugin Inspector. Wenn ein vollständiger Bericht Scanner-Wartungscodes ohne Hinweise zur Behebung durch Autoren enthält, sind diese für OpenClaw-Maintainer und nicht für Plugin-Autoren bestimmt.
...
Das Paket-Root enthält keine `package.json`, daher kann ClawHub das npm-Paket, die Version, Einstiegspunkte oder OpenClaw-Metadaten nicht identifizieren.
...
- Verwenden Sie [Plugins erstellen](/de/plugins/building-plugins) für ein minimales Paketbeispiel und [Plugin-Manifest](/de/plugins/manifest#manifest-versus-packagejson) für die Aufteilung zwischen Paket und Manifest.
- Führen Sie `clawhub package validate <path-to-plugin>` erneut aus.

### package-openclaw-metadata-missing

Das Paket hat eine `package.json`, deklariert jedoch keine OpenClaw-Paketmetadaten.
...
Die Paketmetadaten sind vorhanden, deklarieren jedoch keinen OpenClaw-Runtime-Einstiegspunkt.
...
Das Paket deklariert einen OpenClaw-Einstiegspunkt, aber die referenzierte Datei fehlt in dem validierten Paket.
...
- Füllen Sie `openclaw.install` mit der unterstützten Installationsquelle, etwa `clawhubSpec`, `npmSpec` oder `localPath`.
- Setzen Sie `openclaw.install.defaultChoice`, wenn mehr als eine Installationsquelle verfügbar ist.
...
Die minimale Host-Version des Pakets stimmt nicht mit den OpenClaw-Versionsmetadaten überein, gegen die das Paket gebaut wurde.

- Prüfen Sie `openclaw.install.minHostVersion`.
- Prüfen Sie alle OpenClaw-Build-Metadaten im Paket, etwa die während des Releases verwendete OpenClaw-Version.
- Gleichen Sie die minimale Host-Version an den Host-Versionsbereich an, den das Paket tatsächlich unterstützt.
- Siehe [package.json-Felder, die die Erkennung beeinflussen](/de/plugins/manifest#packagejson-fields-that-affect-discovery).
- Führen Sie `clawhub package validate <path-to-plugin>` erneut aus.
...
- Wenn `openclaw.plugin.json` ebenfalls `version` enthält, aktualisieren Sie sie entsprechend oder entfernen Sie veraltete Manifestversionsmetadaten, wenn Paketmetadaten maßgeblich sind.
...
- Bewahren Sie Paket-Einstiegspunkte, Kompatibilitäts-, Installations-, Setup- und Katalogmetadaten in unterstützten `package.json#openclaw`-Feldern auf.
- Siehe [package.json-Felder, die die Erkennung beeinflussen](/de/plugins/manifest#packagejson-fields-that-affect-discovery).
- Führen Sie `clawhub package validate <path-to-plugin>` erneut aus.
...
- Beheben Sie ungültige Paketmetadaten, defekte Lebenszyklusskripte oder Dateieinträge, die das Packen fehlschlagen lassen.
...
- Verwenden Sie `getSessionEntry(...)` oder `listSessionEntries(...)`, wenn Sie Session-Status
  lesen.
- Verwenden Sie `patchSessionEntry(...)` oder `upsertSessionEntry(...)`, wenn Sie Session-Status
  schreiben.
...
Das Plugin verwendet noch einen veralteten Schreibhelfer für den gesamten Session Store, z. B.
`saveSessionStore` oder `updateSessionStore`.

- Verwenden Sie `patchSessionEntry(...)`, wenn Sie Felder eines bestehenden Session-Eintrags
  aktualisieren.
...
Das Plugin verwendet noch veraltete Helfer für Session-Dateipfade wie
...
- Persistieren Sie keine veralteten Transcript-Dateipfade und machen Sie sich nicht davon abhängig.
- Siehe [Runtime-API](/de/plugins/sdk-runtime#agent-session-state) und
  [Plugin-SDK-Subpaths](/de/plugins/sdk-subpaths).
- Führen Sie `clawhub package validate <path-to-plugin>` erneut aus.
...
Das Plugin verwendet noch den veralteten Helfer für Transcript-Dateiziele
...
- Bevorzugen Sie die strukturierte Transcript-Laufzeitoberfläche, damit OpenClaw die
  richtigen Transaktionsgrenzen und die korrekte Identitätsbehandlung anwenden kann.
...
Das Manifest verwendet noch veraltete `providerAuthEnvVars`-Metadaten für die Provider-Authentifizierung.

- Spiegeln Sie Provider-Metadaten zu Umgebungsvariablen in `setup.providers[].envVars`.
...
- Halten Sie Metadaten zu Channel-Umgebungsvariablen deklarativ, damit OpenClaw den Setup-Status
