...
| `security-manifest-schema-unavailable`  | [Nicht verfügbare Verweise auf Security-Manifest-Schemas entfernen](/de/clawhub/plugin-validation-fixes#security-manifest-schema-unavailable) |
| `unrecognized-security-manifest`        | [Nicht unterstützte Security-Manifestdateien entfernen](/de/clawhub/plugin-validation-fixes#unrecognized-security-manifest)                   |
...
- Fügen Sie Kompatibilitäts- und Installationsmetadaten hinzu, wenn das Paket über ClawHub veröffentlicht oder installiert wird.
...
- Verschieben Sie Laufzeitverhalten in den Plugin-Registrierungscode, und
  beschränken Sie `contracts` auf statische Metadaten zum Besitz von Funktionen.
- Führen Sie `clawhub package validate <path-to-plugin>` erneut aus.
...
- Wenn das Verhalten kein öffentliches SDK hat, behalten Sie die Hilfsfunktion in
...
- Behalten Sie `loadSessionStore(...)` nur bei, solange Ihr deklarierter
  Kompatibilitätsbereich noch ältere OpenClaw-Versionen unterstützt, die es erfordern.
...
- Spiegeln Sie env-gesteuertes Channel-Setup in die aktuellen Setup-, Channel-Konfigurations-
  oder Paket-Channel-Metadaten, die von Ihrer Plugin-Form verwendet werden.
...
- Dokumentieren Sie sicherheitssensibles Verhalten weiterhin in Ihren öffentlichen Paketdokumenten oder
