...
summary: Beheben Sie die Validierungsbefunde für ClawHub-Plugin-Pakete vor der Veröffentlichung
title: Plugin-Validierungskorrekturen
x-i18n:
    generated_at: "2026-07-02T14:02:14Z"
    model: gpt-5.5
...
automatisierten Paketscans anzeigen. Diese Seite behandelt autorbezogene Befunde, also
Befunde, die der Plugin-Autor in den Paketmetadaten, im Manifest, in SDK-
Importen oder im veröffentlichten Artefakt des Pakets beheben kann.
...
| `sdk-session-file-helper`               | [Session-Dateipfad-Helper ersetzen](/de/clawhub/plugin-validation-fixes#sdk-session-file-helper)                                   |
...
| `sdk-session-transcript-low-level`      | [Low-Level-Transcript-Helper ersetzen](/de/clawhub/plugin-validation-fixes#sdk-session-transcript-low-level)                       |
...
- Wenn `openclaw.plugin.json` ebenfalls `version` enthält, aktualisieren Sie sie so, dass sie übereinstimmt, oder entfernen Sie
...
- Belassen Sie Paketeinstiegspunkte, Kompatibilitäts-, Installations-, Setup- und Katalogmetadaten
...
- Verwenden Sie `resolveSessionTranscriptTarget(...)`, wenn der Code ein strukturiertes
  Ziel für Transkriptoperationen benötigt.
