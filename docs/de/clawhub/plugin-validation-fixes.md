...
Befunde, die der Plugin-Autor in seinen Paketmetadaten, seinem Manifest, seinen SDK-
Importen oder seinem veröffentlichten Artefakt beheben kann.
...
| `sdk-session-file-helper`               | [Hilfsfunktionen für Sitzungdateipfade ersetzen](/de/clawhub/plugin-validation-fixes#sdk-session-file-helper)                                   |
...
- Stellen Sie sicher, dass `openclaw.plugin.json` aufgenommen wird, wenn das Paket ein natives
  OpenClaw-Plugin ist.
...
- Vergleichen Sie jedes Top-Level-Feld mit der
...
- Verschieben Sie Package- oder Installationsmetadaten stattdessen in unterstützte `package.json#openclaw`-Felder
...
- Wenn das Verhalten kein öffentliches SDK hat, behalten Sie die Hilfsfunktion in Ihrem Package oder
...
- Vermeiden Sie es, das gesamte Session-Store-Objekt zu laden, zu ändern und zu speichern.
- Behalten Sie Whole-Store-Schreib-Hilfsfunktionen nur bei, solange Ihr deklarierter Kompatibilitätsbereich
...
Das Plugin verwendet noch veraltete Session-Dateipfad-Hilfsfunktionen wie
...
- Persistieren Sie keine Legacy-Transkriptdateipfade und verlassen Sie sich nicht darauf.
- Siehe [Runtime API](/de/plugins/sdk-runtime#agent-session-state) und
  [Plugin-SDK-Subpaths](/de/plugins/sdk-subpaths).
- Führen Sie `clawhub package validate <path-to-plugin>` erneut aus.
...
Das Plugin verwendet noch die veraltete Hilfsfunktion für das Transkriptdatei-Target
...
- Vermeiden Sie es, Legacy-Transkriptdatei-Targets direkt zu lesen oder zu konstruieren.
...
- Spiegeln Sie env-gesteuertes Channel-Setup in die aktuellen Setup-, Channel-Config- oder
  Package-Channel-Metadaten, die von Ihrer Plugin-Form verwendet werden.
...
Das Package liefert `openclaw.security.json` mit einer Schema-Referenz aus, die ClawHub
...
Das Package liefert eine nicht unterstützte Sicherheitsmanifestdatei aus.
...
- Halten Sie sicherheitsrelevantes Verhalten in Ihren öffentlichen Package-Dokumenten oder
  in der README dokumentiert, bis der Manifest-Contract existiert.
- Führen Sie `clawhub package validate <path-to-plugin>` erneut aus.
