...
x-i18n:
    generated_at: "2026-07-04T10:36:16Z"
    model: gpt-5.5
...
automatisierten Paket-Scans anzeigen. Diese Seite behandelt autorenseitige Befunde,
...
Das Paket-Root enthält keine `package.json`, daher kann ClawHub das
...
- Halten Sie dies von der Paketversion getrennt. Die Paketversion beschreibt den
...
- Prüfen Sie OpenClaw-Build-Metadaten im Paket, zum Beispiel die während des Releases
...
- Wenn `openclaw.plugin.json` ebenfalls `version` enthält, aktualisieren Sie sie passend
  oder entfernen Sie veraltete Manifestversionsmetadaten, wenn die Paketmetadaten
  maßgeblich sind.
...
- Beheben Sie ungültige Paketmetadaten, fehlerhafte Lifecycle-Skripte oder `files`-Einträge, die
  das Packen scheitern lassen.
...
- Vergleichen Sie jeden Schlüssel unter `contracts` mit der
  [Referenz zu Verträgen](/de/plugins/manifest#contracts-reference).
...
- Persistieren Sie keine veralteten Transcript-Dateipfade und hängen Sie nicht davon ab.
- Siehe [Runtime-API](/de/plugins/sdk-runtime#agent-session-state) und
  [Plugin-SDK-Subpaths](/de/plugins/sdk-subpaths).
- Führen Sie `clawhub package validate <path-to-plugin>` erneut aus.
...
- Verwenden Sie `publishSessionTranscriptUpdateByIdentity(...)` für Transcript-Update-
  Benachrichtigungen.
...
Das Manifest verwendet noch veraltete Provider-Auth-Metadaten `providerAuthEnvVars`.
