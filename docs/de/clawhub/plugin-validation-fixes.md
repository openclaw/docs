...
x-i18n:
    generated_at: "2026-07-03T17:16:58Z"
    model: gpt-5.5
...
automatisierten Paket-Scans anzeigen. Diese Seite behandelt autorenrelevante Befunde, also
...
Der Paket-Root enthält keine `package.json`, sodass ClawHub das
npm-Paket, die Version, Entrypoints oder OpenClaw-Metadaten nicht identifizieren kann.
...
- Fügen Sie Kompatibilitäts- und Installationsmetadaten hinzu, wenn das Paket veröffentlicht oder
  über ClawHub installiert wird.
...
- Halten Sie Paket-Entrypoints, Kompatibilitäts-, Installations-, Setup- und Katalogmetadaten
...
- Verschieben Sie Laufzeitverhalten in den Plugin-Registrierungscode und beschränken Sie `contracts`
  auf statische Metadaten zur Capability-Inhaberschaft.
- Führen Sie `clawhub package validate <path-to-plugin>` erneut aus.
...
- Behalten Sie `loadSessionStore(...)` nur, solange Ihr deklarierter Kompatibilitätsbereich
...
Das Plugin verwendet noch eine veraltete Schreib-Hilfsfunktion für den gesamten Session Store, z. B.
...
- Vermeiden Sie es, das gesamte Session-Store-Objekt zu laden, zu ändern und zu speichern.
- Behalten Sie Schreib-Hilfsfunktionen für den gesamten Store nur, solange Ihr deklarierter Kompatibilitätsbereich
...
Das Plugin verwendet noch veraltete Hilfsfunktionen für Sitzungdateipfade wie
...
- Persistieren Sie keine Legacy-Transcript-Dateipfade und hängen Sie nicht davon ab.
- Siehe [Runtime-API](/de/plugins/sdk-runtime#agent-session-state) und
  [Plugin-SDK-Subpaths](/de/plugins/sdk-subpaths).
- Führen Sie `clawhub package validate <path-to-plugin>` erneut aus.
...
Das Plugin verwendet noch die veraltete Hilfsfunktion für Transcript-Dateiziele
...
- Verwenden Sie `resolveSessionTranscriptTarget(...)`, wenn der Code ein strukturiertes
  Ziel für Transcript-Operationen benötigt.
- Vermeiden Sie es, Legacy-Transcript-Dateiziele direkt zu lesen oder zu erstellen.
- Behalten Sie die Legacy-Hilfsfunktion nur, solange Ihr deklarierter Kompatibilitätsbereich noch
...
- Verwenden Sie `publishSessionTranscriptUpdateByIdentity(...)` für Benachrichtigungen zu Transcript-Aktualisierungen.
- Bevorzugen Sie die strukturierte Transcript-Laufzeitoberfläche, damit OpenClaw die
  richtigen Transaktionsgrenzen und die richtige Identitätsbehandlung anwenden kann.
- Behalten Sie Low-Level-Transcript-Hilfsfunktionen nur, solange Ihr deklarierter Kompatibilitätsbereich
  noch ältere OpenClaw-Versionen unterstützt, die sie erfordern.
- Siehe [Runtime-API](/de/plugins/sdk-runtime#agent-session-state) und
  [Plugin-SDK-Subpaths](/de/plugins/sdk-subpaths).
- Führen Sie `clawhub package validate <path-to-plugin>` erneut aus.
...
- Behalten Sie `before_agent_start` nur, solange Ihr deklarierter Kompatibilitätsbereich noch
...
- Behalten Sie `providerAuthEnvVars` nur als Kompatibilitätsmetadaten, solange Ihr unterstützter
...
- Behalten Sie `channelEnvVars` nur als Kompatibilitätsmetadaten, solange ältere unterstützte
