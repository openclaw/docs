...
x-i18n:
    generated_at: "2026-07-04T17:56:23Z"
    model: gpt-5.5
...
Drittanbieter-Verzeichnisse dürfen die öffentlichen Lese-Endpunkte verwenden, um ClawHub-Skills aufzulisten oder zu durchsuchen. Bitte cachen Sie Ergebnisse, beachten Sie `429`/`Retry-After`, verlinken Sie Benutzer zurück zum kanonischen ClawHub-Eintrag (`https://clawhub.ai/<owner>/skills/<slug>`) und vermeiden Sie den Eindruck, ClawHub unterstütze die Drittanbieter-Site. Versuchen Sie nicht, versteckte, private oder durch Moderation blockierte Inhalte außerhalb der öffentlichen API-Oberfläche zu spiegeln.

Web-Slug-Abkürzungen werden über Registry-Familien hinweg aufgelöst, API-Clients sollten jedoch die von Lese-Endpunkten zurückgegebenen kanonischen URLs verwenden, statt die Routenpriorität zu rekonstruieren.
...
- Authentifizierte Schreib-Endpunkte sollten kein bloßes `Unauthorized` zurückgeben, wenn
  ...
  gelöschte/gesperrte/deaktivierte Konten sollten jeweils handlungsfähigen Text erhalten, damit CLI-
...
- Wenn keine vertrauenswürdige Client-IP verfügbar ist, verwenden anonyme Anfragen Fallback-Buckets,
  die nur nach Rate-Limit-Art scoped sind. Diese Fallback-Buckets enthalten keine
...
- Ergebnisse werden nach Relevanz zurückgegeben (Embedding-Ähnlichkeit + Boosts für exakte Slug-/Namens-Token + kleiner Popularitäts-Prior).
- Relevanz ist stärker als Popularität. Eine genaue Übereinstimmung mit einem Slug- oder Anzeigenamen-Token kann höher ranken als eine lockerere Übereinstimmung mit deutlich stärkerem Engagement.
...
- Popularität ist logarithmisch skaliert und begrenzt. Skills mit hohem Engagement können niedriger ranken, wenn der Suchtext eine schwächere Übereinstimmung ist.
...
- Verwenden Sie die Begriffe, nach denen Benutzer wörtlich suchen werden, im Anzeigenamen, in der Zusammenfassung und in Tags. Verwenden Sie ein eigenständiges Slug-Token nur, wenn es auch eine stabile Identität ist, die Sie behalten möchten.
- Benennen Sie einen Slug nicht nur um, um eine einzelne Suchanfrage zu bedienen, außer der neue Slug ist langfristig ein besserer kanonischer Name. Alte Slugs werden zu Redirect-Aliasen, aber die kanonische URL, der angezeigte Slug und zukünftige Such-Digests verwenden den neuen Slug.
...
- Alte Slugs, die durch Owner-Umbenennungs-/Merge-Flows entstanden sind, lösen auf den kanonischen Skill auf.
...
- Belege werden für öffentliche Aufrufer redigiert und enthalten rohe Snippets nur für Owner/Moderatoren.

### `POST /api/v1/skills/{slug}/report`

Melden Sie einen Skill zur Überprüfung durch Moderatoren. Meldungen gelten auf Skill-Ebene, sind optional
...
- Scan-Anfrage-Payloads und herunterladbare Berichte laufen nach dem Aufbewahrungsfenster aus dem Scan-Request-Speicher ab.
- Veröffentlichte Scans erfordern Verwaltungszugriff als Owner/Publisher oder Plattformberechtigungen als Moderator/Admin.
- Veröffentlichte Scans schreiben nur dann zurück, wenn `update: true` gesetzt ist und der Scan erfolgreich abgeschlossen wird.
...
- Scan-Jobs sind asynchron. Manuelle Scan-Anfragen werden vor normalen Publish-/Backfill-Arbeiten priorisiert, der Abschluss hängt jedoch weiterhin von der Worker-Verfügbarkeit ab.
...
- Erfordert Verwaltungszugriff als Owner/Publisher auf den Skill oder das Plugin oder Plattformberechtigungen als Moderator/Admin.
...
Nur für Admins verfügbare kanonische Route für Batch-Rescans. Sie akzeptiert dieselbe Payload-Struktur wie das alte `POST /api/v1/skills/-/rescan-batch`.

### `POST /api/v1/skills/-/scan/batch/status`

Nur für Admins verfügbare kanonische Route für den Batch-Status. Sie akzeptiert `{ "jobIds": ["..."] }` und gibt dieselben aggregierten Zähler wie das alte `POST /api/v1/skills/-/rescan-batch/status` zurück.
...
- `security.signals.dependencyRegistry` wird zur Kompatibilität der v1-Antwort beibehalten, aber der Existenz-Scanner für das Dependency-Registry ist außer Betrieb genommen, und dieser Schlüssel ist immer `null`.
...
Auf dem vereinheitlichten Endpunkt `/api/v1/packages` ist es nur für Plugins verfügbar; verwenden Sie
...
- Body: ZIP-Archiv.
- Jeder exportierte Skill hat das Stammverzeichnis `{publisher}/{slug}/`.
...
- Body: ZIP-Archiv.
- Jedes exportierte Plugin hat sein Wurzelverzeichnis unter `{family}/{packageName}/`.
...
- Die Legacy-v1-Filteraliase, die unter `GET /api/v1/plugins` dokumentiert sind, werden ebenfalls
  akzeptiert.
- Die Kategoriefilterung ist ein echter API-Filter, der durch Plugin-Kategorie-Digest-
  Zeilen gestützt wird, keine Umschreibung der Suchabfrage.
- Ergebnisse werden in Relevanzreihenfolge zurückgegeben und derzeit nicht paginiert.
...
- `trust.pending` bedeutet, dass eine oder mehrere Vertrauenseingaben noch auf den Abschluss warten.
- `trust.stale` bedeutet, dass die Vertrauenszusammenfassung aus veralteten Eingaben berechnet wurde und
  vor einer Allow-Entscheidung mit hoher Zuverlässigkeit als aktualisierungsbedürftig behandelt werden sollte.
...
- Dieser Endpunkt ist versionsgenau. Clients sollten ihn aufrufen, nachdem sie die
  ...
  Paketmetadaten.
- Private Pakete geben `404` zurück, sofern der Aufrufer den besitzenden Publisher nicht lesen darf.
- Dieser Endpunkt ist absichtlich enger gefasst als Besitzer-/Moderator-Moderationsendpunkte.
  Er legt die Installationsentscheidung und die öffentliche Erklärung offen, nicht
  Reporteridentitäten, Report-Bodys, private Nachweise oder interne Review-
  Zeitachsen.
...
- Dies ist die OpenClaw-Resolver-Oberfläche; sie vermeidet das Erraten des Archivformats aus
...
Gibt die berechnete Bereitschaft für die künftige OpenClaw-Nutzung zurück.
...
- `packageName` wird npm-namensnormalisiert; das Paket kann für geplante
  Migrationen fehlen.
...
Melden Sie ein Paket zur Überprüfung durch Moderatoren. Reports sind paketbezogen und optional
mit einer Version verknüpft. Sie speisen die Moderationswarteschlange, blenden Downloads aber nicht
automatisch aus und blockieren sie auch nicht von selbst; Moderatoren sollten Release-Moderation verwenden, um
...
- Verwendet den Lesedurchsatz-Bucket, nicht den Download-Bucket.
...
- `dist.tarball`, `dist.integrity` und `dist.shasum` verwenden npm-kompatible
  Felder, damit Benutzer npm bei Bedarf auf den Mirror zeigen lassen können.
...
- Verwendet den Download-Durchsatz-Bucket.
...
- Veröffentlichungen im Namen eines anderen Owners validieren die Berechtigung für den offiziellen Kanal weiterhin gegen das Ziel-Owner-Konto.
...
- Textkörper: `{ "handle": "openclaw", "displayName": "OpenClaw", "memberHandle": "alice", "memberRole": "owner", "trusted": true }`
...
- Textkörper: `{ "handle": "opik", "displayName": "Opik" }`
...
- Textkörper: `{ "handle": "openclaw", "slugs": ["diffs"], "packageNames": ["@openclaw/diffs"], "reason": "reserved for official OpenClaw plugin" }`
...
Die Wiederherstellung migriert außerdem Legacy-Felder `ownerUserId` für die Skills des wiederhergestellten Publishers,
...
100 Zeilen pro Apply-Transaktion begrenzt; größere Wiederherstellungen müssen zuerst eine fortsetzbare Eigentümermigration verwenden.
