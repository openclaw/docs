...
x-i18n:
    generated_at: "2026-07-03T13:22:53Z"
    model: gpt-5.5
...
- `X-RateLimit-Remaining` / `RateLimit-Remaining`: exaktes verbleibendes Budget, wenn vorhanden.
  Erfolgreiche Shard-Anfragen lassen diesen Header aus, statt einen ungefähren globalen Wert zurückzugeben.
...
- Ergebnisse werden in Relevanzreihenfolge zurückgegeben (Embedding-Ähnlichkeit + Boosts für exakte Slug-/Namens-Token + ein kleiner Popularitäts-Prior).
...
- Popularität ist logarithmisch skaliert und gedeckelt. Skills mit hohem Engagement können niedriger ranken, wenn der Query-Text eine schwächere Übereinstimmung ist.
...
- Umbenennungs-Aliase erhalten die Auflösung für alte URLs und Installationen, die über die Registry aufgelöst werden, aber das Suchranking basiert auf den kanonischen Skill-Metadaten, nachdem die Umbenennung indexiert wurde. Vorhandene Statistiken bleiben beim Skill.
...
- Wenn `nonSuspiciousOnly=true`, können cursorbasierte Sortierungen auf einer Seite weniger als `limit` Elemente zurückgeben, weil verdächtige Skills nach dem Abruf der Seite herausgefiltert werden.
- Verwenden Sie `nextCursor`, um die Paginierung fortzusetzen, wenn vorhanden. Eine kurze Seite bedeutet nicht für sich allein das Ende der Ergebnisse.
...
Melden Sie einen Skill zur Moderatorprüfung. Meldungen gelten auf Skill-Ebene, sind optional mit einer Version verknüpft und fließen in die Skill-Meldewarteschlange ein.
...
- Veröffentlichte Scans erfordern Verwaltungszugriff als Eigentümer/Publisher oder Moderator-/Administratorberechtigung der Plattform.
- Veröffentlichte Scans schreiben nur zurück, wenn `update: true` ist und der Scan erfolgreich abgeschlossen wurde.
...
- Scan-Jobs sind asynchron. Manuelle Scan-Anfragen werden vor normaler Veröffentlichungs-/Backfill-Arbeit priorisiert, der Abschluss hängt aber weiterhin von der Worker-Verfügbarkeit ab.
...
- Erfordert Verwaltungszugriff als Eigentümer/Publisher auf den Skill oder das Plugin oder Moderator-/Administratorberechtigung der Plattform.
...
Nur für Administratoren vorgesehene kanonische Route für Batch-Neuscans. Sie akzeptiert dieselbe Payload-Struktur wie das Legacy-`POST /api/v1/skills/-/rescan-batch`.

### `POST /api/v1/skills/-/scan/batch/status`

Nur für Administratoren vorgesehene kanonische Route für Batch-Status. Sie akzeptiert `{ "jobIds": ["..."] }` und gibt dieselben aggregierten Zähler zurück wie das Legacy-`POST /api/v1/skills/-/rescan-batch/status`.
...
- `security.signals.dependencyRegistry` bleibt für die Kompatibilität mit v1-Antworten erhalten, aber der Scanner zur Existenz der Dependency Registry wurde eingestellt und dieser Schlüssel ist immer `null`.
...
Sammlungs-Endpunkt ist für Clients vorgesehen, die bereits wissen, welche installierten
...
- Die Antwort enthält nur Sicherheitsdaten. Sie enthält keine Skill-Card-Daten, keinen Status generierter Karten, keine Artefakt-Dateilisten und keine detaillierten Scanner-Payloads.
- `security.signals` enthält nur unterstützende Nachweise auf Statusebene; verwenden Sie `/scan` oder die ClawHub-Sicherheitsaudit-Seite für vollständige Scanner-Details.
- `security.signals.dependencyRegistry` bleibt für die Kompatibilität mit v1-Antworten erhalten, aber der Scanner zur Existenz der Dependency Registry wurde eingestellt und dieser Schlüssel ist immer `null`.
...
Nur-Plugin-Katalogdurchsicht über Code-Plugin- und Bundle-Plugin-Pakete hinweg.
...
`trending` ist eine Installations-/Download-Bestenliste über sieben Tage und verwendet keine Gesamtwerte aller Zeiten.
...
- Aktuelle GitHub-gestützte Skills mit einem `clean`- oder `suspicious`-Scan enthalten
  `_source_handoff.json` mit `sourceRef: "public-github"`, Repo, Commit, Pfad,
...
Nur-Plugin-Suche über `code-plugin`- und `bundle-plugin`-Pakete.
...
Gibt Detailmetadaten eines Pakets zurück.
...
- Dieser Endpunkt ist absichtlich enger gefasst als Besitzer-/Moderator-Moderations-
  Endpunkte. Er stellt die Installationsentscheidung und öffentliche Erklärung bereit, nicht
...
- Artefakt-Digest
- Quell-Repo- und Commit-Herkunft
...
- `open`: verdächtige, bösartige, ausstehende, quarantined, revoked oder gemeldete Releases.
- `blocked`: quarantined, revoked oder bösartige Releases.
...
Melden Sie ein Paket zur Moderator-Überprüfung. Meldungen gelten auf Paketebene und sind optional
...
- Legacy-Versionen, die nur als ZIP vorliegen, werden absichtlich ausgelassen.
...
- GitHub-basierte Skill-Übergaben proxyen oder spiegeln keine Bytes. Die JSON-Antwort
...
- Optionales Payload-Feld: `migrateOwner`. Wenn `true` zusammen mit `ownerHandle`, kann ein
  ...
  aktuellen als auch beim Ziel-Herausgeber ist. Ohne dieses Opt-in werden Eigentümerwechsel
  abgelehnt.
...
- JSON-Bodies und vom Aufrufer gelieferte `payload.files`- / `payload.artifact`-
...
- Nur der Herausgeber der `openclaw`-Organisation und die persönlichen Herausgeber aktueller Mitglieder der
  `openclaw`-Organisation dürfen im `official`-Kanal veröffentlichen.
- Veröffentlichungen im Auftrag eines anderen validieren die Berechtigung für den Official-Kanal weiterhin gegen das Ziel-Eigentümerkonto.

### `DELETE /api/v1/skills/{slug}` / `POST /api/v1/skills/{slug}/undelete`

Skill soft-deleten / wiederherstellen (Eigentümer, Moderator oder Admin).
...
Nur Admins. Stellt sicher, dass ein Organisations-Herausgeber für ein Handle existiert. Wenn das Handle noch auf einen
Legacy-geteilten Benutzer-/persönlichen Herausgeber zeigt, migriert der Endpunkt es zuerst in einen Organisations-Herausgeber.
...
- Anfragetext: `{ "handle": "openclaw", "displayName": "OpenClaw", "memberHandle": "alice", "memberRole": "owner", "trusted": true }`
...
Authentifizierte Self-Serve-Erstellung eines Organisations-Herausgebers. Erstellt einen neuen Organisations-Herausgeber und fügt den
...
- Anfragetext: `{ "handle": "opik", "displayName": "Opik" }`
...
Nur Admins. Stellt einen persönlichen Herausgeber für einen verifizierten Ersatz-GitHub-OAuth-Prinzipal wieder her,
...
Direktinhaber-Pfade mit der neuen Publisher-Autorität übereinstimmen. Eine aktive geschützte Handle-Reservierung
für das wiederhergestellte Handle wird ebenfalls dem Ersatzbenutzer neu zugewiesen, damit spätere
...
Anwendungstransaktion auf 100 Zeilen begrenzt; größere Wiederherstellungen müssen zuerst eine fortsetzbare Inhabermigration verwenden.
