...
x-i18n:
    generated_at: "2026-06-28T22:32:07Z"
    model: gpt-5.5
...
Ältere `/api/...` und `/api/cli/...` bleiben aus Kompatibilitätsgründen erhalten (siehe `DEPRECATIONS.md`).
OpenAPI: `/api/v1/openapi.json`.

## Wiederverwendung des öffentlichen Katalogs

Drittanbieter-Verzeichnisse dürfen die öffentlichen Lese-Endpunkte verwenden, um ClawHub Skills aufzulisten oder zu durchsuchen. Zwischenspeichern Sie Ergebnisse, beachten Sie `429`/`Retry-After`, verlinken Sie Nutzer zurück zum kanonischen ClawHub-Eintrag (`https://clawhub.ai/<owner>/skills/<slug>`), und vermeiden Sie den Eindruck, ClawHub unterstütze die Drittanbieter-Website. Versuchen Sie nicht, versteckte, private oder durch Moderation blockierte Inhalte außerhalb der öffentlichen API-Oberfläche zu spiegeln.
...
- `X-RateLimit-Remaining` / `RateLimit-Remaining`: genaues verbleibendes Budget, wenn vorhanden.
  Shard-übergreifend erfolgreiche Anfragen lassen diesen Header weg, statt einen ungefähren globalen Wert zurückzugeben.
...
sollten den Antwortkörper als menschenlesbare Zeichenkette lesen. Unbekannte Abfrageparameter werden
...
- Popularität ist logarithmisch skaliert und gedeckelt. Skills mit hohem Engagement können niedriger eingestuft werden, wenn der Abfragetext schwächer übereinstimmt.
...
- Benennen Sie einen Slug nicht nur um, um eine einzelne Abfrage zu bedienen, es sei denn, der neue Slug ist langfristig ein besserer kanonischer Name. Alte Slugs werden zu Weiterleitungsaliasen, aber die kanonische URL, der angezeigte Slug und künftige Such-Digests verwenden den neuen Slug.
- Umbenennungsaliase bewahren die Auflösung für alte URLs und Installationen, die über die Registry auflösen, aber das Suchranking basiert auf den kanonischen Skill-Metadaten, nachdem die Umbenennung indexiert wurde. Bestehende Statistiken bleiben beim Skill.
...
- Wenn `nonSuspiciousOnly=true`, können cursorbasierte Sortierungen auf einer Seite weniger als `limit` Einträge zurückgeben, weil verdächtige Skills nach dem Abrufen der Seite herausgefiltert werden.
...
Meldet einen Skill zur Prüfung durch Moderatoren. Meldungen gelten auf Skill-Ebene, sind optional mit
...
Gibt Sicherheits-Scan-Prüfdetails für eine Skill-Version zurück.
...
- Gibt `queue.queuedAhead` und `queue.position` zurück, solange sich der Auftrag in der Queue befindet, damit Clients anzeigen können, wie viele priorisierte manuelle Scans vor der Anfrage liegen. Sehr große Queues werden begrenzt und mit `queuedAheadIsEstimate: true` gemeldet.
...
Nur für Admins zugängliche kanonische Route für Batch-Rescans. Sie akzeptiert dieselbe Payload-Struktur wie das Legacy-`POST /api/v1/skills/-/rescan-batch`.

### `POST /api/v1/skills/-/scan/batch/status`

Nur für Admins zugängliche kanonische Route für Batch-Status. Sie akzeptiert `{ "jobIds": ["..."] }` und gibt dieselben aggregierten Zähler zurück wie das Legacy-`POST /api/v1/skills/-/rescan-batch/status`.
...
- `security.signals.dependencyRegistry` bleibt für die v1-Antwortkompatibilität erhalten, aber der Scanner für die Existenz in der Dependency Registry ist außer Betrieb genommen, und dieser Schlüssel ist immer `null`.
...
- Die Antwort enthält nur Security-Informationen. Sie enthält keine Skill-Card-Daten, keinen Status der generierten Karte, keine Artefakt-Dateilisten und keine detaillierten Scanner-Payloads.
- `security.signals` enthält nur unterstützende Evidenz auf Statusebene; verwenden Sie `/scan` oder die Security-Audit-Seite von ClawHub für vollständige Scanner-Details.
- `security.signals.dependencyRegistry` bleibt für die v1-Antwortkompatibilität erhalten, aber der Scanner für die Existenz in der Dependency Registry ist außer Betrieb genommen, und dieser Schlüssel ist immer `null`.
...
- Verwenden Sie `/verify`, wenn Sie den Verifizierungs-Umschlag der Skill Card für einen einzelnen Skill benötigen, `/card`, wenn Sie generiertes Karten-Markdown benötigen, und `/scan`, wenn Sie detaillierte Scanner-Daten benötigen.
...
`trending` ist eine Installations-/Download-Bestenliste für sieben Tage und verwendet keine Gesamtwerte seit Beginn.
...
- Öffentlicher Leseendpunkt. Kein Token für Inhaber, Publisher, Moderator oder Admin
  erforderlich.
...
- `trust.reasons` ist die benutzerorientierte und auditierbare Erklärungsliste. Reason-Codes
...
- `trust.stale` bedeutet, dass die Vertrauenszusammenfassung aus veralteten Eingaben berechnet wurde und
  vor einer hochvertrauenswürdigen Zulassungsentscheidung als aktualisierungsbedürftig behandelt werden sollte.
...
- Dieser Endpunkt ist absichtlich enger gefasst als Moderationsendpunkte für Inhaber/Moderatoren.
  ...
  Reporter-Identitäten, Berichtstexte, private Nachweise oder interne Review-
  Zeitpläne.
...
Melden Sie ein Paket zur Moderator-Review. Meldungen sind paketbezogen und optional
mit einer Version verknüpft. Sie fließen in die Moderationswarteschlange ein, blenden aber
Downloads nicht automatisch aus und blockieren sie auch nicht selbst; Moderatoren sollten Release-Moderation verwenden, um
...
- `quarantined`: bis zur Nachprüfung blockiert.
...
- Private Pakete geben `404` zurück, es sei denn, der Aufrufer ist der Owner.
...
Lädt ein gehostetes ZIP einer Skill-Version herunter oder gibt eine GitHub-Quellübergabe für eine
aktuelle GitHub-basierte Skill mit einem `clean`- oder `suspicious`-Scan und ohne gehostete
Version zurück.
...
- JSON-Body mit `files` (auf Basis von storageId) wird ebenfalls akzeptiert.
...
- Optionales Payload-Feld: `migrateOwner`. Wenn `true` zusammen mit `ownerHandle`, kann eine
  bestehende Skill zu diesem Owner verschoben werden, wenn der Akteur Admin/Owner sowohl beim
...
- Veröffentlichungen im Namen eines anderen prüfen die Berechtigung für den offiziellen Kanal weiterhin gegen das Ziel-Owner-Konto.

### `DELETE /api/v1/skills/{slug}` / `POST /api/v1/skills/{slug}/undelete`

Soft-Delete / Wiederherstellung einer Skill (Owner, Moderator oder Admin).
...
Direkteigentümer-Pfade mit der neuen Publisher-Autorität übereinstimmen. Eine aktive Protected-Handle-
