...
x-i18n:
    generated_at: "2026-07-03T02:44:35Z"
    model: gpt-5.5
...
Drittanbieter-Verzeichnisse dürfen die öffentlichen Lese-Endpunkte verwenden, um ClawHub Skills aufzulisten oder zu durchsuchen. Cachen Sie Ergebnisse, beachten Sie `429`/`Retry-After`, verlinken Sie Benutzer zurück zum kanonischen ClawHub-Eintrag (`https://clawhub.ai/<owner>/skills/<slug>`) und vermeiden Sie den Eindruck, ClawHub würde die Drittanbieter-Website unterstützen. Versuchen Sie nicht, verborgene, private oder durch Moderation blockierte Inhalte außerhalb der öffentlichen API-Oberfläche zu spiegeln.
...
aus Kompatibilitätsgründen ignoriert, erkannte Query-Parameter mit ungültigen Werten geben jedoch
`400` zurück.
...
- Relevanz ist stärker als Popularität. Eine präzise Slug- oder Anzeigename-Token-Übereinstimmung kann eine lockerere Übereinstimmung mit deutlich stärkerem Engagement übertreffen.
...
- Popularität ist logarithmisch skaliert und gedeckelt. Skills mit hohem Engagement können niedriger ranken, wenn der Abfragetext eine schwächere Übereinstimmung ist.
...
Leitfaden zur Auffindbarkeit für Publisher:
...
- Benennen Sie einen Slug nicht nur um, um einer einzelnen Abfrage nachzujagen, es sei denn, der neue Slug ist ein besserer langfristiger kanonischer Name. Alte Slugs werden zu Weiterleitungsaliasen, aber die kanonische URL, der angezeigte Slug und zukünftige Such-Digests verwenden den neuen Slug.
...
Meldet einen Skill zur Überprüfung durch Moderatoren. Meldungen beziehen sich auf Skill-Ebene, sind optional
...
Meldung, um den Skill im selben auditierbaren Workflow zu verbergen.
...
Authentifizierter Submit-Endpunkt für neue ClawScan-Aufträge.
...
- Scan-Aufträge sind asynchron. Manuelle Scan-Anfragen werden gegenüber normaler Publish-/Backfill-Arbeit priorisiert, aber der Abschluss hängt weiterhin von der Verfügbarkeit der Worker ab.
...
Admin-only kanonische Route für den Batch-Status. Sie akzeptiert `{ "jobIds": ["..."] }` und gibt dieselben aggregierten Zähler wie das Legacy-`POST /api/v1/skills/-/rescan-batch/status` zurück.
...
- `security.signals.dependencyRegistry` wird für v1-Antwortkompatibilität beibehalten, aber der Scanner für die Existenz im Dependency-Registry ist eingestellt, und dieser Schlüssel ist immer `null`.
...
- `security.signals` enthält nur unterstützende Nachweise auf Status-Ebene; verwenden Sie `/scan` oder die ClawHub-Seite security-audit für vollständige Scanner-Details.
- `security.signals.dependencyRegistry` wird für v1-Antwortkompatibilität beibehalten, aber der Scanner für die Existenz im Dependency-Registry ist eingestellt, und dieser Schlüssel ist immer `null`.
...
- `category` (optional): Plugin-Kategoriefilter. Wird nur unterstützt, wenn die
  Anfrage auf Plugin-Pakete eingeschränkt ist (`/api/v1/plugins`,
...
- Authentifizierte Aufrufer können private Pakete für Herausgeber, denen sie angehören, in Listen-/Suchergebnissen sehen.
- `channel=private` gibt nur Pakete zurück, die der authentifizierte Aufrufer lesen kann.

### `GET /api/v1/packages/search`

Einheitliche Katalogsuche über Skills + Plugin-Pakete.
...
- `isOfficial` (optional): `true` oder `false`
- `category` (optional): Plugin-Kategoriefilter. Wird nur unterstützt, wenn die
  Anfrage auf Plugin-Pakete eingeschränkt ist. Gesteuerte Kategorien und veraltete v1-
...
Katalogansicht nur für Plugins über Code-Plugin- und Bundle-Plugin-Pakete.
...
Veraltete v1-Filteraliase werden bei Lese-Endpunkten weiterhin akzeptiert:
...
`trending` ist eine Rangliste der Installationen/Downloads der letzten sieben Tage und verwendet keine Gesamtzahlen über die gesamte Zeit.
...
Gibt die exakte Sicherheits- und Vertrauenszusammenfassung des Paket-Releases für Installationsclients zurück. Dies ist die öffentliche OpenClaw-Verbrauchsoberfläche zur Entscheidung, ob ein
...
- `trust.moderationState` kann null sein. Es ist `null`, wenn keine manuelle Release-
...
- `trust.blockedFromDownload` ist das Installationsblockierungssignal. OpenClaw und andere
  ...
  Blockierungsregeln aus Scanner- oder Moderationsfeldern erneut abzuleiten.
...
- Dieser Endpunkt ist absichtlich enger gefasst als Moderationsendpunkte für Besitzer/Moderatoren.
  Er macht die Installationsentscheidung und die öffentliche Erklärung verfügbar, nicht
...
- Dies ist die OpenClaw-Resolver-Oberfläche; sie vermeidet, das Archivformat anhand
...
Gibt die berechnete Bereitschaft für zukünftigen OpenClaw-Verbrauch zurück.
...
- `open`: verdächtige, schädliche, ausstehende, isolierte, widerrufene oder gemeldete Releases.
- `blocked`: isolierte, widerrufene oder schädliche Releases.
- `manual`: jedes Release mit einer manuellen Moderationsüberschreibung.
- `all`: jedes Release mit einer manuellen Überschreibung, nicht sauberem Scan-Zustand oder Paketbericht.
...
Meldet ein Paket zur Moderatorprüfung. Berichte gelten auf Paketebene und sind optional
mit einer Version verknüpft. Sie fließen in die Moderationswarteschlange ein, blenden Downloads aber nicht automatisch aus und
blockieren sie nicht selbst; Moderatoren sollten Release-Moderation verwenden, um
Artefakte zu genehmigen, zu isolieren oder zu widerrufen.
...
Streamt die exakten hochgeladenen ClawPack-Tarball-Bytes für npm-Spiegelclients.
...
- Übergaben für GitHub-gestützte Skills proxyn oder spiegeln keine Bytes. Die JSON-Antwort
  ...
  Erfolgspayload eingeschlossen.
- Downloadstatistiken werden als eindeutige Identitäten pro UTC-Tag gezählt (`userId`, wenn das API-Token gültig ist, andernfalls IP).
...
- Nur der Publisher der `openclaw`-Org und die persönlichen Publisher aktueller Mitglieder der `openclaw`-Org
  dürfen im Kanal `official` veröffentlichen.
- Veröffentlichungen im Namen anderer validieren die Berechtigung für den offiziellen Kanal weiterhin gegen das Ziel-Eigentümerkonto.
...
Von Eigentümern initiierte Soft Deletes reservieren den Slug für 30 Tage; danach kann der Slug von
einem anderen Publisher beansprucht werden. Die Delete-Antwort enthält `slugReservedUntil`, wenn dieses Ablaufdatum gilt.
...
Provider-Konto-IDs nennen; veränderliche Handles werden nur als operatorseitige Absicherung verwendet.
...
Skill-Slug-Aliase, Pakete, Paket-Inspector-Warnungen und abgeleitete Such-Digest-Zeilen des wiederhergestellten Publishers, damit
