...
x-i18n:
    generated_at: "2026-07-01T20:15:45Z"
    model: gpt-5.5
...
- Authentifizierte Schreib-Endpunkte sollten kein bloßes `Unauthorized` zurückgeben, wenn
  ...
  gelöschte/gesperrte/deaktivierte Konten sollten jeweils handlungsrelevanten Text erhalten, damit CLI-
...
- `RateLimit-Reset`: Sekunden bis zum Reset (Verzögerung)
...
- Wenn `Retry-After` vorhanden ist, warten Sie diese Anzahl Sekunden vor dem erneuten Versuch.
...
- Verwendet vertrauenswürdige Client-IP-Header, einschließlich `cf-connecting-ip`, nur wenn das
  Deployment vertrauenswürdige weitergeleitete Header explizit aktiviert.
...
- Wenn keine vertrauenswürdige Client-IP verfügbar ist, verwenden anonyme Anfragen Fallback-Buckets,
  ...
  vom Aufrufer gelieferten Pfade, Slugs, Paketnamen, Versionen, Query-Strings oder sonstigen
  Artefaktparameter.

## Fehlerantworten

Öffentliche v1-Fehlerantworten sind Plain Text mit `content-type: text/plain; charset=utf-8`.
...
sollten den Antwortkörper als menschenlesbaren String lesen. Unbekannte Query-Parameter werden
...
- Ergebnisse werden in Relevanzreihenfolge zurückgegeben (Embedding-Ähnlichkeit + exakte Slug-/Name-Token-Boosts + kleiner Popularitätsprior).
- Relevanz ist stärker als Popularität. Eine genaue Übereinstimmung mit einem Slug- oder Anzeigenamen-Token kann höher ranken als eine ungenauere Übereinstimmung mit deutlich stärkerer Interaktion.
...
- Popularität ist logarithmisch skaliert und gedeckelt. Skills mit hoher Interaktion können niedriger ranken, wenn der Query-Text schwächer übereinstimmt.
- Verdächtiger oder ausgeblendeter Moderationsstatus kann einen Skill je nach Aufruferfiltern und aktuellem Moderationsstatus aus der öffentlichen Suche entfernen.

Hinweise zur Auffindbarkeit für Publisher:

- Platzieren Sie die Begriffe, nach denen Benutzer wörtlich suchen werden, im Anzeigenamen, in der Zusammenfassung und in den Tags. Verwenden Sie ein eigenständiges Slug-Token nur, wenn es zugleich eine stabile Identität ist, die Sie beibehalten möchten.
- Benennen Sie einen Slug nicht nur um, um eine einzelne Query zu bedienen, es sei denn, der neue Slug ist langfristig ein besserer kanonischer Name. Alte Slugs werden zu Redirect-Aliassen, aber die kanonische URL, der angezeigte Slug und zukünftige Such-Digests verwenden den neuen Slug.
- Umbenennungs-Aliasse erhalten die Auflösung für alte URLs und Installationen, die über die Registry aufgelöst werden, aber das Suchranking basiert auf den kanonischen Skill-Metadaten, nachdem die Umbenennung indexiert wurde. Bestehende Statistiken bleiben beim Skill.
...
Melden Sie einen Skill zur Prüfung durch Moderatoren. Meldungen gelten auf Skill-Ebene, können optional
mit einer Version verknüpft sein und speisen die Skill-Meldewarteschlange.
...
`status` zurück auf `open` gesetzt wird. Übergeben Sie `finalAction: "hide"` mit einer triagierten
...
- Scan-Anfrage-Payloads und herunterladbare Berichte laufen nach dem Aufbewahrungsfenster aus dem Scan-Request-Store ab.
- Veröffentlichte Scans erfordern Verwaltungszugriff als Eigentümer/Publisher oder Plattform-Moderator-/Administratorberechtigungen.
...
- Scan-Jobs sind asynchron. Manuelle Scan-Anfragen werden gegenüber normaler Publish-/Backfill-Arbeit priorisiert, der Abschluss hängt jedoch weiterhin von der Verfügbarkeit der Worker ab.

### `GET /api/v1/skills/-/scan/{scanId}`

Authentifizierter Polling-Endpunkt für einen eingereichten Scan.
...
- Erfordert Verwaltungszugriff als Eigentümer/Publisher auf den Skill oder das Plugin oder Plattform-Moderator-/Administratorberechtigungen.
...
- `security.signals.dependencyRegistry` bleibt für v1-Antwortkompatibilität erhalten, aber der Scanner für die Existenz im Dependency-Registry ist außer Betrieb genommen, und dieser Schlüssel ist immer `null`.
...
- Die Antwort ist ausschließlich Security-bezogen. Sie enthält keine Skill-Card-Daten, keinen generierten Card-Status, keine Artefakt-Dateilisten und keine detaillierten Scanner-Payloads.
- `security.signals` enthält nur unterstützende Belege auf Statusebene; verwenden Sie `/scan` oder die ClawHub-Seite für Security-Audits für vollständige Scanner-Details.
- `security.signals.dependencyRegistry` bleibt für v1-Antwortkompatibilität erhalten, aber der Scanner für die Existenz im Dependency-Registry ist außer Betrieb genommen, und dieser Schlüssel ist immer `null`.
...
Reiner Plugin-Katalog-Browse über Code-Plugin- und Bundle-Plugin-Pakete.
...
`trending` ist eine Sieben-Tage-Bestenliste für Installationen/Downloads und verwendet keine Gesamtzahlen über alle Zeiten hinweg.
...
- `startDate` (erforderlich): Untere Grenze als Unix-Millisekunden für Skill-`updatedAt`.
- `endDate` (erforderlich): Obere Grenze als Unix-Millisekunden für Skill-`updatedAt`.
...
- Aktuelle GitHub-gestützte Skills mit einem Scan `clean` oder `suspicious` enthalten
...
- `startDate` (erforderlich): Untere Grenze in Unix-Millisekunden für `updatedAt` des Plugins.
- `endDate` (erforderlich): Obere Grenze in Unix-Millisekunden für `updatedAt` des Plugins.
- `limit` (optional): Ganzzahl (1-250), Standardwert `250`.
- `cursor` (optional): Paginierungs-Cursor aus der vorherigen Antwort.
- `family` (optional): `code-plugin` oder `bundle-plugin`. Wenn weggelassen,
  sind beide Plugin-Familien gemeint.
...
Plugin-spezifische Suche über code-plugin- und bundle-plugin-Pakete.
...
- Die unter `GET /api/v1/plugins` dokumentierten Legacy-v1-Filteraliase werden
  ebenfalls akzeptiert.
- Kategoriefilterung ist ein echter API-Filter, der durch Digest-Zeilen für
...
- Sortiersteuerelemente der Browser-UI für die Plugin-Suche ordnen die geladenen
  Relevanzergebnisse neu an und entsprechen dem aktuellen Browse-Verhalten von `/skills`.
...
- Private Pakete geben `404` zurück, es sei denn, der Aufrufer kann den zuständigen Publisher lesen.
...
- Erfordert ein API-Token für den Paketeigentümer, einen Eigentümer/Admin des
  Organisations-Publishers, Plattformmoderator oder Plattformadmin.
...
- ClawPack-Releases enthalten die npm-kompatiblen Felder `npmIntegrity`,
  `npmShasum` und `npmTarballName`.
- `version.sha256hash` sind veraltete Kompatibilitätsmetadaten für alte Clients.
  Sie hashen die exakten ZIP-Bytes, die von `/api/v1/packages/{name}/download`
  zurückgegeben werden. Moderne Clients sollten `version.artifact.sha256`
  verwenden, das das kanonische Release-Artefakt identifiziert.
...
Gibt die exakte Sicherheits- und Vertrauenszusammenfassung des Paket-Releases
für Installationsclients zurück. Dies ist die öffentliche OpenClaw-Nutzungsfläche
zur Entscheidung, ob ein aufgelöstes Release installiert werden kann.

Authentifizierung:

- Öffentlicher Leseendpunkt. Es ist kein Token für Eigentümer, Publisher,
  Moderator oder Admin erforderlich.
...
- `release.releaseId`, `release.version` und `release.createdAt` identifizieren
  das exakte Release, das bewertet wurde.
...
- `trust.moderationState` kann null sein. Der Wert ist `null`, wenn keine manuelle
  Release-Moderation vorhanden ist.
- `trust.blockedFromDownload` ist das Installationssperrsignal. OpenClaw und
  andere Installationsclients sollten die Installation blockieren, wenn dieser
  Wert `true` ist, anstatt Sperrregeln aus Scanner- oder Moderationsfeldern
  erneut abzuleiten.
...
- `trust.pending` bedeutet, dass mindestens eine Vertrauenseingabe noch auf den
  Abschluss wartet.
- `trust.stale` bedeutet, dass die Vertrauenszusammenfassung aus veralteten
  Eingaben berechnet wurde und vor einer Allow-Entscheidung mit hoher Sicherheit
  als aktualisierungsbedürftig behandelt werden sollte.

Hinweise:

- Dieser Endpunkt ist versionsgenau. Clients sollten ihn aufrufen, nachdem sie
  die Paketversion aufgelöst haben, die sie installieren möchten, nicht nur nach
  dem Lesen der neuesten Paketmetadaten.
- Private Pakete geben `404` zurück, es sei denn, der Aufrufer kann den zuständigen Publisher lesen.
- Dieser Endpunkt ist absichtlich enger gefasst als Moderationsendpunkte für
  Eigentümer/Moderatoren. Er legt die Installationsentscheidung und öffentliche
  Erklärung offen, nicht Identitäten von Meldenden, Meldungsinhalte, private
  Belege oder interne Prüfzeitpläne.
...
- Dies ist die OpenClaw-Resolverfläche; sie vermeidet, das Archivformat aus
  einer gemeinsam genutzten URL zu erraten.

### `GET /api/v1/packages/{name}/versions/{version}/artifact/download`

Lädt das Versionsartefakt über den expliziten Resolverpfad herunter.
...
Bereitschaftsprüfungen decken Folgendes ab:
...
- `packageName` wird als npm-Name normalisiert; das Paket kann bei geplanten
  Migrationen fehlen.
- Dies verfolgt nur die Migrationsbereitschaft. Es verändert OpenClaw nicht und
  erzeugt keine ClawPacks.
...
- `open`: verdächtige, bösartige, ausstehende, isolierte, widerrufene oder gemeldete Releases.
- `blocked`: isolierte, widerrufene oder bösartige Releases.
- `manual`: jedes Release mit einer manuellen Moderationsüberschreibung.
- `all`: jedes Release mit einer manuellen Überschreibung, einem nicht bereinigten Scanstatus oder einer Paketmeldung.
...
Meldet ein Paket zur Prüfung durch Moderatoren. Meldungen erfolgen auf Paketebene
und sind optional mit einer Version verknüpft. Sie fließen in die Moderationswarteschlange
ein, blenden Downloads jedoch nicht automatisch aus und blockieren sie nicht
selbst; Moderatoren sollten Release-Moderation verwenden, um Artefakte zu
genehmigen, zu isolieren oder zu widerrufen.
...
- Nur registrybezogene Metadaten werden nicht in das heruntergeladene Archiv injiziert.
...
- Ältere ZIP-only-Versionen werden absichtlich ausgelassen.
...
- Download-Statistiken werden pro UTC-Tag als eindeutige Identitäten gezählt (`userId`, wenn das API-Token gültig ist, andernfalls IP).
...
- Erlaubte Formularfelder sind `payload`, wiederholte `files`-Blobs oder eine einzelne `clawpack`
...
- Verwenden Sie entweder `files` oder `clawpack`, nie beides in derselben Anfrage.
- JSON-Bodys und vom Aufrufer bereitgestellte Metadaten `payload.files` / `payload.artifact`
  werden abgelehnt.
- Direkte Multipart-Veröffentlichungsanfragen sind auf 18 MB begrenzt. ClawPack-Tarballs können
  den Upload-URL-Flow bis zur 120-MB-Tarball-Grenze verwenden.
...
- Veröffentlichungen im Auftrag anderer validieren die Berechtigung für den offiziellen Channel weiterhin gegen das Ziel-Owner-Konto.

### `DELETE /api/v1/skills/{slug}` / `POST /api/v1/skills/{slug}/undelete`

Skill soft-löschen / wiederherstellen (Owner, Moderator oder Admin).
...
Nur Admins. Stellt sicher, dass ein Org-Publisher für einen Handle existiert. Wenn der Handle noch auf einen
...
Aufrufer als Owner hinzu. Dieser Endpunkt migriert keine vorhandenen Benutzer-/persönlichen Handles und markiert
...
Der Endpunkt ist standardmäßig auf Dry-Run eingestellt. Das Anwenden der Wiederherstellung erfordert `dryRun: false` und
...
Skill-Slug-Aliasse, Pakete, Paket-Inspector-Warnungen und abgeleiteten Such-Digest-Zeilen des wiederhergestellten Publishers, damit
direkte Owner-Pfade mit der neuen Publisher-Autorität übereinstimmen. Eine aktive Protected-Handle-
...
100 Zeilen begrenzt; größere Wiederherstellungen müssen zuerst eine wiederaufnehmbare Owner-Migration verwenden.
GitHub-Skill-Quellen sind Publisher-bezogen und werden als geprüft gemeldet, nicht umgeschrieben.
...
Fügt einen Star hinzu oder entfernt ihn (Highlights). Beide Endpunkte sind idempotent.
...
Veröffentlichungen, die ein ClawPack-Tarball bereitstellen, müssen die resultierende Storage-ID als
