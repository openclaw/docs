...
Legacy-Pfade `/api/...` und `/api/cli/...` bleiben aus Kompatibilitätsgründen erhalten (siehe `DEPRECATIONS.md`).
...
- Verwenden Sie Jitter-Backoff, um synchronisierte erneute Versuche zu vermeiden.
...
sollten den Antworttext als menschenlesbaren String lesen. Unbekannte Query-Parameter werden
...
- Nehmen Sie die Begriffe, nach denen Benutzer wörtlich suchen werden, in Anzeigenamen, Zusammenfassung und Tags auf. Verwenden Sie ein eigenständiges Slug-Token nur dann, wenn es auch eine stabile Identität ist, die Sie behalten möchten.
- Benennen Sie einen Slug nicht nur um, um eine einzelne Query zu bedienen, es sei denn, der neue Slug ist ein besserer langfristiger kanonischer Name. Alte Slugs werden zu Redirect-Aliasen, aber die kanonische URL, der angezeigte Slug und zukünftige Such-Digests verwenden den neuen Slug.
...
- `sort` (optional): `updated` (Standard), `recommended` (Alias: `default`), `createdAt` (Alias: `newest`), `downloads`, `stars` (Alias: `rating`), Legacy-Installations-Aliase `installsCurrent`/`installs`/`installsAllTime` werden auf `downloads` abgebildet, `trending`
- `nonSuspiciousOnly` (optional): `true`, um verdächtige (`flagged.suspicious`) Skills auszublenden
- `nonSuspicious` (optional): Legacy-Alias für `nonSuspiciousOnly`
...
- Wenn `nonSuspiciousOnly=true` ist, können Cursor-basierte Sortierungen auf einer Seite weniger als `limit` Elemente zurückgeben, weil verdächtige Skills nach dem Abruf der Seite gefiltert werden.
...
- Alte Slugs, die durch Umbenennungs-/Merge-Flows des Owners erstellt wurden, werden zum kanonischen Skill aufgelöst.
...
- Beweise werden für öffentliche Aufrufer redigiert und enthalten rohe Snippets nur für Owner/Moderatoren.
...
- `version.security` enthält normalisierten Scan-Verifizierungsstatus und Scannerdetails
...
- `moderation` ist eine aktuelle Moderations-Momentaufnahme auf Skill-Ebene, die aus der neuesten Version abgeleitet wird.
...
- Veröffentlichte Scans erfordern Verwaltungszugriff durch Inhaber/Publisher oder Moderator-/Admin-Berechtigung der Plattform.
...
- Scan-Jobs sind asynchron. Manuelle Scan-Anfragen werden vor normaler Veröffentlichungs-/Backfill-Arbeit priorisiert, die Fertigstellung hängt jedoch weiterhin von der Verfügbarkeit der Worker ab.
...
- Erfordert Verwaltungszugriff durch Inhaber/Publisher auf den Skill oder das Plugin oder Moderator-/Admin-Berechtigung der Plattform.
...
Nur für Admins verfügbare kanonische Batch-Rescan-Route. Sie akzeptiert dieselbe Payload-Struktur wie das alte `POST /api/v1/skills/-/rescan-batch`.

### `POST /api/v1/skills/-/scan/batch/status`

Nur für Admins verfügbare kanonische Batch-Status-Route. Sie akzeptiert `{ "jobIds": ["..."] }` und gibt dieselben aggregierten Zähler wie das alte `POST /api/v1/skills/-/rescan-batch/status` zurück.
...
- `security.signals.dependencyRegistry` bleibt für die Kompatibilität der v1-Antwort erhalten, aber der Existenz-Scanner für das Abhängigkeitsregister ist außer Betrieb genommen und dieser Schlüssel ist immer `null`.
- `provenance` ist nur dann `server-resolved-github-import`, wenn ClawHub während der Veröffentlichung oder des Imports ein GitHub-Repo/eine Ref/einen Commit/einen Pfad aufgelöst und gespeichert hat; andernfalls ist es `unavailable`.
...
- Die Antwort enthält nur Security-Daten. Sie enthält keine Skill-Card-Daten, keinen generierten Kartenstatus, keine Artefakt-Dateilisten und keine detaillierten Scanner-Payloads.
- `security.signals` enthält nur unterstützende Nachweise auf Statusebene; verwenden Sie `/scan` oder die ClawHub-Seite für Security-Audits, um vollständige Scanner-Details zu erhalten.
- `security.signals.dependencyRegistry` bleibt für die Kompatibilität der v1-Antwort erhalten, aber der Existenz-Scanner für das Abhängigkeitsregister ist außer Betrieb genommen und dieser Schlüssel ist immer `null`.
...
- Skill-Einträge bleiben durch die Skill-Registry hinterlegt und können weiterhin nur über `POST /api/v1/skills` veröffentlicht werden.
...
`trending` ist eine siebentägige Installations-/Download-Bestenliste und verwendet keine Gesamtzahlen über alle Zeiträume.
...
- Aktuelle GitHub-basierte Skills mit einem `clean`- oder `suspicious`-Scan enthalten
...
- Die unter `GET /api/v1/plugins` dokumentierten Legacy-v1-Filteraliase werden ebenfalls
  akzeptiert.
- Kategoriefilterung ist ein echter API-Filter, der auf Digest-Zeilen für Plugin-Kategorien basiert,
  keine Umschreibung der Suchanfrage.
- Ergebnisse werden nach Relevanz sortiert zurückgegeben und derzeit nicht paginiert.
- Sortiersteuerungen der Browser-UI für die Plugin-Suche ordnen die geladenen Relevanzergebnisse neu an,
...
- Erfordert ein API-Token für den Paketinhaber, einen Owner/Admin des Org-Publishers,
  Platform Moderator oder Platform Admin.
...
zurück. Dies ist die öffentliche OpenClaw-Verbrauchsoberfläche zur Entscheidung, ob ein
aufgelöstes Release installiert werden kann.

Auth:

- Öffentlicher Lese-Endpunkt. Es ist kein Token für Owner, Publisher, Moderator oder Admin
  erforderlich.
...
- `trust.reasons` ist die Liste der nutzerseitigen und Audit-Erklärungen. Reason-Codes
...
- Dieser Endpunkt ist versionsexakt. Clients sollten ihn aufrufen, nachdem sie die
  ...
  Paketmetadaten.
- Private Pakete geben `404` zurück, sofern der Aufrufer den besitzenden Publisher nicht lesen kann.
- Dieser Endpunkt ist absichtlich enger gefasst als Moderationsendpunkte für Owner/Moderatoren.
...
- `open`: verdächtige, schädliche, ausstehende, quarantined, widerrufene oder gemeldete Releases.
- `blocked`: quarantined, widerrufene oder schädliche Releases.
- `manual`: jedes Release mit einer manuellen Moderationsüberschreibung.
- `all`: jedes Release mit einer manuellen Überschreibung, einem nicht sauberen Scan-Status oder einem Paket-Report.
...
Meldet ein Paket zur Moderator-Prüfung. Reports sind paketbezogen und optional
mit einer Version verknüpft. Sie speisen die Moderationswarteschlange, verbergen aber Downloads nicht automatisch
und blockieren sie auch nicht von selbst; Moderatoren sollten Release-Moderation verwenden, um
...
- Erfordert ein API-Token für den Paketeigentümer, ein Publisher-Mitglied,
...
- Ausstehende VirusTotal-Scans blockieren Lesevorgänge nicht; schädliche Releases können an anderer Stelle trotzdem zurückgehalten werden.
- Private Pakete geben `404` zurück, sofern der Aufrufer den besitzenden Publisher nicht lesen kann.
...
- Nur registry-bezogene Metadaten werden nicht in das heruntergeladene Archiv injiziert.
...
- Packuments für scoped Pakete unterstützen sowohl `/api/npm/@scope/name` als auch npm’s
...
- `slug` (erforderlich)
- `hash` (erforderlich): 64 Zeichen langer hexadezimaler sha256 des Bundle-Fingerprints
...
Lädt die ZIP-Datei einer gehosteten Skill-Version herunter oder gibt eine GitHub-Quellenübergabe für einen
...
- GitHub-gestützte Skill-Übergaben proxyn oder spiegeln keine Bytes. Die JSON-Antwort
  ...
  der Erfolgs-Payload einbezogen.
...
- Optionales Payload-Feld: `migrateOwner`. Wenn `true` mit `ownerHandle`, kann ein
  bestehender Skill zu diesem Eigentümer verschoben werden, wenn der Akteur Admin/Eigentümer bei beiden
  aktuellen und Ziel-Publishern ist. Ohne dieses Opt-in werden Eigentümeränderungen
  abgelehnt.
...
Vom Eigentümer ausgelöste Soft-Löschungen reservieren den Slug für 30 Tage; danach kann der Slug von
...
markiert den Publisher nicht als trusted/official.
...
Der Endpunkt verwendet standardmäßig einen Testlauf. Zum Anwenden der Wiederherstellung sind `dryRun: false` und
...
Skill-Slug-Aliasse, Pakete, Paket-Inspector-Warnungen und abgeleitete Such-Digest-Zeilen, sodass
...
- Body: `{ "handle": "gingiris", "nextUserHandle": "gingiris-1031", "previousGitHubProviderAccountId": "123", "nextGitHubProviderAccountId": "456", "reason": "Verifizierte Kontinuität des Kontos für Issue #2555", "confirmIdentityVerified": true, "dryRun": false }`
- Response: `{ "ok": true, "dryRun": false, "recovered": true, "publisherId": "...", "handle": "gingiris", "previousUser": { "userId": "...", "handle": "gingiris", "nextHandle": "gingiris-recovered", "githubProviderAccountId": "123", "authAccountCount": 1 }, "nextUser": { "userId": "...", "handle": "gingiris-1031", "nextHandle": "gingiris", "githubProviderAccountId": "456", "authAccountCount": 1 }, "retiredPersonalPublisher": null, "resourceOwnerMigration": { "limitPerTable": 100, "skills": 1, "skillSlugAliases": 1, "packages": 0, "packageInspectorWarnings": 0, "githubSourcesChecked": 1, "handleReservations": 1 }, "identityVerified": true, "reason": "Verifizierte Kontinuität des Kontos für Issue #2555" }`
...
### Endpunkte zur Übertragung der Ownership
...
Benutzer sperren und eigene Skills hart löschen (nur Moderator/Admin).
...
Benutzersperre aufheben und berechtigte Skills wiederherstellen (nur Admin).

Body:

```json
{ "handle": "user_handle", "reason": "optionaler Grund für das Aufheben der Sperre" }
```

oder

```json
{ "userId": "users_...", "reason": "optionaler Grund für das Aufheben der Sperre" }
```
...
Inhalte wiederherzustellen (nur Admin). Standardmäßig Testlauf, sofern `dryRun` nicht `false` ist.
