...
## Öffentliche Katalog-Wiederverwendung
...
- `X-RateLimit-Remaining` / `RateLimit-Remaining`: exakt verbleibendes Budget, wenn vorhanden.
  Geshardete erfolgreiche Anfragen lassen diesen Header aus, statt einen ungefähren globalen Wert zurückzugeben.
...
- ASCII-Text wird an Wort- und Interpunktionsgrenzen tokenisiert. Zum Beispiel enthält `personal-map` ein eigenständiges `map`-Token, während `amap-jsapi-skill` `amap`, `jsapi` und `skill` enthält; eine Suche nach `map` gibt `personal-map` daher eine stärkere lexikalische Übereinstimmung als `amap-jsapi-skill`.
...
- Benennen Sie einen Slug nicht nur um, um eine einzelne Query zu bedienen, außer der neue Slug ist langfristig ein besserer kanonischer Name. Alte Slugs werden zu Redirect-Aliasen, aber die kanonische URL, der angezeigte Slug und künftige Such-Digests verwenden den neuen Slug.
- Umbenennungs-Aliase erhalten die Auflösung für alte URLs und Installationen, die über die Registry aufgelöst werden, aber das Such-Ranking basiert nach der Indexierung der Umbenennung auf den kanonischen Skill-Metadaten. Bestehende Statistiken bleiben beim Skill.
...
- Wenn `nonSuspiciousOnly=true`, können Cursor-basierte Sortierungen auf einer Seite weniger als `limit` Einträge zurückgeben, weil verdächtige Skills nach dem Seitenabruf gefiltert werden.
...
- Owner und Moderatoren können für versteckte Skills auf Moderationsdetails zugreifen.
...
Melden Sie einen Skill zur Überprüfung durch Moderatoren. Meldungen gelten auf Skill-Ebene, sind optional mit einer Version verknüpft und fließen in die Skill-Meldewarteschlange ein.
...
Moderator-/Admin-Endpunkt für die Skill-Meldeannahme.
...
- Gibt `queue.queuedAhead` und `queue.position` zurück, solange der Scan in der Warteschlange steht, damit Clients anzeigen können, wie viele priorisierte manuelle Scans vor der Anfrage liegen. Sehr große Warteschlangen werden begrenzt und mit `queuedAheadIsEstimate: true` gemeldet.
...
- `security.signals.dependencyRegistry` bleibt für die Kompatibilität der v1-Antwort erhalten, aber der Scanner zur Existenz der Dependency-Registry ist eingestellt und dieser Schlüssel ist immer `null`.
...
- Verwenden Sie `/verify`, wenn Sie den Verifizierungsumschlag der Skill Card für einen einzelnen Skill benötigen, `/card`, wenn Sie generiertes Card-Markdown benötigen, und `/scan`, wenn Sie detaillierte Scanner-Daten benötigen.
...
`trending` ist eine Sieben-Tage-Rangliste für Installationen/Downloads und verwendet keine Gesamtzahlen über die gesamte Laufzeit.
...
- Öffentlicher Lese-Endpunkt. Es ist kein Owner-, Publisher-, Moderator- oder Admin-Token
  erforderlich.
...
- `trust.reasons` ist die benutzerseitige und Audit-Erklärungsliste. Reason-Codes
...
- Status des offiziellen Channels
...
Meldet ein Paket zur Moderator-Prüfung. Meldungen erfolgen auf Paketebene und sind optional
mit einer Version verknüpft. Sie speisen die Moderationswarteschlange, blenden aber Downloads
nicht automatisch aus und blockieren sie nicht selbst; Moderatoren sollten Release-Moderation verwenden, um
...
- Scoped-Package-Packuments unterstützen sowohl `/api/npm/@scope/name` als auch npm's
...
- GitHub-gestützte Skill-Übergaben proxyen oder spiegeln keine Bytes. Die JSON-Antwort
  ...
  Payload-Metadatum eingeschlossen.
...
- Optionales Payload-Feld: `migrateOwner`. Wenn `true` mit `ownerHandle`, kann ein
  vorhandener Skill zu diesem Owner wechseln, wenn der Akteur ein Admin/Owner bei beiden
...
- Direkte Multipart-Veröffentlichungsanfragen sind auf 18MB begrenzt. ClawPack-Tarballs können
  den Upload-URL-Flow bis zur Tarball-Obergrenze von 120MB verwenden.
...
alten geteilten Benutzer-/persönlichen Publisher zeigt, migriert der Endpunkt ihn zuerst in einen Org-Publisher.
...
Provider-Konto-IDs benennen; veränderliche Handles werden nur als operator-facing Schutz verwendet.

Der Endpoint verwendet standardmäßig Dry-run. Zum Anwenden der Wiederherstellung sind `dryRun: false` und
...
### Endpoints zur Verwaltung von Owner-Slugs
...
Eine Benutzersperre aufheben und berechtigte Skills wiederherstellen (nur Admin).
...
Einen Star hinzufügen/entfernen (Highlights). Beide Endpoints sind idempotent.
