...
- `X-RateLimit-Remaining` / `RateLimit-Remaining`: exaktes verbleibendes Budget, wenn vorhanden.
  Erfolgreiche shard-basierte Anfragen lassen diesen Header aus, statt einen ungefähren globalen Wert zurückzugeben.
...
- Relevanz ist stärker als Popularität. Ein präziser Slug- oder Anzeigename-Token-Treffer kann einen lockereren Treffer mit deutlich stärkerem Engagement übertreffen.
...
- Benennen Sie einen Slug nicht nur um, um eine einzelne Query zu verfolgen, es sei denn, der neue Slug ist ein besserer langfristiger kanonischer Name. Alte Slugs werden zu Weiterleitungsaliasen, aber die kanonische URL, der angezeigte Slug und zukünftige Such-Digests verwenden den neuen Slug.
...
- Wenn `nonSuspiciousOnly=true` ist, können cursorbasierte Sortierungen auf einer Seite weniger als `limit` Elemente zurückgeben, weil verdächtige Skills nach dem Abruf der Seite gefiltert werden.
...
- Alte Slugs, die durch Owner-Umbenennungs-/Zusammenführungsflows erstellt wurden, werden auf den kanonischen Skill aufgelöst.
...
Einen Skill zur Moderatorprüfung melden. Meldungen gelten auf Skill-Ebene, sind optional
mit einer Version verknüpft und fließen in die Warteschlange für Skill-Meldungen ein.
...
- `version.security` enthält normalisierten Verifizierungsstatus des Scans und Scanner-Details
...
- Veröffentlichte Scans erfordern Owner-/Publisher-Verwaltungszugriff oder Plattform-Moderator-/Admin-Berechtigung.
...
- Erfordert Owner-/Publisher-Verwaltungszugriff auf den Skill oder das Plugin oder Plattform-Moderator-/Admin-Berechtigung.
...
- `security.signals.dependencyRegistry` wird für die v1-Antwortkompatibilität beibehalten, aber der Scanner für die Existenz im Dependency Registry wurde eingestellt, und dieser Schlüssel ist immer `null`.
...
`trending` ist eine Installations-/Download-Bestenliste über sieben Tage und verwendet keine Gesamtwerte über den gesamten Zeitraum.
...
- Body: ZIP-Archiv.
- Jedes exportierte Plugin hat seine Wurzel unter `{family}/{packageName}/`.
...
- Die Legacy-v1-Filteraliasnamen, die unter `GET /api/v1/plugins` dokumentiert sind, werden ebenfalls
  akzeptiert.
...
- Sortiersteuerelemente der Browser-Benutzeroberfläche für die Plugin-Suche ordnen die geladenen Relevanzergebnisse neu,
...
- Erfordert ein API-Token für den Paketeigentümer, einen Eigentümer/Admin des Organisations-Publishers,
...
- `trust.blockedFromDownload` ist das Signal zur Installationsblockierung. OpenClaw und andere
...
- `all`: jedes Release mit einer manuellen Überschreibung, einem nicht sauberen Scanzustand oder einem Paketbericht.
...
Meldet ein Paket zur Prüfung durch Moderatoren. Berichte sind paketbezogen und optional
mit einer Version verknüpft. Sie speisen die Moderationswarteschlange, verbergen aber nicht automatisch und
blockieren selbst keine Downloads; Moderatoren sollten Release-Moderation verwenden, um
...
- Plugin-/Paketarchive sind ZIP-Dateien mit einer `package/`-Wurzel, damit alte OpenClaw-
...
Lädt ein gehostetes ZIP einer Skill-Version herunter oder gibt eine Übergabe an den GitHub-Quellcode für einen
...
- Optionales Payload-Feld: `migrateOwner`. Wenn `true` mit `ownerHandle`, kann
  ein bestehender Skill zu diesem Owner verschoben werden, wenn der Akteur Admin/Owner sowohl beim
...
- Veröffentlichungen im Auftrag eines anderen validieren die Berechtigung für den offiziellen Channel weiterhin gegen das Ziel-Owner-Konto.
...
älteren gemeinsamen Benutzer-/persönlichen Publisher verweist, migriert der Endpunkt es zuerst in einen Organisations-Publisher.
...
Authentifizierte Self-Service-Erstellung von Organisations-Publishern. Erstellt einen neuen Organisations-Publisher und fügt den
...
Skill-Slug-Aliasse, Pakete, Paket-Inspector-Warnungen und abgeleitete Search-Digest-Zeilen des wiederhergestellten Publishers, damit
...
Sperrt einen Benutzer und löscht zugehörige Skills endgültig (nur Moderator/Admin).
