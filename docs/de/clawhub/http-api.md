...
x-i18n:
    generated_at: "2026-07-02T17:33:55Z"
    model: gpt-5.5
...
- Ergebnisse werden in Relevanzreihenfolge zurückgegeben (Embedding-Ähnlichkeit + exakte Slug-/Namen-Token-Boosts + ein kleiner Popularitätsprior).
- Relevanz ist stärker als Popularität. Eine präzise Übereinstimmung mit einem Slug- oder Anzeigenamen-Token kann höher ranken als eine lockerere Übereinstimmung mit deutlich stärkerer Interaktion.
...
- Popularität wird logarithmisch skaliert und gedeckelt. Skills mit hoher Interaktion können niedriger ranken, wenn der Query-Text schwächer übereinstimmt.
...
- Nehmen Sie die Begriffe, nach denen Nutzer wörtlich suchen werden, in Anzeigenamen, Zusammenfassung und Tags auf. Verwenden Sie ein eigenständiges Slug-Token nur, wenn es auch eine stabile Identität ist, die Sie beibehalten möchten.
...
- Wenn ein Skill unerwartet unsichtbar ist, prüfen Sie zuerst im angemeldeten Zustand den Moderationsstatus mit `clawhub inspect @owner/slug`, bevor Sie rankingbezogene Metadaten ändern.
...
- Wenn `nonSuspiciousOnly=true`, können Cursor-basierte Sortierungen auf einer Seite weniger als `limit` Elemente zurückgeben, weil verdächtige Skills nach dem Abrufen der Seite gefiltert werden.
...
Moderator-/Admin-Endpunkt für eingehende Skill-Meldungen.
...
- `version.security` enthält den normalisierten Status der Scan-Verifizierung und Scannerdetails
...
- Scan-Jobs sind asynchron. Manuelle Scan-Anfragen werden vor normalen Veröffentlichungs-/Backfill-Arbeiten priorisiert, der Abschluss hängt jedoch weiterhin von der Verfügbarkeit der Worker ab.
...
Nur für Admins bestimmte kanonische Route für Batch-Rescans. Sie akzeptiert dieselbe Payload-Struktur wie das alte `POST /api/v1/skills/-/rescan-batch`.

### `POST /api/v1/skills/-/scan/batch/status`

Nur für Admins bestimmte kanonische Route für den Batch-Status. Sie akzeptiert `{ "jobIds": ["..."] }` und gibt dieselben aggregierten Zähler wie das alte `POST /api/v1/skills/-/rescan-batch/status` zurück.
...
- `security.signals.dependencyRegistry` wird aus Gründen der v1-Antwortkompatibilität beibehalten, aber der Scanner für die Existenz in der Abhängigkeitsregistrierung ist außer Betrieb, und dieser Schlüssel ist immer `null`.
...
- `security.signals` enthält nur unterstützende Belege auf Statusebene; verwenden Sie `/scan` oder die ClawHub-Sicherheitsauditseite für vollständige Scanner-Details.
- `security.signals.dependencyRegistry` wird aus Gründen der v1-Antwortkompatibilität beibehalten, aber der Scanner für die Existenz in der Abhängigkeitsregistrierung ist außer Betrieb, und dieser Schlüssel ist immer `null`.
...
- Skills-Einträge bleiben durch die Skills-Registry abgesichert und können weiterhin nur über `POST /api/v1/skills` veröffentlicht werden.
...
- `startDate` (erforderlich): Untere Grenze in Unix-Millisekunden für `updatedAt` des Skill.
- `endDate` (erforderlich): Obere Grenze in Unix-Millisekunden für `updatedAt` des Skill.
...
- Body: ZIP-Archiv.
- Jeder exportierte Skill hat den Stamm unter `{publisher}/{slug}/`.
...
- Sortiersteuerungen der Browser-UI für die Plugin-Suche ordnen die geladenen Relevanzergebnisse neu,
  entsprechend dem aktuellen `/skills`-Durchsuchverhalten.
...
- `trust.stale` bedeutet, dass die Vertrauenszusammenfassung aus veralteten Eingaben berechnet wurde und
  vor einer Allow-Entscheidung mit hohem Vertrauen als aktualisierungsbedürftig behandelt werden sollte.
...
- Dieser Endpunkt ist absichtlich enger gefasst als Moderationsendpunkte für Eigentümer/Moderatoren.
  ...
  Melderidentitäten, Meldungstexte, private Beweise oder interne Review-
  Zeitpläne.
...
- `all`: jedes Release mit manueller Überschreibung, nicht sauberem Scan-Zustand oder Paketmeldung.
...
automatisch aus und blockieren sie nicht von selbst; Moderatoren sollten Release-Moderation verwenden, um
...
Moderator-/Admin-Endpunkt zum Erledigen oder Wiederöffnen von Paketmeldungen.
...
- Zulässige Formularfelder sind `payload`, wiederholte `files`-Blobs oder eine `clawpack`-
  ...
  Upload-URL-Flow zurückgegeben wurde. Veröffentlichungen mit bereitgestellten Storage-IDs müssen außerdem das
...
Nur Admin. Stellt sicher, dass ein Org-Publisher für ein Handle existiert. Wenn das Handle noch auf einen
älteren gemeinsamen Benutzer-/persönlichen Publisher zeigt, migriert der Endpunkt es zuerst in einen Org-Publisher.
...
Der Endpunkt verwendet standardmäßig den Dry-Run. Das Anwenden der Wiederherstellung erfordert `dryRun: false` und
...
Skill-Slug-Aliase, Pakete, Package-Inspector-Warnungen und abgeleitete Such-Digest-Zeilen, sodass
Direkteigentümer-Pfade mit der neuen Publisher-Autorität übereinstimmen. Eine aktive Reservierung für geschützte Handles
...
GitHub-Skill-Quellen sind publisherbezogen und werden als geprüft gemeldet, statt umgeschrieben zu werden.
...
Inhalte wiederherzustellen (nur Admin). Verwendet standardmäßig den Dry-Run, sofern `dryRun` nicht `false` ist.
...
die einen ClawPack-Tarball vorbereiten, müssen die resultierende Speicher-ID als
