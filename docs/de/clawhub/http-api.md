...
Verzeichnisse von Drittanbietern können die öffentlichen Lese-Endpunkte verwenden, um ClawHub Skills aufzulisten oder zu durchsuchen. Bitte cachen Sie Ergebnisse, beachten Sie `429`/`Retry-After`, verlinken Sie Benutzer zurück auf den kanonischen ClawHub-Eintrag (`https://clawhub.ai/<owner>/skills/<slug>`) und vermeiden Sie den Eindruck, ClawHub würde die Drittanbieter-Website unterstützen. Versuchen Sie nicht, ausgeblendete, private oder durch Moderation blockierte Inhalte außerhalb der öffentlichen API-Oberfläche zu spiegeln.
...
- Wenn keine vertrauenswürdige Client-IP verfügbar ist, verwenden anonyme Anfragen Fallback-Buckets, die nur nach Rate-Limit-Art begrenzt sind. Diese Fallback-Buckets enthalten keine vom Aufrufer bereitgestellten Pfade, Slugs, Paketnamen, Versionen, Query-Strings oder andere Artefaktparameter.

## Fehlerantworten

Öffentliche v1-Fehlerantworten sind Klartext mit `content-type: text/plain; charset=utf-8`.
Dies umfasst Validierungsfehler (`400`), fehlende öffentliche Ressourcen (`404`), Authentifizierungs- und Berechtigungsfehler (`401`/`403`), Rate Limits (`429`) und blockierte Downloads. Clients sollten den Antwortkörper als menschenlesbare Zeichenkette lesen. Unbekannte Abfrageparameter werden aus Kompatibilitätsgründen ignoriert, aber erkannte Abfrageparameter mit ungültigen Werten geben `400` zurück.
...
- Ergebnisse werden in Relevanzreihenfolge zurückgegeben (Embedding-Ähnlichkeit + exakte Slug-/Namens-Token-Boosts + eine kleine Popularitätspriorisierung).
- Relevanz ist stärker als Popularität. Eine präzise Übereinstimmung mit einem Slug- oder Anzeigenamen-Token kann höher rangieren als eine ungenauere Übereinstimmung mit deutlich stärkerem Engagement.
...
- Popularität wird logarithmisch skaliert und begrenzt. Skills mit hohem Engagement können niedriger rangieren, wenn der Abfragetext schwächer übereinstimmt.
...
- Benennen Sie einen Slug nicht nur um, um eine einzelne Abfrage zu bedienen, es sei denn, der neue Slug ist langfristig der bessere kanonische Name. Alte Slugs werden zu Redirect-Aliassen, aber die kanonische URL, der angezeigte Slug und zukünftige Such-Digests verwenden den neuen Slug.
...
- Nutzdaten von Scan-Anfragen und herunterladbare Berichte laufen nach dem Aufbewahrungsfenster aus dem Scan-Request-Store ab.
- Veröffentlichte Scans erfordern Verwaltungszugriff als Owner/Publisher oder die Berechtigung eines Plattform-Moderators/-Administrators.
...
- Erfordert Verwaltungszugriff als Owner/Publisher auf den Skill oder das Plugin oder die Berechtigung eines Plattform-Moderators/-Administrators.
...
Nur für Administratoren verfügbare kanonische Route für Batch-Status. Sie akzeptiert `{ "jobIds": ["..."] }` und gibt dieselben aggregierten Zähler zurück wie das Legacy-`POST /api/v1/skills/-/rescan-batch/status`.
...
- `tag` (optional): löst eine Version mit Tag auf (zum Beispiel `latest`).
...
- Skill-Identität, Publisher-Identität und Metadaten der ausgewählten Version sind Top-Level-Felder des Umschlags (`slug`, `displayName`, `publisherHandle`, `version`, `resolvedFrom`, `tag`, `createdAt`), damit Shell-Automatisierung sie lesen kann, ohne verschachtelte Wrapper auszupacken.
...
- `security.signals.dependencyRegistry` bleibt für die v1-Antwortkompatibilität erhalten, aber der Scanner für die Existenz im Dependency Registry wurde eingestellt und dieser Schlüssel ist immer `null`.
...
- Die Antwort ist ausschließlich Security-bezogen. Sie enthält keine Skill-Card-Daten, keinen generierten Kartenstatus, keine Artefakt-Dateilisten und keine detaillierten Scanner-Payloads.
- `security.signals` enthält nur unterstützende Nachweise auf Statusebene; verwenden Sie `/scan` oder die Security-Audit-Seite von ClawHub für vollständige Scanner-Details.
- `security.signals.dependencyRegistry` bleibt für die v1-Antwortkompatibilität erhalten, aber der Scanner für die Existenz im Dependency Registry wurde eingestellt und dieser Schlüssel ist immer `null`.
...
- `category` (optional): Plugin-Kategoriefilter. Wird nur unterstützt, wenn die
  Anfrage auf Plugin-Pakete eingegrenzt ist (`/api/v1/plugins`,
...
- `isOfficial` (optional): `true` oder `false`
- `category` (optional): Plugin-Kategoriefilter. Wird nur unterstützt, wenn die
  Anfrage auf Plugin-Pakete eingegrenzt ist. Kontrollierte Kategorien und
  Legacy-v1-Filteraliase sind unter `GET /api/v1/plugins` dokumentiert.
...
- Kategoriefilterung ist ein echter API-Filter, der durch Plugin-Kategorie-Digest-
  Zeilen gestützt wird, kein Umschreiben der Suchabfrage.
- Ergebnisse werden nach Relevanz sortiert zurückgegeben und werden derzeit nicht paginiert.
- Sortiersteuerungen der Browseroberfläche für die Plugin-Suche ordnen die geladenen Relevanzergebnisse neu an
...
- `trust.moderationState` kann null sein. Er ist `null`, wenn keine manuelle Release-
...
Meldet ein Paket zur Review durch Moderatoren. Meldungen gelten auf Paketebene und sind optional
...
Moderations-/Admin-Endpunkt für die Entgegennahme von Paketmeldungen.

Authentifizierung:

- Erfordert ein API-Token für einen Moderations- oder Admin-Benutzer.
...
Owner-/Moderations-Endpunkt für die Sichtbarkeit der Paketmoderation.

Authentifizierung:

- Erfordert ein API-Token für den Paketeigentümer, ein Publisher-Mitglied, einen Moderations- oder
  Admin-Benutzer.
...
Moderations-/Admin-Endpunkt zum Abschließen oder erneuten Öffnen von Paketmeldungen.
...
Moderations-/Admin-Endpunkt für die Prüfung von Paket-Releases.
...
Lädt eine gehostete Skill-Versions-ZIP herunter oder gibt eine GitHub-Quellenübergabe für einen
...
- Erlaubte Formularfelder sind `payload`, wiederholte `files`-Blobs oder eine `clawpack`-
  ...
  Upload-URL-Flow zurückgegeben wurde. Veröffentlichungen mit gestagten Storage-IDs müssen außerdem das
...
- Nur der `openclaw`-Org-Publisher und persönliche Publisher aktueller Mitglieder der
  `openclaw`-Org dürfen im `official`-Channel veröffentlichen.
- Veröffentlichungen im Auftrag eines anderen validieren die Berechtigung für den offiziellen Channel weiterhin gegen das Ziel-Eigentümerkonto.

### `DELETE /api/v1/skills/{slug}` / `POST /api/v1/skills/{slug}/undelete`

Skill per Soft Delete löschen / wiederherstellen (Eigentümer, Moderation oder Admin).
...
Ausblendungen durch Moderation/Admins und Sicherheitsentfernungen laufen auf diese Weise nicht ab.
...
### Endpunkte zum Übertragen der Inhaberschaft
...
Fügt einen Stern hinzu oder entfernt einen Stern (Highlights). Beide Endpunkte sind idempotent.
