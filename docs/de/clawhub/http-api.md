...
x-i18n:
    generated_at: "2026-05-12T12:53:31Z"
    model: gpt-5.5
    provider: openai
...
Verzeichnisse von Drittanbietern können die öffentlichen Lese-Endpunkte verwenden, um ClawHub Skills aufzulisten oder zu durchsuchen. Cachen Sie Ergebnisse, beachten Sie `429`/`Retry-After`, verlinken Sie Benutzer zurück auf den kanonischen ClawHub-Eintrag (`https://clawhub.ai/<owner>/<slug>`), und vermeiden Sie den Eindruck, ClawHub würde die Drittanbieter-Website empfehlen. Versuchen Sie nicht, versteckte, private oder durch Moderation blockierte Inhalte außerhalb der öffentlichen API-Oberfläche zu spiegeln.
...
- Wenn keine vertrauenswürdige Client-IP verfügbar ist, verwenden anonyme Download-Anfragen einen Endpunkt-spezifischen Fallback-Bucket anstelle eines globalen `ip:unknown`-Buckets. Anonyme Lese-/Schreibanfragen verwenden weiterhin den gemeinsamen Unknown-Bucket, damit Routing ohne IP sichtbar und konservativ bleibt.
...
- Relevanz ist stärker als Popularität. Eine präzise Übereinstimmung mit einem Slug- oder Anzeigenamen-Token kann eine ungenauere Übereinstimmung mit deutlich mehr Downloads übertreffen.
- ASCII-Text wird an Wort- und Zeichensetzungsgrenzen tokenisiert. Beispielsweise enthält `personal-map` ein eigenständiges `map`-Token, während `amap-jsapi-skill` `amap`, `jsapi` und `skill` enthält; eine Suche nach `map` gibt `personal-map` daher eine stärkere lexikalische Übereinstimmung als `amap-jsapi-skill`.
- Downloads werden als kleiner logarithmisch skalierter Prior und als Tie-Breaker verwendet, nicht als primäres Ranking-Signal. Skills mit vielen Downloads können niedriger eingestuft werden, wenn der Suchtext eine schwächere Übereinstimmung ist.
...
- Setzen Sie die Begriffe, nach denen Benutzer wortwörtlich suchen werden, in Anzeigename, Zusammenfassung und Tags. Verwenden Sie ein eigenständiges Slug-Token nur, wenn es auch eine stabile Identität ist, die Sie beibehalten möchten.
- Benennen Sie einen Slug nicht nur um, um eine einzelne Suchanfrage zu bedienen, es sei denn, der neue Slug ist ein besserer langfristiger kanonischer Name. Alte Slugs werden zu Redirect-Aliasen, aber die kanonische URL, der angezeigte Slug und zukünftige Such-Digests verwenden den neuen Slug.
...
- Nachweise werden für öffentliche Aufrufer redigiert und enthalten nur für Owner/Moderatoren Roh-Snippets.
...
- `security.hasScanResult` ist nur dann `true`, wenn ein Scanner ein endgültiges Verdict (`clean`, `suspicious` oder `malicious`) erzeugt hat.
...
- `requiresBrowser`, `requiresDesktop`, `requiresNativeDeps`,
  ...
  (optional): `true`/`1` als Kurzform für Tags zu Umgebungsanforderungen
...
- Dies ist die OpenClaw-Resolver-Oberfläche; sie vermeidet das Erraten des
  Archivformats anhand einer gemeinsam genutzten URL.
...
- Erfordert ein API-Token für einen Moderator oder Adminbenutzer.
...
- Erfordert ein API-Token für einen Adminbenutzer.
...
Melden Sie ein Paket zur Moderatorprüfung. Berichte gelten auf Paketebene und sind optional
...
Moderator-/Admin-Endpunkt für den Eingang von Paketberichten.
...
Moderator-/Admin-Endpunkt zum Auflösen oder Wiederöffnen von Paketberichten.
...
- `slug` (erforderlich)
- `hash` (erforderlich): 64 Zeichen langer Hex-SHA-256 des Bundle-Fingerprints
...
- Optionales Payload-Feld: `migrateOwner`. Wenn `true` mit `ownerHandle`, kann ein
  ...
  aktuellen als auch beim Ziel-Publisher ist. Ohne dieses Opt-in werden Besitzeränderungen
  abgelehnt.
...
älteren gemeinsamen Benutzer-/persönlichen Publisher zeigt, migriert der Endpunkt ihn zuerst in einen Organisations-Publisher.
...
Nur für Admins. Reserviert Root-Slugs und Paketnamen für den rechtmäßigen Besitzer, ohne ein
...
- `merge` blendet das Quelleintrag aus und leitet den Quell-Slug zum Zieleintrag weiter.
...
Fügt einen Stern hinzu bzw. entfernt ihn (Highlights). Beide Endpunkte sind idempotent.
