...
x-i18n:
    generated_at: "2026-07-01T07:56:07Z"
    model: gpt-5.5
...
Veraltete `/api/...` und `/api/cli/...` bleiben aus Kompatibilitätsgründen erhalten (siehe `DEPRECATIONS.md`).
OpenAPI: `/api/v1/openapi.json`.

## Wiederverwendung des öffentlichen Katalogs

Verzeichnisse von Drittanbietern können die öffentlichen Lese-Endpunkte verwenden, um ClawHub-Skills aufzulisten oder zu durchsuchen. Cachen Sie Ergebnisse, beachten Sie `429`/`Retry-After`, verlinken Sie Nutzer zurück auf den kanonischen ClawHub-Eintrag (`https://clawhub.ai/<owner>/skills/<slug>`), und vermeiden Sie den Eindruck, ClawHub würde die Drittanbieter-Website empfehlen. Versuchen Sie nicht, versteckte, private oder durch Moderation blockierte Inhalte außerhalb der öffentlichen API-Oberfläche zu spiegeln.

Web-Slug-Kurzbefehle werden über Registry-Familien hinweg aufgelöst, aber API-Clients sollten die kanonischen URLs verwenden, die von Lese-Endpunkten zurückgegeben werden, statt die Routenpriorität zu rekonstruieren.
...
- Authentifizierte Anfragen (gültiges Bearer-Token): pro Nutzer-Bucket erzwungen.
...
- Wenn `Retry-After` vorhanden ist, warten Sie vor einem erneuten Versuch diese Anzahl Sekunden.
...
- Wenn keine vertrauenswürdige Client-IP verfügbar ist, verwenden anonyme Anfragen Fallback-Buckets,
  die nur nach Art der Ratenbegrenzung scoped sind. Diese Fallback-Buckets enthalten keine
...
- Relevanz ist stärker als Popularität. Eine präzise Übereinstimmung mit einem Slug- oder Anzeigenamen-Token kann vor einer lockereren Übereinstimmung mit deutlich stärkerem Engagement rangieren.
...
- Platzieren Sie die Begriffe, nach denen Nutzer wörtlich suchen werden, im Anzeigenamen, in der Zusammenfassung und in Tags. Verwenden Sie ein eigenständiges Slug-Token nur dann, wenn es auch eine stabile Identität ist, die Sie beibehalten möchten.
- Benennen Sie einen Slug nicht nur um, um einer einzelnen Query nachzujagen, es sei denn, der neue Slug ist langfristig ein besserer kanonischer Name. Alte Slugs werden zu Weiterleitungsaliasen, aber die kanonische URL, der angezeigte Slug und zukünftige Such-Digests verwenden den neuen Slug.
- Umbenennungsaliase erhalten die Auflösung für alte URLs und Installationen, die über die Registry auflösen, aber das Suchranking basiert nach der Indizierung der Umbenennung auf den kanonischen Skill-Metadaten. Vorhandene Statistiken bleiben beim Skill.
...
- `createdAt` ist stabil für Crawls neuer Skills; `updated` ändert sich, wenn vorhandene Skills neu veröffentlicht werden.
- Wenn `nonSuspiciousOnly=true`, können cursorbasierte Sortierungen auf einer Seite weniger als `limit` Elemente zurückgeben, weil verdächtige Skills nach dem Seitenabruf herausgefiltert werden.
...
- Alte Slugs, die durch Umbenennungs-/Zusammenführungsflows von Besitzern erstellt wurden, werden zum kanonischen Skill aufgelöst.
...
- `moderation` wird nur einbezogen, wenn der Skill markiert ist oder der Besitzer ihn anzeigt.
...
Melden Sie einen Skill zur Prüfung durch Moderatoren. Meldungen erfolgen auf Skill-Ebene, sind optional
...
- Scan-Anfrage-Payloads und herunterladbare Berichte verfallen nach dem Aufbewahrungszeitraum aus dem Scan-Anfrage-Speicher.
...
Nur für Administratoren verfügbare kanonische Route für Batch-Rescans. Sie akzeptiert dieselbe Payload-Struktur wie die Legacy-Route `POST /api/v1/skills/-/rescan-batch`.

### `POST /api/v1/skills/-/scan/batch/status`

Nur für Administratoren verfügbare kanonische Route für den Batch-Status. Sie akzeptiert `{ "jobIds": ["..."] }` und gibt dieselben aggregierten Zähler wie die Legacy-Route `POST /api/v1/skills/-/rescan-batch/status` zurück.
...
- `provenance` ist nur dann `server-resolved-github-import`, wenn ClawHub während Veröffentlichung oder Import ein GitHub-Repo/einen Ref/einen Commit/einen Pfad aufgelöst und gespeichert hat; andernfalls ist es `unavailable`.
...
- `items` muss 1-100 eindeutige Paare aus `{ slug, version }` enthalten.
...
Nur-Plugin-Katalog zum Durchsuchen von Code-Plugin- und Bundle-Plugin-Paketen.
...
Veraltete v1-Filteraliase werden auf Lese-Endpunkten weiterhin akzeptiert:
...
`trending` ist eine sieben Tage umfassende Installations-/Download-Rangliste und verwendet keine Gesamtwerte über alle Zeiten.
...
- Body: ZIP-Archiv.
- Jedes exportierte Plugin hat das Stammverzeichnis `{family}/{packageName}/`.
...
- Die veralteten v1-Filteraliase, die unter `GET /api/v1/plugins` dokumentiert sind, werden ebenfalls
  akzeptiert.
- Die Kategoriefilterung ist ein echter API-Filter, der durch Digest-Zeilen der Plugin-Kategorie
...
Dies ist die öffentliche OpenClaw-Nutzungsoberfläche zur Entscheidung, ob ein
...
- `trust.stale` bedeutet, dass die Vertrauenszusammenfassung aus veralteten Eingaben berechnet wurde und
  vor einer hochzuverlässigen Zulassungsentscheidung als aktualisierungsbedürftig behandelt werden sollte.
...
- `packageName` wird als npm-Name normalisiert; das Paket kann bei geplanten
  Migrationen fehlen.
- Dies verfolgt nur die Migrationsbereitschaft. Es verändert OpenClaw nicht und erzeugt keine
  ClawPacks.
...
- `all`: jedes Release mit einer manuellen Überschreibung, nicht sauberem Scanstatus oder Paketbericht.
...
automatisch aus und blockieren sie nicht von selbst; Moderatoren sollten die Release-Moderation verwenden, um
...
- Erfordert ein API-Token für den Paketeigentümer, ein Herausgeber-Mitglied, einen Moderator oder
  Admin-Benutzer.
...
- Verwendet den Leseraten-Bucket, nicht den Download-Bucket.
- Binärdateien geben `415` zurück.
- Dateigrößenlimit: 200KB.
- Ausstehende VirusTotal-Scans blockieren Lesevorgänge nicht; bösartige Releases können andernorts weiterhin zurückgehalten werden.
...
- `dist.tarball`, `dist.integrity` und `dist.shasum` verwenden npm-kompatible
  Felder, damit Benutzer npm auf den Mirror richten können, wenn sie möchten.
- Packuments für Scoped Packages unterstützen sowohl `/api/npm/@scope/name` als auch npm's
...
aktuellen GitHub-gestützten Skill mit einem `clean`- oder `suspicious`-Scan und keiner gehosteten
Version zurück.
...
- Optionales Payload-Feld: `migrateOwner`. Wenn `true` mit `ownerHandle`, kann ein
  ...
  aktuellen als auch beim Ziel-Herausgeber ist. Ohne dieses Opt-in werden Eigentümeränderungen
  abgelehnt.
...
- Zulässige Formularfelder sind `payload`, wiederholte `files`-Blobs oder eine `clawpack`-
  ...
  Upload-URL-Flow zurückgegeben wurde. Gestaffelte Veröffentlichungen per Storage-ID müssen außerdem das
...
- Direkte Multipart-Veröffentlichungsanfragen sind auf 18MB begrenzt. ClawPack-Tarballs können
  den Upload-URL-Flow bis zum Tarball-Limit von 120MB verwenden.
...
- Nur der `openclaw`-Org-Herausgeber und persönliche Herausgeber aktueller Mitglieder der `openclaw`-Org
  dürfen im `official`-Kanal veröffentlichen.
- Veröffentlichungen im Auftrag anderer validieren die Eignung für den offiziellen Kanal weiterhin gegen das Ziel-Eigentümerkonto.

### `DELETE /api/v1/skills/{slug}` / `POST /api/v1/skills/{slug}/undelete`

Soft-löschen / Wiederherstellen eines Skills (Eigentümer, Moderator oder Admin).
...
Wenn vorhanden, wird `reason` als Skill-Moderationshinweis gespeichert und in das Audit-Protokoll kopiert.
...
Nur Admin. Stellt sicher, dass ein Org-Herausgeber für einen Handle existiert. Wenn der Handle noch auf einen
alten gemeinsamen Benutzer-/persönlichen Herausgeber zeigt, migriert der Endpunkt ihn zuerst in einen Org-Herausgeber.
...
Provider-Konto-IDs benennen; veränderliche Handles werden nur als bedienerorientierte Schutzprüfung verwendet.
...
Reservierung für den wiederhergestellten Handle wird ebenfalls dem Ersatzbenutzer neu zugewiesen, sodass spätere
...
Einen Benutzer sperren und eigene Skills hart löschen (nur Moderator/Admin).
...
Eine Benutzersperre aufheben und geeignete Skills wiederherstellen (nur Admin).

Body:

```json
{ "handle": "user_handle", "reason": "optionaler Entsperrgrund" }
```

oder

```json
{ "userId": "users_...", "reason": "optionaler Entsperrgrund" }
```
...
```json
{ "handle": "user_handle", "reason": "Massenveröffentlichungs-Spam", "dryRun": true }
```

oder

```json
{ "userId": "users_...", "reason": "Massenveröffentlichungs-Spam", "dryRun": false }
```
...
Die CLI kann Registry-/Authentifizierungseinstellungen von der Site ermitteln:
