...
x-i18n:
    generated_at: "2026-07-05T05:40:24Z"
    model: gpt-5.5
...
- Authentifizierte Anfragen (gültiger Bearer-Token): pro Benutzer-Bucket durchgesetzt.
...
- `X-RateLimit-Remaining` / `RateLimit-Remaining`: genaues verbleibendes Budget, falls vorhanden.
  Sharded erfolgreiche Anfragen lassen diesen Header weg, statt einen ungefähren globalen Wert zurückzugeben.
- `Retry-After`: Sekunden, die vor dem erneuten Versuch gewartet werden sollen (Verzögerung), bei `429`
...
- Ergebnisse werden nach Relevanzreihenfolge zurückgegeben (Embedding-Ähnlichkeit + exakte Slug-/Namen-Token-Boosts + eine kleine Popularitätsgewichtung).
- Relevanz ist stärker als Popularität. Eine präzise Slug- oder Anzeigenamen-Token-Übereinstimmung kann eine lockerere Übereinstimmung mit deutlich stärkerem Engagement übertreffen.
...
- Popularität ist logarithmisch skaliert und gedeckelt. Skills mit hohem Engagement können niedriger eingestuft werden, wenn der Query-Text schwächer übereinstimmt.
...
- Platzieren Sie die Begriffe, nach denen Benutzer tatsächlich suchen werden, im Anzeigenamen, in der Zusammenfassung und in den Tags. Verwenden Sie ein eigenständiges Slug-Token nur dann, wenn es auch eine stabile Identität ist, die Sie beibehalten möchten.
- Benennen Sie einen Slug nicht nur um, um einer einzelnen Query nachzujagen, es sei denn, der neue Slug ist ein besserer langfristiger kanonischer Name. Alte Slugs werden zu Weiterleitungsaliasen, aber die kanonische URL, der angezeigte Slug und zukünftige Such-Digests verwenden den neuen Slug.
- Umbenennungs-Aliase erhalten die Auflösung für alte URLs und Installationen, die über die Registry aufgelöst werden, aber das Such-Ranking basiert auf den kanonischen Skill-Metadaten, nachdem die Umbenennung indexiert wurde. Vorhandene Statistiken bleiben beim Skill.
...
- Belege werden für öffentliche Aufrufer redigiert und enthalten nur für Owner/Moderatoren Roh-Snippets.

### `POST /api/v1/skills/{slug}/report`

Einen Skill zur Moderatorprüfung melden. Meldungen gelten auf Skill-Ebene, können optional
...
Meldung, um den Skill im selben prüfbaren Workflow auszublenden.
...
- Scan-Anfrage-Payloads und herunterladbare Berichte verfallen nach dem Aufbewahrungsfenster aus dem Scan-Anfrage-Speicher.
...
Authentifizierter Endpunkt für Berichtarchive.
...
Authentifizierter Endpunkt für gespeicherte Berichtarchive übermittelter Versionen.
...
Admin-only kanonische Route für Batch-Rescans. Sie akzeptiert dieselbe Payload-Struktur wie das Legacy-`POST /api/v1/skills/-/rescan-batch`.

### `POST /api/v1/skills/-/scan/batch/status`

Admin-only kanonische Route für Batch-Status. Sie akzeptiert `{ "jobIds": ["..."] }` und gibt dieselben aggregierten Zähler wie das Legacy-`POST /api/v1/skills/-/rescan-batch/status` zurück.

### `GET /api/v1/skills/{slug}/verify`

Gibt den von `clawhub skill verify` verwendeten Verifizierungs-Envelope der Skill Card zurück.
...
- `ok` ist nur dann `true`, wenn die ausgewählte Version eine generierte Skill Card hat, durch Moderation nicht als Malware blockiert ist und die ClawScan-Verifizierung sauber ist.
- Skill-Identität, Publisher-Identität und Metadaten der ausgewählten Version sind Envelope-Felder auf oberster Ebene (`slug`, `displayName`, `publisherHandle`, `version`, `resolvedFrom`, `tag`, `createdAt`), damit Shell-Automatisierung sie lesen kann, ohne verschachtelte Wrapper zu entpacken.
- `security` ist das ClawScan-/Security-Urteil auf oberster Ebene. Automatisierung sollte sich an `ok`, `decision`, `reasons` und `security.status` orientieren.
- `security.signals` enthält unterstützende Scanner-Belege wie `staticScan`, `virusTotal` und `skillSpector`.
- `security.signals.dependencyRegistry` wird für die Kompatibilität von v1-Antworten beibehalten, aber der Existenz-Scanner für die Dependency Registry ist außer Betrieb genommen, und dieser Schlüssel ist immer `null`.
- `provenance` ist nur dann `server-resolved-github-import`, wenn ClawHub während der Veröffentlichung oder des Imports ein GitHub-Repo, einen Ref, einen Commit oder einen Pfad aufgelöst und gespeichert hat; andernfalls ist es `unavailable`.
...
- Die Antwort ist nur securitybezogen. Sie enthält keine Skill-Card-Daten, keinen Status generierter Cards, keine Artefakt-Dateilisten und keine detaillierten Scanner-Payloads.
- `security.signals` enthält nur unterstützende Belege auf Statusebene; verwenden Sie `/scan` oder die ClawHub-Seite security-audit für vollständige Scanner-Details.
- `security.signals.dependencyRegistry` wird für die Kompatibilität von v1-Antworten beibehalten, aber der Existenz-Scanner für die Dependency Registry ist außer Betrieb genommen, und dieser Schlüssel ist immer `null`.
- Das Fehlen einer Skill Card wirkt sich nicht auf `ok`, `decision` oder `reasons` dieses Endpunkts aus; Clients sollten das installierte `skill-card.md` lokal lesen, wenn sie Card-Inhalte benötigen.
- Verwenden Sie `/verify`, wenn Sie den Verifizierungs-Envelope der Skill Card für einen einzelnen Skill benötigen, `/card`, wenn Sie generiertes Card-Markdown benötigen, und `/scan`, wenn Sie detaillierte Scanner-Daten benötigen.
...
Gibt unverarbeiteten Textinhalt zurück.
...
- `category` (optional): Plugin-Kategoriefilter. Nur unterstützt, wenn der
  ...
  Legacy-v1-Filter-Aliase sind unter `GET /api/v1/plugins` dokumentiert.
...
- `isOfficial` (optional): `true` oder `false`
- `category` (optional): Plugin-Kategoriefilter. Nur unterstützt, wenn der
  Request auf Plugin-Pakete beschränkt ist. Kontrollierte Kategorien und Legacy-v1-
  Filter-Aliase sind unter `GET /api/v1/plugins` dokumentiert.
...
- Anonyme Aufrufer sehen nur öffentliche Paketkanäle.
- Authentifizierte Aufrufer können private Pakete für Herausgeber durchsuchen, denen sie angehören.
- `channel=private` gibt nur Pakete zurück, die der authentifizierte Aufrufer lesen kann.
...
Legacy-v1-Filter-Aliase werden auf Lese-Endpunkten weiterhin akzeptiert:
...
`trending` ist eine Installations-/Download-Bestenliste über sieben Tage und verwendet keine Gesamtzahlen über die gesamte Zeit.
Auf dem einheitlichen Endpunkt `/api/v1/packages` gilt es nur für Plugins; verwenden Sie
...
- Ergebnisse werden in Relevanzreihenfolge zurückgegeben und unterstützen derzeit keine Paginierung.
...
Gibt die exakte Sicherheits- und Vertrauenszusammenfassung des Paket-Releases für Installationsclients zurück.
Dies ist die öffentliche OpenClaw-Nutzungsoberfläche, um zu entscheiden, ob ein
...
- `trust.stale` bedeutet, dass die Vertrauenszusammenfassung aus veralteten Eingaben berechnet wurde und
  vor einer Allow-Entscheidung mit hoher Vertrauenswürdigkeit als auffrischungsbedürftig behandelt werden sollte.
...
- Dieser Endpunkt ist absichtlich enger gefasst als Moderationsendpunkte für Besitzer/Moderatoren.
  ...
  Identitäten von Meldenden, Meldungstexte, private Belege oder interne Review-
  Zeitpläne.
...
- Artefakt-Digest
- Source-Repo- und Commit-Provenienz
...
- `packageName` ist npm-name-normalisiert; das Paket kann für geplante
  Migrationen fehlen.
- Dies verfolgt nur die Migrationsbereitschaft. Es mutiert OpenClaw nicht und erzeugt
  keine ClawPacks.
...
- `all`: jedes Release mit einer manuellen Überschreibung, nicht sauberem Scan-Status oder Paketmeldung.
...
Meldet ein Paket zur Moderator-Review. Meldungen sind paketbezogen und optional
mit einer Version verknüpft. Sie fließen in die Moderationswarteschlange ein, blenden aber
Downloads nicht automatisch aus und blockieren sie nicht selbst; Moderatoren sollten Release-Moderation verwenden, um
...
`status` auf `open` kann es ausgelassen werden. Übergeben Sie `finalAction: "quarantine"` oder
...
- Private Pakete geben `404` zurück, es sei denn, der Aufrufer kann den zugehörigen Publisher lesen.
...
- `slug` (erforderlich)
- `hash` (erforderlich): 64-stelliger Hex-SHA256 des Bundle-Fingerabdrucks
...
Lädt ein gehostetes Skill-Version-ZIP herunter oder gibt eine GitHub-Quellübergabe für einen
...
- GitHub-basierte Skill-Übergaben proxyn oder spiegeln keine Bytes. Die JSON-Antwort
  ...
  und `archiveUrl`; Scan-/aktueller Zustand ist ein Gate und nicht als Erfolgs-
  Payload-Metadaten enthalten.
...
- Zulässige Formularfelder sind `payload`, wiederholte `files`-Blobs oder eine `clawpack`-
  ...
  Upload-URL-Flow zurückgegeben wurde. Veröffentlichungen mit gestagter Storage-ID müssen auch das
...
- Nur der `openclaw`-Org-Publisher und die persönlichen Publisher aktueller Mitglieder der `openclaw`-Org
  dürfen im `official`-Kanal veröffentlichen.
- Veröffentlichungen im Auftrag anderer validieren die Berechtigung für den official-Kanal weiterhin gegen das Ziel-Eigentümerkonto.
...
Vom Eigentümer initiierte Soft-Löschungen reservieren den Slug 30 Tage lang; danach kann der Slug von
...
älteren gemeinsam genutzten Benutzer-/persönlichen Publisher zeigt, migriert der Endpunkt es zuerst in einen Org-Publisher.
...
Nur Admins. Stellt einen persönlichen Publisher für einen verifizierten ersetzenden GitHub-OAuth-Principal wieder her,
...
Der Endpunkt verwendet standardmäßig einen Testlauf. Die Anwendung der Wiederherstellung erfordert `dryRun: false` und
...
Die Wiederherstellung migriert außerdem alte `ownerUserId`-Felder für die Skills des wiederhergestellten Publishers,
Skill-Slug-Aliasse, Pakete, Paketinspektor-Warnungen und abgeleitete Such-Digest-Zeilen, sodass
direkte Owner-Pfade mit der neuen Publisher-Autorität übereinstimmen. Eine aktive geschützte Handle-Reservierung
für das wiederhergestellte Handle wird ebenfalls dem Ersatzbenutzer zugewiesen, damit eine spätere
Profilsynchronisierung nicht die konkurrierende Autorität des früheren Benutzers wiederherstellen kann. Jede Primärtabelle ist auf
...
GitHub-Skill-Quellen sind Publisher-gebunden und werden als geprüft gemeldet, nicht umgeschrieben.

- Anfrage-Body: `{ "handle": "gingiris", "nextUserHandle": "gingiris-1031", "previousGitHubProviderAccountId": "123", "nextGitHubProviderAccountId": "456", "reason": "Verified account continuity for issue #2555", "confirmIdentityVerified": true, "dryRun": false }`
...
- `POST /api/v1/skills/{slug}/rename`
  - Anfrage-Body: `{ "newSlug": "new-canonical-slug" }`
  - Antwort: `{ "ok": true, "slug": "new-canonical-slug", "previousSlug": "old-slug" }`
- `POST /api/v1/skills/{slug}/merge`
  - Anfrage-Body: `{ "targetSlug": "canonical-target-slug" }`
...
- `POST /api/v1/skills/{slug}/transfer`
  - Anfrage-Body: `{ "toUserHandle": "target_handle", "message": "optional" }`
...
Inhalte wiederherzustellen (nur Admin). Verwendet standardmäßig einen Testlauf, sofern `dryRun` nicht `false` ist.
...
Wenn Sie selbst hosten, stellen Sie diese Datei bereit (oder setzen Sie `CLAWHUB_REGISTRY` explizit; alt: `CLAWDHUB_REGISTRY`).
