...
x-i18n:
    generated_at: "2026-07-03T00:54:15Z"
    model: gpt-5.5
...
Drittanbieter-Verzeichnisse können die öffentlichen Lese-Endpunkte verwenden, um ClawHub-Skills aufzulisten oder zu durchsuchen. Cachen Sie Ergebnisse, beachten Sie `429`/`Retry-After`, verlinken Sie Benutzer zurück auf den kanonischen ClawHub-Eintrag (`https://clawhub.ai/<owner>/skills/<slug>`), und vermeiden Sie den Eindruck, ClawHub unterstütze die Drittanbieter-Site. Versuchen Sie nicht, ausgeblendete, private oder durch Moderation blockierte Inhalte außerhalb der öffentlichen API-Oberfläche zu spiegeln.
...
- Wenn `Retry-After` vorhanden ist, warten Sie diese Anzahl von Sekunden vor einem erneuten Versuch.
...
- Relevanz ist stärker als Popularität. Eine präzise Übereinstimmung mit einem Slug- oder Anzeigenamen-Token kann eine weniger genaue Übereinstimmung mit deutlich stärkerem Engagement übertreffen.
- ASCII-Text wird an Wort- und Zeichensetzungsgrenzen tokenisiert. Beispielsweise enthält `personal-map` ein eigenständiges `map`-Token, während `amap-jsapi-skill` `amap`, `jsapi` und `skill` enthält; die Suche nach `map` gibt `personal-map` daher eine stärkere lexikalische Übereinstimmung als `amap-jsapi-skill`.
- Popularität ist logarithmisch skaliert und begrenzt. Skills mit hohem Engagement können niedriger eingestuft werden, wenn der Suchtext schwächer übereinstimmt.
...
- Enthält einen normalisierten Verifizierungsstatus sowie scannerspezifische Details.
...
- Veröffentlichte Scans erfordern Verwaltungszugriff als Owner/Publisher oder Plattformmoderator-/Administratorberechtigung.
...
- Erfordert Verwaltungszugriff als Owner/Publisher auf den Skill oder das Plugin oder Plattformmoderator-/Administratorberechtigung.
...
- `security.signals.dependencyRegistry` wird für die Kompatibilität von v1-Antworten beibehalten, aber der Scanner für die Existenz der Dependency Registry ist eingestellt, und dieser Schlüssel ist immer `null`.
- `provenance` ist nur dann `server-resolved-github-import`, wenn ClawHub während Publish oder Import ein GitHub-Repo/eine Ref/einen Commit/einen Pfad aufgelöst und gespeichert hat; andernfalls ist es `unavailable`.
...
- Die Antwort ist ausschließlich sicherheitsbezogen. Sie enthält keine Skill-Card-Daten, keinen Status generierter Karten, keine Artefaktdateilisten und keine detaillierten Scanner-Payloads.
- `security.signals` enthält nur unterstützende Evidenz auf Statusebene; verwenden Sie `/scan` oder die ClawHub-Sicherheitsaudit-Seite für vollständige Scanner-Details.
- `security.signals.dependencyRegistry` wird für die Kompatibilität von v1-Antworten beibehalten, aber der Scanner für die Existenz der Dependency Registry ist eingestellt, und dieser Schlüssel ist immer `null`.
...
- Authentifizierte Aufrufer können private Pakete von Publishern, denen sie angehören, in Listen-/Suchergebnissen sehen.
...
`trending` ist eine Installations-/Download-Bestenliste über sieben Tage und verwendet keine Gesamtwerte über alle Zeiträume.
...
- Sortiersteuerungen der Browseroberfläche für die Plugin-Suche ordnen die geladenen Relevanzergebnisse neu
  und entsprechen damit dem aktuellen Durchsuchverhalten von `/skills`.
...
zurück. Dies ist die öffentliche OpenClaw-Nutzungsfläche zur Entscheidung, ob ein
aufgelöstes Release installiert werden kann.

Authentifizierung:

- Öffentlicher Leseendpunkt. Kein Besitzer-, Publisher-, Moderator- oder Admin-Token ist
  erforderlich.
...
- `trust.reasons` ist die benutzerorientierte und prüfbare Erklärungsliste. Ursachencodes
...
- Dies ist die OpenClaw-Resolver-Fläche; sie vermeidet, das Archivformat aus
...
- `open`: verdächtige, schädliche, ausstehende, quarantined, revoked oder gemeldete Releases.
- `blocked`: quarantined, revoked oder schädliche Releases.
- `manual`: jedes Release mit einer manuellen Moderationsüberschreibung.
- `all`: jedes Release mit einer manuellen Überschreibung, nicht sauberem Scanstatus oder Paketmeldung.
...
`finalAction: "revoke"` mit einer bestätigten Meldung, um die Release-Moderation im selben
auditierbaren Workflow anzuwenden.
...
- Ein JSON-Body mit `files` (storageId-basiert) wird ebenfalls akzeptiert.
...
Vom Eigentümer initiierte Soft-Deletes reservieren den Slug 30 Tage lang, danach kann der Slug von
...
älteren geteilten Benutzer-/persönlichen Publisher zeigt, migriert der Endpunkt ihn zuerst in einen Org-Publisher.
...
Nur für Admins. Stellt einen persönlichen Publisher für einen verifizierten Ersatz-GitHub-OAuth-Prinzipal wieder her,
...
Der Endpunkt ist standardmäßig ein Trockenlauf. Das Anwenden der Wiederherstellung erfordert `dryRun: false` und
`confirmIdentityVerified: true`, nachdem Mitarbeitende die Kontinuität zwischen beiden
GitHub-Prinzipalen unabhängig verifiziert haben. Die Wiederherstellung schlägt im Fail-Closed-Modus fehl, wenn der aktuelle persönliche
...
Skill-Slug-Aliasse, Pakete, Paket-Inspector-Warnungen und abgeleitete Search-Digest-Zeilen, damit
...
Sperren Sie einen Benutzer und löschen Sie seine eigenen Skills dauerhaft (nur Moderator/Admin).
...
Ändern Sie den gespeicherten Grund für eine bestehende Sperre, ohne die Sperre aufzuheben oder
Inhalte wiederherzustellen (nur Admin). Standardmäßig ein Trockenlauf, sofern `dryRun` nicht `false` ist.
