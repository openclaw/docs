...
Drittanbieter-Verzeichnisse können die öffentlichen Lese-Endpunkte verwenden, um ClawHub Skills aufzulisten oder zu durchsuchen. Bitte cachen Sie Ergebnisse, beachten Sie `429`/`Retry-After`, verlinken Sie Benutzer zurück zum kanonischen ClawHub-Eintrag (`https://clawhub.ai/<owner>/<slug>`) und vermeiden Sie den Eindruck, ClawHub würde die Drittanbieter-Website unterstützen. Versuchen Sie nicht, versteckte, private oder durch Moderation blockierte Inhalte außerhalb der öffentlichen API-Oberfläche zu spiegeln.

Web-Slug-Kurzformen werden über Registry-Familien hinweg aufgelöst, aber API-Clients sollten die kanonischen URLs verwenden, die von Lese-Endpunkten zurückgegeben werden, anstatt die Routenpriorität zu rekonstruieren.
...
- Ergebnisse werden in Relevanzreihenfolge zurückgegeben (Embedding-Ähnlichkeit + Boosts für exakte Slug-/Namens-Token + Popularitätsprior aus Downloads).
- Relevanz ist stärker als Popularität. Ein präziser Slug- oder Anzeigenamen-Token-Treffer kann einen ungenaueren Treffer mit deutlich mehr Downloads übertreffen.
- ASCII-Text wird an Wort- und Satzzeichengrenzen tokenisiert. Zum Beispiel enthält `personal-map` ein eigenständiges `map`-Token, während `amap-jsapi-skill` `amap`, `jsapi` und `skill` enthält; eine Suche nach `map` gibt `personal-map` daher einen stärkeren lexikalischen Treffer als `amap-jsapi-skill`.
...
- Nehmen Sie die Begriffe, nach denen Benutzer buchstäblich suchen werden, in Anzeigename, Zusammenfassung und Tags auf. Verwenden Sie ein eigenständiges Slug-Token nur, wenn es auch eine stabile Identität ist, die Sie beibehalten möchten.
- Benennen Sie einen Slug nicht nur um, um eine einzelne Abfrage zu bedienen, es sei denn, der neue Slug ist ein besserer langfristiger kanonischer Name. Alte Slugs werden zu Weiterleitungsaliasen, aber die kanonische URL, der angezeigte Slug und zukünftige Such-Digests verwenden den neuen Slug.
- Umbenennungsaliase erhalten die Auflösung für alte URLs und Installationen, die über die Registry aufgelöst werden, aber das Suchranking basiert nach der Indexierung der Umbenennung auf den kanonischen Skill-Metadaten. Vorhandene Statistiken bleiben beim Skill.
...
- Alte Slugs, die durch Owner-Umbenennungs-/Zusammenführungsflüsse erstellt wurden, werden zum kanonischen Skill aufgelöst.
...
- Beweise werden für öffentliche Aufrufer redigiert und enthalten Roh-Snippets nur für Owner/Moderatoren.

### `POST /api/v1/skills/{slug}/report`

Melden Sie einen Skill zur Überprüfung durch Moderatoren. Meldungen gelten auf Skill-Ebene, können optional mit einer Version verknüpft werden und speisen die Skill-Meldewarteschlange.
...
`note` ist für `confirmed` und `dismissed` erforderlich; es kann weggelassen werden, wenn `status` wieder auf `open` gesetzt wird. Übergeben Sie `finalAction: "hide"` mit einer triagierten Meldung, um den Skill im selben prüfbaren Workflow auszublenden.
...
- `npmMirror` (optional): `true`/`1`, um von ClawPack gestützte Paketversionen
...
- `artifactKind` (optional): `legacy-zip` oder `npm-pack`
- `npmMirror` (optional): `true`/`1`, um nach von ClawPack gestützten Paketversionen
...
- Erfordert ein API-Token für den Paketeigentümer, einen Eigentümer/Administrator des Organisations-Publishers,
...
- `version.artifact.kind` ist `legacy-zip` für Paketarchive der alten Welt oder
  `npm-pack` für von ClawPack gestützte Releases.
...
Gibt die exakte Sicherheits- und Vertrauenszusammenfassung des Paket-Releases für Installationsclients zurück. Dies ist die öffentliche OpenClaw-Konsumoberfläche zur Entscheidung, ob ein
aufgelöstes Release installiert werden kann.

Authentifizierung:

- Öffentlicher Lese-Endpunkt. Es ist kein Eigentümer-, Publisher-, Moderator- oder Administrator-Token
  erforderlich.
...
- Dieser Endpunkt ist absichtlich enger gefasst als Moderationsendpunkte für Eigentümer/Moderatoren.
  ...
  Reporteridentitäten, Berichtsinhalte, private Beweise oder interne Prüfzeitpläne.
...
Administrator-Endpunkt zum Erstellen oder Aktualisieren einer offiziellen Plugin-Migrationszeile.

Authentifizierung:

- Erfordert ein API-Token für einen Administratorbenutzer.
...
Moderator-/Administrator-Endpunkt für Paket-Release-Prüfwarteschlangen.
...
Paket zur Moderatorprüfung melden. Meldungen erfolgen auf Paketebene und sind optional
mit einer Version verknüpft. Sie fließen in die Moderationswarteschlange ein, blenden
Downloads aber nicht automatisch aus und blockieren sie auch nicht selbst; Moderatoren sollten Release-Moderation verwenden, um Artefakte
freizugeben, zu quarantänisieren oder zu widerrufen.
...
Owner-/Moderator-Endpunkt für die Moderationssichtbarkeit eines Pakets.
...
Quarantänisierte und widerrufene Releases geben bei Artefakt-Download-Routen `403` zurück.
Jede Änderung schreibt einen Audit-Log-Eintrag.

### `POST /api/v1/packages/backfill/artifacts`

Nur für Admins verfügbarer Wartungsendpunkt, um ältere Paket-Releases mit
expliziten Artefaktart-Metadaten zu kennzeichnen.
...
- Verwendet das Lesedurchsatz-Bucket, nicht das Download-Bucket.
...
- Packuments für Pakete mit Scope unterstützen sowohl `/api/npm/@scope/name` als auch npms
...
- Verwendet das Download-Durchsatz-Bucket.
...
- Veröffentlichungen im Auftrag anderer validieren die Berechtigung für den official-Kanal weiterhin gegen das Zieleigentümerkonto.
...
- `merge` blendet den Quell-Listing aus und leitet den Quell-Slug zum Ziel-Listing um.
...
Die Rolle eines Benutzers ändern (nur Admin).
...
Einen Star (Hervorhebungen) hinzufügen/entfernen. Beide Endpunkte sind idempotent.
