...
x-i18n:
    generated_at: "2026-06-28T07:41:21Z"
    model: gpt-5.5
...
Drittanbieter-Verzeichnisse dürfen die öffentlichen Lese-Endpunkte verwenden, um ClawHub Skills aufzulisten oder zu durchsuchen. Cachen Sie Ergebnisse, beachten Sie `429`/`Retry-After`, verlinken Sie Benutzer zurück auf den kanonischen ClawHub-Eintrag (`https://clawhub.ai/<owner>/skills/<slug>`), und vermeiden Sie den Eindruck, ClawHub unterstütze die Drittanbieter-Website. Versuchen Sie nicht, verborgene, private oder durch Moderation blockierte Inhalte außerhalb der öffentlichen API-Oberfläche zu spiegeln.

Web-Slug-Kurzformen werden über Registry-Familien hinweg aufgelöst, aber API-Clients sollten die von Lese-Endpunkten zurückgegebenen kanonischen URLs verwenden, anstatt die Routenpriorität zu rekonstruieren.
...
- Wenn der Token fehlt oder ungültig ist, fällt das Verhalten auf IP-Durchsetzung zurück.
...
- `X-RateLimit-Remaining` / `RateLimit-Remaining`: exaktes verbleibendes Budget, falls vorhanden.
  Erfolgreiche sharded Anfragen lassen diesen Header weg, anstatt einen ungefähren globalen Wert zurückzugeben.
...
- Wenn `Retry-After` fehlt, fallen Sie auf `RateLimit-Reset` zurück (oder berechnen Sie ihn aus `X-RateLimit-Reset`).
...
- Wenn keine vertrauenswürdige Client-IP verfügbar ist, verwenden anonyme Anfragen Fallback-Buckets,
  die nur nach Ratenlimit-Art abgegrenzt sind. Diese Fallback-Buckets enthalten keine
  vom Aufrufer gelieferten Pfade, Slugs, Paketnamen, Versionen, Query-Strings oder andere
  Artefaktparameter.
...
sollten den Antwortkörper als für Menschen lesbare Zeichenfolge lesen. Unbekannte Query-Parameter werden
...
- Popularität ist logarithmisch skaliert und gedeckelt. Skills mit hohem Engagement können niedriger rangieren, wenn der Query-Text eine schwächere Übereinstimmung ist.
- Verdächtiger oder verborgener Moderationsstatus kann einen Skill je nach Aufruferfiltern und aktuellem Moderationsstatus aus der öffentlichen Suche entfernen.

Leitlinien zur Auffindbarkeit für Herausgeber:
...
- Benennen Sie einen Slug nicht nur um, um eine einzelne Query abzufangen, es sei denn, der neue Slug ist langfristig ein besserer kanonischer Name. Alte Slugs werden zu Weiterleitungsaliasen, aber die kanonische URL, der angezeigte Slug und zukünftige Such-Digests verwenden den neuen Slug.
...
- Alte Slugs, die durch Owner-Umbenennungs-/Zusammenführungsflüsse erstellt wurden, werden auf den kanonischen Skill aufgelöst.
...
Meldet einen Skill zur Prüfung durch Moderatoren. Meldungen gelten auf Skill-Ebene, sind optional
...
Gibt Versionsmetadaten und Dateiliste zurück.
...
- Scan-Anfrage-Payloads und herunterladbare Berichte laufen nach dem Aufbewahrungsfenster aus dem Scan-Anfrage-Store ab.
...
- Scan-Jobs sind asynchron. Manuelle Scan-Anfragen werden vor normalen Publish-/Backfill-Arbeiten priorisiert, der Abschluss hängt jedoch weiterhin von der Verfügbarkeit der Worker ab.
...
- Gibt `queue.queuedAhead` und `queue.position` zurück, solange der Scan in der Warteschlange ist, damit Clients anzeigen können, wie viele priorisierte manuelle Scans vor der Anfrage liegen. Sehr große Warteschlangen werden begrenzt und mit `queuedAheadIsEstimate: true` gemeldet.
...
Authentifizierter Endpunkt für gespeicherte Berichtsarchive zu eingereichten Versionen.
...
Nur für Admins verfügbare kanonische Batch-Status-Route. Sie akzeptiert `{ "jobIds": ["..."] }` und gibt dieselben aggregierten Zähler zurück wie das Legacy-`POST /api/v1/skills/-/rescan-batch/status`.
...
- Skill-Identität, Publisher-Identität und Metadaten der ausgewählten Version sind Felder auf oberster Umschlagebene (`slug`, `displayName`, `publisherHandle`, `version`, `resolvedFrom`, `tag`, `createdAt`), damit Shell-Automation sie lesen kann, ohne verschachtelte Wrapper zu entpacken.
- `security` ist das ClawScan-/Sicherheitsurteil auf oberster Ebene. Automation sollte sich an `ok`, `decision`, `reasons` und `security.status` orientieren.
- `security.signals` enthält unterstützende Scanner-Nachweise wie `staticScan`, `virusTotal` und `skillSpector`.
- `security.signals.dependencyRegistry` bleibt für die Kompatibilität der v1-Antwort erhalten, aber der Scanner für die Existenz im Dependency-Registry ist außer Betrieb genommen, und dieser Schlüssel ist immer `null`.
...
- Die Antwort enthält nur Sicherheitsdaten. Sie enthält keine Skill-Card-Daten, keinen Status der generierten Karte, keine Artefakt-Dateilisten und keine detaillierten Scanner-Payloads.
...
- `security.signals.dependencyRegistry` bleibt für die Kompatibilität der v1-Antwort erhalten, aber der Scanner für die Existenz im Dependency-Registry ist außer Betrieb genommen, und dieser Schlüssel ist immer `null`.
- Das Fehlen einer Skill Card wirkt sich nicht auf `ok`, `decision` oder `reasons` dieses Endpunkts aus; Clients sollten die installierte `skill-card.md` lokal lesen, wenn sie Karteninhalt benötigen.
...
- Authentifizierte Aufrufer können private Pakete für Herausgeber sehen, denen sie angehören, in Listen-/Suchergebnissen.
...
Plugin-spezifisches Katalog-Browsing über Code-Plugin- und Bundle-Plugin-Pakete hinweg.
...
Legacy-v1-Filteraliase bleiben auf Lese-Endpunkten akzeptiert:
...
`trending` ist eine Installations-/Download-Bestenliste über sieben Tage und verwendet keine Gesamtzahlen über alle Zeiträume.
Auf dem einheitlichen Endpunkt `/api/v1/packages` ist dies Plugin-spezifisch; verwenden Sie
...
Massenexport neuester öffentlicher Skills für Offline-Analysen.
...
- `_errors.json` ist enthalten, wenn einzelne Skills oder Dateien nicht exportiert werden
  konnten.
...
- Sortiersteuerelemente der Browser-Oberfläche für die Plugin-Suche sortieren die geladenen Relevanzergebnisse neu,
...
Clients zurück. Dies ist die öffentliche OpenClaw-Konsumoberfläche für die Entscheidung, ob ein
aufgelöstes Release installiert werden kann.

Authentifizierung:

- Öffentlicher Leseendpunkt. Kein Token für Besitzer, Publisher, Moderator oder Admin
  erforderlich.
...
Meldet ein Paket zur Prüfung durch Moderatoren. Meldungen erfolgen auf Paketebene und können optional
mit einer Version verknüpft sein. Sie speisen die Moderationswarteschlange, blenden Downloads aber nicht
automatisch aus und blockieren sie nicht selbst; Moderatoren sollten Release-Moderation verwenden, um
...
Owner-/Moderator-Endpunkt für Sichtbarkeit der Paketmoderation.
...
Lädt eine gehostete Skill-Version als ZIP herunter oder gibt eine GitHub-Quellübergabe für einen
...
- Übergaben für GitHub-gestützte Skills proxyen oder spiegeln keine Bytes. Die JSON-Antwort
  enthält `sourceRef: "public-github"`, `repo`, `commit`, `path`, `contentHash`,
  und `archiveUrl`; Scan-/aktueller Zustand ist ein Gate und wird nicht als Metadaten der
  Erfolgspayload einbezogen.
...
- Optionales Payload-Feld: `migrateOwner`. Wenn `true` mit `ownerHandle`, kann ein
  vorhandener Skill zu diesem Owner wechseln, wenn der Akteur Admin/Owner sowohl beim
...
- JSON-Bodies und vom Aufrufer bereitgestellte Metadaten in `payload.files` / `payload.artifact`
  werden abgelehnt.
- Direkte Multipart-Veröffentlichungsanfragen sind auf 18 MB begrenzt. ClawPack-Tarballs können
  den Upload-URL-Flow bis zur Tarball-Grenze von 120 MB verwenden.
...
Vom Owner initiierte vorläufige Löschungen reservieren den Slug für 30 Tage; danach kann der Slug von
...
alten gemeinsam genutzten Benutzer-/persönlichen Publisher verweist, migriert der Endpunkt ihn zuerst in einen Org-Publisher.
...
Nur für Admins. Stellt einen persönlichen Publisher für einen verifizierten ersetzenden GitHub-OAuth-Principal wieder her,
ohne Convex-Auth-Kontozeilen zu bearbeiten. Die Anfrage muss beide unveränderlichen GitHub-
Provider-Konto-IDs benennen; veränderliche Handles werden nur als bedienerseitige Absicherung verwendet.
...
GitHub-Principals unabhängig überprüft haben. Die Wiederherstellung schlägt geschlossen fehl, wenn der aktuelle persönliche
...
Skill-Slug-Aliase, Pakete, Paket-Inspector-Warnungen und abgeleitete Such-Digest-Zeilen, damit
Direkteigentümerpfade mit der neuen Publisher-Autorität übereinstimmen. Eine aktive geschützte Handle-Reservierung
...
- Body: `{ "handle": "gingiris", "nextUserHandle": "gingiris-1031", "previousGitHubProviderAccountId": "123", "nextGitHubProviderAccountId": "456", "reason": "Überprüfte Kontokontinuität für Issue #2555", "confirmIdentityVerified": true, "dryRun": false }`
- Response: `{ "ok": true, "dryRun": false, "recovered": true, "publisherId": "...", "handle": "gingiris", "previousUser": { "userId": "...", "handle": "gingiris", "nextHandle": "gingiris-recovered", "githubProviderAccountId": "123", "authAccountCount": 1 }, "nextUser": { "userId": "...", "handle": "gingiris-1031", "nextHandle": "gingiris", "githubProviderAccountId": "456", "authAccountCount": 1 }, "retiredPersonalPublisher": null, "resourceOwnerMigration": { "limitPerTable": 100, "skills": 1, "skillSlugAliases": 1, "packages": 0, "packageInspectorWarnings": 0, "githubSourcesChecked": 1, "handleReservations": 1 }, "identityVerified": true, "reason": "Überprüfte Kontokontinuität für Issue #2555" }`
...
Sperrt einen Benutzer und löscht die eigenen Skills endgültig (nur Moderator/Admin).
...
```json
{
  ...
  "userId": "users_...",
  "handle": "user_handle",
  "previousReason": "automatische Malware-Sperre",
  "nextReason": "Massenveröffentlichungs-Spam",
  "changed": true
}
```
