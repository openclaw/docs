...
x-i18n:
    generated_at: "2026-07-01T15:21:39Z"
    model: gpt-5.5
...
- `Retry-After`: Sekunden Wartezeit vor dem erneuten Versuch (Verzögerung) bei `429`
...
- Wenn keine vertrauenswürdige Client-IP verfügbar ist, verwenden anonyme Anfragen Fallback-Buckets,
  die nur nach Art der Ratenbegrenzung begrenzt sind. Diese Fallback-Buckets enthalten keine
...
sollten den Antworttext als menschenlesbare Zeichenfolge lesen. Unbekannte Abfrageparameter werden
...
- Ergebnisse werden nach Relevanzreihenfolge zurückgegeben (Embedding-Ähnlichkeit + exakte Slug-/Namens-Token-Boosts + ein kleiner Popularitäts-Prior).
- Relevanz ist stärker als Popularität. Eine präzise Slug- oder Anzeigename-Token-Übereinstimmung kann eine ungenauere Übereinstimmung mit deutlich stärkerer Interaktion übertreffen.
...
- Popularität ist logarithmisch skaliert und gedeckelt. Skills mit hoher Interaktion können niedriger eingestuft werden, wenn der Abfragetext eine schwächere Übereinstimmung ist.
...
- Verwenden Sie die Begriffe, nach denen Benutzer wörtlich suchen werden, im Anzeigenamen, in der Zusammenfassung und in Tags. Verwenden Sie ein eigenständiges Slug-Token nur, wenn es auch eine stabile Identität ist, die Sie beibehalten möchten.
- Benennen Sie einen Slug nicht nur um, um eine einzelne Abfrage zu bedienen, es sei denn, der neue Slug ist ein besserer langfristiger kanonischer Name. Alte Slugs werden zu Weiterleitungs-Aliassen, aber die kanonische URL, der angezeigte Slug und zukünftige Such-Digests verwenden den neuen Slug.
- Umbenennungs-Aliasse bewahren die Auflösung für alte URLs und Installationen, die über die Registry auflösen, aber das Suchranking basiert auf den kanonischen Skill-Metadaten, nachdem die Umbenennung indexiert wurde. Bestehende Statistiken bleiben beim Skill.
...
- `sort` (optional): `updated` (Standard), `recommended` (Alias: `default`), `createdAt` (Alias: `newest`), `downloads`, `stars` (Alias: `rating`), veraltete Installations-Aliasse `installsCurrent`/`installs`/`installsAllTime` werden auf `downloads` abgebildet, `trending`
- `nonSuspiciousOnly` (optional): `true`, um verdächtige (`flagged.suspicious`) Skills auszublenden
- `nonSuspicious` (optional): veralteter Alias für `nonSuspiciousOnly`
...
- Alte Slugs, die durch Besitzerumbenennungs-/Zusammenführungsabläufe erstellt wurden, lösen auf den kanonischen Skill auf.
...
- `moderation` ist nur enthalten, wenn der Skill markiert ist oder der Besitzer ihn ansieht.
...
- Besitzer und Moderatoren können auf Moderationsdetails für versteckte Skills zugreifen.
- Öffentliche Aufrufer erhalten nur für bereits markierte sichtbare Skills `200`.
- Nachweise werden für öffentliche Aufrufer redigiert und enthalten Roh-Snippets nur für Besitzer/Moderatoren.

### `POST /api/v1/skills/{slug}/report`

Meldet einen Skill zur Moderatorprüfung. Meldungen gelten auf Skill-Ebene, sind optional
...
Gibt Verifizierungsdetails des Sicherheitsscans für eine Skill-Version zurück.
...
- Scan-Request-Payloads und herunterladbare Berichte laufen nach dem Aufbewahrungsfenster aus dem Scan-Request-Speicher ab.
...
- `security` ist das Top-Level-ClawScan-/Sicherheitsurteil. Automatisierung sollte sich an `ok`, `decision`, `reasons` und `security.status` orientieren.
- `security.signals` enthält unterstützende Scanner-Nachweise wie `staticScan`, `virusTotal` und `skillSpector`.
- `security.signals.dependencyRegistry` bleibt für v1-Antwortkompatibilität erhalten, aber der Scanner zur Existenz der Dependency Registry ist außer Betrieb genommen, und dieser Schlüssel ist immer `null`.
- `provenance` ist nur dann `server-resolved-github-import`, wenn ClawHub während Publish oder Import ein GitHub-Repo/einen Ref/einen Commit/einen Pfad aufgelöst und gespeichert hat; andernfalls ist es `unavailable`.
...
ClawHub-Skill-Versionen sie anzeigen müssen, zum Beispiel die OpenClaw Control UI.
...
- `security.signals` enthält nur unterstützende Nachweise auf Statusebene; verwenden Sie `/scan` oder die ClawHub-Seite für Security Audits für vollständige Scanner-Details.
- `security.signals.dependencyRegistry` bleibt für v1-Antwortkompatibilität erhalten, aber der Scanner zur Existenz der Dependency Registry ist außer Betrieb genommen, und dieser Schlüssel ist immer `null`.
...
Nur-Plugin-Katalogansicht über Code-Plugin- und Bundle-Plugin-Pakete hinweg.
...
Legacy-v1-Filteraliase bleiben für Lese-Endpunkte akzeptiert:
...
`trending` ist eine sieben-Tage-Rangliste für Installationen/Downloads und verwendet keine Gesamtwerte über die gesamte Laufzeit.
...
- Body: ZIP-Archiv.
- Jeder exportierte Plugin hat seinen Stamm unter `{family}/{packageName}/`.
- Jeder exportierte Plugin enthält die gespeicherten Dateien des neuesten Releases.
...
- Die unter `GET /api/v1/plugins` dokumentierten alten v1-Filteraliase werden ebenfalls
  akzeptiert.
...
Clients zurück. Dies ist die öffentliche OpenClaw-Nutzungsfläche für die Entscheidung, ob ein
...
- `trust.scanStatus` ist der wirksame Vertrauensstatus, der aus Scanner-Eingaben
...
- Dieser Endpunkt ist versionsgenau. Clients sollten ihn aufrufen, nachdem sie die
  ...
  Paketmetadaten.
- Private Pakete geben `404` zurück, sofern der Aufrufer den besitzenden Publisher lesen kann.
- Dieser Endpunkt ist absichtlich schmaler als Moderationsendpunkte für Besitzer/Moderatoren.
  Er legt die Installationsentscheidung und öffentliche Erklärung offen, nicht
  Reporteridentitäten, Berichtsinhalte, private Belege oder interne Review-
  Zeitachsen.
...
- Dies ist die OpenClaw-Resolver-Fläche; sie vermeidet das Erraten des Archivformats aus
  einer gemeinsam genutzten URL.
...
- Alte ZIP-Versionen leiten zu `/api/v1/packages/{name}/download?version=` um.
...
- `open`: verdächtige, schädliche, ausstehende, quarantänegesetzte, widerrufene oder gemeldete Releases.
- `blocked`: quarantänegesetzte, widerrufene oder schädliche Releases.
- `manual`: jedes Release mit einer manuellen Moderationsüberschreibung.
- `all`: jedes Release mit einer manuellen Überschreibung, einem nicht sauberen Scanstatus oder Paketbericht.
...
Meldet ein Paket zur Prüfung durch Moderatoren. Berichte gelten auf Paketebene und sind optional
mit einer Version verknüpft. Sie speisen die Moderationswarteschlange, blenden Downloads jedoch nicht automatisch aus und
blockieren sie nicht selbst; Moderatoren sollten Release-Moderation verwenden, um
Artefakte zu genehmigen, in Quarantäne zu setzen oder zu widerrufen.
...
Endpunkt für Paketinhaber/Moderatoren zur Sichtbarkeit der Paketmoderation.
...
Moderator-/Administrator-Endpunkt zum Abschließen oder Wiederöffnen von Paketmeldungen.
...
- `quarantined`: bis zur Nachbearbeitung gesperrt.
- `revoked`: gesperrt, nachdem ein Release zuvor als vertrauenswürdig galt.
...
Gibt den rohen Textinhalt für eine Paketdatei zurück.
...
- Private Pakete geben `404` zurück, sofern der Aufrufer nicht der Inhaber ist.
...
- Optionales Payload-Feld: `migrateOwner`. Bei `true` mit `ownerHandle` kann ein
  ...
  aktuellen als auch beim Ziel-Publisher ist. Ohne dieses Opt-in werden Inhaberwechsel
  abgelehnt.
...
- Zulässige Formularfelder sind `payload`, wiederholte `files`-Blobs oder eine `clawpack`-
  ...
  Upload-URL-Flow zurückgegeben wurde. Veröffentlichungen mit bereitgestellter Storage-ID müssen auch das
...
- JSON-Bodys und vom Aufrufer gelieferte Metadaten `payload.files` / `payload.artifact`
  werden abgelehnt.
...
- Veröffentlichungen im Auftrag anderer validieren die Berechtigung für den official-Channel weiterhin gegen das Zielinhaberkonto.
...
Vom Inhaber ausgelöste Soft Deletes reservieren den Slug für 30 Tage; danach kann der Slug von
einem anderen Publisher beansprucht werden. Die Löschantwort enthält `slugReservedUntil`, wenn dieser Ablauf gilt.
Ausblendungen durch Moderatoren/Administratoren und Sicherheitsentfernungen laufen nicht auf diese Weise ab.
...
Geben Sie für eine neu erstellte Org `memberHandle` an; der ausführende Administrator wird nicht als Mitglied hinzugefügt.
...
Nur Administratoren. Stellt einen persönlichen Publisher für einen verifizierten Ersatz-GitHub-OAuth-Principal
wieder her, ohne Convex Auth-Kontozeilen zu bearbeiten. Die Anfrage muss beide unveränderlichen GitHub-
Provider-Konto-IDs benennen; veränderliche Handles werden nur als bedienerseitige Schutzprüfung verwendet.

Der Endpunkt verwendet standardmäßig einen Dry-Run. Das Anwenden der Wiederherstellung erfordert `dryRun: false` und
...
Skill-Slug-Aliase, Pakete, Paket-Inspector-Warnungen und abgeleitete Such-Digest-Zeilen des wiederhergestellten Publishers, sodass
...
- `rename` erhält den vorherigen Slug als Weiterleitungsalias.
...
Entsperrt einen Benutzer und stellt zulässige Skills wieder her (nur Admin).
...
Ändert den gespeicherten Grund für eine bestehende Sperre, ohne Inhalte zu entsperren oder wiederherzustellen
(nur Admin). Verwendet standardmäßig einen Dry-Run, sofern `dryRun` nicht `false` ist.
...
Fügt einen Star hinzu oder entfernt ihn (Hervorhebungen). Beide Endpunkte sind idempotent.
