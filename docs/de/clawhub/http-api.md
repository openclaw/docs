...
- Wenn keine vertrauenswürdige Client-IP verfügbar ist, verwenden anonyme Anfragen Fallback-Buckets,
  die nur nach Art des Ratenlimits scoped sind. Diese Fallback-Buckets enthalten keine
  vom Aufrufer gelieferten Pfade, Slugs, Paketnamen, Versionen, Query-Strings oder anderen
  Artefaktparameter.
...
- Popularität wird logarithmisch skaliert und begrenzt. Skills mit hohem Engagement können niedriger eingestuft werden, wenn der Query-Text schwächer übereinstimmt.
...
- Wenn `nonSuspiciousOnly=true`, können cursorbasierte Sortierungen auf einer Seite weniger als `limit` Einträge zurückgeben, weil verdächtige Skills nach dem Abruf der Seite herausgefiltert werden.
...
- Belege werden für öffentliche Aufrufer geschwärzt und enthalten Roh-Snippets nur für Owner/Moderatoren.

### `POST /api/v1/skills/{slug}/report`

Melden Sie einen Skill zur Moderatorenprüfung. Meldungen gelten auf Skill-Ebene, sind optional mit
...
- `version.security` enthält den normalisierten Prüfstatus des Scans und Scanner-Details
...
- `version` (optional): bestimmter Versions-String.
...
- `security.hasScanResult` ist nur dann `true`, wenn ein Scanner ein eindeutiges Urteil (`clean`, `suspicious` oder `malicious`) geliefert hat.
...
- Veröffentlichte Scans schreiben nur zurück, wenn `update: true` gesetzt ist und der Scan erfolgreich abgeschlossen wurde.
...
- Scan-Jobs sind asynchron. Manuelle Scan-Anfragen werden vor normaler Veröffentlichungs-/Backfill-Arbeit priorisiert, der Abschluss hängt jedoch weiterhin von der Worker-Verfügbarkeit ab.
...
- `security.signals.dependencyRegistry` bleibt aus Kompatibilitätsgründen für v1-Antworten erhalten, aber der Scanner für die Existenz im Dependency Registry wurde eingestellt und dieser Schlüssel ist immer `null`.
...
Einheitliche Katalogsuche über Skills und Plugin-Pakete.
...
Plugin-spezifisches Durchsuchen des Katalogs über Code-Plugin- und Bundle-Plugin-Pakete.
...
Legacy-v1-Filteraliase werden bei Lese-Endpunkten weiterhin akzeptiert:
...
Legacy-Aliase werden nicht als gespeicherte oder vom Autor deklarierte Kategorienwerte akzeptiert.
...
zurück. Dies ist die öffentliche OpenClaw-Konsumoberfläche, um zu entscheiden, ob ein
aufgelöstes Release installiert werden kann.

Authentifizierung:

- Öffentlicher Lese-Endpunkt. Es ist kein Token für Inhaber, Publisher, Moderator oder Admin
  erforderlich.
...
- `trust.reasons` ist die Liste der nutzerseitigen und Audit-Erklärungen. Grundcodes
...
- Dieser Endpunkt ist versionsexakt. Clients sollten ihn nach dem Auflösen der
...
- ClawPack-Versionen geben ein `npm-pack`-Artefakt, npm-Integritätsfelder, eine
  `tarballUrl` und die Kompatibilitäts-URL für das alte ZIP zurück.
- Dies ist die OpenClaw-Resolver-Oberfläche; sie vermeidet das Raten des Archivformats anhand
...
- Erfordert ein API-Token für einen Moderator- oder Adminnutzer.
...
- Erfordert ein API-Token für einen Adminnutzer.
...
Meldet ein Paket für eine Moderatorprüfung. Meldungen gelten auf Paketebene und sind optional
mit einer Version verknüpft. Sie speisen die Moderationswarteschlange, blenden Downloads aber nicht automatisch aus und
blockieren Downloads auch nicht von selbst; Moderatoren sollten Release-Moderation verwenden, um
...
- Verwendet den Leselimit-Bucket, nicht den Download-Bucket.
...
- Packuments für scoped Pakete unterstützen sowohl `/api/npm/@scope/name` als auch den von npm
  kodierten Anforderungspfad `/api/npm/@scope%2Fname`.
...
- Verwendet den Download-Limit-Bucket.
...
- `slug` (erforderlich)
- `hash` (erforderlich): 64-stelliger Hex-SHA-256 des Bundle-Fingerprints
...
- GitHub-gestützte Skill-Übergaben proxien oder spiegeln keine Bytes. Die JSON-Antwort
  ...
  der erfolgreichen Nutzlast eingeschlossen.
...
- Optionales Nutzlastfeld: `migrateOwner`. Wenn `true` mit `ownerHandle`, kann ein
...
- Optionales Nutzlastfeld: `ownerHandle`. Wenn vorhanden, dürfen nur Admins im Namen dieses Owners veröffentlichen.
...
- Nur der Publisher der `openclaw`-Organisation und persönliche Publisher aktueller Mitglieder der `openclaw`-Organisation
...
Owner-initiierte Soft-Deletes reservieren den Slug für 30 Tage; danach kann der Slug von
...
Der Endpunkt verwendet standardmäßig einen Testlauf. Die Wiederherstellung anzuwenden erfordert `dryRun: false` und
...
Inhalte wiederherzustellen (nur Admin). Standardmäßig ein Testlauf, außer `dryRun` ist `false`.
