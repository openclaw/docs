...
x-i18n:
    generated_at: "2026-07-01T12:56:49Z"
    model: gpt-5.5
...
- Wenn `Retry-After` vorhanden ist, warten Sie vor einem erneuten Versuch so viele Sekunden.
...
- Wenn keine vertrauenswürdige Client-IP verfügbar ist, verwenden anonyme Anfragen Fallback-Buckets,
  die nur nach Art des Ratenlimits begrenzt sind. Diese Fallback-Buckets enthalten keine
...
- `q` (erforderlich): Query-Zeichenkette
...
- Ergebnisse werden in Relevanzreihenfolge zurückgegeben (Embedding-Ähnlichkeit + exakte Slug-/Namens-Token-Gewichtungen + ein kleiner Popularitäts-Prior).
- Relevanz ist stärker als Popularität. Eine präzise Übereinstimmung mit einem Slug- oder Anzeigenamen-Token kann vor einer ungenaueren Übereinstimmung mit deutlich stärkerer Interaktion rangieren.
- ASCII-Text wird an Wort- und Satzzeichengrenzen tokenisiert. Zum Beispiel enthält `personal-map` ein eigenständiges `map`-Token, während `amap-jsapi-skill` `amap`, `jsapi` und `skill` enthält; die Suche nach `map` ergibt daher für `personal-map` eine stärkere lexikalische Übereinstimmung als für `amap-jsapi-skill`.
- Popularität ist logarithmisch skaliert und gedeckelt. Skills mit hoher Interaktion können niedriger rangieren, wenn der Query-Text schwächer übereinstimmt.
...
- Platzieren Sie die Begriffe, nach denen Benutzer wörtlich suchen werden, im Anzeigenamen, in der Zusammenfassung und in Tags. Verwenden Sie ein eigenständiges Slug-Token nur, wenn es auch eine stabile Identität ist, die Sie behalten möchten.
- Benennen Sie einen Slug nicht nur um, um eine einzelne Query zu bedienen, es sei denn, der neue Slug ist ein besserer langfristiger kanonischer Name. Alte Slugs werden zu Weiterleitungsaliasen, aber die kanonische URL, der angezeigte Slug und zukünftige Such-Digests verwenden den neuen Slug.
- Umbenennungsaliase erhalten die Auflösung für alte URLs und Installationen, die über die Registry aufgelöst werden, aber das Suchranking basiert auf den kanonischen Skill-Metadaten, nachdem die Umbenennung indexiert wurde. Bestehende Statistiken bleiben beim Skill.
...
- `cursor` (optional): Paginierungscursor für jede Nicht-`trending`-Sortierung
...
- `trending` rangiert nach Installationen der letzten 7 Tage (telemetriebasiert).
...
- Alte Slugs, die durch Umbenennungs-/Zusammenführungsabläufe von Eigentümern erstellt wurden, werden zum kanonischen Skill aufgelöst.
...
- `moderation` ist nur enthalten, wenn der Skill markiert ist oder der Eigentümer ihn anzeigt.
...
- Eigentümer und Moderatoren können auf Moderationsdetails für versteckte Skills zugreifen.
- Öffentliche Aufrufer erhalten nur für bereits markierte sichtbare Skills `200`.
- Nachweise werden für öffentliche Aufrufer geschwärzt und enthalten Roh-Snippets nur für Eigentümer/Moderatoren.

### `POST /api/v1/skills/{slug}/report`

Melden Sie einen Skill zur Moderatorenprüfung. Meldungen erfolgen auf Skill-Ebene, sind optional
mit einer Version verknüpft und speisen die Skill-Meldungswarteschlange.
...
- Gibt gespeicherte Scan-Ergebnisse für die exakte übermittelte Version zurück, einschließlich blockierter oder ausgeblendeter Versionen.
...
- Skill-Identität, Publisher-Identität und Metadaten der ausgewählten Version sind Felder auf oberster Umschlagebene (`slug`, `displayName`, `publisherHandle`, `version`, `resolvedFrom`, `tag`, `createdAt`), damit Shell-Automatisierung sie ohne Entpacken verschachtelter Wrapper lesen kann.
...
- `security.signals.dependencyRegistry` bleibt für die v1-Antwortkompatibilität erhalten, aber der Scanner für die Existenz der Dependency Registry ist eingestellt und dieser Schlüssel ist immer `null`.
- `provenance` ist nur dann `server-resolved-github-import`, wenn ClawHub während Veröffentlichung oder Import ein GitHub-Repo/eine Ref/einen Commit/einen Pfad aufgelöst und gespeichert hat; andernfalls ist es `unavailable`.
...
- `security.signals.dependencyRegistry` bleibt für die v1-Antwortkompatibilität erhalten, aber der Scanner für die Existenz der Dependency Registry ist eingestellt und dieser Schlüssel ist immer `null`.
- Das Fehlen einer Skill Card wirkt sich nicht auf `ok`, `decision` oder `reasons` dieses Endpunkts aus; Clients sollten installiertes `skill-card.md` lokal lesen, wenn sie Card-Inhalte benötigen.
- Verwenden Sie `/verify`, wenn Sie den Single-Skill-Skill-Card-Verifizierungsumschlag benötigen, `/card`, wenn Sie generiertes Card-Markdown benötigen, und `/scan`, wenn Sie detaillierte Scanner-Daten benötigen.
...
Legacy-v1-Filteraliase bleiben auf Leseendpunkten akzeptiert:
...
`trending` ist eine Sieben-Tage-Bestenliste für Installationen/Downloads und verwendet keine Gesamtzahlen über die gesamte Zeit.
...
- Die Legacy-v1-Filteraliase, die unter `GET /api/v1/plugins` dokumentiert sind, werden ebenfalls
  akzeptiert.
...
- Sortiersteuerungen der Browser-UI für die Plugin-Suche ordnen die geladenen Relevanzergebnisse neu,
...
- Öffentlicher Lese-Endpunkt. Kein Token für Inhaber, Publisher, Moderator oder Admin
  erforderlich.
...
- `trust.blockedFromDownload` ist das Installations-Blockiersignal. OpenClaw und andere
  ...
  Blockierregeln erneut aus Scanner- oder Moderationsfeldern abzuleiten.
...
- `all`: jedes Release mit manueller Überschreibung, nicht sauberem Scan-Status oder Paketbericht.
...
mit einer Version verknüpft. Sie speisen die Moderationswarteschlange, blenden Downloads aber
nicht automatisch aus oder blockieren sie von sich aus; Moderatoren sollten Release-Moderation verwenden, um
...
`finalAction: "revoke"` zusammen mit einer bestätigten Meldung, um Release-Moderation im
...
Unter Quarantäne gestellte und widerrufene Releases geben auf Artefakt-Download-Routen `403` zurück.
...
- Dateigrößenlimit: 200 KB.
- Ausstehende VirusTotal-Scans blockieren Lesezugriffe nicht; schädliche Releases können an anderer Stelle weiterhin zurückgehalten werden.
- Private Pakete geben `404` zurück, sofern der Aufrufer den besitzenden Publisher nicht lesen kann.
...
- Registry-only-Metadaten werden nicht in das heruntergeladene Archiv injiziert.
...
- Scoped-Package-Packuments unterstützen sowohl `/api/npm/@scope/name` als auch npms
  codierten Anforderungspfad `/api/npm/@scope%2Fname`.
...
Lädt ein gehostetes Skill-Versions-ZIP herunter oder gibt eine GitHub-Quellcode-Übergabe für einen
...
- GitHub-gestützte Skill-Übergaben proxyen oder spiegeln keine Bytes. Die JSON-Antwort
  ...
  und `archiveUrl`; Scan-/Current-State ist ein Gate und wird nicht als Erfolgs-
...
- Zulässige Formularfelder sind `payload`, wiederholte `files`-Blobs oder eine `clawpack`-
  ...
  Upload-URL-Flow zurückgegeben wurde. Veröffentlichungen mit gestageter Storage-ID müssen außerdem das
...
- JSON-Bodys und vom Aufrufer gelieferte `payload.files`- / `payload.artifact`-
  Metadaten werden abgelehnt.
- Direkte Multipart-Veröffentlichungsanfragen sind auf 18 MB begrenzt. ClawPack-Tarballs können
  den Upload-URL-Flow bis zum 120-MB-Tarball-Limit verwenden.
...
- Veröffentlichungen im Auftrag anderer validieren die Eignung für den offiziellen Kanal weiterhin gegen das Ziel-Owner-Konto.
...
Nur Admin. Stellt sicher, dass ein Org-Publisher für einen Handle existiert. Wenn der Handle noch auf einen
...
Nur Admin. Stellt einen persönlichen Publisher für einen verifizierten ersetzenden GitHub-OAuth-Principal
wieder her, ohne Convex-Auth-Kontozeilen zu bearbeiten. Die Anfrage muss beide unveränderlichen GitHub-
Provider-Konto-IDs nennen; veränderliche Handles werden nur als operatorseitige Schutzprüfung verwendet.
...
Skill-Slug-Aliase, Pakete, Package-Inspector-Warnungen und abgeleitete Search-Digest-Zeilen, sodass
...
Profilsynchronisierung nicht die konkurrierende Autorität des früheren Benutzers wiederherstellen kann. Jede primäre Tabelle ist pro Apply-Transaktion auf
...
### Endpunkte für Owner-Slug-Verwaltung
...
- `rename` erhält den vorherigen Slug als Redirect-Alias.
- `merge` blendet den Quell-Listing aus und leitet den Quell-Slug zum Ziel-Listing weiter.

### Endpunkte für Eigentumsübertragung
...
Die Sperre eines Benutzers aufheben und berechtigte Skills wiederherstellen (nur Admin).
...
Einen Stern hinzufügen/entfernen (Highlights). Beide Endpunkte sind idempotent.
