...
x-i18n:
    generated_at: "2026-07-05T04:58:14Z"
    model: gpt-5.5
...
Drittanbieter-Verzeichnisse können die öffentlichen Lese-Endpunkte verwenden, um ClawHub-Skills aufzulisten oder zu durchsuchen. Bitte cachen Sie Ergebnisse, beachten Sie `429`/`Retry-After`, verlinken Sie Benutzer zurück zum kanonischen ClawHub-Eintrag (`https://clawhub.ai/<owner>/skills/<slug>`) und vermeiden Sie den Eindruck, ClawHub empfehle die Drittanbieter-Website. Versuchen Sie nicht, verborgene, private oder durch Moderation blockierte Inhalte außerhalb der öffentlichen API-Oberfläche zu spiegeln.
...
- Wenn das Token fehlt/ungültig ist, fällt das Verhalten auf IP-Erzwingung zurück.
...
- `X-RateLimit-Remaining` / `RateLimit-Remaining`: genau verbleibendes Budget, wenn vorhanden.
  Erfolgreiche shard-basierte Anfragen lassen diesen Header weg, statt einen ungefähren globalen Wert zurückzugeben.
- `Retry-After`: Sekunden, die vor einem erneuten Versuch zu warten sind (Verzögerung), bei `429`
...
- Wenn keine vertrauenswürdige Client-IP verfügbar ist, verwenden anonyme Anfragen Fallback-Buckets, die nur nach Rate-Limit-Art abgegrenzt sind. Diese Fallback-Buckets enthalten keine vom Aufrufer gelieferten Pfade, Slugs, Paketnamen, Versionen, Query-Strings oder sonstigen Artefaktparameter.

## Fehlerantworten

Öffentliche v1-Fehlerantworten sind Klartext mit `content-type: text/plain; charset=utf-8`.
Dies umfasst Validierungsfehler (`400`), fehlende öffentliche Ressourcen (`404`), Authentifizierungs- und Berechtigungsfehler (`401`/`403`), Rate Limits (`429`) und blockierte Downloads. Clients sollten den Antworttext als menschenlesbare Zeichenkette lesen. Unbekannte Query-Parameter werden aus Kompatibilitätsgründen ignoriert, aber erkannte Query-Parameter mit ungültigen Werten geben `400` zurück.
...
- Ergebnisse werden nach Relevanz zurückgegeben (Embedding-Ähnlichkeit + genaue Slug-/Namens-Token-Boosts + ein kleiner Popularitäts-Prior).
- Relevanz ist stärker als Popularität. Eine präzise Übereinstimmung mit einem Slug- oder Anzeigenamen-Token kann eine unschärfere Übereinstimmung mit deutlich stärkerem Engagement übertreffen.
- ASCII-Text wird an Wort- und Satzzeichengrenzen tokenisiert. Zum Beispiel enthält `personal-map` ein eigenständiges `map`-Token, während `amap-jsapi-skill` `amap`, `jsapi` und `skill` enthält; eine Suche nach `map` gibt `personal-map` daher eine stärkere lexikalische Übereinstimmung als `amap-jsapi-skill`.
- Popularität ist logarithmisch skaliert und gedeckelt. Skills mit hohem Engagement können niedriger ranken, wenn der Abfragetext schwächer übereinstimmt.
...
- Benennen Sie einen Slug nicht nur um, um eine einzelne Suchanfrage zu bedienen, es sei denn, der neue Slug ist ein besserer langfristiger kanonischer Name. Alte Slugs werden zu Redirect-Aliassen, aber die kanonische URL, der angezeigte Slug und künftige Such-Digests verwenden den neuen Slug.
...
- `cursor` (optional): Paginierungs-Cursor für jede Sortierung außer `trending`
...
- Wenn `nonSuspiciousOnly=true`, können cursorbasierte Sortierungen auf einer Seite weniger als `limit` Elemente zurückgeben, da verdächtige Skills nach dem Abruf der Seite herausgefiltert werden.
...
Melden Sie einen Skill zur Moderatorprüfung. Meldungen gelten auf Skill-Ebene, sind optional mit einer Version verknüpft und speisen die Skill-Meldewarteschlange.
...
Moderator-/Admin-Endpunkt zum Auflösen oder Wiedereröffnen von Skill-Meldungen.
...
- Scan-Anfrage-Payloads und herunterladbare Berichte laufen nach dem Aufbewahrungszeitraum aus dem Scan-Request-Speicher ab.
- Veröffentlichte Scans erfordern Owner-/Publisher-Verwaltungszugriff oder Moderator-/Admin-Berechtigung der Plattform.
- Veröffentlichte Scans schreiben nur dann zurück, wenn `update: true` ist und der Scan erfolgreich abgeschlossen wird.
...
- Scan-Jobs sind asynchron. Manuelle Scan-Anfragen werden vor normaler Publish-/Backfill-Arbeit priorisiert, aber der Abschluss hängt weiterhin von der Worker-Verfügbarkeit ab.
...
- Gibt `queue.queuedAhead` und `queue.position` zurück, während der Job in der Warteschlange ist, damit Clients anzeigen können, wie viele priorisierte manuelle Scans vor der Anfrage liegen. Sehr große Warteschlangen werden begrenzt und mit `queuedAheadIsEstimate: true` gemeldet.
- Wenn verfügbar, enthält `report` Abschnitte für `clawscan`, `skillspector`, `staticAnalysis` und `virustotal`.
...
- `security.signals.dependencyRegistry` bleibt aus Gründen der v1-Antwortkompatibilität erhalten, aber der Scanner für die Existenz des Dependency-Registry ist außer Betrieb genommen und dieser Schlüssel ist immer `null`.
- `provenance` ist nur dann `server-resolved-github-import`, wenn ClawHub während Publish oder Import ein GitHub-Repo, eine Ref, einen Commit oder einen Pfad aufgelöst und gespeichert hat; andernfalls ist es `unavailable`.
...
- Die Antwort ist ausschließlich security-bezogen. Sie enthält keine Skill-Card-Daten, keinen Status generierter Cards, keine Artefakt-Dateilisten und keine detaillierten Scanner-Payloads.
...
- `category` (optional): Plugin-Kategoriefilter. Nur unterstützt, wenn die
  ...
  `family=code-plugin`/`family=bundle-plugin`). Gesteuerte Kategorien und
...
- Authentifizierte Aufrufer können private Pakete für Herausgeber sehen, denen sie in Listen-/Suchergebnissen angehören.
...
- `isOfficial` (optional): `true` oder `false`
- `category` (optional): Plugin-Kategoriefilter. Nur unterstützt, wenn die
  Anfrage auf Plugin-Pakete beschränkt ist. Gesteuerte Kategorien und Legacy-v1-
...
`trending` ist eine Sieben-Tage-Bestenliste für Installationen/Downloads und verwendet keine Gesamtzahlen über die gesamte Laufzeit.
...
- Sortiersteuerelemente der Browser-UI für die Plugin-Suche ordnen die geladenen Relevanzergebnisse neu,
  passend zum aktuellen `/skills`-Browse-Verhalten.
...
- `all`: jedes Release mit einer manuellen Überschreibung, nicht sauberem Scan-Status oder Paketbericht.
...
Meldet ein Paket zur Moderator-Review. Meldungen gelten auf Paketebene und sind optional
...
- `quarantined`: blockiert bis zur weiteren Klärung.
...
Gibt den rohen Textinhalt einer Paketdatei zurück.
...
- Verwendet den Download-Rate-Bucket.
- Download-Header enthalten die ClawHub-SHA-256 plus npm-Integritäts-/Shasum-Metadaten.
...
- Wenn weder `version` noch `tag` angegeben wird, wird die neueste Version verwendet.
...
- Optionales Payload-Feld: `migrateOwner`. Wenn `true` zusammen mit `ownerHandle`, kann ein
  bestehender Skill zu diesem Eigentümer verschoben werden, sofern der Akteur Admin/Eigentümer sowohl beim
  aktuellen als auch beim Ziel-Publisher ist. Ohne dieses Opt-in werden Eigentümerwechsel
  abgelehnt.
...
- Veröffentlichungen im Auftrag eines anderen validieren die Berechtigung für den official-Channel weiterhin gegen das Ziel-Eigentümerkonto.

### `DELETE /api/v1/skills/{slug}` / `POST /api/v1/skills/{slug}/undelete`

Soft-Delete / Wiederherstellen eines Skills (Eigentümer, Moderator oder Admin).
...
Wenn vorhanden, wird `reason` als Skill-Moderationsnotiz gespeichert und in den Audit-Log kopiert.
...
Nur für Admins. Stellt einen persönlichen Publisher für einen verifizierten Ersatz-GitHub-OAuth-Principal
...
Der Endpunkt ist standardmäßig ein Dry-Run. Für die Anwendung der Wiederherstellung sind `dryRun: false` und
`confirmIdentityVerified: true` erforderlich, nachdem Mitarbeitende unabhängig die Kontinuität zwischen beiden
GitHub-Principals überprüft haben. Die Wiederherstellung schlägt geschlossen fehl, wenn der aktuelle persönliche
...
Skill-Slug-Aliasse, Pakete, Paketinspektor-Warnungen und abgeleitete Such-Digest-Zeilen, damit
Direkt-Owner-Pfade mit der neuen Publisher-Autorität übereinstimmen. Eine aktive geschützte Handle-Reservierung
für das wiederhergestellte Handle wird ebenfalls dem Ersatzbenutzer neu zugewiesen, sodass eine spätere
...
100 Zeilen pro Apply-Transaktion begrenzt; größere Wiederherstellungen müssen zuerst eine fortsetzbare Owner-Migration verwenden.
...
- `rename` behält den vorherigen Slug als Redirect-Alias bei.
...
- `POST /api/v1/skills/{slug}/transfer/cancel`
  - Response (Annehmen/Ablehnen/Abbrechen): `{ "ok": true, "skillSlug": "demo-skill?" }`
...
Einen Benutzer sperren und eigene Skills dauerhaft löschen (nur Moderator/Admin).
...
Den gespeicherten Grund für eine vorhandene Sperre ändern, ohne Inhalte zu entsperren oder wiederherzustellen
(nur Admin). Standardmäßig Dry-Run, sofern `dryRun` nicht `false` ist.
...
Einen Star hinzufügen/entfernen (Hervorhebungen). Beide Endpunkte sind idempotent.
