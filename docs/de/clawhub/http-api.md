...
Die Legacy-Pfade `/api/...` und `/api/cli/...` bleiben aus Kompatibilitätsgründen bestehen (siehe `DEPRECATIONS.md`).
...
- `X-RateLimit-Remaining` / `RateLimit-Remaining`: exaktes verbleibendes Budget, falls vorhanden.
  Erfolgreiche geshardete Anfragen lassen diesen Header weg, statt einen ungefähren globalen Wert zurückzugeben.
...
- Wenn `Retry-After` vorhanden ist, warten Sie so viele Sekunden, bevor Sie es erneut versuchen.
- Verwenden Sie jittered Backoff, um synchronisierte Wiederholungsversuche zu vermeiden.
...
- Wenn keine vertrauenswürdige Client-IP verfügbar ist, verwenden anonyme Anfragen Fallback-Buckets, die nur nach Rate-Limit-Art gescoped sind. Diese Fallback-Buckets enthalten keine vom Aufrufer bereitgestellten Pfade, Slugs, Paketnamen, Versionen, Query-Strings oder andere Artefaktparameter.

## Fehlerantworten

Öffentliche v1-Fehlerantworten sind Klartext mit `content-type: text/plain; charset=utf-8`.
Dies umfasst Validierungsfehler (`400`), fehlende öffentliche Ressourcen (`404`), Auth- und Berechtigungsfehler (`401`/`403`), Rate Limits (`429`) und blockierte Downloads. Clients sollten den Antwortkörper als menschenlesbare Zeichenfolge lesen. Unbekannte Query-Parameter werden aus Kompatibilitätsgründen ignoriert, aber erkannte Query-Parameter mit ungültigen Werten geben `400` zurück.
...
- Ergebnisse werden in Relevanzreihenfolge zurückgegeben (Embedding-Ähnlichkeit + exakte Slug-/Namens-Token-Boosts + kleiner Popularitäts-Prior).
- Relevanz ist stärker als Popularität. Eine präzise Übereinstimmung mit einem Slug- oder Anzeigenamen-Token kann höher rangieren als eine lockerere Übereinstimmung mit deutlich stärkerem Engagement.
- ASCII-Text wird an Wort- und Satzzeichengrenzen tokenisiert. Zum Beispiel enthält `personal-map` ein eigenständiges `map`-Token, während `amap-jsapi-skill` `amap`, `jsapi` und `skill` enthält; eine Suche nach `map` ergibt daher für `personal-map` eine stärkere lexikalische Übereinstimmung als für `amap-jsapi-skill`.
- Popularität ist logarithmisch skaliert und gedeckelt. Skills mit hohem Engagement können niedriger rangieren, wenn der Query-Text schwächer übereinstimmt.
...
- Benennen Sie einen Slug nicht nur um, um eine einzelne Query zu bedienen, es sei denn, der neue Slug ist ein besserer langfristiger kanonischer Name. Alte Slugs werden zu Redirect-Aliassen, aber die kanonische URL, der angezeigte Slug und zukünftige Such-Digests verwenden den neuen Slug.
...
- Evidence wird für öffentliche Aufrufer redigiert und enthält Roh-Snippets nur für Owner/Moderatoren.

### `POST /api/v1/skills/{slug}/report`

Melden Sie einen Skill zur Überprüfung durch Moderatoren. Meldungen sind auf Skill-Ebene, optional mit einer Version verknüpft, und speisen die Skill-Meldewarteschlange.
...
- `version.security` enthält, wenn verfügbar, normalisierten Status der Scan-Verifizierung und Scanner-Details (VirusTotal + LLM).
...
- Prüfen Sie beim Abfragen einer historischen Version `moderation.matchesRequestedVersion` und `moderation.sourceVersion`, bevor Sie `moderation` und `security` als denselben Versionskontext behandeln.
...
- Scan-Anfrage-Payloads und herunterladbare Berichte laufen nach Ablauf des Aufbewahrungsfensters aus dem Scan-Request-Speicher ab.
- Veröffentlichte Scans erfordern Owner-/Publisher-Verwaltungszugriff oder Moderator-/Admin-Berechtigung der Plattform.
...
- Gibt während der Warteschlange `queue.queuedAhead` und `queue.position` zurück, damit Clients anzeigen können, wie viele priorisierte manuelle Scans vor der Anfrage liegen. Sehr große Warteschlangen werden begrenzt und mit `queuedAheadIsEstimate: true` gemeldet.
...
- Erfordert Owner-/Publisher-Verwaltungszugriff auf den Skill oder das Plugin oder Moderator-/Admin-Berechtigung der Plattform.
...
Nur für Admins bestimmte kanonische Route für Batch-Rescans. Sie akzeptiert dieselbe Payload-Struktur wie das ältere `POST /api/v1/skills/-/rescan-batch`.

### `POST /api/v1/skills/-/scan/batch/status`

Nur für Admins bestimmte kanonische Route für Batch-Status. Sie akzeptiert `{ "jobIds": ["..."] }` und gibt dieselben aggregierten Zähler wie das ältere `POST /api/v1/skills/-/rescan-batch/status` zurück.
...
- `security.signals.dependencyRegistry` bleibt für die Kompatibilität der v1-Antwort erhalten, aber der Existenz-Scanner der Dependency Registry ist eingestellt, und dieser Schlüssel ist immer `null`.
...
Sammlungs-Endpunkt ist für Clients gedacht, die bereits wissen, welche installierten
...
- Die Antwort enthält nur Sicherheitsinformationen. Sie enthält keine Skill Card-Daten, keinen Status generierter Karten, keine Artefakt-Dateilisten und keine detaillierten Scanner-Payloads.
- `security.signals` enthält nur unterstützende Nachweise auf Statusebene; verwenden Sie `/scan` oder die ClawHub-Security-Audit-Seite für vollständige Scanner-Details.
- `security.signals.dependencyRegistry` bleibt für die Kompatibilität der v1-Antwort erhalten, aber der Existenz-Scanner der Dependency Registry ist eingestellt, und dieser Schlüssel ist immer `null`.
- Das Fehlen einer Skill Card beeinflusst `ok`, `decision` oder `reasons` dieses Endpunkts nicht; Clients sollten das installierte `skill-card.md` lokal lesen, wenn sie Karteninhalte benötigen.
...
- `GET /api/v1/code-plugins` und `GET /api/v1/bundle-plugins` bleiben Aliasse mit festgelegter Familie.
...
Legacy-v1-Filteraliasse werden bei Lese-Endpunkten weiterhin akzeptiert:
...
`trending` ist eine Sieben-Tage-Bestenliste für Installationen/Downloads und verwendet keine Gesamtwerte über die gesamte Zeit.
Auf dem einheitlichen Endpunkt `/api/v1/packages` ist es nur für Plugins vorgesehen; verwenden Sie
...
- `limit` (optional): Ganzzahl (1-100)
- `isOfficial` (optional): `true` oder `false`
- `category` (optional): Filter für Plugin-Kategorie. Aktuelle Werte:
  `channels`, `models`, `memory`, `context`, `voice`, `media`, `web`,
  `tools`, `runtime`, `gateway`, `security`, `other`.
...
- Die alten v1-Filteraliase, die unter `GET /api/v1/plugins` dokumentiert sind, werden ebenfalls
  akzeptiert.
- Kategoriefilterung ist ein echter API-Filter, der auf Digest-Zeilen für Plugin-Kategorien
  basiert, kein Umschreiben der Suchabfrage.
...
- Erfordert ein API-Token für den Paketbesitzer, einen Besitzer/Admin des Organisations-Publishers,
...
- Öffentlicher Lese-Endpunkt. Kein Token für Besitzer, Publisher, Moderator oder Admin
  erforderlich.
...
- `trust.pending` bedeutet, dass eine oder mehrere Vertrauen-Eingaben noch auf Abschluss warten.
- `trust.stale` bedeutet, dass die Vertrauenszusammenfassung aus veralteten Eingaben berechnet wurde und
  vor einer Erlaubnisentscheidung mit hoher Sicherheit als aktualisierungsbedürftig behandelt werden sollte.
...
- Dieser Endpunkt ist absichtlich enger gefasst als Moderationsendpunkte für Besitzer/Moderatoren.
  ...
  Reporter-Identitäten, Berichtsinhalte, private Belege oder interne Review-
  Zeitpläne.
...
- Alte Paketversionen geben ein `legacy-zip`-Artefakt und eine alte ZIP-
  `downloadUrl` zurück.
...
- ClawPack-Versionen streamen die exakt hochgeladenen `.tgz`-Bytes des npm-pack.
- Alte ZIP-Versionen leiten zu `/api/v1/packages/{name}/download?version=` weiter.
...
- `all`: jedes Release mit manueller Überschreibung, nicht sauberem Scanstatus oder Paketbericht.
...
Melden Sie ein Paket zur Moderator-Review. Meldungen erfolgen auf Paketebene und sind optional
mit einer Version verknüpft. Sie speisen die Moderationswarteschlange, blenden Downloads jedoch nicht
automatisch aus und blockieren sie auch nicht eigenständig; Moderatoren sollten Release-Moderation verwenden, um
...
Besitzer-/Moderator-Endpunkt für die Moderationssichtbarkeit von Paketen.
...
- Dateigrößenlimit: 200 KB.
- Ausstehende VirusTotal-Scans blockieren Lesevorgänge nicht; schädliche Releases können andernorts trotzdem zurückgehalten werden.
- Private Pakete geben `404` zurück, sofern der Aufrufer den besitzenden Herausgeber nicht lesen kann.
...
- Übergaben für GitHub-gestützte Skills proxen oder spiegeln keine Bytes. Die JSON-Antwort
  enthält `sourceRef: "public-github"`, `repo`, `commit`, `path`, `contentHash`
  und `archiveUrl`; der Scan-/aktuelle Zustand ist ein Gate und wird nicht als Erfolgs-
...
- Optionales Payload-Feld: `ownerHandle`. Wenn vorhanden, löst die API diesen
  Herausgeber serverseitig auf und verlangt vom Akteur Herausgeberzugriff.
...
- Nur der `openclaw`-Org-Herausgeber und die persönlichen Herausgeber aktueller Mitglieder der
  `openclaw`-Org dürfen im `official`-Kanal veröffentlichen.
- Veröffentlichungen im Namen anderer validieren die Berechtigung für den offiziellen Kanal weiterhin gegen das Ziel-Besitzerkonto.

### `DELETE /api/v1/skills/{slug}` / `POST /api/v1/skills/{slug}/undelete`

Einen Skill soft-löschen / wiederherstellen (Besitzer, Moderator oder Admin).
...
Vom Besitzer initiierte Soft-Löschungen reservieren den Slug für 30 Tage; danach kann der Slug von
einem anderen Herausgeber beansprucht werden. Die Löschantwort enthält `slugReservedUntil`, wenn diese Ablaufzeit gilt.
...
Nur für Admins. Stellt sicher, dass ein Org-Herausgeber für ein Handle existiert. Wenn das Handle noch auf einen
alten gemeinsam genutzten Benutzer-/persönlichen Herausgeber verweist, migriert der Endpunkt es zuerst in einen Org-Herausgeber.
...
Authentifizierte Self-Service-Erstellung von Org-Herausgebern. Erstellt einen neuen Org-Herausgeber und fügt den
Aufrufer als Besitzer hinzu. Dieser Endpunkt migriert keine vorhandenen Benutzer-/persönlichen Handles und
...
Nur für Admins. Stellt einen persönlichen Herausgeber für einen verifizierten Ersatz-GitHub-OAuth-Principal wieder her,
...
GitHub-Principals unabhängig verifiziert haben. Die Wiederherstellung schlägt sicherheitsgerichtet fehl, wenn der aktuelle persönliche
...
Skill-Slug-Aliasse, Pakete, Paket-Inspector-Warnungen und abgeleitete Search-Digest-Zeilen, sodass
Direct-Owner-Pfade mit der neuen Publisher-Autorität übereinstimmen. Eine aktive Protected-Handle-
Reservierung für den wiederhergestellten Handle wird ebenfalls dem Ersatzbenutzer neu zugewiesen, damit spätere
Profilsynchronisierung nicht die konkurrierende Autorität des früheren Benutzers wiederherstellen kann. Jede Primärtabelle ist pro Apply-Transaktion auf
...
- `rename` bewahrt den vorherigen Slug als Redirect-Alias.
