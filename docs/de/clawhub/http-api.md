...
x-i18n:
    generated_at: "2026-07-02T14:00:44Z"
    model: gpt-5.5
...
Drittanbieter-Verzeichnisse dürfen die öffentlichen Leseendpunkte verwenden, um ClawHub-Skills aufzulisten oder zu durchsuchen. Bitte cachen Sie Ergebnisse, beachten Sie `429`/`Retry-After`, verlinken Sie Nutzer zurück zum kanonischen ClawHub-Eintrag (`https://clawhub.ai/<owner>/skills/<slug>`) und vermeiden Sie den Eindruck, ClawHub unterstütze die Drittanbieter-Website. Versuchen Sie nicht, ausgeblendete, private oder durch Moderation blockierte Inhalte außerhalb der öffentlichen API-Oberfläche zu spiegeln.

Web-Slug-Kurzformen werden über Registry-Familien hinweg aufgelöst, API-Clients sollten jedoch die von Leseendpunkten zurückgegebenen kanonischen URLs verwenden, statt die Routenpriorität nachzubilden.
...
- Authentifizierte Anfragen (gültiges Bearer-Token): Durchsetzung pro Nutzer-Bucket.
- Wenn das Token fehlt/ungültig ist, fällt das Verhalten auf IP-Durchsetzung zurück.
- Authentifizierte Schreibendpunkte sollten kein bloßes `Unauthorized` zurückgeben, wenn der Server den Grund kennt. Fehlende Tokens, ungültige/widerrufene Tokens sowie gelöschte/gesperrte/deaktivierte Konten sollten jeweils handlungsorientierten Text erhalten, damit CLI-Clients Nutzern mitteilen können, was sie blockiert hat.
...
- Verwendet vertrauenswürdige Client-IP-Header, einschließlich `cf-connecting-ip`, nur wenn die Bereitstellung vertrauenswürdige Forwarded-Header ausdrücklich aktiviert.
- ClawHub verwendet vertrauenswürdige Forwarding-Header, um Client-IPs am Edge zu identifizieren.
- Wenn keine vertrauenswürdige Client-IP verfügbar ist, verwenden anonyme Anfragen Fallback-Buckets, die nur nach Rate-Limit-Art begrenzt sind. Diese Fallback-Buckets enthalten keine vom Aufrufer gelieferten Pfade, Slugs, Paketnamen, Versionen, Query-Strings oder andere Artefaktparameter.

## Fehlerantworten

Öffentliche v1-Fehlerantworten sind Klartext mit `content-type: text/plain; charset=utf-8`.
Dies umfasst Validierungsfehler (`400`), fehlende öffentliche Ressourcen (`404`), Authentifizierungs- und Berechtigungsfehler (`401`/`403`), Rate Limits (`429`) und blockierte Downloads. Clients sollten den Antworttext als menschenlesbare Zeichenkette lesen. Unbekannte Query-Parameter werden aus Kompatibilitätsgründen ignoriert, erkannte Query-Parameter mit ungültigen Werten geben jedoch `400` zurück.
...
- `q` (erforderlich): Suchzeichenkette
...
- Ergebnisse werden in Relevanzreihenfolge zurückgegeben (Embedding-Ähnlichkeit + Boosts für exakte Slug-/Namen-Token + ein kleiner Popularitäts-Prior).
...
- Popularität ist logarithmisch skaliert und gedeckelt. Skills mit hohem Engagement können niedriger ranken, wenn der Suchtext eine schwächere Übereinstimmung ist.
...
Empfehlungen zur Auffindbarkeit für Publisher:

- Verwenden Sie die Begriffe, nach denen Nutzer wörtlich suchen werden, im Anzeigenamen, in der Zusammenfassung und in Tags. Nutzen Sie ein eigenständiges Slug-Token nur, wenn es auch eine stabile Identität ist, die Sie beibehalten möchten.
- Benennen Sie einen Slug nicht nur um, um eine einzelne Suchanfrage zu bedienen, es sei denn, der neue Slug ist langfristig der bessere kanonische Name. Alte Slugs werden zu Weiterleitungsaliasen, aber die kanonische URL, der angezeigte Slug und zukünftige Suchdigests verwenden den neuen Slug.
- Umbenennungsaliasse erhalten die Auflösung für alte URLs und Installationen, die über die Registry auflösen, aber das Suchranking basiert nach der Indexierung der Umbenennung auf den kanonischen Skill-Metadaten. Bestehende Statistiken bleiben beim Skill.
...
- Wenn `nonSuspiciousOnly=true` ist, können cursorbasierte Sortierungen auf einer Seite weniger als `limit` Einträge zurückgeben, weil verdächtige Skills nach dem Abrufen der Seite herausgefiltert werden.
...
Meldet einen Skill zur Moderatorprüfung. Meldungen gelten auf Skill-Ebene, sind optional mit einer Version verknüpft und speisen die Skill-Meldungswarteschlange.
...
- Veröffentlichte Scans erfordern Verwaltungszugriff als Owner/Publisher oder Plattformberechtigung als Moderator/Admin.
...
- Scan-Jobs sind asynchron. Manuelle Scan-Anfragen werden vor normaler Publish-/Backfill-Arbeit priorisiert, der Abschluss hängt jedoch weiterhin von der Worker-Verfügbarkeit ab.
...
- Erfordert Verwaltungszugriff als Owner/Publisher auf den Skill oder das Plugin oder Plattformberechtigung als Moderator/Admin.
...
Nur für Admins verfügbare kanonische Route für Batch-Rescans. Sie akzeptiert dieselbe Payload-Struktur wie das Legacy-`POST /api/v1/skills/-/rescan-batch`.

### `POST /api/v1/skills/-/scan/batch/status`

Nur für Admins verfügbare kanonische Route für Batch-Status. Sie akzeptiert `{ "jobIds": ["..."] }` und gibt dieselben aggregierten Zähler wie das Legacy-`POST /api/v1/skills/-/rescan-batch/status` zurück.
...
- `security.signals.dependencyRegistry` bleibt für die Kompatibilität mit v1-Antworten erhalten, aber der Scanner für die Existenz im Dependency-Registry wurde eingestellt, und dieser Schlüssel ist immer `null`.
- `provenance` ist nur dann `server-resolved-github-import`, wenn ClawHub beim Veröffentlichen oder Importieren ein GitHub-Repo, eine Ref, einen Commit oder einen Pfad aufgelöst und gespeichert hat; andernfalls ist der Wert `unavailable`.

### `POST /api/v1/skills/-/security-verdicts`

Gibt aktuelle kompakte Sicherheitsurteile für exakte Skill-Versionen zurück. Dieser
Collection-Endpunkt ist für Clients vorgesehen, die bereits wissen, welche installierten
...
- `items` muss 1 bis 100 eindeutige Paare aus `{ slug, version }` enthalten.
- Ergebnisse gelten pro Eintrag; ein fehlender Skill oder eine fehlende Version lässt nicht die gesamte Antwort fehlschlagen.
- Die Antwort ist ausschließlich sicherheitsbezogen. Sie enthält keine Skill-Card-Daten, keinen Status generierter Karten, keine Artefakt-Dateilisten und keine detaillierten Scanner-Payloads.
- `security.signals` enthält nur unterstützende Nachweise auf Status-Ebene; verwenden Sie `/scan` oder die ClawHub-Seite für Sicherheits-Audits für vollständige Scanner-Details.
- `security.signals.dependencyRegistry` bleibt für die Kompatibilität mit v1-Antworten erhalten, aber der Scanner für die Existenz im Dependency-Registry wurde eingestellt, und dieser Schlüssel ist immer `null`.
...
- Anonyme Aufrufer sehen nur öffentliche Paket-Channel.
...
`trending` ist eine Sieben-Tage-Rangliste für Installationen/Downloads und verwendet keine Gesamtwerte über alle Zeiten.
...
- Sortiersteuerelemente der Browseroberfläche für die Plugin-Suche sortieren die geladenen Relevanzergebnisse neu
  und entsprechen damit dem aktuellen Suchverhalten von `/skills`.
...
- Dieser Endpunkt ist absichtlich enger gefasst als Moderationsendpunkte für Besitzer/Moderatoren.
  ...
  Melderidentitäten, Meldeinhalte, private Belege oder interne Review-
  Zeitachsen.
...
- `packageName` ist npm-name-normalisiert; das Paket kann bei geplanten
  Migrationen fehlen.
...
Melden Sie ein Paket für Moderator-Review. Meldungen erfolgen auf Paketebene und sind optional
mit einer Version verknüpft. Sie fließen in die Moderationswarteschlange ein, verbergen oder
blockieren Downloads jedoch nicht automatisch; Moderatoren sollten die Release-Moderation verwenden, um
Artefakte zu genehmigen, in Quarantäne zu versetzen oder zu widerrufen.
...
Moderator-/Administrator-Endpunkt für die Entgegennahme von Paketmeldungen.
...
Endpunkt für Inhaber/Moderatoren zur Sichtbarkeit der Paketmoderation.
...
Moderator-/Administrator-Endpunkt zum Auflösen oder Wiedereröffnen von Paketmeldungen.
...
Moderator-/Administrator-Endpunkt für die Prüfung von Paket-Releases.
...
- Verwendet das Lese-Raten-Bucket, nicht das Download-Bucket.
- Binärdateien geben `415` zurück.
- Dateigrößenlimit: 200 KB.
- Ausstehende VirusTotal-Scans blockieren Lesezugriffe nicht; bösartige Releases können an anderer Stelle dennoch zurückgehalten werden.
...
- Nur Registry-Metadaten werden nicht in das heruntergeladene Archiv injiziert.
- Ausstehende VirusTotal-Scans blockieren Downloads nicht; bösartige Releases geben `403` zurück.
- Private Pakete geben `404` zurück, es sei denn, der Aufrufer ist der Inhaber.
...
- `dist.tarball`, `dist.integrity` und `dist.shasum` verwenden npm-kompatible
  Felder, damit Benutzer npm auf den Spiegel verweisen können, wenn sie möchten.
...
- Optionales Payload-Feld: `migrateOwner`. Wenn `true` mit `ownerHandle`, kann ein
  vorhandener Skill zu diesem Inhaber verschoben werden, wenn der Akteur Administrator/Inhaber sowohl beim
  aktuellen als auch beim Ziel-Publisher ist. Ohne dieses Opt-in werden Inhaberänderungen
  abgelehnt.
...
- Optionales Payload-Feld: `ownerHandle`. Wenn vorhanden, dürfen nur Administratoren im Namen dieses Inhabers veröffentlichen.
...
- Veröffentlichungen im Auftrag anderer validieren die Berechtigung für den official-Kanal weiterhin gegen das Zielinhaberkonto.

### `DELETE /api/v1/skills/{slug}` / `POST /api/v1/skills/{slug}/undelete`

Soft-Delete / Wiederherstellen eines Skills (Inhaber, Moderator oder Administrator).
...
Vom Inhaber initiierte Soft-Deletes reservieren den Slug 30 Tage lang, danach kann der Slug von
einem anderen Publisher beansprucht werden. Die Löschantwort enthält `slugReservedUntil`, wenn dieser Ablauf gilt.
Ausblendungen durch Moderatoren/Administratoren und Sicherheitsentfernungen laufen auf diese Weise nicht ab.
...
Nur Administratoren. Stellt sicher, dass ein Org-Publisher für einen Handle existiert. Wenn der Handle noch auf einen
alten gemeinsamen Benutzer-/persönlichen Publisher zeigt, migriert der Endpunkt ihn zuerst in einen Org-Publisher.
Geben Sie für eine neu erstellte Org `memberHandle` an; der handelnde Administrator wird nicht als Mitglied hinzugefügt.
...
Aufrufer als Inhaber hinzu. Dieser Endpunkt migriert keine vorhandenen Benutzer-/persönlichen Handles und markiert
...
Nur Administratoren. Reserviert Root-Slugs und Paketnamen für einen rechtmäßigen Inhaber, ohne ein
Release zu veröffentlichen. Paketnamen werden zu privaten Platzhalterpaketen ohne Release-Zeilen, sodass derselbe
Inhaber später das echte Code-Plugin- oder Bundle-Plugin-Release unter diesem Namen veröffentlichen kann.
...
Nur Administratoren. Stellt einen persönlichen Publisher für einen verifizierten Ersatz-GitHub-OAuth-Principal wieder her,
...
Der Endpunkt ist standardmäßig ein Testlauf. Die Anwendung der Wiederherstellung erfordert `dryRun: false` und
`confirmIdentityVerified: true`, nachdem Mitarbeitende die Kontinuität zwischen beiden
GitHub-Principals unabhängig verifiziert haben. Die Wiederherstellung verweigert sicher, wenn der aktuelle persönliche
...
Direkteigentümerpfade mit der neuen Publisher-Autorität übereinstimmen. Eine aktive Reservierung eines geschützten Handles
für den wiederhergestellten Handle wird ebenfalls dem Ersatzbenutzer zugewiesen, damit eine spätere
...
100 Zeilen pro Anwendungstransaktion begrenzt; größere Wiederherstellungen müssen zuerst eine fortsetzbare Eigentümermigration verwenden.
...
Einen Benutzer sperren und eigene Skills endgültig löschen (nur Moderator/Admin).
...
Einen Benutzer entsperren und berechtigte Skills wiederherstellen (nur Admin).
...
Den gespeicherten Grund für eine bestehende Sperre ändern, ohne die Sperre aufzuheben oder
Inhalte wiederherzustellen (nur Admin). Standardmäßig ein Testlauf, sofern `dryRun` nicht `false` ist.
...
Eine Benutzerrolle ändern (nur Admin).
