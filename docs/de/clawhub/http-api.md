...
- Authentifizierte Schreib-Endpunkte sollten kein bloßes `Unauthorized` zurückgeben, wenn
  ...
  gelöschte/gesperrte/deaktivierte Konten sollten jeweils verwertbaren Text erhalten, damit CLI-
  Clients Nutzern mitteilen können, wodurch sie blockiert wurden.
...
- Wenn `Retry-After` fehlt, greifen Sie auf `RateLimit-Reset` zurück (oder berechnen Sie aus `X-RateLimit-Reset`).
...
- Nehmen Sie die Begriffe, nach denen Nutzer wörtlich suchen werden, in Anzeigename, Zusammenfassung und Tags auf. Verwenden Sie ein eigenständiges Slug-Token nur, wenn es zugleich eine stabile Identität ist, die Sie beibehalten möchten.
...
Melden Sie einen Skill zur Moderatorenprüfung. Meldungen gelten auf Skill-Ebene, sind optional
...
- `version.security` enthält normalisierten Verifizierungsstatus der Scanprüfung und Scanner-Details
...
- Veröffentlichte Scans erfordern Verwaltungszugriff als Besitzer/Publisher oder Moderator-/Administratorberechtigung auf Plattformebene.
...
- Erfordert Verwaltungszugriff als Besitzer/Publisher auf den Skill oder das Plugin oder Moderator-/Administratorberechtigung auf Plattformebene.
...
Nur für Administratoren zugängliche kanonische Route für Batch-Rescans. Sie akzeptiert dieselbe Payload-Struktur wie die Legacy-Route `POST /api/v1/skills/-/rescan-batch`.

### `POST /api/v1/skills/-/scan/batch/status`

Nur für Administratoren zugängliche kanonische Route für den Batch-Status. Sie akzeptiert `{ "jobIds": ["..."] }` und gibt dieselben aggregierten Zähler zurück wie die Legacy-Route `POST /api/v1/skills/-/rescan-batch/status`.
...
- `security.signals.dependencyRegistry` wird für die Kompatibilität der v1-Antwort beibehalten, aber der Scanner für die Existenz in der Abhängigkeits-Registry wurde eingestellt und dieser Schlüssel ist immer `null`.
- `provenance` ist nur dann `server-resolved-github-import`, wenn ClawHub während der Veröffentlichung oder des Imports ein GitHub-Repository/einen GitHub-Ref/Commit/Pfad aufgelöst und gespeichert hat; andernfalls ist der Wert `unavailable`.
...
- `security.signals` enthält nur unterstützende Nachweise auf Statusebene; verwenden Sie `/scan` oder die ClawHub-Seite für das Security Audit für vollständige Scanner-Details.
- `security.signals.dependencyRegistry` wird für die Kompatibilität der v1-Antwort beibehalten, aber der Scanner für die Existenz in der Abhängigkeits-Registry wurde eingestellt und dieser Schlüssel ist immer `null`.
...
- Authentifizierte Aufrufer können private Pakete für Publisher sehen, zu denen sie gehören, in Listen-/Suchergebnissen.
...
Legacy-v1-Filteraliasse werden an Lese-Endpunkten weiterhin akzeptiert:
...
Auf dem vereinheitlichten Endpunkt `/api/v1/packages` ist es nur für Plugins vorgesehen; verwenden Sie
...
- Die unter `GET /api/v1/plugins` dokumentierten Legacy-v1-Filteraliase werden ebenfalls
  akzeptiert.
- Kategoriefilterung ist ein echter API-Filter, der durch Digest-Zeilen der
...
- Sortiersteuerelemente der Browser-UI für die Plugin-Suche ordnen die geladenen Relevanzergebnisse neu,
  entsprechend dem aktuellen Durchsuchen-Verhalten von `/skills`.
...
zurück. Dies ist die öffentliche OpenClaw-Nutzungsoberfläche, um zu entscheiden, ob ein
...
- ClawPack-Versionen streamen die exakten hochgeladenen `.tgz`-Bytes des npm-pack.
...
blockieren sie auch nicht selbst; Moderatoren sollten die Release-Moderation verwenden, um
...
- Private Pakete geben `404` zurück, es sei denn, der Aufrufer kann den owning Publisher lesen.
...
- Diese Route bleibt ZIP-only. Sie streamt keine ClawPack-`.tgz`-Dateien.
...
- Übergaben für GitHub-basierte Skills proxen oder spiegeln keine Bytes. Die JSON-Antwort
...
- Zulässige Formularfelder sind `payload`, wiederholte `files`-Blobs oder eine `clawpack`-
  ...
  Upload-URL-Flow zurückgegeben wurde. Veröffentlichungen mit vorbereiteter Storage-ID müssen außerdem das
...
- JSON-Bodies und vom Aufrufer gelieferte Metadaten in `payload.files` / `payload.artifact`
  werden abgelehnt.
- Direkte Multipart-Veröffentlichungsanfragen sind auf 18 MB begrenzt. ClawPack-Tarballs dürfen
...
Provider-Konto-IDs nennen; veränderliche Handles werden nur als operatorseitiger Schutz verwendet.
...
GitHub-Principals unabhängig verifiziert haben. Die Wiederherstellung bricht sicher ab, wenn der aktuelle persönliche
...
Skill-Slug-Aliasse, Pakete, Paket-Inspector-Warnungen und abgeleitete Suchdigest-Zeilen, sodass
Direct-Owner-Pfade mit der neuen Publisher-Autorität übereinstimmen. Eine aktive Reservierung für einen geschützten Handle
...
- `POST /api/v1/skills/{slug}/transfer/cancel`
  - Antwort (Annahme/Ablehnung/Abbruch): `{ "ok": true, "skillSlug": "demo-skill?" }`
...
Sperrt einen Benutzer und löscht die ihm gehörenden Skills endgültig (nur Moderator/Admin).
...
Inhalte wiederherzustellen (nur Admin). Verwendet standardmäßig einen Dry-Run, sofern `dryRun` nicht `false` ist.
...
die ein ClawPack-Tarball bereitstellen, müssen die resultierende Storage-ID als
