...
- Status von Sicherheitsprüfung und Moderation
...
ClawHub liest das Frontmatter von `SKILL.md`, um Namen,
...
Beim Veröffentlichen wird ein neuer unveränderlicher Versionseintrag erstellt. Publisher verwenden die `clawhub`-CLI für authentifizierte Registry-Workflows:
...
Verwenden Sie Probeläufe, um die aufgelöste Nutzlast vor dem Upload zu prüfen. Öffentliche Seiten zeigen anschließend die veröffentlichten Metadaten, Dateien, Quellenangaben und den Prüfstatus an.
...
oder blockiert werden, können aus der öffentlichen Suche und den Installationsabläufen verschwinden, bleiben dem
Owner jedoch für Diagnosen sichtbar.

Siehe [Security](/clawhub/security), [Security Audits](/clawhub/security-audits),
...
kanonische ClawHub-Listing zurückverlinken, Ratenbegrenzungen einhalten und keine Empfehlung implizieren.
