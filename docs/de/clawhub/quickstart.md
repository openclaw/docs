...
summary: 'Nutzen Sie ClawHub: Skills oder Plugins finden, installieren, aktualisieren und veröffentlichen.'
x-i18n:
    generated_at: "2026-07-03T02:44:55Z"
    model: gpt-5.5
...
Verwenden Sie OpenClaw, wenn Sie Dinge in OpenClaw installieren. Verwenden Sie die `clawhub` CLI, wenn Sie sich anmelden, veröffentlichen, Ihre eigenen Einträge verwalten oder registry-spezifische Workflows verwenden.

## Einen Skill finden und installieren

Suche aus OpenClaw heraus:
...
OpenClaw speichert, woher der Skill stammt, damit spätere Updates weiterhin über ClawHub aufgelöst werden können.
...
Prüfen Sie vor der Veröffentlichung die Metadaten in `SKILL.md`. Deklarieren Sie erforderliche Umgebungsvariablen, Tools und Berechtigungen, damit Benutzer verstehen können, was der Skill benötigt, bevor sie ihn installieren. Siehe [Skill-Format](/de/clawhub/skill-format).

Für Repositories, die mehrere Skills enthalten, ruft der wiederverwendbare GitHub-Workflow `skill publish` für jeden direkten Skill-Ordner unter `skills/` auf:
...
Veröffentlichen Sie ein Plugin aus einem lokalen Ordner, einem GitHub-Repository, einer GitHub-Referenz oder einem bestehenden Archiv:
...
Verwenden Sie zuerst `--dry-run`, um eine Vorschau der aufgelösten Paketmetadaten, Kompatibilitätsfelder, Quellzuordnung und des Upload-Plans anzuzeigen, ohne zu veröffentlichen.
...
Öffentliche Einträge zeigen den neuesten Scan-Status. Releases, die durch Moderation zurückgehalten oder blockiert werden, können in Such- und Installationsoberflächen ausgeblendet sein, bis das Problem behoben ist.
