...
OpenClaw merkt sich, woher der Skill stammt, damit spätere Aktualisierungen weiterhin über ClawHub aufgelöst werden können.
...
Für Repositories mit mehreren Skills ruft der wiederverwendbare GitHub-Workflow `skill publish` für jeden direkten Skill-Ordner unter `skills/` auf:
...
Verwenden Sie zuerst `--dry-run`, um die aufgelösten Paketmetadaten, Kompatibilitätsfelder, Quellenzuordnung und den Upload-Plan vor der Veröffentlichung in der Vorschau zu prüfen.
...
Verwenden Sie vor der Installation die ClawHub-Webseite oder CLI-Detailbefehle, um Metadaten, Quelllinks, Versionen, Changelogs und den Scan-Status zu prüfen:
...
Öffentliche Einträge zeigen den neuesten Scan-Status. Releases, die durch Moderation zurückgehalten oder blockiert werden, können in Such- und Installationsoberflächen ausgeblendet sein, bis sie geklärt sind.
