...
read_when:
    ...
    - Fehlerbehebung bei Veröffentlichungs-/Synchronisierungsfehlern
...
- `.clawhubignore` (Ignoriermuster für Veröffentlichen/Synchronisieren, veraltet: `.clawdhubignore`)
...
Die Sicherheitsanalyse von ClawHub prüft, ob die Deklarationen Ihres Skills zu dem passen, was er tatsächlich tut. Wenn Ihr Code auf `TODOIST_API_KEY` verweist, Ihr Frontmatter dies aber nicht unter `requires.env`, `primaryEnv` oder `envVars` deklariert, meldet die Analyse eine Metadatenabweichung. Genaue Deklarationen helfen Ihrem Skill, die Prüfung zu bestehen, und helfen Benutzern zu verstehen, was sie installieren.
...
- Einbettungstext umfasst `SKILL.md` plus bis zu etwa 40 Nicht-`.md`-Dateien (Best-Effort-Obergrenze).
