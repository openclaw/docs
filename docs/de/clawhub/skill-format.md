...
x-i18n:
    generated_at: "2026-07-04T06:26:45Z"
    model: gpt-5.5
...
Der GitHub-Importer im Web ist strenger als lokale Veröffentlichung/Synchronisierung. Er findet nur
...
Die Sicherheitsanalyse von ClawHub prüft, ob das, was Ihr Skill deklariert, mit dem übereinstimmt, was er tatsächlich tut. Wenn Ihr Code auf `TODOIST_API_KEY` verweist, Ihr Frontmatter diese Variable aber nicht unter `requires.env`, `primaryEnv` oder `envVars` deklariert, meldet die Analyse einen Metadatenkonflikt. Genaue Deklarationen helfen Ihrem Skill, die Prüfung zu bestehen, und helfen Benutzern zu verstehen, was sie installieren.
...
- Inhaltstypen, die mit `text/` beginnen, werden als Text behandelt; zusätzlich gibt es eine kleine Zulassungsliste (JSON/YAML/TOML/JS/TS/Markdown/SVG).

Grenzwerte (serverseitig):

- Gesamtgröße des Bundles: 50 MB.
- Einbettungstext enthält `SKILL.md` + bis zu ca. 40 Nicht-`.md`-Dateien (Obergrenze nach bestem Aufwand).
...
- Tags sind Zeichenfolgen-Zeiger auf eine Version; `latest` wird häufig verwendet.
...
- ClawHub unterstützt keine bezahlten Skills, keine Preise pro Skill, keine Bezahlschranken und keine Umsatzbeteiligung.
