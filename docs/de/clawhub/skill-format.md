...
summary: Format für Skill-Ordner, erforderliche Dateien, zulässige Dateitypen, Limits.
x-i18n:
    generated_at: "2026-07-01T12:53:49Z"
    model: gpt-5.5
...
Die Sicherheitsanalyse von ClawHub prüft, ob die Deklarationen Ihres Skills mit dem übereinstimmen, was er tatsächlich tut. Wenn Ihr Code `TODOIST_API_KEY` referenziert, Ihr Frontmatter es aber nicht unter `requires.env`, `primaryEnv` oder `envVars` deklariert, meldet die Analyse einen Metadatenkonflikt. Präzise Deklarationen helfen Ihrem Skill, die Prüfung zu bestehen, und helfen Benutzern zu verstehen, was sie installieren.
