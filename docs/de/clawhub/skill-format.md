...
Deklarieren Sie optionale Umgebungsvariablen unter `metadata.openclaw.envVars` und setzen Sie `required: false`. Fügen Sie keine optionalen Einträge zu `requires.env` hinzu, da `requires.env` bedeutet, dass der Skill ohne diese Variablen nicht ausgeführt werden kann.
...
- Inhaltstypen, die mit `text/` beginnen, werden als Text behandelt; außerdem gibt es eine kleine Allowlist (JSON/YAML/TOML/JS/TS/Markdown/SVG).
