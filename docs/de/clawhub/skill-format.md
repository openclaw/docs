...
x-i18n:
    generated_at: "2026-07-01T20:15:49Z"
    model: gpt-5.5
...
- `.clawhubignore` (Ignore-Muster für die Veröffentlichung, Legacy `.clawdhubignore`)
...
| `requires.bins`    | `string[]` | CLI-Binaries, die alle installiert sein müssen.                                                                                                                         |
| `requires.anyBins` | `string[]` | CLI-Binaries, von denen mindestens eines vorhanden sein muss.                                                                                                           |
| `requires.config`  | `string[]` | Pfade zu Konfigurationsdateien, die Ihr Skill liest.                                                                                                                    |
| `primaryEnv`       | `string`   | Die Haupt-Credential-Umgebungsvariable für Ihren Skill.                                                                                                                 |
...
| `skillKey`         | `string`   | Überschreibt den Invocation-Key des Skills.                                                                                                                             |
| `emoji`            | `string`   | Anzeige-Emoji für den Skill.                                                                                                                                            |
| `homepage`         | `string`   | URL zur Homepage oder Dokumentation des Skills.                                                                                                                         |
| `os`               | `string[]` | OS-Einschränkungen (z. B. `["macos"]`, `["linux"]`).                                                                                                                    |
...
Wenn Ihr Skill installierte Abhängigkeiten benötigt, deklarieren Sie diese im `install`-Array:
...
Die Sicherheitsanalyse von ClawHub prüft, ob das, was Ihr Skill deklariert, mit dem übereinstimmt, was er tatsächlich tut. Wenn Ihr Code auf `TODOIST_API_KEY` verweist, Ihr Frontmatter dies aber nicht unter `requires.env`, `primaryEnv` oder `envVars` deklariert, markiert die Analyse eine Metadatenabweichung. Genaue Deklarationen helfen Ihrem Skill, die Prüfung zu bestehen, und helfen Benutzern zu verstehen, was sie installieren.
...
- Die Extension-Allowlist befindet sich in `packages/schema/src/textFiles.ts` (`TEXT_FILE_EXTENSIONS`).
...
- Einbettungstext umfasst `SKILL.md` + bis zu ca. 40 Nicht-`.md`-Dateien (Best-Effort-Limit).
...
- ClawHub unterstützt keine kostenpflichtigen Skills, keine Preisgestaltung pro Skill, keine Paywalls und keine Umsatzbeteiligung.
...
- Wenn Ihr Skill in einen kostenpflichtigen Drittanbieterdienst integriert ist, dokumentieren Sie die externen Kosten und das erforderliche Konto deutlich in den Skill-Anweisungen und Umgebungsdeklarationen (`requires.env` für erforderliche Variablen oder `envVars` mit `required: false` für optionale Variablen).
