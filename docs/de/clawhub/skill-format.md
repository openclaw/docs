...
`SKILL.md`- oder Legacy-`skills.md`-Dateien in öffentlichen Nicht-Fork-Repositorys, die dem
...
Workdir-Installationszustand (von der CLI geschrieben):
...
Die Sicherheitsanalyse von ClawHub prüft, ob die Deklarationen Ihres Skills dem entsprechen, was er tatsächlich tut. Wenn Ihr Code auf `TODOIST_API_KEY` verweist, Ihr Frontmatter dies jedoch nicht unter `requires.env`, `primaryEnv` oder `envVars` deklariert, markiert die Analyse eine Metadatenabweichung. Präzise Deklarationen helfen Ihrem Skill, die Prüfung zu bestehen, und helfen Benutzern zu verstehen, was sie installieren.
...
- Tags sind String-Verweise auf eine Version; `latest` wird häufig verwendet.
