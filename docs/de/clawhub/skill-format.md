...
x-i18n:
    generated_at: "2026-07-03T17:18:25Z"
    model: gpt-5.5
...
Skill-Metadaten werden im YAML-Frontmatter oben in Ihrer `SKILL.md` deklariert. Dadurch erfährt die Registry (und die Sicherheitsanalyse), was Ihr Skill zur Ausführung benötigt.
...
Die Sicherheitsanalyse von ClawHub prüft, ob die Deklarationen Ihres Skills mit dem übereinstimmen, was er tatsächlich tut. Wenn Ihr Code auf `TODOIST_API_KEY` verweist, Ihr Frontmatter sie aber nicht unter `requires.env`, `primaryEnv` oder `envVars` deklariert, markiert die Analyse dies als Metadatenabweichung. Genaue Deklarationen helfen Ihrem Skill, die Prüfung zu bestehen, und helfen Benutzern zu verstehen, was sie installieren.
...
- Paket-Scopes müssen exakt mit dem ClawHub-Publisher-Handle übereinstimmen. Publisher-Handles dürfen Kleinbuchstaben, Zahlen, Bindestriche, Punkte und Unterstriche verwenden; sie müssen mit einem Kleinbuchstaben oder einer Zahl beginnen und enden.
- Paket-Slugs müssen kleingeschrieben und npm-kompatibel sein, zum Beispiel `@example.tools/demo-plugin` oder `demo-plugin`.
...
- Fügen Sie in `SKILL.md` keine widersprüchlichen Lizenzbedingungen hinzu; ClawHub unterstützt keine lizenzbezogenen Überschreibungen pro Skill.
