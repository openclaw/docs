...
| `primaryEnv`       | `string`   | Die wichtigste Umgebungsvariable für Zugangsdaten Ihres Skills.                                                                                                   |
...
Die Sicherheitsanalyse von ClawHub prüft, ob das, was Ihr Skill deklariert, mit dem übereinstimmt, was er tatsächlich tut. Wenn Ihr Code `TODOIST_API_KEY` referenziert, Ihr Frontmatter dies aber nicht unter `requires.env`, `primaryEnv` oder `envVars` deklariert, kennzeichnet die Analyse eine Metadatenabweichung. Präzise Deklarationen helfen Ihrem Skill, die Prüfung zu bestehen, und helfen Benutzern zu verstehen, was sie installieren.
...
- Einbettungstext enthält `SKILL.md` + bis zu ca. 40 Nicht-`.md`-Dateien (Best-Effort-Begrenzung).
