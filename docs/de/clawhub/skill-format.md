...
read_when:
    ...
    - Fehler beim Veröffentlichen/Synchronisieren debuggen
summary: Format für Skills-Ordner, erforderliche Dateien, zulässige Dateitypen, Grenzwerte.
x-i18n:
...
Metadaten lokaler Installation (von der CLI geschrieben):
...
Die Sicherheitsanalyse von ClawHub prüft, ob das, was Ihr Skill deklariert, mit dem übereinstimmt, was er tatsächlich tut. Wenn Ihr Code auf `TODOIST_API_KEY` verweist, Ihr Frontmatter dies aber nicht unter `requires.env`, `primaryEnv` oder `envVars` deklariert, meldet die Analyse einen Metadatenkonflikt. Präzise Deklarationen helfen Ihrem Skill, die Prüfung zu bestehen, und helfen Benutzern zu verstehen, was sie installieren.
