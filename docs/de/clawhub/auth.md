...
x-i18n:
    generated_at: "2026-07-02T08:11:17Z"
    model: gpt-5.5
...
ClawHub verwendet GitHub für die Web-Anmeldung. Die CLI verwendet ClawHub-API-Tokens, die
...
Wenn die Anmeldung Sie wieder in einen abgemeldeten Zustand zurückführt, ist Ihr Konto möglicherweise
nicht in gutem Zustand. Wenn Ihr Konto gesperrt oder deaktiviert wurde, verwenden Sie das
...
Proxy-Regeln nicht erreichen kann, verwenden Sie den Headless-Token-Fluss.
...
Verwenden Sie diesen Fluss für Server, CI-Jobs oder reine Terminal-Umgebungen.
...
Sie können API-Tokens in der ClawHub-Web-UI widerrufen.

Widerrufene, ungültige oder fehlende Tokens geben `401 Unauthorized` zurück. Melden Sie sich erneut
mit `clawhub login` an oder stellen Sie ein neues Token mit `clawhub login --token` bereit.

Gelöschte, gesperrte oder deaktivierte Konten können vorhandene API-Tokens nicht weiter verwenden.
