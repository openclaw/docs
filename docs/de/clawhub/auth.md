...
summary: ClawHub-Anmeldung, API-Tokens, CLI-Anmeldung, Tokenspeicherung und Widerruf.
x-i18n:
...
ClawHub verwendet GitHub für die Web-Anmeldung. Die CLI verwendet ClawHub-API-Token, die über dieses angemeldete Konto erstellt wurden.

## Web-Anmeldung

Melden Sie sich auf [clawhub.ai](https://clawhub.ai) mit GitHub an.

Gelöschte, gesperrte oder deaktivierte Konten können die normale ClawHub-Anmeldung nicht abschließen. Wenn Sie nach der Anmeldung wieder in einen abgemeldeten Zustand zurückkehren, ist Ihr Konto möglicherweise nicht in gutem Zustand.
...
Die CLI gibt einen Einmalcode aus und wartet, während Sie ihn unter `https://clawhub.ai/cli/device` autorisieren.
...
Widerrufene, ungültige oder fehlende Token geben `401 Unauthorized` zurück. Melden Sie sich erneut mit `clawhub login` an oder stellen Sie mit `clawhub login --token` ein frisches Token bereit.
