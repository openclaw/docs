...
Gelöschte, gesperrte oder deaktivierte Konten können die normale ClawHub-Anmeldung
nicht abschließen. Wenn Sie nach der Anmeldung wieder in einen abgemeldeten Zustand
zurückkehren, ist Ihr Konto möglicherweise nicht in einwandfreiem Zustand.

## CLI-Anmeldung

Der Standardablauf für die CLI-Anmeldung öffnet Ihren Browser:
...
3. Nach der Anmeldung mit GitHub erstellt ClawHub ein API-Token.
...
Proxy-Regeln nicht erreichen kann, verwenden Sie den Token-Ablauf ohne lokalen Browser.

## Browserlose Anmeldung
...
Für entfernte Shell-Sitzungen, bei denen Sie an anderer Stelle einen Browser öffnen können, führen Sie Folgendes aus:
...
Standardpfade für die Konfiguration:
...
Widerrufene, ungültige oder fehlende Token geben `401 Unauthorized` zurück. Melden Sie sich
erneut mit `clawhub login` an oder stellen Sie mit `clawhub login --token` ein neues Token bereit.
