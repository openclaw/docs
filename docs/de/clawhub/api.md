...
x-i18n:
    generated_at: "2026-07-03T09:31:47Z"
    model: gpt-5.5
...
- Verlinken Sie bei der Anzeige von Einträgen auf die kanonische ClawHub-Skill-URL, damit Benutzer den Registry-Quelleintrag prüfen können.
- Verwenden Sie kanonische Seiten-URLs in der Form `https://clawhub.ai/<owner>/skills/<slug>`.
- Erwecken Sie nicht den Eindruck, dass ClawHub die Drittanbieter-Website empfiehlt, verifiziert oder betreibt.
- Spiegeln Sie keine ausgeblendeten, privaten oder durch Moderation blockierten Inhalte, indem Sie öffentliche API-Filter oder Auth-Grenzen umgehen.
...
- `X-RateLimit-Remaining` / `RateLimit-Remaining`: exaktes verbleibendes Budget, wenn
  vorhanden; geshardete erfolgreiche Anfragen lassen es aus, statt einen ungefähren
...
- `GET /api/v1/download?slug=&version=&tag=`
  ...
  - Aktuelle GitHub-gestützte Skills mit einem `clean`- oder `suspicious`-Scan geben einen
    JSON-`public-github`-Übergabedeskriptor statt ClawHub-Bytes zurück.
...
- `POST /api/v1/users/reserve` reserviert Root-Slugs und private Platzhalter für No-Release-Pakete für ein Owner-Handle.
