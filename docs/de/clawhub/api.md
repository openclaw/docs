...
- Verlinken Sie beim Anzeigen von Einträgen zurück auf die kanonische ClawHub-Skill-URL, damit Nutzer den Quell-Registry-Eintrag prüfen können.
...
- Authentifizierte Anfragen (gültiger Bearer-Token): pro Nutzer-Bucket.
...
- `X-RateLimit-Remaining` / `RateLimit-Remaining`: exakt verbleibendes Budget, wenn
  vorhanden; sharded erfolgreiche Anfragen lassen es weg, statt einen ungefähren
