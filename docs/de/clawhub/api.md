...
x-i18n:
    generated_at: "2026-07-03T00:54:54Z"
    model: gpt-5.5
...
- Verlinken Sie bei der Anzeige von Einträgen zurück auf die kanonische ClawHub-Skill-URL, damit Benutzer den Quell-Registry-Eintrag prüfen können.
...
- `X-RateLimit-Reset`: Unix-Epoch-Sekunden (absolute Zurücksetzzeit)
...
- `X-RateLimit-Remaining` / `RateLimit-Remaining`: exakt verbleibendes Budget, wenn
  vorhanden; erfolgreich verarbeitete sharded Anfragen lassen es aus, statt einen ungefähren
...
- v1-Fehler sind Klartext (`text/plain; charset=utf-8`), einschließlich `400`,
  `401`, `403`, `404`, `429` und blockierter Download-Antworten.
...
- `GET /api/v1/download?slug=&version=&tag=`
  ...
  - Aktuelle GitHub-gestützte Skills mit einem `clean`- oder `suspicious`-Scan geben statt ClawHub-Bytes einen
    JSON-Übergabedeskriptor `public-github` zurück.
...
- `POST /api/v1/users/reserve` reserviert Root-Slugs und private No-Release-Paket-Platzhalter für einen Owner-Handle.
