...
- Zwischenspeichern Sie Antworten und beachten Sie `429`, `Retry-After` und Rate-Limit-Header, statt aggressiv abzufragen.
...
- `X-RateLimit-Remaining` / `RateLimit-Remaining`: exaktes verbleibendes Budget, wenn
  vorhanden; shardierte erfolgreiche Anfragen lassen es weg, statt einen ungefähren
...
- `GET /api/v1/skills?limit=&cursor=&sort=`
  - `sort`: `updated` (Standard), `recommended` (`default`), `createdAt` (`newest`), `downloads`, `stars` (`rating`), Legacy-Installationsaliasse `installsCurrent`/`installs`/`installsAllTime` werden `downloads` zugeordnet, `trending`
...
- `POST /api/v1/users/reserve` reserviert Root-Slugs und private No-Release-Paketplatzhalter für einen Owner-Handle.
