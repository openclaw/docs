...
x-i18n:
    generated_at: "2026-07-12T01:29:14Z"
    model: gpt-5.6
...
- Speichern Sie Antworten zwischen und beachten Sie `429`, `Retry-After` sowie die Header zur Ratenbegrenzung, statt aggressiv abzufragen.
- Verlinken Sie bei der Anzeige von Einträgen auf die kanonische ClawHub-Skill-URL, damit Benutzer den zugrunde liegenden Registrierungseintrag prüfen können.
...
- Bei fehlendem oder ungültigem Token erfolgt die Durchsetzung nach IP-Adresse.

- Lesen: 3000/min pro IP-Adresse, 12000/min pro Schlüssel
- Schreiben: 300/min pro IP-Adresse, 3000/min pro Schlüssel
- Herunterladen: 1200/min pro IP-Adresse, 6000/min pro Schlüssel
...
- `X-RateLimit-Remaining` / `RateLimit-Remaining`: genaues verbleibendes Kontingent, sofern
  vorhanden; verteilte erfolgreiche Anfragen lassen diesen Wert aus, statt einen ungefähren
...
- Bevorzugen Sie `Retry-After`, wenn dieser Header vorhanden ist.
- Verwenden Sie andernfalls `RateLimit-Reset` oder leiten Sie die Verzögerung aus `X-RateLimit-Reset` ab.
- Ergänzen Sie Wiederholungsversuche um eine zufällige Zeitabweichung.

## Fehler

- v1-Fehler werden als Klartext (`text/plain; charset=utf-8`) zurückgegeben, einschließlich `400`,
...
- `GET /api/v1/search?q=...`
  ...
  - Veralteter Alias: `nonSuspicious=true`
- `GET /api/v1/skills?limit=&cursor=&sort=`
  - `sort`: `updated` (Standard), `recommended` (`default`), `createdAt` (`newest`), `downloads`, `stars` (`rating`), die veralteten Installationsaliase `installsCurrent`/`installs`/`installsAllTime` werden `downloads` zugeordnet, `trending`
  - Ungültige `sort`-Werte geben `400` zurück
  - `cursor` gilt für Sortierungen außer `trending`
  - Optionaler Filter: `nonSuspiciousOnly=true`
  - Veralteter Alias: `nonSuspicious=true`
  - Mit `nonSuspiciousOnly=true` können Cursor-basierte Seiten weniger als `limit` Elemente enthalten; verwenden Sie `nextCursor`, um fortzufahren.
...
- `GET /api/v1/download?slug=&version=&tag=`
  ...
  - Aktuelle GitHub-gestützte Skills mit einem Scanergebnis vom Typ `clean` oder `suspicious` geben statt ClawHub-Bytes
    einen JSON-Übergabedeskriptor vom Typ `public-github` zurück.
- `GET /api/v1/skills/export?startDate=&endDate=&limit=&cursor=`
  - Gehostete Skills werden als gespeicherte Dateien exportiert.
  - Aktuelle GitHub-gestützte Skills mit einem Scanergebnis vom Typ `clean` oder `suspicious` werden
    als Übergabedeskriptoren vom Typ `public-github` exportiert.
- `GET /api/v1/packages?limit=&cursor=&sort=`
  - `sort`: `updated` (Standard), `recommended`, `downloads`, veralteter Alias `installs`
  - Ungültige `sort`-Werte geben `400` zurück
- `GET /api/v1/plugins?limit=&cursor=&sort=`
  - `sort`: `recommended` (Standard), `downloads`, `updated`, veralteter Alias `installs`
...
- `POST /api/v1/users/reserve` reserviert Stamm-Slugs und private Paketplatzhalter ohne Veröffentlichung für einen Besitzer-Handle.

## Veraltet

Die veralteten Endpunkte `/api/*` und `/api/cli/*` sind weiterhin verfügbar. Siehe `DEPRECATIONS.md`.
