...
x-i18n:
    generated_at: "2026-07-04T20:28:07Z"
    model: gpt-5.5
...
- `Retry-After`: Verzögerung in Sekunden, die bei `429` abzuwarten ist
...
- `GET /api/v1/skills?limit=&cursor=&sort=`
  - `sort`: `updated` (Standard), `recommended` (`default`), `createdAt` (`newest`), `downloads`, `stars` (`rating`), Legacy-Installationsaliase `installsCurrent`/`installs`/`installsAllTime` werden auf `downloads` abgebildet, `trending`
  ...
  - `recommended` verwendet Signale für Engagement und Aktualität.
...
- `GET /api/v1/download?slug=&version=&tag=`
  ...
  - Aktuelle GitHub-gestützte Skills mit einem `clean`- oder `suspicious`-Scan geben
    statt ClawHub-Bytes einen JSON-`public-github`-Übergabedeskriptor zurück.
