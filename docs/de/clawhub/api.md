...
summary: 'Öffentliche REST-API (v1): Überblick und Konventionen.'
x-i18n:
    generated_at: "2026-07-01T15:22:27Z"
    model: gpt-5.5
...
- Verlinken Sie bei der Anzeige von Einträgen zurück auf die kanonische ClawHub-Skill-URL, damit Benutzer den Quelleintrag in der Registry prüfen können.
...
- `X-RateLimit-Reset`: Unix-Epoch-Sekunden (absolute Rücksetzzeit)
...
- `X-RateLimit-Remaining` / `RateLimit-Remaining`: exakt verbleibendes Budget, wenn
  vorhanden; sharded erfolgreiche Anfragen lassen es aus, statt einen ungefähren
...
- `POST /api/v1/users/reserve` reserviert Root-Slugs und private No-Release-Package-Platzhalter für einen Owner-Handle.
