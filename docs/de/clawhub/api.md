...
x-i18n:
    generated_at: "2026-07-02T00:48:12Z"
    model: gpt-5.5
...
- Cachen Sie Antworten und beachten Sie `429`, `Retry-After` und Rate-Limit-Header, anstatt aggressiv abzufragen.
...
- `X-RateLimit-Remaining` / `RateLimit-Remaining`: exaktes verbleibendes Budget, wenn
  vorhanden; geshardete erfolgreiche Anfragen lassen es weg, anstatt einen ungefähren
...
- `GET /api/v1/skills?limit=&cursor=&sort=`
  ...
  - Legacy-Alias: `nonSuspicious=true`
  - Mit `nonSuspiciousOnly=true` können cursorbasierte Seiten weniger als `limit` Einträge enthalten; verwenden Sie `nextCursor`, um fortzufahren.
...
- `GET /api/v1/download?slug=&version=&tag=`
  ...
  - Aktuelle GitHub-gestützte Skills mit einem `clean`- oder `suspicious`-Scan geben einen
    JSON-`public-github`-Übergabedeskriptor anstelle von ClawHub-Bytes zurück.
...
- `POST /api/v1/users/reserve` reserviert Root-Slugs und private No-Release-Package-Platzhalter für ein Owner-Handle.
