...
Sie können auf ClawHubs öffentlichen Lese-APIs einen Drittanbieter-Katalog, ein Verzeichnis oder eine Suchoberfläche aufbauen. Öffentliche Skill-Metadaten und Skill-Dateien werden unter ClawHubs Skill-Lizenzregeln veröffentlicht, während die API selbst ratenbegrenzt ist und verantwortungsvoll genutzt werden sollte.
...
- Cachen Sie Antworten und beachten Sie `429`, `Retry-After` und Ratenlimit-Header, statt aggressiv abzufragen.
...
- `GET /api/v1/download?slug=&version=&tag=`
  ...
  - Aktuelle GitHub-gestützte Skills mit einem `clean`- oder `suspicious`-Scan geben
    einen JSON-`public-github`-Übergabedeskriptor statt ClawHub-Bytes zurück.
