...
Sie können auf den öffentlichen Lese-APIs von ClawHub einen Drittanbieter-Katalog, ein Verzeichnis oder eine Suchoberfläche aufbauen. Öffentliche Skill-Metadaten und Skill-Dateien werden gemäß den Skill-Lizenzregeln von ClawHub veröffentlicht, während die API selbst ratenbegrenzt ist und verantwortungsvoll genutzt werden sollte.
...
- Cachen Sie Antworten und beachten Sie `429`, `Retry-After` sowie Ratenlimit-Header, statt aggressiv zu pollen.
- Verlinken Sie beim Anzeigen von Einträgen zurück zur kanonischen ClawHub-Skill-URL, damit Benutzer den Registry-Quelldatensatz prüfen können.
...
Durchsetzung mit Berücksichtigung der Authentifizierung:

- Anonyme Anfragen: pro IP.
- Authentifizierte Anfragen (gültiger Bearer-Token): pro Benutzer-Bucket.
- Fehlender/ungültiger Token fällt auf IP-basierte Durchsetzung zurück.
...
- `X-RateLimit-Reset`: Unix-Epochen-Sekunden (absolute Rücksetzzeit)
...
- `X-RateLimit-Remaining` / `RateLimit-Remaining`: exakt verbleibendes Budget, wenn
  vorhanden; bei geshardeten erfolgreichen Anfragen wird es weggelassen, statt einen ungefähren
...
- `GET /api/v1/skills?limit=&cursor=&sort=`
  - `sort`: `updated` (Standard), `recommended` (`default`), `createdAt` (`newest`), `downloads`, `stars` (`rating`), veraltete Installations-Aliasse `installsCurrent`/`installs`/`installsAllTime` werden auf `downloads` abgebildet, `trending`
...
- `POST /api/v1/users/reserve` reserviert Root-Slugs und private Platzhalter für Pakete ohne Release für ein Owner-Handle.

## Veraltet

Veraltete `/api/*` und `/api/cli/*` sind weiterhin verfügbar. Siehe `DEPRECATIONS.md`.
