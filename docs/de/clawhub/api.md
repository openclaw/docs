...
- Cachen Sie Antworten und respektieren Sie `429`, `Retry-After` und Rate-Limit-Header, statt aggressiv zu pollen.
- Verlinken Sie beim Anzeigen von Einträgen auf die kanonische ClawHub-Skill-URL, damit Benutzer den Quell-Registry-Eintrag prüfen können.
...
- Spiegeln Sie keine verborgenen, privaten oder durch Moderation blockierten Inhalte, indem Sie öffentliche API-Filter oder Auth-Grenzen umgehen.
...
- Fehlender/ungültiger Token fällt auf IP-Durchsetzung zurück.
...
- `X-RateLimit-Reset`: Unix-Epoch-Sekunden (absolute Reset-Zeit)
...
- `X-RateLimit-Remaining` / `RateLimit-Remaining`: exakt verbleibendes Budget, wenn
  vorhanden; erfolgreiche sharded Anfragen lassen es weg, statt einen ungefähren
...
- Unbekannte Query-Parameter werden aus Kompatibilitätsgründen ignoriert.
- Bekannte Query-Parameter mit ungültigen Werten geben `400` zurück.
...
- `GET /api/v1/skills?limit=&cursor=&sort=`
  - `sort`: `updated` (Standard), `recommended` (`default`), `createdAt` (`newest`), `downloads`, `stars` (`rating`), Legacy-Installationsaliasse `installsCurrent`/`installs`/`installsAllTime` werden auf `downloads` abgebildet, `trending`
...
Legacy-`/api/*` und `/api/cli/*` weiterhin verfügbar. Siehe `DEPRECATIONS.md`.
