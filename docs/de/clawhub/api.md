...
- Speichern Sie Antworten zwischen und beachten Sie `429`, `Retry-After` und Rate-Limit-Header, anstatt aggressiv abzufragen.
...
- `Retry-After`: Verzögerung in Sekunden, die bei `429` zu warten ist
...
- `POST /api/v1/users/reserve` reserviert Root-Slugs und private Paketplatzhalter ohne Veröffentlichung für ein Owner-Handle.
