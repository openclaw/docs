...
x-i18n:
    generated_at: "2026-07-02T22:26:58Z"
    model: gpt-5.5
...
- Verlinken Sie bei der Anzeige von Einträgen auf die kanonische ClawHub-Skill-URL, damit Benutzer den Quell-Registry-Eintrag prüfen können.
...
- Spiegeln Sie keine ausgeblendeten, privaten oder durch Moderation blockierten Inhalte, indem Sie öffentliche API-Filter oder Authentifizierungsgrenzen umgehen.
...
Authentifizierungsbewusste Durchsetzung:
...
- `X-RateLimit-Reset`: Unix-Epochen-Sekunden (absoluter Zurücksetzzeitpunkt)
...
- v1-Fehler sind reiner Text (`text/plain; charset=utf-8`), einschließlich `400`,
  `401`, `403`, `404`, `429` und Antworten auf blockierte Downloads.
...
- `GET /api/v1/download?slug=&version=&tag=`
  ...
  - Aktuelle GitHub-gestützte Skills mit einem `clean`- oder `suspicious`-Scan geben statt ClawHub-Bytes einen
    JSON-`public-github`-Übergabe-Deskriptor zurück.
- `GET /api/v1/skills/export?startDate=&endDate=&limit=&cursor=`
  ...
  - Aktuelle GitHub-gestützte Skills mit einem `clean`- oder `suspicious`-Scan werden
    als `public-github`-Übergabe-Deskriptoren exportiert.
