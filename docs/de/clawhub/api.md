...
- Zwischenspeichern Sie Antworten und beachten Sie `429`, `Retry-After` und Rate-Limit-Header, statt aggressiv zu pollen.
- Verlinken Sie beim Anzeigen von Einträgen zurück auf die kanonische ClawHub-Skill-URL, damit Benutzer den Quell-Registry-Eintrag prüfen können.
...
- Spiegeln Sie keine versteckten, privaten oder durch Moderation blockierten Inhalte, indem Sie öffentliche API-Filter oder Authentifizierungsgrenzen umgehen.
...
- `X-RateLimit-Reset`: Unix-Epochen-Sekunden (absolute Zurücksetzzeit)
- `RateLimit-Reset`: Verzögerung in Sekunden bis zum Zurücksetzen
- `X-RateLimit-Remaining` / `RateLimit-Remaining`: genaues verbleibendes Budget, wenn
