...
- Zwischenspeichern Sie Antworten und beachten Sie `429`, `Retry-After` und Ratenlimit-Header, statt aggressiv abzufragen.
- Verlinken Sie beim Anzeigen von Einträgen auf die kanonische ClawHub-Skill-URL zurück, damit Benutzer den Quell-Registry-Eintrag prüfen können.
...
- `GET /api/v1/skills?limit=&cursor=&sort=`
  - `sort`: `updated` (Standard), `recommended` (`default`), `createdAt` (`newest`), `downloads`, `stars` (`rating`), Legacy-Installations-Aliase `installsCurrent`/`installs`/`installsAllTime` werden auf `downloads` abgebildet, `trending`
