...
Sie können auf Grundlage der öffentlichen Lese-APIs von ClawHub einen Drittanbieter-Katalog, ein Verzeichnis oder eine Suchoberfläche erstellen. Öffentliche Skills-Metadaten und Skills-Dateien werden gemäß den Skills-Lizenzregeln von ClawHub veröffentlicht, während die API selbst ratenbegrenzt ist und verantwortungsvoll genutzt werden sollte.
...
- Cachen Sie Antworten und beachten Sie `429`, `Retry-After` und Ratenbegrenzungs-Header, statt aggressiv abzufragen.
- Verlinken Sie beim Anzeigen von Einträgen auf die kanonische ClawHub-Skills-URL, damit Benutzer den Quell-Registry-Eintrag prüfen können.
- Verwenden Sie kanonische Seiten-URLs in der Form `https://clawhub.ai/<owner>/skills/<slug>`.
- Erwecken Sie nicht den Eindruck, dass ClawHub die Drittanbieter-Site befürwortet, verifiziert oder betreibt.
...
- `X-RateLimit-Remaining` / `RateLimit-Remaining`: exakt verbleibendes Budget, wenn
  vorhanden; segmentierte erfolgreiche Anfragen lassen es weg, statt einen ungefähren
...
- `POST /api/v1/users/reserve` reserviert Root-Slugs und private Platzhalter für Pakete ohne Release für einen Owner-Handle.
