...
summary: Vom ClawHub-CLI erfasste Installationstelemetrie und wie Sie diese deaktivieren.
x-i18n:
    generated_at: "2026-07-12T01:30:18Z"
    model: gpt-5.6
...
## Wann Telemetriedaten erfasst werden

Telemetriedaten werden nur gesendet, wenn:

- Sie in der CLI angemeldet sind.
- Sie `clawhub install <slug>` ausführen.
- Die Telemetrie **nicht deaktiviert** ist (siehe „So deaktivieren Sie die Telemetrie“ unten).

Wenn Sie nicht angemeldet sind, werden keine Daten übermittelt.

## Welche Daten wir erfassen

Bei jedem gemeldeten `clawhub install` sendet die CLI nach Möglichkeit ein einzelnes Installationsereignis.
