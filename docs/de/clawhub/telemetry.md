...
- Sie in der CLI **angemeldet** sind (für Sync-/Publish-Abläufe verlangen wir ohnehin Authentifizierung).
...
- `rootId`: einen **SHA-256-Hash** des kanonischen Root-Pfads (der Server sieht nie den Rohpfad).
...
- `skillId` (über Slug aufgelöst; nur Skills, die in der Registry existieren, werden erfasst).
- `firstSeenAt`, `lastSeenAt`.
- `lastVersion` (nach bestem Aufwand; derzeit die mit der Registry abgeglichene Version, sofern bekannt).
- optional `removedAt`, wenn eine zuvor gemeldete Installation aus einem Root verschwindet.

### Was wir _nicht_ erfassen

- Keine unverarbeiteten absoluten Ordnerpfade (nur gehashte `rootId` + ein kurzes Anzeige-Label).
- Keine Dateiinhalte.
...
- Enthält eine Aktion **Telemetrie löschen**, um die gesamte gespeicherte Telemetrie für Ihr Konto zu entfernen.
