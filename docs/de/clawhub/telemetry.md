...
x-i18n:
    generated_at: "2026-05-10T19:26:40Z"
    model: gpt-5.5
    provider: openai
...
- Sie in der CLI **angemeldet** sind (wir verlangen Authentifizierung ohnehin bereits für Sync-/Publish-Abläufe).
...
- `rootId`: ein **SHA-256-Hash** des kanonischen Root-Pfads (der Server sieht den unverarbeiteten Pfad nie).
...
- `skillId` (über den Slug aufgelöst; nur Skills, die in der Registry vorhanden sind, werden nachverfolgt).
- `firstSeenAt`, `lastSeenAt`.
- `lastVersion` (bestmöglich; derzeit die zur Registry passende Version, falls bekannt).
...
- Keine Dateiinhalte.
- Keine Pro-Ausführung-Logs, Prompts oder andere CLI-Ausgaben.
...
- `installsCurrent`: eindeutige Benutzer, bei denen der Skill derzeit in mindestens einem aktiven Root installiert ist.
...
- `installsAllTime` verringert sich nie, es sei denn, Sie löschen die Telemetrie (siehe unten).
...
Dies wird lazy ausgewertet (beim nächsten Telemetriebericht), um Hintergrundjobs zu vermeiden.
