...
summary: Installations-Telemetrie, erfasst über `clawhub sync` + Abwahlmöglichkeit.
x-i18n:
...
- Sie in der CLI **angemeldet** sind (wir verlangen bereits Authentifizierung für Sync-/Publish-Abläufe).
...
- `skillId` (per Slug aufgelöst; nur Skills, die in der Registry existieren, werden nachverfolgt).
- `firstSeenAt`, `lastSeenAt`.
- `lastVersion` (nach bestem Aufwand; derzeit die Registry-abgeglichene Version, falls bekannt).
...
- Keine Dateiinhalte.
- Keine Protokolle einzelner Ausführungen, Prompts oder andere CLI-Ausgaben.
...
Wenn Sie aus mehreren Ordnern synchronisieren, behandeln wir jeden Scan-Root unabhängig. Ein Skill gilt als „derzeit installiert“, wenn er in **irgendeinem** aktiven Root existiert.
