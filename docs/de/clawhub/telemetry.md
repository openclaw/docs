...
summary: Installations-Telemetrie, die über `clawhub sync` erfasst wird, plus Opt-out.
x-i18n:
...
Bei jedem `clawhub sync` meldet die CLI einen **vollständigen Schnappschuss** dessen, was sie gefunden hat, gruppiert nach Scan-Root („Ordner/Root“).

Für jede Root speichern wir:

- `rootId`: ein **SHA-256-Hash** des kanonischen Root-Pfads (der Server sieht niemals den Rohpfad).
- `label`: ein menschenlesbares Label, das aus den letzten zwei Pfadsegmenten abgeleitet wird (Home-Pfade werden mit `~` angezeigt).
- `firstSeenAt`, `lastSeenAt`, optional `expiredAt`.

Für jeden unter einer Root gefundenen Skill speichern wir:

- `skillId` (über Slug aufgelöst; nur Skills, die in der Registry vorhanden sind, werden verfolgt).
- `firstSeenAt`, `lastSeenAt`.
- `lastVersion` (nach bestem Ermessen; derzeit die zur Registry passende Version, falls bekannt).
...
- Keine Dateiinhalte.
- Keine Protokolle pro Ausführung, Prompts oder andere CLI-Ausgaben.
- Keine Verfolgung von Skills, die nicht in die Registry hochgeladen wurden (unbekannte Slugs werden ignoriert).
...
Wenn Sie aus mehreren Ordnern synchronisieren, behandeln wir jede Scan-Root unabhängig. Ein Skill ist „derzeit installiert“, wenn er in **irgendeiner** aktiven Root vorhanden ist.
...
- `installsAllTime` verringert sich nie, außer Sie löschen Telemetrie (siehe unten).
...
Wenn diese Variable gesetzt ist, sendet die CLI während `clawhub sync` keine Telemetrie.
