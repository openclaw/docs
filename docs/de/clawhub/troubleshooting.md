...
summary: Fehlerbehebung bei Problemen mit der Anmeldung, Installation, Veröffentlichung und Aktualisierung sowie mit der API von ClawHub.
x-i18n:
    generated_at: "2026-07-12T21:34:43Z"
    model: gpt-5.6
...
- Prüfen Sie die lokalen Firewall-, VPN- und Proxyregeln, wenn der Callback nie eintrifft.
...
Lesen Sie die Informationen zum erneuten Versuch in der Antwort:

- `Retry-After`: Wartezeit in Sekunden vor einem erneuten Versuch.
...
Die CLI berücksichtigt die standardmäßigen Proxyvariablen:
...
## Veröffentlichung schlägt aufgrund fehlender erforderlicher Metadaten fehl
...
müssen Namen mit Gültigkeitsbereich wie `@example-org/example-plugin` unter dem
...
[Problem zur Beanspruchung einer Organisation/eines Namespace](https://github.com/openclaw/clawhub/issues/new?template=org-namespace-claim.yml)
...
Informationen nicht in öffentliche Issues gehören.
...
## `update` verweigert die Ausführung aufgrund lokaler Änderungen

Die lokalen Dateien entsprechen keiner ClawHub bekannten Version. Wählen Sie eine Option:

- Behalten Sie die lokalen Änderungen bei und überspringen Sie das Update.
...
- Wenn das Paket ausgeblendet, zurückgehalten oder gesperrt ist, kann es möglicherweise erst installiert werden, nachdem
  der Eigentümer das Problem behoben hat.

## Öffentliche API-Anfragen schlagen fehl

- Beachten Sie bei `429` die Header für erneute Versuche und speichern Sie öffentliche Listen-/Suchantworten zwischen.
...
Weitere Informationen zu den Endpunkten finden Sie unter [HTTP-API](/clawhub/http-api).
