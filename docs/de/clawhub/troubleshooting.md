...
x-i18n:
    generated_at: "2026-07-04T15:11:39Z"
    model: gpt-5.5
...
- Prüfen Sie lokale Firewall-, VPN- und Proxy-Regeln, wenn der Callback nie eintrifft.
- Erstellen Sie in Headless-Umgebungen ein API-Token in der ClawHub-Web-UI und führen Sie Folgendes aus:
...
- Wenn Sie einen benutzerdefinierten Konfigurationspfad verwenden, stellen Sie sicher, dass `CLAWHUB_CONFIG_PATH` auf die
  Datei zeigt, die Ihr aktuelles Token enthält.
- Wenn Sie ein API-Token verwenden, stellen Sie sicher, dass es in der Web-UI nicht widerrufen wurde.
...
- `RateLimit-Remaining`: Ihr exakt verbleibendes Budget, wenn der Header vorhanden ist. Bei `429` ist es `0`.
- `RateLimit-Reset` oder `X-RateLimit-Reset`: Zeitpunkt des Zurücksetzens.

Wenn viele Benutzer eine gemeinsame Egress-IP verwenden, können anonyme IP-Limits erreicht werden, auch wenn jede
Person nur wenige Anfragen sendet. Melden Sie sich nach Möglichkeit an und versuchen Sie es nach der
gemeldeten Wartezeit erneut.

## Suche oder Installation schlägt hinter einem Proxy fehl

Die CLI beachtet Standard-Proxy-Variablen:
...
- Wenn Sie den Skill besitzen, melden Sie sich an und prüfen Sie ihn:

```bash
clawhub inspect @openclaw/demo
```

Für Owner sichtbare Diagnoseinformationen können den Scan-, Upload-Gate- oder Moderationsstatus erklären.
...
Sehen Sie sich zuerst die Veröffentlichungs-Payload als Vorschau an:
...
Publishern zu verknüpfen.

- Stellen Sie sicher, dass Sie mit dem GitHub-Konto angemeldet sind, das das Paket besitzt oder veröffentlichen darf.
...
Wenn eine Veröffentlichung fehlschlägt, weil Owner-Handle, Organisations-Namespace, Paket-Scope, Skill-
Slug oder Paketname bereits beansprucht oder reserviert ist, stellen Sie zuerst sicher, dass Sie
mit dem Owner veröffentlichen, der zum Namespace passt. Bei Plugin-Paketen müssen
Scoped Names wie `@example-org/example-plugin` als passender
Owner `example-org` veröffentlicht werden.

Wenn Sie glauben, dass Ihre Organisation, Ihr Projekt oder Ihre Marke der rechtmäßige Namespace-Owner ist, Sie
den aktuellen ClawHub-Owner aber nicht verwalten können, öffnen Sie ein
...
[Organisations- und Namespace-Ansprüche](/clawhub/namespace-claims) für Hinweise zu Nachweisen und dazu, was
...
- Stellen Sie sicher, dass Ihre OpenClaw-Version den angegebenen
  Kompatibilitätsbereich des Pakets erfüllt.
- Wenn das Paket ausgeblendet, zurückgehalten oder blockiert ist, ist es möglicherweise nicht installierbar, bis
  der Owner das Problem behoben hat.
