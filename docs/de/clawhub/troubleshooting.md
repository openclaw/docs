...
summary: Fehlerbehebung bei Anmeldung, Installation, Veröffentlichung, Aktualisierung und API-Problemen in ClawHub.
x-i18n:
...
- Prüfen Sie den exakten Slug oder die Besitzerseite, falls Sie sie kennen.
...
Für Besitzer sichtbare Diagnosen können den Scan-, Upload-Gate- oder Moderationsstatus erklären.
...
Zeigen Sie die Veröffentlichungsnutzlast zuerst in der Vorschau an:

```bash
clawhub package publish <source> --family code-plugin --dry-run
```

## Veröffentlichung schlägt mit einem GitHub-Besitzer- oder Quellfehler fehl
...
Wenn eine Veröffentlichung fehlschlägt, weil Besitzer-Handle, Organisations-Namespace, Paket-Scope, Skill-
Slug oder Paketname bereits beansprucht oder reserviert ist, bestätigen Sie zuerst, dass Sie
mit dem Besitzer veröffentlichen, der zum Namespace passt. Bei Plugin-Paketen müssen
Scoped Names wie `@example-org/example-plugin` als passender Besitzer
`example-org` veröffentlicht werden.

Wenn Sie glauben, dass Ihre Organisation, Ihr Projekt oder Ihre Marke der rechtmäßige Namespace-Besitzer ist, Sie
den aktuellen ClawHub-Besitzer aber nicht verwalten können, öffnen Sie ein
...
- Wenn das Paket verborgen, zurückgehalten oder blockiert ist, ist es möglicherweise nicht installierbar, bis
  der Besitzer das Problem behebt.
