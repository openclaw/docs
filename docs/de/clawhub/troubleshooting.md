...
- `RateLimit-Remaining` und `RateLimit-Limit`: Ihr aktuelles Budget.
- `RateLimit-Reset` oder `X-RateLimit-Reset`: Reset-Zeitpunkt.

Wenn viele Benutzer eine einzige Egress-IP gemeinsam nutzen, können anonyme IP-Limits erreicht werden, auch wenn jede
...
- Prüfen Sie den genauen Slug oder die Owner-Seite, wenn Sie sie kennen.
...
Prüfen Sie für Skills die Frontmatter in `SKILL.md`. Erforderliche Umgebungsvariablen und
...
Verweisen Sie auf die Roots, die gescannt werden sollen:
