...
x-i18n:
    generated_at: "2026-07-03T13:26:35Z"
    model: gpt-5.5
...
- Erstellen Sie in Headless-Umgebungen ein API-Token in der ClawHub-Web-UI und führen Sie aus:
...
- Wenn Sie ein API-Token verwenden, bestätigen Sie, dass es in der Web-UI nicht widerrufen wurde.

## Suche oder Installation gibt `Rate limit exceeded` (429) zurück

Lesen Sie die Wiederholungsinformationen in der Antwort:

- `Retry-After`: Sekunden, die vor dem erneuten Versuch gewartet werden sollen.
- `RateLimit-Limit`: das für diese Anfrage angewendete Limit.
...
Wenn viele Benutzer eine ausgehende IP-Adresse gemeinsam nutzen, können anonyme IP-Limits erreicht werden, auch wenn jede
...
- Prüfen Sie den exakten Slug oder die Owner-Seite, falls Sie ihn kennen.
...
Prüfen Sie für Plugins die Kompatibilitätsmetadaten in `package.json`. Veröffentlichungen von
Code-Plugins benötigen OpenClaw-Kompatibilitätsfelder wie `openclaw.compat.pluginApi` und
`openclaw.build.openclawVersion`.

Zeigen Sie zuerst eine Vorschau der Veröffentlichungs-Payload an:
...
ClawHub verwendet GitHub-Identität und Quellzuordnung, um Pakete mit ihren
Publishern zu verbinden.

- Stellen Sie sicher, dass Sie mit dem GitHub-Konto angemeldet sind, dem das
  Paket gehört oder das es veröffentlichen kann.
...
Wenn eine Veröffentlichung fehlschlägt, weil der Owner-Handle, Organisations-Namespace, Paket-Scope, Skill-
Slug oder Paketname bereits beansprucht oder reserviert ist, bestätigen Sie zuerst, dass Sie
mit dem Owner veröffentlichen, der zum Namespace passt. Für Plugin-Pakete
müssen Scoped Names wie `@example-org/example-plugin` als passender
...
[Issue für Organisations-/Namespace-Ansprüche](https://github.com/openclaw/clawhub/issues/new?template=org-namespace-claim.yml)
...
## `sync` sagt, dass keine Skills gefunden wurden

`sync` sucht nach Ordnern, die `SKILL.md` oder `skill.md` enthalten.

Richten Sie es auf die Roots, die Sie scannen möchten:
...
## `update` verweigert die Ausführung wegen lokaler Änderungen
...
Siehe [HTTP-API](/clawhub/http-api) für Endpunktdetails.
