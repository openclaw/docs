...
(z. B. Docker-Container, Hetzner-VPS mit ausschließlich Proxy-Internetzugang, Unternehmens-
Firewalls).
...
- Die Suche bevorzugt exakte Token-Übereinstimmungen bei Slug/Name vor Download-Popularität. Ein eigenständiges Slug-Token wie `map` entspricht `personal-map` stärker als der Teilstring innerhalb von `amap`.
- Downloads sind ein kleiner Popularitäts-Prior, keine Garantie für eine Spitzenplatzierung.
- Wenn ein Skill erscheinen sollte, dies aber nicht tut, führen Sie angemeldet `clawhub inspect <slug>` aus, um für Besitzer sichtbare Moderationsdiagnosen zu prüfen, bevor Sie Metadaten umbenennen.
...
- `--migrate-owner` verschiebt einen vorhandenen Skill zu `--owner`, während eine neue
  Version veröffentlicht wird. Erfordert Admin-/Eigentümerzugriff auf beide Publisher.
- Eigentümer- und Review-Verhalten wird in `docs/publishing.md` erklärt.
- Einen Skill zu veröffentlichen bedeutet, dass er unter `MIT-0` auf ClawHub veröffentlicht wird.
...
- `--clawscan-note <text>` fügt einen ClawScan-Hinweis hinzu. Dieser Hinweis gibt ClawScan
  Kontext für Verhalten, das andernfalls ungewöhnlich wirken könnte, etwa Netzwerkzugriff,
  nativer Host-Zugriff oder Provider-spezifische Anmeldedaten. Der Hinweis wird in
...
- Löscht einen Skill vorläufig (Eigentümer, Moderator oder Admin).
- Ruft `DELETE /api/v1/skills/{slug}` auf.
- Vom Eigentümer veranlasste vorläufige Löschungen reservieren den Slug für 30 Tage; der Befehl gibt die Ablaufzeit aus.
- `--reason <text>` zeichnet einen Moderationshinweis zum Skill und im Audit-Protokoll auf.
- `--note <text>` ist ein Alias für `--reason`.
- `--yes` überspringt die Bestätigung.

### `undelete <slug>`

- Stellt einen verborgenen Skill wieder her (Eigentümer, Moderator oder Admin).
...
- Verbirgt einen Skill (Eigentümer, Moderator oder Admin).
- Alias für `delete`.

### `unhide <slug>`

- Macht einen Skill wieder sichtbar (Eigentümer, Moderator oder Admin).
- Alias für `undelete`.
...
- Übertragungen an Org-/Publisher-Handles werden nur sofort angewendet, wenn der Akteur
  Admin-Zugriff sowohl auf den aktuellen Eigentümer als auch auf den Ziel-Publisher hat.
- Unterbefehle:
...
- Erfordert den Paketinhaber, einen Owner/Admin des Organisations-Publishers, einen Plattformmoderator
...
- Erfordert Admin-Zugriff auf den aktuellen Paketinhaber und den Ziel-
...
- `.tgz`-Quellen werden als ClawPack behandelt. Die CLI lädt exakt die npm-pack-
  ...
  das Vorbefüllen von Metadaten.
- Code-Plugin-Ordner werden vor dem Upload in einen ClawPack-npm-Tarball gepackt, damit
  OpenClaw-Installationen das genaue Artefakt verifizieren können. Bundle-Plugin-Ordner verwenden weiterhin
  den Veröffentlichungsweg über extrahierte Dateien.
- Bei GitHub-Quellen wird die Quellenzuordnung automatisch aus dem Repo, dem aufgelösten Commit, dem Ref und dem Unterpfad befüllt.
...
- Externe Code-Plugins müssen `openclaw.compat.pluginApi` und
  ...
  Das oberste `package.json.version` wird nicht als Fallback für die Veröffentlichungsvalidierung verwendet.
...
- `pull_request` sollte `dry_run: true` verwenden, damit CI keine Veröffentlichungsspuren hinterlässt.
