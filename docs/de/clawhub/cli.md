...
Die CLI berücksichtigt die Standard-Umgebungsvariablen für HTTP-Proxys für Systeme hinter Unternehmens-Proxys oder eingeschränkten Netzwerken:
...
Dies ist auf Systemen erforderlich, bei denen direkte ausgehende Verbindungen blockiert sind (z. B. Docker-Container, Hetzner-VPS mit Proxy-only-Internet, Unternehmens-Firewalls).
...
- Veralteter Fallback: Wenn `clawhub/config.json` noch nicht existiert, aber `clawdhub/config.json` existiert, verwendet die CLI den veralteten Pfad weiter
...
- Standard: öffnet den Browser unter `<site>/cli/auth` und schließt über den loopback-Callback ab.
...
- Die Suche bevorzugt exakte Slug-/Name-Token-Treffer vor Download-Popularität. Ein eigenständiges Slug-Token wie `map` trifft `personal-map` stärker als die Teilzeichenfolge in `amap`.
- Popularität ist ein kleiner Ranking-Prior, keine Garantie für die oberste Platzierung.
...
- Flags:
  ...
  - `--sort newest|updated|rating|downloads|trending` (Standard: newest). Veraltete Installations-Sortieraliasse funktionieren aus Kompatibilitätsgründen weiterhin.
...
- Sendet angemeldet Best-Effort-Telemetrie, damit aktuelle Installationszähler deaktiviert werden können.
...
- Entfernt die Lockfile-Anheftung von einem installierten Skill, sodass zukünftige Updates ihn ändern können.
...
- `update <skill>` schlägt für angeheftete Skills schnell fehl und fordert Sie auf, zuerst `clawhub unpin <skill>` auszuführen.
...
- `--version <version>` wählt ausdrücklich eine Version aus und veröffentlicht auch dann, wenn der Inhalt einer vorhandenen Version entspricht.
- `--dry-run` löst die Veröffentlichung ohne Upload auf; `--json` gibt ein maschinenlesbares Ergebnis aus.
- `--owner <handle>` veröffentlicht unter einem Organisations-/Benutzer-Publisher-Handle, wenn der Akteur Publisher-Zugriff hat.
- `--migrate-owner` verschiebt einen vorhandenen Skill zu `--owner`, während eine neue Version veröffentlicht wird. Erfordert Admin-/Owner-Zugriff auf beide Publisher.
...
- Veröffentlichte Skills können kostenlos verwendet, geändert und ohne Namensnennung weiterverteilt werden.
...
Der wiederverwendbare Workflow [`skill-publish.yml`](https://github.com/openclaw/clawhub/blob/main/.github/workflows/skill-publish.yml) von ClawHub ruft `skill publish` für einen `skill_path` oder für jeden direkten Skill-Ordner unter `root` auf (Standard: `skills`). Er überspringt unveränderte Skills und verwendet dasselbe automatische Patch-Versionsverhalten.
...
- Scannt das aktuelle workdir, das konfigurierte Skills-Verzeichnis und alle `--root <dir>`-Ordner nach lokalen Skill-Ordnern, die `SKILL.md` oder `skill.md` enthalten.
- Vergleicht jeden lokalen Skill-Fingerprint mit ClawHub und veröffentlicht nur neue oder geänderte Skills.
- Neue Skills werden als `1.0.0` veröffentlicht; geänderte Skills veröffentlichen standardmäßig die nächste Patch-Version. Verwenden Sie `--bump minor|major` für Update-Batches, die einen größeren Semver-Schritt machen sollen.
- `--dry-run` zeigt den Veröffentlichungsplan ohne Upload; `--json` gibt einen maschinenlesbaren Plan aus.
- `--all` veröffentlicht jeden neuen oder geänderten Skill ohne Nachfrage. Ohne `--all` können Sie in interaktiven Terminals die zu veröffentlichenden Skills auswählen.
- `--owner <handle>` veröffentlicht unter einem Organisations-/Benutzer-Publisher-Handle, wenn der Akteur Publisher-Zugriff hat.
- `sync` ist ausschließlich eine einseitige Veröffentlichung. Es installiert, aktualisiert oder lädt nichts herunter und meldet keine Installations-/Download-Telemetrie.
...
- Erfordert `clawhub login`.
- Lädt das gespeicherte Scan-Berichts-ZIP für eine eingereichte Skill- oder Plugin-Version herunter, einschließlich Versionen, die durch ClawHub-Sicherheitsprüfungen blockiert oder ausgeblendet wurden.
...
- `--version` ist erforderlich, damit Autoren die genaue eingereichte Version prüfen, die ClawHub blockiert hat.
...
ClawHub liefert einen offiziellen wiederverwendbaren Workflow unter [`/.github/workflows/skill-publish.yml`](https://github.com/openclaw/clawhub/blob/f96ae4a54ec9b72177220d4db601ebc0ddf5a1fd/.github/workflows/skill-publish.yml) für Skill-Repos und Katalog-Repos.
...
- Vom Owner initiierte Soft Deletes reservieren den Slug für 30 Tage; der Befehl gibt den Ablaufzeitpunkt aus.
- `--version <version>` löscht eine eigene, nicht neueste Version dauerhaft über eine fail-closed,
  ...
  aktuell neueste Version löschen. Plattformmitarbeiter umgehen die Ownership für diesen nur versionsbezogenen Ablauf nicht.
...
- Einen eigenen Skill umbenennen und den vorherigen Slug als Weiterleitungsalias beibehalten.
...
- Übertragungen an Organisations-/Publisher-Handles werden nur sofort angewendet, wenn der Akteur
  Adminzugriff sowohl auf den aktuellen Owner als auch auf den Ziel-Publisher hat.
- Unterbefehle:
...
- Verwenden Sie dies für Plugins und andere Einträge aus Paketfamilien; das oberste `search` bleibt die Suchoberfläche für Skills.
- Flags:
...
- Führt den gebündelten Plugin Inspector der ClawHub CLI gegen einen lokalen Plugin-Paketordner
  aus.
- Standardmäßig wird offline/statisch validiert, ohne einen lokalen
...
[Plugin-Validierungskorrekturen](/de/clawhub/plugin-validation-fixes) und führen Sie den Befehl anschließend erneut aus.
...
- `--version <version>` löscht ein eigenes, nicht neuestes Release dauerhaft über eine fail-closed,
  versionsspezifische Route.
  Gelöschte Versionen können nicht wiederhergestellt oder erneut veröffentlicht werden. Veröffentlichen Sie einen Ersatz, bevor Sie die
  aktuell neueste Version löschen. Dieser nur versionsbezogene Ablauf erfordert den Paket-Owner oder einen
  Admin des Organisations-Publishers; Plattformmitarbeiter umgehen die Paket-Ownership nicht.
- Soft Delete eines gesamten Pakets erfordert den Paket-Owner, einen Owner/Admin des Organisations-Publishers, einen Plattformmoderator
...
- Scoped package names müssen an den passenden Scope-Owner übertragen werden.
...
- Meldet Blocker für offiziellen Status, ClawPack-Verfügbarkeit, Artefakt-Digest,
  Source-Provenienz, OpenClaw-Kompatibilität, Host-Ziele, Umgebungsmetadaten
  und Scanstatus.
- Flags:
  - `--json`: maschinenlesbare Ausgabe.
...
- Erstellt einen Organisations-Publisher, dessen Owner der authentifizierte Benutzer ist.
...
- `.tgz`-Quellen werden als ClawPack behandelt. Die CLI lädt die exakten
  ...
  Validierung und Metadaten-Vorausfüllung.
- Code-Plugin-Ordner werden vor dem Upload in einen ClawPack-npm-Tarball
  gepackt, damit OpenClaw-Installationen das exakte Artefakt verifizieren
  können. Bundle-Plugin-Ordner verwenden weiterhin den Veröffentlichungspfad für
  extrahierte Dateien.
- Bei GitHub-Quellen wird die Quellenzuordnung automatisch aus dem Repository, dem aufgelösten Commit, der Ref und dem Unterpfad befüllt.
- Bei lokalen Ordnern wird die Quellenzuordnung automatisch aus dem lokalen Git erkannt, wenn der Origin-Remote auf GitHub zeigt.
...
- `--dry-run` zeigt die aufgelöste Veröffentlichungs-Payload in der Vorschau an, ohne hochzuladen.
...
- Private GitHub-Repositorys benötigen `GITHUB_TOKEN`.
...
- `openclaw.compat.minGatewayVersion` und
  `openclaw.build.pluginSdkVersion` sind optionale Zusätze, wenn Sie
...
- `pull_request` sollte `dry_run: true` verwenden, damit CI nicht verunreinigt wird.
...
- Zeigt die GitHub Actions-Trusted-Publisher-Konfiguration für ein Paket an.
...
- Hängt eine GitHub Actions-Trusted-Publisher-Konfiguration an ein vorhandenes
  Paket an oder ersetzt sie.
- Das Paket muss zuerst über die normale manuelle oder token-authentifizierte
...
- Künftige echte Veröffentlichungen müssen normales authentifiziertes Veröffentlichen verwenden, bis die Konfiguration
  erneut gesetzt ist.
- Flags:
  - `--json`: maschinenlesbare Ausgabe.
