...
- `--registry <url>`: API-Basis-URL (Standard: ermittelt, sonst `https://clawhub.ai`)
...
Unternehmensproxys oder eingeschränkten Netzwerken:
...
- Veralteter Fallback: Wenn `clawhub/config.json` noch nicht existiert, aber `clawdhub/config.json` vorhanden ist, verwendet die CLI den veralteten Pfad erneut
...
- Interaktiv remote/headless: `clawhub login --device` gibt einen Code aus und wartet, während Sie ihn unter `<site>/cli/device` autorisieren.
...
- Die Suche bevorzugt exakte Token-Übereinstimmungen bei Slug/Name vor Download-Popularität. Ein eigenständiges Slug-Token wie `map` passt stärker zu `personal-map` als zur Teilzeichenfolge innerhalb von `amap`.
- Downloads sind ein kleiner Popularitäts-Prior, keine Garantie für eine Spitzenplatzierung.
- Wenn ein Skill erscheinen sollte, dies aber nicht tut, führen Sie im angemeldeten Zustand `clawhub inspect <slug>` aus, um für Owner sichtbare Moderationsdiagnosen zu prüfen, bevor Sie Metadaten umbenennen.
...
- Wenn der Fingerprint mit einer bekannten Version übereinstimmt: keine Nachfrage.
- Wenn der Fingerprint nicht übereinstimmt:
  - wird standardmäßig abgelehnt
  - wird mit `--force` überschrieben (oder nach Nachfrage, falls interaktiv)
- Angepinnte Skills werden nie durch `--force` aktualisiert.
- `update <slug>` schlägt bei angepinnten Slugs sofort fehl und weist Sie an, zuerst `clawhub unpin <slug>` auszuführen.
...
- `--clawscan-note <text>` fügt eine ClawScan-Notiz hinzu. Diese Notiz gibt ClawScan
  ...
  nativer Host-Zugriff oder provider-spezifische Zugangsdaten. Die Notiz wird in
...
- Vom Owner angestoßene Soft Deletes reservieren den Slug für 30 Tage; der Befehl gibt die Ablaufzeit aus.
- `--reason <text>` zeichnet eine Moderationsnotiz am Skill und im Audit-Log auf.
...
- Benennt einen eigenen Skill um und behält den vorherigen Slug als Redirect-Alias.
...
- Der Quell-Slug wird nicht mehr öffentlich gelistet und wird zu einem Redirect-Alias auf das Ziel.
...
- Übertragungen an Benutzer-Handles erzeugen eine ausstehende Anfrage, die der Empfänger annimmt.
- Übertragungen an Org-/Publisher-Handles werden nur dann sofort angewendet, wenn der Actor
...
- Verwenden Sie dies für Plugins und andere Einträge aus der Paketfamilie; das Top-Level-`search` bleibt die Suchoberfläche für Skills.
- Flags:
...
- `--versions`: listet die Versionshistorie auf (erste Seite).
- `--limit <n>`: maximale Anzahl aufzulistender Versionen (1-100).
- `--files`: listet Dateien für die ausgewählte Version auf.
- `--file <path>`: ruft rohen Dateiinhalt ab (nur Textdateien; 200-KB-Limit).
- `--json`: maschinenlesbare Ausgabe.

### `package download <name>`

- Löst eine Paketversion auf über
...
- Erfordert den Paketinhaber, einen Owner/Admin des Organisations-Publishers, einen Plattform-Moderator
...
- Erfordert Admin-Zugriff sowohl auf den aktuellen Paketinhaber als auch auf den Ziel-
...
- Authentifizierter Befehl, um ein Paket an Moderatoren zu melden.
...
#### Ablauf mit lokalem Ordner

Bei Code-Plugins erstellt die Ordnerveröffentlichung ein ClawPack-Artefakt aus
...
- Fügt Clawdbot-Skill-Roots automatisch hinzu, wenn `~/.clawdbot/clawdbot.json` vorhanden ist:
  ...
  - `skills.load.extraDirs` (gemeinsam genutzte Packs)
