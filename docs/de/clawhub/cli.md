...
- Die Suche bevorzugt exakte Token-Übereinstimmungen bei Slug/Name vor Download-Popularität. Ein eigenständiges Slug-Token wie `map` passt stärker zu `personal-map` als zur Teilzeichenfolge in `amap`.
- Downloads sind ein kleiner Popularitäts-Prior, keine Garantie für eine Top-Platzierung.
- Wenn ein Skill erscheinen sollte, aber nicht erscheint, führen Sie angemeldet `clawhub inspect <slug>` aus, um inhabersichtbare Moderationsdiagnosen zu prüfen, bevor Sie Metadaten umbenennen.
...
- `--tag <tag>`: eine Version mit Tag prüfen (z. B. `latest`).
...
- `--file <path>`: rohen Dateiinhalt abrufen (nur Textdateien; 200KB-Limit).
- `--json`: maschinenlesbare Ausgabe.

### `install <slug>`

- Ermittelt die neueste Version über `/api/v1/skills/<slug>`.
...
- Wenn der Fingerprint nicht übereinstimmt:
  ...
  - überschreibt mit `--force` (oder per Eingabeaufforderung, wenn interaktiv)
- Angeheftete Skills werden durch `--force` nie aktualisiert.
- `update <slug>` schlägt bei angehefteten Slugs sofort fehl und fordert Sie auf, zuerst `clawhub unpin <slug>` auszuführen.
...
- `--clawscan-note <text>` fügt eine ClawScan-Notiz hinzu. Diese Notiz gibt ClawScan
  Kontext für Verhalten, das sonst ungewöhnlich wirken könnte, etwa Netzwerkzugriff,
  Zugriff auf den nativen Host oder provider-spezifische Anmeldedaten. Die Notiz wird für
...
- Löscht einen Skill per Soft Delete (Owner, Moderator oder Admin).
...
- Durchsucht den einheitlichen Paketkatalog oder stöbert darin über `GET /api/v1/packages` und `GET /api/v1/packages/search`.
- Verwenden Sie dies für Plugins und andere Einträge aus Paketfamilien; `search` auf oberster Ebene bleibt die Skill-Suchoberfläche.
- Flags:
...
- Führt einen Soft Delete für ein Paket und alle Releases aus.
- Erfordert den Paket-Owner, einen Owner/Admin eines Organisations-Publishers, einen Plattform-Moderator
...
- Stellt ein per Soft Delete gelöschtes Paket und Releases wieder her.
- Erfordert den Paket-Owner, einen Owner/Admin eines Organisations-Publishers, einen Plattform-Moderator
  oder einen Plattform-Admin.
...
- Paketnamen mit Scope müssen an den passenden Scope-Owner übertragen werden.
...
- Zeigt den betreiberorientierten Migrationsstatus für ein Paket an, das ein
...
- Bei lokalen Ordnern wird die Quellenzuordnung automatisch aus lokalem Git erkannt, wenn das Origin-Remote auf GitHub zeigt.
...
- `--dry-run` zeigt eine Vorschau der aufgelösten Veröffentlichungsnutzlast ohne Upload.
...
- `--clawscan-note <text>` fügt eine ClawScan-Notiz hinzu. Diese Notiz gibt ClawScan
  Kontext für Verhalten, das sonst ungewöhnlich wirken kann, etwa Netzwerkzugriff,
  nativer Host-Zugriff oder Provider-spezifische Zugangsdaten. Die Notiz wird im
  veröffentlichten Release gespeichert.
- Paketnamen mit Scope müssen zum ausgewählten Owner passen. Siehe `docs/publishing.md`.
...
Quellenzuordnung bestätigen können, bevor Sie ein Live-Release erstellen:
...
- `openclaw.compat.minGatewayVersion` und
  `openclaw.build.pluginSdkVersion` sind optionale Extras, falls Sie
...
- Pinnen Sie den wiederverwendbaren Workflow an ein stabiles Tag oder einen vollständigen Commit-SHA. Führen Sie Release-Veröffentlichungen nicht von `@main` aus.
- `pull_request` sollte `dry_run: true` verwenden, damit CI keine verändernden Seiteneffekte hat.
...
- Halten Sie `clawhub_token` für die erste Veröffentlichung, nicht vertrauenswürdige Pakete oder Break-Glass-Veröffentlichungen bereit.
- Der Workflow lädt das JSON-Ergebnis als Artefakt hoch und stellt es als Workflow-Ausgaben bereit.

### `sync`

- Sucht nach lokalen Skill-Ordnern und veröffentlicht neue/geänderte Ordner.
...
- Wird während `sync` gesendet, wenn Sie angemeldet sind, außer `CLAWHUB_DISABLE_TELEMETRY=1` (Legacy `CLAWDHUB_DISABLE_TELEMETRY=1`) ist gesetzt.
