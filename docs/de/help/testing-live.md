...
read_when:
    ...
    - Fehlersuche bei der Anmeldedatenauflösung für Live-Tests
...
x-i18n:
    generated_at: "2026-07-12T15:31:34Z"
    model: gpt-5.6
...
Exportieren Sie den erforderlichen Provider-Schlüssel in die Prozessumgebung, bevor Sie Ad-hoc-Live-
...
`voicecall smoke` ist ein Probelauf, sofern nicht zusätzlich `--yes` angegeben ist; verwenden Sie `--yes` nur,
...
## Live: Funktionsumfang-Prüfung für Android-Nodes
...
- Umfang:
  - Vorbereitete/manuelle Einrichtung (die Testsuite installiert, startet oder koppelt die App nicht).
  - Befehlsweise Gateway-Validierung von `node.invoke` für den ausgewählten Android-Node.
- Erforderliche Voreinrichtung:
  - Android-App ist bereits mit dem Gateway verbunden und gekoppelt.
  - App bleibt im Vordergrund.
  - Berechtigungen/Aufzeichnungszustimmung für die erwarteten erfolgreichen Fähigkeiten sind erteilt.
...
- „Gateway-Smoke-Test“ zeigt Ihnen, ob die vollständige Gateway+Agent-Pipeline für dieses Modell funktioniert (Sitzungen, Verlauf, Tools, Sandbox-Richtlinie usw.).
...
- Aktivierung:
  ...
  - Setzen Sie `OPENCLAW_LIVE_MODELS=modern`, `small` oder `all` (Alias für `modern`), um diese Testsuite tatsächlich auszuführen; andernfalls wird sie übersprungen, sodass `pnpm test:live` allein auf den Gateway-Smoke-Test konzentriert bleibt.
- Modellauswahl:
  - `OPENCLAW_LIVE_MODELS=modern` führt die kuratierte, aussagekräftige Prioritätsliste aus (siehe [Live: Modellmatrix](#live-model-matrix-what-we-cover))
  ...
  - oder `OPENCLAW_LIVE_MODELS="openai/gpt-5.6-luna,anthropic/claude-opus-4-6,..."` (durch Kommas getrennte Zulassungsliste)
  ...
  - Durchläufe mit „modern“/„all“ und „small“ verwenden standardmäßig die Länge ihrer kuratierten Liste als Obergrenze; setzen Sie `OPENCLAW_LIVE_MAX_MODELS=0` für einen vollständigen Durchlauf des ausgewählten Profils oder eine positive Zahl für eine niedrigere Obergrenze.
  - Vollständige Durchläufe verwenden `OPENCLAW_LIVE_TEST_TIMEOUT_MS` als Zeitlimit für den gesamten Direktmodelltest. Standard: 60 Minuten.
  - Direktmodellprüfungen werden standardmäßig mit einer Parallelität von 20 ausgeführt; setzen Sie `OPENCLAW_LIVE_MODEL_CONCURRENCY`, um dies zu überschreiben.
- Providerauswahl:
  - `OPENCLAW_LIVE_PROVIDERS="google,google-antigravity,google-gemini-cli"` (durch Kommas getrennte Zulassungsliste)
- Herkunft der Schlüssel:
  - Standardmäßig: Profilspeicher und Umgebungsvariablen als Rückfalloption
...
- Zweck:
  ...
  - Enthält kleine, isolierte Regressionstests (Beispiel: Reasoning-Wiedergabe und Tool-Aufrufabläufe für OpenAI Responses/Codex Responses)
...
- Ziel:
  ...
  - Eine `agent:dev:*`-Sitzung erstellen/aktualisieren (Modellüberschreibung pro Lauf)
...
- Aktivierung:
  - `pnpm test:live` (oder `OPENCLAW_LIVE_TEST=1`, wenn Vitest direkt aufgerufen wird)
- Modellauswahl:
  - Standard: die kuratierte, aussagekräftige Prioritätsliste (`modern`)
  - `OPENCLAW_LIVE_GATEWAY_MODELS=small` führt die kuratierte Liste kleiner Modelle durch die vollständige Gateway+Agent-Pipeline aus
  ...
  - Gateway-Durchläufe mit „modern“/„all“ und „small“ verwenden standardmäßig die Länge ihrer kuratierten Liste als Obergrenze; setzen Sie `OPENCLAW_LIVE_GATEWAY_MAX_MODELS=0` für einen vollständigen ausgewählten Durchlauf oder eine positive Zahl für eine niedrigere Obergrenze.
- Providerauswahl („OpenRouter für alles“ vermeiden):
  - `OPENCLAW_LIVE_GATEWAY_PROVIDERS="google,google-antigravity,google-gemini-cli,openai,anthropic,zai,minimax"` (durch Kommas getrennte Zulassungsliste)
- Tool- und Bildprüfungen sind in diesem Live-Test immer aktiviert:
  - `read`-Prüfung + `exec+read`-Prüfung (Tool-Belastung)
  - Die Bildprüfung wird ausgeführt, wenn das Modell Unterstützung für Bildeingaben angibt
  - Ablauf (auf hoher Ebene):
    - Der Test erzeugt eine kleine PNG-Datei mit „CAT“ + zufälligem Code (`test/helpers/live-image-probe.ts`)
    - Sendet sie über `agent` mit `attachments: [{ mimeType: "image/png", content: "<base64>" }]`
    - Gateway verarbeitet Anhänge zu `images[]` (`src/gateway/server-methods/agent.ts` + `src/gateway/chat-attachments.ts`)
    - Der eingebettete Agent leitet eine multimodale Benutzernachricht an das Modell weiter
    - Prüfung: Die Antwort enthält `cat` + den Code (OCR-Toleranz: kleinere Fehler sind zulässig)

<Tip>
Um zu sehen, was Sie auf Ihrem Rechner testen können (und wie die genauen `provider/model`-IDs lauten), führen Sie Folgendes aus:
...
## Live: Smoke-Test für CLI-Backends (Claude, Gemini oder andere lokale CLIs)

- Test: `src/gateway/gateway-cli-backend.live.test.ts`
- Ziel: Validierung der Gateway- und Agent-Pipeline mit einem lokalen CLI-Backend, ohne Ihre Standardkonfiguration zu verändern.
- Backend-spezifische Smoke-Test-Standardwerte befinden sich in der `cli-backend.ts`-Definition des zuständigen Plugins.
- Aktivierung:
  - `pnpm test:live` (oder `OPENCLAW_LIVE_TEST=1`, wenn Vitest direkt aufgerufen wird)
  - `OPENCLAW_LIVE_CLI_BACKEND=1`
- Standardwerte:
  - Standard-Provider/-Modell: `claude-cli/claude-sonnet-4-6`
  - Das Verhalten für Befehl, Argumente und Bilder wird durch die Metadaten des zuständigen CLI-Backend-Plugins bestimmt.
- Überschreibungen (optional):
  ...
  - `OPENCLAW_LIVE_CLI_BACKEND_IMAGE_ARG="--image"`, um Bilddateipfade als CLI-Argumente zu übergeben, statt sie in den Prompt einzufügen.
  ...
  - `OPENCLAW_LIVE_CLI_BACKEND_RESUME_PROBE=1`, um einen zweiten Durchlauf zu senden und den Fortsetzungsablauf zu validieren.
  - `OPENCLAW_LIVE_CLI_BACKEND_MODEL_SWITCH_PROBE=1`, um den sitzungsinternen Kontinuitätstest für den Wechsel von Claude Sonnet zu Opus zu aktivieren, wenn das ausgewählte Modell ein Wechselziel unterstützt. Standardmäßig deaktiviert, auch in Docker-Rezepten.
  - `OPENCLAW_LIVE_CLI_BACKEND_MCP_PROBE=1`, um den MCP-/Tool-Loopback-Test zu aktivieren. In Docker-Rezepten standardmäßig deaktiviert.
...
Kostengünstiger Smoke-Test für die Gemini-MCP-Konfiguration:
...
Dabei wird Gemini nicht aufgefordert, eine Antwort zu generieren. Der Test schreibt dieselben Systemeinstellungen, die OpenClaw an Gemini übergibt, und führt anschließend `gemini --debug mcp list` aus, um nachzuweisen, dass ein gespeicherter Server mit `transport: "streamable-http"` in die HTTP-MCP-Struktur von Gemini normalisiert wird und eine Verbindung zu einem lokalen streamfähigen HTTP-MCP-Server herstellen kann.
...
- Er führt den Live-Smoke-Test für das CLI-Backend im Docker-Image des Repositories als Nicht-Root-Benutzer `node` aus.
...
- `codex-cli` ist kein gebündeltes CLI-Backend mehr; verwenden Sie stattdessen `openai/*` mit der Codex-App-Server-Runtime (siehe [Live: Smoke-Test für das Codex-App-Server-Testsystem](#live-codex-app-server-harness-smoke)).
- `pnpm test:docker:live-cli-backend:claude-subscription` erfordert portables OAuth für ein Claude-Code-Abonnement, entweder über `~/.claude/.credentials.json` mit `claudeAiOauth.subscriptionType` oder über `CLAUDE_CODE_OAUTH_TOKEN` aus `claude setup-token`. Zunächst wird die direkte Ausführung von `claude -p` in Docker nachgewiesen, danach werden zwei Gateway-CLI-Backend-Durchläufe ausgeführt, ohne die Umgebungsvariablen für den Anthropic-API-Schlüssel beizubehalten. Dieser Abonnement-Testpfad deaktiviert die Claude-MCP-/Tool- und Bildtests standardmäßig, da er die Nutzungslimits des angemeldeten Abonnements beansprucht und Anthropic das Abrechnungs- und Ratenbegrenzungsverhalten des Claude Agent SDK bzw. von `claude -p` ohne eine OpenClaw-Veröffentlichung ändern kann.
- Claude und Gemini unterstützen über die oben genannten Flags dieselben Tests (Textdurchlauf, Bildklassifizierung, Aufruf des MCP-Tools `cron`, Kontinuität beim Modellwechsel), aber keiner dieser Tests wird standardmäßig ausgeführt – aktivieren Sie sie nach Bedarf mit dem jeweiligen Flag.

## Live: Erreichbarkeit des APNs-HTTP/2-Proxys

- Test: `src/infra/push-apns-http2.live.test.ts`
- Ziel: Tunneln über einen lokalen HTTP-CONNECT-Proxy zum Sandbox-APNs-Endpunkt von Apple, Senden der APNs-HTTP/2-Validierungsanfrage und Überprüfen, dass die echte Antwort `403 InvalidProviderToken` von Apple über den Proxy-Pfad zurückkommt.
- Aktivierung:
...
- Ziel: Den tatsächlichen ACP-Ablauf zur Konversationsbindung mit einem aktiven ACP-Agenten validieren:
...
- Hinweise:
  - Diese Lane verwendet die Gateway-Oberfläche `chat.send` mit ausschließlich Administratoren vorbehaltenen synthetischen Feldern für die Ursprungsroute, damit Tests Nachrichtenkanal-Kontext anhängen können, ohne eine externe Zustellung vorzutäuschen.
  ...
  - Die Cron-MCP-Erstellung für gebundene Sitzungen erfolgt standardmäßig nach bestem Bemühen, da externe ACP-Harnesses MCP-Aufrufe abbrechen können, nachdem der Bindungs-/Bildnachweis erfolgreich war; setzen Sie `OPENCLAW_LIVE_ACP_BIND_REQUIRE_CRON=1`, um diese Cron-Prüfung nach der Bindung strikt durchzuführen.
...
- Standardmäßig führt er den ACP-Bindungs-Smoke-Test nacheinander mit den aggregierten aktiven CLI-Agenten aus: `claude`, `codex`, dann `gemini`.
...
- Er stellt das passende CLI-Authentifizierungsmaterial im Container bereit und installiert anschließend bei Bedarf die angeforderte aktive CLI (`@anthropic-ai/claude-code`, `@openai/codex`, Factory Droid über `https://app.factory.ai/cli`, `@google/gemini-cli` oder `opencode-ai`). Das ACP-Backend selbst ist das eingebettete Paket `acpx/runtime` des offiziellen `acpx`-Plugins.
- Die Droid-Docker-Variante stellt `~/.factory` für Einstellungen bereit, leitet `FACTORY_API_KEY` weiter und benötigt diesen API-Schlüssel, da lokale Factory-OAuth-/Schlüsselbund-Authentifizierung nicht in den Container übertragen werden kann. Sie verwendet den integrierten Registrierungseintrag `droid exec --output-format acp` von ACPX.
- Die OpenCode-Docker-Variante ist eine strikte Regressions-Lane für einen einzelnen Agenten. Sie schreibt ein temporäres Standardmodell für `OPENCODE_CONFIG_CONTENT` aus `OPENCLAW_LIVE_ACP_BIND_OPENCODE_MODEL` (Standardwert `opencode/kimi-k2.6`).
- Direkte Aufrufe der `acpx`-CLI sind nur ein manueller Ausweichweg, um das Verhalten außerhalb des Gateway zu vergleichen. Der Docker-ACP-Bindungs-Smoke-Test verwendet das eingebettete `acpx`-Runtime-Backend von OpenClaw.
...
- Ziel: Den Plugin-eigenen Codex-Harness über die normale Gateway-Methode
  ...
  - ein OpenAI-Modell über `/model <ref> --runtime codex` auswählen
  - einen ersten Gateway-Agentendurchlauf mit der angeforderten Denkstufe senden
  - einen zweiten Durchlauf an dieselbe OpenClaw-Sitzung senden und überprüfen, dass der App-Server-
    Thread fortgesetzt werden kann
  - `/codex status` und `/codex models` über denselben Gateway-Befehlspfad
    ausführen
  - optional zwei von Guardian geprüfte Shell-Prüfungen mit erhöhten Rechten ausführen: einen harmlosen
    Befehl, der genehmigt werden sollte, und einen Upload eines fingierten Geheimnisses, der
...
- Der Smoke-Test erzwingt für Provider/Modell `agentRuntime.id: "codex"`, damit ein defekter Codex-
...
- Authentifizierung: Codex-App-Server-Authentifizierung aus der lokalen Codex-Abonnementanmeldung oder
  `OPENAI_API_KEY`, wenn `OPENCLAW_LIVE_CODEX_HARNESS_AUTH=api-key` gesetzt ist. Docker kann für
  Abonnementausführungen `~/.codex/auth.json` und `~/.codex/config.toml` kopieren.
...
die Inferenz-Auswahlnaht des neuen Onboardings auf, prüft `openai/gpt-5.6` und führt anschließend
einen tatsächlichen Gateway-Durchlauf mit diesem aufgelösten Modell aus.
...
- Er übergibt `OPENAI_API_KEY`, kopiert vorhandene Authentifizierungsdateien der Codex-CLI, installiert
  `@openai/codex` in einem schreibbaren, eingebundenen npm-
...
- Docker aktiviert die Bild-, MCP-/Werkzeug- und Guardian-Prüfungen standardmäßig. Setzen Sie
  ...
  Durchlauf benötigen.
- Docker verwendet dieselbe explizite Codex-Runtime-Konfiguration, sodass veraltete Aliase oder ein OpenClaw-
  Fallback eine Regression des Codex-Harnesses nicht verbergen können.
- Matrixziele werden nacheinander in einem Container ausgeführt. Das Docker-Skript skaliert sein
  standardmäßiges Timeout von 35 Minuten anhand der Anzahl der Ziele; jedes äußere Shell- oder CI-Timeout muss
...
- `google-antigravity/...` verwendet die Antigravity-OAuth-Brücke (Agenten-Endpunkt im Stil von Cloud Code Assist).
- `google-gemini-cli/...` verwendet die lokale Gemini-CLI auf Ihrem Rechner (separate Authentifizierung und Besonderheiten bei den Werkzeugen).
- Gemini-API im Vergleich zur Gemini-CLI:
  - API: OpenClaw ruft die von Google gehostete Gemini-API über HTTP auf (API-Schlüssel/Profil-Authentifizierung); dies ist, was die meisten Benutzer mit „Gemini“ meinen.
  - CLI: OpenClaw ruft ein lokales `gemini`-Programm auf; es verfügt über eine eigene Authentifizierung und kann sich anders verhalten (Streaming-/Werkzeugunterstützung/Versionsabweichungen).
...
- Die Provider `codex` und `codex-cli` sind vom standardmäßigen modernen Durchlauf ausgeschlossen (sie decken das Verhalten des CLI-Backends/ACP ab, das oben separat getestet wird). `openai/gpt-5.5` selbst wird standardmäßig über das Codex-App-Server-Testsystem geleitet; siehe [Live: Smoke-Test des Codex-App-Server-Testsystems](#live-codex-app-server-harness-smoke).
...
- Nehmen Sie mindestens ein bildfähiges Modell (Vision-Varianten der Claude-/Gemini-/OpenAI-Familie usw.) in `OPENCLAW_LIVE_GATEWAY_MODELS` auf, um den Bildtest auszuführen.
...
<Tip>
Codieren Sie nicht „alle Modelle“ fest in die Dokumentation. Maßgeblich ist die Liste, die `discoverModels(...)` auf Ihrem Computer zurückgibt, zusammen mit den verfügbaren Schlüsseln.
</Tip>
...
- Wenn ein Live-Test „no creds“ meldet, führen Sie die Fehlersuche genauso durch wie bei `openclaw models list` / der Modellauswahl.

- Agentenspezifische Authentifizierungsprofile: `~/.openclaw/agents/<agentId>/agent/auth-profiles.json` (das ist in den Live-Tests mit „profile keys“ gemeint)
...
Wenn Sie Umgebungsschlüssel verwenden möchten, exportieren Sie diese vor lokalen Tests oder verwenden Sie die
nachstehenden Docker-Runner mit einer expliziten `OPENCLAW_PROFILE_FILE`.
...
- Umfang:
  ...
  - Nützlich nach Änderungen an der Übermittlung von Comfy-Workflows, dem Polling, Downloads oder der Plugin-Registrierung
...
- Umfang:
  - Erfasst jedes registrierte Plugin für Bilderzeugungs-Provider
  - Verwendet bereits exportierte Provider-Umgebungsvariablen vor der Prüfung
  - Verwendet standardmäßig Live-/Umgebungs-API-Schlüssel vor gespeicherten Authentifizierungsprofilen, sodass veraltete Testschlüssel in `auth-profiles.json` echte Shell-Anmeldedaten nicht verdecken
...
- Optionales Authentifizierungsverhalten:
  - `OPENCLAW_LIVE_REQUIRE_PROFILE_KEYS=1`, um die Authentifizierung über den Profilspeicher zu erzwingen und reine Umgebungsüberschreibungen zu ignorieren

Fügen Sie für den ausgelieferten CLI-Pfad einen `infer`-Smoke-Test hinzu, nachdem der Live-Test
für Provider/Laufzeit erfolgreich war:
...
Dies deckt die Verarbeitung der CLI-Argumente, die Auflösung der Konfiguration/des Standardagenten, die Aktivierung
gebündelter Plugins, die gemeinsame Bilderzeugungs-Laufzeit und die Live-Anfrage an den Provider
...
- Umfang:
  - Führt den gemeinsamen Pfad für gebündelte Musikerzeugungs-Provider aus
  - Deckt derzeit `fal`, `google`, `minimax` und `openrouter` ab
  - Verwendet bereits exportierte Provider-Umgebungsvariablen vor der Prüfung
  - Verwendet standardmäßig Live-/Umgebungs-API-Schlüssel vor gespeicherten Authentifizierungsprofilen, sodass veraltete Testschlüssel in `auth-profiles.json` echte Shell-Anmeldedaten nicht verdecken
  - Überspringt Provider ohne verwendbare Authentifizierung/verwendbares Profil/Modell
  - Führt beide deklarierten Laufzeitmodi aus, sofern verfügbar:
    - `generate` mit einer Eingabe, die nur aus einem Prompt besteht
    - `edit`, wenn der Provider `capabilities.edit.enabled` deklariert
  - `comfy` besitzt eine eigene separate Live-Datei und ist nicht Teil dieses gemeinsamen Durchlaufs
- Optionale Eingrenzung:
...
- Umfang:
  - Testet den gemeinsamen gebündelten Pfad für Videoerzeugungs-Provider über `alibaba`, `byteplus`, `deepinfra`, `fal`, `google`, `minimax`, `openai`, `openrouter`, `pixverse`, `qwen`, `runway`, `together`, `vydra`, `xai`
  - Verwendet standardmäßig den releasesicheren Smoke-Test-Pfad: eine Text-zu-Video-Anfrage pro Provider, einen einsekündigen Lobster-Prompt und eine Obergrenze pro Provider-Vorgang aus `OPENCLAW_LIVE_VIDEO_GENERATION_TIMEOUT_MS` (standardmäßig `180000`)
  - Überspringt FAL standardmäßig, da die providerseitige Warteschlangenlatenz die Release-Dauer dominieren kann; übergeben Sie `OPENCLAW_LIVE_VIDEO_GENERATION_PROVIDERS="fal"` (oder leeren Sie die Überspringliste), um ihn ausdrücklich auszuführen
  - Verwendet vor der Erkennung bereits exportierte Provider-Umgebungsvariablen
  - Verwendet standardmäßig Live-/Umgebungs-API-Schlüssel vor gespeicherten Authentifizierungsprofilen, damit veraltete Testschlüssel in `auth-profiles.json` keine echten Shell-Anmeldedaten verdecken
  ...
  - Setzen Sie `OPENCLAW_LIVE_VIDEO_GENERATION_FULL_MODES=1`, um außerdem deklarierte Transformationsmodi auszuführen, sofern verfügbar:
    - `imageToVideo`, wenn der Provider `capabilities.imageToVideo.enabled` deklariert und der ausgewählte Provider bzw. das ausgewählte Modell im gemeinsamen Durchlauf pufferbasierte lokale Bildeingaben akzeptiert
    - `videoToVideo`, wenn der Provider `capabilities.videoToVideo.enabled` deklariert und der ausgewählte Provider bzw. das ausgewählte Modell im gemeinsamen Durchlauf pufferbasierte lokale Videoeingaben akzeptiert
  - Aktuell deklarierter, aber im gemeinsamen Durchlauf übersprungener `imageToVideo`-Provider:
  ...
  - Providerspezifische Vydra-Abdeckung:
    ...
    - Diese Datei führt Text-zu-Video mit `veo3` sowie einen `kling`-Bild-zu-Video-Testpfad aus, der standardmäßig ein Fixture mit einer Remote-Bild-URL verwendet (`OPENCLAW_LIVE_VYDRA_KLING_IMAGE_URL` zum Überschreiben).
  - Providerspezifische xAI-Abdeckung:
    - `OPENCLAW_LIVE_TEST=1 OPENCLAW_LIVE_XAI_VIDEO=1 pnpm test:live -- extensions/xai/xai.live.test.ts -t "classic Grok Imagine"`
    - Der klassische Fall erzeugt ein quadratisches lokales PNG als erstes Einzelbild, lässt Geometrieangaben aus, fordert einen einsekündigen Bild-zu-Video-Clip an, fragt den Status bis zum Abschluss ab und überprüft den heruntergeladenen Puffer.
  ...
  - Aktuell deklarierte, aber im gemeinsamen Durchlauf übersprungene `videoToVideo`-Provider:
    - `alibaba`, `google`, `openai`, `qwen`, `xai`, da diese Pfade derzeit Remote-Referenz-URLs mit `http(s)` anstelle pufferbasierter lokaler Eingaben erfordern
- Optionale Eingrenzung:
  - `OPENCLAW_LIVE_VIDEO_GENERATION_PROVIDERS="deepinfra,google,openai,runway"`
  - `OPENCLAW_LIVE_VIDEO_GENERATION_MODELS="google/veo-3.1-fast-generate-preview,openai/sora-2,runway/gen4_aleph"`
  - `OPENCLAW_LIVE_VIDEO_GENERATION_SKIP_PROVIDERS=""`, um jeden Provider in den standardmäßigen Durchlauf einzubeziehen, einschließlich FAL
  - `OPENCLAW_LIVE_VIDEO_GENERATION_TIMEOUT_MS=60000`, um die Obergrenze für jeden Provider-Vorgang für einen aggressiven Smoke-Test zu reduzieren
- Optionales Authentifizierungsverhalten:
...
- Einstiegspunkt: `test/e2e/qa-lab/media/hosted-media-provider-live.ts`, der pro ausgewählter Suite `pnpm test:live -- <suite-test-file>` ausführt, sodass das Heartbeat- und Ruhemodusverhalten mit anderen `pnpm test:live`-Ausführungen konsistent bleibt.
- Zweck:
...
- Flags:
  - `--providers <csv>` globaler Providerfilter; `--image-providers` / `--music-providers` / `--video-providers` beschränken einen Filter auf eine Suite
