...
read_when:
    ...
    - Sie möchten die iMessage-Integration in einer Sandbox verwenden
...
- **Kleiner Linux-VPS** für einen dauerhaft aktiven Gateway bei geringen Kosten. Siehe [VPS-Hosting](/de/vps).
...
Verwenden Sie eine macOS-VM nur, wenn Sie ausdrücklich Funktionen benötigen, die ausschließlich unter macOS verfügbar sind, etwa iMessage, oder wenn Sie eine strikte Isolierung von Ihrem täglich genutzten Mac wünschen.
...
- Andere Anbieter gehosteter Macs funktionieren ebenfalls; befolgen Sie deren Dokumentation zu VMs und SSH
...
5. Melden Sie sich per SSH an, installieren Sie OpenClaw und konfigurieren Sie Kanäle.
6. Fertig.
...
2. Aktivieren Sie für die Verwendung der VM ohne Anzeige die automatische Anmeldung: System Settings -> Users & Groups, wählen Sie "Automatically log in as:" und anschließend den VM-Benutzer aus.
...
Folgen Sie den Einrichtungsaufforderungen, um Ihren Modell-Provider (Anthropic, OpenAI usw.) einzurichten.
...
3. Gewähren Sie dem Prozess, der OpenClaw/`imsg` ausführt, vollständigen Festplattenzugriff und die Berechtigung zur Automatisierung.
...
Erstellen Sie vor weiteren Anpassungen einen Snapshot Ihres sauberen Zustands:
...
Halten Sie die VM in Betrieb, indem Sie:

- Ihren Mac an die Stromversorgung angeschlossen lassen
...
| Problem                          | Lösung                                                                                                               |
| -------------------------------- | -------------------------------------------------------------------------------------------------------------------- |
| SSH-Verbindung zur VM nicht möglich | Überprüfen Sie, ob "Remote Login" in den System Settings der VM aktiviert ist                                     |
...
| WhatsApp-QR-Code wird nicht gescannt | Stellen Sie sicher, dass Sie bei der Ausführung von `openclaw channels login` bei der VM (nicht beim Host) angemeldet sind |
