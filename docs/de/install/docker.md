...
title: Docker
x-i18n:
    generated_at: "2026-06-27T17:37:48Z"
    model: gpt-5.5
...
- Wenn Sie auf einem VPS/öffentlichen Host ausführen, lesen Sie
  [Security-Härtung für Netzwerkfreigabe](/de/gateway/security),
...
<Steps>
  ...
  <Step title="Airgapped erneut ausführen">
    ...
    implizite Compose-Pulls und Builds und führt dann den normalen Setup-Ablauf aus, etwa
  ...
  <Step title="Onboarding abschließen">
    ...
    - das Verzeichnis für den geheimen Schlüssel des Auth-Profils erstellen
  ...
  <Step title="Control UI öffnen">
    ...
    gemeinsame Geheimnis in den Einstellungen ein. Das Setup-Skript schreibt standardmäßig ein Token nach `.env`;
...
wenn beide Override-Dateien vorhanden sind.
</Note>
...
| `OPENCLAW_SANDBOX`                         | Für Sandbox-Bootstrap aktivieren (`1`, `true`, `yes`, `on`)           |
...
| `OTEL_EXPORTER_OTLP_PROTOCOL`              | OTLP-Protokoll überschreiben. Heute wird nur `http/protobuf` unterstützt |
| `OTEL_SERVICE_NAME`                        | Dienstname für OpenTelemetry-Ressourcen                               |
| `OTEL_SEMCONV_STABILITY_OPT_IN`            | Die neuesten experimentellen semantischen GenAI-Attribute aktivieren  |
...
nur für brew verfügbare Skill-Abhängigkeitsinstaller aus, wenn es in einem Linux-Container
ohne `brew` läuft; diese Abhängigkeiten müssen durch ein benutzerdefiniertes Image bereitgestellt
oder manuell installiert werden. Für Abhängigkeiten, die als Debian-Pakete verfügbar sind, verwenden Sie
`OPENCLAW_IMAGE_APT_PACKAGES` während des Image-Builds. Der alte Name
`OPENCLAW_DOCKER_APT_PACKAGES` wird weiterhin akzeptiert.
Für Python-Abhängigkeiten verwenden Sie `OPENCLAW_IMAGE_PIP_PACKAGES`. Dies führt während des
Image-Builds `python3 -m pip install --break-system-packages` aus; pinnen Sie daher
...
Der OpenTelemetry-Export erfolgt ausgehend vom Gateway-Container zu Ihrem OTLP-Collector.
Er erfordert keinen veröffentlichten Docker-Port. Wenn Sie das Image lokal bauen und
den gebündelten OpenTelemetry-Exporter innerhalb des Images verfügbar haben möchten,
binden Sie dessen Laufzeitabhängigkeiten ein:
...
paketierten Docker-Installationen, bevor Sie den Export aktivieren. Benutzerdefinierte aus dem Quellcode gebaute Images können
...
`diagnostics.otel.enabled=true` oder verwenden Sie das Konfigurationsbeispiel in [OpenTelemetry-
...
öffentlichen `/metrics`-Port und keinen nicht authentifizierten Reverse-Proxy-Pfad. Siehe
...
Das offizielle OpenClaw-Docker-Image installiert Claude Code nicht vorab. Installieren Sie
Claude Code in dem Container-Benutzer, der OpenClaw ausführt, und melden Sie sich dort an. Persistieren Sie anschließend
dieses Container-Home, damit Image-Upgrades weder die Binärdatei noch den Claude-Authentifizierungszustand
löschen.
...
Bei einer bestehenden Docker-Installation stoppen Sie zuerst den Stack und laden die aktuellen
Docker-`.env`-Werte neu, bevor Sie das Setup erneut ausführen. Das Setup-Skript liest
`.env` nicht selbst; es schreibt `.env` aus der aktuellen Shell und den Standardwerten neu. Führen Sie
für die generierte `.env` Folgendes aus:
...
Wenn Ihre `.env` Werte enthält, die Ihre Shell nicht sourcen kann, exportieren Sie zuerst manuell
die vorhandenen Werte erneut, auf die Sie angewiesen sind, etwa `OPENCLAW_IMAGE`, Ports, Bind-Modus,
...
Führen Sie die restlichen Befehle mit dem generierten Compose-Overlay aus, damit beide Dienste
das persistierte Home mounten. Wenn Ihr Setup auch `docker-compose.override.yml` verwendet,
binden Sie es vor `docker-compose.extra.yml` ein.
...
Melden Sie sich an und prüfen Sie den Status aus demselben persistierten Container-Home heraus:
...
Einstellungen und den Authentifizierungszustand unter `/home/node/.claude` und `/home/node/.claude.json`.
...
<Note>
Für gemeinsam genutzte Produktionsautomatisierung oder vorhersehbare Anthropic-Abrechnung sollten Sie den
Anthropic-API-Key-Pfad bevorzugen. Die Wiederverwendung der Claude CLI folgt der installierten
Version von Claude Code, der Kontoanmeldung, der Abrechnung und dem Update-Verhalten.
</Note>
...
`OPENCLAW_DISABLE_BONJOUR=1`, damit der Gateway nicht in einer Crash-Schleife landet oder die Ankündigung
...
Pfade den Austausch von Containern überstehen. Wenn eine Variable nicht gesetzt ist, fällt die gebündelte
`docker-compose.yml` auf Pfade unter `${HOME}` zurück, oder auf `/tmp`, wenn auch `HOME` selbst
fehlt. Dadurch gibt `docker compose up` in Minimalumgebungen keine Volume-Spezifikation
...
Das Secret-Key-Verzeichnis für Authentifizierungsprofile speichert den lokalen Verschlüsselungsschlüssel, der für
OAuth-gestütztes Token-Material von Authentifizierungsprofilen verwendet wird. Bewahren Sie es zusammen mit Ihrem Docker-Host-Zustand auf,
...
überstehen. Der Gateway-Start erzeugt keine Abhängigkeitsbäume für gebündelte Plugins.
...
[Docker-VM-Runtime - Was wo persistiert](/de/install/docker-vm-runtime#what-persists-where).
  ...
  **Hotspots für Festplattenwachstum:** beobachten Sie `media/`, Sitzungs-JSONL-Dateien, die gemeinsame SQLite-Zustandsdatenbank, installierte Plugin-Paket-Roots und rollierende Dateilogs unter `/tmp/openclaw/`.

  ### Shell-Helfer (optional)

  Installieren Sie für einfachere alltägliche Docker-Verwaltung `ClawDock`:
  ...
  Siehe [ClawDock](/de/install/clawdock) für die vollständige Helferanleitung.
  ...
  <Accordion title="Sicherheitshinweis für gemeinsame Netzwerke">
  ...
  <Accordion title="Docker-Desktop-DNS-Fehler in openclaw-cli">
    ...
    `openclaw-cli`-Sidecar fehl, nachdem `NET_RAW` entfernt wurde. Das zeigt sich als
    `EAI_AGAIN` während npm-gestützter Befehle wie `openclaw plugins install`.
    Behalten Sie die standardmäßige gehärtete Compose-Datei für den normalen Gateway-Betrieb bei. Das
    ...
    Befehl, der Zugriff auf die Paketregistrierung benötigt, nicht als Ihren standardmäßigen Compose-
    Aufruf:
    ...
    Linux-Capabilities auf einem bereits erstellten Container nicht ändern.
  ...
  <Accordion title="Berechtigungen und EACCES">
    ...
    Dieselbe Nichtübereinstimmung kann als Plugin-Warnung erscheinen, etwa
    `blocked plugin candidate: suspicious ownership (... uid=1000, expected uid=0 or root)`
    gefolgt von `plugin present but blocked`. Das bedeutet, dass die Prozess-uid und der
    Besitzer des eingebundenen Plugin-Verzeichnisses nicht übereinstimmen. Bevorzugen Sie, den Container mit der
    Standard-uid 1000 auszuführen und die Besitzrechte des Bind-Mounts zu korrigieren. Führen Sie chown nur für
    `/path/to/openclaw-config/npm` auf `root:root` aus, wenn Sie OpenClaw absichtlich
    langfristig als root ausführen.

  </Accordion>

  <Accordion title="Schnellere Rebuilds">
    Ordnen Sie Ihr Dockerfile so an, dass Dependency-Layer gecacht werden. Dadurch wird vermieden, dass
  ...
  <Accordion title="Container-Optionen für Power-User">
    ...
    2. **System-Dependencies einbacken**: `export OPENCLAW_IMAGE_APT_PACKAGES="git curl jq"`
    3. **Python-Dependencies einbacken**: `export OPENCLAW_IMAGE_PIP_PACKAGES="requests==2.32.5 humanize==4.14.0"`
    4. **Playwright Chromium einbacken**: `export OPENCLAW_INSTALL_BROWSER=1`
    5. **Oder Playwright-Browser in ein persistiertes Volume installieren**:
       ```bash
  ...
  <Accordion title="OpenAI Codex OAuth (headless Docker)">
    Wenn Sie OpenAI Codex OAuth im Assistenten auswählen, wird eine Browser-URL geöffnet. In
    Docker- oder Headless-Setups kopieren Sie die vollständige Weiterleitungs-URL, auf der Sie landen, und fügen
    sie wieder in den Assistenten ein, um die Authentifizierung abzuschließen.
  </Accordion>

  <Accordion title="Metadaten des Basis-Images">
    Das Haupt-Image der Docker-Runtime verwendet `node:24-bookworm-slim` und enthält `tini` als Entry-Point-Init-Prozess (PID 1), damit Zombie-Prozesse bereinigt und Signale in langlebigen Containern korrekt verarbeitet werden. Es veröffentlicht OCI-Basis-Image-Annotationen, darunter `org.opencontainers.image.base.name`,
    `org.opencontainers.image.source` und weitere. Der Node-Basis-Digest wird
    über Dependabot-PRs für Docker-Basis-Images aktualisiert; Release-Builds führen
    keine Distro-Upgrade-Schicht aus. Siehe
...
die Tool-Ausführung des Agenten (Shell, Lesen/Schreiben von Dateien usw.) in isolierten Docker-
Containern aus, während der Gateway selbst auf dem Host bleibt. Dadurch erhalten Sie eine harte Abgrenzung
um nicht vertrauenswürdige oder mandantenfähige Agent-Sitzungen, ohne den gesamten
Gateway zu containerisieren.

Der Sandbox-Umfang kann pro Agent (Standard), pro Sitzung oder geteilt sein. Jeder Umfang
...
<AccordionGroup>
  ...
  <Accordion title="Berechtigungsfehler in der Sandbox">
    Setzen Sie `docker.user` auf eine UID:GID, die zur Eigentümerschaft Ihres eingehängten Workspaces passt,
