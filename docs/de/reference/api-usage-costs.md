...
summary: Prüfen Sie, was Kosten verursachen kann, welche Schlüssel verwendet werden und wie Sie die Nutzung einsehen
title: API-Nutzung und Kosten
x-i18n:
    generated_at: "2026-05-06T07:02:16Z"
    model: gpt-5.5
    provider: openai
...
Dieses Dokument listet **Funktionen, die API-Schlüssel verwenden können**, und wo ihre Kosten erscheinen. Es konzentriert sich auf
...
- Wenn Live-Sitzungsmetadaten spärlich sind, kann `/status` Token-/Cache-
  Zähler und die Bezeichnung des aktiven Laufzeitmodells aus dem neuesten Nutzungs-
  Eintrag im Transkript wiederherstellen. Vorhandene von null verschiedene Live-Werte haben weiterhin Vorrang, und promptgroße
...
- `openclaw status --usage` und `openclaw channels list` zeigen **Nutzungsfenster** des Providers
  (Kontingentschnappschüsse, keine Kosten pro Nachricht).
...
- Hinweis zu MiniMax: Die Rohfelder `usage_percent` / `usagePercent` bedeuten verbleibendes
  ...
  fügt den Modellnamen in die Planbezeichnung ein.
- Die Nutzungs-Authentifizierung für diese Kontingentfenster stammt aus Provider-spezifischen Hooks, wenn
...
der lokalen OpenClaw-Oberfläche abrechnen, z. B. **OpenAI Codex**, **Alibaba Cloud Model Studio
...
**Kostenloses Brave Search-Guthaben:** Jeder Brave-Plan enthält \$5/Monat an erneuerbarem
kostenlosem Guthaben. Der Search-Plan kostet \$5 pro 1.000 Anfragen, sodass das Guthaben
1.000 Anfragen/Monat kostenlos abdeckt. Setzen Sie Ihr Nutzungslimit im Brave-Dashboard,
...
Der Compaction-Schutz kann den Sitzungsverlauf mit dem **aktuellen Modell** zusammenfassen, wodurch
Provider-APIs aufgerufen werden, wenn er ausgeführt wird.
