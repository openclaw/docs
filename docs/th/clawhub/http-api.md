...
ไดเรกทอรีของบุคคลที่สามอาจใช้เอนด์พอยต์อ่านสาธารณะเพื่อแสดงรายการหรือค้นหา Skills ของ ClawHub โปรดแคชผลลัพธ์ เคารพ `429`/`Retry-After` ลิงก์ผู้ใช้กลับไปยังรายการ ClawHub ที่เป็นต้นฉบับ (`https://clawhub.ai/<owner>/skills/<slug>`) และหลีกเลี่ยงการสื่อว่า ClawHub รับรองไซต์บุคคลที่สาม อย่าพยายามมิเรอร์เนื้อหาที่ซ่อน เป็นส่วนตัว หรือถูกบล็อกโดยการกลั่นกรองออกนอกพื้นผิว API สาธารณะ
...
- คำขอที่ผ่านการยืนยันตัวตน (โทเค็น Bearer ที่ถูกต้อง): บังคับใช้ต่อ bucket ของผู้ใช้
- หากโทเค็นขาดหาย/ไม่ถูกต้อง พฤติกรรมจะ fallback ไปใช้การบังคับใช้ตาม IP
- เอนด์พอยต์เขียนที่ผ่านการยืนยันตัวตนไม่ควรส่งคืนเพียง `Unauthorized` เมื่อ
...
- `X-RateLimit-Remaining` / `RateLimit-Remaining`: งบประมาณที่เหลือแบบแน่นอนเมื่อมีอยู่
  คำขอที่สำเร็จแบบ sharded จะละเว้นส่วนหัวนี้แทนการส่งคืนค่ารวมทั่วระบบแบบประมาณ
- `Retry-After`: วินาทีที่ต้องรอก่อนลองใหม่ (ดีเลย์) บน `429`
...
- ใช้ส่วนหัว IP ของไคลเอนต์ที่เชื่อถือได้ รวมถึง `cf-connecting-ip` เฉพาะเมื่อ
  การปรับใช้เปิดใช้ส่วนหัว forwarded ที่เชื่อถือได้อย่างชัดเจน
- ClawHub ใช้ส่วนหัว forwarding ที่เชื่อถือได้เพื่อระบุ IP ของไคลเอนต์ที่ edge
- หากไม่มี IP ของไคลเอนต์ที่เชื่อถือได้ คำขอแบบไม่ระบุตัวตนจะใช้ bucket fallback
  ที่ scoped เฉพาะตามชนิดของขีดจำกัดอัตรา bucket fallback เหล่านี้ไม่รวม
...
ซึ่งรวมถึงความล้มเหลวในการตรวจสอบความถูกต้อง (`400`), ทรัพยากรสาธารณะที่ขาดหาย (`404`), ความล้มเหลวด้านการยืนยันตัวตนและ
...
- ผลลัพธ์ถูกส่งคืนตามลำดับความเกี่ยวข้อง (ความคล้ายคลึงของ embedding + การเพิ่มคะแนนโทเค็น slug/ชื่อที่ตรงแบบ exact + ค่าตั้งต้นความนิยมเล็กน้อย)
- ความเกี่ยวข้องมีน้ำหนักมากกว่าความนิยม การตรงกันของ slug หรือโทเค็น display-name ที่แม่นยำอาจอยู่เหนือการตรงกันที่หลวมกว่าแม้มี engagement สูงกว่ามาก
- ข้อความ ASCII จะถูก tokenize ตามขอบเขตคำและเครื่องหมายวรรคตอน ตัวอย่างเช่น `personal-map` มีโทเค็น `map` แบบแยกเดี่ยว ขณะที่ `amap-jsapi-skill` มี `amap`, `jsapi` และ `skill`; ดังนั้นการค้นหา `map` จึงให้ `personal-map` มี lexical match ที่แรงกว่า `amap-jsapi-skill`
- ความนิยมถูกปรับด้วย log-scale และมีเพดาน Skills ที่มี engagement สูงอาจจัดอันดับต่ำกว่าเมื่อข้อความคำค้นตรงกันน้อยกว่า
...
- ใส่คำที่ผู้ใช้จะค้นหาจริง ๆ ในชื่อที่แสดง สรุป และแท็ก ใช้โทเค็น slug แบบแยกเดี่ยวเฉพาะเมื่อมันเป็นอัตลักษณ์ที่เสถียรซึ่งคุณต้องการเก็บไว้ด้วย
- อย่าเปลี่ยนชื่อ slug เพียงเพื่อไล่ตาม query เดียว เว้นแต่ slug ใหม่จะเป็นชื่อ canonical ระยะยาวที่ดีกว่า slug เก่าจะกลายเป็น redirect alias แต่ URL canonical, slug ที่แสดง และ digest การค้นหาในอนาคตจะใช้ slug ใหม่
- Rename aliases รักษาการ resolve สำหรับ URL เก่าและการติดตั้งที่ resolve ผ่านรีจิสทรี แต่การจัดอันดับการค้นหาจะอิงตามเมทาดาทา Skill canonical หลังจากการเปลี่ยนชื่อถูกจัดทำดัชนีแล้ว สถิติที่มีอยู่จะยังอยู่กับ Skill
- หาก Skill หายไปโดยไม่คาดคิด ให้ตรวจสอบสถานะการกลั่นกรองก่อนด้วย `clawhub inspect @owner/slug` ขณะเข้าสู่ระบบ ก่อนเปลี่ยนเมทาดาทาที่เกี่ยวข้องกับการจัดอันดับ
...
- `cursor` (ไม่บังคับ): เคอร์เซอร์ pagination สำหรับ sort ใด ๆ ที่ไม่ใช่ `trending`
...
- เมื่อ `nonSuspiciousOnly=true` การ sort แบบใช้ cursor อาจส่งคืนรายการน้อยกว่า `limit` ในหน้าหนึ่ง เพราะ Skills ที่น่าสงสัยถูกกรองหลังจากดึงหน้าแล้ว
- ใช้ `nextCursor` เพื่อทำ pagination ต่อเมื่อมีอยู่ หน้าสั้นไม่ได้หมายความว่าเป็นจุดสิ้นสุดของผลลัพธ์เสมอไป
...
- slug เก่าที่สร้างโดย flow การเปลี่ยนชื่อ/รวมเจ้าของจะ resolve ไปยัง Skill canonical
...
- หลักฐานจะถูก redact สำหรับผู้เรียกสาธารณะ และรวม snippet ดิบเฉพาะสำหรับเจ้าของ/ผู้กลั่นกรองเท่านั้น

### `POST /api/v1/skills/{slug}/report`

รายงาน Skill ให้ผู้กลั่นกรองตรวจสอบ รายงานอยู่ระดับ Skill, อาจเชื่อมโยงกับ
...
เอนด์พอยต์ผู้กลั่นกรอง/ผู้ดูแลระบบสำหรับรับรายงาน Skill
...
เอนด์พอยต์ผู้กลั่นกรอง/ผู้ดูแลระบบสำหรับแก้ไขหรือเปิดรายงาน Skill อีกครั้ง
...
- `version.security` รวมสถานะการยืนยันการสแกนที่ normalize แล้วและรายละเอียดของสแกนเนอร์
...
- การสแกนรายการที่เผยแพร่แล้วต้องมีสิทธิ์จัดการของเจ้าของ/ผู้เผยแพร่ หรือสิทธิ์ผู้กลั่นกรอง/ผู้ดูแลระบบแพลตฟอร์ม
...
เอนด์พอยต์โพลที่ต้องยืนยันตัวตนสำหรับสแกนที่ส่งแล้ว

- ส่งคืนสถานะ queued/running/succeeded/failed
- ส่งคืน `queue.queuedAhead` และ `queue.position` ขณะอยู่ในคิว เพื่อให้ไคลเอนต์แสดงได้ว่ามีสแกนด้วยตนเองที่ได้รับลำดับความสำคัญกี่รายการอยู่ข้างหน้าคำขอนี้ คิวที่ใหญ่มากจะถูกจำกัดและรายงานด้วย `queuedAheadIsEstimate: true`
...
- ต้องเป็นสแกนที่สำเร็จแล้ว สแกนที่ยังไม่สิ้นสุดจะส่งคืน `409`
...
เอนด์พอยต์คลังรายงานที่เก็บไว้และต้องยืนยันตัวตนสำหรับเวอร์ชันที่ส่งแล้ว

- ต้องมีสิทธิ์จัดการของเจ้าของ/ผู้เผยแพร่สำหรับทักษะหรือ Plugin หรือสิทธิ์ผู้กลั่นกรอง/ผู้ดูแลระบบแพลตฟอร์ม
- ส่งคืนผลสแกนที่เก็บไว้สำหรับเวอร์ชันที่ส่งแบบตรงกันพอดี รวมถึงเวอร์ชันที่ถูกบล็อกหรือซ่อนอยู่
...
เส้นทางสแกนซ้ำแบบแบตช์ตามแบบฉบับสำหรับผู้ดูแลระบบเท่านั้น รับรูปแบบเพย์โหลดเดียวกับ `POST /api/v1/skills/-/rescan-batch` แบบเดิม

### `POST /api/v1/skills/-/scan/batch/status`

เส้นทางสถานะแบตช์ตามแบบฉบับสำหรับผู้ดูแลระบบเท่านั้น รับ `{ "jobIds": ["..."] }` และส่งคืนตัวนับรวมเดียวกับ `POST /api/v1/skills/-/rescan-batch/status` แบบเดิม
...
- อัตลักษณ์ทักษะ อัตลักษณ์ผู้เผยแพร่ และเมทาดาทาเวอร์ชันที่เลือกเป็นฟิลด์ระดับบนสุดของซองข้อมูล (`slug`, `displayName`, `publisherHandle`, `version`, `resolvedFrom`, `tag`, `createdAt`) เพื่อให้ระบบอัตโนมัติของเชลล์อ่านได้โดยไม่ต้องแกะตัวห่อที่ซ้อนอยู่
...
- `security.signals.dependencyRegistry` ถูกเก็บไว้เพื่อความเข้ากันได้ของการตอบกลับ v1 แต่สแกนเนอร์ตรวจสอบการมีอยู่ของรีจิสทรี dependency ถูกเลิกใช้แล้ว และคีย์นี้จะเป็น `null` เสมอ
...
ส่งคืนผลตัดสินความปลอดภัยแบบย่อปัจจุบันสำหรับเวอร์ชันทักษะที่ตรงกันพอดี เอนด์พอยต์คอลเลกชันนี้มีไว้สำหรับไคลเอนต์ที่รู้อยู่แล้วว่าต้องแสดงเวอร์ชันทักษะ ClawHub ที่ติดตั้งใด เช่น OpenClaw Control UI
...
- ผลลัพธ์แยกตามแต่ละรายการ ทักษะหรือเวอร์ชันที่ขาดหายหนึ่งรายการจะไม่ทำให้การตอบกลับทั้งหมดล้มเหลว
...
- ใช้ `/verify` เมื่อต้องการซองข้อมูลการตรวจสอบ Skill Card สำหรับทักษะเดียว ใช้ `/card` เมื่อต้องการ Markdown ของการ์ดที่สร้างแล้ว และใช้ `/scan` เมื่อต้องการข้อมูลสแกนเนอร์แบบละเอียด
...
- รายการ Skills ยังคงอ้างอิงจากรีจิสทรี Skills และยังเผยแพร่ได้เฉพาะผ่าน `POST /api/v1/skills`
- `POST /api/v1/packages` ยังคงใช้เฉพาะสำหรับการปล่อย Plugin แบบโค้ดและ Plugin แบบบันเดิล
- ผู้เรียกแบบไม่ระบุตัวตนจะเห็นเฉพาะช่องทางแพ็กเกจสาธารณะ
- ผู้เรียกที่ยืนยันตัวตนแล้วจะเห็นแพ็กเกจส่วนตัวของผู้เผยแพร่ที่ตนสังกัดอยู่ในผลลัพธ์รายการ/ค้นหา
...
- ผู้เรียกแบบไม่ระบุตัวตนจะเห็นเฉพาะช่องทางแพ็กเกจสาธารณะ
- ผู้เรียกที่ยืนยันตัวตนแล้วจะค้นหาแพ็กเกจส่วนตัวของผู้เผยแพร่ที่ตนสังกัดอยู่ได้
- `channel=private` จะส่งคืนเฉพาะแพ็กเกจที่ผู้เรียกที่ยืนยันตัวตนแล้วอ่านได้

### `GET /api/v1/plugins`

เรียกดูแค็ตตาล็อกเฉพาะ Plugin ทั่วทั้งแพ็กเกจ Plugin แบบโค้ดและ Plugin แบบบันเดิล
...
`trending` เป็นกระดานผู้นำการติดตั้ง/ดาวน์โหลดเจ็ดวัน และไม่ใช้ยอดรวมตลอดกาล
บนปลายทางรวม `/api/v1/packages` จะใช้เฉพาะกับ Plugin เท่านั้น ใช้
...
- Skills ที่โฮสต์ไว้จะรวมไฟล์เวอร์ชันล่าสุดที่จัดเก็บไว้ และแสดงอยู่ใน
  `_manifest.json` พร้อม `sourceRef: "public-clawhub"`
- Skills ปัจจุบันที่อ้างอิง GitHub พร้อมการสแกน `clean` หรือ `suspicious` จะรวม
...
ส่งออก release Plugin สาธารณะล่าสุดแบบกลุ่มสำหรับการวิเคราะห์แบบออฟไลน์
...
- `startDate` (จำเป็น): ขอบเขตล่างแบบ Unix มิลลิวินาทีสำหรับ `updatedAt` ของ Plugin
- `endDate` (จำเป็น): ขอบเขตบนแบบ Unix มิลลิวินาทีสำหรับ `updatedAt` ของ Plugin
- `limit` (ไม่บังคับ): จำนวนเต็ม (1-250), ค่าเริ่มต้น `250`
...
- ยอมรับนามแฝงตัวกรอง v1 เดิมที่บันทึกไว้ใต้ `GET /api/v1/plugins` ด้วย
- การกรองหมวดหมู่เป็นตัวกรอง API จริงที่รองรับโดยแถว digest หมวดหมู่ Plugin
  ไม่ใช่การเขียนคำค้นใหม่
- ผลลัพธ์จะถูกส่งคืนตามลำดับความเกี่ยวข้อง และปัจจุบันยังไม่มีการแบ่งหน้า
- ตัวควบคุมการเรียงลำดับของ UI เบราว์เซอร์สำหรับการค้นหา Plugin จะเรียงผลลัพธ์ความเกี่ยวข้องที่โหลดมาใหม่
  ให้ตรงกับพฤติกรรมเรียกดู `/skills` ในปัจจุบัน
...
การตรวจสอบยืนยัน เมทาดาทา artifact และข้อมูลการสแกน

หมายเหตุ:

- `version.artifact.kind` คือ `legacy-zip` สำหรับ archive แพ็กเกจแบบเดิม หรือ
  `npm-pack` สำหรับ release ที่รองรับโดย ClawPack
- release ของ ClawPack มีฟิลด์ที่เข้ากันได้กับ npm ได้แก่ `npmIntegrity`, `npmShasum` และ
  `npmTarballName`
- `version.sha256hash` เป็นเมทาดาทาความเข้ากันได้ที่เลิกใช้แล้วสำหรับไคลเอนต์เก่า โดย
  ...
  artifact release ตาม canonical
...
ส่งคืนสรุปความปลอดภัยและความน่าเชื่อถือของ release แพ็กเกจที่แน่นอนสำหรับไคลเอนต์ติดตั้ง
นี่คือ surface การใช้งานสาธารณะของ OpenClaw สำหรับตัดสินใจว่า
release ที่ resolve แล้วสามารถติดตั้งได้หรือไม่

การยืนยันตัวตน:

- endpoint สำหรับอ่านแบบสาธารณะ ไม่ต้องใช้ token ของเจ้าของ, publisher, moderator หรือ admin
...
- `trust.scanStatus` คือสถานะความน่าเชื่อถือที่มีผล ซึ่งได้มาจากข้อมูล scanner
  และการ moderation release ด้วยตนเอง
- `trust.moderationState` เป็น nullable โดยจะเป็น `null` เมื่อไม่มีการ
  moderation release ด้วยตนเอง
...
- `trust.stale` หมายถึงสรุปความน่าเชื่อถือถูกคำนวณจากอินพุตที่ล้าสมัย และ
  ควรถูกมองว่าต้อง refresh ก่อนการตัดสินใจอนุญาตที่มีความมั่นใจสูง

หมายเหตุ:

- endpoint นี้ระบุเวอร์ชันอย่างแน่นอน ไคลเอนต์ควรเรียกหลังจาก resolve
...
- เวอร์ชันแพ็กเกจเดิมจะส่งคืน artifact `legacy-zip` และ
  `downloadUrl` ของ ZIP เดิม
- เวอร์ชัน ClawPack จะส่งคืน artifact `npm-pack`, ฟิลด์ npm integrity,
  `tarballUrl` และ URL ความเข้ากันได้กับ ZIP เดิม
- นี่คือ surface resolver ของ OpenClaw โดยหลีกเลี่ยงการเดารูปแบบ archive จาก
  URL ที่ใช้ร่วมกัน
...
- เวอร์ชัน ClawPack จะ stream ไบต์ `.tgz` ของ npm-pack ที่อัปโหลดไว้แบบตรงตัว
- เวอร์ชัน ZIP เดิมจะ redirect ไปยัง `/api/v1/packages/{name}/download?version=`
...
endpoint ของ moderator สำหรับแสดงรายการแถวการ migrate Plugin ทางการของ OpenClaw
...
endpoint ของ admin สำหรับสร้างหรืออัปเดตแถวการ migrate Plugin ทางการ
...
- `packageName` ถูก normalize เป็น npm-name แพ็กเกจอาจไม่มีอยู่สำหรับการ migrate
  ที่วางแผนไว้
- สิ่งนี้ติดตามเฉพาะ readiness ของการ migrate เท่านั้น ไม่ได้ mutate OpenClaw หรือสร้าง
  ClawPacks

### `GET /api/v1/packages/moderation/queue`

endpoint ของ moderator/admin สำหรับคิวตรวจสอบ release แพ็กเกจ
...
- `open`: release ที่น่าสงสัย เป็นอันตราย รอดำเนินการ ถูกกักกัน ถูกเพิกถอน หรือถูกรายงาน
- `blocked`: release ที่ถูกกักกัน ถูกเพิกถอน หรือเป็นอันตราย
- `manual`: release ใด ๆ ที่มีการ override moderation ด้วยตนเอง
- `all`: release ใด ๆ ที่มี manual override, สถานะการสแกนที่ไม่สะอาด หรือรายงานแพ็กเกจ
...
รายงานแพ็กเกจให้ moderator ตรวจสอบ รายงานอยู่ในระดับแพ็กเกจ และสามารถ
เชื่อมโยงกับเวอร์ชันได้ รายงานเหล่านี้ป้อนเข้าสู่คิว moderation แต่จะไม่ซ่อนอัตโนมัติหรือ
...
ปลายทางสำหรับเจ้าของ/moderator เพื่อดูสถานะการ moderation ของแพ็กเกจ
...
ดาวน์โหลดไฟล์ ZIP archive แบบกำหนดซ้ำได้เดิมสำหรับรีลีสแพ็กเกจ
...
- metadata เฉพาะ registry จะไม่ถูกแทรกเข้าไปใน archive ที่ดาวน์โหลด
...
- แสดงเฉพาะเวอร์ชันที่อัปโหลด tarball แบบ ClawPack npm-pack แล้ว
- เวอร์ชันเดิมที่มีเฉพาะ ZIP ถูกละไว้โดยตั้งใจ
- `dist.tarball`, `dist.integrity` และ `dist.shasum` ใช้ฟิลด์ที่เข้ากันได้กับ npm
  เพื่อให้ผู้ใช้ชี้ npm ไปที่ mirror ได้หากเลือกทำเช่นนั้น
- packument ของแพ็กเกจแบบ scoped รองรับทั้ง `/api/npm/@scope/name` และเส้นทางคำขอของ npm
  ที่เข้ารหัสเป็น `/api/npm/@scope%2Fname`

### `GET /api/npm/{package}/-/{tarball}.tgz`

สตรีมไบต์ของ tarball ClawPack ที่อัปโหลดไว้อย่างตรงกันสำหรับไคลเอนต์ npm mirror

หมายเหตุ:

- ใช้บัคเก็ตอัตราการดาวน์โหลด
- ส่วนหัวดาวน์โหลดมี SHA-256 ของ ClawHub พร้อม metadata integrity/shasum ของ npm
- การ moderation และการตรวจสอบการเข้าถึงแพ็กเกจส่วนตัวยังคงมีผล
...
- `slug` (จำเป็น)
- `hash` (จำเป็น): sha256 แบบ hex 64 อักขระของลายนิ้วมือ bundle
...
ดาวน์โหลด ZIP เวอร์ชัน skill ที่โฮสต์ไว้ หรือส่งคืนการส่งต่อแหล่งที่มาจาก GitHub สำหรับ
skill ปัจจุบันที่อิง GitHub ซึ่งมีการสแกน `clean` หรือ `suspicious` และไม่มี
เวอร์ชันที่โฮสต์ไว้
...
- การส่งต่อ skill ที่อิง GitHub จะไม่ proxy หรือ mirror ไบต์ การตอบกลับ JSON
  ...
  และ `archiveUrl`; สถานะการสแกน/ปัจจุบันเป็น gate และไม่รวมอยู่เป็น metadata
  payload ความสำเร็จ
...
- ต้องใช้การยืนยันตัวตนแบบ Bearer token
- ต้องใช้ `multipart/form-data`
- ฟิลด์ฟอร์มที่อนุญาตคือ `payload`, blob `files` ซ้ำหลายรายการ หรือรายการอ้างอิง tarball
  `clawpack` หนึ่งรายการ `clawpack` อาจเป็น blob `.tgz` หรือ storage id ที่ส่งคืนโดย
  flow upload-url การเผยแพร่ storage-id แบบ staged ต้องรวม
...
- body JSON และ metadata `payload.files` / `payload.artifact`
  ที่ผู้เรียกส่งมาเองจะถูกปฏิเสธ
- คำขอเผยแพร่ multipart โดยตรงถูกจำกัดที่ 18MB tarball ClawPack อาจ
  ใช้ flow upload-url ได้สูงสุดตามขีดจำกัด tarball 120MB
- ฟิลด์ payload ที่ไม่บังคับ: `ownerHandle` เมื่อมีอยู่ เฉพาะ admin เท่านั้นที่เผยแพร่ในนามของเจ้าของนั้นได้
...
- การเผยแพร่ในนามผู้อื่นยังตรวจสอบคุณสมบัติของ official-channel กับบัญชีเจ้าของเป้าหมาย
...
เมื่อมีอยู่ `reason` จะถูกเก็บเป็นหมายเหตุการ moderation ของ skill และคัดลอกเข้า audit log
การ soft delete ที่เริ่มโดยเจ้าของจะจอง slug ไว้ 30 วัน จากนั้น slug จะถูกอ้างสิทธิ์โดย
...
สำหรับ admin เท่านั้น ตรวจให้แน่ใจว่ามีผู้เผยแพร่องค์กรสำหรับ handle หาก handle ยังชี้ไปที่
ผู้ใช้ร่วม/ผู้เผยแพร่ส่วนตัวแบบเดิม ปลายทางจะ migrate ให้เป็นผู้เผยแพร่องค์กรก่อน
...
ผู้เรียกเป็นเจ้าของ ปลายทางนี้ไม่ migrate handle ผู้ใช้/ส่วนตัวที่มีอยู่และ
...
สำหรับ admin เท่านั้น จอง root slug และชื่อแพ็กเกจให้เจ้าของที่ชอบธรรมโดยไม่เผยแพร่
รีลีส ชื่อแพ็กเกจจะกลายเป็นแพ็กเกจ placeholder ส่วนตัวที่ไม่มีแถวรีลีส ดังนั้นเจ้าของเดียวกัน
จึงสามารถเผยแพร่รีลีส code-plugin หรือ bundle-plugin จริงลงในชื่อนั้นภายหลังได้
...
สำหรับ admin เท่านั้น กู้คืนผู้เผยแพร่ส่วนตัวสำหรับ principal ของ GitHub OAuth ทดแทนที่ยืนยันแล้ว
โดยไม่แก้ไขแถวบัญชี Convex Auth คำขอต้องระบุทั้ง id บัญชีผู้ให้บริการ GitHub
ที่เปลี่ยนไม่ได้; handle ที่เปลี่ยนได้ใช้เป็น guard สำหรับ operator เท่านั้น

ค่าเริ่มต้นของ endpoint คือ dry-run การใช้การกู้คืนต้องระบุ `dryRun: false` และ
`confirmIdentityVerified: true` หลังจากเจ้าหน้าที่ตรวจสอบความต่อเนื่องระหว่าง
GitHub principal ทั้งสองรายอย่างอิสระแล้ว การกู้คืนจะล้มเหลวแบบปิดเมื่อ personal
publisher ปัจจุบันของผู้ใช้ปลายทางมี skills, packages หรือแหล่งที่มาของ skill บน GitHub
...
ที่ใช้งานอยู่สำหรับ handle ที่กู้คืนจะถูกโอนให้ผู้ใช้ทดแทนด้วย เพื่อให้การซิงโครไนซ์
โปรไฟล์ในภายหลังไม่สามารถกู้คืนอำนาจที่แข่งขันกันของผู้ใช้เดิมได้ แต่ละตารางหลักถูกจำกัดไว้ที่
100 แถวต่อ apply transaction; การกู้คืนที่ใหญ่กว่านี้ต้องใช้ owner migration ที่ทำต่อได้ก่อน
แหล่งที่มาของ skill บน GitHub อยู่ในขอบเขตของ publisher และถูกรายงานว่าตรวจสอบแล้วแทนที่จะเขียนใหม่
...
- endpoint ทั้งสองต้องใช้ API token auth และใช้งานได้เฉพาะเจ้าของ skill เท่านั้น
...
ปลดแบนผู้ใช้และกู้คืน skills ที่มีสิทธิ์ (เฉพาะ admin)
...
เปลี่ยนเหตุผลที่จัดเก็บไว้สำหรับการแบนที่มีอยู่ โดยไม่ปลดแบนหรือกู้คืน
...
เพิ่ม/ลบ star (highlights) endpoint ทั้งสองเป็น idempotent
...
ที่ stage tarball ClawPack ต้องส่ง storage id ที่ได้เป็น
...
หากคุณโฮสต์เอง ให้ serve ไฟล์นี้ (หรือตั้งค่า `CLAWHUB_REGISTRY` อย่างชัดเจน; `CLAWDHUB_REGISTRY` แบบเดิม)
