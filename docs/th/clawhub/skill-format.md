...
- `.clawhubignore` (รูปแบบการละเว้นสำหรับการเผยแพร่/ซิงก์, เดิมคือ `.clawdhubignore`)
- `.gitignore` (มีผลใช้เช่นกัน)
...
เมทาดาทาของ Skill ถูกประกาศใน YAML frontmatter ที่ด้านบนของ `SKILL.md` ของคุณ สิ่งนี้บอก registry (และการวิเคราะห์ความปลอดภัย) ว่า Skill ของคุณต้องการอะไรเพื่อรัน
...
ประกาศข้อกำหนดรันไทม์ของ Skill ของคุณภายใต้ `metadata.openclaw` (ชื่อแทน: `metadata.clawdbot`, `metadata.clawdis`)
...
ใช้ `requires.env` สำหรับตัวแปรสภาพแวดล้อมที่ต้องมีอยู่ก่อนที่ Skill จะรันได้ ใช้ `envVars` เมื่อคุณต้องการเมทาดาทาต่อแต่ละตัวแปร รวมถึงตัวแปรที่ไม่บังคับด้วย `required: false`
...
| `primaryEnv`       | `string`   | ตัวแปรสภาพแวดล้อมสำหรับข้อมูลรับรองหลักของ Skill ของคุณ                                                                                                  |
...
| `install`          | `array`    | ข้อมูลจำเพาะการติดตั้งสำหรับ dependency (ดูด้านล่าง)                                                                                                  |
| `nix`              | `object`   | ข้อมูลจำเพาะ Nix Plugin (ดู README)                                                                                                                |
| `config`           | `object`   | ข้อมูลจำเพาะ config ของ Clawdbot (ดู README)                                                                                                           |

### ข้อมูลจำเพาะการติดตั้ง

หาก Skill ของคุณต้องการติดตั้ง dependency ให้ประกาศในอาร์เรย์ `install`:
...
การเผยแพร่ยอมรับเฉพาะไฟล์แบบ “ข้อความ”
...
- อย่าเพิ่มเงื่อนไขใบอนุญาตที่ขัดแย้งใน `SKILL.md`; ClawHub ไม่รองรับการแทนที่ใบอนุญาตแบบราย Skill

## Skill แบบชำระเงิน

- ClawHub ไม่รองรับ Skill แบบชำระเงิน การกำหนดราคาราย Skill paywall หรือการแบ่งรายได้
- อย่าเพิ่มเมทาดาทาราคาใน `SKILL.md`; มันไม่ใช่ส่วนหนึ่งของรูปแบบ Skill และจะไม่ทำให้ Skill ที่เผยแพร่แล้วกลายเป็นแบบชำระเงิน
- หาก Skill ของคุณผสานรวมกับบริการบุคคลที่สามแบบชำระเงิน ให้บันทึกค่าใช้จ่ายภายนอกและบัญชีที่จำเป็นอย่างชัดเจนในคำแนะนำของ Skill และการประกาศ env (`requires.env` สำหรับตัวแปรที่จำเป็น หรือ `envVars` พร้อม `required: false` สำหรับตัวแปรที่ไม่บังคับ)
