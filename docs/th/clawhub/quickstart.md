...
x-i18n:
    generated_at: "2026-07-02T01:19:58Z"
    model: gpt-5.5
...
OpenClaw บันทึกว่า skill มาจากที่ใด เพื่อให้การอัปเดตภายหลังยังคงสามารถ
...
ติดตั้ง plugin ที่โฮสต์บน ClawHub พร้อมแหล่งที่มาของ ClawHub อย่างชัดเจน:
...
สภาพแวดล้อมแบบ headless สามารถใช้โทเค็น API จาก UI เว็บของ ClawHub:
...
skill คือโฟลเดอร์ที่มีไฟล์ `SKILL.md` ที่จำเป็น และไฟล์สนับสนุนเสริม
...
ก่อนเผยแพร่ ให้ตรวจสอบ metadata ใน `SKILL.md` ประกาศตัวแปร
สภาพแวดล้อม เครื่องมือ และ permissions ที่จำเป็น เพื่อให้ผู้ใช้เข้าใจว่า
skill ต้องใช้อะไรก่อนติดตั้ง ดู [รูปแบบ skill](/th/clawhub/skill-format)

สำหรับ repositories ที่มีหลาย skills, reusable GitHub workflow จะเรียก
`skill publish` สำหรับแต่ละโฟลเดอร์ skill ระดับแรกใต้ `skills/`:
...
เผยแพร่ plugin จากโฟลเดอร์ในเครื่อง, repo GitHub, ref GitHub หรือ
archive ที่มีอยู่:
...
ใช้ `--dry-run` ก่อนเพื่อดูตัวอย่าง metadata ของ package ที่ resolve แล้ว, fields ความเข้ากันได้,
...
ก่อนติดตั้ง ให้ใช้หน้าเว็บ ClawHub หรือคำสั่ง CLI สำหรับรายละเอียดเพื่อตรวจสอบ
...
รายการสาธารณะแสดงสถานะการสแกนล่าสุด releases ที่ถูกระงับหรือถูกบล็อกโดย
