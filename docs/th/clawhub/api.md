...
คุณสามารถสร้างแค็ตตาล็อก ไดเรกทอรี หรือพื้นผิวการค้นหาของบุคคลที่สามบน public read APIs ของ ClawHub ได้ เมทาดาทา Skills สาธารณะและไฟล์ Skills จะเผยแพร่ภายใต้กฎไลเซนส์ Skills ของ ClawHub ส่วน API เองมีการจำกัดอัตราการใช้งานและควรใช้อย่างรับผิดชอบ

แนวทาง:

- ใช้เอนด์พอยต์อ่านสาธารณะ เช่น `GET /api/v1/skills`, `GET /api/v1/search` และ `GET /api/v1/skills/{slug}` สำหรับรายการแค็ตตาล็อก
- แคชการตอบกลับและเคารพ `429`, `Retry-After` และส่วนหัวจำกัดอัตรา แทนการโพลอย่างหนัก
- ลิงก์กลับไปยัง URL Skills ของ ClawHub ที่เป็นแหล่งหลักเมื่อแสดงรายการ เพื่อให้ผู้ใช้ตรวจสอบระเบียนรีจิสทรีต้นทางได้
- ใช้ URL หน้าหลักในรูปแบบ `https://clawhub.ai/<owner>/<slug>`
...
- การอ่าน: 600/นาที ต่อ IP, 2400/นาที ต่อคีย์
- การเขียน: 45/นาที ต่อ IP, 180/นาที ต่อคีย์
...
- `GET /api/v1/skills?limit=&cursor=&sort=`
  ...
  - อลิแอสเดิม: `nonSuspicious=true`
  - เมื่อใช้ `nonSuspiciousOnly=true` หน้าที่ใช้เคอร์เซอร์อาจมีรายการน้อยกว่า `limit`; ใช้ `nextCursor` เพื่อดำเนินการต่อ
...
- `POST /api/v1/users/reserve` สำรอง root slugs และตัวแทนแพ็กเกจส่วนตัวแบบไม่มีรีลีสสำหรับแฮนเดิลเจ้าของ
