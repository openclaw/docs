...
x-i18n:
    generated_at: "2026-07-04T18:24:14Z"
    model: gpt-5.5
...
คุณสามารถสร้างแค็ตตาล็อก ไดเรกทอรี หรือพื้นผิวการค้นหาของบุคคลที่สามบน API อ่านแบบสาธารณะของ ClawHub ได้ เมตาดาตา Skills สาธารณะและไฟล์ Skills ถูกเผยแพร่ภายใต้กฎสิทธิ์การใช้งาน Skills ของ ClawHub ขณะที่ตัว API เองมีการจำกัดอัตราและควรใช้งานอย่างรับผิดชอบ

แนวทาง:

- ใช้ปลายทางอ่านแบบสาธารณะ เช่น `GET /api/v1/skills`, `GET /api/v1/search` และ `GET /api/v1/skills/{slug}` สำหรับรายการแค็ตตาล็อก
- แคชคำตอบและเคารพ `429`, `Retry-After` และส่วนหัวจำกัดอัตรา แทนการโพลอย่างถี่เกินไป
- ลิงก์กลับไปยัง URL Skills ของ ClawHub ที่เป็นแหล่งมาตรฐานเมื่อแสดงรายการ เพื่อให้ผู้ใช้ตรวจสอบระเบียนรีจิสทรีต้นทางได้
...
- อย่าทำสำเนาเนื้อหาที่ซ่อน เป็นส่วนตัว หรือถูกบล็อกจากการกลั่นกรองโดยข้ามตัวกรอง API สาธารณะหรือขอบเขตการยืนยันตัวตน
...
- โทเค็นที่หายไปหรือไม่ถูกต้องจะย้อนกลับไปใช้การบังคับตาม IP
...
- `X-RateLimit-Remaining` / `RateLimit-Remaining`: งบประมาณที่เหลืออย่างแน่นอนเมื่อ
  มีอยู่; คำขอที่สำเร็จแบบแบ่งชาร์ดจะละเว้นค่านี้แทนการส่งคืนค่าโดยรวม
  แบบประมาณ
...
- `GET /api/v1/skills?limit=&cursor=&sort=`
  - `sort`: `updated` (ค่าเริ่มต้น), `recommended` (`default`), `createdAt` (`newest`), `downloads`, `stars` (`rating`), นามแฝงการติดตั้งเดิม `installsCurrent`/`installs`/`installsAllTime` แมปไปที่ `downloads`, `trending`
  ...
  - `recommended` ใช้สัญญาณการมีส่วนร่วมและความใหม่ล่าสุด
...
- `GET /api/v1/download?slug=&version=&tag=`
  - Skills ที่โฮสต์จะส่งคืนไบต์ ZIP แบบกำหนดแน่นอน
  - Skills ปัจจุบันที่อิง GitHub ซึ่งมีผลสแกน `clean` หรือ `suspicious` จะส่งคืน
    ตัวบรรยายการส่งต่อ JSON `public-github` แทนไบต์จาก ClawHub
- `GET /api/v1/skills/export?startDate=&endDate=&limit=&cursor=`
  - Skills ที่โฮสต์จะถูกส่งออกเป็นไฟล์ที่จัดเก็บไว้
  - Skills ปัจจุบันที่อิง GitHub ซึ่งมีผลสแกน `clean` หรือ `suspicious` จะถูกส่งออก
...
- `POST /api/v1/users/reserve` สงวน root slugs และตัวแทนแพ็กเกจส่วนตัวแบบไม่มีรุ่นเผยแพร่สำหรับแฮนเดิลเจ้าของ
