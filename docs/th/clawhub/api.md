...
คุณสามารถสร้างแค็ตตาล็อก ไดเรกทอรี หรือพื้นผิวการค้นหาของบุคคลที่สามบน API อ่านสาธารณะของ ClawHub ได้ เมทาดาทาของสกิลสาธารณะและไฟล์สกิลเผยแพร่ภายใต้กฎสิทธิ์การใช้งานสกิลของ ClawHub ขณะที่ API เองมีการจำกัดอัตราและควรใช้งานอย่างรับผิดชอบ
...
- ลิงก์กลับไปยัง URL สกิล ClawHub แบบบัญญัติเมื่อแสดงรายการ เพื่อให้ผู้ใช้ตรวจสอบระเบียนรีจิสทรีต้นทางได้
- ใช้ URL หน้าบัญญัติในรูปแบบ `https://clawhub.ai/<owner>/skills/<slug>`
...
- `X-RateLimit-Remaining` / `RateLimit-Remaining`: งบประมาณที่เหลืออย่างแม่นยำเมื่อ
  มีอยู่; คำขอที่สำเร็จแบบแบ่งชาร์ดจะละเว้นค่านี้ แทนการส่งคืนค่าโดยรวมทั่วโลกแบบประมาณ
...
- `GET /api/v1/download?slug=&version=&tag=`
  - สกิลที่โฮสต์ไว้จะส่งคืนไบต์ ZIP แบบกำหนดแน่นอน
  - สกิลปัจจุบันที่รองรับโดย GitHub และมีการสแกน `clean` หรือ `suspicious` จะส่งคืน
    ตัวอธิบายการส่งต่อ JSON `public-github` แทนไบต์ของ ClawHub
- `GET /api/v1/skills/export?startDate=&endDate=&limit=&cursor=`
  - สกิลที่โฮสต์ไว้จะถูกส่งออกเป็นไฟล์ที่จัดเก็บไว้
  - สกิลปัจจุบันที่รองรับโดย GitHub และมีการสแกน `clean` หรือ `suspicious` จะถูกส่งออก
...
- `POST /api/v1/users/reserve` สงวน root slugs และ placeholder แพ็กเกจแบบส่วนตัวที่ไม่มีรุ่นเผยแพร่สำหรับ handle เจ้าของ

## รุ่นเดิม

`/api/*` และ `/api/cli/*` รุ่นเดิมยังพร้อมใช้งาน ดู `DEPRECATIONS.md`
