...
คุณสามารถสร้างแค็ตตาล็อก ไดเรกทอรี หรือพื้นผิวการค้นหาของบุคคลที่สามบน API อ่านสาธารณะของ ClawHub ได้ เมทาดาทา Skills สาธารณะและไฟล์ Skills ถูกเผยแพร่ภายใต้กฎสิทธิ์การใช้งาน Skills ของ ClawHub ขณะที่ตัว API เองมีการจำกัดอัตราการเรียกใช้และควรถูกใช้งานอย่างรับผิดชอบ
...
- ใช้ URL หน้าตามรูปแบบ canonical `https://clawhub.ai/<owner>/skills/<slug>`
- อย่าสื่อว่า ClawHub รับรอง ตรวจสอบยืนยัน หรือดำเนินงานไซต์ของบุคคลที่สาม
...
- `RateLimit-Reset`: จำนวนวินาทีที่หน่วงก่อนรีเซ็ต
- `X-RateLimit-Remaining` / `RateLimit-Remaining`: งบประมาณที่เหลืออย่างแน่นอนเมื่อ
  มีอยู่; คำขอที่สำเร็จแบบ sharded จะละไว้แทนการคืนค่าทั่วโลกโดยประมาณ
...
- `GET /api/v1/skills?limit=&cursor=&sort=`
  - `sort`: `updated` (ค่าเริ่มต้น), `recommended` (`default`), `createdAt` (`newest`), `downloads`, `stars` (`rating`), alias การติดตั้งเดิม `installsCurrent`/`installs`/`installsAllTime` map เป็น `downloads`, `trending`
...
- `GET /api/v1/download?slug=&version=&tag=`
  - Skills ที่โฮสต์ไว้จะคืน byte ZIP แบบกำหนดผลลัพธ์ได้แน่นอน
  - Skills ปัจจุบันที่มี GitHub เป็น backend พร้อมการสแกน `clean` หรือ `suspicious` จะคืน
    descriptor การส่งต่อ JSON `public-github` แทน byte จาก ClawHub
- `GET /api/v1/skills/export?startDate=&endDate=&limit=&cursor=`
  - Skills ที่โฮสต์ไว้จะถูกส่งออกเป็นไฟล์ที่จัดเก็บไว้
  - Skills ปัจจุบันที่มี GitHub เป็น backend พร้อมการสแกน `clean` หรือ `suspicious` จะถูกส่งออก
...
- `POST /api/v1/users/reserve` จอง slug รากและ placeholder package แบบส่วนตัวที่ไม่มี release สำหรับ handle เจ้าของ
