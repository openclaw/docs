...
summary: ภาพรวมและแบบแผนของ Public REST API (v1)
x-i18n:
    generated_at: "2026-07-05T08:56:42Z"
    model: gpt-5.5
...
คุณสามารถสร้างแคตตาล็อก ไดเรกทอรี หรือพื้นผิวการค้นหาของบุคคลที่สามบน public read APIs ของ ClawHub ได้ เมตาดาต้า Skills สาธารณะและไฟล์ Skills ถูกเผยแพร่ภายใต้กฎไลเซนส์ Skills ของ ClawHub ส่วน API เองมีการจำกัดอัตราและควรใช้งานอย่างรับผิดชอบ

แนวทาง:

- ใช้ public read endpoints เช่น `GET /api/v1/skills`, `GET /api/v1/search` และ `GET /api/v1/skills/{slug}` สำหรับรายการแคตตาล็อก
- แคชการตอบกลับและเคารพ `429`, `Retry-After` และส่วนหัว rate-limit แทนการ polling อย่างถี่เกินไป
...
- อย่ามิเรอร์เนื้อหาที่ซ่อนอยู่ เป็นส่วนตัว หรือถูกบล็อกโดยการดูแลเนื้อหา ด้วยการข้ามตัวกรอง API สาธารณะหรือขอบเขต auth
...
- คำขอที่ยืนยันตัวตนแล้ว (โทเค็น Bearer ที่ถูกต้อง): ต่อ bucket ของผู้ใช้
...
- `RateLimit-Reset`: จำนวนวินาทีที่ต้องหน่วงก่อนรีเซ็ต
- `X-RateLimit-Remaining` / `RateLimit-Remaining`: งบประมาณที่เหลืออย่างแม่นยำเมื่อ
  มีอยู่; คำขอที่สำเร็จแบบแบ่ง shard จะละไว้แทนการคืนค่า global
  โดยประมาณ
- `Retry-After`: จำนวนวินาทีที่ต้องหน่วงเพื่อรอเมื่อเป็น `429`
...
- `GET /api/v1/search?q=...`
  ...
  - alias เก่า: `nonSuspicious=true`
- `GET /api/v1/skills?limit=&cursor=&sort=`
  - `sort`: `updated` (ค่าเริ่มต้น), `recommended` (`default`), `createdAt` (`newest`), `downloads`, `stars` (`rating`), alias การติดตั้งเก่า `installsCurrent`/`installs`/`installsAllTime` map ไปยัง `downloads`, `trending`
...
- `GET /api/v1/download?slug=&version=&tag=`
  ...
  - Skills ปัจจุบันที่รองรับโดย GitHub และมีการสแกนเป็น `clean` หรือ `suspicious` จะคืน
    descriptor การส่งต่อ JSON `public-github` แทนไบต์ของ ClawHub
- `GET /api/v1/skills/export?startDate=&endDate=&limit=&cursor=`
  - Skills ที่โฮสต์ไว้จะถูกส่งออกเป็นไฟล์ที่จัดเก็บไว้
  - Skills ปัจจุบันที่รองรับโดย GitHub และมีการสแกนเป็น `clean` หรือ `suspicious` จะถูกส่งออก
    เป็น descriptor การส่งต่อ `public-github`
- `GET /api/v1/packages?limit=&cursor=&sort=`
  - `sort`: `updated` (ค่าเริ่มต้น), `recommended`, `downloads`, alias เก่า `installs`
  - ค่า `sort` ที่ไม่ถูกต้องจะคืน `400`
- `GET /api/v1/plugins?limit=&cursor=&sort=`
  - `sort`: `recommended` (ค่าเริ่มต้น), `downloads`, `updated`, alias เก่า `installs`
...
- `POST /api/v1/users/reserve` สงวน root slugs และ placeholder แพ็กเกจส่วนตัวแบบไม่มี release สำหรับ owner handle
