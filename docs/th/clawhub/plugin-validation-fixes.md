...
read_when:
    ...
    - ClawHub ปฏิเสธหรือแสดงคำเตือนขณะเผยแพร่แพ็กเกจ Plugin
...
title: Plugin แก้ไขการตรวจสอบความถูกต้อง
x-i18n:
    generated_at: "2026-07-03T01:06:14Z"
    model: gpt-5.5
...
ผลการตรวจพบที่ผู้เขียน Plugin สามารถแก้ไขได้ใน metadata ของแพ็กเกจ, manifest, การนำเข้า SDK
หรือ artifact ที่เผยแพร่

หน้านี้ไม่ครอบคลุมผลการตรวจพบด้านความครอบคลุมของ Plugin Inspector ภายใน หากรายงานฉบับเต็ม
...
| `package-install-metadata-incomplete`   | [ทำ metadata การติดตั้งให้ครบถ้วน](/th/clawhub/plugin-validation-fixes#package-install-metadata-incomplete)                               |
...
| `package-min-host-version-drift`        | [จัดให้เวอร์ชัน host ขั้นต่ำตรงกัน](/th/clawhub/plugin-validation-fixes#package-min-host-version-drift)                                   |
...
| `package-npm-pack-unavailable`          | [ทำให้ artifact npm pack ได้](/th/clawhub/plugin-validation-fixes#package-npm-pack-unavailable)                                 |
...
| `legacy-root-sdk-import`                | [แทนที่การนำเข้า root SDK](/th/clawhub/plugin-validation-fixes#legacy-root-sdk-import)                                             |
...
| `sdk-session-transcript-file-target`    | [แทนที่ target ไฟล์ transcript แบบเดิม](/th/clawhub/plugin-validation-fixes#sdk-session-transcript-file-target)                   |
...
| `provider-auth-env-vars`                | [ย้าย env vars ของ provider ไปยัง setup metadata](/th/clawhub/plugin-validation-fixes#provider-auth-env-vars)                             |
| `channel-env-vars`                      | [ทำ mirror env vars ของ channel ใน metadata ปัจจุบัน](/th/clawhub/plugin-validation-fixes#channel-env-vars)                                |
| `security-manifest-schema-unavailable`  | [ลบการอ้างอิง security manifest schema ที่ใช้ไม่ได้](/th/clawhub/plugin-validation-fixes#security-manifest-schema-unavailable) |
...
root ของแพ็กเกจไม่มี `package.json` ดังนั้น ClawHub จึงระบุ
...
แพ็กเกจมี `package.json` แต่ไม่ได้ประกาศ metadata ของแพ็กเกจ
OpenClaw

- เพิ่ม `package.json#openclaw`
- รวม metadata ของ entrypoint เช่น `openclaw.extensions` หรือ
  `openclaw.runtimeExtensions`
- เพิ่ม metadata ความเข้ากันได้และการติดตั้งเมื่อแพ็กเกจจะถูกเผยแพร่หรือ
  ติดตั้งผ่าน ClawHub
...
metadata ของแพ็กเกจมีอยู่แล้ว แต่ไม่ได้ประกาศ entrypoint runtime ของ
OpenClaw
...
- ตรวจสอบ metadata การ build ของ OpenClaw ในแพ็กเกจ เช่น เวอร์ชัน OpenClaw
  ที่ใช้ระหว่าง release
- จัดให้เวอร์ชัน host ขั้นต่ำตรงกับช่วงเวอร์ชัน host ที่แพ็กเกจ
  รองรับจริง
- ดู [ฟิลด์ package.json ที่มีผลต่อการค้นพบ](/th/plugins/manifest#packagejson-fields-that-affect-discovery)
- รัน `clawhub package validate <path-to-plugin>` อีกครั้ง
...
- หาก `openclaw.plugin.json` มี `version` ด้วย ให้อัปเดตให้ตรงกันหรือลบ
  metadata เวอร์ชัน manifest ที่ล้าสมัยเมื่อ metadata แพ็กเกจเป็นแหล่งอ้างอิงหลัก
- เผยแพร่เวอร์ชันแพ็กเกจใหม่หลังจากเปลี่ยน metadata ที่เผยแพร่แล้ว
...
metadata แพ็กเกจ OpenClaw

- ลบฟิลด์ที่ไม่รองรับ เช่น `openclaw.bundle`
- เก็บ metadata ของ Plugin แบบ native ไว้ใน `openclaw.plugin.json`
- เก็บ entrypoint ของแพ็กเกจ, ความเข้ากันได้, การติดตั้ง, setup และ catalog metadata
...
แพ็กเกจไม่สามารถถูก pack เป็น artifact ที่ ClawHub จะตรวจสอบหรือ
เผยแพร่ได้
...
แพ็กเกจสามารถ pack ได้ แต่ artifact ที่ pack แล้วไม่มี
...
- รัน `npm pack --dry-run` และตรวจสอบไฟล์ metadata ที่ถูกรวม
...
- ตรวจสอบให้แน่ใจว่า `openclaw.plugin.json` ถูกรวมเมื่อแพ็กเกจเป็น Plugin
  OpenClaw แบบ native
- อัปเดต `files` หรือ `.npmignore` เพื่อไม่ให้ metadata ของแพ็กเกจถูกยกเว้น
...
- ย้ายพฤติกรรม runtime ไปไว้ในโค้ดการลงทะเบียน Plugin และจำกัด `contracts`
  ให้มีเฉพาะ metadata ความเป็นเจ้าของความสามารถแบบคงที่
- เรียกใช้ `clawhub package validate <path-to-plugin>` อีกครั้ง
...
Plugin นำเข้าจาก barrel รากของ SDK ที่เลิกใช้แล้ว:
`openclaw/plugin-sdk`
...
- ใช้ `openclaw/plugin-sdk/channel-core` สำหรับตัวช่วย entry ของ channel
...
Plugin นำเข้า path ของ SDK ที่สงวนไว้สำหรับ Plugin แบบ bundled หรือความเข้ากันได้
ภายใน
...
- ใช้ [subpath ของ Plugin SDK](/th/plugins/sdk-subpaths) และ
  [การย้ายข้อมูล SDK](/th/plugins/sdk-migration) เพื่อเลือก import ที่รองรับ
- เรียกใช้ `clawhub package validate <path-to-plugin>` อีกครั้ง
...
- หลีกเลี่ยงการโหลด แก้ไข และบันทึกออบเจ็กต์ session store ทั้งก้อน
- เก็บตัวช่วยเขียนทั้ง store ไว้เฉพาะเมื่อช่วงความเข้ากันได้ที่คุณประกาศ
  ยังคงรองรับ OpenClaw เวอร์ชันเก่าที่ต้องใช้มัน
- ดู [Runtime API](/th/plugins/sdk-runtime#agent-session-state) และ
  [subpath ของ Plugin SDK](/th/plugins/sdk-subpaths)
- เรียกใช้ `clawhub package validate <path-to-plugin>` อีกครั้ง
...
Plugin ยังใช้ตัวช่วย path ของไฟล์ session ที่เลิกใช้แล้ว เช่น
`resolveSessionFilePath` หรือ `resolveAndPersistSessionFile`

- ใช้ `getSessionEntry(...)` เพื่ออ่าน metadata ของ session ตาม agent และ identity ของ session
- ใช้ `patchSessionEntry(...)` หรือ `upsertSessionEntry(...)` เพื่อบันทึก metadata ของ session
- ใช้ identity ของ transcript หรือตัวช่วย target เมื่อโค้ดกำลังเตรียมการดำเนินการ
  transcript
...
- ใช้ `resolveSessionTranscriptIdentity(...)` เมื่อโค้ดต้องการเพียง identity ของ session
  แบบสาธารณะ
...
- เก็บตัวช่วยเดิมไว้เฉพาะเมื่อช่วงความเข้ากันได้ที่คุณประกาศยังคง
...
- ควรใช้พื้นผิว runtime ของ transcript แบบมีโครงสร้าง เพื่อให้ OpenClaw ใช้ขอบเขต
  transaction และการจัดการ identity ที่ถูกต้องได้
...
- เก็บ `before_agent_start` ไว้เฉพาะเมื่อช่วงความเข้ากันได้ที่คุณประกาศยังคง
  รองรับ OpenClaw เวอร์ชันเก่าที่ต้องใช้มัน
...
manifest ยังใช้ metadata การ auth ของ provider แบบเดิม `providerAuthEnvVars`

- สะท้อน metadata ของ env-var สำหรับ provider เข้าไปใน `setup.providers[].envVars`
- เก็บ `providerAuthEnvVars` ไว้เป็น metadata เพื่อความเข้ากันได้เท่านั้น ในขณะที่ช่วง
...
manifest ใช้ metadata ของ env-var สำหรับ channel แบบเดิมหรือแบบเก่า โดยไม่มี
metadata ของ setup หรือ config ปัจจุบันตามที่ ClawHub คาดหวัง

- เก็บ metadata ของ env-var สำหรับ channel ให้เป็นแบบประกาศ เพื่อให้ OpenClaw ตรวจสอบสถานะ
  setup ได้โดยไม่ต้องโหลด runtime ของ channel
- สะท้อน setup ของ channel ที่ขับเคลื่อนด้วย env เข้าไปใน setup ปัจจุบัน, config ของ channel หรือ
  metadata ของ channel ในแพ็กเกจที่รูปแบบ Plugin ของคุณใช้
- เก็บ `channelEnvVars` ไว้เป็น metadata เพื่อความเข้ากันได้เท่านั้น ในขณะที่ OpenClaw
...
แพ็กเกจส่ง `openclaw.security.json` มาพร้อมการอ้างอิง schema ที่ ClawHub
...
- เก็บพฤติกรรมที่อ่อนไหวด้านความปลอดภัยไว้ในเอกสารแพ็กเกจสาธารณะหรือ
  README ของคุณ จนกว่าสัญญาของ manifest จะมีอยู่จริง
- เรียกใช้ `clawhub package validate <path-to-plugin>` อีกครั้ง
...
- [จุด entry ของ Plugin](/th/plugins/sdk-entrypoints)
- [ความเข้ากันได้ของ Plugin](/th/plugins/compatibility)
