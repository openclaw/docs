...
x-i18n:
    generated_at: "2026-07-02T22:53:04Z"
    model: gpt-5.5
...
| `package-install-metadata-incomplete`   | [กรอกเมทาดาทาการติดตั้งให้ครบถ้วน](/th/clawhub/plugin-validation-fixes#package-install-metadata-incomplete)                               |
...
| `provider-auth-env-vars`                | [ย้าย env var ของ provider ไปยังเมทาดาทาการตั้งค่า](/th/clawhub/plugin-validation-fixes#provider-auth-env-vars)                             |
...
มีเมทาดาทาของแพ็กเกจอยู่แล้ว แต่ไม่ได้ประกาศ entrypoint รันไทม์
ของ OpenClaw
...
- แยกค่านี้ออกจากเวอร์ชันแพ็กเกจ เวอร์ชันแพ็กเกจอธิบาย
  รุ่นของ Plugin; `openclaw.compat.pluginApi` อธิบาย contract ของ API โฮสต์
- ดู [ฟิลด์ package.json ที่มีผลต่อการค้นพบ](/th/plugins/manifest#packagejson-fields-that-affect-discovery)
- รัน `clawhub package validate <path-to-plugin>` อีกครั้ง
...
ไม่สามารถแพ็กแพ็กเกจให้เป็นอาร์ติแฟกต์ที่ ClawHub จะตรวจสอบหรือ
เผยแพร่ได้

- รัน `npm pack --dry-run` จาก root ของแพ็กเกจ
- แก้ไขเมทาดาทาแพ็กเกจที่ไม่ถูกต้อง, สคริปต์ lifecycle ที่เสีย หรือรายการ files
  ที่ทำให้การแพ็กไม่สำเร็จ
- นำ `private: true` ออกหากแพ็กเกจนี้มีไว้สำหรับการเผยแพร่สาธารณะ
- รัน `clawhub package validate <path-to-plugin>` อีกครั้ง
...
- รัน `npm pack --dry-run` และตรวจสอบไฟล์ที่จะถูกรวมเข้าไป
...
- รัน `npm pack --dry-run` และตรวจสอบไฟล์เมทาดาทาที่ถูกรวมเข้าไป
- ตรวจสอบให้แน่ใจว่า `package.json` มีบล็อก `openclaw` ในอาร์ติแฟกต์ที่แพ็กแล้ว
- ตรวจสอบให้แน่ใจว่า `openclaw.plugin.json` ถูกรวมเข้าไปเมื่อแพ็กเกจเป็น Plugin
  OpenClaw แบบ native
...
- เพิ่มฟิลด์ `name` ที่ไม่ว่างเปล่าลงใน `openclaw.plugin.json`
...
- ย้ายข้อมูลเมทาดาทาของแพ็กเกจหรือการติดตั้งไปยังฟิลด์ `package.json#openclaw` ที่รองรับ
  แทนแมนิเฟสต์
- รัน `clawhub package validate <path-to-plugin>` อีกครั้ง
...
- ย้ายพฤติกรรมรันไทม์ไปยังโค้ดการลงทะเบียน Plugin และให้ `contracts`
  จำกัดอยู่เพียงเมทาดาทาความเป็นเจ้าของความสามารถแบบคงที่
- รัน `clawhub package validate <path-to-plugin>` อีกครั้ง
...
- ใช้ [แนวปฏิบัติการนำเข้า](/th/plugins/building-plugins#import-conventions) และ
  [subpath ของ Plugin SDK](/th/plugins/sdk-subpaths) เพื่อค้นหาการนำเข้าที่แคบลง
- รัน `clawhub package validate <path-to-plugin>` อีกครั้ง

### reserved-sdk-import

Plugin นำเข้า path ของ SDK ที่สงวนไว้สำหรับ Plugin ที่มาพร้อมระบบหรือความเข้ากันได้ภายใน

- แทนที่การนำเข้า SDK ภายในของ OpenClaw ที่สงวนไว้ด้วย
  subpath `openclaw/plugin-sdk/*` สาธารณะที่มีเอกสารกำกับ
...
- คง `loadSessionStore(...)` ไว้เฉพาะขณะที่ช่วงความเข้ากันได้ที่คุณประกาศ
...
- หลีกเลี่ยงการโหลด แก้ไข และบันทึกออบเจ็กต์ session store ทั้งหมด
- คงตัวช่วยเขียนทั้ง store ไว้เฉพาะขณะที่ช่วงความเข้ากันได้ที่คุณประกาศ
...
Plugin ยังใช้ตัวช่วย file-path ของเซสชันที่เลิกใช้แล้ว เช่น
`resolveSessionFilePath` หรือ `resolveAndPersistSessionFile`

- ใช้ `getSessionEntry(...)` เพื่ออ่านเมทาดาทาเซสชันตาม agent และอัตลักษณ์เซสชัน
- ใช้ `patchSessionEntry(...)` หรือ `upsertSessionEntry(...)` เพื่อคงเมทาดาทาเซสชัน
- ใช้อัตลักษณ์ transcript หรือตัวช่วย target เมื่อโค้ดกำลังเตรียมการดำเนินการ transcript
...
- ใช้ `resolveSessionTranscriptIdentity(...)` เมื่อโค้ดต้องการเพียงอัตลักษณ์เซสชันสาธารณะ
- ใช้ `resolveSessionTranscriptTarget(...)` เมื่อโค้ดต้องการเป้าหมายการดำเนินการ transcript แบบมีโครงสร้าง
- หลีกเลี่ยงการอ่านหรือสร้างเป้าหมายไฟล์ transcript แบบเดิมโดยตรง
- คงตัวช่วยเดิมไว้เฉพาะขณะที่ช่วงความเข้ากันได้ที่คุณประกาศยัง
...
- ควรใช้พื้นผิว runtime transcript แบบมีโครงสร้าง เพื่อให้ OpenClaw สามารถใช้
  ขอบเขตทรานแซกชันและการจัดการอัตลักษณ์ที่ถูกต้อง
- คงตัวช่วย transcript ระดับต่ำไว้เฉพาะขณะที่ช่วงความเข้ากันได้ที่คุณประกาศ
  ยังรองรับ OpenClaw เวอร์ชันเก่าที่ต้องใช้สิ่งเหล่านี้
- ดู [Runtime API](/th/plugins/sdk-runtime#agent-session-state) และ
  [subpath ของ Plugin SDK](/th/plugins/sdk-subpaths)
- รัน `clawhub package validate <path-to-plugin>` อีกครั้ง
...
- คง `before_agent_start` ไว้เฉพาะขณะที่ช่วงความเข้ากันได้ที่คุณประกาศยัง
  รองรับ OpenClaw เวอร์ชันเก่าที่ต้องใช้สิ่งนี้
...
แมนิเฟสต์ยังใช้เมทาดาทา auth ของ provider แบบเดิม `providerAuthEnvVars`
...
แมนิเฟสต์ใช้เมทาดาทา env-var ของช่องทางแบบเดิมหรือเก่ากว่า โดยไม่มีเมทาดาทา setup หรือ config ปัจจุบันที่ ClawHub คาดหวัง

- ให้เมทาดาทา env-var ของช่องทางเป็นแบบ declarative เพื่อให้ OpenClaw ตรวจสอบสถานะ setup
...
- คงพฤติกรรมที่ไวต่อความปลอดภัยไว้ในเอกสารแพ็กเกจสาธารณะหรือ
