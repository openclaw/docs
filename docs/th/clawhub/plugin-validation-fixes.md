---
read_when:
    - คุณเรียกใช้ clawhub package validate และต้องแก้ไขประเด็นที่พบของ Plugin
...
x-i18n:
    generated_at: "2026-06-30T22:39:40Z"
    model: gpt-5.5
...
มีไว้สำหรับผู้ดูแล OpenClaw แทนผู้เขียน Plugin
...
- เพิ่มบล็อก `openclaw` เมื่อแพ็กเกจส่งมอบ Plugin OpenClaw
- ใช้ [การสร้าง Plugin](/th/plugins/building-plugins) สำหรับตัวอย่างแพ็กเกจขั้นต่ำ
  ...
  สำหรับการแบ่งส่วนระหว่างแพ็กเกจกับ manifest
...
มีเมทาดาทาของแพ็กเกจแล้ว แต่ไม่ได้ประกาศจุดเข้าใช้งาน runtime ของ
OpenClaw
...
- ใช้เวอร์ชัน API Plugin ของ OpenClaw หรือขอบเขตล่างของ semver ที่คุณสร้างและทดสอบ
  เทียบไว้
...
- เก็บจุดเข้าใช้งานของแพ็กเกจ, ความเข้ากันได้, การติดตั้ง, การตั้งค่า และเมทาดาทาแค็ตตาล็อก
...
- ย้ายพฤติกรรม runtime ไปไว้ในโค้ดการลงทะเบียน Plugin และจำกัด `contracts`
  ให้มีเฉพาะ metadata การเป็นเจ้าของความสามารถแบบสแตติก
- รัน `clawhub package validate <path-to-plugin>` อีกครั้ง
...
- แทนที่การนำเข้าจาก root-barrel ด้วยการนำเข้าจาก public subpath แบบเจาะจง
...
Plugin นำเข้า path ของ SDK ที่สงวนไว้สำหรับ Plugin ที่ bundled หรือความเข้ากันได้ภายใน
...
Plugin ยังใช้ helper ของ whole-session-store ที่เลิกใช้แล้ว
`loadSessionStore`
...
Plugin ยังใช้ helper สำหรับเขียน whole-session-store ที่เลิกใช้แล้ว เช่น
`saveSessionStore` หรือ `updateSessionStore`

- ใช้ `patchSessionEntry(...)` เมื่ออัปเดตฟิลด์บน entry ของ session ที่มีอยู่
- ใช้ `upsertSessionEntry(...)` เมื่อแทนที่หรือสร้าง entry ของ session
- หลีกเลี่ยงการโหลด แก้ไข และบันทึกออบเจ็กต์ session store ทั้งหมด
- เก็บ helper สำหรับเขียนทั้ง store ไว้เฉพาะในช่วงที่ช่วงความเข้ากันได้ที่คุณประกาศ
  ยังรองรับ OpenClaw เวอร์ชันเก่าที่ต้องใช้มัน
- ดู [Runtime API](/th/plugins/sdk-runtime#agent-session-state) และ
  [subpaths ของ Plugin SDK](/th/plugins/sdk-subpaths)
- รัน `clawhub package validate <path-to-plugin>` อีกครั้ง
...
Plugin ยังใช้ helper สำหรับ path ไฟล์ session ที่เลิกใช้แล้ว เช่น
...
- ใช้ `patchSessionEntry(...)` หรือ `upsertSessionEntry(...)` เพื่อคง metadata ของ session ไว้
- ใช้ helper สำหรับตัวตน transcript หรือ target เมื่อโค้ดกำลังเตรียมการดำเนินการกับ
  transcript
- อย่าคงอยู่หรือพึ่งพา path ไฟล์ transcript แบบ legacy
- ดู [Runtime API](/th/plugins/sdk-runtime#agent-session-state) และ
  [subpaths ของ Plugin SDK](/th/plugins/sdk-subpaths)
- รัน `clawhub package validate <path-to-plugin>` อีกครั้ง
...
- เก็บ helper แบบ legacy ไว้เฉพาะในช่วงที่ช่วงความเข้ากันได้ที่คุณประกาศยัง
...
- เก็บ helper transcript ระดับต่ำไว้เฉพาะในช่วงที่ช่วงความเข้ากันได้ที่คุณประกาศ
  ยังรองรับ OpenClaw เวอร์ชันเก่าที่ต้องใช้มัน
- ดู [Runtime API](/th/plugins/sdk-runtime#agent-session-state) และ
  [subpaths ของ Plugin SDK](/th/plugins/sdk-subpaths)
- รัน `clawhub package validate <path-to-plugin>` อีกครั้ง
...
manifest ยังใช้ metadata การยืนยันตัวตนของ provider แบบ legacy `providerAuthEnvVars`

- สะท้อน metadata ของ env-var ของ provider เข้าไปใน `setup.providers[].envVars`
...
manifest ใช้ metadata ของ env-var ของ channel แบบ legacy หรือแบบเก่า โดยไม่มี metadata
setup หรือ config ปัจจุบันที่ ClawHub คาดไว้

- ให้ metadata ของ env-var ของ channel เป็นแบบประกาศ เพื่อให้ OpenClaw ตรวจสอบสถานะ setup
...
- เก็บ `channelEnvVars` ไว้เป็น metadata ด้านความเข้ากันได้เท่านั้น ขณะที่ OpenClaw เวอร์ชันเก่า
...
- นำ URL ของ schema ออก หากมีไว้เพื่อให้คำแนะนำเท่านั้น
- ใช้ schema แบบมีเวอร์ชันที่มีเอกสารกำกับเฉพาะหลังจาก OpenClaw เผยแพร่แล้ว
- รัน `clawhub package validate <path-to-plugin>` อีกครั้ง

### unrecognized-security-manifest

แพ็กเกจมาพร้อมไฟล์ manifest ด้านความปลอดภัยที่ไม่รองรับ

- นำ `openclaw.security.json` ออก จนกว่า OpenClaw จะมีเอกสารสำหรับ schema ของ manifest
  ด้านความปลอดภัยแบบมีเวอร์ชันและพฤติกรรมของ ClawHub
- เก็บพฤติกรรมที่เกี่ยวกับความปลอดภัยไว้ในเอกสารแพ็กเกจสาธารณะหรือ
  README จนกว่าจะมี contract ของ manifest
- รัน `clawhub package validate <path-to-plugin>` อีกครั้ง
