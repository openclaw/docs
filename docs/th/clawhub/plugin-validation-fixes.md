...
x-i18n:
    generated_at: "2026-07-01T08:45:04Z"
    model: gpt-5.5
...
การสแกนแพ็กเกจอัตโนมัติได้ด้วย หน้านี้ครอบคลุมผลการตรวจพบที่ผู้เขียนต้องจัดการ ซึ่งหมายถึง
...
มีรหัสการบำรุงรักษาของ scanner โดยไม่มีคำแนะนำการแก้ไขสำหรับผู้เขียน รหัสเหล่านั้นมีไว้สำหรับ
...
## ผลการตรวจพบที่ผู้เขียนต้องจัดการ
...
| `package-openclaw-unsupported-metadata` | [ลบ metadata ของแพ็กเกจ OpenClaw ที่ไม่รองรับ](/th/clawhub/plugin-validation-fixes#package-openclaw-unsupported-metadata)          |
| `package-npm-pack-unavailable`          | [ทำให้ artifact ของ npm pack ได้](/th/clawhub/plugin-validation-fixes#package-npm-pack-unavailable)                                 |
...
| `package-npm-pack-metadata-missing`     | [รวม metadata ในผลลัพธ์ของ npm pack](/th/clawhub/plugin-validation-fixes#package-npm-pack-metadata-missing)                       |
| `manifest-name-missing`                 | [เพิ่มชื่อที่ใช้แสดงผลของ manifest](/th/clawhub/plugin-validation-fixes#manifest-name-missing)                                           |
...
| `sdk-session-file-helper`               | [แทนที่ helper สำหรับพาธไฟล์ของ session](/th/clawhub/plugin-validation-fixes#sdk-session-file-helper)                                   |
...
| `provider-auth-env-vars`                | [ย้าย env var ของ provider ไปยัง metadata การตั้งค่า](/th/clawhub/plugin-validation-fixes#provider-auth-env-vars)                             |
| `channel-env-vars`                      | [สะท้อน env var ของ channel ใน metadata ปัจจุบัน](/th/clawhub/plugin-validation-fixes#channel-env-vars)                                |
...
แพ็กเกจ npm, เวอร์ชัน, entrypoint หรือ metadata ของ OpenClaw ได้
...
มี metadata ของแพ็กเกจอยู่แล้ว แต่ไม่ได้ประกาศ runtime
entrypoint ของ OpenClaw

- เพิ่ม `openclaw.extensions` สำหรับ entrypoint ของ native Plugin
...
metadata ของแพ็กเกจ OpenClaw

- ลบฟิลด์ที่ไม่รองรับ เช่น `openclaw.bundle`
- เก็บ metadata ของ native Plugin ไว้ใน `openclaw.plugin.json`
- เก็บ entrypoint ของแพ็กเกจ, ความเข้ากันได้, การติดตั้ง, การตั้งค่า และ metadata ของ catalog
...
- แก้ไข metadata แพ็กเกจที่ไม่ถูกต้อง, lifecycle script ที่เสีย หรือรายการ files ที่
...
แพ็กเกจสามารถถูก pack ได้ แต่ artifact ที่ pack แล้วไม่มีไฟล์
...
- ตรวจสอบให้แน่ใจว่า `openclaw.plugin.json` ถูกรวมไว้เมื่อแพ็กเกจเป็น native
  OpenClaw Plugin
- อัปเดต `files` หรือ `.npmignore` เพื่อไม่ให้ metadata ของแพ็กเกจถูกแยกออก
...
- เปรียบเทียบแต่ละฟิลด์ระดับบนสุดกับ
  [เอกสารอ้างอิงฟิลด์แมนิเฟสต์](/th/plugins/manifest#top-level-field-reference)
...
- ย้ายพฤติกรรมรันไทม์ไปไว้ในโค้ดการลงทะเบียน Plugin และจำกัด `contracts`
  ให้มีเฉพาะข้อมูลเมตาความเป็นเจ้าของความสามารถแบบสแตติก
- เรียกใช้ `clawhub package validate <path-to-plugin>` อีกครั้ง
...
- ใช้ [แนวทางการนำเข้า](/th/plugins/building-plugins#import-conventions) และ
  [พาธย่อยของ Plugin SDK](/th/plugins/sdk-subpaths) เพื่อค้นหาการนำเข้าที่แคบลง
- เรียกใช้ `clawhub package validate <path-to-plugin>` อีกครั้ง

### reserved-sdk-import

Plugin นำเข้าพาธ SDK ที่สงวนไว้สำหรับ Plugin ที่บันเดิลมาพร้อมกันหรือความเข้ากันได้ภายใน

- แทนที่การนำเข้า SDK ภายในของ OpenClaw ที่สงวนไว้ด้วยพาธย่อยสาธารณะ
...
- หลีกเลี่ยงการโหลด แก้ไข และบันทึกออบเจ็กต์ session store ทั้งหมด
- เก็บตัวช่วยเขียน whole-store ไว้เฉพาะในขณะที่ช่วงความเข้ากันได้ที่คุณประกาศ
  ยังรองรับ OpenClaw เวอร์ชันเก่าที่ต้องใช้มัน
- ดู [Runtime API](/th/plugins/sdk-runtime#agent-session-state) และ
  [พาธย่อยของ Plugin SDK](/th/plugins/sdk-subpaths)
- เรียกใช้ `clawhub package validate <path-to-plugin>` อีกครั้ง
...
- ใช้ `getSessionEntry(...)` เพื่ออ่านข้อมูลเมตาเซสชันตามตัวตนของเอเจนต์และเซสชัน
- ใช้ `patchSessionEntry(...)` หรือ `upsertSessionEntry(...)` เพื่อคงข้อมูลเมตาเซสชันไว้
- ใช้ตัวช่วยตัวตน transcript หรือเป้าหมายเมื่อโค้ดกำลังเตรียมการดำเนินการ transcript
- อย่าคงไว้หรือพึ่งพาพาธไฟล์ transcript แบบเดิม
- ดู [Runtime API](/th/plugins/sdk-runtime#agent-session-state) และ
  [พาธย่อยของ Plugin SDK](/th/plugins/sdk-subpaths)
- เรียกใช้ `clawhub package validate <path-to-plugin>` อีกครั้ง
...
- ใช้ `resolveSessionTranscriptTarget(...)` เมื่อโค้ดต้องการเป้าหมายการดำเนินการ transcript
  แบบมีโครงสร้าง
- หลีกเลี่ยงการอ่านหรือสร้างเป้าหมายไฟล์ transcript แบบเดิมโดยตรง
- เก็บตัวช่วยแบบเดิมไว้เฉพาะในขณะที่ช่วงความเข้ากันได้ที่คุณประกาศยังรองรับ
...
- เก็บ `before_agent_start` ไว้เฉพาะในขณะที่ช่วงความเข้ากันได้ที่คุณประกาศยังรองรับ
  OpenClaw เวอร์ชันเก่าที่ต้องใช้มัน
...
แมนิเฟสต์ยังใช้ข้อมูลเมตาการยืนยันตัวตน provider แบบเดิม `providerAuthEnvVars`

- สะท้อนข้อมูลเมตา env-var ของ provider ไปยัง `setup.providers[].envVars`
- เก็บ `providerAuthEnvVars` ไว้เฉพาะเป็นข้อมูลเมตาความเข้ากันได้ในขณะที่ช่วง OpenClaw
...
แมนิเฟสต์ใช้ข้อมูลเมตา env-var ของช่องทางแบบเดิมหรือเก่ากว่า โดยไม่มีข้อมูลเมตา setup
...
- สะท้อน setup ช่องทางที่ขับเคลื่อนด้วย env ไปยัง setup ปัจจุบัน, config ช่องทาง หรือ
  ข้อมูลเมตาช่องทางของแพ็กเกจที่รูปแบบ Plugin ของคุณใช้
- เก็บ `channelEnvVars` ไว้เฉพาะเป็นข้อมูลเมตาความเข้ากันได้ในขณะที่ OpenClaw เวอร์ชันเก่า
...
- นำ `openclaw.security.json` ออกจนกว่า OpenClaw จะจัดทำเอกสาร schema แมนิเฟสต์ความปลอดภัย
