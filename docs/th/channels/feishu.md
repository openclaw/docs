...
**สถานะ:** พร้อมใช้งานจริงสำหรับ DM ของบอตและแชตกลุ่ม โหมด WebSocket เป็นโหมดเริ่มต้น; โหมด Webhook เป็นตัวเลือกเสริม

---

## เริ่มต้นอย่างรวดเร็ว

> **ต้องใช้ OpenClaw 2026.4.10 ขึ้นไป** รัน `openclaw --version` เพื่อตรวจสอบ อัปเกรดด้วย `openclaw update`
...
<Steps>
  ...
  ```
  สแกนคิวอาร์โค้ดด้วยแอปมือถือ Feishu/Lark ของคุณเพื่อสร้างบอต Feishu/Lark โดยอัตโนมัติ
  </Step>
  
  <Step title="หลังจากตั้งค่าเสร็จสิ้น ให้รีสตาร์ต Gateway เพื่อให้การเปลี่ยนแปลงมีผล">
  ```bash
...
- `false` — ตอบกลับได้โดยไม่ต้องมี @mention
- การตั้งค่าแทนที่รายกลุ่ม: `channels.feishu.groups.<chat_id>.requireMention`
...
เริ่มต้น Gateway ส่ง DM ไปที่บอต แล้วตรวจสอบบันทึก:

```bash
openclaw logs --follow
```

มองหา `open_id` ในผลลัพธ์ของบันทึก คุณยังสามารถตรวจสอบคำขอจับคู่ที่รอดำเนินการได้:
...
3. ตรวจสอบว่า `groupPolicy` ไม่ได้ตั้งเป็น `"disabled"`
...
4. ตรวจสอบให้แน่ใจว่าได้ให้สิทธิ์การเข้าถึงที่จำเป็นทั้งหมดแล้ว
...
Feishu/Lark รองรับการสตรีมคำตอบผ่านการ์ดแบบโต้ตอบ เมื่อเปิดใช้งาน บอตจะอัปเดตการ์ดแบบเรียลไทม์ระหว่างสร้างข้อความ
...
ตั้งค่า `streaming: false` เพื่อส่งคำตอบแบบสมบูรณ์ในข้อความเดียว

### การปรับการใช้โควตาให้เหมาะสม

ลดจำนวนการเรียกใช้ API ของ Feishu/Lark ด้วยแฟล็กเสริม 2 รายการ:

- `typingIndicator` (ค่าเริ่มต้น `true`): ตั้งค่าเป็น `false` เพื่อข้ามการเรียกใช้ปฏิกิริยาการพิมพ์
...
Feishu/Lark รองรับ ACP สำหรับ DM และข้อความเธรดกลุ่ม ACP ของ Feishu/Lark ขับเคลื่อนด้วยคำสั่งข้อความ — ไม่มีเมนู slash command แบบเนทีฟ ดังนั้นให้ใช้ข้อความ `/acp ...` โดยตรงในการสนทนา
...
`--thread here` ใช้ได้กับ DM และข้อความเธรดของ Feishu/Lark ข้อความติดตามผลในบทสนทนาที่ผูกไว้จะถูกส่งตรงไปยังเซสชัน ACP นั้น
...
ดู [ดู ID ของกลุ่ม/ผู้ใช้](#get-groupuser-ids) สำหรับคำแนะนำในการค้นหา
...
| `channels.feishu.connectionMode`                  | การรับส่งเหตุการณ์ (`websocket` หรือ `webhook`) | `websocket`      |
...
- ⚠️ ข้อความ Rich Text (การจัดรูปแบบแบบ post; ไม่รองรับความสามารถในการเขียนแบบเต็มของ Feishu/Lark)
