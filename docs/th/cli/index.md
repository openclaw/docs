...
x-i18n:
    generated_at: "2026-07-16T18:53:51Z"
    model: gpt-5.6
...
`openclaw` คือจุดเริ่มต้นหลักของ CLI คำสั่งหลักแต่ละคำสั่งมี
หน้าอ้างอิงเฉพาะ หรือมีเอกสารกำกับไว้กับคำสั่งที่คำสั่งนั้นใช้เป็นนามแฝง ดัชนีนี้แสดง
คำสั่ง แฟล็กส่วนกลาง และกฎการจัดรูปแบบเอาต์พุตที่ใช้ทั่วทั้ง CLI

คำสั่งตั้งค่าตามวัตถุประสงค์:

- `openclaw setup` และ `openclaw onboard` ตรวจสอบการอนุมานก่อน จากนั้นจึงเริ่ม OpenClaw เพื่อตั้งค่า Gateway, เวิร์กสเปซ, ช่องทาง, Skills และสถานะระบบ
- `openclaw setup --baseline` สร้างการกำหนดค่าพื้นฐานและเวิร์กสเปซโดยไม่ดำเนินตามขั้นตอนการเริ่มต้นใช้งานแบบมีคำแนะนำ
- `openclaw configure` เปลี่ยนแปลงส่วนที่ระบุของการตั้งค่าที่มีอยู่ ได้แก่ การยืนยันตัวตนของโมเดล, Gateway, ช่องทาง, Plugin หรือ Skills
- `openclaw channels add` กำหนดค่าบัญชีช่องทางหลังจากมีการตั้งค่าพื้นฐานแล้ว เรียกใช้โดยไม่มีแฟล็กสำหรับการตั้งค่าแบบมีคำแนะนำ หรือใช้แฟล็กเฉพาะช่องทางสำหรับสคริปต์

## หน้าคำสั่ง

| หมวดหมู่                         | คำสั่ง                                                                                                                                                                                                                              |
| ---------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| การตั้งค่าและการเริ่มต้นใช้งาน         | [`openclaw`](/cli/openclaw) · [`setup`](/th/cli/setup) · [`onboard`](/th/cli/onboard) · [`configure`](/th/cli/configure) · [`config`](/th/cli/config) · [`completion`](/th/cli/completion) · [`doctor`](/th/cli/doctor) · [`dashboard`](/th/cli/dashboard) |
...
| เครือข่ายและ Node            | [`directory`](/th/cli/directory) · [`nodes`](/th/cli/nodes) · [`devices`](/th/cli/devices) · [`node`](/th/cli/node) · [`worker`](/cli/worker)                                                                                                     |
...
| นามแฝงแบบเดิม               | [`daemon`](/th/cli/daemon) (บริการ Gateway) · [`clawbot`](/th/cli/clawbot) (เนมสเปซ)                                                                                                                                                     |
...
| `--log-level <level>`   | เขียนทับระดับบันทึกเหตุการณ์ส่วนกลางสำหรับเอาต์พุตไปยังไฟล์และคอนโซล                                                 |
| `--no-color`            | ปิดใช้สี ANSI (`NO_COLOR=1` จะได้รับการรองรับเช่นกัน)                                                    |
| `--update`              | รูปแบบย่อของ [`openclaw update`](/th/cli/update) ใช้ได้ทั้งกับซอร์สเช็กเอาต์และการติดตั้งแพ็กเกจ    |
...
OpenClaw ใช้จานสีล็อบสเตอร์สำหรับเอาต์พุต CLI:

| โทเค็น          | เลขฐานสิบหก       | ใช้สำหรับ                             |
| -------------- | --------- | ------------------------------------ |
| `accent`       | `#FF5A2D` | หัวเรื่อง ป้ายกำกับ ไฮไลต์หลัก |
| `accentBright` | `#FF7A3D` | ชื่อคำสั่ง การเน้นข้อความ              |
| `accentDim`    | `#D14A22` | ข้อความไฮไลต์รอง             |
| `info`         | `#FF8A5B` | ค่าข้อมูล                 |
| `success`      | `#2FBF71` | สถานะสำเร็จ                       |
| `warn`         | `#FFB020` | คำเตือน แฟล็กตัวเลือก ค่าสำรอง    |
| `error`        | `#E23D2D` | ข้อผิดพลาด ความล้มเหลว                     |
| `muted`        | `#8B7F77` | การลดการเน้น เมทาดาทา                |

แหล่งข้อมูลหลักของจานสี: `packages/terminal-core/src/palette.ts`

## แผนผังคำสั่ง

<Accordion title="แผนผังคำสั่งฉบับเต็ม">

แผนผังนี้ครอบคลุมคำสั่งหลักและคำสั่งย่อยหลัก คำสั่งย่อยที่ Plugin เพิ่ม
(ตัวอย่างเช่น ภายใต้ `skills`, `plugins` และ `wiki`) มีการพัฒนา
...
ข้อความแชตรองรับคำสั่ง `/...` ดู[คำสั่งเครื่องหมายทับ](/th/tools/slash-commands)

จุดเด่น:

- `/status` - การวินิจฉัยอย่างรวดเร็ว
- `/trace` - บรรทัดติดตาม/ดีบัก Plugin ที่มีขอบเขตเฉพาะเซสชัน
- `/config` - การเปลี่ยนแปลงการกำหนดค่าที่บันทึกถาวร
- `/debug` - การแทนที่การกำหนดค่าสำหรับรันไทม์เท่านั้น (อยู่ในหน่วยความจำ ไม่ใช่ดิสก์ และต้องใช้ `commands.debug: true`)
...
และถูกปรับให้อยู่ในรูปแบบ `X% left` ผู้ให้บริการที่มีช่วงเวลาการใช้งานปัจจุบัน
ได้แก่ Anthropic, Gemini CLI, GitHub Copilot, MiniMax, OpenAI Codex,
Xiaomi และ z.ai

ดูรายละเอียดใน[การติดตามการใช้งาน](/th/concepts/usage-tracking)
