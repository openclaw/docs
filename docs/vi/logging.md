---
read_when:
    - Bạn cần phần tổng quan thân thiện với người mới bắt đầu về ghi nhật ký trong OpenClaw
    - Bạn muốn cấu hình mức nhật ký, định dạng hoặc cơ chế che thông tin nhạy cảm
    - Bạn đang khắc phục sự cố và cần tìm nhật ký nhanh chóng
summary: Nhật ký tệp, đầu ra bảng điều khiển, theo dõi bằng CLI, và tab Nhật ký của Giao diện điều khiển
title: Ghi nhật ký
x-i18n:
...
lưu nhật ký, cách đọc chúng, và cách cấu hình mức nhật ký cùng định dạng nhật ký.
...
Mỗi tệp xoay vòng khi đạt tới `logging.maxFileBytes` (mặc định: 100 MB).
OpenClaw giữ tối đa năm bản lưu trữ được đánh số bên cạnh tệp đang hoạt động, chẳng hạn như
`openclaw-YYYY-MM-DD.1.log`, và tiếp tục ghi vào một nhật ký đang hoạt động mới thay vì
ngăn chặn chẩn đoán.
...
- `--url <url>` / `--token <token>` / `--timeout <ms>`: các cờ RPC Gateway tiêu chuẩn
- `--expect-final`: cờ chờ phản hồi cuối cùng RPC do agent hỗ trợ (được chấp nhận ở đây qua lớp máy khách dùng chung)
...
Khi bạn truyền `--url` rõ ràng, CLI không tự động áp dụng thông tin xác thực từ cấu hình hoặc
môi trường; hãy tự thêm `--token` nếu Gateway đích
yêu cầu xác thực.
...
hoặc hết thời gian chờ trước khi `logs.tail` trả lời, `openclaw logs` tự động chuyển về
nhật ký tệp Gateway đã cấu hình. Các đích `--url` rõ ràng không dùng
cơ chế dự phòng này.
...
Xem [Giao diện điều khiển](/vi/web/control-ui) để biết cách mở.
...
mục này để hiển thị đầu ra có cấu trúc (thời gian, mức, hệ con, thông báo).
...
OpenClaw giữ nguyên các đối số nhật ký có cấu trúc ban đầu cùng với những trường này
để các trình phân tích hiện có đọc các khóa đối số tslog được đánh số vẫn tiếp tục hoạt động.
...
`openclaw gateway` cũng có nhật ký giao thức WebSocket cho lưu lượng RPC:
...
- `logging.level`: mức **nhật ký tệp** (JSONL).
...
`--verbose` chỉ ảnh hưởng đến đầu ra console và độ chi tiết nhật ký WS; nó không thay đổi
mức nhật ký tệp.
...
với các span OTEL và lan truyền `traceparent` của provider.

Các yêu cầu HTTP Gateway và frame WebSocket Gateway thiết lập một phạm vi trace yêu cầu
nội bộ. Nhật ký và sự kiện chẩn đoán phát ra bên trong phạm vi bất đồng bộ đó kế thừa
trace yêu cầu khi chúng không truyền ngữ cảnh trace rõ ràng. Trace chạy agent và
lệnh gọi mô hình trở thành con của trace yêu cầu đang hoạt động, vì vậy nhật ký cục bộ,
ảnh chụp chẩn đoán, span OTEL, và header `traceparent` đáng tin cậy của provider có thể
được nối bằng `traceId` mà không ghi nhật ký nội dung yêu cầu hoặc nội dung mô hình thô.
...
- `pretty`: thân thiện với người đọc, có màu, có dấu thời gian.
- `compact`: đầu ra gọn hơn (tốt nhất cho phiên dài).
- `json`: JSON mỗi dòng (cho bộ xử lý nhật ký).

### Biên tập ẩn

OpenClaw có thể biên tập ẩn token nhạy cảm trước khi chúng xuất hiện trong đầu ra console, nhật ký tệp,
bản ghi nhật ký OTLP, văn bản transcript phiên đã lưu, hoặc payload sự kiện công cụ của Giao diện điều khiển
(đối số khởi động công cụ, payload kết quả một phần/cuối cùng, đầu ra exec dẫn xuất, và tóm tắt bản vá):

- `logging.redactSensitive`: `off` | `tools` (mặc định: `tools`)
- `logging.redactPatterns`: danh sách chuỗi regex để ghi đè bộ mặc định. Mẫu tùy chỉnh được áp dụng bên trên các mặc định tích hợp sẵn cho payload công cụ của Giao diện điều khiển, vì vậy việc thêm một mẫu không bao giờ làm yếu đi việc biên tập ẩn các giá trị đã được mặc định bắt được.

Nhật ký tệp và transcript phiên vẫn là JSONL, nhưng các giá trị bí mật khớp mẫu được
che trước khi dòng hoặc thông báo được ghi vào đĩa. Biên tập ẩn là nỗ lực tối đa:
nó áp dụng cho nội dung thông báo có văn bản và chuỗi nhật ký, không phải mọi
...
OpenClaw vẫn biên tập ẩn các payload ranh giới an toàn có thể được hiển thị cho máy khách UI,
gói hỗ trợ, trình quan sát chẩn đoán, prompt phê duyệt, hoặc công cụ agent.
Ví dụ bao gồm sự kiện gọi công cụ của Giao diện điều khiển, đầu ra `sessions_history`,
...
vẫn có thể thêm các mẫu dành riêng cho dự án trên những bề mặt đó.

## Chẩn đoán và OpenTelemetry

Chẩn đoán là các sự kiện có cấu trúc, máy đọc được cho lượt chạy mô hình và
đo từ xa luồng thông báo (webhook, xếp hàng, trạng thái phiên). Chúng **không**
thay thế nhật ký — chúng cung cấp dữ liệu cho chỉ số, trace, và exporter. Sự kiện được phát ra
trong tiến trình dù bạn có xuất chúng hay không.

Hai bề mặt liền kề:

- **Xuất OpenTelemetry** — gửi chỉ số, trace, và nhật ký qua OTLP/HTTP đến
...
- **Cờ chẩn đoán** — các cờ nhật ký gỡ lỗi được nhắm mục tiêu, định tuyến nhật ký bổ sung đến
...
- **Không truy cập được Gateway?** Trước tiên chạy `openclaw doctor`.
...
- [Cờ chẩn đoán](/vi/diagnostics/flags) — các cờ nhật ký gỡ lỗi được nhắm mục tiêu
- [Nội bộ ghi nhật ký Gateway](/vi/gateway/logging) — kiểu nhật ký WS, tiền tố hệ con, và ghi console
