...
x-i18n:
    generated_at: "2026-07-04T18:06:03Z"
    model: gpt-5.5
...
khi bạn đăng nhập, xuất bản, quản lý danh sách của riêng mình, hoặc dùng
các quy trình dành riêng cho registry.

## Tìm và cài đặt skill
...
Cài đặt plugin được lưu trữ trên ClawHub với nguồn ClawHub tường minh:
...
Xuất bản plugin từ một thư mục cục bộ, repo GitHub, ref GitHub, hoặc một
kho lưu trữ hiện có:
...
Trước tiên dùng `--dry-run` để xem trước metadata gói đã phân giải, các trường
tương thích, thông tin quy nguồn, và kế hoạch tải lên mà không xuất bản.

Plugin mã phải bao gồm metadata tương thích OpenClaw trong `package.json`,
bao gồm `openclaw.compat.pluginApi` và `openclaw.build.openclawVersion`.

## Kiểm tra trước khi cài đặt

Trước khi cài đặt, hãy dùng trang web ClawHub hoặc các lệnh chi tiết CLI để kiểm tra
