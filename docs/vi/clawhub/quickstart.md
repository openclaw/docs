...
x-i18n:
    generated_at: "2026-07-04T20:35:07Z"
    model: gpt-5.5
...
tự động phát hành phiên bản bản vá tiếp theo. Dùng `--dry-run` để xem trước hoặc
`--version` để chọn phiên bản tường minh.
...
Đối với các repository chứa nhiều skill, workflow GitHub có thể tái sử dụng gọi
...
Phát hành plugin từ một thư mục cục bộ, một repo GitHub, một ref GitHub, hoặc một
archive hiện có:
...
Trước tiên dùng `--dry-run` để xem trước siêu dữ liệu package đã phân giải, các
trường tương thích, ghi nhận nguồn, và kế hoạch upload mà không phát hành.
