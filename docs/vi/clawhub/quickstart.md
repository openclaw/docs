...
x-i18n:
    generated_at: "2026-07-04T06:38:02Z"
    model: gpt-5.5
...
ClawHub là một registry cho kỹ năng và plugin của OpenClaw.
...
Kỹ năng là một thư mục có tệp `SKILL.md` bắt buộc và các tệp hỗ trợ tùy chọn.
...
Lệnh sẽ bỏ qua nội dung không thay đổi. Kỹ năng mới bắt đầu ở `1.0.0`; các thay đổi
sau đó tự động xuất bản phiên bản patch tiếp theo. Dùng `--dry-run` để xem trước hoặc
`--version` để chọn một phiên bản tường minh.

Trước khi xuất bản, hãy kiểm tra metadata trong `SKILL.md`. Khai báo các
biến môi trường, công cụ và quyền bắt buộc để người dùng có thể hiểu kỹ năng cần gì
