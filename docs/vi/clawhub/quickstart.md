...
x-i18n:
    generated_at: "2026-07-16T14:10:53Z"
    model: gpt-5.6
...
ClawHub là kho đăng ký Skills và Plugin cho OpenClaw.

Sử dụng OpenClaw khi bạn cài đặt nội dung vào OpenClaw. Sử dụng CLI `clawhub`
khi bạn đăng nhập, phát hành, quản lý các mục đăng của mình hoặc sử dụng
...
OpenClaw ghi lại nguồn của Skills để các bản cập nhật sau này có thể tiếp tục
...
Các môi trường không giao diện có thể sử dụng token API từ giao diện web ClawHub:

```bash
clawhub login --token clh_...
```

## Phát hành một Skills

Skills là một thư mục có tệp `SKILL.md` bắt buộc và các tệp hỗ trợ
tùy chọn.
...
sau đó tự động phát hành phiên bản vá tiếp theo. Sử dụng `--dry-run` để xem trước hoặc
...
cần những gì trước khi cài đặt. Xem [Định dạng Skills](/vi/clawhub/skill-format).
...
Phát hành Plugin từ thư mục cục bộ, kho lưu trữ GitHub, tham chiếu GitHub hoặc
tệp lưu trữ hiện có:
...
Trước tiên, sử dụng `--dry-run` để xem trước siêu dữ liệu gói đã phân giải, các trường
...
Các mục đăng công khai hiển thị trạng thái quét mới nhất. Những bản phát hành bị tạm giữ hoặc chặn bởi
quá trình kiểm duyệt có thể bị ẩn khỏi giao diện tìm kiếm và cài đặt cho đến khi được giải quyết.
