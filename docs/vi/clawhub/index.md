...
ClawHub là kho đăng ký công khai cho Skills và Plugin của OpenClaw.

- Dùng các lệnh `openclaw` gốc để tìm kiếm, cài đặt và cập nhật Skills, cũng như cài đặt Plugin từ ClawHub.
- Dùng CLI `clawhub` riêng cho các quy trình xác thực kho đăng ký, phát hành và xóa/khôi phục xóa.
...
Cài đặt CLI ClawHub khi bạn muốn các quy trình đã xác thực với kho đăng ký như
...
lượt tải xuống, sao và tóm tắt quét bảo mật. Các trang công khai hiển thị trạng thái kho đăng ký
hiện tại để người dùng có thể kiểm tra Skills hoặc Plugin trước khi cài đặt.
...
Các đặc tả Plugin trần an toàn với npm có thể phân giải qua npm trong các đợt chuyển đổi khi ra mắt, và
...
được công bố trước khi quá trình cài đặt từ kho lưu trữ chạy. Khi một phiên bản gói phát hành
...
CLI ClawHub dành cho công việc đã xác thực với kho đăng ký:
...
CLI cũng có các lệnh cài đặt/cập nhật Skills cho quy trình trực tiếp với kho đăng ký:
...
`openclaw.build.openclawVersion`. Xem [CLI](/vi/clawhub/cli) để có tham chiếu lệnh
...
ClawHub chạy các kiểm tra tự động trên Skills và các bản phát hành Plugin đã phát hành. Các bản phát hành bị giữ do quét
...
[Sử dụng được chấp nhận](/clawhub/acceptable-usage) để biết chi tiết về chính sách và thực thi.
...
| `CLAWHUB_REGISTRY`            | Ghi đè URL API kho đăng ký.                    |
