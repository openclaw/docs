...
read_when:
    ...
    - Lựa chọn giữa các quy trình CLI openclaw và clawhub
...
- Sử dụng CLI `clawhub` riêng cho các workflow xác thực registry, xuất bản và xóa/khôi phục xóa.
...
Cài đặt CLI ClawHub khi bạn muốn dùng các workflow đã xác thực với registry như
...
| Plugin mã      | Các gói Plugin OpenClaw kèm siêu dữ liệu tương thích         | `openclaw plugins install clawhub:<package>` |
...
lượt tải xuống, sao và tóm tắt quét bảo mật. Các trang công khai hiển thị trạng
thái registry hiện tại để người dùng có thể kiểm tra một Skill hoặc Plugin trước
khi cài đặt.
...
Các spec Plugin trần an toàn với npm có thể phân giải qua npm trong các giai đoạn
chuyển đổi khi ra mắt, và `npm:<package>` luôn chỉ dùng npm khi cần chỉ rõ nguồn.

Việc cài đặt Plugin xác thực khả năng tương thích `pluginApi` và
`minGatewayVersion` được khai báo trước khi chạy cài đặt archive. Khi một phiên
bản gói xuất bản artifact ClawPack, OpenClaw ưu tiên đúng tệp `.tgz` npm-pack đã
tải lên, xác minh header digest của ClawHub và các byte đã tải xuống, rồi ghi lại
siêu dữ liệu artifact cho các lần cập nhật sau.
...
và ghi lại phiên bản đã cài đặt trong `.clawhub/lock.json`.
...
Xuất bản Plugin từ một thư mục cục bộ, `owner/repo`, `owner/repo@ref`, hoặc một
URL GitHub:
...
Dùng `--dry-run` để xây dựng đúng kế hoạch xuất bản mà không tải lên, và `--json`
...
`openclaw.build.openclawVersion`. Xem [CLI](/vi/clawhub/cli) để biết tài liệu tham
khảo lệnh đầy đủ và [Định dạng Skill](/vi/clawhub/skill-format) để biết siêu dữ liệu
Skill.

## Bảo mật và kiểm duyệt

ClawHub mặc định mở: bất kỳ ai cũng có thể tải lên, nhưng việc xuất bản yêu cầu
một tài khoản GitHub đủ cũ để vượt qua cổng tải lên. Các trang chi tiết công khai
tóm tắt trạng thái quét mới nhất trước khi cài đặt hoặc tải xuống.

ClawHub chạy các kiểm tra tự động trên Skills đã xuất bản và các bản phát hành
Plugin. Các bản phát hành bị giữ lại do quét hoặc bị chặn có thể biến mất khỏi
catalog công khai và các bề mặt cài đặt, trong khi vẫn hiển thị với chủ sở hữu
của chúng trong `/dashboard`.

Người dùng đã đăng nhập có thể báo cáo Skills và gói. Người kiểm duyệt có thể
xem xét báo cáo, ẩn hoặc khôi phục nội dung, và cấm các tài khoản lạm dụng. Xem
...
[Sử dụng được chấp nhận](/vi/clawhub/acceptable-usage) để biết chi tiết về chính
sách và thực thi.

## Telemetry và môi trường

Khi bạn chạy `clawhub install` trong lúc đã đăng nhập, CLI có thể gửi một sự kiện
cài đặt theo nỗ lực tốt nhất để ClawHub có thể tính tổng số lượt cài đặt. Tắt
tính năng này bằng:
...
| Biến                          | Tác dụng                                          |
| ----------------------------- | ------------------------------------------------- |
| `CLAWHUB_SITE`                | Override URL trang web dùng cho đăng nhập trình duyệt. |
