...
read_when:
    ...
    - Xuất bản Skills hoặc plugins lên sổ đăng ký
...
- Dùng CLI `clawhub` riêng cho các workflow xác thực registry, phát hành và xóa/khôi phục xóa.
...
Cài đặt CLI ClawHub khi bạn muốn dùng các workflow được registry xác thực như
...
| Bundle plugins | Các bundle Plugin đã đóng gói để phân phối OpenClaw       | `clawhub package publish <source>`          |
...
được quảng bá trước khi chạy cài đặt archive. Khi một phiên bản gói phát hành một
artifact ClawPack, OpenClaw ưu tiên `.tgz` npm-pack đã tải lên chính xác, xác minh
header digest ClawHub và byte đã tải xuống, rồi ghi lại metadata artifact cho
các lần cập nhật sau.

## CLI ClawHub

CLI ClawHub dành cho công việc được registry xác thực:
...
ClawHub mặc định mở: bất kỳ ai cũng có thể tải lên, nhưng phát hành yêu cầu tài khoản GitHub
...
[Cách sử dụng được chấp nhận](/vi/clawhub/acceptable-usage) để biết chi tiết chính sách và thực thi.
