...
Cài đặt ClawHub CLI khi bạn muốn dùng các quy trình đã xác thực với registry như
...
| Plugin mã nguồn | Các gói plugin OpenClaw với metadata tương thích           | `openclaw plugins install clawhub:<package>` |
| Plugin bundle | Các bundle plugin đã đóng gói để phân phối OpenClaw          | `clawhub package publish <source>`           |
| Souls          | Các bundle `SOUL.md` hiển thị trên onlycrabs.ai              | Luồng phát hành Web và API                  |
...
Các spec plugin an toàn cho npm dạng trần có thể được phân giải qua npm trong các giai đoạn chuyển đổi khi ra mắt, và
...
đã công bố trước khi chạy cài đặt archive. Khi một phiên bản gói phát hành
artifact ClawPack, OpenClaw ưu tiên `.tgz` npm-pack đã tải lên chính xác, xác minh
header digest ClawHub và các byte đã tải xuống, đồng thời ghi lại metadata artifact cho
...
Plugin mã nguồn phải bao gồm metadata tương thích OpenClaw bắt buộc trong
...
ClawHub mặc định là mở: bất kỳ ai cũng có thể tải lên, nhưng phát hành yêu cầu một tài khoản GitHub
