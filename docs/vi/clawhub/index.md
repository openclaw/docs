...
read_when:
    ...
    - Phát hành skills hoặc plugins lên registry
    - Chọn giữa các quy trình CLI của openclaw và clawhub
sidebarTitle: ClawHub
summary: Tổng quan công khai về ClawHub để khám phá, cài đặt, xuất bản, bảo mật và CLI clawhub.
title: ClawHub
x-i18n:
    generated_at: "2026-07-04T20:33:14Z"
    model: gpt-5.5
...
| Plugin bundle | Các bundle plugin đóng gói để phân phối OpenClaw             | `clawhub package publish <source>`           |
...
Các đặc tả plugin npm-safe dạng trần có thể phân giải qua npm trong giai đoạn chuyển đổi ra mắt, và
...
được công bố trước khi cài đặt archive chạy. Khi một phiên bản gói phát hành artifact
...
ClawHub mặc định mở: ai cũng có thể tải lên, nhưng việc phát hành yêu cầu tài khoản GitHub
...
bị giữ lại do quét hoặc bị chặn có thể biến mất khỏi catalog công khai và các bề mặt cài đặt, đồng thời
vẫn hiển thị với chủ sở hữu của chúng trong `/dashboard`.

Người dùng đã đăng nhập có thể báo cáo skills và packages. Điều phối viên có thể xem xét báo cáo,
...
[Sử dụng chấp nhận được](/vi/clawhub/acceptable-usage) để biết chi tiết chính sách và thực thi.

## Telemetry và môi trường

Khi bạn chạy `clawhub install` trong lúc đã đăng nhập, CLI có thể gửi một sự kiện
cài đặt theo nỗ lực tối đa để ClawHub có thể tính số lượt cài đặt tổng hợp. Tắt tính năng này bằng:
...
| Biến                          | Tác dụng                                          |
| ----------------------------- | ------------------------------------------------- |
| `CLAWHUB_SITE`                | Ghi đè URL trang dùng cho đăng nhập trình duyệt.  |
