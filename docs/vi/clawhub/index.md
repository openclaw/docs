...
summary: Tổng quan ClawHub công khai để khám phá, cài đặt, xuất bản, bảo mật và CLI clawhub.
title: ClawHub
x-i18n:
    generated_at: "2026-07-03T02:50:01Z"
    model: gpt-5.5
...
Các đặc tả Plugin dạng npm-safe không tiền tố có thể phân giải qua npm trong các giai đoạn chuyển đổi phát hành, và
`npm:<package>` vẫn chỉ dùng npm khi cần chỉ rõ nguồn.

Các lượt cài đặt Plugin xác thực tương thích `pluginApi` và `minGatewayVersion`
được công bố trước khi quá trình cài đặt archive chạy. Khi một phiên bản gói phát hành
artifact ClawPack, OpenClaw ưu tiên `.tgz` npm-pack đã tải lên chính xác, xác minh
header digest ClawHub và các byte đã tải xuống, đồng thời ghi lại siêu dữ liệu artifact cho
...
cài đặt theo nỗ lực tốt nhất để ClawHub có thể tính số lượt cài đặt tổng hợp. Tắt tính năng này bằng:
