...
title: ClawHub
x-i18n:
    generated_at: "2026-07-02T01:00:37Z"
    model: gpt-5.5
...
siêu dữ liệu nguồn để các lệnh cập nhật về sau có thể tiếp tục dùng ClawHub.

Dùng `clawhub:<package>` khi quá trình cài đặt plugin cần phân giải qua ClawHub.
Các đặc tả plugin an toàn với npm dạng trần có thể phân giải qua npm trong giai đoạn chuyển đổi khi ra mắt, và
`npm:<package>` vẫn chỉ dùng npm khi nguồn phải được nêu rõ.

Việc cài đặt plugin xác thực khả năng tương thích `pluginApi` và `minGatewayVersion`
...
Khi bạn chạy `clawhub install` trong lúc đã đăng nhập, CLI có thể gửi một sự kiện cài đặt theo best-effort
để ClawHub có thể tính tổng số lượt cài đặt. Tắt chức năng này bằng:
