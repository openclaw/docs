...
summary: Tổng quan ClawHub công khai về khám phá, cài đặt, xuất bản, bảo mật và CLI clawhub.
title: ClawHub
x-i18n:
...
Cài đặt CLI ClawHub khi bạn muốn các quy trình đã xác thực với registry như
...
Các đặc tả plugin an toàn với npm dạng trần có thể phân giải qua npm trong giai đoạn chuyển đổi phát hành, và
`npm:<package>` vẫn chỉ dùng npm khi phải chỉ rõ nguồn.

Việc cài đặt plugin xác thực tính tương thích của `pluginApi` và `minGatewayVersion`
đã quảng bá trước khi chạy cài đặt kho lưu trữ. Khi một phiên bản gói xuất bản một
tạo tác ClawPack, OpenClaw ưu tiên tệp `.tgz` npm-pack đã tải lên chính xác, xác minh
header digest của ClawHub và các byte đã tải xuống, đồng thời ghi lại siêu dữ liệu tạo tác cho
...
Những lệnh đó cài đặt Skills vào `./skills` trong thư mục làm việc hiện tại
...
`openclaw.build.openclawVersion`. Xem [CLI](/vi/clawhub/cli) để biết tham chiếu lệnh đầy đủ
...
bị giữ do quét hoặc bị chặn có thể biến mất khỏi danh mục công khai và bề mặt cài đặt trong khi
...
[Sử dụng chấp nhận được](/clawhub/acceptable-usage) để biết chi tiết về chính sách và thực thi.
...
để ClawHub có thể tính toán tổng số lượt cài đặt. Tắt tính năng này bằng:
...
| Biến                      | Tác dụng                                            |
| ----------------------------- | ------------------------------------------------- |
| `CLAWHUB_SITE`                | Ghi đè URL trang được dùng để đăng nhập bằng trình duyệt.     |
...
[Khắc phục sự cố](/vi/clawhub/troubleshooting) để biết tài liệu tham chiếu chuyên sâu hơn.
