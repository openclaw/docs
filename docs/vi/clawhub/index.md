...
read_when:
    ...
    - Đăng Skills hoặc Plugin lên kho đăng ký
...
title: ClawHub
x-i18n:
    generated_at: "2026-07-04T06:38:41Z"
    model: gpt-5.5
...
Cài đặt CLI ClawHub khi bạn muốn các workflow có xác thực registry như
...
| -------------- | ------------------------------------------------------------ | -------------------------------------------- |
| Skills         | Các gói văn bản có phiên bản, gồm `SKILL.md` và tệp hỗ trợ   | `openclaw skills install @openclaw/demo`     |
...
Dùng `clawhub:<package>` khi quá trình cài đặt plugin cần phân giải qua ClawHub.
Các đặc tả plugin an toàn với npm dạng trần có thể phân giải qua npm trong các giai đoạn chuyển đổi khi ra mắt, và
`npm:<package>` vẫn chỉ dùng npm khi nguồn cần được chỉ định rõ ràng.

Các lượt cài đặt plugin xác thực khả năng tương thích `pluginApi` và `minGatewayVersion`
được công bố trước khi cài đặt kho lưu trữ chạy. Khi một phiên bản gói xuất bản một
...
CLI cũng có các lệnh cài đặt/cập nhật Skills cho workflow registry trực tiếp:
...
- `--slug <slug>`: tên URL Skills đã xuất bản.
...
Dùng `--dry-run` để dựng đúng kế hoạch xuất bản mà không tải lên, và `--json`
...
và [Định dạng Skills](/clawhub/skill-format) để xem siêu dữ liệu Skills.
...
ClawHub chạy các kiểm tra tự động trên Skills đã xuất bản và các bản phát hành plugin. Các bản phát hành
...
| Biến                          | Hiệu lực                                          |
| ----------------------------- | ------------------------------------------------- |
| `CLAWHUB_SITE`                | Ghi đè URL trang dùng để đăng nhập bằng trình duyệt. |
