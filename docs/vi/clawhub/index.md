...
| Plugin mã nguồn   | Các gói plugin OpenClaw với siêu dữ liệu tương thích         | `openclaw plugins install clawhub:<package>` |
| Plugin gói | Các gói plugin đã đóng gói để phân phối OpenClaw            | `clawhub package publish <source>`           |
| Souls          | Các gói `SOUL.md` hiển thị trên onlycrabs.ai                      | Các luồng phát hành qua web và API                    |
...
siêu dữ liệu nguồn để các lệnh cập nhật sau này vẫn có thể ở trên ClawHub.

Dùng `clawhub:<package>` khi một lượt cài đặt plugin cần phân giải qua ClawHub.
Các đặc tả plugin an toàn với npm dạng trần có thể phân giải qua npm trong các giai đoạn chuyển đổi khi ra mắt, và
...
được công bố trước khi chạy cài đặt archive. Khi một phiên bản gói phát hành tạo tác
ClawPack, OpenClaw ưu tiên đúng `.tgz` npm-pack đã tải lên, xác minh
header digest của ClawHub và các byte đã tải xuống, rồi ghi lại siêu dữ liệu tạo tác cho
các lần cập nhật sau.

## CLI ClawHub

CLI ClawHub dành cho công việc cần xác thực với registry:
...
đầy đủ và [Định dạng Skills](/vi/clawhub/skill-format) để biết siêu dữ liệu Skills.
...
Khi bạn chạy `clawhub sync` trong khi đã đăng nhập, CLI gửi một ảnh chụp tối thiểu để
