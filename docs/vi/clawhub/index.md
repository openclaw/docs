...
summary: Tổng quan công khai về ClawHub cho khám phá, cài đặt, xuất bản, bảo mật và CLI clawhub.
title: ClawHub
x-i18n:
    generated_at: "2026-07-04T15:23:00Z"
    model: gpt-5.5
...
Các spec plugin trần an toàn cho npm có thể phân giải qua npm trong giai đoạn chuyển đổi lúc ra mắt, và
`npm:<package>` vẫn chỉ dùng npm khi phải nêu nguồn rõ ràng.

Các cài đặt plugin xác thực khả năng tương thích `pluginApi` và `minGatewayVersion`
đã công bố trước khi chạy cài đặt archive. Khi một phiên bản gói xuất bản artifact
ClawPack, OpenClaw ưu tiên tệp `.tgz` npm-pack đã tải lên chính xác, xác minh
header digest của ClawHub và các byte đã tải xuống, rồi ghi metadata artifact cho
...
Dùng `--dry-run` để xây dựng kế hoạch xuất bản chính xác mà không tải lên, và `--json`
...
`openclaw.build.openclawVersion`. Xem [CLI](/vi/clawhub/cli) để có tài liệu tham chiếu lệnh
đầy đủ và [Định dạng Skill](/clawhub/skill-format) để xem metadata Skill.
...
ClawHub chạy kiểm tra tự động trên Skills đã xuất bản và các bản phát hành plugin. Những bản phát hành
...
[Chính sách sử dụng được chấp nhận](/vi/clawhub/acceptable-usage) để biết chi tiết về chính sách và thực thi.

## Telemetry và môi trường

Khi bạn chạy `clawhub install` trong lúc đã đăng nhập, CLI có thể gửi một sự kiện cài đặt
theo cơ chế nỗ lực tối đa để ClawHub có thể tính tổng số lượt cài đặt. Tắt tính năng này bằng:
...
| Biến                          | Tác dụng                                          |
| ----------------------------- | ------------------------------------------------- |
| `CLAWHUB_SITE`                | Ghi đè URL trang web dùng để đăng nhập bằng trình duyệt. |
...
[Khắc phục sự cố](/vi/clawhub/troubleshooting) để có tài liệu tham chiếu chuyên sâu hơn.
