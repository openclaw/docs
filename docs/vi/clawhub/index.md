...
title: ClawHub
x-i18n:
    generated_at: "2026-06-30T22:21:56Z"
    model: gpt-5.5
...
Cài đặt CLI ClawHub khi bạn muốn các quy trình được registry xác thực, chẳng hạn như
...
| Plugin gói     | Các gói plugin đóng gói cho bản phân phối OpenClaw           | `clawhub package publish <source>`         |

ClawHub theo dõi phiên bản semver, thẻ như `latest`, nhật ký thay đổi, tệp,
lượt tải xuống, lượt đánh sao và tóm tắt quét bảo mật. Các trang công khai hiển thị trạng thái registry
...
Các spec plugin npm-safe dạng trần có thể phân giải qua npm trong các giai đoạn chuyển đổi khi ra mắt, và
`npm:<package>` chỉ dùng npm khi cần chỉ rõ nguồn.

Các lượt cài đặt plugin xác thực khả năng tương thích `pluginApi` và `minGatewayVersion`
được quảng bá trước khi cài đặt archive chạy. Khi một phiên bản gói phát hành một
artifact ClawPack, OpenClaw ưu tiên `.tgz` npm-pack đã tải lên chính xác, xác minh
header digest ClawHub và các byte đã tải xuống, rồi ghi siêu dữ liệu artifact cho
...
ClawHub chạy kiểm tra tự động trên các bản phát hành skills và plugin đã phát hành. Các bản phát hành
...
[Chính sách sử dụng chấp nhận được](/clawhub/acceptable-usage) để biết chi tiết về chính sách và thực thi.

## Telemetry và môi trường

Khi bạn chạy `clawhub install` trong lúc đã đăng nhập, CLI có thể gửi sự kiện cài đặt theo nỗ lực tốt nhất
để ClawHub có thể tính số lượt cài đặt tổng hợp. Tắt tính năng này bằng:
