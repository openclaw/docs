...
read_when:
    ...
    - Xuất bản Skills hoặc plugins lên registry
...
Cài đặt ClawHub CLI khi bạn muốn dùng các workflow đã xác thực với registry như
...
Các spec plugin trần an toàn với npm có thể được phân giải qua npm trong các giai đoạn chuyển đổi khi ra mắt, và
...
được công bố trước khi chạy cài đặt archive. Khi một phiên bản gói xuất bản artifact
ClawPack, OpenClaw ưu tiên tệp npm-pack `.tgz` đã tải lên chính xác, xác minh
...
`openclaw.build.openclawVersion`. Xem [CLI](/vi/clawhub/cli) để biết tài liệu tham khảo
lệnh đầy đủ và [Định dạng Skill](/vi/clawhub/skill-format) để biết siêu dữ liệu Skill.
...
[Mức sử dụng được chấp nhận](/vi/clawhub/acceptable-usage) để biết chi tiết về chính sách và thực thi.

## Telemetry và môi trường

Khi bạn chạy `clawhub install` trong lúc đã đăng nhập, CLI có thể gửi một sự kiện cài đặt theo khả năng tốt nhất
