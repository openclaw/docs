...
title: ClawHub
x-i18n:
    generated_at: "2026-07-01T18:13:45Z"
    model: gpt-5.5
...
| Code plugins   | Các gói plugin OpenClaw có siêu dữ liệu tương thích         | `openclaw plugins install clawhub:<package>` |
| Bundle plugins | Các gói plugin đóng gói cho bản phân phối OpenClaw          | `clawhub package publish <source>`           |
...
Các đặc tả plugin dạng npm-safe không có tiền tố có thể phân giải qua npm trong giai đoạn chuyển đổi phát hành, và
`npm:<package>` vẫn chỉ dùng npm khi nguồn cần được nêu rõ.

Quá trình cài đặt plugin xác thực tính tương thích `pluginApi` và `minGatewayVersion`
được công bố trước khi chạy cài đặt archive. Khi một phiên bản gói phát hành
artifact ClawPack, OpenClaw ưu tiên tệp `.tgz` npm-pack đã tải lên chính xác đó, xác minh
...
Phát hành plugins từ một thư mục cục bộ, `owner/repo`, `owner/repo@ref`, hoặc một URL GitHub:
...
Code plugins phải bao gồm siêu dữ liệu tương thích OpenClaw bắt buộc trong
...
ClawHub chạy kiểm tra tự động trên skills và các bản phát hành plugin đã phát hành. Các bản phát hành
...
Người dùng đã đăng nhập có thể báo cáo skills và packages. Kiểm duyệt viên có thể xem xét báo cáo,
...
theo nỗ lực tốt nhất để ClawHub có thể tính số lượt cài đặt tổng hợp. Tắt tính năng này bằng:
