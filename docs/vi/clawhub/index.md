...
title: ClawHub
x-i18n:
    generated_at: "2026-06-30T14:06:44Z"
    model: gpt-5.5
...
Cài đặt CLI ClawHub khi bạn muốn các quy trình được xác thực với registry như
...
Các đặc tả plugin hợp lệ với npm ở dạng trần có thể được phân giải qua npm trong giai đoạn chuyển đổi khi ra mắt, và
`npm:<package>` vẫn chỉ dùng npm khi nguồn phải được nêu rõ.

Các lượt cài đặt plugin xác thực tính tương thích của `pluginApi` và `minGatewayVersion`
được công bố trước khi cài đặt archive chạy. Khi một phiên bản gói phát hành
...
CLI ClawHub dùng cho công việc được xác thực với registry:
...
Phát hành plugin từ một thư mục cục bộ, `owner/repo`, `owner/repo@ref`, hoặc một URL
GitHub:
...
Dùng `--dry-run` để tạo kế hoạch phát hành chính xác mà không tải lên, và `--json`
...
ClawHub chạy các kiểm tra tự động trên Skills và các bản phát hành plugin đã phát hành. Những bản phát hành
...
[Cách sử dụng được chấp nhận](/clawhub/acceptable-usage) để biết chi tiết về chính sách và thực thi.

## Telemetry và môi trường

Khi bạn chạy `clawhub install` trong khi đã đăng nhập, CLI có thể gửi sự kiện cài đặt theo nỗ lực tốt nhất
...
| Biến                          | Tác dụng                                          |
| ----------------------------- | ------------------------------------------------- |
| `CLAWHUB_SITE`                | Ghi đè URL trang web dùng cho đăng nhập trình duyệt. |
...
Xem [Telemetry](/clawhub/telemetry), [API HTTP](/clawhub/http-api), và
