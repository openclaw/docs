...
ClawHub bình thường. Nếu đăng nhập đưa bạn trở lại trạng thái đã đăng xuất, tài
khoản của bạn có thể không ở trạng thái tốt.
...
Token bị thu hồi, không hợp lệ hoặc bị thiếu sẽ trả về `401 Unauthorized`. Hãy
đăng nhập lại bằng `clawhub login` hoặc cung cấp token mới bằng `clawhub login --token`.

Các tài khoản đã bị xóa, bị cấm hoặc bị vô hiệu hóa không thể tiếp tục dùng token
API hiện có.
