...
Nếu đăng nhập đưa bạn trở lại trạng thái đã đăng xuất, tài khoản của bạn có thể không còn ở trạng thái
hợp lệ.
...
Các mã thông báo đã bị thu hồi, không hợp lệ hoặc bị thiếu sẽ trả về `401 Unauthorized`. Đăng nhập lại
bằng `clawhub login` hoặc cung cấp một mã thông báo mới bằng `clawhub login --token`.
