...
Các thư mục của bên thứ ba có thể dùng các endpoint đọc công khai để liệt kê hoặc tìm kiếm Skills trên ClawHub. Vui lòng lưu kết quả vào bộ nhớ đệm, tuân thủ `429`/`Retry-After`, liên kết người dùng về danh sách ClawHub chuẩn (`https://clawhub.ai/<owner>/skills/<slug>`), và tránh ngụ ý rằng ClawHub chứng thực trang bên thứ ba. Không cố gắng sao chép nội dung ẩn, riêng tư, hoặc bị chặn bởi kiểm duyệt ra ngoài bề mặt API công khai.

Các lối tắt slug web phân giải trên nhiều họ registry, nhưng client API nên dùng
các URL chuẩn được endpoint đọc trả về thay vì tự dựng lại độ ưu tiên tuyến.
...
- Các endpoint ghi đã xác thực không nên trả về trống `Unauthorized` khi
  máy chủ biết lý do. Token bị thiếu, token không hợp lệ/bị thu hồi, và
  tài khoản đã xóa/bị cấm/bị vô hiệu hóa nên nhận văn bản có thể hành động để CLI
...
nên đọc phần thân phản hồi như một chuỗi người đọc được. Các tham số truy vấn không xác định được
bỏ qua để tương thích, nhưng các tham số truy vấn được nhận diện với giá trị không hợp lệ trả về
`400`.
...
- Mức độ liên quan mạnh hơn mức độ phổ biến. Một slug chính xác hoặc token tên hiển thị khớp có thể xếp trên một kết quả khớp lỏng hơn nhưng có tương tác cao hơn nhiều.
- Văn bản ASCII được tách token theo ranh giới từ và dấu câu. Ví dụ, `personal-map` chứa token độc lập `map`, trong khi `amap-jsapi-skill` chứa `amap`, `jsapi`, và `skill`; vì vậy tìm kiếm `map` cho `personal-map` một khớp từ vựng mạnh hơn `amap-jsapi-skill`.
- Mức độ phổ biến được co theo log và có trần. Skills có tương tác cao có thể xếp thấp hơn khi văn bản truy vấn khớp yếu hơn.
...
- Đặt các thuật ngữ mà người dùng sẽ thực sự tìm kiếm vào tên hiển thị, tóm tắt, và tag. Chỉ dùng token slug độc lập khi đó cũng là danh tính ổn định mà bạn muốn giữ.
- Không đổi tên slug chỉ để chạy theo một truy vấn, trừ khi slug mới là tên chuẩn dài hạn tốt hơn. Slug cũ trở thành alias chuyển hướng, nhưng URL chuẩn, slug hiển thị, và các bản tóm tắt tìm kiếm trong tương lai dùng slug mới.
- Alias đổi tên giữ khả năng phân giải cho URL cũ và các lần cài đặt phân giải qua registry, nhưng thứ hạng tìm kiếm dựa trên metadata Skill chuẩn sau khi việc đổi tên đã được lập chỉ mục. Thống kê hiện có vẫn gắn với Skill.
...
đặt `status` lại thành `open`. Truyền `finalAction: "hide"` với một báo cáo
đã được triage để ẩn Skill trong cùng workflow có thể kiểm toán.
...
- Nếu không cung cấp `version` lẫn `tag`, sẽ dùng phiên bản mới nhất.
- Bao gồm trạng thái xác minh đã chuẩn hóa cùng với chi tiết riêng của từng trình quét.
- `security.hasScanResult` chỉ là `true` khi một trình quét đã tạo ra phán quyết dứt khoát (`clean`, `suspicious`, hoặc `malicious`).
- `moderation` là ảnh chụp moderation hiện tại ở cấp skill, được dẫn xuất từ phiên bản mới nhất.
...
- Tác vụ quét là bất đồng bộ. Các yêu cầu quét thủ công được ưu tiên trước công việc xuất bản/backfill thông thường, nhưng việc hoàn tất vẫn phụ thuộc vào khả năng sẵn sàng của worker.
...
- Trả về `queue.queuedAhead` và `queue.position` khi đang queued để client có thể hiển thị có bao nhiêu lần quét thủ công được ưu tiên đang đứng trước yêu cầu. Các hàng đợi rất lớn được giới hạn và báo cáo với `queuedAheadIsEstimate: true`.
...
- Yêu cầu một lần quét đã succeeded; các lần quét chưa kết thúc trả về `409`.
...
Endpoint kho lưu trữ báo cáo đã lưu trữ, có xác thực, cho các phiên bản đã gửi.
...
- Trả về kết quả quét đã lưu trữ cho đúng phiên bản đã gửi, bao gồm các phiên bản bị chặn hoặc bị ẩn.
...
Route quét lại hàng loạt chuẩn, chỉ dành cho admin. Nó chấp nhận cùng dạng payload như `POST /api/v1/skills/-/rescan-batch` kế thừa.

### `POST /api/v1/skills/-/scan/batch/status`

Route trạng thái hàng loạt chuẩn, chỉ dành cho admin. Nó chấp nhận `{ "jobIds": ["..."] }` và trả về cùng các bộ đếm tổng hợp như `POST /api/v1/skills/-/rescan-batch/status` kế thừa.
...
- `ok` chỉ là `true` khi phiên bản đã chọn có Skill Card đã tạo, không bị moderation chặn do mã độc, và xác minh ClawScan là sạch.
...
- `security.signals.dependencyRegistry` được giữ lại để tương thích phản hồi v1, nhưng trình quét sự tồn tại của registry dependency đã ngừng hoạt động và khóa này luôn là `null`.
- `provenance` chỉ là `server-resolved-github-import` khi ClawHub đã phân giải và lưu trữ repo/ref/commit/path GitHub trong quá trình xuất bản hoặc nhập; nếu không thì là `unavailable`.

### `POST /api/v1/skills/-/security-verdicts`

Trả về các phán quyết bảo mật nhỏ gọn hiện tại cho đúng các phiên bản skill. Endpoint bộ sưu tập này dành cho các client đã biết những phiên bản skill ClawHub đã cài đặt nào cần hiển thị, chẳng hạn như OpenClaw Control UI.
...
- Phản hồi chỉ dành cho bảo mật. Nó không bao gồm dữ liệu Skill Card, trạng thái thẻ đã tạo, danh sách tệp artifact, hoặc payload chi tiết của trình quét.
...
- Mục Skills vẫn được hỗ trợ bởi sổ đăng ký Skills và vẫn chỉ có thể được xuất bản qua `POST /api/v1/skills`.
...
- Bên gọi đã xác thực có thể thấy các gói riêng tư của những nhà xuất bản mà họ thuộc về trong kết quả liệt kê/tìm kiếm.
...
- Skills hiện tại được hỗ trợ bởi GitHub với bản quét `clean` hoặc `suspicious` bao gồm
...
- Các bí danh bộ lọc v1 cũ được ghi trong `GET /api/v1/plugins` cũng được
  chấp nhận.
- Lọc danh mục là một bộ lọc API thực, được hỗ trợ bởi các hàng digest danh mục
...
- Các bản phát hành ClawPack bao gồm các trường tương thích npm `npmIntegrity`, `npmShasum`, và
  `npmTarballName`.
- `version.sha256hash` là siêu dữ liệu tương thích đã ngừng khuyến nghị cho các client cũ. Nó
...
Trả về tóm tắt chính xác về bảo mật và độ tin cậy của bản phát hành gói cho các
...
- `release.artifactKind`, `release.artifactSha256`, `release.npmIntegrity`,
  `release.npmShasum`, và `release.npmTarballName` có mặt khi biết cho
  artifact phát hành.
- `trust.scanStatus` là trạng thái tin cậy hiệu lực được dẫn xuất từ đầu vào của bộ quét
...
- `trust.blockedFromDownload` là tín hiệu chặn cài đặt. OpenClaw và các
  ...
  dẫn xuất lại quy tắc chặn từ các trường bộ quét hoặc điều phối.
...
- `trust.stale` nghĩa là tóm tắt tin cậy được tính từ đầu vào lỗi thời và
...
Trả về mức độ sẵn sàng đã tính cho việc tiêu thụ OpenClaw trong tương lai.

Các kiểm tra mức độ sẵn sàng bao gồm:

- trạng thái kênh chính thức
- khả dụng của phiên bản mới nhất
- khả dụng của artifact npm-pack ClawPack
- digest artifact
...
- `packageName` được chuẩn hóa theo tên npm; gói có thể thiếu đối với các di chuyển
  đã lên kế hoạch.
...
- Yêu cầu token API của chủ sở hữu gói, thành viên nhà xuất bản, điều phối viên, hoặc
...
`finalAction: "revoke"` với báo cáo đã xác nhận để áp dụng điều phối bản phát hành trong cùng
...
- Dùng bucket giới hạn tốc độ đọc, không dùng bucket tải xuống.
...
skill hiện tại được hỗ trợ bởi GitHub có lần quét `clean` hoặc `suspicious` và không có
...
- Chuyển giao skill được hỗ trợ bởi GitHub không proxy hoặc mirror byte. Phản hồi JSON
  bao gồm `sourceRef: "public-github"`, `repo`, `commit`, `path`, `contentHash`,
  và `archiveUrl`; trạng thái quét/hiện tại là cổng kiểm tra và không được đưa vào dưới dạng
...
- Trường payload tùy chọn: `migrateOwner`. Khi là `true` cùng với `ownerHandle`, một
  ...
  nhà xuất bản hiện tại và nhà xuất bản đích. Nếu không chọn tham gia, thay đổi chủ sở hữu sẽ
  bị từ chối.
...
- Các trường form được phép là `payload`, các blob `files` lặp lại, hoặc một tham chiếu tarball `clawpack`.
  ...
  luồng upload-url. Xuất bản bằng storage-id đã dàn dựng cũng phải bao gồm
...
- Yêu cầu xuất bản multipart trực tiếp bị giới hạn ở 18MB. Tarball ClawPack có thể
  dùng luồng upload-url tới mức giới hạn tarball 120MB.
- Trường payload tùy chọn: `ownerHandle`. Khi có, chỉ quản trị viên mới được xuất bản thay mặt chủ sở hữu đó.
...
- Chỉ nhà xuất bản tổ chức `openclaw` và các nhà xuất bản cá nhân của thành viên hiện tại trong tổ chức `openclaw`
...
Xóa mềm / khôi phục skill (chủ sở hữu, điều phối viên, hoặc quản trị viên).
...
Xóa mềm do chủ sở hữu khởi tạo sẽ giữ riêng slug trong 30 ngày, sau đó slug có thể được
nhà xuất bản khác nhận. Phản hồi xóa bao gồm `slugReservedUntil` khi thời hạn này áp dụng.
Ẩn do điều phối viên/quản trị viên và xóa vì bảo mật không hết hạn theo cách này.
...
Chỉ dành cho quản trị viên. Đảm bảo tồn tại một nhà xuất bản tổ chức cho một handle. Nếu handle vẫn trỏ đến
người dùng chia sẻ kế thừa/nhà xuất bản cá nhân, điểm cuối sẽ di chuyển nó thành nhà xuất bản tổ chức trước.
Với tổ chức mới tạo, cung cấp `memberHandle`; quản trị viên đang thực hiện không được thêm làm thành viên.
...
Chỉ dành cho quản trị viên. Giữ riêng slug gốc và tên gói cho chủ sở hữu hợp pháp mà không xuất bản
...
Chỉ dành cho quản trị viên. Khôi phục nhà xuất bản cá nhân cho một principal GitHub OAuth thay thế đã được xác minh
mà không chỉnh sửa các hàng tài khoản Convex Auth. Yêu cầu phải nêu cả hai id tài khoản nhà cung cấp GitHub
bất biến; các handle có thể thay đổi chỉ được dùng làm lớp bảo vệ hướng tới người vận hành.
...
GitHub principal. Khôi phục sẽ thất bại theo hướng đóng khi publisher cá nhân hiện tại của người dùng đích
...
bí danh slug của skill, packages, cảnh báo trình kiểm tra package và các hàng digest tìm kiếm phát sinh để
...
Nguồn GitHub skill thuộc phạm vi publisher và được báo cáo là đã kiểm tra thay vì được viết lại.
...
- Cả hai điểm cuối đều yêu cầu xác thực bằng token API và chỉ hoạt động với chủ sở hữu skill.
- `rename` giữ lại slug trước đó làm bí danh chuyển hướng.
- `merge` ẩn mục niêm yết nguồn và chuyển hướng slug nguồn sang mục niêm yết đích.
...
Thêm/xóa một sao (điểm nổi bật). Cả hai điểm cuối đều là idempotent.
...
có giai đoạn đưa tarball ClawPack lên phải gửi id lưu trữ kết quả dưới dạng
