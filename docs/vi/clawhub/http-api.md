...
Các thư mục của bên thứ ba có thể dùng các endpoint đọc công khai để liệt kê hoặc tìm kiếm kỹ năng ClawHub. Vui lòng lưu kết quả vào bộ nhớ đệm, tuân thủ `429`/`Retry-After`, liên kết người dùng trở lại danh sách ClawHub chuẩn (`https://clawhub.ai/<owner>/skills/<slug>`), và tránh ngụ ý rằng ClawHub chứng thực trang bên thứ ba. Đừng cố sao chép nội dung bị ẩn, riêng tư hoặc bị chặn kiểm duyệt ra ngoài bề mặt API công khai.
...
- Các endpoint ghi đã xác thực không nên trả về `Unauthorized` trống khi
...
- Độ phổ biến được scale theo log và có giới hạn trần. Các kỹ năng có tương tác cao có thể xếp thấp hơn khi văn bản truy vấn khớp yếu hơn.
...
Hướng dẫn về khả năng khám phá cho nhà xuất bản:

- Đặt các thuật ngữ người dùng sẽ thật sự tìm kiếm vào tên hiển thị, tóm tắt và thẻ. Chỉ dùng một token slug độc lập khi nó cũng là định danh ổn định bạn muốn giữ.
- Đừng đổi tên slug chỉ để chạy theo một truy vấn, trừ khi slug mới là tên chuẩn dài hạn tốt hơn. Slug cũ trở thành bí danh chuyển hướng, nhưng URL chuẩn, slug được hiển thị và các bản tổng hợp tìm kiếm trong tương lai dùng slug mới.
- Bí danh đổi tên giữ nguyên khả năng phân giải cho URL cũ và cài đặt cũ phân giải qua registry, nhưng xếp hạng tìm kiếm dựa trên siêu dữ liệu kỹ năng chuẩn sau khi việc đổi tên đã được lập chỉ mục. Thống kê hiện có vẫn gắn với kỹ năng.
- Nếu một kỹ năng bất ngờ không hiển thị, trước tiên hãy kiểm tra trạng thái kiểm duyệt bằng `clawhub inspect @owner/slug` khi đã đăng nhập trước khi thay đổi siêu dữ liệu liên quan đến xếp hạng.
...
- `createdAt` ổn định cho các lượt crawl kỹ năng mới; `updated` thay đổi khi kỹ năng hiện có được xuất bản lại.
...
- Slug cũ được tạo bởi luồng đổi tên/hợp nhất chủ sở hữu sẽ phân giải về kỹ năng chuẩn.
...
- `metadata` là `null` nếu kỹ năng không có siêu dữ liệu nền tảng.
...
- Chủ sở hữu và moderator có thể truy cập chi tiết kiểm duyệt cho kỹ năng bị ẩn.
- Bên gọi công khai chỉ nhận `200` cho các kỹ năng hiển thị đã bị gắn cờ.
- Bằng chứng được biên tập lại cho bên gọi công khai và chỉ bao gồm đoạn trích thô cho chủ sở hữu/moderator.

### `POST /api/v1/skills/{slug}/report`

Báo cáo một kỹ năng để moderator xem xét. Báo cáo ở cấp kỹ năng, có thể liên kết
...
Endpoint moderator/admin để tiếp nhận báo cáo kỹ năng.
...
- `moderation` là ảnh chụp nhanh kiểm duyệt ở cấp Skill hiện tại, được lấy từ phiên bản mới nhất.
...
Bản quét đã xuất bản dùng JSON:
...
- Bản quét đã xuất bản yêu cầu quyền truy cập quản lý của chủ sở hữu/nhà xuất bản, hoặc quyền điều hành viên/quản trị viên nền tảng.
- Bản quét đã xuất bản chỉ ghi ngược lại khi `update: true` và bản quét hoàn tất thành công.
...
- Tác vụ quét là bất đồng bộ. Yêu cầu quét thủ công được ưu tiên trước công việc xuất bản/chèn bù thông thường, nhưng việc hoàn tất vẫn phụ thuộc vào tình trạng sẵn sàng của worker.

### `GET /api/v1/skills/-/scan/{scanId}`

Điểm cuối thăm dò đã xác thực cho một bản quét đã gửi.

- Trả về trạng thái đang xếp hàng/đang chạy/thành công/thất bại.
- Trả về `queue.queuedAhead` và `queue.position` khi đang xếp hàng để client có thể hiển thị số lượng bản quét thủ công được ưu tiên đang đứng trước yêu cầu. Hàng đợi rất lớn sẽ bị giới hạn và được báo cáo với `queuedAheadIsEstimate: true`.
...
- Yêu cầu bản quét đã thành công; bản quét chưa kết thúc trả về `409`.
...
- `kind` mặc định là `skill`; dùng `kind=plugin` cho bản quét Plugin/gói.
...
- `ok` chỉ là `true` khi phiên bản đã chọn có Skill Card đã tạo, không bị kiểm duyệt chặn vì mã độc, và xác minh ClawScan là sạch.
...
- `security.signals.dependencyRegistry` được giữ lại để tương thích phản hồi v1, nhưng trình quét sự tồn tại của dependency registry đã bị ngừng dùng và khóa này luôn là `null`.
...
Trả về các kết luận bảo mật rút gọn hiện tại cho đúng các phiên bản Skill. Điểm cuối
...
- `GET /api/v1/code-plugins` và `GET /api/v1/bundle-plugins` vẫn là các bí danh cố định theo nhóm.
...
- Skills được lưu trữ bao gồm các tệp của phiên bản được lưu mới nhất và được liệt kê trong
...
- Các bí danh bộ lọc v1 cũ được ghi trong `GET /api/v1/plugins` cũng được
  chấp nhận.
- Lọc theo danh mục là một bộ lọc API thực, dựa trên các hàng tóm lược danh mục Plugin,
...
- Điểm cuối đọc công khai. Không cần token của chủ sở hữu, nhà xuất bản, điều phối viên, hoặc quản trị viên.
...
- `trust.blockedFromDownload` là tín hiệu chặn cài đặt. OpenClaw và các
  ...
  tự suy diễn lại các quy tắc chặn từ trường trình quét hoặc điều phối.
...
- `trust.stale` nghĩa là bản tóm tắt tin cậy được tính từ đầu vào đã lỗi thời và
  nên được coi là cần làm mới trước khi đưa ra quyết định cho phép với độ tin cậy cao.

Ghi chú:

- Điểm cuối này chính xác theo phiên bản. Máy khách nên gọi nó sau khi phân giải
  phiên bản gói mà chúng định cài đặt, không chỉ sau khi đọc siêu dữ liệu gói mới nhất.
- Các gói riêng tư trả về `404` trừ khi bên gọi có thể đọc nhà xuất bản sở hữu.
- Điểm cuối này được chủ ý giữ hẹp hơn các điểm cuối điều phối của chủ sở hữu/điều phối viên.
...
Trả về siêu dữ liệu phân giải artifact tường minh cho một phiên bản gói.
...
- Các phiên bản ClawPack trả về artifact `npm-pack`, các trường toàn vẹn npm, một
  `tarballUrl`, và URL tương thích ZIP kiểu cũ.
- Đây là bề mặt phân giải của OpenClaw; nó tránh việc đoán định dạng kho lưu trữ từ
  một URL dùng chung.

### `GET /api/v1/packages/{name}/versions/{version}/artifact/download`

Tải xuống artifact phiên bản qua đường dẫn phân giải tường minh.

Ghi chú:

- Các phiên bản ClawPack truyền luồng chính xác các byte `.tgz` npm-pack đã tải lên.
...
- khả dụng của artifact ClawPack npm-pack
- tóm lược artifact
...
Điểm cuối dành cho điều phối viên/quản trị viên cho các hàng đợi rà soát bản phát hành gói.
...
- Yêu cầu token API của chủ sở hữu gói, thành viên publisher, moderator, hoặc
  người dùng admin.
...
- Giới hạn kích thước tệp: 200KB.
- Các lượt quét VirusTotal đang chờ không chặn thao tác đọc; bản phát hành độc hại vẫn có thể bị giữ lại ở nơi khác.
- Gói riêng tư trả về `404` trừ khi bên gọi có quyền đọc publisher sở hữu.
...
- Kho lưu trữ Plugin/gói là tệp zip có gốc `package/` để các client OpenClaw cũ
  tiếp tục hoạt động.
- Tuyến này chỉ giữ định dạng ZIP. Nó không stream tệp ClawPack `.tgz`.
...
- Bàn giao skill dựa trên GitHub không proxy hoặc mirror byte. Phản hồi JSON
  ...
  và `archiveUrl`; trạng thái quét/hiện tại là cổng kiểm soát và không được bao gồm dưới dạng siêu dữ liệu payload thành công.
...
- Trường payload tùy chọn: `migrateOwner`. Khi là `true` cùng với `ownerHandle`, một
  ...
  publisher hiện tại và publisher đích. Nếu không chọn rõ tùy chọn này, thay đổi chủ sở hữu
  sẽ bị từ chối.
...
- Chỉ publisher org `openclaw` và publisher cá nhân của các thành viên org `openclaw`
  hiện tại mới có thể phát hành vào kênh `official`.
- Phát hành thay mặt vẫn xác thực điều kiện kênh official dựa trên tài khoản chủ sở hữu đích.
...
Các lần xóa mềm do chủ sở hữu khởi tạo giữ slug trong 30 ngày, sau đó publisher
khác có thể nhận slug. Phản hồi xóa bao gồm `slugReservedUntil` khi thời hạn này áp dụng.
Các thao tác ẩn của moderator/admin và gỡ bỏ vì bảo mật không hết hạn theo cách này.
...
Chỉ admin. Đảm bảo publisher org tồn tại cho một handle. Nếu handle vẫn trỏ tới
publisher người dùng/cá nhân dùng chung kiểu cũ, điểm cuối sẽ di chuyển nó thành publisher org trước.
Với org mới tạo, cung cấp `memberHandle`; admin đang thao tác không được thêm làm thành viên.
...
Tạo publisher org tự phục vụ có xác thực. Tạo một publisher org mới và thêm
...
bất biến; các handle có thể thay đổi chỉ được dùng như một chốt kiểm tra hướng tới operator.
...
chủ thể GitHub. Quá trình khôi phục sẽ bị chặn an toàn khi nhà xuất bản cá nhân hiện tại của
người dùng đích có Skills, gói hoặc nguồn Skills GitHub.
Quá trình khôi phục cũng di chuyển các trường `ownerUserId` cũ cho Skills của nhà xuất bản
được khôi phục, bí danh slug Skills, gói, cảnh báo trình kiểm tra gói và các hàng tóm tắt tìm kiếm phái sinh để
...
Nguồn Skills GitHub thuộc phạm vi nhà xuất bản và được báo cáo là đã kiểm tra thay vì được ghi lại.
...
- `rename` giữ lại slug trước đó dưới dạng bí danh chuyển hướng.
...
Cấm một người dùng và xóa vĩnh viễn Skills do họ sở hữu (chỉ moderator/admin).
...
dựng tạm một tarball ClawPack phải gửi id lưu trữ kết quả dưới dạng
