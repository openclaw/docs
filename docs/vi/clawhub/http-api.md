...
x-i18n:
    generated_at: "2026-07-05T05:12:09Z"
    model: gpt-5.5
...
Các thư mục bên thứ ba có thể dùng các endpoint đọc công khai để liệt kê hoặc tìm kiếm Skills trên ClawHub. Vui lòng lưu cache kết quả, tuân thủ `429`/`Retry-After`, liên kết người dùng quay lại mục ClawHub chính tắc (`https://clawhub.ai/<owner>/skills/<slug>`), và tránh ngụ ý rằng ClawHub xác nhận trang bên thứ ba. Không cố gắng sao chép nội dung ẩn, riêng tư, hoặc bị chặn bởi kiểm duyệt ra ngoài bề mặt API công khai.

Các lối tắt slug trên web phân giải xuyên suốt các họ registry, nhưng client API nên dùng
...
- Các endpoint ghi đã xác thực không nên trả về `Unauthorized` trần khi
  ...
  tài khoản đã xóa/bị cấm/bị vô hiệu hóa nên nhận được văn bản có thể hành động để client
...
- `X-RateLimit-Remaining` / `RateLimit-Remaining`: ngân sách còn lại chính xác khi có mặt.
  Các yêu cầu thành công đã được phân mảnh bỏ qua header này thay vì trả về giá trị toàn cục xấp xỉ.
- `Retry-After`: số giây cần đợi trước khi thử lại (độ trễ) khi gặp `429`
...
- Nếu `Retry-After` tồn tại, hãy đợi số giây đó trước khi thử lại.
...
- Dùng các header IP client đáng tin cậy, bao gồm `cf-connecting-ip`, chỉ khi
  bản triển khai bật rõ ràng các header chuyển tiếp đáng tin cậy.
...
- Nếu không có IP client đáng tin cậy, các yêu cầu ẩn danh dùng bucket dự phòng
  ...
  đường dẫn, slug, tên gói, phiên bản, chuỗi truy vấn do bên gọi cung cấp, hoặc các
...
quyền (`401`/`403`), giới hạn tốc độ (`429`), và các lượt tải xuống bị chặn. Client
...
- Kết quả được trả về theo thứ tự liên quan (độ tương đồng embedding + tăng hạng token slug/tên khớp chính xác + một yếu tố phổ biến nhỏ).
...
- Độ phổ biến được chia theo thang log và bị giới hạn trần. Skills có mức tương tác cao có thể xếp hạng thấp hơn khi văn bản truy vấn khớp yếu hơn.
...
- Đặt các thuật ngữ mà người dùng sẽ thực sự tìm kiếm trong tên hiển thị, phần tóm tắt, và thẻ. Chỉ dùng token slug độc lập khi đó cũng là một danh tính ổn định mà bạn muốn giữ.
...
- Alias đổi tên giữ khả năng phân giải cho URL cũ và lượt cài đặt cũ phân giải qua registry, nhưng xếp hạng tìm kiếm dựa trên metadata Skill chính tắc sau khi việc đổi tên đã được lập chỉ mục. Thống kê hiện có vẫn đi cùng Skill.
- Nếu một Skill bất ngờ không hiển thị, hãy kiểm tra trạng thái kiểm duyệt trước bằng `clawhub inspect @owner/slug` trong khi đã đăng nhập trước khi thay đổi metadata liên quan đến xếp hạng.
...
- `recommended` dùng tín hiệu tương tác và độ gần đây.
...
- Slug cũ được tạo bởi các luồng đổi tên/hợp nhất chủ sở hữu phân giải đến Skill chính tắc.
...
- Chủ sở hữu và người kiểm duyệt có thể truy cập chi tiết kiểm duyệt cho Skills ẩn.
...
Báo cáo một Skill để người kiểm duyệt xem xét. Báo cáo ở cấp Skill, tùy chọn được liên kết
...
Endpoint người kiểm duyệt/quản trị viên để tiếp nhận báo cáo Skill.
...
Endpoint người kiểm duyệt/quản trị viên để giải quyết hoặc mở lại báo cáo Skill.
...
- Bao gồm trạng thái xác minh đã chuẩn hóa cùng các chi tiết riêng của từng trình quét.
...
- Payload yêu cầu quét và báo cáo có thể tải xuống sẽ hết hạn khỏi kho yêu cầu quét sau cửa sổ lưu giữ.
- Quét đã phát hành yêu cầu quyền truy cập quản lý của chủ sở hữu/nhà phát hành, hoặc quyền kiểm duyệt viên/quản trị viên nền tảng.
...
- Tác vụ quét là bất đồng bộ. Các yêu cầu quét thủ công được ưu tiên trước công việc phát hành/điền bù thông thường, nhưng thời điểm hoàn tất vẫn phụ thuộc vào khả năng sẵn sàng của worker.
...
- Trả về `queue.queuedAhead` và `queue.position` khi đang xếp hàng để client có thể hiển thị số lượng lượt quét thủ công ưu tiên đang ở trước yêu cầu. Các hàng đợi rất lớn được giới hạn và báo cáo với `queuedAheadIsEstimate: true`.
...
- Yêu cầu quyền truy cập quản lý của chủ sở hữu/nhà phát hành đối với kỹ năng hoặc Plugin, hoặc quyền kiểm duyệt viên/quản trị viên nền tảng.
...
- Trả về cùng cấu trúc ZIP như các bản tải xuống theo yêu cầu quét.

### `POST /api/v1/skills/-/scan/batch`

Tuyến quét lại hàng loạt chuẩn chỉ dành cho quản trị viên. Tuyến này chấp nhận cùng cấu trúc payload như `POST /api/v1/skills/-/rescan-batch` cũ.
...
- `ok` chỉ là `true` khi phiên bản đã chọn có Thẻ kỹ năng đã được tạo, không bị kiểm duyệt chặn vì phần mềm độc hại, và xác minh ClawScan là sạch.
- Danh tính kỹ năng, danh tính nhà phát hành, và siêu dữ liệu phiên bản đã chọn là các trường phong bì cấp cao nhất (`slug`, `displayName`, `publisherHandle`, `version`, `resolvedFrom`, `tag`, `createdAt`) để tự động hóa shell có thể đọc chúng mà không cần mở các lớp bao lồng nhau.
...
Trả về các kết luận bảo mật rút gọn hiện tại cho đúng các phiên bản kỹ năng. Endpoint bộ sưu tập này dành cho các client đã biết những phiên bản kỹ năng ClawHub đã cài đặt nào cần hiển thị, chẳng hạn như OpenClaw Control UI.
...
- Phản hồi chỉ liên quan đến bảo mật. Phản hồi không bao gồm dữ liệu Thẻ kỹ năng, trạng thái thẻ đã tạo, danh sách tệp tạo phẩm, hoặc payload chi tiết của trình quét.
...
- `security.signals.dependencyRegistry` được giữ lại để tương thích phản hồi v1, nhưng trình quét sự tồn tại của sổ đăng ký phụ thuộc đã bị ngừng sử dụng và khóa này luôn là `null`.
- Việc thiếu Thẻ kỹ năng không ảnh hưởng đến `ok`, `decision`, hoặc `reasons` của endpoint này; client nên đọc `skill-card.md` đã cài đặt tại cục bộ khi cần nội dung thẻ.
...
- `sort` (tùy chọn): `updated` (mặc định), `recommended`, `trending`, `downloads`, bí danh kế thừa `installs`
- `category` (tùy chọn): bộ lọc danh mục Plugin. Chỉ được hỗ trợ khi
  ...
  bí danh bộ lọc v1 kế thừa được ghi tài liệu trong `GET /api/v1/plugins`.
...
- `cursor` (tùy chọn): con trỏ phân trang
- `isOfficial` (tùy chọn): `true` hoặc `false`
- `sort` (tùy chọn): `recommended` (mặc định), `trending`, `downloads`, `updated`, bí danh kế thừa `installs`
- `category` (tùy chọn): bộ lọc danh mục Plugin. Giá trị hiện tại:
  `channels`, `models`, `memory`, `context`, `voice`, `media`, `web`,
  `tools`, `runtime`, `gateway`, `security`, `other`.

Bí danh bộ lọc v1 kế thừa vẫn được chấp nhận trên các endpoint đọc:
...
Trên endpoint hợp nhất `/api/v1/packages`, giá trị này chỉ dành cho Plugin; dùng
`/api/v1/skills?sort=trending` cho danh mục Skills.

Bí danh kế thừa không được chấp nhận làm giá trị danh mục được lưu trữ hoặc do tác giả khai báo.
...
- Skills hiện tại dựa trên GitHub có lần quét `clean` hoặc `suspicious` bao gồm
  ...
  hàm băm nội dung, và URL lưu trữ. Chúng không bao gồm các tệp nguồn do ClawHub lưu trữ.
...
- `_errors.json` được đưa vào khi không thể xuất từng Plugin hoặc tệp riêng lẻ.
...
- Yêu cầu token API cho chủ sở hữu gói, chủ sở hữu/quản trị viên nhà xuất bản tổ chức,
...
- `version.artifact.kind` là `legacy-zip` cho kho lưu trữ gói thế hệ cũ hoặc
...
Trả về tóm tắt bảo mật và độ tin cậy chính xác của bản phát hành gói cho
...
- Điểm cuối đọc công khai. Không yêu cầu token của chủ sở hữu, nhà xuất bản, điều phối viên, hoặc quản trị viên.
...
- `trust.blockedFromDownload` là tín hiệu chặn cài đặt. OpenClaw và các
  ...
  tự suy ra lại quy tắc chặn từ các trường trình quét hoặc điều phối.
...
- `trust.stale` nghĩa là tóm tắt độ tin cậy được tính từ đầu vào lỗi thời và
...
- Điểm cuối này được cố ý thu hẹp hơn các điểm cuối điều phối của chủ sở hữu/điều phối viên.
  Nó công khai quyết định cài đặt và phần giải thích công khai, không công khai
...
- Các phiên bản gói cũ trả về artifact `legacy-zip` và `downloadUrl` ZIP cũ.
- Các phiên bản ClawPack trả về artifact `npm-pack`, các trường integrity npm, `tarballUrl`, và URL tương thích ZIP cũ.
- Đây là bề mặt resolver của OpenClaw; nó tránh suy đoán định dạng kho lưu trữ từ một URL dùng chung.

### `GET /api/v1/packages/{name}/versions/{version}/artifact/download`

Tải xuống artifact phiên bản thông qua đường dẫn resolver tường minh.

Ghi chú:

- Các phiên bản ClawPack stream chính xác các byte `.tgz` của npm-pack đã tải lên.
...
Điểm cuối moderator để liệt kê các hàng di chuyển Plugin OpenClaw chính thức.
...
Điểm cuối admin để tạo hoặc cập nhật một hàng di chuyển Plugin chính thức.
...
- `packageName` được chuẩn hóa theo tên npm; gói có thể bị thiếu đối với các di chuyển đã lên kế hoạch.
- Nội dung này chỉ theo dõi mức độ sẵn sàng di chuyển. Nó không thay đổi OpenClaw hoặc tạo ClawPack.

### `GET /api/v1/packages/moderation/queue`

Điểm cuối moderator/admin cho hàng đợi đánh giá bản phát hành gói.
...
- `manual`: bất kỳ bản phát hành nào có override kiểm duyệt thủ công.
- `all`: bất kỳ bản phát hành nào có override thủ công, trạng thái quét không sạch, hoặc báo cáo gói.
...
Báo cáo một gói để moderator đánh giá. Báo cáo ở cấp gói, có thể tùy chọn liên kết với một phiên bản. Chúng đưa dữ liệu vào hàng đợi kiểm duyệt nhưng tự chúng không tự động ẩn hoặc chặn tải xuống; moderator nên sử dụng kiểm duyệt bản phát hành để phê duyệt, cách ly, hoặc thu hồi artifact.
...
Điểm cuối của chủ sở hữu/moderator để hiển thị thông tin kiểm duyệt gói.

Xác thực:

- Yêu cầu API token cho chủ sở hữu gói, thành viên nhà phát hành, moderator, hoặc
  người dùng admin.
...
`finalAction: "revoke"` với một báo cáo đã xác nhận để áp dụng kiểm duyệt bản phát hành trong cùng
...
- `approved`: đã rà soát thủ công và được phép.
...
- `dist.tarball`, `dist.integrity`, và `dist.shasum` sử dụng các
  trường tương thích với npm để người dùng có thể trỏ npm đến mirror nếu họ chọn.
- Packument của gói scoped hỗ trợ cả `/api/npm/@scope/name` và đường dẫn yêu cầu
  được mã hóa `/api/npm/@scope%2Fname` của npm.

### `GET /api/npm/{package}/-/{tarball}.tgz`

Stream đúng các byte tarball ClawPack đã tải lên cho máy khách mirror npm.
...
- Trường payload tùy chọn: `migrateOwner`. Khi là `true` cùng với `ownerHandle`, một
  ...
  nhà phát hành hiện tại và đích. Nếu không chọn tham gia tùy chọn này, thay đổi chủ sở hữu sẽ
  bị từ chối.
...
- Các trường biểu mẫu được phép là `payload`, các blob `files` lặp lại, hoặc một tham chiếu tarball `clawpack`.
  `clawpack` có thể là blob `.tgz` hoặc id lưu trữ do luồng upload-url trả về.
  Các lượt xuất bản bằng storage-id đã staged cũng phải bao gồm
...
- Gói Plugin yêu cầu `openclaw.plugin.json`. Lượt tải lên ClawPack `.tgz` phải
...
- Chỉ nhà phát hành org `openclaw` và nhà phát hành cá nhân của các thành viên org `openclaw`
  hiện tại mới có thể xuất bản lên kênh `official`.
- Xuất bản thay mặt vẫn xác thực đủ điều kiện kênh official dựa trên tài khoản chủ sở hữu đích.
...
Xóa mềm do chủ sở hữu khởi tạo giữ trước slug trong 30 ngày, sau đó slug có thể được
nhà phát hành khác xác nhận.
Phản hồi xóa bao gồm `slugReservedUntil` khi thời hạn này áp dụng.
...
Chỉ admin. Đảm bảo một nhà phát hành org tồn tại cho một handle. Nếu handle vẫn trỏ đến một
nhà phát hành người dùng/cá nhân dùng chung kế thừa, điểm cuối sẽ di chuyển nó thành một nhà phát hành org trước.
...
Chỉ admin. Giữ trước root slug và tên gói cho chủ sở hữu hợp pháp mà không xuất bản
bản phát hành. Tên gói trở thành gói placeholder riêng tư không có hàng bản phát hành, để cùng
chủ sở hữu sau đó có thể xuất bản bản phát hành code-plugin hoặc bundle-plugin thực sự vào tên đó.
...
Chỉ admin. Khôi phục nhà phát hành cá nhân cho một principal GitHub OAuth thay thế đã xác minh
mà không chỉnh sửa các hàng tài khoản Convex Auth. Yêu cầu phải nêu cả hai id tài khoản provider GitHub
bất biến; handle có thể thay đổi chỉ được dùng làm chốt kiểm tra hướng tới operator.

Endpoint mặc định ở chế độ chạy thử. Để áp dụng khôi phục, cần `dryRun: false` và
`confirmIdentityVerified: true` sau khi nhân viên xác minh độc lập tính liên tục giữa cả hai
chủ thể GitHub. Quá trình khôi phục sẽ fail closed khi publisher cá nhân hiện tại của người dùng đích
có skills, packages hoặc nguồn skill GitHub.
Quá trình khôi phục cũng di chuyển các trường `ownerUserId` cũ cho skills của publisher được khôi phục,
bí danh slug skill, packages, cảnh báo trình kiểm tra package và các hàng digest tìm kiếm dẫn xuất để
các đường dẫn chủ sở hữu trực tiếp khớp với quyền publisher mới. Một đặt trước protected-handle đang hoạt động
cho handle được khôi phục cũng được gán lại cho người dùng thay thế để lần đồng bộ hóa hồ sơ sau này
không thể khôi phục quyền cạnh tranh của người dùng trước đó. Mỗi bảng chính được giới hạn ở
100 hàng cho mỗi giao dịch áp dụng; các đợt khôi phục lớn hơn trước tiên phải dùng một quá trình di chuyển chủ sở hữu có thể tiếp tục.
Nguồn skill GitHub có phạm vi theo publisher và được báo cáo là đã kiểm tra thay vì được ghi lại.
...
- Cả hai endpoint đều yêu cầu xác thực bằng token API và chỉ hoạt động cho chủ sở hữu skill.
...
`POST /api/cli/upload-url` trả về `uploadUrl` và `uploadTicket`. Package
publish có giai đoạn tải lên tarball ClawPack phải gửi id lưu trữ kết quả dưới dạng
