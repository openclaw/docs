...
x-i18n:
    generated_at: "2026-06-30T14:08:56Z"
    model: gpt-5.5
...
Các thư mục của bên thứ ba có thể dùng các endpoint đọc công khai để liệt kê hoặc tìm kiếm Skills trên ClawHub. Vui lòng lưu kết quả vào bộ nhớ đệm, tuân thủ `429`/`Retry-After`, liên kết người dùng về mục niêm yết ClawHub chuẩn (`https://clawhub.ai/<owner>/skills/<slug>`), và tránh ngụ ý rằng ClawHub chứng thực trang web bên thứ ba. Không cố gắng sao chép nội dung bị ẩn, riêng tư, hoặc bị chặn kiểm duyệt bên ngoài bề mặt API công khai.
...
- Các endpoint ghi đã xác thực không nên trả về `Unauthorized` trần trụi khi
  ...
  tài khoản đã bị xóa/cấm/vô hiệu hóa đều nên nhận văn bản có thể hành động để client
...
- Dùng các header IP client đáng tin cậy, bao gồm `cf-connecting-ip`, chỉ khi
  deployment bật rõ ràng các header chuyển tiếp đáng tin cậy.
...
- Nếu không có IP client đáng tin cậy, các yêu cầu ẩn danh dùng bucket dự phòng
  ...
  đường dẫn, slug, tên gói, phiên bản, chuỗi truy vấn, hoặc các tham số artifact khác
  do bên gọi cung cấp.
...
bỏ qua để tương thích, nhưng tham số truy vấn được nhận biết với giá trị không hợp lệ sẽ trả về
`400`.
...
- Kết quả được trả về theo thứ tự liên quan (độ tương đồng embedding + tăng hạng token slug/tên khớp chính xác + một ưu tiên nhỏ về độ phổ biến).
- Độ liên quan mạnh hơn độ phổ biến. Một slug chính xác hoặc token tên hiển thị khớp có thể xếp trên một kết quả khớp lỏng hơn dù có mức tương tác mạnh hơn nhiều.
- Văn bản ASCII được tách token theo ranh giới từ và dấu câu. Ví dụ, `personal-map` chứa một token độc lập `map`, trong khi `amap-jsapi-skill` chứa `amap`, `jsapi`, và `skill`; vì vậy tìm kiếm `map` cho `personal-map` một kết quả khớp từ vựng mạnh hơn `amap-jsapi-skill`.
- Độ phổ biến được co theo log và giới hạn trần. Skills có mức tương tác cao có thể xếp thấp hơn khi văn bản truy vấn khớp yếu hơn.
...
- Đặt các thuật ngữ mà người dùng sẽ thực sự tìm kiếm trong tên hiển thị, tóm tắt, và thẻ. Chỉ dùng token slug độc lập khi nó cũng là một định danh ổn định mà bạn muốn giữ.
- Không đổi tên slug chỉ để theo đuổi một truy vấn trừ khi slug mới là tên chuẩn dài hạn tốt hơn. Slug cũ trở thành bí danh chuyển hướng, nhưng URL chuẩn, slug hiển thị, và các bản tóm tắt tìm kiếm tương lai dùng slug mới.
- Bí danh đổi tên giữ khả năng phân giải cho URL cũ và lượt cài đặt phân giải qua registry, nhưng thứ hạng tìm kiếm dựa trên metadata Skill chuẩn sau khi việc đổi tên đã được lập chỉ mục. Thống kê hiện có vẫn đi cùng Skill.
- Nếu một Skill bị ẩn ngoài dự kiến, hãy kiểm tra trạng thái kiểm duyệt trước bằng `clawhub inspect @owner/slug` khi đã đăng nhập trước khi thay đổi metadata liên quan đến xếp hạng.
...
- `cursor` (tùy chọn): con trỏ phân trang cho bất kỳ cách sắp xếp không phải `trending`
...
- Slug cũ được tạo bởi các luồng đổi tên/hợp nhất chủ sở hữu phân giải đến Skill chuẩn.
- `metadata.os`: ràng buộc hệ điều hành được khai báo trong frontmatter của Skill (ví dụ `["macos"]`, `["linux"]`). `null` nếu không được khai báo.
...
- `moderation` chỉ được đưa vào khi Skill bị gắn cờ hoặc chủ sở hữu đang xem nó.
...
- Chủ sở hữu và người kiểm duyệt có thể truy cập chi tiết kiểm duyệt cho Skills bị ẩn.
...
- `moderation` là ảnh chụp kiểm duyệt hiện tại ở cấp skill, được lấy từ phiên bản mới nhất.
...
- Tải trọng yêu cầu quét và báo cáo có thể tải xuống sẽ hết hạn khỏi kho lưu trữ yêu cầu quét sau thời hạn lưu giữ.
- Quét bản đã phát hành yêu cầu quyền truy cập quản lý của chủ sở hữu/nhà phát hành, hoặc quyền điều hành viên/quản trị viên nền tảng.
- Quét bản đã phát hành chỉ ghi ngược lại khi `update: true` và quá trình quét hoàn tất thành công.
...
- Tác vụ quét là bất đồng bộ. Yêu cầu quét thủ công được ưu tiên trước công việc phát hành/điền bù thông thường, nhưng việc hoàn tất vẫn phụ thuộc vào mức độ sẵn sàng của worker.
...
- Trả về `queue.queuedAhead` và `queue.position` khi đang xếp hàng để client có thể hiển thị số lượt quét thủ công được ưu tiên đang đứng trước yêu cầu. Hàng đợi rất lớn sẽ được giới hạn và báo cáo với `queuedAheadIsEstimate: true`.
...
- Yêu cầu lượt quét đã thành công; lượt quét chưa kết thúc trả về `409`.
...
- Yêu cầu quyền truy cập quản lý của chủ sở hữu/nhà phát hành đối với skill hoặc plugin, hoặc quyền điều hành viên/quản trị viên nền tảng.
...
- Trả về cùng cấu trúc ZIP như bản tải xuống của yêu cầu quét.

### `POST /api/v1/skills/-/scan/batch`

Tuyến quét lại hàng loạt chuẩn chỉ dành cho quản trị viên. Tuyến này chấp nhận cùng cấu trúc tải trọng như `POST /api/v1/skills/-/rescan-batch` cũ.
...
- Danh tính skill, danh tính nhà phát hành và siêu dữ liệu phiên bản đã chọn là các trường phong bì cấp cao nhất (`slug`, `displayName`, `publisherHandle`, `version`, `resolvedFrom`, `tag`, `createdAt`) để tự động hóa shell có thể đọc chúng mà không cần giải nén các lớp bọc lồng nhau.
...
- `provenance` chỉ là `server-resolved-github-import` khi ClawHub đã phân giải và lưu repo/ref/commit/path GitHub trong lúc phát hành hoặc nhập; nếu không thì là `unavailable`.

### `POST /api/v1/skills/-/security-verdicts`

Trả về các phán quyết bảo mật rút gọn hiện tại cho đúng phiên bản skill. Điểm cuối bộ sưu tập này dành cho các client đã biết cần hiển thị những phiên bản skill ClawHub đã cài đặt nào, chẳng hạn như OpenClaw Control UI.
...
- Phản hồi chỉ liên quan đến bảo mật. Nó không bao gồm dữ liệu Thẻ Skill, trạng thái thẻ được tạo, danh sách tệp tạo tác, hoặc tải trọng chi tiết của trình quét.
...
- `security.signals.dependencyRegistry` được giữ lại để tương thích phản hồi v1, nhưng trình quét sự tồn tại của sổ đăng ký phụ thuộc đã ngừng hoạt động và khóa này luôn là `null`.
- Việc thiếu Thẻ Skill không ảnh hưởng đến `ok`, `decision` hoặc `reasons` của điểm cuối này; client nên đọc `skill-card.md` đã cài đặt cục bộ khi cần nội dung thẻ.
...
- Các mục kỹ năng vẫn được hỗ trợ bởi sổ đăng ký kỹ năng và vẫn chỉ có thể được phát hành thông qua `POST /api/v1/skills`.
...
- Người gọi đã xác thực có thể thấy các gói riêng tư cho các nhà phát hành mà họ thuộc về trong kết quả liệt kê/tìm kiếm.
- `channel=private` chỉ trả về các gói mà người gọi đã xác thực có thể đọc.

### `GET /api/v1/packages/search`

Tìm kiếm danh mục hợp nhất trên kỹ năng + các gói plugin.
...
- Người gọi ẩn danh chỉ thấy các kênh gói công khai.
- Người gọi đã xác thực có thể tìm kiếm các gói riêng tư cho các nhà phát hành mà họ thuộc về.
- `channel=private` chỉ trả về các gói mà người gọi đã xác thực có thể đọc.
...
- Các bí danh bộ lọc v1 cũ được ghi tài liệu dưới `GET /api/v1/plugins` cũng
  được chấp nhận.
- Lọc danh mục là một bộ lọc API thực sự được hỗ trợ bởi các hàng digest danh mục
...
- `release.artifactKind`, `release.artifactSha256`, `release.npmIntegrity`,
  `release.npmShasum` và `release.npmTarballName` có mặt khi biết được thông tin của
  artifact phát hành.
...
- `trust.moderationState` có thể là null. Giá trị là `null` khi không có điều phối bản phát hành
  thủ công.
...
Tải xuống artifact phiên bản qua đường dẫn trình phân giải tường minh.
...
Điểm cuối dành cho điều phối viên/quản trị viên cho hàng đợi rà soát bản phát hành gói.
...
`finalAction: "revoke"` cùng với báo cáo đã xác nhận để áp dụng điều phối bản phát hành trong
...
- Kho lưu trữ Plugin/gói là tệp zip có gốc `package/` để các máy khách OpenClaw cũ
  tiếp tục hoạt động.
- Tuyến này chỉ giữ ZIP. Nó không truyền trực tuyến các tệp ClawPack `.tgz`.
...
Trả về packument tương thích với npm cho các phiên bản gói được ClawPack hỗ trợ.
...
- `dist.tarball`, `dist.integrity`, và `dist.shasum` dùng các trường tương thích với npm
  để người dùng có thể trỏ npm đến mirror nếu họ muốn.
...
skill hiện tại được GitHub hỗ trợ với lượt quét `clean` hoặc `suspicious` và không có
...
- Trường payload tùy chọn: `migrateOwner`. Khi là `true` với `ownerHandle`, một
  ...
  nhà xuất bản hiện tại và nhà xuất bản đích. Nếu không có tùy chọn tham gia này, thay đổi chủ sở hữu sẽ
  bị từ chối.
...
- Các trường biểu mẫu được phép là `payload`, các blob `files` lặp lại, hoặc một tham chiếu tarball
  `clawpack`. `clawpack` có thể là blob `.tgz` hoặc id lưu trữ do
  luồng upload-url trả về. Các bản phát hành bằng storage-id đã staged cũng phải bao gồm
...
- Phát hành thay mặt vẫn xác thực điều kiện đủ cho kênh official dựa trên tài khoản chủ sở hữu đích.
...
Các lượt xóa mềm do chủ sở hữu khởi tạo giữ riêng slug trong 30 ngày, sau đó slug có thể được
một nhà xuất bản khác xác nhận quyền sử dụng. Phản hồi xóa bao gồm `slugReservedUntil` khi áp dụng thời hạn này.
Các lượt ẩn của điều phối viên/quản trị viên và các lượt gỡ bỏ vì bảo mật không hết hạn theo cách này.
...
người dùng/nhà xuất bản cá nhân dùng chung kế thừa, điểm cuối sẽ di chuyển nó vào một nhà xuất bản tổ chức trước.
Đối với tổ chức mới tạo, hãy cung cấp `memberHandle`; quản trị viên đang thực hiện không được thêm làm thành viên.
...
Chỉ dành cho quản trị viên. Khôi phục một nhà xuất bản cá nhân cho một danh tính chính GitHub OAuth thay thế đã xác minh
mà không chỉnh sửa các hàng tài khoản Convex Auth. Yêu cầu phải nêu tên cả hai id tài khoản nhà cung cấp GitHub
bất biến; các handle có thể thay đổi chỉ được dùng như một chốt chặn hướng tới người vận hành.
...
danh tính chính GitHub. Khôi phục sẽ thất bại theo hướng đóng khi nhà phát hành cá nhân hiện tại
...
bí danh slug skill, packages, cảnh báo trình kiểm tra package và các hàng bản tóm tắt tìm kiếm phát sinh để
các đường dẫn chủ sở hữu trực tiếp khớp với thẩm quyền nhà phát hành mới. Một đặt trước handle được bảo vệ đang hoạt động
cho handle được khôi phục cũng được gán lại cho người dùng thay thế để đồng bộ hóa hồ sơ sau này
...
Nguồn GitHub skill có phạm vi theo nhà phát hành và được báo cáo là đã kiểm tra thay vì được ghi lại.
...
- `merge` ẩn mục liệt kê nguồn và chuyển hướng slug nguồn sang mục liệt kê đích.
...
Cấm một người dùng và xóa vĩnh viễn skills thuộc sở hữu (chỉ moderator/admin).
...
Thay đổi lý do được lưu cho một lệnh cấm hiện có mà không bỏ cấm hoặc khôi phục
...
đưa tarball ClawPack vào giai đoạn phải gửi id lưu trữ thu được dưới dạng
...
Nếu bạn tự lưu trữ, hãy phục vụ tệp này (hoặc đặt `CLAWHUB_REGISTRY` một cách tường minh; `CLAWDHUB_REGISTRY` cũ).
