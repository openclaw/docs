...
summary: Tham khảo HTTP API (công khai + endpoint CLI + xác thực).
x-i18n:
...
Các thư mục của bên thứ ba có thể dùng các endpoint đọc công khai để liệt kê hoặc tìm kiếm ClawHub skills. Vui lòng lưu đệm kết quả, tuân thủ `429`/`Retry-After`, liên kết người dùng trở lại mục ClawHub chuẩn (`https://clawhub.ai/<owner>/skills/<slug>`), và tránh ngụ ý rằng ClawHub xác nhận trang bên thứ ba. Không cố gắng sao chép nội dung ẩn, riêng tư, hoặc bị chặn kiểm duyệt ra ngoài bề mặt API công khai.

Các lối tắt slug web phân giải trên nhiều họ registry, nhưng client API nên dùng
các URL chuẩn do endpoint đọc trả về thay vì tự dựng lại thứ tự ưu tiên định tuyến.
...
- Các endpoint ghi đã xác thực không nên trả về một `Unauthorized` trần khi
  ...
  tài khoản đã bị xóa/cấm/vô hiệu hóa nên đều nhận văn bản có thể hành động để
...
- Nếu thiếu `Retry-After`, dùng dự phòng `RateLimit-Reset` (hoặc tính từ `X-RateLimit-Reset`).
...
- `highlightedOnly` (tùy chọn): `true` để lọc còn các skill được làm nổi bật
- `nonSuspiciousOnly` (tùy chọn): `true` để ẩn skill đáng ngờ (`flagged.suspicious`)
...
- Kết quả được trả về theo thứ tự liên quan (độ tương đồng embedding + tăng hạng token slug/tên chính xác + một prior phổ biến nhỏ).
- Độ liên quan mạnh hơn độ phổ biến. Một kết quả khớp token slug hoặc tên hiển thị chính xác có thể xếp trên một kết quả khớp lỏng hơn nhưng có mức tương tác mạnh hơn nhiều.
- Văn bản ASCII được tách token theo ranh giới từ và dấu câu. Ví dụ, `personal-map` chứa một token `map` độc lập, trong khi `amap-jsapi-skill` chứa `amap`, `jsapi`, và `skill`; vì vậy tìm kiếm `map` cho `personal-map` một kết quả khớp từ vựng mạnh hơn `amap-jsapi-skill`.
- Độ phổ biến được co theo log và có giới hạn trần. Skill có mức tương tác cao có thể xếp thấp hơn khi văn bản truy vấn khớp yếu hơn.
...
- Đặt các thuật ngữ người dùng sẽ tìm kiếm đúng nguyên văn vào tên hiển thị, tóm tắt, và thẻ. Chỉ dùng token slug độc lập khi nó cũng là một định danh ổn định mà bạn muốn giữ.
- Không đổi tên slug chỉ để bám theo một truy vấn trừ khi slug mới là tên chuẩn dài hạn tốt hơn. Slug cũ trở thành alias chuyển hướng, nhưng URL chuẩn, slug được hiển thị, và bản tóm tắt tìm kiếm trong tương lai dùng slug mới.
- Alias đổi tên giữ khả năng phân giải cho URL cũ và các lượt cài đặt phân giải qua registry, nhưng xếp hạng tìm kiếm dựa trên metadata skill chuẩn sau khi đổi tên đã được lập chỉ mục. Thống kê hiện có vẫn đi cùng skill.
...
- Khi `nonSuspiciousOnly=true`, các kiểu sắp xếp dựa trên con trỏ có thể trả về ít mục hơn `limit` trên một trang vì skill đáng ngờ được lọc sau khi truy xuất trang.
- Dùng `nextCursor` để tiếp tục phân trang khi có mặt. Một trang ngắn tự nó không có nghĩa là hết kết quả.
...
- Owner và moderator có thể truy cập chi tiết kiểm duyệt cho skill bị ẩn.
...
- Nếu không cung cấp `version` hoặc `tag`, hệ thống sẽ dùng phiên bản mới nhất.
- Bao gồm trạng thái xác minh đã chuẩn hóa cùng với các chi tiết riêng theo trình quét.
- `security.hasScanResult` chỉ là `true` khi một trình quét tạo ra kết luận xác định (`clean`, `suspicious`, hoặc `malicious`).
- `moderation` là ảnh chụp kiểm duyệt hiện tại ở cấp kỹ năng, được suy ra từ phiên bản mới nhất.
...
- Các lượt quét đã xuất bản yêu cầu quyền truy cập quản lý của chủ sở hữu/nhà xuất bản, hoặc quyền của điều hành viên/quản trị viên nền tảng.
...
- Tác vụ quét là bất đồng bộ. Các yêu cầu quét thủ công được ưu tiên trước công việc xuất bản/điền bù thông thường, nhưng việc hoàn tất vẫn phụ thuộc vào khả năng sẵn sàng của worker.

### `GET /api/v1/skills/-/scan/{scanId}`

Điểm cuối thăm dò đã xác thực cho lượt quét đã gửi.

- Trả về trạng thái đã xếp hàng/đang chạy/thành công/thất bại.
- Trả về `queue.queuedAhead` và `queue.position` trong khi đang xếp hàng để client có thể hiển thị số lượt quét thủ công ưu tiên đang đứng trước yêu cầu. Các hàng đợi rất lớn được giới hạn và báo cáo với `queuedAheadIsEstimate: true`.
...
- Yêu cầu quyền truy cập quản lý của chủ sở hữu/nhà xuất bản đối với kỹ năng hoặc plugin, hoặc quyền của điều hành viên/quản trị viên nền tảng.
...
- Trả về cùng dạng ZIP như các bản tải xuống từ yêu cầu quét.
...
Trả về bao xác minh Thẻ kỹ năng được `clawhub skill verify` dùng.
...
- `ok` chỉ là `true` khi phiên bản đã chọn có Thẻ kỹ năng đã tạo, không bị kiểm duyệt chặn vì phần mềm độc hại, và xác minh ClawScan là sạch.
- Danh tính kỹ năng, danh tính nhà xuất bản, và siêu dữ liệu phiên bản đã chọn là các trường bao cấp cao nhất (`slug`, `displayName`, `publisherHandle`, `version`, `resolvedFrom`, `tag`, `createdAt`) để tự động hóa shell có thể đọc chúng mà không cần mở các wrapper lồng nhau.
...
- `security.signals.dependencyRegistry` được giữ lại để tương thích phản hồi v1, nhưng trình quét tồn tại registry phụ thuộc đã ngừng hoạt động và khóa này luôn là `null`.
...
Trả về các kết luận bảo mật rút gọn hiện tại cho đúng các phiên bản kỹ năng. Điểm cuối bộ sưu tập này dành cho các client đã biết những phiên bản kỹ năng ClawHub đã cài đặt mà chúng cần hiển thị, chẳng hạn như OpenClaw Control UI.
...
- Phản hồi chỉ liên quan đến bảo mật. Nó không bao gồm dữ liệu Thẻ kỹ năng, trạng thái thẻ đã tạo, danh sách tệp artifact, hoặc payload trình quét chi tiết.
...
- `security.signals.dependencyRegistry` được giữ lại để tương thích phản hồi v1, nhưng trình quét tồn tại registry phụ thuộc đã ngừng hoạt động và khóa này luôn là `null`.
- Việc thiếu Thẻ kỹ năng không ảnh hưởng đến `ok`, `decision`, hoặc `reasons` của điểm cuối này; client nên đọc `skill-card.md` đã cài đặt tại cục bộ khi cần nội dung thẻ.
- Dùng `/verify` khi bạn cần bao xác minh Thẻ kỹ năng cho một kỹ năng, `/card` khi bạn cần markdown thẻ đã tạo, và `/scan` khi bạn cần dữ liệu trình quét chi tiết.
...
- Các mục Skills vẫn được hỗ trợ bởi registry Skills và vẫn chỉ có thể được phát hành qua `POST /api/v1/skills`.
...
- Bên gọi đã xác thực có thể thấy các gói riêng tư của những publisher mà họ thuộc về trong kết quả danh sách/tìm kiếm.
...
- Skills hiện tại được hỗ trợ bởi GitHub với lần quét `clean` hoặc `suspicious` bao gồm
  ...
  hàm băm nội dung, và URL lưu trữ. Chúng không bao gồm các tệp nguồn được ClawHub lưu trữ.
...
- `limit` (tùy chọn): số nguyên (1-250), mặc định `250`.
- `cursor` (tùy chọn): con trỏ phân trang từ phản hồi trước đó.
- `family` (tùy chọn): `code-plugin` hoặc `bundle-plugin`. Nếu bỏ qua thì nghĩa là cả hai họ Plugin.
...
- Lọc theo danh mục là một bộ lọc API thực sự được hỗ trợ bởi các hàng tóm tắt
  danh mục Plugin, không phải là thao tác viết lại truy vấn tìm kiếm.
- Kết quả được trả về theo thứ tự mức độ liên quan và hiện không được phân trang.
- Các điều khiển sắp xếp giao diện trình duyệt cho tìm kiếm Plugin sắp xếp lại
  các kết quả theo mức độ liên quan đã tải, khớp với hành vi duyệt `/skills` hiện tại.
...
- `version.artifact.kind` là `legacy-zip` cho kho lưu trữ gói kiểu cũ hoặc
  `npm-pack` cho bản phát hành được ClawPack hỗ trợ.
...
- Điểm cuối đọc công khai. Không yêu cầu token của chủ sở hữu, nhà phát hành,
  kiểm duyệt viên hoặc quản trị viên.
...
- `release.artifactKind`, `release.artifactSha256`, `release.npmIntegrity`,
  ...
  tạo tác phát hành.
- `trust.scanStatus` là trạng thái tin cậy hiệu dụng được suy ra từ đầu vào trình quét
...
- Gói riêng tư trả về `404` trừ khi bên gọi có thể đọc nhà phát hành sở hữu.
- Điểm cuối này cố ý hẹp hơn các điểm cuối kiểm duyệt của chủ sở hữu/kiểm duyệt viên.
  Nó hiển thị quyết định cài đặt và giải thích công khai, không phải danh tính
  người báo cáo, nội dung báo cáo, bằng chứng riêng tư hoặc dòng thời gian đánh giá nội bộ.
...
- Phiên bản gói cũ trả về một tạo tác `legacy-zip` và `downloadUrl` ZIP cũ.
- Phiên bản ClawPack trả về một tạo tác `npm-pack`, các trường toàn vẹn npm, một
  `tarballUrl` và URL tương thích ZIP cũ.
- Đây là bề mặt bộ phân giải của OpenClaw; nó tránh phải đoán định dạng kho lưu trữ từ
  một URL dùng chung.
...
Điểm cuối kiểm duyệt viên để liệt kê các hàng di trú Plugin OpenClaw chính thức.

Xác thực:

- Yêu cầu token API cho người dùng là kiểm duyệt viên hoặc quản trị viên.
...
- Yêu cầu token API cho người dùng là quản trị viên.
...
- `packageName` được chuẩn hóa theo tên npm; gói có thể thiếu đối với các lần di trú
  đã lên kế hoạch.
- Phần này chỉ theo dõi mức độ sẵn sàng di trú. Nó không thay đổi OpenClaw hoặc tạo
  ClawPack.
...
- `all`: mọi bản phát hành có ghi đè thủ công, trạng thái quét không sạch hoặc báo cáo gói.
...
Báo cáo một gói để kiểm duyệt viên xem xét. Báo cáo ở cấp gói, có thể tùy chọn
...
Endpoint dành cho chủ sở hữu/moderator để xem thông tin kiểm duyệt gói.
...
Endpoint dành cho moderator/admin để xử lý hoặc mở lại báo cáo gói.
...
- Kho lưu trữ Plugin/gói là tệp zip có gốc `package/` để các client OpenClaw cũ
  vẫn hoạt động.
- Tuyến này chỉ hỗ trợ ZIP. Nó không stream tệp ClawPack `.tgz`.
...
Được CLI dùng để ánh xạ dấu vân tay cục bộ đến một phiên bản đã biết.
...
skill hiện tại dựa trên GitHub có lần quét `clean` hoặc `suspicious` và không có phiên bản
được lưu trữ.
...
- Bàn giao skill dựa trên GitHub không proxy hoặc mirror byte. Phản hồi JSON
  ...
  và `archiveUrl`; trạng thái quét/hiện tại là một cổng kiểm tra và không được đưa vào làm siêu dữ liệu payload thành công.
...
- Trường payload không bắt buộc: `migrateOwner`. Khi là `true` cùng với `ownerHandle`, một
  ...
  nhà xuất bản hiện tại và nhà xuất bản đích. Nếu không chọn tham gia điều này, thay đổi chủ sở hữu sẽ
  bị từ chối.
...
- Các trường biểu mẫu được phép là `payload`, các blob `files` lặp lại, hoặc một tham chiếu tarball `clawpack`.
  ...
  Các lần xuất bản bằng storage-id đã staging cũng phải bao gồm
...
- Yêu cầu xuất bản multipart trực tiếp bị giới hạn ở 18MB. Tarball ClawPack có thể
  dùng luồng upload-url lên đến mức trần tarball 120MB.
...
Ẩn bởi moderator/admin và xóa vì bảo mật không hết hạn theo cách này.
...
Chỉ dành cho admin. Đảm bảo một nhà xuất bản tổ chức tồn tại cho một handle. Nếu handle vẫn trỏ đến
người dùng chia sẻ/nhà xuất bản cá nhân kiểu cũ, endpoint sẽ di chuyển nó vào một nhà xuất bản tổ chức trước.
...
bất biến; các handle có thể thay đổi chỉ được dùng làm rào chắn hướng đến operator.

Theo mặc định, endpoint chạy ở chế độ chạy thử. Việc áp dụng khôi phục yêu cầu `dryRun: false` và
`confirmIdentityVerified: true` sau khi nhân viên xác minh độc lập tính liên tục giữa cả hai
chủ thể GitHub. Khôi phục sẽ thất bại theo hướng đóng an toàn khi publisher cá nhân hiện tại của người dùng đích
có Skills, package hoặc nguồn Skill GitHub.
Khôi phục cũng di chuyển các trường `ownerUserId` cũ cho Skills của publisher được khôi phục,
bí danh slug Skill, package, cảnh báo trình kiểm tra package và các hàng bản tóm tắt tìm kiếm dẫn xuất để
...
không thể khôi phục thẩm quyền cạnh tranh của người dùng trước đó. Mỗi bảng chính bị giới hạn ở
...
Nguồn Skill GitHub có phạm vi theo publisher và được báo cáo là đã kiểm tra thay vì được viết lại.
...
Cấm một người dùng và xóa cứng các Skills do họ sở hữu (chỉ moderator/admin).
...
nội dung (chỉ admin). Mặc định chạy thử trừ khi `dryRun` là `false`.
...
Thêm/xóa một sao (mục nổi bật). Cả hai endpoint đều có tính lũy đẳng.
...
tạo tarball ClawPack theo giai đoạn phải gửi id lưu trữ kết quả dưới dạng
