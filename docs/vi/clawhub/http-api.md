...
x-i18n:
    generated_at: "2026-07-05T06:55:55Z"
    model: gpt-5.5
...
Các thư mục của bên thứ ba có thể dùng các endpoint đọc công khai để liệt kê hoặc tìm kiếm Skills trên ClawHub. Vui lòng lưu kết quả vào bộ nhớ đệm, tuân thủ `429`/`Retry-After`, liên kết người dùng trở lại mục ClawHub chuẩn (`https://clawhub.ai/<owner>/skills/<slug>`), và tránh ngụ ý rằng ClawHub xác nhận trang của bên thứ ba. Không cố gắng phản chiếu nội dung bị ẩn, riêng tư hoặc bị chặn kiểm duyệt ra ngoài bề mặt API công khai.

Các lối tắt slug web phân giải trên nhiều họ registry, nhưng client API nên dùng
các URL chuẩn do endpoint đọc trả về thay vì tự dựng lại thứ tự ưu tiên của
tuyến.
...
- Các endpoint ghi đã xác thực không nên trả về trần `Unauthorized` khi
  máy chủ biết lý do. Token bị thiếu, token không hợp lệ/bị thu hồi, và
  tài khoản đã xóa/bị cấm/bị vô hiệu hóa, mỗi trường hợp nên nhận được văn bản có thể hành động để
...
- `X-RateLimit-Remaining` / `RateLimit-Remaining`: ngân sách còn lại chính xác khi có mặt.
  Các yêu cầu thành công được phân mảnh bỏ qua header này thay vì trả về một giá trị toàn cục gần đúng.
...
- Nếu `Retry-After` tồn tại, đợi số giây đó trước khi thử lại.
...
- ClawHub dùng các header chuyển tiếp đáng tin cậy để nhận diện IP client tại edge.
...
nên đọc body phản hồi như một chuỗi dễ đọc cho con người. Các tham số truy vấn không xác định được
...
- Kết quả được trả về theo thứ tự mức độ liên quan (độ tương đồng embedding + tăng hạng token slug/tên khớp chính xác + một ưu tiên phổ biến nhỏ).
- Mức độ liên quan mạnh hơn độ phổ biến. Một slug chính xác hoặc token tên hiển thị khớp có thể xếp hạng cao hơn một kết quả khớp lỏng hơn nhưng có mức độ tương tác mạnh hơn nhiều.
- Văn bản ASCII được tách token theo ranh giới từ và dấu câu. Ví dụ, `personal-map` chứa token `map` độc lập, trong khi `amap-jsapi-skill` chứa `amap`, `jsapi`, và `skill`; do đó tìm kiếm `map` cho `personal-map` một kết quả khớp từ vựng mạnh hơn `amap-jsapi-skill`.
- Độ phổ biến được co theo log và có giới hạn trần. Skills có mức tương tác cao có thể xếp hạng thấp hơn khi văn bản truy vấn khớp yếu hơn.
- Trạng thái kiểm duyệt đáng ngờ hoặc bị ẩn có thể loại bỏ một Skill khỏi tìm kiếm công khai tùy theo bộ lọc của bên gọi và trạng thái kiểm duyệt hiện tại.

Hướng dẫn khả năng được phát hiện cho nhà xuất bản:

- Đưa các thuật ngữ mà người dùng sẽ tìm kiếm đúng nghĩa vào tên hiển thị, tóm tắt và thẻ. Chỉ dùng một token slug độc lập khi nó cũng là danh tính ổn định mà bạn muốn giữ.
- Không đổi tên slug chỉ để chạy theo một truy vấn, trừ khi slug mới là tên chuẩn dài hạn tốt hơn. Slug cũ trở thành bí danh chuyển hướng, nhưng URL chuẩn, slug hiển thị và các bản tóm tắt tìm kiếm trong tương lai dùng slug mới.
- Bí danh đổi tên giữ khả năng phân giải cho URL cũ và cài đặt cũ phân giải qua registry, nhưng xếp hạng tìm kiếm dựa trên siêu dữ liệu Skill chuẩn sau khi đổi tên đã được lập chỉ mục. Thống kê hiện có vẫn gắn với Skill.
...
- `moderation` là ảnh chụp kiểm duyệt ở cấp skill hiện tại, được suy ra từ phiên bản mới nhất.
...
- Các lượt quét đã xuất bản yêu cầu quyền truy cập quản lý của chủ sở hữu/nhà xuất bản, hoặc quyền điều hành viên/quản trị viên nền tảng.
...
- Các tác vụ quét là bất đồng bộ. Yêu cầu quét thủ công được ưu tiên trước công việc xuất bản/điền bù thông thường, nhưng việc hoàn tất vẫn phụ thuộc vào tình trạng sẵn sàng của worker.
...
- Trả về `queue.queuedAhead` và `queue.position` khi đang xếp hàng để client có thể hiển thị số lượt quét thủ công ưu tiên đang đứng trước yêu cầu. Các hàng đợi rất lớn được giới hạn và báo cáo với `queuedAheadIsEstimate: true`.
- Khi có sẵn, `report` chứa các phần `clawscan`, `skillspector`, `staticAnalysis` và `virustotal`.
- Các tác vụ quét thất bại trả về `status: "failed"` cùng `lastError`.

### `GET /api/v1/skills/-/scan/{scanId}/download`

Điểm cuối lưu trữ báo cáo có xác thực.
...
Điểm cuối lưu trữ báo cáo đã lưu có xác thực cho các phiên bản đã gửi.
...
Tuyến quét lại hàng loạt chính tắc chỉ dành cho quản trị viên. Nó chấp nhận cùng cấu trúc payload như `POST /api/v1/skills/-/rescan-batch` cũ.

### `POST /api/v1/skills/-/scan/batch/status`

Tuyến trạng thái hàng loạt chính tắc chỉ dành cho quản trị viên. Nó chấp nhận `{ "jobIds": ["..."] }` và trả về cùng các bộ đếm tổng hợp như `POST /api/v1/skills/-/rescan-batch/status` cũ.
...
Trả về các kết luận bảo mật rút gọn hiện tại cho đúng các phiên bản skill. Điểm cuối bộ sưu tập này dành cho các client đã biết các phiên bản skill ClawHub đã cài đặt mà họ cần hiển thị, chẳng hạn như OpenClaw Control UI.
...
- Kết quả tính theo từng mục; một skill hoặc phiên bản bị thiếu không làm toàn bộ phản hồi thất bại.
...
- Mục Skills vẫn được hỗ trợ bởi registry Skills và vẫn chỉ có thể được phát hành qua `POST /api/v1/skills`.
...
- Bên gọi đã xác thực có thể thấy các gói riêng tư của các publisher mà họ thuộc về trong kết quả danh sách/tìm kiếm.
...
- Bên gọi ẩn danh chỉ thấy các kênh gói công khai.
- Bên gọi đã xác thực có thể tìm kiếm các gói riêng tư của các publisher mà họ thuộc về.
- `channel=private` chỉ trả về các gói mà bên gọi đã xác thực có thể đọc.
...
- `startDate` (bắt buộc): giới hạn dưới Unix milliseconds cho `updatedAt` của Skills.
- `endDate` (bắt buộc): giới hạn trên Unix milliseconds cho `updatedAt` của Skills.
...
- Skills hiện tại được hỗ trợ bởi GitHub có bản quét `clean` hoặc `suspicious` bao gồm
  `_source_handoff.json` với `sourceRef: "public-github"`, repo, commit, path,
  hash nội dung, và URL kho lưu trữ. Chúng không bao gồm các tệp nguồn do ClawHub lưu trữ.
...
- Các bí danh bộ lọc v1 cũ được ghi tài liệu trong `GET /api/v1/plugins` cũng
  được chấp nhận.
- Lọc theo danh mục là bộ lọc API thật, được hỗ trợ bởi các hàng tóm tắt danh mục
...
- Bản phát hành ClawPack bao gồm các trường tương thích npm `npmIntegrity`, `npmShasum`, và
  `npmTarballName`.
- `version.sha256hash` là siêu dữ liệu tương thích đã ngừng khuyến nghị dành cho máy khách cũ. Nó
...
- Điểm cuối đọc công khai. Không yêu cầu mã thông báo của chủ sở hữu, nhà xuất bản, điều hành viên,
  hoặc quản trị viên.
...
- `release.artifactKind`, `release.artifactSha256`, `release.npmIntegrity`,
  ...
  hiện vật phát hành.
- `trust.scanStatus` là trạng thái tin cậy hiệu lực được suy ra từ đầu vào máy quét
...
- `trust.blockedFromDownload` là tín hiệu chặn cài đặt. OpenClaw và các
  ...
  tự suy diễn lại quy tắc chặn từ các trường máy quét hoặc kiểm duyệt.
...
- `trust.stale` nghĩa là tóm tắt tin cậy được tính từ các đầu vào lỗi thời và
...
- Điểm cuối này chính xác theo phiên bản. Máy khách nên gọi nó sau khi phân giải
  ...
  mới nhất.
- Gói riêng tư trả về `404` trừ khi bên gọi có thể đọc nhà xuất bản sở hữu.
- Điểm cuối này được cố ý thu hẹp hơn các điểm cuối kiểm duyệt của chủ sở hữu/điều hành viên.
...
- Phiên bản gói cũ trả về hiện vật `legacy-zip` và `downloadUrl` ZIP
  cũ.
- Phiên bản ClawPack trả về hiện vật `npm-pack`, các trường toàn vẹn npm, một
  `tarballUrl`, và URL tương thích ZIP cũ.
- Đây là bề mặt bộ phân giải OpenClaw; nó tránh đoán định dạng kho lưu trữ từ
  một URL dùng chung.
...
- Phiên bản ClawPack truyền trực tiếp chính xác các byte `.tgz` npm-pack đã tải lên.
- Phiên bản ZIP cũ chuyển hướng tới `/api/v1/packages/{name}/download?version=`.
...
Kiểm tra mức độ sẵn sàng bao gồm:

- trạng thái kênh chính thức
- tính sẵn có của phiên bản mới nhất
- tính sẵn có của hiện vật ClawPack npm-pack
- tóm tắt hiện vật
...
- Yêu cầu mã thông báo API của người dùng điều hành viên hoặc quản trị viên.
...
- `packageName` được chuẩn hóa theo tên npm; gói có thể bị thiếu đối với các lần di chuyển
  đã lên kế hoạch.
...
Điểm cuối điều hành viên/quản trị viên cho các hàng đợi đánh giá bản phát hành gói.
...
- `open`: bản phát hành đáng ngờ, độc hại, đang chờ, bị cách ly, bị thu hồi, hoặc đã được báo cáo.
...
Báo cáo một gói để điều hành viên xem xét. Báo cáo ở cấp gói, tùy chọn
...
- Yêu cầu token API của chủ sở hữu gói, thành viên nhà phát hành, moderator, hoặc
  người dùng admin.
...
- Kho lưu trữ Plugin/gói là tệp zip có gốc `package/` để các máy khách OpenClaw cũ
  tiếp tục hoạt động.
- Tuyến này chỉ giữ định dạng ZIP. Nó không truyền phát tệp ClawPack `.tgz`.
...
Được CLI dùng để ánh xạ vân tay cục bộ tới một phiên bản đã biết.

Tham số truy vấn:

- `slug` (bắt buộc)
- `hash` (bắt buộc): sha256 dạng hex 64 ký tự của vân tay bundle
...
- Bàn giao Skills dựa trên GitHub không proxy hoặc mirror byte. Phản hồi JSON
  ...
  và `archiveUrl`; trạng thái quét/hiện tại là cổng kiểm tra và không được đưa vào làm
...
- Trường payload tùy chọn: `migrateOwner`. Khi là `true` cùng với `ownerHandle`, một
  ...
  nhà phát hành hiện tại và nhà phát hành đích. Nếu không bật tùy chọn này, các thay đổi chủ sở hữu
  sẽ bị từ chối.
...
- Các trường form được phép là `payload`, các blob `files` lặp lại, hoặc một tham chiếu tarball
  `clawpack`. `clawpack` có thể là blob `.tgz` hoặc id lưu trữ được trả về bởi
  luồng upload-url. Các lần phát hành storage-id đã staging cũng phải bao gồm
...
- Body JSON và siêu dữ liệu `payload.files` / `payload.artifact`
  do bên gọi cung cấp đều bị từ chối.
...
- Chỉ nhà phát hành tổ chức `openclaw` và các nhà phát hành cá nhân của thành viên tổ chức `openclaw`
  hiện tại mới có thể phát hành lên kênh `official`.
- Các lần phát hành thay mặt vẫn xác thực điều kiện đủ kênh chính thức theo tài khoản chủ sở hữu đích.
...
Việc ẩn của moderator/admin và các lần gỡ bỏ vì bảo mật không hết hạn theo cách này.
...
Chỉ admin. Đảm bảo một nhà phát hành tổ chức tồn tại cho một handle. Nếu handle vẫn trỏ đến một
người dùng/nhà phát hành cá nhân dùng chung kiểu cũ, điểm cuối sẽ di chuyển nó vào một nhà phát hành tổ chức trước.
Đối với tổ chức mới tạo, hãy cung cấp `memberHandle`; admin thực hiện không được thêm làm thành viên.
...
Chỉ admin. Khôi phục một nhà phát hành cá nhân cho một principal GitHub OAuth thay thế đã xác minh
...
định danh GitHub. Khôi phục thất bại theo hướng đóng khi nhà phát hành cá nhân hiện tại của người dùng đích
có Skills, gói hoặc nguồn Skills GitHub.
Khôi phục cũng di chuyển các trường `ownerUserId` cũ cho Skills của nhà phát hành được khôi phục,
bí danh slug của Skills, gói, cảnh báo trình kiểm tra gói và các hàng tóm tắt tìm kiếm dẫn xuất để
các đường dẫn chủ sở hữu trực tiếp khớp với thẩm quyền nhà phát hành mới. Một đặt chỗ protected-handle đang hoạt động
...
Nguồn Skills GitHub có phạm vi theo nhà phát hành và được báo cáo là đã kiểm tra thay vì được ghi lại.
...
- Cả hai điểm cuối đều yêu cầu xác thực bằng API token và chỉ hoạt động cho chủ sở hữu Skills.
...
Cấm người dùng và xóa vĩnh viễn Skills thuộc sở hữu của họ (chỉ moderator/admin).
...
Bỏ cấm người dùng và khôi phục Skills đủ điều kiện (chỉ admin).
...
Thêm/xóa một sao (mục nổi bật). Cả hai điểm cuối đều có tính lũy đẳng.
