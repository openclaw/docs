...
summary: Tham chiếu API HTTP (công khai + endpoint CLI + xác thực).
x-i18n:
    generated_at: "2026-06-30T22:21:10Z"
    model: gpt-5.5
...
Các thư mục của bên thứ ba có thể dùng các endpoint đọc công khai để liệt kê hoặc tìm kiếm ClawHub Skills. Vui lòng lưu kết quả vào bộ nhớ đệm, tuân thủ `429`/`Retry-After`, liên kết người dùng trở lại mục ClawHub chính tắc (`https://clawhub.ai/<owner>/skills/<slug>`), và tránh ngụ ý rằng ClawHub chứng thực trang của bên thứ ba. Không cố gắng sao chép nội dung bị ẩn, riêng tư hoặc bị chặn bởi kiểm duyệt ra ngoài bề mặt API công khai.

Các lối tắt slug trên web phân giải qua nhiều họ registry, nhưng client API nên dùng
các URL chính tắc do endpoint đọc trả về thay vì tự dựng lại thứ tự ưu tiên của route.
...
- Yêu cầu đã xác thực (token Bearer hợp lệ): áp dụng theo bucket của người dùng.
...
- Các endpoint ghi đã xác thực không nên trả về một `Unauthorized` trống khi
  ...
  tài khoản đã bị xóa/cấm/vô hiệu hóa nên nhận được văn bản có thể hành động để
...
- Kết quả được trả về theo thứ tự độ liên quan (độ tương đồng embedding + tăng điểm token slug/tên khớp chính xác + một prior độ phổ biến nhỏ).
...
- Văn bản ASCII được tách token theo ranh giới từ và dấu câu. Ví dụ, `personal-map` chứa token `map` độc lập, trong khi `amap-jsapi-skill` chứa `amap`, `jsapi`, và `skill`; vì vậy tìm kiếm `map` sẽ cho `personal-map` mức khớp từ vựng mạnh hơn `amap-jsapi-skill`.
- Độ phổ biến được co theo thang log và có giới hạn trần. Skills có mức tương tác cao có thể xếp hạng thấp hơn khi văn bản truy vấn khớp yếu hơn.
- Trạng thái kiểm duyệt đáng ngờ hoặc bị ẩn có thể loại một Skill khỏi tìm kiếm công khai tùy theo bộ lọc của caller và trạng thái kiểm duyệt hiện tại.

Hướng dẫn khả năng khám phá cho nhà xuất bản:

- Đặt các thuật ngữ mà người dùng sẽ tìm kiếm đúng nghĩa vào tên hiển thị, phần tóm tắt, và thẻ. Chỉ dùng token slug độc lập khi đó cũng là danh tính ổn định mà bạn muốn giữ.
- Không đổi tên slug chỉ để chạy theo một truy vấn, trừ khi slug mới là tên chính tắc dài hạn tốt hơn. Slug cũ trở thành bí danh chuyển hướng, nhưng URL chính tắc, slug hiển thị, và các bản tóm tắt tìm kiếm trong tương lai dùng slug mới.
...
- Slug cũ được tạo bởi luồng đổi tên/hợp nhất owner sẽ phân giải tới Skill chính tắc.
- `metadata.os`: giới hạn hệ điều hành được khai báo trong frontmatter của Skill (ví dụ `["macos"]`, `["linux"]`). `null` nếu không được khai báo.
...
- `version.security` bao gồm trạng thái xác minh quét đã chuẩn hóa và chi tiết bộ quét
...
- `moderation` là ảnh chụp kiểm duyệt cấp skill hiện tại, được dẫn xuất từ phiên bản mới nhất.
- Khi truy vấn một phiên bản trong lịch sử, hãy kiểm tra `moderation.matchesRequestedVersion` và `moderation.sourceVersion` trước khi xem `moderation` và `security` là cùng ngữ cảnh phiên bản.
...
Quét bản đã xuất bản dùng JSON:
...
- Quét bản đã xuất bản yêu cầu quyền truy cập quản lý của chủ sở hữu/nhà phát hành, hoặc quyền của kiểm duyệt viên/quản trị viên nền tảng.
- Quét bản đã xuất bản chỉ ghi ngược lại khi `update: true` và quá trình quét hoàn tất thành công.
...
- Tác vụ quét là bất đồng bộ. Yêu cầu quét thủ công được ưu tiên trước công việc xuất bản/điền bù thông thường, nhưng việc hoàn tất vẫn phụ thuộc vào mức sẵn sàng của worker.
...
- Trả về `queue.queuedAhead` và `queue.position` khi đang xếp hàng để client có thể hiển thị có bao nhiêu lần quét thủ công ưu tiên đang đứng trước yêu cầu. Hàng đợi rất lớn được giới hạn và báo cáo với `queuedAheadIsEstimate: true`.
...
- Yêu cầu quyền truy cập quản lý của chủ sở hữu/nhà phát hành đối với skill hoặc Plugin, hoặc quyền của kiểm duyệt viên/quản trị viên nền tảng.
...
- Danh tính skill, danh tính nhà phát hành, và siêu dữ liệu phiên bản đã chọn là các trường phong bì cấp cao nhất (`slug`, `displayName`, `publisherHandle`, `version`, `resolvedFrom`, `tag`, `createdAt`) để tự động hóa shell có thể đọc chúng mà không cần mở các wrapper lồng nhau.
...
- `security.signals.dependencyRegistry` được giữ lại để tương thích phản hồi v1, nhưng trình quét sự tồn tại của sổ đăng ký dependency đã ngừng hoạt động và khóa này luôn là `null`.
- `provenance` chỉ là `server-resolved-github-import` khi ClawHub đã phân giải và lưu repo/ref/commit/path GitHub trong quá trình xuất bản hoặc nhập; nếu không, giá trị là `unavailable`.

### `POST /api/v1/skills/-/security-verdicts`

Trả về các phán quyết bảo mật rút gọn hiện tại cho đúng các phiên bản skill. Điểm cuối bộ sưu tập này dành cho các client đã biết phiên bản skill ClawHub đã cài đặt nào cần hiển thị, chẳng hạn như OpenClaw Control UI.
...
- Kết quả theo từng mục; một skill hoặc phiên bản bị thiếu không làm hỏng toàn bộ phản hồi.
- Phản hồi chỉ về bảo mật. Phản hồi không bao gồm dữ liệu Skill Card, trạng thái thẻ đã tạo, danh sách tệp artifact, hoặc payload trình quét chi tiết.
...
- `category` (tùy chọn): bộ lọc danh mục Plugin. Chỉ được hỗ trợ khi
  ...
  bộ lọc v1 kế thừa được ghi tài liệu trong `GET /api/v1/plugins`.
...
Các bí danh bộ lọc v1 kế thừa vẫn được chấp nhận trên các endpoint đọc:
...
Các bí danh kế thừa không được chấp nhận làm giá trị danh mục được lưu trữ hoặc do tác giả khai báo.
...
- Các Skill hiện tại được hỗ trợ bởi GitHub với lần quét `clean` hoặc `suspicious` bao gồm
...
- `_manifest.json` luôn được bao gồm ở thư mục gốc của ZIP.
...
- Các bí danh bộ lọc v1 cũ được ghi tài liệu trong `GET /api/v1/plugins` cũng
  được chấp nhận.
- Lọc theo danh mục là một bộ lọc API thực, được hỗ trợ bởi các hàng digest danh mục Plugin,
...
- Các bản phát hành ClawPack bao gồm các trường tương thích npm `npmIntegrity`, `npmShasum`, và
  `npmTarballName`.
- `version.sha256hash` là siêu dữ liệu tương thích không còn khuyến nghị cho client cũ. Nó
  băm chính xác các byte ZIP được trả về bởi `/api/v1/packages/{name}/download`.
  Client hiện đại nên dùng `version.artifact.sha256`, trường này xác định
...
- Các gói riêng tư trả về `404` trừ khi bên gọi có thể đọc nhà phát hành sở hữu.
- Endpoint này cố ý hẹp hơn các endpoint điều phối dành cho chủ sở hữu/điều phối viên.
...
- Các phiên bản ClawPack stream chính xác các byte `.tgz` npm-pack đã tải lên.
...
Điểm cuối chủ sở hữu/moderator để xem thông tin kiểm duyệt gói.
...
Trả về nội dung văn bản thô cho tệp gói.
...
- Giới hạn kích thước tệp: 200KB.
- Các lần quét VirusTotal đang chờ xử lý không chặn thao tác đọc; các bản phát hành độc hại vẫn có thể bị giữ lại ở nơi khác.
...
- Kho lưu trữ Plugin/gói là tệp zip có thư mục gốc `package/` để các client OpenClaw cũ
  tiếp tục hoạt động.
- Tuyến này chỉ duy trì dạng ZIP. Nó không truyền trực tuyến các tệp ClawPack `.tgz`.
...
- Các phiên bản kế thừa chỉ có ZIP bị cố ý bỏ qua.
...
- Packument của gói có phạm vi hỗ trợ cả đường dẫn yêu cầu `/api/npm/@scope/name` và
  đường dẫn đã mã hóa `/api/npm/@scope%2Fname` của npm.

### `GET /api/npm/{package}/-/{tarball}.tgz`

Truyền trực tuyến đúng các byte tarball ClawPack đã tải lên cho các client mirror npm.
...
skill hiện tại được hỗ trợ bằng GitHub có lần quét `clean` hoặc `suspicious` và không có phiên bản
được lưu trữ.
...
- Các chuyển giao skill được hỗ trợ bằng GitHub không proxy hoặc mirror byte. Phản hồi JSON
  bao gồm `sourceRef: "public-github"`, `repo`, `commit`, `path`, `contentHash`,
  và `archiveUrl`; trạng thái quét/hiện tại là một gate và không được đưa vào dưới dạng siêu dữ liệu payload thành công.
...
- Trường payload tùy chọn: `ownerHandle`. Khi có, API phân giải publisher đó ở phía máy chủ và yêu cầu tác nhân có quyền truy cập publisher.
- Trường payload tùy chọn: `migrateOwner`. Khi là `true` cùng với `ownerHandle`, một skill hiện có có thể được chuyển sang owner đó nếu tác nhân là admin/owner trên cả publisher hiện tại và publisher đích. Nếu không có opt-in này, các thay đổi owner sẽ bị từ chối.
...
- Các trường form được phép là `payload`, các blob `files` lặp lại, hoặc một tham chiếu tarball `clawpack`. `clawpack` có thể là blob `.tgz` hoặc id lưu trữ do luồng upload-url trả về. Các lần phát hành storage-id theo giai đoạn cũng phải bao gồm `clawpackUploadTicket` được trả về cùng URL tải lên đó.
- Dùng `files` hoặc `clawpack`, không bao giờ dùng cả hai trong cùng một yêu cầu.
- Body JSON và siêu dữ liệu `payload.files` / `payload.artifact` do caller cung cấp bị từ chối.
- Các yêu cầu phát hành multipart trực tiếp bị giới hạn ở 18MB. Tarball ClawPack có thể dùng luồng upload-url đến giới hạn tarball 120MB.
- Trường payload tùy chọn: `ownerHandle`. Khi có, chỉ admin mới có thể phát hành thay mặt owner đó.

Điểm nổi bật về xác thực:

- `family` phải là `code-plugin` hoặc `bundle-plugin`.
- Các gói Plugin yêu cầu `openclaw.plugin.json`. Các bản tải lên ClawPack `.tgz` phải chứa tệp này tại `package/openclaw.plugin.json`.
- Code plugin yêu cầu `package.json`, siêu dữ liệu repo nguồn, siêu dữ liệu commit nguồn, siêu dữ liệu schema cấu hình, `openclaw.compat.pluginApi`, và `openclaw.build.openclawVersion`.
- `openclaw.hostTargets` và `openclaw.environment` là siêu dữ liệu tùy chọn.
- Chỉ publisher org `openclaw` và publisher cá nhân của các thành viên org `openclaw` hiện tại mới có thể phát hành lên kênh `official`.
- Các phát hành thay mặt vẫn xác thực điều kiện đủ của kênh official đối với tài khoản owner đích.
...
Khi có, `reason` được lưu dưới dạng ghi chú kiểm duyệt skill và được sao chép vào nhật ký kiểm toán.
Các lần xóa mềm do owner khởi tạo sẽ giữ slug trong 30 ngày, sau đó slug có thể được
publisher khác nhận. Phản hồi xóa bao gồm `slugReservedUntil` khi thời hạn này áp dụng.
Các lần ẩn bởi moderator/admin và gỡ bỏ vì bảo mật không hết hạn theo cách này.
...
Chỉ admin. Đảm bảo một publisher org tồn tại cho một handle. Nếu handle vẫn trỏ tới một
người dùng/publisher cá nhân dùng chung legacy, endpoint sẽ di chuyển nó thành publisher org trước.
Với một org mới tạo, cung cấp `memberHandle`; admin thực hiện không được thêm làm thành viên.
...
Tạo publisher org tự phục vụ đã xác thực. Tạo một publisher org mới và thêm caller
làm owner. Endpoint này không di chuyển các handle người dùng/cá nhân hiện có và không
đánh dấu publisher là trusted/official.

- Body: `{ "handle": "opik", "displayName": "Opik" }`
- Phản hồi: `{ "ok": true, "publisherId": "...", "handle": "opik", "created": true, "trusted": false }`
- Trả về `409` khi handle đã được dùng bởi một publisher, người dùng, hoặc publisher cá nhân.

### `POST /api/v1/users/reserve`

Chỉ admin. Giữ trước root slug và tên gói cho owner hợp pháp mà không phát hành
bản phát hành. Tên gói trở thành gói placeholder riêng tư không có hàng bản phát hành, để cùng
owner đó có thể phát hành bản phát hành code-plugin hoặc bundle-plugin thật vào tên đó sau này.
...
Chỉ admin. Khôi phục một publisher cá nhân cho principal GitHub OAuth thay thế đã xác minh
mà không chỉnh sửa các hàng tài khoản Convex Auth. Yêu cầu phải nêu cả hai id tài khoản
provider GitHub bất biến; các handle có thể thay đổi chỉ được dùng làm guard hướng tới operator.
...
principal GitHub. Khôi phục sẽ fail closed khi publisher cá nhân hiện tại của người dùng đích
...
bí danh slug của skill, packages, cảnh báo của trình kiểm tra package, và các hàng digest tìm kiếm phái sinh để
...
Nguồn skill GitHub thuộc phạm vi publisher và được báo cáo là đã kiểm tra thay vì được viết lại.
...
- `GET /api/v1/transfers/outgoing`
  - Dạng Response: `{ "transfers": [{ "_id": "...", "skill": { "slug": "demo", "displayName": "Demo" }, "fromUser"|"toUser": { "handle": "..." }, "message": "...", "requestedAt": 0, "expiresAt": 0 }] }`

### `POST /api/v1/users/ban`

Cấm người dùng và xóa cứng các skills thuộc sở hữu (chỉ moderator/admin).
...
Bỏ cấm người dùng và khôi phục các skills đủ điều kiện (chỉ admin).
...
Thêm/xóa một star (đánh dấu nổi bật). Cả hai endpoint đều có tính idempotent.
...
chuẩn bị một tarball ClawPack phải gửi id lưu trữ thu được dưới dạng
`clawpack` và ticket được trả về dưới dạng `clawpackUploadTicket`.

## Khám phá registry (`/.well-known/clawhub.json`)

CLI có thể khám phá các cài đặt registry/xác thực từ site:
