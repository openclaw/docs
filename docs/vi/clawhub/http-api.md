...
summary: Tài liệu tham khảo API HTTP (công khai + các điểm cuối CLI + xác thực).
x-i18n:
...
Các thư mục của bên thứ ba có thể dùng các endpoint đọc công khai để liệt kê hoặc tìm kiếm Skills trên ClawHub. Vui lòng lưu kết quả vào bộ nhớ đệm, tuân thủ `429`/`Retry-After`, liên kết người dùng trở lại danh sách ClawHub chính tắc (`https://clawhub.ai/<owner>/<slug>`), và tránh ngụ ý rằng ClawHub chứng thực trang của bên thứ ba. Không cố gắng sao chép nội dung bị ẩn, riêng tư, hoặc bị chặn do kiểm duyệt ra ngoài bề mặt API công khai.

Các lối tắt slug trên web phân giải qua nhiều họ registry, nhưng client API nên dùng
các URL chính tắc được trả về bởi endpoint đọc thay vì tự dựng lại thứ tự ưu tiên
của route.
...
- Các endpoint ghi đã xác thực không nên trả về một `Unauthorized` trần khi
  ...
  tài khoản bị xóa/cấm/vô hiệu hóa đều nên nhận được văn bản có thể hành động để client
...
- Nếu không có IP client đáng tin cậy, các yêu cầu tải xuống ẩn danh dùng bucket dự phòng theo phạm vi endpoint thay vì một bucket `ip:unknown` toàn cục. Các yêu cầu đọc/ghi ẩn danh vẫn dùng bucket unknown dùng chung để định tuyến thiếu IP vẫn hiển thị và thận trọng.
...
- Kết quả được trả về theo thứ tự mức độ liên quan (độ tương đồng embedding + tăng hạng token slug/tên khớp chính xác + yếu tố phổ biến trước từ lượt tải xuống).
- Mức độ liên quan mạnh hơn mức độ phổ biến. Một token slug hoặc tên hiển thị khớp chính xác có thể xếp trên một kết quả khớp lỏng hơn nhưng có nhiều lượt tải xuống hơn.
- Văn bản ASCII được tách token theo ranh giới từ và dấu câu. Ví dụ, `personal-map` chứa token `map` độc lập, còn `amap-jsapi-skill` chứa `amap`, `jsapi`, và `skill`; vì vậy tìm kiếm `map` cho `personal-map` một kết quả khớp từ vựng mạnh hơn `amap-jsapi-skill`.
- Lượt tải xuống được dùng như một yếu tố trước nhỏ theo thang log và bộ phân xử hòa, không phải tín hiệu xếp hạng chính. Skills có nhiều lượt tải xuống có thể xếp thấp hơn khi văn bản truy vấn khớp yếu hơn.
...
- Đặt các thuật ngữ mà người dùng sẽ tìm kiếm đúng nghĩa vào tên hiển thị, tóm tắt và thẻ. Chỉ dùng token slug độc lập khi đó cũng là một định danh ổn định mà bạn muốn giữ.
- Không đổi tên slug chỉ để theo đuổi một truy vấn, trừ khi slug mới là tên chính tắc dài hạn tốt hơn. Slug cũ trở thành bí danh chuyển hướng, nhưng URL chính tắc, slug được hiển thị và các digest tìm kiếm tương lai dùng slug mới.
...
- Bao gồm trạng thái xác minh đã chuẩn hóa cùng với chi tiết riêng theo scanner.
...
- `security.hasScanResult` là `true` chỉ khi scanner tạo ra phán quyết xác định (`clean`, `suspicious`, hoặc `malicious`).
- `moderation` là snapshot kiểm duyệt cấp Skill hiện tại được suy ra từ phiên bản mới nhất.
...
- `target` / `hostTarget` (tùy chọn): dạng viết tắt của `host:<target>`
...
- Gói riêng tư trả về `404` trừ khi người gọi có thể đọc publisher sở hữu.
...
- Yêu cầu API token cho chủ sở hữu gói, chủ sở hữu/quản trị viên publisher thuộc tổ chức,
...
- Phiên bản gói legacy trả về một artifact `legacy-zip` và một URL tải xuống ZIP legacy
  `downloadUrl`.
- Phiên bản ClawPack trả về một artifact `npm-pack`, các trường toàn vẹn npm, một
...
- Phiên bản ClawPack truyền trực tiếp đúng byte `.tgz` npm-pack đã tải lên.
...
Endpoint cho điều phối viên để liệt kê các hàng migration Plugin OpenClaw official.
...
Endpoint cho quản trị viên để tạo hoặc cập nhật một hàng migration Plugin official.
...
- `packageName` được chuẩn hóa theo tên npm; gói có thể bị thiếu đối với các migration
  đã lên kế hoạch.
- Nội dung này chỉ theo dõi mức độ sẵn sàng migration. Nó không thay đổi OpenClaw hoặc tạo
  ClawPack.

### `GET /api/v1/packages/moderation/queue`

Endpoint cho điều phối viên/quản trị viên dành cho hàng đợi đánh giá bản phát hành gói.
...
Endpoint cho chủ sở hữu/điều phối viên để xem khả năng hiển thị điều phối gói.

Xác thực:

- Yêu cầu API token cho chủ sở hữu gói, thành viên publisher, điều phối viên, hoặc
...
`finalAction: "revoke"` cùng với một báo cáo đã xác nhận để áp dụng kiểm duyệt bản phát hành trong cùng
...
Endpoint dành cho người kiểm duyệt/quản trị viên để rà soát bản phát hành gói.
...
Các bản phát hành bị cách ly và bị thu hồi trả về `403` từ các route tải xuống tạo phẩm.
...
metadata loại tạo phẩm rõ ràng.
...
- Các hàng hiện có dựa trên ClawPack nhưng thiếu `artifactKind` được sửa thành
  `npm-pack`.
- Thao tác này không tạo ClawPack hoặc thay đổi byte tạo phẩm.
...
- Kho lưu trữ Plugin/gói là các tệp zip có gốc `package/` để các máy khách OpenClaw cũ
  tiếp tục hoạt động.
- Route này chỉ duy trì ZIP. Nó không phát trực tuyến các tệp ClawPack `.tgz`.
- Phản hồi bao gồm các header `ETag`, `Digest`, `X-ClawHub-Artifact-Type` và
  `X-ClawHub-Artifact-Sha256` cho kiểm tra toàn vẹn của bộ phân giải.
...
Phát trực tuyến chính xác các byte tarball ClawPack đã tải lên cho máy khách bản sao npm.
...
Tải xuống zip của một phiên bản skill.

Tham số truy vấn:

- `slug` (bắt buộc)
- `version` (không bắt buộc): chuỗi semver
- `tag` (không bắt buộc): tên tag (ví dụ: `latest`)

Ghi chú:

- Nếu không cung cấp `version` hoặc `tag`, phiên bản mới nhất sẽ được dùng.
- Các phiên bản đã xóa mềm trả về `410`.
- Thống kê tải xuống được đếm theo danh tính duy nhất mỗi giờ (`userId` khi API token hợp lệ, nếu không thì IP).
...
- Trường payload không bắt buộc: `migrateOwner`. Khi là `true` với `ownerHandle`, một
  ...
  nhà xuất bản hiện tại và đích. Nếu không bật tùy chọn này, các thay đổi chủ sở hữu sẽ
  bị từ chối.
...
Các lượt ẩn của người kiểm duyệt/quản trị viên và xóa vì bảo mật không hết hạn theo cách này.
...
nhà xuất bản người dùng cá nhân/dùng chung kiểu cũ, endpoint sẽ di chuyển nó vào một nhà xuất bản tổ chức trước.
...
Chỉ dành cho quản trị viên. Giữ trước các slug gốc và tên gói cho chủ sở hữu hợp pháp mà không xuất bản
bản phát hành. Tên gói trở thành các gói giữ chỗ riêng tư không có hàng bản phát hành, để cùng
chủ sở hữu sau này có thể xuất bản bản phát hành code-plugin hoặc bundle-plugin thực sự vào tên đó.
...
Nếu bạn tự host, hãy phục vụ tệp này (hoặc đặt `CLAWHUB_REGISTRY` rõ ràng; `CLAWDHUB_REGISTRY` kiểu cũ).
