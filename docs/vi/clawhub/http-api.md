...
x-i18n:
    generated_at: "2026-07-01T13:08:22Z"
    model: gpt-5.5
...
Các thư mục của bên thứ ba có thể dùng các endpoint đọc công khai để liệt kê hoặc tìm kiếm kỹ năng ClawHub. Vui lòng lưu kết quả vào bộ nhớ đệm, tuân thủ `429`/`Retry-After`, liên kết người dùng trở lại danh sách ClawHub chính tắc (`https://clawhub.ai/<owner>/skills/<slug>`), và tránh ngụ ý rằng ClawHub xác nhận trang của bên thứ ba. Không cố phản chiếu nội dung bị ẩn, riêng tư, hoặc bị chặn bởi kiểm duyệt ngoài bề mặt API công khai.

Các lối tắt slug web phân giải trên nhiều họ registry, nhưng client API nên dùng
các URL chính tắc do endpoint đọc trả về thay vì tự dựng lại độ ưu tiên
của route.
...
- Yêu cầu đã xác thực (token Bearer hợp lệ): thực thi theo bucket của từng người dùng.
...
- Các endpoint ghi đã xác thực không nên trả về chỉ `Unauthorized` khi
  ...
  tài khoản đã xóa/bị cấm/bị vô hiệu hóa nên nhận văn bản có thể hành động để client
...
- `X-RateLimit-Remaining` / `RateLimit-Remaining`: ngân sách còn lại chính xác khi có mặt.
  Các yêu cầu thành công dạng phân mảnh bỏ qua header này thay vì trả về một giá trị toàn cục xấp xỉ.
...
- Nếu có `Retry-After`, hãy chờ bấy nhiêu giây trước khi thử lại.
...
- ClawHub dùng header chuyển tiếp đáng tin cậy để xác định IP client ở biên.
...
nên đọc nội dung phản hồi như một chuỗi người đọc được. Tham số truy vấn không xác định được
...
- `highlightedOnly` (tùy chọn): `true` để lọc còn các kỹ năng được làm nổi bật
...
- Kết quả được trả về theo thứ tự liên quan (độ tương đồng embedding + tăng điểm token slug/tên khớp chính xác + một ưu tiên độ phổ biến nhỏ).
- Mức độ liên quan mạnh hơn độ phổ biến. Một slug chính xác hoặc token tên hiển thị khớp có thể xếp hạng cao hơn một kết quả khớp lỏng hơn nhưng có tương tác mạnh hơn nhiều.
- Văn bản ASCII được tách token tại ranh giới từ và dấu câu. Ví dụ, `personal-map` chứa token `map` độc lập, còn `amap-jsapi-skill` chứa `amap`, `jsapi`, và `skill`; vì vậy tìm kiếm `map` cho `personal-map` một kết quả khớp từ vựng mạnh hơn `amap-jsapi-skill`.
- Độ phổ biến được co theo log và bị giới hạn. Các kỹ năng có tương tác cao có thể xếp hạng thấp hơn khi văn bản truy vấn khớp yếu hơn.
- Trạng thái kiểm duyệt đáng ngờ hoặc bị ẩn có thể loại bỏ một kỹ năng khỏi tìm kiếm công khai tùy theo bộ lọc của bên gọi và trạng thái kiểm duyệt hiện tại.

Hướng dẫn để nhà xuất bản dễ được khám phá:

- Đặt các thuật ngữ mà người dùng sẽ tìm kiếm đúng từng chữ vào tên hiển thị, tóm tắt, và thẻ. Chỉ dùng token slug độc lập khi nó cũng là một định danh ổn định mà bạn muốn giữ.
- Không đổi tên slug chỉ để chạy theo một truy vấn, trừ khi slug mới là tên chính tắc dài hạn tốt hơn. Slug cũ trở thành alias chuyển hướng, nhưng URL chính tắc, slug hiển thị, và bản tóm tắt tìm kiếm trong tương lai dùng slug mới.
- Alias đổi tên giữ khả năng phân giải cho URL cũ và lượt cài đặt cũ phân giải qua registry, nhưng xếp hạng tìm kiếm dựa trên metadata kỹ năng chính tắc sau khi việc đổi tên đã được lập chỉ mục. Thống kê hiện có vẫn gắn với kỹ năng.
- Nếu một kỹ năng bất ngờ không hiển thị, hãy kiểm tra trạng thái kiểm duyệt trước bằng `clawhub inspect @owner/slug` khi đã đăng nhập trước khi thay đổi metadata liên quan đến xếp hạng.
...
- `sort` (tùy chọn): `updated` (mặc định), `recommended` (alias: `default`), `createdAt` (alias: `newest`), `downloads`, `stars` (alias: `rating`), các alias cài đặt cũ `installsCurrent`/`installs`/`installsAllTime` ánh xạ tới `downloads`, `trending`
- `nonSuspiciousOnly` (tùy chọn): `true` để ẩn các kỹ năng đáng ngờ (`flagged.suspicious`)
- `nonSuspicious` (tùy chọn): alias cũ cho `nonSuspiciousOnly`
...
- `recommended` dùng tín hiệu tương tác và mức độ gần đây.
- `trending` xếp hạng theo lượt cài đặt trong 7 ngày gần nhất (dựa trên telemetry).
- `createdAt` ổn định cho các lượt crawl kỹ năng mới; `updated` thay đổi khi kỹ năng hiện có được phát hành lại.
...
- Slug cũ được tạo bởi luồng đổi tên/hợp nhất chủ sở hữu phân giải tới kỹ năng chính tắc.
- `metadata.os`: ràng buộc OS được khai báo trong frontmatter của kỹ năng (ví dụ `["macos"]`, `["linux"]`). `null` nếu không được khai báo.
...
- `moderation` chỉ được bao gồm khi kỹ năng bị gắn cờ hoặc chủ sở hữu đang xem nó.
...
- Bên gọi công khai chỉ nhận `200` cho kỹ năng hiển thị đã bị gắn cờ.
- Bằng chứng được biên tập lại cho bên gọi công khai và chỉ bao gồm đoạn trích thô cho chủ sở hữu/người kiểm duyệt.

### `POST /api/v1/skills/{slug}/report`

Báo cáo một kỹ năng để người kiểm duyệt xem xét. Báo cáo ở cấp kỹ năng, tùy chọn liên kết
...
Endpoint dành cho người kiểm duyệt/quản trị viên để giải quyết hoặc mở lại báo cáo kỹ năng.
...
- Bao gồm trạng thái xác minh đã chuẩn hóa cùng các chi tiết riêng của từng bộ quét.
- `security.hasScanResult` chỉ là `true` khi một bộ quét tạo ra kết luận dứt khoát (`clean`, `suspicious`, hoặc `malicious`).
- `moderation` là ảnh chụp kiểm duyệt hiện tại ở cấp Skill, được dẫn xuất từ phiên bản mới nhất.
...
- Quét đã xuất bản yêu cầu quyền truy cập quản lý của chủ sở hữu/nhà xuất bản, hoặc quyền điều phối viên/quản trị viên nền tảng.
...
- Công việc quét là bất đồng bộ. Yêu cầu quét thủ công được ưu tiên trước công việc xuất bản/lấp đầy thông thường, nhưng việc hoàn tất vẫn phụ thuộc vào mức sẵn sàng của worker.

### `GET /api/v1/skills/-/scan/{scanId}`

Endpoint thăm dò có xác thực cho một lượt quét đã gửi.

- Trả về trạng thái đã xếp hàng/đang chạy/thành công/thất bại.
- Trả về `queue.queuedAhead` và `queue.position` khi đang xếp hàng để client có thể hiển thị có bao nhiêu lượt quét thủ công được ưu tiên đang đứng trước yêu cầu này. Các hàng đợi rất lớn được giới hạn và báo cáo với `queuedAheadIsEstimate: true`.
...
- Yêu cầu một lượt quét đã thành công; các lượt quét chưa kết thúc trả về `409`.
...
- Yêu cầu quyền truy cập quản lý của chủ sở hữu/nhà xuất bản đối với Skill hoặc Plugin, hoặc quyền điều phối viên/quản trị viên nền tảng.
...
- Danh tính Skill, danh tính nhà xuất bản, và siêu dữ liệu phiên bản đã chọn là các trường phong bì cấp cao nhất (`slug`, `displayName`, `publisherHandle`, `version`, `resolvedFrom`, `tag`, `createdAt`) để tự động hóa shell có thể đọc chúng mà không cần mở các wrapper lồng nhau.
- `security` là kết luận ClawScan/bảo mật cấp cao nhất. Tự động hóa nên dựa vào `ok`, `decision`, `reasons`, và `security.status`.
- `security.signals` chứa bằng chứng hỗ trợ từ bộ quét như `staticScan`, `virusTotal`, và `skillSpector`.
- `security.signals.dependencyRegistry` được giữ lại để tương thích phản hồi v1, nhưng bộ quét sự tồn tại của registry phụ thuộc đã ngừng hoạt động và khóa này luôn là `null`.
...
Trả về các kết luận bảo mật rút gọn hiện tại cho đúng phiên bản Skill. Endpoint bộ sưu tập này dành cho các client đã biết những phiên bản ClawHub Skill đã cài đặt mà họ cần hiển thị, chẳng hạn như OpenClaw Control UI.
...
- Kết quả theo từng mục; một Skill hoặc phiên bản bị thiếu không làm thất bại toàn bộ phản hồi.
- Phản hồi chỉ dành cho bảo mật. Nó không bao gồm dữ liệu Skill Card, trạng thái thẻ đã tạo, danh sách tệp hiện vật, hoặc payload bộ quét chi tiết.
- `security.signals` chỉ chứa bằng chứng hỗ trợ ở cấp trạng thái; dùng `/scan` hoặc trang kiểm tra bảo mật ClawHub để xem đầy đủ chi tiết bộ quét.
- `security.signals.dependencyRegistry` được giữ lại để tương thích phản hồi v1, nhưng bộ quét sự tồn tại của registry phụ thuộc đã ngừng hoạt động và khóa này luôn là `null`.
...
- Dùng `/verify` khi bạn cần phong bì xác minh Skill Card cho một Skill, `/card` khi bạn cần markdown thẻ đã tạo, và `/scan` khi bạn cần dữ liệu bộ quét chi tiết.
...
- `category` (tùy chọn): bộ lọc danh mục Plugin. Chỉ được hỗ trợ khi
  yêu cầu được giới hạn trong phạm vi các gói Plugin (`/api/v1/plugins`,
...
- Giá trị không hợp lệ cho `family`, `channel`, `isOfficial`, `featured`,
  `highlightedOnly`, hoặc `sort` trả về `400`. Tham số truy vấn không xác định bị bỏ qua.
- `GET /api/v1/code-plugins` và `GET /api/v1/bundle-plugins` vẫn là các bí danh cố định theo family.
- Mục Skills vẫn được hỗ trợ bởi sổ đăng ký Skills và vẫn chỉ có thể được xuất bản thông qua `POST /api/v1/skills`.
...
- `isOfficial` (tùy chọn): `true` hoặc `false`
- `category` (tùy chọn): bộ lọc danh mục Plugin. Chỉ được hỗ trợ khi
  yêu cầu được giới hạn trong phạm vi các gói Plugin. Các danh mục được kiểm soát và bí danh bộ lọc v1 cũ
  được ghi tài liệu trong `GET /api/v1/plugins`.

Ghi chú:

- Giá trị không hợp lệ cho `family`, `channel`, `isOfficial`, `featured`, hoặc
  `highlightedOnly` trả về `400`. Tham số truy vấn không xác định bị bỏ qua.
- Người gọi ẩn danh chỉ thấy các kênh gói công khai.
...
- Skills hiện tại dựa trên GitHub có lần quét `clean` hoặc `suspicious` bao gồm
...
- Yêu cầu mã thông báo API cho chủ sở hữu gói, chủ sở hữu/quản trị viên nhà phát hành tổ chức,
  điều hành viên nền tảng, hoặc quản trị viên nền tảng.
...
- Các bản phát hành ClawPack bao gồm các trường tương thích npm `npmIntegrity`, `npmShasum`, và
  `npmTarballName`.
- `version.sha256hash` là siêu dữ liệu tương thích không còn khuyến nghị dùng cho máy khách cũ. Nó
...
- Điểm cuối đọc công khai. Không yêu cầu mã thông báo của chủ sở hữu, nhà phát hành, điều hành viên, hoặc quản trị viên.
...
- `release.artifactKind`, `release.artifactSha256`, `release.npmIntegrity`,
  ...
  tạo tác phát hành.
- `trust.scanStatus` là trạng thái tin cậy hiệu lực được suy ra từ đầu vào trình quét
  và kiểm duyệt bản phát hành thủ công.
- `trust.moderationState` có thể null. Nó là `null` khi không có kiểm duyệt bản phát hành
  thủ công.
- `trust.blockedFromDownload` là tín hiệu chặn cài đặt. OpenClaw và các
  ...
  suy luận lại các quy tắc chặn từ trường trình quét hoặc kiểm duyệt.
...
- Điểm cuối này chính xác theo phiên bản. Máy khách nên gọi nó sau khi phân giải
  phiên bản gói mà chúng định cài đặt, không chỉ sau khi đọc siêu dữ liệu
  gói mới nhất.
- Các gói riêng tư trả về `404` trừ khi bên gọi có thể đọc nhà phát hành sở hữu.
- Điểm cuối này có chủ ý hẹp hơn các điểm cuối kiểm duyệt của chủ sở hữu/điều hành viên.
  Nó phơi bày quyết định cài đặt và giải thích công khai, không phải
  danh tính người báo cáo, nội dung báo cáo, bằng chứng riêng tư, hoặc mốc thời gian rà soát
  nội bộ.
...
- Đây là bề mặt trình phân giải của OpenClaw; nó tránh đoán định dạng kho lưu trữ từ
  một URL dùng chung.

### `GET /api/v1/packages/{name}/versions/{version}/artifact/download`

Tải xuống tạo tác phiên bản thông qua đường dẫn trình phân giải rõ ràng.
...
Điểm cuối điều hành viên để liệt kê các hàng di chuyển plugin OpenClaw chính thức.

Xác thực:

- Yêu cầu mã thông báo API cho người dùng điều hành viên hoặc quản trị viên.
...
- `packageName` được chuẩn hóa theo tên npm; gói có thể vắng mặt đối với các di chuyển
  đã lên kế hoạch.
...
Điểm cuối điều hành viên/quản trị viên cho hàng đợi rà soát bản phát hành gói.
...
Báo cáo một gói để điều hành viên rà soát. Báo cáo ở cấp gói, tùy chọn
liên kết với một phiên bản. Chúng đưa dữ liệu vào hàng đợi kiểm duyệt nhưng tự thân không tự động ẩn hoặc
chặn tải xuống; điều hành viên nên dùng kiểm duyệt bản phát hành để
...
Điểm cuối dành cho chủ sở hữu/moderator để xem trạng thái điều phối gói.

Xác thực:

- Yêu cầu API token cho chủ sở hữu gói, thành viên publisher, moderator hoặc
  người dùng admin.
...
Điểm cuối moderator/admin để xử lý hoặc mở lại báo cáo gói.
...
Điểm cuối moderator/admin để rà soát bản phát hành gói.
...
Các bản phát hành bị cách ly và thu hồi trả về `403` từ các tuyến tải xuống artifact.
...
- Giới hạn kích thước tệp: 200KB.
- Các lần quét VirusTotal đang chờ không chặn lượt đọc; bản phát hành độc hại vẫn có thể bị giữ lại ở nơi khác.
...
- Kho lưu trữ Plugin/gói là tệp zip có thư mục gốc `package/` để các client OpenClaw
  cũ tiếp tục hoạt động.
- Tuyến này chỉ duy trì ZIP. Nó không stream tệp ClawPack `.tgz`.
...
- `dist.tarball`, `dist.integrity` và `dist.shasum` dùng các trường tương thích
  npm để người dùng có thể trỏ npm đến mirror nếu muốn.
- Packument gói scoped hỗ trợ cả đường dẫn yêu cầu `/api/npm/@scope/name` và
  đường dẫn mã hóa theo npm `/api/npm/@scope%2Fname`.

### `GET /api/npm/{package}/-/{tarball}.tgz`

Stream đúng các byte tarball ClawPack đã tải lên cho client npm mirror.
...
Tải xuống ZIP phiên bản skill được lưu trữ, hoặc trả về chuyển giao nguồn GitHub cho một
skill hiện tại dựa trên GitHub có kết quả quét `clean` hoặc `suspicious` và không có
...
- Nếu không cung cấp `version` hoặc `tag`, phiên bản mới nhất được dùng.
...
- Chuyển giao skill dựa trên GitHub không proxy hoặc mirror byte. Phản hồi JSON
  ...
  và `archiveUrl`; trạng thái quét/hiện tại là cổng kiểm tra và không được đưa vào
  dưới dạng metadata payload thành công.
- Thống kê tải xuống được đếm theo danh tính duy nhất mỗi ngày UTC (`userId` khi API token hợp lệ, nếu không thì IP).
...
- Trường payload tùy chọn: `migrateOwner`. Khi là `true` cùng với `ownerHandle`, một
  ...
  publisher hiện tại và publisher đích. Nếu không có lựa chọn chủ động này, thay đổi chủ sở hữu
  sẽ bị từ chối.
...
- Các trường biểu mẫu được phép là `payload`, các blob `files` lặp lại, hoặc một tham chiếu tarball `clawpack`.
  `clawpack` có thể là blob `.tgz` hoặc storage id do luồng upload-url trả về.
  Các bản phát hành theo storage-id đã chuẩn bị cũng phải bao gồm
...
- Body JSON và metadata `payload.files` / `payload.artifact`
...
- Gói Plugin yêu cầu `openclaw.plugin.json`. Tải lên `.tgz` ClawPack phải
...
- Chỉ publisher tổ chức `openclaw` và publisher cá nhân của các thành viên tổ chức `openclaw`
  hiện tại mới được phát hành lên kênh `official`.
- Các bản phát hành thay mặt vẫn xác thực điều kiện đủ của kênh official dựa trên tài khoản chủ sở hữu đích.
...
Ẩn bởi moderator/admin và gỡ bỏ vì lý do bảo mật không hết hạn theo cách này.
...
Chỉ admin. Đảm bảo tồn tại một publisher tổ chức cho một handle. Nếu handle vẫn trỏ đến
publisher người dùng/cá nhân dùng chung kiểu cũ, điểm cuối sẽ di chuyển nó vào publisher tổ chức trước.
...
Tạo publisher tổ chức tự phục vụ đã xác thực. Tạo một publisher tổ chức mới và thêm
...
Chỉ admin. Giữ trước slug gốc và tên gói cho chủ sở hữu hợp pháp mà không phát hành
...
Chỉ admin. Khôi phục publisher cá nhân cho một GitHub OAuth principal thay thế đã xác minh
mà không chỉnh sửa các hàng tài khoản Convex Auth. Yêu cầu phải nêu cả hai id tài khoản provider GitHub
bất biến; handle có thể thay đổi chỉ được dùng làm kiểm tra an toàn hướng đến operator.
...
chủ thể GitHub. Khôi phục sẽ từ chối an toàn khi nhà xuất bản cá nhân hiện tại của người dùng đích
có kỹ năng, gói hoặc nguồn kỹ năng GitHub.
Khôi phục cũng di chuyển các trường `ownerUserId` cũ cho kỹ năng của nhà xuất bản được khôi phục,
bí danh slug kỹ năng, gói, cảnh báo trình kiểm tra gói và các hàng bản tóm tắt tìm kiếm phái sinh để
các đường dẫn chủ sở hữu trực tiếp khớp với thẩm quyền nhà xuất bản mới. Một đặt chỗ handle được bảo vệ
đang hoạt động cho handle được khôi phục cũng được gán lại cho người dùng thay thế để lần đồng bộ hóa
hồ sơ sau này không thể khôi phục thẩm quyền cạnh tranh của người dùng cũ. Mỗi bảng chính được giới hạn ở
100 hàng cho mỗi giao dịch áp dụng; các lần khôi phục lớn hơn trước hết phải dùng một quy trình di chuyển chủ sở hữu có thể tiếp tục.
Nguồn kỹ năng GitHub có phạm vi theo nhà xuất bản và được báo cáo là đã kiểm tra thay vì được ghi lại.
...
Cấm một người dùng và xóa vĩnh viễn các kỹ năng do họ sở hữu (chỉ moderator/admin).
...
## Điểm cuối CLI cũ (đã ngừng khuyến nghị)
...
dàn dựng một tarball ClawPack phải gửi mã định danh lưu trữ thu được dưới dạng
