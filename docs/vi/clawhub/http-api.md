...
x-i18n:
    generated_at: "2026-07-04T20:33:03Z"
    model: gpt-5.5
...
Các thư mục bên thứ ba có thể dùng endpoint đọc công khai để liệt kê hoặc tìm kiếm Skills trên ClawHub. Vui lòng lưu kết quả vào bộ nhớ đệm, tuân thủ `429`/`Retry-After`, liên kết người dùng trở lại mục ClawHub chính tắc (`https://clawhub.ai/<owner>/skills/<slug>`), và tránh ngụ ý rằng ClawHub xác nhận trang bên thứ ba. Không cố gắng sao chép nội dung ẩn, riêng tư, hoặc bị chặn bởi kiểm duyệt ra ngoài bề mặt API công khai.

Lối tắt slug web phân giải trên nhiều họ registry, nhưng client API nên dùng
...
- Endpoint ghi đã xác thực không nên trả về chỉ `Unauthorized` khi
  máy chủ biết lý do. Token bị thiếu, token không hợp lệ/bị thu hồi, và
  tài khoản bị xóa/cấm/vô hiệu hóa nên nhận được văn bản có thể hành động để client
...
- Tải xuống: 1200/phút trên mỗi IP, 6000/phút trên mỗi khóa (endpoint tải xuống)
...
- Nếu `Retry-After` tồn tại, hãy chờ bấy nhiêu giây trước khi thử lại.
...
- Kết quả được trả về theo thứ tự liên quan (độ tương đồng embedding + tăng hạng token slug/tên khớp chính xác + một độ ưu tiên phổ biến nhỏ).
...
- Văn bản ASCII được tách token theo ranh giới từ và dấu câu. Ví dụ, `personal-map` chứa token `map` độc lập, trong khi `amap-jsapi-skill` chứa `amap`, `jsapi`, và `skill`; vì vậy tìm kiếm `map` cho `personal-map` kết quả khớp từ vựng mạnh hơn `amap-jsapi-skill`.
- Độ phổ biến được chia theo thang log và có giới hạn trần. Skills có tương tác cao vẫn có thể xếp thấp hơn khi văn bản truy vấn khớp yếu hơn.
...
- Đặt các thuật ngữ mà người dùng sẽ tìm kiếm đúng nguyên văn vào tên hiển thị, tóm tắt, và thẻ. Chỉ dùng token slug độc lập khi đó cũng là danh tính ổn định mà bạn muốn giữ.
- Không đổi tên slug chỉ để chạy theo một truy vấn, trừ khi slug mới là tên chính tắc dài hạn tốt hơn. Slug cũ trở thành bí danh chuyển hướng, nhưng URL chính tắc, slug hiển thị, và bản tóm tắt tìm kiếm trong tương lai dùng slug mới.
...
- Quét đã phát hành yêu cầu quyền truy cập quản lý của chủ sở hữu/nhà phát hành, hoặc quyền điều hành viên/quản trị viên nền tảng.
...
- Công việc quét là bất đồng bộ. Yêu cầu quét thủ công được ưu tiên trước công việc phát hành/backfill thông thường, nhưng việc hoàn tất vẫn phụ thuộc vào khả năng sẵn sàng của worker.
...
- Trả về `queue.queuedAhead` và `queue.position` khi đang xếp hàng để client có thể hiển thị số lượng lượt quét thủ công ưu tiên đang đứng trước yêu cầu. Hàng đợi rất lớn được giới hạn và báo cáo bằng `queuedAheadIsEstimate: true`.
- Khi có sẵn, `report` chứa các phần `clawscan`, `skillspector`, `staticAnalysis`, và `virustotal`.
- Công việc quét thất bại trả về `status: "failed"` cùng với `lastError`.
...
- Danh tính skill, danh tính nhà phát hành, và siêu dữ liệu phiên bản đã chọn là các trường phong bì cấp cao nhất (`slug`, `displayName`, `publisherHandle`, `version`, `resolvedFrom`, `tag`, `createdAt`) để tự động hóa shell có thể đọc chúng mà không cần mở gói các wrapper lồng nhau.
...
- Kết quả được trả theo từng mục; một skill hoặc phiên bản bị thiếu không làm thất bại toàn bộ phản hồi.
- Phản hồi chỉ liên quan đến bảo mật. Phản hồi không bao gồm dữ liệu Skill Card, trạng thái thẻ đã tạo, danh sách tệp tạo phẩm, hoặc payload chi tiết của trình quét.
...
`trending` là bảng xếp hạng cài đặt/tải xuống trong bảy ngày và không sử dụng tổng số toàn thời gian.
...
- Skills hiện tại được hỗ trợ bởi GitHub có lần quét `clean` hoặc `suspicious` bao gồm
...
- Các bí danh bộ lọc v1 cũ được ghi tài liệu trong `GET /api/v1/plugins` cũng
  được chấp nhận.
- Lọc danh mục là một bộ lọc API thực sự, dựa trên các hàng tóm tắt danh mục
...
- Yêu cầu mã thông báo API của chủ sở hữu gói, chủ sở hữu/quản trị viên nhà xuất bản tổ chức,
  người điều hành nền tảng hoặc quản trị viên nền tảng.
...
- Các bản phát hành ClawPack bao gồm các trường tương thích npm `npmIntegrity`, `npmShasum` và
  `npmTarballName`.
- `version.sha256hash` là siêu dữ liệu tương thích đã lỗi thời cho máy khách cũ. Trường này
...
- Điểm cuối đọc công khai. Không yêu cầu mã thông báo của chủ sở hữu, nhà xuất bản, người điều hành hoặc quản trị viên.
...
- `trust.moderationState` có thể là null. Trường này là `null` khi không tồn tại kiểm duyệt bản phát hành
  thủ công.
...
- Điểm cuối này chính xác theo phiên bản. Máy khách nên gọi điểm cuối này sau khi phân giải
  phiên bản gói mà họ định cài đặt, không chỉ sau khi đọc siêu dữ liệu gói mới nhất.
- Các gói riêng tư trả về `404` trừ khi bên gọi có thể đọc nhà xuất bản sở hữu.
- Điểm cuối này có chủ đích hẹp hơn các điểm cuối kiểm duyệt của chủ sở hữu/người điều hành.
...
- Các phiên bản gói cũ trả về một artifact `legacy-zip` và `downloadUrl` ZIP
  cũ.
...
- Các phiên bản ClawPack phát trực tiếp chính xác các byte `.tgz` `npm-pack` đã tải lên.
...
Điểm cuối cho người điều hành để liệt kê các hàng di chuyển Plugin OpenClaw chính thức.

Xác thực:

- Yêu cầu mã thông báo API của người dùng là người điều hành hoặc quản trị viên.
...
Điểm cuối cho quản trị viên để tạo hoặc cập nhật một hàng di chuyển Plugin chính thức.
...
- `packageName` được chuẩn hóa theo tên npm; gói có thể vắng mặt đối với các lần di chuyển đã lên kế hoạch.
- Điều này chỉ theo dõi mức độ sẵn sàng di chuyển. Nó không thay đổi OpenClaw hoặc tạo
  ClawPacks.

### `GET /api/v1/packages/moderation/queue`

Điểm cuối cho người điều hành/quản trị viên dành cho hàng đợi đánh giá bản phát hành gói.
...
Báo cáo một gói để người điều hành đánh giá. Báo cáo ở cấp gói, có thể
được liên kết với một phiên bản. Chúng đưa dữ liệu vào hàng đợi kiểm duyệt nhưng tự thân không tự động ẩn hoặc
chặn tải xuống; người điều hành nên dùng kiểm duyệt bản phát hành để
...
Điểm cuối chủ sở hữu/moderator để hiển thị trạng thái kiểm duyệt gói.
...
`finalAction: "revoke"` với báo cáo đã xác nhận để áp dụng kiểm duyệt bản phát hành trong
...
- Kho lưu trữ Plugin/gói là tệp zip với thư mục gốc `package/` để các máy khách OpenClaw
  cũ tiếp tục hoạt động.
- Tuyến này chỉ giữ ở dạng ZIP. Nó không phát trực tuyến tệp ClawPack `.tgz`.
...
Được CLI sử dụng để ánh xạ một fingerprint cục bộ tới một phiên bản đã biết.
...
- Trường payload tùy chọn: `migrateOwner`. Khi là `true` cùng với `ownerHandle`, một
  ...
  nhà xuất bản hiện tại và nhà xuất bản đích. Nếu không chọn rõ tùy chọn này, các thay đổi chủ sở hữu
  sẽ bị từ chối.
...
- Các trường biểu mẫu được phép là `payload`, các blob `files` lặp lại, hoặc một tham chiếu tarball
  ...
  luồng upload-url trả về. Các lượt xuất bản bằng storage-id đã chuẩn bị cũng phải bao gồm
  `clawpackUploadTicket` được trả về cùng URL tải lên đó.
- Dùng `files` hoặc `clawpack`, không dùng cả hai trong cùng một yêu cầu.
- Thân JSON và siêu dữ liệu `payload.files` / `payload.artifact`
...
- Chỉ nhà xuất bản thuộc org `openclaw` và nhà xuất bản cá nhân của các thành viên org
  `openclaw` hiện tại mới có thể xuất bản lên kênh `official`.
- Xuất bản thay mặt người khác vẫn xác thực điều kiện đủ tư cách kênh official theo tài khoản chủ sở hữu đích.
...
Chỉ admin. Đảm bảo một nhà xuất bản org tồn tại cho một handle. Nếu handle vẫn trỏ tới một
...
Chỉ admin. Giữ trước slug gốc và tên gói cho chủ sở hữu hợp pháp mà không xuất bản một
...
Chỉ admin. Khôi phục một nhà xuất bản cá nhân cho một định danh GitHub OAuth thay thế đã xác minh
mà không chỉnh sửa các hàng tài khoản Convex Auth. Yêu cầu phải nêu cả hai id tài khoản provider GitHub
bất biến; các handle có thể thay đổi chỉ được dùng như một lớp bảo vệ hướng tới người vận hành.
...
danh tính chính GitHub. Khôi phục sẽ thất bại đóng khi nhà xuất bản cá nhân hiện tại của người dùng đích
...
bí danh slug skill, packages, cảnh báo trình kiểm tra package, và các hàng tóm tắt tìm kiếm phái sinh để
các đường dẫn chủ sở hữu trực tiếp khớp với thẩm quyền nhà xuất bản mới. Một đặt chỗ protected-handle
...
Nguồn skill GitHub có phạm vi theo nhà xuất bản và được báo cáo là đã kiểm tra thay vì được viết lại.
...
Cấm người dùng và xóa cứng các skills thuộc sở hữu (chỉ người kiểm duyệt/quản trị viên).
...
Bỏ cấm người dùng và khôi phục các skills đủ điều kiện (chỉ quản trị viên).
...
publish tạm chứa tarball ClawPack phải gửi id lưu trữ kết quả dưới dạng
