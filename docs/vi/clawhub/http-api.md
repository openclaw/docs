...
summary: Tài liệu tham chiếu API HTTP (công khai + endpoint CLI + xác thực).
x-i18n:
    generated_at: "2026-07-04T06:38:11Z"
    model: gpt-5.5
...
Các thư mục của bên thứ ba có thể dùng endpoint đọc công khai để liệt kê hoặc tìm kiếm kỹ năng ClawHub. Vui lòng lưu kết quả vào bộ nhớ đệm, tuân thủ `429`/`Retry-After`, liên kết người dùng trở lại danh sách ClawHub chính tắc (`https://clawhub.ai/<owner>/skills/<slug>`), và tránh ngụ ý rằng ClawHub chứng thực trang web bên thứ ba. Đừng cố sao chép nội dung bị ẩn, riêng tư, hoặc bị chặn kiểm duyệt bên ngoài bề mặt API công khai.

Các lối tắt slug web phân giải trên nhiều họ registry, nhưng client API nên dùng
các URL chính tắc được endpoint đọc trả về thay vì tự dựng lại thứ tự ưu tiên
của tuyến.
...
- Các endpoint ghi đã xác thực không nên trả về chỉ một `Unauthorized` trống khi
...
- `X-RateLimit-Remaining` / `RateLimit-Remaining`: ngân sách còn lại chính xác khi có mặt.
  Các yêu cầu thành công theo shard bỏ qua header này thay vì trả về giá trị toàn cục xấp xỉ.
...
- Nếu không có IP client đáng tin cậy, các yêu cầu ẩn danh dùng bucket dự phòng
  ...
  đường dẫn, slug, tên gói, phiên bản, chuỗi truy vấn, hoặc tham số hiện vật khác
  do bên gọi cung cấp.
...
nên đọc phần thân phản hồi như chuỗi dễ đọc cho con người. Tham số truy vấn không xác định được
...
- `highlightedOnly` (tùy chọn): `true` để lọc chỉ còn kỹ năng được làm nổi bật
- `nonSuspiciousOnly` (tùy chọn): `true` để ẩn kỹ năng đáng ngờ (`flagged.suspicious`)
...
- Kết quả được trả về theo thứ tự liên quan (độ tương đồng embedding + tăng hạng token slug/tên khớp chính xác + một prior nhỏ về mức độ phổ biến).
- Độ liên quan mạnh hơn mức độ phổ biến. Một token slug hoặc tên hiển thị khớp chính xác có thể xếp trên một kết quả khớp lỏng hơn nhưng có mức độ tương tác mạnh hơn nhiều.
...
- Mức độ phổ biến được log-scale và giới hạn trần. Kỹ năng có mức tương tác cao có thể xếp hạng thấp hơn khi văn bản truy vấn khớp yếu hơn.
- Trạng thái kiểm duyệt đáng ngờ hoặc ẩn có thể loại kỹ năng khỏi tìm kiếm công khai tùy theo bộ lọc của bên gọi và trạng thái kiểm duyệt hiện tại.

Hướng dẫn khả năng được nhà xuất bản tìm thấy:

- Đặt các thuật ngữ mà người dùng sẽ thực sự tìm kiếm trong tên hiển thị, tóm tắt, và thẻ. Chỉ dùng token slug độc lập khi nó cũng là danh tính ổn định mà bạn muốn giữ.
- Đừng đổi tên slug chỉ để bám theo một truy vấn, trừ khi slug mới là tên chính tắc dài hạn tốt hơn. Slug cũ trở thành bí danh chuyển hướng, nhưng URL chính tắc, slug hiển thị, và bản tổng hợp tìm kiếm trong tương lai dùng slug mới.
- Bí danh đổi tên giữ khả năng phân giải cho URL cũ và các lượt cài đặt phân giải qua registry, nhưng thứ hạng tìm kiếm dựa trên metadata kỹ năng chính tắc sau khi việc đổi tên đã được lập chỉ mục. Thống kê hiện có vẫn gắn với kỹ năng.
...
- Slug cũ được tạo bởi luồng đổi tên/gộp chủ sở hữu phân giải tới kỹ năng chính tắc.
...
- Chủ sở hữu và điều phối viên có thể truy cập chi tiết kiểm duyệt cho kỹ năng bị ẩn.
...
Endpoint điều phối viên/quản trị viên để giải quyết hoặc mở lại báo cáo kỹ năng.
...
đặt `status` trở lại `open`. Truyền `finalAction: "hide"` với báo cáo đã phân loại
...
- Bao gồm trạng thái xác minh đã chuẩn hóa cùng với chi tiết riêng của trình quét.
- `security.hasScanResult` chỉ là `true` khi một trình quét tạo ra kết luận dứt khoát (`clean`, `suspicious`, hoặc `malicious`).
- `moderation` là ảnh chụp kiểm duyệt hiện tại ở cấp độ skill, được suy ra từ phiên bản mới nhất.
...
- Quét đã xuất bản yêu cầu quyền truy cập quản lý của chủ sở hữu/nhà xuất bản, hoặc quyền kiểm duyệt/quản trị viên nền tảng.
...
- Tác vụ quét là bất đồng bộ. Yêu cầu quét thủ công được ưu tiên trước công việc xuất bản/điền bù thông thường, nhưng việc hoàn tất vẫn phụ thuộc vào mức độ sẵn sàng của worker.
...
- Yêu cầu quyền truy cập quản lý của chủ sở hữu/nhà xuất bản đối với skill hoặc plugin, hoặc quyền kiểm duyệt/quản trị viên nền tảng.
- Trả về kết quả quét đã lưu cho đúng phiên bản đã gửi, bao gồm cả các phiên bản bị chặn hoặc bị ẩn.
- `kind` mặc định là `skill`; dùng `kind=plugin` cho quét plugin/gói.
- Trả về cùng dạng ZIP như các lượt tải xuống yêu cầu quét.

### `POST /api/v1/skills/-/scan/batch`

Tuyến quét lại hàng loạt chuẩn hóa chỉ dành cho quản trị viên. Tuyến này chấp nhận cùng dạng payload như `POST /api/v1/skills/-/rescan-batch` cũ.

### `POST /api/v1/skills/-/scan/batch/status`

Tuyến trạng thái hàng loạt chuẩn hóa chỉ dành cho quản trị viên. Tuyến này chấp nhận `{ "jobIds": ["..."] }` và trả về cùng các bộ đếm tổng hợp như `POST /api/v1/skills/-/rescan-batch/status` cũ.
...
- `ok` chỉ là `true` khi phiên bản đã chọn có Skill Card đã tạo, không bị kiểm duyệt chặn vì phần mềm độc hại, và xác minh ClawScan là sạch.
- Danh tính skill, danh tính nhà xuất bản, và siêu dữ liệu phiên bản đã chọn là các trường phong bì cấp cao nhất (`slug`, `displayName`, `publisherHandle`, `version`, `resolvedFrom`, `tag`, `createdAt`) để tự động hóa shell có thể đọc chúng mà không cần giải nén các wrapper lồng nhau.
...
- `security.signals.dependencyRegistry` được giữ lại để tương thích phản hồi v1, nhưng trình quét sự tồn tại sổ đăng ký phụ thuộc đã ngừng hoạt động và khóa này luôn là `null`.
...
Trả về các kết luận bảo mật rút gọn hiện tại cho đúng phiên bản skill. Điểm cuối
tập hợp này dành cho các client đã biết phiên bản skill ClawHub đã cài đặt
...
- Kết quả được trả theo từng mục; một skill hoặc phiên bản bị thiếu không làm toàn bộ phản hồi thất bại.
- Phản hồi chỉ liên quan đến bảo mật. Nó không bao gồm dữ liệu Skill Card, trạng thái thẻ đã tạo, danh sách tệp hiện vật, hoặc payload chi tiết của trình quét.
- `security.signals` chỉ chứa bằng chứng hỗ trợ ở cấp trạng thái; dùng `/scan` hoặc trang kiểm toán bảo mật ClawHub để xem toàn bộ chi tiết trình quét.
- `security.signals.dependencyRegistry` được giữ lại để tương thích phản hồi v1, nhưng trình quét sự tồn tại sổ đăng ký phụ thuộc đã ngừng hoạt động và khóa này luôn là `null`.
...
- Dùng `/verify` khi bạn cần phong bì xác minh Skill Card cho một skill, `/card` khi bạn cần markdown thẻ đã tạo, và `/scan` khi bạn cần dữ liệu chi tiết từ trình quét.
...
- Các mục kỹ năng vẫn được hậu thuẫn bởi sổ đăng ký kỹ năng và vẫn chỉ có thể được xuất bản qua `POST /api/v1/skills`.
...
Tìm kiếm danh mục hợp nhất trên kỹ năng + gói Plugin.
...
Trên điểm cuối hợp nhất `/api/v1/packages`, giá trị này chỉ dành cho Plugin; dùng
...
- Cần có mã thông báo API.

Tham số truy vấn:

- `startDate` (bắt buộc): cận dưới tính bằng mili giây Unix cho `updatedAt` của kỹ năng.
- `endDate` (bắt buộc): cận trên tính bằng mili giây Unix cho `updatedAt` của kỹ năng.
...
- Các kỹ năng được lưu trữ bao gồm các tệp phiên bản đã lưu mới nhất và được liệt kê trong
  `_manifest.json` với `sourceRef: "public-clawhub"`.
- Các kỹ năng hiện tại dựa trên GitHub có lần quét `clean` hoặc `suspicious` bao gồm
...
- Các bí danh bộ lọc v1 cũ được ghi tài liệu trong `GET /api/v1/plugins` cũng
  được chấp nhận.
- Lọc danh mục là bộ lọc API thật được hỗ trợ bởi các hàng tóm tắt danh mục
  Plugin, không phải là thao tác viết lại truy vấn tìm kiếm.
- Kết quả được trả về theo thứ tự mức độ liên quan và hiện chưa phân trang.
- Các điều khiển sắp xếp giao diện trình duyệt cho tìm kiếm Plugin sắp xếp lại các kết quả mức độ liên quan đã tải,
...
- Yêu cầu mã thông báo API của chủ sở hữu gói, chủ sở hữu/quản trị viên publisher của tổ chức,
...
Trả về một phiên bản gói, bao gồm siêu dữ liệu tệp, tính tương thích,
...
- Các bản phát hành ClawPack bao gồm các trường tương thích với npm là `npmIntegrity`, `npmShasum`, và
  `npmTarballName`.
- `version.sha256hash` là siêu dữ liệu tương thích đã lỗi thời dành cho client cũ. Nó
...
Trả về đúng tóm tắt bảo mật và độ tin cậy của bản phát hành gói cho các
...
- Điểm cuối đọc công khai. Không yêu cầu mã thông báo của chủ sở hữu, publisher, kiểm duyệt viên, hoặc quản trị viên.
...
- `trust.blockedFromDownload` là tín hiệu chặn cài đặt. OpenClaw và các
  ...
  tự suy diễn lại quy tắc chặn từ các trường trình quét hoặc kiểm duyệt.
- `trust.reasons` là danh sách giải thích hướng người dùng và phục vụ kiểm toán. Mã lý do
...
- `trust.stale` nghĩa là tóm tắt tin cậy được tính từ các đầu vào đã lỗi thời và
  nên được xem là cần làm mới trước khi đưa ra quyết định cho phép có độ tin cậy cao.

Ghi chú:

- Điểm cuối này khớp chính xác theo phiên bản. Client nên gọi nó sau khi phân giải
  phiên bản gói mà họ định cài đặt, không chỉ sau khi đọc siêu dữ liệu gói
  mới nhất.
- Các gói riêng tư trả về `404` trừ khi bên gọi có thể đọc publisher sở hữu.
- Điểm cuối này cố ý hẹp hơn các điểm cuối kiểm duyệt dành cho chủ sở hữu/kiểm duyệt viên.
...
- Đây là bề mặt bộ phân giải của OpenClaw; nó tránh suy đoán định dạng kho lưu trữ từ
  một URL dùng chung.

### `GET /api/v1/packages/{name}/versions/{version}/artifact/download`

Tải xuống artifact phiên bản thông qua đường dẫn bộ phân giải rõ ràng.
...
Điểm cuối kiểm duyệt viên để liệt kê các hàng di chuyển Plugin OpenClaw chính thức.

Xác thực:

- Yêu cầu mã thông báo API của người dùng kiểm duyệt viên hoặc quản trị viên.
...
- `packageName` được chuẩn hóa theo tên npm; gói có thể bị thiếu đối với các di chuyển
  đã lên kế hoạch.
- Điều này chỉ theo dõi mức độ sẵn sàng di chuyển. Nó không thay đổi OpenClaw hay tạo
  ClawPack.

### `GET /api/v1/packages/moderation/queue`

Điểm cuối kiểm duyệt viên/quản trị viên cho hàng đợi đánh giá bản phát hành gói.
...
Báo cáo một gói để kiểm duyệt viên đánh giá. Báo cáo ở cấp gói, tùy chọn
liên kết với một phiên bản. Chúng đưa dữ liệu vào hàng đợi kiểm duyệt nhưng tự chúng không tự động ẩn hoặc
chặn tải xuống; kiểm duyệt viên nên dùng kiểm duyệt bản phát hành để
...
`finalAction: "revoke"` cùng với một báo cáo đã xác nhận để áp dụng điều phối bản phát hành trong cùng
...
- `approved`: đã được đánh giá thủ công và cho phép.
...
Tải xuống kho lưu trữ ZIP tất định kiểu cũ cho một bản phát hành gói.
...
- Tuyến này chỉ duy trì ZIP. Nó không phát trực tiếp các tệp ClawPack `.tgz`.
...
Trả về packument tương thích với npm cho các phiên bản gói được hỗ trợ bởi ClawPack.
...
Phát trực tiếp đúng các byte tarball ClawPack đã tải lên cho máy khách mirror npm.
...
Được CLI dùng để ánh xạ một dấu vân tay cục bộ tới một phiên bản đã biết.

Tham số truy vấn:

- `slug` (bắt buộc)
- `hash` (bắt buộc): sha256 hệ hex 64 ký tự của dấu vân tay bundle
...
skill hiện tại được hỗ trợ bởi GitHub có kết quả quét `clean` hoặc `suspicious` và không có
...
- Bàn giao skill được hỗ trợ bởi GitHub không proxy hoặc mirror byte. Phản hồi JSON
  bao gồm `sourceRef: "public-github"`, `repo`, `commit`, `path`, `contentHash`,
  và `archiveUrl`; trạng thái quét/hiện tại là một cổng kiểm soát và không được bao gồm dưới dạng siêu dữ liệu payload
  thành công.
...
- Các trường biểu mẫu được phép là `payload`, các blob `files` lặp lại, hoặc một tham chiếu tarball `clawpack`.
  ...
  luồng upload-url. Các lần xuất bản bằng storage-id đã dàn dựng cũng phải bao gồm
...
- Xuất bản thay mặt người khác vẫn xác thực điều kiện đủ của kênh official dựa trên tài khoản chủ sở hữu đích.
...
Các thao tác ẩn bởi điều phối viên/quản trị viên và gỡ bỏ vì bảo mật không hết hạn theo cách này.
...
Chỉ dành cho quản trị viên. Đảm bảo tồn tại một nhà xuất bản tổ chức cho một handle. Nếu handle vẫn trỏ tới một
người dùng/nhà xuất bản cá nhân dùng chung kiểu cũ, điểm cuối sẽ di chuyển nó vào nhà xuất bản tổ chức trước.
...
bên gọi làm chủ sở hữu. Điểm cuối này không di chuyển handle người dùng/cá nhân hiện có và không
đánh dấu nhà xuất bản là được tin cậy/official.
...
bất biến; các handle có thể thay đổi chỉ được dùng làm chốt kiểm tra hướng tới người vận hành.

Endpoint mặc định là chạy thử. Để áp dụng khôi phục, cần `dryRun: false` và
`confirmIdentityVerified: true` sau khi nhân viên xác minh độc lập tính liên tục giữa cả hai
GitHub principal. Khôi phục thất bại theo hướng đóng khi publisher cá nhân hiện tại của người dùng đích
có skills, packages hoặc nguồn GitHub skill.
Khôi phục cũng di chuyển các trường `ownerUserId` cũ cho skills,
bí danh slug skill, packages, cảnh báo trình kiểm tra package và các hàng search digest dẫn xuất của publisher được khôi phục để
các đường dẫn chủ sở hữu trực tiếp khớp với thẩm quyền publisher mới. Một đặt trước protected-handle đang hoạt động
...
100 hàng cho mỗi giao dịch áp dụng; các lần khôi phục lớn hơn trước tiên phải dùng một quá trình di chuyển chủ sở hữu có thể tiếp tục.
Nguồn GitHub skill có phạm vi theo publisher và được báo cáo là đã kiểm tra thay vì được ghi lại.
...
Cấm một người dùng và xóa cứng các skill thuộc sở hữu của họ (chỉ moderator/admin).
...
Thêm/xóa một star (highlight). Cả hai endpoint đều idempotent.
...
## Endpoint CLI kế thừa (không còn được khuyến nghị)
...
đưa một tarball ClawPack vào staging phải gửi storage id thu được dưới dạng
...
Nếu bạn tự host, hãy phục vụ tệp này (hoặc đặt rõ `CLAWHUB_REGISTRY`; `CLAWDHUB_REGISTRY` kế thừa).
