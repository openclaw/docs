...
x-i18n:
    generated_at: "2026-07-02T00:59:50Z"
    model: gpt-5.5
...
Các thư mục bên thứ ba có thể dùng các endpoint đọc công khai để liệt kê hoặc tìm kiếm Skills trên ClawHub. Vui lòng lưu cache kết quả, tuân thủ `429`/`Retry-After`, liên kết người dùng trở lại mục ClawHub chính tắc (`https://clawhub.ai/<owner>/skills/<slug>`), và tránh ngụ ý rằng ClawHub chứng thực trang bên thứ ba. Không cố gắng sao chép nội dung bị ẩn, riêng tư, hoặc bị chặn bởi kiểm duyệt ra ngoài bề mặt API công khai.

Các lối tắt slug trên web phân giải trên nhiều họ registry, nhưng client API nên dùng
các URL chính tắc được endpoint đọc trả về thay vì dựng lại thứ tự ưu tiên
của route.
...
- Các endpoint ghi đã xác thực không nên trả về chỉ `Unauthorized` khi
  ...
  tài khoản đã bị xóa/cấm/vô hiệu hóa nên mỗi trường hợp nhận văn bản có thể hành động để client
...
- Nếu `Retry-After` tồn tại, hãy chờ đúng số giây đó trước khi thử lại.
...
- ClawHub dùng header chuyển tiếp đáng tin cậy để nhận diện IP client ở edge.
...
nên đọc body phản hồi như một chuỗi dễ đọc cho con người. Tham số truy vấn không xác định sẽ
bị bỏ qua để tương thích, nhưng tham số truy vấn được nhận diện có giá trị không hợp lệ sẽ trả về
`400`.
...
- Kết quả được trả về theo thứ tự liên quan (độ tương đồng embedding + điểm tăng cho token slug/tên khớp chính xác + một độ ưu tiên phổ biến nhỏ).
- Độ liên quan mạnh hơn mức độ phổ biến. Một slug chính xác hoặc token tên hiển thị khớp có thể xếp hạng cao hơn một kết quả khớp lỏng hơn nhưng có tương tác mạnh hơn nhiều.
...
- Mức độ phổ biến được scale theo log và bị giới hạn trần. Skills có mức tương tác cao có thể xếp hạng thấp hơn khi văn bản truy vấn khớp yếu hơn.
...
- Đặt các thuật ngữ mà người dùng sẽ thực sự tìm kiếm vào tên hiển thị, tóm tắt, và tag. Chỉ dùng một token slug độc lập khi nó cũng là một danh tính ổn định mà bạn muốn giữ.
- Không đổi tên slug chỉ để chạy theo một truy vấn, trừ khi slug mới là một tên chính tắc dài hạn tốt hơn. Slug cũ trở thành bí danh chuyển hướng, nhưng URL chính tắc, slug được hiển thị, và các digest tìm kiếm trong tương lai dùng slug mới.
- Bí danh đổi tên giữ nguyên khả năng phân giải cho URL cũ và các lượt cài đặt phân giải qua registry, nhưng thứ hạng tìm kiếm dựa trên metadata Skill chính tắc sau khi việc đổi tên đã được lập chỉ mục. Thống kê hiện có vẫn gắn với Skill.
- Nếu một Skill bất ngờ không hiển thị, hãy kiểm tra trạng thái kiểm duyệt trước bằng `clawhub inspect @owner/slug` khi đã đăng nhập trước khi thay đổi metadata liên quan đến thứ hạng.
...
- Dùng `nextCursor` để tiếp tục phân trang khi có mặt. Một trang ngắn tự thân nó không có nghĩa là đã hết kết quả.
...
- Slug cũ được tạo bởi luồng đổi tên/hợp nhất owner sẽ phân giải đến Skill chính tắc.
...
- Bằng chứng được biên tập lại cho caller công khai và chỉ bao gồm snippet thô cho owner/người kiểm duyệt.

### `POST /api/v1/skills/{slug}/report`

Báo cáo một Skill để người kiểm duyệt xem xét. Báo cáo ở cấp Skill, có thể liên kết tùy chọn
...
- `version.security` bao gồm trạng thái xác minh quét đã được chuẩn hóa và chi tiết scanner
...
- Tác vụ quét là bất đồng bộ. Yêu cầu quét thủ công được ưu tiên trước công việc xuất bản/lấp dữ liệu thông thường, nhưng việc hoàn tất vẫn phụ thuộc vào mức sẵn sàng của worker.
...
- Tác vụ quét thất bại trả về `status: "failed"` kèm `lastError`.
...
- `ok` chỉ là `true` khi phiên bản được chọn có Skill Card đã tạo, không bị kiểm duyệt chặn vì mã độc, và xác minh ClawScan là sạch.
...
Trả về các kết luận bảo mật rút gọn hiện tại cho đúng các phiên bản skill. Endpoint
...
- Các bí danh bộ lọc v1 cũ được ghi tài liệu dưới `GET /api/v1/plugins` cũng
  được chấp nhận.
- Lọc danh mục là một bộ lọc API thực, dựa trên các hàng tóm tắt danh mục Plugin,
  không phải thao tác viết lại truy vấn tìm kiếm.
- Kết quả được trả về theo thứ tự mức độ liên quan và hiện chưa phân trang.
- Các điều khiển sắp xếp của giao diện trình duyệt cho tìm kiếm Plugin sắp xếp lại các kết quả liên quan đã tải,
...
Trả về chính xác bản tóm tắt bảo mật và độ tin cậy của bản phát hành gói cho các
...
- `release.artifactKind`, `release.artifactSha256`, `release.npmIntegrity`,
  `release.npmShasum`, và `release.npmTarballName` có mặt khi được biết đối với
  artifact phát hành.
...
- `trust.moderationState` có thể là null. Giá trị là `null` khi không có kiểm duyệt bản phát hành
  thủ công.
...
- Điểm cuối này khớp chính xác theo phiên bản. Client nên gọi nó sau khi phân giải
  phiên bản gói định cài đặt, không chỉ sau khi đọc siêu dữ liệu gói mới nhất.
- Các gói riêng tư trả về `404` trừ khi bên gọi có thể đọc nhà phát hành sở hữu.
- Điểm cuối này được cố ý giữ hẹp hơn các điểm cuối kiểm duyệt dành cho chủ sở hữu/điều phối viên.
  Nó phơi bày quyết định cài đặt và giải thích công khai, không phơi bày
  danh tính người báo cáo, nội dung báo cáo, bằng chứng riêng tư, hoặc mốc thời gian rà soát nội bộ.
...
Trả về mức sẵn sàng đã tính cho tiêu thụ OpenClaw trong tương lai.
...
- `packageName` được chuẩn hóa theo tên npm; gói có thể vắng mặt đối với các lần di chuyển
  đã lên kế hoạch.
...
Báo cáo một gói để điều phối viên rà soát. Báo cáo ở cấp gói, có thể
liên kết với một phiên bản. Chúng đưa dữ liệu vào hàng đợi kiểm duyệt nhưng tự bản thân không tự động ẩn hoặc
chặn lượt tải xuống; điều phối viên nên dùng kiểm duyệt bản phát hành để
...
Điểm cuối owner/moderator để hiển thị thông tin kiểm duyệt gói.
...
Điểm cuối moderator/admin để xem xét bản phát hành gói.
...
- Kho lưu trữ Plugin/gói là các tệp zip có gốc `package/` để client OpenClaw cũ
  tiếp tục hoạt động.
- Tuyến này chỉ duy trì ZIP. Nó không truyền phát tệp ClawPack `.tgz`.
...
Truyền phát đúng từng byte tarball ClawPack đã tải lên cho client mirror npm.

Ghi chú:

- Dùng bucket giới hạn tốc độ tải xuống.
- Header tải xuống bao gồm SHA-256 của ClawHub cùng siêu dữ liệu integrity/shasum của npm.
- Kiểm duyệt và kiểm tra quyền truy cập gói riêng tư vẫn áp dụng.
...
- Trường payload tùy chọn: `ownerHandle`. Khi có, API phân giải nhà xuất bản đó
  ở phía server và yêu cầu tác nhân có quyền truy cập nhà xuất bản.
- Trường payload tùy chọn: `migrateOwner`. Khi là `true` với `ownerHandle`, một
  skill hiện có có thể chuyển sang chủ sở hữu đó nếu tác nhân là admin/owner trên cả
  nhà xuất bản hiện tại và nhà xuất bản đích. Nếu không có opt-in này, thay đổi chủ sở hữu
  bị từ chối.
...
- Chỉ nhà xuất bản org `openclaw` và nhà xuất bản cá nhân của các thành viên org `openclaw`
  hiện tại mới có thể xuất bản lên kênh `official`.
- Xuất bản thay mặt người khác vẫn xác thực điều kiện đủ để dùng kênh official theo tài khoản chủ sở hữu đích.

### `DELETE /api/v1/skills/{slug}` / `POST /api/v1/skills/{slug}/undelete`

Soft-delete / khôi phục một skill (owner, moderator, hoặc admin).
...
Soft delete do chủ sở hữu khởi tạo giữ slug trong 30 ngày, sau đó slug có thể được
nhà xuất bản khác nhận. Phản hồi xóa bao gồm `slugReservedUntil` khi áp dụng thời điểm hết hạn này.
...
Chỉ admin. Đảm bảo tồn tại một nhà xuất bản org cho một handle. Nếu handle vẫn trỏ tới một
người dùng/nhà xuất bản cá nhân dùng chung kiểu cũ, điểm cuối sẽ di chuyển nó thành nhà xuất bản org trước.
...
Tạo nhà xuất bản org tự phục vụ đã xác thực. Tạo một nhà xuất bản org mới và thêm
bên gọi làm owner. Điểm cuối này không di chuyển các handle người dùng/cá nhân hiện có và
...
chủ sở hữu có thể xuất bản bản phát hành code-plugin hoặc bundle-plugin thật vào tên đó sau.
...
provider GitHub bất biến; các handle có thể thay đổi chỉ được dùng làm rào chắn hướng tới operator.

Điểm cuối mặc định ở chế độ chạy thử. Để áp dụng khôi phục, cần `dryRun: false` và
`confirmIdentityVerified: true` sau khi nhân viên xác minh độc lập tính liên tục giữa cả hai
GitHub principal. Khôi phục sẽ fail closed khi nhà xuất bản cá nhân hiện tại của người dùng đích
...
các đường dẫn chủ sở hữu trực tiếp khớp với thẩm quyền nhà xuất bản mới. Một đặt chỗ protected-handle đang hoạt động
...
100 hàng cho mỗi giao dịch áp dụng; các khôi phục lớn hơn trước tiên phải dùng migration chủ sở hữu có thể tiếp tục.
Nguồn GitHub skill có phạm vi theo nhà xuất bản và được báo cáo là đã kiểm tra thay vì được viết lại.
...
Cấm một người dùng và hard-delete các skill thuộc sở hữu (chỉ moderator/admin).
...
đưa một ClawPack tarball vào giai đoạn trung gian phải gửi id lưu trữ kết quả dưới dạng
`clawpack` và ticket được trả về dưới dạng `clawpackUploadTicket`.

## Khám phá registry (`/.well-known/clawhub.json`)

CLI có thể khám phá thiết lập registry/auth từ site:
...
Nếu bạn tự host, hãy phục vụ tệp này (hoặc đặt rõ ràng `CLAWHUB_REGISTRY`; `CLAWDHUB_REGISTRY` cũ).
