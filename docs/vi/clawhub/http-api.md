...
Các thư mục của bên thứ ba có thể dùng các endpoint đọc công khai để liệt kê hoặc tìm kiếm ClawHub skills. Vui lòng lưu kết quả vào bộ nhớ đệm, tuân thủ `429`/`Retry-After`, liên kết người dùng trở lại danh sách ClawHub chính tắc (`https://clawhub.ai/<owner>/skills/<slug>`), và tránh ngụ ý rằng ClawHub xác nhận trang bên thứ ba. Không cố gắng phản chiếu nội dung bị ẩn, riêng tư, hoặc bị chặn kiểm duyệt ra ngoài bề mặt API công khai.

Các lối tắt slug trên web phân giải qua các họ registry, nhưng client API nên dùng
...
- `X-RateLimit-Remaining` / `RateLimit-Remaining`: ngân sách còn lại chính xác khi có mặt.
  Các yêu cầu thành công được phân mảnh sẽ bỏ qua header này thay vì trả về một giá trị toàn cục xấp xỉ.
...
- Nếu không có IP client đáng tin cậy, các yêu cầu ẩn danh dùng bucket dự phòng
  chỉ được phạm vi theo loại giới hạn tốc độ. Các bucket dự phòng này không bao gồm
...
nên đọc phần thân phản hồi dưới dạng chuỗi người đọc hiểu được. Các tham số truy vấn không xác định được
...
- Kết quả được trả về theo thứ tự liên quan (độ tương đồng embedding + tăng hạng token slug/tên khớp chính xác + một ưu tiên phổ biến nhỏ).
...
- Độ phổ biến được scale theo log và giới hạn trần. Skills có mức tương tác cao có thể xếp hạng thấp hơn khi văn bản truy vấn khớp yếu hơn.
...
- Đặt các thuật ngữ người dùng sẽ tìm kiếm đúng theo nghĩa đen trong tên hiển thị, tóm tắt, và thẻ. Chỉ dùng token slug độc lập khi nó cũng là danh tính ổn định mà bạn muốn giữ.
- Không đổi tên slug chỉ để chạy theo một truy vấn, trừ khi slug mới là tên chính tắc dài hạn tốt hơn. Slug cũ trở thành alias chuyển hướng, nhưng URL chính tắc, slug hiển thị, và bản tóm lược tìm kiếm tương lai dùng slug mới.
- Alias đổi tên giữ khả năng phân giải cho URL cũ và các lượt cài đặt phân giải qua registry, nhưng xếp hạng tìm kiếm dựa trên metadata skill chính tắc sau khi việc đổi tên đã được lập chỉ mục. Các thống kê hiện có vẫn đi cùng skill.
- Nếu một skill bất ngờ không hiển thị, trước tiên hãy kiểm tra trạng thái kiểm duyệt bằng `clawhub inspect @owner/slug` trong khi đã đăng nhập trước khi thay đổi metadata liên quan đến xếp hạng.
...
- `createdAt` ổn định cho các lần crawl skill mới; `updated` thay đổi khi skills hiện có được phát hành lại.
...
- Các slug cũ được tạo bởi luồng đổi tên/hợp nhất chủ sở hữu phân giải tới skill chính tắc.
- `metadata.os`: ràng buộc OS được khai báo trong frontmatter skill (ví dụ `["macos"]`, `["linux"]`). `null` nếu không được khai báo.
...
đặt `status` trở lại `open`. Truyền `finalAction: "hide"` với một báo cáo đã phân loại
...
Trả về chi tiết xác minh quét bảo mật cho phiên bản skill.
...
- Các tác vụ quét là bất đồng bộ. Yêu cầu quét thủ công được ưu tiên trước công việc xuất bản/điền bù thông thường, nhưng thời điểm hoàn tất vẫn phụ thuộc vào mức độ sẵn sàng của worker.
...
- Trả về `queue.queuedAhead` và `queue.position` khi đang queued để client có thể hiển thị số lượt quét thủ công được ưu tiên đang đứng trước yêu cầu. Các hàng đợi rất lớn được giới hạn và báo cáo với `queuedAheadIsEstimate: true`.
...
- Trả về cùng định dạng ZIP như các bản tải xuống yêu cầu quét.

### `POST /api/v1/skills/-/scan/batch`

Tuyến quét lại hàng loạt chuẩn chỉ dành cho quản trị viên. Nó chấp nhận cùng dạng payload với `POST /api/v1/skills/-/rescan-batch` kế thừa.
...
Trả về phong bì xác minh Skill Card được dùng bởi `clawhub skill verify`.
...
- Danh tính skill, danh tính nhà xuất bản, và siêu dữ liệu phiên bản được chọn là các trường cấp cao nhất của phong bì (`slug`, `displayName`, `publisherHandle`, `version`, `resolvedFrom`, `tag`, `createdAt`) để tự động hóa shell có thể đọc chúng mà không cần mở các wrapper lồng nhau.
...
Trả về các phán quyết bảo mật cô đọng hiện tại cho đúng phiên bản skill. Endpoint
...
- Phản hồi chỉ về bảo mật. Nó không bao gồm dữ liệu Skill Card, trạng thái thẻ đã tạo, danh sách tệp artifact, hoặc payload chi tiết của trình quét.
...
- `security.signals.dependencyRegistry` được giữ lại để tương thích phản hồi v1, nhưng trình quét sự tồn tại của registry phụ thuộc đã ngừng hoạt động và khóa này luôn là `null`.
- Việc thiếu Skill Card không ảnh hưởng đến `ok`, `decision`, hoặc `reasons` của endpoint này; client nên đọc `skill-card.md` đã cài đặt ở cục bộ khi cần nội dung thẻ.
...
- Giá trị không hợp lệ cho `family`, `channel`, `isOfficial`, `featured`,
  `highlightedOnly`, hoặc `sort` sẽ trả về `400`. Các tham số truy vấn không xác định sẽ bị bỏ qua.
- `GET /api/v1/code-plugins` và `GET /api/v1/bundle-plugins` vẫn là các bí danh family cố định.
- Các mục kỹ năng vẫn được hỗ trợ bởi sổ đăng ký kỹ năng và vẫn chỉ có thể được xuất bản thông qua `POST /api/v1/skills`.
...
- Bên gọi đã xác thực có thể thấy các gói riêng tư của những nhà xuất bản mà họ thuộc về trong kết quả danh sách/tìm kiếm.
- `channel=private` chỉ trả về các gói mà bên gọi đã xác thực có quyền đọc.
...
- Giá trị không hợp lệ cho `family`, `channel`, `isOfficial`, `featured`, hoặc
  `highlightedOnly` sẽ trả về `400`. Các tham số truy vấn không xác định sẽ bị bỏ qua.
- Bên gọi ẩn danh chỉ thấy các kênh gói công khai.
- Bên gọi đã xác thực có thể tìm kiếm các gói riêng tư của những nhà xuất bản mà họ thuộc về.
- `channel=private` chỉ trả về các gói mà bên gọi đã xác thực có quyền đọc.
...
`trending` là bảng xếp hạng cài đặt/tải xuống trong bảy ngày và không dùng tổng số toàn thời gian.
Trên điểm cuối hợp nhất `/api/v1/packages`, tùy chọn này chỉ dành cho Plugin; dùng
...
- Kỹ năng được lưu trữ bao gồm các tệp phiên bản mới nhất đã lưu và được liệt kê trong
  `_manifest.json` với `sourceRef: "public-clawhub"`.
- Kỹ năng hiện tại được GitHub hỗ trợ có bản quét `clean` hoặc `suspicious` bao gồm
...
- Các bí danh bộ lọc v1 cũ được ghi tài liệu trong `GET /api/v1/plugins` cũng
  được chấp nhận.
- Lọc theo danh mục là một bộ lọc API thực sự, được hỗ trợ bởi các hàng digest
...
- Các bản phát hành ClawPack bao gồm các trường tương thích npm `npmIntegrity`, `npmShasum`, và
  `npmTarballName`.
- `version.sha256hash` là siêu dữ liệu tương thích đã lỗi thời cho các client cũ. Nó
  băm đúng các byte ZIP được trả về bởi `/api/v1/packages/{name}/download`.
  Các client hiện đại nên dùng `version.artifact.sha256`, trường này định danh
...
- `package.name`, `package.displayName`, và `package.family` định danh gói registry
  đã phân giải.
...
- Các gói riêng tư trả về `404` trừ khi bên gọi có thể đọc nhà phát hành sở hữu.
- Điểm cuối này có chủ ý hẹp hơn các điểm cuối điều phối dành cho chủ sở hữu/điều phối viên.
...
Trả về siêu dữ liệu resolver artifact tường minh cho một phiên bản gói.
...
Tải xuống artifact phiên bản qua đường dẫn resolver tường minh.
...
Trả về mức sẵn sàng đã tính toán cho tiêu thụ OpenClaw trong tương lai.
...
Điểm cuối dành cho điều phối viên để liệt kê các hàng di trú Plugin OpenClaw chính thức.
...
- `packageName` được chuẩn hóa theo tên npm; gói có thể chưa tồn tại đối với các di trú
  đã lên kế hoạch.
- Mục này chỉ theo dõi mức sẵn sàng di trú. Nó không thay đổi OpenClaw hoặc tạo
  ClawPack.
...
Endpoint dành cho chủ sở hữu/điều phối viên để xem thông tin kiểm duyệt gói.
...
Endpoint dành cho điều phối viên/quản trị viên để xử lý hoặc mở lại báo cáo gói.
...
Endpoint dành cho điều phối viên/quản trị viên để rà soát bản phát hành gói.
...
Các bản phát hành bị cách ly và bị thu hồi trả về `403` từ các tuyến tải artifact xuống.
Mọi thay đổi đều ghi một mục nhật ký kiểm toán.

### `GET /api/v1/packages/{name}/file`

Trả về nội dung văn bản thô của một tệp gói.
...
- Siêu dữ liệu chỉ dành cho registry không được chèn vào kho lưu trữ tải xuống.
...
- `slug` (bắt buộc)
- `hash` (bắt buộc): sha256 hex 64 ký tự của fingerprint gói
...
- Trường payload tùy chọn: `ownerHandle`. Khi có, API phân giải
  nhà xuất bản đó phía server và yêu cầu tác nhân có quyền truy cập nhà xuất bản.
- Trường payload tùy chọn: `migrateOwner`. Khi là `true` cùng với `ownerHandle`, một
  kỹ năng hiện có có thể được chuyển sang chủ sở hữu đó nếu tác nhân là quản trị viên/chủ sở hữu trên cả
...
- Các trường form được phép là `payload`, các blob `files` lặp lại, hoặc một tham chiếu tarball
  ...
  luồng upload-url. Các lượt phát hành bằng storage-id đã staged cũng phải bao gồm
...
- Phát hành thay mặt vẫn xác thực điều kiện đủ của kênh official theo tài khoản chủ sở hữu đích.

### `DELETE /api/v1/skills/{slug}` / `POST /api/v1/skills/{slug}/undelete`

Xóa mềm / khôi phục một kỹ năng (chủ sở hữu, điều phối viên, hoặc quản trị viên).
...
Ẩn bởi điều phối viên/quản trị viên và gỡ bỏ bảo mật không hết hạn theo cách này.
...
nhà xuất bản người dùng/cá nhân dùng chung kế thừa, endpoint sẽ di chuyển nó thành nhà xuất bản tổ chức trước.
Với một tổ chức mới tạo, cung cấp `memberHandle`; quản trị viên đang thao tác không được thêm làm thành viên.
...
chủ sở hữu có thể phát hành bản phát hành code-plugin hoặc bundle-plugin thật vào tên đó sau này.
...
bất biến; handle có thể thay đổi chỉ được dùng làm chốt kiểm tra hướng đến người vận hành.
...
chủ thể GitHub. Khôi phục sẽ từ chối theo hướng an toàn khi nhà xuất bản cá nhân hiện tại
của người dùng đích có kỹ năng, gói, hoặc nguồn kỹ năng GitHub.
Khôi phục cũng di chuyển các trường `ownerUserId` cũ cho kỹ năng của nhà xuất bản được khôi phục,
bí danh slug kỹ năng, gói, cảnh báo trình kiểm tra gói, và các hàng bản tóm tắt tìm kiếm dẫn xuất để
các đường dẫn chủ sở hữu trực tiếp khớp với quyền hạn nhà xuất bản mới. Một đặt trước handle được bảo vệ
đang hoạt động cho handle được khôi phục cũng được gán lại cho người dùng thay thế để lần đồng bộ hồ sơ sau này
không thể khôi phục quyền hạn cạnh tranh của người dùng cũ. Mỗi bảng chính bị giới hạn ở
100 hàng cho mỗi giao dịch áp dụng; các lần khôi phục lớn hơn trước tiên phải dùng một lần di chuyển chủ sở hữu có thể tiếp tục.
...
Thêm/xóa một dấu sao (nổi bật). Cả hai điểm cuối đều có tính lũy đẳng.
...
`POST /api/cli/upload-url` trả về `uploadUrl` và `uploadTicket`. Các lần xuất bản gói
