...
summary: Tham chiếu API HTTP (công khai + các điểm cuối CLI + xác thực).
x-i18n:
    generated_at: "2026-07-01T18:13:41Z"
    model: gpt-5.5
...
Các thư mục bên thứ ba có thể dùng các endpoint đọc công khai để liệt kê hoặc tìm kiếm ClawHub skills. Vui lòng lưu kết quả vào bộ nhớ đệm, tuân thủ `429`/`Retry-After`, liên kết người dùng trở lại mục ClawHub chính tắc (`https://clawhub.ai/<owner>/skills/<slug>`), và tránh ngụ ý rằng ClawHub chứng thực trang web bên thứ ba. Không cố gắng sao chép nội dung bị ẩn, riêng tư, hoặc bị chặn do kiểm duyệt ra ngoài bề mặt API công khai.

Các lối tắt web slug được phân giải trên nhiều họ registry, nhưng API client nên dùng
...
- Các endpoint ghi đã xác thực không nên trả về `Unauthorized` trần trụi khi
  ...
  tài khoản bị xóa/bị cấm/bị vô hiệu hóa nên mỗi trường hợp đều nhận được văn bản có thể hành động để CLI
...
- `X-RateLimit-Remaining` / `RateLimit-Remaining`: ngân sách còn lại chính xác khi có mặt.
  Các yêu cầu thành công được phân shard sẽ bỏ qua header này thay vì trả về một giá trị toàn cục xấp xỉ.
...
bỏ qua để tương thích, nhưng tham số truy vấn được nhận diện mà có giá trị không hợp lệ sẽ trả về
`400`.
...
- `highlightedOnly` (không bắt buộc): `true` để lọc chỉ còn skills được tô nổi bật
- `nonSuspiciousOnly` (không bắt buộc): `true` để ẩn skills đáng ngờ (`flagged.suspicious`)
- `nonSuspicious` (không bắt buộc): alias cũ cho `nonSuspiciousOnly`
...
- Mức độ liên quan mạnh hơn độ phổ biến. Một slug chính xác hoặc token tên hiển thị khớp có thể xếp trên một kết quả khớp lỏng hơn nhưng có tương tác mạnh hơn nhiều.
...
- Độ phổ biến được chia tỷ lệ log và giới hạn trần. Skills có tương tác cao có thể xếp thấp hơn khi văn bản truy vấn khớp yếu hơn.
...
- Đặt các thuật ngữ mà người dùng sẽ tìm kiếm đúng nguyên văn vào tên hiển thị, tóm tắt, và tag. Chỉ dùng token slug độc lập khi đó cũng là một danh tính ổn định mà bạn muốn giữ.
- Không đổi tên slug chỉ để chạy theo một truy vấn, trừ khi slug mới là tên chính tắc dài hạn tốt hơn. Slug cũ trở thành alias chuyển hướng, nhưng URL chính tắc, slug được hiển thị, và bản tóm tắt tìm kiếm trong tương lai dùng slug mới.
- Alias đổi tên giữ khả năng phân giải cho URL cũ và các lượt cài đặt phân giải qua registry, nhưng xếp hạng tìm kiếm dựa trên metadata skill chính tắc sau khi việc đổi tên đã được lập chỉ mục. Thống kê hiện có vẫn đi cùng skill.
...
- `cursor` (không bắt buộc): con trỏ phân trang cho bất kỳ kiểu sắp xếp nào không phải `trending`
- `sort` (không bắt buộc): `updated` (mặc định), `recommended` (alias: `default`), `createdAt` (alias: `newest`), `downloads`, `stars` (alias: `rating`), các alias cài đặt cũ `installsCurrent`/`installs`/`installsAllTime` ánh xạ tới `downloads`, `trending`
- `nonSuspiciousOnly` (không bắt buộc): `true` để ẩn skills đáng ngờ (`flagged.suspicious`)
- `nonSuspicious` (không bắt buộc): alias cũ cho `nonSuspiciousOnly`
...
- `createdAt` ổn định cho quá trình crawl skill mới; `updated` thay đổi khi skill hiện có được phát hành lại.
- Khi `nonSuspiciousOnly=true`, các kiểu sắp xếp dựa trên cursor có thể trả về ít mục hơn `limit` trên một trang vì skills đáng ngờ được lọc sau khi truy xuất trang.
...
- Slug cũ được tạo bởi các luồng đổi tên/gộp owner phân giải tới skill chính tắc.
...
- `tag` (không bắt buộc): phân giải một phiên bản được gắn tag (ví dụ `latest`).
...
- `moderation` là ảnh chụp điều tiết hiện tại ở cấp Skill, được suy ra từ phiên bản mới nhất.
...
- Tác vụ quét là bất đồng bộ. Các yêu cầu quét thủ công được ưu tiên trước công việc xuất bản/điền ngược thông thường, nhưng việc hoàn tất vẫn phụ thuộc vào mức độ sẵn sàng của worker.

### `GET /api/v1/skills/-/scan/{scanId}`

Điểm cuối thăm dò đã xác thực cho một lượt quét đã gửi.

- Trả về trạng thái đã xếp hàng/đang chạy/thành công/thất bại.
- Trả về `queue.queuedAhead` và `queue.position` khi đang xếp hàng để client có thể hiển thị số lượt quét thủ công được ưu tiên đang đứng trước yêu cầu. Các hàng đợi rất lớn được giới hạn và báo cáo với `queuedAheadIsEstimate: true`.
...
Điểm cuối lưu trữ báo cáo đã xác thực.
...
Tuyến quét lại hàng loạt chuẩn chỉ dành cho quản trị viên. Nó chấp nhận cùng dạng payload như `POST /api/v1/skills/-/rescan-batch` kế thừa.

### `POST /api/v1/skills/-/scan/batch/status`

Tuyến trạng thái hàng loạt chuẩn chỉ dành cho quản trị viên. Nó chấp nhận `{ "jobIds": ["..."] }` và trả về cùng các bộ đếm tổng hợp như `POST /api/v1/skills/-/rescan-batch/status` kế thừa.
...
- `ok` chỉ là `true` khi phiên bản được chọn có Skill Card đã tạo, không bị điều tiết chặn vì phần mềm độc hại, và xác minh ClawScan sạch.
...
- `provenance` chỉ là `server-resolved-github-import` khi ClawHub đã phân giải và lưu repo/ref/commit/path GitHub trong khi xuất bản hoặc nhập; nếu không thì là `unavailable`.

### `POST /api/v1/skills/-/security-verdicts`

Trả về các kết luận bảo mật rút gọn hiện tại cho đúng phiên bản Skill. Điểm cuối
bộ sưu tập này dành cho các client đã biết cần hiển thị những phiên bản
Skill ClawHub đã cài đặt nào, chẳng hạn như OpenClaw Control UI.
...
- `category` (tùy chọn): bộ lọc danh mục Plugin. Chỉ được hỗ trợ khi
  ...
  bí danh bộ lọc v1 kế thừa được ghi trong tài liệu tại `GET /api/v1/plugins`.
...
- Các mục Skills vẫn được hỗ trợ bởi registry Skills và vẫn chỉ có thể được xuất bản qua `POST /api/v1/skills`.
...
Các bí danh bộ lọc v1 kế thừa vẫn được chấp nhận trên các điểm cuối đọc:
...
Trên điểm cuối hợp nhất `/api/v1/packages`, mục này chỉ dành cho Plugin; dùng
...
- Skills được lưu trữ bao gồm các tệp của phiên bản đã lưu mới nhất và được liệt kê trong
...
- Skills hiện tại dựa trên GitHub có bản quét `clean` hoặc `suspicious` bao gồm
  ...
  content hash, và URL kho lưu trữ. Chúng không bao gồm các tệp nguồn được lưu trữ trên ClawHub.
...
- Điểm cuối đọc công khai. Không cần mã thông báo của chủ sở hữu, nhà xuất bản, điều phối viên, hoặc quản trị viên.
...
- `trust.moderationState` có thể null. Nó là `null` khi không có điều phối bản phát hành thủ công.
...
- Đây là bề mặt trình phân giải OpenClaw; nó tránh việc đoán định dạng kho lưu trữ từ
  một URL dùng chung.
...
Điểm cuối dành cho người điều hành/quản trị viên để tiếp nhận báo cáo gói.

Xác thực:

- Yêu cầu token API của người dùng là người điều hành hoặc quản trị viên.
...
Điểm cuối dành cho chủ sở hữu/người điều hành để xem trạng thái kiểm duyệt gói.

Xác thực:

- Yêu cầu token API của chủ sở hữu gói, thành viên nhà phát hành, người điều hành, hoặc
...
Điểm cuối dành cho người điều hành/quản trị viên để giải quyết hoặc mở lại báo cáo gói.
...
`finalAction: "revoke"` cùng với báo cáo đã xác nhận để áp dụng kiểm duyệt bản phát hành trong cùng
...
Điểm cuối dành cho người điều hành/quản trị viên để rà soát bản phát hành gói.
...
- Kho lưu trữ Plugin/gói là tệp zip có thư mục gốc `package/` để các máy khách OpenClaw
  cũ tiếp tục hoạt động.
- Tuyến này chỉ duy trì ZIP. Nó không truyền trực tuyến tệp ClawPack `.tgz`.
...
- Các phiên bản chỉ có ZIP kiểu cũ được cố ý bỏ qua.
...
Truyền trực tuyến chính xác các byte tarball ClawPack đã tải lên cho máy khách mirror npm.
...
kỹ năng hiện tại dựa trên GitHub có lần quét `clean` hoặc `suspicious` và không có phiên bản
được lưu trữ.
...
- Bàn giao kỹ năng dựa trên GitHub không proxy hoặc mirror byte. Phản hồi JSON
  ...
  và `archiveUrl`; trạng thái quét/hiện tại là cổng kiểm tra và không được bao gồm làm siêu dữ liệu
...
- Trường tải trọng không bắt buộc: `ownerHandle`. Khi có, API phân giải
  nhà phát hành đó ở phía máy chủ và yêu cầu tác nhân có quyền truy cập nhà phát hành.
- Trường tải trọng không bắt buộc: `migrateOwner`. Khi là `true` cùng với `ownerHandle`, một
  kỹ năng hiện có có thể chuyển sang chủ sở hữu đó nếu tác nhân là quản trị viên/chủ sở hữu trên cả
  nhà phát hành hiện tại và nhà phát hành đích. Nếu không có lựa chọn tham gia này, các thay đổi
...
- Các trường biểu mẫu được phép là `payload`, blob `files` lặp lại, hoặc một tham chiếu tarball `clawpack`.
  `clawpack` có thể là blob `.tgz` hoặc id lưu trữ do luồng upload-url trả về.
  Các lần phát hành storage-id đã chuẩn bị cũng phải bao gồm
...
- Trường tải trọng không bắt buộc: `ownerHandle`. Khi có, chỉ quản trị viên mới có thể phát hành thay mặt chủ sở hữu đó.

Điểm nổi bật về xác thực:

- `family` phải là `code-plugin` hoặc `bundle-plugin`.
- Gói Plugin yêu cầu `openclaw.plugin.json`. Nội dung tải lên ClawPack `.tgz` phải
...
- Các lần phát hành thay mặt vẫn xác thực điều kiện đủ của kênh chính thức dựa trên tài khoản chủ sở hữu đích.

### `DELETE /api/v1/skills/{slug}` / `POST /api/v1/skills/{slug}/undelete`

Xóa mềm / khôi phục kỹ năng (chủ sở hữu, người điều hành, hoặc quản trị viên).
...
Các lần xóa mềm do chủ sở hữu khởi tạo sẽ giữ trước slug trong 30 ngày, sau đó slug có thể được
nhà phát hành khác nhận. Phản hồi xóa bao gồm `slugReservedUntil` khi thời hạn này được áp dụng.
Ẩn bởi người điều hành/quản trị viên và gỡ bỏ vì lý do bảo mật không hết hạn theo cách này.
...
Chỉ dành cho quản trị viên. Đảm bảo tồn tại một nhà phát hành tổ chức cho một handle. Nếu handle vẫn trỏ tới
người dùng/nhà phát hành cá nhân dùng chung kiểu cũ, điểm cuối sẽ di chuyển nó sang nhà phát hành tổ chức trước.
Với một tổ chức mới tạo, hãy cung cấp `memberHandle`; quản trị viên đang thực hiện không được thêm làm thành viên.
...
không đánh dấu nhà phát hành là tin cậy/chính thức.
...
Chỉ dành cho quản trị viên. Giữ trước root slug và tên gói cho chủ sở hữu hợp pháp mà không phát hành
bản phát hành. Tên gói trở thành gói giữ chỗ riêng tư không có hàng bản phát hành, để cùng
chủ sở hữu có thể phát hành bản phát hành code-plugin hoặc bundle-plugin thật vào tên đó về sau.
...
Chỉ dành cho quản trị viên. Khôi phục một nhà phát hành cá nhân cho principal GitHub OAuth thay thế đã xác minh
mà không chỉnh sửa các hàng tài khoản Convex Auth. Yêu cầu phải nêu cả hai id tài khoản nhà cung cấp GitHub
bất biến; các handle có thể thay đổi chỉ được dùng làm điều kiện bảo vệ hướng tới người vận hành.
...
định danh GitHub. Quá trình khôi phục sẽ từ chối an toàn khi nhà xuất bản cá nhân hiện tại
của người dùng đích có skills, packages hoặc nguồn GitHub skill.
Quá trình khôi phục cũng di chuyển các trường `ownerUserId` cũ cho skills của nhà xuất bản được khôi phục,
bí danh slug của skill, packages, cảnh báo trình kiểm tra package và các hàng bản tóm tắt tìm kiếm dẫn xuất để
các đường dẫn chủ sở hữu trực tiếp khớp với quyền nhà xuất bản mới. Một đặt chỗ protected-handle đang hoạt động
cho handle được khôi phục cũng được gán lại cho người dùng thay thế để lần đồng bộ hóa hồ sơ sau đó
không thể khôi phục quyền cạnh tranh của người dùng trước. Mỗi bảng chính được giới hạn ở
...
Các nguồn GitHub skill thuộc phạm vi nhà xuất bản và được báo cáo là đã kiểm tra thay vì được viết lại.
...
## Điểm cuối CLI cũ (không dùng nữa)
...
`POST /api/cli/upload-url` trả về `uploadUrl` và `uploadTicket`. Các lần xuất bản package
tạo tạm một tarball ClawPack phải gửi id lưu trữ kết quả dưới dạng
...
Nếu tự lưu trữ, hãy phục vụ tệp này (hoặc đặt rõ `CLAWHUB_REGISTRY`; `CLAWDHUB_REGISTRY` cũ).
