...
Các thư mục bên thứ ba có thể dùng các điểm cuối đọc công khai để liệt kê hoặc tìm kiếm ClawHub skills. Vui lòng lưu kết quả vào bộ nhớ đệm, tuân thủ `429`/`Retry-After`, liên kết người dùng trở lại mục ClawHub chính thức (`https://clawhub.ai/<owner>/<slug>`), và tránh ngụ ý rằng ClawHub xác nhận trang bên thứ ba. Không cố sao chép nội dung bị ẩn, riêng tư, hoặc bị chặn kiểm duyệt ra ngoài bề mặt API công khai.

Các lối tắt slug web phân giải trên nhiều họ registry, nhưng client API nên dùng
các URL chính thức được điểm cuối đọc trả về thay vì tự dựng lại thứ tự ưu tiên
định tuyến.
...
- Các điểm cuối ghi đã xác thực không nên trả về `Unauthorized` đơn thuần khi
...
- Nếu không có IP client đáng tin cậy, yêu cầu tải xuống ẩn danh dùng bucket dự phòng theo phạm vi điểm cuối thay vì một bucket `ip:unknown` toàn cục. Yêu cầu đọc/ghi ẩn danh vẫn dùng bucket unknown dùng chung để định tuyến thiếu IP vẫn hiển thị và thận trọng.
...
- Kết quả được trả về theo thứ tự liên quan (độ tương đồng embedding + tăng điểm token slug/tên khớp chính xác + ưu tiên độ phổ biến từ lượt tải xuống).
- Độ liên quan mạnh hơn độ phổ biến. Một slug chính xác hoặc token tên hiển thị khớp có thể xếp hạng cao hơn một kết quả khớp lỏng hơn nhưng có nhiều lượt tải xuống hơn.
...
- Lượt tải xuống được dùng như một ưu tiên nhỏ theo thang log và để phá thế hòa, không phải tín hiệu xếp hạng chính. Skills có nhiều lượt tải có thể xếp thấp hơn khi văn bản truy vấn khớp yếu hơn.
- Trạng thái kiểm duyệt đáng ngờ hoặc bị ẩn có thể loại một skill khỏi tìm kiếm công khai tùy theo bộ lọc của người gọi và trạng thái kiểm duyệt hiện tại.

Hướng dẫn khả năng được khám phá cho nhà xuất bản:

- Đặt các thuật ngữ người dùng sẽ tìm kiếm đúng nghĩa vào tên hiển thị, tóm tắt, và thẻ. Chỉ dùng token slug độc lập khi đó cũng là một định danh ổn định bạn muốn giữ.
- Không đổi tên slug chỉ để theo đuổi một truy vấn, trừ khi slug mới là tên chính thức dài hạn tốt hơn. Slug cũ trở thành bí danh chuyển hướng, nhưng URL chính thức, slug hiển thị, và các digest tìm kiếm tương lai dùng slug mới.
- Bí danh đổi tên giữ khả năng phân giải cho URL cũ và lượt cài đặt phân giải qua registry, nhưng xếp hạng tìm kiếm dựa trên metadata skill chính thức sau khi việc đổi tên đã được lập chỉ mục. Thống kê hiện có vẫn đi cùng skill.
- Nếu một skill bất ngờ không hiển thị, hãy kiểm tra trạng thái kiểm duyệt trước bằng `clawhub inspect <slug>` khi đã đăng nhập trước khi thay đổi metadata liên quan đến xếp hạng.
...
- `createdAt` ổn định cho các lượt thu thập skill mới; `updated` thay đổi khi skills hiện có được xuất bản lại.
...
- Slug cũ được tạo bởi các luồng đổi tên/hợp nhất chủ sở hữu phân giải về skill chính thức.
- `metadata.os`: giới hạn OS được khai báo trong frontmatter của skill (ví dụ `["macos"]`, `["linux"]`). `null` nếu không khai báo.
...
- Bằng chứng được biên tập lại cho người gọi công khai và chỉ bao gồm đoạn trích thô cho chủ sở hữu/người kiểm duyệt.
...
Điểm cuối người kiểm duyệt/quản trị viên để tiếp nhận báo cáo skill.
...
Điểm cuối người kiểm duyệt/quản trị viên để giải quyết hoặc mở lại báo cáo skill.
...
- `moderation` là ảnh chụp nhanh kiểm duyệt cấp skill hiện tại được suy ra từ phiên bản mới nhất.
...
- `externalService`, `binary`, `osPermission` (tùy chọn): dạng viết tắt cho các thẻ yêu cầu
  môi trường có tên
...
- Yêu cầu API token cho chủ sở hữu gói, chủ sở hữu/quản trị viên của nhà xuất bản thuộc tổ chức,
...
- Phiên bản gói legacy trả về artifact `legacy-zip` và
  `downloadUrl` ZIP legacy.
- Phiên bản ClawPack trả về artifact `npm-pack`, các trường integrity của npm, một
  `tarballUrl`, và URL tương thích ZIP legacy.
- Đây là bề mặt trình phân giải OpenClaw; nó tránh việc suy đoán định dạng kho lưu trữ từ
  một URL dùng chung.

### `GET /api/v1/packages/{name}/versions/{version}/artifact/download`

Tải xuống artifact của phiên bản thông qua đường dẫn trình phân giải rõ ràng.

Ghi chú:

- Phiên bản ClawPack truyền trực tiếp đúng các byte `.tgz` npm-pack đã tải lên.
- Phiên bản ZIP legacy chuyển hướng đến `/api/v1/packages/{name}/download?version=`.
...
- `open`: các bản phát hành đáng ngờ, độc hại, đang chờ xử lý, bị cách ly, bị thu hồi, hoặc đã bị báo cáo.
...
Endpoint điều phối viên/quản trị viên cho đầu vào báo cáo gói.
...
`note` là bắt buộc đối với `confirmed` và `dismissed`; có thể bỏ qua trường này khi
...
Endpoint dành cho người kiểm duyệt/quản trị viên để đánh giá bản phát hành gói.
...
Endpoint bảo trì chỉ dành cho quản trị viên để gắn nhãn các bản phát hành gói cũ hơn bằng
...
- Các hàng hiện có dùng ClawPack nhưng thiếu `artifactKind` được sửa thành
  `npm-pack`.
...
- Giới hạn kích thước tệp: 200KB.
- Các lần quét VirusTotal đang chờ không chặn lượt đọc; các bản phát hành độc hại vẫn có thể bị giữ lại ở nơi khác.
- Gói riêng tư trả về `404` trừ khi bên gọi có thể đọc nhà xuất bản sở hữu.
...
- Các kho lưu trữ Plugin/gói là tệp zip với gốc `package/` để các client OpenClaw
...
- Siêu dữ liệu chỉ có trong registry không được chèn vào kho lưu trữ được tải xuống.
...
Trả về packument tương thích npm cho các phiên bản gói dùng ClawPack.

Ghi chú:

- Chỉ liệt kê các phiên bản có tarball npm-pack ClawPack đã tải lên.
- Các phiên bản kế thừa chỉ có ZIP được cố ý bỏ qua.
- `dist.tarball`, `dist.integrity` và `dist.shasum` sử dụng các trường
  tương thích npm để người dùng có thể trỏ npm đến mirror nếu họ chọn.
...
Phát trực tuyến đúng các byte tarball ClawPack đã tải lên cho các client npm mirror.
...
- Thống kê tải xuống được tính là các danh tính duy nhất mỗi giờ (`userId` khi token API hợp lệ, nếu không thì dùng IP).
...
- Trường payload tùy chọn: `migrateOwner`. Khi là `true` cùng với `ownerHandle`, một
  ...
  nhà xuất bản hiện tại và nhà xuất bản đích. Nếu không chọn tham gia, các thay đổi chủ sở hữu sẽ
  bị từ chối.
...
- Chỉ các nhà xuất bản được tin cậy mới có thể xuất bản lên kênh `official`.
- Các lần xuất bản thay mặt vẫn xác thực điều kiện đủ để dùng kênh official dựa trên tài khoản chủ sở hữu đích.
...
nhà xuất bản khác nhận. Phản hồi xóa bao gồm `slugReservedUntil` khi thời điểm hết hạn này áp dụng.
Các lần ẩn của người kiểm duyệt/quản trị viên và các lần gỡ bỏ bảo mật không hết hạn theo cách này.
...
nhà xuất bản người dùng/cá nhân dùng chung kế thừa, endpoint sẽ di chuyển nó sang nhà xuất bản tổ chức trước.
...
- Cả hai endpoint đều yêu cầu xác thực token API và chỉ hoạt động cho chủ sở hữu skill.
...
Cấm một người dùng và xóa cứng các skill thuộc sở hữu (chỉ người kiểm duyệt/quản trị viên).
...
Bỏ cấm một người dùng và khôi phục các skill đủ điều kiện (chỉ quản trị viên).
...
## Endpoint CLI kế thừa (đã ngừng khuyến nghị)
...
Nếu bạn tự lưu trữ, hãy phục vụ tệp này (hoặc đặt `CLAWHUB_REGISTRY` rõ ràng; kế thừa `CLAWDHUB_REGISTRY`).
