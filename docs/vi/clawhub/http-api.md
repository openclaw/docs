...
summary: Tài liệu tham chiếu API HTTP (công khai + điểm cuối CLI + xác thực).
x-i18n:
    generated_at: "2026-07-04T03:52:22Z"
    model: gpt-5.5
...
Các thư mục bên thứ ba có thể dùng các endpoint đọc công khai để liệt kê hoặc tìm kiếm Skills trên ClawHub. Vui lòng lưu kết quả vào bộ nhớ đệm, tuân thủ `429`/`Retry-After`, liên kết người dùng trở lại danh sách ClawHub chuẩn (`https://clawhub.ai/<owner>/skills/<slug>`), và tránh ngụ ý rằng ClawHub chứng thực trang bên thứ ba. Đừng cố phản chiếu nội dung bị ẩn, riêng tư, hoặc bị chặn do kiểm duyệt ra ngoài bề mặt API công khai.
...
- Các endpoint ghi đã xác thực không nên trả về một `Unauthorized` trần khi
  ...
  tài khoản bị xóa/cấm/vô hiệu hóa đều nên nhận văn bản có thể hành động để client
...
- Dùng header IP client tin cậy, bao gồm `cf-connecting-ip`, chỉ khi
  triển khai bật rõ ràng header chuyển tiếp tin cậy.
- ClawHub dùng header chuyển tiếp tin cậy để xác định IP client ở biên.
- Nếu không có IP client tin cậy, yêu cầu ẩn danh dùng các bucket dự phòng
  chỉ được giới hạn theo loại giới hạn tốc độ. Các bucket dự phòng này không bao gồm
  đường dẫn, slug, tên gói, phiên bản, chuỗi truy vấn, hoặc tham số artifact khác
  do bên gọi cung cấp.
...
nên đọc body phản hồi như chuỗi cho người đọc. Các tham số truy vấn không xác định được
...
- Kết quả được trả về theo thứ tự liên quan (độ tương đồng embedding + tăng hạng token slug/tên khớp chính xác + một prior phổ biến nhỏ).
- Độ liên quan mạnh hơn độ phổ biến. Một slug chính xác hoặc token tên hiển thị khớp có thể xếp trên một kết quả khớp lỏng hơn nhưng có mức tương tác mạnh hơn nhiều.
...
- Độ phổ biến được scale theo log và có giới hạn trần. Skills có mức tương tác cao có thể xếp hạng thấp hơn khi văn bản truy vấn khớp yếu hơn.
- Trạng thái kiểm duyệt đáng ngờ hoặc bị ẩn có thể loại bỏ một Skill khỏi tìm kiếm công khai tùy thuộc vào bộ lọc của bên gọi và trạng thái kiểm duyệt hiện tại.

Hướng dẫn khả năng được nhà phát hành tìm thấy:

- Đặt các thuật ngữ mà người dùng sẽ tìm kiếm đúng nguyên văn vào tên hiển thị, tóm tắt, và thẻ. Chỉ dùng token slug độc lập khi nó cũng là một danh tính ổn định mà bạn muốn giữ.
- Đừng đổi tên slug chỉ để đuổi theo một truy vấn trừ khi slug mới là tên chuẩn dài hạn tốt hơn. Slug cũ trở thành bí danh chuyển hướng, nhưng URL chuẩn, slug được hiển thị, và bản tóm lược tìm kiếm trong tương lai dùng slug mới.
...
- Nếu một Skill bất ngờ không hiển thị, hãy kiểm tra trạng thái kiểm duyệt trước bằng `clawhub inspect @owner/slug` khi đã đăng nhập trước khi thay đổi metadata liên quan đến xếp hạng.
...
- `sort` (tùy chọn): `updated` (mặc định), `recommended` (bí danh: `default`), `createdAt` (bí danh: `newest`), `downloads`, `stars` (bí danh: `rating`), bí danh cài đặt cũ `installsCurrent`/`installs`/`installsAllTime` ánh xạ tới `downloads`, `trending`
- `nonSuspiciousOnly` (tùy chọn): `true` để ẩn Skills đáng ngờ (`flagged.suspicious`)
- `nonSuspicious` (tùy chọn): bí danh cũ cho `nonSuspiciousOnly`
...
- `createdAt` ổn định cho crawl Skill mới; `updated` thay đổi khi Skills hiện có được xuất bản lại.
- Khi `nonSuspiciousOnly=true`, các kiểu sắp xếp dựa trên con trỏ có thể trả về ít hơn `limit` mục trên một trang vì Skills đáng ngờ được lọc sau khi lấy trang.
...
- Slug cũ được tạo bởi luồng đổi tên/hợp nhất owner phân giải tới Skill chuẩn.
- `metadata.os`: hạn chế OS được khai báo trong frontmatter của Skill (ví dụ `["macos"]`, `["linux"]`). `null` nếu không được khai báo.
...
tới một phiên bản, và đưa vào hàng đợi báo cáo Skill.
...
- `moderation` là ảnh chụp moderation cấp kỹ năng hiện tại, được suy ra từ phiên bản mới nhất.
...
- Quét đã xuất bản yêu cầu quyền truy cập quản lý của chủ sở hữu/nhà xuất bản, hoặc quyền moderator/admin của nền tảng.
...
- Tác vụ quét là bất đồng bộ. Các yêu cầu quét thủ công được ưu tiên trước công việc xuất bản/backfill thông thường, nhưng thời điểm hoàn tất vẫn phụ thuộc vào khả năng sẵn sàng của worker.
...
- Trả về `queue.queuedAhead` và `queue.position` trong khi đang xếp hàng để client có thể hiển thị số lượt quét thủ công được ưu tiên đang đứng trước yêu cầu. Các hàng đợi rất lớn được giới hạn và báo cáo bằng `queuedAheadIsEstimate: true`.
...
- Yêu cầu một lượt quét đã thành công; các lượt quét chưa ở trạng thái kết thúc trả về `409`.
...
- Yêu cầu quyền truy cập quản lý của chủ sở hữu/nhà xuất bản đối với kỹ năng hoặc plugin, hoặc quyền moderator/admin của nền tảng.
...
- Trả về cùng dạng ZIP như các lượt tải xuống từ yêu cầu quét.

### `POST /api/v1/skills/-/scan/batch`

Tuyến quét lại hàng loạt chuẩn chỉ dành cho admin. Tuyến này chấp nhận cùng dạng tải trọng như `POST /api/v1/skills/-/rescan-batch` cũ.

### `POST /api/v1/skills/-/scan/batch/status`

Tuyến trạng thái hàng loạt chuẩn chỉ dành cho admin. Tuyến này chấp nhận `{ "jobIds": ["..."] }` và trả về cùng các bộ đếm tổng hợp như `POST /api/v1/skills/-/rescan-batch/status` cũ.

### `GET /api/v1/skills/{slug}/verify`

Trả về phong bì xác minh thẻ kỹ năng được `clawhub skill verify` dùng.
...
- `ok` chỉ là `true` khi phiên bản đã chọn có thẻ kỹ năng được tạo, không bị moderation chặn do mã độc, và xác minh ClawScan là sạch.
- Danh tính kỹ năng, danh tính nhà xuất bản và metadata của phiên bản đã chọn là các trường cấp cao nhất trong phong bì (`slug`, `displayName`, `publisherHandle`, `version`, `resolvedFrom`, `tag`, `createdAt`) để tự động hóa shell có thể đọc chúng mà không cần mở các wrapper lồng nhau.
...
- `security.signals.dependencyRegistry` được giữ lại để tương thích phản hồi v1, nhưng trình quét sự tồn tại của dependency registry đã ngừng hoạt động và khóa này luôn là `null`.
- `provenance` chỉ là `server-resolved-github-import` khi ClawHub đã phân giải và lưu repo/ref/commit/path GitHub trong lúc xuất bản hoặc nhập; nếu không, giá trị là `unavailable`.

### `POST /api/v1/skills/-/security-verdicts`

Trả về các kết luận bảo mật cô đọng hiện tại cho đúng các phiên bản kỹ năng. Điểm cuối
...
- Kết quả được trả theo từng mục; một kỹ năng hoặc phiên bản bị thiếu không làm hỏng toàn bộ phản hồi.
- Phản hồi chỉ liên quan đến bảo mật. Nó không bao gồm dữ liệu thẻ kỹ năng, trạng thái thẻ đã tạo, danh sách tệp artifact hoặc tải trọng chi tiết của trình quét.
- `security.signals` chỉ chứa bằng chứng hỗ trợ ở cấp trạng thái; dùng `/scan` hoặc trang security-audit của ClawHub để xem đầy đủ chi tiết trình quét.
- `security.signals.dependencyRegistry` được giữ lại để tương thích phản hồi v1, nhưng trình quét sự tồn tại của dependency registry đã ngừng hoạt động và khóa này luôn là `null`.
- Việc thiếu thẻ kỹ năng không ảnh hưởng đến `ok`, `decision` hoặc `reasons` của điểm cuối này; client nên đọc `skill-card.md` đã cài đặt cục bộ khi cần nội dung thẻ.
- Dùng `/verify` khi bạn cần phong bì xác minh thẻ kỹ năng cho một kỹ năng, `/card` khi bạn cần markdown thẻ đã tạo, và `/scan` khi bạn cần dữ liệu chi tiết của trình quét.
...
- `GET /api/v1/code-plugins` và `GET /api/v1/bundle-plugins` vẫn là các bí danh fixed-family.
...
- Người gọi đã xác thực có thể thấy các gói riêng tư của các nhà phát hành mà họ thuộc về trong kết quả danh sách/tìm kiếm.
...
- Người gọi ẩn danh chỉ thấy các kênh gói công khai.
- Người gọi đã xác thực có thể tìm kiếm các gói riêng tư của các nhà phát hành mà họ thuộc về.
- `channel=private` chỉ trả về các gói mà người gọi đã xác thực có thể đọc.
...
Các bí danh bộ lọc v1 cũ vẫn được chấp nhận trên endpoint đọc:
...
- `startDate` (bắt buộc): cận dưới tính bằng mili giây Unix cho `updatedAt` của Skills.
- `endDate` (bắt buộc): cận trên tính bằng mili giây Unix cho `updatedAt` của Skills.
...
- Skills được lưu trữ bao gồm các tệp phiên bản mới nhất đã lưu và được liệt kê trong
  `_manifest.json` với `sourceRef: "public-clawhub"`.
- Skills hiện tại được hỗ trợ bởi GitHub với lượt quét `clean` hoặc `suspicious` bao gồm
...
- `_errors.json` được bao gồm khi từng Skills hoặc tệp riêng lẻ không thể được
  xuất.
...
- `startDate` (bắt buộc): cận dưới Unix milliseconds cho `updatedAt` của Plugin.
- `endDate` (bắt buộc): cận trên Unix milliseconds cho `updatedAt` của Plugin.
- `limit` (tùy chọn): số nguyên (1-250), mặc định `250`.
- `cursor` (tùy chọn): con trỏ phân trang từ phản hồi trước đó.
...
- Các bí danh bộ lọc v1 cũ được ghi tài liệu trong `GET /api/v1/plugins` cũng
  được chấp nhận.
- Lọc theo danh mục là một bộ lọc API thực sự, dựa trên các hàng tóm tắt danh mục
  Plugin, không phải viết lại truy vấn tìm kiếm.
...
- Yêu cầu API token cho chủ sở hữu gói, chủ sở hữu/quản trị viên publisher tổ chức,
...
- Bản phát hành ClawPack bao gồm các trường tương thích npm `npmIntegrity`, `npmShasum`, và
  `npmTarballName`.
- `version.sha256hash` là siêu dữ liệu tương thích không còn được khuyến nghị cho client cũ. Nó
  băm đúng các byte ZIP được trả về bởi `/api/v1/packages/{name}/download`.
  Client hiện đại nên dùng `version.artifact.sha256`, trường này định danh
...
Trả về tóm tắt bảo mật và độ tin cậy chính xác của bản phát hành gói cho các
client cài đặt. Đây là bề mặt tiêu thụ công khai của OpenClaw để quyết định liệu một
bản phát hành đã phân giải có thể được cài đặt hay không.

Xác thực:

- Endpoint đọc công khai. Không yêu cầu token của chủ sở hữu, publisher, điều phối viên hoặc quản trị viên.
...
- `release.artifactKind`, `release.artifactSha256`, `release.npmIntegrity`,
  ...
  artifact phát hành.
- `trust.scanStatus` là trạng thái tin cậy hiệu lực được dẫn xuất từ đầu vào trình quét
  và kiểm duyệt thủ công bản phát hành.
- `trust.moderationState` có thể là null. Nó là `null` khi không có kiểm duyệt thủ công
  bản phát hành.
...
- `trust.stale` nghĩa là tóm tắt tin cậy được tính từ đầu vào đã lỗi thời và
...
- Gói riêng tư trả về `404` trừ khi bên gọi có thể đọc publisher sở hữu.
- Endpoint này cố ý hẹp hơn các endpoint kiểm duyệt của chủ sở hữu/điều phối viên.
  Nó phơi bày quyết định cài đặt và giải thích công khai, không phải
  danh tính người báo cáo, nội dung báo cáo, bằng chứng riêng tư, hoặc dòng thời gian rà soát nội bộ.

### `GET /api/v1/packages/{name}/versions/{version}/artifact`

Trả về siêu dữ liệu resolver artifact rõ ràng cho một phiên bản gói.

Ghi chú:

- Phiên bản gói cũ trả về artifact `legacy-zip` và `downloadUrl` ZIP cũ.
- Phiên bản ClawPack trả về artifact `npm-pack`, các trường toàn vẹn npm, một
  `tarballUrl`, và URL tương thích ZIP cũ.
- Đây là bề mặt resolver của OpenClaw; nó tránh đoán định dạng kho lưu trữ từ
  một URL dùng chung.

### `GET /api/v1/packages/{name}/versions/{version}/artifact/download`

Tải xuống artifact phiên bản qua đường dẫn resolver rõ ràng.

Ghi chú:

- Phiên bản ClawPack stream đúng các byte `.tgz` npm-pack đã tải lên.
- Phiên bản ZIP cũ chuyển hướng đến `/api/v1/packages/{name}/download?version=`.
- Sử dụng bucket giới hạn tốc độ tải xuống.

### `GET /api/v1/packages/{name}/readiness`

Trả về mức sẵn sàng đã tính cho việc tiêu thụ OpenClaw trong tương lai.
...
- Yêu cầu API token cho người dùng quản trị viên.
...
- `packageName` được chuẩn hóa theo tên npm; gói có thể thiếu đối với các di trú đã lên kế hoạch.
- Phần này chỉ theo dõi mức sẵn sàng di trú. Nó không thay đổi OpenClaw hoặc tạo
  ClawPack.
...
- `open`: bản phát hành đáng ngờ, độc hại, đang chờ, bị cách ly, bị thu hồi, hoặc đã bị báo cáo.
- `blocked`: bản phát hành bị cách ly, bị thu hồi, hoặc độc hại.
...
liên kết với một phiên bản. Chúng đưa vào hàng đợi kiểm duyệt nhưng không tự động ẩn hoặc
...
- Yêu cầu API token của chủ sở hữu gói, thành viên nhà xuất bản, điều phối viên, hoặc
...
Điểm cuối dành cho điều phối viên/quản trị viên để xử lý hoặc mở lại báo cáo gói.
...
`finalAction: "revoke"` cùng với một báo cáo đã xác nhận để áp dụng điều phối bản phát hành trong
...
Điểm cuối dành cho điều phối viên/quản trị viên để đánh giá bản phát hành gói.
...
- `approved`: đã được đánh giá thủ công và được cho phép.
- `quarantined`: bị chặn trong khi chờ theo dõi thêm.
...
- Kho lưu trữ Plugin/gói là tệp zip có gốc `package/` để các máy khách OpenClaw
...
- Phản hồi bao gồm các header `ETag`, `Digest`, `X-ClawHub-Artifact-Type`, và
  `X-ClawHub-Artifact-Sha256` cho các kiểm tra tính toàn vẹn của bộ phân giải.
...
- Chỉ các phiên bản có tarball npm-pack ClawPack đã tải lên mới được liệt kê.
- Các phiên bản chỉ có ZIP kế thừa được cố ý bỏ qua.
...
- Packument của gói có scope hỗ trợ cả `/api/npm/@scope/name` và đường dẫn yêu cầu
  được mã hóa của npm là `/api/npm/@scope%2Fname`.

### `GET /api/npm/{package}/-/{tarball}.tgz`

Stream chính xác các byte tarball ClawPack đã tải lên cho máy khách mirror npm.
...
Tải xuống ZIP của phiên bản skill được lưu trữ, hoặc trả về chuyển giao nguồn GitHub cho một
skill hiện tại dựa trên GitHub có lượt quét `clean` hoặc `suspicious` và không có
...
- Chuyển giao skill dựa trên GitHub không proxy hoặc mirror byte. Phản hồi JSON
  bao gồm `sourceRef: "public-github"`, `repo`, `commit`, `path`, `contentHash`,
  và `archiveUrl`; trạng thái quét/hiện tại là một cổng kiểm soát và không được đưa vào làm siêu dữ liệu
  payload thành công.
- Thống kê tải xuống được tính là các danh tính duy nhất theo mỗi ngày UTC (`userId` khi API token hợp lệ, nếu không thì IP).
...
- Trường payload không bắt buộc: `migrateOwner`. Khi là `true` cùng với `ownerHandle`, một
  skill hiện có có thể chuyển sang chủ sở hữu đó nếu tác nhân là quản trị viên/chủ sở hữu trên cả
  nhà xuất bản hiện tại và nhà xuất bản đích. Nếu không bật tùy chọn này, các thay đổi chủ sở hữu sẽ
  bị từ chối.
...
- Các trường form được phép là `payload`, các blob `files` lặp lại, hoặc một tham chiếu tarball `clawpack`.
  ...
  luồng upload-url trả về. Các lượt xuất bản storage-id đã chuẩn bị cũng phải bao gồm
  `clawpackUploadTicket` được trả về cùng với URL tải lên đó.
...
- Body JSON và siêu dữ liệu `payload.files` / `payload.artifact` do bên gọi cung cấp
  bị từ chối.
- Các yêu cầu xuất bản multipart trực tiếp bị giới hạn ở 18MB. Tarball ClawPack có thể
  dùng luồng upload-url đến mức trần tarball 120MB.
- Trường payload không bắt buộc: `ownerHandle`. Khi có mặt, chỉ quản trị viên mới có thể xuất bản thay mặt chủ sở hữu đó.
...
- Code plugin yêu cầu `package.json`, siêu dữ liệu repo nguồn, siêu dữ liệu commit nguồn,
  siêu dữ liệu lược đồ cấu hình, `openclaw.compat.pluginApi`, và
  `openclaw.build.openclawVersion`.
- `openclaw.hostTargets` và `openclaw.environment` là siêu dữ liệu không bắt buộc.
- Chỉ nhà xuất bản tổ chức `openclaw` và các nhà xuất bản cá nhân của thành viên tổ chức `openclaw`
  hiện tại mới có thể xuất bản lên kênh `official`.
- Các lượt xuất bản thay mặt vẫn xác thực tư cách đủ điều kiện của kênh chính thức dựa trên tài khoản chủ sở hữu đích.
...
Khi có mặt, `reason` được lưu làm ghi chú điều phối skill và được sao chép vào nhật ký kiểm toán.
Các lượt xóa mềm do chủ sở hữu khởi tạo giữ trước slug trong 30 ngày, sau đó slug có thể được
một nhà xuất bản khác yêu cầu. Phản hồi xóa bao gồm `slugReservedUntil` khi thời hạn này áp dụng.
Các lượt ẩn của điều phối viên/quản trị viên và gỡ bỏ vì bảo mật không hết hạn theo cách này.
...
Chỉ dành cho quản trị viên. Đảm bảo một nhà xuất bản tổ chức tồn tại cho một handle. Nếu handle vẫn trỏ đến một
nhà xuất bản người dùng/cá nhân dùng chung kế thừa, điểm cuối sẽ di chuyển nó sang một nhà xuất bản tổ chức trước.
...
đánh dấu nhà xuất bản là đáng tin cậy/chính thức.
...
Chỉ dành cho quản trị viên. Giữ trước root slug và tên gói cho chủ sở hữu hợp pháp mà không xuất bản
bản phát hành. Tên gói trở thành các gói placeholder riêng tư không có hàng bản phát hành, để cùng
chủ sở hữu đó sau này có thể xuất bản bản phát hành code-plugin hoặc bundle-plugin thật vào tên đó.
...
Chỉ dành cho quản trị viên. Khôi phục một nhà xuất bản cá nhân cho một principal GitHub OAuth thay thế đã xác minh
mà không chỉnh sửa các hàng tài khoản Convex Auth. Yêu cầu phải nêu cả hai id tài khoản nhà cung cấp GitHub
bất biến; các handle có thể thay đổi chỉ được dùng như một cơ chế bảo vệ hướng đến người vận hành.
...
chủ thể GitHub. Khôi phục sẽ từ chối an toàn khi nhà phát hành cá nhân hiện tại của người dùng đích
có kỹ năng, gói hoặc nguồn kỹ năng GitHub.
Khôi phục cũng di chuyển các trường `ownerUserId` cũ cho kỹ năng của nhà phát hành được khôi phục,
bí danh slug kỹ năng, gói, cảnh báo của trình kiểm tra gói và các hàng bản tóm tắt tìm kiếm phái sinh để
các đường dẫn chủ sở hữu trực tiếp khớp với thẩm quyền nhà phát hành mới. Một đặt chỗ protected-handle
đang hoạt động cho handle được khôi phục cũng được gán lại cho người dùng thay thế để quá trình
đồng bộ hóa hồ sơ sau này không thể khôi phục thẩm quyền cạnh tranh của người dùng trước đó. Mỗi bảng chính được giới hạn ở
100 hàng cho mỗi giao dịch áp dụng; các lần khôi phục lớn hơn trước tiên phải dùng một quy trình di chuyển chủ sở hữu có thể tiếp tục.
Nguồn kỹ năng GitHub có phạm vi theo nhà phát hành và được báo cáo là đã kiểm tra thay vì được viết lại.
...
Cấm một người dùng và xóa vĩnh viễn các kỹ năng thuộc sở hữu của họ (chỉ moderator/admin).
...
Thêm/xóa một sao (điểm nổi bật). Cả hai endpoint đều có tính idempotent.
...
## Các endpoint CLI cũ (không còn được khuyến nghị)
...
`POST /api/cli/upload-url` trả về `uploadUrl` và `uploadTicket`. Các lần phát hành gói
đưa một tarball ClawPack vào giai đoạn chuẩn bị phải gửi mã định danh lưu trữ kết quả dưới dạng
...
Nếu tự host, hãy phục vụ tệp này (hoặc đặt `CLAWHUB_REGISTRY` một cách rõ ràng; `CLAWDHUB_REGISTRY` cũ).
