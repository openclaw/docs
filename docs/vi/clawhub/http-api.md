...
summary: Tham chiếu HTTP API (công khai + endpoint CLI + xác thực).
x-i18n:
...
Các thư mục bên thứ ba có thể dùng các endpoint đọc công khai để liệt kê hoặc tìm kiếm ClawHub skills. Vui lòng lưu cache kết quả, tuân thủ `429`/`Retry-After`, liên kết người dùng trở lại danh sách ClawHub chuẩn (`https://clawhub.ai/<owner>/skills/<slug>`), và tránh ngụ ý rằng ClawHub chứng thực trang bên thứ ba. Không cố gắng sao chép nội dung bị ẩn, riêng tư, hoặc bị chặn kiểm duyệt ra ngoài bề mặt API công khai.
...
- Các endpoint ghi đã xác thực không nên trả về chỉ `Unauthorized` khi
  ...
  tài khoản đã xóa/bị cấm/bị vô hiệu hóa nên đều nhận được nội dung có thể hành động để CLI
...
- `X-RateLimit-Remaining` / `RateLimit-Remaining`: ngân sách còn lại chính xác khi có mặt.
  Các yêu cầu thành công đã phân mảnh bỏ qua header này thay vì trả về giá trị toàn cục xấp xỉ.
...
- Chỉ dùng các header IP client tin cậy, bao gồm `cf-connecting-ip`, khi
...
nên đọc phần thân phản hồi như một chuỗi có thể đọc được bởi con người. Các tham số truy vấn không xác định được
bỏ qua để tương thích, nhưng tham số truy vấn được nhận diện với giá trị không hợp lệ sẽ trả về
`400`.
...
- Kết quả được trả về theo thứ tự mức độ liên quan (độ tương đồng embedding + tăng điểm token slug/tên khớp chính xác + một mức ưu tiên phổ biến nhỏ).
- Mức độ liên quan mạnh hơn độ phổ biến. Một slug chính xác hoặc token tên hiển thị khớp có thể xếp hạng cao hơn một kết quả khớp lỏng hơn nhưng có mức tương tác mạnh hơn nhiều.
...
- Độ phổ biến được co giãn theo log và có giới hạn trần. Các skill có mức tương tác cao có thể xếp hạng thấp hơn khi văn bản truy vấn là kết quả khớp yếu hơn.
- Trạng thái kiểm duyệt đáng ngờ hoặc bị ẩn có thể loại bỏ một skill khỏi tìm kiếm công khai tùy vào bộ lọc của caller và trạng thái kiểm duyệt hiện tại.

Hướng dẫn khả năng được nhà xuất bản tìm thấy:

- Đặt các thuật ngữ mà người dùng sẽ thực sự tìm kiếm vào tên hiển thị, tóm tắt, và tag. Chỉ dùng token slug độc lập khi đó cũng là một định danh ổn định mà bạn muốn giữ.
- Không đổi tên slug chỉ để theo đuổi một truy vấn trừ khi slug mới là tên chuẩn dài hạn tốt hơn. Slug cũ trở thành alias chuyển hướng, nhưng URL chuẩn, slug hiển thị, và các bản tóm tắt tìm kiếm trong tương lai dùng slug mới.
- Alias đổi tên giữ khả năng phân giải cho URL cũ và các bản cài đặt phân giải qua registry, nhưng xếp hạng tìm kiếm dựa trên metadata skill chuẩn sau khi đổi tên đã được lập chỉ mục. Số liệu thống kê hiện có vẫn ở cùng skill.
...
- `sort` (tùy chọn): `updated` (mặc định), `recommended` (alias: `default`), `createdAt` (alias: `newest`), `downloads`, `stars` (alias: `rating`), alias cài đặt cũ `installsCurrent`/`installs`/`installsAllTime` ánh xạ tới `downloads`, `trending`
- `nonSuspiciousOnly` (tùy chọn): `true` để ẩn các skill đáng ngờ (`flagged.suspicious`)
- `nonSuspicious` (tùy chọn): alias cũ cho `nonSuspiciousOnly`
...
- `createdAt` ổn định cho các lần crawl skill mới; `updated` thay đổi khi skill hiện có được xuất bản lại.
...
- Slug cũ được tạo bởi các luồng đổi tên/hợp nhất owner sẽ phân giải tới skill chuẩn.
...
- Owner và người kiểm duyệt có thể truy cập chi tiết kiểm duyệt cho các skill bị ẩn.
- Caller công khai chỉ nhận `200` cho các skill hiển thị đã được gắn cờ.
- Bằng chứng được biên tập lại cho caller công khai và chỉ bao gồm đoạn trích thô cho owner/người kiểm duyệt.

### `POST /api/v1/skills/{slug}/report`

Báo cáo một skill để người kiểm duyệt xem xét. Báo cáo ở cấp skill, tùy chọn liên kết
...
Endpoint cho người kiểm duyệt/quản trị viên để tiếp nhận báo cáo skill.
...
Endpoint cho người kiểm duyệt/quản trị viên để giải quyết hoặc mở lại báo cáo skill.
...
- Nếu không cung cấp cả `version` lẫn `tag`, sẽ dùng phiên bản mới nhất.
- Bao gồm trạng thái xác minh đã chuẩn hóa cùng các chi tiết riêng theo trình quét.
- `security.hasScanResult` chỉ là `true` khi một trình quét đưa ra phán quyết dứt khoát (`clean`, `suspicious`, hoặc `malicious`).
- `moderation` là ảnh chụp nhanh kiểm duyệt ở cấp kỹ năng hiện tại được dẫn xuất từ phiên bản mới nhất.
...
- Các lượt quét đã xuất bản yêu cầu quyền truy cập quản lý của chủ sở hữu/nhà xuất bản, hoặc quyền hạn điều phối viên/quản trị viên nền tảng.
- Các lượt quét đã xuất bản chỉ ghi ngược khi `update: true` và lượt quét hoàn tất thành công.
...
- Các công việc quét là bất đồng bộ. Yêu cầu quét thủ công được ưu tiên trước công việc xuất bản/điền bổ sung thông thường, nhưng việc hoàn tất vẫn phụ thuộc vào khả năng sẵn sàng của worker.
...
- Trả về `queue.queuedAhead` và `queue.position` khi đang xếp hàng để client có thể hiển thị có bao nhiêu lượt quét thủ công được ưu tiên đang ở trước yêu cầu. Các hàng đợi rất lớn được giới hạn và báo cáo với `queuedAheadIsEstimate: true`.
...
- Yêu cầu quyền truy cập quản lý của chủ sở hữu/nhà xuất bản đối với kỹ năng hoặc plugin, hoặc quyền hạn điều phối viên/quản trị viên nền tảng.
...
- Danh tính kỹ năng, danh tính nhà xuất bản, và siêu dữ liệu phiên bản đã chọn là các trường phong bì cấp cao nhất (`slug`, `displayName`, `publisherHandle`, `version`, `resolvedFrom`, `tag`, `createdAt`) để tự động hóa shell có thể đọc chúng mà không cần giải nén các wrapper lồng nhau.
...
Trả về các phán quyết bảo mật rút gọn hiện tại cho đúng các phiên bản kỹ năng. Điểm cuối bộ sưu tập này dành cho các client đã biết những phiên bản kỹ năng ClawHub đã cài đặt nào cần hiển thị, chẳng hạn như OpenClaw Control UI.
...
- Kết quả theo từng mục; một kỹ năng hoặc phiên bản bị thiếu không làm hỏng toàn bộ phản hồi.
...
- Người gọi đã xác thực có thể thấy các gói riêng tư của publisher mà họ thuộc về trong kết quả danh sách/tìm kiếm.
...
- Người gọi ẩn danh chỉ thấy các kênh gói công khai.
- Người gọi đã xác thực có thể tìm kiếm các gói riêng tư của publisher mà họ thuộc về.
- `channel=private` chỉ trả về các gói mà người gọi đã xác thực có thể đọc.
...
Trên endpoint hợp nhất `/api/v1/packages`, tùy chọn này chỉ dành cho plugin; dùng
...
- `startDate` (bắt buộc): cận dưới Unix mili giây cho `updatedAt` của Skill.
- `endDate` (bắt buộc): cận trên Unix mili giây cho `updatedAt` của Skill.
...
- Các Skills hiện tại được GitHub hỗ trợ có kết quả quét `clean` hoặc `suspicious` bao gồm
  `_source_handoff.json` với `sourceRef: "public-github"`, repo, commit, path,
  content hash, và archive URL. Chúng không bao gồm các tệp nguồn được ClawHub lưu trữ.
...
- Các bí danh bộ lọc v1 cũ được ghi tài liệu trong `GET /api/v1/plugins` cũng được
  chấp nhận.
- Lọc theo danh mục là bộ lọc API thật được hỗ trợ bởi các hàng digest danh mục
...
- Yêu cầu mã thông báo API của chủ sở hữu gói, chủ sở hữu/quản trị viên publisher tổ chức,
...
- Bản phát hành ClawPack bao gồm các trường tương thích npm `npmIntegrity`, `npmShasum`, và
  `npmTarballName`.
- `version.sha256hash` là siêu dữ liệu tương thích đã lỗi thời cho client cũ. Nó
  băm đúng các byte ZIP được trả về bởi `/api/v1/packages/{name}/download`.
  Client hiện đại nên dùng `version.artifact.sha256`, trường xác định
...
- Điểm cuối đọc công khai. Không yêu cầu mã thông báo của chủ sở hữu, publisher, điều phối viên, hoặc quản trị viên.
...
- `trust.moderationState` có thể là null. Nó là `null` khi không có kiểm duyệt bản phát hành thủ công.
- `trust.blockedFromDownload` là tín hiệu chặn cài đặt. OpenClaw và các
  ...
  suy diễn lại quy tắc chặn từ các trường trình quét hoặc kiểm duyệt.
...
- Điểm cuối này chính xác theo phiên bản. Client nên gọi nó sau khi phân giải
  phiên bản gói dự định cài đặt, không chỉ sau khi đọc siêu dữ liệu gói mới nhất.
- Gói riêng tư trả về `404` trừ khi bên gọi có thể đọc publisher sở hữu.
...
- Phiên bản ClawPack truyền đúng các byte `.tgz` npm-pack đã tải lên.
...
Kiểm tra mức sẵn sàng bao gồm:

- trạng thái kênh chính thức
- tính sẵn có của phiên bản mới nhất
- tính sẵn có của artifact ClawPack npm-pack
- digest artifact
...
- `packageName` được chuẩn hóa theo tên npm; gói có thể thiếu đối với các lần di chuyển
  đã lên kế hoạch.
- Trường này chỉ theo dõi mức sẵn sàng di chuyển. Nó không thay đổi OpenClaw hoặc tạo
  ClawPack.
...
liên kết với một phiên bản. Chúng đưa vào hàng đợi kiểm duyệt nhưng tự chúng không tự động ẩn hoặc
...
- Yêu cầu API token của chủ sở hữu gói, thành viên nhà phát hành, moderator, hoặc
  người dùng admin.
...
- Mặc định là bản phát hành mới nhất.
- Skills chuyển hướng tới `GET /api/v1/download`.
- Kho lưu trữ Plugin/gói là các tệp zip có gốc `package/` để các client OpenClaw
  cũ tiếp tục hoạt động.
- Tuyến này chỉ duy trì ZIP. Nó không truyền trực tuyến các tệp ClawPack `.tgz`.
...
- Các phiên bản cũ chỉ có ZIP được chủ động bỏ qua.
- `dist.tarball`, `dist.integrity`, và `dist.shasum` sử dụng các
  trường tương thích với npm để người dùng có thể trỏ npm tới mirror nếu họ chọn.
...
Truyền trực tuyến đúng các byte tarball ClawPack đã tải lên cho client mirror npm.
...
- `slug` (bắt buộc)
- `hash` (bắt buộc): sha256 hex 64 ký tự của fingerprint gói bundle
...
- Bàn giao skill dựa trên GitHub không proxy hoặc mirror byte. Phản hồi JSON
  ...
  và `archiveUrl`; trạng thái quét/hiện tại là một cổng kiểm soát và không được bao gồm dưới dạng
...
- Trường payload tùy chọn: `migrateOwner`. Khi là `true` cùng với `ownerHandle`, một
  skill hiện có có thể chuyển sang owner đó nếu actor là admin/owner trên cả
  publisher hiện tại và publisher đích. Nếu không bật tùy chọn này, các thay đổi owner sẽ
  bị từ chối.
...
- Các trường form được phép là `payload`, các blob `files` lặp lại, hoặc một tham chiếu tarball `clawpack`.
  ...
  luồng upload-url. Các lượt xuất bản bằng storage-id đã staging cũng phải bao gồm
...
- Body JSON và siêu dữ liệu `payload.files` / `payload.artifact`
  do caller cung cấp sẽ bị từ chối.
...
- Trường payload tùy chọn: `ownerHandle`. Khi có, chỉ admin mới có thể xuất bản thay mặt owner đó.
...
- Các lượt xuất bản thay mặt vẫn xác thực điều kiện đủ của kênh official theo tài khoản owner đích.
...
Khi có, `reason` được lưu dưới dạng ghi chú kiểm duyệt skill và được sao chép vào audit log.
Các lượt xóa mềm do owner khởi tạo giữ slug trong 30 ngày, sau đó slug có thể được publisher
khác nhận. Phản hồi xóa bao gồm `slugReservedUntil` khi thời hạn này áp dụng.
Các lượt ẩn của moderator/admin và gỡ bỏ vì lý do bảo mật không hết hạn theo cách này.
...
Chỉ dành cho admin. Đảm bảo tồn tại publisher org cho một handle. Nếu handle vẫn trỏ tới một
publisher người dùng/cá nhân dùng chung kiểu cũ, endpoint sẽ migrate nó thành publisher org trước.
Với org mới tạo, cung cấp `memberHandle`; admin thực hiện không được thêm làm thành viên.
...
caller làm owner. Endpoint này không migrate các handle người dùng/cá nhân hiện có và
...
Chỉ dành cho admin. Giữ trước các slug gốc và tên gói cho owner hợp pháp mà không xuất bản một
bản phát hành. Tên gói trở thành các gói placeholder riêng tư không có hàng release, để cùng
owner đó sau này có thể xuất bản bản phát hành code-plugin hoặc bundle-plugin thật vào tên đó.
...
Chỉ dành cho admin. Khôi phục một publisher cá nhân cho principal GitHub OAuth thay thế đã xác minh
mà không chỉnh sửa các hàng tài khoản Convex Auth. Yêu cầu phải nêu cả hai id tài khoản provider GitHub
bất biến; các handle có thể thay đổi chỉ được dùng làm biện pháp bảo vệ hướng tới operator.
...
GitHub principal. Khôi phục sẽ đóng an toàn khi publisher cá nhân hiện tại của người dùng đích
...
bí danh slug skill, packages, cảnh báo của trình kiểm tra package, và các hàng digest tìm kiếm phát sinh để
...
100 hàng cho mỗi giao dịch áp dụng; các khôi phục lớn hơn trước tiên phải dùng một migration chủ sở hữu có thể tiếp tục.
...
Thêm/xóa một star (điểm nổi bật). Cả hai điểm cuối đều có tính lũy đẳng.
...
đưa ClawPack tarball vào giai đoạn chuẩn bị phải gửi storage id thu được dưới dạng
