...
x-i18n:
    generated_at: "2026-07-02T14:05:19Z"
    model: gpt-5.5
...
Các thư mục của bên thứ ba có thể dùng các điểm cuối đọc công khai để liệt kê hoặc tìm kiếm Skills ClawHub. Vui lòng lưu kết quả vào bộ nhớ đệm, tuân thủ `429`/`Retry-After`, liên kết người dùng trở lại mục niêm yết ClawHub chính tắc (`https://clawhub.ai/<owner>/skills/<slug>`), và tránh ngụ ý rằng ClawHub chứng thực trang của bên thứ ba. Không cố gắng sao chép nội dung bị ẩn, riêng tư hoặc bị chặn bởi kiểm duyệt ra ngoài bề mặt API công khai.

Các lối tắt slug trên web phân giải qua nhiều họ registry, nhưng client API nên dùng
các URL chính tắc do điểm cuối đọc trả về thay vì tự dựng lại thứ tự ưu tiên
của route.
...
- Các điểm cuối ghi đã xác thực không nên trả về `Unauthorized` trần khi
  máy chủ biết lý do. Token bị thiếu, token không hợp lệ/bị thu hồi, và
  tài khoản bị xóa/cấm/vô hiệu hóa mỗi trường hợp nên nhận được văn bản có thể hành động để
...
- Tải xuống: 1200/phút mỗi IP, 6000/phút mỗi key (các điểm cuối tải xuống)
...
- `X-RateLimit-Remaining` / `RateLimit-Remaining`: ngân sách còn lại chính xác khi có mặt.
  Các yêu cầu thành công được sharding sẽ bỏ qua header này thay vì trả về một giá trị toàn cục xấp xỉ.
...
- Nếu không có IP client đáng tin cậy nào, các yêu cầu ẩn danh dùng bucket dự phòng
...
## Điểm cuối công khai (không cần xác thực)
...
- Độ liên quan mạnh hơn độ phổ biến. Một token slug hoặc tên hiển thị khớp chính xác có thể xếp trên một kết quả khớp lỏng hơn nhưng có mức tương tác cao hơn nhiều.
- Văn bản ASCII được tách token theo ranh giới từ và dấu câu. Ví dụ, `personal-map` chứa token độc lập `map`, trong khi `amap-jsapi-skill` chứa `amap`, `jsapi`, và `skill`; vì vậy tìm kiếm `map` sẽ cho `personal-map` mức khớp từ vựng mạnh hơn `amap-jsapi-skill`.
- Độ phổ biến được scale log và có giới hạn trần. Skills có mức tương tác cao có thể xếp hạng thấp hơn khi văn bản truy vấn khớp yếu hơn.
...
- Alias đổi tên giữ khả năng phân giải cho URL cũ và các lượt cài đặt phân giải qua registry, nhưng thứ hạng tìm kiếm dựa trên metadata Skill chính tắc sau khi việc đổi tên đã được lập chỉ mục. Thống kê hiện có vẫn đi cùng Skill.
...
- `createdAt` ổn định cho các crawl Skill mới; `updated` thay đổi khi Skills hiện có được xuất bản lại.
...
- Slug cũ được tạo bởi các luồng đổi tên/gộp owner sẽ phân giải sang Skill chính tắc.
...
Báo cáo một Skill để moderator xem xét. Báo cáo ở cấp Skill, có thể tùy chọn liên kết
...
Điểm cuối moderator/admin để tiếp nhận báo cáo Skill.
...
Điểm cuối moderator/admin để giải quyết hoặc mở lại báo cáo Skill.
...
đặt `status` trở lại `open`. Truyền `finalAction: "hide"` cùng một báo cáo đã triage
...
- Bao gồm trạng thái xác minh đã chuẩn hóa cùng với các chi tiết riêng của từng trình quét.
...
- Quét đã phát hành yêu cầu quyền truy cập quản lý của chủ sở hữu/nhà phát hành, hoặc quyền quản trị viên/kiểm duyệt viên nền tảng.
...
- Tác vụ quét là bất đồng bộ. Yêu cầu quét thủ công được ưu tiên trước công việc phát hành/backfill thông thường, nhưng việc hoàn tất vẫn phụ thuộc vào mức độ sẵn có của worker.
...
- Trả về `queue.queuedAhead` và `queue.position` khi đang xếp hàng để client có thể hiển thị số lượng lượt quét thủ công được ưu tiên đang đứng trước yêu cầu. Hàng đợi rất lớn được giới hạn và báo cáo với `queuedAheadIsEstimate: true`.
...
- Yêu cầu quyền truy cập quản lý của chủ sở hữu/nhà phát hành đối với skill hoặc plugin, hoặc quyền quản trị viên/kiểm duyệt viên nền tảng.
...
- Trả về cùng định dạng ZIP như các lượt tải xuống yêu cầu quét.

### `POST /api/v1/skills/-/scan/batch`

Tuyến quét lại hàng loạt chuẩn chỉ dành cho quản trị viên. Nó chấp nhận cùng định dạng payload như `POST /api/v1/skills/-/rescan-batch` cũ.
...
- Danh tính skill, danh tính nhà phát hành, và siêu dữ liệu phiên bản được chọn là các trường cấp cao nhất của phong bì (`slug`, `displayName`, `publisherHandle`, `version`, `resolvedFrom`, `tag`, `createdAt`) để tự động hóa shell có thể đọc chúng mà không cần mở gói các wrapper lồng nhau.
...
- `security.signals.dependencyRegistry` được giữ lại để tương thích phản hồi v1, nhưng trình quét sự tồn tại của registry phụ thuộc đã bị ngừng sử dụng và khóa này luôn là `null`.
...
bộ sưu tập này dành cho các client đã biết những phiên bản skill
...
- `GET /api/v1/code-plugins` và `GET /api/v1/bundle-plugins` vẫn là bí danh cố định family.
- Mục kỹ năng vẫn được hỗ trợ bởi sổ đăng ký kỹ năng và vẫn chỉ có thể được xuất bản qua `POST /api/v1/skills`.
...
- `category` (tùy chọn): bộ lọc danh mục plugin. Chỉ được hỗ trợ khi
  ...
  bộ lọc v1 cũ được ghi tài liệu trong `GET /api/v1/plugins`.
...
Bí danh bộ lọc v1 cũ vẫn được chấp nhận trên các endpoint đọc:
...
`trending` là bảng xếp hạng lượt cài đặt/tải xuống trong bảy ngày và không dùng tổng số toàn thời gian.
...
- Kỹ năng hiện tại dựa trên GitHub có bản quét `clean` hoặc `suspicious` bao gồm
  ...
  băm nội dung và URL lưu trữ. Chúng không bao gồm tệp nguồn do ClawHub lưu trữ.
- Mỗi kỹ năng bao gồm `_export_skill_meta.json`.
- `_manifest.json` luôn được bao gồm tại gốc ZIP.
- `_errors.json` được bao gồm khi từng kỹ năng hoặc tệp không thể được
  xuất.
...
- Các bí danh bộ lọc v1 cũ được ghi tài liệu dưới `GET /api/v1/plugins` cũng
  được chấp nhận.
- Lọc theo danh mục là một bộ lọc API thật được hỗ trợ bởi các hàng digest danh mục
...
- `release.artifactKind`, `release.artifactSha256`, `release.npmIntegrity`,
  `release.npmShasum`, và `release.npmTarballName` hiện diện khi biết được cho
  artifact bản phát hành.
- `trust.scanStatus` là trạng thái tin cậy hiệu lực được suy ra từ đầu vào bộ quét
...
- `trust.blockedFromDownload` là tín hiệu chặn cài đặt. OpenClaw và các
  ...
  tự suy lại quy tắc chặn từ các trường bộ quét hoặc kiểm duyệt.
...
- Điểm cuối này chính xác theo phiên bản. Máy khách nên gọi nó sau khi phân giải
  ...
  mới nhất.
- Gói riêng tư trả về `404` trừ khi bên gọi có thể đọc nhà phát hành sở hữu.
- Điểm cuối này được chủ ý làm hẹp hơn các điểm cuối kiểm duyệt của chủ sở hữu/điều phối viên.
...
- Đây là bề mặt trình phân giải của OpenClaw; nó tránh suy đoán định dạng kho lưu trữ từ
  một URL dùng chung.
...
Điểm cuối điều phối viên/quản trị viên cho hàng đợi xem xét bản phát hành gói.
...
Báo cáo một gói để điều phối viên xem xét. Báo cáo ở cấp gói, có thể tùy chọn
...
- Kho lưu trữ Plugin/gói là tệp zip có thư mục gốc `package/` để các client OpenClaw cũ
  tiếp tục hoạt động.
- Tuyến này chỉ giữ ZIP. Nó không truyền trực tuyến tệp ClawPack `.tgz`.
...
- `dist.tarball`, `dist.integrity`, và `dist.shasum` dùng các trường tương thích npm
  để người dùng có thể trỏ npm đến bản sao nếu họ chọn.
- Packument của gói có scope hỗ trợ cả `/api/npm/@scope/name` và đường dẫn yêu cầu
  đã mã hóa `/api/npm/@scope%2Fname` của npm.

### `GET /api/npm/{package}/-/{tarball}.tgz`

Truyền trực tuyến đúng các byte tarball ClawPack đã tải lên cho client bản sao npm.
...
- Bàn giao kỹ năng dựa trên GitHub không proxy hoặc sao chép byte. Phản hồi JSON
  bao gồm `sourceRef: "public-github"`, `repo`, `commit`, `path`, `contentHash`,
  và `archiveUrl`; trạng thái quét/hiện tại là một cổng kiểm tra và không được đưa vào làm
...
- Trường payload không bắt buộc: `migrateOwner`. Khi là `true` cùng với `ownerHandle`, một
  kỹ năng hiện có có thể chuyển sang chủ sở hữu đó nếu tác nhân là admin/chủ sở hữu trên cả
...
- Các trường biểu mẫu được phép là `payload`, các blob `files` lặp lại, hoặc một tham chiếu tarball `clawpack`.
  ...
  luồng upload-url. Các lượt phát hành bằng storage-id đã chuẩn bị cũng phải bao gồm
...
- Trường payload không bắt buộc: `ownerHandle`. Khi có mặt, chỉ admin mới có thể phát hành thay mặt chủ sở hữu đó.
...
- Các lượt phát hành thay mặt vẫn xác thực điều kiện đủ của kênh official theo tài khoản chủ sở hữu đích.
...
Khi có mặt, `reason` được lưu làm ghi chú kiểm duyệt kỹ năng và được sao chép vào nhật ký kiểm toán.
...
Chỉ dành cho admin. Đảm bảo nhà phát hành tổ chức tồn tại cho một handle. Nếu handle vẫn trỏ đến một
người dùng/nhà phát hành cá nhân dùng chung kế thừa, điểm cuối sẽ di chuyển nó thành nhà phát hành tổ chức trước.
...
chủ sở hữu có thể phát hành bản phát hành code-plugin hoặc bundle-plugin thật vào tên đó sau.
...
Chỉ dành cho admin. Khôi phục nhà phát hành cá nhân cho một principal GitHub OAuth thay thế đã xác minh
mà không chỉnh sửa các hàng tài khoản Convex Auth. Yêu cầu phải nêu cả hai id tài khoản
nhà cung cấp GitHub bất biến; handle có thể thay đổi chỉ được dùng làm chốt bảo vệ hướng đến người vận hành.
...
GitHub principal. Khôi phục sẽ thất bại đóng khi publisher cá nhân hiện tại của người dùng đích
có skills, packages, hoặc GitHub skill sources.
Khôi phục cũng di chuyển các trường `ownerUserId` cũ cho skills của publisher được khôi phục,
bí danh slug của skill, packages, cảnh báo của trình kiểm tra package, và các hàng search digest phát sinh để
các đường dẫn chủ sở hữu trực tiếp khớp với thẩm quyền publisher mới. Một đặt trước protected-handle đang hoạt động
cho handle được khôi phục cũng được gán lại cho người dùng thay thế để lần đồng bộ hóa hồ sơ sau
...
GitHub skill sources có phạm vi theo publisher và được báo cáo là đã kiểm tra thay vì được ghi lại.
...
tạo giai đoạn cho một tarball ClawPack phải gửi id lưu trữ thu được dưới dạng
`clawpack` và ticket trả về dưới dạng `clawpackUploadTicket`.
