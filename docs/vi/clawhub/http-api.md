...
Các thư mục bên thứ ba có thể dùng các endpoint đọc công khai để liệt kê hoặc tìm kiếm Skills trên ClawHub. Vui lòng lưu kết quả vào bộ nhớ đệm, tuân thủ `429`/`Retry-After`, liên kết người dùng trở lại mục ClawHub chính tắc (`https://clawhub.ai/<owner>/skills/<slug>`), và tránh ngụ ý rằng ClawHub chứng thực trang bên thứ ba. Không cố gắng sao chép nội dung ẩn, riêng tư, hoặc bị chặn kiểm duyệt ra ngoài bề mặt API công khai.

Lối tắt slug web phân giải trên các họ registry, nhưng client API nên dùng
các URL chính tắc được endpoint đọc trả về thay vì dựng lại độ ưu tiên
tuyến.
...
- Endpoint ghi đã xác thực không nên trả về `Unauthorized` trần khi
...
- `X-RateLimit-Remaining` / `RateLimit-Remaining`: ngân sách chính xác còn lại khi có mặt.
...
- Nếu không có IP client tin cậy, yêu cầu ẩn danh dùng bucket dự phòng
...
nên đọc thân phản hồi dưới dạng chuỗi người đọc được. Tham số truy vấn không xác định được
...
- Kết quả được trả về theo thứ tự liên quan (độ tương đồng embedding + tăng điểm token slug/tên khớp chính xác + một yếu tố phổ biến nhỏ).
- Độ liên quan mạnh hơn mức độ phổ biến. Một slug chính xác hoặc token tên hiển thị khớp có thể xếp trên kết quả khớp lỏng hơn nhưng có tương tác mạnh hơn nhiều.
- Văn bản ASCII được tách token theo ranh giới từ và dấu câu. Ví dụ, `personal-map` chứa token `map` độc lập, còn `amap-jsapi-skill` chứa `amap`, `jsapi`, và `skill`; do đó tìm kiếm `map` cho `personal-map` mức khớp từ vựng mạnh hơn `amap-jsapi-skill`.
- Mức độ phổ biến được scale log và có giới hạn trần. Skills có tương tác cao có thể xếp thấp hơn khi văn bản truy vấn khớp yếu hơn.
...
- Đặt các thuật ngữ người dùng sẽ tìm kiếm đúng nghĩa vào tên hiển thị, tóm tắt, và thẻ. Chỉ dùng token slug độc lập khi đó cũng là danh tính ổn định bạn muốn giữ.
- Không đổi tên slug chỉ để chạy theo một truy vấn, trừ khi slug mới là tên chính tắc dài hạn tốt hơn. Slug cũ trở thành bí danh chuyển hướng, nhưng URL chính tắc, slug hiển thị, và bản tóm tắt tìm kiếm tương lai dùng slug mới.
- Bí danh đổi tên giữ phân giải cho URL cũ và lượt cài đặt phân giải qua registry, nhưng xếp hạng tìm kiếm dựa trên metadata Skill chính tắc sau khi việc đổi tên đã được lập chỉ mục. Thống kê hiện có vẫn ở lại với Skill.
...
- Khi `nonSuspiciousOnly=true`, các kiểu sắp xếp dựa trên con trỏ có thể trả về ít mục hơn `limit` trên một trang vì Skills đáng ngờ được lọc sau khi lấy trang.
...
- Slug cũ được tạo bởi luồng đổi tên/hợp nhất owner phân giải về Skill chính tắc.
- `metadata.os`: giới hạn OS được khai báo trong frontmatter Skill (ví dụ `["macos"]`, `["linux"]`). `null` nếu không được khai báo.
...
- `moderation` chỉ được đưa vào khi Skill bị gắn cờ hoặc owner đang xem nó.
...
để ẩn Skill trong cùng workflow có thể kiểm tra được.
...
Quét tệp tải lên cục bộ không còn được hỗ trợ. Các yêu cầu dùng
...
- Quét bản đã phát hành yêu cầu quyền truy cập quản lý của chủ sở hữu/nhà phát hành, hoặc quyền hạn của điều phối viên/quản trị viên nền tảng.
...
- Tác vụ quét là bất đồng bộ. Các yêu cầu quét thủ công được ưu tiên trước công việc phát hành/điền bổ sung thông thường, nhưng việc hoàn tất vẫn phụ thuộc vào mức độ sẵn sàng của worker.
...
- Trả về `queue.queuedAhead` và `queue.position` khi đang xếp hàng để client có thể hiển thị số lượt quét thủ công được ưu tiên đang ở trước yêu cầu. Các hàng đợi rất lớn được giới hạn và báo cáo với `queuedAheadIsEstimate: true`.
...
- Yêu cầu quyền truy cập quản lý của chủ sở hữu/nhà phát hành đối với kỹ năng hoặc plugin, hoặc quyền hạn của điều phối viên/quản trị viên nền tảng.
...
- Danh tính kỹ năng, danh tính nhà phát hành, và siêu dữ liệu phiên bản đã chọn là các trường phong bì cấp cao nhất (`slug`, `displayName`, `publisherHandle`, `version`, `resolvedFrom`, `tag`, `createdAt`) để tự động hóa shell có thể đọc chúng mà không cần mở các lớp bọc lồng nhau.
...
- `provenance` chỉ là `server-resolved-github-import` khi ClawHub đã phân giải và lưu repo/ref/commit/path GitHub trong quá trình phát hành hoặc nhập; nếu không, giá trị là `unavailable`.

### `POST /api/v1/skills/-/security-verdicts`

Trả về các kết luận bảo mật ngắn gọn hiện tại cho đúng phiên bản kỹ năng. Điểm cuối
...
- Phản hồi chỉ liên quan đến bảo mật. Phản hồi không bao gồm dữ liệu Skill Card, trạng thái thẻ đã tạo, danh sách tệp hiện vật, hoặc payload chi tiết của trình quét.
...
- `security.signals.dependencyRegistry` được giữ lại để tương thích phản hồi v1, nhưng trình quét sự tồn tại của sổ đăng ký phụ thuộc đã ngừng hoạt động và khóa này luôn là `null`.
- Việc không có Skill Card không ảnh hưởng đến `ok`, `decision`, hoặc `reasons` của điểm cuối này; client nên đọc `skill-card.md` đã cài đặt ở cục bộ khi cần nội dung thẻ.
- Dùng `/verify` khi bạn cần phong bì xác minh Skill Card cho một kỹ năng, `/card` khi bạn cần markdown thẻ đã tạo, và `/scan` khi bạn cần dữ liệu chi tiết của trình quét.
...
- Các mục Skills vẫn được hỗ trợ bởi sổ đăng ký Skills và vẫn chỉ có thể được xuất bản thông qua `POST /api/v1/skills`.
...
- `startDate` (bắt buộc): giới hạn dưới bằng mili giây Unix cho `updatedAt` của Skills.
- `endDate` (bắt buộc): giới hạn trên bằng mili giây Unix cho `updatedAt` của Skills.
...
- Lọc theo danh mục là một bộ lọc API thực, được hỗ trợ bởi các hàng digest danh mục Plugin,
  không phải là phép viết lại truy vấn tìm kiếm.
...
- `release.artifactKind`, `release.artifactSha256`, `release.npmIntegrity`,
  `release.npmShasum`, và `release.npmTarballName` có mặt khi artifact
  bản phát hành có thông tin.
...
- Phiên bản gói cũ trả về artifact `legacy-zip` và
  `downloadUrl` ZIP cũ.
- Phiên bản ClawPack trả về artifact `npm-pack`, các trường toàn vẹn npm, một
  `tarballUrl`, và URL tương thích ZIP cũ.
- Đây là bề mặt trình phân giải của OpenClaw; nó tránh phải đoán định dạng kho lưu trữ từ
  một URL dùng chung.
...
- Phiên bản ClawPack stream chính xác các byte `.tgz` npm-pack đã tải lên.
- Phiên bản ZIP cũ chuyển hướng đến `/api/v1/packages/{name}/download?version=`.
- Dùng bucket giới hạn tốc độ tải xuống.

### `GET /api/v1/packages/{name}/readiness`

Trả về trạng thái sẵn sàng đã tính toán cho việc tiêu thụ OpenClaw trong tương lai.
...
- `packageName` được chuẩn hóa theo tên npm; gói có thể thiếu đối với các lần di chuyển đã lên kế hoạch.
...
- Yêu cầu mã thông báo API cho người dùng moderator hoặc admin.
...
Điểm cuối chủ sở hữu/moderator để xem trạng thái kiểm duyệt gói.

Xác thực:

- Yêu cầu mã thông báo API cho chủ sở hữu gói, thành viên nhà phát hành, moderator, hoặc
  người dùng admin.
...
quy trình làm việc có thể kiểm toán.
...
Điểm cuối moderator/admin để đánh giá bản phát hành gói.
...
Tải xuống kho lưu trữ ZIP tất định legacy cho một bản phát hành gói.
...
- Bàn giao skill dựa trên GitHub không proxy hoặc mirror byte. Phản hồi JSON
  ...
  payload thành công.
- Thống kê tải xuống được tính theo danh tính duy nhất mỗi ngày UTC (`userId` khi mã thông báo API hợp lệ, nếu không thì IP).
...
Xác thực mã thông báo và trả về handle của người dùng.
...
- Trường payload tùy chọn: `ownerHandle`. Khi có, API phân giải nhà phát hành đó
  ở phía máy chủ và yêu cầu tác nhân có quyền truy cập nhà phát hành.
...
- Xuất bản thay mặt vẫn xác thực điều kiện đủ tư cách kênh official dựa trên tài khoản chủ sở hữu đích.
...
Các lần ẩn của moderator/admin và gỡ bỏ bảo mật không hết hạn theo cách này.
...
Chỉ admin. Đảm bảo nhà phát hành tổ chức tồn tại cho một handle. Nếu handle vẫn trỏ đến một
người dùng/nhà phát hành cá nhân dùng chung legacy, điểm cuối sẽ di chuyển nó vào nhà phát hành tổ chức trước.
Với tổ chức mới tạo, cung cấp `memberHandle`; admin đang hành động không được thêm làm thành viên.
...
Chỉ admin. Khôi phục một nhà phát hành cá nhân cho principal GitHub OAuth thay thế đã được xác minh
mà không chỉnh sửa các hàng tài khoản Convex Auth. Yêu cầu phải nêu cả hai id tài khoản provider GitHub
bất biến; các handle có thể thay đổi chỉ được dùng làm cơ chế bảo vệ hướng tới operator.
...
định danh GitHub chính. Khôi phục sẽ thất bại theo hướng đóng an toàn khi publisher cá nhân hiện tại
...
các đường dẫn chủ sở hữu trực tiếp thống nhất với thẩm quyền publisher mới. Một lượt đặt trước handle được bảo vệ
đang hoạt động cho handle được khôi phục cũng được gán lại cho người dùng thay thế để lần đồng bộ hóa
hồ sơ sau này không thể khôi phục thẩm quyền cạnh tranh của người dùng trước đó. Mỗi bảng chính được giới hạn ở
100 hàng cho mỗi giao dịch áp dụng; các khôi phục lớn hơn trước tiên phải dùng di chuyển chủ sở hữu có thể tiếp tục.
Nguồn kỹ năng GitHub được phạm vi theo publisher và được báo cáo là đã kiểm tra thay vì được ghi lại.
...
chuẩn bị một tarball ClawPack phải gửi mã định danh lưu trữ thu được dưới dạng
