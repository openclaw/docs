...
x-i18n:
    generated_at: "2026-07-03T15:33:34Z"
    model: gpt-5.5
...
- Yêu cầu đã xác thực (token Bearer hợp lệ): thực thi theo bucket từng người dùng.
...
- `X-RateLimit-Remaining` / `RateLimit-Remaining`: hạn mức còn lại chính xác khi có mặt.
...
- Nếu không có IP client đáng tin cậy, yêu cầu ẩn danh dùng các bucket dự phòng
  chỉ được phạm vi hóa theo loại giới hạn tốc độ. Các bucket dự phòng này không bao gồm
  đường dẫn, slug, tên gói, phiên bản, chuỗi truy vấn, hoặc tham số artifact khác do caller cung cấp.
...
- `highlightedOnly` (tùy chọn): `true` để lọc chỉ Skills được làm nổi bật
...
- Kết quả được trả về theo thứ tự liên quan (độ tương đồng embedding + tăng hạng token slug/tên khớp chính xác + một mức ưu tiên nhỏ theo độ phổ biến).
- Độ liên quan mạnh hơn độ phổ biến. Một slug chính xác hoặc token tên hiển thị khớp có thể xếp trên một kết quả khớp lỏng hơn nhưng có tương tác mạnh hơn nhiều.
- Văn bản ASCII được tách token theo ranh giới từ và dấu câu. Ví dụ, `personal-map` chứa token `map` độc lập, còn `amap-jsapi-skill` chứa `amap`, `jsapi`, và `skill`; vì vậy tìm kiếm `map` cho `personal-map` mức khớp từ vựng mạnh hơn `amap-jsapi-skill`.
- Độ phổ biến được tính theo thang log và có giới hạn trần. Skills có tương tác cao có thể xếp thấp hơn khi văn bản truy vấn khớp yếu hơn.
- Trạng thái kiểm duyệt đáng ngờ hoặc bị ẩn có thể loại bỏ một Skill khỏi tìm kiếm công khai tùy thuộc vào bộ lọc của caller và trạng thái kiểm duyệt hiện tại.

Hướng dẫn về khả năng được nhà phát hành khám phá:

- Đặt các thuật ngữ người dùng sẽ thực sự tìm kiếm trong tên hiển thị, tóm tắt, và thẻ. Chỉ dùng một token slug độc lập khi nó cũng là một danh tính ổn định mà bạn muốn giữ.
- Không đổi tên slug chỉ để chạy theo một truy vấn, trừ khi slug mới là tên chuẩn dài hạn tốt hơn. Slug cũ trở thành bí danh chuyển hướng, nhưng URL chuẩn, slug được hiển thị, và các bản tóm tắt tìm kiếm tương lai dùng slug mới.
- Bí danh đổi tên duy trì phân giải cho URL cũ và các lượt cài đặt phân giải qua registry, nhưng thứ hạng tìm kiếm dựa trên siêu dữ liệu Skill chuẩn sau khi đổi tên đã được lập chỉ mục. Thống kê hiện có vẫn đi cùng Skill.
- Nếu một Skill bất ngờ không hiển thị, trước tiên hãy kiểm tra trạng thái kiểm duyệt bằng `clawhub inspect @owner/slug` khi đã đăng nhập trước khi thay đổi siêu dữ liệu liên quan đến xếp hạng.
...
- `cursor` (tùy chọn): con trỏ phân trang cho bất kỳ cách sắp xếp nào không phải `trending`
...
- `createdAt` ổn định cho việc crawl Skill mới; `updated` thay đổi khi Skills hiện có được phát hành lại.
- Khi `nonSuspiciousOnly=true`, các cách sắp xếp dựa trên con trỏ có thể trả về ít mục hơn `limit` trên một trang vì Skills đáng ngờ được lọc sau khi truy xuất trang.
...
- Slug cũ được tạo bởi luồng đổi tên/hợp nhất chủ sở hữu phân giải tới Skill chuẩn.
...
- Chủ sở hữu và điều phối viên có thể truy cập chi tiết kiểm duyệt cho Skills bị ẩn.
- Caller công khai chỉ nhận `200` cho Skills hiển thị đã bị gắn cờ.
- Bằng chứng được biên tập lại cho caller công khai và chỉ bao gồm đoạn trích thô cho chủ sở hữu/điều phối viên.

### `POST /api/v1/skills/{slug}/report`

Báo cáo một Skill để điều phối viên xem xét. Báo cáo ở cấp Skill, tùy chọn liên kết
...
Endpoint điều phối viên/quản trị viên cho việc tiếp nhận báo cáo Skill.
...
- `moderation` là ảnh chụp moderation hiện tại ở cấp skill, được suy ra từ phiên bản mới nhất.
...
Các lượt quét đã xuất bản sử dụng JSON:
...
- Các lượt quét đã xuất bản yêu cầu quyền truy cập quản lý của chủ sở hữu/nhà xuất bản, hoặc quyền moderator/admin của nền tảng.
- Các lượt quét đã xuất bản chỉ ghi ngược lại khi `update: true` và lượt quét hoàn tất thành công.
...
- Các tác vụ quét là bất đồng bộ. Yêu cầu quét thủ công được ưu tiên trước công việc xuất bản/backfill thông thường, nhưng việc hoàn tất vẫn phụ thuộc vào khả năng sẵn sàng của worker.

### `GET /api/v1/skills/-/scan/{scanId}`

Endpoint thăm dò đã xác thực cho một lượt quét đã gửi.

- Trả về trạng thái queued/running/succeeded/failed.
- Trả về `queue.queuedAhead` và `queue.position` khi đang xếp hàng để client có thể hiển thị số lượt quét thủ công ưu tiên đang đứng trước yêu cầu. Hàng đợi rất lớn được giới hạn và báo cáo với `queuedAheadIsEstimate: true`.
...
- Yêu cầu quyền truy cập quản lý của chủ sở hữu/nhà xuất bản đối với skill hoặc plugin, hoặc quyền moderator/admin của nền tảng.
- Trả về kết quả quét đã lưu cho đúng phiên bản đã gửi, bao gồm cả phiên bản bị chặn hoặc bị ẩn.
- `kind` mặc định là `skill`; dùng `kind=plugin` cho các lượt quét plugin/package.
- Trả về cùng cấu trúc ZIP như các bản tải xuống scan-request.

### `POST /api/v1/skills/-/scan/batch`

Tuyến quét lại hàng loạt chuẩn, chỉ dành cho admin. Tuyến này chấp nhận cùng cấu trúc payload như `POST /api/v1/skills/-/rescan-batch` cũ.
...
- `ok` chỉ là `true` khi phiên bản đã chọn có Skill Card đã tạo, không bị moderation chặn vì malware, và xác minh ClawScan là sạch.
...
- `security` là kết luận ClawScan/security cấp cao nhất. Tự động hóa nên dựa vào `ok`, `decision`, `reasons`, và `security.status`.
...
Trả về các kết luận bảo mật rút gọn hiện tại cho các phiên bản skill chính xác. Endpoint
bộ sưu tập này dành cho các client đã biết những phiên bản skill ClawHub đã cài đặt
...
- Phản hồi chỉ về bảo mật. Nó không bao gồm dữ liệu Skill Card, trạng thái card đã tạo, danh sách tệp artifact, hoặc payload chi tiết của trình quét.
...
- `security.signals.dependencyRegistry` được giữ lại để tương thích phản hồi v1, nhưng trình quét sự tồn tại của registry phụ thuộc đã ngừng hoạt động và khóa này luôn là `null`.
- Việc thiếu Skill Card không ảnh hưởng đến `ok`, `decision`, hoặc `reasons` của endpoint này; client nên đọc `skill-card.md` đã cài đặt tại cục bộ khi cần nội dung card.
- Dùng `/verify` khi bạn cần phong bì xác minh Skill Card cho một skill, `/card` khi bạn cần markdown card đã tạo, và `/scan` khi bạn cần dữ liệu trình quét chi tiết.
...
- Mục Skills vẫn được hỗ trợ bởi registry Skills và vẫn chỉ có thể được xuất bản thông qua `POST /api/v1/skills`.
...
- `startDate` (bắt buộc): cận dưới Unix mili giây cho `updatedAt` của Skills.
- `endDate` (bắt buộc): cận trên Unix mili giây cho `updatedAt` của Skills.
...
- Skills hiện tại dựa trên GitHub có bản quét `clean` hoặc `suspicious` bao gồm
  `_source_handoff.json` với `sourceRef: "public-github"`, repo, commit, path,
  băm nội dung và URL kho lưu trữ. Chúng không bao gồm các tệp nguồn do ClawHub lưu trữ.
...
- `_manifest.json` luôn được bao gồm ở gốc ZIP.
- `_errors.json` được bao gồm khi từng Plugin hoặc tệp riêng lẻ không thể được
  xuất.
...
- Các bí danh bộ lọc v1 cũ được ghi tài liệu dưới `GET /api/v1/plugins` cũng được
  chấp nhận.
- Lọc danh mục là một bộ lọc API thật được hỗ trợ bởi các hàng digest danh mục
  Plugin, không phải là viết lại truy vấn tìm kiếm.
...
- Skills cũng có thể phân giải qua tuyến này trong catalog hợp nhất.
...
- Các bản phát hành ClawPack bao gồm các trường tương thích với npm `npmIntegrity`, `npmShasum`, và
  `npmTarballName`.
- `version.sha256hash` là siêu dữ liệu tương thích đã ngừng khuyến nghị cho client cũ. Nó
  băm đúng các byte ZIP được trả về bởi `/api/v1/packages/{name}/download`.
  Client hiện đại nên dùng `version.artifact.sha256`, giá trị này xác định
...
Trả về đúng bản tóm tắt bảo mật và độ tin cậy của bản phát hành gói cho các
...
- Điểm cuối đọc công khai. Không yêu cầu token chủ sở hữu, publisher, điều phối viên, hoặc quản trị viên.
...
- `release.artifactKind`, `release.artifactSha256`, `release.npmIntegrity`,
  ...
  artifact phát hành.
- `trust.scanStatus` là trạng thái tin cậy hiệu lực được dẫn xuất từ dữ liệu đầu vào của trình quét
  và điều phối bản phát hành thủ công.
- `trust.moderationState` có thể là null. Nó là `null` khi không có điều phối bản phát hành
  thủ công.
- `trust.blockedFromDownload` là tín hiệu chặn cài đặt. OpenClaw và các
  ...
  tự dẫn xuất lại quy tắc chặn từ các trường trình quét hoặc điều phối.
...
- Điểm cuối này chính xác theo phiên bản. Client nên gọi nó sau khi phân giải
  phiên bản gói mà chúng định cài đặt, không chỉ sau khi đọc siêu dữ liệu gói
  mới nhất.
- Các gói riêng tư trả về `404` trừ khi bên gọi có thể đọc publisher sở hữu.
- Điểm cuối này được cố ý thu hẹp hơn các điểm cuối điều phối của chủ sở hữu/điều phối viên.
  Nó công khai quyết định cài đặt và giải thích công khai, không phải
...
Điểm cuối điều phối viên để liệt kê các hàng di trú Plugin OpenClaw chính thức.
...
Điểm cuối quản trị viên để tạo hoặc cập nhật một hàng di trú Plugin chính thức.
...
- `packageName` được chuẩn hóa theo tên npm; gói có thể bị thiếu đối với các di trú
  đã lên kế hoạch.
- Nội dung này chỉ theo dõi mức sẵn sàng di trú. Nó không thay đổi OpenClaw hoặc tạo
  ClawPack.
...
- Yêu cầu API token của người dùng moderator hoặc admin.
...
Điểm cuối dành cho chủ sở hữu/moderator để xem thông tin moderation của gói.

Xác thực:

- Yêu cầu API token của chủ sở hữu gói, thành viên publisher, moderator, hoặc
  người dùng admin.
...
`finalAction: "revoke"` với một báo cáo đã xác nhận để áp dụng moderation bản phát hành trong
cùng một workflow có thể kiểm toán.
...
Mọi thay đổi đều ghi một mục audit log.
...
- Dùng read rate bucket, không phải download bucket.
...
- Gói riêng tư trả về `404` trừ khi caller có thể đọc publisher sở hữu.
...
- Kho lưu trữ Plugin/gói là tệp zip có gốc `package/` để các client OpenClaw cũ
  tiếp tục hoạt động.
- Route này chỉ giữ ở dạng ZIP. Nó không stream các tệp ClawPack `.tgz`.
...
- Gói riêng tư trả về `404` trừ khi caller là chủ sở hữu.
...
- Dùng download rate bucket.
- Header tải xuống bao gồm SHA-256 của ClawHub cùng metadata integrity/shasum của npm.
- Các kiểm tra moderation và quyền truy cập gói riêng tư vẫn áp dụng.
...
Tải xuống ZIP phiên bản skill được lưu trữ, hoặc trả về handoff nguồn GitHub cho một
...
- Các phiên bản đã soft-delete trả về `410`.
- Handoff skill dựa trên GitHub không proxy hoặc mirror byte. Phản hồi JSON
  bao gồm `sourceRef: "public-github"`, `repo`, `commit`, `path`, `contentHash`,
  và `archiveUrl`; trạng thái quét/hiện tại là một gate và không được đưa vào dưới dạng metadata payload
  thành công.
...
- Body JSON có `files` (dựa trên storageId) cũng được chấp nhận.
...
- Trường payload tùy chọn: `migrateOwner`. Khi là `true` với `ownerHandle`, một
  ...
  publisher hiện tại và publisher đích. Nếu không opt-in điều này, các thay đổi chủ sở hữu sẽ
  bị từ chối.
...
- Các trường form được phép là `payload`, blob `files` lặp lại, hoặc một tham chiếu tarball `clawpack`.
  `clawpack` có thể là blob `.tgz` hoặc storage id do flow upload-url trả về.
...
- Body JSON và metadata `payload.files` / `payload.artifact` do caller cung cấp
  bị từ chối.
- Yêu cầu xuất bản multipart trực tiếp bị giới hạn ở 18MB. Tarball ClawPack có thể
  dùng flow upload-url lên đến giới hạn tarball 120MB.
...
- Chỉ publisher org `openclaw` và publisher cá nhân của các thành viên org `openclaw`
  hiện tại mới có thể xuất bản lên kênh `official`.
- Các lượt xuất bản thay mặt vẫn xác thực điều kiện đủ để dùng kênh official theo tài khoản chủ sở hữu đích.

### `DELETE /api/v1/skills/{slug}` / `POST /api/v1/skills/{slug}/undelete`

Soft-delete / khôi phục một skill (chủ sở hữu, moderator, hoặc admin).
...
Khi có, `reason` được lưu làm ghi chú moderation của skill và được sao chép vào audit log.
Các lượt soft delete do chủ sở hữu khởi tạo giữ slug trong 30 ngày, sau đó slug có thể được
...
Chỉ admin. Đảm bảo một publisher org tồn tại cho một handle. Nếu handle vẫn trỏ đến một
publisher người dùng/cá nhân dùng chung kiểu cũ, endpoint sẽ di chuyển nó thành publisher org trước.
Đối với org mới tạo, cung cấp `memberHandle`; admin đang thực hiện không được thêm làm thành viên.
...
Tạo publisher org tự phục vụ đã xác thực. Tạo một publisher org mới và thêm
caller làm owner. Endpoint này không di chuyển các handle người dùng/cá nhân hiện có và
không đánh dấu publisher là trusted/official.

- Body: `{ "handle": "opik", "displayName": "Opik" }`
- Phản hồi: `{ "ok": true, "publisherId": "...", "handle": "opik", "created": true, "trusted": false }`
- Trả về `409` khi handle đã được một publisher, người dùng, hoặc publisher cá nhân sử dụng.

### `POST /api/v1/users/reserve`

Chỉ admin. Giữ trước các root slug và tên gói cho chủ sở hữu hợp pháp mà không xuất bản một
bản phát hành. Tên gói trở thành gói placeholder riêng tư không có hàng release, để cùng
chủ sở hữu đó có thể xuất bản bản phát hành code-plugin hoặc bundle-plugin thật vào tên đó sau này.
...
Chỉ admin. Khôi phục một publisher cá nhân cho một principal GitHub OAuth thay thế đã xác minh
mà không chỉnh sửa các hàng tài khoản Convex Auth. Yêu cầu phải nêu cả hai id tài khoản provider GitHub
bất biến; các handle có thể thay đổi chỉ được dùng như một guard hướng đến operator.
...
định danh GitHub. Khôi phục sẽ đóng thất bại khi nhà phát hành cá nhân hiện tại của người dùng đích
có kỹ năng, gói, hoặc nguồn kỹ năng GitHub.
Khôi phục cũng di chuyển các trường `ownerUserId` kế thừa cho các kỹ năng của nhà phát hành được khôi phục,
bí danh slug kỹ năng, gói, cảnh báo trình kiểm tra gói, và các hàng tóm tắt tìm kiếm phái sinh để
...
không thể khôi phục thẩm quyền cạnh tranh của người dùng trước đó. Mỗi bảng chính được giới hạn ở
100 hàng cho mỗi giao dịch áp dụng; các khôi phục lớn hơn trước tiên phải dùng một di chuyển chủ sở hữu có thể tiếp tục.
Nguồn kỹ năng GitHub nằm trong phạm vi nhà phát hành và được báo cáo là đã kiểm tra thay vì được viết lại.
...
Cấm một người dùng và xóa cứng các kỹ năng thuộc sở hữu của họ (chỉ người điều hành/quản trị viên).
...
nội dung (chỉ quản trị viên). Mặc định là chạy thử trừ khi `dryRun` là `false`.
...
## Điểm cuối CLI kế thừa (không còn được khuyến nghị)
...
chuẩn bị một tarball ClawPack phải gửi mã định danh lưu trữ kết quả dưới dạng
`clawpack` và ticket được trả về dưới dạng `clawpackUploadTicket`.

## Khám phá sổ đăng ký (`/.well-known/clawhub.json`)

CLI có thể khám phá các cài đặt sổ đăng ký/xác thực từ trang web:

- `/.well-known/clawhub.json` (JSON, ưu tiên)
- `/.well-known/clawdhub.json` (kế thừa)
...
Nếu bạn tự lưu trữ, hãy phục vụ tệp này (hoặc đặt rõ `CLAWHUB_REGISTRY`; `CLAWDHUB_REGISTRY` kế thừa).
