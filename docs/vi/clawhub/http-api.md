...
x-i18n:
    generated_at: "2026-07-02T22:36:34Z"
    model: gpt-5.5
...
Các thư mục bên thứ ba có thể dùng các endpoint đọc công khai để liệt kê hoặc tìm kiếm Skills trên ClawHub. Vui lòng lưu cache kết quả, tuân thủ `429`/`Retry-After`, liên kết người dùng trở lại trang niêm yết ClawHub chính tắc (`https://clawhub.ai/<owner>/skills/<slug>`), và tránh ngụ ý rằng ClawHub chứng thực trang bên thứ ba. Không cố gắng sao chép nội dung ẩn, riêng tư, hoặc bị chặn bởi kiểm duyệt ra ngoài bề mặt API công khai.

Các lối tắt slug web được phân giải trên nhiều họ registry, nhưng client API nên dùng
các URL chính tắc do endpoint đọc trả về thay vì tự dựng lại độ ưu tiên của route.
...
- Các endpoint ghi đã xác thực không nên trả về chỉ một `Unauthorized` trống khi
  ...
  tài khoản đã bị xóa/cấm/vô hiệu hóa đều nên nhận văn bản có thể hành động để
...
- `X-RateLimit-Remaining` / `RateLimit-Remaining`: ngân sách còn lại chính xác khi có mặt.
  Các yêu cầu thành công được shard sẽ bỏ qua header này thay vì trả về một giá trị toàn cục xấp xỉ.
...
- Dùng các header IP client đáng tin cậy, bao gồm `cf-connecting-ip`, chỉ khi
  triển khai bật tường minh các header chuyển tiếp đáng tin cậy.
...
- Nếu không có IP client đáng tin cậy, các yêu cầu ẩn danh dùng bucket dự phòng
  chỉ được giới hạn theo loại giới hạn tần suất. Các bucket dự phòng này không bao gồm
...
nên đọc phần thân phản hồi như một chuỗi dễ đọc cho con người. Các tham số truy vấn không xác định được
...
- Kết quả được trả về theo thứ tự liên quan (độ tương đồng embedding + tăng điểm token slug/tên khớp chính xác + một độ ưu tiên phổ biến nhỏ).
- Độ liên quan mạnh hơn độ phổ biến. Một slug chính xác hoặc token tên hiển thị khớp có thể xếp hạng cao hơn một kết quả khớp lỏng hơn nhưng có mức tương tác mạnh hơn nhiều.
- Văn bản ASCII được tách token theo ranh giới từ và dấu câu. Ví dụ, `personal-map` chứa một token `map` độc lập, còn `amap-jsapi-skill` chứa `amap`, `jsapi`, và `skill`; do đó tìm kiếm `map` cho `personal-map` một kết quả khớp từ vựng mạnh hơn `amap-jsapi-skill`.
- Độ phổ biến được co giãn theo log và có giới hạn trần. Skills có mức tương tác cao vẫn có thể xếp hạng thấp hơn khi văn bản truy vấn khớp yếu hơn.
- Trạng thái kiểm duyệt đáng ngờ hoặc ẩn có thể loại một Skills khỏi tìm kiếm công khai tùy theo bộ lọc của caller và trạng thái kiểm duyệt hiện tại.

Hướng dẫn khả năng được khám phá cho nhà phát hành:

- Đặt các thuật ngữ mà người dùng sẽ thực sự tìm kiếm vào tên hiển thị, tóm tắt, và tag. Chỉ dùng token slug độc lập khi đó cũng là một danh tính ổn định mà bạn muốn giữ.
- Không đổi tên slug chỉ để chạy theo một truy vấn, trừ khi slug mới là tên chính tắc dài hạn tốt hơn. Slug cũ trở thành alias chuyển hướng, nhưng URL chính tắc, slug hiển thị, và digest tìm kiếm trong tương lai dùng slug mới.
- Alias đổi tên giữ khả năng phân giải cho URL cũ và lượt cài đặt cũ phân giải qua registry, nhưng xếp hạng tìm kiếm dựa trên metadata Skills chính tắc sau khi việc đổi tên đã được lập chỉ mục. Thống kê hiện có vẫn gắn với Skills đó.
- Nếu một Skills bất ngờ không hiển thị, trước tiên hãy kiểm tra trạng thái kiểm duyệt bằng `clawhub inspect @owner/slug` khi đã đăng nhập trước khi thay đổi metadata liên quan đến xếp hạng.
...
- `cursor` (tùy chọn): cursor phân trang cho bất kỳ sắp xếp nào không phải `trending`
- `sort` (tùy chọn): `updated` (mặc định), `recommended` (alias: `default`), `createdAt` (alias: `newest`), `downloads`, `stars` (alias: `rating`), các alias cài đặt cũ `installsCurrent`/`installs`/`installsAllTime` ánh xạ đến `downloads`, `trending`
- `nonSuspiciousOnly` (tùy chọn): `true` để ẩn Skills đáng ngờ (`flagged.suspicious`)
- `nonSuspicious` (tùy chọn): alias cũ cho `nonSuspiciousOnly`
...
- Slug cũ được tạo bởi các luồng đổi tên/hợp nhất owner phân giải đến Skills chính tắc.
- `metadata.os`: các giới hạn OS được khai báo trong frontmatter của Skills (ví dụ `["macos"]`, `["linux"]`). `null` nếu không được khai báo.
...
Báo cáo một Skills để moderator xem xét. Báo cáo ở cấp Skills, có thể liên kết
...
- `limit` (tùy chọn): số nguyên (1-200)
- `cursor` (tùy chọn): cursor phân trang
...
Endpoint moderator/admin để xử lý hoặc mở lại báo cáo Skills.
...
để ẩn Skills trong cùng một quy trình có thể kiểm toán.
...
- `security.hasScanResult` chỉ là `true` khi một trình quét tạo ra phán quyết xác định (`clean`, `suspicious`, hoặc `malicious`).
...
Các lượt quét tải lên cục bộ không còn được hỗ trợ. Các yêu cầu dùng
`multipart/form-data` hoặc `{ "source": { "kind": "upload" } }` trả về `410`.

Các lượt quét đã phát hành dùng JSON:
...
- Các lượt quét đã phát hành yêu cầu quyền truy cập quản lý của chủ sở hữu/nhà xuất bản, hoặc quyền điều phối viên/quản trị viên nền tảng.
- Các lượt quét đã phát hành chỉ ghi ngược lại khi `update: true` và lượt quét hoàn tất thành công.
...
- Tác vụ quét là bất đồng bộ. Các yêu cầu quét thủ công được ưu tiên trước công việc phát hành/nạp bù thông thường, nhưng việc hoàn tất vẫn phụ thuộc vào mức độ sẵn sàng của worker.
...
- Trả về `queue.queuedAhead` và `queue.position` trong khi đang xếp hàng để client có thể hiển thị số lượt quét thủ công được ưu tiên đang đứng trước yêu cầu. Các hàng đợi rất lớn được giới hạn và báo cáo với `queuedAheadIsEstimate: true`.
- Khi có sẵn, `report` chứa các mục `clawscan`, `skillspector`, `staticAnalysis`, và `virustotal`.
- Các tác vụ quét thất bại trả về `status: "failed"` cùng với `lastError`.
...
- `ok` chỉ là `true` khi phiên bản đã chọn có Skill Card được tạo, không bị kiểm duyệt chặn do phần mềm độc hại, và xác minh ClawScan là sạch.
...
- `security.signals` chứa bằng chứng trình quét hỗ trợ như `staticScan`, `virusTotal`, và `skillSpector`.
...
- `security.signals.dependencyRegistry` được giữ lại để tương thích phản hồi v1, nhưng trình quét tồn tại sổ đăng ký phụ thuộc đã ngừng hoạt động và khóa này luôn là `null`.
- Việc thiếu Skill Card không ảnh hưởng đến `ok`, `decision`, hoặc `reasons` của điểm cuối này; client nên đọc `skill-card.md` đã cài đặt cục bộ khi cần nội dung thẻ.
...
- Các mục Skills vẫn được hỗ trợ bởi registry Skills và vẫn chỉ có thể được xuất bản thông qua `POST /api/v1/skills`.
...
Các bí danh cũ không được chấp nhận dưới dạng giá trị danh mục được lưu trữ hoặc do tác giả khai báo.
...
- Skills hiện tại được GitHub hỗ trợ với lần quét `clean` hoặc `suspicious` bao gồm
...
- `startDate` (bắt buộc): cận dưới mili giây Unix cho `updatedAt` của Plugin.
- `endDate` (bắt buộc): cận trên mili giây Unix cho `updatedAt` của Plugin.
...
- Các bí danh bộ lọc v1 cũ được ghi tài liệu trong `GET /api/v1/plugins` cũng
  được chấp nhận.
- Lọc danh mục là một bộ lọc API thực sự, được hỗ trợ bởi các hàng digest danh mục Plugin,
  không phải là ghi lại truy vấn tìm kiếm.
...
- Skills cũng có thể được phân giải qua tuyến này trong catalog hợp nhất.
...
- `version.sha256hash` là siêu dữ liệu tương thích đã lỗi thời cho các máy khách cũ. Nó
  ...
  Máy khách hiện đại nên dùng `version.artifact.sha256`, giá trị nhận diện
...
- Điểm cuối đọc công khai. Không yêu cầu mã thông báo chủ sở hữu, nhà phát hành, điều phối viên hoặc quản trị viên.
...
- `trust.scanStatus` là trạng thái tin cậy hiệu dụng, được suy ra từ đầu vào của trình quét
  và kiểm duyệt bản phát hành thủ công.
- `trust.moderationState` có thể rỗng. Nó là `null` khi không có
  kiểm duyệt bản phát hành thủ công.
...
- `trust.reasons` là danh sách giải thích hướng tới người dùng và kiểm toán. Mã lý do
...
- Các phiên bản gói cũ trả về một hiện vật `legacy-zip` và một
  `downloadUrl` ZIP cũ.
...
- Các phiên bản ZIP cũ chuyển hướng tới `/api/v1/packages/{name}/download?version=`.
...
- tính khả dụng của hiện vật ClawPack npm-pack
- digest hiện vật
...
- Phần này chỉ theo dõi mức độ sẵn sàng di chuyển. Nó không thay đổi OpenClaw hoặc tạo
  ClawPack.

### `GET /api/v1/packages/moderation/queue`

Điểm cuối điều phối viên/quản trị viên cho các hàng đợi đánh giá bản phát hành gói.
...
`finalAction: "revoke"` cùng với báo cáo đã xác nhận để áp dụng kiểm duyệt bản phát hành trong cùng một
...
Điểm cuối dành cho moderator/admin để xem xét bản phát hành gói.
...
- `approved`: đã được xem xét thủ công và được cho phép.
...
- Packument gói có scope hỗ trợ cả `/api/npm/@scope/name` và đường dẫn yêu cầu
...
- Sử dụng bucket tốc độ tải xuống.
...
- Bàn giao skill dựa trên GitHub không proxy hoặc mirror byte. Phản hồi JSON
  ...
  và `archiveUrl`; trạng thái quét/hiện tại là cổng kiểm soát và không được đưa vào làm
  siêu dữ liệu payload thành công.
- Thống kê tải xuống được tính theo danh tính duy nhất mỗi ngày UTC (`userId` khi token API hợp lệ, nếu không thì dùng IP).
...
- Trường payload tùy chọn: `migrateOwner`. Khi là `true` cùng với `ownerHandle`, một
  skill hiện có có thể chuyển sang owner đó nếu actor là admin/owner trên cả publisher
  hiện tại và publisher đích. Nếu không opt-in điều này, các thay đổi owner sẽ
  bị từ chối.
...
- Các trường form được phép là `payload`, blob `files` lặp lại, hoặc một tham chiếu tarball `clawpack`.
  `clawpack` có thể là blob `.tgz` hoặc storage id được trả về bởi
  luồng upload-url. Các publish bằng storage-id đã staged cũng phải bao gồm
  `clawpackUploadTicket` được trả về cùng URL upload đó.
- Dùng `files` hoặc `clawpack`, không bao giờ dùng cả hai trong cùng một yêu cầu.
- Body JSON và siêu dữ liệu `payload.files` / `payload.artifact` do caller cung cấp
  bị từ chối.
- Yêu cầu publish multipart trực tiếp bị giới hạn ở 18MB. Tarball ClawPack có thể
  dùng luồng upload-url tới giới hạn tarball 120MB.
- Trường payload tùy chọn: `ownerHandle`. Khi có mặt, chỉ admin mới có thể publish thay mặt owner đó.

Điểm nổi bật về xác thực:

- `family` phải là `code-plugin` hoặc `bundle-plugin`.
- Gói Plugin yêu cầu `openclaw.plugin.json`. Upload `.tgz` ClawPack phải
...
- Chỉ publisher org `openclaw` và publisher cá nhân của các thành viên org `openclaw`
  hiện tại mới có thể publish lên kênh `official`.
- Các publish thay mặt vẫn xác thực điều kiện đủ tư cách kênh official theo tài khoản owner đích.

### `DELETE /api/v1/skills/{slug}` / `POST /api/v1/skills/{slug}/undelete`

Xóa mềm / khôi phục một skill (owner, moderator, hoặc admin).
...
Các lần xóa mềm do owner khởi tạo giữ slug trong 30 ngày, sau đó slug có thể được
publisher khác nhận. Phản hồi xóa bao gồm `slugReservedUntil` khi thời hạn này áp dụng.
Các lần ẩn của moderator/admin và gỡ bỏ vì bảo mật không hết hạn theo cách này.
...
Chỉ dành cho admin. Đảm bảo một publisher org tồn tại cho một handle. Nếu handle vẫn trỏ tới một
người dùng chia sẻ/publisher cá nhân legacy, endpoint sẽ di chuyển nó thành publisher org trước.
Đối với org mới tạo, cung cấp `memberHandle`; admin đang thao tác không được thêm làm thành viên.
...
caller làm owner. Endpoint này không di chuyển handle người dùng/cá nhân hiện có và không
đánh dấu publisher là trusted/official.
...
Chỉ dành cho admin. Giữ chỗ root slug và tên gói cho owner hợp pháp mà không publish
bản phát hành. Tên gói trở thành gói placeholder riêng tư không có hàng release, để cùng
owner sau đó có thể publish bản phát hành code-plugin hoặc bundle-plugin thật vào tên đó.
...
Chỉ dành cho admin. Khôi phục publisher cá nhân cho một principal GitHub OAuth thay thế đã xác minh
mà không chỉnh sửa các hàng tài khoản Convex Auth. Yêu cầu phải nêu cả hai id tài khoản
nhà cung cấp GitHub bất biến; handle có thể thay đổi chỉ được dùng làm chốt kiểm tra hướng tới operator.
...
GitHub principal. Khôi phục thất bại ở trạng thái đóng khi publisher cá nhân hiện tại của người dùng đích
có kỹ năng, gói hoặc nguồn kỹ năng GitHub.
Khôi phục cũng di chuyển các trường `ownerUserId` cũ cho kỹ năng của publisher được khôi phục,
bí danh slug kỹ năng, gói, cảnh báo của trình kiểm tra gói và các hàng digest tìm kiếm phát sinh để
...
Nguồn kỹ năng GitHub được đặt theo phạm vi publisher và được báo cáo là đã kiểm tra thay vì được viết lại.
...
- Cả hai điểm cuối đều yêu cầu xác thực bằng token API và chỉ hoạt động cho chủ sở hữu kỹ năng.
...
Cấm một người dùng và xóa cứng các kỹ năng thuộc sở hữu của họ (chỉ dành cho người điều hành/quản trị viên).
...
Bỏ cấm một người dùng và khôi phục các kỹ năng đủ điều kiện (chỉ dành cho quản trị viên).
...
nội dung (chỉ dành cho quản trị viên). Mặc định là chạy thử trừ khi `dryRun` là `false`.
...
Thêm/xóa một dấu sao (đánh dấu nổi bật). Cả hai điểm cuối đều có tính lũy đẳng.
...
`POST /api/cli/upload-url` trả về `uploadUrl` và `uploadTicket`. Các lượt phát hành gói
dàn một tarball ClawPack phải gửi id lưu trữ thu được dưới dạng
