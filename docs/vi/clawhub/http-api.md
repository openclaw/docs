...
x-i18n:
    generated_at: "2026-07-01T15:25:38Z"
    model: gpt-5.5
...
Các thư mục bên thứ ba có thể dùng các endpoint đọc công khai để liệt kê hoặc tìm kiếm skills trên ClawHub. Vui lòng lưu kết quả vào bộ nhớ đệm, tuân thủ `429`/`Retry-After`, liên kết người dùng về mục ClawHub chính tắc (`https://clawhub.ai/<owner>/skills/<slug>`), và tránh ngụ ý rằng ClawHub chứng thực trang bên thứ ba. Không cố gắng sao chép nội dung bị ẩn, riêng tư, hoặc bị chặn kiểm duyệt ra ngoài bề mặt API công khai.
...
nên đọc thân phản hồi dưới dạng chuỗi con người đọc được. Tham số truy vấn không xác định được
...
- `highlightedOnly` (tùy chọn): `true` để lọc chỉ còn các skills được làm nổi bật
...
- Kết quả được trả về theo thứ tự liên quan (độ tương đồng embedding + tăng điểm token slug/tên khớp chính xác + một prior nhỏ về mức độ phổ biến).
- Độ liên quan mạnh hơn mức độ phổ biến. Một slug chính xác hoặc token tên hiển thị khớp có thể xếp hạng cao hơn một kết quả khớp lỏng hơn với mức tương tác mạnh hơn nhiều.
...
- Mức độ phổ biến được scale log và có giới hạn trần. Skills có tương tác cao có thể xếp hạng thấp hơn khi văn bản truy vấn khớp yếu hơn.
...
- Đặt các thuật ngữ mà người dùng sẽ thực sự tìm kiếm vào tên hiển thị, tóm tắt, và thẻ. Chỉ dùng token slug độc lập khi nó cũng là một định danh ổn định mà bạn muốn giữ.
- Không đổi tên slug chỉ để bám theo một truy vấn trừ khi slug mới là tên chính tắc dài hạn tốt hơn. Slug cũ trở thành bí danh chuyển hướng, nhưng URL chính tắc, slug hiển thị, và digest tìm kiếm trong tương lai dùng slug mới.
...
- Slug cũ được tạo bởi luồng đổi tên/hợp nhất chủ sở hữu phân giải đến skill chính tắc.
...
Báo cáo một skill để điều phối viên xem xét. Báo cáo ở cấp skill, có thể liên kết
...
Endpoint điều phối viên/quản trị cho tiếp nhận báo cáo skill.
...
Endpoint điều phối viên/quản trị để giải quyết hoặc mở lại báo cáo skill.
...
đặt `status` trở lại `open`. Truyền `finalAction: "hide"` cùng với một báo cáo đã phân loại
...
- `moderation` là ảnh chụp moderation cấp skill hiện tại được suy ra từ phiên bản mới nhất.
...
- Quét bản đã phát hành yêu cầu quyền truy cập quản lý của chủ sở hữu/nhà phát hành, hoặc quyền moderator/admin của nền tảng.
...
- Tác vụ quét là bất đồng bộ. Các yêu cầu quét thủ công được ưu tiên trước công việc phát hành/lấp đầy dữ liệu thông thường, nhưng việc hoàn tất vẫn phụ thuộc vào mức độ sẵn sàng của worker.
...
- Yêu cầu quyền truy cập quản lý của chủ sở hữu/nhà phát hành đối với skill hoặc plugin, hoặc quyền moderator/admin của nền tảng.
...
Tuyến quét lại hàng loạt chính tắc chỉ dành cho admin. Nó chấp nhận cùng dạng payload như `POST /api/v1/skills/-/rescan-batch` cũ.

### `POST /api/v1/skills/-/scan/batch/status`

Tuyến trạng thái hàng loạt chính tắc chỉ dành cho admin. Nó chấp nhận `{ "jobIds": ["..."] }` và trả về cùng các bộ đếm tổng hợp như `POST /api/v1/skills/-/rescan-batch/status` cũ.
...
- `ok` chỉ là `true` khi phiên bản được chọn có Skill Card đã tạo, không bị moderation chặn do phần mềm độc hại, và xác minh ClawScan là sạch.
- Danh tính skill, danh tính nhà phát hành, và siêu dữ liệu phiên bản được chọn là các trường phong bì cấp cao nhất (`slug`, `displayName`, `publisherHandle`, `version`, `resolvedFrom`, `tag`, `createdAt`) để tự động hóa shell có thể đọc chúng mà không cần mở gói các wrapper lồng nhau.
...
- Kết quả theo từng mục; một skill hoặc phiên bản bị thiếu sẽ không làm toàn bộ phản hồi thất bại.
...
- `security.signals.dependencyRegistry` được giữ lại để tương thích phản hồi v1, nhưng trình quét sự tồn tại của dependency registry đã ngừng hoạt động và khóa này luôn là `null`.
- Việc không có Skill Card không ảnh hưởng đến `ok`, `decision`, hoặc `reasons` của endpoint này; client nên đọc `skill-card.md` đã cài đặt cục bộ khi cần nội dung thẻ.
...
- `startDate` (bắt buộc): giới hạn dưới theo mili giây Unix cho `updatedAt` của Skills.
- `endDate` (bắt buộc): giới hạn trên theo mili giây Unix cho `updatedAt` của Skills.
...
- `_errors.json` được bao gồm khi không thể xuất từng Skills hoặc tệp.
...
- Các bí danh bộ lọc v1 kế thừa được ghi tài liệu trong `GET /api/v1/plugins` cũng
  được chấp nhận.
...
- `trust.reasons` là danh sách giải thích hướng đến người dùng và phục vụ kiểm toán. Mã lý do
  là các chuỗi ổn định, ngắn gọn như `manual:quarantined`, `scan:malicious`,
  và `package:malicious`.
- `trust.pending` nghĩa là một hoặc nhiều đầu vào độ tin cậy vẫn đang chờ hoàn tất.
- `trust.stale` nghĩa là bản tóm tắt độ tin cậy được tính từ đầu vào đã lỗi thời và
...
- Điểm cuối này cố ý hẹp hơn các điểm cuối điều phối của chủ sở hữu/điều phối viên.
  ...
  danh tính người báo cáo, nội dung báo cáo, bằng chứng riêng tư, hay dòng thời gian đánh giá
  nội bộ.
...
- Phiên bản gói kế thừa trả về artifact `legacy-zip` và `downloadUrl` ZIP
  kế thừa.
...
Tải xuống artifact của phiên bản qua đường dẫn trình phân giải rõ ràng.

Ghi chú:

- Phiên bản ClawPack truyền trực tiếp các byte `.tgz` npm-pack đã tải lên chính xác.
- Phiên bản ZIP kế thừa chuyển hướng đến `/api/v1/packages/{name}/download?version=`.
...
liên kết với một phiên bản. Chúng đưa vào hàng đợi điều phối nhưng không tự động ẩn hoặc
...
Điểm cuối dành cho owner/moderator để xem trạng thái kiểm duyệt gói.
...
- `dist.tarball`, `dist.integrity`, và `dist.shasum` sử dụng các trường tương thích
  với npm để người dùng có thể trỏ npm tới mirror nếu họ chọn.
...
- Trường payload tùy chọn: `migrateOwner`. Khi là `true` cùng với `ownerHandle`, một
  ...
  nhà xuất bản hiện tại và nhà xuất bản đích. Nếu không chọn tùy chọn này, thay đổi chủ sở hữu sẽ
  bị từ chối.
...
- Yêu cầu xuất bản multipart trực tiếp bị giới hạn ở 18MB. Tarball ClawPack có thể
  sử dụng luồng upload-url tới giới hạn tarball 120MB.
...
- Chỉ nhà xuất bản thuộc tổ chức `openclaw` và nhà xuất bản cá nhân của các thành viên hiện tại
  thuộc tổ chức `openclaw` mới có thể xuất bản lên kênh `official`.
- Các lần xuất bản thay mặt vẫn xác thực điều kiện hợp lệ của kênh official dựa trên tài khoản chủ sở hữu đích.
...
nhà xuất bản khác yêu cầu. Phản hồi xóa bao gồm `slugReservedUntil` khi thời hạn này áp dụng.
...
Đối với tổ chức mới tạo, cung cấp `memberHandle`; admin thực hiện không được thêm làm thành viên.
...
chủ sở hữu sau đó có thể xuất bản bản phát hành code-plugin hoặc bundle-plugin thật vào tên đó.
...
provider GitHub bất biến; các handle có thể thay đổi chỉ được dùng làm chốt kiểm tra dành cho người vận hành.
...
chủ thể GitHub. Khôi phục thất bại theo hướng đóng khi nhà phát hành cá nhân hiện tại của người dùng đích
có kỹ năng, gói hoặc nguồn kỹ năng GitHub.
Khôi phục cũng di chuyển các trường `ownerUserId` kế thừa cho kỹ năng của nhà phát hành được khôi phục,
bí danh slug kỹ năng, gói, cảnh báo của trình kiểm tra gói và các hàng tóm tắt tìm kiếm dẫn xuất để
các đường dẫn chủ sở hữu trực tiếp khớp với thẩm quyền nhà phát hành mới. Một đặt trước handle được bảo vệ
...
Nguồn kỹ năng GitHub có phạm vi theo nhà phát hành và được báo cáo là đã kiểm tra thay vì được ghi lại.
...
Cấm một người dùng và xóa cứng các kỹ năng do họ sở hữu (chỉ dành cho điều phối viên/quản trị viên).
...
Gỡ cấm một người dùng và khôi phục các kỹ năng đủ điều kiện (chỉ dành cho quản trị viên).
...
## Điểm cuối CLI kế thừa (không dùng nữa)
...
Nếu bạn tự lưu trữ, hãy phục vụ tệp này (hoặc đặt `CLAWHUB_REGISTRY` rõ ràng; `CLAWDHUB_REGISTRY` kế thừa).
