...
Các thư mục bên thứ ba có thể dùng các endpoint đọc công khai để liệt kê hoặc tìm kiếm Skills trên ClawHub. Vui lòng lưu kết quả vào bộ nhớ đệm, tuân thủ `429`/`Retry-After`, liên kết người dùng trở lại danh sách ClawHub chính thức (`https://clawhub.ai/<owner>/skills/<slug>`), và tránh hàm ý rằng ClawHub chứng thực trang web bên thứ ba. Không cố gắng sao chép nội dung ẩn, riêng tư, hoặc bị kiểm duyệt chặn ra ngoài bề mặt API công khai.

Các lối tắt slug web phân giải trên nhiều họ registry, nhưng client API nên dùng
các URL chính thức do endpoint đọc trả về thay vì tự dựng lại độ ưu tiên tuyến
đường.
...
- Nếu thiếu token hoặc token không hợp lệ, hành vi sẽ quay về thực thi theo IP.
...
- `X-RateLimit-Remaining` / `RateLimit-Remaining`: ngân sách còn lại chính xác khi có mặt.
  Các yêu cầu thành công đã được phân mảnh bỏ qua header này thay vì trả về một giá trị toàn cục xấp xỉ.
...
nên đọc phần thân phản hồi như một chuỗi dễ đọc cho con người. Các tham số truy vấn không xác định
được bỏ qua để tương thích, nhưng các tham số truy vấn được nhận diện có giá trị không hợp lệ sẽ trả về
`400`.
...
- Kết quả được trả về theo thứ tự độ liên quan (độ tương đồng embedding + tăng hạng token slug/tên khớp chính xác + một ưu tiên nhỏ về độ phổ biến).
...
- Văn bản ASCII được tách token theo ranh giới từ và dấu câu. Ví dụ, `personal-map` chứa token `map` độc lập, trong khi `amap-jsapi-skill` chứa `amap`, `jsapi`, và `skill`; vì vậy tìm kiếm `map` sẽ cho `personal-map` một kết quả khớp từ vựng mạnh hơn `amap-jsapi-skill`.
- Độ phổ biến được chia theo log và giới hạn trần. Skills có mức tương tác cao có thể xếp hạng thấp hơn khi văn bản truy vấn khớp yếu hơn.
...
- Đặt các thuật ngữ mà người dùng sẽ thật sự tìm kiếm trong tên hiển thị, tóm tắt, và thẻ. Chỉ dùng token slug độc lập khi đó cũng là một định danh ổn định mà bạn muốn giữ.
- Không đổi tên slug chỉ để bám theo một truy vấn trừ khi slug mới là tên chính thức dài hạn tốt hơn. Slug cũ trở thành bí danh chuyển hướng, nhưng URL chính thức, slug hiển thị, và bản tóm tắt tìm kiếm trong tương lai sẽ dùng slug mới.
- Bí danh đổi tên giữ nguyên khả năng phân giải cho URL cũ và lượt cài đặt phân giải qua registry, nhưng xếp hạng tìm kiếm dựa trên metadata Skill chính thức sau khi việc đổi tên đã được lập chỉ mục. Thống kê hiện có vẫn gắn với Skill.
...
- Slug cũ được tạo bởi các luồng đổi tên/hợp nhất chủ sở hữu sẽ phân giải tới Skill chính thức.
...
- `moderation` chỉ được bao gồm khi Skill bị gắn cờ hoặc chủ sở hữu đang xem.
...
Báo cáo một Skill để điều phối viên xem xét. Báo cáo ở cấp Skill, có thể liên kết tùy chọn
...
- Payload yêu cầu quét và báo cáo có thể tải xuống sẽ hết hạn khỏi kho yêu cầu quét sau thời hạn lưu giữ.
- Quét đã phát hành yêu cầu quyền truy cập quản lý của chủ sở hữu/nhà phát hành, hoặc quyền quản trị/người kiểm duyệt nền tảng.
...
- Tác vụ quét là bất đồng bộ. Yêu cầu quét thủ công được ưu tiên trước công việc phát hành/lấp dữ liệu thông thường, nhưng thời điểm hoàn tất vẫn phụ thuộc vào mức sẵn sàng của worker.
...
- Trả về `queue.queuedAhead` và `queue.position` khi đang xếp hàng để client có thể hiển thị số lượng lần quét thủ công được ưu tiên đang đứng trước yêu cầu. Các hàng đợi rất lớn được giới hạn và báo cáo với `queuedAheadIsEstimate: true`.
...
Endpoint kho lưu trữ báo cáo đã lưu trữ, đã xác thực cho các phiên bản đã gửi.

- Yêu cầu quyền truy cập quản lý của chủ sở hữu/nhà phát hành đối với kỹ năng hoặc Plugin, hoặc quyền quản trị/người kiểm duyệt nền tảng.
- Trả về kết quả quét đã lưu trữ cho đúng phiên bản đã gửi, bao gồm cả các phiên bản bị chặn hoặc bị ẩn.
...
Tuyến quét lại hàng loạt chuẩn chỉ dành cho quản trị viên. Nó chấp nhận cùng cấu trúc payload như `POST /api/v1/skills/-/rescan-batch` cũ.
...
- Danh tính kỹ năng, danh tính nhà phát hành và siêu dữ liệu phiên bản đã chọn là các trường phong bì cấp cao nhất (`slug`, `displayName`, `publisherHandle`, `version`, `resolvedFrom`, `tag`, `createdAt`) để tự động hóa shell có thể đọc chúng mà không cần giải nén các wrapper lồng nhau.
...
- `provenance` chỉ là `server-resolved-github-import` khi ClawHub đã phân giải và lưu trữ repo/ref/commit/path GitHub trong quá trình phát hành hoặc nhập; nếu không thì là `unavailable`.

### `POST /api/v1/skills/-/security-verdicts`

Trả về các phán quyết bảo mật rút gọn hiện tại cho đúng các phiên bản kỹ năng. Endpoint
...
- `GET /api/v1/code-plugins` và `GET /api/v1/bundle-plugins` vẫn là bí danh theo họ cố định.
- Các mục kỹ năng vẫn được hỗ trợ bởi sổ đăng ký kỹ năng và vẫn chỉ có thể được xuất bản qua `POST /api/v1/skills`.
...
- Bên gọi đã xác thực có thể thấy các gói riêng tư của nhà xuất bản mà họ thuộc về trong kết quả liệt kê/tìm kiếm.
...
- Bên gọi ẩn danh chỉ thấy các kênh gói công khai.
- Bên gọi đã xác thực có thể tìm kiếm các gói riêng tư của nhà xuất bản mà họ thuộc về.
- `channel=private` chỉ trả về các gói mà bên gọi đã xác thực có thể đọc.
...
- `startDate` (bắt buộc): cận dưới mili giây Unix cho `updatedAt` của kỹ năng.
- `endDate` (bắt buộc): cận trên mili giây Unix cho `updatedAt` của kỹ năng.
...
- Mỗi kỹ năng được xuất có gốc tại `{publisher}/{slug}/`.
- Kỹ năng được lưu trữ bao gồm các tệp phiên bản đã lưu mới nhất và được liệt kê trong
  `_manifest.json` với `sourceRef: "public-clawhub"`.
- Kỹ năng hiện tại được hỗ trợ bởi GitHub có lần quét `clean` hoặc `suspicious` bao gồm
...
- Các bí danh bộ lọc v1 cũ được ghi tài liệu trong `GET /api/v1/plugins` cũng
  được chấp nhận.
- Lọc theo danh mục là một bộ lọc API thực sự được hỗ trợ bởi các hàng bản tóm lược
  danh mục Plugin, không phải một thao tác viết lại truy vấn tìm kiếm.
- Kết quả được trả về theo thứ tự mức độ liên quan và hiện không phân trang.
- Các điều khiển sắp xếp UI trình duyệt cho tìm kiếm Plugin sắp xếp lại các kết quả theo mức độ liên quan đã tải,
...
- `version.sha256hash` là siêu dữ liệu tương thích đã lỗi thời dành cho client cũ. Nó
  ...
  Client hiện đại nên dùng `version.artifact.sha256`, trường nhận diện
  artifact phát hành chính tắc.
...
- Điểm cuối đọc công khai. Không yêu cầu token của chủ sở hữu, nhà xuất bản, điều hành viên,
  hoặc quản trị viên.
...
- `trust.blockedFromDownload` là tín hiệu chặn cài đặt. OpenClaw và các
  ...
  suy diễn lại quy tắc chặn từ các trường bộ quét hoặc kiểm duyệt.
...
- Gói riêng tư trả về `404` trừ khi bên gọi có thể đọc nhà xuất bản sở hữu gói.
- Điểm cuối này cố ý hẹp hơn các điểm cuối kiểm duyệt dành cho chủ sở hữu/điều hành viên.
...
- Phiên bản gói cũ trả về artifact `legacy-zip` và `downloadUrl` ZIP
  cũ.
...
- Phiên bản ClawPack truyền chính xác các byte `.tgz` npm-pack đã tải lên.
...
Điểm cuối dành cho điều hành viên để liệt kê các hàng di trú Plugin OpenClaw chính thức.

Xác thực:

- Yêu cầu token API cho người dùng là điều hành viên hoặc quản trị viên.
...
- `packageName` được chuẩn hóa theo tên npm; gói có thể bị thiếu đối với các di trú
  đã lên kế hoạch.
- Mục này chỉ theo dõi mức độ sẵn sàng di trú. Nó không thay đổi OpenClaw hoặc tạo
  ClawPack.

### `GET /api/v1/packages/moderation/queue`

Điểm cuối dành cho điều hành viên/quản trị viên cho hàng đợi rà soát bản phát hành gói.
...
Điểm cuối dành cho chủ sở hữu/điều phối viên để xem khả năng hiển thị kiểm duyệt gói.
...
- Kho lưu trữ Plugin/gói là tệp zip có gốc `package/` để các máy khách OpenClaw cũ
  tiếp tục hoạt động.
- Tuyến này chỉ giữ ZIP. Nó không phát trực tuyến các tệp ClawPack `.tgz`.
...
- Các phiên bản cũ chỉ có ZIP bị cố ý bỏ qua.
...
Phát trực tuyến đúng từng byte tarball ClawPack đã tải lên cho máy khách mirror npm.
...
- Bàn giao skill dựa trên GitHub không proxy hoặc mirror byte. Phản hồi JSON
  ...
  và `archiveUrl`; trạng thái quét/hiện tại là một cổng kiểm tra và không được đưa vào làm siêu dữ liệu
  payload thành công.
...
- Trường payload tùy chọn: `migrateOwner`. Khi là `true` cùng với `ownerHandle`, một
  ...
  nhà xuất bản hiện tại và nhà xuất bản đích. Nếu không opt-in điều này, thay đổi chủ sở hữu
  sẽ bị từ chối.
...
- Các trường biểu mẫu được phép là `payload`, các blob `files` lặp lại, hoặc một tham chiếu tarball
  ...
  luồng upload-url. Các lượt xuất bản storage-id đã dàn dựng cũng phải bao gồm
...
Ẩn bởi điều phối viên/quản trị viên và xóa vì bảo mật không hết hạn theo cách này.
...
người dùng dùng chung/nhà xuất bản cá nhân kiểu cũ, điểm cuối sẽ di chuyển nó vào một nhà xuất bản tổ chức trước.
...
GitHub principal. Quá trình khôi phục sẽ fail closed khi publisher cá nhân hiện tại của người dùng đích
...
bí danh slug của skill, packages, cảnh báo của trình kiểm tra package và các hàng digest tìm kiếm dẫn xuất để
các đường dẫn direct-owner nhất quán với thẩm quyền publisher mới. Một đặt chỗ protected-handle đang hoạt động
cho handle được khôi phục cũng được gán lại cho người dùng thay thế để quá trình đồng bộ hóa hồ sơ sau này
không thể khôi phục thẩm quyền cạnh tranh của người dùng cũ. Mỗi bảng chính được giới hạn ở
100 hàng cho mỗi giao dịch áp dụng; các lần khôi phục lớn hơn trước tiên phải dùng một quá trình di chuyển owner có thể tiếp tục.
Nguồn GitHub skill được scoped theo publisher và được báo cáo là đã kiểm tra thay vì được viết lại.
...
### Điểm cuối quản lý slug của owner
...
- Cả hai điểm cuối đều yêu cầu xác thực bằng token API và chỉ hoạt động cho owner của skill.
...
Cấm một người dùng và xóa cứng các skill thuộc sở hữu (chỉ moderator/admin).
...
Thêm/xóa star (highlights). Cả hai điểm cuối đều idempotent.
...
stage một tarball ClawPack phải gửi id lưu trữ thu được dưới dạng
