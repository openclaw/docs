...
x-i18n:
    generated_at: "2026-07-02T08:28:20Z"
    model: gpt-5.5
...
Các thư mục bên thứ ba có thể dùng các endpoint đọc công khai để liệt kê hoặc tìm kiếm Skills trên ClawHub. Vui lòng lưu cache kết quả, tuân thủ `429`/`Retry-After`, liên kết người dùng trở lại danh sách ClawHub chuẩn (`https://clawhub.ai/<owner>/skills/<slug>`), và tránh ngụ ý rằng ClawHub xác nhận trang bên thứ ba. Không cố gắng sao chép nội dung bị ẩn, riêng tư, hoặc bị chặn do kiểm duyệt ra ngoài bề mặt API công khai.

Các lối tắt slug web phân giải trên nhiều họ registry, nhưng client API nên dùng
các URL chuẩn do endpoint đọc trả về thay vì tự tái dựng mức ưu tiên tuyến
đường.
...
- `X-RateLimit-Remaining` / `RateLimit-Remaining`: ngân sách còn lại chính xác khi có mặt.
  Các yêu cầu thành công đã shard bỏ qua header này thay vì trả về giá trị toàn cục xấp xỉ.
...
nên đọc body phản hồi như một chuỗi dành cho người đọc. Tham số truy vấn không xác định được
...
- Kết quả được trả về theo thứ tự liên quan (độ tương đồng embedding + tăng điểm token slug/tên khớp chính xác + một ưu tiên phổ biến nhỏ).
...
- Độ phổ biến được co giãn theo log và có giới hạn trần. Skills có mức tương tác cao có thể xếp hạng thấp hơn khi văn bản truy vấn là kết quả khớp yếu hơn.
...
- Đừng đổi tên slug chỉ để theo đuổi một truy vấn, trừ khi slug mới là tên chuẩn dài hạn tốt hơn. Slug cũ trở thành alias chuyển hướng, nhưng URL chuẩn, slug hiển thị, và các bản tóm tắt tìm kiếm trong tương lai dùng slug mới.
- Alias đổi tên giữ khả năng phân giải cho URL cũ và các lượt cài đặt phân giải qua registry, nhưng xếp hạng tìm kiếm dựa trên metadata Skill chuẩn sau khi việc đổi tên đã được lập chỉ mục. Thống kê hiện có vẫn đi cùng Skill.
...
- Slug cũ được tạo bởi luồng đổi tên/hợp nhất chủ sở hữu phân giải về Skill chuẩn.
- `metadata.os`: hạn chế hệ điều hành được khai báo trong frontmatter của Skill (ví dụ `["macos"]`, `["linux"]`). `null` nếu không khai báo.
...
- `moderation` là ảnh chụp nhanh kiểm duyệt ở cấp Skill hiện tại, được suy ra từ phiên bản mới nhất.
...
- Các lượt quét đã xuất bản yêu cầu quyền truy cập quản lý của chủ sở hữu/nhà xuất bản, hoặc quyền điều phối viên/quản trị viên nền tảng.
...
- Công việc quét là bất đồng bộ. Các yêu cầu quét thủ công được ưu tiên trước công việc xuất bản/lấp đầy thông thường, nhưng việc hoàn tất vẫn phụ thuộc vào khả năng sẵn có của worker.
...
- Các công việc quét thất bại trả về `status: "failed"` cùng với `lastError`.
...
- Trả về cùng cấu trúc ZIP như các lượt tải xuống yêu cầu quét.
...
Trả về các kết luận bảo mật cô đọng hiện tại cho đúng các phiên bản skill. Điểm cuối
...
- Phản hồi chỉ gồm bảo mật. Nó không bao gồm dữ liệu Skill Card, trạng thái thẻ đã tạo, danh sách tệp tạo tác, hoặc tải trọng chi tiết từ trình quét.
...
- Cần mã thông báo API.

Tham số truy vấn:

- `startDate` (bắt buộc): cận dưới mili giây Unix cho `updatedAt` của Skills.
- `endDate` (bắt buộc): cận trên mili giây Unix cho `updatedAt` của Skills.
...
- Skills hiện tại được hỗ trợ bởi GitHub có bản quét `clean` hoặc `suspicious` bao gồm
  ...
  hàm băm nội dung và URL lưu trữ. Chúng không bao gồm các tệp nguồn được lưu trữ trên ClawHub.
...
Trả về tóm tắt bảo mật và độ tin cậy chính xác của bản phát hành gói cho máy khách
...
- `trust.stale` nghĩa là tóm tắt độ tin cậy được tính từ đầu vào đã lỗi thời và
...
- Điểm cuối này cố ý hẹp hơn các điểm cuối kiểm duyệt của chủ sở hữu/điều phối viên.
  Nó chỉ công khai quyết định cài đặt và giải thích công khai, không công khai
...
- Yêu cầu API token của chủ sở hữu gói, thành viên nhà xuất bản, moderator hoặc
  người dùng admin.
...
- Kho lưu trữ Plugin/gói là tệp zip với gốc `package/` để các client OpenClaw cũ
  tiếp tục hoạt động.
- Tuyến này chỉ hỗ trợ ZIP. Nó không stream các tệp ClawPack `.tgz`.
...
- Bàn giao skill dựa trên GitHub không proxy hoặc mirror byte. Phản hồi JSON
  ...
  và `archiveUrl`; trạng thái quét/hiện tại là cổng kiểm tra và không được đưa vào làm siêu dữ liệu
  payload thành công.
...
- Trường payload tùy chọn: `ownerHandle`. Khi có, API phân giải nhà xuất bản đó
  ở phía máy chủ và yêu cầu actor có quyền truy cập nhà xuất bản.
- Trường payload tùy chọn: `migrateOwner`. Khi là `true` cùng `ownerHandle`, một
  ...
  nhà xuất bản hiện tại và nhà xuất bản đích. Nếu không chọn rõ tùy chọn này, thay đổi chủ sở hữu sẽ
  bị từ chối.
...
- Trường payload tùy chọn: `ownerHandle`. Khi có, chỉ admin mới được xuất bản thay mặt chủ sở hữu đó.
...
- Chỉ nhà xuất bản org `openclaw` và các nhà xuất bản cá nhân của thành viên org `openclaw` hiện tại
  mới được xuất bản vào kênh `official`.
- Xuất bản thay mặt vẫn xác thực điều kiện đủ để dùng kênh chính thức dựa trên tài khoản chủ sở hữu đích.
...
Chỉ admin. Đảm bảo tồn tại một nhà xuất bản org cho một handle. Nếu handle vẫn trỏ đến một
...
Tạo nhà xuất bản org tự phục vụ có xác thực. Tạo một nhà xuất bản org mới và thêm
bên gọi làm chủ sở hữu. Điểm cuối này không di chuyển các handle người dùng/cá nhân hiện có và
không đánh dấu nhà xuất bản là đáng tin cậy/chính thức.
...
provider GitHub bất biến; các handle có thể thay đổi chỉ được dùng làm cơ chế bảo vệ hướng đến operator.
...
GitHub principal. Quá trình khôi phục sẽ đóng an toàn khi publisher cá nhân hiện tại của người dùng đích
có kỹ năng, gói hoặc nguồn kỹ năng GitHub.
Quá trình khôi phục cũng di chuyển các trường `ownerUserId` cũ cho kỹ năng của publisher được khôi phục,
...
Nguồn kỹ năng GitHub có phạm vi theo publisher và được báo cáo là đã kiểm tra thay vì được viết lại.
...
Thêm/xóa một star (điểm nổi bật). Cả hai điểm cuối đều idempotent.
