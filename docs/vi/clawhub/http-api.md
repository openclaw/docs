...
read_when:
    ...
    - Gỡ lỗi các yêu cầu giữa CLI ↔ sổ đăng ký
...
Tất cả đường dẫn v1 nằm trong `/api/v1/...`.
Các đường dẫn cũ `/api/...` và `/api/cli/...` vẫn được giữ để tương thích (xem `DEPRECATIONS.md`).
OpenAPI: `/api/v1/openapi.json`.

## Tái sử dụng danh mục công khai

Các thư mục bên thứ ba có thể dùng các endpoint đọc công khai để liệt kê hoặc tìm kiếm Skills trên ClawHub. Vui lòng lưu kết quả vào bộ nhớ đệm, tuân thủ `429`/`Retry-After`, liên kết người dùng trở lại danh sách ClawHub chính tắc (`https://clawhub.ai/<owner>/<slug>`), và tránh ngụ ý rằng ClawHub chứng thực trang bên thứ ba. Không cố gắng sao chép nội dung ẩn, riêng tư, hoặc bị chặn kiểm duyệt ra ngoài bề mặt API công khai.
...
- Mặc định dùng `cf-connecting-ip` (Cloudflare) cho IP của client.
- ClawHub dùng các header chuyển tiếp đáng tin cậy để xác định IP của client tại edge.
- Nếu không có IP client đáng tin cậy, các yêu cầu tải xuống ẩn danh dùng bucket dự phòng theo phạm vi endpoint thay vì một bucket toàn cục `ip:unknown`. Các yêu cầu đọc/ghi ẩn danh vẫn dùng bucket unknown dùng chung để tuyến thiếu-IP vẫn hiển thị và thận trọng.
...
- Độ liên quan mạnh hơn mức độ phổ biến. Một token slug hoặc tên hiển thị khớp chính xác có thể xếp hạng cao hơn một kết quả khớp lỏng hơn nhưng có nhiều lượt tải xuống hơn nhiều.
...
- Lượt tải xuống được dùng như một độ ưu tiên nhỏ theo thang log và tiêu chí phá hòa, không phải tín hiệu xếp hạng chính. Skills có nhiều lượt tải xuống có thể xếp hạng thấp hơn khi văn bản truy vấn khớp yếu hơn.
...
Hướng dẫn giúp nhà xuất bản dễ được tìm thấy:
...
- Không đổi tên slug chỉ để theo đuổi một truy vấn, trừ khi slug mới là tên chính tắc dài hạn tốt hơn. Slug cũ trở thành bí danh chuyển hướng, nhưng URL chính tắc, slug được hiển thị, và các digest tìm kiếm trong tương lai dùng slug mới.
- Bí danh đổi tên giữ khả năng phân giải cho URL cũ và lượt cài đặt cũ phân giải qua registry, nhưng xếp hạng tìm kiếm dựa trên siêu dữ liệu Skill chính tắc sau khi việc đổi tên đã được lập chỉ mục. Thống kê hiện có vẫn đi theo Skill.
...
- Slug cũ được tạo bởi các luồng đổi tên/gộp owner phân giải về Skill chính tắc.
- `metadata.os`: các hạn chế hệ điều hành được khai báo trong frontmatter của Skill (ví dụ `["macos"]`, `["linux"]`). `null` nếu không được khai báo.
...
- Bằng chứng được biên tập lại cho bên gọi công khai và chỉ bao gồm các đoạn thô cho owner/moderator.

### `POST /api/v1/skills/{slug}/report`

Báo cáo một Skill để moderator xem xét. Báo cáo ở cấp Skill, có thể liên kết
tùy chọn với một phiên bản, và đưa vào hàng đợi báo cáo Skill.
...
- `security.capabilityTags` bao gồm các nhãn năng lực/rủi ro xác định trước như
  `crypto`, `requires-wallet`, `can-make-purchases`, `can-sign-transactions`,
  `requires-oauth-token`, và `posts-externally` khi phát hiện.
- `security.hasScanResult` chỉ là `true` khi scanner tạo ra một kết luận dứt khoát (`clean`, `suspicious`, hoặc `malicious`).
...
- `package.name`, `package.displayName`, và `package.family` xác định gói
  sổ đăng ký đã phân giải.
- `release.releaseId`, `release.version`, và `release.createdAt` xác định
  bản phát hành chính xác đã được đánh giá.
- `release.artifactKind`, `release.artifactSha256`, `release.npmIntegrity`,
  `release.npmShasum`, và `release.npmTarballName` có mặt khi biết rõ đối với
...
- `trust.blockedFromDownload` là tín hiệu chặn cài đặt. OpenClaw và các
  ...
  suy diễn lại quy tắc chặn từ các trường trình quét hoặc điều phối.
...
- Mục này chỉ theo dõi mức sẵn sàng di chuyển. Nó không biến đổi OpenClaw hoặc tạo
  ClawPacks.
...
Báo cáo một gói để người kiểm duyệt xem xét. Báo cáo áp dụng ở cấp gói, tùy chọn
liên kết với một phiên bản. Chúng đưa vào hàng đợi kiểm duyệt nhưng không tự động ẩn hoặc
tự chặn lượt tải xuống; người kiểm duyệt nên dùng kiểm duyệt bản phát hành để
...
Điểm cuối dành cho chủ sở hữu/người kiểm duyệt để xem khả năng hiển thị kiểm duyệt gói.

Xác thực:

- Yêu cầu token API của chủ sở hữu gói, thành viên nhà xuất bản, người kiểm duyệt, hoặc
...
Điểm cuối dành cho người kiểm duyệt/quản trị viên để giải quyết hoặc mở lại báo cáo gói.
...
quy trình làm việc có thể kiểm tra được.
...
Điểm cuối dành cho người kiểm duyệt/quản trị viên để xem xét bản phát hành gói.
...
- Route này chỉ dùng ZIP. Nó không stream tệp ClawPack `.tgz`.
...
- Các lần quét VirusTotal đang chờ không chặn lượt tải xuống; các bản phát hành độc hại trả về `403`.
...
- Các lần xuất bản thay mặt vẫn xác thực điều kiện đủ để dùng kênh chính thức dựa trên tài khoản chủ sở hữu đích.
...
Khi có mặt, `reason` được lưu làm ghi chú kiểm duyệt skill và được sao chép vào nhật ký kiểm tra.
Các thao tác xóa mềm do chủ sở hữu khởi tạo sẽ giữ slug trong 30 ngày, sau đó slug có thể được
một nhà xuất bản khác nhận. Phản hồi xóa bao gồm `slugReservedUntil` khi thời hạn này áp dụng.
Các thao tác ẩn của người kiểm duyệt/quản trị viên và gỡ bỏ vì bảo mật không hết hạn theo cách này.
...
nhà xuất bản người dùng cá nhân/chia sẻ kiểu cũ, điểm cuối sẽ di chuyển nó vào một nhà xuất bản tổ chức trước.
...
Chỉ dành cho quản trị viên. Giữ trước slug gốc và tên gói cho chủ sở hữu hợp pháp mà không xuất bản một
...
- `merge` ẩn mục liệt kê nguồn và chuyển hướng slug nguồn tới mục liệt kê đích.
...
Cấm người dùng và xóa vĩnh viễn các Skills thuộc sở hữu (chỉ moderator/admin).
...
Nếu bạn tự host, hãy cung cấp tệp này (hoặc đặt `CLAWHUB_REGISTRY` một cách tường minh; `CLAWDHUB_REGISTRY` là cũ).
