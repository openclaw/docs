...
Các thư mục bên thứ ba có thể dùng các endpoint đọc công khai để liệt kê hoặc tìm kiếm ClawHub skills. Vui lòng lưu cache kết quả, tuân thủ `429`/`Retry-After`, liên kết người dùng về danh sách ClawHub chuẩn (`https://clawhub.ai/<owner>/skills/<slug>`), và tránh ngụ ý rằng ClawHub chứng thực trang bên thứ ba. Không cố gắng sao chép nội dung ẩn, riêng tư, hoặc bị chặn do kiểm duyệt ra ngoài bề mặt API công khai.
...
- Các endpoint ghi đã xác thực không nên trả về một thông báo `Unauthorized` trần khi
  máy chủ biết lý do. Token bị thiếu, token không hợp lệ/bị thu hồi, và
  tài khoản đã xóa/bị cấm/bị vô hiệu hóa đều nên nhận văn bản có thể hành động để CLI
...
- `X-RateLimit-Remaining` / `RateLimit-Remaining`: ngân sách còn lại chính xác khi có.
...
- Dùng các header IP client đáng tin cậy, bao gồm `cf-connecting-ip`, chỉ khi
  triển khai bật rõ ràng các header forwarded đáng tin cậy.
- ClawHub dùng các header forwarding đáng tin cậy để xác định IP client ở edge.
- Nếu không có IP client đáng tin cậy, các yêu cầu ẩn danh dùng bucket dự phòng
  chỉ theo loại giới hạn tốc độ. Các bucket dự phòng này không bao gồm
  đường dẫn, slug, tên package, phiên bản, chuỗi truy vấn, hoặc các
...
nên đọc body phản hồi dưới dạng chuỗi người đọc được. Tham số truy vấn không xác định được
bỏ qua để tương thích, nhưng tham số truy vấn được nhận diện với giá trị không hợp lệ trả về
`400`.
...
- `highlightedOnly` (tùy chọn): `true` để lọc chỉ các skills được nổi bật
...
- Kết quả được trả về theo thứ tự liên quan (độ tương đồng embedding + tăng điểm token slug/tên khớp chính xác + một prior nhỏ về độ phổ biến).
- Độ liên quan mạnh hơn độ phổ biến. Một slug chính xác hoặc token display-name khớp có thể xếp trên một kết quả khớp lỏng hơn nhưng có mức tương tác cao hơn nhiều.
...
- Đặt các thuật ngữ mà người dùng sẽ tìm kiếm nguyên văn vào tên hiển thị, tóm tắt, và thẻ. Chỉ dùng token slug độc lập khi nó cũng là danh tính ổn định mà bạn muốn giữ.
- Không đổi tên slug chỉ để chạy theo một truy vấn trừ khi slug mới là tên chuẩn dài hạn tốt hơn. Slug cũ trở thành alias chuyển hướng, nhưng URL chuẩn, slug hiển thị, và digest tìm kiếm trong tương lai dùng slug mới.
- Alias đổi tên giữ khả năng phân giải cho URL cũ và cài đặt cũ phân giải qua registry, nhưng xếp hạng tìm kiếm dựa trên metadata skill chuẩn sau khi tên mới đã được lập chỉ mục. Thống kê hiện có vẫn đi cùng skill.
...
- Slug cũ được tạo bởi các luồng đổi tên/gộp owner phân giải về skill chuẩn.
...
- Owner và người kiểm duyệt có thể truy cập chi tiết kiểm duyệt cho skills bị ẩn.
- Caller công khai chỉ nhận `200` cho skills hiển thị đã được gắn cờ.
...
Endpoint cho người kiểm duyệt/admin để tiếp nhận báo cáo skill.
...
Endpoint cho người kiểm duyệt/admin để giải quyết hoặc mở lại báo cáo skill.
...
đặt `status` trở lại `open`. Truyền `finalAction: "hide"` với báo cáo đã được phân loại
...
- `moderation` là ảnh chụp kiểm duyệt hiện tại ở cấp kỹ năng, được dẫn xuất từ phiên bản mới nhất.
...
Các lượt quét đã xuất bản dùng JSON:
...
- Quét đã xuất bản chỉ ghi ngược lại khi `update: true` và lượt quét hoàn tất thành công.
...
- Tác vụ quét là bất đồng bộ. Các yêu cầu quét thủ công được ưu tiên trước công việc xuất bản/lấp đầy thông thường, nhưng việc hoàn tất vẫn phụ thuộc vào tình trạng sẵn có của worker.
...
- Trả về `queue.queuedAhead` và `queue.position` khi đang xếp hàng để client có thể hiển thị số lượt quét thủ công được ưu tiên đang đứng trước yêu cầu. Hàng đợi rất lớn được giới hạn và báo cáo với `queuedAheadIsEstimate: true`.
...
- Yêu cầu quyền truy cập quản lý của chủ sở hữu/nhà xuất bản đối với kỹ năng hoặc Plugin, hoặc quyền điều phối viên/quản trị viên nền tảng.
...
Tuyến quét lại hàng loạt chính tắc chỉ dành cho quản trị viên. Tuyến này chấp nhận cùng dạng payload như `POST /api/v1/skills/-/rescan-batch` cũ.
...
Trả về phong bì xác minh Thẻ kỹ năng được `clawhub skill verify` sử dụng.

Tham số truy vấn:

- `version` (không bắt buộc): chuỗi phiên bản cụ thể.
- `tag` (không bắt buộc): phân giải một phiên bản có gắn thẻ (ví dụ `latest`).

Ghi chú:

- `ok` chỉ là `true` khi phiên bản được chọn có Thẻ kỹ năng đã tạo, không bị kiểm duyệt chặn vì phần mềm độc hại, và xác minh ClawScan là sạch.
- Danh tính kỹ năng, danh tính nhà xuất bản, và siêu dữ liệu phiên bản được chọn là các trường phong bì cấp cao nhất (`slug`, `displayName`, `publisherHandle`, `version`, `resolvedFrom`, `tag`, `createdAt`) để tự động hóa shell có thể đọc mà không cần mở các wrapper lồng nhau.
...
Trả về các phán quyết bảo mật rút gọn hiện tại cho đúng các phiên bản kỹ năng. Điểm cuối
tập hợp này dành cho các client đã biết phiên bản kỹ năng ClawHub đã cài đặt
nào cần hiển thị, chẳng hạn như OpenClaw Control UI.
...
- Kết quả tính theo từng mục; một kỹ năng hoặc phiên bản bị thiếu không làm toàn bộ phản hồi thất bại.
- Phản hồi chỉ liên quan đến bảo mật. Nó không bao gồm dữ liệu Thẻ kỹ năng, trạng thái thẻ đã tạo, danh sách tệp artifact, hoặc payload chi tiết của trình quét.
- `security.signals` chỉ chứa bằng chứng hỗ trợ ở cấp trạng thái; dùng `/scan` hoặc trang kiểm toán bảo mật ClawHub để xem chi tiết đầy đủ của trình quét.
- `security.signals.dependencyRegistry` được giữ lại để tương thích phản hồi v1, nhưng trình quét sự tồn tại của sổ đăng ký phụ thuộc đã ngừng hoạt động và khóa này luôn là `null`.
- Việc không có Thẻ kỹ năng không ảnh hưởng đến `ok`, `decision`, hoặc `reasons` của điểm cuối này; client nên đọc `skill-card.md` đã cài đặt ở cục bộ khi cần nội dung thẻ.
- Dùng `/verify` khi bạn cần phong bì xác minh Thẻ kỹ năng cho một kỹ năng đơn lẻ, `/card` khi bạn cần markdown thẻ đã tạo, và `/scan` khi bạn cần dữ liệu chi tiết của trình quét.
...
- `category` (tùy chọn): bộ lọc danh mục Plugin. Chỉ được hỗ trợ khi
  ...
  bí danh bộ lọc v1 cũ được ghi trong tài liệu ở `GET /api/v1/plugins`.
...
- `GET /api/v1/code-plugins` và `GET /api/v1/bundle-plugins` vẫn là các bí danh family cố định.
- Mục kỹ năng vẫn được hỗ trợ bởi registry kỹ năng và vẫn chỉ có thể được xuất bản qua `POST /api/v1/skills`.
...
- `category` (tùy chọn): bộ lọc danh mục Plugin. Chỉ được hỗ trợ khi
  ...
  bộ lọc v1 cũ được ghi trong tài liệu ở `GET /api/v1/plugins`.
...
- Kỹ năng được lưu trữ trên máy chủ bao gồm các tệp phiên bản đã lưu mới nhất và được liệt kê trong
  `_manifest.json` với `sourceRef: "public-clawhub"`.
- Kỹ năng hiện tại dựa trên GitHub có bản quét `clean` hoặc `suspicious` bao gồm
...
- `_errors.json` được bao gồm khi không thể xuất từng kỹ năng hoặc tệp riêng lẻ.
...
- `_manifest.json` luôn được bao gồm ở gốc ZIP.
- `_errors.json` được bao gồm khi từng Plugin hoặc tệp riêng lẻ không thể
  được xuất.
...
- `version.sha256hash` là siêu dữ liệu tương thích đã lỗi thời cho máy khách cũ. Nó
  ...
  Máy khách hiện đại nên dùng `version.artifact.sha256`, trường xác định
  hiện vật bản phát hành chuẩn.
...
- Điểm cuối đọc công khai. Không yêu cầu token chủ sở hữu, nhà phát hành, điều phối viên hoặc quản trị viên.
...
- `release.artifactKind`, `release.artifactSha256`, `release.npmIntegrity`,
  `release.npmShasum` và `release.npmTarballName` hiện diện khi đã biết đối với
  hiện vật bản phát hành.
...
- `trust.blockedFromDownload` là tín hiệu chặn cài đặt. OpenClaw và các
  ...
  tự suy lại các quy tắc chặn từ trường trình quét hoặc điều phối.
- `trust.reasons` là danh sách giải thích dành cho người dùng và kiểm toán. Mã lý do
  là các chuỗi ổn định, gọn như `manual:quarantined`, `scan:malicious`
...
- `trust.stale` nghĩa là bản tóm tắt tin cậy được tính từ các đầu vào đã lỗi thời và
  nên được coi là cần làm mới trước khi đưa ra quyết định cho phép có độ tin cậy cao.
...
- Các gói riêng tư trả về `404` trừ khi bên gọi có thể đọc nhà phát hành sở hữu.
- Điểm cuối này cố ý hẹp hơn các điểm cuối điều phối dành cho chủ sở hữu/điều phối viên.
  Nó chỉ phơi bày quyết định cài đặt và giải thích công khai, không phơi bày
  danh tính người báo cáo, nội dung báo cáo, bằng chứng riêng tư hoặc mốc thời gian
  đánh giá nội bộ.

### `GET /api/v1/packages/{name}/versions/{version}/artifact`

Trả về siêu dữ liệu bộ phân giải hiện vật rõ ràng cho một phiên bản gói.

Ghi chú:

- Các phiên bản gói cũ trả về hiện vật `legacy-zip` và `downloadUrl` ZIP cũ.
- Các phiên bản ClawPack trả về hiện vật `npm-pack`, các trường toàn vẹn npm,
...
Tải xuống hiện vật phiên bản qua đường dẫn bộ phân giải rõ ràng.
...
Điểm cuối dành cho điều phối viên/quản trị viên cho các hàng đợi đánh giá bản phát hành gói.
...
- `open`: các bản phát hành đáng ngờ, độc hại, đang chờ, bị cách ly, bị thu hồi hoặc được báo cáo.
...
Điểm cuối người điều phối/quản trị viên để tiếp nhận báo cáo gói.

Xác thực:

- Yêu cầu token API cho người dùng là người điều phối hoặc quản trị viên.
...
Điểm cuối chủ sở hữu/người điều phối để xem thông tin điều phối gói.

Xác thực:

- Yêu cầu token API cho chủ sở hữu gói, thành viên nhà phát hành, người điều phối, hoặc
...
Điểm cuối người điều phối/quản trị viên để giải quyết hoặc mở lại báo cáo gói.
...
Điểm cuối người điều phối/quản trị viên để rà soát bản phát hành gói.
...
- Kho lưu trữ Plugin/gói là tệp zip với gốc `package/` để các client OpenClaw cũ
  tiếp tục hoạt động.
- Tuyến này chỉ giữ ZIP. Nó không stream tệp ClawPack `.tgz`.
...
- `dist.tarball`, `dist.integrity`, và `dist.shasum` dùng các trường tương thích npm
  để người dùng có thể trỏ npm tới bản mirror nếu họ chọn.
...
Skills hiện tại dựa trên GitHub có lượt quét `clean` hoặc `suspicious` và không có
...
- Chuyển giao Skills dựa trên GitHub không proxy hoặc mirror byte. Phản hồi JSON
  ...
  payload thành công.
- Thống kê tải xuống được tính là danh tính duy nhất theo ngày UTC (`userId` khi token API hợp lệ, nếu không thì IP).
...
- Trường payload tùy chọn: `migrateOwner`. Khi là `true` cùng với `ownerHandle`, một
  Skills hiện có có thể chuyển sang chủ sở hữu đó nếu tác nhân là quản trị viên/chủ sở hữu trên cả
  nhà phát hành hiện tại và nhà phát hành đích. Nếu không chọn tùy chọn này, thay đổi chủ sở hữu sẽ
  bị từ chối.
...
- Các trường form được phép là `payload`, blob `files` lặp lại, hoặc một tham chiếu tarball `clawpack`.
  ...
  luồng upload-url. Các lượt phát hành storage-id đã dàn dựng cũng phải bao gồm
...
- Chỉ nhà phát hành tổ chức `openclaw` và nhà phát hành cá nhân của các thành viên tổ chức `openclaw` hiện tại
  mới có thể phát hành lên kênh `official`.
- Phát hành thay mặt người khác vẫn xác thực điều kiện đủ cho kênh official dựa trên tài khoản chủ sở hữu đích.

### `DELETE /api/v1/skills/{slug}` / `POST /api/v1/skills/{slug}/undelete`

Xóa mềm / khôi phục một Skills (chủ sở hữu, người điều phối, hoặc quản trị viên).
...
Khi có, `reason` được lưu dưới dạng ghi chú điều phối Skills và được sao chép vào nhật ký kiểm toán.
Các lượt xóa mềm do chủ sở hữu khởi tạo sẽ giữ trước slug trong 30 ngày, sau đó slug có thể được
nhà phát hành khác nhận. Phản hồi xóa bao gồm `slugReservedUntil` khi thời hạn này áp dụng.
Ẩn bởi người điều phối/quản trị viên và gỡ bỏ vì lý do bảo mật không hết hạn theo cách này.
...
người dùng chia sẻ cũ/nhà phát hành cá nhân, điểm cuối sẽ di chuyển nó thành nhà phát hành tổ chức trước.
...
bên gọi làm chủ sở hữu. Điểm cuối này không di chuyển handle người dùng/cá nhân hiện có và
không đánh dấu nhà phát hành là được tin cậy/official.
...
Chỉ dành cho quản trị viên. Khôi phục một nhà phát hành cá nhân cho một principal GitHub OAuth thay thế đã xác minh
mà không chỉnh sửa các hàng tài khoản Convex Auth. Yêu cầu phải nêu cả hai id tài khoản
nhà cung cấp GitHub bất biến; các handle có thể thay đổi chỉ được dùng như một biện pháp bảo vệ dành cho người vận hành.

Điểm cuối mặc định là dry-run. Việc áp dụng khôi phục yêu cầu `dryRun: false` và
`confirmIdentityVerified: true` sau khi nhân viên xác minh độc lập tính liên tục giữa cả hai
GitHub principal. Khôi phục thất bại theo hướng đóng an toàn khi publisher cá nhân hiện tại của người dùng đích
có skills, gói hoặc nguồn skill GitHub.
Khôi phục cũng di chuyển các trường `ownerUserId` cũ cho skills của publisher được khôi phục,
bí danh slug skill, gói, cảnh báo trình kiểm tra gói và các hàng digest tìm kiếm phát sinh để
các đường dẫn chủ sở hữu trực tiếp khớp với quyền publisher mới. Một đặt trước protected-handle
đang hoạt động cho handle được khôi phục cũng được gán lại cho người dùng thay thế để lần
đồng bộ hóa hồ sơ sau này không thể khôi phục quyền cạnh tranh của người dùng cũ. Mỗi bảng chính được giới hạn ở
100 hàng cho mỗi giao dịch áp dụng; các lần khôi phục lớn hơn trước tiên phải dùng migration chủ sở hữu có thể tiếp tục.
Nguồn skill GitHub nằm trong phạm vi publisher và được báo cáo là đã kiểm tra thay vì được ghi lại.
...
- Cả hai điểm cuối đều yêu cầu xác thực bằng API token và chỉ hoạt động cho chủ sở hữu skill.
...
Cấm một người dùng và hard-delete skills thuộc sở hữu của họ (chỉ moderator/admin).
...
Thêm/xóa một star (điểm nổi bật). Cả hai điểm cuối đều có tính idempotent.
...
đưa một tarball ClawPack vào staging phải gửi id lưu trữ thu được dưới dạng
`clawpack` và ticket được trả về dưới dạng `clawpackUploadTicket`.

## Khám phá registry (`/.well-known/clawhub.json`)

CLI có thể khám phá cài đặt registry/auth từ trang web:
...
Nếu bạn self-host, hãy phục vụ tệp này (hoặc đặt rõ `CLAWHUB_REGISTRY`; `CLAWDHUB_REGISTRY` cũ).
