...
x-i18n:
    generated_at: "2026-07-02T17:38:53Z"
    model: gpt-5.5
...
Các thư mục bên thứ ba có thể dùng các endpoint đọc công khai để liệt kê hoặc tìm kiếm Skills trên ClawHub. Vui lòng lưu cache kết quả, tuân thủ `429`/`Retry-After`, liên kết người dùng trở lại mục ClawHub chuẩn (`https://clawhub.ai/<owner>/skills/<slug>`), và tránh ngụ ý rằng ClawHub xác nhận trang bên thứ ba đó. Không cố sao chép nội dung bị ẩn, riêng tư, hoặc bị chặn kiểm duyệt ra ngoài bề mặt API công khai.

Các lối tắt slug web phân giải trên nhiều họ registry, nhưng client API nên dùng
các URL chuẩn được endpoint đọc trả về thay vì tự dựng lại thứ tự ưu tiên
của route.
...
- Các endpoint ghi đã xác thực không nên trả về một `Unauthorized` trần khi
  ...
  tài khoản đã xóa/bị cấm/bị vô hiệu hóa đều nên nhận được nội dung có thể hành động để CLI
...
- ClawHub dùng header chuyển tiếp đáng tin cậy để nhận diện IP client tại biên.
...
- Kết quả được trả về theo thứ tự độ liên quan (độ tương đồng embedding + tăng điểm token slug/tên khớp chính xác + một mức ưu tiên phổ biến nhỏ).
- Độ liên quan mạnh hơn độ phổ biến. Một slug chính xác hoặc token tên hiển thị khớp có thể xếp cao hơn một kết quả khớp lỏng hơn nhưng có mức tương tác mạnh hơn nhiều.
...
- Độ phổ biến được scale log và giới hạn trần. Skills có mức tương tác cao có thể xếp thấp hơn khi văn bản truy vấn khớp yếu hơn.
...
- Đặt các thuật ngữ mà người dùng sẽ tìm kiếm đúng nghĩa trong tên hiển thị, tóm tắt, và thẻ. Chỉ dùng một token slug độc lập khi đó cũng là danh tính ổn định mà bạn muốn giữ.
...
- Bí danh đổi tên giữ khả năng phân giải cho URL cũ và các bản cài đặt phân giải qua registry, nhưng xếp hạng tìm kiếm dựa trên metadata Skill chuẩn sau khi việc đổi tên đã được lập chỉ mục. Thống kê hiện có vẫn đi theo Skill.
...
- Khi `nonSuspiciousOnly=true`, các cách sắp xếp dựa trên con trỏ có thể trả về ít hơn `limit` mục trên một trang vì Skills đáng ngờ được lọc sau khi lấy trang.
...
- `moderation` chỉ được bao gồm khi Skill bị gắn cờ hoặc owner đang xem.
...
- `security.hasScanResult` chỉ là `true` khi một trình quét đưa ra kết luận chắc chắn (`clean`, `suspicious`, hoặc `malicious`).
...
- Tác vụ quét là bất đồng bộ. Các yêu cầu quét thủ công được ưu tiên trước công việc xuất bản/điền lại thông thường, nhưng việc hoàn tất vẫn phụ thuộc vào mức sẵn sàng của worker.
...
- Trả về `queue.queuedAhead` và `queue.position` khi đang xếp hàng để máy khách có thể hiển thị có bao nhiêu lượt quét thủ công ưu tiên đang ở trước yêu cầu. Các hàng đợi rất lớn được giới hạn và báo cáo với `queuedAheadIsEstimate: true`.
...
- `ok` chỉ là `true` khi phiên bản được chọn có Skill Card đã tạo, không bị kiểm duyệt chặn do phần mềm độc hại, và xác minh ClawScan là sạch.
- Danh tính kỹ năng, danh tính nhà xuất bản, và siêu dữ liệu phiên bản được chọn là các trường phong bì cấp cao nhất (`slug`, `displayName`, `publisherHandle`, `version`, `resolvedFrom`, `tag`, `createdAt`) để tự động hóa shell có thể đọc chúng mà không cần mở gói các wrapper lồng nhau.
...
Trả về các kết luận bảo mật nhỏ gọn hiện tại cho đúng các phiên bản kỹ năng. Điểm cuối
bộ sưu tập này dành cho máy khách đã biết những phiên bản kỹ năng
ClawHub đã cài đặt nào cần hiển thị, chẳng hạn OpenClaw Control UI.
...
- `security.signals` chỉ chứa bằng chứng hỗ trợ ở mức trạng thái; dùng `/scan` hoặc trang kiểm toán bảo mật ClawHub để xem đầy đủ chi tiết trình quét.
- `security.signals.dependencyRegistry` được giữ lại để tương thích phản hồi v1, nhưng trình quét sự tồn tại của sổ đăng ký phụ thuộc đã bị ngừng dùng và khóa này luôn là `null`.
- Việc thiếu Skill Card không ảnh hưởng đến `ok`, `decision`, hoặc `reasons` của điểm cuối này; máy khách nên đọc `skill-card.md` đã cài đặt cục bộ khi cần nội dung thẻ.
...
- Skill được lưu trữ bao gồm các tệp phiên bản đã lưu mới nhất và được liệt kê trong
  `_manifest.json` với `sourceRef: "public-clawhub"`.
- Các skill hiện tại được hỗ trợ bởi GitHub có kết quả quét `clean` hoặc `suspicious` bao gồm
  `_source_handoff.json` với `sourceRef: "public-github"`, repo, commit, path,
  hash nội dung và URL kho lưu trữ. Chúng không bao gồm các tệp nguồn được lưu trữ trên ClawHub.
- Mỗi skill bao gồm `_export_skill_meta.json`.
- `_manifest.json` luôn được bao gồm tại gốc ZIP.
- `_errors.json` được bao gồm khi từng skill hoặc tệp riêng lẻ không thể được
  xuất.
...
Trả về đúng phần tóm tắt bảo mật và độ tin cậy của bản phát hành gói cho các
...
- Endpoint đọc công khai. Không yêu cầu token chủ sở hữu, nhà xuất bản, điều phối viên, hoặc quản trị viên.
...
- `trust.stale` nghĩa là phần tóm tắt tin cậy được tính từ các đầu vào đã lỗi thời và
...
- Các phiên bản gói cũ trả về artifact `legacy-zip` và `downloadUrl` ZIP
  cũ.
- Các phiên bản ClawPack trả về artifact `npm-pack`, các trường tính toàn vẹn npm, một
...
- tóm tắt artifact
- repo nguồn và nguồn gốc commit
...
Điểm cuối người kiểm duyệt/quản trị viên để tiếp nhận báo cáo gói.

Xác thực:

- Yêu cầu token API cho người dùng là người kiểm duyệt hoặc quản trị viên.
...
Điểm cuối chủ sở hữu/người kiểm duyệt để hiển thị thông tin kiểm duyệt gói.

Xác thực:

- Yêu cầu token API cho chủ sở hữu gói, thành viên nhà phát hành, người kiểm duyệt, hoặc
...
Điểm cuối người kiểm duyệt/quản trị viên để giải quyết hoặc mở lại báo cáo gói.
...
Điểm cuối người kiểm duyệt/quản trị viên để xem xét bản phát hành gói.
...
- Kho lưu trữ Plugin/gói là tệp zip với gốc `package/` để các máy khách OpenClaw cũ
  tiếp tục hoạt động.
- Tuyến này chỉ giữ định dạng ZIP. Nó không truyền trực tiếp tệp ClawPack `.tgz`.
...
- Packument gói có phạm vi hỗ trợ cả `/api/npm/@scope/name` và đường dẫn yêu cầu
  đã mã hóa của npm là `/api/npm/@scope%2Fname`.

### `GET /api/npm/{package}/-/{tarball}.tgz`

Truyền trực tiếp đúng từng byte tarball ClawPack đã tải lên cho máy khách bản sao npm.

Ghi chú:

- Sử dụng nhóm giới hạn tốc độ tải xuống.
- Header tải xuống bao gồm SHA-256 ClawHub cùng với siêu dữ liệu integrity/shasum của npm.
...
skill hiện tại dựa trên GitHub có lần quét `clean` hoặc `suspicious` và không có
...
- Bàn giao skill dựa trên GitHub không proxy hoặc sao chép byte. Phản hồi JSON
  bao gồm `sourceRef: "public-github"`, `repo`, `commit`, `path`, `contentHash`,
  và `archiveUrl`; trạng thái quét/hiện tại là cổng kiểm soát và không được đưa vào dưới dạng
  siêu dữ liệu payload thành công.
- Thống kê tải xuống được tính theo định danh duy nhất mỗi ngày UTC (`userId` khi token API hợp lệ, nếu không thì IP).
...
- Trường payload tùy chọn: `migrateOwner`. Khi là `true` với `ownerHandle`, một
  ...
  nhà phát hành hiện tại và nhà phát hành đích. Nếu không chọn tham gia, thay đổi chủ sở hữu sẽ
  bị từ chối.
...
- Các trường biểu mẫu được phép là `payload`, blob `files` lặp lại, hoặc một tham chiếu tarball
  `clawpack`. `clawpack` có thể là blob `.tgz` hoặc id lưu trữ được trả về bởi
  luồng upload-url. Các lần xuất bản storage-id đã staged cũng phải bao gồm
...
- Yêu cầu xuất bản multipart trực tiếp bị giới hạn ở 18MB. Tarball ClawPack có thể
  sử dụng luồng upload-url đến giới hạn tarball 120MB.
...
- Chỉ nhà phát hành tổ chức `openclaw` và nhà phát hành cá nhân của các thành viên tổ chức
  `openclaw` hiện tại mới có thể xuất bản lên kênh `official`.
- Xuất bản thay mặt vẫn xác thực điều kiện đủ cho kênh chính thức dựa trên tài khoản chủ sở hữu đích.
...
Xóa mềm do chủ sở hữu khởi tạo sẽ giữ slug trong 30 ngày, sau đó slug có thể được
nhà phát hành khác nhận.
Phản hồi xóa bao gồm `slugReservedUntil` khi thời hạn này được áp dụng.
Ẩn bởi người kiểm duyệt/quản trị viên và gỡ bỏ vì bảo mật không hết hạn theo cách này.
...
Chỉ dành cho quản trị viên. Đảm bảo một nhà phát hành tổ chức tồn tại cho một handle. Nếu handle vẫn trỏ đến một
người dùng/nhà phát hành cá nhân dùng chung kiểu cũ, điểm cuối sẽ di chuyển nó thành nhà phát hành tổ chức trước.
...
Chỉ dành cho quản trị viên. Khôi phục nhà phát hành cá nhân cho một principal GitHub OAuth thay thế đã xác minh
mà không chỉnh sửa các hàng tài khoản Convex Auth. Yêu cầu phải nêu cả hai id tài khoản
nhà cung cấp GitHub bất biến; handle có thể thay đổi chỉ được dùng làm chốt kiểm tra hướng tới người vận hành.
...
danh tính chính GitHub. Quá trình khôi phục sẽ thất bại theo hướng đóng khi publisher cá nhân hiện tại của người dùng đích
...
cho handle được khôi phục cũng được gán lại cho người dùng thay thế để lần đồng bộ hồ sơ sau
không thể khôi phục thẩm quyền cạnh tranh của người dùng trước đó. Mỗi bảng chính được giới hạn ở
100 hàng cho mỗi giao dịch áp dụng; các khôi phục lớn hơn trước tiên phải dùng một quy trình di chuyển chủ sở hữu có thể tiếp tục.
...
Cấm một người dùng và xóa cứng các skills thuộc sở hữu (chỉ moderator/admin).
...
`POST /api/cli/upload-url` trả về `uploadUrl` và `uploadTicket`. Các lượt phát hành package
đưa một tarball ClawPack vào giai đoạn phải gửi id lưu trữ kết quả dưới dạng
...
Nếu bạn tự lưu trữ, hãy phục vụ tệp này (hoặc đặt rõ ràng `CLAWHUB_REGISTRY`; `CLAWDHUB_REGISTRY` cũ).
