...
ClawHub sử dụng **telemetry tối thiểu** để tính **số lượt cài đặt** (những gì thực sự đang được dùng) và hỗ trợ sắp xếp/lọc tốt hơn.
...
Với mỗi skill tìm thấy dưới một gốc, chúng tôi lưu:
...
- Nếu một skill biến mất khỏi một gốc trong lần sync tiếp theo, chúng tôi đánh dấu skill đó là đã gỡ bỏ cho gốc đó.
- Nếu skill bị gỡ bỏ khỏi tất cả các gốc của bạn, nó sẽ không còn được tính vào `installsCurrent`.
- `installsAllTime` không bao giờ giảm trừ khi bạn xóa telemetry (xem bên dưới).

### Trạng thái cũ (120 ngày)

Các gốc không báo cáo telemetry trong **120 ngày** sẽ được đánh dấu là cũ và các lượt cài đặt của chúng sẽ ngừng được tính vào `installsCurrent`.
Việc này được đánh giá lười (ở báo cáo telemetry tiếp theo) để tránh các tác vụ nền.
...
- Bao gồm hành động **Xóa telemetry** để xóa toàn bộ telemetry đã lưu cho tài khoản của bạn.

Những người khác chỉ thấy **bộ đếm lượt cài đặt tổng hợp**; không ai khác có thể thấy các gốc/thư mục của bạn.

Xóa tài khoản của bạn cũng xóa dữ liệu telemetry của bạn.
...
Khi thiết lập như vậy, CLI sẽ không gửi telemetry trong lúc chạy `clawhub sync`.
