...
x-i18n:
    generated_at: "2026-05-11T20:25:10Z"
    model: gpt-5.5
    provider: openai
...
- `removedAt` tùy chọn khi một bản cài đặt từng được báo cáo biến mất khỏi một gốc.
...
Nếu bạn sync từ nhiều thư mục, chúng tôi xử lý từng gốc quét một cách độc lập. Một skill được xem là “hiện đang cài đặt” nếu nó tồn tại trong **bất kỳ** gốc đang hoạt động nào.

### Phát hiện gỡ cài đặt

Vì `sync` báo cáo toàn bộ tập hợp theo từng gốc:

- Nếu một skill biến mất khỏi một gốc trong lần sync tiếp theo, chúng tôi đánh dấu nó là đã bị gỡ cho gốc đó.
...
### Trạng thái lỗi thời (120 ngày)

Các gốc không báo cáo dữ liệu đo từ xa trong **120 ngày** sẽ được đánh dấu là lỗi thời và các bản cài đặt của chúng ngừng được tính vào `installsCurrent`.
...
Khi đặt biến này, CLI sẽ không gửi dữ liệu đo từ xa trong lúc chạy `clawhub sync`.
