...
summary: Cách hoạt động của báo cáo ClawHub, các lượt giữ để kiểm duyệt, danh sách bị ẩn, lệnh cấm và trạng thái tài khoản.
...
[Cách sử dụng được chấp nhận](/vi/clawhub/acceptable-usage). Với các mối quan ngại
...
ClawHub cho lỗ hổng trong skill hoặc plugin bên thứ ba.

Báo cáo tốt phải cụ thể và có thể hành động. Việc lạm dụng báo cáo tự nó cũng có
thể dẫn đến hành động tài khoản.

## Yêu cầu về tổ chức và namespace

Tranh chấp quyền sở hữu tổ chức, thương hiệu, phạm vi gói, định danh chủ sở hữu
hoặc namespace nên dùng quy trình [Yêu cầu về tổ chức và namespace](/vi/clawhub/namespace-claims),
...
riêng tư, giấy tờ định danh cá nhân, token API hoặc token thử thách DNS vào issue công khai.

## Giữ kiểm duyệt

Một số phát hiện nghiêm trọng hoặc vấn đề chính sách có thể đặt nhà xuất bản hoặc
danh sách vào trạng thái giữ kiểm duyệt. Khi điều này xảy ra, nội dung bị ảnh hưởng
...
Giữ kiểm duyệt nhằm bảo vệ người dùng trong khi ClawHub giải quyết các trường hợp
rủi ro cao. Chúng cũng có thể được gỡ bỏ khi xác nhận đó là dương tính giả.

## Danh sách bị ẩn hoặc bị chặn

Một danh sách có thể bị giữ, ẩn, cách ly, thu hồi hoặc không khả dụng theo cách khác
trên các bề mặt cài đặt công khai.
...
Chủ sở hữu vẫn có thể thấy chẩn đoán cho danh sách của chính họ đang bị giữ hoặc bị ẩn.
...
ngưỡng có khả năng cấm của ClawHub có thể kích hoạt cảnh báo tự động. Nếu lần quét đủ điều kiện
tiếp theo sau hạn cảnh báo vẫn đặt nhà xuất bản trong ngưỡng có khả năng cấm,
ClawHub có thể tự động áp dụng hành động tài khoản.
Các tín hiệu đánh giá theo thời gian có độ tin cậy thấp hơn và có giới hạn sẽ không thuộc phạm vi
thực thi tự động.

Tài khoản đã xóa, bị cấm hoặc bị vô hiệu hóa không thể dùng token API ClawHub. Nếu auth CLI
bắt đầu thất bại sau hành động tài khoản, hãy đăng nhập vào web UI để xem trạng thái tài khoản.
Nếu việc đăng nhập hoặc quyền truy cập CLI thông thường bị chặn do lệnh cấm hoặc tài khoản bị vô hiệu hóa,
hãy dùng [biểu mẫu kháng nghị ClawHub](https://appeals.openclaw.ai/) để được xem xét khôi phục.

Nếu email do bộ quét kích hoạt nêu một phiên bản skill hoặc plugin là độc hại,
