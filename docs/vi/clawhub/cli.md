...
summary: 'Tài liệu tham chiếu CLI: lệnh, cờ, cấu hình, tệp khóa, hành vi đồng bộ.'
x-i18n:
...
- `--workdir <dir>`: thư mục làm việc (mặc định: cwd; dùng dự phòng không gian làm việc Clawdbot nếu đã cấu hình)
...
(ví dụ: vùng chứa Docker, Hetzner VPS với internet chỉ qua proxy, tường lửa
doanh nghiệp).
...
- Dự phòng cũ: nếu `clawhub/config.json` chưa tồn tại nhưng `clawdhub/config.json` tồn tại, CLI dùng lại đường dẫn cũ
...
- Tương tác từ xa/headless: `clawhub login --device` in ra một mã và chờ trong khi bạn cấp quyền tại `<site>/cli/device`.
...
- Lượt tải xuống chỉ là một tín hiệu phổ biến nhỏ trước đó, không phải bảo đảm về vị trí hàng đầu.
- Nếu một kỹ năng đáng lẽ xuất hiện nhưng không xuất hiện, hãy chạy `clawhub inspect <slug>` khi đã đăng nhập để kiểm tra chẩn đoán kiểm duyệt mà chủ sở hữu có thể thấy trước khi đổi tên siêu dữ liệu.
...
- `--version <version>`: kiểm tra một phiên bản cụ thể (mặc định: latest).
...
- Từ chối ghi đè kỹ năng đã ghim; hãy chạy `clawhub unpin <slug>` trước.
- Ghi:
...
- Các Skills đã ghim được bỏ qua bởi `update --all` và bị từ chối bởi `update <slug>` trực tiếp.
- Các Skills đã ghim cũng từ chối `install --force` để các byte cục bộ không thể bị thay thế ngoài ý muốn.
...
- `update --all` bỏ qua các slug đã ghim và in bản tóm tắt về những gì vẫn bị đóng băng.

### `skill publish <path>`

- Phát hành qua `POST /api/v1/skills` (multipart).
...
- Các Skills đã phát hành được tự do sử dụng, sửa đổi và phân phối lại mà không cần ghi công.
...
- `--clawscan-note <text>` thêm một ghi chú ClawScan. Ghi chú này cung cấp cho ClawScan
  ngữ cảnh cho hành vi có thể trông bất thường, chẳng hạn như truy cập mạng,
  truy cập máy chủ gốc, hoặc thông tin xác thực dành riêng cho nhà cung cấp. Ghi chú được lưu trên
...
- Xóa mềm một Skill (chủ sở hữu, người kiểm duyệt, hoặc quản trị viên).
- Gọi `DELETE /api/v1/skills/{slug}`.
- Xóa mềm do chủ sở hữu khởi tạo sẽ giữ slug trong 30 ngày; lệnh in thời điểm hết hạn.
- `--reason <text>` ghi lại một ghi chú kiểm duyệt trên Skill và nhật ký kiểm toán.
...
- Chuyển đến handle tổ chức/nhà phát hành chỉ áp dụng ngay lập tức khi tác nhân có
  quyền truy cập quản trị viên vào cả chủ sở hữu hiện tại và nhà phát hành đích.
- Lệnh con:
...
- Dùng mục này cho plugins và các mục họ gói khác; `search` cấp cao nhất vẫn là bề mặt tìm kiếm Skill.
- Cờ:
...
- Dùng mục này cho metadata plugin, khả năng tương thích, xác minh, nguồn, và kiểm tra phiên bản/tệp.
...
- Các phiên bản ZIP kế thừa tải xuống thông qua tuyến ZIP kế thừa.
- Cờ:
...
- Với `--package`, phân giải metadata dự kiến từ ClawHub và so sánh
  tệp cục bộ với metadata artifact đã phát hành.
- Với các cờ digest trực tiếp, xác minh mà không cần tra cứu mạng.
- Cờ:
  - `--package <name>`: tên gói để phân giải metadata artifact dự kiến.
...
- Cờ:
  - `--to <owner>`: định danh nhà phát hành đích.
...
- Báo cáo các điểm chặn đối với trạng thái chính thức, tính sẵn có của ClawPack, mã băm artifact,
...
- Gọi cùng endpoint sẵn sàng được tính toán như `package readiness`, nhưng in ra
...
- Nguồn `.tgz` được xử lý như ClawPack. CLI tải lên đúng byte npm-pack
...
- Thư mục Plugin mã được đóng gói thành tarball npm ClawPack trước khi tải lên để
  bản cài đặt OpenClaw có thể xác minh artifact chính xác. Thư mục Plugin gói vẫn
  dùng đường dẫn phát hành tệp đã trích xuất.
- Với nguồn GitHub, nguồn gốc mã nguồn được tự động điền từ kho, commit đã phân giải, ref và đường dẫn con.
- Với thư mục cục bộ, nguồn gốc mã nguồn được tự động phát hiện từ git cục bộ khi remote origin trỏ tới GitHub.
...
Đối với Plugin mã, phát hành từ thư mục sẽ xây dựng và tải lên artifact ClawPack từ
thư mục gói:
...
- Nếu bạn đang dùng bản phát hành CLI `clawhub` cũ hơn, hãy nâng cấp trước khi phát hành để
  các kiểm tra chuẩn bị cục bộ chạy trước khi tải lên.
...
- Workflow tái sử dụng mặc định `source` là kho gọi.
- Với monorepo, truyền `source_path` để workflow phát hành thư mục gói
  Plugin, ví dụ `source_path: extensions/codex`.
- Ghim workflow tái sử dụng vào một thẻ ổn định hoặc SHA commit đầy đủ. Không chạy phát hành bản phát hành từ `@main`.
- `pull_request` nên dùng `dry_run: true` để CI không tạo tác động bên ngoài.
- Phát hành thật nên được giới hạn ở các sự kiện tin cậy như `workflow_dispatch` hoặc push thẻ.
- Phát hành tin cậy không cần secret chỉ hoạt động trên `workflow_dispatch`; push thẻ vẫn cần `clawhub_token`.
...
- Quét các thư mục Skills cục bộ và phát hành những thư mục mới/đã thay đổi.
- Gốc có thể là bất kỳ thư mục nào: một thư mục Skills hoặc một thư mục Skills đơn lẻ có `SKILL.md`.
- Tự động thêm gốc Skills Clawdbot khi có `~/.clawdbot/clawdbot.json`:
