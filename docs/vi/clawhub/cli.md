...
(ví dụ: vùng chứa Docker, VPS Hetzner có internet chỉ qua proxy, tường lửa
doanh nghiệp).
...
- Phương án dự phòng cũ: nếu `clawhub/config.json` chưa tồn tại nhưng `clawdhub/config.json` có tồn tại, CLI sẽ dùng lại đường dẫn cũ
...
- Không có giao diện: `clawhub login --token clh_...`
- Tương tác từ xa/không có giao diện: `clawhub login --device` in ra một mã và chờ trong khi bạn cấp quyền tại `<site>/cli/device`.
...
- Lượt tải xuống là một tín hiệu ưu tiên nhỏ về mức độ phổ biến, không phải bảo đảm cho vị trí hàng đầu.
- Nếu một kỹ năng lẽ ra phải xuất hiện nhưng không xuất hiện, hãy chạy `clawhub inspect <slug>` khi đã đăng nhập để kiểm tra chẩn đoán kiểm duyệt mà chủ sở hữu có thể thấy trước khi đổi tên siêu dữ liệu.
...
- Đầu ra: `<slug>  v<version>  <age>  <summary>` (summary được rút gọn còn 50 ký tự).
...
- Tải zip xuống qua `/api/v1/download`.
- Giải nén vào `<workdir>/<dir>/<slug>`.
- Từ chối ghi đè kỹ năng đã ghim; chạy `clawhub unpin <slug>` trước.
- Ghi:
...
- Các skill đã ghim sẽ bị `update --all` bỏ qua và bị từ chối khi chạy trực tiếp `update <slug>`.
- Các skill đã ghim cũng từ chối `install --force` để byte cục bộ không thể bị thay thế ngoài ý muốn.
...
- `--owner <handle>` phát hành dưới một handle nhà phát hành của tổ chức/người dùng khi
...
- Các skill đã phát hành được tự do sử dụng, sửa đổi và phân phối lại mà không cần ghi công.
...
- Các lệnh xóa mềm do owner khởi tạo sẽ giữ trước slug trong 30 ngày; lệnh in thời điểm hết hạn.
...
- Slug nguồn ngừng hiển thị công khai và trở thành bí danh chuyển hướng tới đích.
...
- Chuyển tới handle người dùng sẽ tạo một yêu cầu đang chờ mà người nhận chấp nhận.
...
- Dùng lệnh này cho plugin và các mục thuộc họ package khác; `search` cấp cao nhất vẫn là bề mặt tìm kiếm skill.
- Cờ:
...
- `--version <version>`: kiểm tra một phiên bản cụ thể (mặc định: mới nhất).
- `--tag <tag>`: kiểm tra một phiên bản đã gắn tag (ví dụ `latest`).
- `--versions`: liệt kê lịch sử phiên bản (trang đầu tiên).
...
- Phân giải phiên bản package qua
...
- Cờ:
  ...
  - `--tag <tag>`: tải xuống một phiên bản đã gắn tag (mặc định: `latest`).
...
- Yêu cầu chủ sở hữu gói, chủ sở hữu/quản trị viên nhà phát hành của tổ chức, kiểm duyệt viên nền tảng,
...
- Lệnh đã xác thực để báo cáo một gói cho kiểm duyệt viên.
- Gọi `POST /api/v1/packages/{name}/report`.
- Báo cáo ở cấp gói, có thể gắn tùy chọn với một phiên bản, và sẽ hiển thị
  cho kiểm duyệt viên để xem xét.
...
- Hiển thị trạng thái quét gói hiện tại, số lượng báo cáo đang mở, trạng thái
  kiểm duyệt thủ công của bản phát hành mới nhất, trạng thái chặn tải xuống và lý do kiểm duyệt.
- Cờ:
  - `--json`: đầu ra máy đọc được.
...
- Báo cáo các yếu tố chặn trạng thái chính thức, khả năng sẵn có của ClawPack, digest artifact,
  nguồn gốc mã nguồn, tính tương thích OpenClaw, mục tiêu máy chủ, siêu dữ liệu môi trường,
  và trạng thái quét.
- Cờ:
  - `--json`: đầu ra máy đọc được.
...
- Với nguồn GitHub, thông tin quy thuộc nguồn được tự động điền từ repo, commit đã phân giải, ref và đường dẫn con.
- Với thư mục cục bộ, thông tin quy thuộc nguồn được tự động phát hiện từ git cục bộ khi remote origin trỏ đến GitHub.
...
- `--owner <handle>` xuất bản dưới định danh nhà phát hành người dùng hoặc tổ chức khi tác nhân có quyền truy cập nhà phát hành.
- `--clawscan-note <text>` thêm ghi chú ClawScan. Ghi chú này cung cấp cho ClawScan
  ngữ cảnh về hành vi có thể trông bất thường, chẳng hạn như truy cập mạng,
  truy cập máy chủ native, hoặc thông tin xác thực dành riêng cho provider. Ghi chú được lưu trên
...
thông tin quy thuộc nguồn trước khi tạo bản phát hành thực:
...
- `package.json.version` là phiên bản phát hành gói của bạn, nhưng không được dùng làm
  fallback cho xác thực tương thích/xây dựng OpenClaw.
- `openclaw.hostTargets` và `openclaw.environment` là siêu dữ liệu tùy chọn.
  ClawHub có thể hiển thị chúng khi có mặt, nhưng chúng không bắt buộc để xuất bản.
...
- Ghim workflow tái sử dụng vào tag ổn định hoặc SHA commit đầy đủ. Không chạy xuất bản bản phát hành từ `@main`.
- `pull_request` nên dùng `dry_run: true` để CI không gây ô nhiễm.
- Xuất bản thực nên được giới hạn ở các sự kiện đáng tin cậy như `workflow_dispatch` hoặc push tag.
...
- Quét các thư mục skill cục bộ và xuất bản những mục mới/đã thay đổi.
- Gốc có thể là bất kỳ thư mục nào: thư mục skills hoặc một thư mục skill đơn lẻ có `SKILL.md`.
