...
cho HTTP thuần. `NO_PROXY` / `no_proxy` được tuân thủ để bỏ qua proxy cho
...
- Tương tác từ xa/không giao diện: `clawhub login --device` in ra mã và chờ trong khi bạn cấp quyền tại `<site>/cli/device`.
...
- Lượt tải xuống là tín hiệu độ phổ biến nhỏ, không phải bảo đảm cho vị trí hàng đầu.
- Nếu một skill nên xuất hiện nhưng không xuất hiện, hãy chạy `clawhub inspect <slug>` khi đã đăng nhập để kiểm tra chẩn đoán kiểm duyệt hiển thị với chủ sở hữu trước khi đổi tên siêu dữ liệu.
...
- Lấy siêu dữ liệu skill và tệp phiên bản mà không cài đặt.
...
- Hiển thị `pinned` bên cạnh các Skills bị đóng băng bằng `clawhub pin`, bao gồm cả lý do tùy chọn.

### `pin <slug>`

- Đánh dấu một Skills đã cài đặt là đã ghim trong lockfile.
- `--reason <text>` ghi lại lý do Skills bị đóng băng.
- Các Skills đã ghim sẽ được `update --all` bỏ qua và bị từ chối khi chạy trực tiếp `update <slug>`.
- Các Skills đã ghim cũng từ chối `install --force` để byte cục bộ không bị thay thế ngoài ý muốn.

### `unpin <slug>`

- Gỡ ghim trong lockfile khỏi một Skills đã cài đặt để các bản cập nhật sau này có thể sửa đổi Skills đó.
...
- Các Skills đã ghim không bao giờ được cập nhật bởi `--force`.
- `update <slug>` thất bại nhanh đối với slug đã ghim và yêu cầu bạn chạy `clawhub unpin <slug>` trước.
...
- `--migrate-owner` chuyển một Skills hiện có sang `--owner` trong khi xuất bản một
...
- Xuất bản một Skills nghĩa là Skills đó được phát hành theo `MIT-0` trên ClawHub.
- Các Skills đã xuất bản được tự do sử dụng, sửa đổi và phân phối lại mà không cần ghi công.
- ClawHub không hỗ trợ Skills trả phí hoặc định giá theo từng Skills.
...
- Xóa mềm một Skills (owner, moderator, hoặc admin).
- Gọi `DELETE /api/v1/skills/{slug}`.
- Các lệnh xóa mềm do owner khởi tạo sẽ giữ slug trong 30 ngày; lệnh in thời điểm hết hạn.
- `--reason <text>` ghi một ghi chú kiểm duyệt trên Skills và audit log.
- `--note <text>` là bí danh cho `--reason`.
- `--yes` bỏ qua xác nhận.

### `undelete <slug>`

- Khôi phục một Skills bị ẩn (owner, moderator, hoặc admin).
...
- Ẩn một Skills (owner, moderator, hoặc admin).
- Bí danh cho `delete`.

### `unhide <slug>`

- Hiện lại một Skills (owner, moderator, hoặc admin).
- Bí danh cho `undelete`.

### `skill rename <slug> <new-slug>`

- Đổi tên một Skills do bạn sở hữu và giữ slug trước đó làm bí danh chuyển hướng.
- Gọi `POST /api/v1/skills/{slug}/rename`.
- `--yes` bỏ qua xác nhận.

### `skill merge <source-slug> <target-slug>`

- Hợp nhất một Skills do bạn sở hữu vào một Skills khác do bạn sở hữu.
...
- Chuyển đến handle người dùng sẽ tạo yêu cầu đang chờ để người nhận chấp nhận.
- Chuyển đến handle tổ chức/nhà xuất bản chỉ áp dụng ngay lập tức khi actor có
...
- Dùng lệnh này cho Plugin và các mục họ gói khác; `search` cấp cao nhất vẫn là bề mặt tìm kiếm Skills.
- Cờ:
...
- Lấy metadata của gói mà không cài đặt.
...
- Các phiên bản ZIP cũ tải qua route ZIP cũ.
- Cờ:
  - `--version <version>`: tải một phiên bản cụ thể.
  - `--tag <tag>`: tải một phiên bản được gắn thẻ (mặc định: `latest`).
...
- Cờ:
  ...
  - `--sha256 <hex>`: ClawHub SHA-256 mong đợi.
...
- Tên gói có phạm vi phải chuyển tới chủ sở hữu phạm vi tương ứng.
...
- Lệnh đã xác thực để báo cáo một gói tới điều phối viên.
...
- Lệnh dành cho chủ sở hữu để kiểm tra khả năng hiển thị kiểm duyệt gói.
...
- Báo cáo các yếu tố chặn trạng thái chính thức, tính khả dụng của ClawPack, digest của artifact,
...
- Phát hành một code plugin hoặc bundle plugin qua `POST /api/v1/packages`.
- `<source>` chấp nhận:
...
- Nguồn `.tgz` được xem là ClawPack. CLI tải lên đúng byte npm-pack
  và chỉ dùng nội dung `package/` được trích xuất để xác thực và điền trước
  siêu dữ liệu.
- Thư mục code-plugin được đóng gói thành một tarball npm ClawPack trước khi tải lên để
  bản cài đặt OpenClaw có thể xác minh đúng artifact. Thư mục bundle-plugin vẫn
...
- `--clawscan-note <text>` thêm ghi chú ClawScan. Ghi chú này cung cấp cho ClawScan
  ngữ cảnh về hành vi có thể trông bất thường, như truy cập mạng,
  truy cập host gốc, hoặc thông tin xác thực dành riêng cho nhà cung cấp. Ghi chú được lưu trên
...
ghi nhận nguồn trước khi tạo một bản phát hành thực:
...
Đối với code plugin, phát hành thư mục sẽ xây dựng và tải lên một artifact ClawPack từ
thư mục gói:
...
- `openclaw.hostTargets` và `openclaw.environment` là siêu dữ liệu tùy chọn.
  ClawHub có thể hiển thị chúng khi có mặt, nhưng chúng không bắt buộc để phát hành.
...
- Phát hành thực nên được giới hạn trong các sự kiện đáng tin cậy như `workflow_dispatch` hoặc push tag.
- Phát hành đáng tin cậy mà không có secret chỉ hoạt động trên `workflow_dispatch`; push tag vẫn cần `clawhub_token`.
- Giữ `clawhub_token` khả dụng cho lần phát hành đầu tiên, gói không đáng tin cậy, hoặc phát hành khẩn cấp.
- Workflow tải kết quả JSON lên dưới dạng artifact và xuất nó dưới dạng đầu ra workflow.

### `sync`

- Quét các thư mục skill cục bộ và phát hành mục mới/đã thay đổi.
- Gốc có thể là bất kỳ thư mục nào: một thư mục skills hoặc một thư mục skill duy nhất có `SKILL.md`.
- Tự động thêm các gốc skill Clawdbot khi có `~/.clawdbot/clawdbot.json`:
  - `agent.workspace/skills` (tác nhân chính)
  - `routing.agents.*.workspace/skills` (theo tác nhân)
...
- Được gửi trong `sync` khi đã đăng nhập, trừ khi `CLAWHUB_DISABLE_TELEMETRY=1` (`CLAWDHUB_DISABLE_TELEMETRY=1` kế thừa).
