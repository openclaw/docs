...
CLI tuân theo các biến môi trường HTTP proxy tiêu chuẩn cho các hệ thống nằm sau
...
Điều này là bắt buộc trên các hệ thống nơi kết nối đi trực tiếp bị chặn
...
- Phương án dự phòng cũ: nếu `clawhub/config.json` chưa tồn tại nhưng `clawdhub/config.json` tồn tại, CLI dùng lại đường dẫn cũ
...
- Mặc định: mở trình duyệt tới `<site>/cli/auth` và hoàn tất qua lệnh gọi lại local loopback.
- Headless: `clawhub login --token clh_...`
- Tương tác từ xa/headless: `clawhub login --device` in mã và chờ trong khi bạn ủy quyền tại `<site>/cli/device`.
...
- Lượt tải xuống là một tín hiệu độ phổ biến nhỏ ban đầu, không phải bảo đảm về vị trí hàng đầu.
- Nếu một skill đáng lẽ xuất hiện nhưng không xuất hiện, hãy chạy `clawhub inspect <slug>` khi đã đăng nhập để kiểm tra chẩn đoán kiểm duyệt hiển thị với chủ sở hữu trước khi đổi tên siêu dữ liệu.
...
- Lấy siêu dữ liệu skill và các tệp phiên bản mà không cài đặt.
...
- Từ chối ghi đè các skill đã ghim; trước tiên hãy chạy `clawhub unpin <slug>`.
- Ghi:
...
- Các skill đã ghim được bỏ qua bởi `update --all` và bị từ chối bởi `update <slug>` trực tiếp.
- Các skill đã ghim cũng từ chối `install --force` để các byte cục bộ không thể bị thay thế ngoài ý muốn.

### `unpin <slug>`

- Gỡ ghim lockfile khỏi một skill đã cài đặt để các bản cập nhật sau này có thể sửa đổi skill đó.
...
- Dùng lệnh này cho plugins và các mục họ gói khác; `search` cấp cao nhất vẫn là bề mặt tìm kiếm skill.
- Cờ:
...
- `--version <version>`: kiểm tra một phiên bản cụ thể (mặc định: mới nhất).
- `--tag <tag>`: kiểm tra một phiên bản có gắn thẻ (ví dụ `latest`).
- `--versions`: liệt kê lịch sử phiên bản (trang đầu tiên).
...
- Cờ:
  ...
  - `--tag <tag>`: tải xuống một phiên bản có gắn thẻ (mặc định: `latest`).
...
- Báo cáo các yếu tố chặn đối với trạng thái chính thức, tính khả dụng của ClawPack, digest tạo tác,
...
- Hiển thị trạng thái di chuyển dành cho người vận hành đối với một gói có thể thay thế một
...
- Xuất bản một plugin mã hoặc plugin gói qua `POST /api/v1/packages`.
- `<source>` chấp nhận:
  - Đường dẫn thư mục cục bộ: `./my-plugin`
  - Tarball ClawPack npm-pack cục bộ: `./my-plugin-1.2.3.tgz`
...
- Nguồn `.tgz` được xử lý như ClawPack. CLI tải lên đúng các byte npm-pack
  ...
  siêu dữ liệu.
- Thư mục plugin mã được đóng gói thành tarball ClawPack npm trước khi tải lên để
  các bản cài đặt OpenClaw có thể xác minh đúng tạo tác. Thư mục plugin gói vẫn
  dùng đường dẫn xuất bản tệp đã trích xuất.
- Với nguồn GitHub, thông tin ghi nhận nguồn được tự động điền từ repo, commit đã phân giải, ref và đường dẫn con.
...
- Việc xuất bản thật nên được giới hạn ở các sự kiện đáng tin cậy như `workflow_dispatch` hoặc push tag.
- Xuất bản đáng tin cậy không có secret chỉ hoạt động trên `workflow_dispatch`; push tag vẫn cần `clawhub_token`.
- Giữ `clawhub_token` sẵn có cho lần xuất bản đầu tiên, các gói không đáng tin cậy, hoặc xuất bản khẩn cấp.
- Workflow tải kết quả JSON lên dưới dạng tạo tác và hiển thị nó như đầu ra workflow.

### `sync`

- Quét các thư mục Skills cục bộ và xuất bản các mục mới/đã thay đổi.
...
- Được gửi trong khi chạy `sync` khi đã đăng nhập, trừ khi `CLAWHUB_DISABLE_TELEMETRY=1` (cũ `CLAWDHUB_DISABLE_TELEMETRY=1`).
