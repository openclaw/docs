...
summary: 'Tài liệu tham khảo CLI: lệnh, cờ, cấu hình, tệp khóa, hành vi đồng bộ.'
x-i18n:
...
- `--workdir <dir>`: thư mục làm việc (mặc định: cwd; chuyển về không gian làm việc Clawdbot nếu đã cấu hình)
- `--dir <dir>`: thư mục cài đặt bên dưới thư mục làm việc (mặc định: `skills`)
...
CLI tuân theo các biến môi trường proxy HTTP chuẩn cho các hệ thống nằm sau
...
(ví dụ: vùng chứa Docker, VPS Hetzner với internet chỉ qua proxy, tường lửa
doanh nghiệp).
...
- Gọi `/api/v1/search?q=...`.
- Tìm kiếm ưu tiên các kết quả khớp chính xác theo token slug/tên trước độ phổ biến tải xuống. Một token slug độc lập như `map` khớp với `personal-map` mạnh hơn chuỗi con bên trong `amap`.
- Số lượt tải xuống là một tín hiệu phổ biến nhỏ, không phải bảo đảm cho vị trí đầu.
- Nếu một kỹ năng nên xuất hiện nhưng không xuất hiện, hãy chạy `clawhub inspect <slug>` khi đã đăng nhập để kiểm tra chẩn đoán kiểm duyệt mà chủ sở hữu có thể thấy trước khi đổi tên metadata.
...
- Đánh dấu một skill đã cài đặt là pinned trong lockfile.
- `--reason <text>` ghi lại lý do skill bị đóng băng.
- Các skill pinned được `update --all` bỏ qua và bị từ chối bởi `update <slug>` trực tiếp.
- Các skill pinned cũng từ chối `install --force` để các byte cục bộ không thể bị thay thế ngoài ý muốn.

### `unpin <slug>`

- Xóa pin trong lockfile khỏi một skill đã cài đặt để các bản cập nhật trong tương lai có thể sửa đổi skill đó.
...
- Các skill pinned không bao giờ được cập nhật bởi `--force`.
- `update <slug>` thất bại nhanh với các slug pinned và yêu cầu bạn chạy `clawhub unpin <slug>` trước.
- `update --all` bỏ qua các slug pinned và in bản tóm tắt về những gì vẫn bị đóng băng.
...
- Soft-delete một skill (owner, moderator, hoặc admin).
- Gọi `DELETE /api/v1/skills/{slug}`.
- Các soft delete do owner khởi tạo sẽ giữ slug trong 30 ngày; lệnh in thời điểm hết hạn.
- `--reason <text>` ghi lại ghi chú kiểm duyệt trên skill và audit log.
- `--note <text>` là bí danh của `--reason`.
- `--yes` bỏ qua xác nhận.

### `undelete <slug>`

- Khôi phục một skill bị ẩn (owner, moderator, hoặc admin).
...
- Chuyển tới handle người dùng tạo một yêu cầu đang chờ để người nhận chấp nhận.
- Chuyển tới handle tổ chức/nhà xuất bản chỉ áp dụng ngay lập tức khi actor có
  quyền truy cập admin vào cả owner hiện tại và nhà xuất bản đích.
- Lệnh con:
...
- Duyệt hoặc tìm kiếm danh mục package thống nhất qua `GET /api/v1/packages` và `GET /api/v1/packages/search`.
...
- Cờ:
  - `--package <name>`: tên package để phân giải metadata artifact mong đợi.
  - `--version <version>` hoặc `--tag <tag>`: phiên bản package mong đợi.
...
- Hiển thị trạng thái quét gói hiện tại, số lượng báo cáo đang mở, trạng thái kiểm duyệt
...
- Báo cáo các yếu tố chặn đối với trạng thái chính thức, tính sẵn có của ClawPack, digest hiện vật,
...
- Nguồn `.tgz` được xử lý như ClawPack. CLI tải lên đúng byte npm-pack
  và chỉ dùng nội dung `package/` đã trích xuất cho xác thực và
  điền trước siêu dữ liệu.
- Thư mục code-plugin được đóng gói thành một tarball npm ClawPack trước khi tải lên để
  các bản cài đặt OpenClaw có thể xác minh đúng hiện vật. Thư mục bundle-plugin vẫn
  dùng đường dẫn xuất bản tệp đã trích xuất.
- Với nguồn GitHub, ghi nhận nguồn được tự động điền từ kho, commit đã phân giải, ref và subpath.
- Với thư mục cục bộ, ghi nhận nguồn được tự động phát hiện từ git cục bộ khi origin remote trỏ tới GitHub.
...
Đối với code plugin, xuất bản thư mục sẽ tạo và tải lên một hiện vật ClawPack từ
thư mục gói:
...
- Ghim workflow tái sử dụng vào một tag ổn định hoặc SHA commit đầy đủ. Không chạy phát hành xuất bản từ `@main`.
- `pull_request` nên dùng `dry_run: true` để CI không gây ô nhiễm.
- Các lần xuất bản thật nên được giới hạn ở sự kiện tin cậy như `workflow_dispatch` hoặc push tag.
- Xuất bản tin cậy không có secret chỉ hoạt động trên `workflow_dispatch`; push tag vẫn cần `clawhub_token`.
- Giữ `clawhub_token` sẵn có cho lần xuất bản đầu tiên, gói không tin cậy, hoặc xuất bản break-glass.
- Workflow tải kết quả JSON lên dưới dạng hiện vật và phơi bày nó dưới dạng đầu ra workflow.
