...
read_when:
    ...
    - Quyết định có nên cài đặt một kỹ năng hay Plugin hay không
    - Giải thích trạng thái kiểm toán ClawHub, mức độ rủi ro hoặc các phát hiện
sidebarTitle: Security Audits
summary: Cách hiểu kết quả kiểm toán bảo mật ClawHub trước khi cài đặt skill hoặc plugin.
title: Kiểm toán bảo mật
x-i18n:
    generated_at: "2026-07-03T09:45:06Z"
    model: gpt-5.5
...
liệu có điều gì cần chú ý thêm trước khi bản phát hành đó có thể truy cập tệp, tài khoản,
...
Xem thêm [Bảo mật](/clawhub/security), [Cách sử dụng được chấp nhận](/vi/clawhub/acceptable-usage),
...
| `Warn`      | Hãy đặc biệt thận trọng. ClawHub tìm thấy mối lo ngại tác động cao hoặc tín hiệu cảnh báo. |
| `Malicious` | Không cài đặt.                                                            |
| `Pending`   | Kiểm toán chưa hoàn tất.                                                  |
| `Error`     | Không thể hoàn tất kiểm toán.                                             |

`Pass` là tín hiệu trấn an, nhưng không thay thế đánh giá của chính bạn. Điều này quan trọng
...
không có nghĩa là nó độc hại. Nó có nghĩa là kỹ năng có vẻ phù hợp với mục đích, nhưng có thể
...
Phát hiện có thể được gắn nhãn `Info`, `Low`, `Medium`, `High`, hoặc `Critical`. Phát hiện có
mức độ nghiêm trọng cao hơn góp phần mạnh hơn vào mức rủi ro và trạng thái kiểm toán.

Các phát hiện có độ tin cậy thấp được ẩn khỏi phần tổng hợp kiểm toán công khai để trang
...
ClawHub sử dụng VirusTotal làm dữ liệu đo lường mã độc trong ngăn xếp kiểm toán. VirusTotal là một
tiêu chuẩn ngành đáng tin cậy về uy tín tệp và quét mã độc, và quan hệ đối tác của chúng tôi
cho phép ClawHub bổ sung thông tin tình báo bảo mật rộng hơn vào việc đánh giá kỹ năng và Plugin.

VirusTotal đặc biệt hữu ích với các tạo phẩm độc hại đã biết, lượt phát hiện của công cụ và
tín hiệu uy tín bổ sung cho đánh giá có nhận biết tác nhân của ClawHub. Khi có số lượng
công cụ nhà cung cấp, kiểm toán tóm tắt chúng bằng ngôn ngữ rõ ràng, chẳng hạn như:
...
VirusTotal vẫn là dữ liệu đo lường. Nó không thay thế phân tích rủi ro có nhận biết tạo phẩm
của chính ClawHub.

## Phân tích rủi ro

Phân tích rủi ro được vận hành nội bộ bởi ClawScan, hệ thống kiểm toán bảo mật riêng của
ClawHub. Hệ thống này đánh giá từng bản phát hành như một tạo phẩm hướng tới tác nhân: hướng dẫn,
siêu dữ liệu, quyền đã khai báo, tệp, tín hiệu năng lực, tín hiệu quét tĩnh,
phát hiện của SkillSpector, dữ liệu đo lường VirusTotal và ngữ cảnh do nhà xuất bản cung cấp.
Tín hiệu quét tĩnh là ngữ cảnh nội bộ cho đánh giá này; chúng không phải là
một mục kiểm toán công khai độc lập hoặc phán quyết chặn cài đặt.
...
thực thi không an toàn, đầu độc bộ nhớ hoặc ngữ cảnh, và khả năng tự chủ quá mức.
