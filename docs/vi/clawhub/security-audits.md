...
read_when:
    ...
    - Quyết định có nên cài đặt skill hay plugin hay không
...
x-i18n:
    generated_at: "2026-07-03T00:58:23Z"
    model: gpt-5.5
...
Kiểm toán bảo mật ClawHub giúp bạn quyết định liệu một kỹ năng hoặc Plugin có đủ an toàn để
cài đặt hay không. Chúng cho biết một bản phát hành làm gì, yêu cầu quyền hạn nào, và
...
| `Warn`      | Thận trọng hơn. ClawHub tìm thấy mối quan ngại có tác động cao hoặc tín hiệu cảnh báo. |
| `Malicious` | Không cài đặt.                                                            |
| `Pending`   | Kiểm toán chưa hoàn tất.                                                  |
| `Error`     | Không thể hoàn tất kiểm toán.                                             |

`Pass` là tín hiệu trấn an, nhưng không thay thế phán đoán của chính bạn. Điều này quan trọng
nhất với các công cụ có thể xuất bản nội dung, chỉnh sửa dữ liệu, chạy lệnh, đọc tệp hoặc
truy cập hệ thống sản xuất.

## Mức độ rủi ro

Mức độ rủi ro mô tả phạm vi ảnh hưởng: bản phát hành có vẻ có bao nhiêu quyền lực nếu
...
mức độ nghiêm trọng cao hơn đóng góp mạnh hơn vào mức độ rủi ro và trạng thái kiểm toán.

Các phát hiện có độ tin cậy thấp bị ẩn khỏi bản tổng hợp kiểm toán công khai để trang
tập trung vào bằng chứng hữu ích.

## ClawHub kiểm tra gì

ClawHub kiểm toán các hiện vật bản phát hành được gửi lên, bao gồm:

- hướng dẫn kỹ năng hoặc siêu dữ liệu Plugin
- biến môi trường và quyền đã khai báo
- hướng dẫn cài đặt và siêu dữ liệu gói
- tệp đi kèm và bản kê tệp
...
Các trang hiện vật liên kết đến kiểm toán đầy đủ tại:
...
ClawHub sử dụng VirusTotal làm dữ liệu đo từ xa về phần mềm độc hại trong ngăn xếp kiểm toán. VirusTotal là
tiêu chuẩn ngành đáng tin cậy cho danh tiếng tệp và quét phần mềm độc hại, và
quan hệ đối tác của chúng tôi cho phép ClawHub bổ sung thông tin tình báo bảo mật rộng hơn vào việc đánh giá kỹ năng và Plugin.

VirusTotal đặc biệt hữu ích cho các hiện vật độc hại đã biết, lượt phát hiện của công cụ và
tín hiệu danh tiếng bổ trợ cho đánh giá có nhận thức về tác nhân của ClawHub. Khi có
số lượng công cụ của nhà cung cấp, kiểm toán tóm tắt chúng bằng ngôn ngữ đơn giản, chẳng hạn như:
...
Khi ClawHub không có dữ liệu đo từ xa về số lượng nhà cung cấp để tóm tắt, kiểm toán cho biết:

```text
No VirusTotal findings
```

VirusTotal vẫn là dữ liệu đo từ xa. Nó không thay thế phân tích rủi ro có nhận thức về hiện vật
riêng của ClawHub.

## Phân tích rủi ro

Phân tích rủi ro được vận hành nội bộ bởi ClawScan, hệ thống kiểm toán bảo mật riêng của ClawHub.
Nó xem xét từng bản phát hành như một hiện vật hướng tới tác nhân: hướng dẫn,
