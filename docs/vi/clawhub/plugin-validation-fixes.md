...
x-i18n:
    generated_at: "2026-06-30T22:22:14Z"
    model: gpt-5.5
...
các lần quét gói tự động. Trang này bao gồm các phát hiện dành cho tác giả, nghĩa là
các phát hiện mà tác giả Plugin có thể sửa trong siêu dữ liệu gói, manifest, SDK
imports, hoặc artifact đã phát hành của họ.

Trang này không bao gồm các phát hiện về phạm vi kiểm tra nội bộ của Plugin Inspector. Nếu một báo cáo đầy đủ
chứa mã bảo trì của trình quét mà không có hướng dẫn khắc phục cho tác giả, các mã đó
...
| `legacy-root-sdk-import`                | [Thay thế root SDK imports](/vi/clawhub/plugin-validation-fixes#legacy-root-sdk-import)                                             |
| `reserved-sdk-import`                   | [Xóa reserved SDK imports](/vi/clawhub/plugin-validation-fixes#reserved-sdk-import)                                             |
...
| `sdk-session-transcript-file-target`    | [Thay thế các mục tiêu tệp transcript cũ](/vi/clawhub/plugin-validation-fixes#sdk-session-transcript-file-target)                   |
...
| `security-manifest-schema-unavailable`  | [Xóa tham chiếu đến lược đồ security manifest không khả dụng](/vi/clawhub/plugin-validation-fixes#security-manifest-schema-unavailable) |
| `unrecognized-security-manifest`        | [Xóa các tệp security manifest không được hỗ trợ](/vi/clawhub/plugin-validation-fixes#unrecognized-security-manifest)                   |
...
- Dùng phiên bản API Plugin OpenClaw hoặc semver floor mà bạn đã build và kiểm thử
  dựa trên đó.
...
- Nếu `openclaw.plugin.json` cũng có `version`, hãy cập nhật để khớp hoặc xóa
  siêu dữ liệu phiên bản manifest đã cũ khi siêu dữ liệu gói là nguồn chính.
...
- Sửa siêu dữ liệu gói không hợp lệ, lifecycle scripts bị hỏng, hoặc mục files khiến
...
- Chuyển hành vi runtime vào mã đăng ký Plugin, và giữ `contracts`
  chỉ giới hạn ở siêu dữ liệu quyền sở hữu khả năng tĩnh.
- Chạy lại `clawhub package validate <path-to-plugin>`.
...
- Dùng `openclaw/plugin-sdk/channel-core` cho các helper entry của kênh.
...
- Chỉ giữ `loadSessionStore(...)` khi phạm vi tương thích đã khai báo của bạn
  vẫn hỗ trợ các phiên bản OpenClaw cũ hơn cần helper này.
...
- Tránh tải, sửa đổi và lưu toàn bộ đối tượng session store.
- Chỉ giữ các helper ghi toàn bộ store khi phạm vi tương thích đã khai báo của bạn
...
Plugin vẫn dùng helper target tệp transcript đã lỗi thời
...
- Dùng `resolveSessionTranscriptTarget(...)` khi mã cần target thao tác transcript
  có cấu trúc.
- Tránh đọc hoặc dựng trực tiếp các target tệp transcript cũ.
...
Manifest vẫn dùng siêu dữ liệu xác thực provider `providerAuthEnvVars` cũ.
...
Manifest dùng siêu dữ liệu biến môi trường kênh cũ hoặc phiên bản cũ hơn mà không có
...
Gói cung cấp `openclaw.security.json` với tham chiếu schema mà ClawHub
không nhận diện là có sẵn.

- Xóa URL schema nếu nó chỉ mang tính tư vấn.
- Chỉ dùng schema có phiên bản đã được ghi tài liệu sau khi OpenClaw công bố schema đó.
- Chạy lại `clawhub package validate <path-to-plugin>`.
