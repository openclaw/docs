...
summary: Khắc phục các phát hiện kiểm tra hợp lệ gói Plugin ClawHub trước khi xuất bản
title: Sửa lỗi xác thực Plugin
x-i18n:
    generated_at: "2026-07-03T13:38:03Z"
    model: gpt-5.5
...
hoặc tạo tác đã phát hành của họ.

Trang này không đề cập đến các phát hiện về phạm vi kiểm tra nội bộ của Plugin Inspector. Nếu một báo cáo đầy đủ
chứa mã bảo trì trình quét mà không có hướng dẫn khắc phục dành cho tác giả, thì các mã đó
...
| `package-npm-pack-unavailable`          | [Làm cho tạo tác npm có thể đóng gói](/vi/clawhub/plugin-validation-fixes#package-npm-pack-unavailable)                                 |
...
| `sdk-session-file-helper`               | [Thay thế trình trợ giúp đường dẫn tệp phiên](/vi/clawhub/plugin-validation-fixes#sdk-session-file-helper)                                   |
...
| `sdk-session-transcript-low-level`      | [Thay thế trình trợ giúp transcript cấp thấp](/vi/clawhub/plugin-validation-fixes#sdk-session-transcript-low-level)                       |
| `legacy-before-agent-start`             | [Thay thế before_agent_start](/vi/clawhub/plugin-validation-fixes#legacy-before-agent-start)                                        |
| `provider-auth-env-vars`                | [Chuyển env vars của provider sang siêu dữ liệu thiết lập](/vi/clawhub/plugin-validation-fixes#provider-auth-env-vars)                             |
...
Gốc gói không bao gồm `package.json`, vì vậy ClawHub không thể nhận diện
...
- Nếu `openclaw.plugin.json` cũng có `version`, hãy cập nhật nó để khớp hoặc xóa
...
## Tạo tác đã phát hành

### package-npm-pack-unavailable

Gói không thể được đóng gói thành tạo tác mà ClawHub sẽ kiểm tra hoặc
phát hành.

- Chạy `npm pack --dry-run` từ gốc gói.
- Sửa siêu dữ liệu gói không hợp lệ, lifecycle scripts bị hỏng hoặc các mục files
...
Gói có thể được đóng gói, nhưng tạo tác đã đóng gói không bao gồm các
tệp entrypoint được khai báo trong `package.json#openclaw`.
...
Tạo tác đã đóng gói bị thiếu siêu dữ liệu OpenClaw tồn tại trong gói
nguồn của bạn.
...
Plugin nhập một đường dẫn SDK dành riêng cho các plugin được đóng gói kèm hoặc khả năng
tương thích nội bộ.
...
Plugin vẫn dùng trình trợ giúp ghi toàn bộ kho phiên đã ngừng khuyến nghị như
...
- Chỉ giữ các trình trợ giúp ghi toàn bộ kho khi phạm vi tương thích đã khai báo của bạn
  vẫn hỗ trợ các phiên bản OpenClaw cũ hơn cần đến chúng.
- Xem [API runtime](/vi/plugins/sdk-runtime#agent-session-state) và
  [Subpath SDK Plugin](/vi/plugins/sdk-subpaths).
- Chạy lại `clawhub package validate <path-to-plugin>`.
...
- Dùng `getSessionEntry(...)` để đọc siêu dữ liệu phiên theo agent và định danh
  phiên.
- Dùng `patchSessionEntry(...)` hoặc `upsertSessionEntry(...)` để lưu bền siêu dữ liệu
  phiên.
- Dùng định danh bản ghi hoặc trình trợ giúp đích khi mã đang chuẩn bị một thao tác
  bản ghi.
...
- Ưu tiên bề mặt runtime bản ghi có cấu trúc để OpenClaw có thể áp dụng
  đúng ranh giới giao dịch và cách xử lý định danh.
...
Tệp kê khai vẫn dùng siêu dữ liệu xác thực provider cũ `providerAuthEnvVars`.

- Phản chiếu siêu dữ liệu biến môi trường của provider vào `setup.providers[].envVars`.
- Chỉ giữ `providerAuthEnvVars` làm siêu dữ liệu tương thích khi phạm vi OpenClaw
  được hỗ trợ của bạn vẫn cần đến nó.
...
Tệp kê khai dùng siêu dữ liệu biến môi trường kênh cũ hoặc thế hệ cũ mà không có
siêu dữ liệu setup hoặc cấu hình hiện tại mà ClawHub kỳ vọng.
...
- Phản chiếu setup kênh dựa trên biến môi trường vào setup, cấu hình kênh hoặc
  siêu dữ liệu kênh gói hiện tại mà hình dạng plugin của bạn sử dụng.
