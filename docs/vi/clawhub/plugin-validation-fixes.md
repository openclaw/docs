...
x-i18n:
    generated_at: "2026-07-03T17:26:17Z"
    model: gpt-5.5
...
những lần quét gói tự động. Trang này bao gồm các phát hiện dành cho tác giả, tức là
những phát hiện mà tác giả Plugin có thể sửa trong siêu dữ liệu gói, manifest, import SDK
hoặc artifact đã phát hành của gói.

Trang này không bao gồm các phát hiện về phạm vi bao phủ nội bộ của Plugin Inspector. Nếu một báo cáo đầy đủ
chứa các mã bảo trì scanner mà không có hướng dẫn khắc phục dành cho tác giả, thì các mã đó
dành cho các maintainer OpenClaw thay vì tác giả Plugin.
...
| `package-npm-pack-unavailable`          | [Đảm bảo artifact npm có thể được đóng gói](/vi/clawhub/plugin-validation-fixes#package-npm-pack-unavailable)                                 |
...
| `reserved-sdk-import`                   | [Xóa các import SDK dành riêng](/vi/clawhub/plugin-validation-fixes#reserved-sdk-import)                                             |
...
| `sdk-session-transcript-file-target`    | [Thay thế các target tệp transcript cũ](/vi/clawhub/plugin-validation-fixes#sdk-session-transcript-file-target)                   |
...
Thư mục gốc của gói không bao gồm `package.json`, nên ClawHub không thể nhận diện
...
- Dùng [Xây dựng Plugin](/vi/plugins/building-plugins) để xem ví dụ gói tối thiểu
  ...
  để hiểu phần tách biệt giữa gói và manifest.
...
ClawHub không thể xác định cách gói cần được cài đặt hoặc cập nhật.
...
- Căn chỉnh phiên bản host tối thiểu với dải phiên bản host mà gói
  thực sự hỗ trợ.
- Xem [các trường package.json ảnh hưởng đến phát hiện](/vi/plugins/manifest#packagejson-fields-that-affect-discovery).
- Chạy lại `clawhub package validate <path-to-plugin>`.
...
- Sửa siêu dữ liệu gói không hợp lệ, script lifecycle bị lỗi hoặc mục files khiến
...
- Giữ `name` ở dạng con người đọc được và giữ `id` làm mã định danh máy ổn định.
- Xem [Manifest Plugin](/vi/plugins/manifest).
- Chạy lại `clawhub package validate <path-to-plugin>`.
...
- Chuyển hành vi runtime vào mã đăng ký Plugin, và giữ `contracts`
  chỉ giới hạn ở siêu dữ liệu sở hữu năng lực tĩnh.
- Chạy lại `clawhub package validate <path-to-plugin>`.
...
Plugin import từ barrel SDK gốc đã ngừng khuyến nghị:
`openclaw/plugin-sdk`.

- Thay thế import từ barrel gốc bằng import subpath công khai tập trung.
...
Plugin import một đường dẫn SDK dành riêng cho Plugin đóng gói sẵn hoặc khả năng
tương thích nội bộ.
...
- Chỉ giữ `loadSessionStore(...)` trong khi khoảng tương thích đã khai báo của
  bạn vẫn hỗ trợ các phiên bản OpenClaw cũ hơn cần nó.
...
- Dùng `patchSessionEntry(...)` khi cập nhật các trường trên một mục phiên hiện
  có.
- Dùng `upsertSessionEntry(...)` khi thay thế hoặc tạo một mục phiên.
- Tránh tải, thay đổi và lưu toàn bộ đối tượng session store.
- Chỉ giữ các helper ghi toàn bộ store trong khi khoảng tương thích đã khai báo
  của bạn vẫn hỗ trợ các phiên bản OpenClaw cũ hơn cần chúng.
- Xem [API Runtime](/vi/plugins/sdk-runtime#agent-session-state) và
  [Subpath Plugin SDK](/vi/plugins/sdk-subpaths).
- Chạy lại `clawhub package validate <path-to-plugin>`.
...
- Dùng `getSessionEntry(...)` để đọc siêu dữ liệu phiên theo danh tính tác nhân
  và phiên.
- Dùng `patchSessionEntry(...)` hoặc `upsertSessionEntry(...)` để lưu bền siêu dữ
  liệu phiên.
- Dùng danh tính transcript hoặc các helper mục tiêu khi mã đang chuẩn bị một
  thao tác transcript.
- Không lưu bền hoặc phụ thuộc vào các đường dẫn tệp transcript kế thừa.
- Xem [API Runtime](/vi/plugins/sdk-runtime#agent-session-state) và
  [Subpath Plugin SDK](/vi/plugins/sdk-subpaths).
- Chạy lại `clawhub package validate <path-to-plugin>`.
...
Plugin vẫn dùng helper mục tiêu tệp transcript đã ngừng khuyến nghị
...
- Tránh đọc hoặc tạo trực tiếp các mục tiêu tệp transcript kế thừa.
- Chỉ giữ helper kế thừa trong khi khoảng tương thích đã khai báo của bạn vẫn
...
- Dùng `appendSessionTranscriptMessageByIdentity(...)` cho thao tác thêm vào
  transcript.
...
- Ưu tiên bề mặt runtime transcript có cấu trúc để OpenClaw có thể áp dụng đúng
  ranh giới giao dịch và cách xử lý danh tính.
- Chỉ giữ các helper transcript cấp thấp trong khi khoảng tương thích đã khai báo
  của bạn vẫn hỗ trợ các phiên bản OpenClaw cũ hơn cần chúng.
- Xem [API Runtime](/vi/plugins/sdk-runtime#agent-session-state) và
  [Subpath Plugin SDK](/vi/plugins/sdk-subpaths).
- Chạy lại `clawhub package validate <path-to-plugin>`.
...
- Chỉ giữ `before_agent_start` trong khi khoảng tương thích đã khai báo của bạn
...
Manifest vẫn dùng siêu dữ liệu xác thực nhà cung cấp kế thừa
`providerAuthEnvVars`.

- Phản chiếu siêu dữ liệu env-var của nhà cung cấp vào
  `setup.providers[].envVars`.
- Chỉ giữ `providerAuthEnvVars` làm siêu dữ liệu tương thích trong khi khoảng
...
Manifest dùng siêu dữ liệu env-var kênh kế thừa hoặc cũ hơn mà không có siêu dữ
liệu setup hoặc cấu hình hiện tại mà ClawHub mong đợi.

- Giữ siêu dữ liệu env-var kênh ở dạng khai báo để OpenClaw có thể kiểm tra
  trạng thái setup mà không cần tải runtime kênh.
- Phản chiếu setup kênh do env điều khiển vào setup hiện tại, cấu hình kênh,
  hoặc siêu dữ liệu kênh gói được hình dạng Plugin của bạn sử dụng.
...
Gói cung cấp `openclaw.security.json` với tham chiếu schema mà ClawHub không nhận
diện là có sẵn.
...
- Xóa `openclaw.security.json` cho đến khi OpenClaw ghi tài liệu schema manifest
  bảo mật có phiên bản và hành vi ClawHub.
- Giữ hành vi nhạy cảm về bảo mật được ghi tài liệu trong tài liệu gói công khai
