...
x-i18n:
    generated_at: "2026-07-01T13:04:41Z"
    model: gpt-5.5
...
chứa các mã bảo trì máy quét mà không có hướng dẫn khắc phục dành cho tác giả, các mã đó
...
- Kiểm tra bất kỳ siêu dữ liệu build OpenClaw nào trong gói, chẳng hạn như phiên bản OpenClaw
...
- Sửa siêu dữ liệu gói không hợp lệ, các script vòng đời bị hỏng, hoặc các mục files
...
- Chuyển hành vi runtime vào mã đăng ký plugin, và giới hạn `contracts`
  ở siêu dữ liệu quyền sở hữu năng lực tĩnh.
- Chạy lại `clawhub package validate <path-to-plugin>`.
...
Plugin nhập từ barrel SDK gốc đã bị phản đối:
`openclaw/plugin-sdk`.
...
Plugin nhập một đường dẫn SDK dành riêng cho các plugin đóng gói kèm hoặc khả năng tương thích
nội bộ.
...
Plugin vẫn dùng helper toàn bộ session-store đã bị phản đối
`loadSessionStore`.
...
Plugin vẫn dùng helper ghi toàn bộ session-store đã bị phản đối như
...
- Tránh tải, sửa đổi và lưu toàn bộ đối tượng session store.
- Chỉ giữ các helper ghi toàn bộ store khi dải tương thích đã khai báo của bạn
...
Plugin vẫn dùng các helper đường dẫn tệp phiên đã bị phản đối như
`resolveSessionFilePath` hoặc `resolveAndPersistSessionFile`.

- Dùng `getSessionEntry(...)` để đọc siêu dữ liệu phiên theo danh tính tác nhân và phiên.
- Dùng `patchSessionEntry(...)` hoặc `upsertSessionEntry(...)` để lưu siêu dữ liệu phiên.
- Dùng danh tính transcript hoặc các helper đích khi mã đang chuẩn bị một
  thao tác transcript.
...
Plugin vẫn dùng helper đích tệp transcript đã bị phản đối
...
- Tránh đọc hoặc xây dựng trực tiếp các đích tệp transcript cũ.
- Chỉ giữ helper cũ khi dải tương thích đã khai báo của bạn vẫn
...
Plugin vẫn dùng các helper transcript cấp thấp đã bị phản đối như
...
- Chỉ giữ `providerAuthEnvVars` làm siêu dữ liệu tương thích khi dải
...
siêu dữ liệu setup hoặc config hiện tại mà ClawHub kỳ vọng.
...
- Phản chiếu thiết lập kênh dựa trên biến môi trường vào setup hiện tại, cấu hình kênh, hoặc
...
- Xóa `openclaw.security.json` cho đến khi OpenClaw ghi tài liệu schema manifest bảo mật
...
- Giữ hành vi nhạy cảm về bảo mật được ghi tài liệu trong tài liệu gói công khai của bạn hoặc
  README cho đến khi contract manifest tồn tại.
- Chạy lại `clawhub package validate <path-to-plugin>`.
