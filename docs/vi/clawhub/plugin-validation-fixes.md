...
title: Các bản sửa lỗi về kiểm tra hợp lệ Plugin
x-i18n:
    generated_at: "2026-06-28T06:20:55Z"
    model: gpt-5.5
...
| `provider-auth-env-vars`                | [Chuyển các biến môi trường của provider sang siêu dữ liệu thiết lập](/vi/clawhub/plugin-validation-fixes#provider-auth-env-vars)                             |
...
- Nếu `openclaw.plugin.json` cũng có `version`, hãy cập nhật để khớp hoặc xóa
  siêu dữ liệu phiên bản manifest đã lỗi thời khi siêu dữ liệu gói là nguồn chính.
...
- Sửa siêu dữ liệu gói không hợp lệ, lifecycle script bị hỏng hoặc các mục files khiến
...
- Chuyển hành vi runtime vào mã đăng ký plugin, và giữ `contracts`
  chỉ giới hạn ở metadata sở hữu capability tĩnh.
- Chạy lại `clawhub package validate <path-to-plugin>`.
...
- Sử dụng `openclaw/plugin-sdk/channel-core` cho các helper entry kênh.
- Sử dụng [quy ước import](/vi/plugins/building-plugins#import-conventions) và
  [subpath Plugin SDK](/vi/plugins/sdk-subpaths) để tìm import hẹp.
- Chạy lại `clawhub package validate <path-to-plugin>`.

### reserved-sdk-import

Plugin nhập một đường dẫn SDK được dành riêng cho các plugin đóng gói kèm hoặc khả năng
tương thích nội bộ.

- Thay thế các import SDK nội bộ OpenClaw được dành riêng bằng các subpath
  `openclaw/plugin-sdk/*` công khai có tài liệu.
- Nếu hành vi đó không có SDK công khai, hãy giữ helper bên trong gói của bạn hoặc
  yêu cầu một API OpenClaw công khai.
- Sử dụng [subpath Plugin SDK](/vi/plugins/sdk-subpaths) và
...
- Sử dụng `getSessionEntry(...)` hoặc `listSessionEntries(...)` khi đọc trạng thái session.
- Sử dụng `patchSessionEntry(...)` hoặc `upsertSessionEntry(...)` khi ghi trạng thái session.
- Tránh tải, chỉnh sửa và lưu toàn bộ đối tượng session store.
- Chỉ giữ `loadSessionStore(...)` trong khi dải tương thích đã khai báo của bạn
...
- Chuyển công việc chỉnh sửa prompt hoặc context sang `before_prompt_build`.
- Chỉ giữ `before_agent_start` trong khi dải tương thích đã khai báo của bạn vẫn
...
Manifest vẫn dùng metadata auth provider `providerAuthEnvVars` cũ.

- Phản chiếu metadata biến env của provider vào `setup.providers[].envVars`.
- Chỉ giữ `providerAuthEnvVars` dưới dạng metadata tương thích trong khi dải
...
Manifest dùng metadata biến env kênh cũ hoặc đời cũ hơn mà không có metadata
setup hoặc config hiện tại mà ClawHub kỳ vọng.

- Giữ metadata biến env kênh ở dạng khai báo để OpenClaw có thể kiểm tra trạng thái setup
...
- Chỉ giữ `channelEnvVars` dưới dạng metadata tương thích trong khi các phiên bản
...
- Chỉ sử dụng schema có phiên bản đã được tài liệu hóa sau khi OpenClaw phát hành một schema.
- Chạy lại `clawhub package validate <path-to-plugin>`.
...
- Xóa `openclaw.security.json` cho đến khi OpenClaw tài liệu hóa một schema manifest bảo mật
  có phiên bản và hành vi ClawHub.
- Giữ hành vi nhạy cảm về bảo mật được tài liệu hóa trong tài liệu gói công khai hoặc
  README của bạn cho đến khi hợp đồng manifest tồn tại.
- Chạy lại `clawhub package validate <path-to-plugin>`.
