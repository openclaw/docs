...
read_when:
    ...
    - ClawHub từ chối hoặc cảnh báo khi phát hành gói Plugin
...
x-i18n:
    generated_at: "2026-06-28T00:11:40Z"
    model: gpt-5.5
...
các lần quét gói tự động. Trang này bao quát các phát hiện dành cho tác giả, nghĩa là
các phát hiện mà tác giả Plugin có thể sửa trong siêu dữ liệu gói, bản kê khai, phần nhập SDK
hoặc artifact đã xuất bản của họ.

Trang này không bao quát các phát hiện về phạm vi kiểm thử nội bộ của Plugin Inspector. Nếu một báo cáo đầy đủ
...
| `reserved-sdk-import`                   | [Xóa phần nhập SDK được dành riêng](/vi/clawhub/plugin-validation-fixes#reserved-sdk-import)                                             |
...
- Dùng [Xây dựng Plugin](/vi/plugins/building-plugins) để xem ví dụ gói tối thiểu
  ...
  để hiểu phần tách giữa gói và bản kê khai.
...
- Giữ phần này tách biệt với phiên bản gói. Phiên bản gói mô tả bản phát hành
  Plugin; `openclaw.compat.pluginApi` mô tả hợp đồng API của host.
- Xem [các trường package.json ảnh hưởng đến việc khám phá](/vi/plugins/manifest#packagejson-fields-that-affect-discovery).
- Chạy lại `clawhub package validate <path-to-plugin>`.
...
- Sửa siêu dữ liệu gói không hợp lệ, script vòng đời bị hỏng hoặc các mục tệp
  làm cho việc đóng gói thất bại.
...
- Giữ `name` ở dạng con người đọc được và giữ `id` làm id máy ổn định.
- Xem [Bản kê khai Plugin](/vi/plugins/manifest).
- Chạy lại `clawhub package validate <path-to-plugin>`.
...
Plugin nhập một đường dẫn SDK dành riêng cho các Plugin tích hợp sẵn hoặc
tương thích nội bộ.
...
- Chỉ giữ `providerAuthEnvVars` dưới dạng siêu dữ liệu tương thích trong khi phạm vi
...
- Giữ siêu dữ liệu env-var của kênh ở dạng khai báo để OpenClaw có thể kiểm tra
  trạng thái setup mà không cần tải runtime của kênh.
- Phản chiếu setup kênh dựa trên env vào setup hiện tại, config kênh, hoặc
  siêu dữ liệu kênh của gói mà hình dạng Plugin của bạn sử dụng.
- Chỉ giữ `channelEnvVars` dưới dạng siêu dữ liệu tương thích trong khi các phiên bản
