...
x-i18n:
    generated_at: "2026-07-02T08:26:35Z"
    model: gpt-5.5
...
quá trình quét gói tự động. Trang này bao quát các phát hiện dành cho tác giả, nghĩa là
các phát hiện mà tác giả Plugin có thể sửa trong siêu dữ liệu gói, manifest, lượt nhập SDK
hoặc artifact đã phát hành của họ.

Trang này không bao quát các phát hiện về phạm vi kiểm tra nội bộ của Plugin Inspector. Nếu một báo cáo đầy đủ
...
| `legacy-root-sdk-import`                | [Thay thế các lượt nhập SDK gốc](/vi/clawhub/plugin-validation-fixes#legacy-root-sdk-import)                                             |
| `reserved-sdk-import`                   | [Xóa các lượt nhập SDK dành riêng](/vi/clawhub/plugin-validation-fixes#reserved-sdk-import)                                             |
...
| `sdk-session-transcript-file-target`    | [Thay thế đích tệp bản ghi cũ](/vi/clawhub/plugin-validation-fixes#sdk-session-transcript-file-target)                   |
...
- Xóa `private: true` nếu gói này được dự định để phát hành công khai.
- Chạy lại `clawhub package validate <path-to-plugin>`.
...
Plugin nhập một đường dẫn SDK dành riêng cho plugin đóng gói kèm hoặc khả năng
tương thích nội bộ.
...
Plugin vẫn dùng helper whole-session-store đã ngừng khuyến nghị
`loadSessionStore`.
...
Plugin vẫn dùng helper ghi whole-session-store đã ngừng khuyến nghị, chẳng hạn như
...
- Tránh tải, sửa đổi và lưu toàn bộ đối tượng kho phiên.
- Chỉ giữ các helper ghi toàn kho khi phạm vi tương thích đã khai báo của bạn
...
- Dùng `patchSessionEntry(...)` hoặc `upsertSessionEntry(...)` để lưu siêu dữ liệu
  phiên.
- Dùng danh tính bản ghi hoặc helper đích khi mã đang chuẩn bị một thao tác bản ghi.
...
- Chỉ giữ helper cũ khi phạm vi tương thích đã khai báo của bạn vẫn hỗ trợ các
...
- Dùng `appendSessionTranscriptMessageByIdentity(...)` để nối bản ghi.
...
Tệp kê khai dùng siêu dữ liệu biến môi trường kênh cũ hoặc đời trước mà không có
...
- Giữ siêu dữ liệu biến môi trường kênh ở dạng khai báo để OpenClaw có thể kiểm
  tra trạng thái setup mà không tải runtime kênh.
- Phản chiếu setup kênh do biến môi trường điều khiển vào setup hiện tại, cấu hình
  kênh, hoặc siêu dữ liệu kênh gói mà hình dạng plugin của bạn dùng.
...
Gói phân phối `openclaw.security.json` với tham chiếu schema mà ClawHub không
nhận diện là có sẵn.
...
- Xóa `openclaw.security.json` cho đến khi OpenClaw ghi tài liệu schema tệp kê
  khai bảo mật có phiên bản và hành vi ClawHub.
