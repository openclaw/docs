...
x-i18n:
    generated_at: "2026-07-04T03:52:55Z"
    model: gpt-5.5
...
ClawHub xác thực các gói Plugin trước khi phát hành và cũng có thể hiển thị các phát hiện từ các lần quét gói tự động. Trang này đề cập đến các phát hiện dành cho tác giả, tức là các phát hiện mà tác giả Plugin có thể sửa trong siêu dữ liệu gói, manifest, import SDK hoặc artifact đã phát hành của họ.

Trang này không đề cập đến các phát hiện về độ bao phủ nội bộ của Plugin Inspector. Nếu báo cáo đầy đủ chứa các mã bảo trì scanner mà không có hướng dẫn khắc phục dành cho tác giả, các mã đó dành cho maintainer OpenClaw chứ không phải tác giả Plugin.
...
| `sdk-session-transcript-file-target`    | [Thay thế target tệp transcript cũ](/vi/clawhub/plugin-validation-fixes#sdk-session-transcript-file-target)                   |
...
| `provider-auth-env-vars`                | [Di chuyển env vars của provider sang siêu dữ liệu thiết lập](/vi/clawhub/plugin-validation-fixes#provider-auth-env-vars)                             |
| `channel-env-vars`                      | [Phản chiếu env vars của kênh trong siêu dữ liệu hiện tại](/vi/clawhub/plugin-validation-fixes#channel-env-vars)                                |
| `security-manifest-schema-unavailable`  | [Xóa các tham chiếu schema manifest bảo mật không khả dụng](/vi/clawhub/plugin-validation-fixes#security-manifest-schema-unavailable) |
...
Thư mục gốc của gói không bao gồm `package.json`, vì vậy ClawHub không thể xác định gói npm, phiên bản, entrypoint hoặc siêu dữ liệu OpenClaw.

- Thêm `package.json` với `name`, `version` và `type`.
- Thêm khối `openclaw` khi gói cung cấp Plugin OpenClaw.
- Sử dụng [Xây dựng Plugin](/vi/plugins/building-plugins) để xem ví dụ gói tối thiểu và [Manifest Plugin](/vi/plugins/manifest#manifest-versus-packagejson) để biết cách tách gói và manifest.
...
- Bao gồm siêu dữ liệu entrypoint như `openclaw.extensions` hoặc `openclaw.runtimeExtensions`.
- Thêm siêu dữ liệu tương thích và cài đặt khi gói sẽ được phát hành hoặc cài đặt qua ClawHub.
...
- Thêm `openclaw.extensions` cho entrypoint Plugin native.
- Thêm `openclaw.runtimeExtensions` khi gói đã phát hành cần tải JavaScript đã build.
- Giữ tất cả đường dẫn entrypoint bên trong thư mục gói.
- Xem [Điểm entry Plugin](/vi/plugins/sdk-entrypoints) và [các trường package.json ảnh hưởng đến phát hiện](/vi/plugins/manifest#packagejson-fields-that-affect-discovery).
- Chạy lại `clawhub package validate <path-to-plugin>`.

### package-entrypoint-missing

Gói khai báo một entrypoint OpenClaw, nhưng tệp được tham chiếu bị thiếu trong gói đang được xác thực.
...
- Xem [Điểm entry Plugin](/vi/plugins/sdk-entrypoints).
- Chạy lại `clawhub package validate <path-to-plugin>`.
...
- Điền `openclaw.install` bằng nguồn cài đặt được hỗ trợ, chẳng hạn như `clawhubSpec`, `npmSpec` hoặc `localPath`.
...
- Giữ phần này tách biệt với phiên bản gói. Phiên bản gói mô tả bản phát hành Plugin; `openclaw.compat.pluginApi` mô tả contract API host.
- Xem [các trường package.json ảnh hưởng đến phát hiện](/vi/plugins/manifest#packagejson-fields-that-affect-discovery).
- Chạy lại `clawhub package validate <path-to-plugin>`.

### package-min-host-version-drift

Phiên bản host tối thiểu của gói không khớp với siêu dữ liệu phiên bản OpenClaw mà gói đã được build dựa trên đó.

- Kiểm tra `openclaw.install.minHostVersion`.
- Kiểm tra mọi siêu dữ liệu build OpenClaw trong gói, chẳng hạn như phiên bản OpenClaw được dùng trong quá trình phát hành.
- Căn chỉnh phiên bản host tối thiểu với dải phiên bản host mà gói thực sự hỗ trợ.
- Xem [các trường package.json ảnh hưởng đến phát hiện](/vi/plugins/manifest#packagejson-fields-that-affect-discovery).
- Chạy lại `clawhub package validate <path-to-plugin>`.
...
- Nếu `openclaw.plugin.json` cũng có `version`, hãy cập nhật để khớp hoặc xóa siêu dữ liệu phiên bản manifest đã cũ khi siêu dữ liệu gói là nguồn có thẩm quyền.
...
Khối `package.json#openclaw` chứa các trường không phải là siêu dữ liệu gói OpenClaw được hỗ trợ.

- Xóa các trường không được hỗ trợ như `openclaw.bundle`.
- Giữ siêu dữ liệu Plugin native trong `openclaw.plugin.json`.
- Giữ entrypoint gói, khả năng tương thích, cài đặt, thiết lập và siêu dữ liệu catalog trong các trường `package.json#openclaw` được hỗ trợ.
- Xem [các trường package.json ảnh hưởng đến phát hiện](/vi/plugins/manifest#packagejson-fields-that-affect-discovery).
- Chạy lại `clawhub package validate <path-to-plugin>`.
...
Gói không thể được đóng gói thành artifact mà ClawHub sẽ kiểm tra hoặc phát hành.

- Chạy `npm pack --dry-run` từ thư mục gốc của gói.
- Sửa siêu dữ liệu gói không hợp lệ, script vòng đời bị hỏng hoặc các mục files khiến việc đóng gói thất bại.
- Xóa `private: true` nếu gói này dự định được phát hành công khai.
- Chạy lại `clawhub package validate <path-to-plugin>`.

### package-npm-pack-entrypoint-missing

Gói có thể được đóng gói, nhưng artifact đã đóng gói không bao gồm các tệp entrypoint được khai báo trong `package.json#openclaw`.

- Chạy `npm pack --dry-run` và kiểm tra các tệp sẽ được bao gồm.
- Build các entrypoint được tạo trước khi đóng gói.
- Cập nhật `files`, `.npmignore` hoặc đầu ra build để các entrypoint đã khai báo được bao gồm.
- Xem [Điểm entry Plugin](/vi/plugins/sdk-entrypoints).
- Chạy lại `clawhub package validate <path-to-plugin>`.

### package-npm-pack-metadata-missing

Artifact đã đóng gói thiếu siêu dữ liệu OpenClaw đang tồn tại trong gói nguồn của bạn.
...
- Đảm bảo `openclaw.plugin.json` được bao gồm khi gói là Plugin OpenClaw native.
...
- So sánh từng khóa bên dưới `contracts` với
  [tham chiếu contracts](/vi/plugins/manifest#contracts-reference).
- Xóa các khóa contract không được hỗ trợ.
- Di chuyển hành vi thời gian chạy vào mã đăng ký Plugin, và giữ `contracts`
...
- Thay thế các lệnh nhập barrel gốc bằng các lệnh nhập đường dẫn con công khai có phạm vi hẹp.
- Dùng `openclaw/plugin-sdk/plugin-entry` cho `definePluginEntry`.
- Dùng `openclaw/plugin-sdk/channel-core` cho các hàm trợ giúp điểm vào kênh.
- Dùng [Quy ước nhập](/vi/plugins/building-plugins#import-conventions) và
  [đường dẫn con Plugin SDK](/vi/plugins/sdk-subpaths) để tìm lệnh nhập hẹp.
- Chạy lại `clawhub package validate <path-to-plugin>`.

### reserved-sdk-import

Plugin nhập một đường dẫn SDK dành riêng cho các Plugin được đóng gói sẵn hoặc khả năng tương thích
nội bộ.

- Thay thế các lệnh nhập SDK nội bộ dành riêng của OpenClaw bằng các đường dẫn con
  `openclaw/plugin-sdk/*` công khai được ghi tài liệu.
- Nếu hành vi không có SDK công khai, hãy giữ hàm trợ giúp bên trong gói của bạn hoặc
...
Plugin vẫn dùng hàm trợ giúp toàn bộ kho lưu trữ phiên đã ngừng khuyến nghị
`loadSessionStore`.
...
- Dùng `patchSessionEntry(...)` hoặc `upsertSessionEntry(...)` khi ghi trạng thái
  phiên.
- Tránh tải, sửa đổi và lưu toàn bộ đối tượng kho lưu trữ phiên.
- Chỉ giữ `loadSessionStore(...)` khi phạm vi tương thích đã khai báo của bạn
  vẫn hỗ trợ các phiên bản OpenClaw cũ hơn cần hàm này.
...
Plugin vẫn dùng một hàm trợ giúp ghi toàn bộ kho lưu trữ phiên đã ngừng khuyến nghị, chẳng hạn như
...
- Tránh tải, sửa đổi và lưu toàn bộ đối tượng kho lưu trữ phiên.
- Chỉ giữ các hàm trợ giúp ghi toàn bộ kho khi phạm vi tương thích đã khai báo của bạn
...
Plugin vẫn dùng các hàm trợ giúp đường dẫn tệp phiên đã ngừng khuyến nghị, chẳng hạn như
`resolveSessionFilePath` hoặc `resolveAndPersistSessionFile`.

- Dùng `getSessionEntry(...)` để đọc siêu dữ liệu phiên theo tác tử và danh tính
  phiên.
- Dùng `patchSessionEntry(...)` hoặc `upsertSessionEntry(...)` để lưu bền siêu dữ liệu
  phiên.
- Dùng danh tính bản ghi hoặc các hàm trợ giúp đích khi mã đang chuẩn bị một
  thao tác bản ghi.
- Không lưu bền hoặc phụ thuộc vào các đường dẫn tệp bản ghi cũ.
- Xem [API thời gian chạy](/vi/plugins/sdk-runtime#agent-session-state) và
  [đường dẫn con Plugin SDK](/vi/plugins/sdk-subpaths).
- Chạy lại `clawhub package validate <path-to-plugin>`.
...
Plugin vẫn dùng hàm trợ giúp đích tệp bản ghi đã ngừng khuyến nghị
...
- Dùng `resolveSessionTranscriptTarget(...)` khi mã cần một đích thao tác bản ghi
  có cấu trúc.
- Tránh đọc hoặc xây dựng trực tiếp các đích tệp bản ghi cũ.
- Chỉ giữ hàm trợ giúp cũ khi phạm vi tương thích đã khai báo của bạn vẫn
  hỗ trợ các phiên bản OpenClaw cũ hơn cần hàm này.
- Xem [API thời gian chạy](/vi/plugins/sdk-runtime#agent-session-state) và
  [đường dẫn con Plugin SDK](/vi/plugins/sdk-subpaths).
- Chạy lại `clawhub package validate <path-to-plugin>`.
...
Plugin vẫn dùng các hàm trợ giúp bản ghi cấp thấp đã ngừng khuyến nghị, chẳng hạn như
...
- Ưu tiên bề mặt thời gian chạy bản ghi có cấu trúc để OpenClaw có thể áp dụng
  đúng ranh giới giao dịch và cách xử lý danh tính.
- Chỉ giữ các hàm trợ giúp bản ghi cấp thấp khi phạm vi tương thích đã khai báo của bạn
  vẫn hỗ trợ các phiên bản OpenClaw cũ hơn cần chúng.
- Xem [API thời gian chạy](/vi/plugins/sdk-runtime#agent-session-state) và
  [đường dẫn con Plugin SDK](/vi/plugins/sdk-subpaths).
- Chạy lại `clawhub package validate <path-to-plugin>`.
...
- Di chuyển công việc ghi đè mô hình hoặc nhà cung cấp sang `before_model_resolve`.
- Di chuyển công việc sửa đổi prompt hoặc ngữ cảnh sang `before_prompt_build`.
- Chỉ giữ `before_agent_start` khi phạm vi tương thích đã khai báo của bạn vẫn
  hỗ trợ các phiên bản OpenClaw cũ hơn cần hook này.
...
Tệp kê khai dùng siêu dữ liệu biến môi trường kênh cũ hoặc đời cũ mà không có siêu dữ liệu
setup hoặc cấu hình hiện tại mà ClawHub mong đợi.

- Giữ siêu dữ liệu biến môi trường kênh ở dạng khai báo để OpenClaw có thể kiểm tra trạng thái
  setup mà không cần tải thời gian chạy kênh.
- Phản chiếu setup kênh do biến môi trường điều khiển vào setup hiện tại, cấu hình kênh, hoặc
  siêu dữ liệu kênh gói được hình dạng Plugin của bạn sử dụng.
- Chỉ giữ `channelEnvVars` làm siêu dữ liệu tương thích khi các phiên bản OpenClaw
...
Gói cung cấp `openclaw.security.json` với một tham chiếu schema mà ClawHub
không nhận diện là khả dụng.

- Xóa URL schema nếu nó chỉ mang tính khuyến nghị.
- Chỉ dùng schema có phiên bản được ghi tài liệu sau khi OpenClaw phát hành schema đó.
- Chạy lại `clawhub package validate <path-to-plugin>`.
...
- Xóa `openclaw.security.json` cho đến khi OpenClaw ghi tài liệu một schema tệp kê khai bảo mật
  có phiên bản và hành vi ClawHub.
- Giữ hành vi nhạy cảm về bảo mật được ghi tài liệu trong tài liệu gói công khai của bạn hoặc
  README cho đến khi contract tệp kê khai tồn tại.
- Chạy lại `clawhub package validate <path-to-plugin>`.
