...
những phát hiện mà tác giả Plugin có thể sửa trong siêu dữ liệu gói, manifest, SDK
imports, hoặc artifact đã phát hành của họ.

Trang này không đề cập đến các phát hiện về mức bao phủ nội bộ của Plugin Inspector. Nếu một báo cáo đầy đủ
chứa các mã bảo trì của trình quét mà không có hướng dẫn khắc phục dành cho tác giả, thì các mã đó
...
| `provider-auth-env-vars`                | [Di chuyển biến môi trường provider vào siêu dữ liệu thiết lập](/vi/clawhub/plugin-validation-fixes#provider-auth-env-vars)                             |
| `channel-env-vars`                      | [Phản chiếu biến môi trường channel trong siêu dữ liệu hiện tại](/vi/clawhub/plugin-validation-fixes#channel-env-vars)                                |
...
- Sửa siêu dữ liệu gói không hợp lệ, lifecycle scripts bị lỗi, hoặc mục files khiến
...
- Thay vào đó, chuyển siêu dữ liệu gói hoặc cài đặt vào các trường
  `package.json#openclaw` được hỗ trợ thay vì manifest.
- Chạy lại `clawhub package validate <path-to-plugin>`.
...
- Thay thế các import SDK nội bộ OpenClaw dành riêng bằng các subpath
  `openclaw/plugin-sdk/*` công khai đã được ghi trong tài liệu.
...
Manifest vẫn dùng siêu dữ liệu xác thực provider kế thừa
`providerAuthEnvVars`.

- Sao chép siêu dữ liệu env-var của provider vào `setup.providers[].envVars`.
...
Manifest dùng siêu dữ liệu env-var channel kế thừa hoặc cũ hơn mà không có siêu
dữ liệu setup hoặc config hiện tại mà ClawHub mong đợi.

- Giữ siêu dữ liệu env-var channel ở dạng khai báo để OpenClaw có thể kiểm tra
  trạng thái setup mà không cần tải runtime channel.
- Sao chép setup channel dựa trên env vào setup, config channel hoặc siêu dữ
  liệu channel của gói hiện tại được hình dạng Plugin của bạn sử dụng.
...
Gói cung cấp `openclaw.security.json` với tham chiếu schema mà ClawHub không
nhận diện là có sẵn.

- Xóa URL schema nếu nó chỉ mang tính khuyến nghị.
- Chỉ dùng schema có phiên bản được ghi trong tài liệu sau khi OpenClaw công bố
  schema đó.
- Chạy lại `clawhub package validate <path-to-plugin>`.
...
- Xóa `openclaw.security.json` cho đến khi OpenClaw ghi tài liệu về schema
  manifest bảo mật có phiên bản và hành vi của ClawHub.
