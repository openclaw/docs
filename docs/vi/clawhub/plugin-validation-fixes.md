...
x-i18n:
    generated_at: "2026-07-01T18:14:10Z"
    model: gpt-5.5
...
các phát hiện mà tác giả Plugin có thể sửa trong siêu dữ liệu gói, tệp kê khai, các import SDK
hoặc artifact đã phát hành của họ.

Trang này không đề cập đến các phát hiện về mức độ bao phủ nội bộ của Plugin Inspector. Nếu một báo cáo đầy đủ
...
| `manifest-unknown-fields`               | [Gỡ bỏ các trường tệp kê khai không được hỗ trợ](/vi/clawhub/plugin-validation-fixes#manifest-unknown-fields)                                  |
...
| `security-manifest-schema-unavailable`  | [Gỡ bỏ các tham chiếu lược đồ tệp kê khai bảo mật không khả dụng](/vi/clawhub/plugin-validation-fixes#security-manifest-schema-unavailable) |
| `unrecognized-security-manifest`        | [Gỡ bỏ các tệp kê khai bảo mật không được hỗ trợ](/vi/clawhub/plugin-validation-fixes#unrecognized-security-manifest)                   |
...
- Dùng [Xây dựng Plugin](/vi/plugins/building-plugins) để xem ví dụ gói tối thiểu
  ...
  để hiểu phần tách biệt giữa gói và tệp kê khai.
...
- Dùng phiên bản API Plugin OpenClaw hoặc mức sàn semver mà bạn đã build và kiểm thử
  dựa trên đó.
- Giữ thông tin này tách biệt với phiên bản gói. Phiên bản gói mô tả
...
Phiên bản gói và phiên bản tệp kê khai Plugin không thống nhất.
...
- Nếu `openclaw.plugin.json` cũng có `version`, hãy cập nhật để khớp hoặc gỡ bỏ
  siêu dữ liệu phiên bản tệp kê khai đã cũ khi siêu dữ liệu gói là nguồn chính thức.
...
- Gỡ bỏ `private: true` nếu gói này dự kiến được phát hành công khai.
- Chạy lại `clawhub package validate <path-to-plugin>`.

### package-npm-pack-entrypoint-missing

Gói có thể được đóng gói, nhưng artifact đã đóng gói không bao gồm các tệp
điểm vào được khai báo trong `package.json#openclaw`.
...
Artifact đã đóng gói bị thiếu siêu dữ liệu OpenClaw tồn tại trong gói nguồn
của bạn.
...
Plugin nhập từ barrel SDK gốc đã bị loại bỏ:
`openclaw/plugin-sdk`.

- Thay thế các import root-barrel bằng các import subpath công khai có phạm vi hẹp.
...
Plugin nhập một đường dẫn SDK dành riêng cho các plugin đóng gói kèm hoặc khả năng
tương thích nội bộ.
...
Plugin vẫn dùng helper toàn bộ kho phiên đã bị loại bỏ
`loadSessionStore`.
...
Plugin vẫn dùng một helper ghi toàn bộ kho phiên đã bị loại bỏ, chẳng hạn như
...
Plugin vẫn dùng các helper đường dẫn tệp phiên đã bị loại bỏ, chẳng hạn như
...
- Dùng `patchSessionEntry(...)` hoặc `upsertSessionEntry(...)` để lưu siêu dữ liệu
  phiên.
- Dùng danh tính transcript hoặc helper target khi mã đang chuẩn bị một thao tác
  transcript.
- Không lưu hoặc phụ thuộc vào các đường dẫn tệp transcript kế thừa.
- Xem [API runtime](/vi/plugins/sdk-runtime#agent-session-state) và
  [Subpath Plugin SDK](/vi/plugins/sdk-subpaths).
- Chạy lại `clawhub package validate <path-to-plugin>`.
...
Plugin vẫn dùng helper target tệp transcript đã bị loại bỏ
...
- Dùng `resolveSessionTranscriptTarget(...)` khi mã cần target thao tác transcript
  có cấu trúc.
- Tránh đọc hoặc dựng trực tiếp các target tệp transcript kế thừa.
- Chỉ giữ helper kế thừa khi phạm vi tương thích đã khai báo của bạn vẫn hỗ trợ
...
Plugin vẫn dùng các helper transcript cấp thấp đã bị loại bỏ, chẳng hạn như
`appendSessionTranscriptMessage` hoặc `emitSessionTranscriptUpdate`.

- Dùng `appendSessionTranscriptMessageByIdentity(...)` để thêm transcript.
...
- Chỉ giữ các helper transcript cấp thấp khi phạm vi tương thích đã khai báo của
  bạn vẫn hỗ trợ các phiên bản OpenClaw cũ hơn yêu cầu chúng.
- Xem [API runtime](/vi/plugins/sdk-runtime#agent-session-state) và
  [Subpath Plugin SDK](/vi/plugins/sdk-subpaths).
- Chạy lại `clawhub package validate <path-to-plugin>`.
...
Tệp kê khai vẫn dùng siêu dữ liệu xác thực provider `providerAuthEnvVars` kế thừa.
...
Tệp kê khai dùng siêu dữ liệu env-var kênh kế thừa hoặc cũ hơn mà không có siêu
dữ liệu setup hoặc config hiện tại mà ClawHub mong đợi.

- Giữ siêu dữ liệu env-var kênh ở dạng khai báo để OpenClaw có thể kiểm tra trạng
  thái setup mà không cần tải runtime kênh.
- Phản chiếu setup kênh dựa trên env vào setup hiện tại, config kênh, hoặc siêu
  dữ liệu kênh của gói mà dạng plugin của bạn sử dụng.
- Chỉ giữ `channelEnvVars` làm siêu dữ liệu tương thích khi các phiên bản
  OpenClaw cũ hơn còn được hỗ trợ vẫn yêu cầu nó.
...
- Xóa `openclaw.security.json` cho đến khi OpenClaw ghi tài liệu một schema tệp
  kê khai bảo mật có phiên bản và hành vi ClawHub.
- Giữ hành vi nhạy cảm về bảo mật được ghi tài liệu trong tài liệu gói công khai
  hoặc README của bạn cho đến khi contract tệp kê khai tồn tại.
- Chạy lại `clawhub package validate <path-to-plugin>`.
