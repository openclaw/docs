...
x-i18n:
    generated_at: "2026-07-05T07:02:40Z"
    model: gpt-5.5
...
các lần quét gói tự động. Trang này bao gồm các phát hiện dành cho tác giả, tức là
...
chứa mã bảo trì của trình quét mà không có hướng dẫn khắc phục dành cho tác giả, các mã đó
dành cho maintainer của OpenClaw thay vì tác giả Plugin.
...
Gói có thể được đóng gói, nhưng tạo phẩm đã đóng gói không bao gồm các
...
Tạo phẩm đã đóng gói thiếu siêu dữ liệu OpenClaw tồn tại trong gói nguồn
của bạn.
...
Plugin vẫn dùng một helper ghi toàn bộ kho phiên đã ngừng khuyến nghị như
...
- Dùng `patchSessionEntry(...)` hoặc `upsertSessionEntry(...)` để lưu bền metadata
  phiên.
- Dùng danh tính transcript hoặc các helper target khi mã đang chuẩn bị một thao tác
  transcript.
...
- Phản chiếu metadata biến môi trường provider vào `setup.providers[].envVars`.
- Chỉ giữ `providerAuthEnvVars` làm metadata tương thích khi phạm vi OpenClaw
...
Manifest dùng metadata biến môi trường kênh legacy hoặc cũ hơn mà không có
metadata setup hoặc config hiện tại mà ClawHub mong đợi.
...
- Phản chiếu setup kênh dựa trên biến môi trường vào metadata setup, cấu hình kênh,
  hoặc metadata kênh gói hiện tại được dạng plugin của bạn sử dụng.
...
- Xóa `openclaw.security.json` cho đến khi OpenClaw tài liệu hóa một schema manifest
