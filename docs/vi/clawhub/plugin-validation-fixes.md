...
x-i18n:
    generated_at: "2026-07-04T10:46:31Z"
    model: gpt-5.5
...
ClawHub xác thực các gói Plugin trước khi xuất bản và cũng có thể hiển thị các phát hiện từ các lần quét gói tự động. Trang này bao gồm các phát hiện dành cho tác giả, tức là các phát hiện mà tác giả Plugin có thể sửa trong siêu dữ liệu gói, manifest, phần nhập SDK hoặc artifact đã xuất bản của họ.

Trang này không bao gồm các phát hiện về phạm vi bao phủ nội bộ của Plugin Inspector. Nếu một báo cáo đầy đủ chứa các mã bảo trì trình quét mà không có hướng dẫn khắc phục dành cho tác giả, các mã đó dành cho người bảo trì OpenClaw chứ không phải tác giả Plugin.
...
| `sdk-session-transcript-file-target`    | [Thay thế các đích tệp bản ghi phiên cũ](/vi/clawhub/plugin-validation-fixes#sdk-session-transcript-file-target)                   |
...
| `channel-env-vars`                      | [Phản chiếu biến môi trường của channel trong siêu dữ liệu hiện tại](/vi/clawhub/plugin-validation-fixes#channel-env-vars)                                |
...
- Sử dụng [Xây dựng Plugin](/vi/plugins/building-plugins) để xem ví dụ gói tối thiểu và [Manifest Plugin](/vi/plugins/manifest#manifest-versus-packagejson) để biết sự phân tách giữa gói và manifest.
...
- Xem [các trường package.json ảnh hưởng đến việc phát hiện](/vi/plugins/manifest#packagejson-fields-that-affect-discovery).
- Chạy lại `clawhub package validate <path-to-plugin>`.
...
- Thêm `openclaw.runtimeExtensions` khi gói đã xuất bản nên tải JavaScript đã build.
- Giữ tất cả đường dẫn entrypoint bên trong thư mục gói.
- Xem [Điểm vào Plugin](/vi/plugins/sdk-entrypoints) và [các trường package.json ảnh hưởng đến việc phát hiện](/vi/plugins/manifest#packagejson-fields-that-affect-discovery).
- Chạy lại `clawhub package validate <path-to-plugin>`.
...
- Sử dụng phiên bản API Plugin OpenClaw hoặc mức semver tối thiểu mà bạn đã build và kiểm thử.
...
Phiên bản host tối thiểu của gói không khớp với siêu dữ liệu phiên bản OpenClaw mà gói được build dựa trên đó.
...
- Căn chỉnh phiên bản host tối thiểu với phạm vi phiên bản host mà gói thực sự hỗ trợ.
- Xem [các trường package.json ảnh hưởng đến việc phát hiện](/vi/plugins/manifest#packagejson-fields-that-affect-discovery).
- Chạy lại `clawhub package validate <path-to-plugin>`.
...
Gói không thể được đóng gói thành artifact mà ClawHub sẽ kiểm tra hoặc xuất bản.

- Chạy `npm pack --dry-run` từ thư mục gốc của gói.
- Sửa siêu dữ liệu gói không hợp lệ, script vòng đời bị hỏng hoặc mục files khiến việc đóng gói thất bại.
- Xóa `private: true` nếu gói này dự định được xuất bản công khai.
- Chạy lại `clawhub package validate <path-to-plugin>`.
...
Plugin nhập một đường dẫn SDK dành riêng cho Plugin đi kèm hoặc khả năng tương thích
nội bộ.
...
Plugin vẫn dùng trình trợ giúp toàn bộ kho phiên đã ngừng khuyến nghị
`loadSessionStore`.
...
Plugin vẫn dùng một trình trợ giúp ghi toàn bộ kho phiên đã ngừng khuyến nghị như
...
- Tránh tải, sửa đổi và lưu toàn bộ đối tượng kho phiên.
- Chỉ giữ các trình trợ giúp ghi toàn bộ kho khi phạm vi tương thích đã khai báo của bạn
...
- Dùng danh tính bản ghi hoặc trình trợ giúp đích khi mã đang chuẩn bị một
  thao tác bản ghi.
- Không lưu hoặc phụ thuộc vào các đường dẫn tệp bản ghi cũ.
- Xem [API runtime](/vi/plugins/sdk-runtime#agent-session-state) và
  [Subpath SDK Plugin](/vi/plugins/sdk-subpaths).
- Chạy lại `clawhub package validate <path-to-plugin>`.
...
- Tránh đọc hoặc dựng trực tiếp các đích tệp bản ghi cũ.
...
- Dùng `appendSessionTranscriptMessageByIdentity(...)` cho thao tác thêm vào bản ghi.
- Dùng `publishSessionTranscriptUpdateByIdentity(...)` cho thông báo cập nhật bản ghi.
- Ưu tiên bề mặt runtime bản ghi có cấu trúc để OpenClaw có thể áp dụng đúng
  ranh giới giao dịch và xử lý danh tính.
...
- Sao chép siêu dữ liệu biến môi trường của nhà cung cấp vào `setup.providers[].envVars`.
...
- Sao chép setup kênh dựa trên biến môi trường vào setup, cấu hình kênh hoặc
  siêu dữ liệu kênh của gói hiện tại được dạng Plugin của bạn dùng.
...
- Chỉ dùng schema có phiên bản đã được ghi tài liệu sau khi OpenClaw phát hành.
- Chạy lại `clawhub package validate <path-to-plugin>`.
