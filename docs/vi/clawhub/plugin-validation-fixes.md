---
read_when:
    - Bạn đã chạy clawhub package validate và cần sửa các vấn đề được phát hiện về Plugin
...
x-i18n:
    generated_at: "2026-07-02T22:37:03Z"
    model: gpt-5.5
...
những phát hiện mà tác giả Plugin có thể sửa trong siêu dữ liệu gói, tệp kê khai, các lệnh nhập SDK
...
| `sdk-session-store-write`               | [Thay thế các thao tác ghi toàn bộ kho phiên](/vi/clawhub/plugin-validation-fixes#sdk-session-store-write)                                  |
...
| `security-manifest-schema-unavailable`  | [Xóa các tham chiếu schema tệp kê khai bảo mật không khả dụng](/vi/clawhub/plugin-validation-fixes#security-manifest-schema-unavailable) |
...
Gốc gói không bao gồm `package.json`, nên ClawHub không thể xác định
...
- Sử dụng [Xây dựng Plugin](/vi/plugins/building-plugins) để xem ví dụ gói
  tối thiểu và [Tệp kê khai Plugin](/vi/plugins/manifest#manifest-versus-packagejson)
  để biết phần tách giữa gói và tệp kê khai.
...
- Biên dịch gói nếu điểm vào được tạo vào `dist`.
...
- Sử dụng phiên bản API Plugin OpenClaw hoặc mức semver tối thiểu mà bạn đã xây dựng và kiểm thử
  dựa trên đó.
...
mà gói đã được xây dựng dựa trên đó.
...
- Căn chỉnh phiên bản máy chủ tối thiểu với khoảng phiên bản máy chủ mà gói
  thực sự hỗ trợ.
- Xem [các trường package.json ảnh hưởng đến việc khám phá](/vi/plugins/manifest#packagejson-fields-that-affect-discovery).
- Chạy lại `clawhub package validate <path-to-plugin>`.
...
- Giữ các điểm vào gói, siêu dữ liệu tương thích, cài đặt, thiết lập và danh mục
...
- Sửa siêu dữ liệu gói không hợp lệ, các script vòng đời bị hỏng hoặc các mục tệp
...
- Biên dịch các điểm vào được tạo trước khi đóng gói.
- Cập nhật `files`, `.npmignore` hoặc đầu ra bản dựng để các điểm vào đã khai báo
  được bao gồm.
- Xem [Các điểm vào Plugin](/vi/plugins/sdk-entrypoints).
- Chạy lại `clawhub package validate <path-to-plugin>`.
...
- Dùng `patchSessionEntry(...)` hoặc `upsertSessionEntry(...)` khi ghi trạng thái
  phiên.
- Tránh tải, biến đổi, rồi lưu toàn bộ đối tượng session store.
- Chỉ giữ `loadSessionStore(...)` khi dải tương thích đã khai báo của bạn
...
Plugin vẫn dùng một helper ghi toàn bộ session-store đã bị ngừng dùng như
...
- Tránh tải, biến đổi, rồi lưu toàn bộ đối tượng session store.
- Chỉ giữ các helper ghi whole-store khi dải tương thích đã khai báo của bạn
...
Plugin vẫn dùng các helper đường dẫn tệp phiên đã bị ngừng dùng như
`resolveSessionFilePath` hoặc `resolveAndPersistSessionFile`.

- Dùng `getSessionEntry(...)` để đọc metadata phiên theo agent và danh tính
  phiên.
- Dùng `patchSessionEntry(...)` hoặc `upsertSessionEntry(...)` để lưu bền metadata
  phiên.
- Dùng danh tính transcript hoặc các helper đích khi mã đang chuẩn bị một thao tác
  transcript.
...
Plugin vẫn dùng helper đích tệp transcript đã bị ngừng dùng
...
- Chỉ giữ helper legacy khi dải tương thích đã khai báo của bạn vẫn
...
Plugin vẫn dùng các helper transcript cấp thấp đã bị ngừng dùng như
`appendSessionTranscriptMessage` hoặc `emitSessionTranscriptUpdate`.

- Dùng `appendSessionTranscriptMessageByIdentity(...)` cho các thao tác thêm vào transcript.
...
- Chỉ giữ các helper transcript cấp thấp khi dải tương thích đã khai báo của bạn
  vẫn hỗ trợ các phiên bản OpenClaw cũ hơn yêu cầu chúng.
- Xem [API runtime](/vi/plugins/sdk-runtime#agent-session-state) và
  [Subpath SDK Plugin](/vi/plugins/sdk-subpaths).
- Chạy lại `clawhub package validate <path-to-plugin>`.
...
- Chuyển công việc biến đổi prompt hoặc context sang `before_prompt_build`.
- Chỉ giữ `before_agent_start` khi dải tương thích đã khai báo của bạn vẫn
  hỗ trợ các phiên bản OpenClaw cũ hơn yêu cầu nó.
...
Manifest vẫn dùng metadata xác thực provider legacy `providerAuthEnvVars`.

- Phản chiếu metadata env-var của provider vào `setup.providers[].envVars`.
- Chỉ giữ `providerAuthEnvVars` làm metadata tương thích khi dải OpenClaw được hỗ trợ
  của bạn vẫn cần nó.
...
Manifest dùng metadata env-var của kênh legacy hoặc cũ hơn mà không có metadata
...
- Giữ metadata env-var của kênh ở dạng khai báo để OpenClaw có thể kiểm tra trạng thái
  thiết lập mà không cần tải runtime kênh.
- Phản chiếu thiết lập kênh điều khiển bằng env vào setup hiện tại, config kênh, hoặc
  metadata kênh của gói mà hình dạng Plugin của bạn dùng.
