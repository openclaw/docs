...
x-i18n:
    generated_at: "2026-06-28T22:32:36Z"
    model: gpt-5.5
...
# Sửa lỗi xác thực Plugin

ClawHub xác thực các gói Plugin trước khi xuất bản và cũng có thể hiển thị các phát hiện từ các lượt quét gói tự động. Trang này bao gồm các phát hiện dành cho tác giả, nghĩa là các phát hiện mà tác giả Plugin có thể sửa trong siêu dữ liệu gói, manifest, lượt nhập SDK hoặc artifact đã xuất bản của gói.

Trang này không bao gồm các phát hiện về độ bao phủ nội bộ của Plugin Inspector. Nếu một báo cáo đầy đủ chứa các mã bảo trì bộ quét mà không có hướng dẫn khắc phục dành cho tác giả, các mã đó dành cho người bảo trì OpenClaw thay vì tác giả Plugin.
...
| `legacy-root-sdk-import`                | [Thay thế lượt nhập SDK gốc](/vi/clawhub/plugin-validation-fixes#legacy-root-sdk-import)                                             |
| `reserved-sdk-import`                   | [Xóa lượt nhập SDK dành riêng](/vi/clawhub/plugin-validation-fixes#reserved-sdk-import)                                             |
...
| `provider-auth-env-vars`                | [Chuyển biến môi trường nhà cung cấp sang siêu dữ liệu thiết lập](/vi/clawhub/plugin-validation-fixes#provider-auth-env-vars)                             |
...
Thư mục gốc của gói không bao gồm `package.json`, nên ClawHub không thể xác định gói npm, phiên bản, entrypoint hoặc siêu dữ liệu OpenClaw.

- Thêm `package.json` với `name`, `version` và `type`.
- Thêm khối `openclaw` khi gói cung cấp Plugin OpenClaw.
- Sử dụng [Xây dựng Plugin](/vi/plugins/building-plugins) để xem ví dụ gói tối thiểu và [Manifest Plugin](/vi/plugins/manifest#manifest-versus-packagejson) để hiểu phần tách giữa gói và manifest.
...
- Thêm siêu dữ liệu tương thích và cài đặt khi gói sẽ được xuất bản hoặc cài đặt thông qua ClawHub.
...
- Xem [Điểm entry Plugin](/vi/plugins/sdk-entrypoints) và [các trường package.json ảnh hưởng đến khám phá](/vi/plugins/manifest#packagejson-fields-that-affect-discovery).
- Chạy lại `clawhub package validate <path-to-plugin>`.
...
- Điền `openclaw.install` với nguồn cài đặt được hỗ trợ, chẳng hạn như `clawhubSpec`, `npmSpec` hoặc `localPath`.
- Đặt `openclaw.install.defaultChoice` khi có nhiều hơn một nguồn cài đặt khả dụng.
...
- Sử dụng phiên bản API Plugin OpenClaw hoặc ngưỡng semver mà bạn đã build và kiểm thử với nó.
- Giữ phần này tách biệt với phiên bản gói. Phiên bản gói mô tả bản phát hành Plugin; `openclaw.compat.pluginApi` mô tả hợp đồng API host.
- Xem [các trường package.json ảnh hưởng đến khám phá](/vi/plugins/manifest#packagejson-fields-that-affect-discovery).
- Chạy lại `clawhub package validate <path-to-plugin>`.

### package-min-host-version-drift

Phiên bản host tối thiểu của gói không khớp với siêu dữ liệu phiên bản OpenClaw mà gói đã được build dựa trên.

- Kiểm tra `openclaw.install.minHostVersion`.
- Kiểm tra mọi siêu dữ liệu build OpenClaw trong gói, chẳng hạn như phiên bản OpenClaw được sử dụng trong khi phát hành.
...
- Nếu `openclaw.plugin.json` cũng có `version`, hãy cập nhật để khớp hoặc xóa siêu dữ liệu phiên bản manifest cũ khi siêu dữ liệu gói là nguồn thẩm quyền.
...
Không thể đóng gói gói thành artifact mà ClawHub sẽ kiểm tra hoặc xuất bản.
...
Artifact đã đóng gói thiếu siêu dữ liệu OpenClaw tồn tại trong gói nguồn của bạn.
...
Manifest Plugin native không bao gồm tên hiển thị.
...
Plugin nhập từ barrel SDK gốc đã không còn được khuyến nghị:
`openclaw/plugin-sdk`.

- Thay thế các import từ barrel gốc bằng các import subpath công khai, tập trung hơn.
...
- Dùng [Quy ước import](/vi/plugins/building-plugins#import-conventions) và
  [Các subpath Plugin SDK](/vi/plugins/sdk-subpaths) để tìm import hẹp.
- Chạy lại `clawhub package validate <path-to-plugin>`.

### reserved-sdk-import

Plugin nhập một đường dẫn SDK được dành riêng cho plugin được đóng gói kèm hoặc
khả năng tương thích nội bộ.

- Thay thế các import SDK nội bộ OpenClaw được dành riêng bằng các subpath
...
- Dùng [Các subpath Plugin SDK](/vi/plugins/sdk-subpaths) và
  [Di chuyển SDK](/vi/plugins/sdk-migration) để chọn một import được hỗ trợ.
- Chạy lại `clawhub package validate <path-to-plugin>`.

### sdk-load-session-store

Plugin vẫn dùng helper toàn bộ session store đã không còn được khuyến nghị
`loadSessionStore`.
...
- Xem [API runtime](/vi/plugins/sdk-runtime#agent-session-state) và
  [Các subpath Plugin SDK](/vi/plugins/sdk-subpaths).
- Chạy lại `clawhub package validate <path-to-plugin>`.
...
- Chuyển công việc ghi đè mô hình hoặc provider sang `before_model_resolve`.
...
Manifest dùng siêu dữ liệu biến môi trường kênh cũ hoặc đời cũ mà không có
siêu dữ liệu setup hoặc config hiện tại mà ClawHub mong đợi.

- Giữ siêu dữ liệu biến môi trường kênh ở dạng khai báo để OpenClaw có thể kiểm tra
  trạng thái setup mà không cần tải runtime kênh.
- Phản chiếu setup kênh dựa trên biến môi trường vào siêu dữ liệu setup, config kênh, hoặc
  kênh gói hiện tại được hình dạng plugin của bạn sử dụng.
- Chỉ giữ `channelEnvVars` làm siêu dữ liệu tương thích khi các phiên bản
  OpenClaw cũ hơn được hỗ trợ vẫn cần nó.
...
- Xóa URL schema nếu nó chỉ mang tính tham khảo.
- Chỉ dùng schema có phiên bản đã được tài liệu hóa sau khi OpenClaw công bố schema đó.
- Chạy lại `clawhub package validate <path-to-plugin>`.
