...
x-i18n:
    generated_at: "2026-07-03T00:58:39Z"
    model: gpt-5.5
...
những phát hiện mà tác giả Plugin có thể sửa trong siêu dữ liệu gói, manifest, lượt nhập SDK
hoặc tạo phẩm đã xuất bản của họ.
...
Siêu dữ liệu gói tồn tại, nhưng không khai báo một điểm vào runtime
OpenClaw.
...
- Thêm `openclaw.runtimeExtensions` khi gói đã xuất bản cần tải JavaScript
  đã build.
- Giữ mọi đường dẫn điểm vào bên trong thư mục gói.
...
- Nếu `openclaw.plugin.json` cũng có `version`, hãy cập nhật để khớp hoặc xóa
  siêu dữ liệu phiên bản manifest lỗi thời khi siêu dữ liệu gói là nguồn có thẩm quyền.
...
## Tạo phẩm đã xuất bản

### package-npm-pack-unavailable

Gói không thể được đóng gói thành tạo phẩm mà ClawHub sẽ kiểm tra hoặc
xuất bản.

- Chạy `npm pack --dry-run` từ gốc gói.
- Sửa siêu dữ liệu gói không hợp lệ, script vòng đời bị hỏng hoặc các mục tệp khiến
...
Gói có thể được đóng gói, nhưng tạo phẩm đã đóng gói không bao gồm các tệp
...
- Thay thế các import root-barrel bằng import subpath công khai tập trung.
...
- Chỉ giữ `loadSessionStore(...)` khi dải tương thích đã khai báo của bạn vẫn
...
- Tránh tải, sửa đổi và lưu toàn bộ đối tượng kho phiên.
- Chỉ giữ các helper ghi toàn bộ kho khi dải tương thích đã khai báo của bạn vẫn
  hỗ trợ các phiên bản OpenClaw cũ hơn cần chúng.
- Xem [API runtime](/vi/plugins/sdk-runtime#agent-session-state) và
  [subpath Plugin SDK](/vi/plugins/sdk-subpaths).
- Chạy lại `clawhub package validate <path-to-plugin>`.
...
- Dùng `getSessionEntry(...)` để đọc siêu dữ liệu phiên theo agent và danh tính
  phiên.
- Dùng `patchSessionEntry(...)` hoặc `upsertSessionEntry(...)` để lưu bền siêu
  dữ liệu phiên.
- Dùng danh tính transcript hoặc các helper đích khi mã đang chuẩn bị thao tác
  transcript.
...
Plugin vẫn dùng helper đích tệp transcript đã ngừng khuyến nghị
...
- Tránh đọc hoặc xây dựng trực tiếp các đích tệp transcript legacy.
- Chỉ giữ helper legacy khi dải tương thích đã khai báo của bạn vẫn hỗ trợ các
  phiên bản OpenClaw cũ hơn cần nó.
- Xem [API runtime](/vi/plugins/sdk-runtime#agent-session-state) và
  [subpath Plugin SDK](/vi/plugins/sdk-subpaths).
- Chạy lại `clawhub package validate <path-to-plugin>`.
...
- Dùng `appendSessionTranscriptMessageByIdentity(...)` để nối thêm transcript.
...
- Chỉ giữ `before_agent_start` khi dải tương thích đã khai báo của bạn vẫn hỗ trợ
  các phiên bản OpenClaw cũ hơn cần nó.
...
Manifest vẫn dùng siêu dữ liệu xác thực nhà cung cấp legacy
`providerAuthEnvVars`.

- Phản chiếu siêu dữ liệu env-var của nhà cung cấp vào `setup.providers[].envVars`.
- Chỉ giữ `providerAuthEnvVars` làm siêu dữ liệu tương thích khi dải OpenClaw
...
Manifest dùng siêu dữ liệu env-var kênh legacy hoặc cũ hơn mà không có siêu dữ
liệu setup hoặc cấu hình hiện tại mà ClawHub kỳ vọng.
...
- Phản chiếu setup kênh do env điều khiển vào setup hiện tại, cấu hình kênh, hoặc
  siêu dữ liệu kênh của gói được hình dạng Plugin của bạn dùng.
...
- Chỉ dùng schema được định phiên bản đã ghi tài liệu sau khi OpenClaw phát hành
  một schema.
- Chạy lại `clawhub package validate <path-to-plugin>`.
...
- Xóa `openclaw.security.json` cho đến khi OpenClaw ghi tài liệu một schema
  manifest bảo mật được định phiên bản và hành vi ClawHub.
