...
x-i18n:
    generated_at: "2026-07-03T23:36:03Z"
    model: gpt-5.5
...
các lần quét gói tự động. Trang này đề cập đến các phát hiện dành cho tác giả, tức là
những phát hiện mà tác giả Plugin có thể sửa trong siêu dữ liệu gói, bản kê khai, phần nhập SDK
hoặc hiện vật đã phát hành của họ.

Trang này không đề cập đến các phát hiện về phạm vi kiểm tra nội bộ của Plugin Inspector. Nếu một báo cáo đầy đủ
chứa mã bảo trì bộ quét mà không có hướng dẫn khắc phục dành cho tác giả, thì các mã đó
...
| `package-manifest-version-drift`        | [Căn chỉnh phiên bản gói và bản kê khai](/vi/clawhub/plugin-validation-fixes#package-manifest-version-drift)                          |
...
| `package-npm-pack-unavailable`          | [Làm cho hiện vật npm có thể đóng gói](/vi/clawhub/plugin-validation-fixes#package-npm-pack-unavailable)                                 |
...
| `manifest-name-missing`                 | [Thêm tên hiển thị cho bản kê khai](/vi/clawhub/plugin-validation-fixes#manifest-name-missing)                                           |
| `manifest-unknown-fields`               | [Xóa các trường bản kê khai không được hỗ trợ](/vi/clawhub/plugin-validation-fixes#manifest-unknown-fields)                                  |
...
| `sdk-session-file-helper`               | [Thay thế các trợ giúp đường dẫn tệp phiên](/vi/clawhub/plugin-validation-fixes#sdk-session-file-helper)                                   |
| `sdk-session-transcript-file-target`    | [Thay thế các mục tiêu tệp bản chép lời cũ](/vi/clawhub/plugin-validation-fixes#sdk-session-transcript-file-target)                   |
| `sdk-session-transcript-low-level`      | [Thay thế các trợ giúp bản chép lời cấp thấp](/vi/clawhub/plugin-validation-fixes#sdk-session-transcript-low-level)                       |
...
| `security-manifest-schema-unavailable`  | [Xóa các tham chiếu lược đồ bản kê khai bảo mật không khả dụng](/vi/clawhub/plugin-validation-fixes#security-manifest-schema-unavailable) |
| `unrecognized-security-manifest`        | [Xóa các tệp bản kê khai bảo mật không được hỗ trợ](/vi/clawhub/plugin-validation-fixes#unrecognized-security-manifest)                   |
...
- Sử dụng [Xây dựng Plugin](/vi/plugins/building-plugins) để xem ví dụ gói tối thiểu
  và [Bản kê khai Plugin](/vi/plugins/manifest#manifest-versus-packagejson)
  để xem phần tách giữa gói và bản kê khai.
...
- Xem [các trường package.json ảnh hưởng đến khả năng khám phá](/vi/plugins/manifest#packagejson-fields-that-affect-discovery).
- Chạy lại `clawhub package validate <path-to-plugin>`.
...
- Xem [Các điểm vào Plugin](/vi/plugins/sdk-entrypoints) và
  [các trường package.json ảnh hưởng đến khả năng khám phá](/vi/plugins/manifest#packagejson-fields-that-affect-discovery).
- Chạy lại `clawhub package validate <path-to-plugin>`.
...
- Cập nhật siêu dữ liệu nếu điểm vào đã di chuyển.
...
- Giữ phần này tách biệt với phiên bản gói. Phiên bản gói mô tả
  bản phát hành Plugin; `openclaw.compat.pluginApi` mô tả hợp đồng API của máy chủ.
- Xem [các trường package.json ảnh hưởng đến khả năng khám phá](/vi/plugins/manifest#packagejson-fields-that-affect-discovery).
- Chạy lại `clawhub package validate <path-to-plugin>`.
...
Phiên bản gói và phiên bản bản kê khai Plugin không khớp nhau.
...
- Nếu `openclaw.plugin.json` cũng có `version`, hãy cập nhật để khớp hoặc xóa
  siêu dữ liệu phiên bản bản kê khai đã cũ khi siêu dữ liệu gói là nguồn có thẩm quyền.
...
## Hiện vật đã phát hành

### package-npm-pack-unavailable

Gói không thể được đóng gói thành hiện vật mà ClawHub sẽ kiểm tra hoặc
phát hành.

- Chạy `npm pack --dry-run` từ thư mục gốc của gói.
- Sửa siêu dữ liệu gói không hợp lệ, script vòng đời bị hỏng hoặc mục nhập tệp
...
Gói có thể được đóng gói, nhưng hiện vật đã đóng gói không bao gồm các
...
Hiện vật đã đóng gói thiếu siêu dữ liệu OpenClaw tồn tại trong gói
nguồn của bạn.

- Chạy `npm pack --dry-run` và kiểm tra các tệp siêu dữ liệu được bao gồm.
- Đảm bảo `package.json` bao gồm khối `openclaw` trong hiện vật đã đóng gói.
...
- Di chuyển hành vi runtime vào mã đăng ký Plugin, và giữ `contracts`
  giới hạn ở siêu dữ liệu quyền sở hữu năng lực tĩnh.
- Chạy lại `clawhub package validate <path-to-plugin>`.
...
- Thay thế các import root-barrel bằng các import subpath công khai tập trung.
...
Plugin nhập một đường dẫn SDK dành riêng cho Plugin được đóng gói sẵn hoặc khả năng
tương thích nội bộ.
...
- Dùng `patchSessionEntry(...)` hoặc `upsertSessionEntry(...)` khi ghi trạng thái
  phiên.
- Tránh tải, chỉnh sửa và lưu toàn bộ đối tượng kho phiên.
...
Plugin vẫn dùng một helper ghi toàn bộ kho phiên đã ngừng khuyến nghị, chẳng hạn như
...
Plugin vẫn dùng helper đích tệp bản ghi đã ngừng khuyến nghị
...
- Dùng `resolveSessionTranscriptTarget(...)` khi mã cần một đích thao tác bản ghi
  có cấu trúc.
- Tránh đọc hoặc xây dựng trực tiếp các đích tệp bản ghi legacy.
...
Plugin vẫn dùng các helper bản ghi cấp thấp đã ngừng khuyến nghị, chẳng hạn như
...
- Di chuyển công việc chỉnh sửa prompt hoặc ngữ cảnh sang `before_prompt_build`.
...
Bản kê khai vẫn dùng siêu dữ liệu xác thực provider legacy `providerAuthEnvVars`.
...
Bản kê khai dùng siêu dữ liệu biến môi trường kênh legacy hoặc cũ hơn mà không có
...
- Phản chiếu setup kênh dựa trên biến môi trường vào setup, cấu hình kênh hoặc
  siêu dữ liệu kênh gói hiện tại mà hình dạng Plugin của bạn dùng.
- Chỉ giữ `channelEnvVars` làm siêu dữ liệu tương thích trong khi các phiên bản
  OpenClaw cũ hơn được hỗ trợ vẫn yêu cầu nó.
...
Gói phân phối `openclaw.security.json` với một tham chiếu schema mà ClawHub
không nhận ra là có sẵn.

- Xóa URL schema nếu nó chỉ mang tính khuyến nghị.
- Chỉ dùng schema có phiên bản được ghi trong tài liệu sau khi OpenClaw phát hành.
- Chạy lại `clawhub package validate <path-to-plugin>`.

### unrecognized-security-manifest

Gói phân phối một tệp bản kê khai bảo mật không được hỗ trợ.

- Xóa `openclaw.security.json` cho đến khi OpenClaw ghi tài liệu schema bản kê khai
  bảo mật có phiên bản và hành vi ClawHub.
- Giữ hành vi nhạy cảm về bảo mật được ghi trong tài liệu gói công khai hoặc
