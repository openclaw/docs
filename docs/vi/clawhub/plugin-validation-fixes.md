...
x-i18n:
    generated_at: "2026-07-01T08:12:24Z"
    model: gpt-5.5
...
quá trình quét gói tự động. Trang này đề cập đến các phát hiện dành cho tác giả, nghĩa là
các phát hiện mà tác giả Plugin có thể sửa trong siêu dữ liệu gói, manifest, các import SDK
...
chứa mã bảo trì scanner mà không có hướng dẫn khắc phục dành cho tác giả, thì các mã đó
dành cho maintainer OpenClaw chứ không phải tác giả Plugin.
...
| `manifest-unknown-contracts`            | [Xóa khóa contract không được hỗ trợ](/vi/clawhub/plugin-validation-fixes#manifest-unknown-contracts)                                 |
...
| `provider-auth-env-vars`                | [Di chuyển biến môi trường provider sang siêu dữ liệu thiết lập](/vi/clawhub/plugin-validation-fixes#provider-auth-env-vars)                             |
...
- Thêm khối `openclaw` khi gói phát hành một Plugin OpenClaw.
- Sử dụng [Xây dựng Plugin](/vi/plugins/building-plugins) để xem ví dụ gói tối thiểu
  và [Plugin manifest](/vi/plugins/manifest#manifest-versus-packagejson)
  để biết cách tách gói và manifest.
...
Gói khai báo entrypoint OpenClaw, nhưng tệp được tham chiếu bị thiếu
...
Phiên bản gói và phiên bản Plugin manifest không khớp.
...
- Nếu `openclaw.plugin.json` cũng có `version`, hãy cập nhật để khớp hoặc xóa
  siêu dữ liệu phiên bản manifest cũ khi siêu dữ liệu gói là nguồn chính thức.
...
Không thể đóng gói gói thành artifact mà ClawHub sẽ kiểm tra hoặc
phát hành.
...
- Xóa `private: true` nếu gói này dự định phát hành công khai.
- Chạy lại `clawhub package validate <path-to-plugin>`.
...
Artifact đã đóng gói bị thiếu siêu dữ liệu OpenClaw có trong gói nguồn
của bạn.
...
- Chuyển hành vi thời gian chạy vào mã đăng ký Plugin, và giữ `contracts`
  chỉ giới hạn ở metadata quyền sở hữu năng lực tĩnh.
- Chạy lại `clawhub package validate <path-to-plugin>`.

## SDK và di trú tương thích

### legacy-root-sdk-import

Plugin import từ barrel SDK gốc đã bị ngừng dùng:
`openclaw/plugin-sdk`.
...
Plugin import một đường dẫn SDK dành riêng cho các Plugin đóng gói sẵn hoặc khả năng
tương thích nội bộ.
...
- Dùng [Các subpath Plugin SDK](/vi/plugins/sdk-subpaths) và
  [Di trú SDK](/vi/plugins/sdk-migration) để chọn import được hỗ trợ.
- Chạy lại `clawhub package validate <path-to-plugin>`.

### sdk-load-session-store

Plugin vẫn dùng helper toàn bộ kho phiên đã bị ngừng dùng
`loadSessionStore`.
...
- Dùng `patchSessionEntry(...)` hoặc `upsertSessionEntry(...)` khi ghi trạng thái
  phiên.
- Tránh tải, thay đổi và lưu toàn bộ đối tượng kho phiên.
...
Plugin vẫn dùng helper ghi toàn bộ kho phiên đã bị ngừng dùng, chẳng hạn như
...
- Tránh tải, thay đổi và lưu toàn bộ đối tượng kho phiên.
- Chỉ giữ các helper ghi toàn bộ kho trong khi phạm vi tương thích đã khai báo của bạn
...
- Dùng `patchSessionEntry(...)` hoặc `upsertSessionEntry(...)` để lưu metadata phiên.
- Dùng danh tính bản ghi hoặc các helper đích khi mã đang chuẩn bị một thao tác
  bản ghi.
...
- Chỉ giữ helper cũ trong khi phạm vi tương thích đã khai báo của bạn vẫn
...
- Chỉ giữ các helper bản ghi cấp thấp trong khi phạm vi tương thích đã khai báo của bạn
  vẫn hỗ trợ các phiên bản OpenClaw cũ hơn yêu cầu chúng.
- Xem [API thời gian chạy](/vi/plugins/sdk-runtime#agent-session-state) và
  [Các subpath Plugin SDK](/vi/plugins/sdk-subpaths).
- Chạy lại `clawhub package validate <path-to-plugin>`.
...
- Chuyển công việc ghi đè model hoặc nhà cung cấp sang `before_model_resolve`.
...
Tệp kê khai vẫn dùng metadata xác thực nhà cung cấp cũ `providerAuthEnvVars`.

- Phản chiếu metadata env-var của nhà cung cấp vào `setup.providers[].envVars`.
- Chỉ giữ `providerAuthEnvVars` làm metadata tương thích trong khi phạm vi OpenClaw
  được hỗ trợ của bạn vẫn cần nó.
- Xem [tham chiếu setup](/vi/plugins/manifest#setup-reference) và
  [Di trú SDK](/vi/plugins/sdk-migration).
- Chạy lại `clawhub package validate <path-to-plugin>`.

### channel-env-vars

Tệp kê khai dùng metadata env-var kênh cũ hoặc phiên bản cũ mà không có metadata
setup hoặc cấu hình hiện tại mà ClawHub mong đợi.

- Giữ metadata env-var của kênh ở dạng khai báo để OpenClaw có thể kiểm tra trạng thái
  setup mà không cần tải thời gian chạy kênh.
- Phản chiếu setup kênh dựa trên env vào metadata setup, cấu hình kênh, hoặc
  kênh gói hiện tại được hình dạng Plugin của bạn sử dụng.
- Chỉ giữ `channelEnvVars` làm metadata tương thích trong khi các phiên bản OpenClaw
...
Gói phân phối một tệp kê khai bảo mật không được hỗ trợ.

- Xóa `openclaw.security.json` cho đến khi OpenClaw ghi tài liệu về một schema tệp kê khai
