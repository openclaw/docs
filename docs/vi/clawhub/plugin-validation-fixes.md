...
summary: Sửa các phát hiện về xác thực gói Plugin ClawHub trước khi xuất bản
title: Các bản sửa lỗi xác thực Plugin
x-i18n:
    generated_at: "2026-07-04T20:33:32Z"
    model: gpt-5.5
...
Trang này không đề cập đến các phát hiện phạm vi kiểm tra nội bộ của Plugin Inspector. Nếu báo cáo đầy đủ
chứa mã bảo trì trình quét mà không có hướng dẫn khắc phục dành cho tác giả, các mã đó
...
| `sdk-session-store-write`               | [Thay thế thao tác ghi toàn bộ kho lưu trữ phiên](/vi/clawhub/plugin-validation-fixes#sdk-session-store-write)                                  |
...
| `provider-auth-env-vars`                | [Di chuyển biến môi trường nhà cung cấp sang siêu dữ liệu thiết lập](/vi/clawhub/plugin-validation-fixes#provider-auth-env-vars)                             |
...
- Thêm một khối `openclaw` khi gói cung cấp Plugin OpenClaw.
...
Artifact đã đóng gói đang thiếu siêu dữ liệu OpenClaw tồn tại trong gói
nguồn của bạn.
...
- Chỉ giữ `loadSessionStore(...)` trong khi phạm vi tương thích đã khai báo của
  bạn vẫn hỗ trợ các phiên bản OpenClaw cũ hơn yêu cầu nó.
...
- Tránh tải, thay đổi và lưu toàn bộ đối tượng session store.
- Chỉ giữ các helper ghi toàn bộ store trong khi phạm vi tương thích đã khai báo
...
- Dùng `patchSessionEntry(...)` hoặc `upsertSessionEntry(...)` để lưu bền siêu dữ
  liệu phiên.
- Dùng danh tính transcript hoặc helper mục tiêu khi mã đang chuẩn bị một thao
  tác transcript.
- Không lưu bền hoặc phụ thuộc vào các đường dẫn tệp transcript cũ.
- Xem [API thời gian chạy](/vi/plugins/sdk-runtime#agent-session-state) và
  [đường dẫn con SDK Plugin](/vi/plugins/sdk-subpaths).
- Chạy lại `clawhub package validate <path-to-plugin>`.
...
Plugin vẫn dùng helper mục tiêu tệp transcript đã lỗi thời
...
- Tránh đọc hoặc dựng trực tiếp các mục tiêu tệp transcript cũ.
- Chỉ giữ helper cũ trong khi phạm vi tương thích đã khai báo của bạn vẫn hỗ trợ
  các phiên bản OpenClaw cũ hơn yêu cầu nó.
- Xem [API thời gian chạy](/vi/plugins/sdk-runtime#agent-session-state) và
  [đường dẫn con SDK Plugin](/vi/plugins/sdk-subpaths).
- Chạy lại `clawhub package validate <path-to-plugin>`.
...
Plugin vẫn dùng các helper transcript cấp thấp đã lỗi thời như
`appendSessionTranscriptMessage` hoặc `emitSessionTranscriptUpdate`.

- Dùng `appendSessionTranscriptMessageByIdentity(...)` để thêm vào transcript.
- Dùng `publishSessionTranscriptUpdateByIdentity(...)` cho thông báo cập nhật
  transcript.
- Ưu tiên bề mặt thời gian chạy transcript có cấu trúc để OpenClaw có thể áp
  dụng đúng ranh giới giao dịch và cách xử lý danh tính.
- Chỉ giữ các helper transcript cấp thấp trong khi phạm vi tương thích đã khai
  báo của bạn vẫn hỗ trợ các phiên bản OpenClaw cũ hơn yêu cầu chúng.
- Xem [API thời gian chạy](/vi/plugins/sdk-runtime#agent-session-state) và
  [đường dẫn con SDK Plugin](/vi/plugins/sdk-subpaths).
- Chạy lại `clawhub package validate <path-to-plugin>`.
...
- Chỉ giữ `before_agent_start` trong khi phạm vi tương thích đã khai báo của bạn
...
Bản kê khai vẫn dùng siêu dữ liệu xác thực nhà cung cấp `providerAuthEnvVars` cũ.
...
- Giữ siêu dữ liệu biến môi trường kênh ở dạng khai báo để OpenClaw có thể kiểm
  tra trạng thái setup mà không tải thời gian chạy kênh.
- Phản chiếu setup kênh dựa trên biến môi trường vào setup hiện tại, cấu hình
  kênh, hoặc siêu dữ liệu kênh gói mà dạng Plugin của bạn sử dụng.
...
Gói phát hành `openclaw.security.json` với tham chiếu schema mà ClawHub không
nhận ra là có sẵn.

- Xóa URL schema nếu nó chỉ mang tính tư vấn.
- Chỉ dùng schema được đánh phiên bản đã ghi tài liệu sau khi OpenClaw xuất bản.
- Chạy lại `clawhub package validate <path-to-plugin>`.

### unrecognized-security-manifest

Gói phát hành một tệp bản kê khai bảo mật không được hỗ trợ.

- Xóa `openclaw.security.json` cho đến khi OpenClaw ghi tài liệu schema bản kê
  khai bảo mật được đánh phiên bản và hành vi ClawHub.
