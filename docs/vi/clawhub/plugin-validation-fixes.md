...
x-i18n:
    generated_at: "2026-07-01T15:26:11Z"
    model: gpt-5.5
...
những lần quét gói tự động. Trang này bao gồm các phát hiện dành cho tác giả, nghĩa là
...
chứa mã bảo trì trình quét mà không có hướng dẫn khắc phục dành cho tác giả, những mã đó
...
| `package-openclaw-entry-missing`        | [Khai báo điểm vào gói OpenClaw](/vi/clawhub/plugin-validation-fixes#package-openclaw-entry-missing)                               |
...
| `package-npm-pack-entrypoint-missing`   | [Đưa điểm vào vào đầu ra npm pack](/vi/clawhub/plugin-validation-fixes#package-npm-pack-entrypoint-missing)                         |
...
| `legacy-root-sdk-import`                | [Thay thế phần nhập SDK gốc](/vi/clawhub/plugin-validation-fixes#legacy-root-sdk-import)                                            |
| `reserved-sdk-import`                   | [Xóa phần nhập SDK dành riêng](/vi/clawhub/plugin-validation-fixes#reserved-sdk-import)                                             |
...
| `sdk-session-transcript-file-target`    | [Thay thế mục tiêu tệp transcript cũ](/vi/clawhub/plugin-validation-fixes#sdk-session-transcript-file-target)                       |
...
| `unrecognized-security-manifest`        | [Xóa tệp manifest bảo mật không được hỗ trợ](/vi/clawhub/plugin-validation-fixes#unrecognized-security-manifest)                    |

## Siêu dữ liệu gói

### package-json-missing

Thư mục gốc của gói không có `package.json`, vì vậy ClawHub không thể xác định
...
Gói không khai báo khoảng API Plugin OpenClaw mà nó hỗ trợ.
...
- Giữ phần này tách biệt với phiên bản gói. Phiên bản gói mô tả
  bản phát hành Plugin; `openclaw.compat.pluginApi` mô tả contract API của host.
- Xem [các trường package.json ảnh hưởng đến khám phá](/vi/plugins/manifest#packagejson-fields-that-affect-discovery).
- Chạy lại `clawhub package validate <path-to-plugin>`.
...
- Căn chỉnh phiên bản host tối thiểu với khoảng phiên bản host mà gói
  thực sự hỗ trợ.
- Xem [các trường package.json ảnh hưởng đến khám phá](/vi/plugins/manifest#packagejson-fields-that-affect-discovery).
- Chạy lại `clawhub package validate <path-to-plugin>`.
...
- Nếu `openclaw.plugin.json` cũng có `version`, hãy cập nhật để khớp hoặc xóa
  siêu dữ liệu phiên bản manifest lỗi thời khi siêu dữ liệu gói là nguồn chính.
...
- Giữ điểm vào gói, khả năng tương thích, cài đặt, thiết lập và siêu dữ liệu catalog
...
- Sửa siêu dữ liệu gói không hợp lệ, script lifecycle bị hỏng hoặc mục nhập files khiến
...
điểm vào đã khai báo trong `package.json#openclaw`.
...
Artifact đã đóng gói bị thiếu siêu dữ liệu OpenClaw đang tồn tại trong gói
nguồn của bạn.
...
- Chuyển siêu dữ liệu gói hoặc cài đặt vào các trường `package.json#openclaw`
  được hỗ trợ thay vì đặt trong tệp kê khai.
- Chạy lại `clawhub package validate <path-to-plugin>`.
...
- Thay thế các import từ root-barrel bằng các import đường dẫn con công khai tập trung.
...
Plugin nhập một đường dẫn SDK dành riêng cho các Plugin đóng gói sẵn hoặc khả năng
tương thích nội bộ.

- Thay thế các import SDK nội bộ dành riêng của OpenClaw bằng các đường dẫn con
...
Plugin vẫn dùng helper toàn bộ session-store đã lỗi thời
`loadSessionStore`.
...
- Chỉ giữ `loadSessionStore(...)` khi phạm vi tương thích bạn khai báo
  vẫn hỗ trợ các phiên bản OpenClaw cũ cần helper này.
...
Plugin vẫn dùng helper ghi toàn bộ session-store đã lỗi thời như
...
- Tránh tải, thay đổi và lưu toàn bộ đối tượng session store.
- Chỉ giữ các helper ghi toàn bộ store khi phạm vi tương thích bạn khai báo
  vẫn hỗ trợ các phiên bản OpenClaw cũ cần chúng.
- Xem [API runtime](/vi/plugins/sdk-runtime#agent-session-state) và
  [đường dẫn con Plugin SDK](/vi/plugins/sdk-subpaths).
- Chạy lại `clawhub package validate <path-to-plugin>`.
...
Plugin vẫn dùng các helper đường dẫn tệp phiên đã lỗi thời như
`resolveSessionFilePath` hoặc `resolveAndPersistSessionFile`.

- Dùng `getSessionEntry(...)` để đọc siêu dữ liệu phiên theo định danh agent và
  phiên.
- Dùng `patchSessionEntry(...)` hoặc `upsertSessionEntry(...)` để lưu bền siêu dữ liệu
  phiên.
- Dùng định danh bản ghi hoặc các helper mục tiêu khi mã đang chuẩn bị một thao tác
  bản ghi.
- Không lưu bền hoặc phụ thuộc vào các đường dẫn tệp bản ghi legacy.
- Xem [API runtime](/vi/plugins/sdk-runtime#agent-session-state) và
  [đường dẫn con Plugin SDK](/vi/plugins/sdk-subpaths).
- Chạy lại `clawhub package validate <path-to-plugin>`.
...
Plugin vẫn dùng helper mục tiêu tệp bản ghi đã lỗi thời
`resolveSessionTranscriptLegacyFileTarget`.

- Dùng `resolveSessionTranscriptIdentity(...)` khi mã chỉ cần định danh phiên
  công khai.
- Dùng `resolveSessionTranscriptTarget(...)` khi mã cần một mục tiêu thao tác
  bản ghi có cấu trúc.
- Tránh đọc hoặc tạo trực tiếp các mục tiêu tệp bản ghi legacy.
- Chỉ giữ helper legacy khi phạm vi tương thích bạn khai báo vẫn
  hỗ trợ các phiên bản OpenClaw cũ cần nó.
- Xem [API runtime](/vi/plugins/sdk-runtime#agent-session-state) và
  [đường dẫn con Plugin SDK](/vi/plugins/sdk-subpaths).
- Chạy lại `clawhub package validate <path-to-plugin>`.
...
Plugin vẫn dùng các helper bản ghi cấp thấp đã lỗi thời như
`appendSessionTranscriptMessage` hoặc `emitSessionTranscriptUpdate`.

- Dùng `appendSessionTranscriptMessageByIdentity(...)` cho các thao tác thêm vào bản ghi.
- Dùng `publishSessionTranscriptUpdateByIdentity(...)` cho các thông báo cập nhật
  bản ghi.
- Ưu tiên bề mặt runtime bản ghi có cấu trúc để OpenClaw có thể áp dụng
  đúng ranh giới giao dịch và xử lý định danh.
- Chỉ giữ các helper bản ghi cấp thấp khi phạm vi tương thích bạn khai báo
  vẫn hỗ trợ các phiên bản OpenClaw cũ cần chúng.
- Xem [API runtime](/vi/plugins/sdk-runtime#agent-session-state) và
  [đường dẫn con Plugin SDK](/vi/plugins/sdk-subpaths).
- Chạy lại `clawhub package validate <path-to-plugin>`.
...
- Chỉ giữ `before_agent_start` khi phạm vi tương thích bạn khai báo vẫn
  hỗ trợ các phiên bản OpenClaw cũ cần nó.
...
Tệp kê khai vẫn dùng siêu dữ liệu xác thực nhà cung cấp legacy `providerAuthEnvVars`.
...
Tệp kê khai dùng siêu dữ liệu biến môi trường kênh legacy hoặc cũ hơn mà không có
...
- Phản chiếu setup kênh dựa trên biến môi trường vào siêu dữ liệu setup, config kênh,
  hoặc gói kênh hiện tại mà kiểu Plugin của bạn sử dụng.
- Chỉ giữ `channelEnvVars` làm siêu dữ liệu tương thích khi các phiên bản OpenClaw
  cũ hơn còn được hỗ trợ vẫn cần nó.
...
Gói phát hành `openclaw.security.json` với một tham chiếu schema mà ClawHub
không nhận ra là có sẵn.

- Xóa URL schema nếu nó chỉ mang tính tư vấn.
- Chỉ dùng schema có phiên bản đã được ghi tài liệu sau khi OpenClaw xuất bản.
- Chạy lại `clawhub package validate <path-to-plugin>`.
...
- Xóa `openclaw.security.json` cho đến khi OpenClaw ghi tài liệu một schema tệp kê khai
  bảo mật có phiên bản và hành vi ClawHub.
- Tiếp tục ghi tài liệu hành vi nhạy cảm về bảo mật trong tài liệu gói công khai hoặc
