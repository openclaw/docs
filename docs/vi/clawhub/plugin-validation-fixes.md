...
summary: Khắc phục các phát hiện trong quá trình xác thực gói plugin ClawHub trước khi xuất bản
title: Sửa lỗi xác thực Plugin
x-i18n:
    generated_at: "2026-07-01T20:26:29Z"
    model: gpt-5.5
...
các phát hiện mà tác giả Plugin có thể sửa trong siêu dữ liệu gói, manifest, phần
import SDK, hoặc tạo tác đã xuất bản của họ.

Trang này không bao gồm các phát hiện về độ bao phủ nội bộ của Plugin Inspector. Nếu một báo cáo đầy đủ
chứa mã bảo trì trình quét mà không có hướng dẫn khắc phục cho tác giả, các mã đó
...
| `package-entrypoint-missing`            | [Xuất bản điểm vào đã khai báo](/vi/clawhub/plugin-validation-fixes#package-entrypoint-missing)                                  |
...
| `security-manifest-schema-unavailable`  | [Gỡ bỏ tham chiếu schema manifest bảo mật không khả dụng](/vi/clawhub/plugin-validation-fixes#security-manifest-schema-unavailable) |
...
- Nếu `openclaw.plugin.json` cũng có `version`, hãy cập nhật để khớp hoặc gỡ bỏ
  siêu dữ liệu phiên bản manifest đã cũ khi siêu dữ liệu gói là nguồn đáng tin cậy.
- Xuất bản một phiên bản gói mới sau khi thay đổi siêu dữ liệu đã xuất bản.
...
- Giữ các điểm vào gói, thông tin tương thích, cài đặt, thiết lập và siêu dữ liệu danh mục
...
- Sửa siêu dữ liệu gói không hợp lệ, script vòng đời bị hỏng, hoặc các mục files khiến
  quá trình đóng gói thất bại.
- Gỡ bỏ `private: true` nếu gói này được dự định xuất bản công khai.
- Chạy lại `clawhub package validate <path-to-plugin>`.
...
Tạo tác đã đóng gói thiếu siêu dữ liệu OpenClaw tồn tại trong gói
nguồn của bạn.
...
Plugin nhập một đường dẫn SDK dành riêng cho Plugin đóng gói hoặc khả năng tương thích
nội bộ.
...
- Dùng [subpath Plugin SDK](/vi/plugins/sdk-subpaths) và
  [di trú SDK](/vi/plugins/sdk-migration) để chọn một import được hỗ trợ.
- Chạy lại `clawhub package validate <path-to-plugin>`.

### sdk-load-session-store

Plugin vẫn dùng helper whole-session-store đã lỗi thời
`loadSessionStore`.
...
Plugin vẫn dùng một helper ghi whole-session-store đã lỗi thời như
...
- Tránh tải, sửa đổi và lưu toàn bộ đối tượng session store.
- Chỉ giữ các helper ghi whole-store trong khi phạm vi tương thích đã khai báo của bạn
...
- Dùng `getSessionEntry(...)` để đọc siêu dữ liệu phiên theo agent và danh tính
  phiên.
...
- Dùng danh tính transcript hoặc helper target khi mã đang chuẩn bị một
  thao tác transcript.
- Không lưu hoặc phụ thuộc vào các đường dẫn tệp transcript cũ.
- Xem [API runtime](/vi/plugins/sdk-runtime#agent-session-state) và
  [subpath Plugin SDK](/vi/plugins/sdk-subpaths).
- Chạy lại `clawhub package validate <path-to-plugin>`.
...
- Dùng `resolveSessionTranscriptIdentity(...)` khi mã chỉ cần danh tính phiên
  công khai.
- Dùng `resolveSessionTranscriptTarget(...)` khi mã cần một target thao tác
  transcript có cấu trúc.
- Tránh đọc hoặc xây dựng trực tiếp các target tệp transcript cũ.
...
- Dùng `appendSessionTranscriptMessageByIdentity(...)` cho thao tác thêm transcript.
...
Manifest dùng siêu dữ liệu env-var kênh cũ hoặc đời cũ mà không có siêu dữ liệu
setup hoặc cấu hình hiện tại ClawHub mong đợi.

- Giữ siêu dữ liệu env-var của kênh ở dạng khai báo để OpenClaw có thể kiểm tra trạng thái
  setup mà không cần tải runtime kênh.
- Phản chiếu setup kênh do env điều khiển vào setup hiện tại, cấu hình kênh, hoặc
  siêu dữ liệu kênh gói mà hình dạng Plugin của bạn dùng.
