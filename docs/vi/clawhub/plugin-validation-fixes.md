...
x-i18n:
    generated_at: "2026-07-05T05:16:05Z"
    model: gpt-5.5
...
những phát hiện mà tác giả Plugin có thể sửa trong siêu dữ liệu gói, manifest, các import SDK
hoặc artifact đã xuất bản của họ.

Nội dung này không bao gồm các phát hiện về phạm vi bao phủ nội bộ của Plugin Inspector. Nếu một báo cáo đầy đủ
...
| `sdk-session-transcript-file-target`    | [Thay thế các đích tệp transcript legacy](/vi/clawhub/plugin-validation-fixes#sdk-session-transcript-file-target)                   |
...
| `provider-auth-env-vars`                | [Chuyển các biến môi trường nhà cung cấp sang siêu dữ liệu thiết lập](/vi/clawhub/plugin-validation-fixes#provider-auth-env-vars)                             |
...
- Thêm `openclaw.runtimeExtensions` khi gói đã xuất bản cần tải JavaScript
  đã build.
...
- Sử dụng phiên bản API Plugin OpenClaw hoặc semver floor mà bạn đã build và kiểm thử
  dựa trên đó.
- Giữ phần này tách biệt với phiên bản gói. Phiên bản gói mô tả
  bản phát hành Plugin; `openclaw.compat.pluginApi` mô tả contract API máy chủ.
- Xem [các trường package.json ảnh hưởng đến việc khám phá](/vi/plugins/manifest#packagejson-fields-that-affect-discovery).
- Chạy lại `clawhub package validate <path-to-plugin>`.
...
- Nếu `openclaw.plugin.json` cũng có `version`, hãy cập nhật cho khớp hoặc xóa
  siêu dữ liệu phiên bản manifest đã lỗi thời khi siêu dữ liệu gói là nguồn đáng tin cậy.
...
- Giữ entrypoint gói, siêu dữ liệu tương thích, cài đặt, thiết lập và catalog
...
- Sửa siêu dữ liệu gói không hợp lệ, lifecycle script bị hỏng hoặc các mục files
  khiến việc đóng gói thất bại.
- Xóa `private: true` nếu gói này dự kiến được xuất bản công khai.
- Chạy lại `clawhub package validate <path-to-plugin>`.
...
- So sánh từng trường cấp cao nhất với
  [tài liệu tham chiếu trường bản kê khai](/vi/plugins/manifest#top-level-field-reference).
...
- Chuyển hành vi runtime vào mã đăng ký Plugin, và giữ `contracts`
  giới hạn ở siêu dữ liệu quyền sở hữu capability tĩnh.
- Chạy lại `clawhub package validate <path-to-plugin>`.
...
- Thay thế các lệnh nhập root-barrel bằng các lệnh nhập subpath công khai tập trung.
...
- Dùng [Quy ước nhập](/vi/plugins/building-plugins#import-conventions) và
  [Subpath SDK Plugin](/vi/plugins/sdk-subpaths) để tìm lệnh nhập hẹp.
- Chạy lại `clawhub package validate <path-to-plugin>`.

### reserved-sdk-import

Plugin nhập một đường dẫn SDK dành riêng cho Plugin đóng gói sẵn hoặc khả năng
tương thích nội bộ.

- Thay thế các lệnh nhập SDK nội bộ OpenClaw dành riêng bằng các subpath
...
Plugin vẫn dùng một helper ghi toàn bộ session-store đã ngừng khuyến nghị như
...
- Dùng `patchSessionEntry(...)` hoặc `upsertSessionEntry(...)` để lưu bền siêu dữ
  liệu phiên.
- Dùng danh tính bản ghi hội thoại hoặc các helper đích khi mã đang chuẩn bị một
  thao tác bản ghi hội thoại.
- Không lưu bền hoặc phụ thuộc vào các đường dẫn tệp bản ghi hội thoại cũ.
- Xem [API Runtime](/vi/plugins/sdk-runtime#agent-session-state) và
  [Subpath SDK Plugin](/vi/plugins/sdk-subpaths).
- Chạy lại `clawhub package validate <path-to-plugin>`.
...
Plugin vẫn dùng helper đích tệp bản ghi hội thoại đã ngừng khuyến nghị
...
- Tránh đọc hoặc dựng trực tiếp các đích tệp bản ghi hội thoại cũ.
...
Plugin vẫn dùng các helper bản ghi hội thoại cấp thấp đã ngừng khuyến nghị như
`appendSessionTranscriptMessage` hoặc `emitSessionTranscriptUpdate`.

- Dùng `appendSessionTranscriptMessageByIdentity(...)` để thêm vào bản ghi hội thoại.
- Dùng `publishSessionTranscriptUpdateByIdentity(...)` cho thông báo cập nhật bản
  ghi hội thoại.
- Ưu tiên bề mặt runtime bản ghi hội thoại có cấu trúc để OpenClaw có thể áp dụng
  đúng ranh giới giao dịch và xử lý danh tính.
- Chỉ giữ các helper bản ghi hội thoại cấp thấp khi phạm vi tương thích đã khai báo
  của bạn vẫn hỗ trợ các phiên bản OpenClaw cũ hơn yêu cầu chúng.
- Xem [API Runtime](/vi/plugins/sdk-runtime#agent-session-state) và
  [Subpath SDK Plugin](/vi/plugins/sdk-subpaths).
- Chạy lại `clawhub package validate <path-to-plugin>`.
...
Bản kê khai vẫn dùng siêu dữ liệu xác thực provider cũ `providerAuthEnvVars`.
...
Bản kê khai dùng siêu dữ liệu biến môi trường kênh cũ hoặc đời cũ mà không có
...
Gói gửi kèm `openclaw.security.json` với một tham chiếu schema mà ClawHub
...
Gói gửi kèm một tệp bản kê khai bảo mật không được hỗ trợ.
...
- Giữ hành vi nhạy cảm về bảo mật được ghi tài liệu trong tài liệu gói công khai
  hoặc README của bạn cho đến khi contract bản kê khai tồn tại.
- Chạy lại `clawhub package validate <path-to-plugin>`.
