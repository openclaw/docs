...
x-i18n:
    generated_at: "2026-06-30T14:06:54Z"
    model: gpt-5.5
...
| `provider-auth-env-vars`                | [Chuyển biến môi trường của nhà cung cấp vào siêu dữ liệu thiết lập](/vi/clawhub/plugin-validation-fixes#provider-auth-env-vars)                             |
...
Gốc gói không bao gồm `package.json`, vì vậy ClawHub không thể xác định
...
- Giữ siêu dữ liệu điểm vào gói, khả năng tương thích, cài đặt, thiết lập và danh mục
...
- Sửa siêu dữ liệu gói không hợp lệ, script vòng đời bị hỏng hoặc các mục files
...
Artifact đã đóng gói bị thiếu siêu dữ liệu OpenClaw vốn tồn tại trong gói
nguồn của bạn.

- Chạy `npm pack --dry-run` và kiểm tra các tệp siêu dữ liệu được đưa vào.
- Đảm bảo `package.json` bao gồm khối `openclaw` trong artifact đã đóng gói.
- Đảm bảo `openclaw.plugin.json` được đưa vào khi gói là một Plugin OpenClaw
  gốc.
...
- Thay vào đó, chuyển siêu dữ liệu gói hoặc cài đặt vào các trường
  `package.json#openclaw` được hỗ trợ.
- Chạy lại `clawhub package validate <path-to-plugin>`.
...
- Thay thế các lệnh nhập từ barrel gốc bằng các lệnh nhập đường dẫn con công khai tập trung.
...
- Thay thế các lệnh nhập SDK nội bộ OpenClaw được dành riêng bằng các đường dẫn con
...
Plugin vẫn dùng trình trợ giúp toàn bộ kho phiên đã lỗi thời
`loadSessionStore`.
...
Plugin vẫn dùng một trình trợ giúp ghi toàn bộ kho phiên đã lỗi thời, chẳng hạn như
...
Plugin vẫn dùng các trình trợ giúp đường dẫn tệp phiên đã lỗi thời, chẳng hạn như
...
Plugin vẫn dùng trình trợ giúp đích tệp bản ghi đã lỗi thời
...
Plugin vẫn dùng các trình trợ giúp bản ghi cấp thấp đã lỗi thời, chẳng hạn như
`appendSessionTranscriptMessage` hoặc `emitSessionTranscriptUpdate`.

- Dùng `appendSessionTranscriptMessageByIdentity(...)` để nối thêm bản ghi.
...
Manifest vẫn dùng siêu dữ liệu xác thực nhà cung cấp cũ `providerAuthEnvVars`.

- Phản chiếu siêu dữ liệu biến môi trường nhà cung cấp vào `setup.providers[].envVars`.
- Chỉ giữ `providerAuthEnvVars` làm siêu dữ liệu tương thích khi phạm vi OpenClaw được hỗ trợ
  của bạn vẫn cần nó.
...
- Giữ siêu dữ liệu biến môi trường kênh ở dạng khai báo để OpenClaw có thể kiểm tra trạng thái setup
  mà không tải thời gian chạy kênh.
- Phản chiếu setup kênh dựa trên biến môi trường vào siêu dữ liệu setup, cấu hình kênh, hoặc
...
- Chỉ dùng schema có phiên bản đã được ghi tài liệu sau khi OpenClaw xuất bản schema đó.
- Chạy lại `clawhub package validate <path-to-plugin>`.
...
- Giữ hành vi nhạy cảm về bảo mật trong tài liệu gói công khai hoặc
