...
x-i18n:
    generated_at: "2026-07-02T01:00:48Z"
    model: gpt-5.5
...
Trang này không đề cập đến các phát hiện về độ bao phủ nội bộ của Plugin Inspector. Nếu một báo cáo đầy đủ
chứa các mã bảo trì trình quét mà không có hướng dẫn khắc phục dành cho tác giả, thì những mã đó
...
- Nếu `openclaw.plugin.json` cũng có `version`, hãy cập nhật để khớp hoặc xóa
  siêu dữ liệu phiên bản manifest lỗi thời khi siêu dữ liệu gói là nguồn chính xác.
...
Plugin import một đường dẫn SDK được dành riêng cho Plugin đóng gói kèm hoặc
...
- Dùng `patchSessionEntry(...)` khi cập nhật các trường trên một mục session
  hiện có.
- Dùng `upsertSessionEntry(...)` khi thay thế hoặc tạo một mục session.
- Tránh tải, thay đổi và lưu toàn bộ đối tượng session store.
- Chỉ giữ các helper ghi toàn bộ store trong khi dải tương thích đã khai báo của bạn
...
Plugin vẫn dùng các helper đường dẫn tệp session đã ngừng khuyến nghị như
`resolveSessionFilePath` hoặc `resolveAndPersistSessionFile`.

- Dùng `getSessionEntry(...)` để đọc metadata session theo agent và danh tính session.
- Dùng `patchSessionEntry(...)` hoặc `upsertSessionEntry(...)` để lưu bền metadata session.
...
- Chỉ giữ helper cũ trong khi dải tương thích đã khai báo của bạn vẫn
...
- Dùng `appendSessionTranscriptMessageByIdentity(...)` cho thao tác nối thêm transcript.
...
- Chỉ giữ các helper transcript cấp thấp trong khi dải tương thích đã khai báo của bạn
  vẫn hỗ trợ các phiên bản OpenClaw cũ hơn yêu cầu chúng.
- Xem [API runtime](/vi/plugins/sdk-runtime#agent-session-state) và
  [subpath Plugin SDK](/vi/plugins/sdk-subpaths).
- Chạy lại `clawhub package validate <path-to-plugin>`.
...
Manifest vẫn dùng metadata xác thực provider `providerAuthEnvVars` cũ.

- Phản chiếu metadata env-var của provider vào `setup.providers[].envVars`.
- Chỉ giữ `providerAuthEnvVars` làm metadata tương thích trong khi dải OpenClaw
...
Manifest dùng metadata env-var kênh cũ hoặc đời cũ mà không có metadata setup
hoặc config hiện tại mà ClawHub mong đợi.

- Giữ metadata env-var kênh ở dạng khai báo để OpenClaw có thể kiểm tra trạng thái setup
  mà không cần tải runtime kênh.
- Phản chiếu setup kênh dựa trên env vào metadata setup, config kênh hoặc
  metadata kênh của gói hiện tại được dùng bởi hình dạng Plugin của bạn.
