...
x-i18n:
    generated_at: "2026-06-27T17:15:31Z"
    model: gpt-5.5
...
| `provider-auth-env-vars`                | [Di chuyển biến môi trường của nhà cung cấp vào siêu dữ liệu thiết lập](/vi/clawhub/plugin-validation-fixes#provider-auth-env-vars)                             |
...
| `security-manifest-schema-unavailable`  | [Xóa tham chiếu lược đồ manifest bảo mật không khả dụng](/vi/clawhub/plugin-validation-fixes#security-manifest-schema-unavailable) |
...
- Giữ phần này tách biệt với phiên bản gói. Phiên bản gói mô tả
  bản phát hành Plugin; `openclaw.compat.pluginApi` mô tả hợp đồng API của host.
- Xem [các trường package.json ảnh hưởng đến phát hiện](/vi/plugins/manifest#packagejson-fields-that-affect-discovery).
- Chạy lại `clawhub package validate <path-to-plugin>`.
...
- Chuyển metadata gói hoặc cài đặt vào các trường `package.json#openclaw`
...
- Thay thế các import từ barrel gốc bằng các import đường dẫn con công khai tập trung.
...
- Dùng [Quy ước import](/vi/plugins/building-plugins#import-conventions) và
  [đường dẫn con của Plugin SDK](/vi/plugins/sdk-subpaths) để tìm import hẹp.
- Chạy lại `clawhub package validate <path-to-plugin>`.

### reserved-sdk-import

Plugin nhập một đường dẫn SDK dành riêng cho Plugin được đóng gói kèm hoặc
...
- Dùng [đường dẫn con của Plugin SDK](/vi/plugins/sdk-subpaths) và
...
Manifest vẫn dùng metadata xác thực nhà cung cấp `providerAuthEnvVars` cũ.

- Phản chiếu metadata biến môi trường của nhà cung cấp vào `setup.providers[].envVars`.
- Chỉ giữ `providerAuthEnvVars` dưới dạng metadata tương thích khi phạm vi
...
Manifest dùng metadata biến môi trường kênh cũ hoặc đời cũ mà không có metadata
...
- Phản chiếu setup kênh do biến môi trường điều khiển vào metadata setup, cấu
  hình kênh hoặc kênh gói hiện tại được hình dạng Plugin của bạn sử dụng.
- Chỉ giữ `channelEnvVars` dưới dạng metadata tương thích khi các phiên bản
...
- Xóa `openclaw.security.json` cho đến khi OpenClaw ghi tài liệu một schema
  manifest bảo mật có phiên bản và hành vi ClawHub.
- Giữ hành vi nhạy cảm về bảo mật được ghi tài liệu trong tài liệu gói công khai
  hoặc README của bạn cho đến khi hợp đồng manifest tồn tại.
- Chạy lại `clawhub package validate <path-to-plugin>`.
