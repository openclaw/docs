...
x-i18n:
    generated_at: "2026-06-27T17:13:47Z"
    model: gpt-5.5
...
Bạn có thể xây dựng danh mục, thư mục hoặc bề mặt tìm kiếm của bên thứ ba dựa trên các API đọc công khai của ClawHub. Siêu dữ liệu skill công khai và tệp skill được xuất bản theo các quy tắc giấy phép skill của ClawHub, còn bản thân API bị giới hạn tốc độ và nên được sử dụng có trách nhiệm.
...
- `GET /api/v1/download?slug=&version=&tag=`
  - Skill được lưu trữ trả về các byte ZIP xác định.
  - Skill hiện tại dựa trên GitHub có bản quét `clean` hoặc `suspicious` sẽ trả về một
...
- `POST /api/v1/users/reserve` giữ trước slug gốc và phần giữ chỗ gói riêng tư không phát hành cho một handle chủ sở hữu.
