...
Bạn có thể xây dựng danh mục, thư mục hoặc bề mặt tìm kiếm của bên thứ ba trên các API đọc công khai của ClawHub. Siêu dữ liệu skill công khai và tệp skill được phát hành theo các quy tắc giấy phép skill của ClawHub, trong khi bản thân API có giới hạn tốc độ và nên được sử dụng có trách nhiệm.
...
- Lưu phản hồi vào bộ nhớ đệm và tôn trọng `429`, `Retry-After`, cùng các tiêu đề giới hạn tốc độ thay vì thăm dò quá thường xuyên.
- Liên kết trở lại URL skill ClawHub chuẩn khi hiển thị danh sách để người dùng có thể kiểm tra bản ghi nguồn trong registry.
...
- Không sao chép nội dung bị ẩn, riêng tư hoặc bị chặn bởi kiểm duyệt bằng cách vượt qua các bộ lọc API công khai hoặc ranh giới xác thực.
...
- Các tham số truy vấn không xác định bị bỏ qua để tương thích.
...
- `GET /api/v1/skills?limit=&cursor=&sort=`
  ...
  - `cursor` áp dụng cho các cách sắp xếp không phải `trending`
  - Bộ lọc tùy chọn: `nonSuspiciousOnly=true`
  - Bí danh cũ: `nonSuspicious=true`
  - Với `nonSuspiciousOnly=true`, các trang dựa trên con trỏ có thể chứa ít hơn `limit` mục; dùng `nextCursor` để tiếp tục.
  - `recommended` dùng các tín hiệu về mức độ tương tác và tính gần đây.
...
- `GET /api/v1/download?slug=&version=&tag=`
  - Skills được lưu trữ trả về các byte ZIP tất định.
  - Skills hiện tại dựa trên GitHub có kết quả quét `clean` hoặc `suspicious` trả về
    một mô tả bàn giao JSON `public-github` thay vì byte từ ClawHub.
...
- `POST /api/v1/users/reserve` giữ trước các slug gốc và phần giữ chỗ gói riêng tư chưa có bản phát hành cho một handle chủ sở hữu.
