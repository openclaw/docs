...
x-i18n:
    generated_at: "2026-07-03T00:58:57Z"
    model: gpt-5.5
...
Bạn có thể xây dựng danh mục, thư mục hoặc bề mặt tìm kiếm của bên thứ ba trên các API đọc công khai của ClawHub. Siêu dữ liệu kỹ năng công khai và các tệp kỹ năng được phát hành theo quy tắc giấy phép kỹ năng của ClawHub, trong khi bản thân API có giới hạn tốc độ và nên được sử dụng có trách nhiệm.
...
- `X-RateLimit-Remaining` / `RateLimit-Remaining`: ngân sách chính xác còn lại khi
  có mặt; các yêu cầu thành công đã phân mảnh sẽ bỏ qua trường này thay vì trả về
...
- Tham số truy vấn đã biết với giá trị không hợp lệ trả về `400`.
...
- `GET /api/v1/download?slug=&version=&tag=`
  ...
  - Các kỹ năng hiện tại dựa trên GitHub với lần quét `clean` hoặc `suspicious` trả về một
    mô tả chuyển giao JSON `public-github` thay vì byte ClawHub.
- `GET /api/v1/skills/export?startDate=&endDate=&limit=&cursor=`
  - Kỹ năng được lưu trữ được xuất dưới dạng tệp đã lưu.
  - Các kỹ năng hiện tại dựa trên GitHub với lần quét `clean` hoặc `suspicious` được xuất
