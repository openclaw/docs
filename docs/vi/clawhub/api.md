...
Bạn có thể xây dựng danh mục, thư mục hoặc bề mặt tìm kiếm của bên thứ ba dựa trên các API đọc công khai của ClawHub. Siêu dữ liệu skill công khai và các tệp skill được phát hành theo các quy tắc giấy phép skill của ClawHub, trong khi bản thân API bị giới hạn tốc độ và cần được sử dụng có trách nhiệm.
...
- Lưu phản hồi vào bộ nhớ đệm và tôn trọng `429`, `Retry-After` và các header giới hạn tốc độ thay vì polling quá mức.
...
- Không sao chép nội dung bị ẩn, riêng tư hoặc bị chặn do kiểm duyệt bằng cách vượt qua bộ lọc API công khai hoặc ranh giới xác thực.
...
- `POST /api/v1/users/reserve` dành trước root slug và placeholder package không phát hành riêng tư cho một owner handle.
