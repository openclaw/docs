...
x-i18n:
    generated_at: "2026-07-02T22:36:45Z"
    model: gpt-5.5
...
- Lưu phản hồi vào bộ nhớ đệm và tôn trọng các header `429`, `Retry-After` và giới hạn tốc độ thay vì thăm dò quá dồn dập.
...
- `X-RateLimit-Reset`: giây epoch Unix (thời điểm đặt lại tuyệt đối)
...
- `GET /api/v1/download?slug=&version=&tag=`
  - Các skill được lưu trữ trả về byte ZIP xác định.
  - Các skill hiện tại được GitHub hậu thuẫn với bản quét `clean` hoặc `suspicious` trả về
    bộ mô tả bàn giao JSON `public-github` thay vì byte ClawHub.
- `GET /api/v1/skills/export?startDate=&endDate=&limit=&cursor=`
  - Các skill được lưu trữ được xuất dưới dạng tệp đã lưu.
  - Các skill hiện tại được GitHub hậu thuẫn với bản quét `clean` hoặc `suspicious` được xuất
