...
Bạn có thể xây dựng danh mục, thư mục hoặc bề mặt tìm kiếm của bên thứ ba dựa trên các API đọc công khai của ClawHub. Siêu dữ liệu Skills công khai và các tệp Skills được xuất bản theo quy tắc giấy phép Skills của ClawHub, trong khi bản thân API bị giới hạn tốc độ và nên được sử dụng một cách có trách nhiệm.
...
- Lưu phản hồi vào bộ nhớ đệm và tôn trọng các tiêu đề `429`, `Retry-After` và giới hạn tốc độ thay vì thăm dò quá dồn dập.
- Liên kết lại URL Skills ClawHub chính tắc khi hiển thị danh sách để người dùng có thể kiểm tra bản ghi registry nguồn.
...
- `GET /api/v1/skills?limit=&cursor=&sort=`
  ...
  - Bí danh cũ: `nonSuspicious=true`
  - Với `nonSuspiciousOnly=true`, các trang dựa trên cursor có thể chứa ít mục hơn `limit`; sử dụng `nextCursor` để tiếp tục.
...
- `GET /api/v1/download?slug=&version=&tag=`
  ...
  - Skills hiện tại được hậu thuẫn bởi GitHub với bản quét `clean` hoặc `suspicious` trả về
...
- `GET /api/v1/skills/export?startDate=&endDate=&limit=&cursor=`
  ...
  - Skills hiện tại được hậu thuẫn bởi GitHub với bản quét `clean` hoặc `suspicious` được xuất
