...
Bạn có thể xây dựng danh mục, thư mục hoặc bề mặt tìm kiếm của bên thứ ba dựa trên các API đọc công khai của ClawHub. Siêu dữ liệu Skills công khai và tệp Skills được phát hành theo các quy tắc giấy phép Skills của ClawHub, trong khi bản thân API có giới hạn tốc độ và nên được sử dụng một cách có trách nhiệm.
...
- `POST /api/v1/users/reserve` giữ chỗ root slugs và placeholder package riêng tư chưa phát hành cho một handle chủ sở hữu.
