---
read_when:
    - Xây dựng API clients
    - Thêm điểm cuối hoặc schema
...
Bạn có thể xây dựng danh mục, thư mục hoặc bề mặt tìm kiếm của bên thứ ba dựa trên các API đọc công khai của ClawHub. Siêu dữ liệu Skills công khai và các tệp Skills được phát hành theo quy tắc giấy phép Skills của ClawHub, trong khi bản thân API có giới hạn tần suất và nên được sử dụng một cách có trách nhiệm.
...
- Lưu phản hồi vào bộ nhớ đệm và tôn trọng các tiêu đề `429`, `Retry-After` và giới hạn tần suất thay vì thăm dò quá mức.
- Liên kết lại đến URL Skills ClawHub chuẩn khi hiển thị danh sách để người dùng có thể kiểm tra bản ghi registry nguồn.
...
- `X-RateLimit-Remaining` / `RateLimit-Remaining`: ngân sách còn lại chính xác khi
  có mặt; các yêu cầu thành công được phân mảnh sẽ bỏ qua thay vì trả về một giá trị
  toàn cục xấp xỉ
...
- Các tham số truy vấn đã biết nhưng có giá trị không hợp lệ trả về `400`.
...
- `GET /api/v1/skills?limit=&cursor=&sort=`
  ...
  - `recommended` sử dụng tín hiệu tương tác và độ gần đây.
...
- `GET /api/v1/download?slug=&version=&tag=`
  - Skills được lưu trữ trả về byte ZIP tất định.
  - Skills hiện tại được GitHub hậu thuẫn có bản quét `clean` hoặc `suspicious` trả về
...
- `GET /api/v1/skills/export?startDate=&endDate=&limit=&cursor=`
  ...
  - Skills hiện tại được GitHub hậu thuẫn có bản quét `clean` hoặc `suspicious` được xuất
...
- `POST /api/v1/users/reserve` giữ trước root slug và placeholder gói riêng tư chưa phát hành cho handle chủ sở hữu.

## Cũ

`/api/*` và `/api/cli/*` cũ vẫn có sẵn. Xem `DEPRECATIONS.md`.
