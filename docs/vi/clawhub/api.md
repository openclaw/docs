...
x-i18n:
    generated_at: "2026-07-05T07:01:03Z"
    model: gpt-5.5
...
Bạn có thể xây dựng danh mục, thư mục hoặc giao diện tìm kiếm của bên thứ ba dựa trên các API đọc công khai của ClawHub. Siêu dữ liệu kỹ năng công khai và các tệp kỹ năng được phát hành theo quy tắc giấy phép kỹ năng của ClawHub, còn bản thân API có giới hạn tần suất và nên được sử dụng có trách nhiệm.

Hướng dẫn:

- Dùng các endpoint đọc công khai như `GET /api/v1/skills`, `GET /api/v1/search` và `GET /api/v1/skills/{slug}` cho danh sách danh mục.
- Lưu phản hồi vào bộ nhớ đệm và tôn trọng `429`, `Retry-After` cũng như các header giới hạn tần suất thay vì thăm dò quá mức.
- Liên kết ngược về URL kỹ năng ClawHub chính tắc khi hiển thị danh sách để người dùng có thể kiểm tra bản ghi registry nguồn.
...
- `X-RateLimit-Remaining` / `RateLimit-Remaining`: ngân sách còn lại chính xác khi
  có mặt; các yêu cầu thành công được phân mảnh sẽ bỏ qua trường này thay vì trả về
