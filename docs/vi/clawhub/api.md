...
x-i18n:
    generated_at: "2026-07-05T05:16:52Z"
    model: gpt-5.5
...
Bạn có thể xây dựng catalog, thư mục hoặc bề mặt tìm kiếm của bên thứ ba trên các API đọc công khai của ClawHub. Siêu dữ liệu skill công khai và các tệp skill được xuất bản theo quy tắc giấy phép skill của ClawHub, trong khi bản thân API có giới hạn tần suất và nên được sử dụng có trách nhiệm.
...
- Lưu phản hồi vào bộ nhớ đệm và tôn trọng `429`, `Retry-After` và các header giới hạn tần suất thay vì thăm dò quá mức.
- Liên kết lại URL skill ClawHub chuẩn khi hiển thị danh sách để người dùng có thể kiểm tra bản ghi registry nguồn.
...
- `GET /api/v1/download?slug=&version=&tag=`
  ...
  - Skill hiện tại dựa trên GitHub có kết quả quét `clean` hoặc `suspicious` trả về một
    mô tả bàn giao JSON `public-github` thay vì byte ClawHub.
- `GET /api/v1/skills/export?startDate=&endDate=&limit=&cursor=`
  - Skill được lưu trữ được xuất dưới dạng các tệp đã lưu.
  - Skill hiện tại dựa trên GitHub có kết quả quét `clean` hoặc `suspicious` được xuất
...
- `POST /api/v1/users/reserve` giữ trước slug gốc và placeholder gói riêng tư chưa phát hành cho một định danh chủ sở hữu.
