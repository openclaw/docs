...
x-i18n:
    generated_at: "2026-07-03T13:37:16Z"
    model: gpt-5.5
...
Bạn có thể xây dựng một danh mục, thư mục hoặc bề mặt tìm kiếm của bên thứ ba dựa trên các API đọc công khai của ClawHub. Siêu dữ liệu kỹ năng công khai và tệp kỹ năng được phát hành theo quy tắc giấy phép kỹ năng của ClawHub, trong khi bản thân API có giới hạn tốc độ và nên được sử dụng một cách có trách nhiệm.
...
- Lưu phản hồi vào bộ nhớ đệm và tuân thủ `429`, `Retry-After` cùng các header giới hạn tốc độ thay vì polling quá thường xuyên.
...
- `GET /api/v1/download?slug=&version=&tag=`
  - Kỹ năng được lưu trữ trả về byte ZIP tất định.
  - Kỹ năng hiện tại được GitHub hậu thuẫn có bản quét `clean` hoặc `suspicious` trả về một
    descriptor bàn giao JSON `public-github` thay vì byte từ ClawHub.
- `GET /api/v1/skills/export?startDate=&endDate=&limit=&cursor=`
  - Kỹ năng được lưu trữ được xuất dưới dạng tệp đã lưu.
  - Kỹ năng hiện tại được GitHub hậu thuẫn có bản quét `clean` hoặc `suspicious` được xuất
    dưới dạng descriptor bàn giao `public-github`.
...
- `POST /api/v1/users/reserve` giữ trước các slug gốc và phần giữ chỗ gói riêng tư không phát hành cho một handle chủ sở hữu.
