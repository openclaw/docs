...
Bạn có thể xây dựng danh mục, thư mục hoặc bề mặt tìm kiếm của bên thứ ba dựa trên các API đọc công khai của ClawHub. Siêu dữ liệu kỹ năng công khai và các tệp kỹ năng được phát hành theo quy tắc giấy phép kỹ năng của ClawHub, còn bản thân API có giới hạn tốc độ và nên được sử dụng có trách nhiệm.
...
- Lưu phản hồi vào bộ nhớ đệm và tôn trọng `429`, `Retry-After` cùng các header giới hạn tốc độ thay vì thăm dò quá dày.
- Liên kết ngược đến URL kỹ năng ClawHub chính tắc khi hiển thị danh sách để người dùng có thể kiểm tra bản ghi registry nguồn.
...
- Yêu cầu đã xác thực (token Bearer hợp lệ): theo nhóm giới hạn của người dùng.
- Token thiếu/không hợp lệ sẽ chuyển sang thực thi theo IP.
...
- `X-RateLimit-Reset`: giây theo Unix epoch (thời điểm đặt lại tuyệt đối)
...
- `POST /api/v1/users/reserve` giữ trước các root slug và placeholder gói riêng tư chưa phát hành cho một owner handle.
