...
- Lưu phản hồi vào bộ nhớ đệm và tôn trọng `429`, `Retry-After` cũng như các header giới hạn tốc độ thay vì thăm dò quá mức.
- Liên kết ngược về URL Skills chuẩn của ClawHub khi hiển thị danh sách để người dùng có thể kiểm tra bản ghi sổ đăng ký nguồn.
...
- Tham số truy vấn không xác định bị bỏ qua để đảm bảo tương thích.
...
- `GET /api/v1/download?slug=&version=&tag=`
  ...
  - Skills hiện tại dựa trên GitHub có bản quét `clean` hoặc `suspicious` trả về một
    bộ mô tả chuyển giao JSON `public-github` thay vì byte của ClawHub.
...
- `POST /api/v1/users/reserve` giữ trước root slug và phần giữ chỗ gói riêng tư chưa phát hành cho một handle chủ sở hữu.
