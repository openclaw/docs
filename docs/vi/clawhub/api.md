...
x-i18n:
    generated_at: "2026-07-01T15:26:22Z"
    model: gpt-5.5
...
Bạn có thể xây dựng danh mục, thư mục hoặc bề mặt tìm kiếm của bên thứ ba dựa trên các API đọc công khai của ClawHub. Siêu dữ liệu kỹ năng công khai và tệp kỹ năng được xuất bản theo các quy tắc giấy phép kỹ năng của ClawHub, còn bản thân API được giới hạn tốc độ và nên được sử dụng có trách nhiệm.
...
- Lưu phản hồi vào bộ nhớ đệm và tôn trọng các header `429`, `Retry-After` và giới hạn tốc độ thay vì thăm dò quá dày.
- Liên kết trở lại URL kỹ năng ClawHub chính tắc khi hiển thị danh sách để người dùng có thể kiểm tra bản ghi registry nguồn.
...
- `GET /api/v1/download?slug=&version=&tag=`
  ...
  - Kỹ năng hiện tại dựa trên GitHub có bản quét `clean` hoặc `suspicious` trả về
...
- `POST /api/v1/users/reserve` giữ chỗ root slug và trình giữ chỗ gói riêng tư chưa phát hành cho một handle chủ sở hữu.
