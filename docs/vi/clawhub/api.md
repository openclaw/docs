...
x-i18n:
    generated_at: "2026-07-02T01:00:11Z"
    model: gpt-5.5
...
Bạn có thể xây dựng danh mục, thư mục hoặc bề mặt tìm kiếm của bên thứ ba dựa trên các API đọc công khai của ClawHub. Siêu dữ liệu skill công khai và tệp skill được phát hành theo quy tắc giấy phép skill của ClawHub, còn bản thân API có giới hạn tốc độ và nên được sử dụng có trách nhiệm.

Hướng dẫn:

- Dùng các endpoint đọc công khai như `GET /api/v1/skills`, `GET /api/v1/search` và `GET /api/v1/skills/{slug}` cho danh sách danh mục.
- Lưu phản hồi vào bộ nhớ đệm và tôn trọng `429`, `Retry-After` cùng các header giới hạn tốc độ thay vì thăm dò quá thường xuyên.
...
- Token thiếu/không hợp lệ sẽ dùng thực thi theo IP.
...
- `GET /api/v1/download?slug=&version=&tag=`
  ...
  - Skill hiện tại dựa trên GitHub có bản quét `clean` hoặc `suspicious` trả về một
    bộ mô tả bàn giao JSON `public-github` thay vì byte của ClawHub.
...
- `POST /api/v1/users/reserve` giữ trước các slug gốc và phần giữ chỗ gói riêng tư chưa phát hành cho một handle chủ sở hữu.

## Cũ

Các `/api/*` và `/api/cli/*` cũ vẫn còn khả dụng. Xem `DEPRECATIONS.md`.
