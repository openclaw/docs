...
Bạn có thể xây dựng catalog, thư mục hoặc bề mặt tìm kiếm của bên thứ ba dựa trên các API đọc công khai của ClawHub. Siêu dữ liệu kỹ năng công khai và các tệp kỹ năng được phát hành theo quy tắc giấy phép kỹ năng của ClawHub, trong khi bản thân API bị giới hạn tốc độ và nên được sử dụng có trách nhiệm.

Hướng dẫn:

- Dùng các endpoint đọc công khai như `GET /api/v1/skills`, `GET /api/v1/search` và `GET /api/v1/skills/{slug}` cho danh sách catalog.
- Lưu phản hồi vào bộ nhớ đệm và tôn trọng `429`, `Retry-After` và các header giới hạn tốc độ thay vì polling quá dày.
- Liên kết về URL kỹ năng ClawHub chính tắc khi hiển thị danh sách để người dùng có thể kiểm tra bản ghi registry nguồn.
...
- Không mirror nội dung bị ẩn, riêng tư hoặc bị chặn kiểm duyệt bằng cách vượt qua các bộ lọc API công khai hoặc ranh giới auth.
...
- Token thiếu/không hợp lệ sẽ fallback về thực thi theo IP.
...
- `X-RateLimit-Remaining` / `RateLimit-Remaining`: ngân sách còn lại chính xác khi
  có mặt; các yêu cầu thành công được phân mảnh sẽ bỏ qua trường này thay vì trả về một giá trị
  toàn cục xấp xỉ
...
- Tham số truy vấn không xác định bị bỏ qua để tương thích.
...
- `GET /api/v1/download?slug=&version=&tag=`
  - Kỹ năng được lưu trữ trả về byte ZIP xác định.
  - Các kỹ năng hiện tại dựa trên GitHub với lượt quét `clean` hoặc `suspicious` trả về một
    bộ mô tả bàn giao JSON `public-github` thay vì byte ClawHub.
- `GET /api/v1/skills/export?startDate=&endDate=&limit=&cursor=`
  - Kỹ năng được lưu trữ được xuất dưới dạng tệp đã lưu.
  - Các kỹ năng hiện tại dựa trên GitHub với lượt quét `clean` hoặc `suspicious` được xuất
...
- `POST /api/v1/users/reserve` đặt trước slug gốc và placeholder gói riêng tư chưa phát hành cho một handle chủ sở hữu.
