...
Bạn có thể xây dựng danh mục, thư mục hoặc bề mặt tìm kiếm của bên thứ ba trên các API đọc công khai của ClawHub. Siêu dữ liệu Skills công khai và tệp Skills được phát hành theo quy tắc giấy phép Skills của ClawHub, trong khi bản thân API bị giới hạn tốc độ và nên được sử dụng có trách nhiệm.
...
- Token thiếu/không hợp lệ quay về thực thi theo IP.
...
- `X-RateLimit-Remaining` / `RateLimit-Remaining`: ngân sách còn lại chính xác khi
  có mặt; các yêu cầu thành công được phân mảnh sẽ bỏ qua trường này thay vì trả về một giá trị toàn cục
  xấp xỉ
...
- `GET /api/v1/download?slug=&version=&tag=`
  ...
  - Skills hiện tại dựa trên GitHub với lần quét `clean` hoặc `suspicious` trả về một
    mô tả bàn giao JSON `public-github` thay vì byte từ ClawHub.
- `GET /api/v1/skills/export?startDate=&endDate=&limit=&cursor=`
  - Skills được lưu trữ được xuất dưới dạng các tệp đã lưu.
  - Skills hiện tại dựa trên GitHub với lần quét `clean` hoặc `suspicious` được xuất
...
- `POST /api/v1/users/reserve` giữ trước slug gốc và placeholder package riêng tư không phát hành cho một handle chủ sở hữu.
