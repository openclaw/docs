...
Bạn có thể xây dựng danh mục, thư mục hoặc bề mặt tìm kiếm của bên thứ ba dựa trên các API đọc công khai của ClawHub. Siêu dữ liệu skill công khai và các tệp skill được phát hành theo quy tắc giấy phép skill của ClawHub, trong khi bản thân API bị giới hạn tốc độ và nên được sử dụng có trách nhiệm.
...
- Lưu cache phản hồi và tôn trọng các header `429`, `Retry-After` và giới hạn tốc độ thay vì thăm dò quá mức.
- Liên kết lại URL skill ClawHub chính tắc khi hiển thị danh sách để người dùng có thể kiểm tra bản ghi registry nguồn.
- Sử dụng URL trang chính tắc theo dạng `https://clawhub.ai/<owner>/skills/<slug>`.
- Không ngụ ý rằng ClawHub chứng thực, xác minh hoặc vận hành trang web của bên thứ ba.
- Không sao chép nội dung ẩn, riêng tư hoặc bị chặn bởi kiểm duyệt bằng cách vượt qua bộ lọc API công khai hoặc ranh giới xác thực.
...
- `X-RateLimit-Remaining` / `RateLimit-Remaining`: ngân sách còn lại chính xác khi
  có mặt; các yêu cầu thành công được phân mảnh sẽ bỏ qua giá trị này thay vì trả về giá trị
  toàn cục xấp xỉ
...
- `GET /api/v1/skills?limit=&cursor=&sort=`
  - `sort`: `updated` (mặc định), `recommended` (`default`), `createdAt` (`newest`), `downloads`, `stars` (`rating`), bí danh cài đặt kế thừa `installsCurrent`/`installs`/`installsAllTime` ánh xạ tới `downloads`, `trending`
  ...
  - Bí danh kế thừa: `nonSuspicious=true`
  - Với `nonSuspiciousOnly=true`, các trang dựa trên con trỏ có thể chứa ít mục hơn `limit`; sử dụng `nextCursor` để tiếp tục.
...
- `GET /api/v1/download?slug=&version=&tag=`
  ...
  - Skills hiện tại được GitHub hỗ trợ với bản quét `clean` hoặc `suspicious` trả về
    bộ mô tả bàn giao JSON `public-github` thay vì byte ClawHub.
- `GET /api/v1/skills/export?startDate=&endDate=&limit=&cursor=`
  - Skills được lưu trữ được xuất dưới dạng tệp đã lưu.
  - Skills hiện tại được GitHub hỗ trợ với bản quét `clean` hoặc `suspicious` được xuất
...
- `POST /api/v1/users/reserve` dự trữ slug gốc và placeholder gói riêng tư chưa phát hành cho một handle chủ sở hữu.

## Kế thừa

Các `/api/*` và `/api/cli/*` kế thừa vẫn còn khả dụng. Xem `DEPRECATIONS.md`.
