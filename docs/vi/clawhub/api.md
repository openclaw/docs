...
x-i18n:
    generated_at: "2026-07-04T06:37:22Z"
    model: gpt-5.5
...
- Lưu phản hồi vào bộ nhớ đệm và tôn trọng các header `429`, `Retry-After`, và giới hạn tốc độ thay vì polling quá dày.
...
- Dùng URL trang chính tắc ở dạng `https://clawhub.ai/<owner>/skills/<slug>`.
...
- `GET /api/v1/download?slug=&version=&tag=`
  ...
  - Skills hiện tại được hỗ trợ bởi GitHub có bản quét `clean` hoặc `suspicious` trả về
...
- `GET /api/v1/skills/export?startDate=&endDate=&limit=&cursor=`
  ...
  - Skills hiện tại được hỗ trợ bởi GitHub có bản quét `clean` hoặc `suspicious` được xuất
...
- `POST /api/v1/users/reserve` đặt trước root slug và placeholder gói riêng tư không có bản phát hành cho một handle chủ sở hữu.

## Legacy

Legacy `/api/*` và `/api/cli/*` vẫn có sẵn. Xem `DEPRECATIONS.md`.
