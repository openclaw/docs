...
Bạn có thể xây dựng danh mục, thư mục hoặc bề mặt tìm kiếm của bên thứ ba dựa trên các API đọc công khai của ClawHub. Siêu dữ liệu Skills công khai và tệp Skills được phát hành theo quy tắc giấy phép Skills của ClawHub, trong khi bản thân API có giới hạn tần suất và nên được sử dụng một cách có trách nhiệm.
...
- Lưu phản hồi vào bộ nhớ đệm và tuân thủ `429`, `Retry-After` và các tiêu đề giới hạn tần suất thay vì thăm dò quá dồn dập.
...
- Không sao chép nội dung ẩn, riêng tư hoặc bị chặn do kiểm duyệt bằng cách vượt qua bộ lọc API công khai hoặc ranh giới xác thực.
...
Tiêu đề: `X-RateLimit-Limit`, `X-RateLimit-Remaining`, `X-RateLimit-Reset`, `RateLimit-Limit`, `RateLimit-Remaining`, `RateLimit-Reset`, `Retry-After` (trên 429).
...
- `POST /api/v1/users/reserve` giữ trước slug gốc và phần giữ chỗ gói riêng tư chưa phát hành cho một handle chủ sở hữu.

## Cũ

Các `/api/*` và `/api/cli/*` cũ vẫn có sẵn. Xem `DEPRECATIONS.md`.
