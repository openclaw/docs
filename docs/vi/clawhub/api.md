...
x-i18n:
    generated_at: "2026-07-01T13:05:43Z"
    model: gpt-5.5
...
Bạn có thể xây dựng danh mục, thư mục hoặc bề mặt tìm kiếm của bên thứ ba dựa trên các API đọc công khai của ClawHub. Siêu dữ liệu skill công khai và các tệp skill được phát hành theo quy tắc giấy phép skill của ClawHub, trong khi bản thân API có giới hạn tốc độ và cần được sử dụng có trách nhiệm.
...
- Lưu phản hồi vào bộ nhớ đệm và tôn trọng `429`, `Retry-After` cũng như các header giới hạn tốc độ thay vì thăm dò quá dày.
- Liên kết ngược về URL skill ClawHub chuẩn khi hiển thị danh sách để người dùng có thể kiểm tra bản ghi registry nguồn.
...
- Không sao chép nội dung bị ẩn, riêng tư hoặc bị chặn kiểm duyệt bằng cách vượt qua các bộ lọc API công khai hoặc ranh giới xác thực.
...
- Các tham số truy vấn đã biết có giá trị không hợp lệ trả về `400`.
...
- `GET /api/v1/download?slug=&version=&tag=`
  - Skill được lưu trữ trả về byte ZIP xác định.
  - Skill hiện tại dựa trên GitHub có bản quét `clean` hoặc `suspicious` trả về một
    bộ mô tả bàn giao JSON `public-github` thay vì byte ClawHub.
- `GET /api/v1/skills/export?startDate=&endDate=&limit=&cursor=`
  - Skill được lưu trữ được xuất dưới dạng tệp đã lưu.
  - Skill hiện tại dựa trên GitHub có bản quét `clean` hoặc `suspicious` được xuất
...
- `POST /api/v1/users/reserve` đặt trước các slug gốc và placeholder gói riêng tư không phát hành cho một handle chủ sở hữu.
