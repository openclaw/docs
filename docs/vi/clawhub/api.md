...
x-i18n:
    generated_at: "2026-07-02T08:27:05Z"
    model: gpt-5.5
...
Bạn có thể xây dựng danh mục, thư mục, hoặc bề mặt tìm kiếm của bên thứ ba dựa trên các API đọc công khai của ClawHub. Siêu dữ liệu skill công khai và các tệp skill được xuất bản theo quy tắc giấy phép skill của ClawHub, trong khi bản thân API có giới hạn tốc độ và nên được sử dụng một cách có trách nhiệm.

Hướng dẫn:

- Dùng các endpoint đọc công khai như `GET /api/v1/skills`, `GET /api/v1/search`, và `GET /api/v1/skills/{slug}` cho danh sách danh mục.
- Lưu phản hồi vào bộ nhớ đệm và tôn trọng `429`, `Retry-After`, cùng các header giới hạn tốc độ thay vì thăm dò quá mức.
...
- `GET /api/v1/download?slug=&version=&tag=`
  ...
  - Skills hiện tại dựa trên GitHub với kết quả quét `clean` hoặc `suspicious` trả về một
...
- `GET /api/v1/skills/export?startDate=&endDate=&limit=&cursor=`
  ...
  - Skills hiện tại dựa trên GitHub với kết quả quét `clean` hoặc `suspicious` được xuất
...
- `POST /api/v1/users/reserve` đặt trước slug gốc và phần giữ chỗ gói riêng tư không phát hành cho một handle chủ sở hữu.
