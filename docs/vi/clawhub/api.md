...
x-i18n:
    generated_at: "2026-07-04T15:23:05Z"
    model: gpt-5.5
...
Bạn có thể xây dựng danh mục, thư mục hoặc bề mặt tìm kiếm của bên thứ ba trên các API đọc công khai của ClawHub. Siêu dữ liệu skill công khai và tệp skill được xuất bản theo quy tắc giấy phép skill của ClawHub, trong khi bản thân API có giới hạn tốc độ và nên được sử dụng có trách nhiệm.
...
- Lưu phản hồi vào bộ nhớ đệm và tôn trọng `429`, `Retry-After` và các header giới hạn tốc độ thay vì thăm dò quá dồn dập.
...
- `X-RateLimit-Remaining` / `RateLimit-Remaining`: ngân sách còn lại chính xác khi
  có mặt; các yêu cầu thành công đã được phân mảnh sẽ bỏ qua giá trị này thay vì trả về một giá trị
  toàn cục xấp xỉ
...
- Tham số truy vấn đã biết nhưng có giá trị không hợp lệ trả về `400`.
...
- `GET /api/v1/download?slug=&version=&tag=`
  ...
  - Skill hiện tại được GitHub hỗ trợ với lần quét `clean` hoặc `suspicious` trả về một
    bộ mô tả bàn giao JSON `public-github` thay vì byte ClawHub.
- `GET /api/v1/skills/export?startDate=&endDate=&limit=&cursor=`
  - Skill được lưu trữ được xuất dưới dạng tệp đã lưu.
  - Skill hiện tại được GitHub hỗ trợ với lần quét `clean` hoặc `suspicious` được xuất
...
- `POST /api/v1/users/reserve` giữ trước slug gốc và placeholder gói riêng tư không phát hành cho một handle chủ sở hữu.

## Cũ

`/api/*` và `/api/cli/*` cũ vẫn còn khả dụng. Xem `DEPRECATIONS.md`.
