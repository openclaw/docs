...
x-i18n:
    generated_at: "2026-07-03T17:28:10Z"
    model: gpt-5.5
...
Bạn có thể xây dựng catalog, thư mục hoặc giao diện tìm kiếm của bên thứ ba dựa trên các API đọc công khai của ClawHub. Siêu dữ liệu kỹ năng công khai và các tệp kỹ năng được xuất bản theo quy tắc giấy phép kỹ năng của ClawHub, trong khi bản thân API có giới hạn tốc độ và nên được sử dụng có trách nhiệm.

Hướng dẫn:

- Sử dụng các endpoint đọc công khai như `GET /api/v1/skills`, `GET /api/v1/search` và `GET /api/v1/skills/{slug}` cho danh sách catalog.
- Lưu bộ nhớ đệm phản hồi và tôn trọng `429`, `Retry-After` cùng các header giới hạn tốc độ thay vì polling quá mức.
- Liên kết lại URL kỹ năng ClawHub chính tắc khi hiển thị danh sách để người dùng có thể kiểm tra bản ghi registry nguồn.
- Sử dụng URL trang chính tắc ở dạng `https://clawhub.ai/<owner>/skills/<slug>`.
- Không ngụ ý rằng ClawHub chứng thực, xác minh hoặc vận hành trang bên thứ ba.
...
- Thêm jitter cho các lần thử lại.
...
- Tham số truy vấn đã biết có giá trị không hợp lệ sẽ trả về `400`.
...
- `GET /api/v1/download?slug=&version=&tag=`
  ...
  - Kỹ năng hiện tại dựa trên GitHub có kết quả quét `clean` hoặc `suspicious` trả về
    bộ mô tả bàn giao JSON `public-github` thay vì byte từ ClawHub.
- `GET /api/v1/skills/export?startDate=&endDate=&limit=&cursor=`
  - Kỹ năng được lưu trữ được xuất dưới dạng các tệp đã lưu.
  - Kỹ năng hiện tại dựa trên GitHub có kết quả quét `clean` hoặc `suspicious` được xuất
...
- `POST /api/v1/users/reserve` giữ trước các slug gốc và placeholder gói riêng tư chưa phát hành cho một handle chủ sở hữu.
