...
Bạn có thể xây dựng danh mục, thư mục hoặc giao diện tìm kiếm của bên thứ ba dựa trên các API đọc công khai của ClawHub. Siêu dữ liệu kỹ năng công khai và tệp kỹ năng được xuất bản theo các quy tắc giấy phép kỹ năng của ClawHub, trong khi bản thân API có giới hạn tốc độ và nên được sử dụng có trách nhiệm.
...
- `GET /api/v1/download?slug=&version=&tag=`
  - Các kỹ năng được lưu trữ trả về byte ZIP xác định.
  - Các kỹ năng hiện tại dựa trên GitHub có kết quả quét `clean` hoặc `suspicious` trả về
    mô tả bàn giao JSON `public-github` thay vì byte của ClawHub.
- `GET /api/v1/skills/export?startDate=&endDate=&limit=&cursor=`
  - Các kỹ năng được lưu trữ được xuất dưới dạng tệp đã lưu.
  - Các kỹ năng hiện tại dựa trên GitHub có kết quả quét `clean` hoặc `suspicious` được xuất
...
- `POST /api/v1/users/reserve` dành trước các root slug và placeholder gói riêng tư không phát hành cho một owner handle.
