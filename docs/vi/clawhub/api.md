...
x-i18n:
    generated_at: "2026-06-30T22:21:41Z"
    model: gpt-5.5
...
Bạn có thể xây dựng danh mục, thư mục hoặc giao diện tìm kiếm của bên thứ ba dựa trên các API đọc công khai của ClawHub. Metadata Skills công khai và các tệp Skills được phát hành theo quy tắc giấy phép Skills của ClawHub, còn bản thân API có giới hạn tần suất và nên được sử dụng có trách nhiệm.

Hướng dẫn:

- Dùng các endpoint đọc công khai như `GET /api/v1/skills`, `GET /api/v1/search` và `GET /api/v1/skills/{slug}` cho danh sách danh mục.
- Lưu cache phản hồi và tôn trọng các header `429`, `Retry-After` và giới hạn tần suất thay vì polling quá dày.
- Liên kết ngược về URL Skills ClawHub chuẩn khi hiển thị danh sách để người dùng có thể kiểm tra bản ghi registry nguồn.
- Dùng URL trang chuẩn có dạng `https://clawhub.ai/<owner>/skills/<slug>`.
- Không ngụ ý rằng ClawHub chứng thực, xác minh hoặc vận hành trang của bên thứ ba.
- Không mirror nội dung bị ẩn, riêng tư hoặc bị chặn kiểm duyệt bằng cách vượt qua bộ lọc API công khai hoặc ranh giới xác thực.
...
- `X-RateLimit-Remaining` / `RateLimit-Remaining`: ngân sách còn lại chính xác khi
  có mặt; các yêu cầu thành công đã sharding sẽ bỏ qua giá trị này thay vì trả về
...
- `GET /api/v1/skills?limit=&cursor=&sort=`
  - `sort`: `updated` (mặc định), `recommended` (`default`), `createdAt` (`newest`), `downloads`, `stars` (`rating`), các alias cài đặt legacy `installsCurrent`/`installs`/`installsAllTime` ánh xạ tới `downloads`, `trending`
...
- `GET /api/v1/download?slug=&version=&tag=`
  ...
  - Skills hiện tại dựa trên GitHub có kết quả quét `clean` hoặc `suspicious` trả về
    descriptor chuyển giao JSON `public-github` thay vì byte từ ClawHub.
- `GET /api/v1/skills/export?startDate=&endDate=&limit=&cursor=`
  - Skills được host được xuất dưới dạng tệp đã lưu trữ.
  - Skills hiện tại dựa trên GitHub có kết quả quét `clean` hoặc `suspicious` được xuất
    dưới dạng descriptor chuyển giao `public-github`.
- `GET /api/v1/packages?limit=&cursor=&sort=`
  - `sort`: `updated` (mặc định), `recommended`, `downloads`, alias legacy `installs`
  - Giá trị `sort` không hợp lệ trả về `400`
- `GET /api/v1/plugins?limit=&cursor=&sort=`
  - `sort`: `recommended` (mặc định), `downloads`, `updated`, alias legacy `installs`
...
- `POST /api/v1/users/reserve` đặt trước root slug và placeholder gói riêng tư chưa phát hành cho một owner handle.
