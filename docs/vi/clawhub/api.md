...
x-i18n:
    generated_at: "2026-05-10T19:24:36Z"
    model: gpt-5.5
    provider: openai
...
Bạn có thể xây dựng danh mục, thư mục hoặc bề mặt tìm kiếm của bên thứ ba dựa trên các API đọc công khai của ClawHub. Siêu dữ liệu Skills công khai và các tệp Skills được phát hành theo quy tắc giấy phép Skills của ClawHub, trong khi bản thân API có giới hạn tốc độ và nên được sử dụng một cách có trách nhiệm.
...
- `POST /api/v1/users/reserve` đặt trước root slug và placeholder gói riêng tư không phát hành cho một owner handle.
