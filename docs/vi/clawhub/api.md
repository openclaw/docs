...
x-i18n:
    generated_at: "2026-07-03T23:35:33Z"
    model: gpt-5.5
...
Bạn có thể xây dựng danh mục, thư mục hoặc bề mặt tìm kiếm của bên thứ ba dựa trên các API đọc công khai của ClawHub. Siêu dữ liệu skill công khai và các tệp skill được phát hành theo quy tắc giấy phép skill của ClawHub, trong khi bản thân API có giới hạn tốc độ và nên được sử dụng có trách nhiệm.
...
- Lưu cache phản hồi và tôn trọng `429`, `Retry-After` cùng các header giới hạn tốc độ thay vì polling quá dồn dập.
- Liên kết trở lại URL skill ClawHub chính tắc khi hiển thị danh sách để người dùng có thể kiểm tra bản ghi registry nguồn.
...
- `X-RateLimit-Remaining` / `RateLimit-Remaining`: ngân sách còn lại chính xác khi
  có mặt; các yêu cầu thành công được phân shard sẽ bỏ qua giá trị này thay vì trả về một giá trị toàn cục
  xấp xỉ
- `Retry-After`: số giây trì hoãn cần chờ khi có `429`
...
- `GET /api/v1/download?slug=&version=&tag=`
  ...
  - Skills hiện tại dựa trên GitHub có bản quét `clean` hoặc `suspicious` trả về
    descriptor chuyển giao JSON `public-github` thay vì byte ClawHub.
...
- `POST /api/v1/users/reserve` giữ chỗ các slug gốc và placeholder gói riêng tư chưa phát hành cho một handle chủ sở hữu.
