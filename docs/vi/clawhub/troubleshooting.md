---
read_when:
    - CLI ClawHub hoặc các lệnh kho đăng ký của OpenClaw bị lỗi
...
Nếu nhiều người dùng dùng chung một IP đi ra ngoài, giới hạn IP ẩn danh có thể bị chạm tới ngay cả khi mỗi
...
Với skills, hãy kiểm tra frontmatter trong `SKILL.md`. Các biến môi trường và
công cụ bắt buộc nên được khai báo để người dùng và trình quét có thể hiểu gói.

Với plugins, hãy kiểm tra siêu dữ liệu tương thích trong `package.json`. Việc xuất bản code-plugin
