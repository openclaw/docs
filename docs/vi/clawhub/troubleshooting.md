---
read_when:
    - Các lệnh CLI ClawHub hoặc registry OpenClaw bị lỗi
...
x-i18n:
    generated_at: "2026-07-05T07:03:00Z"
    model: gpt-5.5
...
CLI khởi động một máy chủ gọi lại cục bộ ngắn hạn trong quá trình đăng nhập bằng trình duyệt.
...
- Trong môi trường không giao diện, hãy tạo một token API trong giao diện web ClawHub và chạy:
...
- `RateLimit-Remaining`: ngân sách chính xác còn lại của bạn khi header này có mặt. Khi gặp `429`, giá trị là `0`.
...
- Xác nhận bản phát hành là công khai và không bị giữ bởi quét hoặc kiểm duyệt.
...
Với Plugin, hãy kiểm tra siêu dữ liệu tương thích trong `package.json`. Các lượt phát hành
...
## Phát hành thất bại với lỗi chủ sở hữu GitHub hoặc lỗi nguồn
...
## Phát hành thất bại vì namespace đã được nhận hoặc được giữ riêng

Nếu phát hành thất bại vì handle chủ sở hữu, namespace tổ chức, phạm vi gói, slug kỹ năng
hoặc tên gói đã được nhận hoặc được giữ riêng, trước tiên hãy xác nhận rằng bạn đang
phát hành bằng chủ sở hữu khớp với namespace. Với các gói Plugin,
tên có phạm vi như `@example-org/example-plugin` phải được phát hành dưới chủ sở hữu
`example-org` tương ứng.

Nếu bạn tin rằng tổ chức, dự án hoặc thương hiệu của bạn là chủ sở hữu hợp pháp của namespace nhưng
...
Các tệp cục bộ không khớp với bất kỳ phiên bản nào ClawHub biết. Chọn một tùy chọn:
...
- Phát hành bản đã chỉnh sửa của bạn dưới dạng slug mới hoặc bản fork.
...
- Xác nhận phiên bản OpenClaw của bạn đáp ứng phạm vi tương thích
  mà gói công bố.
- Nếu gói bị ẩn, bị giữ hoặc bị chặn, gói có thể không cài đặt được cho đến khi
  chủ sở hữu giải quyết vấn đề.

## Yêu cầu API công khai thất bại

- Tôn trọng header thử lại `429` và lưu cache phản hồi danh sách/tìm kiếm công khai.
- Liên kết người dùng trở lại danh sách ClawHub chuẩn.
- Không sao chép nội dung bị ẩn, riêng tư, bị giữ hoặc bị chặn do kiểm duyệt ra ngoài
