---
read_when:
    - ClawHub CLI hoặc các lệnh registry của OpenClaw không thành công
...
x-i18n:
    generated_at: "2026-07-01T18:14:28Z"
    model: gpt-5.5
...
- Trong môi trường không giao diện, hãy tạo token API trong giao diện web ClawHub rồi chạy:
...
- `RateLimit-Remaining`: ngân sách chính xác còn lại của bạn khi header này hiện diện. Với `429`, giá trị là `0`.
- `RateLimit-Reset` hoặc `X-RateLimit-Reset`: thời điểm đặt lại.

Nếu nhiều người dùng dùng chung một IP đi ra, giới hạn IP ẩn danh có thể bị chạm đến ngay cả khi mỗi
...
Nếu phát hành thất bại vì handle chủ sở hữu, namespace tổ chức, phạm vi gói, slug skill
...
tên có phạm vi như `@example-org/example-plugin` phải được phát hành dưới
...
Xem trước nếu bạn chưa chắc nội dung nào sẽ được phát hành:
...
- Phát hành bản sao đã chỉnh sửa của bạn dưới dạng slug hoặc fork mới.
...
- Dẫn người dùng quay lại danh sách ClawHub chuẩn.
