---
read_when:
    - Lệnh CLI ClawHub hoặc lệnh sổ đăng ký OpenClaw không thành công
    - Không thể cài đặt, phát hành hoặc cập nhật gói
summary: Khắc phục sự cố đăng nhập, cài đặt, xuất bản, cập nhật và các vấn đề API của ClawHub.
x-i18n:
    generated_at: "2026-07-04T06:39:15Z"
    model: gpt-5.5
...
- Trong môi trường không có giao diện đồ họa, hãy tạo một token API trong giao diện web ClawHub rồi chạy:
...
Đối với plugin, hãy kiểm tra siêu dữ liệu tương thích trong `package.json`. Các lượt xuất bản code-plugin
...
## Xuất bản thất bại vì namespace đã được xác nhận hoặc đặt trước

Nếu một lượt xuất bản thất bại vì handle chủ sở hữu, namespace tổ chức, phạm vi gói, slug skill
hoặc tên gói đã được xác nhận hoặc đặt trước, trước tiên hãy xác nhận rằng bạn đang
xuất bản bằng chủ sở hữu khớp với namespace. Đối với các gói plugin,
...
Trỏ nó tới các gốc bạn muốn quét:
...
- Xác nhận phiên bản OpenClaw của bạn đáp ứng phạm vi tương thích
  mà gói quảng bá.
- Nếu gói bị ẩn, bị giữ lại hoặc bị chặn, có thể không thể cài đặt cho đến khi
