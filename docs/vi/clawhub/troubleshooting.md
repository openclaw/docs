...
- Trong môi trường không có giao diện đồ họa, hãy tạo API token trong giao diện web ClawHub và chạy:
...
Với plugin, hãy kiểm tra metadata tương thích trong `package.json`. Việc xuất bản code-plugin
...
xuất bản bằng chủ sở hữu khớp với namespace đó. Với các gói plugin,
các tên có phạm vi như `@example-org/example-plugin` phải được xuất bản với
...
Các tệp cục bộ không khớp với bất kỳ phiên bản nào mà ClawHub biết đến. Chọn một:
