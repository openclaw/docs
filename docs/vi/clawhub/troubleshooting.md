---
read_when:
    - Lệnh ClawHub CLI hoặc lệnh registry OpenClaw thất bại
...
x-i18n:
    generated_at: "2026-06-28T00:12:17Z"
    model: gpt-5.5
...
- `RateLimit-Remaining`: ngân sách còn lại chính xác của bạn khi header có mặt. Khi gặp `429`, giá trị này là `0`.
- `RateLimit-Reset` hoặc `X-RateLimit-Reset`: thời điểm đặt lại.

Nếu nhiều người dùng dùng chung một IP egress, giới hạn IP ẩn danh có thể bị chạm đến ngay cả khi mỗi
...
## Xuất bản thất bại vì namespace đã được nhận hoặc được dành riêng

Nếu việc xuất bản thất bại vì handle chủ sở hữu, namespace tổ chức, scope gói, slug skill
hoặc tên gói đã được nhận hoặc được dành riêng, trước tiên hãy xác nhận rằng bạn đang
xuất bản với chủ sở hữu khớp với namespace đó. Đối với gói plugin,
các tên có scope như `@example-org/example-plugin` phải được xuất bản với
...
[Khiếu nại tổ chức và Namespace](/vi/clawhub/namespace-claims) để biết hướng dẫn về bằng chứng và những gì
...
- Xuất bản bản sao đã chỉnh sửa của bạn dưới dạng slug hoặc fork mới.
