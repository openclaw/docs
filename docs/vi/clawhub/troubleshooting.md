---
read_when:
    - Các lệnh CLI của ClawHub hoặc lệnh sổ đăng ký của OpenClaw không thành công
...
- `RateLimit-Remaining`: ngân sách chính xác còn lại của bạn khi header có mặt. Với `429`, giá trị này là `0`.
...
Đối với plugin, hãy kiểm tra metadata tương thích trong `package.json`. Các lần xuất bản code-plugin
...
## Xuất bản thất bại vì namespace đã được xác nhận hoặc dành riêng
...
xuất bản với chủ sở hữu khớp với namespace. Đối với các gói plugin,
...
Hãy xem trước nếu bạn chưa chắc nội dung nào sẽ được xuất bản:
...
- Tôn trọng header thử lại `429` và cache phản hồi danh sách/tìm kiếm công khai.
