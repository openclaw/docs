...
x-i18n:
    generated_at: "2026-07-02T17:40:34Z"
    model: gpt-5.5
...
công cụ bắt buộc nên được khai báo để người dùng và bộ quét có thể hiểu package.

Đối với plugins, hãy kiểm tra metadata tương thích trong `package.json`. Các lần phát hành
...
ClawHub dùng danh tính GitHub và ghi nhận nguồn để kết nối package với
...
Nếu publish thất bại vì handle chủ sở hữu, namespace tổ chức, phạm vi package, slug Skills,
hoặc tên package đã được xác nhận hoặc giữ riêng, trước tiên hãy xác nhận rằng bạn đang
phát hành với chủ sở hữu khớp với namespace. Đối với package plugin,
...
Các tệp cục bộ không khớp với bất kỳ phiên bản nào ClawHub biết đến. Chọn một:
...
- Kiểm tra trang chi tiết package để biết trạng thái quét và metadata tương thích.
- Xác nhận phiên bản OpenClaw của bạn đáp ứng dải tương thích được package
  công bố.
- Nếu package bị ẩn, bị giữ lại hoặc bị chặn, package đó có thể không cài đặt được cho đến khi
  chủ sở hữu giải quyết vấn đề.

## Yêu cầu Public API thất bại
