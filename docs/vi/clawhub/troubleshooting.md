---
read_when:
    - Các lệnh ClawHub CLI hoặc các lệnh kho đăng ký OpenClaw không thành công
    - Không thể cài đặt, xuất bản hoặc cập nhật gói
summary: Khắc phục các sự cố về đăng nhập, cài đặt, xuất bản, đồng bộ hóa, cập nhật và API của ClawHub.
x-i18n:
    generated_at: "2026-05-12T12:50:20Z"
    model: gpt-5.5
    provider: openai
...
Nếu nhiều người dùng cùng chia sẻ một IP đi ra, giới hạn IP ẩn danh có thể bị chạm tới ngay cả khi mỗi
...
ClawHub dùng danh tính GitHub và thuộc tính nguồn để kết nối các gói với
...
- Tôn trọng các header thử lại `429` và lưu vào bộ nhớ đệm các phản hồi danh sách/tìm kiếm công khai.
- Liên kết người dùng trở lại mục niêm yết ClawHub chính thức.
