---
read_when:
    - Các lệnh CLI ClawHub hoặc lệnh sổ đăng ký OpenClaw bị lỗi
...
x-i18n:
    generated_at: "2026-06-30T22:22:21Z"
    model: gpt-5.5
...
- Trong môi trường headless, hãy tạo API token trong giao diện web ClawHub và chạy:
...
Nếu nhiều người dùng chia sẻ một IP đầu ra, giới hạn IP ẩn danh có thể bị chạm tới ngay cả khi mỗi
...
- Kiểm tra slug chính xác hoặc trang owner nếu bạn biết.
...
Chẩn đoán hiển thị với owner có thể giải thích trạng thái quét, upload-gate hoặc kiểm duyệt.
...
Đối với Plugin, hãy kiểm tra metadata tương thích trong `package.json`. Việc xuất bản
...
## Xuất bản thất bại với lỗi owner GitHub hoặc nguồn
...
## Xuất bản thất bại vì namespace đã được nhận hoặc đặt trước

Nếu xuất bản thất bại vì handle owner, namespace tổ chức, scope gói, slug kỹ năng,
hoặc tên gói đã được nhận hoặc đặt trước, trước tiên hãy xác nhận rằng bạn đang
xuất bản với owner khớp namespace. Đối với gói Plugin,
các tên có scope như `@example-org/example-plugin` phải được xuất bản dưới
owner `example-org` tương ứng.

Nếu bạn tin rằng tổ chức, dự án hoặc thương hiệu của mình là owner namespace hợp pháp nhưng
bạn không thể quản lý owner ClawHub hiện tại, hãy mở một
...
- Không sao chép nội dung ẩn, riêng tư, bị giữ lại hoặc bị chặn kiểm duyệt ra ngoài
