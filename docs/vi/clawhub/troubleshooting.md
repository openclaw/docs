---
read_when:
    - Các lệnh CLI ClawHub hoặc sổ đăng ký OpenClaw thất bại
    - Không thể cài đặt, phát hành hoặc cập nhật một gói
summary: Khắc phục các sự cố về đăng nhập, cài đặt, xuất bản, cập nhật và API của ClawHub.
x-i18n:
    generated_at: "2026-07-01T13:05:00Z"
    model: gpt-5.5
...
- `RateLimit-Remaining`: hạn mức còn lại chính xác của bạn khi header có mặt. Trên `429`, giá trị là `0`.
...
publish bằng chủ sở hữu khớp với namespace. Đối với gói Plugin,
các tên có scope như `@example-org/example-plugin` phải được publish dưới
chủ sở hữu `example-org` tương ứng.

Nếu bạn tin rằng tổ chức, dự án hoặc thương hiệu của bạn là chủ sở hữu namespace hợp pháp nhưng
bạn không thể quản lý chủ sở hữu ClawHub hiện tại, hãy mở một
[issue Khiếu nại tổ chức / namespace](https://github.com/openclaw/clawhub/issues/new?template=org-namespace-claim.yml)
...
Các tệp cục bộ không khớp với bất kỳ phiên bản nào mà ClawHub biết. Hãy chọn một:
...
- Xác nhận phiên bản OpenClaw của bạn đáp ứng phạm vi tương thích
  mà gói quảng bá.
- Nếu gói bị ẩn, bị giữ lại hoặc bị chặn, nó có thể không cài đặt được cho đến khi
  chủ sở hữu giải quyết vấn đề.

## Yêu cầu API công khai thất bại

- Tuân thủ header thử lại `429` và cache phản hồi danh sách/tìm kiếm công khai.
- Liên kết người dùng trở lại danh sách ClawHub chính thức.
- Không sao chép nội dung bị ẩn, riêng tư, bị giữ lại hoặc bị kiểm duyệt chặn ra ngoài
