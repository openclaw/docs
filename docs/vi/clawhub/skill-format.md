...
x-i18n:
    generated_at: "2026-07-04T10:45:47Z"
    model: gpt-5.5
...
Trình nhập GitHub trên web nghiêm ngặt hơn thao tác xuất bản/đồng bộ cục bộ. Nó chỉ phát hiện
...
- `description` được dùng làm phần tóm tắt kỹ năng trong giao diện/tìm kiếm.
...
Khai báo các biến môi trường tùy chọn dưới `metadata.openclaw.envVars` và đặt `required: false`. Không thêm mục tùy chọn vào `requires.env`, vì `requires.env` có nghĩa là kỹ năng không thể chạy nếu thiếu chúng.
...
Phân tích bảo mật của ClawHub kiểm tra rằng những gì kỹ năng của bạn khai báo khớp với những gì nó thực sự làm. Nếu mã của bạn tham chiếu `TODOIST_API_KEY` nhưng frontmatter không khai báo nó dưới `requires.env`, `primaryEnv`, hoặc `envVars`, phân tích sẽ đánh dấu lỗi không khớp siêu dữ liệu. Giữ khai báo chính xác giúp kỹ năng của bạn vượt qua đánh giá và giúp người dùng hiểu họ đang cài đặt gì.
...
- Các loại nội dung bắt đầu bằng `text/` được coi là văn bản; cộng thêm một danh sách cho phép nhỏ (JSON/YAML/TOML/JS/TS/Markdown/SVG).

Giới hạn (phía máy chủ):

- Tổng kích thước bundle: 50MB.
- Văn bản nhúng bao gồm `SKILL.md` + tối đa khoảng 40 tệp không phải `.md` (giới hạn theo nỗ lực tối đa).

## Slug

- Mặc định được suy ra từ tên thư mục.
- Phạm vi gói phải khớp chính xác với handle nhà xuất bản ClawHub. Handle nhà xuất bản có thể dùng chữ cái thường, chữ số, dấu gạch nối, dấu chấm, và dấu gạch dưới; chúng phải bắt đầu và kết thúc bằng một chữ cái thường hoặc chữ số.
- Slug gói phải viết thường và an toàn cho npm, ví dụ `@example.tools/demo-plugin` hoặc `demo-plugin`.
...
- Bất kỳ ai cũng có thể sử dụng, sửa đổi, và phân phối lại kỹ năng đã xuất bản, bao gồm cả mục đích thương mại.
...
- Không thêm siêu dữ liệu giá vào `SKILL.md`; nó không thuộc định dạng kỹ năng và sẽ không làm cho kỹ năng đã xuất bản trở thành trả phí.
- Nếu kỹ năng của bạn tích hợp với một dịch vụ bên thứ ba trả phí, hãy ghi rõ chi phí bên ngoài và tài khoản bắt buộc trong hướng dẫn kỹ năng và khai báo env (`requires.env` cho các biến bắt buộc, hoặc `envVars` với `required: false` cho các biến tùy chọn).
