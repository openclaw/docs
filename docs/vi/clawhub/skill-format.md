...
- `description` được dùng làm tóm tắt kỹ năng trong giao diện người dùng/tìm kiếm.
...
| `install`          | `array`    | Đặc tả cài đặt cho phụ thuộc (xem bên dưới).                                                                                                   |
...
Chỉ các tệp “dựa trên văn bản” mới được publish chấp nhận.
...
- Các loại nội dung bắt đầu bằng `text/` được xử lý là văn bản; cộng thêm một danh sách nhỏ được phép (JSON/YAML/TOML/JS/TS/Markdown/SVG).
...
- Phạm vi gói phải khớp chính xác với handle nhà xuất bản ClawHub. Handle nhà xuất bản có thể dùng chữ cái thường, chữ số, dấu gạch nối, dấu chấm và dấu gạch dưới; chúng phải bắt đầu và kết thúc bằng chữ cái thường hoặc chữ số.
