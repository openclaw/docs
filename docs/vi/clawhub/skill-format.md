...
x-i18n:
    generated_at: "2026-07-03T13:35:59Z"
    model: gpt-5.5
...
| `os`               | `string[]` | Hạn chế HĐH (ví dụ `["macos"]`, `["linux"]`).                                                                                                 |
...
- Các kiểu nội dung bắt đầu bằng `text/` được xử lý là văn bản; cộng thêm một danh sách nhỏ được phép (JSON/YAML/TOML/JS/TS/Markdown/SVG).
...
- Slug gói phải là chữ thường và an toàn theo npm, ví dụ `@example.tools/demo-plugin` hoặc `demo-plugin`.
...
- Đừng thêm siêu dữ liệu giá vào `SKILL.md`; nó không phải là một phần của định dạng kỹ năng và sẽ không làm cho kỹ năng đã xuất bản trở thành trả phí.
- Nếu kỹ năng của bạn tích hợp với một dịch vụ bên thứ ba có phí, hãy ghi rõ chi phí bên ngoài và tài khoản bắt buộc trong hướng dẫn kỹ năng và khai báo env (`requires.env` cho các biến bắt buộc, hoặc `envVars` với `required: false` cho các biến tùy chọn).
