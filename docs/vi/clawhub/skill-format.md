...
x-i18n:
    generated_at: "2026-07-02T17:41:56Z"
    model: gpt-5.5
...
Siêu dữ liệu cài đặt cục bộ (được ghi bởi CLI):

- `<skill>/.clawhub/origin.json` (`.clawdhub` cũ)

Trạng thái cài đặt workdir (được ghi bởi CLI):
...
- Các loại nội dung bắt đầu bằng `text/` được xử lý như văn bản; cộng thêm một danh sách nhỏ được phép (JSON/YAML/TOML/JS/TS/Markdown/SVG).
...
- Bất kỳ ai cũng có thể sử dụng, sửa đổi và phân phối lại các kỹ năng đã xuất bản, bao gồm cho mục đích thương mại.
...
- Không thêm siêu dữ liệu định giá vào `SKILL.md`; nó không phải là một phần của định dạng kỹ năng và sẽ không khiến một kỹ năng đã xuất bản trở thành trả phí.
