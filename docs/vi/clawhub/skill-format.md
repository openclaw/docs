...
x-i18n:
    generated_at: "2026-06-30T22:20:57Z"
    model: gpt-5.5
...
các tệp `SKILL.md` hoặc `skills.md` cũ trong các kho lưu trữ công khai, không phải fork, do
tài khoản GitHub đã đăng nhập sở hữu. Nó không nhập kho lưu trữ riêng tư, fork,
...
---
name: my-skill
description: Tóm tắt ngắn về những gì skill này thực hiện.
version: 1.0.0
---
```
...
Khai báo các biến môi trường tùy chọn dưới `metadata.openclaw.envVars` và đặt `required: false`. Không thêm mục tùy chọn vào `requires.env`, vì `requires.env` nghĩa là skill không thể chạy nếu thiếu chúng.
...
- Bất kỳ ai cũng có thể dùng, sửa đổi và phân phối lại skills đã xuất bản, bao gồm cả mục đích thương mại.
...
- Không thêm siêu dữ liệu giá vào `SKILL.md`; đây không phải là một phần của định dạng skill và sẽ không biến một skill đã xuất bản thành trả phí.
- Nếu skill của bạn tích hợp với một dịch vụ bên thứ ba có trả phí, hãy ghi rõ chi phí bên ngoài và tài khoản bắt buộc trong hướng dẫn skill và khai báo env (`requires.env` cho biến bắt buộc, hoặc `envVars` với `required: false` cho biến tùy chọn).
