...
x-i18n:
    generated_at: "2026-07-05T06:56:39Z"
    model: gpt-5.5
...
Khai báo biến môi trường tùy chọn dưới `metadata.openclaw.envVars` và đặt `required: false`. Không thêm mục tùy chọn vào `requires.env`, vì `requires.env` nghĩa là skill không thể chạy nếu thiếu chúng.
...
Phân tích bảo mật của ClawHub kiểm tra rằng những gì skill của bạn khai báo khớp với những gì nó thực sự làm. Nếu mã của bạn tham chiếu `TODOIST_API_KEY` nhưng frontmatter không khai báo biến đó dưới `requires.env`, `primaryEnv`, hoặc `envVars`, phân tích sẽ gắn cờ không khớp siêu dữ liệu. Giữ các khai báo chính xác giúp skill của bạn vượt qua đánh giá và giúp người dùng hiểu họ đang cài đặt gì.
...
- Phạm vi package phải khớp chính xác với handle nhà xuất bản ClawHub. Handle nhà xuất bản có thể dùng chữ thường, số, dấu gạch nối, dấu chấm, và dấu gạch dưới; chúng phải bắt đầu và kết thúc bằng một chữ thường hoặc số.
...
- Không thêm siêu dữ liệu giá vào `SKILL.md`; đó không phải là một phần của định dạng skill và sẽ không khiến một skill đã xuất bản trở thành trả phí.
