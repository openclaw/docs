...
x-i18n:
    generated_at: "2026-07-03T02:49:51Z"
    model: gpt-5.5
...
Trạng thái cài đặt trong workdir (do CLI ghi):
...
- Máy chủ trích xuất siêu dữ liệu từ frontmatter trong lúc xuất bản.
...
Dùng `requires.env` cho các biến môi trường phải có trước khi kỹ năng có thể chạy. Dùng `envVars` khi bạn cần siêu dữ liệu theo từng biến, bao gồm cả biến tùy chọn với `required: false`.
...
| `requires.anyBins` | `string[]` | Các tệp nhị phân CLI mà phải có ít nhất một tệp tồn tại.                                                                                       |
...
Khai báo biến môi trường tùy chọn dưới `metadata.openclaw.envVars` và đặt `required: false`. Không thêm mục tùy chọn vào `requires.env`, vì `requires.env` nghĩa là kỹ năng không thể chạy nếu thiếu chúng.
...
Phân tích bảo mật của ClawHub kiểm tra rằng nội dung kỹ năng của bạn khai báo khớp với những gì nó thực sự làm. Nếu mã của bạn tham chiếu `TODOIST_API_KEY` nhưng frontmatter không khai báo biến đó dưới `requires.env`, `primaryEnv`, hoặc `envVars`, phân tích sẽ gắn cờ là siêu dữ liệu không khớp. Giữ khai báo chính xác giúp kỹ năng của bạn vượt qua đánh giá và giúp người dùng hiểu họ đang cài đặt gì.
...
- Scope gói phải khớp chính xác với handle nhà xuất bản ClawHub. Handle nhà xuất bản có thể dùng chữ cái thường, chữ số, dấu gạch nối, dấu chấm, và dấu gạch dưới; chúng phải bắt đầu và kết thúc bằng chữ cái thường hoặc chữ số.
...
- Bất kỳ ai cũng có thể dùng, sửa đổi, và phân phối lại các kỹ năng đã publish, bao gồm cả mục đích thương mại.
...
- Không thêm siêu dữ liệu giá vào `SKILL.md`; đó không phải là một phần của định dạng kỹ năng và sẽ không khiến kỹ năng đã publish trở thành trả phí.
- Nếu kỹ năng của bạn tích hợp với dịch vụ bên thứ ba trả phí, hãy ghi rõ chi phí bên ngoài và tài khoản bắt buộc trong hướng dẫn kỹ năng và khai báo môi trường (`requires.env` cho biến bắt buộc, hoặc `envVars` với `required: false` cho biến tùy chọn).
