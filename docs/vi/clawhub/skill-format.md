...
x-i18n:
    generated_at: "2026-05-12T08:44:42Z"
    model: gpt-5.5
    provider: openai
...
Phân tích bảo mật của ClawHub kiểm tra rằng nội dung kỹ năng của bạn khai báo khớp với những gì nó thực sự làm. Nếu mã của bạn tham chiếu `TODOIST_API_KEY` nhưng frontmatter của bạn không khai báo biến đó dưới `requires.env`, `primaryEnv`, hoặc `envVars`, phân tích sẽ gắn cờ sai khớp siêu dữ liệu. Giữ khai báo chính xác giúp kỹ năng của bạn vượt qua đánh giá và giúp người dùng hiểu họ đang cài đặt gì.
...
- Nếu kỹ năng của bạn tích hợp với dịch vụ bên thứ ba có trả phí, hãy ghi rõ chi phí bên ngoài và tài khoản bắt buộc trong hướng dẫn kỹ năng và khai báo môi trường (`requires.env` cho biến bắt buộc, hoặc `envVars` với `required: false` cho biến tùy chọn).
