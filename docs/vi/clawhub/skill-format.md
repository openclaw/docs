...
Siêu dữ liệu kỹ năng được khai báo trong YAML frontmatter ở đầu `SKILL.md` của bạn. Nội dung này cho registry (và phân tích bảo mật) biết kỹ năng của bạn cần gì để chạy.
...
| `install`          | `array`    | Thông số cài đặt cho phụ thuộc (xem bên dưới).                                                                                             |
...
Khai báo biến môi trường tùy chọn dưới `metadata.openclaw.envVars` và đặt `required: false`. Đừng thêm mục tùy chọn vào `requires.env`, vì `requires.env` nghĩa là kỹ năng không thể chạy nếu thiếu chúng.
...
- Đừng thêm siêu dữ liệu giá vào `SKILL.md`; nó không phải là một phần của định dạng kỹ năng và sẽ không biến kỹ năng đã xuất bản thành trả phí.
