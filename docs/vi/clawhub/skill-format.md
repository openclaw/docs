...
Trình nhập GitHub trên web nghiêm ngặt hơn so với publish/sync cục bộ. Nó chỉ phát hiện
...
Khai báo các biến môi trường tùy chọn dưới `metadata.openclaw.envVars` và đặt `required: false`. Không thêm các mục tùy chọn vào `requires.env`, vì `requires.env` nghĩa là kỹ năng không thể chạy nếu thiếu chúng.
...
- Bất kỳ ai cũng có thể sử dụng, chỉnh sửa và phân phối lại kỹ năng đã xuất bản, kể cả cho mục đích thương mại.
...
- Nếu kỹ năng của bạn tích hợp với một dịch vụ bên thứ ba trả phí, hãy ghi rõ chi phí bên ngoài và tài khoản bắt buộc trong hướng dẫn kỹ năng và khai báo env (`requires.env` cho biến bắt buộc, hoặc `envVars` với `required: false` cho biến tùy chọn).
