...
x-i18n:
    generated_at: "2026-07-01T18:14:20Z"
    model: gpt-5.5
...
kho lưu trữ đã lưu trữ/bị vô hiệu hóa, hoặc kho lưu trữ công khai của bên thứ ba.

Siêu dữ liệu cài đặt cục bộ (do CLI ghi):

- `<skill>/.clawhub/origin.json` (`.clawdhub` cũ)

Trạng thái cài đặt thư mục làm việc (do CLI ghi):
...
Nếu kỹ năng của bạn cần cài đặt dependency, hãy khai báo chúng trong mảng `install`:
...
- Tệp script vẫn được quét sau khi tải lên; các tệp PowerShell `.ps1`, `.psm1`, và `.psd1` được chấp nhận như văn bản.
- Content type bắt đầu bằng `text/` được xem là văn bản; cộng thêm một danh sách cho phép nhỏ (JSON/YAML/TOML/JS/TS/Markdown/SVG).
...
- Bất kỳ ai cũng có thể sử dụng, sửa đổi, và phân phối lại các kỹ năng đã xuất bản, kể cả cho mục đích thương mại.
...
- Không thêm siêu dữ liệu giá vào `SKILL.md`; nó không phải là một phần của định dạng kỹ năng và sẽ không làm cho kỹ năng đã xuất bản trở thành trả phí.
