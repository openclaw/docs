...
summary: Định dạng thư mục kỹ năng, các tệp bắt buộc, loại tệp được phép, giới hạn.
x-i18n:
...
các tệp `SKILL.md` hoặc `skills.md` cũ trong kho lưu trữ công khai, không phải fork, thuộc sở hữu của
...
| `os`               | `string[]` | Giới hạn OS (ví dụ `["macos"]`, `["linux"]`).                                                                                              |
...
- Content type bắt đầu bằng `text/` được xem là văn bản; cộng thêm một danh sách nhỏ được phép (JSON/YAML/TOML/JS/TS/Markdown/SVG).
...
- Package scope phải khớp chính xác với handle nhà xuất bản ClawHub. Handle nhà xuất bản có thể dùng chữ cái thường, số, dấu gạch nối, dấu chấm, và dấu gạch dưới; chúng phải bắt đầu và kết thúc bằng chữ cái thường hoặc số.
- Package slug phải viết thường và an toàn với npm, ví dụ `@example.tools/demo-plugin` hoặc `demo-plugin`.
