...
- `.clawhubignore` (mẫu bỏ qua khi publish/sync, `.clawdhubignore` cũ)
...
| `requires.bins`    | `string[]` | Các CLI binary đều phải được cài đặt.                                                                                                                         |
| `requires.anyBins` | `string[]` | Các CLI binary trong đó ít nhất một binary phải tồn tại.                                                                                                      |
...
- Nếu kỹ năng của bạn tích hợp với một dịch vụ bên thứ ba trả phí, hãy ghi rõ chi phí bên ngoài và tài khoản cần thiết trong hướng dẫn kỹ năng và khai báo biến môi trường (`requires.env` cho các biến bắt buộc, hoặc `envVars` với `required: false` cho các biến tùy chọn).
