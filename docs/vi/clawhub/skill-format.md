...
x-i18n:
    generated_at: "2026-07-04T20:35:02Z"
    model: gpt-5.5
...
Trình nhập GitHub trên web nghiêm ngặt hơn so với xuất bản/đồng bộ cục bộ. Nó chỉ phát hiện
...
Dùng `requires.env` cho các biến môi trường bắt buộc phải có trước khi kỹ năng có thể chạy. Dùng `envVars` khi bạn cần siêu dữ liệu theo từng biến, bao gồm các biến tùy chọn với `required: false`.
...
Nếu kỹ năng của bạn cần cài đặt các phụ thuộc, hãy khai báo chúng trong mảng `install`:
...
Khai báo biến môi trường tùy chọn dưới `metadata.openclaw.envVars` và đặt `required: false`. Không thêm các mục tùy chọn vào `requires.env`, vì `requires.env` nghĩa là kỹ năng không thể chạy nếu thiếu chúng.
...
- Các kiểu nội dung bắt đầu bằng `text/` được xem là văn bản; cộng thêm một danh sách cho phép nhỏ (JSON/YAML/TOML/JS/TS/Markdown/SVG).

Giới hạn (phía máy chủ):

- Tổng kích thước bundle: 50MB.
- Văn bản nhúng bao gồm `SKILL.md` + tối đa khoảng 40 tệp không phải `.md` (giới hạn nỗ lực tối đa).

## Slug

- Mặc định được dẫn xuất từ tên thư mục.
- Phạm vi package phải khớp chính xác với handle nhà xuất bản ClawHub. Handle nhà xuất bản có thể dùng chữ cái thường, số, dấu gạch nối, dấu chấm, và dấu gạch dưới; phải bắt đầu và kết thúc bằng chữ cái thường hoặc số.
- Slug package phải viết thường và an toàn với npm, ví dụ `@example.tools/demo-plugin` hoặc `demo-plugin`.
...
- Tất cả kỹ năng xuất bản trên ClawHub được cấp phép theo `MIT-0`.
- Bất kỳ ai cũng có thể dùng, sửa đổi, và phân phối lại kỹ năng đã xuất bản, kể cả cho mục đích thương mại.
...
- Không thêm siêu dữ liệu giá vào `SKILL.md`; đó không phải là một phần của định dạng kỹ năng và sẽ không làm cho kỹ năng đã xuất bản trở thành trả phí.
- Nếu kỹ năng của bạn tích hợp với dịch vụ bên thứ ba có tính phí, hãy ghi rõ chi phí bên ngoài và tài khoản bắt buộc trong hướng dẫn kỹ năng và khai báo môi trường (`requires.env` cho biến bắt buộc, hoặc `envVars` với `required: false` cho biến tùy chọn).
