...
- Các loại nội dung bắt đầu bằng `text/` được xử lý như văn bản; cộng thêm một danh sách cho phép nhỏ (JSON/YAML/TOML/JS/TS/Markdown/SVG).

Giới hạn (phía máy chủ):

- Tổng kích thước bundle: 50MB.
- Văn bản nhúng bao gồm `SKILL.md` + tối đa khoảng 40 tệp không phải `.md` (giới hạn cố gắng tối đa).

## Slug

- Mặc định được suy ra từ tên thư mục.
- Package scope phải khớp chính xác với handle nhà xuất bản ClawHub. Handle nhà xuất bản có thể dùng chữ thường, số, dấu gạch nối, dấu chấm và dấu gạch dưới; chúng phải bắt đầu và kết thúc bằng chữ thường hoặc số.
- Package slug phải là chữ thường và an toàn với npm, ví dụ `@example.tools/demo-plugin` hoặc `demo-plugin`.
...
- Không thêm siêu dữ liệu giá vào `SKILL.md`; đó không phải là một phần của định dạng kỹ năng và sẽ không khiến kỹ năng đã xuất bản trở thành kỹ năng trả phí.
- Nếu kỹ năng của bạn tích hợp với dịch vụ bên thứ ba có trả phí, hãy ghi rõ chi phí bên ngoài và tài khoản bắt buộc trong hướng dẫn kỹ năng và khai báo env (`requires.env` cho biến bắt buộc, hoặc `envVars` với `required: false` cho biến tùy chọn).
