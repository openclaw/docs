...
x-i18n:
    generated_at: "2026-07-04T03:53:22Z"
    model: gpt-5.5
...
- `.clawhubignore` (các mẫu bỏ qua khi phát hành, `.clawdhubignore` cũ)
- `.gitignore` (cũng được áp dụng)

## Nhập từ GitHub

Bộ nhập GitHub trên web nghiêm ngặt hơn publish/sync cục bộ. Nó chỉ phát hiện
...
| `requires.bins`    | `string[]` | Các binary CLI bắt buộc phải được cài đặt đầy đủ.                                                                                                 |
| `requires.anyBins` | `string[]` | Các binary CLI trong đó phải tồn tại ít nhất một binary.                                                                                          |
...
Khai báo các biến môi trường tùy chọn trong `metadata.openclaw.envVars` và đặt `required: false`. Không thêm mục tùy chọn vào `requires.env`, vì `requires.env` nghĩa là kỹ năng không thể chạy nếu thiếu chúng.
...
Chỉ các tệp “dạng văn bản” mới được chấp nhận khi phát hành.

- Danh sách đuôi tệp được phép nằm trong `packages/schema/src/textFiles.ts` (`TEXT_FILE_EXTENSIONS`).
...
- Phạm vi package phải khớp chính xác với handle nhà phát hành ClawHub. Handle nhà phát hành có thể dùng chữ cái thường, số, dấu gạch nối, dấu chấm, và dấu gạch dưới; chúng phải bắt đầu và kết thúc bằng chữ cái thường hoặc số.
...
- Tất cả kỹ năng được phát hành trên ClawHub đều được cấp phép theo `MIT-0`.
- Bất kỳ ai cũng có thể sử dụng, sửa đổi, và phân phối lại các kỹ năng đã phát hành, kể cả cho mục đích thương mại.
...
- Không thêm siêu dữ liệu giá vào `SKILL.md`; đó không phải là một phần của định dạng kỹ năng và sẽ không khiến kỹ năng đã phát hành trở thành kỹ năng trả phí.
