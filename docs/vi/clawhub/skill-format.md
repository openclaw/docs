...
Trình nhập GitHub trên web nghiêm ngặt hơn xuất bản/đồng bộ cục bộ. Nó chỉ phát hiện
các tệp `SKILL.md` hoặc `skills.md` cũ trong các kho lưu trữ công khai, không phải fork, thuộc sở hữu của
tài khoản GitHub đã đăng nhập. Nó không nhập kho lưu trữ riêng tư, fork,
...
Siêu dữ liệu Skill được khai báo trong frontmatter YAML ở đầu `SKILL.md` của bạn. Phần này cho registry (và phân tích bảo mật) biết skill của bạn cần gì để chạy.
...
| `requires.bins`    | `string[]` | Các tệp nhị phân CLI mà tất cả đều phải được cài đặt.                                                                                       |
...
| `os`               | `string[]` | Ràng buộc hệ điều hành (ví dụ `["macos"]`, `["linux"]`).                                                                                    |
...
Khai báo biến môi trường tùy chọn trong `metadata.openclaw.envVars` và đặt `required: false`. Đừng thêm mục tùy chọn vào `requires.env`, vì `requires.env` nghĩa là skill không thể chạy nếu thiếu chúng.
...
Phân tích bảo mật của ClawHub kiểm tra rằng những gì skill của bạn khai báo khớp với những gì nó thực sự làm. Nếu mã của bạn tham chiếu `TODOIST_API_KEY` nhưng frontmatter của bạn không khai báo nó trong `requires.env`, `primaryEnv`, hoặc `envVars`, phân tích sẽ gắn cờ không khớp siêu dữ liệu. Giữ khai báo chính xác giúp skill của bạn vượt qua đánh giá và giúp người dùng hiểu họ đang cài đặt gì.
...
Chỉ các tệp “dựa trên văn bản” được publish chấp nhận.

- Danh sách phần mở rộng được phép nằm trong `packages/schema/src/textFiles.ts` (`TEXT_FILE_EXTENSIONS`).
- Tệp script vẫn được quét sau khi tải lên; các tệp PowerShell `.ps1`, `.psm1`, và `.psd1` được chấp nhận dưới dạng văn bản.
- Kiểu nội dung bắt đầu bằng `text/` được xử lý như văn bản; cộng thêm một danh sách nhỏ được phép (JSON/YAML/TOML/JS/TS/Markdown/SVG).

Giới hạn (phía máy chủ):

- Tổng kích thước gói: 50MB.
- Văn bản nhúng bao gồm `SKILL.md` + tối đa khoảng 40 tệp không phải `.md` (giới hạn nỗ lực tốt nhất).

## Slug

- Mặc định được suy ra từ tên thư mục.
- Phạm vi gói phải khớp chính xác với handle nhà xuất bản ClawHub. Handle nhà xuất bản có thể dùng chữ cái thường, số, dấu gạch nối, dấu chấm và dấu gạch dưới; chúng phải bắt đầu và kết thúc bằng chữ cái thường hoặc số.
- Slug gói phải là chữ thường và an toàn với npm, ví dụ `@example.tools/demo-plugin` hoặc `demo-plugin`.
...
- Tất cả skills được xuất bản trên ClawHub được cấp phép theo `MIT-0`.
- Bất kỳ ai cũng có thể dùng, sửa đổi và phân phối lại skills đã xuất bản, kể cả cho mục đích thương mại.
- Không yêu cầu ghi công.
- Đừng thêm điều khoản giấy phép xung đột trong `SKILL.md`; ClawHub không hỗ trợ ghi đè giấy phép theo từng skill.

## Skills trả phí

- ClawHub không hỗ trợ skills trả phí, định giá theo từng skill, tường phí, hoặc chia sẻ doanh thu.
- Đừng thêm siêu dữ liệu giá vào `SKILL.md`; đó không phải là một phần của định dạng skill và sẽ không biến một skill đã xuất bản thành trả phí.
