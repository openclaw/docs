...
x-i18n:
    generated_at: "2026-05-10T19:26:16Z"
    model: gpt-5.5
    provider: openai
...
- `.clawhubignore` (mẫu bỏ qua cho xuất bản/đồng bộ, `.clawdhubignore` cũ)
...
Trạng thái cài đặt của thư mục làm việc (do CLI ghi):
...
- `description` được dùng làm tóm tắt kỹ năng trong giao diện/tìm kiếm.

## Siêu dữ liệu frontmatter

Siêu dữ liệu kỹ năng được khai báo trong YAML frontmatter ở đầu `SKILL.md` của bạn. Thông tin này cho registry (và phân tích bảo mật) biết kỹ năng của bạn cần gì để chạy.
...
Dùng `requires.env` cho các biến môi trường phải có trước khi kỹ năng có thể chạy. Dùng `envVars` khi bạn cần siêu dữ liệu cho từng biến, bao gồm các biến tùy chọn với `required: false`.
...
| `always`           | `boolean`  | Nếu `true`, kỹ năng luôn hoạt động (không cần cài đặt rõ ràng).                                                                                        |
...
Khai báo các biến môi trường tùy chọn dưới `metadata.openclaw.envVars` và đặt `required: false`. Đừng thêm các mục tùy chọn vào `requires.env`, vì `requires.env` nghĩa là kỹ năng không thể chạy nếu thiếu chúng.
...
Phân tích bảo mật của ClawHub kiểm tra rằng những gì kỹ năng của bạn khai báo khớp với những gì nó thực sự làm. Nếu mã của bạn tham chiếu `TODOIST_API_KEY` nhưng frontmatter của bạn không khai báo biến đó dưới `requires.env`, `primaryEnv`, hoặc `envVars`, phân tích sẽ gắn cờ không khớp siêu dữ liệu. Giữ khai báo chính xác giúp kỹ năng của bạn vượt qua đánh giá và giúp người dùng hiểu họ đang cài đặt gì.
...
Chỉ các tệp “dựa trên văn bản” được chấp nhận khi xuất bản.
...
- Nếu kỹ năng của bạn tích hợp với một dịch vụ bên thứ ba trả phí, hãy ghi rõ chi phí bên ngoài và tài khoản bắt buộc trong hướng dẫn kỹ năng và khai báo biến môi trường (`requires.env` cho các biến bắt buộc, hoặc `envVars` với `required: false` cho các biến tùy chọn).
