...
- `.clawhubignore` (các mẫu bỏ qua khi publish/sync, `.clawdhubignore` cũ)
...
- Các kiểu nội dung bắt đầu bằng `text/` được xem là văn bản; cùng với một danh sách cho phép nhỏ (JSON/YAML/TOML/JS/TS/Markdown/SVG).
...
- Thẻ là các con trỏ dạng chuỗi đến một phiên bản; `latest` thường được dùng.
...
- Không thêm siêu dữ liệu giá vào `SKILL.md`; đó không phải là một phần của định dạng kỹ năng và sẽ không biến một kỹ năng đã publish thành trả phí.
