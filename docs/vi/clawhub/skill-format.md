...
x-i18n:
    generated_at: "2026-06-28T06:22:02Z"
    model: gpt-5.5
...
Siêu dữ liệu skill được khai báo trong frontmatter YAML ở đầu tệp `SKILL.md` của bạn. Phần này cho registry (và phân tích bảo mật) biết skill của bạn cần gì để chạy.
...
Dùng `requires.env` cho các biến môi trường phải có trước khi skill có thể chạy. Dùng `envVars` khi bạn cần siêu dữ liệu cho từng biến, bao gồm các biến tùy chọn với `required: false`.
...
| `os`               | `string[]` | Ràng buộc OS (ví dụ `["macos"]`, `["linux"]`).                                                                                         |
...
Nếu kỹ năng của bạn cần cài đặt các phần phụ thuộc, hãy khai báo chúng trong mảng `install`:
...
- Phạm vi package phải khớp chính xác với mã định danh nhà phát hành ClawHub. Mã định danh nhà phát hành có thể dùng chữ cái thường, chữ số, dấu gạch nối, dấu chấm và dấu gạch dưới; chúng phải bắt đầu và kết thúc bằng chữ cái thường hoặc chữ số.
- Slug package phải viết thường và an toàn cho npm, ví dụ `@example.tools/demo-plugin` hoặc `demo-plugin`.

## Phiên bản + thẻ

- Mỗi lần publish sẽ tạo một phiên bản mới (semver).
