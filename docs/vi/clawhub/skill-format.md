...
x-i18n:
    generated_at: "2026-07-03T09:40:48Z"
    model: gpt-5.5
...
Dùng `requires.env` cho các biến môi trường phải có trước khi kỹ năng có thể chạy. Dùng `envVars` khi bạn cần siêu dữ liệu cho từng biến, bao gồm cả biến tùy chọn với `required: false`.
...
Khai báo các biến môi trường tùy chọn trong `metadata.openclaw.envVars` và đặt `required: false`. Đừng thêm các mục tùy chọn vào `requires.env`, vì `requires.env` nghĩa là kỹ năng không thể chạy nếu thiếu chúng.
...
- Văn bản nhúng bao gồm `SKILL.md` + tối đa khoảng 40 tệp không phải `.md` (giới hạn theo nỗ lực tốt nhất).

## Slug

- Mặc định được suy ra từ tên thư mục.
- Scope gói phải khớp chính xác với định danh nhà xuất bản ClawHub. Định danh nhà xuất bản có thể dùng chữ thường, số, dấu gạch nối, dấu chấm và dấu gạch dưới; chúng phải bắt đầu và kết thúc bằng chữ thường hoặc số.
...
- Tất cả kỹ năng được xuất bản trên ClawHub đều được cấp phép theo `MIT-0`.
- Bất kỳ ai cũng có thể sử dụng, sửa đổi và phân phối lại các kỹ năng đã xuất bản, bao gồm cả mục đích thương mại.
...
- Đừng thêm siêu dữ liệu giá vào `SKILL.md`; đó không phải là một phần của định dạng kỹ năng và sẽ không làm cho kỹ năng đã xuất bản trở thành trả phí.
