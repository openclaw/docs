...
summary: Định dạng thư mục kỹ năng, các tệp bắt buộc, các loại tệp được phép, giới hạn.
x-i18n:
...
- bất kỳ tệp _dựa trên văn bản_ hỗ trợ nào (xem “Tệp được phép”)
...
- Các loại nội dung bắt đầu bằng `text/` được xử lý là văn bản; cộng với một danh sách nhỏ được phép (JSON/YAML/TOML/JS/TS/Markdown/SVG).
...
- Mọi kỹ năng được publish trên ClawHub đều được cấp phép theo `MIT-0`.
...
- Không thêm siêu dữ liệu giá vào `SKILL.md`; đó không phải là một phần của định dạng kỹ năng và sẽ không làm cho kỹ năng đã publish trở thành trả phí.
