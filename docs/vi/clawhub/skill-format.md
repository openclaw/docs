...
x-i18n:
    generated_at: "2026-07-03T23:35:58Z"
    model: gpt-5.5
...
- Máy chủ trích xuất siêu dữ liệu từ frontmatter khi xuất bản.
...
Khai báo yêu cầu runtime của kỹ năng trong `metadata.openclaw` (bí danh: `metadata.clawdbot`, `metadata.clawdis`).
...
| `requires.bins`    | `string[]` | Các binary CLI bắt buộc đều phải được cài đặt.                                                                                                    |
...
| `install`          | `array`    | Đặc tả cài đặt cho dependency (xem bên dưới).                                                                                                     |
...
Khai báo biến môi trường tùy chọn trong `metadata.openclaw.envVars` và đặt `required: false`. Không thêm các mục tùy chọn vào `requires.env`, vì `requires.env` có nghĩa là kỹ năng không thể chạy nếu thiếu chúng.
...
- Không thêm siêu dữ liệu giá vào `SKILL.md`; đó không phải là một phần của định dạng kỹ năng và sẽ không biến kỹ năng đã xuất bản thành kỹ năng trả phí.
- Nếu kỹ năng của bạn tích hợp với một dịch vụ bên thứ ba trả phí, hãy ghi rõ chi phí bên ngoài và tài khoản bắt buộc trong hướng dẫn kỹ năng và khai báo môi trường (`requires.env` cho biến bắt buộc, hoặc `envVars` với `required: false` cho biến tùy chọn).
