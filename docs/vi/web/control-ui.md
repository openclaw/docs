...
x-i18n:
    generated_at: "2026-05-06T09:35:13Z"
    model: gpt-5.5
    provider: openai
...
<AccordionGroup>
  <Accordion title="Ngữ nghĩa gửi và lịch sử">
    ...
    - Các sự kiện `chat` trực tiếp là trạng thái phân phối, trong khi `chat.history` được dựng lại từ bản ghi hội thoại phiên bền vững. Sau các sự kiện kết thúc công cụ, Control UI tải lại lịch sử và chỉ hợp nhất một phần đuôi lạc quan nhỏ; ranh giới bản ghi hội thoại được ghi tài liệu trong [WebChat](/vi/web/webchat).
  ...
  <Accordion title="Giữ lại phần hủy bỏ chưa hoàn tất">
...
<Note>
Web Push độc lập với đường dẫn chuyển tiếp APNS của iOS (xem [Cấu hình](/vi/gateway/configuration) để biết push dựa trên chuyển tiếp) và phương thức `push.test` hiện có, vốn nhắm tới ghép đôi di động native.
</Note>
...
<Warning>
Chỉ dùng `trusted` khi tài liệu nhúng thật sự cần hành vi same-origin. Với hầu hết trò chơi và canvas tương tác do agent tạo ra, `scripts` là lựa chọn an toàn hơn.
</Warning>
...
<AccordionGroup>
  ...
  <Accordion title="Chỉ dùng trong trường hợp khẩn cấp">
    ...
    <Warning>
    `dangerouslyDisableDeviceAuth` tắt các bước kiểm tra danh tính thiết bị của Control UI và là một sự hạ cấp bảo mật nghiêm trọng. Hãy hoàn tác nhanh chóng sau khi dùng trong tình huống khẩn cấp.
    </Warning>
