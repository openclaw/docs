...
summary: Chuyển văn bản thành giọng nói cho các phản hồi gửi đi — nhà cung cấp, cá tính, lệnh dấu gạch chéo và đầu ra theo từng kênh
title: Chuyển văn bản thành giọng nói
x-i18n:
    generated_at: "2026-05-02T10:56:05Z"
    model: gpt-5.5
    provider: openai
...
OpenClaw chọn nhà cung cấp đầu tiên đã cấu hình theo thứ tự tự động chọn của registry.
Công cụ agent `tts` tích hợp chỉ dành cho ý định rõ ràng: chat thông thường vẫn ở dạng
văn bản trừ khi người dùng yêu cầu âm thanh, dùng `/tts`, hoặc bật Auto-TTS/lệnh chỉ thị
giọng nói.
</Note>
...
Nếu nhiều nhà cung cấp được cấu hình, nhà cung cấp đã chọn sẽ được dùng trước và các
nhà cung cấp khác là tùy chọn dự phòng. Tóm tắt tự động dùng `summaryModel` (hoặc
...
qua `node-edge-tts`. Đây là dịch vụ web công khai không có SLA hoặc hạn ngạch đã công bố
— hãy xem là nỗ lực tối đa. Id nhà cung cấp cũ `edge` được chuẩn hóa thành `microsoft`
và `openclaw doctor --fix` viết lại cấu hình đã lưu; cấu hình mới luôn nên dùng
`microsoft`.
</Warning>
...
mô hình, persona hoặc chế độ Auto-TTS khác. Khối agent được deep-merge lên trên
`messages.tts`, vì vậy thông tin xác thực nhà cung cấp có thể nằm trong cấu hình
nhà cung cấp toàn cục:
...
trộn sâu lên các lớp trước đó, để thông tin xác thực nhà cung cấp dùng chung có thể nằm trong
`messages.tts` trong khi một kênh hoặc tài khoản bot chỉ thay đổi giọng, mô hình, danh tính thoại,
hoặc chế độ tự động:
...
Một **danh tính thoại** là một danh tính giọng nói ổn định có thể được áp dụng một cách xác định
trên nhiều nhà cung cấp. Nó có thể ưu tiên một nhà cung cấp, định nghĩa ý định lời nhắc
trung lập với nhà cung cấp, và chứa các liên kết riêng theo nhà cung cấp cho giọng, mô hình, mẫu lời nhắc,
seed, và thiết lập giọng nói.

### Danh tính thoại tối thiểu
...
### Danh tính thoại đầy đủ (lời nhắc trung lập với nhà cung cấp)
...
### Phân giải danh tính thoại

Danh tính thoại đang hoạt động được chọn một cách xác định:

1. Tùy chọn cục bộ `/tts persona <id>`, nếu được đặt.
2. `messages.tts.persona`, nếu được đặt.
3. Không có danh tính thoại.

Việc chọn nhà cung cấp chạy theo thứ tự ưu tiên cấu hình tường minh:

1. Ghi đè trực tiếp (CLI, gateway, Talk, chỉ thị TTS được cho phép).
2. Tùy chọn cục bộ `/tts provider <id>`.
3. `provider` của danh tính thoại đang hoạt động.
4. `messages.tts.provider`.
5. Tự động chọn từ registry.

Với mỗi lần thử nhà cung cấp, OpenClaw trộn cấu hình theo thứ tự này:

1. `messages.tts.providers.<id>`
2. `messages.tts.personas.<persona>.providers.<id>`
3. Ghi đè yêu cầu đáng tin cậy
4. Ghi đè chỉ thị TTS được mô hình phát ra và được cho phép

### Cách nhà cung cấp dùng lời nhắc danh tính thoại

Các trường lời nhắc danh tính thoại (`profile`, `scene`, `sampleContext`, `style`, `accent`,
`pacing`, `constraints`) là **trung lập với nhà cung cấp**. Mỗi nhà cung cấp quyết định cách
sử dụng chúng:

<AccordionGroup>
  <Accordion title="Google Gemini">
    Bọc các trường lời nhắc danh tính thoại trong cấu trúc lời nhắc TTS của Gemini **chỉ khi**
    cấu hình nhà cung cấp Google hiệu lực đặt `promptTemplate: "audio-profile-v1"`
    hoặc `personaPrompt`. Các trường `audioProfile` và `speakerName` cũ hơn vẫn được
    thêm vào đầu dưới dạng văn bản lời nhắc riêng cho Google. Các thẻ âm thanh nội tuyến như
    `[whispers]` hoặc `[laughs]` bên trong một khối `[[tts:text]]` được giữ nguyên
    trong bản chép lời Gemini; OpenClaw không tạo các thẻ này.
  </Accordion>
  <Accordion title="OpenAI">
    Ánh xạ các trường lời nhắc danh tính thoại sang trường `instructions` của yêu cầu **chỉ khi**
    không có `instructions` OpenAI tường minh nào được cấu hình. `instructions` tường minh
    luôn được ưu tiên.
  </Accordion>
  <Accordion title="Nhà cung cấp khác">
    Chỉ dùng các liên kết danh tính thoại riêng theo nhà cung cấp dưới
    `personas.<id>.providers.<provider>`. Các trường lời nhắc danh tính thoại bị bỏ qua
    trừ khi nhà cung cấp triển khai ánh xạ lời nhắc danh tính thoại riêng.
  </Accordion>
</AccordionGroup>

### Chính sách dự phòng

`fallbackPolicy` kiểm soát hành vi khi một danh tính thoại **không có liên kết** cho
...
| `provider-defaults` | Danh tính thoại bị bỏ khỏi quá trình chuẩn bị lời nhắc cho lần thử đó; nhà cung cấp dùng mặc định trung lập của nó trong khi tiếp tục dự phòng sang nhà cung cấp khác. |
...
giọng, mô hình, hoặc tốc độ cho một phản hồi duy nhất, cộng thêm một khối
`[[tts:text]]...[[/tts:text]]` tùy chọn cho các tín hiệu biểu cảm chỉ nên xuất hiện trong
âm thanh:
...
âm thanh. Cơ chế gửi khối phát trực tiếp sẽ loại bỏ chỉ thị khỏi văn bản hiển thị trước khi
kênh nhìn thấy chúng, kể cả khi chúng bị tách qua các khối liền kề.
...
**Cho phép chuyển nhà cung cấp trong khi vẫn giữ các núm điều chỉnh khác có thể cấu hình:**

```json5
{ messages: { tts: { modelOverrides: { enabled: true, allowProvider: true, allowSeed: false } } } }
```

## Lệnh bắt đầu bằng dấu gạch chéo
...
- `/tts persona <id>` ghi tùy chọn danh tính thoại cục bộ; `/tts persona off` xóa nó.
- `/tts latest` đọc phản hồi trợ lý mới nhất từ bản chép lời phiên hiện tại và gửi nó dưới dạng âm thanh một lần. Nó chỉ lưu một hash của phản hồi đó trên mục phiên để ngăn gửi giọng nói trùng lặp.
...
- `/status` hiển thị chế độ TTS đang hoạt động cùng nhà cung cấp, mô hình, giọng, và siêu dữ liệu endpoint tùy chỉnh đã được làm sạch khi TTS được bật.

## Tùy chọn theo người dùng

Lệnh bắt đầu bằng dấu gạch chéo ghi ghi đè cục bộ vào `prefsPath`. Mặc định là
...
| `persona`       | Ghi đè danh tính thoại cục bộ                |
| `maxLength`     | Ngưỡng tóm tắt (mặc định `1500` ký tự)       |
| `summarize`     | Công tắc tóm tắt (mặc định `true`)           |

Các tùy chọn này ghi đè cấu hình hiệu lực từ `messages.tts` cộng với khối
`agents.list[].tts` đang hoạt động cho máy chủ đó.

## Định dạng đầu ra (cố định)

Việc gửi giọng nói TTS được điều khiển bởi khả năng của kênh. Plugin kênh quảng bá
liệu TTS kiểu tin nhắn thoại nên yêu cầu nhà cung cấp dùng đích `voice-note` gốc hay
giữ tổng hợp `audio-file` bình thường và chỉ đánh dấu đầu ra tương thích để gửi dạng giọng nói.

- **Kênh hỗ trợ tin nhắn thoại**: phản hồi tin nhắn thoại ưu tiên Opus (`opus_48000_64` từ ElevenLabs, `opus` từ OpenAI).
  - 48kHz / 64kbps là mức cân bằng tốt cho tin nhắn thoại.
- **Feishu / WhatsApp**: khi phản hồi tin nhắn thoại được tạo dưới dạng MP3/WebM/WAV/M4A
  ...
  kết quả qua tải trọng Baileys `audio` với `ptt: true` và
  `audio/ogg; codecs=opus`. Nếu chuyển đổi thất bại, Feishu nhận tệp gốc
  dưới dạng tệp đính kèm; WhatsApp sẽ gửi thất bại thay vì đăng một tải trọng
  PTT không tương thích.
- **BlueBubbles**: giữ tổng hợp của nhà cung cấp trên đường dẫn tệp âm thanh bình thường; đầu ra MP3
  và CAF được đánh dấu để gửi dưới dạng ghi âm thoại iMessage.
- **Kênh khác**: MP3 (`mp3_44100_128` từ ElevenLabs, `mp3` từ OpenAI).
  - 44.1kHz / 128kbps là mức cân bằng mặc định cho độ rõ của giọng nói.
- **MiniMax**: MP3 (mô hình `speech-2.8-hd`, tốc độ lấy mẫu 32kHz) cho tệp đính kèm âm thanh bình thường. Với các đích tin nhắn thoại do kênh quảng bá, OpenClaw chuyển mã MP3 của MiniMax sang Opus 48kHz bằng `ffmpeg` trước khi gửi khi kênh quảng bá khả năng chuyển mã.
- **Xiaomi MiMo**: MP3 theo mặc định, hoặc WAV khi được cấu hình. Với các đích tin nhắn thoại do kênh quảng bá, OpenClaw chuyển mã đầu ra Xiaomi sang Opus 48kHz bằng `ffmpeg` trước khi gửi khi kênh quảng bá khả năng chuyển mã.
- **CLI cục bộ**: sử dụng `outputFormat` đã cấu hình. Đích tin nhắn thoại được
  chuyển đổi sang Ogg/Opus và đầu ra điện thoại được chuyển đổi sang PCM đơn kênh
  thô 16 kHz bằng `ffmpeg`.
- **Google Gemini**: TTS của Gemini API trả về PCM thô 24kHz. OpenClaw bọc tệp này dưới dạng WAV cho tệp đính kèm âm thanh, chuyển mã sang Opus 48kHz cho đích tin nhắn thoại, và trả về PCM trực tiếp cho Talk/điện thoại.
- **Gradium**: WAV cho tệp đính kèm âm thanh, Opus cho đích tin nhắn thoại, và `ulaw_8000` ở 8 kHz cho điện thoại.
- **Inworld**: MP3 cho tệp đính kèm âm thanh bình thường, `OGG_OPUS` gốc cho đích tin nhắn thoại, và `PCM` thô ở 22050 Hz cho Talk/điện thoại.
- **xAI**: MP3 theo mặc định; `responseFormat` có thể là `mp3`, `wav`, `pcm`, `mulaw`, hoặc `alaw`. OpenClaw sử dụng endpoint TTS REST theo lô của xAI và trả về một tệp đính kèm âm thanh hoàn chỉnh; WebSocket TTS phát trực tuyến của xAI không được đường dẫn nhà cung cấp này sử dụng. Đường dẫn này không hỗ trợ định dạng tin nhắn thoại Opus gốc.
- **Microsoft**: sử dụng `microsoft.outputFormat` (mặc định `audio-24khz-48kbitrate-mono-mp3`).
  - Transport đi kèm chấp nhận một `outputFormat`, nhưng không phải định dạng nào cũng có sẵn từ dịch vụ.
  - Giá trị định dạng đầu ra tuân theo định dạng đầu ra Microsoft Speech (bao gồm Ogg/WebM Opus).
...
- Tóm tắt các phản hồi dài khi bật tóm tắt, bằng cách sử dụng
...
- Trong `mode: "final"`, vẫn gửi TTS chỉ có âm thanh cho các phản hồi cuối cùng được phát trực tuyến
  sau khi luồng văn bản hoàn tất; media đã tạo đi qua cùng quá trình chuẩn hóa media
  của kênh như các tệp đính kèm phản hồi bình thường.

Nếu phản hồi vượt quá `maxLength` và tính năng tóm tắt bị tắt (hoặc không có khóa API cho
mô hình tóm tắt), âm thanh sẽ bị bỏ qua và phản hồi văn bản bình thường được gửi.
...
## Định dạng đầu ra theo kênh
  ...
  | Mục tiêu                              | Định dạng                                                                                                                                |
  | ------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------- |
  | Feishu / Matrix / Telegram / WhatsApp | Các phản hồi ghi âm ưu tiên **Opus** (`opus_48000_64` từ ElevenLabs, `opus` từ OpenAI). 48 kHz / 64 kbps cân bằng giữa độ rõ và dung lượng. |
  | Các kênh khác                         | **MP3** (`mp3_44100_128` từ ElevenLabs, `mp3` từ OpenAI). 44.1 kHz / 128 kbps là mặc định cho lời nói.                                  |
  ...
  - **Chuyển mã Feishu / WhatsApp:** Khi một phản hồi ghi âm đến dưới dạng MP3/WebM/WAV/M4A, Plugin kênh sẽ chuyển mã sang Ogg/Opus 48 kHz bằng `ffmpeg`. WhatsApp gửi qua Baileys với `ptt: true` và `audio/ogg; codecs=opus`. Nếu chuyển đổi thất bại: Feishu quay lại đính kèm tệp gốc; WhatsApp gửi thất bại thay vì đăng tải một tải trọng PTT không tương thích.
  - **MiniMax / Xiaomi MiMo:** MP3 mặc định (32 kHz cho MiniMax `speech-2.8-hd`); được chuyển mã sang Opus 48 kHz cho các mục tiêu ghi âm qua `ffmpeg`.
  - **CLI cục bộ:** Sử dụng `outputFormat` đã cấu hình. Các mục tiêu ghi âm được chuyển đổi sang Ogg/Opus và đầu ra điện thoại sang PCM mono 16 kHz thô.
  - **Google Gemini:** Trả về PCM 24 kHz thô. OpenClaw bọc thành WAV cho tệp đính kèm, chuyển mã sang Opus 48 kHz cho các mục tiêu ghi âm, trả về PCM trực tiếp cho Talk/điện thoại.
  - **Inworld:** Tệp đính kèm MP3, ghi âm `OGG_OPUS` gốc, `PCM` 22050 Hz thô cho Talk/điện thoại.
  - **xAI:** MP3 theo mặc định; `responseFormat` có thể là `mp3|wav|pcm|mulaw|alaw`. Sử dụng endpoint REST theo lô của xAI — TTS qua WebSocket phát trực tuyến **không** được dùng. Định dạng ghi âm Opus gốc **không** được hỗ trợ.
  - **Microsoft:** Sử dụng `microsoft.outputFormat` (mặc định `audio-24khz-48kbitrate-mono-mp3`). Telegram `sendVoice` chấp nhận OGG/MP3/M4A; dùng OpenAI/ElevenLabs nếu bạn cần tin nhắn thoại Opus được đảm bảo. Nếu định dạng Microsoft đã cấu hình thất bại, OpenClaw thử lại bằng MP3.
  ...
  <Accordion title="Top-level messages.tts.*">
    <ParamField path="auto" type='"off" | "always" | "inbound" | "tagged"'>
      Chế độ Auto-TTS. `inbound` chỉ gửi âm thanh sau một tin nhắn thoại đến; `tagged` chỉ gửi âm thanh khi phản hồi bao gồm chỉ thị `[[tts:...]]` hoặc một khối `[[tts:text]]`.
    </ParamField>
    ...
    <ParamField path="provider" type="string">
      ID nhà cung cấp giọng nói. Khi chưa đặt, OpenClaw dùng nhà cung cấp đã cấu hình đầu tiên theo thứ tự tự chọn của registry. `provider: "edge"` cũ được `openclaw doctor --fix` ghi lại thành `"microsoft"`.
    </ParamField>
    <ParamField path="persona" type="string">
      ID persona đang hoạt động từ `personas`. Được chuẩn hóa thành chữ thường.
    </ParamField>
    <ParamField path="personas.<id>" type="object">
      Danh tính lời nói ổn định. Các trường: `label`, `description`, `provider`, `fallbackPolicy`, `prompt`, `providers.<provider>`. Xem [Personas](#personas).
    </ParamField>
  ...
  <Accordion title="Azure Speech">
    ...
    <ParamField path="voiceNoteOutputFormat" type="string">Azure `X-Microsoft-OutputFormat` cho đầu ra ghi âm. Mặc định `ogg-24khz-16bit-mono-opus`.</ParamField>
  </Accordion>
  ...
  <Accordion title="ElevenLabs">
    ...
    <ParamField path="seed" type="number">Số nguyên `0..4294967295` để có tính xác định theo nỗ lực tốt nhất.</ParamField>
  ...
  <Accordion title="Google Gemini">
    ...
    <ParamField path="audioProfile" type="string">Prompt phong cách bằng ngôn ngữ tự nhiên được thêm vào trước văn bản được nói.</ParamField>
    <ParamField path="speakerName" type="string">Nhãn người nói tùy chọn được thêm vào trước văn bản được nói khi prompt của bạn dùng người nói được đặt tên.</ParamField>
  ...
  <Accordion title="Microsoft (no API key)">
    ...
    <ParamField path="edge.*" type="object" deprecated>Bí danh kế thừa. Chạy `openclaw doctor --fix` để ghi lại cấu hình đã lưu thành `providers.microsoft`.</ParamField>
  </Accordion>
  ...
  <Accordion title="MiniMax">
    ...
    <ParamField path="pitch" type="number">Số nguyên `-12..12`. Mặc định `0`. Giá trị thập phân bị cắt bỏ phần lẻ trước khi gửi yêu cầu.</ParamField>
  </Accordion>
  ...
  <Accordion title="OpenAI">
    ...
    <ParamField path="extraBody / extra_body" type="Record<string, unknown>">Các trường JSON bổ sung được gộp vào thân yêu cầu `/audio/speech` sau các trường TTS của OpenAI đã tạo. Dùng tùy chọn này cho các endpoint tương thích OpenAI như Kokoro cần các khóa riêng theo nhà cung cấp như `lang`; các khóa prototype không an toàn bị bỏ qua.</ParamField>
    <ParamField path="baseUrl" type="string">
      Ghi đè endpoint TTS của OpenAI. Thứ tự phân giải: cấu hình → `OPENAI_TTS_BASE_URL` → `https://api.openai.com/v1`. Các giá trị không mặc định được xem là endpoint TTS tương thích OpenAI, nên tên mô hình và giọng nói tùy chỉnh được chấp nhận.
    </ParamField>
  </Accordion>
  ...
  <Accordion title="OpenRouter">
    ...
    <ParamField path="baseUrl" type="string">Mặc định `https://openrouter.ai/api/v1`. `https://openrouter.ai/v1` kế thừa được chuẩn hóa.</ParamField>
  ...
  <Accordion title="Volcengine (BytePlus Seed Speech)">
    ...
    <ParamField path="appId / token / cluster" type="string" deprecated>Các trường Volcengine Speech Console kế thừa. Env: `VOLCENGINE_TTS_APPID`, `VOLCENGINE_TTS_TOKEN`, `VOLCENGINE_TTS_CLUSTER` (mặc định `volcano_tts`).</ParamField>
  </Accordion>
...
các client không hiển thị caption trên ghi chú thoại một cách nhất quán.

Công cụ chấp nhận các trường `channel` và `timeoutMs` tùy chọn; `timeoutMs` là
thời gian chờ yêu cầu nhà cung cấp theo từng lệnh gọi, tính bằng mili giây.

## Gateway RPC

| Phương thức       | Mục đích                                      |
| ----------------- | -------------------------------------------- |
| `tts.status`      | Đọc trạng thái TTS hiện tại và lần thử cuối. |
