...
<Info>
  ...
  các trang dùng nhiều JS hoặc đăng nhập, hãy dùng [Trình duyệt web](/vi/tools/browser). Để
  tải một URL cụ thể, hãy dùng [Web Fetch](/vi/tools/web-fetch).
</Info>
...
| [Kimi](/vi/tools/kimi-search)                | AI tổng hợp + trích dẫn; thất bại với các phương án dự phòng chat không có nền tảng | --                                               | `KIMI_API_KEY` / `MOONSHOT_API_KEY`                                                     |
...
8. **Exa** -- `EXA_API_KEY` hoặc `plugins.entries.exa.config.webSearch.apiKey`; `plugins.entries.exa.config.webSearch.baseUrl` tùy chọn ghi đè endpoint Exa (thứ tự 65)
...
<Note>
  ...
  khóa nhà cung cấp đã chọn -- các SecretRef không được chọn vẫn không hoạt động, vì vậy bạn có thể
  giữ nhiều nhà cung cấp được cấu hình mà không tốn chi phí phân giải cho
...
<Warning>
  ...
  SearXNG chỉ chấp nhận `http://` cho máy chủ mạng riêng tin cậy hoặc loopback;
...
<Note>
  ...
  và lấy thread. Với thống kê tương tác theo từng bài đăng như repost,
  trả lời, dấu trang hoặc lượt xem, nên dùng tra cứu nhắm đích cho URL bài đăng chính xác
  hoặc ID trạng thái. Các tìm kiếm từ khóa rộng có thể tìm đúng bài đăng nhưng trả về
  siêu dữ liệu theo bài đăng kém đầy đủ hơn. Một mẫu tốt là: tìm bài đăng trước, sau đó
...
- [Trình duyệt web](/vi/tools/browser) -- tự động hóa trình duyệt đầy đủ cho các trang nặng JS
