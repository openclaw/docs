...
x-i18n:
    generated_at: "2026-05-03T21:33:19Z"
    model: gpt-5.5
    provider: openai
...
Câu trả lời nhanh cùng phần khắc phục sự cố chuyên sâu hơn cho các thiết lập thực tế (phát triển cục bộ, VPS, nhiều agent, OAuth/API key, chuyển đổi dự phòng model). Để chẩn đoán runtime, xem [Khắc phục sự cố](/vi/gateway/troubleshooting). Để xem tham chiếu cấu hình đầy đủ, xem [Cấu hình](/vi/gateway/configuration).
...
1. **Trạng thái nhanh (kiểm tra đầu tiên)**
   ...
   Tóm tắt cục bộ nhanh: OS + cập nhật, khả năng truy cập gateway/dịch vụ, agent/session, cấu hình provider + vấn đề runtime (khi Gateway truy cập được).
...
5. **Theo dõi log mới nhất**
   ...
   Log tệp tách riêng với log dịch vụ; xem [Ghi log](/vi/logging) và [Khắc phục sự cố](/vi/gateway/troubleshooting).
...
Hỏi đáp lần chạy đầu tiên — cài đặt, onboard, tuyến xác thực, gói đăng ký, lỗi ban đầu —
...
<AccordionGroup>
  ...
  <Accordion title="Đề xuất giá trị">
    ...
    trợ lý mạnh mẽ trên **phần cứng của riêng bạn**, có thể truy cập từ các ứng dụng chat bạn đã dùng, với
    session có trạng thái, bộ nhớ, và công cụ - mà không giao quyền kiểm soát quy trình làm việc của bạn cho một
    SaaS được lưu trữ.

    Điểm nổi bật:

    - **Thiết bị của bạn, dữ liệu của bạn:** chạy Gateway ở bất cứ đâu bạn muốn (Mac, Linux, VPS) và giữ
      workspace + lịch sử session cục bộ.
    ...
    - **Không phụ thuộc model:** dùng Anthropic, OpenAI, MiniMax, OpenRouter, v.v., với định tuyến
      và chuyển đổi dự phòng theo từng agent.
    - **Tùy chọn chỉ cục bộ:** chạy model cục bộ để **toàn bộ dữ liệu có thể ở lại trên thiết bị của bạn** nếu bạn muốn.
    - **Định tuyến nhiều agent:** tách agent theo kênh, tài khoản, hoặc tác vụ, mỗi agent có
      workspace và mặc định riêng.
    - **Mã nguồn mở và dễ tùy biến:** kiểm tra, mở rộng, và tự lưu trữ mà không bị khóa vào nhà cung cấp.

    Tài liệu: [Gateway](/vi/gateway), [Kênh](/vi/channels), [Nhiều agent](/vi/concepts/multi-agent),
    [Bộ nhớ](/vi/concepts/memory).

  </Accordion>

  <Accordion title="Tôi vừa thiết lập xong - nên làm gì trước?">
    Dự án khởi đầu phù hợp:
    ...
    - Kết nối Gmail và tự động hóa tóm tắt hoặc nhắc theo dõi.

    Nó có thể xử lý tác vụ lớn, nhưng hoạt động tốt nhất khi bạn chia chúng thành các giai đoạn và
    dùng sub-agent cho công việc song song.
  ...
  <Accordion title="Năm trường hợp sử dụng hằng ngày hàng đầu cho OpenClaw là gì?">
    ...
    - **Nhắc việc và theo dõi:** lời nhắc và checklist do Cron hoặc Heartbeat điều khiển.
    - **Tự động hóa trình duyệt:** điền biểu mẫu, thu thập dữ liệu, và lặp lại tác vụ web.
    - **Phối hợp giữa các thiết bị:** gửi tác vụ từ điện thoại, để Gateway chạy trên server, và nhận kết quả lại trong chat.

  </Accordion>

  <Accordion title="OpenClaw có thể hỗ trợ tạo lead, outreach, quảng cáo, và blog cho SaaS không?">
    Có, cho **nghiên cứu, đánh giá chất lượng, và soạn thảo**. Nó có thể quét site, xây dựng danh sách rút gọn,
    tóm tắt prospect, và viết bản nháp outreach hoặc nội dung quảng cáo.

    Với **outreach hoặc chạy quảng cáo**, hãy để con người tham gia kiểm soát. Tránh spam, tuân thủ luật địa phương và
  ...
  <Accordion title="Ưu điểm so với Claude Code cho phát triển web là gì?">
    ...
    - **Bộ nhớ + workspace liên tục** qua các session
...
<AccordionGroup>
  <Accordion title="Làm thế nào để tùy chỉnh skills mà không làm repo bị dirty?">
    Dùng override được quản lý thay vì chỉnh sửa bản sao trong repo. Đặt thay đổi của bạn trong `~/.openclaw/skills/<name>/SKILL.md` (hoặc thêm thư mục qua `skills.load.extraDirs` trong `~/.openclaw/openclaw.json`). Thứ tự ưu tiên là `<workspace>/skills` → `<workspace>/.agents/skills` → `~/.agents/skills` → `~/.openclaw/skills` → đi kèm → `skills.load.extraDirs`, vì vậy override được quản lý vẫn thắng skills đi kèm mà không chạm vào git. Nếu bạn cần skill được cài toàn cục nhưng chỉ hiển thị với một số agent, giữ bản sao dùng chung trong `~/.openclaw/skills` và kiểm soát khả năng hiển thị bằng `agents.defaults.skills` và `agents.list[].skills`. Chỉ các chỉnh sửa xứng đáng upstream mới nên nằm trong repo và được gửi dưới dạng PR.
  </Accordion>

  <Accordion title="Tôi có thể tải skills từ thư mục tùy chỉnh không?">
    Có. Thêm thư mục bổ sung qua `skills.load.extraDirs` trong `~/.openclaw/openclaw.json` (ưu tiên thấp nhất). Thứ tự ưu tiên mặc định là `<workspace>/skills` → `<workspace>/.agents/skills` → `~/.agents/skills` → `~/.openclaw/skills` → đi kèm → `skills.load.extraDirs`. `clawhub` cài vào `./skills` theo mặc định, OpenClaw xem đây là `<workspace>/skills` trong session tiếp theo. Nếu skill chỉ nên hiển thị với một số agent nhất định, kết hợp điều đó với `agents.defaults.skills` hoặc `agents.list[].skills`.
  </Accordion>

  <Accordion title="Làm thế nào để dùng các model khác nhau cho các tác vụ khác nhau?">
    Hiện nay các mẫu được hỗ trợ là:

    - **Cron job**: job cô lập có thể đặt override `model` cho từng job.
    - **Sub-agent**: định tuyến tác vụ đến các agent riêng với model mặc định khác nhau.
    - **Chuyển theo nhu cầu**: dùng `/model` để chuyển model của session hiện tại bất cứ lúc nào.

    Xem [Cron job](/vi/automation/cron-jobs), [Định tuyến nhiều agent](/vi/concepts/multi-agent), và [Lệnh slash](/vi/tools/slash-commands).

  </Accordion>

  <Accordion title="Bot bị đơ khi làm việc nặng. Làm thế nào để offload việc đó?">
    Dùng **sub-agent** cho tác vụ dài hoặc song song. Sub-agent chạy trong session riêng,
    ...
    Mẹo token: tác vụ dài và sub-agent đều tiêu thụ token. Nếu bạn lo về chi phí, đặt
    model rẻ hơn cho sub-agent qua `agents.defaults.subagents.model`.

    Tài liệu: [Sub-agent](/vi/tools/subagents), [Tác vụ nền](/vi/automation/tasks).

  </Accordion>

  <Accordion title="Session subagent gắn với thread hoạt động như thế nào trên Discord?">
    Dùng binding thread. Bạn có thể bind một thread Discord với subagent hoặc mục tiêu session để tin nhắn tiếp theo trong thread đó vẫn ở trên session đã bind.
    ...
    - Tự động bind khi spawn: `channels.discord.threadBindings.spawnSessions` mặc định là `true`; đặt thành `false` để tắt spawn session gắn với thread.

    Tài liệu: [Sub-agent](/vi/tools/subagents), [Discord](/vi/channels/discord), [Tham chiếu cấu hình](/vi/gateway/configuration-reference), [Lệnh slash](/vi/tools/slash-commands).

  </Accordion>

  <Accordion title="Một subagent đã hoàn tất, nhưng cập nhật hoàn tất được gửi sai nơi hoặc không bao giờ được đăng. Tôi nên kiểm tra gì?">
    Kiểm tra tuyến requester đã được phân giải trước:

    - Việc gửi subagent ở chế độ hoàn tất ưu tiên mọi thread đã bind hoặc tuyến hội thoại khi có.
    - Nếu origin hoàn tất chỉ mang một kênh, OpenClaw fallback về tuyến đã lưu của session requester (`lastChannel` / `lastTo` / `lastAccountId`) để việc gửi trực tiếp vẫn có thể thành công.
    - Nếu không có tuyến đã bind lẫn tuyến đã lưu có thể dùng, việc gửi trực tiếp có thể thất bại và kết quả fallback sang gửi session theo hàng đợi thay vì đăng ngay vào chat.
    - Mục tiêu không hợp lệ hoặc đã cũ vẫn có thể buộc fallback hàng đợi hoặc lỗi gửi cuối cùng.
    - Nếu câu trả lời assistant hiển thị cuối cùng của child là token im lặng chính xác `NO_REPLY` / `no_reply`, hoặc chính xác `ANNOUNCE_SKIP`, OpenClaw cố ý chặn thông báo thay vì đăng tiến trình cũ trước đó.
    - Nếu child timeout sau khi chỉ có tool call, thông báo có thể rút gọn điều đó thành một bản tóm tắt tiến trình một phần ngắn thay vì phát lại output công cụ thô.
    ...
    Tài liệu: [Sub-agent](/vi/tools/subagents), [Tác vụ nền](/vi/automation/tasks), [Công cụ session](/vi/concepts/session-tool).
  ...
  <Accordion title="Cron hoặc lời nhắc không chạy. Tôi nên kiểm tra gì?">
    ...
    Tài liệu: [Cron job](/vi/automation/cron-jobs), [Tự động hóa & Tác vụ](/vi/automation).
  ...
  <Accordion title="Cron đã chạy, nhưng không có gì được gửi đến kênh. Tại sao?">
    ...
    - Kết quả cô lập im lặng (chỉ có `NO_REPLY` / `no_reply`) được xem là cố ý không thể gửi, nên runner cũng chặn việc gửi dự phòng đã xếp hàng.
    ...
    cho phần văn bản cuối cùng mà agent chưa tự gửi.
  ...
  <Accordion title="Tại sao một lần chạy cron cô lập lại đổi mô hình hoặc thử lại một lần?">
    ...
    Cron cô lập có thể lưu lại một lần chuyển giao mô hình lúc chạy và thử lại khi lần chạy
    đang hoạt động ném `LiveSessionModelSwitchError`. Lần thử lại giữ nguyên provider/mô hình
    đã chuyển, và nếu lần chuyển đó mang theo một ghi đè hồ sơ xác thực mới, cron
    ...
    cron sẽ hủy thay vì lặp vô hạn.
  ...
  <Accordion title="Làm cách nào để cài đặt Skills trên Linux?">
    Dùng các lệnh `openclaw skills` native hoặc đặt skills vào workspace của bạn. Giao diện Skills trên macOS không khả dụng trên Linux.
    ...
    `openclaw skills install` native ghi vào thư mục `skills/`
    ...
    `agents.list[].skills` nếu bạn muốn thu hẹp những agent có thể thấy nó.
  ...
  <Accordion title="OpenClaw có thể chạy tác vụ theo lịch hoặc liên tục trong nền không?">
    ...
    - **Tác vụ Cron** cho các tác vụ đã lập lịch hoặc lặp lại (tồn tại qua các lần khởi động lại).
  ...
  <Accordion title="Tôi có thể chạy skills chỉ dành cho Apple macOS từ Linux không?">
    Không trực tiếp. skills trên macOS được chặn theo `metadata.openclaw.os` cùng các binary bắt buộc, và skills chỉ xuất hiện trong system prompt khi chúng đủ điều kiện trên **máy chủ Gateway**. Trên Linux, skills chỉ dành cho `darwin` (như `apple-notes`, `apple-reminders`, `things-mac`) sẽ không tải trừ khi bạn ghi đè cơ chế chặn.
  ...
  <Accordion title="Bạn có tích hợp Notion hoặc HeyGen không?">
    ...
    Nếu bạn muốn tích hợp native, hãy mở một yêu cầu tính năng hoặc xây dựng một skill
    nhắm đến các API đó.
    ...
    Các cài đặt native nằm trong thư mục `skills/` của workspace đang hoạt động. Với skills dùng chung giữa các agent, đặt chúng trong `~/.openclaw/skills/<name>/SKILL.md`. Nếu chỉ một số agent nên thấy một cài đặt dùng chung, cấu hình `agents.defaults.skills` hoặc `agents.list[].skills`. Một số skills cần binary được cài qua Homebrew; trên Linux điều đó có nghĩa là Linuxbrew (xem mục Hỏi đáp Homebrew Linux ở trên). Xem [Skills](/vi/tools/skills), [Cấu hình Skills](/vi/tools/skills-config), và [ClawHub](/vi/tools/clawhub).

  </Accordion>

  <Accordion title="Làm cách nào để dùng Chrome hiện có đã đăng nhập với OpenClaw?">
    ...
    Đường dẫn này có thể dùng trình duyệt máy cục bộ hoặc một Node trình duyệt đã kết nối. Nếu Gateway chạy ở nơi khác, hãy chạy máy chủ Node trên máy có trình duyệt hoặc dùng CDP từ xa.

    Giới hạn hiện tại của `existing-session` / `user`:

    - các hành động dựa trên ref, không dựa trên CSS-selector
...
<AccordionGroup>
  <Accordion title="Có tài liệu sandboxing riêng không?">
    Có. Xem [Sandboxing](/vi/gateway/sandboxing). Với thiết lập dành riêng cho Docker (toàn bộ Gateway trong Docker hoặc image sandbox), xem [Docker](/vi/install/docker).
  </Accordion>
  ...
  <Accordion title="Docker có vẻ bị giới hạn - làm cách nào để bật đầy đủ tính năng?">
    ...
    - Duy trì `/home/node` với `OPENCLAW_HOME_VOLUME` để cache tồn tại.
    - Đưa các phụ thuộc hệ thống vào image bằng `OPENCLAW_DOCKER_APT_PACKAGES`.
  ...
  <Accordion title="Tôi có thể giữ DM riêng tư nhưng làm cho nhóm công khai/sandboxed bằng một agent không?">
    ...
    Hướng dẫn thiết lập + cấu hình mẫu: [Nhóm: DM cá nhân + nhóm công khai](/vi/channels/groups#pattern-personal-dms-public-groups-single-agent)
  ...
  <Accordion title="Làm cách nào để bind một thư mục máy chủ vào sandbox?">
    Đặt `agents.defaults.sandbox.docker.binds` thành `["host:path:mode"]` (ví dụ: `"/home/user/src:/src:ro"`). Bind toàn cục + theo agent sẽ được hợp nhất; bind theo agent bị bỏ qua khi `scope: "shared"`. Dùng `:ro` cho bất cứ thứ gì nhạy cảm và nhớ rằng bind bỏ qua các rào chắn hệ thống tệp của sandbox.

    OpenClaw xác thực nguồn bind dựa trên cả đường dẫn đã chuẩn hóa và đường dẫn canonical được phân giải qua ancestor sâu nhất đang tồn tại. Điều đó nghĩa là các lối thoát qua symlink-parent vẫn fail closed ngay cả khi segment đường dẫn cuối chưa tồn tại, và kiểm tra allowed-root vẫn áp dụng sau khi phân giải symlink.

    Xem [Sandboxing](/vi/gateway/sandboxing#custom-bind-mounts) và [Sandbox so với Tool Policy so với Elevated](/vi/gateway/sandbox-vs-tool-policy-vs-elevated#bind-mounts-security-quick-check) để biết ví dụ và ghi chú an toàn.
  ...
  <Accordion title="Bộ nhớ hoạt động như thế nào?">
    ...
    OpenClaw cũng chạy một **lần flush bộ nhớ trước Compaction im lặng** để nhắc mô hình
  ...
  <Accordion title="Bộ nhớ cứ quên mọi thứ. Làm cách nào để nó ghi nhớ lâu?">
    ...
    nó sẽ biết cần làm gì. Nếu nó vẫn tiếp tục quên, hãy xác minh Gateway đang dùng cùng
  ...
  <Accordion title="Bộ nhớ có tồn tại mãi mãi không? Giới hạn là gì?">
    Các tệp bộ nhớ nằm trên ổ đĩa và tồn tại cho đến khi bạn xóa chúng. Giới hạn là
    dung lượng lưu trữ của bạn, không phải mô hình. **Ngữ cảnh phiên** vẫn bị giới hạn bởi
    cửa sổ ngữ cảnh của mô hình, nên các cuộc trò chuyện dài có thể bị compact hoặc cắt ngắn. Đó là lý do
  ...
  <Accordion title="Tìm kiếm bộ nhớ ngữ nghĩa có yêu cầu khóa API OpenAI không?">
    ...
    **không** cấp quyền truy cập embeddings, nên **đăng nhập bằng Codex (OAuth hoặc
    ...
    Nếu bạn không đặt rõ nhà cung cấp, OpenClaw sẽ tự động chọn một nhà cung cấp khi
    có thể phân giải khóa API (hồ sơ xác thực, `models.providers.*.apiKey`, hoặc biến môi trường).
    Nó ưu tiên OpenAI nếu phân giải được khóa OpenAI, nếu không thì Gemini nếu phân giải
    được khóa Gemini, sau đó Voyage, rồi Mistral. Nếu không có khóa từ xa nào, tìm kiếm
    bộ nhớ sẽ vẫn bị tắt cho đến khi bạn cấu hình nó. Nếu bạn đã cấu hình và có sẵn
...
<AccordionGroup>
  <Accordion title="Tất cả dữ liệu dùng với OpenClaw có được lưu cục bộ không?">
    ...
    - **Mặc định là cục bộ:** phiên, tệp bộ nhớ, cấu hình và không gian làm việc nằm trên máy chủ Gateway
      (`~/.openclaw` + thư mục không gian làm việc của bạn).
    - **Bắt buộc phải từ xa:** thông điệp bạn gửi đến nhà cung cấp mô hình (Anthropic/OpenAI/v.v.) đi tới
    ...
    Liên quan: [Không gian làm việc của tác tử](/vi/concepts/agent-workspace), [Bộ nhớ](/vi/concepts/memory).
  ...
  <Accordion title="OpenClaw lưu dữ liệu ở đâu?">
    ...
    | `$OPENCLAW_STATE_DIR/agents/<agentId>/agent/auth.json`          | Tệp tương thích cũ (các mục `api_key` tĩnh đã được lọc bỏ)         |
    | `$OPENCLAW_STATE_DIR/credentials/`                              | Trạng thái nhà cung cấp (ví dụ `whatsapp/<accountId>/creds.json`) |
    | `$OPENCLAW_STATE_DIR/agents/`                                   | Trạng thái theo từng tác tử (agentDir + phiên)                     |
    | `$OPENCLAW_STATE_DIR/agents/<agentId>/sessions/`                | Lịch sử hội thoại và trạng thái (theo tác tử)                      |
    | `$OPENCLAW_STATE_DIR/agents/<agentId>/sessions/sessions.json`   | Siêu dữ liệu phiên (theo tác tử)                                   |

    Đường dẫn một tác tử cũ: `~/.openclaw/agent/*` (được di chuyển bởi `openclaw doctor`).

    **Không gian làm việc** của bạn (AGENTS.md, tệp bộ nhớ, skills, v.v.) là riêng biệt và được cấu hình qua `agents.defaults.workspace` (mặc định: `~/.openclaw/workspace`).

  </Accordion>

  <Accordion title="AGENTS.md / SOUL.md / USER.md / MEMORY.md nên nằm ở đâu?">
    Các tệp này nằm trong **không gian làm việc của tác tử**, không phải `~/.openclaw`.

    - **Không gian làm việc (theo tác tử)**: `AGENTS.md`, `SOUL.md`, `IDENTITY.md`, `USER.md`,
    ...
    Xem [Không gian làm việc của tác tử](/vi/concepts/agent-workspace) và [Bộ nhớ](/vi/concepts/memory).

  </Accordion>

  <Accordion title="Chiến lược sao lưu được khuyến nghị">
    Đặt **không gian làm việc của tác tử** của bạn trong một kho git **riêng tư** và sao lưu nó ở một nơi
    riêng tư (ví dụ GitHub riêng tư). Việc này ghi lại bộ nhớ + các tệp AGENTS/SOUL/USER,
    ...
    **Không** commit bất cứ thứ gì dưới `~/.openclaw` (thông tin đăng nhập, phiên, token, hoặc payload bí mật đã mã hóa).
    Nếu bạn cần khôi phục đầy đủ, hãy sao lưu riêng cả không gian làm việc và thư mục trạng thái
    (xem câu hỏi về di chuyển ở trên).

    Tài liệu: [Không gian làm việc của tác tử](/vi/concepts/agent-workspace).
  ...
  <Accordion title="Tác tử có thể làm việc bên ngoài không gian làm việc không?">
    Có. Không gian làm việc là **cwd mặc định** và điểm neo bộ nhớ, không phải sandbox cứng.
    Đường dẫn tương đối được phân giải bên trong không gian làm việc, nhưng đường dẫn tuyệt đối có thể truy cập các
    vị trí khác trên máy chủ trừ khi sandboxing được bật. Nếu bạn cần cô lập, hãy dùng
    [`agents.defaults.sandbox`](/vi/gateway/sandboxing) hoặc cài đặt sandbox theo từng tác tử. Nếu bạn
    muốn một repo là thư mục làm việc mặc định, hãy trỏ `workspace` của tác tử đó
    tới gốc repo. Repo OpenClaw chỉ là mã nguồn; hãy giữ
    không gian làm việc riêng biệt trừ khi bạn cố ý muốn tác tử làm việc bên trong nó.
...
<AccordionGroup>
  <Accordion title="Cấu hình dùng định dạng nào? Nó nằm ở đâu?">
  ...
  <Accordion title='Tôi đã đặt gateway.bind: "lan" (hoặc "tailnet") và giờ không có gì lắng nghe / UI báo chưa được ủy quyền'>
    Các bind không phải loopback **yêu cầu đường dẫn xác thực gateway hợp lệ**. Trên thực tế, điều đó nghĩa là:
    ...
    - Với xác thực bằng mật khẩu, hãy đặt `gateway.auth.mode: "password"` cùng với `gateway.auth.password` (hoặc `OPENCLAW_GATEWAY_PASSWORD`).
    - Nếu `gateway.auth.token` / `gateway.auth.password` được cấu hình rõ qua SecretRef và không phân giải được, quá trình phân giải sẽ đóng an toàn (không bị dự phòng từ xa che khuất).
    - Thiết lập Control UI dùng bí mật chia sẻ xác thực qua `connect.params.auth.token` hoặc `connect.params.auth.password` (được lưu trong cài đặt app/UI). Các chế độ mang danh tính như Tailscale Serve hoặc `trusted-proxy` dùng header yêu cầu thay thế. Tránh đặt bí mật dùng chung trong URL.
  ...
  <Accordion title="Tại sao bây giờ tôi cần token trên localhost?">
    OpenClaw thực thi xác thực gateway theo mặc định, bao gồm loopback. Trong đường dẫn mặc định thông thường, điều đó nghĩa là xác thực bằng token: nếu không có đường dẫn xác thực rõ ràng nào được cấu hình, lúc khởi động gateway sẽ phân giải sang chế độ token và tự động tạo một token, lưu vào `gateway.auth.token`, nên **các client WS cục bộ phải xác thực**. Việc này chặn các tiến trình cục bộ khác gọi Gateway.

    Nếu bạn muốn một đường dẫn xác thực khác, bạn có thể chọn rõ chế độ mật khẩu (hoặc, với reverse proxy nhận biết danh tính, `trusted-proxy`). Nếu bạn **thực sự** muốn mở loopback, hãy đặt rõ `gateway.auth.mode: "none"` trong cấu hình của bạn. Doctor có thể tạo token cho bạn bất cứ lúc nào: `openclaw doctor --generate-gateway-token`.
  ...
  <Accordion title="Làm thế nào để tắt các khẩu hiệu CLI hài hước?">
    ...
    - Nếu bạn không muốn có banner nào, đặt biến môi trường `OPENCLAW_HIDE_BANNER=1`.
  ...
  <Accordion title="Làm thế nào để bật tìm kiếm web (và lấy nội dung web)?">
    ...
    **Khuyến nghị:** chạy `openclaw configure --section web` và chọn một nhà cung cấp.
    ...
    Cấu hình tìm kiếm web theo từng nhà cung cấp hiện nằm dưới `plugins.entries.<plugin>.config.webSearch.*`.
    ...
    Cấu hình dự phòng truy xuất web của Firecrawl nằm dưới `plugins.entries.firecrawl.config.webFetch.*`.
    ...
    - Nếu bỏ qua `tools.web.fetch.provider`, OpenClaw sẽ tự động phát hiện nhà cung cấp dự phòng truy xuất đầu tiên đã sẵn sàng từ các thông tin xác thực có sẵn. Hiện tại nhà cung cấp được đóng gói là Firecrawl.
  ...
  <Accordion title="config.apply đã xóa sạch cấu hình của tôi. Làm thế nào để khôi phục và tránh việc này?">
    ...
    - `openclaw doctor --fix` sở hữu việc sửa chữa và có thể khôi phục cấu hình tốt đã biết gần nhất trong khi lưu tệp bị từ chối dưới dạng `openclaw.json.clobbered.*`.
    ...
    - Nếu việc này ngoài dự kiến, hãy báo lỗi và đính kèm cấu hình bạn biết gần nhất hoặc bất kỳ bản sao lưu nào.
    - Một tác nhân lập trình cục bộ thường có thể dựng lại cấu hình hoạt động được từ nhật ký hoặc lịch sử.

    Tránh việc này:

    - Dùng `openclaw config set` cho các thay đổi nhỏ.
    - Dùng `openclaw configure` để chỉnh sửa tương tác.
    - Dùng `config.schema.lookup` trước khi bạn không chắc về đường dẫn chính xác hoặc hình dạng trường; nó trả về một nút schema nông cộng với tóm tắt con trực tiếp để đi sâu.
    - Dùng `config.patch` cho các chỉnh sửa RPC một phần; chỉ dùng `config.apply` để thay thế toàn bộ cấu hình.
    - Nếu bạn đang dùng công cụ `gateway` chỉ dành cho chủ sở hữu từ một lần chạy tác nhân, nó vẫn sẽ từ chối ghi vào `tools.exec.ask` / `tools.exec.security` (bao gồm các bí danh cũ `tools.bash.*` được chuẩn hóa về cùng các đường dẫn exec được bảo vệ).
  ...
  <Accordion title="Làm thế nào để chạy Gateway trung tâm với các worker chuyên biệt trên nhiều thiết bị?">
    ...
    - **Tác nhân phụ:** tạo công việc nền từ tác nhân chính khi bạn muốn chạy song song.
  ...
  <Accordion title="Trình duyệt OpenClaw có thể chạy headless không?">
    ...
    Mặc định là `false` (headful). Headless dễ kích hoạt các kiểm tra chống bot hơn trên một số trang. Xem [Trình duyệt](/vi/tools/browser).

    Headless dùng **cùng công cụ Chromium** và hoạt động với hầu hết tự động hóa (biểu mẫu, nhấp chuột, thu thập dữ liệu, đăng nhập). Các khác biệt chính:

    - Không có cửa sổ trình duyệt hiển thị (dùng ảnh chụp màn hình nếu bạn cần hình ảnh).
    - Một số trang nghiêm ngặt hơn về tự động hóa ở chế độ headless (CAPTCHA, chống bot).
...
<AccordionGroup>
  <Accordion title="Lệnh được lan truyền giữa Telegram, gateway và node như thế nào?">
  ...
  <Accordion title="Tác nhân của tôi có thể truy cập máy tính của tôi như thế nào nếu Gateway được lưu trữ từ xa?">
    ...
    gọi công cụ `node.*` (màn hình, camera, hệ thống) trên máy cục bộ của bạn qua Gateway WebSocket.

    Thiết lập điển hình:

    1. Chạy Gateway trên máy chủ luôn bật (VPS/máy chủ tại nhà).
    2. Đặt máy chủ Gateway + máy tính của bạn trong cùng tailnet.
    ...
    Nhắc nhở bảo mật: ghép đôi node macOS cho phép `system.run` trên máy đó. Chỉ
  ...
  <Accordion title="Hai phiên bản OpenClaw có thể nói chuyện với nhau không (cục bộ + VPS)?">
    Có. Không có cầu nối "bot-to-bot" tích hợp sẵn, nhưng bạn có thể kết nối nó theo vài
    cách đáng tin cậy:
    ...
    **Cầu nối CLI (chung):** chạy một script gọi Gateway còn lại bằng
    ...
    lắng nghe. Nếu một bot ở trên VPS từ xa, trỏ CLI của bạn tới Gateway từ xa đó
    ...
    Mẹo: thêm một rào chắn để hai bot không lặp vô tận (chỉ khi được nhắc tới, danh sách
  ...
  <Accordion title="Tôi có cần các VPS riêng cho nhiều tác nhân không?">
    Không. Một Gateway có thể lưu trữ nhiều tác nhân, mỗi tác nhân có không gian làm việc, mặc định mô hình,
    và định tuyến riêng. Đó là thiết lập bình thường, rẻ hơn và đơn giản hơn nhiều so với chạy
    một VPS cho mỗi tác nhân.

    Chỉ dùng các VPS riêng khi bạn cần cách ly cứng (ranh giới bảo mật) hoặc các
  ...
  <Accordion title="Có lợi ích gì khi dùng node trên laptop cá nhân thay vì SSH từ VPS không?">
    ...
    nhẹ (một VPS nhỏ hoặc hộp cỡ Raspberry Pi là đủ; 4 GB RAM là dư), nên một
    thiết lập phổ biến là một máy chủ luôn bật cộng với laptop của bạn làm node.
    ...
    SSH ổn cho truy cập shell tùy dịp, nhưng node đơn giản hơn cho các quy trình tác nhân đang diễn ra và
  ...
  <Accordion title="Node có chạy dịch vụ gateway không?">
    ...
    tới gateway (node iOS/Android, hoặc "chế độ node" macOS trong ứng dụng trên thanh menu). Với máy chủ node
    headless và điều khiển CLI, xem [CLI máy chủ Node](/vi/cli/node).

    Cần khởi động lại hoàn toàn cho các thay đổi `gateway`, `discovery`, và `canvasHost`.
  ...
  <Accordion title="Có cách API / RPC để áp dụng cấu hình không?">
    ...
    - `config.patch`: cập nhật một phần an toàn (được ưu tiên cho hầu hết chỉnh sửa RPC); tải lại nóng khi có thể và khởi động lại khi cần
  ...
  <Accordion title="Cấu hình tối thiểu hợp lý cho lần cài đặt đầu tiên">
    ...
    Cấu hình này đặt workspace của bạn và giới hạn những người có thể kích hoạt bot.
  ...
  <Accordion title="Làm cách nào để thiết lập Tailscale trên VPS và kết nối từ Mac của tôi?">
    ...
    Nếu bạn muốn có UI Điều khiển mà không cần SSH, hãy dùng Tailscale Serve trên VPS:

    ```bash
    openclaw gateway --tailscale serve
    ```

    Cách này giữ gateway ràng buộc với loopback và phơi bày HTTPS qua Tailscale. Xem [Tailscale](/vi/gateway/tailscale).

  </Accordion>

  <Accordion title="Làm cách nào để kết nối một Node Mac với Gateway từ xa (Tailscale Serve)?">
    Serve phơi bày **UI Điều khiển Gateway + WS**. Các Node kết nối qua cùng điểm cuối Gateway WS.

    Thiết lập khuyến nghị:

    1. **Đảm bảo VPS + Mac nằm trên cùng tailnet**.
    2. **Dùng ứng dụng macOS ở chế độ Từ xa** (đích SSH có thể là tên máy chủ tailnet).
       Ứng dụng sẽ tạo đường hầm cho cổng Gateway và kết nối như một Node.
  ...
  <Accordion title="Tôi nên cài đặt trên laptop thứ hai hay chỉ thêm một Node?">
    ...
    **Node**. Cách này giữ một Gateway duy nhất và tránh cấu hình trùng lặp. Các công cụ Node cục bộ
    ...
    Chỉ cài đặt Gateway thứ hai khi bạn cần **cách ly chặt chẽ** hoặc hai bot hoàn toàn riêng biệt.
...
<AccordionGroup>
  ...
  <Accordion title="Tôi khởi động Gateway qua dịch vụ và các biến môi trường của tôi biến mất. Bây giờ làm gì?">
  ...
  <Accordion title='Tôi đã đặt COPILOT_GITHUB_TOKEN, nhưng trạng thái mô hình hiển thị "Shell env: off." Tại sao?'>
    ...
    **không** có nghĩa là biến môi trường của bạn bị thiếu - nó chỉ có nghĩa là OpenClaw sẽ không tự động tải
    ...
    Nếu Gateway chạy như một dịch vụ (launchd/systemd), nó sẽ không kế thừa
    môi trường shell của bạn. Sửa bằng một trong các cách sau:
...
<AccordionGroup>
  ...
  <Accordion title="Phiên có tự động đặt lại nếu tôi không bao giờ gửi /new không?">
    ...
    Đặt thành một giá trị dương để bật hết hạn khi nhàn rỗi. Khi được bật, tin nhắn **tiếp theo**
    sau khoảng thời gian nhàn rỗi sẽ bắt đầu một ID phiên mới cho khóa trò chuyện đó.
  ...
  <Accordion title="Có cách nào tạo một đội các phiên bản OpenClaw (một CEO và nhiều agent) không?">
    ...
    và nhiều agent xử lý với workspace và mô hình riêng.
  ...
  <Accordion title="Tại sao ngữ cảnh bị cắt giữa tác vụ? Làm cách nào để ngăn điều đó?">
    Ngữ cảnh phiên bị giới hạn bởi cửa sổ mô hình. Cuộc trò chuyện dài, đầu ra công cụ lớn hoặc nhiều
  ...
  <Accordion title="Làm cách nào để đặt lại hoàn toàn OpenClaw nhưng vẫn giữ cài đặt?">
    ...
    Đặt lại đầy đủ không tương tác:
    ...
    - Nếu bạn dùng hồ sơ (`--profile` / `OPENCLAW_PROFILE`), hãy đặt lại từng thư mục trạng thái (mặc định là `~/.openclaw-<profile>`).
    - Đặt lại dev: `openclaw gateway --dev --reset` (chỉ dành cho dev; xóa cấu hình dev + thông tin xác thực + phiên + workspace).

  </Accordion>

  <Accordion title='Tôi gặp lỗi "context too large" - làm cách nào để đặt lại hoặc compact?'>
    ...
    - Bật hoặc tinh chỉnh **cắt tỉa phiên** (`agents.defaults.contextPruning`) để rút gọn đầu ra công cụ cũ.
  ...
  <Accordion title='Tại sao tôi thấy "LLM request rejected: messages.content.tool_use.input field required"?'>
    Đây là lỗi xác thực của nhà cung cấp: mô hình phát ra một khối `tool_use` mà không có
  ...
  <Accordion title="Tại sao tôi nhận được tin nhắn Heartbeat mỗi 30 phút?">
    Heartbeat chạy mặc định mỗi **30m** (**1h** khi dùng xác thực OAuth). Tinh chỉnh hoặc tắt chúng:
    ...
    Nếu `HEARTBEAT.md` tồn tại nhưng thực tế rỗng (chỉ có dòng trống và tiêu đề markdown
  ...
  <Accordion title="Tại sao OpenClaw không trả lời trong nhóm?">
    ...
    - Gating bằng mention đang bật (mặc định). Bạn phải @mention bot (hoặc khớp `mentionPatterns`).
    - Bạn đã cấu hình `channels.whatsapp.groups` mà không có `"*"` và nhóm chưa nằm trong allowlist.
  ...
  <Accordion title="Nhóm/luồng có chia sẻ ngữ cảnh với DM không?">
    Trò chuyện trực tiếp mặc định gộp vào phiên chính. Nhóm/kênh có khóa phiên riêng, và chủ đề Telegram / luồng Discord là các phiên riêng. Xem [Nhóm](/vi/channels/groups) và [Tin nhắn nhóm](/vi/channels/group-messages).
  </Accordion>

  <Accordion title="Tôi có thể tạo bao nhiêu workspace và agent?">
    Không có giới hạn cứng. Hàng chục (thậm chí hàng trăm) vẫn ổn, nhưng hãy chú ý:

    - **Dung lượng đĩa tăng:** phiên + transcript nằm dưới `~/.openclaw/agents/<agentId>/sessions/`.
    - **Chi phí token:** nhiều agent hơn nghĩa là mức dùng mô hình đồng thời cao hơn.
    - **Chi phí vận hành:** hồ sơ xác thực theo từng agent, workspace và định tuyến kênh.
...
<AccordionGroup>
  ...
  <Accordion title='"another gateway instance is already listening" nghĩa là gì?'>
    OpenClaw áp đặt khóa thời gian chạy bằng cách bind trình lắng nghe WebSocket ngay khi khởi động (mặc định `ws://127.0.0.1:18789`). Nếu bind thất bại với `EADDRINUSE`, nó ném `GatewayLockError` cho biết một phiên bản khác đang lắng nghe.
  ...
  <Accordion title="Làm thế nào để chạy OpenClaw ở chế độ từ xa (client kết nối tới Gateway ở nơi khác)?">
    Đặt `gateway.mode: "remote"` và trỏ tới URL WebSocket từ xa, tùy chọn kèm thông tin xác thực từ xa bằng bí mật dùng chung:
  ...
  <Accordion title='Control UI báo "unauthorized" (hoặc cứ kết nối lại). Giờ làm gì?'>
    ...
    - Khi có `AUTH_TOKEN_MISMATCH`, client đáng tin cậy có thể thử lại có giới hạn một lần với token thiết bị đã lưu trong bộ nhớ đệm khi gateway trả về gợi ý thử lại (`canRetryWithDeviceToken=true`, `recommendedNextStep=retry_with_device_token`).
    - Lần thử lại bằng token đã lưu đó hiện tái sử dụng các phạm vi đã duyệt được lưu cùng token thiết bị. Bên gọi `deviceToken` tường minh / `scopes` tường minh vẫn giữ tập phạm vi đã yêu cầu thay vì kế thừa phạm vi đã lưu.
    - Ngoài đường dẫn thử lại đó, thứ tự ưu tiên xác thực khi kết nối là token/mật khẩu dùng chung tường minh trước, rồi `deviceToken` tường minh, rồi token thiết bị đã lưu, rồi bootstrap token.
    - Kiểm tra phạm vi bootstrap token có tiền tố vai trò. Danh sách cho phép toán tử bootstrap tích hợp chỉ thỏa các yêu cầu toán tử; node hoặc các vai trò không phải toán tử khác vẫn cần phạm vi dưới tiền tố vai trò riêng của chúng.
    ...
    - Nếu vẫn lệch sau một lần thử lại, xoay/phê duyệt lại token thiết bị đã ghép cặp:
      - `openclaw devices list`
    ...
    - Nếu lệnh xoay đó báo bị từ chối, hãy kiểm tra hai điều:
      ...
      - giá trị `--scope` tường minh không được vượt quá phạm vi toán tử hiện tại của bên gọi
  ...
  <Accordion title="Tôi đặt gateway.bind tailnet nhưng nó không thể bind và không có gì lắng nghe">
...
<AccordionGroup>
  <Accordion title="Log ở đâu?">
    ...
    Bạn có thể đặt đường dẫn ổn định qua `logging.file`. Mức log tệp được kiểm soát bởi `logging.level`. Độ chi tiết của console được kiểm soát bởi `--verbose` và `logging.consoleLevel`.
  ...
  <Accordion title="Tôi đã đóng terminal trên Windows - làm thế nào để khởi động lại OpenClaw?">
    ...
    **1) WSL2 (khuyến nghị):** Gateway chạy bên trong Linux.
    ...
    Nếu bạn chưa từng cài dịch vụ, hãy khởi động nó ở foreground:
  ...
  <Accordion title="Gateway đã bật nhưng phản hồi không bao giờ đến. Tôi nên kiểm tra gì?">
    Bắt đầu bằng quét tình trạng nhanh:
    ...
    - Ghép cặp kênh/danh sách cho phép đang chặn phản hồi (kiểm tra cấu hình kênh + log).
    - WebChat/Dashboard đang mở mà không có token phù hợp.

    Nếu bạn ở từ xa, xác nhận tunnel/kết nối Tailscale đang hoạt động và
  ...
  <Accordion title='"Disconnected from gateway: no reason" - giờ làm gì?'>
    Điều này thường có nghĩa là UI mất kết nối WebSocket. Hãy kiểm tra:
  ...
  <Accordion title="Telegram setMyCommands thất bại. Tôi nên kiểm tra gì?">
    ...
    - `BOT_COMMANDS_TOO_MUCH`: menu Telegram có quá nhiều mục. OpenClaw đã cắt bớt theo giới hạn của Telegram và thử lại với ít lệnh hơn, nhưng vẫn cần bỏ một số mục menu. Giảm các lệnh plugin/skill/tùy chỉnh, hoặc tắt `channels.telegram.commands.native` nếu bạn không cần menu.
    ...
    Nếu Gateway ở xa, hãy chắc chắn rằng bạn đang xem log trên máy chủ Gateway.
  ...
  <Accordion title="TUI không hiển thị đầu ra. Tôi nên kiểm tra gì?">
    ...
    kênh, hãy chắc chắn tính năng gửi đã được bật (`/deliver on`).
  ...
  <Accordion title="Làm thế nào để dừng hoàn toàn rồi khởi động Gateway?">
    ...
    Dùng cách này khi Gateway chạy trong nền như một daemon.
...
<AccordionGroup>
  <Accordion title="Skill của tôi đã tạo một hình ảnh/PDF, nhưng không có gì được gửi">
    Tệp đính kèm gửi ra từ agent phải bao gồm một dòng `MEDIA:<path-or-url>` (trên dòng riêng). Xem [Thiết lập trợ lý OpenClaw](/vi/start/openclaw) và [Agent send](/vi/tools/agent-send).
    ...
    - `tools.fs.workspaceOnly=false` cho phép `MEDIA:` gửi các tệp cục bộ trên máy chủ mà agent đã có thể đọc, nhưng chỉ với phương tiện và các loại tài liệu an toàn (hình ảnh, âm thanh, video, PDF và tài liệu Office). Tệp văn bản thuần và tệp trông giống bí mật vẫn bị chặn.
...
<AccordionGroup>
  <Accordion title="Có an toàn khi cho phép OpenClaw nhận DM đến không?">
    Xem DM đến là đầu vào không đáng tin cậy. Các mặc định được thiết kế để giảm rủi ro:

    - Hành vi mặc định trên các kênh hỗ trợ DM là **ghép cặp**:
      - Người gửi không xác định nhận một mã ghép cặp; bot không xử lý tin nhắn của họ.
      - Phê duyệt bằng: `openclaw pairing approve --channel <channel> [--account <id>] <code>`
      - Yêu cầu đang chờ được giới hạn ở **3 cho mỗi kênh**; kiểm tra `openclaw pairing list --channel <channel> [--account <id>]` nếu không nhận được mã.
  ...
  <Accordion title="Prompt injection chỉ là mối lo cho bot công khai phải không?">
    ...
    - dùng một agent "đọc" chỉ đọc hoặc đã tắt công cụ để tóm tắt nội dung không đáng tin cậy
    ...
    - sandbox và allowlist công cụ nghiêm ngặt
  ...
  <Accordion title="Tôi có thể trao cho nó quyền tự chủ với tin nhắn văn bản của mình không, và điều đó có an toàn không?">
    Chúng tôi **không** khuyến nghị trao quyền tự chủ hoàn toàn với tin nhắn cá nhân của bạn. Mẫu an toàn nhất là:

    - Giữ DM ở **chế độ ghép cặp** hoặc allowlist chặt.
  ...
  <Accordion title="Tôi có thể dùng model rẻ hơn cho các tác vụ trợ lý cá nhân không?">
    Có, **nếu** agent chỉ chat và đầu vào đáng tin cậy. Các tầng nhỏ hơn
    dễ bị chiếm quyền chỉ dẫn hơn, nên tránh dùng chúng cho agent có bật công cụ
  ...
  <Accordion title="Tôi đã chạy /start trong Telegram nhưng không nhận được mã ghép cặp">
    Mã ghép cặp được gửi **chỉ** khi người gửi không xác định nhắn tin cho bot và
  ...
  <Accordion title="WhatsApp: nó có nhắn tin cho danh bạ của tôi không? Ghép cặp hoạt động thế nào?">
    Không. Chính sách DM mặc định của WhatsApp là **ghép cặp**. Người gửi không xác định chỉ nhận mã ghép cặp và tin nhắn của họ **không được xử lý**. OpenClaw chỉ trả lời các cuộc chat mà nó nhận được hoặc các lệnh gửi rõ ràng do bạn kích hoạt.
...
<AccordionGroup>
  ...
  <Accordion title="Làm thế nào để dừng/hủy một tác vụ đang chạy?">
    ...
    Hầu hết lệnh phải được gửi dưới dạng tin nhắn **độc lập** bắt đầu bằng `/`, nhưng một vài shortcut (như `/status`) cũng hoạt động inline cho người gửi trong allowlist.
  ...
  <Accordion title='Làm thế nào để gửi tin nhắn Discord từ Telegram? ("Cross-context messaging denied")'>
    ...
    Khởi động lại Gateway sau khi chỉnh sửa config.

  </Accordion>

  <Accordion title='Vì sao có cảm giác bot "phớt lờ" các tin nhắn dồn dập?'>
    Chế độ hàng đợi kiểm soát cách tin nhắn mới tương tác với một lượt chạy đang diễn ra. Dùng `/queue` để đổi chế độ:

    - `steer` - xếp hàng mọi chỉ đạo đang chờ cho ranh giới model tiếp theo trong lượt chạy hiện tại
    - `queue` - chỉ đạo kiểu cũ từng cái một
    - `followup` - chạy tin nhắn từng cái một
    - `collect` - gom tin nhắn và trả lời một lần
    - `steer-backlog` - chỉ đạo ngay, rồi xử lý backlog
    - `interrupt` - hủy lượt chạy hiện tại và bắt đầu mới

    Chế độ mặc định là `steer`. Bạn có thể thêm tùy chọn như `debounce:0.5s cap:25 drop:summarize` cho các chế độ followup. Xem [Hàng đợi lệnh](/vi/concepts/queue) và [Hàng đợi chỉ đạo](/vi/concepts/queue-steering).
...
<AccordionGroup>
  <Accordion title='Mô hình mặc định cho Anthropic khi có khóa API là gì?'>
    Trong OpenClaw, thông tin đăng nhập và lựa chọn mô hình là riêng biệt. Việc đặt `ANTHROPIC_API_KEY` (hoặc lưu khóa API Anthropic trong hồ sơ xác thực) sẽ bật xác thực, nhưng mô hình mặc định thực tế là bất cứ mô hình nào bạn cấu hình trong `agents.defaults.model.primary` (ví dụ: `anthropic/claude-sonnet-4-6` hoặc `anthropic/claude-opus-4-6`). Nếu bạn thấy `No credentials found for profile "anthropic:default"`, điều đó có nghĩa là Gateway không tìm thấy thông tin đăng nhập Anthropic trong `auth-profiles.json` dự kiến cho agent đang chạy.
  </Accordion>
</AccordionGroup>

---

Vẫn bị kẹt? Hãy hỏi trên [Discord](https://discord.com/invite/clawd) hoặc mở một [thảo luận trên GitHub](https://github.com/openclaw/openclaw/discussions).

## Liên quan

- [Câu hỏi thường gặp khi chạy lần đầu](/vi/help/faq-first-run) — cài đặt, thiết lập ban đầu, xác thực, gói đăng ký, lỗi ban đầu
- [Câu hỏi thường gặp về mô hình](/vi/help/faq-models) — lựa chọn mô hình, chuyển đổi dự phòng, hồ sơ xác thực
- [Khắc phục sự cố](/vi/help/troubleshooting) — xử lý bước đầu theo triệu chứng
