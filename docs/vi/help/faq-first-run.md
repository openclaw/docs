---
read_when:
    - Cài đặt mới, quy trình thiết lập ban đầu bị kẹt hoặc lỗi khi chạy lần đầu
...
---
  ...
  Hỏi đáp khởi động nhanh và thiết lập lần chạy đầu tiên. Với các tác vụ hằng ngày, mô hình, xác thực, phiên,
  ...
  <Accordion title="Tôi bị kẹt, cách nhanh nhất để xử lý">
    Dùng một tác nhân AI cục bộ có thể **thấy máy của bạn**. Cách này hiệu quả hơn nhiều so với hỏi
    ...
    của bạn (PATH, dịch vụ, quyền, tệp xác thực). Hãy cung cấp cho chúng **bản checkout nguồn đầy đủ** qua
    cài đặt dạng có thể chỉnh sửa (git):
    ...
    Nếu bạn phát hiện lỗi thật hoặc có bản sửa, vui lòng tạo GitHub issue hoặc gửi PR:
  ...
  <Accordion title="Heartbeat liên tục bỏ qua. Các lý do bỏ qua nghĩa là gì?">
    ...
    Trong chế độ tác vụ, dấu thời gian đến hạn chỉ được đẩy tới sau khi một lượt chạy heartbeat thật
    hoàn tất. Các lượt chạy bị bỏ qua không đánh dấu tác vụ là đã hoàn tất.
  ...
  <Accordion title="Cách được khuyến nghị để cài đặt và thiết lập OpenClaw">
    Repo khuyến nghị chạy từ nguồn và dùng onboarding:
    ...
    Trình hướng dẫn cũng có thể tự động build tài sản UI. Sau onboarding, bạn thường chạy Gateway trên cổng **18789**.

    Từ nguồn (người đóng góp/dev):
  ...
  <Accordion title="Làm cách nào mở dashboard sau onboarding?">
    Trình hướng dẫn mở trình duyệt của bạn với URL dashboard sạch (không gắn token) ngay sau onboarding và cũng in liên kết trong phần tóm tắt. Giữ tab đó mở; nếu nó không tự mở, hãy sao chép/dán URL đã in trên cùng máy.
  </Accordion>

  <Accordion title="Làm cách nào xác thực dashboard trên localhost so với từ xa?">
    **Localhost (cùng máy):**
    ...
    **Không trên localhost:**
    ...
    - **Tailscale Serve** (khuyến nghị): giữ bind loopback, chạy `openclaw gateway --tailscale serve`, mở `https://<magicdns>/`. Nếu `gateway.auth.allowTailscale` là `true`, header danh tính đáp ứng xác thực Control UI/WebSocket (không cần dán shared secret, giả định máy chủ gateway đáng tin cậy); HTTP API vẫn yêu cầu xác thực shared-secret trừ khi bạn chủ ý dùng private-ingress `none` hoặc xác thực HTTP trusted-proxy.
      Các lần thử xác thực Serve đồng thời không hợp lệ từ cùng một client được tuần tự hóa trước khi bộ giới hạn failed-auth ghi nhận chúng, nên lần thử lại sai thứ hai có thể đã hiển thị `retry later`.
    - **Tailnet bind**: chạy `openclaw gateway --bind tailnet --token "<token>"` (hoặc cấu hình xác thực bằng mật khẩu), mở `http://<tailscale-ip>:18789/`, rồi dán shared secret khớp vào cài đặt dashboard.
    - **Reverse proxy nhận biết danh tính**: giữ Gateway phía sau proxy đáng tin cậy, cấu hình `gateway.auth.mode: "trusted-proxy"`, rồi mở URL proxy. Proxy loopback cùng máy chủ yêu cầu đặt rõ `gateway.auth.trustedProxy.allowLoopback = true`.
  ...
  <Accordion title="Vì sao có hai cấu hình phê duyệt exec cho phê duyệt qua chat?">
    ...
    - Nếu chat đã hỗ trợ lệnh và trả lời, `/approve` trong cùng chat hoạt động qua đường dùng chung.
    ...
    - Khi có thẻ/nút phê duyệt gốc, UI gốc đó là đường chính; tác nhân chỉ nên đưa vào lệnh `/approve` thủ công nếu kết quả công cụ nói rằng phê duyệt qua chat không khả dụng hoặc phê duyệt thủ công là đường duy nhất.
    - Chỉ dùng `approvals.exec` khi lời nhắc cũng phải được chuyển tiếp tới chat khác hoặc phòng vận hành rõ ràng.
    - Chỉ dùng `channels.<channel>.execApprovals.target: "channel"` hoặc `"both"` khi bạn chủ ý muốn lời nhắc phê duyệt được đăng lại vào phòng/chủ đề gốc.
    - Phê duyệt Plugin lại tách riêng: mặc định dùng `/approve` trong cùng chat, có thể chuyển tiếp bằng `approvals.plugin`, và chỉ một số kênh gốc giữ xử lý plugin-approval-native ở trên.

    Bản ngắn gọn: chuyển tiếp dùng để định tuyến, cấu hình client gốc dùng cho UX phong phú hơn theo từng kênh.
  ...
  <Accordion title="Nó có chạy trên Raspberry Pi không?">
    ...
    ổ đĩa là đủ cho mục đích cá nhân, đồng thời ghi chú rằng **Raspberry Pi 4 có thể chạy được**.

    Nếu bạn muốn dư tài nguyên hơn (nhật ký, media, dịch vụ khác), **khuyến nghị 2GB**, nhưng đó
    không phải mức tối thiểu bắt buộc.

    Mẹo: một Pi/VPS nhỏ có thể lưu trữ Gateway, và bạn có thể ghép cặp **node** trên laptop/điện thoại của mình cho
  ...
  <Accordion title="Có mẹo nào cho cài đặt trên Raspberry Pi không?">
    Bản ngắn gọn: nó chạy được, nhưng hãy chuẩn bị cho vài điểm chưa mượt.

    - Dùng hệ điều hành **64-bit** và giữ Node >= 22.
    - Ưu tiên **cài đặt dạng có thể chỉnh sửa (git)** để bạn có thể xem nhật ký và cập nhật nhanh.
  ...
  <Accordion title="Nó bị kẹt ở wake up my friend / onboarding không nở. Giờ làm gì?">
    ...
    Nếu Gateway ở xa, đảm bảo tunnel/kết nối Tailscale đang hoạt động và UI
  ...
  <Accordion title="Tôi xem điểm mới trong phiên bản mới nhất ở đâu?">
    ...
    **Sửa lỗi** (cộng thêm tài liệu/phần khác khi cần).
  ...
  <Accordion title="Khác biệt giữa stable và beta">
    **Stable** và **beta** là **npm dist-tags**, không phải các dòng mã riêng biệt:
    ...
    Thông thường, một bản phát hành stable sẽ lên **beta** trước, rồi một bước
    ...
    trỏ đến **cùng một phiên bản** sau khi thăng cấp.
  ...
  <Accordion title="Tôi cài đặt phiên bản beta như thế nào và beta khác dev ở điểm nào?">
    ...
    **Dev** là đầu nhánh đang thay đổi của `main` (git); khi được phát hành, nó dùng npm dist-tag `dev`.
    ...
    Bộ cài đặt Windows (PowerShell):
    [https://openclaw.ai/install.ps1](https://openclaw.ai/install.ps1)

    Chi tiết hơn: [Kênh phát triển](/vi/install/development-channels) và [Cờ của bộ cài đặt](/vi/install/installer).
  ...
  <Accordion title="Làm thế nào để thử các phần mới nhất?">
    ...
    2. **Cài đặt có thể chỉnh sửa (từ trang bộ cài đặt):**
  ...
  <Accordion title="Cài đặt và onboarding thường mất bao lâu?">
    ...
    Nếu bị treo, hãy dùng [Bộ cài đặt bị kẹt](#quick-start-and-first-run-setup)
    và vòng lặp gỡ lỗi nhanh trong [Tôi bị kẹt](#quick-start-and-first-run-setup).

  </Accordion>

  <Accordion title="Bộ cài đặt bị kẹt? Làm thế nào để nhận thêm phản hồi?">
    Chạy lại bộ cài đặt với **đầu ra chi tiết**:
    ...
    Để cài đặt dạng có thể chỉnh sửa (git):
    ...
    Tùy chọn khác: [Cờ của bộ cài đặt](/vi/install/installer).

  </Accordion>

  <Accordion title="Cài đặt trên Windows báo không tìm thấy git hoặc không nhận diện openclaw">
    ...
    **1) lỗi npm spawn git / không tìm thấy git**
    ...
    - Đóng rồi mở lại PowerShell, sau đó chạy lại bộ cài đặt.

    **2) openclaw không được nhận diện sau khi cài đặt**

    - Thư mục bin global của npm không có trong PATH.
    - Kiểm tra đường dẫn:
    ...
    Nếu bạn muốn thiết lập Windows mượt mà nhất, hãy dùng **WSL2** thay vì Windows native.
  ...
  <Accordion title="Tài liệu chưa trả lời câu hỏi của tôi - làm thế nào để nhận câu trả lời tốt hơn?">
    ...
    Chi tiết hơn: [Cài đặt](/vi/install) và [Cờ của bộ cài đặt](/vi/install/installer).
  ...
  <Accordion title="Tôi cài đặt OpenClaw trên Linux như thế nào?">
    ...
    - Bộ cài đặt + cập nhật: [Cài đặt & cập nhật](/vi/install/updating).
  ...
  <Accordion title="Onboarding thực sự làm gì?">
    ...
    - **Thiết lập mô hình/xác thực** (OAuth nhà cung cấp, khóa API, Anthropic setup-token, cộng với các tùy chọn mô hình cục bộ như LM Studio)
  ...
  <Accordion title="Tôi có cần đăng ký Claude hoặc OpenAI để chạy công cụ này không?">
    ...
    **mô hình chỉ cục bộ** để dữ liệu của bạn ở lại trên thiết bị. Gói đăng ký (Claude
    ...
    Với Anthropic trong OpenClaw, phân chia thực tế là:
    ...
    - **Claude CLI / xác thực đăng ký Claude trong OpenClaw**: nhân viên Anthropic
      cho chúng tôi biết việc sử dụng này lại được cho phép, và OpenClaw đang xem việc sử dụng `claude -p`
      là được chấp thuận cho tích hợp này trừ khi Anthropic công bố chính sách
      mới
    ...
    OpenClaw cũng hỗ trợ các tùy chọn kiểu đăng ký được lưu trữ khác, bao gồm
  ...
  <Accordion title="Tôi có thể dùng đăng ký Claude Max mà không cần khóa API không?">
    ...
    Nhân viên Anthropic cho chúng tôi biết việc sử dụng Claude CLI kiểu OpenClaw lại được cho phép, vì vậy
    OpenClaw xem xác thực đăng ký Claude và việc sử dụng `claude -p` là được chấp thuận
  ...
  <Accordion title="Bạn có hỗ trợ xác thực đăng ký Claude (Claude Pro hoặc Max) không?">
    ...
    Nhân viên Anthropic cho chúng tôi biết việc sử dụng này lại được cho phép, vì vậy OpenClaw xem
    việc tái sử dụng Claude CLI và sử dụng `claude -p` là được chấp thuận cho tích hợp này
    ...
    lựa chọn an toàn hơn, dễ dự đoán hơn. Nếu bạn muốn các tùy chọn lưu trữ
...
<AccordionGroup>
  <Accordion title="Tại sao tôi thấy HTTP 429 rate_limit_error từ Anthropic?">
    Điều đó nghĩa là **quota/giới hạn tốc độ Anthropic** của bạn đã cạn cho cửa sổ hiện tại. Nếu bạn
    dùng **Claude CLI**, hãy đợi cửa sổ được đặt lại hoặc nâng cấp gói của bạn. Nếu bạn
    ...
    bản beta ngữ cảnh 1M của Anthropic (`context1m: true`). Điều đó chỉ hoạt động khi
    thông tin xác thực của bạn đủ điều kiện cho tính phí ngữ cảnh dài (tính phí API key hoặc
  ...
  <Accordion title="AWS Bedrock có được hỗ trợ không?">
    Có. OpenClaw có một nhà cung cấp **Amazon Bedrock (Converse)** tích hợp sẵn. Khi có các dấu hiệu env AWS, OpenClaw có thể tự động phát hiện danh mục Bedrock dạng streaming/văn bản và hợp nhất nó thành một nhà cung cấp `amazon-bedrock` ngầm định; nếu không, bạn có thể bật rõ ràng `plugins.entries.amazon-bedrock.config.discovery.enabled` hoặc thêm một mục nhà cung cấp thủ công. Xem [Amazon Bedrock](/vi/providers/bedrock) và [Nhà cung cấp mô hình](/vi/providers/models). Nếu bạn muốn một luồng khóa được quản lý, proxy tương thích OpenAI đặt trước Bedrock vẫn là một lựa chọn hợp lệ.
  </Accordion>
  ...
  <Accordion title="Xác thực Codex hoạt động như thế nào?">
    ...
    xác thực thuê bao ChatGPT/Codex cùng thực thi máy chủ ứng dụng Codex gốc. Chỉ dùng
    `openai-codex/gpt-5.5` khi bạn muốn Codex OAuth thông qua trình chạy PI mặc định.
    Dùng `openai/gpt-5.5` không có ghi đè runtime Codex để
    truy cập trực tiếp bằng API key OpenAI.
  ...
  <Accordion title="Vì sao OpenClaw vẫn nhắc đến openai-codex?">
    ...
    - `openai/gpt-5.5` + `agentRuntime.id: "codex"` = xác thực thuê bao ChatGPT/Codex với runtime Codex gốc
    - `openai-codex/gpt-5.5` = tuyến Codex OAuth trong PI
    - `openai/gpt-5.5` không có ghi đè runtime Codex = tuyến API key OpenAI trực tiếp trong PI
    - `openai-codex:...` = id hồ sơ xác thực, không phải tham chiếu mô hình

    Nếu bạn muốn đường dẫn tính phí/giới hạn trực tiếp của OpenAI Platform, đặt
    `OPENAI_API_KEY`. Nếu bạn muốn xác thực thuê bao ChatGPT/Codex, đăng nhập bằng
    `openclaw models auth login --provider openai-codex`. Với runtime Codex gốc,
    giữ tham chiếu mô hình là `openai/gpt-5.5` và đặt
    `agentRuntime.id: "codex"`. Chỉ dùng tham chiếu mô hình `openai-codex/*` cho các lượt chạy PI.
  ...
  <Accordion title="Vì sao giới hạn Codex OAuth có thể khác ChatGPT web?">
    ...
    thành quyền truy cập API trực tiếp. Nếu bạn muốn đường dẫn tính phí/giới hạn trực tiếp của OpenAI Platform,
    dùng `openai/*` với API key.
  ...
  <Accordion title="Bạn có hỗ trợ xác thực thuê bao OpenAI (Codex OAuth) không?">
    ...
    OpenAI cho phép rõ ràng việc dùng OAuth thuê bao trong công cụ/quy trình làm việc bên ngoài
  ...
  <Accordion title="Làm sao thiết lập Gemini CLI OAuth?">
  ...
  <Accordion title="Mô hình cục bộ có ổn cho trò chuyện thông thường không?">
    Thường là không. OpenClaw cần ngữ cảnh lớn + độ an toàn cao; thẻ nhỏ sẽ cắt ngắn và rò rỉ. Nếu bắt buộc, hãy chạy bản dựng mô hình **lớn nhất** bạn có thể chạy cục bộ (LM Studio) và xem [/gateway/local-models](/vi/gateway/local-models). Mô hình nhỏ hơn/lượng tử hóa làm tăng rủi ro prompt-injection - xem [Bảo mật](/vi/gateway/security).
  </Accordion>

  <Accordion title="Làm sao giữ lưu lượng mô hình lưu trữ trong một khu vực cụ thể?">
    Chọn các endpoint ghim theo khu vực. OpenRouter cung cấp các tùy chọn lưu trữ tại Hoa Kỳ cho MiniMax, Kimi và GLM; chọn biến thể lưu trữ tại Hoa Kỳ để giữ dữ liệu trong khu vực. Bạn vẫn có thể liệt kê Anthropic/OpenAI cùng các mục này bằng cách dùng `models.mode: "merge"` để các phương án dự phòng vẫn sẵn có trong khi tôn trọng nhà cung cấp theo khu vực mà bạn chọn.
  </Accordion>
  ...
  <Accordion title="Tôi có phải mua Mac Mini để cài đặt không?">
    ...
    mua nó làm máy chủ luôn bật, nhưng một VPS nhỏ, máy chủ tại nhà hoặc máy dạng Raspberry Pi-class cũng dùng được.

    Bạn chỉ cần Mac **cho các công cụ chỉ hỗ trợ macOS**. Với iMessage, dùng [BlueBubbles](/vi/channels/bluebubbles) (khuyến nghị) - máy chủ BlueBubbles chạy trên bất kỳ Mac nào, còn Gateway có thể chạy trên Linux hoặc nơi khác. Nếu bạn muốn các công cụ chỉ hỗ trợ macOS khác, hãy chạy Gateway trên Mac hoặc ghép nối một nút macOS.

    Tài liệu: [BlueBubbles](/vi/channels/bluebubbles), [Nút](/vi/nodes), [Chế độ Mac từ xa](/vi/platforms/mac/remote).
  ...
  <Accordion title="Tôi có cần Mac mini để hỗ trợ iMessage không?">
    ...
    Mac nào cũng được. **Dùng [BlueBubbles](/vi/channels/bluebubbles)** (khuyến nghị) cho iMessage - máy chủ BlueBubbles chạy trên macOS, còn Gateway có thể chạy trên Linux hoặc nơi khác.
    ...
    Tài liệu: [BlueBubbles](/vi/channels/bluebubbles), [Nút](/vi/nodes),
  ...
  <Accordion title="Nếu tôi mua Mac mini để chạy OpenClaw, tôi có thể kết nối nó với MacBook Pro của mình không?">
    ...
    **nút** (thiết bị đồng hành). Các nút không chạy Gateway - chúng cung cấp thêm
    khả năng như màn hình/camera/canvas và `system.run` trên thiết bị đó.

    Mẫu phổ biến:

    - Gateway trên Mac mini (luôn bật).
    - MacBook Pro chạy ứng dụng macOS hoặc máy chủ nút và ghép nối với Gateway.
    - Dùng `openclaw nodes status` / `openclaw nodes list` để xem nó.

    Tài liệu: [Nút](/vi/nodes), [CLI nút](/vi/cli/nodes).
  ...
  <Accordion title="Tôi có thể dùng Bun không?">
    ...
    Nếu bạn vẫn muốn thử nghiệm với Bun, hãy làm trên gateway không dùng cho sản xuất
  ...
  <Accordion title="Nhiều người có thể dùng một số WhatsApp với các phiên bản OpenClaw khác nhau không?">
    Có, thông qua **định tuyến đa tác nhân**. Gắn **DM** WhatsApp của từng người gửi (peer `kind: "direct"`, người gửi E.164 như `+15551234567`) với một `agentId` khác nhau, để mỗi người có workspace và kho phiên riêng. Phản hồi vẫn đến từ **cùng tài khoản WhatsApp**, và kiểm soát truy cập DM (`channels.whatsapp.dmPolicy` / `channels.whatsapp.allowFrom`) là toàn cục cho mỗi tài khoản WhatsApp. Xem [Định tuyến đa tác nhân](/vi/concepts/multi-agent) và [WhatsApp](/vi/channels/whatsapp).
  </Accordion>
  ...
  <Accordion title="Khác biệt giữa cài đặt git có thể hack và cài đặt npm">
    - **Cài đặt có thể hack (git):** checkout đầy đủ mã nguồn, có thể chỉnh sửa, phù hợp nhất cho người đóng góp.
  ...
  <Accordion title="Tôi nên chạy Gateway trên laptop hay VPS?">
    ...
    ít ma sát nhất và chấp nhận chế độ ngủ/khởi động lại, hãy chạy cục bộ.

    **Laptop (Gateway cục bộ)**

    - **Ưu điểm:** không tốn chi phí máy chủ, truy cập trực tiếp vào tệp cục bộ, cửa sổ trình duyệt đang chạy.
    - **Nhược điểm:** máy ngủ/mạng rớt = mất kết nối, cập nhật/khởi động lại OS gây gián đoạn, máy phải luôn thức.

    **VPS / đám mây**

    - **Ưu điểm:** luôn bật, mạng ổn định, không gặp vấn đề máy tính xách tay ngủ, dễ duy trì hoạt động.
    - **Nhược điểm:** thường chạy không giao diện (dùng ảnh chụp màn hình), chỉ truy cập tệp từ xa, bạn phải dùng SSH để cập nhật.

    **Ghi chú riêng cho OpenClaw:** WhatsApp/Telegram/Slack/Mattermost/Discord đều hoạt động tốt từ VPS. Đánh đổi thực sự duy nhất là **trình duyệt không giao diện** so với cửa sổ hiển thị. Xem [Trình duyệt](/vi/tools/browser).

    **Mặc định được khuyến nghị:** VPS nếu trước đây bạn từng gặp tình trạng Gateway mất kết nối. Cục bộ rất phù hợp khi bạn đang chủ động dùng Mac và muốn truy cập tệp cục bộ hoặc tự động hóa UI với trình duyệt hiển thị.
  ...
  <Accordion title="Việc chạy OpenClaw trên một máy chuyên dụng quan trọng đến mức nào?">
    ...
    - **Máy chủ chuyên dụng (VPS/Mac mini/Pi):** luôn bật, ít bị gián đoạn do ngủ/khởi động lại hơn, quyền gọn gàng hơn, dễ duy trì hoạt động hơn.
    ...
    Nếu bạn muốn cả hai lợi ích, hãy giữ Gateway trên một máy chủ chuyên dụng và ghép nối máy tính xách tay của bạn làm một **Node** cho các công cụ màn hình/camera/exec cục bộ. Xem [Nodes](/vi/nodes).
  ...
  <Accordion title="Yêu cầu VPS tối thiểu và OS được khuyến nghị là gì?">
    ...
    - **Tối thiểu tuyệt đối:** 1 vCPU, 1GB RAM, khoảng 500MB ổ đĩa.
    - **Khuyến nghị:** 1-2 vCPU, 2GB RAM trở lên để có dư địa (nhật ký, phương tiện, nhiều kênh). Các công cụ Node và tự động hóa trình duyệt có thể tiêu tốn nhiều tài nguyên.

    OS: dùng **Ubuntu LTS** (hoặc bất kỳ Debian/Ubuntu hiện đại nào). Lộ trình cài đặt Linux được kiểm thử tốt nhất ở đó.
  ...
  <Accordion title="Tôi có thể chạy OpenClaw trong VM không và yêu cầu là gì?">
    ...
    Nếu bạn đang dùng Windows, **WSL2 là cách thiết lập kiểu VM dễ nhất** và có khả năng tương thích
