...
summary: 'FAQ: mặc định mô hình, lựa chọn, bí danh, chuyển đổi, chuyển đổi dự phòng và hồ sơ xác thực'
...
---
  ...
  <Accordion title='What is the "default model"?'>
    ...
    Mô hình được tham chiếu dưới dạng `provider/model` (ví dụ: `openai/gpt-5.5` hoặc `anthropic/claude-sonnet-4-6`). Nếu bạn bỏ qua nhà cung cấp, OpenClaw trước tiên sẽ thử một bí danh, sau đó thử một kết quả khớp nhà cung cấp đã cấu hình duy nhất cho đúng id mô hình đó, và chỉ sau đó mới quay về nhà cung cấp mặc định đã cấu hình như một đường tương thích đã ngừng khuyến nghị. Nếu nhà cung cấp đó không còn cung cấp mô hình mặc định đã cấu hình, OpenClaw sẽ quay về nhà cung cấp/mô hình đã cấu hình đầu tiên thay vì hiển thị một mặc định nhà cung cấp đã bị gỡ bỏ và lỗi thời. Bạn vẫn nên đặt `provider/model` **một cách rõ ràng**.

  </Accordion>

  <Accordion title="What model do you recommend?">
    **Mặc định được khuyến nghị:** dùng mô hình thế hệ mới nhất mạnh nhất có trong ngăn xếp nhà cung cấp của bạn.
    **Đối với tác nhân có bật công cụ hoặc đầu vào không đáng tin cậy:** ưu tiên sức mạnh mô hình hơn chi phí.
    **Đối với trò chuyện thường lệ/rủi ro thấp:** dùng các mô hình dự phòng rẻ hơn và định tuyến theo vai trò tác nhân.

    MiniMax có tài liệu riêng: [MiniMax](/vi/providers/minimax) và
    [Mô hình cục bộ](/vi/gateway/local-models).

    Nguyên tắc kinh nghiệm: dùng **mô hình tốt nhất bạn có thể chi trả** cho công việc rủi ro cao, và một mô hình rẻ hơn
  ...
  <Accordion title="How do I switch models without wiping my config?">
    ...
    Đối với chỉnh sửa RPC, trước tiên hãy kiểm tra bằng `config.schema.lookup` và ưu tiên `config.patch`. Payload tra cứu cung cấp cho bạn đường dẫn đã chuẩn hóa, tài liệu/ràng buộc schema nông, và tóm tắt con trực tiếp.
  ...
  <Accordion title="Can I use self-hosted models (llama.cpp, vLLM, Ollama)?">
    ...
    - `Cloud + Local` cung cấp cho bạn các mô hình đám mây cùng với các mô hình Ollama cục bộ của bạn
  ...
  <Accordion title="What do OpenClaw, Flawd, and Krill use for models?">
    ...
    - Đối với các tác nhân nhạy cảm về bảo mật/có bật công cụ, hãy dùng mô hình thế hệ mới nhất mạnh nhất có sẵn.
  ...
  <Accordion title="How do I switch models on the fly (without restarting)?">
    ...
    `/model` (và `/model list`) hiển thị một bộ chọn đánh số, gọn. Chọn theo số:

    ```
    /model 3
    ```

    Bạn cũng có thể buộc dùng một hồ sơ xác thực cụ thể cho nhà cung cấp (theo phiên):
  ...
  <Accordion title="If two providers expose the same model id, which one does /model use?">
    ...
    Ví dụ, `qianfan/deepseek-v4-flash` và `deepseek/deepseek-v4-flash` là các tham chiếu mô hình khác nhau dù cả hai đều chứa `deepseek-v4-flash`. OpenClaw không nên âm thầm chuyển từ nhà cung cấp này sang nhà cung cấp khác chỉ vì id mô hình trần khớp nhau.

    Một tham chiếu `/model` do người dùng chọn cũng nghiêm ngặt đối với chính sách dự phòng. Nếu nhà cung cấp/mô hình đã chọn đó không khả dụng, phản hồi sẽ thất bại rõ ràng thay vì trả lời từ `agents.defaults.model.fallbacks`. Chuỗi dự phòng đã cấu hình vẫn áp dụng cho mặc định đã cấu hình, mô hình chính của cron job, và trạng thái dự phòng được tự động chọn.

    Nếu một lần chạy bắt đầu từ một ghi đè không thuộc phiên được phép dùng dự phòng, OpenClaw thử nhà cung cấp/mô hình được yêu cầu trước, rồi các dự phòng đã cấu hình, và chỉ sau đó mới đến mô hình chính đã cấu hình. Điều đó ngăn các id mô hình trần trùng lặp nhảy thẳng về nhà cung cấp mặc định.
  ...
  <Accordion title="Can I use GPT 5.5 for daily tasks and Codex 5.5 for coding?">
    ...
    - **Tác nhân lập trình Codex gốc:** đặt `agents.defaults.model.primary` thành `openai/gpt-5.5`. Đăng nhập bằng `openclaw models auth login --provider openai-codex` khi bạn muốn xác thực thuê bao ChatGPT/Codex.
    - **Tác vụ OpenAI API trực tiếp bên ngoài vòng lặp tác nhân:** cấu hình `OPENAI_API_KEY` cho hình ảnh, embedding, speech, realtime, và các bề mặt OpenAI API không thuộc tác nhân khác.
    - **Xác thực khóa API cho tác nhân OpenAI:** dùng `/model openai/gpt-5.5` với một hồ sơ khóa API `openai-codex` có thứ tự.
    - **Tác nhân phụ:** định tuyến tác vụ lập trình đến một tác nhân tập trung vào Codex với mô hình `openai/gpt-5.5` riêng.
  ...
  <Accordion title="How do I configure fast mode for GPT 5.5?">
    ...
    Đối với OpenAI, chế độ nhanh ánh xạ tới `service_tier = "priority"` trên các yêu cầu Responses gốc được hỗ trợ. Ghi đè `/fast` theo phiên thắng mặc định cấu hình.

    Xem [Chế độ suy nghĩ và nhanh](/vi/tools/thinking) và [Chế độ nhanh OpenAI](/vi/providers/openai#fast-mode).
  ...
  <Accordion title='Why do I see "Model ... is not allowed" and then no reply?'>
    ...
    `agents.defaults.models`, thêm wildcard nhà cung cấp như `"provider/*": {}` cho catalog nhà cung cấp động, xóa danh sách cho phép, hoặc chọn một mô hình từ `/model list`.
  ...
  <Accordion title='Why do I see "Unknown model: minimax/MiniMax-M2.7"?'>
    ...
    2. Đảm bảo MiniMax đã được cấu hình (wizard hoặc JSON), hoặc xác thực MiniMax
       ...
       cho `minimax-portal`).
    3. Dùng đúng id mô hình (phân biệt hoa thường) cho đường dẫn xác thực của bạn:
  ...
  <Accordion title="Can I use MiniMax as my default and OpenAI for complex tasks?">
    Có. Dùng **MiniMax làm mặc định** và chuyển đổi mô hình **theo phiên** khi cần.
    Dự phòng dành cho **lỗi**, không phải "tác vụ khó", vì vậy hãy dùng `/model` hoặc một tác nhân riêng.

    **Tùy chọn A: chuyển đổi theo phiên**
    ...
    **Tùy chọn B: tác nhân riêng**
    ...
    - Mặc định của Tác nhân A: MiniMax
    - Mặc định của Tác nhân B: OpenAI
    - Định tuyến theo tác nhân hoặc dùng `/agent` để chuyển đổi
  ...
  <Accordion title="Làm cách nào để thêm mô hình từ các nhà cung cấp khác như OpenRouter hoặc Z.AI?">
    ...
    **Không tìm thấy khóa API cho nhà cung cấp sau khi thêm một agent mới**
    ...
    - Hoặc chỉ sao chép các hồ sơ `api_key` / `token` tĩnh có thể mang theo từ kho xác thực của agent chính sang kho xác thực của agent mới.
    - Với hồ sơ OAuth, hãy đăng nhập từ agent mới khi nó cần tài khoản riêng; nếu không, OpenClaw có thể đọc xuyên tới agent mặc định/chính mà không cần sao chép refresh token.
...
<AccordionGroup>
  <Accordion title="Chuyển đổi dự phòng hoạt động như thế nào?">
    ...
    Thời gian chờ áp dụng cho các hồ sơ bị lỗi (backoff theo hàm mũ), vì vậy OpenClaw có thể tiếp tục phản hồi ngay cả khi một nhà cung cấp bị giới hạn tốc độ hoặc tạm thời gặp lỗi.
    ...
    cửa sổ sử dụng định kỳ (`weekly/monthly limit reached`) là các giới hạn
    ...
    văn bản thanh toán rõ ràng trên `401` hoặc `403`, OpenClaw vẫn có thể giữ nó trong
    luồng thanh toán, nhưng các bộ so khớp văn bản riêng theo nhà cung cấp vẫn chỉ áp dụng trong phạm vi
    ...
    `rate_limit`, không phải một trạng thái vô hiệu hóa thanh toán dài hạn.
    ...
    exceeded` vẫn đi theo đường Compaction/thử lại thay vì chuyển sang dự phòng
    mô hình.
    ...
    error`), và lỗi nhà cung cấp bận như `ModelNotReadyException` là các tín hiệu
    hết thời gian/quá tải đáng để chuyển đổi dự phòng khi ngữ cảnh nhà cung cấp
    khớp.
  ...
  <Accordion title='Thông báo "No credentials found for profile anthropic:default" nghĩa là gì?'>
    Nó có nghĩa là hệ thống đã cố dùng ID hồ sơ xác thực `anthropic:default`, nhưng không tìm thấy thông tin xác thực cho hồ sơ đó trong kho xác thực dự kiến.
    ...
    **Danh sách kiểm tra khắc phục:**
    ...
    - **Xác nhận biến môi trường của bạn được Gateway tải**
      - Nếu bạn đặt `ANTHROPIC_API_KEY` trong shell nhưng chạy Gateway qua systemd/launchd, nó có thể không kế thừa biến này. Hãy đặt nó trong `~/.openclaw/.env` hoặc bật `env.shellEnv`.
  ...
  <Accordion title="Tại sao hệ thống cũng thử Google Gemini rồi thất bại?">
    ...
    **Yêu cầu LLM bị từ chối: yêu cầu chữ ký suy nghĩ (Google Antigravity)**

    Nguyên nhân: lịch sử phiên chứa **các khối suy nghĩ không có chữ ký** (thường do
    một luồng bị hủy/dở dang). Google Antigravity yêu cầu chữ ký cho các khối suy nghĩ.

    Cách khắc phục: OpenClaw hiện loại bỏ các khối suy nghĩ chưa ký cho Google Antigravity Claude. Nếu lỗi vẫn xuất hiện, hãy bắt đầu một **phiên mới** hoặc đặt `/thinking off` cho agent đó.
...
<AccordionGroup>
  <Accordion title="Hồ sơ xác thực là gì?">
    ...
    Để kiểm tra hồ sơ đã lưu mà không xuất bí mật, hãy chạy `openclaw models auth list` (tùy chọn `--provider <id>` hoặc `--json`). Xem [CLI mô hình](/vi/cli/models#auth-profiles) để biết chi tiết.
  ...
  <Accordion title="Tôi có thể kiểm soát hồ sơ xác thực nào được thử trước không?">
    ...
    OpenClaw có thể tạm thời bỏ qua một hồ sơ nếu nó đang trong **thời gian chờ** ngắn (giới hạn tốc độ/hết thời gian/lỗi xác thực) hoặc trạng thái **bị vô hiệu hóa** dài hơn (thanh toán/không đủ tín dụng). Để kiểm tra điều này, chạy `openclaw models status --json` và xem `auth.unusableProfiles`. Tinh chỉnh: `auth.cooldowns.billingBackoffHours*`.

    Thời gian chờ do giới hạn tốc độ có thể theo phạm vi mô hình. Một hồ sơ đang trong thời gian chờ
    cho một mô hình vẫn có thể dùng được cho một mô hình cùng họ trên cùng nhà cung cấp,
...
- [FAQ — thiết lập khởi động nhanh và chạy lần đầu](/vi/help/faq-first-run)
