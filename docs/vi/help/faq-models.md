...
read_when:
    ...
    - Hiểu hồ sơ xác thực và cách quản lý chúng
...
---
  ...
  <Accordion title='“Mô hình mặc định” là gì?'>
    ...
    Mô hình được tham chiếu dưới dạng `provider/model` (ví dụ: `openai/gpt-5.5` hoặc `anthropic/claude-sonnet-4-6`). Nếu bạn bỏ qua provider, OpenClaw trước tiên sẽ thử một bí danh, sau đó là một provider đã cấu hình duy nhất khớp với chính xác model id đó, và chỉ sau đó mới quay về provider mặc định đã cấu hình như một đường dẫn tương thích đã lỗi thời. Nếu provider đó không còn cung cấp mô hình mặc định đã cấu hình, OpenClaw sẽ quay về provider/model đã cấu hình đầu tiên thay vì hiển thị mặc định provider đã bị xóa và lỗi thời. Bạn vẫn nên đặt `provider/model` **một cách rõ ràng**.

  </Accordion>

  <Accordion title="Bạn khuyến nghị mô hình nào?">
    **Mặc định được khuyến nghị:** dùng mô hình thế hệ mới nhất mạnh nhất có trong ngăn xếp provider của bạn.
    **Với agent có bật công cụ hoặc đầu vào không đáng tin cậy:** ưu tiên sức mạnh mô hình hơn chi phí.
    **Với chat thường lệ/rủi ro thấp:** dùng các mô hình dự phòng rẻ hơn và định tuyến theo vai trò agent.
    ...
    cho chat thường lệ hoặc tóm tắt. Bạn có thể định tuyến mô hình theo từng agent và dùng sub-agent để
  ...
  <Accordion title="Làm thế nào để chuyển mô hình mà không xóa sạch cấu hình?">
    ...
    Tránh dùng `config.apply` với một đối tượng một phần trừ khi bạn muốn thay thế toàn bộ cấu hình.
    Với chỉnh sửa RPC, hãy kiểm tra bằng `config.schema.lookup` trước và ưu tiên `config.patch`. Payload lookup cung cấp cho bạn đường dẫn đã chuẩn hóa, tài liệu/ràng buộc schema nông, và tóm tắt các mục con trực tiếp.
  ...
  <Accordion title="Làm thế nào để chuyển mô hình ngay lập tức (không cần khởi động lại)?">
    ...
    `/model` (và `/model list`) hiển thị bộ chọn gọn, đánh số. Chọn theo số:

    ```
    /model 3
    ```

    Bạn cũng có thể ép dùng một hồ sơ xác thực cụ thể cho provider (theo từng phiên):
    ...
    **Làm thế nào để bỏ ghim một hồ sơ tôi đã đặt bằng @profile?**
    ...
    Nếu bạn muốn quay lại mặc định, hãy chọn từ `/model` (hoặc gửi `/model <default provider/model>`).
  ...
  <Accordion title="Nếu hai provider cung cấp cùng một model id, /model dùng provider nào?">
    ...
    Ví dụ, `qianfan/deepseek-v4-flash` và `deepseek/deepseek-v4-flash` là các tham chiếu mô hình khác nhau dù cả hai đều chứa `deepseek-v4-flash`. OpenClaw không nên âm thầm chuyển từ provider này sang provider kia chỉ vì model id trần khớp.

    Tham chiếu `/model` do người dùng chọn cũng nghiêm ngặt với chính sách dự phòng. Nếu provider/model đã chọn đó không khả dụng, phản hồi sẽ thất bại rõ ràng thay vì trả lời từ `agents.defaults.model.fallbacks`. Chuỗi dự phòng đã cấu hình vẫn áp dụng cho các mặc định đã cấu hình, primary của cron job, và trạng thái dự phòng được tự động chọn.

    Nếu một lượt chạy bắt đầu từ một override không thuộc phiên được phép dùng dự phòng, OpenClaw thử provider/model được yêu cầu trước, rồi đến các dự phòng đã cấu hình, và chỉ sau đó mới đến primary đã cấu hình. Điều đó ngăn các model id trần trùng lặp nhảy thẳng về provider mặc định.
  ...
  <Accordion title="Tôi có thể dùng GPT 5.5 cho tác vụ hằng ngày và Codex 5.5 cho lập trình không?">
    ...
    - **Agent lập trình Codex gốc:** đặt `agents.defaults.model.primary` thành `openai/gpt-5.5`. Đăng nhập bằng `openclaw models auth login --provider openai` khi bạn muốn xác thực bằng gói đăng ký ChatGPT/Codex.
    - **Tác vụ API OpenAI trực tiếp ngoài vòng lặp agent:** cấu hình `OPENAI_API_KEY` cho hình ảnh, embedding, giọng nói, realtime, và các bề mặt API OpenAI không thuộc agent khác.
    - **Xác thực agent OpenAI bằng API key:** dùng `/model openai/gpt-5.5` với hồ sơ API-key `openai` có thứ tự.
    - **Sub-agent:** định tuyến tác vụ lập trình đến agent tập trung vào Codex với mô hình `openai/gpt-5.5` riêng.
  ...
  <Accordion title="Làm thế nào để cấu hình chế độ nhanh cho GPT 5.5?">
    ...
    Với OpenAI, chế độ nhanh ánh xạ tới `service_tier = "priority"` trên các yêu cầu Responses gốc được hỗ trợ. Override `/fast` theo phiên thắng mặc định cấu hình. Các lượt app-server của Codex chỉ có thể nhận tier khi bắt đầu lượt, nên `auto` áp dụng trên lượt mô hình tiếp theo do OpenClaw khởi động thay vì bên trong một lượt app-server đang chạy.
  ...
  <Accordion title='Vì sao tôi thấy "Model ... is not allowed" rồi không có phản hồi?'>
    ...
    override theo phiên. Chọn một mô hình không nằm trong danh sách đó sẽ trả về:
    ...
    Lỗi đó được trả về **thay vì** một phản hồi bình thường. Cách sửa: thêm chính xác mô hình vào
    `agents.defaults.models`, thêm wildcard provider như `"provider/*": {}` cho catalog provider động, xóa allowlist, hoặc chọn một mô hình từ `/model list`.
    Nếu lệnh cũng bao gồm `--runtime codex`, hãy cập nhật allowlist trước rồi thử lại
  ...
  <Accordion title='Vì sao tôi thấy "Unknown model: minimax/MiniMax-M3"?'>
    ...
    2. Đảm bảo MiniMax đã được cấu hình (wizard hoặc JSON), hoặc xác thực MiniMax
       ...
       cho `minimax-portal`).
    3. Dùng chính xác model id (phân biệt chữ hoa chữ thường) cho đường dẫn xác thực của bạn:
       `minimax/MiniMax-M3`, `minimax/MiniMax-M2.7`, hoặc
       `minimax/MiniMax-M2.7-highspeed` cho thiết lập API-key, hoặc
  ...
  <Accordion title="Tôi có thể dùng MiniMax làm mặc định và OpenAI cho tác vụ phức tạp không?">
    Có. Dùng **MiniMax làm mặc định** và chuyển mô hình **theo phiên** khi cần.
  ...
  <Accordion title="opus / sonnet / gpt có phải là lối tắt tích hợp sẵn không?">
    Có. OpenClaw cung cấp một vài cách viết tắt mặc định (chỉ được áp dụng khi mô hình tồn tại trong `agents.defaults.models`):
  ...
  <Accordion title="Làm cách nào để thêm mô hình từ các nhà cung cấp khác như OpenRouter hoặc Z.AI?">
    ...
    Nếu bạn tham chiếu một nhà cung cấp/mô hình nhưng thiếu khóa nhà cung cấp bắt buộc, bạn sẽ nhận được lỗi xác thực runtime (ví dụ: `No API key found for provider "zai"`).

    **Không tìm thấy khóa API cho nhà cung cấp sau khi thêm tác nhân mới**

    Điều này thường có nghĩa là **tác nhân mới** có kho xác thực trống. Xác thực theo từng tác nhân và
    được lưu trong:
    ...
    - Với hồ sơ OAuth, hãy đăng nhập từ tác nhân mới khi tác nhân đó cần tài khoản riêng; nếu không, OpenClaw có thể đọc xuyên qua tác nhân mặc định/chính mà không cần sao chép refresh token.

    **Không** dùng lại `agentDir` cho nhiều tác nhân; việc đó gây xung đột xác thực/phiên.
...
<AccordionGroup>
  <Accordion title="Chuyển đổi dự phòng hoạt động như thế nào?">
    ...
    Thời gian chờ áp dụng cho các hồ sơ đang lỗi (backoff theo hàm mũ), nên OpenClaw vẫn có thể phản hồi ngay cả khi một nhà cung cấp bị giới hạn tốc độ hoặc tạm thời gặp lỗi.
    ...
    nhánh thanh toán, nhưng các bộ khớp văn bản riêng theo nhà cung cấp vẫn chỉ nằm trong phạm vi
    ...
    `rate_limit`, không phải một lệnh vô hiệu hóa thanh toán dài hạn.
    ...
    exceeded` vẫn nằm trên đường dẫn Compaction/thử lại thay vì chuyển sang
    dự phòng mô hình.
    ...
    như Anthropic thuần `An unknown error occurred`, OpenRouter thuần
    ...
    tín hiệu timeout/quá tải đáng để chuyển đổi dự phòng khi ngữ cảnh nhà cung cấp
    khớp.
  ...
  <Accordion title='&quot;Không tìm thấy thông tin xác thực cho hồ sơ anthropic:default&quot; nghĩa là gì?'>
    Điều đó có nghĩa là hệ thống đã cố dùng ID hồ sơ xác thực `anthropic:default`, nhưng không tìm thấy thông tin xác thực cho nó trong kho xác thực mong đợi.
    ...
    **Danh sách kiểm tra khắc phục:**
    ...
    - **Xác nhận biến môi trường của bạn được Gateway tải**
      - Nếu bạn đặt `ANTHROPIC_API_KEY` trong shell nhưng chạy Gateway qua systemd/launchd, nó có thể không kế thừa biến đó. Đặt biến trong `~/.openclaw/.env` hoặc bật `env.shellEnv`.
  ...
  <Accordion title="Tại sao nó cũng thử Google Gemini và thất bại?">
    ...
    Khắc phục: hoặc cung cấp xác thực Google, hoặc xóa/tránh các mô hình Google trong `agents.defaults.model.fallbacks` / bí danh để dự phòng không định tuyến tới đó.
    ...
    **Yêu cầu LLM bị từ chối: cần chữ ký thinking (Google Antigravity)**
    ...
    Khắc phục: OpenClaw hiện loại bỏ các khối thinking chưa ký cho Google Antigravity Claude. Nếu lỗi vẫn xuất hiện, hãy bắt đầu một **phiên mới** hoặc đặt `/thinking off` cho tác nhân đó.
...
<AccordionGroup>
  ...
  <Accordion title="Tôi có thể kiểm soát hồ sơ xác thực nào được thử trước không?">
    Có. Cấu hình hỗ trợ siêu dữ liệu tùy chọn cho hồ sơ và thứ tự theo từng nhà cung cấp (`auth.order.<provider>`). Điều này **không** lưu bí mật; nó ánh xạ ID sang nhà cung cấp/chế độ và đặt thứ tự luân phiên.

    OpenClaw có thể tạm thời bỏ qua một hồ sơ nếu hồ sơ đó đang trong **thời gian chờ** ngắn (giới hạn tốc độ/timeout/lỗi xác thực) hoặc trạng thái **bị vô hiệu hóa** dài hơn (thanh toán/không đủ tín dụng). Để kiểm tra điều này, chạy `openclaw models status --json` và kiểm tra `auth.unusableProfiles`. Tinh chỉnh: `auth.cooldowns.billingBackoffHours*`.

    Thời gian chờ giới hạn tốc độ có thể theo phạm vi mô hình. Một hồ sơ đang trong thời gian chờ
    ...
    Bạn cũng có thể đặt ghi đè thứ tự **theo từng tác nhân** (được lưu trong `auth-state.json` của tác nhân đó) qua CLI:
  ...
  <Accordion title="OAuth so với khóa API - khác nhau ở điểm nào?">
    ...
    - **Đăng nhập OAuth / CLI** thường tận dụng quyền truy cập thuê bao khi
      nhà cung cấp hỗ trợ. Với Anthropic, backend Claude CLI của OpenClaw dùng
      Claude Code `claude -p`; Anthropic hiện xem đó là cách dùng Agent
      SDK/lập trình, với tín dụng Agent SDK hằng tháng riêng bắt đầu từ
      ngày 15 tháng 6 năm 2026.
    - **Khóa API** dùng hình thức thanh toán theo token.
...
- [Câu hỏi thường gặp](/vi/help/faq) — FAQ chính
- [FAQ — khởi động nhanh và thiết lập lần đầu](/vi/help/faq-first-run)
