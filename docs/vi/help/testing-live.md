---
read_when:
    - Chạy kiểm thử khói ma trận mô hình trực tiếp / phần phụ trợ CLI / ACP / nhà cung cấp phương tiện
...
x-i18n:
    generated_at: "2026-05-02T10:44:27Z"
    model: gpt-5.5
    provider: openai
...
Để bắt đầu nhanh, chạy QA, bộ kiểm thử đơn vị/tích hợp và luồng Docker, xem
...
## Trực tiếp: các lệnh kiểm thử khói hồ sơ cục bộ

Nạp `~/.profile` trước khi kiểm tra trực tiếp tùy ý để khóa nhà cung cấp và đường dẫn công cụ
...
Kiểm thử khói phương tiện an toàn:
...
Kiểm thử khói mức sẵn sàng cuộc gọi thoại an toàn:
...
Plivo, kiểm tra mức sẵn sàng thành công yêu cầu một URL webhook công khai; các
phương án dự phòng local loopback/chỉ cục bộ/riêng tư bị từ chối theo thiết kế.

## Trực tiếp: quét khả năng Node Android
...
- Phạm vi:
  - Thiết lập có điều kiện/thủ công trước (bộ kiểm thử không cài đặt/chạy/ghép cặp ứng dụng).
...
- Thiết lập trước bắt buộc:
  ...
  - Quyền/đồng ý ghi nhận đã được cấp cho các khả năng bạn kỳ vọng sẽ đạt.
...
- “Kiểm thử khói Gateway” cho biết toàn bộ pipeline gateway+tác tử có hoạt động cho mô hình đó hay không (phiên, lịch sử, công cụ, chính sách sandbox, v.v.).

### Lớp 1: Hoàn tất mô hình trực tiếp (không Gateway)
...
- Đặt `OPENCLAW_LIVE_MODELS=modern` (hoặc `all`, bí danh cho modern) để thực sự chạy bộ này; nếu không, bộ này sẽ bỏ qua để giữ `pnpm test:live` tập trung vào kiểm thử khói Gateway
- Cách chọn mô hình:
  ...
  - Các lượt quét toàn diện dùng `OPENCLAW_LIVE_TEST_TIMEOUT_MS` cho thời gian chờ của toàn bộ kiểm thử mô hình trực tiếp. Mặc định: 60 phút.
  - Các probe mô hình trực tiếp mặc định chạy song song 20 luồng; đặt `OPENCLAW_LIVE_MODEL_CONCURRENCY` để ghi đè.
...
- Lý do tồn tại:
  ...
  - Chứa các hồi quy nhỏ, cô lập (ví dụ: luồng phát lại reasoning + gọi công cụ của OpenAI Responses/Codex Responses)

### Lớp 2: Gateway + kiểm thử khói tác tử dev (điều "@openclaw" thực sự làm)
...
- Chi tiết probe (để bạn có thể giải thích lỗi nhanh):
  - probe `read`: kiểm thử ghi một file nonce trong workspace và yêu cầu tác tử `read` nó rồi echo nonce lại.
  - probe `exec+read`: kiểm thử yêu cầu tác tử `exec`-ghi một nonce vào file tạm, rồi `read` nó lại.
...
- Probe công cụ + hình ảnh luôn bật trong kiểm thử trực tiếp này:
  - probe `read` + probe `exec+read` (tải căng công cụ)
  ...
  - Luồng (cấp cao):
    ...
    - Xác nhận: câu trả lời chứa `cat` + mã (dung sai OCR: cho phép lỗi nhỏ)
...
- Mục tiêu: xác thực pipeline Gateway + tác tử bằng một backend CLI cục bộ, không chạm vào cấu hình mặc định của bạn.
- Mặc định kiểm thử khói theo từng backend nằm trong định nghĩa `cli-backend.ts` của Plugin sở hữu.
- Bật:
  - `pnpm test:live` (hoặc `OPENCLAW_LIVE_TEST=1` nếu gọi Vitest trực tiếp)
...
- Ghi đè (tùy chọn):
  ...
  - `OPENCLAW_LIVE_CLI_BACKEND_IMAGE_ARG="--image"` để truyền đường dẫn file hình ảnh dưới dạng đối số CLI thay vì chèn vào prompt.
  ...
  - `OPENCLAW_LIVE_CLI_BACKEND_MODEL_SWITCH_PROBE=1` để chọn tham gia probe tính liên tục cùng phiên Claude Sonnet -> Opus khi mô hình đã chọn hỗ trợ đích chuyển đổi. Công thức Docker mặc định tắt tùy chọn này để tăng độ tin cậy tổng hợp.
  - `OPENCLAW_LIVE_CLI_BACKEND_MCP_PROBE=1` để chọn tham gia probe local loopback MCP/công cụ. Công thức Docker mặc định tắt tùy chọn này trừ khi được yêu cầu rõ ràng.
...
Kiểm thử khói cấu hình MCP Gemini giá rẻ:
...
mà OpenClaw cung cấp cho Gemini, rồi chạy `gemini --debug mcp list` để chứng minh rằng một
máy chủ `transport: "streamable-http"` đã lưu được chuẩn hóa sang dạng MCP HTTP của Gemini
và có thể kết nối tới một máy chủ MCP streamable-HTTP cục bộ.
...
- Nó chạy kiểm thử khói backend CLI trực tiếp bên trong image Docker của repo với tư cách người dùng `node` không phải root.
- Nó phân giải metadata kiểm thử khói CLI từ Plugin sở hữu, rồi cài đặt gói CLI Linux tương ứng (`@anthropic-ai/claude-code`, `@openai/codex` hoặc `@google/gemini-cli`) vào một prefix có thể ghi được lưu cache tại `OPENCLAW_DOCKER_CLI_TOOLS_DIR` (mặc định: `~/.cache/openclaw/docker-cli-tools`).
- `pnpm test:docker:live-cli-backend:claude-subscription` yêu cầu OAuth đăng ký Claude Code di động thông qua `~/.claude/.credentials.json` với `claudeAiOauth.subscriptionType` hoặc `CLAUDE_CODE_OAUTH_TOKEN` từ `claude setup-token`. Đầu tiên nó chứng minh `claude -p` trực tiếp trong Docker, rồi chạy hai lượt backend CLI Gateway mà không giữ lại biến env khóa API Anthropic. Làn đăng ký này mặc định tắt các probe MCP/công cụ và hình ảnh của Claude vì Claude hiện định tuyến việc sử dụng ứng dụng bên thứ ba qua tính phí sử dụng bổ sung thay vì giới hạn gói đăng ký thông thường.
- Kiểm thử khói backend CLI trực tiếp hiện thực thi cùng một luồng đầu cuối cho Claude, Codex và Gemini: lượt văn bản, lượt phân loại hình ảnh, rồi gọi công cụ MCP `cron` được xác minh qua CLI Gateway.
- Kiểm thử khói mặc định của Claude cũng vá phiên từ Sonnet sang Opus và xác minh phiên đã tiếp tục vẫn nhớ một ghi chú trước đó.

## Trực tiếp: kiểm thử khói gắn ACP (`/acp spawn ... --bind here`)

- Kiểm thử: `src/gateway/gateway-acp-bind.live.test.ts`
- Mục tiêu: xác thực luồng bind cuộc hội thoại ACP thật với một agent ACP trực tiếp:
  - gửi `/acp spawn <agent> --bind here`
  - bind một cuộc hội thoại kênh tin nhắn tổng hợp tại chỗ
...
- Mặc định:
  - agent ACP trong Docker: `claude,codex,gemini`
  - agent ACP cho lệnh trực tiếp `pnpm test:live ...`: `claude`
...
- Ghi chú:
  - Lane này dùng bề mặt `chat.send` của gateway với các trường originating-route tổng hợp chỉ dành cho quản trị viên để kiểm thử có thể gắn ngữ cảnh kênh tin nhắn mà không giả vờ gửi ra bên ngoài.
  - Khi `OPENCLAW_LIVE_ACP_BIND_AGENT_COMMAND` chưa được đặt, kiểm thử dùng sổ đăng ký agent tích hợp sẵn của Plugin `acpx` nhúng cho agent harness ACP đã chọn.
  - Việc tạo MCP cron phiên đã bind mặc định là nỗ lực tối đa vì các harness ACP bên ngoài có thể hủy lệnh gọi MCP sau khi bằng chứng bind/hình ảnh đã vượt qua; đặt `OPENCLAW_LIVE_ACP_BIND_REQUIRE_CRON=1` để làm cho probe cron sau bind trở nên nghiêm ngặt.
...
Công thức Docker cho một agent:
...
- Theo mặc định, nó chạy smoke ACP bind với các agent CLI trực tiếp tổng hợp theo thứ tự: `claude`, `codex`, rồi `gemini`.
...
- Nó nạp `~/.profile`, đưa vật liệu xác thực CLI tương ứng vào container, rồi cài đặt CLI trực tiếp được yêu cầu (`@anthropic-ai/claude-code`, `@openai/codex`, Factory Droid qua `https://app.factory.ai/cli`, `@google/gemini-cli`, hoặc `opencode-ai`) nếu còn thiếu. Bản thân backend ACP là gói `acpx/runtime` nhúng từ Plugin `acpx` chính thức.
- Biến thể Docker Droid đưa `~/.factory` vào để dùng thiết lập, chuyển tiếp `FACTORY_API_KEY`, và yêu cầu khóa API đó vì xác thực OAuth/keyring cục bộ của Factory không thể chuyển vào container. Nó dùng mục sổ đăng ký tích hợp sẵn của ACPX là `droid exec --output-format acp`.
- Biến thể Docker OpenCode là một lane hồi quy nghiêm ngặt cho một agent. Nó ghi mô hình mặc định `OPENCODE_CONFIG_CONTENT` tạm thời từ `OPENCLAW_LIVE_ACP_BIND_OPENCODE_MODEL` (mặc định `opencode/kimi-k2.6`) sau khi nạp `~/.profile`, và `pnpm test:docker:live-acp-bind:opencode` yêu cầu bản ghi trợ lý đã bind thay vì chấp nhận lượt bỏ qua sau bind chung.
- Các lệnh gọi CLI `acpx` trực tiếp chỉ là đường dẫn thủ công/giải pháp tạm để so sánh hành vi bên ngoài Gateway. Smoke ACP bind Docker kiểm tra backend runtime `acpx` nhúng của OpenClaw.

## Trực tiếp: smoke harness máy chủ ứng dụng Codex
...
- Mục tiêu: xác thực harness Codex do Plugin sở hữu thông qua phương thức gateway
  ...
  - gửi lượt agent gateway đầu tiên đến `openai/gpt-5.5` với harness Codex bị ép dùng
  ...
  - tùy chọn chạy hai probe shell nâng quyền đã được Guardian xem xét: một
    lệnh vô hại nên được phê duyệt và một lượt tải lên bí mật giả nên bị
...
- Xác thực: xác thực máy chủ ứng dụng Codex từ đăng nhập gói đăng ký Codex cục bộ. Các
  smoke Docker cũng có thể cung cấp `OPENAI_API_KEY` cho các probe không phải Codex khi áp dụng,
  cùng với bản sao tùy chọn của `~/.codex/auth.json` và `~/.codex/config.toml`.
...
- Nó nạp `~/.profile` đã mount, truyền `OPENAI_API_KEY`, sao chép tệp xác thực Codex CLI
  khi có, cài đặt `@openai/codex` vào prefix npm đã mount có thể ghi,
  đưa cây mã nguồn vào, rồi chỉ chạy kiểm thử trực tiếp harness Codex.
- Docker bật các probe hình ảnh, MCP/công cụ, và Guardian theo mặc định. Đặt
  ...
  `OPENCLAW_LIVE_CODEX_HARNESS_GUARDIAN_PROBE=0` khi bạn cần lượt chạy gỡ lỗi
  hẹp hơn.
- Docker cũng xuất `OPENCLAW_AGENT_HARNESS_FALLBACK=none`, khớp với cấu hình
  kiểm thử trực tiếp để các bí danh cũ hoặc fallback PI không thể che giấu hồi quy
  harness Codex.

### Công thức trực tiếp được khuyến nghị

Danh sách cho phép hẹp, rõ ràng là nhanh nhất và ít flaky nhất:
...
- `google-gemini-cli/...` dùng Gemini CLI cục bộ trên máy của bạn (xác thực riêng + khác biệt công cụ).
- Gemini API so với Gemini CLI:
  - API: OpenClaw gọi Gemini API được Google lưu trữ qua HTTP (khóa API / xác thực hồ sơ); đây là ý nghĩa mà hầu hết người dùng nói đến khi nhắc “Gemini”.
  - CLI: OpenClaw gọi ra một binary `gemini` cục bộ; nó có xác thực riêng và có thể hoạt động khác đi (hỗ trợ streaming/công cụ/lệch phiên bản).
...
Bao gồm ít nhất một mô hình có khả năng xử lý hình ảnh trong `OPENCLAW_LIVE_GATEWAY_MODELS` (các biến thể Claude/Gemini/OpenAI có khả năng vision, v.v.) để kiểm tra probe hình ảnh.
...
Những nhà cung cấp khác bạn có thể đưa vào ma trận trực tiếp (nếu có thông tin xác thực/cấu hình):
...
<Tip>
Không hardcode "tất cả mô hình" trong tài liệu. Danh sách có thẩm quyền là bất cứ thứ gì `discoverModels(...)` trả về trên máy của bạn cộng với các khóa hiện có.
</Tip>

## Thông tin xác thực (không bao giờ commit)

Kiểm thử trực tiếp phát hiện thông tin xác thực giống như cách CLI làm. Hệ quả thực tế:

- Nếu CLI hoạt động, các kiểm thử live sẽ tìm thấy cùng các khóa đó.
- Nếu một kiểm thử live báo “không có thông tin xác thực”, hãy gỡ lỗi giống như cách bạn gỡ lỗi `openclaw models list` / lựa chọn mô hình.

- Hồ sơ xác thực theo từng agent: `~/.openclaw/agents/<agentId>/agent/auth-profiles.json` (đây là ý nghĩa của “khóa hồ sơ” trong các kiểm thử live)
- Cấu hình: `~/.openclaw/openclaw.json` (hoặc `OPENCLAW_CONFIG_PATH`)
- Thư mục trạng thái cũ: `~/.openclaw/credentials/` (được sao chép vào home live được dàn dựng khi có, nhưng không phải là kho khóa hồ sơ chính)
- Các lần chạy live cục bộ mặc định sao chép cấu hình đang hoạt động, các tệp `auth-profiles.json` theo từng agent, `credentials/` cũ, và các thư mục xác thực CLI bên ngoài được hỗ trợ vào một home kiểm thử tạm thời; các home live được dàn dựng bỏ qua `workspace/` và `sandboxes/`, đồng thời các ghi đè đường dẫn `agents.*.workspace` / `agentDir` bị loại bỏ để các phép thăm dò không chạm vào workspace thật trên máy chủ của bạn.
...
## BytePlus live cho kế hoạch lập trình
...
## ComfyUI live cho media workflow
...
- Phạm vi:
  - Thực thi các đường dẫn ảnh, video và `music_generate` của comfy được đóng gói
...
- Phạm vi:
  - Liệt kê mọi Plugin nhà cung cấp tạo ảnh đã đăng ký
  - Tải các biến env nhà cung cấp còn thiếu từ login shell của bạn (`~/.profile`) trước khi thăm dò
  - Mặc định dùng khóa API live/env trước các hồ sơ xác thực đã lưu, để các khóa kiểm thử lỗi thời trong `auth-profiles.json` không che khuất thông tin xác thực shell thật
  - Bỏ qua các nhà cung cấp không có auth/profile/model dùng được
  - Chạy từng nhà cung cấp đã cấu hình qua runtime tạo ảnh dùng chung:
    - `<provider>:generate`
...
- Hành vi xác thực tùy chọn:
  - `OPENCLAW_LIVE_REQUIRE_PROFILE_KEYS=1` để buộc xác thực bằng kho hồ sơ và bỏ qua các ghi đè chỉ từ env

Đối với đường dẫn CLI được phát hành, hãy thêm một smoke `infer` sau khi kiểm thử live
...
Nội dung này bao phủ việc phân tích đối số CLI, phân giải cấu hình/default-agent, kích hoạt
Plugin đóng gói, runtime tạo ảnh dùng chung và yêu cầu nhà cung cấp live.
Các phụ thuộc Plugin được kỳ vọng đã có trước khi runtime load.
...
- Phạm vi:
  ...
  - Tải biến env nhà cung cấp từ login shell của bạn (`~/.profile`) trước khi thăm dò
  - Mặc định dùng khóa API live/env trước các hồ sơ xác thực đã lưu, để các khóa kiểm thử lỗi thời trong `auth-profiles.json` không che khuất thông tin xác thực shell thật
  - Bỏ qua các nhà cung cấp không có auth/profile/model dùng được
...
- Phạm vi:
  ...
  - Mặc định dùng đường dẫn smoke an toàn cho phát hành: các nhà cung cấp không phải FAL, một yêu cầu text-to-video cho mỗi nhà cung cấp, prompt lobster một giây, và giới hạn thao tác theo từng nhà cung cấp từ `OPENCLAW_LIVE_VIDEO_GENERATION_TIMEOUT_MS` (mặc định là `180000`)
  ...
  - Đặt `OPENCLAW_LIVE_VIDEO_GENERATION_FULL_MODES=1` để cũng chạy các chế độ transform đã khai báo khi có:
    - `imageToVideo` khi nhà cung cấp khai báo `capabilities.imageToVideo.enabled` và nhà cung cấp/mô hình đã chọn chấp nhận đầu vào ảnh cục bộ dựa trên buffer trong lượt quét dùng chung
  ...
  - Các nhà cung cấp `imageToVideo` đã khai báo nhưng bị bỏ qua hiện tại trong lượt quét dùng chung:
    - `vydra` vì `veo3` đóng gói chỉ hỗ trợ text và `kling` đóng gói yêu cầu URL ảnh từ xa
  - Phạm vi bao phủ riêng cho Vydra theo nhà cung cấp:
    - `OPENCLAW_LIVE_TEST=1 OPENCLAW_LIVE_VYDRA_VIDEO=1 pnpm test:live -- extensions/vydra/vydra.live.test.ts`
    - tệp đó chạy text-to-video `veo3` cùng một lane `kling` mặc định dùng fixture URL ảnh từ xa
  ...
  - Các nhà cung cấp `videoToVideo` đã khai báo nhưng bị bỏ qua hiện tại trong lượt quét dùng chung:
    ...
    - `openai` vì lane dùng chung hiện tại thiếu bảo đảm quyền truy cập video inpaint/remix theo từng tổ chức
- Thu hẹp tùy chọn:
  ...
  - `OPENCLAW_LIVE_VIDEO_GENERATION_TIMEOUT_MS=60000` để giảm giới hạn từng thao tác của nhà cung cấp cho một lần chạy smoke nhanh
- Hành vi xác thực tùy chọn:
  - `OPENCLAW_LIVE_REQUIRE_PROFILE_KEYS=1` để buộc xác thực bằng kho hồ sơ và bỏ qua các ghi đè chỉ từ env
...
- Mục đích:
  - Chạy các bộ live dùng chung cho ảnh, nhạc và video thông qua một entrypoint gốc repo
  - Tự động tải các biến env nhà cung cấp còn thiếu từ `~/.profile`
  - Mặc định tự động thu hẹp từng bộ xuống các nhà cung cấp hiện có auth dùng được
  - Tái sử dụng `scripts/test-live.mjs`, nên hành vi Heartbeat và chế độ yên lặng vẫn nhất quán
- Ví dụ:
