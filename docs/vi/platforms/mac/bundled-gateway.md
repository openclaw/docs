...
đang chạy (hoặc gắn vào một Gateway cục bộ hiện có nếu Gateway đó đã chạy).

## Cài đặt CLI (bắt buộc cho chế độ cục bộ)

Node 24 là runtime mặc định trên Mac. Node 22 LTS, hiện là `22.14+`, vẫn hoạt động để tương thích. Sau đó cài đặt `openclaw` ở phạm vi toàn cục:
...
sử dụng nội bộ: nó ưu tiên npm trước, sau đó là pnpm, rồi bun nếu đó là trình quản lý
gói duy nhất được phát hiện. Node vẫn là runtime Gateway được khuyến nghị.
...
- Ứng dụng macOS sở hữu việc cài đặt/cập nhật LaunchAgent trong chế độ Local.
...
- Nếu một Gateway đã chạy trên cổng được cấu hình, ứng dụng sẽ gắn vào
