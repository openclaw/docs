---
read_when:
    - Bạn muốn điều chỉnh thông tin xác thực, thiết bị hoặc các giá trị mặc định của tác nhân một cách tương tác
...
Đối với tìm kiếm web, `openclaw configure --section web` cho phép bạn chọn nhà cung cấp
và cấu hình thông tin xác thực của họ. Một số nhà cung cấp cũng hiển thị các lời nhắc
...
- Sau khi ghi cấu hình cục bộ, configure sẽ hiện thực hóa các phụ thuộc runtime Plugin đóng gói mới được yêu cầu. Đây là bước sửa chữa trình quản lý gói phạm vi hẹp, không phải một lần chạy `openclaw doctor` đầy đủ. Cấu hình Gateway từ xa không cài đặt các phụ thuộc Plugin cục bộ.
...
- Nếu bạn chạy bước cài đặt daemon, xác thực token yêu cầu token và `gateway.auth.token` được quản lý bằng SecretRef, configure sẽ xác thực SecretRef nhưng không lưu các giá trị token văn bản thuần đã phân giải vào siêu dữ liệu môi trường của dịch vụ supervisor.
- Nếu xác thực token yêu cầu token và SecretRef token đã cấu hình chưa được phân giải, configure sẽ chặn cài đặt daemon với hướng dẫn khắc phục có thể thực hiện.
