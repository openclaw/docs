...
summary: Bổ sung hỗ trợ nhà cung cấp mô hình Microsoft Foundry cho OpenClaw.
title: Plugin Microsoft Foundry
x-i18n:
    generated_at: "2026-07-16T15:39:12Z"
    model: gpt-5.6
...
- Các họ mô hình GPT, `o*`, `computer-use-preview` và DeepSeek-V4 mặc định sử dụng
  `openai-responses`.
- MAI-DS-R1 và các bản triển khai hoàn thành trò chuyện khác sử dụng `openai-completions`,
...
- MAI-DS-R1 được ghi nhận là có khả năng suy luận thông qua nội dung suy luận, không phải
  thông qua `reasoning_effort`. Siêu dữ liệu token ngữ cảnh và đầu ra của mô hình này là
  163,840 token.

Các bản triển khai Anthropic Claude trong Microsoft Foundry sử dụng định dạng API
Anthropic Messages, không phải định dạng `/openai/v1` tương thích với OpenAI. Hãy cấu hình chúng dưới dạng
nhà cung cấp `anthropic-messages` tùy chỉnh cho đến khi Plugin Microsoft Foundry có
thời gian chạy Anthropic gốc. Khi tên bản triển khai Foundry khác với
ID mô hình Claude, hãy đặt `params.canonicalModelId` trong mục nhập mô hình để OpenClaw
có thể áp dụng các hợp đồng giao tiếp dành riêng cho mô hình, ánh xạ `/think off` chính xác và
bảo toàn phần suy nghĩ đã ký một cách an toàn.
...
Sử dụng tên của một bản triển khai hình ảnh MAI đã triển khai làm tham chiếu mô hình. Nhà cung cấp
không khai báo mô hình hình ảnh mặc định vì API MAI yêu cầu tên bản triển khai
của bạn trong trường `model` của yêu cầu:
...
Quá trình tạo chỉ bằng lời nhắc gọi điểm cuối tạo MAI của Microsoft Foundry:
`/mai/v1/images/generations`. Các thao tác chỉnh sửa bằng hình ảnh tham chiếu gọi
`/mai/v1/images/edits` và chỉ giới hạn ở các bản triển khai `MAI-Image-2.5-Flash` và
`MAI-Image-2.5`.

Quá trình tạo chỉ bằng lời nhắc có thể sử dụng tên bản triển khai tùy chỉnh khi chỉ cần cấu hình
...
- Kích thước: mặc định `1024x1024`; cả chiều rộng và chiều cao phải ít nhất là 768 px.
- Tổng số pixel: chiều rộng × chiều cao phải tối đa là 1,048,576.
...
- `supports MAI image deployments only`: mô hình hình ảnh đã chọn trỏ đến một
  bản triển khai không phải MAI. Sử dụng mô hình hình ảnh MAI đã triển khai cho `image_generate`.
