---
read_when:
    - Bạn đang cài đặt, cấu hình hoặc kiểm tra Plugin huggingface
summary: Thêm hỗ trợ nhà cung cấp mô hình Hugging Face cho OpenClaw.
title: Plugin Hugging Face
x-i18n:
    generated_at: "2026-05-02T20:51:21Z"
    model: gpt-5.5
    provider: openai
    source_hash: d3f400aed24e630718ce5977e417ff47a8a985a0a385942e1ee6224d830b53e6
    source_path: plugins/reference/huggingface.md
    workflow: 16
    postprocess_version: locale-links-v1
---

# Plugin Hugging Face

Thêm hỗ trợ nhà cung cấp mô hình Hugging Face cho OpenClaw.

## Phân phối

- Gói: `@openclaw/huggingface-provider`
- Tuyến cài đặt: được bao gồm trong OpenClaw

## Bề mặt

providers: huggingface

## Tài liệu liên quan

- [huggingface](/vi/providers/huggingface)
