...
title: xAI
x-i18n:
    generated_at: "2026-05-06T09:28:23Z"
    model: gpt-5.5
    provider: openai
...
<Steps>
  <Step title="Tạo khóa API">
    Tạo một khóa API trong [bảng điều khiển xAI](https://console.x.ai/).
  </Step>
...
<Note>
OpenClaw sử dụng xAI Responses API làm phương tiện truyền tải xAI đi kèm. Cùng một
`XAI_API_KEY` cũng có thể cung cấp năng lực cho `web_search` dựa trên Grok, `x_search`
hạng nhất, và `code_execution` từ xa.
Nếu bạn lưu khóa xAI dưới `plugins.entries.xai.config.webSearch.apiKey`,
nhà cung cấp mô hình xAI đi kèm cũng tái sử dụng khóa đó làm phương án dự phòng.
Đặt `plugins.entries.xai.config.webSearch.baseUrl` để định tuyến `web_search` của Grok
và, theo mặc định, `x_search` qua một proxy xAI Responses của người vận hành.
...
Plugin này cũng phân giải chuyển tiếp các ID `grok-4*` và `grok-code-fast*` mới hơn khi
chúng tuân theo cùng một dạng API.

<Tip>
`grok-4.3`, `grok-4-fast`, `grok-4-1-fast`, và các biến thể `grok-4.20-beta-*`
là các tham chiếu Grok hiện hỗ trợ hình ảnh trong danh mục đi kèm.
</Tip>

## Phạm vi tính năng của OpenClaw

Plugin đi kèm ánh xạ bề mặt API công khai hiện tại của xAI vào các hợp đồng
nhà cung cấp và công cụ dùng chung của OpenClaw. Những năng lực không khớp với hợp đồng dùng chung
...
cuộc gọi thoại trực tiếp, và Responses API cho các công cụ mô hình, tìm kiếm, và
thực thi mã. Những tính năng cần các hợp đồng OpenClaw khác, chẳng hạn như
phiên giọng nói thời gian thực, được ghi tài liệu ở đây dưới dạng năng lực thượng nguồn
...
<AccordionGroup>
  ...
  <Accordion title="Tạo video">
    ...
    - Thời lượng: 1-15 giây cho tạo/hình ảnh thành video, 1-10 giây khi
      sử dụng vai trò `reference_image`, 2-10 giây cho mở rộng
    ...
    <Warning>
    Bộ đệm video cục bộ không được chấp nhận. Sử dụng URL `http(s)` từ xa cho
  ...
  <Accordion title="Tạo hình ảnh">
    ...
    lưu trữ và phân phối qua đường dẫn tệp đính kèm kênh thông thường. Hình ảnh tham chiếu
    cục bộ được chuyển đổi thành URL dữ liệu; tham chiếu `http(s)` từ xa được
    chuyển qua nguyên trạng.
    ...
    điều khiển hình ảnh dùng chung giữa các nhà cung cấp; các núm điều chỉnh chỉ có ở gốc không được hỗ trợ
    được chủ ý không hiển thị qua `image_generate`.
    </Note>
  ...
  <Accordion title="Chuyển văn bản thành giọng nói">
    ...
    qua WebSocket, nhưng hợp đồng nhà cung cấp giọng nói của OpenClaw hiện mong đợi
    một bộ đệm âm thanh hoàn chỉnh trước khi phân phối phản hồi.
    </Note>
  ...
  <Accordion title="Chuyển giọng nói thành văn bản">
    ...
    phiên âm theo từng lần gọi. Gợi ý prompt được bề mặt OpenClaw dùng chung
  ...
  <Accordion title="Chuyển giọng nói thành văn bản phát trực tuyến">
    ...
    `plugins.entries.voice-call.config.streaming.providers.xai`. Các khóa được
    hỗ trợ là `apiKey`, `baseUrl`, `sampleRate`, `encoding` (`pcm`, `mulaw`, hoặc
    ...
    Giọng nói Discord hiện ghi các đoạn ngắn và dùng đường dẫn phiên âm hàng loạt
    `tools.media.audio` thay vào đó.
    </Note>

  </Accordion>

  <Accordion title="Cấu hình x_search">
    Plugin xAI đi kèm hiển thị `x_search` dưới dạng công cụ OpenClaw để tìm kiếm
  ...
  <Accordion title="Cấu hình thực thi mã">
    Plugin xAI đi kèm hiển thị `code_execution` dưới dạng công cụ OpenClaw để
  ...
  <Accordion title="Ghi chú nâng cao">
    ...
    - Các yêu cầu xAI native mặc định dùng `tool_stream: true`. Đặt
    ...
    - Grok `web_search` đọc `plugins.entries.xai.config.webSearch.baseUrl`.
      ...
      quay về URL cơ sở tìm kiếm web của Grok.
    - `x_search` và `code_execution` thuộc sở hữu của Plugin xAI đi kèm thay vì được mã hóa cứng vào runtime model lõi.
