...
<Warning>
Máy khách Anthropic của OpenClaw tự động nối thêm `/v1` vào URL cơ sở, vì vậy hãy dùng
...
Tất cả mô hình Synthetic đều có chi phí `0` (đầu vào/đầu ra/bộ nhớ đệm).
...
`openclaw models list --provider synthetic` để xem tất cả mô hình có sẵn trong
tài khoản của bạn.
</Tip>

<AccordionGroup>
  <Accordion title="Danh sách mô hình được phép">
    Nếu bạn bật danh sách mô hình được phép (`agents.defaults.models`), hãy thêm mọi
