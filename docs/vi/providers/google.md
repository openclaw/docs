...
summary: Thiết lập Google Gemini (khóa API + OAuth, tạo hình ảnh, hiểu phương tiện, TTS, tìm kiếm web)
title: Google (Gemini)
x-i18n:
...
<Tabs>
  ...
  <Tab title="Gemini CLI (OAuth)">
    **Phù hợp nhất cho:** tái sử dụng phiên đăng nhập Gemini CLI hiện có qua PKCE OAuth thay vì dùng một API key riêng.
    ...
    báo cáo có hạn chế tài khoản khi dùng OAuth theo cách này. Bạn tự chịu rủi ro khi sử dụng.
    </Warning>
    ...
    <Steps>
      <Step title="Cài đặt Gemini CLI">
        ...
        OpenClaw hỗ trợ cả cài đặt Homebrew và cài đặt npm toàn cục, bao gồm
...
tồn tại cho proxy của người vận hành hoặc các endpoint Gemini API tương thích; khi bị bỏ qua,
...
các điều khiển lập luận bí danh của Gemini 3, Gemini 3.1 và `gemini-*-latest` sang
...
Đường dẫn batch Gemini TTS của Google trả về âm thanh được tạo trong phản hồi
`generateContent` đã hoàn tất. Để có các cuộc trò chuyện nói với độ trễ thấp nhất, hãy dùng
nhà cung cấp giọng nói thời gian thực của Google được hỗ trợ bởi Gemini Live API thay vì batch
TTS.
...
Gemini API TTS dùng lời nhắc ngôn ngữ tự nhiên để kiểm soát phong cách. Đặt
`audioProfile` để thêm trước một lời nhắc phong cách có thể tái sử dụng trước văn bản được đọc. Đặt
...
chẳng hạn như `[whispers]` hoặc `[laughs]`. Để giữ các thẻ không xuất hiện trong phản hồi chat hiển thị
...
<Note>
API key trong Google Cloud Console bị giới hạn cho Gemini API là hợp lệ với
...
Phiên âm Gemini API được bật mà không có `languageCodes`; Google
...
bị ràng buộc. Các nhà cung cấp giọng nói thời gian thực chỉ chạy ở backend cũng có thể chạy qua
giao thức vận chuyển chuyển tiếp Gateway chung, giúp giữ thông tin xác thực của nhà cung cấp trên Gateway.
</Note>

Để xác minh trực tiếp cho maintainer, chạy
`OPENAI_API_KEY=... GEMINI_API_KEY=... node --import tsx scripts/dev/realtime-talk-live-smoke.ts`.
Bài smoke cũng bao phủ các đường dẫn backend/WebRTC của OpenAI; nhánh Google tạo cùng
dạng token Live API bị ràng buộc mà Control UI Talk dùng, mở endpoint
WebSocket của trình duyệt, gửi payload thiết lập ban đầu, và chờ
`setupComplete`.
...
<AccordionGroup>
  <Accordion title="Direct Gemini cache reuse">
    ...
    chuyển một handle `cachedContent` đã cấu hình vào các yêu cầu Gemini.
  ...
  <Accordion title="Gemini CLI usage notes">
    Khi dùng nhà cung cấp OAuth `google-gemini-cli`, OpenClaw dùng đầu ra
    `stream-json` của Gemini CLI theo mặc định và chuẩn hóa mức sử dụng từ payload
    `stats` cuối cùng. Các override `--output-format json` cũ vẫn dùng
