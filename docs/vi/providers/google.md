...
summary: Thiết lập Google Gemini (khóa API + OAuth, tạo hình ảnh, hiểu nội dung đa phương tiện, chuyển văn bản thành giọng nói, tìm kiếm trên web)
title: Google (Gemini)
x-i18n:
    generated_at: "2026-05-02T10:50:35Z"
    model: gpt-5.5
    provider: openai
...
<Tabs>
  ...
  <Tab title="Gemini CLI (OAuth)">
    **Phù hợp nhất cho:** tái sử dụng thông tin đăng nhập Gemini CLI hiện có qua PKCE OAuth thay vì một khóa API riêng.
    ...
    **Biến môi trường:**
    ...
    `GOOGLE_CLOUD_PROJECT_ID` trên máy chủ gateway và thử lại.
    </Note>
...
tồn tại cho proxy vận hành hoặc các endpoint tương thích Gemini API; khi bị bỏ qua,
...
ghi lại `thinkingBudget` thành `thinkingLevel` được Google hỗ trợ cho Gemma 4.
...
<Note>
Xem [Tạo hình ảnh](/vi/tools/image-generation) để biết tham số công cụ dùng chung, lựa chọn nhà cung cấp và hành vi chuyển dự phòng.
</Note>
...
- Chế độ: luồng văn bản sang video, hình ảnh sang video và tham chiếu một video
...
<Note>
Xem [Tạo video](/vi/tools/video-generation) để biết tham số công cụ dùng chung, lựa chọn nhà cung cấp và hành vi chuyển dự phòng.
</Note>
...
- Các lần chạy dựa trên phiên sẽ tách ra thông qua luồng tác vụ/trạng thái dùng chung, bao gồm `action: "status"`
...
<Note>
Xem [Tạo nhạc](/vi/tools/music-generation) để biết tham số công cụ dùng chung, lựa chọn nhà cung cấp và hành vi chuyển dự phòng.
</Note>
...
Gemini API TTS dùng lời nhắc ngôn ngữ tự nhiên để điều khiển phong cách. Đặt
`audioProfile` để thêm lời nhắc phong cách có thể tái sử dụng trước văn bản được đọc. Đặt
...
chẳng hạn như `[whispers]` hoặc `[laughs]`. Để không hiển thị thẻ trong phản hồi trò chuyện
nhưng vẫn gửi chúng đến TTS, hãy đặt chúng bên trong khối `[[tts:text]]...[[/tts:text]]`:
...
<Note>
Khóa API của Google Cloud Console bị giới hạn cho Gemini API hợp lệ với nhà
cung cấp này. Đây không phải là đường dẫn Cloud Text-to-Speech API riêng.
</Note>
...
vì Google Live có thể trả về bản chép lời mà không có âm thanh cho `temperature: 0`.
...
<Note>
Control UI Talk hỗ trợ các phiên Google Live trên trình duyệt với token dùng một lần
có ràng buộc. Các nhà cung cấp giọng nói thời gian thực chỉ chạy ở backend cũng có thể chạy qua
phương thức truyền relay Gateway chung, giữ thông tin xác thực của nhà cung cấp trên Gateway.
</Note>

Để kiểm chứng trực tiếp dành cho maintainer, chạy
`OPENAI_API_KEY=... GEMINI_API_KEY=... node --import tsx scripts/dev/realtime-talk-live-smoke.ts`.
Nhánh Google tạo cùng dạng token Live API có ràng buộc mà Control
...
<AccordionGroup>
  <Accordion title="Direct Gemini cache reuse">
    ...
    - Nếu cả hai cùng có mặt, `cachedContent` được ưu tiên
  ...
  <Accordion title="Gemini CLI JSON usage notes">
    Khi sử dụng nhà cung cấp OAuth `google-gemini-cli`, OpenClaw chuẩn hóa
