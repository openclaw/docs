...
title: Google (Gemini)
x-i18n:
    generated_at: "2026-05-07T13:23:57Z"
    model: gpt-5.5
    provider: openai
...
tạo hình ảnh, hiểu phương tiện (hình ảnh/âm thanh/video), chuyển văn bản thành giọng nói và tìm kiếm web qua
Gemini Grounding.
...
- Tùy chọn runtime: `agents.defaults.agentRuntime.id: "google-gemini-cli"`
  tái sử dụng OAuth của Gemini CLI trong khi vẫn giữ tham chiếu mô hình theo dạng chính tắc là `google/*`.
...
<Tabs>
  ...
  <Tab title="Gemini CLI (OAuth)">
    **Phù hợp nhất cho:** tái sử dụng phiên đăng nhập Gemini CLI hiện có qua PKCE OAuth thay vì dùng một khóa API riêng.
    ...
    báo cáo việc tài khoản bị hạn chế khi dùng OAuth theo cách này. Hãy tự chịu rủi ro khi sử dụng.
    </Warning>
    ...
    **Biến môi trường:**
    ...
    Tham chiếu mô hình `google-gemini-cli/*` là các bí danh tương thích kế thừa. Cấu hình
...
Nhà cung cấp tìm kiếm web `gemini` đi kèm sử dụng grounding của Gemini Google Search.
...
tồn tại cho proxy của nhà vận hành hoặc endpoint Gemini API tương thích; khi bị bỏ qua,
...
viết lại `thinkingBudget` thành một `thinkingLevel` được Google hỗ trợ cho Gemma 4.
Đặt suy nghĩ thành `off` sẽ giữ trạng thái suy nghĩ bị vô hiệu hóa thay vì ánh xạ sang
`MINIMAL`.
</Tip>
...
<Note>
Xem [Tạo hình ảnh](/vi/tools/image-generation) để biết tham số công cụ dùng chung, lựa chọn nhà cung cấp và hành vi chuyển đổi khi lỗi.
</Note>
...
<Note>
Xem [Tạo video](/vi/tools/video-generation) để biết tham số công cụ dùng chung, lựa chọn nhà cung cấp và hành vi chuyển đổi khi lỗi.
</Note>
...
<Note>
Xem [Tạo nhạc](/vi/tools/music-generation) để biết tham số công cụ dùng chung, lựa chọn nhà cung cấp và hành vi chuyển đổi khi lỗi.
</Note>

## Chuyển văn bản thành giọng nói

Nhà cung cấp giọng nói `google` đi kèm sử dụng đường dẫn Gemini API TTS với
...
`generateContent` hoàn tất. Để có các cuộc trò chuyện nói với độ trễ thấp nhất, hãy dùng
nhà cung cấp giọng nói thời gian thực Google được hỗ trợ bởi Gemini Live API thay vì TTS
theo lô.
...
`audioProfile` để thêm một lời nhắc phong cách có thể tái sử dụng vào trước văn bản được đọc. Đặt
...
chẳng hạn như `[whispers]` hoặc `[laughs]`. Để giữ các thẻ ngoài phản hồi trò chuyện hiển thị
trong khi vẫn gửi chúng tới TTS, hãy đặt chúng bên trong một khối `[[tts:text]]...[[/tts:text]]`:
...
<Note>
Khóa API Google Cloud Console bị giới hạn cho Gemini API là hợp lệ với
...
OpenClaw điều chỉnh âm thanh cầu nối điện thoại/Meet sang luồng Gemini PCM Live API và
...
<Note>
Control UI Talk hỗ trợ các phiên trình duyệt Google Live với token dùng một lần có giới hạn.
Các nhà cung cấp giọng nói thời gian thực chỉ chạy ở backend cũng có thể chạy qua cơ chế truyền tiếp
Gateway chung, cơ chế này giữ thông tin xác thực của nhà cung cấp trên Gateway.
</Note>

Để kiểm chứng trực tiếp dành cho maintainer, chạy
`OPENAI_API_KEY=... GEMINI_API_KEY=... node --import tsx scripts/dev/realtime-talk-live-smoke.ts`.
Nhánh Google tạo cùng dạng token Live API có giới hạn mà Control
...
<AccordionGroup>
  <Accordion title="Tái sử dụng trực tiếp bộ nhớ đệm Gemini">
    ...
    truyền handle `cachedContent` đã cấu hình vào các yêu cầu Gemini.
    ...
    - Mức sử dụng trúng bộ nhớ đệm Gemini được chuẩn hóa vào OpenClaw `cacheRead` từ
  ...
  <Accordion title="Môi trường và thiết lập daemon">
