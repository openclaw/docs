...
Plugin Google cung cấp quyền truy cập vào các mô hình Gemini thông qua Google AI Studio, cùng với tính năng tạo hình ảnh, hiểu nội dung đa phương tiện (hình ảnh/âm thanh/video), chuyển văn bản thành giọng nói và tìm kiếm web qua Gemini Grounding.

- Nhà cung cấp: `google`
- Xác thực: `GEMINI_API_KEY` hoặc `GOOGLE_API_KEY`
- API: Google Gemini API
- Tùy chọn runtime: `agentRuntime.id: "google-gemini-cli"` tái sử dụng OAuth của Gemini CLI trong khi vẫn giữ tham chiếu mô hình ở dạng chuẩn là `google/*`.
...
<Tabs>
  <Tab title="Khóa API">
    **Phù hợp nhất cho:** quyền truy cập Gemini API tiêu chuẩn thông qua Google AI Studio.
    ...
    <Tip>
    Cả `GEMINI_API_KEY` và `GOOGLE_API_KEY` đều được chấp nhận. Hãy dùng biến mà bạn đã cấu hình.
    </Tip>

  </Tab>

  <Tab title="Gemini CLI (OAuth)">
    **Phù hợp nhất cho:** đăng nhập bằng tài khoản Google thông qua OAuth của Gemini CLI thay vì sử dụng khóa API riêng.
    ...
    <Steps>
      <Step title="Cài đặt Gemini CLI">
        ...
        OpenClaw hỗ trợ cả bản cài đặt bằng Homebrew và bản cài đặt npm toàn cục, bao gồm
    ...
    **Biến môi trường:**
    ...
    <Note>
    Nếu đăng nhập thất bại trước khi quy trình trình duyệt bắt đầu, hãy đảm bảo lệnh `gemini`
    cục bộ đã được cài đặt và nằm trên `PATH`.
    </Note>

    Tính năng tự động phát hiện trong quy trình thiết lập ban đầu sẽ liệt kê phiên đăng nhập Gemini CLI hiện có nhưng không bao giờ
    tự động kiểm tra phiên đó vì Gemini CLI không có cơ chế thăm dò không dùng công cụ. Chọn OAuth của Gemini CLI
    hoặc khóa Gemini API để tiếp tục.

    Các tham chiếu mô hình `google-gemini-cli/*` là bí danh tương thích cũ. Cấu hình
    mới nên sử dụng tham chiếu mô hình `google/*` cùng runtime `google-gemini-cli`
...
<Note>
`google/gemini-3-pro-preview` đã ngừng hoạt động vào 2026-03-09; hãy dùng `google/gemini-3.1-pro-preview` thay thế. Chạy lại quy trình thiết lập khóa Gemini API (`openclaw onboard --auth-choice gemini-api-key` hoặc `openclaw models auth login --provider google`) sẽ ghi lại mô hình mặc định đã cấu hình nhưng lỗi thời thành mô hình hiện tại.
</Note>
...
Nhà cung cấp tìm kiếm web `gemini` đi kèm sử dụng tính năng Grounding với Google Search của Gemini.
Cấu hình khóa tìm kiếm chuyên dụng tại `plugins.entries.google.config.webSearch`,
hoặc cho phép khóa đó tái sử dụng `models.providers.google.apiKey` sau `GEMINI_API_KEY`:
...
dành cho proxy của nhà vận hành hoặc các điểm cuối tương thích với Gemini API; khi bỏ qua,
...
`thinkingLevel` để các lần chạy mặc định/độ trễ thấp không gửi
giá trị `thinkingBudget` đã bị vô hiệu hóa.

`/think adaptive` giữ nguyên ngữ nghĩa suy nghĩ động của Google thay vì chọn
một cấp độ OpenClaw cố định. Gemini 3 và Gemini 3.1 bỏ qua `thinkingLevel` cố định để
Google có thể chọn cấp độ; Gemini 2.5 gửi giá trị sentinel động
`thinkingBudget: -1` của Google.

Các mô hình Gemma 4 (ví dụ `gemma-4-26b-a4b-it`) hỗ trợ chế độ suy nghĩ. OpenClaw
ghi lại `thinkingBudget` thành một `thinkingLevel` được Google hỗ trợ cho Gemma 4.
Đặt chế độ suy nghĩ thành `off` sẽ giữ trạng thái suy nghĩ bị vô hiệu hóa thay vì ánh xạ sang
`MINIMAL`.

Gemini 2.5 Pro chỉ hoạt động ở chế độ suy nghĩ và từ chối
...
Plugin `google` đi kèm cũng đăng ký tính năng tạo video thông qua
công cụ `video_generate` dùng chung.

- Mô hình video mặc định: `google/veo-3.1-fast-generate-preview`
- Chế độ: chuyển văn bản thành video, chuyển hình ảnh thành video và quy trình tham chiếu một video
- Hỗ trợ `aspectRatio` (`16:9`, `9:16`) và `resolution` (`720P`, `1080P`); Veo hiện không hỗ trợ đầu ra âm thanh
- Thời lượng được hỗ trợ: **4, 6 hoặc 8 giây** (các giá trị khác được điều chỉnh thành giá trị được phép gần nhất)
...
Plugin `google` đi kèm cũng đăng ký tính năng tạo nhạc thông qua
công cụ `music_generate` dùng chung.

- Mô hình âm nhạc mặc định: `google/lyria-3-clip-preview`
- Cũng hỗ trợ `google/lyria-3-pro-preview`
- Điều khiển câu lệnh: `lyrics` và `instrumental`
- Định dạng đầu ra: mặc định là `mp3`, cùng với `wav` trên `google/lyria-3-pro-preview`
- Đầu vào tham chiếu: tối đa 10 hình ảnh
- Các lần chạy có phiên hỗ trợ sẽ tách ra thông qua quy trình tác vụ/trạng thái dùng chung, bao gồm `action: "status"`

Để sử dụng Google làm nhà cung cấp âm nhạc mặc định:
...
`generateContent` hoàn tất. Để có các cuộc hội thoại bằng giọng nói với độ trễ thấp nhất, hãy sử dụng
    ...
    tts: {
      ...
      providers: {
        google: {
          ...
          speakerVoice: "Kore",
          audioProfile: "Nói một cách chuyên nghiệp với giọng điệu điềm tĩnh.",
        },
      },
    },
  },
...
Gemini API TTS sử dụng câu lệnh ngôn ngữ tự nhiên để kiểm soát phong cách. Đặt
`audioProfile` để thêm một câu lệnh phong cách có thể tái sử dụng trước văn bản được đọc. Đặt
`speakerName` khi văn bản câu lệnh đề cập đến một người nói có tên.
...
<Note>
Khóa API Google Cloud Console bị giới hạn ở Gemini API vẫn hợp lệ cho
...
duy trì các lệnh gọi công cụ trên hợp đồng giọng nói thời gian thực dùng chung. Để `temperature`
ở trạng thái chưa đặt trừ khi bạn cần thay đổi việc lấy mẫu; OpenClaw bỏ qua các giá trị không dương
vì Google Live có thể trả về bản chép lời mà không có âm thanh đối với `temperature: 0`.
Tính năng chép lời của Gemini API được bật mà không cần `languageCodes`; SDK Google hiện tại
từ chối gợi ý mã ngôn ngữ trên đường dẫn API này.
</Note>

<Note>
Gemini 3.1 Live chấp nhận văn bản hội thoại qua đầu vào thời gian thực và sử dụng
cơ chế gọi hàm tuần tự. OpenClaw bỏ qua `NON_BLOCKING` cũ, việc lập lịch
phản hồi hàm và các trường hội thoại cảm xúc cho mô hình này. Ưu tiên
`thinkingLevel`; các giá trị dương đã cấu hình của `thinkingBudget` được ánh xạ tới
...
có giới hạn. Các nhà cung cấp giọng nói thời gian thực chỉ dành cho backend cũng có thể chạy qua
phương thức truyền tiếp Gateway chung, giúp giữ thông tin xác thực của nhà cung cấp trên Gateway.
</Note>

Để người bảo trì xác minh trực tiếp, hãy chạy
`OPENAI_API_KEY=... GEMINI_API_KEY=... node --import tsx scripts/dev/realtime-talk-live-smoke.ts`.
Bài kiểm tra nhanh cũng bao quát các đường dẫn backend/WebRTC của OpenAI; nhánh Google tạo cùng
dạng token Live API có giới hạn được Talk trong Control UI sử dụng, mở điểm cuối WebSocket
của trình duyệt, gửi tải trọng thiết lập ban đầu và chờ
`setupComplete`.
...
<AccordionGroup>
  <Accordion title="Tái sử dụng trực tiếp bộ nhớ đệm Gemini">
    Đối với các lần chạy trực tiếp bằng Gemini API (`api: "google-generative-ai"`), OpenClaw
    ...
    - Các tham số từ phạm vi cụ thể hơn (cấp mô hình thay vì toàn cục) luôn được ưu tiên.
  ...
  <Accordion title="Lưu ý sử dụng Gemini CLI">
    ...
    - Văn bản phản hồi truyền trực tuyến đến từ các sự kiện `message` của trợ lý.
