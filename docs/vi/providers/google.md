...
summary: Thiết lập Google Gemini (khóa API + OAuth, tạo hình ảnh, hiểu nội dung phương tiện, TTS, tìm kiếm web)
title: Google (Gemini)
x-i18n:
...
<Tabs>
  ...
  <Tab title="Gemini CLI (OAuth)">
    **Phù hợp nhất cho:** tái sử dụng phiên đăng nhập Gemini CLI hiện có qua PKCE OAuth thay vì dùng khóa API riêng.
    ...
    <Steps>
      <Step title="Cài đặt Gemini CLI">
        ...
        OpenClaw hỗ trợ cả cài đặt bằng Homebrew và cài đặt npm toàn cục, bao gồm
    ...
    Mã mô hình Gemini API của Gemini 3.1 Pro là `gemini-3.1-pro-preview`. OpenClaw chấp nhận dạng ngắn hơn `google/gemini-3.1-pro` như một bí danh tiện lợi và chuẩn hóa nó trước các lệnh gọi nhà cung cấp.
    ...
    **Biến môi trường:**
    ...
    <Note>
    Nếu đăng nhập thất bại trước khi luồng trình duyệt bắt đầu, hãy bảo đảm lệnh cục bộ `gemini`
    đã được cài đặt và nằm trên `PATH`.
    </Note>

    Tham chiếu mô hình `google-gemini-cli/*` là các bí danh tương thích cũ. Các
    cấu hình mới nên dùng tham chiếu mô hình `google/*` cùng với runtime `google-gemini-cli`
...
Nhà cung cấp tìm kiếm web `gemini` đi kèm dùng Gemini Google Search grounding.
...
tồn tại cho proxy vận hành hoặc các endpoint tương thích với Gemini API; khi bị bỏ qua,
...
các điều khiển lập luận của bí danh Gemini 3, Gemini 3.1 và `gemini-*-latest` sang
`thinkingLevel` để các lần chạy mặc định/độ trễ thấp không gửi các giá trị
`thinkingBudget` đã tắt.

`/think adaptive` giữ ngữ nghĩa tư duy động của Google thay vì chọn
một cấp OpenClaw cố định. Gemini 3 và Gemini 3.1 bỏ qua `thinkingLevel` cố định để
Google có thể chọn cấp; Gemini 2.5 gửi sentinel động của Google
`thinkingBudget: -1`.
...
Đặt tư duy thành `off` giữ nguyên trạng thái tắt tư duy thay vì ánh xạ sang
`MINIMAL`.
</Tip>
...
<Note>
Xem [Tạo hình ảnh](/vi/tools/image-generation) để biết các tham số công cụ dùng chung, lựa chọn nhà cung cấp và hành vi chuyển đổi khi lỗi.
</Note>
...
<Note>
Xem [Tạo video](/vi/tools/video-generation) để biết các tham số công cụ dùng chung, lựa chọn nhà cung cấp và hành vi chuyển đổi khi lỗi.
</Note>
...
- Các lần chạy dựa trên phiên tách ra thông qua luồng tác vụ/trạng thái dùng chung, bao gồm `action: "status"`
...
<Note>
Xem [Tạo nhạc](/vi/tools/music-generation) để biết các tham số công cụ dùng chung, lựa chọn nhà cung cấp và hành vi chuyển đổi khi lỗi.
</Note>

## Chuyển văn bản thành giọng nói

Nhà cung cấp giọng nói `google` đi kèm dùng đường dẫn Gemini API TTS với
...
`speakerName` khi văn bản prompt của bạn nhắc đến một người nói có tên.

Gemini API TTS cũng chấp nhận các thẻ âm thanh biểu cảm trong ngoặc vuông trong văn bản,
chẳng hạn như `[whispers]` hoặc `[laughs]`. Để giữ các thẻ khỏi phản hồi chat hiển thị
trong khi vẫn gửi chúng tới TTS, hãy đặt chúng bên trong khối `[[tts:text]]...[[/tts:text]]`:
...
<Note>
Khóa API Google Cloud Console bị giới hạn ở Gemini API là hợp lệ cho
...
giữ các lệnh gọi công cụ trên hợp đồng thoại thời gian thực dùng chung. Để `temperature`
...
<Note>
Control UI Talk hỗ trợ các phiên Google Live trên trình duyệt bằng token một lần
bị ràng buộc. Các nhà cung cấp thoại thời gian thực chỉ chạy ở backend cũng có thể chạy qua
truyền tải chuyển tiếp Gateway chung, giúp giữ thông tin xác thực của nhà cung cấp trên Gateway.
</Note>

Để xác minh trực tiếp dành cho maintainer, chạy
`OPENAI_API_KEY=... GEMINI_API_KEY=... node --import tsx scripts/dev/realtime-talk-live-smoke.ts`.
Nhánh Google tạo cùng dạng token Live API bị ràng buộc mà Control
UI Talk dùng, mở endpoint WebSocket của trình duyệt, gửi payload thiết lập ban đầu,
và chờ `setupComplete`.

## Cấu hình nâng cao

<AccordionGroup>
  <Accordion title="Tái sử dụng bộ nhớ đệm Gemini trực tiếp">
    Đối với các lần chạy Gemini API trực tiếp (`api: "google-generative-ai"`), OpenClaw
    truyền handle `cachedContent` đã cấu hình sang các yêu cầu Gemini.

    - Cấu hình tham số theo từng mô hình hoặc toàn cục bằng
      `cachedContent` hoặc `cached_content` cũ
    - Nếu cả hai đều có, `cachedContent` thắng
    - Giá trị ví dụ: `cachedContents/prebuilt-context`
    - Mức sử dụng trúng bộ nhớ đệm Gemini được chuẩn hóa thành `cacheRead` của OpenClaw từ
      `cachedContentTokenCount` phía upstream
  ...
  <Accordion title="Ghi chú sử dụng JSON của Gemini CLI">
    ...
    - Văn bản phản hồi đến từ trường `response` trong JSON của CLI.
    - Mức sử dụng dùng dự phòng `stats` khi CLI để `usage` trống.
