...
Plugin này sử dụng `matrix-js-sdk` chính thức và hỗ trợ DM, phòng, luồng, phương tiện, phản ứng, cuộc thăm dò, vị trí và E2EE.
...
4. Bắt đầu một DM với bot, hoặc mời bot vào một phòng (xem [tự động tham gia](#auto-join) - lời mời mới chỉ có hiệu lực khi `autoJoin` cho phép).
...
<Warning>
Đặt `autoJoin: "allowlist"` cùng với `autoJoinAllowlist` để giới hạn các lời mời bot chấp nhận, hoặc `autoJoin: "always"` để chấp nhận mọi lời mời.

`autoJoinAllowlist` chỉ chấp nhận mục tiêu ổn định: `!roomId:server`, `#alias:server`, hoặc `*`. Tên phòng thuần bị từ chối; các mục alias được phân giải dựa trên homeserver, không dựa trên trạng thái do phòng được mời tuyên bố.
</Warning>
...
Thiết lập này ảnh hưởng tới khả năng hiển thị ngữ cảnh bổ sung, không ảnh hưởng việc chính tin nhắn inbound có thể kích hoạt trả lời hay không.
Ủy quyền trigger vẫn đến từ `groupPolicy`, `groups`, `groupAllowFrom`, và các thiết lập chính sách DM.
...
Theo mặc định, OpenClaw chặn homeserver Matrix riêng/nội bộ để bảo vệ khỏi SSRF trừ khi bạn
chủ động chọn tham gia theo từng tài khoản.
...
Lựa chọn bật này chỉ cho phép các đích riêng tư/nội bộ đáng tin cậy. Các máy chủ nhà công khai dùng văn bản rõ như
...
khi cấu hình đích gửi rõ ràng, tác vụ cron, liên kết hoặc danh sách cho phép.
OpenClaw giữ khóa phiên nội bộ ở dạng chuẩn tắc để lưu trữ, vì vậy các khóa chữ thường đó
