...
Trạng thái: văn bản + tệp đính kèm DM được hỗ trợ; gửi tệp trong kênh/nhóm yêu cầu `sharePointSiteId` + quyền Graph (xem [Gửi tệp trong cuộc trò chuyện nhóm](#sending-files-in-group-chats)). Cuộc thăm dò được gửi qua Adaptive Cards. Hành động tin nhắn cung cấp `upload-file` rõ ràng cho các lượt gửi ưu tiên tệp.

## Plugin đi kèm

Microsoft Teams được phân phối dưới dạng Plugin đi kèm trong các bản phát hành OpenClaw hiện tại, nên bản dựng đóng gói thông thường không cần cài đặt riêng.
...
Dùng gói trần để theo thẻ phát hành chính thức hiện tại. Chỉ ghim phiên bản chính xác khi bạn cần bản cài đặt có thể tái lập.
...
<Note>
Teams CLI hiện đang ở giai đoạn xem trước. Lệnh và cờ có thể thay đổi giữa các bản phát hành.
</Note>
...
Các lựa chọn thay thế: `ngrok http 3978` hoặc `tailscale funnel 3978` (nhưng các tùy chọn này có thể thay đổi URL trong mỗi phiên).
...
Đối với triển khai production, hãy cân nhắc dùng [xác thực liên kết](/vi/channels/msteams#federated-authentication-certificate-plus-managed-identity) (chứng chỉ hoặc managed identity) thay vì client secret.

<Note>
Cuộc trò chuyện nhóm bị chặn theo mặc định (`channels.msteams.groupPolicy: "allowlist"`). Để cho phép trả lời nhóm, hãy đặt `channels.msteams.groupAllowFrom`, hoặc dùng `groupPolicy: "open"` để cho phép bất kỳ thành viên nào (được kiểm soát bằng mention).
</Note>

## Mục tiêu

- Trò chuyện với OpenClaw qua DM Teams, cuộc trò chuyện nhóm hoặc kênh.
- Giữ định tuyến xác định: câu trả lời luôn quay lại kênh nơi chúng đến.
- Mặc định dùng hành vi kênh an toàn (yêu cầu mention trừ khi được cấu hình khác).

## Ghi cấu hình

Theo mặc định, Microsoft Teams được phép ghi các cập nhật cấu hình do `/config set|unset` kích hoạt (yêu cầu `commands.config: true`).
...
- Đừng dựa vào so khớp UPN/tên hiển thị cho danh sách cho phép — chúng có thể thay đổi. OpenClaw tắt so khớp tên trực tiếp theo mặc định; chỉ bật rõ ràng bằng `channels.msteams.dangerouslyAllowNameMatching: true`.
...
- Đặt `groupPolicy: "open"` để cho phép bất kỳ thành viên nào (mặc định vẫn được kiểm soát bằng mention).
...
- Khi `groupPolicy="allowlist"` và có danh sách cho phép team, chỉ team/kênh được liệt kê mới được chấp nhận (được kiểm soát bằng mention).
- Wizard cấu hình chấp nhận các mục `Team/Channel` và lưu chúng cho bạn.
- Khi khởi động, OpenClaw phân giải tên trong danh sách cho phép của team/kênh và người dùng thành ID (khi quyền Graph cho phép)
  và ghi log ánh xạ; tên team/kênh chưa phân giải được giữ nguyên như đã nhập nhưng mặc định bị bỏ qua khi định tuyến, trừ khi `channels.msteams.dangerouslyAllowNameMatching: true` được bật.
...
2. Đặt **Messaging endpoint** thành URL Webhook của bạn:
   ...
   - Dev cục bộ: Dùng một tunnel (xem [Phát triển cục bộ](#local-development-tunneling) bên dưới)
...
Đối với triển khai production, OpenClaw hỗ trợ **xác thực liên kết** như một lựa chọn an toàn hơn thay cho client secret. Có hai phương thức:
...
1. Tạo hoặc lấy chứng chỉ (định dạng PEM có khóa riêng).
...
- `MSTEAMS_MANAGED_IDENTITY_CLIENT_ID=<client-id>` (chỉ dành cho managed identity do người dùng gán)
...
5. **Đảm bảo quyền truy cập mạng** đến IMDS (`169.254.169.254`) — nếu dùng NetworkPolicy, hãy thêm quy tắc egress cho phép lưu lượng đến `169.254.169.254/32` trên cổng 80.

### So sánh loại xác thực

| Phương thức           | Cấu hình                                      | Ưu điểm                            | Nhược điểm                            |
| --------------------- | -------------------------------------------- | ---------------------------------- | ------------------------------------- |
| **Client secret**     | `appPassword`                                | Thiết lập đơn giản                 | Cần xoay vòng secret, kém an toàn hơn |
| **Chứng chỉ**         | `authType: "federated"` + `certificatePath`  | Không có secret dùng chung qua mạng | Tốn công quản lý chứng chỉ            |
| **Managed Identity**  | `authType: "federated"` + `useManagedIdentity` | Không mật khẩu, không cần quản lý secret | Cần hạ tầng Azure                     |
...
Teams không thể truy cập `localhost`. Dùng một đường hầm phát triển bền vững để URL của bạn giữ nguyên giữa các phiên:
...
Phương án thay thế: `ngrok http 3978` hoặc `tailscale funnel 3978` (URL có thể thay đổi ở mỗi phiên).
...
- `MSTEAMS_CERTIFICATE_PATH` (federated + chứng chỉ)
...
- Đối với tra cứu chéo nhóm: quyền Graph Application `User.Read.All` với sự đồng ý của quản trị viên
...
- Lịch sử luồng đã lấy được lọc theo danh sách cho phép người gửi (`allowFrom` / `groupAllowFrom`), vì vậy việc gieo ngữ cảnh luồng chỉ bao gồm tin nhắn từ người gửi được phép.
...
- Có thể giới hạn lịch sử DM bằng `channels.msteams.dmHistoryLimit` (lượt người dùng). Ghi đè theo từng người dùng: `channels.msteams.dms["<user_id>"].historyLimit`.

## Quyền RSC Teams hiện tại (manifest)

Đây là các **quyền resourceSpecific hiện có** trong manifest ứng dụng Teams của chúng tôi. Chúng chỉ áp dụng bên trong nhóm/cuộc trò chuyện nơi ứng dụng được cài đặt.
...
Để cập nhật một ứng dụng Teams đã cài đặt (ví dụ: để thêm quyền RSC):
...
Sau khi cập nhật, hãy cài đặt lại ứng dụng trong từng nhóm để quyền mới có hiệu lực, và **thoát hoàn toàn rồi khởi chạy lại Teams** (không chỉ đóng cửa sổ) để xóa metadata ứng dụng đã lưu trong bộ nhớ đệm.

<details>
<summary>Cập nhật manifest thủ công (không dùng CLI)</summary>

1. Cập nhật `manifest.json` của bạn bằng các thiết lập mới
...
**Tóm lại:** RSC dành cho lắng nghe thời gian thực; Graph API dành cho truy cập lịch sử. Để bắt kịp các tin nhắn bị lỡ khi ngoại tuyến, bạn cần Graph API với `ChannelMessage.Read.All` (cần sự đồng ý của quản trị viên).
...
4. **Thoát hoàn toàn rồi khởi chạy lại Teams** để xóa metadata ứng dụng đã lưu trong bộ nhớ đệm.

**Quyền bổ sung cho mention người dùng:** @mention người dùng hoạt động sẵn cho người dùng trong cuộc trò chuyện. Tuy nhiên, nếu bạn muốn tìm kiếm và mention động những người dùng **không ở trong cuộc trò chuyện hiện tại**, hãy thêm quyền `User.Read.All` (Application) và cấp sự đồng ý của quản trị viên.

## Hạn chế đã biết

### Hết thời gian chờ Webhook

Teams gửi tin nhắn qua Webhook HTTP. Nếu quá trình xử lý mất quá lâu (ví dụ: phản hồi LLM chậm), bạn có thể thấy:
...
- Adaptive Cards được hỗ trợ cho các cuộc thăm dò và các lần gửi trình bày ngữ nghĩa (xem bên dưới)
...
- `channels.msteams.allowFrom`: danh sách cho phép DM (khuyến nghị dùng ID đối tượng AAD). Trình hướng dẫn phân giải tên thành ID trong khi thiết lập khi có quyền truy cập Graph.
...
- `channels.msteams.mediaAuthAllowHosts`: danh sách cho phép gắn tiêu đề Authorization khi thử lại media (mặc định là máy chủ Graph + Bot Framework).
...
- `channels.msteams.teams.<teamId>.tools`: ghi đè chính sách công cụ mặc định theo từng nhóm (`allow`/`deny`/`alsoAllow`) được dùng khi không có ghi đè kênh.
- `channels.msteams.teams.<teamId>.toolsBySender`: ghi đè chính sách công cụ mặc định theo từng nhóm và từng người gửi (hỗ trợ ký tự đại diện `"*"`).
...
- `channels.msteams.certificateThumbprint`: thumbprint chứng chỉ (tùy chọn, không bắt buộc để xác thực).
...
| ------------------------ | --------------------------------------------------------- | ----------------------------- |
| **Bài đăng** (cổ điển)   | Tin nhắn xuất hiện dưới dạng thẻ với phản hồi dạng luồng bên dưới | `thread` (mặc định)           |
...
- `top-level` trong kênh kiểu Bài đăng → phản hồi xuất hiện dưới dạng các bài đăng cấp cao nhất riêng biệt thay vì trong luồng
...
- **DM:** Hình ảnh và tệp đính kèm hoạt động qua API tệp bot Teams.
- **Kênh/nhóm:** Tệp đính kèm nằm trong bộ nhớ M365 (SharePoint/OneDrive). Payload Webhook chỉ bao gồm một đoạn HTML thay thế, không phải byte tệp thực tế. **Cần quyền Graph API** để tải xuống tệp đính kèm trong kênh.
- Đối với các lần gửi ưu tiên tệp rõ ràng, dùng `action=upload-file` với `media` / `filePath` / `path`; `message` tùy chọn trở thành văn bản/bình luận đi kèm, và `filename` ghi đè tên đã tải lên.
...
Tiêu đề Authorization chỉ được gắn cho các máy chủ trong `channels.msteams.mediaAuthAllowHosts` (mặc định là máy chủ Graph + Bot Framework). Giữ danh sách này nghiêm ngặt (tránh hậu tố đa tenant).
...
| **Hình ảnh (mọi ngữ cảnh)** | Mã hóa Base64 nội tuyến                     | Hoạt động ngay không cần cấu hình thêm           |

### Vì sao cuộc trò chuyện nhóm cần SharePoint

Bot không có ổ OneDrive cá nhân (endpoint Graph API `/me/drive` không hoạt động cho định danh ứng dụng). Để gửi tệp trong cuộc trò chuyện nhóm/kênh, bot tải lên một **site SharePoint** và tạo liên kết chia sẻ.
...
Chia sẻ theo từng người dùng bảo mật hơn vì chỉ người tham gia cuộc trò chuyện mới có thể truy cập tệp. Nếu thiếu quyền `Chat.Read.All`, bot sẽ quay về chia sẻ toàn tổ chức.
...
Gửi payload trình bày ngữ nghĩa đến người dùng hoặc cuộc hội thoại Teams bằng công cụ `message` hoặc CLI. OpenClaw kết xuất chúng thành Teams Adaptive Cards từ hợp đồng trình bày chung.
...
<Note>
Nếu không có tiền tố `user:`, tên sẽ mặc định được phân giải dưới dạng nhóm hoặc đội. Luôn dùng `user:` khi nhắm tới người dùng theo tên hiển thị.
</Note>
...
- Xem `/gateway/configuration` để biết `dmPolicy` và kiểm soát bằng allowlist.

## ID đội và kênh (Lỗi thường gặp)

Tham số truy vấn `groupId` trong URL của Teams **KHÔNG** phải là ID đội được dùng cho cấu hình. Thay vào đó, hãy trích xuất ID từ đường dẫn URL:
...
https://teams.microsoft.com/l/team/19%3ABk4j...%40thread.tacv2/conversations?groupId=...
                                    └────────────────────────────┘
                                    ID cuộc trò chuyện của đội (giải mã URL phần này)
```
...
- **Bỏ qua** tham số truy vấn `groupId` cho định tuyến OpenClaw. Đây là ID nhóm Microsoft Entra, không phải ID cuộc trò chuyện Bot Framework được dùng trong các hoạt động Teams đến.
...
- **Không có phản hồi trong kênh:** theo mặc định cần có mention; đặt `channels.msteams.requireMention=false` hoặc cấu hình theo từng đội/kênh.
- **Không khớp phiên bản (Teams vẫn hiển thị manifest cũ):** gỡ bỏ + thêm lại ứng dụng và thoát hoàn toàn Teams để làm mới.
- **401 Unauthorized từ Webhook:** Dự kiến xảy ra khi kiểm thử thủ công mà không có Azure JWT - nghĩa là endpoint có thể truy cập được nhưng xác thực thất bại. Dùng Azure Web Chat để kiểm thử đúng cách.

### Lỗi tải manifest lên

- **"Icon file cannot be empty":** Manifest tham chiếu đến các tệp biểu tượng có dung lượng 0 byte. Tạo biểu tượng PNG hợp lệ (32x32 cho `outline.png`, 192x192 cho `color.png`).
- **"webApplicationInfo.Id already in use":** Ứng dụng vẫn đang được cài trong một đội/chat khác. Tìm và gỡ cài đặt trước, hoặc chờ 5-10 phút để thay đổi được truyền đi.
- **"Something went wrong" khi tải lên:** Thay vào đó hãy tải lên qua [https://admin.teams.microsoft.com](https://admin.teams.microsoft.com), mở DevTools của trình duyệt (F12) → tab Network, rồi kiểm tra nội dung phản hồi để xem lỗi thực tế.
...
2. Tải lại ứng dụng lên và cài đặt lại trong đội/chat
3. Kiểm tra xem quản trị viên tổ chức của bạn có chặn quyền RSC không
4. Xác nhận bạn đang dùng đúng scope: `ChannelMessage.Read.Group` cho đội, `ChatMessage.Read.Chat` cho chat nhóm
...
- [Nhóm](/vi/channels/groups) — hành vi chat nhóm và kiểm soát bằng mention
