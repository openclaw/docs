...
title: SMS
x-i18n:
    generated_at: "2026-07-16T14:09:01Z"
    model: gpt-5.6
...
OpenClaw nhận và gửi SMS qua số điện thoại Twilio hoặc Messaging Service. Gateway đăng ký một tuyến Webhook đến (mặc định `/webhooks/sms`), mặc định xác thực chữ ký yêu cầu Twilio và gửi phản hồi trở lại qua Messages API của Twilio.
...
| `accountSid`                            | —               | Account SID của Twilio (`AC...`).                                |
| `authToken`                             | —               | Auth Token của Twilio; chuỗi văn bản thuần hoặc SecretRef.          |
...
| `dangerouslyDisableSignatureValidation` | `false`         | Bỏ qua kiểm tra `X-Twilio-Signature`; chỉ dành cho kiểm thử đường hầm cục bộ. |
...
Nếu cả `fromNumber` và `messagingServiceSid` đều hiện diện sau khi phân giải cấu hình và biến môi trường, `fromNumber` sẽ được sử dụng.
...
Các mục `allowFrom` phải là số điện thoại E.164, chẳng hạn như `+15551234567`. Các tiền tố `sms:` và `twilio-sms:` được chấp nhận và chuẩn hóa. Đối với trợ lý riêng tư, ưu tiên `dmPolicy: "allowlist"` với các số điện thoại được chỉ định rõ ràng:
...
CLI yêu cầu `--target` rõ ràng. `defaultTo` dành cho các đường dẫn tự động hóa và gửi do tác nhân khởi tạo, trong đó đích có thể được phân giải từ cấu hình kênh.
...
3. Gửi một SMS từ điện thoại của bạn đến số Twilio.
4. Chạy `openclaw pairing list sms`.
5. Phê duyệt mã ghép nối bằng `openclaw pairing approve sms <CODE>`.
6. Gửi một SMS khác và xác nhận tác tử phản hồi.

Để chỉ kiểm thử gửi đi, hãy dùng:

```bash
openclaw message send --channel sms --target sms:+15557654321 --message "Kiểm thử SMS OpenClaw"
```
...
openclaw pairing list sms
openclaw pairing approve sms <CODE>
imsg send --to "+15551234567" --service sms --text "chỉ trả lời SMS pong" --json
```
...
Kiểm tra để đảm bảo `publicWebhookUrl` khớp chính xác với URL được cấu hình trong Twilio, bao gồm lược đồ, máy chủ, đường dẫn và chuỗi truy vấn. Twilio ký chuỗi URL công khai, vì vậy việc proxy viết lại URL và các tên máy chủ thay thế có thể làm hỏng quá trình xác thực chữ ký.
...
Kiểm tra URL và phương thức Webhook **Messaging** của số Twilio. URL phải trỏ đến URL Webhook SMS và dùng `POST`. Đồng thời xác nhận Gateway có thể được truy cập từ Internet công cộng hoặc thông qua đường hầm của bạn.
...
Kiểm tra `dmPolicy` và `allowFrom`. Với chính sách `pairing` mặc định, người gửi phải được phê duyệt trước khi các lượt tương tác thông thường của tác tử được xử lý.
