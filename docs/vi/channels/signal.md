...
read_when:
    ...
    - Gỡ lỗi gửi/nhận trên Signal
summary: Hỗ trợ Signal qua signal-cli (JSON-RPC + SSE), đường dẫn thiết lập và mô hình số điện thoại
title: Signal
x-i18n:
...
- Một số điện thoại có thể nhận một SMS xác minh (cho đường dẫn đăng ký qua SMS).
...
Dùng cách này khi bạn muốn một số bot chuyên dụng thay vì liên kết một tài khoản ứng dụng Signal hiện có.
...
4. Chạy đăng ký lại ngay lập tức (mã thông báo captcha hết hạn nhanh):
...
- Ghép nối là trao đổi mã thông báo mặc định cho DM Signal. Chi tiết: [Ghép nối](/vi/channels/pairing)
- Người gửi chỉ có UUID (từ `sourceUuid`) được lưu dưới dạng `uuid:<id>` trong `channels.signal.allowFrom`.

Nhóm:

- `channels.signal.groupPolicy = open | allowlist | disabled`.
- `channels.signal.groupAllowFrom` kiểm soát những nhóm hoặc người gửi nào có thể kích hoạt phản hồi nhóm khi đặt `allowlist`; mục nhập có thể là ID nhóm Signal (thô, `group:<id>`, hoặc `signal:group:<id>`), số điện thoại người gửi, giá trị `uuid:<id>`, hoặc `*`.
...
- Việc đưa một nhóm Signal vào danh sách cho phép thông qua `groupAllowFrom` tự nó không tắt cổng yêu cầu nhắc đến. Một mục `channels.signal.groups["<group-id>"]` được cấu hình cụ thể sẽ xử lý mọi tin nhắn nhóm trừ khi đặt `requireMention=true`.
- Ghi chú runtime: nếu `channels.signal` hoàn toàn bị thiếu, runtime sẽ quay về `groupPolicy="allowlist"` cho kiểm tra nhóm (ngay cả khi `channels.defaults.groupPolicy` được đặt).
...
- Tệp đính kèm ghi chú thoại dùng tên tệp `signal-cli` làm dự phòng MIME khi thiếu `contentType`, vì vậy phiên âm âm thanh vẫn có thể phân loại ghi nhớ thoại AAC.
...
## Phản ứng (công cụ nhắn tin)

- Dùng `message action=react` với `channel=signal`.
- Đích: người gửi E.164 hoặc UUID (dùng `uuid:<id>` từ đầu ra ghép nối; UUID trần cũng hoạt động).
...
- `channels.signal.reactionLevel`: `off | ack | minimal | extensive`.
  - `off`/`ack` tắt phản ứng của agent (công cụ nhắn tin `react` sẽ báo lỗi).
...
- Tin nhắn nhóm bị bỏ qua: cổng người gửi nhóm/nhắc đến chặn việc gửi.
...
- Xác minh SMS chỉ cần cho quy trình đăng ký hoặc khôi phục, nhưng mất quyền kiểm soát số/tài khoản có thể làm việc đăng ký lại phức tạp hơn.
...
- `channels.signal.httpUrl`: URL đầy đủ của daemon (ghi đè host/port).
...
- `channels.signal.groupAllowFrom`: danh sách cho phép nhóm; chấp nhận ID nhóm Signal (dạng thô, `group:<id>`, hoặc `signal:group:<id>`), số E.164 của người gửi, hoặc giá trị `uuid:<id>`.
- `channels.signal.groups`: ghi đè theo từng nhóm, được khóa bằng ID nhóm Signal (hoặc `"*"`). Các trường được hỗ trợ: `requireMention`, `tools`, `toolsBySender`.
...
- `channels.signal.mediaMaxMb`: giới hạn media gửi đến/gửi đi (MB).
