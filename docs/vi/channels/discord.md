...
Sẵn sàng cho DMs và kênh guild thông qua Discord gateway chính thức.

<CardGroup cols={3}>
  <Card title="Ghép nối" icon="link" href="/vi/channels/pairing">
    DMs trên Discord mặc định dùng chế độ ghép nối.
  </Card>
...
Bạn sẽ cần tạo một ứng dụng mới có bot, thêm bot vào máy chủ của bạn, rồi ghép nối bot đó với OpenClaw. Chúng tôi khuyến nghị thêm bot vào máy chủ riêng của bạn. Nếu bạn chưa có, hãy [tạo một máy chủ trước](https://support.discord.com/hc/en-us/articles/204849977-How-do-I-create-a-server) (chọn **Create My Own > For me and my friends**).

<Steps>
  <Step title="Tạo ứng dụng và bot Discord">
    Đi tới [Discord Developer Portal](https://discord.com/developers/applications) và nhấp **New Application**. Đặt tên chẳng hạn như "OpenClaw".

    Nhấp **Bot** trong thanh bên. Đặt **Username** thành tên bạn gọi tác tử OpenClaw của mình.
  ...
  <Step title="Bật privileged intents">
    ...
    - **Presence Intent** (tùy chọn; chỉ cần cho cập nhật hiện diện)
  ...
  <Step title="Sao chép token bot của bạn">
    ...
    Sao chép token và lưu lại ở đâu đó. Đây là **Bot Token** của bạn và bạn sẽ cần dùng đến nó ngay sau đây.

  </Step>

  <Step title="Tạo URL mời và thêm bot vào máy chủ của bạn">
    Nhấp **OAuth2** trong thanh bên. Bạn sẽ tạo một URL mời với các quyền phù hợp để thêm bot vào máy chủ của mình.
    ...
    **Quyền văn bản**
    ...
    Đây là bộ cơ bản cho các kênh văn bản thông thường. Nếu bạn định đăng trong thread Discord, bao gồm các quy trình kênh diễn đàn hoặc kênh media tạo hoặc tiếp tục một thread, hãy bật thêm **Send Messages in Threads**.
    Sao chép URL được tạo ở cuối trang, dán vào trình duyệt, chọn máy chủ của bạn, rồi nhấp **Continue** để kết nối. Bây giờ bạn sẽ thấy bot trong máy chủ Discord.
  ...
  <Step title="Bật Developer Mode và thu thập ID của bạn">
    ...
    1. Nhấp **User Settings** (biểu tượng bánh răng cạnh avatar của bạn) → **Advanced** → bật **Developer Mode**
  ...
  <Step title="Cho phép DMs từ thành viên máy chủ">
    ...
    Điều này cho phép thành viên máy chủ (bao gồm bot) gửi DM cho bạn. Giữ tùy chọn này bật nếu bạn muốn dùng Discord DMs với OpenClaw. Nếu bạn chỉ định dùng kênh guild, bạn có thể tắt DMs sau khi ghép nối.

  </Step>

  <Step title="Đặt token bot của bạn một cách an toàn (đừng gửi trong chat)">
    Token bot Discord của bạn là bí mật (giống như mật khẩu). Hãy đặt nó trên máy đang chạy OpenClaw trước khi nhắn tin cho tác tử của bạn.
...
```
    ...
    Với các bản cài đặt dịch vụ được quản lý, hãy chạy `openclaw gateway install` từ shell có `DISCORD_BOT_TOKEN`, hoặc lưu biến này trong `~/.openclaw/.env`, để dịch vụ có thể phân giải env SecretRef sau khi khởi động lại.
    Nếu host của bạn bị chặn hoặc bị giới hạn tốc độ bởi lần tra cứu ứng dụng khởi động của Discord, hãy đặt ID ứng dụng/client Discord từ Developer Portal để quá trình khởi động có thể bỏ qua lệnh gọi REST đó. Dùng `channels.discord.applicationId` cho tài khoản mặc định, hoặc `channels.discord.accounts.<accountId>.applicationId` khi bạn chạy nhiều bot Discord.
  ...
  <Step title="Cấu hình OpenClaw và ghép nối">
    ...
    <Tabs>
      <Tab title="Hỏi tác tử của bạn">
        Trò chuyện với tác tử OpenClaw của bạn trên bất kỳ kênh hiện có nào (ví dụ: Telegram) và nói với nó. Nếu Discord là kênh đầu tiên của bạn, hãy dùng tab CLI / cấu hình thay thế.
...
```
        ...
        Với thiết lập bằng script hoặc từ xa, hãy ghi cùng khối JSON5 bằng `openclaw config patch --file ./discord.patch.json5 --dry-run` rồi chạy lại không có `--dry-run`. Các giá trị `token` dạng văn bản thuần được hỗ trợ. Giá trị SecretRef cũng được hỗ trợ cho `channels.discord.token` qua các provider env/file/exec. Xem [Quản lý bí mật](/vi/gateway/secrets).

        Với nhiều bot Discord, hãy giữ token bot và ID ứng dụng của từng bot trong tài khoản tương ứng. `channels.discord.applicationId` cấp cao nhất được các tài khoản kế thừa, vì vậy chỉ đặt ở đó khi mọi tài khoản nên dùng cùng một ID ứng dụng.
...
```
    ...
    Bây giờ bạn sẽ có thể trò chuyện với tác tử của mình trong Discord qua DM.
...
Nếu hai tài khoản Discord được bật phân giải ra cùng một token bot, OpenClaw chỉ khởi động một gateway monitor cho token đó. Token lấy từ cấu hình thắng env dự phòng mặc định; nếu không, tài khoản được bật đầu tiên sẽ thắng và tài khoản trùng lặp được báo cáo là đã tắt.
Với các lệnh gọi outbound nâng cao (công cụ message/hành động kênh), `token` rõ ràng theo từng lệnh gọi được dùng cho lệnh gọi đó. Điều này áp dụng cho các hành động gửi và đọc/thăm dò (ví dụ read/search/fetch/thread/pins/permissions). Chính sách tài khoản/thiết lập thử lại vẫn đến từ tài khoản đã chọn trong snapshot runtime đang hoạt động.
</Note>

## Khuyến nghị: Thiết lập không gian làm việc guild

Sau khi DMs hoạt động, bạn có thể thiết lập máy chủ Discord của mình thành một không gian làm việc đầy đủ, nơi mỗi kênh có phiên tác tử riêng với ngữ cảnh riêng. Điều này được khuyến nghị cho các máy chủ riêng chỉ có bạn và bot của bạn.

<Steps>
  <Step title="Thêm máy chủ của bạn vào danh sách cho phép guild">
    Điều này cho phép tác tử của bạn phản hồi trong bất kỳ kênh nào trên máy chủ của bạn, không chỉ DMs.
...
```
  ...
  <Step title="Cho phép phản hồi không cần @mention">
    Theo mặc định, tác tử của bạn chỉ phản hồi trong kênh guild khi được @mentioned. Với máy chủ riêng, có lẽ bạn muốn nó phản hồi mọi tin nhắn.

    Trong kênh guild, các phản hồi cuối thông thường của trợ lý mặc định vẫn ở chế độ riêng tư. Đầu ra Discord hiển thị phải được gửi rõ ràng bằng công cụ `message`, để tác tử có thể theo dõi âm thầm theo mặc định và chỉ đăng khi nó quyết định phản hồi kênh là hữu ích.

    Điều này có nghĩa là model được chọn phải gọi công cụ một cách đáng tin cậy. Nếu Discord hiển thị trạng thái đang nhập và log cho thấy có sử dụng token nhưng không có tin nhắn nào được đăng, hãy kiểm tra log phiên để tìm văn bản trợ lý có `didSendViaMessagingTool: false`. Điều đó nghĩa là model đã tạo câu trả lời cuối riêng tư thay vì gọi `message(action=send)`. Chuyển sang model gọi công cụ mạnh hơn, hoặc dùng cấu hình bên dưới để khôi phục các phản hồi cuối tự động kiểu cũ.
...
```
        ...
        Để khôi phục các phản hồi cuối tự động kiểu cũ cho phòng nhóm/kênh, đặt `messages.groupChat.visibleReplies: "automatic"`.
...
Bây giờ hãy tạo một vài kênh trên máy chủ Discord của bạn và bắt đầu trò chuyện. Tác tử của bạn có thể thấy tên kênh, và mỗi kênh có phiên biệt lập riêng — vì vậy bạn có thể thiết lập `#coding`, `#home`, `#research`, hoặc bất cứ gì phù hợp với quy trình làm việc của bạn.

## Model runtime

- Gateway sở hữu kết nối Discord.
- Định tuyến phản hồi có tính xác định: phản hồi gửi đến từ Discord sẽ trả lại Discord.
- Siêu dữ liệu guild/kênh Discord được thêm vào lời nhắc mô hình dưới dạng ngữ cảnh không đáng tin cậy,
  không phải dưới dạng tiền tố phản hồi hiển thị với người dùng. Nếu mô hình sao chép phong bì đó
  trở lại, OpenClaw sẽ loại bỏ siêu dữ liệu đã sao chép khỏi phản hồi gửi đi và khỏi
  ngữ cảnh phát lại trong tương lai.
- Theo mặc định (`session.dmScope=main`), chat trực tiếp dùng chung phiên chính của agent (`agent:main:main`).
...
- Lệnh slash gốc chạy trong các phiên lệnh tách biệt (`agent:<agentId>:discord:slash:<userId>`), trong khi vẫn mang `CommandTargetSessionKey` tới phiên hội thoại đã định tuyến.
- Việc phân phối thông báo cron/heartbeat chỉ có văn bản tới Discord dùng câu trả lời cuối cùng
  hiển thị với assistant một lần. Tải trọng media và thành phần có cấu trúc vẫn là
  nhiều tin nhắn khi agent phát ra nhiều tải trọng có thể phân phối.

## Kênh diễn đàn

Kênh diễn đàn và media của Discord chỉ chấp nhận bài đăng luồng. OpenClaw hỗ trợ hai cách tạo chúng:

- Gửi tin nhắn tới diễn đàn cha (`channel:<forumId>`) để tự động tạo luồng. Tiêu đề luồng dùng dòng đầu tiên không rỗng trong tin nhắn của bạn.
...
OpenClaw hỗ trợ vùng chứa thành phần Discord v2 cho tin nhắn của agent. Dùng công cụ tin nhắn với tải trọng `components`. Kết quả tương tác được định tuyến trở lại agent như tin nhắn gửi đến thông thường và tuân theo các cài đặt `replyToMode` Discord hiện có.
...
Theo mặc định, thành phần chỉ dùng một lần. Đặt `components.reusable=true` để cho phép dùng nút, mục chọn và biểu mẫu nhiều lần cho đến khi chúng hết hạn.

Để giới hạn ai có thể bấm nút, hãy đặt `allowedUsers` trên nút đó (ID người dùng Discord, tag hoặc `*`). Khi được cấu hình, người dùng không khớp sẽ nhận một thông báo từ chối tạm thời.

Các lệnh slash `/model` và `/models` mở bộ chọn mô hình tương tác với các menu thả xuống nhà cung cấp, mô hình và runtime tương thích cùng bước Gửi. `/models add` đã ngừng dùng và hiện trả về thông báo ngừng dùng thay vì đăng ký mô hình từ chat. Phản hồi của bộ chọn là tạm thời và chỉ người dùng gọi lệnh mới có thể dùng.
...
<Tabs>
  <Tab title="DM policy">
    ...
    Mức ưu tiên khi dùng nhiều tài khoản:
    ...
    ID số trần thường phân giải thành ID kênh khi mặc định kênh đang hoạt động, nhưng các ID được liệt kê trong `allowFrom` DM hiệu lực của tài khoản sẽ được coi là mục tiêu DM người dùng để tương thích.
  ...
  <Tab title="DM access groups">
    ...
    Tên nhóm truy cập được chia sẻ giữa các kênh tin nhắn. Dùng `type: "message.senders"` cho nhóm tĩnh có thành viên được biểu diễn bằng cú pháp `allowFrom` thông thường của từng kênh, hoặc `type: "discord.channelAudience"` khi đối tượng `ViewChannel` hiện tại của một kênh Discord nên xác định tư cách thành viên động. Hành vi nhóm truy cập dùng chung được ghi lại tại đây: [Nhóm truy cập](/vi/channels/access-groups).
...
```
    ...
    Ví dụ: cho phép bất kỳ ai có thể thấy `#maintainers` gửi DM tới bot, đồng thời giữ DM đóng với mọi người khác.
...
```
    ...
    Tra cứu thất bại theo hướng đóng. Nếu Discord trả về `Missing Access`, tra cứu thành viên thất bại, hoặc kênh thuộc về một guild khác, người gửi DM sẽ được coi là không được ủy quyền.
  ...
  <Tab title="Guild policy">
    ...
    - khớp trực tiếp theo tên/tag bị tắt theo mặc định; chỉ bật `channels.discord.dangerouslyAllowNameMatching: true` dưới dạng chế độ tương thích phá kính khẩn cấp
...
```
  ...
  <Tab title="Mentions and group DMs">
    Tin nhắn guild được kiểm soát bằng lượt đề cập theo mặc định.

    Phát hiện lượt đề cập bao gồm:
...
- `commands.native=false` bỏ qua việc đăng ký và dọn dẹp lệnh slash của Discord trong lúc khởi động. Các lệnh đã đăng ký trước đó có thể vẫn hiển thị trong Discord cho đến khi bạn xóa chúng khỏi ứng dụng Discord.
- Xác thực lệnh gốc dùng cùng các allowlist/chính sách Discord như xử lý tin nhắn thông thường.
- Các lệnh vẫn có thể hiển thị trong giao diện Discord đối với người dùng không được ủy quyền; việc thực thi vẫn áp dụng xác thực OpenClaw và trả về "không được ủy quyền".
...
<AccordionGroup>
  <Accordion title="Reply tags and native replies">
    ...
    `first` luôn gắn tham chiếu trả lời gốc ngầm định vào tin nhắn Discord gửi ra đầu tiên của lượt.
  ...
  <Accordion title="Live stream preview">
    OpenClaw có thể phát trực tiếp bản nháp trả lời bằng cách gửi một tin nhắn tạm thời và chỉnh sửa nó khi văn bản đến. `channels.discord.streaming` nhận `off` | `partial` | `block` | `progress` (mặc định). `progress` giữ một bản nháp trạng thái có thể chỉnh sửa và cập nhật nó bằng tiến trình công cụ cho đến khi gửi kết quả cuối cùng; `streamMode` là bí danh runtime cũ. Chạy `openclaw doctor --fix` để viết lại cấu hình đã lưu sang khóa chuẩn.

    Đặt `channels.discord.streaming.mode` thành `off` để tắt chỉnh sửa bản xem trước Discord. Nếu phát trực tiếp theo khối của Discord được bật tường minh, OpenClaw bỏ qua luồng xem trước để tránh phát trực tiếp hai lần.
...
```
    ...
    - `block` phát các đoạn có kích thước bản nháp (dùng `draftChunk` để tinh chỉnh kích thước và điểm ngắt, bị giới hạn theo `textChunkLimit`).
    - Kết quả cuối cùng dạng phương tiện, lỗi và trả lời tường minh sẽ hủy các chỉnh sửa xem trước đang chờ.
    ...
    - `streaming.preview.commandText` / `streaming.progress.commandText` kiểm soát chi tiết lệnh/thực thi trong các dòng tiến trình ngắn gọn: `raw` (mặc định) hoặc `status` (chỉ nhãn công cụ).

    Ẩn văn bản lệnh/thực thi thô trong khi vẫn giữ các dòng tiến trình ngắn gọn:
    ...
    Phát trực tiếp bản xem trước chỉ dùng văn bản; trả lời phương tiện sẽ quay về cách gửi thông thường. Khi phát trực tiếp `block` được bật tường minh, OpenClaw bỏ qua luồng xem trước để tránh phát trực tiếp hai lần.
  ...
  <Accordion title="History, context, and thread behavior">
    ...
    - Phiên thread kế thừa lựa chọn `/model` cấp phiên của kênh cha làm phương án dự phòng chỉ cho model; lựa chọn `/model` cục bộ của thread vẫn được ưu tiên và lịch sử transcript cha không được sao chép trừ khi bật kế thừa transcript.
    - `channels.discord.thread.inheritParent` (mặc định `false`) chọn để các auto-thread mới được gieo từ transcript cha. Ghi đè theo từng tài khoản nằm trong `channels.discord.accounts.<id>.thread.inheritParent`.
    ...
    Chủ đề kênh được đưa vào dưới dạng ngữ cảnh **không đáng tin cậy**. Allowlists kiểm soát ai có thể kích hoạt agent, không phải là ranh giới biên tập lại toàn bộ ngữ cảnh bổ sung.

  </Accordion>

  <Accordion title="Thread-bound sessions for subagents">
    Discord có thể ràng buộc một thread với một mục tiêu phiên để các tin nhắn tiếp theo trong thread đó tiếp tục định tuyến tới cùng phiên (bao gồm cả phiên subagent).
...
```
    ...
    - `session.threadBindings.*` đặt giá trị mặc định toàn cục.
    - `channels.discord.threadBindings.*` ghi đè hành vi Discord.
    - `spawnSessions` kiểm soát tự động tạo/ràng buộc thread cho `sessions_spawn({ thread: true })` và các lần tạo thread ACP. Mặc định: `true`.
    - `defaultSpawnContext` kiểm soát ngữ cảnh subagent gốc cho các lần tạo bị ràng buộc với thread. Mặc định: `"fork"`.
  ...
  <Accordion title="Persistent ACP channel bindings">
    Đối với không gian làm việc ACP ổn định "luôn bật", hãy cấu hình các ràng buộc ACP có kiểu ở cấp cao nhất nhắm tới cuộc trò chuyện Discord.
...
```
    ...
    - Trong kênh hoặc thread đã ràng buộc, `/new` và `/reset` đặt lại cùng phiên ACP tại chỗ. Các ràng buộc thread tạm thời có thể ghi đè phân giải mục tiêu khi đang hoạt động.
    - `spawnSessions` kiểm soát tạo/ràng buộc thread con qua `--thread auto|here`.
  ...
  <Accordion title="Config writes">
    ...
    Điều này ảnh hưởng tới các luồng `/config set|unset` (khi các tính năng lệnh được bật).
...
```
    ...
    - nếu tra cứu thất bại, tin nhắn được proxy sẽ được xử lý như tin nhắn bot và bị bỏ trừ khi `allowBots=true`

  </Accordion>

  <Accordion title="Outbound mention aliases">
    Dùng `mentionAliases` khi agent cần đề cập gửi đi xác định được cho người dùng Discord đã biết. Khóa là handle không có `@` ở đầu; giá trị là ID người dùng Discord. Handle không xác định, `@everyone`, `@here` và đề cập bên trong code span Markdown được giữ nguyên.
...
```
    ...
    - 2: Đang nghe
    - 3: Đang xem
    - 4: Tùy chỉnh (dùng văn bản hoạt động làm trạng thái trạng thái; emoji là tùy chọn)
    - 5: Đang thi đấu
...
```
    ...
    Tự động hiện diện ánh xạ trạng thái sẵn sàng lúc chạy sang trạng thái Discord: healthy => online, degraded hoặc unknown => idle, exhausted hoặc unavailable => dnd. Các phần ghi đè văn bản tùy chọn:
  ...
  <Accordion title="Approvals in Discord">
    ...
    Discord cũng kết xuất các nút phê duyệt dùng chung được các kênh trò chuyện khác sử dụng. Bộ điều hợp Discord nguyên sinh chủ yếu thêm định tuyến DM cho người phê duyệt và phát tán đến kênh.
    ...
    Nếu runtime phê duyệt nguyên sinh của Discord không hoạt động, OpenClaw giữ cho
    lời nhắc `/approve <id> <decision>` cục bộ, xác định, vẫn hiển thị. Nếu
    runtime đang hoạt động nhưng không thể gửi thẻ nguyên sinh đến bất kỳ đích nào,
...
OpenClaw dùng thành phần Discord v2 cho phê duyệt exec và dấu mốc xuyên ngữ cảnh. Hành động tin nhắn Discord cũng có thể chấp nhận `components` cho giao diện tùy chỉnh (nâng cao; yêu cầu xây dựng payload thành phần qua công cụ discord), trong khi `embeds` cũ vẫn khả dụng nhưng không được khuyến nghị.
...
5. Bật lệnh nguyên sinh (`commands.native` hoặc `channels.discord.commands.native`).
...
- Ghi đè `systemPrompt` Discord theo kênh áp dụng cho các lượt transcript giọng nói của kênh thoại đó.
- Các lượt transcript giọng nói suy ra trạng thái chủ sở hữu từ `allowFrom` của Discord (hoặc `dm.allowFrom`); người nói không phải chủ sở hữu không thể truy cập công cụ chỉ dành cho chủ sở hữu (ví dụ `gateway` và `cron`).
- Giọng nói Discord là tùy chọn cho cấu hình chỉ văn bản; đặt `channels.discord.voice.enabled=true` (hoặc giữ một khối `channels.discord.voice` hiện có) để bật lệnh `/vc`, runtime giọng nói và Gateway intent `GuildVoiceStates`.
- `channels.discord.intents.voiceStates` có thể ghi đè rõ ràng đăng ký intent trạng thái giọng nói. Để trống để intent đi theo trạng thái bật giọng nói hiệu lực.
- `voice.daveEncryption` và `voice.decryptionFailureTolerance` được truyền tiếp tới tùy chọn tham gia của `@discordjs/voice`.
...
- Phát lại giọng nói không dừng chỉ vì người dùng khác bắt đầu nói. Để tránh vòng lặp phản hồi, OpenClaw bỏ qua thu âm giọng nói mới khi TTS đang phát; hãy nói sau khi phát lại kết thúc cho lượt tiếp theo.
- `voice.captureSilenceGraceMs` kiểm soát thời gian OpenClaw chờ sau khi Discord báo một người nói đã dừng trước khi hoàn tất đoạn âm thanh đó cho STT. Mặc định: `2500`; tăng giá trị này nếu Discord chia các khoảng nghỉ bình thường thành các transcript từng phần rời rạc.
- Khi ElevenLabs là nhà cung cấp TTS được chọn, phát lại giọng nói Discord dùng TTS truyền phát và bắt đầu từ luồng phản hồi của nhà cung cấp. Các nhà cung cấp không hỗ trợ truyền phát quay về đường dẫn tệp tạm đã tổng hợp.
- OpenClaw cũng theo dõi lỗi giải mã nhận và tự động khôi phục bằng cách rời/tham gia lại kênh thoại sau các lỗi lặp lại trong một khoảng thời gian ngắn.
...
- Sự kiện nhận `The operation was aborted` là dự kiến khi OpenClaw hoàn tất một đoạn người nói đã thu; chúng là chẩn đoán chi tiết, không phải cảnh báo.

Pipeline kênh thoại:

- Thu PCM Discord được chuyển đổi thành tệp tạm WAV.
- `tools.media.audio` xử lý STT, ví dụ `openai/gpt-4o-mini-transcribe`.
- Transcript được gửi qua lối vào và định tuyến Discord trong khi LLM phản hồi chạy với chính sách đầu ra giọng nói ẩn công cụ `tts` của agent và yêu cầu trả về văn bản, vì giọng nói Discord sở hữu bước phát TTS cuối cùng.
- `voice.model`, khi được đặt, chỉ ghi đè LLM phản hồi cho lượt kênh thoại này.
- `voice.tts` được hợp nhất đè lên `messages.tts`; các nhà cung cấp hỗ trợ truyền phát cấp dữ liệu trực tiếp cho trình phát, nếu không thì tệp âm thanh kết quả được phát trong kênh đã tham gia.
...
```
  ...
  <Accordion title="Require mention false nhưng vẫn bị chặn">
    ...
    - `groupPolicy="allowlist"` mà không có allowlist guild/channel khớp
    - `requireMention` được cấu hình sai vị trí (phải nằm dưới `channels.discord.guilds` hoặc mục channel)
  ...
  <Accordion title="Lượt Discord chạy lâu hoặc phản hồi trùng lặp">
    ...
    Các nút chỉnh hàng đợi Gateway của Discord:
    ...
    - thiết lập này chỉ kiểm soát công việc listener của Gateway Discord, không kiểm soát thời lượng lượt agent

    Discord không áp dụng timeout do channel sở hữu cho các lượt agent đã xếp hàng. Listener tin nhắn bàn giao ngay lập tức, và các lượt chạy Discord đã xếp hàng giữ nguyên thứ tự theo phiên cho đến khi vòng đời phiên/tool/runtime hoàn tất hoặc hủy công việc.
...
```
  ...
  <Accordion title="Cảnh báo timeout khi tra cứu metadata Gateway">
    OpenClaw lấy metadata Discord `/gateway/bot` trước khi kết nối. Các lỗi tạm thời sẽ chuyển sang URL Gateway mặc định của Discord và được giới hạn tần suất trong nhật ký.

    Các nút chỉnh timeout metadata:
    ...
    - env dự phòng khi config chưa đặt: `OPENCLAW_DISCORD_GATEWAY_INFO_TIMEOUT_MS`
    - mặc định: `30000` (30 giây), tối đa: `120000`

  </Accordion>

  <Accordion title="Khởi động lại do timeout Gateway READY">
    OpenClaw chờ sự kiện Gateway `READY` của Discord trong quá trình khởi động và sau khi runtime kết nối lại. Các thiết lập nhiều tài khoản có giãn cách khởi động có thể cần khoảng chờ READY khi khởi động dài hơn mặc định.

    Các nút chỉnh timeout READY:
    ...
    - env dự phòng khi config khởi động chưa đặt: `OPENCLAW_DISCORD_READY_TIMEOUT_MS`
    ...
    - env dự phòng khi config runtime chưa đặt: `OPENCLAW_DISCORD_RUNTIME_READY_TIMEOUT_MS`
    - mặc định runtime: `30000` (30 giây), tối đa: `120000`

  </Accordion>

  <Accordion title="Sai khác khi kiểm tra quyền">
    Kiểm tra quyền bằng `channels status --probe` chỉ hoạt động với ID channel dạng số.

    Nếu bạn dùng khóa slug, việc khớp khi runtime vẫn có thể hoạt động, nhưng probe không thể xác minh đầy đủ quyền.
  ...
  <Accordion title="Vòng lặp bot với bot">
    Theo mặc định, tin nhắn do bot viết sẽ bị bỏ qua.

    Nếu bạn đặt `channels.discord.allowBots=true`, hãy dùng quy tắc mention và allowlist chặt chẽ để tránh hành vi vòng lặp.
...
- media/thử lại: `mediaMaxMb` (giới hạn lượt tải lên Discord chiều đi, mặc định `100MB`), `retry`
- hành động: `actions.*`
...
<CardGroup cols={2}>
  ...
  <Card title="Nhóm" icon="users" href="/vi/channels/groups">
    Hành vi chat nhóm và allowlist.
  </Card>
  <Card title="Định tuyến channel" icon="route" href="/vi/channels/channel-routing">
  ...
  <Card title="Định tuyến đa agent" icon="sitemap" href="/vi/concepts/multi-agent">
    Ánh xạ guild và channel tới agent.
  </Card>
  <Card title="Lệnh slash" icon="terminal" href="/vi/tools/slash-commands">
    Hành vi lệnh native.
  </Card>
