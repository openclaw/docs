...
read_when:
    ...
    - Bạn muốn triển khai lên đám mây bằng một cú nhấp với Giao diện điều khiển dựa trên trình duyệt
summary: Triển khai OpenClaw trên Northflank bằng mẫu một cú nhấp
title: Northflank
x-i18n:
...
Triển khai OpenClaw trên Northflank bằng mẫu một cú nhấp và truy cập thông qua Control UI trên web.
Đây là lộ trình "không cần terminal trên máy chủ" dễ nhất: Northflank chạy Gateway cho bạn.
...
- Lưu trữ bền vững thông qua Northflank Volume (`/data`) để `openclaw.json`,
  `auth-profiles.json` theo từng agent, trạng thái kênh/nhà cung cấp, phiên và
