---
read_when:
    - Bạn cần giải thích không gian làm việc của agent hoặc cấu trúc tệp của nó
...
`openclaw onboard`, `openclaw configure`, hoặc `openclaw setup` sẽ tạo không gian làm việc và khởi tạo các tệp bootstrap nếu chúng bị thiếu.

<Note>
Bản sao seed sandbox chỉ chấp nhận các tệp thông thường nằm trong không gian làm việc; bí danh symlink/hardlink phân giải ra ngoài không gian làm việc nguồn sẽ bị bỏ qua.
</Note>
...
Các bản cài đặt cũ hơn có thể đã tạo `~/openclaw`. Giữ nhiều thư mục không gian làm việc có thể gây nhầm lẫn về xác thực hoặc lệch trạng thái, vì mỗi lần chỉ có một không gian làm việc hoạt động.

<Note>
**Khuyến nghị:** chỉ giữ một không gian làm việc đang hoạt động. Nếu bạn không còn dùng các thư mục bổ sung, hãy lưu trữ hoặc chuyển chúng vào Thùng rác (ví dụ `trash ~/openclaw`). Nếu bạn cố ý giữ nhiều không gian làm việc, hãy bảo đảm `agents.defaults.workspace` trỏ tới thư mục đang hoạt động.
...
Đây là các tệp tiêu chuẩn mà OpenClaw mong đợi bên trong không gian làm việc:

<AccordionGroup>
  <Accordion title="AGENTS.md - operating instructions">
    Hướng dẫn vận hành cho tác nhân và cách tác nhân nên dùng bộ nhớ. Được tải khi bắt đầu mọi phiên. Đây là nơi phù hợp cho quy tắc, mức ưu tiên và chi tiết về "cách hành xử".
  </Accordion>
  ...
  <Accordion title="IDENTITY.md - name, vibe, emoji">
    Tên, phong thái và emoji của tác nhân. Được tạo/cập nhật trong nghi thức bootstrap.
  </Accordion>
  <Accordion title="TOOLS.md - local tool conventions">
    Ghi chú về công cụ cục bộ và quy ước của bạn. Không kiểm soát tính khả dụng của công cụ; chỉ là hướng dẫn.
  </Accordion>
  ...
  <Accordion title="BOOT.md - startup checklist">
    Danh sách kiểm tra khởi động tùy chọn, chạy tự động khi Gateway khởi động lại (khi [hook nội bộ](/vi/automation/hooks) được bật). Giữ ngắn; dùng công cụ tin nhắn cho các lượt gửi ra ngoài.
  </Accordion>
  <Accordion title="BOOTSTRAP.md - first-run ritual">
    Nghi thức chạy lần đầu một lần duy nhất. Chỉ được tạo cho một không gian làm việc hoàn toàn mới. Xóa tệp này sau khi hoàn tất nghi thức.
  </Accordion>
  ...
  <Accordion title="MEMORY.md - curated long-term memory (optional)">
    Bộ nhớ dài hạn đã tuyển chọn: các sự kiện, tùy chọn, quyết định và tóm tắt ngắn có độ bền. Giữ nhật ký chi tiết trong `memory/YYYY-MM-DD.md` để công cụ bộ nhớ có thể truy xuất theo yêu cầu mà không đưa chúng vào mọi prompt. Chỉ tải `MEMORY.md` trong phiên chính, riêng tư (không phải ngữ cảnh chia sẻ/nhóm). Xem [Bộ nhớ](/vi/concepts/memory) để biết workflow và cơ chế xả bộ nhớ tự động.
  </Accordion>
  <Accordion title="skills/ - workspace skills (optional)">
    Skills dành riêng cho không gian làm việc. Vị trí Skills có độ ưu tiên cao nhất cho không gian làm việc đó. Ghi đè Skills của tác nhân dự án, Skills của tác nhân cá nhân, Skills được quản lý, Skills đóng gói sẵn và `skills.load.extraDirs` khi tên trùng nhau.
  </Accordion>
  <Accordion title="canvas/ - Canvas UI files (optional)">
    Tệp giao diện người dùng Canvas cho hiển thị nút (ví dụ `canvas/index.html`).
  </Accordion>
</AccordionGroup>

<Note>
Nếu thiếu bất kỳ tệp bootstrap nào, OpenClaw sẽ chèn một dấu hiệu "missing file" vào phiên và tiếp tục. Các tệp bootstrap lớn sẽ bị cắt ngắn khi chèn; điều chỉnh giới hạn bằng `agents.defaults.bootstrapMaxChars` (mặc định: 20000) và `agents.defaults.bootstrapTotalMaxChars` (mặc định: 60000). `openclaw setup` có thể tạo lại các mặc định bị thiếu mà không ghi đè tệp hiện có.
</Note>
...
- `~/.openclaw/agents/<agentId>/agent/codex-home/` (tài khoản runtime Codex riêng cho từng tác nhân, cấu hình, Skills, Plugin và trạng thái luồng native)
...
Nếu bạn cần di chuyển phiên hoặc cấu hình, hãy sao chép chúng riêng và giữ chúng ngoài hệ thống quản lý phiên bản.

## Sao lưu Git (khuyến nghị, riêng tư)

Xem không gian làm việc như bộ nhớ riêng tư. Đưa nó vào một repo git **riêng tư** để có bản sao lưu và có thể khôi phục.

Chạy các bước này trên máy nơi Gateway chạy (đó là nơi không gian làm việc tồn tại).

<Steps>
  <Step title="Initialize the repo">
    Nếu đã cài git, các không gian làm việc hoàn toàn mới sẽ được khởi tạo tự động. Nếu không gian làm việc này chưa phải là repo, hãy chạy:
  ...
  <Step title="Add a private remote">
    <Tabs>
      <Tab title="GitHub web UI">
        ...
        2. Không khởi tạo với README (tránh xung đột hợp nhất).
...
- Bản dump thô của đoạn chat hoặc tệp đính kèm nhạy cảm.
...
<Steps>
  ...
  <Step title="Seed missing files">
    Chạy `openclaw setup --workspace <path>` để seed mọi tệp bị thiếu.
  </Step>
...
- Định tuyến đa tác nhân có thể dùng các không gian làm việc khác nhau cho từng tác nhân. Xem [Định tuyến kênh](/vi/channels/channel-routing) để biết cấu hình định tuyến.
