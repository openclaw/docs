...
read_when:
    ...
    - Bạn muốn cấu hình mẫu hoặc lệnh CLI để thiết lập ban đầu cho các nhà cung cấp mô hình
sidebarTitle: Model providers
summary: Tổng quan về nhà cung cấp mô hình với cấu hình ví dụ + quy trình CLI
title: Các nhà cung cấp mô hình
x-i18n:
...
Tham chiếu cho **nhà cung cấp LLM/mô hình** (không phải kênh trò chuyện như WhatsApp/Telegram). Để biết quy tắc chọn mô hình, xem [Mô hình](/vi/concepts/models).
...
<AccordionGroup>
  ...
  <Accordion title="OpenAI provider/runtime split">
    Các tuyến họ OpenAI phụ thuộc vào tiền tố:

    - `openai/<model>` mặc định dùng harness máy chủ ứng dụng Codex gốc cho lượt agent. Đây là thiết lập thuê bao ChatGPT/Codex thông thường.
    - `openai-codex/<model>` là cấu hình cũ mà doctor viết lại thành `openai/<model>`.
    - `openai/<model>` cộng với `agentRuntime.id: "pi"` của nhà cung cấp/mô hình dùng PI cho các tuyến khóa API rõ ràng hoặc tương thích.

    Xem [OpenAI](/vi/providers/openai) và [harness Codex](/vi/plugins/codex-harness). Nếu việc tách nhà cung cấp/runtime khó hiểu, hãy đọc [Runtime agent](/vi/concepts/agent-runtimes) trước.

    Tự động bật Plugin tuân theo cùng ranh giới: tham chiếu agent `openai/*` bật Plugin Codex cho tuyến mặc định, và `agentRuntime.id: "codex"` rõ ràng của nhà cung cấp/mô hình hoặc tham chiếu `codex/<model>` cũ cũng yêu cầu Plugin đó.

    GPT-5.5 mặc định có sẵn qua harness máy chủ ứng dụng Codex gốc trên `openai/gpt-5.5`, và qua PI chỉ khi chính sách runtime của nhà cung cấp/mô hình chọn rõ `pi`.

  </Accordion>
  <Accordion title="CLI runtimes">
    Runtime CLI dùng cùng cách tách: chọn tham chiếu mô hình chuẩn như `anthropic/claude-*`, `google/gemini-*`, hoặc `openai/gpt-*`, rồi đặt chính sách runtime của nhà cung cấp/mô hình thành `claude-cli`, `google-gemini-cli`, hoặc `codex-cli` khi bạn muốn backend CLI cục bộ.

    Các tham chiếu cũ `claude-cli/*`, `google-gemini-cli/*`, và `codex-cli/*` được di chuyển lại về tham chiếu nhà cung cấp chuẩn, với runtime được ghi riêng.
...
Hầu hết logic riêng cho từng nhà cung cấp nằm trong Plugin nhà cung cấp (`registerProvider(...)`) trong khi OpenClaw giữ vòng lặp suy luận chung. Plugin sở hữu onboarding, danh mục mô hình, ánh xạ biến môi trường xác thực, chuẩn hóa transport/cấu hình, dọn dẹp schema công cụ, phân loại chuyển đổi dự phòng, làm mới OAuth, báo cáo sử dụng, hồ sơ suy nghĩ/lập luận, và nhiều hơn nữa.

Danh sách đầy đủ các hook SDK nhà cung cấp và ví dụ Plugin đi kèm nằm trong [Plugin nhà cung cấp](/vi/plugins/sdk-provider-plugins). Một nhà cung cấp cần trình thực thi yêu cầu hoàn toàn tùy chỉnh là một bề mặt mở rộng riêng và sâu hơn.
...
<AccordionGroup>
  <Accordion title="Key sources and priority">
    ...
    Với nhà cung cấp Google, `GOOGLE_API_KEY` cũng được đưa vào làm dự phòng. Thứ tự chọn khóa giữ nguyên ưu tiên và loại bỏ giá trị trùng lặp.

  </Accordion>
  <Accordion title="When rotation kicks in">
    - Yêu cầu chỉ được thử lại với khóa tiếp theo khi có phản hồi giới hạn tốc độ (ví dụ `429`, `rate_limit`, `quota`, `resource exhausted`, `Too many concurrent requests`, `ThrottlingException`, `concurrency limit reached`, `workers_ai ... quota limit exceeded`, hoặc thông báo giới hạn sử dụng định kỳ).
...
- Xác minh tài khoản/tính khả dụng của mô hình bằng `openclaw models list --provider openai` nếu một cài đặt hoặc khóa API cụ thể hoạt động khác đi.
- CLI: `openclaw onboard --auth-choice openai-api-key`
- Transport mặc định là `auto`; OpenClaw chuyển lựa chọn transport cho pi-ai.
...
- Header ghi nhận ẩn của OpenClaw (`originator`, `version`, `User-Agent`) chỉ áp dụng trên lưu lượng OpenAI gốc tới `api.openai.com`, không áp dụng cho proxy tương thích OpenAI chung
- Các tuyến OpenAI gốc cũng giữ `store` của Responses, gợi ý prompt-cache, và định hình payload tương thích lập luận OpenAI; tuyến proxy thì không
- `openai/gpt-5.3-codex-spark` được chủ ý ẩn trong OpenClaw vì yêu cầu OpenAI API live từ chối nó và danh mục Codex hiện tại không hiển thị nó
...
- Yêu cầu Anthropic công khai trực tiếp hỗ trợ nút bật/tắt `/fast` dùng chung và `params.fastMode`, bao gồm lưu lượng xác thực bằng khóa API và OAuth gửi tới `api.anthropic.com`; OpenClaw ánh xạ điều đó sang `service_tier` của Anthropic (`auto` so với `standard_only`)
- Cấu hình Claude CLI ưu tiên giữ tham chiếu mô hình chuẩn và chọn backend CLI
  riêng: `anthropic/claude-opus-4-7` với
  `agentRuntime.id: "claude-cli"` theo phạm vi mô hình. Tham chiếu cũ
  `claude-cli/claude-opus-4-7` vẫn hoạt động để tương thích.

<Note>
Nhân viên Anthropic cho chúng tôi biết việc sử dụng Claude CLI kiểu OpenClaw lại được cho phép, vì vậy OpenClaw xem việc tái sử dụng Claude CLI và dùng `claude -p` là được chấp thuận cho tích hợp này, trừ khi Anthropic công bố chính sách mới. Setup-token Anthropic vẫn có sẵn như một đường dẫn token OpenClaw được hỗ trợ, nhưng OpenClaw hiện ưu tiên tái sử dụng Claude CLI và `claude -p` khi có.
</Note>
...
- Tham chiếu harness máy chủ ứng dụng Codex gốc: `openai/gpt-5.5`
- Tài liệu harness máy chủ ứng dụng Codex gốc: [harness Codex](/vi/plugins/codex-harness)
- Tham chiếu mô hình cũ: `codex/gpt-*`
- Ranh giới Plugin: `openai-codex/*` tải Plugin OpenAI; Plugin máy chủ ứng dụng Codex gốc chỉ được chọn bởi runtime harness Codex hoặc tham chiếu `codex/*` cũ.
...
- Header ghi nhận ẩn của OpenClaw (`originator`, `version`, `User-Agent`) chỉ được đính kèm trên lưu lượng Codex gốc tới `chatgpt.com/backend-api`, không áp dụng cho proxy tương thích OpenAI chung
- Chia sẻ cùng cấu hình nút bật/tắt `/fast` và `params.fastMode` như `openai/*` trực tiếp; OpenClaw ánh xạ điều đó sang `service_tier=priority`
...
- Với tuyến thuê bao phổ biến cộng runtime Codex gốc, đăng nhập bằng xác thực `openai-codex` nhưng cấu hình `openai/gpt-5.5`; lượt agent OpenAI chọn Codex theo mặc định.
- Chỉ dùng `agentRuntime.id: "pi"` của nhà cung cấp/mô hình khi bạn muốn tuyến tương thích qua PI; nếu không, giữ `openai/gpt-5.5` trên harness Codex mặc định.
- Các tham chiếu cũ `openai-codex/gpt-5.1*`, `openai-codex/gpt-5.2*`, và `openai-codex/gpt-5.3*` bị ẩn vì tài khoản OAuth ChatGPT/Codex từ chối chúng; thay vào đó dùng `openai-codex/gpt-5.5` hoặc tuyến runtime Codex gốc.
...
### Tùy chọn lưu trữ kiểu thuê bao khác

<CardGroup cols={3}>
  <Card title="GLM models" href="/vi/providers/glm">
    Gói Z.AI Coding Plan hoặc endpoint API chung.
  </Card>
...
- Bí danh: `google/gemini-3.1-pro` được chấp nhận và chuẩn hóa thành mã định danh Gemini API trực tiếp của Google, `google/gemini-3.1-pro-preview`
...
Các phản hồi JSON của Gemini CLI được phân tích từ `response`; usage dự phòng sang `stats`, với `stats.cached` được chuẩn hóa thành OpenClaw `cacheRead`.
...
- CLI: `openclaw onboard --auth-choice zai-api-key`
  ...
  - `zai-api-key` tự động phát hiện endpoint Z.AI khớp; `zai-coding-global`, `zai-coding-cn`, `zai-global`, và `zai-cn` buộc dùng một bề mặt cụ thể
...
- Catalog dự phòng tĩnh phát hành kèm `kilocode/kilo/auto`; cơ chế khám phá trực tiếp `https://api.kilo.ai/api/gateway/models` có thể mở rộng catalog runtime thêm nữa.
...
### Các Plugin nhà cung cấp đi kèm
...
<AccordionGroup>
  <Accordion title="OpenRouter">
    Chỉ áp dụng các header ghi nhận ứng dụng và marker Anthropic `cache_control` trên các route `openrouter.ai` đã xác minh. Các ref DeepSeek, Moonshot và ZAI đủ điều kiện cache-TTL cho bộ nhớ đệm prompt do OpenRouter quản lý nhưng không nhận marker cache của Anthropic. Là một đường dẫn kiểu proxy tương thích OpenAI, nó bỏ qua phần định dạng chỉ dành cho OpenAI gốc (`serviceTier`, Responses `store`, gợi ý bộ nhớ đệm prompt, tương thích suy luận OpenAI). Các ref dựa trên Gemini chỉ giữ quy trình làm sạch thought-signature proxy-Gemini.
  </Accordion>
  <Accordion title="Kilo Gateway">
    Các ref dựa trên Gemini đi theo cùng đường dẫn làm sạch proxy-Gemini; `kilocode/kilo/auto` và các ref proxy không hỗ trợ suy luận khác bỏ qua việc chèn suy luận proxy.
  </Accordion>
  <Accordion title="MiniMax">
    Quy trình onboarding bằng khóa API ghi các định nghĩa mô hình chat M2.7 chỉ văn bản rõ ràng; hiểu hình ảnh vẫn nằm trên nhà cung cấp media `MiniMax-VL-01` do Plugin sở hữu.
  </Accordion>
  <Accordion title="NVIDIA">
    ID mô hình dùng namespace `nvidia/<vendor>/<model>` (ví dụ `nvidia/nvidia/nemotron-...` cùng với `nvidia/moonshotai/kimi-k2.5`); các bộ chọn giữ nguyên thành phần `<provider>/<model-id>` theo nghĩa đen trong khi khóa chuẩn gửi tới API vẫn chỉ có một tiền tố.
  </Accordion>
  <Accordion title="xAI">
    Dùng đường dẫn xAI Responses. `grok-4.3` là mô hình chat mặc định được đóng gói. `/fast` hoặc `params.fastMode: true` ghi lại `grok-3`, `grok-3-mini`, `grok-4`, và `grok-4-0709` thành các biến thể `*-fast` tương ứng. `tool_stream` bật theo mặc định; tắt qua `agents.defaults.models["xai/<model>"].params.tool_stream=false`.
  </Accordion>
  <Accordion title="Cerebras">
    Được phân phối dưới dạng Plugin nhà cung cấp `cerebras` đóng gói. GLM dùng `zai-glm-4.7`; URL cơ sở tương thích OpenAI là `https://api.cerebras.ai/v1`.
  </Accordion>
</AccordionGroup>
...
Nhiều Plugin nhà cung cấp đóng gói bên dưới đã công bố một catalog mặc định. Chỉ dùng các mục `models.providers.<id>` rõ ràng khi bạn muốn ghi đè URL cơ sở, header hoặc danh sách mô hình mặc định.

Các kiểm tra năng lực mô hình của Gateway cũng đọc metadata `models.providers.<id>.models[]` rõ ràng. Nếu một mô hình tùy chỉnh hoặc proxy chấp nhận hình ảnh, đặt `input: ["text", "image"]` trên mô hình đó để WebChat và các đường dẫn tệp đính kèm có nguồn gốc từ node chuyển hình ảnh dưới dạng đầu vào mô hình gốc thay vì ref media chỉ văn bản.

`agents.defaults.models["provider/model"]` chỉ kiểm soát khả năng hiển thị mô hình, alias và metadata theo mô hình cho agent. Nó không tự đăng ký mô hình runtime mới. Đối với mô hình nhà cung cấp tùy chỉnh, cũng thêm `models.providers.<provider>.models[]` với ít nhất `id` khớp.

### Moonshot AI (Kimi)

Moonshot được phân phối dưới dạng Plugin nhà cung cấp đóng gói. Dùng nhà cung cấp tích hợp theo mặc định, và chỉ thêm mục `models.providers.moonshot` rõ ràng khi bạn cần ghi đè URL cơ sở hoặc metadata mô hình:
...
Quy trình onboarding mặc định dùng bề mặt lập trình, nhưng catalog `volcengine/*` tổng quát cũng được đăng ký cùng lúc.

Trong các bộ chọn mô hình khi onboarding/cấu hình, lựa chọn xác thực Volcengine ưu tiên cả các hàng `volcengine/*` và `volcengine-plan/*`. Nếu các mô hình đó chưa được tải, OpenClaw sẽ quay về catalog chưa lọc thay vì hiển thị bộ chọn trống được giới hạn theo nhà cung cấp.
...
Quy trình onboarding mặc định dùng bề mặt lập trình, nhưng catalog `byteplus/*` tổng quát cũng được đăng ký cùng lúc.

Trong các bộ chọn mô hình khi onboarding/cấu hình, lựa chọn xác thực BytePlus ưu tiên cả các hàng `byteplus/*` và `byteplus-plan/*`. Nếu các mô hình đó chưa được tải, OpenClaw sẽ quay về catalog chưa lọc thay vì hiển thị bộ chọn trống được giới hạn theo nhà cung cấp.
...
<Note>
Trên đường dẫn phát trực tuyến tương thích Anthropic của MiniMax, OpenClaw tắt thinking theo mặc định trừ khi bạn đặt rõ ràng, và `/fast on` viết lại `MiniMax-M2.7` thành `MiniMax-M2.7-highspeed`.
</Note>
...
<AccordionGroup>
  <Accordion title="Trường tùy chọn mặc định">
    Với các nhà cung cấp tùy chỉnh, `reasoning`, `input`, `cost`, `contextWindow` và `maxTokens` là tùy chọn. Khi bị bỏ qua, OpenClaw mặc định là:
  ...
  <Accordion title="Quy tắc định hình tuyến proxy">
    - Với `api: "openai-completions"` trên các điểm cuối không phải gốc (bất kỳ `baseUrl` không rỗng nào có host không phải `api.openai.com`), OpenClaw buộc `compat.supportsDeveloperRole: false` để tránh lỗi 400 từ nhà cung cấp đối với các vai trò `developer` không được hỗ trợ.
    - Các tuyến tương thích OpenAI kiểu proxy cũng bỏ qua việc định hình yêu cầu chỉ dành riêng cho OpenAI gốc: không có `service_tier`, không có Responses `store`, không có Completions `store`, không có gợi ý prompt-cache, không có định hình payload tương thích reasoning của OpenAI, và không có header quy nguồn OpenClaw ẩn.
    - Với các proxy Completions tương thích OpenAI cần trường dành riêng cho nhà cung cấp, đặt `agents.defaults.models["provider/model"].params.extra_body` (hoặc `extraBody`) để hợp nhất JSON bổ sung vào body yêu cầu gửi đi.
    ...
    - Với các mô hình cục bộ chậm hoặc host LAN/tailnet từ xa, đặt `models.providers.<id>.timeoutSeconds`. Thiết lập này mở rộng xử lý yêu cầu HTTP của nhà cung cấp mô hình, bao gồm kết nối, header, phát trực tuyến body và tổng thời gian hủy guarded-fetch, mà không tăng thời gian chờ runtime của toàn bộ agent.
    - Các lệnh gọi HTTP tới nhà cung cấp mô hình cho phép câu trả lời DNS fake-IP của Surge, Clash và sing-box trong `198.18.0.0/15` và `fc00::/7` chỉ cho hostname `baseUrl` của nhà cung cấp đã cấu hình. Các đích riêng tư, loopback, link-local và metadata khác vẫn cần chọn tham gia rõ ràng bằng `models.providers.<id>.request.allowPrivateNetwork: true`.
    - Nếu `baseUrl` rỗng/bị bỏ qua, OpenClaw giữ hành vi OpenAI mặc định (phân giải tới `api.openai.com`).
    - Vì an toàn, `compat.supportsDeveloperRole: true` được đặt rõ ràng vẫn bị ghi đè trên các điểm cuối `openai-completions` không phải gốc.
    - Với `api: "anthropic-messages"` trên các điểm cuối không trực tiếp (bất kỳ nhà cung cấp nào khác `anthropic` chuẩn, hoặc một `models.providers.anthropic.baseUrl` tùy chỉnh có host không phải điểm cuối `api.anthropic.com` công khai), OpenClaw sẽ chặn các header beta Anthropic ngầm định như `claude-code-20250219`, `interleaved-thinking-2025-05-14` và dấu hiệu OAuth, để các proxy tùy chỉnh tương thích Anthropic không từ chối các cờ beta không được hỗ trợ. Đặt rõ `models.providers.<id>.headers["anthropic-beta"]` nếu proxy của bạn cần các tính năng beta cụ thể.
