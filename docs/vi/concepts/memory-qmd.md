...
cùng OpenClaw. Nó kết hợp BM25, tìm kiếm vector và xếp hạng lại trong một
binary duy nhất, đồng thời có thể lập chỉ mục nội dung ngoài các tệp bộ nhớ
workspace của bạn.
...
- **Tự động dự phòng** -- nếu QMD không khả dụng, OpenClaw sẽ chuyển về công cụ
  tích hợp sẵn một cách liền mạch.
...
- QMD phải có trong `PATH` của gateway.
- macOS và Linux hoạt động ngay lập tức. Windows được hỗ trợ tốt nhất qua WSL2.
...
OpenClaw tạo một home QMD tự chứa trong
...
Nó ưu tiên các dạng collection và truy vấn MCP hiện tại của QMD, nhưng vẫn dự phòng về
các cờ mẫu collection thay thế và tên công cụ MCP cũ hơn khi cần.
Quá trình đối chiếu khi khởi động cũng tạo lại các collection được quản lý đã lỗi thời về
các mẫu chính tắc của chúng khi một collection QMD cũ hơn có cùng tên vẫn còn
tồn tại.
...
- OpenClaw tạo collection từ các tệp bộ nhớ workspace của bạn và mọi
  ...
  được mở và định kỳ sau đó (mặc định 5 phút một lần). Các lần làm mới này
  chạy qua subprocess QMD, không phải một lượt crawl hệ thống tệp trong tiến trình.
  Các chế độ semantic cũng chạy `qmd embed`.
- Collection workspace mặc định theo dõi `MEMORY.md` cộng với cây `memory/`.
  `memory.md` viết thường không được lập chỉ mục làm tệp bộ nhớ gốc.
- Scanner riêng của QMD bỏ qua các đường dẫn ẩn và các thư mục dependency/build phổ biến
  như `.git`, `.cache`, `node_modules`, `vendor`, `dist` và
  `build`. Khởi động Gateway không khởi tạo QMD theo mặc định, nên khởi động lạnh
  tránh import runtime bộ nhớ hoặc tạo watcher sống lâu trước khi bộ nhớ được dùng lần đầu.
- Nếu bạn vẫn muốn QMD được khởi tạo khi gateway khởi động, hãy đặt
  `memory.qmd.update.startup` thành `idle` hoặc `immediate`. Với
  `memory.qmd.update.onBoot: true`, khởi động sẽ chạy lần làm mới ban đầu. Với
  `onBoot: false`, khởi động bỏ qua lần làm mới ngay lập tức đó nhưng vẫn mở
  trình quản lý sống lâu khi các khoảng thời gian cập nhật hoặc embed được cấu hình, để QMD có thể
  sở hữu watcher và bộ hẹn giờ định kỳ của nó.
- Tìm kiếm dùng `searchMode` đã cấu hình (mặc định: `search`; cũng hỗ trợ
  ...
  kiểm tra sẵn sàng vector semantic và bảo trì embedding trong chế độ đó. Nếu một chế độ
  thất bại, OpenClaw thử lại bằng `qmd query`.
- Khi `searchMode` là `query`, đặt `memory.qmd.rerank` thành `false` để dùng đường dẫn
  truy vấn hybrid của QMD mà không có reranker. OpenClaw truyền `--no-rerank` cho
  đường dẫn CLI QMD trực tiếp và `rerank: false` cho công cụ truy vấn MCP của QMD. Tùy chọn này
  yêu cầu QMD 2.1 trở lên.
- Với các bản phát hành QMD quảng bá bộ lọc nhiều collection, OpenClaw nhóm
  các collection cùng nguồn vào một lượt gọi tìm kiếm QMD. Các bản phát hành QMD cũ hơn
  giữ đường dự phòng tương thích theo từng collection.
- Nếu QMD thất bại hoàn toàn, OpenClaw chuyển về công cụ SQLite tích hợp sẵn.
  Các lần thử lặp lại trong lượt chat sẽ lùi lại một thời gian ngắn sau lỗi mở để một
  binary bị thiếu hoặc dependency sidecar bị hỏng không tạo ra một cơn bão thử lại;
  `openclaw memory status` và các probe CLI chạy một lần vẫn kiểm tra lại QMD trực tiếp.
...
Các collection bản ghi phiên vẫn ở trong nhóm nguồn riêng, nên các tìm kiếm hỗn hợp
`memory` + `sessions` vẫn cung cấp đầu vào cho bộ đa dạng hóa kết quả từ cả hai
nguồn.
...
riêng trước khi hợp nhất và khử trùng lặp kết quả.

Để kiểm tra contract đã cài đặt theo cách thủ công, chạy:
...
nên bạn có thể tinh chỉnh QMD toàn cục mà không thêm cấu hình OpenClaw mới:
...
Sau khi đổi mô hình embedding, hãy chạy lại embeddings để chỉ mục khớp với
...
riêng dưới `~/.openclaw/agents/<id>/qmd/sessions/`.
...
Khi phạm vi từ chối một tìm kiếm, OpenClaw ghi log cảnh báo với kênh và
loại chat được suy ra để dễ gỡ lỗi kết quả trống hơn.
...
chạy như một service, hãy tạo symlink:
`sudo ln -s ~/.bun/bin/qmd /usr/local/bin/qmd`.

Nếu `qmd --version` hoạt động trong shell của bạn nhưng OpenClaw vẫn báo cáo
`spawn qmd ENOENT`, tiến trình gateway có khả năng có `PATH` khác với
...
**Nhiều subprocess QMD trong khi tìm kiếm?** Cập nhật QMD nếu có thể. OpenClaw dùng
một tiến trình cho các tìm kiếm nhiều collection cùng nguồn chỉ khi QMD đã cài đặt
quảng bá hỗ trợ nhiều bộ lọc `-c`; nếu không, nó giữ đường dự phòng cũ hơn
theo từng collection để đảm bảo đúng đắn.

**QMD chỉ dùng BM25 vẫn cố build llama.cpp?** Đặt
`memory.qmd.searchMode = "search"`. OpenClaw xem chế độ đó là chỉ lexical,
không chạy probe trạng thái vector QMD hoặc bảo trì embedding, và để
các kiểm tra sẵn sàng semantic cho thiết lập `vsearch` hoặc `query`.

**Tìm kiếm bị hết thời gian?** Tăng `memory.qmd.limits.timeoutMs` (mặc định: 4000ms).
Đặt thành `120000` cho phần cứng chậm hơn.

**Kết quả trống trong chat nhóm?** Kiểm tra `memory.qmd.scope` -- mặc định chỉ
cho phép các phiên trực tiếp và kênh.

**Tìm kiếm bộ nhớ gốc đột nhiên quá rộng?** Khởi động lại gateway hoặc chờ
lần đối chiếu khởi động tiếp theo. OpenClaw tạo lại các collection được quản lý đã lỗi thời
về các mẫu `MEMORY.md` và `memory/` chính tắc khi phát hiện xung đột cùng tên.

**Repo tạm hiển thị trong workspace gây `ENAMETOOLONG` hoặc lập chỉ mục bị hỏng?**
Traversal của QMD hiện đi theo hành vi scanner QMD bên dưới thay vì
...
traversal an toàn với chu trình hoặc điều khiển loại trừ tường minh.

## Cấu hình

Để xem toàn bộ bề mặt cấu hình (`memory.qmd.*`), các chế độ tìm kiếm, khoảng thời gian cập nhật,
quy tắc phạm vi và mọi núm điều chỉnh khác, xem
