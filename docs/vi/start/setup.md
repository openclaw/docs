...
- **Quy trình mới nhất (dev):** tự chạy Gateway qua `pnpm gateway:watch`, rồi để ứng dụng macOS kết nối ở chế độ Cục bộ.

## Điều kiện tiên quyết (từ mã nguồn)

- Khuyến nghị Node 24 (Node 22 LTS, hiện là `22.14+`, vẫn được hỗ trợ)
- Cần có `pnpm` cho các checkout mã nguồn. OpenClaw tải các plugin đi kèm từ các gói workspace pnpm `extensions/*` ở chế độ dev, vì vậy `npm install` ở root không chuẩn bị đầy đủ cây mã nguồn.
...
Nếu bạn muốn “100% tùy chỉnh cho tôi” _và_ dễ cập nhật, hãy giữ tùy chỉnh của bạn trong:

- **Cấu hình:** `~/.openclaw/openclaw.json` (kiểu JSON/JSON5)
- **Workspace:** `~/.openclaw/workspace` (skills, prompts, memories; biến nó thành repo git riêng tư)
...
### 0) (Tùy chọn) Cũng chạy ứng dụng macOS từ mã nguồn
...
`gateway:watch` khởi động hoặc khởi động lại tiến trình theo dõi Gateway trong một phiên tmux có tên và tự động attach từ các terminal tương tác. Các shell không tương tác vẫn ở trạng thái detached và in ra `tmux attach -t openclaw-gateway-watch-main`; dùng `OPENCLAW_GATEWAY_WATCH_ATTACH=0 pnpm gateway:watch` để giữ một lần chạy tương tác ở trạng thái detached, hoặc `pnpm gateway:watch:raw` cho chế độ theo dõi foreground. Watcher tải lại khi có thay đổi liên quan trong mã nguồn, cấu hình và metadata plugin đi kèm. Nếu Gateway được theo dõi thoát trong lúc khởi động, `gateway:watch` chạy `openclaw doctor --fix --non-interactive` một lần rồi thử lại; đặt `OPENCLAW_GATEWAY_WATCH_AUTO_DOCTOR=0` để tắt lượt sửa chữa chỉ dành cho dev đó.
`pnpm openclaw setup` là bước khởi tạo cấu hình/workspace cục bộ một lần cho checkout mới.
`pnpm gateway:watch` không rebuild `dist/control-ui`, vì vậy hãy chạy lại `pnpm ui:build` sau các thay đổi trong `ui/` hoặc dùng `pnpm ui:dev` trong khi phát triển Control UI.
...
Dùng phần này khi debug auth hoặc quyết định cần sao lưu gì:

- **WhatsApp**: `~/.openclaw/credentials/whatsapp/<accountId>/creds.json`
- **Token bot Telegram**: config/env hoặc `channels.telegram.tokenFile` (chỉ file thông thường; symlink bị từ chối)
- **Token bot Discord**: config/env hoặc SecretRef (nhà cung cấp env/file/exec)
- **Token Slack**: config/env (`channels.slack.*`)
- **Danh sách cho phép pairing**:
...
- **Import OAuth kế thừa**: `~/.openclaw/credentials/oauth.json`
  Chi tiết thêm: [Bảo mật](/vi/gateway/security#credential-storage-map).

## Cập nhật (không phá thiết lập của bạn)
...
Các bản cài đặt Linux dùng dịch vụ **user** systemd. Theo mặc định, systemd dừng các dịch vụ user khi logout/idle, làm tắt Gateway. Onboarding cố gắng bật lingering cho bạn (có thể nhắc nhập sudo). Nếu vẫn chưa bật, hãy chạy:

```bash
sudo loginctl enable-linger $USER
```

Với máy chủ luôn bật hoặc nhiều người dùng, hãy cân nhắc dùng dịch vụ **system** thay vì dịch vụ user (không cần lingering). Xem [Runbook Gateway](/vi/gateway) để biết ghi chú về systemd.
