---
read_when:
    - Bạn cần biết hành vi chi tiết của một bước `openclaw onboard` cụ thể
...
x-i18n:
    generated_at: "2026-07-19T06:02:27Z"
    model: gpt-5.6
...
Trang này trình bày chi tiết hành vi, đầu ra và cơ chế nội bộ của quy trình thiết lập ban đầu theo từng bước.
...
từng nhà cung cấp), hãy xem [`openclaw onboard`](/vi/cli/onboard).

## Trình hướng dẫn thực hiện những gì

Chế độ cục bộ (mặc định) hướng dẫn bạn qua:

- Thiết lập mô hình và xác thực (Anthropic, OAuth gói đăng ký OpenAI Code, xAI, OpenCode, điểm cuối tùy chỉnh và các luồng xác thực khác do nhà cung cấp sở hữu)
...
- Cài đặt daemon (LaunchAgent, đơn vị người dùng systemd hoặc Windows Scheduled Task gốc với phương án dự phòng là thư mục Startup)
- Kiểm tra tình trạng
- Thiết lập Skills
...
<Steps>
  ...
  <Step title="Không gian làm việc">
    ...
    - Khởi tạo các tệp không gian làm việc cần thiết cho lần chạy đầu tiên.
    - Bố cục không gian làm việc: [Không gian làm việc của tác tử](/vi/concepts/agent-workspace).

  </Step>
  <Step title="Gateway">
    - Yêu cầu nhập cổng, liên kết, chế độ xác thực và phạm vi truy cập qua Tailscale.
    - Khuyến nghị: duy trì bật xác thực bằng token ngay cả với loopback để các máy khách WS cục bộ phải xác thực.
    - Trong chế độ token, quy trình thiết lập tương tác cung cấp:
      - **Tạo/lưu token dạng văn bản thuần** (mặc định)
      - **Dùng SecretRef** (tùy chọn tham gia)
    - Trong chế độ mật khẩu, quy trình thiết lập tương tác cũng hỗ trợ lưu trữ dạng văn bản thuần hoặc SecretRef.
  ...
  <Step title="Cài đặt daemon">
    ...
    - Windows gốc: ưu tiên Scheduled Task
      - Nếu việc tạo tác vụ bị từ chối, OpenClaw sẽ dùng phương án dự phòng là mục đăng nhập theo người dùng trong thư mục Startup và khởi động Gateway ngay lập tức.
      - Scheduled Tasks vẫn được ưu tiên vì cung cấp trạng thái trình giám sát tốt hơn.
  ...
  <Step title="Kiểm tra tình trạng">
    ...
    - `openclaw status --deep` thêm phép thăm dò tình trạng Gateway trực tiếp vào đầu ra trạng thái, bao gồm cả phép thăm dò kênh khi được hỗ trợ.

  </Step>
  <Step title="Skills">
    - Đọc các kỹ năng hiện có và kiểm tra yêu cầu.
    - Cho phép bạn chọn trình quản lý Node: npm, pnpm hoặc bun.
    - Cài đặt các phần phụ thuộc tùy chọn cho các kỹ năng đi kèm đáng tin cậy khi
      trình cài đặt bắt buộc có sẵn.
    - Bỏ qua các trình cài đặt Homebrew, uv và Go không có sẵn, sau đó nhóm các
      kỹ năng bị ảnh hưởng cùng với hướng dẫn thiết lập thủ công. Chạy `openclaw doctor` sau khi cài đặt
      các điều kiện tiên quyết còn thiếu.

  </Step>
  <Step title="Hoàn tất">
    - Bản tóm tắt và các bước tiếp theo, bao gồm các tùy chọn ứng dụng iOS, Android và macOS.

  </Step>
</Steps>

<Note>
Nếu không phát hiện được GUI, trình hướng dẫn sẽ in hướng dẫn chuyển tiếp cổng SSH cho giao diện điều khiển thay vì mở trình duyệt.
...
<Steps>
  <Step title="Khám phá (tùy chọn)">
    ...
    sẽ đề nghị tìm kiếm các beacon Gateway Bonjour/mDNS trước khi chuyển sang
  ...
  <Step title="Phương thức kết nối">
    Khi chọn một beacon, hãy chọn WebSocket trực tiếp hoặc đường hầm SSH:
    - **Trực tiếp**: kết nối qua `wss://` và yêu cầu tin cậy dấu vân tay
      TLS được phát hiện (ghim theo cơ chế tin cậy trong lần sử dụng đầu tiên; chỉ ghim nếu bạn chấp nhận).
...
<Note>
Nếu Gateway chỉ dùng loopback và không thể khám phá, hãy dùng đường hầm SSH hoặc tailnet theo cách thủ công.
`ws://` dạng văn bản thuần được chấp nhận đối với loopback, địa chỉ IP riêng dạng ký tự, `.local` và URL `*.ts.net` của Tailnet; các tên DNS riêng khác cần `OPENCLAW_ALLOW_INSECURE_PRIVATE_WS=1`.
</Note>
...
thay vì thoát. Các lần chạy `--auth-choice` rõ ràng vẫn dừng ngay khi có lỗi để phục vụ tự động hóa.
...
<AccordionGroup>
  ...
  <Accordion title="CLI Anthropic Claude">
    Đường dẫn cục bộ được ưu tiên trong quy trình thiết lập ban đầu/cấu hình tương tác; tái sử dụng phiên đăng nhập CLI Claude hiện có khi khả dụng.
  </Accordion>
  ...
  <Accordion title="Khóa API OpenAI">
    ...
    Việc thêm hoặc xác thực lại OpenAI giữ nguyên mô hình chính được chỉ định rõ
    hiện có, bao gồm `openai/gpt-5.5`. Nếu tài khoản không cung cấp GPT-5.6,
    hãy chọn `openai/gpt-5.5` một cách rõ ràng; OpenClaw không âm thầm hạ cấp mô hình đó.

  </Accordion>
  <Accordion title="OAuth xAI (Grok)">
    Đăng nhập bằng trình duyệt dành cho các tài khoản SuperGrok hoặc X Premium đủ điều kiện. Đây là
  ...
  <Accordion title="Mã thiết bị xAI (Grok)">
    Đăng nhập bằng trình duyệt phù hợp với môi trường từ xa bằng một mã ngắn thay vì lệnh gọi lại
  ...
  <Accordion title="MiniMax">
    Cấu hình được tự động ghi. Giá trị mặc định khi lưu trữ trên máy chủ là `MiniMax-M3`; thiết lập bằng khóa API sử dụng
  ...
  <Accordion title="Ollama (các mô hình mở trên đám mây và cục bộ)">
    ...
    `Cloud only` sử dụng `OLLAMA_API_KEY` cùng với `https://ollama.com`.
    Các chế độ dựa trên máy chủ yêu cầu URL cơ sở (mặc định `http://127.0.0.1:11434`), phát hiện các mô hình có sẵn và đề xuất giá trị mặc định.
  ...
  <Accordion title="Nhà cung cấp tùy chỉnh">
    Hoạt động với các điểm cuối tương thích với OpenAI, OpenAI Responses và Anthropic.

    Quy trình khởi tạo tương tác hỗ trợ các lựa chọn lưu trữ khóa API giống như những luồng khóa API của nhà cung cấp khác:
    ...
    Quy trình khởi tạo suy luận khả năng hỗ trợ hình ảnh đối với các ID mô hình thị giác phổ biến (GPT-4o/4.1/5.x, Claude 3/4, Gemini, Qwen-VL, LLaVA, Pixtral và các mô hình tương tự) và chỉ hỏi khi không nhận diện được tên mô hình.

    Các cờ chế độ không tương tác:
    - `--auth-choice custom-api-key`
    - `--custom-base-url`
    - `--custom-model-id`
    - `--custom-api-key` (không bắt buộc; dự phòng về `CUSTOM_API_KEY`)
    - `--custom-provider-id` (không bắt buộc)
    - `--custom-compatibility <openai|openai-responses|anthropic>` (không bắt buộc; mặc định `openai`)
    - `--custom-image-input` / `--custom-text-input` (không bắt buộc; ghi đè khả năng đầu vào mô hình được suy luận)
...
- Chọn mô hình mặc định trong các tùy chọn được phát hiện hoặc nhập nhà cung cấp và mô hình theo cách thủ công.
- Khi quy trình khởi tạo bắt đầu từ một lựa chọn xác thực nhà cung cấp, trình chọn mô hình sẽ tự động ưu tiên
  ...
  cũng khớp với các biến thể gói lập trình của họ (`volcengine-plan/*`,
  `byteplus-plan/*`).
- Nếu bộ lọc nhà cung cấp ưu tiên đó không có kết quả, trình chọn sẽ quay về
  toàn bộ danh mục thay vì không hiển thị mô hình nào.
- Trình hướng dẫn chạy kiểm tra mô hình và cảnh báo nếu mô hình đã cấu hình không được nhận diện hoặc thiếu xác thực.
...
- Hành vi khởi tạo mặc định lưu khóa API dưới dạng giá trị văn bản thuần trong hồ sơ xác thực.
- `--secret-input-mode ref` bật chế độ tham chiếu thay cho việc lưu khóa dưới dạng văn bản thuần.
  Trong quá trình thiết lập tương tác, bạn có thể chọn một trong hai:
...
- Chế độ tham chiếu tương tác chạy xác thực sơ bộ nhanh trước khi lưu.
  ...
  - Nếu xác thực sơ bộ thất bại, quy trình khởi tạo sẽ hiển thị lỗi và cho phép bạn thử lại.
- Ở chế độ không tương tác, `--secret-input-mode ref` chỉ được hỗ trợ bởi biến môi trường.
  ...
  - Các cờ khóa nội tuyến (ví dụ: `--openai-api-key`) yêu cầu phải đặt biến môi trường đó; nếu không, quy trình khởi tạo sẽ thất bại ngay.
  - Đối với nhà cung cấp tùy chỉnh, chế độ không tương tác `ref` lưu `models.providers.<id>.apiKey` dưới dạng `{ source: "env", provider: "default", id: "CUSTOM_API_KEY" }`.
...
- Thông tin xác thực Gateway hỗ trợ lựa chọn văn bản thuần và SecretRef trong quá trình thiết lập tương tác:
...
<Note>
Mẹo dành cho môi trường không giao diện và máy chủ: hoàn tất OAuth trên một máy có trình duyệt, sau đó sao chép
...
- `session.dmScope` (quy trình khởi tạo giữ nguyên các giá trị tường minh và nếu không thì để trường này chưa đặt, nhờ đó giá trị mặc định `main` giữ tất cả tin nhắn trực tiếp trên các kênh trong phiên chính cuốn chiếu của agent—đây là mặc định dành cho agent cá nhân. Đối với hộp thư đến dùng chung hoặc nhiều người dùng, hãy sử dụng `per-channel-peer`; `openclaw security audit` khuyến nghị cách ly khi phát hiện lưu lượng tin nhắn trực tiếp từ nhiều người dùng)
- `channels.telegram.botToken`, `channels.discord.token`, `channels.matrix.*`, `channels.signal.*`, `channels.imessage.*`
- Danh sách cho phép của kênh (Discord, iMessage, Signal, Slack, Telegram, WhatsApp) khi bạn chọn tham gia trong lời nhắc; Discord và Slack cũng phân giải các tên đã nhập thành ID
...
`openclaw agents add` ghi `agents.list[]` và `bindings` không bắt buộc.
...
Sau khi kiểm tra quyền truy cập mô hình thành công, quy trình khởi tạo tương tác cổ điển trên macOS quét tên ứng dụng và ID gói mà không yêu cầu quyền riêng tư của macOS. Quy trình này tìm kiếm trong các danh mục plugin chính thức và ClawHub, sau đó yêu cầu mô hình đã cấu hình loại bỏ các kết quả khớp tên sai và đề xuất các plugin hoặc skills phù hợp. Các kết quả khớp được đề xuất được chọn theo mặc định; các kết quả khớp không bắt buộc cần được chọn tường minh.

Màn hình kết quả liệt kê các ứng dụng được phát hiện và hiển thị: "Tên ứng dụng được đối chiếu bằng mô hình đã cấu hình của bạn và tìm kiếm ClawHub." Đặt `wizard.appRecommendations` thành `false` để tắt cả bước khởi tạo này lẫn quyền truy cập của Gateway vào danh mục ứng dụng của node. Quá trình quét không được sử dụng trong quy trình bắt đầu nhanh hoặc khởi tạo không phải macOS.

## Thiết lập không tương tác

`--non-interactive` yêu cầu `--accept-risk` (xác nhận rằng các agent có
quyền năng lớn và quyền truy cập toàn bộ hệ thống tiềm ẩn rủi ro):
...
Tài liệu tham khảo đầy đủ về các cờ và ví dụ dành riêng cho nhà cung cấp: [`openclaw onboard`](/vi/cli/onboard), [Tự động hóa CLI](/vi/start/wizard-cli-automation).
...
Các ứng dụng khách (ứng dụng macOS và giao diện điều khiển) có thể hiển thị các bước mà không cần triển khai lại logic khởi tạo.

## Hành vi thiết lập Signal

- Tải hiện vật phát hành phù hợp từ các bản phát hành GitHub chính thức của `signal-cli` (bản dựng gốc, chỉ Linux x86-64)
...
- Windows gốc chưa được hỗ trợ; hãy chạy quy trình khởi tạo trong WSL2 để có đường dẫn cài đặt Linux
