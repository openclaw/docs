...
summary: Cấu hình công cụ (chính sách, các tùy chọn bật/tắt thử nghiệm, công cụ được hỗ trợ bởi nhà cung cấp) và thiết lập nhà cung cấp/URL cơ sở tùy chỉnh
...
`tools.*` các khóa cấu hình và thiết lập nhà cung cấp tùy chỉnh / base-URL. Đối với tác nhân, kênh và các khóa cấu hình cấp cao nhất khác, hãy xem [Tham chiếu cấu hình](/vi/gateway/configuration-reference).
...
<Note>
Quy trình onboarding cục bộ mặc định các cấu hình cục bộ mới thành `tools.profile: "coding"` khi chưa đặt (các hồ sơ rõ ràng hiện có được giữ nguyên).
</Note>
...
| `group:openclaw`   | Tất cả công cụ tích hợp (không bao gồm provider plugins)                                                                |
...
- `/elevated on|off|ask|full` lưu trạng thái theo từng phiên; chỉ thị nội dòng áp dụng cho một tin nhắn duy nhất.
...
Các kiểm tra an toàn vòng lặp công cụ **bị tắt theo mặc định**. Đặt `enabled: true` để kích hoạt phát hiện. Thiết lập có thể được định nghĩa toàn cục trong `tools.loopDetection` và ghi đè theo từng tác nhân tại `agents.list[].tools.loopDetection`.
...
Cấu hình khả năng hiểu phương tiện đến (hình ảnh/âm thanh/video):
...
<AccordionGroup>
  <Accordion title="Media model entry fields">
    **Mục nhập nhà cung cấp** (`type: "provider"` hoặc được bỏ qua):
    ...
    **Mục nhập CLI** (`type: "cli"`):
    ...
    - `args`: đối số theo mẫu (hỗ trợ `{{MediaPath}}`, `{{Prompt}}`, `{{MaxChars}}`, v.v.; `openclaw doctor --fix` di chuyển các phần giữ chỗ `{input}` đã lỗi thời sang `{{MediaPath}}`)
    ...
    **Trường chung:**
    ...
    - `tools.media.image.timeoutSeconds` và các mục nhập `timeoutSeconds` của mô hình hình ảnh tương ứng cũng áp dụng khi tác nhân gọi công cụ `image` rõ ràng.
    ...
    **Trường hoàn tất bất đồng bộ:**
    ...
    - `asyncCompletion.directSend`: cờ tương thích đã lỗi thời. Các tác vụ phương tiện bất đồng bộ đã hoàn tất vẫn được trung gian qua phiên của người yêu cầu để tác nhân nhận kết quả, quyết định cách thông báo cho người dùng và dùng công cụ tin nhắn khi việc gửi từ nguồn yêu cầu điều đó.
...
<AccordionGroup>
  <Accordion title="Visibility scopes">
    ...
    - `agent`: bất kỳ phiên nào thuộc id tác nhân hiện tại (có thể bao gồm người dùng khác nếu bạn chạy các phiên theo người gửi dưới cùng một id tác nhân).
    - `all`: bất kỳ phiên nào. Nhắm mục tiêu liên tác nhân vẫn yêu cầu `tools.agentToAgent`.
    - Giới hạn sandbox: khi phiên hiện tại đang ở trong sandbox và `agents.defaults.sandbox.sessionToolsVisibility="spawned"`, phạm vi hiển thị bị ép thành `tree` ngay cả khi `tools.sessions.visibility="all"`.
...
<AccordionGroup>
  <Accordion title="Ghi chú về tệp đính kèm">
    - Tệp đính kèm chỉ được hỗ trợ cho `runtime: "subagent"`. ACP runtime từ chối chúng.
    - Tệp được hiện thực hóa vào không gian làm việc con tại `.openclaw/attachments/<uuid>/` cùng với `.manifest.json`.
    - Nội dung tệp đính kèm được tự động biên tập khỏi phần lưu giữ bản ghi.
    - Đầu vào Base64 được xác thực bằng các kiểm tra nghiêm ngặt về bảng chữ cái/phần đệm và lớp bảo vệ kích thước trước khi giải mã.
...
Các cờ công cụ tích hợp thử nghiệm. Mặc định tắt trừ khi áp dụng quy tắc tự động bật strict-agentic GPT-5.
...
- Mặc định: `false` trừ khi `agents.defaults.embeddedPi.executionContract` (hoặc ghi đè theo từng agent) được đặt thành `"strict-agentic"` cho một lượt chạy OpenAI hoặc OpenAI Codex thuộc họ GPT-5. Đặt `true` để buộc bật công cụ ngoài phạm vi đó, hoặc `false` để giữ công cụ tắt ngay cả với các lượt chạy strict-agentic GPT-5.
- Khi được bật, prompt hệ thống cũng thêm hướng dẫn sử dụng để mô hình chỉ dùng công cụ này cho công việc đáng kể và duy trì tối đa một bước `in_progress`.
...
- `allowAgents`: danh sách cho phép mặc định gồm các ID agent đích cho `sessions_spawn` khi agent yêu cầu không đặt `subagents.allowAgents` của riêng nó (`["*"]` = bất kỳ; mặc định: chỉ cùng agent).
...
<AccordionGroup>
  <Accordion title="Xác thực và thứ tự ưu tiên hợp nhất">
    ...
    - Ghi đè gốc cấu hình agent bằng `OPENCLAW_AGENT_DIR` (hoặc `PI_CODING_AGENT_DIR`, bí danh biến môi trường cũ).
    - Thứ tự ưu tiên hợp nhất cho các ID nhà cung cấp khớp nhau:
      - Giá trị `baseUrl` không rỗng trong `models.json` của agent được ưu tiên.
      - Giá trị `apiKey` không rỗng trong agent chỉ được ưu tiên khi nhà cung cấp đó không được SecretRef quản lý trong ngữ cảnh cấu hình/auth-profile hiện tại.
      ...
      - `apiKey`/`baseUrl` của agent bị rỗng hoặc thiếu sẽ dùng lại `models.providers` trong cấu hình.
      - `contextWindow`/`maxTokens` của mô hình khớp nhau dùng giá trị cao hơn giữa cấu hình tường minh và giá trị danh mục ngầm định.
      - `contextTokens` của mô hình khớp nhau giữ nguyên giới hạn runtime tường minh khi có; dùng nó để giới hạn ngữ cảnh hiệu dụng mà không thay đổi metadata gốc của mô hình.
      - Dùng `models.mode: "replace"` khi bạn muốn cấu hình ghi lại hoàn toàn `models.json`.
      - Việc lưu giữ marker lấy nguồn làm thẩm quyền: marker được ghi từ ảnh chụp cấu hình nguồn đang hoạt động (trước phân giải), không phải từ giá trị secret runtime đã phân giải.
...
<AccordionGroup>
  <Accordion title="Danh mục cấp cao nhất">
    ...
    - `models.providers`: ánh xạ nhà cung cấp tùy chỉnh được đánh khóa bằng ID nhà cung cấp.
  ...
  <Accordion title="Kết nối và xác thực nhà cung cấp">
    - `models.providers.*.api`: bộ chuyển đổi yêu cầu (`openai-completions`, `openai-responses`, `anthropic-messages`, `google-generative-ai`, v.v.). Với các backend `/v1/chat/completions` tự lưu trữ như MLX, vLLM, SGLang và hầu hết máy chủ cục bộ tương thích OpenAI, dùng `openai-completions`. Nhà cung cấp tùy chỉnh có `baseUrl` nhưng không có `api` mặc định dùng `openai-completions`; chỉ đặt `openai-responses` khi backend hỗ trợ `/v1/responses`.
    - `models.providers.*.apiKey`: thông tin xác thực của nhà cung cấp (ưu tiên SecretRef/thay thế env).
  ...
  <Accordion title="Ghi đè truyền tải yêu cầu">
    ...
    - `request.proxy`: ghi đè HTTP proxy. Chế độ: `"env-proxy"` (dùng biến env `HTTP_PROXY`/`HTTPS_PROXY`), `"explicit-proxy"` (với `url`). Cả hai chế độ đều chấp nhận đối tượng con `tls` tùy chọn.
    ...
    - `request.allowPrivateNetwork`: khi `true`, cho phép HTTPS tới `baseUrl` khi DNS phân giải tới dải riêng tư, CGNAT hoặc tương tự, thông qua lớp bảo vệ fetch HTTP của nhà cung cấp (operator tự chọn bật cho các endpoint tự lưu trữ tương thích OpenAI đáng tin cậy). Các URL luồng nhà cung cấp mô hình local loopback như `localhost`, `127.0.0.1` và `[::1]` được tự động cho phép trừ khi trường này được đặt tường minh thành `false`; máy chủ LAN, tailnet và DNS riêng vẫn cần chọn bật. WebSocket dùng cùng `request` cho header/TLS nhưng không dùng cổng SSRF fetch đó. Mặc định `false`.

  </Accordion>
  <Accordion title="Mục danh mục mô hình">
    - `models.providers.*.models`: các mục danh mục mô hình tường minh của nhà cung cấp.
    - `models.providers.*.models.*.input`: phương thức đầu vào của mô hình. Dùng `["text"]` cho mô hình chỉ văn bản và `["text", "image"]` cho mô hình ảnh/vision gốc. Tệp đính kèm ảnh chỉ được chèn vào lượt agent khi mô hình được chọn được đánh dấu là có khả năng xử lý ảnh.
    - `models.providers.*.models.*.contextWindow`: metadata cửa sổ ngữ cảnh gốc của mô hình. Trường này ghi đè `contextWindow` cấp nhà cung cấp cho mô hình đó.
    - `models.providers.*.models.*.contextTokens`: giới hạn ngữ cảnh runtime tùy chọn. Trường này ghi đè `contextTokens` cấp nhà cung cấp; dùng nó khi bạn muốn ngân sách ngữ cảnh hiệu dụng nhỏ hơn `contextWindow` gốc của mô hình; `openclaw models list` hiển thị cả hai giá trị khi chúng khác nhau.
    - `models.providers.*.models.*.compat.supportsDeveloperRole`: gợi ý tương thích tùy chọn. Với `api: "openai-completions"` cùng `baseUrl` không rỗng và không phải gốc (host không phải `api.openai.com`), OpenClaw buộc giá trị này thành `false` tại runtime. `baseUrl` rỗng/bị bỏ qua giữ hành vi OpenAI mặc định.
    - `models.providers.*.models.*.compat.requiresStringContent`: gợi ý tương thích tùy chọn cho các endpoint chat tương thích OpenAI chỉ nhận chuỗi. Khi `true`, OpenClaw làm phẳng các mảng `messages[].content` chỉ gồm văn bản thuần thành chuỗi thường trước khi gửi yêu cầu.
    - `models.providers.*.models.*.compat.strictMessageKeys`: gợi ý tương thích tùy chọn cho các endpoint chat tương thích OpenAI nghiêm ngặt. Khi `true`, OpenClaw rút gọn các đối tượng thông điệp Chat Completions gửi đi chỉ còn `role` và `content` trước khi gửi yêu cầu.
    - `models.providers.*.models.*.compat.thinkingFormat`: gợi ý payload suy nghĩ tùy chọn. Dùng `"qwen"` cho `enable_thinking` cấp cao nhất, hoặc `"qwen-chat-template"` cho `chat_template_kwargs.enable_thinking` trên các máy chủ tương thích OpenAI thuộc họ Qwen có hỗ trợ kwargs chat-template cấp yêu cầu, chẳng hạn như vLLM.
...
Quy trình onboarding nhà cung cấp tùy chỉnh tương tác suy luận đầu vào ảnh cho các ID mô hình vision phổ biến như GPT-4o, Claude, Gemini, Qwen-VL, LLaVA, Pixtral, InternVL, Mllama, MiniCPM-V và GLM-4V, đồng thời bỏ qua câu hỏi bổ sung cho các họ chỉ văn bản đã biết. ID mô hình không xác định vẫn nhắc hỏi về hỗ trợ ảnh. Onboarding không tương tác dùng cùng suy luận; truyền `--custom-image-input` để buộc metadata có khả năng xử lý ảnh hoặc `--custom-text-input` để buộc metadata chỉ văn bản.
...
<AccordionGroup>
  ...
  <Accordion title="MiniMax M2.7 (trực tiếp)">
    ...
    Đặt `MINIMAX_API_KEY`. Lối tắt: `openclaw onboard --auth-choice minimax-global-api` hoặc `openclaw onboard --auth-choice minimax-cn-api`. Danh mục mô hình mặc định chỉ dùng M2.7. Trên đường dẫn phát trực tuyến tương thích với Anthropic, OpenClaw tắt tư duy MiniMax theo mặc định trừ khi bạn tự đặt `thinking` một cách rõ ràng. `/fast on` hoặc `params.fastMode: true` viết lại `MiniMax-M2.7` thành `MiniMax-M2.7-highspeed`.
  ...
  <Accordion title="Moonshot AI (Kimi)">
    ...
    Các điểm cuối Moonshot gốc công bố khả năng tương thích sử dụng phát trực tuyến trên transport `openai-completions` dùng chung, và OpenClaw xác định điều đó dựa trên năng lực điểm cuối thay vì chỉ dựa vào id nhà cung cấp tích hợp sẵn.
  ...
  <Accordion title="OpenCode">
    ...
    Đặt `OPENCODE_API_KEY` (hoặc `OPENCODE_ZEN_API_KEY`). Sử dụng tham chiếu `opencode/...` cho catalog Zen hoặc tham chiếu `opencode-go/...` cho catalog Go. Lối tắt: `openclaw onboard --auth-choice opencode-zen` hoặc `openclaw onboard --auth-choice opencode-go`.
  ...
  <Accordion title="Synthetic (Anthropic-compatible)">
    ...
    URL cơ sở nên bỏ `/v1` (client Anthropic sẽ tự thêm). Lối tắt: `openclaw onboard --auth-choice synthetic-api-key`.
