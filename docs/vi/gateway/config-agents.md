---
read_when:
    - Điều chỉnh các giá trị mặc định của tác nhân (mô hình, suy luận, không gian làm việc, Heartbeat, phương tiện, Skills)
    - Cấu hình định tuyến và liên kết đa tác nhân
    - Điều chỉnh phiên, việc chuyển phát tin nhắn và hành vi chế độ trò chuyện
summary: Các giá trị mặc định của tác nhân, định tuyến đa tác nhân, phiên, tin nhắn và cấu hình trò chuyện
...
khóa cấp cao nhất khác, xem [Tài liệu tham chiếu cấu hình](/vi/gateway/configuration-reference).
...
- `"continuation-skip"`: các lượt tiếp nối an toàn (sau một phản hồi của assistant đã hoàn tất) bỏ qua việc chèn lại bootstrap của workspace, giúp giảm kích thước prompt. Các lần chạy Heartbeat và thử lại sau Compaction vẫn dựng lại ngữ cảnh.
- `"never"`: tắt bootstrap của workspace và việc chèn tệp ngữ cảnh trên mọi lượt. Chỉ dùng tùy chọn này cho các tác tử tự sở hữu hoàn toàn vòng đời prompt của chúng (công cụ ngữ cảnh tùy chỉnh, thời gian chạy native tự dựng ngữ cảnh riêng, hoặc quy trình chuyên biệt không dùng bootstrap). Các lượt Heartbeat và khôi phục sau Compaction cũng bỏ qua việc chèn.
...
Số ký tự tối đa cho mỗi tệp bootstrap của workspace trước khi cắt bớt. Mặc định: `12000`.
...
Tổng số ký tự tối đa được chèn trên tất cả các tệp bootstrap của workspace. Mặc định: `60000`.
...
Số đếm thô/đã chèn chi tiết và các trường tinh chỉnh cấu hình vẫn nằm trong chẩn đoán như báo cáo ngữ cảnh/trạng thái và nhật ký; ngữ cảnh người dùng/thời gian chạy WebChat thường lệ chỉ nhận thông báo khôi phục ngắn gọn.
...
OpenClaw có nhiều ngân sách prompt/ngữ cảnh khối lượng lớn, và chúng được
chủ ý tách theo hệ thống con thay vì tất cả đi qua một núm điều khiển chung.

- `agents.defaults.bootstrapMaxChars` /
  `agents.defaults.bootstrapTotalMaxChars`:
  chèn bootstrap workspace thông thường.
- `agents.defaults.startupContext.*`:
  phần mở đầu một lần cho lần chạy mô hình khi đặt lại/khởi động, bao gồm các
  tệp `memory/*.md` hằng ngày gần đây. Các lệnh trò chuyện trần `/new` và `/reset`
  được xác nhận mà không gọi mô hình.
- `skills.limits.*`:
  danh sách Skills thu gọn được chèn vào system prompt.
...
- `memoryGetMaxChars`: giới hạn đoạn trích `memory_get` mặc định trước khi
  thêm siêu dữ liệu cắt bớt và thông báo tiếp tục.
- `memoryGetDefaultLines`: cửa sổ dòng mặc định của `memory_get` khi bỏ qua
  `lines`.
...
Giới hạn toàn cục cho danh sách Skills thu gọn được chèn vào system prompt. Điều này
...
Ghi đè theo tác tử cho ngân sách prompt của Skills.
...
- `imageGenerationModel`: chấp nhận chuỗi (`"provider/model"`) hoặc đối tượng (`{ primary, fallbacks }`).
  ...
  - Nếu bị bỏ qua, `image_generate` vẫn có thể suy luận giá trị mặc định của nhà cung cấp có xác thực. Nó thử nhà cung cấp mặc định hiện tại trước, rồi đến các nhà cung cấp tạo hình ảnh đã đăng ký còn lại theo thứ tự ID nhà cung cấp.
- `musicGenerationModel`: chấp nhận chuỗi (`"provider/model"`) hoặc đối tượng (`{ primary, fallbacks }`).
  ...
  - Nếu bị bỏ qua, `music_generate` vẫn có thể suy luận giá trị mặc định của nhà cung cấp có xác thực. Nó thử nhà cung cấp mặc định hiện tại trước, rồi đến các nhà cung cấp tạo nhạc đã đăng ký còn lại theo thứ tự ID nhà cung cấp.
...
- `videoGenerationModel`: chấp nhận chuỗi (`"provider/model"`) hoặc đối tượng (`{ primary, fallbacks }`).
  ...
  - Nếu bị bỏ qua, `video_generate` vẫn có thể suy luận giá trị mặc định của nhà cung cấp có xác thực. Nó thử nhà cung cấp mặc định hiện tại trước, rồi đến các nhà cung cấp tạo video đã đăng ký còn lại theo thứ tự ID nhà cung cấp.
  - Nếu bạn chọn trực tiếp một nhà cung cấp/mô hình, hãy cấu hình cả xác thực/khóa API nhà cung cấp tương ứng.
...
- `pdfModel`: chấp nhận chuỗi (`"provider/model"`) hoặc đối tượng (`{ primary, fallbacks }`).
  ...
  - Nếu bị bỏ qua, công cụ PDF dự phòng về `imageModel`, rồi đến mô hình phiên/mặc định đã phân giải.
...
- `toolProgressDetail`: chế độ chi tiết cho tóm tắt công cụ `/verbose` và các dòng công cụ bản nháp tiến trình. Giá trị: `"explain"` (mặc định, nhãn ngắn gọn cho người dùng) hoặc `"raw"` (nối thêm lệnh/chi tiết thô khi có). `agents.list[].toolProgressDetail` theo từng agent ghi đè mặc định này.
- `reasoningDefault`: mức hiển thị lập luận mặc định cho agent. Giá trị: `"off"`, `"on"`, `"stream"`. `agents.list[].reasoningDefault` theo từng agent ghi đè mặc định này. Mặc định lập luận đã cấu hình chỉ được áp dụng cho chủ sở hữu, người gửi được ủy quyền, hoặc ngữ cảnh gateway quản trị viên vận hành khi không đặt ghi đè lập luận theo từng tin nhắn hoặc phiên.
- `elevatedDefault`: mức đầu ra nâng cao mặc định cho agent. Giá trị: `"off"`, `"on"`, `"ask"`, `"full"`. Mặc định: `"on"`.
- `model.primary`: định dạng `provider/model` (ví dụ `openai/gpt-5.5` cho truy cập bằng khóa API hoặc `openai-codex/gpt-5.5` cho Codex OAuth). Nếu bạn bỏ qua nhà cung cấp, OpenClaw thử bí danh trước, rồi đến kết quả khớp nhà cung cấp đã cấu hình duy nhất cho đúng ID mô hình đó, và chỉ sau đó mới dự phòng về nhà cung cấp mặc định đã cấu hình (hành vi tương thích đã lỗi thời, vì vậy hãy ưu tiên `provider/model` tường minh). Nếu nhà cung cấp đó không còn cung cấp mô hình mặc định đã cấu hình, OpenClaw dự phòng về nhà cung cấp/mô hình đã cấu hình đầu tiên thay vì hiển thị một mặc định nhà cung cấp đã bị xóa và cũ.
- `models`: danh mục mô hình đã cấu hình và allowlist cho `/model`. Mỗi mục có thể bao gồm `alias` (lối tắt) và `params` (riêng cho nhà cung cấp, ví dụ `temperature`, `maxTokens`, `cacheRetention`, `context1m`, `responsesServerCompaction`, `responsesCompactThreshold`, `chat_template_kwargs`, `extra_body`/`extraBody`).
  ...
  - Các luồng cấu hình/onboarding theo phạm vi nhà cung cấp hợp nhất các mô hình nhà cung cấp đã chọn vào bản đồ này và giữ nguyên những nhà cung cấp không liên quan đã được cấu hình.
...
- Thứ tự ưu tiên hợp nhất `params` (cấu hình): `agents.defaults.params` (nền toàn cục) bị ghi đè bởi `agents.defaults.models["provider/model"].params` (theo mô hình), rồi `agents.list[].params` (khớp ID agent) ghi đè theo khóa. Xem [Prompt Caching](/vi/reference/prompt-caching) để biết chi tiết.
- `params.extra_body`/`params.extraBody`: JSON truyền qua nâng cao được hợp nhất vào thân yêu cầu `api: "openai-completions"` cho proxy tương thích OpenAI. Nếu nó xung đột với các khóa yêu cầu đã tạo, phần thân bổ sung sẽ thắng; các tuyến completions không gốc vẫn loại bỏ `store` chỉ dành cho OpenAI sau đó.
- `params.chat_template_kwargs`: đối số mẫu trò chuyện tương thích vLLM/OpenAI được hợp nhất vào thân yêu cầu cấp cao nhất `api: "openai-completions"`. Với `vllm/nemotron-3-*` khi tắt thinking, Plugin vLLM đi kèm tự động gửi `enable_thinking: false` và `force_nonempty_content: true`; `chat_template_kwargs` tường minh ghi đè mặc định đã tạo, và `extra_body.chat_template_kwargs` vẫn có quyền ưu tiên cuối cùng. Với điều khiển thinking Qwen vLLM, đặt `params.qwenThinkingFormat` thành `"chat-template"` hoặc `"top-level"` trên mục mô hình đó.
- `compat.supportedReasoningEfforts`: danh sách mức nỗ lực lập luận tương thích OpenAI theo từng mô hình. Bao gồm `"xhigh"` cho các endpoint tùy chỉnh thực sự chấp nhận nó; OpenClaw sau đó hiển thị `/think xhigh` trong menu lệnh, các hàng phiên Gateway, xác thực bản vá phiên, xác thực CLI agent, và xác thực `llm-task` cho nhà cung cấp/mô hình đã cấu hình đó. Dùng `compat.reasoningEffortMap` khi backend muốn một giá trị riêng của nhà cung cấp cho mức chuẩn.
...
- `agentRuntime`: chính sách runtime agent cấp thấp mặc định. ID bị bỏ qua mặc định là OpenClaw Pi. Dùng `id: "pi"` để buộc harness PI tích hợp sẵn, `id: "auto"` để cho phép các harness Plugin đã đăng ký nhận các mô hình được hỗ trợ và dùng PI khi không có harness nào khớp, một ID harness đã đăng ký như `id: "codex"` để yêu cầu harness đó, hoặc một bí danh backend CLI được hỗ trợ như `id: "claude-cli"`. Runtime Plugin tường minh sẽ thất bại đóng khi harness không khả dụng hoặc thất bại. Giữ tham chiếu mô hình ở dạng chuẩn `provider/model`; chọn Codex, Claude CLI, Gemini CLI, và các backend thực thi khác thông qua cấu hình runtime thay vì tiền tố nhà cung cấp runtime cũ. Xem [Runtime agent](/vi/concepts/agent-runtimes) để biết điều này khác với lựa chọn nhà cung cấp/mô hình như thế nào.
- Các trình ghi cấu hình làm thay đổi các trường này (ví dụ `/models set`, `/models set-image`, và các lệnh thêm/xóa dự phòng) lưu dạng đối tượng chuẩn và giữ nguyên danh sách dự phòng hiện có khi có thể.
...
hoặc khi bạn muốn một backend CLI được hỗ trợ như Claude CLI. Để hiểu
mô hình khái niệm, xem [Runtime agent](/vi/concepts/agent-runtimes).
...
- `id: "auto"` cho phép các harness Plugin đã đăng ký nhận các lượt được hỗ trợ và dùng PI khi không có harness nào khớp. Runtime Plugin tường minh như `id: "codex"` yêu cầu harness đó và thất bại đóng nếu nó không khả dụng hoặc thất bại.
...
- Lựa chọn harness được ghim theo ID phiên sau lần chạy nhúng đầu tiên. Thay đổi cấu hình/môi trường ảnh hưởng đến phiên mới hoặc phiên đã đặt lại, không ảnh hưởng transcript hiện có. Các phiên cũ có lịch sử transcript nhưng không có ghim đã ghi nhận được xử lý như đã ghim PI. `/status` báo cáo runtime hiệu lực, ví dụ `Runtime: OpenClaw Pi Default` hoặc `Runtime: OpenAI Codex`.
- Điều này chỉ kiểm soát thực thi lượt agent văn bản. Tạo phương tiện, thị giác, PDF, nhạc, video và TTS vẫn dùng thiết lập nhà cung cấp/mô hình của chúng.
...
Các mô hình Z.AI bật `tool_stream` theo mặc định để phát luồng lệnh gọi công cụ. Đặt `agents.defaults.models["zai/<model>"].params.tool_stream` thành `false` để tắt tính năng này.
...
- `isolatedSession`: khi là true, mỗi Heartbeat chạy trong một phiên mới, không có lịch sử hội thoại trước đó. Cùng mẫu cô lập như Cron `sessionTarget: "isolated"`. Giảm chi phí token trên mỗi Heartbeat từ khoảng 100K xuống khoảng 2-5K token.
- `skipWhenBusy`: khi là true, các lượt chạy Heartbeat trì hoãn trên các lane bận bổ sung: subagent hoặc công việc lệnh lồng nhau. Các lane Cron luôn trì hoãn Heartbeat, ngay cả khi không có cờ này.
...
- `provider`: ID của một provider Plugin Compaction đã đăng ký. Khi được đặt, `summarize()` của provider được gọi thay cho tóm tắt LLM tích hợp. Quay về tích hợp nếu thất bại. Đặt provider sẽ buộc `mode: "safeguard"`. Xem [Compaction](/vi/concepts/compaction).
...
- `identifierPolicy`: `strict` (mặc định), `off`, hoặc `custom`. `strict` thêm hướng dẫn tích hợp về việc giữ lại mã định danh mờ trước phần tóm tắt Compaction.
...
- `postCompactionSections`: tên phần H2/H3 trong AGENTS.md tùy chọn để chèn lại sau Compaction. Mặc định là `["Session Startup", "Red Lines"]`; đặt `[]` để tắt chèn lại. Khi chưa đặt hoặc được đặt rõ ràng thành cặp mặc định đó, các tiêu đề cũ `Every Session`/`Safety` cũng được chấp nhận làm phương án dự phòng kế thừa.
- `model`: ghi đè `provider/model-id` tùy chọn chỉ cho tóm tắt Compaction. Dùng khi phiên chính nên giữ một mô hình nhưng các bản tóm tắt Compaction nên chạy trên mô hình khác; khi chưa đặt, Compaction dùng mô hình chính của phiên.
...
- `memoryFlush`: lượt agentic im lặng trước auto-compaction để lưu các ký ức bền vững. Đặt `model` thành provider/model chính xác như `ollama/qwen3:8b` khi lượt dọn dẹp này nên chạy trên mô hình cục bộ; ghi đè này không kế thừa chuỗi fallback của phiên đang hoạt động. Bị bỏ qua khi workspace ở chế độ chỉ đọc.
...
- Khối hình ảnh không bao giờ bị cắt bớt/xóa.
...
- `humanDelay`: tạm dừng ngẫu nhiên giữa các câu trả lời theo khối. `natural` = 800–2500ms. Ghi đè theo agent: `agents.list[].humanDelay`.

Xem [Streaming](/vi/concepts/streaming) để biết chi tiết hành vi + chia khối.
...
- Mặc định: `instant` cho trò chuyện trực tiếp/lượt nhắc, `message` cho trò chuyện nhóm không được nhắc.
- Ghi đè theo phiên: `session.typingMode`, `session.typingIntervalSeconds`.

Xem [Chỉ báo nhập](/vi/concepts/typing-indicators).

<a id="agentsdefaultssandbox"></a>

### `agents.defaults.sandbox`

Cơ chế sandbox tùy chọn cho tác tử nhúng. Xem [Cơ chế sandbox](/vi/gateway/sandboxing) để đọc hướng dẫn đầy đủ.
...
Khi chọn `backend: "openshell"`, các thiết lập riêng cho runtime chuyển sang
...
- `strictHostKeyChecking` / `updateHostKeys`: nút điều chỉnh chính sách khóa máy chủ OpenSSH
...
- `shared`: container và workspace dùng chung (không cô lập chéo phiên)
...
- `mirror`: gieo từ xa từ cục bộ trước khi exec, đồng bộ lại sau exec; workspace cục bộ vẫn là bản chuẩn
- `remote`: gieo từ xa một lần khi sandbox được tạo, sau đó giữ workspace từ xa làm bản chuẩn
...
Giao vận là SSH vào sandbox OpenShell, nhưng Plugin sở hữu vòng đời sandbox và đồng bộ mirror tùy chọn.

**`setupCommand`** chạy một lần sau khi tạo container (qua `sh -lc`). Cần luồng mạng đi ra, gốc có thể ghi, người dùng root.

**Container mặc định là `network: "none"`** — đặt thành `"bridge"` (hoặc một mạng bridge tùy chỉnh) nếu tác tử cần truy cập đi ra.
...
**`docker.binds`** gắn thêm thư mục máy chủ; các bind toàn cục và theo tác tử được hợp nhất.
...
Quyền truy cập quan sát noVNC dùng xác thực VNC theo mặc định và OpenClaw phát một URL token ngắn hạn (thay vì lộ mật khẩu trong URL dùng chung).
...
- `cdpSourceRange` tùy chọn hạn chế lưu lượng CDP đi vào tại biên container theo một dải CIDR (ví dụ `172.21.0.1/32`).
- `sandbox.browser.binds` chỉ gắn thêm thư mục máy chủ vào container trình duyệt sandbox. Khi được đặt (bao gồm `[]`), nó thay thế `docker.binds` cho container trình duyệt.
- Mặc định khởi chạy được định nghĩa trong `scripts/sandbox-browser-entrypoint.sh` và được tinh chỉnh cho máy chủ container:
  ...
  - `OPENCLAW_BROWSER_DISABLE_EXTENSIONS=0` bật lại tiện ích mở rộng nếu quy trình của bạn
    phụ thuộc vào chúng.
...
Dựng image (từ một checkout mã nguồn):
...
Đối với cài đặt npm không có checkout mã nguồn, xem [Cơ chế sandbox § Image và thiết lập](/vi/gateway/sandboxing#images-and-setup) để biết các lệnh `docker build` nội tuyến.

### `agents.list` (ghi đè theo tác tử)

Dùng `agents.list[].tts` để cung cấp cho một tác tử nhà cung cấp TTS, giọng nói, mô hình,
kiểu, hoặc chế độ auto-TTS riêng. Khối tác tử được deep-merge lên trên
`messages.tts` toàn cục, nên thông tin xác thực dùng chung có thể nằm ở một nơi trong khi từng
tác tử chỉ ghi đè các trường giọng nói hoặc nhà cung cấp mà chúng cần. Ghi đè của tác tử đang hoạt động
...
- `default`: khi đặt nhiều mục, mục đầu tiên được ưu tiên (ghi cảnh báo). Nếu không đặt mục nào, mục đầu tiên trong danh sách là mặc định.
- `model`: dạng chuỗi đặt một mô hình chính nghiêm ngặt cho từng tác nhân, không có mô hình dự phòng; dạng đối tượng `{ primary }` cũng nghiêm ngặt trừ khi bạn thêm `fallbacks`. Dùng `{ primary, fallbacks: [...] }` để cho phép tác nhân đó dùng dự phòng, hoặc `{ primary, fallbacks: [] }` để làm rõ hành vi nghiêm ngặt. Các tác vụ Cron chỉ ghi đè `primary` vẫn kế thừa các dự phòng mặc định trừ khi bạn đặt `fallbacks: []`.
- `params`: tham số luồng cho từng tác nhân được hợp nhất đè lên mục mô hình đã chọn trong `agents.defaults.models`. Dùng mục này cho các ghi đè riêng của tác nhân như `cacheRetention`, `temperature`, hoặc `maxTokens` mà không cần sao chép toàn bộ danh mục mô hình.
- `tts`: các ghi đè chuyển văn bản thành giọng nói tùy chọn cho từng tác nhân. Khối này hợp nhất sâu đè lên `messages.tts`, vì vậy hãy giữ thông tin xác thực nhà cung cấp dùng chung và chính sách dự phòng trong `messages.tts`, rồi chỉ đặt các giá trị riêng theo persona như nhà cung cấp, giọng, mô hình, kiểu, hoặc chế độ tự động tại đây.
- `skills`: danh sách cho phép skill tùy chọn cho từng tác nhân. Nếu bỏ qua, tác nhân kế thừa `agents.defaults.skills` khi được đặt; danh sách tường minh sẽ thay thế mặc định thay vì hợp nhất, và `[]` nghĩa là không có skills.
- `thinkingDefault`: mức suy nghĩ mặc định tùy chọn cho từng tác nhân (`off | minimal | low | medium | high | xhigh | adaptive | max`). Ghi đè `agents.defaults.thinkingDefault` cho tác nhân này khi không đặt ghi đè theo tin nhắn hoặc phiên. Hồ sơ nhà cung cấp/mô hình đã chọn kiểm soát các giá trị hợp lệ; với Google Gemini, `adaptive` giữ suy nghĩ động do nhà cung cấp sở hữu (`thinkingLevel` bị bỏ qua trên Gemini 3/3.1, `thinkingBudget: -1` trên Gemini 2.5).
- `reasoningDefault`: khả năng hiển thị suy luận mặc định tùy chọn cho từng tác nhân (`on | off | stream`). Ghi đè `agents.defaults.reasoningDefault` cho tác nhân này khi không đặt ghi đè suy luận theo tin nhắn hoặc phiên.
...
- `agentRuntime`: ghi đè chính sách runtime cấp thấp tùy chọn cho từng tác nhân. Dùng `{ id: "codex" }` để biến một tác nhân thành chỉ dùng Codex, trong khi các tác nhân khác giữ dự phòng PI mặc định ở chế độ `auto`.
- `runtime`: bộ mô tả runtime tùy chọn cho từng tác nhân. Dùng `type: "acp"` với các mặc định `runtime.acp` (`agent`, `backend`, `mode`, `cwd`) khi tác nhân nên mặc định dùng các phiên harness ACP.
...
- `subagents.allowAgents`: danh sách cho phép id tác nhân cho các đích `sessions_spawn.agentId` tường minh (`["*"]` = bất kỳ; mặc định: chỉ cùng tác nhân). Bao gồm id của bên yêu cầu khi các lệnh gọi `agentId` tự nhắm đích nên được cho phép.
- Bộ bảo vệ kế thừa sandbox: nếu phiên của bên yêu cầu nằm trong sandbox, `sessions_spawn` từ chối các đích sẽ chạy không có sandbox.
...
Chạy nhiều tác nhân cô lập bên trong một Gateway. Xem [Đa tác nhân](/vi/concepts/multi-agent).
...
- `type` (tùy chọn): `route` cho định tuyến bình thường (thiếu type sẽ mặc định là route), `acp` cho các ràng buộc cuộc trò chuyện ACP lâu dài.
...
Đối với các mục `type: "acp"`, OpenClaw phân giải theo danh tính cuộc trò chuyện chính xác (`match.channel` + tài khoản + `match.peer.id`) và không dùng thứ tự tầng ràng buộc định tuyến ở trên.

### Hồ sơ truy cập theo tác nhân
...
- **`identityLinks`**: ánh xạ các id chuẩn sang các peer có tiền tố nhà cung cấp để chia sẻ phiên liên kênh. Các lệnh neo như `/dock_discord` dùng cùng ánh xạ để chuyển tuyến trả lời của phiên đang hoạt động sang một peer kênh đã liên kết khác; xem [Neo kênh](/vi/concepts/channel-docking).
- **`reset`**: chính sách đặt lại chính. `daily` đặt lại vào giờ địa phương `atHour`; `idle` đặt lại sau `idleMinutes`. Khi cấu hình cả hai, chính sách nào hết hạn trước sẽ thắng. Độ mới của đặt lại hằng ngày dùng `sessionStartedAt` của hàng phiên; độ mới của đặt lại khi không hoạt động dùng `lastInteractionAt`. Các lần ghi nền/sự kiện hệ thống như Heartbeat, đánh thức Cron, thông báo exec và ghi sổ Gateway có thể cập nhật `updatedAt`, nhưng chúng không giữ cho phiên hằng ngày/khi không hoạt động luôn mới.
...
- **`maintenance`**: điều khiển dọn dẹp + lưu giữ kho phiên.
  ...
  - `maxEntries`: số mục tối đa trong `sessions.json` (mặc định `500`). Runtime ghi dọn dẹp theo lô với một vùng đệm ngưỡng cao nhỏ cho các giới hạn kích thước sản xuất; `openclaw sessions cleanup --enforce` áp dụng giới hạn ngay lập tức.
  - `rotateBytes`: không dùng nữa và bị bỏ qua; `openclaw doctor --fix` xóa trường này khỏi cấu hình cũ.
...
- **`threadBindings`**: mặc định toàn cục cho các tính năng phiên gắn với luồng.
  ...
  - `idleHours`: thời gian không hoạt động mặc định tính bằng giờ để tự bỏ tập trung (`0` tắt; nhà cung cấp có thể ghi đè)
...
Cách phân giải (mục cụ thể nhất thắng): tài khoản → kênh → toàn cục. `""` tắt và dừng cascade. `"auto"` suy ra `[{identity.name}]`.
...
- `messages.statusReactions.enabled`: bật phản ứng trạng thái vòng đời trên Slack, Discord và Telegram.
  Trên Slack và Discord, khi chưa đặt thì phản ứng trạng thái vẫn được bật nếu phản ứng xác nhận đang hoạt động.
  Trên Telegram, hãy đặt rõ thành `true` để bật phản ứng trạng thái vòng đời.

### Debounce đầu vào

Gộp các tin nhắn chỉ có văn bản được gửi nhanh từ cùng một người gửi thành một lượt agent duy nhất. Phương tiện/tệp đính kèm sẽ flush ngay lập tức. Lệnh điều khiển bỏ qua debounce.
...
- `modelOverrides` được bật theo mặc định; `modelOverrides.allowProvider` mặc định là `false` (phải chủ động bật).
- Khóa API dự phòng sang `ELEVENLABS_API_KEY`/`XI_API_KEY` và `OPENAI_API_KEY`.
- Các provider giọng nói đi kèm thuộc sở hữu của Plugin. Nếu `plugins.allow` được đặt, hãy bao gồm từng Plugin provider TTS bạn muốn dùng, ví dụ `microsoft` cho Edge TTS. Mã provider cũ `edge` được chấp nhận làm bí danh cho `microsoft`.
...
- Khi `providers.openai.baseUrl` trỏ đến một endpoint không phải OpenAI, OpenClaw xem đó là máy chủ TTS tương thích OpenAI và nới lỏng việc xác thực model/giọng nói.
...
- `talk.provider` phải khớp với một khóa trong `talk.providers` khi nhiều provider Trò chuyện được cấu hình.
- Các khóa Trò chuyện phẳng cũ (`talk.voiceId`, `talk.voiceAliases`, `talk.modelId`, `talk.outputFormat`, `talk.apiKey`) chỉ để tương thích. Chạy `openclaw doctor --fix` để ghi lại cấu hình đã lưu vào `talk.providers.<provider>`.
...
- Dự phòng `ELEVENLABS_API_KEY` chỉ áp dụng khi chưa cấu hình khóa API Trò chuyện.
...
- `speechLocale` đặt mã locale BCP 47 được nhận dạng giọng nói Trò chuyện trên iOS/macOS sử dụng. Để trống để dùng mặc định của thiết bị.
- `silenceTimeoutMs` kiểm soát thời gian chế độ Trò chuyện chờ sau khi người dùng im lặng trước khi gửi bản chép lời. Khi chưa đặt, cửa sổ tạm dừng mặc định của nền tảng được giữ nguyên (`700 ms on macOS and Android, 900 ms on iOS`).
