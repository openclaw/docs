...
x-i18n:
    generated_at: "2026-05-02T10:40:50Z"
    model: gpt-5.5
    provider: openai
...
`messages.*` và `talk.*`. Đối với kênh, công cụ, runtime Gateway và các khóa cấp cao nhất
khác, xem [Tham chiếu cấu hình](/vi/gateway/configuration-reference).
...
Bỏ qua việc tạo các tệp workspace tùy chọn được chọn trong khi vẫn ghi các tệp bootstrap bắt buộc. Giá trị hợp lệ: `SOUL.md`, `USER.md`, `HEARTBEAT.md` và `IDENTITY.md`.
...
- `"continuation-skip"`: các lượt tiếp tục an toàn (sau phản hồi assistant đã hoàn tất) bỏ qua việc chèn lại bootstrap workspace, giúp giảm kích thước prompt. Các lần chạy Heartbeat và các lần thử lại sau Compaction vẫn xây dựng lại ngữ cảnh.
- `"never"`: tắt bootstrap workspace và chèn tệp ngữ cảnh trên mọi lượt. Chỉ dùng tùy chọn này cho các agent tự quản lý toàn bộ vòng đời prompt của chúng (công cụ ngữ cảnh tùy chỉnh, runtime native tự xây dựng ngữ cảnh riêng, hoặc quy trình chuyên biệt không cần bootstrap). Các lượt Heartbeat và khôi phục sau Compaction cũng bỏ qua việc chèn.
...
Số ký tự tối đa cho mỗi tệp bootstrap workspace trước khi bị cắt ngắn. Mặc định: `12000`.
...
- `agents.defaults.startupContext.*`:
  phần mở đầu dùng một lần cho lần chạy model khi đặt lại/khởi động, bao gồm các tệp
  `memory/*.md` hằng ngày gần đây. Các lệnh chat trần `/new` và `/reset`
  được xác nhận mà không gọi model.
- `skills.limits.*`:
...
- `memory.qmd.limits.*`:
  kích thước đoạn trích tìm kiếm bộ nhớ được lập chỉ mục và kích thước chèn.

Chỉ dùng override tương ứng cho từng agent khi một agent cần ngân sách khác:
...
Kiểm soát phần mở đầu khởi động lượt đầu được chèn trong các lần chạy model khi đặt lại/khởi động.
...
Override theo từng agent cho các núm `contextLimits` dùng chung. Các trường bị bỏ qua kế thừa
...
Override theo từng agent cho ngân sách prompt Skills.
...
Giá trị cao hơn giữ lại nhiều chi tiết trực quan hơn.
...
Múi giờ cho ngữ cảnh system prompt (không phải dấu thời gian tin nhắn). Dùng múi giờ của host làm dự phòng.
...
- `imageGenerationModel`: chấp nhận một chuỗi (`"provider/model"`) hoặc một đối tượng (`{ primary, fallbacks }`).
  ...
  - Nếu bị bỏ qua, `image_generate` vẫn có thể suy ra mặc định nhà cung cấp dựa trên xác thực. Nó thử nhà cung cấp mặc định hiện tại trước, sau đó đến các nhà cung cấp tạo hình ảnh đã đăng ký còn lại theo thứ tự ID nhà cung cấp.
- `musicGenerationModel`: chấp nhận một chuỗi (`"provider/model"`) hoặc một đối tượng (`{ primary, fallbacks }`).
  ...
  - Nếu bị bỏ qua, `music_generate` vẫn có thể suy ra mặc định nhà cung cấp dựa trên xác thực. Nó thử nhà cung cấp mặc định hiện tại trước, sau đó đến các nhà cung cấp tạo nhạc đã đăng ký còn lại theo thứ tự ID nhà cung cấp.
  - Nếu bạn chọn trực tiếp một nhà cung cấp/mô hình, cũng hãy cấu hình xác thực/khóa API nhà cung cấp tương ứng.
- `videoGenerationModel`: chấp nhận một chuỗi (`"provider/model"`) hoặc một đối tượng (`{ primary, fallbacks }`).
  ...
  - Nếu bị bỏ qua, `video_generate` vẫn có thể suy ra mặc định nhà cung cấp dựa trên xác thực. Nó thử nhà cung cấp mặc định hiện tại trước, sau đó đến các nhà cung cấp tạo video đã đăng ký còn lại theo thứ tự ID nhà cung cấp.
  - Nếu bạn chọn trực tiếp một nhà cung cấp/mô hình, cũng hãy cấu hình xác thực/khóa API nhà cung cấp tương ứng.
...
- Thứ tự ưu tiên hợp nhất `params` (cấu hình): `agents.defaults.params` (nền toàn cục) bị ghi đè bởi `agents.defaults.models["provider/model"].params` (theo mô hình), sau đó `agents.list[].params` (khớp ID agent) ghi đè theo khóa. Xem [Lưu đệm prompt](/vi/reference/prompt-caching) để biết chi tiết.
...
- `compat.supportedReasoningEfforts`: danh sách mức nỗ lực reasoning tương thích OpenAI theo từng mô hình. Bao gồm `"xhigh"` cho các endpoint tùy chỉnh thực sự chấp nhận nó; khi đó OpenClaw hiển thị `/think xhigh` trong menu lệnh, hàng phiên Gateway, xác thực bản vá phiên, xác thực CLI agent, và xác thực `llm-task` cho nhà cung cấp/mô hình đã cấu hình đó. Dùng `compat.reasoningEffortMap` khi backend cần một giá trị riêng theo nhà cung cấp cho một mức chuẩn.
...
- `agentRuntime`: chính sách runtime agent cấp thấp mặc định. ID bị bỏ qua sẽ mặc định là OpenClaw Pi. Dùng `id: "pi"` để buộc dùng harness PI tích hợp sẵn, `id: "auto"` để cho các harness plugin đã đăng ký nhận các mô hình được hỗ trợ, một ID harness đã đăng ký như `id: "codex"`, hoặc một alias backend CLI được hỗ trợ như `id: "claude-cli"`. Đặt `fallback: "none"` để tắt dự phòng PI tự động. Các runtime plugin rõ ràng như `codex` mặc định đóng kín khi lỗi trừ khi bạn đặt `fallback: "pi"` trong cùng phạm vi ghi đè. Giữ tham chiếu mô hình theo chuẩn `provider/model`; chọn Codex, Claude CLI, Gemini CLI và các backend thực thi khác thông qua cấu hình runtime thay vì các tiền tố nhà cung cấp runtime cũ. Xem [Runtime agent](/vi/concepts/agent-runtimes) để biết điều này khác với lựa chọn nhà cung cấp/mô hình như thế nào.
- Các trình ghi cấu hình sửa đổi các trường này (ví dụ `/models set`, `/models set-image`, và các lệnh thêm/xóa dự phòng) lưu dạng đối tượng chuẩn và giữ các danh sách dự phòng hiện có khi có thể.
- `maxConcurrent`: số lần chạy agent song song tối đa trên các phiên (mỗi phiên vẫn được tuần tự hóa). Mặc định: 4.

### `agents.defaults.agentRuntime`

`agentRuntime` kiểm soát executor cấp thấp nào chạy lượt agent. Hầu hết
...
- `fallback`: `"pi"` hoặc `"none"`. Trong `id: "auto"`, fallback bị bỏ qua mặc định là `"pi"` để cấu hình cũ có thể tiếp tục dùng PI khi không có harness plugin nào nhận một lần chạy. Trong chế độ runtime plugin rõ ràng, chẳng hạn `id: "codex"`, fallback bị bỏ qua mặc định là `"none"` để harness bị thiếu sẽ thất bại thay vì âm thầm dùng PI. Ghi đè runtime không kế thừa fallback từ phạm vi rộng hơn; đặt `fallback: "pi"` cùng với runtime rõ ràng khi bạn cố ý muốn dự phòng tương thích đó. Lỗi của harness plugin đã chọn luôn được hiển thị trực tiếp.
...
- Với triển khai chỉ dùng Codex, đặt `model: "openai/gpt-5.5"` và `agentRuntime.id: "codex"`. Bạn cũng có thể đặt rõ ràng `agentRuntime.fallback: "none"` để dễ đọc; đây là mặc định cho runtime plugin rõ ràng.
- Với triển khai Claude CLI, ưu tiên `model: "anthropic/claude-opus-4-7"` cộng với `agentRuntime.id: "claude-cli"`. Các tham chiếu mô hình cũ `claude-cli/claude-opus-4-7` vẫn hoạt động để tương thích, nhưng cấu hình mới nên giữ lựa chọn nhà cung cấp/mô hình theo chuẩn và đặt backend thực thi trong `agentRuntime.id`.
- Các khóa chính sách runtime cũ hơn được `openclaw doctor --fix` viết lại thành `agentRuntime`.
- Lựa chọn harness được ghim theo ID phiên sau lần chạy nhúng đầu tiên. Thay đổi cấu hình/env ảnh hưởng đến phiên mới hoặc phiên được đặt lại, không ảnh hưởng đến transcript hiện có. Các phiên cũ có lịch sử transcript nhưng không có ghim đã ghi nhận được xem là đã ghim PI. `/status` báo cáo runtime hiệu lực, ví dụ `Runtime: OpenClaw Pi Default` hoặc `Runtime: OpenAI Codex`.
- Điều này chỉ kiểm soát thực thi lượt agent dạng văn bản. Tạo media, thị giác, PDF, nhạc, video, và TTS vẫn dùng cài đặt nhà cung cấp/mô hình của chúng.
...
Các bí danh bạn đã cấu hình luôn được ưu tiên so với giá trị mặc định.

Các mô hình Z.AI GLM-4.x tự động bật chế độ tư duy, trừ khi bạn đặt `--thinking off` hoặc tự định nghĩa `agents.defaults.models["zai/<model>"].params.thinking`.
...
Các mô hình Anthropic Claude 4.6 mặc định dùng tư duy `adaptive` khi không đặt mức tư duy rõ ràng.
...
- `"off"` chỉ tắt lớp thân thiện; hợp đồng hành vi GPT-5 đã gắn nhãn vẫn được bật.
...
- `suppressToolErrorWarnings`: khi là true, chặn payload cảnh báo lỗi công cụ trong các lượt chạy Heartbeat.
...
- `skipWhenBusy`: khi là true, các lượt chạy Heartbeat trì hoãn trên các luồng bận bổ sung: công việc subagent hoặc lệnh lồng nhau. Các luồng Cron luôn trì hoãn Heartbeat, ngay cả khi không có cờ này.
- Theo agent: đặt `agents.list[].heartbeat`. Khi bất kỳ agent nào định nghĩa `heartbeat`, **chỉ các agent đó** chạy Heartbeat.
- Heartbeat chạy trọn lượt agent — khoảng cách ngắn hơn tiêu tốn nhiều token hơn.
...
- `mode`: `default` hoặc `safeguard` (tóm tắt theo khúc cho lịch sử dài). Xem [Compaction](/vi/concepts/compaction).
- `provider`: ID của Plugin nhà cung cấp Compaction đã đăng ký. Khi được đặt, `summarize()` của nhà cung cấp sẽ được gọi thay vì tính năng tóm tắt LLM tích hợp. Quay về tích hợp sẵn khi gặp lỗi. Việc đặt nhà cung cấp buộc `mode: "safeguard"`. Xem [Compaction](/vi/concepts/compaction).
- `timeoutSeconds`: số giây tối đa cho phép cho một thao tác Compaction trước khi OpenClaw hủy thao tác đó. Mặc định: `900`.
- `keepRecentTokens`: ngân sách điểm cắt Pi để giữ nguyên văn phần đuôi bản chép lời gần đây nhất. `/compact` thủ công tôn trọng giá trị này khi được đặt rõ ràng; nếu không, Compaction thủ công là một điểm kiểm tra cứng.
- `identifierPolicy`: `strict` (mặc định), `off`, hoặc `custom`. `strict` thêm hướng dẫn tích hợp về giữ lại mã định danh mờ đục vào đầu quá trình tóm tắt Compaction.
- `identifierInstructions`: văn bản tùy chỉnh tùy chọn về bảo toàn mã định danh, được dùng khi `identifierPolicy=custom`.
...
- `midTurnPrecheck`: kiểm tra áp lực vòng lặp công cụ Pi tùy chọn. Khi `enabled: true`, OpenClaw kiểm tra áp lực ngữ cảnh sau khi kết quả công cụ được thêm vào và trước lệnh gọi mô hình tiếp theo. Nếu ngữ cảnh không còn vừa, nó hủy lần thử hiện tại trước khi gửi prompt và dùng lại đường khôi phục precheck hiện có để cắt bớt kết quả công cụ hoặc compact rồi thử lại. Hoạt động với cả hai chế độ Compaction `default` và `safeguard`. Mặc định: tắt.
- `postCompactionSections`: tên phần H2/H3 AGENTS.md tùy chọn để chèn lại sau Compaction. Mặc định là `["Session Startup", "Red Lines"]`; đặt `[]` để tắt việc chèn lại. Khi chưa đặt hoặc được đặt rõ ràng thành cặp mặc định đó, các tiêu đề `Every Session`/`Safety` cũ hơn cũng được chấp nhận làm phương án dự phòng kế thừa.
- `model`: ghi đè `provider/model-id` tùy chọn chỉ cho tóm tắt Compaction. Dùng tùy chọn này khi phiên chính cần giữ một mô hình nhưng tóm tắt Compaction nên chạy trên mô hình khác; khi chưa đặt, Compaction dùng mô hình chính của phiên.
- `maxActiveTranscriptBytes`: ngưỡng byte tùy chọn (`number` hoặc chuỗi như `"20mb"`) kích hoạt Compaction cục bộ bình thường trước một lượt chạy khi JSONL đang hoạt động vượt quá ngưỡng. Yêu cầu `truncateAfterCompaction` để Compaction thành công có thể xoay sang một bản chép lời kế tiếp nhỏ hơn. Tắt khi chưa đặt hoặc là `0`.
- `notifyUser`: khi là `true`, gửi thông báo ngắn cho người dùng khi Compaction bắt đầu và khi hoàn tất (ví dụ: "Compacting context..." và "Compaction complete"). Bị tắt theo mặc định để giữ Compaction im lặng.
- `memoryFlush`: lượt agentic im lặng trước auto-compaction để lưu ký ức bền vững. Đặt `model` thành một nhà cung cấp/mô hình chính xác như `ollama/qwen3:8b` khi lượt dọn dẹp này nên ở trên mô hình cục bộ; ghi đè không kế thừa chuỗi dự phòng của phiên đang hoạt động. Bỏ qua khi workspace ở chế độ chỉ đọc.
...
- Việc cắt tỉa trước tiên cắt mềm các kết quả công cụ quá lớn, sau đó xóa cứng các kết quả công cụ cũ hơn nếu cần.
...
- Các kênh không phải Telegram cần đặt rõ `*.blockStreaming: true` để bật phản hồi dạng khối.
...
- `humanDelay`: khoảng tạm dừng ngẫu nhiên giữa các phản hồi dạng khối. `natural` = 800-2500ms. Ghi đè theo từng tác nhân: `agents.list[].humanDelay`.

Xem [Truyền luồng](/vi/concepts/streaming) để biết hành vi và chi tiết chia đoạn.
...
Cơ chế sandbox tùy chọn cho tác nhân nhúng. Xem [Sandbox](/vi/gateway/sandboxing) để đọc hướng dẫn đầy đủ.
...
- `ro`: workspace sandbox tại `/workspace`, workspace của tác nhân được gắn chỉ đọc tại `/agent`
- `rw`: workspace của tác nhân được gắn đọc/ghi tại `/workspace`
...
- `mirror`: gieo từ xa từ cục bộ trước khi exec, đồng bộ ngược lại sau khi exec; workspace cục bộ vẫn là bản chuẩn
...
**`setupCommand`** chạy một lần sau khi tạo container (qua `sh -lc`). Cần truy cập mạng đi ra, gốc có thể ghi và người dùng root.
...
`sandbox.docker.dangerouslyAllowContainerNamespaceJoin: true` (biện pháp khẩn cấp).
...
**Trình duyệt sandbox** (`sandbox.browser.enabled`): Chromium + CDP trong container. URL noVNC được chèn vào system prompt. Không cần `browser.enabled` trong `openclaw.json`.
...
- `cdpSourceRange` tùy chọn giới hạn lưu lượng CDP đi vào tại rìa container theo một dải CIDR (ví dụ `172.21.0.1/32`).
...
- Mặc định khởi chạy được định nghĩa trong `scripts/sandbox-browser-entrypoint.sh` và tinh chỉnh cho máy chủ container:
  ...
  - Mặc định là đường cơ sở của ảnh container; dùng ảnh trình duyệt tùy chỉnh với entrypoint
    tùy chỉnh để thay đổi mặc định của container.

</Accordion>

Sandbox trình duyệt và `sandbox.docker.binds` chỉ dành cho Docker.

Dựng ảnh (từ checkout mã nguồn):
...
Với cài đặt npm không có checkout mã nguồn, xem [Sandbox § Ảnh và thiết lập](/vi/gateway/sandboxing#images-and-setup) để biết các lệnh `docker build` nội tuyến.
...
- `model`: dạng chuỗi đặt một primary nghiêm ngặt theo từng tác tử, không có fallback mô hình; dạng đối tượng `{ primary }` cũng nghiêm ngặt trừ khi bạn thêm `fallbacks`. Dùng `{ primary, fallbacks: [...] }` để bật fallback cho tác tử đó, hoặc `{ primary, fallbacks: [] }` để làm rõ hành vi nghiêm ngặt. Các tác vụ Cron chỉ ghi đè `primary` vẫn kế thừa fallback mặc định trừ khi bạn đặt `fallbacks: []`.
- `params`: tham số luồng theo từng tác tử được hợp nhất lên trên mục mô hình đã chọn trong `agents.defaults.models`. Dùng mục này cho các ghi đè riêng của tác tử như `cacheRetention`, `temperature`, hoặc `maxTokens` mà không phải sao chép toàn bộ danh mục mô hình.
- `tts`: ghi đè chuyển văn bản thành giọng nói tùy chọn theo từng tác tử. Khối này hợp nhất sâu lên trên `messages.tts`, vì vậy hãy giữ thông tin xác thực provider dùng chung và chính sách fallback trong `messages.tts`, rồi chỉ đặt các giá trị riêng theo persona như provider, giọng, mô hình, kiểu, hoặc chế độ tự động tại đây.
- `skills`: danh sách cho phép Skills tùy chọn theo từng tác tử. Nếu bỏ qua, tác tử kế thừa `agents.defaults.skills` khi được đặt; danh sách tường minh thay thế mặc định thay vì hợp nhất, và `[]` nghĩa là không có Skills.
- `thinkingDefault`: mức suy nghĩ mặc định tùy chọn theo từng tác tử (`off | minimal | low | medium | high | xhigh | adaptive | max`). Ghi đè `agents.defaults.thinkingDefault` cho tác tử này khi không đặt ghi đè theo từng tin nhắn hoặc phiên. Hồ sơ provider/mô hình đã chọn kiểm soát giá trị nào hợp lệ; với Google Gemini, `adaptive` giữ suy nghĩ động do provider sở hữu (`thinkingLevel` bị bỏ qua trên Gemini 3/3.1, `thinkingBudget: -1` trên Gemini 2.5).
- `reasoningDefault`: khả năng hiển thị reasoning mặc định tùy chọn theo từng tác tử (`on | off | stream`). Ghi đè `agents.defaults.reasoningDefault` cho tác tử này khi không đặt ghi đè reasoning theo từng tin nhắn hoặc phiên.
...
- `agentRuntime`: ghi đè chính sách runtime cấp thấp tùy chọn theo từng tác tử. Dùng `{ id: "codex" }` để khiến một tác tử chỉ dùng Codex trong khi các tác tử khác vẫn giữ fallback PI mặc định ở chế độ `auto`.
- `runtime`: bộ mô tả runtime tùy chọn theo từng tác tử. Dùng `type: "acp"` với mặc định `runtime.acp` (`agent`, `backend`, `mode`, `cwd`) khi tác tử nên mặc định dùng các phiên harness ACP.
...
- `subagents.allowAgents`: danh sách cho phép id tác tử cho các đích `sessions_spawn.agentId` tường minh (`["*"]` = bất kỳ; mặc định: chỉ cùng tác tử). Bao gồm id của bên yêu cầu khi các lệnh gọi `agentId` tự nhắm đến chính mình cần được cho phép.
- Chốt bảo vệ kế thừa sandbox: nếu phiên của bên yêu cầu bị sandbox, `sessions_spawn` từ chối các đích sẽ chạy không sandbox.
...
- `type` (tùy chọn): `route` cho định tuyến thông thường (thiếu type sẽ mặc định là route), `acp` cho các binding hội thoại ACP liên tục.
...
- **`scope`**: chiến lược nhóm phiên cơ sở cho ngữ cảnh trò chuyện nhóm.
  - `per-sender` (mặc định): mỗi người gửi nhận một phiên biệt lập trong ngữ cảnh kênh.
  - `global`: tất cả người tham gia trong một ngữ cảnh kênh dùng chung một phiên duy nhất (chỉ dùng khi ngữ cảnh dùng chung là chủ đích).
- **`dmScope`**: cách nhóm tin nhắn trực tiếp.
  ...
  - `per-channel-peer`: biệt lập theo kênh + người gửi (được khuyến nghị cho hộp thư đến nhiều người dùng).
  - `per-account-channel-peer`: biệt lập theo tài khoản + kênh + người gửi (được khuyến nghị cho nhiều tài khoản).
- **`identityLinks`**: ánh xạ id chuẩn sang các peer có tiền tố nhà cung cấp để chia sẻ phiên xuyên kênh. Các lệnh dock như `/dock_discord` dùng cùng ánh xạ để chuyển tuyến trả lời của phiên đang hoạt động sang một peer kênh đã liên kết khác; xem [Dock kênh](/vi/concepts/channel-docking).
- **`reset`**: chính sách đặt lại chính. `daily` đặt lại vào giờ địa phương `atHour`; `idle` đặt lại sau `idleMinutes`. Khi cả hai đều được cấu hình, mục hết hạn trước sẽ thắng. Độ mới của đặt lại hằng ngày dùng `sessionStartedAt` của hàng phiên; độ mới của đặt lại khi nhàn rỗi dùng `lastInteractionAt`. Các ghi nền/sự kiện hệ thống như Heartbeat, đánh thức Cron, thông báo thực thi và ghi sổ Gateway có thể cập nhật `updatedAt`, nhưng chúng không giữ cho các phiên hằng ngày/nhàn rỗi còn mới.
...
- **`maintenance`**: dọn dẹp kho phiên + điều khiển lưu giữ.
  ...
  - `resetArchiveRetention`: lưu giữ cho các bản lưu trữ transcript `*.reset.<timestamp>`. Mặc định là `pruneAfter`; đặt `false` để tắt.
  - `maxDiskBytes`: ngân sách dung lượng đĩa tùy chọn cho thư mục phiên. Ở chế độ `warn`, nó ghi cảnh báo; ở chế độ `enforce`, nó xóa các artifact/phiên cũ nhất trước.
...
- **`threadBindings`**: mặc định toàn cục cho các tính năng phiên gắn với luồng.
  ...
  - `idleHours`: tự động bỏ tập trung khi không hoạt động mặc định, tính bằng giờ (`0` tắt; nhà cung cấp có thể ghi đè)
  ...
  - `defaultSpawnContext`: ngữ cảnh subagent gốc mặc định cho các lần spawn gắn với luồng (`"fork"` hoặc `"isolated"`). Mặc định là `"fork"`.
...
Độ phân giải (cụ thể nhất thắng): tài khoản → kênh → toàn cục. `""` tắt và dừng phân tầng. `"auto"` suy ra `[{identity.name}]`.
...
- Mặc định là `identity.emoji` của agent đang hoạt động, nếu không thì `"👀"`. Đặt `""` để tắt.
...
- `messages.statusReactions.enabled`: bật phản ứng trạng thái vòng đời trên Slack, Discord và Telegram.
  Trên Slack và Discord, khi chưa đặt, phản ứng trạng thái vẫn được bật khi phản ứng xác nhận đang hoạt động.
  Trên Telegram, đặt rõ ràng thành `true` để bật phản ứng trạng thái vòng đời.

### Debounce đầu vào

Gộp các tin nhắn chỉ có văn bản được gửi nhanh từ cùng một người gửi thành một lượt agent duy nhất. Media/tệp đính kèm flush ngay lập tức. Lệnh điều khiển bỏ qua debounce.
...
- Khi `providers.openai.baseUrl` trỏ đến một endpoint không phải OpenAI, OpenClaw xem đó là máy chủ TTS tương thích OpenAI và nới lỏng kiểm tra model/giọng nói.
...
- `talk.provider` phải khớp với một khóa trong `talk.providers` khi nhiều nhà cung cấp Talk được cấu hình.
- Các khóa Talk phẳng cũ (`talk.voiceId`, `talk.voiceAliases`, `talk.modelId`, `talk.outputFormat`, `talk.apiKey`) chỉ để tương thích và được tự động di chuyển vào `talk.providers.<provider>`.
...
- `silenceTimeoutMs` điều khiển thời gian chế độ Talk chờ sau khi người dùng im lặng trước khi gửi transcript. Khi chưa đặt, giữ cửa sổ tạm dừng mặc định của nền tảng (`700 ms trên macOS và Android, 900 ms trên iOS`).
