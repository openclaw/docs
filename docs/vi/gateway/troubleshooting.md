...
read_when:
    ...
    - Bạn cần các mục hướng dẫn xử lý sự cố ổn định, dựa trên triệu chứng, kèm các lệnh chính xác
sidebarTitle: Troubleshooting
summary: Sổ tay vận hành xử lý sự cố chuyên sâu cho Gateway, kênh, tự động hóa, nút và trình duyệt
title: Khắc phục sự cố
x-i18n:
    generated_at: "2026-05-03T21:32:59Z"
    model: gpt-5.5
    provider: openai
...
Trang này là sổ tay vận hành chuyên sâu. Bắt đầu tại [/help/troubleshooting](/vi/help/troubleshooting) nếu bạn muốn xem trước quy trình phân loại nhanh.
...
<Warning>
Chỉ dành cho hạ cấp có chủ đích hoặc khôi phục khẩn cấp: đặt `OPENCLAW_ALLOW_OLDER_BINARY_DESTRUCTIVE_ACTIONS=1` cho một lệnh duy nhất. Để biến này chưa đặt trong vận hành bình thường.
</Warning>
...
| `AUTH_DEVICE_TOKEN_MISMATCH` | Mã thông báo theo thiết bị đã lưu trong bộ nhớ đệm đã cũ hoặc bị thu hồi.                                                                                                                                                 | Xoay vòng/phê duyệt lại mã thông báo thiết bị bằng [CLI thiết bị](/vi/cli/devices), rồi kết nối lại.                                                                                                                                                                                                        |
...
Dùng mục này khi khởi động Gateway thất bại với `Invalid config` hoặc nhật ký hot reload cho biết
...
- `Gateway accepted the WebSocket connection, but follow-up read diagnostics failed` → kết nối hoạt động, nhưng toàn bộ tập RPC chẩn đoán đã hết thời gian chờ hoặc thất bại. Xem đây là một Gateway có thể tiếp cận với chẩn đoán suy giảm; so sánh `connect.ok` và `connect.rpcOk` trong đầu ra `--json`.
...
- [Nhiều Gateway trên cùng một host](/vi/gateway#multiple-gateways-same-host)
- [Truy cập từ xa](/vi/gateway/remote)
