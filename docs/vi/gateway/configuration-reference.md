...
summary: Tham chiếu cấu hình Gateway cho các khóa OpenClaw cốt lõi, giá trị mặc định và liên kết đến các tham chiếu hệ thống con chuyên biệt
title: Tham chiếu cấu hình
x-i18n:
    generated_at: "2026-07-02T01:01:31Z"
    model: gpt-5.5
...
Tham chiếu cấu hình lõi cho `~/.openclaw/openclaw.json`. Để xem tổng quan theo định hướng tác vụ, xem [Cấu hình](/vi/gateway/configuration).

Bao quát các bề mặt cấu hình chính của OpenClaw và liên kết ra ngoài khi một hệ thống con có tham chiếu chuyên sâu riêng. Các danh mục lệnh do kênh và plugin sở hữu, cùng các nút điều chỉnh bộ nhớ sâu/QMD, nằm trên các trang riêng thay vì trên trang này.

Nguồn xác thực trong mã:

- `openclaw config schema` in JSON Schema trực tiếp dùng cho xác thực và Control UI, với siêu dữ liệu bundled/plugin/channel được hợp nhất khi có
...
Các khóa cấu hình theo kênh đã được chuyển sang trang riêng - xem
[Cấu hình - kênh](/vi/gateway/config-channels) cho `channels.*`,
bao gồm Slack, Discord, Telegram, WhatsApp, Matrix, iMessage, và các
kênh bundled khác (xác thực, kiểm soát truy cập, nhiều tài khoản, chặn theo mention).

## Giá trị mặc định của agent, đa agent, phiên và tin nhắn

Đã chuyển sang trang riêng - xem
...
- `talk.*` (chế độ Talk)
  - `talk.consultThinkingLevel`: ghi đè mức thinking cho toàn bộ lượt chạy agent OpenClaw phía sau các lượt tư vấn thời gian thực Control UI Talk
  - `talk.consultFastMode`: ghi đè chế độ nhanh một lần cho các lượt tư vấn thời gian thực Control UI Talk
  ...
  - `talk.realtime.consultRouting`: phương án dự phòng relay Gateway cho các bản chép lời Talk thời gian thực đã hoàn tất bỏ qua `openclaw_agent_consult`

## Công cụ và nhà cung cấp tùy chỉnh

Chính sách công cụ, công tắc thử nghiệm, cấu hình công cụ dựa trên nhà cung cấp, và thiết lập nhà cung cấp / base-URL tùy chỉnh đã được chuyển sang trang riêng - xem
[Cấu hình - công cụ và nhà cung cấp tùy chỉnh](/vi/gateway/config-tools).

## Model

Định nghĩa nhà cung cấp, danh sách model được phép, và thiết lập nhà cung cấp tùy chỉnh nằm trong
...
- `models.providers.*.localService`: trình quản lý tiến trình theo yêu cầu tùy chọn cho
  máy chủ model cục bộ. OpenClaw thăm dò endpoint tình trạng đã cấu hình, khởi động
  `command` tuyệt đối khi cần, chờ sẵn sàng, rồi gửi yêu cầu model. Xem [Dịch vụ model cục bộ](/vi/gateway/local-model-services).
- `models.pricing.enabled`: kiểm soát bootstrap định giá nền
  bắt đầu sau khi sidecar và kênh đạt đường sẵn sàng Gateway. Khi là `false`,
  Gateway bỏ qua việc lấy danh mục định giá OpenRouter và LiteLLM; các giá trị
...
OpenClaw nhúng cùng các adapter runtime khác tiêu thụ. Các lệnh `openclaw mcp list`,
...
- `mcp.servers`: định nghĩa máy chủ MCP stdio hoặc từ xa có tên cho các runtime
  ...
  `type: "http"` là alias riêng của CLI mà `openclaw mcp set` và
...
- `mcp.servers.<name>.timeout` / `requestTimeoutMs`: thời gian chờ yêu cầu MCP
  theo từng máy chủ, tính bằng giây hoặc mili giây.
- `mcp.servers.<name>.connectTimeout` / `connectionTimeoutMs`: thời gian chờ
  kết nối theo từng máy chủ, tính bằng giây hoặc mili giây.
- `mcp.servers.<name>.supportsParallelToolCalls`: gợi ý đồng thời tùy chọn cho
  các adapter có thể chọn có phát lệnh gọi công cụ MCP song song hay không.
...
- `mcp.servers.<name>.oauth`: ghi đè phạm vi OAuth, URL chuyển hướng và URL
  siêu dữ liệu client tùy chọn.
- `mcp.servers.<name>.sslVerify`, `clientCert`, `clientKey`: điều khiển TLS HTTP
  cho endpoint riêng và TLS hai chiều.
- `mcp.servers.<name>.toolFilter`: lựa chọn công cụ tùy chọn theo từng máy chủ. `include`
  giới hạn các công cụ MCP được phát hiện vào tên khớp; `exclude` ẩn các
  tên khớp. Mục nhập là tên công cụ MCP chính xác hoặc glob `*` đơn giản. Máy chủ có
...
- `mcp.servers.<name>.codex`: điều khiển phép chiếu app-server Codex tùy chọn.
  Khối này là siêu dữ liệu OpenClaw chỉ cho thread app-server Codex; nó không
  ảnh hưởng tới phiên ACP, cấu hình harness Codex chung, hoặc adapter runtime khác.
  `codex.agents` không rỗng giới hạn máy chủ vào các id agent OpenClaw đã liệt kê.
  Danh sách agent theo phạm vi rỗng, trắng hoặc không hợp lệ bị xác thực cấu hình
  từ chối và bị đường phép chiếu runtime bỏ qua thay vì trở thành toàn cục.
  ...
  trước khi truyền cấu hình `mcp_servers` gốc cho Codex. Bỏ qua khối này để
...
- `mcp.sessionIdleTtlMs`: TTL nhàn rỗi cho runtime MCP bundled theo phạm vi phiên.
  Các lượt chạy nhúng một lần yêu cầu dọn dẹp khi kết thúc chạy; TTL này là chốt chặn cho
  các phiên sống lâu và caller tương lai.
- Thay đổi dưới `mcp.*` áp dụng nóng bằng cách hủy các runtime MCP phiên được cache.
...
- Khám phá runtime cũng tôn trọng thông báo thay đổi danh sách công cụ MCP bằng cách loại bỏ
  danh mục đã cache cho phiên đó. Máy chủ quảng bá tài nguyên hoặc
  prompt nhận công cụ tiện ích để liệt kê/đọc tài nguyên và liệt kê/lấy
...
- `allowBundled`: danh sách cho phép tùy chọn chỉ cho skills bundled (không ảnh hưởng tới skills được quản lý/workspace).
- `load.extraDirs`: gốc skill dùng chung bổ sung (độ ưu tiên thấp nhất).
...
- `install.preferBrew`: khi đúng, ưu tiên trình cài Homebrew khi `brew` có sẵn
  trước khi quay về các loại trình cài khác.
- `install.nodeManager`: tùy chọn trình cài node cho thông số `metadata.openclaw.install`
  (`npm` | `pnpm` | `yarn` | `bun`).
- `install.allowUploadedArchives`: cho phép client Gateway `operator.admin` đáng tin cậy
  cài đặt kho lưu trữ zip riêng được stage qua `skills.upload.*`
  (mặc định: false). Điều này chỉ bật đường uploaded-archive; các cài đặt ClawHub
  thông thường không cần nó.
- `entries.<skillKey>.enabled: false` vô hiệu hóa một skill ngay cả khi đã bundled/cài đặt.
...
- Được tải từ các thư mục gói hoặc bundle trong `~/.openclaw/extensions` và `<workspace>/.openclaw/extensions`, cộng với các tệp hoặc thư mục được liệt kê trong `plugins.load.paths`.
...
- Cơ chế phát hiện chấp nhận các Plugin OpenClaw gốc cùng các bundle Codex và bundle Claude tương thích, bao gồm các bundle bố cục mặc định Claude không có manifest.
...
- `plugins.entries.<id>.hooks.allowPromptInjection`: khi là `false`, lõi chặn `before_prompt_build` và bỏ qua các trường thay đổi prompt từ `before_agent_start` cũ, đồng thời vẫn giữ `modelOverride` và `providerOverride` cũ. Áp dụng cho hook Plugin gốc và các thư mục hook do bundle cung cấp được hỗ trợ.
- `plugins.entries.<id>.hooks.allowConversationAccess`: khi là `true`, các Plugin không đóng gói đáng tin cậy có thể đọc nội dung cuộc trò chuyện thô từ các hook có kiểu như `llm_input`, `llm_output`, `before_model_resolve`, `before_agent_reply`, `before_agent_run`, `before_agent_finalize` và `agent_end`.
...
- `plugins.entries.<id>.subagent.allowedModels`: danh sách cho phép tùy chọn gồm các đích `provider/model` chuẩn cho các ghi đè subagent đáng tin cậy. Chỉ dùng `"*"` khi bạn chủ ý muốn cho phép bất kỳ mô hình nào.
...
- `plugins.entries.<id>.llm.allowedModels`: danh sách cho phép tùy chọn gồm các đích `provider/model` chuẩn cho các ghi đè hoàn tất LLM của Plugin đáng tin cậy. Chỉ dùng `"*"` khi bạn chủ ý muốn cho phép bất kỳ mô hình nào.
...
- Thiết lập tài khoản/thời gian chạy của Plugin kênh nằm trong `channels.<id>` và nên được mô tả bằng metadata `channelConfigs` trong manifest của Plugin sở hữu, không phải bằng một sổ đăng ký tùy chọn OpenClaw trung tâm.

### Cấu hình Plugin khung chạy Codex

Plugin `codex` đi kèm sở hữu các thiết lập khung chạy máy chủ ứng dụng Codex gốc trong
`plugins.entries.codex.config`. Xem
[Tham chiếu khung chạy Codex](/vi/plugins/codex-harness-reference) để biết toàn bộ bề mặt cấu hình
và [Khung chạy Codex](/vi/plugins/codex-harness) để biết mô hình thời gian chạy.

`codexPlugins` chỉ áp dụng cho các phiên chọn khung chạy Codex gốc.
Nó không bật Plugin Codex cho các lần chạy nhà cung cấp OpenClaw, các ràng buộc cuộc trò chuyện
ACP, hoặc bất kỳ khung chạy không phải Codex nào.
...
- `plugins.entries.codex.config.codexPlugins.enabled`: bật hỗ trợ
  Plugin/ứng dụng Codex gốc cho khung chạy Codex. Mặc định: `false`.
- `plugins.entries.codex.config.codexPlugins.allow_destructive_actions`:
  ...
  để từ chối chúng, `"auto"` để định tuyến các phê duyệt do Codex yêu cầu qua
  phê duyệt Plugin OpenClaw, hoặc `"ask"` để nhắc cho mọi hành động ghi/phá hủy
  của Plugin mà không có phê duyệt bền vững. Chế độ `"ask"` xóa các ghi đè
  phê duyệt theo công cụ Codex bền vững cho ứng dụng bị ảnh hưởng và chọn người
  đánh giá phê duyệt là con người cho ứng dụng đó trước khi luồng Codex bắt đầu.
  Mặc định: `true`.
...
Mỗi ứng dụng Plugin được chấp nhận dùng `"ask"` sẽ định tuyến các yêu cầu phê duyệt
của ứng dụng đó đến người đánh giá là con người. Các ứng dụng khác và phê duyệt luồng
không phải ứng dụng giữ người đánh giá đã cấu hình của chúng, nên các chính sách Plugin
hỗn hợp không kế thừa hành vi `"ask"`.

`codexPlugins.enabled` là chỉ thị bật toàn cục. Các mục Plugin rõ ràng được ghi bởi
quá trình di chuyển là tập hợp cài đặt bền vững và đủ điều kiện sửa chữa.
`plugins["*"]` không được hỗ trợ, không có công tắc `install`, và các giá trị
`marketplacePath` cục bộ cố ý không phải là trường cấu hình vì chúng phụ thuộc vào host.

Các kiểm tra sẵn sàng `app/list` được lưu cache trong một giờ và được làm mới
bất đồng bộ khi cũ. Cấu hình ứng dụng luồng Codex được tính khi thiết lập phiên
khung chạy Codex, không phải ở mọi lượt; dùng `/new`, `/reset`, hoặc khởi động lại
Gateway sau khi thay đổi cấu hình Plugin gốc.
...
- `plugins.entries.firecrawl.config.webFetch`: thiết lập nhà cung cấp tìm nạp web Firecrawl.
  ...
  - `baseUrl`: URL cơ sở API Firecrawl (mặc định: `https://api.firecrawl.dev`; các ghi đè tự host phải trỏ đến endpoint riêng tư/nội bộ).
...
- `plugins.entries.memory-core.config.dreaming`: thiết lập dreaming bộ nhớ. Xem [Dreaming](/vi/concepts/dreaming) để biết các pha và ngưỡng.
  ...
  - `model`: ghi đè mô hình subagent Dream Diary tùy chọn. Yêu cầu `plugins.entries.memory-core.subagent.allowModelOverride: true`; ghép với `allowedModels` để giới hạn đích. Lỗi không có mô hình sẽ thử lại một lần với mô hình mặc định của phiên; lỗi tin cậy hoặc danh sách cho phép không âm thầm dự phòng.
...
- Các Plugin bundle Claude đã bật cũng có thể đóng góp mặc định OpenClaw nhúng từ `settings.json`; OpenClaw áp dụng chúng như thiết lập agent đã được làm sạch, không phải như bản vá cấu hình OpenClaw thô.
...
`commitments` kiểm soát bộ nhớ theo dõi suy luận: OpenClaw có thể phát hiện các lần kiểm tra lại từ các lượt trò chuyện và gửi chúng qua các lần chạy Heartbeat.
...
- Chỉ đặt `ssrfPolicy.dangerouslyAllowPrivateNetwork: true` khi bạn chủ ý tin cậy điều hướng trình duyệt mạng riêng.
...
- Hồ sơ `existing-session` có thể đặt `userDataDir` để nhắm tới một hồ sơ
  trình duyệt dựa trên Chromium cụ thể như Brave hoặc Edge.
- Hồ sơ `existing-session` có thể đặt `cdpUrl` khi Chrome đã chạy
  phía sau một endpoint phát hiện DevTools HTTP(S) hoặc endpoint WS(S) trực tiếp. Trong
  chế độ đó, OpenClaw truyền endpoint cho Chrome MCP thay vì dùng tự động kết nối;
...
- Hồ sơ `openclaw` được quản lý cục bộ tự động gán `cdpPort` và `cdpUrl`; chỉ đặt
  `cdpUrl` tường minh cho hồ sơ CDP từ xa hoặc đính kèm endpoint existing-session.
...
- Hồ sơ được quản lý cục bộ dùng `browser.localLaunchTimeoutMs` cho phát hiện HTTP Chrome CDP
...
- `assistant`: ghi đè danh tính Control UI. Quay về danh tính tác nhân đang hoạt động nếu không có.
...
- Nếu cả `gateway.auth.token` và `gateway.auth.password` đều được cấu hình (bao gồm SecretRefs), hãy đặt rõ `gateway.auth.mode` thành `token` hoặc `password`. Các luồng khởi động và cài đặt/sửa chữa dịch vụ sẽ thất bại khi cả hai đều được cấu hình nhưng mode chưa được đặt.
...
- `gateway.auth.mode: "trusted-proxy"`: ủy quyền xác thực trình duyệt/người dùng cho một reverse proxy nhận biết danh tính và tin cậy các header danh tính từ `gateway.trustedProxies` (xem [Xác thực Trusted Proxy](/vi/gateway/trusted-proxy-auth)). Chế độ này mặc định kỳ vọng một nguồn proxy **không phải loopback**; các reverse proxy loopback cùng máy chủ yêu cầu đặt rõ `gateway.auth.trustedProxy.allowLoopback = true`. Các bên gọi nội bộ cùng máy chủ có thể dùng `gateway.auth.password` làm phương án dự phòng trực tiếp cục bộ; `gateway.auth.token` vẫn loại trừ lẫn nhau với chế độ trusted-proxy.
- `gateway.auth.allowTailscale`: khi là `true`, các header danh tính Tailscale Serve có thể đáp ứng xác thực Control UI/WebSocket (được xác minh qua `tailscale whois`). Các điểm cuối HTTP API **không** dùng xác thực header Tailscale đó; thay vào đó chúng tuân theo chế độ xác thực HTTP thông thường của gateway. Luồng không token này giả định máy chủ gateway là đáng tin cậy. Mặc định là `true` khi `tailscale.mode = "serve"`.
- `gateway.auth.rateLimit`: bộ giới hạn xác thực thất bại tùy chọn. Áp dụng theo IP khách và theo phạm vi xác thực (shared-secret và device-token được theo dõi độc lập). Các lần thử bị chặn trả về `429` + `Retry-After`.
  - Trên đường dẫn Control UI Tailscale Serve bất đồng bộ, các lần thử thất bại cho cùng `{scope, clientIp}` được tuần tự hóa trước khi ghi thất bại. Vì vậy, các lần thử sai đồng thời từ cùng một khách có thể kích hoạt bộ giới hạn ở yêu cầu thứ hai thay vì cả hai cùng chạy qua như các lỗi không khớp đơn thuần.
...
- Các lần thử xác thực WS có origin từ trình duyệt luôn bị tiết lưu với miễn trừ loopback bị tắt (phòng thủ theo chiều sâu chống brute force localhost dựa trên trình duyệt).
...
- `tailscale.serviceName`: tên Service Tailscale tùy chọn cho chế độ Serve, chẳng
  hạn như `svc:openclaw`. Khi được đặt, OpenClaw truyền giá trị đó cho `tailscale serve
--service` để Control UI có thể được phơi bày qua một Service được đặt tên thay
  vì hostname của thiết bị. Giá trị phải dùng định dạng tên Service `svc:<dns-label>`
...
- `tailscale.preserveFunnel`: khi là `true` và `tailscale.mode = "serve"`, OpenClaw
  ...
  Mặc định là `false`.
- `controlUi.allowedOrigins`: danh sách cho phép origin trình duyệt tường minh cho các kết nối Gateway WebSocket. Bắt buộc cho các origin trình duyệt công khai không phải loopback. Các lần tải UI LAN/Tailnet riêng tư cùng origin từ loopback, RFC1918/link-local, `.local`, `.ts.net`, hoặc các máy chủ Tailscale CGNAT được chấp nhận mà không cần bật phương án dự phòng Host-header.
- `controlUi.chatMessageMaxWidth`: chiều rộng tối đa tùy chọn cho các tin nhắn trò chuyện Control UI được nhóm. Chấp nhận các giá trị chiều rộng CSS bị ràng buộc như `960px`, `82%`, `min(1280px, 82%)`, và `calc(100% - 2rem)`.
- `controlUi.dangerouslyAllowHostHeaderOriginFallback`: chế độ nguy hiểm bật phương án dự phòng origin Host-header cho các triển khai cố ý dựa vào chính sách origin Host-header.
- `remote.transport`: `ssh` (mặc định) hoặc `direct` (ws/wss). Với `direct`, `remote.url` phải là `wss://` cho các máy chủ công khai; `ws://` dạng văn bản thuần chỉ được chấp nhận cho loopback, LAN, link-local, `.local`, `.ts.net`, và các máy chủ Tailscale CGNAT.
- `remote.remotePort`: cổng gateway trên máy chủ SSH từ xa. Mặc định là `18789`; dùng giá trị này khi cổng tunnel cục bộ khác với cổng gateway từ xa.
- `gateway.remote.token` / `.password` là các trường thông tin đăng nhập của máy khách từ xa. Tự chúng không cấu hình xác thực gateway.
- `gateway.push.apns.relay.baseUrl`: URL HTTPS cơ sở cho relay APNs bên ngoài được dùng sau khi các bản dựng iOS dựa trên relay xuất bản đăng ký lên gateway. Các bản dựng App Store/TestFlight công khai dùng relay OpenClaw được lưu trữ. URL relay tùy chỉnh phải khớp với một đường dẫn bản dựng/triển khai iOS được tách riêng có chủ ý, trong đó URL relay trỏ tới relay đó.
- `gateway.push.apns.relay.timeoutMs`: thời gian chờ gửi từ gateway tới relay tính bằng mili giây. Mặc định là `10000`.
- Các đăng ký dựa trên relay được ủy quyền cho một danh tính gateway cụ thể. Ứng dụng iOS được ghép cặp gọi `gateway.identity.get`, bao gồm danh tính đó trong đăng ký relay, và chuyển tiếp một quyền gửi theo phạm vi đăng ký tới gateway. Gateway khác không thể dùng lại đăng ký đã lưu đó.
...
- `gateway.handshakeTimeoutMs`: thời gian chờ bắt tay Gateway WebSocket trước xác thực tính bằng mili giây. Mặc định: `15000`. `OPENCLAW_HANDSHAKE_TIMEOUT_MS` có ưu tiên khi được đặt. Tăng giá trị này trên các máy chủ tải cao hoặc công suất thấp, nơi các máy khách cục bộ có thể kết nối khi phần khởi động làm nóng vẫn đang ổn định.
- `gateway.channelHealthCheckMinutes`: khoảng thời gian giám sát sức khỏe kênh tính bằng phút. Đặt `0` để tắt khởi động lại do bộ giám sát sức khỏe trên toàn cục. Mặc định: `5`.
...
- `channels.<provider>.accounts.<accountId>.healthMonitor.enabled`: ghi đè theo tài khoản cho các kênh nhiều tài khoản. Khi được đặt, nó có ưu tiên hơn ghi đè cấp kênh.
- Các đường dẫn gọi gateway cục bộ chỉ có thể dùng `gateway.remote.*` làm phương án dự phòng khi `gateway.auth.*` chưa được đặt.
- Nếu `gateway.auth.token` / `gateway.auth.password` được cấu hình tường minh qua SecretRef và không phân giải được, quá trình phân giải thất bại đóng (không có phương án dự phòng từ xa che giấu).
- `trustedProxies`: IP reverse proxy kết thúc TLS hoặc chèn header máy khách được chuyển tiếp. Chỉ liệt kê các proxy bạn kiểm soát. Các mục loopback vẫn hợp lệ cho thiết lập proxy/phát hiện cục bộ cùng máy chủ (ví dụ Tailscale Serve hoặc một reverse proxy cục bộ), nhưng chúng **không** làm cho các yêu cầu loopback đủ điều kiện cho `gateway.auth.mode: "trusted-proxy"`.
...
- `gateway.nodes.pairing.autoApproveCidrs`: danh sách cho phép CIDR/IP tùy chọn để tự động phê duyệt ghép cặp thiết bị node lần đầu khi không có phạm vi được yêu cầu. Tùy chọn này bị tắt khi chưa đặt. Điều này không tự động phê duyệt ghép cặp operator/trình duyệt/Control UI/WebChat, và không tự động phê duyệt nâng cấp vai trò, phạm vi, metadata, hoặc khóa công khai.
- `gateway.nodes.allowCommands` / `gateway.nodes.denyCommands`: định hình cho phép/từ chối toàn cục cho các lệnh node đã khai báo sau khi ghép cặp và đánh giá danh sách cho phép nền tảng. Dùng `allowCommands` để chọn bật các lệnh node nguy hiểm như `camera.snap`, `camera.clip`, và `screen.record`; `denyCommands` loại bỏ một lệnh ngay cả khi mặc định nền tảng hoặc cho phép tường minh lẽ ra sẽ bao gồm lệnh đó. Sau khi một node thay đổi danh sách lệnh đã khai báo của nó, hãy từ chối và phê duyệt lại ghép cặp thiết bị đó để gateway lưu snapshot lệnh đã cập nhật.
- `gateway.tools.deny`: tên công cụ bổ sung bị chặn cho HTTP `POST /tools/invoke` (mở rộng danh sách từ chối mặc định).
- `gateway.tools.allow`: xóa tên công cụ khỏi danh sách từ chối HTTP mặc định cho
  bên gọi owner/admin. Điều này không nâng cấp các bên gọi `operator.write`
  mang danh tính thành quyền truy cập owner/admin; `cron`, `gateway`, và `nodes` vẫn
  không khả dụng cho các bên gọi không phải owner ngay cả khi được đưa vào danh sách cho phép.
...
Chạy nhiều gateway trên một máy chủ với các cổng và thư mục trạng thái riêng:
...
- `autoGenerate`: tự động tạo cặp chứng chỉ/khóa tự ký cục bộ khi các tệp tường minh chưa được cấu hình; chỉ dùng cho cục bộ/phát triển.
- `certPath`: đường dẫn hệ thống tệp tới tệp chứng chỉ TLS.
- `keyPath`: đường dẫn hệ thống tệp tới tệp khóa riêng TLS; giữ giới hạn quyền truy cập.
- `caPath`: đường dẫn gói CA tùy chọn cho xác minh máy khách hoặc chuỗi tin cậy tùy chỉnh.
...
- `hooks.token` nên khác với xác thực shared-secret Gateway đang hoạt động (`gateway.auth.token` / `OPENCLAW_GATEWAY_TOKEN` hoặc `gateway.auth.password` / `OPENCLAW_GATEWAY_PASSWORD`); khi phát hiện dùng lại, quá trình khởi động sẽ ghi log cảnh báo bảo mật không gây lỗi.
- `openclaw security audit` gắn cờ việc dùng lại xác thực hook/Gateway là phát hiện nghiêm trọng, bao gồm xác thực mật khẩu Gateway chỉ được cung cấp tại thời điểm audit (`--auth password --password <password>`). Chạy `openclaw doctor --fix` để xoay vòng `hooks.token` đã lưu bị dùng lại, rồi cập nhật các bộ gửi hook bên ngoài để dùng token hook mới.
...
- `POST /hooks/<name>` → được phân giải qua `hooks.mappings`
  - Giá trị `sessionKey` của ánh xạ được render từ mẫu được xem là do bên ngoài cung cấp và cũng yêu cầu `hooks.allowRequestSessionKey=true`.
...
- `transform` có thể trỏ đến một mô-đun JS/TS trả về một hành động hook.
  ...
  - Giữ `hooks.transformsDir` trong `~/.openclaw/hooks/transforms`; thư mục Skills trong workspace bị từ chối. Nếu `openclaw doctor` báo cáo đường dẫn này không hợp lệ, hãy di chuyển mô-đun transform vào thư mục transforms của hooks hoặc xóa `hooks.transformsDir`.
...
- Nếu bạn giữ định tuyến theo từng thư đó, hãy đặt `hooks.allowRequestSessionKey: true` và ràng buộc `hooks.allowedSessionKeyPrefixes` để khớp với namespace Gmail, ví dụ `["hook:", "hook:gmail:"]`.
...
- Không chạy một `gog gmail watch serve` riêng song song với Gateway.
...
- Node WebViews thường không gửi header xác thực; sau khi một node được ghép đôi và kết nối, Gateway quảng bá URL capability theo phạm vi node để truy cập canvas/A2UI.
- URL capability được ràng buộc với phiên WS node đang hoạt động và hết hạn nhanh. Không dùng phương án dự phòng dựa trên IP.
...
- Hostname mặc định là hostname hệ thống khi đó là nhãn DNS hợp lệ, nếu không sẽ quay về `openclaw`. Ghi đè bằng `OPENCLAW_MDNS_HOSTNAME`.
...
Ghi một vùng DNS-SD unicast dưới `~/.openclaw/dns/`. Để khám phá xuyên mạng, ghép với máy chủ DNS (khuyến nghị CoreDNS) + DNS phân tách của Tailscale.
...
- Tham chiếu `auth-profiles.json` được bao gồm trong phân giải lúc chạy và phạm vi kiểm tra.
...
- Đường dẫn nhà cung cấp file và exec sẽ thất bại theo hướng đóng khi không thể xác minh ACL của Windows. Chỉ đặt `allowInsecurePath: true` cho các đường dẫn đáng tin cậy không thể xác minh.
...
- Tham chiếu bí mật được phân giải tại thời điểm kích hoạt thành một bản chụp trong bộ nhớ, sau đó các đường dẫn yêu cầu chỉ đọc bản chụp đó.
...
- Các bản đồ `auth-profiles.json` phẳng kế thừa như `{ "provider": { "apiKey": "..." } }` không phải là định dạng runtime; `openclaw doctor --fix` ghi lại chúng thành hồ sơ khóa API `provider:default` chuẩn với bản sao lưu `.legacy-flat.*.bak`.
- Hồ sơ chế độ OAuth (`auth.profiles.<id>.mode = "oauth"`) không hỗ trợ thông tin xác thực auth-profile dựa trên SecretRef.
- Thông tin xác thực runtime tĩnh đến từ các ảnh chụp nhanh đã phân giải trong bộ nhớ; các mục `auth.json` tĩnh kế thừa sẽ được xóa sạch khi được phát hiện.
- OAuth kế thừa nhập từ `~/.openclaw/credentials/oauth.json`.
...
- `billingBackoffHours`: thời gian chờ lùi cơ sở tính bằng giờ khi một hồ sơ thất bại do lỗi thanh toán/không đủ tín dụng thực sự (mặc định: `5`). Văn bản thanh toán rõ ràng vẫn có thể rơi vào đây ngay cả trên phản hồi `401`/`403`, nhưng các bộ khớp văn bản riêng theo nhà cung cấp vẫn giới hạn trong nhà cung cấp sở hữu chúng (ví dụ OpenRouter `Key limit exceeded`). Các thông báo HTTP `402` có thể thử lại về cửa sổ sử dụng hoặc giới hạn chi tiêu của tổ chức/không gian làm việc vẫn đi theo đường dẫn `rate_limit`.
- `billingBackoffHoursByProvider`: ghi đè tùy chọn theo từng nhà cung cấp cho số giờ chờ lùi thanh toán.
- `billingMaxHours`: giới hạn tính bằng giờ cho tăng trưởng hàm mũ của chờ lùi thanh toán (mặc định: `24`).
...
- `overloadedProfileRotations`: số lần xoay vòng auth-profile tối đa trong cùng nhà cung cấp đối với lỗi quá tải trước khi chuyển sang phương án dự phòng theo mô hình (mặc định: `1`). Các dạng nhà cung cấp bận như `ModelNotReadyException` rơi vào đây.
- `overloadedBackoffMs`: độ trễ cố định trước khi thử lại một lần xoay vòng nhà cung cấp/hồ sơ quá tải (mặc định: `0`).
- `rateLimitedProfileRotations`: số lần xoay vòng auth-profile tối đa trong cùng nhà cung cấp đối với lỗi giới hạn tốc độ trước khi chuyển sang phương án dự phòng theo mô hình (mặc định: `1`). Nhóm giới hạn tốc độ đó bao gồm văn bản theo dạng nhà cung cấp như `Too many concurrent requests`, `ThrottlingException`, `concurrency limit reached`, `workers_ai ... quota limit exceeded`, và `resource exhausted`.
...
- `stuckSessionWarnMs`: ngưỡng tuổi không có tiến triển tính bằng ms để phân loại các phiên xử lý chạy lâu là `session.long_running`, `session.stalled`, hoặc `session.stuck`. Phản hồi, công cụ, trạng thái, khối, và tiến trình ACP đặt lại bộ hẹn giờ; các chẩn đoán `session.stuck` lặp lại sẽ lùi lại khi không thay đổi.
- `stuckSessionAbortMs`: ngưỡng tuổi không có tiến triển tính bằng ms trước khi công việc đang hoạt động bị kẹt đủ điều kiện có thể được hủy-xả để phục hồi. Khi không đặt, OpenClaw dùng cửa sổ chạy nhúng mở rộng an toàn hơn, tối thiểu 5 phút và gấp 3 lần `stuckSessionWarnMs`.
- `memoryPressureSnapshot`: ghi lại ảnh chụp nhanh ổn định đã biên tập trước OOM khi áp lực bộ nhớ đạt `critical` (mặc định: `false`). Đặt thành `true` để thêm lần quét/ghi tệp gói ổn định trong khi vẫn giữ các sự kiện áp lực bộ nhớ bình thường.
...
- `otel.captureContent`: tùy chọn ghi lại nội dung thô cho thuộc tính span OTEL. Mặc định là tắt. Boolean `true` ghi lại nội dung thông điệp/công cụ không thuộc hệ thống; dạng đối tượng cho phép bạn bật rõ ràng `inputMessages`, `outputMessages`, `toolInputs`, `toolOutputs`, `systemPrompt`, và `toolDefinitions`.
- `OTEL_SEMCONV_STABILITY_OPT_IN=gen_ai_latest_experimental`: công tắc môi trường cho dạng span suy luận GenAI thử nghiệm mới nhất, bao gồm tên span `{gen_ai.operation.name} {gen_ai.request.model}`, loại span `CLIENT`, và `gen_ai.provider.name` thay cho `gen_ai.system` kế thừa. Theo mặc định, span giữ `openclaw.model.call` và `gen_ai.system` để tương thích; metric GenAI dùng thuộc tính ngữ nghĩa có giới hạn.
- `OPENCLAW_OTEL_PRELOADED=1`: công tắc môi trường cho host đã đăng ký sẵn SDK OpenTelemetry toàn cục. Khi đó OpenClaw bỏ qua khởi động/tắt SDK do Plugin sở hữu trong khi vẫn giữ trình nghe chẩn đoán hoạt động.
- `OTEL_EXPORTER_OTLP_TRACES_ENDPOINT`, `OTEL_EXPORTER_OTLP_METRICS_ENDPOINT`, và `OTEL_EXPORTER_OTLP_LOGS_ENDPOINT`: biến môi trường endpoint riêng theo tín hiệu được dùng khi khóa cấu hình tương ứng chưa đặt.
- `cacheTrace.enabled`: ghi nhật ký ảnh chụp nhanh trace bộ nhớ đệm cho các lần chạy nhúng (mặc định: `false`).
...
- `enabled`: cổng tính năng ACP toàn cục (mặc định: `true`; đặt `false` để ẩn khả năng điều phối và spawn ACP).
...
- `defaultAgent`: id agent đích ACP dự phòng khi spawn không chỉ định đích rõ ràng.
...
- `stream.maxChunkChars`: kích thước khối tối đa trước khi tách phép chiếu khối được stream.
...
- `stream.tagVisibility`: bản ghi tên thẻ tới ghi đè hiển thị boolean cho các sự kiện được stream.
...
- `cli.banner.taglineMode` kiểm soát kiểu khẩu hiệu của biểu ngữ:
  ...
  - `"off"`: không có văn bản khẩu hiệu (tiêu đề/phiên bản biểu ngữ vẫn hiển thị).
- Để ẩn toàn bộ biểu ngữ (không chỉ khẩu hiệu), đặt env `OPENCLAW_HIDE_BANNER=1`.
...
Các bản dựng hiện tại không còn bao gồm bridge TCP. Các nút kết nối qua Gateway WebSocket. Các khóa `bridge.*` không còn là một phần của schema cấu hình (xác thực sẽ thất bại cho đến khi chúng bị gỡ bỏ; `openclaw doctor --fix` có thể loại bỏ các khóa không xác định).
...
- `webhookToken`: token bearer dùng cho việc gửi POST Webhook cron (`delivery.mode = "webhook"`), nếu bỏ qua thì không gửi header xác thực.
- `webhook`: URL Webhook dự phòng cũ đã ngừng khuyến nghị (http/https), được `openclaw doctor --fix` dùng để di chuyển các tác vụ đã lưu vẫn có `notify: true`; việc gửi khi chạy dùng `delivery.mode="webhook"` theo từng tác vụ cộng với `delivery.to`, hoặc `delivery.completionDestination` khi giữ nguyên gửi thông báo.
...
- `maxAttempts`: số lần thử lại tối đa cho tác vụ cron khi gặp lỗi tạm thời (mặc định: `3`; phạm vi: `0`-`10`).
...
Các tác vụ chạy một lần vẫn bật cho đến khi hết số lần thử lại, sau đó tắt trong khi vẫn giữ trạng thái lỗi cuối cùng. Các tác vụ lặp lại dùng cùng chính sách thử lại tạm thời để chạy lại sau backoff trước khung lịch tiếp theo; lỗi vĩnh viễn hoặc hết số lần thử lại tạm thời sẽ quay về lịch lặp bình thường với backoff lỗi.
...
- `to`: đích thông báo rõ ràng hoặc URL Webhook. Bắt buộc đối với chế độ Webhook.
...
- Khi không đặt đích lỗi toàn cục hoặc theo từng tác vụ, các tác vụ vốn đã gửi qua `announce` sẽ quay về đích thông báo chính đó khi lỗi.
...
Xem [Tác vụ Cron](/vi/automation/cron-jobs). Các lượt thực thi cron cô lập được theo dõi như [tác vụ nền](/vi/automation/tasks).
...
- Khóa cùng cấp: hợp nhất sau includes (ghi đè các giá trị đã include).
- Includes lồng nhau: tối đa 10 cấp sâu.
...
- Các lần ghi do OpenClaw sở hữu chỉ thay đổi một phần cấp cao nhất được hỗ trợ bởi include một tệp sẽ ghi xuyên qua tệp được include đó. Ví dụ, `plugins install` cập nhật `plugins: { $include: "./plugins.json5" }` trong `plugins.json5` và giữ nguyên `openclaw.json`.
- Include gốc, mảng include và include có ghi đè cùng cấp là chỉ đọc đối với các lần ghi do OpenClaw sở hữu; các lần ghi đó fail-closed thay vì làm phẳng cấu hình.
- Lỗi: thông báo rõ ràng cho tệp bị thiếu, lỗi phân tích cú pháp, include vòng tròn, định dạng đường dẫn không hợp lệ và độ dài quá mức.
