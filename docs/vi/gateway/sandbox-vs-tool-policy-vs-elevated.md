...
status: active
summary: 'Vì sao một công cụ bị chặn: môi trường thực thi cách ly, chính sách cho phép/từ chối công cụ, và các cổng kiểm soát thực thi nâng quyền'
title: Môi trường cô lập so với chính sách công cụ so với quyền nâng cao
x-i18n:
...
Xem [Hộp cát](/vi/gateway/sandboxing) để biết ma trận đầy đủ (phạm vi, mount workspace, image).

### Bind mount (kiểm tra nhanh về bảo mật)

- `docker.binds` _xuyên qua_ hệ thống tệp của hộp cát: bất cứ thứ gì bạn mount đều hiển thị bên trong container với chế độ bạn đặt (`:ro` hoặc `:rw`).
...
- **Chính sách công cụ hộp cát** (chỉ áp dụng khi đang trong hộp cát): `tools.sandbox.tools.allow`/`tools.sandbox.tools.deny` và `agents.list[].tools.sandbox.tools.*`
...
- `/exec` chỉ thay đổi mặc định của phiên cho người gửi được ủy quyền; nó không cấp quyền truy cập công cụ.
  Khóa công cụ của provider chấp nhận cả `provider` (ví dụ `google-antigravity`) hoặc `provider/model` (ví dụ `openai/gpt-5.4`).
...
Ở chế độ `"non-main"`, khóa nhóm/kênh _không_ phải là main. Dùng khóa phiên main (được hiển thị bởi `sandbox explain`) hoặc chuyển chế độ sang `"off"`.

## Liên quan

- [Hộp cát](/vi/gateway/sandboxing) -- tham chiếu đầy đủ về hộp cát (chế độ, phạm vi, backend, image)
- [Hộp cát và công cụ đa agent](/vi/tools/multi-agent-sandbox-tools) -- ghi đè và thứ tự ưu tiên theo từng agent
