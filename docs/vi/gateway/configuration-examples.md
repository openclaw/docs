...
Các ví dụ dưới đây được căn chỉnh với schema cấu hình hiện tại. Để xem tài liệu tham khảo đầy đủ và ghi chú theo từng trường, hãy xem [Cấu hình](/vi/gateway/configuration).
...
### Kho repo Skills cùng cấp được liên kết tượng trưng

Dùng cấu hình này khi một gốc Skills tích hợp sẵn chứa liên kết tượng trưng đến một repo cùng cấp, ví dụ `~/.agents/skills/manager -> ~/Projects/manager/skills`.
...
- `extraDirs` quét repo cùng cấp dưới dạng một gốc Skills rõ ràng.
- `allowSymlinkTargets` cho phép các thư mục Skills được liên kết tượng trưng phân giải đến gốc đích thực đáng tin cậy đó mà không cho phép thoát liên kết tượng trưng tùy ý.
- Để cho phép Skill Workshop ghi qua cùng đích liên kết tượng trưng đáng tin cậy đó, hãy đặt `skills.workshop.allowSymlinkTargetWrites: true`.
...
thiết bị Node trong lần đầu bằng CIDR hoặc IP chính xác:
...
Tùy chọn này vẫn tắt khi chưa được đặt. Nó chỉ áp dụng cho ghép đôi `role: node` mới
không có phạm vi được yêu cầu. Máy khách operator/trình duyệt và các nâng cấp vai trò,
...
Nếu nhiều người có thể gửi DM cho bot của bạn (nhiều mục trong `allowFrom`, phê duyệt ghép đôi cho nhiều người, hoặc `dmPolicy: "open"`), hãy bật **chế độ DM bảo mật** để DM từ những người gửi khác nhau mặc định không dùng chung một ngữ cảnh:
...
Với Discord/Slack/Google Chat/Microsoft Teams/Mattermost/IRC, việc ủy quyền người gửi mặc định ưu tiên ID.
Chỉ bật khớp trực tiếp theo tên/email/biệt danh có thể thay đổi bằng `dangerouslyAllowNameMatching: true` của từng kênh nếu bạn chấp nhận rõ ràng rủi ro đó.
