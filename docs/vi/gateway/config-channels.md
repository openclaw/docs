...
read_when:
    ...
    - Kiểm tra chính sách tin nhắn trực tiếp, chính sách nhóm hoặc cơ chế kiểm soát khi được nhắc đến
...
title: Cấu hình — kênh
x-i18n:
    generated_at: "2026-05-10T19:33:25Z"
    model: gpt-5.5
    provider: openai
...
thiết lập nhiều tài khoản, chặn theo lượt nhắc, và các khóa theo từng kênh cho Slack, Discord,
Telegram, WhatsApp, Matrix, iMessage và các Plugin kênh được đóng gói kèm khác.

Đối với tác tử, công cụ, runtime Gateway và các khóa cấp cao nhất khác, xem
...
| ------------------- | --------------------------------------------------------------- |
| `pairing` (mặc định) | Người gửi không xác định nhận được mã ghép nối dùng một lần; chủ sở hữu phải phê duyệt |
...
Nếu toàn bộ khối nhà cung cấp bị thiếu (`channels.<provider>` vắng mặt), chính sách nhóm runtime quay về `allowlist` (đóng khi lỗi) kèm cảnh báo khi khởi động.
</Note>
...
Dùng `channels.defaults` cho hành vi chính sách nhóm và Heartbeat dùng chung trên các nhà cung cấp:
...
- `channels.defaults.heartbeat.useIndicator`: kết xuất đầu ra Heartbeat kiểu chỉ báo nhỏ gọn.
...
- Các mục `bindings[]` cấp cao nhất với `type: "acp"` cấu hình liên kết ACP bền vững cho chủ đề diễn đàn (dùng `chatId:topic:topicId` chính tắc trong `match.peer.id`). Ngữ nghĩa trường được chia sẻ trong [Tác tử ACP](/vi/tools/acp-agents#persistent-channel-bindings).
...
- Các cuộc gọi trực tiếp đi ra cung cấp `token` Discord tường minh sẽ dùng token đó cho cuộc gọi; cài đặt thử lại/chính sách của tài khoản vẫn lấy từ tài khoản được chọn trong ảnh chụp runtime đang hoạt động.
...
- Slug guild dùng chữ thường và thay khoảng trắng bằng `-`; khóa kênh dùng tên đã slug hóa (không có `#`). Ưu tiên ID guild.
...
- `channels.discord.mentionAliases` ánh xạ văn bản `@handle` gửi đi ổn định sang ID người dùng Discord trước khi gửi, để có thể nhắc đến đồng đội đã biết một cách xác định ngay cả khi bộ nhớ đệm thư mục tạm thời trống. Ghi đè theo từng tài khoản nằm dưới `channels.discord.accounts.<accountId>.mentionAliases`.
...
- `channels.discord.threadBindings` kiểm soát định tuyến gắn với luồng Discord:
  - `enabled`: ghi đè Discord cho các tính năng phiên gắn với luồng (`/focus`, `/unfocus`, `/agents`, `/session idle`, `/session max-age`, và gửi/định tuyến đã ràng buộc)
  - `idleHours`: ghi đè Discord cho tự động bỏ tập trung khi không hoạt động, tính bằng giờ (`0` để tắt)
  - `maxAgeHours`: ghi đè Discord cho tuổi tối đa cứng, tính bằng giờ (`0` để tắt)
  - `spawnSessions`: công tắc cho `sessions_spawn({ thread: true })` và tạo/ràng buộc luồng tự động khi ACP sinh luồng (mặc định: `true`)
...
- `channels.discord.voice` bật hội thoại kênh thoại Discord và các ghi đè tự động tham gia + LLM + TTS tùy chọn. Cấu hình Discord chỉ văn bản để tắt thoại theo mặc định; đặt `channels.discord.voice.enabled=true` để chọn bật.
- `channels.discord.voice.model` tùy chọn ghi đè mô hình LLM dùng cho phản hồi kênh thoại Discord.
- `channels.discord.voice.daveEncryption` và `channels.discord.voice.decryptionFailureTolerance` chuyển tiếp tới tùy chọn DAVE của `@discordjs/voice` (`true` và `24` theo mặc định).
...
- `channels.discord.voice.reconnectGraceMs` kiểm soát thời gian một phiên thoại đã ngắt kết nối được phép đi vào tín hiệu kết nối lại trước khi OpenClaw hủy phiên đó (`15000` theo mặc định).
- Phát lại thoại Discord không bị ngắt bởi sự kiện bắt đầu nói của người dùng khác. Để tránh vòng lặp phản hồi, OpenClaw bỏ qua thu thoại mới trong khi TTS đang phát.
- OpenClaw cũng cố gắng khôi phục nhận thoại bằng cách rời/tham gia lại phiên thoại sau các lỗi giải mã lặp lại.
- `channels.discord.streaming` là khóa chế độ luồng chuẩn. Discord mặc định là `streaming.mode: "progress"` để tiến độ công cụ/công việc xuất hiện trong một tin nhắn xem trước được chỉnh sửa; đặt `streaming.mode: "off"` để tắt. Các giá trị kế thừa `streamMode` và `streaming` boolean vẫn là bí danh runtime; chạy `openclaw doctor --fix` để ghi lại cấu hình đã lưu.
- `channels.discord.autoPresence` ánh xạ khả dụng runtime sang trạng thái hiện diện của bot (healthy => online, degraded => idle, exhausted => dnd) và cho phép ghi đè văn bản trạng thái tùy chọn.
...
- `channels.discord.execApprovals`: gửi phê duyệt exec gốc Discord và ủy quyền người phê duyệt.
  ...
  - `target`: nơi gửi lời nhắc phê duyệt. `"dm"` (mặc định) gửi tới DM của người phê duyệt, `"channel"` gửi tới kênh khởi nguồn, `"both"` gửi tới cả hai. Khi target bao gồm `"channel"`, nút chỉ dùng được bởi người phê duyệt đã phân giải.
...
- `socketMode` chuyển tiếp tinh chỉnh truyền tải Socket Mode của Slack SDK tới API bộ nhận Bolt công khai. Chỉ dùng khi điều tra thời gian chờ ping/pong hoặc hành vi websocket cũ.
- `botToken`, `appToken`, `signingSecret`, và `userToken` chấp nhận chuỗi văn bản thuần
  hoặc đối tượng SecretRef.
- Ảnh chụp tài khoản Slack hiển thị các trường nguồn/trạng thái theo từng thông tin xác thực như
...
- `channels.slack.streaming.mode` là khóa chế độ luồng Slack chuẩn. `channels.slack.streaming.nativeTransport` kiểm soát truyền tải phát luồng gốc của Slack. Các giá trị kế thừa `streamMode`, `streaming` boolean, và `nativeStreaming` vẫn là bí danh runtime; chạy `openclaw doctor --fix` để ghi lại cấu hình đã lưu.
- `unfurlLinks` và `unfurlMedia` chuyển tiếp boolean hiển thị bản xem trước liên kết và phương tiện của `chat.postMessage` Slack cho phản hồi bot. Bỏ qua chúng để giữ hành vi mặc định của Slack; đặt chúng tại `channels.slack.accounts.<accountId>` để ghi đè mặc định cấp cao nhất cho một tài khoản.
...
**Cô lập phiên luồng:** `thread.historyScope` là theo từng luồng (mặc định) hoặc dùng chung trên kênh. `thread.inheritParent` sao chép bản ghi hội thoại của kênh cha sang luồng mới.

- Phát luồng gốc Slack cộng với trạng thái luồng “đang nhập...” kiểu trợ lý Slack yêu cầu đích luồng trả lời. DM cấp cao nhất mặc định không nằm trong luồng, vì vậy chúng vẫn có thể phát luồng qua bản xem trước bài nháp đăng-và-sửa của Slack thay vì hiển thị bản xem trước luồng/trạng thái gốc kiểu luồng.
...
để xem dist-tag hiện tại trước khi ghim một phiên bản.
...
- Callback slash gốc được xác thực bằng token theo từng lệnh do Mattermost trả về trong quá trình đăng ký slash command. Nếu đăng ký thất bại hoặc không có lệnh nào được kích hoạt, OpenClaw từ chối callback với `Unauthorized: invalid command token.`
- Với máy chủ callback riêng tư/tailnet/nội bộ, Mattermost có thể yêu cầu `ServiceSettings.AllowedUntrustedInternalConnections` bao gồm máy chủ/tên miền callback. Dùng giá trị máy chủ/tên miền, không dùng URL đầy đủ.
...
- `channels.mattermost.groups.<channelId>.requireMention`: ghi đè cơ chế yêu cầu mention theo từng kênh (`"*"` cho mặc định).
...
- `channels.signal.configWrites`: cho phép hoặc từ chối việc ghi cấu hình do Signal khởi tạo.
- `channels.signal.defaultAccount` tùy chọn ghi đè lựa chọn tài khoản mặc định khi khớp với một id tài khoản đã cấu hình.

### iMessage

OpenClaw khởi chạy `imsg rpc` (JSON-RPC qua stdio). Không cần daemon hoặc cổng. Đây là đường dẫn ưu tiên cho các thiết lập iMessage OpenClaw mới khi máy chủ có thể cấp quyền đối với cơ sở dữ liệu Messages và Automation.
...
Nếu Gateway không chạy trên máy Mac đã đăng nhập Messages, hãy giữ `channels.imessage.enabled=true` và đặt `channels.imessage.cliPath` thành một wrapper SSH chạy `imsg "$@"` trên máy Mac đó. Đường dẫn `imsg` cục bộ mặc định chỉ dành cho macOS.
...
- Yêu cầu Full Disk Access tới cơ sở dữ liệu Messages.
- Ưu tiên đích `chat_id:<id>`. Dùng `imsg chats --limit 20` để liệt kê cuộc trò chuyện.
...
- SCP dùng kiểm tra khóa máy chủ nghiêm ngặt, nên hãy đảm bảo khóa máy chủ chuyển tiếp đã tồn tại trong `~/.ssh/known_hosts`.
...
- `channels.matrix.network.dangerouslyAllowPrivateNetwork` cho phép homeserver riêng tư/nội bộ. `proxy` và tùy chọn mạng này là các cơ chế kiểm soát độc lập.
...
- Các phép kiểm tra trạng thái Matrix và tra cứu thư mục trực tiếp dùng cùng chính sách proxy như lưu lượng runtime.
...
- Cấu hình kênh IRC đầy đủ (máy chủ/cổng/TLS/kênh/danh sách cho phép/cơ chế yêu cầu mention) được ghi lại trong [IRC](/vi/channels/irc).
...
- Nếu bạn thêm một tài khoản không mặc định bằng `openclaw channels add` (hoặc onboarding kênh) trong khi vẫn đang dùng cấu hình kênh cấp cao nhất một tài khoản, OpenClaw trước tiên sẽ nâng cấp các giá trị một tài khoản cấp cao nhất thuộc phạm vi tài khoản vào bản đồ tài khoản kênh để tài khoản ban đầu tiếp tục hoạt động. Hầu hết kênh chuyển chúng vào `channels.<channel>.accounts.default`; Matrix có thể giữ lại một đích được đặt tên/mặc định khớp sẵn có.
...
- `openclaw doctor --fix` cũng sửa các hình dạng hỗn hợp bằng cách chuyển các giá trị một tài khoản cấp cao nhất thuộc phạm vi tài khoản vào tài khoản đã nâng cấp được chọn cho kênh đó. Hầu hết kênh dùng `accounts.default`; Matrix có thể giữ lại một đích được đặt tên/mặc định khớp sẵn có.
...
### Cơ chế yêu cầu mention trong trò chuyện nhóm

Tin nhắn nhóm mặc định **yêu cầu mention** (mention metadata hoặc mẫu regex an toàn). Áp dụng cho WhatsApp, Telegram, Discord, Google Chat và cuộc trò chuyện nhóm iMessage.

Trả lời hiển thị được kiểm soát riêng. Phòng nhóm/kênh mặc định là `messages.groupChat.visibleReplies: "message_tool"`: OpenClaw vẫn xử lý lượt, nhưng các trả lời cuối thông thường vẫn ở chế độ riêng tư và đầu ra phòng hiển thị yêu cầu `message(action=send)`. Chỉ đặt `"automatic"` khi bạn muốn hành vi cũ, trong đó trả lời thông thường được đăng lại vào phòng. Để áp dụng cùng hành vi trả lời hiển thị chỉ qua công cụ cho cả trò chuyện trực tiếp, hãy đặt `messages.visibleReplies: "message_tool"`; harness Codex cũng dùng hành vi chỉ qua công cụ đó làm mặc định cho trò chuyện trực tiếp chưa đặt.

Trả lời hiển thị chỉ qua công cụ yêu cầu model/runtime gọi công cụ một cách đáng tin cậy. Nếu nhật ký phiên hiển thị văn bản assistant với `didSendViaMessagingTool: false`, model đã tạo câu trả lời cuối riêng tư thay vì gọi công cụ nhắn tin. Chuyển sang một model gọi công cụ mạnh hơn cho kênh đó, hoặc đặt `messages.groupChat.visibleReplies: "automatic"` để khôi phục trả lời cuối hiển thị theo hành vi cũ.

Nếu công cụ nhắn tin không khả dụng theo chính sách công cụ đang hoạt động, OpenClaw quay về trả lời hiển thị tự động thay vì âm thầm chặn phản hồi. `openclaw doctor` cảnh báo về điểm không khớp này.

Gateway tải lại nóng cấu hình `messages` sau khi tệp được lưu. Chỉ khởi động lại khi việc theo dõi tệp hoặc tải lại cấu hình bị tắt trong bản triển khai.

**Các kiểu nhắc đến:**

- **Nhắc đến bằng siêu dữ liệu**: Nhắc đến @ gốc của nền tảng. Bị bỏ qua trong chế độ tự trò chuyện của WhatsApp.
...
- Cổng kiểm soát nhắc đến chỉ được áp dụng khi có thể phát hiện (nhắc đến gốc hoặc ít nhất một mẫu).
...
`messages.visibleReplies` là mặc định toàn cục cho lượt nguồn; `messages.groupChat.visibleReplies` ghi đè giá trị đó cho lượt nguồn nhóm/kênh. Khi `messages.visibleReplies` chưa được đặt, một harness có thể cung cấp mặc định riêng cho trực tiếp/nguồn; harness Codex mặc định là `message_tool`. Danh sách cho phép của kênh và cổng kiểm soát nhắc đến vẫn quyết định một lượt có được xử lý hay không.
...
Đưa số của chính bạn vào `allowFrom` để bật chế độ tự trò chuyện (bỏ qua nhắc đến @ gốc, chỉ phản hồi các mẫu văn bản):
...
- Khối này cấu hình các bề mặt lệnh. Để xem danh mục lệnh tích hợp sẵn + đi kèm hiện tại, hãy xem [Lệnh gạch chéo](/vi/tools/slash-commands).
- Trang này là **tham chiếu khóa cấu hình**, không phải danh mục lệnh đầy đủ. Các lệnh do kênh/Plugin sở hữu như `/bot-ping` `/bot-help` `/bot-logs` của QQ Bot, `/card` của LINE, `/pair` để ghép nối thiết bị, `/dreaming` cho bộ nhớ, `/phone` để điều khiển điện thoại và `/voice` của Talk được ghi tài liệu trong các trang kênh/Plugin tương ứng cùng với [Lệnh gạch chéo](/vi/tools/slash-commands).
...
- `config: true` bật `/config` (đọc/ghi `openclaw.json`). Với các máy khách Gateway `chat.send`, thao tác ghi `/config set|unset` bền vững cũng yêu cầu `operator.admin`; `/config show` chỉ đọc vẫn khả dụng cho các máy khách toán tử thông thường có phạm vi ghi.
...
- Với các kênh nhiều tài khoản, `channels.<provider>.accounts.<id>.configWrites` cũng kiểm soát thao tác ghi nhắm đến tài khoản đó (ví dụ `/allowlist --config --account <id>` hoặc `/config set channels.<provider>.accounts.<id>...`).
- `restart: false` tắt `/restart` và các hành động của công cụ khởi động lại Gateway. Mặc định: `true`.
- `ownerAllowFrom` là danh sách cho phép chủ sở hữu tường minh dành cho các lệnh/công cụ chỉ chủ sở hữu. Nó tách biệt với `allowFrom`.
- `ownerDisplay: "hash"` băm mã định danh chủ sở hữu trong lời nhắc hệ thống. Đặt `ownerDisplaySecret` để kiểm soát việc băm.
...
- Bản đồ tài liệu lệnh:
  - danh mục tích hợp sẵn + đi kèm: [Lệnh gạch chéo](/vi/tools/slash-commands)
