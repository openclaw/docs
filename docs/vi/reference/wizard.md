...
read_when:
    ...
    - Tự động hóa quy trình bắt đầu sử dụng bằng chế độ không tương tác
    - Gỡ lỗi hành vi onboarding
sidebarTitle: Onboarding Reference
summary: 'Tài liệu tham khảo đầy đủ về quy trình thiết lập ban đầu qua CLI: mọi bước, cờ và trường cấu hình'
title: Tham khảo về quy trình thiết lập ban đầu
x-i18n:
    generated_at: "2026-07-19T06:02:11Z"
    model: gpt-5.6
...
Để xem tổng quan cấp cao, hãy xem [Onboarding (CLI)](/vi/start/wizard). Để biết hành vi và đầu ra
theo từng bước, hãy xem [Tài liệu tham khảo thiết lập CLI](/vi/start/wizard-cli-reference).

## Chi tiết luồng (chế độ cục bộ)

<Steps>
  <Step title="Đặt lại (không bắt buộc)">
    - `--reset` đặt lại trạng thái trước khi chạy thiết lập; nếu không có tùy chọn này, việc chạy lại quy trình onboarding
      sẽ giữ cấu hình hiện có và tái sử dụng cấu hình đó làm giá trị mặc định.
    - `--reset-scope` kiểm soát những gì `--reset` xóa: `config` (chỉ tệp cấu hình
      ), `config+creds+sessions` (mặc định), hoặc `full` (cũng xóa
      workspace).
    - Nếu tệp cấu hình không hợp lệ, quy trình onboarding sẽ dừng và yêu cầu bạn chạy
  ...
  <Step title="Xác nhận rủi ro">
    - Lần chạy đầu tiên (hoặc bất kỳ lần chạy nào trước khi `wizard.securityAcknowledgedAt` được đặt)
      yêu cầu bạn xác nhận rằng bạn hiểu các agent rất mạnh và việc cấp toàn quyền
      truy cập hệ thống tiềm ẩn rủi ro.
    - `--non-interactive` yêu cầu chỉ định rõ `--accept-risk`; nếu không có tùy chọn này,
      quy trình onboarding sẽ thoát với lỗi thay vì hiển thị lời nhắc.
    - Các lần chạy tương tác sẽ hiển thị lời nhắc xác nhận thay cho cờ này; nếu từ chối,
  ...
  <Step title="Mô hình/Xác thực">
    ...
    <Note>
    Mẹo cho máy không giao diện/máy chủ: hoàn tất OAuth trên máy có trình duyệt, sau đó sao chép
  ...
  <Step title="Gateway">
    ...
    - Ở chế độ token, thiết lập tương tác cung cấp:
      ...
      - Quickstart tái sử dụng các SecretRef `gateway.auth.token` hiện có trên các nhà cung cấp `env`, `file` và `exec` để thăm dò trong quy trình onboarding/bootstrap bảng điều khiển.
  ...
  <Step title="Kênh">
    - [WhatsApp](/vi/channels/whatsapp): đăng nhập QR không bắt buộc.
    ...
    - Discord, Feishu, Microsoft Teams, QQ Bot, Slack và các kênh khác được phân phối dưới dạng
      Plugin mà quy trình onboarding có thể cài đặt giúp bạn. Danh mục đầy đủ: [Kênh](/vi/channels).
  ...
  <Step title="Cài đặt daemon">
    ...
    - Nếu cả `gateway.auth.token` và `gateway.auth.password` đều được cấu hình nhưng `gateway.auth.mode` chưa được đặt, quá trình cài đặt daemon sẽ bị chặn cho đến khi chế độ được đặt rõ ràng.
...
<Note>
Nếu không phát hiện thấy GUI, quy trình thiết lập ban đầu sẽ in hướng dẫn chuyển tiếp cổng SSH cho Control UI thay vì mở trình duyệt.
Nếu thiếu các tài nguyên Control UI, quy trình thiết lập ban đầu sẽ thử xây dựng chúng; phương án dự phòng là `pnpm ui:build` (tự động cài đặt các phần phụ thuộc của UI).
</Note>
...
Thêm `--json` để nhận bản tóm tắt có thể đọc bằng máy.
...
Các ví dụ lệnh dành riêng cho nhà cung cấp có trong [Tự động hóa CLI](/vi/start/wizard-cli-automation#provider-specific-examples).
Sử dụng trang tham chiếu này để xem ngữ nghĩa của các cờ và thứ tự các bước.
...
Các ứng dụng khách (ứng dụng macOS, Control UI) có thể hiển thị các bước mà không cần triển khai lại logic thiết lập ban đầu.
...
và các tài liệu lưu trữ/hỗ trợ.

Một số kênh được cung cấp dưới dạng plugin. Khi bạn chọn một kênh trong quá trình thiết lập, quy trình thiết lập ban đầu
