---
read_when:
    - Tra cứu một bước onboarding hoặc cờ cụ thể
    - Tự động hóa quy trình giới thiệu bằng chế độ không tương tác
    - Gỡ lỗi hành vi onboarding
sidebarTitle: Onboarding Reference
summary: 'Tham chiếu đầy đủ cho quá trình onboarding bằng CLI: mọi bước, cờ và trường cấu hình'
title: Tham chiếu onboarding
x-i18n:
...
<Steps>
  <Step title="Phát hiện cấu hình hiện có">
    ...
    - Chạy lại onboarding **không** xóa bất kỳ thứ gì trừ khi bạn chọn rõ ràng **Đặt lại**
      (hoặc truyền `--reset`).
    - CLI `--reset` mặc định là `config+creds+sessions`; dùng `--reset-scope full`
      để xóa cả không gian làm việc.
  ...
  <Step title="Mô hình/Xác thực">
    ...
    - **Khóa API Anthropic**: lựa chọn trợ lý Anthropic ưu tiên trong onboarding/configure.
    - **Anthropic setup-token**: vẫn có trong onboarding/configure, mặc dù OpenClaw hiện ưu tiên tái sử dụng Claude CLI khi có sẵn.
    ...
    - **Gói đăng ký OpenAI Code (Codex) (ghép đôi thiết bị)**: luồng ghép đôi trình duyệt với mã thiết bị tồn tại trong thời gian ngắn.
      - Đặt `agents.defaults.model` thành `openai/gpt-5.5` thông qua runtime Codex khi mô hình chưa được đặt hoặc đã thuộc họ OpenAI.
    ...
    - **xAI (Grok) OAuth / khóa API**: đăng nhập bằng xAI OAuth khi được chọn, hoặc nhắc nhập `XAI_API_KEY` trên đường dẫn khóa API, và cấu hình xAI làm nhà cung cấp mô hình.
    ...
    - **MiniMax**: cấu hình được tự động ghi; mặc định hosted là `MiniMax-M3`.
    ...
    - Chọn mô hình mặc định từ các tùy chọn được phát hiện (hoặc nhập thủ công nhà cung cấp/mô hình). Để có chất lượng tốt nhất và giảm rủi ro prompt-injection, hãy chọn mô hình thế hệ mới nhất mạnh nhất có trong ngăn xếp nhà cung cấp của bạn.
    - Onboarding chạy kiểm tra mô hình và cảnh báo nếu mô hình đã cấu hình không xác định hoặc thiếu xác thực.
    - Chế độ lưu trữ khóa API mặc định là giá trị hồ sơ xác thực dạng văn bản thuần. Dùng `--secret-input-mode ref` để lưu các tham chiếu dựa trên biến môi trường thay thế (ví dụ `keyRef: { source: "env", provider: "default", id: "OPENAI_API_KEY" }`).
    ...
    <Note>
    Mẹo cho headless/máy chủ: hoàn tất OAuth trên một máy có trình duyệt, rồi sao chép
  ...
  <Step title="Không gian làm việc">
    ...
    - Khởi tạo các tệp không gian làm việc cần thiết cho nghi thức bootstrap của agent.
    - Bố cục đầy đủ của không gian làm việc + hướng dẫn sao lưu: [Không gian làm việc của agent](/vi/concepts/agent-workspace)
  ...
  <Step title="Gateway">
    ...
    - Trong chế độ token, thiết lập tương tác cung cấp:
      ...
      - Quickstart tái sử dụng các SecretRef `gateway.auth.token` hiện có trên các nhà cung cấp `env`, `file`, và `exec` cho probe onboarding/dashboard bootstrap.
      - Nếu SecretRef đó đã được cấu hình nhưng không thể phân giải, onboarding sẽ thất bại sớm với thông báo cách sửa rõ ràng thay vì âm thầm hạ cấp xác thực runtime.
  ...
  <Step title="Kênh">
    ...
    - [iMessage](/vi/channels/imessage): đường dẫn CLI `imsg` + quyền truy cập DB Messages; dùng một wrapper SSH khi Gateway chạy ngoài máy Mac.
  ...
  <Step title="Tìm kiếm web">
    ...
    - Các nhà cung cấp dựa trên API có thể dùng biến môi trường hoặc cấu hình hiện có để thiết lập nhanh; các nhà cung cấp không cần khóa sẽ dùng các điều kiện tiên quyết riêng của nhà cung cấp đó.
  ...
  <Step title="Cài đặt daemon">
    - macOS: LaunchAgent
      - Yêu cầu phiên người dùng đã đăng nhập; với headless, dùng LaunchDaemon tùy chỉnh (không được phân phối kèm).
    - Linux (và Windows qua WSL2): đơn vị người dùng systemd
      - Onboarding cố gắng bật lingering qua `loginctl enable-linger <user>` để Gateway vẫn hoạt động sau khi đăng xuất.
    ...
    - Nếu xác thực token yêu cầu token và `gateway.auth.token` được quản lý bằng SecretRef, cài đặt daemon sẽ xác thực nó nhưng không lưu các giá trị token văn bản thuần đã phân giải vào siêu dữ liệu môi trường dịch vụ supervisor.
    - Nếu xác thực token yêu cầu token và SecretRef token đã cấu hình chưa được phân giải, cài đặt daemon sẽ bị chặn với hướng dẫn có thể hành động.
  ...
  <Step title="Kiểm tra sức khỏe">
    ...
    - Mẹo: `openclaw status --deep` thêm probe sức khỏe gateway trực tiếp vào đầu ra trạng thái, bao gồm probe kênh khi được hỗ trợ (yêu cầu gateway có thể truy cập được).
...
Nếu thiếu tài nguyên Control UI, onboarding sẽ cố gắng build chúng; phương án dự phòng là `pnpm ui:build` (tự động cài đặt phụ thuộc UI).
</Note>
...
Thêm `--json` để có tóm tắt mà máy có thể đọc.

Gateway token SecretRef trong chế độ không tương tác:
...
Ví dụ lệnh theo từng nhà cung cấp nằm trong [Tự động hóa CLI](/vi/start/wizard-cli-automation#provider-specific-examples).
Dùng trang tham khảo này để biết ngữ nghĩa cờ và thứ tự các bước.
...
Gateway phơi bày luồng onboarding qua RPC (`wizard.start`, `wizard.next`, `wizard.cancel`, `wizard.status`).
...
Một số kênh được phân phối dưới dạng Plugin. Khi bạn chọn một kênh trong quá trình thiết lập, thiết lập ban đầu
sẽ nhắc cài đặt kênh đó (npm hoặc một đường dẫn cục bộ) trước khi có thể cấu hình.
