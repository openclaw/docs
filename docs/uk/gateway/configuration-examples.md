...
x-i18n:
    generated_at: "2026-04-29T10:51:12Z"
    model: gpt-5.5
    provider: openai
...
Нижче наведені приклади узгоджені з поточною схемою конфігурації. Повну довідку й примітки для кожного поля див. у [Конфігурації](/uk/gateway/configuration).
...
Залишайте сполучення пристроїв ручним, якщо ви не контролюєте мережевий шлях. Для спеціальної
лабораторії або підмережі tailnet можна увімкнути автоматичне схвалення
пристроїв вузлів під час першого сполучення з точними CIDR або IP-адресами:
...
оновлення публічного ключа все одно потребують ручного схвалення.

### Безпечний режим DM (спільна скринька / DM для кількох користувачів)
...
Для Discord/Slack/Google Chat/Microsoft Teams/Mattermost/IRC авторизація відправника типово спершу використовує ID.
Увімкніть пряме змінюване зіставлення за іменем/email/ніком через `dangerouslyAllowNameMatching: true` для кожного каналу лише тоді, коли ви явно приймаєте цей ризик.
...
- ID провайдерів відрізняються (номери телефонів, ID користувачів, ID каналів). Використовуйте документацію провайдера, щоб підтвердити формат.
- Необов’язкові розділи, які можна додати пізніше: `web`, `browser`, `ui`, `discovery`, `canvasHost`, `talk`, `signal`, `imessage`.
- Див. [Провайдери](/uk/providers) і [Усунення несправностей](/uk/gateway/troubleshooting), щоб отримати докладніші нотатки з налаштування.
