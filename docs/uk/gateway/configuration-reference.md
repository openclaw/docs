...
Охоплює основні поверхні конфігурації OpenClaw і надає посилання назовні, коли підсистема має власний глибший довідник. Каталоги команд і глибокі параметри пам’яті/QMD, що належать каналам і plugin, розміщено на окремих сторінках, а не на цій.
...
## Значення за замовчуванням агента, мультиагентність, сесії та повідомлення
...
Політика інструментів, експериментальні перемикачі, конфігурація інструментів на основі провайдерів і налаштування
користувацьких провайдерів / базових URL перенесено на окрему сторінку — див.
...
{
  mcp: {
    // Необов’язково. За замовчуванням: 600000 мс (10 хвилин). Установіть 0, щоб вимкнути витіснення під час простою.
...
- Зміни в `mcp.*` застосовуються гаряче шляхом знищення кешованих session MCP runtime.
...
- `entries.<skillKey>.apiKey`: спрощене поле для skills, які оголошують основну змінну середовища (plaintext string або об’єкт SecretRef).
...
- Виявлення приймає нативні OpenClaw plugins, а також сумісні пакети Codex і пакети Claude, включно з пакетами Claude стандартного макета без маніфесту.
...
- `plugins.entries.<id>.hooks.allowPromptInjection`: якщо `false`, ядро блокує `before_prompt_build` та ігнорує поля, що змінюють prompt, із застарілого `before_agent_start`, зберігаючи при цьому застарілі `modelOverride` і `providerOverride`. Застосовується до нативних plugin hooks і підтримуваних каталогів hooks, що надаються пакетами.
- `plugins.entries.<id>.hooks.allowConversationAccess`: якщо `true`, довірені plugins не з bundled можуть читати необроблений вміст розмови з типізованих hooks, таких як `llm_input`, `llm_output`, `before_agent_finalize` і `agent_end`.
- `plugins.entries.<id>.subagent.allowModelOverride`: явна довіра цьому plugin запитувати перевизначення `provider` і `model` для кожного запуску фонових підзапусків субагента.
- `plugins.entries.<id>.subagent.allowedModels`: необов’язковий список дозволу канонічних цілей `provider/model` для довірених перевизначень субагента. Використовуйте `"*"` лише якщо ви свідомо хочете дозволити будь-яку модель.
- `plugins.entries.<id>.config`: визначений plugin об’єкт конфігурації (валідується схемою нативного OpenClaw plugin, якщо вона доступна).
- Налаштування облікових записів/середовища виконання channel plugin розміщуються в `channels.<id>` і мають бути описані метаданими `channelConfigs` у маніфесті plugin-власника, а не центральним реєстром параметрів OpenClaw.
- `plugins.entries.firecrawl.config.webFetch`: налаштування провайдера веб-отримання Firecrawl.
  ...
  - `timeoutSeconds`: тайм-аут запиту збирання в секундах (за замовчуванням: `60`).
...
- `plugins.entries.memory-core.config.dreaming`: налаштування dreaming пам’яті. Див. [Dreaming](/uk/concepts/dreaming) щодо фаз і порогів.
  ...
  - `model`: необов’язкове перевизначення моделі субагента Dream Diary. Потребує `plugins.entries.memory-core.subagent.allowModelOverride: true`; поєднуйте з `allowedModels`, щоб обмежити цілі.
...
- Увімкнені Claude bundle plugins також можуть надавати вбудовані значення Pi за замовчуванням із `settings.json`; OpenClaw застосовує їх як санітизовані налаштування агента, а не як сирі патчі конфігурації OpenClaw.
...
- У строгому режимі кінцеві точки віддалених профілів CDP (`profiles.*.cdpUrl`) підлягають тому самому блокуванню приватної мережі під час перевірок доступності/виявлення.
...
- Віддалені профілі підтримують лише приєднання (start/stop/reset вимкнено).
...
- Якщо сервіс CDP, яким керує зовнішня система, доступний через loopback, установіть для цього
...
- Локальні керовані профілі використовують `browser.localLaunchTimeoutMs` для HTTP-виявлення Chrome CDP
  ...
  запускається успішно, але перевірки готовності випереджають завершення запуску. Обидва значення мають бути
...
- `assistant`: перевизначення ідентичності для Control UI. Як запасний варіант використовується ідентичність активного агента.
...
- **Примітка для Docker**: стандартний bind `loopback` слухає `127.0.0.1` усередині контейнера. Із мостовою мережею Docker (`-p 18789:18789`) трафік надходить через `eth0`, тому Gateway недоступний. Використовуйте `--network host` або встановіть `bind: "lan"` (або `bind: "custom"` з `customBindHost: "0.0.0.0"`), щоб слухати на всіх інтерфейсах.
- **Auth**: обов’язкова за замовчуванням. Bind не на loopback вимагає auth Gateway. На практиці це означає спільний token/password або reverse proxy з урахуванням ідентичності з `gateway.auth.mode: "trusted-proxy"`. Майстер онбордингу за замовчуванням генерує token.
- Якщо налаштовано і `gateway.auth.token`, і `gateway.auth.password` (включно з SecretRef), явно встановіть `gateway.auth.mode` у `token` або `password`. Запуск і потоки встановлення/відновлення сервісу завершуються помилкою, коли налаштовано обидва, а mode не встановлено.
...
- `gateway.auth.mode: "trusted-proxy"`: делегувати auth reverse proxy з урахуванням ідентичності та довіряти заголовкам ідентичності від `gateway.trustedProxies` (див. [Trusted Proxy Auth](/uk/gateway/trusted-proxy-auth)). Цей режим очікує джерело proxy **не на loopback**; reverse proxy loopback на тому самому хості не задовольняють auth trusted-proxy.
- `gateway.auth.allowTailscale`: коли `true`, заголовки ідентичності Tailscale Serve можуть задовольняти auth для Control UI/WebSocket (перевіряються через `tailscale whois`). Кінцеві точки HTTP API **не** використовують цю auth через заголовки Tailscale; вони дотримуються звичайного режиму HTTP auth Gateway. Цей потік без token припускає, що хост Gateway є довіреним. За замовчуванням `true`, коли `tailscale.mode = "serve"`.
- `gateway.auth.rateLimit`: необов’язковий лімітатор невдалих спроб auth. Застосовується для кожної IP-адреси клієнта і для кожної області auth (shared-secret і device-token відстежуються окремо). Заблоковані спроби повертають `429` + `Retry-After`.
  - В асинхронному шляху Tailscale Serve Control UI невдалі спроби для того самого `{scope, clientIp}` серіалізуються перед записом невдачі. Тому одночасні хибні спроби від одного клієнта можуть спрацювати на лімітатор уже на другому запиті, замість того щоб обидві «проскочили» як прості невідповідності.
  - `gateway.auth.rateLimit.exemptLoopback` за замовчуванням має значення `true`; установіть `false`, якщо ви свідомо хочете, щоб трафік localhost теж підлягав rate limiting (для тестових конфігурацій або суворих proxy-розгортань).
- Спроби WS auth із походження браузера завжди обмежуються без винятку для loopback (додатковий захист від browser-based brute force на localhost).
- На loopback ці блокування для браузерних походжень ізолюються за нормалізованим значенням `Origin`,
  ...
  інше походження.
- `tailscale.mode`: `serve` (лише tailnet, bind на loopback) або `funnel` (публічний, вимагає auth).
- `controlUi.allowedOrigins`: явний allowlist походжень браузера для підключень Gateway WebSocket. Обов’язковий, коли очікуються клієнти браузера з походжень не на loopback.
- `controlUi.dangerouslyAllowHostHeaderOriginFallback`: небезпечний режим, що вмикає резервну логіку походження через заголовок Host для розгортань, які навмисно покладаються на політику походження за заголовком Host.
...
- `OPENCLAW_ALLOW_INSECURE_PRIVATE_WS=1`: клієнтське
  аварійне перевизначення через змінну середовища процесу, яке дозволяє незашифрований `ws://` до довірених IP
  приватної мережі; за замовчуванням незашифрований трафік усе ще дозволено лише для loopback. Еквівалента в `openclaw.json`
...
- `gateway.push.apns.relay.baseUrl`: базовий HTTPS URL для зовнішнього ретранслятора APNs, який використовується офіційними/TestFlight збірками iOS після того, як вони публікують реєстрації на основі relay до Gateway. Цей URL має збігатися з URL relay, скомпільованим у збірку iOS.
- `gateway.push.apns.relay.timeoutMs`: тайм-аут надсилання від Gateway до relay у мілісекундах. За замовчуванням `10000`.
- Реєстрації на основі relay делегуються конкретній ідентичності Gateway. Спарений застосунок iOS отримує `gateway.identity.get`, включає цю ідентичність у реєстрацію relay і передає Gateway grant на надсилання в межах реєстрації. Інший Gateway не може повторно використати цю збережену реєстрацію.
...
- `gateway.channelHealthCheckMinutes`: інтервал монітора стану каналу в хвилинах. Установіть `0`, щоб глобально вимкнути перезапуски монітора стану. За замовчуванням: `5`.
...
- `channels.<provider>.healthMonitor.enabled`: відмова від перезапусків монітора стану на рівні каналу при збереженні глобального монітора ввімкненим.
- `channels.<provider>.accounts.<accountId>.healthMonitor.enabled`: перевизначення на рівні облікового запису для каналів із кількома обліковими записами. Якщо встановлено, воно має пріоритет над перевизначенням на рівні каналу.
- Локальні шляхи виклику Gateway можуть використовувати `gateway.remote.*` як резервний варіант лише коли `gateway.auth.*` не встановлено.
- Якщо `gateway.auth.token` / `gateway.auth.password` явно налаштовано через SecretRef і їх не вдалося розв’язати, розв’язання завершується із забороною за замовчуванням (без маскування резервним варіантом remote).
- `trustedProxies`: IP-адреси reverse proxy, які завершують TLS або вставляють заголовки forwarded-client. Указуйте лише proxy, якими ви керуєте. Записи loopback усе ще дійсні для конфігурацій proxy на тому самому хості/локального визначення (наприклад, Tailscale Serve або локальний reverse proxy), але вони **не** роблять запити loopback придатними для `gateway.auth.mode: "trusted-proxy"`.
- `allowRealIpFallback`: коли `true`, Gateway приймає `X-Real-IP`, якщо `X-Forwarded-For` відсутній. За замовчуванням `false` для поведінки із забороною за замовчуванням.
- `gateway.nodes.pairing.autoApproveCidrs`: необов’язковий allowlist CIDR/IP для автоматичного схвалення першого спарювання пристрою node без запитаних областей дії. Якщо не встановлено, вимкнено. Це не схвалює автоматично спарювання operator/browser/Control UI/WebChat і не схвалює автоматично оновлення ролі, області дії, метаданих або відкритого ключа.
...
- `gateway.tools.allow`: вилучає назви інструментів зі стандартного списку deny.
...
- Необов’язковий заголовок посиленого захисту response:
  - `gateway.http.securityHeaders.strictTransportSecurity` (встановлюйте лише для керованих вами HTTPS origins; див. [Trusted Proxy Auth](/uk/gateway/trusted-proxy-auth#tls-termination-and-hsts))
...
- `keyPath`: шлях файлової системи до файла приватного TLS key; обмежуйте права доступу.
...
- `deferralTimeoutMs`: необов’язковий максимальний час у мс, щоб чекати на операції в процесі перед примусовим перезапуском. Не вказуйте його або встановіть `0`, щоб чекати необмежено довго й періодично логувати попередження про те, що ще залишилося в очікуванні.
...
- Якщо mapping або preset використовує шаблонізований `sessionKey`, установіть `hooks.allowedSessionKeyPrefixes` і `hooks.allowRequestSessionKey=true`. Статичні ключі mapping цього явного дозволу не потребують.
...
- `POST /hooks/<name>` → розв’язується через `hooks.mappings`
  - Значення `sessionKey` у mapping, згенеровані шаблоном, вважаються зовнішньо наданими й також вимагають `hooks.allowRequestSessionKey=true`.
...
- `agentId` спрямовує до конкретного агента; невідомі ID повертаються до значення за замовчуванням.
...
- `allowRequestSessionKey`: дозволити викликам `/hooks/agent` і session key у mapping, керованих шаблонами, задавати `sessionKey` (за замовчуванням: `false`).
...
- `deliver: true` надсилає фінальну відповідь до каналу; `channel` за замовчуванням має значення `last`.
...
- Node WebView зазвичай не надсилають заголовки auth; після спарювання й підключення node Gateway рекламує URL можливостей із областю дії node для доступу до canvas/A2UI.
- URL можливостей прив’язані до активної WS-сесії node та швидко спливають. Резервний варіант на основі IP не використовується.
...
- `minimal` (за замовчуванням): не включає `cliPath` + `sshPort` до записів TXT.
- `full`: включає `cliPath` + `sshPort`.
- Ім’я хоста за замовчуванням використовує системне ім’я хоста, якщо воно є дійсною DNS-міткою, інакше використовується `openclaw`. Перевизначте через `OPENCLAW_MDNS_HOSTNAME`.
...
- `shellEnv`: імпортує відсутні очікувані ключі з профілю вашої shell входу.
...
Посилання на секрети є адитивними: plaintext значення й надалі працюють.
...
- `secrets apply` працює з підтримуваними шляхами облікових даних у `openclaw.json`.
- Посилання в `auth-profiles.json` включено до покриття під час розв’язання на етапі виконання та аудиту.
...
- Шляхи провайдерів file і exec завершуються із забороною за замовчуванням, коли перевірка Windows ACL недоступна. Установлюйте `allowInsecurePath: true` лише для довірених шляхів, які неможливо перевірити.
- Провайдер `exec` вимагає абсолютний шлях `command` і використовує корисні навантаження протоколу через stdin/stdout.
- За замовчуванням символьні посилання в шляхах command відхиляються. Установіть `allowSymlinkCommand: true`, щоб дозволити шляхи-символьні посилання з перевіркою розв’язаного цільового шляху.
...
- Статичні облікові дані під час виконання надходять із розв’язаних знімків у пам’яті; застарілі статичні записи `auth.json` очищуються при виявленні.
...
- `billingBackoffHours`: базовий backoff у годинах, коли профіль зазнає збою через справжні
  помилки billing/недостатнього кредиту (за замовчуванням: `5`). Явний текст про billing
...
- `billingBackoffHoursByProvider`: необов’язкові перевизначення backoff billing у годинах для кожного провайдера.
- `billingMaxHours`: верхня межа в годинах для експоненційного зростання backoff billing (за замовчуванням: `24`).
...
- `overloadedProfileRotations`: максимальна кількість ротацій auth-профілю в межах одного провайдера для помилок перевантаження перед переходом до резервної моделі (за замовчуванням: `1`). Сюди потрапляють форми «провайдер зайнятий», такі як `ModelNotReadyException`.
...
- `rateLimitedProfileRotations`: максимальна кількість ротацій auth-профілю в межах одного провайдера для помилок rate limit перед переходом до резервної моделі (за замовчуванням: `1`). Цей кошик rate limit включає текстові форми провайдера, такі як `Too many concurrent requests`, `ThrottlingException`, `concurrency limit reached`, `workers_ai ... quota limit exceeded` і `resource exhausted`.
...
- `maxFileBytes`: максимальний розмір активного файла журналу в байтах перед ротацією (додатне ціле число; за замовчуванням: `104857600` = 100 MB). OpenClaw зберігає до п’яти пронумерованих архівів поруч з активним файлом.
- `redactSensitive` / `redactPatterns`: маскування за принципом best-effort для виводу в консоль, файлів журналу, записів журналів OTLP і збереженого тексту транскриптів сесій.
...
- `enabled`: головний перемикач виводу інструментування (за замовчуванням: `true`).
...
- `otel.tracesEndpoint` / `otel.metricsEndpoint` / `otel.logsEndpoint`: необов’язкові кінцеві точки OTLP для конкретних сигналів. Якщо встановлено, вони перевизначають `otel.endpoint` лише для відповідного сигналу.
...
- `otel.captureContent`: явне ввімкнення захоплення сирого вмісту для атрибутів span OTEL. За замовчуванням вимкнено. Булеве значення `true` захоплює вміст повідомлень/інструментів, крім системного; форма об’єкта дає змогу явно ввімкнути `inputMessages`, `outputMessages`, `toolInputs`, `toolOutputs` і `systemPrompt`.
- `OTEL_SEMCONV_STABILITY_OPT_IN=gen_ai_latest_experimental`: перемикач середовища для найновіших експериментальних атрибутів провайдера span GenAI. За замовчуванням span зберігають застарілий атрибут `gen_ai.system` для сумісності; metrics GenAI використовують обмежені семантичні атрибути.
- `OPENCLAW_OTEL_PRELOADED=1`: перемикач середовища для хостів, які вже зареєстрували глобальний SDK OpenTelemetry. Тоді OpenClaw пропускає запуск/завершення SDK, що належить plugin, залишаючи діагностичні слухачі активними.
- `OTEL_EXPORTER_OTLP_TRACES_ENDPOINT`, `OTEL_EXPORTER_OTLP_METRICS_ENDPOINT` і `OTEL_EXPORTER_OTLP_LOGS_ENDPOINT`: змінні середовища для кінцевих точок конкретних сигналів, які використовуються, коли відповідний ключ конфігурації не встановлено.
...
- `auto.enabled`: увімкнути фонове автооновлення для пакетних інсталяцій (за замовчуванням: `false`).
- `auto.stableDelayHours`: мінімальна затримка в годинах перед автоматичним застосуванням stable-каналу (за замовчуванням: `6`; максимум: `168`).
- `auto.stableJitterHours`: додаткове вікно розподілу stable-каналу в годинах (за замовчуванням: `12`; максимум: `168`).
...
- `dispatch.enabled`: незалежний gate для dispatch ходу ACP-сесії (за замовчуванням: `true`). Установіть `false`, щоб залишити ACP-команди доступними, але заблокувати виконання.
- `backend`: id backend середовища виконання ACP за замовчуванням (має відповідати зареєстрованому ACP runtime plugin).
  Якщо встановлено `plugins.allow`, додайте id plugin backend (наприклад `acpx`), інакше bundled plugin за замовчуванням не завантажиться.
- `defaultAgent`: резервний id цільового ACP-агента, коли spawn не задають явну ціль.
- `allowedAgents`: allowlist id агентів, дозволених для ACP runtime sessions; порожнє значення означає відсутність додаткових обмежень.
...
- `stream.repeatSuppression`: пригнічувати повторювані рядки стану/інструмента в межах ходу (за замовчуванням: `true`).
...
- `runtime.ttlMinutes`: TTL простою в хвилинах для воркерів ACP-сесій до можливості очищення.
...
- `sessionRetention`: скільки часу зберігати завершені сесії ізольованих запусків cron перед очищенням із `sessions.json`. Також керує очищенням архівованих видалених транскриптів cron. За замовчуванням: `24h`; установіть `false`, щоб вимкнути.
...
- `webhookToken`: bearer token, що використовується для POST-доставки Cron Webhook (`delivery.mode = "webhook"`); якщо не вказано, заголовок auth не надсилається.
...
Застосовується лише до одноразових завдань cron. Для періодичних завдань використовується окрема обробка помилок.
...
- `includeSkipped`: зараховувати послідовні пропуски запусків до порога сповіщення (за замовчуванням: `false`). Пропущені запуски відстежуються окремо й не впливають на backoff для помилок виконання.
- `mode`: режим доставки — `"announce"` надсилає через повідомлення каналу; `"webhook"` надсилає POST до налаштованого Webhook.
- `accountId`: необов’язковий id облікового запису або каналу для визначення області доставки сповіщень.
...
- Ціль за замовчуванням для сповіщень про збої Cron для всіх завдань.
...
- Якщо не встановлено ні глобальну, ні індивідуальну ціль збоїв, завдання, які вже доставляють через `announce`, при збої використовують як резервний варіант цю основну ціль announce.
...
- Шляхи: розв’язуються відносно файла, що включає, але мають залишатися в межах каталогу конфігурації верхнього рівня (`dirname` від `openclaw.json`). Абсолютні форми/форми з `../` дозволені лише якщо після розв’язання вони все одно залишаються в межах цього кордону.
- Записи, якими керує OpenClaw і які змінюють лише один розділ верхнього рівня, підтриманий однофайловим include, записуються безпосередньо до цього включеного файла. Наприклад, `plugins install` оновлює `plugins: { $include: "./plugins.json5" }` у `plugins.json5` і залишає `openclaw.json` без змін.
- Кореневі включення, масиви include та включення з сусідніми перевизначеннями є лише для читання для записів, якими керує OpenClaw; такі записи завершуються із забороною за замовчуванням замість сплощення конфігурації.
