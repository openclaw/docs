...
summary: Надайте OpenAI-сумісний HTTP-ендпоінт /v1/chat/completions із Gateway
...
- довірена HTTP-автентифікація з передаванням ідентичності (`gateway.auth.mode="trusted-proxy"`):
  маршрутизуйте через налаштований identity-aware проксі й дозвольте йому вставити
...
- `gateway.auth.mode="token"` або `"password"` + `Authorization: Bearer ...`
  ...
  - відновлює повний стандартний набір операторських областей дії:
