...
<AccordionGroup>
  <Accordion title="Нотатки щодо вкладень">
    ...
    - Файли матеріалізуються в дочірньому робочому просторі в `.openclaw/attachments/<uuid>/` з `.manifest.json`.
...
<AccordionGroup>
  <Accordion title="Каталог верхнього рівня">
    ...
    - `models.providers`: мапа користувацьких провайдерів, ключована за ID провайдера.
      - Безпечні редагування: використовуйте `openclaw config set models.providers.<id> '<json>' --strict-json --merge` або `openclaw config set models.providers.<id>.models '<json-array>' --strict-json --merge` для додавальних оновлень. `config set` відмовляє в руйнівних замінах, якщо не передати `--replace`.
