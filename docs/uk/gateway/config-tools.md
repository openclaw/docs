...
<Note>
Локальний онбординг за замовчуванням задає для нових локальних конфігурацій `tools.profile: "coding"`, якщо значення не задано (наявні явні профілі зберігаються).
</Note>
...
<AccordionGroup>
  <Accordion title="Автентифікація та пріоритет об’єднання">
...
<AccordionGroup>
  ...
  <Accordion title="Локальні моделі (LM Studio)">
    Див. [Локальні моделі](/uk/gateway/local-models). TL;DR: запускайте велику локальну модель через LM Studio Responses API на серйозному обладнанні; залишайте розміщені моделі об’єднаними як резервний варіант.
  </Accordion>
...
- [Конфігурація — channels](/uk/gateway/config-channels)
