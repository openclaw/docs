---
redirect: /install/raspberry-pi
summary: Переспрямування на /install/raspberry-pi
title: Raspberry Pi (платформа)
x-i18n:
    generated_at: "2026-07-12T13:28:35Z"
    model: gpt-5.6
    postprocess_version: locale-links-v1
    provider: openai
    source_hash: 30119001c6b1ab6ba7524a51de08c1e93999c9ff6acea41a3a36609e300442c9
    source_path: platforms/raspberry-pi.md
    workflow: 16
---

Цю сторінку переміщено до розділу [Raspberry Pi](/uk/install/raspberry-pi).

## Пов’язані матеріали

- [Огляд встановлення](/uk/install)
- [Сервер Linux](/uk/vps)
- [Платформи](/uk/platforms)
