---
redirect: /install/digitalocean
summary: Переспрямування на /install/digitalocean
title: DigitalOcean (платформа)
x-i18n:
    generated_at: "2026-07-12T13:25:56Z"
    model: gpt-5.6
    postprocess_version: locale-links-v1
    provider: openai
    source_hash: d2b63c0f6d16f081a6610bf45b1e3a58e2e037d3562423287a53be9549d7cdd6
    source_path: platforms/digitalocean.md
    workflow: 16
---

Цю сторінку переміщено до розділу [DigitalOcean](/uk/install/digitalocean).

## Пов’язані матеріали

- [Огляд установлення](/uk/install)
- [Хостинг на VPS](/uk/vps)
