...
Feishu/Lark — це універсальна платформа для спільної роботи, де команди спілкуються, обмінюються документами, керують календарями та виконують роботу разом.

**Статус:** готово до використання у production для особистих повідомлень бота та групових чатів. WebSocket є режимом за замовчуванням; режим Webhook необов'язковий.
...
<Steps>
  ...
  ```
  Виберіть ручне налаштування, щоб вставити App ID і App Secret з Feishu Open Platform, або виберіть налаштування через QR, щоб автоматично створити бота. Якщо мобільний застосунок Feishu для внутрішнього ринку не реагує на QR-код, запустіть налаштування знову та виберіть ручне налаштування.
  </Step>
...
- Тільки широкомовні `@all` і `@_all` не вважаються згадками бота. Повідомлення, яке згадує і `@all`, і бота напряму, все одно рахується як згадка бота.
...
### Дозволити всі групи, але й далі вимагати @згадку
...
Відкрийте групу у Feishu/Lark, натисніть значок меню у верхньому правому куті та перейдіть до **Налаштувань**. ID групи (`chat_id`) указано на сторінці налаштувань.
...
Знайдіть `open_id` у виводі журналу. Також можна перевірити очікувані запити на сполучення:
...
3. У Feishu Open Platform створіть самостійно створений застосунок і скопіюйте його App ID та App Secret
...
глобальної конфігурації TTS, тому налаштування Feishu із кількома ботами можуть зберігати спільні
...
- `textChunkLimit` - розмір вихідного текстового фрагмента (за замовчуванням: `2000` символів)
...
Установіть `streaming: false`, щоб надсилати повну відповідь одним повідомленням. `blockStreaming` вимкнено за замовчуванням; вмикайте це лише тоді, коли хочете скидати завершені блоки асистента перед фінальною відповіддю.
...
`--thread here` працює для особистих повідомлень і повідомлень у гілках Feishu/Lark. Подальші повідомлення у прив'язаній розмові спрямовуються безпосередньо до цього сеансу ACP.
...
Див. [Отримати ID групи/користувача](#get-groupuser-ids), щоб дізнатися поради з пошуку.

---

## Ізоляція агента для кожного користувача (динамічне створення агентів)

Увімкніть `dynamicAgentCreation`, щоб автоматично створювати **ізольовані екземпляри агентів** для кожного користувача особистих повідомлень. Кожен користувач отримує власні:
...
Це важливо для публічних ботів, коли ви хочете, щоб кожен користувач мав власний приватний досвід роботи з ШІ-асистентом.

<Note>
Динамічні прив'язування включають нормалізований Feishu `accountId`, тому облікові записи за замовчуванням і іменовані облікові записи спрямовують кожного відправника до правильного динамічного агента.

Якщо іменований обліковий запис створив динамічного агента без області видимості у старішому випуску, цей застарілий агент усе ще враховується в `maxAgents`. Перш ніж видаляти його, підтвердьте, що він не використовується обліковим записом за замовчуванням, або тимчасово збільште `maxAgents`; OpenClaw не може безпечно визначити, якому обліковому запису належить неоднозначний застарілий стан.
</Note>
...
Коли новий користувач надсилає своє перше особисте повідомлення:
...
3. Реєструє агента та створює прив'язування для цього користувача
4. Допоміжний засіб робочого простору забезпечує bootstrap-файли (`AGENTS.md`, `SOUL.md`, `USER.md` тощо) під час першого доступу
5. Спрямовує всі майбутні повідомлення від цього користувача до його виділеного агента
...
`session.dmScope` керує тим, як особисті повідомлення зіставляються із сеансами агента. Це **глобальне налаштування**, яке впливає на всі канали.

| Значення                    | Поведінка                                                           | Найкраще для                                                     |
| --------------------------- | ------------------------------------------------------------------- | ---------------------------------------------------------------- |
| `"main"`                    | DM кожного користувача зіставляється з основним сеансом його агента | Ботів для одного користувача, де потрібно автоматично завантажувати `USER.md` / `SOUL.md` |
| `"per-channel-peer"`        | Кожна комбінація (канал + користувач) отримує окремий сеанс         | Публічних багатокористувацьких ботів, яким потрібна сильніша ізоляція |
| `"per-account-channel-peer"` | Кожна комбінація (обліковий запис + канал + користувач) отримує окремий сеанс | Багатооблікових ботів, яким потрібна ізоляція сеансів на рівні облікового запису |

**Компроміс**: Використання `"main"` вмикає автоматичне завантаження bootstrap-файлів (`USER.md`, `SOUL.md`, `MEMORY.md`), але означає, що всі DM в усіх каналах спільно використовують один шаблон ключів сеансу. Для публічних багатокористувацьких ботів, де ізоляція важливіша за автоматичне bootstrap-завантаження, розгляньте `"per-channel-peer"` і керуйте bootstrap-файлами вручну.
...
Список усіх створених робочих просторів:
...
- **Межа безпеки**: Це механізм ізоляції контексту повідомлень, а не межа безпеки для ворожих співорендарів. Процес агента та середовище хоста є спільними.
...
| `channels.feishu.groups.<chat_id>.requireMention`        | Перевизначення @згадки для групи; явні ID також допускають групу в режимі списку дозволених | успадковується                      |
...
| `channels.feishu.streaming`                              | Потоковий вивід картки                                                         | `true`                              |
| `channels.feishu.blockStreaming`                         | Потокова відповідь завершеного блока                                           | `false`                             |
...
| `channels.feishu.tools.base`                             | Псевдонім для `channels.feishu.tools.bitable`; явний `bitable` має пріоритет, коли встановлено обидва | `true`                              |
| `channels.feishu.accounts.<id>.tools.bitable`            | Шлюз інструментів Bitable/Base для облікового запису                           | успадковується                      |
| `channels.feishu.accounts.<id>.tools.base`               | Псевдонім для облікового запису для `tools.bitable`                            | успадковується                      |
...
сирого JSON `file_key`. Коли налаштовано `tools.media.audio`, OpenClaw
завантажує ресурс голосової нотатки та запускає спільну транскрипцію аудіо перед
ходом агента, тож агент отримує розшифрований усний текст. Якщо Feishu включає
текст транскрипту безпосередньо в аудіонавантаження, цей текст використовується без ще одного
виклику ASR. Без провайдера транскрипції аудіо агент усе одно отримує
плейсхолдер `<media:audio>` разом зі збереженим вкладенням, а не сире навантаження
ресурсу Feishu.
...
- ⚠️ Форматований текст (форматування в стилі допису; не підтримує повні можливості створення контенту Feishu/Lark)

Нативні аудіобульбашки Feishu/Lark використовують тип повідомлення Feishu `audio` і потребують
завантаження медіа Ogg/Opus (`file_type: "opus"`). Наявні медіафайли `.opus` і `.ogg`
надсилаються безпосередньо як нативне аудіо. MP3/WAV/M4A та інші ймовірні аудіоформати
перекодовуються у 48 кГц Ogg/Opus за допомогою `ffmpeg` лише тоді, коли відповідь запитує голосову
доставку (`audioAsVoice` / інструмент повідомлень `asVoice`, зокрема відповіді голосових нотаток TTS).
Звичайні вкладення MP3 залишаються звичайними файлами. Якщо `ffmpeg` відсутній або
конвертація завершується невдало, OpenClaw повертається до файлового вкладення та записує причину в журнал.

### Потоки й відповіді

- ✅ Вбудовані відповіді
- ✅ Відповіді в потоці
- ✅ Медіавідповіді залишаються прив’язаними до потоку під час відповіді на повідомлення потоку
...
ключ тематичної сесії. Якщо нативна подія початку теми не містить `thread_id`, OpenClaw
отримує його з Feishu перед маршрутизацією ходу. Звичайні групові відповіді, які
OpenClaw перетворює на потоки, і далі використовують ID кореневого повідомлення відповіді (`om_*`), щоб
перший хід і наступний хід залишалися в одній сесії.
