...
Використовуйте пакет без версії, щоб відстежувати поточний офіційний release tag. Закріплюйте точну версію лише тоді, коли потрібне відтворюване встановлення.
...
Установіть і автентифікуйте devtunnel CLI, якщо ще цього не зробили ([посібник для початку роботи](https://learn.microsoft.com/en-us/azure/developer/dev-tunnels/get-started)).
...
- Реєструє бота (за замовчуванням керовано Teams - підписка Azure не потрібна)

Вивід покаже `CLIENT_ID`, `CLIENT_SECRET`, `TENANT_ID` і **Teams App ID** - занотуйте їх для наступних кроків. Також буде запропоновано встановити застосунок у Teams напряму.
...
Або використайте змінні середовища напряму: `MSTEAMS_APP_ID`, `MSTEAMS_APP_PASSWORD`, `MSTEAMS_TENANT_ID`.
...
<Note>
Групові чати за замовчуванням заблоковані (`channels.msteams.groupPolicy: "allowlist"`). Щоб дозволити групові відповіді, задайте `channels.msteams.groupAllowFrom` або використайте `groupPolicy: "open"`, щоб дозволити будь-якого учасника (з обмеженням за згадкою).
</Note>
...
- За замовчуванням використовувати безпечну поведінку каналів (згадки потрібні, якщо не налаштовано інакше).

## Записи конфігурації

За замовчуванням Microsoft Teams дозволено записувати оновлення конфігурації, ініційовані `/config set|unset` (потребує `commands.config: true`).
...
- За замовчуванням: `channels.msteams.dmPolicy = "pairing"`. Невідомі відправники ігноруються, доки їх не схвалять.
- `channels.msteams.allowFrom` має використовувати стабільні object ID AAD.
- Не покладайтеся на зіставлення UPN/display-name для allowlists - вони можуть змінюватися. OpenClaw за замовчуванням вимикає пряме зіставлення імен; явно увімкніть його через `channels.msteams.dangerouslyAllowNameMatching: true`.
- Майстер може зіставляти імена з ID через Microsoft Graph, якщо облікові дані це дозволяють.

**Доступ груп**

- За замовчуванням: `channels.msteams.groupPolicy = "allowlist"` (заблоковано, якщо не додати `groupAllowFrom`). Використовуйте `channels.defaults.groupPolicy`, щоб перевизначити значення за замовчуванням, коли воно не задане.
- `channels.msteams.groupAllowFrom` керує тим, які відправники можуть запускати обробку в групових чатах/каналах (відступає до `channels.msteams.allowFrom`).
- Задайте `groupPolicy: "open"`, щоб дозволити будь-якого учасника (за замовчуванням усе одно з обмеженням за згадкою).
- Щоб не дозволяти **жодні канали**, задайте `channels.msteams.groupPolicy: "disabled"`.
...
**Allowlist Teams + каналів**

- Обмежуйте групові/канальні відповіді, перелічуючи teams і канали в `channels.msteams.teams`.
- Ключі мають використовувати стабільні Teams conversation IDs із посилань Teams, а не змінювані display names.
- Коли `groupPolicy="allowlist"` і присутній allowlist teams, приймаються лише перелічені teams/канали (з обмеженням за згадкою).
- Майстер налаштування приймає записи `Team/Channel` і зберігає їх для вас.
- Під час запуску OpenClaw зіставляє назви team/channel і користувачів в allowlist з ID (коли дозволи Graph це дозволяють)
  і логує зіставлення; нерозв’язані назви team/channel зберігаються як введені, але за замовчуванням ігноруються для маршрутизації, якщо не ввімкнено `channels.msteams.dangerouslyAllowNameMatching: true`.
...
3. Зберіть **пакет застосунку Teams**, який посилається на бота та включає дозволи RSC нижче.
...
6. Gateway за замовчуванням слухає Bot Framework webhook-трафік на `/api/messages`.
...
Використовуйте Azure Managed Identity для безпарольної автентифікації. Це ідеально для розгортань в інфраструктурі Azure (AKS, App Service, Azure VMs), де доступна managed identity.
...
- Federated identity credential створено в Entra ID app registration
...
Для розгортань AKS, які використовують ідентичність робочого навантаження:
...
| **Сертифікат**         | `authType: "federated"` + `certificatePath`   | Немає спільного секрету в мережі           | Додаткове керування сертифікатами                |
...
Teams не може звертатися до `localhost`. Використовуйте постійний тунель для розробки, щоб ваша URL-адреса залишалася незмінною між сеансами:
...
OpenClaw надає дію `member-info` на основі Graph для Microsoft Teams, щоб агенти й автоматизації могли отримувати відомості про учасників каналу (відображуване ім’я, електронна пошта, роль) безпосередньо з Microsoft Graph.
...
Дію обмежено параметром `channels.msteams.actions.memberInfo` (за замовчуванням: увімкнено, коли доступні облікові дані Graph).
...
- Контекст цитованого вкладення (`ReplyTo*`, отриманий з HTML-відповіді Teams) наразі передається як отриманий.
- Іншими словами, списки дозволених визначають, хто може запускати агента; сьогодні фільтруються лише окремі додаткові шляхи контексту.
...
Мінімальний коректний приклад з обов’язковими полями. Замініть ідентифікатори й URL-адреси.
...
- `authorization.permissions.resourceSpecific` має містити читання/надсилання для каналу, якщо вам потрібен трафік каналу.
...
- **Вміст зображень або файлів** у каналах/групах (корисне навантаження містить лише HTML-заглушку).
- Завантаження вкладень, збережених у SharePoint/OneDrive.
- Читання історії повідомлень (поза подією живого Webhook).
...
## Медіа й історія з Graph (потрібно для каналів)
...
**Додатковий дозвіл для згадок користувачів:** Згадки користувачів через @mention працюють одразу для користувачів у розмові. Однак, якщо ви хочете динамічно шукати та згадувати користувачів, яких **немає в поточній розмові**, додайте дозвіл `User.Read.All` (Application) і надайте згоду адміністратора.
...
- `channels.msteams.allowFrom`: список дозволених DM (рекомендовано AAD object IDs). Майстер розпізнає імена в ID під час налаштування, коли доступ до Graph доступний.
- `channels.msteams.dangerouslyAllowNameMatching`: аварійний перемикач для повторного ввімкнення зіставлення за змінними UPN/відображуваними іменами та прямої маршрутизації за назвою команди/каналу.
...
- `channels.msteams.mediaAuthAllowHosts`: список дозволених хостів для додавання заголовків Authorization під час повторних спроб завантаження медіа (за замовчуванням хости Graph + Bot Framework).
...
- `channels.msteams.teams.<teamId>.tools`: типові перевизначення політики інструментів для окремої команди (`allow`/`deny`/`alsoAllow`), які використовуються, коли перевизначення каналу відсутнє.
...
- Ключі `toolsBySender` мають використовувати явні префікси:
  `id:`, `e164:`, `username:`, `name:` (застарілі ключі без префікса й далі зіставляються лише з `id:`).
- `channels.msteams.actions.memberInfo`: увімкнути або вимкнути дію отримання інформації про учасника на основі Graph (за замовчуванням увімкнено, коли облікові дані Graph доступні).
...
- `channels.msteams.useManagedIdentity`: увімкнути автентифікацію managed identity (federated mode).
- `channels.msteams.managedIdentityClientId`: client ID для user-assigned managed identity.
- `channels.msteams.sharePointSiteId`: SharePoint site ID для завантаження файлів у групові чати/канали (див. [Надсилання файлів у групових чатах](#sending-files-in-group-chats)).
...
| ------------------------ | --------------------------------------------------------- | --------------------------- |
| **Дописи** (класичний)   | Повідомлення відображаються як картки з відповідями в потоках під ними | `thread` (за замовчуванням) |
| **Потоки** (як у Slack)  | Повідомлення йдуть лінійно, більше схоже на Slack         | `top-level`                 |

**Проблема:** Teams API не надає інформації про те, який стиль інтерфейсу використовує канал. Якщо використати неправильний `replyStyle`:

- `thread` у каналі зі стилем Threads → відповіді виглядають незручно вкладеними
- `top-level` у каналі зі стилем Posts → відповіді з’являються як окремі дописи верхнього рівня замість відповідей у потоці
...
Коли бот надсилає відповідь у канал, `replyStyle` визначається від найконкретнішого перевизначення до значення за замовчуванням. Перше значення, що не є `undefined`, перемагає:
...
4. **Неявне значення за замовчуванням** — виводиться з `requireMention`:
   - `requireMention: true` → `thread`
   - `requireMention: false` → `top-level`

Якщо ви задаєте `requireMention: false` глобально без явного `replyStyle`, згадки в каналах зі стилем Posts відображатимуться як дописи верхнього рівня, навіть коли вхідне повідомлення було відповіддю в потоці. Закріпіть `replyStyle: "thread"` на глобальному рівні, рівні команди або каналу, щоб уникнути несподіванок.

### Збереження контексту потоку

Коли діє `replyStyle: "thread"` і бота @mentioned зсередини потоку каналу, OpenClaw повторно приєднує корінь початкового потоку до вихідного посилання на розмову (`19:…@thread.tacv2;messageid=<root>`), щоб відповідь потрапила в той самий потік. Це працює як для живих надсилань (у межах поточного turn), так і для проактивних надсилань після завершення терміну дії контексту turn Bot Framework (наприклад, довготривалі агенти, відповіді на виклики інструментів у черзі через `mcp__openclaw__message`).

Корінь потоку береться зі збереженого `threadId` у посиланні на розмову. Старіші збережені посилання, що передують `threadId`, повертаються до `activityId` (будь-якої вхідної activity, яка останньою ініціалізувала розмову), тому наявні розгортання продовжують працювати без повторної ініціалізації.

Коли діє `replyStyle: "top-level"`, на вхідні повідомлення з потоків каналів навмисно відповідають новими дописами верхнього рівня — суфікс потоку не додається. Це правильна поведінка для каналів зі стилем Threads; якщо ви бачите дописи верхнього рівня там, де очікували відповіді в потоках, ваш `replyStyle` налаштовано неправильно для цього каналу.

## Вкладення та зображення

**Поточні обмеження:**

- **DMs:** Зображення та вкладення файлів працюють через файлові API бота Teams.
- **Канали/групи:** Вкладення зберігаються в M365 storage (SharePoint/OneDrive). Корисне навантаження Webhook містить лише HTML-заглушку, а не фактичні байти файлу. **Для завантаження вкладень із каналів потрібні дозволи Graph API**.
...
Без дозволів Graph повідомлення каналів із зображеннями будуть отримані лише як текст (вміст зображення недоступний для бота).
За замовчуванням OpenClaw завантажує медіа лише з hostnames Microsoft/Teams. Перевизначте через `channels.msteams.mediaAllowHosts` (використовуйте `["*"]`, щоб дозволити будь-який хост).
Заголовки Authorization додаються лише для хостів у `channels.msteams.mediaAuthAllowHosts` (за замовчуванням хости Graph + Bot Framework). Тримайте цей список суворим (уникайте multi-tenant suffixes).
...
| **Зображення (будь-який контекст)** | Base64-encoded inline                         | Працює одразу                                  |

### Чому групові чати потребують SharePoint

Боти не мають особистого диска OneDrive (endpoint Graph API `/me/drive` не працює для application identities). Щоб надсилати файли в групових чатах/каналах, бот завантажує їх на **SharePoint site** і створює посилання для спільного доступу.
...
Спільний доступ на рівні користувачів безпечніший, оскільки доступ до файлу мають лише учасники чату. Якщо дозвіл `Chat.Read.All` відсутній, бот повертається до спільного доступу в межах організації.
...
| Будь-який контекст + зображення                  | Base64-encoded inline (працює без SharePoint)       |

### Місце зберігання файлів

Завантажені файли зберігаються в папці `/OpenClawShared/` у стандартній бібліотеці документів налаштованого SharePoint site.
...
- Опитування ще не публікують автоматично підсумки результатів (за потреби перегляньте файл сховища).

## Картки презентацій

Надсилайте семантичні presentation-навантаження користувачам Teams або в розмови за допомогою інструмента `message` чи CLI. OpenClaw відтворює їх як адаптивні картки Teams із загального контракту presentation.

Параметр `presentation` приймає семантичні блоки. Коли задано `presentation`, текст повідомлення необов’язковий.
...
- Проактивні повідомлення можливі лише **після того**, як користувач взаємодіяв, бо в цей момент ми зберігаємо посилання на розмову.
- Див. `/gateway/configuration` щодо `dmPolicy` і керування через allowlist.
...
- Ключ команди = сегмент шляху після `/team/` (URL-декодований, наприклад `19:Bk4j...@thread.tacv2`; старіші тенанти можуть показувати `@thread.skype`, що також є допустимим)
...
- **Немає відповідей у каналі:** за замовчуванням потрібні згадки; задайте `channels.msteams.requireMention=false` або налаштуйте окремо для команди/каналу.
- **Невідповідність версій (Teams досі показує старий manifest):** видаліть і повторно додайте застосунок, а також повністю закрийте Teams, щоб оновити дані.
- **401 Unauthorized від webhook:** очікувано під час ручного тестування без Azure JWT - означає, що endpoint доступний, але автентифікація не вдалася. Використовуйте Azure Web Chat для коректного тестування.

### Помилки завантаження manifest

- **"Icon file cannot be empty":** manifest посилається на файли піктограм розміром 0 байтів. Створіть дійсні PNG-піктограми (32x32 для `outline.png`, 192x192 для `color.png`).
- **"webApplicationInfo.Id already in use":** застосунок досі встановлено в іншій команді/чаті. Спершу знайдіть і видаліть його або зачекайте 5-10 хвилин на поширення змін.
