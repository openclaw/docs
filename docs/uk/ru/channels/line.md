...
повідомлення, template повідомлення та швидкі відповіді. Реакції та треди
не підтримуються.
...
вхідні події (POST) одразу після перевірки підпису та корисного навантаження; обробка
агентом продовжується асинхронно.
...
- Перевірка підпису LINE залежить від тіла запиту (HMAC за необробленим тілом), тому OpenClaw застосовує суворі обмеження розміру тіла та тайм-аут до автентифікації перед перевіркою.
- OpenClaw обробляє події Webhook з перевірених необроблених байтів запиту. Значення `req.body`, перетворені проміжним ПЗ вище за ланцюжком, ігноруються для збереження цілісності підпису.
...
Особисті повідомлення за замовчуванням потребують сполучення. Невідомі відправники отримують код сполучення, а їхні
...
- `channels.line.allowFrom`: дозволені ID користувачів LINE для особистих повідомлень; `dmPolicy: "open"` потребує `["*"]`
...
- На статичні групи доступу відправників можна посилатися з `allowFrom`, `groupAllowFrom` і групового `allowFrom` через `accessGroup:<name>`.
- Примітка про середовище виконання: якщо `channels.line` повністю відсутній, середовище виконання повертається до `groupPolicy="allowlist"` для перевірок груп (навіть якщо задано `channels.defaults.groupPolicy`).
...
Plugin LINE підтримує надсилання зображень, відео та аудіофайлів через інструмент повідомлень агента. Медіа надсилається через специфічний для LINE шлях доставки з відповідною обробкою попереднього перегляду та відстеження:
