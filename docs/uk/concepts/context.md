...
summary: 'Контекст: що бачить модель, як він формується та як його інспектувати'
title: Контекст
x-i18n:
    generated_at: "2026-05-06T01:42:04Z"
    model: gpt-5.5
    provider: openai
...
Контекст — це _не те саме_, що "памʼять": памʼять може зберігатися на диску й повторно завантажуватися пізніше; контекст — це те, що міститься в поточному вікні моделі.
...
Повний розподіл: [Системний промпт](/uk/concepts/system-prompt).
...
Інструкції Skills _не_ включаються за замовчуванням. Очікується, що модель `read` файл `SKILL.md` Skills **лише тоді, коли потрібно**.
...
- **Обрізання** видаляє старі результати інструментів із промпта _в памʼяті_, щоб звільнити місце у вікні контексту, але не переписує транскрипт сесії - повна історія все ще доступна для перегляду на диску.
...
[Рушій контексту](/uk/concepts/context-engine) для повного
...
<CardGroup cols={2}>
  <Card title="Рушій контексту" href="/uk/concepts/context-engine" icon="puzzle-piece">
