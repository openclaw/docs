...
x-i18n:
    generated_at: "2026-05-06T01:09:40Z"
    model: gpt-5.5
    provider: openai
...
---
  ...
  Запитання й відповіді щодо моделей і профілів автентифікації. Про налаштування, сесії, Gateway, канали та
  ...
  <Accordion title='Що таке "типова модель"?'>
    ...
    Моделі зазначаються як `provider/model` (приклад: `openai/gpt-5.5` або `openai-codex/gpt-5.5`). Якщо ви опускаєте провайдера, OpenClaw спершу пробує псевдонім, потім унікальний збіг налаштованого провайдера для цього точного ідентифікатора моделі, і лише після цього повертається до налаштованого типового провайдера як застарілого шляху сумісності. Якщо цей провайдер більше не надає налаштовану типову модель, OpenClaw повертається до першої налаштованої пари провайдер/модель замість того, щоб показувати застаріле типове значення видаленого провайдера. Вам усе одно слід **явно** задавати `provider/model`.
  ...
  <Accordion title="Яку модель ви рекомендуєте?">
    ...
    **Для агентів із підтримкою інструментів або недовіреним вводом:** надавайте перевагу силі моделі, а не вартості.
    **Для звичайного чату з низькими ризиками:** використовуйте дешевші резервні моделі та маршрутизуйте за роллю агента.
    ...
    Важливе попередження: слабші або надмірно квантовані моделі вразливіші до ін’єкцій у prompt
  ...
  <Accordion title="Як перемикати моделі, не стираючи конфігурацію?">
    ...
    Для редагувань через RPC спершу перегляньте за допомогою `config.schema.lookup` і віддавайте перевагу `config.patch`. Payload lookup дає нормалізований шлях, стислу документацію/обмеження схеми та підсумки безпосередніх дочірніх елементів.
  ...
  <Accordion title="Чи можу я використовувати самостійно розміщені моделі (llama.cpp, vLLM, Ollama)?">
    ...
    Примітка щодо безпеки: менші або сильно квантовані моделі вразливіші до ін’єкцій у prompt.
    Ми наполегливо рекомендуємо **великі моделі** для будь-якого бота, який може використовувати інструменти.
    Якщо ви все ж хочете малі моделі, увімкніть sandboxing і суворі allowlists інструментів.
  ...
  <Accordion title="Які моделі використовують OpenClaw, Flawd і Krill?">
    ...
    - Для агентів, чутливих до безпеки або з підтримкою інструментів, використовуйте найсильнішу модель останнього покоління з доступних.
  ...
  <Accordion title="Як перемикати моделі на льоту (без перезапуску)?">
    ...
    Також можна примусово вибрати певний профіль автентифікації для провайдера (для окремої сесії):
  ...
  <Accordion title="Чи можу я використовувати GPT 5.5 для щоденних завдань, а Codex 5.5 для програмування?">
    ...
    - **Нативний агент програмування Codex:** встановіть `agents.defaults.model.primary` на `openai/gpt-5.5`, а `agents.defaults.agentRuntime.id` на `"codex"`. Увійдіть за допомогою `openclaw models auth login --provider openai-codex`, коли хочете використовувати автентифікацію підписки ChatGPT/Codex.
    ...
    - **Субагенти:** маршрутизуйте завдання програмування до агента лише для Codex із власною моделлю та типовим `agentRuntime`.
  ...
  <Accordion title="Як налаштувати швидкий режим для GPT 5.5?">
    Використовуйте або перемикач сесії, або типове значення конфігурації:

    - **Для окремої сесії:** надішліть `/fast on`, коли сесія використовує `openai/gpt-5.5` або `openai-codex/gpt-5.5`.
    - **Типове значення для моделі:** встановіть `agents.defaults.models["openai/gpt-5.5"].params.fastMode` або `agents.defaults.models["openai-codex/gpt-5.5"].params.fastMode` на `true`.
    ...
    Для OpenAI швидкий режим відповідає `service_tier = "priority"` у підтримуваних нативних запитах Responses. Сесійні перевизначення `/fast` мають пріоритет над типовими значеннями конфігурації.
  ...
  <Accordion title='Чому я бачу "Unknown model: minimax/MiniMax-M2.7"?'>
    Це означає, що **провайдер не налаштований** (не знайдено конфігурації провайдера MiniMax або профілю автентифікації), тому модель не може бути resolved.
    ...
    2. Переконайтеся, що MiniMax налаштовано (майстер або JSON), або що автентифікація MiniMax
       існує в env/профілях автентифікації, щоб відповідний провайдер можна було injected
  ...
  <Accordion title="Чи можу я використовувати MiniMax як типову модель, а OpenAI для складних завдань?">
    ...
    **Варіант B: окремі агенти**
    ...
    Документація: [Моделі](/uk/concepts/models), [Багатоагентна маршрутизація](/uk/concepts/multi-agent), [MiniMax](/uk/providers/minimax), [OpenAI](/uk/providers/openai).
  ...
  <Accordion title="Як визначити або перевизначити скорочення моделей (псевдоніми)?">
    ...
    Потім `/model sonnet` (або `/<alias>`, коли підтримується) перетворюється на цей ідентифікатор моделі.
  ...
  <Accordion title="Як додати моделі від інших провайдерів, як-от OpenRouter або Z.AI?">
    ...
    **Не знайдено API key для постачальника після додавання нового агента**
    ...
    - Для профілів OAuth увійдіть із нового агента, коли йому потрібен власний обліковий запис; інакше OpenClaw може читати дані через стандартного/основного агента без клонування refresh token.
...
<AccordionGroup>
  <Accordion title="Як працює відмовостійке перемикання?">
    ...
    До профілів із помилками застосовуються періоди очікування (експоненційний backoff), тому OpenClaw може продовжувати відповідати, навіть коли постачальник має обмеження частоти запитів або тимчасово не працює.
    ...
    обмеження частоти, що потребують відмовостійкого перемикання.

    Деякі відповіді, схожі на білінгові, не є `402`, а деякі HTTP-відповіді `402`
    також залишаються в цьому тимчасовому кошику. Якщо постачальник повертає
    явний білінговий текст для `401` або `403`, OpenClaw усе одно може залишити це
    в білінговій гілці, але текстові зіставлячі, специфічні для постачальника, залишаються обмеженими
    ...
    як-от Anthropic без додаткового тексту `An unknown error occurred`, OpenRouter без додаткового тексту
    ...
    сигнали тайм-ауту/перевантаження, що потребують відмовостійкого перемикання, коли контекст постачальника
    збігається.
    Загальний внутрішній текст резервного перемикання, як-от `LLM request failed with an unknown
    error.`, залишається консервативним і сам собою не запускає резервне перемикання моделі.
  ...
  <Accordion title='Що означає "No credentials found for profile anthropic:default"?'>
    ...
    **Контрольний список виправлення:**
    ...
    - **Підтвердьте, що вашу змінну середовища завантажує Gateway**
      - Якщо ви задали `ANTHROPIC_API_KEY` у своїй оболонці, але запускаєте Gateway через systemd/launchd, він може її не успадкувати. Додайте її в `~/.openclaw/.env` або увімкніть `env.shellEnv`.
    ...
    - **Базово перевірте стан моделі/автентифікації**
    ...
    **Контрольний список виправлення для "No credentials found for profile anthropic"**
    ...
    - **Якщо натомість ви хочете використати API key**
    ...
    - **Підтвердьте, що запускаєте команди на хості gateway**
      - У віддаленому режимі профілі автентифікації зберігаються на машині gateway, а не на вашому ноутбуку.

  </Accordion>

  <Accordion title="Чому він також спробував Google Gemini і завершився помилкою?">
    Якщо ваша конфігурація моделі містить Google Gemini як fallback (або ви перемкнулися на скорочення Gemini), OpenClaw спробує його під час резервного перемикання моделі. Якщо ви не налаштували облікові дані Google, ви побачите `No API key found for provider "google"`.

    Виправлення: або надайте автентифікацію Google, або вилучіть/уникайте моделей Google у `agents.defaults.model.fallbacks` / псевдонімах, щоб fallback не маршрутизував туди.

    **Запит LLM відхилено: потрібна thinking signature (Google Antigravity)**

    Причина: історія сеансу містить **thinking blocks without signatures** (часто з
    перерваного/часткового потоку). Google Antigravity вимагає signatures для thinking blocks.

    Виправлення: OpenClaw тепер видаляє unsigned thinking blocks для Google Antigravity Claude. Якщо це все ще з’являється, почніть **новий сеанс** або задайте `/thinking off` для цього агента.
...
Див. також: [/concepts/oauth](/uk/concepts/oauth) (потоки OAuth, зберігання token, шаблони кількох облікових записів)

<AccordionGroup>
  <Accordion title="Що таке профіль автентифікації?">
    Профіль автентифікації — це іменований запис облікових даних (OAuth або API key), прив’язаний до постачальника. Профілі зберігаються в:
  ...
  <Accordion title="Чи можу я керувати тим, який профіль автентифікації пробується першим?">
    Так. Конфігурація підтримує необов’язкові метадані для профілів і порядок для кожного постачальника (`auth.order.<provider>`). Це **не** зберігає секрети; воно зіставляє ID з постачальником/режимом і задає порядок ротації.

    OpenClaw може тимчасово пропустити профіль, якщо він перебуває в короткому **періоді очікування** (обмеження частоти/тайм-аути/помилки автентифікації) або довшому стані **вимкнено** (білінг/недостатньо кредитів). Щоб переглянути це, запустіть `openclaw models status --json` і перевірте `auth.unusableProfiles`. Налаштування: `auth.cooldowns.billingBackoffHours*`.

    Періоди очікування через обмеження частоти можуть бути обмежені конкретною моделлю. Профіль, який перебуває в періоді очікування
    для однієї моделі, все ще може бути придатним для спорідненої моделі в того самого постачальника,
    ...
    Щоб перевірити, що саме буде фактично випробувано, використайте:
    ...
    `excluded_by_auth_order` для цього профілю, а не пробує його мовчки.
  ...
  <Accordion title="OAuth чи API key — у чому різниця?">
    ...
    - **OAuth** часто використовує доступ за передплатою (де застосовно).
