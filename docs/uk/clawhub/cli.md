...
- `--dir <dir>`: каталог установлення в workdir (за замовчуванням: `skills`)
- `--site <url>`: базовий URL для входу через браузер (за замовчуванням: `https://clawhub.ai`)
- `--registry <url>`: базовий URL API (за замовчуванням: виявлений, інакше `https://clawhub.ai`)
...
- Друкує збережений API-токен у стандартний вивід.
...
- Популярність є невеликим попереднім чинником ранжування, а не гарантією верхнього місця.
- Якщо навичка має з'являтися, але не з'являється, виконайте `clawhub inspect @owner/slug` після входу, щоб перевірити видимі власнику діагностичні дані модерації перед перейменуванням метаданих.
...
- `--file <path>`: отримати сирий вміст файлу (лише текстові файли; ліміт 200KB).
- `--json`: машиночитаний вивід.

### `install @owner/slug`

- Визначає останню версію для названого власника та навички.
- Завантажує zip через `/api/v1/download`.
- Розпаковує в `<workdir>/<dir>/<slug>`.
- Відмовляється перезаписувати закріплені навички; спершу виконайте `clawhub unpin <skill>`.
- Записує:
...
- Надсилає telemetry за принципом найкращого зусилля після входу, щоб поточні лічильники встановлень можна було
  деактивувати.
...
- `update <skill>` швидко завершується помилкою для закріплених навичок і каже спершу виконати `clawhub unpin <skill>`.
...
під `root` (за замовчуванням: `skills`). Він пропускає незмінені навички та використовує
таку саму автоматичну поведінку patch-версії.

Установіть `dry_run: true`, щоб переглянути без токена. Реальні публікації вимагають
...
- Запускає ClawHub ClawScan через `POST /api/v1/skills/-/scan`, потім опитує, доки сканування не стане terminal.
- Сканування асинхронні та можуть потребувати часу для завершення. Поки вони в черзі, spinner термінала показує поточну пріоритетну позицію сканування та скільки сканувань попереду.
- Опубліковані сканування вимагають права власності або доступу керування видавцем. Модератори/admins можуть використовувати той самий backend через `clawhub-admin`.
...
- Сканування локальних шляхів більше не підтримуються. Завантажте нову версію, потім використайте `scan download`, щоб отримати збережені результати сканування для цієї надісланої версії.
...
- Вимагає `clawhub login`.
- Завантажує збережений ZIP-звіт сканування для надісланої версії навички або Plugin, включно з версіями, які були заблоковані або приховані перевірками безпеки ClawHub.
- Завантаження навичок використовують slug навички та за замовчуванням мають `--kind skill`.
- Завантаження Plugin використовують назву пакета та вимагають `--kind plugin`.
- `--version` обов'язковий, щоб автори перевіряли точну надіслану версію, яку ClawHub заблокував.
...
- `root` за замовчуванням має значення `skills` для репозиторіїв каталогів.
...
- `--version <version>` назавжди видаляє одну власну не найновішу версію через fail-closed,
...
- `--reason <text>` записує нотатку модерації для м’якого видалення всієї навички та журналу аудиту.
...
- Відновлення версії не передбачено; назавжди видалені версії не можна відновити.
- Викликає `POST /api/v1/skills/{slug}/undelete`.
- `--reason <text>` записує нотатку модерації для навички та журналу аудиту.
- `--note <text>` є псевдонімом для `--reason`.
- `--yes` пропускає підтвердження.
...
- Перейменовує власну навичку та зберігає попередній слаг як псевдонім перенаправлення.
...
- Вихідний слаг більше не відображається публічно та стає псевдонімом перенаправлення до цільової навички.
...
- Передавання на користувацькі handles створює запит в очікуванні, який приймає одержувач.
- Передавання на handles організацій/видавців застосовується негайно лише тоді, коли виконавець має
...
- `--version <version>`: інспектувати певну версію (типово: latest).
...
- Прапорці:
  - `--version <version>`: завантажити певну версію.
...
- Із прямими прапорцями дайджестів перевіряє без мережевого запиту.
- Прапорці:
...
- Запускає вбудований у ClawHub CLI Plugin Inspector для локальної теки пакета
  плагіна.
- Типово виконує офлайн/статичну валідацію без пошуку або імпорту локального
  checkout OpenClaw.
- Жорсткі помилки сумісності завершують команду з ненульовим кодом. Знахідки лише з попередженнями виводяться, але
  команда завершується з нульовим кодом.
- Прапорці:
...
[виправлення валідації Plugin](/uk/clawhub/plugin-validation-fixes), потім повторно запустіть команду.

### `package delete <name>`

- Без `--version` м’яко видаляє пакет і всі релізи.
- `--version <version>` назавжди видаляє один власний не найновіший реліз через fail-closed,
  специфічний для версії маршрут.
  Видалені версії не можна відновити або повторно опублікувати. Опублікуйте заміну перед видаленням
  поточної найновішої версії. Цей потік лише для версій вимагає власника пакета або адміністратора видавця організації; співробітники платформи не обходять право власності на пакет.
...
- Прапорці:
  - `--version <version>`: назавжди видалити одну не найновішу версію.
...
- Прапорці:
  - `--version <version>`: необов’язкова версія пакета для прив’язки до скарги.
...
- Викликає ту саму обчислювану кінцеву точку готовності, що й `package readiness`, але виводить
  стан із фокусом на міграцію, найновішу версію, стан офіційного пакета, перевірки та
  блокери.
- Прапорці:
  - `--json`: машинозчитуваний вивід.
...
- Публікує Plugin коду або пакетний Plugin через `POST /api/v1/packages`.
- `<source>` приймає:
...
- Папки Plugin коду пакуються в tarball npm ClawPack перед завантаженням, щоб
  інсталяції OpenClaw могли перевірити точний артефакт. Папки пакетних Plugin досі
...
- Зовнішні Plugin коду мають явно оголошувати `openclaw.compat.pluginApi` і
...
Для Plugin коду публікація папки створює та завантажує артефакт ClawPack з
папки пакета:
...
Зовнішні Plugin коду потребують невеликого обсягу метаданих OpenClaw у
...
ClawHub також постачає офіційний перевикористовуваний workflow за адресою
...
- Перевикористовуваний workflow за замовчуванням задає `source` як репозиторій викликача.
- Для монорепозиторіїв передайте `source_path`, щоб workflow публікував папку пакета Plugin,
  наприклад `source_path: extensions/codex`.
- Закріпіть перевикористовуваний workflow за стабільним тегом або повним SHA коміту. Не запускайте публікацію релізів з `@main`.
...
- Довірена публікація без secret працює лише для `workflow_dispatch`; push тегів усе ще потребують `clawhub_token`.
- Тримайте `clawhub_token` доступним для першої публікації, недовірених пакетів або екстрених публікацій.
- Workflow завантажує результат JSON як артефакт і надає його як виходи workflow.
...
- Прикріплює або замінює конфігурацію довіреного видавця GitHub Actions для наявного
  пакета.
- Пакет потрібно спершу створити через звичайний ручний або автентифікований токеном
...
- Прапорці:
  ...
  - `--environment <name>`: необов’язкове середовище GitHub Actions з точним збігом.
  - `--json`: машинозчитуваний результат.
...
- Надсилання звітності виконується best-effort. Команди інсталяції не завершуються помилкою, якщо телеметрія
  недоступна.
