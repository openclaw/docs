...
- Друкує збережений токен API у stdout.
...
- Вивід містить slug навички, ідентифікатор власника, відображувану назву та оцінку релевантності.
...
- Якщо навичка має з'являтися, але не з'являється, виконайте `clawhub inspect @owner/slug` після входу, щоб перевірити видимі власнику діагностичні дані модерації, перш ніж перейменовувати метадані.

### `explore`

- Виводить найновіші навички через `/api/v1/skills?limit=...&sort=createdAt` (відсортовано за `createdAt` desc).
- Прапорці:
...
- Визначає latest-версію для названого власника та навички.
...
- Якщо fingerprint збігається з відомою версією: без підказки.
- Якщо fingerprint не збігається:
  - за замовчуванням відмовляється
  - перезаписує з `--force` (або з підказкою, якщо інтерактивно)
...
- `--owner <handle>` публікує від імені ідентифікатора видавця організації/користувача, коли
...
всередині `root` (за замовчуванням: `skills`). Він пропускає незмінені навички та використовує
таку саму поведінку автоматичної patch-версії.
...
- `--owner <handle>` публікує від імені ідентифікатора видавця організації/користувача, коли
  актор має доступ видавця.
- `sync` — це лише одностороння публікація. Вона не встановлює, не оновлює, не завантажує й не
...
- Сканування асинхронні й можуть потребувати часу для завершення. Поки вони в черзі, terminal spinner показує поточну пріоритетну позицію сканування та скільки сканувань попереду.
...
- Сканування локальних шляхів більше не підтримуються. Завантажте нову версію, а потім використайте `scan download`, щоб отримати збережені результати сканування для цієї поданої версії.
...
- Потребує `clawhub login`.
- Завантажує збережений ZIP-звіт сканування для поданої версії навички або Plugin, включно з версіями, які ClawHub заблокував або приховав через перевірки безпеки.
...
- `owner` відповідає прапорцю CLI `--owner`; опустіть його, щоб публікувати як автентифікований користувач.
...
- `--reason <text>` записує модераційну нотатку для м’якого видалення всієї навички та журналу аудиту.
...
- Відновлення версій не передбачено; остаточно видалені версії не можна відновити.
- Викликає `POST /api/v1/skills/{slug}/undelete`.
- `--reason <text>` записує модераційну нотатку для навички та журналу аудиту.
- `--note <text>` є псевдонімом для `--reason`.
- `--yes` пропускає підтвердження.
...
- Перейменовує належну власнику навичку та зберігає попередній slug як псевдонім перенаправлення.
- Викликає `POST /api/v1/skills/{slug}/rename`.
- `--yes` пропускає підтвердження.

### `skill merge <source> <target>`

- Об’єднує одну належну власнику навичку з іншою належною власнику навичкою.
- Вихідний slug перестає відображатися публічно та стає псевдонімом перенаправлення на ціль.
...
- Прапорці:
  ...
  - `--openclaw <path>`: перевірити щодо явно вказаного локального checkout OpenClaw.
...
- `--version <version>` остаточно видаляє один належний власнику не найновіший реліз через fail-closed,
  версієспецифічний маршрут.
  Видалені версії не можна відновити або опублікувати повторно. Опублікуйте заміну перед видаленням
  поточної найновішої версії. Цей потік лише для версій вимагає власника пакета або адміністратора
...
- Повідомлення мають рівень пакета, можуть необов’язково прив’язуватися до версії та стають видимими
...
- Викликає ту саму обчислену кінцеву точку readiness, що й `package readiness`, але виводить
  зосереджений на міграції стан, найновішу версію, стан офіційного пакета, перевірки та
  блокери.
- Прапорці:
  - `--json`: машинозчитуваний вивід.
...
- Завершується помилкою, якщо handle вже використовується наявним видавцем, користувачем або зарезервованим маршрутом.
...
- Метадані автоматично визначаються з `package.json`, `openclaw.plugin.json` і
  справжніх маркерів пакета OpenClaw, таких як `.codex-plugin/plugin.json`,
...
Для кодових Plugin публікація папки створює та завантажує артефакт ClawPack з
папки пакета:
...
- Зафіксуйте reusable workflow на стабільному тегу або повному SHA коміту. Не запускайте публікацію релізу з `@main`.
- `pull_request` має використовувати `dry_run: true`, щоб CI не забруднював стан.
- Реальні публікації слід обмежувати довіреними подіями, такими як `workflow_dispatch` або push тегів.
...
- Використовуйте це після встановлення конфігурації, щоб підтвердити репозиторій, назву файлу workflow
  і необов’язкову прив’язку environment.
- Прапорці:
...
- Прикріплює або замінює конфігурацію довіреного видавця GitHub Actions для наявного
  пакета.
- Спочатку пакет має бути створений через звичайну ручну або token-authenticated
...
- `--workflow-filename <file>` має відповідати імені файлу workflow у
  `.github/workflows/`.
- `--environment <name>` необов’язковий. Коли налаштовано, environment GitHub Actions
...
- Прапорці:
  ...
  - `--environment <name>`: необов’язкове environment GitHub Actions із точним збігом.
  - `--json`: машиночитний результат.
...
- Використовуйте це як rollback, якщо workflow, репозиторій або прив’язку environment потрібно
  вимкнути або створити повторно.
- Майбутні реальні публікації мають використовувати звичайну автентифіковану публікацію, доки конфігурацію
