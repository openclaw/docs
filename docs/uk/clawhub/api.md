...
x-i18n:
    generated_at: "2026-07-01T13:20:16Z"
    model: gpt-5.5
...
Ви можете побудувати сторонній каталог, директорію або поверхню пошуку на основі публічних API читання ClawHub. Публічні метадані Skills і файли Skills публікуються відповідно до ліцензійних правил Skills ClawHub, тоді як сам API має обмеження частоти запитів і його слід використовувати відповідально.
...
- Додавайте посилання на канонічну URL-адресу Skills ClawHub під час показу списків, щоб користувачі могли переглянути вихідний запис реєстру.
...
- `X-RateLimit-Remaining` / `RateLimit-Remaining`: точний залишок бюджету, коли
  наявний; шардовані успішні запити опускають його замість повернення приблизного
...
- `POST /api/v1/users/reserve` резервує кореневі slugs і приватні заповнювачі пакетів без випуску для дескриптора власника.
