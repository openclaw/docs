...
x-i18n:
    generated_at: "2026-07-02T22:47:08Z"
    model: gpt-5.5
...
Ви можете створити сторонній каталог, директорію або пошукову поверхню на основі публічних API читання ClawHub. Публічні метадані Skills і файли Skills публікуються за ліцензійними правилами ClawHub для Skills, тоді як сам API має обмеження частоти запитів і повинен використовуватися відповідально.
...
- Додавайте посилання на канонічний URL Skill у ClawHub під час показу списків, щоб користувачі могли переглянути вихідний запис реєстру.
...
- Не дзеркальте прихований, приватний або заблокований модерацією вміст шляхом обходу фільтрів публічного API чи меж автентифікації.
...
- `GET /api/v1/skills?limit=&cursor=&sort=`
  - `sort`: `updated` (типово), `recommended` (`default`), `createdAt` (`newest`), `downloads`, `stars` (`rating`), застарілі псевдоніми встановлення `installsCurrent`/`installs`/`installsAllTime` мапляться на `downloads`, `trending`
...
- `POST /api/v1/users/reserve` резервує кореневі слаги та приватні заповнювачі пакетів без випуску для handle власника.
