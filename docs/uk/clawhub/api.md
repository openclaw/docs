...
x-i18n:
    generated_at: "2026-07-05T08:20:39Z"
    model: gpt-5.5
...
Ви можете створити сторонній каталог, довідник або пошукову поверхню на основі публічних API читання ClawHub. Публічні метадані навичок і файли навичок публікуються за правилами ліцензування навичок ClawHub, тоді як сам API має обмеження частоти запитів і має використовуватися відповідально.
...
- Під час показу списків додавайте посилання на канонічну URL-адресу навички ClawHub, щоб користувачі могли переглянути вихідний запис реєстру.
...
- `X-RateLimit-Remaining` / `RateLimit-Remaining`: точний залишковий бюджет, коли
  наявний; сегментовані успішні запити пропускають його замість повернення приблизного
...
- `GET /api/v1/skills?limit=&cursor=&sort=`
  - `sort`: `updated` (типово), `recommended` (`default`), `createdAt` (`newest`), `downloads`, `stars` (`rating`), застарілі псевдоніми встановлення `installsCurrent`/`installs`/`installsAllTime` відображаються на `downloads`, `trending`
...
- `GET /api/v1/download?slug=&version=&tag=`
  - Розміщені навички повертають детерміновані ZIP-байти.
  - Поточні навички на основі GitHub зі скануванням `clean` або `suspicious` повертають
...
- `GET /api/v1/skills/export?startDate=&endDate=&limit=&cursor=`
  ...
  - Поточні навички на основі GitHub зі скануванням `clean` або `suspicious` експортуються
