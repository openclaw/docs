...
Ви можете створити сторонній каталог, директорію або поверхню пошуку на основі публічних API читання ClawHub. Публічні метадані навичок і файли навичок публікуються згідно з правилами ліцензії ClawHub для навичок, а сам API має обмеження частоти запитів і повинен використовуватися відповідально.
...
- Посилайтеся на канонічну URL-адресу навички ClawHub під час відображення списків, щоб користувачі могли переглянути вихідний запис реєстру.
...
- Не дзеркальте прихований, приватний або заблокований модерацією вміст шляхом обходу фільтрів публічного API або меж автентифікації.
...
- `X-RateLimit-Remaining` / `RateLimit-Remaining`: точний залишковий бюджет, коли
  наявний; шардовані успішні запити пропускають його, а не повертають приблизне
  глобальне значення
- `Retry-After`: секунди затримки для очікування при `429`
...
- Надавайте перевагу `Retry-After`, коли він наявний.
- Інакше використовуйте `RateLimit-Reset` або виводьте затримку з `X-RateLimit-Reset`.
- Додавайте джитер до повторних спроб.

## Помилки

- Помилки v1 є простим текстом (`text/plain; charset=utf-8`), включно з `400`,
...
- `GET /api/v1/skills?limit=&cursor=&sort=`
  ...
  - `cursor` застосовується до сортувань, що не є `trending`
...
- `GET /api/v1/download?slug=&version=&tag=`
  - Розміщені навички повертають детерміновані байти ZIP.
  - Поточні навички з бекендом GitHub зі сканом `clean` або `suspicious` повертають
    JSON-дескриптор передачі `public-github` замість байтів ClawHub.
- `GET /api/v1/skills/export?startDate=&endDate=&limit=&cursor=`
  - Розміщені навички експортуються як збережені файли.
  - Поточні навички з бекендом GitHub зі сканом `clean` або `suspicious` експортуються
...
- `POST /api/v1/users/reserve` резервує кореневі слаги та приватні заповнювачі пакетів без випусків для облікового імені власника.
