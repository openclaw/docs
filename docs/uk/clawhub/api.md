...
Ви можете створити сторонній каталог, директорію або пошукову поверхню поверх публічних API читання ClawHub. Публічні метадані Skills і файли Skills публікуються за правилами ліцензування Skills у ClawHub, тоді як сам API має обмеження частоти запитів і має використовуватися відповідально.
...
- `X-RateLimit-Remaining` / `RateLimit-Remaining`: точний залишковий бюджет, коли
  присутній; сегментовані успішні запити пропускають його замість повернення приблизного
...
- `POST /api/v1/users/reserve` резервує кореневі slugs і приватні no-release заповнювачі пакетів для handle власника.
