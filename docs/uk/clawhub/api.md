...
x-i18n:
    generated_at: "2026-06-30T14:22:14Z"
    model: gpt-5.5
...
Ви можете побудувати сторонній каталог, директорію або пошукову поверхню на основі публічних API читання ClawHub. Публічні метадані навичок і файли навичок публікуються за правилами ліцензування навичок ClawHub, тоді як сам API має обмеження швидкості, і його слід використовувати відповідально.

Рекомендації:

- Використовуйте публічні кінцеві точки читання, такі як `GET /api/v1/skills`, `GET /api/v1/search` і `GET /api/v1/skills/{slug}`, для списків каталогу.
- Кешуйте відповіді та поважайте `429`, `Retry-After` і заголовки обмеження швидкості замість агресивного опитування.
- Додавайте посилання на канонічний URL навички ClawHub під час відображення списків, щоб користувачі могли переглянути вихідний запис реєстру.
...
- `X-RateLimit-Remaining` / `RateLimit-Remaining`: точний залишковий бюджет, коли
  наявний; шардовані успішні запити опускають його, а не повертають приблизне
  глобальне значення
...
- `GET /api/v1/skills?limit=&cursor=&sort=`
  - `sort`: `updated` (за замовчуванням), `recommended` (`default`), `createdAt` (`newest`), `downloads`, `stars` (`rating`), застарілі псевдоніми встановлення `installsCurrent`/`installs`/`installsAllTime` зіставляються з `downloads`, `trending`
...
- `GET /api/v1/download?slug=&version=&tag=`
  ...
  - Поточні навички на базі GitHub зі скануванням `clean` або `suspicious` повертають
...
- `GET /api/v1/skills/export?startDate=&endDate=&limit=&cursor=`
  ...
  - Поточні навички на базі GitHub зі скануванням `clean` або `suspicious` експортуються
