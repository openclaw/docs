...
read_when:
    ...
    - ClawHub відхилив або видав попередження щодо публікації пакета Plugin
    - Ви оновлюєте метадані пакета Plugin перед випуском
summary: Виправте виявлені під час перевірки пакета Plugin ClawHub проблеми перед публікацією
...
автоматизованих сканувань пакетів. Ця сторінка охоплює проблеми, орієнтовані на автора, тобто
...
| `package-openclaw-metadata-missing`     | [Додайте блок пакета openclaw](/uk/clawhub/plugin-validation-fixes#package-openclaw-metadata-missing)                              |
...
| `sdk-session-file-helper`               | [Замініть допоміжні функції шляхів файлів сеансів](/uk/clawhub/plugin-validation-fixes#sdk-session-file-helper)                         |
| `sdk-session-transcript-file-target`    | [Замініть застарілі файлові цілі стенограм](/uk/clawhub/plugin-validation-fixes#sdk-session-transcript-file-target)                     |
| `sdk-session-transcript-low-level`      | [Замініть низькорівневі допоміжні функції стенограм](/uk/clawhub/plugin-validation-fixes#sdk-session-transcript-low-level)              |
...
- Включіть метадані точки входу, наприклад `openclaw.extensions` або
...
- Зберіть пакет, якщо точку входу генерується в `dist`.
...
- Видаліть непідтримувані поля, наприклад `openclaw.bundle`.
- Тримайте нативні метадані Plugin в `openclaw.plugin.json`.
- Тримайте точки входу пакета, сумісність, встановлення, налаштування й каталогові метадані
...
- Виправте недійсні метадані пакета, зламані сценарії життєвого циклу або записи файлів, які
...
- Якщо поведінка не має публічного SDK, тримайте допоміжний засіб усередині свого пакета або
...
Plugin усе ще використовує застарілий допоміжний засіб усього сховища сесій
`loadSessionStore`.
...
- Залишайте `loadSessionStore(...)` лише доки ваш оголошений діапазон сумісності
  усе ще підтримує старіші версії OpenClaw, яким він потрібен.
...
Plugin усе ще використовує застарілий допоміжний засіб запису всього сховища сесій, як-от
`saveSessionStore` або `updateSessionStore`.

- Використовуйте `patchSessionEntry(...)` під час оновлення полів в наявному записі
  сесії.
...
- Залишайте допоміжні засоби запису всього сховища лише доки ваш оголошений діапазон сумісності
  усе ще підтримує старіші версії OpenClaw, яким вони потрібні.
- Див. [Runtime API](/uk/plugins/sdk-runtime#agent-session-state) і
  [Підшляхи Plugin SDK](/uk/plugins/sdk-subpaths).
- Повторно запустіть `clawhub package validate <path-to-plugin>`.
...
Plugin усе ще використовує застарілі допоміжні засоби шляхів до файлів сесій, як-от
...
Plugin усе ще використовує застарілий допоміжний засіб цілі файлу transcript
...
- Уникайте прямого читання або конструювання застарілих цілей файлів transcript.
...
Plugin усе ще використовує застарілі низькорівневі допоміжні засоби transcript, як-от
...
- Використовуйте `publishSessionTranscriptUpdateByIdentity(...)` для сповіщень про оновлення
  transcript.
- Віддавайте перевагу структурованій runtime-поверхні transcript, щоб OpenClaw міг застосовувати
...
Plugin усе ще використовує застарілий hook `before_agent_start`.

- Перемістіть роботу з перевизначення моделі або провайдера до `before_model_resolve`.
- Перемістіть роботу зі змінення prompt або context до `before_prompt_build`.
...
- Дзеркально перенесіть метадані env-var провайдера до `setup.providers[].envVars`.
...
- Зберігайте метадані env-var каналу декларативними, щоб OpenClaw міг перевіряти статус setup
  без завантаження runtime каналу.
- Дзеркально перенесіть керований env setup каналу до поточних метаданих setup, config каналу або
...
- Видаліть `openclaw.security.json`, доки OpenClaw не задокументує версіоновану схему маніфесту безпеки
  і поведінку ClawHub.
- Зберігайте поведінку, чутливу до безпеки, задокументованою у публічній документації вашого пакета або
