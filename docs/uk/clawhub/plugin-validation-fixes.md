---
read_when:
    - Ви виконали `clawhub package validate` і маєте виправити знахідки Plugin
...
x-i18n:
    generated_at: "2026-07-02T22:46:44Z"
    model: gpt-5.5
...
| `sdk-session-store-write`               | [Замініть записи в усьому сховищі сесій](/uk/clawhub/plugin-validation-fixes#sdk-session-store-write)                          |
| `sdk-session-file-helper`               | [Замініть помічники шляхів до файлів сесій](/uk/clawhub/plugin-validation-fixes#sdk-session-file-helper)                       |
...
- Використовуйте [Побудова plugin](/uk/plugins/building-plugins) для мінімального прикладу
...
Метадані пакета існують, але не оголошують точку входу runtime OpenClaw.
...
- Виправте некоректні метадані пакета, зламані lifecycle scripts або записи files, через які
...
- Запустіть `npm pack --dry-run` і перевірте файли, які буде додано.
...
- Див. [Побудова plugin](/uk/plugins/building-plugins).
- Повторно запустіть `clawhub package validate <path-to-plugin>`.
...
- Див. [Plugin manifest](/uk/plugins/manifest).
- Повторно запустіть `clawhub package validate <path-to-plugin>`.
...
- Перенесіть поведінку під час виконання до коду реєстрації плагіна, а `contracts`
...
- Замініть імпорти з кореневого бареля на зосереджені імпорти з публічних підшляхів.
...
- Використовуйте `getSessionEntry(...)`, щоб читати метадані сеансу за агентом і
  ідентичністю сеансу.
...
- Використовуйте `publishSessionTranscriptUpdateByIdentity(...)` для сповіщень про оновлення
  транскрипту.
- Віддавайте перевагу структурованій поверхні часу виконання транскриптів, щоб OpenClaw міг застосувати
  правильні межі транзакцій і обробку ідентичності.
- Залишайте низькорівневі допоміжні засоби транскриптів лише доки оголошений вами діапазон сумісності
  ще підтримує старіші версії OpenClaw, яким вони потрібні.
- Див. [API часу виконання](/uk/plugins/sdk-runtime#agent-session-state) і
  [підшляхи Plugin SDK](/uk/plugins/sdk-subpaths).
- Повторно запустіть `clawhub package validate <path-to-plugin>`.
...
- Перенесіть роботу зі зміни підказки або контексту до `before_prompt_build`.
...
- Віддзеркальте налаштування каналу, кероване змінними середовища, у поточні метадані setup, конфігурації каналу або
...
- Див. [Plugin manifest](/uk/plugins/manifest) і
