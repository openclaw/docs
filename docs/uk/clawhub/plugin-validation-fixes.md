...
x-i18n:
    generated_at: "2026-07-01T20:34:31Z"
    model: gpt-5.5
...
ClawHub перевіряє пакети Plugin перед публікацією, а також може показувати результати автоматизованого сканування пакетів. Ця сторінка описує результати для авторів, тобто результати, які автор Plugin може виправити в метаданих пакета, маніфесті, імпортах SDK або опублікованому артефакті.
...
| `sdk-session-store-write`               | [Замініть записи до сховища всієї сесії](/uk/clawhub/plugin-validation-fixes#sdk-session-store-write)                                  |
...
| `security-manifest-schema-unavailable`  | [Видаліть посилання на недоступні схеми маніфестів безпеки](/uk/clawhub/plugin-validation-fixes#security-manifest-schema-unavailable) |
| `unrecognized-security-manifest`        | [Видаліть непідтримувані файли маніфестів безпеки](/uk/clawhub/plugin-validation-fixes#unrecognized-security-manifest)                   |
...
- Скористайтеся [Створенням plugins](/uk/plugins/building-plugins) для мінімального прикладу пакета та [Маніфестом Plugin](/uk/plugins/manifest#manifest-versus-packagejson) для розділення пакета й маніфесту.
...
Мінімальна версія хоста пакета не відповідає метаданим версії OpenClaw, з якими було зібрано пакет.
...
- Якщо `openclaw.plugin.json` також має `version`, оновіть її до відповідності або видаліть застарілі метадані версії маніфесту, коли метадані пакета є авторитетними.
...
- Виправте недійсні метадані пакета, зламані lifecycle scripts або записи files, через які пакування завершується помилкою.
- Видаліть `private: true`, якщо цей пакет призначено для публічного опублікування.
- Повторно запустіть `clawhub package validate <path-to-plugin>`.
...
- Перемістіть поведінку runtime у код реєстрації Plugin і залиште `contracts`
  обмеженим статичними метаданими володіння можливостями.
- Повторно виконайте `clawhub package validate <path-to-plugin>`.
...
- Використовуйте `openclaw/plugin-sdk/channel-core` для допоміжних функцій точок входу каналів.
...
Plugin досі використовує застарілу допоміжну функцію всього сховища сесії
`loadSessionStore`.
...
- Використовуйте `patchSessionEntry(...)` або `upsertSessionEntry(...)` під час запису стану
  сесії.
- Уникайте завантаження, змінення та збереження всього об'єкта сховища сесії.
...
Plugin досі використовує застарілу допоміжну функцію запису всього сховища сесії, як-от
...
Plugin досі використовує застарілі допоміжні функції шляхів до файлів сесії, як-от
`resolveSessionFilePath` або `resolveAndPersistSessionFile`.

- Використовуйте `getSessionEntry(...)`, щоб читати метадані сесії за агентом та ідентичністю
  сесії.
...
- Використовуйте задокументовану версійовану схему лише після того, як OpenClaw опублікує її.
- Повторно виконайте `clawhub package validate <path-to-plugin>`.
...
- Видаліть `openclaw.security.json`, доки OpenClaw не задокументує версійовану схему маніфесту
