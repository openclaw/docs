...
x-i18n:
    generated_at: "2026-07-02T14:10:40Z"
    model: gpt-5.5
...
автоматизованих сканувань пакетів. Ця сторінка описує висновки для авторів, тобто
...
- Використовуйте [Створення plugins](/uk/plugins/building-plugins) для мінімального прикладу пакета
...
- Додайте `openclaw.extensions` для точок входу нативних Plugin.
...
Пакет оголошує точку входу OpenClaw, але зазначений файл відсутній
...
- Використовуйте версію API Plugin OpenClaw або мінімальну semver-версію, проти якої ви збирали й тестували.
...
Блок `package.json#openclaw` містить поля, що не підтримуються
...
- Перенесіть метадані пакета або встановлення в підтримувані поля `package.json#openclaw`
  замість маніфесту.
- Повторно запустіть `clawhub package validate <path-to-plugin>`.
...
- Перенесіть поведінку виконання в код реєстрації Plugin, а `contracts`
...
- Замініть імпорти з кореневого бареля на зосереджені публічні імпорти з підшляхів.
...
- Див. [API виконання](/uk/plugins/sdk-runtime#agent-session-state) і
...
- Використовуйте `publishSessionTranscriptUpdateByIdentity(...)` для сповіщень про оновлення
  транскрипту.
- Надавайте перевагу структурованій поверхні виконання транскриптів, щоб OpenClaw міг застосувати
...
- Віддзеркальте setup каналу на основі змінних середовища в поточні метадані setup, конфігурації каналу або
...
- Залишайте поведінку, чутливу до безпеки, задокументованою в публічній документації вашого пакета або
