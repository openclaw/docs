...
read_when:
    ...
    - ClawHub відхилив публікацію пакета Plugin або видав щодо неї попередження
...
x-i18n:
    generated_at: "2026-06-30T14:22:55Z"
    model: gpt-5.5
...
Вона не описує внутрішні знахідки покриття Plugin Inspector. Якщо повний звіт
...
| `package-npm-pack-entrypoint-missing`   | [Включіть точки входу до виводу npm pack](/uk/clawhub/plugin-validation-fixes#package-npm-pack-entrypoint-missing)                  |
| `package-npm-pack-metadata-missing`     | [Включіть метадані до виводу npm pack](/uk/clawhub/plugin-validation-fixes#package-npm-pack-metadata-missing)                       |
...
| `sdk-session-file-helper`               | [Замініть допоміжні засоби шляхів до файлів сеансів](/uk/clawhub/plugin-validation-fixes#sdk-session-file-helper)                                   |
...
Пакет оголошує точку входу OpenClaw, але згаданий файл відсутній
...
- Використовуйте версію API Plugin OpenClaw або мінімальну semver-версію, відносно якої ви
  збирали й тестували.
...
- Виправте недійсні метадані пакета, зламані lifecycle-скрипти або записи файлів, через які
...
- Якщо поведінка не має публічного SDK, залиште допоміжний засіб усередині свого пакета або
  попросіть публічний OpenClaw API.
...
- Залишайте `loadSessionStore(...)` лише доки оголошений вами діапазон сумісності
  досі підтримує старіші версії OpenClaw, яким це потрібно.
...
Plugin досі використовує застарілий допоміжний засіб запису цілого сховища сеансів, як-от
...
- Уникайте завантаження, змінювання та збереження всього об’єкта сховища сеансів.
- Залишайте допоміжні засоби запису цілого сховища лише доки оголошений вами діапазон сумісності
...
- Використовуйте ідентичність transcript або допоміжні засоби цілі, коли код готує
  операцію transcript.
- Не зберігайте й не покладайтеся на застарілі шляхи файлів transcript.
- Див. [Runtime API](/uk/plugins/sdk-runtime#agent-session-state) і
  [підшляхи Plugin SDK](/uk/plugins/sdk-subpaths).
- Повторно запустіть `clawhub package validate <path-to-plugin>`.
...
- Залишайте застарілий допоміжний засіб лише доки оголошений вами діапазон сумісності досі
...
- Використовуйте `publishSessionTranscriptUpdateByIdentity(...)` для сповіщень про оновлення
  transcript.
- Надавайте перевагу структурованій поверхні середовища виконання transcript, щоб OpenClaw міг застосувати
...
- Перемістіть роботу зі зміною prompt або context до `before_prompt_build`.
- Залишайте `before_agent_start` лише доки оголошений вами діапазон сумісності досі
  підтримує старіші версії OpenClaw, яким він потрібен.
...
- Віддзеркальте налаштування каналу, кероване змінними середовища, у поточні метадані setup, config каналу або
  package channel metadata, які використовує форма вашого Plugin.
