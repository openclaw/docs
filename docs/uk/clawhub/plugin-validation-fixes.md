...
summary: Виправити зауваження валідації пакета Plugin для ClawHub перед публікацією
...
вони призначені для супровідників OpenClaw, а не для авторів плагінів.
...
Пакет має `package.json`, але він не оголошує метадані пакета OpenClaw.
...
- Задайте `openclaw.install.defaultChoice`, коли доступно кілька джерел встановлення.
...
відносно якої пакет було зібрано.
...
Маніфест нативного плагіна не містить відображуваного імені.

- Додайте непорожнє поле `name` до `openclaw.plugin.json`.
- Тримайте `name` зрозумілим для людей, а `id` — стабільним машинним ідентифікатором.
- Див. [Маніфест Plugin](/uk/plugins/manifest).
- Повторно виконайте `clawhub package validate <path-to-plugin>`.
