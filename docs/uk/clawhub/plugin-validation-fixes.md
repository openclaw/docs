...
x-i18n:
    generated_at: "2026-07-01T08:28:56Z"
    model: gpt-5.5
...
ClawHub перевіряє пакети плагінів перед публікацією, а також може показувати висновки з
автоматизованого сканування пакетів. Ця сторінка охоплює висновки для авторів, тобто
висновки, які автор плагіна може виправити в метаданих пакета, маніфесті, імпортах SDK
...
Метадані пакета існують, але вони не оголошують точку входу runtime
OpenClaw.
...
- Оновіть метадані, якщо точка входу перемістилася.
...
- Використовуйте версію API плагіна OpenClaw або мінімальну semver-версію, з якою ви збирали й тестували
  пакет.
...
- Виправте некоректні метадані пакета, несправні lifecycle-скрипти або записи files, які
...
- Натомість перенесіть метадані пакета або встановлення до підтримуваних полів
...
- Перенесіть поведінку середовища виконання до коду реєстрації Plugin, а `contracts`
...
- Залишайте застарілий допоміжний засіб лише доки оголошений діапазон сумісності ще
...
- Залишайте `before_agent_start` лише доки оголошений діапазон сумісності ще
  підтримує старіші версії OpenClaw, яким він потрібен.
...
- Документуйте безпекочутливу поведінку в публічній документації пакета або
