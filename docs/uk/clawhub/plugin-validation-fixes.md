...
x-i18n:
    generated_at: "2026-07-01T13:18:02Z"
    model: gpt-5.5
...
автоматизованого сканування пакетів. Ця сторінка описує знахідки для авторів, тобто
...
- Використайте [Створення plugins](/uk/plugins/building-plugins) для мінімального прикладу пакета
...
Пакет оголошує точку входу OpenClaw, але в пакеті, що перевіряється, немає
посиланого файлу.
...
ClawHub не може визначити, як потрібно встановлювати або оновлювати пакет.

- Заповніть `openclaw.install` підтримуваним джерелом встановлення, наприклад
  `clawhubSpec`, `npmSpec` або `localPath`.
- Установіть `openclaw.install.defaultChoice`, коли доступно більш ніж одне джерело
  встановлення.
...
- Використовуйте версію API Plugin OpenClaw або мінімальну semver-версію, під яку ви створювали й тестували.
...
- Виправте недійсні метадані пакета, пошкоджені lifecycle scripts або записи файлів, через які
...
- Див. [Plugin-маніфест](/uk/plugins/manifest).
- Повторно запустіть `clawhub package validate <path-to-plugin>`.
...
- Перенесіть поведінку середовища виконання до коду реєстрації Plugin і обмежте `contracts`
...
- Якщо поведінка не має публічного SDK, залиште допоміжну функцію всередині свого пакета або
  запитайте публічний OpenClaw API.
...
Plugin досі використовує застарілу допоміжну функцію для всього сховища сеансів
`loadSessionStore`.
...
- Використовуйте `patchSessionEntry(...)` або `upsertSessionEntry(...)` під час запису стану
  сеансу.
- Уникайте завантаження, змінення й збереження всього об’єкта сховища сеансів.
...
- Уникайте завантаження, змінення й збереження всього об’єкта сховища сеансів.
- Залишайте допоміжні функції запису всього сховища лише доки оголошений вами діапазон сумісності
...
- Залишайте застарілу допоміжну функцію лише доки оголошений вами діапазон сумісності ще
...
- Надавайте перевагу структурованому runtime-інтерфейсу transcript, щоб OpenClaw міг застосувати
  правильні межі транзакцій і обробку ідентичності.
- Залишайте низькорівневі допоміжні функції transcript лише доки оголошений вами діапазон сумісності
  ще підтримує старіші версії OpenClaw, яким вони потрібні.
- Див. [Runtime API](/uk/plugins/sdk-runtime#agent-session-state) і
  [підшляхи Plugin SDK](/uk/plugins/sdk-subpaths).
- Повторно запустіть `clawhub package validate <path-to-plugin>`.
