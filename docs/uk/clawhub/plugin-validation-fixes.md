...
x-i18n:
    generated_at: "2026-07-02T08:44:41Z"
    model: gpt-5.5
...
ClawHub перевіряє пакети Plugin перед публікацією, а також може показувати результати автоматизованих сканувань пакетів. Ця сторінка описує результати, призначені для авторів, тобто ті, які автор Plugin може виправити в метаданих пакета, маніфесті, імпортах SDK або опублікованому артефакті.

Вона не охоплює внутрішні результати покриття Plugin Inspector. Якщо повний звіт містить коди обслуговування сканера без настанов щодо виправлення для автора, вони призначені для мейнтейнерів OpenClaw, а не для авторів Plugin.
...
| `sdk-session-store-write`               | [Замініть записи до всього сховища сеансів](/uk/clawhub/plugin-validation-fixes#sdk-session-store-write)                       |
| `sdk-session-file-helper`               | [Замініть помічники шляхів файлів сеансу](/uk/clawhub/plugin-validation-fixes#sdk-session-file-helper)                         |
...
- Використовуйте [Створення plugins](/uk/plugins/building-plugins) як мінімальний приклад пакета та [Маніфест Plugin](/uk/plugins/manifest#manifest-versus-packagejson) для розділення пакета й маніфесту.
...
- Додайте метадані точки входу, як-от `openclaw.extensions` або `openclaw.runtimeExtensions`.
...
Пакет оголошує точку входу OpenClaw, але посиланий файл відсутній у пакеті, який перевіряється.
...
- Використовуйте версію API Plugin OpenClaw або мінімальну semver-версію, на основі якої ви збирали й тестували.
...
Блок `package.json#openclaw` містить поля, які не підтримуються метаданими пакета OpenClaw.

- Вилучіть непідтримувані поля, як-от `openclaw.bundle`.
- Тримайте нативні метадані Plugin в `openclaw.plugin.json`.
- Тримайте точки входу пакета, сумісність, встановлення, налаштування та метадані каталогу в підтримуваних полях `package.json#openclaw`.
- Див. [поля package.json, що впливають на виявлення](/uk/plugins/manifest#packagejson-fields-that-affect-discovery).
- Запустіть повторно `clawhub package validate <path-to-plugin>`.
...
- Виправте некоректні метадані пакета, зламані lifecycle scripts або записи files, через які пакування не вдається.
- Вилучіть `private: true`, якщо цей пакет призначений для публічної публікації.
- Запустіть повторно `clawhub package validate <path-to-plugin>`.

### package-npm-pack-entrypoint-missing

Пакет можна запакувати, але запакований артефакт не містить файли точок входу, оголошені в `package.json#openclaw`.

- Запустіть `npm pack --dry-run` і перегляньте файли, які буде включено.
- Зберіть згенеровані точки входу перед пакуванням.
- Оновіть `files`, `.npmignore` або вивід збірки, щоб оголошені точки входу було включено.
- Див. [Точки входу Plugin](/uk/plugins/sdk-entrypoints).
- Запустіть повторно `clawhub package validate <path-to-plugin>`.
...
- Зробіть `name` зручним для читання людиною, а `id` залиште стабільним машинним ідентифікатором.
- Див. [Маніфест Plugin](/uk/plugins/manifest).
...
- Перемістіть поведінку runtime у код реєстрації plugin, а `contracts`
...
- Використовуйте `openclaw/plugin-sdk/channel-core` для допоміжних функцій входу каналу.
...
- Якщо поведінка не має публічного SDK, залиште допоміжну функцію у своєму пакеті або
  запитайте публічний API OpenClaw.
...
Plugin досі використовує застарілу допоміжну функцію всього session store
`loadSessionStore`.

- Використовуйте `getSessionEntry(...)` або `listSessionEntries(...)` під час читання стану сеансу.
- Використовуйте `patchSessionEntry(...)` або `upsertSessionEntry(...)` під час запису стану сеансу.
- Уникайте завантаження, змінювання та збереження всього об’єкта session store.
...
Plugin досі використовує застарілу допоміжну функцію запису всього session store, як-от
`saveSessionStore` або `updateSessionStore`.

- Використовуйте `patchSessionEntry(...)` під час оновлення полів наявного запису сеансу.
- Використовуйте `upsertSessionEntry(...)` під час заміни або створення запису сеансу.
- Уникайте завантаження, змінювання та збереження всього об’єкта session store.
- Залишайте допоміжні функції запису всього store лише доки ваш оголошений діапазон сумісності
...
Plugin досі використовує застарілі допоміжні функції шляхів до файлів сеансу, як-от
`resolveSessionFilePath` або `resolveAndPersistSessionFile`.

- Використовуйте `getSessionEntry(...)`, щоб читати метадані сеансу за агентом і ідентичністю сеансу.
- Використовуйте `patchSessionEntry(...)` або `upsertSessionEntry(...)`, щоб зберігати метадані сеансу.
- Використовуйте ідентичність transcript або допоміжні функції target, коли код готує
  операцію transcript.
...
- Уникайте прямого читання або створення застарілих file targets transcript.
...
- Перемістіть роботу з перевизначення model або provider до `before_model_resolve`.
- Перемістіть роботу зі зміни prompt або context до `before_prompt_build`.
...
- Залишайте `providerAuthEnvVars` лише як метадані сумісності, доки ваш підтримуваний
  діапазон OpenClaw ще цього потребує.
...
- Зберігайте метадані env-var каналу декларативними, щоб OpenClaw міг перевіряти стан setup
  без завантаження runtime каналу.
- Віддзеркальте налаштування каналу, кероване env, у поточні метадані setup, channel config або
  package channel, які використовує форма вашого plugin.
- Залишайте `channelEnvVars` лише як метадані сумісності, доки старіші підтримувані
  версії OpenClaw ще цього потребують.
- Див. [Маніфест Plugin](/uk/plugins/manifest) і
  [Channel plugins](/uk/plugins/sdk-channel-plugins).
- Повторно запустіть `clawhub package validate <path-to-plugin>`.
