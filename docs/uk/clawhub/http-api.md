...
Застарілі `/api/...` та `/api/cli/...` залишаються для сумісності (див. `DEPRECATIONS.md`).
OpenAPI: `/api/v1/openapi.json`.

## Повторне використання публічного каталогу

Сторонні каталоги можуть використовувати публічні кінцеві точки читання, щоб перелічувати або шукати ClawHub Skills. Кешуйте результати, дотримуйтеся `429`/`Retry-After`, посилайте користувачів назад на канонічну сторінку ClawHub (`https://clawhub.ai/<owner>/skills/<slug>`) і не створюйте враження, що ClawHub схвалює сторонній сайт. Не намагайтеся дзеркалити прихований, приватний або заблокований модерацією вміст поза поверхнею публічного API.
...
- Автентифіковані кінцеві точки запису не повинні повертати голе `Unauthorized`, коли
  ...
  видалені/забанені/вимкнені облікові записи мають кожен отримувати дієвий текст, щоб CLI
...
- Якщо довірений IP клієнта недоступний, анонімні запити використовують резервні кошики,
  обмежені лише типом обмеження швидкості. Ці резервні кошики не включають
  надані викликачем шляхи, slugs, назви пакетів, версії, рядки запиту або інші
  параметри артефактів.

## Відповіді з помилками

Публічні відповіді з помилками v1 є простим текстом з `content-type: text/plain; charset=utf-8`.
...
- Результати повертаються в порядку релевантності (схожість embedding + підсилення точних токенів slug/назви + невеликий пріор популярності).
- Релевантність сильніша за популярність. Точний збіг токена slug або відображуваної назви може випередити менш точний збіг із значно сильнішою взаємодією.
...
- Популярність масштабується логарифмічно й обмежується. Skills з високою взаємодією можуть ранжуватися нижче, коли текст запиту має слабший збіг.
- Підозрілий або прихований стан модерації може вилучити Skill з публічного пошуку залежно від фільтрів викликача та поточного статусу модерації.

Настанови щодо доступності для видавців:

- Розміщуйте терміни, які користувачі буквально шукатимуть, у відображуваній назві, зведенні та тегах. Використовуйте окремий токен slug лише тоді, коли це також стабільна ідентичність, яку ви хочете зберегти.
- Не перейменовуйте slug лише для того, щоб наздогнати один запит, якщо новий slug не є кращою довгостроковою канонічною назвою. Старі slugs стають псевдонімами переспрямування, але канонічна URL-адреса, відображуваний slug і майбутні пошукові дайджести використовують новий slug.
- Псевдоніми перейменування зберігають розв’язання для старих URL-адрес та встановлень, що розв’язуються через реєстр, але пошукове ранжування базується на канонічних метаданих Skill після індексації перейменування. Наявна статистика залишається зі Skill.
...
- Старі slugs, створені потоками перейменування/об’єднання власника, розв’язуються до канонічного Skill.
...
- `moderation` включається лише тоді, коли Skill позначений або власник його переглядає.
...
Повідомити про Skill для перевірки модератором. Звіти мають рівень Skill, необов’язково пов’язані
з версією, і надходять у чергу звітів про Skills.
...
встановлення `status` назад на `open`. Передайте `finalAction: "hide"` з розглянутим
...
- `security.hasScanResult` дорівнює `true` лише тоді, коли сканер сформував остаточний вердикт (`clean`, `suspicious` або `malicious`).
- `moderation` — це поточний знімок модерації на рівні навички, отриманий із найновішої версії.
- Під час запиту історичної версії перевіряйте `moderation.matchesRequestedVersion` і `moderation.sourceVersion`, перш ніж трактувати `moderation` і `security` як один і той самий контекст версії.
...
- Потребує управлінського доступу власника/видавця до навички або плагіна, або повноважень модератора/адміністратора платформи.
...
- Повертає таку саму структуру ZIP, як і завантаження запитів на сканування.

### `POST /api/v1/skills/-/scan/batch`

Канонічний маршрут пакетного повторного сканування лише для адміністраторів. Він приймає таку саму форму корисного навантаження, як і застарілий `POST /api/v1/skills/-/rescan-batch`.

### `POST /api/v1/skills/-/scan/batch/status`

Канонічний маршрут статусу пакетного сканування лише для адміністраторів. Він приймає `{ "jobIds": ["..."] }` і повертає ті самі агреговані лічильники, що й застарілий `POST /api/v1/skills/-/rescan-batch/status`.
...
- `ok` дорівнює `true` лише тоді, коли вибрана версія має згенеровану картку навички, не заблокована модерацією як шкідливе ПЗ і перевірка ClawScan є чистою.
...
- `provenance` має значення `server-resolved-github-import` лише тоді, коли ClawHub визначив і зберіг repo/ref/commit/path GitHub під час публікації або імпорту; інакше воно має значення `unavailable`.
...
- Використовуйте `/verify`, коли потрібна оболонка перевірки картки однієї навички, `/card`, коли потрібен згенерований Markdown картки, і `/scan`, коли потрібні докладні дані сканерів.
...
- `category` (необов’язковий): фільтр категорії plugin. Підтримується лише коли
  ...
  `/api/v1/code-plugins`, `/api/v1/bundle-plugins` або кінцевими точками пакетів з
...
- Записи skills і надалі спираються на реєстр skills і все ще можуть публікуватися лише через `POST /api/v1/skills`.
...
`trending` — це таблиця лідерів установлень/завантажень за сім днів, яка не використовує загальні підсумки за весь час.
...
- Розміщені skills включають файли останньої збереженої версії та перелічені в
  `_manifest.json` із `sourceRef: "public-clawhub"`.
- Поточні skills на основі GitHub зі скануванням `clean` або `suspicious` включають
  `_source_handoff.json` із `sourceRef: "public-github"`, репозиторієм, комітом, шляхом,
  хешем вмісту та URL архіву. Вони не включають вихідні файли, розміщені в ClawHub.
- Кожен skill включає `_export_skill_meta.json`.
...
- `limit` (необов’язковий): ціле число (1-250), стандартне значення `250`.
- `cursor` (необов’язковий): курсор пагінації з попередньої відповіді.
- `family` (необов’язковий): `code-plugin` або `bundle-plugin`. Якщо опущено, означає обидві
  родини Plugin.
...
- `trust.moderationState` може бути null. Він має значення `null`, коли немає ручної
  модерації релізу.
...
- `trust.reasons` — список пояснень для користувача й аудиту. Коди причин
  ...
  і `package:malicious`.
- `trust.pending` означає, що один або кілька вхідних параметрів довіри все ще очікують завершення.
- `trust.stale` означає, що підсумок довіри було обчислено із застарілих вхідних даних і
  його слід вважати таким, що потребує оновлення перед рішенням дозволити з високою впевненістю.
...
- Ця кінцева точка навмисно вужча за кінцеві точки модерації власника/модератора.
  ...
  особи репортерів, тексти звітів, приватні докази або внутрішні часові шкали
  перевірки.
...
Повертає обчислену готовність до майбутнього споживання OpenClaw.

Перевірки готовності охоплюють:

- офіційний статус каналу
...
Кінцева точка модератора для перелічення рядків міграції офіційних Plugin OpenClaw.
...
- `status` (необов’язковий): `open` (стандартне значення), `blocked`, `manual` або `all`
- `limit` (необов’язковий): ціле число (1-100)
- `cursor` (необов’язковий): курсор пагінації
...
пов’язані з версією. Вони потрапляють до черги модерації, але самі по собі не приховують автоматично й не
...
Модераторська/адміністраторська кінцева точка для приймання звітів про пакети.
...
Кінцева точка для власника/модератора, що надає видимість модерації пакета.
...
Модераторська/адміністраторська кінцева точка для вирішення або повторного відкриття звітів про пакети.
...
`note` обов’язкове для `confirmed` і `dismissed`; його можна не вказувати під час
...
Модераторська/адміністраторська кінцева точка для перевірки релізу пакета.
...
- Використовує бакет ліміту читання, а не бакет завантаження.
...
Повертає сумісний з npm packument для версій пакетів на базі ClawPack.

Примітки:

- У списку наведено лише версії із завантаженими tarball-архівами ClawPack npm-pack.
...
- Packument scoped-пакетів підтримують як `/api/npm/@scope/name`, так і
  закодований шлях запиту npm `/api/npm/@scope%2Fname`.

### `GET /api/npm/{package}/-/{tarball}.tgz`

Транслює точні байти завантаженого tarball-архіву ClawPack для клієнтів дзеркала npm.

Примітки:

- Використовує бакет ліміту завантаження.
- Заголовки завантаження містять SHA-256 ClawHub, а також метадані npm integrity/shasum.
- Перевірки модерації та доступу до приватного пакета все одно застосовуються.
...
Завантажує ZIP розміщеної версії навички або повертає передачу на джерело GitHub для
поточної навички на базі GitHub зі скануванням `clean` або `suspicious` і без розміщеної
версії.
...
- Передачі навичок на базі GitHub не проксіюють і не дзеркалять байти. JSON-відповідь
  містить `sourceRef: "public-github"`, `repo`, `commit`, `path`, `contentHash`
  і `archiveUrl`; стан сканування/актуальності є gate і не включається як метадані
  успішного payload.
...
- Дозволені поля форми: `payload`, повторювані blob-файли `files` або одне посилання на tarball
  `clawpack`. `clawpack` може бути blob `.tgz` або storage id, повернений
...
- JSON-тіла й надані викликачем метадані `payload.files` / `payload.artifact`
  відхиляються.
- Прямі multipart-запити публікації обмежено 18MB. Tarball-архіви ClawPack можуть
...
- Лише видавець організації `openclaw` і персональні видавці поточних учасників організації `openclaw`
  можуть публікувати в канал `official`.
- Публікації від імені іншого все одно перевіряють право на official-канал щодо цільового облікового запису власника.

### `DELETE /api/v1/skills/{slug}` / `POST /api/v1/skills/{slug}/undelete`

Soft-delete / відновлення навички (власник, модератор або адміністратор).
...
Якщо `reason` присутнє, воно зберігається як примітка модерації навички й копіюється в журнал аудиту.
Soft-delete, ініційований власником, резервує slug на 30 днів, після чого slug може отримати
інший видавець. Відповідь видалення містить `slugReservedUntil`, коли застосовується цей строк.
Приховування модератором/адміністратором і вилучення з міркувань безпеки не спливають таким чином.
...
- Повертає `409`, коли handle уже використовується видавцем, користувачем або персональним видавцем.
...
без редагування рядків облікового запису Convex Auth. Запит має називати обидва незмінні
ідентифікатори облікових записів провайдера GitHub; змінні handle використовуються лише як захисна перевірка для оператора.

Кінцева точка за замовчуванням працює як пробний запуск. Для застосування відновлення потрібні `dryRun: false` і
...
шляхи безпосереднього власника узгоджувалися з новими повноваженнями видавця. Активне резервування захищеного handle
...
Джерела навичок GitHub мають область дії видавця і повідомляються як перевірені, а не переписуються.
