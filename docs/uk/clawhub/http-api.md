...
x-i18n:
    generated_at: "2026-07-04T06:48:00Z"
    model: gpt-5.5
...
Сторонні каталоги можуть використовувати публічні кінцеві точки читання, щоб перелічувати або шукати Skills ClawHub. Кешуйте результати, дотримуйтеся `429`/`Retry-After`, посилайте користувачів назад на канонічний запис ClawHub (`https://clawhub.ai/<owner>/skills/<slug>`) і не створюйте враження, що ClawHub схвалює сторонній сайт. Не намагайтеся дзеркалити прихований, приватний або заблокований модерацією вміст поза публічною поверхнею API.

Скорочення веб-slug розв’язуються між сімействами реєстру, але API-клієнти мають використовувати
канонічні URL-адреси, повернені кінцевими точками читання, замість відтворення пріоритету
маршрутів.
...
- Використовує довірені заголовки IP клієнта, зокрема `cf-connecting-ip`, лише коли
  розгортання явно вмикає довірені перенаправлені заголовки.
- ClawHub використовує довірені заголовки перенаправлення для визначення IP клієнтів на edge.
- Якщо довірений IP клієнта недоступний, анонімні запити використовують резервні кошики,
  ...
  надані викликачем шляхи, slugs, назви пакетів, версії, рядки запитів або інші
  параметри артефактів.

## Відповіді з помилками

Публічні відповіді v1 з помилками є простим текстом із `content-type: text/plain; charset=utf-8`.
Це включає помилки валідації (`400`), відсутні публічні ресурси (`404`), збої автентифікації та
...
- Результати повертаються в порядку релевантності (схожість embedding + підсилення точних токенів slug/назви + невеликий апріор популярності).
- Релевантність сильніша за популярність. Точний збіг токена slug або відображуваної назви може випередити менш точний збіг зі значно сильнішою взаємодією.
...
- Додайте терміни, які користувачі буквально шукатимуть, у відображувану назву, summary і теги. Використовуйте окремий токен slug лише тоді, коли це також стабільна ідентичність, яку ви хочете зберегти.
- Не перейменовуйте slug лише для того, щоб наздогнати один запит, якщо новий slug не є кращою довгостроковою канонічною назвою. Старі slugs стають псевдонімами перенаправлення, але канонічна URL-адреса, показаний slug і майбутні пошукові дайджести використовують новий slug.
...
- `moderation` включається лише тоді, коли skill позначено прапорцем або його переглядає власник.
...
з версією, і надходять до черги звітів про skill.
...
- Містить нормалізований статус перевірки та специфічні для сканера відомості.
...
- Під час запиту історичної версії перевірте `moderation.matchesRequestedVersion` і `moderation.sourceVersion`, перш ніж вважати `moderation` і `security` одним і тим самим контекстом версії.
...
- Корисні дані запитів сканування та завантажувані звіти видаляються зі сховища запитів сканування після періоду зберігання.
...
- Опубліковані сканування записуються назад лише коли `update: true` і сканування завершується успішно.
...
- Потребує доступу до керування власника/видавця навички або плагіна, або повноважень модератора/адміністратора платформи.
...
- `security` — це верхньорівневий вердикт ClawScan/безпеки. Автоматизація має орієнтуватися на `ok`, `decision`, `reasons` і `security.status`.
- `security.signals` містить допоміжні докази сканерів, як-от `staticScan`, `virusTotal` і `skillSpector`.
- `security.signals.dependencyRegistry` збережено для сумісності відповіді v1, але сканер наявності реєстру залежностей виведено з експлуатації, і цей ключ завжди `null`.
- `provenance` має значення `server-resolved-github-import` лише тоді, коли ClawHub під час публікації або імпорту визначив і зберіг repo/ref/commit/path GitHub; інакше це `unavailable`.
...
- Результати подаються для кожного елемента; одна відсутня навичка або версія не призводить до помилки всієї відповіді.
- Відповідь містить лише безпекові дані. Вона не містить даних картки навички, статусу згенерованої картки, списків файлів артефактів або докладних корисних даних сканерів.
- `security.signals` містить лише допоміжні докази рівня статусу; для повних відомостей сканерів використовуйте `/scan` або сторінку security-audit ClawHub.
- `security.signals.dependencyRegistry` збережено для сумісності відповіді v1, але сканер наявності реєстру залежностей виведено з експлуатації, і цей ключ завжди `null`.
...
- `category` (необов’язково): фільтр категорії plugin. Підтримується лише тоді, коли
  ...
  `/api/v1/code-plugins`, `/api/v1/bundle-plugins` або кінцеві точки пакетів із
...
- Записи Skills і надалі підтримуються реєстром Skills, і їх досі можна публікувати лише через `POST /api/v1/skills`.
...
Перегляд каталогу лише для plugin серед пакетів code-plugin і bundle-plugin.
...
`trending` — це семиденна таблиця лідерів за встановленнями/завантаженнями, яка не використовує сумарні показники за весь час.
На уніфікованій кінцевій точці `/api/v1/packages` вона стосується лише plugin; використовуйте
...
- `startDate` (обов’язково): нижня межа в мілісекундах Unix для `updatedAt` Skills.
- `endDate` (обов’язково): верхня межа в мілісекундах Unix для `updatedAt` Skills.
...
- Поточні Skills на базі GitHub зі скануванням `clean` або `suspicious` включають
  `_source_handoff.json` із `sourceRef: "public-github"`, репозиторієм, комітом, шляхом,
  хешем вмісту й URL архіву. Вони не включають вихідні файли, розміщені на ClawHub.
...
Пошук лише за plugin серед пакетів code-plugin і bundle-plugin.
...
- Фільтрація за категоріями є справжнім API-фільтром, який спирається на рядки дайджесту категорій plugin,
  а не переписуванням пошукового запиту.
- Результати повертаються в порядку релевантності та наразі не мають пагінації.
...
- `version.sha256hash` — це застарілі метадані сумісності для старих клієнтів. Вони
  хешують точні байти ZIP, які повертає `/api/v1/packages/{name}/download`.
...
Повертає точний підсумок безпеки та довіри релізу пакета для клієнтів встановлення.
Це публічна поверхня споживання OpenClaw для ухвалення рішення, чи можна
...
Повідомляє про пакет для модераторської перевірки. Звіти мають рівень пакета, необов’язково
пов’язані з версією. Вони потрапляють у чергу модерації, але самі по собі не приховують автоматично й
...
самому аудиторському робочому процесі.
...
- Використовує кошик обмеження швидкості читання, а не кошик завантажень.
...
- Packument пакетів зі scope підтримують як `/api/npm/@scope/name`, так і закодований
...
- Передачі skill на основі GitHub не проксіюють і не дзеркалять байти. JSON-відповідь
  ...
  і `archiveUrl`; стан сканування/актуальності є шлюзом і не включається як метадані успішного
  payload.
...
- Необов’язкове поле payload: `migrateOwner`. Коли `true` разом із `ownerHandle`, наявний
...
- Дозволені поля форми: `payload`, повторювані blob `files` або одне посилання на tarball `clawpack`.
  `clawpack` може бути blob `.tgz` або storage id, поверненим
  flow upload-url. Публікації зі staged storage-id також мають містити
...
- Code plugins вимагають `package.json`, метадані репозиторію джерела, метадані коміту джерела,
...
- Публікації від імені іншого все ще перевіряють право на official-channel щодо облікового запису цільового власника.
...
Якщо присутнє, `reason` зберігається як примітка модерації skill і копіюється в журнал аудиту.
Soft-delete, ініційований власником, резервує slug на 30 днів, після чого slug може бути заявлений
іншим видавцем. Відповідь delete містить `slugReservedUntil`, коли застосовується це завершення строку.
Приховування модератором/адміністратором і вилучення з міркувань безпеки не завершуються таким чином.
...
Лише для адміністратора. Гарантує, що для handle існує видавець org. Якщо handle все ще вказує на
застарілого спільного користувача/особистого видавця, ендпойнт спочатку мігрує його у видавця org.
...
Автентифіковане self-serve створення видавця org. Створює нового видавця org і додає
викликача як власника. Цей ендпойнт не мігрує наявні user/personal handles і не
...
без редагування рядків облікового запису Convex Auth. Запит має назвати обидва незмінні
ідентифікатори облікового запису провайдера GitHub; змінні handles використовуються лише як захист для оператора.

Кінцева точка за замовчуванням працює в режимі dry-run. Застосування відновлення потребує `dryRun: false` і
...
шляхи прямого власника узгоджувалися з новою владою видавця. Активне резервування захищеного handle
для відновленого handle також перепризначається користувачу-заміни, щоб подальша
синхронізація профілю не могла відновити конкуруючу владу колишнього користувача. Кожна основна таблиця обмежена
100 рядками на транзакцію застосування; більші відновлення мають спершу використати відновлювану міграцію власника.
Джерела GitHub Skills мають область видавця й повідомляються як перевірені, а не переписані.
...
- `merge` приховує вихідний список і переспрямовує вихідний slug на цільовий список.
...
Забанити користувача й остаточно видалити Skills, якими він володіє (лише moderator/admin).
...
вмісту (лише admin). За замовчуванням працює в режимі dry-run, якщо `dryRun` не дорівнює `false`.
...
Додати/видалити зірку (highlights). Обидві кінцеві точки ідемпотентні.
...
які готують tarball ClawPack на цьому етапі, мають надсилати отриманий storage id як
