...
x-i18n:
    generated_at: "2026-07-01T08:27:41Z"
    model: gpt-5.5
...
Сторонні каталоги можуть використовувати публічні кінцеві точки читання, щоб показувати список або виконувати пошук ClawHub Skills. Кешуйте результати, дотримуйтеся `429`/`Retry-After`, повертайте користувачів посиланням до канонічного списку ClawHub (`https://clawhub.ai/<owner>/skills/<slug>`) і не створюйте враження, що ClawHub схвалює сторонній сайт. Не намагайтеся дзеркалити прихований, приватний або заблокований модерацією вміст поза публічною поверхнею API.

Веб-скорочення slug розв’язуються між сімействами реєстру, але клієнтам API слід використовувати
...
- ClawHub використовує довірені заголовки пересилання, щоб ідентифікувати IP клієнта на edge.
- Якщо довірена IP-адреса клієнта недоступна, анонімні запити використовують резервні кошики,
  ...
  надані викликачем шляхи, slugs, назви пакетів, версії, рядки запиту чи інші
...
- Результати повертаються в порядку релевантності (подібність embedding + підсилення точного токена slug/назви + невеликий попередній показник популярності).
- Релевантність сильніша за популярність. Точний збіг токена slug або display-name може випередити вільніший збіг із набагато сильнішою взаємодією.
...
- Розміщуйте терміни, які користувачі буквально шукатимуть, у відображуваній назві, короткому описі та тегах. Використовуйте самостійний токен slug лише тоді, коли це також стабільна ідентичність, яку ви хочете зберегти.
...
- `moderation` включається лише коли Skill позначено або власник його переглядає.
...
- Публічні викликачи отримують `200` лише для вже позначених видимих Skills.
- Докази редагуються для публічних викликачів і містять необроблені фрагменти лише для власників/модераторів.

### `POST /api/v1/skills/{slug}/report`

Повідомити про Skill для перевірки модератором. Звіти діють на рівні Skill, необов’язково пов’язані
...
Кінцева точка модератора/адміністратора для приймання звітів про Skill.
...
Кінцева точка модератора/адміністратора для вирішення або повторного відкриття звітів про Skill.
...
- Повертає статус у черзі/виконується/успішно/не вдалося.
- Повертає `queue.queuedAhead` і `queue.position`, поки запит перебуває в черзі, щоб клієнти могли показати, скільки пріоритетних ручних сканувань стоїть перед цим запитом. Дуже великі черги обмежуються та повідомляються з `queuedAheadIsEstimate: true`.
...
- Ідентичність навички, ідентичність видавця та метадані вибраної версії є полями конверта верхнього рівня (`slug`, `displayName`, `publisherHandle`, `version`, `resolvedFrom`, `tag`, `createdAt`), щоб автоматизація оболонки могла читати їх без розпаковування вкладених обгорток.
...
- `provenance` має значення `server-resolved-github-import` лише тоді, коли ClawHub визначив і зберіг репозиторій/ref/commit/path GitHub під час публікації або імпорту; інакше має значення `unavailable`.
...
- Відповідь стосується лише безпеки. Вона не містить даних Skill Card, статусу згенерованої картки, списків файлів артефактів або докладних корисних навантажень сканерів.
...
Уніфікований каталоговий endpoint для:
...
- `category` (необов’язковий): фільтр категорії плагінів. Підтримується лише тоді, коли
  ...
  `/api/v1/code-plugins`, `/api/v1/bundle-plugins` або endpoints пакетів із
...
Застарілі псевдоніми фільтрів v1 і надалі приймаються на endpoints читання:
...
На уніфікованому endpoint `/api/v1/packages` він стосується лише плагінів; використовуйте
...
- `_manifest.json` завжди додається в корінь ZIP.
- `_errors.json` додається, коли окремі Skills або файли не вдалося
  експортувати.
...
- `_errors.json` включається, коли окремі плагіни або файли не вдалося
  експортувати.
...
- `version.artifact.kind` має значення `legacy-zip` для старих архівів пакетів або
...
- Ця кінцева точка є точною для версії. Клієнтам слід викликати її після розв’язання
...
- Ця кінцева точка навмисно вужча за кінцеві точки модерації для власників/модераторів.
  ...
  ідентичності авторів звітів, тіла звітів, приватні докази або внутрішні графіки
  перевірки.
...
- Це поверхня розв’язувача OpenClaw; вона уникає вгадування формату архіву за
...
- `open`: підозрілі, шкідливі, очікувані, поміщені в карантин, відкликані релізи або релізи зі звітами.
- `blocked`: релізи в карантині, відкликані або шкідливі.
...
пов’язуються з версією. Вони потрапляють до черги модерації, але самі по собі не приховують автоматично
та не блокують завантаження; модераторам слід використовувати модерацію релізів, щоб
...
`note` є обов’язковим для `confirmed` і `dismissed`; його можна пропустити, коли
`status` встановлюється назад на `open`. Передайте `finalAction: "quarantine"` або
...
Для релізів у стані карантину або відкликання маршрути завантаження артефактів повертають `403`.
...
- Використовує бакет обмеження швидкості читання, а не бакет завантажень.
...
- Packument scoped-пакетів підтримують і `/api/npm/@scope/name`, і npm-шлях запиту
...
- Використовує бакет обмеження швидкості завантаження.
...
Завантажує ZIP розміщеної версії skill або повертає передачу на джерело GitHub для
...
- Передачі skill на основі GitHub не проксіюють і не дзеркалять байти. JSON-відповідь
  ...
  і `archiveUrl`; стан сканування/поточний стан є умовою доступу й не включається як метадані
  успішного payload.
...
- Необов’язкове поле payload: `migrateOwner`. Коли `true` з `ownerHandle`,
  наявний skill може перейти до цього власника, якщо актор є адміністратором/власником у обох
  видавців: поточного й цільового. Без цієї явної згоди зміни власника
  відхиляються.
...
- Дозволені поля форми: `payload`, повторювані blob-файли `files` або одне посилання на tarball `clawpack`.
  `clawpack` може бути blob `.tgz` або storage id, повернутим потоком upload-url.
  Публікації зі staged storage-id також мають містити `clawpackUploadTicket`,
  повернутий разом із цим URL завантаження.
...
- JSON-тіла та надані викликачем метадані `payload.files` / `payload.artifact`
  відхиляються.
- Прямі multipart-запити на публікацію обмежено 18 МБ. Tarball-файли ClawPack можуть
...
- Публікації від імені когось усе ще перевіряють право на канал official щодо цільового облікового запису власника.
...
Коли `reason` присутній, він зберігається як нотатка модерації skill і копіюється в журнал аудиту.
М’які видалення, ініційовані власником, резервують slug на 30 днів, після чого slug може бути заявлений
іншим видавцем. Відповідь видалення містить `slugReservedUntil`, коли застосовується цей строк завершення.
...
Для новоствореного org надайте `memberHandle`; чинний адміністратор не додається як учасник.
...
GitHub principals. Відновлення завершується із закритою відмовою, якщо поточний особистий
publisher цільового користувача має skills, packages або джерела GitHub skills.
Відновлення також мігрує застарілі поля `ownerUserId` для skills відновленого publisher,
псевдонімів slug skills, packages, попереджень інспектора packages і похідних рядків пошукового дайджесту, щоб
шляхи прямого власника узгоджувалися з новою владою publisher. Активне резервування protected-handle
для відновленого handle також перепризначається користувачу-заміні, щоб подальша
синхронізація профілю не могла відновити конкурувальну владу попереднього користувача. Кожна основна таблиця обмежена
100 рядками на транзакцію застосування; більші відновлення мають спочатку використати відновлювану міграцію власника.
Джерела GitHub skills мають область дії publisher і звітуються як перевірені, а не переписуються.
...
- `merge` приховує вихідний список і перенаправляє вихідний slug на цільовий список.
...
Змінити збережену причину наявного бану без розбанення чи відновлення
...
CLI може виявляти налаштування registry/auth із сайту:
...
Якщо ви розміщуєте самостійно, надайте цей файл (або явно задайте `CLAWHUB_REGISTRY`; застаріле `CLAWDHUB_REGISTRY`).
