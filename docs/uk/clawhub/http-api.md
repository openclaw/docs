...
Сторонні каталоги можуть використовувати публічні кінцеві точки читання, щоб показувати або шукати ClawHub Skills. Кешуйте результати, дотримуйтеся `429`/`Retry-After`, спрямовуйте користувачів назад до канонічного запису ClawHub (`https://clawhub.ai/<owner>/skills/<slug>`) і не створюйте враження, що ClawHub схвалює сторонній сайт. Не намагайтеся дзеркалювати прихований, приватний або заблокований модерацією вміст поза публічною поверхнею API.

Скорочення веб-слагів розв’язуються між сімействами реєстру, але клієнти API мають використовувати канонічні URL-адреси, які повертають кінцеві точки читання, замість реконструювання пріоритету маршрутів.
...
- Автентифіковані запити (дійсний Bearer-токен): застосовуються для кожного користувацького бакета.
- Якщо токен відсутній або недійсний, поведінка повертається до застосування за IP-адресою.
- Автентифіковані кінцеві точки запису не мають повертати лише `Unauthorized`, коли сервер знає причину. Відсутні токени, недійсні/відкликані токени та видалені/забанені/вимкнені облікові записи мають отримувати дієвий текст, щоб клієнти CLI могли повідомити користувачам, що саме їх заблокувало.
...
- Використовує довірені заголовки IP-адреси клієнта, зокрема `cf-connecting-ip`, лише коли розгортання явно вмикає довірені переспрямовані заголовки.
- ClawHub використовує довірені заголовки переспрямування для визначення IP-адрес клієнтів на edge.
- Якщо довірена IP-адреса клієнта недоступна, анонімні запити використовують резервні бакети, обмежені лише видом ліміту швидкості. Ці резервні бакети не містять наданих викликачем шляхів, слагів, назв пакетів, версій, рядків запиту або інших параметрів артефактів.

## Відповіді з помилками

Публічні відповіді з помилками v1 є звичайним текстом із `content-type: text/plain; charset=utf-8`.
Сюди входять помилки валідації (`400`), відсутні публічні ресурси (`404`), помилки автентифікації та дозволів (`401`/`403`), ліміти швидкості (`429`) і заблоковані завантаження. Клієнти мають читати тіло відповіді як зрозумілий людині рядок. Невідомі параметри запиту ігноруються для сумісності, але розпізнані параметри запиту з недійсними значеннями повертають `400`.
...
- Результати повертаються в порядку релевантності (подібність embedding + підсилення за точними токенами слага/назви + невеликий попередній показник популярності).
...
- Популярність логарифмічно масштабується і має верхню межу. Skills із високою залученістю можуть ранжуватися нижче, коли текст запиту має слабший збіг.
...
- Не перейменовуйте слаг лише для того, щоб підлаштуватися під один запит, якщо новий слаг не є кращою довгостроковою канонічною назвою. Старі слаги стають псевдонімами перенаправлення, але канонічна URL-адреса, показаний слаг і майбутні пошукові дайджести використовують новий слаг.
...
- `sort` (необов’язково): `updated` (типово), `recommended` (псевдонім: `default`), `createdAt` (псевдонім: `newest`), `downloads`, `stars` (псевдонім: `rating`), застарілі псевдоніми встановлень `installsCurrent`/`installs`/`installsAllTime` відображаються на `downloads`, `trending`
- `nonSuspiciousOnly` (необов’язково): `true`, щоб приховати підозрілі (`flagged.suspicious`) Skills
- `nonSuspicious` (необов’язково): застарілий псевдонім для `nonSuspiciousOnly`
...
Повідомити про Skill для перевірки модератором. Повідомлення стосуються рівня Skill, можуть бути пов’язані з версією та потрапляють до черги повідомлень про Skills.
...
`note` обов’язковий для `confirmed` і `dismissed`; його можна пропустити, коли `status` повертається до `open`. Передайте `finalAction: "hide"` з тріажованим повідомленням, щоб приховати Skill у тому самому аудитовному робочому процесі.
...
- Корисні навантаження запитів сканування та звіти для завантаження видаляються зі сховища scan-request після завершення періоду зберігання.
...
- Завдання сканування асинхронні. Ручні запити сканування мають вищий пріоритет за звичайну роботу публікації/зворотного заповнення, але завершення все одно залежить від доступності воркерів.
...
- Повертає таку саму структуру ZIP, як і завантаження scan-request.

### `POST /api/v1/skills/-/scan/batch`

Канонічний маршрут пакетного повторного сканування лише для адміністраторів. Він приймає таку саму структуру корисного навантаження, як застарілий `POST /api/v1/skills/-/rescan-batch`.

### `POST /api/v1/skills/-/scan/batch/status`

Канонічний маршрут пакетного статусу лише для адміністраторів. Він приймає `{ "jobIds": ["..."] }` і повертає ті самі агреговані лічильники, що й застарілий `POST /api/v1/skills/-/rescan-batch/status`.
...
- `ok` має значення `true` лише тоді, коли вибрана версія має згенеровану Skill Card, не заблокована модерацією як шкідливе ПЗ і перевірка ClawScan є чистою.
...
- `provenance` має значення `server-resolved-github-import` лише тоді, коли ClawHub розв'язав і зберіг repo/ref/commit/path GitHub під час публікації або імпорту; інакше значення — `unavailable`.
...
- Відповідь стосується лише безпеки. Вона не містить даних Skill Card, статусу згенерованої картки, списків файлів артефактів або докладних корисних навантажень сканера.
- `security.signals` містить лише допоміжні докази рівня статусу; використовуйте `/scan` або сторінку security-audit ClawHub для повних подробиць сканера.
- `security.signals.dependencyRegistry` збережено для сумісності відповіді v1, але сканер існування реєстру залежностей вилучено, і цей ключ завжди має значення `null`.
...
- Записи Skills і надалі базуються на реєстрі Skills і все ще можуть публікуватися лише через `POST /api/v1/skills`.
...
`trending` — це семиденна таблиця лідерів установлень/завантажень і вона не використовує підсумки за весь час.
На уніфікованій кінцевій точці `/api/v1/packages` вона призначена лише для plugin; використовуйте
...
- Розміщені Skills включають файли найновішої збереженої версії та перелічені в
...
- Поточні Skills на базі GitHub зі скануванням `clean` або `suspicious` включають
  ...
  хешем вмісту та URL архіву. Вони не включають вихідні файли, розміщені на ClawHub.
- Кожен Skill включає `_export_skill_meta.json`.
- `_manifest.json` завжди включається в корені ZIP.
...
Масовий експорт останніх публічних релізів плагінів для офлайн-аналізу.
...
- Кожен експортований плагін містить збережені файли останнього релізу.
...
перевіркою, метаданими артефакту й даними сканування.
...
- Публічна кінцева точка читання. Токен власника, видавця, модератора або адміністратора не
  потрібен.
...
Повідомляє про пакет для модераторського перегляду. Звіти мають рівень пакета й можуть необов’язково
бути пов’язані з версією. Вони подаються до черги модерації, але самі по собі не приховують автоматично й не
...
самому придатному для аудиту workflow.
...
- Метадані лише з registry не впроваджуються в завантажений архів.
...
- Packument для scoped-пакетів підтримує і `/api/npm/@scope/name`, і закодований npm
...
- `slug` (обов’язково)
- `hash` (обов’язково): 64-символьний hex sha256 fingerprint bundle
...
Завантажує hosted ZIP версії skill або повертає handoff джерела GitHub для
поточного skill на основі GitHub зі скануванням `clean` або `suspicious` і без hosted
версії.
...
- Handoff для skill на основі GitHub не проксіює і не дзеркалить байти. JSON-відповідь
  містить `sourceRef: "public-github"`, `repo`, `commit`, `path`, `contentHash`
  і `archiveUrl`; стан сканування/current є gate і не входить до metadata payload успіху.
- Статистика завантажень рахується як унікальні ідентичності за UTC-день (`userId`, коли API-токен чинний, інакше IP).

## Endpoint-и автентифікації (Bearer token)

Усі endpoint-и потребують:
...
- Необов’язкове поле payload: `ownerHandle`. Якщо воно присутнє, API розв’язує цього
  видавця на server-side і вимагає, щоб actor мав доступ до видавця.
- Необов’язкове поле payload: `migrateOwner`. Коли `true` з `ownerHandle`, наявний
  skill може перейти до цього власника, якщо actor є адміністратором/власником і в поточного,
  і в цільового видавців. Без цього opt-in зміни власника
  відхиляються.
...
- Дозволені поля форми: `payload`, повторювані blob-и `files` або одне посилання tarball
  ...
  `clawpackUploadTicket`, повернений з цим upload URL.
- Використовуйте або `files`, або `clawpack`, ніколи обидва в одному запиті.
- JSON body і надані викликачем metadata `payload.files` / `payload.artifact`
  відхиляються.
- Прямі multipart publish-запити обмежені 18 МБ. Tarball-и ClawPack можуть
  використовувати upload-url flow до ліміту tarball 120 МБ.
...
- Code plugins потребують `package.json`, metadata source repo, metadata source commit,
  metadata config schema, `openclaw.compat.pluginApi` і
  `openclaw.build.openclawVersion`.
- `openclaw.hostTargets` і `openclaw.environment` є необов’язковими metadata.
- Лише видавець org `openclaw` і особисті видавці поточних учасників org `openclaw`
  можуть публікувати в канал `official`.
- Публікації on-behalf усе ще перевіряють eligibility official-channel щодо облікового запису цільового власника.

### `DELETE /api/v1/skills/{slug}` / `POST /api/v1/skills/{slug}/undelete`

Soft-delete / відновити skill (власник, модератор або адміністратор).
...
Якщо `reason` присутній, він зберігається як нотатка модерації skill і копіюється в журнал аудиту.
Soft deletes, ініційовані власником, резервують slug на 30 днів, після чого slug може бути заявлений
іншим видавцем. Відповідь delete містить `slugReservedUntil`, коли застосовується цей строк.
...
Автентифіковане self-serve створення org-видавця. Створює нового org-видавця і додає
викликача як власника. Цей endpoint не мігрує наявні handles користувачів/особисті handles і
...
Лише для адміністратора. Резервує root slugs і назви пакетів для rightful owner без публікації
...
без редагування рядків облікових записів Convex Auth. Запит має називати обидва immutable GitHub
provider account ids; mutable handles використовуються лише як operator-facing guard.

За замовчуванням кінцева точка працює в режимі dry-run. Щоб застосувати відновлення, потрібні `dryRun: false` і
`confirmIdentityVerified: true` після того, як співробітники незалежно перевірять безперервність між обома
GitHub principals. Відновлення завершується безпечною відмовою, якщо поточний особистий
publisher користувача призначення має skills, packages або джерела GitHub skill.
Відновлення також мігрує застарілі поля `ownerUserId` для skills відновленого publisher,
псевдонімів slug skills, packages, попереджень інспектора package і похідних рядків search digest, щоб
шляхи direct-owner узгоджувалися з новими повноваженнями publisher. Активне резервування protected-handle
...
100 рядками на транзакцію застосування; для більших відновлень спершу потрібно використати відновлювану міграцію власника.
Джерела GitHub skill мають область дії publisher і звітуються як перевірені, а не переписуються.
...
які розміщують tarball ClawPack на проміжному етапі, мають надіслати отриманий storage id як
...
Якщо ви self-host, подавайте цей файл (або явно встановіть `CLAWHUB_REGISTRY`; застаріле `CLAWDHUB_REGISTRY`).
