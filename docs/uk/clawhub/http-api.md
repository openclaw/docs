...
x-i18n:
    generated_at: "2026-06-28T00:10:43Z"
    model: gpt-5.5
...
- Використовуйте відступ із випадковим розкидом, щоб уникнути синхронізованих повторних спроб.
- Якщо `Retry-After` відсутній, використовуйте запасний варіант `RateLimit-Reset` (або обчисліть із `X-RateLimit-Reset`).
...
- ClawHub використовує довірені заголовки пересилання, щоб ідентифікувати IP-адреси клієнтів на межі.
- Якщо довірена IP-адреса клієнта недоступна, анонімні запити використовують запасні кошики,
  обмежені лише типом обмеження частоти. Ці запасні кошики не включають
...
- `recommended` використовує сигнали залучення та давності.
...
- Старі слаги, створені потоками перейменування/об’єднання власника, розв’язуються в канонічний skill.
...
Поскаржитися на skill для перевірки модератором. Скарги мають рівень skill, необов’язково пов’язані
з версією, і надходять до черги скарг на skill.
...
Кінцева точка модератора/адміністратора для розв’язання або повторного відкриття скарг на Skills.
...
повернення `status` до `open`. Передайте `finalAction: "hide"` з розібраною
...
- Містить нормалізований статус перевірки та специфічні для сканера подробиці.
...
- Корисні навантаження запитів сканування та звіти, доступні для завантаження, видаляються зі сховища scan-request після завершення періоду зберігання.
- Опубліковані сканування потребують доступу власника/публікатора до керування або повноважень модератора/адміністратора платформи.
- Опубліковані сканування записують результат назад лише коли `update: true` і сканування успішно завершується.
...
- Потребує доступу власника/публікатора до керування навичкою або plugin, або повноважень модератора/адміністратора платформи.
...
- `ok` має значення `true` лише коли вибрана версія має згенеровану Skill Card, не заблокована модерацією як шкідливе ПЗ і перевірка ClawScan чиста.
- Ідентичність навички, ідентичність публікатора та метадані вибраної версії є полями верхнього рівня конверта (`slug`, `displayName`, `publisherHandle`, `version`, `resolvedFrom`, `tag`, `createdAt`), щоб автоматизація оболонки могла читати їх без розпаковування вкладених обгорток.
...
- `security.signals` містить допоміжні докази сканерів, такі як `staticScan`, `virusTotal` і `skillSpector`.
- `security.signals.dependencyRegistry` збережено для сумісності відповіді v1, але сканер існування реєстру залежностей виведено з експлуатації, і цей ключ завжди `null`.
- `provenance` має значення `server-resolved-github-import` лише коли ClawHub розв’язав і зберіг GitHub repo/ref/commit/path під час публікації або імпорту; інакше значення — `unavailable`.
...
- `security.signals` містить лише допоміжні докази на рівні статусу; використовуйте `/scan` або сторінку аудиту безпеки ClawHub для повних подробиць сканерів.
- `security.signals.dependencyRegistry` збережено для сумісності відповіді v1, але сканер існування реєстру залежностей виведено з експлуатації, і цей ключ завжди `null`.
- Відсутність Skill Card не впливає на `ok`, `decision` або `reasons` цієї точки; клієнти мають читати встановлений `skill-card.md` локально, коли їм потрібен вміст картки.
...
- `family` (необов’язковий): `code-plugin` або `bundle-plugin`. Якщо пропущено,
  означає обидві родини plugin.
...
Пошук лише plugins серед пакетів code-plugin і bundle-plugin.
...
- `version.artifact.kind` дорівнює `legacy-zip` для пакетних архівів старого світу або
...
- `version.sha256hash` — застарілі метадані сумісності для старих клієнтів. Вони
  хешують точні ZIP-байти, які повертає `/api/v1/packages/{name}/download`.
...
Повертає точний підсумок безпеки й довіри для релізу пакета для клієнтів
...
Кінцева точка модератора для перелічення рядків міграції офіційних plugins OpenClaw.
...
Повідомляє про пакет для перевірки модератором. Звіти діють на рівні пакета, необов’язково
...
- Packument для пакетів з областю видимості підтримують і `/api/npm/@scope/name`, і npm
...
Завантажує ZIP розміщеної версії skill або повертає передавання на джерело GitHub для
...
- Необов’язкове поле payload: `ownerHandle`. Коли воно присутнє, API розв’язує цього
  видавця на серверному боці та вимагає від актора доступу до видавця.
- Необов’язкове поле payload: `migrateOwner`. Коли `true` з `ownerHandle`, наявна
  ...
  поточного, і в цільового видавця. Без цієї явної згоди зміни власника
  відхиляються.
...
- Дозволені поля форми: `payload`, повторювані blob-об’єкти `files` або одне посилання на tarball
  `clawpack`. `clawpack` може бути blob-об’єктом `.tgz` або storage id, повернутим
  потоком upload-url. Публікації зі staged storage-id також мають містити
  `clawpackUploadTicket`, повернутий із тим URL завантаження.
...
- Публікації від імені іншого користувача все одно перевіряють придатність до official-channel щодо облікового запису цільового власника.
...
М’які видалення, ініційовані власником, резервують slug на 30 днів, після чого slug може забрати
...
provider account ids; змінювані handle використовуються лише як guard, видимий оператору.

За замовчуванням кінцева точка працює в режимі dry-run. Застосування відновлення потребує `dryRun: false` і
...
аліасів slug навичок, пакетів, попереджень інспектора пакетів і похідних рядків пошукового дайджесту, щоб
шляхи прямого власника узгоджувалися з новим повноваженням видавця. Активне захищене
резервування handle для відновленого handle також перепризначається користувачу-заміні, щоб подальша
синхронізація профілю не могла відновити конкуруюче повноваження попереднього користувача. Кожна основна таблиця обмежена
100 рядками на транзакцію застосування; більші відновлення мають спочатку використати відновлювану міграцію власника.
Джерела навичок GitHub мають область дії видавця й повідомляються як перевірені, а не переписані.
...
- `rename` зберігає попередній slug як аліас перенаправлення.
...
- `query` (необов'язково): аліас для `q`
