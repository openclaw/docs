...
Сторонні каталоги можуть використовувати публічні кінцеві точки читання, щоб виводити список або шукати Skills ClawHub. Кешуйте результати, дотримуйтеся `429`/`Retry-After`, спрямовуйте користувачів назад до канонічного списку ClawHub (`https://clawhub.ai/<owner>/<slug>`) і не створюйте враження, що ClawHub схвалює сторонній сайт. Не намагайтеся дзеркалити прихований, приватний або заблокований модерацією вміст поза поверхнею публічного API.

Веб-скорочення slug розв’язуються між родинами реєстру, але клієнти API мають використовувати
...
- Автентифіковані запити (дійсний Bearer-токен): застосовуються за користувацьким кошиком.
- Якщо токен відсутній або недійсний, поведінка повертається до застосування за IP.
- Автентифіковані кінцеві точки запису не мають повертати просте `Unauthorized`, коли
...
- ClawHub використовує довірені заголовки переспрямування, щоб ідентифікувати IP клієнтів на краю мережі.
- Якщо довірений IP клієнта недоступний, анонімні запити завантаження використовують резервний кошик у межах кінцевої точки замість одного глобального кошика `ip:unknown`. Анонімні запити читання/запису й надалі використовують спільний невідомий кошик, щоб маршрутизація без IP залишалася видимою та консервативною.
...
- Результати повертаються в порядку релевантності (подібність embedding + підсилення точних токенів slug/назви + пріор популярності із завантажень).
...
- Не перейменовуйте slug лише заради одного запиту, якщо новий slug не є кращою довгостроковою канонічною назвою. Старі slug стають псевдонімами перенаправлення, але канонічна URL-адреса, показаний slug і майбутні пошукові дайджести використовують новий slug.
...
- Власники та модератори можуть отримувати доступ до деталей модерації прихованих Skills.
...
Повідомити про Skill для перевірки модератором. Звіти належать до рівня Skill, необов’язково пов’язані
з версією та потрапляють до черги звітів про Skill.
...
`status` знову встановлюється на `open`. Передайте `finalAction: "hide"` із тріажованим
...
- `moderation` — це поточний знімок модерації рівня Skill, отриманий з останньої версії.
...
- Записи Skills залишаються підтриманими реєстром Skills і все ще можуть публікуватися лише через `POST /api/v1/skills`.
- `POST /api/v1/packages` як і раніше призначений лише для релізів code-plugin і bundle-plugin.
...
- Потребує API-токен для власника пакета, власника/адміністратора видавця-організації,
...
можливостями, верифікацією, метаданими артефакта та даними сканування.
...
- Релізи ClawPack включають npm-сумісні поля `npmIntegrity`, `npmShasum` і
  `npmTarballName`.
...
- Застарілі ZIP-версії перенаправляють до `/api/v1/packages/{name}/download?version=`.
...
- Потребує API-токен для користувача-модератора або адміністратора.
...
- Потребує API-токен для користувача-адміністратора.
...
Повідомити про пакет для перевірки модератором. Скарги діють на рівні пакета, за потреби
пов’язані з версією. Вони потрапляють у чергу модерації, але самі по собі не приховують автоматично й не
...
- Потребує API-токен для власника пакета, учасника видавця, модератора або
...
Кінцева точка модератора/адміністратора для розв’язання або повторного відкриття скарг на пакети.
...
`finalAction: "revoke"` з підтвердженим звітом, щоб застосувати модерацію випуску в тому
...
Ендпойнт модератора/адміністратора для перевірки випуску пакета.
...
Ендпойнт обслуговування лише для адміністраторів, який позначає старіші випуски пакетів
...
- Наявні рядки на основі ClawPack, у яких бракує `artifactKind`, відновлюються як
  `npm-pack`.
- Це не генерує ClawPacks і не змінює байти артефактів.
...
- За замовчуванням використовується останній випуск.
- Використовує квоту читання, а не квоту завантаження.
- Двійкові файли повертають `415`.
- Обмеження розміру файлу: 200 КБ.
- Очікувані сканування VirusTotal не блокують читання; шкідливі випуски все ще можуть бути приховані в інших місцях.
- Приватні пакети повертають `404`, якщо викликач не може читати видавця-власника.
...
- Відповіді містять заголовки `ETag`, `Digest`, `X-ClawHub-Artifact-Type` і
  `X-ClawHub-Artifact-Sha256` для перевірок цілісності розв’язувача.
...
- Застарілі версії лише з ZIP навмисно опущені.
...
- Packument для scoped-пакетів підтримує як `/api/npm/@scope/name`, так і закодований
  шлях запиту npm `/api/npm/@scope%2Fname`.

### `GET /api/npm/{package}/-/{tarball}.tgz`

Транслює точні байти завантаженого tarball-файлу ClawPack для клієнтів дзеркала npm.

Примітки:

- Використовує квоту завантаження.
...
- Публікації від імені іншого користувача все одно перевіряють право на офіційний канал для облікового запису цільового власника.
...
Коли `reason` присутнє, воно зберігається як нотатка модерації skill і копіюється до журналу аудиту.
...
Лише для адміністраторів. Забезпечує існування org-видавця для handle. Якщо handle досі вказує на
застарілого спільного користувача/персонального видавця, ендпойнт спершу мігрує його в org-видавця.
...
власник пізніше може опублікувати справжній випуск code-plugin або bundle-plugin під цією назвою.
...
- `merge` приховує вихідний listing і переспрямовує вихідний slug на цільовий listing.
...
Блокує користувача й остаточно видаляє належні йому skills (лише модератор/адміністратор).
...
Знімає блокування з користувача й відновлює придатні skills (лише адміністратор).
...
Перелічує користувачів або виконує пошук користувачів (лише адміністратор).
...
## Застарілі ендпойнти CLI (не рекомендовано)
