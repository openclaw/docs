...
Сторонні каталоги можуть використовувати публічні кінцеві точки читання, щоб перелічувати або шукати ClawHub Skills. Кешуйте результати, дотримуйтеся `429`/`Retry-After`, спрямовуйте користувачів назад до канонічного запису ClawHub (`https://clawhub.ai/<owner>/<slug>`) і не створюйте враження, що ClawHub підтримує сторонній сайт. Не намагайтеся дзеркалити прихований, приватний або заблокований модерацією вміст поза публічною поверхнею API.

Скорочення веб-slug розв’язуються між сімействами реєстру, але клієнтам API слід використовувати
...
- Автентифіковані запити (дійсний Bearer-токен): застосовуються для бакета користувача.
- Якщо токен відсутній або недійсний, поведінка повертається до обмеження за IP.
...
- Сумісність із застарілим форматом: `X-RateLimit-Limit`, `X-RateLimit-Remaining`, `X-RateLimit-Reset`
...
- Використовуйте backoff із джитером, щоб уникнути синхронізованих повторних спроб.
...
- Якщо довірена IP-адреса клієнта недоступна, анонімні запити завантаження використовують резервний бакет, обмежений кінцевою точкою, замість одного глобального бакета `ip:unknown`. Анонімні запити читання/запису все ще використовують спільний невідомий бакет, щоб маршрутизація з відсутньою IP залишалася видимою та консервативною.
...
- Результати повертаються в порядку релевантності (подібність embedding + підсилення точних токенів slug/назви + попередній пріоритет популярності на основі завантажень).
- Релевантність сильніша за популярність. Точний збіг токена slug або відображуваної назви може випередити менш точний збіг із набагато більшою кількістю завантажень.
...
- Завантаження використовуються як невеликий логарифмічно масштабований попередній пріоритет і критерій вирішення нічиєї, а не як основний сигнал ранжування. Skills з великою кількістю завантажень можуть ранжуватися нижче, коли текст запиту має слабший збіг.
- Підозрілий або прихований стан модерації може вилучити Skill з публічного пошуку залежно від фільтрів викликача та поточного стану модерації.

Настанови щодо видимості для видавців:

- Додавайте терміни, які користувачі буквально шукатимуть, у відображувану назву, summary і теги. Використовуйте окремий токен slug лише тоді, коли це також стабільна ідентичність, яку ви хочете зберегти.
- Не перейменовуйте slug лише для переслідування одного запиту, якщо новий slug не є кращою довгостроковою канонічною назвою. Старі slug стають alias перенаправлення, але канонічна URL-адреса, відображуваний slug і майбутні пошукові дайджести використовують новий slug.
- Alias перейменування зберігають розв’язання для старих URL-адрес і встановлень, які розв’язуються через реєстр, але пошукове ранжування базується на канонічних метаданих Skill після індексації перейменування. Наявна статистика залишається зі Skill.
- Якщо Skill неочікувано невидимий, спершу перевірте стан модерації за допомогою `clawhub inspect <slug>` після входу в систему, перш ніж змінювати метадані, пов’язані з ранжуванням.
...
- `sort` (необов’язково): `updated` (за замовчуванням), `createdAt` (alias: `newest`), `downloads`, `stars` (alias: `rating`), `installsCurrent` (alias: `installs`), `installsAllTime`, `trending`
- `nonSuspiciousOnly` (необов’язково): `true`, щоб приховати підозрілі (`flagged.suspicious`) Skills
- `nonSuspicious` (необов’язково): застарілий alias для `nonSuspiciousOnly`
...
з версією та надходять до черги звітів Skill.
...
Кінцева точка модератора/адміністратора для вирішення або повторного відкриття звітів про Skills.
...
повернення `status` до `open`. Передайте `finalAction: "hide"` із розглянутим
звітом, щоб приховати Skill у тому самому аудійованому робочому процесі.
...
- Містить нормалізований стан перевірки плюс деталі, специфічні для сканера.
...
- `moderation` є поточним знімком модерації на рівні Skill, похідним від останньої версії.
- Під час запиту історичної версії перевірте `moderation.matchesRequestedVersion` і `moderation.sourceVersion`, перш ніж трактувати `moderation` і `security` як один і той самий контекст версії.
...
- Записи Skills залишаються підтримані реєстром Skills і все ще можуть публікуватися лише через `POST /api/v1/skills`.
...
Уніфікований пошук каталогу серед Skills + пакетів Plugin.
...
Ендпойнт модератора для переліку рядків міграції офіційних OpenClaw Plugin.
...
Ендпойнт адміністратора для створення або оновлення рядка міграції офіційного Plugin.
...
Ендпойнт модератора/адміністратора для черг перевірки випусків пакетів.
...
- `open`: підозрілі, шкідливі, очікувані, поміщені в карантин, відкликані або повідомлені випуски.
...
Повідомляє про пакет для перевірки модератором. Звіти діють на рівні пакета, опційно
пов’язані з версією. Вони наповнюють чергу модерації, але самі собою не приховують
і не блокують завантаження автоматично; модератори мають використовувати модерацію випусків, щоб
...
Кожна зміна записує елемент журналу аудиту.

### `POST /api/v1/packages/backfill/artifacts`

Адміністративна кінцева точка обслуговування для позначення старіших релізів пакетів
...
- Наявні рядки на базі ClawPack без `artifactKind` відновлюються як
  `npm-pack`.
- Це не генерує ClawPack і не змінює байти артефактів.

### `GET /api/v1/packages/{name}/file`

Повертає необроблений текстовий вміст для файла пакета.
...
- Застарілі версії лише ZIP навмисно опущено.
...
- Packument пакетів зі scope підтримують як `/api/npm/@scope/name`, так і закодований npm
...
- Публікації від імені іншого все ще перевіряють право на канал `official` за обліковим записом цільового власника.
...
Коли присутнє, `reason` зберігається як нотатка модерації skill і копіюється в журнал аудиту.
М'які видалення, ініційовані власником, резервують slug на 30 днів, після чого slug може бути заявлений
іншим видавцем. Відповідь видалення містить `slugReservedUntil`, коли застосовується цей термін.
Приховування модератором/адміністратором і вилучення з міркувань безпеки не мають такого строку дії.
...
Лише для адміністратора. Гарантує, що org publisher існує для handle. Якщо handle все ще вказує на
застарілого спільного користувача/особистого видавця, кінцева точка спочатку мігрує його в org publisher.
...
власник міг пізніше опублікувати реальний реліз code-plugin або bundle-plugin у цю назву.
...
Забанити користувача та повністю видалити належні йому skills (лише модератор/адміністратор).
...
Розбанити користувача та відновити придатні skills (лише адміністратор).
...
CLI може виявляти налаштування реєстру/автентифікації з сайту:
