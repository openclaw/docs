...
x-i18n:
    generated_at: "2026-07-04T20:41:27Z"
    model: gpt-5.5
...
- Автентифіковані запити (дійсний токен Bearer): застосовується за кошиком користувача.
...
- Сумісність із застарілим API: `X-RateLimit-Limit`, `X-RateLimit-Reset`
...
- `X-RateLimit-Remaining` / `RateLimit-Remaining`: точний залишковий бюджет, коли присутній.
  Шардовані успішні запити опускають цей заголовок замість повернення приблизного глобального значення.
...
- Використовуйте відступ із джитером, щоб уникнути синхронізованих повторних спроб.
...
- ClawHub використовує довірені заголовки переспрямування, щоб ідентифікувати IP клієнтів на межі.
- Якщо довірений IP клієнта недоступний, анонімні запити використовують резервні кошики,
  обмежені лише видом обмеження частоти. Ці резервні кошики не включають
  надані викликачем шляхи, слаги, назви пакетів, версії, рядки запиту чи інші
...
мають читати тіло відповіді як рядок, зрозумілий людині. Невідомі параметри запиту
...
- `highlightedOnly` (необов’язковий): `true`, щоб фільтрувати до виділених навичок
...
- Результати повертаються в порядку релевантності (схожість embedding + підсилення точних токенів слага/назви + невеликий попередній коефіцієнт популярності).
- Релевантність сильніша за популярність. Точний збіг токена слага або відображуваної назви може випередити менш точний збіг із набагато сильнішою залученістю.
- ASCII-текст токенізується за межами слів і пунктуації. Наприклад, `personal-map` містить самостійний токен `map`, тоді як `amap-jsapi-skill` містить `amap`, `jsapi` і `skill`; тому пошук `map` дає `personal-map` сильніший лексичний збіг, ніж `amap-jsapi-skill`.
- Популярність масштабується логарифмічно й обмежується верхньою межею. Навички з високою залученістю можуть ранжуватися нижче, коли текст запиту має слабший збіг.
...
Рекомендації щодо доступності для видавців:

- Додавайте терміни, які користувачі буквально шукатимуть, у відображувану назву, резюме й теги. Використовуйте самостійний токен слага лише тоді, коли це також стабільна ідентичність, яку ви хочете зберегти.
- Не перейменовуйте слаг лише для того, щоб підлаштуватися під один запит, якщо новий слаг не є кращою довгостроковою канонічною назвою. Старі слаги стають псевдонімами переспрямування, але канонічний URL, показаний слаг і майбутні пошукові дайджести використовують новий слаг.
- Псевдоніми перейменування зберігають розв’язання для старих URL і встановлень, які розв’язуються через реєстр, але пошукове ранжування базується на канонічних метаданих навички після індексації перейменування. Наявна статистика залишається з навичкою.
...
- `sort` (необов’язковий): `updated` (типово), `recommended` (псевдонім: `default`), `createdAt` (псевдонім: `newest`), `downloads`, `stars` (псевдонім: `rating`), застарілі псевдоніми встановлень `installsCurrent`/`installs`/`installsAllTime` зіставляються з `downloads`, `trending`
- `nonSuspiciousOnly` (необов’язковий): `true`, щоб приховати підозрілі (`flagged.suspicious`) навички
- `nonSuspicious` (необов’язковий): застарілий псевдонім для `nonSuspiciousOnly`
...
- `createdAt` стабільний для сканування нових навичок; `updated` змінюється, коли наявні навички публікуються повторно.
- Коли `nonSuspiciousOnly=true`, сортування на основі курсора може повернути менше ніж `limit` елементів на сторінці, бо підозрілі навички фільтруються після отримання сторінки.
...
- Докази редагуються для публічних викликачів і включають необроблені фрагменти лише для власників/модераторів.
...
Кінцева точка модератора/адміністратора для розв’язання або повторного відкриття звітів про навички.
...
звітом, щоб приховати навичку в тому самому аудитованому робочому процесі.
...
Повертає деталі перевірки безпекового сканування для версії навички.

Параметри запиту:

- `version` (необов’язковий): конкретний рядок версії.
- `tag` (необов’язковий): розв’язати теговану версію (наприклад, `latest`).

Примітки:

- Якщо не вказано ні `version`, ні `tag`, використовується остання версія.
- Містить нормалізований статус перевірки, а також специфічні для сканера деталі.
...
- Під час запиту історичної версії перевіряйте `moderation.matchesRequestedVersion` і `moderation.sourceVersion`, перш ніж вважати `moderation` і `security` контекстом однієї версії.

### `POST /api/v1/skills/-/scan`

Автентифікована кінцева точка надсилання для нових завдань ClawScan.

Локальні сканування завантажень більше не підтримуються. Запити з використанням
...
- Повертає `queue.queuedAhead` і `queue.position`, поки запит перебуває в черзі, щоб клієнти могли показати, скільки пріоритетних ручних сканувань перебуває перед цим запитом. Дуже великі черги обмежуються й повідомляються з `queuedAheadIsEstimate: true`.
...
- Потребує успішного сканування; нетермінальні сканування повертають `409`.
...
- Потребує управлінського доступу власника/видавця до навички або plugin, або повноважень модератора/адміністратора платформи.
...
- `ok` дорівнює `true` лише тоді, коли вибрана версія має згенеровану Skill Card, не заблокована модерацією як шкідливе ПЗ, а перевірка ClawScan чиста.
- Ідентичність навички, ідентичність видавця та метадані вибраної версії є полями конверта верхнього рівня (`slug`, `displayName`, `publisherHandle`, `version`, `resolvedFrom`, `tag`, `createdAt`), щоб shell-автоматизація могла читати їх без розпаковування вкладених обгорток.
...
- `provenance` дорівнює `server-resolved-github-import` лише тоді, коли ClawHub розв’язав і зберіг GitHub repo/ref/commit/path під час публікації або імпорту; інакше він має значення `unavailable`.
...
- Відповідь містить лише дані безпеки. Вона не містить даних Skill Card, статусу згенерованої картки, списків файлів артефактів або докладних корисних навантажень сканера.
...
- `category` (необов’язково): фільтр категорії plugin. Підтримується лише коли
...
- Записи skills і надалі базуються на реєстрі skills, і їх досі можна публікувати лише через `POST /api/v1/skills`.
- `POST /api/v1/packages` досі призначено лише для випусків code-plugin і bundle-plugin.
...
Уніфікований пошук у каталозі серед skills + пакетів plugins.
...
- `isOfficial` (необов’язково): `true` або `false`
- `category` (необов’язково): фільтр категорії plugin. Підтримується лише коли
  запит обмежено пакетами plugins. Контрольовані категорії та застарілі
...
Застарілі псевдоніми фільтрів v1 і надалі приймаються на кінцевих пунктах читання:
...
`trending` — це рейтинг встановлень/завантажень за сім днів, і він не використовує загальні підсумки за весь час.
На уніфікованому кінцевому пункті `/api/v1/packages` він застосовується лише до plugins; використовуйте
...
- `cursor` (необов’язково): курсор пагінації з попередньої відповіді.
- `family` (необов’язково): `code-plugin` або `bundle-plugin`. Якщо пропущено, маються на увазі обидві
  родини Plugin.
...
Пошук лише Plugin у пакетах code-plugin і bundle-plugin.
...
Повертає точне зведення безпеки й довіри для релізу пакета для клієнтів встановлення.
Це публічна поверхня споживання OpenClaw для визначення, чи можна встановити
вирішений реліз.
...
- `trust.moderationState` може бути null. Воно дорівнює `null`, коли ручної модерації релізу
  немає.
...
- `trust.stale` означає, що зведення довіри було обчислено на основі застарілих вхідних даних і
  має вважатися таким, що потребує оновлення перед рішенням про дозвіл із високою впевненістю.

Примітки:

- Ця кінцева точка є точною до версії. Клієнтам слід викликати її після вирішення
...
- Ця кінцева точка навмисно вужча за кінцеві точки модерації для власників/модераторів.
  ...
  ідентичності репортерів, тіла звітів, приватні докази чи внутрішні часові шкали
  перевірки.
...
- `open`: підозрілі, шкідливі, очікувані, карантиновані, відкликані або повідомлені релізи.
- `blocked`: карантиновані, відкликані або шкідливі релізи.
- `manual`: будь-який реліз із ручним перевизначенням модерації.
- `all`: будь-який реліз із ручним перевизначенням, не-чистим станом сканування або звітом щодо пакета.
...
Повідомляє про пакет для перевірки модератором. Звіти мають рівень пакета й можуть необов’язково
бути пов’язані з версією. Вони потрапляють до черги модерації, але самі по собі не приховують автоматично
і не блокують завантаження; модератори мають використовувати модерацію релізів, щоб
схвалювати, карантинувати або відкликати артефакти.
...
- Обмеження розміру файлу: 200 КБ.
- Очікувані сканування VirusTotal не блокують читання; шкідливі релізи все ще можуть утримуватися в інших місцях.
- Приватні пакети повертають `404`, якщо викликач не може читати власницького видавця.
...
- Типово використовується найновіший реліз.
- Skills перенаправляються до `GET /api/v1/download`.
...
- Packument-и scoped package підтримують і `/api/npm/@scope/name`, і закодований npm
...
Завантажує ZIP розміщеної версії skill або повертає передавання до джерела GitHub для
...
- Передавання skill на основі GitHub не проксіюють і не дзеркалять байти. JSON-відповідь
...
- Необов’язкове поле payload: `migrateOwner`. Коли `true` з `ownerHandle`, наявний
  ...
  поточному й цільовому видавцях. Без цієї явної згоди зміни власника
  відхиляються.
...
- Дозволені поля форми: `payload`, повторювані blobs `files` або одне посилання на tarball
  `clawpack`. `clawpack` може бути blob `.tgz` або storage id, повернутим
...
- Прямі multipart-запити публікації обмежені 18 МБ. Tarball-файли ClawPack можуть
  використовувати потік upload-url до обмеження tarball 120 МБ.
...
- Публікації від імені іншого все ще перевіряють право на official-channel щодо цільового облікового запису власника.
...
Якщо `reason` присутнє, воно зберігається як примітка модерації skill і копіюється в журнал аудиту.
...
Приховування модератором/адміністратором і безпекові вилучення не спливають таким чином.
...
Для новоствореної org вкажіть `memberHandle`; чинний адміністратор не додається як учасник.
...
власник може згодом опублікувати справжній реліз code-plugin або bundle-plugin під цією назвою.
...
без редагування рядків облікового запису Convex Auth. Запит має називати обидва незмінні ідентифікатори
облікового запису провайдера GitHub; змінні handle використовуються лише як захист для оператора.

Кінцева точка за замовчуванням працює в режимі холостого запуску. Застосування відновлення потребує `dryRun: false` і
...
Джерела навичок GitHub мають область дії видавця та повідомляються як перевірені, а не переписуються.
...
- Обидві кінцеві точки потребують автентифікації токеном API і працюють лише для власника навички.
- `rename` зберігає попередній slug як псевдонім переспрямування.
- `merge` приховує вихідний запис і переспрямовує вихідний slug до цільового запису.
...
Зняти бан із користувача та відновити придатні навички (лише адміністратор).
...
Змінити збережену причину наявного бану без зняття бану чи відновлення
вмісту (лише адміністратор). За замовчуванням працює в режимі холостого запуску, якщо `dryRun` не дорівнює `false`.
...
`POST /api/cli/upload-url` повертає `uploadUrl` і `uploadTicket`. Публікації
пакетів, які готують tarball ClawPack, мають надсилати отриманий ідентифікатор сховища як
