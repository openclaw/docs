...
summary: Довідка HTTP API (публічні ендпоінти + ендпоінти CLI + автентифікація).
x-i18n:
...
Сторонні каталоги можуть використовувати публічні кінцеві точки читання для виведення списку або пошуку ClawHub Skills. Кешуйте результати, дотримуйтеся `429`/`Retry-After`, спрямовуйте користувачів назад до канонічного списку ClawHub (`https://clawhub.ai/<owner>/skills/<slug>`) і не створюйте враження схвалення стороннього сайту з боку ClawHub. Не намагайтеся дзеркалити прихований, приватний або заблокований модерацією вміст поза публічною поверхнею API.

Скорочення вебслагів розв’язуються між сімействами реєстру, але клієнтам API слід використовувати
...
- ClawHub використовує довірені заголовки пересилання для визначення IP-адрес клієнтів на edge.
- Якщо довірена IP-адреса клієнта недоступна, анонімні запити використовують резервні кошики,
  обмежені лише типом rate-limit. Ці резервні кошики не містять
  наданих викликачем шляхів, слагів, назв пакетів, версій, рядків запиту або інших
...
- Результати повертаються в порядку релевантності (подібність embedding + підсилення точних токенів слага/назви + невеликий пріоритет популярності).
...
- Популярність масштабується логарифмічно й обмежується. Skills із високим залученням можуть ранжуватися нижче, коли текст запиту має слабший збіг.
...
- Розміщуйте терміни, які користувачі буквально шукатимуть, у відображуваній назві, резюме та тегах. Використовуйте окремий токен слага лише тоді, коли це також стабільна ідентичність, яку ви хочете зберегти.
- Не перейменовуйте слаг лише для просування одного запиту, якщо новий слаг не є кращою довгостроковою канонічною назвою. Старі слаги стають псевдонімами переспрямування, але канонічна URL-адреса, показаний слаг і майбутні пошукові дайджести використовують новий слаг.
...
- Якщо Skill несподівано невидимий, спочатку перевірте стан модерації за допомогою `clawhub inspect @owner/slug` під час входу в систему, перш ніж змінювати метадані, пов’язані з ранжуванням.
...
- `sort` (необов’язково): `updated` (типово), `recommended` (псевдонім: `default`), `createdAt` (псевдонім: `newest`), `downloads`, `stars` (псевдонім: `rating`), застарілі псевдоніми встановлень `installsCurrent`/`installs`/`installsAllTime` відповідають `downloads`, `trending`
- `nonSuspiciousOnly` (необов’язково): `true`, щоб приховати підозрілі (`flagged.suspicious`) Skills
- `nonSuspicious` (необов’язково): застарілий псевдонім для `nonSuspiciousOnly`
...
Повідомити про Skill для перегляду модератором. Повідомлення мають рівень Skill, необов’язково пов’язані
з версією та потрапляють до черги повідомлень про Skill.
...
Кінцева точка модератора/адміністратора для розв’язання або повторного відкриття повідомлень про Skills.
...
повернення `status` до `open`. Передайте `finalAction: "hide"` з тріажованим
...
- Корисні навантаження запитів сканування та звіти для завантаження видаляються зі сховища scan-request після завершення вікна зберігання.
...
- Завдання сканування є асинхронними. Ручні запити сканування мають пріоритет перед звичайною роботою публікації/зворотного заповнення, але завершення все одно залежить від доступності воркерів.
...
- `kind` за замовчуванням дорівнює `skill`; використовуйте `kind=plugin` для сканувань plugin/package.
...
Повертає оболонку перевірки Skill Card, яку використовує `clawhub skill verify`.
...
- `ok` дорівнює `true` лише тоді, коли вибрана версія має згенеровану Skill Card, не заблокована модерацією як шкідливе ПЗ, а перевірка ClawScan є чистою.
- Ідентичність skill, ідентичність видавця та метадані вибраної версії є полями верхнього рівня оболонки (`slug`, `displayName`, `publisherHandle`, `version`, `resolvedFrom`, `tag`, `createdAt`), щоб shell-автоматизація могла читати їх без розпаковування вкладених обгорток.
...
- `security.signals.dependencyRegistry` збережено для сумісності відповіді v1, але сканер існування реєстру залежностей вилучено, і цей ключ завжди дорівнює `null`.
...
версії ClawHub skill їм потрібно показати, наприклад OpenClaw Control UI.
...
- Результати повертаються для кожного елемента; одна відсутня skill або версія не призводить до помилки всієї відповіді.
...
- Використовуйте `/verify`, коли потрібна оболонка перевірки Skill Card для однієї skill, `/card`, коли потрібен згенерований Markdown картки, і `/scan`, коли потрібні детальні дані сканерів.
...
- `mcp-tooling`, `data` і `automation` розв’язуються як `tools`.
- `observability` і `deployment` розв’язуються як `gateway`.
- `dev-tools` розв’язується як `runtime`.

`trending` — це семиденний рейтинг установлень/завантажень, який не використовує загальні підсумки за весь час.
На уніфікованій кінцевій точці `/api/v1/packages` він стосується лише plugin; використовуйте
...
- Hosted skills включають найновіші файли збереженої версії та перелічуються в
...
- Фільтрація за категорією є реальним API-фільтром, що спирається на рядки дайджесту категорій плагінів,
  а не переписуванням пошукового запиту.
- Результати повертаються в порядку релевантності й наразі не мають пагінації.
- Елементи керування сортуванням у браузерному UI для пошуку плагінів перевпорядковують завантажені результати релевантності,
...
верифікацією, метаданими артефакта й даними сканування.
...
- `release.artifactKind`, `release.artifactSha256`, `release.npmIntegrity`,
  ...
  артефакта випуску.
- `trust.scanStatus` є ефективним статусом довіри, виведеним із вхідних даних сканерів
  і ручної модерації випуску.
- `trust.moderationState` може бути null. Він дорівнює `null`, коли ручної модерації випуску
  немає.
- `trust.blockedFromDownload` є сигналом блокування інсталяції. OpenClaw та інші
...
- Це поверхня резолвера OpenClaw; вона уникає вгадування формату архіву за
  спільним URL.
...
- дайджест артефакта
- походження вихідного репозиторію й коміту
...
необов’язково бути пов’язані з версією. Вони потрапляють у чергу модерації, але самі по собі не приховують автоматично й не
...
самому контрольованому робочому процесі.
...
- Метадані лише з реєстру не додаються до завантаженого архіву.
...
- Packument-и scoped-пакетів підтримують як `/api/npm/@scope/name`, так і npm
  закодований шлях запиту `/api/npm/@scope%2Fname`.

### `GET /api/npm/{package}/-/{tarball}.tgz`

Передає потоково точні байти завантаженого tarball ClawPack для клієнтів npm-дзеркала.
...
Використовується CLI для зіставлення локального fingerprint з відомою версією.
...
- Передавання для навичок на основі GitHub не проксіюють і не дзеркалюють байти. JSON-відповідь
  містить `sourceRef: "public-github"`, `repo`, `commit`, `path`, `contentHash`
  і `archiveUrl`; стан сканування/поточний стан є умовою допуску й не входить до метаданих
  успішного payload.
...
- Необов’язкове поле payload: `migrateOwner`. Коли `true` разом з `ownerHandle`, наявна
  ...
  і в цільового видавців. Без цього явного вибору зміни власника
  відхиляються.
...
- Дозволені поля форми: `payload`, повторювані blob-и `files` або одне посилання на tarball
  `clawpack`. `clawpack` може бути blob-ом `.tgz` або storage id, повернутим
  потоком upload-url. Публікації з підготовленим storage-id також мають містити
...
- JSON-тіла та надані викликачем метадані `payload.files` / `payload.artifact`
  відхиляються.
- Прямі multipart-запити публікації обмежено 18 МБ. Tarball-и ClawPack можуть
...
- Лише видавець організації `openclaw` і персональні видавці поточних учасників організації
  `openclaw` можуть публікувати в канал `official`.
- Публікації від імені іншого все одно перевіряють право на канал official щодо облікового запису цільового власника.
...
Коли `reason` присутнє, воно зберігається як примітка модерації навички й копіюється до журналу аудиту.
...
Лише для адміністратора. Забезпечує існування видавця-організації для handle. Якщо handle досі вказує на
застарілого спільного користувача/персонального видавця, кінцева точка спочатку мігрує його у видавця-організацію.
Для новоствореної організації надайте `memberHandle`; чинного адміністратора не додають як учасника.
...
Автентифіковане самостійне створення видавця-організації. Створює нового видавця-організацію й додає
...
Лише для адміністратора. Відновлює персонального видавця для перевіреного нового принципала GitHub OAuth
без редагування рядків облікових записів Convex Auth. Запит має називати обидва незмінні ідентифікатори
облікового запису провайдера GitHub; змінювані handle використовуються лише як запобіжник для оператора.

Кінцева точка за замовчуванням працює в режимі сухого запуску. Застосування відновлення потребує `dryRun: false` і
`confirmIdentityVerified: true` після того, як співробітники незалежно перевірять безперервність між обома
принципалами GitHub. Відновлення завершується без змін, якщо поточний особистий
видавець користувача призначення має skills, packages або джерела навичок GitHub.
...
шляхи прямого власника узгоджувалися з новими повноваженнями видавця. Активне захищене
резервування handle для відновленого handle також перепризначається користувачу-заміни, щоб подальша
синхронізація профілю не могла відновити конкуруючі повноваження попереднього користувача. Кожна основна таблиця обмежена
100 рядками на транзакцію застосування; для більших відновлень спочатку потрібно використати відновлювану міграцію власника.
...
- Обидві кінцеві точки потребують автентифікації токеном API й працюють лише для власника навички.
...
Забанити користувача й остаточно видалити належні йому навички (лише moderator/admin).
...
Зняти бан із користувача й відновити придатні навички (лише admin).
...
контенту (лише admin). За замовчуванням працює в режимі сухого запуску, якщо `dryRun` не дорівнює `false`.
...
Додати/видалити зірку (позначки). Обидві кінцеві точки ідемпотентні.
...
Якщо ви самостійно розміщуєте сервіс, подавайте цей файл (або явно встановіть `CLAWHUB_REGISTRY`; застарілий `CLAWDHUB_REGISTRY`).
