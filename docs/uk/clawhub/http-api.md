...
Сторонні каталоги можуть використовувати публічні кінцеві точки читання для переліку або пошуку ClawHub Skills. Кешуйте результати, дотримуйтеся `429`/`Retry-After`, спрямовуйте користувачів назад до канонічного списку ClawHub (`https://clawhub.ai/<owner>/<slug>`) і уникайте натяків на підтримку стороннього сайту з боку ClawHub. Не намагайтеся дзеркалити прихований, приватний або заблокований модерацією вміст поза поверхнею публічного API.

Вебскорочення slug розв’язуються між сімействами реєстру, але клієнтам API слід використовувати
канонічні URL-адреси, повернені кінцевими точками читання, замість відновлення пріоритету
маршрутів.
...
- `Retry-After`: скільки секунд чекати перед повтором (затримка) для `429`
...
- Якщо `Retry-After` існує, зачекайте стільки секунд перед повтором.
- Використовуйте backoff із jitter, щоб уникнути синхронізованих повторів.
- Якщо `Retry-After` відсутній, використовуйте fallback до `RateLimit-Reset` (або обчисліть з `X-RateLimit-Reset`).
...
- Якщо довірена IP-адреса клієнта недоступна, анонімні запити завантаження використовують fallback-кошик у межах кінцевої точки замість одного глобального кошика `ip:unknown`. Анонімні запити читання/запису й далі використовують спільний невідомий кошик, щоб маршрутизація без IP залишалася видимою та консервативною.
...
- Результати повертаються в порядку релевантності (схожість embedding + підсилення для точних токенів slug/назви + апріорна популярність із завантажень).
...
- ASCII-текст токенізується за межами слів і розділових знаків. Наприклад, `personal-map` містить окремий токен `map`, тоді як `amap-jsapi-skill` містить `amap`, `jsapi` і `skill`; тому пошук `map` дає `personal-map` сильніший лексичний збіг, ніж `amap-jsapi-skill`.
- Завантаження використовуються як невеликий логарифмічно масштабований апріорний сигнал і критерій розв’язання нічиєї, а не як основний сигнал ранжування. Skills з великою кількістю завантажень можуть ранжуватися нижче, коли текст запиту має слабший збіг.
...
Настанови щодо видимості видавця:
...
- Не перейменовуйте slug лише задля одного запиту, якщо новий slug не є кращою довгостроковою канонічною назвою. Старі slug стають псевдонімами перенаправлення, але канонічна URL-адреса, відображуваний slug і майбутні пошукові дайджести використовують новий slug.
...
- Якщо Skill несподівано невидимий, спершу перевірте стан модерації за допомогою `clawhub inspect <slug>` після входу, перш ніж змінювати метадані, пов’язані з ранжуванням.
...
- `moderation` включається лише тоді, коли Skill позначений або його переглядає власник.
...
Повідомити про Skill для перевірки модератором. Звіти є на рівні Skill, необов’язково пов’язані
...
- Включає нормалізований статус перевірки плюс специфічні для сканера деталі.
...
- `moderation` — це поточний знімок модерації на рівні Skill, виведений з останньої версії.
...
- Записи Skill і надалі підтримуються реєстром Skill та все ще можуть публікуватися лише через `POST /api/v1/skills`.
...
Уніфікований пошук у каталозі по Skills і пакетах Plugin.
...
можливостями, перевіркою, метаданими артефакта й даними сканування.
...
- Версії застарілих пакетів повертають артефакт `legacy-zip` і застарілу ZIP
  `downloadUrl`.
...
- Версії застарілого ZIP переспрямовують на `/api/v1/packages/{name}/download?version=`.
...
Ендпойнт модератора для переліку рядків міграції офіційних Plugin OpenClaw.

Автентифікація:

- Потребує API-токена для користувача-модератора або адміністратора.
...
- Потребує API-токена для користувача-адміністратора.
...
Повідомляє про пакет для перевірки модератором. Звіти мають рівень пакета та можуть бути
необов’язково пов’язані з версією. Вони потрапляють до черги модерації, але самі по собі
не приховують автоматично та не блокують завантаження; модератори мають використовувати модерацію випусків, щоб
...
Ендпойнт модератора/адміністратора для прийому звітів про пакети.
...
- Потребує API-токена для власника пакета, учасника видавця, модератора або
...
тому самому аудитовному робочому процесі.
...
Адміністративна службова кінцева точка для позначення старіших релізів пакетів
...
- Наявні рядки на основі ClawPack, у яких відсутній `artifactKind`, виправляються як
  `npm-pack`.
...
- Packument для пакетів зі scope підтримує як `/api/npm/@scope/name`, так і закодований
...
- Необов’язкове поле payload: `ownerHandle`. Якщо воно є, API визначає цього
...
- Необов’язкове поле payload: `migrateOwner`. Коли `true` з `ownerHandle`, наявний
  skill може перейти до цього власника, якщо actor є адміністратором/власником і в
...
- Вимагає автентифікації Bearer-токеном.
- Бажано: `multipart/form-data` з JSON `payload` + blob-файлами `files[]`.
- JSON-тіло з `files` (на основі storageId) також приймається.
- Необов’язкове поле payload: `ownerHandle`. Якщо воно є, лише адміністратори можуть публікувати від імені цього власника.
...
- Code plugins вимагають `package.json`, метадані репозиторію source, метадані коміту
  source, метадані схеми конфігурації, `openclaw.compat.pluginApi` і
...
М’яке видалення / відновлення skill (власник, модератор або адміністратор).
...
Якщо `reason` є, він зберігається як примітка модерації skill і копіюється в журнал аудиту.
М’які видалення, ініційовані власником, резервують slug на 30 днів, після чого slug може бути заявлений
іншим видавцем. Відповідь на видалення містить `slugReservedUntil`, коли застосовується цей термін.
...
власник може пізніше опублікувати справжній реліз code-plugin або bundle-plugin із цією назвою.
...
Додає/видаляє зірку (виділення). Обидві кінцеві точки є ідемпотентними.
