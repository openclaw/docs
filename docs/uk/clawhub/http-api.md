...
x-i18n:
    generated_at: "2026-07-02T22:45:48Z"
    model: gpt-5.5
...
Сторонні каталоги можуть використовувати публічні кінцеві точки читання, щоб показувати або шукати навички ClawHub. Кешуйте результати, дотримуйтеся `429`/`Retry-After`, посилайте користувачів назад на канонічну сторінку ClawHub (`https://clawhub.ai/<owner>/skills/<slug>`) і не створюйте враження, що ClawHub схвалює сторонній сайт. Не намагайтеся дзеркалити прихований, приватний або заблокований модерацією вміст поза публічною поверхнею API.
...
- Автентифіковані запити (чинний Bearer token): застосовується за кошиком користувача.
...
- Автентифіковані кінцеві точки запису не мають повертати лише `Unauthorized`, коли
  ...
  видалені/забанені/вимкнені облікові записи мають отримувати текст із дією, щоб CLI
...
- `Retry-After`: скільки секунд чекати перед повторною спробою (затримка) для `429`
...
- Використовує заголовки IP довіреного клієнта, зокрема `cf-connecting-ip`, лише коли
  розгортання явно вмикає довірені переслані заголовки.
- ClawHub використовує довірені переслані заголовки, щоб визначати IP клієнтів на edge.
- Якщо IP довіреного клієнта недоступний, анонімні запити використовують резервні кошики,
  обмежені лише типом обмеження частоти. Ці резервні кошики не містять
...
- Релевантність важливіша за популярність. Точний збіг токена slug або відображуваної назви може випередити менш точний збіг із набагато сильнішою залученістю.
...
- Не перейменовуйте slug лише заради одного запиту, якщо новий slug не є кращою довгостроковою канонічною назвою. Старі slugs стають псевдонімами перенаправлення, але канонічна URL-адреса, відображуваний slug і майбутні пошукові дайджести використовують новий slug.
- Псевдоніми перейменування зберігають розв’язання для старих URL-адрес та встановлень, які розв’язуються через реєстр, але пошукове ранжування базується на канонічних метаданих навички після індексації перейменування. Наявна статистика залишається з навичкою.
...
- `sort` (необов’язково): `updated` (типово), `recommended` (псевдонім: `default`), `createdAt` (псевдонім: `newest`), `downloads`, `stars` (псевдонім: `rating`), застарілі псевдоніми встановлень `installsCurrent`/`installs`/`installsAllTime` мапляться на `downloads`, `trending`
- `nonSuspiciousOnly` (необов’язково): `true`, щоб приховати підозрілі (`flagged.suspicious`) навички
- `nonSuspicious` (необов’язково): застарілий псевдонім для `nonSuspiciousOnly`
...
- Старі slugs, створені потоками перейменування/злиття власника, розв’язуються до канонічної навички.
...
- Власники та модератори можуть отримувати доступ до деталей модерації прихованих навичок.
- Публічні викликачи отримують `200` лише для вже позначених видимих навичок.
- Докази редагуються для публічних викликачів і містять сирі фрагменти лише для власників/модераторів.

### `POST /api/v1/skills/{slug}/report`

Повідомте про навичку для перевірки модератором. Звіти стосуються рівня навички, необов’язково пов’язані
з версією, і надходять у чергу звітів про навички.
...
звітом, щоб приховати навичку в тому самому придатному для аудиту робочому процесі.
...
- Містить нормалізований стан перевірки, а також деталі, специфічні для сканера.
...
- Опубліковані сканування потребують керівного доступу власника/публікатора або повноважень модератора/адміністратора платформи.
- Опубліковані сканування записують результат назад лише тоді, коли `update: true` і сканування успішно завершується.
...
- Повертає стан у черзі/виконується/успішно/помилка.
- Повертає `queue.queuedAhead` і `queue.position`, поки запит перебуває в черзі, щоб клієнти могли показати, скільки пріоритизованих ручних сканувань стоїть перед цим запитом. Дуже великі черги обмежуються й повідомляються з `queuedAheadIsEstimate: true`.
...
- Потребує керівного доступу власника/публікатора до навички або Plugin, або повноважень модератора/адміністратора платформи.
...
- `ok` має значення `true` лише тоді, коли вибрана версія має згенеровану Skill Card, не заблокована модерацією як шкідливе ПЗ, а перевірка ClawScan є чистою.
- Ідентичність навички, ідентичність публікатора та метадані вибраної версії є полями верхнього рівня конверта (`slug`, `displayName`, `publisherHandle`, `version`, `resolvedFrom`, `tag`, `createdAt`), щоб shell-автоматизація могла читати їх без розпакування вкладених обгорток.
...
- `provenance` має значення `server-resolved-github-import` лише тоді, коли ClawHub визначив і зберіг репозиторій/ref/commit/path GitHub під час публікації або імпорту; інакше значення — `unavailable`.
...
- Відповідь містить лише дані безпеки. Вона не містить даних Skill Card, стану згенерованої картки, списків файлів артефактів або детальних корисних навантажень сканера.
- `security.signals` містить лише допоміжні докази на рівні стану; використовуйте `/scan` або сторінку аудиту безпеки ClawHub для повних деталей сканера.
- `security.signals.dependencyRegistry` збережено для сумісності відповіді v1, але сканер наявності реєстру залежностей виведено з експлуатації, і цей ключ завжди має значення `null`.
...
- `category` (необов'язково): фільтр категорії Plugin-а. Підтримується лише коли
  запит обмежено пакетами Plugin-ів (`/api/v1/plugins`,
...
- `POST /api/v1/packages` все ще призначений лише для випусків code-plugin і bundle-plugin.
...
Уніфікований пошук у каталозі серед Skills і пакетів Plugin-ів.
...
- `isOfficial` (необов'язково): `true` або `false`
- `category` (необов'язково): фільтр категорії Plugin-а. Підтримується лише коли
  запит обмежено пакетами Plugin-ів. Керовані категорії та застарілі псевдоніми
...
Перегляд каталогу лише Plugin-ів серед пакетів code-plugin і bundle-plugin.
...
- `category` (необов'язково): фільтр категорії Plugin-а. Поточні значення:
...
На уніфікованій кінцевій точці `/api/v1/packages` вона застосовується лише до Plugin-ів; використовуйте
...
- Тіло: ZIP-архів.
- Кожна експортована Skills має корінь у `{publisher}/{slug}/`.
...
- Кожна Skills містить `_export_skill_meta.json`.
...
- Застарілі псевдоніми фільтрів v1, задокументовані в `GET /api/v1/plugins`, також
  приймаються.
- Фільтрація за категорією є справжнім API-фільтром, що спирається на рядки дайджесту
...
- `trust.moderationState` може бути null. Він має значення `null`, коли ручної модерації
  релізу немає.
...
- Ця кінцева точка навмисно вужча за кінцеві точки модерації для власників/модераторів.
  ...
  ідентичності репортерів, тіла звітів, приватні докази або внутрішні часові лінії
  перевірки.
...
- Версії ClawPack потоково передають точні завантажені байти npm-pack `.tgz`.
...
- `open`: підозрілі, шкідливі, очікувані, карантиновані, відкликані або поскаржені релізи.
...
Повідомляє про пакет для перевірки модератором. Звіти належать до рівня пакета й можуть
бути необов’язково пов’язані з версією. Вони потрапляють у чергу модерації, але самі по собі не приховують автоматично й не
...
- Очікувані сканування VirusTotal не блокують читання; шкідливі релізи все ще можуть бути недоступними в інших місцях.
...
- Метадані, доступні лише в реєстрі, не вставляються в завантажений архів.
...
Транслює точні байти завантаженого tarball ClawPack для клієнтів дзеркала npm.
...
поточної skill на основі GitHub зі скануванням `clean` або `suspicious` і без розміщеної
версії.
...
- Передачі skill на основі GitHub не проксіюють і не дзеркалять байти. JSON-відповідь
  ...
  і `archiveUrl`; стан сканування/актуальності є перевіркою доступу й не включається як метадані
  успішного payload.
...
- Необов’язкове поле payload: `migrateOwner`. Коли `true` з `ownerHandle`,
  наявна skill може перейти до цього власника, якщо actor є адміністратором/власником як у
  поточного, так і цільового видавців. Без цього явного ввімкнення зміни власника
  відхиляються.
...
- Дозволені поля форми: `payload`, повторювані blob-файли `files` або одне посилання на tarball
  ...
  потоком upload-url. Публікації з підготовленими storage-id також мають містити
...
- Публікації від імені іншого користувача все одно перевіряють право на канал official для облікового запису цільового власника.
...
Коли `reason` присутнє, воно зберігається як примітка модерації skill і копіюється в журнал аудиту.
...
Приховування модератором/адміністратором і видалення з міркувань безпеки не спливають таким чином.
...
власник пізніше міг опублікувати справжній реліз code-plugin або bundle-plugin під цією назвою.
...
облікових записів provider GitHub; змінювані handle використовуються лише як guard, видимий оператору.
...
суб’єктами GitHub. Відновлення завершується безпечною відмовою, якщо поточний персональний
...
Заблокувати користувача й безповоротно видалити належні йому навички (лише модератор/адміністратор).
...
вмісту (лише адміністратор). За замовчуванням виконується пробний запуск, якщо `dryRun` не дорівнює `false`.
...
Додати/прибрати зірочку (виділення). Обидві кінцеві точки ідемпотентні.
...
які розміщують tarball ClawPack на проміжному етапі, мають надіслати отриманий ідентифікатор сховища як
