...
Сторонні каталоги можуть використовувати публічні кінцеві точки читання, щоб перелічувати або шукати Skills ClawHub. Кешуйте результати, дотримуйтеся `429`/`Retry-After`, посилайте користувачів назад на канонічний запис ClawHub (`https://clawhub.ai/<owner>/skills/<slug>`) і уникайте натяків на схвалення стороннього сайту з боку ClawHub. Не намагайтеся дзеркалити прихований, приватний або заблокований модерацією вміст поза поверхнею публічного API.

Скорочення веб-слагів розпізнаються між сімействами реєстру, але API-клієнти мають використовувати
...
- ClawHub використовує довірені заголовки пересилання для ідентифікації IP-адрес клієнтів на edge.
...
Публічні відповіді про помилки v1 є звичайним текстом із `content-type: text/plain; charset=utf-8`.
Це включає помилки валідації (`400`), відсутні публічні ресурси (`404`), помилки автентифікації і
дозволів (`401`/`403`), обмеження швидкості (`429`) та заблоковані завантаження. Клієнти
...
- `highlightedOnly` (необов’язковий): `true`, щоб відфільтрувати до виділених Skills
...
- Релевантність сильніша за популярність. Точний збіг токена слага або відображуваної назви може випередити слабший збіг із набагато сильнішим залученням.
...
- Псевдоніми перейменування зберігають розпізнавання для старих URL-адрес і встановлень, які розпізнаються через реєстр, але пошукове ранжування базується на канонічних метаданих Skill після індексації перейменування. Наявна статистика залишається зі Skill.
- Якщо Skill несподівано невидимий, спершу перевірте стан модерації за допомогою `clawhub inspect @owner/slug` після входу в систему, перш ніж змінювати метадані, пов’язані з ранжуванням.
...
- `sort` (необов’язковий): `updated` (за замовчуванням), `recommended` (псевдонім: `default`), `createdAt` (псевдонім: `newest`), `downloads`, `stars` (псевдонім: `rating`), застарілі псевдоніми встановлення `installsCurrent`/`installs`/`installsAllTime` зіставляються з `downloads`, `trending`
- `nonSuspiciousOnly` (необов’язковий): `true`, щоб приховати підозрілі (`flagged.suspicious`) Skills
- `nonSuspicious` (необов’язковий): застарілий псевдонім для `nonSuspiciousOnly`
...
- Використовуйте `nextCursor`, щоб продовжити пагінацію, коли він присутній. Коротка сторінка сама по собі не означає завершення результатів.
...
- Старі слаги, створені потоками перейменування/злиття власника, розпізнаються як канонічний Skill.
...
Повідомити про Skill для перегляду модератором. Повідомлення створюються на рівні Skill, необов’язково пов’язані
з версією, і надходять до черги повідомлень про Skill.
...
`status` встановлюється назад на `open`. Передайте `finalAction: "hide"` із обробленим
повідомленням, щоб приховати Skill у тому самому аудитовному workflow.
...
- `tag` (необов’язковий): розпізнати версію з тегом (наприклад `latest`).
...
- Корисні дані запитів сканування та звіти, доступні для завантаження, видаляються зі сховища scan-request після завершення періоду зберігання.
...
- Опубліковані сканування записують результати назад лише коли `update: true` і сканування завершується успішно.
...
- Завдання сканування асинхронні. Ручні запити сканування мають пріоритет над звичайною роботою публікації/дозаповнення, але завершення все одно залежить від доступності воркерів.
...
- Повертає `queue.queuedAhead` і `queue.position`, поки запит перебуває в черзі, щоб клієнти могли показати, скільки пріоритетних ручних сканувань стоїть попереду цього запиту. Дуже великі черги обмежуються й повідомляються з `queuedAheadIsEstimate: true`.
...
- Потребує доступу власника/видавця до керування skill або plugin, або повноважень модератора/адміністратора платформи.
...
Повертає конверт перевірки картки Skill, який використовується `clawhub skill verify`.
...
- `ok` має значення `true` лише тоді, коли вибрана версія має згенеровану картку Skill, не заблокована модерацією як шкідливе ПЗ, а перевірка ClawScan чиста.
...
- `provenance` має значення `server-resolved-github-import` лише тоді, коли ClawHub розв’язав і зберіг GitHub repo/ref/commit/path під час публікації або імпорту; інакше має значення `unavailable`.
...
- Результати надаються для кожного елемента; один відсутній skill або версія не спричиняє помилки всієї відповіді.
- Відповідь містить лише дані безпеки. Вона не містить даних картки Skill, статусу згенерованої картки, списків файлів артефактів або докладних корисних даних сканерів.
- `security.signals` містить лише допоміжні докази рівня статусу; використовуйте `/scan` або сторінку аудиту безпеки ClawHub для повних деталей сканерів.
- `security.signals.dependencyRegistry` збережено для сумісності відповіді v1, але сканер наявності реєстру залежностей виведено з експлуатації, і цей ключ завжди має значення `null`.
- Відсутність картки Skill не впливає на `ok`, `decision` або `reasons` цієї кінцевої точки; клієнти мають читати встановлений `skill-card.md` локально, коли їм потрібен вміст картки.
- Використовуйте `/verify`, коли вам потрібен конверт перевірки картки Skill для одного skill, `/card`, коли потрібен згенерований markdown картки, і `/scan`, коли потрібні докладні дані сканерів.
...
- Записи Skills надалі спираються на реєстр Skills і все ще можуть публікуватися лише через `POST /api/v1/skills`.
...
`trending` — це семиденний рейтинг встановлень/завантажень і не використовує загальні підсумки за весь час.
В уніфікованій кінцевій точці `/api/v1/packages` він призначений лише для plugin; використовуйте
`/api/v1/skills?sort=trending` для каталогу Skills.

Застарілі псевдоніми не приймаються як збережені або оголошені автором значення категорії.
...
- Тіло: ZIP-архів.
- Кожна експортована Skill має корінь у `{publisher}/{slug}/`.
- Hosted Skills містять файли останньої збереженої версії та перелічені в
  `_manifest.json` із `sourceRef: "public-clawhub"`.
- Поточні Skills, підкріплені GitHub, зі скануванням `clean` або `suspicious` містять
...
- Кожна Skill містить `_export_skill_meta.json`.
...
- Елементи керування сортуванням у браузерному UI для пошуку Plugin переупорядковують завантажені релевантні результати,
...
- `trust.stale` означає, що підсумок довіри було обчислено із застарілих вхідних даних і
  перед рішенням із високою впевненістю про дозвіл його слід вважати таким, що потребує оновлення.
...
- Ця кінцева точка навмисно вужча за кінцеві точки модерації для власників/модераторів.
  ...
  ідентичності репортерів, тексти звітів, приватні докази чи внутрішні часові шкали
  перевірки.
...
- Перелічуються лише версії із завантаженими tarball-файлами npm-pack ClawPack.
...
- Packument для scoped-пакетів підтримує як `/api/npm/@scope/name`, так і
...
- Необов’язкове поле payload: `migrateOwner`. Коли `true` разом з `ownerHandle`, наявний
  ...
  та цільового видавців. Без цієї явної згоди зміни власника
  відхиляються.
...
- Дозволені поля форми: `payload`, повторювані blob-файли `files` або одне посилання на
  tarball `clawpack`. `clawpack` може бути blob `.tgz` або storage id, поверненим
...
Коли `reason` наявний, він зберігається як примітка модерації skill і копіюється в журнал аудиту.
...
Лише для адміністратора. Гарантує існування org-видавця для handle. Якщо handle все ще вказує на
застарілого спільного користувача/персонального видавця, ендпойнт спочатку мігрує його в org-видавця.
...
викликача як власника. Цей ендпойнт не мігрує наявні користувацькі/персональні handle і
...
Лише для адміністратора. Відновлює персонального видавця для перевіреного замінного principal GitHub OAuth
без редагування рядків облікових записів Convex Auth. Запит має вказувати обидва незмінні ідентифікатори облікових записів
провайдера GitHub; змінювані handle використовуються лише як захист для оператора.
...
GitHub principals. Відновлення завершується закритою відмовою, коли поточний персональний
publisher користувача призначення має skills, packages або GitHub skill sources.
Відновлення також мігрує застарілі поля `ownerUserId` для skills відновленого publisher,
аліасів slug skills, packages, попереджень package inspector і похідних рядків search digest, щоб
шляхи direct-owner узгоджувалися з новими повноваженнями publisher. Активне бронювання protected-handle
...
100 рядками на транзакцію застосування; більші відновлення мають спочатку використати відновлювану міграцію owner.
GitHub skill sources мають область дії publisher і повідомляються як перевірені, а не переписуються.
...
### Кінцеві точки керування slug owner
...
- Обидві кінцеві точки потребують автентифікації API token і працюють лише для owner skill.
- `rename` зберігає попередній slug як redirect alias.
- `merge` приховує вихідний список і перенаправляє вихідний slug до цільового списку.

### Кінцеві точки передавання ownership
...
Додати/видалити star (highlights). Обидві кінцеві точки є ідемпотентними.
...
які готують tarball ClawPack, мають надіслати отриманий storage id як
`clawpack`, а повернений ticket як `clawpackUploadTicket`.

## Виявлення registry (`/.well-known/clawhub.json`)
