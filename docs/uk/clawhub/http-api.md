...
Скорочення веб-слагів розв’язуються між родинами реєстру, але API-клієнти мають використовувати
...
- Автентифіковані запити (чинний токен Bearer): застосовується за кошиком користувача.
...
- ClawHub використовує довірені заголовки forwarding, щоб ідентифікувати IP клієнтів на edge.
- Якщо довірений IP клієнта недоступний, анонімні запити використовують резервні кошики,
  ...
  надані викликачем шляхи, слаги, назви пакетів, версії, рядки запиту або інші
  параметри артефактів.

## Відповіді з помилками

Публічні відповіді з помилками v1 є звичайним текстом із `content-type: text/plain; charset=utf-8`.
Це включає збої валідації (`400`), відсутні публічні ресурси (`404`), збої автентифікації та
...
- Результати повертаються в порядку релевантності (схожість embedding + підсилення точних токенів слага/назви + невеликий пріоритет популярності).
...
- Популярність логарифмічно масштабується та обмежується. Навички з високою залученістю можуть ранжуватися нижче, коли текст запиту має слабший збіг.
...
- Псевдоніми перейменування зберігають розв’язання для старих URL-адрес та встановлень, що розв’язуються через реєстр, але пошукове ранжування базується на канонічних метаданих навички після індексації перейменування. Наявна статистика залишається з навичкою.
...
Кінцева точка модератора/адміністратора для вирішення або повторного відкриття скарг на навички.
...
встановлення `status` назад на `open`. Передайте `finalAction: "hide"` із розглянутою
скаргою, щоб приховати навичку в тому самому аудитовному workflow.
...
- Під час запиту історичної версії перевірте `moderation.matchesRequestedVersion` і `moderation.sourceVersion`, перш ніж трактувати `moderation` і `security` як контекст однієї версії.

### `POST /api/v1/skills/-/scan`

Автентифікований кінцевий пункт надсилання для нових завдань ClawScan.
...
Автентифікований кінцевий пункт опитування для надісланого сканування.

- Повертає статус у черзі/виконується/успішно/невдало.
- Повертає `queue.queuedAhead` і `queue.position`, поки запит перебуває в черзі, щоб клієнти могли показати, скільки пріоритетних ручних сканувань попереду цього запиту. Дуже великі черги обмежуються та повідомляються з `queuedAheadIsEstimate: true`.
...
Автентифікований кінцевий пункт збереженого архіву звіту для надісланих версій.

- Потребує доступу керування власника/видавця до skill або plugin, або повноважень модератора/адміністратора платформи.
...
- Ідентичність skill, ідентичність видавця та метадані вибраної версії є полями верхнього рівня конверта (`slug`, `displayName`, `publisherHandle`, `version`, `resolvedFrom`, `tag`, `createdAt`), щоб автоматизація shell могла читати їх без розпакування вкладених обгорток.
...
- `provenance` має значення `server-resolved-github-import` лише тоді, коли ClawHub розв’язав і зберіг repo/ref/commit/path GitHub під час публікації або імпорту; інакше має значення `unavailable`.
...
Застарілі псевдоніми фільтрів v1 надалі приймаються на кінцевих точках читання:
...
На уніфікованій кінцевій точці `/api/v1/packages` він застосовується лише до plugin; використовуйте
...
- `startDate` (обов’язковий): нижня межа в мілісекундах Unix для plugin `updatedAt`.
- `endDate` (обов’язковий): верхня межа в мілісекундах Unix для plugin `updatedAt`.
...
- Застарілі псевдоніми фільтрів v1, задокументовані в `GET /api/v1/plugins`, також
  приймаються.
- Фільтрація за категорією є справжнім фільтром API, підкріпленим рядками дайджесту
...
- Потрібен токен API власника пакета, власника/адміністратора видавця організації,
...
Повертає точне зведення безпеки й довіри для випуску пакета для клієнтів
установлення. Це публічна поверхня споживання OpenClaw для визначення, чи можна
...
- `trust.reasons` — список пояснень для користувача й аудиту. Коди причин
  ...
  і `package:malicious`.
- `trust.pending` означає, що одні або кілька вхідних даних довіри все ще очікують завершення.
- `trust.stale` означає, що зведення довіри було обчислено із застарілих вхідних даних і
...
- Ця кінцева точка є точною до версії. Клієнти мають викликати її після розв’язання
  версії пакета, яку вони мають намір установити, а не лише після читання найновіших
  метаданих пакета.
- Приватні пакети повертають `404`, якщо викликач не може читати видавця-власника.
- Ця кінцева точка навмисно вужча за кінцеві точки модерації для власників/модераторів.
  Вона розкриває рішення щодо встановлення й публічне пояснення, а не
  особи авторів скарг, тексти скарг, приватні докази або внутрішні часові лінії
  перевірки.
...
- Потрібен токен API для користувача-модератора або адміністратора.
...
- Потрібен токен API для користувача-адміністратора.
...
- `open`: підозрілі, шкідливі, очікувані, карантинні, відкликані або поскаржені випуски.
- `blocked`: карантинні, відкликані або шкідливі випуски.
- `manual`: будь-який випуск із ручним перевизначенням модерації.
- `all`: будь-який випуск із ручним перевизначенням, не-чистим станом сканування або скаргою на пакет.
...
Повідомляє про пакет для перевірки модератором. Скарги діють на рівні пакета, необов’язково
пов’язані з версією. Вони потрапляють до черги модерації, але самі по собі автоматично не приховують і
не блокують завантаження; модераторам слід використовувати модерацію випусків, щоб
...
Кінцева точка модератора/адміністратора для розв'язання або повторного відкриття звітів про пакети.
...
- Використовує бакет обмеження швидкості читання, а не бакет завантаження.
...
- Цей маршрут залишається лише ZIP. Він не стрімить файли ClawPack `.tgz`.
...
- Packument для пакетів зі scope підтримує як `/api/npm/@scope/name`, так і npm
  закодований шлях запиту `/api/npm/@scope%2Fname`.

### `GET /api/npm/{package}/-/{tarball}.tgz`

Стрімить точні байти завантаженого tarball ClawPack для клієнтів npm-дзеркала.
...
поточної skill на базі GitHub зі скануванням `clean` або `suspicious` і без розміщеної
версії.
...
- Необов'язкове поле payload: `migrateOwner`. Коли `true` разом з `ownerHandle`,
  наявна skill може перейти до цього власника, якщо актор є адміністратором/власником у обох
  видавців, поточного й цільового. Без цього явного ввімкнення зміни власника
  відхиляються.
...
- Дозволені поля форми: `payload`, повторювані blob-файли `files` або одне посилання
  на tarball `clawpack`. `clawpack` може бути blob-файлом `.tgz` або id сховища, поверненим
...
- Публікації від імені іншого все ще перевіряють право на канал official щодо облікового запису цільового власника.
...
Лише для адміністраторів. Забезпечує існування видавця організації для handle. Якщо handle все ще вказує на
...
Лише для адміністраторів. Резервує кореневі slug і назви пакетів для правомірного власника без публікації
релізу. Назви пакетів стають приватними пакетами-заповнювачами без рядків релізів, тож той самий
власник може пізніше опублікувати реальний реліз code-plugin або bundle-plugin з цією назвою.
...
облікових записів провайдера GitHub; змінні handle використовуються лише як guard, видимий оператору.
...
суб’єктами GitHub. Відновлення завершується з відмовою, якщо поточний особистий
...
шляхи прямого власника узгоджувалися з новим повноваженням видавця. Активне захищене
резервування дескриптора для відновленого дескриптора також перепризначається користувачу-заміни, щоб подальша
синхронізація профілю не могла відновити конкуруюче повноваження колишнього користувача. Кожна основна таблиця обмежена
100 рядками на транзакцію застосування; більші відновлення спочатку мають використати відновлювану міграцію власника.
