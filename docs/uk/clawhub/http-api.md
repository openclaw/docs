...
x-i18n:
    generated_at: "2026-07-02T01:11:19Z"
    model: gpt-5.5
...
Сторонні каталоги можуть використовувати публічні кінцеві точки читання, щоб виводити список або шукати навички ClawHub. Кешуйте результати, дотримуйтеся `429`/`Retry-After`, посилайте користувачів назад на канонічний запис ClawHub (`https://clawhub.ai/<owner>/skills/<slug>`) і не створюйте враження, що ClawHub схвалює сторонній сайт. Не намагайтеся дзеркалити прихований, приватний або заблокований модерацією вміст поза публічною поверхнею API.
...
- Анонімні запити: застосовуються на IP.
- Автентифіковані запити (дійсний Bearer-токен): застосовуються на користувацький кошик.
...
- `highlightedOnly` (необов’язково): `true`, щоб фільтрувати лише виділені навички
...
- Релевантність сильніша за популярність. Точний збіг токена slug або відображуваної назви може випередити ширший збіг із значно вищою залученістю.
...
- Популярність масштабується логарифмічно й обмежується. Навички з високою залученістю можуть ранжуватися нижче, коли текст запиту збігається слабше.
...
- Не перейменовуйте slug лише для переслідування одного запиту, якщо новий slug не є кращою довгостроковою канонічною назвою. Старі slug стають псевдонімами перенаправлення, але канонічна URL-адреса, відображуваний slug і майбутні пошукові дайджести використовують новий slug.
- Псевдоніми перейменування зберігають розв’язання для старих URL-адрес і встановлень, що розв’язуються через реєстр, але пошукове ранжування базується на канонічних метаданих навички після індексації перейменування. Наявна статистика залишається з навичкою.
...
- `createdAt` стабільний для сканувань нових навичок; `updated` змінюється, коли наявні навички публікуються повторно.
...
- `metadata` є `null`, якщо навичка не має метаданих платформи.
...
- Власники й модератори можуть отримувати доступ до подробиць модерації для прихованих навичок.
...
з версією, і потрапляють до черги повідомлень про навички.
...
повідомленням, щоб приховати навичку в тому самому аудитовному робочому процесі.
...
- `version.security` включає нормалізований статус перевірки сканування та подробиці сканера
  (VirusTotal + LLM), коли доступно.

### `GET /api/v1/skills/{slug}/scan`

Повертає подробиці перевірки безпекового сканування для версії навички.
...
- Під час запиту історичної версії перевіряйте `moderation.matchesRequestedVersion` і `moderation.sourceVersion`, перш ніж розглядати `moderation` і `security` як контекст однієї версії.

### `POST /api/v1/skills/-/scan`

Автентифікована точка надсилання для нових завдань ClawScan.
...
- Завдання сканування є асинхронними. Ручні запити на сканування мають пріоритет над звичайною роботою публікації/зворотного заповнення, але завершення все одно залежить від доступності worker.

### `GET /api/v1/skills/-/scan/{scanId}`

Автентифікована точка опитування для надісланого сканування.

- Повертає статус queued/running/succeeded/failed.
- Повертає `queue.queuedAhead` і `queue.position`, поки запит у черзі, щоб клієнти могли показати, скільки пріоритетних ручних сканувань стоїть перед цим запитом. Дуже великі черги обмежуються й повідомляються з `queuedAheadIsEstimate: true`.
...
Автентифікована точка архіву звіту.
...
Автентифікована точка збереженого архіву звіту для надісланих версій.

- Потребує доступу до керування власника/видавця для навички або плагіна, або повноважень модератора/адміністратора платформи.
- Повертає збережені результати сканування для точно надісланої версії, включно із заблокованими або прихованими версіями.
...
Повертає оболонку перевірки картки навички, яку використовує `clawhub skill verify`.
...
- `ok` має значення `true` лише коли вибрана версія має згенеровану картку навички, не заблокована модерацією як шкідлива та має чисту перевірку ClawScan.
- Ідентичність навички, ідентичність видавця та метадані вибраної версії є полями верхнього рівня оболонки (`slug`, `displayName`, `publisherHandle`, `version`, `resolvedFrom`, `tag`, `createdAt`), щоб shell-автоматизація могла читати їх без розпакування вкладених обгорток.
...
- `provenance` має значення `server-resolved-github-import` лише коли ClawHub розв’язав і зберіг repo/ref/commit/path GitHub під час публікації або імпорту; інакше має значення `unavailable`.

### `POST /api/v1/skills/-/security-verdicts`

Повертає поточні компактні вердикти безпеки для точних версій навичок. Ця
точка колекції призначена для клієнтів, які вже знають, які встановлені
...
- Результати надаються для кожного елемента; одна відсутня навичка або версія не призводить до помилки всієї відповіді.
...
- `security.signals.dependencyRegistry` збережено для сумісності відповіді v1, але сканер існування реєстру залежностей вилучено, і цей ключ завжди має значення `null`.
- Відсутність картки навички не впливає на `ok`, `decision` або `reasons` цієї точки; клієнти мають читати встановлений `skill-card.md` локально, коли їм потрібен вміст картки.
- Використовуйте `/verify`, коли потрібна оболонка перевірки картки для однієї навички, `/card`, коли потрібен згенерований markdown картки, і `/scan`, коли потрібні докладні дані сканерів.
...
- Записи Skill і надалі підтримуються реєстром Skill, і їх усе ще можна публікувати лише через `POST /api/v1/skills`.
- `POST /api/v1/packages` усе ще призначено лише для релізів code-plugin і bundle-plugin.
...
Уніфікований пошук у каталозі серед Skills + пакетів плагінів.
...
`trending` — це семиденна таблиця лідерів за встановленнями/завантаженнями й не використовує загальні підсумки за весь час.
На уніфікованій кінцевій точці `/api/v1/packages` вона призначена лише для плагінів; використовуйте
`/api/v1/skills?sort=trending` для каталогу Skill.
...
- Розміщені Skills містять найновіші збережені файли версії та перелічені в
  `_manifest.json` із `sourceRef: "public-clawhub"`.
- Поточні GitHub-підтримувані Skills зі скануванням `clean` або `suspicious` містять
...
Масовий експорт останніх публічних випусків плагінів для офлайн-аналізу.
...
- `cursor` (необов’язковий): курсор пагінації з попередньої відповіді.
- `family` (необов’язковий): `code-plugin` або `bundle-plugin`. Якщо не вказано, охоплює обидві
  родини плагінів.
...
- `_manifest.json` завжди міститься в корені ZIP.
- `_errors.json` додається, коли окремі плагіни або файли не вдалося
  експортувати.
...
- Застарілі псевдоніми фільтрів v1, задокументовані в `GET /api/v1/plugins`, також
  приймаються.
- Фільтрація за категоріями є справжнім API-фільтром на основі рядків дайджесту категорій
  плагінів, а не переписуванням пошукового запиту.
...
- `version.vtAnalysis`, `version.llmAnalysis` і `version.staticScan` додаються,
...
установлення. Це публічна поверхня споживання OpenClaw для вирішення, чи можна
...
- `trust.moderationState` може бути null. Він дорівнює `null`, коли ручної модерації
  випуску немає.
...
- Ця кінцева точка є точною до версії. Клієнти мають викликати її після розв’язання
  версії пакета, яку вони мають намір установити, а не лише після читання останніх
  метаданих пакета.
- Приватні пакети повертають `404`, якщо викликач не може читати власника-видавця.
- Ця кінцева точка навмисно вужча за кінцеві точки модерації для власників/модераторів.
  ...
  ідентичності репортерів, тексти звітів, приватні докази чи внутрішні графіки
  розгляду.
...
Кінцева точка модератора/адміністратора для черг перегляду випусків пакетів.
...
Повідомити про пакет для модераторського розгляду. Звіти мають рівень пакета й можуть
необов’язково бути пов’язані з версією. Вони потрапляють до черги модерації, але самі по собі не приховують автоматично й
...
`note` потрібне для `confirmed` і `dismissed`; його можна опустити під час
...
- Використовує кошик ліміту читання, а не кошик ліміту завантажень.
- Двійкові файли повертають `415`.
- Обмеження розміру файлу: 200 КБ.
- Очікувані сканування VirusTotal не блокують читання; шкідливі релізи все одно можуть утримуватися в інших місцях.
...
- Packument для пакетів зі scope підтримує як `/api/npm/@scope/name`, так і
...
Завантажує ZIP розміщеної версії skill або повертає передачу до джерела GitHub для
...
- Необов’язкове поле payload: `migrateOwner`. Коли `true` з `ownerHandle`, наявний
  skill може перейти до цього власника, якщо актор є адміністратором/власником у обох
  видавців: поточного й цільового. Без цього явного дозволу зміни власника
  відхиляються.
...
- Дозволені поля форми: `payload`, повторювані blob-об’єкти `files` або одне посилання на tarball `clawpack`.
  `clawpack` може бути blob-об’єктом `.tgz` або storage id, поверненим потоком
  upload-url. Публікації зі staged storage-id також мають містити
...
Коли `reason` присутнє, воно зберігається як примітка модерації skill і копіюється до журналу аудиту.
М’які видалення, ініційовані власником, резервують slug на 30 днів, після чого slug може бути заявлений
іншим видавцем. Відповідь видалення містить `slugReservedUntil`, коли це завершення терміну дії застосовується.
...
Лише для адміністратора. Гарантує існування org-видавця для handle. Якщо handle усе ще вказує на
...
Автентифіковане самообслуговуване створення org-видавця. Створює нового org-видавця й додає
...
Лише для адміністратора. Відновлює персонального видавця для перевіреного replacement GitHub OAuth principal
без редагування рядків облікових записів Convex Auth. Запит має назвати обидва незмінні ідентифікатори облікових записів
провайдера GitHub; змінні handle використовуються лише як запобіжник для оператора.
...
ідентифікаторами GitHub. Відновлення завершується без внесення змін, коли поточний персональний
...
- Обидві кінцеві точки потребують автентифікації токеном API та працюють лише для власника навички.
- `rename` зберігає попередній slug як псевдонім перенаправлення.
- `merge` приховує вихідний список і перенаправляє вихідний slug до цільового списку.
...
Заборонити користувача та остаточно видалити належні йому навички (лише модератор/адміністратор).
...
Скасувати заборону користувача та відновити придатні навички (лише адміністратор).
...
Змінити збережену причину наявної заборони без скасування заборони або відновлення
вмісту (лише адміністратор). За замовчуванням працює як тестовий запуск, якщо `dryRun` не дорівнює `false`.
...
які розміщують tarball ClawPack на цьому етапі, мають надсилати отриманий ідентифікатор сховища як
...
Якщо ви самостійно розгортаєте сервіс, надайте цей файл (або явно встановіть `CLAWHUB_REGISTRY`; застаріле `CLAWDHUB_REGISTRY`).
