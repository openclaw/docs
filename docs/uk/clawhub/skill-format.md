...
Вебімпортер GitHub суворіший за локальну публікацію/синхронізацію. Він виявляє лише файли
...
Оголосіть runtime-вимоги вашої навички в `metadata.openclaw` (псевдоніми: `metadata.clawdbot`, `metadata.clawdis`).
...
Використовуйте `requires.env` для змінних середовища, які мають бути наявні до запуску навички. Використовуйте `envVars`, коли потрібні метадані для окремих змінних, зокрема необов’язкових змінних із `required: false`.
...
Якщо ваша навичка потребує встановлених залежностей, оголосіть їх у масиві `install`:
...
- Текст для вбудовування включає `SKILL.md` + до приблизно 40 файлів, що не є `.md` (ліміт застосовується за можливості).

## Slugs

- Типово виводяться з назви папки.
- Області пакетів мають точно відповідати ідентифікатору видавця ClawHub. Ідентифікатори видавців можуть містити малі літери, цифри, дефіси, крапки та підкреслення; вони мають починатися й закінчуватися малою літерою або цифрою.
...
- Не додавайте конфліктні ліцензійні умови в `SKILL.md`; ClawHub не підтримує перевизначення ліцензій для окремих навичок.

## Платні навички

- ClawHub не підтримує платні навички, ціни для окремих навичок, платний доступ або розподіл доходу.
