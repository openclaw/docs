...
x-i18n:
    generated_at: "2026-06-30T22:32:01Z"
    model: gpt-5.5
...
файли `SKILL.md` або застарілі файли `skills.md` у публічних репозиторіях, що не є форками й належать
...
---
name: my-skill
description: Короткий опис того, що робить ця навичка.
version: 1.0.0
---
```
...
---
name: my-skill
description: Керування завданнями через Todoist API.
metadata:
  openclaw:
    requires:
      env:
  ...
  openclaw:
    ...
    envVars:
      ...
      - name: TODOIST_PROJECT_ID
        required: false
        description: Необов’язковий типовий ID проєкту, коли користувач не вказує його.
```
...
metadata:
  openclaw:
    ...
    envVars:
      ...
      - name: TODOIST_PROJECT_ID
        required: false
        description: Необов’язковий типовий ID проєкту.
    emoji: "\u2705"
...
- Текст для вбудовування включає `SKILL.md` + до приблизно 40 файлів не `.md` (обмеження застосовується за можливості).
...
- Слаги пакетів мають бути в нижньому регістрі й сумісними з npm, наприклад `@example.tools/demo-plugin` або `demo-plugin`.
