...
- `.clawhubignore` (шаблони ігнорування для публікації/синхронізації, застаріле `.clawdhubignore`)
...
Оголосіть необов’язкові змінні середовища в `metadata.openclaw.envVars` і встановіть `required: false`. Не додавайте необов’язкові записи до `requires.env`, тому що `requires.env` означає, що навичка не може запускатися без них.
...
- Текст для вбудовування включає `SKILL.md` + до приблизно 40 файлів не `.md` (ліміт застосовується за можливості).
...
- Не додавайте метадані ціноутворення до `SKILL.md`; це не частина формату навички й не зробить опубліковану навичку платною.
