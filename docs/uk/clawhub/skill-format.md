...
read_when:
    - Публікація Skills
    - Налагодження збоїв публікування/синхронізації
summary: Формат папки Skills, обов’язкові файли, дозволені типи файлів, обмеження.
x-i18n:
    generated_at: "2026-05-13T02:51:49Z"
    model: gpt-5.5
    provider: openai
...
- `.clawhubignore` (шаблони ігнорування для publish/sync, застарілий `.clawdhubignore`)
...
Publish приймає лише “текстові” файли.
...
- Текст для embedding включає `SKILL.md` + до приблизно 40 файлів не `.md` (ліміт за принципом найкращого зусилля).
