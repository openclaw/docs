...
x-i18n:
    generated_at: "2026-07-04T06:48:24Z"
    model: gpt-5.5
...
файли `SKILL.md` або застарілі `skills.md` у публічних репозиторіях, які не є форками і належать
...
| `envVars`          | `array`    | Оголошення змінних середовища з `name`, необов’язковим `required` і необов’язковим `description`. Задайте `required: false` для необов’язкових env vars. |
...
Оголошуйте необов’язкові змінні середовища в `metadata.openclaw.envVars` і задавайте `required: false`. Не додавайте необов’язкові записи до `requires.env`, тому що `requires.env` означає, що навичка не може працювати без них.
...
- Загальний розмір бандла: 50 МБ.
- Текст для embedding містить `SKILL.md` + до приблизно 40 файлів, що не є `.md` (обмеження за принципом найкращої спроби).
