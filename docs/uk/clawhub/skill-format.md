...
x-i18n:
    generated_at: "2026-07-01T20:33:11Z"
    model: gpt-5.5
...
файли `SKILL.md` або застарілі `skills.md` у публічних, не форкнутих репозиторіях, що належать
...
Оголосіть вимоги до середовища виконання вашого Skill у `metadata.openclaw` (псевдоніми: `metadata.clawdbot`, `metadata.clawdis`).
...
Використовуйте `requires.env` для змінних середовища, які мають бути наявні перед запуском Skill. Використовуйте `envVars`, коли вам потрібні метадані для окремих змінних, зокрема необов’язкові змінні з `required: false`.
...
- Типи вмісту, що починаються з `text/`, розглядаються як текст; плюс невеликий список дозволених типів (JSON/YAML/TOML/JS/TS/Markdown/SVG).

Обмеження (на боці сервера):

- Загальний розмір пакета: 50MB.
- Текст для вбудовування включає `SKILL.md` + до приблизно 40 файлів не `.md` (обмеження за найкращою спробою).

## Слаги

- Типово утворюються з назви папки.
- Області пакетів мають точно збігатися з ідентифікатором видавця ClawHub. Ідентифікатори видавців можуть використовувати малі літери, цифри, дефіси, крапки та підкреслення; вони мають починатися й закінчуватися малою літерою або цифрою.
...
- Будь-хто може використовувати, змінювати та поширювати опубліковані skills, зокрема комерційно.
- Зазначення авторства не потрібне.
- Не додавайте суперечливі ліцензійні умови в `SKILL.md`; ClawHub не підтримує перевизначення ліцензії для окремих skills.
