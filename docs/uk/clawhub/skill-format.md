...
Вебімпортер GitHub суворіший, ніж локальна публікація/синхронізація. Він виявляє лише файли
...
Оголошуйте необов’язкові змінні середовища в `metadata.openclaw.envVars` і задавайте `required: false`. Не додавайте необов’язкові записи до `requires.env`, оскільки `requires.env` означає, що навичка не може запускатися без них.
