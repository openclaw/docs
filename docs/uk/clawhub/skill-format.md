...
`SKILL.md` або застарілі `skills.md` у публічних репозиторіях, які не є форками та належать
обліковому запису GitHub, з якого виконано вхід. Він не імпортує приватні репозиторії, форки,
...
Оголошуйте вимоги середовища виконання вашого skill у `metadata.openclaw` (псевдоніми: `metadata.clawdbot`, `metadata.clawdis`).
...
Якщо ваш skill потребує встановлених залежностей, оголосіть їх у масиві `install`:
...
- Текст для embedding включає `SKILL.md` + до приблизно 40 файлів не з розширенням `.md` (best-effort cap).
...
- Будь-хто може використовувати, змінювати та повторно поширювати опубліковані skills, зокрема комерційно.
...
- ClawHub не підтримує платні skills, ціноутворення для окремих skills, paywall або розподіл доходу.
- Не додавайте метадані ціни до `SKILL.md`; це не є частиною формату skill і не зробить опублікований skill платним.
- Якщо ваш skill інтегрується з платним стороннім сервісом, чітко задокументуйте зовнішню вартість і потрібний обліковий запис в інструкціях skill та оголошеннях env (`requires.env` для обов’язкових змінних або `envVars` з `required: false` для необов’язкових змінних).
