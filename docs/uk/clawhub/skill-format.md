...
x-i18n:
    generated_at: "2026-07-02T08:45:32Z"
    model: gpt-5.5
...
- `description` використовується як підсумок навички в UI/пошуку.
...
Оголосіть необов’язкові змінні середовища в `metadata.openclaw.envVars` і встановіть `required: false`. Не додавайте необов’язкові записи до `requires.env`, бо `requires.env` означає, що навичка не може працювати без них.
...
- Типи вмісту, що починаються з `text/`, розглядаються як текст; також є невеликий список дозволених типів (JSON/YAML/TOML/JS/TS/Markdown/SVG).

Обмеження (на боці сервера):

- Загальний розмір пакета: 50 МБ.
- Текст для вбудовування включає `SKILL.md` + до приблизно 40 файлів не з розширенням `.md` (орієнтовне обмеження).
...
- Slugs пакетів мають бути в нижньому регістрі й сумісні з npm, наприклад `@example.tools/demo-plugin` або `demo-plugin`.
