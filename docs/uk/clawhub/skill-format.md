...
x-i18n:
    generated_at: "2026-07-04T18:18:25Z"
    model: gpt-5.5
...
| `envVars`          | `array`    | Оголошення змінних середовища з `name`, необов’язковим `required` і необов’язковим `description`. Задайте `required: false` для необов’язкових змінних середовища. |
...
Оголосіть необов’язкові змінні середовища в `metadata.openclaw.envVars` і задайте `required: false`. Не додавайте необов’язкові записи до `requires.env`, тому що `requires.env` означає, що навичка не може працювати без них.
...
- Список дозволених розширень наведено в `packages/schema/src/textFiles.ts` (`TEXT_FILE_EXTENSIONS`).
- Файли скриптів усе одно скануються після завантаження; файли PowerShell `.ps1`, `.psm1` і `.psd1` приймаються як текст.
- Типи вмісту, що починаються з `text/`, вважаються текстом; також діє невеликий список дозволених форматів (JSON/YAML/TOML/JS/TS/Markdown/SVG).

Обмеження (на боці сервера):

- Загальний розмір пакета: 50MB.
- Текст для embedding включає `SKILL.md` + до приблизно 40 файлів, що не є `.md` (обмеження застосовується за можливості).
...
- Slug пакетів мають бути в нижньому регістрі й безпечними для npm, наприклад `@example.tools/demo-plugin` або `demo-plugin`.
