...
x-i18n:
    generated_at: "2026-07-03T02:55:20Z"
    model: gpt-5.5
...
файли `SKILL.md` або застарілі `skills.md` у публічних репозиторіях, що не є форками і належать
...
Використовуйте `requires.env` для змінних середовища, які мають бути наявні до запуску навички. Використовуйте `envVars`, коли потрібні метадані для кожної змінної, зокрема необов’язкових змінних із `required: false`.
...
| `envVars`          | `array`    | Оголошення змінних середовища з `name`, необов’язковим `required` і необов’язковим `description`. Установіть `required: false` для необов’язкових env vars. |
...
- Текст для embedding включає `SKILL.md` + до приблизно 40 файлів не `.md` (обмеження за принципом best-effort).

## Slugs

- За замовчуванням виводяться з назви папки.
- Області пакетів мають точно збігатися з ClawHub publisher handle. Publisher handles можуть містити малі літери, цифри, дефіси, крапки й підкреслення; вони мають починатися й закінчуватися малою літерою або цифрою.
- Package slugs мають бути в нижньому регістрі та безпечними для npm, наприклад `@example.tools/demo-plugin` або `demo-plugin`.
...
- Не додавайте суперечливі умови ліцензії в `SKILL.md`; ClawHub не підтримує перевизначення ліцензії для окремих навичок.
