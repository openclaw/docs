...
x-i18n:
    generated_at: "2026-07-02T17:46:48Z"
    model: gpt-5.5
...
`SKILL.md` або застарілі `skills.md` у публічних репозиторіях, які не є форками й належать
...
Оголошуйте необов’язкові змінні середовища в `metadata.openclaw.envVars` і задавайте `required: false`. Не додавайте необов’язкові записи до `requires.env`, оскільки `requires.env` означає, що навичка не може працювати без них.
...
- Текст для вбудовування включає `SKILL.md` + до приблизно 40 файлів, що не є `.md` (обмеження за принципом найкращих зусиль).
