...
x-i18n:
    generated_at: "2026-07-02T01:14:21Z"
    model: gpt-5.5
...
| `requires.config`  | `string[]` | Шляхи до файлів конфігурації, які читає ваша навичка.                                                                                            |
...
- Текст для вбудовування містить `SKILL.md` + до приблизно 40 файлів не `.md` (обмеження з найкращим зусиллям).

## Slugs

- За замовчуванням виводяться з назви папки.
- Області пакунків мають точно збігатися з ідентифікатором видавця ClawHub. Ідентифікатори видавців можуть використовувати малі літери, цифри, дефіси, крапки й підкреслення; вони мають починатися та закінчуватися малою літерою або цифрою.
- Slugs пакунків мають бути в нижньому регістрі та безпечними для npm, наприклад `@example.tools/demo-plugin` або `demo-plugin`.
