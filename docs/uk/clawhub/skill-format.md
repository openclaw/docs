...
x-i18n:
    generated_at: "2026-06-28T05:07:35Z"
    model: gpt-5.5
...
Використовуйте `requires.env` для змінних середовища, які мають бути наявні перед запуском навички. Використовуйте `envVars`, коли потрібні метадані для окремих змінних, зокрема необов’язкові змінні з `required: false`.
...
Оголошуйте необов’язкові змінні середовища в `metadata.openclaw.envVars` і встановлюйте `required: false`. Не додавайте необов’язкові записи до `requires.env`, оскільки `requires.env` означає, що навичка не може працювати без них.
...
- Текст для вбудовування включає `SKILL.md` + до приблизно 40 файлів не `.md` (ліміт за принципом найкращого зусилля).

## Slugs

- Типово виводяться з назви папки.
- Області пакетів мають точно збігатися з handle видавця ClawHub. Handles видавців можуть містити малі літери, цифри, дефіси, крапки та підкреслення; вони мають починатися й закінчуватися малою літерою або цифрою.
...
- Не додавайте конфліктні ліцензійні умови в `SKILL.md`; ClawHub не підтримує перевизначення ліцензії для окремих навичок.

## Платні навички

- ClawHub не підтримує платні навички, ціни для окремих навичок, paywalls або розподіл доходу.
- Не додавайте цінові метадані до `SKILL.md`; це не є частиною формату навички й не зробить опубліковану навичку платною.
- Якщо ваша навичка інтегрується з платним стороннім сервісом, чітко задокументуйте зовнішню вартість і потрібний обліковий запис в інструкціях навички та оголошеннях середовища (`requires.env` для обов’язкових змінних або `envVars` з `required: false` для необов’язкових змінних).
