...
- `description` використовується як короткий опис Skill в інтерфейсі/пошуку.
...
Використовуйте `requires.env` для змінних середовища, які мають бути наявні перед запуском Skill. Використовуйте `envVars`, коли потрібні метадані для окремих змінних, зокрема необов’язкові змінні з `required: false`.
...
- Текст для embedding містить `SKILL.md` + до приблизно 40 файлів, що не є `.md` (обмеження за принципом best-effort).
...
- Усі Skills, опубліковані в ClawHub, ліцензуються за `MIT-0`.
...
- Не додавайте конфліктні ліцензійні умови в `SKILL.md`; ClawHub не підтримує перевизначення ліцензії для окремих Skills.

## Платні Skills

- ClawHub не підтримує платні Skills, ціноутворення для окремих Skills, paywalls або розподіл доходу.
