...
x-i18n:
    generated_at: "2026-07-05T05:34:14Z"
    model: gpt-5.5
...
Оголошуйте вимоги до середовища виконання вашої навички в `metadata.openclaw` (псевдоніми: `metadata.clawdbot`, `metadata.clawdis`).
...
- Текст для вбудовування містить `SKILL.md` + до приблизно 40 файлів, що не є `.md` (обмеження за принципом найкращої спроби).
...
- Не додавайте конфліктні умови ліцензування в `SKILL.md`; ClawHub не підтримує перевизначення ліцензій для окремих навичок.

## Платні навички

- ClawHub не підтримує платні навички, ціни для окремих навичок, платний доступ або розподіл доходу.
- Не додавайте метадані ціноутворення до `SKILL.md`; вони не є частиною формату навички й не зроблять опубліковану навичку платною.
- Якщо ваша навичка інтегрується з платною сторонньою службою, чітко задокументуйте зовнішню вартість і потрібний обліковий запис в інструкціях навички та оголошеннях середовища (`requires.env` для обов’язкових змінних або `envVars` з `required: false` для необов’язкових змінних).
