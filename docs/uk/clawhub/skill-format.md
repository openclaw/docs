...
Аналіз безпеки ClawHub перевіряє, чи те, що оголошує ваша навичка, відповідає тому, що вона фактично робить. Якщо ваш код посилається на `TODOIST_API_KEY`, але ваш frontmatter не оголошує його в `requires.env`, `primaryEnv` або `envVars`, аналіз позначить невідповідність метаданих. Точність оголошень допомагає вашій навичці пройти перевірку й допомагає користувачам зрозуміти, що вони встановлюють.
...
- Типи вмісту, що починаються з `text/`, розглядаються як текст; додатково діє невеликий список дозволених типів (JSON/YAML/TOML/JS/TS/Markdown/SVG).
