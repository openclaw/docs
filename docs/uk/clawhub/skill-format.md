...
## Метадані фронтматера

Метадані навички оголошуються в YAML-фронтматері на початку вашого `SKILL.md`. Це повідомляє реєстру (і аналізу безпеки), що потрібно вашій навичці для запуску.
...
Оголосіть необов’язкові змінні середовища в `metadata.openclaw.envVars` і встановіть `required: false`. Не додавайте необов’язкові записи до `requires.env`, оскільки `requires.env` означає, що навичка не може працювати без них.
...
- Якщо ваша навичка інтегрується з платною сторонньою службою, чітко задокументуйте зовнішню вартість і потрібний обліковий запис в інструкціях навички та оголошеннях змінних середовища (`requires.env` для обов’язкових змінних або `envVars` із `required: false` для необов’язкових змінних).
