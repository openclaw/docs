...
x-i18n:
    generated_at: "2026-05-01T04:46:32Z"
    model: gpt-5.5
    provider: openai
...
Установіть локальні аудіозалежності та налаштуйте бекенд-провайдер голосу
реального часу. OpenAI є стандартним; Google Gemini Live також працює з
...
закріплення вузла і, для приєднань Chrome у реальному часі, аудіоміст BlackHole/SoX
та перевірки відкладеного вступу в реальному часі. Для приєднань лише для спостереження перевірте той самий
...
Plugin `voice-call` і облікові дані Twilio. Сприймайте будь-яку перевірку `ok: false`
як блокер для перевіреного транспорту й режиму, перш ніж просити агента
приєднатися. Використовуйте `openclaw googlemeet setup --json` для скриптів або машинно-читабельного
виводу. Використовуйте `--transport chrome`, `--transport chrome-node` або `--transport twilio`,
щоб попередньо перевірити конкретний транспорт до того, як агент його спробує.
...
надання OpenClaw дозволу на мікрофон/камеру та обходить шлях Meet **Use
...
`realtimeReady`, `audioInputActive`, `audioOutputActive`, останні позначки часу вводу/виводу,
...
Локальні приєднання Chrome використовують браузерний профіль OpenClaw із виконаним входом. Режим реального часу
...
вузловий хост у VM. Один раз увімкніть комплектний Plugin на VM, щоб вузол
оголошував команду Chrome:

Що де працює:

- Хост Gateway: OpenClaw Gateway, робочий простір агента, ключі моделі/API, провайдер
  реального часу та конфігурація Google Meet Plugin.
...
Установіть або оновіть OpenClaw у VM, а потім увімкніть там комплектний Plugin:
...
незашифрованого WebSocket, якщо ви явно не дозволите це для цієї довіреної приватної мережі:
...
підключено кілька здатних вузлів, задайте `chromeNode.node` як id вузла,
...
- Chrome відкривається, але не може приєднатися: увійдіть у профіль браузера всередині VM або
  ...
  браузерну автоматизацію OpenClaw через проксі браузера вузла; переконайтеся, що конфігурація браузера
...
- Дубльовані вкладки Meet: залиште `chrome.reuseExistingTab: true` увімкненим. OpenClaw
  ...
  створення браузерної зустрічі повторно використовує вкладку `https://meet.google.com/new`
  або запиту облікового запису Google, що вже виконується, перед відкриттям іншої.
...
Стандартна конфігурація Chrome realtime використовує два зовнішні інструменти:

- `sox`: утиліта командного рядка для аудіо. Plugin використовує явні команди
  пристрою CoreAudio для стандартного аудіомоста PCM16 24 кГц.
...
Маршрутизуйте аудіо мікрофона й динаміків Chrome через локальний аудіоміст OpenClaw.
...
телефонний дозвін. Google Meet має надати номер дозвону та PIN для
...
Коли делегування Twilio під’єднане, `googlemeet setup` містить успішні перевірки
...
перейти на браузерну автоматизацію. Налаштуйте OAuth, коли потрібні офіційне створення
через API, визначення простору або попередні перевірки Meet Media API.
...
все одно приєднуються через профіль Chrome із виконаним входом, BlackHole/SoX і під’єднаний
...
Якщо вам потрібні лише браузерні приєднання через Chrome, повністю пропустіть OAuth.
...
Якщо присутні і значення конфігурації, і значення середовища, Plugin спочатку
використовує конфігурацію, а потім fallback до середовища.

Згода OAuth охоплює створення просторів Meet, доступ на читання просторів Meet і доступ
...
Щоб також довести ввімкнення Google Meet API і область доступу `spaces.create`, запустіть
...
не має потрібної області доступу або обліковий запис Google не може отримати доступ до цього простору
...
Після створення записів конференції Meet виведіть артефакти зустрічі та відвідуваність:
...
З `--meeting` команди `artifacts` і `attendance` за замовчуванням використовують найновіший запис конференції. Передайте `--all-conference-records`, якщо потрібні всі збережені записи для цієї зустрічі.
...
`--today` шукає в сьогоднішньому календарі `primary` подію Calendar із посиланням Google Meet. Використовуйте `--event <query>` для пошуку відповідного тексту події та `--calendar <id>` для неосновного календаря. Пошук у календарі потребує свіжого входу OAuth, який включає readonly-область для подій Calendar. `calendar-events` попередньо показує відповідні події Meet і позначає подію, яку виберуть `latest`, `artifacts`, `attendance` або `export`.
...
`artifacts` повертає метадані запису конференції, а також метадані ресурсів учасників, записів, транскриптів, структурованих записів транскрипту та розумних нотаток, коли Google надає їх для зустрічі. Використовуйте `--no-transcript-entries`, щоб пропустити пошук записів для великих зустрічей. `attendance` розгортає учасників у рядки сесій учасників із часом першої й останньої появи, загальною тривалістю сесії, прапорцями запізнення/раннього виходу та об’єднанням дубльованих ресурсів учасників за користувачем, що ввійшов, або відображуваним іменем. Передайте `--no-merge-duplicates`, щоб залишити сирі ресурси учасників окремо, `--late-after-minutes`, щоб налаштувати визначення запізнення, і `--early-before-minutes`, щоб налаштувати визначення раннього виходу.

`export` записує папку, що містить `summary.md`, `attendance.csv`, `transcript.md`, `artifacts.json`, `attendance.json` і `manifest.json`. `manifest.json` фіксує вибраний вхід, параметри експорту, записи конференції, вихідні файли, лічильники, джерело токена, подію Calendar, якщо її було використано, і будь-які попередження про часткове отримання. Передайте `--zip`, щоб також записати переносний архів поруч із папкою. Передайте `--include-doc-bodies`, щоб експортувати текст пов’язаних транскриптів і розумних нотаток Google Docs через Google Drive `files.export`; для цього потрібен свіжий вхід OAuth, який включає readonly-область Drive Meet. Без `--include-doc-bodies` експорти містять лише метадані Meet і структуровані записи транскрипту. Якщо Google повертає часткову помилку артефакту, наприклад помилку списку розумних нотаток, запису транскрипту або тіла документа Drive, зведення й manifest зберігають попередження замість збою всього експорту. Використовуйте `--dry-run`, щоб отримати ті самі дані артефактів/відвідуваності та вивести JSON manifest без створення папки чи ZIP. Це корисно перед записом великого експорту або коли агенту потрібні лише лічильники, вибрані записи та попередження.
...
Запустіть захищену живу перевірку на реальній збереженій зустрічі:
...
Середовище живої перевірки:

- `OPENCLAW_LIVE_TEST=1` вмикає захищені живі тести.
- `OPENCLAW_GOOGLE_MEET_LIVE_MEETING` вказує на збережений Meet URL, код або `spaces/{id}`.
...
Якщо резервний браузерний варіант натрапляє на вхід Google або блокування дозволів Meet до того, як зможе створити URL, метод Gateway повертає відповідь із помилкою, а інструмент `google_meet` повертає структуровані деталі замість звичайного рядка:
...
Створення Meet за замовчуванням виконує приєднання. Транспорт Chrome або Chrome-node все одно потребує профілю Google Chrome із виконаним входом, щоб приєднатися через браузер. Якщо з профілю виконано вихід, OpenClaw повідомляє `manualActionRequired: true` або помилку резервного браузерного варіанта й просить оператора завершити вхід Google перед повторною спробою.
...
Спільний шлях Chrome у реальному часі потребує лише ввімкненого Plugin, BlackHole, SoX і ключа постачальника голосу backend у реальному часі. OpenAI є типовим; установіть `realtime.provider: "google"`, щоб використовувати Google Gemini Live:
...
- `chrome.waitForInCallMs: 20000`: чекати, доки вкладка Meet повідомить про перебування у виклику, перед запуском вступу в реальному часі
- `chrome.audioFormat: "pcm16-24khz"`: аудіоформат пари команд. Використовуйте `"g711-ulaw-8khz"` лише для застарілих/користувацьких пар команд, які все ще видають телефонне аудіо.
...
- `realtime.introMessage`: коротка усна перевірка готовності, коли міст реального часу підключається; установіть `""`, щоб приєднатися мовчки
- `realtime.agentId`: необов’язковий ідентифікатор агента OpenClaw для `openclaw_agent_consult`; за замовчуванням `main`
...
`voiceCall.enabled` за замовчуванням має значення `true`; з транспортом Twilio він делегує фактичний PSTN-виклик і DTMF до Plugin Voice Call. Якщо `voice-call` не ввімкнено, Google Meet усе ще може перевірити й записати план набору, але не може виконати Twilio-виклик.
...
VM. В обох випадках модель реального часу й `openclaw_agent_consult` працюють на хості
...
запустити відому фразу й повернути стан справності `inCall`, коли хост Chrome може
...
`status` містить стан справності Chrome, коли доступно:
...
Інструмент консультації запускає звичайного агента OpenClaw у фоновому режимі з контекстом нещодавньої
стенограми зустрічі й повертає стислу усну відповідь у голосовий сеанс реального часу.
Після цього голосова модель може промовити цю відповідь назад у зустріч.
...
лінія Meet має консультуватися з окремим робочим простором агента OpenClaw, типовими моделями,
політикою інструментів, пам’яттю та історією сеансів.

`realtime.toolPolicy` керує запуском консультації:

- `safe-read-only`: надає інструмент консультації й обмежує звичайного агента до
  `read`, `web_search`, `web_fetch`, `x_search`, `memory_search` і
  `memory_get`.
- `owner`: надає інструмент консультації й дозволяє звичайному агенту використовувати звичайну
...
Щоб примусово запустити усну перевірку готовності після повного входу Chrome у виклик:

```bash
openclaw googlemeet speak meet_... "Say exactly: I'm here and listening."
```

Для повної перевірки join-and-speak:
...
Для перевірки Twilio використовуйте зустріч, яка надає дані телефонного набору:
...
- Надати Chrome дозволи на мікрофон/камеру, коли з’явиться нативний запит дозволу
  Chrome.
- Закрити або відновити застряглий діалог дозволів Meet.
...
чекати справжнього стану зустрічі. Для браузерного резервного режиму лише створення OpenClaw
...
коли облікові дані OAuth налаштовані. Без облікових даних OAuth він переходить до резервного
браузера на закріпленому вузлі Chrome. Підтвердьте:
...
- Для браузерного резервного режиму: `defaultTransport: "chrome-node"` і
  ...
  `googlemeet.chrome`.
- Для браузерного резервного режиму: профіль Chrome OpenClaw на цьому вузлі виконано вхід
  у Google і він може відкрити `https://meet.google.com/new`.
- Для браузерного резервного режиму: повторні спроби повторно використовують наявну вкладку
...
- Для браузерного резервного режиму: якщо інструмент повертає `manualActionRequired: true`, використовуйте
...
- Для браузерного резервного режиму: якщо Meet показує "Do you want people to hear you in the
  meeting?", залиште вкладку відкритою. OpenClaw має натиснути **Use microphone** або, для
  резервного режиму лише створення, **Continue without microphone** через автоматизацію
  браузера й продовжити чекати згенерований URL Meet. Якщо він не може, помилка
...
Використовуйте `mode: "realtime"` для прослуховування/відповіді мовленням. `mode: "transcribe"` навмисно
не запускає дуплексний голосовий міст реального часу. `googlemeet test-speech`
завжди перевіряє шлях реального часу й повідомляє, чи були помічені байти виходу моста
...
висловлювання, але OpenClaw не побачив, щоб нові байти виходу дійшли до аудіомоста
Chrome.
...
вхідного/вихідного аудіо, останні часові мітки аудіо, лічильники байтів і URL браузера.
...
`twilio-voice-call-plugin` дає збій, коли `voice-call` не дозволено або не ввімкнено.
Додайте його до `plugins.allow`, увімкніть `plugins.entries.voice-call` і перезавантажте
Gateway.

`twilio-voice-call-credentials` дає збій, коли в бекенді Twilio немає SID
облікового запису, токена автентифікації або номера абонента. Налаштуйте їх на хості Gateway:
