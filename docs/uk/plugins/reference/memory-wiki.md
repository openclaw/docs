---
read_when:
    - Ви встановлюєте, налаштовуєте або проводите аудит Plugin memory-wiki
summary: Постійний компілятор вікі та зручне для Obsidian сховище знань для OpenClaw.
title: Plugin Memory Wiki
x-i18n:
    generated_at: "2026-05-02T15:19:46Z"
    model: gpt-5.5
    provider: openai
    source_hash: 461c9e3e52db968fc98b058c70070585f272ad27024392a6af0ce3d2f0237635
    source_path: plugins/reference/memory-wiki.md
    workflow: 16
    postprocess_version: locale-links-v1
---

# Plugin Memory Wiki

Постійний компілятор вікі та сховище знань, зручне для Obsidian, для OpenClaw.

## Поширення

- Пакет: `@openclaw/memory-wiki`
- Шлях встановлення: входить до OpenClaw

## Поверхня

контракти: інструменти; Skills

## Пов’язані документи

- [memory-wiki](/uk/plugins/memory-wiki)
