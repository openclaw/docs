...
окрему довідкову сторінку, або документується разом із командою, для якої є псевдонімом; цей
індекс містить команди, глобальні прапорці та правила оформлення виводу, які
...
| Повідомлення й агенти | [`message`](/uk/cli/message) · [`agent`](/uk/cli/agent) · [`agents`](/uk/cli/agents) · [`acp`](/uk/cli/acp) · [`mcp`](/uk/cli/mcp)                                                                                                                         |
...
| Підключення та канали | [`pairing`](/uk/cli/pairing) · [`qr`](/uk/cli/qr) · [`channels`](/uk/cli/channels)                                                                                                                                                                     |
...
| Plugins (необов’язкові) | [`voicecall`](/uk/cli/voicecall) (якщо встановлено)                                                                                                                                                                                          |
...
| `--container <name>`   | Спрямовує виконання до іменованого контейнера                         |
| `--no-color`           | Вимикає кольори ANSI (`NO_COLOR=1` також підтримується)               |
| `--update`             | Скорочення для [`openclaw update`](/uk/cli/update) (лише для встановлень із джерела) |
...
- Гіперпосилання OSC-8 відображаються як клікабельні посилання там, де це підтримується; інакше
  CLI повертається до звичайних URL-адрес.
...
- `/debug` — перевизначення конфігурації лише для середовища виконання (пам’ять, не диск; потрібен `commands.debug: true`).

## Відстеження використання

`openclaw status --usage` і Control UI показують використання/квоту провайдера, коли
доступні облікові дані OAuth/API. Дані надходять безпосередньо з кінцевих точок використання провайдера
і нормалізуються до `X% left`. Провайдери з поточними вікнами використання:
Anthropic, GitHub Copilot, Gemini CLI, OpenAI Codex, MiniMax,
Xiaomi і z.ai.

Докладніше див. у [Відстеження використання](/uk/concepts/usage-tracking).
