...
read_when:
    ...
    - Ви хочете керувати браузером, що працює на іншому пристрої, через хост Node
...
title: Браузер
x-i18n:
    generated_at: "2026-04-25T10:48:59Z"
    model: gpt-5.4
    provider: openai
...
Якщо `start` завершується помилкою `not reachable after start`, спочатку перевірте готовність CDP. Якщо `start` і `tabs` успішні, але `open` або `navigate` не спрацьовують, площина керування браузером справна, а збій зазвичай пов’язаний із політикою SSRF навігації.
...
- Для профілів `attachOnly` і віддаленого CDP `openclaw browser stop` закриває активний сеанс керування та скидає тимчасові перевизначення емуляції, навіть якщо OpenClaw не запускав процес браузера самостійно.
- Для локальних керованих профілів `openclaw browser stop` зупиняє породжений процес браузера.
- `openclaw browser start --headless` застосовується лише до цього запиту `start` і лише коли OpenClaw запускає локальний керований браузер. Це не переписує `browser.headless` або конфігурацію профілю й не має ефекту для браузера, який уже запущено.
- На хостах Linux без `DISPLAY` або `WAYLAND_DISPLAY` локальні керовані профілі автоматично працюють у headless-режимі, якщо лише `OPENCLAW_BROWSER_HEADLESS=0`, `browser.headless=false` або `browser.profiles.<name>.headless=false` явно не вимагає видимого браузера.

## Якщо команда відсутня

Якщо `openclaw browser` — це невідома команда, перевірте `plugins.allow` у `~/.openclaw/openclaw.json`.
...
`browser.enabled=true` не повертає підкоманду CLI, якщо список дозволених плагінів виключає `browser`.

Пов’язано: [інструмент Browser](/uk/tools/browser#missing-browser-command-or-tool)
...
`tabs` спочатку повертає `suggestedTargetId`, потім стабільний `tabId`, наприклад `t1`, необов’язкову мітку та сирий `targetId`. Агентам слід передавати `suggestedTargetId` назад у `focus`, `close`, знімки та дії. Ви можете призначити мітку за допомогою `open --label`, `tab new --label` або `tab label`; також приймаються мітки, ідентифікатори вкладок, сирі ідентифікатори цілей і унікальні префікси ідентифікаторів цілей.
...
- Профілі `existing-session` / `user` підтримують знімки екрана сторінки та знімки з `--ref` із виводу знімка, але не підтримують знімки екрана CSS `--element`.
- `--labels` накладає на знімок екрана поточні посилання знімка.
- `snapshot --urls` додає виявлені адреси посилань до AI-знімків, щоб агенти могли вибирати прямі цілі навігації замість вгадування лише за текстом посилання.
...
Керовані профілі Chrome зберігають звичайні завантаження, запущені кліком, у каталог завантажень OpenClaw (`/tmp/openclaw/downloads` типово або налаштований тимчасовий кореневий каталог). Використовуйте `waitfordownload` або `download`, коли агенту потрібно дочекатися конкретного файлу й повернути його шлях; ці явні очікувачі перебирають на себе наступне завантаження.
...
- дії на основі знімків використовують ref, а не селектори CSS
- `browser.actionTimeoutMs` за замовчуванням задає для підтримуваних запитів `act` значення 60000 мс, коли виклики не вказують `timeoutMs`; `timeoutMs` для конкретного виклику все одно має пріоритет.
...
Якщо Gateway працює на іншому пристрої, ніж браузер, запустіть **node host** на пристрої, де є Chrome/Brave/Edge/Chromium. Gateway проксуватиме дії браузера до цього вузла (окремий сервер керування браузером не потрібен).

Використовуйте `gateway.nodes.browser.mode` для керування автоматичною маршрутизацією та `gateway.nodes.browser.node`, щоб закріпити конкретний Node, якщо підключено кілька.
