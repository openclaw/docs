...
title: Вебпошук
x-i18n:
    generated_at: "2026-05-02T05:03:48Z"
    model: gpt-5.5
    provider: openai
    source_hash: 488f6206d02e4b511ac3350072562a4713fa5c1e0fc180bb76e880eed7048d38
...
<Steps>
  <Step title="Choose a provider">
    ...
    не потребують ключа, інші використовують API-ключі. Докладніше дивіться на сторінках
    провайдерів нижче.
  </Step>
  <Step title="Configure">
    ...
    ```
    Це зберігає провайдера й усі потрібні облікові дані. Ви також можете задати змінну
...
<CardGroup cols={2}>
  ...
  <Card title="Gemini" icon="sparkles" href="/uk/tools/gemini-search">
    Відповіді, синтезовані AI, з цитуваннями через обґрунтування Google Search.
  </Card>
  <Card title="Grok" icon="zap" href="/uk/tools/grok-search">
    Відповіді, синтезовані AI, з цитуваннями через вебобґрунтування xAI.
  </Card>
  ...
  <Card title="Perplexity" icon="search" href="/uk/tools/perplexity-search">
    Структуровані результати з керуванням витягуванням вмісту й фільтрацією доменів.
  </Card>
...
Прямі моделі OpenAI Responses автоматично використовують розміщений OpenAI інструмент `web_search`, коли вебпошук OpenClaw увімкнено й не закріплено керованого провайдера. Це поведінка, що належить провайдеру, у вбудованому Plugin OpenAI, і вона застосовується лише до нативного API-трафіку OpenAI, а не до OpenAI-сумісних проксі-базових URL або маршрутів Azure. Установіть `tools.web.search.provider` на іншого провайдера, наприклад `brave`, щоб зберегти керований інструмент `web_search` для моделей OpenAI, або встановіть `tools.web.search.enabled: false`, щоб вимкнути і керований пошук, і нативний пошук OpenAI.
...
- `mode: "cached"` є типовим і рекомендованим налаштуванням
...
Якщо `provider` не задано, OpenClaw перевіряє провайдерів у цьому порядку й використовує
...
11. **Ollama Web Search** -- резервний варіант без ключа через ваш налаштований локальний хост Ollama, коли він доступний і виконано вхід через `ollama signin`; може повторно використовувати bearer-автентифікацію провайдера Ollama, коли хост її потребує, і може виконувати прямий пошук `https://ollama.com`, якщо налаштовано `OLLAMA_API_KEY` (порядок 110)
12. **SearXNG** -- `SEARXNG_BASE_URL` або `plugins.entries.searxng.config.webSearch.baseUrl` (порядок 200)

Якщо провайдера не виявлено, використовується Brave як резервний варіант (ви отримаєте помилку
про відсутній ключ із пропозицією налаштувати його).

<Note>
  Усі поля ключів провайдера підтримують об'єкти SecretRef. SecretRef в області Plugin
...
Специфічна для провайдера конфігурація (API-ключі, базові URL, режими) міститься в
...
після своєї спеціальної конфігурації вебпошуку й `GEMINI_API_KEY`. Приклади дивіться на
...
Це окремий подальший крок усередині шляху Grok, а не окремий вибір постачальника
...
<Warning>
  ...
  Firecrawl і Tavily підтримують лише `query` та `count` через `web_search`
  -- використовуйте їхні спеціалізовані інструменти для розширених параметрів.
</Warning>

## x_search

`x_search` запитує дописи X (колишній Twitter) за допомогою xAI та повертає
...
<Note>
  ...
  взаємодій з окремим дописом, як-от репости, відповіді, закладки або перегляди,
  віддавайте перевагу цільовому пошуку за точною URL-адресою допису або ID
  статусу. Широкі пошуки за ключовими словами можуть знайти потрібний допис, але
  повернути менш повні метадані окремого допису. Добрий шаблон: спочатку знайдіть
  допис, а потім виконайте другий запит `x_search`, зосереджений на цьому точному
  дописі.
</Note>
...
застарілого `tools.web.search.grok.baseUrl` і, нарешті, до публічної кінцевої
точки xAI.
...
| ---------------------------- | ------------------------------------------------------ |
| `query`                      | Пошуковий запит (обов’язково)                          |
| `allowed_x_handles`          | Обмежити результати конкретними X-іменами              |
| `excluded_x_handles`         | Виключити конкретні X-імена                            |
...
- [Web Fetch](/uk/tools/web-fetch) -- отримати URL-адресу та витягти читабельний вміст
