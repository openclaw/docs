...
title: Вебпошук
x-i18n:
    generated_at: "2026-05-07T15:15:42Z"
    model: gpt-5.5
    provider: openai
...
<Steps>
  <Step title="Виберіть провайдера">
    Виберіть провайдера й виконайте потрібне налаштування. Деякі провайдери
    не потребують ключів, а інші використовують API-ключі. Докладніше див.
  ...
  <Step title="Налаштуйте">
    ...
    ```
    Це зберігає провайдера й потрібні облікові дані. Ви також можете задати env
...
<CardGroup cols={2}>
  ...
  <Card title="Exa" icon="brain" href="/uk/tools/exa-search">
    Нейронний + ключовий пошук із видобуванням вмісту (виділення, текст, підсумки).
  </Card>
  <Card title="Firecrawl" icon="flame" href="/uk/tools/firecrawl">
    Структуровані результати. Найкраще поєднувати з `firecrawl_search` і `firecrawl_scrape` для глибокого видобування.
  </Card>
  ...
  <Card title="Kimi" icon="moon" href="/uk/tools/kimi-search">
    Відповіді, синтезовані ШІ, з цитуваннями через вебпошук Moonshot; незаземлені резервні чати явно завершуються помилкою.
  </Card>
  ...
  <Card title="Perplexity" icon="search" href="/uk/tools/perplexity-search">
    Структуровані результати з керуванням видобуванням вмісту й фільтрацією доменів.
  </Card>
  ...
  <Card title="Tavily" icon="globe" href="/uk/tools/tavily">
    Структуровані результати з глибиною пошуку, фільтрацією тем і `tavily_extract` для видобування URL.
  </Card>
</CardGroup>
...
| [Exa](/uk/tools/exa-search)                  | Структуровані + видобуті                                       | Нейронний/ключовий режим, дата, видобування вмісту | `EXA_API_KEY`                                                                           |
...
| [Kimi](/uk/tools/kimi-search)                | Синтезовані ШІ + цитування; помилка для незаземлених резервних чатів | --                                               | `KIMI_API_KEY` / `MOONSHOT_API_KEY`                                                     |
...
Прямі моделі OpenAI Responses автоматично використовують розміщений в OpenAI інструмент `web_search`, коли вебпошук OpenClaw увімкнено й не закріплено керованого провайдера. Це поведінка, що належить провайдеру, у вбудованому Plugin OpenAI, і вона застосовується лише до нативного трафіку OpenAI API, а не до OpenAI-сумісних проксі-базових URL або маршрутів Azure. Задайте для `tools.web.search.provider` іншого провайдера, наприклад `brave`, щоб зберегти керований інструмент `web_search` для моделей OpenAI, або задайте `tools.web.search.enabled: false`, щоб вимкнути і керований пошук, і нативний пошук OpenAI.

## Нативний вебпошук Codex

Моделі з підтримкою Codex можуть за бажанням використовувати провайдер-нативний інструмент Responses `web_search` замість керованої функції `web_search` OpenClaw.
...
- Керований `web_search` і надалі застосовується до моделей без Codex
...
довірених API-хостів провайдера OpenClaw дозволяє DNS-відповіді fake-IP від
Surge, Clash і sing-box у `198.18.0.0/15` та `fc00::/7` лише для цього імені хоста провайдера.
Інші приватні, loopback, link-local і metadata-призначення залишаються заблокованими.

Це автоматичне дозволення не застосовується до довільних URL `web_fetch`. Для
...
Списки провайдерів у документації та потоках налаштування впорядковані за алфавітом. Автовиявлення зберігає
окремий порядок пріоритету.

Якщо `provider` не задано, OpenClaw перевіряє провайдерів у такому порядку й використовує
першого готового:

Спершу провайдери з API:
...
11. **Ollama Web Search** -- резервний варіант без ключа через ваш налаштований локальний хост Ollama, коли він доступний і в ньому виконано вхід за допомогою `ollama signin`; може повторно використовувати bearer auth провайдера Ollama, коли хост цього потребує, і може викликати прямий пошук `https://ollama.com`, коли налаштовано з `OLLAMA_API_KEY` (порядок 110)
...
яка попросить налаштувати його).

<Note>
  Усі поля ключів провайдерів підтримують об’єкти SecretRef. SecretRefs у межах Plugin
  під `plugins.entries.<plugin>.config.webSearch.apiKey` розв’язуються для
  вбудованих API-провайдерів вебпошуку, зокрема Brave, Exa, Firecrawl,
  ...
  обрано через автовиявлення. У режимі автовиявлення OpenClaw розв’язує лише
...
після своєї спеціальної конфігурації вебпошуку та `GEMINI_API_KEY`. Приклади дивіться на
сторінках провайдерів.

`tools.web.search.provider` перевіряється за ідентифікаторами провайдерів вебпошуку,
оголошеними в маніфестах вбудованих і встановлених plugin. Помилка введення на кшталт `"brvae"`
спричиняє помилку перевірки конфігурації замість тихого повернення до автовиявлення. Якщо
...
- `web_fetch` поза sandbox може використовувати встановлені провайдери plugin, які оголошують
  `contracts.webFetchProviders`; sandbox-вибірки залишаються лише вбудованими
- наразі вбудованим провайдером web-fetch є Firecrawl, налаштований у
...
Це окремий наступний крок усередині шляху Grok, а не окремий вибір провайдера вебпошуку
верхнього рівня. Якщо ви виберете іншого провайдера, OpenClaw не
...
<Tabs>
  ...
  <Tab title="Змінна середовища">
    ...
    Для встановлення Gateway розмістіть її в `~/.openclaw/.env`.
...
<Warning>
  ...
  діапазони свіжості Brave вимагають і дати початку, і дати завершення.
  ...
  `date_before`, перетворюючи їх на часові діапазони Google Search grounding.
...
синтезовані AI відповіді з цитуваннями. Він приймає запити природною мовою й
...
<Note>
  У документації xAI `x_search` описано як такий, що підтримує пошук за ключовими словами, семантичний пошук, пошук користувачів
  і отримання тредів. Для статистики взаємодій з окремими дописами, як-от репости,
  відповіді, закладки або перегляди, надавайте перевагу цільовому пошуку точної URL-адреси допису
  або ідентифікатора статусу. Широкі пошуки за ключовими словами можуть знайти потрібний допис, але повернути менш
