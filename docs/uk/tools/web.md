...
read_when:
    ...
    - Ви хочете зрозуміти автоматичне виявлення та резервний перехід провайдера
...
x-i18n:
    generated_at: "2026-05-02T06:12:30Z"
    model: gpt-5.5
    provider: openai
...
<Info>
  ...
  сайтів із великою кількістю JS або входу в обліковий запис використовуйте [Веббраузер](/uk/tools/browser). Для
...
<CardGroup cols={2}>
  ...
  <Card title="Exa" icon="brain" href="/uk/tools/exa-search">
    Нейронний і ключовий пошук із витяганням вмісту (виділення, текст, підсумки).
  </Card>
  ...
  <Card title="Gemini" icon="sparkles" href="/uk/tools/gemini-search">
    Згенеровані ШІ відповіді з цитуваннями через прив’язку до Google Search.
  </Card>
  <Card title="Grok" icon="zap" href="/uk/tools/grok-search">
    Згенеровані ШІ відповіді з цитуваннями через веб-прив’язку xAI.
  </Card>
  <Card title="Kimi" icon="moon" href="/uk/tools/kimi-search">
    Згенеровані ШІ відповіді з цитуваннями через вебпошук Moonshot; резервні варіанти чату без прив’язки явно завершуються помилкою.
  </Card>
  ...
  <Card title="Tavily" icon="globe" href="/uk/tools/tavily">
    Структуровані результати з глибиною пошуку, фільтрацією за темами та `tavily_extract` для витягання URL.
  </Card>
</CardGroup>
...
| [Gemini](/uk/tools/gemini-search)            | Згенеровані ШІ + цитування                                     | --                                               | `GEMINI_API_KEY`                                                                        |
| [Grok](/uk/tools/grok-search)                | Згенеровані ШІ + цитування                                     | --                                               | `XAI_API_KEY`                                                                           |
| [Kimi](/uk/tools/kimi-search)                | Згенеровані ШІ + цитування; завершується помилкою для резервних чатів без прив’язки | --                                               | `KIMI_API_KEY` / `MOONSHOT_API_KEY`                                                     |
...
Моделі Direct OpenAI Responses автоматично використовують розміщений OpenAI інструмент `web_search`, коли вебпошук OpenClaw увімкнено й не закріплено керованого провайдера. Це поведінка, що належить провайдеру, у вбудованому OpenAI Plugin і застосовується лише до нативного трафіку OpenAI API, а не до базових URL OpenAI-сумісних проксі чи маршрутів Azure. Задайте `tools.web.search.provider` на іншого провайдера, наприклад `brave`, щоб зберегти керований інструмент `web_search` для моделей OpenAI, або задайте `tools.web.search.enabled: false`, щоб вимкнути і керований пошук, і нативний пошук OpenAI.

## Нативний вебпошук Codex

Моделі з підтримкою Codex можуть необов’язково використовувати провайдерний нативний інструмент Responses `web_search` замість керованої функції OpenClaw `web_search`.
...
11. **Ollama Web Search** -- резервний варіант без ключа через налаштований вами локальний хост Ollama, коли він доступний і вхід виконано за допомогою `ollama signin`; може повторно використовувати авторизацію bearer провайдера Ollama, коли вона потрібна хосту, і може викликати прямий пошук `https://ollama.com`, коли налаштовано `OLLAMA_API_KEY` (порядок 110)
12. **SearXNG** -- `SEARXNG_BASE_URL` або `plugins.entries.searxng.config.webSearch.baseUrl` (порядок 200)

Якщо провайдера не виявлено, буде використано Brave як резервний варіант (ви отримаєте помилку про відсутній ключ,
яка запропонує налаштувати його).

<Note>
  Усі поля ключів провайдера підтримують об’єкти SecretRef. SecretRef на рівні Plugin
...
Конфігурація, специфічна для провайдера (ключі API, базові URL, режими), міститься в
...
після спеціальної конфігурації вебпошуку та `GEMINI_API_KEY`. Приклади дивіться на
сторінках провайдерів.

Вибір резервного провайдера `web_fetch` окремий:

- виберіть його за допомогою `tools.web.fetch.provider`
- або опустіть це поле й дозвольте OpenClaw автоматично визначити першого готового постачальника web-fetch
  з доступних облікових даних
- `web_fetch` без пісочниці може використовувати встановлені постачальники Plugin, які оголошують
  `contracts.webFetchProviders`; отримання в пісочниці залишається лише вбудованим
- нині вбудований постачальник web-fetch — Firecrawl, налаштований у
  `plugins.entries.firecrawl.config.webFetch.*`

Коли ви вибираєте **Kimi** під час `openclaw onboard` або
`openclaw configure --section web`, OpenClaw також може попросити:
...
OpenClaw також може запропонувати необовʼязкове налаштування `x_search` із тим самим ключем.
Це окремий наступний крок усередині шляху Grok, а не окремий вибір постачальника
...
<Tabs>
  <Tab title="Файл конфігурації">
    Запустіть `openclaw configure --section web` або встановіть ключ напряму:
...
| Параметр              | Опис                                                  |
| --------------------- | ----------------------------------------------------- |
| `query`               | Пошуковий запит (обовʼязково)                        |
...
<Warning>
  ...
  діапазони свіжості Brave вимагають і початкову, і кінцеву дати.
...
необовʼязкові структуровані фільтри. OpenClaw вмикає вбудований інструмент xAI `x_search`
...
<Note>
  ...
  семантичний пошук, пошук користувачів і отримання гілок. Для статистики
  взаємодій на рівні окремого допису, як-от репости, відповіді, закладки або
  перегляди, надавайте перевагу цільовому пошуку за точною URL-адресою допису
  або ID статусу. Широкий пошук за ключовими словами може знайти потрібний допис,
  але повернути менш повні метадані на рівні допису. Добрий шаблон: спочатку
  знайдіть допис, а потім виконайте другий запит `x_search`, зосереджений на
  цьому точному дописі.
</Note>
...
`plugins.entries.xai.config.xSearch.baseUrl` задано. Якщо це поле опущено,
...
| ---------------------------- | ----------------------------------------------------- |
| `query`                      | Пошуковий запит (обовʼязково)                        |
| `allowed_x_handles`          | Обмежити результати конкретними handles X            |
| `excluded_x_handles`         | Виключити конкретні handles X                        |
...
Якщо ви використовуєте профілі інструментів або allowlist-и, додайте `web_search`, `x_search` або `group:web`:
...
- [Web Fetch](/uk/tools/web-fetch) -- отримати URL і витягти придатний для читання вміст
