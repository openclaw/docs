...
summary: Створюйте та редагуйте зображення за допомогою налаштованих провайдерів (OpenAI, Google Gemini, fal, MiniMax, ComfyUI, Vydra, xAI)
title: Генерація зображень
x-i18n:
    generated_at: "2026-04-22T23:11:32Z"
    model: gpt-5.4
    provider: openai
...
<Note>
Інструмент з’являється, лише коли доступний щонайменше один провайдер генерації зображень. Якщо ви не бачите `image_generate` в інструментах свого агента, налаштуйте `agents.defaults.imageGenerationModel` або задайте API-ключ провайдера.
</Note>
...
Агент автоматично викликає `image_generate`. Додавати інструмент до списку дозволених не потрібно — його ввімкнено за замовчуванням, коли доступний провайдер.
...
Не всі провайдери підтримують усі параметри. Коли резервний провайдер підтримує близький геометричний варіант замість точно запитаного, OpenClaw перед надсиланням переналаштовує запит на найближчий підтримуваний розмір, співвідношення сторін або роздільну здатність. Справді непідтримувані перевизначення все одно зазначаються в результаті інструмента.
...
4. **Автовиявлення** — використовує лише типові значення провайдерів, підкріплені автентифікацією:
   ...
   - далі решта зареєстрованих провайдерів генерації зображень у порядку id провайдера

Якщо провайдер не спрацьовує (помилка автентифікації, ліміт запитів тощо), автоматично пробується наступний кандидат. Якщо не спрацьовують усі, помилка міститиме подробиці кожної спроби.

Примітки:

- Автовиявлення враховує автентифікацію. Типове значення провайдера потрапляє до списку кандидатів, лише коли OpenClaw справді може автентифікувати цей провайдер.
- Автовиявлення ввімкнено за замовчуванням. Задайте
...
- Використовуйте `action: "list"`, щоб переглянути наразі зареєстрованих провайдерів, їхні типові моделі та підказки щодо auth env vars.
...
`gpt-image-2` підтримує як генерацію зображень за текстом, так і редагування
...
| Редагування/референс  | Так (до 5 зображень) | Так (до 5 зображень) | Так (1 зображення)  | Так (1 зображення, референс суб’єкта) | Так (1 зображення, налаштовується workflow) | Ні      | Так (до 5 зображень) |
