...
read_when:
    ...
    - Налаштування провайдера TTS, ланцюжка резервного переходу або персони
...
x-i18n:
    generated_at: "2026-05-06T01:53:16Z"
    model: gpt-5.5
    provider: openai
...
аудіовкладення в інших місцях, а також потоки PCM/Ulaw для телефонії та Talk.

TTS — це половина виведення мовлення в режимі `stt-tts` Talk. Нативні для постачальника
`realtime` сеанси Talk синтезують мовлення всередині постачальника реального часу
замість виклику цього шляху TTS, тоді як `transcription` сеанси не синтезують
...
Вбудований інструмент агента `tts` призначений лише для явного наміру: звичайний чат лишається
...
Якщо налаштовано кілька постачальників, спершу використовується вибраний, а
інші є запасними варіантами. Автоматичний підсумок використовує `summaryModel` (або
`agents.defaults.model.primary`), тому цей постачальник також має бути автентифікований,
якщо ви лишаєте підсумки ввімкненими.

<Warning>
Вбудований постачальник **Microsoft** використовує онлайн-сервіс neural TTS
Microsoft Edge через `node-edge-tts`. Це публічний вебсервіс без опублікованих
SLA або квоти — розглядайте його як сервіс найкращого зусилля. Застарілий ідентифікатор постачальника `edge`
...
попередній набір і адаптуйте блок постачальника:
...
голосом, моделлю, персоною або режимом Auto-TTS. Блок агента глибоко зливається поверх
`messages.tts`, тому облікові дані постачальника можуть лишатися в глобальній конфігурації постачальника:
...
постачальника — це перевизначить глобальну `messages.tts.persona` лише для цього агента.
...
глибоко об’єднуються поверх попередніх шарів, тож спільні облікові дані провайдера можуть лишатися в
...
нейтральний щодо провайдерів намір підказки та містити прив’язки, специфічні для провайдерів, для голосів, моделей, шаблонів підказок,
...
`pacing`, `constraints`) є **нейтральними щодо провайдерів**. Кожен провайдер сам вирішує, як
їх використовувати:

<AccordionGroup>
  <Accordion title="Google Gemini">
    Обгортає поля підказки персони у структуру підказки Gemini TTS **лише тоді, коли**
    ...
    досі додаються на початок як специфічний для Google текст підказки. Вбудовані аудіотеги, як-от
  ...
  <Accordion title="OpenAI">
    ...
    явні `instructions` для OpenAI не налаштовано. Явні `instructions`
    завжди мають перевагу.
  </Accordion>
...
Увесь запит TTS завершується помилкою лише тоді, коли **кожного** провайдера, якого було спробувано, пропущено
...
їх через сеанс Talk або запит передавання. Відкриття голосового сеансу не має
...
блок `[[tts:text]]...[[/tts:text]]` для експресивних підказок, які мають з’являтися
лише в аудіо:
...
Коли `messages.tts.auto` має значення `"tagged"`, **директиви обов’язкові**, щоб запускати
...
`/tts` є вбудованою командою Discord — текстова `/tts ...` досі працює.
...
`~/.openclaw/settings/tts.json`; перевизначте за допомогою env var `OPENCLAW_TTS_PREFS`
...
чи TTS у голосовому стилі має просити провайдерів про нативну ціль `voice-note`, чи
...
- **Feishu / WhatsApp**: коли відповідь голосовою нотаткою створено як MP3/WebM/WAV/M4A
  або інший імовірний аудіофайл, Plugin каналу транскодує його у 48 кГц
  ...
  `audio/ogg; codecs=opus`. Якщо конвертація завершується помилкою, Feishu отримує оригінальний
  файл як вкладення; надсилання WhatsApp завершується помилкою, а не публікацією несумісного
  PTT-навантаження.
- **BlueBubbles**: залишає синтез провайдера на звичайному шляху аудіофайлів; вихідні MP3
...
- **Локальний CLI**: використовує налаштований `outputFormat`. Цілі голосових нотаток
  конвертуються в Ogg/Opus, а вихід для телефонії конвертується в сирий моно PCM 16 кГц
  за допомогою `ffmpeg`.
...
- Узагальнює довгі відповіді, коли ввімкнено узагальнення, використовуючи
  `summaryModel` (або `agents.defaults.model.primary`).
- Додає згенероване аудіо до відповіді.
- У `mode: "final"` усе одно надсилає аудіо-only TTS для потокових фінальних відповідей
  після завершення текстового потоку; згенеровані медіа проходять таку саму
  нормалізацію медіа каналу, як і звичайні вкладення відповідей.
...
## Формати виводу за каналами
  ...
  <Accordion title="Top-level messages.tts.*">
    ...
    <ParamField path="providers.<id>" type="object">
      Налаштування, якими володіє провайдер, з ключами за ідентифікатором провайдера мовлення. Застарілі прямі блоки (`messages.tts.openai`, `.elevenlabs`, `.microsoft`, `.edge`) переписуються за допомогою `openclaw doctor --fix`; комітьте лише `messages.tts.providers.<id>`.
    </ParamField>
  ...
  <Accordion title="Azure Speech">
    <ParamField path="apiKey" type="string">Повертається до `AZURE_SPEECH_KEY`, `AZURE_SPEECH_API_KEY` або `SPEECH_KEY`.</ParamField>
    ...
    <ParamField path="endpoint" type="string">Необов’язкове перевизначення ендпойнта Azure Speech (псевдонім `baseUrl`).</ParamField>
  ...
  <Accordion title="ElevenLabs">
    ...
    <ParamField path="seed" type="number">Ціле число `0..4294967295` для детермінізму на основі найкращих зусиль.</ParamField>
  ...
  <Accordion title="Google Gemini">
    ...
    <ParamField path="audioProfile" type="string">Запит стилю природною мовою, доданий перед мовленим текстом.</ParamField>
    <ParamField path="speakerName" type="string">Необов’язкова мітка мовця, додана перед мовленим текстом, коли ваш запит використовує іменованого мовця.</ParamField>
    <ParamField path="promptTemplate" type='"audio-profile-v1"'>Установіть `audio-profile-v1`, щоб обгорнути активні поля запиту персони в детерміновану структуру запиту Gemini TTS.</ParamField>
    <ParamField path="personaPrompt" type="string">Додатковий текст запиту персони, специфічний для Google, доданий до приміток режисера шаблону.</ParamField>
  ...
  <Accordion title="Local CLI (tts-local-cli)">
    ...
    <ParamField path="timeoutMs" type="number">Час очікування команди в мілісекундах. Типово `120000`.</ParamField>
  ...
  <Accordion title="MiniMax">
    ...
    <ParamField path="pitch" type="number">Ціле число `-12..12`. Типово `0`. Дробові значення обрізаються перед запитом.</ParamField>
  </Accordion>
  ...
  <Accordion title="OpenAI">
    ...
    <ParamField path="instructions" type="string">Явне поле OpenAI `instructions`. Якщо встановлено, поля промпта персони **не** зіставляються автоматично.</ParamField>
    <ParamField path="extraBody / extra_body" type="Record<string, unknown>">Додаткові поля JSON, що об’єднуються з тілами запитів `/audio/speech` після згенерованих полів OpenAI TTS. Використовуйте це для сумісних з OpenAI кінцевих точок, як-от Kokoro, які потребують специфічних для провайдера ключів на кшталт `lang`; небезпечні ключі прототипу ігноруються.</ParamField>
    <ParamField path="baseUrl" type="string">
      Перевизначте кінцеву точку OpenAI TTS. Порядок визначення: config → `OPENAI_TTS_BASE_URL` → `https://api.openai.com/v1`. Нетипові значення розглядаються як сумісні з OpenAI кінцеві точки TTS, тому власні назви моделей і голосів приймаються.
    </ParamField>
  </Accordion>
  ...
  <Accordion title="OpenRouter">
    ...
    <ParamField path="baseUrl" type="string">Типово `https://openrouter.ai/api/v1`. Застарілий `https://openrouter.ai/v1` нормалізується.</ParamField>
    ...
    <ParamField path="speed" type="number">Власне для провайдера перевизначення швидкості.</ParamField>
  </Accordion>

  <Accordion title="Volcengine (BytePlus Seed Speech)">
    <ParamField path="apiKey" type="string">Env: `VOLCENGINE_TTS_API_KEY` або `BYTEPLUS_SEED_SPEECH_API_KEY`.</ParamField>
    <ParamField path="resourceId" type="string">Типово `seed-tts-1.0`. Env: `VOLCENGINE_TTS_RESOURCE_ID`. Використовуйте `seed-tts-2.0`, коли ваш проєкт має право на TTS 2.0.</ParamField>
    <ParamField path="appKey" type="string">Заголовок ключа застосунку. Типово `aGjiRDfUWi`. Env: `VOLCENGINE_TTS_APP_KEY`.</ParamField>
    ...
    <ParamField path="speedRatio" type="number">Власне для провайдера співвідношення швидкості.</ParamField>
    <ParamField path="emotion" type="string">Власний для провайдера тег емоції.</ParamField>
  ...
  <Accordion title="xAI">
    ...
    <ParamField path="voiceId" type="string">Типово `eve`. Живі голоси: `ara`, `eve`, `leo`, `rex`, `sal`, `una`.</ParamField>
