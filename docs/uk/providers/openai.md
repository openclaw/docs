...
title: OpenAI
x-i18n:
    generated_at: "2026-04-23T18:47:32Z"
    model: gpt-5.4
    provider: openai
...
---
  ...
  | Моделі за підпискою Codex | провайдер моделей `openai-codex/<model>` | Так                                                     |
  | Вебпошук на боці сервера | нативний інструмент OpenAI Responses      | Так, коли вебпошук увімкнено й провайдера не закріплено |
  | Зображення               | `image_generate`                          | Так                                                     |
  | Відео                    | `video_generate`                          | Так                                                     |
  | Синтез мовлення          | `messages.tts.provider: "openai"` / `tts` | Так                                                     |
    ...
    **Найкраще для:** прямого доступу до API та оплати за використання.
    ...
    <Steps>
      <Step title="Отримайте свій API key">
        Створіть або скопіюйте API key на [інформаційній панелі OpenAI Platform](https://platform.openai.com/api-keys).
      </Step>
    ...
    <Warning>
    OpenClaw **не** надає `openai/gpt-5.3-codex-spark` на шляху прямого API. Реальні запити до OpenAI API відхиляють цю модель. Spark доступний лише для Codex.
    </Warning>
    ...
    **Найкраще для:** використання вашої підписки ChatGPT/Codex замість окремого API key. Codex cloud потребує входу в ChatGPT.
    ...
    <Steps>
      <Step title="Запустіть Codex OAuth">
        ...
        Для headless або несумісних із callback середовищ додайте `--device-code`, щоб увійти через потік коду пристрою ChatGPT замість callback браузера на localhost:
    ...
    <Note>
    Цей маршрут навмисно відокремлено від `openai/gpt-5.4`. Використовуйте `openai/*` з API key для прямого доступу до Platform, а `openai-codex/*` — для доступу за підпискою Codex.
    </Note>
    ...
    <Note>
    Онбординг більше не імпортує матеріали OAuth з `~/.codex`. Увійдіть через OAuth у браузері (типово) або через наведений вище потік коду пристрою — OpenClaw керує отриманими обліковими даними у власному сховищі автентифікації агентів.
    </Note>

    ### Обмеження контекстного вікна

    OpenClaw розглядає метадані моделі та фактичне обмеження контексту під час виконання як окремі значення.
    ...
    <Note>
    Використовуйте `contextWindow`, щоб задати власні метадані моделі. Використовуйте `contextTokens`, щоб обмежити бюджет контексту під час виконання.
    </Note>
...
`gpt-image-2` є типовою моделлю як для генерації зображень із тексту в OpenAI, так і для редагування зображень. `gpt-image-1` і далі можна використовувати як явне перевизначення моделі, але нові робочі процеси OpenAI для зображень мають використовувати `openai/gpt-image-2`.
...
OpenClaw додає спільний внесок у промпт GPT-5 для запусків сімейства GPT-5 у різних провайдерів. Він застосовується за id моделі, тому `openai/gpt-5.4`, `openai-codex/gpt-5.4`, `openrouter/openai/gpt-5.4`, `opencode/gpt-5.4` та інші сумісні посилання на GPT-5 отримують однакове накладання. Старіші моделі GPT-4.x — ні.

Вбудований нативний провайдер harness Codex (`codex/*`) використовує ту саму поведінку GPT-5 і накладання Heartbeat через інструкції розробника app-server Codex, тому сесії `codex/gpt-5.x` зберігають ту саму послідовність виконання та проактивні вказівки Heartbeat, хоча рештою промпта harness керує Codex.
...
<Note>
Застарілий параметр `plugins.entries.openai.config.personality` і далі зчитується як сумісний запасний варіант, коли спільний параметр `agents.defaults.promptOverlays.gpt5.personality` не задано.
</Note>
...
<AccordionGroup>
  ...
  <Accordion title="Перетворення мовлення в текст">
    ...
    спільна конфігурація аудіомедіа або запит на транскрибування для конкретного виклику.
...
- Ви хочете зберігати трафік в межах наявного тенанта Azure
...
OpenClaw 2026.4.22 або новішої версії. Попередні версії трактують будь-який
власний `openai.baseUrl` як публічну кінцеву точку OpenAI і не працюватимуть з Azure
deployment для зображень.
</Note>
...
Azure OpenAI прив’язує моделі до deployment. Для запитів Azure на генерацію зображень,
...
Те саме правило імен deployment застосовується і до викликів генерації зображень,
...
значення `background` для `gpt-image-2`), або надавати їх лише в окремих версіях
...
набір параметрів, що підтримується вашим конкретним deployment і версією API в
порталі Azure.

<Note>
Azure OpenAI використовує нативну транспортну поведінку та поведінку сумісності, але не отримує
...
<AccordionGroup>
  ...
  <Accordion title="Швидкий режим">
    ...
    Коли його увімкнено, OpenClaw зіставляє швидкий режим із пріоритетною обробкою OpenAI (`service_tier = "priority"`). Наявні значення `service_tier` зберігаються, а швидкий режим не переписує `reasoning` або `text.verbosity`.
  ...
  <Accordion title="Server-side Compaction (Responses API)">
    ...
    <Note>
    `responsesServerCompaction` керує лише впровадженням `context_management`. Прямі моделі OpenAI Responses і далі примусово встановлюють `store: true`, якщо сумісність не задає `supportsStore: false`.
    </Note>
  ...
  <Accordion title="Нативні та OpenAI-сумісні маршрути">
    ...
    **Проксі/сумісні маршрути:**
    ...
    - Не примушують використовувати строгі схеми інструментів або заголовки, специфічні для нативних маршрутів

    Azure OpenAI використовує нативну транспортну поведінку та поведінку сумісності, але не отримує приховані заголовки атрибуції.
