---
redirect: /tools/perplexity-search
summary: Переспрямування на /tools/perplexity-search
title: Пошук Perplexity
x-i18n:
    generated_at: "2026-05-05T16:52:14Z"
    model: gpt-5.5
    provider: openai
    source_hash: 1c41e7de5f183075e494182024214e3bbd4a93706de42a4ee6933a0c079d8cd0
    source_path: perplexity.md
    workflow: 16
---

Цю сторінку переміщено до [пошуку Perplexity](/uk/tools/perplexity-search).

## Пов’язане

- [Вебінструменти](/uk/tools/web)
