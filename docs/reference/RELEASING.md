...
- Every regular final release includes the npm package, macOS app, signed standalone Android APK, and signed Windows Hub installers. macOS, iOS, and Android publication may finish in parallel after core publication; verify those native releases separately before announcing all platforms complete. Windows installer verification remains a GitHub release finalization gate; Windows Hub asset promotion never delays npm publication. Beta releases normally validate and publish the npm/package path first, with native app build/sign/notarize/promote reserved for regular final unless explicitly requested.
...
6. Run `OpenClaw Release Publish` from trusted `main` with the same `tag`, the same `npm_dist_tag`, the selected `windows_node_tag`, its saved `windows_node_installer_digests`, the saved `preflight_run_id`, `full_release_validation_run_id`, and `full_release_validation_run_attempt`. It starts plugin npm and ClawHub in parallel, then promotes the prepared OpenClaw npm package once plugin npm succeeds.
