...
summary: OpenClaw 插件的测试实用工具和模式
title: 插件测试
x-i18n:
    generated_at: "2026-06-28T07:42:56Z"
    model: gpt-5.5
...
OpenClaw 插件测试工具、模式和 lint 强制执行参考。
...
这些测试辅助子路径是 OpenClaw 自身内置插件测试的仓库本地源码入口点。它们不是面向第三方插件的包导出，并且可能导入 Vitest 或其他仅限仓库使用的测试依赖。
...
在 OpenClaw 仓库内，为新的内置插件测试优先使用下面这些聚焦子路径。宽泛的 `openclaw/plugin-sdk/testing` 聚合导出仅用于旧版兼容性。仓库防护规则会拒绝来自 `plugin-sdk/testing` 和 `plugin-sdk/test-utils` 的新真实导入；这些名称仅作为兼容性记录测试的已弃用兼容性表面保留。
...
| `AUTH_PROFILE_RUNTIME_CONTRACT`                      | Native Agent 运行时适配器的共享凭证配置文件契约夹具。从 `plugin-sdk/agent-runtime-test-contracts` 导入                                   |
| `DELIVERY_NO_REPLY_RUNTIME_CONTRACT`                 | Native Agent 运行时适配器的共享发送抑制契约夹具。从 `plugin-sdk/agent-runtime-test-contracts` 导入                                       |
| `OUTCOME_FALLBACK_RUNTIME_CONTRACT`                  | Native Agent 运行时适配器的共享回退分类契约夹具。从 `plugin-sdk/agent-runtime-test-contracts` 导入                                       |
...
| `installChannelOutboundPayloadContractSuite`         | 安装渠道出站 payload 契约用例。从 `plugin-sdk/channel-contract-testing` 导入                                                              |
...
| `createPluginSetupWizardStatus`                      | 为渠道插件构建设置状态 helper。从 `plugin-sdk/plugin-test-runtime` 导入                                                                   |
| `describeOpenAIProviderRuntimeContract`              | 安装提供商族运行时契约检查。从 `plugin-sdk/provider-test-contracts` 导入                                                                  |
...
| `removeAckReactionAfterReply`                        | 回复发送后移除确认反应。从 `plugin-sdk/channel-feedback` 导入                                                                            |
...
| `mockNodeBuiltinModule`                              | 安装窄范围 Node 内置 Vitest mock。从 `plugin-sdk/test-node-mocks` 导入                                                                    |
| `createSandboxTestContext`                           | 构建沙箱测试上下文。从 `plugin-sdk/test-fixtures` 导入                                                                                    |
| `writeSkill`                                         | 写入 skill 夹具。从 `plugin-sdk/test-fixtures` 导入                                                                                       |
| `makeAgentAssistantMessage`                          | 构建智能体 transcript 消息夹具。从 `plugin-sdk/test-fixtures` 导入                                                                        |
...
| `buildProviderPluginMethodChoice`                    | 构建提供商向导选择 ID 以用于断言                                                                                                         |
| `setProviderWizardProvidersResolverForTest`          | 为隔离测试注入提供商向导提供商                                                                                      |
| `createProviderUsageFetch`                           | 构建提供商用量 fetch 固件                                                                                                      |
...
内置插件契约套件也使用 SDK 测试子路径，用于仅测试使用的
注册表、清单、公共产物和运行时 fixture 辅助工具。依赖内置 OpenClaw 清单的仅核心
套件保留在 `src/plugins/contracts` 下。
将新的扩展测试放在有文档说明且聚焦的 SDK 子路径上，例如
`plugin-sdk/plugin-test-api`、`plugin-sdk/channel-contract-testing`、
`plugin-sdk/agent-runtime-test-contracts`、`plugin-sdk/channel-test-helpers`、
`plugin-sdk/plugin-test-contracts`、`plugin-sdk/plugin-test-runtime`、
`plugin-sdk/provider-test-contracts`、`plugin-sdk/provider-http-test-mocks`、
`plugin-sdk/test-env` 或 `plugin-sdk/test-fixtures`，而不是直接导入
宽泛的 `plugin-sdk/testing` 兼容性 barrel、仓库 `src/**` 文件或仓库
`test/helpers/*` 桥接。
...
向 `register(api)` 传入手写 `api` mock 的单元测试不会覆盖
OpenClaw 的加载器验收门槛。请为你的插件依赖的每个注册表面至少添加一个
由加载器支撑的冒烟测试，尤其是钩子和记忆这类独占能力。

当必需元数据缺失，或插件调用了不属于自己的能力 API 时，真实加载器会使插件注册失败。
...
其已弃用的 `runtime.config.loadConfig()` 和 `runtime.config.writeConfigFile(...)`
mock 默认会抛错，以便测试捕获对兼容性 API 的新用法。只有当测试明确覆盖旧版兼容行为时，
才覆盖这些 mock。
...
OpenClaw 使用 Vitest，并配置了 V8 覆盖率阈值。对于插件测试：
