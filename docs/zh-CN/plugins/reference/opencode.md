...
会话会显示在会话侧边栏的 **OpenCode** 分组中，并可通过官方的 `opencode --pure db ... --format json`
和 `opencode --pure export` 命令以只读方式浏览对话记录。受限环境和 `--pure`
模式可防止目录浏览加载项目插件或继承无关的
Gateway 网关凭据。

在 **Config > Plugins > OpenCode** 下关闭 **OpenCode Session Catalog** 即可
禁用发现功能。此功能默认启用。
