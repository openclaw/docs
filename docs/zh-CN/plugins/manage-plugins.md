...
大多数插件工作流只需要几个命令：搜索、安装、重启 Gateway 网关、
验证，并在你不再需要该插件时卸载。
...
对脚本使用 `--json`。当插件包声明了 `dependencies` 或
...
`plugins list` 是一次冷库存检查。它会显示 OpenClaw 能从配置、
...
当你需要证明插件已注册运行时表面，例如工具、钩子、服务、Gateway 网关方法，
...
如果插件是从 npm dist-tag（例如 `@beta`）安装的，后续的
...
卸载会移除插件的配置条目、插件索引记录、允许/拒绝列表条目，以及适用时的
...
你可以将外部插件发布到 [ClawHub](https://clawhub.ai)、npmjs.com，或
两者都发布。

### 发布到 ClawHub

ClawHub 是 OpenClaw 插件的主要公共发现表面。它会在安装前为用户提供
可搜索的元数据、版本历史和注册表扫描结果。
...
如果同一个包也可在 ClawHub 上获取，`npm:` 会跳过 ClawHub 查找并
强制使用 npm 解析。
