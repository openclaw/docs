...
- **`openclaw/plugin-sdk/config-runtime`** —— 一个宽泛的配置兼容性 barrel，在迁移窗口期内仍然保留已弃用的直接加载/写入辅助工具。
...
带有渠道品牌色彩的辅助接口属于私有 monorepo 快捷方式，而不是稳定的插件契约。请改用范围更窄的通用 SDK 子路径。在内置插件工作区内部，应将 provider 自有的辅助工具保留在该插件自己的 `api.ts` 或 `runtime-api.ts` 中。
...
<Steps>
  <Step title="迁移运行时配置加载/写入辅助工具">
    ...
    `api.runtime.config.writeConfigFile(...)`。优先使用已传入当前活动调用路径的配置。需要当前进程快照的长生命周期处理器可以使用 `api.runtime.config.current()`。长生命周期智能体工具应在 `execute` 内使用工具上下文的 `ctx.getRuntimeConfig()`，这样即使工具创建于配置写入之前，也仍能看到刷新后的运行时配置。
    ...
    变更结果会包含一个带类型的 `followUp` 摘要，用于测试和日志记录；Gateway 网关 仍负责应用或调度重启。
    `loadConfig` 和 `writeConfigFile` 在迁移窗口期内仍作为外部插件的已弃用兼容性辅助工具保留，并会以 `runtime-config-load-write` 兼容性代码发出一次警告。内置插件和仓库运行时代码受到
    `pnpm check:deprecated-internal-config-api` 和
    `pnpm check:no-runtime-action-load-config` 中扫描器防护措施的保护：新的生产插件用法会直接失败，直接配置写入会失败，Gateway 网关 服务器方法必须使用请求运行时快照，运行时渠道发送/操作/客户端辅助工具必须从其边界接收配置，而长生命周期运行时模块不允许存在任何环境式 `loadConfig()` 调用。
    ...
    内置插件及其测试受到扫描器保护，不能使用这个宽泛的 barrel，因此导入和 mock 都会局限于其实际所需的行为。这个宽泛的 barrel 仍然存在以保持外部兼容性，但新代码不应依赖它。
  ...
  <Step title="将原生审批处理器迁移到能力事实">
    ...
    - 向 `createChannelManager(...)` 传入 `channelRuntime` 时，请提供真实的 `createPluginRuntime().channel` 接口。部分 stub 将被拒绝。
  ...
  <Step title="替换宽泛的 infra-runtime 导入">
    ...
    内置插件受到扫描器保护，不能使用 `infra-runtime`，因此仓库代码无法回退到这个宽泛的 barrel。
...
## 导入路径参考
  ...
  | `plugin-sdk/channel-config-schema` | 配置 schema 构建器 | 共享渠道配置 schema 原语和仅限通用构建器 |
  ...
  | `plugin-sdk/approval-auth-runtime` | 审批 auth 辅助工具 | approver 解析、同聊天操作 auth |
  ...
  | `plugin-sdk/provider-web-search-config-contract` | provider web-search 配置辅助工具 | 针对不需要插件启用连接逻辑的 provider，提供窄范围 web-search 配置/凭证辅助工具 |
...
运行时接口和 manifest。它们当前仍可使用，但将在未来的主要版本中移除。
...
<AccordionGroup>
  ...
  <Accordion title="渠道运行时 shim 和渠道动作辅助工具">
    ...
    （cards、buttons、selects），而不是声明它接受哪些原始
    action 名称。
  ...
  <Accordion title="纯文本渠道 envelope → BodyForAgent">
    ...
    **新版**：`BodyForAgent` 加上结构化用户上下文块。渠道
    插件通过带类型字段附加路由元数据（线程、主题、reply-to、reactions），
    而不是将它们拼接进提示字符串中。
  ...
  <Accordion title="Thinking 策略钩子 → resolveThinkingProfile">
    ...
    **新版**：单个 `resolveThinkingProfile(ctx)`，返回一个
    `ProviderThinkingProfile`，包含规范 `id`、可选 `label` 和
...
- [插件架构内部机制](/zh-CN/plugins/architecture) — 架构深度解析
