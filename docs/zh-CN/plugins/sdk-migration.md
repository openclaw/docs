...
title: 插件 SDK 迁移
x-i18n:
    generated_at: "2026-05-06T01:52:53Z"
    model: gpt-5.5
    provider: openai
...
旧插件系统提供了两个完全开放的接口面，让插件可以从单一入口点导入所需的任何内容：

- **`openclaw/plugin-sdk/compat`** — 单一导入，会重新导出数十个辅助工具。它最初用于在新的插件架构构建期间，让较旧的基于钩子的插件继续工作。
- **`openclaw/plugin-sdk/infra-runtime`** — 一个宽泛的运行时辅助 barrel，混合了系统事件、Heartbeat 状态、投递队列、fetch/proxy 辅助工具、文件辅助工具、审批类型和无关实用工具。
...
- **`api.registerEmbeddedExtensionFactory(...)`** — 一个已移除的仅 Pi 内置扩展钩子，可观察嵌入式运行器事件，例如 `tool_result`。

这些宽泛的导入接口面现在已**弃用**。它们在运行时仍可工作，但新插件不得使用它们，现有插件也应在下一次重大版本移除它们之前完成迁移。仅 Pi 的嵌入式扩展工厂注册 API 已被移除；请改用工具结果中间件。

OpenClaw 不会在引入替代方案的同一变更中移除或重新解释已有文档说明的插件行为。破坏性合约变更必须先经过兼容适配器、诊断、文档和弃用窗口。这适用于 SDK 导入、插件清单字段、设置 API、钩子和运行时注册行为。

<Warning>
  向后兼容层将在未来的重大版本中移除。届时仍从这些接口面导入的插件将会中断。仅 Pi 的嵌入式扩展工厂注册已不再加载。
</Warning>
...
- **API 接口面不清晰** — 无法判断哪些导出是稳定的，哪些是内部的

现代插件 SDK 修复了这一点：每个导入路径（`openclaw/plugin-sdk/\<subpath\>`）都是小型、自包含的模块，具有明确用途和有文档说明的合约。

内置渠道的旧版提供商便捷 seam 也已移除。带渠道品牌的辅助 seam 是私有单仓库捷径，不是稳定的插件合约。请改用狭窄的通用 SDK 子路径。在内置插件工作区内，将提供商拥有的辅助工具保留在该插件自己的 `api.ts` 或 `runtime-api.ts` 中。

当前内置提供商示例：

- Anthropic 将 Claude 专属流辅助工具保留在自己的 `api.ts` / `contract-api.ts` seam 中
- OpenAI 将提供商构建器、默认模型辅助工具和实时提供商构建器保留在自己的 `api.ts` 中
- OpenRouter 将提供商构建器和新手引导/配置辅助工具保留在自己的 `api.ts` 中

## Talk 和实时语音迁移计划

实时语音、电话、会议和浏览器 Talk 代码正在从表面本地轮次记账迁移到由 `openclaw/plugin-sdk/realtime-voice` 导出的共享 Talk 会话控制器。新的控制器拥有通用 Talk 事件信封、活动轮次状态、采集状态、输出音频状态、近期事件历史和过期轮次拒绝。提供商插件应继续拥有供应商特定的实时会话；表面插件应继续拥有采集、播放、电话和会议差异处理。

此次 Talk 迁移有意采用破坏性清理方式：

1. 将共享控制器/运行时原语保留在 `plugin-sdk/realtime-voice` 中。
2. 将内置表面迁移到共享控制器：浏览器中继、托管房间 handoff、语音通话实时、语音通话流式 STT、Google Meet 实时和原生 push-to-talk。
3. 用最终的 `talk.session.*` 和 `talk.client.*` API 替换旧的 Talk RPC 族。
4. 在 Gateway 网关 `hello-ok.features.events` 中公布一个实时 Talk 事件渠道：`talk.event`。
5. 删除旧的实时 HTTP 端点以及任何请求时指令覆盖路径。

除非是在实现低级适配器或测试夹具，新代码不应直接调用 `createTalkEventSequencer(...)`。请优先使用共享控制器，这样没有轮次 id 就无法发出轮次作用域事件，过期的 `turnEnd` / `turnCancel` 调用无法清除更新的活动轮次，并且输出音频生命周期事件可在电话、会议、浏览器中继、托管房间 handoff 和原生 Talk 客户端之间保持一致。
...
浏览器拥有的 WebRTC/提供商 websocket 会话使用 `talk.client.create`，因为浏览器拥有提供商协商和媒体传输，而 Gateway 网关拥有凭证、指令和工具策略。`talk.session.*` 是 Gateway 网关管理表面的通用接口，适用于 gateway-relay 实时、gateway-relay 转录，以及托管房间原生 STT/TTS 会话。

对于将实时选择器放在 `talk.provider` / `talk.providers` 旁边的旧版配置，应使用 `openclaw doctor --fix` 修复；运行时 Talk 不会将语音/TTS 提供商配置重新解释为实时提供商配置。
...
| `stt-tts`       | `managed-room`  | `agent-consult` | 原生/客户端房间 | Push-to-talk 和 walkie-talkie 风格的房间，其中客户端拥有采集/播放，Gateway 网关拥有轮次状态。 |
| `stt-tts`       | `managed-room`  | `direct-tools`  | 原生/客户端房间 | 仅限管理员的房间模式，用于可信的第一方表面直接执行 Gateway 网关工具操作。                  |
...
| `talk.session.submitToolResult` | `realtime/gateway-relay`                                | 完成中继发出的提供商工具调用。                                           |
...
4. 在测试中覆盖两条路径
5. 记录弃用和迁移路径
6. 仅在已公告的迁移窗口之后移除，通常是在重大版本中

  维护者可以使用
  `pnpm plugins:boundary-report` 审核当前迁移队列。使用 `pnpm plugins:boundary-report:summary`
  获取紧凑计数，使用 `--owner <id>` 查看单个插件或兼容性所有者，并在 CI 门禁应因到期兼容性记录、跨所有者保留 SDK 导入或未使用的保留 SDK 子路径而失败时使用
  `pnpm plugins:boundary-report:ci`。报告会按移除日期分组弃用的兼容性记录，统计本地代码/文档引用，显示跨所有者保留 SDK 导入，并汇总私有 memory-host SDK 桥接，以便兼容性清理保持显式，而不是依赖临时搜索。保留 SDK 子路径必须有跟踪的所有者用法；未使用的保留辅助导出应从公共 SDK 中移除。

  如果某个清单字段仍被接受，插件作者就可以继续使用它，直到文档和诊断另有说明。新代码应优先使用文档化的替代项，但现有插件不应在普通次要版本发布期间中断。
  ...
  <Step title="迁移运行时配置加载/写入辅助函数">
    ...
    `api.runtime.config.writeConfigFile(...)`。优先使用已经传入活动调用路径的配置。需要当前进程快照的长生命周期处理程序可以使用 `api.runtime.config.current()`。长生命周期智能体工具应在
    `execute` 内使用工具上下文的 `ctx.getRuntimeConfig()`，这样在配置写入前创建的工具仍能看到刷新的运行时配置。

    配置写入必须通过事务性辅助函数，并选择写入后策略：
    ...
    当调用方知道更改需要干净重启 Gateway 网关时，使用 `afterWrite: { mode: "restart", reason: "..." }`；只有当调用方拥有后续处理并有意抑制重载规划器时，才使用
    `afterWrite: { mode: "none", reason: "..." }`。
    变更结果包含一个类型化的 `followUp` 摘要，用于测试和日志记录；
    Gateway 网关仍负责应用或调度重启。
    `loadConfig` 和 `writeConfigFile` 在迁移窗口期间仍作为面向外部插件的弃用兼容性辅助函数保留，并会使用
    `runtime-config-load-write` 兼容性代码警告一次。内置插件和仓库运行时代码受
    `pnpm check:deprecated-internal-config-api` 和
    `pnpm check:no-runtime-action-load-config` 中的扫描器护栏保护：新的生产插件用法会直接失败，直接配置写入会失败，Gateway 网关服务器方法必须使用请求运行时快照，运行时渠道发送/action/client 辅助函数必须从其边界接收配置，而长生命周期运行时模块允许的环境式 `loadConfig()` 调用数量为零。

    新插件代码还应避免导入宽泛的
    `openclaw/plugin-sdk/config-runtime` 兼容性桶。使用与任务匹配的窄 SDK 子路径：
    ...
    内置插件及其测试会被扫描器护栏约束，禁止使用这个宽泛桶，以便导入和 mock 保持局部于它们所需的行为。该宽泛桶仍为外部兼容性存在，但新代码不应依赖它。
  ...
  <Step title="将 Pi 工具结果扩展迁移到中间件">
    ...
    外部插件不能注册工具结果中间件，因为它可以在模型看到高信任度工具输出之前重写该输出。
  ...
  <Step title="将原生审批处理程序迁移到能力事实">
    ...
    - 不要从原生审批处理程序发送插件拥有的重新路由通知；核心现在根据实际投递结果负责 routed-elsewhere 通知
  ...
  <Step title="替换宽泛的 infra-runtime 导入">
    `openclaw/plugin-sdk/infra-runtime` 仍为外部兼容性存在，但新代码应导入它实际需要的聚焦辅助函数表面：

    | 需求 | 导入 |
    | --- | --- |
    | 系统事件队列辅助函数 | `openclaw/plugin-sdk/system-event-runtime` |
    | Heartbeat 事件和可见性辅助函数 | `openclaw/plugin-sdk/heartbeat-runtime` |
    ...
    | 可感知 dispatcher 的 fetch | `openclaw/plugin-sdk/runtime-fetch` |
    | 代理和受保护 fetch 辅助函数 | `openclaw/plugin-sdk/fetch-runtime` |
    ...
    | 传输协议就绪等待 | `openclaw/plugin-sdk/transport-ready-runtime` |
    ...
    内置插件受扫描器护栏约束，禁止使用 `infra-runtime`，因此仓库代码不能回退到这个宽泛桶。

  </Step>

  <Step title="迁移渠道路由辅助函数">
    新渠道路由代码应使用 `openclaw/plugin-sdk/channel-route`。
    较旧的 route-key 和 comparable-target 名称会在迁移窗口期间保留为兼容性别名，但新插件应使用直接描述行为的路由名称：
    ...
    现代路由助手会在原生审批、回复抑制、入站去重、cron 投递和会话路由中
    一致地规范化 `{ channel, to, accountId, threadId }`。如果你的插件拥有自定义目标
    语法，请使用 `resolveChannelRouteTargetWithParser(...)` 将该
    解析器适配到相同的路由目标契约中。
...
## 导入路径参考
  ...
  | `plugin-sdk/setup-runtime` | 设置时运行时辅助工具 | 可安全导入的设置补丁适配器、查找备注辅助工具、`promptResolvedAllowFrom`, `splitSetupEntries`, 委托式设置代理 |
  ...
  | `plugin-sdk/channel-config-schema` | 配置模式构建器 | 共享渠道配置模式原语，以及仅通用构建器 |
  | `plugin-sdk/bundled-channel-config-schema` | 内置配置模式 | 仅限 OpenClaw 维护的内置插件；新插件必须定义插件本地模式 |
  | `plugin-sdk/channel-config-schema-legacy` | 已弃用的内置配置模式 | 仅兼容性别名；对维护中的内置插件使用 `plugin-sdk/bundled-channel-config-schema` |
  | `plugin-sdk/telegram-command-config` | Telegram 命令配置辅助工具 | 命令名称规范化、描述修剪、重复/冲突校验 |
  | `plugin-sdk/channel-policy` | 群组/私信策略解析 | `resolveChannelGroupRequireMention` |
  | `plugin-sdk/channel-lifecycle` | 账号状态和草稿流生命周期辅助工具 | `createAccountStatusSink`, 草稿预览最终化辅助工具 |
  ...
  | `plugin-sdk/security-runtime` | 安全辅助工具 | 共享信任、私信门控、根目录边界内文件/路径辅助工具、外部内容和密钥收集辅助工具 |
  ...
  | `plugin-sdk/fetch-runtime` | 包装后的 fetch/代理辅助工具 | `resolveFetch`, 代理辅助工具、EnvHttpProxyAgent 选项辅助工具 |
  ...
  | `plugin-sdk/provider-web-search-contract` | 提供商 Web 搜索契约辅助工具 | 窄 Web 搜索配置/凭证契约辅助工具，例如 `createWebSearchProviderContractFields`、`enablePluginInConfig`、`resolveProviderWebSearchPluginConfig`，以及限定作用域的凭证设置器/获取器 |
  | `plugin-sdk/provider-web-search` | 提供商 Web 搜索辅助工具 | Web 搜索提供商注册/缓存/运行时辅助工具 |
  | `plugin-sdk/provider-tools` | 提供商工具/模式兼容性辅助工具 | `ProviderToolCompatFamily`、`buildProviderToolCompatFamilyHooks`、Gemini 模式清理 + 诊断，以及 xAI 兼容性辅助工具，例如 `resolveXaiModelCompatPatch` / `applyXaiModelCompat` |
  ...
  | `plugin-sdk/provider-transport-runtime` | 提供商传输辅助工具 | 原生提供商传输辅助工具，例如受保护的 fetch、传输消息转换和可写传输事件流 |
  ...
  | `plugin-sdk/speech` | 语音辅助工具 | 语音提供商类型，以及面向提供商的指令、注册表、验证辅助工具和 OpenAI 兼容的 TTS 构建器 |
  ...
  | `plugin-sdk/realtime-voice` | 实时语音辅助工具 | 提供商类型、注册表/解析辅助工具、桥接会话辅助工具、共享智能体回话队列、转录/事件健康状态、回声抑制和快速上下文咨询辅助工具 |
  ...
  | `plugin-sdk/channel-config-primitives` | 渠道配置原语 | 窄渠道配置模式原语 |
  ...
  | `plugin-sdk/direct-dm` | 直接私信辅助工具 | 共享直接私信认证/防护辅助工具 |
  ...
  | `plugin-sdk/memory-core` | 内置 memory-core 辅助工具 | 内存管理器/配置/文件/CLI 辅助工具表面 |
  ...
  | `plugin-sdk/memory-host-core` | 内存宿主核心运行时别名 | 内存宿主核心运行时辅助工具的供应商中立别名 |
  | `plugin-sdk/memory-host-events` | 内存宿主事件日志别名 | 内存宿主事件日志辅助工具的供应商中立别名 |
  | `plugin-sdk/memory-host-files` | 内存宿主文件/运行时别名 | 内存宿主文件/运行时辅助工具的供应商中立别名 |
  | `plugin-sdk/memory-host-markdown` | 受管 Markdown 辅助工具 | 面向内存相邻插件的共享受管 Markdown 辅助工具 |
  | `plugin-sdk/memory-host-search` | 主动记忆搜索门面 | 惰性主动记忆搜索管理器运行时门面 |
  | `plugin-sdk/memory-host-status` | 内存宿主 Status 别名 | 内存宿主 Status 辅助工具的供应商中立别名 |
  | `plugin-sdk/testing` | 测试实用工具 | 旧版宽泛兼容性桶；优先使用聚焦测试子路径，例如 `plugin-sdk/plugin-test-runtime`、`plugin-sdk/channel-test-helpers`、`plugin-sdk/channel-target-testing`、`plugin-sdk/test-env` 和 `plugin-sdk/test-fixtures` |
</Accordion>
...
<AccordionGroup>
  ...
  <Accordion title="渠道运行时 shim 和渠道 actions 辅助工具">
    ...
    会随原始 “actions” 渠道导出一起弃用。改为通过语义化
    `presentation` 表面暴露能力——渠道插件声明它们会渲染什么
    （卡片、按钮、选择器），而不是接受哪些原始
    action 名称。
  ...
  <Accordion title="纯文本渠道信封 → BodyForAgent">
    ...
    **新**：`BodyForAgent` 加结构化用户上下文块。渠道
    插件将路由元数据（线程、主题、回复目标、回应）作为
    类型化字段附加，而不是将它们拼接进提示词字符串。
    `formatAgentEnvelope(...)` 辅助工具仍支持合成的
    面向助手的信封，但入站纯文本信封正在
    退出。
  ...
  <Accordion title="Thinking 策略钩子 → resolveThinkingProfile">
    ...
    **新**：单个 `resolveThinkingProfile(ctx)`，返回一个
    ...
    按等级排序的 level 列表。OpenClaw 会按 profile
    排名自动降级过时的已存储值。

    实现一个钩子，而不是三个。旧版钩子在
    弃用窗口期间仍会工作，但不会与 profile 结果组合。
  ...
  <Accordion title="提供商环境变量查找 → setup.providers[].envVars">
    ...
    **新**：将相同的环境变量查找镜像到清单上的
    `setup.providers[].envVars`。这会将设置/Status 环境元数据整合到一个
    位置，并避免仅为回答环境变量
    查找而启动插件运行时。
  ...
  <Accordion title="记忆插件注册 → registerMemoryCapability">
    ...
    **新**：memory-state API 上的一次调用——
    ...
    相同 slot，单次注册调用。增量记忆辅助工具
    （`registerMemoryPromptSupplement`、`registerMemoryCorpusSupplement`、
    `registerMemoryEmbeddingProvider`）不受影响。

  </Accordion>

  <Accordion title="Subagent 会话消息类型重命名">
  ...
  <Accordion title="runtime.tasks.flow → runtime.tasks.managedFlows">
    ...
    **新**：`runtime.tasks.managedFlows` 为会从 flow
    创建、更新、取消或运行子任务的插件保留托管 TaskFlow 变更
  ...
  <Accordion title="嵌入式 extension 工厂 → 智能体工具结果中间件">
    已在上面的“如何迁移 → 将 Pi 工具结果 extension 迁移到
    中间件”中介绍。此处为完整性而列出：已移除的仅 Pi
...
<Note>
Extension 级别的弃用项（位于 `extensions/` 下的内置渠道/提供商插件内）
会在它们自己的 `api.ts` 和 `runtime-api.ts`
barrel 中跟踪。它们不会影响第三方插件契约，也未在此处列出。
...
| ---------------------- | ----------------------------------------------------------------------- |
| **现在**               | 已弃用表面会发出运行时警告                                              |
| **下一个主版本**       | 已弃用表面将被移除；仍使用它们的插件会失败                              |

所有核心插件都已经迁移。外部插件应在下一个主版本之前迁移。
