---
read_when:
    - 你会看到 OPENCLAW_PLUGIN_SDK_COMPAT_DEPRECATED 警告
...
title: 插件 SDK 迁移
x-i18n:
    generated_at: "2026-04-29T11:23:05Z"
    model: gpt-5.5
    provider: openai
...
- **`openclaw/plugin-sdk/compat`** — 单个导入，会重新导出数十个辅助工具。它的引入是为了在构建新插件架构期间，让较旧的基于钩子的插件继续工作。
- **`openclaw/plugin-sdk/infra-runtime`** — 一个宽泛的运行时辅助工具桶，将系统事件、心跳状态、交付队列、fetch/proxy 辅助工具、文件辅助工具、审批类型和无关的工具混在一起。
- **`openclaw/plugin-sdk/config-runtime`** — 一个宽泛的配置兼容工具桶，在迁移窗口期间仍携带已弃用的直接加载/写入辅助工具。
...
这些宽泛的导入表面现在已**弃用**。它们在运行时仍可工作，但新插件不得使用它们，现有插件也应在下一个主要版本移除它们之前完成迁移。仅 Pi 的嵌入式扩展工厂注册 API 已被移除；请改用工具结果中间件。

OpenClaw 不会在引入替代方案的同一次变更中移除或重新解释已文档化的插件行为。破坏性契约变更必须先经过兼容适配器、诊断、文档和弃用窗口。这适用于 SDK 导入、清单字段、设置 API、钩子和运行时注册行为。

<Warning>
  向后兼容层将在未来的主要版本中移除。仍从这些表面导入的插件届时会中断。仅 Pi 的嵌入式扩展工厂注册已经不再加载。
</Warning>
...
现代插件 SDK 修复了这个问题：每个导入路径（`openclaw/plugin-sdk/\<subpath\>`）都是一个小型、自包含模块，具有明确用途和文档化契约。

面向内置渠道的旧版提供商便捷衔接也已移除。带渠道品牌的辅助衔接是私有单体仓库捷径，不是稳定的插件契约。请改用狭窄的通用 SDK 子路径。在内置插件工作区内，将提供商拥有的辅助工具保留在该插件自己的 `api.ts` 或 `runtime-api.ts` 中。

当前的内置提供商示例：

- Anthropic 将 Claude 专属流辅助工具保留在自己的 `api.ts` / `contract-api.ts` 衔接中
...
1. 添加新契约
2. 通过兼容适配器保留旧行为的接线
3. 发出诊断或警告，命名旧路径和替代路径
4. 在测试中覆盖两条路径
5. 记录弃用和迁移路径
6. 仅在已宣布的迁移窗口之后移除，通常是在主要版本中

维护者可以使用 `pnpm plugins:boundary-report` 审核当前迁移队列。使用 `pnpm plugins:boundary-report:summary` 获取紧凑计数，使用 `--owner <id>` 查看单个插件或兼容性所有者；当 CI 门禁应因到期兼容性记录、跨所有者保留 SDK 导入或未使用的保留 SDK 子路径而失败时，使用 `pnpm plugins:boundary-report:ci`。该报告按移除日期分组已弃用的兼容性记录，统计本地代码/文档引用，暴露跨所有者保留 SDK 导入，并汇总私有 memory-host SDK 桥接，使兼容性清理保持显式，而不是依赖临时搜索。保留 SDK 子路径必须有被跟踪的所有者用法；未使用的保留辅助工具导出应从公共 SDK 中移除。

如果某个清单字段仍被接受，插件作者可以继续使用，直到文档和诊断另有说明。新代码应优先使用文档化替代方案，但现有插件不应在普通次要版本发布期间中断。

## 如何迁移

<Steps>
  <Step title="迁移运行时配置加载/写入辅助工具">
    内置插件应停止直接调用 `api.runtime.config.loadConfig()` 和 `api.runtime.config.writeConfigFile(...)`。优先使用已经传入当前调用路径的配置。需要当前进程快照的长生命周期处理程序可以使用 `api.runtime.config.current()`。长生命周期智能体工具应在 `execute` 内使用工具上下文的 `ctx.getRuntimeConfig()`，这样在配置写入前创建的工具仍能看到刷新后的运行时配置。
    ...
    当调用方知道该变更需要干净地重启 Gateway 网关时，使用 `afterWrite: { mode: "restart", reason: "..." }`；只有当调用方拥有后续处理并有意抑制重载规划器时，才使用 `afterWrite: { mode: "none", reason: "..." }`。变更结果包含一个带类型的 `followUp` 摘要，用于测试和日志记录；Gateway 网关仍负责应用或调度重启。`loadConfig` 和 `writeConfigFile` 在迁移窗口期间仍作为已弃用的外部插件兼容辅助工具保留，并通过 `runtime-config-load-write` 兼容代码警告一次。内置插件和仓库运行时代码受到扫描器护栏保护，位于 `pnpm check:deprecated-internal-config-api` 和 `pnpm check:no-runtime-action-load-config`：新的生产插件用法会直接失败，直接配置写入会失败，Gateway 网关服务器方法必须使用请求运行时快照，运行时渠道发送/操作/客户端辅助工具必须从其边界接收配置，且长生命周期运行时模块允许的环境 `loadConfig()` 调用数为零。

    新插件代码还应避免导入宽泛的 `openclaw/plugin-sdk/config-runtime` 兼容工具桶。请使用与任务匹配的狭窄 SDK 子路径：
    ...
    | 分组策略运行时辅助工具 | `openclaw/plugin-sdk/runtime-group-policy` |
    | 密钥输入解析 | `openclaw/plugin-sdk/secret-input-runtime` |
    | 模型/会话覆盖 | `openclaw/plugin-sdk/model-session-runtime` |

    内置插件及其测试由扫描器防护，避免使用这个宽泛工具桶，从而让导入和 mock 保持在其所需行为的本地范围内。这个宽泛工具桶仍为外部兼容性存在，但新代码不应依赖它。

  </Step>

  <Step title="将 Pi 工具结果扩展迁移到中间件">
    内置插件必须将仅 Pi 的 `api.registerEmbeddedExtensionFactory(...)` 工具结果处理程序替换为运行时中立的中间件。
  ...
  <Step title="将原生审批处理程序迁移到能力事实">
    支持审批的渠道插件现在通过 `approvalCapability.nativeRuntime` 加上共享运行时上下文注册表公开原生审批行为。

    关键变更：

    - 将 `approvalCapability.handler.loadRuntime(...)` 替换为 `approvalCapability.nativeRuntime`
    - 将审批专属认证/交付从旧版 `plugin.auth` / `plugin.approvals` 接线迁移到 `approvalCapability`
    - `ChannelPlugin.approvals` 已从公共渠道插件契约中移除；将交付/原生/渲染字段迁移到 `approvalCapability`
    - `plugin.auth` 仅保留用于渠道登录/登出流程；核心不再读取其中的审批认证钩子
    - 通过 `openclaw/plugin-sdk/channel-runtime-context` 注册渠道拥有的运行时对象，例如客户端、令牌或 Bolt 应用
    - 不要从原生审批处理程序发送插件拥有的重路由通知；核心现在根据实际交付结果负责路由到其他位置的通知
  ...
  <Step title="审计 Windows 包装器回退行为">
    ...
    如果你的调用方并非有意依赖 shell 回退，不要设置 `allowShellFallback`，而是处理抛出的错误。
  ...
  <Step title="替换宽泛的 infra-runtime 导入">
    为了外部兼容性，`openclaw/plugin-sdk/infra-runtime` 仍然存在，但新代码应导入它实际需要的聚焦辅助函数接口：
    ...
    内置插件会由扫描器防止使用 `infra-runtime`，因此仓库代码无法回退到宽泛的桶式导出。

  </Step>

  <Step title="迁移渠道路由辅助函数">
    新的渠道路由代码应使用 `openclaw/plugin-sdk/channel-route`。在迁移窗口期间，较旧的 route-key 和 comparable-target 名称仍会作为兼容别名保留，但新插件应使用直接描述行为的路由名称：
    ...
    现代路由辅助函数会在原生审批、回复抑制、入站去重、cron 投递和会话路由中一致地规范化 `{ channel, to, accountId, threadId }`。如果你的插件拥有自定义目标语法，请使用 `resolveChannelRouteTargetWithParser(...)` 将该解析器适配到同一路由目标契约中。
...
## 导入路径参考
  ...
  | `plugin-sdk/channel-reply-pipeline` | 回复前缀、正在输入和来源递送接线 | `createChannelReplyPipeline`, `resolveChannelSourceReplyDeliveryMode` |
  ...
  | `plugin-sdk/channel-config-schema-legacy` | 已弃用的内置配置模式 | 仅兼容别名；维护中的内置插件请使用 `plugin-sdk/bundled-channel-config-schema` |
  ...
  | `plugin-sdk/outbound-runtime` | 出站运行时辅助工具 | 出站递送、身份/发送委托、会话、格式化和载荷规划辅助工具 |
  ...
  | `plugin-sdk/approval-runtime` | 审批提示辅助工具 | Exec/插件审批载荷、审批能力/配置档案辅助工具、原生审批路由/运行时辅助工具，以及结构化审批显示路径格式化 |
  | `plugin-sdk/approval-auth-runtime` | 审批授权辅助工具 | 审批人解析、同聊天操作授权 |
  | `plugin-sdk/approval-client-runtime` | 审批客户端辅助工具 | 原生 exec 审批配置档案/过滤器辅助工具 |
  | `plugin-sdk/approval-delivery-runtime` | 审批递送辅助工具 | 原生审批能力/递送适配器 |
  ...
  | `plugin-sdk/approval-handler-runtime` | 审批处理程序辅助工具 | 更广泛的审批处理程序运行时辅助工具；当更窄的适配器/Gateway 网关接缝足够时优先使用它们 |
  ...
  | `plugin-sdk/delivery-queue-runtime` | 递送队列辅助工具 | `drainPendingDeliveries` |
  ...
  | `plugin-sdk/webhook-request-guards` | Webhook 正文防护辅助工具 | 请求正文读取/限制辅助工具 |
  | `plugin-sdk/reply-runtime` | 共享回复运行时 | 入站分发、心跳、回复规划器、分块 |
  | `plugin-sdk/reply-dispatch-runtime` | 精简回复分发辅助工具 | 最终化、提供商分发和对话标签辅助工具 |
  ...
  | `plugin-sdk/logging-core` | 日志辅助工具 | 子系统日志记录器和遮蔽辅助工具 |
  ...
  | `plugin-sdk/provider-selection-runtime` | 提供商选择辅助工具 | 已配置或自动的提供商选择以及原始提供商配置合并 |
  ...
  | `plugin-sdk/provider-web-search-contract` | 提供商 web-search 契约辅助工具 | 窄范围 web-search 配置/凭证契约辅助工具，例如 `createWebSearchProviderContractFields`、`enablePluginInConfig`、`resolveProviderWebSearchPluginConfig`，以及有作用域的凭证设置器/获取器 |
  ...
  | `plugin-sdk/provider-stream` | 提供商流包装器辅助工具 | `ProviderStreamFamily`、`buildProviderStreamFamilyHooks`、`composeProviderStreamWrappers`、流包装器类型，以及共享的 Anthropic/Bedrock/DeepSeek V4/Google/Kilocode/Moonshot/OpenAI/OpenRouter/Z.A.I/MiniMax/Copilot 包装器辅助工具 |
  ...
  | `plugin-sdk/text-runtime` | 共享文本辅助工具 | 智能体可见文本剥离、Markdown 渲染/分块/表格辅助工具、遮蔽辅助工具、指令标签辅助工具、安全文本实用工具，以及相关文本/日志辅助工具 |
  ...
  | `plugin-sdk/image-generation` | 图像生成辅助工具 | 图像生成提供商类型，以及图像资源/数据 URL 辅助工具和 OpenAI 兼容图像提供商构建器 |
  ...
  | `plugin-sdk/interactive-runtime` | 交互式回复辅助工具 | 交互式回复载荷规范化/规约 |
  | `plugin-sdk/channel-config-primitives` | 渠道配置基元 | 窄范围渠道配置架构基元 |
  | `plugin-sdk/channel-config-writes` | 渠道配置写入辅助工具 | 渠道配置写入授权辅助工具 |
  | `plugin-sdk/channel-plugin-common` | 共享渠道前置导入 | 共享渠道插件前置导出 |
  ...
  | `plugin-sdk/memory-host-files` | 记忆宿主文件/运行时别名 | 供应商中立的记忆宿主文件/运行时辅助工具别名 |
  | `plugin-sdk/memory-host-markdown` | 托管 Markdown 辅助工具 | 面向记忆相关插件的共享托管 Markdown 辅助工具 |
  | `plugin-sdk/memory-host-search` | 活跃记忆搜索外观 | 延迟活跃记忆搜索管理器运行时外观 |
  | `plugin-sdk/memory-host-status` | 记忆宿主状态别名 | 供应商中立的记忆宿主状态辅助工具别名 |
  | `plugin-sdk/testing` | 测试实用工具 | 旧版宽泛兼容桶；优先使用聚焦的测试子路径，例如 `plugin-sdk/plugin-test-runtime`、`plugin-sdk/channel-test-helpers`、`plugin-sdk/channel-target-testing`、`plugin-sdk/test-env` 和 `plugin-sdk/test-fixtures` |
</Accordion>
...
预留的内置插件辅助 seam 已从公开 SDK
导出映射中退役，只有明确记录的兼容 facade 例外，例如为已发布的
`@openclaw/discord@2026.3.13` 包保留的已弃用 `plugin-sdk/discord`
shim。特定所有者的辅助工具位于对应所有者的插件包内部；共享宿主行为应通过通用 SDK
契约迁移，例如 `plugin-sdk/gateway-runtime`、`plugin-sdk/security-runtime`
和 `plugin-sdk/plugin-config-runtime`。

使用与任务匹配的最窄导入。如果找不到某个导出，请检查 `src/plugin-sdk/`
中的源代码，或询问维护者应由哪个通用契约来拥有它。

## 活跃弃用项

这些是适用于插件 SDK、提供商契约、运行时表面和 manifest 的更窄弃用项。每一项今天仍可使用，但会在未来的主要版本中移除。每项下面的条目会把旧 API 映射到它的规范替代项。
...
<AccordionGroup>
  ...
  <Accordion title="渠道运行时 shim 和渠道动作辅助工具">
    `openclaw/plugin-sdk/channel-runtime` 是用于较旧
    渠道插件的兼容 shim。新代码不要导入它；请使用
    `openclaw/plugin-sdk/channel-runtime-context` 注册运行时
    对象。

    `openclaw/plugin-sdk/channel-actions` 中的 `channelActions*` 辅助工具会与原始 “actions” 渠道导出一起
    弃用。改为通过语义化的 `presentation` 表面暴露能力，也就是渠道插件
    声明它们渲染什么（卡片、按钮、选择器），而不是它们接受哪些原始
    动作名称。
  ...
  <Accordion title="纯文本渠道 envelope → BodyForAgent">
    ...
    **新**：`BodyForAgent` 加结构化用户上下文块。渠道
    插件会把路由元数据（线程、主题、回复目标、反应）作为
    类型化字段附加，而不是把它们拼接进提示字符串。对于合成的
    面向助手 envelope，`formatAgentEnvelope(...)` 辅助工具仍受支持，
    但入站纯文本 envelope 正在逐步退出。
  ...
  <Accordion title="Thinking policy 钩子 → resolveThinkingProfile">
    **旧**（`ProviderThinkingPolicy` 上的三个独立钩子）：
    `isBinaryThinking(ctx)`、`supportsXHighThinking(ctx)` 和
    `resolveDefaultThinkingLevel(ctx)`。

    **新**：单个 `resolveThinkingProfile(ctx)`，返回包含规范 `id`、可选 `label`
    和分级 level 列表的
    `ProviderThinkingProfile`。OpenClaw 会按 profile
    排名自动降级过时的已存储值。
  ...
  <Accordion title="外部 OAuth 提供商 fallback → contracts.externalAuthProviders">
    **旧**：实现 `resolveExternalOAuthProfiles(...)`，但不在插件 manifest 中
    声明提供商。
  ...
  <Accordion title="提供商环境变量查找 → setup.providers[].envVars">
    ...
    **新**：把相同的环境变量查找镜像到 manifest 上的 `setup.providers[].envVars`。
  ...
  <Accordion title="runtime.tasks.flow → runtime.tasks.managedFlows">
    ...
    **新**：`runtime.tasks.managedFlows` 为需要从 flow
  ...
  <Accordion title="嵌入式扩展工厂 → 智能体 tool-result middleware">
    已在上文 “如何迁移 → 将 Pi tool-result 扩展迁移到
    middleware” 中说明。为完整起见也列在这里：已移除的仅限 Pi 的
...
barrel 中跟踪。它们不会影响第三方插件契约，也不会在此处列出。如果你直接使用某个内置插件的本地 barrel，请在升级前阅读该 barrel 中的
弃用注释。
</Note>
...
| ---------------------- | ----------------------------------------------------------------------- |
| **现在**               | 已弃用表面会发出运行时警告                                              |
| **下一个主要版本**     | 已弃用表面将被移除；仍在使用它们的插件会失败                            |

所有核心插件都已经迁移完成。外部插件应在下一个主要版本之前迁移。
