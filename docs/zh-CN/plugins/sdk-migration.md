...
title: 插件 SDK 迁移
x-i18n:
    generated_at: "2026-07-01T12:48:16Z"
    model: gpt-5.5
...
OpenClaw 已从宽泛的向后兼容层迁移到现代插件架构，并使用聚焦且有文档说明的导入。如果你的插件是在新架构之前构建的，本指南可帮助你迁移。
...
- **`openclaw/plugin-sdk/infra-runtime`** - 一个宽泛的运行时辅助 barrel，混合了系统事件、Heartbeat 状态、投递队列、fetch/proxy 辅助工具、文件辅助工具、审批类型和不相关的工具。
- **`openclaw/plugin-sdk/config-runtime`** - 一个宽泛的配置兼容性 barrel，在迁移窗口期间仍携带已弃用的直接加载/写入辅助工具。
- **`openclaw/extension-api`** - 一个桥接层，让插件可以直接访问主机侧辅助工具，例如嵌入式智能体 runner。
- **`api.registerEmbeddedExtensionFactory(...)`** - 一个已移除的仅适用于嵌入式 runner 的内置扩展钩子，它曾可以观察嵌入式 runner 事件，例如 `tool_result`。

这些宽泛的导入表面现在已**弃用**。它们在运行时仍然可用，但新插件不得使用它们，现有插件也应在下一个主版本移除它们之前完成迁移。仅适用于嵌入式 runner 的扩展工厂注册 API 已被移除；请改用工具结果中间件。

OpenClaw 不会在引入替代方案的同一个变更中移除或重新解释已有文档说明的插件行为。破坏性契约变更必须先经过兼容性适配器、诊断、文档和弃用窗口。这适用于 SDK 导入、清单字段、设置 API、钩子和运行时注册行为。
...
- **循环依赖** - 宽泛的重新导出让导入循环很容易出现
- **API 表面不清晰** - 无法判断哪些导出是稳定的，哪些是内部的

现代插件 SDK 修复了这一点：每个导入路径（`openclaw/plugin-sdk/\<subpath\>`）都是一个小型、自包含的模块，具备清晰用途和有文档说明的契约。

面向内置渠道的旧版提供商便利接口也已移除。带渠道品牌的辅助接口是私有单体仓库快捷方式，不是稳定的插件契约。请改用窄范围的通用 SDK 子路径。在内置插件工作区内部，请将提供商拥有的辅助工具保留在该插件自己的 `api.ts` 或 `runtime-api.ts` 中。

当前内置提供商示例：

- Anthropic 将 Claude 特定的流辅助工具保留在自己的 `api.ts` / `contract-api.ts` 接口中
- OpenAI 将提供商构建器、默认模型辅助工具和 realtime 提供商构建器保留在自己的 `api.ts` 中
- OpenRouter 将提供商构建器和新手引导/配置辅助工具保留在自己的 `api.ts` 中

## Talk 和实时语音迁移计划

实时语音、电话、会议和浏览器 Talk 代码正在从表面本地的轮次记账迁移到由 `openclaw/plugin-sdk/realtime-voice` 导出的共享 Talk 会话控制器。新控制器拥有通用 Talk 事件信封、活跃轮次状态、采集状态、输出音频状态、近期事件历史和过期轮次拒绝。提供商插件应继续拥有供应商特定的实时会话；表面插件应继续拥有采集、播放、电话和会议差异处理。

这次 Talk 迁移有意采用破坏性但干净的方式：
...
4. 在 Gateway 网关 `hello-ok.features.events` 中公告一个实时 Talk 事件渠道：`talk.event`。
5. 删除旧的 realtime HTTP 端点以及任何请求时指令覆盖路径。

新代码不应直接调用 `createTalkEventSequencer(...)`，除非它正在实现低层适配器或测试夹具。请优先使用共享控制器，这样没有轮次 ID 就无法发出轮次范围事件，过期的 `turnEnd` / `turnCancel` 调用无法清除更新的活跃轮次，并且输出音频生命周期事件在电话、会议、浏览器中继、托管房间交接和原生 Talk 客户端之间保持一致。
...
浏览器拥有的 WebRTC/提供商 websocket 会话使用 `talk.client.create`，因为浏览器拥有提供商协商和媒体传输，而 Gateway 网关拥有凭证、指令和工具策略。`talk.session.*` 是由 Gateway 网关管理的通用表面，适用于 gateway-relay realtime、gateway-relay transcription，以及托管房间原生 STT/TTS 会话。

将 realtime 选择器放在 `talk.provider` / `talk.providers` 旁边的旧版配置应使用 `openclaw doctor --fix` 修复；运行时 Talk 不会将语音/TTS 提供商配置重新解释为实时提供商配置。

受支持的 `talk.session.create` 组合有意保持精简：

| 模式            | 传输       | 大脑           | 所有者              | 说明                                                                                                              |
| --------------- | --------------- | --------------- | ------------------ | ------------------------------------------------------------------------------------------------------------------ |
| `realtime`      | `gateway-relay` | `agent-consult` | Gateway 网关            | 全双工提供商音频通过 Gateway 网关桥接；工具调用通过 agent-consult 工具路由。      |
| `transcription` | `gateway-relay` | `none`          | Gateway 网关            | 仅流式 STT；调用方发送输入音频并接收转写事件。                                        |
| `stt-tts`       | `managed-room`  | `agent-consult` | 原生/客户端房间 | 按键通话和对讲机风格房间，客户端拥有采集/播放，Gateway 网关拥有轮次状态。 |
| `stt-tts`       | `managed-room`  | `direct-tools`  | 原生/客户端房间 | 仅限管理员的房间模式，面向可信的一方表面，可直接执行 Gateway 网关工具动作。                  |
...
统一控制词表也被有意保持为窄范围：
  ...
  | 方法                            | 适用范围                                                | 契约                                                                                                                                                                                     |
  | ------------------------------- | ------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
  | `talk.session.appendAudio`      | `realtime/gateway-relay`、`transcription/gateway-relay` | 将一个 base64 PCM 音频分块追加到由同一个 Gateway 网关连接拥有的提供商会话。                                                                                                             |
  | `talk.session.startTurn`        | `stt-tts/managed-room`                                  | 启动一个托管房间用户轮次。                                                                                                                                                              |
  | `talk.session.endTurn`          | `stt-tts/managed-room`                                  | 在过期轮次校验后结束当前轮次。                                                                                                                                                          |
  ...
  | `talk.session.submitToolResult` | `realtime/gateway-relay`                                | 完成由中继发出的提供商工具调用；传入 `options.willContinue` 表示临时输出，或传入 `options.suppressResponse` 以在不生成另一条助手响应的情况下满足该调用。                                |
  | `talk.session.steer`            | 智能体支持的 Talk 会话                                  | 将口述的 `status`、`steer`、`cancel` 或 `followup` 控制发送到从 Talk 会话解析出的当前嵌入式运行。                                                                                        |
  ...
  <Step title="Migrate runtime config load/write helpers">
    ...
    当调用方知道该变更需要干净重启 Gateway 网关时，使用
    `afterWrite: { mode: "restart", reason: "..." }`；只有当调用方拥有后续处理并且有意
    抑制重载规划器时，才使用
    ...
    `pnpm check:no-runtime-action-load-config` 中的扫描器护栏保护：
    新的生产插件用法会直接失败，直接配置写入会失败，Gateway 网关服务器方法必须使用
    请求运行时快照，运行时频道发送/动作/客户端辅助函数
    必须从其边界接收配置，并且长生命周期运行时模块
    不允许有任何环境式 `loadConfig()` 调用。

    新插件代码还应避免导入宽泛的
    `openclaw/plugin-sdk/config-runtime` 兼容性桶。使用与任务匹配的窄
    SDK 子路径：
    ...
    桶，因此导入和 mock 会保持局限于它们需要的行为。该宽泛
    桶仍然存在用于外部兼容性，但新代码不应
    依赖它。
  ...
  <Step title="Migrate embedded tool-result extensions to middleware">
    ...
    已安装插件在被显式启用并且在
    `contracts.agentToolResultMiddleware` 中声明每个目标运行时时，也可以注册工具结果中间件。
    未声明的已安装中间件
    注册会被拒绝。
  ...
  <Step title="Replace with focused imports">
    ...
    | 会话存储帮助程序 | `api.runtime.agent.session.*` |

  </Step>

  <Step title="替换宽泛的 infra-runtime 导入">
    `openclaw/plugin-sdk/infra-runtime` 仍然为外部兼容性而存在，
    但新代码应导入它实际需要的聚焦帮助程序表面：

    | 需求 | 导入 |
    | --- | --- |
    | 系统事件队列帮助程序 | `openclaw/plugin-sdk/system-event-runtime` |
    | Heartbeat 唤醒、事件和可见性帮助程序 | `openclaw/plugin-sdk/heartbeat-runtime` |
    ...
    | 安全本地文件/媒体路径帮助程序 | `openclaw/plugin-sdk/file-access-runtime` |
    | 感知调度器的 fetch | `openclaw/plugin-sdk/runtime-fetch` |
    | 代理和受保护的 fetch 帮助程序 | `openclaw/plugin-sdk/fetch-runtime` |
    | SSRF 调度器策略类型 | `openclaw/plugin-sdk/ssrf-dispatcher` |
    | 审批请求/解决类型 | `openclaw/plugin-sdk/approval-runtime` |
    | 审批回复载荷和命令帮助程序 | `openclaw/plugin-sdk/approval-reply-runtime` |
    | 错误格式化帮助程序 | `openclaw/plugin-sdk/error-runtime` |
    | 传输就绪等待 | `openclaw/plugin-sdk/transport-ready-runtime` |
    | 安全令牌帮助程序 | `openclaw/plugin-sdk/secure-random-runtime` |
    ...
    内置插件会受到扫描器保护，避免使用 `infra-runtime`，因此仓库代码
    无法回退到宽泛的 barrel。

  </Step>

  <Step title="迁移渠道路由帮助程序">
    新的渠道路由代码应使用 `openclaw/plugin-sdk/channel-route`。
    旧的 route-key 和 comparable-target 名称在迁移窗口期间仍保留为兼容性别名，
    但新插件应使用直接描述行为的路由名称：
    ...
    现代路由帮助程序会在原生审批、回复抑制、入站去重、
    cron 投递和会话路由中一致地规范化 `{ channel, to, accountId, threadId }`。

    不要新增对 `ChannelMessagingAdapter.parseExplicitTarget`、
    由解析器支持的 loaded-route 帮助程序（`parseExplicitTargetForLoadedChannel`
    或 `resolveRouteTargetForLoadedChannel`）或
    来自 `plugin-sdk/channel-route` 的 `resolveChannelRouteTargetWithParser(...)`
    的使用。这些钩子已弃用，仅在迁移窗口期间为旧插件保留。
    新的渠道插件应使用 `messaging.targetResolver.resolveTarget(...)` 来进行目标 ID 规范化
    和目录未命中回退，在核心需要早期对端类型时使用
    `messaging.inferTargetChatType(...)`，并使用 `messaging.resolveOutboundSessionRoute(...)`
    处理提供商原生会话和线程身份。
...
## 导入路径参考
  ...
  | `plugin-sdk/channel-config-schema-legacy` | 已弃用的内置配置 schema | 仅兼容性别名；维护的内置插件请使用 `plugin-sdk/bundled-channel-config-schema` |
  ...
  | `plugin-sdk/channel-lifecycle` | 已弃用的兼容性 facade | 使用 `plugin-sdk/channel-outbound` |
  | `plugin-sdk/inbound-envelope` | 入站信封辅助工具 | 共享路由 + 信封构建器辅助工具 |
  | `plugin-sdk/channel-inbound` | 入站接收辅助工具 | 上下文构建、格式化、根目录、运行器、预备回复分发，以及分发谓词 |
  | `plugin-sdk/messaging-targets` | 已弃用的目标解析导入路径 | 使用 `plugin-sdk/channel-targets` 获取通用目标解析辅助工具，使用 `plugin-sdk/channel-route` 进行路由比较，并使用插件自有的 `messaging.targetResolver` / `messaging.resolveOutboundSessionRoute` 进行提供商特定的目标解析 |
  | `plugin-sdk/outbound-media` | 出站媒体辅助工具 | 共享出站媒体加载 |
  | `plugin-sdk/outbound-send-deps` | 已弃用的兼容性 facade | 使用 `plugin-sdk/channel-outbound` |
  | `plugin-sdk/channel-outbound` | 出站消息生命周期辅助工具 | 消息适配器、回执、持久发送辅助工具、实时预览/流式传输辅助工具、回复选项、生命周期辅助工具、出站身份和载荷规划 |
  | `plugin-sdk/channel-streaming` | 已弃用的兼容性 facade | 使用 `plugin-sdk/channel-outbound` |
  | `plugin-sdk/outbound-runtime` | 已弃用的兼容性 facade | 使用 `plugin-sdk/channel-outbound` |
  | `plugin-sdk/thread-bindings-runtime` | 线程绑定辅助工具 | 线程绑定生命周期和适配器辅助工具 |
  | `plugin-sdk/agent-media-payload` | 旧版媒体载荷辅助工具 | 面向旧版字段布局的 Agent 媒体载荷构建器 |
  ...
  | `plugin-sdk/gateway-runtime` | Gateway 网关辅助工具 | Gateway 网关客户端、事件循环就绪启动辅助工具、通告的 LAN 主机解析，以及渠道状态补丁辅助工具 |
  ...
  | `plugin-sdk/approval-auth-runtime` | 审批鉴权辅助工具 | 审批人解析、同一聊天操作鉴权 |
  ...
  | `plugin-sdk/approval-handler-adapter-runtime` | 审批适配器辅助工具 | 面向热渠道入口点的轻量原生审批适配器加载辅助工具 |
  | `plugin-sdk/approval-handler-runtime` | 审批处理器辅助工具 | 更宽范围的审批处理器运行时辅助工具；当更窄的适配器/Gateway 网关边界足够时，优先使用它们 |
  ...
  | `plugin-sdk/security-runtime` | 安全辅助工具 | 共享信任、私信门控、根目录约束文件/路径辅助工具、外部内容和密钥收集辅助工具 |
  ...
  | `plugin-sdk/command-auth` | 命令门控和命令表面辅助工具 | `resolveControlCommandGate`、发送方授权辅助工具、命令注册表辅助工具，包括动态参数菜单格式化 |
  ...
  | `plugin-sdk/reply-history` | 回复历史辅助工具 | `createChannelHistoryWindow`；已弃用的映射辅助兼容性导出，例如 `buildPendingHistoryContextFromMap`、`recordPendingHistoryEntry` 和 `clearHistoryEntriesIfEnabled` |
  ...
  | `plugin-sdk/logging-core` | 日志辅助工具 | 子系统日志器和密文遮盖辅助工具 |
  ...
  | `plugin-sdk/self-hosted-provider-setup` | 聚焦 OpenAI 兼容自托管提供商的设置辅助工具 | 相同的自托管提供商发现/配置辅助工具 |
  | `plugin-sdk/provider-auth-runtime` | 提供商运行时凭证辅助工具 | 运行时 API key 解析辅助工具 |
  | `plugin-sdk/provider-auth-api-key` | 提供商 API key 设置辅助工具 | API key 新手引导/配置写入辅助工具 |
  ...
  | `plugin-sdk/provider-web-search-contract` | 提供商 Web 搜索契约辅助工具 | 窄 Web 搜索配置/凭据契约辅助工具，例如 `createWebSearchProviderContractFields`、`enablePluginInConfig`、`resolveProviderWebSearchPluginConfig`，以及作用域凭据 setter/getter |
  ...
  | `plugin-sdk/provider-usage` | 提供商用量辅助工具 | `fetchClaudeUsage`、`fetchGeminiUsage`、`fetchGithubCopilotUsage` 和其他提供商用量辅助工具 |
  | `plugin-sdk/provider-stream` | 提供商流包装辅助工具 | `ProviderStreamFamily`、`buildProviderStreamFamilyHooks`、`composeProviderStreamWrappers`、流包装类型，以及共享 Anthropic/Bedrock/DeepSeek V4/Google/Kilocode/Moonshot/OpenAI/OpenRouter/Z.A.I/MiniMax/Copilot 包装辅助工具 |
  ...
  | `plugin-sdk/speech` | Speech 辅助工具 | Speech 提供商类型，以及面向提供商的指令、注册表、验证辅助工具和 OpenAI 兼容 TTS 构建器 |
  | `plugin-sdk/speech-core` | 共享 Speech 核心 | Speech 提供商类型、注册表、指令、规范化 |
  | `plugin-sdk/realtime-transcription` | 实时转录辅助工具 | 提供商类型、注册表辅助工具和共享 WebSocket 会话辅助工具 |
  | `plugin-sdk/realtime-voice` | 实时语音辅助工具 | 提供商类型、注册表/解析辅助工具、桥接会话辅助工具、共享 Agent 回话队列、活跃运行语音控制、转录/事件健康、回声抑制、咨询问题匹配、强制咨询协调、轮次上下文跟踪、输出活动跟踪，以及快速上下文咨询辅助工具 |
  ...
  | `plugin-sdk/memory-host-core` | 记忆宿主核心运行时别名 | 供应商中立的记忆宿主核心运行时辅助工具别名 |
  | `plugin-sdk/memory-host-events` | 记忆宿主事件日志别名 | 供应商中立的记忆宿主事件日志辅助工具别名 |
  | `plugin-sdk/memory-host-files` | 已弃用的记忆文件/运行时别名 | 使用 `plugin-sdk/memory-core-host-runtime-files` |
  | `plugin-sdk/memory-host-markdown` | 托管 Markdown 辅助工具 | 供记忆相邻插件使用的共享托管 Markdown 辅助工具 |
  ...
  | `plugin-sdk/testing` | 测试工具 | 仓库本地已弃用的兼容桶导出；使用聚焦的仓库本地测试子路径，例如 `plugin-sdk/plugin-test-runtime`、`plugin-sdk/channel-test-helpers`、`plugin-sdk/channel-target-testing`、`plugin-sdk/test-env` 和 `plugin-sdk/test-fixtures` |
</Accordion>

该表有意只列出通用迁移子集，而不是完整的 SDK
表面。编译器入口点清单位于
`scripts/lib/plugin-sdk-entrypoints.json`；包导出根据
公共子集生成。

保留的内置插件辅助接缝已从公共 SDK
导出映射中移除，只有明确记录的兼容性门面除外，例如为已发布的
`@openclaw/discord@2026.3.13` 包保留的已弃用
`plugin-sdk/discord` shim。所有者专属辅助函数位于
所属插件包内部；共享主机行为应通过通用 SDK
...
每一项今天仍可工作，但会在未来的主要版本中移除。每项下方的条目
将旧 API 映射到其规范替代项。
...
<AccordionGroup>
  ...
  <Accordion title="渠道运行时 shim 和渠道动作辅助函数">
    ...
    会随原始 “actions” 渠道导出一起弃用。请改为通过语义化
  ...
  <Accordion title="deactivate 钩子 → gateway_stop">
    ...
    **新**：`api.on("gateway_stop", handler)`。事件和上下文是相同的
    ...
    `deactivate` 将继续作为已弃用的兼容性别名接线，直到
    2026-08-16 之后。
  ...
  <Accordion title="提供商发现类型 → 提供商目录类型">
    ...
    另有旧版 `ProviderCapabilities` 静态包 - 提供商插件
  ...
  <Accordion title="Thinking 策略钩子 → resolveThinkingProfile">
    ...
    **新**：单个 `resolveThinkingProfile(ctx)`，返回带有规范 `id`、
    可选 `label` 和排序级别列表的
    `ProviderThinkingProfile`。OpenClaw 会按配置文件
    排名自动降级过期的已存储值。

    上下文包括 `provider`、`modelId`、可选合并后的 `reasoning`，
    以及可选合并后的模型 `compat` 事实。提供商插件可以使用这些
    目录事实，仅在已配置的请求契约支持时才暴露模型专属配置文件。

    实现一个钩子，而不是三个。旧版钩子在弃用窗口期间
    仍可工作，但不会与配置文件结果组合。

  </Accordion>

  <Accordion title="外部认证提供商 → contracts.externalAuthProviders">
    **旧**：实现外部认证钩子，但未在插件清单中声明
    提供商。
  ...
  <Accordion title="提供商环境变量查找 → setup.providers[].envVars">
    ...
    **新**：将相同的环境变量查找镜像到清单上的
    `setup.providers[].envVars`。这会把设置/状态环境元数据整合到一个
    位置，并避免仅为回答环境变量查找而启动插件运行时。

    `providerAuthEnvVars` 将通过兼容性适配器继续受支持，
    直到弃用窗口关闭。
  ...
  <Accordion title="Memory 嵌入提供商 API">
    ...
    **新**：`api.registerEmbeddingProvider(...)` 加
    ...
    通用嵌入提供商契约可在记忆之外复用，并且是新提供商的
    受支持路径。记忆专属注册 API 会在现有提供商迁移期间继续
    作为已弃用兼容性接线保留。插件检查会将非内置用法报告为
    兼容性债务。
  ...
  <Accordion title="runtime.tasks.flow → runtime.tasks.managedFlows">
    ...
    **新**：`runtime.tasks.managedFlows` 为需要从流中创建、更新、
    取消或运行子任务的插件保留托管 TaskFlow 变更
  ...
  <Accordion title="嵌入式扩展工厂 → 智能体工具结果中间件">
    已在上方“如何迁移 → 将嵌入式工具结果扩展迁移到
    中间件”中介绍。这里为完整性再次列出：已移除的仅限嵌入式运行器
...
在各自的 `api.ts` 和 `runtime-api.ts` barrel 中跟踪。
它们不会影响第三方插件契约，也未在此处列出。如果你直接消费内置插件的
本地 barrel，请在升级前阅读该 barrel 中的弃用注释。
</Note>
...
| ---------------------- | ----------------------------------------------------------------------- |
| **现在** | 已弃用接口面会发出运行时警告 |
| **下一个主版本发布** | 已弃用接口面将被移除；仍在使用它们的插件将失败 |

所有核心插件都已完成迁移。外部插件应在下一个主版本发布前迁移。
