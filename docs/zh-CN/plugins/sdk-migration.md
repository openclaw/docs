...
title: 插件 SDK 迁移
x-i18n:
    generated_at: "2026-04-28T01:47:24Z"
    model: gpt-5.4
    provider: openai
...
OpenClaw 已从宽泛的向后兼容层迁移到现代插件架构，提供了聚焦且有文档说明的导入路径。如果你的插件是在新架构之前构建的，本指南将帮助你完成迁移。

## 正在发生的变化

旧版插件系统提供了两个高度开放的接口面，让插件可以通过单一入口点导入所需的任何内容：

- **`openclaw/plugin-sdk/compat`** —— 一个单一导入入口，会重新导出数十个辅助工具。它的引入是为了在构建新插件架构期间，保持旧版基于钩子的插件继续工作。
- **`openclaw/plugin-sdk/infra-runtime`** —— 一个宽泛的运行时辅助工具 barrel，混合了系统事件、心跳状态、投递队列、fetch/proxy 辅助工具、文件辅助工具、审批类型以及其他无关工具。
...
- **`api.registerEmbeddedExtensionFactory(...)`** —— 一个已移除、仅限 Pi 的内置扩展钩子，可观察诸如 `tool_result` 之类的嵌入式运行器事件。

这些宽泛的导入接口现已被**弃用**。它们在运行时仍然有效，但新插件不得再使用它们，现有插件也应在下一个主版本移除它们之前完成迁移。仅限 Pi 的嵌入式扩展工厂注册 API 已被移除；请改用工具结果中间件。

OpenClaw 不会在引入替代方案的同一次变更中移除或重新解释已有文档说明的插件行为。破坏契约的变更必须先经过兼容适配器、诊断、文档以及弃用窗口。这一规则适用于 SDK 导入、manifest 字段、设置 API、钩子以及运行时注册行为。

<Warning>
  向后兼容层将在未来的主版本中移除。到那时，仍从这些接口导入的插件将会失效。仅限 Pi 的嵌入式扩展工厂注册现在已经不再加载。
</Warning>
...
带有渠道品牌的辅助工具接口属于私有单体仓库捷径，而不是稳定的插件契约。请改用狭窄、通用的 SDK 子路径。在内置插件工作区中，应将提供商自有辅助工具保留在该插件自己的 `api.ts` 或 `runtime-api.ts` 中。

当前内置提供商示例：

- Anthropic 将 Claude 专用流式辅助工具保留在自己的 `api.ts` / `contract-api.ts` 接口中
- OpenAI 将 provider 构建器、默认模型辅助工具和 realtime provider 构建器保留在自己的 `api.ts` 中
- OpenRouter 将 provider 构建器和新手引导/配置辅助工具保留在自己的 `api.ts` 中
...
如果某个 manifest 字段仍然被接受，插件作者就可以继续使用它，直到文档和诊断另有说明。新代码应优先使用文档说明的替代方案，但现有插件不应在普通的小版本发布中被破坏。
...
<Steps>
  <Step title="迁移运行时配置加载/写入辅助工具">
    ...
    `api.runtime.config.writeConfigFile(...)`。应优先使用当前活动调用路径中已经传入的配置。需要当前进程快照的长生命周期处理器可以使用 `api.runtime.config.current()`。长生命周期的智能体工具应在 `execute` 内使用工具上下文的 `ctx.getRuntimeConfig()`，这样在配置写入之前创建的工具仍然可以看到刷新后的运行时配置。
    ...
    `afterWrite: { mode: "restart", reason: "..." }`；只有当调用方自行负责后续处理，并且有意抑制重载规划器时，才使用
    `afterWrite: { mode: "none", reason: "..." }`。
    变更结果会包含一个类型化的 `followUp` 摘要，供测试和日志使用；Gateway 网关仍然负责执行或调度重启。
    在迁移窗口期间，`loadConfig` 和 `writeConfigFile` 仍作为面向外部插件的已弃用兼容辅助工具保留，并会通过 `runtime-config-load-write` 兼容代码发出一次警告。内置插件和仓库运行时代码受
    `pnpm check:deprecated-internal-config-api` 和
    `pnpm check:no-runtime-action-load-config`
    扫描器护栏保护：新的生产插件用法会直接失败，直接配置写入会失败，Gateway 网关服务器方法必须使用请求运行时快照，运行时渠道发送/动作/客户端辅助工具必须从其边界接收配置，而长生命周期运行时模块中允许存在的环境式 `loadConfig()` 调用数量为零。
    ...
    内置插件及其测试受到扫描器保护，不得依赖这个宽泛 barrel，这样导入和 mock 才能保持局部且只针对其所需行为。
    该宽泛 barrel 为了外部兼容性仍然存在，但新代码不应依赖它。
  ...
  <Step title="将原生审批处理器迁移到能力事实">
    ...
    - 不要从原生审批处理器发送插件自有的改道路由通知；核心现在会基于实际投递结果负责“已路由到其他位置”的通知
    - 当把 `channelRuntime` 传给 `createChannelManager(...)` 时，请提供真实的 `createPluginRuntime().channel` 接口。部分 stub 将被拒绝。
  ...
  <Step title="审查 Windows wrapper 回退行为">
    如果你的插件使用 `openclaw/plugin-sdk/windows-spawn`，那么在 Windows 上无法解析的
    ...
    const program = applyWindowsSpawnProgramPolicy({
      candidate,
      // 仅对有意接受 shell 中介回退的受信任兼容调用方设置此项。
      allowShellFallback: true,
    });
    ```
  ...
  <Step title="替换宽泛的 infra-runtime 导入">
    `openclaw/plugin-sdk/infra-runtime` 为了外部兼容性仍然存在，但新代码应导入其实际所需的聚焦辅助工具接口：
    ...
    内置插件受到扫描器保护，不得使用 `infra-runtime`，因此仓库代码不会退回到这个宽泛 barrel。

  </Step>

  <Step title="迁移渠道路由辅助工具">
    新的渠道路由代码应使用 `openclaw/plugin-sdk/channel-route`。
    较旧的 route-key 和 comparable-target 名称在迁移窗口期间仍作为兼容别名保留，但新插件应使用能够直接描述行为的路由名称：
...
## 导入路径参考
  ...
  | `plugin-sdk/account-helpers` | 狭窄的账户辅助工具 | 账户列表/账户动作辅助工具 |
  ...
  | `plugin-sdk/channel-config-schema-legacy` | 已弃用的内置配置 schema | 仅为兼容别名；维护中的内置插件请使用 `plugin-sdk/bundled-channel-config-schema` |
  ...
  | `plugin-sdk/runtime-env` | 狭窄的运行时环境辅助工具 | 日志器/运行时环境、超时、重试和退避辅助工具 |
  ...
  | `plugin-sdk/approval-handler-runtime` | 审批处理器辅助工具 | 更宽泛的审批处理器运行时辅助工具；如果更狭窄的 adapter/gateway 接口已经足够，应优先使用它们 |
  ...
  | `plugin-sdk/reply-dispatch-runtime` | 狭窄的回复分发辅助工具 | 最终化、provider 分发和会话标签辅助工具 |
  ...
  | `plugin-sdk/provider-web-search-config-contract` | provider Web 搜索配置辅助工具 | 适用于不需要插件启用连接逻辑的 provider 的狭窄 web-search 配置/凭证辅助工具 |
  | `plugin-sdk/provider-web-search-contract` | provider Web 搜索契约辅助工具 | 狭窄的 web-search 配置/凭证契约辅助工具，例如 `createWebSearchProviderContractFields`、`enablePluginInConfig`、`resolveProviderWebSearchPluginConfig` 以及有作用域的凭证 setter/getter |
  | `plugin-sdk/provider-web-search` | provider Web 搜索辅助工具 | web-search provider 注册/缓存/运行时辅助工具 |
  ...
  | `plugin-sdk/text-runtime` | 共享文本辅助工具 | 对智能体可见文本的剥离、markdown 渲染/分块/表格辅助工具、脱敏辅助工具、directive-tag 辅助工具、安全文本工具，以及相关文本/日志辅助工具 |
  | `plugin-sdk/text-chunking` | 文本分块辅助工具 | 出站文本分块辅助工具 |
  | `plugin-sdk/speech` | 语音辅助工具 | 语音 provider 类型，以及面向 provider 的指令、注册表、校验辅助工具和 OpenAI 兼容的 TTS 构建器 |
  ...
  | `plugin-sdk/image-generation` | 图像生成辅助工具 | 图像生成 provider 类型，以及图像资源/data URL 辅助工具 |
  ...
  | `plugin-sdk/memory-host-core` | Memory 宿主核心运行时别名 | 面向厂商中立的 Memory 宿主核心运行时辅助工具别名 |
  | `plugin-sdk/memory-host-events` | Memory 宿主事件日志别名 | 面向厂商中立的 Memory 宿主事件日志辅助工具别名 |
  | `plugin-sdk/memory-host-files` | Memory 宿主文件/运行时别名 | 面向厂商中立的 Memory 宿主文件/运行时辅助工具别名 |
  | `plugin-sdk/memory-host-markdown` | 托管 Markdown 辅助工具 | 面向 Memory 邻接插件的共享托管 Markdown 辅助工具 |
  | `plugin-sdk/memory-host-search` | 活跃 Memory 搜索外观 | 惰性 active-memory search-manager 运行时外观 |
  | `plugin-sdk/memory-host-status` | Memory 宿主状态别名 | 面向厂商中立的 Memory 宿主状态辅助工具别名 |
...
该列表仍然包含一些内置插件辅助工具接口，例如
`plugin-sdk/feishu`、`plugin-sdk/feishu-setup`、`plugin-sdk/zalo`、
`plugin-sdk/zalo-setup` 和 `plugin-sdk/matrix*`。这些接口仍然会继续导出，以支持内置插件维护和兼容性，但它们被有意省略在常见迁移表之外，也不是新插件代码的推荐目标。
...
`plugin-sdk/github-copilot-token` 当前公开的是狭窄的令牌辅助工具接口：
`DEFAULT_COPILOT_API_BASE_URL`、
`deriveCopilotApiBaseUrlFromToken` 和 `resolveCopilotApiToken`。

请使用与任务最匹配的最狭窄导入路径。如果你找不到某个导出，请检查
`src/plugin-sdk/` 下的源码，或在 Discord 中提问。

## 当前活跃的弃用项

这些更狭窄的弃用项适用于整个插件 SDK、provider 契约、
运行时接口和 manifest。它们目前仍然可用，但会在未来的主版本中移除。每个条目下方都说明了旧 API 到其规范替代项的映射关系。
...
<AccordionGroup>
  <Accordion title="command-auth help builders → command-status">
    ...
    **新版（`openclaw/plugin-sdk/command-status`）**：签名相同、导出相同——只是改为从更狭窄的子路径导入。`command-auth`
    会将它们作为兼容 stub 重新导出。
  ...
  <Accordion title="Channel runtime shim and channel actions helpers">
    ...
    辅助工具也会随着原始 “actions” 渠道导出一起弃用。请改为通过具有语义的
    `presentation` 接口来暴露能力——渠道插件声明它们能够渲染什么
  ...
  <Accordion title="Plaintext channel envelopes → BodyForAgent">
    ...
    **新版**：`BodyForAgent` 加上结构化的用户上下文块。渠道
    插件会将路由元数据（线程、主题、reply-to、reactions）作为
    类型化字段附加，而不是把它们拼接进提示字符串中。
    `formatAgentEnvelope(...)` 辅助工具仍然支持用于构造面向助手的合成
  ...
  <Accordion title="Provider discovery types → provider catalog types">
    四个 discovery 类型别名现在都只是 catalog 时代
    类型的轻量包装：
    ...
    以及旧版的 `ProviderCapabilities` 静态对象——provider 插件
    应通过 provider 运行时契约附加能力事实，而不是使用静态对象。
  ...
  <Accordion title="Thinking policy hooks → resolveThinkingProfile">
    ...
    **新版**：单一的 `resolveThinkingProfile(ctx)`，返回一个
    ...
    排序后的级别列表。OpenClaw 会根据 profile 排名，自动降级过时的已存储值。
  ...
  <Accordion title="Provider env-var lookup → setup.providers[].envVars">
    ...
    **新版**：将相同的环境变量查找镜像到 manifest 中的 `setup.providers[].envVars`
    上。这样可以将设置/状态环境变量元数据集中到同一个
    位置，并避免仅仅为了回答环境变量查找而启动插件运行时。
  ...
  <Accordion title="Memory plugin registration → registerMemoryCapability">
    ...
    **新版**：在 memory-state API 上统一调用一次——
    `registerMemoryCapability(pluginId, { promptBuilder, flushPlanResolver, runtime })`。

    相同插槽，单次注册调用。增量式 Memory 辅助工具
  ...
  <Accordion title="Embedded extension factories → agent tool-result middleware">
    上文“如何迁移 → 将 Pi 工具结果扩展迁移到中间件”
    已经覆盖。这里为了完整性再次列出：已移除的仅限 Pi 的
    `api.registerEmbeddedExtensionFactory(...)` 路径，已被
    `api.registerAgentToolResultMiddleware(...)` 所替代，并且需要在
...
<Note>
扩展级别的弃用项（位于 `extensions/` 下内置渠道/provider 插件内部）
会在它们各自的 `api.ts` 和 `runtime-api.ts`
barrel 中跟踪。它们不影响第三方插件契约，因此不会在这里列出。
