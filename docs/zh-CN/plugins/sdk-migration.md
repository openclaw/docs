...
title: 插件 SDK 迁移
x-i18n:
    generated_at: "2026-04-28T00:32:57Z"
    model: gpt-5.4
    provider: openai
...
OpenClaw 已经从宽泛的向后兼容层迁移到现代插件架构，后者提供聚焦且有文档说明的导入路径。如果你的插件是在新架构之前构建的，本指南将帮助你完成迁移。

## 正在发生什么变化

旧插件系统提供了两个高度开放的接口，使插件能够通过单一入口点导入所需的任何内容：

- **`openclaw/plugin-sdk/compat`** — 单一导入路径，重新导出数十个辅助工具。它的引入是为了在构建新插件架构期间，让较旧的基于钩子的插件继续工作。
- **`openclaw/plugin-sdk/infra-runtime`** — 宽泛的运行时辅助 barrel，混合了系统事件、心跳状态、投递队列、fetch/proxy 辅助工具、文件辅助工具、审批类型以及其他不相关的实用工具。
- **`openclaw/plugin-sdk/config-runtime`** — 宽泛的配置兼容性 barrel，在迁移窗口期间仍然保留了已弃用的直接加载/写入辅助工具。
- **`openclaw/extension-api`** — 一个桥接层，使插件能够直接访问宿主侧辅助工具，例如嵌入式智能体运行器。
- **`api.registerEmbeddedExtensionFactory(...)`** — 已移除的、仅限 Pi 的内置扩展钩子，它曾可观察嵌入式运行器事件，例如 `tool_result`。

这些宽泛的导入接口现已被**弃用**。它们在运行时仍然可用，但新插件不得再使用它们，现有插件应在下一个主要版本移除它们之前完成迁移。仅限 Pi 的嵌入式扩展工厂注册 API 已被移除；请改用工具结果中间件。

OpenClaw 不会在引入替代方案的同一次变更中移除或重新解释已有文档说明的插件行为。破坏性契约变更必须先经过兼容性适配器、诊断、文档和弃用窗口。这一原则适用于 SDK 导入、manifest 字段、设置 API、钩子和运行时注册行为。

<Warning>
  向后兼容层将在未来的主要版本中移除。
  仍然从这些接口导入的插件会在那时失效。
  仅限 Pi 的嵌入式扩展工厂注册已无法再加载。
</Warning>
...
现代插件 SDK 修复了这些问题：每个导入路径（`openclaw/plugin-sdk/\<subpath\>`）都是一个小型、独立的模块，具备清晰的用途和有文档说明的契约。

面向内置渠道的旧版提供商便利接口也已移除。
带有渠道品牌的辅助接口属于私有 mono-repo 快捷方式，而不是稳定的插件契约。请改用更窄、更通用的 SDK 子路径。在内置插件工作区内部，请将提供商自有辅助工具保留在该插件自己的 `api.ts` 或 `runtime-api.ts` 中。

当前内置提供商示例：

- Anthropic 将 Claude 专用的流辅助工具保留在其自己的 `api.ts` /
  `contract-api.ts` 接口中
- OpenAI 将提供商构建器、默认模型辅助工具和实时提供商构建器保留在其自己的 `api.ts` 中
- OpenRouter 将提供商构建器以及新手引导/配置辅助工具保留在其自己的 `api.ts` 中
...
4. 在测试中覆盖两条路径
5. 记录弃用和迁移路径
6. 仅在公告的迁移窗口结束后移除，通常是在主要版本中

如果某个 manifest 字段仍然被接受，那么在文档和诊断另有说明之前，插件作者都可以继续使用它。新代码应优先使用有文档说明的替代方案，但现有插件不应在普通次要版本发布期间损坏。
...
<Steps>
  <Step title="迁移运行时配置加载/写入辅助工具">
    ...
    `api.runtime.config.writeConfigFile(...)`。应优先使用当前活跃调用路径中已经传入的配置。对于需要当前进程快照的长生命周期处理器，可以使用 `api.runtime.config.current()`。长生命周期的智能体工具应在 `execute` 内使用工具上下文的 `ctx.getRuntimeConfig()`，这样即使工具创建于配置写入之前，也仍能看到已刷新的运行时配置。
    ...
    当调用方明确知道该变更需要干净重启 Gateway 网关时，请使用
    `afterWrite: { mode: "restart", reason: "..." }`；只有当调用方自行负责后续处理并且有意抑制重载规划器时，才使用
    `afterWrite: { mode: "none", reason: "..." }`。
    变更结果包含一个带类型的 `followUp` 摘要，供测试和日志使用；Gateway 网关仍负责应用或调度重启。
    `loadConfig` 和 `writeConfigFile` 在迁移窗口期间仍作为外部插件的已弃用兼容性辅助工具保留，并会通过 `runtime-config-load-write` 兼容性代码发出一次警告。内置插件和仓库运行时代码受以下扫描器护栏保护：
    `pnpm check:deprecated-internal-config-api` 和
    `pnpm check:no-runtime-action-load-config`：新的生产插件用法会被直接判定失败，直接配置写入会失败，Gateway 网关服务器方法必须使用请求运行时快照，运行时渠道发送/操作/客户端辅助工具必须从其边界接收配置，长生命周期运行时模块不得存在环境式 `loadConfig()` 调用。

    新插件代码也应避免导入宽泛的
    `openclaw/plugin-sdk/config-runtime`
    兼容性 barrel。请使用与任务匹配的窄化 SDK 子路径：
    ...
    内置插件及其测试对这个宽泛 barrel 受扫描器保护，因此导入和 mock 都会保持局部化，只覆盖其所需行为。这个宽泛 barrel 仍然存在以保持外部兼容性，但新代码不应依赖它。
  ...
  <Step title="将原生审批处理器迁移到能力事实">
    ...
    - 不要从原生审批处理器发送由插件拥有的改道通知；核心现在根据实际投递结果统一负责“已路由到其他位置”的通知
    - 在将 `channelRuntime` 传入 `createChannelManager(...)` 时，必须提供真实的 `createPluginRuntime().channel` 接口。部分 stub 会被拒绝。
  ...
  <Step title="审查 Windows 包装器回退行为">
    ...
    那么无法解析的 Windows `.cmd`/`.bat` 包装器现在会默认失败关闭，除非你显式传入
  ...
  <Step title="替换宽泛的 infra-runtime 导入">
    `openclaw/plugin-sdk/infra-runtime` 仍然保留以兼容外部插件，但新代码应导入其实际需要的聚焦辅助接口：
    ...
    内置插件受扫描器保护，不能使用 `infra-runtime`，因此仓库代码无法回退到这个宽泛 barrel。

  </Step>

  <Step title="迁移渠道路由辅助工具">
    新的渠道路由代码应使用 `openclaw/plugin-sdk/channel-route`。
    在迁移窗口期间，旧的 route-key 和 comparable-target 名称仍保留为兼容性别名，但新插件应使用能够更直接描述行为的路由命名：
...
## 导入路径参考
  ...
  | `plugin-sdk/channel-reply-pipeline` | 回复前缀 + 正在输入 wiring | `createChannelReplyPipeline` |
  ...
  | `plugin-sdk/approval-handler-runtime` | 审批处理器辅助工具 | 更宽泛的审批处理器运行时辅助工具；若较窄的 adapter/gateway 接口已足够，应优先使用它们 |
  ...
  | `plugin-sdk/security-runtime` | 安全辅助工具 | 共享 trust、私信门控、外部内容和 Secret 收集辅助工具 |
  ...
  | `plugin-sdk/reply-dispatch-runtime` | 窄化的回复分发辅助工具 | 完成、提供商分发和会话标签辅助工具 |
  ...
  | `plugin-sdk/provider-web-search-config-contract` | provider web-search 配置辅助工具 | 面向不需要插件启用 wiring 的 provider 的窄化 web-search 配置/凭证辅助工具 |
  | `plugin-sdk/provider-web-search-contract` | provider web-search 契约辅助工具 | 窄化 web-search 配置/凭证契约辅助工具，例如 `createWebSearchProviderContractFields`、`enablePluginInConfig`、`resolveProviderWebSearchPluginConfig`，以及作用域化凭证 setter/getter |
  ...
  | `plugin-sdk/media-generation-runtime` | 共享媒体生成辅助工具 | 共享故障转移辅助工具、候选项选择，以及用于图像/视频/音乐生成的缺失模型提示 |
  ...
  | `plugin-sdk/channel-config-primitives` | 渠道配置原语 | 窄化的渠道 config-schema 原语 |
  ...
  | `plugin-sdk/memory-core-host-engine-embeddings` | Memory 宿主嵌入引擎 | Memory 嵌入契约、注册表访问、本地 provider 以及通用批处理/远程辅助工具；具体远程 provider 位于各自所属插件中 |
  ...
  | `plugin-sdk/memory-host-markdown` | 托管 Markdown 辅助工具 | 用于与 Memory 相邻插件的共享托管 Markdown 辅助工具 |
  | `plugin-sdk/memory-host-search` | 活动 Memory 搜索门面 | 惰性活动记忆搜索管理器运行时门面 |
...
`plugin-sdk/github-copilot-token` 当前暴露的是窄化的令牌辅助接口：
...
这些是跨插件 SDK、provider 契约、运行时接口和 manifest 生效的更窄粒度弃用项。它们目前仍然可用，但将在未来的主要版本中移除。每一项下方的条目都给出了从旧 API 到规范替代方案的映射。
...
<AccordionGroup>
  ...
  <Accordion title="提及门控辅助工具 → resolveInboundMentionDecision">
    ...
    **新版**：`resolveInboundMentionDecision({ facts, policy })`——返回单个决策对象，而不是拆分成两个调用。
  ...
  <Accordion title="渠道运行时 shim 和渠道操作辅助工具">
    ...
    会和原始的“actions”渠道导出一起被弃用。请改为通过语义化的
    `presentation` 接口暴露能力——渠道插件应声明自己能渲染什么
    （卡片、按钮、选择器），而不是声明自己接受哪些原始操作名称。
  ...
  <Accordion title="纯文本渠道 envelope → BodyForAgent">
    ...
    **新版**：`BodyForAgent` 加上结构化用户上下文块。
    渠道插件会将路由元数据（线程、主题、reply-to、reactions）作为带类型字段附加，
    而不是把它们拼接进一个提示字符串中。
    `formatAgentEnvelope(...)` 辅助工具仍然支持用于合成的、面向助手的 envelope，
    但入站纯文本 envelope 正在逐步淘汰。
  ...
  <Accordion title="Provider discovery 类型 → provider catalog 类型">
    ...
    此外还有旧版的 `ProviderCapabilities` 静态对象——provider 插件
    应通过 provider 运行时契约附加 capability 事实，而不是使用静态对象。
  ...
  <Accordion title="Thinking 策略钩子 → resolveThinkingProfile">
    ...
    **新版**：单一的 `resolveThinkingProfile(ctx)`，它返回一个
    ...
    以及按等级排序的级别列表。OpenClaw 会根据 profile 排名自动降级过时的已存储值。

    现在只需实现一个钩子，而不是三个。旧钩子在弃用窗口期间仍然可用，
    但不会与 profile 结果进行组合。
  ...
  <Accordion title="外部 OAuth provider 回退 → contracts.externalAuthProviders">
    **旧版**：实现 `resolveExternalOAuthProfiles(...)`，
    但不在插件 manifest 中声明该 provider。

    **新版**：在插件 manifest 中声明 `contracts.externalAuthProviders`
    **并且**实现 `resolveExternalAuthProfiles(...)`。旧的“认证回退”
    路径会在运行时发出警告，并将在未来被移除。
  ...
  <Accordion title="Provider 环境变量查找 → setup.providers[].envVars">
    ...
    **新版**：将相同的环境变量查找信息镜像到 manifest 中的
    `setup.providers[].envVars`。这样可以把设置/Status 环境变量元数据集中到同一个位置，
  ...
  <Accordion title="Memory 插件注册 → registerMemoryCapability">
    ...
    **新版**：在 memory-state API 上使用一次调用——
    ...
    槽位相同，只是改为单次注册调用。增量 Memory 辅助工具
  ...
  <Accordion title="runtime.tasks.flow → runtime.tasks.flows">
    ...
    **新版**：`runtime.tasks.flows`（复数）返回基于 DTO 的 TaskFlow 访问方式，
    它是导入安全的，不需要加载完整的任务运行时。
  ...
  <Accordion title="嵌入式扩展工厂 → 智能体工具结果中间件">
    上文“如何迁移 → 将 Pi 工具结果扩展迁移到中间件”中已覆盖。
    这里为了完整性再次列出：已移除的仅限 Pi 的
...
barrel 中进行跟踪。它们不影响第三方插件契约，因此未在此列出。
...
| ---------------------- | ----------------------------------------------------------------------- |
| **现在** | 已弃用接口会发出运行时警告 |
| **下一个主要版本** | 已弃用接口将被移除；仍在使用它们的插件会失效 |
