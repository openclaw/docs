---
read_when:
    - 你想安装与 Codex、Claude 或 Cursor 兼容的捆绑包
    - 你需要了解 OpenClaw 如何将 bundle 内容映射到原生功能
    - 你正在调试包检测或缺失能力
summary: 将 Codex、Claude 和 Cursor 捆绑包作为 OpenClaw 插件安装并使用
title: 插件包
x-i18n:
    generated_at: "2026-06-27T02:37:04Z"
    model: gpt-5.5
...
和 **Cursor**。这些称为 **bundle**，即内容和元数据包，
OpenClaw 会将其映射到 Skills、钩子和 MCP 工具等原生功能。

<Info>
  bundle **不同于** OpenClaw 原生插件。原生插件在进程内运行，
...
不会要求作者将它们重写为 OpenClaw 原生插件，而是检测这些格式，
并将其受支持的内容映射到原生功能集。这意味着你可以安装 Claude 命令包或 Codex 技能 bundle
并立即使用。

## 安装 bundle

<Steps>
  <Step title="从目录、归档或 marketplace 安装">
    ```bash
    # Local directory
...
## OpenClaw 从 bundle 映射哪些内容

并非所有 bundle 功能今天都能在 OpenClaw 中运行。下面列出了已可用的内容，
以及已检测但尚未接线的内容。
...
| 功能       | 映射方式                                                                                       | 适用对象     |
| ------------- | ------------------------------------------------------------------------------------------------- | -------------- |
| Skill 内容 | bundle skill 根会作为普通 OpenClaw skills 加载                                                 | 所有格式    |
| 命令      | `commands/` 和 `.cursor/commands/` 会被视为 skill 根                                        | Claude、Cursor |
| 钩子包    | OpenClaw 风格的 `HOOK.md` + `handler.ts` 布局                                                   | Codex          |
| MCP 工具     | bundle MCP 配置会合并到嵌入式 OpenClaw 设置中；加载受支持的 stdio 和 HTTP 服务器 | 所有格式    |
...
- bundle skill 根会作为普通 OpenClaw skill 根加载
- Claude `commands` 根会被视为额外的 skill 根
- Cursor `.cursor/commands` 根会被视为额外的 skill 根
...
- bundle 钩子根**只有**在使用普通 OpenClaw 钩子包
  布局时才工作。今天这主要是兼容 Codex 的情况：
  - `HOOK.md`
...
- OpenClaw 会将 bundle MCP 配置合并到有效的嵌入式 OpenClaw 设置中，作为
  `mcpServers`
- OpenClaw 会在嵌入式 OpenClaw 智能体轮次期间公开受支持的 bundle MCP 工具，方式是
  启动 stdio 服务器或连接到 HTTP 服务器
- `coding` 和 `messaging` 工具配置文件默认包含 bundle MCP 工具；
  使用 `tools.deny: ["bundle-mcp"]` 可为某个智能体或 Gateway 网关选择退出
- 项目本地的嵌入式智能体设置仍会在 bundle 默认值之后应用，因此工作区
  设置可在需要时覆盖 bundle MCP 条目
- bundle MCP 工具目录会在注册前以确定性方式排序，因此
...
OpenClaw 会以 `serverName__toolName` 形式注册 bundle MCP 工具，名称对提供商安全。
例如，键为 `"vigil-harbor"` 的服务器公开
`memory_search` 工具时，会注册为 `vigil-harbor__memory_search`。

- `A-Za-z0-9_-` 之外的字符会替换为 `-`
- 以非字母开头的片段会加上字母前缀，因此 `12306` 等数字
  服务器键会变成对提供商安全的工具前缀
- 服务器前缀上限为 30 个字符
- 完整工具名称上限为 64 个字符
- 空服务器名称回退为 `mcp`
- 发生冲突的净化后名称会用数字后缀消歧
- 最终公开的工具顺序按安全名称确定，以保持重复嵌入式智能体
  轮次的缓存稳定
- 配置文件过滤会将来自同一个 bundle MCP 服务器的所有工具视为归属
  `bundle-mcp` 的插件，因此配置文件允许列表和拒绝列表可以包含
  单个公开工具名称或 `bundle-mcp` 插件键

#### 嵌入式 OpenClaw 设置

- 启用 bundle 时，Claude `settings.json` 会作为默认嵌入式 OpenClaw 设置导入
- OpenClaw 会在应用 shell 覆盖键之前对其进行净化
...
- 目前只有受支持的 stdio 支持的 LSP 服务器可以运行；不受支持的
...
<AccordionGroup>
  <Accordion title="Codex bundle">
    ...
    Codex bundle 在使用 skill 根和 OpenClaw 风格
    钩子包目录（`HOOK.md` + `handler.ts`）时最适合 OpenClaw。
...
- 第三方兼容 bundle 不会获得启动时 `npm install` 修复。它们
  应通过 `openclaw plugins install` 安装，并在已安装插件目录中附带所需的所有内容。
- OpenClaw 拥有的内置插件要么以轻量形式随核心一起发布，要么
  可通过插件安装器下载。Gateway 网关启动时永远不会为它们运行
  包管理器。
- `openclaw doctor --fix` 会移除旧版暂存依赖目录，并且可以
  恢复本地插件索引中缺失但配置引用的可下载插件。
...
- Skills 和钩子包路径必须留在插件根内（经过边界检查）
- 设置文件使用相同的边界检查读取
- 受支持的 stdio MCP 服务器可以作为子进程启动

这让 bundle 默认更安全，但你仍应将第三方
bundle 视为其公开功能的受信任内容。
...
<AccordionGroup>
  ...
  <Accordion title="Claude 命令文件未出现">
    确保 bundle 已启用，并且 markdown 文件位于已检测的
    `commands/` 或 `skills/` 根中。
  </Accordion>

  <Accordion title="Claude 设置未应用">
    仅支持来自 `settings.json` 的嵌入式 OpenClaw 设置。OpenClaw
    不会将 bundle 设置视为原始配置补丁。
  </Accordion>
