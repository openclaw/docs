...
title: 插件 SDK 子路径
x-i18n:
    generated_at: "2026-07-12T14:41:56Z"
    model: gpt-5.6
...
插件 SDK 通过 `openclaw/plugin-sdk/` 下的一组精简公共子路径公开。本页按用途分组列出常用子路径。以下三个文件定义了该接口面：

- `scripts/lib/plugin-sdk-entrypoints.json`：由构建系统编译的、持续维护的入口点清单。
- `scripts/lib/plugin-sdk-private-local-only-subpaths.json`：仅限仓库本地使用的测试/内部子路径。软件包导出内容为清单减去此列表。
- `src/plugin-sdk/entrypoints.ts`：用于已弃用子路径、保留的内置辅助程序、受支持的内置 facade 和插件自有公共接口面的分类元数据。

维护者使用 `pnpm plugin-sdk:surface` 审核公共导出数量，并使用 `pnpm plugins:boundary-report:summary` 审核活跃的保留辅助程序子路径；未使用的保留辅助程序导出会导致 CI 报告失败，而不会作为休眠的兼容性债务继续留在公共 SDK 中。
...
| `plugin-sdk/migration`         | 迁移提供商条目辅助程序，例如 `createMigrationItem`、原因常量、条目状态标记、脱敏辅助程序和 `summarizeMigrationItems`                                                                                     |
| `plugin-sdk/migration-runtime` | 运行时迁移辅助程序，例如 `copyMigrationFileItem`、`resolvePlannedMigrationTargets`、`withCachedMigrationConfigRuntime` 和 `writeMigrationReport`                                                        |
| `plugin-sdk/health`            | 面向内置健康检查使用方的 Doctor 健康检查注册、检测、修复、选择、严重性和发现类型                                                                                                                        |
| `plugin-sdk/config-schema`     | 已弃用。根 `openclaw.json` Zod schema（`OpenClawSchema`）；应改为定义插件本地 schema，并使用 `plugin-sdk/json-schema-runtime` 验证                                                                        |

### 已弃用的兼容性和测试辅助程序

为支持旧版插件，已弃用的子路径仍会导出，但新代码应使用下方的专用 SDK 子路径。维护的列表位于 `scripts/lib/plugin-sdk-deprecated-public-subpaths.json`；CI 会拒绝内置生产代码从中导入。`plugin-sdk/compat`、`plugin-sdk/config-types`、`plugin-sdk/infra-runtime` 和 `plugin-sdk/text-runtime` 等宽泛 barrel 仅用于兼容，而 `plugin-sdk/zod` 是兼容性再导出：请直接从 `zod` 导入 `zod`。宽泛的领域 barrel `plugin-sdk/agent-runtime`、`plugin-sdk/channel-lifecycle`、`plugin-sdk/channel-runtime`、`plugin-sdk/cli-runtime`、`plugin-sdk/conversation-runtime`、`plugin-sdk/hook-runtime`、`plugin-sdk/media-runtime`、`plugin-sdk/plugin-runtime` 和 `plugin-sdk/security-runtime` 同样已弃用，推荐改用专用子路径。

OpenClaw 基于 Vitest 的测试辅助程序子路径仅限仓库本地使用，不再作为软件包导出：`agent-runtime-test-contracts`、`channel-contract-testing`、`channel-target-testing`、`channel-test-helpers`、`plugin-state-test-runtime`、`plugin-test-api`、`plugin-test-contracts`、`plugin-test-runtime`、`provider-http-test-mocks`、`provider-test-contracts`、`reply-payload-testing`、`sqlite-runtime-testing`、`test-env`、`test-fixtures`、`test-node-mocks` 和 `testing`。私有内置辅助程序接口面 `ssrf-runtime-internal` 和 `codex-native-task-runtime` 也仅限仓库本地使用。

### 保留的内置插件辅助程序子路径

`plugin-sdk/codex-mcp-projection` 是唯一的保留子路径：它是内置 Codex 插件自有的兼容性接口面，而不是通用 SDK API。软件包契约防护机制会阻止跨所有者插件导入，并且当保留子路径不再被导入时，CI 会失败。
`plugin-sdk/codex-native-task-runtime` 仅限仓库本地使用，不是软件包导出。

`src/plugin-sdk/entrypoints.ts` 还会跟踪受支持的内置 facade，即在通用契约取代它们之前，由相应内置插件支持的 SDK 入口点：`plugin-sdk/discord`、`plugin-sdk/lmstudio`、`plugin-sdk/lmstudio-runtime`、`plugin-sdk/matrix`、`plugin-sdk/mattermost`、`plugin-sdk/memory-core-engine-runtime`、`plugin-sdk/provider-zai-endpoint`、`plugin-sdk/qa-runner-runtime`、`plugin-sdk/telegram-account`、`plugin-sdk/tts-runtime` 和 `plugin-sdk/zalouser`。其中一些也已不推荐用于新代码；请参阅下方各行的说明。
  ...
  <Accordion title="渠道子路径">
    ...
    | `plugin-sdk/account-core` | 多账户配置/操作门控辅助函数和默认账户回退辅助函数 |
    | `plugin-sdk/account-id` | `DEFAULT_ACCOUNT_ID`、账户 ID 规范化辅助函数 |
    | `plugin-sdk/account-resolution` | 账户查找及默认回退辅助函数 |
    ...
    | `plugin-sdk/chat-channel-ids` | `BUNDLED_CHAT_CHANNEL_IDS`、`BUNDLED_CHAT_CHANNEL_ENVELOPE_PREFIXES`、`ChatChannelId`。规范的内置/官方聊天渠道 ID，以及供需要识别带信封前缀文本而无需硬编码自有表的插件使用的格式化器标签/别名。 |
    ...
    | `plugin-sdk/channel-ingress-runtime` | 面向已迁移渠道接收路径的实验性高层级渠道入口运行时解析器和路由事实构建器。应优先使用此路径，而不是在每个插件中分别组装有效允许列表、命令允许列表和旧版投影。请参阅[频道入口 API](/zh-CN/plugins/sdk-channel-ingress)。 |
    | `plugin-sdk/channel-lifecycle` | 已弃用的兼容性门面。请使用 `plugin-sdk/channel-outbound`。 |
    | `plugin-sdk/channel-outbound` | 消息生命周期契约，以及回复管道选项、回执、实时预览/流式传输、生命周期辅助函数、出站身份、载荷规划、持久发送和消息发送上下文辅助函数。请参阅[渠道出站 API](/zh-CN/plugins/sdk-channel-outbound)。 |
    | `plugin-sdk/channel-message` | `plugin-sdk/channel-outbound` 的已弃用兼容性别名，另含旧版回复分发门面。 |
    | `plugin-sdk/channel-message-runtime` | `plugin-sdk/channel-outbound` 的已弃用兼容性别名，另含旧版回复分发门面。 |
    | `plugin-sdk/inbound-envelope` | 共享入口路由和信封构建辅助函数 |
    | `plugin-sdk/inbound-reply-dispatch` | 已弃用的兼容性门面。入口运行器和分发谓词请使用 `plugin-sdk/channel-inbound`，消息投递辅助函数请使用 `plugin-sdk/channel-outbound`。 |
    ...
    | `plugin-sdk/conversation-runtime` | 已弃用的宽泛聚合入口，涵盖对话/线程绑定、配对和已配置绑定辅助函数；请优先使用专用绑定子路径，例如 `plugin-sdk/thread-bindings-runtime` 和 `plugin-sdk/session-binding-runtime` |
    ...
    | `plugin-sdk/direct-dm-guard-policy` | 精简的私信加密前防护策略辅助函数 |
    | `plugin-sdk/discord` | 为已发布的 `@openclaw/discord@2026.3.13` 和受跟踪的所有者兼容性保留的已弃用 Discord 兼容性门面；新插件应使用通用渠道 SDK 子路径 |
    | `plugin-sdk/telegram-account` | 为受跟踪的所有者兼容性保留的已弃用 Telegram 账户解析兼容性门面；新插件应使用注入的运行时辅助函数或通用渠道 SDK 子路径 |
    | `plugin-sdk/zalouser` | 为仍导入发送者命令授权的已发布 Lark/Zalo 软件包保留的已弃用 Zalo Personal 兼容性门面；新插件应使用通用渠道 SDK 子路径 |
    | `plugin-sdk/interactive-runtime` | 语义化消息呈现、投递和旧版交互式回复辅助函数。请参阅[消息呈现](/zh-CN/plugins/message-presentation) |
    | `plugin-sdk/channel-inbound` | 用于事件分类、上下文构建、格式化、根目录、去抖、提及匹配、提及策略和入口日志记录的共享入口辅助函数 |
    | `plugin-sdk/channel-inbound-debounce` | 精简的入口去抖辅助函数 |
    | `plugin-sdk/channel-mention-gating` | 精简的提及策略、提及标记和提及文本辅助函数，不包含更宽泛的入口运行时表面 |
    ...
    | `plugin-sdk/channel-route` | 共享路由规范化、由解析器驱动的目标解析、线程 ID 字符串化、去重/紧凑路由键、已解析目标类型，以及路由/目标比较辅助函数 |
    | `plugin-sdk/channel-targets` | 目标解析辅助函数；路由比较调用方应使用 `plugin-sdk/channel-route` |
    | `plugin-sdk/channel-contract` | 渠道契约类型 |
    | `plugin-sdk/channel-feedback` | 反馈/表情回应连接 |
  </Accordion>

已弃用的渠道辅助函数族仅为已发布插件的兼容性而保留。移除计划如下：在外部插件迁移窗口期间保留它们，让仓库内/内置插件继续使用 `channel-inbound` 和 `channel-outbound`，然后在下一次 SDK 重大清理中移除兼容性子路径。这适用于旧版渠道消息/运行时、渠道流式传输、直接私信访问、入口辅助函数分支、回复选项和配对路径函数族。
  ...
  <Accordion title="提供商子路径">
    ...
    | `plugin-sdk/provider-auth-runtime` | 提供商身份验证运行时辅助函数：OAuth 回环流程、令牌交换、身份验证持久化和 API 密钥解析 |
    ...
    | `plugin-sdk/embedding-providers` | 通用嵌入提供商类型和读取辅助函数，包括 `EmbeddingProviderAdapter`、`getEmbeddingProvider(...)` 和 `listEmbeddingProviders(...)`；插件通过 `api.registerEmbeddingProvider(...)` 注册提供商，从而强制执行清单所有权 |
    ...
    | `plugin-sdk/provider-stream` | `ProviderStreamFamily`、`buildProviderStreamFamilyHooks`、`composeProviderStreamWrappers`、流包装器类型、纯文本工具调用兼容，以及共享的 Anthropic/Google/Kilocode/MiniMax/Moonshot/OpenAI/OpenRouter/Z.AI 包装器辅助函数 |
    | `plugin-sdk/provider-stream-shared` | 公共共享提供商流包装器辅助函数，包括 `composeProviderStreamWrappers`、`createOpenAICompatibleCompletionsThinkingOffWrapper`、`createPlainTextToolCallCompatWrapper`、`createPayloadPatchStreamWrapper`、`createToolStreamWrapper`、`normalizeOpenAICompatibleReasoningPayload`、`setQwenChatTemplateThinking`，以及 Anthropic/DeepSeek/OpenAI 兼容流实用工具 |
    | `plugin-sdk/provider-transport-runtime` | 原生提供商传输辅助函数，例如受保护的 fetch、工具结果文本提取、传输消息转换和可写传输事件流 |
...
提供商用量快照通常会报告一个或多个配额 `windows`，每个窗口都包含
标签、已用百分比和可选的重置时间。对于提供余额或
账户状态文本而非可重置配额窗口的提供商，应返回
`summary` 和空的 `windows` 数组，而不是虚构百分比。
OpenClaw 会在状态输出中显示该摘要文本；仅当
用量端点失败或未返回任何可用的用量数据时，才使用 `error`。

  <Accordion title="身份验证和安全子路径">
    | 子路径 | 主要导出 |
    | --- | --- |
    | `plugin-sdk/command-auth` | 已弃用的宽泛命令授权表面（`resolveControlCommandGate`、包括动态参数菜单格式化在内的命令注册表辅助函数、发送者授权辅助函数）；请使用渠道入口/运行时授权或命令状态辅助函数 |
    | `plugin-sdk/command-status` | 命令/帮助消息构建器，例如 `buildCommandsMessagePaginated` 和 `buildHelpMessage` |
    | `plugin-sdk/approval-auth-runtime` | 审批者解析和同一聊天内的操作身份验证辅助函数 |
    ...
    | `plugin-sdk/approval-reference-runtime` | 面向传输受限审批回调的确定性持久定位器辅助函数 |
    | `plugin-sdk/approval-handler-adapter-runtime` | 面向高频渠道入口点的轻量级原生审批适配器加载辅助函数 |
    | `plugin-sdk/approval-handler-runtime` | 更宽泛的审批处理程序运行时辅助函数；当更精简的适配器/Gateway 网关接缝足够时，优先使用它们 |
    | `plugin-sdk/approval-native-runtime` | 原生审批目标、账户绑定、路由门控、转发回退和本地原生 Exec 提示抑制辅助函数 |
    | `plugin-sdk/approval-reaction-runtime` | 硬编码的审批表情回应绑定、表情回应提示负载、表情回应目标存储、表情回应提示文本辅助函数，以及本地原生 Exec 提示抑制的兼容性导出 |
    ...
    | `plugin-sdk/provider-auth-login-flow-runtime` | 面向私有渠道和 Web UI 设备代码配对的延迟加载提供商身份验证登录流程辅助函数 |
    ...
    | `plugin-sdk/channel-secret-basic-runtime` | 面向非 TTS 渠道/插件密钥表面的精简密钥契约导出 |
    | `plugin-sdk/channel-secret-tts-runtime` | 精简的嵌套渠道 TTS 密钥赋值辅助函数 |
    | `plugin-sdk/secret-ref-runtime` | 用于密钥契约/配置解析的精简 SecretRef 类型定义、解析和计划目标路径查找 |
    | `plugin-sdk/secret-provider-integration` | 仅类型的 SecretRef 提供商集成清单和预设契约，适用于发布外部密钥提供商预设的插件 |
    | `plugin-sdk/security-runtime` | 已弃用的宽泛 barrel，涵盖信任、私信门控、限定根目录的文件/路径辅助函数（包括仅创建写入）、同步/异步原子文件替换、同级临时写入、跨设备移动回退、私有文件存储辅助函数、符号链接父目录防护、外部内容、敏感文本遮盖、常量时间密钥比较和密钥收集辅助函数；请优先使用聚焦的安全/SSRF/密钥子路径 |
    | `plugin-sdk/ssrf-policy` | 主机允许列表和私有网络 SSRF 策略辅助函数 |
    | `plugin-sdk/ssrf-dispatcher` | 不含宽泛基础设施运行时表面的精简固定分派器辅助函数 |
    | `plugin-sdk/ssrf-runtime` | 固定分派器、受 SSRF 防护的 fetch、SSRF 错误和 SSRF 策略辅助函数 |
  ...
  <Accordion title="Runtime and storage subpaths">
    | 子路径 | 主要导出 |
    | --- | --- |
    | `plugin-sdk/runtime` | 运行时/日志/备份辅助函数、插件安装路径警告和进程辅助函数 |
    | `plugin-sdk/runtime-env` | 精简的运行时环境、日志记录器、超时、重试和退避辅助函数 |
    | `plugin-sdk/browser-config` | 受支持的浏览器配置门面，用于规范化配置文件/默认值、解析 CDP URL 和提供浏览器控制身份验证辅助函数 |
    | `plugin-sdk/agent-harness-task-runtime` | 面向使用宿主签发任务作用域的 harness 支持型智能体的通用任务生命周期和完成结果投递辅助函数 |
    | `plugin-sdk/codex-mcp-projection` | 预留的内置 Codex 辅助函数，用于将用户 MCP 服务器配置投射到 Codex 线程配置中；不适用于第三方插件 |
    | `plugin-sdk/codex-native-task-runtime` | 仓库本地的内置 Codex 辅助函数，用于原生任务镜像/运行时接线；不是软件包导出 |
    | `plugin-sdk/channel-runtime-context` | 通用渠道运行时上下文注册和查找辅助函数 |
    | `plugin-sdk/matrix` | 面向旧版第三方渠道软件包的已弃用 Matrix 兼容性门面；新插件应直接导入 `plugin-sdk/run-command` |
    | `plugin-sdk/mattermost` | 面向旧版第三方渠道软件包的已弃用 Mattermost 兼容性门面；新插件应直接导入通用 SDK 子路径 |
    | `plugin-sdk/runtime-store` | `createPluginRuntimeStore` |
    | `plugin-sdk/plugin-runtime` | 面向插件命令/钩子/HTTP/交互辅助函数的已弃用宽泛桶式导出；优先使用聚焦的插件运行时子路径 |
    | `plugin-sdk/hook-runtime` | 面向 webhook/内部钩子管道辅助函数的已弃用宽泛桶式导出；优先使用聚焦的钩子/插件运行时子路径 |
    ...
    | `plugin-sdk/cli-runtime` | 面向 CLI 格式化、等待、版本、参数调用和延迟命令组辅助函数的已弃用宽泛桶式导出；优先使用聚焦的 CLI/运行时子路径 |
    | `plugin-sdk/qa-live-transport-scenarios` | 共享的实时传输 QA 场景 ID、基线覆盖辅助函数和场景选择辅助函数 |
    | `plugin-sdk/qa-runner-runtime` | 受支持的门面，通过 CLI 命令界面公开插件 QA 场景 |
    | `plugin-sdk/tts-runtime` | 受支持的文本转语音配置架构和运行时辅助函数门面 |
    | `plugin-sdk/gateway-method-runtime` | 为声明 `contracts.gatewayMethodDispatch: ["authenticated-request"]` 的插件 HTTP 路由预留的 Gateway 网关方法分派辅助函数 |
    | `plugin-sdk/gateway-runtime` | Gateway 网关客户端、事件循环就绪的客户端启动辅助函数、Gateway 网关 CLI RPC、Gateway 网关协议错误、公布的 LAN 主机解析和渠道状态补丁辅助函数 |
    | `plugin-sdk/config-contracts` | 面向插件配置结构（例如 `OpenClawConfig`）和渠道/提供商配置类型的聚焦纯类型配置界面 |
    ...
    | `plugin-sdk/text-autolink-runtime` | 不依赖宽泛文本桶式导出的文件引用自动链接检测 |
    | `plugin-sdk/reply-runtime` | 共享的入站/回复运行时辅助函数、分块、分派、Heartbeat 和回复规划器 |
    | `plugin-sdk/reply-dispatch-runtime` | 精简的回复分派/完成和对话标签辅助函数 |
    | `plugin-sdk/reply-history` | 共享的短时间窗口回复历史辅助函数。新的消息轮次代码应使用 `createChannelHistoryWindow`；底层映射辅助函数仅保留为已弃用的兼容性导出 |
    | `plugin-sdk/reply-reference` | `createReplyReferencePlanner` |
    | `plugin-sdk/reply-chunking` | 精简的文本/Markdown 分块辅助函数 |
    | `plugin-sdk/session-store-runtime` | 会话工作流辅助函数（`getSessionEntry`、`listSessionEntries`、`patchSessionEntry`、`upsertSessionEntry`）、修复/生命周期辅助函数（`deleteSessionEntry`、`cleanupSessionLifecycleArtifacts`、`resolveSessionStoreBackupPaths`）、过渡期 `sessionFile` 值的标记辅助函数、按会话身份有界读取最近的用户/助手转录文本、会话存储路径/会话键辅助函数以及更新时间读取，且不导入宽泛的配置写入/维护功能 |
    | `plugin-sdk/session-transcript-runtime` | 转录身份、限定作用域的目标/读取/写入辅助函数、可见消息条目投射、更新发布、写入锁和转录记忆命中键 |
    | `plugin-sdk/sqlite-runtime` | 面向第一方运行时的聚焦 SQLite 智能体架构、路径和事务辅助函数，不含数据库生命周期控制 |
    | `plugin-sdk/cron-store-runtime` | Cron 存储路径/加载/保存辅助函数 |
    | `plugin-sdk/state-paths` | 状态/OAuth 目录路径辅助函数 |
    | `plugin-sdk/plugin-state-runtime` | 插件边车 SQLite 键控状态类型，以及面向插件自有数据库的集中式连接 pragma 和 WAL 维护设置 |
    ...
    | `plugin-sdk/sandbox` | 沙箱后端类型和 SSH/OpenShell 命令辅助函数，包括快速失败的执行命令预检 |
    ...
    | `plugin-sdk/acp-runtime-backend` | 面向启动时加载插件的轻量级 ACP 后端注册和回复分派辅助函数 |
    | `plugin-sdk/acp-binding-resolve-runtime` | 不导入生命周期启动功能的只读 ACP 绑定解析 |
    ...
    | `plugin-sdk/agent-harness` | 面向底层智能体 harness 的实验性可信插件界面：harness 类型、活动运行的 steer/abort 辅助函数、OpenClaw 工具桥接辅助函数、运行时计划工具策略辅助函数、终端结果分类、工具进度格式化/详情辅助函数和尝试结果实用工具 |
    ...
    | `plugin-sdk/delivery-queue-runtime` | 出站待投递项排空辅助函数 |
    ...
    | `plugin-sdk/exec-approvals-runtime` | 不依赖宽泛基础设施运行时桶式导出的 Exec 审批策略文件辅助函数 |
    | `plugin-sdk/infra-runtime` | 已弃用的兼容性垫片；请使用上述聚焦的运行时子路径 |
    ...
    | `plugin-sdk/runtime-fetch` | 支持调度器的运行时 fetch，不导入代理/受保护 fetch 功能 |
    | `plugin-sdk/inline-image-data-url-runtime` | 不依赖宽泛媒体运行时界面的内联图像数据 URL 清理和签名探测辅助函数 |
    | `plugin-sdk/response-limit-runtime` | 不依赖宽泛媒体运行时界面的有界响应正文读取器 |
    | `plugin-sdk/session-binding-runtime` | 当前对话绑定状态，不包含已配置的绑定路由或配对存储 |
    | `plugin-sdk/context-visibility-runtime` | 上下文可见性解析和补充上下文过滤，不导入宽泛配置/安全功能 |
    | `plugin-sdk/string-coerce-runtime` | 精简的基础记录/字符串强制转换和规范化辅助函数，不导入 Markdown/日志功能 |
    | `plugin-sdk/host-runtime` | 主机名和 SCP 主机规范化辅助函数 |
    | `plugin-sdk/retry-runtime` | 重试配置和重试运行器辅助函数 |
    | `plugin-sdk/agent-runtime` | 面向智能体目录/身份/工作区辅助函数的已弃用宽泛桶式导出，包括 `resolveAgentDir`、`resolveDefaultAgentDir` 和已弃用的 `resolveOpenClawAgentDir` 兼容性导出；优先使用聚焦的智能体/运行时子路径 |
  ...
  <Accordion title="能力和测试子路径">
    | 子路径 | 主要导出 |
    | --- | --- |
    | `plugin-sdk/media-runtime` | 已弃用的宽泛媒体桶形导出，包括 `saveRemoteMedia`、`saveResponseMedia`、`readRemoteMediaBuffer` 和已弃用的 `fetchRemoteMedia`；应优先使用 `plugin-sdk/media-store`、`plugin-sdk/media-mime`、`plugin-sdk/outbound-media` 和能力运行时子路径；当需要将 URL 转换为 OpenClaw 媒体时，应先使用存储辅助函数，再读取缓冲区 |
    ...
    | `plugin-sdk/media-generation-runtime` | 共享的媒体生成故障转移辅助函数、候选项选择和模型缺失提示消息 |
    | `plugin-sdk/media-understanding` | 媒体理解提供商类型，以及面向提供商的图像、音频和结构化提取辅助导出 |
    | `plugin-sdk/text-chunking` | 出站文本和 Markdown 分块/渲染辅助函数、Markdown 表格转换、指令标签移除和安全文本实用工具 |
    ...
    | `plugin-sdk/realtime-bootstrap-context` | 实时配置文件引导辅助函数，用于注入大小受限的 `IDENTITY.md`、`USER.md` 和 `SOUL.md` 上下文 |
    | `plugin-sdk/realtime-voice` | 实时语音提供商类型、注册表辅助函数和共享的实时语音行为辅助函数，包括输出活动跟踪 |
    | `plugin-sdk/image-generation` | 图像生成提供商类型，以及图像资源/数据 URL 辅助函数和 OpenAI 兼容图像提供商构建器 |
    | `plugin-sdk/image-generation-core` | 共享的图像生成类型、故障转移、身份验证和注册表辅助函数 |
    | `plugin-sdk/music-generation` | 音乐生成提供商/请求/结果类型 |
    | `plugin-sdk/music-generation-core` | 已弃用的共享音乐生成类型、故障转移辅助函数、提供商查找和模型引用解析；应优先使用插件自有的音乐提供商接口 |
    | `plugin-sdk/video-generation` | 视频生成提供商/请求/结果类型 |
    | `plugin-sdk/video-generation-core` | 共享的视频生成类型、故障转移辅助函数、提供商查找和模型引用解析 |
    | `plugin-sdk/transcripts` | 共享的转录源提供商类型、注册表辅助函数、会话描述符和话语元数据 |
    ...
    | `plugin-sdk/testing` | 供旧版 OpenClaw 测试使用的仓库本地已弃用兼容性桶形导出。新的仓库测试应改为导入聚焦的本地测试子路径，例如 `plugin-sdk/agent-runtime-test-contracts`、`plugin-sdk/plugin-test-runtime`、`plugin-sdk/channel-test-helpers`、`plugin-sdk/test-env` 或 `plugin-sdk/test-fixtures` |
    | `plugin-sdk/plugin-test-api` | 仓库本地的精简 `createTestPluginApi` 辅助函数，用于在不导入仓库测试辅助桥接层的情况下直接执行插件注册单元测试 |
    | `plugin-sdk/agent-runtime-test-contracts` | 仓库本地的原生 Agent Runtimes 适配器契约夹具，用于身份验证、交付、故障转移、工具钩子、提示词叠加层、模式和转录投影测试 |
    | `plugin-sdk/channel-test-helpers` | 面向渠道的仓库本地测试辅助函数，用于通用操作/设置/状态契约、目录断言、账户启动生命周期、发送配置传递、运行时模拟、状态问题、出站交付和钩子注册 |
    | `plugin-sdk/channel-target-testing` | 面向渠道测试的仓库本地共享目标解析错误用例套件 |
    | `plugin-sdk/channel-contract-testing` | 仓库本地的精简渠道契约测试辅助函数，不依赖宽泛的测试桶形导出 |
    | `plugin-sdk/plugin-test-contracts` | 仓库本地的插件包、注册、公开工件、直接导入、运行时 API 和导入副作用契约辅助函数 |
    | `plugin-sdk/plugin-state-test-runtime` | 仓库本地的插件状态存储、入口队列和状态数据库测试辅助函数 |
    | `plugin-sdk/provider-test-contracts` | 仓库本地的提供商运行时、身份验证、设备发现、新手引导、目录、向导、媒体能力、重放策略、实时 STT 实况音频、Web 搜索/获取和流契约辅助函数 |
    | `plugin-sdk/provider-http-test-mocks` | 可选择启用的仓库本地 Vitest HTTP/身份验证模拟，用于测试会调用 `plugin-sdk/provider-http` 的提供商 |
    | `plugin-sdk/reply-payload-testing` | 用于向回复载荷夹具附加元数据的仓库本地辅助函数 |
    | `plugin-sdk/sqlite-runtime-testing` | 供第一方测试使用的仓库本地 SQLite 生命周期辅助函数 |
    | `plugin-sdk/test-fixtures` | 仓库本地的通用 CLI 运行时捕获、沙箱上下文、技能写入器、智能体消息、系统事件、模块重新加载、内置插件路径、终端文本、分块、身份验证令牌和类型化用例夹具 |
  ...
  <Accordion title="记忆子路径">
    ...
    | `plugin-sdk/memory-core-engine-runtime` | 已弃用的记忆索引/搜索运行时门面；应优先使用供应商中立的记忆宿主子路径 |
    ...
    | `plugin-sdk/memory-core-host-multimodal` | 已弃用的记忆宿主多模态辅助函数；应优先使用供应商中立的记忆宿主子路径 |
    | `plugin-sdk/memory-core-host-query` | 已弃用的记忆宿主查询辅助函数；应优先使用供应商中立的记忆宿主子路径 |
  ...
  <Accordion title="保留的内置辅助函数子路径">
    保留的内置辅助函数 SDK 子路径是面向内置插件代码的精简、所有者专用接口。SDK 清单会跟踪这些子路径，以确保软件包构建和别名解析保持确定性，但它们并非通用的插件开发 API。新的可复用宿主契约应使用通用 SDK 子路径，例如 `plugin-sdk/gateway-runtime`、`plugin-sdk/ssrf-runtime` 和 `plugin-sdk/plugin-config-runtime`。

    | 子路径 | 所有者和用途 |
    | --- | --- |
    | `plugin-sdk/codex-mcp-projection` | 内置 Codex plugin 辅助函数，用于将用户的 MCP 服务器配置投影到 Codex app-server 线程配置中（保留的软件包导出） |
    | `plugin-sdk/codex-native-task-runtime` | 内置 Codex plugin 辅助函数，用于将 Codex app-server 原生子智能体镜像到 OpenClaw 任务状态（仅限仓库本地，不属于软件包导出） |
