---
read_when:
    - 为插件导入选择正确的插件 SDK 子路径
    - 审计内置插件子路径和辅助接口面
summary: 插件 SDK 子路径目录：哪些导入位于何处，按领域分组
title: 插件 SDK 子路径
x-i18n:
    generated_at: "2026-07-01T10:57:32Z"
    model: gpt-5.5
...
插件 SDK 作为一组位于 `openclaw/plugin-sdk/` 下的窄公共子路径暴露。本页按用途分组列出常用子路径。生成的编译器入口点清单位于 `scripts/lib/plugin-sdk-entrypoints.json`；包导出是在减去 `scripts/lib/plugin-sdk-private-local-only-subpaths.json` 中列出的仓库本地测试/内部子路径之后的公共子集。维护者可以使用 `pnpm plugin-sdk:surface` 审计公共导出数量，并使用 `pnpm plugins:boundary-report:summary` 审计活跃的保留 helper 子路径；未使用的保留 helper 导出会使 CI 报告失败，而不是作为休眠的兼容性债务留在公共 SDK 中。

插件编写指南请参阅 [插件 SDK 概览](/zh-CN/plugins/sdk-overview)。
...
| `plugin-sdk/migration`         | 迁移提供商条目 helper，例如 `createMigrationItem`、原因常量、条目状态标记、脱敏 helper 和 `summarizeMigrationItems`                                                    |
...
已弃用的子路径仍会为旧插件导出，但新代码应使用下面聚焦的 SDK 子路径。维护的列表是 `scripts/lib/plugin-sdk-deprecated-public-subpaths.json`；CI 会拒绝内置生产代码从中导入。`compat`、`config-types`、`infra-runtime`、`text-runtime` 和 `zod` 等宽泛 barrel 仅用于兼容。请直接从 `zod` 导入 `zod`。

OpenClaw 的 Vitest 支持的测试 helper 子路径仅限仓库本地使用，不再作为包导出：`agent-runtime-test-contracts`、`channel-contract-testing`、`channel-target-testing`、`channel-test-helpers`、`plugin-test-api`、`plugin-test-contracts`、`plugin-test-runtime`、`provider-http-test-mocks`、`provider-test-contracts`、`test-env`、`test-fixtures`、`test-node-mocks` 和 `testing`。

### 保留的内置插件 helper 子路径

这些子路径是插件所有的兼容性表面，供其所属的内置插件使用，而不是通用 SDK API：`plugin-sdk/codex-mcp-projection` 和 `plugin-sdk/codex-native-task-runtime`。跨所有者的扩展导入会被包契约护栏阻止。
...
<AccordionGroup>
  <Accordion title="频道子路径">
    ...
    | `plugin-sdk/setup` | 共享设置向导辅助函数、设置翻译器、allowlist 提示、设置状态构建器 |
    ...
    | `plugin-sdk/account-helpers` | 窄作用域账户列表/账户操作辅助函数 |
    | `plugin-sdk/access-groups` | 访问组 allowlist 解析和已脱敏组诊断辅助函数 |
    ...
    | `plugin-sdk/bundled-channel-config-schema` | 仅用于受维护内置插件的内置 OpenClaw 频道配置 schema |
    | `plugin-sdk/chat-channel-ids` | `BUNDLED_CHAT_CHANNEL_IDS`, `BUNDLED_CHAT_CHANNEL_ENVELOPE_PREFIXES`, `ChatChannelId`。规范内置/官方聊天频道 ID，以及供需要识别带信封前缀文本且无需硬编码自身表的插件使用的格式化标签/别名。 |
    ...
    | `plugin-sdk/command-gating` | 窄作用域命令授权门控辅助函数 |
    | `plugin-sdk/channel-policy` | `resolveChannelGroupRequireMention` |
    | `plugin-sdk/channel-ingress` | 已弃用的低层频道入口兼容性门面。新的接收路径应使用 `plugin-sdk/channel-ingress-runtime`。 |
    | `plugin-sdk/channel-ingress-runtime` | 实验性高层频道入口运行时解析器，以及用于已迁移频道接收路径的路由事实构建器。优先使用它，而不是在每个插件中组装有效 allowlist、命令 allowlist 和旧版投影。参见 [频道入口 API](/zh-CN/plugins/sdk-channel-ingress)。 |
    | `plugin-sdk/channel-lifecycle` | 已弃用的兼容性门面。使用 `plugin-sdk/channel-outbound`。 |
    | `plugin-sdk/channel-outbound` | 消息生命周期契约，以及回复流水线选项、回执、实时预览/流式传输、生命周期辅助函数、出站身份、载荷规划、持久发送和消息发送上下文辅助函数。参见 [Channel outbound API](/zh-CN/plugins/sdk-channel-outbound)。 |
    | `plugin-sdk/channel-message` | `plugin-sdk/channel-outbound` 的已弃用兼容性别名，外加旧版回复分发门面。 |
    | `plugin-sdk/channel-message-runtime` | `plugin-sdk/channel-outbound` 的已弃用兼容性别名，外加旧版回复分发门面。 |
    | `plugin-sdk/inbound-envelope` | 共享入站路由 + 信封构建器辅助函数 |
    | `plugin-sdk/inbound-reply-dispatch` | 已弃用的兼容性门面。入站运行器和分发谓词使用 `plugin-sdk/channel-inbound`，消息投递辅助函数使用 `plugin-sdk/channel-outbound`。 |
    ...
    | `plugin-sdk/poll-runtime` | 窄作用域投票规范化辅助函数 |
    | `plugin-sdk/thread-bindings-runtime` | 线程绑定生命周期和适配器辅助函数 |
    | `plugin-sdk/agent-media-payload` | 旧版 agent 媒体载荷构建器 |
    ...
    | `plugin-sdk/channel-status` | 共享频道状态快照/摘要辅助函数 |
    | `plugin-sdk/channel-config-primitives` | 窄作用域频道配置 schema 原语 |
    | `plugin-sdk/channel-config-writes` | 频道配置写入授权辅助函数 |
    | `plugin-sdk/channel-plugin-common` | 共享频道插件前置导出 |
    ...
    | `plugin-sdk/direct-dm-guard-policy` | 窄作用域私信预加密保护策略辅助函数 |
    | `plugin-sdk/discord` | 已弃用的 Discord 兼容性门面，用于已发布的 `@openclaw/discord@2026.3.13` 和跟踪中的所有者兼容性；新插件应使用通用频道 SDK 子路径 |
    | `plugin-sdk/telegram-account` | 已弃用的 Telegram 账户解析兼容性门面，用于跟踪中的所有者兼容性；新插件应使用注入的运行时辅助函数或通用频道 SDK 子路径 |
    | `plugin-sdk/zalouser` | 已弃用的 Zalo Personal 兼容性门面，用于仍导入发送者命令授权的已发布 Lark/Zalo 包；新插件应使用 `plugin-sdk/command-auth` |
    | `plugin-sdk/interactive-runtime` | 语义消息呈现、投递和旧版交互式回复辅助函数。参见 [Message Presentation](/zh-CN/plugins/message-presentation) |
    | `plugin-sdk/channel-inbound` | 用于事件分类、上下文构建、格式化、根、debounce、提及匹配、提及策略和入站日志的共享入站辅助函数 |
    | `plugin-sdk/channel-inbound-debounce` | 窄作用域入站 debounce 辅助函数 |
    | `plugin-sdk/channel-mention-gating` | 窄作用域提及策略、提及标记和提及文本辅助函数，不包含更广的入站运行时表面 |
    ...
    | `plugin-sdk/channel-actions` | 频道消息操作辅助函数，以及为插件兼容性保留的已弃用原生 schema 辅助函数 |
    ...
    | `plugin-sdk/channel-secret-runtime` | 窄作用域密钥契约辅助函数，例如 `collectSimpleChannelFieldAssignments`, `getChannelSurface`, `pushAssignment` 和密钥目标类型 |
  </Accordion>

已弃用的频道辅助函数家族仅为已发布插件的兼容性保持可用。移除计划是：在外部插件迁移窗口期间保留它们，让仓库/内置插件保持使用 `channel-inbound` 和 `channel-outbound`，然后在下一次主要 SDK 清理中移除这些兼容性子路径。这适用于旧的频道消息/运行时、频道流式传输、私信访问、入站辅助函数分支、回复选项以及配对路径家族。
  ...
  <Accordion title="提供商子路径">
    ...
    | `plugin-sdk/provider-auth` | `createProviderApiKeyAuthMethod`、`ensureApiKeyFromOptionEnvOrPrompt`、`upsertAuthProfile`、`upsertApiKeyProfile`、`writeOAuthCredentials`、OpenAI Codex 认证导入辅助函数、已弃用的 `resolveOpenClawAgentDir` 兼容导出 |
    ...
    | `plugin-sdk/provider-catalog-live-runtime` | 用于受保护的 `/models` 风格发现的实时提供商模型目录辅助函数：`buildLiveModelProviderConfig`、`fetchLiveProviderModelRows`、`getCachedLiveProviderModelRows`、`fetchLiveProviderModelIds`、`LiveModelCatalogHttpError`、`clearLiveCatalogCacheForTests`、模型 ID 过滤、TTL 缓存和静态回退 |
    ...
    | `plugin-sdk/provider-web-fetch-contract` | 狭窄的 Web fetch 配置/选择契约辅助函数，例如 `enablePluginInConfig` 和 `WebFetchProviderPlugin` |
    | `plugin-sdk/provider-web-fetch` | Web fetch 提供商注册/缓存辅助函数 |
    | `plugin-sdk/provider-web-search-config-contract` | 面向不需要插件启用布线的提供商的狭窄 Web 搜索配置/凭证辅助函数 |
    | `plugin-sdk/provider-web-search-contract` | 狭窄的 Web 搜索配置/凭证契约辅助函数，例如 `createWebSearchProviderContractFields`、`enablePluginInConfig`、`resolveProviderWebSearchPluginConfig`，以及限定作用域的凭证设置器/获取器 |
    | `plugin-sdk/provider-web-search` | Web 搜索提供商注册/缓存/运行时辅助函数 |
    | `plugin-sdk/embedding-providers` | 通用嵌入提供商类型和读取辅助函数，包括 `EmbeddingProviderAdapter`、`getEmbeddingProvider(...)` 和 `listEmbeddingProviders(...)`；插件通过 `api.registerEmbeddingProvider(...)` 注册提供商，以便强制执行清单所有权 |
    ...
    | `plugin-sdk/provider-stream` | `ProviderStreamFamily`、`buildProviderStreamFamilyHooks`、`composeProviderStreamWrappers`、流包装器类型、纯文本工具调用兼容，以及共享的 Anthropic/Bedrock/DeepSeek V4/Google/Kilocode/Moonshot/OpenAI/OpenRouter/Z.A.I/MiniMax/Copilot 包装器辅助函数 |
    ...
    | `plugin-sdk/provider-transport-runtime` | 原生提供商传输辅助函数，例如受保护的 fetch、工具结果文本提取、传输消息转换，以及可写传输事件流 |
...
提供商用量快照通常会报告一个或多个配额 `windows`，每个窗口都有
一个标签、已用百分比，以及可选的重置时间。如果提供商公开的是余额或
账户状态文本，而不是可重置的配额窗口，则应返回
带有空 `windows` 数组的 `summary`，而不是编造百分比。
...
用量端点失败或未返回可用的用量数据时，才使用 `error`。
  ...
  <Accordion title="认证和安全子路径">
    ...
    | `plugin-sdk/approval-handler-adapter-runtime` | 用于热路径渠道入口点的轻量级原生审批适配器加载辅助函数 |
    | `plugin-sdk/approval-handler-runtime` | 更宽泛的审批处理器运行时辅助函数；当更狭窄的适配器/Gateway 网关接缝足够时，优先使用它们 |
    | `plugin-sdk/approval-native-runtime` | 原生审批目标、账户绑定、路由门控、转发回退，以及本地原生 exec 提示抑制辅助函数 |
    | `plugin-sdk/approval-reaction-runtime` | 硬编码审批反应绑定、反应提示负载、反应目标存储，以及用于本地原生 exec 提示抑制的兼容导出 |
    ...
    | `plugin-sdk/channel-contract-testing` | 不包含宽泛测试 barrel 的狭窄渠道契约测试辅助函数 |
    | `plugin-sdk/command-auth-native` | 原生命令认证、动态参数菜单格式化，以及原生会话目标辅助函数 |
    | `plugin-sdk/command-detection` | 共享命令检测辅助函数 |
    | `plugin-sdk/command-primitives-runtime` | 用于热路径渠道路径的轻量级命令文本谓词 |
    | `plugin-sdk/command-surface` | 命令正文规范化和命令表面辅助函数 |
    | `plugin-sdk/allow-from` | `formatAllowFromLowercase` |
    | `plugin-sdk/channel-secret-runtime` | 用于渠道/插件密钥表面的狭窄密钥契约集合辅助函数 |
    | `plugin-sdk/secret-ref-runtime` | 用于密钥契约/配置解析的狭窄 `coerceSecretRef` 和 SecretRef 类型辅助函数 |
    | `plugin-sdk/secret-provider-integration` | 面向发布外部密钥提供商预设的插件的仅类型 SecretRef 提供商集成清单和预设契约 |
    | `plugin-sdk/security-runtime` | 共享信任、私信门控、根目录边界文件/路径辅助函数，包括仅创建写入、同步/异步原子文件替换、同级临时写入、跨设备移动回退、私有文件存储辅助函数、符号链接父级保护、外部内容、敏感文本遮盖、常量时间密钥比较，以及密钥集合辅助函数 |
  ...
  <Accordion title="运行时和存储子路径">
    ...
    | `plugin-sdk/browser-config` | 支持的浏览器配置门面，用于规范化 profile/defaults、CDP URL 解析和浏览器控制鉴权辅助工具 |
    | `plugin-sdk/agent-harness-task-runtime` | 面向使用宿主签发任务作用域的 harness-backed agents 的通用任务生命周期和完成投递辅助工具 |
    | `plugin-sdk/codex-mcp-projection` | 预留的内置 Codex 辅助工具，用于将用户 MCP 服务器配置投射到 Codex thread 配置；不适用于第三方插件 |
    | `plugin-sdk/codex-native-task-runtime` | 私有内置 Codex 辅助工具，用于原生任务镜像/运行时接线；不适用于第三方插件 |
    | `plugin-sdk/channel-runtime-context` | 通用频道运行时上下文注册和查找辅助工具 |
    ...
    | `plugin-sdk/gateway-method-runtime` | 预留的 Gateway 网关方法分发辅助工具，用于声明 `contracts.gatewayMethodDispatch: ["authenticated-request"]` 的插件 HTTP 路由 |
    | `plugin-sdk/gateway-runtime` | Gateway 网关客户端、事件循环就绪客户端启动辅助工具、Gateway 网关 CLI RPC、Gateway 网关协议错误、已公布的 LAN 主机解析，以及频道状态补丁辅助工具 |
    ...
    | `plugin-sdk/telegram-command-config` | Telegram 命令名/描述规范化，以及重复/冲突检查，即使内置 Telegram 契约表面不可用也可使用 |
    | `plugin-sdk/text-autolink-runtime` | 不依赖宽泛文本 barrel 的文件引用自动链接检测 |
    | `plugin-sdk/approval-reaction-runtime` | 硬编码的审批 reaction 绑定、reaction 提示 payload、reaction 目标存储，以及用于本地原生 exec 提示抑制的兼容性导出 |
    | `plugin-sdk/approval-runtime` | Exec/插件审批辅助工具、审批能力构建器、鉴权/profile 辅助工具、原生路由/运行时辅助工具，以及结构化审批显示路径格式化 |
    | `plugin-sdk/reply-runtime` | 共享的入站/回复运行时辅助工具、分块、分发、Heartbeat、回复规划器 |
    | `plugin-sdk/reply-dispatch-runtime` | 窄范围的回复分发/完成和 conversation-label 辅助工具 |
    | `plugin-sdk/reply-history` | 共享的短窗口回复历史辅助工具。新的消息轮次代码应使用 `createChannelHistoryWindow`；较低层的 map 辅助工具仅保留为已弃用的兼容性导出 |
    | `plugin-sdk/reply-reference` | `createReplyReferencePlanner` |
    | `plugin-sdk/reply-chunking` | 窄范围的文本/Markdown 分块辅助工具 |
    | `plugin-sdk/session-store-runtime` | 会话工作流辅助工具（`getSessionEntry`、`listSessionEntries`、`patchSessionEntry`、`upsertSessionEntry`）、按会话身份读取有界的近期用户/助手 transcript 文本、旧版会话存储路径/会话键辅助工具、updated-at 读取，以及仅用于过渡的整存储/文件路径兼容性辅助工具 |
    | `plugin-sdk/session-transcript-runtime` | Transcript 身份、限定作用域的目标/读/写辅助工具、更新发布、写锁，以及 transcript 记忆命中键 |
    | `plugin-sdk/sqlite-runtime` | 面向第一方运行时的聚焦 SQLite agent-schema、路径和事务辅助工具 |
    | `plugin-sdk/cron-store-runtime` | Cron 存储路径/加载/保存辅助工具 |
    | `plugin-sdk/state-paths` | State/OAuth 目录路径辅助工具 |
    | `plugin-sdk/plugin-state-runtime` | 插件 sidecar SQLite keyed-state 类型，以及面向插件自有数据库的集中连接 pragma 和 WAL 维护设置 |
    ...
    | `plugin-sdk/tool-plugin` | 定义一个简单的类型化 agent-tool 插件，并暴露用于清单生成的静态元数据 |
    ...
    | `plugin-sdk/agent-config-primitives` | 窄范围的 Agent Runtimes 配置 schema 基元 |
    ...
    | `plugin-sdk/agent-harness` | 面向低层 agent harness 的实验性受信插件表面：harness 类型、active-run steer/abort 辅助工具、OpenClaw 工具桥接辅助工具、runtime-plan 工具策略辅助工具、终端结果分类、工具进度格式化/详情辅助工具，以及 attempt result 实用工具 |
    ...
    | `plugin-sdk/fetch-runtime` | 包装后的 fetch、代理、EnvHttpProxyAgent 选项和 pinned lookup 辅助工具 |
    | `plugin-sdk/runtime-fetch` | 可感知 dispatcher 的运行时 fetch，不导入 proxy/guarded-fetch |
    | `plugin-sdk/inline-image-data-url-runtime` | 不依赖宽泛媒体运行时表面的内联图片 data URL 清理器和签名嗅探辅助工具 |
    | `plugin-sdk/response-limit-runtime` | 不依赖宽泛媒体运行时表面的有界响应正文读取器 |
    | `plugin-sdk/session-binding-runtime` | 当前对话绑定状态，不包含已配置绑定路由或配对存储 |
    | `plugin-sdk/session-store-runtime` | 不导入宽泛配置写入/维护的会话存储辅助工具 |
    | `plugin-sdk/sqlite-runtime` | 不包含数据库生命周期控制的聚焦 SQLite agent-schema、路径和事务辅助工具 |
    | `plugin-sdk/context-visibility-runtime` | 不导入宽泛配置/安全内容的上下文可见性解析和补充上下文过滤 |
    | `plugin-sdk/string-coerce-runtime` | 不导入 markdown/日志内容的窄范围 primitive record/字符串强制转换和规范化辅助工具 |
  ...
  <Accordion title="能力和测试子路径">
    | 子路径 | 关键导出项 |
    | --- | --- |
    | `plugin-sdk/media-runtime` | 共享媒体获取/转换/存储辅助函数，包括 `saveRemoteMedia`、`saveResponseMedia`、`readRemoteMediaBuffer` 和已弃用的 `fetchRemoteMedia`；当 URL 应成为 OpenClaw 媒体时，优先使用存储辅助函数，再读取缓冲区 |
    | `plugin-sdk/media-mime` | 精准的 MIME 规范化、文件扩展名映射、MIME 检测和媒体类型辅助函数 |
    | `plugin-sdk/media-store` | 精准的媒体存储辅助函数，例如 `saveMediaBuffer` 和 `saveMediaStream` |
    | `plugin-sdk/media-generation-runtime` | 共享媒体生成故障转移辅助函数、候选选择和缺失模型消息 |
    | `plugin-sdk/media-understanding` | 媒体理解提供商类型，以及面向提供商的图像/音频/结构化提取辅助导出项 |
    | `plugin-sdk/text-chunking` | 文本和 Markdown 分块/渲染辅助函数、Markdown 表格转换、指令标签剥离和安全文本工具 |
    | `plugin-sdk/text-chunking` | 出站文本分块辅助函数 |
    | `plugin-sdk/speech` | 语音提供商类型，以及面向提供商的指令、注册表、校验、OpenAI 兼容 TTS 构建器和语音辅助导出项 |
    | `plugin-sdk/speech-core` | 共享语音提供商类型、注册表、指令、规范化和语音辅助导出项 |
    ...
    | `plugin-sdk/image-generation-core` | 共享图像生成类型、故障转移、凭证和注册表辅助函数 |
    ...
    | `plugin-sdk/testing` | 面向旧版 OpenClaw 测试的仓库本地已弃用兼容性聚合导出入口。新的仓库测试应改为导入聚焦的本地测试子路径，例如 `plugin-sdk/agent-runtime-test-contracts`、`plugin-sdk/plugin-test-runtime`、`plugin-sdk/channel-test-helpers`、`plugin-sdk/test-env` 或 `plugin-sdk/test-fixtures` |
    | `plugin-sdk/plugin-test-api` | 仓库本地的最小 `createTestPluginApi` 辅助函数，用于不导入仓库测试辅助桥接的直接插件注册单元测试 |
    | `plugin-sdk/agent-runtime-test-contracts` | 仓库本地原生 Agent runtime 适配器契约夹具，用于凭证、投递、回退、工具钩子、提示词覆盖、schema 和转录投影测试 |
    | `plugin-sdk/channel-test-helpers` | 仓库本地面向渠道的测试辅助函数，用于通用操作/设置/状态契约、目录断言、账号启动生命周期、发送配置线程传递、运行时 mock、状态问题、出站投递和钩子注册 |
    | `plugin-sdk/channel-target-testing` | 用于渠道测试的仓库本地共享目标解析错误场景套件 |
    | `plugin-sdk/plugin-test-contracts` | 仓库本地插件包、注册、公共产物、直接导入、运行时 API 和导入副作用契约辅助函数 |
    | `plugin-sdk/provider-test-contracts` | 仓库本地提供商运行时、凭证、发现、新手引导、目录、向导、媒体能力、重放策略、实时 STT 实况音频、Web 搜索/获取和流契约辅助函数 |
    | `plugin-sdk/provider-http-test-mocks` | 仓库本地可选启用的 Vitest HTTP/凭证 mock，用于测试会执行 `plugin-sdk/provider-http` 的提供商测试 |
    | `plugin-sdk/test-fixtures` | 仓库本地通用 CLI 运行时捕获、沙箱上下文、skill 写入器、agent 消息、系统事件、模块重新加载、内置插件路径、终端文本、分块、凭证令牌和类型化用例夹具 |
  ...
  <Accordion title="记忆子路径">
    ...
    | `plugin-sdk/memory-core-host-engine-foundation` | 记忆主机基础引擎导出项 |
    | `plugin-sdk/memory-core-host-engine-embeddings` | 记忆主机嵌入契约、注册表访问、本地提供商，以及通用批处理/远程辅助函数。此表面上的 `registerMemoryEmbeddingProvider` 已弃用；新提供商请使用通用嵌入提供商 API。 |
    | `plugin-sdk/memory-core-host-engine-qmd` | 记忆主机 QMD 引擎导出项 |
    | `plugin-sdk/memory-core-host-engine-storage` | 记忆主机存储引擎导出项 |
    ...
    | `plugin-sdk/memory-host-core` | 面向记忆主机核心运行时辅助函数的供应商中立别名 |
    | `plugin-sdk/memory-host-events` | 面向记忆主机事件日志辅助函数的供应商中立别名 |
    | `plugin-sdk/memory-host-files` | 已弃用的兼容性别名；请使用 `plugin-sdk/memory-core-host-runtime-files` |
    | `plugin-sdk/memory-host-markdown` | 用于记忆相关插件的共享托管 Markdown 辅助函数 |
    | `plugin-sdk/memory-host-search` | 用于搜索管理器访问的主动记忆运行时 facade |
    | `plugin-sdk/memory-host-status` | 已弃用的兼容性别名；请使用 `plugin-sdk/memory-core-host-status` |
  </Accordion>

  <Accordion title="保留的内置辅助子路径">
    保留的内置辅助 SDK 子路径是面向内置插件代码的精准所有者特定表面。
    它们会在 SDK 清单中跟踪，使包构建和别名保持确定性，
    但它们不是通用插件编写 API。新的可复用主机契约应使用通用 SDK 子路径，
