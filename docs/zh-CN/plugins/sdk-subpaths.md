...
---
  ...
  本页按用途分组，列出常用子路径。生成的完整列表包含 200 多个子路径，位于 `scripts/lib/plugin-sdk-entrypoints.json`；
  ...
  <Accordion title="渠道子路径">
    ...
    | `plugin-sdk/account-core` | 多账户配置 / action-gate 辅助工具，以及默认账户回退辅助工具 |
    ...
    | `plugin-sdk/telegram-command-config` | Telegram 自定义命令规范化 / 验证辅助工具，带 bundled-contract 回退 |
    | `plugin-sdk/command-gating` | 精简的命令授权 gate 辅助工具 |
    ...
    | `plugin-sdk/interactive-runtime` | 语义消息呈现、投递以及旧版交互式回复辅助工具。参见 [消息呈现](/zh-CN/plugins/message-presentation) |
    | `plugin-sdk/channel-inbound` | 入站 debounce、提及匹配、提及策略辅助工具以及 envelope 辅助工具的兼容 barrel |
    | `plugin-sdk/channel-inbound-debounce` | 精简的入站 debounce 辅助工具 |
    | `plugin-sdk/channel-mention-gating` | 不包含更宽泛入站运行时接口的精简提及策略和提及文本辅助工具 |
    ...
    | `plugin-sdk/channel-actions` | 渠道消息 action 辅助工具，以及为插件兼容性保留的已弃用原生 schema 辅助工具 |
  ...
  <Accordion title="认证与安全子路径">
    ...
    | `plugin-sdk/approval-auth-runtime` | 审批人解析以及同一聊天中的 action-auth 辅助工具 |
    | `plugin-sdk/approval-client-runtime` | 原生 exec 审批 profile / 过滤器辅助工具 |
    ...
    | `plugin-sdk/approval-handler-runtime` | 更宽泛的审批处理器运行时辅助工具；如果更精简的 adapter / gateway 接口已足够，优先使用它们 |
    ...
    | `plugin-sdk/channel-contract-testing` | 不依赖宽泛 testing barrel 的精简渠道契约测试辅助工具 |
    ...
    | `plugin-sdk/security-runtime` | 共享 trust、私信 gate、外部内容以及 secret 收集辅助工具 |
    | `plugin-sdk/ssrf-policy` | 主机 allowlist 和私有网络 SSRF 策略辅助工具 |
    | `plugin-sdk/ssrf-dispatcher` | 不依赖宽泛基础设施运行时接口的精简 pinned-dispatcher 辅助工具 |
  ...
  <Accordion title="运行时与存储子路径">
    ...
    | `plugin-sdk/telegram-command-config` | Telegram 命令名称 / 描述规范化以及重复 / 冲突检查，即使 bundled Telegram 契约接口不可用也可使用 |
    ...
    | `plugin-sdk/reply-dispatch-runtime` | 精简的回复分发 / 收尾以及会话标签辅助工具 |
    ...
    | `plugin-sdk/status-helpers` | 共享渠道 / 账户状态摘要辅助工具、运行时状态默认值以及 issue 元数据辅助工具 |
    ...
    | `plugin-sdk/run-command` | 带计时功能的命令运行器，输出已规范化的 stdout / stderr 结果 |
    ...
    | `plugin-sdk/acp-binding-resolve-runtime` | 不触发生命周期启动导入的只读 ACP 绑定解析 |
    ...
    | `plugin-sdk/agent-harness` | 面向受信任插件的实验性低层级智能体 harness 接口：harness 类型、活动运行 steer / abort 辅助工具、OpenClaw 工具桥接辅助工具、工具进度格式化 / 详情辅助工具，以及 attempt 结果工具函数 |
    ...
    | `plugin-sdk/runtime-fetch` | 感知 dispatcher 的运行时 fetch，不导入 proxy / guarded-fetch |
    | `plugin-sdk/response-limit-runtime` | 不依赖宽泛 media 运行时接口的有界响应体读取器 |
    | `plugin-sdk/session-binding-runtime` | 当前会话绑定状态，不包含已配置绑定路由或配对存储 |
    | `plugin-sdk/session-store-runtime` | 不导入宽泛配置写入 / 维护逻辑的会话存储读取辅助工具 |
    | `plugin-sdk/context-visibility-runtime` | 不导入宽泛配置 / 安全逻辑的上下文可见性解析和补充上下文过滤 |
    | `plugin-sdk/string-coerce-runtime` | 不导入 markdown / 日志逻辑的精简基元记录 / 字符串强制转换与规范化辅助工具 |
  ...
  <Accordion title="能力与测试子路径">
    ...
    | `plugin-sdk/text-runtime` | 共享文本 / Markdown / 日志辅助工具，例如对智能体可见文本的剥离、Markdown 渲染 / 分块 / 表格辅助工具、脱敏辅助工具、directive-tag 辅助工具以及安全文本工具函数 |
