...
---
  ...
  <Accordion title="运行时与存储子路径">
    ...
    | `plugin-sdk/agent-harness` | 面向受信任插件的实验性低层智能体 harness 接口：harness 类型、活动运行 steer / abort 辅助函数、OpenClaw 工具桥接辅助函数，以及尝试结果工具函数 |
  ...
  <Accordion title="Memory 子路径">
    ...
    | `plugin-sdk/memory-core-host-engine-embeddings` | Memory 主机 embedding 契约、注册表访问、本地 provider，以及通用批处理 / 远程辅助函数 |
