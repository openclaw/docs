...
title: 插件 SDK 子路径
x-i18n:
    generated_at: "2026-04-28T03:09:11Z"
    model: gpt-5.4
    provider: openai
...
---
  ...
  本页按用途分组，列出常用子路径。生成的完整列表（超过 200 个子路径）位于 `scripts/lib/plugin-sdk-entrypoints.json`；
  ...
  | `plugin-sdk/testing`                      | 面向旧版插件测试的宽泛兼容性 barrel；新的扩展测试应优先使用聚焦的测试子路径                                                                                                  |
  | `plugin-sdk/plugin-test-api`              | 用于直接插件注册单元测试的最小化 `OpenClawPluginApi` mock 构建器                                                                                                             |
  | `plugin-sdk/agent-runtime-test-contracts` | 面向原生智能体运行时适配器契约的夹具，涵盖 auth profile、投递抑制、回退分类、工具钩子、提示词覆盖、schema 和转录修复                                                         |
  ...
  | `plugin-sdk/plugin-test-contracts`        | 插件注册、包清单、公开构件、运行时 API、导入副作用，以及直接导入契约辅助工具                                                                                                |
  ...
  | `plugin-sdk/provider-http-test-mocks`     | 为使用 `plugin-sdk/provider-http` 的提供商测试提供可选启用的 Vitest HTTP / 认证 mocks                                                                                       |
  | `plugin-sdk/test-env`                     | 测试环境、fetch / 网络、可释放 HTTP 服务器、传入请求、实时测试、临时文件系统和时间控制夹具                                                                                  |
  | `plugin-sdk/test-fixtures`                | 通用 CLI、沙箱、Skills、智能体消息、系统事件、模块重载、内置插件路径、终端、分块、认证令牌，以及类型化测试用例夹具                                                         |
  ...
  <Accordion title="渠道子路径">
    ...
    | `plugin-sdk/setup` | 共享设置向导辅助工具、allowlist 提示和设置状态构建器 |
    ...
    | `plugin-sdk/account-helpers` | 聚焦的账户列表 / 账户操作辅助工具 |
    | `plugin-sdk/channel-pairing` | `createChannelPairingController` |
    | `plugin-sdk/channel-reply-pipeline` | `createChannelReplyPipeline` |
    | `plugin-sdk/channel-config-helpers` | `createHybridChannelConfigAdapter` |
    | `plugin-sdk/channel-config-schema` | 共享渠道配置 schema 原语和通用构建器 |
    | `plugin-sdk/bundled-channel-config-schema` | 仅供受维护的内置插件使用的内置 OpenClaw 渠道配置 schemas |
    | `plugin-sdk/channel-config-schema-legacy` | 面向内置渠道配置 schemas 的已弃用兼容别名 |
    | `plugin-sdk/telegram-command-config` | 带有内置契约回退的 Telegram 自定义命令规范化 / 校验辅助工具 |
    | `plugin-sdk/command-gating` | 聚焦的命令授权门控辅助工具 |
    ...
    | `plugin-sdk/poll-runtime` | 聚焦的投票规范化辅助工具 |
    ...
    | `plugin-sdk/channel-config-primitives` | 聚焦的渠道配置 schema 原语 |
    ...
    | `plugin-sdk/channel-inbound` | 面向入站去抖动、提及匹配、提及策略辅助工具和信封辅助工具的兼容性 barrel |
    | `plugin-sdk/channel-inbound-debounce` | 聚焦的入站去抖动辅助工具 |
    | `plugin-sdk/channel-mention-gating` | 不包含更宽泛入站运行时接口的聚焦型提及策略、提及标记和提及文本辅助工具 |
    | `plugin-sdk/channel-envelope` | 聚焦的入站信封格式化辅助工具 |
    ...
    | `plugin-sdk/channel-route` | 共享路由规范化、基于解析器的目标解析、线程 ID 字符串化、去重 / 压缩路由键、已解析目标类型，以及路由 / 目标比较辅助工具 |
    | `plugin-sdk/channel-targets` | 目标解析辅助工具；路由比较调用方应使用 `plugin-sdk/channel-route` |
    | `plugin-sdk/channel-contract` | 渠道契约类型 |
    | `plugin-sdk/channel-feedback` | 反馈 / reaction 接线 |
    | `plugin-sdk/channel-secret-runtime` | 聚焦的 secret 契约辅助工具，例如 `collectSimpleChannelFieldAssignments`、`getChannelSurface`、`pushAssignment` 和 secret 目标类型 |
  </Accordion>
  ...
  <Accordion title="提供商子路径">
    ...
    | `plugin-sdk/lmstudio-runtime` | 受支持的 LM Studio 运行时 facade，用于本地服务器默认值、模型发现、请求头和已加载模型辅助工具 |
    | `plugin-sdk/provider-setup` | 精选的本地 / 自托管提供商设置辅助工具 |
    | `plugin-sdk/self-hosted-provider-setup` | 聚焦于兼容 OpenAI 的自托管提供商设置辅助工具 |
    ...
    | `plugin-sdk/provider-model-shared` | `ProviderReplayFamily`, `buildProviderReplayFamilyHooks`, `normalizeModelCompat`、共享 replay-policy 构建器、提供商端点辅助工具，以及 `normalizeNativeXaiModelId` 等模型 ID 规范化辅助工具 |
    ...
    | `plugin-sdk/provider-web-fetch-contract` | 聚焦的 web-fetch 配置 / 选择契约辅助工具，例如 `enablePluginInConfig` 和 `WebFetchProviderPlugin` |
    | `plugin-sdk/provider-web-fetch` | web-fetch 提供商注册 / 缓存辅助工具 |
    | `plugin-sdk/provider-web-search-config-contract` | 面向不需要插件启用接线的提供商的聚焦型 web 搜索配置 / 凭证辅助工具 |
    | `plugin-sdk/provider-web-search-contract` | 聚焦的 web 搜索配置 / 凭证契约辅助工具，例如 `createWebSearchProviderContractFields`、`enablePluginInConfig`、`resolveProviderWebSearchPluginConfig` 以及作用域化凭证 setter / getter |
    | `plugin-sdk/provider-web-search` | web 搜索提供商注册 / 缓存 / 运行时辅助工具 |
    | `plugin-sdk/provider-tools` | `ProviderToolCompatFamily`, `buildProviderToolCompatFamilyHooks`、Gemini schema 清理 + 诊断，以及 `resolveXaiModelCompatPatch` / `applyXaiModelCompat` 等 xAI 兼容辅助工具 |
    ...
    | `plugin-sdk/group-activation` | 聚焦的群组激活模式和命令解析辅助工具 |
  </Accordion>
  ...
  <Accordion title="认证与安全子路径">
    ...
    | `plugin-sdk/approval-auth-runtime` | 审批人解析和同一聊天中的操作认证辅助工具 |
    ...
    | `plugin-sdk/approval-handler-runtime` | 范围更广的审批处理器运行时辅助工具；如果更窄的 adapter / gateway 接缝已足够，应优先使用它们 |
    ...
    | `plugin-sdk/reply-dedupe` | 聚焦的入站回复去重重置辅助工具 |
    | `plugin-sdk/channel-contract-testing` | 不包含宽泛 testing barrel 的聚焦型渠道契约测试辅助工具 |
    ...
    | `plugin-sdk/command-surface` | 命令体规范化和命令 surface 辅助工具 |
    | `plugin-sdk/allow-from` | `formatAllowFromLowercase` |
    | `plugin-sdk/channel-secret-runtime` | 面向渠道 / 插件 secret surface 的聚焦型 secret 契约收集辅助工具 |
    | `plugin-sdk/secret-ref-runtime` | 面向 secret 契约 / 配置解析的聚焦型 `coerceSecretRef` 和 SecretRef 类型辅助工具 |
    ...
    | `plugin-sdk/ssrf-dispatcher` | 不包含宽泛基础设施运行时接口的聚焦型 pinned-dispatcher 辅助工具 |
  ...
  <Accordion title="运行时与存储子路径">
    ...
    | `plugin-sdk/runtime-env` | 聚焦的运行时环境、logger、超时、重试和退避辅助工具 |
    | `plugin-sdk/browser-config` | 受支持的浏览器配置 facade，用于规范化 profile / 默认值、CDP URL 解析和浏览器控制认证辅助工具 |
    ...
    | `plugin-sdk/gateway-runtime` | Gateway 网关客户端、Gateway 网关 CLI RPC、Gateway 网关协议错误，以及渠道状态补丁辅助工具 |
    ...
    | `plugin-sdk/reply-dispatch-runtime` | 聚焦的回复分发 / 完成和会话标签辅助工具 |
    ...
    | `plugin-sdk/reply-chunking` | 聚焦的文本 / Markdown 分块辅助工具 |
    ...
    | `plugin-sdk/agent-config-primitives` | 聚焦的智能体运行时配置 schema 原语 |
    ...
    | `plugin-sdk/agent-harness` | 面向低层 Agent harness 的实验性受信插件接口：harness 类型、活动运行 steer / abort 辅助工具、OpenClaw 工具桥接辅助工具、运行时计划工具策略辅助工具、终端结果分类、工具进度格式化 / 详情辅助工具，以及尝试结果工具函数 |
    ...
    | `plugin-sdk/fetch-runtime` | 包装后的 fetch、proxy 和固定查找辅助工具 |
    | `plugin-sdk/runtime-fetch` | 不引入 proxy / guarded-fetch 导入、具备 dispatcher 感知能力的运行时 fetch |
    ...
    | `plugin-sdk/string-coerce-runtime` | 不依赖 Markdown / 日志导入的聚焦型原始记录 / 字符串强制转换与规范化辅助工具 |
  ...
  <Accordion title="能力与测试子路径">
    ...
    | `plugin-sdk/media-store` | 聚焦的媒体存储辅助工具，例如 `saveMediaBuffer` |
    ...
    | `plugin-sdk/text-runtime` | 共享文本 / Markdown / 日志辅助工具，例如去除对助手可见文本、Markdown 渲染 / 分块 / 表格辅助工具、脱敏辅助工具、directive 标签辅助工具和安全文本工具 |
    | `plugin-sdk/text-chunking` | 出站文本分块辅助工具 |
    | `plugin-sdk/speech` | 语音提供商类型，以及面向提供商的 directive、注册表、校验、兼容 OpenAI 的 TTS 构建器和语音辅助导出 |
    ...
    | `plugin-sdk/image-generation` | 图像生成提供商类型，以及图像资源 / data URL 辅助工具和兼容 OpenAI 的图像提供商构建器 |
    ...
    | `plugin-sdk/testing` | 面向旧版插件测试的宽泛兼容性 barrel。新的扩展测试应改为导入聚焦的 SDK 子路径，例如 `plugin-sdk/agent-runtime-test-contracts`、`plugin-sdk/plugin-test-runtime`、`plugin-sdk/channel-test-helpers`、`plugin-sdk/test-env` 或 `plugin-sdk/test-fixtures` |
    | `plugin-sdk/plugin-test-api` | 最小化 `createTestPluginApi` 辅助工具，用于直接插件注册单元测试，无需导入仓库测试辅助桥接 |
    | `plugin-sdk/agent-runtime-test-contracts` | 面向原生智能体运行时适配器契约的夹具，用于 auth、投递、回退、工具钩子、提示词覆盖、schema 和转录投影测试 |
    | `plugin-sdk/channel-test-helpers` | 面向渠道的测试辅助工具，用于通用操作 / 设置 / Status 契约、目录断言、账户启动生命周期、发送配置线程化、运行时 mocks、Status 问题、出站投递和钩子注册 |
  ...
  <Accordion title="内存子路径">
    ...
    | `plugin-sdk/memory-core-host-engine-embeddings` | 内存主机 embedding 契约、注册表访问、本地提供商和通用批处理 / 远程辅助工具 |
    ...
    | `plugin-sdk/memory-core-host-secret` | 内存主机 secret 辅助工具 |
    ...
    | `plugin-sdk/memory-host-core` | 面向供应商中立的内存主机核心运行时辅助工具别名 |
    | `plugin-sdk/memory-host-events` | 面向供应商中立的内存主机事件日志辅助工具别名 |
    | `plugin-sdk/memory-host-files` | 面向供应商中立的内存主机文件 / 运行时辅助工具别名 |
    | `plugin-sdk/memory-host-markdown` | 面向内存相关插件的共享托管 Markdown 辅助工具 |
    | `plugin-sdk/memory-host-search` | 用于访问搜索管理器的活动内存运行时 facade |
    | `plugin-sdk/memory-host-status` | 面向供应商中立的内存主机 Status 辅助工具别名 |
