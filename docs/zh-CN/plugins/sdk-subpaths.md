...
---
  ...
  <Accordion title="运行时与存储子路径">
    ...
    | `plugin-sdk/agent-harness` | 面向受信任插件、仍属实验性的底层 Agent harness 接口：harness 类型、活动运行 steer / abort 辅助工具、OpenClaw 工具桥接辅助工具、工具进度格式化 / 细节辅助工具，以及 attempt 结果工具 |
  ...
  <Accordion title="能力与测试子路径">
    ...
    | `plugin-sdk/text-runtime` | 共享文本 / markdown / 日志辅助工具，例如对智能体可见文本的剥离、markdown 渲染 / 分块 / 表格辅助工具、脱敏辅助工具、directive-tag 辅助工具以及安全文本工具 |
