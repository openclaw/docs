...
title: 插件 SDK 概览
x-i18n:
    generated_at: "2026-04-05T10:06:20Z"
    model: gpt-5.4
    provider: openai
...
每个子路径都是一个小型、独立的模块。这样可以保持快速启动，并
防止循环依赖问题。对于特定于渠道的入口/构建辅助函数，
优先使用 `openclaw/plugin-sdk/channel-core`；将 `openclaw/plugin-sdk/core` 用于
更广泛的总入口表面以及共享辅助函数，例如
...
barrel 中组合通用 SDK 子路径，而核心
则应使用这些插件本地 barrel，或者仅在需求确实跨渠道时
添加一个狭义的通用 SDK
契约。

生成的导出映射仍然包含一小组内置插件辅助
接缝，例如 `plugin-sdk/feishu`、`plugin-sdk/feishu-setup`、
`plugin-sdk/zalo`、`plugin-sdk/zalo-setup` 和 `plugin-sdk/matrix*`。这些
子路径仅为内置插件维护和兼容性而存在；它们在下面的常用表格中被有意省略，
也不是新的第三方插件推荐使用的导入路径。
...
除非文档页面明确将某个子路径提升为公共接口，否则应将它们视为
实现细节/兼容性表面。
...
<AccordionGroup>
  <Accordion title="渠道子路径">
    ...
    | `plugin-sdk/channel-plugin-common` | 共享渠道插件序章导出 |
  ...
  <Accordion title="提供商子路径">
    ...
    | `plugin-sdk/provider-model-shared` | `ProviderReplayFamily`、`buildProviderReplayFamilyHooks`、`normalizeModelCompat`、共享 replay-policy 构建器、提供商端点辅助函数，以及诸如 `normalizeNativeXaiModelId` 之类的模型 id 规范化辅助函数 |
  ...
  <Accordion title="运行时与存储子路径">
    ...
    | `plugin-sdk/fetch-runtime` | 包装过的 fetch、代理和 pinned lookup 辅助函数 |
  ...
  <Accordion title="能力与测试子路径">
    | 子路径 | 关键导出 |
    | --- | --- |
    | `plugin-sdk/media-runtime` | 共享媒体抓取/转换/存储辅助函数以及媒体负载构建器 |
    | `plugin-sdk/media-understanding` | 媒体理解提供商类型以及面向提供商的图片/音频辅助导出 |
  ...
  <Accordion title="内存子路径">
    | 子路径 | 关键导出 |
    | --- | --- |
    | `plugin-sdk/memory-core` | 面向 manager/config/file/CLI 辅助函数的内置 memory-core 辅助表面 |
    ...
    | `plugin-sdk/memory-core-host-secret` | 内存宿主密钥辅助函数 |
  ...
  <Accordion title="保留的内置辅助子路径">
    | 系列 | 当前子路径 | 预期用途 |
    | --- | --- | --- |
    | Browser | `plugin-sdk/browser-config-support`、`plugin-sdk/browser-support` | 内置 Browser 插件支持辅助函数 |
...
`update.*`）始终保持为 `operator.admin`，即使插件尝试分配一个
更窄的 Gateway 网关方法作用域也是如此。对于插件自有方法，
优先使用插件专用前缀。
...
- `descriptors`：用于根 CLI 帮助、路由和惰性插件 CLI 注册的
  解析时命令描述符
...
- 用户配置仍然优先生效。OpenClaw 会将 `agents.defaults.cliBackends.<id>` 合并到
  插件默认值之上，然后再运行 CLI。
...
| 方法                                         | 注册内容                                 |
| ---------------------------------------------- | ---------------------------------------- |
| `api.registerMemoryEmbeddingProvider(adapter)` | 供活动插件使用的内存嵌入适配器           |

- `registerMemoryPromptSection`、`registerMemoryFlushPlan` 和
  `registerMemoryRuntime` 仅供内存插件独占使用。
...
- `before_tool_call`：返回 `{ block: true }` 为终止性决定。一旦任一处理器设置它，优先级更低的处理器将被跳过。
- `before_tool_call`：返回 `{ block: false }` 会被视为未做决定（与省略 `block` 相同），而不是覆盖。
- `before_install`：返回 `{ block: true }` 为终止性决定。一旦任一处理器设置它，优先级更低的处理器将被跳过。
- `before_install`：返回 `{ block: false }` 会被视为未做决定（与省略 `block` 相同），而不是覆盖。
- `message_sending`：返回 `{ cancel: true }` 为终止性决定。一旦任一处理器设置它，优先级更低的处理器将被跳过。
...
OpenClaw 已经运行时优先使用活动运行时配置快照。如果运行时
快照尚不存在，它们会回退到磁盘上已解析的配置文件。

当某个辅助函数是有意特定于提供商、且尚不属于通用 SDK
子路径时，提供商插件也可以公开一个狭义的插件本地契约 barrel。
当前的内置示例：Anthropic
提供商将其 Claude
流辅助函数保留在它自己的公共 `api.ts` / `contract-api.ts` 接缝中，而不是将 Anthropic beta-header 和 `service_tier` 逻辑提升到通用
`plugin-sdk/*` 契约中。
...
<Warning>
  ...
  如果某个辅助函数确实需要共享，请将其提升为中立的 SDK 子路径，
