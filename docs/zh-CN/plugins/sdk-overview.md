...
title: 插件 SDK 概览
x-i18n:
    generated_at: "2026-04-05T08:40:59Z"
    model: gpt-5.4
    provider: openai
    source_hash: d8da51d34879067c7b8cdfb371caddff7ad2bc84d161efaba2c7083149b06e47
...
每个子路径都是一个小型、自包含模块。这样可以保持启动快速，并避免循环依赖问题。对于渠道专属的入口 / 构建辅助函数，优先使用 `openclaw/plugin-sdk/channel-core`；将 `openclaw/plugin-sdk/core` 保留给更广泛的总入口表面和共享辅助函数，例如
`buildChannelConfigSchema`。

不要添加或依赖带有提供商名称的便捷边界，例如
`openclaw/plugin-sdk/slack`、`openclaw/plugin-sdk/discord`、
`openclaw/plugin-sdk/signal`、`openclaw/plugin-sdk/whatsapp` 或
`openclaw/plugin-sdk/whatsapp-surface`。内置插件应在它们自己的 `api.ts` 或 `runtime-api.ts` barrel 中组合通用
SDK 子路径，而核心则应使用这些插件本地 barrel，或者在需求确实跨渠道时添加狭义的通用 SDK
契约。

生成的导出映射仍然包含少量内置插件辅助边界，例如
`plugin-sdk/feishu`、`plugin-sdk/feishu-setup`、
`plugin-sdk/zalo`、`plugin-sdk/zalo-setup` 和 `plugin-sdk/matrix*`。这些
子路径仅用于内置插件维护和兼容性；下方的常用表格中刻意省略了它们，并且它们不是新第三方插件的推荐导入路径。

## 子路径参考

下面按用途分组列出了最常用的子路径。完整的 200+ 子路径生成列表位于 `scripts/lib/plugin-sdk-entrypoints.json`。

保留的内置插件辅助子路径仍会出现在该生成列表中。
除非某个文档页面明确将其提升为公开接口，否则请将其视为实现细节 / 兼容性表面。
...
<AccordionGroup>
  <Accordion title="渠道子路径">
    ...
    | `plugin-sdk/account-helpers` | 狭义账户列表 / 账户操作辅助函数 |
    ...
    | `plugin-sdk/direct-dm` | 共享私信认证 / 守卫辅助函数 |
  ...
  <Accordion title="提供商子路径">
    ...
    | `plugin-sdk/self-hosted-provider-setup` | 聚焦的 OpenAI 兼容自托管提供商设置辅助函数 |
    ...
    | `plugin-sdk/provider-model-shared` | `ProviderReplayFamily`、`buildProviderReplayFamilyHooks`、`normalizeModelCompat`、共享 replay-policy 构建器、provider-endpoint 辅助函数，以及如 `normalizeNativeXaiModelId` 之类的 model-id 规范化辅助函数 |
    ...
    | `plugin-sdk/provider-tools` | `ProviderToolCompatFamily`、`buildProviderToolCompatFamilyHooks`、Gemini schema 清理 + diagnostics，以及如 `resolveXaiModelCompatPatch` / `applyXaiModelCompat` 之类的 xAI 兼容辅助函数 |
  ...
  <Accordion title="认证和安全子路径">
    ...
    | `plugin-sdk/approval-client-runtime` | 原生 exec approval profile / 过滤辅助函数 |
    | `plugin-sdk/approval-delivery-runtime` | 原生 approval 能力 / 传递适配器 |
    | `plugin-sdk/approval-native-runtime` | 原生 approval 目标 + 账户绑定辅助函数 |
    | `plugin-sdk/approval-reply-runtime` | exec / 插件 approval 回复载荷辅助函数 |
  ...
  <Accordion title="运行时和存储子路径">
    ...
    | `plugin-sdk/runtime-env` | 狭义运行时环境、logger、超时、重试和回退辅助函数 |
    ...
    | `plugin-sdk/telegram-command-config` | Telegram 命令名 / 描述规范化以及重复 / 冲突检查，即使内置 Telegram 契约表面不可用也是如此 |
    | `plugin-sdk/approval-runtime` | exec / 插件 approval 辅助函数、approval 能力构建器、认证 / profile 辅助函数、原生路由 / 运行时辅助函数 |
    ...
    | `plugin-sdk/run-command` | 带定时的命令运行器，并返回规范化的 stdout / stderr 结果 |
    ...
    | `plugin-sdk/acp-runtime` | ACP 运行时 / 会话辅助函数 |
    ...
    | `plugin-sdk/fetch-runtime` | 包装过的 fetch、代理和固定查找辅助函数 |
  ...
  <Accordion title="能力和测试子路径">
    ...
    | `plugin-sdk/media-understanding-runtime` | 媒体理解运行器 facade 和类型化结果辅助函数 |
    ...
    | `plugin-sdk/speech-runtime` | Speech 核心运行时 facade，用于 TTS 解析与合成 |
    ...
    | `plugin-sdk/video-generation` | 视频生成提供商类型 |
    | `plugin-sdk/video-generation-core` | 共享视频生成类型、故障转移辅助函数、提供商查找和 model-ref 解析 |
    | `plugin-sdk/video-generation-runtime` | 共享运行时 `generateVideo` / `listRuntimeVideoGenerationProviders` facade |
  ...
  <Accordion title="保留的内置辅助子路径">
    | 家族 | 当前生成的子路径 | 预期用途 |
    | --- | --- | --- |
    | Browser | `plugin-sdk/browser`、`plugin-sdk/browser-runtime`、`plugin-sdk/browser-config-support`、`plugin-sdk/browser-support` | 内置浏览器插件维护和兼容性 |
    ...
    | 渠道专属辅助函数 | `plugin-sdk/googlechat`、`plugin-sdk/whatsapp-surface`、`plugin-sdk/zalouser`、`plugin-sdk/bluebubbles`、`plugin-sdk/bluebubbles-policy`、`plugin-sdk/mattermost`、`plugin-sdk/mattermost-policy`、`plugin-sdk/feishu-conversation`、`plugin-sdk/msteams`、`plugin-sdk/nextcloud-talk`、`plugin-sdk/nostr`、`plugin-sdk/tlon`、`plugin-sdk/twitch` | 内置渠道兼容性 / 辅助边界。`plugin-sdk/whatsapp-surface` 当前导出 `DEFAULT_WEB_MEDIA_BYTES`、WhatsApp 认证 / 账户辅助函数、目录配置辅助函数、群组策略辅助函数、出站目标解析以及狭义的 `WebChannelStatus` / `WebInboundMessage` / `WebListenerCloseReason` / `WebMonitorTuning` 类型。 |
    | 提供商专属辅助函数 | `plugin-sdk/openai`、`plugin-sdk/moonshot`、`plugin-sdk/qwen`、`plugin-sdk/qwen-definitions`、`plugin-sdk/modelstudio`、`plugin-sdk/modelstudio-definitions`、`plugin-sdk/provider-moonshot`、`plugin-sdk/together`、`plugin-sdk/amazon-bedrock`、`plugin-sdk/anthropic-vertex`、`plugin-sdk/cloudflare-ai-gateway`、`plugin-sdk/byteplus`、`plugin-sdk/chutes`、`plugin-sdk/deepseek`、`plugin-sdk/google`、`plugin-sdk/huggingface`、`plugin-sdk/kimi-coding`、`plugin-sdk/kilocode`、`plugin-sdk/minimax`、`plugin-sdk/mistral`、`plugin-sdk/nvidia`、`plugin-sdk/opencode`、`plugin-sdk/opencode-go`、`plugin-sdk/qianfan`、`plugin-sdk/sglang`、`plugin-sdk/synthetic`、`plugin-sdk/venice`、`plugin-sdk/vllm`、`plugin-sdk/xai`、`plugin-sdk/volcengine` | 内置提供商专属辅助边界；优先使用规范的 `qwen*`，并将 `modelstudio*` 保留为兼容性别名 |
...
`update.*`）始终保持为 `operator.admin`，即使插件尝试为其分配
更窄的 gateway 方法作用域也是如此。对于插件自有方法，请优先使用插件专属前缀。

### CLI 注册 metadata

`api.registerCli(registrar, opts?)` 接受两种顶层 metadata：
...
这种急切兼容路径仍然受支持，但它不会为解析时惰性加载安装
descriptor 支持的占位符。

### CLI 后端注册

`api.registerCliBackend(...)` 允许插件拥有本地
AI CLI 后端（例如 `claude-cli` 或 `codex-cli`）的默认配置。
...
| 方法                                       | 注册内容                           |
| ------------------------------------------ | ---------------------------------- |
| `api.registerContextEngine(id, factory)`   | 上下文引擎（一次只能有一个活动）   |
...
| 方法                                           | 注册内容                                       |
| ---------------------------------------------- | ---------------------------------------------- |
| `api.registerMemoryEmbeddingProvider(adapter)` | 为活动插件注册记忆嵌入适配器                   |

- `registerMemoryPromptSection`、`registerMemoryFlushPlan` 和
  `registerMemoryRuntime` 仅供记忆插件使用。
...
- `before_tool_call`：返回 `{ block: true }` 是终止性结果。一旦某个处理器设置了它，就会跳过更低优先级的处理器。
- `before_tool_call`：返回 `{ block: false }` 会被视为未作决定（与省略 `block` 相同），而不是覆盖。
- `before_install`：返回 `{ block: true }` 是终止性结果。一旦某个处理器设置了它，就会跳过更低优先级的处理器。
- `before_install`：返回 `{ block: false }` 会被视为未作决定（与省略 `block` 相同），而不是覆盖。
- `message_sending`：返回 `{ cancel: true }` 是终止性结果。一旦某个处理器设置了它，就会跳过更低优先级的处理器。
...
| `api.registrationMode`   | `PluginRegistrationMode`  | 当前加载模式；`"setup-runtime"` 是完整入口之前的轻量启动 / 设置窗口                   |
...
通过 facade 加载的内置插件公开表面（`api.ts`、`runtime-api.ts`、
`index.ts`、`setup-entry.ts` 以及类似的公开入口文件）现在在 OpenClaw 已经运行时会优先使用
活动运行时配置快照。如果运行时快照尚不存在，
它们会回退到磁盘上已解析的配置文件。

当某个辅助函数是有意为提供商专属，并且尚不属于通用 SDK
子路径时，提供商插件也可以暴露一个狭义的插件本地契约 barrel。当前的内置示例是：Anthropic
提供商将其 Claude 流辅助函数保留在自己的公开 `api.ts` / `contract-api.ts` 边界中，而不是将 Anthropic beta-header 和 `service_tier` 逻辑提升到通用的
`plugin-sdk/*` 契约。
...
- [设置和配置](/plugins/sdk-setup) — 打包、清单、配置 schema
- [测试](/plugins/sdk-testing) — 测试工具和 lint 规则
- [SDK 迁移](/plugins/sdk-migration) — 从已弃用表面迁移
- [插件内部机制](/plugins/architecture) — 深入架构和能力模型
