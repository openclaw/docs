...
内置引擎是默认的 Memory 后端。它将你的内存索引存储在每个智能体各自的 SQLite 数据库中，入门时不需要任何额外依赖。
...
如果要强制使用内置的本地嵌入提供商，请在 OpenClaw 旁边安装可选的
`node-llama-cpp` 运行时包，然后将 `local.modelPath` 指向一个 GGUF 文件：
...
自动检测会按表中显示的顺序，选择第一个能够解析其 API 密钥的提供商。设置 `memorySearch.provider` 可覆盖此行为。

## 索引如何工作

OpenClaw 会将 `MEMORY.md` 和 `memory/*.md` 索引为多个分块（约 400 个 token，带 80 个 token 重叠），并将其存储到每个智能体各自的 SQLite 数据库中。

- **索引位置：** `~/.openclaw/memory/<agentId>.sqlite`
- **存储维护：** SQLite WAL 附属文件会通过定期检查点和关闭时检查点进行限制。
- **文件监视：** 内存文件的更改会触发带防抖的重新索引（1.5 秒）。
...
如果你需要重排、查询扩展，或希望为工作区之外的目录建立索引，可以考虑切换到 [QMD](/zh-CN/concepts/memory-qmd)。
...
仅当 `memorySearch.local.modelPath` 指向现有本地文件时，如果提供商设置为 `auto`，才会优先考虑本地嵌入。

**结果过时？** 运行 `openclaw memory index --force` 以重建索引。监视器在极少数边缘情况下可能会漏掉更改。

**`sqlite-vec` 无法加载？** OpenClaw 会自动回退到进程内余弦相似度计算。请检查日志以查看具体的加载错误。
