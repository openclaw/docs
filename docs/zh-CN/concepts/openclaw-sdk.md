---
read_when:
    - 你正在构建一个与 OpenClaw 通信的外部应用、脚本、仪表盘、CI 作业或 IDE 扩展
    - 你正在应用 SDK 和插件 SDK 之间做选择
    - 你正在集成 Gateway 网关的智能体运行、会话、事件、审批、模型或工具
sidebarTitle: App SDK
summary: 面向外部应用、脚本、仪表板、CI 作业和 IDE 扩展的公共 OpenClaw 应用 SDK
title: OpenClaw 应用 SDK
x-i18n:
    generated_at: "2026-05-01T08:21:09Z"
    model: gpt-5.5
    provider: openai
...
**OpenClaw 应用 SDK** 是面向 OpenClaw 进程之外应用的公开客户端 API。当脚本、仪表盘、CI 作业、IDE 扩展或其他外部应用需要连接到 Gateway 网关、启动智能体运行、流式传输事件、等待结果、取消工作，或检查 Gateway 网关资源时，请使用 `@openclaw/sdk`。

<Note>
  App SDK 不同于 [插件 SDK](/zh-CN/plugins/sdk-overview)。
  `@openclaw/sdk` 从 OpenClaw 外部与 Gateway 网关通信。
  `openclaw/plugin-sdk/*` 仅适用于在 OpenClaw 内部运行，并注册提供商、渠道、工具、钩子或受信任运行时的插件。
</Note>
...
| `oc.agents`               | 就绪   | 列出、创建、更新、删除智能体，并获取智能体句柄。                           |
| `Agent.run()`             | 就绪   | 启动一个 Gateway 网关 `agent` 运行并返回一个 `Run`。                        |
| `oc.runs`                 | 就绪   | 创建、获取、等待、取消运行，并流式传输运行事件。                           |
| `Run.events()`            | 就绪   | 流式传输规范化的单次运行事件，并为快速运行提供重放。                       |
| `Run.wait()`              | 就绪   | 调用 `agent.wait` 并返回稳定的 `RunResult`。                                |
| `Run.cancel()`            | 就绪   | 按运行 id 调用 `sessions.abort`，可用时带上会话键。                         |
| `oc.sessions`             | 就绪   | 创建、解析、发送到、修补、压缩会话，并获取会话句柄。                       |
| `Session.send()`          | 就绪   | 调用 `sessions.send` 并返回一个 `Run`。                                     |
| `oc.models`               | 就绪   | 调用 `models.list` 和当前 `models.authStatus` 状态 RPC。                    |
| `oc.tools`                | 就绪   | 通过策略流水线列出、限定范围并调用 Gateway 网关工具。                      |
| `oc.artifacts`            | 就绪   | 列出、获取并下载 Gateway 网关转录制品。                                    |
| `oc.approvals`            | 就绪   | 通过 Gateway 网关审批 RPC 列出并解决执行审批。                             |
...
`new OpenClaw({ gateway: "ws://..." })` 等同于 `url`。构造函数接受 `gateway: "auto"` 选项，但自动 Gateway 网关发现尚不是独立的 SDK 功能；当应用尚不知道如何发现 Gateway 网关时，请传入 `url`。
...
像 `openai/gpt-5.5` 这样的提供商限定模型引用会被拆分为 Gateway 网关的 `provider` 和 `model` 覆盖项。`timeoutMs` 在 SDK 中保持毫秒单位，并会为 `agent` RPC 转换为 Gateway 网关超时秒数。

`run.wait()` 使用 Gateway 网关的 `agent.wait` RPC。如果等待截止时间到期而运行仍处于活动状态，则返回 `status: "accepted"`，而不是假装运行本身超时。运行时超时、已中止运行和已取消运行会被规范化为 `timed_out` 或 `cancelled`。
...
SDK 会将原始 Gateway 网关事件规范化为稳定的 `OpenClawEvent` 封套：
...
`Run.events()` 会将事件过滤到一个运行 id，并为快速运行重放已看到的事件。这意味着记录的流程是安全的：
...
工具辅助函数暴露 Gateway 网关目录、有效工具视图和直接 Gateway 网关工具调用。`oc.tools.invoke()` 会返回类型化封套，而不会因策略或审批拒绝而抛出。
...
制品辅助函数为会话、运行或任务上下文暴露 Gateway 网关制品投影。每次调用都需要一个显式的 `sessionKey`、`runId` 或 `taskId` 范围：
...
SDK 包含我们想要的产品模型名称，但它不会默默假装 Gateway 网关 RPC 已存在。这些调用目前会抛出明确的不支持错误：
...
单次运行的 `workspace`、`runtime`、`environment` 和 `approvals` 字段已按未来形态进行类型化，但当前 Gateway 网关不支持在 `agent` RPC 上覆盖这些项。如果调用方传入这些字段，SDK 会在提交运行前抛出，以免工作意外以默认工作区、运行时、环境或审批行为执行。

## App SDK 与插件 SDK

当代码位于 OpenClaw 外部时，使用 App SDK：
...
App SDK 代码应从 `@openclaw/sdk` 导入。插件代码应从文档化的 `openclaw/plugin-sdk/*` 子路径导入。不要混用这两套契约。
