...
x-i18n:
    generated_at: "2026-05-05T14:12:40Z"
    model: gpt-5.5
    provider: openai
...
**OpenClaw 应用 SDK** 是 OpenClaw 进程外应用的公共客户端 API。当脚本、仪表盘、CI 作业、IDE 扩展或其他外部应用想要连接到 Gateway 网关、启动智能体运行、流式传输事件、等待结果、取消工作或检查 Gateway 网关资源时，请使用 `@openclaw/sdk`。
...
| `Run.events()`            | 就绪       | 流式传输规范化的单运行事件，并为快速运行提供重放。                                |
...
| `oc.models`               | 就绪       | 调用 `models.list` 和当前 `models.authStatus` Status RPC。                         |
| `oc.tools`                | 就绪       | 通过策略管道列出、限定作用域并调用 Gateway 网关工具。                             |
| `oc.artifacts`            | 就绪       | 列出、获取并下载 Gateway 网关转录制品。                                           |
| `oc.approvals`            | 就绪       | 通过 Gateway 网关审批 RPC 列出并解析执行审批。                                    |
| `oc.environments`         | 部分支持   | 列出 Gateway 网关本地和节点环境候选项；创建/删除尚未接入。                       |
| `oc.rawEvents()`          | 就绪       | 为高级使用者公开原始 Gateway 网关事件。                                           |
| `normalizeGatewayEvent()` | 就绪       | 将原始 Gateway 网关事件转换为稳定的 SDK 事件形状。                                |

该 SDK 还导出这些表面使用的核心类型：
...
`new OpenClaw({ gateway: "ws://..." })` 等同于 `url`。构造函数接受 `gateway: "auto"` 选项，但自动 Gateway 网关发现还不是独立的 SDK 功能；当应用尚不知道如何发现 Gateway 网关时，请传入 `url`。
...
`run.wait()` 使用 Gateway 网关 `agent.wait` RPC。如果等待截止时间到期时运行仍处于活动状态，则返回 `status: "accepted"`，而不是假装运行本身已超时。运行时超时、中止的运行和已取消的运行会被规范化为 `timed_out` 或 `cancelled`。
...
该 SDK 将原始 Gateway 网关事件规范化为稳定的 `OpenClawEvent` 信封：
...
`Run.events()` 将事件过滤为一个运行 ID，并为快速运行重放已经看到的事件。这意味着以下已记录流程是安全的：
...
工具助手公开 Gateway 网关目录、有效工具视图和直接 Gateway 网关工具调用。`oc.tools.invoke()` 返回一个类型化信封，而不是因策略或审批拒绝而抛出异常。
...
制品助手公开用于会话、运行或任务上下文的 Gateway 网关制品投影。每次调用都需要一个显式的 `sessionKey`、`runId` 或 `taskId` 作用域：
...
SDK 包含我们想要的产品模型名称，但不会悄悄假装 Gateway 网关 RPC 存在。这些调用目前会抛出明确的不支持错误：
...
单运行的 `workspace`、`runtime`、`environment` 和 `approvals` 字段被类型化为未来形状，但当前 Gateway 网关不支持在 `agent` RPC 上使用这些覆盖项。如果调用者传入它们，SDK 会在提交运行之前抛出异常，确保工作不会意外地以默认工作区、运行时、环境或审批行为执行。
...
应用 SDK 代码应从 `@openclaw/sdk` 导入。插件代码应从文档化的 `openclaw/plugin-sdk/*` 子路径导入。不要混用这两种契约。
