...
title: 流式传输和分块
x-i18n:
    generated_at: "2026-05-06T01:07:31Z"
    model: gpt-5.5
    provider: openai
...
目前没有向渠道消息发送的 **真正 token 增量流式传输**。预览流式传输基于消息（发送 + 编辑/追加）。
...
- Discord 软上限：`channels.discord.maxLinesPerMessage`（默认 17）会拆分较高的回复，以避免 UI 裁剪。

**边界语义：**

- `text_end`：只要 chunker 发出块就开始流式传输；在每个 `text_end` 时刷新。
...
`MEDIA:` 指令是普通投递元数据。当分块流式传输提前发送媒体块时，OpenClaw 会记住该轮次的这次投递。如果最终助手载荷重复同一个媒体 URL，最终投递会去除重复媒体，而不是再次发送附件。

完全重复的最终载荷会被抑制。如果最终载荷在已经流式传输的媒体周围添加了不同文本，OpenClaw 仍会发送新文本，同时保持媒体只投递一次。这可以防止在 Telegram 等渠道中出现重复语音备注或文件，例如智能体在流式传输期间发出 `MEDIA:`，而提供商也在完成的回复中包含它。

## 分块算法（低/高边界）

块分块由 `EmbeddedBlockChunker` 实现：

- **低边界：** 缓冲区达到 `minChars` 前不发出（除非强制）。
- **高边界：** 优先在 `maxChars` 前拆分；如果强制，则在 `maxChars` 处拆分。
- **断点偏好：** `paragraph` → `newline` → `sentence` → `whitespace` → 硬断开。
- **代码围栏：** 永不在围栏内拆分；在 `maxChars` 处强制拆分时，会关闭 + 重新打开围栏以保持 Markdown 有效。

`maxChars` 会被限制到渠道的 `textChunkLimit`，因此不能超过每个渠道的上限。

## 合并（合并已流式传输的块）

启用分块流式传输时，OpenClaw 可以在发送前 **合并连续块分块**。这可以减少“单行刷屏”，同时仍然提供渐进式输出。

- 合并会等待 **空闲间隔**（`idleMs`）后再刷新。
- 缓冲区受 `maxChars` 限制，超过时会刷新。
- `minChars` 会阻止极小片段发送，直到累积足够文本（最终刷新始终会发送剩余文本）。
...
启用分块流式传输时，你可以在块回复之间（第一个块之后）添加 **随机暂停**。这会让多气泡回复显得更自然。
...
- **在末尾流式传输全部内容：** `blockStreamingBreak: "message_end"`（刷新一次；如果很长，可能有多个分块）。
- **不使用分块流式传输：** `blockStreamingDefault: "off"`（只有最终回复）。

**渠道注意事项：** 除非显式将 `*.blockStreaming` 设置为 `true`，否则分块流式传输为 **关闭**。渠道可以在没有块回复的情况下流式传输实时预览（`channels.<channel>.streaming`）。
...
`streaming.mode: "block"` 是适用于 Discord 和 Telegram 等可编辑渠道的预览流式传输模式。它不会在这些渠道启用渠道块投递。如果你想要普通块回复，请使用 `streaming.block.enabled` 或旧版 `blockStreaming` 渠道键。Microsoft Teams 是例外：它没有草稿预览块传输，因此 `streaming.mode: "block"` 会映射到 Teams 块投递，而不是原生 partial/progress 流式传输。
...
- Slack 原生流式传输和 Slack 助手线程 Status 需要回复线程目标。顶层私信不会显示这种线程式预览，但仍可使用 Slack 草稿预览帖子和编辑。
...
- 当预览已可见约一分钟时，会发送新的最终消息，而不是原地编辑，然后清理预览，使 Telegram 的时间戳反映回复完成时间。
...
- 最终媒体、错误和显式回复载荷会取消待处理预览而不刷新新草稿，然后使用正常投递。
...
- 最终媒体/错误载荷和进度最终内容不会创建一次性草稿消息；只有可编辑预览的文本/块最终内容才会刷新待处理草稿文本。

Mattermost：

- 将思考、工具活动和部分回复文本流式传输到单个草稿预览帖子中，并在最终答案可以安全发送时原地完成。
- 如果预览帖子被删除或在完成时不可用，则回退为发送新的最终帖子。
- 最终媒体/错误载荷会在正常投递前取消待处理预览更新，而不是刷新临时预览帖子。

Matrix：

- 当最终文本可以复用预览事件时，草稿预览会原地完成。
- 仅媒体、错误和回复目标不匹配的最终内容会在正常投递前取消待处理预览更新；已经可见的过期预览会被撤回。

### 工具进度预览更新

预览流式传输还可以包含 **工具进度** 更新，即“正在搜索网页”、“正在读取文件”或“正在调用工具”等短 Status 行；这些行会在工具运行期间出现在同一条预览消息中，先于最终回复。这会让多步骤工具轮次在第一次思考预览和最终答案之间保持视觉上的活动状态，而不是静默无变化。

支持的表面：

- 当预览流式传输处于活动状态时，**Discord**、**Slack**、**Telegram** 和 **Matrix** 默认会将工具进度流式传输到实时预览编辑中。Microsoft Teams 在个人聊天中使用其原生进度流。
- Telegram 自 `v2026.4.22` 起已启用工具进度预览更新；保持启用可保留该已发布行为。
- **Mattermost** 已经将工具活动折叠进其单个草稿预览帖子中（见上文）。
- 工具进度编辑遵循当前预览流式传输模式；当预览流式传输为 `off`，或分块流式传输已接管消息时，会跳过这些编辑。在 Telegram 上，`streaming.mode: "off"` 表示仅最终内容：通用进度闲聊也会被抑制，而不是作为独立 Status 消息投递；审批提示、媒体载荷和错误仍会正常路由。
- 若要保留预览流式传输但隐藏工具进度行，请将该渠道的 `streaming.preview.toolProgress` 设置为 `false`。若要在隐藏命令/执行文本的同时保持工具进度行可见，请将 `streaming.preview.commandText` 设置为 `"status"`，或将 `streaming.progress.commandText` 设置为 `"status"`；默认值为 `"raw"`，以保留已发布行为。该策略由使用 OpenClaw 紧凑进度渲染器的草稿/进度渠道共享，包括 Discord、Matrix、Microsoft Teams、Mattermost、Slack 草稿预览和 Telegram。若要完全禁用预览编辑，请将 `streaming.mode` 设置为 `off`。
- Telegram 选中引用回复是例外：当 `replyToMode` 不是 `"off"` 且存在选中引用文本时，OpenClaw 会跳过该轮次的答案预览流，因此工具进度预览行无法渲染。没有选中引用文本的当前消息回复仍会保持预览流式传输。详见 [Telegram 渠道文档](/zh-CN/channels/telegram)。
...
在另一个紧凑型进度渠道键下使用相同结构，例如 `channels.discord`、`channels.matrix`、`channels.msteams`、`channels.mattermost`，或 Slack 草稿预览。对于进度草稿模式，将相同策略放在 `streaming.progress` 下：
