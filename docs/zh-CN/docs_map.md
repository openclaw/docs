...
summary: OpenClaw 文档页面生成的标题映射
title: 文档地图
x-i18n:
    generated_at: "2026-07-04T17:49:25Z"
    model: gpt-5.5
...
此文件由 `docs/**/*.md` 和 `docs/**/*.mdx` 标题生成，用于帮助智能体导航文档树。
...
- Headings:
  ...
  - H3: 钩子包
  - H2: 内置钩子
  - H3: session-memory 详细信息
  - H3: bootstrap-extra-files 配置
  - H3: command-logger 详细信息
  - H3: compaction-notifier 详细信息
  - H3: boot-md 详细信息
  - H2: 插件钩子
  - H2: 配置
  - H2: CLI 参考
...
- 标题：
  ...
  - H2：成员信息操作
  - H2：历史上下文
  - H2：当前 Teams RSC 权限（Manifest）
  - H2：示例 Teams Manifest（已脱敏）
  - H3：Manifest 注意事项（必备字段）
  - H3：更新现有应用
  - H2：能力：仅 RSC 与 Graph
  - H3：仅使用 Teams RSC（应用已安装，无 Graph API 权限）
  - H3：使用 Teams RSC + Microsoft Graph Application 权限
  - H3：RSC 与 Graph API
  - H2：支持 Graph 的媒体 + 历史记录（频道必需）
  - H2：已知限制
  - H3：Webhook 超时
...
- 路由：/channels/slack
- 标题：
  - H2：选择 Socket Mode 或 HTTP Request URL
  - H3：中继模式
  - H2：安装
  - H2：快速设置
  - H2：Socket Mode 传输调优
...
- 标题:
  ...
  - H2: 它是什么
  - H2: 设置（快速路径）
  - H3: 1) 创建机器人令牌（Zalo Bot Platform）
  - H3: 2) 配置令牌（环境变量或配置）
  - H2: 工作原理（行为）
  - H2: 限制
  - H2: 访问控制（私信）
...
- 标题：
  ...
  - H3：其他内置提供商插件
  - H4：值得了解的特殊行为
  - H2：通过 models.providers 配置的提供商（自定义/base URL）
  - H3：Moonshot AI（Kimi）
  - H3：Kimi 编程
...
- 标题：
  ...
  - H2：配置键（概览）
  - H3：安全编辑允许列表
  - H2：“Model is not allowed”（以及回复为什么停止）
  - H2：在聊天中切换模型（/model）
  - H2：CLI 命令
  - H3：models list
  - H3：models status
  - H2：扫描（OpenRouter 免费模型）
  - H2：Models 注册表（models.json）
  - H2：相关
...
- 路由: /gateway/bonjour
- 标题:
  - H2: 通过 Tailscale 使用广域 Bonjour（Unicast DNS-SD）
  - H3: Gateway 网关配置（推荐）
...
- 标题：
  - H2：术语
  - H2：为什么我们同时保留直连和 SSH
  - H2：设备发现输入（客户端如何得知 Gateway 网关位置）
...
- 标题：
  ...
  - H3：你的 tailnet 中的常驻 Gateway 网关
  - H3：家用桌面运行 Gateway 网关
...
- 标题：
  ...
  - H2：分裂脑安装和较新配置防护
  - H2：回滚后的协议不匹配
  - H2：Skills 符号链接因路径逃逸被跳过
  - H2：Anthropic 429：长上下文需要额外用量
  - H2：上游 403 被阻止响应
  - H2：本地 OpenAI 兼容后端通过直接探测，但智能体运行失败
  - H2：没有回复
  - H2：Dashboard Control UI 连接
  - H3：凭证详情代码快速映射
  - H2：Gateway 网关服务未运行
  - H2：macOS Gateway 网关静默停止响应，然后在你触碰仪表盘时恢复
...
- 标题：
  ...
  - H3：第 2 层：Gateway 网关 + 开发智能体冒烟（“@openclaw” 实际执行的内容）
...
- 标题：
  ...
  - H2：插件安装因缺少 openclaw extensions 而失败
  - H2：安装策略阻止插件安装或更新
  - H2：插件存在但被可疑所有权阻止
  - H2：决策树
  - H2：相关
...
- 标题：
  - H2：推荐默认值（大多数用户）
  - H2：macOS VM 选项
  - H3：你的 Apple Silicon Mac 上的本地 VM（Lume）
  - H3：托管式 Mac 提供商（云）
  - H2：快速路径（Lume，有经验的用户）
  - H2：你需要什么（Lume）
  - H2：1) 安装 Lume
...
- 标题:
  ...
  - H3: 运行位置说明
  - H3: 启动节点主机（前台）
  - H3: 通过 SSH 隧道的远程 Gateway 网关（loopback 绑定）
  - H3: 启动节点主机（服务）
  - H3: 配对 + 命名
  - H3: 将命令加入允许列表
  - H3: 将 exec 指向该节点
...
- 标题:
  ...
  - H3: Context engine 组装必须投影到 Codex 输入中
  - H3: Prompt-cache 稳定性很重要
  - H3: 运行时选择语义不变
  - H2: 实施计划
  - H3: 1. 导出或重新放置可复用的 context-engine attempt 辅助工具
  - H3: 2. 添加 Codex context projection 辅助工具
  - H3: 3. 在线程启动前接入 bootstrap
  - H3: 4. 在线程/start / 线程/resume 和 turn/start 前接入 assemble
  - H3: 5. 保持 prompt-cache 稳定格式
  - H3: 6. 在 transcript 镜像后接入 post-turn
...
- 标题:
  ...
  - H3: 1) 启动 Gateway 网关
  - H3: 2) 验证设备发现（可选）
  - H4: 通过单播 DNS-SD 在 Tailnet（Vienna ⇄ London）中设备发现
  - H3: 3) 从 Android 连接
  - H3: 在线状态存活信标
  - H3: 4) 批准配对（CLI）
  - H3: 5) 验证节点已连接
...
- 标题：
  ...
  - H2：配置形态
  - H2：高级后端钩子
  - H3：ownsNativeCompaction：退出 OpenClaw 压缩
  - H2：MCP 工具桥接
  - H2：用户配置
  - H2：验证
  - H2：清单
...
- 标题：
  ...
  - H2: 安全拒绝 fallback（Claude Fable 5）
  - H3: 存在原因
  - H3: 工作方式
  - H3: 可观测性和计费
  - H3: 范围
...
- 标题：
  ...
  - H3: 第 0 阶段：冻结边界
  - H3: 第 1 阶段：完成全局控制平面
  - H3: 第 2 阶段：引入按 Agent 数据库
  - H3: 第 3 阶段：替换会话存储 API
  - H3: 第 4 阶段：迁移转录、ACP 流、轨迹和 VFS
  - H3: 第 5 阶段：备份、恢复、Vacuum 和验证
...
- 标题：
  - H2: 主要旋钮
  - H3: cacheRetention（全局默认、模型和按 Agent）
...
- 标题：
  ...
  - H3：3.7 收集与外泄（AML.TA0009、AML.TA0010）
...
- 标题：
  ...
  - H2：重要注意事项
  - H2：复现结果
  - H3：Gateway 网关暴露和开放 Gateway 网关错误配置
  - H3：Node exec 管线（最高风险能力）
  - H3：配对存储（私信门控）
  - H3：入口门控（提及 + 控制命令绕过）
  - H3：路由/会话键隔离
  - H2：v1++：其他有界模型（并发、重试、追踪正确性）
  - H3：配对存储并发 / 幂等性
  - H3：入口追踪关联 / 幂等性
...
- 标题：
  ...
  - H3：macOS IPC 流程
  - H2：常见问题
  - H3：accountId 和 threadId 何时会用于审批目标？
