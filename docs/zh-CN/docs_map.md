...
title: 文档地图
x-i18n:
    generated_at: "2026-07-05T17:40:57Z"
    model: gpt-5.5
...
- 标题：
  ...
  - H3：仅使用 Teams RSC（已安装 app，无 Graph API 权限）
...
- 标题：
  ...
  - H2：所有者和审批系统
  - H2：自动接受设置
  - H2：通过 Urbit settings store 热重载
  - H2：投递目标（CLI/cron）
  - H2：内置 skill
  - H2：能力
  - H2：故障排查
...
- 标题：
  ...
  - H3: Claude CLI 细节
  - H2: 会话
  - H2: 来自 claude-cli 会话的 fallback prelude
  - H2: 图片
  - H2: 输入和输出
  - H2: 插件拥有的默认值
  - H2: 文本转换叠加层
...
- 标题：
  ...
  - H3：完全访问（无沙箱）
  - H3：只读工具 + 只读工作区
  - H3：无文件系统 / shell 访问（允许提供商消息）
  - H2：浏览器控制风险
  - H3：浏览器 SSRF 策略（默认严格）
  - H2：网络暴露
  - H3：绑定、端口、防火墙
  - H3：使用 UFW 的 Docker 端口发布
...
- 标题：
  ...
  - H2：插件安装失败，缺少 openclaw extensions
  - H2：安装策略阻止插件安装或更新
  - H2：插件存在但因可疑所有权被阻止
  - H2：决策树
  - H2：相关内容
...
- 标题:
  ...
  - H2: 自动检测（默认）
  - H2: 配置示例
  - H3: 提供商 + CLI 备用方案（OpenAI + Whisper CLI）
  - H3: 仅提供商并带权限范围门控
  - H3: 仅提供商（Deepgram）
...
- 标题：
  ...
  - H3：第 0 阶段：冻结边界
  - H3：第 1 阶段：完成全局控制平面
  - H3：第 2 阶段：引入每 Agent 数据库
  - H3：第 3 阶段：替换会话存储 API
  - H3：第 4 阶段：迁移转录记录、ACP 流、轨迹和 VFS
...
- 标题:
  ...
  - H3: Heartbeat 保温
  - H2: 提供商行为
  - H3: Anthropic（直连 API 和 Vertex AI）
  - H3: OpenAI（直连 API）
  - H3: Amazon Bedrock
  - H3: OpenRouter
...
- 标题:
  ...
  - H2: 成本估算（显示时）
  - H2: 缓存 TTL 和剪枝影响
  - H3: 示例：使用 Heartbeat 保持 1 小时缓存温热
...
- 标题：
  ...
  - H2: 复现结果
  - H2: 声明和目标
  - H3: Gateway 暴露和开放 Gateway 网关配置错误
  - H3: Node exec 管线（最高风险能力）
  - H3: 配对存储（私信门控）
  - H3: 入口门控（提及和控制命令绕过）
  - H3: 路由和会话键隔离
  - H2: v1++ 模型：并发、重试、跟踪正确性
  - H3: 配对存储并发和幂等性
  - H3: 入口跟踪关联和幂等性
...
- 标题：
  ...
  - H2: 必需配置
  - H2: acpx 后端的插件设置
  - H3: acpx runtime 启动探针
  - H3: 自动适配器下载
  - H3: 插件工具 MCP 桥
  - H3: OpenClaw 工具 MCP 桥
  - H3: runtime 操作超时配置
  - H3: 健康探针智能体配置
  - H2: 权限配置
  - H3: permissionMode
...
- 标题:
  ...
  - H2: YOLO 模式（无审批）
  - H3: 持久化 gateway-host “never prompt” 设置
  - H3: 本地快捷方式
  - H3: Node 主机
  - H3: 仅会话快捷方式
  - H2: 允许列表（按智能体）
