...
- 标题：
  ...
  - H3: Anthropic API key + MiniMax 备用
  - H3: 工作 Bot（受限访问）
  - H3: 仅本地模型
  - H2: 提示
  - H2: 相关
...
- 标题:
  ...
  - H3: 7. 规范化用量和 prompt-cache 运行时上下文
  - H3: 8. 压缩策略
...
- 标题：
  ...
  - H2: 成本估算（显示时）
  - H2: 缓存 TTL 和剪枝影响
  - H3: 示例：用 Heartbeat 为 1 小时缓存保温
  - H3: 示例：混合流量与按 Agent 配置的缓存策略
  - H3: Anthropic 1M 上下文
  - H2: 降低 token 压力的提示
  - H2: 相关
