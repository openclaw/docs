...
title: 文档地图
x-i18n:
    generated_at: "2026-07-12T21:23:58Z"
    model: gpt-5.6
...
此文件根据 `docs/**/*.md` 和 `docs/**/*.mdx` 中的标题生成，用于帮助智能体浏览文档树。
...
- 标题：
  ...
  - H2：使用方法
  - H2：选择智能体
  - H2：通过 acpx 使用（Codex、Claude 及其他 ACP 客户端）
  - H2：Zed 编辑器设置
  - H2：会话映射
  - H2：选项
  - H3：acp 客户端选项
...
- 标题：
  ...
  - H2: 组件和流程
  - H3: Gateway 网关（守护进程）
  - H3: 客户端（Mac 应用 / CLI / Web 管理界面）
  - H3: 节点（macOS / iOS / Android / 无头设备）
  - H3: WebChat
  - H2: 连接生命周期（单个客户端）
  - H2: 线协议（摘要）
  - H2: 配对和本地信任
...
- 标题：
  ...
  - H2：检查模型身份验证状态
  - H2：API 密钥轮换（Gateway 网关）
  - H2：Gateway 网关运行时移除提供商身份验证
  - H2：控制使用哪项凭据
...
- 标题：
  ...
  - H3：推荐的起始配置
  - H2：扩展示例（主要选项）
  - H3：通过符号链接连接的同级 Skills 仓库
  - H2：常见模式
  - H3：共享 Skills 基线并覆盖一项配置
  - H3：多平台设置
  - H3：受信任节点网络自动审批
...
- 标题：
  ...
  - H2：标签策略
  - H2：PromQL 用法示例
  - H2：如何选择 Prometheus 或 OpenTelemetry 导出
  - H2：故障排查
  - H2：相关内容
...
- 标题：
  - H1：通过远程 Gateway 网关运行 OpenClaw.app
  - H2：设置
  - H2：工作原理
  - H2：相关内容
...
- 标题：
  ...
  - H3：日志与对话记录
  - H2：安全基线（复制/粘贴）
  - H3：使用独立号码（WhatsApp、Signal、Telegram）
  - H2：事件响应
  - H3：遏制
...
- 标题：
  ...
  - H2：长上下文触发 Anthropic 429，需要额外用量
  - H2：上游 403 阻止响应
  ...
  - H3：身份验证详细代码速查表
  - H2：Gateway 网关服务未运行
  - H2：macOS Gateway 网关悄然停止响应，触碰仪表板后恢复
  - H2：存在重复 Gateway 网关/节点 LaunchAgent 时的 macOS launchd 监督循环
  - H2：Gateway 网关在高内存使用期间退出
  - H2：Gateway 网关拒绝了无效配置
  - H2：Gateway 网关探测警告
  - H2：渠道已连接，但消息未流转
  - H2：Cron 与 Heartbeat 交付
  - H2：节点已配对，但工具失败
  - H2：浏览器工具失败
  - H2：升级后某些功能突然中断
  - H2：相关内容
...
- 标题：
  ...
  - H3：第 2 层：Gateway 网关 + 开发智能体冒烟测试（“@openclaw”的实际作用）
  ...
  - H2：凭据（切勿提交）
  - H2：Deepgram 实时测试（音频转写）
  - H2：BytePlus 编码计划实时测试
...
- 标题：
  ...
  - H2：本地 OpenAI 兼容后端可直接运行，但在 OpenClaw 中失败
...
- 标题：
  - H2：推荐的默认选项（适合大多数用户）
  - H2：macOS 虚拟机选项
  - H3：在你的 Apple 芯片 Mac 上运行本地虚拟机（Lume）
  - H3：托管式 Mac 提供商（云端）
...
- 路由：/install/updating
- 标题：
  - H2：推荐方式：openclaw update
  - H2：在 npm 和 git 安装方式之间切换
  - H2：替代方式：重新运行安装程序
...
- 标题：
  ...
  - H3：启动节点主机（前台）
  - H3：通过 SSH 隧道连接远程 Gateway 网关（local loopback 绑定）
  - H3：启动节点主机（服务）
  - H3：配对 + 命名
  - H3：由节点托管的 MCP 服务器
  - H3：由节点托管的 Skills
...
- 标题：
  ...
  - H2：非目标（v1）
  - H2：先例（我们借鉴什么、反转什么）
  - H2：架构决策：循环在工作节点上运行，推理通过 Gateway 网关进行
  - H2：组件
  - H3：1. 环境状态机 + 提供商契约
  - H3：2. 工作节点引导：在 Box 上安装 OpenClaw
  - H3：3. 传输：一切均通过 SSH
...
- 标题：
  ...
  - H3：openclaw.plugin.json 中生成的元数据已过时
  - H3：package.json 中的 openclaw.extensions 必须包含 ./dist/index.js
  - H3：找不到包 'typebox'
  - H3：安装后未显示工具
  - H2：另请参阅
...
- 标题：
  ...
  - H2：入门指南
  - H2：跨计算机使用 Claude 会话
  - H2：思考默认值（Claude Sonnet 5、Mythos 5、Fable 5、4.8 和 4.6）
...
- 标题：
  ...
  - H2：成本估算（显示时）
  - H2：缓存 TTL 和剪枝的影响
  - H3：示例：使用 Heartbeat 让 1h 缓存保持温热
...
- 标题：
  ...
  - H2：Control UI 的关键规则
  - H2：分层验证
  - H3：第 1 层：验证 Chrome 是否正在 Windows 上提供 CDP 服务
...
- 标题：
  ...
  - H2：内置 Skill 允许列表
  - H2：按 Skill 设置的条目（skills.entries）
  - H2：Agent 允许列表（agents）
  - H2：工作坊（skills.workshop）
  - H2：通过符号链接连接的 Skill 根目录
