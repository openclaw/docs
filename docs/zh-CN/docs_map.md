...
title: 文档地图
x-i18n:
    generated_at: "2026-07-02T22:22:12Z"
    model: gpt-5.5
...
- 标题：
  - H2：概览
  - H2：组件和流程
  - H3：Gateway 网关（daemon）
  - H3：客户端（mac app / CLI / web admin）
  - H3：节点（macOS / iOS / Android / headless）
  - H3：WebChat
  - H2：连接生命周期（单个客户端）
  - H2：线协议（摘要）
  - H2：配对 + 本地信任
...
- 标题：
  ...
  - H3：阶段 2：持久发送核心
  - H3：阶段 3：渠道入站桥接
  - H3：阶段 4：预备 Dispatcher 桥接
  - H3：阶段 5：统一实时生命周期
  - H3：阶段 6：公共 SDK
  - H3：阶段 7：所有发送方
  - H3：阶段 8：移除 Turn-Named 兼容性
  - H2：测试计划
  - H2：开放问题
  - H2：验收标准
  - H2：相关
...
- 标题：
  ...
  - H3：其他内置提供商插件
  - H4：值得了解的特殊行为
  - H2：通过 models.providers 配置提供商（自定义/base URL）
  - H3：Moonshot AI（Kimi）
  - H3：Kimi coding
...
- 标题：
  ...
  - H3：Anthropic API key + MiniMax 后备
  - H3：工作 Bot（受限访问）
  - H3：仅本地模型
  - H2：提示
  - H2：相关
...
- 标题：
  - H2：术语
  - H2：为什么同时保留直连和 SSH
  - H2：设备发现输入（客户端如何得知 Gateway 网关在哪里）
...
- 标题:
  ...
  - H3: tailnet 中持续运行的 Gateway 网关
  - H3: 家用桌面电脑运行 Gateway 网关
...
- 标题:
  ...
  - H3: 锁定 Gateway 网关 WebSocket（本地鉴权）
...
- 标题:
  ...
  - H2: 上游 403 阻止响应
  - H2: 本地 OpenAI 兼容后端直接探测通过，但 agent 运行失败
  - H2: 没有回复
...
- 标题：
  - H2：Live：本地冒烟命令
  - H2：Live：Android 节点能力扫描
  - H2：Live：模型冒烟（配置文件键名）
  - H3：第 1 层：直接模型补全（无 Gateway 网关）
  - H3：第 2 层：Gateway 网关 + dev 智能体冒烟（“@openclaw” 实际执行的内容）
  - H2：Live：CLI 后端冒烟（Claude、Gemini 或其他本地 CLI）
  - H2：Live：APNs HTTP/2 代理可达性
  - H2：Live：ACP 绑定冒烟（/acp spawn ... --bind here）
  - H2：Live：Codex app-server harness 冒烟
  - H3：推荐的 Live 配方
  - H2：Live：模型矩阵（覆盖范围）
  - H3：现代冒烟集（工具调用 + 图像）
  ...
  - H2：凭证（绝不提交）
  - H2：Deepgram Live（音频转写）
  - H2：BytePlus 编码计划 Live
  - H2：ComfyUI 工作流媒体 Live
  - H2：图像生成 Live
  - H2：音乐生成 Live
  - H2：视频生成 Live
  - H2：媒体 Live harness
...
- 标题：
  ...
  - H3：1) 启动 Gateway 网关
  - H3：2) 验证设备发现（可选）
  - H4：Tailnet（Vienna ⇄ London）通过单播 DNS-SD 设备发现
  - H3：3) 从 Android 连接
  - H3：Presence 存活信标
  - H3：4) 批准配对（CLI）
  - H3：5) 验证节点已连接
  - H3：6) 聊天 + 历史记录
  - H3：7) Canvas + 相机
  - H4：Gateway 网关 Canvas Host（推荐用于网页内容）
...
- 标题：
  - H2: 主要旋钮
  - H3: cacheRetention（全局默认值、模型和按 Agent）
...
- 标题：
  ...
  - H3: 解决方案 2：使用带仅附加模式的 Snap Chromium
  - H3: 验证浏览器是否正常工作
  - H3: 配置参考
