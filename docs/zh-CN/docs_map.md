...
- 标题：
  ...
  - H3：其他内置提供商插件
  - H4：值得了解的特殊情况
  - H2：通过 models.providers 的提供商（自定义/基础 URL）
  - H3：Moonshot AI（Kimi）
  - H3：Kimi 编程
...
- 标题：
  - H2：术语
  - H2：为什么同时保留直连和 SSH
  - H2：发现输入（客户端如何知道 Gateway 网关在哪里）
  - H3：1）Bonjour / DNS-SD 发现
  - H4：服务信标详情
  - H3：2）Tailnet（跨网络）
  - H3：3）手动 / SSH 目标
  - H2：传输选择（客户端策略）
...
- 标题：
  ...
  - H2：Rescue-Bot 快速开始
  - H2：为什么这样可行
  - H2：`--profile rescue onboard` 会改变什么
  - H2：通用多 Gateway 网关设置
  - H2：隔离检查清单
  - H2：端口映射（派生）
  - H2：浏览器 / CDP 说明（常见踩坑）
...
- 标题:
  ...
  - H2: 我应该运行哪个套件？
  - H2: 实时（触网）测试
  - H2: Docker 运行器（可选的“可在 Linux 中工作”检查）
  - H2: 文档完整性检查
  - H2: 离线回归（CI 安全）
...
- 标题:
  ...
  - H3: 运行时选择语义不变
  - H2: 实施计划
  - H3: 1. 导出或迁移可复用的 context-engine attempt 辅助函数
  - H3: 2. 添加 Codex context 投影辅助函数
  - H3: 3. 在 Codex 线程启动前接入 bootstrap
  - H3: 4. 在线程 start/resume 和 turn/start 前接入 assemble
...
- 标题：
  ...
  - H3: 3.7 收集和渗出（AML.TA0009，AML.TA0010）
...
- 标题：
  ...
  - H3: 重大换壳（2026 年 1 月 27 日）
  - H3: 最终形态（2026 年 1 月 30 日）
  - H3: 机器人购物狂潮（2025 年 12 月 3 日）
  - H2: 圣典
  - H2: Lobster 信条
...
- 标题:
  ...
  - H3: 解决方案 2：将 Snap Chromium 与仅附加模式配合使用
  - H3: 验证浏览器是否可用
  - H3: 配置参考
...
- 标题:
  ...
  - H2: YOLO 模式（无需审批）
  - H3: 持久化 Gateway 网关主机“never prompt”设置
  - H3: 本地快捷方式
  - H3: Node 主机
  - H3: 仅会话快捷方式
  - H2: 允许列表（按智能体）
