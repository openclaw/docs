...
- 标题：
  ...
  - H2：上游 403 阻止响应
  - H2：本地 OpenAI-compatible 后端通过直接探测，但 Agent 运行失败
  - H2：没有回复
...
- 标题：
  ...
  - H2：CLI 启动和命令分析
  - H2：Gateway 网关监视模式
  - H2：Dev 配置文件 + dev Gateway 网关（--dev）
  - H2：原始流日志（OpenClaw）
...
- 标题：
  ...
  - H3: 运行时选择语义不变
  - H2: 实施计划
  - H3: 1. 导出或迁移可复用的上下文引擎 attempt 辅助工具
  ...
  - H4: 回合内 Codex 原生 contextCompaction 事件
  - H3: 9. 会话重置和绑定行为
  - H3: 10. 错误处理
  - H2: 测试计划
  - H3: 单元测试
...
- 标题：
  - H1：GitHub Copilot agent runtime 插件
  - H2：分发
  - H2：表面
  - H2：相关文档
