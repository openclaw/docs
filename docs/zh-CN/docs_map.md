...
title: 文档地图
x-i18n:
    generated_at: "2026-07-02T00:43:52Z"
    model: gpt-5.5
...
- 标题：
  ...
  - H3: 群聊
  - H2: 群组配置示例
  - H3: 允许所有群组，不需要 @mention
  - H3: 允许所有群组，仍需要 @mention
  - H3: 仅允许指定群组
  - H3: 限制群组内的发送者
  - H2: 获取群组/用户 ID
...
- 标题：
  ...
  - H2: 私有 API 操作
  - H2: 配置写入
  - H2: 合并拆分发送的私信（一次组合中的命令 + URL）
  - H3: 场景和智能体看到的内容
  - H2: bridge 或 Gateway 网关重启后的入站恢复
  - H3: 操作者可见信号
  - H3: 迁移
  - H2: 故障排除
  - H2: 配置参考指针
...
- 标题：
  ...
  - H2：提示词组装 + 系统提示词
  - H2：Hook 点（你可以拦截的位置）
  - H3：内部 hooks（Gateway 网关 hooks）
  - H3：插件 hooks（智能体 + Gateway 网关生命周期）
  - H2：流式传输 + 部分回复
  - H2：工具执行 + 消息工具
  - H2：回复塑形 + 抑制
  - H2：压缩 + 重试
...
- 路由：/gateway/bonjour
- 标题：
  - H2：通过 Tailscale 的广域 Bonjour（单播 DNS-SD）
  - H3：Gateway 网关配置（推荐）
  - H3：一次性 DNS 服务器设置（Gateway 网关主机）
  - H3：Tailscale DNS 设置
  - H3：Gateway 网关监听器安全性（推荐）
  - H2：宣告什么
  - H2：服务类型
  - H2：TXT 键（非机密提示）
  - H2：在 macOS 上调试
...
- 标题：
  ...
  - H2：工作原理
  - H2：会话
  - H2：来自 claude-cli 会话的 fallback 前导内容
  - H2：图像（直通）
  - H2：输入 / 输出
  - H2：默认值（插件拥有）
  - H2：插件拥有的默认值
...
- 标题：
  - H2：术语
  - H2：为什么我们同时保留直连和 SSH
  - H2：设备发现输入（客户端如何得知 Gateway 网关的位置）
...
- 标题：
  ...
  - H3：带自定义 Gateway 网关的按 Agent OpenShell
  - H2：生命周期管理
  - H3：何时重新创建
  - H2：安全加固
  - H2：当前限制
...
- 标题：
  ...
  - H2：Anthropic 429 需要额外用量以支持长上下文
  - H2：上游 403 阻止响应
  - H2：本地 OpenAI 兼容后端通过直接探测，但 Agent 运行失败
  - H2：没有回复
  - H2：Dashboard Control UI 连接
  - H3：凭证详情代码快速映射
  - H2：Gateway 网关服务未运行
  - H2：macOS gateway 静默停止响应，然后在你触摸 dashboard 后恢复
...
- 标题:
  ...
  - H3: 4. 在线程/start、线程/resume 和轮次/start 前接入 assemble
  - H3: 5. 保持提示缓存稳定格式
  - H3: 6. 在转写镜像后接入 post-turn
  - H3: 7. 规范化使用量和提示缓存运行时上下文
  - H3: 8. 压缩策略
...
- 标题：
  - H2：bundle 存在的原因
  - H2：安装 bundle
  - H2：OpenClaw 从 bundle 映射什么
  - H3：当前支持
  - H4：Skill 内容
  - H4：钩子包
  - H4：面向嵌入式 OpenClaw 的 MCP
  - H4：嵌入式 OpenClaw 设置
  - H4：嵌入式 OpenClaw LSP
  - H3：已检测但不执行
  - H2：bundle 格式
...
- 标题：
  - H2: 主要旋钮
  - H3: cacheRetention（全局默认、模型和每 Agent）
...
- 标题：
  ...
  - H2: 压缩：它是什么
  - H2: 压缩分块边界和工具配对
  - H2: 自动压缩何时发生（OpenClaw 运行时）
  - H2: 压缩设置（reserveTokens, keepRecentTokens）
  - H2: 可插拔压缩提供商
  - H2: 用户可见界面
  - H2: 静默内务处理（NOREPLY）
  - H2: 压缩前“memory flush”（已实现）
  - H2: 故障排除清单
  - H2: 相关
...
- 标题：
  ...
  - H2: 成本估算（显示时）
  - H2: 缓存 TTL 和修剪影响
  - H3: 示例：用 Heartbeat 让 1 小时缓存保持预热
...
- 标题：
  ...
  - H4: T-EXFIL-001：通过 webfetch 盗取数据
...
- 标题：
  ...
  - H3: Node exec 管道（最高风险能力）
  - H3: 配对存储（私信门控）
  - H3: 入口门控（提及 + 控制命令绕过）
  - H3: 路由/会话键隔离
  - H2: v1++：其他有界模型（并发、重试、跟踪正确性）
  - H3: 配对存储并发 / 幂等性
  - H3: 入口跟踪关联 / 幂等性
...
- 标题:
  ...
  - H3: 根本原因
  - H3: 方案 1：安装 Google Chrome（推荐）
  - H3: 方案 2：使用 Snap Chromium 的仅附加模式
  - H3: 验证浏览器是否工作
  - H3: 配置参考
