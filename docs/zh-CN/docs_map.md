...
title: 文档地图
x-i18n:
    generated_at: "2026-07-03T09:21:55Z"
    model: gpt-5.5
...
- 标题：
  ...
  - H2：提示词组装 + 系统提示词
  - H2：Hook 点（可以拦截的位置）
  - H3：内部 hook（Gateway 网关 hook）
  - H3：插件 hook（智能体 + Gateway 网关生命周期）
  - H2：流式传输 + 局部回复
  - H2：工具执行 + 消息工具
  - H2：回复塑形 + 抑制
  - H2：压缩 + 重试
...
- 标题：
  ...
  - H3：配置用户时区 + 格式
  - H2：时间格式检测（自动）
  - H2：工具 payload + 连接器（原始提供商时间 + 规范化字段）
  - H2：相关文档
...
- 标题：
  ...
  - H2：工作原理
  - H2：会话
  - H2：来自 `claude-cli` 会话的 fallback 前奏
  - H2：图片（透传）
  - H2：输入 / 输出
  - H2：默认值（插件拥有）
  - H2：插件拥有的默认值
...
- 标题：
  ...
  - H3：推荐入门配置
  - H2：展开示例（主要选项）
  - H3：符号链接的同级 Skills 仓库
  - H2：常见模式
  - H3：带一个覆盖项的共享 Skills 基线
  - H3：多平台设置
  - H3：受信任节点网络自动审批
...
- 标题：
  ...
  - H3：最小远程设置
  - H3：带 GPU 的镜像模式
  - H3：带自定义 Gateway 网关的按智能体 OpenShell
  - H2：生命周期管理
  - H3：何时重新创建
  - H2：安全加固
  - H2：当前限制
...
- 标题：
  ...
  - H3：tailnet 中的常开 Gateway 网关
  - H3：家庭桌面端运行 Gateway 网关
  ...
  - H2：macOS app 远程模式
  - H2：安全规则（远程/VPN）
  - H3：macOS：通过 LaunchAgent 使用持久 SSH 隧道
  - H4：步骤 1：添加 SSH 配置
...
- 标题：
  ...
  - H3：认证详情代码快速映射
  - H2：Gateway 网关服务未运行
  - H2：macOS Gateway 网关静默停止响应，然后在你触碰 dashboard 时恢复
...
- 标题：
  ...
  - H2: 我应该运行哪个套件？
  - H2: 实时（触网）测试
  - H2: Docker 运行器（可选的“在 Linux 中可工作”检查）
  - H2: 文档完整性检查
  - H2: 离线回归（CI 安全）
  - H2: 智能体可靠性评估（Skills）
  - H2: 契约测试（插件和渠道形状）
...
- 标题：
  ...
  - H3：运行时选择语义不变
  - H2：实施计划
  - H3：1. 导出或迁移可复用的上下文引擎尝试帮助程序
  - H3：2. 添加 Codex 上下文投射帮助程序
  - H3：3. 在线程启动前接入引导流程
...
- 标题：
  ...
  - H3：1) 启动 Gateway 网关
  - H3：2) 验证设备发现（可选）
  - H4：Tailnet（维也纳 ⇄ 伦敦）通过单播 DNS-SD 进行设备发现
  - H3：3) 从 Android 连接
  - H3：Presence 存活信标
  - H3：4) 批准配对（CLI）
  - H3：5) 验证节点已连接
...
- 标题:
  ...
  - H3: Node exec 流水线（最高风险能力）
  - H3: 配对存储（私信门控）
  - H3: 入口门控（提及 + 控制命令绕过）
  - H3: 路由/会话键隔离
  - H2: v1++：其他有界模型（并发、重试、轨迹正确性）
  - H3: 配对存储并发 / 幂等性
  - H3: 入口轨迹关联 / 幂等性
...
- 标题:
  ...
  - H3: 2) 将 macOS 应用指向你正在运行的 Gateway 网关
  - H3: 3) 验证
  - H3: 常见坑点
  - H2: 凭据存储映射
  - H2: 更新（不破坏你的设置）
...
- 标题：
  ...
  - H2：Control UI 的关键规则
  - H2：分层验证
  - H3：第 1 层：验证 Chrome 正在 Windows 上提供 CDP
  - H3：第 2 层：验证 WSL2 可以访问该 Windows 端点
...
- 标题：
  ...
  - H2：Personas
  - H3：最小 persona
  - H3：完整 persona（提供商中立提示）
  - H3：Persona 解析
  - H3：提供商如何使用 persona 提示
  - H3：回退策略
  - H2：模型驱动指令
  - H2：斜杠命令
  - H2：按用户偏好
