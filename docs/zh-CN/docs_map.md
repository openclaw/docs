...
summary: 为 OpenClaw 文档页面生成的标题地图
title: 文档地图
x-i18n:
    generated_at: "2026-06-30T13:48:12Z"
    model: gpt-5.5
...
此文件由 `docs/**/*.md` 和 `docs/**/*.mdx` 的标题生成，用于帮助智能体浏览文档树。
不要手动编辑；运行 `pnpm docs:map:gen`。
...
- 标题：
  - H2: 初学者介绍（2 分钟）
  - H2: 可见回复
  - H2: 上下文可见性和 allowlist
  - H2: 会话键
  - H2: 模式：个人私信 + 公开群组（单个智能体）
  - H2: 显示标签
  - H2: 群组策略
  - H2: 提及门控（默认）
  - H2: 作用域配置的提及模式
...
- 标题：
  ...
  - H2: 安全默认值
  - H2: 访问控制
  - H3: 常见陷阱：allowFrom 用于私信，而不是频道
  - H2: 回复触发（提及）
  - H2: 安全说明（推荐用于公共频道）
  - H3: 频道中的每个人使用相同工具
  - H3: 每个发送者使用不同工具（所有者拥有更多权限）
  - H2: NickServ
  - H2: 环境变量
  - H2: 故障排除
  - H2: 相关内容
...
- 标题：
  ...
  - H3: 第 2 步：获取凭证
  - H3: 第 3 步：配置消息端点
  - H3: 第 4 步：启用 Teams 频道
  - H3: 第 5 步：构建 Teams App Manifest
...
- 标题：
  ...
  - H3: 通过 Telegram 配对（推荐用于 iOS）
  - H3: 批准节点设备
  - H3: 可选的可信 CIDR 节点自动批准
  - H3: 节点配对状态存储
  - H3: 说明
  - H2: 相关文档
...
- 路由：/channels/slack
- 标题：
  - H2: 选择 Socket Mode 或 HTTP 请求 URL
  - H3: 中继模式
  - H2: 安装
  - H2: 快速设置
  - H2: Socket Mode 传输调优
...
- 标题：
  ...
  - H2: 它是什么
  - H2: 设置（快速路径）
  - H3: 1) 创建 bot 令牌（Zalo Bot Platform）
  - H3: 2) 配置令牌（环境变量或配置）
  - H2: 工作原理（行为）
  - H2: 限制
  - H2: 访问控制（私信）
...
- 标题:
  ...
  - H2: 配置键（概览）
  - H3: 安全的允许列表编辑
  - H2: “Model is not allowed”（以及为什么回复会停止）
  - H2: 在聊天中切换模型（/model）
  - H2: CLI 命令
  - H3: models list
  - H3: models status
...
- 标题:
  ...
  - H2: Anthropic：Claude CLI 和 token 兼容性
  - H2: Anthropic 说明
  - H2: 检查模型认证状态
...
- 路由: /gateway/bonjour
- 标题:
  - H2: 通过 Tailscale 的广域 Bonjour（Unicast DNS-SD）
  - H3: Gateway 网关配置（推荐）
...
- 标题:
  - H2: 术语
  - H2: 为什么同时保留直连和 SSH
  - H2: 设备发现输入（客户端如何知道 Gateway 网关位置）
...
- 标题:
  ...
  - H2: Rescue-Bot 快速开始
  - H2: 为什么这样可行
  - H2: `--profile rescue onboard` 的更改内容
  - H2: 通用多 Gateway 网关设置
  - H2: 隔离检查清单
  - H2: 端口映射（派生）
  - H2: 浏览器/CDP 说明（常见坑）
...
- 标题:
  ...
  - H3: 最小远程设置
  - H3: 带 GPU 的镜像模式
  - H3: 使用自定义 Gateway 网关的每 Agent OpenShell
  - H2: 生命周期管理
  - H3: 何时重新创建
  - H2: 安全加固
  - H2: 当前限制
...
- 标题:
  ...
  - H3: 角色
  - H3: 作用域（操作员）
  - H3: Caps/commands/permissions（节点）
  - H2: 在线状态
  - H3: 节点后台存活事件
  - H2: 广播事件作用域
  - H2: 常见 RPC 方法族
...
- 标题:
  ...
  - H3: 第 1 步：添加 SSH 配置
  - H3: 第 2 步：复制 SSH 密钥
  - H3: 第 3 步：配置远程 Gateway 网关身份验证
  - H3: 第 4 步：启动 SSH 隧道
  - H3: 第 5 步：重启 OpenClaw.app
  - H2: 登录时自动启动隧道
  - H3: 创建 PLIST 文件
  - H3: 加载 Launch Agent
  - H2: 故障排除
...
- 标题：
  ...
  - H3：tailnet 中的常驻 Gateway 网关
  - H3：家用桌面端运行 Gateway 网关
...
- 标题：
  - H2：先确定范围：个人助理安全模型
  - H2：快速检查：openclaw security audit
  - H3：已发布包依赖锁定
  - H3：部署和主机信任
  - H3：安全文件操作
  ...
  - H2：本地会话日志存储在磁盘上
  - H2：节点执行（system.run）
  - H2：动态 Skills（watcher / 远程节点）
  - H2：威胁模型
  - H2：核心概念：智能之前先做访问控制
  - H2：命令授权模型
  - H2：控制平面工具风险
  - H2：插件
  - H2：私信访问模型：配对、allowlist、开放、禁用
  - H2：私信会话隔离（多用户模式）
  - H3：安全私信模式（推荐）
  - H2：私信和群组的 allowlist
  - H2：提示注入（它是什么，为什么重要）
  ...
  - H3：锁定 Gateway 网关 WebSocket（本地凭证）
  ...
  - H3：示例：完全访问（无沙箱）
  - H3：示例：只读工具 + 只读工作区
  - H3：示例：无文件系统/shell 访问（允许提供商消息传递）
  - H2：事件响应
  - H3：遏制
  - H3：轮换（如果密钥泄漏，假定已被攻破）
  - H3：审计
...
- 标题：
  ...
  - H2：上游 403 阻止的响应
  - H2：本地 OpenAI 兼容后端直接探测通过但 Agent 运行失败
  - H2：没有回复
  - H2：Dashboard Control UI 连接
  - H3：凭证详情代码快速映射
  - H2：Gateway 网关服务未运行
  - H2：macOS gateway 静默停止响应，然后在你触碰 dashboard 时恢复
...
- 标题：
  ...
  - H2：插件生命周期跟踪
  - H2：CLI 启动和命令性能分析
  - H2：Gateway 网关 watch 模式
  - H2：开发配置文件 + 开发 gateway（--dev）
  - H2：原始流日志（OpenClaw）
  - H2：原始 OpenAI 兼容 chunk 日志
  - H2：安全说明
  - H2：在 VSCode 中调试
  - H3：设置
  - H3：说明
...
- 标题：
  ...
  - H2：环境变量和 .env 加载
  - H2：会话和多个聊天
  - H2：Models、故障转移和凭证配置文件
  - H2：Gateway 网关：端口、“already running”和远程模式
  - H2：日志和调试
  - H2：媒体和附件
  - H2：安全和访问控制
  - H2：聊天命令、中止任务，以及“它不会停止”
  - H2：杂项
  - H2：相关
...
- 标题:
  ...
  - H2: 我应该运行哪个套件？
  - H2: 实时（触网）测试
  - H2: Docker 运行器（可选“在 Linux 中工作”检查）
  - H2: 文档完整性检查
  - H2: 离线回归（CI 安全）
...
- 标题：
  ...
  - H2：期望行为
  - H2：设计约束
  - H3：Codex app-server 仍是原生线程状态的权威来源
  - H3：Context engine 组装必须投射到 Codex 输入中
  - H3：Prompt-cache 稳定性很重要
  - H3：运行时选择语义不变
  - H2：实现计划
  - H3：1. 导出或迁移可复用的 context-engine attempt 辅助工具
  - H3：2. 添加 Codex 上下文投射辅助工具
  - H3：3. 在线程启动前接入 bootstrap
  - H3：4. 在线程/启动、线程/恢复和轮次/启动前接入 assemble
  - H3：5. 保持 prompt-cache 稳定格式
...
- 标题：
  ...
  - H3：1) 启动 Gateway 网关
  - H3：2) 验证设备发现（可选）
  - H4：Tailnet（Vienna ⇄ London）通过单播 DNS-SD 发现
  - H3：3) 从 Android 连接
  - H3：在线状态保活信标
  - H3：4) 批准配对（CLI）
  - H3：5) 验证节点已连接
...
- 标题:
  ...
  - H3: 浏览器已打开但 Agent 无法加入
  - H3: 会议创建失败
  - H3: Agent 已加入但不说话
  - H3: Twilio 设置检查失败
  - H3: Twilio 通话已开始但从未进入会议
  - H2: 备注
  - H2: 相关
...
- 标题:
  ...
  - H2: 降级规则
  - H2: 提供商映射
  - H2: Presentation 与 InteractiveReply 对比
  - H2: 投递固定
  - H2: 插件作者清单
  - H2: 相关文档
...
- 标题：
  - H2: 主要旋钮
  - H3: cacheRetention（全局默认值、模型和每 Agent）
...
- 标题：
  ...
  - H2: 压缩：它是什么
  - H2: 压缩块边界和工具配对
  - H2: 自动压缩发生时间（OpenClaw 运行时）
  - H2: 压缩设置（reserveTokens、keepRecentTokens）
  - H2: 可插拔压缩提供商
  - H2: 用户可见表面
  - H2: 静默 housekeeping（NOREPLY）
  - H2: 压缩前“记忆刷新”（已实现）
  - H2: 故障排除清单
  - H2: 相关
...
- 标题:
  ...
  - H2: 成本估算（显示时）
  - H2: 缓存 TTL 和裁剪影响
  - H3: 示例：用 heartbeat 保持 1 小时缓存温热
  - H3: 示例：混合流量与按 Agent 的缓存策略
  - H3: Anthropic 1M 上下文
  - H2: 降低 token 压力的技巧
  - H2: 相关
...
- 标题:
  ...
  - H3: 3.7 收集和外传（AML.TA0009, AML.TA0010）
...
- 标题:
  ...
  - H2: 重要注意事项
  - H2: 复现结果
  - H3: Gateway 网关暴露和开放 Gateway 网关配置错误
  - H3: Node exec 管线（最高风险能力）
  - H3: 配对存储（私信门禁）
  - H3: 入口门禁（提及 + 控制命令绕过）
  - H3: 路由/会话键隔离
  - H2: v1++：额外有界模型（并发、重试、跟踪正确性）
  - H3: 配对存储并发/幂等性
  - H3: 入口跟踪关联/幂等性
...
- 标题：
  - H2：问题：“Failed to start Chrome CDP on port 18800”
  - H3：根本原因
  - H3：解决方案 1：安装 Google Chrome（推荐）
  - H3：解决方案 2：使用带 Attach-Only Mode 的 Snap Chromium
  - H3：验证浏览器是否正常工作
  - H3：配置参考
  - H3：问题：“No Chrome tabs found for profile="user"”
  - H2：相关
...
- 标题：
  ...
  - H3：当审批发送到某个会话时，该会话中的任何人都能批准它们吗？
  - H2：相关
...
- 标题：
  ...
  - H2：YOLO 模式（无审批）
  - H3：持久 Gateway 网关主机“never prompt”设置
  - H3：本地快捷方式
  - H3：Node 主机
  - H3：仅会话快捷方式
  - H2：允许列表（每个智能体）
