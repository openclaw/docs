...
title: 文档地图
x-i18n:
    generated_at: "2026-07-04T15:07:47Z"
    model: gpt-5.5
...
- 标题：
  ...
  - H3：其他内置提供商插件
  - H4：值得了解的特殊行为
  - H2：通过 models.providers 的提供商（自定义/base URL）
  - H3：Moonshot AI（Kimi）
  - H3：Kimi coding
...
- 标题：
  ...
  - H2：上游 403 阻止响应
  - H2：本地 OpenAI 兼容后端直接探测通过但智能体运行失败
  - H2：无回复
...
- 标题：
  ...
  - H3：1) 启动 Gateway 网关
  - H3：2) 验证设备发现（可选）
  - H4：通过单播 DNS-SD 进行 Tailnet（Vienna ⇄ London）发现
  - H3：3) 从 Android 连接
  - H3：Presence 存活信标
  - H3：4) 批准配对（CLI）
  - H3：5) 验证节点已连接
