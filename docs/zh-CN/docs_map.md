...
title: 文档地图
x-i18n:
    generated_at: "2026-07-04T10:26:39Z"
    model: gpt-5.5
...
- 标题：
  ...
  - H2: 实时：模型冒烟测试（profile 键）
  - H3: 第 1 层：直接模型补全（无 Gateway 网关）
  - H3: 第 2 层：Gateway 网关 + dev agent 冒烟测试（“@openclaw” 实际执行的内容）
...
- 标题:
  ...
  - H2: 自动检测（默认）
  - H2: 配置示例
  - H3: 提供商 + CLI 后备（OpenAI + Whisper CLI）
  - H3: 仅提供商并使用作用域门控
  - H3: 仅提供商（Deepgram）
...
- 标题:
  ...
  - H3: 7. 规范化使用量和 prompt-cache 运行时上下文
  - H3: 8. 压缩策略
...
- 标题：
  ...
  - H2: 构建 UI
  - H2: 空白 Control UI 页面
  - H2: 调试/测试：dev server + 远程 Gateway 网关
  - H2: 相关
