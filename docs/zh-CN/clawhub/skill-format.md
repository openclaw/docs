...
Web GitHub 导入器比本地发布/同步更严格。它只会发现已登录 GitHub 账号拥有的公开、非 fork 仓库中的
...
- 如果你的技能集成了付费第三方服务，请在技能说明和环境变量声明中清楚记录外部费用和所需账号（对必需变量使用 `requires.env`，或对可选变量使用带有 `required: false` 的 `envVars`）。
