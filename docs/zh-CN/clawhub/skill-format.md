...
x-i18n:
    generated_at: "2026-07-02T13:57:52Z"
    model: gpt-5.5
...
网页版 GitHub 导入器比本地发布/同步更严格。它只会发现由已登录 GitHub 账号拥有的公开、非 fork 仓库中的 `SKILL.md` 或旧版 `skills.md` 文件。它不会导入私有仓库、fork、已归档/已禁用仓库，或第三方公开仓库。
...
ClawHub 的安全分析会检查你的技能声明的内容是否与实际行为匹配。如果你的代码引用了 `TODOIST_API_KEY`，但你的 frontmatter 没有在 `requires.env`、`primaryEnv` 或 `envVars` 下声明它，分析会标记为元数据不匹配。保持声明准确有助于你的技能通过审核，也能帮助用户理解他们正在安装什么。
...
- 在 ClawHub 上发布的所有技能均使用 `MIT-0` 许可。
...
- 不要向 `SKILL.md` 添加价格元数据；它不是技能格式的一部分，也不会让已发布技能变为付费技能。
- 如果你的技能集成了付费第三方服务，请在技能说明和环境变量声明中清楚记录外部成本和所需账号（必需变量用 `requires.env`，可选变量用带有 `required: false` 的 `envVars`）。
