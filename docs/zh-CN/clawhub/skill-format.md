...
使用 `requires.env` 声明技能运行前必须存在的环境变量。当你需要逐变量元数据时，请使用 `envVars`，包括带有 `required: false` 的可选变量。
...
- 默认从文件夹名称派生。
- 包 scope 必须与 ClawHub 发布者句柄完全匹配。发布者句柄可以使用小写字母、数字、连字符、点和下划线；必须以小写字母或数字开头和结尾。
- 包 slug 必须是小写且 npm-safe，例如 `@example.tools/demo-plugin` 或 `demo-plugin`。
...
- 在 ClawHub 上发布的所有技能都使用 `MIT-0` 许可。
...
- 不要向 `SKILL.md` 添加价格元数据；它不是技能格式的一部分，也不会让已发布的技能变为付费。
- 如果你的技能集成了付费第三方服务，请在技能说明和环境变量声明中清楚记录外部费用和所需账号（必填变量使用 `requires.env`，可选变量使用带有 `required: false` 的 `envVars`）。
