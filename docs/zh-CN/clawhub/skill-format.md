...
x-i18n:
    generated_at: "2026-07-01T18:07:33Z"
    model: gpt-5.5
...
Web 版 GitHub 导入器比本地发布/同步更严格。它只会发现已登录 GitHub 账户拥有的公开、非 fork 仓库中的
...
- Markdown，可带可选的 YAML 前置元数据。
- 服务器会在发布期间从前置元数据中提取元数据。
- `description` 会作为 UI/搜索中的技能摘要。

## 前置元数据

技能元数据在你的 `SKILL.md` 顶部的 YAML 前置元数据中声明。这会告诉注册表（以及安全分析）你的技能运行时需要什么。
...
对于技能运行前必须存在的环境变量，使用 `requires.env`。当你需要逐变量元数据时使用 `envVars`，包括带有 `required: false` 的可选变量。
...
ClawHub 的安全分析会检查你的技能声明的内容是否与它实际执行的操作一致。如果你的代码引用了 `TODOIST_API_KEY`，但你的前置元数据没有在 `requires.env`、`primaryEnv` 或 `envVars` 下声明它，分析会标记元数据不匹配。保持声明准确有助于你的技能通过审核，也能帮助用户理解他们正在安装什么。
...
- 如果你的技能集成了付费第三方服务，请在技能说明和环境变量声明中清楚记录外部成本和所需账户（必需变量使用 `requires.env`，可选变量使用带有 `required: false` 的 `envVars`）。
