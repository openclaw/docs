...
- 如果你的技能集成了付费第三方服务，请在技能说明和环境变量声明中清楚记录外部成本和所需账户（对必需变量使用 `requires.env`，或对可选变量使用带 `required: false` 的 `envVars`）。
