...
x-i18n:
    generated_at: "2026-05-13T02:52:00Z"
    model: gpt-5.5
    provider: openai
...
技能元数据在你的 `SKILL.md` 顶部的 YAML frontmatter 中声明。它会告诉注册表（以及安全分析）你的技能运行需要什么。
...
| `primaryEnv`       | `string`   | 你的技能的主要凭据环境变量。                                                                                                                |
...
ClawHub 的安全分析会检查你的技能声明的内容是否与它实际执行的操作匹配。如果你的代码引用 `TODOIST_API_KEY`，但你的 frontmatter 没有在 `requires.env`、`primaryEnv` 或 `envVars` 下声明它，分析就会标记元数据不匹配。保持声明准确有助于你的技能通过审核，也有助于用户理解他们正在安装什么。
...
- 如果你的技能集成了付费第三方服务，请在技能说明和环境声明中清晰记录外部成本和所需账户（必需变量使用 `requires.env`，可选变量使用带有 `required: false` 的 `envVars`）。
