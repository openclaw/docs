...
x-i18n:
    generated_at: "2026-07-01T20:11:55Z"
    model: gpt-5.5
...
已登录 GitHub 账户拥有的公开、非 fork 仓库中的
...
对技能运行前必须存在的环境变量使用 `requires.env`。当你需要按变量提供元数据时，请使用 `envVars`，包括带有 `required: false` 的可选变量。
...
| `primaryEnv`       | `string`   | 你的技能使用的主凭证环境变量。                                                                                                      |
| `envVars`          | `array`    | 包含 `name`、可选 `required` 和可选 `description` 的环境变量声明。对可选环境变量设置 `required: false`。 |
...
在 `metadata.openclaw.envVars` 下声明可选环境变量，并设置 `required: false`。不要把可选条目添加到 `requires.env`，因为 `requires.env` 表示技能没有它们就无法运行。
...
ClawHub 的安全分析会检查你的技能声明的内容是否与它实际执行的操作匹配。如果你的代码引用了 `TODOIST_API_KEY`，但你的 frontmatter 没有在 `requires.env`、`primaryEnv` 或 `envVars` 下声明它，分析会标记元数据不匹配。保持声明准确有助于你的技能通过审核，也能帮助用户理解他们正在安装的内容。
...
- 默认从文件夹名称派生。
- 包作用域必须与 ClawHub 发布者句柄完全匹配。发布者句柄可以使用小写字母、数字、连字符、点和下划线；它们必须以小写字母或数字开头和结尾。
- 包 slug 必须为小写且对 npm 安全，例如 `@example.tools/demo-plugin` 或 `demo-plugin`。

## 版本控制 + 标签

- 每次发布都会创建一个新版本（semver）。
- 标签是指向某个版本的字符串指针；常用的是 `latest`。
...
- 不要向 `SKILL.md` 添加定价元数据；它不是技能格式的一部分，也不会让已发布技能变为付费技能。
- 如果你的技能集成了付费第三方服务，请在技能说明和环境变量声明中清楚记录外部费用和所需账户（必需变量使用 `requires.env`，可选变量使用带有 `required: false` 的 `envVars`）。
