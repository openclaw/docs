...
read_when:
    - 发布 Skills
    - 调试发布失败
summary: Skills 文件夹格式、必需文件、允许的文件类型、限制。
x-i18n:
    generated_at: "2026-07-02T17:30:48Z"
    model: gpt-5.5
...
- 任意辅助性的_文本类_文件（见“允许的文件”）
- `.clawhubignore`（用于发布的忽略模式，旧版 `.clawdhubignore`）
- `.gitignore`（同样会被遵循）

## GitHub 导入

Web GitHub 导入器比本地发布/同步更严格。它只会发现已登录 GitHub 账户拥有的公开、非 fork 仓库中的 `SKILL.md` 或旧版 `skills.md` 文件。它不会导入私有仓库、fork、已归档/禁用的仓库，或第三方公开仓库。
...
- 发布期间，服务器会从 frontmatter 中提取元数据。
- `description` 会作为 UI/搜索中的技能摘要。
...
在 `metadata.openclaw.envVars` 下声明可选环境变量，并设置 `required: false`。不要把可选条目添加到 `requires.env`，因为 `requires.env` 表示没有它们技能就无法运行。
...
ClawHub 的安全分析会检查你的技能声明的内容是否与它实际执行的内容匹配。如果你的代码引用了 `TODOIST_API_KEY`，但你的 frontmatter 没有在 `requires.env`、`primaryEnv` 或 `envVars` 下声明它，分析就会标记元数据不匹配。保持声明准确有助于你的技能通过审核，也有助于用户理解他们正在安装什么。
...
- 包 slug 必须为小写并且 npm 安全，例如 `@example.tools/demo-plugin` 或 `demo-plugin`。
...
- 如果你的技能集成了付费第三方服务，请在技能说明和环境变量声明中清楚记录外部成本和所需账户（必需变量使用 `requires.env`，可选变量使用带 `required: false` 的 `envVars`）。
