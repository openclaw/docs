...
Web GitHub 导入器比本地发布/同步更严格。它只会发现
...
对技能运行前必须存在的环境变量使用 `requires.env`。当你需要按变量提供元数据时使用 `envVars`，包括带有 `required: false` 的可选变量。
...
| `always`           | `boolean`  | 如果为 `true`，技能会始终处于活跃状态（无需显式安装）。                                                                                      |
...
ClawHub 的安全分析会检查你的技能声明的内容是否与它实际执行的操作匹配。如果你的代码引用了 `TODOIST_API_KEY`，但你的 frontmatter 没有在 `requires.env`、`primaryEnv` 或 `envVars` 下声明它，分析会标记元数据不匹配。保持声明准确有助于你的技能通过审核，也能帮助用户了解他们正在安装什么。
...
- 如果你的技能集成了付费第三方服务，请在技能说明和环境变量声明中清楚记录外部成本和所需账号（必需变量使用 `requires.env`，可选变量使用带 `required: false` 的 `envVars`）。
