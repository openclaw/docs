...
x-i18n:
    generated_at: "2026-07-03T23:26:05Z"
    model: gpt-5.5
...
- 任意支持性的_基于文本_的文件（参见“允许的文件”）
- `.clawhubignore`（发布时的忽略模式，旧版 `.clawdhubignore`）
- `.gitignore`（同样会被遵循）

## GitHub 导入

网页端 GitHub 导入器比本地发布/同步更严格。它只会在已登录 GitHub 账号拥有的公开、非 fork 仓库中发现 `SKILL.md` 或旧版 `skills.md` 文件。它不会导入私有仓库、fork、已归档/已禁用的仓库，或第三方公开仓库。
...
对 Skill 运行前必须存在的环境变量使用 `requires.env`。当你需要每个变量的元数据时使用 `envVars`，包括带 `required: false` 的可选变量。
...
| `requires.config`  | `string[]` | 你的 Skill 会读取的配置文件路径。                                                                                                      |
| `primaryEnv`       | `string`   | 你的 Skill 的主凭证环境变量。                                                                                                          |
| `envVars`          | `array`    | 环境变量声明，包含 `name`、可选 `required` 和可选 `description`。对可选环境变量设置 `required: false`。                                |
| `always`           | `boolean`  | 如果为 `true`，Skill 始终处于激活状态（无需显式安装）。                                                                                |
...
| `nix`              | `object`   | Nix 插件规格（参见 README）。                                                                                                          |
| `config`           | `object`   | Clawdbot 配置规格（参见 README）。                                                                                                     |
...
在 `metadata.openclaw.envVars` 下声明可选环境变量，并设置 `required: false`。不要把可选条目添加到 `requires.env`，因为 `requires.env` 表示没有它们 Skill 就无法运行。
...
ClawHub 的安全分析会检查你的 Skill 声明的内容是否与它实际执行的内容匹配。如果你的代码引用 `TODOIST_API_KEY`，但你的 frontmatter 没有在 `requires.env`、`primaryEnv` 或 `envVars` 下声明它，分析会标记元数据不匹配。保持声明准确有助于你的 Skill 通过审核，也能帮助用户了解他们正在安装什么。
...
- 默认从文件夹名称派生。
- Package scopes 必须与 ClawHub 发布者 handle 完全匹配。发布者 handle 可以使用小写字母、数字、连字符、点和下划线；必须以小写字母或数字开头和结尾。
- Package slug 必须为小写且 npm-safe，例如 `@example.tools/demo-plugin` 或 `demo-plugin`。
...
- 在 ClawHub 上发布的所有 Skills 都按 `MIT-0` 授权。
...
- ClawHub 不支持付费 Skills、按 Skill 定价、付费墙或收益分成。
- 不要向 `SKILL.md` 添加定价元数据；它不是 Skill 格式的一部分，也不会让已发布的 Skill 变成付费。
- 如果你的 Skill 集成了付费第三方服务，请在 Skill 说明和环境变量声明中清楚记录外部成本和所需账号（必需变量用 `requires.env`，可选变量用带 `required: false` 的 `envVars`）。
