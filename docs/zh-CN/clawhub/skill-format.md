...
x-i18n:
    generated_at: "2026-06-30T22:05:32Z"
    model: gpt-5.5
...
ClawHub 的安全分析会检查你的技能声明的内容是否与它实际执行的内容一致。如果你的代码引用了 `TODOIST_API_KEY`，但 frontmatter 没有在 `requires.env`、`primaryEnv` 或 `envVars` 下声明它，分析会标记元数据不匹配。保持声明准确有助于你的技能通过审核，也有助于用户理解他们正在安装什么。
...
- 如果你的技能集成了付费第三方服务，请在技能说明和环境声明中清楚记录外部费用和所需账号（必需变量使用 `requires.env`，或对可选变量使用带 `required: false` 的 `envVars`）。
