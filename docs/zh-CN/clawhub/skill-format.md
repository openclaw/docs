...
- `.clawhubignore`（用于发布/同步的忽略模式，旧版 `.clawdhubignore`）
- `.gitignore`（同样会被遵循）
...
Skill 元数据在你的 `SKILL.md` 顶部的 YAML frontmatter 中声明。这会告诉注册表（以及安全分析）你的技能运行时需要什么。
...
对技能运行前必须存在的环境变量使用 `requires.env`。当你需要每个变量的元数据时使用 `envVars`，包括带 `required: false` 的可选变量。
...
在 `metadata.openclaw.envVars` 下声明可选环境变量，并设置 `required: false`。不要把可选条目添加到 `requires.env`，因为 `requires.env` 表示技能没有这些变量就无法运行。
...
ClawHub 的安全分析会检查你的技能声明内容是否与其实际行为匹配。如果你的代码引用 `TODOIST_API_KEY`，但你的 frontmatter 没有在 `requires.env`、`primaryEnv` 或 `envVars` 下声明它，分析会标记元数据不匹配。保持声明准确有助于你的技能通过审核，也有助于用户理解他们正在安装什么。
...
- 总 bundle 大小：50MB。
- Embedding 文本包含 `SKILL.md` + 最多约 40 个非 `.md` 文件（尽力上限）。

## Slugs

- 默认从文件夹名称派生。
- 必须是小写且 URL 安全：`^[a-z0-9][a-z0-9-]*$`。
...
- ClawHub 上发布的所有 Skills 都采用 `MIT-0` 许可。
...
- 不要向 `SKILL.md` 添加定价元数据；它不是 Skill 格式的一部分，也不会让已发布的技能变成付费技能。
- 如果你的技能集成了付费第三方服务，请在技能说明和环境变量声明中清楚记录外部费用和所需账户（必需变量用 `requires.env`，可选变量用带 `required: false` 的 `envVars`）。
