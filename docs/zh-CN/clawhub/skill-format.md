...
ClawHub 的安全分析会检查你的技能声明的内容是否与它实际执行的操作匹配。如果你的代码引用了 `TODOIST_API_KEY`，但你的 frontmatter 没有在 `requires.env`、`primaryEnv` 或 `envVars` 下声明它，分析会标记为元数据不匹配。保持声明准确有助于你的技能通过审查，也有助于用户理解他们正在安装什么。
...
- 默认从文件夹名称派生。
- 包作用域必须与 ClawHub 发布者 handle 完全匹配。发布者 handle 可以使用小写字母、数字、连字符、点和下划线；必须以小写字母或数字开头和结尾。
...
- 如果你的技能集成了付费第三方服务，请在技能说明和环境变量声明中清晰记录外部成本和必需账号（对必需变量使用 `requires.env`，或对可选变量使用带 `required: false` 的 `envVars`）。
