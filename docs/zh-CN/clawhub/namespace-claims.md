...
summary: 如何针对组织、品牌、所有者句柄、包作用域、技能短标识或命名空间所有权争议请求 ClawHub 审核。
title: 组织和命名空间声明
x-i18n:
...
ClawHub 使用所有者 handle、组织 handle、技能 slug、插件包名称和包作用域作为公共命名空间。如果某个命名空间看起来属于现实世界中的项目、品牌、包生态系统或组织，但在 ClawHub 上已经被声明、保留、具有误导性或存在争议，请通过
...
当你认为 ClawHub 工作人员应审核某个命名空间是否因现实世界所有权而需要被保留、转让、重命名、隐藏、隔离、添加别名或以其他方式更改时，请发起命名空间声明。

示例包括：

- 与你的 GitHub 组织、项目、公司或社区匹配的组织 handle
- 诸如 `@example-org/*` 这样的包作用域，它应只允许匹配的 ClawHub 所有者发布
...
首先确认你正在使用与命名空间匹配的所有者进行发布。对于插件包，诸如 `@example-org/example-plugin` 这样的作用域名称必须以匹配的 `example-org` 所有者发布。
...
命名空间审核不保证每个匹配的名称都会被转让。工作人员会权衡公开证据、现有使用情况、安全风险和用户影响。
