...
summary: 如何为组织、品牌、所有者账号名、包作用域、技能标识段或命名空间所有权争议请求 ClawHub 审核。
title: 组织和命名空间声明
x-i18n:
...
现实中的项目、品牌、包生态系统或组织，但已经在 ClawHub 上被
声明、保留、误导性使用或存在争议，请通过
...
现实所有权而被保留、转让、重命名、隐藏、隔离、添加别名
或以其他方式更改时，请提交命名空间声明。

示例包括：

- 与你的 GitHub 组织、项目、公司或社区匹配的组织 handle
- 像 `@example-org/*` 这样的包 scope，它应只由匹配的
  ClawHub 所有者发布
- 看起来在冒充某个项目的技能 slug 或插件包名
- 品牌、商标、项目重命名或包历史争议
- 被删除、不活跃或无法联系的所有者阻塞了 rightful 命名空间
  所有者

如果该条目除所有权争议外还存在不安全、恶意或误导性问题，
也请遵循相关的审核或安全指南。命名空间声明
表单用于所有权审核，而不是紧急漏洞披露。

## 提交前

先确认你正使用与命名空间匹配的所有者发布。
...
- 命名该命名空间的官方项目文档
- 域名或官方电子邮件域名证明
- npm、PyPI、crates.io 或其他包注册表的 scope 控制权
- 可公开讨论的商标、品牌或项目所有权证据
- 源仓库历史、包历史或公开重命名通知
- 指向有争议的 ClawHub 所有者、技能、插件、包或 issue 的链接

说明每个链接证明了什么。工作人员应能在不需要私有凭据
或密钥的情况下理解其关系。
