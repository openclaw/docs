...
在阻止直接出站连接的系统上，这是必需的（例如 Docker 容器、仅允许代理联网的 Hetzner VPS、企业防火墙）。
...
- 调用 `/api/v1/search?q=...`。
- 搜索会先偏好精确的 slug/name token 匹配，再考虑下载热度。像 `map` 这样的独立 slug token 对 `personal-map` 的匹配强度高于对 `amap` 内部子字符串的匹配。
- 下载量只是一个小的热度先验，并不保证排在首位。
...
- 固定的技能会被 `update --all` 跳过，并被直接 `update <slug>` 拒绝。
- 固定的技能也会拒绝 `install --force`，避免本地字节内容被意外替换。
...
- 当执行者有发布者访问权限时，`--owner <handle>` 会在组织/用户发布者 handle 下发布。
...
- 转移到组织/发布者 handle 仅在执行者同时拥有当前所有者和目标发布者的 admin 访问权限时立即生效。
- 子命令：
...
- 将它用于插件和其他软件包系列条目；顶层 `search` 仍然是技能搜索界面。
- 标志：
...
- 将包转让给另一个发布者。
- 需要对当前包所有者和目标发布者都拥有管理员访问权限，
  除非由平台管理员执行。
- 带作用域的包名必须转让给匹配的作用域所有者。
...
- 显示面向运维人员的迁移状态，用于可能替换
  内置 OpenClaw 插件的包。
- 调用与 `package readiness` 相同的计算型就绪端点，但会打印
  侧重迁移的状态、最新版本、官方包状态、检查项和
  阻碍项。
- 标志：
  - `--json`：机器可读输出。
...
- 元数据会从 `package.json`、`openclaw.plugin.json` 和
  真实 OpenClaw bundle 标记自动检测，例如 `.codex-plugin/plugin.json`、
  `.claude-plugin/plugin.json` 和 `.cursor-plugin/plugin.json`。
- `.tgz` 源会被视为 ClawPack。CLI 会上传精确的 npm-pack
  字节，并且仅使用提取出的 `package/` 内容进行验证和
  元数据预填。
- 代码插件文件夹会在上传前打包为 ClawPack npm tarball，
  以便 OpenClaw 安装可以验证精确构件。bundle 插件文件夹仍然
  使用提取文件发布路径。
- 对于 GitHub 源，源归因会从仓库、解析后的提交、ref 和子路径自动填充。
- 对于本地文件夹，当 origin 远程指向 GitHub 时，会从本地 git 自动检测源归因。
...
- `--clawscan-note <text>` 添加 ClawScan 备注。该备注为 ClawScan
  提供上下文，用于解释可能看起来不寻常的行为，例如网络访问、
...
先使用 `--dry-run`，这样你可以在创建实时发布版本前确认解析后的包元数据和
源归因：
...
- `pull_request` 应使用 `dry_run: true`，以便 CI 保持不产生污染。
- 真实发布应限制在可信事件中，例如 `workflow_dispatch` 或标签推送。
- 无需 secret 的可信发布仅适用于 `workflow_dispatch`；标签推送仍然需要 `clawhub_token`。
...
- 根目录可以是任意文件夹：一个技能目录，或一个包含 `SKILL.md` 的单个技能文件夹。
