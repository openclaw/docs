...
设置其中任一变量后，CLI 会通过指定代理路由出站请求。`HTTPS_PROXY` 用于 HTTPS 请求，`HTTP_PROXY` 用于普通 HTTP。`NO_PROXY` / `no_proxy` 会被遵循，用于绕过特定主机或域名的代理。
...
- 调用 `/api/v1/search?q=...`。
- 搜索会优先匹配精确的 slug/name 词元，然后才考虑下载热度。像 `map` 这样的独立 slug 词元会比 `amap` 内部的子字符串更强地匹配 `personal-map`。
- 下载量只是一个很小的人气先验，不保证排在最前。
- 如果某个技能应该出现却没有出现，请在登录状态下运行 `clawhub inspect <slug>`，在重命名元数据前检查所有者可见的审核诊断信息。
...
- Pinned 技能会被 `update --all` 跳过，并会被直接 `update <slug>` 拒绝。
- Pinned 技能也会拒绝 `install --force`，以免本地字节被意外替换。

### `unpin <slug>`

- 从已安装的技能中移除锁文件 pin，以便后续更新可以修改它。
...
- `update <slug>` 对 pinned slug 会快速失败，并提示你先运行 `clawhub unpin <slug>`。
...
- 当执行者拥有 publisher 访问权限时，`--owner <handle>` 会发布到组织/用户 publisher handle 下。
- `--migrate-owner` 会在发布新版本时将现有技能移动到 `--owner`。需要对两个 publisher 都拥有管理员/所有者访问权限。
- 所有者和审核行为在 `docs/publishing.md` 中说明。
- 发布技能意味着它会在 ClawHub 上以 `MIT-0` 发布。
- 已发布的技能可以免费使用、修改和再分发，无需署名。
- ClawHub 不支持付费技能或按技能定价。
- `--clawscan-note <text>` 添加 ClawScan 备注。此备注为 ClawScan 提供上下文，用于说明可能看起来异常的行为，例如网络访问、原生主机访问或提供商专属凭据。该备注会存储在已发布版本上。
- 旧版别名：`publish <path>`。
...
- 只有当执行者对当前所有者和目标 publisher 都拥有管理员访问权限时，转移到组织/publisher handle 才会立即生效。
- 子命令：
...
- 通过 `GET /api/v1/packages` 和 `GET /api/v1/packages/search` 浏览或搜索统一 package 目录。
- 将它用于插件和其他 package-family 条目；顶层 `search` 仍然是技能搜索界面。
- 标志：
...
- 通过 `GET /api/v1/packages/{name}/versions/{version}/artifact` 解析 package 版本。
...
- 标志：
  - `--package <name>`：用于解析预期 artifact 元数据的 package 名称。
  - `--version <version>` 或 `--tag <tag>`：预期的 package 版本。
...
- 举报是包级别的，可选择绑定到某个版本，并会对版主可见，
  供其审查。
- 举报本身不会自动隐藏包或阻止下载。
- 标志：
  - `--version <version>`：可选，要附加到举报的包版本。
  - `--reason <text>`：必填的举报原因。
  - `--json`：机器可读输出。
...
- 显示一个可能替换
  OpenClaw 内置插件的包的面向操作员的迁移状态。
- 调用与 `package readiness` 相同的计算型就绪端点，但会输出
...
- 元数据会从 `package.json`、`openclaw.plugin.json`，以及
  真实的 OpenClaw 包标记（例如 `.codex-plugin/plugin.json`、
  `.claude-plugin/plugin.json` 和 `.cursor-plugin/plugin.json`）自动检测。
- `.tgz` 源会被视为 ClawPack。CLI 上传精确的 npm-pack
  字节，并且只使用解压后的 `package/` 内容进行验证和
  元数据预填。
- 代码插件文件夹会先打包成 ClawPack npm tarball 再上传，这样
  OpenClaw 安装时可以验证精确的工件。包插件文件夹仍然
  使用解压文件发布路径。
...
- `--dry-run` 会预览解析后的发布载荷，而不会上传。
- `--json` 会为 CI 输出机器可读内容。
- 当操作者拥有发布者访问权限时，`--owner <handle>` 会以用户或组织发布者句柄发布。
- `--clawscan-note <text>` 添加 ClawScan 说明。此说明为 ClawScan
  提供上下文，用于解释可能看起来异常的行为，例如网络访问、
  原生主机访问或提供商特定凭证。该说明会存储在
  已发布的版本上。
...
先使用 `--dry-run`，这样你可以在创建真实发布版本之前确认解析后的包元数据和
来源归属：
...
- `package.json.version` 是你的包发布版本，但不会用作
...
- 如果你使用的是较旧的 `clawhub` CLI 发布版本，请在发布前升级，
  以便本地预检检查在上传前运行。
...
- 对于 monorepo，传入 `source_path`，以便工作流发布插件
  包文件夹，例如 `source_path: extensions/codex`。
- 将可复用工作流固定到稳定标签或完整提交 SHA。不要从 `@main` 运行发布发布流程。
- `pull_request` 应使用 `dry_run: true`，这样 CI 不会产生污染。
- 真实发布应限制为可信事件，例如 `workflow_dispatch` 或标签推送。
- 无密钥的可信发布仅适用于 `workflow_dispatch`；标签推送仍然需要 `clawhub_token`。
- 保持 `clawhub_token` 可用，用于首次发布、不可信包或应急发布。
- 工作流会将 JSON 结果作为工件上传，并将其作为工作流输出暴露。

### `sync`

- 扫描本地技能文件夹并发布新增/已更改的技能。
- 根目录可以是任何文件夹：技能目录，或包含 `SKILL.md` 的单个技能文件夹。
