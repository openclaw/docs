...
对于处于企业代理或受限网络后的系统，CLI 会遵循标准 HTTP 代理环境变量：
...
- 适合把本地登录 token 通过管道传给 CI 密钥设置命令。
...
- 搜索会先偏好精确的 slug/名称 token 匹配，然后才考虑下载热度。像 `map` 这样的独立 slug token 与 `personal-map` 的匹配强度高于 `amap` 内部的子字符串。
- 热度只是一个小的排序先验，并不保证排在最前。
...
- 在 lockfile 中将已安装技能标记为固定。
- `--reason <text>` 记录该技能被冻结的原因。
- 固定技能会被 `update --all` 跳过，并会拒绝直接 `update <skill>`。
- 固定技能也会拒绝 `install --force`，因此本地字节不会被意外替换。

### `unpin <skill>`

- 从已安装技能中移除 lockfile 固定标记，以便未来更新可以修改它。
...
- `update --all` 会跳过固定 slug，并打印哪些内容保持冻结的摘要。

### `skill publish <path>`

- 将本地 bundle 指纹与 ClawHub 比较；当内容已发布时成功退出。
- 新技能默认使用 `1.0.0`；已更改技能默认使用下一个 patch 版本。
- `--version <version>` 显式选择版本，即使内容匹配现有版本也会发布。
- `--dry-run` 在不上传的情况下解析发布；`--json` 打印机器可读结果。
- 当执行者拥有发布者访问权限时，`--owner <handle>` 会以组织/用户发布者 handle 发布。
...
workflow 会为一个 `skill_path` 调用 `skill publish`，或为 `root`（默认：`skills`）下每个直接技能文件夹调用它。它会跳过未更改的技能，并使用相同的自动 patch 版本行为。

设置 `dry_run: true` 可在没有 token 的情况下预览。真实发布需要 `clawhub_token` secret。

### `sync`

- 扫描当前 workdir、已配置的 skills 目录，以及任何 `--root <dir>` 文件夹，寻找包含 `SKILL.md` 或 `skill.md` 的本地技能文件夹。
- 将每个本地技能指纹与 ClawHub 比较，并只发布新的或已更改的技能。
- 新技能发布为 `1.0.0`；已更改技能默认发布为下一个 patch 版本。对于应移动到更大 semver 步长的更新批次，使用 `--bump minor|major`。
- `--dry-run` 在不上传的情况下显示发布计划；`--json` 打印机器可读计划。
- `--all` 会不经提示发布每个新的或已更改的技能。没有 `--all` 时，交互式终端会让你选择要发布的技能。
- 当执行者拥有发布者访问权限时，`--owner <handle>` 会以组织/用户发布者 handle 发布。
...
- 需要 `clawhub login`。
- 通过 `POST /api/v1/skills/-/scan` 运行 ClawHub ClawScan，然后轮询直到扫描终止。
- 扫描是异步的，可能需要一段时间才能完成。排队期间，终端 spinner 会显示当前优先扫描位置以及前面还有多少个扫描。
...
- `--version <version>` 会通过故障关闭的版本专用路由永久删除一个所拥有的非最新版本。
  已删除的版本无法恢复或重新发布。删除当前最新版本前，请先发布替代版本。
  平台工作人员不会在此仅限版本的流程中绕过所有权。
- `--reason <text>` 会在整个技能软删除和审计日志中记录一条审核备注。
...
- 重命名一个所拥有的技能，并将之前的 slug 保留为重定向别名。
- 调用 `POST /api/v1/skills/{slug}/rename`。
- `--yes` 跳过确认。

### `skill merge <source> <target>`

- 将一个所拥有的技能合并到另一个所拥有的技能中。
...
- 转移到组织/发布者 handle 只有在执行者对当前所有者和目标发布者都拥有
  管理员访问权限时才会立即生效。
- 子命令：
...
- 搭配 `--package` 时，会从 ClawHub 解析预期元数据，并将本地文件与已发布构件元数据进行比较。
...
如果验证报告包、清单、SDK 导入或构件问题，请参阅
[插件验证修复](/clawhub/plugin-validation-fixes)，然后重新运行该命令。

### `package delete <name>`

- 不带 `--version` 时，软删除一个包及所有发布版本。
- `--version <version>` 会通过故障关闭的版本专用路由永久删除一个所拥有的非最新发布版本。
  已删除的版本无法恢复或重新发布。删除当前最新版本前，请先发布替代版本。
  此仅限版本的流程要求包所有者或组织发布者管理员；平台工作人员不会绕过包所有权。
- 整包软删除要求包所有者、组织发布者所有者/管理员、平台版主或平台管理员。
- 标志：
...
- 恢复一个已软删除的包及发布版本。
- 不支持版本取消删除；永久删除的版本无法恢复。
- 要求包所有者、组织发布者所有者/管理员、平台版主或平台管理员。
...
- 将包转移给另一个发布者。
- 要求对当前包所有者和目标发布者都拥有管理员访问权限，除非由平台管理员执行。
...
- 举报属于包级别，可选地绑定到某个版本，并会对版主可见以供审核。
- 举报本身不会自动隐藏包或阻止下载。
- 标志：
...
- 显示当前包扫描状态、未处理举报数量、最新发布版本的手动审核状态、下载阻止状态和审核原因。
- 标志：
  - `--json`：机器可读输出。
...
- 显示面向操作员的迁移状态，用于可能替代内置 OpenClaw 插件的包。
...
- `.tgz` 源会被视为 ClawPack。CLI 会上传精确的 npm-pack 字节，并且只使用提取出的 `package/` 内容进行验证和元数据预填。
- 代码插件文件夹会在上传前打包为 ClawPack npm tarball，以便 OpenClaw 安装可以验证精确制品。bundle 插件文件夹仍然使用提取文件发布路径。
- 对于 GitHub 源，源归因会从仓库、解析后的提交、ref 和子路径自动填充。
- 对于本地文件夹，当 origin remote 指向 GitHub 时，源归因会从本地 git 自动检测。
...
- `--json` 会为 CI 输出机器可读内容。
- `--owner <handle>` 会在执行者拥有发布者访问权限时，以用户或组织发布者 handle 发布。
...
先使用 `--dry-run`，这样你可以在创建真实发布前确认解析后的包元数据和源归因：
...
- `package.json.version` 是你的包发布版本，但不会作为 OpenClaw 兼容性/构建验证的回退。
...
- 如果你使用的是较旧的 `clawhub` CLI 版本，请在发布前升级，以便本地预检检查在上传前运行。
- 如果验证报告了修复代码，请参阅
  [插件验证修复](/clawhub/plugin-validation-fixes)。

#### GitHub Actions

ClawHub 还为插件仓库提供了一个官方可复用工作流：
...
- 对于 monorepo，传入 `source_path`，这样工作流会发布插件包文件夹，例如 `source_path: extensions/codex`。
- 将可复用工作流固定到稳定 tag 或完整提交 SHA。不要从 `@main` 运行发布。
- `pull_request` 应使用 `dry_run: true`，这样 CI 不会造成污染。
- 真实发布应限制为受信任事件，例如 `workflow_dispatch` 或 tag push。
- 不带 secret 的受信任发布仅适用于 `workflow_dispatch`；tag push 仍需要 `clawhub_token`。
- 保持 `clawhub_token` 可用，用于首次发布、不受信任的包，或应急发布。
...
- ClawHub 会在此命令运行时验证配置的 GitHub 仓库。
  公共仓库可以通过公共 GitHub 元数据验证。私有仓库要求 ClawHub 拥有对该 GitHub 仓库的访问权限，例如通过未来的 ClawHub GitHub App 安装或其他已授权的 GitHub 集成。
- 标志：
