...
x-i18n:
    generated_at: "2026-06-27T01:30:44Z"
    model: gpt-5.5
    postprocess_version: locale-links-v1
    provider: openai
    source_hash: ea78b556a9821b8c4b3e4f23d0985551add6314403981c10339e35ac91083899
...
这在直接出站连接被阻止的系统上是必需的（例如 Docker 容器、仅代理联网的 Hetzner VPS、企业防火墙）。
...
- 远程/无头交互式：`clawhub login --device` 会打印代码，并在你于 `<site>/cli/device` 授权期间等待。
...
- 搜索会先优先精确匹配 slug/name token，再考虑下载热度。像 `map` 这样的独立 slug token 与 `personal-map` 的匹配强度高于 `amap` 内部的子字符串。
- 热度只是一个小的排序先验，不保证置顶。
- 如果某个技能应出现但未出现，请在登录状态下运行 `clawhub inspect @owner/slug`，先检查所有者可见的审核诊断，再重命名元数据。
...
- 登录状态下发送尽力而为的 telemetry，以便停用当前安装计数。
- 交互式：请求确认。
...
- Pinned 技能也会拒绝 `install --force`，因此本地字节不会被意外替换。

### `unpin <skill>`

- 从已安装技能中移除 lockfile pin，使未来更新可以修改它。
...
- `update <skill>` 会对 pinned 技能快速失败，并提示你先运行 `clawhub unpin <skill>`。
- `update --all` 会跳过 pinned slug，并打印保留冻结内容的摘要。

### `skill publish <path>`

- 将本地 bundle fingerprint 与 ClawHub 比较，当内容已经发布时成功退出。
- 新技能默认使用 `1.0.0`；已更改的技能默认使用下一个 patch 版本。
- `--version <version>` 会显式选择版本，并且即使内容匹配现有版本也会发布。
- `--dry-run` 解析发布但不上传；`--json` 打印机器可读结果。
- `--owner <handle>` 会在 actor 具有发布者访问权限时，以组织/用户发布者 handle 发布。
- `--migrate-owner` 会在发布新版本时将现有技能移动到 `--owner`。需要对两个发布者都有 admin/owner 访问权限。
- 所有者和 review 行为在 `docs/publishing.md` 中说明。
- 发布技能意味着它在 ClawHub 上以 `MIT-0` 发布。
- 已发布的技能可自由使用、修改和重新分发，无需署名。
...
workflow 会对一个 `skill_path` 调用 `skill publish`，或对 `root` 下的每个直接技能文件夹调用（默认：`skills`）。它会跳过未更改的技能，并使用相同的自动 patch 版本行为。
...
- 将每个本地技能 fingerprint 与 ClawHub 比较，仅发布新的或已更改的技能。
- 新技能发布为 `1.0.0`；已更改的技能默认发布下一个 patch 版本。对于应按更大 semver 步长推进的更新批次，使用 `--bump minor|major`。
- `--dry-run` 显示发布计划但不上传；`--json` 打印机器可读计划。
- `--all` 会发布每个新的或已更改的技能且不提示。没有 `--all` 时，交互式终端可让你选择要发布的技能。
- `--owner <handle>` 会在 actor 具有发布者访问权限时，以组织/用户发布者 handle 发布。
...
- 扫描是异步的，可能需要一些时间才能完成。排队期间，终端 spinner 会显示当前优先扫描位置，以及前面还有多少扫描。
- 已发布扫描需要所有权或发布者管理访问权限。Moderator/admin 可以通过 `clawhub-admin` 使用同一后端。
...
- 不再支持本地路径扫描。上传新版本，然后使用 `scan download` 获取该提交版本已存储的扫描结果。
...
- 需要 `clawhub login`。
- 下载提交的技能或插件版本的已存储扫描报告 ZIP，包括被 ClawHub 安全检查阻止或隐藏的版本。
...
ClawHub 在
[`/.github/workflows/skill-publish.yml`](https://github.com/openclaw/clawhub/blob/447a5b0275701e6c9fd33e0369e6b68fb7c125cc/.github/workflows/skill-publish.yml)
提供官方可复用 workflow，适用于技能仓库和目录仓库。
...
- `--version <version>` 通过故障关闭的版本专用路由永久删除一个拥有的非最新版本。
  已删除的版本无法恢复或重新发布。删除当前最新版本前，请先发布替代版本。平台人员不会在这个仅限版本的流程中绕过所有权。
...
- 重命名一个拥有的技能，并将之前的 slug 保留为重定向别名。
...
- 转移给组织/发布者 handle 只有在操作者同时拥有当前所有者和目标发布者的管理员访问权限时才会立即生效。
- 子命令：
...
- 使用 `--package` 时，会从 ClawHub 解析预期元数据，并将本地文件与已发布的制品元数据进行比较。
- 使用直接摘要标志时，无需网络查询即可验证。
- 标志：
  - `--package <name>`：用于解析预期制品元数据的包名称。
...
- 硬性兼容性错误会以非零状态退出。仅警告的发现会打印出来，但以零状态退出。
- 标志：
...
如果验证报告了包、清单、SDK 导入或制品发现，请参阅
[插件验证修复](/zh-CN/clawhub/plugin-validation-fixes)，然后重新运行该命令。

### `package delete <name>`

- 不带 `--version` 时，软删除一个包及其所有发布版本。
- `--version <version>` 通过故障关闭的版本专用路由永久删除一个拥有的非最新发布版本。
  已删除的版本无法恢复或重新发布。删除当前最新版本前，请先发布替代版本。这个仅限版本的流程要求包所有者或组织发布者管理员执行；平台人员不会绕过包所有权。
- 整包软删除要求包所有者、组织发布者所有者/管理员、平台版主或平台管理员执行。
- 标志：
...
- 恢复一个已软删除的包及其发布版本。
- 不存在版本恢复；永久删除的版本无法恢复。
- 要求包所有者、组织发布者所有者/管理员、平台版主或平台管理员执行。
...
- 将包转移给另一个发布者。
- 要求同时拥有当前包所有者和目标发布者的管理员访问权限，除非由平台管理员执行。
- Scoped package 名称必须转移给匹配的 scope 所有者。
...
- 举报是包级别的，可选择关联到某个版本，并会对版主可见以供审核。
- 举报本身不会自动隐藏包或阻止下载。
- 标志：
...
- 显示当前包扫描状态、开放举报数量、最新发布版本的人工审核状态、下载阻止状态和审核原因。
- 标志：
  - `--json`：机器可读输出。
...
- 报告官方状态、ClawPack 可用性、制品摘要、来源证明、OpenClaw 兼容性、宿主目标、环境元数据和扫描状态的阻塞项。
- 标志：
  - `--json`：机器可读输出。
...
- 为可能替代内置 OpenClaw 插件的包显示面向操作者的迁移状态。
- 调用与 `package readiness` 相同的计算型就绪状态端点，但打印面向迁移的状态、最新版本、官方包状态、检查和阻塞项。
- 标志：
  - `--json`：机器可读输出。
...
- `.tgz` 源会被视为 ClawPack。CLI 会上传精确的 npm-pack 字节，并且只将提取出的 `package/` 内容用于验证和元数据预填充。
- 代码插件文件夹会在上传前打包为 ClawPack npm tarball，以便 OpenClaw 安装可以验证精确的构件。bundle 插件文件夹仍使用提取文件发布路径。
...
- 外部代码插件必须显式声明 `openclaw.compat.pluginApi` 和 `openclaw.build.openclawVersion`。
  顶层 `package.json.version` 不会作为发布验证的回退。
- `--dry-run` 会预览解析后的发布载荷，而不上传。
- `--json` 会为 CI 发出机器可读输出。
- 当执行者拥有发布者访问权限时，`--owner <handle>` 会在用户或组织发布者 handle 下发布。
- scoped package 名称必须匹配所选 owner。参见 `docs/publishing.md`。
- 现有标志（`--family`、`--name`、`--version`、`--source-repo`、`--source-commit`、`--source-ref`、`--source-path`）仍可用作覆盖项。
...
先使用 `--dry-run`，这样你可以在创建实时发布前确认解析后的包元数据和源归属：
...
- 如果验证报告了 remediation code，请参见 [插件验证修复](/zh-CN/clawhub/plugin-validation-fixes)。

#### GitHub Actions

ClawHub 还为插件仓库提供了官方可复用 workflow：
[`/.github/workflows/package-publish.yml`](https://github.com/openclaw/clawhub/blob/447a5b0275701e6c9fd33e0369e6b68fb7c125cc/.github/workflows/package-publish.yml)。
...
- 对于 monorepo，传入 `source_path`，让 workflow 发布插件包文件夹，例如 `source_path: extensions/codex`。
- 将可复用 workflow 固定到稳定标签或完整 commit SHA。不要从 `@main` 运行发布。
- `pull_request` 应使用 `dry_run: true`，让 CI 不产生污染。
- 真实发布应限于可信事件，例如 `workflow_dispatch` 或 tag push。
- 不带 secret 的可信发布仅适用于 `workflow_dispatch`；tag push 仍需要 `clawhub_token`。
- 保留 `clawhub_token`，用于首次发布、不受信任的包或紧急发布。
- workflow 会将 JSON 结果作为 artifact 上传，并将其暴露为 workflow outputs。

### `package trusted-publisher get <name>`

- 显示包的 GitHub Actions 可信发布者配置。
- 设置配置后使用此命令，确认仓库、workflow 文件名以及可选环境 pin。
- 标志：
  - `--json`：机器可读输出。
...
- 必须先通过普通手动发布或令牌认证的 `clawhub package publish` 创建包。
...
- 从包中移除可信发布者配置。
- 如果需要禁用或重新创建 workflow、仓库或环境 pin，请将此作为回滚方式。
- 在重新设置配置前，未来的真实发布必须使用普通认证发布。
- 标志：
  - `--json`：机器可读输出。
