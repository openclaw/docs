...
CLI 遵循标准 HTTP 代理环境变量，适用于位于企业代理或受限网络后的系统：

- `HTTPS_PROXY` / `https_proxy`
- `HTTP_PROXY` / `http_proxy`
- `NO_PROXY` / `no_proxy`

设置其中任一变量时，CLI 会通过指定代理路由出站请求。`HTTPS_PROXY` 用于 HTTPS 请求，`HTTP_PROXY` 用于普通 HTTP。`NO_PROXY` / `no_proxy` 会被遵循，以便对特定主机或域名绕过代理。

在直接出站连接被阻止的系统上需要这样做（例如 Docker 容器、仅允许代理上网的 Hetzner VPS、企业防火墙）。
...
- 远程/无头交互式：`clawhub login --device` 会打印一个代码，并在你于 `<site>/cli/device` 授权期间等待。
...
- 调用 `/api/v1/search?q=...`。
- 搜索会优先考虑精确的 slug/name 词元匹配，然后才是下载热度。像 `map` 这样的独立 slug 词元会比 `amap` 内部的子字符串更强地匹配 `personal-map`。
- 下载量只是一个小的热度先验，并不保证排在顶部。
...
- 在通过 `clawhub pin` 冻结的 Skills 旁显示 `pinned`，包括可选原因。

### `pin <slug>`

- 在锁文件中将已安装的技能标记为 pinned。
- `--reason <text>` 记录该技能被冻结的原因。
- Pinned 技能会被 `update --all` 跳过，并且直接 `update <slug>` 会被拒绝。
- Pinned 技能也会拒绝 `install --force`，因此本地字节不会被意外替换。

### `unpin <slug>`

- 从已安装技能中移除锁文件 pin，使未来更新可以修改它。
...
- `update <slug>` 会对 pinned slug 快速失败，并告诉你先运行 `clawhub unpin <slug>`。
- `update --all` 会跳过 pinned slug，并打印保留冻结项的摘要。
...
- `--migrate-owner` 会在发布新版本的同时，将现有技能移动到 `--owner`。需要两个发布者的 admin/owner 访问权限。
- 所有者和审核行为在 `docs/publishing.md` 中说明。
- 发布技能意味着它会在 ClawHub 上以 `MIT-0` 发布。
- 已发布的技能可自由使用、修改和再分发，无需署名。
- ClawHub 不支持付费技能或按技能定价。
- `--clawscan-note <text>` 添加 ClawScan 备注。此备注为 ClawScan 提供上下文，用于说明可能看起来异常的行为，例如网络访问、本地主机访问或提供商特定凭证。备注会存储在已发布版本上。
- 旧版别名：`publish <path>`。
...
- `--reason <text>` 在技能和审计日志上记录一条 moderation 备注。
- `--note <text>` 是 `--reason` 的别名。
- `--yes` 跳过确认。

### `undelete <slug>`

- 恢复隐藏的技能（所有者、moderator 或 admin）。
...
- 重命名一个你拥有的技能，并将先前的 slug 保留为重定向别名。
...
- 所有权转移工作流。
- 转移到用户 handle 会创建待处理请求，由接收者接受。
- 只有当操作者同时拥有当前所有者和目标发布者的 admin 访问权限时，转移到组织/发布者 handle 才会立即生效。
- 子命令：
...
- 将其用于插件和其他软件包系列条目；顶层 `search` 仍然是技能搜索界面。
- 标志：
...
- 通过 `GET /api/v1/packages/{name}/versions/{version}/artifact` 解析软件包版本。
- 从解析器的 `downloadUrl` 下载 artifact。
- 对所有 artifacts 验证 ClawHub SHA-256。
- 对于 ClawPack npm-pack artifacts，还会验证 npm `sha512` integrity、npm shasum 以及 tarball 的 `package.json` 名称/版本。
...
- 为本地 artifact 计算 ClawHub SHA-256、npm `sha512` integrity 和 npm shasum。
- 使用 `--package` 时，从 ClawHub 解析预期元数据，并将本地文件与已发布 artifact 元数据比较。
- 使用直接 digest 标志时，无需网络查找即可验证。
- 标志：
...
- 显示面向操作员的包迁移状态，该包可能会替代一个
  内置 OpenClaw 插件。
- 调用与 `package readiness` 相同的计算型就绪端点，但会打印
  以迁移为中心的状态、最新版本、官方包状态、检查项和
  阻碍项。
- 标志：
  - `--json`: 机器可读输出。
...
- 元数据会从 `package.json`、`openclaw.plugin.json` 和
  真实 OpenClaw 捆绑标记（例如 `.codex-plugin/plugin.json`、
  `.claude-plugin/plugin.json` 和 `.cursor-plugin/plugin.json`）中自动检测。
- `.tgz` 来源会被视为 ClawPack。CLI 会上传精确的 npm-pack
  字节，并且仅使用提取出的 `package/` 内容进行验证和
  元数据预填。
- 代码插件文件夹会在上传前打包成 ClawPack npm tarball，以便
  OpenClaw 安装可以验证精确构件。捆绑插件文件夹仍然
  使用提取文件发布路径。
- 对于 GitHub 来源，来源归因会从仓库、解析后的提交、ref 和子路径自动填充。
- 对于本地文件夹，当 origin 远程指向 GitHub 时，会从本地 git 自动检测来源归因。
...
- `--clawscan-note <text>` 会添加 ClawScan 备注。此备注为 ClawScan
  提供上下文，用于解释可能看起来异常的行为，例如网络访问、
  原生主机访问或提供商特定凭证。该备注会存储在
  已发布的发布版本上。
- 作用域包名必须匹配所选所有者。参见 `docs/publishing.md`。
...
先使用 `--dry-run`，这样你可以在创建实时发布版本之前确认解析出的包元数据和
来源归因：
...
外部代码插件需要在 `package.json` 中包含少量 OpenClaw 元数据。
此最小清单足以成功发布：
...
- 对于 monorepo，传入 `source_path`，使工作流发布插件
  包文件夹，例如 `source_path: extensions/codex`。
- 将可复用工作流固定到稳定标签或完整提交 SHA。不要从 `@main` 运行发布。
- `pull_request` 应使用 `dry_run: true`，这样 CI 保持无污染。
- 真实发布应限制为受信任事件，例如 `workflow_dispatch` 或标签推送。
- 没有密钥的受信任发布仅适用于 `workflow_dispatch`；标签推送仍需要 `clawhub_token`。
...
- 根可以是任意文件夹：技能目录，或带有 `SKILL.md` 的单个技能文件夹。
- 当 `~/.clawdbot/clawdbot.json` 存在时，会自动添加 Clawdbot 技能根：
