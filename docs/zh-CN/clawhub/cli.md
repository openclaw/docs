...
对于位于企业代理或受限网络后的系统，CLI 会遵循标准的 HTTP 代理环境变量：

- `HTTPS_PROXY` / `https_proxy`
- `HTTP_PROXY` / `http_proxy`
- `NO_PROXY` / `no_proxy`

设置其中任意变量后，CLI 会通过指定代理路由出站请求。`HTTPS_PROXY` 用于 HTTPS 请求，`HTTP_PROXY`
用于普通 HTTP。`NO_PROXY` / `no_proxy` 会被遵循，以便为特定主机或域名绕过代理。
...
- 远程/无界面交互式：`clawhub login --device` 会打印一个代码，并在你到 `<site>/cli/device` 授权时等待。
...
- 调用 `/api/v1/search?q=...`。
- 搜索会先优先匹配精确的 slug/名称词元，再考虑下载热度。独立的 slug 词元（例如 `map`）比 `amap` 内部的子串更强地匹配 `personal-map`。
- 下载量只是一个很小的热度先验，并不保证排在顶部。
- 如果某个技能应该出现但没有出现，请在已登录状态下运行 `clawhub inspect <slug>`，先检查仅所有者可见的审核诊断，再重命名元数据。
...
- Pinned 技能会被 `update --all` 跳过，并且直接执行 `update <slug>` 会被拒绝。
- Pinned 技能也会拒绝 `install --force`，因此本地字节不会被意外替换。

### `unpin <slug>`

- 从已安装的技能中移除锁文件 pin，让未来更新可以修改它。
...
- `update <slug>` 会对 pinned slug 快速失败，并提示你先运行 `clawhub unpin <slug>`。
- `update --all` 会跳过 pinned slug，并打印保持冻结项的摘要。

### `skill publish <path>`

- 通过 `POST /api/v1/skills`（multipart）发布。
- 需要语义化版本：`--version 1.2.3`。
- 当操作方拥有发布者访问权限时，`--owner <handle>` 会在组织/用户发布者句柄下发布。
- `--migrate-owner` 会在发布新版本时，将现有技能移动到 `--owner`。需要对两个发布者都拥有 admin/owner 访问权限。
- Owner 和审核行为在 `docs/publishing.md` 中说明。
- 发布技能意味着它会在 ClawHub 上以 `MIT-0` 发布。
- 已发布的技能可以免费使用、修改和再分发，无需署名。
- ClawHub 不支持付费技能或按技能定价。
- `--clawscan-note <text>` 会添加 ClawScan 说明。此说明为 ClawScan 提供上下文，用于解释否则可能显得异常的行为，例如网络访问、原生主机访问或提供商专用凭证。该说明会存储在已发布版本上。
- 旧版别名：`publish <path>`。
...
- 软删除技能（owner、moderator 或 admin）。
- 调用 `DELETE /api/v1/skills/{slug}`。
- Owner 发起的软删除会保留该 slug 30 天；命令会打印过期时间。
- `--reason <text>` 在技能和审计日志上记录审核说明。
- `--note <text>` 是 `--reason` 的别名。
- `--yes` 跳过确认。

### `undelete <slug>`

- 恢复隐藏的技能（owner、moderator 或 admin）。
...
- 隐藏技能（owner、moderator 或 admin）。
- `delete` 的别名。

### `unhide <slug>`

- 取消隐藏技能（owner、moderator 或 admin）。
- `undelete` 的别名。

### `skill rename <slug> <new-slug>`

- 重命名自有技能，并将旧 slug 保留为重定向别名。
...
- 所有权转移工作流。
- 转移给用户句柄时，会创建待处理请求，由接收者接受。
- 转移给组织/发布者句柄时，只有当操作方同时拥有当前 owner 和目标发布者的 admin 访问权限时才会立即生效。
- 子命令：
...
- 使用 `--package` 时，会从 ClawHub 解析预期元数据，并将本地文件与已发布工件元数据进行比较。
...
- 举报以包为级别，可选择关联到某个版本，并会对审核员可见以供审查。
- 举报本身不会自动隐藏包或阻止下载。
- 标志：
...
- 调用与 `package readiness` 相同的计算型就绪端点，但会打印以迁移为重点的状态、最新版本、官方包状态、检查项和阻碍项。
- 标志：
  - `--json`：机器可读输出。
...
- 元数据会从 `package.json`、`openclaw.plugin.json` 和真实的 OpenClaw 捆绑标记自动检测，例如 `.codex-plugin/plugin.json`、
...
- 代码插件文件夹会在上传前打包为 ClawPack npm tarball，以便 OpenClaw 安装可以验证精确工件。捆绑插件文件夹仍使用提取文件发布路径。
...
- `--json` 会为 CI 输出机器可读内容。
- 当执行者拥有发布者访问权限时，`--owner <handle>` 会在用户或组织发布者标识下发布。
- `--clawscan-note <text>` 会添加 ClawScan 备注。此备注会向 ClawScan 提供行为上下文，用于说明否则可能看起来异常的行为，例如网络访问、原生主机访问或特定提供商凭据。该备注会存储在已发布的发布版本上。
...
先使用 `--dry-run`，以便在创建真实发布版本之前确认解析后的包元数据和来源归属：
...
对于代码插件，文件夹发布会从包文件夹构建并上传 ClawPack 工件：
...
- `openclaw.hostTargets` 和 `openclaw.environment` 是可选元数据。
  ClawHub 可能会在存在时展示它们，但发布并不要求它们。
- 如果你想发布更详细的兼容性元数据，`openclaw.compat.minGatewayVersion` 和
  `openclaw.build.pluginSdkVersion` 是可选的额外项。
- 如果你使用较旧的 `clawhub` CLI 版本，请在发布前升级，以便本地预检在上传前运行。
