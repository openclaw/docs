...
设置其中任一变量时，CLI 会通过指定代理路由出站请求。`HTTPS_PROXY` 用于 HTTPS 请求，`HTTP_PROXY` 用于普通 HTTP。`NO_PROXY` / `no_proxy` 会被遵循，用于绕过特定主机或域名的代理。
...
- 远程/无头交互模式：`clawhub login --device` 会打印一个代码，并在你到 `<site>/cli/device` 授权时等待。
...
- 适合将本地登录令牌通过管道传给 CI 密钥设置命令。

### `star <skill>` / `unstar <skill>`

- 将某个技能添加到你的重点关注中，或将其移除。
...
- 搜索会先偏好精确的 slug/名称 token 匹配，再考虑下载热度。像 `map` 这样的独立 slug token 匹配 `personal-map` 的强度高于匹配 `amap` 内部的子字符串。
- 热度只是一个较小的排序先验，并不保证排在最前。
- 如果某个技能应该出现却没有出现，请在登录状态下运行 `clawhub inspect @owner/slug`，先检查所有者可见的审核诊断信息，再重命名元数据。
...
- 对使用 `clawhub pin` 冻结的技能显示 `pinned`，包括可选原因。

### `pin <skill>`

- 在 lockfile 中将已安装的技能标记为固定。
- `--reason <text>` 记录技能被冻结的原因。
- 固定技能会被 `update --all` 跳过，直接 `update <skill>` 也会被拒绝。
- 固定技能也会拒绝 `install --force`，以免本地字节被意外替换。

### `unpin <skill>`

- 从已安装技能中移除 lockfile 固定标记，使未来更新可以修改它。
...
- 如果指纹不匹配：
  - 默认拒绝
  - 使用 `--force` 覆盖（如果是交互模式，也可通过提示覆盖）
- 固定技能永远不会被 `--force` 更新。
- `update <skill>` 会对固定技能快速失败，并提示你先运行 `clawhub unpin <skill>`。
- `update --all` 会跳过固定的 slug，并打印哪些内容保持冻结的摘要。

### `skill publish <path>`

- 将本地 bundle 指纹与 ClawHub 比较，如果内容已发布则成功退出。
- 新技能默认使用 `1.0.0`；已变更技能默认使用下一个补丁版本。
- `--version <version>` 会显式选择版本，并且即使内容匹配已有版本也会发布。
- `--dry-run` 会解析发布但不上传；`--json` 打印机器可读结果。
- `--owner <handle>` 会在执行者拥有发布者访问权限时，以组织/用户发布者 handle 发布。
- `--migrate-owner` 会在发布新版本时将现有技能移动到 `--owner`。需要两个发布者的管理员/所有者访问权限。
...
ClawHub 的可复用 [`skill-publish.yml`](https://github.com/openclaw/clawhub/blob/main/.github/workflows/skill-publish.yml) workflow 会为一个 `skill_path` 调用 `skill publish`，或为 `root` 下每个直接技能文件夹调用它（默认：`skills`）。它会跳过未变更的技能，并使用相同的自动补丁版本行为。
...
- 将每个本地技能指纹与 ClawHub 比较，只发布新增或已变更的技能。
- 新技能发布为 `1.0.0`；已变更技能默认发布下一个补丁版本。对于需要按更大 semver 步长移动的更新批次，使用 `--bump minor|major`。
- `--dry-run` 显示发布计划但不上传；`--json` 打印机器可读计划。
- `--all` 会在不提示的情况下发布每个新增或已变更的技能。没有 `--all` 时，交互式终端会让你选择要发布的技能。
- `--owner <handle>` 会在执行者拥有发布者访问权限时，以组织/用户发布者 handle 发布。
...
- 扫描是异步的，可能需要一些时间完成。排队期间，终端 spinner 会显示当前优先扫描位置以及前方还有多少扫描。
- 已发布扫描需要所有权或发布者管理访问权限。审核员/管理员可以通过 `clawhub-admin` 使用同一后端。
...
- 不再支持本地路径扫描。上传新版本，然后使用 `scan download` 检索该提交版本已存储的扫描结果。
...
ClawHub 在 [`/.github/workflows/skill-publish.yml`](https://github.com/openclaw/clawhub/blob/8f98128aab28627477a3858081a13b76cba6f5d6/.github/workflows/skill-publish.yml) 提供一个官方可复用 workflow，用于技能仓库和目录仓库。
...
- 不带 `--version` 时，软删除一个技能（所有者、审核员或管理员）。
- 调用 `DELETE /api/v1/skills/{slug}`。
- 所有者发起的软删除会保留该 slug 30 天；命令会打印到期时间。
- `--version <version>` 通过一个故障关闭的版本专用路由，永久删除一个已拥有的非最新版本。
...
- 重命名一个已拥有的技能，并将先前的 slug 保留为重定向别名。
...
- 所有权转移工作流。
- 转移给用户 handle 会创建一个待处理请求，由接收方接受。
- 只有当操作者同时拥有当前所有者和目标发布者的管理员访问权限时，转移给组织/发布者 handle 才会立即生效。
- 子命令：
...
- 通过 `GET /api/v1/packages` 和 `GET /api/v1/packages/search` 浏览或搜索统一的包目录。
...
- 通过
  `GET /api/v1/packages/{name}/versions/{version}/artifact` 解析包版本。
- 从解析器的 `downloadUrl` 下载构件。
- 对所有构件验证 ClawHub SHA-256。
- 对于 ClawPack npm-pack 构件，还会验证 npm `sha512` 完整性、npm shasum，以及压缩包的 `package.json` 名称/版本。
...
- 硬兼容性错误会以非零状态退出。仅警告的发现会被打印，但以零状态退出。
- 标志：
...
如果验证报告了包、清单、SDK 导入或构件发现，请参阅
[插件验证修复](/zh-CN/clawhub/plugin-validation-fixes)，然后重新运行该命令。

### `package delete <name>`

- 不带 `--version` 时，软删除一个包及其所有发布。
- `--version <version>` 通过一个故障关闭的版本专用路由，永久删除一个已拥有的非最新发布。
  已删除的版本无法恢复或重新发布。删除当前最新版本前，请先发布替代版本。此仅限版本的流程要求包所有者或组织发布者管理员执行；平台工作人员不会绕过包所有权。
- 整包软删除要求包所有者、组织发布者所有者/管理员、平台审核员或平台管理员执行。
- 标志：
...
- 恢复一个已软删除的包及其发布。
- 不支持恢复版本；永久删除的版本无法恢复。
- 要求包所有者、组织发布者所有者/管理员、平台审核员或平台管理员执行。
...
- 将包转移给另一个发布者。
- 除非由平台管理员执行，否则要求同时拥有当前包所有者和目标发布者的管理员访问权限。
- 作用域包名必须转移给匹配的作用域所有者。
...
- 显示当前包扫描状态、打开的举报数量、最新发布的人工审核状态、下载阻止状态和审核原因。
- 标志：
  - `--json`：机器可读输出。
...
- 报告官方状态、ClawPack 可用性、构件摘要、来源出处、OpenClaw 兼容性、主机目标、环境元数据和扫描状态方面的阻塞项。
- 标志：
  - `--json`：机器可读输出。
...
- 显示面向操作者的迁移状态，用于可能替代内置 OpenClaw 插件的包。
- 调用与 `package readiness` 相同的计算型就绪状态端点，但打印聚焦迁移的状态、最新版本、官方包状态、检查项和阻塞项。
- 标志：
  - `--json`：机器可读输出。
...
- 创建一个由已认证用户拥有的组织发布者。
- handle 会规范化为小写，并且可带或不带 `@` 传入。
- 新创建的组织发布者默认不是受信任/官方发布者。
- 如果 handle 已被现有发布者、用户或保留路由使用，则会失败。
...
- 元数据会从 `package.json`、`openclaw.plugin.json` 以及真实的 OpenClaw 捆绑标记自动检测，例如 `.codex-plugin/plugin.json`、`.claude-plugin/plugin.json` 和 `.cursor-plugin/plugin.json`。
- `.tgz` 来源会被视为 ClawPack。CLI 会上传精确的 npm-pack 字节，并且仅将解压出的 `package/` 内容用于验证和元数据预填充。
- 代码插件文件夹会在上传前打包为 ClawPack npm tarball，以便 OpenClaw 安装可以验证精确 artifact。捆绑插件文件夹仍然使用解压文件发布路径。
- 对于 GitHub 来源，来源归属会根据仓库、解析后的提交、ref 和子路径自动填充。
...
- 外部代码插件必须显式声明 `openclaw.compat.pluginApi` 和 `openclaw.build.openclawVersion`。
  顶层 `package.json.version` 不会作为发布验证的 fallback。
- `--dry-run` 会预览解析后的发布 payload，而不会上传。
- `--json` 会为 CI 输出机器可读结果。
- 当执行者拥有发布者访问权限时，`--owner <handle>` 会以用户或组织发布者 handle 发布。
- Scoped package 名称必须与所选 owner 匹配。参见 `docs/publishing.md`。
...
先使用 `--dry-run`，这样你可以在创建实时发布前确认解析后的 package 元数据和来源归属：
...
对于代码插件，文件夹发布会从 package 文件夹构建并上传 ClawPack artifact：
...
- `package.json.version` 是你的 package 发布版本，但不会作为 OpenClaw 兼容性/构建验证的 fallback。
- `openclaw.hostTargets` 和 `openclaw.environment` 是可选元数据。
  ClawHub 在存在时可能会展示它们，但发布并不要求它们。
...
- 如果验证报告了修复代码，请参见
  [插件验证修复](/zh-CN/clawhub/plugin-validation-fixes)。

#### GitHub Actions

ClawHub 还为插件仓库提供一个官方可复用 workflow：
...
- 对于 monorepo，请传入 `source_path`，以便 workflow 发布插件 package 文件夹，例如 `source_path: extensions/codex`。
- 将可复用 workflow pin 到稳定 tag 或完整提交 SHA。不要从 `@main` 运行发布。
- `pull_request` 应使用 `dry_run: true`，这样 CI 不会产生污染。
- 真实发布应限制为受信任事件，例如 `workflow_dispatch` 或 tag push。
- 不带 secret 的受信任发布仅适用于 `workflow_dispatch`；tag push 仍然需要 `clawhub_token`。
- 保留 `clawhub_token` 以用于首次发布、不受信任的 package 或 break-glass 发布。
- workflow 会将 JSON 结果作为 artifact 上传，并将其作为 workflow outputs 暴露。

### `package trusted-publisher get <name>`

- 显示 package 的 GitHub Actions 受信任发布者配置。
- 设置配置后使用此命令，以确认仓库、workflow 文件名和可选 environment pin。
- 标志：
  - `--json`：机器可读输出。
...
- 为现有 package 附加或替换 GitHub Actions 受信任发布者配置。
- package 必须先通过普通手动方式或基于 token 认证的 `clawhub package publish` 创建。
- 配置设置完成后，未来受支持的 GitHub Actions 发布可以使用 OIDC/受信任发布，而无需长期有效的 ClawHub token。
- `--repository <repo>` 必须是 `owner/repo`。
- `--workflow-filename <file>` 必须匹配 `.github/workflows/` 中的 workflow 文件名。
- `--environment <name>` 是可选的。配置后，OIDC claim 中的 GitHub Actions environment 必须完全匹配。
- ClawHub 会在此命令运行时验证已配置的 GitHub 仓库。
