...
CLI 遵循标准 HTTP 代理环境变量，适用于位于企业代理或受限网络之后的系统：
...
- 搜索会优先匹配精确的 slug/名称词元，然后再考虑下载热度。像 `map` 这样的独立 slug 词元会比 `amap` 内部的子字符串更强地匹配 `personal-map`。
- 热度只是一个较小的排序先验，不保证排在顶部。
...
- 已固定技能也会拒绝 `install --force`，以免本地字节被意外替换。
...
- `update --all` 会跳过已固定 slug，并打印保留冻结内容的摘要。

### `skill publish <path>`

- 将本地 bundle 指纹与 ClawHub 比较，如果内容已发布，则成功退出。
- 新技能默认使用 `1.0.0`；已更改技能默认使用下一个补丁版本。
- `--version <version>` 显式选择版本，即使内容匹配现有版本也会发布。
- `--dry-run` 在不上传的情况下解析发布；`--json` 打印机器可读结果。
- 当操作者拥有发布者访问权限时，`--owner <handle>` 会在组织/用户发布者 handle 下发布。
- `--migrate-owner` 会在发布新版本时将现有技能移动到 `--owner`。需要对两个发布者都拥有管理员/所有者访问权限。
- 所有者和审核行为在 `docs/publishing.md` 中说明。
- 发布技能意味着它会在 ClawHub 上以 `MIT-0` 发布。
- 已发布技能可自由使用、修改和重新分发，无需署名。
...
工作流会对一个 `skill_path` 调用 `skill publish`，或对 `root` 下每个直接技能文件夹调用（默认：`skills`）。它会跳过未更改的技能，并使用相同的自动补丁版本行为。
...
- 将每个本地技能指纹与 ClawHub 比较，并且只发布新的或已更改的技能。
- 新技能发布为 `1.0.0`；已更改技能默认发布下一个补丁版本。对应该移动更大 semver 步长的更新批次，请使用 `--bump minor|major`。
- `--dry-run` 在不上传的情况下显示发布计划；`--json` 打印机器可读计划。
- `--all` 会发布每个新的或已更改的技能，不提示。没有 `--all` 时，交互式终端会让你选择要发布的技能。
- 当操作者拥有发布者访问权限时，`--owner <handle>` 会在组织/用户发布者 handle 下发布。
...
- 需要 `clawhub login`。
- 通过 `POST /api/v1/skills/-/scan` 运行 ClawHub ClawScan，然后轮询直到扫描达到终态。
...
- `root` 对目录仓库默认为 `skills`。
- 传入 `skill_path: skills/review-helper` 以处理一个技能文件夹。
...
- `--version <version>` 会通过失效关闭的版本专用路由，永久删除一个自己拥有的非最新版本。
...
- 重命名一个自己拥有的技能，并将先前的 slug 保留为重定向别名。
...
- 将此命令用于插件和其他包系列条目；顶层 `search` 仍然是技能搜索界面。
- 标志：
...
- 默认执行离线/静态验证，不定位或导入本地 OpenClaw checkout。
...
- 标志：
  ...
  - `--allow-execute`：允许在隔离工作区中执行运行时捕获。
...
如果验证报告包、清单、SDK 导入或工件发现，请参阅
[插件验证修复](/zh-CN/clawhub/plugin-validation-fixes)，然后重新运行命令。

### `package delete <name>`

- 不带 `--version` 时，软删除一个包及其所有发布版本。
- `--version <version>` 会通过失效关闭的版本专用路由，永久删除一个自己拥有的非最新发布版本。
...
- 举报属于包级别，可选择关联到某个版本，并会对版主可见以供审核。
- 举报本身不会自动隐藏包或阻止下载。
- 标志：
...
- 显示面向操作者的包迁移状态，该包可能会替换一个内置 OpenClaw 插件。
- 调用与 `package readiness` 相同的计算型就绪状态端点，但会打印侧重迁移的状态、最新版本、官方包状态、检查项和阻塞项。
- 标志：
  - `--json`：机器可读输出。
...
- 代码插件文件夹会在上传前打包成 ClawPack npm tarball，以便 OpenClaw 安装可以验证精确 artifact。捆绑插件文件夹仍使用提取文件的发布路径。
- 对于 GitHub 源，源归属会根据仓库、解析出的 commit、ref 和子路径自动填充。
...
先使用 `--dry-run`，这样你可以在创建真实发布前确认解析出的 package 元数据和源归属：
...
外部代码插件需要在 `package.json` 中包含少量 OpenClaw 元数据。这个最小 manifest 足以成功发布：
...
- 如果你想发布更详细的兼容性元数据，`openclaw.compat.minGatewayVersion` 和 `openclaw.build.pluginSdkVersion` 是可选的额外项。
- 如果你使用的是较旧的 `clawhub` CLI 版本，请在发布前升级，以便本地预检能在上传前运行。
- 如果验证报告了修复代码，请参见 [插件验证修复](/zh-CN/clawhub/plugin-validation-fixes)。

#### GitHub Actions

ClawHub 还为插件仓库提供官方可复用工作流，位于 [`/.github/workflows/package-publish.yml`](https://github.com/openclaw/clawhub/blob/bdb23c3a9ffe77cb4184fdd13897ce535fb2d703/.github/workflows/package-publish.yml)。
...
- 对于 monorepo，传入 `source_path`，让工作流发布插件 package 文件夹，例如 `source_path: extensions/codex`。
...
- 为首次发布、不受信任的 package 或应急发布保留可用的 `clawhub_token`。
- 工作流会将 JSON 结果作为 artifact 上传，并将其作为工作流输出公开。

### `package trusted-publisher get <name>`

- 显示 package 的 GitHub Actions 可信发布者配置。
- 设置配置后使用此命令确认仓库、工作流文件名和可选的环境 pin。
- 标志：
  - `--json`：机器可读输出。
...
- 为现有 package 附加或替换 GitHub Actions 可信发布者配置。
- 必须先通过普通手动或 token 认证的 `clawhub package publish` 创建 package。
...
- ClawHub 会在此命令运行时验证已配置的 GitHub 仓库。
  公共仓库可以通过公共 GitHub 元数据验证。私有仓库要求 ClawHub 拥有该仓库的 GitHub 访问权限，例如通过未来的 ClawHub GitHub App 安装或其他已授权的 GitHub 集成。
- 标志：
...
- 从 package 中移除可信发布者配置。
- 如果需要禁用或重新创建工作流、仓库或环境 pin，请将此作为回滚方式。
...
- 登录后，在 `clawhub install <slug>` 之后发送，除非设置了 `CLAWHUB_DISABLE_TELEMETRY=1`。
