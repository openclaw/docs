...
x-i18n:
    generated_at: "2026-07-03T15:19:17Z"
    model: gpt-5.5
...
设置这些变量中的任意一个时，CLI 会通过指定代理路由出站请求。`HTTPS_PROXY` 用于 HTTPS 请求，`HTTP_PROXY` 用于普通 HTTP。`NO_PROXY` / `no_proxy` 会被遵循，用于对特定主机或域名绕过代理。

在直接出站连接被阻止的系统上需要这样做（例如 Docker 容器、仅允许代理访问互联网的 Hetzner VPS、企业防火墙）。
...
- 远程/无头交互：`clawhub login --device` 会打印一个代码，并在你到 `<site>/cli/device` 授权时等待。
...
- 适合将本地登录 token 通过管道传入 CI secret 设置命令。
...
- 对用 `clawhub pin` 冻结的技能显示 `pinned`，包括可选原因。

### `pin <skill>`

- 在 lockfile 中将已安装技能标记为 pinned。
- `--reason <text>` 记录该技能被冻结的原因。
- Pinned 技能会被 `update --all` 跳过，并被直接 `update <skill>` 拒绝。
- Pinned 技能也会拒绝 `install --force`，避免本地字节被意外替换。

### `unpin <skill>`

- 从已安装技能移除 lockfile pin，使未来更新可以修改它。
...
- 如果 fingerprint 不匹配：
  - 默认拒绝
  - 使用 `--force` 覆盖（如果是交互式，也可以通过提示确认）
- Pinned 技能永远不会被 `--force` 更新。
- `update <skill>` 对 pinned 技能会快速失败，并提示你先运行 `clawhub unpin <skill>`。
- `update --all` 会跳过 pinned slug，并打印哪些内容保持冻结的摘要。

### `skill publish <path>`

- 将本地 bundle fingerprint 与 ClawHub 比较，如果内容已发布则成功退出。
- 新技能默认使用 `1.0.0`；已变更技能默认使用下一个 patch 版本。
- `--version <version>` 显式选择版本，即使内容匹配现有版本也会发布。
- `--dry-run` 会解析发布但不上传；`--json` 打印机器可读结果。
- 当操作者拥有发布者访问权限时，`--owner <handle>` 会在某个组织/用户发布者 handle 下发布。
- `--migrate-owner` 会在发布新版本时将现有技能移动到 `--owner`。需要对两个发布者都有 admin/owner 访问权限。
- 所有者和审核行为在 `docs/publishing.md` 中说明。
- 发布一个技能意味着它在 ClawHub 上以 `MIT-0` 发布。
- 已发布技能可以自由使用、修改和再分发，无需署名。
...
ClawHub 的可复用 [`skill-publish.yml`](https://github.com/openclaw/clawhub/blob/main/.github/workflows/skill-publish.yml) workflow 会对一个 `skill_path` 调用 `skill publish`，或对 `root` 下的每个直接技能文件夹调用它（默认：`skills`）。它会跳过未变更的技能，并使用相同的自动 patch-version 行为。

设置 `dry_run: true` 可以在没有 token 的情况下预览。真实发布需要 `clawhub_token` secret。

### `sync`

- 扫描当前 workdir、已配置的 skills 目录，以及任意 `--root <dir>` 文件夹，查找包含 `SKILL.md` 或 `skill.md` 的本地技能文件夹。
- 将每个本地技能 fingerprint 与 ClawHub 比较，只发布新的或已变更的技能。
- 新技能发布为 `1.0.0`；已变更技能默认发布下一个 patch 版本。对于应该跨越更大 semver 步长的更新批次，使用 `--bump minor|major`。
- `--dry-run` 显示发布计划但不上传；`--json` 打印机器可读计划。
- `--all` 会不经提示发布每个新的或已变更的技能。如果没有 `--all`，交互式终端会让你选择要发布的技能。
- 当操作者拥有发布者访问权限时，`--owner <handle>` 会在某个组织/用户发布者 handle 下发布。
- `sync` 仅单向发布。它不会安装、更新、下载，也不会报告安装/下载遥测。
...
- 扫描是异步的，可能需要一些时间才能完成。在排队期间，终端 spinner 会显示当前优先扫描位置以及前面还有多少扫描。
- 已发布扫描需要所有权或发布者管理访问权限。版主/admin 可以通过 `clawhub-admin` 使用相同后端。
- `--update` 仅在配合 `--slug` 时有效；它会将成功的已发布扫描结果写回所选版本。
...
- 插件下载使用 package name，并需要 `--kind plugin`。
...
ClawHub 在 [`/.github/workflows/skill-publish.yml`](https://github.com/openclaw/clawhub/blob/76b4f36bb0f7409ed7cb9c6fd6f1ccf81396ee88/.github/workflows/skill-publish.yml) 提供官方可复用 workflow，适用于技能 repo 和 catalog repo。
...
- 只有当操作者对当前所有者和目标发布者都具有管理员访问权限时，转移到组织/发布者 handle 才会立即生效。
- 子命令：
...
- `--version <version>`：检查特定版本（默认值：最新）。
- `--tag <tag>`：检查带标签的版本（例如 `latest`）。
- `--versions`：列出版本历史（第一页）。
- `--limit <n>`：要列出的最大版本数（1-100）。
- `--files`：列出所选版本的文件。
- `--file <path>`：获取原始文件内容（仅限文本文件；限制 200KB）。
- `--json`：机器可读输出。
...
- 与 `--package` 一起使用时，会从 ClawHub 解析预期元数据，并将本地文件与已发布的工件元数据进行比较。
- 与直接摘要标志一起使用时，无需网络查询即可验证。
- 标志：
...
如果验证报告了包、清单、SDK 导入或工件发现，请参阅
[插件验证修复](/zh-CN/clawhub/plugin-validation-fixes)，然后重新运行该命令。

### `package delete <name>`

- 不带 `--version` 时，软删除一个包和所有发布版本。
- `--version <version>` 会通过故障关闭、特定版本的路由，永久删除一个拥有的非最新发布版本。
  已删除的版本无法恢复或重新发布。删除当前最新版本前，请先发布替代版本。此仅限版本的流程要求包所有者或组织发布者管理员执行；平台工作人员不会绕过包所有权。
- 整包软删除要求操作者是包所有者、组织发布者所有者/管理员、平台版主或平台管理员。
- 标志：
...
- 将包转移给另一个发布者。
- 除非由平台管理员执行，否则要求对当前包所有者和目标发布者都具有管理员访问权限。
- 作用域包名必须转移给匹配的作用域所有者。
...
- 显示当前包扫描状态、打开的举报数量、最新发布版本的人工审核状态、下载阻止状态以及审核原因。
- 标志：
  - `--json`：机器可读输出。
...
- 调用与 `package readiness` 相同的计算就绪端点，但打印以迁移为重点的状态、最新版本、官方包状态、检查和阻塞项。
- 标志：
  - `--json`：机器可读输出。
...
- 创建一个由已认证用户拥有的组织发布者。
- handle 会被规范化为小写，并且可带或不带 `@` 传入。
- 新创建的组织发布者默认不受信任/不是官方发布者。
...
- 元数据会从 `package.json`、`openclaw.plugin.json` 以及
  真实的 OpenClaw 包标记自动检测，例如 `.codex-plugin/plugin.json`、
  `.claude-plugin/plugin.json` 和 `.cursor-plugin/plugin.json`。
- `.tgz` 源会被视为 ClawPack。CLI 会上传精确的 npm-pack
  字节，并仅使用提取出的 `package/` 内容进行验证和
  元数据预填。
- 代码插件文件夹会在上传前打包为 ClawPack npm tarball，以便
  OpenClaw 安装可以验证精确制品。包插件文件夹仍然
  使用提取文件发布路径。
- 对于 GitHub 源，源归因会从仓库、解析后的提交、ref 和子路径自动填充。
- 对于本地文件夹，当 origin 远程指向 GitHub 时，源归因会从本地 git 自动检测。
- 外部代码插件必须显式声明 `openclaw.compat.pluginApi` 和
  `openclaw.build.openclawVersion`。
  顶层 `package.json.version` 不会作为发布验证的 fallback 使用。
- `--dry-run` 会预览解析后的发布载荷，而不上传。
- `--json` 会为 CI 输出机器可读结果。
- `--owner <handle>` 会在操作者拥有发布者访问权限时，以用户或组织发布者 handle 发布。
- 作用域包名必须匹配所选 owner。参见 `docs/publishing.md`。
...
先使用 `--dry-run`，这样你可以在创建真实发布之前确认解析后的包元数据和
源归因：
...
外部代码插件需要在
`package.json` 中包含少量 OpenClaw 元数据。这个最小 manifest 足以成功发布：
...
- `package.json.version` 是你的包发布版本，但不会作为
  OpenClaw 兼容性/构建验证的 fallback 使用。
- `openclaw.hostTargets` 和 `openclaw.environment` 是可选元数据。
  ClawHub 可能会在存在时展示它们，但发布不要求这些字段。
- 如果你想发布
  更详细的兼容性元数据，`openclaw.compat.minGatewayVersion` 和
...
- 保留可用的 `clawhub_token`，用于首次发布、不受信任的包或紧急发布。
- 该工作流会将 JSON 结果作为制品上传，并将其作为工作流输出公开。

### `package trusted-publisher get <name>`

- 显示某个包的 GitHub Actions 可信发布者配置。
- 在设置配置后使用此命令，确认仓库、工作流文件名
  和可选的 environment pin。
- 标志：
  - `--json`：机器可读输出。
...
- 为现有
  包附加或替换 GitHub Actions 可信发布者配置。
- 必须先通过普通手动或基于 token 身份验证的
  `clawhub package publish` 创建包。
- 设置配置后，未来受支持的 GitHub Actions 发布可以使用
  OIDC/可信发布，而无需长期有效的 ClawHub token。
- `--repository <repo>` 必须是 `owner/repo`。
- `--workflow-filename <file>` 必须匹配
  `.github/workflows/` 中的工作流文件名。
- `--environment <name>` 是可选的。配置后，OIDC claim 中的 GitHub Actions
  environment 必须完全匹配。
- ClawHub 会在此命令运行时验证已配置的 GitHub 仓库。
  公共仓库可以通过公共 GitHub 元数据验证。私有
  仓库要求 ClawHub 拥有对该仓库的 GitHub 访问权限，例如
  通过未来的 ClawHub GitHub App 安装或其他已授权的
  GitHub 集成。
- 标志：
...
- 从包中移除可信发布者配置。
- 如果工作流、仓库或 environment pin 需要
  禁用或重新创建，可将此命令用作回滚。
- 在重新设置配置之前，未来的真实发布必须使用普通的身份验证发布。
- 标志：
  - `--json`：机器可读输出。
...
- 登录后，在 `clawhub install <slug>` 之后发送，除非
