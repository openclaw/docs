...
这在直接出站连接被阻止的系统上是必需的（例如 Docker 容器、只能通过代理联网的 Hetzner VPS、企业防火墙）。
...
- 远程/无头交互式：`clawhub login --device` 会打印一个代码，并在你前往 `<site>/cli/device` 授权期间等待。
...
- 调用 `/api/v1/search?q=...`。
- 搜索会优先考虑精确的短标识/名称词元匹配，然后才是下载热度。像 `map` 这样的独立短标识词元会比 `amap` 内部的子串更强地匹配 `personal-map`。
- 下载量只是一个较小的热度先验，并不保证排名靠前。
- 如果某个技能应该出现但没有出现，请在登录状态下运行 `clawhub inspect <slug>`，在重命名元数据前检查所有者可见的审核诊断信息。
...
- 已固定的技能会被 `update --all` 跳过，并且会拒绝直接执行 `update <slug>`。
- 已固定的技能也会拒绝 `install --force`，因此本地字节不会被意外替换。

### `unpin <slug>`

- 从已安装的技能中移除锁文件固定标记，使后续更新可以修改它。
...
- 当操作者具有发布者访问权限时，`--owner <handle>` 会在组织/用户发布者 handle 下发布。
...
- `--clawscan-note <text>` 添加 ClawScan 备注。此备注为 ClawScan 提供上下文，用于说明可能看起来异常的行为，例如网络访问、原生主机访问，或特定提供商凭据。该备注存储在已发布版本上。
- 旧版别名：`publish <path>`。
...
- 所有者发起的软删除会为该 slug 保留 30 天；命令会打印到期时间。
...
- 只有当操作者同时具有当前所有者和目标发布者的管理员访问权限时，转移到组织/发布者 handle 才会立即生效。
- 子命令：
...
- 对插件和其他软件包系列条目使用此命令；顶层 `search` 仍然是技能搜索入口。
- 标志：
...
- 获取软件包元数据而不安装。
- 将此命令用于插件元数据、兼容性、验证、来源，以及版本/文件检查。
- `--version <version>`：检查特定版本（默认：latest）。
...
- 配合 `--package` 使用时，从 ClawHub 解析预期元数据，并将本地文件与已发布的构件元数据进行比较。
...
- 将包转移给另一个发布者。
- 需要对当前包所有者和目标发布者都具有管理员访问权限，除非由平台管理员执行。
...
- 举报本身不会自动隐藏包或阻止下载。
- 标志：
  - `--version <version>`：可选的包版本，用于关联到举报。
  - `--reason <text>`：必需的举报原因。
  - `--json`：机器可读输出。
...
- 报告官方状态、ClawPack 可用性、构件摘要、来源出处、OpenClaw 兼容性、主机目标、环境元数据和扫描状态方面的阻断项。
- 标志：
  - `--json`：机器可读输出。
...
- 显示面向操作员的迁移状态，适用于可能替换某个内置 OpenClaw 插件的包。
- 调用与 `package readiness` 相同的计算型就绪端点，但输出以迁移为中心的状态、最新版本、官方包状态、检查项和阻断项。
- 标志：
  - `--json`：机器可读输出。
...
- `.tgz` 源会被视为 ClawPack。CLI 会上传精确的 npm-pack 字节，并仅将解压出的 `package/` 内容用于验证和元数据预填。
- 代码插件文件夹会在上传前打包成 ClawPack npm tarball，以便 OpenClaw 安装可以验证精确构件。Bundle 插件文件夹仍使用解压文件发布路径。
- 对于 GitHub 源，来源归因会从仓库、解析后的提交、ref 和子路径自动填充。
...
- `--json` 会为 CI 输出机器可读结果。
- `--owner <handle>` 会在执行者具有发布者访问权限时，以用户或组织发布者句柄发布。
- `--clawscan-note <text>` 添加 ClawScan 说明。此说明为 ClawScan 提供上下文，用于解释可能看起来异常的行为，例如网络访问、原生主机访问或提供商特定凭证。该说明会存储在已发布版本上。
...
先使用 `--dry-run`，这样你可以在创建真实发布版本前确认解析后的包元数据和来源归因：
...
- `package.json.version` 是你的包发布版本，但它不会用作 OpenClaw 兼容性/构建验证的回退值。
- `openclaw.hostTargets` 和 `openclaw.environment` 是可选元数据。
  ClawHub 在存在这些元数据时可能会展示它们，但发布并不要求提供。
- 如果你想发布更详细的兼容性元数据，`openclaw.compat.minGatewayVersion` 和 `openclaw.build.pluginSdkVersion` 是可选附加项。
- 如果你使用的是较旧的 `clawhub` CLI 发布版本，请在发布前升级，以便本地预检检查在上传前运行。
...
- 无需密钥的可信发布仅适用于 `workflow_dispatch`；标签推送仍需要 `clawhub_token`。
...
- 根目录可以是任意文件夹：一个技能目录，或包含 `SKILL.md` 的单个技能文件夹。
- 当 `~/.clawdbot/clawdbot.json` 存在时，自动添加 Clawdbot 技能根目录：
