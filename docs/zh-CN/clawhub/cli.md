...
设置其中任意变量后，CLI 会通过指定代理路由出站请求。`HTTPS_PROXY` 用于 HTTPS 请求，`HTTP_PROXY` 用于普通 HTTP。`NO_PROXY` / `no_proxy` 会被遵循，以便对特定主机或域名绕过代理。
...
- 默认：打开浏览器访问 `<site>/cli/auth`，并通过回环回调完成。
- 无头：`clawhub login --token clh_...`
- 远程/无头交互：`clawhub login --device` 会打印一个代码，并在你通过 `<site>/cli/device` 授权时等待。
...
- 搜索会优先匹配精确的 slug/name 词元，然后才考虑下载热度。像 `map` 这样的独立 slug 词元会比 `amap` 中的子字符串更强地匹配 `personal-map`。
- 热度只是一个很小的排名先验，并不保证排在最前。
- 如果某个技能应当出现但没有出现，请在登录后运行 `clawhub inspect @owner/slug`，先检查所有者可见的审核诊断，再重命名元数据。
...
- `update --all` 会跳过已固定的 slug，并打印仍保持冻结的内容摘要。

### `skill publish <path>`

- 将本地包指纹与 ClawHub 比较，当内容已经发布时成功退出。
- 新技能默认为 `1.0.0`；已变更技能默认为下一个 patch 版本。
- `--version <version>` 显式选择一个版本，即使内容匹配现有版本也会发布。
- `--dry-run` 解析发布但不上传；`--json` 打印机器可读结果。
- `--owner <handle>` 在执行者有发布者访问权限时，以组织/用户发布者 handle 发布。
...
工作流会针对一个 `skill_path` 调用 `skill publish`，或者针对 `root` 下每个直接技能文件夹调用（默认：`skills`）。它会跳过未变更的技能，并使用相同的自动 patch 版本行为。

设置 `dry_run: true` 可以在没有令牌的情况下预览。真实发布需要 `clawhub_token` 密钥。

### `sync`

- 扫描当前 workdir、已配置的 skills 目录，以及任何 `--root <dir>` 文件夹，查找包含 `SKILL.md` 或 `skill.md` 的本地技能文件夹。
- 将每个本地技能指纹与 ClawHub 比较，只发布新的或已变更的技能。
- 新技能发布为 `1.0.0`；已变更技能默认发布下一个 patch 版本。对于应当按更大 semver 步长移动的更新批次，使用 `--bump minor|major`。
- `--dry-run` 显示发布计划但不上传；`--json` 打印机器可读计划。
- `--all` 发布每个新的或已变更技能且不提示。没有 `--all` 时，交互式终端允许你选择要发布的技能。
- `--owner <handle>` 在执行者有发布者访问权限时，以组织/用户发布者 handle 发布。
...
- 扫描是异步的，可能需要一些时间才能完成。排队时，终端 spinner 会显示当前优先扫描位置以及前方还有多少次扫描。
...
- `root` 对目录仓库默认使用 `skills`。
- 传入 `skill_path: skills/review-helper` 可处理一个技能文件夹。
- `owner` 映射到 CLI 的 `--owner` 标志；省略它则以已认证用户身份发布。
...
- 由所有者发起的软删除会保留该 slug 30 天；该命令会打印过期时间。
- `--version <version>` 通过一个失败即关闭、
  特定版本的路由永久删除一个归属自己的非最新版本。
  已删除的版本无法恢复或重新发布。删除当前最新版本前，请先发布替代版本。平台工作人员不会在这个仅版本流程中绕过所有权限制。
...
- 重命名归属自己的技能，并将之前的 slug 保留为重定向别名。
...
- 所有权转移工作流。
- 转移到用户 handle 时会创建一个待处理请求，由接收者接受。
- 转移到组织/发布者 handle 时，只有当操作者同时拥有当前所有者和目标发布者的
  管理员访问权限，才会立即生效。
- 子命令：
...
- 将此命令用于插件和其他包族条目；顶层 `search` 仍然是技能搜索入口。
- 标志：
...
- 对 ClawPack npm-pack 制品，还会验证 npm `sha512` 完整性、
...
- 带 `--package` 时，从 ClawHub 解析预期元数据，并将
  本地文件与已发布的制品元数据进行比较。
...
- 默认使用离线/静态验证，不定位或导入本地
  OpenClaw checkout。
- 硬性兼容性错误会以非零状态退出。仅警告发现会打印出来，但
  以零状态退出。
- 标志：
...
- `--version <version>` 通过一个失败即关闭、
  特定版本的路由永久删除一个归属自己的非最新发布版本。
  已删除的版本无法恢复或重新发布。删除当前最新版本前，请先发布替代版本。此仅版本流程要求包所有者或组织发布者
  管理员执行；平台工作人员不会绕过包所有权。
- 整个包的软删除要求包所有者、组织发布者所有者/管理员、平台
  版主或平台管理员执行。
- 标志：
...
- 显示面向操作人员的包迁移状态，该包可能替代一个
  内置 OpenClaw 插件。
- 调用与 `package readiness` 相同的计算型 readiness 端点，但打印
  以迁移为重点的状态、最新版本、官方包状态、检查项和
  阻断项。
- 标志：
  - `--json`：机器可读输出。
...
- 代码插件文件夹会在上传前打包为 ClawPack npm tarball，以便 OpenClaw 安装可以验证精确的构件。包插件文件夹仍使用提取文件发布路径。
- 对于 GitHub 源，源归属会从仓库、解析后的提交、引用和子路径自动填充。
...
先使用 `--dry-run`，这样你可以在创建实际发布前确认解析后的包元数据和源归属：
...
- `openclaw.hostTargets` 和 `openclaw.environment` 是可选元数据。ClawHub 可能会在存在时展示它们，但发布并不要求它们。
...
- 将可复用工作流固定到稳定 tag 或完整提交 SHA。不要从 `@main` 运行发布发布。
- `pull_request` 应使用 `dry_run: true`，这样 CI 不会产生污染。
- 真实发布应限制在可信事件，例如 `workflow_dispatch` 或 tag push。
- 无 secret 的可信发布仅适用于 `workflow_dispatch`；tag push 仍需要 `clawhub_token`。
- 为首次发布、不可信包或紧急发布保留 `clawhub_token`。
- 工作流会将 JSON 结果作为构件上传，并将其暴露为工作流输出。

### `package trusted-publisher get <name>`

- 显示包的 GitHub Actions 可信发布者配置。
- 设置配置后使用此命令，确认仓库、工作流文件名和可选的环境固定项。
- 标志：
  - `--json`：机器可读输出。
...
- 必须先通过普通手动或 token 认证的 `clawhub package publish` 创建包。
- 设置配置后，未来受支持的 GitHub Actions 发布可以使用 OIDC/可信发布，而不需要长期有效的 ClawHub token。
...
- ClawHub 会在运行此命令时验证配置的 GitHub 仓库。公共仓库可以通过公共 GitHub 元数据验证。私有仓库要求 ClawHub 拥有该 GitHub 仓库的访问权限，例如通过未来的 ClawHub GitHub App 安装或其他已授权 GitHub 集成。
- 标志：
