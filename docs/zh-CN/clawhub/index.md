...
title: ClawHub
x-i18n:
    generated_at: "2026-07-04T15:07:27Z"
    model: gpt-5.5
...
ClawHub 跟踪语义化版本、`latest` 等标签、变更日志、文件、下载量、星标和安全扫描摘要。公共页面会显示当前注册表状态，让用户可以在安装前检查某个 Skill 或插件。

## 原生 OpenClaw 流程

原生 OpenClaw 命令会安装到当前活动的 OpenClaw 工作区，并持久化来源元数据，以便之后的更新命令可以继续使用 ClawHub。

当插件安装应通过 ClawHub 解析时，使用 `clawhub:<package>`。在发布切换期间，裸的 npm 安全插件规格可能会通过 npm 解析；当必须显式指定来源时，`npm:<package>` 保持仅使用 npm。
...
ClawHub 默认开放：任何人都可以上传，但发布需要 GitHub 账号达到足够账号年龄才能通过上传门禁。公共详情页会在安装或下载前汇总最新扫描状态。

ClawHub 会对已发布的 Skills 和插件版本运行自动检查。被扫描保留或屏蔽的版本可能会从公共目录和安装界面消失，但其所有者仍可在 `/dashboard` 中看到。
...
当你在登录状态下运行 `clawhub install` 时，CLI 可能会尽力发送一次安装事件，以便 ClawHub 计算汇总安装数量。可用以下方式禁用：
