...
- 使用单独的 `clawhub` CLI 执行注册表认证、发布，以及删除/取消删除工作流。
...
ClawHub 跟踪 semver 版本、`latest` 等标签、变更日志、文件、下载量、星标和安全扫描摘要。公共页面会显示当前注册表状态，方便用户在安装 Skills 或插件之前检查它。

## 原生 OpenClaw 流程

原生 OpenClaw 命令会安装到当前活动的 OpenClaw 工作区，并持久化源元数据，这样后续更新命令可以继续使用 ClawHub。

当插件安装应通过 ClawHub 解析时，使用 `clawhub:<package>`。在发布切换期间，裸 npm 安全插件规格可能会通过 npm 解析；当必须显式指定源时，`npm:<package>` 仍仅使用 npm。

插件安装会在归档安装运行之前，验证声明的 `pluginApi` 和 `minGatewayVersion` 兼容性。当某个包版本发布 ClawPack 构件时，OpenClaw 会优先使用确切上传的 npm-pack `.tgz`，验证 ClawHub 摘要标头和下载字节，并记录构件元数据以供后续更新使用。
...
ClawHub 默认开放：任何人都可以上传，但发布需要 GitHub 账户足够老，能通过上传门禁。公共详情页会在安装或下载之前汇总最新扫描状态。

ClawHub 会对已发布的 Skills 和插件版本运行自动检查。被扫描保留或阻止的版本可能会从公共目录和安装表面消失，但其所有者仍可在 `/dashboard` 中看到它们。

已登录用户可以举报 Skills 和包。审核员可以审查举报、隐藏或恢复内容，并封禁滥用账户。政策和执行详情见 [安全](/zh-CN/clawhub/security)、[安全审计](/clawhub/security-audits)、[审核与账户安全](/clawhub/moderation) 和 [可接受使用](/clawhub/acceptable-usage)。

## 遥测和环境

当你在已登录状态下运行 `clawhub install` 时，CLI 可能会发送尽力而为的安装事件，以便 ClawHub 计算聚合安装数量。可通过以下方式禁用：
