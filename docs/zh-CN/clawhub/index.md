...
当你需要注册表已认证工作流（例如发布或删除/恢复删除）时，安装 ClawHub CLI：
...
ClawHub 跟踪 semver 版本、`latest` 等标签、变更日志、文件、下载量、收藏数和安全扫描摘要。公共页面会显示当前注册表状态，以便用户在安装 Skills 或插件前进行检查。

## 原生 OpenClaw 流程

原生 OpenClaw 命令会安装到活跃的 OpenClaw 工作区，并持久化来源元数据，使后续更新命令可以继续使用 ClawHub。

当插件安装应通过 ClawHub 解析时，使用 `clawhub:<package>`。裸 npm 安全插件规范在发布切换期间可能通过 npm 解析，而当来源必须明确时，`npm:<package>` 会保持仅使用 npm。

插件安装会在归档安装运行前验证声明的 `pluginApi` 和 `minGatewayVersion` 兼容性。当某个包版本发布 ClawPack 构件时，OpenClaw 会优先使用精确上传的 npm-pack `.tgz`，验证 ClawHub 摘要标头和下载字节，并记录构件元数据以用于后续更新。
...
ClawHub 默认开放：任何人都可以上传，但发布需要 GitHub 账号足够早创建以通过上传门禁。公共详情页会在安装或下载前汇总最新扫描状态。

ClawHub 会对已发布的 Skills 和插件发布版本运行自动检查。被扫描暂停或阻止的发布版本可能会从公共目录和安装表面消失，同时仍对其所有者在 `/dashboard` 中可见。

已登录用户可以举报 Skills 和包。审核员可以审查举报、隐藏或恢复内容，并封禁滥用账号。有关策略和执行细节，请参阅[安全](/zh-CN/clawhub/security)、[安全审计](/clawhub/security-audits)、[审核与账号安全](/clawhub/moderation)和[可接受使用](/zh-CN/clawhub/acceptable-usage)。
