...
title: ClawHub
x-i18n:
    generated_at: "2026-07-02T17:33:06Z"
    model: gpt-5.5
...
ClawHub 跟踪 semver 版本、`latest` 等标签、变更日志、文件、下载量、星标和安全扫描摘要。公共页面会显示当前注册表状态，让用户在安装前检查某个 Skill 或插件。

## 原生 OpenClaw 流程

原生 OpenClaw 命令会安装到当前活动的 OpenClaw 工作区，并持久化来源元数据，使后续更新命令能够继续使用 ClawHub。

当插件安装需要通过 ClawHub 解析时，请使用 `clawhub:<package>`。在发布切换期间，不带前缀且符合 npm 规范的插件规格可能会通过 npm 解析；当必须明确来源时，`npm:<package>` 会保持仅使用 npm。
...
该 CLI 也提供用于直接注册表工作流的 Skill 安装/更新命令：
...
ClawHub 默认开放：任何人都可以上传，但发布需要 GitHub 账号足够旧，才能通过上传门槛。公共详情页会在安装或下载前汇总最新扫描状态。

ClawHub 会对已发布的 Skills 和插件版本运行自动检查。被扫描保留或阻止的版本可能会从公共目录和安装入口消失，但其所有者仍可在 `/dashboard` 中看到。
...
当你在已登录状态下运行 `clawhub install` 时，CLI 可能会尽力发送一次安装事件，让 ClawHub 能够计算聚合安装数量。使用以下方式禁用：
