...
ClawHub 跟踪 semver 版本、`latest` 等标签、变更日志、文件、下载量、星标和安全扫描摘要。公共页面会显示当前注册表状态，让用户能在安装技能或插件前先进行检查。
...
插件安装会在归档安装运行前验证声明的 `pluginApi` 和 `minGatewayVersion` 兼容性。当某个包版本发布 ClawPack 构件时，OpenClaw 会优先使用准确上传的 npm-pack `.tgz`，验证 ClawHub 摘要头和下载的字节，并记录构件元数据以供后续更新使用。
...
代码插件必须在 `package.json` 中包含必需的 OpenClaw 兼容性元数据，包括 `openclaw.compat.pluginApi` 和 `openclaw.build.openclawVersion`。请参阅 [CLI](/zh-CN/clawhub/cli) 获取完整命令参考，并参阅 [Skill 格式](/clawhub/skill-format) 获取技能元数据。
...
已登录用户可以举报 Skills 和包。审核员可以审查举报、隐藏或恢复内容，并封禁滥用账号。请参阅 [安全](/zh-CN/clawhub/security)、[安全审计](/clawhub/security-audits)、[审核与账号安全](/clawhub/moderation) 和 [可接受使用](/zh-CN/clawhub/acceptable-usage) 了解策略和执行细节。
