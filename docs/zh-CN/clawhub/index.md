...
ClawHub 跟踪 semver 版本、`latest` 等标签、变更日志、文件、下载量、星标和安全扫描摘要。公共页面显示当前注册表状态，以便用户在安装前检查某个 Skill 或插件。

## 原生 OpenClaw 流程

原生 OpenClaw 命令会安装到当前活动的 OpenClaw 工作区，并持久保存来源元数据，以便后续更新命令可以继续使用 ClawHub。

当插件安装应通过 ClawHub 解析时，使用 `clawhub:<package>`。在发布切换期间，裸 npm 安全插件规格可能会通过 npm 解析；当来源必须明确时，`npm:<package>` 保持仅使用 npm。
...
使用 `--dry-run` 构建精确的发布计划而不上传，使用 `--json` 获得便于 CI 使用的输出。
...
ClawHub 会对已发布的 Skills 和插件版本运行自动检查。被扫描保留或拦截的版本可能会从公共目录和安装表面消失，但其所有者仍可在 `/dashboard` 中看到。
...
当你在已登录状态下运行 `clawhub install` 时，CLI 可能会尽力发送安装事件，以便 ClawHub 计算汇总安装数量。使用以下方式禁用：
