...
title: ClawHub
x-i18n:
    generated_at: "2026-07-05T04:43:25Z"
    model: gpt-5.5
...
ClawHub 跟踪语义化版本、`latest` 等标签、变更日志、文件、下载量、星标和安全扫描摘要。公共页面会显示当前注册表状态，以便用户在安装 Skills 或插件前进行检查。
...
使用 `--dry-run` 构建准确的发布计划而不上传，使用 `--json` 输出适合 CI 的结果。
...
ClawHub 默认开放：任何人都可以上传，但发布需要一个足够旧、能通过上传门禁的 GitHub 账号。公共详情页面会在安装或下载前汇总最新扫描状态。

ClawHub 会对已发布的 Skills 和插件版本运行自动检查。被扫描暂扣或被阻止的版本可能会从公共目录和安装表面消失，但其所有者仍可在 `/dashboard` 中看到。

已登录用户可以举报 Skills 和包。审核员可以审查举报、隐藏或恢复内容，并封禁滥用账号。有关政策和执行细节，请参阅[安全](/clawhub/security)、[安全审计](/zh-CN/clawhub/security-audits)、[审核和账号安全](/clawhub/moderation)以及[可接受使用](/clawhub/acceptable-usage)。
...
更深入的参考资料见[遥测](/zh-CN/clawhub/telemetry)、[HTTP API](/clawhub/http-api)和[故障排除](/clawhub/troubleshooting)。
