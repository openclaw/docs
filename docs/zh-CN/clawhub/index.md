...
- 使用单独的 `clawhub` CLI 进行注册表认证、发布，以及删除/取消删除工作流。
...
当插件安装应通过 ClawHub 解析时，使用 `clawhub:<package>`。裸 npm 安全插件规格可能会在启动切换期间通过 npm 解析，而当必须显式指定来源时，`npm:<package>` 会保持仅使用 npm。
...
CLI 也提供 Skills 安装/更新命令，用于直接的注册表工作流：
...
ClawHub 默认开放：任何人都可以上传，但发布需要 GitHub 账号达到足够的注册时长以通过上传门槛。公共详情页会在安装或下载前汇总最新扫描状态。

ClawHub 会对已发布的 Skills 和插件版本运行自动检查。被扫描保留或阻止的版本可能会从公共目录和安装表面消失，但其所有者仍可在 `/dashboard` 中看到。

已登录用户可以举报 Skills 和包。审核员可以审查举报、隐藏或恢复内容，并封禁滥用账号。政策和执行细节见 [安全](/zh-CN/clawhub/security)、[安全审计](/clawhub/security-audits)、[审核与账号安全](/clawhub/moderation) 和 [可接受使用](/zh-CN/clawhub/acceptable-usage)。
