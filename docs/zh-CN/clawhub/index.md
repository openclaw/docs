...
ClawHub 跟踪 semver 版本、`latest` 等标签、变更日志、文件、下载量、星标和安全扫描摘要。公共页面会显示当前注册表状态，让用户在安装 Skill 或插件前可以先检查。

## 原生 OpenClaw 流程

原生 OpenClaw 命令会安装到当前活动的 OpenClaw 工作区，并持久化源元数据，以便后续更新命令可以继续使用 ClawHub。
...
ClawHub 会对已发布的 Skills 和插件发布版运行自动检查。被扫描暂扣或阻止的发布版可能会从公共目录和安装表面消失，但其所有者仍可在 `/dashboard` 中看到。

已登录用户可以举报 Skills 和包。审核员可以审查举报、隐藏或恢复内容，并封禁滥用账号。策略和执行详情见 [安全](/zh-CN/clawhub/security)、[安全审计](/clawhub/security-audits)、[审核和账号安全](/clawhub/moderation) 以及 [可接受使用](/zh-CN/clawhub/acceptable-usage)。

## 遥测和环境

当你在已登录状态下运行 `clawhub install` 时，CLI 可能会发送尽力而为的安装事件，以便 ClawHub 计算汇总安装次数。可通过以下方式禁用：
