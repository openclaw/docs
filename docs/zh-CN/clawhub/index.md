...
- 使用单独的 `clawhub` CLI 处理注册表认证、发布，以及删除/恢复删除工作流。
...
当插件安装应通过 ClawHub 解析时，请使用 `clawhub:<package>`。在发布切换期间，裸 npm 安全插件规格可能会通过 npm 解析；当来源必须显式指定时，`npm:<package>` 仍然只使用 npm。

插件安装会在归档安装运行之前验证所声明的 `pluginApi` 和 `minGatewayVersion` 兼容性。当某个包版本发布 ClawPack 构件时，OpenClaw 会优先使用精确上传的 npm-pack `.tgz`，验证 ClawHub 摘要标头和下载字节，并记录构件元数据以供后续更新使用。
...
使用 `--dry-run` 构建精确的发布计划而不上传，使用 `--json` 生成适合 CI 的输出。
...
ClawHub 默认开放：任何人都可以上传，但发布需要 GitHub 账号足够旧以通过上传门禁。公共详情页面会在安装或下载前汇总最新扫描状态。

ClawHub 会对已发布的 Skills 和插件版本运行自动检查。被扫描保留或阻止的版本可能会从公共目录和安装表面消失，同时仍会在 `/dashboard` 中对其所有者可见。

已登录用户可以举报 Skills 和包。审核员可以审查举报、隐藏或恢复内容，并封禁滥用账号。政策和执行详情请参阅 [安全](/zh-CN/clawhub/security)、[安全审计](/clawhub/security-audits)、[审核和账号安全](/clawhub/moderation) 以及 [可接受使用](/clawhub/acceptable-usage)。

## 遥测和环境

当你在登录状态下运行 `clawhub install` 时，CLI 可能会尽力发送安装事件，以便 ClawHub 计算聚合安装数量。使用以下方式禁用：
...
更深入的参考资料请参阅 [遥测](/clawhub/telemetry)、[HTTP API](/clawhub/http-api) 和 [故障排查](/zh-CN/clawhub/troubleshooting)。
