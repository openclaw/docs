...
summary: 面向公开使用的 ClawHub 概览，用于发现、安装、发布、安全以及 clawhub CLI。
title: ClawHub
x-i18n:
...
ClawHub 跟踪 semver 版本、`latest` 等标签、变更日志、文件、下载量、星标和安全扫描摘要。公共页面显示当前注册表状态，便于用户在安装 Skills 或插件前进行检查。

## 原生 OpenClaw 流程

原生 OpenClaw 命令会安装到当前活动的 OpenClaw 工作区，并持久化来源元数据，让后续更新命令可以继续使用 ClawHub。

当插件安装应通过 ClawHub 解析时，使用 `clawhub:<package>`。在发布切换期间，符合 npm 命名规则的裸插件规格可能会通过 npm 解析；当必须显式指定来源时，`npm:<package>` 会保持仅使用 npm。
...
ClawHub 会对已发布的 Skills 和插件版本运行自动检查。被扫描保留或被阻止的版本可能会从公共目录和安装界面中消失，但其所有者仍可在 `/dashboard` 中看到。
...
当你在已登录状态下运行 `clawhub install` 时，该 CLI 可能会发送尽力而为的安装事件，让 ClawHub 可以计算聚合安装次数。使用以下方式禁用：
