...
x-i18n:
    generated_at: "2026-07-01T18:07:04Z"
    model: gpt-5.5
...
ClawHub 读取 `SKILL.md` frontmatter 来了解技能名称、
描述、要求、环境变量和元数据。准确的
...
当你希望注册表成为可信来源时，请使用显式的 ClawHub 安装源：
...
使用 dry run 在上传前预览已解析的载荷。随后，公开页面会
...
更新工作流，适合希望在完整 OpenClaw 工作区之外使用注册表管理技能文件夹的用户。

## 安全状态

ClawHub 对发布开放，但版本仍然受上传门禁、
自动检查、用户报告和审核员操作约束。
