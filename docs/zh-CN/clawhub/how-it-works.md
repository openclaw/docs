...
描述、要求、环境变量和元数据。准确的元数据很重要，
因为它可以帮助用户决定是否安装该技能，并帮助自动扫描检测
声明行为与观察到的行为之间的不匹配。
...
元数据。包记录可以包含 API 兼容性、最低 Gateway 网关版本、
主机目标、环境要求和构件摘要。
...
使用 dry run 在上传前预览解析后的载荷。随后，公开页面会展示
...
OpenClaw 会记录安装源元数据，以便之后的更新可以解析同一个
注册表包。对于希望在完整 OpenClaw 工作区之外使用注册表管理的
技能文件夹的用户，ClawHub CLI 也支持直接的技能安装和更新工作流。
...
公开页面会在可用时显示扫描摘要。被保留、隐藏或阻止的内容
可能会从公开搜索和安装流程中消失，同时仍对所有者可见，
用于诊断。

请参阅 [安全](/clawhub/security)、[安全审计](/clawhub/security-audits)、
[审核和账号安全](/zh-CN/clawhub/moderation) 以及
[可接受使用](/clawhub/acceptable-usage)。

## API 访问

ClawHub 暴露公共读取 API，用于发现、搜索、包详情和
下载。第三方目录可以使用这些 API，前提是它们链接回
规范的 ClawHub 条目、遵守速率限制，并避免暗示背书。

请参阅 [公共 API](/zh-CN/clawhub/api) 和 [HTTP API](/clawhub/http-api)。
