...
x-i18n:
    generated_at: "2026-07-04T06:21:08Z"
    model: gpt-5.5
...
ClawHub 读取 `SKILL.md` frontmatter，以了解技能名称、描述、要求、环境变量和元数据。准确的元数据很重要，因为它能帮助用户决定是否安装该技能，并帮助自动扫描检测声明行为与观察到的行为之间的不匹配。
...
OpenClaw 会记录安装源元数据，以便之后更新时能解析同一个注册表软件包。对于希望在完整 OpenClaw 工作区之外使用注册表托管技能文件夹的用户，ClawHub CLI 也支持直接的技能安装和更新工作流。
