...
read_when:
    - 了解列表项、版本、安装、发布和审核
summary: ClawHub 列表、版本、安装、发布、扫描和更新如何运作。
x-i18n:
...
条目页面是用户在安装前查看某个技能或插件声称能做什么的规范位置。
...
OpenClaw 会记录安装源元数据，以便后续更新能够解析同一个注册表包。对于希望在完整 OpenClaw 工作区之外使用注册表管理的技能文件夹的用户，ClawHub CLI 也支持直接的技能安装和更新工作流。

## 安全状态

ClawHub 对发布开放，但发布版本仍然受上传门控、自动检查、用户报告和管理员操作约束。
...
ClawHub 提供用于发现、搜索、包详情和下载的公开读取 API。第三方目录可以使用这些 API，前提是它们链接回规范的 ClawHub 条目、遵守速率限制，并避免暗示背书。
