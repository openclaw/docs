...
x-i18n:
    generated_at: "2026-07-05T04:48:31Z"
    model: gpt-5.5
...
注册表包。对于希望在完整 OpenClaw 工作区之外使用注册表管理技能文件夹的
用户，ClawHub CLI 也支持直接的技能安装和
更新工作流。

## 安全状态

ClawHub 对发布开放，但发布版本仍需经过上传关卡、
自动检查、用户报告和审核员处理。

公开页面会在可用时显示扫描摘要。被暂挂、隐藏
或阻止的内容可能会从公开搜索和安装流程中消失，同时仍然
对所有者可见，以便进行诊断。
...
ClawHub 暴露用于发现、搜索、包详情和
下载的公开读取 API。第三方目录可以使用这些 API，前提是它们链接回
权威 ClawHub 列表，遵守速率限制，并避免暗示认可。
