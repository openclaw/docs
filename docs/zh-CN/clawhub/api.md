...
你可以基于 ClawHub 的公共只读 API 构建第三方目录、索引或搜索界面。公共技能元数据和技能文件会按照 ClawHub 的技能许可规则发布，而 API 本身有速率限制，应负责任地使用。
...
- `X-RateLimit-Remaining` / `RateLimit-Remaining`：存在时表示精确剩余预算；分片成功请求会省略它，而不是返回近似的全局值
...
- `POST /api/v1/skills`（发布，推荐使用 multipart）
