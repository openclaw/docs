...
x-i18n:
    generated_at: "2026-06-27T10:14:53Z"
    model: gpt-5.5
...
你可以基于 ClawHub 的公共读取 API 构建第三方目录、索引或搜索界面。公共 skill 元数据和 skill 文件会根据 ClawHub 的 skill 许可规则发布，而 API 本身有速率限制，应负责任地使用。
...
- `X-RateLimit-Reset`：Unix epoch 秒数（绝对重置时间）
- `RateLimit-Reset`：距离重置的延迟秒数
- `X-RateLimit-Remaining` / `RateLimit-Remaining`：存在时表示准确的剩余额度；分片的成功请求会省略它，而不是返回近似的全局值
...
- `POST /api/v1/users/reserve` 为 owner handle 保留根 slug 和私有的无发布 package 占位符。
