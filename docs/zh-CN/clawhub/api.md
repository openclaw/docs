...
x-i18n:
    generated_at: "2026-07-03T13:14:53Z"
    model: gpt-5.5
...
你可以基于 ClawHub 的公共读取 API 构建第三方目录、索引或搜索界面。公共 Skills 元数据和 Skills 文件按 ClawHub 的 Skills 许可规则发布，而 API 本身有速率限制，应负责任地使用。

指南：

- 对目录列表使用公共读取端点，例如 `GET /api/v1/skills`、`GET /api/v1/search` 和 `GET /api/v1/skills/{slug}`。
- 缓存响应，并遵守 `429`、`Retry-After` 和速率限制标头，而不是激进轮询。
- 显示列表时链接回规范的 ClawHub Skills URL，让用户可以检查源注册表记录。
...
- `GET /api/v1/download?slug=&version=&tag=`
  ...
  - 当前由 GitHub 支持且扫描结果为 `clean` 或 `suspicious` 的 Skills 会返回 JSON `public-github` 移交描述符，而不是 ClawHub 字节。
...
- `POST /api/v1/skills`（发布，优先使用 multipart）
