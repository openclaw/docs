...
你可以基于 ClawHub 的公共读取 API 构建第三方目录、列表或搜索界面。公共技能元数据和技能文件会按 ClawHub 的技能许可规则发布，而 API 本身有速率限制，应负责任地使用。

指南：

- 使用公共读取端点（如 `GET /api/v1/skills`、`GET /api/v1/search` 和 `GET /api/v1/skills/{slug}`）获取目录列表。
- 缓存响应，并遵守 `429`、`Retry-After` 和速率限制标头，而不是高频轮询。
- 显示列表时链接回规范 ClawHub 技能 URL，以便用户检查源注册表记录。
...
`429` 会包含 `X-RateLimit-Remaining`、`RateLimit-Remaining` 和 `Retry-After`。

语义：

- `X-RateLimit-Reset`：Unix 纪元秒数（绝对重置时间）
- `RateLimit-Reset`：距离重置的延迟秒数
- `X-RateLimit-Remaining` / `RateLimit-Remaining`：存在时为精确剩余额度；分片的成功请求会省略它，而不是返回近似的全局值
