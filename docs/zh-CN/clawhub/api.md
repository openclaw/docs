...
- `GET /api/v1/download?slug=&version=&tag=`
  - 托管 Skills 会返回确定性的 ZIP 字节。
  - 当前由 GitHub 支持、且扫描结果为 `clean` 或 `suspicious` 的 Skills 会返回 JSON `public-github` 交接描述符，而不是 ClawHub 字节。
- `GET /api/v1/skills/export?startDate=&endDate=&limit=&cursor=`
  - 托管 Skills 会作为已存储文件导出。
  - 当前由 GitHub 支持、且扫描结果为 `clean` 或 `suspicious` 的 Skills 会作为 `public-github` 交接描述符导出。
...
- `POST /api/v1/users/reserve` 会为所有者 handle 保留根 slug 和私有的无发布版本 package 占位符。
