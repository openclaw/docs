...
x-i18n:
    generated_at: "2026-06-27T01:30:08Z"
    model: gpt-5.5
...
你可以基于 ClawHub 的公开读取 API 构建第三方目录、索引或搜索界面。公开技能元数据和技能文件会按 ClawHub 的技能许可规则发布，而 API 本身有速率限制，应负责任地使用。

指南：

- 使用 `GET /api/v1/skills`、`GET /api/v1/search` 和 `GET /api/v1/skills/{slug}` 等公开读取端点获取目录列表。
- 缓存响应并遵守 `429`、`Retry-After` 和速率限制标头，避免高频轮询。
- 展示列表时链接回规范的 ClawHub 技能 URL，让用户可以检查源注册表记录。
- 使用 `https://clawhub.ai/<owner>/skills/<slug>` 形式的规范页面 URL。
- 不要暗示 ClawHub 背书、验证或运营该第三方站点。
- 不要通过绕过公开 API 过滤器或身份验证边界来镜像隐藏、私有或被审核屏蔽的内容。
...
- `POST /api/v1/users/reserve` 为所有者 handle 预留根 slug 和私有无发布版本包占位符。
