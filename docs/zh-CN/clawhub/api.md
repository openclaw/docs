...
你可以基于 ClawHub 的公共读取 API 构建第三方目录、索引或搜索界面。公共技能元数据和技能文件会按照 ClawHub 的技能许可规则发布，而 API 本身有速率限制，应以负责任的方式使用。

指南：

- 使用公共读取端点，例如 `GET /api/v1/skills`、`GET /api/v1/search` 和 `GET /api/v1/skills/{slug}` 来获取目录列表。
- 缓存响应并遵守 `429`、`Retry-After` 和速率限制标头，而不是激进轮询。
...
- v1 错误为纯文本（`text/plain; charset=utf-8`），包括 `400`、`401`、`403`、`404`、`429` 和被阻止的下载响应。
- 为了兼容性，未知查询参数会被忽略。
- 已知查询参数的值无效时会返回 `400`。
...
- `GET /api/v1/skills?limit=&cursor=&sort=`
  ...
  - 旧版别名：`nonSuspicious=true`
  - 使用 `nonSuspiciousOnly=true` 时，基于游标的页面可能包含少于 `limit` 的条目；使用 `nextCursor` 继续。
...
- `POST /api/v1/users/reserve` 为所有者 handle 预留根 slug 和私有无发布包占位符。
