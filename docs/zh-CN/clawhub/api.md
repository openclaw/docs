...
x-i18n:
    generated_at: "2026-07-05T04:48:34Z"
    model: gpt-5.5
...
你可以基于 ClawHub 的公共只读 API 构建第三方目录、索引或搜索界面。公共技能元数据和技能文件会按 ClawHub 的技能许可规则发布，而 API 本身有速率限制，应负责任地使用。

准则：

- 使用公共只读端点，例如 `GET /api/v1/skills`、`GET /api/v1/search` 和 `GET /api/v1/skills/{slug}` 来获取目录列表。
- 缓存响应，并遵守 `429`、`Retry-After` 和速率限制标头，不要激进轮询。
- 显示列表时链接回规范的 ClawHub 技能 URL，让用户可以检查源注册表记录。
