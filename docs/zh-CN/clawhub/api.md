...
x-i18n:
    generated_at: "2026-05-10T19:24:34Z"
    model: gpt-5.5
    provider: openai
...
- `GET /api/v1/skills?limit=&cursor=&sort=`
  ...
  - 旧版别名：`nonSuspicious=true`
  - 使用 `nonSuspiciousOnly=true` 时，基于 cursor 的页面包含的项目数可能少于 `limit`；使用 `nextCursor` 继续。
...
- `POST /api/v1/users/reserve` 为所有者 handle 保留根 slugs 和私有无发布包占位符。
