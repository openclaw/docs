...
x-i18n:
    generated_at: "2026-07-01T07:50:35Z"
    model: gpt-5.5
...
你可以基于 ClawHub 的公开读取 API 构建第三方目录、索引或搜索界面。公开的 skill 元数据和 skill 文件会按照 ClawHub 的 skill 许可规则发布，而 API 本身有速率限制，应负责任地使用。

指南：

- 使用公开读取端点，例如 `GET /api/v1/skills`、`GET /api/v1/search` 和 `GET /api/v1/skills/{slug}` 来生成目录列表。
- 缓存响应，并遵守 `429`、`Retry-After` 和速率限制标头，而不是激进轮询。
- 显示列表时链接回规范的 ClawHub skill URL，以便用户检查源注册表记录。
- 使用 `https://clawhub.ai/<owner>/skills/<slug>` 形式的规范页面 URL。
- 不要暗示 ClawHub 认可、验证或运营该第三方站点。
- 不要通过绕过公开 API 过滤器或认证边界来镜像隐藏、私有或被审核屏蔽的内容。
...
- 否则使用 `RateLimit-Reset`，或根据 `X-RateLimit-Reset` 推导延迟。
- 为重试添加抖动。

## 错误

- v1 错误是纯文本（`text/plain; charset=utf-8`），包括 `400`、`401`、`403`、`404`、`429` 以及被屏蔽的下载响应。
- 未知查询参数会被忽略以保持兼容性。
- 已知查询参数的值无效时会返回 `400`。
...
- `GET /api/v1/skills?limit=&cursor=&sort=`
  - `sort`：`updated`（默认）、`recommended`（`default`）、`createdAt`（`newest`）、`downloads`、`stars`（`rating`），旧版安装别名 `installsCurrent`/`installs`/`installsAllTime` 会映射到 `downloads`，`trending`
  - 无效的 `sort` 值会返回 `400`
  - `cursor` 适用于非 `trending` 排序
  - 可选过滤器：`nonSuspiciousOnly=true`
  - 旧版别名：`nonSuspicious=true`
  - 使用 `nonSuspiciousOnly=true` 时，基于 cursor 的页面可能包含少于 `limit` 的条目；使用 `nextCursor` 继续。
...
- `GET /api/v1/download?slug=&version=&tag=`
  - 托管的 skills 会返回确定性的 ZIP 字节。
  - 当前由 GitHub 支持且扫描结果为 `clean` 或 `suspicious` 的 skills 会返回 JSON `public-github` 交接描述符，而不是 ClawHub 字节。
- `GET /api/v1/skills/export?startDate=&endDate=&limit=&cursor=`
  - 托管的 skills 会导出为已存储文件。
  - 当前由 GitHub 支持且扫描结果为 `clean` 或 `suspicious` 的 skills 会导出为 `public-github` 交接描述符。
...
- `POST /api/v1/users/reserve` 会为所有者 handle 保留根 slug 和私有无发布 package 占位符。
