...
x-i18n:
    generated_at: "2026-07-04T10:26:12Z"
    model: gpt-5.5
...
你可以基于 ClawHub 的公共读取 API 构建第三方目录、索引或搜索界面。公共技能元数据和技能文件按照 ClawHub 的技能许可规则发布，而 API 本身有速率限制，应负责任地使用。
...
- 匿名请求：按 IP。
- 已认证请求（有效 Bearer token）：按用户配额桶。
- 缺失/无效 token 会回退到按 IP 限制。
...
- `GET /api/v1/skills?limit=&cursor=&sort=`
  ...
  - 旧版别名：`nonSuspicious=true`
  - 使用 `nonSuspiciousOnly=true` 时，基于 cursor 的页面可能少于 `limit` 项；使用 `nextCursor` 继续。
