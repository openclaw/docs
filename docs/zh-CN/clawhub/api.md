...
x-i18n:
    generated_at: "2026-07-05T01:53:55Z"
    model: gpt-5.5
...
你可以基于 ClawHub 的公共读取 API 构建第三方目录、索引或搜索界面。公共技能元数据和技能文件按 ClawHub 的技能许可证规则发布，而 API 本身有速率限制，应负责任地使用。

指南：

- 使用 `GET /api/v1/skills`、`GET /api/v1/search` 和 `GET /api/v1/skills/{slug}` 等公共读取端点来获取目录列表。
- 缓存响应，并遵守 `429`、`Retry-After` 和速率限制标头，避免激进轮询。
- 展示列表时链接回规范的 ClawHub 技能 URL，方便用户检查源注册记录。
...
- `GET /api/v1/download?slug=&version=&tag=`
  - 托管技能会返回确定性的 ZIP 字节。
  - 当前由 GitHub 支撑且扫描结果为 `clean` 或 `suspicious` 的技能会返回 JSON `public-github` 交接描述符，而不是 ClawHub 字节。
- `GET /api/v1/skills/export?startDate=&endDate=&limit=&cursor=`
  - 托管技能会导出为已存储的文件。
  - 当前由 GitHub 支撑且扫描结果为 `clean` 或 `suspicious` 的技能会导出为 `public-github` 交接描述符。
...
- `POST /api/v1/users/reserve` 为所有者 handle 保留根 slug 和私有无发布 package 占位符。
