...
你可以基于 ClawHub 的公共读取 API 构建第三方目录、索引或搜索界面。公共 Skills 元数据和 Skills 文件会根据 ClawHub 的 Skills 许可规则发布，而 API 本身有速率限制，应负责任地使用。
...
- `POST /api/v1/users/reserve` 为 owner handle 预留根 slug 和私有无发布包占位符。
