...
- 展示列表时链接回规范的 ClawHub skill URL，以便用户检查源注册记录。
...
- `GET /api/v1/skills?limit=&cursor=&sort=`
  ...
  - 旧版别名：`nonSuspicious=true`
  - 使用 `nonSuspiciousOnly=true` 时，基于 cursor 的页面可能包含少于 `limit` 个条目；使用 `nextCursor` 继续。
...
- `POST /api/v1/users/reserve` 为所有者 handle 保留根 slugs 和私有无发布 package 占位符。
