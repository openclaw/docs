...
x-i18n:
    generated_at: "2026-07-01T10:56:32Z"
    model: gpt-5.5
...
你可以基于 ClawHub 的公共读取 API 构建第三方目录、索引或搜索界面。公共技能元数据和技能文件会按照 ClawHub 的技能许可规则发布，而 API 本身有速率限制，应负责任地使用。

指南：

- 使用公共读取端点，例如 `GET /api/v1/skills`、`GET /api/v1/search` 和 `GET /api/v1/skills/{slug}` 来获取目录列表。
- 缓存响应，并尊重 `429`、`Retry-After` 和速率限制标头，而不是激进轮询。
...
- 匿名请求：按 IP。
- 已身份验证的请求（有效 Bearer token）：按用户桶。
- 缺失或无效 token 会回退到按 IP 强制执行。
...
- `GET /api/v1/skills/export?startDate=&endDate=&limit=&cursor=`
  - 托管技能会作为已存储文件导出。
  - 当前由 GitHub 支持且扫描结果为 `clean` 或 `suspicious` 的技能会作为 `public-github` 交接描述符导出。
...
- `POST /api/v1/users/reserve` 会为 owner handle 保留根 slug 和私有无发布包占位符。
