...
- 不要通过绕过公共 API 过滤器或身份验证边界来镜像隐藏、私有或被审核阻止的内容。
...
`X-RateLimit-Remaining`、`RateLimit-Remaining` 和 `Retry-After` 会在 `429` 时包含。
...
- `POST /api/v1/users/reserve` 会为所有者句柄预留根 slug 和私有无发布包占位符。
