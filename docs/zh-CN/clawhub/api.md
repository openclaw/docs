...
你可以基于 ClawHub 的公共读取 API 构建第三方目录、索引或搜索界面。公共 Skills 元数据和 Skills 文件依据 ClawHub 的 Skills 许可规则发布，而 API 本身设有速率限制，应负责任地使用。

指南：

- 使用公共读取端点（例如 `GET /api/v1/skills`、`GET /api/v1/search` 和 `GET /api/v1/skills/{slug}`）获取目录列表。
- 缓存响应并遵循 `429`、`Retry-After` 和速率限制标头，而不是频繁轮询。
- 显示列表时链接回规范的 ClawHub Skills URL，以便用户查看源注册表记录。
...
- 匿名请求：按 IP。
- 已通过身份验证的请求（有效的 Bearer 令牌）：按用户配额桶。
- 缺失或无效的令牌将回退到按 IP 实施限制。
...
- `X-RateLimit-Remaining` / `RateLimit-Remaining`：存在时表示确切的剩余配额；分片请求成功时会省略该值，而不是返回近似的全局值
- `Retry-After`：收到 `429` 时应等待的延迟秒数
...
- 否则使用 `RateLimit-Reset`，或根据 `X-RateLimit-Reset` 推导延迟时间。
- 为重试添加抖动。

## 错误

- v1 错误采用纯文本（`text/plain; charset=utf-8`），包括 `400`、`401`、`403`、`404`、`429` 和下载被阻止时的响应。
- 为保持兼容性，未知查询参数会被忽略。
- 值无效的已知查询参数会返回 `400`。
...
- `GET /api/v1/skills?limit=&cursor=&sort=`
  ...
  - 旧版别名：`nonSuspicious=true`
  - 使用 `nonSuspiciousOnly=true` 时，基于游标的页面包含的条目可能少于 `limit` 个；请使用 `nextCursor` 继续。
...
- `POST /api/v1/users/reserve` 为所有者用户名保留根 slug 和无发布版本的私有软件包占位符。
