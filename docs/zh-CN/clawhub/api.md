...
x-i18n:
    generated_at: "2026-05-11T20:22:19Z"
    model: gpt-5.5
    provider: openai
...
你可以基于 ClawHub 的公开读取 API 构建第三方目录、索引或搜索界面。公开技能元数据和技能文件会按照 ClawHub 的技能许可规则发布，而 API 本身有速率限制，应负责任地使用。

指南：

- 使用公开读取端点，例如 `GET /api/v1/skills`、`GET /api/v1/search` 和 `GET /api/v1/skills/{slug}` 来生成目录列表。
- 缓存响应，并遵守 `429`、`Retry-After` 和速率限制标头，而不是激进地轮询。
- 展示列表时链接回规范的 ClawHub 技能 URL，以便用户检查源注册表记录。
- 使用格式为 `https://clawhub.ai/<owner>/<slug>` 的规范页面 URL。
...
- `POST /api/v1/users/reserve` 为所有者句柄保留根 slug 和私有没有发布版本的软件包占位符。
