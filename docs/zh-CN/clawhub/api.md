...
你可以基于 ClawHub 的公共读取 API 构建第三方目录、索引或搜索界面。公共 skill 元数据和 skill 文件会按照 ClawHub 的 skill 许可规则发布，而 API 本身有速率限制，应负责任地使用。

指南：

- 使用公共读取端点，例如 `GET /api/v1/skills`、`GET /api/v1/search` 和 `GET /api/v1/skills/{slug}` 来展示目录列表。
- 缓存响应，并遵守 `429`、`Retry-After` 和速率限制标头，避免激进轮询。
- 显示列表时链接回规范的 ClawHub skill URL，以便用户检查源注册表记录。
- 使用 `https://clawhub.ai/<owner>/<slug>` 形式的规范页面 URL。
...
- 匿名请求：按 IP。
- 已认证请求（有效的 Bearer token）：按用户桶。
- 缺失/无效 token 会回退到按 IP 执行限制。

- 读取：每个 IP 600/分钟，每个 key 2400/分钟
- 写入：每个 IP 45/分钟，每个 key 180/分钟
...
- `POST /api/v1/users/reserve` 为 owner handle 预留根 slug 和私有无发布 package 占位符。
