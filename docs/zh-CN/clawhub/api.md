...
你可以基于 ClawHub 的公共读取 API 构建第三方目录、索引或搜索界面。公共 Skills 元数据和 Skills 文件会按照 ClawHub 的 Skills 许可规则发布，而 API 本身有速率限制，应负责任地使用。

指南：

- 使用公共读取端点，例如 `GET /api/v1/skills`、`GET /api/v1/search` 和 `GET /api/v1/skills/{slug}` 来获取目录列表。
- 缓存响应，并遵守 `429`、`Retry-After` 和速率限制标头，而不是激进轮询。
- 展示列表时链接回规范的 ClawHub Skills URL，以便用户检查源注册记录。
...
- `POST /api/v1/users/reserve` 会为所有者 handle 保留根 slug 和私有未发布 package 占位符。
