...
你可以基于 ClawHub 的公共读取 API 构建第三方目录、索引或搜索界面。公共 Skills 元数据和 Skills 文件依据 ClawHub 的 Skills 许可规则发布，而 API 本身有速率限制，应负责任地使用。

指南：

- 使用 `GET /api/v1/skills`、`GET /api/v1/search` 和 `GET /api/v1/skills/{slug}` 等公共读取端点获取目录列表。
...
出现 `429` 时还会包含 `X-RateLimit-Remaining`、`RateLimit-Remaining` 和 `Retry-After`。

语义：

- `X-RateLimit-Reset`：Unix 纪元秒数（绝对重置时间）
- `RateLimit-Reset`：距离重置的延迟秒数
- `X-RateLimit-Remaining` / `RateLimit-Remaining`：存在时表示确切的剩余配额；对于经过分片处理的成功请求，将省略该值，而不是返回近似的全局值
...
- `GET /api/v1/download?slug=&version=&tag=`
  ...
  - 当前由 GitHub 支持且扫描结果为 `clean` 或 `suspicious` 的 Skills 返回 JSON `public-github` 移交描述符，而不是 ClawHub 字节。
...
- `POST /api/v1/users/reserve` 为所有者标识预留根 slug 和私有的无发行版软件包占位符。
