...
x-i18n:
    generated_at: "2026-07-01T20:11:25Z"
    model: gpt-5.5
...
- v1 错误是纯文本（`text/plain; charset=utf-8`），包括 `400`、`401`、`403`、`404`、`429` 和下载被拦截的响应。
- 为兼容性，未知查询参数会被忽略。
- 已知查询参数的无效值会返回 `400`。
...
- `POST /api/v1/users/reserve` 为所有者 handle 预留根 slug 和私有的无发布 package 占位符。
