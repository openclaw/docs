...
你可以在 ClawHub 的公共读取 API 之上构建第三方目录、索引或搜索界面。公共技能元数据和技能文件按照 ClawHub 的技能许可规则发布，而 API 本身有速率限制，应负责任地使用。

指南：

- 使用公共读取端点，例如 `GET /api/v1/skills`、`GET /api/v1/search` 和 `GET /api/v1/skills/{slug}` 来获取目录列表。
- 缓存响应，并遵守 `429`、`Retry-After` 和速率限制标头，而不是激进轮询。
- 显示列表时链接回规范的 ClawHub 技能 URL，让用户可以查看来源注册记录。
...
- v1 错误是纯文本（`text/plain; charset=utf-8`），包括 `400`、`401`、`403`、`404`、`429` 和下载被阻止的响应。
- 为兼容性考虑，未知查询参数会被忽略。
- 已知查询参数如果值无效，会返回 `400`。
...
- `GET /api/v1/skills/export?startDate=&endDate=&limit=&cursor=`
  - 托管技能会以已存储文件的形式导出。
  - 当前由 GitHub 支持且扫描结果为 `clean` 或 `suspicious` 的技能会以 `public-github` 交接描述符的形式导出。
