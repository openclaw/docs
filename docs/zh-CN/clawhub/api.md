...
你可以基于 ClawHub 的公开读取 API 构建第三方目录、目录站或搜索界面。公开 Skills 元数据和 Skills 文件会依据 ClawHub 的 Skills 许可规则发布，而 API 本身有速率限制，应负责任地使用。
...
- 读取：每个 IP 600/min，每个密钥 2400/min
- 写入：每个 IP 45/min，每个密钥 180/min
