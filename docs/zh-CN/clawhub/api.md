...
x-i18n:
    generated_at: "2026-07-02T22:21:40Z"
    model: gpt-5.5
...
你可以基于 ClawHub 的公共读取 API 构建第三方目录、目录站点或搜索界面。公共技能元数据和技能文件按照 ClawHub 的技能许可规则发布，而 API 本身受速率限制，应负责任地使用。

指南：

- 使用 `GET /api/v1/skills`、`GET /api/v1/search` 和 `GET /api/v1/skills/{slug}` 等公共读取端点获取目录列表。
- 缓存响应并遵守 `429`、`Retry-After` 和速率限制标头，避免激进轮询。
...
- 下载：每个 IP 1200/min，每个密钥 6000/min

标头：`X-RateLimit-Limit`、`X-RateLimit-Reset`、`RateLimit-Limit`、`RateLimit-Reset`；
`429` 中会包含 `X-RateLimit-Remaining`、`RateLimit-Remaining` 和 `Retry-After`。
...
- `POST /api/v1/users/reserve` 为所有者句柄保留根 slug 和私有无发布包占位符。
