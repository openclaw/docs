...
你可以基于 ClawHub 的公开读取 API 构建第三方目录、索引或搜索界面。公开技能元数据和技能文件根据 ClawHub 的技能许可规则发布，而 API 本身有速率限制，应负责任地使用。

指南：

- 使用公开读取端点，例如 `GET /api/v1/skills`、`GET /api/v1/search` 和 `GET /api/v1/skills/{slug}` 来获取目录列表。
- 缓存响应，并遵守 `429`、`Retry-After` 和速率限制标头，而不是激进轮询。
- 显示列表时链接回权威的 ClawHub 技能 URL，以便用户检查源注册表记录。
- 使用格式为 `https://clawhub.ai/<owner>/<slug>` 的权威页面 URL。
...
- 缺失/无效令牌会回退到按 IP 强制执行。

- 读取：每 IP 600/分钟，每密钥 2400/分钟
- 写入：每 IP 45/分钟，每密钥 180/分钟

标头：`X-RateLimit-Limit`、`X-RateLimit-Remaining`、`X-RateLimit-Reset`、`RateLimit-Limit`、`RateLimit-Remaining`、`RateLimit-Reset`、`Retry-After`（在 429 上）。
