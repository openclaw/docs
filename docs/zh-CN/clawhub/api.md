...
你可以基于 ClawHub 的公开读取 API 构建第三方目录、索引或搜索界面。公开 Skills 元数据和 Skills 文件会按照 ClawHub 的 Skills 许可规则发布，而 API 本身有速率限制，应负责任地使用。

指南：

- 使用公开读取端点（例如 `GET /api/v1/skills`、`GET /api/v1/search` 和 `GET /api/v1/skills/{slug}`）获取目录列表。
...
- 不要通过绕过公开 API 过滤器或认证边界来镜像隐藏、私有或被审核拦截的内容。
...
- `POST /api/v1/users/reserve` 为所有者 handle 保留根 slug 和私有无发布版本的软件包占位符。
