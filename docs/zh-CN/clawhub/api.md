...
x-i18n:
    generated_at: "2026-07-02T17:32:34Z"
    model: gpt-5.5
...
你可以基于 ClawHub 的公共读取 API 构建第三方目录、索引或搜索界面。公共 Skill 元数据和 Skill 文件按 ClawHub 的 Skill 许可规则发布，而 API 本身有速率限制，应负责任地使用。

指南：

- 使用公共读取端点（如 `GET /api/v1/skills`、`GET /api/v1/search` 和 `GET /api/v1/skills/{slug}`）获取目录列表。
- 缓存响应，并遵守 `429`、`Retry-After` 和速率限制标头，避免激进轮询。
- 显示列表时链接回规范的 ClawHub Skill URL，让用户可以检查源注册表记录。
...
- `GET /api/v1/download?slug=&version=&tag=`
  ...
  - 当前由 GitHub 支持且扫描结果为 `clean` 或 `suspicious` 的 Skill 会返回 JSON `public-github` 移交描述符，而不是 ClawHub 字节。
- `GET /api/v1/skills/export?startDate=&endDate=&limit=&cursor=`
  - 托管 Skill 会作为已存储文件导出。
  - 当前由 GitHub 支持且扫描结果为 `clean` 或 `suspicious` 的 Skill 会作为 `public-github` 移交描述符导出。
