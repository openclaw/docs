...
- `X-RateLimit-Remaining` / `RateLimit-Remaining`：存在时表示准确剩余额度；分片成功请求会省略它，而不是返回近似的全局值
- `Retry-After`：遇到 `429` 时应等待的延迟秒数
...
- `GET /api/v1/download?slug=&version=&tag=`
  - 托管技能会返回确定性的 ZIP 字节。
  - 当前由 GitHub 支持、扫描结果为 `clean` 或 `suspicious` 的技能会返回 JSON `public-github` 移交描述符，而不是 ClawHub 字节。
- `GET /api/v1/skills/export?startDate=&endDate=&limit=&cursor=`
  - 托管技能会作为已存储文件导出。
  - 当前由 GitHub 支持、扫描结果为 `clean` 或 `suspicious` 的技能会作为 `public-github` 移交描述符导出。
...
- `POST /api/v1/users/reserve` 会为所有者 handle 预留根 slug 和私有没有发布版本的软件包占位符。
