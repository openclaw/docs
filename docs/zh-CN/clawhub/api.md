...
你可以基于 ClawHub 的公共读取 API 构建第三方目录、索引或搜索界面。公共技能元数据和技能文件会按照 ClawHub 的技能许可证规则发布，而 API 本身有速率限制，应负责任地使用。

指南：

- 使用公共读取端点，例如 `GET /api/v1/skills`、`GET /api/v1/search` 和 `GET /api/v1/skills/{slug}` 来获取目录列表。
- 缓存响应，并尊重 `429`、`Retry-After` 和速率限制标头，而不是激进轮询。
- 显示列表时链接回规范的 ClawHub 技能 URL，以便用户可以检查源注册表记录。
...
- 读取：每个 IP 600/分钟，每个密钥 2400/分钟
- 写入：每个 IP 45/分钟，每个密钥 180/分钟
