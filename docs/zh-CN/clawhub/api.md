...
x-i18n:
    generated_at: "2026-07-03T00:51:21Z"
    model: gpt-5.5
...
你可以基于 ClawHub 的公共读取 API 构建第三方目录、索引或搜索界面。公共 Skills 元数据和 Skills 文件会按照 ClawHub 的 Skills 许可规则发布，而 API 本身受速率限制，应负责任地使用。

指南：

- 使用公共读取端点（如 `GET /api/v1/skills`、`GET /api/v1/search` 和 `GET /api/v1/skills/{slug}`）获取目录列表。
- 缓存响应，并遵守 `429`、`Retry-After` 和速率限制标头，而不是高频轮询。
- 展示列表时链接回规范的 ClawHub Skills URL，让用户可以查看源注册表记录。
- 使用形如 `https://clawhub.ai/<owner>/skills/<slug>` 的规范页面 URL。
- 不要暗示 ClawHub 背书、验证或运营该第三方站点。
- 不要通过绕过公共 API 过滤器或身份验证边界来镜像隐藏、私有或被审核拦截的内容。
...
- `X-RateLimit-Remaining` / `RateLimit-Remaining`：存在时表示精确的剩余额度；分片成功请求会省略它，而不是返回近似的全局值
...
- 存在 `Retry-After` 时优先使用。
- 否则使用 `RateLimit-Reset`，或根据 `X-RateLimit-Reset` 推导延迟。
- 为重试添加抖动。

## 错误

- v1 错误是纯文本（`text/plain; charset=utf-8`），包括 `400`、`401`、`403`、`404`、`429` 和被拦截的下载响应。
- 为了兼容性，未知查询参数会被忽略。
- 已知查询参数带有无效值时返回 `400`。
...
- `GET /api/v1/skills?limit=&cursor=&sort=`
  ...
  - 旧版别名：`nonSuspicious=true`
  - 使用 `nonSuspiciousOnly=true` 时，基于游标的页面包含的项目数可能少于 `limit`；使用 `nextCursor` 继续。
  - `recommended` 使用互动度和近因信号。
...
- `GET /api/v1/download?slug=&version=&tag=`
  ...
  - 当前由 GitHub 支持、且扫描结果为 `clean` 或 `suspicious` 的 Skills 会返回 JSON `public-github` 移交描述符，而不是 ClawHub 字节。
- `GET /api/v1/skills/export?startDate=&endDate=&limit=&cursor=`
  - 托管的 Skills 会导出为已存储文件。
  - 当前由 GitHub 支持、且扫描结果为 `clean` 或 `suspicious` 的 Skills 会导出为 `public-github` 移交描述符。
...
- `POST /api/v1/users/reserve` 会为所有者 handle 保留根 slug 和私有无发布包占位符。

## 旧版

旧版 `/api/*` 和 `/api/cli/*` 仍可用。请参阅 `DEPRECATIONS.md`。
