...
x-i18n:
    generated_at: "2026-07-05T04:42:51Z"
    model: gpt-5.5
...
- 缓存响应，并遵守 `429`、`Retry-After` 和速率限制标头，而不是频繁轮询。
...
- `POST /api/v1/users/reserve` 为所有者 handle 预留根 slug 和私有未发布包占位符。
