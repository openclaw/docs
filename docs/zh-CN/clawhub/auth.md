...
已删除、被封禁或被禁用的账户无法完成正常的 ClawHub 登录。如果登录后返回到未登录状态，你的账户可能状态不正常。
...
如果你的浏览器因为防火墙、VPN 或代理规则无法访问本地回调，请使用无头令牌流程。
...
CLI 会打印一个一次性代码，并在你于 `https://clawhub.ai/cli/device` 授权时等待。
