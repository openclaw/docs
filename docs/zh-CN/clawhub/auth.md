...
已删除、封禁或禁用的账号无法完成正常的 ClawHub 登录。如果登录后又返回到未登录状态，你的账号可能状态异常。
...
如果由于防火墙、VPN 或代理规则导致浏览器无法访问本地回调，请使用无头令牌流程。

## 无头登录

在 ClawHub Web UI 中创建一个令牌，然后将它传递给 CLI：
...
已撤销、无效或缺失的令牌会返回 `401 Unauthorized`。请使用 `clawhub login` 重新登录，或使用 `clawhub login --token` 提供一个新的令牌。

已删除、封禁或禁用的账号无法继续使用现有 API 令牌。
