...
已删除、封禁或停用的账户无法完成正常的 ClawHub 登录。如果登录后又回到未登录状态，你的账户可能状态不正常。如果你的账户被封禁或停用，并且你认为这是误判，请使用 [ClawHub 申诉表单](https://appeals.openclaw.ai/)。
...
在 ClawHub Web UI 中创建一个令牌，然后将其传递给 CLI：
...
已删除、封禁或停用的账户无法继续使用现有 API 令牌。如果你的账户被封禁或停用，并且你认为这是误判，请使用 [ClawHub 申诉表单](https://appeals.openclaw.ai/)。
