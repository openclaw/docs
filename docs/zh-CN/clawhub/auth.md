...
已删除、被封禁或被停用的账户无法完成正常的 ClawHub 登录。如果登录后又回到未登录状态，你的账户可能状态不佳。如果你的账户被封禁或停用，并且你认为这是误判，请使用 [ClawHub 申诉表](https://appeals.openclaw.ai/)。
...
4. 浏览器会重定向回本地回调地址。
5. CLI 会把令牌存储到你的 ClawHub 配置文件中。

如果由于防火墙、VPN 或代理规则导致你的浏览器无法访问本地回调地址，请使用无头令牌流程。
...
已删除、被封禁或被停用的账户无法继续使用现有 API 令牌。如果你的账户被封禁或停用，并且你认为这是误判，请使用 [ClawHub 申诉表](https://appeals.openclaw.ai/)。
