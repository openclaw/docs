...
x-i18n:
    generated_at: "2026-05-10T19:24:50Z"
    model: gpt-5.5
    provider: openai
...
已删除、被封禁或被禁用的账号无法完成正常的 ClawHub 登录。如果登录后又回到未登录状态，你的账号可能状态异常。
...
你可以在 ClawHub Web UI 中吊销 API 令牌。

已吊销、无效或缺失的令牌会返回 `401 Unauthorized`。使用 `clawhub login` 重新登录，或使用 `clawhub login --token` 提供新的令牌。

已删除、被封禁或被禁用的账号无法继续使用现有 API 令牌。
