...
已删除、被封禁或被禁用的账户无法完成正常的 ClawHub 登录。如果登录后又返回到已登出状态，你的账户可能状态异常。如果你的账户被封禁或禁用，而你认为这是误判，请使用 [ClawHub 申诉表单](https://appeals.openclaw.ai/)。
...
在 ClawHub 网页 UI 中创建一个令牌，然后将其传给 CLI：
...
已删除、被封禁或被禁用的账户无法继续使用现有 API 令牌。如果你的账户被封禁或禁用，而你认为这是误判，请使用 [ClawHub 申诉表单](https://appeals.openclaw.ai/)。
