...
x-i18n:
    generated_at: "2026-06-27T10:15:49Z"
    model: gpt-5.5
...
第三方目录可以使用公开读取端点来列出或搜索 ClawHub Skills。请缓存结果，遵守 `429`/`Retry-After`，将用户链接回规范的 ClawHub 列表页（`https://clawhub.ai/<owner>/skills/<slug>`），并避免暗示 ClawHub 为第三方站点背书。不要尝试在公共 API 表面之外镜像隐藏、私有或因审核而屏蔽的内容。
...
- 当服务器知道原因时，已认证写入端点不应返回裸露的 `Unauthorized`。缺失令牌、无效/已撤销令牌，以及已删除/已封禁/已禁用账号都应获得可操作的文本，以便 CLI 客户端告诉用户是什么阻止了他们。
...
这包括验证失败（`400`）、缺失的公共资源（`404`）、认证和权限失败（`401`/`403`）、速率限制（`429`）以及被阻止的下载。客户端应将响应正文读取为人类可读字符串。为兼容性，未知查询参数会被忽略，但已识别查询参数的无效值会返回 `400`。
...
- 结果按相关性顺序返回（嵌入相似度 + 精确 slug/name token 提升 + 小幅热门度先验）。
- 相关性强于热门度。精确的 slug 或显示名称 token 匹配可以排在互动量强得多但匹配更宽松的结果之前。
...
- 热门度采用对数缩放并设置上限。当查询文本匹配较弱时，高互动 Skills 也可能排名更低。
- 根据调用方过滤器和当前审核状态，可疑或隐藏的审核状态可能会将某个 Skill 从公开搜索中移除。

发布者可发现性指南：

- 将用户会实际搜索的术语放在显示名称、摘要和标签中。仅当独立 slug token 也是你想保留的稳定标识时才使用它。
- 不要仅仅为了追逐某个查询而重命名 slug，除非新的 slug 是更好的长期规范名称。旧 slug 会成为重定向别名，但规范 URL、显示的 slug 和未来搜索摘要会使用新 slug。
- 重命名别名会为旧 URL 以及通过注册表解析的安装保留解析能力，但搜索排名基于重命名索引完成后的规范 Skill 元数据。现有统计数据会保留在该 Skill 上。
...
- 当 `nonSuspiciousOnly=true` 时，基于游标的排序在页面检索后会过滤可疑 Skills，因此某一页返回的条目数可能少于 `limit`。
...
- 公开调用方仅对已标记且可见的 Skills 获得 `200`。
- 面向公开调用方时，证据会被修订，并且仅为所有者/审核员包含原始片段。

### `POST /api/v1/skills/{slug}/report`

报告一个 Skill 以供审核员审查。报告是 Skill 级别的，可选链接到某个版本，并进入 Skill 报告队列。
...
`confirmed` 和 `dismissed` 需要 `note`；将 `status` 设置回 `open` 时可以省略它。对已分诊报告传递 `finalAction: "hide"`，可在同一个可审计工作流中隐藏该 Skill。
...
- 返回已排队/运行中/成功/失败状态。
- 排队期间返回 `queue.queuedAhead` 和 `queue.position`，以便客户端显示当前请求前面有多少个已优先处理的手动扫描。非常大的队列会被限制，并以 `queuedAheadIsEstimate: true` 报告。
...
- 只有当所选版本已生成技能卡、未被审核判定为恶意软件阻止，并且 ClawScan 验证为干净时，`ok` 才为 `true`。
...
- `security.signals.dependencyRegistry` 为了 v1 响应兼容性而保留，但依赖注册表存在性扫描器已退役，并且此键始终为 `null`。
...
- `security.signals` 仅包含状态级支持证据；如需完整扫描器详情，请使用 `/scan` 或 ClawHub security-audit 页面。
- `security.signals.dependencyRegistry` 为了 v1 响应兼容性而保留，但依赖注册表存在性扫描器已退役，并且此键始终为 `null`。
...
- 对于旧世界包归档，`version.artifact.kind` 为 `legacy-zip`；对于 ClawPack 支持的发布版，为 `npm-pack`。
- ClawPack 发布版包含与 npm 兼容的 `npmIntegrity`、`npmShasum` 和
  `npmTarballName` 字段。
- `version.sha256hash` 是面向旧客户端的已弃用兼容性元数据。它会对 `/api/v1/packages/{name}/download` 返回的精确 ZIP 字节进行哈希。
...
返回供安装客户端使用的精确包发布版安全和信任摘要。这是 OpenClaw 用于判断已解析发布版是否可安装的公开消费表面。
...
- `trust.scanStatus` 是从扫描器输入和手动发布版审核派生出的有效信任状态。
- `trust.moderationState` 可为空。当不存在手动发布版审核时，它为 `null`。
...
- `trust.stale` 表示信任摘要是从过时输入计算得出的，在做出高置信度允许决策前，应视为需要刷新。

说明：

- 此端点精确到版本。客户端应在解析出计划安装的包版本后调用它，而不是仅在读取最新包元数据后调用。
- 私有包会返回 `404`，除非调用方可以读取所属发布者。
- 此端点有意比所有者/版主审核端点更窄。它公开安装决策和公开解释，而不是举报者身份、举报正文、私有证据或内部审核时间线。
...
- `open`：可疑、恶意、待处理、已隔离、已撤销或已举报的发布版。
- `blocked`：已隔离、已撤销或恶意的发布版。
- `manual`：任何带有手动审核覆盖的发布版。
- `all`：任何带有手动覆盖、非 clean 扫描状态或包举报的发布版。
...
举报一个包以供版主审核。举报是包级别的，可选择链接到某个版本。它们会进入审核队列，但本身不会自动隐藏或阻止下载；版主应使用发布版审核来批准、隔离或撤销构件。
...
所有者/主持人用于查看软件包审核可见性的端点。

认证：

- 需要软件包所有者、发布者成员、主持人或管理员用户的 API 令牌。
...
对于 `confirmed` 和 `dismissed`，`note` 是必需的；将 `status` 重新设为 `open` 时可以省略。对已确认的举报传入 `finalAction: "quarantine"` 或 `finalAction: "revoke"`，可在同一个可审计工作流中应用发布版本审核。
...
- `revoked`：某个发布版本先前受信任，之后被阻止。

被隔离和撤销的发布版本会在制品下载路由返回 `403`。每次更改都会写入一条审计日志记录。
...
- 私有软件包返回 `404`，除非调用者是所有者。

### `GET /api/npm/{package}`

为基于 ClawPack 的软件包版本返回与 npm 兼容的 packument。

说明：

- 只列出已上传 ClawPack npm-pack tarball 的版本。
- 仅旧版 ZIP 的版本会被有意省略。
- `dist.tarball`、`dist.integrity` 和 `dist.shasum` 使用与 npm 兼容的字段，因此用户可以在需要时将 npm 指向该镜像。
...
下载托管的技能版本 ZIP，或者为当前由 GitHub 支持、扫描结果为 `clean` 或 `suspicious` 且没有托管版本的技能返回 GitHub 源交接信息。
...
- 如果既未提供 `version` 也未提供 `tag`，则使用最新版本。
- 软删除的版本返回 `410`。
- 由 GitHub 支持的技能交接不会代理或镜像字节。JSON 响应包含 `sourceRef: "public-github"`、`repo`、`commit`、`path`、`contentHash` 和 `archiveUrl`；扫描/当前状态是一个门控条件，不会作为成功载荷元数据包含在内。
- 下载统计按 UTC 日期统计唯一身份（API 令牌有效时为 `userId`，否则为 IP）。
...
- 可选载荷字段：`migrateOwner`。当它与 `ownerHandle` 一起为 `true` 时，如果操作者同时是当前发布者和目标发布者的管理员/所有者，现有技能可以移动到该所有者。如果没有此显式选择，所有者更改会被拒绝。
...
- 使用 `files` 或 `clawpack` 之一，绝不要在同一请求中同时使用。
- JSON 正文以及调用者提供的 `payload.files` / `payload.artifact` 元数据会被拒绝。
...
- 代发发布仍会针对目标所有者账户验证官方渠道资格。

### `DELETE /api/v1/skills/{slug}` / `POST /api/v1/skills/{slug}/undelete`

软删除/恢复技能（所有者、主持人或管理员）。
...
所有者发起的软删除会保留 slug 30 天，之后该 slug 可以由其他发布者认领。删除响应在适用此到期机制时包含 `slugReservedUntil`。
主持人/管理员隐藏和安全移除不会以这种方式到期。
...
已认证的自助组织发布者创建。创建一个新的组织发布者，并将调用者添加为所有者。此端点不会迁移现有用户/个人 handle，也不会将发布者标记为受信任/官方。
...
仅限管理员。为合法所有者保留根 slug 和软件包名称，而不发布发布版本。软件包名称会变成没有发布行的私有占位软件包，因此同一所有者以后可以将真正的 code-plugin 或 bundle-plugin 发布版本发布到该名称下。
...
仅限管理员。为已验证的替代 GitHub OAuth 主体恢复个人发布者，而不编辑 Convex Auth 账户行。请求必须命名两个不可变的 GitHub provider account id；可变 handle 仅用作面向操作者的防护。

端点默认进行试运行。应用恢复需要在工作人员独立验证两个 GitHub 主体之间的连续性后设置 `dryRun: false` 和
`confirmIdentityVerified: true`。
当目标用户当前的个人发布者拥有技能、包或 GitHub 技能来源时，恢复会以关闭状态失败。
恢复还会迁移已恢复发布者的技能、技能 slug 别名、包、包检查器警告和派生搜索摘要行中的旧版 `ownerUserId` 字段，以便直接所有者路径与新的发布者权限保持一致。已恢复 handle 的有效受保护 handle 预留也会重新分配给替代用户，这样后续个人资料同步就无法恢复原用户的竞争权限。每个主表在每次应用事务中限制为
...
- `POST /api/v1/skills/{slug}/transfer/cancel`
  - Response（接受/拒绝/取消）：`{ "ok": true, "skillSlug": "demo-skill?" }`
