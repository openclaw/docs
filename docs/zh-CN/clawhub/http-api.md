...
read_when:
    - 添加/更改端点
    - 调试 CLI ↔ 注册表请求
summary: HTTP API 参考（公开 + CLI 端点 + 凭证）。
x-i18n:
...
第三方目录可以使用公开读取端点来列出或搜索 ClawHub 技能。请缓存结果，遵守 `429`/`Retry-After`，将用户链接回规范的 ClawHub 列表页（`https://clawhub.ai/<owner>/skills/<slug>`），并避免暗示 ClawHub 认可该第三方站点。不要尝试在公开 API 表面之外镜像隐藏、私有或被审核拦截的内容。
...
- 已认证写入端点在服务器知道原因时，不应返回空泛的 `Unauthorized`。缺失 token、无效/已撤销 token，以及已删除/被封禁/已禁用的账号，都应返回可操作的文本，让 CLI 客户端能够告诉用户阻塞原因。
...
- 如果存在 `Retry-After`，等待对应秒数后再重试。
- 使用带抖动的退避，以避免同步重试。
- 如果缺少 `Retry-After`，回退到 `RateLimit-Reset`（或从 `X-RateLimit-Reset` 计算）。

IP 来源：

- 仅当部署明确启用可信转发 header 时，才会使用可信客户端 IP header，包括 `cf-connecting-ip`。
...
这包括验证失败（`400`）、缺失公开资源（`404`）、认证和权限失败（`401`/`403`）、速率限制（`429`）以及被阻止的下载。客户端应将响应正文作为人类可读字符串读取。未知查询参数会被忽略以保持兼容性，但已识别查询参数若值无效则返回 `400`。
...
- 结果按相关性顺序返回（embedding 相似度 + 精确 slug/name token 加权 + 少量受欢迎度先验）。
- 相关性强于受欢迎度。精确的 slug 或显示名称 token 匹配，可能排在参与度强得多但匹配更宽泛的结果前面。
...
- 受欢迎度会进行对数缩放并设置上限。当查询文本匹配较弱时，高参与度技能的排名可能更低。
- 可疑或隐藏的审核状态可能会根据调用方筛选器和当前审核状态，从公开搜索中移除技能。

发布者可发现性指导：

- 将用户会直接搜索的术语放入显示名称、摘要和标签中。仅当独立 slug token 也是你想保留的稳定身份时，才使用它。
- 不要仅为了追逐某个查询而重命名 slug，除非新 slug 是更好的长期规范名称。旧 slug 会成为重定向别名，但规范 URL、显示的 slug 以及未来搜索摘要会使用新 slug。
- 重命名别名会保留旧 URL 和通过注册表解析的安装的解析能力，但搜索排名会基于重命名完成索引后的规范技能元数据。现有统计数据会保留在该技能上。
- 如果某个技能意外不可见，请先在登录状态下使用 `clawhub inspect @owner/slug` 检查审核状态，再修改与排名相关的元数据。
...
- `recommended` 使用参与度和近期性信号。
- `trending` 按最近 7 天的安装量排名（基于遥测）。
- `createdAt` 对新技能爬取是稳定的；`updated` 会在现有技能重新发布时变化。
- 当 `nonSuspiciousOnly=true` 时，基于游标的排序可能在某一页返回少于 `limit` 的条目，因为可疑技能是在页面取回后过滤的。
...
- 公开调用方仅会对已标记且可见的技能获得 `200`。
- 公开调用方的证据会被脱敏，只有所有者/审核员能看到原始片段。

### `POST /api/v1/skills/{slug}/report`

举报技能以供审核员审查。举报是技能级别的，可选链接到某个版本，并进入技能举报队列。
...
`note` 对 `confirmed` 和 `dismissed` 是必需的；将 `status` 设置回 `open` 时可以省略。对已分流处理的举报传入 `finalAction: "hide"`，可在同一可审计工作流中隐藏该技能。
...
- 扫描请求负载和可下载报告会在保留窗口之后从扫描请求存储中过期。
...
- 扫描作业是异步的。手动扫描请求的优先级高于普通发布/回填工作，但完成仍取决于工作器可用性。
...
- 需要对该技能或插件拥有所有者/发布者管理访问权限，或具备平台审核员/管理员权限。
...
- 只有当选定版本已生成技能卡、未被审核以恶意软件原因阻止，并且 ClawScan 验证为干净时，`ok` 才为 `true`。
- 技能身份、发布者身份和选定版本元数据是顶层信封字段（`slug`、`displayName`、`publisherHandle`、`version`、`resolvedFrom`、`tag`、`createdAt`），因此 shell 自动化无需解包嵌套包装器即可读取它们。
...
- 响应仅包含安全信息。它不包含技能卡数据、生成卡状态、工件文件列表或详细扫描器负载。
- `security.signals` 仅包含状态级支持证据；完整扫描器详细信息请使用 `/scan` 或 ClawHub 安全审计页面。
- `security.signals.dependencyRegistry` 为 v1 响应兼容性而保留，但依赖注册表存在性扫描器已退役，此键始终为 `null`。
- 缺少技能卡不会影响此端点的 `ok`、`decision` 或 `reasons`；当客户端需要卡片内容时，应在本地读取已安装的 `skill-card.md`。
- 当你需要单个技能的技能卡验证信封时使用 `/verify`，需要生成的卡片 Markdown 时使用 `/card`，需要详细扫描器数据时使用 `/scan`。
...
- `startDate`（必需）：技能 `updatedAt` 的 Unix 毫秒下限。
- `endDate`（必需）：技能 `updatedAt` 的 Unix 毫秒上限。
...
仅限插件的搜索，覆盖 code-plugin 和 bundle-plugin 软件包。
...
- 类别过滤是真实的 API 过滤器，由插件类别摘要行提供支持，而不是搜索查询重写。
- 结果按相关性顺序返回，目前不分页。
- 插件搜索的浏览器 UI 排序控件会重排已加载的相关性结果，
  与当前 `/skills` 浏览行为一致。
...
- 需要软件包所有者、组织发布者所有者/管理员、平台审核员或平台管理员的 API token。
...
- 对于旧体系的软件包归档，`version.artifact.kind` 为 `legacy-zip`；对于由 ClawPack 支持的发布版本，则为
  `npm-pack`。
...
返回安装客户端所需的精确软件包发布安全性和信任摘要。这是公共 OpenClaw 消费界面，用于决定已解析的发布版本是否可以安装。
...
- `trust.moderationState` 可为 null。没有手动发布审核时，它为 `null`。
- `trust.blockedFromDownload` 是安装阻止信号。当该值为 `true` 时，OpenClaw 和其他安装客户端应阻止安装，而不是从扫描器或审核字段重新推导阻止规则。
...
- 此端点精确到版本。客户端应在解析出打算安装的软件包版本后调用它，而不只是读取最新软件包元数据后调用。
- 私有软件包返回 `404`，除非调用方可以读取所属发布者。
- 此端点有意比所有者/审核员审核端点更窄。它公开安装决策和公开说明，而不公开报告者身份、报告正文、私有证据或内部审核时间线。
...
报告软件包以供审核员审核。报告是软件包级别的，可选择链接到某个版本。它们会进入审核队列，但本身不会自动隐藏或阻止下载；审核员应使用发布版本审核来批准、隔离或撤销构件。
...
`note` 对 `confirmed` 和 `dismissed` 是必填项；将 `status` 设回 `open` 时可以省略。对于已确认的报告，传入 `finalAction: "quarantine"` 或 `finalAction: "revoke"` 可在同一个可审计工作流中应用发布版本审核。
...
被隔离和撤销的发布版本会从工件下载路由返回 `403`。每次变更都会写入一条审计日志记录。
...
为基于 ClawPack 的包版本返回兼容 npm 的 packument。

说明：

- 仅列出已上传 ClawPack npm-pack tarball 的版本。
- 有意省略旧版仅 ZIP 版本。
- `dist.tarball`、`dist.integrity` 和 `dist.shasum` 使用兼容 npm 的字段，因此用户可以在需要时将 npm 指向该镜像。
...
下载托管的 Skill 版本 ZIP，或者对于当前由 GitHub 支持、扫描结果为 `clean` 或 `suspicious` 且没有托管版本的 Skill，返回 GitHub 源码交接信息。
...
- 可选载荷字段：`ownerHandle`。存在时，API 会在服务端解析该发布者，并要求操作者拥有发布者访问权限。
- 可选载荷字段：`migrateOwner`。当它与 `ownerHandle` 一起为 `true` 时，如果操作者同时是当前和目标发布者的管理员/所有者，现有 Skill 可以迁移到该所有者。没有此选择加入时，所有者变更会被拒绝。
...
- 允许的表单字段是 `payload`、重复的 `files` blob，或一个 `clawpack` tarball 引用。`clawpack` 可以是 `.tgz` blob，也可以是上传 URL 流程返回的存储 ID。基于暂存 storage-id 的发布还必须包含随该上传 URL 返回的 `clawpackUploadTicket`。
- 同一请求中只能使用 `files` 或 `clawpack`，不能同时使用。
- JSON 主体以及调用方提供的 `payload.files` / `payload.artifact` 元数据会被拒绝。
- 直接 multipart 发布请求上限为 18MB。ClawPack tarball 可以通过上传 URL 流程使用，最高为 120MB tarball 上限。
...
- Code plugins 需要 `package.json`、源码仓库元数据、源码提交元数据、配置 schema 元数据、`openclaw.compat.pluginApi` 和 `openclaw.build.openclawVersion`。
...
所有者发起的软删除会保留 slug 30 天，之后该 slug 可由另一个发布者声明。删除响应会在适用此过期时间时包含 `slugReservedUntil`。
...
已认证的自助式组织发布者创建。创建新的组织发布者并将调用方添加为所有者。此端点不会迁移现有用户/个人 handle，也不会将发布者标记为 trusted/official。
...
仅管理员。为合法所有者保留根 slug 和包名，而不发布发布版本。包名会成为没有发布版本行的私有占位包，因此同一所有者之后可以将真实的 code-plugin 或 bundle-plugin 发布版本发布到该名称。
...
仅管理员。为经过验证的替代 GitHub OAuth 主体恢复个人发布者，而不编辑 Convex Auth 账号行。请求必须同时指定两个不可变的 GitHub 提供商账号 ID；可变 handle 仅用作面向操作员的保护。

该端点默认使用演练模式。应用恢复需要在工作人员独立验证两个
GitHub 身份主体之间的连续性后设置 `dryRun: false` 和
`confirmIdentityVerified: true`。当目标用户当前的个人发布者拥有技能、包或
GitHub 技能来源时，恢复会以拒绝方式失败。
恢复还会迁移已恢复发布者的技能、技能 slug 别名、包、包检查器警告和派生搜索摘要行中的旧版 `ownerUserId` 字段，使直接所有者路径与新的发布者权威保持一致。已恢复 handle 的活动受保护 handle 预留也会重新分配给替代用户，这样后续个人资料同步就无法恢复原用户的竞争性权威。每个主表在每次应用事务中限制为
100 行；更大的恢复必须先使用可恢复的所有者迁移。
GitHub 技能来源按发布者限定作用域，并报告为已检查，而不是重写。
...
更改现有封禁的已存储原因，但不解除封禁或恢复内容（仅限管理员）。除非 `dryRun` 为 `false`，否则默认使用演练模式。
