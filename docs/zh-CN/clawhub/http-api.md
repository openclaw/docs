...
x-i18n:
    generated_at: "2026-07-01T20:10:57Z"
    model: gpt-5.5
...
- 当服务器知道原因时，已认证写入端点不应返回裸 `Unauthorized`。缺失令牌、无效/已吊销令牌，以及已删除/封禁/禁用账号都应获得可操作文本，以便 CLI 客户端可以告知用户是什么阻止了他们。
...
这包括验证失败（`400`）、缺失公共资源（`404`）、认证和权限失败（`401`/`403`）、速率限制（`429`）以及被拦截的下载。客户端应将响应正文作为人类可读字符串读取。未知查询参数会被忽略以保持兼容性，但已识别查询参数带有无效值时会返回 `400`。
...
- 结果按相关性顺序返回（嵌入相似度 + 精确 slug/name 词元提升 + 小幅受欢迎度先验）。
- 相关性强于受欢迎度。精确 slug 或显示名称词元匹配可以排在互动度高得多但匹配更宽泛的结果之前。
- ASCII 文本会按单词和标点边界分词。例如，`personal-map` 包含独立的 `map` 词元，而 `amap-jsapi-skill` 包含 `amap`、`jsapi` 和 `skill`；因此搜索 `map` 时，`personal-map` 的词汇匹配强于 `amap-jsapi-skill`。
- 受欢迎度会进行对数缩放并设上限。当查询文本匹配较弱时，高互动技能可能排名更低。
- 可疑或隐藏的审核状态可能会根据调用方筛选器和当前审核状态，将某个技能从公共搜索中移除。

发布者可发现性指南：

- 将用户会逐字搜索的词放入显示名称、摘要和标签中。仅当独立 slug 词元也是你想保留的稳定身份时才使用它。
- 不要为了追逐某个查询而重命名 slug，除非新 slug 是更好的长期规范名称。旧 slug 会成为重定向别名，但规范 URL、显示的 slug 和未来搜索摘要会使用新 slug。
- 重命名别名会保留旧 URL 和通过注册表解析的安装的解析能力，但搜索排名基于重命名完成索引后的规范技能元数据。现有统计会留在该技能上。
- 如果某个技能意外不可见，请在更改排名相关元数据之前，先在登录状态下使用 `clawhub inspect @owner/slug` 检查审核状态。
...
- 当 `nonSuspiciousOnly=true` 时，基于游标的排序可能会在某页返回少于 `limit` 个条目，因为可疑技能是在页面取回后过滤的。
...
- 公共调用方仅会对已经被标记且可见的技能获得 `200`。
- 公共调用方看到的证据会被遮盖，并且只有所有者/审核员会看到原始片段。

### `POST /api/v1/skills/{slug}/report`

举报技能以供审核员审查。举报是技能级别的，可选链接到某个版本，并会进入技能举报队列。
...
`confirmed` 和 `dismissed` 需要 `note`；将 `status` 设回 `open` 时可以省略它。将 `finalAction: "hide"` 与已分诊举报一起传入，可在同一个可审计工作流中隐藏该技能。
...
- 只有当扫描器产生了明确判定（`clean`、`suspicious` 或 `malicious`）时，`security.hasScanResult` 才为 `true`。
...
- 扫描任务是异步的。手动扫描请求会优先于普通发布/回填工作，但完成时间仍取决于 worker 可用性。
...
- 排队期间返回 `queue.queuedAhead` 和 `queue.position`，以便客户端显示有多少个优先级手动扫描排在该请求之前。非常大的队列会被限制，并以 `queuedAheadIsEstimate: true` 报告。
...
- 只有当所选版本有已生成的技能卡、未被审核阻止为恶意软件，并且 ClawScan 验证为干净时，`ok` 才为 `true`。
- 技能身份、发布者身份和所选版本元数据是顶层信封字段（`slug`、`displayName`、`publisherHandle`、`version`、`resolvedFrom`、`tag`、`createdAt`），因此 shell 自动化可以读取它们，而无需解包嵌套包装器。
...
- 只有当 ClawHub 在发布或导入期间解析并存储了 GitHub repo/ref/commit/path 时，`provenance` 才为 `server-resolved-github-import`；否则为 `unavailable`。
...
- 响应仅包含安全信息。它不包含技能卡数据、生成卡状态、工件文件列表或详细扫描器载荷。
...
- `category`（可选）：插件分类过滤器。仅当请求限定为插件包（`/api/v1/plugins`、`/api/v1/code-plugins`、`/api/v1/bundle-plugins`，或带有 `family=code-plugin`/`family=bundle-plugin` 的包端点）时受支持。受控分类和旧版 v1 过滤器别名记录在 `GET /api/v1/plugins` 下。

说明：

- `family`、`channel`、`isOfficial`、`featured`、`highlightedOnly` 或 `sort` 的无效值会返回 `400`。未知查询参数会被忽略。
- `GET /api/v1/code-plugins` 和 `GET /api/v1/bundle-plugins` 仍保留为固定 family 的别名。
...
- `channel=private` 仅返回经过身份验证的调用者可读取的包。
...
- `channel`（可选）：`official`、`community` 或 `private`
- `isOfficial`（可选）：`true` 或 `false`
- `category`（可选）：插件分类过滤器。仅当请求限定为插件包时受支持。受控分类和旧版 v1 过滤器别名记录在 `GET /api/v1/plugins` 下。
...
返回面向安装客户端的精确包发布版本安全和信任摘要。这是公开的 OpenClaw 使用表面，用于决定
已解析的发布版本是否可以安装。
...
- `release.releaseId`、`release.version` 和 `release.createdAt` 标识
  已评估的精确发布版本。
- 当发布工件已知时，会存在 `release.artifactKind`、`release.artifactSha256`、`release.npmIntegrity`、
...
- `packageName` 会按 npm 名称规范化；对于计划中的
  迁移，包可以不存在。
- 这只跟踪迁移就绪状态。它不会改变 OpenClaw 或生成
  ClawPack。

### `GET /api/v1/packages/moderation/queue`

版主/管理员端点，用于包发布版本审核队列。
...
`note` 对 `confirmed` 和 `dismissed` 是必需的；将 `status` 重新设置为 `open` 时可以省略。传入 `finalAction: "quarantine"` 或 `finalAction: "revoke"` 并配合已确认的报告，可在同一个可审计工作流中应用发布版本审核。
...
隔离和撤销的发布版本会从构件下载路由返回 `403`。每次更改都会写入一条审计日志条目。
...
为 npm 镜像客户端流式传输上传的 ClawPack tarball 的精确字节。
...
下载托管的 skill 版本 ZIP，或者针对当前由 GitHub 支持、扫描结果为 `clean` 或 `suspicious` 且没有托管版本的 skill，返回 GitHub 源码交接信息。
...
- 软删除版本返回 `410`。
- 由 GitHub 支持的 skill 交接不会代理或镜像字节。JSON 响应包含 `sourceRef: "public-github"`、`repo`、`commit`、`path`、`contentHash` 和 `archiveUrl`；扫描/当前状态是准入门槛，不会作为成功载荷元数据包含。
...
- 可选载荷字段：`migrateOwner`。当它与 `ownerHandle` 一起为 `true` 时，如果操作者同时是当前发布者和目标发布者的管理员/所有者，现有 skill 可以移动到该所有者。没有此选择加入时，所有者更改会被拒绝。
...
- 允许的表单字段为 `payload`、重复的 `files` blob，或一个 `clawpack` tarball 引用。`clawpack` 可以是 `.tgz` blob，也可以是 upload-url 流程返回的存储 id。暂存的存储 id 发布还必须包含随该上传 URL 返回的 `clawpackUploadTicket`。
- 同一请求中只能使用 `files` 或 `clawpack`，不能同时使用二者。
...
所有者发起的软删除会将 slug 保留 30 天，之后该 slug 可以被另一个发布者认领。删除响应会在此到期适用时包含 `slugReservedUntil`。
...
仅限管理员。确保某个 handle 存在组织发布者。如果该 handle 仍指向旧版共享用户/个人发布者，此端点会先将其迁移到组织发布者。对于新创建的组织，提供 `memberHandle`；执行操作的管理员不会被添加为成员。`memberRole` 默认为 `owner`。
...
经过认证的自助组织发布者创建。创建新的组织发布者，并将调用方添加为所有者。此端点不会迁移现有用户/个人 handle，也不会将发布者标记为受信任/官方。
...
仅限管理员。为合法所有者保留根 slug 和包名称，而不发布发布版本。包名称会变成没有发布版本行的私有占位包，因此同一所有者之后可以将真正的 code-plugin 或 bundle-plugin 发布版本发布到该名称。
...
仅限管理员。为已验证的替换 GitHub OAuth 主体恢复个人发布者，而不编辑 Convex Auth 账户行。请求必须命名两个不可变 GitHub provider account id；可变 handle 仅用作面向操作者的防护。

该端点默认使用试运行。应用恢复需要在工作人员独立验证两个
GitHub 主体之间的连续性后设置 `dryRun: false` 和
`confirmIdentityVerified: true`。当目标用户当前的个人发布者拥有技能、包或 GitHub 技能来源时，恢复会失败并保持关闭状态。
恢复还会迁移已恢复发布者的技能、技能 slug 别名、包、包检查器警告和派生搜索摘要行中的旧版 `ownerUserId` 字段，以便直接所有者路径与新的发布者权限一致。已恢复 handle 的有效受保护 handle 预留也会重新分配给替换用户，这样后续个人资料同步就无法恢复原用户的竞争权限。每个主表在每次应用事务中限制为
100 行；更大的恢复必须先使用可恢复的所有者迁移。
GitHub 技能来源按发布者限定，并报告为已检查而不是被重写。
...
更改现有封禁存储的原因，不解除封禁或恢复内容（仅限管理员）。除非 `dryRun` 为 `false`，否则默认使用试运行。
