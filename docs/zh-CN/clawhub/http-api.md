...
第三方目录可以使用公开读取端点来列出或搜索 ClawHub Skills。请缓存结果，遵守 `429`/`Retry-After`，将用户链接回规范的 ClawHub 列表页（`https://clawhub.ai/<owner>/skills/<slug>`），并避免暗示 ClawHub 为该第三方站点背书。不要尝试在公开 API 表面之外镜像隐藏、私有或被审核屏蔽的内容。
...
- 匿名请求：按 IP 强制执行。
- 已认证请求（有效 Bearer token）：按用户 bucket 强制执行。
- 如果 token 缺失/无效，行为会回退到 IP 强制执行。
- 当服务器知道原因时，已认证写入端点不应返回裸的 `Unauthorized`。缺失 token、无效/已撤销 token，以及已删除/被封禁/已禁用账户，都应得到可操作文本，以便 CLI 客户端可以告诉用户是什么阻止了他们。
...
- `Retry-After`：在 `429` 时，重试前需要等待的秒数（延迟）
...
- 如果没有可用的受信任客户端 IP，匿名请求会使用仅按速率限制类型划分作用域的回退 bucket。这些回退 bucket 不包含调用方提供的路径、slug、package 名称、版本、查询字符串或其他 artifact 参数。

## 错误响应

公开 v1 错误响应是纯文本，带有 `content-type: text/plain; charset=utf-8`。
这包括验证失败（`400`）、缺失的公开资源（`404`）、认证和权限失败（`401`/`403`）、速率限制（`429`）以及被屏蔽的下载。客户端应将响应正文读取为人类可读的字符串。未知查询参数会为了兼容性被忽略，但已识别查询参数的无效值会返回 `400`。
...
- 相关性强于流行度。精确的 slug 或显示名称 token 匹配可以排在互动度高得多但匹配更宽松的结果之前。
...
- 流行度会按对数缩放并封顶。当查询文本匹配较弱时，高互动 Skills 可能排名更低。
- 根据调用方筛选条件和当前审核状态，可疑或隐藏的审核状态可能会将某个 Skill 从公开搜索中移除。
...
- 重命名别名会保留旧 URL 和通过注册表解析的安装的解析能力，但搜索排名基于重命名已索引后的规范 Skill 元数据。现有统计会保留在该 Skill 上。
- 如果某个 Skill 意外不可见，请在登录状态下先用 `clawhub inspect @owner/slug` 检查审核状态，再更改与排名相关的元数据。
...
- 当 `nonSuspiciousOnly=true` 时，基于游标的排序可能会在某一页返回少于 `limit` 的条目，因为可疑 Skills 是在页面检索后过滤的。
...
- 只有当 Skill 被标记或所有者正在查看时，才包含 `moderation`。
...
- 公开调用方只能对已经标记且可见的 Skills 获得 `200`。
- 面向公开调用方时，证据会被遮盖，并且只有所有者/审核员能看到原始片段。

### `POST /api/v1/skills/{slug}/report`

举报某个 Skill 以供审核员审查。举报是 Skill 级别，可选链接到某个版本，并会进入 Skill 举报队列。
...
`confirmed` 和 `dismissed` 需要 `note`；将 `status` 设回 `open` 时可以省略。对已分诊举报传入 `finalAction: "hide"`，即可在同一可审计工作流中隐藏该 Skill。
...
- 扫描作业是异步的。手动扫描请求优先于普通发布/回填工作，但完成时间仍取决于工作进程可用性。
...
- 排队期间返回 `queue.queuedAhead` 和 `queue.position`，以便客户端显示该请求前方还有多少个优先级更高的手动扫描。非常大的队列会受到限制，并以 `queuedAheadIsEstimate: true` 报告。
- 可用时，`report` 包含 `clawscan`、`skillspector`、`staticAnalysis` 和 `virustotal` 部分。
- 失败的扫描作业会返回 `status: "failed"` 以及 `lastError`。

### `GET /api/v1/skills/-/scan/{scanId}/download`

已认证的报告归档端点。

- 需要已成功完成的扫描；非终止状态扫描返回 `409`。
...
- 需要对该 Skills 或插件拥有所有者/发布者管理访问权限，或拥有平台审核员/管理员权限。
...
- 只有当所选版本具有已生成的 Skill Card、未被审核判定为恶意软件阻止，并且 ClawScan 验证结果为 clean 时，`ok` 才为 `true`。
- Skills 标识、发布者标识和所选版本元数据是顶层信封字段（`slug`、`displayName`、`publisherHandle`、`version`、`resolvedFrom`、`tag`、`createdAt`），因此 shell 自动化无需解包嵌套包装即可读取它们。
- `security` 是顶层 ClawScan/安全结论。自动化应依据 `ok`、`decision`、`reasons` 和 `security.status` 做判断。
...
- 只有当 ClawHub 在发布或导入期间解析并存储了 GitHub 仓库/ref/提交/路径时，`provenance` 才为 `server-resolved-github-import`；否则为 `unavailable`。

### `POST /api/v1/skills/-/security-verdicts`

返回精确 Skills 版本的当前紧凑安全结论。此
集合端点面向已经知道需要显示哪些已安装
...
- 结果按条目返回；某个 Skills 或版本缺失不会导致整个响应失败。
- 响应仅包含安全信息。它不包括 Skill Card 数据、已生成卡片状态、工件文件列表或详细扫描器载荷。
- `security.signals` 仅包含状态级支持证据；如需完整扫描器详细信息，请使用 `/scan` 或 ClawHub security-audit 页面。
- `security.signals.dependencyRegistry` 为了 v1 响应兼容性而保留，但依赖注册表存在性扫描器已退役，此键始终为 `null`。
- Skill Card 缺失不会影响此端点的 `ok`、`decision` 或 `reasons`；客户端在需要卡片内容时，应在本地读取已安装的 `skill-card.md`。
- 当你需要单个 Skills 的 Skill Card 验证信封时使用 `/verify`，需要已生成的卡片 Markdown 时使用 `/card`，需要详细扫描器数据时使用 `/scan`。
...
- `category`（可选）：插件类别筛选器。仅当请求限定为插件包时支持（`/api/v1/plugins`、`/api/v1/code-plugins`、`/api/v1/bundle-plugins`，或带有 `family=code-plugin`/`family=bundle-plugin` 的包端点）。受控类别和旧版 v1 筛选器别名记录在 `GET /api/v1/plugins` 下。
...
- `startDate`（必需）：Skills `updatedAt` 的 Unix 毫秒下限。
- `endDate`（必需）：Skills `updatedAt` 的 Unix 毫秒上限。
...
- 正文：ZIP 归档。
- 每个导出的 Skills 根目录为 `{publisher}/{slug}/`。
- 托管 Skills 包含最新存储版本文件，并在 `_manifest.json` 中以 `sourceRef: "public-clawhub"` 列出。
- 当前由 GitHub 支持且扫描结果为 `clean` 或 `suspicious` 的 Skills 包含 `_source_handoff.json`，其中包含 `sourceRef: "public-github"`、仓库、提交、路径、内容哈希和归档 URL。它们不包含 ClawHub 托管的源文件。
...
- 需要包所有者、组织发布者所有者/管理员、平台仲裁员或平台管理员的 API 令牌。
...
- 对旧世界包归档，`version.artifact.kind` 为 `legacy-zip`；对 ClawPack 支持的发布版，为
  `npm-pack`。
- ClawPack 发布版包含 npm 兼容的 `npmIntegrity`、`npmShasum` 和
  `npmTarballName` 字段。
- `version.sha256hash` 是面向旧客户端的已弃用兼容性元数据。它对 `/api/v1/packages/{name}/download` 返回的精确 ZIP 字节进行哈希。
  现代客户端应使用 `version.artifact.sha256`，它标识规范发布工件。
- 当扫描数据存在时，会包含 `version.vtAnalysis`、`version.llmAnalysis` 和 `version.staticScan`。
- 私有包会返回 `404`，除非调用方可以读取所属发布者。

### `GET /api/v1/packages/{name}/versions/{version}/security`

返回供安装客户端使用的精确包发布版安全与信任摘要。这是公开的 OpenClaw 消费表面，用于决定已解析的发布版是否可以安装。

凭证：

- 公开读取端点。不需要所有者、发布者、仲裁员或管理员令牌。
...
- `release.releaseId`、`release.version` 和 `release.createdAt` 标识已评估的精确发布版。
- 当发布工件已知时，会提供 `release.artifactKind`、`release.artifactSha256`、`release.npmIntegrity`、
  `release.npmShasum` 和 `release.npmTarballName`。
- `trust.scanStatus` 是从扫描器输入和手动发布仲裁派生出的有效信任状态。
- `trust.moderationState` 可为空。当不存在手动发布仲裁时，它为 `null`。
- `trust.blockedFromDownload` 是安装阻止信号。当此值为 `true` 时，OpenClaw 和其他安装客户端应阻止安装，而不是从扫描器或仲裁字段重新推导阻止规则。
- `trust.reasons` 是面向用户和审计的解释列表。原因代码是稳定、紧凑的字符串，例如 `manual:quarantined`、`scan:malicious`
  和 `package:malicious`。
- `trust.pending` 表示一个或多个信任输入仍在等待完成。
- `trust.stale` 表示信任摘要是根据过时输入计算的，在作出高置信度允许决策前应视为需要刷新。

说明：

- 此端点精确到版本。客户端应在解析出它们打算安装的包版本后调用它，而不是只在读取最新包元数据后调用。
- 私有包会返回 `404`，除非调用方可以读取所属发布者。
- 此端点有意比所有者/仲裁员仲裁端点更窄。它公开安装决策和公开解释，而不公开报告者身份、报告正文、私有证据或内部审核时间线。
...
- 这仅跟踪迁移就绪状态。它不会变更 OpenClaw，也不会生成 ClawPack。

### `GET /api/v1/packages/moderation/queue`

用于包发布版审核队列的仲裁员/管理员端点。
...
- `open`：可疑、恶意、待处理、已隔离、已撤销或被举报的发布版。
- `blocked`：已隔离、已撤销或恶意的发布版。
- `manual`：任何带有手动仲裁覆盖的发布版。
- `all`：任何带有手动覆盖、非干净扫描状态或包举报的发布版。
...
举报一个包以供仲裁员审核。举报是包级别的，可选地关联到一个版本。它们会进入仲裁队列，但本身不会自动隐藏或阻止下载；仲裁员应使用发布版仲裁来批准、隔离或撤销工件。
...
`note` 对 `confirmed` 和 `dismissed` 是必填项；当将 `status` 设置回 `open` 时可以省略。对已确认的报告传入 `finalAction: "quarantine"` 或 `finalAction: "revoke"`，即可在同一个可审计工作流中应用发布版本审核。
...
- `revoked`：在发布版本先前受信任后阻止。

隔离和撤销的发布版本会从构件下载路由返回 `403`。
每次变更都会写入一条审计日志。
...
- 插件/包归档是带有 `package/` 根目录的 zip 文件，因此旧版 OpenClaw 客户端可继续工作。
...
为 npm 镜像客户端流式传输完全一致的已上传 ClawPack tarball 字节。
...
下载托管的技能版本 ZIP，或为当前由 GitHub 支持、扫描结果为 `clean` 或 `suspicious` 且没有托管版本的技能返回 GitHub 源交接。
...
- 软删除的版本返回 `410`。
- 由 GitHub 支持的技能交接不会代理或镜像字节。JSON 响应包含 `sourceRef: "public-github"`、`repo`、`commit`、`path`、`contentHash` 和 `archiveUrl`；扫描/当前状态是门控条件，不会作为成功载荷元数据包含在内。
...
- 可选载荷字段：`migrateOwner`。当它与 `ownerHandle` 一起为 `true` 时，如果执行者同时是当前发布者和目标发布者的管理员/所有者，则现有技能可以移动到该所有者。没有此显式选择时，所有者变更会被拒绝。
...
- 允许的表单字段为 `payload`、重复的 `files` blob，或一个 `clawpack` tarball 引用。`clawpack` 可以是 `.tgz` blob，也可以是 upload-url 流程返回的存储 ID。使用暂存存储 ID 发布时，还必须包含该上传 URL 返回的 `clawpackUploadTicket`。
- 使用 `files` 或 `clawpack` 之一，绝不要在同一请求中同时使用两者。
- JSON 正文以及调用方提供的 `payload.files` / `payload.artifact` 元数据会被拒绝。
- 直接 multipart 发布请求上限为 18MB。ClawPack tarball 可使用 upload-url 流程，最高达到 120MB tarball 上限。
- 可选载荷字段：`ownerHandle`。存在时，仅管理员可以代表该所有者发布。
...
由所有者发起的软删除会保留 slug 30 天，之后该 slug 可被另一位发布者认领。
当此到期机制适用时，删除响应会包含 `slugReservedUntil`。
...
仅管理员。确保某个句柄存在组织发布者。如果该句柄仍指向旧版共享用户/个人发布者，该端点会先将其迁移为组织发布者。
对于新创建的组织，请提供 `memberHandle`；执行操作的管理员不会被添加为成员。
`memberRole` 默认值为 `owner`。
...
仅管理员。为合法所有者保留根 slug 和包名称，而不发布版本。包名称会成为没有发布版本行的私有占位包，因此同一所有者之后可以将真正的 code-plugin 或 bundle-plugin 发布版本发布到该名称。
...
仅管理员。为经过验证的替代 GitHub OAuth 主体恢复个人发布者，而不编辑 Convex Auth 账户行。请求必须指定两个不可变 GitHub provider account id；可变句柄仅用作面向操作员的保护。

端点默认使用试运行。应用恢复需要在工作人员独立验证两个 GitHub 主体之间的连续性后设置 `dryRun: false` 和
`confirmIdentityVerified: true`。当目标用户当前的个人发布者拥有技能、包或 GitHub 技能源时，恢复会失败即关闭。
恢复还会迁移已恢复发布者的技能、技能 slug 别名、包、包检查器警告和派生搜索摘要行中的旧版 `ownerUserId` 字段，以便直接所有者路径与新的发布者权限一致。已恢复 handle 的活动受保护 handle 预留也会重新分配给替换用户，这样后续的个人资料同步无法恢复前用户的竞争权限。每个主表在每个应用事务中限制为
100 行；更大的恢复必须先使用可恢复的所有者迁移。
GitHub 技能源按发布者限定，并报告为已检查，而不是被重写。
...
- `merge` 会隐藏源列表页，并将源 slug 重定向到目标列表页。
...
更改现有封禁的已存储原因，不解封用户，也不恢复内容（仅限管理员）。除非 `dryRun` 为 `false`，否则默认使用试运行。
...
`POST /api/cli/upload-url` 会返回 `uploadUrl` 和 `uploadTicket`。暂存 ClawPack tarball 的包发布必须将生成的存储 ID 作为
