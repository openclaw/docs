...
x-i18n:
    generated_at: "2026-07-02T00:42:29Z"
    model: gpt-5.5
...
第三方目录可以使用公开读取端点列出或搜索 ClawHub 技能。请缓存结果，遵守 `429`/`Retry-After`，将用户链接回规范的 ClawHub 列表页（`https://clawhub.ai/<owner>/skills/<slug>`），并避免暗示 ClawHub 为该第三方站点背书。不要尝试在公共 API 表面之外镜像隐藏、私有或因审核被屏蔽的内容。
...
- 当服务器知道原因时，已认证写入端点不应返回裸的 `Unauthorized`。缺失令牌、无效/已撤销令牌，以及已删除/被封禁/已禁用账户都应获得可操作文本，以便 CLI 客户端能告诉用户是什么阻止了他们。
...
- 仅当部署显式启用受信任转发标头时，才使用受信任的客户端 IP 标头，包括 `cf-connecting-ip`。
- ClawHub 使用受信任的转发标头在边缘识别客户端 IP。
- 如果没有可用的受信任客户端 IP，匿名请求会使用仅按速率限制类型限定作用域的回退桶。这些回退桶不包含调用方提供的路径、slug、包名、版本、查询字符串或其他制品参数。

## 错误响应

公共 v1 错误响应为纯文本，`content-type: text/plain; charset=utf-8`。
这包括验证失败（`400`）、缺失的公共资源（`404`）、认证和权限失败（`401`/`403`）、速率限制（`429`）以及被屏蔽的下载。客户端应将响应正文读取为人类可读字符串。未知查询参数会因兼容性被忽略，但已识别查询参数的无效值会返回 `400`。
...
- `q`（必需）：查询字符串
- `limit`（可选）：整数
- `highlightedOnly`（可选）：`true` 表示过滤为高亮技能
...
- 结果按相关性顺序返回（嵌入相似度 + 精确 slug/名称 token 加权 + 少量受欢迎程度先验）。
- 相关性强于受欢迎程度。精确的 slug 或显示名称 token 匹配可以排在参与度强得多但匹配更宽松的结果前面。
...
- 受欢迎程度按对数缩放并设有上限。当查询文本匹配较弱时，高参与度技能的排名可能更低。
- 可疑或隐藏的审核状态可能会根据调用方过滤器和当前审核状态，从公共搜索中移除技能。

发布者可发现性指引：

- 将用户会实际搜索的术语放在显示名称、摘要和标签中。仅当独立 slug token 也是你想保留的稳定身份时才使用它。
- 不要仅为追逐某个查询而重命名 slug，除非新 slug 是更好的长期规范名称。旧 slug 会成为重定向别名，但规范 URL、显示的 slug 和未来搜索摘要会使用新 slug。
- 重命名别名会为旧 URL 和通过注册表解析的安装保留解析能力，但搜索排名基于重命名索引完成后的规范技能元数据。现有统计信息会保留在该技能上。
...
- 证据会对公共调用方做脱敏处理，并且只为所有者/审核员包含原始片段。

### `POST /api/v1/skills/{slug}/report`

举报技能以供审核员审查。举报是技能级别的，可选择链接到某个版本，并进入技能举报队列。
...
`confirmed` 和 `dismissed` 需要 `note`；将 `status` 设置回 `open` 时可以省略它。对已分流的举报传入 `finalAction: "hide"`，可在同一个可审计工作流中隐藏该技能。
...
- 只有当 `update: true` 且扫描成功完成时，已发布扫描才会写回结果。
...
- 扫描作业是异步的。手动扫描请求会优先于普通发布/回填工作处理，但完成时间仍取决于工作进程可用性。
...
- 排队时返回 `queue.queuedAhead` 和 `queue.position`，以便客户端显示该请求前面还有多少个优先级更高的手动扫描。非常大的队列会被限制，并以 `queuedAheadIsEstimate: true` 报告。
- 可用时，`report` 包含 `clawscan`、`skillspector`、`staticAnalysis` 和 `virustotal` 部分。
- 失败的扫描作业返回 `status: "failed"`，并带有 `lastError`。
...
- 需要该技能或插件的所有者/发布者管理访问权限，或平台审核员/管理员权限。
- 返回精确提交版本的已存储扫描结果，包括被拦截或隐藏的版本。
- `kind` 默认为 `skill`；对于插件/包扫描，请使用 `kind=plugin`。
- 返回与扫描请求下载相同形状的 ZIP。
...
- 只有当所选版本已生成技能卡、未被审核判定为恶意软件而拦截，并且 ClawScan 验证为 clean 时，`ok` 才为 `true`。
- 技能身份、发布者身份和所选版本元数据是顶层封套字段（`slug`、`displayName`、`publisherHandle`、`version`、`resolvedFrom`、`tag`、`createdAt`），因此 shell 自动化无需解包嵌套包装即可读取它们。
...
- 响应仅包含安全信息。它不包含技能卡数据、已生成卡片状态、制品文件列表或详细扫描器载荷。
- `security.signals` 仅包含状态级别的支持性证据；如需完整扫描器详情，请使用 `/scan` 或 ClawHub 安全审计页面。
- `security.signals.dependencyRegistry` 保留用于 v1 响应兼容性，但依赖注册表存在性扫描器已停用，此键始终为 `null`。
- 技能卡缺失不会影响此端点的 `ok`、`decision` 或 `reasons`；客户端在需要卡片内容时，应在本地读取已安装的 `skill-card.md`。
- 当你需要单技能的技能卡验证封套时使用 `/verify`，需要生成的卡片 Markdown 时使用 `/card`，需要详细扫描器数据时使用 `/scan`。
...
- `category`（可选）：插件类别过滤器。仅当请求限定到插件包时支持
  （`/api/v1/plugins`、`/api/v1/code-plugins`、`/api/v1/bundle-plugins`，或带有
...
- `GET /api/v1/code-plugins` 和 `GET /api/v1/bundle-plugins` 保持为固定 family 的别名。
- 技能条目仍由技能注册表提供支持，并且仍然只能通过 `POST /api/v1/skills` 发布。
- `POST /api/v1/packages` 仍然仅用于代码插件和捆绑插件发布。
- 匿名调用方只能看到公开包频道。
...
- `channel`（可选）：`official`、`community` 或 `private`
- `isOfficial`（可选）：`true` 或 `false`
- `category`（可选）：插件类别过滤器。仅当请求限定到插件包时支持。受控类别和旧版 v1
...
- 当前由 GitHub 支持、且扫描结果为 `clean` 或 `suspicious` 的技能包含
  `_source_handoff.json`，其中有 `sourceRef: "public-github"`、仓库、提交、路径、
  内容哈希和归档 URL。它们不包含 ClawHub 托管的源文件。
- 每个技能都包含 `_export_skill_meta.json`。
- `_manifest.json` 始终包含在 ZIP 根目录下。
...
- 类别筛选是真实的 API 筛选器，由插件类别摘要行支持，而不是搜索查询重写。
- 结果按相关性顺序返回，目前不分页。
...
- 对于旧式包归档，`version.artifact.kind` 为 `legacy-zip`；对于由 ClawPack 支持的版本，值为 `npm-pack`。
...
返回安装客户端使用的精确包发布安全和信任摘要。这是公开的 OpenClaw 消费接口，用于决定已解析版本是否可以安装。
...
- 当发布产物已知时，会出现 `release.artifactKind`、`release.artifactSha256`、`release.npmIntegrity`、
  `release.npmShasum` 和 `release.npmTarballName`。
- `trust.scanStatus` 是从扫描器输入和手动发布审核派生出的有效信任状态。
- `trust.moderationState` 可为空。没有手动发布审核时为 `null`。
- `trust.blockedFromDownload` 是安装阻止信号。当此值为 `true` 时，OpenClaw 和其他安装客户端应阻止安装，
...
- `trust.stale` 表示信任摘要是根据过时输入计算的，在做出高置信度允许决策前应视为需要刷新。

说明：

- 此端点精确到版本。客户端应在解析出打算安装的包版本后调用它，而不是只在读取最新包元数据后调用。
- 私有包会返回 `404`，除非调用方可以读取所属发布者。
- 此端点有意比所有者/审核员审核端点更窄。它公开安装决策和公开解释，
  而不公开举报者身份、举报正文、私有证据或内部审核时间线。
...
- 旧版包版本返回 `legacy-zip` 产物和旧版 ZIP `downloadUrl`。
- ClawPack 版本返回 `npm-pack` 产物、npm 完整性字段、`tarballUrl` 以及旧版 ZIP 兼容性 URL。
- 这是 OpenClaw 解析器接口；它避免通过共享 URL 猜测归档格式。
...
举报一个包以供审核员审查。举报是包级别的，可选择关联到某个版本。
它们会进入审核队列，但本身不会自动隐藏或阻止下载；审核员应使用发布审核来批准、隔离或撤销产物。
...
- 需要包所有者、发布者成员、版主或管理员用户的 API token。
...
对于 `confirmed` 和 `dismissed`，`note` 为必填；将 `status` 设回 `open` 时可以省略。对已确认的报告传入 `finalAction: "quarantine"` 或 `finalAction: "revoke"`，即可在同一个可审计工作流中应用发布审核。
...
被隔离和被撤销的发布会从制品下载路由返回 `403`。每次更改都会写入一条审计日志条目。
...
返回由 ClawPack 支撑的包版本的 npm 兼容 packument。
...
下载托管的 Skills 版本 ZIP，或者针对当前 GitHub 支撑且扫描状态为 `clean` 或 `suspicious`、并且没有托管版本的 Skills，返回 GitHub 源交接信息。
...
- 如果未提供 `version` 或 `tag`，则使用最新版本。
- 软删除版本返回 `410`。
- GitHub 支撑的 Skills 交接不会代理或镜像字节。JSON 响应包含 `sourceRef: "public-github"`、`repo`、`commit`、`path`、`contentHash` 和 `archiveUrl`；扫描/当前状态是门控条件，不会作为成功载荷元数据包含在内。
...
- 可选载荷字段：`migrateOwner`。当它与 `ownerHandle` 一起为 `true` 时，如果操作者同时是当前发布者和目标发布者的管理员/所有者，现有 Skills 可以迁移到该所有者。如果没有此选择加入，所有者更改会被拒绝。
...
- 允许的表单字段是 `payload`、重复的 `files` blob，或一个 `clawpack` tarball 引用。`clawpack` 可以是 `.tgz` blob，也可以是由 upload-url 流程返回的 storage id。基于暂存 storage-id 的发布还必须包含随该上传 URL 返回的 `clawpackUploadTicket`。
- 使用 `files` 或 `clawpack` 二选一，同一请求中不能同时使用。
- JSON body 以及调用方提供的 `payload.files` / `payload.artifact` 元数据会被拒绝。
...
- 代码插件需要 `package.json`、源仓库元数据、源 commit 元数据、配置 schema 元数据、`openclaw.compat.pluginApi` 和 `openclaw.build.openclawVersion`。
...
存在时，`reason` 会存储为 Skills 审核备注，并复制到审计日志中。
所有者发起的软删除会保留 slug 30 天，之后该 slug 可由其他发布者认领。删除响应在适用此过期时间时包含 `slugReservedUntil`。
...
已认证的自助组织发布者创建。创建新的组织发布者，并将调用方添加为所有者。此端点不会迁移现有用户/个人 handle，也不会将发布者标记为受信任/官方。
...
仅管理员。为合法所有者保留根 slug 和包名称，而不发布任何 release。包名称会变成没有 release 行的私有占位包，因此同一所有者之后可以将真实的 code-plugin 或 bundle-plugin release 发布到该名称。
...
仅管理员。为已验证的替代 GitHub OAuth 主体恢复个人发布者，而无需编辑 Convex Auth 账号行。请求必须命名两个不可变 GitHub provider account id；可变 handle 仅用作面向操作者的保护条件。

端点默认使用 dry-run。应用恢复需要在工作人员独立验证两个 GitHub 主体之间的连续性后设置 `dryRun: false` 和
`confirmIdentityVerified: true`。当目标用户当前的个人发布者拥有技能、包或 GitHub 技能来源时，恢复会失败并关闭。
恢复还会迁移已恢复发布者的技能、技能 slug 别名、包、包检查器警告和派生搜索摘要行中的旧版 `ownerUserId` 字段，使直接所有者路径与新的发布者权限保持一致。已恢复 handle 的活跃受保护 handle 预留也会重新分配给替代用户，这样后续个人资料同步就无法恢复前用户的竞争权限。每个主表在每个应用事务中限制为
100 行；更大的恢复必须先使用可恢复的所有者迁移。
GitHub 技能来源按发布者作用域处理，并报告为已检查，而不是被重写。
...
更改现有封禁存储的原因，不解除封禁或恢复内容（仅限管理员）。除非 `dryRun` 为 `false`，否则默认为 dry-run。
...
`POST /api/cli/upload-url` 返回 `uploadUrl` 和 `uploadTicket`。用于暂存 ClawPack tarball 的包发布必须将生成的存储 ID 作为 `clawpack` 发送，并将返回的 ticket 作为 `clawpackUploadTicket` 发送。
