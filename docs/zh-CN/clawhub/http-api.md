...
第三方目录可以使用公共读取端点来列出或搜索 ClawHub 技能。请缓存结果，遵守 `429`/`Retry-After`，将用户链接回规范的 ClawHub 列表页（`https://clawhub.ai/<owner>/skills/<slug>`），并避免暗示 ClawHub 为该第三方站点背书。不要尝试在公共 API 表面之外镜像隐藏、私有或被审核屏蔽的内容。
...
- 如果令牌缺失或无效，行为会回退到 IP 执行。
- 当服务器知道原因时，已认证写入端点不应返回裸 `Unauthorized`。缺失令牌、无效/已撤销令牌，以及已删除/被封禁/被禁用的账号都应获得可操作的文本，这样 CLI 客户端可以告诉用户是什么阻止了他们。

- 读取：每 IP 3000/min，每密钥 12000/min
- 写入：每 IP 300/min，每密钥 3000/min
- 下载：每 IP 1200/min，每密钥 6000/min（下载端点）

标头：

- 旧版兼容：`X-RateLimit-Limit`、`X-RateLimit-Reset`
- 标准化：`RateLimit-Limit`、`RateLimit-Reset`
- `429` 时：`X-RateLimit-Remaining: 0` 和 `RateLimit-Remaining: 0`
- `429` 时：`Retry-After`
...
- 如果没有可用的受信任客户端 IP，匿名请求会使用仅按速率限制类型限定作用域的回退桶。这些回退桶不包含调用方提供的路径、slug、包名、版本、查询字符串或其他构件参数。

## 错误响应

公共 v1 错误响应是纯文本，带有 `content-type: text/plain; charset=utf-8`。
这包括验证失败（`400`）、缺失公共资源（`404`）、认证和权限失败（`401`/`403`）、速率限制（`429`）以及被阻止的下载。客户端应将响应正文读取为人类可读字符串。未知查询参数会因兼容性被忽略，但已识别查询参数如果值无效，则返回 `400`。
...
- 结果按相关性顺序返回（嵌入相似度 + 精确 slug/名称词元提升 + 小幅流行度先验）。
- 相关性强于流行度。精确的 slug 或显示名称词元匹配可能超过互动量强得多但匹配更宽松的结果。
...
- 流行度会经过对数缩放并设置上限。当查询文本匹配较弱时，高互动技能可能排名更低。
- 可疑或隐藏的审核状态可能会根据调用方筛选器和当前审核状态，将技能从公共搜索中移除。

发布者可发现性指南：

- 将用户会直接搜索的术语放入显示名称、摘要和标签。只有当独立 slug 词元也是你想保留的稳定身份时，才使用它。
- 不要仅为了追逐某个查询而重命名 slug，除非新 slug 是更好的长期规范名称。旧 slug 会成为重定向别名，但规范 URL、显示的 slug 和未来的搜索摘要会使用新 slug。
- 重命名别名会为通过注册表解析的旧 URL 和安装保留解析能力，但搜索排名基于重命名完成索引后的规范技能元数据。现有统计数据会保留在该技能上。
- 如果某个技能意外不可见，请先在已登录状态下使用 `clawhub inspect @owner/slug` 检查审核状态，然后再更改与排名相关的元数据。
...
- `recommended` 使用互动度和近期性信号。
- `trending` 按最近 7 天的安装量排名（基于遥测）。
- `createdAt` 对新技能抓取是稳定的；`updated` 会在现有技能重新发布时变化。
- 当 `nonSuspiciousOnly=true` 时，基于游标的排序在某一页返回的条目数可能少于 `limit`，因为可疑技能是在页面检索后过滤的。
- 当存在 `nextCursor` 时，使用它继续分页。页面较短本身并不表示结果结束。
...
- 公共调用方看到的证据会被脱敏，并且只有所有者/审核员可看到原始片段。

### `POST /api/v1/skills/{slug}/report`

报告技能以供审核员审查。报告是技能级别的，可选择链接到某个版本，并会进入技能报告队列。
...
`note` 对 `confirmed` 和 `dismissed` 是必需的；将 `status` 设回 `open` 时可以省略。对已分诊报告传入 `finalAction: "hide"`，可在同一个可审计工作流中隐藏该技能。
...
- 扫描作业是异步的。手动扫描请求优先级高于普通发布/回填工作，但完成时间仍取决于工作器可用性。
...
- 返回已排队/运行中/成功/失败状态。
- 排队期间返回 `queue.queuedAhead` 和 `queue.position`，以便客户端显示该请求前面还有多少个优先级更高的手动扫描。非常大的队列会被限制，并以 `queuedAheadIsEstimate: true` 报告。
...
- 只有当所选版本具有已生成的技能卡片、未被审核判定为恶意软件阻止，并且 ClawScan 验证为干净时，`ok` 才为 `true`。
...
- `security` 是顶层 ClawScan/安全结论。自动化应以 `ok`、`decision`、`reasons` 和 `security.status` 为准。
- `security.signals` 包含支持性的扫描器证据，例如 `staticScan`、`virusTotal` 和 `skillSpector`。
- `security.signals.dependencyRegistry` 会保留用于 v1 响应兼容性，但依赖注册表存在性扫描器已停用，并且此键始终为 `null`。
...
- 响应仅包含安全信息。它不包含技能卡片数据、已生成卡片状态、产物文件列表或详细扫描器载荷。
- `security.signals` 仅包含状态级支持证据；如需完整扫描器详细信息，请使用 `/scan` 或 ClawHub 安全审计页面。
- `security.signals.dependencyRegistry` 会保留用于 v1 响应兼容性，但依赖注册表存在性扫描器已停用，并且此键始终为 `null`。
- 缺少技能卡片不会影响此端点的 `ok`、`decision` 或 `reasons`；客户端在需要卡片内容时，应在本地读取已安装的 `skill-card.md`。
- 当你需要单个技能的技能卡片验证信封时使用 `/verify`，需要已生成的卡片 Markdown 时使用 `/card`，需要详细扫描器数据时使用 `/scan`。
...
- `category`（可选）：插件类别过滤器。仅在请求范围限定为插件包（`/api/v1/plugins`、`/api/v1/code-plugins`、`/api/v1/bundle-plugins`，或带有 `family=code-plugin`/`family=bundle-plugin` 的包端点）时支持。受控类别和旧版 v1 过滤器别名记录在 `GET /api/v1/plugins` 下。
...
- 技能条目仍由技能注册表支撑，并且仍只能通过 `POST /api/v1/skills` 发布。
...
- `channel`（可选）：`official`、`community` 或 `private`
- `isOfficial`（可选）：`true` 或 `false`
- `category`（可选）：插件类别过滤器。仅在请求范围限定为插件包时支持。受控类别和旧版 v1 过滤器别名记录在 `GET /api/v1/plugins` 下。
...
- 正文：ZIP 归档。
- 每个导出的技能都以 `{publisher}/{slug}/` 为根路径。
- 托管技能包含最新存储版本文件，并在 `_manifest.json` 中以 `sourceRef: "public-clawhub"` 列出。
- 当前以 GitHub 为来源的技能，如果扫描结果为 `clean` 或 `suspicious`，则包含 `_source_handoff.json`，其中含有 `sourceRef: "public-github"`、仓库、提交、路径、内容哈希和归档 URL。它们不包含 ClawHub 托管的源文件。
...
- `limit`（可选）：整数（1-250），默认 `250`。
- `cursor`（可选）：来自上一个响应的分页游标。
- `family`（可选）：`code-plugin` 或 `bundle-plugin`。省略表示两个插件族。
...
在 `code-plugin` 和 `bundle-plugin` 包之间执行仅限插件的搜索。
...
- 结果按相关性顺序返回，目前不分页。
- 浏览器 UI 中的插件搜索排序控件会重新排序已加载的相关性结果，
  与当前 `/skills` 浏览行为一致。
...
返回一个包版本，包括文件元数据、兼容性、验证、产物元数据和扫描数据。

说明：

- 对于旧世界包归档，`version.artifact.kind` 为 `legacy-zip`；对于由 ClawPack 支持的发布版本，则为
  `npm-pack`。
...
- `version.sha256hash` 是面向旧客户端的已弃用兼容性元数据。它对 `/api/v1/packages/{name}/download` 返回的精确 ZIP 字节进行哈希。
  现代客户端应使用 `version.artifact.sha256`，它用于标识规范发布产物。
- 当扫描数据存在时，会包含 `version.vtAnalysis`、`version.llmAnalysis` 和 `version.staticScan`。
- 私有包返回 `404`，除非调用方可以读取所属发布者。

### `GET /api/v1/packages/{name}/versions/{version}/security`

返回用于安装客户端的精确包发布安全与信任摘要。这是 OpenClaw 用于判断已解析发布版本是否可安装的公开消费表面。
...
- 当发布产物已知时，会提供 `release.artifactKind`、`release.artifactSha256`、`release.npmIntegrity`、
...
- `trust.stale` 表示信任摘要是基于过期输入计算的，在做出高置信度允许决策前应视为需要刷新。

说明：

- 此端点精确到版本。客户端应在解析出它们打算安装的包版本后调用它，而不是仅在读取最新包元数据后调用。
- 私有包返回 `404`，除非调用方可以读取所属发布者。
...
- 旧版包版本返回一个 `legacy-zip` 产物和旧版 ZIP `downloadUrl`。
- ClawPack 版本返回一个 `npm-pack` 产物、npm 完整性字段、一个
...
返回用于未来 OpenClaw 消费的已计算就绪状态。

就绪检查覆盖：

- 官方渠道状态
- 最新版本可用性
- ClawPack npm-pack 产物可用性
- 产物摘要
- 源仓库和提交来源
- OpenClaw 兼容性元数据
- 主机目标
...
- `bundledPluginId` 会被规范化为小写，并作为稳定的 upsert 键。
- `packageName` 会按 npm 名称规范化；对于计划中的迁移，包可以缺失。
- 这只跟踪迁移就绪状态。它不会修改 OpenClaw 或生成 ClawPack。
...
- `open`：可疑、恶意、待处理、已隔离、已撤销或已被报告的发布版本。
- `blocked`：已隔离、已撤销或恶意的发布版本。
- `manual`：任何带有手动仲裁覆盖的发布版本。
...
报告一个包以供仲裁员审核。报告是包级别的，可选链接到一个版本。它们会进入仲裁队列，但本身不会自动隐藏或阻止下载；仲裁员应使用发布仲裁来批准、隔离或撤销产物。
...
`note` 对 `confirmed` 和 `dismissed` 是必需的；将 `status` 设回 `open` 时可以省略。对已确认的报告传入 `finalAction: "quarantine"` 或 `finalAction: "revoke"`，可在同一个可审计工作流中应用发布版本审核。
...
- `revoked`：在发布版本先前受信任后被阻止。

已隔离和已撤销的发布版本会从构件下载路由返回 `403`。每次变更都会写入一条审计日志记录。
...
- 旧版仅 ZIP 的版本会有意省略。
- `dist.tarball`、`dist.integrity` 和 `dist.shasum` 使用 npm 兼容字段，因此用户可以在愿意时将 npm 指向该镜像。
...
- 可选载荷字段：`ownerHandle`。存在时，API 会在服务端解析该发布者，并要求操作者具有发布者访问权限。
- 可选载荷字段：`migrateOwner`。当它与 `ownerHandle` 一起为 `true` 时，如果操作者同时是当前发布者和目标发布者的管理员/所有者，现有技能可以迁移到该所有者。没有此显式选择时，所有者变更会被拒绝。
...
- 允许的表单字段为 `payload`、重复的 `files` blob，或一个 `clawpack` tarball 引用。`clawpack` 可以是 `.tgz` blob，也可以是 upload-url 流程返回的存储 ID。基于暂存 storage-id 的发布还必须包含随该上传 URL 返回的 `clawpackUploadTicket`。
- 使用 `files` 或 `clawpack` 二者之一，绝不要在同一个请求中同时使用。
- JSON 正文和调用者提供的 `payload.files` / `payload.artifact` 元数据会被拒绝。
...
所有者发起的软删除会保留 slug 30 天，之后该 slug 可由其他发布者认领。适用此到期规则时，删除响应会包含 `slugReservedUntil`。
...
经过身份验证的自助式组织发布者创建。创建新的组织发布者，并将调用者添加为所有者。此端点不会迁移现有用户/个人 handle，也不会将发布者标记为受信任/官方。
...
仅管理员。为合法所有者保留根 slug 和包名，而不发布发布版本。包名会成为没有发布版本行的私有占位包，因此同一所有者之后可以将真正的 code-plugin 或 bundle-plugin 发布版本发布到该名称下。
...
仅管理员。为已验证的替代 GitHub OAuth 主体恢复个人发布者，而不编辑 Convex Auth 账号行。请求必须命名两个不可变的 GitHub 提供商账号 ID；可变 handle 仅用作面向操作者的保护检查。

该端点默认是 dry-run。应用恢复需要在工作人员独立验证两个 GitHub 主体之间的连续性后设置 `dryRun: false` 和
`confirmIdentityVerified: true`。当目标用户当前的个人发布者拥有 skills、packages 或 GitHub skill sources 时，恢复会失败并关闭。
恢复还会迁移已恢复发布者的 skills、skill slug aliases、packages、package inspector warnings 和 derived search digest rows 的旧版 `ownerUserId` 字段，使直接所有者路径与新的发布者权限保持一致。已恢复 handle 的有效 protected-handle reservation 也会重新分配给替换用户，因此后续 profile synchronization 无法恢复前用户的竞争权限。每个主表在每次应用事务中限制为
100 行；更大的恢复必须先使用可恢复的所有者迁移。
GitHub skill sources 以发布者为作用域，并报告为已检查，而不是被重写。
...
- 两个端点都需要 API token auth，并且仅适用于 skill owner。
- `rename` 会将先前的 slug 保留为重定向别名。
- `merge` 会隐藏源 listing，并将源 slug 重定向到目标 listing。
...
- `GET /api/v1/transfers/outgoing`
  - Response 形状：`{ "transfers": [{ "_id": "...", "skill": { "slug": "demo", "displayName": "Demo" }, "fromUser"|"toUser": { "handle": "..." }, "message": "...", "requestedAt": 0, "expiresAt": 0 }] }`

### `POST /api/v1/users/ban`

封禁用户并硬删除其拥有的 skills（仅 moderator/admin）。
...
内容（仅 admin）。除非 `dryRun` 为 `false`，否则默认为 dry-run。
...
添加/移除 star（highlights）。两个端点都是幂等的。
