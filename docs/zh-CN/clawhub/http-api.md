...
x-i18n:
    generated_at: "2026-06-27T01:31:34Z"
    model: gpt-5.5
...
第三方目录可以使用公共读取端点来列出或搜索 ClawHub 技能。请缓存结果，遵守 `429`/`Retry-After`，将用户链接回规范的 ClawHub 列表页（`https://clawhub.ai/<owner>/skills/<slug>`），并避免暗示 ClawHub 认可该第三方站点。不要尝试在公共 API 表面之外镜像隐藏、私有或因审核被阻止的内容。

网页 slug 快捷方式会跨注册表系列解析，但 API 客户端应使用读取端点返回的规范 URL，而不是重新构造路由优先级。
...
- `Retry-After`：遇到 `429` 时重试前需要等待的秒数（延迟）
...
- 如果没有可用的可信客户端 IP，匿名请求会使用仅按速率限制类型限定范围的回退桶。这些回退桶不包含调用方提供的路径、slug、包名、版本、查询字符串或其他构件参数。

## 错误响应

公共 v1 错误响应是纯文本，`content-type: text/plain; charset=utf-8`。
这包括验证失败（`400`）、缺失公共资源（`404`）、认证和权限失败（`401`/`403`）、速率限制（`429`）以及被阻止的下载。客户端应将响应正文读取为人类可读字符串。为兼容性起见，未知查询参数会被忽略，但已识别且值无效的查询参数会返回 `400`。
...
- `q`（必需）：查询字符串
- `limit`（可选）：整数
- `highlightedOnly`（可选）：设为 `true` 以过滤到高亮技能
...
- 结果按相关性顺序返回（embedding 相似度 + 精确 slug/名称 token 加权 + 小幅流行度先验）。
- 相关性强于流行度。精确的 slug 或显示名称 token 匹配可以排在互动量强得多但匹配更宽泛的结果之前。
...
- 流行度会进行对数缩放并设置上限。当查询文本匹配较弱时，高互动技能也可能排名更低。
- 可疑或隐藏的审核状态可能会根据调用方过滤器和当前审核状态，从公共搜索中移除某个技能。

发布者可发现性指导：

- 将用户会实际搜索的词放入显示名称、摘要和标签中。仅当独立 slug token 也是你想保留的稳定身份时才使用它。
- 除非新 slug 是更好的长期规范名称，否则不要只是为了追逐某个查询而重命名 slug。旧 slug 会成为重定向别名，但规范 URL、显示的 slug 和未来搜索摘要会使用新 slug。
- 重命名别名会保留旧 URL 和通过注册表解析的安装的解析能力，但重命名完成索引后，搜索排名基于规范技能元数据。现有统计会保留在该技能上。
- 如果某个技能意外不可见，请先在登录状态下使用 `clawhub inspect @owner/slug` 检查审核状态，然后再更改排名相关元数据。
...
- 当 `nonSuspiciousOnly=true` 时，基于游标的排序可能在某一页返回少于 `limit` 项，因为可疑技能会在页面取回后被过滤。
...
- 面向公共调用方时，证据会被脱敏，并且只有所有者/审核员才能看到原始片段。

### `POST /api/v1/skills/{slug}/report`

举报一个技能以供审核员审查。举报是技能级别的，可选链接到某个版本，并会进入技能举报队列。
...
`note` 对 `confirmed` 和 `dismissed` 是必需的；将 `status` 设回 `open` 时可以省略。传入 `finalAction: "hide"` 可在同一个可审计工作流中隐藏已分流的举报对应技能。
...
- 排队期间返回 `queue.queuedAhead` 和 `queue.position`，以便客户端显示该请求前面还有多少个优先级手动扫描。非常大的队列会被限制，并以 `queuedAheadIsEstimate: true` 报告。
...
- 只有当所选版本已生成技能卡片、未被审核判定为恶意软件而屏蔽，并且 ClawScan 验证为干净时，`ok` 才为 `true`。
- 技能身份、发布者身份和所选版本元数据是顶层封装字段（`slug`、`displayName`、`publisherHandle`、`version`、`resolvedFrom`、`tag`、`createdAt`），因此 shell 自动化无需解包嵌套包装器即可读取它们。
...
- 为了兼容 v1 响应，保留了 `security.signals.dependencyRegistry`，但依赖注册表存在性扫描器已退役，因此此键始终为 `null`。
...
- 响应仅包含安全信息。它不包含技能卡片数据、已生成卡片状态、制品文件列表或详细扫描器载荷。
...
- 需要单个技能的技能卡片验证封装时使用 `/verify`，需要已生成卡片 Markdown 时使用 `/card`，需要详细扫描器数据时使用 `/scan`。
...
- 需要包所有者、组织发布者所有者/管理员、平台主持人或平台管理员的 API 令牌。
...
- 对于旧式包归档，`version.artifact.kind` 为 `legacy-zip`；对于由 ClawPack 支持的发布版本，则为
  `npm-pack`。
- ClawPack 发布版本包含与 npm 兼容的 `npmIntegrity`、`npmShasum` 和
  `npmTarballName` 字段。
- `version.sha256hash` 是面向旧客户端的已弃用兼容性元数据。它会哈希
  `/api/v1/packages/{name}/download` 返回的精确 ZIP 字节。
...
返回用于安装客户端的精确包发布安全和信任摘要。这是 OpenClaw 用于判断已解析发布版本是否可安装的公开消费接口。

认证：

- 公开读取端点。不需要所有者、发布者、主持人或管理员令牌。
...
- 私有包会返回 `404`，除非调用方可以读取所属发布者。
- 此端点有意比所有者/主持人审核端点更窄。它公开安装决策和公开解释，而不公开举报者身份、举报正文、私有证据或内部评审时间线。
...
- 这仅跟踪迁移就绪状态。它不会更改 OpenClaw，也不会生成 ClawPack。

### `GET /api/v1/packages/moderation/queue`

用于包发布版本评审队列的主持人/管理员端点。
...
举报一个包以供主持人评审。举报是包级别的，可选关联到某个版本。它们会进入审核队列，但本身不会自动隐藏或阻止下载；主持人应使用发布审核来批准、隔离或撤销工件。
...
- `revoked`：在发布版本之前受信任后被阻止。

隔离和吊销的发布版本会从制品下载路由返回 `403`。每次更改都会写入一条审计日志条目。
...
- 作用域软件包 packument 同时支持 `/api/npm/@scope/name` 和 npm 编码的 `/api/npm/@scope%2Fname` 请求路径。
...
下载托管的技能版本 ZIP，或为当前由 GitHub 支持、扫描结果为 `clean` 或 `suspicious` 且没有托管版本的技能返回 GitHub 源码交接。
...
- 可选载荷字段：`migrateOwner`。当它与 `ownerHandle` 一起为 `true` 时，如果操作者同时是当前发布者和目标发布者的管理员/所有者，现有技能可以迁移到该所有者。没有此显式选择时，所有者更改会被拒绝。
...
- 允许的表单字段为 `payload`、重复的 `files` blob，或一个 `clawpack` tarball 引用。`clawpack` 可以是 `.tgz` blob，也可以是上传 URL 流程返回的存储 id。使用暂存 storage-id 发布时，还必须包含该上传 URL 返回的 `clawpackUploadTicket`。
- 使用 `files` 或 `clawpack` 之一，绝不要在同一个请求中同时使用两者。
...
存在时，`reason` 会作为技能审核备注存储，并复制到审计日志中。所有者发起的软删除会保留 slug 30 天，之后该 slug 可由其他发布者认领。删除响应会在适用此过期时间时包含 `slugReservedUntil`。主持人/管理员隐藏和安全移除不会以这种方式过期。
...
仅管理员。确保某个 handle 存在组织发布者。如果该 handle 仍指向旧版共享用户/个人发布者，端点会先将其迁移为组织发布者。对于新建组织，请提供 `memberHandle`；执行操作的管理员不会被添加为成员。`memberRole` 默认为 `owner`。
...
已认证的自助式组织发布者创建。创建新的组织发布者并将调用方添加为所有者。此端点不会迁移现有用户/个人 handle，也不会将发布者标记为可信/official。
...
仅管理员。为合法所有者保留根 slug 和软件包名称，而不发布发布版本。软件包名称会变成没有发布版本行的私有占位软件包，因此同一所有者稍后可以将真正的 code-plugin 或 bundle-plugin 发布版本发布到该名称。
...
仅管理员。为已验证的替代 GitHub OAuth 主体恢复个人发布者，而不编辑 Convex Auth 账号行。请求必须命名两个不可变 GitHub 提供商账号 ID；可变 handle 仅用作面向操作者的保护检查。

该端点默认使用 dry-run。应用恢复需要在工作人员独立验证两个 GitHub 主体之间的连续性后设置 `dryRun: false` 和
`confirmIdentityVerified: true`。
当目标用户当前的个人发布者拥有技能、包或 GitHub 技能来源时，恢复会以失败关闭方式处理。
恢复还会迁移已恢复发布者的技能、技能 slug 别名、包、包检查器警告以及派生搜索摘要行中的旧版 `ownerUserId` 字段，使直接所有者路径与新的发布者权限保持一致。已恢复 handle 的有效受保护 handle 预留也会重新分配给替换用户，以便后续个人资料同步无法恢复前用户的竞争性权限。每个主表在每次应用事务中限制为
100 行；更大的恢复必须先使用可恢复的所有者迁移。
GitHub 技能来源按发布者范围管理，并报告为已检查，而不是重写。
...
- 两个端点都需要 API token 身份验证，并且仅对技能所有者可用。
...
更改现有封禁的已存储原因，但不解除封禁或恢复内容（仅限管理员）。除非 `dryRun` 为 `false`，否则默认使用 dry-run。
