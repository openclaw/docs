...
x-i18n:
    generated_at: "2026-07-01T18:06:20Z"
    model: gpt-5.5
...
- 当服务器知道原因时，已认证写入端点不应返回裸的 `Unauthorized`。缺失令牌、无效/已撤销令牌，以及已删除/封禁/禁用的账户都应获得可操作的文本，以便 CLI 客户端可以告诉用户是什么阻止了他们。
...
- 如果没有可用的可信客户端 IP，匿名请求会使用仅按速率限制种类限定范围的回退桶。这些回退桶不包含调用方提供的路径、slug、包名、版本、查询字符串或其他制品参数。

## 错误响应

公共 v1 错误响应是纯文本，`content-type: text/plain; charset=utf-8`。
这包括验证失败（`400`）、缺失公共资源（`404`）、认证和权限失败（`401`/`403`）、速率限制（`429`）以及被阻止的下载。客户端应将响应正文读取为人类可读字符串。未知查询参数会出于兼容性被忽略，但已识别查询参数的无效值会返回 `400`。
...
- 结果按相关性顺序返回（嵌入相似度 + 精确 slug/名称词元提升 + 小幅人气先验）。
- 相关性强于人气。精确的 slug 或显示名称词元匹配可以排在参与度高得多但匹配更宽松的结果之前。
...
- 人气会进行对数缩放并设置上限。当查询文本匹配较弱时，高参与度技能可能排名更低。
- 可疑或隐藏的审核状态可能会根据调用方筛选条件和当前审核状态，从公共搜索中移除某个技能。
...
- 重命名别名会保留旧 URL 和通过注册表解析的安装的解析能力，但搜索排名基于重命名已完成索引后的规范技能元数据。现有统计数据会保留在该技能上。
...
- 当 `nonSuspiciousOnly=true` 时，基于游标的排序可能会在一页中返回少于 `limit` 的项目，因为可疑技能会在页面检索后被筛除。
- 存在 `nextCursor` 时使用它继续分页。短页本身并不表示结果结束。
...
- 证据会对公共调用方进行脱敏，且仅对所有者/审核员包含原始片段。
...
`confirmed` 和 `dismissed` 需要 `note`；将 `status` 设回 `open` 时可以省略。传入 `finalAction: "hide"` 可在同一个可审计工作流中隐藏已分诊举报对应的技能。
...
- 扫描请求载荷和可下载报告会在保留窗口结束后从扫描请求存储中到期。
...
- 只有当所选版本已生成技能卡片、未被审核标记为恶意软件阻止，并且 ClawScan 验证为 clean 时，`ok` 才为 `true`。
...
- 响应仅包含安全信息。不包含技能卡片数据、已生成卡片状态、工件文件列表或详细扫描器载荷。
...
- 当你需要单个技能的技能卡片验证信封时，请使用 `/verify`；当你需要生成的卡片 Markdown 时，请使用 `/card`；当你需要详细扫描器数据时，请使用 `/scan`。
...
- 对旧世界包归档，`version.artifact.kind` 为 `legacy-zip`；对 ClawPack 支持的版本，则为
  `npm-pack`。
...
返回供安装客户端使用的精确包发布安全与信任摘要。这是公共 OpenClaw 消费界面，用于决定解析出的版本是否可以安装。
...
- `trust.moderationState` 可为空。当没有手动发布审核时，它为 `null`。
...
- 此端点精确到版本。客户端应在解析出要安装的包版本后调用它，而不是只在读取最新包元数据后调用。
- 私有包返回 `404`，除非调用方可以读取所属发布者。
- 此端点有意比所有者/版主审核端点更窄。它公开安装决策和公共说明，而不是报告者身份、报告正文、私有证据或内部审核时间线。
...
报告一个包以供版主审核。报告是包级别的，可选择关联到某个版本。它们会进入审核队列，但本身不会自动隐藏或阻止下载；版主应使用发布审核来批准、隔离或撤销构件。
...
`note` 对 `confirmed` 和 `dismissed` 是必需的；将 `status` 设置回 `open` 时可以省略。传入 `finalAction: "quarantine"` 或 `finalAction: "revoke"` 并配合已确认报告，可在同一个可审计工作流中应用发布审核。
...
- `revoked`：在发布版本此前受信任后被阻止。

隔离和撤销的发布版本会从工件下载路由返回 `403`。每次更改都会写入审计日志条目。
...
为 npm 镜像客户端流式传输确切已上传的 ClawPack tarball 字节。
...
下载托管技能版本 ZIP，或为当前由 GitHub 支持、扫描结果为 `clean` 或 `suspicious` 且没有托管版本的技能返回 GitHub 源码交接。
...
- 下载统计按 UTC 日期以唯一身份计数（API 令牌有效时为 `userId`，否则为 IP）。
...
- 也接受带有 `files`（基于 storageId）的 JSON 主体。
- 可选载荷字段：`ownerHandle`。存在时，API 会在服务器端解析该发布者，并要求执行者拥有发布者访问权限。
- 可选载荷字段：`migrateOwner`。当它与 `ownerHandle` 一起为 `true` 时，如果执行者在当前发布者和目标发布者上都是管理员/所有者，则现有技能可以移动到该所有者。没有此显式选择时，所有者变更会被拒绝。
...
- 允许的表单字段为 `payload`、重复的 `files` blob，或一个 `clawpack` tarball 引用。`clawpack` 可以是 `.tgz` blob，也可以是上传 URL 流程返回的 storage id。使用暂存 storage-id 发布时，还必须包含随该上传 URL 返回的 `clawpackUploadTicket`。
- 使用 `files` 或 `clawpack` 二者之一，切勿在同一请求中同时使用。
...
- 插件包需要 `openclaw.plugin.json`。ClawPack `.tgz` 上传必须在 `package/openclaw.plugin.json` 处包含它。
...
由所有者发起的软删除会保留 slug 30 天，之后该 slug 可由另一个发布者声明。适用此到期时，删除响应会包含 `slugReservedUntil`。
...
仅管理员。为合法所有者保留根 slug 和包名称，而不发布发布版本。包名称会变成没有发布行的私有占位包，因此同一所有者稍后可以将真正的 code-plugin 或 bundle-plugin 发布版本发布到该名称下。
...
仅管理员。为已验证的替换 GitHub OAuth principal 恢复个人发布者，而无需编辑 Convex Auth 账号行。请求必须命名两个不可变 GitHub provider 账号 ID；可变 handle 仅用作面向操作员的防护。

该端点默认执行试运行。应用恢复需要在工作人员独立验证两个
GitHub 主体之间的连续性后设置 `dryRun: false` 和
`confirmIdentityVerified: true`。当目标用户当前的个人发布者拥有技能、包或 GitHub 技能来源时，恢复会失败并关闭。
恢复还会迁移已恢复发布者的技能、技能 slug 别名、包、包检查器警告和派生搜索摘要行中的旧版 `ownerUserId` 字段，使直接所有者路径与新的发布者权限保持一致。已恢复 handle 的有效受保护 handle 保留也会重新分配给替代用户，避免后续个人资料同步恢复前用户的竞争权限。每个主表在每次应用事务中限制为
100 行；更大的恢复必须先使用可恢复的所有者迁移。
GitHub 技能来源按发布者限定作用域，并报告为已检查，而不是被重写。
...
更改现有封禁的已存储原因，而不解除封禁或恢复内容（仅限管理员）。除非 `dryRun` 为 `false`，否则默认执行试运行。
...
`POST /api/cli/upload-url` 返回 `uploadUrl` 和 `uploadTicket`。将 ClawPack tarball 暂存的包发布必须将生成的存储 id 作为
