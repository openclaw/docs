...
x-i18n:
    generated_at: "2026-07-03T13:15:40Z"
    model: gpt-5.5
...
第三方目录可以使用公共读取端点来列出或搜索 ClawHub 技能。请缓存结果，遵守 `429`/`Retry-After`，将用户链接回规范的 ClawHub 列表（`https://clawhub.ai/<owner>/skills/<slug>`），并避免暗示 ClawHub 认可该第三方站点。不要尝试在公共 API 表面之外镜像隐藏、私有或被审核阻止的内容。
...
- 已认证写入端点在服务器知道原因时，不应返回裸 `Unauthorized`。缺失令牌、无效/已吊销令牌，以及已删除/封禁/禁用账户，都应获得可操作的文本，以便 CLI 客户端可以告诉用户被什么阻止。
...
- 如果没有可用的可信客户端 IP，匿名请求会使用仅按速率限制类型限定范围的回退桶。这些回退桶不包括调用方提供的路径、slug、包名、版本、查询字符串或其他制品参数。

## 错误响应

公共 v1 错误响应是纯文本，带有 `content-type: text/plain; charset=utf-8`。
这包括验证失败（`400`）、缺失的公共资源（`404`）、认证和权限失败（`401`/`403`）、速率限制（`429`）以及被阻止的下载。客户端应将响应正文作为人类可读字符串读取。未知查询参数会为了兼容性而被忽略，但值无效的已识别查询参数会返回 `400`。
...
- 相关性强于流行度。精确的 slug 或显示名称词元匹配，可能会排在参与度高得多但匹配更宽松的结果之前。
- ASCII 文本会按单词和标点边界分词。例如，`personal-map` 包含独立的 `map` 词元，而 `amap-jsapi-skill` 包含 `amap`、`jsapi` 和 `skill`；因此搜索 `map` 时，`personal-map` 的词法匹配会强于 `amap-jsapi-skill`。
- 流行度会进行对数缩放并设置上限。当查询文本匹配较弱时，高参与度技能的排名可能更低。
- 可疑或隐藏的审核状态可能会根据调用方筛选条件和当前审核状态，从公共搜索中移除技能。

发布者可发现性指南：

- 将用户会按字面搜索的术语放入显示名称、摘要和标签中。仅当独立 slug 词元也是你想保留的稳定身份时才使用它。
- 不要只为追逐某个查询而重命名 slug，除非新 slug 是更好的长期规范名称。旧 slug 会成为重定向别名，但规范 URL、显示的 slug 和未来搜索摘要会使用新 slug。
- 重命名别名会为旧 URL 和通过注册表解析的安装保留解析能力，但搜索排名基于重命名索引后规范的技能元数据。现有统计数据仍保留在该技能上。
- 如果技能意外不可见，在更改与排名相关的元数据前，请先登录并使用 `clawhub inspect @owner/slug` 检查审核状态。
...
- 面向公共调用方的证据会被脱敏，并且仅为所有者/审核员包含原始片段。

### `POST /api/v1/skills/{slug}/report`

报告技能以供审核员审查。报告是技能级别，可选链接到某个版本，并进入技能报告队列。
...
`note` 对 `confirmed` 和 `dismissed` 是必需的；将 `status` 设回 `open` 时可以省略。对已分诊报告传入 `finalAction: "hide"`，可在同一可审计工作流中隐藏该技能。
...
- 只有当扫描器生成了确定性判定（`clean`、`suspicious` 或 `malicious`）时，`security.hasScanResult` 才为 `true`。
...
- 扫描作业是异步的。手动扫描请求的优先级高于常规发布/回填工作，但完成时间仍取决于 worker 可用性。
...
- 只有当选定版本拥有已生成的技能卡、未被审核标记为恶意软件阻止，并且 ClawScan 验证为干净时，`ok` 才为 `true`。
- 技能身份、发布者身份和选定版本元数据是顶层封套字段（`slug`、`displayName`、`publisherHandle`、`version`、`resolvedFrom`、`tag`、`createdAt`），因此 shell 自动化无需解包嵌套包装器即可读取它们。
...
- `security.signals.dependencyRegistry` 为了 v1 响应兼容性而保留，但依赖注册表存在性扫描器已停用，且此键始终为 `null`。
...
返回精确技能版本的当前紧凑安全判定。此集合端点面向已知道需要显示哪些已安装
...
- 响应仅包含安全信息。它不包含技能卡数据、生成的卡状态、制品文件列表或详细扫描器载荷。
- `security.signals` 仅包含状态级支持证据；如需完整扫描器详细信息，请使用 `/scan` 或 ClawHub 安全审计页面。
- `security.signals.dependencyRegistry` 为了 v1 响应兼容性而保留，但依赖注册表存在性扫描器已停用，且此键始终为 `null`。
- 缺少技能卡不会影响此端点的 `ok`、`decision` 或 `reasons`；客户端需要卡内容时，应在本地读取已安装的 `skill-card.md`。
- 需要单个技能的技能卡验证封套时使用 `/verify`，需要生成的卡 Markdown 时使用 `/card`，需要详细扫描器数据时使用 `/scan`。
...
- `category`（可选）：插件类别筛选器。仅当请求限定为插件包（`/api/v1/plugins`、
...
- `channel`（可选）：`official`、`community` 或 `private`
- `isOfficial`（可选）：`true` 或 `false`
- `category`（可选）：插件类别筛选器。仅当请求限定为插件包时支持。受控类别和旧版 v1
...
仅面向插件的搜索，覆盖 code-plugin 和 bundle-plugin 包。
...
返回安装客户端使用的精确包发布安全和信任摘要。这是公开的 OpenClaw 消费接口，用于决定解析出的发布版本是否可以安装。
...
- `trust.stale` 表示信任摘要是根据过期输入计算的，在做出高置信度允许决策前，应将其视为需要刷新。

说明：

- 此端点精确到版本。客户端应在解析出打算安装的包版本后调用它，而不仅是在读取最新包元数据后调用。
- 私有包会返回 `404`，除非调用方可以读取所属发布者。
- 此端点故意比所有者/版主审核端点更窄。它公开安装决策和公开解释，而不公开报告者身份、报告正文、私有证据或内部审核时间线。
...
报告一个包以供版主审核。报告是包级别的，可选择关联到某个版本。它们会进入审核队列，但本身不会自动隐藏或阻止下载；版主应使用发布审核来批准、隔离或撤销工件。
...
`note` 对 `confirmed` 和 `dismissed` 是必需的；将 `status` 设回 `open` 时可以省略。对已确认的报告传入 `finalAction: "quarantine"` 或 `finalAction: "revoke"`，即可在同一个可审计工作流中应用发布版本审核。
...
已隔离和已撤销的发布版本会从制品下载路由返回 `403`。每次变更都会写入审计日志条目。
...
- 二进制文件返回 `415`。
- 文件大小限制：200KB。
- 等待中的 VirusTotal 扫描不会阻止读取；恶意发布版本仍可能在其他地方被扣留。
...
- 仅注册表元数据不会注入到下载的归档中。
- 等待中的 VirusTotal 扫描不会阻止下载；恶意发布版本返回 `403`。
- 私有包返回 `404`，除非调用方是所有者。
...
下载托管的技能版本 ZIP，或者对于当前由 GitHub 支持且扫描结果为 `clean` 或 `suspicious`、没有托管版本的技能，返回 GitHub 源码交接。
...
- 可选载荷字段：`migrateOwner`。当与 `ownerHandle` 一起为 `true` 时，如果操作者同时是当前和目标发布者的管理员/所有者，现有技能可以移动到该所有者。没有此选择加入时，所有者变更会被拒绝。
...
由所有者发起的软删除会保留 slug 30 天，之后该 slug 可以由其他发布者声明。
...
仅限管理员。为合法所有者保留根 slug 和包名称，而不发布发布版本。包名称会变成没有发布版本行的私有占位包，因此同一所有者稍后可以将真正的 code-plugin 或 bundle-plugin 发布版本发布到该名称下。
...
仅限管理员。为已验证的替换 GitHub OAuth 主体恢复个人发布者，而不编辑 Convex Auth 账号行。请求必须指定两个不可变的 GitHub provider 账号 ID；可变 handle 仅用作面向操作者的防护。

端点默认使用试运行。应用恢复需要在工作人员独立验证两个 GitHub 主体之间的连续性后设置 `dryRun: false` 和
`confirmIdentityVerified: true`。
当目标用户当前的个人发布者拥有 Skills、包或 GitHub skill 来源时，恢复会安全失败。
恢复还会迁移已恢复发布者的 Skills、skill slug 别名、包、包检查器警告和派生搜索摘要行中的旧版 `ownerUserId` 字段，使直接所有者路径与新的发布者权限保持一致。已恢复 handle 的活动受保护 handle 预留也会重新分配给替换用户，避免后续个人资料同步恢复前用户的竞争权限。每个主表在每次应用事务中限制为
100 行；更大的恢复必须先使用可恢复的所有者迁移。
GitHub skill 来源按发布者作用域处理，并报告为已检查，而不是被重写。
...
- 两个端点都需要 API 令牌认证，并且仅适用于 skill 所有者。
...
更改现有封禁的已存储原因，不解除封禁或恢复内容（仅限管理员）。除非 `dryRun` 为 `false`，否则默认为试运行。
