...
- 当服务器知道原因时，已认证写入端点不应返回空泛的 `Unauthorized`。缺失令牌、无效/已撤销令牌，以及已删除/封禁/禁用的账户，都应获得可操作文本，这样 CLI 客户端就能告诉用户是什么阻止了他们。
...
- 如果没有可用的可信客户端 IP，匿名下载请求会使用端点范围的回退桶，而不是一个全局 `ip:unknown` 桶。匿名读取/写入请求仍使用共享未知桶，因此缺失 IP 的路由仍保持可见且保守。
...
- `q`（必需）：查询字符串
- `limit`（可选）：整数
- `highlightedOnly`（可选）：设为 `true` 可过滤为高亮技能
- `nonSuspiciousOnly`（可选）：设为 `true` 可隐藏可疑（`flagged.suspicious`）技能
...
- 结果按相关性顺序返回（嵌入相似度 + 精确 slug/名称词元提升 + 来自下载量的人气先验）。
- 相关性强于人气。精确的 slug 或显示名称词元匹配可能排在下载量多得多但匹配更宽泛的结果之前。
...
- 下载量会作为较小的对数缩放先验和并列决胜因素使用，而不是主要排名信号。当查询文本匹配较弱时，高下载量技能也可能排名更低。
- 可疑或隐藏的审核状态可能会根据调用者过滤条件和当前审核状态，将技能从公共搜索中移除。

发布者可发现性指南：

- 将用户会实际搜索的词放入显示名称、摘要和标签中。仅当独立 slug 词元也是你想保留的稳定身份时，才使用它。
- 不要只是为了追逐一个查询而重命名 slug，除非新 slug 是更好的长期规范名称。旧 slug 会成为重定向别名，但规范 URL、显示的 slug 和未来搜索摘要都会使用新 slug。
- 重命名别名会为旧 URL 以及通过注册表解析的安装保留解析能力，但搜索排名基于重命名完成索引后的规范技能元数据。现有统计数据会留在该技能上。
...
- `trending` 按过去 7 天内的安装量排名（基于遥测）。
- `createdAt` 对新技能爬取是稳定的；当现有技能重新发布时，`updated` 会变化。
- 当 `nonSuspiciousOnly=true` 时，基于游标的排序可能在某一页返回少于 `limit` 的条目，因为可疑技能是在页面检索后过滤的。
...
- 公共调用者仅能对已标记且可见的技能获得 `200`。
- 面向公共调用者的证据会被删改，并且只有所有者/审核员才能看到原始片段。
...
`confirmed` 和 `dismissed` 需要 `note`；将 `status` 设回 `open` 时可以省略。对已分诊报告传入 `finalAction: "hide"`，可在同一可审计工作流中隐藏该技能。
...
- 包含规范化验证状态以及扫描器特定详情。
- `security.capabilityTags` 会在检测到时包含确定性的能力/风险标签，例如 `crypto`、`requires-wallet`、`can-make-purchases`、`can-sign-transactions`、`requires-oauth-token` 和 `posts-externally`。
...
- `externalService`、`binary`、`osPermission`（可选）：命名环境需求标签的简写
- `artifactKind`（可选）：`legacy-zip` 或 `npm-pack`
- `npmMirror`（可选）：`true`/`1`，用于显示可通过 npm 镜像获取的 ClawPack 支持的包版本
...
- `target` / `hostTarget`、`os`、`arch`、`libc`、`requiresBrowser`、
  ...
  `osPermission` 可作为常用能力标签的简写
- `artifactKind`（可选）：`legacy-zip` 或 `npm-pack`
- `npmMirror`（可选）：`true`/`1`，用于搜索可通过 npm 镜像获取的 ClawPack 支持的包版本
...
- 对于旧式包归档，`version.artifact.kind` 为 `legacy-zip`；对于 ClawPack 支持的发布版本，则为
  `npm-pack`。
...
举报一个包以供版主审核。举报是包级别的，可选择链接到某个版本。它们会进入审核队列，但本身不会自动隐藏或阻止下载；版主应使用发布版本审核来批准、隔离或撤销工件。
...
`note` 是 `confirmed` 和 `dismissed` 的必填项；在将
`status` 设置回 `open` 时可以省略。传递 `finalAction: "quarantine"` 或
...
仅管理员可用的维护端点，用于为较旧的包发布标记显式制品类型元数据。
...
- 首选：使用 `payload` JSON + `files[]` blob 的 `multipart/form-data`。
- 也接受带 `files`（基于 storageId）的 JSON 正文。
- 可选 payload 字段：`ownerHandle`。存在时，API 会在服务器端解析该发布者，并要求执行者拥有发布者访问权限。
- 可选 payload 字段：`migrateOwner`。当它与 `ownerHandle` 一起为 `true` 时，如果执行者同时是当前发布者和目标发布者的管理员/所有者，现有技能可以移动到该所有者。如果未显式选择此项，则会拒绝所有者变更。
...
- 插件包需要 `openclaw.plugin.json`。ClawPack `.tgz` 上传必须在
  `package/openclaw.plugin.json` 包含它。
- 代码插件需要 `package.json`、源仓库元数据、源提交元数据、配置 schema 元数据、`openclaw.compat.pluginApi` 和
...
其他发布者认领。删除响应在适用此到期时包含 `slugReservedUntil`。
...
仅管理员可用。为合法所有者保留根 slug 和包名，而不发布版本。包名会成为没有发布行的私有占位包，因此同一所有者稍后可以将真实的 code-plugin 或 bundle-plugin 发布到该名称。
