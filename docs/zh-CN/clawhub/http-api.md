...
x-i18n:
    generated_at: "2026-07-06T21:46:20Z"
    model: gpt-5.5
...
第三方目录可以使用公共读取端点来列出或搜索 ClawHub 技能。请缓存结果，遵守 `429`/`Retry-After`，将用户链接回规范的 ClawHub 条目（`https://clawhub.ai/<owner>/skills/<slug>`），并避免暗示 ClawHub 为该第三方站点背书。不要尝试在公共 API 表面之外镜像隐藏、私有或被审核阻止的内容。
...
- 已认证写入端点在服务器知道原因时，不应返回裸 `Unauthorized`。缺失 token、无效/已撤销 token，以及已删除/封禁/禁用账户，都应获得可操作文本，以便 CLI 客户端能告知用户是什么阻止了他们。
...
- 如果没有可用的可信客户端 IP，匿名请求使用仅按速率限制类型限定范围的回退桶。这些回退桶不包括调用方提供的路径、slug、包名、版本、查询字符串或其他工件参数。

## 错误响应

公共 v1 错误响应是纯文本，带 `content-type: text/plain; charset=utf-8`。
这包括验证失败（`400`）、缺失公共资源（`404`）、认证和权限失败（`401`/`403`）、速率限制（`429`）以及被阻止的下载。客户端应将响应正文读取为人类可读字符串。未知查询参数会因兼容性被忽略，但已识别查询参数带无效值时会返回 `400`。
...
- 相关性强于流行度。精确 slug 或显示名称 token 匹配可以排在互动更强但匹配更宽松的结果之前。
...
- 流行度经过对数缩放并设有上限。当查询文本匹配较弱时，高互动技能也可能排名更低。
- 可疑或隐藏的审核状态可能会根据调用方筛选条件和当前审核状态，将技能从公共搜索中移除。

发布者可发现性指导：

- 将用户会实际搜索的词放在显示名称、摘要和标签中。仅当独立 slug token 也是你想保留的稳定身份时，才使用它。
- 不要仅为追逐某个查询而重命名 slug，除非新 slug 是更好的长期规范名称。旧 slug 会成为重定向别名，但规范 URL、显示的 slug 和未来搜索摘要会使用新 slug。
- 重命名别名会为旧 URL 和通过注册表解析的安装保留解析能力，但搜索排名基于重命名被索引后的规范技能元数据。现有统计会保留在该技能上。
- 如果某个技能意外不可见，请在登录状态下先用 `clawhub inspect @owner/slug` 检查审核状态，再更改排名相关元数据。
...
- `createdAt` 对新技能抓取稳定；`updated` 会在现有技能重新发布时变化。
- 当 `nonSuspiciousOnly=true` 时，基于游标的排序可能在某一页返回少于 `limit` 个条目，因为可疑技能会在页面检索后被过滤。
...
- 面向公共调用方时，证据会被编辑，并且仅对所有者/审核员包含原始片段。

### `POST /api/v1/skills/{slug}/report`

报告技能以供审核员审查。报告是技能级别的，可选择关联到某个版本，并会进入技能报告队列。
...
`note` 对 `confirmed` 和 `dismissed` 是必需的；将 `status` 设置回 `open` 时可以省略。对已分流处理的报告传入 `finalAction: "hide"`，可在同一个可审计工作流中隐藏该技能。
...
- 只有当所选版本具有已生成的技能卡、未被审核判定为恶意软件阻止，并且 ClawScan 验证为干净时，`ok` 才为 `true`。
...
返回精确技能版本的当前精简安全判定。此集合端点面向已经知道需要显示哪些已安装 ClawHub 技能版本的客户端，例如 OpenClaw Control UI。
...
- `security.signals` 仅包含状态级别的支持性证据；如需完整扫描器详细信息，请使用 `/scan` 或 ClawHub 安全审计页面。
- `security.signals.dependencyRegistry` 为了 v1 响应兼容性而保留，但依赖注册表存在性扫描器已退役，此键始终为 `null`。
- 技能卡缺失不会影响此端点的 `ok`、`decision` 或 `reasons`；当客户端需要卡片内容时，应在本地读取已安装的 `skill-card.md`。
- 需要单技能技能卡验证信封时使用 `/verify`，需要已生成的卡片 Markdown 时使用 `/card`，需要详细扫描器数据时使用 `/scan`。
...
- 匿名调用者只能看到公开包渠道。
- 经过身份验证的调用者可以在列表/搜索结果中看到其所属发布者的私有包。
- `channel=private` 只返回经过身份验证的调用者可读取的包。
...
- 托管技能包含最新存储版本文件，并在 `_manifest.json` 中列出，带有 `sourceRef: "public-clawhub"`。
- 当前由 GitHub 支撑且扫描结果为 `clean` 或 `suspicious` 的技能包含 `_source_handoff.json`，其中带有 `sourceRef: "public-github"`、仓库、提交、路径、内容哈希和归档 URL。它们不包含 ClawHub 托管的源文件。
...
- 对于旧世界包归档，`version.artifact.kind` 为 `legacy-zip`；对于 ClawPack 支持的版本，值为 `npm-pack`。
- ClawPack 版本包含 npm 兼容的 `npmIntegrity`、`npmShasum` 和 `npmTarballName` 字段。
- `version.sha256hash` 是旧客户端的已弃用兼容性元数据。它对 `/api/v1/packages/{name}/download` 返回的确切 ZIP 字节进行哈希。现代客户端应使用 `version.artifact.sha256`，它标识规范版本工件。
- 当扫描数据存在时，会包含 `version.vtAnalysis`、`version.llmAnalysis` 和 `version.staticScan`。
- 私有包返回 `404`，除非调用方可以读取所属发布者。

### `GET /api/v1/packages/{name}/versions/{version}/security`

返回安装客户端所需的确切包版本安全和信任摘要。这是 OpenClaw 用于判断已解析版本是否可安装的公开消费表面。
...
- `release.releaseId`、`release.version` 和 `release.createdAt` 标识被评估的确切版本。
...
- 私有包返回 `404`，除非调用方可以读取所属发布者。
- 此端点有意比所有者/审核员审核端点更窄。它公开安装决策和公开解释，而不公开报告者身份、报告正文、私有证据或内部审核时间线。
...
报告一个包以供审核员审查。报告是包级别的，可选择链接到某个版本。它们会进入审核队列，但本身不会自动隐藏或阻止下载；审核员应使用版本审核来批准、隔离或撤销工件。
...
`confirmed` 和 `dismissed` 需要 `note`；将 `status` 设回 `open` 时可以省略它。对已确认的举报传入 `finalAction: "quarantine"` 或 `finalAction: "revoke"`，可在同一个可审计工作流中应用发布版本审核。
...
- `revoked`：发布版本先前受信任，之后被阻止。

被隔离和撤销的发布版本会在工件下载路由返回 `403`。每次变更都会写入一条审计日志条目。
...
- 插件/软件包归档是带有 `package/` 根目录的 zip 文件，因此旧 OpenClaw 客户端可以继续工作。
...
下载托管的技能版本 ZIP，或者对于当前由 GitHub 支持、扫描结果为 `clean` 或 `suspicious` 且没有托管版本的技能，返回 GitHub 源码交接信息。
...
- 下载统计按 UTC 日期以唯一身份计数（API token 有效时为 `userId`，否则为 IP）。
...
- 可选载荷字段：`ownerHandle`。存在时，API 会在服务端解析该发布者，并要求行为者拥有发布者访问权限。
- 可选载荷字段：`migrateOwner`。当它与 `ownerHandle` 一起为 `true` 时，如果行为者同时是当前发布者和目标发布者的管理员/所有者，现有技能可以移动到该所有者。没有此显式选择时，所有者变更会被拒绝。
...
- 允许的表单字段是 `payload`、重复的 `files` blob，或一个 `clawpack` tarball 引用。`clawpack` 可以是 `.tgz` blob，也可以是由 upload-url 流程返回的 storage id。基于暂存 storage-id 的发布还必须包含该上传 URL 一同返回的 `clawpackUploadTicket`。
- 在同一请求中只能使用 `files` 或 `clawpack`，不能同时使用。
- JSON body 以及调用方提供的 `payload.files` / `payload.artifact` 元数据会被拒绝。
- 直接 multipart 发布请求上限为 18MB。ClawPack tarball 可以使用 upload-url 流程，最高到 120MB tarball 上限。
...
所有者发起的软删除会保留该 slug 30 天，之后该 slug 可以由另一个发布者认领。删除响应在适用此过期时间时包含 `slugReservedUntil`。
...
仅限管理员。确保某个 handle 存在组织发布者。如果该 handle 仍指向旧版共享用户/个人发布者，该端点会先将其迁移到组织发布者。对于新创建的组织，请提供 `memberHandle`；执行操作的管理员不会被添加为成员。`memberRole` 默认为 `owner`。
...
已认证的自助式组织发布者创建。创建新的组织发布者，并将调用方添加为所有者。此端点不会迁移现有用户/个人 handle，也不会将发布者标记为 trusted/official。
...
仅限管理员。为合法所有者保留根 slug 和软件包名称，而不发布发布版本。软件包名称会成为没有发布版本行的私有占位软件包，因此同一所有者之后可以将真实的 code-plugin 或 bundle-plugin 发布版本发布到该名称。
...
仅限管理员。为已验证的替代 GitHub OAuth principal 恢复个人发布者，而不编辑 Convex Auth 账户行。请求必须指定两个不可变的 GitHub 提供商账户 ID；可变 handle 仅用作面向操作员的保护。

端点默认使用试运行。应用恢复需要在工作人员独立验证两个 GitHub 主体之间的连续性后设置 `dryRun: false` 和
`confirmIdentityVerified: true`。当目标用户当前的个人发布者拥有技能、包或 GitHub 技能源时，恢复会失败并关闭。
恢复还会迁移已恢复发布者的技能、技能 slug 别名、包、包检查器警告和派生搜索摘要行中的旧版 `ownerUserId` 字段，使直接所有者路径与新的发布者权限保持一致。已恢复 handle 的有效受保护 handle 预留也会重新分配给替代用户，这样后续配置文件同步就无法恢复前用户的竞争权限。每个主表在每个应用事务中限制为
100 行；更大的恢复必须先使用可恢复的所有者迁移。
GitHub 技能源按发布者作用域管理，并报告为已检查，而不是重写。
...
封禁用户并硬删除其拥有的技能（仅限 moderator/admin）。
...
更改现有封禁的已存储原因，不解除封禁或恢复内容（仅限 admin）。除非 `dryRun` 为 `false`，否则默认为试运行。
