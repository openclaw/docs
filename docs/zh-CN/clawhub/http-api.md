...
旧版 `/api/...` 和 `/api/cli/...` 会保留以保持兼容性（请参阅 `DEPRECATIONS.md`）。
OpenAPI：`/api/v1/openapi.json`。

## 公共目录复用

第三方目录可以使用公共读取端点来列出或搜索 ClawHub Skills。请缓存结果，遵守 `429`/`Retry-After`，将用户链接回规范 ClawHub 列表（`https://clawhub.ai/<owner>/skills/<slug>`），并避免暗示 ClawHub 为第三方站点背书。不要尝试在公共 API 表面之外镜像隐藏、私有或因审核而被阻止的内容。
...
- 匿名请求：按 IP 强制执行。
- 已认证请求（有效 Bearer 令牌）：按用户桶强制执行。
- 如果令牌缺失或无效，行为会回退到 IP 强制执行。
- 当服务器知道原因时，已认证写入端点不应返回裸 `Unauthorized`。缺失令牌、无效/已撤销令牌，以及已删除/已封禁/已禁用账号，都应获得可操作文本，以便 CLI 客户端告知用户是什么阻止了他们。
...
- `X-RateLimit-Remaining` / `RateLimit-Remaining`：存在时表示确切剩余额度。
...
- 如果没有可用的受信任客户端 IP，匿名请求会使用仅按速率限制类别限定作用域的回退桶。这些回退桶不包含调用者提供的路径、slug、包名、版本、查询字符串或其他工件参数。

## 错误响应

公共 v1 错误响应是纯文本，`content-type: text/plain; charset=utf-8`。
这包括验证失败（`400`）、缺失公共资源（`404`）、认证和权限失败（`401`/`403`）、速率限制（`429`）以及被阻止的下载。客户端应将响应体读取为人类可读字符串。未知查询参数会因兼容性而被忽略，但带有无效值的已识别查询参数会返回 `400`。
...
- 结果按相关性顺序返回（嵌入相似度 + 精确 slug/名称 token 提升 + 少量流行度先验）。
...
- 流行度按对数缩放并封顶。当查询文本匹配较弱时，高参与度 Skills 的排名可能更低。
- 根据调用者过滤器和当前审核状态，可疑或隐藏的审核状态可能会从公共搜索中移除某个 Skill。

发布者可发现性指导：

- 将用户会逐字搜索的术语放入显示名称、摘要和标签中。仅当独立 slug token 也是你想保留的稳定身份时才使用它。
- 不要仅为追逐某个查询而重命名 slug，除非新 slug 是更好的长期规范名称。旧 slug 会成为重定向别名，但规范 URL、显示的 slug 和未来搜索摘要会使用新 slug。
- 重命名别名会为旧 URL 和通过注册表解析的安装保留解析能力，但搜索排名基于重命名索引完成后的规范 Skill 元数据。现有统计数据会保留在该 Skill 上。
...
- `trending` 按最近 7 天安装量排名（基于遥测）。
- `createdAt` 对新 Skill 抓取稳定；`updated` 会在现有 Skills 重新发布时变化。
- 当 `nonSuspiciousOnly=true` 时，基于游标的排序可能会在某一页返回少于 `limit` 的项目，因为可疑 Skills 是在页面检索后过滤的。
...
- `moderation` 仅在 Skill 被标记或所有者正在查看时包含。
...
- 公共调用者仅会对已标记且可见的 Skills 获得 `200`。
- 证据会对公共调用者脱敏，并且仅对所有者/审核员包含原始片段。

### `POST /api/v1/skills/{slug}/report`

举报某个 Skill 以供审核员审查。举报为 Skill 级别，可选链接到某个版本，并会进入 Skill 举报队列。
...
`confirmed` 和 `dismissed` 需要 `note`；将 `status` 设置回 `open` 时可以省略它。对已分流的举报传入 `finalAction: "hide"`，可在同一个可审计工作流中隐藏该 Skill。
...
- 扫描作业是异步的。手动扫描请求会优先于普通发布/回填工作处理，但完成时间仍取决于工作器可用性。
...
- 排队期间返回 `queue.queuedAhead` 和 `queue.position`，以便客户端显示该请求前面还有多少个优先级更高的手动扫描。非常大的队列会受到边界限制，并以 `queuedAheadIsEstimate: true` 报告。
...
- 只有当所选版本已生成 Skill Card、未被审核标记为恶意软件阻止，并且 ClawScan 验证为 clean 时，`ok` 才为 `true`。
...
- `security.signals.dependencyRegistry` 保留用于 v1 响应兼容性，但依赖注册表存在性扫描器已退役，此键始终为 `null`。
...
- 响应仅包含安全信息。它不包含 Skill Card 数据、生成的卡片状态、构件文件列表或详细扫描器载荷。
...
- `GET /api/v1/code-plugins` 和 `GET /api/v1/bundle-plugins` 仍然是固定系列别名。
...
- 当前由 GitHub 支持且扫描结果为 `clean` 或 `suspicious` 的技能包含 `_source_handoff.json`，其中带有 `sourceRef: "public-github"`、仓库、提交、路径、内容哈希和归档 URL。它们不包含 ClawHub 托管的源文件。
...
仅针对插件的搜索，覆盖 code-plugin 和 bundle-plugin 包。
...
- 对于旧体系的包归档，`version.artifact.kind` 为 `legacy-zip`；对于由 ClawPack 支持的发布版本，则为 `npm-pack`。
...
返回给安装客户端使用的精确包发布安全和信任摘要。这是用于判断已解析版本是否可安装的公开 OpenClaw 消费接口。
...
- `trust.stale` 表示信任摘要基于过时输入计算，应在做出高置信度允许决定前视为需要刷新。

说明：

- 此端点精确到版本。客户端应在解析出打算安装的包版本后调用它，而不是只在读取最新包元数据后调用。
- 私有包会返回 `404`，除非调用方可以读取所属发布者。
- 此端点有意比所有者/审核员审核端点更窄。它公开安装决定和公开说明，而不公开报告者身份、报告正文、私有证据或内部审查时间线。
...
- ClawPack 版本返回 `npm-pack` 工件、npm 完整性字段、`tarballUrl` 以及旧版 ZIP 兼容性 URL。
...
- 这仅跟踪迁移就绪度。它不会修改 OpenClaw 或生成 ClawPack。
...
报告包以供审核员审查。报告是包级别的，可选择关联到某个版本。它们会进入审核队列，但本身不会自动隐藏或阻止下载；审核员应使用发布审核来批准、隔离或撤销工件。
...
`note` 对 `confirmed` 和 `dismissed` 是必需的；将 `status` 重新设置为 `open` 时可以省略。对已确认的报告传入 `finalAction: "quarantine"` 或 `finalAction: "revoke"`，可在同一个可审计工作流中应用发布审核。
...
- `revoked`：在某个发布先前被信任后阻止。

隔离和吊销的发布会从产物下载路由返回 `403`。每次变更都会写入一条审计日志。
...
下载托管的技能版本 ZIP，或针对当前由 GitHub 支持、扫描为 `clean` 或 `suspicious` 且没有托管版本的技能返回 GitHub 源交接。
...
- 可选载荷字段：`migrateOwner`。当它与 `ownerHandle` 一起为 `true` 时，如果操作者同时是当前发布者和目标发布者的管理员/所有者，则现有技能可以移动到该所有者。没有此显式选择时，所有者变更会被拒绝。
...
- 允许的表单字段为 `payload`、重复的 `files` blob，或一个 `clawpack` tarball 引用。`clawpack` 可以是 `.tgz` blob，也可以是由 upload-url 流程返回的存储 id。分阶段 storage-id 发布还必须包含随该上传 URL 返回的 `clawpackUploadTicket`。
- 使用 `files` 或 `clawpack` 二选一，切勿在同一个请求中同时使用。
...
- 代发布仍会根据目标所有者账户验证 official 渠道资格。
...
所有者发起的软删除会保留该 slug 30 天，之后该 slug 可由其他发布者认领。当此过期适用时，删除响应会包含 `slugReservedUntil`。
...
仅管理员。确保某个 handle 存在组织发布者。如果该 handle 仍指向旧版共享用户/个人发布者，该端点会先将其迁移为组织发布者。对于新创建的组织，提供 `memberHandle`；执行操作的管理员不会被添加为成员。`memberRole` 默认值为 `owner`。
...
已认证的自助式组织发布者创建。创建一个新的组织发布者并将调用方添加为所有者。此端点不会迁移现有用户/个人 handle，也不会将发布者标记为 trusted/official。
...
仅管理员。为合法所有者保留根 slug 和包名，而不发布 release。包名会成为没有 release 行的私有占位包，因此同一所有者稍后可以将真实的 code-plugin 或 bundle-plugin release 发布到该名称下。
...
仅管理员。在不编辑 Convex Auth 账户行的情况下，为已验证的替换 GitHub OAuth 主体恢复个人发布者。请求必须命名两个不可变 GitHub provider account id；可变 handle 仅用作面向操作员的防护。

该端点默认执行 dry-run。应用恢复需要在工作人员独立验证两个 GitHub 主体之间的连续性后设置 `dryRun: false` 和
`confirmIdentityVerified: true`。
当目标用户当前的个人发布者已有技能、包或 GitHub 技能源时，恢复会失败并关闭。
恢复还会迁移已恢复发布者的技能、技能 slug 别名、包、包检查器警告以及派生搜索摘要行中的旧版 `ownerUserId` 字段，以便直接所有者路径与新的发布者权限保持一致。已恢复 handle 的活动受保护 handle 预留也会重新分配给替换用户，这样后续的个人资料同步就无法恢复原用户的竞争权限。每个主表在每次应用事务中限制为
100 行；更大的恢复必须先使用可恢复的所有者迁移。
GitHub 技能源按发布者划分，并报告为已检查，而不是被重写。
...
更改现有封禁中存储的原因，而不解封或恢复内容（仅限管理员）。除非 `dryRun` 为 `false`，否则默认执行 dry-run。
...
`POST /api/cli/upload-url` 返回 `uploadUrl` 和 `uploadTicket`。暂存 ClawPack tar 包的包发布必须将生成的存储 ID 作为
