...
第三方目录可以使用公共读取端点来列出或搜索 ClawHub 技能。请缓存结果、遵守 `429`/`Retry-After`、将用户链接回规范的 ClawHub 列表页（`https://clawhub.ai/<owner>/skills/<slug>`），并避免暗示 ClawHub 为第三方站点背书。不要尝试在公共 API 表面之外镜像隐藏、私有或因审核而被阻止的内容。
...
- 当服务器知道原因时，已认证写入端点不应返回裸 `Unauthorized`。缺失令牌、无效/已撤销令牌，以及已删除/被封禁/已禁用账号都应获得可操作文本，以便 CLI 客户端能告诉用户是什么阻止了他们。
...
- 如果没有可用的受信任客户端 IP，匿名请求会使用仅按速率限制种类划分的回退桶。这些回退桶不包括调用方提供的路径、slug、包名、版本、查询字符串或其他构件参数。

## 错误响应

公共 v1 错误响应是纯文本，带有 `content-type: text/plain; charset=utf-8`。
这包括验证失败（`400`）、缺失的公共资源（`404`）、认证和权限失败（`401`/`403`）、速率限制（`429`）以及被阻止的下载。客户端应将响应正文读取为人类可读字符串。未知查询参数会为兼容性而被忽略，但已识别查询参数的无效值会返回 `400`。
...
- 结果按相关性顺序返回（嵌入相似度 + 精确 slug/name 词元加权 + 小幅流行度先验）。
- 相关性强于流行度。精确的 slug 或显示名称词元匹配，可以排在参与度高得多但匹配更宽松的结果之前。
- ASCII 文本会按词和标点边界分词。例如，`personal-map` 包含独立的 `map` 词元，而 `amap-jsapi-skill` 包含 `amap`、`jsapi` 和 `skill`；因此搜索 `map` 时，`personal-map` 会比 `amap-jsapi-skill` 获得更强的词法匹配。
- 流行度会进行对数缩放并设置上限。当查询文本匹配较弱时，高参与度技能的排名可能更低。
- 根据调用方筛选器和当前审核状态，可疑或隐藏的审核状态可能会从公共搜索中移除某个技能。

发布者可发现性指导：

- 将用户会字面搜索的术语放入显示名称、摘要和标签中。仅当独立 slug 词元也是你想保留的稳定身份时才使用它。
...
- 当 `nonSuspiciousOnly=true` 时，基于游标的排序可能在一页中返回少于 `limit` 的条目，因为可疑技能是在页面检索后被过滤的。
- 存在 `nextCursor` 时用它继续分页。短页本身并不表示结果已结束。
...
- 面向公共调用方的证据会被脱敏，并且仅向所有者/审核员包含原始片段。
...
`note` 对 `confirmed` 和 `dismissed` 是必需的；将 `status` 设置回 `open` 时可以省略。传递 `finalAction: "hide"` 和已分流报告，以便在同一个可审计工作流中隐藏该技能。
...
- 返回排队/运行中/成功/失败状态。
- 排队时返回 `queue.queuedAhead` 和 `queue.position`，以便客户端显示该请求前面有多少个优先级更高的手动扫描。超大队列会被限制，并通过 `queuedAheadIsEstimate: true` 报告。
...
- 需要对技能或插件拥有拥有者/发布者管理访问权限，或平台审核员/管理员权限。
...
仅限管理员的规范批量重新扫描路由。它接受与旧版 `POST /api/v1/skills/-/rescan-batch` 相同的载荷形态。
...
- 只有当所选版本已生成 Skill Card、未被审核判定为恶意软件而阻止，并且 ClawScan 验证为干净时，`ok` 才为 `true`。
...
返回精确技能版本的当前紧凑安全结论。此集合端点适用于已经知道需要显示哪些已安装 ClawHub 技能版本的客户端，例如 OpenClaw Control UI。
...
- 技能条目仍由技能注册表支持，并且仍然只能通过 `POST /api/v1/skills` 发布。
...
- `version.artifact.kind` 对旧式包归档为 `legacy-zip`，对 ClawPack 支持的版本为 `npm-pack`。
- ClawPack 版本包含 npm 兼容的 `npmIntegrity`、`npmShasum` 和 `npmTarballName` 字段。
- `version.sha256hash` 是面向旧客户端的已弃用兼容性元数据。它会哈希 `/api/v1/packages/{name}/download` 返回的精确 ZIP 字节。现代客户端应使用 `version.artifact.sha256`，它标识规范发布制品。
- 当扫描数据存在时，会包含 `version.vtAnalysis`、`version.llmAnalysis` 和 `version.staticScan`。
- 私有包返回 `404`，除非调用方可以读取所属发布者。

### `GET /api/v1/packages/{name}/versions/{version}/security`

返回安装客户端所需的精确包版本安全与信任摘要。这是公共 OpenClaw 消费界面，用于判断已解析版本是否可以安装。
...
- 当发布制品已知时，会出现 `release.artifactKind`、`release.artifactSha256`、`release.npmIntegrity`、`release.npmShasum` 和 `release.npmTarballName`。
...
- `trust.stale` 表示信任摘要是基于过期输入计算的，在做出高置信度允许决策之前应将其视为需要刷新。

说明：

- 此端点精确到版本。客户端应在解析出要安装的包版本后调用它，而不只是读取最新包元数据后调用。
- 私有包返回 `404`，除非调用方可以读取所属发布者。
- 此端点有意比所有者/版主审核端点更窄。它公开安装决策和公开解释，而不公开举报者身份、举报正文、私有证据或内部审查时间线。
...
- ClawPack 版本返回 `npm-pack` 制品、npm integrity 字段、`tarballUrl` 和旧版 ZIP 兼容 URL。
...
- 这仅跟踪迁移就绪状态。它不会修改 OpenClaw，也不会生成 ClawPack。
...
- `open`：可疑、恶意、待处理、隔离、已撤销或已举报的版本。
- `blocked`：隔离、已撤销或恶意的版本。
...
举报一个包以供版主审查。举报是包级别的，可选择链接到某个版本。它们会进入审核队列，但本身不会自动隐藏或阻止下载；版主应使用版本审核来批准、隔离或撤销制品。
...
`confirmed` 和 `dismissed` 需要 `note`；将 `status` 设回 `open` 时可以省略它。传入 `finalAction: "quarantine"` 或
`finalAction: "revoke"` 并配合已确认报告，可在同一个可审计工作流中应用发布审核。
...
为 ClawPack 支持的软件包版本返回与 npm 兼容的 packument。

说明：

- 仅列出已上传 ClawPack npm-pack tarball 的版本。
- 旧版仅 ZIP 版本会被有意省略。
- `dist.tarball`、`dist.integrity` 和 `dist.shasum` 使用与 npm 兼容的
  字段，因此用户可以按需将 npm 指向该镜像。
- 带作用域软件包的 packument 同时支持 `/api/npm/@scope/name` 和 npm 的
  编码请求路径 `/api/npm/@scope%2Fname`。

### `GET /api/npm/{package}/-/{tarball}.tgz`

为 npm 镜像客户端流式传输准确的已上传 ClawPack tarball 字节。
...
下载托管 Skills 版本 ZIP，或在当前 GitHub 支持的 Skills 具有 `clean` 或 `suspicious` 扫描且没有托管版本时，返回 GitHub 源交接。
...
- 可选载荷字段：`ownerHandle`。存在时，API 会在服务器端解析该
  发布者，并要求操作者拥有发布者访问权限。
- 可选载荷字段：`migrateOwner`。当它为 `true` 且带有 `ownerHandle` 时，如果
  操作者同时是当前和目标发布者的管理员/所有者，现有 Skills 可以迁移到该所有者。
  没有此显式选择时，所有者变更会被拒绝。
...
- 允许的表单字段为 `payload`、重复的 `files` blob，或一个 `clawpack`
  tarball 引用。`clawpack` 可以是 `.tgz` blob，也可以是上传 URL 流程返回的 storage id。
  分阶段 storage-id 发布还必须包含该上传 URL 返回的
  `clawpackUploadTicket`。
- 使用 `files` 或 `clawpack`，同一个请求中不能同时使用两者。
- JSON 正文和调用方提供的 `payload.files` / `payload.artifact`
  元数据会被拒绝。
- 直接 multipart 发布请求上限为 18MB。ClawPack tarball 可以
  通过上传 URL 流程使用最高 120MB 的 tarball 上限。
...
存在时，`reason` 会存储为 Skills 审核说明，并复制到审计日志。
所有者发起的软删除会保留该 slug 30 天，之后该 slug 可由
另一个发布者认领。删除响应在适用此到期规则时包含 `slugReservedUntil`。
...
仅管理员。为合法所有者保留根 slug 和软件包名称，而不发布
发布版本。软件包名称会变为没有发布行的私有占位软件包，因此同一
所有者稍后可以将真正的 code-plugin 或 bundle-plugin 发布发布到该名称。
...
无需编辑 Convex Auth 账户行。请求必须同时指定两个不可变 GitHub
provider account id；可变句柄仅作为面向操作者的保护检查。

端点默认执行演练。应用恢复需要在工作人员独立验证两个 GitHub 主体之间的连续性后设置 `dryRun: false` 和
`confirmIdentityVerified: true`。当目标用户当前的个人发布者拥有技能、包或 GitHub 技能源时，恢复会失败并保持关闭。
恢复还会迁移已恢复发布者的技能、技能 slug 别名、包、包检查器警告以及派生搜索摘要行中的旧版 `ownerUserId` 字段，使直接所有者路径与新的发布者权限保持一致。已恢复 handle 的有效受保护 handle 预留也会重新分配给替换用户，确保后续个人资料同步无法恢复前用户的竞争性权限。每个主表在每次应用事务中限制为
100 行；更大的恢复必须先使用可恢复的所有者迁移。
GitHub 技能源按发布者划分作用域，并报告为已检查，而不是被重写。
...
- 两个端点都需要 API 令牌身份验证，并且仅对技能所有者生效。
...
更改现有封禁存储的原因，不解除封禁也不恢复
内容（仅限管理员）。除非 `dryRun` 为 `false`，否则默认执行演练。
