...
x-i18n:
    generated_at: "2026-07-02T07:54:49Z"
    model: gpt-5.5
...
第三方目录可以使用公共读取端点来列出或搜索 ClawHub skills。请缓存结果，遵守 `429`/`Retry-After`，将用户链接回规范 ClawHub 列表（`https://clawhub.ai/<owner>/skills/<slug>`），并避免暗示 ClawHub 认可该第三方站点。不要尝试在公共 API 表面之外镜像隐藏、私有或被审核屏蔽的内容。
...
- 当服务器知道原因时，已认证写入端点不应返回裸 `Unauthorized`。缺失 token、无效/已吊销 token，以及已删除/已封禁/已禁用账号都应获得可操作文本，以便 CLI 客户端能告诉用户是什么阻止了它们。
...
- 如果没有可用的受信任客户端 IP，匿名请求会使用仅按速率限制类型划分的回退桶。这些回退桶不包括调用方提供的路径、slug、包名、版本、查询字符串或其他制品参数。

## 错误响应

公共 v1 错误响应是纯文本，带有 `content-type: text/plain; charset=utf-8`。
这包括验证失败（`400`）、缺失公共资源（`404`）、认证和权限失败（`401`/`403`）、速率限制（`429`）以及被阻止的下载。客户端应将响应正文读取为人类可读字符串。为保持兼容性，未知查询参数会被忽略，但已识别查询参数的无效值会返回 `400`。
...
- `q`（必需）：查询字符串
- `limit`（可选）：整数
- `highlightedOnly`（可选）：设为 `true` 以筛选为高亮 skills
...
- 相关性强于流行度。精确的 slug 或显示名称 token 匹配可以排在互动度强得多但匹配更宽泛的结果前面。
...
- 流行度采用对数缩放并设有上限。当查询文本匹配较弱时，高互动 skills 的排名可能更低。
...
- 重命名别名会保留旧 URL 和通过注册表解析的安装的解析能力，但重命名完成索引后，搜索排名基于规范 skill 元数据。现有统计数据会保留在该 skill 上。
...
- 当 `nonSuspiciousOnly=true` 时，基于游标的排序可能会在某一页返回少于 `limit` 的条目，因为可疑 skills 会在页面检索后被筛除。
...
- 对公共调用方，证据会被脱敏；只有所有者/审核员会看到原始片段。

### `POST /api/v1/skills/{slug}/report`

举报某个 skill 以供审核员审查。举报以 skill 为级别，可选链接到某个版本，并进入 skill 举报队列。
...
`confirmed` 和 `dismissed` 需要 `note`；将 `status` 设回 `open` 时可以省略。传递 `finalAction: "hide"` 以及已分诊的举报，可在同一个可审计工作流中隐藏该 skill。
...
- 响应为 `202`，并返回 `{ "ok": true, "scanId": "...", "jobId": "...", "status": "queued", "sourceKind": "published", "update": false, "queue": { "queuedAhead": 0, "queuedAheadIsEstimate": false, "position": 1, "running": 0, "runningIsEstimate": false, "note": "Scans are asynchronous and may take time to complete." } }`。
...
- 只有当所选版本已生成 Skill Card、未被审核标记为恶意软件阻止，且 ClawScan 验证为 clean 时，`ok` 才为 `true`。
...
- `security.signals.dependencyRegistry` 为 v1 响应兼容性而保留，但依赖注册表存在性扫描器已停用，此键始终为 `null`。
...
- 对于旧体系包归档，`version.artifact.kind` 为 `legacy-zip`；
...
返回安装客户端所需的精确包发布安全和信任摘要。这是公共 OpenClaw 消费面，用于判断
已解析的发布是否可以安装。

认证：

- 公共读取端点。不需要所有者、发布者、审核员或管理员令牌。
...
- `release.releaseId`、`release.version` 和 `release.createdAt` 标识
  已评估的精确发布。
- 当发布工件的信息已知时，会出现 `release.artifactKind`、`release.artifactSha256`、`release.npmIntegrity`、
  `release.npmShasum` 和 `release.npmTarballName`。
- `trust.scanStatus` 是从扫描器输入和手动发布审核
  派生出的有效信任状态。
...
- `trust.stale` 表示信任摘要是根据过时输入计算的，
  在做出高置信度允许决策之前，应将其视为需要刷新。

说明：

- 此端点精确到版本。客户端应在解析出
  打算安装的包版本后调用它，而不是只在读取最新
  包元数据后调用。
- 私有包返回 `404`，除非调用方可以读取所属发布者。
- 此端点有意比所有者/审核员审核
  端点更窄。它暴露安装决策和公开解释，而不是
...
- `revoked`：此前受信任的发布版本已被阻止。

被隔离和撤销的发布版本会从制品下载路由返回 `403`。每次变更都会写入一条审计日志条目。
...
下载托管的 skill 版本 ZIP，或者对于当前由 GitHub 支持、扫描结果为 `clean` 或 `suspicious` 且没有托管版本的 skill，返回 GitHub 源交接信息。
...
- 如果既未提供 `version` 也未提供 `tag`，则使用最新版本。
- 软删除版本返回 `410`。
- GitHub 支持的 skill 交接不会代理或镜像字节。JSON 响应包含 `sourceRef: "public-github"`、`repo`、`commit`、`path`、`contentHash` 和 `archiveUrl`；扫描/当前状态是门禁，不作为成功载荷元数据包含。
...
- 可选载荷字段：`migrateOwner`。当它与 `ownerHandle` 一起为 `true` 时，如果操作者同时是当前发布者和目标发布者的管理员/所有者，现有 skill 可以移动到该所有者。没有此显式选择时，所有者变更会被拒绝。
...
- 使用 `files` 或 `clawpack` 二者之一，绝不要在同一请求中同时使用。
- JSON 正文和调用者提供的 `payload.files` / `payload.artifact` 元数据会被拒绝。
- 直接 multipart 发布请求上限为 18MB。ClawPack tarball 可以通过 upload-url 流程使用，最高可达 120MB tarball 上限。
...
删除响应会在适用此过期时间时包含 `slugReservedUntil`。
...
已认证的自助式组织发布者创建。创建新的组织发布者，并将调用者添加为所有者。此端点不会迁移现有用户/个人 handle，也不会将发布者标记为受信任/官方。
...
仅限管理员。为合法所有者保留根 slug 和包名称，而不发布发布版本。包名称会成为没有发布版本行的私有占位包，因此同一所有者以后可以将真实的 code-plugin 或 bundle-plugin 发布版本发布到该名称下。
...
仅限管理员。为已验证的替代 GitHub OAuth 主体恢复个人发布者，而不编辑 Convex Auth 账户行。请求必须指定两个不可变的 GitHub provider 账户 id；可变 handle 仅用作面向操作者的保护。

该端点默认使用试运行。应用恢复需要在工作人员独立验证两个 GitHub 主体之间的连续性后设置 `dryRun: false` 和
`confirmIdentityVerified: true`。
当目标用户当前的个人发布者拥有 Skills、软件包或 GitHub 技能来源时，恢复会失败并关闭。
恢复还会迁移被恢复发布者的 Skills、技能 slug 别名、软件包、软件包检查器警告以及派生搜索摘要行中的旧版 `ownerUserId` 字段，以便直接所有者路径与新的发布者权限保持一致。已恢复 handle 的活动受保护 handle 预留也会重新分配给替代用户，因此后续个人资料同步无法恢复原用户的竞争权限。每个主表在每个应用事务中限制为
...
## 注册表设备发现（`/.well-known/clawhub.json`）
