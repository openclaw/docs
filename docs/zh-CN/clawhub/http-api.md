...
x-i18n:
    generated_at: "2026-07-05T04:42:53Z"
    model: gpt-5.5
...
第三方目录可以使用公共读取端点来列出或搜索 ClawHub Skills。请缓存结果，遵守 `429`/`Retry-After`，将用户链接回规范的 ClawHub 列表页（`https://clawhub.ai/<owner>/skills/<slug>`），并避免暗示 ClawHub 认可该第三方站点。不要尝试在公共 API 表面之外镜像隐藏、私有或被审核阻止的内容。
...
- 如果令牌缺失/无效，行为会回退到 IP 强制执行。
- 当服务器知道原因时，已认证写入端点不应返回裸的 `Unauthorized`。令牌缺失、令牌无效/已吊销，以及账户已删除/被封禁/已禁用，都应获得可操作的文本，以便 CLI 客户端能告诉用户是什么阻止了他们。
...
- 如果没有可用的受信任客户端 IP，匿名请求会使用仅按速率限制类型划分作用域的回退桶。这些回退桶不包括调用方提供的路径、slug、包名称、版本、查询字符串或其他制品参数。

## 错误响应

公共 v1 错误响应是纯文本，`content-type: text/plain; charset=utf-8`。
这包括验证失败（`400`）、缺失的公共资源（`404`）、认证和权限失败（`401`/`403`）、速率限制（`429`）以及被阻止的下载。客户端应将响应正文读取为人类可读的字符串。未知查询参数会因兼容性而被忽略，但已识别查询参数的无效值会返回 `400`。
...
- 相关性强于流行度。精确的 slug 或显示名称 token 匹配可以排在互动更强但匹配更宽泛的结果前面。
...
- 流行度经过对数缩放并设有上限。当查询文本匹配较弱时，高互动 Skills 可能排名更低。
- 可疑或隐藏的审核状态可能会根据调用方筛选器和当前审核状态，将某个 Skill 从公共搜索中移除。

发布者可发现性指南：

- 将用户会逐字搜索的术语放入显示名称、摘要和标签中。仅当独立的 slug token 也是你想保留的稳定身份时才使用它。
- 不要仅为追逐一个查询而重命名 slug，除非新 slug 是更好的长期规范名称。旧 slug 会成为重定向别名，但规范 URL、显示的 slug 和未来搜索摘要会使用新 slug。
- 重命名别名会为旧 URL 和通过注册表解析的安装保留解析能力，但搜索排名会基于重命名完成索引后的规范 Skill 元数据。现有统计信息会保留在该 Skill 上。
...
- `createdAt` 对新 Skill 爬取是稳定的；当现有 Skills 重新发布时，`updated` 会变化。
- 当 `nonSuspiciousOnly=true` 时，基于游标的排序可能在一页上返回少于 `limit` 个项目，因为可疑 Skills 会在页面检索后被过滤。
- 存在 `nextCursor` 时使用它继续分页。短页本身并不意味着结果结束。
...
- 仅当 Skill 被标记或所有者正在查看它时，才包含 `moderation`。
...
- 面向公共调用方时，证据会被删改，并且仅对所有者/审核员包含原始片段。

### `POST /api/v1/skills/{slug}/report`

举报某个 Skill 以供审核员审查。举报是 Skill 级别的，可选关联到某个版本，并会进入 Skill 举报队列。
...
`note` 对 `confirmed` 和 `dismissed` 是必需的；将 `status` 设回 `open` 时可以省略。对已分诊举报传入 `finalAction: "hide"`，可在同一个可审计工作流中隐藏该 Skill。
...
- 返回已排队/运行中/成功/失败状态。
- 排队期间返回 `queue.queuedAhead` 和 `queue.position`，以便客户端显示该请求前面有多少个优先级手动扫描。超大队列会被限制，并以 `queuedAheadIsEstimate: true` 报告。
...
- 需要对该技能或插件拥有拥有者/发布者管理访问权限，或拥有平台审核员/管理员权限。
...
返回 `clawhub skill verify` 使用的技能卡片验证封装。
...
- 只有当所选版本拥有生成的技能卡片、未被审核标记为恶意软件阻止，并且 ClawScan 验证为干净时，`ok` 才为 `true`。
- 技能身份、发布者身份和所选版本元数据是顶层封装字段（`slug`、`displayName`、`publisherHandle`、`version`、`resolvedFrom`、`tag`、`createdAt`），因此 shell 自动化无需解包嵌套包装即可读取它们。
...
- `security.signals.dependencyRegistry` 为了 v1 响应兼容性而保留，但依赖注册表存在性扫描器已退役，因此此键始终为 `null`。
...
- `security.signals.dependencyRegistry` 为了 v1 响应兼容性而保留，但依赖注册表存在性扫描器已退役，因此此键始终为 `null`。
- 技能卡片缺失不会影响此端点的 `ok`、`decision` 或 `reasons`；客户端在需要卡片内容时，应在本地读取已安装的 `skill-card.md`。
- 当你需要单个技能的技能卡片验证封装时使用 `/verify`，需要生成的卡片 Markdown 时使用 `/card`，需要详细扫描器数据时使用 `/scan`。
...
- `category`（可选）：插件类别过滤器。仅在请求限定为插件包（`/api/v1/plugins`、`/api/v1/code-plugins`、`/api/v1/bundle-plugins`，或带有 `family=code-plugin`/`family=bundle-plugin` 的包端点）时支持。受控类别和旧版 v1 过滤器别名记录在 `GET /api/v1/plugins` 下。
...
- `channel=private` 只返回已认证调用者可以读取的包。
...
- `channel`（可选）：`official`、`community` 或 `private`
- `isOfficial`（可选）：`true` 或 `false`
- `category`（可选）：插件类别过滤器。仅在请求限定为插件包时支持。受控类别和旧版 v1 过滤器别名记录在 `GET /api/v1/plugins` 下。
...
- 当前由 GitHub 支持且扫描结果为 `clean` 或 `suspicious` 的 skills 包含 `_source_handoff.json`，其中含有 `sourceRef: "public-github"`、repo、commit、path、content hash 和 archive URL。它们不包含 ClawHub 托管的源文件。
...
- `GET /api/v1/plugins` 下记录的旧版 v1 过滤别名也会被
  接受。
...
返回用于安装
客户端的确切包发布安全性和信任摘要。这是公共 OpenClaw 消费表面，用于判断
已解析的发布版本是否可以安装。
...
- `trust.scanStatus` 是从扫描器输入
  和手动发布审核派生出的有效信任状态。
- `trust.moderationState` 可为 null。当不存在手动发布
  审核时，它为 `null`。
...
对已确认的报告传入 `finalAction: "quarantine"` 或
...
- 二进制文件返回 `415`。
- 文件大小限制：200KB。
- 待处理的 VirusTotal 扫描不会阻止读取；恶意发布仍可能在其他位置被 withheld。
...
- `dist.tarball`、`dist.integrity` 和 `dist.shasum` 使用 npm 兼容
  字段，因此用户可按需将 npm 指向该镜像。
- Scoped 包 packument 同时支持 `/api/npm/@scope/name` 和 npm 的
...
下载托管技能版本 ZIP，或为当前由 GitHub 支持、扫描状态为 `clean` 或 `suspicious` 且没有托管版本的技能返回 GitHub 源码交接。
...
- 由 GitHub 支持的技能交接不会代理或镜像字节。JSON 响应
  ...
  和 `archiveUrl`；扫描/当前状态是一个门禁，不会作为成功
  payload 元数据包含。
- 下载统计按 UTC 日的唯一身份计数（API 令牌有效时使用 `userId`，否则使用 IP）。
...
- 可选 payload 字段：`ownerHandle`。存在时，API 会在服务器端解析该
  发布者，并要求执行者拥有发布者访问权限。
- 可选 payload 字段：`migrateOwner`。当它与 `ownerHandle` 一起为 `true` 时，如果执行者在当前和目标发布者两边都是 admin/owner，
  现有技能可以迁移到该所有者。没有此显式选择时，所有者变更会被
  拒绝。
...
- 允许的表单字段是 `payload`、重复的 `files` blob，或一个 `clawpack`
  tarball 引用。`clawpack` 可以是 `.tgz` blob，也可以是上传 URL 流程返回的 storage id。暂存 storage-id 发布还必须包含随该上传 URL 返回的
...
- 直接 multipart 发布请求上限为 18MB。ClawPack tarball 可
  使用上传 URL 流程，最高到 120MB tarball 上限。
...
- 插件包需要 `openclaw.plugin.json`。ClawPack `.tgz` 上传必须
  在 `package/openclaw.plugin.json` 中包含它。
- 代码插件需要 `package.json`、源码仓库元数据、源码 commit
  元数据、配置 schema 元数据、`openclaw.compat.pluginApi` 和
  `openclaw.build.openclawVersion`。
- `openclaw.hostTargets` 和 `openclaw.environment` 是可选元数据。
- 只有 `openclaw` 组织发布者以及当前 `openclaw` 组织成员的
  个人发布者可以发布到 `official` 渠道。
- 代表发布仍会根据目标所有者账号验证 official 渠道资格。
...
所有者发起的软删除会保留 slug 30 天，之后该 slug 可被
另一个发布者认领。当此到期时间适用时，删除响应会包含 `slugReservedUntil`。
...
旧版共享用户/个人发布者，端点会先将其迁移为组织发布者。
对于新创建的组织，提供 `memberHandle`；执行操作的 admin 不会被添加为成员。
...
仅限 admin。为合法所有者预留根 slug 和包名，而不发布
发布。包名会成为没有发布行的私有占位包，因此同一
所有者稍后可以将真正的 code-plugin 或 bundle-plugin 发布到该名称下。
...
仅限 admin。为已验证的替换 GitHub OAuth 主体恢复个人发布者，
无需编辑 Convex Auth 账号行。请求必须指定两个不可变 GitHub
提供商账号 ID；可变 handle 仅用作面向操作员的防护。

端点默认采用试运行。应用恢复需要在工作人员独立验证两个
GitHub 主体之间的连续性后设置 `dryRun: false` 和
`confirmIdentityVerified: true`。当目标用户当前的个人
发布者拥有技能、包或 GitHub 技能来源时，恢复会失败并保持关闭。
恢复还会为已恢复发布者的技能、技能 slug 别名、包、包检查器警告以及派生搜索摘要行迁移旧版 `ownerUserId` 字段，使
直接所有者路径与新的发布者权限一致。已恢复 handle 的活动受保护 handle
保留项也会重新分配给替代用户，这样之后的个人资料同步无法恢复前用户的竞争权限。每个主表在每个应用事务中限制为
100 行；更大的恢复必须先使用可恢复的所有者迁移。
GitHub 技能来源按发布者设定作用域，并报告为已检查，而不是重写。
...
更改现有封禁已存储的原因，但不解除封禁或恢复
内容（仅限管理员）。除非 `dryRun` 为 `false`，否则默认采用试运行。
