...
旧版 `/api/...` 和 `/api/cli/...` 会保留以实现兼容（见 `DEPRECATIONS.md`）。
OpenAPI：`/api/v1/openapi.json`。

## 公共目录复用

第三方目录可以使用公共读取端点来列出或搜索 ClawHub Skills。请缓存结果，遵守 `429`/`Retry-After`，将用户链接回规范的 ClawHub 列表页（`https://clawhub.ai/<owner>/skills/<slug>`），并避免暗示 ClawHub 为该第三方站点背书。不要尝试在公共 API 表面之外镜像隐藏、私有或被审核屏蔽的内容。
...
- 当服务器知道原因时，已认证写入端点不应返回裸的 `Unauthorized`。缺失 token、无效/已吊销 token，以及已删除/封禁/禁用的账号，都应获得可操作的文本，以便 CLI 客户端能告诉用户是什么阻止了它们。

- 读取：每个 IP 3000/分钟，每个 key 12000/分钟
- 写入：每个 IP 300/分钟，每个 key 3000/分钟
- 下载：每个 IP 1200/分钟，每个 key 6000/分钟（下载端点）

标头：

- 旧版兼容：`X-RateLimit-Limit`、`X-RateLimit-Reset`
- 标准化：`RateLimit-Limit`、`RateLimit-Reset`
- 对 `429`：`X-RateLimit-Remaining: 0` 和 `RateLimit-Remaining: 0`
- 对 `429`：`Retry-After`
...
- 如果没有可用的可信客户端 IP，匿名请求会使用仅按速率限制类型确定作用域的回退桶。这些回退桶不包含调用者提供的路径、slug、包名、版本、查询字符串或其他构件参数。

## 错误响应

公共 v1 错误响应是纯文本，带有 `content-type: text/plain; charset=utf-8`。
这包括校验失败（`400`）、缺失的公共资源（`404`）、认证和权限失败（`401`/`403`）、速率限制（`429`）以及被阻止的下载。客户端应将响应正文读取为人类可读的字符串。未知查询参数会为了兼容性被忽略，但带有无效值的已识别查询参数会返回 `400`。
...
- `q`（必需）：查询字符串
- `limit`（可选）：整数
- `highlightedOnly`（可选）：`true` 表示过滤到高亮的 Skills
- `nonSuspiciousOnly`（可选）：`true` 表示隐藏可疑（`flagged.suspicious`）的 Skills
...
- 结果按相关性顺序返回（embedding 相似度 + 精确 slug/name token 加权 + 小幅热门度先验）。
- 相关性强于热门度。精确的 slug 或显示名称 token 匹配可以排在互动强得多但匹配更松散的结果前面。
- ASCII 文本会按词和标点边界分词。例如，`personal-map` 包含独立的 `map` token，而 `amap-jsapi-skill` 包含 `amap`、`jsapi` 和 `skill`；因此搜索 `map` 会让 `personal-map` 获得比 `amap-jsapi-skill` 更强的词法匹配。
- 热门度会按对数缩放并设置上限。当查询文本匹配较弱时，高互动 Skills 可能排名更低。
- 可疑或隐藏的审核状态可能会根据调用方过滤器和当前审核状态，将某个 Skill 从公共搜索中移除。

发布者可发现性指南：

- 将用户会按字面搜索的术语放入显示名称、摘要和标签中。仅当独立的 slug token 也是你想保留的稳定身份时，才使用它。
- 不要仅为了追逐某个查询而重命名 slug，除非新 slug 是更好的长期规范名称。旧 slug 会成为重定向别名，但规范 URL、显示的 slug 和未来的搜索摘要会使用新 slug。
- 重命名别名会保留旧 URL 和通过注册表解析的安装的解析能力，但搜索排名基于重命名索引完成后的规范 Skill 元数据。现有统计会保留在该 Skill 上。
...
- 当 `nonSuspiciousOnly=true` 时，基于游标的排序可能在某一页返回少于 `limit` 的项目，因为可疑 Skills 会在页面检索后被过滤。
...
- 证据会对公共调用方脱敏，并且只有所有者/审核员会包含原始片段。

### `POST /api/v1/skills/{slug}/report`

举报 Skill 以供审核员审查。举报是 Skill 级别的，可选关联到某个版本，并会进入 Skill 举报队列。
...
`note` 对 `confirmed` 和 `dismissed` 是必需的；将 `status` 设置回 `open` 时可以省略。传入 `finalAction: "hide"` 与已分诊的举报一起使用，可以在同一个可审计工作流中隐藏该 Skill。
...
- 包含规范化的验证状态以及扫描器专属的详细信息。
- 只有当扫描器给出明确判定（`clean`、`suspicious` 或 `malicious`）时，`security.hasScanResult` 才为 `true`。
- `moderation` 是从最新版本派生出的当前技能级别审核快照。
...
- 已发布扫描只有在 `update: true` 且扫描成功完成时才会回写。
...
- 需要对该技能或插件拥有所有者/发布者管理访问权限，或平台版主/管理员权限。
...
- 只有当选定版本已生成 Skill Card、未被审核作为恶意软件阻止，且 ClawScan 验证为 clean 时，`ok` 才为 `true`。
...
- 结果按条目返回；单个技能或版本缺失不会导致整个响应失败。
- 响应仅包含安全信息。它不包含 Skill Card 数据、已生成卡片状态、工件文件列表或详细扫描器载荷。
- `security.signals` 只包含状态级别的支持性证据；如需完整扫描器详情，请使用 `/scan` 或 ClawHub 安全审计页面。
- `security.signals.dependencyRegistry` 为了 v1 响应兼容性而保留，但依赖注册表存在性扫描器已退役，此键始终为 `null`。
...
- `version.sha256hash` 是面向旧客户端的已弃用兼容性元数据。它
  哈希 `/api/v1/packages/{name}/download` 返回的确切 ZIP 字节。
...
返回面向安装客户端的确切包发布安全与信任摘要。这是公开的 OpenClaw 消费表面，用于判断
已解析的发布版本是否可以安装。
...
`note` 对 `confirmed` 和 `dismissed` 是必需的；将 `status` 设回 `open` 时可以省略。传入 `finalAction: "quarantine"` 或 `finalAction: "revoke"`，可在确认报告的同时，在同一个可审计工作流中应用发布审核操作。
...
- 二进制文件返回 `415`。
- 文件大小限制：200KB。
- 待处理的 VirusTotal 扫描不会阻止读取；恶意发布仍可能在其他位置被暂扣。
...
返回由 ClawPack 支持的包版本的兼容 npm 的 packument。
...
下载托管的技能版本 ZIP，或者对于当前由 GitHub 支持、扫描结果为 `clean` 或 `suspicious` 且没有托管版本的技能，返回 GitHub 源码移交信息。
...
- 软删除版本返回 `410`。
- GitHub 支持的技能移交不会代理或镜像字节。JSON 响应包含 `sourceRef: "public-github"`、`repo`、`commit`、`path`、`contentHash` 和 `archiveUrl`；扫描/当前状态是门控条件，不作为成功载荷元数据包含。
- 下载统计按 UTC 日期按唯一身份计数（API 令牌有效时为 `userId`，否则为 IP）。
...
- 可选载荷字段：`migrateOwner`。当它与 `ownerHandle` 一起为 `true` 时，如果操作者在当前发布者和目标发布者上都是管理员/所有者，现有技能可以移动到该所有者。如果没有此选择加入，所有者更改会被拒绝。
...
- 代表他人发布时，仍会根据目标所有者账户验证 official-channel 资格。
...
对于新建组织，提供 `memberHandle`；执行操作的管理员不会被添加为成员。
...
仅管理员。为合法所有者保留根 slug 和包名称，而不发布版本。包名称会成为没有发布行的私有占位包，因此同一所有者之后可以将真正的 code-plugin 或 bundle-plugin 版本发布到该名称。
...
仅管理员。为经过验证的替代 GitHub OAuth 主体恢复个人发布者，而不编辑 Convex Auth 账户行。请求必须命名两个不可变 GitHub 提供商账户 id；可变 handle 只用作面向操作者的防护。

该端点默认使用试运行。应用恢复需要在工作人员独立验证两个
GitHub 主体之间的连续性后设置 `dryRun: false` 和
`confirmIdentityVerified: true`。当目标用户当前的个人发布者拥有技能、包或 GitHub 技能来源时，恢复会保守失败。
恢复还会迁移已恢复发布者的技能、技能 slug 别名、包、包检查器警告和派生搜索摘要行中的旧版 `ownerUserId` 字段，以便直接所有者路径与新的发布者权限保持一致。已恢复 handle 的活跃受保护 handle 保留记录也会重新分配给替换用户，避免后续个人资料同步恢复前用户的竞争性权限。每个主表在每次应用事务中限制为
100 行；更大的恢复必须先使用可恢复的所有者迁移。
GitHub 技能来源按发布者限定作用域，并报告为已检查而不是重写。
...
内容（仅限管理员）。除非 `dryRun` 为 `false`，否则默认使用试运行。
...
`POST /api/cli/upload-url` 返回 `uploadUrl` 和 `uploadTicket`。暂存 ClawPack tarball 的包发布必须将生成的 storage id 作为
