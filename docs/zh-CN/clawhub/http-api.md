...
旧版 `/api/...` 和 `/api/cli/...` 会保留以兼容（见 `DEPRECATIONS.md`）。
OpenAPI：`/api/v1/openapi.json`。

## 公共目录复用

第三方目录可以使用公共读取端点来列出或搜索 ClawHub 技能。请缓存结果，遵守 `429`/`Retry-After`，将用户链接回标准 ClawHub 列表（`https://clawhub.ai/<owner>/skills/<slug>`），并避免暗示 ClawHub 为该第三方站点背书。不要尝试在公共 API 表面之外镜像隐藏、私有或被审核拦截的内容。

Web slug 快捷方式会跨注册表族解析，但 API 客户端应使用读取端点返回的标准 URL，而不是重构路由优先级。
...
- 当服务器知道原因时，已认证写入端点不应返回裸 `Unauthorized`。缺失令牌、无效/已吊销令牌，以及已删除/已封禁/已停用账号都应获得可操作文本，以便 CLI 客户端可以告知用户是什么阻止了它们。
...
- `X-RateLimit-Remaining` / `RateLimit-Remaining`：存在时表示精确剩余配额。
...
- 仅当部署显式启用受信任的转发标头时，才会使用受信任的客户端 IP 标头，包括 `cf-connecting-ip`。
- ClawHub 使用受信任的转发标头在边缘识别客户端 IP。
- 如果没有可用的受信任客户端 IP，匿名请求会使用仅按速率限制类型限定范围的回退桶。这些回退桶不包括调用方提供的路径、slug、包名、版本、查询字符串或其他工件参数。

## 错误响应

公共 v1 错误响应是纯文本，`content-type: text/plain; charset=utf-8`。
这包括验证失败（`400`）、缺失公共资源（`404`）、认证和权限失败（`401`/`403`）、速率限制（`429`）以及被拦截的下载。客户端应将响应正文读取为人类可读字符串。未知查询参数会因兼容性而被忽略，但已识别查询参数的无效值会返回 `400`。
...
- `q`（必需）：查询字符串
- `limit`（可选）：整数
- `highlightedOnly`（可选）：`true` 表示仅筛选高亮技能
...
- 结果按相关性顺序返回（嵌入相似度 + 精确 slug/名称词元加权 + 小幅热门度先验）。
- 相关性强于热门度。精确的 slug 或显示名称词元匹配可以排在互动更强但匹配更宽泛的结果之前。
...
- 热门度会按对数缩放并设置上限。当查询文本匹配较弱时，高互动技能也可能排名更低。
- 可疑或隐藏的审核状态可能会根据调用方筛选器和当前审核状态从公共搜索中移除技能。

发布者可发现性指南：

- 将用户会直接搜索的词放入显示名称、摘要和标签中。仅当独立 slug 词元也是你想保留的稳定身份时才使用它。
- 不要仅为追逐某个查询而重命名 slug，除非新 slug 是更好的长期标准名称。旧 slug 会成为重定向别名，但标准 URL、显示的 slug 和未来搜索摘要会使用新 slug。
- 重命名别名会保留旧 URL 和通过注册表解析的安装的解析能力，但搜索排名基于重命名完成索引后的标准技能元数据。现有统计会保留在该技能上。
- 如果技能意外不可见，请在更改与排名相关的元数据之前，先在已登录状态下用 `clawhub inspect @owner/slug` 检查审核状态。
...
- 当 `nonSuspiciousOnly=true` 时，基于游标的排序在页面上返回的项目数可能少于 `limit`，因为可疑技能会在页面检索后被筛除。
- 存在 `nextCursor` 时，用它继续分页。短页面本身并不表示结果结束。
...
- 由所有者重命名/合并流程创建的旧 slug 会解析到标准技能。
- `metadata.os`：技能 frontmatter 中声明的 OS 限制（例如 `["macos"]`、`["linux"]`）。如果未声明，则为 `null`。
...
- 面向公共调用方的证据会被编辑隐藏，并且仅向所有者/审核员包含原始片段。

### `POST /api/v1/skills/{slug}/report`

报告技能以供审核员审查。报告是技能级别的，可选关联到某个版本，并进入技能报告队列。
...
`note` 对 `confirmed` 和 `dismissed` 是必需的；将 `status` 设回 `open` 时可以省略。对已分流报告传入 `finalAction: "hide"`，可在同一套可审计工作流中隐藏该技能。
...
- 响应为 `202`，并包含 `{ "ok": true, "scanId": "...", "jobId": "...", "status": "queued", "sourceKind": "published", "update": false, "queue": { "queuedAhead": 0, "queuedAheadIsEstimate": false, "position": 1, "running": 0, "runningIsEstimate": false, "note": "Scans are asynchronous and may take time to complete." } }`。
- 扫描作业是异步的。手动扫描请求会优先于常规发布/回填工作，但完成时间仍取决于工作进程可用性。
...
- 返回排队中/运行中/成功/失败状态。
- 排队期间返回 `queue.queuedAhead` 和 `queue.position`，以便客户端显示请求前还有多少个已优先处理的手动扫描。非常大的队列会被设定上限，并以 `queuedAheadIsEstimate: true` 报告。
- 可用时，`report` 包含 `clawscan`、`skillspector`、`staticAnalysis` 和 `virustotal` 区段。
...
仅限管理员的规范批量重新扫描路由。它接受与旧版 `POST /api/v1/skills/-/rescan-batch` 相同的载荷结构。

### `POST /api/v1/skills/-/scan/batch/status`

仅限管理员的规范批量状态路由。它接受 `{ "jobIds": ["..."] }`，并返回与旧版 `POST /api/v1/skills/-/rescan-batch/status` 相同的聚合计数器。

### `GET /api/v1/skills/{slug}/verify`

返回 `clawhub skill verify` 使用的技能卡片验证封装对象。
...
- 仅当所选版本具有已生成的技能卡片、未因审核中的恶意软件规则被阻止，并且 ClawScan 验证为干净时，`ok` 才为 `true`。
- 技能身份、发布者身份和所选版本元数据是顶层封装对象字段（`slug`、`displayName`、`publisherHandle`、`version`、`resolvedFrom`、`tag`、`createdAt`），因此 shell 自动化可以不解包嵌套包装器就读取它们。
- `security` 是顶层 ClawScan/安全结论。自动化应基于 `ok`、`decision`、`reasons` 和 `security.status` 做判断。
...
- `items` 必须包含 1 到 100 个唯一的 `{ slug, version }` 对。
- 结果按项目返回；某个技能或版本缺失不会导致整个响应失败。
- 响应仅包含安全信息。它不包含技能卡片数据、生成卡片状态、工件文件列表或详细扫描器载荷。
- `security.signals` 仅包含状态级支持证据；如需完整扫描器详情，请使用 `/scan` 或 ClawHub 安全审计页面。
- `security.signals.dependencyRegistry` 保留用于 v1 响应兼容性，但依赖注册表存在性扫描器已停用，此键始终为 `null`。
- 技能卡片缺失不会影响此端点的 `ok`、`decision` 或 `reasons`；当客户端需要卡片内容时，应在本地读取已安装的 `skill-card.md`。
- 当你需要单个技能的技能卡片验证封装对象时使用 `/verify`，需要生成的卡片 Markdown 时使用 `/card`，需要详细扫描器数据时使用 `/scan`。
...
- 当前由 GitHub 支持且扫描结果为 `clean` 或 `suspicious` 的技能包含 `_source_handoff.json`，其中有 `sourceRef: "public-github"`、仓库、提交、路径、内容哈希和归档 URL。它们不包含 ClawHub 托管的源文件。
...
仅限插件的搜索，覆盖 code-plugin 和 bundle-plugin 包。
...
- 分类过滤是真实的 API 过滤器，由插件分类摘要行支持，而不是搜索查询重写。
- 结果按相关性顺序返回，目前不分页。
...
- 对于旧世界包归档，`version.artifact.kind` 为 `legacy-zip`；对于由 ClawPack 支持的发布版本，则为 `npm-pack`。
- ClawPack 发布版本包含 npm 兼容的 `npmIntegrity`、`npmShasum` 和
  `npmTarballName` 字段。
- `version.sha256hash` 是面向旧客户端的已弃用兼容性元数据。它对 `/api/v1/packages/{name}/download` 返回的确切 ZIP 字节进行哈希。
...
返回供安装客户端使用的确切包发布安全与信任摘要。这是 OpenClaw 用于判断已解析发布版本是否可以安装的公开消费表面。
...
- `release.releaseId`、`release.version` 和 `release.createdAt` 标识已评估的确切发布版本。
...
- `trust.stale` 表示信任摘要是根据过期输入计算的，在作出高置信度允许决策前应视为需要刷新。

说明：

- 此端点精确到版本。客户端应在解析出计划安装的包版本后调用它，而不是只在读取最新包元数据后调用。
- 私有包返回 `404`，除非调用方可以读取所属发布者。
- 此端点刻意比所有者/版主审核端点更窄。它公开安装决策和公开解释，而不公开报告者身份、报告正文、私有证据或内部审查时间线。
...
- ClawPack 版本会流式传输确切上传的 npm-pack `.tgz` 字节。
- 旧版 ZIP 版本重定向到 `/api/v1/packages/{name}/download?version=`。
- 使用下载速率桶。

### `GET /api/v1/packages/{name}/readiness`

返回为未来 OpenClaw 消费计算的就绪状态。
...
- `open`：可疑、恶意、待处理、隔离、撤销或已报告的发布版本。
- `blocked`：隔离、撤销或恶意的发布版本。
...
报告一个包以供版主审查。报告是包级别的，可选择关联到某个版本。它们会进入审核队列，但本身不会自动隐藏或阻止下载；版主应使用发布版本审核来批准、隔离或撤销构件。
...
- 需要软件包所有者、发布者成员、审核员或管理员用户的 API 令牌。
...
- `revoked`：某个发布先前受信任后被阻止。

被隔离和被撤销的发布会从制品下载路由返回 `403`。每次更改都会写入审计日志条目。
...
返回由 ClawPack 支撑的软件包版本的 npm 兼容 packument。
...
- `slug`（必填）
- `hash`（必填）：bundle 指纹的 64 字符十六进制 sha256
...
下载托管的 Skill 版本 ZIP，或者对当前由 GitHub 支撑且扫描结果为 `clean` 或 `suspicious`、并且没有托管版本的 Skill 返回 GitHub 源交接信息。
...
- 如果未提供 `version` 或 `tag`，则使用最新版本。
- 软删除版本返回 `410`。
- 由 GitHub 支撑的 Skill 交接不会代理或镜像字节。JSON 响应包含 `sourceRef: "public-github"`、`repo`、`commit`、`path`、`contentHash` 和 `archiveUrl`；扫描/当前状态是门禁，不会作为成功载荷元数据包含。
- 下载统计按 UTC 日期以唯一身份计数（API 令牌有效时使用 `userId`，否则使用 IP）。
...
- 也接受包含 `files`（基于 storageId）的 JSON 主体。
- 可选载荷字段：`ownerHandle`。存在时，API 会在服务器端解析该发布者，并要求执行者拥有发布者访问权限。
- 可选载荷字段：`migrateOwner`。当它与 `ownerHandle` 一起为 `true` 时，如果执行者同时是当前发布者和目标发布者的管理员/所有者，现有 Skill 可以迁移到该所有者。如果没有此显式选择，所有者更改会被拒绝。
...
- 允许的表单字段是 `payload`、重复的 `files` blob，或一个 `clawpack` tarball 引用。`clawpack` 可以是 `.tgz` blob，也可以是由 upload-url 流程返回的存储 id。使用暂存 storage-id 发布时，还必须包含随该上传 URL 返回的 `clawpackUploadTicket`。
- 在同一个请求中只能使用 `files` 或 `clawpack`，不能同时使用。
- JSON 主体和调用方提供的 `payload.files` / `payload.artifact` 元数据会被拒绝。
- 直接 multipart 发布请求上限为 18MB。ClawPack tarball 可以使用 upload-url 流程，最高达到 120MB tarball 上限。
- 可选载荷字段：`ownerHandle`。存在时，只有管理员可以代表该所有者发布。

验证要点：

- `family` 必须是 `code-plugin` 或 `bundle-plugin`。
- 插件软件包需要 `openclaw.plugin.json`。ClawPack `.tgz` 上传必须在 `package/openclaw.plugin.json` 中包含它。
- 代码插件需要 `package.json`、源仓库元数据、源提交元数据、配置架构元数据、`openclaw.compat.pluginApi` 和 `openclaw.build.openclawVersion`。
...
存在时，`reason` 会作为 Skill 审核说明存储，并复制到审计日志中。所有者发起的软删除会保留 slug 30 天，之后该 slug 可由其他发布者认领。删除响应会在此到期适用时包含 `slugReservedUntil`。审核员/管理员隐藏和安全移除不会以这种方式到期。
...
仅限管理员。确保某个 handle 存在组织发布者。如果该 handle 仍指向旧版共享用户/个人发布者，端点会先将其迁移为组织发布者。对于新创建的组织，提供 `memberHandle`；执行操作的管理员不会被添加为成员。`memberRole` 默认为 `owner`。

- 主体：`{ "handle": "openclaw", "displayName": "OpenClaw", "memberHandle": "alice", "memberRole": "owner", "trusted": true }`
...
- 主体：`{ "handle": "opik", "displayName": "Opik" }`
- 响应：`{ "ok": true, "publisherId": "...", "handle": "opik", "created": true, "trusted": false }`
- 当 handle 已被发布者、用户或个人发布者使用时返回 `409`。

### `POST /api/v1/users/reserve`

仅限管理员。为正当所有者预留根 slug 和软件包名称，而不发布发布版本。软件包名称会变成没有发布行的私有占位软件包，因此同一个所有者之后可以将真实的 code-plugin 或 bundle-plugin 发布版本发布到该名称。

- 主体：`{ "handle": "openclaw", "slugs": ["diffs"], "packageNames": ["@openclaw/diffs"], "reason": "reserved for official OpenClaw plugin" }`
- 响应：`{ "ok": true, "succeeded": 2, "failed": 0, "results": [{ "kind": "slug", "name": "diffs", "ok": true, "action": "reserved" }] }`

### `POST /api/v1/users/publisher-recovery`

仅限管理员。为已验证的替代 GitHub OAuth 主体恢复个人发布者，而不编辑 Convex Auth 账户行。请求必须命名两个不可变的 GitHub 提供商账户 id；可变 handle 仅用作面向操作员的保护。

端点默认使用试运行。应用恢复需要在工作人员独立验证两个 GitHub 主体之间的连续性后，设置 `dryRun: false` 和
`confirmIdentityVerified: true`。当目标用户当前的个人发布者拥有技能、软件包或 GitHub 技能来源时，恢复会以关闭状态失败。
恢复还会迁移已恢复发布者的技能、技能 slug 别名、软件包、软件包检查器警告以及派生搜索摘要行中的旧版 `ownerUserId` 字段，使直接所有者路径与新的发布者权限保持一致。已恢复 handle 的活动受保护 handle 保留也会重新分配给替代用户，防止后续个人资料同步恢复原用户的竞争权限。每个主表在每次应用事务中限制为
...
更改现有封禁的已存储原因，不解除封禁或恢复内容（仅限管理员）。除非 `dryRun` 为 `false`，否则默认试运行。
...
参见 `DEPRECATIONS.md` 了解移除计划。

`POST /api/cli/upload-url` 返回 `uploadUrl` 和 `uploadTicket`。发布暂存 ClawPack tarball 的软件包时，必须将生成的存储 ID 作为
