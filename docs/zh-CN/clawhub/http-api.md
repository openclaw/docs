...
x-i18n:
    generated_at: "2026-06-30T13:45:58Z"
    model: gpt-5.5
...
第三方目录可以使用公共读取端点来列出或搜索 ClawHub Skills。请缓存结果，遵守 `429`/`Retry-After`，将用户链接回规范 ClawHub 列表（`https://clawhub.ai/<owner>/skills/<slug>`），并避免暗示 ClawHub 为该第三方站点背书。不要尝试在公共 API 表面之外镜像隐藏、私有或被审核拦截的内容。

Web slug 快捷方式会跨注册表系列解析，但 API 客户端应使用读取端点返回的规范 URL，而不是自行重构路由优先级。
...
- 当服务器知道原因时，已认证写入端点不应返回裸 `Unauthorized`。缺失 token、无效/已撤销 token，以及已删除/封禁/禁用账号，都应获得可操作的文本，以便 CLI 客户端能告知用户是什么阻止了他们。
...
- 发生 `429` 时：`X-RateLimit-Remaining: 0` 和 `RateLimit-Remaining: 0`
- 发生 `429` 时：`Retry-After`
...
- `Retry-After`：发生 `429` 时，重试前需要等待的秒数（延迟）
...
这包括验证失败（`400`）、缺失的公共资源（`404`）、认证和权限失败（`401`/`403`）、速率限制（`429`）以及被拦截的下载。客户端应将响应正文读取为人类可读的字符串。未知查询参数会被忽略以保持兼容性，但已识别且值无效的查询参数会返回 `400`。
...
- `q`（必需）：查询字符串
- `limit`（可选）：整数
- `highlightedOnly`（可选）：`true` 表示过滤为高亮 Skills
...
- 相关性强于热度。精确的 slug 或显示名称词元匹配，可以排在互动强得多但匹配更宽松的结果之前。
- ASCII 文本会按单词和标点边界分词。例如，`personal-map` 包含独立的 `map` 词元，而 `amap-jsapi-skill` 包含 `amap`、`jsapi` 和 `skill`；因此搜索 `map` 时，`personal-map` 的词汇匹配会强于 `amap-jsapi-skill`。
- 热度按对数缩放并设有上限。当查询文本匹配较弱时，高互动 Skills 也可能排名更低。
- 根据调用方过滤条件和当前审核状态，可疑或隐藏的审核状态可能会将某个 Skill 从公共搜索中移除。

发布者可发现性指南：

- 将用户会实际搜索的术语放入显示名称、摘要和标签中。仅当独立 slug 词元也是你希望保留的稳定身份时才使用它。
- 不要仅为追逐某个查询而重命名 slug，除非新 slug 是更好的长期规范名称。旧 slug 会成为重定向别名，但规范 URL、显示的 slug 和未来搜索摘要会使用新 slug。
- 重命名别名会为旧 URL 和通过注册表解析的安装保留解析能力，但搜索排名会基于重命名索引完成后的规范 Skill 元数据。现有统计信息会保留在该 Skill 上。
...
- `createdAt` 对新 Skill 抓取保持稳定；`updated` 会在现有 Skills 重新发布时变化。
- 当 `nonSuspiciousOnly=true` 时，基于游标的排序可能在一页中返回少于 `limit` 个条目，因为可疑 Skills 会在取回页面后被过滤。
...
- 只有当 Skill 被标记，或所有者正在查看它时，才会包含 `moderation`。
...
- 公共调用方只有对已经标记且可见的 Skills 才会获得 `200`。
- 面向公共调用方的证据会被脱敏，并且仅对所有者/版主包含原始片段。

### `POST /api/v1/skills/{slug}/report`

举报某个 Skill 以供版主审核。举报属于 Skill 级别，可选链接到某个版本，并会进入 Skill 举报队列。
...
`confirmed` 和 `dismissed` 需要 `note`；将 `status` 设回 `open` 时可以省略它。对已分流的举报传入 `finalAction: "hide"`，可在同一可审计工作流中隐藏该 Skill。
...
- `version.security` 会在可用时包含规范化的扫描验证状态和扫描器详情（VirusTotal + LLM）。
...
- 只有当扫描器产生明确判定（`clean`、`suspicious` 或 `malicious`）时，`security.hasScanResult` 才为 `true`。
- `moderation` 是从最新版本派生的当前技能级审核快照。
- 查询历史版本时，在将 `moderation` 和 `security` 视为同一版本上下文之前，先检查 `moderation.matchesRequestedVersion` 和 `moderation.sourceVersion`。
...
- 扫描作业是异步的。手动扫描请求会优先于普通发布/回填工作，但完成仍取决于 worker 可用性。
...
返回 `clawhub skill verify` 使用的技能卡验证封套。
...
- 只有当所选版本已生成技能卡、未被审核判定为恶意软件阻止，并且 ClawScan 验证为 clean 时，`ok` 才为 `true`。
- 技能身份、发布者身份和所选版本元数据是顶层封套字段（`slug`、`displayName`、`publisherHandle`、`version`、`resolvedFrom`、`tag`、`createdAt`），因此 shell 自动化可以在不解包嵌套包装器的情况下读取它们。
- `security` 是顶层 ClawScan/安全判定。自动化应基于 `ok`、`decision`、`reasons` 和 `security.status`。
...
返回确切技能版本的当前紧凑安全判定。此集合端点适用于已经知道需要显示哪些已安装 ClawHub 技能版本的客户端，例如 OpenClaw Control UI。
...
- 响应仅包含安全信息。它不包含技能卡数据、已生成卡状态、工件文件列表或详细扫描器载荷。
- `security.signals` 仅包含状态级支持证据；如需完整扫描器详细信息，请使用 `/scan` 或 ClawHub 安全审计页面。
- `security.signals.dependencyRegistry` 保留用于 v1 响应兼容性，但依赖注册表存在性扫描器已停用，此键始终为 `null`。
- 技能卡缺失不会影响此端点的 `ok`、`decision` 或 `reasons`；当客户端需要卡内容时，应在本地读取已安装的 `skill-card.md`。
- 当你需要单个技能的技能卡验证封套时使用 `/verify`，需要生成的卡 Markdown 时使用 `/card`，需要详细扫描器数据时使用 `/scan`。
...
- `category`（可选）：插件分类筛选器。仅当请求限定为插件包（`/api/v1/plugins`、
  `/api/v1/code-plugins`、`/api/v1/bundle-plugins`，或带有
  `family=code-plugin`/`family=bundle-plugin` 的包端点）时支持。受控分类和
...
- `GET /api/v1/code-plugins` 和 `GET /api/v1/bundle-plugins` 仍然是固定族别名。
...
- `channel`（可选）：`official`、`community` 或 `private`
- `isOfficial`（可选）：`true` 或 `false`
- `category`（可选）：插件分类筛选器。仅当请求限定为插件包时支持。受控分类和旧版 v1
...
- `category`（可选）：插件分类筛选器。当前值：
  `channels`、`models`、`memory`、`context`、`voice`、`media`、`web`、
  `tools`、`runtime`、`gateway`、`security`、`other`。

旧版 v1 筛选别名在读取端点上仍会被接受：
...
在统一 `/api/v1/packages` 端点上，它仅适用于插件；Skill 目录请使用
`/api/v1/skills?sort=trending`。

旧版别名不能作为已存储或作者声明的分类值被接受。
...
- 当前由 GitHub 支持且扫描结果为 `clean` 或 `suspicious` 的 Skills 包含
  `_source_handoff.json`，其中包含 `sourceRef: "public-github"`、仓库、提交、路径、
...
- 对旧式包归档，`version.artifact.kind` 为 `legacy-zip`；对
...
返回用于安装客户端的精确包发布安全与信任摘要。这是公开的 OpenClaw 消费接口，用于决定
已解析的发布版本是否可以安装。
...
- `release.releaseId`、`release.version` 和 `release.createdAt` 标识
  被评估的精确发布版本。
- 当发布制品已知时，会提供 `release.artifactKind`、`release.artifactSha256`、`release.npmIntegrity`、
  `release.npmShasum` 和 `release.npmTarballName`。
- `trust.scanStatus` 是从扫描器输入和手动发布审核推导出的有效信任状态。
- `trust.moderationState` 可为空。当不存在手动发布审核时，它为 `null`。
- `trust.blockedFromDownload` 是安装阻止信号。OpenClaw 和其他
  安装客户端应在该值为 `true` 时阻止安装，而不是
  从扫描器或审核字段重新推导阻止规则。
...
- 旧版包版本返回 `legacy-zip` 制品和旧版 ZIP
  `downloadUrl`。
- ClawPack 版本返回 `npm-pack` 制品、npm 完整性字段、
...
- `all`：任何带有手动覆盖、非 clean 扫描状态或包举报的发布版本。
...
举报一个包以供审核员审核。举报是包级别的，可选择
关联到一个版本。它们会进入审核队列，但本身不会自动隐藏或
阻止下载；审核员应使用发布版本审核来
批准、隔离或撤销制品。
...
- 需要包所有者、发布者成员、审核员或管理员用户的 API token。
...
`confirmed` 和 `dismissed` 必须提供 `note`；将 `status` 设回 `open` 时可以省略。对已确认的报告传入 `finalAction: "quarantine"` 或 `finalAction: "revoke"`，可在同一个可审计工作流中应用发布版本审核。
...
- `revoked`：在发布版本先前被信任后阻止。

被隔离和撤销的发布版本会从制品下载路由返回 `403`。每次更改都会写入一条审计日志。
...
- 二进制文件返回 `415`。
- 文件大小限制：200KB。
- 待处理的 VirusTotal 扫描不会阻止读取；恶意发布版本仍可能在其他位置被拒绝提供。
- 私有包返回 `404`，除非调用方可以读取所属发布者。
...
- 响应包含 `ETag`、`Digest`、`X-ClawHub-Artifact-Type` 和 `X-ClawHub-Artifact-Sha256` 头，用于解析器完整性检查。
- 仅注册表元数据不会注入到下载的归档中。
...
下载托管的 skill 版本 ZIP，或为当前由 GitHub 支持、扫描结果为 `clean` 或 `suspicious` 且没有托管版本的 skill 返回 GitHub 源交接。
...
- 软删除版本返回 `410`。
- GitHub 支持的 skill 交接不会代理或镜像字节。JSON 响应包含 `sourceRef: "public-github"`、`repo`、`commit`、`path`、`contentHash` 和 `archiveUrl`；扫描/当前状态是门禁，不会作为成功载荷元数据包含。
- 下载统计按 UTC 日以唯一身份计数（API token 有效时使用 `userId`，否则使用 IP）。
...
- 可选 payload 字段：`ownerHandle`。存在时，API 会在服务器端解析该发布者，并要求行为者拥有发布者访问权限。
- 可选 payload 字段：`migrateOwner`。当与 `ownerHandle` 一起为 `true` 时，如果行为者是当前发布者和目标发布者双方的管理员/所有者，则现有 skill 可以移动到该所有者。如果没有此显式选择，所有者更改会被拒绝。
...
- 允许的表单字段为 `payload`、重复的 `files` blob，或一个 `clawpack` tarball 引用。`clawpack` 可以是 `.tgz` blob，也可以是 upload-url 流程返回的 storage id。使用暂存 storage-id 发布时，还必须包含与该 upload URL 一起返回的 `clawpackUploadTicket`。
- 在同一个请求中只能使用 `files` 或 `clawpack`，不能同时使用两者。
...
- Code plugins 需要 `package.json`、源码仓库元数据、源码 commit 元数据、配置 schema 元数据、`openclaw.compat.pluginApi` 和 `openclaw.build.openclawVersion`。
- `openclaw.hostTargets` 和 `openclaw.environment` 是可选元数据。
- 只有 `openclaw` org 发布者和当前 `openclaw` org 成员的个人发布者可以发布到 `official` channel。
- 代发发布仍会根据目标所有者账户验证 official-channel 资格。

### `DELETE /api/v1/skills/{slug}` / `POST /api/v1/skills/{slug}/undelete`

软删除 / 恢复 skill（所有者、审核员或管理员）。
...
存在时，`reason` 会存储为 skill 审核说明，并复制到审计日志中。
由所有者发起的软删除会保留 slug 30 天，之后该 slug 可由其他发布者认领。删除响应会在此过期规则适用时包含 `slugReservedUntil`。
...
仅管理员。确保某个 handle 存在 org 发布者。如果该 handle 仍指向旧版共享用户/个人发布者，此端点会先将其迁移为 org 发布者。
对于新创建的 org，请提供 `memberHandle`；执行操作的管理员不会被添加为成员。
...
已认证的自助 org 发布者创建。创建新的 org 发布者，并将调用方添加为所有者。此端点不会迁移现有用户/个人 handle，也不会将发布者标记为 trusted/official。
...
仅管理员。为合法所有者保留根 slug 和包名称，而不发布 release。包名称会变成没有 release 行的私有占位包，因此同一所有者之后可以将真正的 code-plugin 或 bundle-plugin 发布版本发布到该名称。
...
仅管理员。为已验证的替代 GitHub OAuth 主体恢复个人发布者，而不编辑 Convex Auth account 行。请求必须指定两个不可变的 GitHub provider account id；可变 handle 仅用作面向操作员的保护。

端点默认以试运行模式执行。应用恢复需要在工作人员独立验证两个
GitHub 主体之间的连续性后设置 `dryRun: false` 和
`confirmIdentityVerified: true`。当目标用户当前的个人发布者拥有技能、软件包或 GitHub 技能源时，
恢复会以关闭方式失败。
恢复还会为已恢复发布者的技能、技能 slug 别名、软件包、软件包检查器警告和派生的搜索摘要行迁移旧版 `ownerUserId` 字段，以便
直接所有者路径与新的发布者权限保持一致。已恢复 handle 的活动受保护 handle
预留也会重新分配给替代用户，防止后续
个人资料同步恢复原用户的竞争权限。每个主表在每次应用事务中限制为
100 行；更大的恢复必须先使用可恢复的所有者迁移。
GitHub 技能源按发布者限定作用域，并报告为已检查，而不是重写。
