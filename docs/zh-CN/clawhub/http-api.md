...
Web slug 快捷路径会跨注册表族解析，但 API 客户端应使用读取端点返回的规范 URL，而不是自行重建路由优先级。
...
- 如果令牌缺失或无效，行为会回退到按 IP 执行。
- 当服务器知道原因时，已认证写入端点不应返回裸 `Unauthorized`。缺失令牌、无效或已撤销令牌，以及已删除、被封禁或被禁用的账户都应获得可操作文本，以便 CLI 客户端可以告知用户是什么阻止了他们。
...
- 如果没有可用的受信任客户端 IP，匿名请求会使用仅按速率限制类型划分的回退桶。这些回退桶不包含调用方提供的路径、slug、包名、版本、查询字符串或其他构件参数。

## 错误响应

公共 v1 错误响应是纯文本，`content-type: text/plain; charset=utf-8`。
这包括验证失败（`400`）、缺失的公共资源（`404`）、认证和权限失败（`401`/`403`）、速率限制（`429`）以及被屏蔽的下载。客户端应将响应正文读取为人类可读字符串。未知查询参数会被忽略以保持兼容性，但已识别查询参数如果值无效会返回 `400`。
...
- 相关性强于流行度。精确的 slug 或显示名称词元匹配，可以排在互动量高得多但匹配更宽泛的结果之前。
- ASCII 文本按单词和标点边界进行分词。例如，`personal-map` 包含独立的 `map` 词元，而 `amap-jsapi-skill` 包含 `amap`、`jsapi` 和 `skill`；因此搜索 `map` 会让 `personal-map` 获得比 `amap-jsapi-skill` 更强的词法匹配。
- 流行度会进行对数缩放并设置上限。当查询文本匹配较弱时，高互动技能可能排名更低。
- 可疑或隐藏的审核状态可能会根据调用方筛选器和当前审核状态，将某个技能从公共搜索中移除。

发布者可发现性指南：

- 将用户会实际搜索的词放入显示名称、摘要和标签中。仅当独立 slug 词元也是你希望保留的稳定身份时，才使用它。
- 不要仅为了追逐某个查询而重命名 slug，除非新 slug 是更好的长期规范名称。旧 slug 会成为重定向别名，但规范 URL、显示的 slug 和未来搜索摘要会使用新 slug。
- 重命名别名会为旧 URL 和通过注册表解析的安装保留解析能力，但在重命名完成索引后，搜索排名会基于规范技能元数据。现有统计数据会继续保留在该技能上。
- 如果某个技能意外不可见，请在更改与排名相关的元数据前，先在登录状态下使用 `clawhub inspect @owner/slug` 检查审核状态。
...
- `recommended` 使用互动和时效性信号。
- `trending` 按过去 7 天安装量排名（基于遥测）。
- `createdAt` 对新技能爬取保持稳定；`updated` 会在现有技能重新发布时变化。
- 当 `nonSuspiciousOnly=true` 时，基于游标的排序可能会在某一页返回少于 `limit` 的条目，因为可疑技能是在页面取回后再过滤的。
- 存在 `nextCursor` 时，使用它继续分页。短页本身并不意味着结果已结束。
...
`note` 对 `confirmed` 和 `dismissed` 是必需的；将 `status` 设置回 `open` 时可以省略。对已分流的报告传入 `finalAction: "hide"`，可在同一个可审计工作流中隐藏该技能。
...
- 需要对技能或插件具有所有者/发布者管理访问权限，或具有平台审核员/管理员权限。
...
- 只有当所选版本已生成 Skill Card、未被审核以恶意软件原因阻止，并且 ClawScan 验证为干净时，`ok` 才为 `true`。
- 技能身份、发布者身份和所选版本元数据是顶层信封字段（`slug`、`displayName`、`publisherHandle`、`version`、`resolvedFrom`、`tag`、`createdAt`），因此 shell 自动化可以读取它们，而无需拆开嵌套包装器。
...
- `security.signals.dependencyRegistry` 为 v1 响应兼容性而保留，但依赖注册表存在性扫描器已退役，并且此键始终为 `null`。
...
- 当你需要单个技能的 Skill Card 验证信封时使用 `/verify`，需要已生成的卡片 Markdown 时使用 `/card`，需要详细扫描器数据时使用 `/scan`。
...
- `category`（可选）：插件类别过滤器。仅当请求限定在插件包（`/api/v1/plugins`、
...
- 匿名调用方只能看到公开包频道。
- 已认证调用方可以在列表/搜索结果中看到自己所属发布者的私有包。
...
- `channel`（可选）：`official`、`community` 或 `private`
- `isOfficial`（可选）：`true` 或 `false`
- `category`（可选）：插件类别过滤器。仅当请求限定在插件包时支持。受控类别和旧版 v1
...
- 当前由 GitHub 支撑且扫描结果为 `clean` 或 `suspicious` 的技能包含
  `_source_handoff.json`，其中带有 sourceRef: "public-github"、repo、commit、path、
...
- 对旧世界包归档，`version.artifact.kind` 为 `legacy-zip`；对 ClawPack 支持的发布，
  则为 `npm-pack`。
...
- `version.sha256hash` 是面向旧客户端的已弃用兼容性元数据。它会对
  ...
  现代客户端应使用 `version.artifact.sha256`，它用于标识规范发布构件。
- 当扫描数据存在时，会包含 `version.vtAnalysis`、`version.llmAnalysis` 和 `version.staticScan`。
- 私有包返回 `404`，除非调用方可以读取拥有该包的发布者。

### `GET /api/v1/packages/{name}/versions/{version}/security`

返回安装客户端所需的精确包发布安全和信任摘要。这是 OpenClaw 用来判断
已解析发布是否可安装的公开消费面。
...
- ClawPack 版本返回 `npm-pack` 构件、npm 完整性字段、`tarballUrl` 以及旧版 ZIP 兼容性 URL。
...
举报一个包以供版主审核。举报是包级别的，可选择关联到版本。
它们会进入审核队列，但本身不会自动隐藏或阻止下载；版主应使用发布审核来批准、隔离或撤销构件。
...
对于 `confirmed` 和 `dismissed`，`note` 是必需的；将 `status` 设回 `open` 时可以省略。对已确认的报告传入 `finalAction: "quarantine"` 或 `finalAction: "revoke"`，即可在同一个可审计工作流中应用发布审核操作。
...
- `revoked`：某个发布版本此前受信任，之后被阻止。

被隔离和撤销的发布版本会在制品下载路由返回 `403`。每次更改都会写入一条审计日志条目。
...
下载托管的技能版本 ZIP，或者对于当前由 GitHub 支持、扫描结果为 `clean` 或 `suspicious` 且没有托管版本的技能，返回 GitHub 源交接信息。
...
- 可选载荷字段：`ownerHandle`。存在时，API 会在服务器端解析该发布者，并要求行为者拥有发布者访问权限。
- 可选载荷字段：`migrateOwner`。当它与 `ownerHandle` 一起为 `true` 时，如果行为者同时是当前和目标发布者的管理员/所有者，则现有技能可以移动到该所有者。如果没有此显式选择，所有者更改会被拒绝。
...
- 允许的表单字段为 `payload`、重复的 `files` blob，或一个 `clawpack` tarball 引用。`clawpack` 可以是 `.tgz` blob，也可以是 upload-url 流程返回的 storage id。使用暂存 storage-id 发布时，还必须包含该上传 URL 返回的 `clawpackUploadTicket`。
- 使用 `files` 或 `clawpack` 之一，不能在同一请求中同时使用两者。
- JSON 正文以及调用方提供的 `payload.files` / `payload.artifact` 元数据会被拒绝。
- 直接 multipart 发布请求上限为 18MB。ClawPack tarball 可通过 upload-url 流程使用最高 120MB 的 tarball 上限。
...
存在时，`reason` 会作为技能审核说明存储，并复制到审计日志中。
所有者发起的软删除会保留 slug 30 天，之后该 slug 可由其他发布者认领。删除响应会在适用此到期规则时包含 `slugReservedUntil`。
...
仅限管理员。确保某个 handle 存在对应的组织发布者。如果该 handle 仍指向旧版共享用户/个人发布者，此端点会先将其迁移为组织发布者。
...
仅限管理员。为合法所有者预留根 slug 和包名称，而不发布版本。包名称会成为没有发布行的私有占位包，因此同一所有者之后可以将真实的 code-plugin 或 bundle-plugin 发布版本发布到该名称下。
...
仅限管理员。为已验证的替代 GitHub OAuth 主体恢复个人发布者，而不编辑 Convex Auth 账户行。请求必须同时指定两个不可变的 GitHub 提供商账户 ID；可变 handle 仅用作面向操作者的防护。

该端点默认执行 dry-run。应用恢复需要在工作人员独立验证两个 GitHub 主体之间的连续性后，设置 `dryRun: false` 和
`confirmIdentityVerified: true`。当目标用户当前的个人发布者已有 skills、packages 或 GitHub skill sources 时，恢复会失败关闭。
恢复还会迁移已恢复发布者的 skills、skill slug aliases、packages、package inspector warnings 和派生 search digest rows 的旧版 `ownerUserId` 字段，使直接所有者路径与新的发布者权威保持一致。已恢复 handle 的活动 protected-handle reservation 也会重新分配给替代用户，避免后续个人资料同步恢复前用户的竞争性权威。每个主表在每次应用事务中限制为
...
- 两个端点都需要 API token auth，且仅适用于 skill 所有者。
...
更改现有封禁的已存储原因，而不解封或恢复内容（仅限 admin）。除非 `dryRun` 为 `false`，否则默认执行 dry-run。
...
`POST /api/cli/upload-url` 会返回 `uploadUrl` 和 `uploadTicket`。发布用于暂存 ClawPack tarball 的 Package 时，必须将生成的存储 id 作为
`clawpack` 发送，并将返回的票据作为 `clawpackUploadTicket` 发送。

## Registry 设备发现（`/.well-known/clawhub.json`）
