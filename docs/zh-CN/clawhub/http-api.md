...
旧版 `/api/...` 和 `/api/cli/...` 会保留以兼容（参见 `DEPRECATIONS.md`）。
OpenAPI：`/api/v1/openapi.json`。

## 公共目录复用

第三方目录可以使用公共读取端点来列出或搜索 ClawHub 技能。请缓存结果，遵守 `429`/`Retry-After`，将用户链接回规范 ClawHub 列表页（`https://clawhub.ai/<owner>/skills/<slug>`），并避免暗示 ClawHub 为该第三方站点背书。不要尝试在公共 API 表面之外镜像隐藏、私有或被审核拦截的内容。
...
- 当服务器知道原因时，已认证写入端点不应返回裸 `Unauthorized`。缺失令牌、无效/已撤销令牌以及已删除/已封禁/已禁用账户都应获得可操作文本，以便 CLI 客户端能告诉用户是什么阻止了他们。
...
这包括验证失败（`400`）、缺失的公共资源（`404`）、认证和权限失败（`401`/`403`）、速率限制（`429`）以及被拦截的下载。客户端应将响应正文读取为人类可读字符串。未知查询参数会被忽略以保持兼容，但已识别的查询参数如果值无效会返回 `400`。
...
- 结果按相关性顺序返回（嵌入相似度 + 精确 slug/名称 token 加权 + 小幅人气先验）。
- 相关性强于人气。精确的 slug 或显示名称 token 匹配可以排在参与度更高但匹配更宽松的结果之前。
...
- 人气会进行对数缩放并设置上限。当查询文本匹配较弱时，高参与度技能的排名可能更低。
- 可疑或隐藏的审核状态可能会根据调用方筛选器和当前审核状态，将技能从公共搜索中移除。

发布者可发现性指南：

- 将用户会实际搜索的词放入显示名称、摘要和标签。仅当独立 slug token 同时也是你想保留的稳定身份时才使用它。
- 不要仅为了追逐某个查询而重命名 slug，除非新 slug 是更好的长期规范名称。旧 slug 会成为重定向别名，但规范 URL、显示的 slug 和未来搜索摘要会使用新 slug。
- 重命名别名会为通过注册表解析的旧 URL 和安装保留解析能力，但搜索排名会基于重命名完成索引后的规范技能元数据。现有统计数据会留在该技能上。
- 如果某个技能意外不可见，请在登录后先用 `clawhub inspect @owner/slug` 检查审核状态，再更改与排名相关的元数据。
...
- 当 `nonSuspiciousOnly=true` 时，基于游标的排序可能会在某一页返回少于 `limit` 的项目，因为可疑技能是在页面检索后过滤的。
- 当存在 `nextCursor` 时，使用它继续分页。短页面本身并不意味着结果结束。
...
- 仅当技能被标记或所有者正在查看时才包含 `moderation`。
...
- 面向公共调用方时证据会被编辑，仅对所有者/审核员包含原始片段。

### `POST /api/v1/skills/{slug}/report`

举报技能以供审核员审查。举报是技能级别的，可选关联到某个版本，并会进入技能举报队列。
...
`confirmed` 和 `dismissed` 需要 `note`；将 `status` 设回 `open` 时可以省略。对已分流的举报传入 `finalAction: "hide"`，可在同一个可审计工作流中隐藏该技能。
...
- `version.security` 在可用时包含标准化扫描验证状态和扫描器详情（VirusTotal + LLM）。
...
- 排队时返回 `queue.queuedAhead` 和 `queue.position`，以便客户端显示该请求前面有多少个已优先处理的手动扫描。非常大的队列会被限制并以 `queuedAheadIsEstimate: true` 报告。
...
- 需要对该技能或插件具有所有者/发布者管理访问权限，或具有平台审核员/管理员权限。
...
- 只有当所选版本已生成 Skill Card、未被审核作为恶意软件阻止，并且 ClawScan 验证结果为 clean 时，`ok` 才为 `true`。
- 技能身份、发布者身份和所选版本元数据是顶层信封字段（`slug`、`displayName`、`publisherHandle`、`version`、`resolvedFrom`、`tag`、`createdAt`），因此 shell 自动化可以在不解包嵌套包装的情况下读取它们。
...
返回确切技能版本的当前紧凑安全判定。此集合端点面向已经知道需要显示哪些已安装 ClawHub 技能版本的客户端，例如 OpenClaw Control UI。
...
- `mcp-tooling`、`data` 和 `automation` 会解析为 `tools`。
- `observability` 和 `deployment` 会解析为 `gateway`。
- `dev-tools` 会解析为 `runtime`。
...
- 正文：ZIP 归档。
- 每个导出的 Skill 都以 `{publisher}/{slug}/` 为根。
- 托管 Skills 包含最新存储版本文件，并在 `_manifest.json` 中以 `sourceRef: "public-clawhub"` 列出。
- 当前由 GitHub 支持且扫描结果为 `clean` 或 `suspicious` 的 Skills 会包含 `_source_handoff.json`，其中带有 `sourceRef: "public-github"`、仓库、提交、路径、内容哈希和归档 URL。它们不包含 ClawHub 托管的源文件。
...
- 也接受 `GET /api/v1/plugins` 下记录的旧版 v1 筛选别名。
- 分类筛选是由插件分类摘要行支持的真实 API 筛选器，而不是搜索查询重写。
- 结果按相关性顺序返回，目前不分页。
...
- 对于旧世界包归档，`version.artifact.kind` 为 `legacy-zip`；对于 ClawPack 支持的版本，则为 `npm-pack`。
- ClawPack 版本包含兼容 npm 的 `npmIntegrity`、`npmShasum` 和 `npmTarballName` 字段。
- `version.sha256hash` 是面向旧客户端的已弃用兼容性元数据。它对 `/api/v1/packages/{name}/download` 返回的精确 ZIP 字节进行哈希。现代客户端应使用 `version.artifact.sha256`，它标识规范版本构件。
- 当存在扫描数据时，会包含 `version.vtAnalysis`、`version.llmAnalysis` 和 `version.staticScan`。
- 私有包会返回 `404`，除非调用方可以读取所属发布者。

### `GET /api/v1/packages/{name}/versions/{version}/security`

返回供安装客户端使用的精确包版本安全与信任摘要。这是公开的 OpenClaw 消费界面，用于判断已解析版本是否可以安装。

认证：

- 公开读取端点。不需要所有者、发布者、审核员或管理员 token。
...
- 当版本构件已知时，会包含 `release.artifactKind`、`release.artifactSha256`、`release.npmIntegrity`、`release.npmShasum` 和 `release.npmTarballName`。
...
- 此端点精确到版本。客户端应在解析其打算安装的包版本后调用它，而不仅是在读取最新包元数据后调用。
- 私有包会返回 `404`，除非调用方可以读取所属发布者。
- 此端点有意比所有者/审核员审核端点更窄。它公开安装决策和公开说明，而不是举报者身份、报告正文、私有证据或内部审核时间线。
...
- ClawPack 版本返回 `npm-pack` 构件、npm integrity 字段、`tarballUrl` 以及旧版 ZIP 兼容 URL。
- 这是 OpenClaw 解析器界面；它避免从共享 URL 猜测归档格式。
...
报告一个包以供审核员审核。报告是包级别的，也可以选择关联到某个版本。它们会进入审核队列，但本身不会自动隐藏或阻止下载；审核员应使用版本审核来批准、隔离或撤销构件。
...
- `revoked`：在某个发布曾被信任后被阻止。

隔离和撤销的发布会从制品下载路由返回 `403`。每次更改都会写入一条审计日志记录。
...
- 插件/包归档是带有 `package/` 根目录的 zip 文件，因此旧 OpenClaw 客户端仍可工作。
...
下载托管的技能版本 ZIP，或者对当前由 GitHub 支持、扫描结果为 `clean` 或 `suspicious` 且没有托管版本的技能，返回 GitHub 源交接信息。
...
- 下载统计按每个 UTC 日的唯一身份计数（API 令牌有效时为 `userId`，否则为 IP）。
...
- 可选载荷字段：`migrateOwner`。当它与 `ownerHandle` 一起为 `true` 时，如果操作者同时是当前和目标发布者的管理员/所有者，现有技能可以移动到该所有者。没有此选择加入时，所有者更改会被拒绝。
...
- 允许的表单字段为 `payload`、重复的 `files` blob，或一个 `clawpack` tarball 引用。`clawpack` 可以是 `.tgz` blob，也可以是由 upload-url 流程返回的存储 id。暂存的 storage-id 发布还必须包含随该上传 URL 返回的 `clawpackUploadTicket`。
- 使用 `files` 或 `clawpack` 之一，不要在同一请求中同时使用两者。
- JSON 正文以及调用方提供的 `payload.files` / `payload.artifact` 元数据会被拒绝。
- 直接 multipart 发布请求上限为 18MB。ClawPack tarball 可以使用 upload-url 流程，最高可达 120MB tarball 上限。
...
所有者发起的软删除会保留 slug 30 天，之后该 slug 可由另一发布者认领。适用此过期规则时，删除响应包含 `slugReservedUntil`。
...
仅管理员可用。确保某个 handle 存在组织发布者。如果该 handle 仍指向旧版共享用户/个人发布者，端点会先将其迁移为组织发布者。对于新创建的组织，请提供 `memberHandle`；执行操作的管理员不会被添加为成员。`memberRole` 默认为 `owner`。
...
仅管理员可用。为合法所有者保留根 slug 和包名称，而不发布版本。包名称会成为没有发布行的私有占位包，因此同一所有者之后可以将真正的 code-plugin 或 bundle-plugin 版本发布到该名称下。
...
仅管理员可用。为已验证的替代 GitHub OAuth 主体恢复个人发布者，而不编辑 Convex Auth 账号行。请求必须指定两个不可变的 GitHub 提供商账号 id；可变 handle 仅用作面向操作者的保护检查。

端点默认执行试运行。应用恢复需要在工作人员独立核验两个 GitHub 主体之间的连续性后，设置 `dryRun: false` 和
`confirmIdentityVerified: true`。当目标用户当前的个人
发布者拥有技能、软件包或 GitHub 技能来源时，恢复会以安全关闭方式失败。
恢复还会迁移已恢复发布者的技能、技能 slug 别名、软件包、软件包检查器警告以及派生搜索摘要行中的旧版 `ownerUserId` 字段，使
直接所有者路径与新的发布者权威保持一致。已恢复 handle 的有效受保护 handle
预留也会重新分配给替代用户，确保后续
个人资料同步无法恢复前用户的竞争性权威。每个主表在每次应用事务中限制为
100 行；更大的恢复必须先使用可恢复的所有者迁移。
GitHub 技能来源的作用域是发布者，并会报告为已检查，而不是被重写。
...
更改现有封禁的已存储原因，不解除封禁，也不恢复
内容（仅限管理员）。除非 `dryRun` 为 `false`，否则默认执行试运行。
