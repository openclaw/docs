...
第三方目录可以使用公共读取端点来列出或搜索 ClawHub 技能。请缓存结果，遵守 `429`/`Retry-After`，将用户链接回规范的 ClawHub 列表页（`https://clawhub.ai/<owner>/skills/<slug>`），并避免暗示 ClawHub 认可该第三方站点。不要尝试在公共 API 表面之外镜像隐藏、私有或被审核阻止的内容。
...
- 当服务器知道原因时，已认证写入端点不应返回裸 `Unauthorized`。缺失 token、无效/已撤销 token，以及已删除/封禁/禁用的账号都应获得可操作的文本，以便 CLI 客户端告知用户阻塞原因。
...
- 如果没有可用的受信任客户端 IP，匿名请求会使用仅按速率限制类别限定范围的回退桶。这些回退桶不包含调用者提供的路径、slug、包名、版本、查询字符串或其他制品参数。

## 错误响应

公共 v1 错误响应是纯文本，`content-type: text/plain; charset=utf-8`。
这包括验证失败（`400`）、缺失公共资源（`404`）、认证和权限失败（`401`/`403`）、速率限制（`429`）以及被阻止的下载。客户端应将响应正文读取为人类可读的字符串。未知查询参数会被忽略以保持兼容性，但已识别的查询参数如果值无效会返回 `400`。
...
- 相关性比流行度更强。精确的 slug 或显示名称 token 匹配可以排在互动量高得多但匹配更宽松的结果之前。
...
- 流行度采用对数缩放并设置上限。当查询文本匹配较弱时，高互动技能可能排名更低。
- 可疑或隐藏的审核状态可能会根据调用者筛选器和当前审核状态，从公共搜索中移除某个技能。

发布者可发现性指南：

- 将用户会实际搜索的术语放入显示名称、摘要和标签中。仅当独立 slug token 也是你想保留的稳定身份时，才使用它。
- 不要仅为了追逐某个查询而重命名 slug，除非新 slug 是更好的长期规范名称。旧 slug 会成为重定向别名，但规范 URL、显示的 slug 和未来搜索摘要会使用新 slug。
- 重命名别名会保留旧 URL 和通过注册表解析的安装的解析能力，但搜索排名基于重命名已索引后的规范技能元数据。现有统计信息会保留在该技能上。
...
- `createdAt` 对新技能抓取保持稳定；`updated` 会在现有技能重新发布时改变。
- 当 `nonSuspiciousOnly=true` 时，基于游标的排序可能会在某页返回少于 `limit` 的条目，因为可疑技能是在页面检索之后过滤的。
...
- 只有当技能被标记或所有者正在查看它时，才包含 `moderation`。
...
- 公共调用者仅会对已标记且可见的技能获得 `200`。
- 公共调用者看到的证据会被编辑隐藏，只有所有者/审核员会看到原始片段。

### `POST /api/v1/skills/{slug}/report`

举报技能以供审核员审查。举报以技能为级别，可选择链接到某个版本，并进入技能举报队列。
...
`confirmed` 和 `dismissed` 需要 `note`；将 `status` 设回 `open` 时可以省略。对已分流处理的举报传入 `finalAction: "hide"`，可在同一个可审计工作流中隐藏该技能。
...
- 返回排队/运行中/成功/失败状态。
- 排队时返回 `queue.queuedAhead` 和 `queue.position`，以便客户端显示该请求前面还有多少个优先手动扫描。非常大的队列会被限制，并以 `queuedAheadIsEstimate: true` 报告。
...
用于已提交版本的需身份验证存储报告归档端点。

- 需要对技能或插件拥有所有者/发布者管理访问权限，或平台版主/管理员权限。
...
- 响应仅包含安全信息。它不包括 Skill Card 数据、生成的卡片状态、构件文件列表或详细扫描器负载。
- `security.signals` 仅包含状态级支持证据；如需完整扫描器详情，请使用 `/scan` 或 ClawHub 安全审计页面。
- `security.signals.dependencyRegistry` 保留用于 v1 响应兼容性，但依赖注册表存在性扫描器已退役，此键始终为 `null`。
- 缺少 Skill Card 不会影响此端点的 `ok`、`decision` 或 `reasons`；客户端在需要卡片内容时，应在本地读取已安装的 `skill-card.md`。
...
- `category`（可选）：插件类别筛选器。仅当请求限定为插件包（`/api/v1/plugins`、`/api/v1/code-plugins`、`/api/v1/bundle-plugins`，或带有 `family=code-plugin`/`family=bundle-plugin` 的 package 端点）时支持。受控类别和旧版 v1 筛选器别名记录在 `GET /api/v1/plugins` 下。
...
- 匿名调用者只能看到公开 package 频道。
- 已认证调用者可以在列表/搜索结果中看到他们所属发布者的私有 package。
- `channel=private` 仅返回已认证调用者可读取的 package。

### `GET /api/v1/packages/search`

跨 Skills + 插件 package 的统一目录搜索。
...
- `channel`（可选）：`official`、`community` 或 `private`
- `isOfficial`（可选）：`true` 或 `false`
- `category`（可选）：插件类别筛选器。仅当请求限定为插件 package 时支持。受控类别和旧版 v1 筛选器别名记录在 `GET /api/v1/plugins` 下。

说明：

- `family`、`channel`、`isOfficial`、`featured` 或 `highlightedOnly` 的值无效时返回 `400`。未知查询参数会被忽略。
- 匿名调用者只能看到公开 package 频道。
- 已认证调用者可以搜索他们所属发布者的私有 package。
- `channel=private` 仅返回已认证调用者可读取的 package。

### `GET /api/v1/plugins`

仅插件目录浏览，覆盖 code-plugin 和 bundle-plugin package。
...
- 当前由 GitHub 支持且扫描结果为 `clean` 或 `suspicious` 的 Skills 包含
  ...
  content hash 和 archive URL。它们不包含 ClawHub 托管的源文件。
...
返回供安装客户端使用的精确包发布版本安全与信任摘要。这是公共 OpenClaw 消费表面，用于决定某个
已解析发布版本是否可以安装。

认证：

- 公共读取端点。不需要所有者、发布者、审核员或管理员 token。
...
- 私有包返回 `404`，除非调用方可以读取所属发布者。
- 此端点有意比所有者/审核员审核
  端点更窄。它暴露安装决定和公开说明，而不是
  报告者身份、报告正文、私有证据或内部审查
  时间线。
...
- `bundledPluginId` 会标准化为小写，并且是稳定的 upsert 键。
...
`note` 对 `confirmed` 和 `dismissed` 是必需的；将 `status` 设回 `open` 时可以省略。对于已确认的报告，传入 `finalAction: "quarantine"` 或 `finalAction: "revoke"`，可在同一个可审计工作流中应用发布审核。
...
- `revoked`：在某个发布先前受信任后被阻止。

隔离和撤销的发布会从制品下载路由返回 `403`。每次变更都会写入一条审计日志。
...
- 插件/包归档是带有 `package/` 根目录的 zip 文件，因此旧版 OpenClaw 客户端仍能工作。
...
下载托管的 skill 版本 ZIP，或在当前由 GitHub 支持的 skill 扫描为 `clean` 或 `suspicious` 且没有托管版本时，返回 GitHub 源码交接信息。
...
- 软删除的版本返回 `410`。
- 由 GitHub 支持的 skill 交接不会代理或镜像字节。JSON 响应包含 `sourceRef: "public-github"`、`repo`、`commit`、`path`、`contentHash` 和 `archiveUrl`；扫描/当前状态是准入条件，不会作为成功载荷元数据包含。
...
- 可选载荷字段：`migrateOwner`。当它与 `ownerHandle` 一起为 `true` 时，如果执行者同时是当前和目标发布者的管理员/所有者，则现有 skill 可以移动到该所有者。没有此显式选择时，所有者变更会被拒绝。
...
- 允许的表单字段是 `payload`、重复的 `files` blob，或一个 `clawpack` tarball 引用。`clawpack` 可以是 `.tgz` blob，或由 upload-url 流程返回的存储 id。分阶段的 storage-id 发布还必须包含与该上传 URL 一起返回的 `clawpackUploadTicket`。
...
所有者发起的软删除会保留 slug 30 天，之后该 slug 可由其他发布者声明。删除响应会在此过期适用时包含 `slugReservedUntil`。
...
仅管理员。确保某个 handle 存在组织发布者。如果该 handle 仍指向旧版共享用户/个人发布者，该端点会先将其迁移为组织发布者。对于新创建的组织，提供 `memberHandle`；执行操作的管理员不会被添加为成员。`memberRole` 默认为 `owner`。
...
仅管理员。为合法所有者保留根 slug 和包名称，而不发布版本。包名称会成为没有发布行的私有占位包，因此同一所有者稍后可以将真正的 code-plugin 或 bundle-plugin 版本发布到该名称。
...
仅管理员。为已验证的替换 GitHub OAuth 主体恢复个人发布者，而不编辑 Convex Auth 账户行。请求必须指定两个不可变的 GitHub 提供商账户 id；可变 handle 仅用作面向操作员的保护。

该端点默认执行试运行。应用恢复需要在工作人员独立验证两个 GitHub 主体之间的连续性后设置 `dryRun: false` 和
`confirmIdentityVerified: true`。当目标用户当前的个人
发布者拥有技能、包或 GitHub 技能源时，恢复会以关闭方式失败。
恢复还会迁移已恢复发布者的技能、技能 slug 别名、包、包检查器警告以及派生搜索摘要行中的旧版 `ownerUserId` 字段，以便
直接所有者路径与新的发布者权限保持一致。已恢复 handle 的活动受保护 handle
保留项也会重新分配给替代用户，使后续
资料同步无法恢复前用户的竞争权限。每个主表在每次应用事务中限制为
100 行；更大的恢复必须先使用可恢复的所有者迁移。
GitHub 技能源按发布者限定范围，并报告为已检查，而不是被重写。
...
`POST /api/cli/upload-url` 会返回 `uploadUrl` 和 `uploadTicket`。暂存
ClawPack tarball 的包发布必须将生成的存储 id 作为
