...
旧版 `/api/...` 和 `/api/cli/...` 保留用于兼容性（请参阅 `DEPRECATIONS.md`）。
OpenAPI：`/api/v1/openapi.json`。

## 公共目录复用

第三方目录可以使用公共读取端点列出或搜索 ClawHub skills。请缓存结果，遵守 `429`/`Retry-After`，将用户链接回规范的 ClawHub 列表（`https://clawhub.ai/<owner>/skills/<slug>`），并避免暗示 ClawHub 认可第三方站点。不要试图在公共 API 表面之外镜像隐藏、私有或被审核屏蔽的内容。
...
- 当服务器知道原因时，已认证写入端点不应返回裸 `Unauthorized`。缺失令牌、无效/已撤销令牌，以及已删除/封禁/禁用账户都应获得可操作文本，以便 CLI 客户端可以告诉用户是什么阻止了他们。
...
- `X-RateLimit-Reset`：绝对 Unix epoch 秒
- `RateLimit-Reset`：距离重置的秒数（延迟）
- `X-RateLimit-Remaining` / `RateLimit-Remaining`：存在时表示精确剩余预算。
...
- 使用带抖动的退避，避免同步重试。
- 如果 `Retry-After` 缺失，则回退到 `RateLimit-Reset`（或根据 `X-RateLimit-Reset` 计算）。
...
这包括验证失败（`400`）、缺失的公共资源（`404`）、认证和权限失败（`401`/`403`）、速率限制（`429`）以及被阻止的下载。客户端应将响应正文读取为人类可读字符串。未知查询参数会出于兼容性被忽略，但已识别且值无效的查询参数会返回 `400`。
...
- 结果按相关性顺序返回（嵌入相似度 + 精确 slug/name 词元提升 + 少量流行度先验）。
- 相关性强于流行度。精确的 slug 或显示名称词元匹配可能会排在参与度强得多但匹配更宽松的结果之前。
...
- 流行度经过对数缩放并封顶。当查询文本匹配较弱时，高参与度 skills 的排名可能更低。
- 可疑或隐藏的审核状态可能会根据调用方筛选器和当前审核状态，从公共搜索中移除某个 skill。

发布者可发现性指南：

- 将用户会实际搜索的词放在显示名称、摘要和标签中。仅当独立 slug 词元也是你想保留的稳定身份时才使用它。
- 不要只是为了追逐某个查询而重命名 slug，除非新 slug 是更好的长期规范名称。旧 slug 会变成重定向别名，但规范 URL、显示的 slug 和未来搜索摘要会使用新 slug。
- 重命名别名会保留旧 URL 和通过注册表解析的安装的解析能力，但搜索排名基于重命名已建立索引后的规范 skill 元数据。现有统计会留在该 skill 上。
- 如果某个 skill 意外不可见，请在更改排名相关元数据之前，先在登录状态下使用 `clawhub inspect @owner/slug` 检查审核状态。
...
- 当 `nonSuspiciousOnly=true` 时，基于游标的排序可能在一页中返回少于 `limit` 的项目，因为可疑 skills 会在页面检索后被过滤。
...
- 所有者重命名/合并流程创建的旧 slug 会解析到规范 skill。
- `metadata.os`：skill frontmatter 中声明的 OS 限制（例如 `["macos"]`、`["linux"]`）。如果未声明则为 `null`。
- `metadata.systems`：Nix system 目标（例如 `["aarch64-darwin", "x86_64-linux"]`）。如果未声明则为 `null`。
...
- 面向公共调用方时，证据会被遮盖；只有所有者/版主可看到原始片段。

### `POST /api/v1/skills/{slug}/report`

报告一个 skill 以供版主审核。报告为 skill 级别，可选链接到一个版本，并会进入 skill 报告队列。
...
`note` 对 `confirmed` 和 `dismissed` 是必需的；将 `status` 设置回 `open` 时可以省略。将 `finalAction: "hide"` 与已分流处理的报告一起传入，可以在同一个可审计工作流中隐藏该 skill。
...
- `version.security` 包含规范化的扫描验证状态和扫描器详情
  （VirusTotal + LLM），如果可用。
...
- 只有当扫描器产出了明确结论（`clean`、`suspicious` 或 `malicious`）时，`security.hasScanResult` 才为 `true`。
...
- 扫描请求载荷和可下载报告会在保留窗口结束后从扫描请求存储中失效。
- 已发布扫描需要所有者/发布者管理访问权限，或平台审核员/管理员权限。
- 只有在 `update: true` 且扫描成功完成时，已发布扫描才会写回。
...
- 返回排队/运行中/成功/失败状态。
- 排队期间返回 `queue.queuedAhead` 和 `queue.position`，以便客户端显示请求前方有多少个优先级更高的手动扫描。非常大的队列会被限制，并以 `queuedAheadIsEstimate: true` 报告。
...
- 只有当所选版本已生成技能卡片、未被审核判定为恶意软件阻止，并且 ClawScan 验证为干净时，`ok` 才为 `true`。
- 技能身份、发布者身份和所选版本元数据是顶层信封字段（`slug`、`displayName`、`publisherHandle`、`version`、`resolvedFrom`、`tag`、`createdAt`），因此 shell 自动化无需解包嵌套包装即可读取它们。
- `security` 是顶层 ClawScan/安全结论。自动化应以 `ok`、`decision`、`reasons` 和 `security.status` 为依据。
...
返回精确技能版本的当前紧凑安全结论。此集合端点面向已经知道需要展示哪些已安装 ClawHub 技能版本的客户端，例如 OpenClaw Control UI。
...
- `security.signals` 仅包含状态级支持证据；完整扫描器详情请使用 `/scan` 或 ClawHub security-audit 页面。
- `security.signals.dependencyRegistry` 为 v1 响应兼容性而保留，但依赖注册表存在性扫描器已停用，并且此键始终为 `null`。
...
- `category`（可选）：插件类别过滤器。仅当请求限定到插件包（`/api/v1/plugins`、
...
仅插件目录浏览，涵盖 code-plugin 和 bundle-plugin 包。
...
`trending` 是七天安装/下载排行榜，不使用总历史累计值。
...
- 托管技能包含最新存储版本文件，并列在
  `_manifest.json` 中，带有 `sourceRef: "public-clawhub"`。
...
- `family`（可选）：`code-plugin` 或 `bundle-plugin`。省略表示两个插件族都包含。
...
跨 code-plugin 和 bundle-plugin 包的仅插件搜索。
...
- 对旧世界包归档，`version.artifact.kind` 为 `legacy-zip`；对 ClawPack 支持的发布，为 `npm-pack`。
- ClawPack 发布包含兼容 npm 的 `npmIntegrity`、`npmShasum` 和 `npmTarballName` 字段。
- `version.sha256hash` 是面向旧客户端的已弃用兼容性元数据。它哈希的是 `/api/v1/packages/{name}/download` 返回的精确 ZIP 字节。现代客户端应使用 `version.artifact.sha256`，它标识规范发布工件。
- 当存在扫描数据时，会包含 `version.vtAnalysis`、`version.llmAnalysis` 和 `version.staticScan`。
- 私有包会返回 `404`，除非调用方可以读取所属发布者。

### `GET /api/v1/packages/{name}/versions/{version}/security`

返回面向安装客户端的精确包发布安全和信任摘要。这是公开的 OpenClaw 消费表面，用于决定解析出的发布是否可以安装。
...
- `release.releaseId`、`release.version` 和 `release.createdAt` 标识已评估的精确发布。
- 当发布工件已知时，会提供 `release.artifactKind`、`release.artifactSha256`、`release.npmIntegrity`、`release.npmShasum` 和 `release.npmTarballName`。
- `trust.scanStatus` 是从扫描器输入和手动发布审核派生出的有效信任状态。
- `trust.moderationState` 可为空。当不存在手动发布审核时，它为 `null`。
- `trust.blockedFromDownload` 是安装阻止信号。OpenClaw 和其他安装客户端应在此值为 `true` 时阻止安装，而不是根据扫描器或审核字段重新推导阻止规则。
...
- `trust.stale` 表示信任摘要是根据过时输入计算的，在做出高置信度允许决策之前应将其视为需要刷新。

说明：

- 此端点精确到版本。客户端应在解析出打算安装的包版本后调用它，而不是只在读取最新包元数据后调用。
- 私有包会返回 `404`，除非调用方可以读取所属发布者。
- 此端点故意比所有者/版主审核端点更窄。它公开安装决策和公开解释，而不公开报告者身份、报告正文、私有证据或内部审查时间线。
...
- 旧版包版本返回一个 `legacy-zip` 工件和一个旧版 ZIP `downloadUrl`。
- ClawPack 版本返回一个 `npm-pack` 工件、npm 完整性字段、一个 `tarballUrl` 以及旧版 ZIP 兼容性 URL。
...
报告一个包以供版主审查。报告是包级别的，可选择链接到某个版本。它们会进入审核队列，但本身不会自动隐藏或阻止下载；版主应使用发布审核来批准、隔离或撤销工件。
...
`note` 对 `confirmed` 和 `dismissed` 是必需的；将 `status` 设回 `open` 时可以省略。对已确认的报告传入 `finalAction: "quarantine"` 或 `finalAction: "revoke"`，可在同一个可审计工作流中应用发布审核。
...
- `revoked`：在某个发布此前受信任后将其阻止。

被隔离和撤销的发布会从工件下载路由返回 `403`。
每次变更都会写入一条审计日志记录。
...
下载托管的 Skills 版本 ZIP，或对于当前由 GitHub 支持、扫描结果为 `clean` 或 `suspicious` 且没有托管版本的 Skills，返回 GitHub 源交接信息。
...
- 如果既未提供 `version` 也未提供 `tag`，则使用最新版本。
- 软删除的版本返回 `410`。
- 由 GitHub 支持的 Skills 交接不会代理或镜像字节。JSON 响应包含 `sourceRef: "public-github"`、`repo`、`commit`、`path`、`contentHash` 和 `archiveUrl`；扫描/当前状态是门禁，不会作为成功负载元数据包含。
- 下载统计按 UTC 日期计算唯一身份（API 令牌有效时使用 `userId`，否则使用 IP）。
...
- 可选负载字段：`ownerHandle`。存在时，API 会在服务端解析该发布者，并要求操作者拥有发布者访问权限。
- 可选负载字段：`migrateOwner`。当其为 `true` 且带有 `ownerHandle` 时，如果操作者是当前发布者和目标发布者双方的管理员/所有者，现有 Skills 可以移动到该所有者。没有此选择加入时，所有者变更会被拒绝。
...
- 允许的表单字段是 `payload`、重复的 `files` blob，或一个 `clawpack` tarball 引用。`clawpack` 可以是 `.tgz` blob，也可以是 upload-url 流程返回的存储 id。基于暂存 storage-id 的发布还必须包含随该上传 URL 返回的 `clawpackUploadTicket`。
- 使用 `files` 或 `clawpack` 二选一，不能在同一请求中同时使用。
- JSON 正文和调用方提供的 `payload.files` / `payload.artifact` 元数据会被拒绝。
- 直接 multipart 发布请求限制为 18MB。ClawPack tarball 可以使用 upload-url 流程，最高达到 120MB tarball 上限。
- 可选负载字段：`ownerHandle`。存在时，只有管理员可以代表该所有者发布。
...
存在时，`reason` 会作为 Skills 审核备注存储，并复制到审计日志中。
由所有者发起的软删除会保留该 slug 30 天，之后该 slug 可由另一个发布者认领。
当此过期时间适用时，删除响应包含 `slugReservedUntil`。
...
仅管理员。确保某个 handle 存在一个组织发布者。如果该 handle 仍指向旧版共享用户/个人发布者，该端点会先将其迁移为组织发布者。
...
已认证的自助组织发布者创建。创建新的组织发布者并将调用方添加为所有者。此端点不会迁移现有用户/个人 handle，也不会将发布者标记为受信任/官方。
...
仅管理员。为合法所有者保留根 slug 和包名称，而不发布任何发布版本。包名称会变成没有发布行的私有占位包，因此同一所有者之后可以将真正的 code-plugin 或 bundle-plugin 发布到该名称下。
...
仅管理员。为已验证的替换 GitHub OAuth 主体恢复个人发布者，而不编辑 Convex Auth 账户行。请求必须命名两个不可变的 GitHub 提供商账户 id；可变 handle 仅用作面向操作者的保护检查。

该端点默认采用试运行。应用恢复需要在工作人员独立验证两个 GitHub 主体之间的连续性后，设置 `dryRun: false` 和
`confirmIdentityVerified: true`。当目标用户当前的个人发布者拥有技能、包或 GitHub 技能来源时，恢复会以失败关闭方式拒绝。
恢复还会迁移已恢复发布者的技能、技能 slug 别名、包、包检查器警告以及派生搜索摘要行中的旧版 `ownerUserId` 字段，以便直接所有者路径与新的发布者权限保持一致。已恢复 handle 的有效受保护 handle
预留也会重新分配给替代用户，避免后续个人资料同步恢复前用户的竞争权限。每个主表在每次应用事务中限制为
100 行；更大的恢复必须先使用可恢复的所有者迁移。
GitHub 技能来源按发布者划分范围，并报告为已检查，而不是被重写。
...
在不解封或恢复内容的情况下更改现有封禁的已存储原因（仅限管理员）。除非 `dryRun` 为 `false`，否则默认采用试运行。
