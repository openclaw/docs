...
x-i18n:
    generated_at: "2026-07-05T17:40:10Z"
    model: gpt-5.5
...
旧版 `/api/...` 和 `/api/cli/...` 会保留以实现兼容性（见 `DEPRECATIONS.md`）。
OpenAPI：`/api/v1/openapi.json`。

## 公共目录复用

第三方目录可以使用公共读取端点来列出或搜索 ClawHub skills。请缓存结果，遵守 `429`/`Retry-After`，将用户链接回规范的 ClawHub 列表页（`https://clawhub.ai/<owner>/skills/<slug>`），并避免暗示 ClawHub 认可该第三方网站。不要尝试在公共 API 表面之外镜像隐藏、私有或因审核被阻止的内容。

Web slug 快捷方式会跨注册表系列解析，但 API 客户端应使用读取端点返回的规范 URL，而不是重构路由优先级。
...
- 匿名请求：按 IP 强制执行。
- 已认证请求（有效的 Bearer token）：按用户桶强制执行。
- 如果 token 缺失/无效，行为会回退到按 IP 强制执行。
- 已认证写入端点在服务器知道原因时，不应返回裸露的 `Unauthorized`。缺失 token、无效/已撤销 token，以及已删除/封禁/禁用的账号都应获得可操作的文本，以便 CLI 客户端告知用户被阻止的原因。
...
- 如果没有可用的受信任客户端 IP，匿名请求会使用仅按速率限制类型划分的回退桶。这些回退桶不包括调用方提供的路径、slug、包名、版本、查询字符串或其他工件参数。

## 错误响应

公共 v1 错误响应为纯文本，`content-type: text/plain; charset=utf-8`。
这包括验证失败（`400`）、缺失的公共资源（`404`）、认证和权限失败（`401`/`403`）、速率限制（`429`）以及被阻止的下载。客户端应将响应正文读取为人类可读字符串。未知查询参数会出于兼容性被忽略，但已识别的查询参数如果值无效会返回 `400`。
...
- 相关性强于流行度。精确的 slug 或显示名称 token 匹配，可能排在参与度强很多但匹配更宽泛的结果之前。
- ASCII 文本按单词和标点边界分词。例如，`personal-map` 包含独立的 `map` token，而 `amap-jsapi-skill` 包含 `amap`、`jsapi` 和 `skill`；因此搜索 `map` 时，`personal-map` 的词法匹配会强于 `amap-jsapi-skill`。
- 流行度经过对数缩放并设有上限。当查询文本匹配较弱时，高参与度 skills 的排名可能更低。
- 可疑或隐藏的审核状态可能会根据调用方筛选器和当前审核状态，将 skill 从公共搜索中移除。

发布者可发现性指南：

- 将用户会实际搜索的术语放入显示名称、摘要和标签。只有当独立的 slug token 也是你想保留的稳定身份时，才使用它。
- 不要仅为了追逐某个查询而重命名 slug，除非新 slug 是更好的长期规范名称。旧 slug 会成为重定向别名，但规范 URL、显示的 slug 和未来搜索摘要会使用新 slug。
- 重命名别名会为旧 URL 以及通过注册表解析的安装保留解析能力，但重命名完成索引后，搜索排名会基于规范 skill 元数据。现有统计数据会保留在该 skill 上。
- 如果某个 skill 意外不可见，请先在登录状态下用 `clawhub inspect @owner/slug` 检查审核状态，然后再更改与排名相关的元数据。
...
- 当 `nonSuspiciousOnly=true` 时，基于游标的排序可能在某一页返回少于 `limit` 个项目，因为可疑 skills 会在页面检索后被过滤。
...
- `metadata.os`：skill frontmatter 中声明的 OS 限制（例如 `["macos"]`、`["linux"]`）。未声明则为 `null`。
- `metadata.systems`：Nix 系统目标（例如 `["aarch64-darwin", "x86_64-linux"]`）。未声明则为 `null`。
- 如果 skill 没有平台元数据，`metadata` 为 `null`。
- 仅当 skill 被标记，或所有者正在查看它时，才包含 `moderation`。
...
举报 skill 以供审核员审查。举报是 skill 级别的，可选择链接到某个版本，并会进入 skill 举报队列。
...
`note` 对 `confirmed` 和 `dismissed` 是必需的；将 `status` 设回 `open` 时可以省略。对已分流处理的举报传入 `finalAction: "hide"`，可在同一个可审计工作流中隐藏该 skill。
...
- 需要对技能或插件具有所有者/发布者管理访问权限，或平台审核员/管理员权限。
...
- 只有当所选版本已生成 Skill Card、未被审核判定为恶意软件阻止，并且 ClawScan 验证为 clean 时，`ok` 才为 `true`。
...
- `security` 是顶层 ClawScan/安全判定。自动化应依据 `ok`、`decision`、`reasons` 和 `security.status` 进行判断。
- `security.signals` 包含支持性的扫描器证据，例如 `staticScan`、`virusTotal` 和 `skillSpector`。
- 为了兼容 v1 响应，保留了 `security.signals.dependencyRegistry`，但依赖注册表存在性扫描器已退役，并且该键始终为 `null`。
...
- `security.signals` 仅包含状态级别的支持性证据；如需完整扫描器详细信息，请使用 `/scan` 或 ClawHub security-audit 页面。
- 为了兼容 v1 响应，保留了 `security.signals.dependencyRegistry`，但依赖注册表存在性扫描器已退役，并且该键始终为 `null`。
...
- `category`（可选）：插件类别过滤器。仅当请求限定为插件包（`/api/v1/plugins`、`/api/v1/code-plugins`、`/api/v1/bundle-plugins`，或带有 `family=code-plugin`/`family=bundle-plugin` 的包端点）时受支持。受控类别和旧版 v1 过滤器别名记录在 `GET /api/v1/plugins` 下。
...
- `channel`（可选）：`official`、`community` 或 `private`
- `isOfficial`（可选）：`true` 或 `false`
- `category`（可选）：插件类别过滤器。仅当请求限定为插件包时受支持。受控类别和旧版 v1 过滤器别名记录在 `GET /api/v1/plugins` 下。
...
`trending` 是七天安装/下载排行榜，不使用全时累计总数。
...
- 正文：ZIP 归档。
- 每个导出的技能以 `{publisher}/{slug}/` 为根。
- 托管技能包含最新存储版本文件，并在 `_manifest.json` 中以 `sourceRef: "public-clawhub"` 列出。
- 带有 `clean` 或 `suspicious` 扫描结果的当前 GitHub 支撑技能包含 `_source_handoff.json`，其中包含 `sourceRef: "public-github"`、仓库、提交、路径、内容哈希和归档 URL。它们不包含 ClawHub 托管的源文件。
...
- 单个插件的导出元数据存储在 `__clawhub_export/{family}/{packageName}/plugin_meta.json`。
- `_manifest.json` 始终包含在 ZIP 根目录。
...
- `version.sha256hash` 是面向旧客户端的已弃用兼容性元数据。它对 `/api/v1/packages/{name}/download` 返回的精确 ZIP 字节进行哈希。现代客户端应使用 `version.artifact.sha256`，它标识规范发布工件。
- 当扫描数据存在时，会包含 `version.vtAnalysis`、`version.llmAnalysis` 和 `version.staticScan`。
- 私有包返回 `404`，除非调用方可以读取所属发布者。

### `GET /api/v1/packages/{name}/versions/{version}/security`

返回供安装客户端使用的精确包发布安全和信任摘要。这是 OpenClaw 用于决定解析后的发布版本能否安装的公开消费接口。
...
- `package.name`、`package.displayName` 和 `package.family` 标识解析后的注册表包。
- `release.releaseId`、`release.version` 和 `release.createdAt` 标识已评估的精确发布版本。
- 当发布工件的相关信息已知时，会提供 `release.artifactKind`、`release.artifactSha256`、`release.npmIntegrity`、`release.npmShasum` 和 `release.npmTarballName`。
- `trust.scanStatus` 是由扫描器输入和手动发布审核派生出的有效信任状态。
...
- `trust.stale` 表示信任摘要是根据过时输入计算的，在做出高置信度允许决策前，应将其视为需要刷新。

说明：

- 此端点精确到版本。客户端应在解析出打算安装的包版本后调用它，而不是只在读取最新包元数据后调用。
- 私有包返回 `404`，除非调用方可以读取所属发布者。
- 此端点有意比所有者/版主审核端点更窄。它公开安装决策和公开解释，而不公开报告者身份、报告正文、私有证据或内部审核时间线。
...
- 旧版包版本返回一个 `legacy-zip` 工件和旧版 ZIP `downloadUrl`。
- ClawPack 版本返回一个 `npm-pack` 工件、npm integrity 字段、一个 `tarballUrl`，以及旧版 ZIP 兼容 URL。
...
- 这仅跟踪迁移就绪状态。它不会改变 OpenClaw，也不会生成 ClawPack。
...
报告一个包以供版主审核。报告是包级别的，可选择链接到某个版本。它们会进入审核队列，但本身不会自动隐藏或阻止下载；版主应使用发布审核来批准、隔离或撤销工件。
...
`confirmed` 和 `dismissed` 需要 `note`；将 `status` 设回 `open` 时可以省略。对已确认报告传入 `finalAction: "quarantine"` 或 `finalAction: "revoke"`，可在同一个可审计工作流中应用发布审核。
...
被隔离和撤销的发布会从制品下载路由返回 `403`。每次变更都会写入一条审计日志条目。
...
- 此路由保持仅 ZIP。它不流式传输 ClawPack `.tgz` 文件。
...
下载托管的技能版本 ZIP，或者为当前 GitHub 支持的技能返回 GitHub 源交接；该技能必须具有 `clean` 或 `suspicious` 扫描结果且没有托管版本。
...
- 如果既未提供 `version` 也未提供 `tag`，则使用最新版本。
- 软删除版本返回 `410`。
- GitHub 支持的技能交接不会代理或镜像字节。JSON 响应包含 `sourceRef: "public-github"`、`repo`、`commit`、`path`、`contentHash` 和 `archiveUrl`；扫描/当前状态是门禁，不会作为成功负载元数据包含。
...
- 首选：使用带有 `payload` JSON + `files[]` blob 的 `multipart/form-data`。
- 也接受带有 `files`（基于 storageId）的 JSON 正文。
- 可选负载字段：`ownerHandle`。存在时，API 会在服务器端解析该发布者，并要求操作者具有发布者访问权限。
- 可选负载字段：`migrateOwner`。当它与 `ownerHandle` 一起为 `true` 时，如果操作者同时是当前发布者和目标发布者的管理员/所有者，现有技能可以迁移到该所有者。没有此选择加入时，所有者变更会被拒绝。
...
- 允许的表单字段是 `payload`、重复的 `files` blob，或一个 `clawpack` tarball 引用。`clawpack` 可以是 `.tgz` blob，也可以是 upload-url 流程返回的存储 ID。基于暂存 storage-id 的发布还必须包含该上传 URL 返回的 `clawpackUploadTicket`。
- 使用 `files` 或 `clawpack` 二者之一，不要在同一个请求中同时使用。
...
所有者发起的软删除会保留 slug 30 天，之后该 slug 可由另一个发布者认领。
此过期适用时，删除响应包含 `slugReservedUntil`。
...
已认证的自助式组织发布者创建。创建新的组织发布者，并将调用方添加为所有者。此端点不会迁移现有用户/个人 handle，也不会将发布者标记为受信任/官方。
...
仅管理员。为合法所有者保留根 slug 和包名称，而不发布版本。包名称会成为没有发布行的私有占位包，因此同一所有者稍后可以将真实 code-plugin 或 bundle-plugin 发布发布到该名称。
...
仅管理员。为已验证的替代 GitHub OAuth 主体恢复个人发布者，而不编辑 Convex Auth 账户行。请求必须命名两个不可变 GitHub 提供商账户 ID；可变 handle 仅用作面向操作员的保护。

端点默认执行空运行。应用恢复需要在工作人员独立验证两个 GitHub 身份主体之间的连续性后设置 `dryRun: false` 和
`confirmIdentityVerified: true`。当目标用户当前的个人
发布者拥有技能、包或 GitHub 技能来源时，恢复会以关闭方式失败。
恢复还会迁移已恢复发布者的技能、技能 slug 别名、包、包检查器警告和派生搜索摘要行中的旧版 `ownerUserId` 字段，以便
直接所有者路径与新的发布者权限保持一致。已恢复 handle 的活动受保护 handle 预留也会重新分配给替换用户，因此之后的
...
内容（仅限管理员）。除非 `dryRun` 为 `false`，否则默认为空运行。
