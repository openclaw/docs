...
第三方目录可以使用公共读取端点来列出或搜索 ClawHub Skills。请缓存结果，遵守 `429`/`Retry-After`，将用户链接回规范的 ClawHub 列表页（`https://clawhub.ai/<owner>/<slug>`），并避免暗示 ClawHub 为第三方站点背书。不要试图在公共 API 表面之外镜像隐藏、私有或被审核阻止的内容。

Web slug 快捷方式会跨注册表族解析，但 API 客户端应使用读取端点返回的规范 URL，而不是重构路由优先级。
...
- 当服务器知道原因时，已认证写入端点不应返回裸 `Unauthorized`。缺失 token、无效/已撤销 token，以及已删除/封禁/禁用的账户都应获得可操作文本，以便 CLI 客户端可以告诉用户是什么阻止了他们。

- 读取：每 IP 每分钟 600 次，每 key 每分钟 2400 次
- 写入：每 IP 每分钟 45 次，每 key 每分钟 180 次
- 下载：每 IP 每分钟 30 次，每 key 每分钟 180 次（`/api/v1/download`）
...
- 如果没有可用的受信任客户端 IP，匿名下载请求会使用端点作用域的回退桶，而不是一个全局 `ip:unknown` 桶。匿名读取/写入请求仍使用共享 unknown 桶，因此缺失 IP 的路由仍然可见且保守。
...
- 相关性强于人气。精确的 slug 或显示名称 token 匹配可以排在下载量多得多但匹配更宽松的结果之前。
...
- 下载量用作小幅对数缩放先验和并列排序依据，而不是主要排名信号。当查询文本匹配较弱时，高下载量 Skills 也可能排名较低。
- 可疑或隐藏的审核状态可能会根据调用方筛选器和当前审核状态，从公共搜索中移除某个 Skills。

发布者可发现性指引：

- 将用户会逐字搜索的术语放在显示名称、摘要和标签中。仅当独立 slug token 也是你想保留的稳定身份时才使用它。
- 不要仅仅为了追逐某个查询而重命名 slug，除非新的 slug 是更好的长期规范名称。旧 slug 会变成重定向别名，但规范 URL、显示的 slug 和未来搜索摘要会使用新 slug。
- 重命名别名会保留旧 URL 和通过注册表解析的安装的解析能力，但搜索排名基于重命名完成索引后的规范 Skills 元数据。现有统计数据会随 Skills 保留。
- 如果某个 Skills 意外不可见，请在更改排名相关元数据之前，先登录并使用 `clawhub inspect <slug>` 检查审核状态。
...
- `createdAt` 对新 Skills 抓取是稳定的；`updated` 会在现有 Skills 重新发布时变化。
- 当 `nonSuspiciousOnly=true` 时，基于游标的排序可能在一页返回少于 `limit` 个条目，因为可疑 Skills 会在页面检索后被过滤。
...
- 公共调用方只有对已标记且可见的 Skills 才会获得 `200`。
- 证据会对公共调用方脱敏，并且仅向所有者/审核员包含原始片段。

### `POST /api/v1/skills/{slug}/report`

报告某个 Skills 以供审核员审查。报告是 Skills 级别的，可选链接到某个版本，并会进入 Skills 报告队列。
...
供 Skills 所有者/发布者对某个 Skills 的审核提出申诉的端点。

认证：

- 需要 Skills 所有者或发布者成员的 API token。
...
对于隐藏、移除、可疑、恶意或扫描器标记的 Skills 结果，申诉都会被接受。ClawHub 每个 Skills 保留一个开放申诉。
...
请求对最新发布的 Skills 版本进行安全重新扫描。

认证：

- 需要 Skills 所有者、发布者管理员、平台审核员或平台管理员的 API token。
...
`confirmed` 和 `dismissed` 需要 `note`；将 `status` 设回 `open` 时可以省略。对已分诊报告传入 `finalAction: "hide"`，可在同一个可审计工作流中隐藏该 Skills。

### `GET /api/v1/skills/-/appeals`

供审核员/管理员接收 Skills 申诉的端点。
...
供审核员/管理员接受、拒绝或重新打开 Skills 申诉的端点。
`accepted` 和 `rejected` 需要 `note`；将 `status` 设回 `open` 时可以省略。对已接受的申诉传入 `finalAction: "restore"`，可让该 Skills 重新可用。
...
- `version.security` 在可用时包含规范化的扫描验证状态和扫描器详细信息
  （VirusTotal + LLM）。
...
- `externalService`、`binary`、`osPermission`（可选）：命名环境要求标签的简写
- `artifactKind`（可选）：`legacy-zip` 或 `npm-pack`
- `npmMirror`（可选）：`true`/`1`，用于显示可通过 npm mirror 获取的 ClawPack 支持的包版本
...
- `artifactKind`（可选）：`legacy-zip` 或 `npm-pack`
- `npmMirror`（可选）：`true`/`1`，用于搜索可通过 npm mirror 获取的 ClawPack 支持的包版本
...
- `version.artifact.kind` 对旧式包归档为 `legacy-zip`，对 ClawPack 支持的发布为
  `npm-pack`。
...
- 旧版包版本返回一个 `legacy-zip` 构件和一个旧版 ZIP
  `downloadUrl`。
- ClawPack 版本返回一个 `npm-pack` 构件、npm integrity 字段、一个
...
- 这只跟踪迁移就绪性。它不会改变 OpenClaw，也不会生成
  ClawPack。
...
举报一个包以供审核员审查。举报是包级别的，可选择关联到某个版本。它们会进入审核队列，但本身不会自动隐藏或阻止下载；审核员应使用发布审核来批准、隔离或撤销构件。
...
只有已隔离、已撤销、可疑或恶意的发布才接受申诉。ClawHub 为每个发布保留一个开放申诉。
...
- 需要包所有者、发布者管理员、平台版主或平台管理员的 API 令牌。
- 所有者和发布者管理员受每个发布版本的所有者恢复限制约束。平台版主和管理员不受此限制，但 ClawHub 仍然只允许每个发布版本有一个活动的重新扫描。
...
当 `accepted` 和 `rejected` 时必须提供 `note`；将 `status` 设回 `open` 时可以省略。对已接受的申诉传入 `finalAction: "approve"`，可在同一个可审计工作流中批准受影响的发布版本。
...
当 `confirmed` 和 `dismissed` 时必须提供 `note`；将 `status` 设回 `open` 时可以省略。对已确认的举报传入 `finalAction: "quarantine"` 或 `finalAction: "revoke"`，可在同一个可审计工作流中应用发布版本审核。
...
- `revoked`：此前受信任的发布版本被阻止。

隔离和吊销的发布版本会从制品下载路由返回 `403`。每次更改都会写入一条审计日志条目。

### `POST /api/v1/packages/backfill/artifacts`

仅限管理员的维护端点，用于为较旧的包发布版本标记显式的制品类型元数据。
...
为 ClawPack 支持的包版本返回 npm 兼容 packument。
...
- Scoped 包 packument 同时支持 `/api/npm/@scope/name` 和 npm 的编码请求路径 `/api/npm/@scope%2Fname`。

### `GET /api/npm/{package}/-/{tarball}.tgz`

为 npm 镜像客户端流式传输精确的已上传 ClawPack tarball 字节。
...
- 可选 payload 字段：`ownerHandle`。存在时，API 会在服务器端解析该发布者，并要求操作者具有发布者访问权限。
- 可选 payload 字段：`migrateOwner`。当与 `ownerHandle` 一起为 `true` 时，如果操作者在当前和目标发布者上都是管理员/所有者，现有技能可以移动到该所有者。没有此选择加入时，所有者更改会被拒绝。
...
所有者发起的软删除会将 slug 保留 30 天，之后该 slug 可由其他发布者认领。
当此过期适用时，删除响应会包含 `slugReservedUntil`。
...
仅限管理员。确保某个 handle 的组织发布者存在。如果该 handle 仍指向旧版共享用户/个人发布者，该端点会先将其迁移为组织发布者。
...
仅限管理员。为合法所有者保留根 slug 和包名，而不发布版本。包名会变成没有发布记录行的私有占位包，因此同一所有者以后可以将真正的 code-plugin 或 bundle-plugin 版本发布到该名称下。
