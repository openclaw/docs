...
第三方目录可以使用公共读取端点列出或搜索 ClawHub 技能。请缓存结果，遵守 `429`/`Retry-After`，将用户链接回规范的 ClawHub 列表页（`https://clawhub.ai/<owner>/skills/<slug>`），并避免暗示 ClawHub 为第三方网站背书。不要尝试在公共 API 表面之外镜像隐藏、私有或被审核拦截的内容。
...
- 当服务器知道原因时，已认证写入端点不应返回裸 `Unauthorized`。缺失令牌、无效/已撤销令牌，以及已删除/封禁/禁用的账号都应得到可操作的文本，以便 CLI 客户端可以告诉用户阻止原因。
...
- `X-RateLimit-Remaining` / `RateLimit-Remaining`：存在时为精确剩余配额。
...
- 如果没有可用的受信任客户端 IP，匿名请求会使用仅按速率限制类型限定范围的回退桶。这些回退桶不包含调用方提供的路径、slug、包名、版本、查询字符串或其他构件参数。

## 错误响应

公共 v1 错误响应是纯文本，`content-type: text/plain; charset=utf-8`。
这包括验证失败（`400`）、缺失公共资源（`404`）、认证和权限失败（`401`/`403`）、速率限制（`429`）以及被拦截的下载。客户端应将响应正文作为人类可读字符串读取。未知查询参数会被忽略以保持兼容性，但已识别的查询参数如果值无效会返回 `400`。
...
- 结果按相关性顺序返回（嵌入相似度 + 精确 slug/name 词元加权 + 少量流行度先验）。
- 相关性强于流行度。精确的 slug 或显示名称词元匹配，可以排在互动强得多但匹配更宽泛的结果之前。
...
- 重命名别名会保留旧 URL 和通过注册表解析的安装项的解析能力，但搜索排名基于重命名被索引后的规范技能元数据。现有统计数据会保留在该技能上。
...
- 当 `nonSuspiciousOnly=true` 时，基于游标的排序可能在单页返回少于 `limit` 个条目，因为可疑技能是在页面取回后过滤的。
...
- 证据会为公共调用方进行脱敏，并且仅对所有者/审核员包含原始片段。
...
`note` 对 `confirmed` 和 `dismissed` 是必需的；将 `status` 设置回 `open` 时可以省略。将 `finalAction: "hide"` 与已分流的报告一起传入，可在同一可审计工作流中隐藏该技能。
...
- 扫描作业是异步的。手动扫描请求优先级高于普通发布/回填工作，但完成时间仍取决于 worker 可用性。
...
- 只有当所选版本具有已生成的技能卡、未被审核标记为恶意软件阻止，并且 ClawScan 验证为干净时，`ok` 才为 `true`。
...
- 当你需要单技能技能卡验证信封时，请使用 `/verify`；当你需要已生成的卡片 Markdown 时，请使用 `/card`；当你需要详细扫描器数据时，请使用 `/scan`。
...
- 匿名调用者只能看到公开包频道。
- 经过身份验证的调用者可以在列表/搜索结果中看到他们所属发布者的私有包。
- `channel=private` 只返回经过身份验证的调用者可读取的包。

### `GET /api/v1/packages/search`

在 Skills 和插件包之间进行统一目录搜索。
...
- ClawPack 发布版本包含兼容 npm 的 `npmIntegrity`、`npmShasum` 和
  `npmTarballName` 字段。
- `version.sha256hash` 是面向旧客户端的已弃用兼容性元数据。它会
...
返回供安装客户端使用的精确包发布安全性和信任摘要。这是公开的 OpenClaw 消费接口，用于决定某个
已解析发布版本是否可以安装。
...
- `release.artifactKind`、`release.artifactSha256`、`release.npmIntegrity`、
  `release.npmShasum` 和 `release.npmTarballName` 在发布制品已知时存在。
...
- `trust.stale` 表示信任摘要是根据过期输入计算的，在做出高置信度允许决策前
  应视为需要刷新。
...
- 私有包会返回 `404`，除非调用方可以读取所属发布者。
- 此端点刻意比所有者/版主审核
  端点更窄。它公开安装决策和公开解释，而不是
  举报者身份、报告正文、私有证据或内部审核
  时间线。
...
- `packageName` 会按 npm 名称规范化；对于计划中的
  迁移，包可以缺失。
- 这只跟踪迁移就绪状态。它不会改变 OpenClaw，也不会生成
  ClawPack。
...
`note` 对 `confirmed` 和 `dismissed` 是必填项；将 `status` 设回 `open` 时可以省略。对已确认的举报传入 `finalAction: "quarantine"` 或 `finalAction: "revoke"`，可在同一个可审计工作流中应用发布版本审核。
...
- `revoked`：在某个发布版本先前受信任后阻止。

被隔离和被撤销的发布版本会从制品下载路由返回 `403`。
每次更改都会写入一条审计日志条目。
...
下载托管的 Skills 版本 ZIP，或者对当前由 GitHub 支持且扫描结果为 `clean` 或 `suspicious`、没有托管版本的 Skills 返回 GitHub 源码交接。
...
- 软删除的版本返回 `410`。
- 由 GitHub 支持的 Skills 交接不会代理或镜像字节。JSON 响应包含 `sourceRef: "public-github"`、`repo`、`commit`、`path`、`contentHash` 和 `archiveUrl`；扫描/当前状态是门禁，不会作为成功载荷元数据包含。
...
- 可选载荷字段：`migrateOwner`。当它与 `ownerHandle` 一起为 `true` 时，如果操作者是当前和目标发布者双方的管理员/所有者，现有 Skills 可以移动到该所有者。没有这个显式选择时，所有者更改会被拒绝。
...
- 使用 `files` 或 `clawpack` 其中之一，绝不要在同一个请求中同时使用两者。
...
所有者发起的软删除会保留 slug 30 天，之后该 slug 可由另一发布者认领。
...
仅限管理员。确保某个 handle 存在组织发布者。如果该 handle 仍指向旧版共享用户/个人发布者，该端点会先将其迁移为组织发布者。
对于新创建的组织，请提供 `memberHandle`；执行操作的管理员不会作为成员添加。
...
已认证的自助组织发布者创建。创建新的组织发布者，并将调用者添加为所有者。此端点不会迁移现有用户/个人 handle，也不会将该发布者标记为受信任/官方。
...
仅限管理员。为合法所有者保留根 slug 和软件包名称，而不发布版本。软件包名称会变成没有发布行的私有占位软件包，因此同一所有者之后可以将真正的 code-plugin 或 bundle-plugin 发布版本发布到该名称下。
...
仅限管理员。为已验证的替代 GitHub OAuth principal 恢复个人发布者，而不编辑 Convex Auth 账号行。请求必须命名两个不可变 GitHub 提供商账号 ID；可变 handle 仅用作面向操作者的防护。

端点默认采用试运行。应用恢复需要在工作人员独立验证两个 GitHub 主体之间的连续性后，设置 `dryRun: false` 和
`confirmIdentityVerified: true`。当目标用户当前的个人发布者拥有技能、包或 GitHub 技能源时，
恢复会以关闭方式失败。
恢复还会迁移已恢复发布者的技能、技能 slug 别名、包、包检查器警告和派生搜索摘要行中的旧版 `ownerUserId` 字段，以便
直接所有者路径与新的发布者权限保持一致。已恢复 handle 的活动受保护 handle
保留项也会重新分配给替代用户，防止后续个人资料同步恢复前用户的竞争权限。每个主表在每次应用事务中限制为
100 行；更大的恢复必须先使用可恢复的所有者迁移。
GitHub 技能源按发布者作用域处理，并报告为已检查，而不是重写。
...
`POST /api/cli/upload-url` 返回 `uploadUrl` 和 `uploadTicket`。暂存 ClawPack tarball 的包发布
必须将生成的存储 id 作为 `clawpack` 发送，并将返回的票据作为 `clawpackUploadTicket` 发送。
