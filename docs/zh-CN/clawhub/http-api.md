...
x-i18n:
    generated_at: "2026-07-01T02:57:03Z"
    model: gpt-5.5
...
- 当服务器知道原因时，已认证写入端点不应返回裸的 `Unauthorized`。缺失 token、无效/已撤销 token，以及已删除/封禁/禁用账户，都应获得可操作的文本，以便 CLI 客户端能告知用户是什么阻止了操作。
...
- 如果没有可用的受信任客户端 IP，匿名请求会使用仅按速率限制类型限定作用域的回退桶。这些回退桶不包含调用方提供的路径、slug、包名、版本、查询字符串或其他工件参数。

## 错误响应

公共 v1 错误响应是纯文本，带有 `content-type: text/plain; charset=utf-8`。
这包括校验失败（`400`）、缺失的公共资源（`404`）、认证和权限失败（`401`/`403`）、速率限制（`429`）以及被阻止的下载。客户端应将响应正文读取为人类可读字符串。未知查询参数会被忽略以兼容，但已识别查询参数的无效值会返回 `400`。
...
- 结果按相关性顺序返回（嵌入相似度 + 精确 slug/名称 token 加权 + 少量人气先验）。
- 相关性强于人气。精确的 slug 或显示名称 token 匹配，可能会排在参与度高得多但匹配更宽松的结果之前。
...
- 将用户会逐字搜索的术语放入显示名称、摘要和标签中。仅当独立的 slug token 也是你想保留的稳定身份时，才使用它。
- 不要仅仅为了追逐某个查询而重命名 slug，除非新的 slug 是更好的长期规范名称。旧 slug 会成为重定向别名，但规范 URL、显示的 slug 和未来搜索摘要会使用新的 slug。
- 重命名别名会保留旧 URL 以及通过注册表解析的安装的解析能力，但搜索排名基于重命名已索引后的规范技能元数据。现有统计数据会保留在该技能上。
- 如果某个技能意外不可见，请先在登录状态下用 `clawhub inspect @owner/slug` 检查审核状态，然后再更改与排名相关的元数据。
...
- 当 `nonSuspiciousOnly=true` 时，基于游标的排序可能会在一页中返回少于 `limit` 的项目，因为可疑技能是在页面检索后过滤的。
...
- 仅当技能被标记或所有者正在查看它时，才会包含 `moderation`。
...
- 对公共调用方会隐去证据，并且仅对所有者/审核员包含原始片段。

### `POST /api/v1/skills/{slug}/report`

报告技能以供审核员审查。报告是技能级别的，可选链接到某个版本，并会进入技能报告队列。
...
`note` 对 `confirmed` 和 `dismissed` 是必需的；当将 `status` 设置回 `open` 时可以省略。传递 `finalAction: "hide"` 以及已分诊报告，可在同一可审计工作流中隐藏该技能。
...
- 扫描作业是异步的。手动扫描请求会优先于常规发布/回填工作，但完成仍取决于工作器可用性。
...
- 返回排队/运行中/成功/失败状态。
- 排队期间返回 `queue.queuedAhead` 和 `queue.position`，以便客户端显示该请求之前还有多少个优先级更高的手动扫描。非常大的队列会被限制，并以 `queuedAheadIsEstimate: true` 报告。
...
- 只有当所选版本已生成技能卡、未被审核标记为恶意软件阻止，并且 ClawScan 验证为干净时，`ok` 才为 `true`。
...
- 响应仅包含安全信息。它不包含技能卡数据、生成卡片状态、构件文件列表或详细扫描器载荷。
...
- 当前由 GitHub 支持且扫描结果为 `clean` 或 `suspicious` 的技能会包含
...
- 对于旧式包归档，`version.artifact.kind` 为 `legacy-zip`；对于由 ClawPack 支持的发布版本，为 `npm-pack`。
- ClawPack 发布版本包含与 npm 兼容的 `npmIntegrity`、`npmShasum` 和 `npmTarballName` 字段。
- `version.sha256hash` 是面向旧客户端的已弃用兼容性元数据。它对 `/api/v1/packages/{name}/download` 返回的精确 ZIP 字节进行哈希。现代客户端应使用 `version.artifact.sha256`，它标识规范发布制品。
- 当存在扫描数据时，会包含 `version.vtAnalysis`、`version.llmAnalysis` 和 `version.staticScan`。
- 私有包返回 `404`，除非调用方可以读取所属发布者。

### `GET /api/v1/packages/{name}/versions/{version}/security`

返回供安装客户端使用的精确包发布安全与信任摘要。这是公开的 OpenClaw 消费接口，用于决定某个已解析发布版本是否可以安装。
...
- `trust.blockedFromDownload` 是安装阻止信号。OpenClaw 和其他安装客户端应在该值为 `true` 时阻止安装，而不是从扫描器或审核字段重新推导阻止规则。
...
- 此端点精确到版本。客户端应在解析出准备安装的包版本后调用它，而不只是读取最新包元数据后调用。
- 私有包返回 `404`，除非调用方可以读取所属发布者。
- 此端点有意比所有者/版主审核端点更窄。它公开的是安装决策和公开说明，而不是举报者身份、举报正文、私有证据或内部审核时间线。
...
- 这只跟踪迁移就绪状态。它不会变更 OpenClaw，也不会生成 ClawPacks。
...
举报一个包以供版主审核。举报是包级别的，可选择关联到某个版本。它们会进入审核队列，但本身不会自动隐藏或阻止下载；版主应使用发布审核来批准、隔离或撤销制品。
...
隔离和撤销的发布会从制品下载路由返回 `403`。每次变更都会写入一条审计日志条目。
...
- 带作用域的包 packument 同时支持 `/api/npm/@scope/name` 和 npm 的编码请求路径 `/api/npm/@scope%2Fname`。
...
下载托管的技能版本 ZIP，或者在当前由 GitHub 支持的技能具有 `clean` 或 `suspicious` 扫描结果且没有托管版本时，返回 GitHub 源码交接信息。
...
- 软删除版本返回 `410`。
- 由 GitHub 支持的技能交接不会代理或镜像字节。JSON 响应包含 `sourceRef: "public-github"`、`repo`、`commit`、`path`、`contentHash` 和 `archiveUrl`；扫描/当前状态是门禁条件，不会作为成功载荷元数据包含。
...
- 允许的表单字段是 `payload`、重复的 `files` blob，或一个 `clawpack` tarball 引用。`clawpack` 可以是 `.tgz` blob，也可以是 upload-url 流程返回的存储 id。使用暂存 storage-id 发布时，还必须包含随该上传 URL 返回的 `clawpackUploadTicket`。
- 同一个请求中只能使用 `files` 或 `clawpack`，不能同时使用二者。
- JSON 正文以及调用方提供的 `payload.files` / `payload.artifact` 元数据会被拒绝。
- 直接 multipart 发布请求上限为 18MB。ClawPack tarball 可以通过 upload-url 流程使用，最高达到 120MB tarball 上限。
...
- 代发布仍会根据目标所有者账户验证 official-channel 资格。
...
存在时，`reason` 会存储为技能审核备注，并复制到审计日志中。所有者发起的软删除会保留 slug 30 天，之后该 slug 可由另一位发布者认领。删除响应在此过期适用时包含 `slugReservedUntil`。版主/管理员隐藏和安全移除不会以这种方式过期。
...
仅管理员。确保某个 handle 存在组织发布者。如果该 handle 仍指向旧版共享用户/个人发布者，端点会先将它迁移为组织发布者。对于新创建的组织，请提供 `memberHandle`；当前管理员不会被添加为成员。`memberRole` 默认为 `owner`。
...
仅管理员。为合法所有者保留根 slug 和包名，而不发布版本。包名会变成没有发布行的私有占位包，因此同一所有者之后可以将真正的 code-plugin 或 bundle-plugin 发布到该名称。
...
仅管理员。为经过验证的替代 GitHub OAuth 主体恢复个人发布者，而不编辑 Convex Auth 账户行。请求必须指定两个不可变的 GitHub provider account id；可变 handle 仅用作面向操作者的保护条件。

端点默认使用空运行。应用恢复需要在工作人员独立验证两个
GitHub 主体之间的连续性后设置 `dryRun: false` 和
`confirmIdentityVerified: true`。当目标用户当前的个人
发布者拥有 Skills、包或 GitHub Skills 源时，恢复会失败并保持关闭。
恢复还会迁移已恢复发布者的 Skills、Skills slug 别名、包、包检查器警告以及派生搜索摘要行中的旧版 `ownerUserId` 字段，以便
直接所有者路径与新的发布者权限保持一致。已恢复 handle 的有效受保护 handle
保留也会重新分配给替换用户，防止后续
个人资料同步恢复前用户的竞争权限。每个主表在每次应用事务中限制为
100 行；更大的恢复必须先使用可恢复的所有者迁移。
GitHub Skills 源按发布者限定作用域，并报告为已检查，而不是被重写。
...
- 两个端点都需要 API 令牌认证，并且仅适用于 Skills 所有者。
...
更改现有封禁存储的原因，而不解除封禁或恢复
内容（仅限管理员）。除非 `dryRun` 为 `false`，否则默认使用空运行。
...
移除计划请参见 `DEPRECATIONS.md`。

`POST /api/cli/upload-url` 会返回 `uploadUrl` 和 `uploadTicket`。暂存 ClawPack tarball 的包
发布必须将生成的存储 id 作为
