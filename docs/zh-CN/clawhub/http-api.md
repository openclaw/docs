...
第三方目录可以使用公共读取端点来列出或搜索 ClawHub Skills。请缓存结果，遵守 `429`/`Retry-After`，将用户链接回规范的 ClawHub 列表页（`https://clawhub.ai/<owner>/skills/<slug>`），并避免暗示 ClawHub 为该第三方站点背书。不要尝试在公共 API 表面之外镜像隐藏、私有或被审核阻止的内容。
...
- 当服务器知道原因时，已认证写入端点不应返回裸露的 `Unauthorized`。缺失令牌、无效/已吊销令牌以及已删除/封禁/禁用账号都应获得可操作的文本，以便 CLI 客户端能告诉用户是什么阻止了他们。
...
- 如果没有可用的受信任客户端 IP，匿名请求会使用仅按速率限制种类限定范围的回退桶。这些回退桶不包括调用方提供的路径、slug、包名、版本、查询字符串或其他构件参数。

## 错误响应

公共 v1 错误响应是纯文本，带有 `content-type: text/plain; charset=utf-8`。
这包括验证失败（`400`）、缺失公共资源（`404`）、认证和权限失败（`401`/`403`）、速率限制（`429`）以及被阻止的下载。客户端应将响应正文读取为人类可读字符串。未知查询参数会被忽略以保持兼容性，但已识别查询参数的无效值会返回 `400`。
...
- 相关性比流行度更强。精确的 slug 或显示名称 token 匹配可以排在互动量强得多但匹配更宽松的结果之前。
...
- 流行度按对数缩放并设有上限。当查询文本匹配较弱时，高互动量 Skills 仍可能排名更低。
- 可疑或隐藏的审核状态可能会根据调用方筛选器和当前审核状态，将某个 Skill 从公共搜索中移除。

发布者可发现性指南：

- 将用户会逐字搜索的术语放入显示名称、摘要和标签中。只有当独立 slug token 也是你想保留的稳定身份时，才使用它。
- 不要仅为了追逐某个查询而重命名 slug，除非新 slug 是更好的长期规范名称。旧 slug 会成为重定向别名，但规范 URL、显示的 slug 和未来的搜索摘要会使用新 slug。
- 重命名别名会为旧 URL 以及通过注册表解析的安装保留解析能力，但重命名完成索引后，搜索排名基于规范 Skill 元数据。现有统计数据会留在该 Skill 上。
- 如果某个 Skill 意外不可见，请在更改排名相关元数据前，先在登录状态下使用 `clawhub inspect @owner/slug` 检查审核状态。
...
- 当 `nonSuspiciousOnly=true` 时，基于游标的排序可能在某页返回少于 `limit` 的项目，因为可疑 Skills 会在页面检索后被过滤。
...
报告一个 Skill 以供审核员审查。报告是 Skill 级别，可选链接到某个版本，并进入 Skill 报告队列。
...
`note` 对 `confirmed` 和 `dismissed` 是必需的；将 `status` 设置回 `open` 时可以省略。对已分诊报告传入 `finalAction: "hide"`，可在同一个可审计工作流中隐藏该 Skill。
...
- `version.security` 包括规范化的扫描验证状态和扫描器详情（VirusTotal + LLM），如果可用。
...
- 扫描作业是异步的。手动扫描请求优先于普通发布/回填工作，但完成时间仍取决于 worker 可用性。
...
- 排队期间返回 `queue.queuedAhead` 和 `queue.position`，以便客户端显示该请求前面有多少个优先级更高的手动扫描。非常大的队列会被限定并以 `queuedAheadIsEstimate: true` 报告。
...
- 需要对技能或插件拥有所有者/发布者管理访问权限，或拥有平台版主/管理员权限。
...
返回 `clawhub skill verify` 使用的 Skill Card 验证封套。
...
- 技能身份、发布者身份和所选版本元数据都是顶层封套字段（`slug`、`displayName`、`publisherHandle`、`version`、`resolvedFrom`、`tag`、`createdAt`），因此 shell 自动化无需解包嵌套包装器即可读取它们。
...
- 结果按项目返回；某一个技能或版本缺失不会导致整个响应失败。
- 响应仅包含安全信息。不包含 Skill Card 数据、已生成卡片状态、工件文件列表或详细扫描器载荷。
...
- 当你需要单个技能的 Skill Card 验证封套时使用 `/verify`；需要已生成的卡片 markdown 时使用 `/card`；需要详细扫描器数据时使用 `/scan`。
...
- `channel`（可选）：`official`、`community` 或 `private`
- `isOfficial`（可选）：`true` 或 `false`
- `category`（可选）：插件类别过滤器。仅当请求范围限定为插件包时受支持。受控类别和旧版 v1
...
- 托管技能包含最新存储版本文件，并列在
  `_manifest.json` 中，且带有 `sourceRef: "public-clawhub"`。
...
返回面向安装客户端的确切包发布安全性和信任摘要。这是用于判断
已解析发布是否可安装的公开 OpenClaw 消费入口。
...
- `trust.scanStatus` 是从扫描器输入
  和人工发布审核派生出的有效信任状态。
- `trust.moderationState` 可为空。当不存在人工发布
  审核时，它为 `null`。
...
- `trust.stale` 表示信任摘要是根据过时输入计算得出的，
  在做出高置信度允许决策前应视为需要刷新。

说明：

- 此端点精确到版本。客户端应在解析出
  打算安装的包版本后调用它，而不仅仅是在读取最新
  包元数据后调用。
- 私有包返回 `404`，除非调用方可以读取所属发布者。
- 此端点有意比所有者/审核员审核
...
- `open`：可疑、恶意、待处理、隔离、撤销或已举报的发布。
- `blocked`：隔离、撤销或恶意的发布。
- `manual`：任何带有人工审核覆盖的发布。
- `all`：任何带有人工覆盖、非干净扫描状态或包举报的发布。
...
对于 `confirmed` 和 `dismissed`，必须提供 `note`；将 `status` 设回 `open` 时可以省略。传入 `finalAction: "quarantine"` 或 `finalAction: "revoke"`，可在确认的报告上以同一个可审计工作流应用发布审核操作。
...
隔离和撤销的发布版本会从工件下载路由返回 `403`。每次变更都会写入审计日志条目。
...
- 插件/包归档是带有 `package/` 根目录的 zip 文件，因此旧版 OpenClaw 客户端仍可继续工作。
...
下载托管 Skills 版本 ZIP，或为当前由 GitHub 支撑、扫描结果为 `clean` 或 `suspicious` 且没有托管版本的 Skills 返回 GitHub 源交接信息。
...
- 软删除版本返回 `410`。
- 由 GitHub 支撑的 Skills 交接不会代理或镜像字节。JSON 响应包含 `sourceRef: "public-github"`、`repo`、`commit`、`path`、`contentHash` 和 `archiveUrl`；扫描/当前状态是门禁，不会作为成功载荷元数据包含。
...
- 可选载荷字段：`migrateOwner`。当它与 `ownerHandle` 一起为 `true` 时，如果操作者是当前和目标发布者双方的管理员/所有者，现有 Skills 可以移动到该所有者。没有此显式选择时，所有者变更会被拒绝。
...
- 允许的表单字段是 `payload`、重复的 `files` blob，或一个 `clawpack` tarball 引用。`clawpack` 可以是 `.tgz` blob，也可以是上传 URL 流程返回的存储 ID。使用暂存存储 ID 发布时，还必须包含该上传 URL 返回的 `clawpackUploadTicket`。
- 同一个请求中只能使用 `files` 或 `clawpack`，不能同时使用。
- JSON 正文以及调用方提供的 `payload.files` / `payload.artifact` 元数据会被拒绝。
- 直接 multipart 发布请求上限为 18MB。ClawPack tarball 可使用上传 URL 流程，最高可达 120MB tarball 上限。
...
- 代表他人发布时，仍会针对目标所有者账户验证官方频道资格。

### `DELETE /api/v1/skills/{slug}` / `POST /api/v1/skills/{slug}/undelete`

软删除/恢复 Skills（所有者、审核员或管理员）。
...
所有者发起的软删除会保留该 slug 30 天，之后另一个发布者可以声明该 slug。
...
- 当该 handle 已被发布者、用户或个人发布者使用时返回 `409`。

### `POST /api/v1/users/reserve`

仅限管理员。为合法所有者保留根 slug 和包名，而不发布任何发布版本。包名会变成没有发布版本行的私有占位包，因此同一个所有者之后可以将真正的 code-plugin 或 bundle-plugin 发布版本发布到该名称下。
...
仅限管理员。为已验证的替代 GitHub OAuth 主体恢复个人发布者，而不编辑 Convex Auth 账户行。请求必须指定两个不可变 GitHub 提供商账户 ID；可变 handle 仅用作面向操作者的保护校验。
...
恢复还会迁移已恢复发布者的技能、
技能 slug 别名、包、包检查器警告和派生搜索摘要行中的旧版 `ownerUserId` 字段，以便
直接所有者路径与新的发布者权限保持一致。已恢复 handle 的有效受保护 handle
预留也会重新分配给替代用户，这样后续的
个人资料同步无法恢复原用户的竞争权限。每个主表在每个应用事务中限制为
100 行；更大的恢复必须先使用可恢复的所有者迁移。
GitHub 技能源以发布者为作用域，并报告为已检查，而不是被重写。
