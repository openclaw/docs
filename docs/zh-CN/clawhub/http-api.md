...
x-i18n:
    generated_at: "2026-07-03T23:25:33Z"
    model: gpt-5.5
...
旧版 `/api/...` 和 `/api/cli/...` 为兼容性保留（参见 `DEPRECATIONS.md`）。
OpenAPI：`/api/v1/openapi.json`。

## 公共目录复用

第三方目录可以使用公共读取端点列出或搜索 ClawHub 技能。请缓存结果，遵守 `429`/`Retry-After`，将用户链接回规范的 ClawHub 列表页（`https://clawhub.ai/<owner>/skills/<slug>`），并避免暗示 ClawHub 为该第三方站点背书。不要尝试在公共 API 表面之外镜像隐藏、私有或被审核阻止的内容。
...
- 当服务器知道原因时，已认证写入端点不应返回裸露的 `Unauthorized`。缺失令牌、无效/已吊销令牌，以及已删除/封禁/禁用的账户都应获得可操作的文本，以便 CLI 客户端能告诉用户是什么阻止了它们。
...
- `Retry-After`：在 `429` 上，重试前等待的秒数（延迟）
...
- 如果没有可用的受信任客户端 IP，匿名请求会使用仅按速率限制类型划分范围的回退桶。这些回退桶不包含调用者提供的路径、slug、包名、版本、查询字符串或其他产物参数。

## 错误响应

公共 v1 错误响应是纯文本，带有 `content-type: text/plain; charset=utf-8`。
这包括验证失败（`400`）、缺失的公共资源（`404`）、认证和权限失败（`401`/`403`）、速率限制（`429`）以及被阻止的下载。客户端应将响应正文读取为人类可读字符串。未知查询参数会因兼容性被忽略，但具有无效值的已识别查询参数会返回 `400`。
...
- 结果按相关性顺序返回（嵌入相似度 + 精确 slug/名称词元加权 + 少量流行度先验）。
- 相关性强于流行度。精确的 slug 或显示名称词元匹配可以排在参与度强得多但匹配更宽松的结果之前。
- ASCII 文本会按单词和标点边界进行词元化。例如，`personal-map` 包含独立的 `map` 词元，而 `amap-jsapi-skill` 包含 `amap`、`jsapi` 和 `skill`；因此搜索 `map` 会让 `personal-map` 获得比 `amap-jsapi-skill` 更强的词法匹配。
- 流行度采用对数缩放并设有上限。当查询文本匹配较弱时，高参与度技能可能排名更低。
- 可疑或隐藏的审核状态可能会根据调用方筛选器和当前审核状态，从公共搜索中移除技能。

发布者可发现性指南：

- 将用户会按字面搜索的术语放入显示名称、摘要和标签中。仅当独立 slug 词元也是你想保留的稳定身份时才使用它。
- 不要仅为了追逐某个查询而重命名 slug，除非新 slug 是更好的长期规范名称。旧 slug 会变成重定向别名，但规范 URL、显示的 slug 和未来搜索摘要会使用新 slug。
- 重命名别名会为通过注册表解析的旧 URL 和安装保留解析能力，但搜索排名基于重命名已索引后的规范技能元数据。现有统计会保留在该技能上。
- 如果某个技能意外不可见，请在登录状态下先用 `clawhub inspect @owner/slug` 检查审核状态，再更改与排名相关的元数据。
...
- 当 `nonSuspiciousOnly=true` 时，基于游标的排序可能在单页返回少于 `limit` 个项目，因为可疑技能会在页面检索后被过滤。
...
- 证据会对公共调用方进行脱敏，并且仅为所有者/审核员包含原始片段。

### `POST /api/v1/skills/{slug}/report`

举报技能以供审核员审查。举报属于技能级别，可选择链接到某个版本，并会进入技能举报队列。
...
`note` 对 `confirmed` 和 `dismissed` 是必需的；将 `status` 设回 `open` 时可以省略。传入 `finalAction: "hide"` 和已分诊的举报，可在同一个可审计工作流中隐藏该技能。
...
- 只有当所选版本有生成的 Skill Card、未被审核标记为恶意软件阻止，并且 ClawScan 验证为 clean 时，`ok` 才为 `true`。
...
- `security` 是顶层 ClawScan/security 结论。自动化应基于 `ok`、`decision`、`reasons` 和 `security.status` 判断。
...
- 响应仅包含安全信息。它不包含 Skill Card 数据、已生成卡片状态、制品文件列表或详细扫描器载荷。
...
- 当你需要单技能 Skill Card 验证信封时使用 `/verify`，需要已生成卡片 Markdown 时使用 `/card`，需要详细扫描器数据时使用 `/scan`。
...
- `category`（可选）：插件类别过滤器。仅当请求范围限定为插件包（`/api/v1/plugins`、`/api/v1/code-plugins`、`/api/v1/bundle-plugins`，或带有 `family=code-plugin`/`family=bundle-plugin` 的包端点）时受支持。受控类别和旧版 v1 过滤器别名记录在 `GET /api/v1/plugins` 下。
...
- `channel`（可选）：`official`、`community` 或 `private`
- `isOfficial`（可选）：`true` 或 `false`
- `category`（可选）：插件类别过滤器。仅当请求范围限定为插件包时受支持。受控类别和旧版 v1 过滤器别名记录在 `GET /api/v1/plugins` 下。
...
- ClawPack 发布版本包含兼容 npm 的 `npmIntegrity`、`npmShasum` 和
  `npmTarballName` 字段。
- `version.sha256hash` 是面向旧客户端的已弃用兼容性元数据。它会哈希 `/api/v1/packages/{name}/download` 返回的精确 ZIP 字节。
...
返回面向安装客户端的精确包发布安全和信任摘要。这是 OpenClaw 用来决定已解析发布版本是否可安装的公开消费表面。
...
- `trust.stale` 表示信任摘要是根据过时输入计算的，在做出高置信度允许决策之前应视为需要刷新。

说明：

- 此端点精确到版本。客户端应在解析出计划安装的包版本后调用它，而不只是读取最新包元数据后调用。
- 私有包返回 `404`，除非调用方可以读取所属发布者。
- 此端点有意比所有者/版主审核端点更窄。它公开安装决策和公开说明，而不公开报告者身份、报告正文、私有证据或内部审查时间线。
...
- `manual`：带有手动审核覆盖的任何发布版本。
- `all`：带有手动覆盖、非干净扫描状态或包报告的任何发布版本。
...
报告一个包以供版主审查。报告是包级别的，可选链接到一个版本。它们会进入审核队列，但本身不会自动隐藏或阻止下载；版主应使用发布版本审核来批准、隔离或撤销构件。
...
对于 `confirmed` 和 `dismissed`，`note` 是必填项；将 `status` 设回 `open` 时可以省略。对已确认的报告传入 `finalAction: "quarantine"` 或 `finalAction: "revoke"`，可在同一个可审计工作流中应用发布审核。
...
- `revoked`：在某个版本先前被信任后阻止。

被隔离和撤销的版本会在制品下载路由返回 `403`。每次变更都会写入一条审计日志。
...
- 二进制文件返回 `415`。
- 文件大小限制：200KB。
- 待处理的 VirusTotal 扫描不会阻止读取；恶意版本仍可能在其他位置被拒绝提供。
...
下载托管的技能版本 ZIP，或为当前由 GitHub 支持且扫描结果为 `clean` 或 `suspicious`、没有托管版本的技能返回 GitHub 源码交接。
...
- 软删除的版本返回 `410`。
- 由 GitHub 支持的技能交接不会代理或镜像字节。JSON 响应包含 `sourceRef: "public-github"`、`repo`、`commit`、`path`、`contentHash` 和 `archiveUrl`；扫描/当前状态是门控条件，不会作为成功载荷元数据包含。
...
- 可选载荷字段：`migrateOwner`。当它与 `ownerHandle` 一起为 `true` 时，如果操作者同时是当前发布者和目标发布者的管理员/所有者，现有技能可以移动到该所有者。没有此显式选择时，所有者变更会被拒绝。
...
- 允许的表单字段为 `payload`、重复的 `files` blob，或一个 `clawpack` tarball 引用。`clawpack` 可以是 `.tgz` blob，也可以是 upload-url 流程返回的 storage id。基于暂存 storage-id 的发布还必须包含随该上传 URL 返回的 `clawpackUploadTicket`。
- 使用 `files` 或 `clawpack` 之一，同一个请求中绝不能同时使用二者。
- JSON 正文以及调用方提供的 `payload.files` / `payload.artifact` 元数据会被拒绝。
- 直接 multipart 发布请求上限为 18MB。ClawPack tarball 可以通过 upload-url 流程使用最高 120MB 的 tarball 上限。
...
- 代发布仍会针对目标所有者账户验证 official-channel 资格。
...
所有者发起的软删除会保留该 slug 30 天，之后该 slug 可由另一个发布者认领。
...
仅管理员。确保某个 handle 存在对应的组织发布者。如果该 handle 仍指向旧版共享用户/个人发布者，该端点会先将其迁移为组织发布者。
...
已认证的自助组织发布者创建。创建新的组织发布者并将调用方添加为所有者。此端点不会迁移现有用户/个人 handle，也不会将发布者标记为 trusted/official。
...
仅管理员。为合法所有者保留根 slug 和软件包名称，而不发布版本。软件包名称会变为没有版本行的私有占位软件包，因此同一个所有者之后可以将真实的 code-plugin 或 bundle-plugin 版本发布到该名称。
...
仅管理员。为已验证的替换 GitHub OAuth 主体恢复个人发布者，而不编辑 Convex Auth 账户行。请求必须同时命名两个不可变的 GitHub provider account id；可变 handle 仅用作面向操作者的保护。

端点默认使用试运行。应用恢复需要在工作人员独立核实两个 GitHub 主体之间的连续性后设置 `dryRun: false` 和
`confirmIdentityVerified: true`。当目标用户当前的个人发布者拥有技能、包或 GitHub 技能源时，恢复会失败并关闭。
恢复还会迁移已恢复发布者的技能、技能 slug 别名、包、包检查器警告以及派生搜索摘要行中的旧版 `ownerUserId` 字段，以便直接所有者路径与新的发布者权限保持一致。已恢复 handle 的有效 protected-handle 预留也会重新分配给替换用户，这样后续个人资料同步就无法恢复原用户的竞争权限。每个主表在每次应用事务中限制为
