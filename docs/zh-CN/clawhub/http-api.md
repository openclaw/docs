...
旧版 `/api/...` 和 `/api/cli/...` 为兼容性而保留（参见 `DEPRECATIONS.md`）。
OpenAPI：`/api/v1/openapi.json`。

## 公开目录复用

第三方目录可以使用公开读取端点来列出或搜索 ClawHub Skills。请缓存结果，遵守 `429`/`Retry-After`，将用户链接回规范的 ClawHub 列表（`https://clawhub.ai/<owner>/skills/<slug>`），并避免暗示 ClawHub 为第三方站点背书。不要尝试在公开 API 表面之外镜像隐藏、私有或被审核屏蔽的内容。
...
- 当服务器知道原因时，已认证写入端点不应返回裸 `Unauthorized`。缺失令牌、无效/已吊销令牌，以及已删除/封禁/禁用的账户都应获得可操作文本，以便 CLI 客户端告诉用户被阻止的原因。
...
- `RateLimit-Reset`：距重置的秒数（延迟）
- `X-RateLimit-Remaining` / `RateLimit-Remaining`：存在时为精确剩余额度。
  分片成功请求会省略此标头，而不是返回近似全局值。
- `Retry-After`：`429` 时重试前应等待的秒数（延迟）
...
公开 v1 错误响应是纯文本，`content-type: text/plain; charset=utf-8`。
这包括验证失败（`400`）、缺失的公开资源（`404`）、认证和权限失败（`401`/`403`）、速率限制（`429`）以及被阻止的下载。客户端应将响应正文读取为人类可读字符串。未知查询参数会因兼容性被忽略，但带无效值的已识别查询参数会返回 `400`。
...
- `q`（必需）：查询字符串
- `limit`（可选）：整数
- `highlightedOnly`（可选）：设为 `true` 以筛选高亮 Skills
- `nonSuspiciousOnly`（可选）：设为 `true` 以隐藏可疑（`flagged.suspicious`）Skills
...
- 结果按相关性顺序返回（嵌入相似度 + 精确 slug/名称 token 加权 + 小幅热度先验）。
- 相关性强于热度。精确 slug 或显示名称 token 匹配可以超过参与度高得多但匹配更宽松的结果。
...
- 热度经过对数缩放并设有上限。当查询文本匹配较弱时，高参与度 Skills 可能排名更低。
- 根据调用方筛选器和当前审核状态，可疑或隐藏的审核状态可能会从公开搜索中移除某个 Skill。

发布者可发现性指引：

- 将用户会实际搜索的词放入显示名称、摘要和标签。仅当独立 slug token 也是你想保留的稳定身份时才使用它。
- 不要仅为追逐某个查询而重命名 slug，除非新 slug 是更好的长期规范名称。旧 slug 会成为重定向别名，但规范 URL、显示的 slug 和未来搜索摘要会使用新 slug。
- 重命名别名会为旧 URL 和通过注册表解析的安装保留解析能力，但搜索排名会基于重命名已索引后的规范 Skill 元数据。现有统计数据会保留在该 Skill 上。
- 如果某个 Skill 意外不可见，请在更改排名相关元数据之前，先在登录状态下用 `clawhub inspect @owner/slug` 检查审核状态。
...
- `createdAt` 对新 Skill 抓取是稳定的；现有 Skills 重新发布时，`updated` 会变化。
- 当 `nonSuspiciousOnly=true` 时，基于游标的排序可能会在一页中返回少于 `limit` 的项目，因为可疑 Skills 在页面取回后才被筛除。
- 存在 `nextCursor` 时，使用它继续分页。短页本身不表示结果结束。
...
- 公开调用方只会对已标记的可见 Skills 获得 `200`。
- 对公开调用方，证据会被遮盖，并且仅对所有者/审核员包含原始片段。

### `POST /api/v1/skills/{slug}/report`

举报 Skill 供审核员审查。举报是 Skill 级别的，可选关联到某个版本，并会进入 Skill 举报队列。
...
`note` 对 `confirmed` 和 `dismissed` 是必需的；将 `status` 设回 `open` 时可以省略。传递 `finalAction: "hide"` 以在同一个可审计工作流中通过已分诊举报隐藏该 Skill。
...
- 包含规范化后的验证状态以及特定于扫描器的详细信息。
- 只有当扫描器生成明确结论（`clean`、`suspicious` 或 `malicious`）时，`security.hasScanResult` 才为 `true`。
- `moderation` 是从最新版本派生的当前技能级审核快照。
- 查询历史版本时，先检查 `moderation.matchesRequestedVersion` 和 `moderation.sourceVersion`，再将 `moderation` 和 `security` 视为同一版本上下文。
...
- 扫描请求载荷和可下载报告会在保留窗口之后从扫描请求存储中到期。
...
- 扫描任务是异步的。手动扫描请求会优先于普通发布/回填工作，但完成时间仍取决于工作器可用性。
...
- 返回排队/运行中/成功/失败状态。
- 排队时返回 `queue.queuedAhead` 和 `queue.position`，让客户端可以显示该请求前面还有多少个优先级更高的手动扫描。非常大的队列会被限制，并以 `queuedAheadIsEstimate: true` 报告。
...
- 只有当所选版本已有生成的技能卡、未被审核判定为恶意软件阻止，并且 ClawScan 验证为干净时，`ok` 才为 `true`。
...
- 响应仅包含安全信息。不包括技能卡数据、已生成卡状态、工件文件列表或详细扫描器载荷。
...
- `channel`（可选）：`official`、`community` 或 `private`
- `isOfficial`（可选）：`true` 或 `false`
- `category`（可选）：插件类别过滤器。仅当请求限定到插件包时受支持。受控类别和旧版 v1
...
- `startDate`（必填）：Skill `updatedAt` 的 Unix 毫秒下界。
- `endDate`（必填）：Skill `updatedAt` 的 Unix 毫秒上界。
...
- 正文：ZIP 归档。
- 每个导出的插件以 `{family}/{packageName}/` 为根。
- 每个导出的插件包含最新发布的已存储文件。
- 每个插件的导出元数据存储在
...
- 对于旧世界包归档，`version.artifact.kind` 为 `legacy-zip`；对于 ClawPack 支持的发布，则为 `npm-pack`。
- ClawPack 发布包含 npm 兼容的 `npmIntegrity`、`npmShasum` 和
  `npmTarballName` 字段。
- `version.sha256hash` 是面向旧客户端的已弃用兼容性元数据。它对 `/api/v1/packages/{name}/download`
  返回的确切 ZIP 字节进行哈希。现代客户端应使用 `version.artifact.sha256`，它用于标识规范发布制品。
- 当扫描数据存在时，会包含 `version.vtAnalysis`、`version.llmAnalysis` 和 `version.staticScan`。
- 私有包返回 `404`，除非调用方可以读取所属发布者。

### `GET /api/v1/packages/{name}/versions/{version}/security`

返回安装客户端使用的确切包发布安全和信任摘要。这是 OpenClaw 用于判断已解析发布是否可以安装的公开消费表面。
...
- `trust.scanStatus` 是根据扫描器输入和手动发布审核派生的有效信任状态。
- `trust.moderationState` 可为 null。当不存在手动发布审核时，它为 `null`。
- `trust.blockedFromDownload` 是安装阻止信号。当此值为 `true` 时，OpenClaw 和其他安装客户端应阻止安装，而不是从扫描器或审核字段重新推导阻止规则。
- `trust.reasons` 是面向用户和审计的解释列表。原因代码是稳定、紧凑的字符串，例如 `manual:quarantined`、
  `scan:malicious` 和 `package:malicious`。
- `trust.pending` 表示一个或多个信任输入仍在等待完成。
- `trust.stale` 表示信任摘要是根据过期输入计算的，在作出高置信度允许决策前，应将其视为需要刷新。

说明：

- 此端点精确到版本。客户端应在解析出打算安装的包版本后调用它，而不是只在读取最新包元数据后调用。
- 私有包返回 `404`，除非调用方可以读取所属发布者。
- 此端点有意比所有者/审核员审核端点更窄。它公开安装决策和公开解释，而不公开举报者身份、举报正文、私有证据或内部审查时间线。
...
举报包以供审核员审查。举报是包级别的，可选择链接到某个版本。它们会进入审核队列，但本身不会自动隐藏或阻止下载；审核员应使用发布审核来批准、隔离或撤销制品。
...
`confirmed` 和 `dismissed` 需要 `note`；将 `status` 设回 `open` 时可以省略。对已确认的报告传入 `finalAction: "quarantine"` 或 `finalAction: "revoke"`，可在同一个可审计工作流中应用发布版本审核操作。
...
- `revoked`：在发布版本此前受信任之后阻止。

被隔离和撤销的发布版本会从构件下载路由返回 `403`。每次更改都会写入一条审计日志条目。
...
下载托管的 skill 版本 ZIP，或者针对具有 `clean` 或 `suspicious` 扫描结果且没有托管版本的当前 GitHub 支持 skill，返回 GitHub 源码交接。
...
- 下载统计按 UTC 日期的唯一身份计数（API 令牌有效时使用 `userId`，否则使用 IP）。
...
- 可选载荷字段：`migrateOwner`。当它与 `ownerHandle` 一起为 `true` 时，如果操作者同时是当前发布者和目标发布者的管理员/所有者，现有 skill 可以移动到该所有者。如果没有此显式选择，所有者更改会被拒绝。
...
- 允许的表单字段为 `payload`、重复的 `files` blob，或一个 `clawpack` tarball 引用。`clawpack` 可以是 `.tgz` blob，也可以是 upload-url 流程返回的 storage id。使用预暂存 storage-id 发布时，还必须包含该上传 URL 返回的 `clawpackUploadTicket`。
- 使用 `files` 或 `clawpack` 其中之一，绝不要在同一请求中同时使用两者。
- JSON 请求体和调用方提供的 `payload.files` / `payload.artifact` 元数据会被拒绝。
...
所有者发起的软删除会保留 slug 30 天，之后该 slug 可由其他发布者认领。删除响应在此过期规则适用时包含 `slugReservedUntil`。
...
仅限管理员。确保某个 handle 存在组织发布者。如果该 handle 仍指向旧版共享用户/个人发布者，该端点会先将其迁移为组织发布者。对于新创建的组织，请提供 `memberHandle`；执行操作的管理员不会被添加为成员。`memberRole` 默认为 `owner`。
...
仅限管理员。为合法所有者保留根 slug 和软件包名称，而不发布发布版本。软件包名称会成为没有发布版本行的私有占位软件包，因此同一所有者之后可以将真正的 code-plugin 或 bundle-plugin 发布版本发布到该名称下。
...
仅限管理员。为已验证的替换 GitHub OAuth principal 恢复个人发布者，而无需编辑 Convex Auth 账户行。请求必须命名两个不可变的 GitHub 提供商账户 ID；可变 handle 仅用作面向操作者的保护检查。

端点默认执行试运行。应用恢复需要在工作人员独立验证两个 GitHub 主体之间的连续性后设置 `dryRun: false` 和
`confirmIdentityVerified: true`。当目标用户当前的个人发布者拥有 Skills、包或 GitHub Skill 源时，恢复会以关闭方式失败。
恢复还会迁移已恢复发布者的 Skills、Skill slug 别名、包、包检查器警告以及派生搜索摘要行中的旧版 `ownerUserId` 字段，以便直接所有者路径与新的发布者权限保持一致。已恢复 handle 的有效受保护 handle 预留也会重新分配给替代用户，这样后续资料同步就无法恢复原用户的竞争权限。每个主表在每次应用事务中限制为
100 行；更大的恢复必须先使用可恢复的所有者迁移。
GitHub Skill 源按发布者确定范围，并报告为已检查，而不是被重写。
...
- 两个端点都需要 API 令牌认证，并且仅对 Skill 所有者可用。
...
更改现有封禁的已存储原因，而不解封或恢复内容（仅 admin）。除非 `dryRun` 为 `false`，否则默认执行试运行。
