...
第三方目录可以使用公共读取端点列出或搜索 ClawHub 技能。请缓存结果，遵守 `429`/`Retry-After`，将用户链接回规范的 ClawHub 列表页（`https://clawhub.ai/<owner>/skills/<slug>`），并避免暗示 ClawHub 认可该第三方站点。不要尝试在公共 API 表面之外镜像隐藏、私有或被审核阻止的内容。
...
- 当服务器知道原因时，已认证写入端点不应返回裸 `Unauthorized`。缺失令牌、无效/已撤销令牌，以及已删除/已封禁/已禁用账户，都应获得可操作的文本，以便 CLI 客户端能够告诉用户阻止原因。
...
这包括验证失败（`400`）、缺失公共资源（`404`）、认证和权限失败（`401`/`403`）、速率限制（`429`）以及被阻止的下载。客户端应将响应正文读取为人类可读字符串。未知查询参数会因兼容性而被忽略，但已识别的查询参数如果值无效则返回 `400`。
...
- 相关性强于流行度。精确的 slug 或显示名称 token 匹配，可能排在互动更强但匹配更宽泛的结果之前。
- ASCII 文本会按单词和标点边界分词。例如，`personal-map` 包含独立的 `map` token，而 `amap-jsapi-skill` 包含 `amap`、`jsapi` 和 `skill`；因此搜索 `map` 时，`personal-map` 比 `amap-jsapi-skill` 获得更强的词法匹配。
- 流行度采用对数缩放并设有上限。当查询文本匹配较弱时，高互动技能的排名可能更低。
- 根据调用方筛选器和当前审核状态，可疑或隐藏的审核状态可能会从公共搜索中移除某个技能。

发布者可发现性指南：

- 将用户会直接搜索的词放在显示名称、摘要和标签中。仅当独立的 slug token 也是你想保留的稳定身份时才使用它。
- 不要仅为追逐某个查询而重命名 slug，除非新 slug 是更好的长期规范名称。旧 slug 会成为重定向别名，但规范 URL、显示的 slug 和未来搜索摘要会使用新 slug。
- 重命名别名会为旧 URL 和通过注册表解析的安装保留解析能力，但搜索排名基于重命名已索引后的规范技能元数据。现有统计会保留在该技能上。
- 如果技能意外不可见，请先在登录状态下使用 `clawhub inspect @owner/slug` 检查审核状态，再更改与排名相关的元数据。
...
- 当 `nonSuspiciousOnly=true` 时，基于游标的排序在单页上返回的项目数可能少于 `limit`，因为可疑技能是在页面取回后过滤的。
...
- 面向公共调用方的证据会被脱敏，且仅对所有者/审核员包含原始片段。
...
`note` 对 `confirmed` 和 `dismissed` 是必需的；将 `status` 设置回 `open` 时可以省略。传入 `finalAction: "hide"` 并附带已分流的报告，可在同一个可审计工作流中隐藏该技能。
...
- 只有当扫描器给出确定结论（`clean`、`suspicious` 或 `malicious`）时，`security.hasScanResult` 才为 `true`。
...
- `kind` 默认是 `skill`；插件/包扫描请使用 `kind=plugin`。
- 返回与扫描请求下载相同形状的 ZIP。

### `POST /api/v1/skills/-/scan/batch`

仅限管理员使用的规范批量重新扫描路由。它接受与旧版 `POST /api/v1/skills/-/rescan-batch` 相同的载荷形状。
...
- `security.signals.dependencyRegistry` 保留用于 v1 响应兼容性，但依赖注册表存在性扫描器已停用，此键始终为 `null`。
- 技能卡缺失不会影响此端点的 `ok`、`decision` 或 `reasons`；客户端在需要卡片内容时应在本地读取已安装的 `skill-card.md`。
- 当你需要单个技能的技能卡验证信封时，请使用 `/verify`；需要已生成卡片 Markdown 时，请使用 `/card`；需要详细扫描器数据时，请使用 `/scan`。
...
- 对于旧世界包归档，`version.artifact.kind` 为 `legacy-zip`；对于由 ClawPack 支持的发布版本，为
  `npm-pack`。
...
返回用于安装客户端的精确包发布安全与信任摘要。这是公开的 OpenClaw 消费界面，用于决定已解析的发布版本是否可以安装。
...
- `trust.stale` 表示信任摘要是从过时输入计算得出的，在做出高置信度允许决定前应视为需要刷新。

说明：

- 此端点精确到版本。客户端应在解析出要安装的包版本后调用它，而不仅是在读取最新包元数据后调用。
- 私有包返回 `404`，除非调用方可以读取所属发布者。
- 此端点有意比所有者/审核员审核端点更窄。它公开安装决定和公开解释，而不公开举报者身份、举报正文、私有证据或内部审核时间线。
...
- ClawPack 版本返回 `npm-pack` 构件、npm integrity 字段、`tarballUrl` 和旧版 ZIP 兼容 URL。
...
举报一个包以供审核员审核。举报是包级别的，可以选择关联到一个版本。它们会进入审核队列，但本身不会自动隐藏或阻止下载；审核员应使用发布审核来批准、隔离或撤销构件。
...
`note` 对 `confirmed` 和 `dismissed` 是必填项；当将 `status` 重新设置为 `open` 时可以省略。对已确认的报告传入 `finalAction: "quarantine"` 或 `finalAction: "revoke"`，可在同一个可审计工作流中应用发布版本审核。
...
- 二进制文件返回 `415`。
- 文件大小限制：200KB。
- 待处理的 VirusTotal 扫描不会阻止读取；恶意发布版本仍可能在其他位置被保留不提供。
...
返回用于基于 ClawPack 的包版本的 npm 兼容 packument。
...
下载托管的技能版本 ZIP，或者对于当前基于 GitHub、扫描结果为 `clean` 或 `suspicious` 且没有托管版本的技能，返回 GitHub 源码移交。
...
- 如果既未提供 `version` 也未提供 `tag`，则使用最新版本。
- 软删除的版本返回 `410`。
- 基于 GitHub 的技能移交不会代理或镜像字节。JSON 响应包含 `sourceRef: "public-github"`、`repo`、`commit`、`path`、`contentHash` 和 `archiveUrl`；扫描/当前状态是门禁，不会作为成功载荷元数据包含。
...
- 可选载荷字段：`migrateOwner`。当它与 `ownerHandle` 一起为 `true` 时，如果操作者同时是当前和目标发布者的管理员/所有者，现有技能可以移动到该所有者。没有此显式选择时，所有者变更会被拒绝。
...
- 允许的表单字段为 `payload`、重复的 `files` blob，或一个 `clawpack` tarball 引用。`clawpack` 可以是 `.tgz` blob，也可以是 upload-url 流程返回的存储 ID。基于暂存 storage-id 的发布还必须包含该上传 URL 返回的 `clawpackUploadTicket`。
- 使用 `files` 或 `clawpack`，同一请求中绝不能同时使用二者。
- JSON 正文和调用方提供的 `payload.files` / `payload.artifact` 元数据会被拒绝。
- 直接 multipart 发布请求上限为 18MB。ClawPack tarball 可使用 upload-url 流程，最高为 120MB tarball 上限。
...
- 代发仍会根据目标所有者账户验证 official-channel 资格。
...
所有者发起的软删除会保留 slug 30 天，之后该 slug 可由另一个发布者认领。删除响应会在此到期规则适用时包含 `slugReservedUntil`。
...
仅限管理员。为合法所有者保留根 slug 和包名称，而不发布发布版本。包名称会成为没有发布版本行的私有占位包，因此同一所有者稍后可以将真正的 code-plugin 或 bundle-plugin 发布版本发布到该名称下。
...
仅限管理员。为已验证的替代 GitHub OAuth 主体恢复个人发布者，而不编辑 Convex Auth 账户行。请求必须指定两个不可变的 GitHub 提供商账户 ID；可变 handle 仅用作面向操作者的保护。

该端点默认执行试运行。应用恢复需要在工作人员独立验证两个 GitHub 主体之间的连续性后设置 `dryRun: false` 和
`confirmIdentityVerified: true`。当目标用户当前的个人发布者拥有技能、包或 GitHub 技能来源时，恢复会以失败关闭方式终止。
恢复还会迁移已恢复发布者的技能、技能 slug 别名、包、包检查器警告以及派生搜索摘要行中的旧版 `ownerUserId` 字段，使直接所有者路径与新的发布者权限保持一致。已恢复 handle 的有效受保护 handle 保留也会重新分配给替代用户，因此之后的个人资料同步无法恢复前用户的竞争权限。每个主表在每次应用事务中限制为
100 行；更大的恢复必须先使用可恢复的所有者迁移。
GitHub 技能来源是按发布者限定的，并会报告为已检查，而不是被重写。
...
更改现有封禁中存储的原因，不解除封禁，也不恢复内容（仅限管理员）。除非 `dryRun` 为 `false`，否则默认为试运行。
