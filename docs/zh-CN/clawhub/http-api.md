...
x-i18n:
    generated_at: "2026-07-03T09:21:38Z"
    model: gpt-5.5
...
第三方目录可以使用公共读取端点来列出或搜索 ClawHub 技能。请缓存结果，遵守 `429`/`Retry-After`，将用户链接回规范的 ClawHub 列表页（`https://clawhub.ai/<owner>/skills/<slug>`），并避免暗示 ClawHub 为第三方站点背书。不要尝试在公共 API 表面之外镜像隐藏、私有或被审核阻止的内容。
...
- 当服务器知道原因时，已认证写入端点不应返回裸的 `Unauthorized`。缺失 token、无效/已吊销 token，以及已删除/已封禁/已禁用账户，都应获得可操作的文本，以便 CLI 客户端能告知用户是什么阻止了他们。
...
- 使用带抖动的退避，避免同步重试。
- 如果 `Retry-After` 缺失，请回退到 `RateLimit-Reset`（或从 `X-RateLimit-Reset` 计算）。

IP 来源：

- 仅当部署显式启用受信任的转发标头时，才使用受信任的客户端 IP 标头，包括 `cf-connecting-ip`。
- ClawHub 使用受信任的转发标头在边缘识别客户端 IP。
- 如果没有可用的受信任客户端 IP，匿名请求会使用仅按速率限制类型划分作用域的回退桶。这些回退桶不包含调用方提供的路径、slug、包名、版本、查询字符串或其他工件参数。

## 错误响应

公共 v1 错误响应是纯文本，带有 `content-type: text/plain; charset=utf-8`。
这包括验证失败（`400`）、缺失公共资源（`404`）、身份验证和权限失败（`401`/`403`）、速率限制（`429`）以及被阻止的下载。客户端应将响应正文读取为人类可读的字符串。未知查询参数会被忽略以保持兼容性，但具有无效值的已识别查询参数会返回 `400`。
...
- 相关性强于流行度。精确的 slug 或显示名称 token 匹配，可以排在互动度高得多但匹配更宽松的结果之前。
- ASCII 文本会按词和标点边界切分。例如，`personal-map` 包含独立的 `map` token，而 `amap-jsapi-skill` 包含 `amap`、`jsapi` 和 `skill`；因此搜索 `map` 时，`personal-map` 会比 `amap-jsapi-skill` 获得更强的词法匹配。
- 流行度经过对数缩放并设有上限。当查询文本匹配较弱时，高互动度技能的排名可能更低。
- 可疑或隐藏的审核状态可能会根据调用方筛选条件和当前审核状态，将技能从公共搜索中移除。

发布者可发现性指南：

- 将用户会实际搜索的词放入显示名称、摘要和标签中。仅当独立 slug token 也是你想保留的稳定身份时才使用它。
- 不要仅为追逐某个查询而重命名 slug，除非新 slug 是更好的长期规范名称。旧 slug 会成为重定向别名，但规范 URL、显示的 slug 和未来的搜索摘要会使用新 slug。
- 重命名别名会为通过注册表解析的旧 URL 和安装保留解析能力，但搜索排名会基于重命名索引后的规范技能元数据。现有统计信息会继续归属于该技能。
- 如果某个技能意外不可见，请先在登录状态下使用 `clawhub inspect @owner/slug` 检查审核状态，再更改与排名相关的元数据。
...
- `recommended` 使用互动度和新近度信号。
- `trending` 按过去 7 天的安装量排名（基于遥测）。
- `createdAt` 对新技能抓取是稳定的；`updated` 会在现有技能重新发布时变化。
- 当 `nonSuspiciousOnly=true` 时，基于游标的排序可能会在某一页返回少于 `limit` 的项目，因为可疑技能是在页面检索后被筛除的。
...
- 公共调用方只会对已标记且可见的技能获得 `200`。
- 面向公共调用方的证据会被编辑，仅对所有者/审核员包含原始片段。
...
`note` 对 `confirmed` 和 `dismissed` 是必填项；将 `status` 设置回 `open` 时可以省略。传入 `finalAction: "hide"` 与已分诊的报告一起使用，可在同一个可审计工作流中隐藏该技能。
...
- 排队时返回 `queue.queuedAhead` 和 `queue.position`，以便客户端显示请求前面有多少个优先级更高的手动扫描。非常大的队列会被设定上限，并以 `queuedAheadIsEstimate: true` 报告。
...
- 只有当所选版本具有已生成的 Skill Card、未被审核标记为恶意软件阻止，并且 ClawScan 验证为 clean 时，`ok` 才为 `true`。
...
- `security.signals.dependencyRegistry` 为 v1 响应兼容性而保留，但依赖项注册表存在性扫描器已退役，此键始终为 `null`。
...
- 响应仅包含安全信息。它不包含 Skill Card 数据、生成的卡片状态、工件文件列表或详细扫描器载荷。
- `security.signals` 只包含状态级支持证据；完整扫描器详情请使用 `/scan` 或 ClawHub 安全审计页面。
- `security.signals.dependencyRegistry` 为 v1 响应兼容性而保留，但依赖项注册表存在性扫描器已退役，此键始终为 `null`。
- Skill Card 缺失不会影响此端点的 `ok`、`decision` 或 `reasons`；当客户端需要卡片内容时，应在本地读取已安装的 `skill-card.md`。
...
- `category`（可选）：插件类别过滤器。仅当请求限定为插件包（`/api/v1/plugins`、
  ...
  `family=code-plugin`/`family=bundle-plugin` 的包端点）时受支持。受控类别和
...
- Skill 条目仍由 Skill 注册表支撑，并且仍只能通过 `POST /api/v1/skills` 发布。
...
- `channel`（可选）：`official`、`community` 或 `private`
- `isOfficial`（可选）：`true` 或 `false`
- `category`（可选）：插件类别过滤器。仅当请求限定为插件包时受支持。受控类别和旧版 v1
...
- 当前由 GitHub 支撑且扫描结果为 `clean` 或 `suspicious` 的 Skills 包含
...
- 当单个 Skill 或文件无法导出时，会包含 `_errors.json`。
...
- 结果按相关性顺序返回，目前不分页。
- 插件搜索的浏览器 UI 排序控件会对已加载的相关性结果重新排序，与当前 `/skills` 浏览行为一致。
...
- 需要包所有者、组织发布者所有者/管理员、平台协管员或平台管理员的 API 令牌。
...
- 对于旧式包归档，`version.artifact.kind` 为 `legacy-zip`；对于基于 ClawPack 的发布版本，则为 `npm-pack`。
- ClawPack 发布版本包含 npm 兼容的 `npmIntegrity`、`npmShasum` 和 `npmTarballName` 字段。
- `version.sha256hash` 是面向旧客户端的已弃用兼容性元数据。它会对 `/api/v1/packages/{name}/download` 返回的确切 ZIP 字节进行哈希。现代客户端应使用 `version.artifact.sha256`，它标识规范发布制品。
- 当扫描数据存在时，会包含 `version.vtAnalysis`、`version.llmAnalysis` 和 `version.staticScan`。
- 私有包会返回 `404`，除非调用者可以读取所属发布者。

### `GET /api/v1/packages/{name}/versions/{version}/security`

返回面向安装客户端的确切包发布安全和信任摘要。这是公开的 OpenClaw 消费接口，用于判断已解析的发布版本是否可安装。

身份验证：

- 公开读取端点。不需要所有者、发布者、协管员或管理员令牌。
...
- 当发布制品已知时，会包含 `release.artifactKind`、`release.artifactSha256`、`release.npmIntegrity`、`release.npmShasum` 和 `release.npmTarballName`。
- `trust.scanStatus` 是从扫描器输入和手动发布审核派生出的有效信任状态。
- `trust.moderationState` 可为空。当不存在手动发布审核时，它为 `null`。
- `trust.blockedFromDownload` 是安装阻断信号。当此值为 `true` 时，OpenClaw 和其他安装客户端应阻止安装，而不是从扫描器或审核字段重新推导阻断规则。
...
- `trust.stale` 表示信任摘要是基于过时输入计算的，在作出高置信度允许决策前，应视为需要刷新。

说明：

- 此端点精确到版本。客户端应在解析出打算安装的包版本后调用它，而不是只在读取最新包元数据后调用。
- 私有包会返回 `404`，除非调用者可以读取所属发布者。
- 此端点有意比所有者/协管员审核端点更窄。它公开安装决策和公开解释，而不公开报告者身份、报告正文、私有证据或内部审核时间线。
...
- ClawPack 版本返回 `npm-pack` 制品、npm 完整性字段、`tarballUrl` 以及旧版 ZIP 兼容性 URL。
...
- 这只跟踪迁移就绪状态。它不会修改 OpenClaw，也不会生成 ClawPack。
...
举报一个包以供协管员审核。举报是包级别的，也可以选择关联到某个版本。它们会进入审核队列，但本身不会自动隐藏或阻止下载；协管员应使用发布审核来批准、隔离或撤销制品。
...
用于接收软件包报告的版主/admin 端点。

认证：

- 需要版主或 admin 用户的 API token。
...
`note` 对 `confirmed` 和 `dismissed` 是必需的；将 `status` 设回 `open` 时可以省略。
传入 `finalAction: "quarantine"` 或 `finalAction: "revoke"` 并配合已确认的报告，可在同一条可审计工作流中应用发布审核。
...
- `revoked`：某个发布先前受信任，之后被阻止。

隔离和撤销的发布会从构件下载路由返回 `403`。
每次变更都会写入一条审计日志条目。
...
- 二进制文件返回 `415`。
- 文件大小限制：200KB。
- 待处理的 VirusTotal 扫描不会阻止读取；恶意发布仍可能在其他地方被扣留。
...
- 旧版仅 ZIP 版本会有意省略。
- `dist.tarball`、`dist.integrity` 和 `dist.shasum` 使用 npm 兼容字段，
  因此用户可以在需要时将 npm 指向镜像。
...
下载托管的技能版本 ZIP，或针对当前 GitHub 支持且扫描结果为 `clean` 或 `suspicious`、没有托管版本的技能返回 GitHub 源码交接信息。
...
- 如果既未提供 `version` 也未提供 `tag`，则使用最新版本。
- 软删除的版本返回 `410`。
- GitHub 支持的技能交接不会代理或镜像字节。JSON 响应包含
  `sourceRef: "public-github"`、`repo`、`commit`、`path`、`contentHash`
  和 `archiveUrl`；扫描/当前状态是门禁，不会作为成功载荷元数据包含。
...
- 可选 payload 字段：`ownerHandle`。存在时，API 会在服务端解析该发布者，并要求操作者具有发布者访问权限。
- 可选 payload 字段：`migrateOwner`。当与 `ownerHandle` 一起为 `true` 时，如果操作者是当前发布者和目标发布者双方的 admin/owner，现有技能可以移动到该所有者。没有此显式选择时，所有者变更会被拒绝。
...
- 允许的表单字段是 `payload`、重复的 `files` blob，或一个 `clawpack`
  tarball 引用。`clawpack` 可以是 `.tgz` blob，也可以是 upload-url 流程返回的 storage id。基于暂存 storage-id 的发布还必须包含随该上传 URL 返回的
  `clawpackUploadTicket`。
- 同一个请求中只能使用 `files` 或 `clawpack`，不能同时使用。
- JSON body 和调用方提供的 `payload.files` / `payload.artifact`
  元数据会被拒绝。
- 直接 multipart 发布请求上限为 18MB。ClawPack tarball 可以使用 upload-url 流程，最高达到 120MB tarball 上限。
- 可选 payload 字段：`ownerHandle`。存在时，只有 admin 可以代表该所有者发布。
...
- 代码插件需要 `package.json`、源代码仓库元数据、源代码 commit
...
- 代表他人发布仍会针对目标所有者账号验证 official-channel 资格。
...
另一个发布者认领。此过期适用时，删除响应包含 `slugReservedUntil`。
...
仅限 admin。确保某个 handle 存在组织发布者。如果该 handle 仍指向
旧版共享用户/个人发布者，端点会先将它迁移为组织发布者。
对于新创建的组织，请提供 `memberHandle`；执行操作的 admin 不会被添加为成员。
...
已认证的自助组织发布者创建。创建新的组织发布者，并将
调用方添加为所有者。此端点不会迁移现有用户/个人 handle，也不会
将发布者标记为 trusted/official。
...
release。软件包名称会成为没有 release 行的私有占位软件包，因此同一
所有者之后可以将真实的 code-plugin 或 bundle-plugin 发布到该名称。
...
仅限 admin。为已验证的替代 GitHub OAuth 主体恢复个人发布者，
而不编辑 Convex Auth 账号行。请求必须命名两个不可变的 GitHub
provider account id；可变 handle 仅用作面向操作者的防护。

该端点默认使用试运行。应用恢复需要在工作人员独立核验两个 GitHub 身份主体之间的连续性后，设置 `dryRun: false` 和
`confirmIdentityVerified: true`。
当目标用户当前的个人发布者拥有技能、包或 GitHub 技能来源时，恢复会默认失败关闭。
恢复还会迁移已恢复发布者的技能、技能 slug 别名、包、包检查器警告以及派生搜索摘要行中的旧版 `ownerUserId` 字段，使直接所有者路径与新的发布者权限保持一致。已恢复 handle 的活动受保护 handle 预留也会重新分配给替代用户，因此后续个人资料同步无法恢复前用户的竞争权限。每个主表在每次应用事务中限制为
100 行；更大的恢复必须先使用可恢复的所有者迁移。
GitHub 技能来源是按发布者划分的，并会报告为已检查，而不是被重写。
...
更改现有封禁存储的原因，而不解除封禁或恢复内容（仅限管理员）。除非 `dryRun` 为 `false`，否则默认使用试运行。
...
`POST /api/cli/upload-url` 返回 `uploadUrl` 和 `uploadTicket`。发布暂存 ClawPack tarball 的包时，必须将生成的存储 id 作为
