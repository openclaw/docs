...
x-i18n:
    generated_at: "2026-07-04T03:34:34Z"
    model: gpt-5.5
...
第三方目录可以使用公共读取端点列出或搜索 ClawHub 技能。请缓存结果，遵守 `429`/`Retry-After`，将用户链接回规范的 ClawHub 列表页（`https://clawhub.ai/<owner>/skills/<slug>`），并避免暗示 ClawHub 认可该第三方站点。不要尝试在公共 API 表面之外镜像隐藏、私有或被审核拦截的内容。
...
- 当服务器知道原因时，已认证写入端点不应返回裸 `Unauthorized`。缺失 token、无效/已撤销 token，以及已删除/被封禁/已禁用账号，都应分别返回可操作的文本，以便 CLI 客户端能告诉用户阻塞原因。
...
这包括验证失败（`400`）、缺失的公共资源（`404`）、认证和权限失败（`401`/`403`）、速率限制（`429`）以及被阻止的下载。客户端应将响应正文读取为人类可读字符串。未知查询参数会因兼容性被忽略，但已识别查询参数的无效值会返回 `400`。
...
- 结果按相关性顺序返回（嵌入相似度 + 精确 slug/名称 token 加权 + 小幅流行度先验）。
...
- 流行度经过对数缩放并设有上限。当查询文本匹配较弱时，高参与度技能可能排名更低。
- 可疑或隐藏的审核状态可能会根据调用方筛选器和当前审核状态，将某个技能从公共搜索中移除。

发布者可发现性指导：

- 将用户会逐字搜索的术语放入显示名称、摘要和标签。只有当独立 slug token 也是你想保留的稳定身份时才使用它。
- 不要仅为了追逐某个查询而重命名 slug，除非新 slug 是更好的长期规范名称。旧 slug 会成为重定向别名，但规范 URL、显示的 slug 和未来的搜索摘要会使用新 slug。
- 重命名别名会为旧 URL 以及通过注册表解析的安装保留解析能力，但搜索排名基于重命名索引完成后的规范技能元数据。现有统计数据会保留在该技能上。
- 如果某个技能意外不可见，请先在登录状态下用 `clawhub inspect @owner/slug` 检查审核状态，再更改排名相关元数据。
...
- 当 `nonSuspiciousOnly=true` 时，基于游标的排序可能在一页中返回少于 `limit` 的条目，因为可疑技能是在页面检索后过滤的。
...
- 只有当技能被标记，或所有者正在查看它时，才包含 `moderation`。
...
- 对公共调用方，证据会被脱敏，并且仅为所有者/版主包含原始片段。

### `POST /api/v1/skills/{slug}/report`

举报技能以供版主审查。举报是技能级别的，可选链接到某个版本，并会进入技能举报队列。
...
`note` 对 `confirmed` 和 `dismissed` 是必需的；将 `status` 设置回 `open` 时可以省略。传递 `finalAction: "hide"` 和一个已分诊的举报，可在同一个可审计工作流中隐藏该技能。
...
- 包含规范化的验证状态以及扫描器特定详情。
- 仅当扫描器生成了明确判定（`clean`、`suspicious` 或 `malicious`）时，`security.hasScanResult` 才为 `true`。
...
- 扫描请求负载和可下载报告会在保留窗口结束后从扫描请求存储中过期。
- 已发布扫描需要所有者/发布者管理权限，或平台审核员/管理员权限。
- 仅当 `update: true` 且扫描成功完成时，已发布扫描才会写回。
...
- 排队期间返回 `queue.queuedAhead` 和 `queue.position`，以便客户端显示该请求前面还有多少个优先处理的手动扫描。超大队列会被限制，并以 `queuedAheadIsEstimate: true` 报告。
...
用于已提交版本的需认证已存储报告归档端点。

- 需要对该技能或插件拥有所有者/发布者管理权限，或拥有平台审核员/管理员权限。
...
仅管理员使用的规范批量重新扫描路由。它接受与旧版 `POST /api/v1/skills/-/rescan-batch` 相同的负载形态。

### `POST /api/v1/skills/-/scan/batch/status`

仅管理员使用的规范批量状态路由。它接受 `{ "jobIds": ["..."] }`，并返回与旧版 `POST /api/v1/skills/-/rescan-batch/status` 相同的聚合计数器。
...
- 仅当所选版本拥有已生成的技能卡片、未被审核标记为恶意软件阻止，并且 ClawScan 验证为 clean 时，`ok` 才为 `true`。
...
- `security` 是顶层 ClawScan/安全判定。自动化应依据 `ok`、`decision`、`reasons` 和 `security.status`。
- `security.signals` 包含支持性的扫描器证据，例如 `staticScan`、`virusTotal` 和 `skillSpector`。
- `security.signals.dependencyRegistry` 为 v1 响应兼容性而保留，但依赖注册表存在性扫描器已退役，此键始终为 `null`。
- 仅当 ClawHub 在发布或导入期间解析并存储了 GitHub 仓库/ref/commit/path 时，`provenance` 才为 `server-resolved-github-import`；否则为 `unavailable`。

### `POST /api/v1/skills/-/security-verdicts`

返回精确技能版本的当前紧凑安全判定。此集合端点面向已经知道需要展示哪些已安装 ClawHub 技能版本的客户端，例如 OpenClaw Control UI。
...
- 响应仅包含安全信息。它不包含技能卡片数据、已生成卡片状态、工件文件列表或详细扫描器负载。
...
- `category`（可选）：插件类别过滤器。仅当请求限定为插件包（`/api/v1/plugins`、`/api/v1/code-plugins`、`/api/v1/bundle-plugins`，或带有 `family=code-plugin`/`family=bundle-plugin` 的包端点）时支持。受控类别和旧版 v1 过滤器别名记录在 `GET /api/v1/plugins` 下。
...
- `channel`（可选）：`official`、`community` 或 `private`
- `isOfficial`（可选）：`true` 或 `false`
- `category`（可选）：插件类别过滤器。仅当请求限定为插件包时支持。受控类别和旧版 v1 过滤器别名记录在 `GET /api/v1/plugins` 下。
...
- 对于旧体系的包归档，`version.artifact.kind` 为 `legacy-zip`；对于由 ClawPack 支持的版本，为 `npm-pack`。
- ClawPack 版本包含与 npm 兼容的 `npmIntegrity`、`npmShasum` 和 `npmTarballName` 字段。
- `version.sha256hash` 是面向旧客户端的已弃用兼容性元数据。它对 `/api/v1/packages/{name}/download` 返回的精确 ZIP 字节进行哈希。
  现代客户端应使用 `version.artifact.sha256`，它标识规范版本工件。
- 当扫描数据存在时，会包含 `version.vtAnalysis`、`version.llmAnalysis` 和 `version.staticScan`。
- 私有包返回 `404`，除非调用方可以读取所属发布者。

### `GET /api/v1/packages/{name}/versions/{version}/security`

返回供安装客户端使用的精确包版本安全与信任摘要。这是公开的 OpenClaw 消费表面，用于决定已解析版本是否可以安装。
...
- `release.releaseId`、`release.version` 和 `release.createdAt` 标识经过评估的精确版本。
- 当版本工件已知时，会提供 `release.artifactKind`、`release.artifactSha256`、`release.npmIntegrity`、
...
- `trust.stale` 表示信任摘要是根据过期输入计算的，在做出高置信度允许决定前应视为需要刷新。

说明：

- 此端点精确到版本。客户端应在解析出打算安装的包版本后调用它，而不仅是在读取最新包元数据后调用。
- 私有包返回 `404`，除非调用方可以读取所属发布者。
- 此端点有意比所有者/版主审核端点更窄。它公开安装决定和公开解释，而不是举报者身份、举报正文、私有证据或内部审查时间线。
...
- 这仅跟踪迁移就绪状态。它不会修改 OpenClaw 或生成 ClawPack。
...
举报一个包以供版主审查。举报是包级别的，可选关联到某个版本。它们会进入审核队列，但本身不会自动隐藏或阻止下载；版主应使用版本审核来批准、隔离或撤销工件。
...
主持人/admin 用于接收包报告的端点。

认证：

- 需要主持人或 admin 用户的 API 令牌。
...
所有者/主持人用于查看包审核可见性的端点。

认证：

- 需要包所有者、发布者成员、主持人或 admin 用户的 API 令牌。
...
隔离和撤销的发布会在制品下载路由返回 `403`。每次更改都会写入一条审计日志条目。
...
- 插件/包归档是带有 `package/` 根目录的 zip 文件，因此旧 OpenClaw 客户端会继续工作。
...
- 作用域包的 packument 同时支持 `/api/npm/@scope/name` 和 npm 的编码请求路径 `/api/npm/@scope%2Fname`。
...
下载托管的 skill 版本 ZIP；或者对于当前由 GitHub 支持、扫描结果为 `clean` 或 `suspicious` 且没有托管版本的 skill，返回 GitHub 源交接。
...
- 如果未提供 `version` 或 `tag`，则使用最新版本。
- 软删除版本返回 `410`。
- GitHub 支持的 skill 交接不会代理或镜像字节。JSON 响应包含 `sourceRef: "public-github"`、`repo`、`commit`、`path`、`contentHash` 和 `archiveUrl`；扫描/当前状态是门禁，不作为成功载荷元数据包含在内。
...
- 可选载荷字段：`migrateOwner`。当它与 `ownerHandle` 一起为 `true` 时，如果操作者是当前发布者和目标发布者双方的 admin/owner，现有 skill 可以移动到该所有者。没有此显式选择时，会拒绝所有者变更。
...
- 允许的表单字段是 `payload`、重复的 `files` blob，或一个 `clawpack` tarball 引用。`clawpack` 可以是 `.tgz` blob，也可以是上传 URL 流程返回的 storage id。使用暂存 storage-id 发布时，还必须包含该上传 URL 返回的 `clawpackUploadTicket`。
- 在同一请求中使用 `files` 或 `clawpack`，不能同时使用两者。
- 会拒绝 JSON 正文和调用方提供的 `payload.files` / `payload.artifact` 元数据。
- 直接 multipart 发布请求上限为 18MB。ClawPack tarball 可以使用上传 URL 流程，最高达到 120MB tarball 上限。
- 可选载荷字段：`ownerHandle`。存在时，只有 admin 可以代表该所有者发布。
...
- 代发仍会根据目标所有者账户验证 official 频道资格。

### `DELETE /api/v1/skills/{slug}` / `POST /api/v1/skills/{slug}/undelete`

软删除/恢复 skill（所有者、主持人或 admin）。
...
存在时，`reason` 会作为 skill 审核备注存储，并复制到审计日志中。
所有者发起的软删除会保留 slug 30 天，之后另一个发布者可以认领该 slug。适用此到期时，删除响应会包含 `slugReservedUntil`。
主持人/admin 隐藏和安全移除不会以这种方式到期。
...
仅限 admin。确保某个 handle 存在对应的组织发布者。如果该 handle 仍指向旧版共享用户/个人发布者，该端点会先将其迁移为组织发布者。对于新创建的组织，请提供 `memberHandle`；执行操作的 admin 不会被添加为成员。`memberRole` 默认为 `owner`。
...
已认证用户可自助创建组织发布者。创建新的组织发布者，并将调用方添加为所有者。此端点不会迁移现有用户/个人 handle，也不会将发布者标记为受信任/official。
...
仅限 admin。为合法所有者保留根 slug 和包名称，而不发布版本。包名称会成为没有发布行的私有占位包，因此同一所有者稍后可以将真正的 code-plugin 或 bundle-plugin 版本发布到该名称。
...
仅限 admin。为已验证的替代 GitHub OAuth 主体恢复个人发布者，而不编辑 Convex Auth 账户行。请求必须命名两个不可变的 GitHub 提供商账户 ID；可变 handle 仅用作面向操作者的保护。

端点默认进行试运行。应用恢复需要在工作人员独立验证两个 GitHub 主体之间的连续性后设置 `dryRun: false` 和
`confirmIdentityVerified: true`。当目标用户当前的个人发布者拥有技能、包或 GitHub 技能来源时，恢复会以关闭方式失败。
恢复还会迁移已恢复发布者的技能、技能 slug 别名、包、包检查器警告和派生搜索摘要行中的旧版 `ownerUserId` 字段，以便直接所有者路径与新的发布者权限保持一致。已恢复 handle 的活动受保护 handle 保留也会重新分配给替代用户，使后续个人资料同步无法恢复前用户的竞争权限。每个主要表在每次应用事务中限制为
100 行；更大的恢复必须先使用可恢复的所有者迁移。GitHub 技能来源按发布者作用域处理，并报告为已检查，而不是被重写。
...
更改现有封禁存储的原因，而不解封或恢复内容（仅限管理员）。除非 `dryRun` 为 `false`，否则默认为试运行。
