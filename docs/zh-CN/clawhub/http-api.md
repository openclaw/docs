...
旧版 `/api/...` 和 `/api/cli/...` 保留用于兼容性（参见 `DEPRECATIONS.md`）。
OpenAPI：`/api/v1/openapi.json`。

## 公共目录复用

第三方目录可以使用公共读取端点来列出或搜索 ClawHub 技能。请缓存结果、遵守 `429`/`Retry-After`、将用户链接回规范的 ClawHub 列表页（`https://clawhub.ai/<owner>/skills/<slug>`），并避免暗示 ClawHub 为第三方站点背书。不要尝试在公共 API 表面之外镜像隐藏、私有或被审核屏蔽的内容。
...
- 当服务器知道原因时，已认证写入端点不应返回裸 `Unauthorized`。缺失令牌、无效/已撤销令牌，以及已删除/封禁/停用的账户都应得到可操作文本，以便 CLI 客户端能告知用户被阻止的原因。
...
- 如果没有可用的受信任客户端 IP，匿名请求会使用仅按速率限制类型限定范围的回退桶。这些回退桶不包含调用方提供的路径、slug、包名、版本、查询字符串或其他工件参数。

## 错误响应

公共 v1 错误响应是纯文本，带有 `content-type: text/plain; charset=utf-8`。
这包括验证失败（`400`）、缺失的公共资源（`404`）、认证和权限失败（`401`/`403`）、速率限制（`429`）以及被阻止的下载。客户端应将响应体读取为人类可读字符串。未知查询参数会为了兼容性被忽略，但已识别查询参数的无效值会返回 `400`。
...
- `q`（必填）：查询字符串
- `limit`（可选）：整数
- `highlightedOnly`（可选）：`true` 表示筛选为精选技能
...
- 结果按相关性顺序返回（嵌入相似度 + 精确 slug/名称 token 加权 + 少量流行度先验）。
- 相关性强于流行度。精确的 slug 或显示名称 token 匹配可以排在参与度强得多但匹配更宽松的结果之前。
- ASCII 文本会按单词和标点边界进行分词。例如，`personal-map` 包含独立的 `map` token，而 `amap-jsapi-skill` 包含 `amap`、`jsapi` 和 `skill`；因此搜索 `map` 会让 `personal-map` 比 `amap-jsapi-skill` 获得更强的词法匹配。
- 流行度按对数缩放并设置上限。当查询文本匹配较弱时，高参与度技能可能排名更低。
- 根据调用方筛选器和当前审核状态，可疑或隐藏的审核状态可能会从公共搜索中移除某个技能。

发布者可发现性指导：

- 将用户会实际搜索的术语放入显示名称、摘要和标签中。只有当独立 slug token 也是你想保留的稳定身份时才使用它。
- 不要仅为了追逐某个查询而重命名 slug，除非新 slug 是更好的长期规范名称。旧 slug 会成为重定向别名，但规范 URL、显示的 slug 和未来的搜索摘要会使用新 slug。
- 重命名别名会为旧 URL 和通过注册表解析的安装保留解析能力，但搜索排名基于重命名完成索引后的规范技能元数据。现有统计会保留在该技能上。
...
- `limit`（可选）：整数（1–200）
- `cursor`（可选）：除 `trending` 排序以外任何排序的分页游标
...
- 当 `nonSuspiciousOnly=true` 时，基于游标的排序可能在某页返回少于 `limit` 的项目，因为可疑技能是在页面检索后过滤的。
- 存在 `nextCursor` 时使用它继续分页。短页面本身不表示结果结束。
...
- 公共调用方只会对已经标记且可见的技能获得 `200`。
- 对公共调用方，证据会被脱敏；只有所有者/审核员才能看到原始片段。

### `POST /api/v1/skills/{slug}/report`

报告技能以供审核员审查。报告是技能级别的，可选关联到某个版本，并会进入技能报告队列。
...
`confirmed` 和 `dismissed` 需要 `note`；将 `status` 设置回 `open` 时可以省略。对已分诊报告传入 `finalAction: "hide"`，可在同一个可审计工作流中隐藏技能。
...
- 扫描作业是异步的。手动扫描请求的优先级高于普通发布/回填工作，但完成时间仍取决于工作进程可用性。
...
- 失败的扫描作业会返回带有 `lastError` 的 `status: "failed"`。

### `GET /api/v1/skills/-/scan/{scanId}/download`

已认证的报告归档端点。

- 需要已成功完成的扫描；非终态扫描返回 `409`。
...
- 需要对该技能或插件拥有所有者/发布者管理访问权限，或平台审核员/管理员权限。
...
仅管理员可用的规范批量重新扫描路由。它接受与旧版 `POST /api/v1/skills/-/rescan-batch` 相同的载荷形态。
...
- 只有当所选版本具有已生成的技能卡、未被审核按恶意软件拦截，并且 ClawScan 验证为干净时，`ok` 才为 `true`。
- 技能身份、发布者身份和所选版本元数据是顶层信封字段（`slug`、`displayName`、`publisherHandle`、`version`、`resolvedFrom`、`tag`、`createdAt`），以便 shell 自动化无需解包嵌套包装即可读取它们。
- `security` 是顶层 ClawScan/安全结论。自动化应以 `ok`、`decision`、`reasons` 和 `security.status` 为键。
- `security.signals` 包含支持性扫描器证据，例如 `staticScan`、`virusTotal` 和 `skillSpector`。
- `security.signals.dependencyRegistry` 为 v1 响应兼容性而保留，但依赖注册表存在性扫描器已退役，因此该键始终为 `null`。
- 只有在发布或导入期间 ClawHub 解析并存储了 GitHub 仓库/ref/commit/path 时，`provenance` 才为 `server-resolved-github-import`；否则为 `unavailable`。
...
- 响应仅包含安全信息。它不包含技能卡数据、已生成卡片状态、工件文件列表或详细扫描器载荷。
- `security.signals` 仅包含状态级支持证据；如需完整扫描器详情，请使用 `/scan` 或 ClawHub 安全审计页面。
- `security.signals.dependencyRegistry` 为 v1 响应兼容性而保留，但依赖注册表存在性扫描器已退役，因此该键始终为 `null`。
- 技能卡缺失不会影响此端点的 `ok`、`decision` 或 `reasons`；客户端需要卡片内容时，应在本地读取已安装的 `skill-card.md`。
- 需要单个技能的技能卡验证信封时使用 `/verify`，需要已生成的卡片 Markdown 时使用 `/card`，需要详细扫描器数据时使用 `/scan`。
...
- `category`（可选）：插件类别筛选器。仅当请求范围限定为插件包（`/api/v1/plugins`、`/api/v1/code-plugins`、`/api/v1/bundle-plugins`，或带有 `family=code-plugin`/`family=bundle-plugin` 的包端点）时受支持。受控类别和旧版 v1 筛选器别名记录在 `GET /api/v1/plugins` 下。
...
- skill 条目仍由 skill registry 支持，并且仍只能通过 `POST /api/v1/skills` 发布。
...
- `channel`（可选）：`official`、`community` 或 `private`
- `isOfficial`（可选）：`true` 或 `false`
- `category`（可选）：插件类别筛选器。仅当请求范围限定为插件包时受支持。受控类别和旧版 v1 筛选器别名记录在 `GET /api/v1/plugins` 下。
...
- 正文：ZIP 归档。
- 每个导出的 skill 根目录为 `{publisher}/{slug}/`。
- 托管 skills 包含最新存储版本文件，并在 `_manifest.json` 中以 `sourceRef: "public-clawhub"` 列出。
- 当前由 GitHub 支持且扫描结果为 `clean` 或 `suspicious` 的 skills 包含 `_source_handoff.json`，其中含有 `sourceRef: "public-github"`、仓库、提交、路径、内容哈希和归档 URL。它们不包含 ClawHub 托管的源文件。
...
- 结果按相关性顺序返回，目前不分页。
- 插件搜索的浏览器 UI 排序控件会重新排序已加载的相关性结果，与当前 `/skills` 浏览行为一致。
...
- `version.artifact.kind` 对旧世界包归档为 `legacy-zip`，对 ClawPack 支持的版本为 `npm-pack`。
- ClawPack 版本包含兼容 npm 的 `npmIntegrity`、`npmShasum` 和 `npmTarballName` 字段。
- `version.sha256hash` 是面向旧客户端的已弃用兼容性元数据。它对 `/api/v1/packages/{name}/download` 返回的精确 ZIP 字节进行哈希。现代客户端应使用 `version.artifact.sha256`，它标识规范版本工件。
- 当扫描数据存在时，会包含 `version.vtAnalysis`、`version.llmAnalysis` 和 `version.staticScan`。
- 私有包会返回 `404`，除非调用方可以读取所属发布者。

### `GET /api/v1/packages/{name}/versions/{version}/security`

返回面向安装客户端的精确包版本安全与信任摘要。这是公共 OpenClaw 消费表面，用于决定解析出的版本是否可以安装。
...
- `release.releaseId`、`release.version` 和 `release.createdAt` 标识已评估的精确版本。
- 当版本工件已知时，会提供 `release.artifactKind`、`release.artifactSha256`、`release.npmIntegrity`、`release.npmShasum` 和 `release.npmTarballName`。
- `trust.scanStatus` 是从扫描器输入和手动版本审核派生出的有效信任状态。
- `trust.moderationState` 可为空。当不存在手动版本审核时，它为 `null`。
- `trust.blockedFromDownload` 是安装阻止信号。OpenClaw 和其他安装客户端应在此值为 `true` 时阻止安装，而不是从扫描器或审核字段重新推导阻止规则。
- `trust.reasons` 是面向用户和审计的说明列表。原因代码是稳定、紧凑的字符串，例如 `manual:quarantined`、`scan:malicious` 和 `package:malicious`。
- `trust.pending` 表示一个或多个信任输入仍在等待完成。
- `trust.stale` 表示信任摘要是根据过期输入计算的，在做出高置信度允许决策前应视为需要刷新。

说明：

- 此端点精确到版本。客户端应在解析出打算安装的包版本后调用它，而不只是读取最新包元数据后调用。
- 私有包会返回 `404`，除非调用方可以读取所属发布者。
- 此端点有意比所有者/审核员审核端点更窄。它公开安装决策和公开说明，而不是举报者身份、举报正文、私有证据或内部审核时间线。
...
- 这只跟踪迁移就绪状态。它不会更改 OpenClaw 或生成 ClawPacks。
...
- `all`：任何带有手动覆盖、非 clean 扫描状态或包举报的版本。
...
举报一个包以供审核员审核。举报是包级别的，可选关联到某个版本。它们会进入审核队列，但自身不会自动隐藏或阻止下载；审核员应使用版本审核来批准、隔离或撤销工件。
...
用于 package 审核可见性的所有者/版主端点。

身份验证：

- 需要 package 所有者、publisher 成员、版主或管理员用户的 API token。
...
`note` 在 `confirmed` 和 `dismissed` 时是必需的；将 `status` 设回 `open` 时可以省略。传入 `finalAction: "quarantine"` 或 `finalAction: "revoke"`，并配合已确认的举报，以便在同一个可审计工作流中应用发布审核。
...
用于 package release 审核的版主/管理员端点。
...
- `revoked`：release 先前受信任后被阻止。

隔离和撤销的 release 会从 artifact 下载路由返回 `403`。每次更改都会写入一条审计日志记录。
...
- 二进制文件返回 `415`。
- 文件大小限制：200KB。
- 待处理的 VirusTotal 扫描不会阻止读取；恶意 release 仍可能在其他位置被扣留。
- 私有 package 返回 `404`，除非调用方可以读取所属 publisher。

### `GET /api/v1/packages/{name}/download`

下载 package release 的旧版确定性 ZIP 归档。
...
- Plugin/package 归档是带有 `package/` 根目录的 zip 文件，因此旧版 OpenClaw 客户端可继续工作。
- 此路由保持仅支持 ZIP。它不会流式传输 ClawPack `.tgz` 文件。
- 响应包含 `ETag`、`Digest`、`X-ClawHub-Artifact-Type` 和 `X-ClawHub-Artifact-Sha256` 标头，用于 resolver 完整性检查。
- 仅 registry 使用的元数据不会注入下载的归档中。
- 待处理的 VirusTotal 扫描不会阻止下载；恶意 release 返回 `403`。
- 私有 package 返回 `404`，除非调用方是所有者。

### `GET /api/npm/{package}`

返回由 ClawPack 支持的 package 版本的 npm 兼容 packument。
...
- Scoped package packument 同时支持 `/api/npm/@scope/name` 和 npm 的编码请求路径 `/api/npm/@scope%2Fname`。
...
- 审核和私有 package 访问检查仍然适用。

### `GET /api/v1/resolve`

CLI 用于将本地 fingerprint 映射到已知版本。
...
下载托管的 skill 版本 ZIP，或对于当前 GitHub 支持、扫描结果为 `clean` 或 `suspicious` 且没有托管版本的 skill，返回 GitHub 源码交接信息。
...
- 如果未提供 `version` 或 `tag`，则使用最新版本。
- 软删除的版本返回 `410`。
- GitHub 支持的 skill 交接不会代理或镜像字节。JSON 响应包含 `sourceRef: "public-github"`、`repo`、`commit`、`path`、`contentHash` 和 `archiveUrl`；扫描/当前状态是闸门，不会作为成功载荷元数据包含。
...
- 可选 payload 字段：`ownerHandle`。存在时，API 会在服务端解析该 publisher，并要求 actor 拥有 publisher 访问权限。
- 可选 payload 字段：`migrateOwner`。当它与 `ownerHandle` 一起为 `true` 时，如果 actor 同时是当前和目标 publisher 的管理员/所有者，现有 skill 可以迁移到该所有者。如果没有此选择加入，所有者更改会被拒绝。
...
- 使用 `files` 或 `clawpack` 之一，不能在同一请求中同时使用。
- JSON body 和调用方提供的 `payload.files` / `payload.artifact` 元数据会被拒绝。
- 直接 multipart 发布请求上限为 18MB。ClawPack tarball 可通过 upload-url 流程使用，最高为 120MB tarball 上限。
- 可选 payload 字段：`ownerHandle`。存在时，只有管理员可以代表该所有者发布。

验证重点：

- `family` 必须是 `code-plugin` 或 `bundle-plugin`。
- 插件 package 需要 `openclaw.plugin.json`。ClawPack `.tgz` 上传必须在 `package/openclaw.plugin.json` 中包含它。
- 代码插件需要 `package.json`、源码仓库元数据、源码 commit 元数据、配置 schema 元数据、`openclaw.compat.pluginApi` 和 `openclaw.build.openclawVersion`。
- `openclaw.hostTargets` 和 `openclaw.environment` 是可选元数据。
- 只有 `openclaw` 组织 publisher 和当前 `openclaw` 组织成员的个人 publisher 可以发布到 `official` 渠道。
- 代为发布仍会根据目标所有者账号验证 official-channel 资格。
...
存在时，`reason` 会存储为 skill 审核备注，并复制到审计日志中。所有者发起的软删除会保留 slug 30 天，之后该 slug 可由另一个 publisher 认领。删除响应在此到期规则适用时包含 `slugReservedUntil`。版主/管理员隐藏和安全移除不会以这种方式到期。
...
仅管理员。确保某个 handle 存在组织 publisher。如果该 handle 仍指向旧版共享用户/个人 publisher，该端点会先将其迁移为组织 publisher。对于新创建的组织，请提供 `memberHandle`；执行操作的管理员不会被添加为成员。`memberRole` 默认为 `owner`。
...
经过身份验证的自助组织 publisher 创建。创建新的组织 publisher，并将调用方添加为所有者。此端点不会迁移现有用户/个人 handle，也不会将 publisher 标记为受信任/官方。

- Body：`{ "handle": "opik", "displayName": "Opik" }`
- 响应：`{ "ok": true, "publisherId": "...", "handle": "opik", "created": true, "trusted": false }`
- 当 handle 已被 publisher、用户或个人 publisher 使用时，返回 `409`。

### `POST /api/v1/users/reserve`

仅管理员。为合法所有者预留 root slug 和 package 名称，而不发布 release。Package 名称会变成没有 release 行的私有占位 package，因此同一所有者之后可以将真正的 code-plugin 或 bundle-plugin release 发布到该名称。
...
仅管理员。为已验证的替换 GitHub OAuth principal 恢复个人 publisher，而不编辑 Convex Auth 账号行。请求必须命名两个不可变 GitHub provider account id；可变 handle 只用作面向操作员的保护检查。

端点默认使用试运行。应用恢复需要在工作人员独立验证两个 GitHub 主体之间的连续性后，设置 `dryRun: false` 和
`confirmIdentityVerified: true`。
当目标用户当前的个人发布者拥有技能、包或 GitHub 技能来源时，恢复会失败并保持关闭。
恢复还会迁移已恢复发布者的技能、技能 slug 别名、包、包检查器警告以及派生搜索摘要行中的旧版 `ownerUserId` 字段，使
直接所有者路径与新的发布者权限保持一致。已恢复 handle 的有效受保护 handle
预留也会重新分配给替换用户，这样后续
资料同步无法恢复前用户的竞争性权限。每个主表在每次应用事务中限制为
100 行；更大的恢复必须先使用可恢复的所有者迁移。
GitHub 技能来源按发布者划分作用域，并报告为已检查，而不是被重写。
...
在不解除封禁或恢复内容的情况下，更改现有封禁存储的原因（仅限管理员）。除非 `dryRun` 为 `false`，否则默认使用试运行。
...
## Registry 发现（`/.well-known/clawhub.json`）

CLI 可以从站点发现 registry/认证设置：
