...
x-i18n:
    generated_at: "2026-07-05T01:53:52Z"
    model: gpt-5.5
...
- 当服务器知道原因时，已认证写入端点不应返回裸 `Unauthorized`。缺失令牌、无效/已撤销令牌，以及已删除/已封禁/已禁用账号都应获得可操作的文本，以便 CLI 客户端可以告诉用户是什么阻止了他们。

- 读取：每个 IP 3000/分钟，每个密钥 12000/分钟
- 写入：每个 IP 300/分钟，每个密钥 3000/分钟
- 下载：每个 IP 1200/分钟，每个密钥 6000/分钟（下载端点）
...
- 如果没有可用的受信任客户端 IP，匿名请求会使用仅按速率限制类型划分范围的回退桶。这些回退桶不包含调用方提供的路径、slug、包名、版本、查询字符串或其他工件参数。

## 错误响应

公共 v1 错误响应是纯文本，`content-type: text/plain; charset=utf-8`。
这包括验证失败（`400`）、缺失公共资源（`404`）、身份验证和权限失败（`401`/`403`）、速率限制（`429`）以及被阻止的下载。客户端应将响应正文读取为人类可读字符串。未知查询参数会被忽略以保持兼容性，但已识别查询参数带无效值时会返回 `400`。
...
- 相关性强于流行度。精确的 slug 或显示名称 token 匹配可以排在互动更强但匹配更宽泛的结果之前。
- ASCII 文本会按词和标点边界分词。例如，`personal-map` 包含独立的 `map` token，而 `amap-jsapi-skill` 包含 `amap`、`jsapi` 和 `skill`；因此搜索 `map` 会让 `personal-map` 比 `amap-jsapi-skill` 获得更强的词法匹配。
- 流行度经过对数缩放并设有上限。当查询文本匹配较弱时，高互动技能也可能排名更低。
- 根据调用方筛选条件和当前审核状态，可疑或隐藏的审核状态可能会使技能从公共搜索中移除。
...
- `trending` 按最近 7 天的安装量排序（基于遥测）。
- `createdAt` 对新技能抓取是稳定的；`updated` 会在现有技能重新发布时变化。
- 当 `nonSuspiciousOnly=true` 时，基于游标的排序可能在一页中返回少于 `limit` 的项目，因为可疑技能会在页面取回后被过滤。
...
- 证据会为公共调用方做脱敏处理，并且只有所有者/审核员才会看到原始片段。
...
`note` 对 `confirmed` 和 `dismissed` 是必需的；将 `status` 设回 `open` 时可以省略。传入 `finalAction: "hide"` 与已分诊报告一起使用，可在同一个可审计工作流中隐藏该技能。
...
- `version.security` 在可用时包含规范化扫描验证状态和扫描器详情
  （VirusTotal + LLM）。
...
- 失败的扫描任务会返回 `status: "failed"` 以及 `lastError`。
...
- 需要对技能或插件拥有所有者/发布者管理权限，或拥有平台审核员/管理员权限。
...
- 只有当选中的版本已生成技能卡、未被审核标记为恶意软件阻止，并且 ClawScan 验证为干净时，`ok` 才为 `true`。
...
- 响应仅包含安全信息。它不包含技能卡数据、生成的卡状态、工件文件列表或详细扫描器载荷。
- `security.signals` 只包含状态级别的支持证据；完整扫描器详情请使用 `/scan` 或 ClawHub 安全审计页面。
- `security.signals.dependencyRegistry` 为 v1 响应兼容性而保留，但依赖注册表存在性扫描器已退役，此键始终为 `null`。
- 技能卡缺失不会影响此端点的 `ok`、`decision` 或 `reasons`；当客户端需要卡内容时，应在本地读取已安装的 `skill-card.md`。
- 需要单技能技能卡验证信封时使用 `/verify`，需要生成的卡 Markdown 时使用 `/card`，需要详细扫描器数据时使用 `/scan`。
...
- `GET /api/v1/plugins` 下记录的旧版 v1 筛选器别名也会
  被接受。
- 类别筛选是由插件类别摘要
  行支持的真实 API 筛选器，而不是搜索查询改写。
- 结果按相关性顺序返回，目前不分页。
...
- `version.artifact.kind` 对旧式软件包归档为 `legacy-zip`，
  对基于 ClawPack 的发布版本为 `npm-pack`。
...
返回安装客户端所需的精确软件包发布安全与信任摘要。
这是 OpenClaw 用于判断已解析发布版本是否可安装的公开消费接口。
...
- 此端点精确到版本。客户端应在解析出打算安装的
  软件包版本之后调用它，而不是只在读取最新
  软件包元数据之后调用。
- 私有软件包会返回 `404`，除非调用方可以读取所属发布者。
- 此端点有意比所有者/版主审核
  端点更窄。它暴露安装决定和公开解释，而不是
...
- 旧版软件包版本会返回 `legacy-zip` 构件和旧版 ZIP
  `downloadUrl`。
- ClawPack 版本会返回 `npm-pack` 构件、npm 完整性字段、
...
- `packageName` 会按 npm 名称规范化；对于计划中的
  迁移，软件包可以不存在。
- 这仅跟踪迁移就绪状态。它不会改变 OpenClaw 或生成
  ClawPack。

### `GET /api/v1/packages/moderation/queue`

用于软件包发布审查队列的版主/管理员端点。
...
- `all`：任何带有手动覆盖、非干净扫描状态或软件包举报的发布版本。
...
对于 `confirmed` 和 `dismissed`，`note` 为必填；将 `status` 设回 `open` 时可以省略。传入 `finalAction: "quarantine"` 或 `finalAction: "revoke"`，并配合已确认的报告，即可在同一套可审计工作流中应用发布审核。
...
- `revoked`：在某个发布先前受信任之后阻止。

隔离和撤销的发布会从构件下载路由返回 `403`。每次变更都会写入一条审计日志条目。
...
下载托管的 skill 版本 ZIP，或在当前 GitHub 支持的 skill 扫描为 `clean` 或 `suspicious` 且没有托管版本时，返回 GitHub 源码交接信息。
...
- 可选 payload 字段：`migrateOwner`。当它与 `ownerHandle` 一起为 `true` 时，如果执行者同时是当前发布者和目标发布者的管理员/所有者，现有 skill 可以迁移到该所有者。没有此选择加入时，所有者变更会被拒绝。
...
- 允许的表单字段为 `payload`、重复的 `files` blob，或一个 `clawpack` tarball 引用。`clawpack` 可以是 `.tgz` blob，也可以是上传 URL 流程返回的 storage id。暂存的 storage-id 发布还必须包含随该上传 URL 返回的 `clawpackUploadTicket`。
- 使用 `files` 或 `clawpack`，同一请求中绝不能同时使用两者。
...
所有者发起的软删除会保留 slug 30 天，之后该 slug 可由其他发布者认领。删除响应会在适用此过期机制时包含 `slugReservedUntil`。
...
仅管理员。确保某个 handle 存在组织发布者。如果该 handle 仍指向旧版共享用户/个人发布者，该端点会先将其迁移为组织发布者。对于新创建的组织，请提供 `memberHandle`；执行操作的管理员不会被添加为成员。`memberRole` 默认为 `owner`。
...
仅管理员。为合法所有者保留根 slug 和软件包名称，而不发布 release。软件包名称会变成没有 release 行的私有占位软件包，因此同一所有者之后可以将真正的 code-plugin 或 bundle-plugin 发布到该名称。
...
仅管理员。为已验证的替代 GitHub OAuth principal 恢复个人发布者，而不编辑 Convex Auth 账号行。请求必须命名两个不可变 GitHub provider account id；可变 handle 仅用作面向操作员的保护检查。
...
恢复还会迁移已恢复发布者的技能、技能 slug 别名、包、包检查器警告和派生搜索摘要行中的旧版 `ownerUserId` 字段，使
直接所有者路径与新的发布者权限保持一致。已恢复句柄的活动受保护句柄
预留也会重新分配给替换用户，因此后续
个人资料同步无法恢复前用户的竞争权限。每个主表在每次应用事务中限制为
100 行；更大的恢复必须先使用可恢复的所有者迁移。
GitHub 技能来源属于发布者作用域，并报告为已检查，而不是被重写。
