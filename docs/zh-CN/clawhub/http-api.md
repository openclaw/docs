...
旧版 `/api/...` 和 `/api/cli/...` 会为兼容性保留（见 `DEPRECATIONS.md`）。
OpenAPI：`/api/v1/openapi.json`。

## 公共目录复用

第三方目录可以使用公共读取端点来列出或搜索 ClawHub 技能。请缓存结果，遵守 `429`/`Retry-After`，将用户链接回规范的 ClawHub 列表页（`https://clawhub.ai/<owner>/skills/<slug>`），并避免暗示 ClawHub 为第三方站点背书。不要尝试在公共 API 表面之外镜像隐藏、私有或被审核拦截的内容。
...
- 当服务器知道原因时，已认证写入端点不应返回裸的 `Unauthorized`。缺失 token、无效/已吊销 token，以及已删除/被封禁/已禁用的账户都应获得可操作文本，以便 CLI 客户端能告诉用户是什么阻止了他们。
...
这包括验证失败（`400`）、缺失公共资源（`404`）、认证和权限失败（`401`/`403`）、速率限制（`429`）以及被拦截的下载。客户端应将响应正文读取为人类可读字符串。为兼容性，未知查询参数会被忽略，但带有无效值的已识别查询参数会返回 `400`。
...
- `q`（必需）：查询字符串
- `limit`（可选）：整数
- `highlightedOnly`（可选）：设为 `true` 以过滤为高亮技能
...
- 结果按相关性顺序返回（嵌入相似度 + 精确 slug/名称 token 提升 + 小的热度先验）。
- 相关性强于热度。精确的 slug 或显示名称 token 匹配可能排在互动更强但匹配更宽泛的结果前面。
- ASCII 文本会按单词和标点边界分词。例如，`personal-map` 包含独立的 `map` token，而 `amap-jsapi-skill` 包含 `amap`、`jsapi` 和 `skill`；因此搜索 `map` 时，`personal-map` 的词法匹配强于 `amap-jsapi-skill`。
- 热度经过对数缩放并设有上限。当查询文本匹配较弱时，高互动技能也可能排名更低。
- 可疑或隐藏的审核状态可能会根据调用方过滤器和当前审核状态，将技能从公共搜索中移除。

发布者可发现性指南：

- 将用户会逐字搜索的术语放入显示名称、摘要和标签中。仅当独立 slug token 也是你想保留的稳定身份时才使用它。
- 不要为了追逐某个查询而重命名 slug，除非新 slug 是更好的长期规范名称。旧 slug 会成为重定向别名，但规范 URL、显示的 slug 和未来的搜索摘要会使用新 slug。
- 重命名别名会为旧 URL 和通过注册表解析的安装保留解析能力，但搜索排名基于重命名被索引后的规范技能元数据。现有统计会留在该技能上。
- 如果某个技能意外不可见，请先在登录状态下用 `clawhub inspect @owner/slug` 检查审核状态，再更改与排名相关的元数据。
...
- `recommended` 使用互动和近期信号。
- `trending` 按过去 7 天的安装量排名（基于遥测）。
- `createdAt` 对新技能抓取是稳定的；`updated` 会在现有技能重新发布时变化。
- 当 `nonSuspiciousOnly=true` 时，基于游标的排序可能在某一页返回少于 `limit` 个条目，因为可疑技能会在页面取回后被过滤。
...
- 对公共调用方，证据会被脱敏；只有所有者/审核员会包含原始片段。

### `POST /api/v1/skills/{slug}/report`

举报技能以供审核员审查。举报是技能级别的，可以选择链接到某个版本，并会进入技能举报队列。
...
`confirmed` 和 `dismissed` 需要 `note`；将 `status` 设回 `open` 时可以省略。将 `finalAction: "hide"` 与已分诊的举报一起传入，可在同一个可审计工作流中隐藏技能。
...
- 可用时，`version.security` 包含规范化的扫描验证状态和扫描器详情（VirusTotal + LLM）。
...
- 扫描作业是异步的。手动扫描请求的优先级高于正常发布/回填工作，但完成时间仍取决于 worker 可用性。
...
- 只有当选定版本具有生成的 Skill Card、未被审核按恶意软件阻止，并且 ClawScan 验证为 clean 时，`ok` 才为 `true`。
...
- `security` 是顶层 ClawScan/security 结论。自动化应基于 `ok`、`decision`、`reasons` 和 `security.status` 做判断。
...
- Skill 条目仍由 Skills 注册表支持，并且仍只能通过 `POST /api/v1/skills` 发布。
- `POST /api/v1/packages` 仍仅用于 code-plugin 和 bundle-plugin 发布。
- 匿名调用方只能看到公开包频道。
- 经过身份验证的调用方可以在列表/搜索结果中看到其所属发布者的私有包。
...
- `family`（可选）：`code-plugin` 或 `bundle-plugin`。省略表示两个插件系列都包括。

响应：

- 正文：ZIP 归档。
- 每个导出的插件以 `{family}/{packageName}/` 为根目录。
- 每个导出的插件都包含最新发布版本中存储的文件。
...
- `limit`（可选）：整数（1-100）
- `isOfficial`（可选）：`true` 或 `false`
- `category`（可选）：插件类别过滤器。当前值：`channels`、`models`、`memory`、`context`、`voice`、`media`、`web`、`tools`、`runtime`、`gateway`、`security`、`other`。
...
- `version.sha256hash` 是面向旧客户端的已弃用兼容性元数据。它会哈希 `/api/v1/packages/{name}/download` 返回的精确 ZIP 字节。现代客户端应使用 `version.artifact.sha256`，它标识规范发布工件。
- 当扫描数据存在时，会包含 `version.vtAnalysis`、`version.llmAnalysis` 和 `version.staticScan`。
- 私有包会返回 `404`，除非调用方可以读取所属发布者。

### `GET /api/v1/packages/{name}/versions/{version}/security`

返回供安装客户端使用的精确包发布安全和信任摘要。这是公开的 OpenClaw 消费表面，用于决定已解析的发布版本是否可以安装。
...
- `trust.stale` 表示信任摘要是根据过期输入计算的，在做出高置信度允许决策之前，应视为需要刷新。

说明：

- 此端点精确到版本。客户端应在解析出打算安装的包版本后调用它，而不是只在读取最新包元数据后调用。
- 私有包会返回 `404`，除非调用方可以读取所属发布者。
- 此端点故意比所有者/审核员审核端点更窄。它公开安装决策和公开说明，而不是举报者身份、举报正文、私有证据或内部审核时间线。
...
- 这是 OpenClaw 解析器表面；它避免根据共享 URL 猜测归档格式。
...
- `phase`（可选）：`planned`、`published`、`clawpack-ready`、`legacy-zip-only`、`metadata-ready`、`blocked`、`ready-for-openclaw` 或 `all`（默认）。
- `limit`（可选）：整数（1-100）
- `cursor`（可选）：分页游标
...
- `packageName` 会按 npm-name 规范化；对于计划中的迁移，包可以缺失。
...
举报包以供审核员审核。举报是包级别的，可以选择关联到某个版本。它们会进入审核队列，但本身不会自动隐藏或阻止下载；审核员应使用发布审核来批准、隔离或撤销工件。
...
对于 `confirmed` 和 `dismissed`，必须提供 `note`；将 `status` 设回 `open` 时可以省略。对于已确认的举报，传入 `finalAction: "quarantine"` 或 `finalAction: "revoke"` 可在同一个可审计工作流中应用发布版本审核。
...
- `revoked`：此前受信任的发布版本现已被阻止。

被隔离和撤销的发布版本会在工件下载路由返回 `403`。每次变更都会写入一条审计日志记录。
...
下载托管的技能版本 ZIP，或者对于具备 `clean` 或 `suspicious` 扫描结果且没有托管版本的当前 GitHub 支持技能，返回 GitHub 源交接信息。
...
- 可选载荷字段：`migrateOwner`。当与 `ownerHandle` 一起设为 `true` 时，如果操作者同时是当前发布者和目标发布者的管理员/所有者，现有技能可以迁移到该所有者。如果没有此显式选择，所有者变更会被拒绝。
...
- 允许的表单字段为 `payload`、重复的 `files` blob，或一个 `clawpack` tarball 引用。`clawpack` 可以是 `.tgz` blob，也可以是 upload-url 流程返回的存储 ID。使用暂存 storage-id 发布时，还必须包含随该上传 URL 返回的 `clawpackUploadTicket`。
- 使用 `files` 或 `clawpack` 其中之一，不要在同一个请求中同时使用二者。
...
- 代表他人发布仍会根据目标所有者账号验证 official-channel 资格。
...
仅管理员。为合法所有者保留根 slug 和软件包名称，而不发布发布版本。软件包名称会成为没有发布版本行的私有占位软件包，因此同一所有者以后可以将真实的 code-plugin 或 bundle-plugin 发布版本发布到该名称下。
...
该端点默认采用试运行。应用恢复需要在工作人员独立验证两个 GitHub 主体之间的连续性后设置 `dryRun: false` 和
`confirmIdentityVerified: true`。当目标用户当前的个人
发布者已有技能、包或 GitHub 技能来源时，恢复会失败并保持关闭状态。
恢复还会迁移被恢复发布者的技能、技能 slug 别名、包、包检查器警告和派生搜索摘要行中的旧版 `ownerUserId` 字段，使
直接所有者路径与新的发布者权限保持一致。被恢复 handle 的活动受保护 handle
预留也会重新分配给替换用户，避免之后的个人资料同步恢复前用户的竞争性权限。每个主表在每个应用事务中限制为
...
参见 `DEPRECATIONS.md` 获取移除计划。

`POST /api/cli/upload-url` 会返回 `uploadUrl` 和 `uploadTicket`。暂存 ClawPack tar 包的
包发布必须将生成的存储 ID 作为
