...
x-i18n:
    generated_at: "2026-07-06T10:46:51Z"
    model: gpt-5.5
...
第三方目录可以使用公开读取端点来列出或搜索 ClawHub Skills。请缓存结果，遵守 `429`/`Retry-After`，将用户链接回规范的 ClawHub 列表页（`https://clawhub.ai/<owner>/skills/<slug>`），并避免暗示 ClawHub 为第三方站点背书。不要尝试在公开 API 表面之外镜像隐藏、私有或被审核屏蔽的内容。
...
- 当服务器知道原因时，已认证写入端点不应返回裸 `Unauthorized`。缺失令牌、无效/已撤销令牌，以及已删除/封禁/禁用的账号都应获得可操作的文本，以便 CLI 客户端能告诉用户阻止原因。
...
- `X-RateLimit-Remaining` / `RateLimit-Remaining`：存在时为精确的剩余额度。
...
这包括验证失败（`400`）、缺失的公开资源（`404`）、认证和权限失败（`401`/`403`）、速率限制（`429`）以及被屏蔽的下载。客户端应将响应正文读取为人类可读字符串。未知查询参数会出于兼容性被忽略，但具有无效值的已识别查询参数会返回 `400`。
...
- 结果按相关性顺序返回（嵌入相似度 + 精确 slug/名称词元加权 + 小幅流行度先验）。
- 相关性强于流行度。精确 slug 或显示名称词元匹配可以排在参与度高得多但匹配更宽松的结果之前。
- ASCII 文本会按单词和标点边界分词。例如，`personal-map` 包含独立的 `map` 词元，而 `amap-jsapi-skill` 包含 `amap`、`jsapi` 和 `skill`；因此搜索 `map` 会让 `personal-map` 比 `amap-jsapi-skill` 获得更强的词法匹配。
- 流行度经过对数缩放并设有上限。当查询文本匹配较弱时，高参与度 Skills 的排名可能更低。
- 可疑或隐藏的审核状态可能会根据调用方筛选器和当前审核状态，从公开搜索中移除某个 Skill。

发布者可发现性指南：

- 将用户会逐字搜索的术语放入显示名称、摘要和标签中。仅当独立 slug 词元也是你想保留的稳定身份时才使用它。
- 不要仅为了追逐某个查询而重命名 slug，除非新 slug 是更好的长期规范名称。旧 slug 会变成重定向别名，但规范 URL、显示的 slug 和未来搜索摘要会使用新 slug。
- 重命名别名会为通过注册表解析的旧 URL 和安装保留解析能力，但搜索排名基于重命名索引完成后的规范 Skill 元数据。现有统计数据会保留在该 Skill 上。
- 如果某个 Skill 意外不可见，请先在登录状态下使用 `clawhub inspect @owner/slug` 检查审核状态，再更改排名相关元数据。
...
- 当 `nonSuspiciousOnly=true` 时，基于游标的排序可能会在某一页返回少于 `limit` 的项目，因为可疑 Skills 会在页面检索后被过滤。
- 存在 `nextCursor` 时，使用它继续分页。短页面本身并不表示结果结束。
...
- `moderation` 仅在 Skill 被标记或所有者正在查看它时包含。
...
- 公开调用方只能对已标记且可见的 Skills 获得 `200`。
- 对公开调用方会遮蔽证据，并且仅对所有者/审核员包含原始片段。

### `POST /api/v1/skills/{slug}/report`

报告某个 Skill 以供审核员审查。报告处于 Skill 级别，可选择链接到某个版本，并会进入 Skill 报告队列。
...
`note` 对 `confirmed` 和 `dismissed` 是必需的；将 `status` 设置回 `open` 时可以省略。传递 `finalAction: "hide"` 可在同一个可审计工作流中隐藏已分诊报告对应的 Skill。
...
- 要求对该技能或插件拥有所有者/发布者管理访问权限，或拥有平台审核员/管理员权限。
...
- 只有当所选版本已生成 Skill Card、未被审核标记为恶意软件阻止，并且 ClawScan 验证为干净时，`ok` 才为 `true`。
...
- `security` 是顶层 ClawScan/安全判定。自动化应基于 `ok`、`decision`、`reasons` 和 `security.status` 做判断。
- `security.signals` 包含支持性的扫描器证据，例如 `staticScan`、`virusTotal` 和 `skillSpector`。
- 为兼容 v1 响应，保留了 `security.signals.dependencyRegistry`，但依赖注册表存在性扫描器已退役，此键始终为 `null`。
...
- 响应仅包含安全信息。它不包含 Skill Card 数据、已生成卡片状态、构件文件列表或详细扫描器载荷。
...
- 当你需要单个技能的 Skill Card 验证信封时，使用 `/verify`；当你需要生成的卡片 Markdown 时，使用 `/card`；当你需要详细扫描器数据时，使用 `/scan`。
...
- `category`（可选）：插件类别过滤器。仅当请求限定到插件包（`/api/v1/plugins`、`/api/v1/code-plugins`、`/api/v1/bundle-plugins`，或带有 `family=code-plugin`/`family=bundle-plugin` 的包端点）时支持。受控类别和旧版 v1 过滤器别名记录在 `GET /api/v1/plugins` 下。
...
- `channel`（可选）：`official`、`community` 或 `private`
- `isOfficial`（可选）：`true` 或 `false`
- `category`（可选）：插件类别过滤器。仅当请求限定到插件包时支持。受控类别和旧版 v1 过滤器别名记录在 `GET /api/v1/plugins` 下。
...
- 当前由 GitHub 支持且扫描结果为 `clean` 或 `suspicious` 的技能包含 `_source_handoff.json`，其中含有 `sourceRef: "public-github"`、仓库、提交、路径、内容哈希和归档 URL。它们不包含 ClawHub 托管的源文件。
...
在 code-plugin 和 bundle-plugin 包中执行仅插件搜索。
...
- 类别过滤是真正的 API 过滤器，由插件类别摘要行支持，而不是搜索查询重写。
- 结果按相关性顺序返回，目前不分页。
- 浏览器 UI 中的插件搜索排序控件会重排已加载的相关性结果，
  与当前 `/skills` 浏览行为一致。
...
- 对于旧世界包归档，`version.artifact.kind` 为 `legacy-zip`；对于基于 ClawPack 的发布，为
  `npm-pack`。
- ClawPack 发布包含与 npm 兼容的 `npmIntegrity`、`npmShasum` 和
  `npmTarballName` 字段。
...
返回面向安装客户端的精确包发布安全和信任摘要。这是 OpenClaw 用于判断已解析发布能否安装的公开消费表面。
...
- 当发布制品的信息已知时，会出现 `release.artifactKind`、`release.artifactSha256`、`release.npmIntegrity`、
...
- `trust.blockedFromDownload` 是安装阻止信号。当此值为 `true` 时，OpenClaw 和其他安装客户端应阻止安装，而不是从扫描器或审核字段重新派生阻止规则。
...
- 此端点精确到版本。客户端应在解析出打算安装的包版本后调用它，而不是只读取最新包元数据后调用。
- 私有包返回 `404`，除非调用方可以读取所属发布者。
- 此端点有意比所有者/版主审核端点更窄。它公开安装决策和公开解释，而不公开举报者身份、举报正文、私有证据或内部审核时间线。
...
- `open`：可疑、恶意、待处理、已隔离、已撤销或已被举报的发布。
...
举报一个包以供版主审核。举报属于包级别，可选择关联到某个版本。它们会进入审核队列，但本身不会自动隐藏或阻止下载；版主应使用发布审核来批准、隔离或撤销制品。
...
对于 `confirmed` 和 `dismissed`，`note` 为必填；将 `status` 重新设为 `open` 时可以省略。传入 `finalAction: "quarantine"` 或 `finalAction: "revoke"` 并配合已确认的举报，可在同一个可审计工作流中应用发布审核。
...
被隔离和被撤销的发布版本会在制品下载路由返回 `403`。每次变更都会写入一条审计日志记录。
...
- 二进制文件返回 `415`。
- 文件大小限制：200KB。
- 待处理的 VirusTotal 扫描不会阻止读取；恶意发布版本仍可能在其他地方被拦截。
- 私有包返回 `404`，除非调用方可以读取所属发布者。
...
下载托管的 skill 版本 ZIP，或对于当前由 GitHub 支撑、扫描结果为 `clean` 或 `suspicious` 且没有托管版本的 skill，返回 GitHub 源码交接信息。
...
- 可选载荷字段：`migrateOwner`。当其与 `ownerHandle` 同时为 `true` 时，如果操作者同时是当前发布者和目标发布者的管理员/所有者，现有 skill 可以迁移到该所有者。没有此显式选择时，所有者变更会被拒绝。
...
- 允许的表单字段为 `payload`、重复的 `files` blob，或一个 `clawpack` tarball 引用。`clawpack` 可以是 `.tgz` blob，也可以是上传 URL 流程返回的存储 ID。使用暂存 storage-id 发布时，还必须包含该上传 URL 一起返回的 `clawpackUploadTicket`。
- 只能使用 `files` 或 `clawpack` 之一，绝不能在同一请求中同时使用。
- JSON 正文以及调用方提供的 `payload.files` / `payload.artifact` 元数据会被拒绝。
- 直接 multipart 发布请求上限为 18MB。ClawPack tarball 可以使用上传 URL 流程，最高到 120MB tarball 上限。
...
- 代发布仍会根据目标所有者账号验证官方渠道资格。

### `DELETE /api/v1/skills/{slug}` / `POST /api/v1/skills/{slug}/undelete`

软删除/恢复一个 skill（所有者、版主或管理员）。
...
所有者发起的软删除会保留 slug 30 天，之后该 slug 可被另一个发布者认领。
当此到期规则适用时，删除响应会包含 `slugReservedUntil`。
...
仅限管理员。为合法所有者预留根 slug 和包名，而不发布发布版本。包名会变成没有发布版本行的私有占位包，因此同一所有者稍后可以将真正的 code-plugin 或 bundle-plugin 发布版本发布到该名称。
...
仅限管理员。为已验证的替代 GitHub OAuth 主体恢复个人发布者，而不编辑 Convex Auth 账号行。请求必须指定两个不可变的 GitHub 提供商账号 ID；可变 handle 仅用作面向操作者的保护检查。
...
恢复还会迁移已恢复发布者的技能、技能 slug 别名、包、包检查器警告以及派生搜索摘要行中的旧版 `ownerUserId` 字段，使直接所有者路径与新的发布者权限保持一致。已恢复 handle 的活动受保护 handle 预留也会重新分配给替代用户，这样后续的个人资料同步无法恢复前用户的竞争性权限。每个主表在每次应用事务中限制为
...
内容（仅限管理员）。除非 `dryRun` 为 `false`，否则默认为 dry-run。
