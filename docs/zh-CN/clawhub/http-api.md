...
x-i18n:
    generated_at: "2026-07-02T22:21:36Z"
    model: gpt-5.5
...
旧版 `/api/...` 和 `/api/cli/...` 会保留用于兼容性（参见 `DEPRECATIONS.md`）。
OpenAPI：`/api/v1/openapi.json`。

## 公共目录复用

第三方目录可以使用公共读取端点来列出或搜索 ClawHub skills。请缓存结果，遵守 `429`/`Retry-After`，将用户链接回规范的 ClawHub 列表页（`https://clawhub.ai/<owner>/skills/<slug>`），并避免暗示 ClawHub 为第三方站点背书。不要尝试在公共 API 表面之外镜像隐藏、私有或被审核阻止的内容。
...
- 匿名请求：按 IP 执行。
- 已认证请求（有效的 Bearer token）：按用户 bucket 执行。
- 如果 token 缺失/无效，行为会回退到 IP 执行。
- 当服务器知道原因时，已认证写入端点不应返回裸 `Unauthorized`。缺失 token、无效/已撤销 token，以及已删除/封禁/禁用账户都应获得可操作文本，以便 CLI 客户端能告诉用户是什么阻止了它们。
...
- `X-RateLimit-Remaining` / `RateLimit-Remaining`：存在时表示精确的剩余额度。
  分片的成功请求会省略此 header，而不是返回近似的全局值。
...
- 仅当部署显式启用受信任的转发 headers 时，才使用受信任的客户端 IP headers，包括 `cf-connecting-ip`。
- ClawHub 使用受信任的转发 headers 在边缘识别客户端 IP。
- 如果没有可用的受信任客户端 IP，匿名请求会使用仅按速率限制类型划分作用域的 fallback buckets。这些 fallback buckets 不包含调用方提供的路径、slug、包名、版本、查询字符串或其他工件参数。

## 错误响应

公共 v1 错误响应是纯文本，带有 `content-type: text/plain; charset=utf-8`。
这包括验证失败（`400`）、缺失公共资源（`404`）、认证和权限失败（`401`/`403`）、速率限制（`429`）以及被阻止的下载。客户端应将响应正文读取为人类可读的字符串。未知查询参数会为了兼容性而被忽略，但带有无效值的已识别查询参数会返回 `400`。
...
- `q`（必需）：查询字符串
- `limit`（可选）：整数
- `highlightedOnly`（可选）：`true` 表示筛选到高亮 skills
...
- 结果按相关性顺序返回（embedding 相似度 + 精确 slug/name token 加权 + 少量人气先验）。
- 相关性强于人气。精确的 slug 或显示名称 token 匹配可以排在参与度强得多但匹配更宽松的结果之前。
...
- 人气会按对数缩放并封顶。当查询文本匹配较弱时，高参与度 skills 的排名可能更低。
...
- 重命名别名会为旧 URL 和通过注册表解析的安装保留解析能力，但搜索排名基于重命名索引后的规范 skill 元数据。现有统计数据会留在该 skill 上。
- 如果某个 skill 意外不可见，请先在登录状态下使用 `clawhub inspect @owner/slug` 检查审核状态，然后再更改与排名相关的元数据。
...
- `createdAt` 对新 skill 抓取是稳定的；当现有 skills 重新发布时，`updated` 会变化。
- 当 `nonSuspiciousOnly=true` 时，基于 cursor 的排序可能会在某页返回少于 `limit` 个项目，因为可疑 skills 是在页面检索后被筛除的。
- 存在 `nextCursor` 时使用它继续分页。页面较短本身并不表示结果已结束。
...
- 公共调用方仅能对已标记的可见 skills 获得 `200`。
- 对公共调用方会编辑证据，并且仅对所有者/审核员包含原始片段。

### `POST /api/v1/skills/{slug}/report`

举报某个 skill 以供审核员审查。举报是 skill 级别的，可选链接到某个版本，并进入 skill 举报队列。
...
`confirmed` 和 `dismissed` 需要 `note`；将 `status` 设置回 `open` 时可以省略它。传入 `finalAction: "hide"` 和已 triage 的举报，可在同一个可审计工作流中隐藏该 skill。
...
- 扫描请求负载和可下载报告会在保留窗口结束后从扫描请求存储中到期。
- 已发布扫描需要拥有者/发布者管理访问权限，或平台审核员/管理员权限。
...
- 扫描作业是异步的。手动扫描请求会优先于普通发布/回填工作，但完成时间仍取决于工作进程可用性。
...
- 排队期间返回 `queue.queuedAhead` 和 `queue.position`，以便客户端显示该请求前面还有多少个优先级更高的手动扫描。非常大的队列会被设定上限，并以 `queuedAheadIsEstimate: true` 报告。
...
- 需要对该技能或插件拥有拥有者/发布者管理访问权限，或具备平台审核员/管理员权限。
...
仅限管理员使用的规范批量重新扫描路由。它接受与旧版 `POST /api/v1/skills/-/rescan-batch` 相同的负载形状。
...
- 技能身份、发布者身份和所选版本元数据是顶层信封字段（`slug`、`displayName`、`publisherHandle`、`version`、`resolvedFrom`、`tag`、`createdAt`），因此 shell 自动化无需解包嵌套包装即可读取它们。
- `security` 是顶层 ClawScan/安全判定。自动化应基于 `ok`、`decision`、`reasons` 和 `security.status` 判断。
- `security.signals` 包含支持性扫描器证据，例如 `staticScan`、`virusTotal` 和 `skillSpector`。
- `security.signals.dependencyRegistry` 为 v1 响应兼容性而保留，但依赖注册表存在性扫描器已停用，并且此键始终为 `null`。
...
- 响应仅包含安全信息。它不包含 Skill Card 数据、已生成卡片状态、构件文件列表或详细扫描器负载。
...
- `category`（可选）：插件类别过滤器。仅当请求范围限定为插件包（`/api/v1/plugins`、
...
- `channel`（可选）：`official`、`community` 或 `private`
- `isOfficial`（可选）：`true` 或 `false`
- `category`（可选）：插件类别过滤器。仅当请求范围限定为插件包时支持。受控类别和旧版 v1
...
旧版别名不接受作为已存储或作者声明的类别值。

### `GET /api/v1/skills/export`

批量导出最新公共 Skills，用于离线分析。
...
- `GET /api/v1/plugins` 下记录的旧版 v1 过滤器别名也会被
  接受。
...
返回安装客户端使用的精确包发布安全与信任摘要。
这是公开的 OpenClaw 消费接口，用于决定解析后的发布版本是否可以安装。
...
- 此端点精确到版本。客户端应在解析出
  打算安装的包版本后调用它，而不是仅在读取最新
  包元数据后调用。
- 私有包返回 `404`，除非调用方可以读取所属发布者。
- 此端点刻意比所有者/版主审核
  端点更窄。它公开安装决策和公开说明，而不公开
...
- `packageName` 按 npm 名称规范化；对于计划中的
  迁移，包可以不存在。
- 这只跟踪迁移就绪状态。它不会修改 OpenClaw 或生成
  ClawPack。
...
- `open`：可疑、恶意、待处理、隔离、撤销或已举报的发布版本。
- `blocked`：隔离、撤销或恶意发布版本。
- `manual`：带有手动审核覆盖的任意发布版本。
- `all`：带有手动覆盖、非干净扫描状态或包举报的任意发布版本。
...
用于包审核可见性的 owner/moderator 端点。

凭证：

- 需要包 owner、publisher 成员、moderator 或 admin 用户的 API token。
...
用于解决或重新打开包报告的 moderator/admin 端点。
...
`note` 对 `confirmed` 和 `dismissed` 是必填项；将 `status` 设回 `open` 时可以省略。对已确认的报告传入 `finalAction: "quarantine"` 或 `finalAction: "revoke"`，可在同一可审计工作流中应用发布审核。
...
隔离和撤销的发布会从 artifact 下载路由返回 `403`。每次更改都会写入一条审计日志条目。
...
- 私有包返回 `404`，除非调用方可以读取所属 publisher。
...
- 插件/包归档是带有 `package/` 根目录的 zip 文件，因此旧版 OpenClaw 客户端会继续工作。
...
由 CLI 用于将本地 fingerprint 映射到已知版本。

查询参数：

- `slug`（必填）
- `hash`（必填）：bundle fingerprint 的 64 字符十六进制 sha256
...
下载托管的技能版本 ZIP，或者为当前 GitHub 支持的技能返回 GitHub 源码交接；该技能需要有 `clean` 或 `suspicious` 扫描结果且没有托管版本。
...
- 软删除的版本返回 `410`。
- GitHub 支持的技能交接不会代理或镜像字节。JSON 响应包含 `sourceRef: "public-github"`、`repo`、`commit`、`path`、`contentHash` 和 `archiveUrl`；扫描/当前状态是门控条件，不会作为成功 payload 元数据包含。
- 下载统计按每个 UTC 日的唯一身份计数（API token 有效时使用 `userId`，否则使用 IP）。
...
- 可选 payload 字段：`ownerHandle`。存在时，API 在服务端解析该 publisher，并要求 actor 具有 publisher 访问权限。
- 可选 payload 字段：`migrateOwner`。当与 `ownerHandle` 一起为 `true` 时，如果 actor 同时是当前和目标 publisher 的 admin/owner，现有技能可以移动到该 owner。没有此显式选择时，owner 更改会被拒绝。
...
- 允许的表单字段是 `payload`、重复的 `files` blob，或一个 `clawpack` tarball 引用。`clawpack` 可以是 `.tgz` blob，也可以是 upload-url 流程返回的 storage id。基于暂存 storage-id 的发布还必须包含随该上传 URL 返回的 `clawpackUploadTicket`。
- 在同一请求中只能使用 `files` 或 `clawpack`，不能同时使用。
- JSON body 和调用方提供的 `payload.files` / `payload.artifact` 元数据会被拒绝。
- 直接 multipart 发布请求上限为 18MB。ClawPack tarball 可使用 upload-url 流程，最高可达 120MB tarball 上限。
...
- 只有 `openclaw` org publisher 以及当前 `openclaw` org 成员的个人 publisher 可以发布到 `official` 频道。
- 代表发布仍会根据目标 owner 账户验证 official-channel 资格。

### `DELETE /api/v1/skills/{slug}` / `POST /api/v1/skills/{slug}/undelete`

软删除/恢复技能（owner、moderator 或 admin）。
...
owner 发起的软删除会保留 slug 30 天，之后该 slug 可由另一个 publisher 认领。删除响应在此到期适用时包含 `slugReservedUntil`。
moderator/admin 隐藏和安全移除不会以这种方式到期。
...
仅 admin。确保某个 handle 存在 org publisher。如果该 handle 仍指向旧版共享用户/个人 publisher，该端点会先将其迁移为 org publisher。
对于新创建的 org，提供 `memberHandle`；执行操作的 admin 不会被添加为成员。
...
已认证的自助式 org publisher 创建。创建新的 org publisher，并将调用方添加为 owner。此端点不会迁移现有用户/个人 handle，也不会将 publisher 标记为 trusted/official。

- Body：`{ "handle": "opik", "displayName": "Opik" }`
- Response：`{ "ok": true, "publisherId": "...", "handle": "opik", "created": true, "trusted": false }`
- 当 handle 已被 publisher、用户或个人 publisher 使用时返回 `409`。

### `POST /api/v1/users/reserve`

仅 admin。为合法 owner 保留根 slug 和包名，而不发布 release。包名会变成没有 release 行的私有占位包，因此同一 owner 稍后可以将真正的 code-plugin 或 bundle-plugin 发布到该名称。
...
仅 admin。为已验证的替换 GitHub OAuth principal 恢复个人 publisher，而无需编辑 Convex Auth 账户行。请求必须命名两个不可变的 GitHub provider account id；可变 handle 仅用作面向 operator 的防护。

端点默认采用试运行。应用恢复需要在工作人员独立验证两个 GitHub 主体之间的连续性后设置 `dryRun: false` 和
`confirmIdentityVerified: true`。
当目标用户当前的个人发布者拥有 Skills、包或 GitHub 技能来源时，恢复会失败并关闭。
恢复还会迁移已恢复发布者的 Skills、技能 slug 别名、包、包检查器警告以及派生搜索摘要行中的旧版 `ownerUserId` 字段，使直接所有者路径与新的发布者权限保持一致。已恢复 handle 的活动受保护 handle 预留也会重新分配给替换用户，这样后续个人资料同步无法恢复原用户的竞争权限。每个主表在每次应用事务中限制为
100 行；更大的恢复必须先使用可恢复的所有者迁移。
GitHub 技能来源按发布者限定范围，并报告为已检查，而不是被重写。
...
更改现有封禁存储的原因，不解除封禁或恢复内容（仅限管理员）。除非 `dryRun` 为 `false`，否则默认为试运行。
...
`POST /api/cli/upload-url` 返回 `uploadUrl` 和 `uploadTicket`。暂存 ClawPack tarball 的包发布必须将生成的存储 ID 作为 `clawpack` 发送，并将返回的票据作为 `clawpackUploadTicket` 发送。
