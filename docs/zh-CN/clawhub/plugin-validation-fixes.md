...
title: 插件验证修复
x-i18n:
    generated_at: "2026-07-01T18:07:00Z"
    model: gpt-5.5
...
ClawHub 会在发布前验证插件包，也可以显示来自自动包扫描的发现项。本页涵盖面向作者的发现项，也就是插件作者可以在其包元数据、清单、SDK 导入或已发布制品中修复的发现项。

它不涵盖内部 Plugin Inspector 覆盖率发现项。如果完整报告包含没有作者修复指导的扫描器维护代码，那些是给 OpenClaw 维护者看的，而不是给插件作者看的。
...
| `provider-auth-env-vars`                | [将提供商环境变量移到设置元数据](/zh-CN/clawhub/plugin-validation-fixes#provider-auth-env-vars)                             |
...
- 使用[构建插件](/zh-CN/plugins/building-plugins)查看最小包示例，并使用[插件清单](/zh-CN/plugins/manifest#manifest-versus-packagejson)了解包与清单的分工。
...
- 将 `openclaw.install.minHostVersion` 用于最低 OpenClaw 主机版本。
- 参见[影响设备发现的 package.json 字段](/zh-CN/plugins/manifest#packagejson-fields-that-affect-discovery)。
- 重新运行 `clawhub package validate <path-to-plugin>`。
...
- 将它与包版本分开。包版本描述插件发布；`openclaw.compat.pluginApi` 描述主机 API 契约。
- 参见[影响设备发现的 package.json 字段](/zh-CN/plugins/manifest#packagejson-fields-that-affect-discovery)。
- 重新运行 `clawhub package validate <path-to-plugin>`。

### package-min-host-version-drift

包的最低主机版本与该包构建所依据的 OpenClaw 版本元数据不匹配。
...
- 如果 `openclaw.plugin.json` 也有 `version`，请将其更新为匹配值；如果包元数据是权威来源，则移除过时的清单版本元数据。
- 更改已发布元数据后，发布新的包版本。
...
- 确保打包后的制品中 `package.json` 包含 `openclaw` 块。
- 当该包是原生 OpenClaw 插件时，确保包含 `openclaw.plugin.json`。
...
- 将包或安装元数据移动到受支持的 `package.json#openclaw` 字段中，
  而不是放在清单里。
- 重新运行 `clawhub package validate <path-to-plugin>`。
...
- 当代码需要结构化 transcript 操作目标时，使用 `resolveSessionTranscriptTarget(...)`。
...
插件仍在使用已弃用的低层级 transcript 辅助函数，例如
...
- 优先使用结构化 transcript 运行时表面，以便 OpenClaw 可以应用
  正确的事务边界和身份处理。
- 仅当你声明的兼容性范围仍支持需要它们的旧版 OpenClaw 时，
  才保留低层级 transcript 辅助函数。
- 参见 [运行时 API](/zh-CN/plugins/sdk-runtime#agent-session-state) 和
  [插件 SDK 子路径](/zh-CN/plugins/sdk-subpaths)。
- 重新运行 `clawhub package validate <path-to-plugin>`。
...
- 将模型或提供商覆盖工作移动到 `before_model_resolve`。
- 将提示词或上下文变更工作移动到 `before_prompt_build`。
- 仅当你声明的兼容性范围仍支持需要它的旧版 OpenClaw 时，
...
清单使用了旧版或较旧的渠道环境变量元数据，缺少 ClawHub 期望的当前
设置或配置元数据。

- 保持渠道环境变量元数据的声明式形式，以便 OpenClaw 可以在不加载渠道运行时的情况下检查设置状态。
- 将由环境驱动的渠道设置镜像到你的插件形态使用的当前设置、渠道配置或
  包渠道元数据中。
