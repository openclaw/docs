...
title: 插件验证修复
x-i18n:
    generated_at: "2026-07-02T17:33:09Z"
    model: gpt-5.5
...
ClawHub 会在发布前验证插件包，也可以显示来自自动包扫描的发现。本页涵盖面向作者的发现，也就是插件作者可以在其包元数据、清单、SDK 导入或已发布制品中修复的发现。
...
- 如果 `openclaw.plugin.json` 也有 `version`，请更新它以匹配；或者在包元数据具有权威性时移除过时的清单版本元数据。
- 更改已发布元数据后，发布新的包版本。
...
- 将原生插件元数据保存在 `openclaw.plugin.json` 中。
- 将包入口点、兼容性、安装、设置和目录元数据保存在受支持的 `package.json#openclaw` 字段中。
- 参见 [影响发现的 package.json 字段](/zh-CN/plugins/manifest#packagejson-fields-that-affect-discovery)。
- 重新运行 `clawhub package validate <path-to-plugin>`。
...
- 修复无效包元数据、损坏的生命周期脚本，或会导致打包失败的 files 条目。
...
- 更新 `files` 或 `.npmignore`，确保包元数据未被排除。
...
- 保持 `name` 便于人工阅读，并将 `id` 保持为稳定的机器 ID。
...
- 避免加载、修改和保存整个会话存储对象。
- 仅在你声明的兼容性范围仍支持需要它的较旧 OpenClaw 版本时，
...
- 避免加载、修改和保存整个会话存储对象。
- 仅在你声明的兼容性范围仍支持需要它们的较旧 OpenClaw 版本时，
  才保留整存储写入辅助函数。
- 参见 [运行时 API](/zh-CN/plugins/sdk-runtime#agent-session-state)和
  [插件 SDK 子路径](/zh-CN/plugins/sdk-subpaths)。
- 重新运行 `clawhub package validate <path-to-plugin>`。
...
- 优先使用结构化转录运行时表面，以便 OpenClaw 应用正确的事务边界和身份处理。
- 仅在你声明的兼容性范围仍支持需要它们的较旧 OpenClaw 版本时，
  才保留底层转录辅助函数。
- 参见 [运行时 API](/zh-CN/plugins/sdk-runtime#agent-session-state)和
  [插件 SDK 子路径](/zh-CN/plugins/sdk-subpaths)。
...
包随附的 `openclaw.security.json` 包含 ClawHub
未识别为可用的 schema 引用。
