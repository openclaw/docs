...
read_when:
    ...
    - 你正在发布前更新插件包元数据
summary: 修复 ClawHub 插件包验证发现的问题后再发布
title: 插件验证修复
x-i18n:
    generated_at: "2026-07-16T11:26:24Z"
    model: gpt-5.6
...
ClawHub 会在发布前验证插件包，也可以显示自动包扫描的发现项。本页介绍面向作者的发现项，即插件作者可以通过修改包元数据、插件清单、SDK 导入或已发布工件来修复的发现项。

本页不介绍 Plugin Inspector 的内部覆盖率发现项。如果完整报告包含没有作者修复指导的扫描器维护代码，则这些代码面向 OpenClaw 维护者，而不是插件作者。
...
| `package-manifest-version-drift`        | [对齐包和插件清单版本](/zh-CN/clawhub/plugin-validation-fixes#package-manifest-version-drift)                          |
...
| `manifest-unknown-fields`               | [移除不受支持的插件清单字段](/zh-CN/clawhub/plugin-validation-fixes#manifest-unknown-fields)                                  |
...
| `unrecognized-security-manifest`        | [移除不受支持的安全插件清单文件](/zh-CN/clawhub/plugin-validation-fixes#unrecognized-security-manifest)                   |
...
- 参阅[构建插件](/zh-CN/plugins/building-plugins)中的最小包示例，并参阅[插件清单](/zh-CN/plugins/manifest#manifest-versus-packagejson)了解包与插件清单的职责划分。
- 重新运行 `clawhub package validate <path-to-plugin>`。

### package-openclaw-metadata-missing

包包含 `package.json`，但未声明 OpenClaw 包元数据。

- 添加 `package.json#openclaw`。
- 包含入口点元数据，例如 `openclaw.extensions` 或 `openclaw.runtimeExtensions`。
- 当包将通过 ClawHub 发布或安装时，添加兼容性和安装元数据。
- 参阅[影响发现的 package.json 字段](/zh-CN/plugins/manifest#packagejson-fields-that-affect-discovery)。
- 重新运行 `clawhub package validate <path-to-plugin>`。
...
- 确保所有入口点路径均位于包目录内。
- 参阅[插件入口点](/zh-CN/plugins/sdk-entrypoints)和[影响发现的 package.json 字段](/zh-CN/plugins/manifest#packagejson-fields-that-affect-discovery)。
- 重新运行 `clawhub package validate <path-to-plugin>`。
...
- 使用构建和测试时所针对的 OpenClaw 插件 API 版本或 semver 下限。
...
- 如果 `openclaw.plugin.json` 也包含 `version`，请将其更新为一致的版本；如果包元数据是权威来源，则移除过时的插件清单版本元数据。
- 更改已发布的元数据后，发布新的包版本。
...
- 将包或安装元数据移入受支持的 `package.json#openclaw` 字段，而不是放在清单中。
- 重新运行 `clawhub package validate <path-to-plugin>`。
...
- 移除不受支持的契约键。
- 将运行时行为移入插件注册代码，并确保 `contracts` 仅包含静态能力所有权元数据。
- 重新运行 `clawhub package validate <path-to-plugin>`。
...
- 将保留的 OpenClaw 内部 SDK 导入替换为文档所列的公共 `openclaw/plugin-sdk/*` 子路径。
- 如果该行为没有公共 SDK，请将辅助函数保留在你的包内，或请求提供公共 OpenClaw API。
...
- 避免加载、修改并保存整个会话存储对象。
- 仅当声明的兼容范围仍支持需要 `loadSessionStore(...)` 的旧版 OpenClaw 时，才保留它。
- 请参阅[运行时 API](/zh-CN/plugins/sdk-runtime#agent-session-state) 和[插件 SDK 子路径](/zh-CN/plugins/sdk-subpaths)。
- 重新运行 `clawhub package validate <path-to-plugin>`。

### sdk-session-store-write

插件仍在使用已弃用的完整会话存储写入辅助函数，例如 `saveSessionStore` 或 `updateSessionStore`。

- 更新现有会话条目的字段时，使用 `patchSessionEntry(...)`。
- 替换或创建会话条目时，使用 `upsertSessionEntry(...)`。
- 避免加载、修改并保存整个会话存储对象。
- 仅当声明的兼容范围仍支持需要完整存储写入辅助函数的旧版 OpenClaw 时，才保留这些函数。
- 请参阅[运行时 API](/zh-CN/plugins/sdk-runtime#agent-session-state) 和[插件 SDK 子路径](/zh-CN/plugins/sdk-subpaths)。
- 重新运行 `clawhub package validate <path-to-plugin>`。

### sdk-session-file-helper

插件仍在使用已弃用的会话文件路径辅助函数，例如 `resolveSessionFilePath` 或 `resolveAndPersistSessionFile`。

- 使用 `getSessionEntry(...)`，按智能体和会话标识读取会话元数据。
- 使用 `patchSessionEntry(...)` 或 `upsertSessionEntry(...)` 持久化会话元数据。
- 当代码准备执行记录操作时，使用记录标识或目标辅助函数。
- 不要持久化或依赖旧版记录文件路径。
- 请参阅[运行时 API](/zh-CN/plugins/sdk-runtime#agent-session-state) 和[插件 SDK 子路径](/zh-CN/plugins/sdk-subpaths)。
- 重新运行 `clawhub package validate <path-to-plugin>`。
...
插件仍在使用已弃用的记录文件目标辅助函数 `resolveSessionTranscriptLegacyFileTarget`。

- 当代码仅需要公共会话标识时，使用 `resolveSessionTranscriptIdentity(...)`。
- 当代码需要结构化记录操作目标时，使用 `resolveSessionTranscriptTarget(...)`。
- 避免直接读取或构造旧版记录文件目标。
- 仅当声明的兼容范围仍支持需要旧版辅助函数的旧版 OpenClaw 时，才保留该函数。
- 请参阅[运行时 API](/zh-CN/plugins/sdk-runtime#agent-session-state) 和[插件 SDK 子路径](/zh-CN/plugins/sdk-subpaths)。
- 重新运行 `clawhub package validate <path-to-plugin>`。

### sdk-session-transcript-low-level

插件仍在使用已弃用的低级记录辅助函数，例如 `appendSessionTranscriptMessage` 或 `emitSessionTranscriptUpdate`。

- 使用 `appendSessionTranscriptMessageByIdentity(...)` 追加记录。
- 使用 `publishSessionTranscriptUpdateByIdentity(...)` 发送记录更新通知。
- 优先使用结构化记录运行时接口，以便 OpenClaw 应用正确的事务边界和标识处理。
- 仅当声明的兼容范围仍支持需要低级记录辅助函数的旧版 OpenClaw 时，才保留这些函数。
- 请参阅[运行时 API](/zh-CN/plugins/sdk-runtime#agent-session-state) 和[插件 SDK 子路径](/zh-CN/plugins/sdk-subpaths)。
- 重新运行 `clawhub package validate <path-to-plugin>`。
...
- 仅当声明的兼容范围仍支持需要 `before_agent_start` 的旧版 OpenClaw 时，才保留它。
- 请参阅[钩子](/zh-CN/plugins/hooks)和[插件兼容性](/zh-CN/plugins/compatibility)。
- 重新运行 `clawhub package validate <path-to-plugin>`。
...
- 仅当支持的 OpenClaw 版本范围仍需要 `providerAuthEnvVars` 时，才将其保留为兼容性元数据。
- 请参阅[设置参考](/zh-CN/plugins/manifest#setup-reference)和 [SDK 迁移](/zh-CN/plugins/sdk-migration)。
- 重新运行 `clawhub package validate <path-to-plugin>`。

### channel-env-vars

清单使用旧版或较旧的渠道环境变量元数据，但未提供 ClawHub 所需的当前设置或配置元数据。

- 确保渠道环境变量元数据采用声明式形式，以便 OpenClaw 无需加载渠道运行时即可检查设置状态。
- 将环境变量驱动的渠道设置同步到插件形态所使用的当前设置、渠道配置或包渠道元数据中。
- 仅当支持的旧版 OpenClaw 仍需要 `channelEnvVars` 时，才将其保留为兼容性元数据。
- 请参阅[插件清单](/zh-CN/plugins/manifest)和[渠道插件](/zh-CN/plugins/sdk-channel-plugins)。
- 重新运行 `clawhub package validate <path-to-plugin>`。
...
包附带 `openclaw.security.json`，其中引用的架构未被 ClawHub 识别为可用架构。

- 如果该架构 URL 仅供参考，请将其移除。
- 仅在 OpenClaw 发布有文档记录的版本化架构后，才使用该架构。
- 重新运行 `clawhub package validate <path-to-plugin>`。
...
- 移除 `openclaw.security.json`，直至 OpenClaw 发布版本化安全清单架构和 ClawHub 行为文档。
- 在清单契约建立之前，请在包的公开文档或 README 中记录安全敏感行为。
- 重新运行 `clawhub package validate <path-to-plugin>`。
