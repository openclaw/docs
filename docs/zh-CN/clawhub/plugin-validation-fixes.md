...
title: 插件验证修复
x-i18n:
    generated_at: "2026-06-27T01:33:24Z"
    model: gpt-5.5
...
本页不涵盖内部 Plugin Inspector 覆盖率发现项。如果完整报告包含没有作者修复指南的扫描器维护代码，这些是给 OpenClaw 维护者看的，而不是给插件作者看的。
...
- 如果 `openclaw.plugin.json` 也有 `version`，请更新它以匹配；或者在包元数据是权威来源时移除过时的清单版本元数据。
- 更改已发布元数据后，发布新的包版本。
...
- 运行 `npm pack --dry-run` 并检查被包含的元数据文件。
- 确保 `package.json` 在打包构件中包含 `openclaw` 块。
- 当该包是原生 OpenClaw 插件时，确保包含 `openclaw.plugin.json`。
- 更新 `files` 或 `.npmignore`，确保包元数据不会被排除。
- 参阅[构建插件](/zh-CN/plugins/building-plugins)。
- 重新运行 `clawhub package validate <path-to-plugin>`。
...
清单使用旧版或较旧的渠道环境变量元数据，缺少 ClawHub 当前期望的
设置或配置元数据。

- 保持渠道环境变量元数据为声明式，以便 OpenClaw 可以在不加载渠道运行时的情况下检查设置状态。
- 将环境驱动的渠道设置镜像到当前设置、渠道配置，或
  你的插件形态使用的包渠道元数据。
