...
read_when:
    ...
    - ClawHub 在插件包发布时拒绝或发出警告
    - 你正在发布前更新插件包元数据
summary: 在发布前修复 ClawHub 插件包验证发现的问题
title: 插件验证修复
x-i18n:
    generated_at: "2026-06-30T13:45:51Z"
    model: gpt-5.5
...
ClawHub 会在发布前验证插件包，也可以显示来自自动化包扫描的发现项。本页涵盖面向作者的发现项，也就是插件作者可以在其包元数据、清单、SDK 导入或已发布产物中修复的发现项。

它不涵盖内部 Plugin Inspector 覆盖率发现项。如果完整报告包含没有作者修复指导的扫描器维护代码，这些代码面向 OpenClaw 维护者，而不是插件作者。
...
| `package-openclaw-metadata-missing`     | [添加包 openclaw 块](/zh-CN/clawhub/plugin-validation-fixes#package-openclaw-metadata-missing)                            |
...
| `package-npm-pack-unavailable`          | [让 npm 产物可打包](/zh-CN/clawhub/plugin-validation-fixes#package-npm-pack-unavailable)                                 |
...
| `manifest-unknown-fields`               | [移除不支持的清单字段](/zh-CN/clawhub/plugin-validation-fixes#manifest-unknown-fields)                                  |
...
| `sdk-session-transcript-file-target`    | [替换旧版 transcript 文件目标](/zh-CN/clawhub/plugin-validation-fixes#sdk-session-transcript-file-target)                   |
| `sdk-session-transcript-low-level`      | [替换低层级 transcript 辅助函数](/zh-CN/clawhub/plugin-validation-fixes#sdk-session-transcript-low-level)                       |
| `legacy-before-agent-start`             | [替换 before_agent_start](/zh-CN/clawhub/plugin-validation-fixes#legacy-before-agent-start)                                        |
| `provider-auth-env-vars`                | [将提供商环境变量移动到设置元数据](/zh-CN/clawhub/plugin-validation-fixes#provider-auth-env-vars)                             |
| `channel-env-vars`                      | [在当前元数据中镜像渠道环境变量](/zh-CN/clawhub/plugin-validation-fixes#channel-env-vars)                                |
| `security-manifest-schema-unavailable`  | [移除不可用的安全清单 schema 引用](/zh-CN/clawhub/plugin-validation-fixes#security-manifest-schema-unavailable) |
| `unrecognized-security-manifest`        | [移除不支持的安全清单文件](/zh-CN/clawhub/plugin-validation-fixes#unrecognized-security-manifest)                   |
...
- 当包发布 OpenClaw 插件时，添加 `openclaw` 块。
- 使用 [Building plugins](/zh-CN/plugins/building-plugins) 查看最小包示例，并使用 [Plugin manifest](/zh-CN/plugins/manifest#manifest-versus-packagejson) 了解包与清单的拆分。
- 重新运行 `clawhub package validate <path-to-plugin>`。

### package-openclaw-metadata-missing

包有 `package.json`，但它没有声明 OpenClaw 包元数据。

- 添加 `package.json#openclaw`。
- 包含入口点元数据，例如 `openclaw.extensions` 或 `openclaw.runtimeExtensions`。
- 当包将通过 ClawHub 发布或安装时，添加兼容性和安装元数据。
- 参见 [影响发现的 package.json 字段](/zh-CN/plugins/manifest#packagejson-fields-that-affect-discovery)。
- 重新运行 `clawhub package validate <path-to-plugin>`。

### package-openclaw-entry-missing

包元数据存在，但它没有声明 OpenClaw 运行时入口点。

- 为原生插件入口点添加 `openclaw.extensions`。
- 当已发布包应加载构建后的 JavaScript 时，添加 `openclaw.runtimeExtensions`。
- 保持所有入口点路径位于包目录内。
- 参见 [Plugin entry points](/zh-CN/plugins/sdk-entrypoints) 和 [影响发现的 package.json 字段](/zh-CN/plugins/manifest#packagejson-fields-that-affect-discovery)。
- 重新运行 `clawhub package validate <path-to-plugin>`。

### package-entrypoint-missing

包声明了 OpenClaw 入口点，但被引用的文件在正在验证的包中缺失。

- 检查 `openclaw.extensions`、`openclaw.runtimeExtensions`、`openclaw.setupEntry` 和 `openclaw.runtimeSetupEntry` 中的每个路径。
- 如果入口点生成到 `dist`，请构建该包。
- 如果入口点已移动，请更新元数据。
- 参见 [Plugin entry points](/zh-CN/plugins/sdk-entrypoints)。
- 重新运行 `clawhub package validate <path-to-plugin>`。
...
- 用受支持的安装来源填充 `openclaw.install`，例如 `clawhubSpec`、`npmSpec` 或 `localPath`。
- 当有多个安装来源可用时，设置 `openclaw.install.defaultChoice`。
- 使用 `openclaw.install.minHostVersion` 表示最低 OpenClaw 宿主版本。
- 参见 [影响发现的 package.json 字段](/zh-CN/plugins/manifest#packagejson-fields-that-affect-discovery)。
- 重新运行 `clawhub package validate <path-to-plugin>`。

### package-plugin-api-compat-missing

该包未声明其支持的 OpenClaw 插件 API 范围。

- 将 `openclaw.compat.pluginApi` 添加到 `package.json`。
- 使用你构建和测试所针对的 OpenClaw 插件 API 版本或 semver 下限。
- 将它与包版本分开。包版本描述插件发布；`openclaw.compat.pluginApi` 描述宿主 API 契约。
- 参见 [影响发现的 package.json 字段](/zh-CN/plugins/manifest#packagejson-fields-that-affect-discovery)。
- 重新运行 `clawhub package validate <path-to-plugin>`。

### package-min-host-version-drift

包的最低宿主版本与该包构建时所针对的 OpenClaw 版本元数据不匹配。

- 检查 `openclaw.install.minHostVersion`。
- 检查包中的任何 OpenClaw 构建元数据，例如发布期间使用的 OpenClaw 版本。
- 将最低宿主版本与该包实际支持的宿主版本范围对齐。
- 参见 [影响发现的 package.json 字段](/zh-CN/plugins/manifest#packagejson-fields-that-affect-discovery)。
- 重新运行 `clawhub package validate <path-to-plugin>`。
...
- 首选 `package.json#version` 作为包发布版本。
- 如果 `openclaw.plugin.json` 也有 `version`，请将其更新为匹配，或者在包元数据为权威来源时移除过时的清单版本元数据。
- 更改已发布元数据后，发布新的包版本。
- 参见 [Plugin manifest](/zh-CN/plugins/manifest)。
- 重新运行 `clawhub package validate <path-to-plugin>`。

### package-openclaw-unsupported-metadata

`package.json#openclaw` 块包含 OpenClaw 包元数据不支持的字段。
...
该包无法打包成 ClawHub 将检查或发布的产物。

- 从包根目录运行 `npm pack --dry-run`。
- 修复无效的包元数据、损坏的生命周期脚本，或导致打包失败的 files 条目。
- 如果此包打算公开发布，请移除 `private: true`。
- 重新运行 `clawhub package validate <path-to-plugin>`。

### package-npm-pack-entrypoint-missing

该包可以打包，但打包后的产物不包含 `package.json#openclaw` 中声明的入口点文件。

- 运行 `npm pack --dry-run` 并检查将被包含的文件。
- 在打包前构建生成的入口点。
- 更新 `files`、`.npmignore` 或构建输出，使声明的入口点被包含。
- 参见 [Plugin entry points](/zh-CN/plugins/sdk-entrypoints)。
- 重新运行 `clawhub package validate <path-to-plugin>`。

### package-npm-pack-metadata-missing

打包后的产物缺少源包中存在的 OpenClaw 元数据。

- 运行 `npm pack --dry-run` 并检查包含的元数据文件。
- 确保 `package.json` 在打包产物中包含 `openclaw` 块。
- 当包是原生 OpenClaw 插件时，确保包含 `openclaw.plugin.json`。
- 更新 `files` 或 `.npmignore`，使包元数据不被排除。
- 参见 [Building plugins](/zh-CN/plugins/building-plugins)。
- 重新运行 `clawhub package validate <path-to-plugin>`。
...
- 保持 `name` 便于人类阅读，并保持 `id` 作为稳定的机器 ID。
...
- 将保留的 OpenClaw 内部 SDK 导入替换为已记录的公开
  `openclaw/plugin-sdk/*` 子路径。
- 如果该行为没有公开 SDK，请将辅助函数保留在你的包内，或请求一个公开的 OpenClaw API。
...
- 避免加载、修改并保存整个会话存储对象。
- 仅当你声明的兼容性范围仍支持需要它的较旧 OpenClaw 版本时，
...
插件仍在使用已弃用的整会话存储写入辅助函数，例如
`saveSessionStore` 或 `updateSessionStore`。

- 更新现有会话条目上的字段时使用 `patchSessionEntry(...)`。
- 替换或创建会话条目时使用 `upsertSessionEntry(...)`。
- 避免加载、修改并保存整个会话存储对象。
- 仅当你声明的兼容性范围仍支持需要它们的较旧 OpenClaw 版本时，
  才保留整存储写入辅助函数。
- 参见 [运行时 API](/zh-CN/plugins/sdk-runtime#agent-session-state)和
  [插件 SDK 子路径](/zh-CN/plugins/sdk-subpaths)。
- 重新运行 `clawhub package validate <path-to-plugin>`。
...
- 将模型或提供商覆盖工作移到 `before_model_resolve`。
- 将提示词或上下文修改工作移到 `before_prompt_build`。
- 仅当你声明的兼容性范围仍支持需要它的较旧 OpenClaw 版本时，
  才保留 `before_agent_start`。
- 参见 [钩子](/zh-CN/plugins/hooks)和
...
清单使用了旧版或较旧的渠道环境变量元数据，缺少 ClawHub 期望的当前设置或配置元数据。

- 保持渠道环境变量元数据为声明式，以便 OpenClaw 可以在不加载渠道运行时的情况下检查设置状态。
- 将环境变量驱动的渠道设置镜像到你的插件形态所使用的当前设置、渠道配置或包渠道元数据中。
- 仅当较旧的受支持 OpenClaw 版本仍需要它时，才将 `channelEnvVars`
  保留为兼容性元数据。
...
包随附的 `openclaw.security.json` 包含 ClawHub 无法识别为可用的 schema 引用。

- 如果 schema URL 仅用于建议说明，请移除它。
- 仅在 OpenClaw 发布已记录的版本化 schema 后使用它。
- 重新运行 `clawhub package validate <path-to-plugin>`。
...
- 在 OpenClaw 记录版本化安全清单 schema 和 ClawHub 行为之前，移除 `openclaw.security.json`。
- 在清单契约存在之前，请将安全敏感行为记录在你的公开包文档或 README 中。
- 重新运行 `clawhub package validate <path-to-plugin>`。
