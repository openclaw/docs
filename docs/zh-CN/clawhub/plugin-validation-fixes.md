...
read_when:
    ...
    - 你正在发布前更新插件包元数据
summary: 修复发布前的 ClawHub 插件包验证发现
title: 插件验证修复
x-i18n:
...
ClawHub 会在发布前验证插件包，也可以显示来自自动化包扫描的发现项。本页涵盖面向作者的发现项，也就是插件作者可以在其包元数据、清单、SDK 导入或已发布构件中修复的发现项。

本页不涵盖内部 Plugin Inspector 覆盖率发现项。如果完整报告包含没有作者修复指南的扫描器维护代码，那些是面向 OpenClaw 维护者的，而不是插件作者。
...
| `sdk-session-file-helper`               | [替换会话文件路径帮助程序](/zh-CN/clawhub/plugin-validation-fixes#sdk-session-file-helper)                                   |
| `sdk-session-transcript-file-target`    | [替换旧版转录文件目标](/zh-CN/clawhub/plugin-validation-fixes#sdk-session-transcript-file-target)                   |
| `sdk-session-transcript-low-level`      | [替换低级转录帮助程序](/zh-CN/clawhub/plugin-validation-fixes#sdk-session-transcript-low-level)                       |
...
- 当包随附 OpenClaw 插件时，添加 `openclaw` 块。
...
- 如果 `openclaw.plugin.json` 也有 `version`，请将其更新为匹配，或在包元数据具有权威性时移除过时的清单版本元数据。
- 更改已发布元数据后，发布新的包版本。
...
- 避免加载、变更并保存整个会话存储对象。
- 仅当你声明的兼容范围仍支持需要它们的旧版 OpenClaw 时，才保留整存储写入辅助函数。
- 参见 [运行时 API](/zh-CN/plugins/sdk-runtime#agent-session-state)和
  [插件 SDK 子路径](/zh-CN/plugins/sdk-subpaths)。
- 重新运行 `clawhub package validate <path-to-plugin>`。
...
- 当代码正在准备转录记录操作时，使用转录记录身份或目标辅助函数。
- 不要持久化或依赖旧版转录记录文件路径。
- 参见 [运行时 API](/zh-CN/plugins/sdk-runtime#agent-session-state)和
  [插件 SDK 子路径](/zh-CN/plugins/sdk-subpaths)。
- 重新运行 `clawhub package validate <path-to-plugin>`。
...
插件仍在使用已弃用的转录记录文件目标辅助函数
`resolveSessionTranscriptLegacyFileTarget`。

- 当代码只需要公共会话身份时，使用 `resolveSessionTranscriptIdentity(...)`。
- 当代码需要结构化转录记录操作目标时，使用 `resolveSessionTranscriptTarget(...)`。
- 避免直接读取或构造旧版转录记录文件目标。
...
插件仍在使用已弃用的低级转录记录辅助函数，例如
`appendSessionTranscriptMessage` 或 `emitSessionTranscriptUpdate`。

- 使用 `appendSessionTranscriptMessageByIdentity(...)` 追加转录记录。
- 使用 `publishSessionTranscriptUpdateByIdentity(...)` 发送转录记录更新通知。
- 优先使用结构化转录记录运行时表面，以便 OpenClaw 能应用正确的事务边界和身份处理。
- 仅当你声明的兼容范围仍支持需要它们的旧版 OpenClaw 时，才保留低级转录记录辅助函数。
- 参见 [运行时 API](/zh-CN/plugins/sdk-runtime#agent-session-state)和
  [插件 SDK 子路径](/zh-CN/plugins/sdk-subpaths)。
- 重新运行 `clawhub package validate <path-to-plugin>`。
...
清单使用了旧版或较旧的渠道环境变量元数据，缺少 ClawHub 预期的当前设置或配置元数据。
...
- 仅在 OpenClaw 发布文档化的版本化 schema 后再使用它。
- 重新运行 `clawhub package validate <path-to-plugin>`。
...
- 在清单 contract 存在之前，将安全敏感行为记录在你的公开包文档或 README 中。
- 重新运行 `clawhub package validate <path-to-plugin>`。
