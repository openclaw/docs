...
- 使用[构建插件](/zh-CN/plugins/building-plugins)查看最小包示例，并参考[插件清单](/zh-CN/plugins/manifest#manifest-versus-packagejson)了解包与清单的分工。
...
- 用受支持的安装来源填写 `openclaw.install`，例如 `clawhubSpec`、`npmSpec` 或 `localPath`。
...
- 如果 `openclaw.plugin.json` 也有 `version`，请更新它以匹配，或在包元数据具有权威性时移除过时的清单版本元数据。
- 更改已发布元数据后，发布新的包版本。
...
- 确保 `package.json` 在打包后的工件中包含 `openclaw` 块。
- 当包是原生 OpenClaw 插件时，确保包含 `openclaw.plugin.json`。
- 更新 `files` 或 `.npmignore`，避免包元数据被排除。
...
- 将包或安装元数据移动到支持的 `package.json#openclaw` 字段中，
  而不是放在清单中。
- 重新运行 `clawhub package validate <path-to-plugin>`。
...
- 使用 [导入约定](/zh-CN/plugins/building-plugins#import-conventions)和
  [插件 SDK 子路径](/zh-CN/plugins/sdk-subpaths)来找到窄导入。
- 重新运行 `clawhub package validate <path-to-plugin>`。
...
- 将 prompt 或上下文修改工作移动到 `before_prompt_build`。
- 仅当你声明的兼容性范围仍支持需要它的旧版 OpenClaw 时，
