...
read_when:
    ...
    - 你正在发布前更新插件包元数据
summary: 发布前修复 ClawHub 插件包验证发现项
title: 插件验证修复
x-i18n:
    generated_at: "2026-07-04T03:35:08Z"
    model: gpt-5.5
...
ClawHub 会在发布前验证插件包，也可以显示来自自动化包扫描的发现。此页面涵盖面向作者的发现，也就是插件作者可以在其包元数据、清单、SDK 导入或已发布构件中修复的发现。
...
包根目录未包含 `package.json`，因此 ClawHub 无法识别 npm 包、版本、入口点或 OpenClaw 元数据。
...
- 将其与包版本分开。包版本描述插件发布；`openclaw.compat.pluginApi` 描述宿主 API 合约。
- 请参阅 [影响设备发现的 package.json 字段](/zh-CN/plugins/manifest#packagejson-fields-that-affect-discovery)。
- 重新运行 `clawhub package validate <path-to-plugin>`。

### package-min-host-version-drift

包最低宿主版本与该包构建时所依据的 OpenClaw 版本元数据不匹配。
...
- 对渠道入口帮助函数使用 `openclaw/plugin-sdk/channel-core`。
...
- 仅在你支持的 OpenClaw 范围仍需要它时，将 `providerAuthEnvVars`
  作为兼容性元数据保留。
...
清单使用的是旧版或较旧的渠道环境变量元数据，缺少 ClawHub 期望的当前
设置或配置元数据。

- 保持渠道环境变量元数据为声明式，以便 OpenClaw 可以在不加载渠道运行时的情况下检查设置状态。
- 将环境驱动的渠道设置镜像到你的插件形态所使用的当前设置、渠道配置或
  包渠道元数据中。
- 仅在较旧的受支持 OpenClaw 版本仍需要它时，将 `channelEnvVars`
  作为兼容性元数据保留。
- 参见 [Plugin Manifest](/zh-CN/plugins/manifest) 和
