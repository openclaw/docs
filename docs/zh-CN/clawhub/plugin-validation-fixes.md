...
| `manifest-unknown-contracts`            | [移除不受支持的契约键名](/zh-CN/clawhub/plugin-validation-fixes#manifest-unknown-contracts)                                 |
...
- 参考[构建插件](/zh-CN/plugins/building-plugins)了解最小包示例，并参考[插件清单](/zh-CN/plugins/manifest#manifest-versus-packagejson)了解包与清单的拆分。
...
- 如果 `openclaw.plugin.json` 也有 `version`，请将其更新为匹配版本；如果包元数据是权威来源，则移除过时的清单版本元数据。
- 更改已发布元数据后，发布新的包版本。
...
- 使用[导入约定](/zh-CN/plugins/building-plugins#import-conventions)和
  [插件 SDK 子路径](/zh-CN/plugins/sdk-subpaths)来找到窄化的导入。
- 重新运行 `clawhub package validate <path-to-plugin>`。
...
- 保持渠道环境变量元数据为声明式，这样 OpenClaw 可以在不加载渠道运行时的情况下
  检查设置状态。
- 将环境变量驱动的渠道设置镜像到当前设置、渠道配置或
  你的插件形态使用的包渠道元数据中。
