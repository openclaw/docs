---
read_when:
    - 你运行了 clawhub package validate，并需要修复插件检查结果
...
| `sdk-load-session-store`                | [替换整会话存储访问](/zh-CN/clawhub/plugin-validation-fixes#sdk-load-session-store)                                   |
...
- 使用 [Building plugins](/zh-CN/plugins/building-plugins) 查看最小软件包示例，并使用 [Plugin manifest](/zh-CN/plugins/manifest#manifest-versus-packagejson) 了解软件包与清单的分工。
...
- 当已发布的软件包应加载构建后的 JavaScript 时，添加 `openclaw.runtimeExtensions`。
- 保持所有入口点路径位于软件包目录内。
...
ClawHub 无法判断应如何安装或更新软件包。

- 使用受支持的安装来源填充 `openclaw.install`，例如 `clawhubSpec`、`npmSpec` 或 `localPath`。
...
- 使用你构建和测试时所依据的 OpenClaw 插件 API 版本或 semver 下限。
- 将其与软件包版本分开。软件包版本描述插件发布；`openclaw.compat.pluginApi` 描述宿主 API 契约。
- 参见 [影响设备发现的 package.json 字段](/zh-CN/plugins/manifest#packagejson-fields-that-affect-discovery)。
- 重新运行 `clawhub package validate <path-to-plugin>`。

### package-min-host-version-drift

软件包最低宿主版本与该软件包构建时所依据的 OpenClaw 版本元数据不匹配。

- 检查 `openclaw.install.minHostVersion`。
- 检查软件包中的任何 OpenClaw 构建元数据，例如发布期间使用的 OpenClaw 版本。
- 将最低宿主版本与软件包实际支持的宿主版本范围对齐。
- 参见 [影响设备发现的 package.json 字段](/zh-CN/plugins/manifest#packagejson-fields-that-affect-discovery)。
- 重新运行 `clawhub package validate <path-to-plugin>`。
...
- 优先使用 `package.json#version` 作为软件包发布版本。
- 如果 `openclaw.plugin.json` 也有 `version`，请将其更新为匹配值；或者在软件包元数据为权威来源时，移除陈旧的清单版本元数据。
- 更改已发布元数据后，发布新的软件包版本。
...
软件包无法打包成 ClawHub 将检查或发布的产物。

- 从软件包根目录运行 `npm pack --dry-run`。
- 修复无效的软件包元数据、损坏的生命周期脚本，或导致打包失败的 files 条目。
- 如果此软件包旨在公开发布，请移除 `private: true`。
- 重新运行 `clawhub package validate <path-to-plugin>`。
...
- 在打包前构建生成的入口点。
- 更新 `files`、`.npmignore` 或构建输出，以包含声明的入口点。
- 参见 [插件入口点](/zh-CN/plugins/sdk-entrypoints)。
- 重新运行 `clawhub package validate <path-to-plugin>`。
...
- 更新 `files` 或 `.npmignore`，确保软件包元数据不会被排除。
...
- 将 `contracts` 下的每个键与
  [合约参考](/zh-CN/plugins/manifest#contracts-reference)进行比较。
- 移除不受支持的合约键。
...
- 如果该行为没有公共 SDK，请将辅助函数保留在你的包内，或请求一个公共 OpenClaw API。
...
清单仍在使用旧版 `providerAuthEnvVars` 提供商认证元数据。
...
清单使用了旧版或较旧的渠道环境变量元数据，缺少 ClawHub 当前期望的设置或配置元数据。

- 保持渠道环境变量元数据为声明式，以便 OpenClaw 无需加载渠道运行时即可检查设置状态。
- 将由环境变量驱动的渠道设置镜像到你的插件形态所使用的当前设置、渠道配置或包渠道元数据中。
- 仅当较旧的受支持 OpenClaw 版本仍需要它时，才将 `channelEnvVars` 保留为兼容性元数据。
...
包随附的 `openclaw.security.json` 带有一个 ClawHub 不认为可用的 schema 引用。

- 如果 schema URL 仅用于提示，请将其移除。
- 仅在 OpenClaw 发布已记录的版本化 schema 后，才使用它。
- 重新运行 `clawhub package validate <path-to-plugin>`。
...
- 在清单合约存在之前，将安全敏感行为记录在你的公共包文档或 README 中。
- 重新运行 `clawhub package validate <path-to-plugin>`。
