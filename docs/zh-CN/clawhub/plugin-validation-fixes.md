...
title: 插件验证修复
x-i18n:
    generated_at: "2026-06-27T10:17:23Z"
    model: gpt-5.5
...
- 保持 `name` 适合人类阅读，并保持 `id` 作为稳定的机器 ID。
- 参见[插件清单](/zh-CN/plugins/manifest)。
- 重新运行 `clawhub package validate <path-to-plugin>`。

### manifest-unknown-fields

插件清单有 OpenClaw 不支持的顶级字段。
...
- 使用[导入约定](/zh-CN/plugins/building-plugins#import-conventions)和
  [插件 SDK 子路径](/zh-CN/plugins/sdk-subpaths)来找到窄范围导入。
- 重新运行 `clawhub package validate <path-to-plugin>`。
...
清单使用了旧版或较旧的渠道环境变量元数据，缺少 ClawHub
当前期望的设置或配置元数据。
...
package 随附的 `openclaw.security.json` 中包含 ClawHub
无法识别为可用的 schema 引用。

- 如果 schema URL 仅用于建议说明，请将其移除。
- 仅在 OpenClaw 发布已记录的版本化 schema 后，才使用该 schema。
- 重新运行 `clawhub package validate <path-to-plugin>`。
...
- 在清单契约存在之前，将安全敏感行为记录在你的公共 package 文档或
  README 中。
- 重新运行 `clawhub package validate <path-to-plugin>`。
