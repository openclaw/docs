...
title: 插件验证修复
x-i18n:
    generated_at: "2026-07-01T05:27:51Z"
    model: gpt-5.5
...
ClawHub 会在发布前验证插件包，也可以显示来自自动化包扫描的发现。本页涵盖面向作者的发现，也就是插件作者可以在其包元数据、清单、SDK 导入或已发布工件中修复的发现。
...
| `package-npm-pack-unavailable`          | [让 npm 工件可打包](/zh-CN/clawhub/plugin-validation-fixes#package-npm-pack-unavailable)                                 |
...
- 当发布的包应加载已构建的 JavaScript 时，添加 `openclaw.runtimeExtensions`。
- 保持所有入口点路径位于包目录内。
...
包可以打包，但打包后的工件不包含 `package.json#openclaw` 中声明的入口点文件。

- 运行 `npm pack --dry-run` 并检查将包含的文件。
- 在打包前构建生成的入口点。
- 更新 `files`、`.npmignore` 或构建输出，确保已声明的入口点被包含。
- 参见[插件入口点](/zh-CN/plugins/sdk-entrypoints)。
- 重新运行 `clawhub package validate <path-to-plugin>`。

### package-npm-pack-metadata-missing

打包后的工件缺少你的源包中存在的 OpenClaw 元数据。
...
- 使用 `publishSessionTranscriptUpdateByIdentity(...)` 发送轨迹更新通知。
...
- 保持渠道环境变量元数据的声明式写法，以便 OpenClaw 可以在不加载渠道运行时的情况下检查设置状态。
- 将环境变量驱动的渠道设置镜像到当前设置、渠道配置或你的插件形态使用的包渠道元数据中。
...
- 仅在 OpenClaw 发布了文档化的版本化 schema 后再使用它。
- 重新运行 `clawhub package validate <path-to-plugin>`。
...
- 在清单契约存在之前，请将安全敏感行为记录在你的公共包文档或 README 中。
- 重新运行 `clawhub package validate <path-to-plugin>`。
