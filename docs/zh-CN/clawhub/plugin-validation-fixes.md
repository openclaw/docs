...
title: 插件验证修复
x-i18n:
    generated_at: "2026-07-01T10:56:55Z"
    model: gpt-5.5
...
ClawHub 会在发布前验证插件软件包，也可以显示来自自动化软件包扫描的发现。本页涵盖面向作者的发现，也就是插件作者可以在其软件包元数据、清单、SDK 导入或已发布产物中修复的发现。

它不涵盖内部 Plugin Inspector 覆盖率发现。如果完整报告包含扫描器维护代码，但没有作者修复指南，这些代码面向 OpenClaw 维护者，而不是插件作者。
...
| 代码                                    | 从这里开始                                                                                                                  |
| --------------------------------------- | --------------------------------------------------------------------------------------------------------------------------- |
| `package-json-missing`                  | [添加软件包元数据](/zh-CN/clawhub/plugin-validation-fixes#package-json-missing)                                                   |
| `package-openclaw-metadata-missing`     | [添加软件包 openclaw 块](/zh-CN/clawhub/plugin-validation-fixes#package-openclaw-metadata-missing)                            |
| `package-openclaw-entry-missing`        | [声明 OpenClaw 软件包入口点](/zh-CN/clawhub/plugin-validation-fixes#package-openclaw-entry-missing)                         |
...
| `package-manifest-version-drift`        | [对齐软件包和清单版本](/zh-CN/clawhub/plugin-validation-fixes#package-manifest-version-drift)                          |
| `package-openclaw-unsupported-metadata` | [移除不支持的 OpenClaw 软件包元数据](/zh-CN/clawhub/plugin-validation-fixes#package-openclaw-unsupported-metadata)          |
| `package-npm-pack-unavailable`          | [使 npm 产物可打包](/zh-CN/clawhub/plugin-validation-fixes#package-npm-pack-unavailable)                                 |
...
软件包根目录不包含 `package.json`，因此 ClawHub 无法识别 npm 软件包、版本、入口点或 OpenClaw 元数据。

- 添加包含 `name`、`version` 和 `type` 的 `package.json`。
- 当软件包交付 OpenClaw 插件时，添加 `openclaw` 块。
- 查看 [构建插件](/zh-CN/plugins/building-plugins) 获取最小软件包示例，并查看 [插件清单](/zh-CN/plugins/manifest#manifest-versus-packagejson) 了解软件包与清单的拆分。
- 重新运行 `clawhub package validate <path-to-plugin>`。

### package-openclaw-metadata-missing

软件包有 `package.json`，但没有声明 OpenClaw 软件包元数据。

- 添加 `package.json#openclaw`。
- 包含入口点元数据，例如 `openclaw.extensions` 或 `openclaw.runtimeExtensions`。
- 当软件包将通过 ClawHub 发布或安装时，添加兼容性和安装元数据。
- 查看 [影响设备发现的 package.json 字段](/zh-CN/plugins/manifest#packagejson-fields-that-affect-discovery)。
- 重新运行 `clawhub package validate <path-to-plugin>`。

### package-openclaw-entry-missing

软件包元数据存在，但没有声明 OpenClaw 运行时入口点。

- 为原生插件入口点添加 `openclaw.extensions`。
- 当发布的软件包应加载已构建的 JavaScript 时，添加 `openclaw.runtimeExtensions`。
- 保持所有入口点路径位于软件包目录内。
- 查看 [插件入口点](/zh-CN/plugins/sdk-entrypoints) 和 [影响设备发现的 package.json 字段](/zh-CN/plugins/manifest#packagejson-fields-that-affect-discovery)。
- 重新运行 `clawhub package validate <path-to-plugin>`。

### package-entrypoint-missing

软件包声明了 OpenClaw 入口点，但被引用的文件在正在验证的软件包中缺失。

- 检查 `openclaw.extensions`、`openclaw.runtimeExtensions`、`openclaw.setupEntry` 和 `openclaw.runtimeSetupEntry` 中的每个路径。
- 如果入口点生成到 `dist` 中，请构建软件包。
- 如果入口点已移动，请更新元数据。
- 查看 [插件入口点](/zh-CN/plugins/sdk-entrypoints)。
- 重新运行 `clawhub package validate <path-to-plugin>`。

### package-install-metadata-incomplete

ClawHub 无法判断软件包应如何安装或更新。

- 使用支持的安装来源填充 `openclaw.install`，例如 `clawhubSpec`、`npmSpec` 或 `localPath`。
- 当有多个安装来源可用时，设置 `openclaw.install.defaultChoice`。
- 使用 `openclaw.install.minHostVersion` 表示最低 OpenClaw 主机版本。
- 查看 [影响设备发现的 package.json 字段](/zh-CN/plugins/manifest#packagejson-fields-that-affect-discovery)。
- 重新运行 `clawhub package validate <path-to-plugin>`。

### package-plugin-api-compat-missing

软件包没有声明其支持的 OpenClaw 插件 API 范围。

- 将 `openclaw.compat.pluginApi` 添加到 `package.json`。
- 使用你构建并测试时所依据的 OpenClaw 插件 API 版本或 semver 下限。
- 将其与软件包版本分开。软件包版本描述插件发布；`openclaw.compat.pluginApi` 描述主机 API 契约。
- 查看 [影响设备发现的 package.json 字段](/zh-CN/plugins/manifest#packagejson-fields-that-affect-discovery)。
- 重新运行 `clawhub package validate <path-to-plugin>`。

### package-min-host-version-drift

软件包最低主机版本与该软件包构建时所依据的 OpenClaw 版本元数据不匹配。

- 检查 `openclaw.install.minHostVersion`。
- 检查软件包中的任何 OpenClaw 构建元数据，例如发布期间使用的 OpenClaw 版本。
- 将最低主机版本与软件包实际支持的主机版本范围对齐。
- 查看 [影响设备发现的 package.json 字段](/zh-CN/plugins/manifest#packagejson-fields-that-affect-discovery)。
- 重新运行 `clawhub package validate <path-to-plugin>`。
...
- 首选使用 `package.json#version` 作为软件包发布版本。
- 如果 `openclaw.plugin.json` 也有 `version`，请将其更新为匹配值，或在软件包元数据为权威来源时移除过期的清单版本元数据。
- 更改已发布元数据后，发布新的软件包版本。
- 查看 [插件清单](/zh-CN/plugins/manifest)。
- 重新运行 `clawhub package validate <path-to-plugin>`。

### package-openclaw-unsupported-metadata

`package.json#openclaw` 块包含不受支持的 OpenClaw 软件包元数据字段。

- 移除不支持的字段，例如 `openclaw.bundle`。
- 将原生插件元数据保留在 `openclaw.plugin.json` 中。
- 将软件包入口点、兼容性、安装、设置和目录元数据保留在受支持的 `package.json#openclaw` 字段中。
- 查看 [影响设备发现的 package.json 字段](/zh-CN/plugins/manifest#packagejson-fields-that-affect-discovery)。
- 重新运行 `clawhub package validate <path-to-plugin>`。
...
软件包无法打包为 ClawHub 将检查或发布的产物。

- 从软件包根目录运行 `npm pack --dry-run`。
- 修复会导致打包失败的无效软件包元数据、损坏的生命周期脚本或 files 条目。
- 如果该软件包打算公开发布，请移除 `private: true`。
- 重新运行 `clawhub package validate <path-to-plugin>`。

### package-npm-pack-entrypoint-missing

软件包可以打包，但打包后的产物不包含 `package.json#openclaw` 中声明的入口点文件。

- 运行 `npm pack --dry-run` 并检查将被包含的文件。
- 打包前构建生成的入口点。
- 更新 `files`、`.npmignore` 或构建输出，使已声明的入口点被包含。
- 查看 [插件入口点](/zh-CN/plugins/sdk-entrypoints)。
- 重新运行 `clawhub package validate <path-to-plugin>`。

### package-npm-pack-metadata-missing

打包后的产物缺少源软件包中存在的 OpenClaw 元数据。

- 运行 `npm pack --dry-run` 并检查包含的元数据文件。
- 确保 `package.json` 在打包后的产物中包含 `openclaw` 块。
- 当软件包是原生 OpenClaw 插件时，确保包含 `openclaw.plugin.json`。
- 更新 `files` 或 `.npmignore`，使软件包元数据不会被排除。
- 查看 [构建插件](/zh-CN/plugins/building-plugins)。
- 重新运行 `clawhub package validate <path-to-plugin>`。
...
- 保持 `name` 便于人类阅读，并将 `id` 保持为稳定的机器 ID。
...
插件导入了保留给内置插件或内部兼容性的 SDK 路径。

- 将保留的 OpenClaw 内部 SDK 导入替换为文档化的公共
  `openclaw/plugin-sdk/*` 子路径。
- 如果该行为没有公共 SDK，请将辅助工具保留在你的包内，或
  请求一个公共 OpenClaw API。
...
- 将提示词或上下文变更工作移至 `before_prompt_build`。
- 仅当你声明的兼容性范围仍支持需要它的旧版 OpenClaw 时，
...
清单使用了旧版或较旧的渠道环境变量元数据，但缺少 ClawHub 期望的当前
设置或配置元数据。

- 保持渠道环境变量元数据为声明式，以便 OpenClaw 无需加载渠道运行时即可检查设置状态。
- 将环境变量驱动的渠道设置镜像到你的插件形态所使用的当前设置、渠道配置或
  包渠道元数据中。
...
包附带的 `openclaw.security.json` 包含 ClawHub 未识别为可用的 schema 引用。

- 如果 schema URL 仅用于提示，请移除它。
- 仅在 OpenClaw 发布文档化的版本化 schema 后，才使用该 schema。
- 重新运行 `clawhub package validate <path-to-plugin>`。
