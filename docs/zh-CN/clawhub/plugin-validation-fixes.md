...
title: 插件验证修复
x-i18n:
    generated_at: "2026-07-02T07:55:08Z"
    model: gpt-5.5
...
ClawHub 会在发布前验证插件包，也可以显示来自自动包扫描的发现。本页涵盖面向作者的发现，也就是插件作者可以在其包元数据、清单、SDK 导入或已发布构件中修复的发现。

它不涵盖内部 Plugin Inspector 覆盖率发现。如果完整报告包含没有作者修复指导的扫描器维护代码，那些是给 OpenClaw 维护者看的，而不是给插件作者看的。
...
| `manifest-unknown-contracts`            | [移除不支持的合约键](/zh-CN/clawhub/plugin-validation-fixes#manifest-unknown-contracts)                                 |
...
- 将它与包版本分开。包版本描述插件发布；`openclaw.compat.pluginApi` 描述主机 API 合约。
- 参阅 [影响设备发现的 package.json 字段](/zh-CN/plugins/manifest#packagejson-fields-that-affect-discovery)。
- 重新运行 `clawhub package validate <path-to-plugin>`。

### package-min-host-version-drift

包的最低主机版本与包构建所依据的 OpenClaw 版本元数据不匹配。
...
- 参阅 [Building plugins](/zh-CN/plugins/building-plugins)。
- 重新运行 `clawhub package validate <path-to-plugin>`。
...
- 保持 `name` 为人类可读，并保持 `id` 作为稳定的机器 ID。
...
- 将保留的 OpenClaw 内部 SDK 导入替换为已文档化的公开
  `openclaw/plugin-sdk/*` 子路径。
- 如果该行为没有公开 SDK，请将辅助函数保留在你的包内，或
  请求公开的 OpenClaw API。
...
- 优先使用结构化转录运行时表面，以便 OpenClaw 可以应用正确的
  事务边界和身份处理。
- 仅当你声明的兼容范围仍支持需要它们的旧版 OpenClaw 时，
  才保留低层转录辅助函数。
- 请参阅 [运行时 API](/zh-CN/plugins/sdk-runtime#agent-session-state)和
...
清单使用了旧版或较旧的渠道环境变量元数据，缺少 ClawHub 预期的当前
设置或配置元数据。
...
包随附的 `openclaw.security.json` 包含 ClawHub 不识别为可用的 schema
引用。

- 如果 schema URL 仅作为建议，请将其移除。
