...
read_when:
    ...
    - ClawHub 拒绝插件包发布或给出警告
    - 你正在发布前更新插件包元数据
summary: 发布前修复 ClawHub 插件包验证问题
title: 插件验证修复
x-i18n:
    generated_at: "2026-06-30T22:05:12Z"
    model: gpt-5.5
...
它不涵盖内部 Plugin Inspector 覆盖率发现项。如果完整报告包含没有作者修复指导的扫描器维护代码，这些代码面向的是 OpenClaw 维护者，而不是插件作者。
...
- 当存在多个安装来源时，设置 `openclaw.install.defaultChoice`。
...
包的最低宿主版本与包构建所依据的 OpenClaw 版本元数据不匹配。
...
- 如果 `openclaw.plugin.json` 也有 `version`，请更新它以匹配，或在包元数据具有权威性时移除陈旧的清单版本元数据。
- 更改已发布元数据后，发布新的包版本。
...
包可以打包，但打包后的产物未包含 `package.json#openclaw` 中声明的入口点文件。
...
- 在 `openclaw.plugin.json` 中添加一个非空的 `name` 字段。
- 保持 `name` 便于人类阅读，并保持 `id` 作为稳定的机器 ID。
- 参阅 [Plugin Manifest](/zh-CN/plugins/manifest)。
- 重新运行 `clawhub package validate <path-to-plugin>`。
...
- 使用 [导入约定](/zh-CN/plugins/building-plugins#import-conventions) 和
  [插件 SDK 子路径](/zh-CN/plugins/sdk-subpaths) 找到更窄的导入。
- 重新运行 `clawhub package validate <path-to-plugin>`。
...
- 参阅 [运行时 API](/zh-CN/plugins/sdk-runtime#agent-session-state) 和
...
- 当代码需要结构化轨迹操作目标时，使用 `resolveSessionTranscriptTarget(...)`。
- 避免直接读取或构造旧版轨迹文件目标。
- 仅当你声明的兼容性范围仍支持需要它的旧版 OpenClaw 时，
  才保留旧版辅助函数。
- 参阅 [运行时 API](/zh-CN/plugins/sdk-runtime#agent-session-state) 和
...
- 使用 `appendSessionTranscriptMessageByIdentity(...)` 追加轨迹。
- 使用 `publishSessionTranscriptUpdateByIdentity(...)` 发送轨迹更新
  通知。
- 优先使用结构化轨迹运行时表面，以便 OpenClaw 可以应用正确的
  事务边界和身份处理。
- 仅当你声明的兼容性范围仍支持需要它们的旧版 OpenClaw 时，
  才保留低层轨迹辅助函数。
- 参阅 [运行时 API](/zh-CN/plugins/sdk-runtime#agent-session-state) 和
...
- 仅当你支持的 OpenClaw 范围仍需要它时，才将 `providerAuthEnvVars`
  保留为兼容性元数据。
- 参阅 [设置参考](/zh-CN/plugins/manifest#setup-reference) 和
  [SDK 迁移](/zh-CN/plugins/sdk-migration)。
- 重新运行 `clawhub package validate <path-to-plugin>`。

### channel-env-vars

清单使用旧版或较旧的渠道环境变量元数据，缺少 ClawHub 期望的当前
设置或配置元数据。

- 保持渠道环境变量元数据为声明式，以便 OpenClaw 无需加载渠道运行时
  即可检查设置状态。
- 将环境变量驱动的渠道设置镜像到你的插件形态所使用的当前设置、渠道
  配置或包渠道元数据中。
- 仅当较旧的受支持 OpenClaw 版本仍需要它时，才将 `channelEnvVars`
  保留为兼容性元数据。
- 参阅 [Plugin Manifest](/zh-CN/plugins/manifest) 和
...
包随附的 `openclaw.security.json` 包含 ClawHub 未识别为可用的 schema
引用。

- 如果 schema URL 仅供建议参考，请移除它。
- 仅在 OpenClaw 发布了已记录的版本化 schema 后，才使用它。
- 重新运行 `clawhub package validate <path-to-plugin>`。
...
- 在 OpenClaw 记录版本化安全清单 schema 和 ClawHub 行为之前，移除
