...
它不涵盖内部 Plugin Inspector 覆盖率发现。如果完整报告包含没有作者补救指导的扫描器维护代码，这些代码面向 OpenClaw 维护者，而不是插件作者。
...
包的最低主机版本与该包构建时基于的 OpenClaw 版本元数据不匹配。
...
- 让 `name` 保持人类可读，并将 `id` 保持为稳定的机器 ID。
- 请参阅 [插件清单](/zh-CN/plugins/manifest)。
- 重新运行 `clawhub package validate <path-to-plugin>`。
...
- 如果该行为没有公共 SDK，请将辅助函数保留在你的 package 内，或
  请求一个公共 OpenClaw API。
...
- 将由环境变量驱动的渠道设置镜像到当前设置、渠道配置，或
  你的插件形态使用的 package 渠道元数据中。
...
package 随附的 `openclaw.security.json` 使用了 ClawHub
无法识别为可用的 schema 引用。
...
- 移除 `openclaw.security.json`，直到 OpenClaw 文档化版本化安全
  清单 schema 和 ClawHub 行为。
- 在清单 contract 存在之前，将安全敏感行为记录在你的公开 package 文档或
  README 中。
- 重新运行 `clawhub package validate <path-to-plugin>`。
