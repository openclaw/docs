...
title: 插件验证修复
x-i18n:
    generated_at: "2026-07-04T10:26:45Z"
    model: gpt-5.5
...
它不涵盖内部 Plugin Inspector 覆盖率发现。如果完整报告包含没有作者修复指导的扫描器维护代码，这些是给 OpenClaw 维护者看的，而不是给插件作者看的。
...
| `manifest-unknown-contracts`            | [移除不受支持的合约键](/zh-CN/clawhub/plugin-validation-fixes#manifest-unknown-contracts)                                 |
...
- 将其与包版本分开。包版本描述插件发布；`openclaw.compat.pluginApi` 描述主机 API 合约。
- 参见 [影响设备发现的 package.json 字段](/zh-CN/plugins/manifest#packagejson-fields-that-affect-discovery)。
- 重新运行 `clawhub package validate <path-to-plugin>`。

### package-min-host-version-drift

包最低主机版本与该包构建时所针对的 OpenClaw 版本元数据不匹配。
...
- 如果 `openclaw.plugin.json` 也有 `version`，请将其更新为匹配版本；或者当包元数据是权威来源时，移除过时的清单版本元数据。
- 更改已发布元数据后，发布新的包版本。
...
插件仍在使用已弃用的 whole-session-store 辅助函数
`loadSessionStore`。
...
插件仍在使用已弃用的 whole-session-store 写入辅助函数，例如
...
- 保持渠道环境变量元数据为声明式，以便 OpenClaw 可以在不加载渠道
  运行时的情况下检查设置状态。
...
- 参见 [Plugin manifest](/zh-CN/plugins/manifest) 和
  [Channel Plugins](/zh-CN/plugins/sdk-channel-plugins)。
- 重新运行 `clawhub package validate <path-to-plugin>`。
...
包附带的 `openclaw.security.json` 包含 ClawHub 不识别为可用的
schema 引用。
