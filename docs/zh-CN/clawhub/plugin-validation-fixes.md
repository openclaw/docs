...
title: 插件验证修复
x-i18n:
    generated_at: "2026-07-01T15:19:11Z"
    model: gpt-5.5
...
ClawHub 会在发布前验证插件包，也可以显示来自自动化包扫描的发现项。本页涵盖面向作者的发现项，也就是插件作者可以在其包元数据、清单、SDK 导入或已发布制品中修复的发现项。

本页不涵盖内部 Plugin Inspector 覆盖率发现项。如果完整报告包含没有作者修复指导的扫描器维护代码，这些代码是给 OpenClaw 维护者看的，而不是给插件作者看的。
...
| `sdk-session-transcript-low-level`      | [替换低层级转录辅助函数](/zh-CN/clawhub/plugin-validation-fixes#sdk-session-transcript-low-level)                       |
...
包中有 `package.json`，但没有声明 OpenClaw 包元数据。
...
- 如果 `openclaw.plugin.json` 也有 `version`，请将其更新为一致；或者在包元数据具有权威性时移除陈旧的清单版本元数据。
- 更改已发布元数据后，发布新的包版本。
...
- 使用 `patchSessionEntry(...)` 或 `upsertSessionEntry(...)` 来持久化会话
  元数据。
- 当代码在准备 transcript 操作时，使用 transcript 身份或目标辅助工具。
...
插件仍在使用已弃用的 transcript 文件目标辅助工具
...
插件仍在使用已弃用的低层 transcript 辅助工具，例如
...
- 优先使用结构化的 transcript 运行时表面，以便 OpenClaw 可以应用正确的
  事务边界和身份处理。
- 仅在你声明的兼容范围仍支持需要它们的旧版 OpenClaw 时，
  才保留低层 transcript 辅助工具。
- 参见 [运行时 API](/zh-CN/plugins/sdk-runtime#agent-session-state)和
  [插件 SDK 子路径](/zh-CN/plugins/sdk-subpaths)。
- 重新运行 `clawhub package validate <path-to-plugin>`。
...
- 将环境变量驱动的渠道设置镜像到你的插件形态使用的当前设置、渠道配置或
  包渠道元数据中。
- 仅在较旧的受支持 OpenClaw 版本仍需要它时，才将 `channelEnvVars`
  保留为兼容性元数据。
...
- 如果 schema URL 仅用于建议性说明，请移除它。
- 仅在 OpenClaw 发布已记录的版本化 schema 后再使用。
- 重新运行 `clawhub package validate <path-to-plugin>`。
...
- 移除 `openclaw.security.json`，直到 OpenClaw 记录版本化安全
  清单 schema 和 ClawHub 行为。
- 在清单 contract 存在之前，将安全敏感行为记录在你的公开包文档或
  README 中。
- 重新运行 `clawhub package validate <path-to-plugin>`。
