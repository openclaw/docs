...
ClawHub 会在发布前验证插件包，也可以显示来自自动化包扫描的发现。本页涵盖面向作者的发现，也就是插件作者可以在其包元数据、清单、SDK 导入或发布产物中修复的发现。

本页不涵盖内部 Plugin Inspector 覆盖率发现。如果完整报告包含没有作者修复指导的扫描器维护代码，这些代码面向的是 OpenClaw 维护者，而不是插件作者。
...
- 当发布的包应加载构建后的 JavaScript 时，添加 `openclaw.runtimeExtensions`。
- 将所有入口点路径保持在包目录内。
...
- 如果 `openclaw.plugin.json` 也有 `version`，请将其更新为匹配，或在包元数据为权威来源时移除过时的清单版本元数据。
- 更改已发布元数据后，发布新的包版本。
...
- 如果该行为没有公共 SDK，请将辅助工具保留在你的软件包内，或
  请求公共 OpenClaw API。
...
清单使用旧版或较早的渠道环境变量元数据，缺少 ClawHub 预期的当前
设置或配置元数据。

- 保持渠道环境变量元数据为声明式，以便 OpenClaw 可以在不加载渠道
  运行时的情况下检查设置状态。
- 将环境变量驱动的渠道设置镜像到你的插件形态使用的当前设置、渠道
  配置或软件包渠道元数据中。
...
软件包随附的 `openclaw.security.json` 包含 ClawHub 不识别为可用的
schema 引用。
...
- 在清单契约存在之前，将安全敏感行为记录在你的公共软件包文档或
  README 中。
- 重新运行 `clawhub package validate <path-to-plugin>`。
