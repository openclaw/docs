...
title: 插件验证修复
x-i18n:
    generated_at: "2026-07-05T04:56:22Z"
    model: gpt-5.5
...
ClawHub 会在发布前验证插件包，也可以显示来自自动化包扫描的发现。此页面涵盖面向作者的发现，也就是插件作者可以在其包元数据、清单、SDK 导入或已发布产物中修复的发现。
...
| `package-npm-pack-unavailable`          | [让 npm 产物可以打包](/zh-CN/clawhub/plugin-validation-fixes#package-npm-pack-unavailable)                                 |
...
| `sdk-session-file-helper`               | [替换会话文件路径帮助器](/zh-CN/clawhub/plugin-validation-fixes#sdk-session-file-helper)                                   |
| `sdk-session-transcript-file-target`    | [替换旧版转录文件目标](/zh-CN/clawhub/plugin-validation-fixes#sdk-session-transcript-file-target)                   |
| `sdk-session-transcript-low-level`      | [替换低层转录帮助器](/zh-CN/clawhub/plugin-validation-fixes#sdk-session-transcript-low-level)                       |
...
- 使用你基于其构建并测试的 OpenClaw 插件 API 版本或 semver 下限。
...
包最低宿主版本与包构建时所基于的 OpenClaw 版本元数据不匹配。
...
- 如果 `openclaw.plugin.json` 也有 `version`，请更新它以匹配，或在包元数据为权威来源时移除陈旧的清单版本元数据。
- 更改已发布元数据后，发布新的包版本。
...
- 修复无效包元数据、损坏的生命周期脚本或导致打包失败的 files 条目。
...
清单使用旧版或较旧的渠道环境变量元数据，而没有 ClawHub 当前预期的
设置或配置元数据。

- 保持渠道环境变量元数据声明式，以便 OpenClaw 可以在不加载渠道运行时的情况下
  检查设置状态。
