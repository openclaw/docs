...
title: 插件验证修复
x-i18n:
    generated_at: "2026-07-05T01:54:33Z"
    model: gpt-5.5
...
ClawHub 会在发布前验证插件包，也可以显示自动包扫描得到的发现。本页涵盖面向作者的发现，也就是插件作者可以在其包元数据、清单、SDK 导入或已发布制品中修复的发现。

本页不涵盖内部插件检查器覆盖率发现。如果完整报告包含没有作者修复指导的扫描器维护代码，那些代码面向 OpenClaw 维护者，而不是插件作者。
...
| `sdk-session-transcript-low-level`      | [替换低层 transcript 辅助函数](/zh-CN/clawhub/plugin-validation-fixes#sdk-session-transcript-low-level)                       |
...
- 使用你构建并测试所基于的 OpenClaw 插件 API 版本或 semver 下限。
...
包的最低主机版本与该包构建所基于的 OpenClaw 版本元数据不匹配。
...
- 如果 `openclaw.plugin.json` 也有 `version`，请更新它以匹配；或者当包元数据是权威来源时，移除过时的清单版本元数据。
- 更改已发布的元数据后，发布新的包版本。
...
该包无法打包成 ClawHub 会检查或发布的制品。

- 从包根目录运行 `npm pack --dry-run`。
- 修复无效的包元数据、损坏的生命周期脚本，或导致打包失败的 files 条目。
- 如果该包 intended for public publishing，请移除 `private: true`。
- 重新运行 `clawhub package validate <path-to-plugin>`。
...
- 运行 `npm pack --dry-run` 并检查将会包含的文件。
- 在打包前构建生成的入口点。
...
- 避免加载、修改并保存整个会话存储对象。
- 仅当你声明的兼容性范围仍支持需要它的旧版 OpenClaw 时，才保留
  `loadSessionStore(...)`。
...
- 避免加载、修改并保存整个会话存储对象。
- 仅当你声明的兼容性范围仍支持需要它们的旧版 OpenClaw 时，才保留整个存储写入辅助工具。
- 请参阅 [运行时 API](/zh-CN/plugins/sdk-runtime#agent-session-state)和
  [插件 SDK 子路径](/zh-CN/plugins/sdk-subpaths)。
- 重新运行 `clawhub package validate <path-to-plugin>`。
...
- 避免直接读取或构造旧版转录文件目标。
- 仅当你声明的兼容性范围仍支持需要它的旧版 OpenClaw 时，才保留旧版辅助工具。
- 请参阅 [运行时 API](/zh-CN/plugins/sdk-runtime#agent-session-state)和
  [插件 SDK 子路径](/zh-CN/plugins/sdk-subpaths)。
- 重新运行 `clawhub package validate <path-to-plugin>`。
...
- 仅当你声明的兼容性范围仍支持需要它们的旧版 OpenClaw 时，才保留低级转录辅助工具。
- 请参阅 [运行时 API](/zh-CN/plugins/sdk-runtime#agent-session-state)和
  [插件 SDK 子路径](/zh-CN/plugins/sdk-subpaths)。
- 重新运行 `clawhub package validate <path-to-plugin>`。
...
- 仅当你声明的兼容性范围仍支持需要它的旧版 OpenClaw 时，才保留 `before_agent_start`。
...
- 仅当你支持的 OpenClaw 范围仍需要它时，才将 `providerAuthEnvVars` 作为兼容性元数据保留。
...
清单使用旧版或较旧的渠道环境变量元数据，缺少 ClawHub 预期的当前设置或配置元数据。

- 保持渠道环境变量元数据为声明式，这样 OpenClaw 可以在不加载渠道运行时的情况下检查设置状态。
- 将环境变量驱动的渠道设置镜像到你的插件形态所使用的当前设置、渠道配置或包渠道元数据中。
- 仅当仍受支持的旧版 OpenClaw 需要它时，才将 `channelEnvVars` 作为兼容性元数据保留。
...
包随附的 `openclaw.security.json` 包含一个 ClawHub 不识别为可用的 schema 引用。

- 如果 schema URL 仅作建议用途，请将其移除。
- 仅在 OpenClaw 发布版本化 schema 后，才使用已记录的版本化 schema。
- 重新运行 `clawhub package validate <path-to-plugin>`。
