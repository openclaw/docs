...
ClawHub 会在发布前验证插件包，也可以显示自动包扫描发现的问题。本页涵盖面向作者的发现项，也就是插件作者可以在其包元数据、清单、SDK 导入或已发布产物中修复的发现项。

本页不涵盖内部 Plugin Inspector 覆盖率发现项。如果完整报告包含没有作者修复指引的扫描器维护代码，这些代码面向 OpenClaw 维护者，而不是插件作者。
...
- 如果 `openclaw.plugin.json` 也有 `version`，请将其更新为匹配值；或者当包元数据是权威来源时，移除过时的清单版本元数据。
- 更改已发布元数据后，发布新的包版本。
...
- 将提示词或上下文修改工作移动到 `before_prompt_build`。
...
- 将环境变量驱动的渠道设置镜像到当前设置、频道配置或你的插件形态所使用的包渠道元数据中。
- 仅当较旧的受支持 OpenClaw 版本仍需要它时，才将 `channelEnvVars` 作为兼容性元数据保留。
...
包随附的 `openclaw.security.json` 带有一个 ClawHub 无法识别为可用的 schema 引用。
