...
read_when:
    ...
    - ClawHub 拒绝了插件包发布或给出了警告
    - 你正在发布前更新插件包元数据
summary: 修复 ClawHub 插件包发布前的验证发现
title: 插件验证修复
x-i18n:
    generated_at: "2026-07-05T17:40:24Z"
    model: gpt-5.5
...
ClawHub 会在发布前验证插件包，也可以显示来自自动化包扫描的发现。此页面涵盖面向作者的发现，也就是插件作者可以在其包元数据、清单、SDK 导入或已发布制品中修复的发现。

它不涵盖内部 Plugin Inspector 覆盖率发现。如果完整报告包含没有作者修复指引的扫描器维护代码，这些代码面向 OpenClaw 维护者，而不是插件作者。
...
| `package-npm-pack-unavailable`          | [让 npm 制品可打包](/zh-CN/clawhub/plugin-validation-fixes#package-npm-pack-unavailable)                                 |
...
包最低宿主版本与包构建所针对的 OpenClaw 版本元数据不匹配。
...
包可以打包，但打包后的制品不包含 `package.json#openclaw` 中声明的入口点文件。
...
- 确保 `package.json` 在打包制品中包含 `openclaw` 块。
...
- 使用 `getSessionEntry(...)` 按 agent 和会话标识读取会话元数据。
- 使用 `patchSessionEntry(...)` 或 `upsertSessionEntry(...)` 持久化会话元数据。
- 当代码在准备 transcript 操作时，使用 transcript 标识或目标辅助函数。
...
- 当代码只需要公共会话标识时，使用 `resolveSessionTranscriptIdentity(...)`。
...
- 优先使用结构化的 transcript 运行时表面，以便 OpenClaw 能应用正确的事务边界和标识处理。
- 仅当你声明的兼容范围仍支持需要它们的旧版 OpenClaw 时，
...
包随附的 `openclaw.security.json` 使用了 ClawHub
无法识别为可用的 schema 引用。

- 如果 schema URL 仅用于建议性说明，请将其移除。
