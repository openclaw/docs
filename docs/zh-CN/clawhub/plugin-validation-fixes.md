...
title: 插件验证修复
x-i18n:
    generated_at: "2026-07-02T00:43:09Z"
    model: gpt-5.5
...
ClawHub 会在发布前验证插件包，也可以显示自动包扫描得出的发现项。本页涵盖面向作者的发现项，也就是插件作者可以在其包元数据、清单、SDK 导入或已发布产物中修复的发现项。
...
| `sdk-session-transcript-low-level`      | [替换底层 transcript 辅助函数](/zh-CN/clawhub/plugin-validation-fixes#sdk-session-transcript-low-level)                       |
...
包有 `package.json`，但它未声明 OpenClaw 包元数据。
...
包的最低宿主版本与该包构建所基于的 OpenClaw 版本元数据不匹配。
...
- 对频道入口辅助函数使用 `openclaw/plugin-sdk/channel-core`。
...
- 当代码正在准备轨迹记录操作时，使用轨迹记录身份或目标辅助函数。
- 不要持久化或依赖旧版轨迹记录文件路径。
- 请参阅 [运行时 API](/zh-CN/plugins/sdk-runtime#agent-session-state) 和
  [插件 SDK 子路径](/zh-CN/plugins/sdk-subpaths)。
- 重新运行 `clawhub package validate <path-to-plugin>`。
...
插件仍在使用已弃用的轨迹记录文件目标辅助函数
`resolveSessionTranscriptLegacyFileTarget`。

- 当代码只需要公共会话身份时，使用 `resolveSessionTranscriptIdentity(...)`。
- 当代码需要结构化轨迹记录操作目标时，使用 `resolveSessionTranscriptTarget(...)`。
- 避免直接读取或构造旧版轨迹记录文件目标。
- 仅当你声明的兼容性范围仍支持需要它的较旧 OpenClaw 版本时，
  才保留旧版辅助函数。
- 请参阅 [运行时 API](/zh-CN/plugins/sdk-runtime#agent-session-state) 和
...
插件仍在使用已弃用的低级轨迹记录辅助函数，例如
`appendSessionTranscriptMessage` 或 `emitSessionTranscriptUpdate`。

- 使用 `appendSessionTranscriptMessageByIdentity(...)` 追加轨迹记录。
- 使用 `publishSessionTranscriptUpdateByIdentity(...)` 发送轨迹记录更新通知。
- 优先使用结构化轨迹记录运行时表面，以便 OpenClaw 能应用正确的事务边界和身份处理。
- 仅当你声明的兼容性范围仍支持需要它们的较旧 OpenClaw 版本时，
  才保留低级轨迹记录辅助函数。
- 请参阅 [运行时 API](/zh-CN/plugins/sdk-runtime#agent-session-state) 和
  [插件 SDK 子路径](/zh-CN/plugins/sdk-subpaths)。
...
清单使用旧版或较旧的频道环境变量元数据，缺少 ClawHub 期望的当前设置或配置元数据。

- 保持频道环境变量元数据为声明式，以便 OpenClaw 可以在不加载频道运行时的情况下检查设置状态。
- 将环境变量驱动的频道设置镜像到当前设置、频道配置，或你的插件形态使用的包频道元数据中。
...
包随附的 `openclaw.security.json` 带有一个 ClawHub
无法识别为可用的 schema 引用。
