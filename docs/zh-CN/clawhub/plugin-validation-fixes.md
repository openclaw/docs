---
read_when:
    - 你运行了 clawhub package validate，现在需要修复插件检查发现的问题
...
ClawHub 会在发布前验证插件包，也可以显示自动化包扫描的发现。本页介绍面向作者的发现，即插件作者可以通过修复包元数据、清单、SDK 导入或已发布工件来解决的发现。

本页不涵盖内部 Plugin Inspector 覆盖率发现。如果完整报告包含没有作者修复指导的扫描器维护代码，这些代码面向 OpenClaw 维护者，而非插件作者。
...
- 有关最小包示例，请参阅[构建插件](/zh-CN/plugins/building-plugins)；有关包与清单的职责划分，请参阅[插件清单](/zh-CN/plugins/manifest#manifest-versus-packagejson)。
...
- 使用支持的安装源填写 `openclaw.install`，例如 `clawhubSpec`、`npmSpec` 或 `localPath`。
...
- 将其与包版本分开。包版本描述插件版本；`openclaw.compat.pluginApi` 描述主机 API 契约。
- 请参阅[影响发现的 package.json 字段](/zh-CN/plugins/manifest#packagejson-fields-that-affect-discovery)。
- 重新运行 `clawhub package validate <path-to-plugin>`。
...
- 如果 `openclaw.plugin.json` 也包含 `version`，请更新该值以保持一致；如果包元数据是权威来源，则移除过时的清单版本元数据。
- 更改已发布的元数据后，发布新的包版本。
...
- 确保打包后的工件中，`package.json` 包含 `openclaw` 块。
- 当包是原生 OpenClaw 插件时，确保包含 `openclaw.plugin.json`。
- 更新 `files` 或 `.npmignore`，以确保包元数据不被排除。
...
- 保持 `name` 易于人类阅读，并将 `id` 保持为稳定的机器标识符。
- 请参阅[插件清单](/zh-CN/plugins/manifest)。
- 重新运行 `clawhub package validate <path-to-plugin>`。
...
- 将包或安装元数据移至受支持的 `package.json#openclaw` 字段中，
  而不是放在清单中。
- 重新运行 `clawhub package validate <path-to-plugin>`。
...
- 使用[导入约定](/zh-CN/plugins/building-plugins#import-conventions)和
  [插件 SDK 子路径](/zh-CN/plugins/sdk-subpaths)查找范围最小的导入。
- 重新运行 `clawhub package validate <path-to-plugin>`。
...
- 避免加载、修改并保存整个会话存储对象。
- 仅当你声明的兼容性范围仍支持需要这些辅助函数的旧版 OpenClaw 时，
  才继续使用完整存储写入辅助函数。
- 请参阅[运行时 API](/zh-CN/plugins/sdk-runtime#agent-session-state)和
  [插件 SDK 子路径](/zh-CN/plugins/sdk-subpaths)。
- 重新运行 `clawhub package validate <path-to-plugin>`。
...
- 使用 `publishSessionTranscriptUpdateByIdentity(...)` 发送转录更新
  通知。
- 优先使用结构化转录运行时接口，以便 OpenClaw 可以应用
  正确的事务边界和身份处理。
- 仅当你声明的兼容性范围仍支持需要这些辅助函数的旧版
  OpenClaw 时，才继续使用低级转录辅助函数。
- 请参阅[运行时 API](/zh-CN/plugins/sdk-runtime#agent-session-state)和
  [插件 SDK 子路径](/zh-CN/plugins/sdk-subpaths)。
- 重新运行 `clawhub package validate <path-to-plugin>`。
...
清单使用旧版或较早的渠道环境变量元数据，但缺少 ClawHub 所需的当前
设置或配置元数据。

- 保持渠道环境变量元数据为声明式，以便 OpenClaw 无需加载渠道运行时
  即可检查设置状态。
- 将由环境变量驱动的渠道设置同步到你的插件形态所使用的当前设置、渠道配置或
  包渠道元数据中。
- 仅当较旧但仍受支持的 OpenClaw 版本仍需要它时，才将 `channelEnvVars`
  保留为兼容性元数据。
...
包中包含 `openclaw.security.json`，但其架构引用未被 ClawHub
识别为可用架构。

- 如果架构 URL 仅用于提供参考，请将其移除。
...
- 在 OpenClaw 为版本化安全清单架构和 ClawHub 行为提供文档之前，
  移除 `openclaw.security.json`。
- 在清单契约形成之前，将安全敏感行为记录在包的公开文档或
  README 中。
- 重新运行 `clawhub package validate <path-to-plugin>`。
