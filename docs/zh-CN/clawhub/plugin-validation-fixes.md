...
title: 插件验证修复
x-i18n:
    generated_at: "2026-07-04T06:21:44Z"
    model: gpt-5.5
...
ClawHub 会在发布前验证插件包，也可以显示自动化包扫描的发现项。本页涵盖面向作者的发现项，也就是插件作者可以在其包元数据、清单、SDK 导入或已发布工件中修复的发现项。

它不涵盖内部 Plugin Inspector 覆盖率发现项。如果完整报告包含没有作者修复指导的扫描器维护代码，这些代码是面向 OpenClaw 维护者的，而不是插件作者。
...
- 如果 `openclaw.plugin.json` 也有 `version`，请更新它以匹配，或在包元数据权威时移除陈旧的清单版本元数据。
- 更改已发布的元数据后，发布新的包版本。
...
- 保持渠道环境变量元数据的声明式形式，以便 OpenClaw 能在不加载渠道运行时的情况下检查设置状态。
- 将由环境变量驱动的渠道设置镜像到你的插件形态使用的当前设置、渠道配置或软件包渠道元数据中。
...
软件包随附了 `openclaw.security.json`，其中包含 ClawHub
无法识别为可用的 schema 引用。
...
- 在清单契约存在之前，将安全敏感行为记录在你的公共软件包文档或 README 中。
- 重新运行 `clawhub package validate <path-to-plugin>`。
