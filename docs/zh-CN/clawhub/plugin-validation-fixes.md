...
ClawHub 会在发布前验证插件包，也可以显示来自自动化包扫描的发现。本页涵盖面向作者的发现，即插件作者可以在其包元数据、清单、SDK 导入或已发布产物中修复的发现。

它不涵盖内部 Plugin Inspector 覆盖率发现。如果完整报告包含没有作者修复指南的扫描器维护代码，这些代码面向的是 OpenClaw 维护者，而不是插件作者。
...
- 使用 [构建插件](/zh-CN/plugins/building-plugins) 查看最小包示例，并查看 [插件清单](/zh-CN/plugins/manifest#manifest-versus-packagejson) 了解包与清单的拆分。
...
包可以打包，但打包产物不包含 `package.json#openclaw` 中声明的入口点文件。
...
- 优先使用结构化 transcript 运行时表面，以便 OpenClaw 应用正确的
  事务边界和身份处理。
- 仅当你声明的兼容性范围仍支持需要它们的旧版 OpenClaw 时，
...
- 将 prompt 或上下文修改工作移到 `before_prompt_build`。
- 仅当你声明的兼容性范围仍支持需要它的旧版 OpenClaw 时，
...
- 保持渠道环境变量元数据为声明式，以便 OpenClaw 能够在不加载渠道运行时的情况下
  检查设置状态。
- 将由环境变量驱动的渠道设置镜像到你的插件形态所使用的当前设置、渠道配置或
  包渠道元数据中。
...
包随附的 `openclaw.security.json` 包含 ClawHub 无法识别为可用的
schema 引用。
