---
read_when:
    - 你运行了 clawhub package validate，需要修复插件问题
    - ClawHub 在插件包发布时拒绝或发出警告
    - 你正在发布前更新插件包元数据
summary: 发布前修复 ClawHub 插件包验证发现的问题
title: 插件验证修复
x-i18n:
    generated_at: "2026-07-02T13:57:56Z"
    model: gpt-5.5
...
ClawHub 会在发布前验证插件包，也可以显示自动化包扫描的发现项。本页涵盖面向作者的发现项，也就是插件作者可以在其包元数据、清单、SDK 导入或已发布产物中修复的发现项。

它不涵盖内部 Plugin Inspector 覆盖率发现项。如果完整报告包含没有作者修复指导的扫描器维护代码，那些代码面向 OpenClaw 维护者，而不是插件作者。
...
| `reserved-sdk-import`                   | [移除保留 SDK 导入](/zh-CN/clawhub/plugin-validation-fixes#reserved-sdk-import)                                             |
...
- 查看 [Building plugins](/zh-CN/plugins/building-plugins) 获取最小包示例，并查看 [Plugin manifest](/zh-CN/plugins/manifest#manifest-versus-packagejson) 了解包与清单的划分。
...
- 保持所有入口点路径都在包目录内。
- 请参阅 [插件入口点](/zh-CN/plugins/sdk-entrypoints) 和 [影响发现的 package.json 字段](/zh-CN/plugins/manifest#packagejson-fields-that-affect-discovery)。
- 重新运行 `clawhub package validate <path-to-plugin>`。
...
- 用支持的安装来源填充 `openclaw.install`，例如 `clawhubSpec`、`npmSpec` 或 `localPath`。
- 当有多个安装来源可用时，设置 `openclaw.install.defaultChoice`。
- 将 `openclaw.install.minHostVersion` 用于最低 OpenClaw 宿主版本。
- 请参阅 [影响发现的 package.json 字段](/zh-CN/plugins/manifest#packagejson-fields-that-affect-discovery)。
- 重新运行 `clawhub package validate <path-to-plugin>`。

### package-plugin-api-compat-missing

包没有声明它支持的 OpenClaw 插件 API 范围。

- 将 `openclaw.compat.pluginApi` 添加到 `package.json`。
- 使用你构建并测试所针对的 OpenClaw 插件 API 版本或 semver 下限。
- 将其与包版本分开。包版本描述插件发布；`openclaw.compat.pluginApi` 描述宿主 API 契约。
- 请参阅 [影响发现的 package.json 字段](/zh-CN/plugins/manifest#packagejson-fields-that-affect-discovery)。
- 重新运行 `clawhub package validate <path-to-plugin>`。

### package-min-host-version-drift

包最低宿主版本与该包构建所针对的 OpenClaw 版本元数据不匹配。
...
- 如果 `openclaw.plugin.json` 也有 `version`，请更新它以匹配；当包元数据具有权威性时，移除过时的清单版本元数据。
- 更改已发布元数据后，发布新的包版本。
- 请参阅 [Plugin manifest](/zh-CN/plugins/manifest)。
- 重新运行 `clawhub package validate <path-to-plugin>`。
...
包无法打包成 ClawHub 将检查或发布的产物。

- 从包根目录运行 `npm pack --dry-run`。
- 修复无效的包元数据、损坏的生命周期脚本，或导致打包失败的 files 条目。
- 如果此包预期用于公开发布，请移除 `private: true`。
- 重新运行 `clawhub package validate <path-to-plugin>`。

### package-npm-pack-entrypoint-missing

包可以打包，但打包后的产物不包含 `package.json#openclaw` 中声明的入口点文件。
...
- 请参阅 [Building plugins](/zh-CN/plugins/building-plugins)。
- 重新运行 `clawhub package validate <path-to-plugin>`。
...
- 向 `openclaw.plugin.json` 添加一个非空的 `name` 字段。
- 让 `name` 保持人类可读，并让 `id` 作为稳定的机器 ID。
...
- 将保留的 OpenClaw 内部 SDK 导入替换为有文档说明的公共
...
- 避免加载、修改并保存整个会话存储对象。
- 只有当你声明的兼容性范围仍支持需要它的较旧 OpenClaw 版本时，
...
- 避免加载、修改并保存整个会话存储对象。
- 只有当你声明的兼容性范围仍支持需要它们的较旧 OpenClaw 版本时，
  才保留完整存储写入辅助函数。
- 参见 [运行时 API](/zh-CN/plugins/sdk-runtime#agent-session-state)和
  [插件 SDK 子路径](/zh-CN/plugins/sdk-subpaths)。
- 重新运行 `clawhub package validate <path-to-plugin>`。
...
- 使用 `patchSessionEntry(...)` 或 `upsertSessionEntry(...)` 持久化会话
  元数据。
- 当代码正在准备 transcript 操作时，使用 transcript 身份或目标辅助函数。
...
- 优先使用结构化 transcript 运行时表面，以便 OpenClaw 能应用正确的
  事务边界和身份处理。
- 只有当你声明的兼容性范围仍支持需要它们的较旧 OpenClaw 版本时，
...
- 只有当你支持的 OpenClaw 范围仍需要它时，才将 `providerAuthEnvVars`
  保留为兼容性元数据。
...
清单使用旧版或较旧的渠道环境变量元数据，缺少 ClawHub 期望的当前设置或配置元数据。

- 保持渠道环境变量元数据声明式，这样 OpenClaw 可以在不加载渠道运行时的情况下
  检查设置状态。
- 将环境变量驱动的渠道设置镜像到你的插件形态所使用的当前设置、渠道配置或
  包渠道元数据中。
- 只有当较旧的受支持 OpenClaw 版本仍需要它时，才将 `channelEnvVars`
  保留为兼容性元数据。
...
- 只有在 OpenClaw 发布有文档说明的版本化 schema 后，才使用它。
- 重新运行 `clawhub package validate <path-to-plugin>`。
...
- 在 OpenClaw 记录版本化安全清单 schema 和 ClawHub 行为之前，
