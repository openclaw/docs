...
title: 插件验证修复
x-i18n:
    generated_at: "2026-07-02T22:22:07Z"
    model: gpt-5.5
...
ClawHub 会在发布前验证插件包，也可以显示自动包扫描发现的问题。本页介绍面向作者的问题，也就是插件作者可以在其包元数据、清单、SDK 导入或已发布产物中修复的问题。

它不涵盖内部 Plugin Inspector 覆盖率问题。如果完整报告包含没有作者修复指导的扫描器维护代码，那些代码面向 OpenClaw 维护者，而不是插件作者。
...
| `channel-env-vars`                      | [在当前元数据中镜像频道环境变量](/zh-CN/clawhub/plugin-validation-fixes#channel-env-vars)                                |
...
- 参考 [Building plugins](/zh-CN/plugins/building-plugins) 获取最小包示例，并参考 [插件清单](/zh-CN/plugins/manifest#manifest-versus-packagejson) 了解包与清单的拆分。
...
- 使用你构建和测试所依据的 OpenClaw 插件 API 版本或 semver 下限。
...
- 如果 `openclaw.plugin.json` 也有 `version`，请更新它以匹配；或者当包元数据具有权威性时，移除过时的清单版本元数据。
- 更改已发布元数据后，发布新的包版本。
...
- 在打包前构建生成的入口点。
- 更新 `files`、`.npmignore` 或构建输出，确保包含已声明的入口点。
- 参见 [插件入口点](/zh-CN/plugins/sdk-entrypoints)。
- 重新运行 `clawhub package validate <path-to-plugin>`。
...
- 确保打包后的产物中的 `package.json` 包含 `openclaw` 块。
- 当包是原生 OpenClaw 插件时，确保包含 `openclaw.plugin.json`。
- 更新 `files` 或 `.npmignore`，确保不会排除包元数据。
...
- 使用 [导入约定](/zh-CN/plugins/building-plugins#import-conventions)和
  [插件 SDK 子路径](/zh-CN/plugins/sdk-subpaths)查找窄导入。
- 重新运行 `clawhub package validate <path-to-plugin>`。
...
- 避免加载、修改并保存整个会话存储对象。
- 仅当你声明的兼容性范围仍支持需要它的旧版 OpenClaw 时，才保留 `loadSessionStore(...)`。
...
- 避免加载、修改并保存整个会话存储对象。
- 仅当你声明的兼容性范围仍支持需要它们的旧版 OpenClaw 时，才保留完整存储写入辅助函数。
- 参见 [运行时 API](/zh-CN/plugins/sdk-runtime#agent-session-state)和
  [插件 SDK 子路径](/zh-CN/plugins/sdk-subpaths)。
- 重新运行 `clawhub package validate <path-to-plugin>`。
...
- 避免直接读取或构造旧版转录文件目标。
- 仅当你声明的兼容性范围仍支持需要它的旧版 OpenClaw 时，才保留旧版辅助函数。
- 参见 [运行时 API](/zh-CN/plugins/sdk-runtime#agent-session-state)和
  [插件 SDK 子路径](/zh-CN/plugins/sdk-subpaths)。
- 重新运行 `clawhub package validate <path-to-plugin>`。
...
- 仅当你声明的兼容性范围仍支持需要它们的旧版 OpenClaw 时，才保留低层转录辅助函数。
- 参见 [运行时 API](/zh-CN/plugins/sdk-runtime#agent-session-state)和
  [插件 SDK 子路径](/zh-CN/plugins/sdk-subpaths)。
- 重新运行 `clawhub package validate <path-to-plugin>`。
...
- 将 prompt 或上下文修改工作移至 `before_prompt_build`。
...
包随附的 `openclaw.security.json` 中包含 ClawHub 不识别为可用的 schema 引用。

- 如果 schema URL 仅用于建议说明，请移除它。
- 仅在 OpenClaw 发布已文档化的版本化 schema 后，才使用该 schema。
- 重新运行 `clawhub package validate <path-to-plugin>`。
...
- 在清单合同存在之前，将安全敏感行为记录在你的公共包文档或 README 中。
- 重新运行 `clawhub package validate <path-to-plugin>`。
