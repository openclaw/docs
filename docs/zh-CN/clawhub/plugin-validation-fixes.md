...
title: 插件验证修复
x-i18n:
    generated_at: "2026-06-28T05:08:40Z"
    model: gpt-5.5
...
ClawHub 会在发布前验证插件包，也可以显示来自自动化包扫描的发现。本页涵盖面向作者的发现，也就是插件作者可以在其包元数据、清单、SDK 导入或已发布产物中修复的发现。

本页不涵盖内部 Plugin Inspector 覆盖率发现。如果完整报告包含没有作者修复指导的扫描器维护代码，这些内容是给 OpenClaw 维护者使用的，而不是给插件作者使用的。
...
- 为 Native Codex plugins 入口点添加 `openclaw.extensions`。
...
- 如果 `openclaw.plugin.json` 也有 `version`，请将其更新为匹配版本；当包元数据是权威来源时，也可以移除陈旧的清单版本元数据。
- 更改已发布元数据后，发布新的包版本。
...
- 确保打包产物中的 `package.json` 包含 `openclaw` 块。
...
- 保持 `name` 便于人类阅读，并将 `id` 作为稳定的机器 ID。
- 参见 [插件清单](/zh-CN/plugins/manifest)。
- 重新运行 `clawhub package validate <path-to-plugin>`。

### manifest-unknown-fields

插件清单具有 OpenClaw 不支持的顶级字段。

- 将每个顶级字段与
  [清单字段参考](/zh-CN/plugins/manifest#top-level-field-reference)对照。
...
- 将 `contracts` 下的每个键与
  [contracts 参考](/zh-CN/plugins/manifest#contracts-reference)对照。
- 移除不受支持的 contract 键。
...
- 如果该行为没有公共 SDK，请将辅助工具保留在你的包内，或请求公共 OpenClaw API。
...
清单使用了旧版或较早的渠道环境变量元数据，缺少 ClawHub 预期的当前设置或配置元数据。

- 保持渠道环境变量元数据为声明式，以便 OpenClaw 无需加载渠道运行时即可检查设置状态。
- 将由环境变量驱动的渠道设置镜像到你的插件形态所使用的当前设置、渠道配置或包渠道元数据中。
- 仅在较旧且受支持的 OpenClaw 版本仍需要它时，才将 `channelEnvVars`
  保留为兼容性元数据。
...
包随附的 `openclaw.security.json` 包含 ClawHub 不识别为可用的 schema 引用。
