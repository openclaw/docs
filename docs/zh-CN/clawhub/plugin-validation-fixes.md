...
title: 插件验证修复
x-i18n:
    generated_at: "2026-07-06T10:47:45Z"
    model: gpt-5.5
...
ClawHub 会在发布前验证插件包，也可以显示来自自动化包扫描的发现项。此页面涵盖面向作者的发现项，也就是插件作者可以在其包元数据、清单、SDK 导入或已发布工件中修复的发现项。
...
包的最低宿主版本与包构建时所依据的 OpenClaw 版本元数据不匹配。
...
- 如果 `openclaw.plugin.json` 也有 `version`，请更新它以匹配；或者当包元数据具有权威性时，移除陈旧的清单版本元数据。
- 更改已发布元数据后，发布新的包版本。
...
包无法打包成 ClawHub 将检查或发布的工件。

- 从包根目录运行 `npm pack --dry-run`。
- 修复无效包元数据、损坏的生命周期脚本，或导致打包失败的 files 条目。
...
- 如果该架构 URL 仅用于建议，请移除它。
- 仅在 OpenClaw 发布文档化的版本化架构后使用它。
- 重新运行 `clawhub package validate <path-to-plugin>`。
...
- 移除 `openclaw.security.json`，直到 OpenClaw 文档化版本化的安全
  清单架构和 ClawHub 行为。
