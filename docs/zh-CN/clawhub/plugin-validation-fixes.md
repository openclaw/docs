...
read_when:
    ...
    - 你正在发布前更新插件包元数据
summary: 修复发布前 ClawHub 插件包验证发现的问题
title: 插件验证修复
x-i18n:
...
它不涵盖内部 Plugin Inspector 覆盖率发现。如果完整报告包含没有作者修复指南的扫描器维护代码，那些代码面向 OpenClaw 维护者，而不是插件作者。
...
| `sdk-session-file-helper`               | [替换会话文件路径辅助工具](/zh-CN/clawhub/plugin-validation-fixes#sdk-session-file-helper)                                   |
| `sdk-session-transcript-file-target`    | [替换旧版转录文件目标](/zh-CN/clawhub/plugin-validation-fixes#sdk-session-transcript-file-target)                   |
| `sdk-session-transcript-low-level`      | [替换低层转录辅助工具](/zh-CN/clawhub/plugin-validation-fixes#sdk-session-transcript-low-level)                       |
...
- 查看[构建插件](/zh-CN/plugins/building-plugins)获取最小包示例，并查看[插件清单](/zh-CN/plugins/manifest#manifest-versus-packagejson)了解包与清单的拆分。
...
- 保持所有入口点路径位于包目录内。
- 参见[插件入口点](/zh-CN/plugins/sdk-entrypoints)和[影响发现的 package.json 字段](/zh-CN/plugins/manifest#packagejson-fields-that-affect-discovery)。
- 重新运行 `clawhub package validate <path-to-plugin>`。

### package-entrypoint-missing

包声明了 OpenClaw 入口点，但被引用文件在正在验证的包中缺失。
...
- 使用支持的安装来源填写 `openclaw.install`，例如 `clawhubSpec`、`npmSpec` 或 `localPath`。
- 当可用安装来源超过一个时，设置 `openclaw.install.defaultChoice`。
...
包最低宿主版本与构建该包时所依据的 OpenClaw 版本元数据不匹配。
...
- 如果 `openclaw.plugin.json` 也有 `version`，请将其更新为匹配值，或在包元数据具有权威性时移除陈旧的清单版本元数据。
- 更改已发布元数据后，发布新的包版本。
...
- 修复会导致打包失败的无效包元数据、损坏的生命周期脚本或 files 条目。
- 如果该包意图公开发布，请移除 `private: true`。
- 重新运行 `clawhub package validate <path-to-plugin>`。

### package-npm-pack-entrypoint-missing

该包可以打包，但打包后的构件未包含 `package.json#openclaw` 中声明的入口点文件。

- 运行 `npm pack --dry-run` 并检查将被包含的文件。
- 打包前构建生成的入口点。
- 更新 `files`、`.npmignore` 或构建输出，确保声明的入口点被包含。
- 参见[插件入口点](/zh-CN/plugins/sdk-entrypoints)。
- 重新运行 `clawhub package validate <path-to-plugin>`。
...
- 将包或安装元数据移到受支持的 `package.json#openclaw` 字段中，
  而不是放在清单里。
- 重新运行 `clawhub package validate <path-to-plugin>`。
...
- 避免加载、修改并保存整个会话存储对象。
- 仅当你声明的兼容范围仍支持需要它的旧版 OpenClaw 时，
...
- 避免加载、修改并保存整个会话存储对象。
- 仅当你声明的兼容范围仍支持需要它们的旧版 OpenClaw 时，
  才保留整存储写入辅助函数。
- 参见[运行时 API](/zh-CN/plugins/sdk-runtime#agent-session-state)和
  [插件 SDK 子路径](/zh-CN/plugins/sdk-subpaths)。
- 重新运行 `clawhub package validate <path-to-plugin>`。
...
插件仍在使用已弃用的轨迹文件目标辅助函数
`resolveSessionTranscriptLegacyFileTarget`。

- 当代码只需要公共会话身份时，使用 `resolveSessionTranscriptIdentity(...)`。
- 当代码需要结构化的轨迹操作目标时，使用 `resolveSessionTranscriptTarget(...)`。
- 避免直接读取或构造旧版轨迹文件目标。
- 仅当你声明的兼容范围仍支持需要它的旧版 OpenClaw 时，
  才保留旧版辅助函数。
- 参见[运行时 API](/zh-CN/plugins/sdk-runtime#agent-session-state)和
...
插件仍在使用已弃用的低层轨迹辅助函数，例如
`appendSessionTranscriptMessage` 或 `emitSessionTranscriptUpdate`。

- 对轨迹追加使用 `appendSessionTranscriptMessageByIdentity(...)`。
- 对轨迹更新通知使用 `publishSessionTranscriptUpdateByIdentity(...)`。
- 优先使用结构化轨迹运行时表面，以便 OpenClaw 可以应用正确的事务边界和身份处理。
- 仅当你声明的兼容范围仍支持需要它们的旧版 OpenClaw 时，
  才保留低层轨迹辅助函数。
- 参见[运行时 API](/zh-CN/plugins/sdk-runtime#agent-session-state)和
  [插件 SDK 子路径](/zh-CN/plugins/sdk-subpaths)。
...
- 仅当你支持的 OpenClaw 范围仍需要它时，
  才将 `providerAuthEnvVars` 保留为兼容性元数据。
...
包随附的 `openclaw.security.json` 包含一个 ClawHub 无法识别为可用的 schema 引用。

- 如果该 schema URL 仅用于建议，请移除它。
...
- 移除 `openclaw.security.json`，直到 OpenClaw 文档化版本化安全清单 schema 和 ClawHub 行为。
