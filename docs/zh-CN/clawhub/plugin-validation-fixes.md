...
title: 插件验证修复
x-i18n:
    generated_at: "2026-07-03T09:22:13Z"
    model: gpt-5.5
...
ClawHub 会在发布前验证插件包，也可以显示自动包扫描的发现项。本页涵盖面向作者的发现项，也就是插件作者可以在其包元数据、清单、SDK 导入或已发布构件中修复的发现项。
...
| `package-openclaw-metadata-missing`     | [添加包 openclaw 区块](/zh-CN/clawhub/plugin-validation-fixes#package-openclaw-metadata-missing)                            |
...
- 当包发布 OpenClaw 插件时，添加 `openclaw` 区块。
...
- 使用你构建并测试时所针对的 OpenClaw 插件 API 版本或 semver 下限。
...
包的最低主机版本与构建该包时所针对的 OpenClaw 版本元数据不匹配。
...
`package.json#openclaw` 区块包含不受支持的 OpenClaw 包元数据字段。
...
- 确保打包后的构件中的 `package.json` 包含 `openclaw` 区块。
...
- 优先使用结构化的转录运行时表面，以便 OpenClaw 应用正确的事务边界和身份处理。
- 仅当你声明的兼容范围仍支持需要它们的旧版 OpenClaw 时，
  才保留低层转录辅助函数。
- 请参阅 [运行时 API](/zh-CN/plugins/sdk-runtime#agent-session-state)和
  [插件 SDK 子路径](/zh-CN/plugins/sdk-subpaths)。
...
清单使用了旧版或较旧的渠道环境变量元数据，但没有 ClawHub 期望的当前设置或配置元数据。

- 保持渠道环境变量元数据为声明式，以便 OpenClaw 能够在不加载渠道运行时的情况下检查设置状态。
...
包随附了 `openclaw.security.json`，其中包含 ClawHub 无法识别为可用的 schema 引用。

- 如果 schema URL 仅用于建议，请移除它。
- 只有在 OpenClaw 发布文档化的版本化 schema 后，才使用它。
- 重新运行 `clawhub package validate <path-to-plugin>`。
