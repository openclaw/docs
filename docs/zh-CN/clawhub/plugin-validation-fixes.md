---
read_when:
    - 你运行了 clawhub package validate，需要修复插件检查结果
    - ClawHub 拒绝了插件包发布或发出警告
    - 你正在发布前更新插件包元数据
summary: 发布前修复 ClawHub 插件包验证发现的问题
title: 插件验证修复
x-i18n:
    generated_at: "2026-07-03T13:15:28Z"
    model: gpt-5.5
...
ClawHub 会在发布前验证插件包，也可以显示来自自动化包扫描的发现。本页涵盖面向作者的发现，也就是插件作者可以在其包元数据、清单、SDK 导入或已发布构件中修复的发现。

本页不涵盖内部 Plugin Inspector 覆盖率发现。如果完整报告包含没有作者补救指导的扫描器维护代码，这些代码面向 OpenClaw 维护者，而不是插件作者。
...
| `package-npm-pack-unavailable`          | [让 npm 构件可打包](/zh-CN/clawhub/plugin-validation-fixes#package-npm-pack-unavailable)                                 |
...
| `sdk-session-file-helper`               | [替换会话文件路径助手](/zh-CN/clawhub/plugin-validation-fixes#sdk-session-file-helper)                                   |
| `sdk-session-transcript-file-target`    | [替换旧版转录文件目标](/zh-CN/clawhub/plugin-validation-fixes#sdk-session-transcript-file-target)                   |
| `sdk-session-transcript-low-level`      | [替换低级转录助手](/zh-CN/clawhub/plugin-validation-fixes#sdk-session-transcript-low-level)                       |
...
- 添加带有 `name`、`version` 和 `type` 的 `package.json`。
...
包有 `package.json`，但未声明 OpenClaw 包元数据。
...
- 当已发布的包应加载已构建的 JavaScript 时，添加 `openclaw.runtimeExtensions`。
- 保持所有入口点路径位于包目录内。
- 参见 [Plugin entry points](/zh-CN/plugins/sdk-entrypoints) 和 [影响设备发现的 package.json 字段](/zh-CN/plugins/manifest#packagejson-fields-that-affect-discovery)。
- 重新运行 `clawhub package validate <path-to-plugin>`。
...
- 当可用安装来源不止一个时，设置 `openclaw.install.defaultChoice`。
...
- 如果 `openclaw.plugin.json` 也有 `version`，请更新它以匹配；或者当包元数据是权威来源时，移除陈旧的清单版本元数据。
- 更改已发布元数据后，发布新的包版本。
...
- 将包入口点、兼容性、安装、设置和目录元数据保留在支持的 `package.json#openclaw` 字段中。
- 参见 [影响设备发现的 package.json 字段](/zh-CN/plugins/manifest#packagejson-fields-that-affect-discovery)。
- 重新运行 `clawhub package validate <path-to-plugin>`。
...
打包后的构件缺少源包中存在的 OpenClaw 元数据。

- 运行 `npm pack --dry-run` 并检查已包含的元数据文件。
- 确保 `package.json` 在打包后的构件中包含 `openclaw` 块。
- 当包是原生 OpenClaw 插件时，确保包含 `openclaw.plugin.json`。
- 更新 `files` 或 `.npmignore`，使包元数据不会被排除。
...
- 向 `openclaw.plugin.json` 添加非空 `name` 字段。
- 保持 `name` 易于人类阅读，并保持 `id` 作为稳定的机器 ID。
...
- 将保留的 OpenClaw 内部 SDK 导入替换为已文档化的公共
  `openclaw/plugin-sdk/*` 子路径。
- 如果该行为没有公共 SDK，请将辅助函数保留在你的包内，或
  请求一个公共 OpenClaw API。
...
- 当代码在准备 transcript 操作时，使用 transcript 身份或目标辅助函数。
- 不要持久化或依赖旧版 transcript 文件路径。
- 请参阅[运行时 API](/zh-CN/plugins/sdk-runtime#agent-session-state)和
  [插件 SDK 子路径](/zh-CN/plugins/sdk-subpaths)。
- 重新运行 `clawhub package validate <path-to-plugin>`。
...
插件仍在使用已弃用的 transcript 文件目标辅助函数
`resolveSessionTranscriptLegacyFileTarget`。

- 当代码只需要公共会话身份时，使用 `resolveSessionTranscriptIdentity(...)`。
- 当代码需要结构化的 transcript 操作目标时，使用 `resolveSessionTranscriptTarget(...)`。
- 避免直接读取或构造旧版 transcript 文件目标。
- 仅当你声明的兼容性范围仍支持需要它的旧版 OpenClaw 时，
  才保留旧版辅助函数。
- 请参阅[运行时 API](/zh-CN/plugins/sdk-runtime#agent-session-state)和
  [插件 SDK 子路径](/zh-CN/plugins/sdk-subpaths)。
...
插件仍在使用已弃用的低层 transcript 辅助函数，例如
`appendSessionTranscriptMessage` 或 `emitSessionTranscriptUpdate`。

- 对 transcript 追加使用 `appendSessionTranscriptMessageByIdentity(...)`。
- 对 transcript 更新通知使用 `publishSessionTranscriptUpdateByIdentity(...)`。
- 优先使用结构化 transcript 运行时表面，这样 OpenClaw 可以应用正确的事务边界和身份处理。
- 仅当你声明的兼容性范围仍支持需要它们的旧版 OpenClaw 时，
  才保留低层 transcript 辅助函数。
- 请参阅[运行时 API](/zh-CN/plugins/sdk-runtime#agent-session-state)和
  [插件 SDK 子路径](/zh-CN/plugins/sdk-subpaths)。
- 重新运行 `clawhub package validate <path-to-plugin>`。
...
- 仅当你支持的 OpenClaw 范围仍需要它时，
  才将 `providerAuthEnvVars` 作为兼容性元数据保留。
...
清单使用了旧版或较旧的渠道环境变量元数据，但缺少 ClawHub 期望的当前设置或配置元数据。

- 保持渠道环境变量元数据为声明式，这样 OpenClaw 无需加载渠道运行时即可检查设置状态。
- 将环境驱动的渠道设置镜像到你的插件形态所使用的当前设置、渠道配置或包渠道元数据中。
- 仅当较旧的受支持 OpenClaw 版本仍需要它时，
  才将 `channelEnvVars` 作为兼容性元数据保留。
...
包发布了带有 schema 引用的 `openclaw.security.json`，但 ClawHub
无法识别该 schema 为可用。
...
- 在 OpenClaw 文档化版本化安全清单 schema 和 ClawHub 行为之前，移除 `openclaw.security.json`。
