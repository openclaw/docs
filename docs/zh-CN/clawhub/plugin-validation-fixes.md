...
title: 插件验证修复
x-i18n:
    generated_at: "2026-07-05T11:05:35Z"
    model: gpt-5.5
...
ClawHub 会在发布前验证插件包，也可以显示来自自动化包扫描的发现。此页面覆盖面向作者的发现，也就是插件作者可以在其包元数据、清单、SDK 导入或已发布制品中修复的发现。

它不覆盖内部 Plugin Inspector 覆盖率发现。如果完整报告包含没有作者修复指导的扫描器维护代码，这些代码面向 OpenClaw 维护者，而不是插件作者。
...
包的最低主机版本与该包构建时依据的 OpenClaw 版本元数据不匹配。
...
- 如果 `openclaw.plugin.json` 也有 `version`，请将其更新为匹配；或者当包元数据具有权威性时，移除过时的清单版本元数据。
- 更改已发布元数据后，发布新的包版本。
...
- 将 package 或安装元数据移入受支持的 `package.json#openclaw` 字段，
  而不是放在清单中。
- 重新运行 `clawhub package validate <path-to-plugin>`。
...
- 使用 [导入约定](/zh-CN/plugins/building-plugins#import-conventions)和
  [插件 SDK 子路径](/zh-CN/plugins/sdk-subpaths)来查找更窄的导入。
- 重新运行 `clawhub package validate <path-to-plugin>`。
...
- 如果该行为没有公共 SDK，请将辅助工具保留在你的 package 内，或请求公共 OpenClaw API。
...
- 避免加载、变更并保存整个会话存储对象。
- 仅在你声明的兼容性范围仍支持需要它的旧版 OpenClaw 时，才保留 `loadSessionStore(...)`。
...
- 避免加载、变更并保存整个会话存储对象。
- 仅在你声明的兼容性范围仍支持需要它们的旧版 OpenClaw 时，才保留整存储写入辅助工具。
- 参见 [运行时 API](/zh-CN/plugins/sdk-runtime#agent-session-state)和
  [插件 SDK 子路径](/zh-CN/plugins/sdk-subpaths)。
- 重新运行 `clawhub package validate <path-to-plugin>`。
...
- 当代码正在准备 transcript 操作时，使用 transcript 身份或目标辅助工具。
...
- 仅在你声明的兼容性范围仍支持需要它的旧版 OpenClaw 时，才保留旧版辅助工具。
- 参见 [运行时 API](/zh-CN/plugins/sdk-runtime#agent-session-state)和
  [插件 SDK 子路径](/zh-CN/plugins/sdk-subpaths)。
- 重新运行 `clawhub package validate <path-to-plugin>`。
...
- 优先使用结构化 transcript 运行时表面，以便 OpenClaw 能应用正确的事务边界和身份处理。
- 仅在你声明的兼容性范围仍支持需要它们的旧版 OpenClaw 时，才保留低层 transcript 辅助工具。
- 参见 [运行时 API](/zh-CN/plugins/sdk-runtime#agent-session-state)和
  [插件 SDK 子路径](/zh-CN/plugins/sdk-subpaths)。
- 重新运行 `clawhub package validate <path-to-plugin>`。
...
- 将提示或上下文变更工作移至 `before_prompt_build`。
- 仅在你声明的兼容性范围仍支持需要它的旧版 OpenClaw 时，才保留 `before_agent_start`。
...
- 仅在你支持的 OpenClaw 范围仍需要它时，才将 `providerAuthEnvVars` 保留为兼容性元数据。
...
- 将环境变量驱动的渠道设置镜像到你的插件形态所使用的当前设置、渠道配置或 package 渠道元数据中。
- 仅在较旧的受支持 OpenClaw 版本仍需要它时，才将 `channelEnvVars` 保留为兼容性元数据。
...
package 随附的 `openclaw.security.json` 带有 ClawHub 不识别为可用的 schema 引用。

- 如果 schema URL 仅供提示，请将其移除。
- 仅在 OpenClaw 发布已文档化的版本化 schema 后再使用它。
- 重新运行 `clawhub package validate <path-to-plugin>`。

### unrecognized-security-manifest

package 随附了不受支持的安全清单文件。

- 在 OpenClaw 文档化版本化安全清单 schema 和 ClawHub 行为之前，移除 `openclaw.security.json`。
- 在清单契约存在之前，将安全敏感行为记录在你的公共 package 文档或 README 中。
- 重新运行 `clawhub package validate <path-to-plugin>`。
