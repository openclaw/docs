...
ClawHub 会在发布前验证插件包，也可以显示来自自动化包扫描的发现项。本页介绍面向作者的发现项，也就是插件作者可以在其包元数据、清单、SDK 导入或已发布制品中修复的发现项。

它不涵盖内部 Plugin Inspector 覆盖率发现项。如果完整报告包含没有作者修复指引的扫描器维护代码，这些代码面向 OpenClaw 维护者，而不是插件作者。
...
- 参考 [构建插件](/zh-CN/plugins/building-plugins) 获取最小包示例，并参考 [插件清单](/zh-CN/plugins/manifest#manifest-versus-packagejson) 了解包与清单的拆分。
...
- 如果 `openclaw.plugin.json` 也有 `version`，请将其更新为匹配值，或者在包元数据具有权威性时移除陈旧的清单版本元数据。
- 更改已发布元数据后，发布新的包版本。
...
插件仍在使用已弃用的低级 transcript 辅助工具，例如
...
- 优先使用结构化的 transcript 运行时表面，以便 OpenClaw 可以应用
  正确的事务边界和身份处理。
- 仅当你声明的兼容性范围仍支持需要它们的旧版 OpenClaw 时，
  才保留低级 transcript 辅助工具。
- 参见 [运行时 API](/zh-CN/plugins/sdk-runtime#agent-session-state)和
  [插件 SDK 子路径](/zh-CN/plugins/sdk-subpaths)。
- 重新运行 `clawhub package validate <path-to-plugin>`。
...
- 仅在 OpenClaw 发布文档化的版本化 schema 后才使用它。
- 重新运行 `clawhub package validate <path-to-plugin>`。
