...
- 使用 `openclaw.install.minHostVersion` 设置最低 OpenClaw 宿主版本。
- 参见[影响设备发现的 package.json 字段](/zh-CN/plugins/manifest#packagejson-fields-that-affect-discovery)。
- 重新运行 `clawhub package validate <path-to-plugin>`。
...
- 优先使用结构化转录运行时表面，以便 OpenClaw 能应用正确的
  事务边界和身份处理。
- 仅当你声明的兼容范围仍支持需要它们的旧版 OpenClaw 时，
  才保留低层转录辅助函数。
- 参见[运行时 API](/zh-CN/plugins/sdk-runtime#agent-session-state)和
...
包随附的 `openclaw.security.json` 包含一个 ClawHub 不识别为可用的
schema 引用。

- 如果 schema URL 只是建议性信息，请将其移除。
