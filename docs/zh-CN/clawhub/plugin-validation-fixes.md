...
read_when:
    ...
    - ClawHub 在发布插件包时拒绝或发出警告
    - 你正在发布前更新插件包元数据
summary: 发布前修复 ClawHub 插件包验证发现的问题
title: 插件验证修复
x-i18n:
    generated_at: "2026-07-04T20:24:02Z"
    model: gpt-5.5
...
ClawHub 会在发布前验证插件包，也可以显示自动化包扫描的发现。本页涵盖面向作者的发现，也就是插件作者可以在自己的包元数据、清单、SDK 导入或已发布产物中修复的发现。
...
包声明了 OpenClaw 入口点，但被验证的包中缺少引用的文件。
...
包的最低主机版本与该包构建时所针对的 OpenClaw 版本元数据不匹配。
...
- 保持 `name` 便于人类阅读，并保持 `id` 作为稳定的机器标识。
...
- 如果该行为没有公开 SDK，请将辅助函数保留在你的包内，或请求公开的 OpenClaw API。
...
- 将由环境驱动的渠道设置镜像到你的插件形态所使用的当前设置、渠道配置或包渠道元数据中。
...
包随附的 `openclaw.security.json` 包含 ClawHub 不认可为可用的架构引用。

- 如果架构 URL 仅用于建议说明，请将其移除。
- 仅在 OpenClaw 发布文档化的版本化架构后，才使用该架构。
- 重新运行 `clawhub package validate <path-to-plugin>`。
...
- 在 OpenClaw 文档化版本化安全清单架构和 ClawHub 行为之前，移除 `openclaw.security.json`。
