...
read_when:
    ...
    - 你正在发布前更新插件包元数据
summary: 发布前修复 ClawHub 插件包验证发现
title: 插件验证修复
x-i18n:
    generated_at: "2026-06-28T05:04:14Z"
    model: gpt-5.5
...
ClawHub 会在发布前验证插件包，也可以显示来自自动包扫描的发现。此页面涵盖面向作者的发现，也就是插件作者可以在其包元数据、清单、SDK 导入或已发布产物中修复的发现。
...
- 使用 [Building Plugins](/zh-CN/plugins/building-plugins) 查看最小包示例，并参考 [Plugin Manifest](/zh-CN/plugins/manifest#manifest-versus-packagejson) 了解包与清单的分离。
...
- 当包会通过 ClawHub 发布或安装时，添加兼容性和安装元数据。
...
包声明了 OpenClaw 入口点，但在正在验证的包中缺少引用的文件。
...
- 用支持的安装源填写 `openclaw.install`，例如 `clawhubSpec`、`npmSpec` 或 `localPath`。
...
- 将它与包版本分开。包版本描述插件发布；`openclaw.compat.pluginApi` 描述宿主 API 合约。
- 参见 [影响设备发现的 package.json 字段](/zh-CN/plugins/manifest#packagejson-fields-that-affect-discovery)。
- 重新运行 `clawhub package validate <path-to-plugin>`。
...
- 保持 `name` 适合人类阅读，并将 `id` 保持为稳定的机器 ID。
- 参见 [Plugin Manifest](/zh-CN/plugins/manifest)。
- 重新运行 `clawhub package validate <path-to-plugin>`。
...
- 使用[导入约定](/zh-CN/plugins/building-plugins#import-conventions)和
  [插件 SDK 子路径](/zh-CN/plugins/sdk-subpaths)来查找窄导入。
- 重新运行 `clawhub package validate <path-to-plugin>`。
...
清单使用了旧版或更早的渠道环境变量元数据，缺少 ClawHub 当前期望的
设置或配置元数据。
...
包随附的 `openclaw.security.json` 包含一个 ClawHub
无法识别为可用的 schema 引用。
