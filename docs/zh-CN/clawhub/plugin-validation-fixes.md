...
read_when:
    ...
    - 你正在发布前更新插件包元数据
summary: 修复发布前的 ClawHub 插件包验证发现的问题
title: 插件验证修复
x-i18n:
    generated_at: "2026-07-03T02:42:22Z"
    model: gpt-5.5
...
ClawHub 会在发布前验证插件包，也可以显示来自自动包扫描的发现。此页面涵盖面向作者的发现，也就是插件作者可以在其包元数据、清单、SDK 导入或已发布构件中修复的发现。

它不涵盖内部 Plugin Inspector 覆盖率发现。如果完整报告包含没有作者修复指导的扫描器维护代码，那些代码面向 OpenClaw 维护者，而不是插件作者。
...
| `package-npm-pack-unavailable`          | [让 npm 构件可以打包](/zh-CN/clawhub/plugin-validation-fixes#package-npm-pack-unavailable)                                 |
...
| `manifest-unknown-contracts`            | [移除不支持的契约键名](/zh-CN/clawhub/plugin-validation-fixes#manifest-unknown-contracts)                                 |
...
| `sdk-session-store-write`               | [替换整会话存储写入](/zh-CN/clawhub/plugin-validation-fixes#sdk-session-store-write)                                  |
...
- 使用 [Building Plugins](/zh-CN/plugins/building-plugins) 查看最小包示例，并使用 [插件清单](/zh-CN/plugins/manifest#manifest-versus-packagejson) 了解包与清单的分工。
...
- 使用你构建并测试所依据的 OpenClaw 插件 API 版本或 semver 下限。
...
包的最低宿主版本与该包构建所依据的 OpenClaw 版本元数据不匹配。
...
- 如果 `openclaw.plugin.json` 也有 `version`，请更新它以匹配，或在包元数据为权威来源时移除过时的清单版本元数据。
- 更改已发布元数据后，发布新的包版本。
...
该包无法打包成 ClawHub 将检查或发布的构件。

- 从包根目录运行 `npm pack --dry-run`。
- 修复无效的包元数据、损坏的生命周期脚本，或导致打包失败的 files 条目。
- 如果此包 intended for public publishing，请移除 `private: true`。
- 重新运行 `clawhub package validate <path-to-plugin>`。

### package-npm-pack-entrypoint-missing

包可以打包，但打包后的构件不包含 `package.json#openclaw` 中声明的入口点文件。
...
打包后的构件缺少你源包中存在的 OpenClaw 元数据。

- 运行 `npm pack --dry-run` 并检查包含的元数据文件。
- 确保 `package.json` 在打包后的构件中包含 `openclaw` 块。
- 当该包是原生 OpenClaw 插件时，确保 `openclaw.plugin.json` 被包含。
...
- 使用 [导入约定](/zh-CN/plugins/building-plugins#import-conventions)和
  [插件 SDK 子路径](/zh-CN/plugins/sdk-subpaths)查找更窄的导入。
- 重新运行 `clawhub package validate <path-to-plugin>`。
...
- 如果该行为没有公共 SDK，请将辅助函数保留在你的包内，或请求公共 OpenClaw API。
...
- 避免加载、修改并保存整个会话存储对象。
- 仅当你声明的兼容范围仍支持需要它的旧版 OpenClaw 时，才保留 `loadSessionStore(...)`。
...
- 避免加载、修改并保存整个会话存储对象。
- 仅当你声明的兼容范围仍支持需要这些辅助函数的旧版 OpenClaw 时，才保留完整存储写入辅助函数。
- 参见 [运行时 API](/zh-CN/plugins/sdk-runtime#agent-session-state)和
  [插件 SDK 子路径](/zh-CN/plugins/sdk-subpaths)。
- 重新运行 `clawhub package validate <path-to-plugin>`。
...
- 避免直接读取或构造旧版转录文件目标。
- 仅当你声明的兼容范围仍支持需要它的旧版 OpenClaw 时，才保留旧版辅助函数。
- 参见 [运行时 API](/zh-CN/plugins/sdk-runtime#agent-session-state)和
  [插件 SDK 子路径](/zh-CN/plugins/sdk-subpaths)。
- 重新运行 `clawhub package validate <path-to-plugin>`。
...
- 优先使用结构化转录运行时表面，以便 OpenClaw 可以应用正确的事务边界和身份处理。
- 仅当你声明的兼容范围仍支持需要这些辅助函数的旧版 OpenClaw 时，才保留低层级转录辅助函数。
- 参见 [运行时 API](/zh-CN/plugins/sdk-runtime#agent-session-state)和
  [插件 SDK 子路径](/zh-CN/plugins/sdk-subpaths)。
- 重新运行 `clawhub package validate <path-to-plugin>`。
...
- 仅当你声明的兼容范围仍支持需要它的旧版 OpenClaw 时，才保留 `before_agent_start`。
...
- 仅当你支持的 OpenClaw 范围仍需要它时，才将 `providerAuthEnvVars` 保留为兼容性元数据。
...
- 保持渠道环境变量元数据声明式，以便 OpenClaw 可以在不加载渠道运行时的情况下检查设置状态。
...
包随附的 `openclaw.security.json` 使用了 ClawHub 不识别为可用的 schema 引用。

- 如果 schema URL 仅用于建议说明，请将其移除。
...
- 在清单契约存在之前，将安全敏感行为记录在你的公共包文档或 README 中。
- 重新运行 `clawhub package validate <path-to-plugin>`。
