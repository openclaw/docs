---
read_when:
    - 你运行了 `clawhub package validate`，需要修复插件发现项
    - ClawHub 拒绝了插件包发布或发出警告
    - 你正在发布前更新插件软件包元数据
summary: 发布前修复 ClawHub 插件包验证发现的问题
title: 插件验证修复
x-i18n:
    generated_at: "2026-07-03T00:51:51Z"
    model: gpt-5.5
...
ClawHub 会在发布前验证插件包，也可以显示自动化包扫描中的发现。此页面涵盖面向作者的发现，即插件作者可以在其包元数据、清单、SDK 导入或已发布构件中修复的发现。

它不涵盖内部 Plugin Inspector 覆盖率发现。如果完整报告包含没有作者修复指导的扫描器维护代码，这些代码面向 OpenClaw 维护者，而不是插件作者。
...
- 使用[构建插件](/zh-CN/plugins/building-plugins)查看最小包示例，并使用[插件清单](/zh-CN/plugins/manifest#manifest-versus-packagejson)了解包与清单的拆分。
...
- 向 `package.json` 添加 `openclaw.compat.pluginApi`。
...
包的最低宿主版本与该包构建所针对的 OpenClaw 版本元数据不匹配。
...
- 如果 `openclaw.plugin.json` 也有 `version`，请将其更新为匹配值；当包元数据是权威来源时，移除陈旧的清单版本元数据。
- 更改已发布元数据后，发布新的包版本。
...
- 在打包前构建生成的入口点。
- 更新 `files`、`.npmignore` 或构建输出，以便包含已声明的入口点。
- 请参阅[插件入口点](/zh-CN/plugins/sdk-entrypoints)。
- 重新运行 `clawhub package validate <path-to-plugin>`。
...
插件仍在使用已弃用的完整 session store 写入辅助函数，例如
`saveSessionStore` 或 `updateSessionStore`。

- 更新现有会话条目上的字段时，使用 `patchSessionEntry(...)`。
- 替换或创建会话条目时，使用 `upsertSessionEntry(...)`。
- 避免加载、修改并保存整个 session store 对象。
- 仅在你声明的兼容范围仍支持需要它们的旧版 OpenClaw 时，
  才保留完整 store 写入辅助函数。
- 请参阅 [运行时 API](/zh-CN/plugins/sdk-runtime#agent-session-state)和
  [插件 SDK 子路径](/zh-CN/plugins/sdk-subpaths)。
- 重新运行 `clawhub package validate <path-to-plugin>`。
...
- 优先使用结构化 transcript 运行时表面，以便 OpenClaw 应用正确的事务边界和身份处理。
- 仅在你声明的兼容范围仍支持需要它们的旧版 OpenClaw 时，
...
- 保持渠道环境变量元数据声明式，以便 OpenClaw 无需加载渠道运行时即可检查设置状态。
- 将由环境变量驱动的渠道设置镜像到你的插件形态所使用的当前设置、渠道配置或包渠道元数据中。
- 仅在较旧的受支持 OpenClaw 版本仍需要它时，
...
- 仅在 OpenClaw 发布文档化的版本化 schema 后使用它。
- 重新运行 `clawhub package validate <path-to-plugin>`。
...
- 在清单 contract 存在之前，将安全敏感行为记录在你的公共包文档或
  README 中。
- 重新运行 `clawhub package validate <path-to-plugin>`。
