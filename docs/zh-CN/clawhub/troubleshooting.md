...
x-i18n:
    generated_at: "2026-07-14T13:30:47Z"
    model: gpt-5.6
...
- `RateLimit-Remaining`：存在此响应头时，你的确切剩余额度。在 `429` 上，其值为 `0`。
- `RateLimit-Reset` 或 `X-RateLimit-Reset`：重置时间。

如果许多用户共享同一个出口 IP，即使每个人只发送少量请求，也可能触发匿名 IP 限制。请尽可能登录，并在报告的延迟时间后重试。
...
如果发布因所有者账号、组织命名空间、软件包作用域、Skills slug 或软件包名称已被占用或保留而失败，请先确认你使用的发布所有者与该命名空间匹配。对于插件软件包，
`@example-org/example-plugin` 等带作用域的名称必须以
匹配的 `example-org` 所有者身份发布。

如果你认为自己的组织、项目或品牌是该命名空间的合法所有者，但
无法管理当前的 ClawHub 所有者，请创建
[组织/命名空间认领问题](https://github.com/openclaw/clawhub/issues/new?template=org-namespace-claim.yml)，
并提供公开且不敏感的证明。有关证据指引以及不应放入公开问题的内容，请参阅
