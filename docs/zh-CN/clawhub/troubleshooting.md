...
x-i18n:
    generated_at: "2026-06-28T05:09:02Z"
    model: gpt-5.5
...
- `RateLimit-Remaining`：当该标头存在时，你的准确剩余额度。在 `429` 时，它是 `0`。
...
对于插件，请检查 `package.json` 兼容性元数据。代码插件发布需要 OpenClaw 兼容性字段，例如 `openclaw.compat.pluginApi` 和
...
如果发布因所有者 handle、组织命名空间、包作用域、Skills slug 或包名称已被声明或保留而失败，请先确认你正在使用与该命名空间匹配的所有者发布。对于插件包，像 `@example-org/example-plugin` 这样的带作用域名称必须以匹配的 `example-org` 所有者发布。

如果你认为你的组织、项目或品牌是该命名空间的合法所有者，但你无法管理当前 ClawHub 所有者，请使用公开、非敏感的证明提交
[组织 / 命名空间声明问题](https://github.com/openclaw/clawhub/issues/new?template=org-namespace-claim.yml)。
有关证据指导以及哪些内容不要放入公开问题，请参阅
[组织和命名空间声明](/zh-CN/clawhub/namespace-claims)。
...
- 如果该包被隐藏、暂扣或阻止，则在所有者解决问题之前，它可能无法安装。
