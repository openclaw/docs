...
x-i18n:
    generated_at: "2026-07-04T20:24:17Z"
    model: gpt-5.5
...
- `RateLimit-Remaining`：存在该标头时你的确切剩余额度。在 `429` 时，它是 `0`。
...
- 确保你使用拥有或可发布该包的 GitHub 账号登录。
- 检查源 URL 是否公开或可供 ClawHub 访问。
- 对于 GitHub 源，请使用 `owner/repo`、`owner/repo@ref` 或完整的 GitHub URL。

## 发布失败，因为命名空间已被认领或保留

如果发布失败是因为所有者 handle、组织命名空间、包作用域、技能 slug 或包名称已被认领或保留，请先确认你正使用与该命名空间匹配的所有者进行发布。对于插件包，`@example-org/example-plugin` 这类带作用域的名称必须以匹配的 `example-org` 所有者发布。

如果你认为你的组织、项目或品牌是合法的命名空间所有者，但无法管理当前 ClawHub 所有者，请提交一个
[组织 / 命名空间认领 issue](https://github.com/openclaw/clawhub/issues/new?template=org-namespace-claim.yml)
并附上公开、非敏感的证明。参阅
[组织和命名空间认领](/clawhub/namespace-claims)，了解证据指南以及哪些内容不要放入公开 issue。
...
- 如果该包被隐藏、暂缓或阻止，可能要等所有者解决问题后才能安装。

## Public API 请求失败

- 遵守 `429` 重试标头，并缓存公共列表/搜索响应。
- 将用户链接回规范的 ClawHub 列表。
- 不要在 public API 表面之外镜像隐藏、私有、暂缓或被审核阻止的内容。
