...
x-i18n:
    generated_at: "2026-07-01T18:07:18Z"
    model: gpt-5.5
...
- `RateLimit-Remaining`：当此标头存在时，你的精确剩余额度。在 `429` 上，它为 `0`。
...
ClawHub 使用 GitHub 身份和来源归因，将包与其发布者关联起来。

- 确保你使用拥有或可以发布该包的 GitHub 账户登录。
- 检查来源 URL 是公开的，或 ClawHub 可访问。
- 对于 GitHub 来源，使用 `owner/repo`、`owner/repo@ref` 或完整的 GitHub URL。

## 发布失败，因为命名空间已被认领或保留

如果发布失败是因为所有者 handle、组织命名空间、包 scope、技能 slug 或包名已被认领或保留，请先确认你正使用与该命名空间匹配的所有者发布。对于插件包，像 `@example-org/example-plugin` 这样的 scoped 名称必须以匹配的 `example-org` 所有者发布。

如果你认为你的组织、项目或品牌是该命名空间的合法所有者，但你无法管理当前的 ClawHub 所有者，请使用公开、非敏感的证明打开一个 [组织 / 命名空间认领 issue](https://github.com/openclaw/clawhub/issues/new?template=org-namespace-claim.yml)。请参阅 [组织和命名空间认领](/clawhub/namespace-claims)，了解证据指导以及哪些内容不应放入公开 issue。
...
本地文件不匹配 ClawHub 已知的任何版本。选择一种方式：
...
- 如果包被隐藏、拦截或阻止，在所有者解决问题之前，它可能无法安装。

## 公共 API 请求失败

- 遵守 `429` 重试标头，并缓存公共列表/搜索响应。
- 将用户引回规范的 ClawHub 列表页。
- 不要在公共 API 表面之外镜像隐藏、私有、被拦截或被审核阻止的内容。

请参阅 [HTTP API](/clawhub/http-api) 了解端点详情。
