...
x-i18n:
    generated_at: "2026-07-04T15:08:06Z"
    model: gpt-5.5
...
- 在无头环境中，在 ClawHub Web UI 中创建一个 API 令牌并运行：
...
- `RateLimit-Remaining`：当标头存在时，你的精确剩余额度。在 `429` 时，它是 `0`。
- `RateLimit-Reset` 或 `X-RateLimit-Reset`：重置时间。

如果许多用户共享同一个出站 IP，即使每个人只发送少量请求，也可能触发匿名 IP 限制。尽可能登录，并在报告的延迟后重试。
...
- 如果你知道精确的 slug 或所有者页面，请检查它。
- 确认发布是公开的，并且没有被扫描或审核流程暂挂。
- 如果你拥有该技能，请登录并检查它：
...
ClawHub 使用 GitHub 身份和来源归属来将包连接到其发布者。
...
如果发布失败是因为所有者 handle、组织命名空间、包 scope、技能 slug 或包名已被认领或保留，请先确认你正在使用与该命名空间匹配的所有者进行发布。对于插件包，诸如 `@example-org/example-plugin` 这样的 scoped 名称必须以匹配的 `example-org` 所有者发布。

如果你认为你的组织、项目或品牌是该命名空间的合法所有者，但你无法管理当前的 ClawHub 所有者，请使用公开、非敏感的证明打开一个 [Org / Namespace Claim issue](https://github.com/openclaw/clawhub/issues/new?template=org-namespace-claim.yml)。请参阅 [Org and Namespace Claims](/clawhub/namespace-claims)，了解证据指南以及哪些内容不应放入公开 issue。
...
- 如果包被隐藏、暂挂或阻止，它可能需要等所有者解决问题后才能安装。
