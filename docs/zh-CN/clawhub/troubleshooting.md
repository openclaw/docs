...
x-i18n:
    generated_at: "2026-06-27T01:34:40Z"
    model: gpt-5.5
...
- 如果回调一直未到达，请检查本地防火墙、VPN 和代理规则。
- 在无头环境中，请在 ClawHub 网页 UI 中创建 API 令牌并运行：
...
- `RateLimit-Limit`：应用到此请求的限制。
- `RateLimit-Remaining`：当响应头存在时，你的准确剩余额度。在 `429` 时，它为 `0`。
...
- 如果你知道准确的 slug 或所有者页面，请检查它。
- 确认该发布是公开的，且未被扫描或审核流程暂缓。
- 如果你拥有该技能，请登录并检查它：
...
## 发布失败，出现 GitHub 所有者或来源错误

ClawHub 使用 GitHub 身份和来源归属将包与其发布者关联起来。

- 确保你已使用拥有该包或可发布该包的 GitHub 账号登录。
- 检查源 URL 是公开的，或 ClawHub 可访问。
- 对于 GitHub 来源，请使用 `owner/repo`、`owner/repo@ref` 或完整的 GitHub URL。

## 发布失败，因为命名空间已被声明或保留

如果发布失败是因为所有者 handle、组织命名空间、包 scope、技能 slug 或包名已被声明或保留，请先确认你正使用与命名空间匹配的所有者进行发布。对于插件包，诸如 `@example-org/example-plugin` 的 scoped 名称必须作为匹配的 `example-org` 所有者发布。

如果你认为自己的组织、项目或品牌是该命名空间的合法所有者，但你无法管理当前 ClawHub 所有者，请提交一个 [组织 / 命名空间声明问题](https://github.com/openclaw/clawhub/issues/new?template=org-namespace-claim.yml)，并提供公开、非敏感的证明。请参阅[组织和命名空间声明](/zh-CN/clawhub/namespace-claims)，了解证据指南以及哪些内容不应放入公开 issue。
...
本地文件与 ClawHub 知道的任何版本都不匹配。请选择一种方式：
...
- 如果该包被隐藏、暂缓或阻止，它可能无法安装，直到所有者解决该问题。

## 公共 API 请求失败

- 遵守 `429` 重试响应头，并缓存公共列表/搜索响应。
