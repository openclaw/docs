...
read_when:
    ...
    - 无法安装、发布或更新软件包
summary: 故障排查 ClawHub 登录、安装、发布、更新和 API 问题。
x-i18n:
...
- `RateLimit-Remaining`：当该标头存在时，你的精确剩余额度。在 `429` 时，它是 `0`。
...
ClawHub 使用 GitHub 身份和来源归属将包关联到其发布者。

- 确保你已使用拥有或可发布该包的 GitHub 账户登录。
- 检查来源 URL 是否公开，或 ClawHub 是否可访问。
- 对于 GitHub 来源，使用 `owner/repo`、`owner/repo@ref` 或完整的 GitHub URL。

## 发布失败，因为命名空间已被认领或保留

如果发布失败是因为所有者 handle、组织命名空间、包作用域、技能 slug 或包名称已被认领或保留，请先确认你正在使用与该命名空间匹配的所有者发布。对于插件包，诸如 `@example-org/example-plugin` 的 scoped 名称必须作为匹配的 `example-org` 所有者发布。

如果你认为你的组织、项目或品牌是该命名空间的合法所有者，但你无法管理当前的 ClawHub 所有者，请使用公开的非敏感证明提交一个 [Org / Namespace Claim issue](https://github.com/openclaw/clawhub/issues/new?template=org-namespace-claim.yml)。请参阅 [Org and Namespace Claims](/clawhub/namespace-claims)，了解证据指南以及哪些内容不应放入公开 issue。
...
本地文件与 ClawHub 已知的任何版本都不匹配。选择其中一种方式：
...
- 如果该包被隐藏、保留或阻止，在所有者解决问题之前，它可能无法安装。
