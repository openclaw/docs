...
read_when:
    ...
    - 软件包无法安装、发布或更新
summary: ClawHub 登录、安装、发布、同步、更新和 API 问题故障排除。
x-i18n:
    generated_at: "2026-05-12T00:57:58Z"
    model: gpt-5.5
    provider: openai
...
如果多个用户共享同一个出口 IP，即使每个人只发送少量请求，也可能触发匿名 IP 限制。尽可能登录，并在报告的延迟后重试。
...
## 发布失败并提示 GitHub 所有者或来源错误

ClawHub 使用 GitHub 身份和来源归属来将包与其发布者关联。
...
- 不要在公共 API 表面之外镜像隐藏、私有、暂扣或因审核被阻止的内容。
