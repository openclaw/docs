...
- `RateLimit-Remaining`：当存在该标头时，你的确切剩余额度。在 `429` 上，它是 `0`。
...
ClawHub 使用 GitHub 身份和来源归属将包连接到其
发布者。
...
如果发布因所有者 handle、组织命名空间、包 scope、技能
slug 或包名已被认领或保留而失败，请先确认你正使用与该命名空间匹配的所有者发布。对于插件包，
像 `@example-org/example-plugin` 这样的 scoped 名称必须以匹配的
`example-org` 所有者发布。

如果你认为你的组织、项目或品牌是合法的命名空间所有者，但
无法管理当前的 ClawHub 所有者，请提交一个
[组织 / 命名空间认领议题](https://github.com/openclaw/clawhub/issues/new?template=org-namespace-claim.yml)，
并附上公开的非敏感证明。请参阅
[组织和命名空间认领](/clawhub/namespace-claims)，了解证据指南以及
哪些内容不应放入公开议题。
...
- 不要在公共 API 表面之外镜像隐藏、私有、被拦截或被审核封禁的内容。
