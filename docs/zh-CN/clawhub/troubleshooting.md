...
- `RateLimit-Remaining` 和 `RateLimit-Limit`：你的当前额度。
...
对于技能，请检查 `SKILL.md` frontmatter。应声明所需的环境变量和工具，以便用户和扫描器理解该包。
...
ClawHub 使用 GitHub 身份和来源归因来将包连接到它们的发布者。

- 确保你已使用拥有该包或有权发布该包的 GitHub 账户登录。
...
- 如果包被隐藏、暂扣或阻止，在所有者解决问题之前，它可能无法安装。
