...
x-i18n:
    generated_at: "2026-07-02T07:55:15Z"
    model: gpt-5.5
...
- 在无头环境中，在 ClawHub 网页界面中创建一个 API 令牌并运行：
...
- `RateLimit-Remaining`：当此标头存在时，你的准确剩余额度。在 `429` 时，它为 `0`。
...
如果发布失败的原因是所有者 handle、组织命名空间、包作用域、技能 slug 或包名称已被认领或保留，请先确认你正使用与该命名空间匹配的所有者进行发布。对于插件包，像 `@example-org/example-plugin` 这样的作用域名称必须以匹配的 `example-org` 所有者发布。

如果你认为你的组织、项目或品牌是该命名空间的合法所有者，但无法管理当前的 ClawHub 所有者，请提交一个包含公开且非敏感证明的[组织 / 命名空间认领议题](https://github.com/openclaw/clawhub/issues/new?template=org-namespace-claim.yml)。有关证据指南以及哪些内容不应放入公开议题，请参阅[组织和命名空间认领](/clawhub/namespace-claims)。
...
- 如果该包被隐藏、暂扣或阻止，在所有者解决问题之前，它可能无法安装。
