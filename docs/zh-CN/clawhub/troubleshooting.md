...
ClawHub 使用 GitHub 身份和源归属将包与其发布者关联起来。

- 确保你已使用拥有该包或有权发布该包的 GitHub 账号登录。
- 检查源 URL 是公开的，或可供 ClawHub 访问。
- 对于 GitHub 源，请使用 `owner/repo`、`owner/repo@ref` 或完整 GitHub URL。

## 发布失败，因为命名空间已被认领或保留

如果发布失败是因为所有者 handle、组织命名空间、包作用域、技能 slug 或包名称已被认领或保留，请先确认你正在使用与该命名空间匹配的所有者发布。对于插件包，像 `@example-org/example-plugin` 这样的作用域名称必须以匹配的 `example-org` 所有者发布。

如果你认为你的组织、项目或品牌是合法的命名空间所有者，但你无法管理当前的 ClawHub 所有者，请打开一个
[组织 / 命名空间认领 issue](https://github.com/openclaw/clawhub/issues/new?template=org-namespace-claim.yml)
并提供公开、非敏感的证明。请参阅
[组织和命名空间认领](/clawhub/namespace-claims)，了解证据指南以及哪些内容不应放入公开 issue。
...
- 如果该包被隐藏、暂挂或阻止，在所有者解决问题之前，它可能无法安装。
