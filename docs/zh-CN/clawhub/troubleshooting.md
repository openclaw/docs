...
x-i18n:
    generated_at: "2026-07-05T04:43:19Z"
    model: gpt-5.5
...
- `Retry-After`：重试前要等待的秒数。
- `RateLimit-Limit`：应用到此请求的限制。
- `RateLimit-Remaining`：存在该标头时，你的精确剩余额度。在 `429` 上，它是 `0`。
...
如果发布失败是因为所有者 handle、组织命名空间、包作用域、技能 slug 或包名称已被认领或保留，请先确认你是使用与该命名空间匹配的所有者进行发布。对于插件包，诸如 `@example-org/example-plugin` 的带作用域名称必须以匹配的 `example-org` 所有者发布。

如果你认为你的组织、项目或品牌是该命名空间的合法所有者，但无法管理当前 ClawHub 所有者，请打开一个[组织 / 命名空间认领 issue](https://github.com/openclaw/clawhub/issues/new?template=org-namespace-claim.yml)，并提供公开、非敏感的证明。请参阅[组织和命名空间认领](/clawhub/namespace-claims)，了解证据指南以及哪些内容不应放入公开 issue。
...
- 如果该包被隐藏、搁置或阻止，可能需要所有者解决问题后才能安装。

## 公共 API 请求失败

- 遵守 `429` 重试标头，并缓存公共列表 / 搜索响应。
- 将用户链接回规范的 ClawHub 条目。
- 不要在公共 API 表面之外镜像隐藏、私有、搁置或因审核被阻止的内容。
