...
ClawHub 使用 GitHub 身份和来源归属将软件包连接到其
发布者。

- 确保你已使用拥有该软件包或可以发布该软件包的 GitHub 账户登录。
- 检查来源 URL 是否公开，或 ClawHub 是否可访问。
- 对于 GitHub 来源，请使用 `owner/repo`、`owner/repo@ref` 或完整的 GitHub URL。

## 发布失败，因为命名空间已被认领或保留

如果发布失败是因为所有者句柄、组织命名空间、软件包作用域、技能
slug 或软件包名称已被认领或保留，请先确认你正在使用与该命名空间匹配的所有者进行
发布。对于插件软件包，诸如 `@example-org/example-plugin` 这样的
作用域名称必须以匹配的 `example-org` 所有者身份发布。
...
并提供公开、非敏感的证明。请参阅
[组织和命名空间认领](/zh-CN/clawhub/namespace-claims)，了解证据指南以及
哪些内容不应放入公开问题。
...
- 确认你的 OpenClaw 版本满足软件包声明的
  兼容性范围。
- 如果软件包处于隐藏、暂缓或阻止状态，在所有者解决问题之前可能无法
  安装。
