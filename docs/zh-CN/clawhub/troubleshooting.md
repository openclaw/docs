...
x-i18n:
    generated_at: "2026-07-03T17:13:11Z"
    model: gpt-5.5
...
- `RateLimit-Remaining`：当该标头存在时，表示你的精确剩余额度。在 `429` 时，它是 `0`。
- `RateLimit-Reset` 或 `X-RateLimit-Reset`：重置时间。

如果许多用户共享一个出口 IP，即使每个人只发送少量请求，也可能触发匿名 IP 限制。尽可能登录，并在报告的延迟之后重试。
...
## 发布失败，并出现 GitHub 所有者或来源错误

ClawHub 使用 GitHub 身份和来源归因来将包连接到其发布者。

- 确保你使用拥有该包或可以发布该包的 GitHub 账号登录。
- 检查来源 URL 是公开的，或可供 ClawHub 访问。
- 对于 GitHub 来源，请使用 `owner/repo`、`owner/repo@ref` 或完整的 GitHub URL。

## 发布失败，因为命名空间已被声明或保留

如果发布失败是因为所有者 handle、组织命名空间、包 scope、技能 slug 或包名已被声明或保留，请先确认你正在使用与该命名空间匹配的所有者进行发布。对于插件包，像 `@example-org/example-plugin` 这样的带 scope 名称必须以匹配的 `example-org` 所有者发布。

如果你认为你的组织、项目或品牌是该命名空间的合法所有者，但你无法管理当前的 ClawHub 所有者，请打开一个
[组织 / 命名空间声明 issue](https://github.com/openclaw/clawhub/issues/new?template=org-namespace-claim.yml)
并提供公开、非敏感的证明。有关证据指南以及哪些内容不应放入公开 issue，请参见
[组织和命名空间声明](/clawhub/namespace-claims)。
...
- 如果该包被隐藏、暂挂或阻止，可能在所有者解决问题之前无法安装。

## 公共 API 请求失败

- 遵守 `429` 重试标头，并缓存公共列表/搜索响应。
- 将用户链接回规范的 ClawHub 列表。
- 不要在公共 API 表面之外镜像隐藏、私有、暂挂或被审核阻止的内容。

有关端点详情，请参见 [HTTP API](/clawhub/http-api)。
