...
x-i18n:
    generated_at: "2026-07-03T00:51:58Z"
    model: gpt-5.5
...
如果多个用户共享一个出口 IP，即使每个人只发送少量请求，也可能触发匿名 IP 限制。尽可能登录，并在报告的延迟之后重试。
...
- 确保你已使用拥有该包或可以发布该包的 GitHub 账号登录。
- 检查来源 URL 是公开的，或 ClawHub 可以访问。
- 对于 GitHub 来源，请使用 `owner/repo`、`owner/repo@ref` 或完整的 GitHub URL。

## 发布失败，因为命名空间已被声明或保留

如果发布失败的原因是所有者 handle、组织命名空间、包 scope、skill slug 或包名已被声明或保留，请先确认你正在使用与该命名空间匹配的所有者发布。对于插件包，诸如 `@example-org/example-plugin` 的带 scope 名称必须以匹配的 `example-org` 所有者发布。

如果你认为你的组织、项目或品牌是该命名空间的合法所有者，但你无法管理当前 ClawHub 所有者，请提交一个包含公开、非敏感证明的 [组织 / 命名空间声明 issue](https://github.com/openclaw/clawhub/issues/new?template=org-namespace-claim.yml)。有关证据指南以及哪些内容不要放入公开 issue，请参阅[组织和命名空间声明](/clawhub/namespace-claims)。
...
- 如果该包被隐藏、拦截或阻止，在所有者解决问题之前，它可能无法安装。
