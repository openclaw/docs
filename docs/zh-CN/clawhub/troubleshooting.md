...
x-i18n:
    generated_at: "2026-07-06T10:48:33Z"
    model: gpt-5.5
...
- 如果一直收不到回调，请检查本地防火墙、VPN 和代理规则。
- 在无头环境中，请在 ClawHub Web UI 中创建 API token，然后运行：
...
- `RateLimit-Remaining`：存在该标头时，你的确切剩余额度。在 `429` 时，它为 `0`。
...
如果发布因所有者 handle、组织命名空间、包 scope、Skills slug 或包名已被认领或保留而失败，请先确认你是使用与该命名空间匹配的所有者在发布。对于插件包，像 `@example-org/example-plugin` 这样的 scoped 名称必须以匹配的 `example-org` 所有者发布。

如果你认为你的组织、项目或品牌是该命名空间的合法所有者，但你无法管理当前 ClawHub 所有者，请使用公开、非敏感的证明打开一个 [Org / Namespace Claim issue](https://github.com/openclaw/clawhub/issues/new?template=org-namespace-claim.yml)。有关证据指南以及哪些内容不应放入公开 issue，请参阅 [组织和命名空间声明](/clawhub/namespace-claims)。
...
- 如果该包被隐藏、拦截或阻止，在所有者解决问题之前可能无法安装。

## 公共 API 请求失败

- 遵守 `429` 重试标头，并缓存公共列表/搜索响应。
- 将用户链接回规范的 ClawHub 列表页。
- 不要在公共 API 表面之外镜像隐藏、私有、被拦截或因审核而被阻止的内容。
