...
- `RateLimit-Remaining`：当响应头存在时，你的精确剩余额度。在 `429` 时，它是 `0`。
...
如果发布失败是因为所有者 handle、组织命名空间、包 scope、技能 slug 或包名已被认领或保留，请先确认你正在使用与该命名空间匹配的所有者进行发布。对于插件包，诸如 `@example-org/example-plugin` 的 scoped 名称必须以匹配的 `example-org` 所有者发布。

如果你认为你的组织、项目或品牌是该命名空间的合法所有者，但你无法管理当前的 ClawHub 所有者，请提交一个
[组织 / 命名空间认领 issue](https://github.com/openclaw/clawhub/issues/new?template=org-namespace-claim.yml)
并提供公开、非敏感的证明。请参阅
[组织和命名空间认领](/zh-CN/clawhub/namespace-claims)，了解证据指南以及哪些内容不应放入公开 issue。
...
- 如果包被隐藏、暂留或阻止，在所有者解决问题之前，它可能无法安装。

## 公共 API 请求失败

- 遵守 `429` 重试响应头，并缓存公共列表/搜索响应。
- 将用户链接回规范的 ClawHub 列表。
- 不要在公共 API surface 之外镜像隐藏、私有、暂留或被审核阻止的内容。
