...
- `RateLimit-Remaining`：当存在该标头时，你的精确剩余额度。在 `429` 时，它是 `0`。
...
对于插件，请检查 `package.json` 兼容性元数据。code-plugin 发布需要
...
如果发布失败是因为所有者 handle、组织命名空间、包作用域、技能
slug 或包名称已被声明或保留，请先确认你正在使用与该命名空间匹配的所有者发布。对于插件包，
诸如 `@example-org/example-plugin` 的带作用域名称必须由匹配的
`example-org` 所有者发布。

如果你认为你的组织、项目或品牌是该命名空间的合法所有者，但
你无法管理当前 ClawHub 所有者，请提交
[组织 / 命名空间声明问题](https://github.com/openclaw/clawhub/issues/new?template=org-namespace-claim.yml)
并提供公开、非敏感证明。参阅
[组织和命名空间声明](/clawhub/namespace-claims)，了解证据指南以及哪些内容
不应放入公开 issue。
...
参阅 [HTTP API](/clawhub/http-api) 了解端点详情。
