...
x-i18n:
    generated_at: "2026-06-30T13:46:39Z"
    model: gpt-5.5
...
- `RateLimit-Remaining`：当该标头存在时，你确切的剩余配额。在 `429` 上，它是 `0`。
...
对于技能，请检查 `SKILL.md` frontmatter。必需的环境变量和工具应当声明，以便用户和扫描器理解该包。
...
如果发布失败是因为所有者 handle、组织命名空间、包 scope、技能 slug 或包名称已被声明或保留，请先确认你正在使用与命名空间匹配的所有者发布。对于插件包，像 `@example-org/example-plugin` 这样的 scoped 名称必须以匹配的 `example-org` 所有者发布。

如果你认为你的组织、项目或品牌是该命名空间的合法所有者，但你无法管理当前 ClawHub 所有者，请提交一个 [组织 / 命名空间声明 issue](https://github.com/openclaw/clawhub/issues/new?template=org-namespace-claim.yml)，并提供公开、非敏感的证明。请参阅 [组织和命名空间声明](/clawhub/namespace-claims)，了解证据指南以及哪些内容不应放入公开 issue。
...
- 如果包被隐藏、暂挂或阻止，在所有者解决问题之前，它可能无法安装。
