...
x-i18n:
    generated_at: "2026-07-01T10:57:02Z"
    model: gpt-5.5
...
- `RateLimit-Remaining`：当该标头存在时，你的准确剩余配额。在 `429` 时，它为 `0`。
...
ClawHub 使用 GitHub 身份和源归属来将包与其发布者关联起来。

- 确保你已使用拥有该包或可发布该包的 GitHub 账户登录。
- 检查源 URL 是否公开，或 ClawHub 是否可访问。
- 对于 GitHub 源，使用 `owner/repo`、`owner/repo@ref` 或完整的 GitHub URL。

## 发布因命名空间已被声明或保留而失败

如果发布因所有者句柄、组织命名空间、包作用域、技能 slug 或包名已被声明或保留而失败，请先确认你正在使用与该命名空间匹配的所有者发布。对于插件包，诸如 `@example-org/example-plugin` 这样的作用域名称必须以匹配的 `example-org` 所有者身份发布。

如果你认为你的组织、项目或品牌是该命名空间的合法所有者，但无法管理当前 ClawHub 所有者，请使用公开、非敏感的证明提交一个 [Org / Namespace Claim issue](https://github.com/openclaw/clawhub/issues/new?template=org-namespace-claim.yml)。有关证据指导以及哪些内容不应放入公开 issue，请参阅 [Org and Namespace Claims](/clawhub/namespace-claims)。
...
- 如果该包处于隐藏、暂挂或阻止状态，则可能在所有者解决问题之前无法安装。
