...
x-i18n:
    generated_at: "2026-06-28T05:05:45Z"
    model: gpt-5.5
...
- `RateLimit-Remaining`：当该标头存在时，你的精确剩余额度。在 `429` 上，它为 `0`。
...
ClawHub 使用 GitHub 身份和来源归属来将包与其发布者关联起来。
...
如果发布失败是因为所有者 handle、组织命名空间、包 scope、技能 slug 或包名称已被认领或保留，请先确认你正在使用与该命名空间匹配的所有者进行发布。对于插件包，诸如 `@example-org/example-plugin` 这样的 scoped 名称必须以匹配的 `example-org` 所有者发布。

如果你认为你的组织、项目或品牌是该命名空间的合法所有者，但无法管理当前的 ClawHub 所有者，请提交一个包含公开、非敏感证明的 [Org / Namespace Claim issue](https://github.com/openclaw/clawhub/issues/new?template=org-namespace-claim.yml)。请参阅 [Org and Namespace Claims](/zh-CN/clawhub/namespace-claims)，了解证据指南以及哪些内容不应放入公开 issue。
...
- 如果该包被隐藏、暂扣或阻止，在所有者解决问题之前可能无法安装。
