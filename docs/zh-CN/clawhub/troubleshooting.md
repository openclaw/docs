...
x-i18n:
    generated_at: "2026-07-05T04:56:37Z"
    model: gpt-5.5
...
- 在无头环境中，在 ClawHub 网页界面中创建一个 API 令牌，然后运行：
...
- 确保你已使用拥有或可发布该软件包的 GitHub 账户登录。
- 检查来源 URL 是公开的，或可供 ClawHub 访问。
- 对于 GitHub 来源，请使用 `owner/repo`、`owner/repo@ref` 或完整的 GitHub URL。

## 发布失败，因为命名空间已被认领或保留

如果发布失败是因为所有者 handle、组织命名空间、软件包作用域、技能 slug 或软件包名称已被认领或保留，请先确认你正在使用与该命名空间匹配的所有者进行发布。对于插件软件包，像 `@example-org/example-plugin` 这样的作用域名称必须以匹配的 `example-org` 所有者身份发布。

如果你认为你的组织、项目或品牌是该命名空间的合法所有者，但你无法管理当前的 ClawHub 所有者，请使用公开、非敏感的证明提交一个[组织 / 命名空间认领议题](https://github.com/openclaw/clawhub/issues/new?template=org-namespace-claim.yml)。有关证据指南以及不应放入公开议题的内容，请参阅[组织和命名空间认领](/clawhub/namespace-claims)。
...
- 检查软件包详情页上的扫描状态和兼容性元数据。
- 确认你的 OpenClaw 版本满足该软件包声明的兼容性范围。
- 如果该软件包被隐藏、暂挂或阻止，在所有者解决问题之前，它可能无法安装。
