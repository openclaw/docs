...
x-i18n:
    generated_at: "2026-07-03T13:16:09Z"
    model: gpt-5.5
...
如果发布失败是因为所有者 handle、组织命名空间、包 scope、技能 slug 或包名已被认领或保留，请先确认你正在使用与该命名空间匹配的所有者发布。对于插件包，诸如 `@example-org/example-plugin` 这样的 scoped 名称必须以匹配的 `example-org` 所有者身份发布。

如果你认为你的组织、项目或品牌是该命名空间的合法所有者，但你无法管理当前的 ClawHub 所有者，请提交一个 [组织 / 命名空间认领 issue](https://github.com/openclaw/clawhub/issues/new?template=org-namespace-claim.yml)，并提供公开、非敏感的证明。有关证据指南以及哪些内容不应放入公开 issue，请参阅[组织和命名空间认领](/clawhub/namespace-claims)。
...
- 如果该包处于隐藏、暂挂或阻止状态，则在所有者解决问题之前可能无法安装。
