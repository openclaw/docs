...
x-i18n:
    generated_at: "2026-07-02T17:33:33Z"
    model: gpt-5.5
...
ClawHub 使用 GitHub 身份和源归属来将包关联到其发布者。

- 确保你使用拥有或可以发布该包的 GitHub 账号登录。
- 检查源 URL 是公开的，或 ClawHub 可访问。
- 对于 GitHub 源，请使用 `owner/repo`、`owner/repo@ref` 或完整的 GitHub URL。

## 发布失败，因为命名空间已被认领或保留

如果发布失败是因为所有者 handle、组织命名空间、包 scope、技能 slug 或包名称已被认领或保留，请先确认你正在使用与该命名空间匹配的所有者发布。对于插件包，像 `@example-org/example-plugin` 这样的 scoped 名称必须以匹配的 `example-org` 所有者发布。

如果你认为自己的组织、项目或品牌是该命名空间的合法所有者，但无法管理当前的 ClawHub 所有者，请提交一个包含公开、非敏感证明的 [组织 / 命名空间认领 issue](https://github.com/openclaw/clawhub/issues/new?template=org-namespace-claim.yml)。请参阅[组织和命名空间认领](/clawhub/namespace-claims)，了解证据指南以及哪些内容不应放入公开 issue。
...
- 如果该包被隐藏、挂起或阻止，它可能需要等所有者解决问题后才能安装。
