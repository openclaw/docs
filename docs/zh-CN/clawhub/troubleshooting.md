...
read_when:
    ...
    - 软件包无法安装、发布或更新
summary: 故障排除 ClawHub 登录、安装、发布、更新和 API 问题。
x-i18n:
    generated_at: "2026-07-02T22:22:50Z"
    model: gpt-5.5
...
- `RateLimit-Remaining`：存在该标头时，你的准确剩余配额。在 `429` 上，它是 `0`。
...
## 发布失败并出现 GitHub 所有者或来源错误

ClawHub 使用 GitHub 身份和来源归属将包连接到其发布者。

- 确保你使用拥有该包或可以发布该包的 GitHub 账号登录。
- 检查源 URL 是否公开，或 ClawHub 是否可以访问。
- 对于 GitHub 来源，请使用 `owner/repo`、`owner/repo@ref` 或完整的 GitHub URL。

## 发布失败，因为命名空间已被声明或保留

如果发布失败是因为所有者 handle、组织命名空间、包 scope、技能 slug 或包名已被声明或保留，请先确认你正在使用与命名空间匹配的所有者发布。对于插件包，像 `@example-org/example-plugin` 这样的 scoped 名称必须以匹配的 `example-org` 所有者发布。

如果你认为你的组织、项目或品牌是该命名空间的合法所有者，但你无法管理当前 ClawHub 所有者，请使用公开的非敏感证明打开一个 [Org / Namespace Claim issue](https://github.com/openclaw/clawhub/issues/new?template=org-namespace-claim.yml)。有关证据指导以及哪些内容不应放入公开 issue，请参阅 [Org and Namespace Claims](/clawhub/namespace-claims)。
...
- 检查包详情页面，查看扫描状态和兼容性元数据。
- 确认你的 OpenClaw 版本满足该包声明的兼容性范围。
- 如果包被隐藏、拦截或阻止，可能要等所有者解决问题后才能安装。
