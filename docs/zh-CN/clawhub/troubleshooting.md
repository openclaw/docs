...
- `RateLimit-Remaining`：存在此标头时，你的精确剩余额度。在 `429` 上，它为 `0`。
...
如果发布失败是因为所有者句柄、组织命名空间、包作用域、技能 slug 或包名称已被认领或保留，请先确认你正在使用与该命名空间匹配的所有者进行发布。对于插件包，诸如 `@example-org/example-plugin` 这样的作用域名称必须以匹配的 `example-org` 所有者身份发布。

如果你认为你的组织、项目或品牌是该命名空间的合法所有者，但你无法管理当前的 ClawHub 所有者，请使用公开的非敏感证明提交一个 [组织 / 命名空间认领议题](https://github.com/openclaw/clawhub/issues/new?template=org-namespace-claim.yml)。有关证据指导以及哪些内容不应放入公开议题，请参阅[组织和命名空间认领](/clawhub/namespace-claims)。
