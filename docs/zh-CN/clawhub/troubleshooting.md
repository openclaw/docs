...
- `RateLimit-Remaining`：当存在此标头时，你的精确剩余额度。在 `429` 时，它为 `0`。
- `RateLimit-Reset` 或 `X-RateLimit-Reset`：重置时间。

如果许多用户共用一个出口 IP，即使每个人只发送少量请求，也可能触发匿名 IP 限制。尽可能登录，并在报告的延迟后重试。
...
- 确保你已使用拥有或可以发布该包的 GitHub 账户登录。
- 检查来源 URL 是公开的，或 ClawHub 可访问。
- 对于 GitHub 来源，请使用 `owner/repo`、`owner/repo@ref` 或完整的 GitHub URL。

## 发布失败，因为命名空间已被认领或保留

如果发布失败是因为所有者 handle、组织命名空间、包作用域、技能 slug 或包名称已被认领或保留，请先确认你是使用与该命名空间匹配的所有者进行发布。对于插件包，诸如 `@example-org/example-plugin` 这样的 scoped 名称必须以匹配的 `example-org` 所有者发布。

如果你认为你的组织、项目或品牌是该命名空间的合法所有者，但无法管理当前的 ClawHub 所有者，请提交一个 [Org / Namespace Claim issue](https://github.com/openclaw/clawhub/issues/new?template=org-namespace-claim.yml)，并提供公开、非敏感的证明。请参阅 [组织和命名空间认领](/clawhub/namespace-claims)，了解证据指南以及哪些内容不应放入公开 issue。
...
- 如果该包被隐藏、拦截或阻止，在所有者解决问题之前，可能无法安装。
