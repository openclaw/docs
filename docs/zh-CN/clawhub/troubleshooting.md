...
x-i18n:
    generated_at: "2026-07-04T17:48:32Z"
    model: gpt-5.5
...
- 在无头环境中，在 ClawHub Web 界面创建 API token，然后运行：
...
- 如果你使用 API token，请确认它没有在 Web 界面中被撤销。
...
- `RateLimit-Remaining`：当此标头存在时，表示你的精确剩余额度。在 `429` 上，它是 `0`。
...
如果发布失败是因为所有者账号名、组织命名空间、包作用域、技能 slug 或包名称已被认领或保留，请先确认你正在使用与该命名空间匹配的所有者进行发布。对于插件包，像 `@example-org/example-plugin` 这样的作用域名称必须以匹配的 `example-org` 所有者发布。

如果你认为你的组织、项目或品牌是该命名空间的合法所有者，但你无法管理当前 ClawHub 所有者，请使用公开的非敏感证明打开一个[组织 / 命名空间申领议题](https://github.com/openclaw/clawhub/issues/new?template=org-namespace-claim.yml)。请参阅[组织和命名空间申领](/clawhub/namespace-claims)，了解证据指南以及哪些内容不要放入公开议题。
...
- 如果该包被隐藏、扣留或阻止，它可能在所有者解决问题之前无法安装。

## 公共 API 请求失败

- 遵循 `429` 重试标头，并缓存公共列表/搜索响应。
- 将用户链接回规范的 ClawHub 列表页。
- 不要在公共 API 表面之外镜像隐藏、私有、扣留或被审核阻止的内容。
