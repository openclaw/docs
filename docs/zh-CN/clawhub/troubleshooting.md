...
read_when:
    ...
    - 软件包无法安装、发布或更新
summary: ClawHub 登录、安装、发布、更新和 API 问题故障排除。
x-i18n:
    generated_at: "2026-07-01T05:28:08Z"
    model: gpt-5.5
...
- 在无头环境中，请在 ClawHub 网页 UI 中创建 API 令牌，然后运行：
...
- 如果你使用 API 令牌，请确认它没有在网页 UI 中被撤销。
...
- `RateLimit-Remaining`：当该标头存在时，你的确切剩余额度。在 `429` 时，它是 `0`。
- `RateLimit-Reset` 或 `X-RateLimit-Reset`：重置时间。

如果许多用户共享同一个出站 IP，即使每个人只发送少量请求，也可能触发匿名 IP 限制。尽可能登录，并在报告的延迟之后重试。
...
- 检查来源 URL 是否公开或可被 ClawHub 访问。
- 对于 GitHub 来源，请使用 `owner/repo`、`owner/repo@ref` 或完整的 GitHub URL。

## 发布因命名空间被认领或保留而失败

如果发布失败是因为所有者 handle、组织命名空间、包 scope、技能 slug 或包名称已被认领或保留，请先确认你是使用与该命名空间匹配的所有者进行发布。对于插件包，`@example-org/example-plugin` 这样的 scoped 名称必须作为匹配的 `example-org` 所有者发布。

如果你认为你的组织、项目或品牌是该命名空间的合法所有者，但无法管理当前 ClawHub 所有者，请提交一个 [Org / Namespace Claim issue](https://github.com/openclaw/clawhub/issues/new?template=org-namespace-claim.yml)，并提供公开、非敏感的证明。有关证据指南以及哪些内容不应放入公开 issue，请参阅 [Org and Namespace Claims](/clawhub/namespace-claims)。
...
本地文件与 ClawHub 已知的任何版本都不匹配。选择一种处理方式：
...
- 检查包详情页中的扫描状态和兼容性元数据。
- 确认你的 OpenClaw 版本满足该包声明的兼容范围。
- 如果该包被隐藏、挂起或阻止，在所有者解决问题之前可能无法安装。

## 公共 API 请求失败

- 遵守 `429` 重试标头，并缓存公共列表/搜索响应。
- 将用户链接回规范 ClawHub 列表页。
