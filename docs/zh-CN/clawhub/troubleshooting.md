...
x-i18n:
    generated_at: "2026-06-27T10:18:38Z"
    model: gpt-5.5
...
- `RateLimit-Remaining`：当该标头存在时，你的精确剩余额度。在 `429` 时，它为 `0`。
- `RateLimit-Reset` 或 `X-RateLimit-Reset`：重置时间。

如果多个用户共享一个出口 IP，即使每个人只发送少量请求，也可能触发匿名 IP 限制。尽可能登录，并在报告的延迟后重试。
...
ClawHub 使用 GitHub 身份和来源归属将软件包连接到其发布者。

- 确保你已使用拥有该软件包或可发布该软件包的 GitHub 账号登录。
- 检查来源 URL 是否公开或可供 ClawHub 访问。
- 对于 GitHub 来源，请使用 `owner/repo`、`owner/repo@ref` 或完整的 GitHub URL。

## 发布失败，因为命名空间已被认领或保留

如果发布因所有者 handle、组织命名空间、软件包 scope、技能 slug 或软件包名称已被认领或保留而失败，请先确认你是使用与该命名空间匹配的所有者发布。对于插件软件包，诸如 `@example-org/example-plugin` 的 scoped 名称必须以匹配的 `example-org` 所有者发布。

如果你认为你的组织、项目或品牌是该命名空间的合法所有者，但无法管理当前的 ClawHub 所有者，请打开一个 [组织 / 命名空间认领议题](https://github.com/openclaw/clawhub/issues/new?template=org-namespace-claim.yml)，并提供公开、非敏感的证明。请参阅[组织和命名空间认领](/zh-CN/clawhub/namespace-claims)，了解证据指南以及哪些内容不应放入公开议题。
...
- 检查软件包详情页面，查看扫描状态和兼容性元数据。
- 确认你的 OpenClaw 版本满足该软件包声明的兼容性范围。
- 如果软件包被隐藏、暂挂或阻止，在所有者解决问题之前可能无法安装。

## 公共 API 请求失败

- 遵守 `429` 重试标头，并缓存公共列表 / 搜索响应。
- 将用户链接回规范的 ClawHub 列表页。
- 不要在公共 API surface 之外镜像隐藏、私有、暂挂或因审核被阻止的内容。

请参阅 [HTTP API](/zh-CN/clawhub/http-api) 了解 endpoint 详情。
