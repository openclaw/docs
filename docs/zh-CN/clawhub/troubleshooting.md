...
- 确保你已使用拥有或可以发布该包的 GitHub 账号登录。
- 检查来源 URL 是公开的，或 ClawHub 可以访问。
- 对于 GitHub 来源，请使用 `owner/repo`、`owner/repo@ref` 或完整的 GitHub URL。

## 发布失败，因为命名空间已被声明或保留

如果发布失败是因为所有者 handle、组织命名空间、包 scope、技能 slug 或包名称已被声明或保留，请先确认你正在使用与该命名空间匹配的所有者发布。对于插件包，像 `@example-org/example-plugin` 这样的带 scope 名称必须以匹配的 `example-org` 所有者发布。

如果你认为你的组织、项目或品牌是该命名空间的正当所有者，但你无法管理当前的 ClawHub 所有者，请使用公开且非敏感的证明提交 [Org / Namespace Claim issue](https://github.com/openclaw/clawhub/issues/new?template=org-namespace-claim.yml)。请参阅 [Org and Namespace Claims](/clawhub/namespace-claims)，了解证据指南以及哪些内容不应放入公开 issue。
...
- 不要在公共 API surface 之外镜像隐藏、私有、被拦截或因审核被阻止的内容。
