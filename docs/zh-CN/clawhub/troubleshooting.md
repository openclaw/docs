...
x-i18n:
    generated_at: "2026-07-03T23:26:12Z"
    model: gpt-5.5
...
- 如果你使用自定义配置路径，请确认 `CLAWHUB_CONFIG_PATH` 指向包含当前令牌的
  文件。
...
如果许多用户共享同一个出站 IP，即使每个人只发送少量请求，也可能触发匿名 IP
限制。尽可能登录，并在报告的延迟后重试。
...
对于插件，请检查 `package.json` 兼容性元数据。code-plugin 发布
...
ClawHub 使用 GitHub 身份和来源归属将包与其
发布者关联起来。

- 确保你使用拥有该包或可发布该包的 GitHub 账户登录。
- 检查来源 URL 是否公开或 ClawHub 可访问。
- 对于 GitHub 来源，请使用 `owner/repo`、`owner/repo@ref` 或完整的 GitHub URL。

## 发布因命名空间已被声明或保留而失败

如果发布失败是因为所有者 handle、组织命名空间、包 scope、技能
slug 或包名称已被声明或保留，请先确认你正在使用与该命名空间匹配的所有者
发布。对于插件包，`@example-org/example-plugin` 这样的 scoped 名称必须由
匹配的 `example-org` 所有者发布。

如果你认为你的组织、项目或品牌是该命名空间的合法所有者，但
你无法管理当前的 ClawHub 所有者，请提交
[组织 / 命名空间声明 issue](https://github.com/openclaw/clawhub/issues/new?template=org-namespace-claim.yml)，
并附上公开、非敏感的证明。有关证据指南以及哪些内容
不应放入公开 issue，请参阅
...
本地文件与 ClawHub 已知的任何版本都不匹配。选择其一：
