...
x-i18n:
    generated_at: "2026-07-05T01:55:19Z"
    model: gpt-5.5
...
- `RateLimit-Remaining`：存在该标头时，你的准确剩余额度。在 `429` 时，它为 `0`。
...
如果发布失败是因为所有者 handle、组织命名空间、包 scope、技能 slug 或包名称已被认领或保留，请先确认你正在使用与该命名空间匹配的所有者进行发布。对于插件包，像 `@example-org/example-plugin` 这样的 scoped 名称必须以匹配的 `example-org` 所有者发布。

如果你认为你的组织、项目或品牌是该命名空间的正当所有者，但无法管理当前的 ClawHub 所有者，请提交一个 [组织 / 命名空间认领 issue](https://github.com/openclaw/clawhub/issues/new?template=org-namespace-claim.yml)，并附上公开的、非敏感的证明。请参阅[组织和命名空间认领](/clawhub/namespace-claims)，了解证据指南以及哪些内容不应放入公开 issue。
