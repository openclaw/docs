...
对于 ClawHub 本身的漏洞，请使用 GitHub Security Advisories。良好的 ClawHub 安全公告报告包括以下方面的 bug：
...
真实用户影响的示例包括确认被利用、用户数据或密钥暴露、因平台故障导致恶意内容触达用户，或任何需要用户轮换凭据、更新本地软件或采取其他防护措施的问题。

用户安装的软件中的漏洞会公开披露，例如用户需要在本地更新的 ClawHub CLI 包、二进制文件、库或其他发布制品。

## 相关页面

关于安装时审计标签、风险等级、发现项和解读，请参阅[安全审计](/clawhub/security-audits)。

关于市场报告、审核暂挂、隐藏列表、封禁和账户状态，请参阅[审核与账户安全](/clawhub/moderation)。
