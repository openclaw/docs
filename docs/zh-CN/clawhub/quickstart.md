...
x-i18n:
    generated_at: "2026-07-12T14:21:50Z"
    model: gpt-5.6
...
OpenClaw 会记录 Skill 的来源，以便后续更新能够继续通过 ClawHub 解析。
...
Skill 是一个包含必需的 `SKILL.md` 文件以及可选支持文件的文件夹。
...
该命令会跳过未更改的内容。新 Skill 的初始版本为 `1.0.0`；后续更改会自动发布下一个补丁版本。使用 `--dry-run` 进行预览，或使用 `--version` 选择明确的版本。

发布前，请检查 `SKILL.md` 中的元数据。声明所需的环境变量、工具和权限，以便用户在安装 Skill 前了解其需求。请参阅 [Skill 格式](/zh-CN/clawhub/skill-format)。
...
公开条目会显示最新的扫描状态。因审核而被暂缓或阻止的版本，在问题解决前可能不会显示在搜索和安装界面中。
