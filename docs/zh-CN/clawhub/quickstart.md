...
x-i18n:
    generated_at: "2026-06-30T13:46:43Z"
    model: gpt-5.5
...
当你要把内容安装到 OpenClaw 中时，请使用 OpenClaw。当你要登录、发布、管理自己的上架条目，或使用注册表专用工作流时，请使用 `clawhub` CLI。
...
技能是一个文件夹，其中必须包含 `SKILL.md` 文件，并可选包含支持文件。
...
请先使用 `--dry-run` 预览已解析的包元数据、兼容性字段、来源归属和上传计划，而不会实际发布。
...
公开上架条目会显示最新扫描状态。因审核而被暂挂或阻止的发布版本可能会从搜索和安装界面中隐藏，直到问题解决。
