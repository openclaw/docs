...
当你要把内容安装到 OpenClaw 中时，请使用 OpenClaw。当你要登录、发布、管理自己的列表，或使用注册表特定的工作流时，请使用 `clawhub` CLI。
...
登录后，`sync` 也可能发送一个最小安装快照，用于汇总安装次数。关于报告的内容以及如何选择退出，请参见[遥测](/zh-CN/clawhub/telemetry)。
