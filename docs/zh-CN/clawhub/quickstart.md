...
x-i18n:
    generated_at: "2026-07-06T10:47:02Z"
    model: gpt-5.5
...
OpenClaw 会记录技能来源，因此后续更新可以继续通过 ClawHub 解析。
...
发布前，请检查 `SKILL.md` 中的元数据。声明必需的环境变量、工具和权限，让用户在安装前了解该技能需要什么。请参阅 [Skill format](/zh-CN/clawhub/skill-format)。

对于包含多个技能的代码库，可复用的 GitHub workflow 会对 `skills/` 下每个直接技能文件夹调用 `skill publish`：
...
从本地文件夹、GitHub 代码库、GitHub ref 或现有归档发布插件：
...
公开列表会显示最新的扫描状态。被审核暂缓或阻止的发布可能会在问题解决前从搜索和安装界面中隐藏。
