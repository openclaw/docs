...
x-i18n:
    generated_at: "2026-07-03T17:12:54Z"
    model: gpt-5.5
...
当你要把内容安装到 OpenClaw 中时，请使用 OpenClaw。登录、发布、管理自己的列表或使用注册表特定工作流时，请使用 `clawhub` CLI。
...
OpenClaw 会记录技能的来源，以便之后的更新可以继续通过 ClawHub 解析。
...
该命令会跳过未更改的内容。新技能从 `1.0.0` 开始；之后的更改会自动发布下一个 patch 版本。使用 `--dry-run` 进行预览，或使用 `--version` 选择明确版本。
...
公开列表会显示最新扫描状态。被审核暂缓或阻止的发布可能会从搜索和安装界面中隐藏，直到问题解决。
