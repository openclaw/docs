...
x-i18n:
    generated_at: "2026-07-02T22:21:55Z"
    model: gpt-5.5
...
将内容安装到 OpenClaw 时使用 OpenClaw。登录、发布、管理你自己的条目，或使用注册表特定工作流时，使用 `clawhub` CLI。
...
无头环境可以使用来自 ClawHub Web UI 的 API token：
...
对于包含多个技能的仓库，可复用的 GitHub workflow 会为 `skills/` 下的每个直接技能文件夹调用 `skill publish`：
...
公开条目会显示最新扫描状态。被审核暂缓或阻止的发布版本可能会在问题解决前从搜索和安装界面中隐藏。
