...
当你要把内容安装到 OpenClaw 中时，请使用 OpenClaw。当你要登录、发布、管理自己的列表或使用注册表特定工作流时，请使用 `clawhub` CLI。
...
登录后，`sync` 还可能发送一个最小安装快照，用于汇总安装计数。查看 [遥测](/zh-CN/clawhub/telemetry)，了解会上报哪些内容以及如何选择退出。
...
公开列表会显示最新扫描状态。被审核暂挂或阻止的版本在解决前可能会从搜索和安装界面中隐藏。
