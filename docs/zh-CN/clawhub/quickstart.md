...
在向 OpenClaw 安装内容时使用 OpenClaw。在登录、发布、管理你自己的列表，或使用注册表特定工作流时，使用 `clawhub` CLI。
...
发布前，请检查 `SKILL.md` 中的元数据。声明所需的环境变量、工具和权限，以便用户在安装技能前了解它需要什么。参见[技能格式](/zh-CN/clawhub/skill-format)。

对于包含多个技能的仓库，可复用的 GitHub 工作流会为 `skills/` 下的每个直属技能文件夹调用 `skill publish`：
...
公开列表会显示最新扫描状态。被审核暂缓或阻止的发布版本，在问题解决前可能会从搜索和安装界面中隐藏。
