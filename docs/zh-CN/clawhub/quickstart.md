...
read_when:
    ...
    - 从注册表安装技能或插件
    - 发布到 ClawHub
summary: 开始使用 ClawHub：查找、安装、更新并发布技能或插件。
x-i18n:
    generated_at: "2026-07-03T00:51:47Z"
    model: gpt-5.5
...
当你要把内容安装到 OpenClaw 中时，请使用 OpenClaw。当你要登录、发布、管理自己的列表，或使用注册中心专属工作流时，请使用 `clawhub` CLI。
...
技能是一个文件夹，其中包含必需的 `SKILL.md` 文件，也可以包含可选的支持文件。
...
对于包含多个技能的仓库，可复用的 GitHub 工作流会为 `skills/` 下每个直接的技能文件夹调用 `skill publish`：
...
先使用 `--dry-run` 预览解析后的包元数据、兼容性字段、来源归属和上传计划，而不执行发布。
...
公开列表会显示最新的扫描状态。被审核暂挂或阻止的发布版本可能会在问题解决前从搜索和安装界面中隐藏。
