...
x-i18n:
    generated_at: "2026-06-27T01:33:54Z"
    model: gpt-5.5
...
OpenClaw 会记录该技能的来源，以便之后的更新可以继续通过 ClawHub 解析。
...
该命令会跳过未更改的内容。新技能从 `1.0.0` 开始；之后的更改会自动发布下一个补丁版本。使用 `--dry-run` 预览，或使用 `--version` 选择显式版本。
...
公开列表会显示最新扫描状态。被审核暂缓或阻止的发布版本可能会在问题解决前从搜索和安装界面中隐藏。
