...
将内容安装到 OpenClaw 中时，请使用 OpenClaw。登录、发布、管理自己的列表，或使用注册表特定工作流时，请使用 `clawhub` CLI。
...
先使用 `--dry-run` 预览解析后的包元数据、兼容性字段、来源归因和上传计划，而不实际发布。
...
登录后，`sync` 也可能会发送最小安装快照，用于聚合安装数量。有关报告内容和如何选择退出，请参见 [遥测](/zh-CN/clawhub/telemetry)。
...
公开列表会显示最新扫描状态。被审核暂缓或阻止的发布版本，在解决前可能会从搜索和安装界面中隐藏。
