...
x-i18n:
    generated_at: "2026-07-04T20:23:57Z"
    model: gpt-5.5
...
ClawHub 是用于 OpenClaw 技能和插件的注册表。

在将内容安装到 OpenClaw 时使用 OpenClaw。在登录、发布、管理你自己的列表，或使用注册表专用工作流时使用 `clawhub` CLI。
...
该命令会跳过未变更的内容。新技能从 `1.0.0` 开始；后续更改会自动发布下一个补丁版本。使用 `--dry-run` 预览，或使用 `--version` 选择显式版本。
...
先使用 `--dry-run` 预览解析出的包元数据、兼容性字段、来源归属和上传计划，而不进行发布。
...
公开列表会显示最新扫描状态。被审核暂挂或阻止的发布版本，在问题解决前可能会从搜索和安装界面中隐藏。
