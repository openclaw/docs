...
当你要把内容安装到 OpenClaw 中时，请使用 OpenClaw。当你要登录、发布、管理自己的条目，或使用注册表专属工作流时，请使用 `clawhub` CLI。
...
发布前，请检查 `SKILL.md` 中的元数据。声明所需的环境变量、工具和权限，让用户在安装前了解该技能需要什么。请参阅 [Skill format](/zh-CN/clawhub/skill-format)。
...
请先使用 `--dry-run` 预览解析出的包元数据、兼容性字段、来源归属和上传计划，而不实际发布。
...
公开条目会显示最新扫描状态。被审核保留或阻止的发布版本可能会在问题解决前从搜索和安装界面中隐藏。
