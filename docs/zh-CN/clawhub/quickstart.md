...
当你将内容安装到 OpenClaw 中时，请使用 OpenClaw。当你登录、发布、管理自己的列表，或使用注册表特定工作流时，请使用 `clawhub` CLI。
...
安装带有显式 ClawHub 来源、由 ClawHub 托管的插件：
...
技能是一个文件夹，其中包含必需的 `SKILL.md` 文件以及可选的支持文件。
...
`sync` 会扫描技能文件夹，并发布尚未同步的新技能或已变更的技能。
...
登录后，`sync` 也可能发送最小安装快照，用于汇总安装计数。请参阅[遥测](/zh-CN/clawhub/telemetry)，了解会上报什么以及如何选择退出。
...
公开列表会显示最新扫描状态。被审核搁置或阻止的发布版本可能会从搜索和安装界面中隐藏，直到问题解决。
