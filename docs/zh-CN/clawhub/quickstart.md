...
无头环境可以使用 ClawHub Web UI 中的 API token：
...
对于包含多个技能的仓库，可复用的 GitHub workflow 会对 `skills/` 下每个直接技能文件夹调用 `skill publish`：
...
公开列表会显示最新的扫描状态。因审核而被暂挂或阻止的发布可能会从搜索和安装界面中隐藏，直到问题解决。
