...
在将内容安装到 OpenClaw 中时使用 OpenClaw。在登录、发布、管理你自己的列表，或使用注册表特定工作流时，使用 `clawhub` CLI。
...
登录后，`sync` 也可能发送一个最小安装快照，用于汇总安装计数。参见[遥测](/zh-CN/clawhub/telemetry)，了解会报告哪些内容以及如何选择退出。
