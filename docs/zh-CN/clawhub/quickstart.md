...
x-i18n:
    generated_at: "2026-07-05T04:49:06Z"
    model: gpt-5.5
...
在向 OpenClaw 安装内容时使用 OpenClaw。登录、发布、管理自己的列表或使用注册表特定工作流时，使用 `clawhub` CLI。
...
先使用 `--dry-run` 预览解析后的包元数据、兼容性字段、来源归属和上传计划，而不会实际发布。
...
公开列表会显示最新扫描状态。被审核保留或拦截的发布版本可能会从搜索和安装界面中隐藏，直到问题解决。
