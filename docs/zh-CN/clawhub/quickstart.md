...
x-i18n:
    generated_at: "2026-06-28T20:42:17Z"
    model: gpt-5.5
...
在将内容安装到 OpenClaw 中时，请使用 OpenClaw。在登录、发布、管理你自己的列表，或使用注册表特定工作流时，请使用 `clawhub` CLI。
...
发布前，请检查 `SKILL.md` 中的元数据。声明必需的环境变量、工具和权限，以便用户在安装前了解该技能需要什么。请参阅 [Skill 格式](/zh-CN/clawhub/skill-format)。
...
公开列表会显示最新扫描状态。因审核而被暂缓或阻止的发布版本，在问题解决前可能会从搜索和安装界面中隐藏。
