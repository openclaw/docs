...
将内容安装到 OpenClaw 中时，请使用 OpenClaw。当你要登录、发布、管理自己的列表，或使用注册表特定工作流时，请使用 `clawhub` CLI。
...
发布前，请检查 `SKILL.md` 中的元数据。声明必需的环境变量、工具和权限，以便用户在安装前了解该技能需要什么。参见 [Skill format](/zh-CN/clawhub/skill-format)。
...
先使用 `--dry-run`，即可在不发布的情况下预览已解析的包元数据、兼容性字段、来源归属和上传计划。
...
公开列表会显示最新扫描状态。被审核暂缓或阻止的发布版本可能会从搜索和安装界面中隐藏，直到问题解决。
