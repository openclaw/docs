...
x-i18n:
    generated_at: "2026-07-05T04:43:27Z"
    model: gpt-5.5
...
当你把内容安装到 OpenClaw 中时，请使用 OpenClaw。当你登录、发布、管理自己的条目，或使用注册表专用工作流时，请使用 `clawhub` CLI。
...
技能是一个文件夹，其中必须包含 `SKILL.md` 文件，也可以包含可选的支持文件。
...
对于包含多个技能的代码库，可复用的 GitHub 工作流会为 `skills/` 下每个直接技能文件夹调用 `skill publish`：
...
公开条目会显示最新扫描状态。因审核而被暂缓或阻止的发布版本，在解决之前可能会从搜索和安装界面中隐藏。
