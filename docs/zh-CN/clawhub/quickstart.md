...
将内容安装到 OpenClaw 时使用 OpenClaw。登录、发布、管理你自己的列表，或使用注册表特定工作流时，使用 `clawhub` CLI。
...
发布前，请检查 `SKILL.md` 中的元数据。声明必需的环境变量、工具和权限，以便用户在安装前了解技能需要什么。参见[技能格式](/zh-CN/clawhub/skill-format)。
...
安装前，使用 ClawHub 网页或 CLI 详情命令检查元数据、源链接、版本、更新日志和扫描状态：
...
公开列表会显示最新扫描状态。被审核保留或阻止的发布可能会从搜索和安装界面隐藏，直到问题解决。
