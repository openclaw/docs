...
title: 安装
x-i18n:
    generated_at: "2026-06-27T02:18:14Z"
    model: gpt-5.5
...
- **macOS、Linux 或 Windows** - Windows 用户可以从原生 Windows Hub 应用、PowerShell CLI 安装器或 WSL2 Gateway 网关开始。参见 [Windows](/zh-CN/platforms/windows)。
- 只有从源码构建时才需要 `pnpm`
...
所有标志和 CI/自动化选项请参见 [安装器内部机制](/zh-CN/install/installer)。
...
`~/.openclaw`）下，而不依赖系统级 Node 安装时使用：

```bash
curl -fsSL https://openclaw.ai/install-cli.sh | bash
```

它默认支持 npm 安装，也支持在相同前缀流程下进行 git 检出安装。完整参考：[安装器内部机制](/zh-CN/install/installer#install-clish)。

已经安装？使用 `openclaw update --channel dev` 和 `openclaw update --channel stable` 在软件包安装和 git 安装之间切换。参见
...
<Tabs>
  <Tab title="npm">
    ...
    <Note>
    托管安装器会为 OpenClaw 软件包安装清除 npm 新鲜度过滤器，例如 `min-release-age`。如果你使用 npm 手动安装，你自己的 npm 策略仍然适用。
    </Note>
  ...
  <Tab title="pnpm">
    ...
    <Note>
    pnpm 要求显式批准带有构建脚本的软件包。首次安装后运行 `pnpm approve-builds -g`。
    </Note>
  ...
  <Tab title="bun">
    ...
    <Note>
    Bun 支持全局 CLI 安装路径。对于 Gateway 网关运行时，Node 仍然是推荐的守护进程运行时。
    </Note>
...
或者跳过链接，在仓库内使用 `pnpm openclaw ...`。完整开发工作流请参见 [设置](/zh-CN/start/setup)。
...
- 原生 Windows：优先使用计划任务；如果任务创建被拒绝，则回退到每用户启动文件夹登录项
...
<CardGroup cols={3}>
  ...
  <Card title="Fly.io" href="/zh-CN/install/fly">
    在 Fly.io 上部署。
  </Card>
  <Card title="Hetzner" href="/zh-CN/install/hetzner">
    Hetzner 部署。
  </Card>
  <Card title="GCP" href="/zh-CN/install/gcp">
    Google Cloud 部署。
  </Card>
  <Card title="Azure" href="/zh-CN/install/azure">
    Azure 部署。
  </Card>
  <Card title="Railway" href="/zh-CN/install/railway">
    Railway 部署。
  </Card>
  <Card title="Render" href="/zh-CN/install/render">
    Render 部署。
  </Card>
  <Card title="Northflank" href="/zh-CN/install/northflank">
    Northflank 部署。
  </Card>
</CardGroup>
...
然后打开一个新终端。更多详情请参见 [Node 设置](/zh-CN/install/node)。
