...
read_when:
    ...
    - 你想比较本地与托管式 macOS 虚拟机选项
summary: 需要隔离或 iMessage 时，在沙箱隔离的 macOS VM（本地或托管）中运行 OpenClaw
title: macOS 虚拟机
x-i18n:
...
- 如果你想完全控制，并为浏览器自动化使用**住宅 IP**，请选择**专用硬件**（Mac mini 或 Linux 主机）。许多网站会屏蔽数据中心 IP，因此本地浏览通常效果更好。
- **混合方案**：将 Gateway 网关放在便宜的 VPS 上，并在需要浏览器/UI 自动化时将你的 Mac 连接为**节点**。参见 [节点](/zh-CN/nodes) 和 [Gateway 网关远程](/zh-CN/gateway/remote)。

只有在你明确需要 iMessage 等仅限 macOS 的能力，或希望与日常使用的 Mac 严格隔离时，才使用 macOS VM。

## macOS VM 选项

### 在你的 Apple Silicon Mac 上运行本地 VM（Lume）

使用 [Lume](https://cua.ai/docs/lume) 在现有 Apple Silicon Mac 上的沙箱隔离 macOS VM 中运行 OpenClaw。这会带来：
...
- 其他托管 Mac 厂商也可以；按照它们的 VM + SSH 文档操作

一旦你获得 macOS VM 的 SSH 访问权限，请继续下面的 [安装 OpenClaw](#6-install-openclaw)。
...
1. 启用 SSH：系统设置 -> 通用 -> 共享，启用“远程登录”。
2. 如需无头 VM 使用，请启用自动登录：系统设置 -> 用户与群组，选择“自动登录为：”，并选择 VM 用户。
...
VM 会在后台运行；OpenClaw 的守护进程会保持 Gateway 网关运行。检查状态：
...
这是在 macOS 上运行的核心优势。使用带 `imsg` 的 [iMessage](/zh-CN/channels/imessage) 将 Messages 添加到 OpenClaw。
...
重启 Gateway 网关。你的智能体现在可以发送和接收 iMessage 消息。完整设置详情：[iMessage 渠道](/zh-CN/channels/imessage)。
...
如需真正常开，请考虑使用专用 Mac mini 或小型 VPS。参见 [VPS 托管](/zh-CN/vps)。

## 故障排查

| 问题                     | 解决方案                                                                            |
| ------------------------ | ----------------------------------------------------------------------------------- |
| 无法 SSH 登录 VM         | 检查 VM 的系统设置中是否已启用“远程登录”                                            |
