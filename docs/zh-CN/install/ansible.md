...
使用 **[openclaw-ansible](https://github.com/openclaw/openclaw-ansible)** 将 OpenClaw 部署到生产服务器 -- 一个采用安全优先架构的自动化安装程序。
...
5. **OpenClaw** -- 基于主机运行，而不是容器化运行
6. **Systemd 服务** -- 自动启动并启用安全加固

<Note>
Gateway 网关直接在主机上运行（不在 Docker 中）。智能体沙箱隔离是可选的；此 playbook 会安装 Docker，因为它是默认沙箱后端。详情和其他后端见 [沙箱隔离](/zh-CN/gateway/sandboxing)。
</Note>
...
Docker 是为智能体沙箱（隔离的工具执行）而安装的，不用于运行 Gateway 网关本身。沙箱配置见 [多 Agent 沙盒和工具](/zh-CN/tools/multi-agent-sandbox-tools)。
...
Ansible 安装程序会为 OpenClaw 设置手动更新。标准更新流程见 [更新](/zh-CN/install/updating)。
