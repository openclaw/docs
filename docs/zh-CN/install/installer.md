...
title: 安装程序内部机制
x-i18n:
    generated_at: "2026-07-11T20:37:36Z"
    model: gpt-5.6
...
OpenClaw 提供三个安装脚本，均由 `openclaw.ai` 托管。
...
<Tip>
建议大多数 macOS/Linux/WSL 交互式安装使用此脚本。
</Tip>
...
<Steps>
  ...
  <Step title="默认确保安装 Node.js 24">
    检查 Node 版本，并在需要时安装 Node 24（macOS 使用 Homebrew，Linux 使用 NodeSource 的 apt/dnf/yum 设置脚本）。在 macOS 上，仅当安装程序需要使用 Homebrew 安装 Node 或 Git 时才会安装 Homebrew。为保持兼容性，仍支持 Node 22.19+ 和 23.11+。
    在 Alpine/musl Linux 上，安装程序使用 apk 软件包而非 NodeSource；配置的 Alpine 软件源必须提供受支持的 Node 版本（撰写本文时为 Alpine 3.21 或更高版本）。
  </Step>
  <Step title="确保安装 Git">
  ...
  <Step title="安装 OpenClaw">
    ...
    - `git` 方式：克隆或更新仓库，使用 pnpm 安装依赖并构建，然后在 `~/.local/bin/openclaw` 安装封装脚本

  </Step>
  <Step title="安装后任务">
    - 解析刚安装的 `openclaw` 二进制文件，以便执行后续命令
    - 对于尚未配置的安装，会先启动新手引导，再执行 Doctor 或 Gateway 网关探测。使用 `--no-onboard` 或没有 TTY 时，会输出稍后完成设置所需的命令。
    - 对于已配置的安装，会尽力刷新并重启已加载的 Gateway 网关服务，然后运行 Doctor。升级时会尽可能更新插件；在已启用提示但无交互终端的运行中，则输出手动命令。
    - 运行 `--verify` 时，会检查已安装版本；仅在配置存在后才检查 Gateway 健康。
...
对于无效的方式选择或无效的 `--install-method` 值，脚本会以退出码 `2` 退出。
...
| `--beta`                                | 如果 beta dist-tag 可用则使用它，否则回退到 `latest`                           |
| `--git-dir \| --dir <path>`             | 检出目录（默认：`~/openclaw`）                                                  |
| `--no-git-update`                       | 对现有检出目录跳过 `git pull`                                                   |
| `--no-prompt`                           | 禁用提示                                                                        |
| `--no-onboard`                          | 跳过新手引导                                                                    |
| `--onboard`                             | 启用新手引导                                                                    |
| `--verify`                              | 运行安装后冒烟验证（`--version`，以及已加载时的 Gateway 健康）                  |
| `--dry-run`                             | 输出操作但不应用更改                                                            |
...
| `OPENCLAW_BETA=0\|1`                              | 如果 beta 可用则使用它                                        |
...
<Info>
此脚本专为需要将所有内容安装在本地前缀下
（默认为 `~/.openclaw`）且不依赖系统 Node 的环境而设计。默认支持 npm 安装，
也支持在相同前缀流程下通过 git 检出进行安装。
</Info>
...
<Steps>
  <Step title="安装本地 Node 运行时">
    ...
    在 Alpine/musl Linux 上，由于 Node 不为这个固定运行时发布兼容的 tarball，因此会使用 `apk` 安装 `nodejs` 和 `npm`，并将该运行时链接到前缀封装脚本路径中。Alpine 软件源必须提供受支持的 Node 版本（22.19+、23.11+ 或 24+）；如果较旧的软件源仅提供 Node 20 或 21，请使用 Alpine 3.21 或更高版本。
  </Step>
  <Step title="确保安装 Git">
  ...
  <Step title="在前缀下安装 OpenClaw">
    ...
    - `git` 方式：克隆或更新检出目录（默认为 `~/openclaw`），并仍将封装脚本写入 `<prefix>/bin/openclaw`
...
| `--node-version <ver>`                  | Node 版本（默认值：`22.22.2`）                                               |
| `--json`                                | 输出 NDJSON 事件                                                              |
...
<Note>
对于 npm 安装，`openclaw@main` 和其他 GitHub 源规范不能作为有效的 `--version` 目标。请改用 `--install-method git --version main`。
</Note>
...
<Steps>
  <Step title="确保 PowerShell 和 Windows 环境可用">
    需要 PowerShell 5 或更高版本。
  </Step>
  <Step title="默认确保 Node.js 24 可用">
    如果缺失，将依次尝试通过 winget、Chocolatey 和 Scoop 安装。如果没有可用的包管理器，脚本会将官方 Node.js 24 Windows zip 下载到 `%LOCALAPPDATA%\OpenClaw\deps\portable-node`，并将其添加到当前进程和用户的 PATH。为保持兼容性，仍支持 Node 22.19+ 和 23.11+。
  </Step>
  <Step title="安装 OpenClaw">
    - `npm` 方式（默认）：使用所选 `-Tag` 执行 npm 全局安装，并从可写的安装程序临时目录启动，因此即使 shell 在 `C:\` 等受保护目录中打开也能正常工作
    - `git` 方式：克隆或更新仓库，使用 pnpm 安装和构建，并在 `%USERPROFILE%\.local\bin\openclaw.cmd` 安装包装脚本。如果缺少 Git，脚本会在 `%LOCALAPPDATA%\OpenClaw\deps\portable-git` 下引导安装用户本地的 MinGit，并将其添加到当前进程和用户的 PATH。
  ...
  <Step title="处理失败">
    `iwr ... | iex` 和脚本块安装会报告终止错误，但不会关闭当前 PowerShell 会话。直接使用 `powershell -File` / `pwsh -File` 安装时，仍会以非零状态退出，以便自动化流程检测失败。
  </Step>
</Steps>
...
<Note>
如果使用 `-InstallMethod git` 且缺少 Git，脚本会先尝试引导安装用户本地的 MinGit，然后再输出 Git for Windows 链接。
</Note>
...
<AccordionGroup>
  <Accordion title="为什么需要 Git？">
    `git` 安装方式需要 Git。对于 `npm` 安装，仍会检查或安装 Git，以免依赖项使用 git URL 时发生 `spawn git ENOENT` 错误。
  </Accordion>

  <Accordion title="为什么 npm 在 Linux 上遇到 EACCES？">
    某些 Linux 配置会将 npm 的全局前缀指向 root 所有的路径。`install.sh` 可以将前缀切换为 `~/.npm-global`，并将 PATH 导出语句追加到 shell rc 文件中（如果这些文件存在）。
  </Accordion>
  ...
  <Accordion title='Windows：“openclaw is not recognized”'>
    运行 `npm config get prefix`，将该目录添加到你的用户 PATH（在 Windows 上无需添加 `\bin` 后缀），然后重新打开 PowerShell。
  </Accordion>

  <Accordion title="Windows：如何获取详细的安装程序输出">
    `install.ps1` 不提供 `-Verbose` 开关。
    可使用 PowerShell 跟踪进行脚本级诊断：
