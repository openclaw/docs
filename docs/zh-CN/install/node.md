...
read_when:
    ...
    - 你已安装 OpenClaw，但 `openclaw` 显示找不到命令
    - npm install -g 因权限或 PATH 问题而失败
summary: 安装并配置用于 OpenClaw 的 Node.js - 版本要求、安装选项和 PATH 故障排查
title: Node.js
x-i18n:
    generated_at: "2026-07-05T11:24:31Z"
    model: gpt-5.5
...
OpenClaw 需要 **Node 22.19+、Node 23.11+ 或 Node 24+**。**Node 24 是安装、CI 和发布工作流的默认且推荐运行时**；Node 22 仍通过活跃 LTS 线获得支持。[安装脚本](/zh-CN/install#alternative-install-methods)会自动检测并安装 Node；当你想自行设置 Node（版本、PATH、全局安装）时，请使用本页。
...
推荐默认使用 `v24.x.x` 或更高版本。`v22.19.x` 或更高版本是受支持的 Node 22 LTS 路径（方便时请升级到 Node 24）。不支持 `v23.11.0` 之前的 Node 23 构建。如果缺少 Node，或版本超出支持范围，请选择下面的一种安装方法。
...
<Tabs>
  <Tab title="macOS">
    **Homebrew**（推荐）：
    ...
    或从 [nodejs.org](https://nodejs.org/) 下载 macOS 安装器。
  ...
  <Tab title="Windows">
    ...
    **Chocolatey：**
    ...
    或从 [nodejs.org](https://nodejs.org/) 下载 Windows 安装器。

  </Tab>
</Tabs>

<Accordion title="使用版本管理器（nvm、fnm、mise、asdf）">
  版本管理器让你可以轻松在 Node 版本之间切换。常见选项：
...
```
  ...
  <Warning>
  请在你的 shell 启动文件（`~/.zshrc` 或 `~/.bashrc`）中初始化版本管理器。如果跳过此步骤，新终端会话中可能找不到 `openclaw`，因为 PATH 不会包含 Node 的 bin 目录。
  </Warning>
</Accordion>
