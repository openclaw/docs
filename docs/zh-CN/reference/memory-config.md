...
summary: 记忆搜索、嵌入提供商、QMD、混合搜索和多模态索引的所有配置开关
title: 记忆配置参考
x-i18n:
    generated_at: "2026-06-27T03:16:01Z"
    model: gpt-5.5
...
此页面列出 OpenClaw 记忆搜索的每个配置开关。有关概念概览，请参阅：
...
请参阅 [主动记忆](/zh-CN/concepts/active-memory)，了解激活模型、插件拥有的配置、转录持久化和安全 rollout 模式。
</Note>
...
| `fallback` | `string`  | `"none"`         | 主提供商失败时使用的 fallback 适配器 ID                                                                                                                                                                                                                                                  |
...
显式的非本地提供商会 fail closed。如果你将 `memorySearch.provider` 设置为
...
提供商/凭证配置，切换到可访问的提供商，或在你想要有意使用仅 FTS 召回时设置
`provider: "none"`。

### 自定义提供商 ID

`memorySearch.provider` 可以指向自定义 `models.providers.<id>` 条目，用于记忆专用提供商适配器（如 `ollama`），或用于 OpenAI 兼容模型 API（如 `openai-responses` / `openai-completions`）。OpenClaw 会解析该提供商的 `api` 所有者作为嵌入适配器，同时保留自定义提供商 ID，用于端点、凭证和模型前缀处理。这让多 GPU 或多主机设置可以将记忆嵌入专用于特定本地端点：
...
| GitHub Copilot | `COPILOT_GITHUB_TOKEN`, `GH_TOKEN`, `GITHUB_TOKEN` | 通过设备登录的凭证配置       |
...
<AccordionGroup>
  ...
  <Accordion title="OpenAI-compatible input types">
    OpenAI 兼容嵌入端点可以选择加入提供商专用的 `input_type` 请求字段。这适用于需要为查询嵌入和文档嵌入使用不同标签的非对称嵌入模型。
    ...
    更改这些值会影响提供商批量索引的嵌入缓存身份；当上游模型对这些标签的处理不同时，应随后执行记忆重新索引。

  </Accordion>
  <Accordion title="Bedrock">
    ### Bedrock 嵌入配置

    Bedrock 使用 AWS SDK 默认凭证链，不需要 API 密钥。如果 OpenClaw 在带有 Bedrock 启用实例角色的 EC2 上运行，只需设置提供商和模型：
    ...
    **身份验证：**Bedrock 凭证使用标准 AWS SDK 凭证解析顺序：
  ...
  <Accordion title="Local (GGUF + llama.cpp)">
    ...
    默认模型：`embeddinggemma-300m-qat-Q8_0.gguf`（约 0.6 GB，自动下载）。源码检出仍需要原生构建审批：先运行 `pnpm approve-builds`，再运行 `pnpm rebuild node-llama-cpp`。
...
<Tabs>
  ...
  <Tab title="Temporal decay (recency)">
    ...
    常青文件（`MEMORY.md`，以及 `memory/` 中未标日期的文件）永不衰减。
...
对于 Agent 作用域的跨 Agent 转录搜索，请使用 `agents.list[].memorySearch.qmd.extraCollections`，而不是 `memory.qmd.paths`。这些额外集合遵循相同的 `{ path, name, pattern? }` 形状，但它们会按 Agent 合并，并且当路径指向当前工作区之外时，可以保留显式共享名称。如果同一个解析后的路径同时出现在 `memory.qmd.paths` 和 `memorySearch.qmd.extraCollections` 中，QMD 会保留第一个条目并跳过重复项。
...
可用于 `openai`、`gemini` 和 `voyage`。对于大规模回填，OpenAI 批量处理通常最快且成本最低。

`remote.nonBatchConcurrency` 控制由本地/自托管提供商使用的内联嵌入调用，以及在提供商批量 API 未启用时由托管提供商使用的内联嵌入调用。Ollama 对非批量索引默认使用 `1`，以避免压垮较小的本地主机；在更大的机器上可设置更高的值。
...
<Warning>
会话索引是选择启用的，并且会异步运行。结果可能略有滞后。会话日志存储在磁盘上，因此请将文件系统访问视为信任边界。
</Warning>
...
`rerank: false` 只会更改 QMD `query` 模式，并且需要 QMD 2.1 或更新版本。在直接 CLI 模式下，OpenClaw 会传递 `--no-rerank`；在由 mcporter 支持的 MCP 模式下，它会向 QMD 的统一查询工具传递 `rerank: false`。保持未设置即可使用 QMD 的默认查询重排序行为。

OpenClaw 优先使用当前的 QMD 集合和 MCP 查询形态，但会在需要时尝试兼容的集合模式标志和旧版 MCP 工具名称，以保持旧版 QMD 发布版本可用。当 QMD 声明支持多个集合过滤器时，同源集合会通过一个 QMD 进程搜索；旧版 QMD 构建会保留按集合的兼容路径。同源表示持久记忆集合会分组在一起，而会话转录集合会保留为单独分组，因此来源多样化仍然同时拥有两类输入。
...
<AccordionGroup>
  <Accordion title="更新计划">
    ...
    | `update.onBoot`           | `boolean` | `true`  | 长生命周期 QMD 管理器打开时刷新；设置为 false 可跳过立即启动更新 |
  ...
  <Accordion title="作用域">
    控制哪些会话可以接收 QMD 搜索结果。与 [`session.sendPolicy`](/zh-CN/gateway/config-agents#session) 使用相同的 schema：
...
启用 Gateway 网关启动时的 QMD 初始化后，OpenClaw 只会为符合条件的智能体启动 QMD。如果 `update.onBoot` 为 true 且未配置 interval/embed 维护，启动会使用一次性管理器执行启动刷新，然后关闭它。如果配置了 update 或 embed interval，启动会打开长生命周期 QMD 管理器，以便它拥有 watcher 和 interval timer；`update.onBoot: false` 只会跳过立即启动刷新。
...
Dreaming 会作为一次定时扫描运行，并将内部 light/deep/REM 阶段作为实现细节。
...
| `phases.deep.maxPromotedSnippetTokens` | `number`  | `160`         | 从每个提升到 `MEMORY.md` 的短期召回片段中保留的最大估算 token 数；来源元数据仍然可见 |
...
- `dreaming.model` 使用现有插件子智能体信任门控；启用前请设置 `plugins.entries.memory-core.subagent.allowModelOverride: true`。
- 当配置的模型不可用时，Dream Diary 会使用会话默认模型重试一次。信任或 allowlist 失败会被记录到日志中，不会静默重试。
