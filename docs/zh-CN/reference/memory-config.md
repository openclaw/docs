...
summary: 用于记忆搜索、嵌入提供商、QMD、混合搜索和多模态索引的所有配置选项
title: Memory 配置参考
x-i18n:
    generated_at: "2026-04-29T07:18:13Z"
    model: gpt-5.5
    provider: openai
...
此页面列出 OpenClaw 内存搜索的每个配置旋钮。概念概览见：
...
<CardGroup cols={2}>
  ...
  <Card title="主动内存" href="/zh-CN/concepts/active-memory">
    用于交互式会话的内存子智能体。
  </Card>
</CardGroup>
...
<Note>
如果你要找的是**主动内存**功能开关和子智能体配置，它位于 `plugins.entries.active-memory` 下，而不是 `memorySearch`。
...
有关激活模型、插件自有配置、对话记录持久化和安全发布模式，请参阅[主动内存](/zh-CN/concepts/active-memory)。
</Note>
...
<Steps>
  <Step title="local">
    如果配置了 `memorySearch.local.modelPath` 且文件存在，则选择此项。
  </Step>
  <Step title="github-copilot">
    如果可以解析 GitHub Copilot 令牌（环境变量或身份验证配置文件），则选择此项。
  </Step>
  <Step title="openai">
    如果可以解析 OpenAI 密钥，则选择此项。
  </Step>
  <Step title="gemini">
    如果可以解析 Gemini 密钥，则选择此项。
  </Step>
  <Step title="voyage">
    如果可以解析 Voyage 密钥，则选择此项。
  </Step>
  <Step title="mistral">
    如果可以解析 Mistral 密钥，则选择此项。
  </Step>
  <Step title="deepinfra">
    如果可以解析 DeepInfra 密钥，则选择此项。
  </Step>
  <Step title="bedrock">
    如果 AWS SDK 凭证链可以解析（实例角色、访问密钥、配置文件、SSO、Web 身份或共享配置），则选择此项。
  </Step>
</Steps>
...
`memorySearch.provider` 可以指向自定义 `models.providers.<id>` 条目。OpenClaw 会解析该提供商的 `api` 所有者以用于嵌入适配器，同时保留自定义提供商 ID，用于端点、身份验证和模型前缀处理。这让多 GPU 或多主机设置可以将内存嵌入专门指向某个本地端点：
...
<AccordionGroup>
  ...
  <Accordion title="OpenAI 兼容输入类型">
    OpenAI 兼容嵌入端点可以选择启用提供商特定的 `input_type` 请求字段。这对非对称嵌入模型很有用，因为这些模型需要为查询和文档嵌入使用不同标签。
    ...
    当上游模型以不同方式处理这些标签时，更改这些值会影响提供商批量索引的嵌入缓存身份，之后应重建内存索引。

  </Accordion>
  <Accordion title="Bedrock">
    Bedrock 使用 AWS SDK 默认凭证链，不需要 API 密钥。如果 OpenClaw 在 EC2 上运行并使用已启用 Bedrock 的实例角色，只需设置提供商和模型：
  ...
  <Accordion title="本地（GGUF + node-llama-cpp）">
    ...
    | `local.contextSize`   | `number \| "auto"` | `4096`                 | 嵌入上下文的上下文窗口大小。4096 覆盖典型分块（128–512 个 token），同时限制非权重 VRAM。在受限主机上降至 1024–2048。`"auto"` 使用模型训练时的最大值，不建议用于 8B+ 模型（Qwen3-Embedding-8B：40 960 个 token → 约 32 GB VRAM，而 4096 时约为 8.8 GB）。 |
    ...
    如果 `provider` 是 `auto`，只有当 `local.modelPath` 指向现有本地文件时，才会选择 `local`。`hf:` 和 HTTP(S) 模型引用仍可通过 `provider: "local"` 显式使用，但在模型可用于磁盘之前，它们不会让 `auto` 选择本地。
...
未设置时使用提供商默认值：本地/自托管提供商（如 `local`、`ollama` 和 `lmstudio`）为 600 秒，托管提供商为 120 秒。当本地 CPU 密集型嵌入批次运行正常但速度较慢时，请增大此值。
</ParamField>
...
<Tabs>
  ...
  <Tab title="时间衰减（新近度）">
    ...
    常青文件（`MEMORY.md`、`memory/` 中没有日期的文件）永远不会衰减。
...
路径可以是绝对路径，也可以是相对于工作区的路径。目录会被递归扫描以查找 `.md` 文件。符号链接处理取决于当前后端：内置引擎会忽略符号链接，而 QMD 遵循底层 QMD 扫描器的行为。

对于智能体作用域的跨智能体转录搜索，请使用 `agents.list[].memorySearch.qmd.extraCollections`，而不是 `memory.qmd.paths`。这些额外集合遵循相同的 `{ path, name, pattern? }` 结构，但它们会按智能体合并，并且当路径指向当前工作区之外时，可以保留显式共享名称。如果同一个解析后的路径同时出现在 `memory.qmd.paths` 和 `memorySearch.qmd.extraCollections` 中，QMD 会保留第一个条目并跳过重复项。
...
<Note>
仅适用于 `extraPaths` 中的文件。默认记忆根目录仍然仅支持 Markdown。需要 `gemini-embedding-2-preview`。`fallback` 必须为 `"none"`。
</Note>
...
可用于 `openai`、`gemini` 和 `voyage`。对于大型回填，OpenAI 批处理通常最快且成本最低。

`remote.nonBatchConcurrency` 控制本地/自托管提供商以及提供商批处理 API 未启用时托管提供商使用的内联嵌入调用。Ollama 对非批量索引默认使用 `1`，以避免压垮较小的本地主机；在更大的机器上可设置更高的值。
...
<Warning>
会话索引是选择启用的，并且异步运行。结果可能略微过时。会话日志存储在磁盘上，因此应将文件系统访问视为信任边界。
</Warning>
...
| `sessions.retentionDays` | `number`  | --       | 转录记录保留期限                                                       |
...
OpenClaw 优先使用当前的 QMD collection 和 MCP query 形态，但会在需要时尝试兼容的 collection pattern flags 和旧版 MCP 工具名称，以保持旧版 QMD release 可用。当 QMD 宣告支持多个 collection filter 时，同源 collection 会通过一个 QMD 进程搜索；旧版 QMD build 继续使用逐 collection 的兼容路径。同源意味着持久化 memory collection 会分组在一起，而会话 transcript collection 会保持为单独的一组，因此 source diversification 仍然同时拥有两类输入。

<Note>
QMD 模型覆盖保留在 QMD 侧，而不是 OpenClaw 配置。如果你需要全局覆盖 QMD 的模型，请在 Gateway 网关运行时环境中设置 `QMD_EMBED_MODEL`、`QMD_RERANK_MODEL` 和 `QMD_GENERATE_MODEL` 等环境变量。
</Note>
...
<AccordionGroup>
  <Accordion title="更新计划">
    ...
    | `update.onBoot`           | `boolean` | `true`  | 启动时在 QMD 子进程中刷新 |
  ...
  <Accordion title="范围">
    ...
    随附默认设置允许 direct 和渠道会话，同时仍拒绝群组。

    默认仅限私信。`match.keyPrefix` 匹配规范化的会话 key；`match.rawKeyPrefix` 匹配包含 `agent:<id>:` 的原始 key。
...
QMD 启动刷新会在 Gateway 网关启动期间使用一次性子进程路径。当 memory search 为交互式使用打开时，长驻 QMD manager 仍然负责常规文件 watcher 和 interval timer。
...
Dreaming 作为一次计划 sweep 运行，并将内部 light/deep/REM 阶段用作实现细节。

有关概念性行为和 slash command，请参阅 [Dreaming](/zh-CN/concepts/dreaming)。
...
- `dreaming.model` 使用现有插件子智能体信任门；启用前请设置 `plugins.entries.memory-core.subagent.allowModelOverride: true`。
