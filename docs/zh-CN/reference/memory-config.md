...
title: 记忆配置参考
x-i18n:
    generated_at: "2026-04-29T07:52:03Z"
    model: gpt-5.5
    provider: openai
...
本页列出 OpenClaw 记忆搜索的每个配置旋钮。有关概念概览，请参阅：
...
<Note>
如果你在寻找 **主动记忆** 功能开关和子智能体配置，它位于 `plugins.entries.active-memory` 下，而不是 `memorySearch`。

主动记忆使用双门控模型：

1. 插件必须已启用，并且目标为当前智能体 ID
2. 请求必须是符合条件的交互式持久聊天会话

有关激活模型、插件自有配置、转录持久化和安全推出模式，请参阅 [主动记忆](/zh-CN/concepts/active-memory)。
</Note>
...
<Steps>
  ...
  <Step title="github-copilot">
    如果可以解析 GitHub Copilot token（环境变量或认证配置文件），则选择它。
  </Step>
  <Step title="openai">
    如果可以解析 OpenAI 密钥，则选择它。
  </Step>
  <Step title="gemini">
    如果可以解析 Gemini 密钥，则选择它。
  </Step>
  <Step title="voyage">
    如果可以解析 Voyage 密钥，则选择它。
  </Step>
  <Step title="mistral">
    如果可以解析 Mistral 密钥，则选择它。
  </Step>
  <Step title="deepinfra">
    如果可以解析 DeepInfra 密钥，则选择它。
  </Step>
  <Step title="bedrock">
    如果 AWS SDK 凭证链可以解析（实例角色、访问密钥、配置文件、SSO、Web 身份或共享配置），则选择它。
  </Step>
</Steps>
...
`memorySearch.provider` 可以指向自定义 `models.providers.<id>` 条目。OpenClaw 会为嵌入适配器解析该提供商的 `api` 所有者，同时保留自定义提供商 ID，用于端点、认证和模型前缀处理。这让多 GPU 或多主机设置可以将记忆嵌入专用于特定本地端点：
...
<AccordionGroup>
  ...
  <Accordion title="OpenAI 兼容输入类型">
    OpenAI 兼容嵌入端点可以选择加入提供商特定的 `input_type` 请求字段。这对于非对称嵌入模型很有用，这类模型要求查询和文档嵌入使用不同标签。
    ...
    更改这些值会影响提供商批量索引的嵌入缓存身份；如果上游模型对这些标签的处理不同，之后应执行一次记忆重建索引。

  </Accordion>
  <Accordion title="Bedrock">
    Bedrock 使用 AWS SDK 默认凭证链，不需要 API 密钥。如果 OpenClaw 在 EC2 上运行，并带有启用了 Bedrock 的实例角色，只需设置提供商和模型：
    ...
    **认证：** Bedrock 认证使用标准 AWS SDK 凭证解析顺序：
    ...
    区域会从 `AWS_REGION`、`AWS_DEFAULT_REGION`、`amazon-bedrock` 提供商 `baseUrl` 解析，或默认使用 `us-east-1`。
    ...
    **IAM 权限：** IAM 角色或用户需要：
    ...
    为实现最小权限，请将 `InvokeModel` 范围限定到特定模型：
  ...
  <Accordion title="本地（GGUF + node-llama-cpp）">
    ...
    | `local.contextSize`   | `number \| "auto"` | `4096`                 | 嵌入上下文的上下文窗口大小。4096 可覆盖典型分块（128–512 个 token），同时限制非权重 VRAM。资源受限的主机可降至 1024–2048。`"auto"` 使用模型训练时的最大值，不建议用于 8B+ 模型（Qwen3-Embedding-8B：40 960 个 token → 约 32 GB VRAM，而 4096 时约 8.8 GB）。 |

    默认模型：`embeddinggemma-300m-qat-Q8_0.gguf`（约 0.6 GB，自动下载）。需要原生构建：先运行 `pnpm approve-builds`，再运行 `pnpm rebuild node-llama-cpp`。

    使用独立 CLI 验证 Gateway 网关使用的同一个提供商路径：
    ...
    如果 `provider` 是 `auto`，只有当 `local.modelPath` 指向现有本地文件时才会选择 `local`。`hf:` 和 HTTP(S) 模型引用仍可通过 `provider: "local"` 显式使用，但在模型可用于磁盘之前，它们不会让 `auto` 选择本地。
...
未设置时使用提供商默认值：`local`、`ollama` 和 `lmstudio` 等本地/自托管提供商为 600 秒，托管提供商为 120 秒。当本地 CPU 密集型嵌入批次健康但较慢时，请增大此值。
</ParamField>
...
路径可以是绝对路径，也可以是相对于工作区的路径。目录会被递归扫描以查找 `.md` 文件。符号链接处理取决于当前后端：内置引擎会忽略符号链接，而 QMD 会遵循底层 QMD 扫描器的行为。

对于按 agent 作用域进行的跨 agent 转录搜索，请使用 `agents.list[].memorySearch.qmd.extraCollections`，而不是 `memory.qmd.paths`。这些额外集合遵循相同的 `{ path, name, pattern? }` 形状，但它们会按 agent 合并，并且当路径指向当前工作区之外时，可以保留显式的共享名称。如果同一个解析后的路径同时出现在 `memory.qmd.paths` 和 `memorySearch.qmd.extraCollections` 中，QMD 会保留第一个条目并跳过重复项。

---

## 多模态内存（Gemini）

使用 Gemini Embedding 2 在 Markdown 旁一起索引图像和音频：
...
<Note>
仅适用于 `extraPaths` 中的文件。默认内存根目录仍仅支持 Markdown。需要 `gemini-embedding-2-preview`。`fallback` 必须是 `"none"`。
</Note>
...
`remote.nonBatchConcurrency` 控制本地/自托管提供商使用的内联嵌入调用，以及托管提供商在提供商批处理 API 未启用时使用的内联嵌入调用。Ollama 在非批量索引时默认使用 `1`，以避免压垮较小的本地主机；在更大的机器上可以设置更高的值。

这不同于 `sync.embeddingBatchTimeoutSeconds`，后者控制内联嵌入调用的超时时间。

---

## 会话记忆搜索（实验性）

索引会话记录，并通过 `memory_search` 展示：
...
<Warning>
会话索引是选择启用的，并且会异步运行。结果可能会稍微滞后。会话日志存储在磁盘上，因此应将文件系统访问视为信任边界。
</Warning>
...
| `sessions.retentionDays` | `number`  | --       | 会话记录保留期                                                                    |
...
OpenClaw 优先使用当前的 QMD collection 和 MCP 查询形态，但会在需要时尝试兼容的 collection pattern 标志和旧版 MCP 工具名称，以保持旧版 QMD release 可用。当 QMD 声明支持多个 collection 过滤器时，同源 collection 会通过一个 QMD 进程搜索；旧版 QMD 构建仍保留按 collection 的兼容路径。同源表示持久 memory collection 会被归为一组，而会话 transcript collection 仍保留为单独一组，因此来源多样化仍然同时包含两类输入。
...
<AccordionGroup>
  <Accordion title="Update schedule">
    ...
    | `update.onBoot`           | `boolean` | `true`  | 长驻 QMD manager 打开时刷新；也控制选择加入的启动刷新 |
    ...
    | `update.waitForBootSync`  | `boolean` | `false` | 阻塞 manager 打开，直到其初始刷新完成 |
    | `update.embedInterval`    | `string`  | --      | 单独的嵌入节奏                |
    | `update.commandTimeoutMs` | `number`  | --      | QMD 命令的超时时间              |
    | `update.updateTimeoutMs`  | `number`  | --      | QMD update 操作的超时时间     |
  ...
  <Accordion title="Scope">
    ...
    随附默认值允许直聊和渠道会话，同时仍拒绝群组。

    默认仅限私信。`match.keyPrefix` 匹配规范化后的会话 key；`match.rawKeyPrefix` 匹配包含 `agent:<id>:` 的原始 key。

  </Accordion>
  <Accordion title="Citations">
    `memory.citations` 适用于所有 backend：

    | Value            | Behavior                                            |
    | ---------------- | --------------------------------------------------- |
    | `auto`（默认） | 在 snippet 中包含 `Source: <path#line>` footer    |
    | `on`             | 始终包含 footer                               |
    | `off`            | 省略 footer（path 仍会在内部传递给智能体） |

  </Accordion>
</AccordionGroup>

QMD 启动刷新会在 Gateway 网关启动期间使用一次性子进程路径。当 memory search 为交互式使用而打开时，长驻 QMD manager 仍然拥有常规文件 watcher 和间隔 timer。
...
Dreaming 作为一次计划 sweep 运行，并将内部 light/deep/REM phase 作为实现细节使用。

有关概念行为和 slash command，请参阅 [Dreaming](/zh-CN/concepts/dreaming)。
...
- Dreaming 将人类可读的叙事输出写入 `DREAMS.md`（或已有的 `dreams.md`）。
- `dreaming.model` 使用现有插件子智能体 trust gate；启用前请设置 `plugins.entries.memory-core.subagent.allowModelOverride: true`。
- 当配置的模型不可用时，Dream Diary 会使用会话默认模型重试一次。trust 或 allowlist 失败会被记录日志，不会静默重试。
- light/deep/REM phase policy 和 threshold 属于内部行为，不是面向用户的配置。
