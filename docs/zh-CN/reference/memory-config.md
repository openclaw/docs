...
title: 记忆配置参考
x-i18n:
    generated_at: "2026-05-01T20:40:36Z"
    model: gpt-5.5
    provider: openai
...
<Note>
如果你正在寻找 **主动记忆** 功能开关和子智能体配置，它位于 `plugins.entries.active-memory` 下，而不是 `memorySearch`。
...
请参阅 [主动记忆](/zh-CN/concepts/active-memory)，了解激活模型、插件拥有的配置、转录持久化和安全发布模式。
</Note>
...
| 键        | 类型      | 默认值          | 描述                                                                                                                                                                                                                        |
| ---------- | --------- | ---------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `provider` | `string`  | 自动检测    | 嵌入适配器 ID，例如 `bedrock`、`deepinfra`、`gemini`、`github-copilot`、`local`、`mistral`、`ollama`、`openai` 或 `voyage`；也可以是一个已配置的 `models.providers.<id>`，其 `api` 指向这些适配器之一 |
...
<Steps>
  <Step title="local">
    如果已配置 `memorySearch.local.modelPath` 且文件存在，则选择此项。
  </Step>
  <Step title="github-copilot">
    如果可以解析 GitHub Copilot token（环境变量或认证配置文件），则选择此项。
  </Step>
  <Step title="openai">
    如果可以解析 OpenAI key，则选择此项。
  </Step>
  <Step title="gemini">
    如果可以解析 Gemini key，则选择此项。
  </Step>
  <Step title="voyage">
    如果可以解析 Voyage key，则选择此项。
  </Step>
  <Step title="mistral">
    如果可以解析 Mistral key，则选择此项。
  </Step>
  <Step title="deepinfra">
    如果可以解析 DeepInfra key，则选择此项。
  </Step>
  <Step title="bedrock">
    如果 AWS SDK 凭证链可解析（实例角色、访问密钥、配置文件、SSO、Web 身份或共享配置），则选择此项。
  </Step>
</Steps>
...
<AccordionGroup>
  ...
  <Accordion title="OpenAI-compatible input types">
    OpenAI 兼容的嵌入端点可以选择加入提供商特定的 `input_type` 请求字段。这对需要为查询和文档嵌入使用不同标签的非对称嵌入模型很有用。
    ...
    更改这些值会影响提供商批量索引的嵌入缓存标识；如果上游模型对这些标签的处理不同，之后应重建记忆索引。

  </Accordion>
  <Accordion title="Bedrock">
    Bedrock 使用 AWS SDK 默认凭证链，不需要 API key。如果 OpenClaw 在带有启用 Bedrock 的实例角色的 EC2 上运行，只需设置提供商和模型：
  ...
  <Accordion title="本地（GGUF + node-llama-cpp）">
    ...
    | `local.contextSize`   | `number \| "auto"` | `4096`                 | 嵌入上下文的上下文窗口大小。4096 覆盖典型分块（128–512 个 token），同时限制非权重 VRAM。在资源受限的主机上可降低到 1024–2048。`"auto"` 使用模型训练时的最大值，不建议用于 8B+ 模型（Qwen3-Embedding-8B：40 960 个 token → 约 32 GB VRAM，而 4096 时约 8.8 GB）。 |
    ...
    使用独立 CLI 验证 Gateway 网关使用的相同提供商路径：
    ...
    如果 `provider` 是 `auto`，只有当 `local.modelPath` 指向现有本地文件时，才会选择 `local`。仍然可以通过 `provider: "local"` 显式使用 `hf:` 和 HTTP(S) 模型引用，但在模型可用于磁盘之前，它们不会让 `auto` 选择本地。
...
<Tabs>
  ...
  <Tab title="时间衰减（近因性）">
    | 键                           | 类型      | 默认值 | 描述                   |
    | ---------------------------- | --------- | ------ | ---------------------- |
    | `temporalDecay.enabled`      | `boolean` | `false` | 启用近因性加成         |
    | `temporalDecay.halfLifeDays` | `number`  | `30`   | 每 N 天分数减半        |

    长青文件（`MEMORY.md`，以及 `memory/` 中没有日期的文件）永不衰减。
...
路径可以是绝对路径，也可以是相对于工作区的路径。目录会被递归扫描以查找 `.md` 文件。符号链接处理取决于活动后端：内置引擎会忽略符号链接，而 QMD 会遵循底层 QMD 扫描器的行为。

对于智能体作用域的跨智能体转录搜索，请使用 `agents.list[].memorySearch.qmd.extraCollections`，而不是 `memory.qmd.paths`。这些额外集合遵循相同的 `{ path, name, pattern? }` 形状，但它们会按智能体合并，并且当路径指向当前工作区之外时，可以保留显式共享名称。如果同一个解析后的路径同时出现在 `memory.qmd.paths` 和 `memorySearch.qmd.extraCollections` 中，QMD 会保留第一个条目并跳过重复项。
...
适用于 `openai`、`gemini` 和 `voyage`。对于大型回填，OpenAI 批量通常最快且成本最低。

`remote.nonBatchConcurrency` 控制本地/自托管提供商使用的内联嵌入调用，以及提供商批量 API 未启用时托管提供商使用的内联嵌入调用。Ollama 的非批量索引默认值为 `1`，以避免压垮较小的本地主机；在更大的机器上可设置更高的值。
...
<Warning>
会话索引是选择启用的，并且会异步运行。结果可能略微陈旧。会话日志保存在磁盘上，因此应将文件系统访问视为信任边界。
</Warning>
...
| 键                       | 类型      | 默认值   | 描述                                                                   |
| ------------------------ | --------- | -------- | ---------------------------------------------------------------------- |
| `command`                | `string`  | `qmd`    | QMD 可执行文件路径；当服务 `PATH` 与你的 shell 不同时，设置绝对路径    |
...
OpenClaw 优先使用当前的 QMD collection 和 MCP query 形态，但会在需要时尝试兼容的 collection pattern 标志和旧版 MCP 工具名称，以保持旧版 QMD 发布版本可用。当 QMD 声明支持多个 collection 过滤器时，同源 collection 会通过一个 QMD 进程搜索；旧版 QMD 构建版本继续使用逐 collection 的兼容路径。同源表示持久化记忆 collection 会被归为一组，而会话 transcript collection 会保留为独立分组，这样来源多样化仍然同时具备两类输入。
...
<AccordionGroup>
  <Accordion title="更新调度">
    | 键                        | 类型      | 默认值  | 描述                                  |
    | ------------------------- | --------- | ------- | ------------------------------------- |
    | `update.interval`         | `string`  | `5m`    | 刷新间隔                              |
    | `update.debounceMs`       | `number`  | `15000` | 对文件变更进行防抖                    |
    | `update.onBoot`           | `boolean` | `true`  | 在长驻 QMD 管理器打开时刷新；也控制可选的启动刷新 |
...
QMD 启动刷新会在 Gateway 网关启动期间使用一次性子进程路径。当记忆搜索为交互式使用打开时，长驻 QMD 管理器仍然负责常规文件观察器和间隔计时器。
...
Dreaming 作为一次定时扫描运行，并将内部 light/deep/REM 阶段作为实现细节。
...
- `dreaming.model` 使用现有插件子智能体信任门；在启用它之前设置 `plugins.entries.memory-core.subagent.allowModelOverride: true`。
