...
除非另有说明，否则所有记忆搜索设置都位于 `openclaw.json` 的 `agents.defaults.memorySearch` 下（也可以通过每个智能体的 `agents.list[].memorySearch` 覆盖）。

<Note>
如果你要查找的是**主动记忆**功能开关和子智能体配置，它们位于 `plugins.entries.active-memory` 下，而不是 `memorySearch`。
...
有关激活模型、插件自有配置、对话记录持久化和安全推出模式，请参阅[主动记忆](/zh-CN/concepts/active-memory)。
</Note>
...
Voyage、本地 GGUF 模型或兼容 OpenAI 的 `/v1/embeddings` 端点。
仍使用 `provider: "auto"` 的旧版配置会解析为 `openai`。

<Warning>
更改嵌入提供商、模型、提供商设置、来源、作用域、分块方式或分词器，可能会导致现有 SQLite 向量索引不兼容。
OpenClaw 不会自动重新嵌入所有内容，而是暂停向量搜索并报告索引标识警告。
准备就绪后，可使用 `openclaw memory status --index --agent <id>` 或
`openclaw memory index --force --agent <id>` 重建索引。
</Warning>

当未设置 `provider`、存在旧版 `provider: "auto"`，或通过
`provider: "none"` 有意选择仅 FTS 模式时，即使嵌入不可用，记忆召回仍可使用词法 FTS 排名。

显式指定的非本地提供商会采用失败关闭策略。如果你将 `memorySearch.provider`
设置为具体的远程支持提供商，例如 Bedrock、DeepInfra、Gemini、GitHub
Copilot、LM Studio、Mistral、Ollama、OpenAI、Voyage 或兼容 OpenAI
的自定义提供商，而该提供商在运行时不可用，`memory_search`
将返回不可用结果，而不会静默改用仅 FTS 召回。请修复提供商或身份验证配置、
切换到可访问的提供商；如果你确实希望仅使用 FTS 召回，请设置
`provider: "none"`。

### 自定义提供商 ID

`memorySearch.provider` 可以指向自定义的 `models.providers.<id>` 条目，用于 `ollama` 等记忆专用提供商适配器，或 `openai-responses` / `openai-completions` 等兼容 OpenAI 的模型 API。OpenClaw 会根据该提供商的 `api` 所属方解析嵌入适配器，同时保留自定义提供商 ID，以处理端点、身份验证和模型前缀。这样，多 GPU 或多主机设置便可将记忆嵌入专门分配给特定的本地端点：
...
远程嵌入需要 API 密钥。Bedrock 改用 AWS SDK 默认凭据链（实例角色、SSO、访问密钥或 Bedrock API 密钥）。

| 提供商         | 环境变量                                            | 配置键                              |
| -------------- | --------------------------------------------------- | ----------------------------------- |
| Bedrock        | AWS 凭据链或 `AWS_BEARER_TOKEN_BEDROCK`             | 无需 API 密钥                       |
...
对于不应继承全局 OpenAI 聊天凭据的通用兼容 OpenAI
...
<AccordionGroup>
  ...
  <Accordion title="兼容 OpenAI 的输入类型">
    兼容 OpenAI 的嵌入端点可以选择使用提供商特定的 `input_type` 请求字段。这对于要求查询嵌入和文档嵌入使用不同标签的非对称嵌入模型很有用。
    ...
    更改这些值会影响提供商批量索引的嵌入缓存标识；如果上游模型对这些标签采用不同处理方式，之后应重新构建记忆索引。

  </Accordion>
  <Accordion title="Bedrock">
    ### Bedrock 嵌入配置

    Bedrock 使用 AWS SDK 默认凭据链以及经 OpenClaw 检查的持有者令牌，因此配置中不会存储 API 密钥。如果 OpenClaw 在具有 Bedrock 权限的实例角色的 EC2 上运行，只需设置提供商和模型：
    ...
    **区域：**按以下顺序解析：`memorySearch.remote.baseUrl` 覆盖项、`models.providers.amazon-bedrock.baseUrl` 配置、`AWS_REGION`、`AWS_DEFAULT_REGION`，最后默认为 `us-east-1`。
  ...
  <Accordion title="本地（GGUF + llama.cpp）">
    ...
    | `local.contextSize`   | `number \| "auto"` | `4096`                  | 嵌入上下文的上下文窗口大小。4096 可覆盖典型文本块（128-512 个令牌），同时限制模型权重以外的显存占用。在资源受限的主机上可降低至 1024-2048。`"auto"` 使用模型训练时的最大值——不建议用于 8B+ 模型（Qwen3-Embedding-8B：最高 40 960 个令牌，可能将显存占用推高至约 32 GB）。 |

    请先安装官方 llama.cpp 提供商：`openclaw plugins install @openclaw/llama-cpp-provider`。
    默认模型：`embeddinggemma-300m-qat-Q8_0.gguf`（约 0.6 GB，自动下载）。源代码检出仍需批准原生构建：先运行 `pnpm approve-builds`，再运行 `pnpm rebuild node-llama-cpp`。

    使用独立 CLI 验证与 Gateway 网关所用路径相同的提供商路径：
    ...
    数值型 `local.contextSize` 值也会用于指导 node-llama-cpp 自动放置 GPU 层，以便同时容纳模型权重和请求的嵌入上下文。运行时加载后，`openclaw memory status --deep` 会报告上次已知的 llama.cpp 后端、设备、卸载情况、请求的上下文，以及带时间戳的内存信息；被动状态查询不会加载模型。

    若要使用本地 GGUF 嵌入，请显式设置 `provider: "local"`。显式本地配置支持 `hf:` 和 HTTP(S) 模型引用（通过 node-llama-cpp 的模型解析），但它们不会更改默认提供商。
...
未设置时使用提供商默认值：`local`、`ollama` 和 `lmstudio` 等本地/自托管提供商为 600 秒，托管提供商为 120 秒。当受本地 CPU 限制的嵌入批次运行正常但速度较慢时，请增大此值。
</ParamField>
...
| `watchDebounceMs`              | `number`  | `1500` | 合并快速连续文件监视事件的防抖窗口                     |
| `intervalMinutes`              | `number`  | `0`    | 定期重新索引的时间间隔（分钟；`0` 表示禁用）           |
| `sessions.postCompactionForce` | `boolean` | `true` | 压缩触发转录更新后，强制重新索引会话                   |

<ParamField path="chunking.tokens" type="number">
  在嵌入前拆分记忆源时使用的分块大小，以 token 为单位（默认值：400）。
</ParamField>
...
<Note>
更改 `chunking.tokens` 或 `chunking.overlap` 会改变分块边界，并使现有索引标识失效（参见“提供商选择”下的警告）。
</Note>
...
| `minScore`   | `number` | `0.35` | 纳入命中结果的最低相关性分数 |

以下配置位于 `memorySearch.query.hybrid` 下：

| 键                    | 类型      | 默认值 | 说明                              |
| --------------------- | --------- | ------ | --------------------------------- |
| `enabled`             | `boolean` | `true` | 启用 BM25 与向量混合搜索          |
...
<Tabs>
  ...
  <Tab title="时间衰减（新近度）">
    ...
    常驻文件（`MEMORY.md`、`memory/` 中不带日期的文件）永不衰减。
...
路径可以是绝对路径，也可以是相对于工作区的路径。系统会递归扫描目录中的 `.md` 文件。符号链接的处理方式取决于当前启用的后端：内置引擎会跳过符号链接，而 QMD 则遵循其底层扫描器的行为。

对于 Agent 范围内的跨 Agent 对话记录搜索，请使用 `agents.list[].memorySearch.qmd.extraCollections`，而不是 `memory.qmd.paths`。这些额外集合采用相同的 `{ path, name, pattern? }` 结构，但会按 Agent 合并；当路径指向当前工作区之外时，还可以保留明确指定的共享名称。如果同一个解析后的路径同时出现在 `memory.qmd.paths` 和 `memorySearch.qmd.extraCollections` 中，QMD 会保留第一个条目并跳过重复项。

---

## 多模态记忆（Gemini）

使用 Gemini Embedding 2 为 Markdown、图像和音频一并建立索引：

| 键                        | 类型       | 默认值     | 说明                                   |
| ------------------------- | ---------- | ---------- | -------------------------------------- |
| `multimodal.enabled`      | `boolean`  | `false`    | 启用多模态索引                         |
| `multimodal.modalities`   | `string[]` | --         | `["image"]`、`["audio"]` 或 `["all"]` |
| `multimodal.maxFileBytes` | `number`   | `10485760` | 可建立索引的最大文件大小（10 MiB）     |

<Note>
仅适用于 `extraPaths` 中的文件。默认记忆根目录仍仅支持 Markdown。需要使用 `gemini-embedding-2-preview`。`fallback` 必须为 `"none"`。
</Note>
...
| 键                 | 类型      | 默认值   | 说明                              |
| ------------------ | --------- | -------- | --------------------------------- |
| `cache.enabled`    | `boolean` | `true`   | 在 SQLite 中缓存文本块嵌入        |
| `cache.maxEntries` | `number`  | 未设置   | 缓存嵌入数量的尽力而为上限        |

防止在重新索引或更新转录记录时对未更改的文本重复生成嵌入。将 `maxEntries` 保持未设置可使用无界缓存；当磁盘空间增长比重新索引的峰值速度更重要时，请设置此项。设置后，一旦缓存超过限制，将优先清理最早的条目（按最后更新时间排序）。
...
| `remote.batch.timeoutMinutes` | `number`  | `60`    | 批处理超时时间       |

适用于 `gemini`、`openai` 和 `voyage`。对于大规模回填，OpenAI 批处理通常速度最快且成本最低。

`remote.nonBatchConcurrency` 控制本地/自托管提供商使用的内联嵌入调用，以及未启用提供商批量 API 时托管提供商使用的内联嵌入调用。为避免给配置较低的本地主机造成过大压力，Ollama 的非批量索引并发数默认为 `1`；在性能更强的机器上可设置更高的值。
...
为会话转录记录建立索引，并通过 `memory_search` 提供搜索结果：
...
<Warning>
会话索引需要主动启用，并以异步方式运行。结果可能会略有延迟。会话日志存储在磁盘上，因此应将文件系统访问视为信任边界。
</Warning>

会话转录记录的匹配结果也遵循
[`tools.sessions.visibility`](/zh-CN/gateway/config-tools#toolssessions)。默认的
`tree` 可见性仅公开当前会话及其派生的会话。如果要从一个会话（例如私信）中
回忆由 Gateway 网关分派、属于同一智能体但无关的另一个会话，请有意将可见性扩大到
`agent`（仅当还需要跨智能体回忆且智能体间策略允许时，才使用 `all`）。

以下示例将这些设置放在 `agents.defaults` 下。如果只有一个智能体需要索引和搜索会话转录记录，
也可以在该智能体的覆盖配置中应用等效的 `memorySearch` 设置。

用于同一智能体从 Gateway 网关到私信的回忆：
...
---
  ...
  | `includeDefaultMemory`   | `boolean` | `true`   | 自动为 `MEMORY.md` 和 `memory/**/*.md` 建立索引                                        |
  | `paths[]`                | `array`   | --       | 额外路径：`{ name, path, pattern? }`                                                    |
  | `sessions.enabled`       | `boolean` | `false`  | 将会话记录导出到 QMD                                                                   |
  | `sessions.retentionDays` | `number`  | --       | 会话记录保留期限                                                                       |
  | `sessions.exportDir`     | `string`  | --       | 导出目录                                                                               |

  `searchMode: "search"` 仅使用词法搜索/BM25。在此模式下，OpenClaw 不会运行语义向量就绪探测或 QMD 嵌入维护，包括执行 `memory status --deep` 时；`vsearch` 和 `query` 仍要求 QMD 向量和嵌入处于就绪状态。

  `rerank: false` 仅更改 QMD 的 `query` 模式，并要求 QMD 2.1 或更高版本。在直接 CLI 模式下，OpenClaw 会传递 `--no-rerank`；在由 mcporter 支持的 MCP 模式下，它会向 QMD 的统一查询工具传递 `rerank: false`。保持未设置可使用 QMD 默认的查询重排序行为。

  OpenClaw 优先使用当前的 QMD 集合和 MCP 查询格式，但会在需要时尝试兼容的集合模式标志和旧版 MCP 工具名称，以保持旧版 QMD 可用。当 QMD 声明支持多个集合筛选器时，将使用单个 QMD 进程搜索同源集合；旧版 QMD 构建则继续使用按集合划分的兼容路径。同源是指将持久记忆集合（默认记忆文件和自定义路径）归为一组，而会话记录集合保持为单独的一组，以确保来源多样化仍包含两类输入。
  ...
  所有设置都位于 `memory.qmd.mcporter` 下。它通过长期运行的 `mcporter` MCP 守护进程路由 QMD 搜索，而不是为每次查询生成 `qmd` 进程，从而减少较大模型的冷启动开销。

  | 键            | 类型      | 默认值  | 说明                                                                   |
  | ------------- | --------- | ------- | ---------------------------------------------------------------------- |
  | `enabled`     | `boolean` | `false` | 通过 mcporter 路由 QMD 调用，而不是为每个请求生成 `qmd` 进程           |
  | `serverName`  | `string`  | `qmd`   | 运行 `qmd mcp` 且配置了 `lifecycle: keep-alive` 的 mcporter 服务器名称 |
  | `startDaemon` | `boolean` | `true`  | 当 `enabled` 为 true 时自动启动 mcporter 守护进程                       |

  要求已安装 `mcporter` 且其位于 PATH 中，并已配置运行 `qmd mcp` 的 mcporter 服务器。对于可接受按查询生成进程开销的简单本地设置，请保持禁用。
  ...
  <Accordion title="更新计划">
    ...
    | `update.onBoot`           | `boolean` | `true`   | 长期运行的 QMD 管理器打开时刷新；设为 false 可跳过启动后的立即更新                     |
    | `update.startup`          | `string`  | `off`    | 可选的 Gateway 网关启动时 QMD 初始化方式：`off`、`idle` 或 `immediate`                 |
    ...
    | `update.commandTimeoutMs` | `number`  | `30000`  | QMD 维护命令（列出/添加集合）的超时时间                                                |
  ...
  <Accordion title="作用域">
    控制哪些会话可以接收 QMD 搜索结果。其模式与 [`session.sendPolicy`](/zh-CN/gateway/config-agents#session) 相同：
    ...
    发布时的默认设置仅允许私信/直接会话，并拒绝群组和其他渠道类型。`match.keyPrefix` 匹配规范化后的会话键；`match.rawKeyPrefix` 匹配包含 `agent:<id>:` 的原始键。
...
启用 Gateway 网关启动时的 QMD 初始化后，OpenClaw 只会为符合条件的智能体启动 QMD。如果 `update.onBoot` 为 true，并且未配置定时更新或嵌入维护，启动过程会使用一次性管理器执行启动刷新，然后将其关闭。如果配置了更新或嵌入间隔，启动过程会打开长期运行的 QMD 管理器，使其管理监视器和间隔计时器；`update.onBoot: false` 只会跳过启动后立即执行的刷新。
...
Dreaming 以一次定时扫描运行，并将内部的浅层、深层和 REM 阶段作为实现细节。

有关概念性行为和斜杠命令，请参阅 [Dreaming](/zh-CN/concepts/dreaming)。
...
| `frequency`                            | `string`  | `0 3 * * *`  | 完整 Dreaming 扫描的可选 cron 运行频率                                                                                       |
...
- 当配置的模型不可用时，Dream Diary 会使用会话默认模型重试一次。信任或允许列表校验失败会被记录到日志中，并且不会静默重试。
- 浅层、深层和 REM 阶段的策略及阈值属于内部行为，而不是面向用户的配置。
