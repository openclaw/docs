---
read_when:
    - 你想了解项目背景或贡献者致谢
summary: 项目起源、贡献者和许可证。
title: 致谢
x-i18n:
    generated_at: "2026-05-06T05:01:07Z"
    model: gpt-5.5
    provider: openai
    source_hash: 4cda0d96a66d4a6fcf81553d39315ebe01e7ebb4a21bb2994e9a503825bc7fb6
    source_path: reference/credits.md
    workflow: 16
    postprocess_version: locale-links-v1
---

## 名称

OpenClaw = CLAW + TARDIS，因为每只太空龙虾都需要一台时间和空间机器。

## 致谢

- **Peter Steinberger** ([@steipete](https://x.com/steipete)) - 创作者，龙虾低语者
- **Mario Zechner** ([@badlogicc](https://x.com/badlogicgames)) - Pi 创作者，安全渗透测试者
- **Clawd** - 要求更好名称的太空龙虾

## 核心贡献者

- **Maxim Vovshin** (@Hyaxia, [36747317+Hyaxia@users.noreply.github.com](mailto:36747317+Hyaxia@users.noreply.github.com)) - Blogwatcher skill
- **Nacho Iacovino** (@nachoiacovino, [nacho.iacovino@gmail.com](mailto:nacho.iacovino@gmail.com)) - 位置解析（Telegram 和 WhatsApp）
- **Vincent Koc** ([@vincentkoc](https://github.com/vincentkoc), [@vincent_koc](https://x.com/vincent_koc)) - 智能体、遥测、钩子、安全

## 许可证

MIT - 像海洋中的龙虾一样自由。

> “我们都只是在玩自己的提示词。”（一位 AI，大概是 token 过量）

## 相关

- [Token 使用量和费用](/zh-CN/reference/token-use)
- [发布策略](/zh-CN/reference/RELEASING)
