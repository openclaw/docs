...
title: 新手引导参考
x-i18n:
    generated_at: "2026-07-12T14:45:34Z"
    model: gpt-5.6
...
如需高层概览，请参阅[新手引导（CLI）](/zh-CN/start/wizard)。如需了解分步
...
<Steps>
  <Step title="重置（可选）">
    - `--reset` 会在设置运行前重置状态；如果不使用该选项，重新运行新手引导时
      会保留现有配置，并将其复用为默认值。
    - `--reset-scope` 控制 `--reset` 移除的内容：`config`（仅配置文件）、
      `config+creds+sessions`（默认）或 `full`（还会移除
      工作区）。
    - 如果配置文件无效，新手引导会停止并提示你先运行
  ...
  <Step title="风险确认">
    - 首次运行（或在设置 `wizard.securityAcknowledgedAt` 前的任何运行）
      会要求你确认已了解智能体功能强大，而完整的
      系统访问权限存在风险。
    - `--non-interactive` 必须显式搭配 `--accept-risk`；如果缺少该选项，
      新手引导会报错退出，而不会显示提示。
    - 交互式运行会显示确认提示，而不是使用该标志；拒绝确认会
      取消设置。

  </Step>
  <Step title="模型/身份验证">
    - **Anthropic API key**：如果存在则使用 `ANTHROPIC_API_KEY`，否则提示输入密钥，然后保存该密钥供守护进程使用。
    - **Anthropic Claude CLI**：如果已存在 Claude CLI 登录，这是首选的本地路径；OpenClaw 仍支持 Anthropic setup-token 身份验证作为替代方案。
    - **OpenAI Code (Codex) subscription (OAuth)**：浏览器流程；粘贴 `code#state`。
      - 在未设置主模型的全新设置中，通过 Codex 运行时将 `agents.defaults.model` 设置为 `openai/gpt-5.6-sol`。
    - **OpenAI Code (Codex) subscription (device pairing)**：使用短期设备代码的浏览器配对流程。
      - 在未设置主模型的全新设置中，通过 Codex 运行时将 `agents.defaults.model` 设置为 `openai/gpt-5.6-sol`。
    - **OpenAI API key**：如果存在则使用 `OPENAI_API_KEY`，否则提示输入密钥，然后将其存储在身份验证配置文件中。
      - 在未设置主模型的全新设置中，将 `agents.defaults.model` 设置为 `openai/gpt-5.6`；不带限定的直接 API 模型 ID 会解析到 Sol 层级。
    - 添加或重新验证 OpenAI 时，会保留现有的显式主模型，包括 `openai/gpt-5.5`。如果账户未提供 GPT-5.6，请显式选择 `openai/gpt-5.5`；OpenClaw 不会静默降级模型。
    - **xAI OAuth**：使用设备代码的浏览器登录，无需 localhost 回调，因此也适用于 SSH/Docker/VPS（`--auth-choice xai-oauth`）。
    - **xAI API key**：提示输入 `XAI_API_KEY`（`--auth-choice xai-api-key`）。
    - `--auth-choice xai-device-code` 仍可作为同一 xAI OAuth 设备代码流程的仅手动兼容别名；新脚本请使用 `xai-oauth`。
    - **OpenCode**：提示输入 `OPENCODE_API_KEY`（或 `OPENCODE_ZEN_API_KEY`，可从 https://opencode.ai/auth 获取），并让你选择 Zen 或 Go 目录。
    - **Ollama**：首先提供 **Cloud + Local**、**Cloud only** 或 **Local only**。`Cloud only` 会提示输入 `OLLAMA_API_KEY` 并使用 `https://ollama.com`；基于主机的模式会提示输入 Ollama 基础 URL（默认 `http://127.0.0.1:11434`），发现可用模型，并在需要时自动拉取选中的本地模型；`Cloud + Local` 还会检查该 Ollama 主机是否已登录以访问云端。
    ...
    - **MiniMax**：自动写入配置；托管模式默认使用 `MiniMax-M3`。
      API 密钥设置使用 `minimax/...`，OAuth 设置使用
      `minimax-portal/...`。
    - 更多详情：[MiniMax](/zh-CN/providers/minimax)
    - **StepFun**：自动为中国或全球端点上的 StepFun 标准版或 Step Plan 写入配置。
    - 标准版当前默认使用 `step-3.5-flash`；Step Plan 还包括 `step-3.5-flash-2603`。
    ...
    - **自定义提供商**：适用于兼容 OpenAI、兼容 OpenAI Responses 或兼容 Anthropic 的端点。非交互式标志：`--auth-choice custom-api-key`、`--custom-base-url`、`--custom-model-id`、`--custom-api-key`（可选；回退到 `CUSTOM_API_KEY`）、`--custom-provider-id`（可选；根据基础 URL 自动派生）、`--custom-compatibility openai|openai-responses|anthropic`（默认 `openai`）、`--custom-image-input` / `--custom-text-input`（覆盖推断出的视觉模型检测结果）。
    - **跳过**：暂不配置身份验证。
    - 从检测到的选项中选择默认模型（或手动输入提供商/模型）。为获得最佳质量并降低提示注入风险，请选择你的提供商栈中可用的最强最新一代模型。
    - 新手引导会运行模型检查，并在配置的模型未知或缺少身份验证时发出警告。
    - API 密钥存储模式默认为明文身份验证配置文件值。使用 `--secret-input-mode ref` 可改为存储由环境变量支持的引用（例如 `keyRef: { source: "env", provider: "default", id: "OPENAI_API_KEY" }`）；引用的环境变量必须已设置，否则新手引导会立即失败。
    ...
    <Note>
    无头环境/服务器提示：在有浏览器的计算机上完成 OAuth，然后将
  ...
  <Step title="工作区">
    ...
    - 植入智能体引导启动流程所需的工作区文件。
    - 完整的工作区布局 + 备份指南：[Agent 工作区](/zh-CN/concepts/agent-workspace)

  </Step>
  <Step title="Gateway 网关">
    - 端口（默认 **18789**）、绑定、身份验证模式、Tailscale 暴露。
    - 身份验证建议：即使对于环回，也保留 **Token**，以便本地 WS 客户端必须进行身份验证。
    - 在令牌模式下，交互式设置提供：
      - **生成/存储明文令牌**（默认）
      - **使用 SecretRef**（选择启用）
      - 快速开始会复用 `env`、`file` 和 `exec` 提供商中的现有 `gateway.auth.token` SecretRef，用于新手引导探测/仪表板引导启动。
      - 如果已配置该 SecretRef 但无法解析，新手引导会提前失败并显示明确的修复消息，而不会静默降低运行时身份验证强度。
    - 在密码模式下，交互式设置同样支持明文或 SecretRef 存储。
    - 非交互式令牌 SecretRef 路径：`--gateway-token-ref-env <ENV_VAR>`。
      - 要求新手引导进程环境中存在非空环境变量。
      - 不能与 `--gateway-token` 组合使用。
    - 仅当你完全信任所有本地进程时才禁用身份验证。
    - 非环回绑定仍需要身份验证。
  ...
  <Step title="渠道">
    ...
    - [Signal](/zh-CN/channels/signal)（插件）：可选的 `signal-cli` 安装 + 账户配置。
    ...
    - 私信安全性：默认为配对。第一条私信会发送代码；通过 `openclaw pairing approve <channel> <code>` 批准，或使用允许列表。
  ...
  <Step title="Web 搜索">
    ...
    - 基于 API 的提供商可使用环境变量或现有配置进行快速设置；无需密钥的提供商则使用各自特定的前置条件。
    - 使用 `--skip-search` 跳过。
  ...
  <Step title="守护进程安装">
    ...
    - 原生 Windows：首先使用 Scheduled Task；如果任务创建被拒绝，OpenClaw 会回退到每用户的 Startup 文件夹登录项，并立即启动 Gateway 网关。
    - **运行时选择：**Node（推荐；WhatsApp/Telegram 必需——Bun 在重新连接时可能导致内存损坏）。交互式模式仅提供 Node；`--daemon-runtime bun` 仅可通过 CLI 使用。
    - 如果令牌身份验证需要令牌，且 `gateway.auth.token` 由 SecretRef 管理，守护进程安装会验证该令牌，但不会将解析出的明文令牌值持久化到监控程序服务的环境元数据中。
    - 如果令牌身份验证需要令牌，而配置的令牌 SecretRef 未解析，守护进程安装会被阻止，并提供可操作的指导。
  ...
  <Step title="健康检查">
    ...
    - 提示：`openclaw status --deep` 会将实时 Gateway 健康探测添加到状态输出中，包括受支持时的渠道探测（需要可访问的 Gateway 网关）。

  </Step>
  <Step title="Skills（推荐）">
    - 读取可用技能并检查要求。
    - 让你选择 Node 管理器：**npm / pnpm / bun**。
    - 为受信任的内置技能自动安装可选依赖项（部分依赖项在 macOS 上使用 Homebrew）。
    - 跳过缺少 Homebrew、uv 或 Go 安装程序前置条件的技能，将它们与手动设置指导归为一组，并在前置条件安装完成后引导你运行 `openclaw doctor`。

  </Step>
  <Step title="完成">
    - 摘要 + 后续步骤，包括用于选择终端、浏览器或稍后处理的 **你希望如何孵化你的智能体？** 提示。
...
如果缺少 Control UI 资源，新手引导会尝试构建它们；回退命令为 `pnpm ui:build`（自动安装 UI 依赖项）。
</Note>

## 非交互式模式

使用 `--non-interactive --accept-risk` 自动化新手引导或编写脚本（该
标志是必需的风险确认；如果没有该标志，新手引导会
报错退出）：
...
<Note>
`--json` **不**表示非交互模式。用于脚本时，请使用 `--non-interactive --accept-risk`（以及 `--workspace`）。
</Note>
...
- `tools.profile`（本地新手引导会在未设置时默认为 `"coding"`；已有的显式值会予以保留）
- `gateway.*`（模式、绑定、身份验证、Tailscale）
- `session.dmScope`（本地新手引导会在未设置时将其默认为 `"per-channel-peer"`；已有的显式值会予以保留。详情：[CLI 设置参考](/zh-CN/start/wizard-cli-reference#outputs-and-internals)）
- `channels.telegram.botToken`、`channels.discord.token`、`channels.matrix.*`、`channels.signal.*`、`channels.imessage.*`
- 在渠道提示中选择启用时，会写入渠道私信允许列表。Discord、Matrix、Microsoft Teams 和 Slack 会尽可能将名称解析为 ID；其他渠道直接使用 ID（例如数字形式的 Telegram 发送者 ID 或 WhatsApp 电话号码）。
...
部分渠道以插件形式提供。在设置期间选择此类渠道时，新手引导会先提示你安装该插件（通过 npm 或本地路径），之后才能进行配置。
