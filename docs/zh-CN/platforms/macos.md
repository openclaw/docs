...
title: macOS 应用
x-i18n:
    generated_at: "2026-05-05T23:38:19Z"
    model: gpt-5.5
    provider: openai
...
- **本地**（默认）：如果已有正在运行的本地 Gateway 网关，应用会附加到它；
  否则会通过 `openclaw gateway install` 启用 launchd 服务。
- **远程**：应用通过 SSH/Tailscale 连接到 Gateway 网关，并且绝不启动
  本地进程。
  应用会启动本地**节点主机服务**，让远程 Gateway 网关能够访问这台 Mac。
  应用不会把 Gateway 网关作为子进程生成。
...
运行具名配置文件时，将标签替换为 `ai.openclaw.<profile>`。

如果 LaunchAgent 尚未安装，请从应用启用它，或运行
...
节点会报告一个 `permissions` 映射，让智能体可以判断允许哪些操作。

节点服务 + 应用 IPC：

- 无头节点主机服务运行时（远程模式），它会作为一个节点连接到 Gateway 网关 WS。
- `system.run` 通过本地 Unix 套接字在 macOS 应用（UI/TCC 上下文）中执行；提示 + 输出会保留在应用内。
...
- `allowlist` 条目是已解析二进制路径的 glob 模式，或用于通过 PATH 调用的命令的裸命令名。
- 包含 shell 控制或展开语法（`&&`、`||`、`;`、`|`、`` ` ``、`$`、`<`、`>`、`(`、`)`）的原始 shell 命令文本会被视为允许列表未命中，并需要显式批准（或将 shell 二进制加入允许列表）。
- 在提示中选择“始终允许”会将该命令添加到允许列表。
- `system.run` 环境覆盖会经过过滤（丢弃 `PATH`、`DYLD_*`、`LD_*`、`NODE_OPTIONS`、`PYTHON*`、`PERL*`、`RUBYOPT`、`SHELLOPTS`、`PS4`），然后与应用的环境合并。
- 对于 shell 包装器（`bash|sh|zsh ... -c/-lc`），请求范围内的环境覆盖会缩减为一个小型显式允许列表（`TERM`、`LANG`、`LC_*`、`COLORTERM`、`NO_COLOR`、`FORCE_COLOR`）。
- 在允许列表模式中，对于始终允许的决策，已知分发包装器（`env`、`nice`、`nohup`、`stdbuf`、`timeout`）会持久化内部可执行文件路径，而不是包装器路径。如果无法安全解包，则不会自动持久化任何允许列表条目。
...
- 没有 `key` 时，应用会提示确认。
- 没有 `key` 时，应用会对确认提示实施较短的消息长度限制，并忽略 `deliver` / `to` / `channel`。
- 使用有效的 `key` 时，运行是无人值守的（面向个人自动化）。
...
带同步支持的路径可能会增加延迟，并偶尔导致会话和凭证出现文件锁/同步竞争。
...
<Tip>
与 `openclaw gateway discover --json` 比较，查看 macOS 应用的发现管线（`local.` 加上已配置的广域域名，并带有广域和 Tailscale Serve 回退）是否不同于 Node CLI 基于 `dns-sd` 的发现。
</Tip>

## 远程连接管线（SSH 隧道）

当 macOS 应用以**远程**模式运行时，它会打开一条 SSH 隧道，使本地 UI
组件可以像访问 localhost 上的 Gateway 网关一样与远程 Gateway 网关通信。

### 控制隧道（Gateway 网关 WebSocket 端口）

- **用途：** 健康检查、状态、Web Chat、配置，以及其他控制平面调用。
...
- **IP 报告：** SSH 隧道使用 loopback，因此 Gateway 网关会将节点
  IP 视为 `127.0.0.1`。如果你希望显示真实客户端
  IP，请使用 **Direct (ws/wss)** 传输（参见 [macOS 远程访问](/zh-CN/platforms/mac/remote)）。

有关设置步骤，请参阅 [macOS 远程访问](/zh-CN/platforms/mac/remote)。有关协议
详情，请参阅 [Gateway 网关协议](/zh-CN/gateway/protocol)。
