...
<Warning>
Anthropic 员工告诉我们，OpenClaw 风格的 Claude CLI 使用方式已重新允许，因此
...
- [在你的 Pro 或 Max 方案中使用 Claude Code](https://support.claude.com/en/articles/11145838-using-claude-code-with-your-pro-or-max-plan)
- [在你的 Team 或 Enterprise 方案中使用 Claude Code](https://support.anthropic.com/en/articles/11845131-using-claude-code-with-your-team-or-enterprise-plan/)
...
<Tabs>
  ...
  <Tab title="Claude CLI">
    **适合：** 在没有单独 API 密钥的情况下复用现有 Claude CLI 登录。
    ...
    优先使用规范的 Anthropic 模型引用，再加上 CLI 运行时覆盖：
    ...
    兼容性，但新的配置应将提供商/模型选择保留为
    `anthropic/*`，并将执行后端放入 `agentRuntime.id`。
...
在 OpenClaw 中，如果未设置显式思考级别，Claude 4.6 模型默认使用 `adaptive` 思考。

可使用 `/think:<level>` 按消息覆盖，或在模型参数中覆盖：
...
<AccordionGroup>
  ...
  <Accordion title="媒体理解（图片和 PDF）">
    内置 Anthropic 插件会注册图片和 PDF 理解。OpenClaw
