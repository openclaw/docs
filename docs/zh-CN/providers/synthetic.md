...
title: Synthetic
x-i18n:
    generated_at: "2026-04-05T10:07:01Z"
    model: gpt-5.4
    provider: openai
...
Synthetic 提供 Anthropic 兼容端点。OpenClaw 将其注册为
...
注意：OpenClaw 的 Anthropic 客户端会在 base URL 后附加 `/v1`，因此请使用
...
- 如果你启用了模型 allowlist（`agents.defaults.models`），请添加你
  计划使用的每一个模型。
- 提供商规则请参见[模型提供商](/zh-CN/concepts/model-providers)。
