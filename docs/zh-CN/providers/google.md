...
用于运营方代理或兼容的 Gemini API 端点；省略时，Gemini Web 搜索会复用 `models.providers.google.baseUrl`。参见
...
- 输出格式：默认是 `mp3`，在 `google/lyria-3-pro-preview` 上还支持 `wav`
- 引用输入：最多 10 张图像
...
例如 `[whispers]` 或 `[laughs]`。要在将标签发送到 TTS 的同时让它们不出现在可见的聊天回复中，请把它们放入 `[[tts:text]]...[[/tts:text]]`
块中：
...
保持未设置；OpenClaw 会省略非正值，因为 Google Live 对 `temperature: 0`
可能返回没有音频的转录。
