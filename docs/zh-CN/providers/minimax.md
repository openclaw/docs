...
title: MiniMax
x-i18n:
    generated_at: "2026-06-27T03:05:55Z"
    model: gpt-5.5
...
<Tabs>
  <Tab title="OAuth（Coding Plan）">
    **适合：** 通过 OAuth 快速设置 MiniMax Coding Plan，无需 API key。
  ...
  <Tab title="API key">
    **适合：** 使用 Anthropic 兼容 API 的托管 MiniMax。
    ...
    <Warning>
    在 Anthropic 兼容的流式传输路径上，除非你显式设置 `thinking`，否则 OpenClaw 默认会禁用 MiniMax M2.x 思考。M2.x 的流式传输端点会以 OpenAI 风格的 delta 分块发出 `reasoning_content`，而不是原生 Anthropic thinking 块；如果隐式启用，可能会把内部推理泄露到可见输出中。MiniMax-M3（以及前向兼容的 M3.x）不受此默认值影响：M3 会发出正确的 Anthropic thinking 块，并且需要启用 thinking 才能生成可见内容，因此 OpenClaw 会让 M3 继续使用提供商省略/自适应的 thinking 路径。
    </Warning>

    <Note>
    API-key 设置使用 `minimax` 提供商 ID。模型引用遵循 `minimax/MiniMax-M3` 形式。
    </Note>
...
若要使用 MiniMax 进行图像生成，请将其设为图像生成提供商：
...
插件使用与文本模型相同的 `MINIMAX_API_KEY` 或 OAuth 凭证。如果 MiniMax 已经设置好，则无需额外配置。

`minimax` 和 `minimax-portal` 都会使用相同的 `image-01` 模型注册 `image_generate`。API-key 设置使用 `MINIMAX_API_KEY`；OAuth 设置则可改用内置的 `minimax-portal` 凭证路径。

图像生成始终使用 MiniMax 专用图像端点（`/v1/image_generation`），并忽略 `models.providers.minimax.baseUrl`，因为该字段用于配置聊天/Anthropic 兼容的基础 URL。设置 `MINIMAX_API_HOST=https://api.minimaxi.com` 可将图像生成路由到中国端点；默认全局端点是 `https://api.minimax.io`。

当新手引导或 API-key 设置写入显式 `models.providers.minimax` 条目时，OpenClaw 会将 `MiniMax-M3`、`MiniMax-M2.7` 和 `MiniMax-M2.7-highspeed` 实体化为聊天模型。M3 声明支持文本和图像输入；图像理解仍通过插件拥有的 `MiniMax-VL-01` 媒体提供商单独公开。
...
内置 `minimax` 插件会将 MiniMax T2A v2 注册为 `messages.tts` 的语音提供商。
...
- 凭证解析顺序为 `messages.tts.providers.minimax.apiKey`，然后是 `minimax-portal` OAuth/token 凭证配置，然后是 Token Plan 环境键（`MINIMAX_OAUTH_TOKEN`、`MINIMAX_CODE_PLAN_KEY`、`MINIMAX_CODING_API_KEY`），最后是 `MINIMAX_API_KEY`。
- 如果未配置 TTS 主机，OpenClaw 会复用已配置的 `minimax-portal` OAuth 主机，并去掉 Anthropic 兼容路径后缀，例如 `/anthropic`。
- 普通音频附件保持 MP3。
- Feishu 和 Telegram 等语音笔记目标会通过 `ffmpeg` 从 MiniMax MP3 转码为 48kHz Opus，因为 Feishu/Lark 文件 API 对原生音频消息只接受 `file_type: "opus"`。
- MiniMax T2A 接受小数形式的 `speed` 和 `vol`，但 `pitch` 会作为整数发送；OpenClaw 会在 API 请求前截断小数 `pitch` 值。
...
| `messages.tts.providers.minimax.speakerVoiceId` | `MINIMAX_TTS_VOICE_ID` | `English_expressive_narrator` | 用于语音输出的声音 ID。 |
...
因此，即使内置文本提供商目录也包含支持 M3 图像的聊天引用，自动媒体路由也可以使用 MiniMax 图像理解。

### Web 搜索

MiniMax 插件还会通过 MiniMax Token Plan
...
- 搜索保持在提供商 id `minimax` 上；OAuth CN/global 设置可以通过 `models.providers.minimax-portal.baseUrl` 间接引导区域，并可通过 `MINIMAX_OAUTH_TOKEN` 提供 bearer 认证
...
<AccordionGroup>
  ...
  <Accordion title="思考默认值">
    在 `api: "anthropic-messages"` 上，除非 params/config 中已显式设置 thinking，否则 OpenClaw 会为 MiniMax M2.x 模型注入 `thinking: { type: "disabled" }`。

    这会阻止 M2.x 的流式端点在 OpenAI 风格的 delta 分块中发出 `reasoning_content`，避免内部推理泄漏到可见输出中。

    MiniMax-M3（以及 M3.x）不受此限制：M3 会发出正确的 Anthropic thinking 块，并在 thinking 被禁用时返回带有 `stop_reason: "end_turn"` 的空 `content` 数组，因此包装器会让 M3 保持在提供商省略/自适应 thinking 路径上。
  ...
  <Accordion title="回退示例">
    **最适合：** 将你最强的最新一代模型保留为主模型，并故障转移到 MiniMax M2.7。下面的示例使用 Opus 作为具体主模型；请替换为你偏好的最新一代主模型。
  ...
  <Accordion title="Coding Plan 用法详情">
    ...
    - 配置后，用量轮询会从 `models.providers.minimax-portal.baseUrl` 或 `models.providers.minimax.baseUrl` 派生主机，因此使用 `https://api.minimax.io/anthropic` 的 global 设置会轮询 `api.minimax.io`。缺失或格式错误的基础 URL 会保留 CN 回退以保持兼容性。
    - OpenClaw 会将 MiniMax coding-plan 用量规范化为其他提供商使用的相同 `% left` 显示。MiniMax 原始的 `usage_percent` / `usagePercent` 字段表示剩余额度，而不是已消耗额度，因此 OpenClaw 会将其取反。存在基于计数的字段时，以这些字段为准。
    - 当 API 返回 `model_remains` 时，OpenClaw 会优先使用聊天模型条目，在需要时从 `start_time` / `end_time` 派生窗口标签，并在计划标签中包含所选模型名称，以便更容易区分 coding-plan 窗口。
...
- 默认聊天模型：`MiniMax-M3`
- 备用聊天模型：`MiniMax-M2.7`、`MiniMax-M2.7-highspeed`
- 新手引导和直接 API-key 设置会为 M3 和两个 M2.7 变体写入模型定义
...
<AccordionGroup>
  <Accordion title='"未知模型：minimax/MiniMax-M3"'>
    这通常表示 **MiniMax 提供商未配置**（未找到匹配的提供商条目，也未找到 MiniMax 认证配置文件/环境变量 key）。此检测的修复包含在 **2026.1.12** 中。修复方式：
