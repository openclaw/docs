...
SGLang 通过 OpenAI 兼容的 HTTP API 提供开放权重模型。OpenClaw 使用 `openai-completions` 提供商家族连接到 SGLang，并自动发现可用模型。

| 属性                      | 值                                                           |
| ------------------------- | ------------------------------------------------------------ |
| 提供商 id                 | `sglang`                                                     |
| 插件                      | 内置，`enabledByDefault: true`                               |
| 凭证环境变量              | `SGLANG_API_KEY`（如果服务器没有凭证，使用任何非空值）       |
| 新手引导标志              | `--auth-choice sglang`                                       |
| API                       | OpenAI 兼容（`openai-completions`）                          |
| 默认基础 URL              | `http://127.0.0.1:30000/v1`                                  |
| 默认模型占位符            | `sglang/Qwen/Qwen3-8B`                                       |
| 流式使用量                | 是（`supportsStreamingUsage: true`）                         |
| 计价                      | 标记为外部免费（`modelPricing.external: false`）             |

当你使用 `SGLANG_API_KEY` 选择启用时，OpenClaw 还会从 SGLang **自动发现**可用模型。配置自定义 SGLang 基础 URL 时，也可以在 `agents.defaults.models` 中使用 `sglang/*` 来保持动态发现。请参阅下方的 [模型发现（隐式提供商）](#model-discovery-implicit-provider)。

## 入门指南

<Steps>
  <Step title="启动 SGLang">
    使用 OpenAI 兼容服务器启动 SGLang。你的基础 URL 应公开
...
当设置了 `SGLANG_API_KEY`（或存在凭证配置文件）且你**没有**
...
<Note>
如果你显式设置 `models.providers.sglang`，OpenClaw 默认会使用你声明的
模型。当你希望 OpenClaw 查询该已配置提供商的 `/models` 端点并包含
所有公布的 SGLang 模型时，请将 `"sglang/*": {}` 添加到 `agents.defaults.models`。
</Note>
...
<AccordionGroup>
  <Accordion title="代理式行为">
    SGLang 被视为代理式的 OpenAI 兼容 `/v1` 后端，而不是
    原生 OpenAI 端点。
  ...
  <Accordion title="故障排查">
    ...
    **凭证错误**
    ...
    如果请求因凭证错误而失败，请设置与
    你的服务器配置匹配的真实 `SGLANG_API_KEY`，或在
    `models.providers.sglang` 下显式配置提供商。

    <Tip>
    如果你运行的 SGLang 未启用身份验证，`SGLANG_API_KEY`
    的任何非空值都足以选择启用模型发现。
    </Tip>
