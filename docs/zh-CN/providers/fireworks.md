...
read_when:
    ...
    - 你正在调试 Fireworks 上 Kimi 关闭思考的行为
summary: Fireworks 设置（凭证 + 模型选择）
title: 烟花
x-i18n:
    generated_at: "2026-05-05T23:50:52Z"
    model: gpt-5.5
    provider: openai
...
<Note>
  OpenClaw 将所有 Fireworks Kimi 模型固定为 `thinking: off`，因为 Fireworks 会在生产环境中拒绝 Kimi 思考参数。直接通过 [Moonshot](/zh-CN/providers/moonshot) 路由同一模型会保留 Kimi 推理输出。请参阅[思考模式](/zh-CN/tools/thinking)，了解如何在提供商之间切换。
</Note>
...
<AccordionGroup>
  ...
  <Accordion title="为什么对 Kimi 强制关闭思考">
  ...
  <Accordion title="守护进程的环境可用性">
    ...
    <Warning>
      仅放在 `~/.profile` 中的密钥不会帮助 launchd 或 systemd 守护进程，除非该环境也导入到了那里。请在 `~/.openclaw/.env` 中设置密钥，或通过 `env.shellEnv` 设置密钥，使 Gateway 网关进程能够读取它。
    </Warning>
