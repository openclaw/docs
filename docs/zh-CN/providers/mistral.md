...
summary: 通过 OpenClaw 使用 Mistral 模型和 Voxtral 转写
title: Mistral
x-i18n:
...
| 嵌入模型         | `mistral-embed`                             |
| Voxtral 批量处理 | `voxtral-mini-latest`（音频转录）           |
...
| `mistral/mistral-small-latest`   | 文本、图像   | 262,144 | 16,384   | 最新版 Mistral Small 4；可调整 `reasoning_effort`        |
| `mistral/mistral-small-2603`     | 文本、图像   | 262,144 | 16,384   | 固定版本 Mistral Small 4；可调整 `reasoning_effort`      |
...
<Note>
OpenClaw 将 Mistral 实时 STT 默认设置为 8 kHz 的 `pcm_mulaw`，以便 Voice Call 可以直接转发 Twilio 媒体帧。仅当你的上游流已经是原始 PCM 时，才使用 `encoding: "pcm_s16le"` 和匹配的 `sampleRate`。
</Note>
...
<AccordionGroup>
  <Accordion title="可调推理">
    `mistral/mistral-small-latest`、`mistral/mistral-small-2603` 和 `mistral/mistral-medium-3-5` 支持通过 `reasoning_effort` 在 Chat Completions API 上使用[可调推理](https://docs.mistral.ai/studio-api/conversations/reasoning/adjustable)（`none` 会最大限度减少输出中的额外思考；`high` 会在最终答案之前显示完整的思考轨迹）。

    OpenClaw 将会话的 **thinking** 级别映射到 Mistral API：
    ...
    <Warning>
    避免将 Medium 3.5 推理模式与 `temperature: 0` 结合使用；据报告，Mistral HTTP API 会以 400 响应拒绝同时使用 `reasoning_effort="high"` 和 `temperature: 0` 的请求。保持 temperature 未设置，或者关闭 thinking 或将其设为 minimal，让 OpenClaw 发送 `reasoning_effort: "none"`，然后再设置较低的 temperature。
    </Warning>
    ...
    <Note>
    其他内置的 Mistral 目录模型不使用此参数。如果你需要 Mistral 原生的推理优先行为，请继续使用 `magistral-*` 模型。
    </Note>
  ...
  <Accordion title="身份验证和基础 URL">
    ...
    - 提供商基础 URL 默认为 `https://api.mistral.ai/v1`，并接受标准的 OpenAI 兼容聊天补全请求格式。
