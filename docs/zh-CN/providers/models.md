...
read_when:
    - 你想选择一个模型提供商
    - 你想要 LLM 凭证和模型选择的快速设置示例
summary: OpenClaw 支持的模型提供商（LLM）
title: 模型提供商快速开始
x-i18n:
    generated_at: "2026-06-27T03:06:10Z"
    model: gpt-5.5
...
OpenClaw 可以使用许多大语言模型提供商。选择一个、完成身份验证，然后将默认
...
- `anthropic-vertex` - 安装 `@openclaw/anthropic-vertex-provider`，在 Vertex 凭证可用时支持 Google Vertex 上的隐式 Anthropic；没有单独的新手引导身份验证选项
...
如需完整提供商目录（xAI、Groq、Mistral 等）和高级配置，
