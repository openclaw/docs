...
title: OpenAI
x-i18n:
    generated_at: "2026-04-21T01:47:06Z"
    model: gpt-5.4
    provider: openai
...
- **API 密钥** — 直接访问 OpenAI Platform，并按量计费（`openai/*` 模型）
...
<Tabs>
    ...
    **最适合：** 直接 API 访问和按量计费。
    ...
    <Steps>
      <Step title="获取你的 API 密钥">
        在 [OpenAI Platform 控制台](https://platform.openai.com/api-keys) 创建或复制 API 密钥。
      </Step>
      <Step title="运行新手引导">
        ```bash
  ...
  <Tab title="Codex 订阅">
    **最适合：** 使用你的 ChatGPT/Codex 订阅，而不是单独的 API 密钥。Codex 云端需要使用 ChatGPT 登录。
    ...
    <Note>
    这一路径与 `openai/gpt-5.4` 是有意分开的。要直接访问 Platform，请将 API 密钥用于 `openai/*`；要使用 Codex 订阅访问，请使用 `openai-codex/*`。
    </Note>
    ...
    <Tip>
    如果新手引导复用了现有的 Codex CLI 登录，这些凭证会继续由 Codex CLI 管理。过期后，OpenClaw 会先重新读取外部 Codex 来源，再将刷新后的凭证写回 Codex 存储。
    </Tip>
...
<Note>
有关共享工具参数、提供商选择和故障切换行为，参见 [图像生成](/zh-CN/tools/image-generation)。
</Note>
...
<Note>
有关共享工具参数、提供商选择和故障切换行为，参见 [视频生成](/zh-CN/tools/video-generation)。
</Note>

## GPT-5 提示词贡献

OpenClaw 会为 `openai/*` 和 `openai-codex/*` 的 GPT-5 系列运行添加一个 OpenAI 专用的 GPT-5 提示词贡献。它位于内置的 OpenAI 插件中，适用于 `gpt-5`、`gpt-5.2`、`gpt-5.4` 和 `gpt-5.4-mini` 等模型 id，不适用于较早的 GPT-4.x 模型。

GPT-5 提示词贡献默认会添加一组带标签的行为约定，用于约束输出形态、工具持续执行、依赖检查、并行查找、完成检查、验证和自主性。对于匹配的 GPT-5 模型，这些指导始终启用。友好交互风格层是独立的，并且可配置。

| Value                  | Effect                         |
| ---------------------- | ------------------------------ |
| `"friendly"` (default) | 启用友好交互风格层             |
| `"on"`                 | `"friendly"` 的别名            |
| `"off"`                | 仅禁用友好风格层               |
...
<AccordionGroup>
  ...
  <Accordion title="实时转写">
    内置的 `openai` 插件为 Voice Call 插件注册实时转写功能。
    ...
    <Note>
    使用 WebSocket 连接到 `wss://api.openai.com/v1/realtime`，并使用 G.711 u-law 音频。
    </Note>
...
<AccordionGroup>
  <Accordion title="传输方式（WebSocket 与 SSE）">
    对于 `openai/*` 和 `openai-codex/*`，OpenClaw 都默认优先使用 WebSocket，并在失败时回退到 SSE（`"auto"`）。
    ...
    | Value | Behavior |
    |-------|----------|
    | `"auto"` (default) | 优先 WebSocket，失败时回退到 SSE |
    | `"sse"` | 仅强制使用 SSE |
  ...
  <Accordion title="WebSocket 预热">
    对于 `openai/*`，OpenClaw 默认启用 WebSocket 预热，以减少首轮延迟。
  ...
  <Accordion title="快速模式">
    ...
    启用后，OpenClaw 会将快速模式映射为 OpenAI 的优先处理（`service_tier = "priority"`）。现有的 `service_tier` 值会被保留，快速模式也不会重写 `reasoning` 或 `text.verbosity`。
  ...
  <Accordion title="严格智能体 GPT 模式">
    对于 `openai/*` 和 `openai-codex/*` 上的 GPT-5 系列运行，OpenClaw 可以使用更严格的嵌入式执行约定：
    ...
    - 当工具操作可用时，不再将仅规划的轮次视为成功推进
    - 使用“立即行动”的引导重试该轮次
    - 对于较大的工作自动启用 `update_plan`
    - 如果模型持续只做规划而不行动，则显式显示阻塞状态

    <Note>
    仅适用于 OpenAI 和 Codex 的 GPT-5 系列运行。其他提供商和较旧的模型系列仍保持默认行为。
    </Note>

  </Accordion>

  <Accordion title="原生路由与 OpenAI-compatible 路由">
    OpenClaw 会将直接 OpenAI、Codex 和 Azure OpenAI 端点，与通用的 OpenAI-compatible `/v1` 代理区别对待：

    **原生路由**（`openai/*`、`openai-codex/*`、Azure OpenAI）：
    - 当显式禁用推理时，保留 `reasoning: { effort: "none" }`
    - 默认将工具 schema 设为严格模式
    - 仅在已验证的原生主机上附加隐藏归因头
