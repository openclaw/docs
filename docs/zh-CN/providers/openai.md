...
title: OpenAI
x-i18n:
    generated_at: "2026-04-25T17:31:03Z"
    model: gpt-5.4
    provider: openai
...
OpenAI 为 GPT 模型提供开发者 API。OpenClaw 支持三种 OpenAI 系列路径。模型前缀用于选择路径：
...
provider、模型、运行时和渠道是彼此独立的层。如果你把这些标签混在一起了，请先阅读 [Agent Runtimes](/zh-CN/concepts/agent-runtimes)，再修改配置。
...
| 使用 ChatGPT/Codex 订阅认证的 GPT-5.5 | `openai-codex/gpt-5.5` | 用于 Codex OAuth 的默认 PI 路径。是订阅方案的首选。 |
...
<Note>
GPT-5.5 同时可通过直接的 OpenAI Platform API 密钥访问和订阅/OAuth 路径获得。对于直接的 `OPENAI_API_KEY` 流量，请使用 `openai/gpt-5.5`；对于通过 PI 的 Codex OAuth，请使用 `openai-codex/gpt-5.5`；对于原生 Codex app-server harness，请使用带有 `embeddedHarness.runtime: "codex"` 的 `openai/gpt-5.5`。
</Note>
...
| 服务器端网页搜索 | 原生 OpenAI Responses 工具 | 是，在启用 web search 且未固定 provider 时 |
| 图像 | `image_generate` | 是 |
| 视频 | `video_generate` | 是 |
...
<Tabs>
  <Tab title="API 密钥（OpenAI Platform）">
    **最适合：** 直接访问 API 和按量计费。
    ...
    <Note>
    `openai/*` 是直接的 OpenAI API 密钥路径，除非你显式强制使用 Codex app-server harness。对于通过默认 PI 运行器的 Codex OAuth，请使用 `openai-codex/*`；对于原生 Codex app-server 执行，请使用带 `embeddedHarness.runtime: "codex"` 的 `openai/gpt-5.5`。
    </Note>
    ...
    <Warning>
    OpenClaw **不会**公开 `openai/gpt-5.3-codex-spark`。实时 OpenAI API 请求会拒绝该模型，而当前的 Codex 目录也不会公开它。
    </Warning>
    ...
    **最适合：** 使用你的 ChatGPT/Codex 订阅，而不是单独的 API 密钥。Codex cloud 需要 ChatGPT 登录。
    ...
    <Steps>
      <Step title="运行 Codex OAuth">
        ...
        对于无头环境或不适合回调的环境，可添加 `--device-code`，使用 ChatGPT 设备码流程登录，而不是 localhost 浏览器回调：
    ...
    <Note>
    对于认证/profile 命令，请继续使用 `openai-codex` provider id。`openai-codex/*` 模型前缀同时也是用于 Codex OAuth 的显式 PI 路径。它不会选择或自动启用内置的 Codex app-server harness。
    </Note>
    ...
    <Note>
    新手引导不再从 `~/.codex` 导入 OAuth 资料。请使用浏览器 OAuth（默认）或上述设备码流程登录 —— OpenClaw 会在自己的智能体认证存储中管理生成的凭证。
    </Note>

    ### Status 指示器

    聊天中的 `/status` 会显示当前会话正在使用的模型运行时。默认的 PI harness 显示为 `Runtime: OpenClaw Pi Default`。当选择内置的 Codex app-server harness 时，`/status` 会显示 `Runtime: OpenAI Codex`。已有会话会保留其记录的 harness id，因此如果你修改了 `embeddedHarness` 并希望 `/status` 反映新的 PI/Codex 选择，请使用 `/new` 或 `/reset`。
    ...
    较小的默认上限在实践中通常具有更好的延迟和质量表现。你可以使用 `contextTokens` 覆盖它：
    ...
    当上游 Codex 目录中存在 `gpt-5.5` 元数据时，OpenClaw 会使用它。如果实时 Codex 发现遗漏了 `openai-codex/gpt-5.5` 这一行，而账户又已通过认证，OpenClaw 会合成这一 OAuth 模型条目，这样 cron、子智能体和已配置的默认模型运行就不会因 `Unknown model` 而失败。
...
它支持通过同一个 `openai/gpt-image-2` 模型引用进行基于 OpenAI API 密钥的图像生成，以及通过 Codex OAuth 的图像生成。
...
<Note>
共享工具参数、provider 选择和故障切换行为，请参见 [图像生成](/zh-CN/tools/image-generation)。
</Note>

`gpt-image-2` 是 OpenAI 文生图和图像编辑的默认模型。`gpt-image-1` 仍可作为显式模型覆盖继续使用，但新的 OpenAI 图像工作流应使用 `openai/gpt-image-2`。

对于 Codex OAuth 安装，请继续使用相同的 `openai/gpt-image-2` 引用。当配置了 `openai-codex` OAuth profile 时，OpenClaw 会解析该已存储的 OAuth 访问令牌，并通过 Codex Responses 后端发送图像请求。它不会先尝试 `OPENAI_API_KEY`，也不会在该请求上悄悄回退到 API 密钥。只有当你希望走直接 OpenAI Images API 路径时，才应显式配置带有 API 密钥、自定义 base URL 或 Azure 端点的 `models.providers.openai`。
如果该自定义图像端点位于受信任的局域网/私有地址上，还需要设置 `browser.ssrfPolicy.dangerouslyAllowPrivateNetwork: true`；若没有此显式选择加入，OpenClaw 会继续阻止私有/内部的 OpenAI 兼容图像端点。

生成：

```
/tool image_generate model=openai/gpt-image-2 prompt="一张为 macOS 上的 OpenClaw 制作的精美发布海报" size=3840x2160 count=1
```

编辑：

```
/tool image_generate model=openai/gpt-image-2 prompt="保留对象形状，将材质改为半透明玻璃" image=/path/to/reference.png size=1024x1536
```
...
<Note>
共享工具参数、provider 选择和故障切换行为，请参见 [视频生成](/zh-CN/tools/video-generation)。
</Note>

## GPT-5 提示词贡献

OpenClaw 会为跨 provider 的 GPT-5 系列运行添加一层共享的 GPT-5 提示词贡献。它按模型 id 生效，因此 `openai-codex/gpt-5.5`、`openai/gpt-5.5`、`openrouter/openai/gpt-5.5`、`opencode/gpt-5.5` 以及其他兼容的 GPT-5 引用都会收到相同的覆盖层。较旧的 GPT-4.x 模型则不会。

内置的原生 Codex harness 会通过 Codex app-server developer instructions 使用相同的 GPT-5 行为和心跳覆盖层，因此即使 `openai/gpt-5.x` 会话通过 `embeddedHarness.runtime: "codex"` 被强制走 Codex，仍会保持相同的后续跟进和主动心跳指导，尽管其余 harness 提示词由 Codex 接管。

GPT-5 贡献为 persona 持久性、执行安全、工具纪律、输出形态、完成检查和验证添加了一份带标签的行为契约。渠道特定的回复和静默消息行为仍保留在共享的 OpenClaw 系统提示词与出站投递策略中。对于匹配的模型，GPT-5 指导始终启用。友好型交互风格层则是独立且可配置的。

| 值 | 效果 |
| ---------------------- | ------------------------------------------- |
| `"friendly"`（默认） | 启用友好型交互风格层 |
| `"on"` | `"friendly"` 的别名 |
| `"off"` | 仅禁用友好风格层 |
...
<AccordionGroup>
  ...
  <Accordion title="语音转文本">
    ...
    - 在 OpenClaw 中，只要入站音频转录使用
      `tools.media.audio`，就会受到支持，包括 Discord 语音渠道片段和渠道
      音频附件
    ...
    当共享音频媒体配置或按次调用的转录请求提供了语言和提示词提示时，这些内容会被转发给 OpenAI。
  ...
  <Accordion title="实时转录">
    ...
    <Note>
    使用到 `wss://api.openai.com/v1/realtime` 的 WebSocket 连接，并采用 G.711 u-law（`g711_ulaw` / `audio/pcmu`）音频。该流式 provider 用于 Voice Call 的实时转录路径；Discord 语音目前则记录短片段，并改用批量 `tools.media.audio` 转录路径。
    </Note>
...
内置的 `openai` provider 可通过覆盖 base URL，将图像生成请求发送到 Azure OpenAI 资源。在图像生成路径上，OpenClaw 会检测 `models.providers.openai.baseUrl` 上的 Azure 主机名，并自动切换到 Azure 的请求格式。

<Note>
实时语音使用单独的配置路径
（`plugins.entries.voice-call.config.realtime.providers.openai.azureEndpoint`），
不受 `models.providers.openai.baseUrl` 影响。其 Azure 设置请参见 [语音与语音识别](#voice-and-speech) 下的 **实时语音** 手风琴部分。
</Note>
...
如需通过内置 `openai` provider 使用 Azure 图像生成，请将
...
其他 base URL（公共 OpenAI、OpenAI 兼容代理）会继续使用标准的 OpenAI 图像请求格式。

<Note>
`openai` provider 的图像生成路径要支持 Azure 路由，需要 OpenClaw 2026.4.22 或更高版本。更早版本会将任意自定义 `openai.baseUrl` 都当作公共 OpenAI 端点处理，因此在 Azure 图像 deployment 上会失败。
</Note>

### API 版本

设置 `AZURE_OPENAI_API_VERSION` 可为 Azure 图像生成路径固定特定的 Azure 预览版或正式版版本：
...
Azure OpenAI 会将模型绑定到 deployment。对于通过内置 `openai` provider 路由的 Azure 图像生成请求，OpenClaw 中的 `model` 字段必须是你在 Azure 门户中配置的 **Azure deployment 名称**，而不是公共的 OpenAI 模型 id。
...
`uaenorth`）。在创建 deployment 之前，请先检查 Microsoft 当前的区域列表，并确认你的区域提供了对应模型。

### 参数差异

Azure OpenAI 和公共 OpenAI 并不总是接受相同的图像参数。Azure 可能会拒绝公共 OpenAI 允许的选项（例如 `gpt-image-2` 上某些 `background` 值），或者只在特定模型版本上公开这些选项。这些差异来自 Azure 和底层模型，而不是 OpenClaw。如果 Azure 请求因验证错误而失败，请在 Azure 门户中检查你的特定 deployment 和 API 版本所支持的参数集。

<Note>
Azure OpenAI 使用原生传输和兼容行为，但不会接收 OpenClaw 的隐藏归因头 —— 请参见下方 [高级配置](#advanced-configuration) 中的 **原生与 OpenAI 兼容路径** 手风琴部分。

对于 Azure 上的聊天或 Responses 流量（图像生成之外），请使用新手引导流程或专用的 Azure provider 配置 —— 仅设置 `openai.baseUrl` 并不会采用 Azure 的 API/认证格式。另有一个单独的 `azure-openai-responses/*` provider；请参见下方的“服务器端压缩”手风琴部分。
</Note>
...
<AccordionGroup>
  <Accordion title="传输（WebSocket 与 SSE）">
    OpenClaw 对 `openai/*` 和 `openai-codex/*` 都采用 WebSocket 优先、SSE 回退（`"auto"`）的策略。
  ...
  <Accordion title="优先处理（service_tier）">
    OpenAI 的 API 通过 `service_tier` 公开优先处理能力。你可以在 OpenClaw 中按模型设置它：
  ...
  <Accordion title="服务器端压缩（Responses API）">
    ...
    这既适用于内置的 Pi harness 路径，也适用于嵌入式运行使用的 OpenAI provider 钩子。原生 Codex app-server harness 会通过 Codex 自行管理上下文，并通过 `agents.defaults.embeddedHarness.runtime` 单独配置。
    ...
    <Note>
    `responsesServerCompaction` 仅控制 `context_management` 注入。直接的 OpenAI Responses 模型仍会强制设置 `store: true`，除非兼容性设置了 `supportsStore: false`。
    </Note>

  </Accordion>

  <Accordion title="严格智能体式 GPT 模式">
    对于 `openai/*` 上的 GPT-5 系列运行，OpenClaw 可使用更严格的嵌入式执行契约：
    ...
    - 当存在可用工具操作时，不再将“仅计划”轮次视为成功进展
    - 通过“立即行动”的引导重试该轮次
    - 对实质性工作自动启用 `update_plan`
    - 如果模型持续计划而不行动，则显式呈现阻塞状态

    <Note>
    仅适用于 OpenAI 和 Codex 的 GPT-5 系列运行。其他 provider 和较旧模型系列保持默认行为。
    </Note>
