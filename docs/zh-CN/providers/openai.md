...
title: OpenAI
x-i18n:
    generated_at: "2026-04-22T13:02:09Z"
    model: gpt-5.4
    provider: openai
...
<Tabs>
  ...
  <Tab title="Codex 订阅">
    **最适合：** 使用你的 ChatGPT/Codex 订阅，而不是单独的 API 密钥。Codex cloud 需要 ChatGPT 登录。
    ...
    | `openai-codex/gpt-5.3-codex-spark` | ChatGPT/Codex OAuth | Codex 登录（取决于权益） |

    <Note>
    这一路径特意与 `openai/gpt-5.4` 分开。使用 API 密钥搭配 `openai/*` 进行直接 Platform 访问，使用 `openai-codex/*` 进行 Codex 订阅访问。
    </Note>
    ...
    <Tip>
    如果新手引导复用了现有的 Codex CLI 登录，这些凭证会继续由 Codex CLI 管理。过期时，OpenClaw 会先重新读取外部 Codex 来源，再将刷新的凭证写回 Codex 存储。
    </Tip>

    ### 上下文窗口上限

    OpenClaw 会将模型元数据与运行时上下文上限视为两个独立的值。
...
OpenClaw 会为 `openai/*` 和 `openai-codex/*` 的 GPT-5 系列运行添加一层 OpenAI 专用的 GPT-5 提示词贡献。它位于内置的 OpenAI 插件中，适用于 `gpt-5`、`gpt-5.2`、`gpt-5.4` 和 `gpt-5.4-mini` 等模型 id，不适用于较早的 GPT-4.x 模型。

内置的原生 Codex harness 提供商（`codex/*`）也会通过 Codex app-server 开发者指令应用同样的 GPT-5 行为和心跳覆盖层，因此即使 Codex 负责其余 harness 提示词，`codex/gpt-5.x` 会话仍会保持相同的后续跟进和主动心跳指导。

GPT-5 贡献添加了一个带标签的行为契约，用于规范人格持续性、执行安全、工具纪律、输出形态、完成检查和验证。特定渠道的回复和静默消息行为仍保留在共享的 OpenClaw 系统提示词和出站传递策略中。对于匹配的模型，GPT-5 指导始终启用。友好交互风格层是独立的，并且可配置。
...
<AccordionGroup>
  ...
  <Accordion title="快速模式">
    ...
    启用后，OpenClaw 会将快速模式映射为 OpenAI 的优先处理（`service_tier = "priority"`）。现有的 `service_tier` 值会被保留，快速模式不会重写 `reasoning` 或 `text.verbosity`。
  ...
  <Accordion title="优先处理（service_tier）">
    ...
    <Warning>
    `serviceTier` 仅会转发到原生 OpenAI 端点（`api.openai.com`）和原生 Codex 端点（`chatgpt.com/backend-api`）。如果你通过代理路由任一提供商，OpenClaw 会保持 `service_tier` 原样不变。
    </Warning>
  ...
  <Accordion title="服务端压缩（Responses API）">
    ...
    <Note>
    `responsesServerCompaction` 仅控制 `context_management` 的注入。直接 OpenAI Responses 模型仍会强制设置 `store: true`，除非兼容性将 `supportsStore: false`。
    </Note>
  ...
  <Accordion title="严格智能体 GPT 模式">
    ...
    <Note>
    仅适用于 OpenAI 和 Codex 的 GPT-5 系列运行。其他提供商和较早的模型系列保持默认行为。
    </Note>

  </Accordion>

  <Accordion title="原生路由与 OpenAI 兼容路由">
    OpenClaw 会以不同方式处理直接 OpenAI、Codex 和 Azure OpenAI 端点，以及通用的 OpenAI 兼容 `/v1` 代理：
