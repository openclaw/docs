...
title: OpenAI
x-i18n:
    generated_at: "2026-05-06T18:07:08Z"
    model: gpt-5.5
    provider: openai
...
OpenAI 为 GPT 模型提供开发者 API，Codex 也可通过 OpenAI 的 Codex 客户端作为 ChatGPT 计划的编程智能体使用。OpenClaw 将这些表面分开，以便配置保持可预测。

OpenClaw 支持三条 OpenAI 系列路线。大多数想要 Codex 行为的 ChatGPT/Codex 订阅用户应使用原生 Codex 应用服务器运行时。模型前缀选择提供商/模型名称；单独的运行时设置选择由谁执行嵌入式 Agent loop：

- **API 密钥** - 使用按量计费的直接 OpenAI Platform 访问（`openai/*` 模型）
- **使用原生 Codex 运行时的 Codex 订阅** - ChatGPT/Codex 登录加 Codex 应用服务器执行（`openai/*` 模型加 `agents.defaults.agentRuntime.id: "codex"`）
- **通过 PI 使用 Codex 订阅** - 使用普通 OpenClaw PI 运行器的 ChatGPT/Codex 登录（`openai-codex/*` 模型）

OpenAI 明确支持在外部工具和 OpenClaw 这类工作流中使用订阅 OAuth。

提供商、模型、运行时和渠道是不同层级。如果这些标签混在一起，请先阅读 [Agent Runtimes](/zh-CN/concepts/agent-runtimes)，再修改配置。

## 快速选择

| 目标                                                 | 使用                                             | 说明                                                                      |
| ---------------------------------------------------- | ------------------------------------------------ | ------------------------------------------------------------------------- |
| 使用原生 Codex 运行时的 ChatGPT/Codex 订阅 | `openai/gpt-5.5` 加 `agentRuntime.id: "codex"` | 推荐给大多数用户的 Codex 设置。使用 `openai-codex` 身份验证登录。 |
| 直接 API 密钥计费                               | `openai/gpt-5.5`                                 | 设置 `OPENAI_API_KEY` 或运行 OpenAI API 密钥新手引导。                    |
| 通过 PI 使用 ChatGPT/Codex 订阅身份验证           | `openai-codex/gpt-5.5`                           | 仅在你有意使用普通 PI 运行器时使用。                |
...
| `openai-codex`                     | 提供商前缀   | 通过普通 OpenClaw PI 运行器的 OpenAI Codex OAuth/订阅路线。                      |
| `codex` 插件                     | 插件            | 提供原生 Codex 应用服务器运行时和 `/codex` 聊天控制的内置 OpenClaw 插件。 |
| `agentRuntime.id: codex`           | Agent 运行时     | 强制嵌入式轮次使用原生 Codex 应用服务器 harness。                                     |
| `/codex ...`                       | 聊天命令集  | 从对话中绑定/控制 Codex 应用服务器线程。                                        |
| `runtime: "acp", agentId: "codex"` | ACP 会话路线 | 通过 ACP/acpx 运行 Codex 的显式回退路径。                                          |

这意味着一个配置可以有意同时包含 `openai-codex/*` 和 `codex` 插件。当你想通过 PI 使用 Codex OAuth，同时也想让原生 `/codex` 聊天控制可用时，这是有效的。`openclaw doctor` 会对这种组合发出警告，以便你确认这是有意为之；它不会重写配置。

<Note>
GPT-5.5 可通过直接 OpenAI Platform API 密钥访问和订阅/OAuth 路线使用。对于 ChatGPT/Codex 订阅加原生 Codex 执行，请使用 `openai/gpt-5.5` 并设置 `agentRuntime.id: "codex"`。`openai-codex/gpt-5.5` 仅用于通过 PI 使用 Codex OAuth；或者在没有 Codex 运行时覆盖时使用 `openai/gpt-5.5` 进行直接 `OPENAI_API_KEY` 流量。
</Note>
...
如果内置 `codex` 插件已启用，但 `openai-codex/*` 仍通过 PI 解析，`openclaw doctor` 会发出警告并保持路线不变。
</Note>
...
| Codex 应用服务器 harness  | 使用 `agentRuntime.id: codex` 的 `openai/<model>`             | 是                                                    |
...
对于需要非对称嵌入标签的兼容 OpenAI 的端点，请在 `memorySearch` 下设置 `queryInputType` 和 `documentInputType`。OpenClaw 会将它们作为特定于提供商的 `input_type` 请求字段转发：查询嵌入使用 `queryInputType`；已索引的记忆分块和批量索引使用 `documentInputType`。完整示例请参阅[记忆配置参考](/zh-CN/reference/memory-config#provider-specific-config)。
...
<Tabs>
  <Tab title="API 密钥（OpenAI Platform）">
    **最适合：** 直接 API 访问和按量计费。
    ...
    <Note>
    `openai/*` 是直接 OpenAI API 密钥路线，除非你显式强制使用 Codex 应用服务器 harness。通过默认 PI 运行器使用 Codex OAuth 时，请使用 `openai-codex/*`；原生 Codex 应用服务器执行请使用 `openai/gpt-5.5` 并设置 `agentRuntime.id: "codex"`。
    </Note>
  ...
  <Tab title="Codex 订阅">
    **最适合：** 使用你的 ChatGPT/Codex 订阅通过原生 Codex 应用服务器执行，而不是使用单独的 API 密钥。Codex 云需要 ChatGPT 登录。
    ...
    <Steps>
      <Step title="运行 Codex OAuth">
        ...
        对于无头或不适合回调的设置，添加 `--device-code`，以使用 ChatGPT 设备码流程登录，而不是 localhost 浏览器回调：
    ...
    <Warning>
    不要配置较旧的 `openai-codex/gpt-5.1*`、`openai-codex/gpt-5.2*` 或 `openai-codex/gpt-5.3*` 模型引用。ChatGPT/Codex OAuth 账号现在会拒绝这些模型。PI OAuth 路线请使用 `openai-codex/gpt-5.5`；原生 Codex 运行时执行请使用 `openai/gpt-5.5` 并设置 `agentRuntime.id: "codex"`。
    </Warning>

    <Note>
    继续为认证/配置文件命令使用 `openai-codex` provider id。
    `openai-codex/*` 模型前缀也是 Codex OAuth 的显式 PI 路由。
    它不会选择或自动启用内置的 Codex app-server harness。对于
    常见的订阅加原生运行时设置，请使用 `openai-codex` 登录，但将模型引用保持为
    `openai/gpt-5.5`，并设置
    ...
    如果要改为在普通 PI runner 上保留 Codex OAuth，请使用
    ...
    使用这些命令查看你的默认智能体正在使用哪个模型、运行时和认证路由：
    ...
    如果 2026.5.5 的 `doctor --fix` 运行将 GPT-5.5 订阅设置从
    `openai-codex/gpt-5.5` 改成了 `openai/gpt-5.5`，请将默认智能体切回
    Codex OAuth PI 路由：
    ...
    如果 `models auth list --provider openai-codex` 没有显示可用配置文件，请重新登录：
    ...
    `openai-codex/*` 表示通过 PI 使用 ChatGPT/Codex OAuth。`openai/*` 搭配
    `agentRuntime.id: "codex"` 表示原生 Codex app-server 执行。

    ### Status 指示器

    聊天中的 `/status` 会显示当前会话正在使用哪个模型运行时。
    默认 PI harness 会显示为 `Runtime: OpenClaw Pi Default`。选择
    ...
    只有在有意使用该 PI 订阅认证路由时，才保持配置不变。当你想要原生
    Codex app-server 执行时，请切换到 `openai/<model>` 加
    `agentRuntime.id: "codex"`。
    ...
    较小的默认上限在实践中具有更好的延迟和质量特征。请用 `contextTokens` 覆盖它：
    ...
    如果实时 Codex 发现遗漏了 `openai-codex/gpt-5.5` 行，而账号已完成认证，
    OpenClaw 会合成该 OAuth 模型行，使 cron、子智能体和配置的默认模型运行不会因
    `Unknown model` 而失败。
...
`agentRuntime.id: "codex"`，但它的认证仍基于账号。OpenClaw
按以下顺序选择认证：

1. 绑定到智能体的显式 OpenClaw `openai-codex` 认证配置文件。
2. app-server 的现有账号，例如本地 Codex CLI ChatGPT 登录。
3. 仅对于本地 stdio app-server 启动，当 app-server 报告没有账号且仍需要
   OpenAI 认证时，依次使用 `CODEX_API_KEY`、然后
   `OPENAI_API_KEY`。

这意味着，本地 ChatGPT/Codex 订阅登录不会仅仅因为 Gateway 网关进程也有用于直接
OpenAI 模型或嵌入的 `OPENAI_API_KEY` 而被替换。环境变量 API 密钥回退只用于本地 stdio 无账号路径；
它不会发送到 WebSocket app-server 连接。选择订阅样式的 Codex
配置文件时，OpenClaw 还会从生成的 stdio app-server 子进程中排除
`CODEX_API_KEY` 和 `OPENAI_API_KEY`，并通过 app-server 登录 RPC 发送所选凭证。
...
| 宽高比 / 分辨率           | 不转发到 OpenAI Images API         | 安全时映射到支持的尺寸               |
...
仍会被接受。OpenClaw 还会通过将默认 `openai/gpt-image-2` 透明请求改写为
`gpt-image-1.5` 来保护公共 OpenAI 和
...
从输入文件开始时，请对 `openclaw infer image edit` 使用相同的
`--output-format` 和 `--background` 标志。
`--openai-background` 仍可作为 OpenAI 专用别名使用。

对于 Codex OAuth 安装，请保留同一个 `openai/gpt-image-2` 引用。当配置了
`openai-codex` OAuth 配置文件时，OpenClaw 会解析该已存储的 OAuth
访问令牌，并通过 Codex Responses 后端发送图像请求。对于该请求，
它不会先尝试 `OPENAI_API_KEY`，也不会静默回退到 API 密钥。
当你想要改用直接 OpenAI Images API 路由时，请用 API 密钥、
自定义基础 URL 或 Azure 端点显式配置 `models.providers.openai`。
如果该自定义图像端点位于受信任的 LAN/私有地址，还要设置
`browser.ssrfPolicy.dangerouslyAllowPrivateNetwork: true`；除非存在此选择加入，
...
OpenClaw 会为跨提供商的 GPT-5 系列运行添加共享的 GPT-5 提示词贡献。它按模型 id 应用，因此 `openai-codex/gpt-5.5`、`openai/gpt-5.5`、`openrouter/openai/gpt-5.5`、`opencode/gpt-5.5` 和其他兼容的 GPT-5 引用都会收到同一覆盖层。较旧的 GPT-4.x 模型不会收到。

内置原生 Codex harness 通过 Codex app-server developer instructions 使用相同的 GPT-5 行为和 Heartbeat 覆盖层，因此通过 `agentRuntime.id: "codex"` 强制使用的 `openai/gpt-5.x` 会话会保持相同的跟进和主动 Heartbeat 指导，即使 harness 提示词的其余部分由 Codex 拥有。

GPT-5 贡献会为人设持久性、执行安全、工具纪律、输出形态、完成检查和验证添加带标签的行为契约。特定渠道的回复和静默消息行为仍保留在共享的 OpenClaw 系统提示词和出站投递策略中。对于匹配的模型，GPT-5 指导始终启用。友好交互风格层是独立的，并且可配置。
...
<AccordionGroup>
  <Accordion title="语音合成（TTS）">
    ...
    `extraBody` 会在 OpenClaw 生成的字段之后合并到 `/audio/speech` 请求 JSON 中，因此可用于需要 `lang` 等额外键的 OpenAI 兼容端点。原型键会被忽略。
  ...
  <Accordion title="语音转文本">
    ...
    - 在 OpenClaw 中，任何使用入站音频转录的地方都支持
      `tools.media.audio`，包括 Discord 语音频道片段和渠道
      音频附件
    ...
    当共享音频媒体配置或单次转录请求提供语言和提示提示时，它们会转发给 OpenAI。
  ...
  <Accordion title="实时转录">
    ...
    <Note>
    使用 WebSocket 连接到 `wss://api.openai.com/v1/realtime`，并采用 G.711 u-law（`g711_ulaw` / `audio/pcmu`）音频。此流式传输提供商用于 Voice Call 的实时转录路径；Discord 语音目前会录制短片段，并改用批量 `tools.media.audio` 转录路径。
    </Note>
  ...
  <Accordion title="实时语音">
    ...
    短期客户端密钥，以及浏览器与
    OpenAI Realtime API 直接进行的 WebRTC SDP 交换。维护者可使用
    `OPENAI_API_KEY=... GEMINI_API_KEY=... node --import tsx scripts/dev/realtime-talk-live-smoke.ts`
    进行实时验证；OpenAI 这一段会在 Node 中签发客户端密钥，用假麦克风媒体生成浏览器 SDP offer，
    将其提交给 OpenAI，并在不记录密钥的情况下应用 SDP answer。
    </Note>
...
设置见 [语音和语音识别](#voice-and-speech) 下的 **实时语音**
折叠项。
</Note>
...
要通过内置的 `openai` 提供商进行 Azure 图像生成，请将
...
OpenClaw 2026.4.22 或更高版本。早期版本会把任何自定义
`openai.baseUrl` 都当作公共 OpenAI 端点处理，并且在访问 Azure
图像部署时失败。
</Note>

### API 版本

设置 `AZURE_OPENAI_API_VERSION`，为 Azure 图像生成路径固定特定的 Azure preview 或 GA 版本：
...
Azure OpenAI 会将模型绑定到部署。对于通过内置 `openai` 提供商路由的 Azure 图像生成请求，OpenClaw 中的 `model` 字段
必须是你在 Azure 门户中配置的 **Azure 部署名称**，而不是
公共 OpenAI 模型 ID。

如果你创建了名为 `gpt-image-2-prod`、用于服务 `gpt-image-2` 的部署：
...
版本上开放它们。这些差异来自 Azure 和底层模型，而不是
...
<AccordionGroup>
  ...
  <Accordion title="快速模式">
    ...
    <Note>
    会话覆盖优先于配置。在会话 UI 中清除会话覆盖后，该会话会恢复为配置的默认值。
    </Note>

  </Accordion>

  <Accordion title="优先处理（service_tier）">
    OpenAI 的 API 通过 `service_tier` 暴露优先处理能力。可在 OpenClaw 中按模型设置：
  ...
  <Accordion title="原生路由与 OpenAI 兼容路由">
    ...
    **原生路由**（`openai/*`、Azure OpenAI）：
    ...
    - 默认将工具 schema 设为严格模式
    - 仅在已验证的原生主机上附加隐藏归因标头
    - 保留仅 OpenAI 使用的请求塑形（`service_tier`、`store`、reasoning-compat、prompt-cache 提示）
    ...
    **代理/兼容路由：**
    ...
    - 接受面向 vLLM 等 OpenAI 兼容 Completions 代理的 `params.chat_template_kwargs`
