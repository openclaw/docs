---
read_when:
    - 你希望将本地 ComfyUI 工作流与 OpenClaw 配合使用
    - 你希望将 Comfy Cloud 用于图像、视频或音乐工作流
    - 你需要内置 comfy 插件的配置键
summary: 在 OpenClaw 中设置基于 ComfyUI 工作流的图像、视频和音乐生成
title: ComfyUI
x-i18n:
    generated_at: "2026-07-12T14:42:35Z"
    model: gpt-5.6
...
OpenClaw 内置了一个 `comfy` 插件，用于工作流驱动的 ComfyUI 运行。该插件完全由工作流驱动：OpenClaw 不会将通用的 `size`、`aspectRatio`、`resolution`、`durationSeconds` 或 TTS 风格的控制项映射到你的图中。
...
- 通过工作流 JSON 生成和编辑图像（编辑接受 1 张上传的参考图像）
- 通过工作流 JSON 生成视频，支持文生视频或图生视频（1 张参考图像）
- 通过共享的 `music_generate` 工具生成音乐/音频，可选择提供 1 张参考图像
- 从配置的节点下载输出；如果未配置节点，则从所有匹配的输出节点下载
...
<Tabs>
    ...
    **最适合：** 在你的计算机或局域网上运行自己的 ComfyUI 实例。
    ...
    <Steps>
      ...
      <Step title="准备工作流 JSON">
        导出或创建 ComfyUI 工作流 JSON 文件。记下提示词输入节点和你希望 OpenClaw 从中读取内容的输出节点的节点 ID。
      </Step>
      <Step title="配置提供商">
      ...
      <Step title="设置默认模型">
        对于你配置的能力，将 OpenClaw 指向 `comfy/workflow` 模型：
  ...
  <Tab title="Comfy Cloud">
    **最适合：** 无需管理本地 GPU 资源，即可在 Comfy Cloud 上运行工作流。
    ...
    <Steps>
      ...
      <Step title="配置提供商">
        ...
        <Tip>
        云模式下，`baseUrl` 默认为 `https://cloud.comfy.org`。仅在使用自定义云端点时设置 `baseUrl`。
        </Tip>
      </Step>
      <Step title="设置默认模型">
        ```json5
...
| `allowPrivateNetwork` | 布尔值                  | 允许在云模式下使用私有/局域网 `baseUrl`，或使用本地私有 DNS FQDN。                   |

<Note>
在 `local` 模式下，无需设置 `allowPrivateNetwork` 即可使用环回/私有 IP 字面量和单标签服务名称，例如 `http://comfyui:8188`。看起来像公网域名的私有 DNS FQDN（例如 `https://comfy.local.example.com`）需要设置 `allowPrivateNetwork: true`。对私有源的信任仍限定在已配置的方案、主机名和端口范围内；本地重定向不能离开已配置的主机名，而云端到公共 CDN 的重定向会使用默认 SSRF 策略进行检查。
</Note>
...
| `outputNodeId`               | 否   | --       | 要从中读取输出的节点 ID。如果省略，则使用所有匹配的输出节点。                |
| `pollIntervalMs`             | 否   | `1500`   | 轮询作业完成状态的间隔，以毫秒为单位。                                       |
...
`apiKey` 接受字符串字面量或[密钥引用](/zh-CN/gateway/configuration-reference#secrets)对象。
...
<AccordionGroup>
  ...
  <Accordion title="视频工作流">
    ...
    <Note>
    OpenClaw 不会将输入视频传递给 Comfy 工作流。仅支持将文本提示词和单张参考图像作为输入。
    </Note>

  </Accordion>

  <Accordion title="音乐工作流">
    内置插件注册了一个音乐生成提供商，用于生成由工作流定义的音频或音乐输出，并通过共享的 `music_generate` 工具提供。它接受可选的参考图像（最多 1 张）：
  ...
  <Accordion title="向后兼容性">
    现有的顶层图像配置（不包含嵌套的 `image` 部分）仍然有效：
    ...
    OpenClaw 会将该旧版结构视为图像工作流配置。你无需立即迁移，但对于新设置，建议使用嵌套的 `image` / `video` / `music` 部分。如果你只使用图像生成，旧版扁平配置与新的嵌套 `image` 部分在功能上等效。
