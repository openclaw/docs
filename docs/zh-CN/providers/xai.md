...
OpenClaw 内置了一个用于 Grok 模型的 `xai` 提供商插件。推荐方式是使用符合条件的 SuperGrok 或 X Premium 订阅进行 Grok OAuth 身份验证。Gateway 网关、配置、路由和工具均保留在本地；只有 Grok 请求会发送到 xAI 的 API。

OAuth 不需要 xAI API key，也不需要 Grok Build 应用。由于 OpenClaw 使用 xAI 的共享 OAuth 客户端，xAI 仍可能在授权界面上显示 Grok Build。
...
<Steps>
  ...
  <Step title="现有安装">
    仅登录 xAI；不要只为连接 Grok 而重新运行完整的新手引导：
  ...
  <Step title="API key 方式">
    对于 xAI Console key 以及需要基于 key 的提供商配置的媒体功能，API key 设置仍然可用：
...
<Note>
OpenClaw 使用 xAI Responses API 作为内置的 xAI 传输方式。通过 `openclaw models auth login --provider xai --method oauth` 或 `--method api-key` 获取的同一凭据，也可用于 `web_search`（提供商 ID 为 `grok`）、`x_search`、`code_execution`、语音/转录以及 xAI 图像/视频生成。如果你将 xAI key 存储在 `plugins.entries.xai.config.webSearch.apiKey` 下，内置的 xAI 模型提供商也会将其作为回退凭据复用。
</Note>
...
- xAI 决定哪些账号可以获得 OAuth API 令牌。如果某个账号不符合条件，请使用 API key 方式，或在 xAI 侧检查订阅状态。

<Tip>
通过 SSH、Docker 或 VPS 登录时，请使用 `xai-oauth`。OpenClaw 会输出一个 URL 和短代码；远程进程轮询 xAI 等待令牌交换完成期间，你可以在任意本地浏览器中完成登录。
</Tip>

## 内置目录

模型选择器中可选的 ID。插件仍会解析现有配置中的旧版 Grok 3、Grok 4、Grok 4 Fast、Grok 4.1 Fast 和 Grok Code ID；请参阅[旧版兼容性和动态别名](#legacy-compatibility-and-moving-aliases)。
...
<Tip>
如果可用，请使用 `grok-4.5` 进行通用聊天、编码和智能体工作。Grok 4.3 仍是适用于各地区的安全设置默认值；`grok-build-0.1` 和两个带日期的 Grok 4.20 变体仍可选择。
</Tip>

## 功能覆盖范围

内置插件将受支持的 xAI API 映射到 OpenClaw 的共享提供商和工具契约。无法适配共享契约的能力列在下方或已知限制中。

| xAI 能力                   | OpenClaw 功能表面                         | 状态                                                   |
| -------------------------- | ----------------------------------------- | ------------------------------------------------------ |
...
<Note>
OpenClaw 使用 xAI 的 REST 图像/视频/TTS/STT API 进行媒体生成和批量转录，使用 xAI 的流式 STT WebSocket 进行实时语音通话转录，并使用 Responses API 执行聊天、搜索和代码执行工具。
</Note>

### 旧版快速模式兼容性

`/fast on` 或 `agents.defaults.models["xai/<model>"].params.fastMode: true`
仍会按如下方式重写旧版 xAI 配置。这些目标 ID 仅为兼容性而保留；新配置请使用当前可选模型。
...
带日期的 0309 ID 是可选的目录条目。OpenClaw 会原样发送所有其他当前 Grok 4.20 别名，以便 xAI 继续控制稳定版、最新版、测试版、实验版和带日期别名的语义。全局 `grok-latest` 别名也会原样保留。

xAI 已停用以下确切 ID。OpenClaw 将它们作为隐藏的兼容性行保留，用于已发布的配置，其限制和定价与当前重定向目标一致：

| 已停用的 ID                                                         | 当前行为                         |
| ------------------------------------------------------------------- | -------------------------------- |
| `grok-4-1-fast-reasoning`, `grok-4-fast-reasoning`, `grok-4-0709`   | Grok 4.3，推理强度为 `low`       |
| `grok-4-1-fast-non-reasoning`, `grok-4-fast-non-reasoning`, `grok-3` | Grok 4.3，禁用推理               |
| `grok-code-fast-1`                                                  | Grok Build 0.1                   |
| `grok-imagine-image-pro`                                            | Grok Imagine Image Quality       |

`openclaw doctor --fix` 会更新持久化的 xAI 服务端工具默认值和已停用的高质量图像 slug，删除过时的已生成目录行，并修复有效 4.20 行中过时的上下文元数据。它不会将有效的 4.20 `beta-latest` 别名固定到带日期的快照。

## 功能

<Warning>
  `x_search` 和 `code_execution` 在 xAI 的服务器上运行。xAI 对每 1,000 次工具调用收取 $5，此外还会收取模型输入和输出令牌的费用。省略每个工具的 `enabled` 设置时，OpenClaw 仅在当前使用 xAI 模型时公开该工具。已知的非 xAI 模型提供商需要为每个工具显式设置 `enabled: true`；提供商缺失或无法解析时会以关闭状态失败。始终需要 xAI 身份验证，且 `enabled: false` 会对所有提供商禁用该工具。
</Warning>

<AccordionGroup>
  <Accordion title="网页搜索">
    内置的 `grok` 网页搜索提供商优先使用 xAI OAuth，然后回退到 `XAI_API_KEY` 或插件网页搜索 key：
  ...
  <Accordion title="视频生成">
    ...
    - Video 1.5 模式：仅支持图生视频，并且必须恰好提供一张首帧图像
    - 宽高比：`1:1`、`16:9`、`9:16`、`4:3`、`3:4`、`3:2`、`2:3`；省略时，经典模式和 Video 1.5 图生视频都会继承源图像的宽高比
  ...
  <Accordion title="图像生成">
    ...
    OpenClaw 会请求 xAI 返回 `b64_json` 图像响应，以便通过常规渠道附件路径
    存储和传送生成的媒体。本地参考图像会转换为数据 URL；远程 `http(s)` 引用
    会保持不变并直接传递。
    ...
    OpenClaw 目前仅转发跨提供商共用的图像控制项；
    这些仅限原生接口的选项不会通过 `image_generate` 公开。
    </Note>

  </Accordion>

  <Accordion title="文本转语音">
    内置 `xai` 插件通过共享 `tts` 提供商接口注册文本转语音功能。

    - 语音：来自 xAI 的实时身份验证目录；使用
      `openclaw infer tts voices --provider xai` 列出
    - 离线回退语音：`ara`、`eve`、`leo`、`rex`、`sal`
    - 默认语音：`eve`
    - 即使账户自定义语音 ID 不在内置目录响应中，也会被转发
    ...
    <Note>
    OpenClaw 使用 xAI 的批处理 `/v1/tts` 端点和经过身份验证的
  ...
  <Accordion title="语音转文本">
    内置 `xai` 插件通过 OpenClaw 的媒体理解转录接口注册批量语音转文本功能。
    ...
    可以通过共享音频媒体配置或每次调用的转录请求提供语言。
    共享 OpenClaw 接口接受提示词提示，但 xAI REST STT 集成仅转发文件和语言，
    因为只有这两项能映射到当前公开的 xAI 端点。
  ...
  <Accordion title="x_search 配置">
    内置 xAI 插件将 `x_search` 公开为 OpenClaw 工具，用于通过 Grok
    搜索 X（原 Twitter）内容。
  ...
  <Accordion title="代码执行配置">
    内置 xAI 插件将 `code_execution` 公开为 OpenClaw 工具，用于在
    xAI 的沙箱环境中远程执行代码。
  ...
  <Accordion title="已知限制">
    - xAI 身份验证可以使用 API 密钥、环境变量、插件配置回退，或通过符合条件的
      xAI 账户使用 OAuth。OAuth 使用设备代码验证，无需 localhost 回调。xAI 决定
      哪些账户可以接收 OAuth API 令牌，并且同意页面可能会显示 Grok Build，
      即使 OpenClaw 不需要 Grok Build 应用。
    - OpenClaw 目前未公开 xAI 多智能体模型系列。xAI 通过 Responses API
      提供这些模型，但它们不接受 OpenClaw 共享 Agent loop 所使用的客户端工具
      或自定义工具。请参阅
      [xAI 多智能体限制](https://docs.x.ai/developers/model-capabilities/text/multi-agent#limitations)。
    - xAI Realtime 语音尚未注册为 OpenClaw 提供商。它需要不同于批处理 STT
      或流式转录的双向语音会话契约。
    - 在共享 `image_generate` 工具具备相应的跨提供商控制项之前，
      不会公开 xAI 图像 `quality`、图像 `mask` 和原生 `auto` 宽高比。
  </Accordion>
  ...
  <Accordion title="高级说明">
    ...
    - 原生 xAI 请求默认使用 `tool_stream: true`。将
      ...
      可将其禁用。
    - 内置 xAI 包装器会在发送原生 xAI 请求前，移除不受支持的包含计数架构边界
      以及不受支持的推理 *effort* 负载键。Grok 4.5 支持低、中和
      高 effort（默认高）。Grok 4.3 支持无、低、中和高
      effort（默认低）。其他具备推理能力的 xAI 模型不公开
      可配置的 effort 控制项，但仍会请求
      `include: ["reasoning.encrypted_content"]`，以便在后续轮次中重放先前加密的推理内容。
    - `web_search`、`x_search` 和 `code_execution` 会作为 OpenClaw
      工具公开。OpenClaw 仅将每个工具所需的特定 xAI 内置工具附加到该工具的请求，
      而不是将所有原生工具附加到每一轮聊天。
    ...
    - `x_search` 和 `code_execution` 由内置 xAI 插件所有，
      而不是硬编码到核心模型运行时中。
...
xAI 媒体路径由单元测试和选择性启用的实时测试套件覆盖。运行实时探测前，
...
该提供商专用的实时文件会合成普通 TTS、适合电话通信的 PCM
TTS，通过 xAI 批量 STT 转录音频，通过 xAI
实时 STT 流式传输相同的 PCM，生成文生图输出，并编辑参考图像。
共享图像实时文件通过 OpenClaw 的运行时选择、回退、规范化和媒体附件路径，
验证同一个 xAI 提供商。可选启用的 Video 1.5 用例会提交一张生成的
1080P 首帧图像，并验证已完成视频的下载。
