...
- 更新 `resolveTheme()`，使 `custom` 映射现有系列行为：
  ...
  - `custom + system` -> 根据操作系统偏好得到 `custom` 或 `custom-light`
...
- `light` 和 `dark` 是已规范化的 OpenClaw token 映射，而不是原始 tweakcn 载荷。
...
- 在 `Claw`、`Knot` 和 `Dash` 旁边添加一张 `Custom` 主题卡片。
