...
title: CLI 设置参考
x-i18n:
    generated_at: "2026-07-16T12:03:32Z"
    model: gpt-5.6
...
此页面涵盖分步新手引导的行为、输出和内部机制。
如需操作演示，请参阅[新手引导（CLI）](/zh-CN/start/wizard)。如需完整的 CLI 标志
参考（每个 `--flag`、非交互式示例、特定于提供商的
...
- Web 搜索提供商（可选）
- 守护进程安装（LaunchAgent、systemd 用户单元，或原生 Windows 计划任务并以启动文件夹作为后备方案）
- 健康检查
- Skills 设置

远程模式将此计算机配置为连接到其他位置的 Gateway 网关。它
不会在远程主机上安装或修改任何内容。
...
<Steps>
  <Step title="检测现有配置">
    ...
    - 再次运行向导不会清除任何内容，除非你明确选择“重置”（或传入 `--reset`）。
    - CLI `--reset` 默认为 `config+creds+sessions`；如需同时移除工作区，请使用 `--reset-scope full`。
  ...
  <Step title="模型和身份验证">
    - 完整选项矩阵位于[身份验证和模型选项](#auth-and-model-options)。
  ...
  <Step title="Gateway 网关">
    ...
    - 建议：即使使用回环地址，也应保持令牌身份验证启用，以便本地 WS 客户端必须进行身份验证。
    - 在令牌模式下，交互式设置提供：
      - **生成/存储明文令牌**（默认）
      - **使用 SecretRef**（选择启用）
  ...
  <Step title="Web 搜索">
    ...
    - 使用 `--skip-search` 跳过此步骤；稍后使用 `openclaw configure --section web` 重新配置。
  ...
  <Step title="守护进程安装">
    ...
    - 原生 Windows：优先使用计划任务
      - 如果创建任务遭到拒绝，OpenClaw 会回退到每用户启动文件夹登录项，并立即启动 Gateway 网关。
      - 计划任务仍是首选，因为它们能提供更好的监管程序状态。
  ...
  <Step title="Skills">
    ...
    - 当所需的安装程序可用时，为受信任的内置 Skills 安装可选
      依赖项。
    - 跳过不可用的 Homebrew、uv 和 Go 安装程序，然后将受影响的
      Skills 分组并提供手动设置指南。安装缺失的前置条件后，运行
      `openclaw doctor`。
...
如果缺少 Control UI 资源，向导会尝试构建它们；后备方案是 `pnpm ui:build`（自动安装 UI 依赖项）。
</Note>
...
<Steps>
  ...
  <Step title="连接方式">
    ...
    - **SSH 隧道**：输出需要先运行的 `ssh -N -L 18789:127.0.0.1:18789 <user>@<host>`
      命令，然后连接到本地隧道端点。
  </Step>
  <Step title="身份验证">
...
<Note>
如果 Gateway 网关仅绑定回环地址且无法被发现，请手动使用 SSH 隧道或 tailnet。
对于回环地址、私有 IP 字面量、`.local` 和 Tailnet `*.ts.net` URL，可以使用明文 `ws://`；其他私有 DNS 名称需要 `OPENCLAW_ALLOW_INSECURE_PRIVATE_WS=1`。
</Note>

## 身份验证和模型选项

如果提供商设置步骤在交互式新手引导中失败（例如使用 CLI 复用选项
但本地未登录），向导会显示错误并返回提供商选择器，
而不是退出。显式 `--auth-choice` 运行仍会快速失败，以便用于自动化。
...
<AccordionGroup>
  ...
  <Accordion title="Anthropic Claude CLI">
    交互式新手引导/配置中的首选本地路径；如果存在 Claude CLI 登录，则复用它。
  </Accordion>
  <Accordion title="OpenAI Code 订阅（OAuth）">
    浏览器流程；粘贴 `code#state`。

    在没有主模型的全新设置中，通过 Codex 运行时将 `agents.defaults.model` 设置为
    `openai/gpt-5.6-sol`。
  ...
  <Accordion title="OpenAI API 密钥">
    ...
    在没有主模型的全新设置中，将 `agents.defaults.model` 设置为
    `openai/gpt-5.6`；不带前缀的直接 API 模型 ID 会解析到 Sol 层级。

    添加 OpenAI 或重新进行身份验证时，会保留现有的显式主
    模型，包括 `openai/gpt-5.5`。如果账号不提供 GPT-5.6，
    请显式选择 `openai/gpt-5.5`；OpenClaw 不会静默降级。

  </Accordion>
  <Accordion title="xAI (Grok) OAuth">
    面向符合条件的 SuperGrok 或 X Premium 账户的浏览器登录方式。这是大多数用户
    推荐使用的 xAI 路径。OpenClaw 会存储生成的身份验证
    配置文件，供 Grok 模型、Grok `web_search`、`x_search` 和 `code_execution` 使用。
  </Accordion>
  <Accordion title="xAI (Grok) 设备代码">
    使用短代码而非 localhost
    回调的远程友好型浏览器登录方式。可在 SSH、Docker 或 VPS 主机上使用此方式。
  </Accordion>
  <Accordion title="xAI (Grok) API key">
    提示输入 `XAI_API_KEY`，并将 xAI 配置为模型提供商。当你希望使用
    xAI Console API key 而非订阅 OAuth 时，请使用此方式。
  </Accordion>
  <Accordion title="OpenCode">
    提示输入 `OPENCODE_API_KEY`（或 `OPENCODE_ZEN_API_KEY`），并允许你选择 Zen 或 Go 目录（一个 API key 可同时用于两者）。
  ...
  <Accordion title="MiniMax">
    配置会自动写入。托管模式默认值为 `MiniMax-M3`；API key 设置使用
  ...
  <Accordion title="Ollama（云端和本地开放模型）">
    ...
    基于主机的模式会提示输入基础 URL（默认值为 `http://127.0.0.1:11434`），发现可用模型并建议默认值。
    `Cloud + Local` 还会检查该 Ollama 主机是否已登录以使用云端访问。
    更多详情：[Ollama](/zh-CN/providers/ollama)。
  </Accordion>
  <Accordion title="Moonshot 和 Kimi Coding">
    系统会自动写入 Moonshot（Kimi K2）和 Kimi Coding 配置。
    更多详情：[Moonshot AI（Kimi + Kimi Coding）](/zh-CN/providers/moonshot)。
  </Accordion>
  <Accordion title="自定义提供商">
    适用于 OpenAI 兼容、OpenAI Responses 兼容和 Anthropic 兼容的端点。

    交互式新手引导支持与其他提供商 API key 流程相同的 API key 存储选项：
    - **立即粘贴 API key**（明文）
    - **使用机密引用**（环境变量引用或已配置的提供商引用，并进行预检验证）

    新手引导会推断常见视觉模型 ID（GPT-4o/4.1/5.x、Claude 3/4、Gemini、Qwen-VL、LLaVA、Pixtral 及类似模型）的图像支持能力，仅当模型名称未知时才会询问。
...
- 默认情况下，新手引导会将 API key 以明文值持久化到身份验证配置文件中。
...
- 交互式引用模式会在保存前运行快速预检验证。
  ...
  - 如果预检失败，新手引导会显示错误并允许重试。
- 在非交互式模式下，`--secret-input-mode ref` 仅由环境变量提供。
...
<Note>
无头和服务器提示：在配有浏览器的计算机上完成 OAuth，然后将
...
- `session.dmScope`（未设置时，本地新手引导会将其默认为 `per-channel-peer`；保留现有显式值）
- `channels.telegram.botToken`、`channels.discord.token`、`channels.matrix.*`、`channels.signal.*`、`channels.imessage.*`
- 在提示期间选择启用时的渠道允许列表（Discord、iMessage、Signal、Slack、Telegram、WhatsApp）；Discord 和 Slack 还会将输入的名称解析为 ID
...
<Note>
部分渠道以插件形式提供。在设置期间选择这些渠道时，向导会
在配置渠道之前提示安装插件（npm 或本地路径）。
</Note>

## 非交互式设置

`--non-interactive` 要求使用 `--accept-risk`（确认智能体
功能强大，拥有完整系统访问权限存在风险）：
...
客户端（macOS 应用和 Control UI）可以呈现各步骤，而无需重新实现新手引导逻辑。

## Signal 设置行为

- 从官方 `signal-cli` GitHub 发布中下载适当的发布资产（原生构建，仅限 Linux x86-64）
- 在其他平台（macOS、非 x64 Linux）上，改为通过 Homebrew 安装
- 将发布资产安装存储在 `~/.openclaw/tools/signal-cli/<version>/` 下
- 在配置中写入 `channels.signal.cliPath`
- 尚不支持原生 Windows；请在 WSL2 内运行新手引导，以使用 Linux 安装路径
