...
read_when:
    - 设置新机器
    - 你想要“最新且最好”的功能，同时不破坏你的个人设置
summary: OpenClaw 的高级设置和开发工作流
title: 设置
x-i18n:
    generated_at: "2026-05-06T03:39:54Z"
    model: gpt-5.5
    provider: openai
...
- **旧版 OAuth 导入**：`~/.openclaw/credentials/oauth.json`
  更多详情：[Security](/zh-CN/gateway/security#credential-storage-map)。
...
Linux 安装使用 systemd **用户**服务。默认情况下，systemd 会在退出登录/空闲时停止用户服务，这会终止 Gateway 网关。新手引导会尝试为你启用 linger（可能会提示输入 sudo）。如果仍未启用，请运行：

```bash
sudo loginctl enable-linger $USER
```

对于常驻或多用户服务器，请考虑使用**系统**服务而不是用户服务（无需 linger）。systemd 说明见 [Gateway 网关运行手册](/zh-CN/gateway)。

## 相关文档

- [Gateway 网关运行手册](/zh-CN/gateway)（flags、监管、端口）
