...
title: 配置
x-i18n:
    generated_at: "2026-06-27T01:35:43Z"
    model: gpt-5.5
...
配置辅助命令用于对 `openclaw.json` 进行非交互式编辑：按路径 get/set/patch/unset/file/schema/validate 值，并打印当前生效的配置文件。不带子命令运行时会打开配置向导（等同于 `openclaw configure`）。

<Note>
当 `OPENCLAW_NIX_MODE=1` 时，OpenClaw 会将 `openclaw.json` 视为不可变。`config get`、`config file`、`config schema` 和 `config validate` 等只读命令仍然可用，但配置写入命令会拒绝执行。智能体应改为编辑该安装的 Nix 源；对于第一方 nix-openclaw 发行版，请使用 [nix-openclaw 快速开始](https://github.com/openclaw/nix-openclaw#quick-start)，并在 `programs.openclaw.config` 或 `instances.<name>.config` 下设置值。
</Note>
...
<AccordionGroup>
  <Accordion title="包含内容">
    ...
    - 当运行时清单可加载时，尽力提供实时插件 + 渠道 schema 元数据。
    - 即使当前配置无效，也会提供干净的回退 schema。

  </Accordion>
  <Accordion title="相关运行时 RPC">
    `config.schema.lookup` 会返回一个规范化配置路径，其中包含浅层 schema 节点（`title`、`description`、`type`、`enum`、`const`、常见边界）、匹配的 UI 提示元数据以及直接子项摘要。可在 Control UI 或自定义客户端中用于按路径下钻。
  </Accordion>
</AccordionGroup>
...
路径使用点号或方括号表示法。在 shell 示例中请引用方括号表示法路径，这样 zsh 等 shell 就不会在 OpenClaw 收到路径前将 `[0]` 展开为 glob：
...
值会在可能时按 JSON5 解析；否则会被视为字符串。使用 `--strict-json` 可要求进行 JSON5 解析。`--json` 仍作为旧版别名受支持。
...
`config get <path> --json` 会将原始值作为 JSON 打印，而不是打印为终端格式化文本。

<Note>
默认情况下，对象赋值会替换目标路径。常用于保存用户新增条目的受保护 map/list 路径，例如 `agents.defaults.models`、`models.providers`、`models.providers.<id>.models`、`plugins.entries` 和 `auth.profiles`，会拒绝会移除现有条目的替换操作，除非你传入 `--replace`。
</Note>
...
<Warning>
不受支持的运行时可变表面会拒绝 SecretRef 赋值（例如 `hooks.token`、`commands.ownerDisplaySecret`、Discord 线程绑定 webhook token，以及 WhatsApp 凭证 JSON）。请参阅 [SecretRef 凭证表面](/zh-CN/reference/secretref-credential-surface)。
</Warning>
...
当你想粘贴或通过管道传入配置形状的补丁，而不是运行许多基于路径的 `config set` 命令时，请使用 `config patch`。输入是一个 JSON5 对象。对象会递归合并，数组和标量值会替换目标值，`null` 会删除目标路径。
...
当某个对象或数组必须完全变为提供的值，而不是被递归修补时，请使用 `--replace-path <path>`：

```bash
openclaw config patch --file ./discord.patch.json5 --replace-path 'channels.discord.guilds["123"].channels'
```

`--dry-run` 会在不写入的情况下运行 schema 和 SecretRef 可解析性检查。默认情况下，试运行期间会跳过由 exec 支持的 SecretRef；当你有意让试运行执行提供商命令时，请添加 `--allow-exec`。
...
<AccordionGroup>
  <Accordion title="试运行行为">
    - 构建器模式：对更改的 ref/提供商运行 SecretRef 可解析性检查。
    ...
    - 默认情况下，试运行期间会跳过 Exec SecretRef 检查，以避免命令副作用。
    - 将 `--allow-exec` 与 `--dry-run` 搭配使用，可选择启用 exec SecretRef 检查（这可能会执行提供商命令）。
    - `--allow-exec` 仅用于试运行；如果不与 `--dry-run` 一起使用会报错。
...
<AccordionGroup>
  <Accordion title="如果 dry-run 失败">
    - `config schema validation failed`：你变更后的配置形状无效；修复路径/值或 provider/ref 对象形状。
    - `Config policy validation failed: unsupported SecretRef usage`：将该凭证移回明文/字符串输入，并仅在支持的表面上保留 SecretRefs。
    ...
    - 对于批量模式，请修复失败条目，并在写入前重新运行 `--dry-run`。
...
`openclaw config set` 和其他 OpenClaw 拥有的配置写入器会在提交到磁盘前验证完整的变更后配置。如果新 payload 未通过 schema 验证，或看起来像破坏性覆盖，活动配置会保持不变，被拒绝的 payload 会作为 `openclaw.json.rejected.*` 保存在旁边。

<Warning>
活动配置路径必须是常规文件。不支持对符号链接的 `openclaw.json` 布局进行写入；请改用 `OPENCLAW_CONFIG_PATH` 直接指向真实文件。
</Warning>
...
如果写入被拒绝，请检查保存的 payload 并修复完整配置形状：
...
仍然允许直接用编辑器写入，但正在运行的 Gateway 网关会在验证通过前将其视为不受信任。无效的直接编辑会导致启动失败或被热重载跳过；Gateway 网关不会重写 `openclaw.json`。运行 `openclaw doctor --fix` 以修复带前缀/被覆盖的配置，或恢复最后一个已知良好的副本。参见 [Gateway 故障排除](/zh-CN/gateway/troubleshooting#gateway-rejected-invalid-config)。
...
在 `openclaw config validate` 通过后，你可以使用本地 TUI，让嵌入式智能体在同一终端中验证每项变更时，将活动配置与文档进行比较：
