...
title: 新手引导
x-i18n:
    generated_at: "2026-07-04T20:24:28Z"
    model: gpt-5.5
...
用于本地或远程 Gateway 网关设置的完整引导式新手引导。当你希望 OpenClaw 在一个流程中引导完成模型凭证、工作区、Gateway 网关、渠道、技能和健康检查时使用它。
...
`--flow import` 使用由插件拥有的迁移提供程序，例如 Hermes。它只会针对全新的 OpenClaw 设置运行；如果已有配置、凭证、会话或工作区记忆/身份文件存在，请在导入前重置或选择全新设置。

`--modern` 会启动 Crestodian 对话式新手引导预览。不带
`--modern` 时，`openclaw onboard` 会保留经典新手引导流程。

在交互式终端中，裸 `openclaw`（无子命令）会按配置
状态路由：

- 如果活动配置文件缺失，或没有作者编写的设置（为空或
  仅含元数据），它会启动这个经典新手引导流程。
- 如果配置文件存在但验证失败，它会启动
  [Crestodian](/zh-CN/cli/crestodian) 进行修复。
- 如果配置文件有效，它会打开普通的智能体 TUI，可以是本地运行，
  也可以连接到可访问的已配置 Gateway 网关。在已配置安装中，
  可在 TUI 内使用 `/crestodian` 或 `openclaw crestodian` 进入 Crestodian。

明文 `ws://` 可用于 local loopback、私有 IP 字面量、`.local` 和
Tailnet `*.ts.net` Gateway 网关 URL。对于其他受信任的私有 DNS 名称，请在新手引导进程环境中设置
`OPENCLAW_ALLOW_INSECURE_PRIVATE_WS=1`。

## 语言区域

交互式新手引导会将 CLI 向导语言区域用于固定设置文案。解析
顺序为：
...
对于由凭证配置文件支持的提供商，这会写入 `keyRef` 条目；对于自定义提供商，这会将 `models.providers.<id>.apiKey` 写为环境变量引用（例如 `{ source: "env", provider: "default", id: "CUSTOM_API_KEY" }`）。

非交互式 `ref` 模式契约：

- 在新手引导进程环境中设置提供商环境变量（例如 `OPENAI_API_KEY`）。
- 不要传入内联密钥标志（例如 `--openai-api-key`），除非该环境变量也已设置。
- 如果传入了内联密钥标志但缺少必需的环境变量，新手引导会快速失败并给出指引。
...
- 使用 `--install-daemon` 时，如果令牌凭证需要令牌，由 SecretRef 管理的 Gateway 网关令牌会被验证，但不会以已解析明文形式持久化到 supervisor 服务环境元数据中。
- 使用 `--install-daemon` 时，如果令牌模式需要令牌，并且已配置的令牌 SecretRef 未解析，新手引导会失败关闭并给出修复指引。
- 使用 `--install-daemon` 时，如果同时配置了 `gateway.auth.token` 和 `gateway.auth.password`，且未设置 `gateway.auth.mode`，新手引导会阻止安装，直到显式设置模式。
- 本地新手引导会将 `gateway.mode="local"` 写入配置。如果后续配置文件缺少 `gateway.mode`，请将其视为配置损坏或不完整的手动编辑，而不是有效的本地模式捷径。
- 当所选设置路径需要时，本地新手引导会安装选定的可下载插件。
- 远程新手引导只会为远程 Gateway 网关写入连接信息，不会安装本地插件包。
- `--allow-unconfigured` 是一个独立的 Gateway 网关运行时逃生口。它并不表示新手引导可以省略 `gateway.mode`。
...
- 除非传入 `--skip-health`，否则新手引导会等待可访问的本地 Gateway 网关，然后才会成功退出。
- `--install-daemon` 会先启动托管 Gateway 网关安装路径。不使用它时，你必须已经有本地 Gateway 网关在运行，例如 `openclaw gateway run`。
...
- 在原生 Windows 上，`--install-daemon` 会先尝试定时任务；如果任务创建被拒绝，则回退到每用户启动文件夹登录项。
...
<Note>
`--auth-choice zai-api-key` 会为你的密钥自动检测最佳 Z.AI 端点和模型。
Coding Plan 端点优先使用 `zai/glm-5.2`；通用 API 端点使用
...
- `--token-profile-id <id>` — 凭证配置文件 ID。通用令牌存储默认使用 `<provider>:manual`；由提供商拥有的设置流程可能使用自己的默认值，例如 `anthropic:default`。
...
- `--skip-hooks` — 在新手引导期间跳过 webhook / 钩子设置提示。

输出抑制：

- `--suppress-gateway-token-output` — 抑制携带令牌的 Gateway 网关/UI 输出（令牌提示、带嵌入令牌的自动登录 URL，以及自动启动 Control UI）。适用于共享终端和 CI 环境。
...
<AccordionGroup>
  <Accordion title="Flow types">
    ...
    - `manual`：用于端口、绑定和凭证的完整提示（`advanced` 的别名）。
    - `import`：运行检测到的迁移提供商，预览计划，然后在确认后应用。

  </Accordion>
  <Accordion title="Provider prefiltering">
    当凭证选择隐含首选提供商时，新手引导会将默认模型和允许列表选择器预筛选到该提供商。对于 Volcengine 和 BytePlus，这也会匹配 coding-plan 变体（`volcengine-plan/*`、`byteplus-plan/*`）。
  ...
  <Accordion title="Web-search follow-ups">
    ...
    - **Grok** 可以使用相同的 xAI OAuth 配置文件或 API 密钥提供可选的 `x_search` 设置，并选择 `x_search` 模型。
  ...
  <Accordion title="Other behaviors">
    ...
    - 自定义提供商：连接任意 OpenAI 或 Anthropic 兼容端点，包括未列出的托管提供商。使用 Unknown 进行自动检测。
    - 如果检测到 Hermes 状态，新手引导会提供迁移流程。使用 [迁移](/zh-CN/cli/migrate) 获取演练计划、覆盖模式、报告和精确映射。
...
将 `openclaw setup` 用作同一个引导式新手引导入口点。当你只需要基线配置/工作区时，使用 `openclaw setup --baseline`；之后如需定向更改，使用 `openclaw configure`；如需仅渠道设置，使用 `openclaw channels add`。
