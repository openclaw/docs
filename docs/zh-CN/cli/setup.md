...
read_when:
    ...
    - 你想设置默认工作区路径
    - 脚本需要仅基线设置标志
summary: '`openclaw setup` 的 CLI 参考（新手引导的别名，可通过标志使用基础设置）'
title: 设置
x-i18n:
    generated_at: "2026-07-11T20:25:46Z"
    model: gpt-5.6
...
它首先验证并持久化推理配置，然后启动 Crestodian，以配置
工作区、Gateway 网关、渠道、Skills 和健康状态。当你只需初始化配置/工作区文件夹而不运行向导时，请使用 `--baseline`。

在引导模式下，`--workspace <dir>` 是向 Crestodian 提议的工作区；
只有在你批准该提议后才会持久化。基线、经典和
...
[CLI 自动化](/zh-CN/start/wizard-cli-automation)。`openclaw onboard --modern` 是受推理门控的 Crestodian 助手的兼容性别名，`setup` 没有对应选项。
...
| 标志                       | 说明                                                                                           |
| -------------------------- | ----------------------------------------------------------------------------------------------------- |
| `--workspace <dir>`        | 引导模式中的工作区提议；基线、经典和非交互式设置会直接将其持久化。 |
| `--baseline`               | 不运行新手引导，创建基线配置、工作区和会话文件夹。                                  |
| `--wizard`                 | 为兼容性而接受；设置默认运行新手引导。                                         |
| `--non-interactive`        | 在无提示的情况下运行新手引导。                                                                       |
| `--accept-risk`            | 确认智能体具有完整系统访问权限的风险；与 `--non-interactive` 一起使用时为必需项。                         |
| `--mode <mode>`            | 新手引导模式：`local` 或 `remote`。                                                                 |
| `--flow <flow>`            | 新手引导流程：`quickstart`、`advanced`、`manual` 或 `import`。                                        |
| `--reset`                  | 在新手引导前重置配置、凭据和会话（仅当使用 `--reset-scope full` 时才重置工作区）。   |
| `--reset-scope <scope>`    | 重置范围：`config`、`config+creds+sessions` 或 `full`。                                            |
| `--import-from <provider>` | 新手引导期间运行迁移所使用的提供商。                                                          |
| `--import-source <path>`   | `--import-from` 的源智能体主目录。                                                                |
| `--import-secrets`         | 在新手引导迁移期间导入受支持的秘密信息。                                                 |
...
`openclaw setup --baseline` 保留旧版仅基线行为：它会
...
- 完成基线设置后，运行 `openclaw setup` 或 `openclaw onboard` 以体验完整的引导流程，运行 `openclaw configure` 进行针对性更改，或运行 `openclaw channels add` 添加渠道账户。
- 如果检测到 Hermes 状态，交互式新手引导可以自动提供迁移选项。导入式新手引导要求全新设置；如需在新手引导之外使用试运行计划、备份和覆盖模式，请使用[迁移](/zh-CN/cli/migrate)。
