---
read_when:
    - 你想要一个 Gateway 网关的终端 UI（适合远程使用）
    - 你想从脚本传递 url/token/session
    - 你想在没有 Gateway 网关的情况下以本地嵌入模式运行 TUI
    - 你需要使用 openclaw chat 或 openclaw tui --local
...
打开连接到 Gateway 网关的终端 UI，或在本地嵌入式模式下运行它。
...
| 标志                  | 默认值                                   | 描述                                                                        |
| --------------------- | ----------------------------------------- | ---------------------------------------------------------------------------------- |
| `--local`             | `false`                                   | 使用本地嵌入式智能体运行时运行，而不是 Gateway 网关。                 |
| `--url <url>`         | 配置中的 `gateway.remote.url`          | Gateway 网关 WebSocket URL。                                                             |
| `--token <token>`     | （无）                                    | 如果需要，则使用 Gateway 网关令牌。                                                         |
| `--password <pass>`   | （无）                                    | 如果需要，则使用 Gateway 网关密码。                                                      |
| `--session <key>`     | `main`（或作用域为全局时的 `global`） | 会话键。在智能体工作区内，除非加前缀，否则会自动选择该智能体。 |
...
| `--history-limit <n>` | `200`                                     | 附加连接时要加载的历史条目数。                                                 |
...
- 可行时，`tui` 会解析已配置的 Gateway 网关身份验证 SecretRefs，用于令牌/密码身份验证
  （`env`/`file`/`exec` 提供商）。
- 从已配置的智能体工作区目录内启动时，TUI 会为会话键默认值自动选择
  该智能体（除非 `--session` 被显式设置为
  `agent:<id>:...`）。
- 若要在页脚中为非本地、基于 URL 的连接显示 Gateway 网关主机名，
  请运行 `openclaw config set tui.footer.showRemoteHost true`。默认关闭；
  对回环或嵌入式本地连接永不显示。
- 本地模式会直接使用嵌入式智能体运行时。大多数本地工具可用，
  但仅 Gateway 网关支持的功能不可用。
- 本地模式会将 `/auth [provider]` 添加到 TUI 命令界面。
- 插件审批门禁在本地模式下仍然适用：需要审批的工具会在终端中
  提示进行决策，不会静默自动批准任何内容。
...
重新运行 `openclaw config validate`。参见 [TUI](/zh-CN/web/tui) 和
[配置](/zh-CN/cli/config)。
