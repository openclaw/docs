...
title: 消息
x-i18n:
    generated_at: "2026-06-27T01:39:09Z"
    model: gpt-5.5
...
- 当存在 `--channel` 或带渠道前缀的目标时，`openclaw message` 会将所选渠道解析到其所属插件；否则，它会加载已配置的渠道插件，用于默认渠道推断。
...
- 如果缓存未命中，当提供商支持时，OpenClaw 会尝试实时目录查找。
...
- 在可能的情况下，解析范围限定为活动操作目标：
  - 设置了 `--channel` 时按渠道限定范围（或从 `discord:...` 这样的带前缀目标推断）
  - 设置了 `--account` 时按账号限定范围（渠道全局 + 所选账号表面）
  - 省略 `--account` 时，OpenClaw 不会强制使用 `default` 账号 SecretRef 范围
- 无关渠道上未解析的 SecretRef 不会阻塞定向消息操作。
- 如果所选渠道/账号的 SecretRef 未解析，该命令会针对该操作以关闭方式失败。
...
- `send`
  ...
  - Telegram + WhatsApp：`--force-document`（将图片、GIF 和视频作为文档发送，以避免渠道压缩）
...
- `react`
  ...
  - 注意：`--remove` 需要 `--emoji`（省略 `--emoji` 可在支持的地方清除自己的反应；参见 /tools/reactions）
  - 仅 WhatsApp：`--participant`、`--from-me`
  - Signal 群组反应：必须提供 `--target-author` 或 `--target-author-uuid`
  - Nextcloud Talk：仅支持添加反应；`--remove` 会被拒绝并给出明确错误（参见 /tools/reactions）
...
- `read`
  ...
  - 仅 Slack：`--message-id` 读取特定 Slack 消息时间戳；与 `--thread-id` 组合可读取精确线程回复。
  - 仅 Discord：`--around`
...
核心会根据渠道能力，将同一个 `presentation` 载荷渲染为 Discord 组件、Slack 块、Telegram 内联按钮、Mattermost props，或 Teams/Feishu 卡片。完整契约和回退规则见 [消息呈现](/zh-CN/plugins/message-presentation)。
...
Telegram web app 按钮仅在用户与
机器人之间的私聊中受支持。使用 `web_app` 的旧 JSON 载荷仍可解析，但 `webApp` 是规范的呈现字段。
