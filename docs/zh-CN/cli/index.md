...
read_when:
    - 查找正确的 `openclaw` 子命令
    - 查找全局标志或输出样式规则
summary: OpenClaw CLI 索引：命令列表、全局标志及各命令页面链接
title: CLI 参考
x-i18n:
    generated_at: "2026-07-11T20:25:44Z"
    model: gpt-5.6
...
`openclaw` 是主要的 CLI 入口点。每个核心命令都有专门的参考页面，或与其别名对应的命令一起记录；本索引列出了适用于整个 CLI 的命令、全局标志和输出样式规则。

按用途划分的设置命令：

- `openclaw setup` 和 `openclaw onboard` 会先验证推理功能，然后启动 Crestodian，以设置 Gateway 网关、工作区、渠道、Skills 和健康状态。
- `openclaw setup --baseline` 会创建基线配置和工作区，而不会进入引导式新手引导流程。
- `openclaw configure` 会修改现有设置中的特定部分：模型身份验证、Gateway 网关、渠道、插件或 Skills。
- `openclaw channels add` 会在基线已存在后配置渠道账户；不带标志运行可进入引导式设置，带渠道专用标志运行则适用于脚本。
...
| 标志                    | 用途                                                                                                 |
| ----------------------- | ------------------------------------------------------------------------------------------------------- |
| `--dev`                 | 将状态隔离在 `~/.openclaw-dev` 下，将 Gateway 网关默认端口设为 19001，并相应调整派生端口              |
...
- 在支持的环境中，OSC-8 超链接会呈现为可点击链接；否则
  CLI 会回退为纯 URL。
- `--json`（以及支持时的 `--plain`）会禁用样式，以生成整洁的输出。
...
| 令牌          | 十六进制值       | 用途                             |
| -------------- | --------- | ------------------------------------ |
| `accent`       | `#FF5A2D` | 标题、标签、主要突出显示 |
| `accentBright` | `#FF7A3D` | 命令名称、强调              |
| `accentDim`    | `#D14A22` | 次要突出显示文本             |
| `info`         | `#FF8A5B` | 信息值                 |
| `success`      | `#2FBF71` | 成功状态                       |
...
独立演进；运行 `<command> --help` 可查看权威的当前列表。
...
- `/config`——持久化的配置更改。
- `/debug`——仅运行时的配置覆盖（存储在内存中，而非磁盘；需要 `commands.debug: true`）。

## 用量跟踪

当 OAuth/API 凭据可用时，`openclaw status --usage` 和 Control UI 会显示提供商的用量/配额。数据直接来自提供商的用量端点，并统一为 `X% left` 格式。目前提供用量周期数据的提供商包括：Anthropic、Gemini CLI、GitHub Copilot、MiniMax、OpenAI Codex、Xiaomi 和 z.ai。

有关详情，请参阅[用量跟踪](/zh-CN/concepts/usage-tracking)。
