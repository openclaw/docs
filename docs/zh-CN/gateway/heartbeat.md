...
title: Heartbeat
x-i18n:
    generated_at: "2026-05-12T23:30:23Z"
    model: gpt-5.5
    provider: openai
...
<Note>
**Heartbeat 与 cron？** 请参阅 [自动化](/zh-CN/automation)，了解何时使用两者。
</Note>
...
<Warning>
不要将 `start` 和 `end` 设置为相同时间（例如 `08:00` 到 `08:00`）。这会被视为零宽窗口，因此 Heartbeat 总会被跳过。
</Warning>
