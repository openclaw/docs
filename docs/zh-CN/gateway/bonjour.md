...
对 `local.` 的多播浏览是一种**仅限 LAN 的便捷机制**。对于跨网络发现，
同一个信标也可以通过已配置的广域 DNS-SD 域发布。发现
仍然是尽力而为的机制，**不能**替代 SSH 或基于 Tailnet 的连接。

## 通过 Tailscale 使用广域 Bonjour（单播 DNS-SD）

如果节点和 Gateway 网关位于不同网络，多播 mDNS 无法跨越该
...
3. 配置 Tailscale **split DNS**，使你选择的域通过该
...
只有 Gateway 网关会广播 `_openclaw-gw._tcp`。
...
Gateway 网关会广播一些非密钥的小提示，以便让 UI 流程更方便：
...
- iOS/Android 节点应将基于发现的直连视为**仅 TLS**，并在信任首次出现的指纹前要求用户进行明确确认。
...
- **浏览有效但解析失败**：请保持机器名简洁（避免使用 emoji 或
  标点符号），然后重启 Gateway 网关。服务实例名称派生自
  主机名，因此过于复杂的名称可能会让某些解析器混淆。

## 转义的实例名（`\032`）

Bonjour/DNS-SD 经常会将服务实例名中的字节转义为十进制 `\DDD`
序列（例如空格会变成 `\032`）。
...
- 设备发现策略和传输方式选择：[设备发现](/gateway/discovery)
