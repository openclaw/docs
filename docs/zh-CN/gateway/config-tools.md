...
title: 配置 — 工具和自定义提供商
x-i18n:
    generated_at: "2026-05-10T19:32:52Z"
    model: gpt-5.5
    provider: openai
...
`tools.*` 配置键和自定义提供商 / 基础 URL 设置。对于智能体、渠道和其他顶级配置键，请参阅[配置参考](/zh-CN/gateway/configuration-reference)。
...
<Note>
本地新手引导会在未设置时将新的本地配置默认设为 `tools.profile: "coding"`（保留现有的显式配置档）。
</Note>
...
- 每个智能体覆盖项（`agents.list[].tools.elevated`）只能进一步收紧限制。
- `/elevated on|off|ask|full` 会按会话存储状态；内联指令只应用于单条消息。
- 提升权限的 `exec` 会绕过沙箱隔离，并使用配置的逃逸路径（默认是 `gateway`，或者当 `exec` 目标为 `node` 时使用 `node`）。
...
工具循环安全检查**默认禁用**。设置 `enabled: true` 以启用检测。可以在 `tools.loopDetection` 中全局定义设置，并在 `agents.list[].tools.loopDetection` 中按智能体覆盖。
...
<ParamField path="detectors.knownPollNoProgress" type="boolean">
  对已知轮询工具（`process.poll`、`command_status` 等）的情况发出警告/阻止。
</ParamField>
...
<AccordionGroup>
  <Accordion title="媒体模型条目字段">
    ...
    **通用字段：**
    ...
    - `capabilities`：可选列表（`image`、`audio`、`video`）。默认值：`openai`/`anthropic`/`minimax` → 图片，`google` → 图片+音频+视频，`groq` → 音频。
    - `prompt`、`maxChars`、`maxBytes`、`timeoutSeconds`、`language`：按条目覆盖。
    - 当 agent 调用显式的 `image` 工具时，`tools.media.image.timeoutSeconds` 和匹配的图片模型 `timeoutSeconds` 条目也会生效。
    - 失败会回退到下一个条目。
    ...
    **异步完成字段：**
    ...
    - `asyncCompletion.directSend`：已弃用的兼容性标志。已完成的异步媒体任务保持由请求者会话中介，这样 agent 会收到结果，决定如何告知用户，并在源递送需要时使用消息工具。
...
控制会话工具（`sessions_list`、`sessions_history`、`sessions_send`）可以定向哪些会话。

默认值：`tree`（当前会话 + 由它派生的会话，例如子 agent）。
...
<AccordionGroup>
  <Accordion title="可见性范围">
    - `self`：仅当前会话键。
    - `tree`：当前会话 + 由当前会话派生的会话（子 agent）。
    - `agent`：属于当前 agent ID 的任何会话（如果你在同一 agent ID 下按发送者运行会话，则可能包括其他用户）。
    - `all`：任何会话。跨 agent 定向仍然需要 `tools.agentToAgent`。
    - 沙箱限制：当当前会话已沙箱隔离且 `agents.defaults.sandbox.sessionToolsVisibility="spawned"` 时，即使 `tools.sessions.visibility="all"`，可见性也会被强制为 `tree`。
...
<AccordionGroup>
  <Accordion title="附件说明">
    ...
    - 文件会在子工作区的 `.openclaw/attachments/<uuid>/` 中具现化，并带有 `.manifest.json`。
    - 附件内容会自动从转录持久化中遮盖。
    - Base64 输入会通过严格的字母表/填充检查和解码前大小防护进行验证。
...
实验性内置工具标志。默认关闭，除非严格智能体 GPT-5 自动启用规则适用。
...
- 默认：`false`，除非 `agents.defaults.embeddedPi.executionContract`（或单智能体覆盖）针对 OpenAI 或 OpenAI Codex GPT-5 系列运行设置为 `"strict-agentic"`。设置为 `true` 可在该范围之外强制开启此工具；设置为 `false` 可即使对严格智能体 GPT-5 运行也保持关闭。
- 启用后，系统提示还会添加使用指导，使模型只将其用于实质性工作，并且最多保持一个步骤为 `in_progress`。
...
- `model`：已生成子智能体的默认模型。如果省略，子智能体会继承调用方的模型。
...
<AccordionGroup>
  <Accordion title="身份验证和合并优先级">
    ...
    - 匹配提供商 ID 的合并优先级：
      ...
      - 非空智能体 `apiKey` 值仅在该提供商未由当前配置/认证配置文件上下文中的 SecretRef 管理时优先。
      ...
      - 标记持久化以源为准：标记会从活动源配置快照（解析前）写入，而不是从已解析的运行时密钥值写入。
...
<AccordionGroup>
  ...
  <Accordion title="提供商连接和身份验证">
    - `models.providers.*.api`：请求适配器（`openai-completions`、`openai-responses`、`anthropic-messages`、`google-generative-ai` 等）。对于自托管的 `/v1/chat/completions` 后端，例如 MLX、vLLM、SGLang 和大多数兼容 OpenAI 的本地服务器，使用 `openai-completions`。带有 `baseUrl` 但没有 `api` 的自定义提供商默认使用 `openai-completions`；仅当后端支持 `/v1/responses` 时才设置 `openai-responses`。
    ...
    - `models.providers.*.timeoutSeconds`：可选的单提供商模型 HTTP 请求超时，单位为秒，包括连接、headers、body 和总请求中止处理。
  ...
  <Accordion title="请求传输覆盖">
    ...
    - `request.allowPrivateNetwork`：当为 `true` 时，如果 DNS 解析到私有、CGNAT 或类似网段，则通过提供商 HTTP fetch 防护允许对 `baseUrl` 使用 HTTPS（操作员针对受信任自托管 OpenAI 兼容端点的显式选择）。除非此项明确设置为 `false`，否则诸如 `localhost`、`127.0.0.1` 和 `[::1]` 的回环模型提供商流 URL 会自动允许；LAN、tailnet 和私有 DNS 主机仍需要显式选择。WebSocket 对 headers/TLS 使用相同的 `request`，但不使用该 fetch SSRF gate。默认 `false`。

  </Accordion>
  <Accordion title="模型目录条目">
    - `models.providers.*.models`：显式提供商模型目录条目。
    - `models.providers.*.models.*.input`：模型输入模态。对纯文本模型使用 `["text"]`，对原生图像/视觉模型使用 `["text", "image"]`。只有在所选模型标记为支持图像时，图像附件才会注入智能体轮次。
    - `models.providers.*.models.*.contextWindow`：原生模型上下文窗口元数据。这会覆盖该模型的提供商级 `contextWindow`。
    - `models.providers.*.models.*.contextTokens`：可选的运行时上下文上限。这会覆盖提供商级 `contextTokens`；当你希望有效上下文预算小于模型原生 `contextWindow` 时使用它；`openclaw models list` 会在两个值不同时显示它们。
    - `models.providers.*.models.*.compat.supportsDeveloperRole`：可选的兼容性提示。对于带有非空非原生 `baseUrl`（主机不是 `api.openai.com`）的 `api: "openai-completions"`，OpenClaw 会在运行时将其强制为 `false`。空的/省略的 `baseUrl` 保持默认 OpenAI 行为。
    ...
    - `models.providers.*.models.*.compat.strictMessageKeys`：面向严格 OpenAI 兼容聊天端点的可选兼容性提示。当为 `true` 时，OpenClaw 会在发送请求前将传出的 Chat Completions 消息对象剥离为 `role` 和 `content`。
    - `models.providers.*.models.*.compat.thinkingFormat`：可选 thinking 载荷提示。对于顶层 `enable_thinking` 使用 `"qwen"`，或对于支持请求级 chat-template kwargs 的 Qwen 系列 OpenAI 兼容服务器（例如 vLLM）上的 `chat_template_kwargs.enable_thinking` 使用 `"qwen-chat-template"`。
...
交互式自定义提供商新手引导会为常见视觉模型 ID 推断图像输入，例如 GPT-4o、Claude、Gemini、Qwen-VL、LLaVA、Pixtral、InternVL、Mllama、MiniCPM-V 和 GLM-4V，并且会跳过已知纯文本系列的额外问题。未知模型 ID 仍会提示是否支持图像。非交互式新手引导使用相同推断；传入 `--custom-image-input` 可强制图像能力元数据，或传入 `--custom-text-input` 可强制纯文本元数据。
...
<AccordionGroup>
  ...
  <Accordion title="Local models (LM Studio)">
    参见 [Local Models](/zh-CN/gateway/local-models)。简而言之：在性能足够的硬件上，通过 LM Studio Responses API 运行大型本地模型；保留托管模型合并配置以便回退。
  </Accordion>
  ...
  <Accordion title="Moonshot AI (Kimi)">
    ...
    原生 Moonshot 端点会在共享的 `openai-completions` 传输上声明流式用量兼容性，OpenClaw 会根据端点能力启用它，而不是只根据内置提供商 ID。
  ...
  <Accordion title="Z.AI (GLM-4.7)">
    ...
    设置 `ZAI_API_KEY`。`z.ai/*` 和 `z-ai/*` 也可作为别名。快捷方式：`openclaw onboard --auth-choice zai-api-key`。
