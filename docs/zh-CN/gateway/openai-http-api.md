...
summary: 通过 Gateway 网关暴露一个兼容 OpenAI 的 /v1/chat/completions HTTP 端点
title: OpenAI 聊天补全
x-i18n:
...
底层会将请求作为普通 Gateway 网关智能体运行来执行（与 `openclaw agent` 相同代码路径），因此路由/权限/配置与你的 Gateway 网关一致。
...
请将此端点视为该 Gateway 网关实例的**完整操作者访问权限**表面。

- 这里的 HTTP bearer 身份验证不是狭义的按用户限定范围模型。
- 此端点的有效 Gateway 网关 token/password 应被视为所有者/操作者凭证。
- 请求会通过与可信操作者操作相同的控制平面智能体路径运行。
- 此端点没有单独的非所有者/按用户工具边界；一旦调用方在这里通过 Gateway 网关身份验证，OpenClaw 就会将该调用方视为此 Gateway 网关的可信操作者。
- 对于共享密钥身份验证模式（`token` 和 `password`），即使调用方发送更窄的 `x-openclaw-scopes` 标头，端点也会恢复正常的完整操作者默认值。
- 携带可信身份的 HTTP 模式（例如可信代理身份验证或 `gateway.auth.mode="none"`）会在存在 `x-openclaw-scopes` 时遵循它，否则回退到正常的操作者默认范围集。
- 如果目标智能体策略允许敏感工具，此端点就可以使用它们。
- 仅将此端点保留在回环/tailnet/私有入口中；不要将它直接暴露到公共互联网。
...
- 携带可信身份的 HTTP 模式（例如可信代理身份验证，或私有入口上的 `gateway.auth.mode="none"`）
  ...
  - 当标头不存在时回退到正常的操作者默认范围集
  - 只有当调用方显式缩窄范围并省略 `operator.admin` 时，才会失去所有者语义
...
<AccordionGroup>
  ...
  <Accordion title="为什么包含 `openclaw/default`？">
    ...
    这意味着即使真实的默认智能体 ID 在不同环境之间变化，客户端也可以继续使用一个可预测的 ID。
...
- `tool_choice: "required"`（运行时尚未强制执行；实现强制执行后将受支持）
...
如果 `stream_options.include_usage=true`，会在 `[DONE]` 之前发出尾随的用量分块。

### 工具后续循环

收到 `tool_calls` 后，客户端应执行所请求的函数，并发送包含以下内容的后续请求：
...
- 如果你想为该智能体使用特定后端提供商/模型，请设置智能体的正常默认模型，或发送 `x-openclaw-model`
...
如果它返回 `openclaw/default`，大多数 Open WebUI 设置都可以使用相同的 Base URL 和 token 连接。
