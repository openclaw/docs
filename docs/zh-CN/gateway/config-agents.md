...
title: 配置 —— 智能体
x-i18n:
    generated_at: "2026-04-24T18:43:58Z"
    model: gpt-5.4
    provider: openai
...
- 将 `agents.list[].skills: []` 设为空，表示不使用任何 Skills。
...
- `"continuation-skip"`：安全的续接轮次（在助手完成响应之后）会跳过工作区引导内容的重新注入，从而减小提示词大小。Heartbeat 运行和压缩后重试仍会重建上下文。
...
OpenClaw 有多个高容量的提示词/上下文预算，这些预算是有意按子系统拆分的，而不是都通过一个通用开关统一控制。
...
- `imageGenerationModel`：接受字符串（`"provider/model"`）或对象（`{ primary, fallbacks }`）两种形式。
  ...
  - 如果你直接选择某个 provider/model，也要配置匹配的提供商凭证（例如，`google/*` 对应 `GEMINI_API_KEY` 或 `GOOGLE_API_KEY`，`openai/gpt-image-2` 对应 `OPENAI_API_KEY` 或 OpenAI Codex OAuth，`fal/*` 对应 `FAL_KEY`）。
  - 如果省略此项，`image_generate` 仍可推断出有认证支持的提供商默认值。它会先尝试当前默认提供商，然后按提供商 id 顺序尝试其余已注册的图像生成提供商。
- `musicGenerationModel`：接受字符串（`"provider/model"`）或对象（`{ primary, fallbacks }`）两种形式。
  ...
  - 如果省略此项，`music_generate` 仍可推断出有认证支持的提供商默认值。它会先尝试当前默认提供商，然后按提供商 id 顺序尝试其余已注册的音乐生成提供商。
  - 如果你直接选择某个 provider/model，也要同时配置匹配的提供商认证/API 密钥。
- `videoGenerationModel`：接受字符串（`"provider/model"`）或对象（`{ primary, fallbacks }`）两种形式。
  - 由共享的视频生成能力和内置的 `video_generate` 工具使用。
  - 典型值：`qwen/wan2.6-t2v`、`qwen/wan2.6-i2v`、`qwen/wan2.6-r2v`、`qwen/wan2.6-r2v-flash` 或 `qwen/wan2.7-r2v`。
  - 如果省略此项，`video_generate` 仍可推断出有认证支持的提供商默认值。它会先尝试当前默认提供商，然后按提供商 id 顺序尝试其余已注册的视频生成提供商。
  - 如果你直接选择某个 provider/model，也要同时配置匹配的提供商认证/API 密钥。
...
- `model.primary`：格式为 `provider/model`（例如，`openai/gpt-5.4` 用于 API key 访问，或 `openai-codex/gpt-5.5` 用于 Codex OAuth）。如果你省略 provider，OpenClaw 会先尝试别名，然后尝试与该确切模型 id 唯一匹配的已配置提供商，最后才回退到已配置的默认提供商（这是已弃用的兼容行为，因此推荐显式写成 `provider/model`）。如果该提供商不再暴露已配置的默认模型，OpenClaw 会回退到第一个已配置的 provider/model，而不是继续暴露过期的已移除提供商默认值。
- `models`：用于 `/model` 的已配置模型目录和允许列表。每个条目可包含 `alias`（快捷别名）和 `params`（提供商特定参数，例如 `temperature`、`maxTokens`、`cacheRetention`、`context1m`、`responsesServerCompaction`、`responsesCompactThreshold`）。
  ...
  - 以提供商为作用域的配置/新手引导流程会将所选提供商模型合并到此映射中，并保留已配置的其他无关提供商。
...
- `embeddedHarness`：默认的底层嵌入式智能体运行时策略。使用 `runtime: "auto"` 可让已注册的插件 harness 接管支持的模型，使用 `runtime: "pi"` 可强制使用内置 PI harness，或使用已注册的 harness id，如 `runtime: "codex"`。将 `fallback: "none"` 设为禁用自动 PI 回退。新的 Codex harness 配置应将模型引用保持为规范形式 `openai/*`，并在此处选择 harness，而不要使用旧版 `codex/*` 模型引用。
- 会修改这些字段的配置写入器（例如 `/models set`、`/models set-image` 以及故障切换 add/remove 命令）会保存为规范对象形式，并尽可能保留现有的故障切换列表。
...
- `fallback`：`"pi"` 或 `"none"`。`"pi"` 会在未选中任何插件 harness 时保留内置 PI harness 作为兼容性回退。`"none"` 会在缺失或不支持所选插件 harness 时直接失败，而不是静默使用 PI。所选插件 harness 的失败始终会直接暴露出来。
...
- 首次嵌入式运行后，harness 选择会按会话 id 固定。配置/环境变量更改只影响新的或已重置的会话，不影响现有转录。带有转录历史但没有记录固定项的旧会话会被视为固定到 PI。`/status` 会在 `Fast` 旁显示非 PI 的 harness id，例如 `codex`。
...
用于纯文本回退运行（无工具调用）的可选 CLI 后端。当 API 提供商失败时，这可以作为备份方案。
...
用固定字符串替换整个由 OpenClaw 组装的系统提示词。可在默认级别（`agents.defaults.systemPromptOverride`）或按智能体级别（`agents.list[].systemPromptOverride`）设置。按智能体的值优先；空值或仅包含空白的值会被忽略。适用于受控的提示词实验。
...
按模型家族应用的、与提供商无关的提示词叠加层。GPT-5 系列模型 id 会在各提供商之间接收共享行为契约；`personality` 只控制友好交互风格这一层。
  ...
  agents: {
    defaults: {
      heartbeat: {
        ...
        includeSystemPromptSection: true, // 默认值：true；false 表示从系统提示词中省略 Heartbeat 部分
        lightContext: false, // 默认值：false；true 表示在工作区引导文件中仅保留 HEARTBEAT.md
        isolatedSession: false, // 默认值：false；true 表示每次 heartbeat 都在全新会话中运行（无对话历史）
        session: "main",
        to: "+15555550123",
...
- `timeoutSeconds`：heartbeat 智能体轮次在被中止前允许的最长秒数。不设置时使用 `agents.defaults.timeoutSeconds`。
- `directPolicy`：直发/私信投递策略。`allow`（默认）允许向直达目标投递。`block` 会阻止向直达目标投递，并输出 `reason=dm-blocked`。
- `lightContext`：设为 true 时，heartbeat 运行使用轻量级引导上下文，并且在工作区引导文件中仅保留 `HEARTBEAT.md`。
- `isolatedSession`：设为 true 时，每次 heartbeat 都在无任何先前对话历史的全新会话中运行。隔离模式与 cron `sessionTarget: "isolated"` 相同。可将每次 heartbeat 的 token 成本从约 100K 降至约 2-5K。
  ...
  agents: {
    defaults: {
      compaction: {
        ...
        notifyUser: true, // 在 compaction 开始和完成时向用户发送简短通知（默认值：false）
        memoryFlush: {
          enabled: true,
...
- `mode`：`default` 或 `safeguard`（针对长历史的分块摘要）。参见[上下文压缩](/zh-CN/concepts/compaction)。
- `provider`：已注册的 compaction provider 插件 id。设置后，会调用该提供商的 `summarize()`，而不是使用内置 LLM 摘要。失败时会回退到内置方式。设置提供商会强制使用 `mode: "safeguard"`。参见[上下文压缩](/zh-CN/concepts/compaction)。
...
- `postCompactionSections`：compaction 后重新注入的可选 `AGENTS.md` H2/H3 章节名。默认值为 `["Session Startup", "Red Lines"]`；设为 `[]` 可禁用重新注入。当未设置，或显式设为这组默认值时，也接受旧版的 `Every Session`/`Safety` 标题作为兼容回退。
- `model`：仅用于 compaction 摘要的可选 `provider/model-id` 覆盖项。当主会话应保留一个模型，而 compaction 摘要应在另一个模型上运行时使用；未设置时，compaction 使用会话的主模型。
...
在将内容发送给 LLM 之前，从内存中的上下文中裁剪**旧的工具结果**。**不会**修改磁盘上的会话历史。
...
- 裁剪会先对过大的工具结果进行软裁剪，如果仍有需要，再对更旧的工具结果进行硬清除。
...
- `workspaceRoot`：用于按作用域工作区的远端绝对根目录
- `identityFile` / `certificateFile` / `knownHostsFile`：传递给 OpenSSH 的现有本地文件
- `identityData` / `certificateData` / `knownHostsData`：内联内容或 SecretRef，OpenClaw 会在运行时将其写入临时文件
...
- 在创建或重建后对远端工作区执行一次初始化
- 然后将远端 SSH 工作区保持为规范副本
...
- `mirror`：在执行前将本地内容初始化到远端，执行后再同步回本地；本地工作区保持为规范副本
- `remote`：在创建沙箱时仅对远端初始化一次，此后远端工作区保持为规范副本

在 `remote` 模式下，于 OpenClaw 外部在主机本地做出的编辑，在初始化步骤之后不会自动同步到沙箱中。
传输方式是通过 SSH 连接到 OpenShell 沙箱，但插件负责沙箱生命周期以及可选的镜像同步。

**`setupCommand`** 在容器创建后仅运行一次（通过 `sh -lc`）。需要网络出口、可写根文件系统和 root 用户。

**容器默认使用 `network: "none"`** —— 如果智能体需要对外访问，请将其设置为 `"bridge"`（或自定义 bridge 网络）。
...
noVNC 观察者访问默认使用 VNC 认证，并且 OpenClaw 会发出短时效 token URL（而不是在共享 URL 中暴露密码）。
...
- `cdpSourceRange` 可选，用于在容器边界将 CDP 入站访问限制到某个 CIDR 范围（例如 `172.21.0.1/32`）。
...
- 启动默认值定义在 `scripts/sandbox-browser-entrypoint.sh` 中，并针对容器主机进行了调优：
  ...
  - 默认值是容器镜像的基线；如需更改容器默认行为，请使用带有自定义
    ...
    list: [
      {
        ...
        thinkingDefault: "high", // 按智能体覆盖默认 thinking 级别
        reasoningDefault: "on", // 按智能体覆盖默认 reasoning 可见性
        fastModeDefault: false, // 按智能体覆盖默认 fast mode
...
- `id`：稳定的智能体 id（必填）。
- `default`：若设置了多个，以第一个为准（会记录警告）。如果一个都没设置，则列表中的第一个条目为默认值。
- `model`：字符串形式仅覆盖 `primary`；对象形式 `{ primary, fallbacks }` 同时覆盖两者（`[]` 表示禁用全局故障切换）。仅覆盖 `primary` 的 cron 任务仍会继承默认故障切换，除非你设置 `fallbacks: []`。
- `params`：按智能体的流参数，会合并到 `agents.defaults.models` 中选定模型条目之上。用它来做智能体专属覆盖，例如 `cacheRetention`、`temperature` 或 `maxTokens`，而无需复制整个模型目录。
- `skills`：可选的按智能体 Skills 允许列表。如果省略，则在设置了 `agents.defaults.skills` 时继承默认值；显式列表会替换默认值而不是合并，`[]` 表示无 Skills。
...
- `fastModeDefault`：可选的按智能体默认 fast mode（`true | false`）。当未设置按消息或按会话 fast-mode 覆盖时生效。
- `embeddedHarness`：可选的按智能体底层 harness 策略覆盖。使用 `{ runtime: "codex", fallback: "none" }` 可让某个智能体仅使用 Codex，而其他智能体保留默认的 PI 回退。
- `runtime`：可选的按智能体运行时描述符。当智能体应默认使用 ACP harness 会话时，可使用 `type: "acp"` 配合 `runtime.acp` 默认值（`agent`、`backend`、`mode`、`cwd`）。
...
- 沙箱继承保护：如果请求方会话位于沙箱中，`sessions_spawn` 会拒绝会导致目标在非沙箱中运行的请求。
...
在同一个 Gateway 网关中运行多个彼此隔离的智能体。参见[多智能体](/zh-CN/concepts/multi-agent)。
...
- `type`（可选）：普通路由使用 `route`（未设置 type 时默认就是 route），持久 ACP 对话绑定使用 `acp`。
- `match.channel`（必填）
...
```json5
{
  session: {
    ...
    threadBindings: {
      ...
      maxAgeHours: 0, // 默认按小时计的硬性最大存续时间（`0` 表示禁用）
    },
...
- **`reset`**：主重置策略。`daily` 会在本地时间的 `atHour` 重置；`idle` 会在空闲 `idleMinutes` 后重置。如果两者都已配置，则以先到期者为准。
- **`resetByType`**：按类型覆盖（`direct`、`group`、`thread`）。旧版 `dm` 也可作为 `direct` 的别名。
...
- **`mainKey`**：旧版字段。运行时始终将 `"main"` 用于主直聊分桶。
- **`agentToAgent.maxPingPongTurns`**：智能体之间交流时，允许的最大来回回复轮次数（整数，范围：`0`–`5`）。`0` 表示禁用乒乓式链式回复。
- **`sendPolicy`**：按 `channel`、`chatType`（`direct|group|channel`，旧版 `dm` 可作别名）、`keyPrefix` 或 `rawKeyPrefix` 匹配。第一个 deny 优先。
- **`maintenance`**：会话存储清理 + 保留控制。
  - `mode`：`warn` 只发出警告；`enforce` 则实际执行清理。
  - `pruneAfter`：陈旧条目的保留期限截止（默认 `30d`）。
...
- **`threadBindings`**：线程绑定会话功能的全局默认值。
  ...
  - `maxAgeHours`：默认按小时计的硬性最大存续时间（`0` 表示禁用；提供商可覆盖）
...
- `messages.statusReactions.enabled`：在 Slack、Discord 和 Telegram 上启用生命周期状态反应。
  在 Slack 和 Discord 上，未设置时如果确认反应处于启用状态，则状态反应保持启用。
  在 Telegram 上，需要显式设为 `true` 才会启用生命周期状态反应。

### 入站去抖

将同一发送者快速连续发送的纯文本消息合并为一次智能体轮次。媒体/附件会立即刷新。控制命令会绕过去抖。
...
- `openai.baseUrl` 会覆盖 OpenAI TTS 端点。解析顺序为：配置，然后是 `OPENAI_TTS_BASE_URL`，最后是 `https://api.openai.com/v1`。
...
- `silenceTimeoutMs` 控制 Talk 模式在用户静音后等待多久才发送转录。未设置时保持平台默认停顿窗口（`macOS 和 Android 上为 700 ms，iOS 上为 900 ms`）。
