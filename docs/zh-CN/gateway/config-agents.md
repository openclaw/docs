...
summary: 智能体默认设置、多 Agent 路由、会话、消息和 Talk 配置
title: 配置 — 智能体
x-i18n:
...
例如上下文/状态报告和日志；常规 WebChat 用户/运行时上下文只会
获得简洁的恢复通知。
...
- `agents.defaults.startupContext.*`：
  ...
  `memory/*.md` 文件。裸聊天 `/new` 和 `/reset` 命令会在不调用模型的情况下得到确认。
- `skills.limits.*`：
  注入系统提示词的紧凑技能列表。
...
- `params.preserveThinking`：仅限 Z.AI 的保留 thinking 选择加入项。启用且 thinking 开启时，OpenClaw 会发送 `thinking.clear_thinking: false` 并重放先前的 `reasoning_content`；参阅 [Z.AI thinking 和保留 thinking](/zh-CN/providers/zai#thinking-and-preserved-thinking)。
...
除非你设置 `--thinking off` 或自行定义 `agents.defaults.models["zai/<model>"].params.thinking`，否则 Z.AI GLM-4.x 模型会自动启用思考模式。
...
- `postCompactionSections`：压缩后要重新注入的可选 AGENTS.md H2/H3 分节名称。默认为 `["Session Startup", "Red Lines"]`；设置 `[]` 可禁用重新注入。未设置或显式设置为该默认成对值时，也接受较旧的 `Every Session`/`Safety` 标题作为旧版回退。
...
在 `remote` 模式下，在主机本地、OpenClaw 之外进行的编辑不会在初始化步骤之后自动同步到沙箱中。
