...
status: active
summary: 工具被阻止的原因：沙箱运行时、工具允许/拒绝策略和提权 exec 门禁
title: 沙箱、工具策略与提权
x-i18n:
    generated_at: "2026-05-05T16:51:30Z"
    model: gpt-5.5
    provider: openai
...
3. **提权**（`tools.elevated.*`、`agents.list[].tools.elevated.*`）是一个**仅限 exec 的逃生口**，用于在你处于沙箱隔离时在沙箱外运行（默认是 `gateway`，或者当 exec 目标配置为 `node` 时为 `node`）。

## 快速调试

使用检查器查看 OpenClaw _实际_ 正在做什么：
...
- `"off"`：所有内容都在主机上运行。
- `"non-main"`：只有非 main 会话会被沙箱隔离（群组/渠道中常见的“意外”来源）。
- `"all"`：所有内容都会被沙箱隔离。
...
- `scope: "shared"` 会忽略每个 agent 的绑定挂载（只应用全局绑定挂载）。
- OpenClaw 会验证绑定源两次：先验证规范化后的源路径，然后在通过最深的现有祖先解析后再次验证。符号链接父级逃逸无法绕过被阻止路径或允许根目录检查。
- 不存在的叶子路径仍会被安全检查。如果 `/workspace/alias-out/new-file` 通过符号链接父级解析到被阻止路径，或解析到配置的允许根目录之外，该绑定会被拒绝。
- 绑定 `/var/run/docker.sock` 实际上会把主机控制权交给沙箱；只有在明确需要时才这样做。
...
- `/exec` 只会更改已授权发送者的会话默认值；它不会授予工具访问权限。
...
- 如果你处于沙箱隔离状态，`/elevated on`（或带有 `elevated: true` 的 `exec`）会在沙箱外运行（仍可能需要审批）。
- 使用 `/elevated full` 可跳过该会话的 exec 审批。
- 如果你已经在直接运行，提权实际上是无操作（仍受门控约束）。
- 提权**不是**按技能作用域划分的，也**不会**覆盖工具允许/拒绝规则。
- 提权不会从 `host=auto` 授予任意跨主机覆盖；它遵循正常的 exec 目标规则，并且只有在已配置/会话目标已经是 `node` 时才保留 `node`。
- `/exec` 与提权是分开的。它只会为已授权发送者调整每个会话的 exec 默认值。
...
- 禁用沙箱：`agents.defaults.sandbox.mode=off`（或每个 agent 的 `agents.list[].sandbox.mode=off`）
- 在沙箱内允许该工具：
  - 从 `tools.sandbox.tools.deny` 中移除它（或每个 agent 的 `agents.list[].tools.sandbox.tools.deny`）
  - 或将它添加到 `tools.sandbox.tools.allow`（或每个 agent 的 allow）
...
- [多 Agent 沙箱和工具](/zh-CN/tools/multi-agent-sandbox-tools) -- 每个 agent 的覆盖规则和优先级
