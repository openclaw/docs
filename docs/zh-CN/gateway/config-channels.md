...
read_when:
    ...
    - 按渠道配置键名的故障排除
    - 审核私信政策、群组政策或提及门控
summary: 渠道配置：涵盖 Slack、Discord、Telegram、WhatsApp、Matrix、iMessage 等渠道的访问控制、配对和每个渠道的密钥
title: 配置 — 渠道
x-i18n:
    generated_at: "2026-05-02T06:09:01Z"
    model: gpt-5.5
    provider: openai
...
使用 `channels.modelByChannel` 将特定渠道 ID 固定到某个模型。值接受 `provider/model` 或已配置的模型别名。当会话尚未有模型覆盖时（例如通过 `/model` 设置），渠道映射会生效。
...
- `channels.defaults.contextVisibility`：所有渠道的默认补充上下文可见性模式。取值：`all`（默认，包含所有引用/线程/历史上下文）、`allowlist`（仅包含来自允许列表发送者的上下文）、`allowlist_quote`（与 allowlist 相同，但保留明确的引用/回复上下文）。各渠道覆盖：`channels.<channel>.contextVisibility`。
...
- Bot 令牌：`channels.telegram.botToken` 或 `channels.telegram.tokenFile`（仅限常规文件；拒绝符号链接），默认账号回退使用 `TELEGRAM_BOT_TOKEN`。
...
- 在多账号设置（2 个或更多账号 ID）中，设置明确的默认值（`channels.telegram.defaultAccount` 或 `channels.telegram.accounts.default`）以避免回退路由；当它缺失或无效时，`openclaw doctor` 会发出警告。
- `configWrites: false` 会阻止由 Telegram 发起的配置写入（超级群 ID 迁移、`/config set|unset`）。
...
- 令牌：`channels.discord.token`，默认账户的回退值为 `DISCORD_BOT_TOKEN`。
...
- 服务器 slug 使用小写并将空格替换为 `-`；渠道键使用 slug 化名称（不含 `#`）。优先使用服务器 ID。
- 默认忽略由机器人撰写的消息。`allowBots: true` 会启用它们；使用 `allowBots: "mentions"` 仅接受提及该机器人的机器人消息（自己的消息仍会被过滤）。
...
- `channels.discord.threadBindings` 控制 Discord 线程绑定路由：
  ...
  - `maxAgeHours`：Discord 对硬性最大时长小时数的覆盖（`0` 表示禁用）
  - `spawnSessions`：用于 `sessions_spawn({ thread: true })` 和 ACP 线程生成自动创建/绑定线程的开关（默认：`true`）
  - `defaultSpawnContext`：线程绑定生成的原生子智能体上下文（默认是 `"fork"`）
- 带有 `type: "acp"` 的顶层 `bindings[]` 条目会为渠道和线程配置持久 ACP 绑定（在 `match.peer.id` 中使用渠道/线程 ID）。字段语义在 [ACP 智能体](/zh-CN/tools/acp-agents#channel-specific-settings) 中共享。
- `channels.discord.ui.components.accentColor` 设置 Discord components v2 容器的强调色。
- `channels.discord.voice` 启用 Discord 语音渠道对话，以及可选的自动加入 + LLM + TTS 覆盖。纯文本 Discord 配置默认关闭语音；设置 `channels.discord.voice.enabled=true` 以启用。
...
- `channels.discord.execApprovals`：Discord 原生 exec 审批投递和审批人授权。
  ...
  - `target`：发送审批提示的位置。`"dm"`（默认）发送到审批人私信，`"channel"` 发送到来源渠道，`"both"` 发送到两者。当目标包含 `"channel"` 时，按钮只能由已解析的审批人使用。
...
- **HTTP 模式**需要 `botToken` 加 `signingSecret`（在根级别或每账户级别）。
...
- Slack 账户快照会公开每凭证的来源/状态字段，例如 `botTokenSource`、`botTokenStatus`、`appTokenStatus`，以及在 HTTP 模式下的 `signingSecretStatus`。`configured_unavailable` 表示账户通过 SecretRef 配置，但当前命令/运行时路径无法解析密钥值。
...
- Slack 原生流式传输加 Slack 助手风格的 “is typing...” 线程状态需要回复线程目标。顶层私信默认保持在线程外，因此它们会使用 `typingReaction` 或普通投递，而不是线程风格预览。
- `typingReaction` 会在回复运行时向入站 Slack 消息添加一个临时回应，然后在完成时移除它。使用 Slack emoji 短代码，例如 `"hourglass_flowing_sand"`。
...
- 原生 slash 回调使用 Mattermost 在 slash command 注册期间返回的每命令 token 进行认证。如果注册失败或没有命令被激活，OpenClaw 会用 `Unauthorized: invalid command token.` 拒绝回调。
...
OpenClaw 会生成 `imsg rpc`（通过 stdio 传输 JSON-RPC）。不需要守护进程或端口。
...
- 完整 Teams 配置（凭据、webhook、私信/群组策略、按团队/按渠道覆盖）记录在 [Microsoft Teams](/zh-CN/channels/msteams)。
...
- 完整 IRC 渠道配置（host/port/TLS/channels/allowlists/mention gating）记录在 [IRC](/zh-CN/channels/irc)。
...
- 如果你通过 `openclaw channels add`（或渠道新手引导）添加非默认账户，同时仍使用单账户顶层渠道配置，OpenClaw 会先将账户作用域的顶层单账户值提升到渠道账户映射中，以便原账户继续工作。大多数渠道会将它们移入 `channels.<channel>.accounts.default`；Matrix 则可以保留现有的匹配命名/默认目标。
- 现有仅渠道绑定（无 `accountId`）继续匹配默认账户；账户作用域绑定仍然是可选的。
- `openclaw doctor --fix` 也会通过将账户作用域的顶层单账户值移入为该渠道选择的提升账户来修复混合形状。大多数渠道使用 `accounts.default`；Matrix 则可以保留现有的匹配命名/默认目标。

### 其他插件渠道

许多插件渠道配置为 `channels.<id>`，并在各自专门的渠道页面中记录（例如 Feishu、Matrix、LINE、Nostr、Zalo、Nextcloud Talk、Synology Chat 和 Twitch）。
...
可见回复单独控制。群组/渠道房间默认为 `messages.groupChat.visibleReplies: "message_tool"`：OpenClaw 仍会处理该轮对话，但普通最终回复保持私密，可见房间输出需要 `message(action=send)`。仅当你想要旧版行为，即普通回复会发回房间时，才设置 `"automatic"`。要将相同的仅工具可见回复行为也应用于直接聊天，请设置 `messages.visibleReplies: "message_tool"`；Codex harness 也将该仅工具行为用作未设置时的直接聊天默认值。

如果消息工具在活动工具策略下不可用，OpenClaw 会回退到自动可见回复，而不是静默抑制响应。`openclaw doctor` 会警告此不匹配。
...
`messages.groupChat.historyLimit` 设置全局默认值。渠道可以使用 `channels.<channel>.historyLimit`（或按账户）覆盖。设置为 `0` 可禁用。

`messages.visibleReplies` 是全局来源轮次默认值；`messages.groupChat.visibleReplies` 会为群组/渠道来源轮次覆盖它。当 `messages.visibleReplies` 未设置时，harness 可以提供自己的直接/来源默认值；Codex harness 默认为 `message_tool`。渠道允许列表和提及门控仍会决定是否处理某一轮。
...
- 此块配置命令界面。有关当前内置 + 内置命令目录，请参阅 [Slash Commands](/zh-CN/tools/slash-commands)。
- 本页是**配置键参考**，不是完整命令目录。由渠道/插件拥有的命令，例如 QQ Bot `/bot-ping` `/bot-help` `/bot-logs`、LINE `/card`、设备配对 `/pair`、memory `/dreaming`、phone-control `/phone` 和 Talk `/voice`，记录在其渠道/插件页面以及 [Slash Commands](/zh-CN/tools/slash-commands) 中。
- 文本命令必须是以 `/` 开头的**独立**消息。
...
- `mcp: true` 为 `mcp.servers` 下的 OpenClaw 管理的 MCP 服务器配置启用 `/mcp`。
- `plugins: true` 启用 `/plugins`，用于插件发现、安装以及启用/禁用控制。
- `channels.<provider>.configWrites` 按渠道门控配置变更（默认：true）。
- 对于多账户渠道，`channels.<provider>.accounts.<id>.configWrites` 也会门控针对该账户的写入（例如 `/allowlist --config --account <id>` 或 `/config set channels.<provider>.accounts.<id>...`）。
...
- 命令文档映射：
  - 内置 + 内置目录：[Slash Commands](/zh-CN/tools/slash-commands)
