...
title: 配置
x-i18n:
    generated_at: "2026-05-10T19:33:14Z"
    model: gpt-5.5
    provider: openai
...
文档。将此页面用于面向任务的指导，并将
[配置参考](/zh-CN/gateway/configuration-reference) 用于更全面的
字段映射和默认值。

<Tip>
**刚开始使用配置？** 从 `openclaw onboard` 开始进行交互式设置，或查看 [配置示例](/zh-CN/gateway/configuration-examples)指南，获取完整的可复制粘贴配置。
</Tip>
...
SecretRef 详细信息（包括用于 `env`/`file`/`exec` 的 `secrets.providers`）位于 [密钥管理](/zh-CN/gateway/secrets)。
