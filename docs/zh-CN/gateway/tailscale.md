...
summary: 为 Gateway 网关仪表板集成的 Tailscale Serve/Funnel
title: Tailscale
x-i18n:
...
# Tailscale（Gateway 网关仪表板）

OpenClaw 可以为 Gateway 网关仪表板和 WebSocket 端口自动配置 Tailscale **Serve**（tailnet）或 **Funnel**（公开访问）。这样可以让 Gateway 网关继续绑定在 loopback 上，同时由 Tailscale 提供 HTTPS、路由，以及（对于 Serve）身份头。

## 模式

- `serve`：通过 `tailscale serve` 提供仅限 tailnet 的 Serve。gateway 保持在 `127.0.0.1` 上。
...
- `trusted-proxy`（具备身份感知能力的反向代理；参见[受信任代理认证](/gateway/trusted-proxy-auth)）

当 `tailscale.mode = "serve"` 且 `gateway.auth.allowTailscale` 为 `true` 时，
控制 UI/WebSocket 认证可以使用 Tailscale 身份头
（`tailscale-user-login`），而无需提供 token/password。OpenClaw 会通过本地 Tailscale
守护进程（`tailscale whois`）解析 `x-forwarded-for` 地址，并在接受前将其与头部进行匹配，从而验证该身份。
只有当请求来自 loopback，并带有 Tailscale 的 `x-forwarded-for`、`x-forwarded-proto` 和 `x-forwarded-host`
头时，OpenClaw 才会将其视为 Serve 请求。
HTTP API 端点（例如 `/v1/*`、`/tools/invoke` 和 `/api/channels/*`）
**不会**使用 Tailscale 身份头认证。它们仍然遵循 gateway 的
常规 HTTP 认证模式：默认使用共享密钥认证，或使用有意配置的 trusted-proxy / 私有入口 `none` 设置。
这种无 token 流程假定 gateway 主机是可信的。如果同一主机上可能运行不受信任的本地代码，请禁用 `gateway.auth.allowTailscale`，并改为要求 token/password 认证。
...
优先使用 `OPENCLAW_GATEWAY_PASSWORD`，不要把密码提交到磁盘中。
...
- `gateway.bind: "auto"` 会优先选择 loopback；如果你只想使用 Tailnet，请使用 `tailnet`。
...
不要将 Funnel 用于浏览器控制；应像对待 operator 访问一样对待节点配对。
...
- Funnel 仅支持通过 TLS 使用 `443`、`8443` 和 `10000` 端口。
