...
read_when:
    ...
    - 你需要给定发现项对应的修复键/路径
    - 你正在对一次安全审计运行中的严重性进行分流
summary: openclaw security audit 发出的 checkIds 参考目录
title: 安全审计检查
x-i18n:
    generated_at: "2026-07-01T05:28:40Z"
    model: gpt-5.5
...
`openclaw security audit` 会输出以 `checkId` 为键的结构化发现。本页是这些 ID 的参考目录。有关高层威胁模型和加固指南，请参阅[安全](/zh-CN/gateway/security)。

你在真实部署中最可能看到的高价值 `checkId` 值（并非详尽列表）：
...
| `fs.auth_profiles.perms_readable`                             | 警告          | 其他人可以读取 API keys 和 OAuth 令牌                                            | `agents/<agentId>/agent/auth-profiles.json` 权限                                                    | 是      |
...
| `gateway.loopback_no_auth`                                    | 严重      | 经反向代理的回环可能变为未认证                                  | `gateway.auth.*`, 代理设置                                                                        | 否       |
| `gateway.trusted_proxies_missing`                             | 警告          | 存在反向代理标头但未受信任                                    | `gateway.trustedProxies`                                                                             | 否       |
| `gateway.http.no_auth`                                        | 警告/严重 | Gateway 网关 HTTP API 在 `auth.mode="none"` 下可访问                                  | `gateway.auth.mode`, `gateway.http.endpoints.*`, `plugins.entries.admin-http-rpc`                    | 否       |
...
| `gateway.control_ui.allowed_origins_required`                 | 严重      | 非回环 Control UI 缺少显式浏览器源允许列表                    | `gateway.controlUi.allowedOrigins`                                                                   | 否       |
| `gateway.control_ui.allowed_origins_wildcard`                 | 警告/严重 | `allowedOrigins=["*"]` 会禁用浏览器源允许列表                          | `gateway.controlUi.allowedOrigins`                                                                   | 否       |
| `gateway.control_ui.host_header_origin_fallback`              | 警告/严重 | 启用 Host 标头源回退（降低 DNS 重绑定加固强度）              | `gateway.controlUi.dangerouslyAllowHostHeaderOriginFallback`                                         | 否       |
...
| `gateway.auth_no_rate_limit`                                  | 警告          | 暴露的认证若没有速率限制，会增加暴力破解风险                        | `gateway.auth.rateLimit`                                                                             | 否       |
| `gateway.trusted_proxy_auth`                                  | 严重      | 代理身份现在会成为认证边界                                         | `gateway.auth.mode="trusted-proxy"`                                                                  | 否       |
| `gateway.trusted_proxy_no_proxies`                            | 严重      | 没有受信任代理 IP 的可信代理认证不安全                               | `gateway.trustedProxies`                                                                             | 否       |
| `gateway.trusted_proxy_no_user_header`                        | 严重      | 可信代理认证无法安全解析用户身份                               | `gateway.auth.trustedProxy.userHeader`                                                               | 否       |
| `gateway.trusted_proxy_no_allowlist`                          | 警告          | 可信代理认证会接受任何已认证的上游用户                           | `gateway.auth.trustedProxy.allowUsers`                                                               | 否       |
| `gateway.trusted_proxy_allow_loopback`                        | 警告          | 受信任代理身份验证接受明确允许的 loopback 代理来源                 | `gateway.auth.trustedProxy.allowLoopback`                                                            | 否       |
...
| `hooks.token_reuse_gateway_token`                             | 严重      | Hook 入口 token 也会解锁 Gateway 网关身份验证                                         | `hooks.token`, `gateway.auth.token`, `gateway.auth.password`                                         | 否       |
| `hooks.token_too_short`                                       | 警告          | Hook 入口更容易被暴力破解                                                   | `hooks.token`                                                                                        | 否       |
| `hooks.default_session_key_unset`                             | 警告          | Hook agent 运行会扇出到生成的按请求会话                          | `hooks.defaultSessionKey`                                                                            | 否       |
| `hooks.allowed_agent_ids_unrestricted`                        | 警告/严重 | 已通过身份验证的 Hook 调用方可路由到任何已配置的 agent                         | `hooks.allowedAgentIds`                                                                              | 否       |
| `hooks.request_session_key_enabled`                           | 警告/严重 | 外部调用方可以选择 sessionKey                                                | `hooks.allowRequestSessionKey`                                                                       | 否       |
| `hooks.request_session_key_prefixes_missing`                  | 警告/严重 | 外部会话键形状没有约束                                              | `hooks.allowedSessionKeyPrefixes`                                                                    | 否       |
| `hooks.path_root`                                             | 严重      | Hook 路径为 `/`，使入口更容易冲突或误路由                       | `hooks.path`                                                                                         | 否       |
| `hooks.installs_unpinned_npm_specs`                           | 警告          | Hook 安装记录未固定到不可变 npm 规格                           | Hook 安装元数据                                                                                | 否       |
...
| `sandbox.dangerous_seccomp_profile`                           | 严重      | 沙箱 seccomp profile 削弱容器隔离                                  | `agents.*.sandbox.docker.securityOpt`                                                                | 否       |
| `sandbox.dangerous_apparmor_profile`                          | 严重      | 沙箱 AppArmor profile 削弱容器隔离                                 | `agents.*.sandbox.docker.securityOpt`                                                                | 否       |
...
| `tools.exec.host_sandbox_no_sandbox_defaults`                 | 警告          | 沙箱关闭时，`exec host=sandbox` 会失败关闭                                 | `tools.exec.host`, `agents.defaults.sandbox.mode`                                                    | 否       |
| `tools.exec.host_sandbox_no_sandbox_agents`                   | 警告          | 沙箱关闭时，按 agent 配置的 `exec host=sandbox` 会失败关闭                       | `agents.list[].tools.exec.host`, `agents.list[].sandbox.mode`                                        | 否       |
...
| `tools.exec.agent_skill_mcp_boundary_drift`                   | 警告          | 存在 agent skill 允许列表，同时主机 exec 可访问 MCP 客户端/注册表  | `agents.list[].tools.exec.*`, 沙箱/OS 隔离, MCP 服务器凭据                           | 否       |
...
| `tools.exec.auto_allow_skills_enabled`                        | 警告          | Exec 审批隐式信任 skill bins                                           | 主机审批文件                                                                                  | 否       |
| `tools.exec.allowlist_interpreter_without_strict_inline_eval` | 警告          | 解释器允许列表允许 inline eval，而无需强制重新审批                  | `tools.exec.strictInlineEval`, `agents.list[].tools.exec.strictInlineEval`, exec 审批允许列表 | 否       |
| `tools.exec.safe_bins_interpreter_unprofiled`                 | 警告          | `safeBins` 中没有显式配置文件的解释器/运行时 bins 会扩大 exec 风险   | `tools.exec.safeBins`, `tools.exec.safeBinProfiles`, `agents.list[].tools.exec.*`                    | 否       |
| `tools.exec.safe_bins_broad_behavior`                         | 警告          | `safeBins` 中的宽泛行为工具会削弱低风险 stdin-filter 信任模型      | `tools.exec.safeBins`, `agents.list[].tools.exec.safeBins`                                           | 否       |
...
| `security.exposure.open_groups_with_elevated`                 | 严重      | 开放私信/群组 + 提升权限工具会产生高影响的提示注入路径           | 顶层或嵌套私信策略路径、账号覆盖、`channels.*.groupPolicy`                     | 否       |
| `security.exposure.open_groups_with_runtime_or_fs`            | 严重/警告 | 开放私信/群组可以在没有沙箱/工作区保护的情况下触达命令/文件工具        | 私信/群组策略路径、`tools.profile/deny`、`tools.fs.workspaceOnly`、`agents.*.sandbox.mode`       | 否       |
| `security.trust_model.multi_user_heuristic`                   | 警告          | 配置看起来是多用户配置，而 Gateway 网关信任模型是个人助理              | 拆分信任边界，或共享用户加固（`sandbox.mode`、工具拒绝/工作区范围限定）      | 否       |
| `tools.profile_minimal_overridden`                            | 警告          | 智能体覆盖绕过了全局最小配置档                                        | `agents.list[].tools.profile`                                                                        | 否       |
| `plugins.tools_reachable_permissive_policy`                   | 警告          | 扩展工具在宽松上下文中可触达                                     | `tools.profile` + 工具允许/拒绝                                                                    | 否       |
| `models.legacy`                                               | 警告          | 旧版模型系列仍在配置中                                           | 模型选择                                                                                      | 否       |
| `models.weak_tier`                                            | 警告          | 已配置模型低于当前推荐档位                                | 模型选择                                                                                      | 否       |
| `models.small_params`                                         | 严重/信息 | 小模型 + 不安全工具表面会提高注入风险                             | 模型选择 + 沙箱/工具策略                                                                   | 否       |
| `summary.attack_surface`                                      | 信息          | 凭证、频道、工具和暴露态势的汇总摘要                         | 多个键（参见发现详情）                                                                   | 否       |
