...
summary: 常见问题：快速开始和初次运行设置 — 安装、新手引导、凭证、订阅、初始失败
title: 常见问题：首次运行设置
x-i18n:
    generated_at: "2026-07-05T01:57:15Z"
    model: gpt-5.5
...
---
  ...
  <Accordion title="我卡住了，最快解决问题的方法">
    使用一个可以**看到你的机器**的本地 AI 智能体。这比在 Discord 里询问有效得多，因为大多数“我卡住了”的情况都是**本地配置或环境问题**，远程协助者无法检查。

    - **Claude Code**: [https://www.anthropic.com/claude-code/](https://www.anthropic.com/claude-code/)
    - **OpenAI Codex**: [https://openai.com/codex/](https://openai.com/codex/)

    这些工具可以读取仓库、运行命令、检查日志，并帮助修复你的机器级设置（PATH、服务、权限、凭证文件）。通过可改造的（git）安装，把**完整源码检出**提供给它们：

    ```bash
    curl -fsSL https://openclaw.ai/install.sh | bash -s -- --install-method git
    ```

    这会**从 git 检出**安装 OpenClaw，因此智能体可以读取代码和文档，并基于你正在运行的确切版本进行推理。之后你随时可以通过不带 `--install-method git` 重新运行安装器切回稳定版。

    小提示：让智能体**规划并监督**修复（逐步进行），然后只执行必要的命令。这样可以让变更更小，也更容易审计。
  ...
  <Accordion title="新手引导后如何打开仪表板？">
    向导会在新手引导之后立即用一个干净的（不带令牌的）仪表板 URL 打开你的浏览器，并且也会在摘要中打印链接。保持该标签页打开；如果没有启动，请在同一台机器上复制/粘贴打印出的 URL。
  </Accordion>

  <Accordion title="如何在 localhost 和远程环境中认证仪表板？">
    **Localhost（同一台机器）：**

    - 打开 `http://127.0.0.1:18789/`。
    - 如果它要求 shared-secret 认证，请将配置的令牌或密码粘贴到 Control UI 设置中。
    - 令牌来源：`gateway.auth.token`（或 `OPENCLAW_GATEWAY_TOKEN`）。
    - 密码来源：`gateway.auth.password`（或 `OPENCLAW_GATEWAY_PASSWORD`）。
    - 如果还没有配置 shared secret，请用 `openclaw doctor --generate-gateway-token` 生成令牌。

    **不在 localhost 上：**

    - **Tailscale Serve**（推荐）：保持绑定 local loopback，运行 `openclaw gateway --tailscale serve`，打开 `https://<magicdns>/`。如果 `gateway.auth.allowTailscale` 为 `true`，身份标头会满足 Control UI/WebSocket 认证（无需粘贴 shared secret，假定 Gateway 网关主机可信）；HTTP APIs 仍需要 shared-secret 认证，除非你有意使用 private-ingress `none` 或 trusted-proxy HTTP 认证。
      来自同一客户端的并发错误 Serve 认证尝试会在 failed-auth 限制器记录之前被串行化，因此第二次错误重试可能已经显示 `retry later`。
    - **Tailnet 绑定**：运行 `openclaw gateway --bind tailnet --token "<token>"`（或配置密码认证），打开 `http://<tailscale-ip>:18789/`，然后在仪表板设置中粘贴匹配的 shared secret。
    - **身份感知反向代理**：将 Gateway 网关放在可信代理后面，配置 `gateway.auth.mode: "trusted-proxy"`，然后打开代理 URL。同主机 local loopback 代理需要显式设置 `gateway.auth.trustedProxy.allowLoopback = true`。
    - **SSH 隧道**：`ssh -N -L 18789:127.0.0.1:18789 user@host`，然后打开 `http://127.0.0.1:18789/`。隧道上仍适用 shared-secret 认证；如果提示，请粘贴配置的令牌或密码。

    有关绑定模式和认证详情，请参阅[仪表板](/zh-CN/web/dashboard)和 [Web 表面](/zh-CN/web)。
  ...
  <Accordion title="为什么聊天审批有两个 exec 审批配置？">
    ...
    - 当原生审批卡片/按钮可用时，该原生 UI 是主要路径；只有当工具结果说明聊天审批不可用，或手动审批是唯一路径时，智能体才应包含手动 `/approve` 命令。
    - 仅在提示也必须转发到其他聊天或明确的运维房间时使用 `approvals.exec`。
    - 仅当你明确希望审批提示发回原始房间/话题时，才使用 `channels.<channel>.execApprovals.target: "channel"` 或 `"both"`。
    - 插件审批又是另一套：默认使用同一聊天的 `/approve`，可选 `approvals.plugin` 转发，并且只有部分原生渠道在此基础上保留插件审批原生处理。

    简短版：转发用于路由，原生客户端配置用于更丰富的渠道特定 UX。
  ...
  <Accordion title="它能在 Raspberry Pi 上运行吗？">
    可以。Gateway 网关很轻量，文档列出的个人使用要求是 **512MB-1GB RAM**、**1 核**以及约 **500MB** 磁盘，并说明 **Raspberry Pi 4 可以运行它**。

    如果你想要额外余量（日志、媒体、其他服务），**推荐 2GB**，但这不是硬性最低要求。

    小提示：小型 Raspberry Pi/VPS 可以托管 Gateway 网关，你可以在笔记本/手机上配对**节点**，用于本地屏幕/摄像头/canvas 或命令执行。参阅[节点](/zh-CN/nodes)。

  </Accordion>

  <Accordion title="Raspberry Pi 安装有什么建议？">
    简短版：可以工作，但可能会遇到一些不够顺滑的地方。
  ...
  <Accordion title="它卡在“唤醒我的朋友”/新手引导无法孵化。现在怎么办？">
    该屏幕依赖 Gateway 网关可达且已认证。TUI 也会在首次孵化时自动发送“Wake up, my friend!”。如果你看到这一行但**没有回复**，并且令牌数保持为 0，说明智能体从未运行。
  ...
  <Accordion title="我能否在不重新做新手引导的情况下，把设置迁移到新机器（Mac mini）？">
    可以。复制**状态目录**和**工作区**，然后运行一次 Doctor。只要你复制**两个**位置，就能让你的 bot 保持“完全相同”（记忆、会话历史、凭证和渠道状态）：
    ...
    **重要：**如果你只是将工作区提交/推送到 GitHub，你备份的是**记忆和 bootstrap 文件**，但**不是**会话历史或凭证。它们位于 `~/.openclaw/` 下（例如 `~/.openclaw/agents/<agentId>/sessions/`）。

    相关：[迁移](/zh-CN/install/migrating)、[磁盘上的内容位置](/zh-CN/help/faq#where-things-live-on-disk)、
  ...
  <Accordion title="在哪里查看最新版本的新内容？">
    ...
    最新条目位于顶部。如果顶部章节标记为**未发布**，下一个带日期的章节就是最新已发布版本。条目按**亮点**、**变更**和**修复**分组（需要时还会有文档/其他章节）。

  </Accordion>

  <Accordion title="无法访问 docs.openclaw.ai（SSL 错误）">
    某些 Comcast/Xfinity 连接会被 Xfinity Advanced Security 错误地阻止访问 `docs.openclaw.ai`。禁用它或将 `docs.openclaw.ai` 加入允许列表，然后重试。
    请在这里报告，帮助我们解除阻止：[https://spa.xfinity.com/check_url_status](https://spa.xfinity.com/check_url_status)。

    如果你仍然无法访问网站，文档已镜像到 GitHub：
  ...
  <Accordion title="稳定版和测试版的区别">
    **稳定版**和**测试版**是 **npm dist-tags**，不是独立的代码线：
    ...
    通常，稳定版本会先发布到 **beta**，然后通过显式
    晋级步骤将同一版本移动到 `latest`。维护者也可以在需要时
    直接发布到 `latest`。这就是为什么晋级后测试版和稳定版可以
    指向**同一版本**。

    查看变更内容：
    [https://github.com/openclaw/openclaw/blob/main/CHANGELOG.md](https://github.com/openclaw/openclaw/blob/main/CHANGELOG.md)

    有关安装单行命令以及测试版和 dev 的区别，请参阅下面的折叠项。

  </Accordion>

  <Accordion title="如何安装测试版，测试版和 dev 有什么区别？">
    **测试版**是 npm dist-tag `beta`（晋级后可能与 `latest` 匹配）。
  ...
  <Accordion title="安装和新手引导通常需要多长时间？">
    ...
    - **安装：** 2-5 分钟
    - **QuickStart 新手引导：** 通常几分钟
    - **完整新手引导：** 当提供商登录、渠道配对、daemon 安装、
      网络下载、Skills 或可选插件需要额外设置时会更久

    CLI 向导会预先显示此时间线。你可以跳过可选步骤，稍后
    使用 `openclaw configure` 返回。

    如果它卡住，请使用[安装器卡住](#quick-start-and-first-run-setup)
  ...
  <Accordion title="Windows 安装提示找不到 git 或无法识别 openclaw">
    ...
    **2) 安装后无法识别 openclaw**
    ...
    设置，PowerShell 安装器和 WSL2 Gateway 网关路径均受支持。
  ...
  <Accordion title="Windows exec 输出显示中文乱码 - 我该怎么办？">
    ...
    如果你仍能在最新 OpenClaw 上复现此问题，请在以下位置跟踪/报告：
  ...
  <Accordion title="文档没有回答我的问题 - 如何获得更好的答案？">
    ...
    从该文件夹中向你的 bot（或 Claude/Codex）提问，这样它就可以读取仓库并准确回答。
  ...
  <Accordion title="云/VPS 安装指南在哪里？">
    ...
    它在云中的工作方式：**Gateway 网关在服务器上运行**，你通过 Control UI（或 Tailscale/SSH）
    从笔记本电脑/手机访问它。你的状态 + 工作区
    位于服务器上，因此请将主机视为事实来源并进行备份。

    你可以将**节点**（Mac/iOS/Android/headless）配对到该云 Gateway 网关，以访问
    本地屏幕/相机/canvas，或在笔记本电脑上运行命令，同时将
    Gateway 网关保留在云中。
  ...
  <Accordion title="我可以让 OpenClaw 自行更新吗？">
    ...
    可能提示确认。更安全的做法：作为操作者从 shell 运行更新。
    ...
    文档：[更新](/zh-CN/cli/update)、[更新中](/zh-CN/install/updating)。
  ...
  <Accordion title="新手引导实际会做什么？">
    ...
    它还会在主提示开始前设定持续时间预期，并在你的
    已配置模型未知或缺少凭证时发出警告。

  </Accordion>

  <Accordion title="运行这个需要 Claude 或 OpenAI 订阅吗？">
    不需要。你可以使用 **API key**（Anthropic/OpenAI/其他）运行 OpenClaw，或者使用
    **仅本地模型**，这样你的数据会留在你的设备上。订阅（Claude
    ...
    - **OpenClaw 中的 Claude CLI / Claude 订阅凭证**：Anthropic 员工
      ...
      用法视为此集成的获准用法，除非 Anthropic 发布新的
      政策
  ...
  <Accordion title="可以在没有 API key 的情况下使用 Claude Max 订阅吗？">
    ...
    此集成的获准用法，除非 Anthropic 发布新的政策。如果你想要
  ...
  <Accordion title="你们支持 Claude 订阅凭证（Claude Pro 或 Max）吗？">
    ...
    Anthropic 员工告诉我们，这种用法再次被允许，因此 OpenClaw 会将
    Claude CLI 复用和 `claude -p` 用法视为此集成的获准用法，
    除非 Anthropic 发布新的政策。

    Anthropic setup-token 仍可作为受支持的 OpenClaw token 路径使用，但 OpenClaw 现在优先使用 Claude CLI 复用和可用时的 `claude -p`。
...
<a id="why-am-i-seeing-http-429-ratelimiterror-from-anthropic"></a>
  ...
  <AccordionGroup>
  <Accordion title="为什么我会看到 Anthropic 返回 HTTP 429 rate_limit_error？">
    这表示当前窗口内你的 **Anthropic 配额/速率限制** 已耗尽。如果你
    ...
    Anthropic 的 1M 上下文窗口（支持 GA 的 1M Claude 4.x 模型，或旧版
    `context1m: true` 配置）。这只有在你的凭证符合长上下文计费条件时才可用
  ...
  <Accordion title="你们支持 OpenAI 订阅凭证（Codex OAuth）吗？">
    是。OpenClaw 完整支持 **OpenAI Code (Codex) 订阅 OAuth**。
  ...
  <Accordion title="如何设置 Gemini CLI OAuth？">
    ...
    这会将 OAuth 令牌存储在 Gateway 网关主机的凭证配置档案中。详情：[模型提供商](/zh-CN/concepts/model-providers)。

  </Accordion>

  <Accordion title="本地模型适合随意聊天吗？">
    通常不适合。OpenClaw 需要大上下文和强安全性；小显存卡会截断并泄漏。如果必须使用，请在本地运行你能承载的**最大**模型构建（LM Studio），并参见 [/gateway/local-models](/zh-CN/gateway/local-models)。更小/量化模型会增加提示注入风险 - 参见 [安全](/zh-CN/gateway/security)。
  </Accordion>

  <Accordion title="如何将托管模型流量保持在特定区域？">
    选择固定区域的端点。OpenRouter 为 MiniMax、Kimi 和 GLM 提供美国托管选项；选择美国托管变体即可让数据留在区域内。你仍可以通过使用 `models.mode: "merge"` 同时列出 Anthropic/OpenAI，这样在遵守你所选区域化提供商的同时仍保留回退能力。
  </Accordion>
  ...
  <Accordion title="我必须买一台 Mac Mini 才能安装吗？">
    ...
    只有在使用 **macOS 专用工具**时才需要 Mac。对于 iMessage，请在任何已登录 Messages 的 Mac 上配合 `imsg` 使用 [iMessage](/zh-CN/channels/imessage)。如果 Gateway 网关运行在 Linux 或其他地方，请将 `channels.imessage.cliPath` 设置为一个 SSH 包装器，在那台 Mac 上运行 `imsg`。如果你想使用其他 macOS 专用工具，请在 Mac 上运行 Gateway 网关，或配对一个 macOS 节点。
  ...
  <Accordion title="iMessage 支持需要 Mac mini 吗？">
    ...
    任何 Mac 都可以。**使用 [iMessage](/zh-CN/channels/imessage)** 搭配 `imsg`；Gateway 网关可以运行在那台 Mac 上，也可以运行在别处并通过 SSH 包装器 `cliPath` 调用。

    常见设置：

    - 在 Linux/VPS 上运行 Gateway 网关，并将 `channels.imessage.cliPath` 设置为一个 SSH 包装器，在已登录 Messages 的 Mac 上运行 `imsg`。
  ...
  <Accordion title="可以使用 Bun 吗？">
    **不推荐**使用 Bun。我们看到了一些运行时错误，尤其是在 WhatsApp 和 Telegram 上。
  ...
  <Accordion title="Telegram：allowFrom 里应该填什么？">
    ...
    设置流程只要求填写数字用户 ID。如果你的配置里已有旧版 `@username` 条目，`openclaw doctor --fix` 可以尝试解析它们。
  ...
  <Accordion title="多个人可以用同一个 WhatsApp 号码连接不同 OpenClaw 实例吗？">
    可以，通过**多智能体路由**实现。将每个发送者的 WhatsApp **私信**（对端 `kind: "direct"`，发送者 E.164 如 `+15551234567`）绑定到不同的 `agentId`，这样每个人都有自己的工作区和会话存储。回复仍来自**同一个 WhatsApp 账号**，且私信访问控制（`channels.whatsapp.dmPolicy` / `channels.whatsapp.allowFrom`）在每个 WhatsApp 账号上是全局的。参见 [多智能体路由](/zh-CN/concepts/multi-agent) 和 [WhatsApp](/zh-CN/channels/whatsapp)。
  </Accordion>

  <Accordion title='我可以运行一个“快速聊天”智能体和一个“用于编码的 Opus”智能体吗？'>
    可以。使用多智能体路由：为每个智能体设置自己的默认模型，然后将入站路由（提供商账号或特定对端）绑定到各个智能体。示例配置位于 [多智能体路由](/zh-CN/concepts/multi-agent)。另见 [Models](/zh-CN/concepts/models) 和 [配置](/zh-CN/gateway/configuration)。
  </Accordion>
  ...
  <Accordion title="Homebrew 可以在 Linux 上使用吗？">
    ...
    较新的构建还会在 Linux systemd 服务中前置常见用户 bin 目录（例如 `~/.local/bin`、`~/.npm-global/bin`、`~/.local/share/pnpm`、`~/.bun/bin`），并在设置时遵循 `PNPM_HOME`、`NPM_CONFIG_PREFIX`、`BUN_INSTALL`、`VOLTA_HOME`、`ASDF_DATA_DIR`、`NVM_DIR` 和 `FNM_DIR`。
  ...
  <Accordion title="我应该在笔记本电脑还是 VPS 上运行 Gateway 网关？">
    ...
    摩擦最小，并且可以接受睡眠/重启，就在本地运行。

    **笔记本电脑（本地 Gateway 网关）**

    - **优点：** 无服务器成本，可直接访问本地文件，有实时浏览器窗口。
    - **缺点：** 睡眠/网络掉线 = 断开连接，OS 更新/重启会中断，必须保持唤醒。

    **VPS / 云**

    - **优点：** 始终在线，网络稳定，没有笔记本电脑睡眠问题，更容易保持运行。
    - **缺点：** 通常以无头模式运行（使用截图），只能远程访问文件，更新时必须使用 SSH。

    **OpenClaw 专属说明：** WhatsApp/Telegram/Slack/Mattermost/Discord 都可以在 VPS 上正常工作。唯一真正的取舍是**无头浏览器**与可见窗口。参见 [浏览器](/zh-CN/tools/browser)。

    **推荐默认选择：** 如果你之前遇到过 Gateway 网关断连，请使用 VPS。当你正在主动使用 Mac，并且想要本地文件访问或使用可见浏览器进行 UI 自动化时，本地运行很合适。

  </Accordion>

  <Accordion title="在专用机器上运行 OpenClaw 有多重要？">
    不是必需的，但**建议用于提高可靠性和隔离性**。

    - **专用主机（VPS/Mac mini/Raspberry Pi）：** 始终在线，睡眠/重启中断更少，权限更清晰，更容易保持运行。
    - **共享笔记本电脑/台式机：** 完全适合测试和主动使用，但当机器睡眠或更新时会出现暂停。

    如果你想兼得两者优势，请将 Gateway 网关放在专用主机上，并将你的笔记本电脑配对为用于本地屏幕/摄像头/exec 工具的**节点**。参见 [节点](/zh-CN/nodes)。
  ...
  <Accordion title="最低 VPS 要求和推荐 OS 是什么？">
    ...
    - **推荐：** 1-2 vCPU，2GB RAM 或更多以留出余量（日志、媒体、多个渠道）。节点工具和浏览器自动化可能很消耗资源。

    OS：使用 **Ubuntu LTS**（或任何现代 Debian/Ubuntu）。Linux 安装路径在这里测试最充分。
  ...
  <Accordion title="我可以在 VM 中运行 OpenClaw 吗？要求是什么？">
    可以。把 VM 当作 VPS 一样对待：它需要始终在线、可访问，并且拥有足够
    的 RAM 来运行 Gateway 网关和你启用的任何渠道。
    ...
    如果你在 Windows 上，请使用 **Windows Hub** 进行桌面设置；或者在
    你明确需要具有广泛工具兼容性的 Linux 风格 Gateway 网关 VM 时使用 WSL2。
