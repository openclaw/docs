...
---
  ...
  快速开始和首次运行问答。日常操作、模型、凭证、会话和故障排除，请参见主 [常见问题](/zh-CN/help/faq)。

  ## 快速开始和首次运行设置

  <AccordionGroup>
  <Accordion title="我卡住了，最快解决问题的方法">
    使用一个可以**查看你的机器**的本地 AI 智能体。这比在 Discord 里询问有效得多，因为大多数“我卡住了”的情况都是**本地配置或环境问题**，远程协助者无法检查。

    - **Claude Code**：[https://www.anthropic.com/claude-code/](https://www.anthropic.com/claude-code/)
    - **OpenAI Codex**：[https://openai.com/codex/](https://openai.com/codex/)

    这些工具可以读取仓库、运行命令、检查日志，并帮助修复你的机器级设置（PATH、服务、权限、凭证文件）。通过可修改的（git）安装方式，把**完整源代码 checkout** 提供给它们：

    ```bash
    curl -fsSL https://openclaw.ai/install.sh | bash -s -- --install-method git
    ```

    这会**从 git checkout** 安装 OpenClaw，因此智能体可以读取代码 + 文档，并推理你正在运行的确切版本。之后你随时可以通过不带 `--install-method git` 重新运行安装器切回稳定版。

    提示：让智能体**规划并监督**修复（逐步执行），然后只执行必要的命令。这样能让变更更小，也更容易审计。
  ...
  <Accordion title="Heartbeat 一直跳过。跳过原因是什么意思？">
    ...
    - `no-tasks-due`：`HEARTBEAT.md` 任务模式已启用，但所有任务间隔都还未到期
    - `alerts-disabled`：所有 Heartbeat 可见性都已禁用（`showOk`、`showAlerts` 和 `useIndicator` 全部关闭）

    在任务模式下，到期时间戳只会在一次真实 Heartbeat 运行完成后推进。被跳过的运行不会将任务标记为已完成。
  ...
  <Accordion title="安装和设置 OpenClaw 的推荐方式">
    ...
    向导也可以自动构建 UI 资产。新手引导完成后，你通常会在 **18789** 端口运行 Gateway 网关。
  ...
  <Accordion title="新手引导后如何打开仪表盘？">
    向导会在新手引导结束后立即用干净的（非 token 化）仪表盘 URL 打开你的浏览器，并在摘要中打印链接。保持该标签页打开；如果它没有启动，请在同一台机器上复制/粘贴打印出的 URL。
  </Accordion>
  ...
  <Accordion title="如何在 localhost 和远程环境中认证仪表盘？">
    **Localhost（同一台机器）：**
    ...
    - 如果它要求 shared-secret 凭证，请将配置的 token 或密码粘贴到 Control UI 设置中。
    - Token 来源：`gateway.auth.token`（或 `OPENCLAW_GATEWAY_TOKEN`）。
    - 密码来源：`gateway.auth.password`（或 `OPENCLAW_GATEWAY_PASSWORD`）。
    - 如果尚未配置 shared secret，请使用 `openclaw doctor --generate-gateway-token` 生成 token。

    **不在 localhost 上：**

    - **Tailscale Serve**（推荐）：保持绑定 local loopback，运行 `openclaw gateway --tailscale serve`，打开 `https://<magicdns>/`。如果 `gateway.auth.allowTailscale` 为 `true`，身份标头会满足 Control UI/WebSocket 凭证要求（无需粘贴 shared secret，前提是假定 Gateway 网关主机可信）；HTTP API 仍然需要 shared-secret 凭证，除非你有意使用 private-ingress `none` 或 trusted-proxy HTTP 凭证。
      同一客户端的错误并发 Serve 凭证尝试会在失败凭证限流器记录之前被串行化，因此第二次错误重试可能已经显示 `retry later`。
    - **Tailnet 绑定**：运行 `openclaw gateway --bind tailnet --token "<token>"`（或配置密码凭证），打开 `http://<tailscale-ip>:18789/`，然后在仪表盘设置中粘贴匹配的 shared secret。
    - **具备身份感知能力的反向代理**：将 Gateway 网关置于可信代理后面，配置 `gateway.auth.mode: "trusted-proxy"`，然后打开代理 URL。同主机 local loopback 代理需要显式设置 `gateway.auth.trustedProxy.allowLoopback = true`。
    - **SSH 隧道**：`ssh -N -L 18789:127.0.0.1:18789 user@host`，然后打开 `http://127.0.0.1:18789/`。shared-secret 凭证在隧道上仍然适用；如有提示，请粘贴配置的 token 或密码。

    有关绑定模式和凭证详情，请参见 [仪表盘](/zh-CN/web/dashboard) 和 [Web 表面](/zh-CN/web)。
  ...
  <Accordion title="为什么聊天审批有两套 exec 审批配置？">
    ...
    主机 exec 策略仍然是真正的审批关口。聊天配置只控制审批提示出现在哪里，以及人们如何回答它们。

    在大多数设置中，你**不**需要两者都配置：

    - 如果聊天已经支持命令和回复，同聊天 `/approve` 会通过共享路径工作。
    - 如果受支持的原生渠道可以安全推断审批者，OpenClaw 现在会在 `channels.<channel>.execApprovals.enabled` 未设置或为 `"auto"` 时自动启用 DM 优先的原生审批。
    - 当原生审批卡片/按钮可用时，该原生 UI 是主要路径；只有当工具结果表明聊天审批不可用或手动审批是唯一路径时，智能体才应包含手动 `/approve` 命令。
    - 仅当提示还必须转发到其他聊天或显式运维房间时，才使用 `approvals.exec`。
    - 仅当你明确希望审批提示发回原始房间/话题时，才使用 `channels.<channel>.execApprovals.target: "channel"` 或 `"both"`。
    - 插件审批又是独立的：它们默认使用同聊天 `/approve`，可选 `approvals.plugin` 转发，并且只有部分原生渠道会在其上保留插件审批原生处理。

    简短版本：转发用于路由，原生客户端配置用于更丰富的渠道专属 UX。
    请参见 [Exec 审批](/zh-CN/tools/exec-approvals)。
  ...
  <Accordion title="它能在 Raspberry Pi 上运行吗？">
    可以。Gateway 网关很轻量，文档列出的个人使用充足配置为 **512MB-1GB RAM**、**1 核**以及约 **500MB** 磁盘，并说明 **Raspberry Pi 4 可以运行它**。

    如果你想要更多余量（日志、媒体、其他服务），**推荐 2GB**，但这不是硬性最低要求。

    提示：小型 Pi/VPS 可以托管 Gateway 网关，你可以在笔记本电脑/手机上配对**节点**，用于本地屏幕/摄像头/canvas 或命令执行。请参见 [节点](/zh-CN/nodes)。

  </Accordion>

  <Accordion title="Raspberry Pi 安装有什么提示？">
    简短版本：它可以运行，但可能会遇到一些粗糙边角。

    - 使用 **64 位** OS，并保持 Node >= 22。
    - 优先使用**可修改的（git）安装**，这样你可以查看日志并快速更新。
    - 先不启用渠道/Skills，然后逐个添加它们。
  ...
  <Accordion title="它卡在 wake up my friend / 新手引导无法孵化。现在怎么办？">
    该屏幕依赖 Gateway 网关可访问且已认证。TUI 也会在首次孵化时自动发送“Wake up, my friend!”。如果你看到那一行但**没有回复**，且 token 保持为 0，则智能体从未运行。
    ...
    如果 Gateway 网关是远程的，请确保隧道/Tailscale 连接已启动，并且 UI 指向正确的 Gateway 网关。请参见 [远程访问](/zh-CN/gateway/remote)。

  </Accordion>

  <Accordion title="我能把设置迁移到新机器（Mac mini）而不重新做新手引导吗？">
    可以。复制**状态目录**和**工作区**，然后运行一次 Doctor。只要你复制**两个**位置，这会让你的 bot 保持“完全相同”（记忆、会话历史、凭证和渠道状态）：
    ...
    **重要：**如果你只将工作区提交/推送到 GitHub，你备份的是**记忆 + bootstrap 文件**，但**不是**会话历史或凭证。它们位于 `~/.openclaw/` 下（例如 `~/.openclaw/agents/<agentId>/sessions/`）。
  ...
  <Accordion title="在哪里查看最新版本的新内容？">
    ...
    最新条目在顶部。如果顶部小节标记为 **Unreleased**，则下一个带日期的小节就是最新已发布版本。条目按 **Highlights**、**Changes** 和 **Fixes** 分组（需要时还会有文档/其他小节）。

  </Accordion>

  <Accordion title="无法访问 docs.openclaw.ai（SSL 错误）">
    某些 Comcast/Xfinity 连接会通过 Xfinity Advanced Security 错误拦截 `docs.openclaw.ai`。禁用它或将 `docs.openclaw.ai` 加入允许列表，然后重试。
  ...
  <Accordion title="稳定版和 beta 版的区别">
    **稳定版**和 **beta 版**是 **npm dist-tags**，不是不同的代码线：

    - `latest` = 稳定版
    - `beta` = 用于测试的早期构建

    通常，一个稳定版本会先落到 **beta**，然后通过一个明确的
    提升步骤将同一个版本移动到 `latest`。维护者也可以在需要时
    直接发布到 `latest`。这就是为什么 beta 版和稳定版在提升后可能
    指向**同一个版本**。
  ...
  <Accordion title="Windows exec 输出显示乱码中文，该怎么办？">
    ...
    如果你仍然能在最新版 OpenClaw 上复现此问题，请在以下位置跟踪/报告：
  ...
  <Accordion title="文档没有回答我的问题，如何获得更好的答案？">
    ...
    从那个文件夹中向你的 bot（或 Claude/Codex）提问，这样它就能读取 repo 并精确回答。
  ...
  <Accordion title="我可以让 OpenClaw 自行更新吗？">
    ...
    可能会提示确认。更安全的做法：由操作员从 shell 运行更新。
  ...
  <Accordion title="运行这个需要 Claude 或 OpenAI 订阅吗？">
    ...
    **仅本地模型**，让你的数据保留在你的设备上。订阅（Claude
    Pro/Max 或 OpenAI Codex）是对这些提供商进行身份验证的可选方式。
    ...
    - **OpenClaw 中的 Claude CLI / Claude 订阅凭证**：Anthropic 员工
      ...
      用法视为此集成的受许可用法，除非 Anthropic 发布新
      政策
  ...
  <Accordion title="我可以在没有 API key 的情况下使用 Claude Max 订阅吗？">
    ...
    此集成的受许可用法，除非 Anthropic 发布新政策。如果你想要
  ...
  <Accordion title="你们支持 Claude 订阅凭证（Claude Pro 或 Max）吗？">
    ...
    Claude CLI 复用和 `claude -p` 用法视为此集成的受许可用法，
    除非 Anthropic 发布新政策。
...
<AccordionGroup>
  <Accordion title="为什么我会看到来自 Anthropic 的 HTTP 429 rate_limit_error？">
    这表示你的 **Anthropic 配额/速率限制**在当前窗口内已耗尽。如果你
    使用 **Claude CLI**，请等待窗口重置或升级你的计划。如果你
    使用 **Anthropic API key**，请检查 Anthropic Console
    中的使用量/计费，并根据需要提高限制。

    如果消息明确是：
    `Extra usage is required for long context requests`，该请求正在尝试使用
    Anthropic 的 1M 上下文 beta（`context1m: true`）。这只在你的
    凭证符合长上下文计费条件（API key 计费，或启用 Extra Usage 的
    OpenClaw Claude 登录路径）时才有效。

    提示：设置一个**备用模型**，这样当某个提供商受速率限制时，OpenClaw 仍可继续回复。
  ...
  <Accordion title="Codex 凭证如何工作？">
    ...
    访问仍可用于非 Agent 的 OpenAI API 表面，也可通过有序的
    `openai-codex` API-key profile 用于 Agent
    模型。
  ...
  <Accordion title="为什么 OpenClaw 仍然提到 openai-codex？">
    ...
    - `openai/gpt-5.5` 加上有序的 `openai-codex` API-key profile = OpenAI Agent 模型的 API-key 凭证
    ...
    `openai/gpt-5.5`；`openai-codex/*` 模型引用属于旧版配置，
    `openclaw doctor --fix` 会重写它。

  </Accordion>

  <Accordion title="为什么 Codex OAuth 限制可能不同于 ChatGPT 网页版？">
    Codex OAuth 使用由 OpenAI 管理、依赖套餐的配额窗口。实际使用中，
    即使二者绑定到同一账号，这些限制也可能不同于 ChatGPT 网站/应用体验。

    OpenClaw 可以在 `openclaw models status` 中显示当前可见的提供商用量/配额窗口，
    但它不会把 ChatGPT 网页版权益虚构或规范化为直接 API 访问。如果你想使用直接的 OpenAI Platform
  ...
  <Accordion title="如何设置 Gemini CLI OAuth？">
    Gemini CLI 使用**插件凭证流程**，不是在 `openclaw.json` 中配置 client id 或 secret。

    步骤：

    1. 在本地安装 Gemini CLI，让 `gemini` 位于 `PATH` 中
    ...
    这会把 OAuth token 存储到 Gateway 网关主机上的 auth profile 中。详情：[模型提供商](/zh-CN/concepts/model-providers)。

  </Accordion>

  <Accordion title="本地模型适合随意聊天吗？">
    通常不适合。OpenClaw 需要大上下文和强安全性；小显卡会截断并泄漏。如果必须使用，请在本地运行你能运行的**最大**模型构建（LM Studio），并参见 [/gateway/local-models](/zh-CN/gateway/local-models)。更小/量化的模型会增加提示注入风险 - 参见 [安全](/zh-CN/gateway/security)。
  </Accordion>

  <Accordion title="如何让托管模型流量保持在特定区域？">
    选择区域固定的端点。OpenRouter 为 MiniMax、Kimi 和 GLM 提供美国托管选项；选择美国托管变体即可让数据保持在该区域内。你仍可通过 `models.mode: "merge"` 同时列出 Anthropic/OpenAI，以便在尊重所选区域化提供商的同时保留备用项。
  </Accordion>
  ...
  <Accordion title="我必须买一台 Mac Mini 才能安装它吗？">
    ...
    只有在使用 **macOS 专属工具**时才需要 Mac。对于 iMessage，请在任何已登录 Messages 的 Mac 上使用带 `imsg` 的 [iMessage](/zh-CN/channels/imessage)。如果 Gateway 网关运行在 Linux 或其他地方，请将 `channels.imessage.cliPath` 设置为一个 SSH 包装器，用来在那台 Mac 上运行 `imsg`。如果你想使用其他 macOS 专属工具，请在 Mac 上运行 Gateway 网关，或配对一个 macOS 节点。
  ...
  <Accordion title="我需要 Mac mini 才能支持 iMessage 吗？">
    ...
    任意 Mac 都可以。**使用带 `imsg` 的 [iMessage](/zh-CN/channels/imessage)**；Gateway 网关可以运行在那台 Mac 上，也可以运行在其他地方并使用 SSH 包装器 `cliPath`。

    常见设置：

    - 在 Linux/VPS 上运行 Gateway 网关，并将 `channels.imessage.cliPath` 设置为一个 SSH 包装器，用来在已登录 Messages 的 Mac 上运行 `imsg`。
  ...
  <Accordion title="我可以使用 Bun 吗？">
    **不推荐**使用 Bun。我们看到过运行时错误，尤其是在 WhatsApp 和 Telegram 中。
  ...
  <Accordion title="Telegram：allowFrom 中应该填写什么？">
    ...
    - 私信你的 bot，然后调用 `https://api.telegram.org/bot<bot_token>/getUpdates` 并读取 `message.from.id`。
  ...
  <Accordion title="多个人可以在不同 OpenClaw 实例中使用同一个 WhatsApp 号码吗？">
    可以，通过**多 Agent 路由**实现。将每个发送者的 WhatsApp **私信**（peer `kind: "direct"`，发送者 E.164 格式如 `+15551234567`）绑定到不同的 `agentId`，这样每个人都有自己的工作区和会话存储。回复仍然来自**同一个 WhatsApp 账号**，并且私信访问控制（`channels.whatsapp.dmPolicy` / `channels.whatsapp.allowFrom`）按 WhatsApp 账号全局生效。参见 [Multi-Agent Routing](/zh-CN/concepts/multi-agent) 和 [WhatsApp](/zh-CN/channels/whatsapp)。
  </Accordion>

  <Accordion title='我可以运行一个“快速聊天”Agent 和一个“用于编码的 Opus”Agent 吗？'>
    可以。使用多 Agent 路由：给每个 Agent 设置自己的默认模型，然后将入站路由（提供商账号或特定 peer）绑定到各个 Agent。示例配置位于 [Multi-Agent Routing](/zh-CN/concepts/multi-agent)。另见 [Models](/zh-CN/concepts/models) 和 [配置](/zh-CN/gateway/configuration)。
  </Accordion>
  ...
  <Accordion title="我以后可以在 npm 和 git 安装之间切换吗？">
    ...
    Doctor 后续操作，刷新目标渠道的插件源，并在你未传入 `--no-restart` 时
    重启 Gateway 网关。
  ...
  <Accordion title="我应该在笔记本电脑还是 VPS 上运行 Gateway 网关？">
    简短回答：**如果你想要 24/7 可靠性，请使用 VPS**。如果你想要最低的上手阻力，并且可以接受休眠/重启，就在本地运行。
    ...
    **OpenClaw 特定说明：** WhatsApp/Telegram/Slack/Mattermost/Discord 都可以在 VPS 上正常工作。唯一真正的取舍是**无头浏览器**还是可见窗口。参见 [浏览器](/zh-CN/tools/browser)。

    **推荐默认选择：** 如果你以前遇到过 Gateway 网关断开连接，请使用 VPS。当你正在主动使用 Mac，并且需要本地文件访问或使用可见浏览器进行 UI 自动化时，本地运行很适合。

  </Accordion>

  <Accordion title="在专用机器上运行 OpenClaw 有多重要？">
    不是必需的，但**建议这样做以获得可靠性和隔离性**。

    - **专用主机（VPS/Mac mini/Pi）：** 始终在线，休眠/重启中断更少，权限更清晰，更容易保持运行。
    - **共享笔记本电脑/台式机：** 完全适合测试和主动使用，但机器休眠或更新时会出现暂停。

    如果你想两全其美，可以将 Gateway 网关放在专用主机上，并将你的笔记本电脑配对为用于本地屏幕/摄像头/exec 工具的**节点**。参见 [节点](/zh-CN/nodes)。
    如需安全指导，请阅读 [安全](/zh-CN/gateway/security)。

  </Accordion>

  <Accordion title="最低 VPS 要求和推荐操作系统是什么？">
    OpenClaw 很轻量。对于一个基本 Gateway 网关 + 一个聊天渠道：

    - **绝对最低要求：** 1 vCPU、1GB RAM、约 500MB 磁盘。
    - **推荐：** 1-2 vCPU、2GB RAM 或更多，以留出余量（日志、媒体、多个渠道）。节点工具和浏览器自动化可能会占用大量资源。
  ...
  <Accordion title="我可以在 VM 中运行 OpenClaw 吗？有什么要求？">
    可以。像对待 VPS 一样对待 VM：它需要始终在线、可访问，并且有足够的
    ...
    如果你使用 Windows，**WSL2 是最简单的 VM 风格设置**，并且具有最佳工具链
