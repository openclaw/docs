...
title: 常见问题：模型和凭证
x-i18n:
    generated_at: "2026-05-06T01:07:29Z"
    model: gpt-5.5
    provider: openai
...
---
  ...
  <Accordion title='什么是“默认模型”？'>
    ...
    模型以 `provider/model` 的形式引用（示例：`openai/gpt-5.5` 或 `openai-codex/gpt-5.5`）。如果省略提供商，OpenClaw 会先尝试别名，然后尝试与该精确模型 ID 匹配的唯一已配置提供商，最后才会回退到已配置的默认提供商，这是一条已弃用的兼容路径。如果该提供商不再公开已配置的默认模型，OpenClaw 会回退到第一个已配置的提供商/模型，而不是暴露一个已经过时的已移除提供商默认值。你仍应**显式**设置 `provider/model`。
  ...
  <Accordion title="如何在不清空配置的情况下切换模型？">
    ...
    对于 RPC 编辑，先用 `config.schema.lookup` 检查，并优先使用 `config.patch`。lookup 负载会给出规范化路径、浅层 schema 文档/约束，以及直接子项摘要。
    用于部分更新。
  ...
  <Accordion title="如何动态切换模型（无需重启）？">
    ...
    `/model`（和 `/model list`）会显示一个紧凑的编号选择器。按编号选择：

    ```
    /model 3
    ```

    你也可以为提供商强制指定一个特定凭证配置文件（按会话）：
    ...
    提示：`/model status` 会显示当前活动的智能体、正在使用哪个 `auth-profiles.json` 文件，以及接下来会尝试哪个凭证配置文件。
  ...
  <Accordion title="我可以用 GPT 5.5 处理日常任务，用 Codex 5.5 编码吗？">
    ...
    - **原生 Codex 编码智能体：**将 `agents.defaults.model.primary` 设置为 `openai/gpt-5.5`，并将 `agents.defaults.agentRuntime.id` 设置为 `"codex"`。当你想使用 ChatGPT/Codex 订阅凭证时，使用 `openclaw models auth login --provider openai-codex` 登录。
    - **通过 PI 直接执行 OpenAI API 任务：**使用 `/model openai/gpt-5.5`，不要覆盖 Codex 运行时，并配置 `OPENAI_API_KEY`。
    - **通过 PI 使用 Codex OAuth：**只有当你有意使用带 Codex OAuth 的普通 PI runner 时，才使用 `/model openai-codex/gpt-5.5`。
  ...
  <Accordion title="如何为 GPT 5.5 配置快速模式？">
    ...
    请参阅 [Thinking 和快速模式](/zh-CN/tools/thinking) 与 [OpenAI 快速模式](/zh-CN/providers/openai#fast-mode)。
  ...
  <Accordion title='为什么我会看到“Model ... is not allowed”，然后没有回复？'>
    ...
    如果该命令还包含 `--runtime codex`，请先添加模型，然后重试
  ...
  <Accordion title="我可以将 MiniMax 作为默认值，并将 OpenAI 用于复杂任务吗？">
    ...
    fallback 用于**错误**，而不是“困难任务”，所以请使用 `/model` 或单独的智能体。
  ...
  <Accordion title="opus / sonnet / gpt 是内置快捷方式吗？">
    是。OpenClaw 自带一些默认简写（仅当模型存在于 `agents.defaults.models` 中时应用）：

    - `opus` → `anthropic/claude-opus-4-6`
    - `sonnet` → `anthropic/claude-sonnet-4-6`
    - `gpt` → API key 设置中的 `openai/gpt-5.5`，或在配置为 Codex OAuth 时的 `openai-codex/gpt-5.5`
  ...
  <Accordion title="如何添加来自 OpenRouter 或 Z.AI 等其他提供商的模型？">
    ...
    如果你引用了某个提供商/模型，但缺少必需的提供商密钥，就会遇到运行时凭证错误（例如 `No API key found for provider "zai"`）。

    **添加新智能体后找不到提供商的 API key**

    这通常表示**新智能体**的凭证存储为空。凭证按智能体隔离，并存储在：
    ...
    - 或者只将可移植的静态 `api_key` / `token` 配置档从主智能体的凭证存储复制到新智能体的凭证存储中。
    - 对于 OAuth 配置档，当新智能体需要自己的账号时，请从该新智能体登录；否则 OpenClaw 可以读取默认/主智能体，而不必克隆刷新令牌。

    **不要**跨智能体复用 `agentDir`；这会导致凭证/会话冲突。
...
<AccordionGroup>
  <Accordion title="故障转移如何工作？">
    ...
    冷却会应用于失败的配置档（指数退避），因此即使某个提供商受限流影响或暂时失败，OpenClaw 也可以继续响应。

    限流桶包含的情况不只是普通的 `429` 响应。OpenClaw
    还会将 `Too many concurrent requests`、
    `ThrottlingException`、`concurrency limit reached`、
    `workers_ai ... quota limit exceeded`、`resource exhausted` 以及周期性的
    使用窗口限制（`weekly/monthly limit reached`）视为值得触发故障转移的
    限流。

    有些看起来像计费问题的响应并不是 `402`，而有些 HTTP `402`
    响应也会留在这个临时桶中。如果提供商在 `401` 或 `403` 上返回
    明确的计费文本，OpenClaw 仍然可以将其保留在
    计费通道中，但特定提供商的文本匹配器会限定在拥有它们的
    提供商范围内（例如 OpenRouter `Key limit exceeded`）。如果 `402`
    消息看起来像可重试的使用窗口限制或
    组织/工作区消费限额（`daily limit reached, resets tomorrow`、
    ...
    匹配时，将提供商范围内的临时形态视为值得触发故障转移的超时/过载信号，
    例如 Anthropic 裸 `An unknown error occurred`、OpenRouter 裸
    `Provider returned error`、类似 `Unhandled stop reason:
    error` 的停止原因错误、带有临时服务器文本的 JSON `api_error` 载荷
    （`internal server error`、`unknown error, 520`、`upstream error`、`backend
    error`），以及类似 `ModelNotReadyException` 的提供商繁忙错误。
  ...
  <Accordion title='“No credentials found for profile anthropic:default”是什么意思？'>
    这表示系统尝试使用凭证配置档 ID `anthropic:default`，但在预期的凭证存储中找不到对应凭证。

    **修复清单：**

    - **确认凭证配置档存放在哪里**（新路径与旧路径）
    ...
    - **对模型/凭证状态做完整性检查**
      - 使用 `openclaw models status` 查看已配置的模型以及提供商是否已通过凭证验证。

    **“No credentials found for profile anthropic”的修复清单**

    这表示本次运行被固定到某个 Anthropic 凭证配置档，但 Gateway 网关
    无法在其凭证存储中找到该配置档。
  ...
  <Accordion title="为什么它还尝试了 Google Gemini 并失败？">
    如果你的模型配置包含 Google Gemini 作为回退模型（或你切换到了 Gemini 简写），OpenClaw 会在模型回退期间尝试它。如果你尚未配置 Google 凭证，你会看到 `No API key found for provider "google"`。
    ...
    **LLM 请求被拒绝：需要思考签名（Google Antigravity）**
    ...
    修复：OpenClaw 现在会为 Google Antigravity Claude 剥离未签名的思考块。如果它仍然出现，请启动一个**新会话**，或为该智能体设置 `/thinking off`。
...
<AccordionGroup>
  <Accordion title="什么是凭证配置档？">
    凭证配置档是绑定到某个提供商的命名凭证记录（OAuth 或 API key）。配置档位于：

    ```
    ~/.openclaw/agents/<agentId>/agent/auth-profiles.json
    ```

    若要在不输出密钥的情况下检查已保存的配置档，请运行 `openclaw models auth list`（可选加上 `--provider <id>` 或 `--json`）。详情请参阅 [Models CLI](/zh-CN/cli/models#auth-profiles)。
  ...
  <Accordion title="我可以控制先尝试哪个凭证配置档吗？">
    可以。配置支持为配置档设置可选元数据，并按提供商设置顺序（`auth.order.<provider>`）。这**不会**存储密钥；它会将 ID 映射到提供商/模式，并设置轮换顺序。

    如果某个配置档处于短暂**冷却**（限流/超时/凭证失败）或较长的**禁用**状态（计费/余额不足），OpenClaw 可能会临时跳过它。若要检查这一点，请运行 `openclaw models status --json` 并查看 `auth.unusableProfiles`。调优项：`auth.cooldowns.billingBackoffHours*`。

    限流冷却可以按模型限定作用域。某个配置档对一个模型
    处于冷却时，仍然可以用于同一提供商上的兄弟模型，
    而计费/禁用窗口仍会阻止整个配置档。
    ...
    如果某个已存储的配置档被显式顺序省略，probe 会为该配置档报告
