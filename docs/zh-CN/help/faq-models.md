...
---
  ...
  模型和身份验证配置问答。有关设置、会话、Gateway 网关、渠道和
  ...
  <Accordion title='什么是“默认模型”？'>
    ...
    模型以 `provider/model` 的形式引用（例如：`openai/gpt-5.5` 或 `anthropic/claude-sonnet-4-6`）。如果省略提供商，OpenClaw 会先尝试别名，然后尝试与该精确模型 ID 匹配的唯一已配置提供商，最后才会作为已弃用的兼容路径回退到已配置的默认提供商。如果该提供商不再公开已配置的默认模型，OpenClaw 会回退到第一个已配置的提供商/模型，而不是暴露过时的已移除提供商默认值。你仍应**显式**设置 `provider/model`。
  ...
  <Accordion title="你推荐什么模型？">
    ...
    **对于常规/低风险聊天：**使用更便宜的备用模型，并按智能体角色路由。
    ...
    经验法则：对高风险工作使用你**能负担得起的最佳模型**，对常规聊天或摘要使用更便宜的
    模型。你可以按智能体路由模型，并使用子智能体来
    并行处理长任务（每个子智能体都会消耗 token）。请参阅 [Models](/zh-CN/concepts/models) 和
    [子智能体](/zh-CN/tools/subagents)。

    强烈警告：较弱/过度量化的模型更容易受到提示注入和不安全行为的影响。请参阅 [安全](/zh-CN/gateway/security)。
  ...
  <Accordion title="如何在不清空配置的情况下切换模型？">
    ...
    除非你打算替换整个配置，否则请避免使用带部分对象的 `config.apply`。
  ...
  <Accordion title="如何即时切换模型（无需重启）？">
    ...
    提示：`/model status` 会显示当前活动的智能体、正在使用哪个 `auth-profiles.json` 文件，以及接下来会尝试哪个身份验证配置。
  ...
  <Accordion title="我可以用 GPT 5.5 处理日常任务，用 Codex 5.5 编码吗？">
    ...
    - **Native Codex 编码智能体：**将 `agents.defaults.model.primary` 设置为 `openai/gpt-5.5`。当你想使用 ChatGPT/Codex 订阅身份验证时，使用 `openclaw models auth login --provider openai-codex` 登录。
    - **智能体循环之外的直接 OpenAI API 任务：**为图像、嵌入、语音、实时和其他非智能体 OpenAI API 表面配置 `OPENAI_API_KEY`。
    - **OpenAI 智能体 API 密钥身份验证：**使用 `/model openai/gpt-5.5`，并配置一个有序的 `openai-codex` API 密钥配置。
    - **子智能体：**将编码任务路由到一个以 Codex 为重点、拥有自己的 `openai/gpt-5.5` 模型的智能体。
  ...
  <Accordion title="如何为 GPT 5.5 配置快速模式？">
    ...
    对于 OpenAI，快速模式会映射到受支持的原生 Responses 请求上的 `service_tier = "priority"`。会话 `/fast` 覆盖优先于配置默认值。

    请参阅 [Thinking 和快速模式](/zh-CN/tools/thinking) 以及 [OpenAI 快速模式](/zh-CN/providers/openai#fast-mode)。
  ...
  <Accordion title='为什么我会看到“Model ... is not allowed”，然后没有回复？'>
    ...
    `agents.defaults.models`，为动态提供商目录添加类似 `"provider/*": {}` 的提供商通配符，移除 allowlist，或从 `/model list` 中选择一个模型。
    如果命令还包含 `--runtime codex`，请先更新 allowlist，然后重试同一个
  ...
  <Accordion title='为什么我会看到“Unknown model: minimax/MiniMax-M2.7”？'>
    ...
    2. 确保已配置 MiniMax（向导或 JSON），或确保 MiniMax 身份验证
       存在于环境变量/身份验证配置中，以便可以注入匹配的提供商
  ...
  <Accordion title="我可以把 MiniMax 作为默认值，并把 OpenAI 用于复杂任务吗？">
  ...
  <Accordion title="如何定义/覆盖模型快捷方式（别名）？">
    ...
    然后 `/model sonnet`（或在受支持时使用 `/<alias>`）会解析到该模型 ID。
  ...
  <Accordion title="如何添加来自 OpenRouter 或 Z.AI 等其他提供商的模型？">
    ...
    **添加新智能体后找不到提供商的 API key**
    ...
    - 或者只从主智能体的认证存储中复制可移植的静态 `api_key` / `token` profile 到新智能体的认证存储。
    - 对于 OAuth profile，当新智能体需要自己的账号时，请从新智能体登录；否则 OpenClaw 可以读取默认/主智能体，而无需克隆刷新令牌。
...
<AccordionGroup>
  <Accordion title="故障转移如何工作？">
    ...
    1. 同一提供商内的**认证 profile 轮换**。
    2. **模型回退**到 `agents.defaults.model.fallbacks` 中的下一个模型。

    冷却时间会应用到失败的 profile（指数退避），因此即使提供商受到速率限制或临时失败，OpenClaw 仍可继续响应。

    速率限制桶包含的范围不只是普通的 `429` 响应。OpenClaw
    ...
    使用窗口限制（`weekly/monthly limit reached`）这类消息视为值得触发故障转移的
    速率限制。

    一些看起来像账单问题的响应并不是 `402`，而一些 HTTP `402`
    响应也会留在该临时桶中。如果提供商在 `401` 或 `403` 上返回
    明确的账单文本，OpenClaw 仍可将其保留在
    账单通道中，但特定提供商的文本匹配器只限定在拥有它们的
  ...
  <Accordion title='“No credentials found for profile anthropic:default”是什么意思？'>
    ...
    **“No credentials found for profile anthropic”的修复检查清单**
    ...
    这表示本次运行固定到了某个 Anthropic 认证 profile，但 Gateway 网关
    无法在其认证存储中找到它。
    ...
    - **确认你是在 gateway 主机上运行命令**
      - 在远程模式下，认证 profile 位于 gateway 机器上，而不是你的笔记本电脑上。
  ...
  <Accordion title="为什么它还尝试了 Google Gemini 并失败？">
    ...
    修复：提供 Google 认证，或在 `agents.defaults.model.fallbacks` / aliases 中移除/避免 Google 模型，避免回退路由到那里。

    **LLM 请求被拒绝：需要 thinking signature（Google Antigravity）**

    原因：会话历史包含**没有签名的思考块**（通常来自
    已中止/部分完成的流）。Google Antigravity 要求思考块带有签名。

    修复：OpenClaw 现在会为 Google Antigravity Claude 剥离未签名的思考块。如果仍然出现，请启动一个**新会话**，或为该智能体设置 `/thinking off`。

  </Accordion>
</AccordionGroup>

## 认证 profile：它们是什么以及如何管理

相关：[/concepts/oauth](/zh-CN/concepts/oauth)（OAuth 流程、令牌存储、多账号模式）

<AccordionGroup>
  <Accordion title="什么是认证 profile？">
    认证 profile 是绑定到提供商的具名凭证记录（OAuth 或 API key）。profile 位于：

    ```
    ~/.openclaw/agents/<agentId>/agent/auth-profiles.json
    ```

    要在不转储密钥的情况下检查已保存的 profile，请运行 `openclaw models auth list`（可选 `--provider <id>` 或 `--json`）。详见 [Models CLI](/zh-CN/cli/models#auth-profiles)。
  ...
  <Accordion title="典型的 profile ID 有哪些？">
    ...
    - `anthropic:default`（没有邮箱身份时很常见）
    - OAuth 身份使用 `anthropic:<email>`
    - 你选择的自定义 ID（例如 `anthropic:work`）

  </Accordion>

  <Accordion title="我可以控制先尝试哪个认证 profile 吗？">
    可以。配置支持为 profile 提供可选元数据，并按提供商设置顺序（`auth.order.<provider>`）。这**不会**存储密钥；它会将 ID 映射到提供商/模式，并设置轮换顺序。

    如果某个 profile 处于短暂**冷却**（速率限制/超时/认证失败）或较长的**禁用**状态（账单/余额不足），OpenClaw 可能会临时跳过它。要检查这一点，请运行 `openclaw models status --json` 并查看 `auth.unusableProfiles`。调优项：`auth.cooldowns.billingBackoffHours*`。

    速率限制冷却可以按模型限定。某个 profile 对一个模型
    正在冷却时，仍可能可用于同一提供商下的同级模型，
    而账单/禁用窗口仍会阻止整个 profile。
    ...
    如果某个已存储 profile 被显式顺序省略，probe 会为该 profile 报告
