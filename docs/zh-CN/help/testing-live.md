...
[测试](/zh-CN/help/testing)。本页介绍 **live**（会触及网络）的测试套件：模型矩阵、CLI 后端、ACP、媒体提供商 live 测试，以及凭证处理。

## Live：本地冒烟命令

在执行临时 live 检查前，在进程环境中导出所需的提供商密钥。
...
除非同时提供 `--yes`，否则 `voicecall smoke` 是一次空运行。只有当你明确想要发起真实通知电话时，才使用 `--yes`。对于 Twilio、Telnyx 和 Plivo，成功的就绪检查需要一个公共 webhook URL；按设计会拒绝仅本地的 loopback/私有回退。

## Live：Android 节点能力扫描

- 测试：`src/gateway/android-node.capabilities.live.test.ts`
- 脚本：`pnpm android:test:integration`
- 目标：调用已连接 Android 节点**当前通告的每个命令**，并断言命令契约行为。
- 范围：
  - 需要预置/手动设置（该套件不会安装/运行/配对应用）。
  - 针对所选 Android 节点逐个命令进行 Gateway 网关 `node.invoke` 验证。
- 必需的预设置：
...
- “Direct model” 告诉我们提供商/模型是否至少能使用给定密钥作答。
- “Gateway smoke” 告诉我们完整 Gateway 网关 + agent 管线是否适用于该模型（会话、历史、工具、沙箱策略等）。
...
- 目标：
  ...
  - 对每个模型运行一次小型补全（以及必要时的定向回归）
- 启用方式：
  - `pnpm test:live`（或在直接调用 Vitest 时使用 `OPENCLAW_LIVE_TEST=1`）
- 设置 `OPENCLAW_LIVE_MODELS=modern`、`small` 或 `all`（modern 的别名）以实际运行该套件；否则会跳过，以便让 `pnpm test:live` 聚焦于 Gateway 网关冒烟
- 选择模型的方式：
  - `OPENCLAW_LIVE_MODELS=modern` 运行现代允许列表（Opus/Sonnet 4.6+、GPT-5.2 + Codex、Gemini 3、DeepSeek V4、GLM 5.1、MiniMax M3、Grok 4.3）
  - `OPENCLAW_LIVE_MODELS=small` 运行受限小模型允许列表（Qwen 8B/9B 本地兼容路由、Ollama Gemma、OpenRouter Qwen/GLM 和 Z.AI GLM）
  - `OPENCLAW_LIVE_MODELS=all` 是现代允许列表的别名
  - 或 `OPENCLAW_LIVE_MODELS="openai/gpt-5.5,anthropic/claude-opus-4-6,..."`（逗号允许列表）
  - 本地 Ollama 小模型运行默认使用 `http://127.0.0.1:11434`；仅当使用 LAN、自定义或 Ollama Cloud 端点时设置 `OPENCLAW_LIVE_OLLAMA_BASE_URL`。
  - modern/all 和 small 扫描默认使用各自的精选上限；设置 `OPENCLAW_LIVE_MAX_MODELS=0` 可进行完整的已选 profile 扫描，或设置正数使用更小上限。
...
### 第 2 层：Gateway 网关 + 开发 agent 冒烟（“@openclaw”实际执行的内容）
...
- 选择模型的方式：
  ...
  - `OPENCLAW_LIVE_GATEWAY_MODELS=small` 通过完整 Gateway 网关 + agent 管线运行相同的受限小模型允许列表
  - `OPENCLAW_LIVE_GATEWAY_MODELS=all` 是现代允许列表的别名
  - 或设置 `OPENCLAW_LIVE_GATEWAY_MODELS="provider/model"`（或逗号列表）以缩小范围
  - modern/all 和 small Gateway 网关扫描默认使用各自的精选上限；设置 `OPENCLAW_LIVE_GATEWAY_MAX_MODELS=0` 可进行完整的已选扫描，或设置正数使用更小上限。
...
- 工具 + 图像探测在此 live 测试中始终开启：
  ...
  - 当模型通告支持图像输入时运行图像探测
  - 流程（高层）：
    - 测试生成一张带有 “CAT” + 随机代码的小 PNG（`test/helpers/live-image-probe.ts`）
...
<Tip>
要查看你机器上可测试的内容（以及精确的 `provider/model` id），请运行：
...
- 目标：使用本地 CLI 后端验证 Gateway 网关 + agent 管线，而不触及你的默认配置。
...
- 覆盖（可选）：
  ...
  - `OPENCLAW_LIVE_CLI_BACKEND_IMAGE_PROBE=1` 发送真实图像附件（路径会注入到提示中）。Docker 配方默认关闭，除非明确请求。
  ...
  - `OPENCLAW_LIVE_CLI_BACKEND_MODEL_SWITCH_PROBE=1` 在所选模型支持切换目标时，选择加入 Claude Sonnet -> Opus 同会话连续性探测。Docker 配方默认关闭，以提高聚合可靠性。
  - `OPENCLAW_LIVE_CLI_BACKEND_MCP_PROBE=1` 选择加入 MCP/工具 local loopback 探测。Docker 配方默认关闭，除非明确请求。
...
- 它在仓库 Docker 镜像内以非 root 的 `node` 用户运行 live CLI 后端冒烟。
- 它从所属插件解析 CLI 冒烟元数据，然后将匹配的 Linux CLI 包（`@anthropic-ai/claude-code` 或 `@google/gemini-cli`）安装到 `OPENCLAW_DOCKER_CLI_TOOLS_DIR` 处的缓存可写前缀（默认：`~/.cache/openclaw/docker-cli-tools`）。
- `pnpm test:docker:live-cli-backend:claude-subscription` 需要通过 `~/.claude/.credentials.json` 中带有 `claudeAiOauth.subscriptionType` 的配置，或来自 `claude setup-token` 的 `CLAUDE_CODE_OAUTH_TOKEN`，提供可移植 Claude Code 订阅 OAuth。它会先在 Docker 中证明直接 `claude -p` 可用，然后在不保留 Anthropic API 密钥环境变量的情况下运行两轮 Gateway 网关 CLI 后端。该订阅通道默认禁用 Claude MCP/工具和图像探测，因为它会消耗已登录订阅的使用额度，并且 Anthropic 可以在不发布 OpenClaw 版本的情况下更改 Claude Agent SDK / `claude -p` 的计费和速率限制行为。
- live CLI 后端冒烟现在会对 Claude 和 Gemini 执行相同的端到端流程：文本轮次、图像分类轮次，然后通过 Gateway 网关 CLI 验证 MCP `cron` 工具调用。
- Claude 的默认冒烟还会将会话从 Sonnet 修补为 Opus，并验证恢复的会话仍记得早先的笔记。

## Live：APNs HTTP/2 代理可达性

- 测试：`src/infra/push-apns-http2.live.test.ts`
- 目标：通过本地 HTTP CONNECT 代理隧道连接到 Apple 的沙箱 APNs 端点，发送 APNs HTTP/2 验证请求，并断言 Apple 真实的 `403 InvalidProviderToken` 响应会通过代理路径返回。
- 启用：
...
- 说明：
  - 此通道使用 Gateway 网关的 `chat.send` 表面，并带有仅管理员可用的合成 originating-route 字段，因此测试可以附加消息渠道上下文，而无需伪装成对外投递。
  - 当未设置 `OPENCLAW_LIVE_ACP_BIND_AGENT_COMMAND` 时，测试会使用嵌入式 `acpx` 插件的内置智能体注册表，选择对应的 ACP harness 智能体。
  - 默认情况下，已绑定会话的 cron MCP 创建是尽力而为，因为外部 ACP harness 可能会在绑定/图像证明通过后取消 MCP 调用；设置 `OPENCLAW_LIVE_ACP_BIND_REQUIRE_CRON=1` 可让绑定后的 cron 探针变为严格模式。
...
- Droid Docker 变体会暂存 `~/.factory` 作为设置，转发 `FACTORY_API_KEY`，并且需要该 API key，因为本地 Factory OAuth/keyring 凭证无法移植到容器中。它使用 ACPX 的内置 `droid exec --output-format acp` 注册表条目。
- OpenCode Docker 变体是严格的单智能体回归通道。它会从 `OPENCLAW_LIVE_ACP_BIND_OPENCODE_MODEL`（默认值为 `opencode/kimi-k2.6`）写入临时 `OPENCODE_CONFIG_CONTENT` 默认模型，并且 `pnpm test:docker:live-acp-bind:opencode` 要求存在已绑定的助手转录，而不是接受通用的绑定后跳过。
- 直接 `acpx` CLI 调用仅是用于在 Gateway 网关之外比较行为的手动/绕过路径。Docker ACP 绑定冒烟测试会测试 OpenClaw 的嵌入式 `acpx` 运行时后端。
...
- 目标：通过普通 Gateway 网关的
  ...
  - 加载内置的 `codex` 插件
  - 选择 `openai/gpt-5.5`，该模型默认通过 Codex 路由 OpenAI 智能体轮次
  - 向 `openai/gpt-5.5` 发送第一个 Gateway 网关智能体轮次，并选择 Codex harness
  - 向同一个 OpenClaw 会话发送第二个轮次，并验证 app-server
    线程可以恢复
  - 通过同一个 Gateway 网关命令路径运行 `/codex status` 和 `/codex models`
  - 可选运行两个经 Guardian 审查的提升权限 shell 探针：一个应被批准的良性
    命令，以及一个应被拒绝、从而让智能体回问的伪密钥上传
...
- 冒烟测试会强制 provider/model `agentRuntime.id: "codex"`，因此损坏的 Codex
  harness 不能通过静默回退到 OpenClaw 而通过测试。
- 凭证：来自本地 Codex 订阅登录的 Codex app-server 凭证。Docker
  冒烟测试也可以在适用时为非 Codex 探针提供 `OPENAI_API_KEY`，
...
- 它会传递 `OPENAI_API_KEY`，在存在时复制 Codex CLI 凭证文件，将
  `@openai/codex` 安装到可写的挂载 npm
  前缀中，暂存源代码树，然后仅运行 Codex-harness 实时测试。
- Docker 默认启用图像、MCP/工具和 Guardian 探针。当你需要更窄的调试
  运行时，设置
...
- Docker 使用相同的显式 Codex 运行时配置，因此旧版别名或 OpenClaw
  回退无法隐藏 Codex harness 回归。

### 推荐的实时配方

范围窄且显式的 allowlist 最快，也最不容易波动：
...
- `google-gemini-cli/...` 使用你机器上的本地 Gemini CLI（独立凭证 + 工具差异）。
...
没有固定的“CI 模型列表”（实时测试是选择启用的），但以下是在带 key 的开发机器上建议定期覆盖的**推荐**模型。
...
在 `OPENCLAW_LIVE_GATEWAY_MODELS` 中包含至少一个支持图像的模型（Claude/Gemini/OpenAI 支持视觉的变体等），以测试图像探针。

### 聚合器 / 替代网关

如果你已启用 key，我们也支持通过以下方式测试：

- OpenRouter：`openrouter/...`（数百个模型；使用 `openclaw models scan` 查找支持工具 + 图像的候选模型）
- OpenCode：用于 Zen 的 `opencode/...` 和用于 Go 的 `opencode-go/...`（通过 `OPENCODE_API_KEY` / `OPENCODE_ZEN_API_KEY` 进行凭证）
...
<Tip>
不要在文档中硬编码“所有模型”。权威列表是你的机器上 `discoverModels(...)` 返回的内容，以及可用的键名。
</Tip>
...
- 如果实时测试提示 `"no creds"`，请按调试 `openclaw models list` / 模型选择的同样方式调试。

- 按智能体的认证配置：`~/.openclaw/agents/<agentId>/agent/auth-profiles.json`（这就是实时测试中“profile keys”的含义）
- 配置：`~/.openclaw/openclaw.json`（或 `OPENCLAW_CONFIG_PATH`）
- 旧版状态目录：`~/.openclaw/credentials/`（存在时会复制到暂存的实时测试主目录，但不是主要的配置键存储）
- 本地实时运行默认会把活动配置、按智能体的 `auth-profiles.json` 文件、旧版 `credentials/`，以及受支持的外部 CLI 认证目录复制到临时测试主目录；暂存的实时测试主目录会跳过 `workspace/` 和 `sandboxes/`，并剥离 `agents.*.workspace` / `agentDir` 路径覆盖，以确保探测不会触及你的真实主机工作区。

如果你想依赖环境变量键，请在本地测试前导出它们，或使用下面的
...
- 范围：
  - 枚举每个已注册的图像生成提供商插件
  - 探测前使用已经导出的提供商环境变量
  - 默认优先使用实时/环境变量 API key，而不是已存储的认证配置，因此 `auth-profiles.json` 中过期的测试键不会遮蔽真实的 shell 凭据
  - 跳过没有可用认证/配置/模型的提供商
...
- 可选认证行为：
  - `OPENCLAW_LIVE_REQUIRE_PROFILE_KEYS=1` 用于强制使用配置存储认证，并忽略仅环境变量的覆盖
...
- 范围：
  - 覆盖共享的内置视频生成提供商路径
  - 默认使用适合发布的冒烟路径：非 FAL 提供商、每个提供商一个文本转视频请求、一秒龙虾提示词，以及来自 `OPENCLAW_LIVE_VIDEO_GENERATION_TIMEOUT_MS` 的按提供商操作上限（默认为 `180000`）
  - 默认跳过 FAL，因为提供商侧队列延迟可能主导发布时间；传入 `--video-providers fal` 或 `OPENCLAW_LIVE_VIDEO_GENERATION_PROVIDERS="fal"` 可显式运行它
  - 探测前使用已经导出的提供商环境变量
  - 默认优先使用实时/环境变量 API key，而不是已存储的认证配置，因此 `auth-profiles.json` 中过期的测试键不会遮蔽真实的 shell 凭据
  - 跳过没有可用认证/配置/模型的提供商
  - 默认只运行 `generate`
  - 设置 `OPENCLAW_LIVE_VIDEO_GENERATION_FULL_MODES=1` 后，可用时也运行声明的转换模式：
  ...
  - 当前在共享扫描中已声明但跳过的 `videoToVideo` 提供商：
    ...
    - `openai`，因为当前共享通道缺少特定组织的视频编辑访问保证
- 可选缩小范围：
