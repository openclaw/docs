...
对于快速开始、QA 运行器、单元/集成套件和 Docker 流程，请参阅
[测试](/zh-CN/help/testing)。本页介绍 **联网**（会触达网络）的测试
套件：模型矩阵、CLI 后端、ACP 和媒体提供商联网测试，以及
凭证处理。

## 联网：本地配置文件冒烟命令

在临时联网检查前先 source `~/.profile`，确保提供商密钥和本地工具
路径与你的 shell 匹配：
...
除非同时存在 `--yes`，否则 `voicecall smoke` 是一次空运行。仅在你有意发起真实通知通话时才使用 `--yes`。对于 Twilio、Telnyx 和
Plivo，成功的就绪检查需要一个公开的 webhook URL；按设计会拒绝仅本地
loopback/私有回退。
...
- “直接模型”告诉我们，在给定密钥下，提供商/模型本身是否能回答。
- “Gateway 网关冒烟”告诉我们完整的 gateway+智能体流水线是否适用于该模型（会话、历史、工具、沙箱策略等）。
...
- 设置 `OPENCLAW_LIVE_MODELS=modern`（或 `all`，modern 的别名）才会实际运行此套件；否则会跳过，以便让 `pnpm test:live` 专注于 Gateway 网关冒烟
- 如何选择模型：
  ...
  - 或 `OPENCLAW_LIVE_MODELS="openai/gpt-5.5,openai-codex/gpt-5.5,anthropic/claude-opus-4-6,..."`（逗号 allowlist）
  - Modern/all 扫描默认使用精选的高信号上限；设置 `OPENCLAW_LIVE_MAX_MODELS=0` 可执行完整现代扫描，或设置正数来使用更小上限。
  - 完整扫描使用 `OPENCLAW_LIVE_TEST_TIMEOUT_MS` 作为整个直接模型测试超时时间。默认值：60 分钟。
...
### 第 2 层：Gateway 网关 + 开发智能体冒烟（“@openclaw” 实际做的事）
...
- 如何选择模型：
  ...
  - Modern/all Gateway 网关扫描默认使用精选的高信号上限；设置 `OPENCLAW_LIVE_GATEWAY_MAX_MODELS=0` 可执行完整现代扫描，或设置正数来使用更小上限。
...
## 联网：CLI 后端冒烟（Claude、Codex、Gemini 或其他本地 CLI）
...
- 覆盖（可选）：
  ...
  - `OPENCLAW_LIVE_CLI_BACKEND_IMAGE_PROBE=1` 发送真实图像附件（路径会注入到提示中）。Docker 配方默认关闭此项，除非显式请求。
  ...
  - `OPENCLAW_LIVE_CLI_BACKEND_MODEL_SWITCH_PROBE=1` 在所选模型支持切换目标时，选择加入 Claude Sonnet -> Opus 同会话连续性探测。Docker 配方为聚合可靠性默认关闭此项。
...
- 它以非 root 的 `node` 用户在仓库 Docker 镜像内运行联网 CLI 后端冒烟。
...
- `pnpm test:docker:live-cli-backend:claude-subscription` 需要通过包含 `claudeAiOauth.subscriptionType` 的 `~/.claude/.credentials.json` 或来自 `claude setup-token` 的 `CLAUDE_CODE_OAUTH_TOKEN` 提供可移植的 Claude Code 订阅 OAuth。它会先在 Docker 中证明直接 `claude -p` 可用，然后在不保留 Anthropic API key 环境变量的情况下运行两轮 Gateway 网关 CLI 后端。此订阅路径默认禁用 Claude MCP/工具和图像探测，因为 Claude 目前会将第三方应用使用路由到额外用量计费，而不是普通订阅计划限制。
- 联网 CLI 后端冒烟现在会对 Claude、Codex 和 Gemini 演练同一个端到端流程：文本轮次、图像分类轮次，然后通过 Gateway 网关 CLI 验证 MCP `cron` 工具调用。
- Claude 的默认冒烟还会把会话从 Sonnet 修补为 Opus，并验证恢复的会话仍然记得之前的备注。

## 联网：ACP 绑定冒烟（`/acp spawn ... --bind here`）
...
- 目标：使用实时 ACP 智能体验证真实的 ACP conversation-bind 流程：
  ...
  - 就地绑定一个合成的消息渠道会话
  - 在同一个会话上发送普通后续消息
  - 验证后续消息落入已绑定 ACP 会话的 transcript
- 启用：
...
- 备注：
  - 此通道使用 Gateway 网关 `chat.send` 接口面，并带有仅限管理员的合成 originating-route 字段，这样测试就可以附加消息渠道上下文，而不假装对外投递。
  - 未设置 `OPENCLAW_LIVE_ACP_BIND_AGENT_COMMAND` 时，测试会为所选 ACP harness 智能体使用内嵌 `acpx` 插件的内置智能体注册表。
  - 已绑定会话的 cron MCP 创建默认是尽力而为，因为外部 ACP harness 可能会在绑定/图像证明通过后取消 MCP 调用；设置 `OPENCLAW_LIVE_ACP_BIND_REQUIRE_CRON=1` 可使该绑定后 cron 探针变为严格模式。
...
- 它会读取 `~/.profile`，把匹配的 CLI 认证材料暂存到容器中，然后在缺失时安装请求的实时 CLI（`@anthropic-ai/claude-code`、`@openai/codex`、通过 `https://app.factory.ai/cli` 安装的 Factory Droid、`@google/gemini-cli` 或 `opencode-ai`）。ACP 后端本身是来自 `acpx` 插件的内置捆绑 `acpx/runtime` 包。
- Droid Docker 变体会暂存 `~/.factory` 用于设置，转发 `FACTORY_API_KEY`，并且要求该 API key，因为本地 Factory OAuth/keyring 认证无法移植到容器中。它使用 ACPX 内置的 `droid exec --output-format acp` 注册表条目。
- OpenCode Docker 变体是严格的单智能体回归通道。它在读取 `~/.profile` 后，根据 `OPENCLAW_LIVE_ACP_BIND_OPENCODE_MODEL`（默认 `opencode/kimi-k2.6`）写入临时 `OPENCODE_CONFIG_CONTENT` 默认模型，并且 `pnpm test:docker:live-acp-bind:opencode` 要求已绑定的 assistant transcript，而不是接受通用的绑定后跳过。
...
- 目标：通过普通 Gateway 网关
  ...
  - 可选运行两个经过 Guardian 审查的提权 shell 探针：一个应获批准的良性
...
- 冒烟测试会设置 `OPENCLAW_AGENT_HARNESS_FALLBACK=none`，这样损坏的 Codex
  harness 就无法通过静默回退到 PI 而通过测试。
...
- 它会读取挂载的 `~/.profile`，传递 `OPENAI_API_KEY`，在存在时复制 Codex CLI
  ...
  前缀，暂存源代码树，然后只运行 Codex harness 实时测试。
...
- Docker 还会导出 `OPENCLAW_AGENT_HARNESS_FALLBACK=none`，与实时
  测试配置一致，这样旧版别名或 PI 回退就无法隐藏 Codex harness
  回归。

### 推荐的实时配方

窄范围、显式的允许列表最快且最不容易波动：
...
- Gemini API 与 Gemini CLI：
  ...
  - CLI：OpenClaw shell 调用本地 `gemini` 二进制文件；它有自己的认证，并且行为可能不同（流式传输/工具支持/版本偏差）。

## 实时：模型矩阵（覆盖范围）

没有固定的 “CI 模型列表”（实时测试是选择启用），但下面是建议在有密钥的开发机器上定期覆盖的**推荐**模型。
...
在 `OPENCLAW_LIVE_GATEWAY_MODELS` 中至少包含一个支持图像的模型（Claude/Gemini/OpenAI 视觉能力变体等），以执行图像探针。
...
<Tip>
不要在文档中硬编码“所有模型”。权威列表是你机器上 `discoverModels(...)` 返回的内容，加上可用密钥所支持的内容。
</Tip>

## 凭据（切勿提交）

实时测试会用与 CLI 相同的方式发现凭据。实际影响：

- 如果 CLI 可用，真实服务测试应该能找到相同的键。
- 如果真实服务测试提示“no creds”，请按调试 `openclaw models list` / 模型选择的方式来调试。

- 按智能体划分的认证配置文件：`~/.openclaw/agents/<agentId>/agent/auth-profiles.json`（这就是真实服务测试中“profile keys”的含义）
- 配置：`~/.openclaw/openclaw.json`（或 `OPENCLAW_CONFIG_PATH`）
- 旧版状态目录：`~/.openclaw/credentials/`（存在时会复制到暂存的真实服务主目录中，但它不是主要的配置文件键存储位置）
- 本地真实服务运行默认会把活动配置、按智能体划分的 `auth-profiles.json` 文件、旧版 `credentials/`，以及受支持的外部 CLI 认证目录复制到临时测试主目录；暂存的真实服务主目录会跳过 `workspace/` 和 `sandboxes/`，并移除 `agents.*.workspace` / `agentDir` 路径覆盖，这样探测就不会访问你的真实主机工作区。

如果你想依赖环境键（例如在你的 `~/.profile` 中导出的键），请在 `source ~/.profile` 后运行本地测试，或使用下面的 Docker 运行器（它们可以把 `~/.profile` 挂载到容器中）。

## Deepgram 真实服务测试（音频转录）
...
- 范围：
  ...
  - 默认优先使用真实服务/环境 API key，而不是已存储的认证配置文件，因此 `auth-profiles.json` 中过期的测试键不会遮蔽真实的 shell 凭证
...
- 可选认证行为：
  - 设置 `OPENCLAW_LIVE_REQUIRE_PROFILE_KEYS=1` 以强制使用配置文件存储认证，并忽略仅环境变量覆盖

对于已发布的 CLI 路径，在提供商/运行时真实服务测试通过后添加一次 `infer` 冒烟测试：
...
这会覆盖 CLI 参数解析、配置/默认智能体解析、内置插件激活、共享图像生成运行时，以及真实服务提供商请求。插件依赖应在运行时加载前存在。
...
- 范围：
  ...
  - 当前共享通道覆盖范围：
    ...
    - `minimax`：`generate`
    - `comfy`：单独的 Comfy 真实服务测试文件，不在此共享扫描中
- 可选缩小范围：
...
- 范围：
  - 覆盖共享的内置视频生成提供商路径
  - 默认使用发布安全的冒烟路径：非 FAL 提供商、每个提供商一次文本转视频请求、一秒龙虾提示词，以及来自 `OPENCLAW_LIVE_VIDEO_GENERATION_TIMEOUT_MS` 的按提供商操作上限（默认 `180000`）
  - 默认跳过 FAL，因为提供商侧队列延迟可能占用主要发布时间；传入 `--video-providers fal` 或 `OPENCLAW_LIVE_VIDEO_GENERATION_PROVIDERS="fal"` 可显式运行它
  - 在探测前，从你的登录 shell（`~/.profile`）加载提供商环境变量
  - 默认优先使用真实服务/环境 API key，而不是已存储的认证配置文件，因此 `auth-profiles.json` 中过期的测试键不会遮蔽真实的 shell 凭证
  - 跳过没有可用认证/配置文件/模型的提供商
  - 默认只运行 `generate`
  - 设置 `OPENCLAW_LIVE_VIDEO_GENERATION_FULL_MODES=1`，以便在可用时同时运行已声明的转换模式：
...
- 目的：
  - 通过一个仓库原生入口点运行共享图像、音乐和视频真实服务测试套件
