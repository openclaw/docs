...
title: Web 搜索
x-i18n:
    generated_at: "2026-05-02T06:11:18Z"
    model: gpt-5.5
    provider: openai
...
`web_search` 工具使用你配置的提供商搜索网络并返回结果。结果会按查询缓存 15 分钟（可配置）。
...
<Info>
  ...
  JS 密集型网站或登录，请使用 [Web Browser](/zh-CN/tools/browser)。若要获取特定 URL，请使用 [Web Fetch](/zh-CN/tools/web-fetch)。
</Info>
...
<Steps>
  <Step title="Choose a provider">
    选择一个提供商并完成所有必需设置。有些提供商无需密钥，而另一些使用 API 密钥。详情请参阅下面的提供商页面。
  </Step>
  <Step title="Configure">
    ```bash
    openclaw configure --section web
    ```
    这会存储提供商和任何所需凭证。你也可以设置环境变量（例如 `BRAVE_API_KEY`），并针对 API 支持的提供商跳过此步骤。
  </Step>
  <Step title="Use it">
...
<CardGroup cols={2}>
  ...
  <Card title="DuckDuckGo" icon="bird" href="/zh-CN/tools/duckduckgo-search">
    免密钥后备方案。无需 API 密钥。基于 HTML 的非官方集成。
  </Card>
  ...
  <Card title="Firecrawl" icon="flame" href="/zh-CN/tools/firecrawl">
    结构化结果。与 `firecrawl_search` 和 `firecrawl_scrape` 搭配用于深度提取效果最佳。
  </Card>
  ...
  <Card title="Kimi" icon="moon" href="/zh-CN/tools/kimi-search">
    通过 Moonshot web search 提供带引用的 AI 合成答案；未 grounding 的聊天后备方案会明确失败。
  </Card>
...
| [Kimi](/zh-CN/tools/kimi-search)                | AI 合成 + 引用；未 grounding 的聊天后备方案会失败              | --                                               | `KIMI_API_KEY` / `MOONSHOT_API_KEY`                                                     |
...
| [Ollama Web Search](/zh-CN/tools/ollama-search) | 结构化摘要片段                                                 | --                                               | 已登录的本地主机无需；直接 `https://ollama.com` 搜索需要 `OLLAMA_API_KEY`               |
...
当 OpenClaw web search 已启用且没有固定托管提供商时，直接 OpenAI Responses 模型会自动使用 OpenAI 托管的 `web_search` 工具。这是内置 OpenAI 插件中的提供商自有行为，并且只适用于原生 OpenAI API 流量，不适用于 OpenAI 兼容代理基础 URL 或 Azure 路由。将 `tools.web.search.provider` 设置为其他提供商（例如 `brave`），可为 OpenAI 模型保留托管 `web_search` 工具；或将 `tools.web.search.enabled: false` 设置为禁用托管搜索和原生 OpenAI 搜索。
...
如果已启用原生 Codex 搜索，但当前模型不支持 Codex，OpenClaw 会保留正常的托管 `web_search` 行为。
...
11. **Ollama Web Search** -- 当你配置的本地 Ollama 主机可访问且已通过 `ollama signin` 登录时，通过该主机提供免密钥后备；当主机需要时，可以复用 Ollama provider bearer auth；在配置了 `OLLAMA_API_KEY` 时，可以直接调用 `https://ollama.com` 搜索（顺序 110）
12. **SearXNG** -- `SEARXNG_BASE_URL` 或 `plugins.entries.searxng.config.webSearch.baseUrl`（顺序 200）

如果未检测到提供商，它会回退到 Brave（你会收到提示配置密钥的缺失密钥错误）。

<Note>
  所有提供商密钥字段都支持 SecretRef 对象。对于内置的 API 支持 web search 提供商，位于 `plugins.entries.<plugin>.config.webSearch.apiKey` 下的插件作用域 SecretRef 会被解析，包括 Brave、Exa、Firecrawl、Gemini、Grok、Kimi、MiniMax、Perplexity 和 Tavily，无论该提供商是通过 `tools.web.search.provider` 显式选中，还是通过自动检测选中。在自动检测模式下，OpenClaw 只解析所选提供商的密钥 -- 未选中的 SecretRef 保持不活跃，因此你可以配置多个提供商，而无需为未使用的提供商支付解析成本。
</Note>
...
提供商特定配置（API 密钥、基础 URL、模式）位于 `plugins.entries.<plugin>.config.webSearch.*` 下。Gemini 还可以复用 `models.providers.google.apiKey` 和 `models.providers.google.baseUrl`，作为其专用 web-search 配置和 `GEMINI_API_KEY` 之后较低优先级的后备。示例请参阅提供商页面。

`web_fetch` 后备提供商选择是独立的：

- 使用 `tools.web.fetch.provider` 选择它
- 或省略该字段，让 OpenClaw 从可用凭证中自动检测第一个就绪的网页抓取提供商
- 非沙箱隔离的 `web_fetch` 可以使用声明了 `contracts.webFetchProviders` 的已安装插件提供商；沙箱隔离的抓取仍仅限内置提供商
- 目前内置的网页抓取提供商是 Firecrawl，在 `plugins.entries.firecrawl.config.webFetch.*` 下配置

当你在 `openclaw onboard` 或 `openclaw configure --section web` 期间选择 **Kimi** 时，OpenClaw 还可以要求提供：
...
当你在 `openclaw onboard` 或 `openclaw configure --section web` 期间选择 Grok 时，OpenClaw 还可以使用同一个密钥提供可选的 `x_search` 设置。
这是 Grok 路径内的单独后续步骤，而不是单独的顶层 Web 搜索提供商选项。如果你选择其他提供商，OpenClaw 不会显示 `x_search` 提示。
...
<Warning>
  ...
  Gemini、Grok 和 Kimi 会返回一个带引用的综合答案。它们接受 `count` 以兼容共享工具，但该参数不会改变基于检索的答案形态。Gemini 支持 `freshness`、`date_after` 和 `date_before`，方式是将它们转换为 Google Search grounding 时间范围。
  ...
  SearXNG 仅允许受信任的私有网络或 local loopback 主机使用 `http://`；公共 SearXNG 端点必须使用 `https://`。
  Firecrawl 和 Tavily 通过 `web_search` 仅支持 `query` 和 `count`，高级选项请使用它们的专用工具。
</Warning>

## x_search

`x_search` 使用 xAI 查询 X（前 Twitter）帖子，并返回带引用的 AI 综合答案。它接受自然语言查询和可选的结构化过滤器。OpenClaw 只会在服务此工具调用的请求中启用内置 xAI `x_search` 工具。

<Note>
  xAI 文档说明 `x_search` 支持关键词搜索、语义搜索、用户搜索和线程抓取。对于每条帖子的互动统计，例如转发、回复、书签或浏览量，优先使用精确帖子 URL 或状态 ID 进行定向查找。宽泛的关键词搜索可能会找到正确帖子，但返回的每条帖子元数据可能不够完整。一个好的模式是：先定位帖子，然后再运行第二次 `x_search` 查询，聚焦于该精确帖子。
</Note>
...
当设置了 `plugins.entries.xai.config.xSearch.baseUrl` 时，`x_search` 会向 `<baseUrl>/responses` 发帖。如果省略该字段，它会回退到 `plugins.entries.xai.config.webSearch.baseUrl`，然后是旧版 `tools.web.search.grok.baseUrl`，最后是公共 xAI 端点。
...
- [Web 浏览器](/zh-CN/tools/browser) -- 面向 JS 密集型站点的完整浏览器自动化
- [Grok 搜索](/zh-CN/tools/grok-search) -- 作为 `web_search` 提供商的 Grok
