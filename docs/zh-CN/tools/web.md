...
title: Web 搜索
x-i18n:
    generated_at: "2026-05-02T05:11:00Z"
    model: gpt-5.5
    provider: openai
...
<Info>
  ...
  JS 密集型网站或登录场景，请使用 [Web Browser](/zh-CN/tools/browser)。如需
...
<Steps>
  <Step title="选择提供商">
    选择一个提供商并完成所有必需设置。有些提供商
    无需密钥，而其他提供商使用 API key。详情请参阅下面的提供商页面。
  </Step>
  <Step title="配置">
    ```bash
    openclaw configure --section web
    ```
    这会存储提供商以及任何所需凭据。你也可以设置一个环境变量
...
<CardGroup cols={2}>
  <Card title="Brave Search" icon="shield" href="/zh-CN/tools/brave-search">
    带摘要的结构化结果。支持 `llm-context` 模式、国家/语言过滤器。提供免费层。
  </Card>
  ...
  <Card title="Firecrawl" icon="flame" href="/zh-CN/tools/firecrawl">
    结构化结果。与 `firecrawl_search` 和 `firecrawl_scrape` 搭配使用时，最适合深度提取。
  </Card>
...
当 OpenClaw Web 搜索启用且未固定托管提供商时，直接 OpenAI Responses 模型会自动使用 OpenAI 托管的 `web_search` 工具。这是内置 OpenAI 插件中的提供商自有行为，并且只适用于原生 OpenAI API 流量，不适用于 OpenAI 兼容代理基准 URL 或 Azure 路由。将 `tools.web.search.provider` 设置为其他提供商（例如 `brave`），即可为 OpenAI 模型保留托管的 `web_search` 工具；或者设置 `tools.web.search.enabled: false` 来同时禁用托管搜索和原生 OpenAI 搜索。
...
10. **DuckDuckGo** -- 无需账户或 API key 的无密钥 HTML 回退方案（顺序 100）
11. **Ollama Web Search** -- 当你配置的本地 Ollama 主机可访问且已通过 `ollama signin` 登录时，通过该主机提供的无密钥回退方案；当主机需要时可复用 Ollama provider bearer auth，并且在配置 `OLLAMA_API_KEY` 后可调用直接 `https://ollama.com` 搜索（顺序 110）
...
<Note>
  所有提供商密钥字段都支持 SecretRef 对象。对于内置的 API 支持 Web 搜索提供商，
  位于 `plugins.entries.<plugin>.config.webSearch.apiKey` 下的插件作用域 SecretRef
  会被解析，包括 Brave、Exa、Firecrawl、
  Gemini、Grok、Kimi、MiniMax、Perplexity 和 Tavily，
  无论该提供商是通过 `tools.web.search.provider` 显式选择，
  还是通过自动检测选择。在自动检测模式下，OpenClaw 只解析
  已选提供商的密钥 -- 未选中的 SecretRef 会保持非活动状态，因此你可以
...
提供商特定配置（API keys、基准 URL、模式）位于
`plugins.entries.<plugin>.config.webSearch.*` 下。Gemini 还可以在专用 Web 搜索配置和 `GEMINI_API_KEY` 之后，以较低优先级回退方式复用
...
- 或省略该字段，让 OpenClaw 从可用凭据中自动检测第一个已就绪的 Web 获取
  提供商
- 非沙箱隔离的 `web_fetch` 可以使用声明
  `contracts.webFetchProviders` 的已安装插件提供商；沙箱隔离获取保持仅内置
- 目前内置的 Web 获取提供商是 Firecrawl，配置位于
...
对于 `x_search`，配置 `plugins.entries.xai.config.xSearch.*`。它使用与 Grok Web 搜索相同的 `XAI_API_KEY` 后备值。
旧版 `tools.web.x_search.*` 配置会由 `openclaw doctor --fix` 自动迁移。
当你在 `openclaw onboard` 或 `openclaw configure --section web` 期间选择 Grok 时，OpenClaw 也可以提供使用同一密钥的可选 `x_search` 设置。
这是 Grok 路径中的一个单独后续步骤，而不是单独的顶层 Web 搜索提供商选择。如果你选择其他提供商，OpenClaw 不会显示 `x_search` 提示。
...
<Warning>
  ...
  Gemini、Grok 和 Kimi 会返回一个带引用的综合答案。它们接受 `count` 以兼容共享工具，但该参数不会改变有依据答案的形态。Gemini 支持 `freshness`、`date_after` 和 `date_before`，会将它们转换为 Google Search grounding 时间范围。
  ...
  SearXNG 仅对受信任的私有网络或 local loopback 主机接受 `http://`；公开的 SearXNG 端点必须使用 `https://`。
  Firecrawl 和 Tavily 通过 `web_search` 仅支持 `query` 和 `count`，高级选项请使用它们的专用工具。
</Warning>

## x_search

`x_search` 使用 xAI 查询 X（原 Twitter）帖子，并返回带引用的 AI 综合答案。它接受自然语言查询和可选的结构化过滤器。OpenClaw 只会在服务于此工具调用的请求上启用内置 xAI `x_search` 工具。

<Note>
  xAI 文档说明 `x_search` 支持关键词搜索、语义搜索、用户搜索和主题串获取。对于每帖互动统计信息，例如转帖、回复、收藏或浏览量，优先针对确切帖子 URL 或状态 ID 进行定向查询。广泛的关键词搜索可能找到正确的帖子，但返回的每帖元数据可能不够完整。一个好的模式是：先定位帖子，然后运行第二次 `x_search` 查询，聚焦于该确切帖子。
</Note>
...
- [Web 浏览器](/zh-CN/tools/browser) -- 针对 JS 密集型网站的完整浏览器自动化
