...
title: 网页搜索
x-i18n:
    generated_at: "2026-05-07T01:54:30Z"
    model: gpt-5.5
    provider: openai
...
OpenClaw 还包含用于搜索 X（原 Twitter）帖子的 `x_search`，以及用于轻量级 URL 获取的 `web_fetch`。在这一阶段，`web_fetch` 保持本地执行，而 `web_search` 和 `x_search` 可以在底层使用 xAI Responses。

<Info>
  `web_search` 是一个轻量级 HTTP 工具，不是浏览器自动化。对于
  JS 较重的网站或登录场景，请使用 [Web Browser](/zh-CN/tools/browser)。如需
...
<Steps>
  <Step title="选择提供商">
    ...
    另一些则使用 API key。详情请参阅下方的提供商页面。
  </Step>
  <Step title="配置">
    ```bash
    openclaw configure --section web
    ```
    这会存储提供商以及任何所需凭证。你也可以设置环境变量
...
<CardGroup cols={2}>
  <Card title="Brave Search" icon="shield" href="/zh-CN/tools/brave-search">
    提供带摘要片段的结构化结果。支持 `llm-context` 模式、国家/地区和语言筛选。提供免费层级。
  </Card>
  ...
  <Card title="Kimi" icon="moon" href="/zh-CN/tools/kimi-search">
    通过 Moonshot Web 搜索提供带引用的 AI 综合答案；未 grounded 的聊天备用方案会明确失败。
  </Card>
  ...
  <Card title="Tavily" icon="globe" href="/zh-CN/tools/tavily">
    提供结构化结果，并支持搜索深度、主题筛选，以及用于 URL 提取的 `tavily_extract`。
  </Card>
</CardGroup>
...
| [Kimi](/zh-CN/tools/kimi-search)                | AI 综合答案 + 引用；在未 grounded 的聊天备用方案上失败         | --                                               | `KIMI_API_KEY` / `MOONSHOT_API_KEY`                                                     |
...
| [Ollama Web Search](/zh-CN/tools/ollama-search) | 结构化摘要片段                                                 | --                                               | 已登录本地主机无需密钥；直接搜索 `https://ollama.com` 时使用 `OLLAMA_API_KEY`           |
...
当 OpenClaw Web 搜索已启用且未固定托管提供商时，直接的 OpenAI Responses 模型会自动使用 OpenAI 托管的 `web_search` 工具。这是内置 OpenAI 插件中的提供商自有行为，并且只适用于原生 OpenAI API 流量，不适用于 OpenAI 兼容的代理 base URL 或 Azure 路由。将 `tools.web.search.provider` 设置为其他提供商（如 `brave`）可为 OpenAI 模型保留托管的 `web_search` 工具，或设置 `tools.web.search.enabled: false` 以同时禁用托管搜索和原生 OpenAI 搜索。
...
如果已启用原生 Codex 搜索，但当前模型不支持 Codex，OpenClaw 会保留普通的托管 `web_search` 行为。

## 网络安全

托管的 `web_search` 提供商调用会使用 OpenClaw 的受保护 fetch 路径。对于
受信任的提供商 API 主机，OpenClaw 仅对该提供商主机名允许 Surge、Clash 和 sing-box 的 fake-IP
DNS 答案使用 `198.18.0.0/15` 和 `fc00::/7`。
其他私有、loopback、link-local 和 metadata 目标仍会被阻止。

此自动放行不适用于任意 `web_fetch` URL。对于
`web_fetch`，只有在你的受信任代理拥有这些合成范围时，才应显式启用
`tools.web.fetch.ssrfPolicy.allowRfc2544BenchmarkRange` 和
`tools.web.fetch.ssrfPolicy.allowIpv6UniqueLocalRange`。

## 设置 Web 搜索

文档和设置流程中的提供商列表按字母顺序排列。自动检测会保留
单独的优先级顺序。
...
10. **DuckDuckGo** -- 无需密钥的 HTML 备用方案，不需要账号或 API key（顺序 100）
11. **Ollama Web Search** -- 当你配置的本地 Ollama 主机可访问且已通过 `ollama signin` 登录时，通过它提供无需密钥的备用方案；当主机需要时可复用 Ollama provider bearer auth，并且在配置 `OLLAMA_API_KEY` 后可调用直接的 `https://ollama.com` 搜索（顺序 110）
...
<Note>
  所有提供商密钥字段都支持 SecretRef 对象。对于内置的 API 支持
  Web 搜索提供商，`plugins.entries.<plugin>.config.webSearch.apiKey`
  下插件作用域的 SecretRef 会被解析，包括 Brave、Exa、Firecrawl、
  Gemini、Grok、Kimi、MiniMax、Perplexity 和 Tavily，
  无论提供商是通过 `tools.web.search.provider` 显式选择，还是
  通过自动检测选中。在自动检测模式下，OpenClaw 只会解析
  选中提供商的密钥 -- 未选中的 SecretRef 会保持未激活，因此你可以
...
`models.providers.google.apiKey` 和 `models.providers.google.baseUrl`，作为低于其专用 Web 搜索配置和 `GEMINI_API_KEY` 优先级的
fallback。示例见提供商页面。

`tools.web.search.provider` 会根据内置和已安装插件清单声明的 Web 搜索提供商 ID，
以及已知可安装的提供商插件进行验证。像 `"brvae"` 这样的拼写错误会导致配置验证失败，
而不是静默回退到自动检测。如果配置的提供商已知，但所属插件不可用，OpenClaw 会保持启动韧性并报告警告，
这样你可以运行 `openclaw doctor --fix` 来安装或启用该插件。
同样的警告行为也适用于过期插件证据，例如卸载第三方插件后遗留的
`plugins.entries.<plugin>` 块。

`web_fetch` fallback 提供商选择是独立的：

- 使用 `tools.web.fetch.provider` 选择它
- 或省略该字段，让 OpenClaw 从可用凭证中自动检测第一个就绪的 web-fetch 提供商
- 非沙箱隔离的 `web_fetch` 可以使用声明了 `contracts.webFetchProviders` 的已安装插件提供商；沙箱隔离的 fetch 保持仅限内置
...
对于 `x_search`，配置 `plugins.entries.xai.config.xSearch.*`。它使用与 Grok Web 搜索相同的
...
OpenClaw 还可以使用同一 key 提供可选的 `x_search` 设置。
这是 Grok 路径中的一个独立后续步骤，不是单独的顶层 Web 搜索提供商选择。
如果你选择另一个提供商，OpenClaw 不会显示 `x_search` 提示。
...
| `max_tokens_per_page` | 单页 token 限制，默认 2048（仅 Perplexity）           |

<Warning>
  并非所有参数都适用于所有提供商。Brave `llm-context` 模式会拒绝
  `ui_lang`；`date_before` 也需要 `date_after`，因为 Brave 自定义新鲜度范围要求同时提供开始和结束日期。
  Gemini、Grok 和 Kimi 会返回一个带引用的综合答案。它们接受 `count` 以兼容共享工具，
  但这不会改变有依据答案的形态。Gemini 支持 `freshness`、`date_after` 和
  `date_before`，会将它们转换为 Google Search grounding 时间范围。
  ...
  SearXNG 仅对可信的私有网络或 local loopback 主机接受 `http://`；
...
`x_search` 使用 xAI 查询 X（原 Twitter）帖子，并返回带引用的
AI 综合答案。它接受自然语言查询和可选的结构化筛选器。OpenClaw 仅在服务此工具调用的请求中启用内置 xAI `x_search`
工具。

<Note>
  xAI 文档说明 `x_search` 支持关键词搜索、语义搜索、用户搜索和 thread fetch。
  对于转发、回复、书签或浏览量等单帖互动统计，优先对精确帖子 URL
  或状态 ID 进行定向查找。宽泛的关键词搜索可能找到正确帖子，但返回的单帖元数据不够完整。
  一个好的模式是：先定位帖子，然后运行第二个聚焦该精确帖子的 `x_search` 查询。
</Note>
...
- [Grok Search](/zh-CN/tools/grok-search) -- 作为 `web_search` 提供商的 Grok
- [Ollama Web Search](/zh-CN/tools/ollama-search) -- 通过你的 Ollama 主机进行免 key Web 搜索
