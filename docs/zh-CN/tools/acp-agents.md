...
title: ACP 智能体
x-i18n:
    generated_at: "2026-07-05T01:59:06Z"
    model: gpt-5.5
...
每个 ACP 会话生成都会作为一个[后台任务](/zh-CN/automation/tasks)进行跟踪。

<Note>
**ACP 是外部运行框架路径，不是默认 Codex 路径。** 原生
Codex app-server 插件拥有 `/codex ...` 控制和用于 Agent 轮次的默认
`openai/gpt-*` 嵌入式运行时；ACP 拥有
`/acp ...` 控制和 `sessions_spawn({ runtime: "acp" })` 会话。

如果你想让 Codex 或 Claude Code 作为外部 MCP 客户端直接连接到现有的
OpenClaw 频道对话，请使用 [`openclaw mcp serve`](/zh-CN/cli/mcp)，而不是 ACP。
</Note>
...
| 通过 OpenClaw 运行 Claude Code、Gemini CLI、显式 Codex ACP 或其他外部运行框架 | 本页面                             | 绑定聊天的会话、`/acp spawn`、`sessions_spawn({ runtime: "acp" })`、后台任务、运行时控制                                                                                   |
...
| 将本地 AI CLI 复用为纯文本回退模型                                              | [CLI 后端](/zh-CN/gateway/cli-backends) | 不是 ACP。没有 OpenClaw 工具，没有 ACP 控制，没有运行框架运行时                                                                                                                               |
...
源码检出可以在 `pnpm install` 后使用本地 `extensions/acpx` 工作区插件。
运行 `/acp doctor` 进行就绪检查。

只有当 ACP **确实可用**时，OpenClaw 才会向 Agent 说明 ACP 生成能力：
ACP 必须已启用，调度不能被禁用，当前会话不能被沙箱阻止，并且必须已加载运行时后端。
如果不满足这些条件，ACP 插件 Skills 和
`sessions_spawn` ACP 指引会保持隐藏，这样 Agent 就不会建议不可用的后端。

<AccordionGroup>
  <Accordion title="首次运行注意事项">
    - 如果设置了 `plugins.allow`，它就是限制性的插件清单，并且**必须**包含 `acpx`；否则已安装的 ACP 后端会被有意阻止，`/acp doctor` 会报告缺少的 allowlist 条目。
    - Codex ACP 适配器随 `acpx` 插件一起暂存，并在可行时本地启动。
    - Codex ACP 使用隔离的 `CODEX_HOME` 运行；OpenClaw 会从宿主 Codex 配置复制受信任的项目条目以及安全的模型/提供商路由配置，而身份验证、通知和钩子仍保留在宿主配置中。
    - 其他目标运行框架适配器在首次使用时可能仍会按需通过 `npx` 获取。
    - 该运行框架的供应商身份验证仍必须存在于宿主上。
    - 如果宿主没有 npm 或网络访问权限，首次运行的适配器获取会失败，直到缓存已预热或通过其他方式安装适配器。

  </Accordion>
  <Accordion title="运行时前置条件">
    ACP 会启动真实的外部运行框架进程。OpenClaw 负责路由、
    后台任务状态、投递、绑定和策略；运行框架负责自己的提供商登录、
    模型目录、文件系统行为和原生工具。

    在归因于 OpenClaw 之前，请验证：

    - `/acp doctor` 报告后端已启用且健康。
    - 设置该 allowlist 时，目标 id 被 `acp.allowedAgents` 允许。
    - 运行框架命令可以在 Gateway 网关宿主上启动。
    - 该运行框架存在提供商身份验证（`claude`、`codex`、`gemini`、`opencode`、`droid` 等）。
    - 所选模型对该运行框架存在 - 模型 id 不能跨运行框架移植。
    - 请求的 `cwd` 存在且可访问，或省略 `cwd`，让后端使用其默认值。
    - 权限模式与工作匹配。非交互式会话无法点击原生权限提示，因此写入/Exec 密集型编码运行通常需要一个可以无头继续执行的 ACPX 权限配置文件。
...
| 运行框架 id | 典型后端                                | 说明                                                                               |
| ---------- | ---------------------------------------------- | ----------------------------------------------------------------------------------- |
| `claude`   | Claude Code ACP 适配器                        | 需要宿主上有 Claude Code 身份验证。                                              |
...
| `droid`    | Factory Droid CLI                              | 需要运行框架环境中的 Factory/Droid 身份验证或 `FACTORY_API_KEY`。        |
| `gemini`   | Gemini CLI ACP 适配器                         | 需要 Gemini CLI 身份验证或 API key 设置。                                          |
| `iflow`    | iFlow CLI                                      | 适配器可用性和模型控制取决于已安装的 CLI。                 |
| `kilocode` | Kilo Code CLI                                  | 适配器可用性和模型控制取决于已安装的 CLI。                 |
| `kimi`     | Kimi/Moonshot CLI                              | 需要宿主上有 Kimi/Moonshot 身份验证。                                            |
| `kiro`     | Kiro CLI                                       | 适配器可用性和模型控制取决于已安装的 CLI。                 |
| `opencode` | OpenCode ACP 适配器                           | 需要 OpenCode CLI/提供商身份验证。                                                |
| `openclaw` | 通过 `openclaw acp` 的 OpenClaw Gateway 网关桥接 | 让支持 ACP 的运行框架回连 OpenClaw Gateway 网关会话。                 |
| `qwen`     | Qwen Code / Qwen CLI                           | 需要宿主上有 Qwen 兼容身份验证。                                          |

可以在 acpx 自身中配置自定义 acpx Agent 别名，但 OpenClaw
...
<AccordionGroup>
  <Accordion title="生命周期详情">
    ...
    - 父级拥有的 ACP 会话即使运行时会话是持久的，也会被视为后台工作；完成和跨表面投递会通过父任务通知器进行，而不是像普通面向用户的聊天会话那样运行。
    - 任务维护会关闭终止或孤立的父级拥有的一次性 ACP 会话。只要仍存在活跃对话绑定，持久 ACP 会话就会被保留；没有活跃绑定的陈旧持久会话会被关闭，这样它们就无法在所属任务完成或其任务记录消失后被静默恢复。
    ...
    - `close` 会从 OpenClaw 的角度结束 ACP 会话并移除绑定。如果运行框架支持恢复，它可能仍会保留自己的上游历史。
    - `acpx` 插件会在 `close` 后清理 OpenClaw 拥有的包装器和适配器进程树，并在 Gateway 网关启动期间回收陈旧的 OpenClaw 拥有的 ACPX 孤儿进程。
    - 空闲运行时工作进程在 `acp.runtime.ttlMinutes` 后符合清理条件；存储的会话元数据仍可供 `/acp sessions` 使用。

  </Accordion>
  <Accordion title="原生 Codex 路由规则">
    当启用**原生 Codex 插件**时，应路由到该插件的自然语言触发示例：
    ...
    Codex 原生工具（例如 shell/apply-patch）在 Codex 内执行。
    ...
    插件可在 Codex 最终确定回答前请求再进行一次
    模型调用。该中继有意保持保守：它不会修改 Codex 原生工具
  ...
  <Accordion title="ACP 路由自然语言触发条件">
    ...
    在支持时绑定到当前对话或线程，并
    将后续跟进路由到该会话，直到关闭/过期。只有当 ACP/acpx 被显式指定，
    或原生 Codex 插件不适用于所请求的操作时，
    Codex 才会走这条路径。

    对于 `sessions_spawn`，只有在 ACP
    已启用、请求方未被沙箱隔离且 ACP 运行时
    后端已加载时，才会公开 `runtime: "acp"`。`acp.dispatch.enabled=false`
    会暂停自动 ACP 线程调度，但不会隐藏或阻止显式
...
插件已启用时，使用**原生 Codex app-server** 进行 Codex 对话绑定/控制。
当你需要 OpenClaw 原生委托运行时，使用**子智能体**。
...
- **需要通过原始 CLI 进行简单本地文本后备？** 使用 CLI 后端。
...
生成的 ACP 会话 - 没有子线程，仍是同一聊天表面。OpenClaw 继续
掌管传输、认证、安全和投递。该对话中的后续消息会
路由到同一会话；`/new` 和 `/reset` 会就地重置
会话；`/acp close` 会移除绑定。
...
<AccordionGroup>
  <Accordion title="绑定规则与互斥性">
    ...
    - 如果你在没有 `--cwd` 的情况下生成到另一个 ACP 智能体，OpenClaw 默认继承**目标智能体的**工作区。缺失的继承路径（`ENOENT`/`ENOTDIR`）会回退到后端默认值；其他访问错误（例如 `EACCES`）会作为生成错误呈现。
  ...
  <Accordion title="线程绑定会话">
    ...
    - `acp.enabled=true`
    - `acp.dispatch.enabled` 默认开启（设为 `false` 可暂停自动 ACP 线程调度；显式 `sessions_spawn({ runtime: "acp" })` 调用仍可工作）。
...
- 对于没有显式 `cwd` 的跨智能体 ACP spawn，OpenClaw 会从智能体配置继承目标智能体工作区。
- 缺失的继承工作区路径会回退到后端默认 cwd；非缺失的访问失败会作为 spawn 错误暴露。
...
<Tabs>
  <Tab title="From sessions_spawn">
    ...
    <Note>
    `runtime` 默认是 `subagent`，因此需要为 ACP 会话显式设置 `runtime: "acp"`。如果省略 `agentId`，OpenClaw 会在已配置时使用 `acp.defaultAgent`。`mode: "session"` 需要 `thread: true`，以保持持久的绑定对话。
    </Note>
...
<ParamField path="cwd" type="string">
  请求的运行时工作目录（由后端/运行时策略验证）。如果省略，ACP spawn 会在已配置时继承目标智能体工作区；缺失的继承路径会回退到后端默认值，而真实访问错误会被返回。
</ParamField>
...
<ParamField path="streamTo" type='"parent"'>
  `"parent"` 会把初始 ACP 运行进度摘要作为系统事件流式传回请求方会话。接受的响应包括 `streamLogPath`，它指向会话范围的 JSONL 日志（`<sessionId>.acp-stream.jsonl`），你可以跟踪它以查看完整中继历史。默认情况下，父级进度流会显示助手注释和 ACP 状态进度，除非设置了 `streaming.progress.commentary=false`。当未配置流模式时，Discord 还会默认将父级预览设为进度模式。状态进度仍遵循 `acp.stream.tagVisibility`，因此 `plan` 等标签会保持隐藏，除非显式启用。
</ParamField>

ACP `sessions_spawn` 运行使用 `agents.defaults.subagents.runTimeoutSeconds` 作为其默认子轮次限制。该工具不接受每次调用的超时覆盖。

<ParamField path="model" type="string">
  ACP 子会话的显式模型覆盖。Codex ACP spawn 会在 `session/new` 之前将 `openai/gpt-5.4` 等 OpenAI ref 规范化为 Codex ACP 启动配置；`openai/gpt-5.4/high` 等斜杠形式也会设置 Codex ACP 推理强度。
  省略时，`sessions_spawn({ runtime: "acp" })` 会在已配置时使用现有子智能体模型默认值（`agents.defaults.subagents.model` 或 `agents.list[].subagents.model`）；否则会让 ACP harness 使用自身默认模型。
  其他 harness 必须声明 ACP `models` 并支持 `session/set_model`；否则 OpenClaw/acpx 会清晰失败，而不是静默回退到目标智能体默认值。
</ParamField>
<ParamField path="thinking" type="string">
  显式 thinking/推理强度。对于 Codex ACP，`minimal` 映射到低强度，`low`/`medium`/`high`/`xhigh` 直接映射，`off` 会省略推理强度启动覆盖。
  省略时，ACP spawn 会对所选模型使用现有子智能体 thinking 默认值，以及按模型配置的 `agents.defaults.models["provider/model"].params.thinking`。
</ParamField>
...
<AccordionGroup>
  <Accordion title="Interactive ACP sessions">
    ...
    绑定对话中的后续消息会直接路由到 ACP 会话，并且 ACP 输出会送回同一个渠道/线程/话题。

    OpenClaw 发送给 harness 的内容：

    - 普通绑定后续消息会作为提示文本发送，只有在 harness/后端支持附件时才会附带附件。
    - `/acp` 管理命令和本地 Gateway 网关命令会在 ACP 分发之前被拦截。
    - 运行时生成的完成事件会按目标具体化。OpenClaw 智能体会收到 OpenClaw 内部运行时上下文信封；外部 ACP harness 会收到包含子结果和指令的纯提示。原始 `<<<BEGIN_OPENCLAW_INTERNAL_CONTEXT>>>` 信封绝不应发送给外部 harness，也不应作为 ACP 用户转录文本持久化。
    - ACP 转录条目使用用户可见的触发文本或纯完成提示。内部事件元数据会尽可能在 OpenClaw 中保持结构化，并且不会被视为用户编写的聊天内容。

  </Accordion>
  <Accordion title="Parent-owned one-shot ACP sessions">
    由另一个智能体运行 spawn 的一次性 ACP 会话是后台子级，类似于子智能体：

    - 父级使用 `sessions_spawn({ runtime: "acp", mode: "run" })` 请求工作。
    - 子级在自己的 ACP harness 会话中运行。
    - 子轮次运行在原生子智能体 spawn 所用的同一后台通道上，因此缓慢的 ACP harness 不会阻塞无关的主会话工作。
    - 完成报告会通过任务完成宣布路径回传。OpenClaw 会先把内部完成元数据转换成纯 ACP 提示，再发送给外部 harness，因此 harness 不会看到仅 OpenClaw 使用的运行时上下文标记。
    - 当需要面向用户的回复时，父级会用正常助手语气重写子结果。

    **不要**将此路径视为父级和子级之间的点对点聊天。子级已经有返回父级的完成通道。

  </Accordion>
  <Accordion title="sessions_send and A2A delivery">
    `sessions_send` 可以在 spawn 之后定位另一个会话。对于普通对等会话，OpenClaw 会在注入消息后使用智能体到智能体（A2A）后续路径：
    ...
    该 A2A 路径是对等发送的回退方案，用于发送方需要可见后续回复的场景。当无关会话可以看到 ACP 目标并向其发送消息时，它会保持启用，例如在宽松的 `tools.sessions.visibility` 设置下。

    OpenClaw 仅在请求者是其自己由父级拥有的一次性 ACP 子级的父级时，才会跳过 A2A 后续处理。在这种情况下，在任务完成之上运行 A2A 可能会用子级结果唤醒父级，将父级回复转发回子级，并形成父/子回声循环。对于这种拥有的子级情况，`sessions_send` 结果会报告 `delivery.status="skipped"`，因为完成路径已经负责交付结果。

  </Accordion>
  <Accordion title="恢复现有会话">
    使用 `resumeSessionId` 继续之前的 ACP 会话，而不是重新开始。智能体会通过 `session/load` 重放其对话历史，因此可以带着之前的完整上下文继续。
    ...
    - `resumeSessionId` 是主机本地的 ACP/harness 恢复 ID，不是 OpenClaw 渠道会话键；OpenClaw 在分派前仍会检查 ACP 生成策略和目标智能体策略，而 ACP 后端或 harness 负责加载该上游 ID 的授权。
  ...
  <Accordion title="部署后冒烟测试">
    Gateway 网关部署后，运行一次实时端到端检查，而不是只相信单元测试：

    1. 验证目标主机上已部署的 Gateway 网关版本和提交。
    2. 打开一个通向实时智能体的临时 ACPX bridge 会话。
    3. 要求该智能体调用 `sessions_spawn`，使用 `runtime: "acp"`、`agentId: "codex"`、`mode: "run"`，任务为 `Reply with exactly LIVE-ACP-SPAWN-OK`。
    4. 验证 `accepted=yes`、真实的 `childSessionKey`，且没有验证器错误。
    5. 清理临时 bridge 会话。

    将门禁保持在 `mode: "run"`，并跳过 `streamTo: "parent"` -
    绑定线程的 `mode: "session"` 和流中继路径是单独、更丰富的集成检查。
...
- 外部 harness 可根据自身 CLI 权限和所选 `cwd` 进行读写。
...
- 如果请求者会话是沙箱隔离的，则 ACP 生成会同时阻止 `sessions_spawn({ runtime: "acp" })` 和 `/acp spawn`。
- 使用 `runtime: "acp"` 的 `sessions_spawn` 不支持 `sandbox: "require"`。

## 会话目标解析

大多数 `/acp` 操作都接受可选会话目标（`session-key`、`session-id` 或 `session-label`）。
...
`set`、`cwd`、`permissions`、`timeout`、`model` 和 `reset-options`）需要来自外部渠道的
所有者身份，以及来自内部 Gateway 网关客户端的 `operator.admin`。获得授权的非所有者发送者仍可使用 `sessions`、`doctor`、
`install` 和 `help`。

`/acp status` 会显示有效运行时选项，以及运行时级和后端级会话标识符。当后端缺少某项能力时，不支持的控制错误会清晰显示。`/acp sessions` 会读取当前已绑定或请求者会话的存储；目标令牌（`session-key`、`session-id` 或 `session-label`）会通过 Gateway 网关会话发现来解析，包括自定义的按智能体 `session.store` 根目录。
...
| 命令                         | 映射到                               | 说明                                                                                                                                                                                                 |
| ---------------------------- | ------------------------------------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `/acp model <id>`            | 运行时配置键 `model`                 | 对于 Codex ACP，OpenClaw 会将 `openai/<model>` 规范化为适配器模型 ID，并将类似 `openai/gpt-5.4/high` 的斜杠 reasoning 后缀映射到 `reasoning_effort`。                                                |
...
关于 acpx harness 配置（Claude Code / Codex / Gemini CLI 别名）、plugin-tools 和 OpenClaw-tools MCP bridges，以及 ACP 权限模式，请参阅
...
| `ACP dispatch is disabled by policy (acp.dispatch.enabled=false)` | 已禁用来自普通线程消息的自动调度。 | 设置 `acp.dispatch.enabled=true` 以恢复自动线程路由；显式 `sessions_spawn({ runtime: "acp" })` 调用仍然可用。 |
| `ACP agent "<id>" is not allowed by policy` | 智能体不在允许列表中。 | 使用允许的 `agentId`，或更新 `acp.allowedAgents`。 |
| `/acp doctor` 在启动后立即报告后端未就绪 | 后端插件缺失、已禁用、被允许/拒绝策略阻止，或其配置的可执行文件不可用。 | 安装/启用后端插件，重新运行 `/acp doctor`；如果仍然不健康，请检查后端安装或策略错误。 |
| 找不到 harness 命令 | 适配器 CLI 未安装、外部插件缺失，或非 Codex 适配器的首次运行 `npx` 拉取失败。 | 运行 `/acp doctor`，在 Gateway 网关主机上安装/预热适配器，或显式配置 acpx 智能体命令。 |
| harness 返回模型未找到 | 模型 ID 对另一个提供商/harness 有效，但不适用于这个 ACP 目标。 | 使用该 harness 列出的模型，在 harness 中配置模型，或省略覆盖项。 |
| harness 返回供应商凭证错误 | OpenClaw 健康，但目标 CLI/提供商未登录。 | 在 Gateway 网关主机环境中登录或提供所需的提供商密钥。 |
| `Unable to resolve session target: ...` | 键/ID/标签令牌错误。 | 运行 `/acp sessions`，复制准确的键/标签，然后重试。 |
| `--bind here requires running /acp spawn inside an active ... conversation` | 在没有活跃可绑定对话的情况下使用了 `--bind here`。 | 移动到目标聊天/渠道后重试，或使用未绑定的生成。 |
| `Conversation bindings are unavailable for <channel>.` | 适配器缺少当前对话 ACP 绑定能力。 | 在支持时使用 `/acp spawn ... --thread ...`，配置顶层 `bindings[]`，或移动到支持的渠道。 |
| `--thread here requires running /acp spawn inside an active ... thread` | 在线程上下文之外使用了 `--thread here`。 | 移动到目标线程，或使用 `--thread auto`/`off`。 |
| `Only <user-id> can rebind this channel/conversation/thread.` | 另一个用户拥有活跃绑定目标。 | 以所有者身份重新绑定，或使用不同的对话或线程。 |
...
| `Cannot apply --model ... did not advertise model support` | 目标 harness 未公开通用 ACP 模型切换。 | 使用声明支持 ACP `models`/`session/set_model` 的 harness，使用 Codex ACP 模型引用，或如果该 harness 有自己的启动标志，则直接在 harness 中配置模型。 |
...
| ACP 会话过早失败且输出很少 | 权限提示被 `permissionMode`/`nonInteractivePermissions` 阻止。 | 检查 Gateway 网关日志中的 `AcpRuntimeError`。对于完整权限，设置 `permissionMode=approve-all`；对于优雅降级，设置 `nonInteractivePermissions=deny`。 |
| ACP 会话在完成工作后无限期停滞 | harness 进程已结束，但 ACP 会话未报告完成。 | 更新 OpenClaw；当前 acpx 清理会在关闭和 Gateway 网关启动时回收 OpenClaw 所拥有的过期包装器和适配器进程。 |
| harness 看到 `<<<BEGIN_OPENCLAW_INTERNAL_CONTEXT>>>` | 内部事件信封泄漏到了 ACP 边界之外。 | 更新 OpenClaw 并重新运行完成流程；外部 harness 应只接收普通完成提示。 |

<Note>
`Command blocked by PreToolUse hook: Native hook relay unavailable` 属于原生 Codex hook 中继，
而不是 ACP/acpx。在绑定的 Codex 聊天中，使用 `/new` 或 `/reset` 启动新
会话；如果它成功一次后在下一次原生工具调用时再次返回，请重启 Codex app-server
或 OpenClaw Gateway 网关，而不是重复 `/new`。参见 [Codex harness 故障排除](/zh-CN/plugins/codex-harness#troubleshooting)。
</Note>
