...
title: 斜杠命令
x-i18n:
    generated_at: "2026-07-11T21:02:33Z"
    model: gpt-5.6
...
当对话绑定到 ACP 会话时，普通文本会路由至 ACP
harness。Gateway 网关管理命令仍在本地处理：`/acp ...` 始终由
OpenClaw 命令处理器接收；只要相应界面启用了命令处理，
`/status` 和 `/unfocus` 也会留在本地处理。

## 三种命令类型

<CardGroup cols={3}>
  <Card title="Commands" icon="terminal">
    由 Gateway 网关处理的独立 `/...` 消息。它必须是消息中的
    唯一内容。
  </Card>
  <Card title="Directives" icon="sliders">
    `/think`、`/fast`、`/verbose`、`/trace`、`/reasoning`、`/elevated`、
    `/exec`、`/model`、`/queue`——在模型看到消息前从中移除。单独发送时
    会持久保存会话设置；与其他文本一起发送时则作为内联提示。
  </Card>
...
<AccordionGroup>
  <Accordion title="Directive behavior details">
    - 指令会在模型看到消息前从中移除。
    - 在**仅含指令**的消息中（消息只包含指令），这些指令会
      持久保存到会话，并返回确认回复。
    - 在包含其他文本的**普通聊天**消息中，这些指令作为内联提示，
      **不会**持久保存会话设置。
    - 指令仅适用于**已授权的发送者**。如果设置了 `commands.allowFrom`，
      它将是唯一使用的允许列表；否则，授权来自渠道允许列表/配对以及
      `commands.useAccessGroups`。对于未授权的发送者，指令会被视为纯文本。
  </Accordion>
</AccordionGroup>
...
<ParamField path="commands.native" type='boolean | "auto"' default='"auto"'>
  注册原生命令。自动模式：Discord/Telegram 启用；Slack 禁用；
  对不支持原生命令的提供商忽略此设置。可通过
  `channels.<provider>.commands.native` 按渠道覆盖。在 Discord 上，`false`
  会跳过斜杠命令注册；此前注册的命令可能会一直显示，直至被移除。
</ParamField>

<ParamField path="commands.nativeSkills" type='boolean | "auto"' default='"auto"'>
  在支持时以原生方式注册 Skills 命令。自动模式：
  Discord/Telegram 启用；Slack 禁用。可通过
  `channels.<provider>.commands.nativeSkills` 覆盖。
</ParamField>

<ParamField path="commands.bash" type="boolean" default="false">
  启用 `! <cmd>` 以运行主机 shell 命令（别名为 `/bash <cmd>`）。需要配置
  `tools.elevated` 允许列表。
</ParamField>

<ParamField path="commands.bashForegroundMs" type="number" default="2000">
  bash 在切换至后台模式前的等待时长（`0` 表示
  立即转入后台）。
</ParamField>
...
<ParamField path="commands.ownerAllowFrom" type="string[]">
  面向仅限所有者命令界面的显式所有者允许列表。它与
  `commands.allowFrom` 和私信配对访问权限相互独立。
</ParamField>

<ParamField path="channels.<channel>.commands.enforceOwnerForCommands" type="boolean" default="false">
  按渠道设置：要求仅限所有者的命令使用所有者身份。当设为 `true` 时，
  发送者必须匹配 `commands.ownerAllowFrom`，或拥有内部 `operator.admin`
  权限范围。`allowFrom` 中的通配符条目**不足以**满足要求。
</ParamField>
...
<ParamField path="commands.allowFrom" type="object">
  ...
  **唯一**授权来源。使用 `"*"` 设置全局默认值；提供商特定的键会覆盖该值。
</ParamField>

<ParamField path="commands.useAccessGroups" type="boolean" default="true">
  未设置 `commands.allowFrom` 时，对命令强制执行允许列表和策略。
</ParamField>
...
- **插件命令：**插件中的 `registerCommand()` 调用

可用性取决于配置标志、渠道界面以及已安装并启用的
插件。
...
### 核心命令
  ...
  <Accordion title="会话和运行">
    ...
    | `/reset [soft [message]]` | 原位重置当前会话。`soft` 会保留对话记录，丢弃复用的 CLI 后端会话 ID，并重新运行启动流程 |
    ...
    <Note>
      Control UI 会拦截输入的 `/new`，创建并切换到新的
      仪表板会话；但如果配置了 `session.dmScope: "main"`，
      且当前父会话是智能体的主会话，则 `/new`
      会原位重置主会话。输入 `/reset` 仍会执行 Gateway 网关的
      原位重置。若要清除会话中固定的
      模型选择，请使用 `/model default`。
    </Note>
  ...
  <Accordion title="模型和运行控制">
    ...
    | `/models [provider] [page] [limit=<n>\|all]` | 列出已配置或可通过身份验证使用的提供商或模型 |
    | `/queue <mode>` | 管理活动运行的队列行为。请参阅[队列](/zh-CN/concepts/queue)和[队列引导](/zh-CN/concepts/queue-steering) |
    | `/steer <message>` | 向活动运行注入指导。别名：`/tell`。请参阅 [Steer](/zh-CN/tools/steer) |

    <AccordionGroup>
      <Accordion title="verbose / trace / fast / reasoning 安全性">
        - `/verbose` 用于调试——正常使用时请保持**关闭**。
        - `/trace` 只会显示插件自身的跟踪/调试行；常规详细信息仍保持关闭。
        - `/fast auto|on|off` 会持久保存会话覆盖设置；使用会话 UI 中的 `inherit` 选项将其清除。
        - `/fast` 因提供商而异：OpenAI/Codex 会将其映射为 `service_tier=priority`；直接向 Anthropic 发出的请求会将其映射为 `service_tier=auto` 或 `standard_only`。
        - `/reasoning`、`/verbose` 和 `/trace` 在群组环境中存在风险——它们可能会泄露内部推理过程或插件诊断信息。在群聊中请将其保持关闭。

      </Accordion>
      <Accordion title="模型切换详情">
        - `/model` 会立即将新模型持久化到会话中。
        - 如果智能体处于空闲状态，下一次运行会立即使用该模型。
        - 如果运行正在进行，切换会被标记为待处理，并在下一个可安全重试的节点应用。
  ...
  <Accordion title="设备发现和状态">
    ...
    | `/status plugins` | 显示详细的插件健康状况：加载错误、隔离状态、渠道插件故障、依赖问题和兼容性通知。需要启用 `commands.plugins: true` |
    | `/goal [status\|start\|edit\|pause\|resume\|complete\|block\|clear] ...` | 管理当前会话的持久化[目标](/zh-CN/tools/goal) |
    | `/diagnostics [note]` | 仅所有者可用的支持报告流程。每次都会请求 Exec 审批 |
    | `/crestodian <request>` | 从所有者私信中运行 Crestodian 设置和修复助手 |
    | `/tasks` | 列出当前会话中正在运行或最近运行的后台任务 |
    | `/context [list\|detail\|map\|json]` | 说明上下文的组装方式 |
    | `/whoami` | 显示你的发送者 ID。别名：`/id` |
    | `/usage off\|tokens\|full\|reset\|cost` | 控制每次响应的用量页脚（`reset`/`inherit`/`clear`/`default` 会清除会话覆盖设置，以重新继承已配置的默认值），或输出本地费用摘要 |
  </Accordion>
  ...
  <Accordion title="Skills、允许列表和审批">
    ...
    | `/learn [request]` | 通过 [Skill Workshop](/zh-CN/tools/skill-workshop)，根据当前对话或指定来源起草一个可供审查的 Skill |
    | `/allowlist [list\|add\|remove] ...` | 管理允许列表条目。仅支持文本 |
    | `/approve <id> <decision>` | 处理 Exec 或插件审批提示 |
    | `/btw <question>` | 在不更改会话上下文的情况下提出旁支问题。别名：`/side`。参见 [BTW](/zh-CN/tools/btw) |
  </Accordion>
  ...
  <Accordion title="子智能体和 ACP">
    ...
    | `/unfocus` | 移除当前线程绑定 |
    | `/agents` | 列出当前会话中已绑定线程的智能体 |
  </Accordion>

  <Accordion title="仅限所有者的写入和管理操作">
    | 命令 | 要求 | 说明 |
    | --- | --- | --- |
    ...
    | `/plugins list\|inspect\|show\|get\|install\|enable\|disable` | `commands.plugins: true` | 检查或更改插件状态。写入操作仅限所有者。别名：`/plugin` |
    | `/debug show\|set\|unset\|reset` | `commands.debug: true` | 仅作用于运行时的配置覆盖。仅限所有者 |
  ...
  <Accordion title="语音、TTS 和渠道控制">
    ...
    | `/bash <command>` | 运行宿主机 Shell 命令。别名：`! <command>`。需要 `commands.bash: true` |
...
| 命令                                                    | 说明                                                                                                                                                                                                       |
| ------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `/dreaming [on\|off\|status\|help]`                     | 开启或关闭记忆梦境功能（所有者或 Gateway 网关管理员）。参阅 [Dreaming](/zh-CN/concepts/dreaming)                                                                                                                 |
| `/pair [qr\|status\|pending\|approve\|cleanup\|notify]` | 管理设备配对。参阅[配对](/zh-CN/channels/pairing)                                                                                                                                                                |
| `/phone status\|arm ...\|disarm`                        | 临时启用高风险节点命令（摄像头/屏幕/计算机/写入）。参阅[计算机使用](/zh-CN/nodes/computer-use)                                                                                                                    |
| `/voice status\|list\|set <voiceId>`                    | 管理 Talk 模式语音配置。Discord 原生命令名称：`/talkvoice`                                                                                                                                                 |
| `/card ...`                                             | 发送 LINE 富卡片预设。参阅 [LINE](/zh-CN/channels/line)                                                                                                                                                          |
| `/codex <action> ...`                                   | 绑定、引导并检查 Codex 应用服务器 Codex harness（状态、线程、恢复、模型、快速模式、权限、压缩、审查、MCP、Skills 等）。参阅 [Codex harness](/zh-CN/plugins/codex-harness) |
...
- 名称会被清理为 `a-z0-9_`（最多 32 个字符）；发生冲突时会添加数字后缀。
...
<AccordionGroup>
  ...
  <Accordion title="原生命令参数">
    当省略必需参数时，Discord 会为动态选项使用自动补全和按钮菜单。
    Telegram 和 Slack 会为包含选项的命令显示按钮菜单。动态选项会根据
    目标会话模型解析，因此 `/think` 级别等模型特定选项会遵循会话的
    `/model` 覆盖设置。
  </Accordion>
</AccordionGroup>
...
`/tools` 回答的是一个运行时问题：**在此对话中，这个智能体目前可以使用什么**
，而不是静态配置目录。
...
结果限定于当前会话。更改智能体、渠道、线程、发送者授权或模型
都可能改变输出。如需编辑配置档案和覆盖设置，请使用 Control UI 的 Tools 面板或配置界面。
...
/model status      # 显示包含端点和 API 模式的详细视图
```

在 Discord 上，`/model` 和 `/models` 会打开一个包含提供商和
模型下拉列表的交互式选择器。该选择器遵循 `agents.defaults.models`，
包括 `provider/*` 条目。
...
`/mcp show` 会遮盖包含凭据的字段、已识别的凭据标志值
和已知具有密钥格式的参数。在群组中运行时，
配置会通过私信发送给所有者；如果没有可用的所有者私信路由，
命令将以安全关闭方式失败，并要求所有者在私聊中重试。
...
<Note>
  写入操作仅限所有者。默认禁用——使用 `commands.plugins: true` 启用。
</Note>
...
插件运行时，以用于新的智能体轮次。由于插件源模块发生了变化，
`/plugins install` 会自动重启托管的 Gateway 网关。
...
`/trace` 无需启用完整详细模式，即可显示限定于当前会话的插件跟踪/调试行。
它不能替代 `/debug`（运行时覆盖设置）或 `/verbose`（常规工具输出）。

## `/btw`：附带问题

`/btw` 用于快速询问与当前会话上下文相关的附带问题。别名：`/side`。
...
- 使用当前会话作为背景上下文。
- 在 Codex harness 会话中，作为临时 Codex 附属线程运行。
- **不会**更改后续会话上下文。
- 不会写入对话记录历史。

有关完整行为，请参阅 [BTW 附带问题](/zh-CN/tools/btw)。
...
<AccordionGroup>
  <Accordion title="各界面的会话范围">
    ...
    - **`/login codex`** 仅通过私聊或 Web UI 响应路径发送设备配对码。从 Telegram 群组/话题调用时，会要求所有者改为私信机器人。
    - **`/stop`** 以活动聊天会话为目标，中止当前运行。

  </Accordion>
  <Accordion title="Slack 特有说明">
    `channels.slack.slashCommand` 支持单个 `/openclaw` 风格的命令。
    启用 `commands.native: true` 后，为每个内置命令创建一个 Slack 斜杠命令。
    请注册 `/agentstatus`（而不是 `/status`），因为 Slack 保留了
    `/status`。文本命令 `/status` 在 Slack 消息中仍然有效。
  </Accordion>
  <Accordion title="快速路径和内联快捷方式">
    - 来自允许列表中发送者的纯命令消息会被立即处理（绕过队列和模型）。
    - 内联快捷方式（`/help`、`/commands`、`/status`、`/whoami`）也可以嵌入普通消息中使用，并会在模型看到剩余文本之前被移除。
    - 未经授权的纯命令消息会被静默忽略；内联 `/...` 标记会被视为纯文本。

  </Accordion>
  <Accordion title="参数说明">
    - 命令和参数之间可使用可选的 `:`（`/think: high`、`/send: on`）。
...
- **提供商用量/配额**（例如“Claude 剩余 80%”）会在启用用量跟踪时，显示在当前模型提供商的 `/status` 中。
- 当实时会话快照信息不完整时，`/status` 中的**令牌/缓存行**可以回退到最新的对话记录用量条目。
- **执行与运行时：**`/status` 使用 `Execution` 报告实际生效的沙箱路径，并使用 `Runtime` 报告会话的运行方：`OpenClaw Default`、`OpenAI Codex`、CLI 后端或 ACP 后端。
- **每次响应的令牌数/费用：**由 `/usage off|tokens|full` 控制。
- `/model status` 提供模型/身份验证/端点信息，不提供用量信息。
