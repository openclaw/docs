...
title: 斜杠命令
x-i18n:
    generated_at: "2026-06-30T13:48:47Z"
    model: gpt-5.5
...
Gateway 网关处理作为独立消息发送、并以 `/` 开头的命令。
仅主机 bash 命令使用 `! <cmd>`（`/bash <cmd>` 作为别名）。

当对话绑定到 ACP 会话时，普通文本会路由到 ACP
harness。Gateway 网关管理命令保持本地处理：`/acp ...` 始终会到达
...
<CardGroup cols={3}>
  ...
  <Card title="Directives" icon="sliders">
    ...
    `/exec`、`/model`、`/queue` — 在模型看到消息之前从消息中移除。
    单独发送时会持久化会话设置；与其他文本一起发送时会作为行内提示。
  </Card>
  <Card title="Inline shortcuts" icon="bolt">
    `/help`、`/commands`、`/status`、`/whoami` — 立即运行，并会在模型看到剩余文本之前
    被移除。仅限授权发送者。
  </Card>
</CardGroup>
...
<AccordionGroup>
  <Accordion title="Directive behavior details">
    - 指令会在模型看到消息之前从消息中移除。
    - 在**仅指令**消息中（消息只包含指令），它们会
      持久化到会话，并回复确认。
    - 在包含其他文本的**普通聊天**消息中，它们会作为行内提示，并且
      **不会**持久化会话设置。
    - 指令仅适用于**授权发送者**。如果设置了 `commands.allowFrom`，
      它就是唯一使用的允许列表；否则授权来自
      频道允许列表/配对以及 `commands.useAccessGroups`。未授权
      发送者的指令会被当作纯文本处理。
  </Accordion>
</AccordionGroup>
...
<ParamField path="commands.native" type='boolean | "auto"' default='"auto"'>
  注册原生命令。自动：对 Discord/Telegram 开启；对 Slack 关闭；
  对不支持原生能力的提供商忽略。可用
  `channels.<provider>.commands.native` 按频道覆盖。在 Discord 上，`false` 会跳过斜杠命令
  注册；之前注册的命令可能会保持可见，直到被移除。
</ParamField>

<ParamField path="commands.nativeSkills" type='boolean | "auto"' default='"auto"'>
  在支持时以原生方式注册技能命令。自动：对
...
<ParamField path="commands.ownerAllowFrom" type="string[]">
  ...
  `commands.allowFrom` 和私信配对访问权限。
</ParamField>

<ParamField path="channels.<channel>.commands.enforceOwnerForCommands" type="boolean" default="false">
  按频道配置：仅所有者命令需要所有者身份。当为 `true` 时，
  发送者必须匹配 `commands.ownerAllowFrom`，或持有内部 `operator.admin`
  scope。通配符 `allowFrom` 条目**不足以**满足要求。
</ParamField>
...
<AccordionGroup>
  <Accordion title="Sessions and runs">
    ...
    | `/reset [soft [message]]` | 就地重置当前会话。`soft` 会保留转录，丢弃复用的 CLI 后端会话 id，并重新运行启动流程 |
    ...
    | `/session max-age <duration\|off>` | 管理线程绑定的最大年龄过期 |
    | `/export-session [path]` | 将当前会话导出为 HTML。别名：`/export` |
    | `/export-trajectory [path]` | 为当前会话导出 JSONL 轨迹包。别名：`/trajectory` |

    <Note>
      Control UI 会拦截输入的 `/new`，以创建并切换到一个新的
      dashboard 会话，但当配置了 `session.dmScope: "main"` 且
      当前父级是智能体的主会话时除外 — 在这种情况下，`/new`
      会就地重置主会话。输入的 `/reset` 仍会运行 Gateway 网关的
      就地重置。当你想清除固定的
  ...
  <Accordion title="Model and run controls">
    ...
    | `/models [provider] [page] [limit=<n>\|all]` | 列出已配置/凭证可用的提供商或模型 |
    | `/queue <mode>` | 管理活动运行队列行为。参见 [Queue](/zh-CN/concepts/queue) 和 [Queue steering](/zh-CN/concepts/queue-steering) |
    | `/steer <message>` | 向活动运行注入指导。别名：`/tell`。参见 [Steer](/zh-CN/tools/steer) |

    <AccordionGroup>
      <Accordion title="verbose / trace / fast / reasoning safety">
        - `/verbose` 用于调试 — 正常使用时保持**关闭**。
        - `/trace` 只会暴露插件拥有的 trace/debug 行；普通详细闲聊保持关闭。
        ...
        - `/reasoning`、`/verbose` 和 `/trace` 在群组场景中有风险 — 它们可能会暴露内部推理或插件诊断。请在群聊中保持关闭。

      </Accordion>
      <Accordion title="Model switching details">
        - `/model` 会立即将新模型持久化到会话。
        - 如果智能体处于空闲状态，下一次运行会立即使用它。
        - 如果有运行正在活动中，切换会被标记为待处理，并在下一个干净的重试点应用。
  ...
  <Accordion title="Discovery and status">
    ...
    | `/status` | 显示执行/运行时状态、Gateway 网关和系统 uptime、插件健康状况，以及提供商使用量/配额 |
    | `/status plugins` | 显示详细的插件健康状况：加载错误、隔离、频道失败、依赖问题、兼容性通知 |
    ...
    | `/usage off\|tokens\|full\|reset\|cost` | 控制每条响应的用量页脚（`reset`/`inherit`/`clear`/`default` 会清除会话覆盖，以重新继承已配置的默认值），或打印本地成本摘要 |
  </Accordion>
  ...
  <Accordion title="Skills, allowlists, approvals">
    ...
    | `/btw <question>` | 在不改变会话上下文的情况下询问一个旁支问题。别名：`/side`。参见 [BTW](/zh-CN/tools/btw) |
  </Accordion>
...
Dock 命令需要 `session.identityLinks`。源发送者和目标对等方
必须位于同一个身份组中。
...
- 名称会清理为 `a-z0-9_`（最长 32 个字符）；冲突会添加数字后缀。
...
<AccordionGroup>
  <Accordion title="Skill 命令分发">
    ...
    （确定性，不涉及模型）。示例：`/prose`（OpenProse 插件）
    — 参见 [OpenProse](/zh-CN/prose)。

  </Accordion>
  <Accordion title="原生命令参数">
    Discord 会在省略必需参数时，为动态选项和按钮菜单使用自动补全。
    Telegram 和 Slack 会为带有选项的命令显示按钮菜单。动态选项会针对目标会话模型解析，因此像 `/think` 级别这样的模型
    专用选项会跟随会话的 `/model` 覆盖。
  </Accordion>
</AccordionGroup>
...
结果按会话作用域限定。更改智能体、渠道、线程、发送者
授权或模型都可能改变输出。如需编辑配置文件和覆盖，
...
在 Discord 上，`/model` 和 `/models` 会打开一个带有提供商和
...
`/trace` 会显示按会话作用域限定的插件跟踪/调试行，而无需完整详细
...
完整行为请参见 [BTW 旁支问题](/zh-CN/tools/btw)。

## 表面说明

<AccordionGroup>
  <Accordion title="每个表面的会话作用域">
  ...
  <Accordion title="快速路径和内联快捷方式">
    - 来自允许列表发送者的仅命令消息会立即处理（绕过队列 + 模型）。
    - 内联快捷方式（`/help`、`/commands`、`/status`、`/whoami`）也可以嵌入普通消息中，并会在模型看到剩余文本前被剥离。
...
- 启用用量跟踪时，当前模型提供商的**提供商用量/配额**（例如 “Claude 剩余 80%”）会显示在 `/status` 中。
- 当实时会话快照较稀疏时，`/status` 中的**令牌/缓存行**可以回退到最新的转录用量条目。
