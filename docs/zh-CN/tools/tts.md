...
当前内置的 Microsoft 语音提供商通过 `node-edge-tts` 库，使用 Microsoft Edge 的在线神经网络 TTS 服务。它是托管服务（不是本地服务），使用 Microsoft 端点，并且不需要 API 密钥。  
`node-edge-tts` 提供语音配置选项和输出格式，但并非所有选项都受该服务支持。使用 `edge` 的旧版配置和指令输入仍然可用，并会规范化为 `microsoft`。

由于此路径是一个未公布 SLA 或配额的公共网络服务，请将其视为尽力而为。如果你需要有保障的限额和支持，请使用 OpenAI 或 ElevenLabs。

## 可选键

如果你想使用 Azure Speech、ElevenLabs、Google Gemini、Gradium、Inworld、MiniMax、OpenAI、Volcengine、Vydra、xAI 或 Xiaomi MiMo：

- `AZURE_SPEECH_KEY` 加 `AZURE_SPEECH_REGION`（也接受  
  `AZURE_SPEECH_API_KEY`、`SPEECH_KEY` 和 `SPEECH_REGION`）
- `ELEVENLABS_API_KEY`（或 `XI_API_KEY`）
- `GEMINI_API_KEY`（或 `GOOGLE_API_KEY`）
- `GRADIUM_API_KEY`
- `INWORLD_API_KEY`
- `MINIMAX_API_KEY`；MiniMax TTS 也接受通过以下方式进行 Token Plan 认证：  
...
如果配置了多个提供商，将优先使用选定的提供商，其他提供商作为回退选项。  
自动摘要使用已配置的 `summaryModel`（或 `agents.defaults.model.primary`），因此如果你启用了摘要，对应提供商也必须已完成认证。
...
不是。自动 TTS 默认**关闭**。请在配置中使用  
`messages.tts.auto` 启用，或在本地使用 `/tts on` 启用。

当未设置 `messages.tts.provider` 时，OpenClaw 会按注册表自动选择顺序，选择第一个已配置的语音提供商。
...
当某个智能体需要使用不同的提供商、语音、模型、风格或自动 TTS 模式时，请使用 `agents.list[].tts`。智能体块会在 `messages.tts` 之上执行深度合并，因此提供商凭证可以保留在全局提供商配置中。
...
`AZURE_SPEECH_KEY` -> `AZURE_SPEECH_API_KEY` -> `SPEECH_KEY`，区域的解析顺序为  
`messages.tts.providers.azure-speech.region` -> `AZURE_SPEECH_REGION` ->  
`SPEECH_REGION`。新配置应使用 `azure-speech`；`azure` 可作为提供商别名使用。
...
MiniMax TTS 的认证解析顺序为 `messages.tts.providers.minimax.apiKey`，然后是已存储的 `minimax-portal` OAuth/token 配置文件，再然后是 Token Plan 环境变量键（`MINIMAX_OAUTH_TOKEN`、`MINIMAX_CODE_PLAN_KEY`、`MINIMAX_CODING_API_KEY`），最后是 `MINIMAX_API_KEY`。当未显式设置 TTS `baseUrl` 时，OpenClaw 可以复用已配置的 `minimax-portal` OAuth 主机用于 Token Plan 语音。
...
Google Gemini TTS 使用 Gemini API 密钥路径。这里可使用限制为 Gemini API 的 Google Cloud Console API 密钥，其类型与内置 Google 图像生成提供商所使用的密钥相同。解析顺序为  
...
`apiKey` 的值必须是从 Inworld 仪表板（Workspace > API Keys）中逐字复制的 Base64 编码凭证字符串。提供商会将其作为 `Authorization: Basic <apiKey>` 发送，不会进行任何额外编码，因此不要传入原始 bearer token，也不要自行进行 Base64 编码。该密钥可回退到 `INWORLD_API_KEY` 环境变量。完整设置请参见 [Inworld provider](/zh-CN/providers/inworld)。
...
Volcengine TTS 使用的是来自 Speech Console 的 BytePlus Seed Speech API 密钥，而不是 Doubao 模型提供商所使用的 OpenAI 兼容 `VOLCANO_ENGINE_API_KEY`。解析顺序为 `messages.tts.providers.volcengine.apiKey` ->  
`VOLCENGINE_TTS_API_KEY` -> `BYTEPLUS_SEED_SPEECH_API_KEY`。旧版 AppID/token 认证仍可通过 `messages.tts.providers.volcengine.appId` / `token` 或  
...
xAI TTS 使用与内置 Grok 模型提供商相同的 `XAI_API_KEY` 路径。解析顺序为 `messages.tts.providers.xai.apiKey` -> `XAI_API_KEY`。当前可用的语音有 `ara`、`eve`、`leo`、`rex`、`sal` 和 `una`；默认值为 `eve`。`language` 接受 BCP-47 标签或 `auto`。
...
Xiaomi MiMo TTS 使用与内置 Xiaomi 模型提供商相同的 `XIAOMI_API_KEY` 路径。语音提供商 id 是 `xiaomi`；`mimo` 也可作为别名使用。目标文本会作为 assistant message 发送，以匹配 Xiaomi 的 TTS 契约。可选的 `style` 会作为用户指令发送，不会被朗读。
...
Local CLI TTS 会在 Gateway 网关主机上运行已配置的命令。`{{Text}}`、`{{OutputPath}}`、`{{OutputDir}}` 和 `{{OutputBase}}` 占位符会在 `args` 中展开；如果不存在 `{{Text}}` 占位符，OpenClaw 会将要朗读的文本写入 stdin。`outputFormat` 接受 `mp3`、`opus` 或 `wav`。语音便笺目标会被转码为 Ogg/Opus，电话输出会使用 `ffmpeg` 转码为原始 16 kHz 单声道 PCM。旧版提供商别名 `cli` 仍然可用，但新配置应使用 `tts-local-cli`。
...
- `auto`：自动 TTS 模式（`off`、`always`、`inbound`、`tagged`）。
  - `inbound` 仅会在收到入站语音消息后发送音频。
  - `tagged` 仅会在回复中包含 `[[tts:key=value]]` 指令或 `[[tts:text]]...[[/tts:text]]` 块时发送音频。
...
- `providers.<id>`：由提供商拥有的设置，按语音提供商 id 作为键。
...
- `providers.azure-speech.endpoint` / `providers.azure-speech.baseUrl`：可选的 Azure Speech endpoint/base URL 覆盖值。
- `providers.azure-speech.voice`：Azure 语音 ShortName（默认值为 `en-US-JennyNeural`）。
- `providers.azure-speech.lang`：SSML 语言代码（默认值为 `en-US`）。
- `providers.azure-speech.outputFormat`：用于标准音频输出的 Azure `X-Microsoft-OutputFormat`（默认值为 `audio-24khz-48kbitrate-mono-mp3`）。
- `providers.azure-speech.voiceNoteOutputFormat`：用于语音便笺输出的 Azure `X-Microsoft-OutputFormat`（默认值为 `ogg-24khz-16bit-mono-opus`）。
...
- `providers.openai.baseUrl`：覆盖 OpenAI TTS endpoint。
  ...
  - 非默认值会被视为 OpenAI 兼容 TTS endpoint，因此接受自定义模型名和语音名。
...
- `providers.minimax.baseUrl`：覆盖 MiniMax API base URL（默认值为 `https://api.minimax.io`，环境变量：`MINIMAX_API_HOST`）。
- `providers.minimax.model`：TTS 模型（默认值为 `speech-2.8-hd`，环境变量：`MINIMAX_TTS_MODEL`）。
- `providers.minimax.voiceId`：语音标识符（默认值为 `English_expressive_narrator`，环境变量：`MINIMAX_TTS_VOICE_ID`）。
- `providers.minimax.speed`：播放速度 `0.5..2.0`（默认值为 1.0）。
- `providers.minimax.vol`：音量 `(0, 10]`（默认值为 1.0；必须大于 0）。
- `providers.minimax.pitch`：整数音高偏移 `-12..12`（默认值为 0）。调用 MiniMax T2A 前会截断小数值，因为 API 会拒绝非整数音高值。
...
- `providers.tts-local-cli.timeoutMs`：命令超时时间，单位为毫秒（默认值为 `120000`）。
- `providers.tts-local-cli.cwd`：可选的命令工作目录。
- `providers.tts-local-cli.env`：命令可选的字符串环境变量覆盖值。
- `providers.inworld.baseUrl`：覆盖 Inworld API base URL（默认值为 `https://api.inworld.ai`）。
- `providers.inworld.voiceId`：Inworld 语音标识符（默认值为 `Sarah`）。
- `providers.inworld.modelId`：Inworld TTS 模型（默认值为 `inworld-tts-1.5-max`；也支持 `inworld-tts-1.5-mini`、`inworld-tts-1-max`、`inworld-tts-1`）。
- `providers.inworld.temperature`：采样温度 `0..2`（可选）。
- `providers.google.model`：Gemini TTS 模型（默认值为 `gemini-3.1-flash-tts-preview`）。
- `providers.google.voiceName`：Gemini 预置语音名称（默认值为 `Kore`；也接受 `voice`）。
- `providers.google.audioProfile`：在朗读文本前追加的自然语言风格提示。
- `providers.google.speakerName`：当你的 TTS 提示使用具名说话人时，附加在朗读文本前的可选说话人标签。
...
- `providers.gradium.baseUrl`：覆盖 Gradium API base URL（默认值为 `https://api.gradium.ai`）。
- `providers.gradium.voiceId`：Gradium 语音标识符（默认值为 Emma，`YTpq7expH9539ERJ`）。
- `providers.volcengine.apiKey`：BytePlus Seed Speech API 密钥（环境变量：`VOLCENGINE_TTS_API_KEY` 或 `BYTEPLUS_SEED_SPEECH_API_KEY`）。
- `providers.volcengine.resourceId`：BytePlus Seed Speech resource id（默认值为 `seed-tts-1.0`，环境变量：`VOLCENGINE_TTS_RESOURCE_ID`；当你的 BytePlus 项目具有 TTS 2.0 权限时，请使用 `seed-tts-2.0`）。
- `providers.volcengine.appKey`：BytePlus Seed Speech app key header（默认值为 `aGjiRDfUWi`，环境变量：`VOLCENGINE_TTS_APP_KEY`）。
...
- `providers.volcengine.cluster`：旧版 Volcengine TTS 集群（默认值为 `volcano_tts`，环境变量：`VOLCENGINE_TTS_CLUSTER`）。
- `providers.volcengine.voice`：语音类型（默认值为 `en_female_anna_mars_bigtts`，环境变量：`VOLCENGINE_TTS_VOICE`）。
...
- `providers.xai.baseUrl`：覆盖 xAI TTS base URL（默认值为 `https://api.x.ai/v1`，环境变量：`XAI_BASE_URL`）。
- `providers.xai.voiceId`：xAI 语音 id（默认值为 `eve`；当前可用语音：`ara`、`eve`、`leo`、`rex`、`sal`、`una`）。
- `providers.xai.language`：BCP-47 语言代码或 `auto`（默认值为 `en`）。
- `providers.xai.responseFormat`：`mp3`、`wav`、`pcm`、`mulaw` 或 `alaw`（默认值为 `mp3`）。
- `providers.xai.speed`：提供商原生速度覆盖值。
- `providers.xiaomi.apiKey`：Xiaomi MiMo API 密钥（环境变量：`XIAOMI_API_KEY`）。
- `providers.xiaomi.baseUrl`：覆盖 Xiaomi MiMo API base URL（默认值为 `https://api.xiaomimimo.com/v1`，环境变量：`XIAOMI_BASE_URL`）。
- `providers.xiaomi.model`：TTS 模型（默认值为 `mimo-v2.5-tts`，环境变量：`XIAOMI_TTS_MODEL`；也支持 `mimo-v2-tts`）。
- `providers.xiaomi.voice`：MiMo 语音 id（默认值为 `mimo_default`，环境变量：`XIAOMI_TTS_VOICE`）。
- `providers.xiaomi.format`：`mp3` 或 `wav`（默认值为 `mp3`，环境变量：`XIAOMI_TTS_FORMAT`）。
...
- `providers.openrouter.baseUrl`：覆盖 OpenRouter TTS base URL（默认值为 `https://openrouter.ai/api/v1`；旧版 `https://openrouter.ai/v1` 会被规范化）。
- `providers.openrouter.model`：OpenRouter TTS 模型 id（默认值为 `hexgrad/kokoro-82m`；也接受 `modelId`）。
- `providers.openrouter.voice`：提供商专用语音 id（默认值为 `af_alloy`；也接受 `voiceId`）。
- `providers.openrouter.responseFormat`：`mp3` 或 `pcm`（默认值为 `mp3`）。
- `providers.openrouter.speed`：提供商原生速度覆盖值。
- `providers.microsoft.enabled`：允许使用 Microsoft 语音（默认值为 `true`；无需 API 密钥）。
...
- `providers.microsoft.outputFormat`：Microsoft 输出格式（例如 `audio-24khz-48kbitrate-mono-mp3`）。
  - 有效值请参见 Microsoft Speech 输出格式；并非所有格式都受内置的 Edge 支持传输层支持。
...
启用后，模型可以发出 `[[tts:...]]` 指令，为单条回复覆盖语音设置；还可以附带可选的 `[[tts:text]]...[[/tts:text]]` 块，用于提供仅应出现在音频中的表现性标签（笑声、歌唱提示等）。
...
[[tts:text]](laughs) 再把这首歌读一遍。[[/tts:text]]
```
...
这些设置会覆盖该主机上由 `messages.tts` 加当前激活的  
`agents.list[].tts` 块共同生成的最终配置。

## 输出格式（固定）

- **Feishu / Matrix / Telegram / WhatsApp**：语音便笺回复优先使用 Opus（ElevenLabs 使用 `opus_48000_64`，OpenAI 使用 `opus`）。
  - 48 kHz / 64 kbps 是语音消息较好的折中方案。
- **Feishu**：当语音便笺回复生成的是 MP3/WAV/M4A 或其他可能的音频文件时，Feishu 插件会在发送原生 `audio` 气泡前，使用 `ffmpeg` 将其转码为 48 kHz Ogg/Opus。如果转换失败，Feishu 会接收原始文件作为附件。
...
- **Xiaomi MiMo**：默认使用 MP3，或在配置时使用 WAV。对于 Feishu 和 Telegram 等语音便笺目标，OpenClaw 会在投递前使用 `ffmpeg` 将 Xiaomi 输出转码为 48 kHz Opus。
- **Local CLI**：使用已配置的 `outputFormat`。语音便笺目标会被转换为 Ogg/Opus，电话输出会使用 `ffmpeg` 转换为原始 16 kHz 单声道 PCM。
- **Google Gemini**：Gemini API TTS 返回原始 24 kHz PCM。OpenClaw 会将其封装为 WAV 用于音频附件，将其转码为 48 kHz Opus 用于语音便笺目标，并直接为 Talk/电话返回 PCM。
...
- **xAI**：默认使用 MP3；`responseFormat` 可为 `mp3`、`wav`、`pcm`、`mulaw` 或 `alaw`。OpenClaw 使用 xAI 的批处理 REST TTS endpoint，并返回完整音频附件；此提供商路径不使用 xAI 的流式 TTS WebSocket。此路径不支持原生 Opus 语音便笺格式。
- **Microsoft**：使用 `microsoft.outputFormat`（默认值为 `audio-24khz-48kbitrate-mono-mp3`）。
  - 内置传输层接受 `outputFormat`，但并非服务提供所有格式。
...
回复 -> 是否启用 TTS？
  ...
  是  -> 是否有媒体 / MEDIA: / 过短？
          ...
          否  -> 长度 > 限制？
                   ...
                   是  -> 是否启用摘要？
                            否  -> 发送文本
                            是  -> 摘要化（summaryModel 或 agents.defaults.model.primary）
                                      -> TTS -> 附加音频
```
...
Discord 说明：`/tts` 是 Discord 的内置命令，因此 OpenClaw 会在该平台注册 `/voice` 作为原生命令。文本形式的 `/tts ...` 仍然可用。
...
`tts` 工具会将文本转换为语音，并返回一个音频附件用于回复投递。当渠道是 Feishu、Matrix、Telegram 或 WhatsApp 时，音频会作为语音消息而不是文件附件投递。  
当 `ffmpeg` 可用时，Feishu 可以在此路径上对非 Opus 的 TTS 输出进行转码。  
