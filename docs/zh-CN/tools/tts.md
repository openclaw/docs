...
OpenClaw 可以使用 ElevenLabs、Google Gemini、Gradium、Local CLI、Microsoft、MiniMax、OpenAI、Vydra、xAI 或 Xiaomi MiMo 将发送出去的回复转换为音频。
...
内置的 Microsoft 语音提供商当前通过 `node-edge-tts` 库使用 Microsoft Edge 的在线神经网络 TTS 服务。它是一个托管服务（不是本地服务），使用 Microsoft 的端点，并且不需要 API 密钥。
`node-edge-tts` 提供语音配置选项和输出格式，但并非所有选项都受该服务支持。使用 `edge` 的旧版配置和指令输入仍然可用，并会规范化为 `microsoft`。

由于这一路径是没有公开 SLA 或配额说明的公共 Web 服务，请将其视为尽力而为的服务。如果你需要有保证的限制和支持，请使用 OpenAI 或 ElevenLabs。
...
自动摘要使用已配置的 `summaryModel`（或 `agents.defaults.model.primary`），因此如果你启用摘要，那个提供商也必须完成身份验证。
...
不是。自动 TTS 默认**关闭**。可通过配置中的 `messages.tts.auto` 或本地使用 `/tts on` 启用。

当 `messages.tts.provider` 未设置时，OpenClaw 会按照注册表自动选择顺序，选取第一个已配置的语音提供商。
...
MiniMax TTS 认证解析顺序为 `messages.tts.providers.minimax.apiKey`，然后是已存储的 `minimax-portal` OAuth/token 配置文件，再然后是 Token Plan 环境变量键（`MINIMAX_OAUTH_TOKEN`、`MINIMAX_CODE_PLAN_KEY`、`MINIMAX_CODING_API_KEY`），最后是 `MINIMAX_API_KEY`。当未显式设置 TTS `baseUrl` 时，OpenClaw 可以复用已配置的 `minimax-portal` OAuth 主机用于 Token Plan 语音。
...
Google Gemini TTS 使用 Gemini API 密钥路径。这里可使用限制为 Gemini API 的 Google Cloud Console API 密钥，其样式与内置 Google 图像生成提供商使用的密钥相同。解析顺序为 `messages.tts.providers.google.apiKey` -> `models.providers.google.apiKey` -> `GEMINI_API_KEY` -> `GOOGLE_API_KEY`。
...
xAI TTS 使用与内置 Grok 模型提供商相同的 `XAI_API_KEY` 路径。解析顺序为 `messages.tts.providers.xai.apiKey` -> `XAI_API_KEY`。当前可用的语音为 `ara`、`eve`、`leo`、`rex`、`sal` 和 `una`；默认值为 `eve`。`language` 接受 BCP-47 标签或 `auto`。
...
Xiaomi MiMo TTS 使用与内置 Xiaomi 模型提供商相同的 `XIAOMI_API_KEY` 路径。语音提供商 id 是 `xiaomi`；`mimo` 可作为别名。目标文本会作为 assistant 消息发送，符合 Xiaomi 的 TTS 协议。可选的 `style` 会作为用户指令发送，不会被朗读。
...
Local CLI TTS 会在 Gateway 网关主机上运行已配置的命令。`{{Text}}`、`{{OutputPath}}`、`{{OutputDir}}` 和 `{{OutputBase}}` 占位符会在 `args` 中展开；如果不存在 `{{Text}}` 占位符，OpenClaw 会将要朗读的文本写入 stdin。`outputFormat` 接受 `mp3`、`opus` 或 `wav`。
...
- `summaryModel`：用于自动摘要的可选低成本模型；默认是 `agents.defaults.model.primary`。
...
- 旧版直接提供商块（`messages.tts.openai`、`messages.tts.elevenlabs`、`messages.tts.microsoft`、`messages.tts.edge`）可通过 `openclaw doctor --fix` 修复；已提交的配置应使用 `messages.tts.providers.<id>`。
...
- `apiKey` 值会回退到环境变量（`ELEVENLABS_API_KEY`/`XI_API_KEY`、`GEMINI_API_KEY`/`GOOGLE_API_KEY`、`GRADIUM_API_KEY`、`MINIMAX_API_KEY`、`OPENAI_API_KEY`、`VYDRA_API_KEY`、`XAI_API_KEY`、`XIAOMI_API_KEY`）。
...
- `providers.minimax.pitch`：整数音高偏移 `-12..12`（默认 0）。在调用 MiniMax T2A 前，小数值会被截断，因为 API 不接受非整数的音高值。
...
当 `messages.tts.auto` 为 `tagged` 时，必须有这些指令才能触发音频。

启用后，模型可以发出 `[[tts:...]]` 指令来覆盖单条回复的语音设置，还可以附带可选的 `[[tts:text]]...[[/tts:text]]` 块，以提供只应出现在音频中的表现性标签（笑声、唱歌提示等）。
...
- **Feishu / Matrix / Telegram / WhatsApp**：语音便笺回复优先使用 Opus（ElevenLabs 使用 `opus_48000_64`，OpenAI 使用 `opus`）。
  - 48 kHz / 64 kbps 是语音消息中较好的平衡点。
- **Feishu**：当语音便笺回复生成为 MP3/WAV/M4A 或其他可能的音频文件时，Feishu 插件会在发送原生 `audio` 气泡前，使用 `ffmpeg` 将其转码为 48 kHz Ogg/Opus。如果转换失败，Feishu 将收到原始文件作为附件。
- **其他渠道**：MP3（ElevenLabs 使用 `mp3_44100_128`，OpenAI 使用 `mp3`）。
  - 44.1 kHz / 128 kbps 是语音清晰度的默认平衡设置。
- **MiniMax**：普通音频附件使用 MP3（`speech-2.8-hd` 模型，32 kHz 采样率）。对于 Feishu 和 Telegram 等语音便笺目标，OpenClaw 会在发送前使用 `ffmpeg` 将 MiniMax 的 MP3 转码为 48 kHz Opus。
...
- 跳过过短的回复（少于 10 个字符）。
- 启用时，使用 `agents.defaults.model.primary`（或 `summaryModel`）对较长回复生成摘要。
- 将生成的音频附加到回复中。

如果回复超过 `maxLength`，并且摘要已关闭（或摘要模型没有 API 密钥），则会跳过音频并发送普通文本回复。
...
- 命令需要授权发送者（仍适用 allowlist/owner 规则）。
...
- OpenAI 和 ElevenLabs API 失败现在会包含解析后的提供商错误详情和请求 id（当提供商返回时），这些信息会显示在 TTS 错误/日志中。

## 智能体工具

`tts` 工具会将文本转换为语音，并返回用于回复投递的音频附件。当渠道为 Feishu、Matrix、Telegram 或 WhatsApp 时，音频会作为语音消息而不是文件附件发送。
当 `ffmpeg` 可用时，Feishu 在此路径上可以对非 Opus 的 TTS 输出进行转码。
WhatsApp 会将可见文本与 PTT 语音便笺音频分开发送，因为客户端对语音便笺上的字幕显示并不一致。
它接受可选的 `channel` 和 `timeoutMs` 字段；`timeoutMs` 是每次调用的提供商请求超时时间（毫秒）。
