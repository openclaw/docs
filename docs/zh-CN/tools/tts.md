...
1. 本地 `/tts persona <id>` 偏好设置，如果已设置。
...
<AccordionGroup>
  ...
  <Accordion title="Other providers">
    只使用 `personas.<id>.providers.<provider>` 下的提供商专用 persona 绑定。除非提供商实现自己的 persona 提示映射，否则会忽略 persona 提示字段。
  </Accordion>
</AccordionGroup>
...
- `/tts persona <id>` 会写入本地 persona 偏好设置；`/tts persona off` 会清除它。
...
<AccordionGroup>
  <Accordion title="Top-level messages.tts.*">
    ...
    <ParamField path="providers.<id>" type="object">
      由提供商拥有、按语音提供商 ID 键控的设置。旧版直接块（`messages.tts.openai`、`.elevenlabs`、`.microsoft`、`.edge`）会由 `openclaw doctor --fix` 重写；只提交 `messages.tts.providers.<id>`。
    </ParamField>
  ...
  <Accordion title="Azure Speech">
    ...
    <ParamField path="voiceNoteOutputFormat" type="string">语音便签输出的 Azure `X-Microsoft-OutputFormat`。默认 `ogg-24khz-16bit-mono-opus`。</ParamField>
  </Accordion>
  ...
  <Accordion title="Google Gemini">
    ...
    <ParamField path="personaPrompt" type="string">附加到模板导演注释中的 Google 专用额外角色提示文本。</ParamField>
  ...
  <Accordion title="OpenAI">
    ...
    <ParamField path="instructions" type="string">显式 OpenAI `instructions` 字段。设置后，persona prompt 字段**不会**自动映射。</ParamField>
    <ParamField path="extraBody / extra_body" type="Record<string, unknown>">额外 JSON 字段，会在生成的 OpenAI TTS 字段之后合并进 `/audio/speech` 请求体。可用于 OpenAI 兼容端点，例如 Kokoro 需要 `lang` 之类提供商特定键；不安全的原型键会被忽略。</ParamField>
  ...
  <Accordion title="Volcengine（BytePlus Seed Speech）">
    ...
    <ParamField path="resourceId" type="string">默认值 `seed-tts-1.0`。环境变量：`VOLCENGINE_TTS_RESOURCE_ID`。当你的项目有 TTS 2.0 权益时使用 `seed-tts-2.0`。</ParamField>
    <ParamField path="appKey" type="string">App key 标头。默认值 `aGjiRDfUWi`。环境变量：`VOLCENGINE_TTS_APP_KEY`。</ParamField>
  ...
  <Accordion title="xAI">
    ...
    <ParamField path="voiceId" type="string">默认值 `eve`。实时语音：`ara`、`eve`、`leo`、`rex`、`sal`、`una`。</ParamField>
  ...
  <Accordion title="Xiaomi MiMo">
    ...
    <ParamField path="model" type="string">默认值 `mimo-v2.5-tts`。环境变量：`XIAOMI_TTS_MODEL`。也支持 `mimo-v2-tts`。</ParamField>
