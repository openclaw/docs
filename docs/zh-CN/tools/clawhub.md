...
title: ClawHub
x-i18n:
    generated_at: "2026-05-02T17:02:37Z"
    model: gpt-5.5
    provider: openai
...
- 使用单独的 `clawhub` CLI 处理注册表身份验证、发布、删除/取消删除和同步工作流。
...
<Tabs>
  <Tab title="Skills">
    ...
    持久保存来源元数据，这样之后的 `update` 调用可以继续使用 ClawHub。
  ...
  <Tab title="插件">
    ...
    `plugins search` 会查询 ClawHub 插件目录，并输出可直接安装的
    软件包名称。不带前缀且符合 npm 安全命名的插件规格也会先尝试在 ClawHub
    中解析，然后再尝试 npm：
    ...
    如果你只想使用 npm 解析而不进行 ClawHub 查询，请使用 `npm:<package>`：
    ...
    `minGatewayVersion` 兼容性，因此不兼容的主机会尽早失败并关闭流程，而不是部分安装
    该软件包。当某个软件包版本发布 ClawPack 工件时，
    OpenClaw 会优先使用上传的精确 npm-pack `.tgz`，验证 ClawHub 摘要标头和
    下载的字节，并记录 ClawPack 摘要元数据以供之后
...
系列。如果某个 ClawHub 软件包实际上是一个 skill，OpenClaw 会停止并
改为指向 `openclaw skills install <slug>`。

匿名 ClawHub 插件安装也会对私有软件包失败并关闭流程。
社区或其他非官方渠道仍可安装，但 OpenClaw
会发出警告，以便操作员在启用前审查来源和验证结果。
</Note>
...
能力。注册表会跟踪使用信号（star、下载量），以
提升排序和可见性。每次发布都会创建一个新的 semver
版本，并且注册表会保留版本历史，便于用户审计
变更。
...
| 所有者恢复仪表板         | 发布者可以在 `/dashboard` 查看因扫描而暂缓的自有内容。              |
...
<AccordionGroup>
  <Accordion title="安全扫描">
    ...
    release 运行自动化安全检查。公开详情页会汇总当前结果，扫描器
    行会链接到 VirusTotal、ClawScan 和静态
    分析的专用详情页。

    因扫描被暂缓或阻止的 release 可能不会出现在公共目录和
...
<AccordionGroup>
  <Accordion title="身份验证（登录 / 注销 / whoami）">
    ```bash
  ...
  <Accordion title="安装 / 更新 / 列出">
    ...
    - `--version <version>` — 安装或更新到特定版本（`update` 上仅支持单个 slug）。
  ...
  <Accordion title="请求重新扫描">
    ...
    发布的 skill 版本或插件 release 为目标。在非交互式运行中，请传入
    `--yes`。

    JSON 响应包含目标类型、名称、版本、重新扫描状态，以及
    该版本或 release 的剩余/最大请求次数。
...
已发布的包应附带**构建后的 JavaScript**，并让 `runtimeExtensions` 指向该输出。Git 检出安装在没有构建文件时仍可回退到 TypeScript 源码，但构建后的运行时入口可避免在启动、Doctor 和插件加载路径中进行运行时 TypeScript 编译。
...
<AccordionGroup>
  ...
  <Accordion title="同步扫描和回退根目录">
    `clawhub sync` 会先扫描你的当前工作目录。如果未找到 Skills，它会回退到已知的旧位置（例如 `~/openclaw/skills` 和 `~/.openclaw/skills`）。这用于在没有额外标志的情况下找到较旧的 Skill 安装。
  </Accordion>
