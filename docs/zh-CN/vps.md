...
title: Linux 服务器
x-i18n:
    generated_at: "2026-06-27T03:36:51Z"
    model: gpt-5.5
...
在任何 Linux 服务器或云 VPS 上运行 OpenClaw Gateway 网关。本页帮助你选择提供商，说明云部署的工作方式，并覆盖适用于各处的通用 Linux 调优。
...
<CardGroup cols={2}>
  ...
  <Card title="Oracle Cloud" href="/zh-CN/install/oracle">始终免费的 ARM 层</Card>
...
- 安全默认值：让 Gateway 网关保持在 loopback，并通过 SSH 隧道或 Tailscale Serve 访问。
...
在公共 VPS 上安装 OpenClaw 之前，先决定你想如何管理这台主机本身。

- 如果你想要仅限 Tailnet 的管理员访问，请先安装 Tailscale，将 VPS 加入你的 tailnet，通过 Tailscale IP 或 MagicDNS 名称验证第二个 SSH 会话，然后限制公共 SSH。
- 如果你不使用 Tailscale，请在暴露更多服务之前，为你的 SSH 路径应用等效的加固。
- 这与 Gateway 网关访问是分开的。你仍然可以让 OpenClaw 绑定到 loopback，并为仪表板使用 SSH 隧道或 Tailscale Serve。

Tailscale 专用的 Gateway 网关选项位于 [Tailscale](/zh-CN/gateway/tailscale)。

## VPS 上的共享公司智能体

当每个用户都处于同一个信任边界内，并且智能体仅用于业务时，为团队运行单个智能体是一种有效设置。

- 将它放在专用运行时上（VPS/VM/容器 + 专用 OS 用户/账户）。
- 不要让该运行时登录个人 Apple/Google 账户或个人浏览器/密码管理器配置文件。
- 如果用户之间存在对抗关系，请按 Gateway 网关/主机/OS 用户拆分。

安全模型详情：[安全](/zh-CN/gateway/security)。

## 将节点与 VPS 一起使用

你可以将 Gateway 网关保留在云端，并在你的本地设备（Mac/iOS/Android/无头设备）上配对**节点**。节点提供本地屏幕/摄像头/canvas 和 `system.run` 能力，而 Gateway 网关仍留在云端。
...
- `OPENCLAW_NO_RESPAWN=1` 让常规 Gateway 网关重启保持在进程内，这可以避免额外的进程交接，并在小型主机上保持 PID 跟踪简单。
- 首次命令运行会预热缓存；后续运行会更快。
- 关于 Raspberry Pi 的具体事项，请参阅 [Raspberry Pi](/zh-CN/install/raspberry-pi)。
...
- 保持重启行为显式：
  ...
  - `RestartSec=2`
  - `TimeoutStartSec=90`
- 对状态/缓存路径优先使用 SSD 支持的磁盘，以降低随机 I/O 冷启动成本。
...
如果你有意安装了系统单元，请改为通过 `sudo systemctl edit openclaw-gateway.service` 编辑
`openclaw-gateway.service`。

`Restart=` 策略如何帮助自动恢复：
[systemd 可以自动化服务恢复](https://www.redhat.com/en/blog/systemd-automate-recovery)。

关于 Linux OOM 行为、子进程受害者选择和 `exit 137`
