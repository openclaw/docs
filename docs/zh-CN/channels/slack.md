...
<Tabs>
  ...
  <Tab title="提及和渠道用户">
    ...
    每个渠道的控制项（`channels.slack.channels.<id>`；仅能通过启动解析名称，或使用 `dangerouslyAllowNameMatching`）：
...
斜杠会话使用隔离键，例如 `agent:<agentId>:slack:slash:<userId>`，但仍会通过 `CommandTargetSessionKey` 将命令执行路由到目标会话。
