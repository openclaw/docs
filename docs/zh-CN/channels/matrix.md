...
title: Matrix
x-i18n:
    generated_at: "2026-05-11T20:21:13Z"
    model: gpt-5.5
    provider: openai
...
3. 重启 Gateway 网关。
4. 与机器人开启私信，或邀请它加入房间（请参阅[自动加入](#auto-join) - 只有 `autoJoin` 允许时，新邀请才会落地）。
...
如果匹配的 `MATRIX_*` 环境变量已经存在，并且所选账号没有保存的认证信息，向导会提供一个环境变量快捷方式。若要在保存允许列表前解析房间名称，请运行 `openclaw channels resolve --channel matrix "Project Room"`。启用 E2EE 时，向导会写入配置，并运行与 [`openclaw matrix encryption setup`](#encryption-and-verification) 相同的引导流程。
...
`autoJoinAllowlist` 只接受稳定目标：`!roomId:server`、`#alias:server` 或 `*`。纯房间名称会被拒绝；别名条目会根据 homeserver 解析，而不是根据受邀房间声称的状态解析。
</Warning>
