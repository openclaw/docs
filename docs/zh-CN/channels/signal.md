...
read_when:
    - 设置 Signal 支持
    - 调试 Signal 发送/接收
summary: 通过 `signal-cli`（JSON-RPC + SSE）支持 Signal、设置路径以及号码模型
title: Signal
x-i18n:
    generated_at: "2026-04-24T03:37:17Z"
    model: gpt-5.4
    provider: openai
...
- 通过 `signal-cli` 提供 Signal 渠道（不是内嵌的 libsignal）。
...
- 如果你在**个人 Signal 账户**上运行机器人，它会忽略你自己发送的消息（循环保护）。
- 如果你想实现“我给机器人发消息，它会回复我”，请使用一个**单独的机器人号码**。
...
多账户支持：使用 `channels.signal.accounts` 配置每个账户的独立配置和可选的 `name`。共享模式请参见 [`gateway/configuration`](/zh-CN/gateway/config-channels#multi-account-all-channels)。

## 设置路径 B：注册专用机器人号码（短信，Linux）

如果你想使用一个专用机器人号码，而不是链接现有的 Signal 应用账户，请使用此方式。

1. 获取一个可以接收短信的号码（或者用于座机的语音验证）。
...
3. 尽可能使用与浏览器会话相同的外部 IP 运行命令。
4. 立即再次运行注册命令（验证码令牌过期很快）：
...
5. 配对你的私信发送方：
   ...
   - 在你的手机上将机器人号码保存为联系人，以避免显示 “Unknown contact”。

重要提示：使用 `signal-cli` 注册某个电话号码账户，可能会使该号码的主 Signal 应用会话失去认证。更推荐使用专用机器人号码；如果你需要保留现有手机应用设置，请使用 QR 链接模式。
...
如果你想自己管理 `signal-cli`（例如 JVM 冷启动慢、容器初始化，或共享 CPU 的情况），可以单独运行守护进程，并让 OpenClaw 指向它：
...
这样会跳过 OpenClaw 内部的自动启动和启动等待。对于自动启动时的慢启动情况，可设置 `channels.signal.startupTimeoutMs`。
...
- 仅 UUID 的发送方（来自 `sourceUuid`）会以 `uuid:<id>` 的形式存储在 `channels.signal.allowFrom` 中。

群组：

- `channels.signal.groupPolicy = open | allowlist | disabled`。
- 当设置为 `allowlist` 时，`channels.signal.groupAllowFrom` 控制哪些对象可以在群组中触发。
...
- 运行时说明：如果完全缺少 `channels.signal`，运行时在群组检查中会回退为 `groupPolicy="allowlist"`（即使已设置 `channels.defaults.groupPolicy`）。
...
- 可选的按换行分块：设置 `channels.signal.chunkMode="newline"`，可在按长度分块前先按空行（段落边界）拆分。
- 支持附件（从 `signal-cli` 获取 base64）。
- 默认媒体上限：`channels.signal.mediaMaxMb`（默认 8）。
- 使用 `channels.signal.ignoreAttachments` 可跳过媒体下载。
- 群组历史上下文使用 `channels.signal.historyLimit`（或 `channels.signal.accounts.*.historyLimit`），回退到 `messages.groupChat.historyLimit`。设为 `0` 可禁用（默认 50）。
...
- 目标：发送方的 E.164 或 UUID（使用配对输出中的 `uuid:<id>`；裸 UUID 也可用）。
...
- 守护进程可达但没有回复：检查账户/守护进程设置（`httpUrl`、`account`）和接收模式。
- 私信被忽略：发送方正在等待配对批准。
- 群组消息被忽略：群组发送方/提及门控阻止了投递。
- 编辑后出现配置校验错误：运行 `openclaw doctor --fix`。
- 诊断中看不到 Signal：确认 `channels.signal.enabled: true`。
...
如需分诊流程，请参见：[/channels/troubleshooting](/zh-CN/channels/troubleshooting)。
...
- 短信验证仅在注册或恢复流程中需要，但如果失去对该号码/账户的控制，可能会使重新注册更加复杂。
...
- `channels.signal.httpUrl`：完整的守护进程 URL（会覆盖 host/port）。
- `channels.signal.httpHost`、`channels.signal.httpPort`：守护进程绑定地址（默认 `127.0.0.1:8080`）。
- `channels.signal.autoStart`：自动拉起守护进程（当未设置 `httpUrl` 时默认 true）。
- `channels.signal.startupTimeoutMs`：启动等待超时时间，单位毫秒（上限 120000）。
...
- `channels.signal.allowFrom`：私信允许列表（E.164 或 `uuid:<id>`）。`open` 需要 `"*"`。Signal 不支持用户名；请使用手机号/UUID 标识。
- `channels.signal.groupPolicy`：`open | allowlist | disabled`（默认：allowlist）。
- `channels.signal.groupAllowFrom`：群组发送方允许列表。
- `channels.signal.groups`：按 Signal 群组 id（或 `"*"`）设置的每群组覆盖项。支持的字段：`requireMention`、`tools`、`toolsBySender`。
- `channels.signal.accounts.<id>.groups`：用于多账户设置时，`channels.signal.groups` 的每账户版本。
- `channels.signal.historyLimit`：作为上下文包含的最大群组消息数（`0` 表示禁用）。
- `channels.signal.dmHistoryLimit`：私信历史限制，按用户轮次计。每用户覆盖项：`channels.signal.dms["<phone_or_uuid>"].historyLimit`。
- `channels.signal.textChunkLimit`：出站分块大小（字符数）。
- `channels.signal.chunkMode`：`length`（默认）或 `newline`，表示在按长度分块前，先按空行（段落边界）拆分。
