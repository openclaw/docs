...
x-i18n:
    generated_at: "2026-05-06T03:39:52Z"
    model: gpt-5.5
    provider: openai
...
Status：支持文本 + 私信附件；渠道/群组文件发送需要 `sharePointSiteId` + Graph 权限（参见[在群聊中发送文件](#sending-files-in-group-chats)）。投票通过 Adaptive Cards 发送。消息操作为以文件优先的发送显式暴露 `upload-file`。

## 内置插件

Microsoft Teams 在当前 OpenClaw 版本中作为内置插件随附，因此普通打包构建不需要单独安装。

如果你使用的是较旧构建，或使用排除了内置 Teams 的自定义安装，请直接安装 npm 包：

```bash
openclaw plugins install @openclaw/msteams
```

使用裸包名可以跟随当前官方发布标签。仅在需要可复现安装时固定精确版本。
...
[`@microsoft/teams.cli`](https://www.npmjs.com/package/@microsoft/teams.cli) 会在一条命令中处理机器人注册、清单创建和凭据生成。
...
输出会显示 `CLIENT_ID`、`CLIENT_SECRET`、`TENANT_ID` 和一个 **Teams App ID** - 记下这些值，供后续步骤使用。它还会提供直接在 Teams 中安装应用的选项。
...
对于生产部署，建议使用[联合身份验证](/zh-CN/channels/msteams#federated-authentication-certificate-plus-managed-identity)（证书或托管身份），而不是客户端密钥。

<Note>
默认情况下会阻止群聊（`channels.msteams.groupPolicy: "allowlist"`）。要允许群组回复，请设置 `channels.msteams.groupAllowFrom`，或使用 `groupPolicy: "open"` 允许任何成员（受提及门控）。
</Note>
...
- 不要依赖 UPN/显示名称匹配作为允许列表依据 - 它们可能会变化。OpenClaw 默认禁用直接名称匹配；若要启用，请显式设置 `channels.msteams.dangerouslyAllowNameMatching: true`。
...
- 设置 `groupPolicy: "open"` 允许任何成员（默认仍受提及门控）。
- 要允许**没有渠道**，请设置 `channels.msteams.groupPolicy: "disabled"`。
...
- 键应使用来自 Teams 链接的稳定 Teams 会话 ID，而不是可变显示名称。
- 当 `groupPolicy="allowlist"` 且存在 teams 允许列表时，仅接受列出的团队/渠道（受提及门控）。
...
- 添加 RSC 权限（参见[当前 Teams RSC 权限清单](#current-teams-rsc-permissions-manifest)）。
...
对于生产部署，OpenClaw 支持**联合身份验证**，作为比客户端密钥更安全的替代方案。可使用两种方法：
...
使用 Azure Managed Identity 进行无密码身份验证。这非常适合部署在 Azure 基础设施（AKS、App Service、Azure VM）上且托管身份可用的场景。
...
5. **确保可以网络访问** IMDS（`169.254.169.254`）：如果使用 NetworkPolicy，请添加一条出站规则，允许到 `169.254.169.254/32` 的 80 端口流量。
...
OpenClaw 为 Microsoft Teams 暴露一个由 Graph 支持的 `member-info` 操作，使智能体和自动化可以直接从 Microsoft Graph 解析渠道成员详细信息（显示名称、电子邮件、角色）。
...
这些是我们 Teams 应用清单中的**现有 resourceSpecific 权限**。它们只在安装该应用的团队/聊天内生效。
...
- `ChatMessage.Read.Chat`（应用）- 无需 @mention 即可接收所有群组聊天消息
...
更新后，请在每个团队中重新安装该应用以使新权限生效，并**完全退出并重新启动 Teams**（不只是关闭窗口）以清除缓存的应用元数据。
...
**结论：**RSC 用于实时监听；Graph API 用于历史访问。要在离线期间追赶遗漏的消息，你需要带 `ChannelMessage.Read.All` 的 Graph API（需要管理员同意）。
...
Teams 通过 HTTP webhook 递送消息。如果处理耗时过长（例如 LLM 响应较慢），你可能会看到：

- Gateway 网关超时
- Teams 重试消息（导致重复）
- 回复丢失

OpenClaw 会通过快速返回并主动发送回复来处理这一点，但非常慢的响应仍可能导致问题。

### 格式化

Teams markdown 比 Slack 或 Discord 更受限制：

- 基本格式可以正常使用：**粗体**、_斜体_、`code`、链接
- 复杂 Markdown（表格、嵌套列表）可能无法正确渲染
- Adaptive Cards 支持投票和语义化展示发送（见下文）
...
- `channels.msteams.mediaAllowHosts`：入站附件主机允许列表（默认包含 Microsoft/Teams 域）。
- `channels.msteams.mediaAuthAllowHosts`：在媒体重试时允许附加 Authorization 标头的主机允许列表（默认包含 Graph + Bot Framework 主机）。
- `channels.msteams.requireMention`：要求在频道/群组中 @提及（默认 true）。
...
- `toolsBySender` 键应使用显式前缀：
  `id:`、`e164:`、`username:`、`name:`（旧版无前缀键仍然仅映射到 `id:`）。
...
| **线程**（类似 Slack） | 消息以线性方式流动，更类似 Slack                   | `top-level`              |

**问题：** Teams API 不会暴露频道使用哪种 UI 样式。如果使用错误的 `replyStyle`：

- 在线程样式频道中使用 `thread` → 回复会以别扭的嵌套形式显示
- 在帖子样式频道中使用 `top-level` → 回复会显示为单独的顶层帖子，而不是线程内回复
...
- **频道/群组：** 附件存储在 M365 存储中（SharePoint/OneDrive）。Webhook 载荷只包含 HTML 存根，不包含实际文件字节。**需要 Graph API 权限** 才能下载频道附件。
- 对于显式的文件优先发送，请使用带有 `media` / `filePath` / `path` 的 `action=upload-file`；可选的 `message` 会成为随附文本/评论，`filename` 会覆盖上传名称。

如果没有 Graph 权限，包含图片的频道消息将以纯文本形式接收（机器人无法访问图片内容）。
默认情况下，OpenClaw 仅从 Microsoft/Teams 主机名下载媒体。可使用 `channels.msteams.mediaAllowHosts` 覆盖（使用 `["*"]` 允许任何主机）。
Authorization 标头只会附加到 `channels.msteams.mediaAuthAllowHosts` 中的主机（默认包含 Graph + Bot Framework 主机）。请保持此列表严格（避免多租户后缀）。

## 在群聊中发送文件

机器人可以使用 FileConsentCard 流程（内置）在私信中发送文件。但是，**在群聊/频道中发送文件** 需要额外设置：
...
机器人没有个人 OneDrive 驱动器（`/me/drive` Graph API 端点不适用于应用程序身份）。要在群聊/频道中发送文件，机器人会上传到一个 **SharePoint 站点** 并创建分享链接。
...
按用户分享更安全，因为只有聊天参与者可以访问该文件。如果缺少 `Chat.Read.All` 权限，机器人会回退到组织范围分享。
...
使用 `message` 工具或 CLI 向 Teams 用户或会话发送语义化展示载荷。OpenClaw 会根据通用展示契约将它们渲染为 Teams Adaptive Cards。
...
有关目标格式详情，请见下方的 [目标格式](#target-formats)。
...
- 团队键名 = `/team/` 后面的路径片段（URL 解码后，例如 `19:Bk4j...@thread.tacv2`；较旧的租户可能显示 `@thread.skype`，这同样有效）
- 渠道键名 = `/channel/` 后面的路径片段（URL 解码后）
- 对 OpenClaw 路由，请**忽略** `groupId` 查询参数。它是 Microsoft Entra 组 ID，不是传入 Teams 活动中使用的 Bot Framework 会话 ID。
...
- **图片未在渠道中显示：** Graph 权限或管理员同意缺失。重新安装 Teams 应用，并完全退出后重新打开 Teams。
- **渠道中没有响应：** 默认需要提及；设置 `channels.msteams.requireMention=false`，或按团队/渠道配置。
- **版本不匹配（Teams 仍显示旧清单）：** 移除并重新添加应用，然后完全退出 Teams 以刷新。
- **webhook 返回 401 Unauthorized：** 手动测试且没有 Azure JWT 时符合预期，表示端点可访问但认证失败。请使用 Azure Web Chat 正确测试。
