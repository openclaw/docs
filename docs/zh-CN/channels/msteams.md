...
summary: Microsoft Teams 机器人的支持状态、功能和配置
title: Microsoft Teams
x-i18n:
    generated_at: "2026-05-06T16:37:15Z"
    model: gpt-5.5
    provider: openai
...
Status：支持文本 + 私信附件；渠道/群组文件发送需要 `sharePointSiteId` + Graph 权限（请参阅[在群聊中发送文件](#sending-files-in-group-chats)）。投票通过 Adaptive Cards 发送。消息操作为优先发送文件的发送方式公开了显式的 `upload-file`。

## 内置插件

Microsoft Teams 在当前 OpenClaw 版本中作为内置插件提供，因此普通打包构建中不需要单独安装。

如果你使用的是较旧版本，或自定义安装排除了内置 Teams，请直接安装 npm 包：

```bash
openclaw plugins install @openclaw/msteams
```

使用裸包名可以跟随当前官方发布标签。只有在需要可复现安装时，才固定精确版本。
...
[`@microsoft/teams.cli`](https://www.npmjs.com/package/@microsoft/teams.cli) 通过一条命令处理 bot 注册、清单创建和凭证生成。
...
如果尚未安装并认证 devtunnel CLI，请先完成（[入门指南](https://learn.microsoft.com/en-us/azure/developer/dev-tunnels/get-started)）。
...
输出会显示 `CLIENT_ID`、`CLIENT_SECRET`、`TENANT_ID` 和一个 **Teams App ID** - 请记下这些值以供后续步骤使用。它还会提示你直接在 Teams 中安装应用。
...
`teams app create` 会提示你安装应用 - 选择 “Install in Teams”。如果你跳过了它，可以稍后获取链接：
...
这会对 bot 注册、AAD 应用配置、清单有效性和 SSO 设置运行诊断。

对于生产部署，请考虑使用[联合认证](/zh-CN/channels/msteams#federated-authentication-certificate-plus-managed-identity)（证书或托管身份）替代客户端密钥。

<Note>
群聊默认被阻止（`channels.msteams.groupPolicy: "allowlist"`）。要允许群组回复，请设置 `channels.msteams.groupAllowFrom`，或使用 `groupPolicy: "open"` 允许任何成员（受提及门控）。
</Note>

## 目标

- 通过 Teams 私信、群聊或渠道与 OpenClaw 交互。
- 保持路由确定性：回复始终返回到消息来源渠道。
...
- 键应使用 Teams 链接中的稳定 Teams 会话 ID，而不是可变显示名称。
...
- 启动时，OpenClaw 会将团队/渠道和用户允许列表名称解析为 ID（当 Graph 权限允许时）
  并记录映射；未解析的团队/渠道名称会按输入保留，但默认在路由时被忽略，除非启用 `channels.msteams.dangerouslyAllowNameMatching: true`。
...
<Warning>
创建新的多租户 bot 已在 2025-07-31 后弃用。新 bot 请使用 **Single Tenant**。
</Warning>
...
当插件可用且 `msteams` 配置存在并带有凭证时，Teams 渠道会自动启动。
...
对于生产部署，OpenClaw 支持**联合认证**，作为客户端密钥之外更安全的替代方案。可使用两种方法：
...
使用 Azure 托管身份进行无密码认证。这非常适合部署在可用托管身份的 Azure 基础设施（AKS、App Service、Azure VM）上。
...
5. **确保网络可以访问** IMDS（`169.254.169.254`）- 如果使用 NetworkPolicy，请添加一条出站规则，允许访问 `169.254.169.254/32` 的 80 端口。
...
Teams 无法访问 `localhost`。请使用持久开发隧道，让你的 URL 在不同会话之间保持不变：
...
OpenClaw 为 Microsoft Teams 暴露了一个基于 Graph 的 `member-info` 操作，让智能体和自动化流程可以直接从 Microsoft Graph 解析渠道成员详细信息（显示名称、电子邮件、角色）。
...
- `channels.msteams.historyLimit` 控制有多少条最近的渠道/群组消息会被包装进提示。
- 回退到 `messages.groupChat.historyLimit`。设置为 `0` 可禁用（默认 50）。
- 获取到的线程历史会按发送者允许列表（`allowFrom` / `groupAllowFrom`）过滤，因此线程上下文种子只包含来自允许发送者的消息。
- 引用附件上下文（从 Teams 回复 HTML 派生的 `ReplyTo*`）目前会按收到时原样传递。
...
这些是我们 Teams 应用清单中**现有的 resourceSpecific 权限**。它们只在安装了该应用的团队/聊天内生效。

**对于渠道（团队范围）：**

- `ChannelMessage.Read.Group` (Application) - 在没有 @mention 的情况下接收所有渠道消息
...
- `ChatMessage.Read.Chat` (Application) - 在没有 @mention 的情况下接收所有群聊消息
...
更新后，请在每个团队中重新安装该应用，使新权限生效，并**完全退出并重新启动 Teams**（不要只是关闭窗口）以清除缓存的应用元数据。
...
**结论：**RSC 用于实时监听；Graph API 用于历史访问。若要在离线后补收错过的消息，你需要具有 `ChannelMessage.Read.All` 的 Graph API（需要管理员同意）。

## 启用 Graph 的媒体 + 历史（渠道必需）

如果你需要**渠道**中的图片/文件，或想获取**消息历史**，则必须启用 Microsoft Graph 权限并授予管理员同意。
...
**用户提及的额外权限：**对话中的用户 @mentions 可开箱即用。不过，如果你想动态搜索并提及**不在当前对话中**的用户，请添加 `User.Read.All` (Application) 权限并授予管理员同意。
...
- `channels.msteams.mediaAuthAllowHosts`：媒体重试时附加 Authorization 标头的允许列表（默认使用 Graph + Bot Framework 主机）。
...
- `channels.msteams.actions.memberInfo`：启用或禁用由 Graph 支持的成员信息操作（默认：Graph 凭据可用时启用）。
...
Teams 最近在同一底层数据模型上引入了两种渠道界面样式：

| 样式                     | 说明                                      | 推荐的 `replyStyle` |
| ------------------------ | ----------------------------------------- | ------------------- |
| **帖子**（经典）         | 消息显示为卡片，下方带有话题串回复        | `thread`（默认）    |
| **话题串**（类似 Slack） | 消息线性流动，更像 Slack                  | `top-level`         |

**问题：** Teams API 不会公开渠道使用哪种界面样式。如果使用错误的 `replyStyle`：

- 在话题串样式渠道中使用 `thread` → 回复会以别扭的嵌套形式显示
- 在帖子样式渠道中使用 `top-level` → 回复会显示为单独的顶层帖子，而不是位于话题串中
...
- **渠道/群组：** 附件位于 M365 存储（SharePoint/OneDrive）中。Webhook 载荷只包含 HTML 存根，不包含实际文件字节。**需要 Graph API 权限**才能下载渠道附件。
- 对于显式文件优先发送，使用带有 `media` / `filePath` / `path` 的 `action=upload-file`；可选的 `message` 会成为随附文本/评论，`filename` 会覆盖上传名称。

如果没有 Graph 权限，包含图片的渠道消息将以纯文本形式接收（机器人无法访问图片内容）。
默认情况下，OpenClaw 只会从 Microsoft/Teams 主机名下载媒体。可用 `channels.msteams.mediaAllowHosts` 覆盖（使用 `["*"]` 允许任意主机）。
Authorization 标头只会附加到 `channels.msteams.mediaAuthAllowHosts` 中的主机（默认使用 Graph + Bot Framework 主机）。保持此列表严格（避免多租户后缀）。
...
机器人没有个人 OneDrive 驱动器（`/me/drive` Graph API 端点不适用于应用程序身份）。要在群聊/渠道中发送文件，机器人会上传到一个 **SharePoint 站点**并创建分享链接。
...
| 权限                                    | 分享行为                                      |
| --------------------------------------- | --------------------------------------------- |
| 仅 `Sites.ReadWrite.All`                | 组织范围分享链接（组织中任何人都可访问）      |
| `Sites.ReadWrite.All` + `Chat.Read.All` | 按用户分享链接（只有聊天成员可以访问）        |

按用户分享更安全，因为只有聊天参与者可以访问文件。如果缺少 `Chat.Read.All` 权限，机器人会回退到组织范围分享。
...
| 群聊 + 文件 + 无 `sharePointSiteId`              | 尝试 OneDrive 上传（可能失败），只发送文本       |
| 个人聊天 + 文件                                  | FileConsentCard 流程（无需 SharePoint 即可工作） |
| 任意上下文 + 图片                                | Base64 编码内联（无需 SharePoint 即可工作）      |

### 文件存储位置

上传的文件会存储在所配置 SharePoint 站点默认文档库的 `/OpenClawShared/` 文件夹中。
...
使用 `message` 工具或 CLI，将语义化演示载荷发送给 Teams 用户或对话。OpenClaw 会根据通用演示契约将其渲染为 Teams Adaptive Cards。
...
<Note>
没有 `user:` 前缀时，名称默认会按群组或团队解析。按显示名称定位人员时，请始终使用 `user:`。
</Note>

## 主动消息

- 只有在用户互动**之后**，才能发送主动消息，因为我们会在那时存储会话引用。
...
- 团队键 = `/team/` 后面的路径段（URL 解码后，例如 `19:Bk4j...@thread.tacv2`；旧租户可能显示 `@thread.skype`，这也是有效的）
- 渠道键 = `/channel/` 后面的路径段（URL 解码后）
- 对于 OpenClaw 路由，**忽略** `groupId` 查询参数。它是 Microsoft Entra 群组 ID，不是传入 Teams 活动中使用的 Bot Framework 会话 ID。
...
- **渠道中没有响应：** 默认要求提及；设置 `channels.msteams.requireMention=false`，或按团队/渠道配置。
- **版本不匹配（Teams 仍显示旧清单）：** 移除并重新添加应用，然后完全退出 Teams 以刷新。
- **webhook 返回 401 Unauthorized：** 手动测试时未携带 Azure JWT 是预期情况，表示端点可达但认证失败。请使用 Azure Web Chat 正确测试。
...
1. 验证 `webApplicationInfo.id` 与你的 bot App ID 完全匹配
2. 重新上传应用，并在团队/聊天中重新安装
3. 检查你的组织管理员是否阻止了 RSC 权限
4. 确认你使用的是正确范围：团队使用 `ChannelMessage.Read.Group`，群组聊天使用 `ChatMessage.Read.Chat`
...
- [Teams bot 文件处理](https://learn.microsoft.com/en-us/microsoftteams/platform/bots/how-to/bots-filesv4)（渠道/群组需要 Graph）
