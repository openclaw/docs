...
title: Google Chat
x-i18n:
    generated_at: "2026-05-03T22:00:27Z"
    model: gpt-5.5
    provider: openai
...
Status：可下载插件，适用于通过 Google Chat API webhook（仅 HTTP）的私信 + 空间。
...
6. **启用应用状态**：
   - 保存后，**刷新页面**。
   - 查找 **App status** 部分（保存后通常位于顶部或底部附近）。
...
9. 启动 Gateway 网关。Google Chat 会向你的 webhook 路径 POST。

## 添加到 Google Chat

Gateway 网关运行并且你的电子邮件已添加到可见性列表后：

1. 前往 [Google Chat](https://chat.google.com/)。
2. 点击 **Direct Messages** 旁边的 **+**（加号）图标。
3. 在搜索栏（你通常添加人员的位置）中，输入你在 Google Cloud Console 中配置的 **App name**。
   - **注意**：该机器人不会出现在 “Marketplace” 浏览列表中，因为它是一个私有应用。你必须按名称搜索它。
4. 从结果中选择你的机器人。
...
Google Chat webhook 需要一个公共 HTTPS 端点。为安全起见，**仅将 `/googlechat` 路径暴露到互联网**。将 OpenClaw 仪表板和其他敏感端点保留在你的私有网络中。

### 选项 A：Tailscale Funnel（推荐）

使用 Tailscale Serve 提供私有仪表板，并使用 Funnel 提供公共 webhook 路径。这样会保持 `/` 私有，同时仅暴露 `/googlechat`。
...
> 注意：此配置会在重启后保留。以后如需移除，请运行 `tailscale funnel reset` 和 `tailscale serve reset`。

### 选项 B：反向代理（Caddy）

如果你使用 Caddy 之类的反向代理，请仅代理特定路径：
...
配置你的 tunnel 入站规则，使其仅路由 webhook 路径：
...
1. Google Chat 将 webhook POST 发送到 Gateway 网关。每个请求都包含一个 `Authorization: Bearer <token>` 标头。
   - 当该标头存在时，OpenClaw 会在读取/解析完整 webhook 正文之前验证 bearer auth。
   - 支持在正文中携带 `authorizationEventObject.systemIdToken` 的 Google Workspace Add-on 请求，并通过更严格的预认证正文预算处理。
...
4. 私信访问默认通过配对完成。未知发送者会收到配对码；使用以下命令批准：
   - `openclaw pairing approve googlechat <code>`
5. 群组空间默认要求 @ 提及。如果提及检测需要应用的用户名，请使用 `botUser`。
...
- `dangerouslyAllowNameMatching` 会重新启用用于 allowlist 的可变电子邮件主体匹配（应急兼容模式）。
- 启用 `actions.reactions` 后，可通过 `reactions` 工具和 `channels action` 使用回应。
- 消息操作暴露 `send` 用于文本，暴露 `upload-file` 用于显式附件发送。`upload-file` 接受 `media` / `filePath` / `path`，以及可选的 `message`、`filename` 和线程目标。
...
1. **未配置渠道**：你的配置中缺少 `channels.googlechat` 部分。使用以下命令验证：
...
- 检查 `openclaw channels status --probe` 以查看认证错误或缺失的受众配置。
- 如果没有消息到达，请确认 Chat 应用的 webhook URL + 事件订阅。
- 如果提及门控阻止回复，请将 `botUser` 设置为应用的用户资源名称，并验证 `requireMention`。
- 发送测试消息时使用 `openclaw logs --follow`，以查看请求是否到达 Gateway 网关。
