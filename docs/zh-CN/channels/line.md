...
title: LINE
x-i18n:
    generated_at: "2026-07-12T14:18:21Z"
    model: gpt-5.6
...
接收器运行在 Gateway 网关上，并使用你的渠道访问令牌和渠道密钥进行
身份验证。
...
Gateway 网关会响应 LINE 的 webhook 验证（GET），并在签名和负载验证后立即确认
已签名的入站事件（POST）；智能体处理将异步继续。
...
- OpenClaw 使用已验证的原始请求字节处理 webhook 事件。为保障签名完整性，会忽略由上游中间件转换的 `req.body` 值。
...
- 每群组覆盖配置：`channels.line.groups.<groupId>.allowFrom`（以及 `enabled`、`requireMention`、`systemPrompt`、`skills`）
- `allowFrom`、`groupAllowFrom` 和每群组的 `allowFrom` 可以通过 `accessGroup:<name>` 引用静态发送者访问组；请参阅[访问组](/zh-CN/channels/access-groups)。
- 运行时注意事项：如果完全缺少 `channels.line`，运行时会回退到使用 `groupPolicy="allowlist"` 执行群组检查（即使已设置 `channels.defaults.groupPolicy`）。
...
- 流式响应会被缓冲；智能体工作期间，LINE 会显示加载
  动画并接收完整的分块。
- 媒体下载大小受 `channels.line.mediaMaxMb` 限制（默认值为 10）。
...
- **图像**：作为 LINE 图像消息发送；预览图像默认为媒体 URL。
- **视频**：需要预览图像；请将 `channelData.line.previewImageUrl` 设置为图像 URL。
- **音频**：作为 LINE 音频消息发送；除非设置了 `channelData.line.durationMs`，否则时长默认为 60 秒。

如果设置了 `channelData.line.mediaKind`，则从中获取媒体类型；否则根据
其他 LINE 选项或 URL 文件后缀推断，并以图像作为回退类型。

出站媒体 URL 必须是长度不超过 2000 个字符的公共 HTTPS URL。OpenClaw
会在将 URL 交给 LINE 前验证目标主机名，并拒绝 local loopback、
链路本地和专用网络目标。

不含 LINE 特定选项的通用媒体发送使用图像路由。
