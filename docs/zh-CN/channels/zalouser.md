...
summary: 通过原生 zca-js（QR 登录）支持 Zalo Personal 账号、能力和配置
title: Zalo Personal
x-i18n:
    generated_at: "2026-06-27T01:29:41Z"
    model: gpt-5.5
...
状态：实验性。此集成通过 OpenClaw 内的原生 `zca-js` 自动化一个**个人 Zalo 账号**。
...
Zalo Personal 在当前 OpenClaw 版本中作为内置插件发布，因此普通的
打包构建无需单独安装。

如果你使用的是较旧构建，或排除了 Zalo Personal 的自定义安装，
请直接安装 npm 包：
...
- 适用于 Zalo Bot API 不可用的“个人账号”用例。

## 命名

渠道 ID 是 `zalouser`，以明确表示它自动化的是一个**个人 Zalo 用户账号**（非官方）。我们将 `zalo` 保留给未来可能的官方 Zalo API 集成。
...
如果原始名称保留在配置中，启动时只有在启用 `channels.zalouser.dangerouslyAllowNameMatching: true` 时才会解析它。没有这个显式选择时，运行时发送者检查仅基于 ID，原始名称会被忽略，不用于授权。
...
- 使用以下配置限制为允许列表：
  ...
  - `channels.zalouser.groupAllowFrom`（控制允许群组中哪些发送者可以触发机器人；静态发送者访问组可以用 `accessGroup:<name>` 引用）
- 阻止所有群组：`channels.zalouser.groupPolicy = "disabled"`。
- 配置向导可以提示输入群组允许列表。
- 启动时，OpenClaw 只有在启用 `channels.zalouser.dangerouslyAllowNameMatching: true` 时才会将允许列表中的群组/用户名称解析为 ID 并记录映射。
- 群组允许列表匹配默认仅基于 ID。除非启用 `channels.zalouser.dangerouslyAllowNameMatching: true`，否则未解析的名称会被忽略，不用于认证。
...
- 群组历史限制默认使用 `messages.groupChat.historyLimit`（回退值 `50`）。你可以用 `channels.zalouser.historyLimit` 按账号覆盖。
...
账号会映射到 OpenClaw 状态中的 `zalouser` 配置档案。示例：
...
Zalo Personal 插件也可以从环境变量读取配置档案选择：

- `ZALOUSER_PROFILE`：当渠道或账号配置中未设置 `profile` 时使用的配置档案名称。
- `ZCA_PROFILE`：旧版回退配置档案名称，仅在未设置 `ZALOUSER_PROFILE` 时使用。

配置档案名称会选择 OpenClaw 状态中保存的 Zalo 登录凭证。解析顺序为：
...
对于多账号设置，建议在配置中为每个账号设置 `profile`，这样
一个环境变量不会导致多个账号共享同一个登录
会话。

## 正在输入、反应和送达确认

- OpenClaw 会在派发回复前发送正在输入事件（尽力而为）。
- 渠道动作中支持 `zalouser` 的消息反应动作 `react`。
  - 使用 `remove: true` 从消息中移除特定的反应 emoji。
