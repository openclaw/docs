...
渠道 ID 为 `zalouser`，用于明确表示这会自动化一个**个人 Zalo 用户账号**（非官方）。我们保留 `zalo`，用于未来可能的官方 Zalo API 集成。
...
- 群组历史限制默认使用 `messages.groupChat.historyLimit`（回退值为 `50`）。你可以使用 `channels.zalouser.historyLimit` 按账号覆盖。
