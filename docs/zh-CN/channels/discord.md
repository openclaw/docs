...
title: Discord
x-i18n:
    generated_at: "2026-07-03T02:41:56Z"
    model: gpt-5.5
...
准备通过官方 Discord Gateway 网关支持私信和服务器频道。
...
你需要创建一个带 Bot 的新应用，将 Bot 添加到你的服务器，并将它配对到 OpenClaw。我们建议将你的 Bot 添加到你自己的私有服务器。如果你还没有服务器，请先[创建一个](https://support.discord.com/hc/en-us/articles/204849977-How-do-I-create-a-server)（选择 **Create My Own > For me and my friends**）。
...
<Steps>
  ...
  <Step title="生成邀请 URL 并将 Bot 添加到你的服务器">
    ...
    **Text Permissions**
    ...
    复制底部生成的 URL，将它粘贴到浏览器中，选择你的服务器，然后点击 **Continue** 进行连接。你现在应该能在 Discord 服务器中看到你的 Bot。
  ...
  <Step title="启用 Developer Mode 并收集你的 ID">
    ...
    1. 点击 **User Settings**（你头像旁边的齿轮图标）→ 在侧边栏中滚动到 **Developer** → 开启 **Developer Mode**
  ...
  <Step title="允许来自服务器成员的私信">
    为了让配对正常工作，Discord 需要允许你的 Bot 给你发送私信。右键点击你的**服务器图标** → **Privacy Settings** → 开启 **Direct Messages**。

    这会允许服务器成员（包括 Bot）给你发送私信。如果你想通过 Discord 私信使用 OpenClaw，请保持此项启用。如果你只计划使用服务器频道，可以在配对后禁用私信。

  </Step>

  <Step title="安全设置你的 Bot token（不要在聊天中发送）">
    你的 Discord Bot token 是机密（类似密码）。在给你的智能体发消息前，先在运行 OpenClaw 的机器上设置它。
...
```
    ...
    对于托管服务安装，请在存在 `DISCORD_BOT_TOKEN` 的 shell 中运行 `openclaw gateway install`，或将该变量存储在 `~/.openclaw/.env` 中，这样服务重启后就能解析 env SecretRef。
    如果你的主机被 Discord 的启动应用查找阻止或限速，请从 Developer Portal 设置 Discord application/client ID，这样启动时可以跳过该 REST 调用。默认账户使用 `channels.discord.applicationId`；运行多个 Discord Bot 时使用 `channels.discord.accounts.<accountId>.applicationId`。
  ...
  <Step title="配置 OpenClaw 并配对">
    ...
    <Tabs>
      <Tab title="询问你的智能体">
        在任何已有频道（例如 Telegram）中与你的 OpenClaw 智能体聊天并告诉它。如果 Discord 是你的第一个频道，请改用 CLI / 配置标签页。
...
```
        ...
        对于脚本化或远程设置，请使用 `openclaw config patch --file ./discord.patch.json5 --dry-run` 写入相同的 JSON5 区块，然后去掉 `--dry-run` 重新运行。支持明文 `token` 值。`channels.discord.token` 也支持跨 env/file/exec 提供商的 SecretRef 值。参见 [Secrets Management](/zh-CN/gateway/secrets)。

        对于多个 Discord Bot，请将每个 Bot token 和 application ID 放在各自账户下。顶层 `channels.discord.applicationId` 会被账户继承，因此只有当每个账户都应使用相同 application ID 时才在那里设置。
...
```
  ...
  <Step title="批准首次私信配对">
    等待 Gateway 网关运行后，在 Discord 中给你的 Bot 发送私信。它会回复一个配对码。

    <Tabs>
      <Tab title="询问你的智能体">
        在你的已有频道中将配对码发送给你的智能体：
...
```
    ...
    你现在应该可以通过私信在 Discord 中与你的智能体聊天。

  </Step>
</Steps>

<Note>
Token 解析会感知账户。配置中的 token 值优先于 env 回退。`DISCORD_BOT_TOKEN` 仅用于默认账户。
如果两个已启用的 Discord 账户解析到同一个 Bot token，OpenClaw 只会为该 token 启动一个 Gateway 网关监视器。来自配置的 token 优先于默认 env 回退；否则第一个启用的账户获胜，重复账户会报告为已禁用。
对于高级出站调用（消息工具/频道操作），显式的逐调用 `token` 会用于该调用。这适用于发送和读取/探测类操作（例如 read/search/fetch/thread/pins/permissions）。账户策略/重试设置仍来自活跃运行时快照中选定的账户。
</Note>

## 推荐：设置服务器工作区

私信可用后，你可以将 Discord 服务器设置为完整工作区，其中每个频道都有自己的智能体会话和独立上下文。对于只有你和你的 Bot 的私有服务器，建议这样做。

<Steps>
  <Step title="将你的服务器添加到服务器允许列表">
    这会让你的智能体能够在你服务器上的任何频道中响应，而不只是私信。
...
```
  ...
  <Step title="允许无需 @mention 即可响应">
    默认情况下，你的智能体只会在服务器频道中被 @提及时响应。对于私有服务器，你可能希望它响应每条消息。

    在服务器频道中，普通回复默认会自动发布。对于共享的常驻房间，可以选择启用 `messages.groupChat.visibleReplies: "message_tool"`，这样智能体可以潜伏，只在它判断频道回复有用时才发布。这最适合 GPT 5.5 等最新一代、工具可靠的模型。除非工具发送，否则环境房间事件会保持安静。完整潜伏模式配置见 [Ambient room events](/zh-CN/channels/ambient-room-events)。

    如果 Discord 显示正在输入，日志也显示 token 使用量，但没有发布消息，请检查该轮次是否被配置为环境房间事件，或是否启用了 message-tool 可见回复。

    <Tabs>
      <Tab title="询问你的智能体">
        > “允许我的智能体在此服务器上无需被 @提及即可响应”
      </Tab>
      <Tab title="配置">
...
```
        ...
        若要要求可见群组/频道回复通过 message-tool 发送，请设置 `messages.groupChat.visibleReplies: "message_tool"`。
  ...
  <Step title="规划服务器频道中的记忆">
    ...
    <Tabs>
      ...
      <Tab title="手动">
        如果你需要每个频道都有共享上下文，请将稳定说明放在 `AGENTS.md` 或 `USER.md` 中（它们会注入到每个会话）。将长期笔记保存在 `MEMORY.md` 中，并在需要时通过记忆工具访问。
      </Tab>
    </Tabs>
...
- 面向 Discord 的纯文本 cron/Heartbeat 公告投递只使用最终的
  assistant 可见答案一次。当智能体发出多个可投递载荷时，媒体和结构化组件载荷仍保持
  多消息形式。
...
组件回调默认在 30 分钟后过期。设置 `channels.discord.agentComponents.ttlMs` 可更改默认 Discord 账号的回调注册表生命周期，或设置 `channels.discord.accounts.<accountId>.agentComponents.ttlMs` 可在多账号设置中覆盖某个账号。该值以毫秒为单位，必须是正整数，并且上限为 `86400000`（24 小时）。更长的 TTL 对需要按钮保持可用的评审或审批工作流很有用，但也会延长旧 Discord 消息仍可触发操作的窗口。优先使用适合工作流的最短 TTL；当过期回调会令人意外时，请保留默认值。

`/model` 和 `/models` 斜杠命令会打开交互式模型选择器，其中包含提供商、模型和兼容运行时下拉菜单，以及提交步骤。`/models add` 已弃用，现在会返回弃用消息，而不是从聊天中注册模型。选择器回复是仅自己可见的，且只有调用用户可以使用。Discord 选择菜单最多限制为 25 个选项，因此当你希望选择器只为所选提供商（例如 `openai` 或 `vllm`）显示动态发现的模型时，请向 `agents.defaults.models` 添加 `provider/*` 条目。
...
<Tabs>
  <Tab title="DM policy">
    ...
    - 当命名账号自己的 `allowFrom` 和旧版 `dm.allowFrom` 均未设置时，它们会继承 `channels.discord.allowFrom`。
    ...
    当频道默认值处于活动状态时，裸数字 ID 通常会解析为频道 ID，但为兼容起见，账号有效私信 `allowFrom` 中列出的 ID 会被视为用户私信目标。
  ...
  <Tab title="Access groups">
    ...
    访问组名称在各消息渠道之间共享。对于成员以每个渠道正常 `allowFrom` 语法表示的静态组，使用 `type: "message.senders"`；当 Discord 频道当前的 `ViewChannel` 受众应动态定义成员资格时，使用 `type: "discord.channelAudience"`。共享访问组行为记录在此处：[访问组](/zh-CN/channels/access-groups)。
...
```
    ...
    示例：允许任何能看到 `#maintainers` 的人给机器人发私信，同时对其他所有人关闭私信。
...
```
    ...
    查询会失败关闭。如果 Discord 返回 `Missing Access`、成员查询失败，或频道属于其他服务器，则私信发送者会被视为未授权。
  ...
  <Tab title="Guild policy">
    ...
    - 可选发送者允许列表：`users`（推荐稳定 ID）和 `roles`（仅角色 ID）；如果配置了其中任一项，发送者匹配 `users` 或 `roles` 时即被允许
...
```
    ...
    如果你只设置 `DISCORD_BOT_TOKEN` 且未创建 `channels.discord` 块，运行时回退值为 `groupPolicy="allowlist"`（日志中会有警告），即使 `channels.defaults.groupPolicy` 为 `open` 也是如此。
  ...
  <Tab title="Mentions and group DMs">
    ...
    编写出站 Discord 消息时，请使用规范的 mention 语法：用户用 `<@USER_ID>`，频道用 `<#CHANNEL_ID>`，角色用 `<@&ROLE_ID>`。不要使用旧版 `<@!USER_ID>` 昵称 mention 形式。
    ...
    - 可通过 `dm.groupChannels` 设置可选允许列表（频道 ID 或 slug）
...
使用 `bindings[].match.roles` 按角色 ID 将 Discord 服务器成员路由到不同智能体。基于角色的绑定仅接受角色 ID，并且会在 peer 或 parent-peer 绑定之后、guild-only 绑定之前求值。如果某个绑定还设置了其他匹配字段（例如 `peer` + `guildId` + `roles`），则所有已配置字段都必须匹配。
...
- 对于未授权用户，命令可能仍会在 Discord UI 中可见；执行时仍会强制执行 OpenClaw 鉴权并返回“not authorized”。
...
<AccordionGroup>
  <Accordion title="回复标签和原生回复">
    ...
    `first` 始终会把该轮次的第一条发往 Discord 的消息附加隐式原生回复引用。
    `batched` 仅在入站事件是多条消息的防抖批次时附加 Discord 的隐式原生回复引用。这在你希望主要针对含义不明确的突发聊天使用原生回复，而不是每个单消息轮次都使用时很有用。
  ...
  <Accordion title="链接预览">
    Discord 默认会为 URL 生成富链接嵌入。OpenClaw 默认会抑制发往 Discord 的出站消息上的这些生成嵌入，因此智能体发送的 URL 会保持为纯链接，除非你选择启用：
...
```
  ...
  <Accordion title="实时流预览">
    OpenClaw 可以通过发送临时消息，并在文本到达时编辑它来流式传输草稿回复。`channels.discord.streaming` 接受 `off` | `partial` | `block` | `progress`（默认）。`progress` 保留一条可编辑的状态草稿，并用工具进度更新它，直到最终送达；共享起始标签是一行滚动内容，因此当出现足够多工作内容后，它会像其余内容一样滚动离开。`streamMode` 是旧版运行时别名。运行 `openclaw doctor --fix` 可将持久化配置重写为规范键。

    将 `channels.discord.streaming.mode` 设置为 `off` 可禁用 Discord 预览编辑。如果显式启用了 Discord 分块流式传输，OpenClaw 会跳过预览流，以避免双重流式传输。
...
```
    ...
    - 工具/进度行在可用时会呈现为紧凑的表情符号 + 标题 + 详情，例如 `🛠️ Bash: run tests` 或 `🔎 Web Search: for "query"`。
    - `streaming.progress.commentary`（默认 `false`）选择在临时进度草稿中包含助手 commentary/preamble 文本。Commentary 会在显示前清理，保持临时状态，并且不会改变最终答案送达。
  ...
  <Accordion title="历史、上下文和线程行为">
    ...
    - 线程会话会继承父频道的会话级 `/model` 选择，作为仅模型回退；线程本地 `/model` 选择仍优先，并且除非启用 transcript 继承，否则不会复制父 transcript 历史。
    ...
    频道主题会作为**不受信任**的上下文注入。Allowlist 控制谁可以触发智能体，而不是完整的补充上下文删改边界。
  ...
  <Accordion title="子智能体的线程绑定会话">
    ...
    - `/session max-age <duration|off>` 查看/更新聚焦绑定的硬性最长生命周期
...
```
    ...
    - `spawnSessions` 控制为 `sessions_spawn({ thread: true })` 和 ACP 线程 spawn 自动创建/绑定线程。默认值：`true`。
    - `defaultSpawnContext` 控制线程绑定 spawn 的原生子智能体上下文。默认值：`"fork"`。
...
```
    ...
    - `/acp spawn codex --bind here` 会原地绑定当前频道或线程，并让未来消息保持在同一 ACP 会话上。线程消息会继承父频道绑定。
    - 在已绑定的频道或线程中，`/new` 和 `/reset` 会原地重置同一个 ACP 会话。临时线程绑定在活动期间可以覆盖目标解析。
  ...
  <Accordion title="Reaction 通知">
    ...
    Reaction 事件会转换为系统事件并附加到路由到的 Discord 会话。
...
```
  ...
  <Accordion title="Gateway 网关代理">
    使用 `channels.discord.proxy` 通过 HTTP(S) 代理路由 Discord Gateway 网关 WebSocket 流量和启动 REST 查询（应用程序 ID + allowlist 解析）。
    Discord Gateway 网关 WebSocket 代理是显式配置的；WebSocket 连接不会继承 Gateway 网关进程的环境代理环境变量。配置 `channels.discord.proxy` 后，启动 REST 查询会使用此代理。
...
```
  ...
  <Accordion title="Approvals in Discord">
    ...
    当 `enabled` 未设置或为 `"auto"`，且至少可以从 `execApprovals.approvers` 或 `commands.ownerAllowFrom` 解析出一个审批人时，Discord 会自动启用原生 Exec 审批。Discord 不会从频道 `allowFrom`、旧版 `dm.allowFrom` 或直接消息 `defaultTo` 推断 Exec 审批人。设置 `enabled: false` 可显式禁用 Discord 作为原生审批客户端。

    对于 `/diagnostics` 和 `/export-trajectory` 等敏感的仅所有者可用群组命令，OpenClaw 会私下发送审批提示和最终结果。当调用的所有者有 Discord 所有者路由时，它会先尝试 Discord 私信；如果不可用，则回退到 `commands.ownerAllowFrom` 中第一个可用的所有者路由，例如 Telegram。

    当 `target` 为 `channel` 或 `both` 时，审批提示会在频道中可见。只有已解析的审批人可以使用按钮；其他用户会收到一条临时拒绝消息。审批提示包含命令文本，因此只应在可信频道中启用频道投递。如果无法从会话键推导出频道 ID，OpenClaw 会回退到私信投递。

    Discord 也会渲染其他聊天渠道使用的共享审批按钮。原生 Discord 适配器主要增加审批人私信路由和频道扇出。
    当这些按钮存在时，它们是主要审批 UX；只有当工具结果表示
    聊天审批不可用，或手动审批是唯一路径时，OpenClaw
    才应包含手动 `/approve` 命令。
    如果 Discord 原生审批运行时未激活，OpenClaw 会保持
    本地确定性的 `/approve <id> <decision>` 提示可见。如果
    运行时已激活，但原生卡片无法投递到任何目标，
    OpenClaw 会使用待处理审批中的确切 `/approve`
    命令，在同一聊天中发送回退通知。

    Gateway 网关认证和审批解析遵循共享 Gateway 网关客户端契约（`plugin:` ID 通过 `plugin.approval.resolve` 解析；其他 ID 通过 `exec.approval.resolve` 解析）。审批默认 30 分钟后过期。
...
OpenClaw 将 Discord components v2 用于 Exec 审批和跨上下文标记。Discord 消息操作也可以接受 `components` 来构建自定义 UI（高级用法；需要通过 discord 工具构造组件载荷），而旧版 `embeds` 仍可使用，但不推荐。
...
- `channels.discord.agentComponents.ttlMs` 控制已发送 Discord 组件回调的注册保持时长（默认 `1800000`，最大 `86400000`）。使用 `channels.discord.accounts.<id>.agentComponents.ttlMs` 为每个账户设置。
...
- `voice.mode` 控制对话路径。默认值是 `agent-proxy`：实时语音前端处理轮次时序、打断和播放，通过 `openclaw_agent_consult` 将实质性工作委托给路由后的 OpenClaw 智能体，并像处理来自该说话人的已输入 Discord 提示一样处理结果。`stt-tts` 保留较旧的批处理 STT 加 TTS 流程。`bidi` 允许实时模型直接对话，同时暴露 `openclaw_agent_consult` 供 OpenClaw 大脑使用。
...
- `voice.model` 覆盖 Discord 语音响应和实时咨询使用的 OpenClaw 智能体大脑。保持未设置时继承路由后的智能体模型。它独立于 `voice.realtime.model`。
- `voice.followUsers` 允许机器人跟随选定用户加入、移动和离开 Discord 语音。行为规则和示例见[在语音中跟随用户](#follow-users-in-voice)。
- `agent-proxy` 通过 `discord-voice` 路由语音；这会保留说话人和目标会话的常规所有者/工具授权，但会隐藏智能体 `tts` 工具，因为 Discord 语音负责播放。默认情况下，`agent-proxy` 会为所有者说话人的咨询提供等同所有者的完整工具访问权限（`voice.realtime.toolPolicy: "owner"`），并强烈优先在给出实质性回答前咨询 OpenClaw 智能体（`voice.realtime.consultPolicy: "always"`）。在默认的 `always` 模式下，实时层不会在咨询答案前自动说填充语；它会捕获并转写语音，然后说出路由后的 OpenClaw 答案。如果多个强制咨询答案在 Discord 仍在播放第一个答案时完成，后续精确语音答案会排队，直到播放空闲，而不是在句子中途替换语音。
...
- 默认情况下，实时语音模式会在实时提供商指令中包含小型 `IDENTITY.md`、`USER.md` 和 `SOUL.md` 画像文件，这样快速直接轮次会保留与路由后的 OpenClaw 智能体相同的身份、用户定位和人格。将 `voice.realtime.bootstrapContextFiles` 设置为子集可自定义此行为，或设置为 `[]` 以禁用。支持的实时引导文件仅限这些画像文件；`AGENTS.md` 保留在常规智能体上下文中。注入的画像上下文不会替代 `openclaw_agent_consult` 来处理工作区工作、当前事实、记忆查找或工具支持的操作。
- 在 OpenAI `agent-proxy` 实时模式下，设置 `voice.realtime.requireWakeName: true`，让 Discord 实时语音保持静默，直到转写以唤醒名称开头或结尾。配置的唤醒名称必须是一到两个词。如果未设置 `voice.realtime.wakeNames`，OpenClaw 使用路由后的智能体 `name` 加 `OpenClaw`，并回退到智能体 id 加 `OpenClaw`。唤醒名称门控会禁用实时提供商自动响应，将已接受的轮次通过 OpenClaw 智能体咨询路径路由，并在最终转写到达前从部分转写中识别到前置唤醒名称时，给出简短的语音确认。
- OpenAI 实时提供商接受当前 Realtime 2 事件名称，以及用于输出音频和转写事件的旧版 Codex 兼容别名，因此兼容的提供商快照可以发生漂移，而不会丢失助手音频。
- `voice.realtime.bargeIn` 控制 Discord 说话人开始事件是否打断活动的实时播放。如果未设置，它会跟随实时提供商的输入音频打断设置。
- `voice.realtime.minBargeInAudioEndMs` 控制 OpenAI 实时插话截断音频前的最短助手播放时长。默认值：`250`。在低回声房间中设置为 `0` 可立即打断；在回声较重的扬声器设置中可提高该值。
- 对于 Discord 播放中的 OpenAI 语音，设置 `voice.tts.provider: "openai"`，并在 `voice.tts.providers.openai.speakerVoice` 下选择文本转语音语音。`cedar` 是当前 OpenAI TTS 模型上一个不错的偏男性化选择。
- 按频道配置的 Discord `systemPrompt` 覆盖会应用到该语音频道的语音转写轮次。
- 语音转写轮次会从 Discord `allowFrom`（或 `dm.allowFrom`）派生所有者状态，用于受所有者限制的命令和频道操作。智能体工具可见性遵循路由会话的已配置工具策略。
- Discord 语音对纯文本配置是选择启用的；设置 `channels.discord.voice.enabled=true`（或保留现有 `channels.discord.voice` 块）以启用 `/vc` 命令、语音运行时和 `GuildVoiceStates` Gateway 网关意图。
- `channels.discord.intents.voiceStates` 可以显式覆盖语音状态意图订阅。保持未设置时，意图会跟随有效的语音启用状态。
- 如果 `voice.autoJoin` 对同一服务器有多个条目，OpenClaw 会加入该服务器最后配置的频道。
- `voice.allowedChannels` 是可选的驻留允许列表。保持未设置时，允许 `/vc join` 加入任何已授权的 Discord 语音频道。设置后，`/vc join`、启动自动加入和机器人语音状态移动都会限制在列出的 `{ guildId, channelId }` 条目内。将其设置为空数组会拒绝所有 Discord 语音加入。如果 Discord 将机器人移动到允许列表之外，OpenClaw 会离开该频道，并在有可用目标时重新加入配置的自动加入目标。
...
- OpenClaw 使用内置的 `libopus-wasm` 编解码器来接收 Discord 语音并播放实时原始 PCM。它随附固定版本的 libopus WebAssembly 构建，不需要原生 opus 插件。
...
- 在 `stt-tts` 模式下，语音播放不会仅仅因为另一个用户开始说话而停止。为避免反馈回路，OpenClaw 会在 TTS 播放期间忽略新的语音捕获；请在播放结束后再说下一轮。实时模式会将说话人开始事件作为插话信号转发给实时提供商。
- 在实时模式下，扬声器回声进入开放麦克风可能看起来像插话，并打断播放。对于回声较重的 Discord 房间，设置 `voice.realtime.providers.openai.interruptResponseOnInputAudio: false`，以阻止 OpenAI 在输入音频上自动打断。若仍希望 Discord 说话人开始事件打断活动播放，请添加 `voice.realtime.bargeIn: true`。OpenAI 实时桥接会将短于 `voice.realtime.minBargeInAudioEndMs` 的播放截断视为可能的回声/噪声并忽略，记录为已跳过，而不是清除 Discord 播放。
- `voice.captureSilenceGraceMs` 控制 OpenClaw 在 Discord 报告说话人停止后等待多久，再将该音频片段最终确定用于 STT。默认值：`2000`；如果 Discord 将正常停顿拆成断续的部分转写，请提高此值。
...
- 当 OpenClaw 最终确定已捕获的说话人片段时，预期会出现 `The operation was aborted` 接收事件；它们是详细诊断，不是警告。
- 详细的 Discord 语音日志会为每个已接受的说话人片段包含一行有界 STT 转写预览，因此调试时可以看到用户侧和智能体回复侧，而不会倾倒无界的转写文本。
- 在 `agent-proxy` 模式下，强制咨询回退会跳过可能不完整的转写片段，例如以 `...` 结尾的文本或类似 `and` 的尾随连接词，以及明显不可操作的结束语，例如 “be right back” 或 “bye”。当这避免陈旧的排队答案时，日志会显示 `forced agent consult skipped reason=...`。
...
- 当被跟随的用户加入允许的语音频道时，OpenClaw 会加入该频道。当用户移动时，OpenClaw 会跟随移动。当活动的被跟随用户断开连接时，OpenClaw 会离开。
- 如果多个被跟随用户在同一服务器中，并且活动的被跟随用户离开，OpenClaw 会先移动到另一个已跟踪的被跟随用户频道，再离开该服务器。如果多个被跟随用户同时移动，最新观察到的语音状态事件优先。
- `allowedChannels` 仍然适用。处于不允许频道中的被跟随用户会被忽略，跟随拥有的会话会移动到另一个被跟随用户或离开。
- OpenClaw 会在启动时以及按有界间隔协调遗漏的语音状态事件。协调会采样已配置的服务器，并限制每次运行的 REST 查找次数，因此非常大的 `followUsers` 列表可能需要多个间隔才能收敛。
- 如果 Discord 或管理员在机器人跟随用户时移动机器人，OpenClaw 会重建语音会话，并在目标被允许时保留跟随所有权。如果机器人被移动到 `allowedChannels` 之外，OpenClaw 会离开，并在存在已配置目标时重新加入。
- DAVE 接收恢复可能会在反复解密失败后离开并重新加入同一频道。跟随拥有的会话会在该恢复路径中保留其跟随所有权，因此后续被跟随用户断开连接时仍会离开该频道。

在加入模式之间选择：

- 对于个人或操作员设置，如果机器人应该在你进入语音时自动位于语音中，请使用 `followUsers`。
- 对于即使没有被跟踪用户在语音中也应存在的固定房间机器人，请使用 `autoJoin`。
- 对于一次性加入或自动语音在线状态会令人意外的房间，请使用 `/vc join`。
...
- 转写文本会通过 Discord 入口和路由发送，同时响应 LLM 会以语音输出策略运行，该策略会隐藏智能体 `tts` 工具并要求返回文本，因为 Discord 语音负责最终的 TTS 播放。
- 设置 `voice.model` 时，它只会覆盖此语音频道轮次的响应 LLM。
- `voice.tts` 会覆盖合并到 `messages.tts` 之上；支持流式传输的提供商会直接向播放器供给音频，否则会在已加入的频道中播放生成的音频文件。
...
没有 `voice.agentSession` 块时，每个语音频道都会获得自己的路由式 OpenClaw 会话。例如，`/vc join channel:234567890123456789` 会与该 Discord 语音频道对应的会话对话。实时模型只是语音前端；实质性请求会交给已配置的 OpenClaw 智能体。如果实时模型没有调用 consult 工具就生成了最终转写，OpenClaw 会强制执行 consult 作为兜底，因此默认行为仍然像是在与智能体对话。
...
在 `agent-proxy` 模式下，机器人会加入已配置的语音频道，但 OpenClaw 智能体轮次会使用目标频道的正常路由会话和智能体。实时语音会话会把返回的结果朗读回语音频道。监督智能体仍然可以根据其工具策略使用普通消息工具，包括在这是正确操作时发送单独的 Discord 消息。

当委托的 OpenClaw 运行处于活动状态时，新的 Discord 语音转写会在启动另一个智能体轮次之前被视为实时运行控制。“status”、“cancel that”、“use the smaller fix” 或 “when you're done also check tests”等短语会被分类为活动会话的状态、取消、Steer 或后续输入。状态、取消、已接受的 Steer 和后续结果都会朗读回语音频道，以便调用者知道 OpenClaw 是否处理了该请求。

有用的目标形式：

- `target: "channel:123456789012345678"` 会通过 Discord 文本频道会话路由。
- `target: "123456789012345678"` 会被视为频道目标。
- `target: "dm:123456789012345678"` 或 `target: "user:123456789012345678"` 会通过该私信会话路由。
...
当模型通过开启的麦克风听到自己的 Discord 播放，但你仍希望通过说话打断它时，请使用此配置。OpenClaw 会阻止 OpenAI 因原始输入音频而自动打断，同时 `bargeIn: true` 会让 Discord 说话者开始事件和已处于活动状态的说话者音频，在下一个捕获轮次到达 OpenAI 之前取消活动的实时响应。`audioEndMs` 低于 `minBargeInAudioEndMs` 的非常早期插话信号会被视为可能的回声/噪声并被忽略，因此模型不会在第一个播放帧处被切断。
...
- 队列中的精确语音时：`discord voice: realtime exact speech queued ... queued=... outputAudioMs=... outputActive=...`，随后是 `discord voice: realtime exact speech dequeued reason=player-idle ...`
...
要调试音频被切断的问题，请把实时语音日志作为时间线阅读：

1. `realtime audio playback started` 表示 Discord 已开始播放助手音频。从此点起，桥接会开始统计助手输出块、Discord PCM 字节、提供商实时字节以及合成音频时长。
2. `realtime speaker turn opened` 标记某个 Discord 说话者变为活动状态。如果播放已经处于活动状态且启用了 `bargeIn`，后面可能会出现 `barge-in detected source=speaker-start`。
3. `realtime input audio started` 标记该说话者轮次收到的第一个实际音频帧。此处的 `outputActive=true` 或非零 `outputAudioMs` 表示麦克风在助手播放仍处于活动状态时发送输入。
4. `barge-in detected source=active-speaker-audio` 表示 OpenClaw 在助手播放处于活动状态时看到了实时说话者音频。这有助于区分真实打断和没有有用音频的 Discord 说话者开始事件。
5. `barge-in requested reason=...` 表示 OpenClaw 要求实时提供商取消或截断活动响应。它包含 `outputAudioMs`、`outputActive` 和 `playbackChunks`，因此你可以看到打断前助手音频实际播放了多少。
...
7. `realtime speaker turn closed` 会汇总捕获到的输入轮次。`chunks=0` 或 `hasAudio=false` 表示说话者轮次已打开，但没有可用音频到达实时桥接。`interruptedPlayback=true` 表示该输入轮次与助手输出重叠，并触发了插话逻辑。

有用字段：

- `outputAudioMs`：日志行之前由实时提供商生成的助手音频时长。
- `audioMs`：播放停止前 OpenClaw 统计到的助手音频时长。
...
- 伴随 `source=active-speaker-audio`、较小的 `outputAudioMs` 且同一用户在附近的立即切断，通常表示扬声器回声进入了麦克风。提高 `voice.realtime.minBargeInAudioEndMs`，降低扬声器音量，使用耳机，或设置 `voice.realtime.providers.openai.interruptResponseOnInputAudio: false`。
- `source=speaker-start` 后跟 `speaker turn closed ... hasAudio=false` 表示 Discord 报告了说话者开始，但没有音频到达 OpenClaw。这可能是暂时性的 Discord 语音事件、噪声门行为，或客户端短暂按下麦克风。
- 没有附近的插话或 `provider-clear-audio` 时出现 `audio playback stopped reason=stream-close`，表示本地 Discord 播放流意外结束。检查前面的提供商和 Discord 播放器日志。
- `capture ignored during playback (barge-in disabled)` 表示 OpenClaw 在助手音频处于活动状态时有意丢弃输入。如果你希望语音打断播放，请启用 `voice.realtime.bargeIn`。
- `barge-in ignored ... outputActive=false` 表示 Discord 或提供商 VAD 报告了语音，但 OpenClaw 没有可打断的活动播放。这不应切断音频。

凭证按组件解析：`voice.model` 使用 LLM 路由凭证，`tools.media.audio` 使用 STT 凭证，`messages.tts`/`voice.tts` 使用 TTS 凭证，`voice.realtime.providers` 或提供商的普通凭证配置使用实时提供商凭证。

### 语音消息

Discord 语音消息会显示波形预览，并要求使用 OGG/Opus 音频。OpenClaw 会自动生成波形，但需要 Gateway 网关主机上有 `ffmpeg` 和 `ffprobe`，以便检查和转换。

- 提供一个**本地文件路径**（URL 会被拒绝）。
- 省略文本内容（Discord 拒绝同一 payload 中同时包含文本和语音消息）。
...
<AccordionGroup>
  ...
  <Accordion title="Guild 消息意外被阻止">
    ...
    - 如果存在 guild `channels` 映射，则只允许列出的 channels
...
```
  ...
  <Accordion title="长时间运行的 Discord 轮次或重复回复">
    ...
    Discord gateway 队列旋钮：

    - 单账号：`channels.discord.eventQueue.listenerTimeout`
    - 多账号：`channels.discord.accounts.<accountId>.eventQueue.listenerTimeout`
    - 这只控制 Discord gateway listener 工作，不控制 agent 轮次生命周期

    Discord 不会对排队的 agent 轮次应用 channel 自有的超时。消息监听器会立即交接，排队的 Discord 运行会保留每个会话的顺序，直到 session/tool/runtime 生命周期完成或中止该工作。
...
```
  ...
  <Accordion title="Gateway 网关 metadata lookup timeout warnings">
    OpenClaw 会在连接前获取 Discord `/gateway/bot` 元数据。瞬时失败会回退到 Discord 的默认 gateway URL，并在日志中限频记录。
  ...
  <Accordion title="Gateway 网关 READY timeout restarts">
    OpenClaw 在启动期间以及 runtime 重连后等待 Discord 的 gateway `READY` 事件。带启动错峰的多账号设置可能需要比默认值更长的启动 READY 窗口。
    ...
    - runtime 单账号：`channels.discord.gatewayRuntimeReadyTimeoutMs`
    - runtime 多账号：`channels.discord.accounts.<accountId>.gatewayRuntimeReadyTimeoutMs`
    - 配置未设置时的 runtime 环境变量回退：`OPENCLAW_DISCORD_RUNTIME_READY_TIMEOUT_MS`
    - runtime 默认：`30000`（30 秒），最大：`120000`

  </Accordion>

  <Accordion title="权限审计不匹配">
    `channels status --probe` 权限检查只适用于数字 channel ID。

    如果你使用 slug 键，runtime 匹配仍可工作，但 probe 无法完整验证权限。
  ...
  <Accordion title="Bot 到 bot 循环">
    ...
    OpenClaw 还随附共享的 [bot 循环保护](/zh-CN/channels/bot-loop-protection)。每当 `allowBots` 允许 bot 编写的消息进入 dispatch 时，Discord 会将入站事件映射为 `(account, channel, bot pair)` 事实，通用配对防护会在该配对超过配置的事件预算后抑制它。该防护可防止失控的双 bot 循环，这类循环以前必须靠 Discord 速率限制来停止；它不会影响单 bot 部署，也不会影响保持在预算内的一次性 bot 回复。

    默认设置（设置 `allowBots` 时生效）：

    - `maxEventsPerWindow: 20` -- bot 配对可在滑动窗口内交换 20 条消息
    - `windowSeconds: 60` -- 滑动窗口长度
    - `cooldownSeconds: 60` -- 一旦预算触发，任一方向的每条额外 bot 到 bot 消息都会被丢弃一分钟
...
```
  ...
  <Accordion title="Voice STT 因 DecryptionFailed(...) 丢弃">
    ...
    - 如果自动重新加入后失败仍然继续，请收集日志，并与 [discord.js #11419](https://github.com/discordjs/discord.js/issues/11419) 和 [discord.js #11449](https://github.com/discordjs/discord.js/pull/11449) 中的上游 DAVE 接收历史进行比较
...
- 将 bot tokens 视为机密（受监管环境中建议使用 `DISCORD_BOT_TOKEN`）。
- 授予最小权限的 Discord 权限。
- 如果命令部署/状态过期，请重启 gateway，并使用 `openclaw channels status --probe` 重新检查。
