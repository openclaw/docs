...
- 向论坛父级（`channel:<forumId>`）发送消息以自动创建帖子串。帖子串标题使用你消息中的第一行非空内容。
...
论坛父级不接受 Discord 组件。如果你需要组件，请发送到帖子串本身（`channel:<threadId>`）。
...
<AccordionGroup>
  <Accordion title="使用了不允许的 intent，或 bot 看不到 guild 消息">
