...
广播组让多个智能体能够同时处理并响应同一条消息。这使你可以创建专业化的智能体团队，在同一个 WhatsApp 群组或私信中协作，并且全部使用同一个电话号码。

当前范围：**仅限 WhatsApp**（Web 渠道）。

广播组会在频道允许列表和群组激活规则之后评估。在 WhatsApp 群组中，这意味着当 OpenClaw 通常会回复时会发生广播（例如：被提及时，取决于你的群组设置）。

实时 WhatsApp QA 通道包含 `whatsapp-broadcast-group-fanout`，用于验证一条被提及的群组消息可以从两个已配置的智能体产生不同的可见回复。
...
添加一个顶层 `broadcast` 区块（与 `bindings` 并列）。键是 WhatsApp 对端 ID：
...
<Steps>
  ...
  <Step title="如果广播不适用">
    OpenClaw 会分发到普通路由，或分发到路由期间选择的已配置 ACP 会话路由。
  </Step>
</Steps>
...
- **群组上下文缓冲区**（最近用于上下文的群组消息）按对端共享，因此所有广播智能体在触发时都会看到相同上下文
...
在群组 `120363403215116621@g.us` 中，使用智能体 `["alfred", "baerbel"]`：
...
<AccordionGroup>
  <Accordion title="1. 让智能体保持聚焦">
    ...
    ✅ **好：** 每个智能体只有一个任务。❌ **差：** 一个通用的 “dev-helper” 智能体。
...
<Note>
**优先级：** `broadcast` 优先于普通路由绑定。已配置的 ACP 绑定（`bindings[].type="acp"`）是排他的：当其中一个匹配时，OpenClaw 会分发到已配置的 ACP 会话，而不是扇出广播。
</Note>
...
<AccordionGroup>
  ...
  <Accordion title="只有一个智能体响应">
    **原因：** 对端 ID 可能位于普通路由绑定中但不在 `broadcast` 中，或者它可能匹配了一个排他的已配置 ACP 绑定。
...
1. **最大智能体数量：** 没有硬性限制，但 10 个以上智能体可能会较慢。
2. **共享上下文：** 智能体看不到彼此的响应（这是有意设计）。
