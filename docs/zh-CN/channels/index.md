...
title: 聊天渠道
x-i18n:
    generated_at: "2026-05-01T21:34:15Z"
    model: gpt-5.5
    provider: openai
...
所有渠道都支持文本；媒体和回应因渠道而异。

## 投递说明

- 包含 markdown 图片语法的 Telegram 回复，例如 `![alt](url)`，
  在可行时会在最终出站路径上转换为媒体回复。
- Slack 多人私信会按群聊路由，因此群组策略、提及
  行为和群组会话规则适用于 MPIM 对话。
- WhatsApp 设置为按需安装：新手引导可以在
  插件包安装前显示设置流程，Gateway 网关只会在该渠道实际处于活动状态时加载 WhatsApp 运行时。

## 支持的渠道

- [BlueBubbles](/zh-CN/channels/bluebubbles) — **推荐用于 iMessage**；使用 BlueBubbles macOS 服务器 REST API，提供完整功能支持（内置插件；编辑、撤回、效果、回应、群组管理 — 编辑功能目前在 macOS 26 Tahoe 上损坏）。
...
- [iMessage（旧版）](/zh-CN/channels/imessage) — 通过 imsg CLI 实现的旧版 macOS 集成（已弃用，新设置请使用 BlueBubbles）。
- [IRC](/zh-CN/channels/irc) — 经典 IRC 服务器；支持渠道 + 私信，并带有配对/允许列表控制。
...
- [Nostr](/zh-CN/channels/nostr) — 通过 NIP-04 实现去中心化私信（内置插件）。
...
- [Voice Call](/zh-CN/plugins/voice-call) — 通过 Plivo 或 Twilio 提供电话功能（插件，单独安装）。
- [WebChat](/zh-CN/web/webchat) — 基于 WebSocket 的 Gateway 网关 WebChat UI。
- [微信](/zh-CN/channels/wechat) — 通过二维码登录使用腾讯 iLink Bot 插件；仅支持私聊（外部插件）。
...
- 最快的设置通常是 **Telegram**（简单的机器人 token）。WhatsApp 需要二维码配对，并且
  会在磁盘上存储更多状态。
- 群组行为因渠道而异；请参阅 [群组](/zh-CN/channels/groups)。
- 出于安全考虑，会强制执行私信配对和允许列表；请参阅 [安全](/zh-CN/gateway/security)。
