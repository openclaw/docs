...
title: 聊天渠道
x-i18n:
    generated_at: "2026-06-27T01:23:06Z"
    model: gpt-5.5
...
- WhatsApp 设置是按需安装：新手引导可以在
  插件包安装之前显示设置流程，而 Gateway 网关只会在该渠道实际启用时
  加载外部 ClawHub/npm 插件。
- 接受机器人撰写的入站消息的渠道可以使用共享的
  [机器人循环保护](/zh-CN/channels/bot-loop-protection)，防止机器人之间
  无限互相回复。
- 支持的常驻房间可以使用[环境房间事件](/zh-CN/channels/ambient-room-events)，
  让未提及智能体的房间闲聊成为安静上下文，除非智能体使用
  `message` 工具发送消息。
...
- [iMessage](/zh-CN/channels/imessage) - 在已登录的 Mac 上通过 `imsg` 桥接实现原生 macOS 集成（或在 Gateway 网关运行于其他位置时使用 SSH 包装器），包括用于回复、tapback、效果、附件和群组管理的私有 API 操作。当主机权限和 Messages 访问满足要求时，这是新 OpenClaw iMessage 设置的首选方式。
...
- [Raft](/zh-CN/channels/raft) - 用于人与智能体协作的 Raft CLI 唤醒桥接（外部插件）。
...
- [语音通话](/zh-CN/plugins/voice-call) - 通过 Plivo 或 Twilio 实现电话能力（插件，单独安装）。
