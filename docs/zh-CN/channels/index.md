...
title: 聊天渠道
x-i18n:
    generated_at: "2026-05-02T07:01:18Z"
    model: gpt-5.5
    provider: openai
...
OpenClaw 可以通过你已经使用的任何聊天应用与你对话。每个渠道都通过 Gateway 网关连接。
...
- WhatsApp 设置是按需安装：新手引导可以在
  插件包安装之前显示设置流程，并且 Gateway 网关仅在该渠道实际启用时
  加载 WhatsApp 运行时。

## 支持的渠道

- [BlueBubbles](/zh-CN/channels/bluebubbles) — **iMessage 推荐选择**；使用 BlueBubbles macOS 服务器 REST API，完整支持功能（内置插件；编辑、撤回、特效、回应、群组管理 — 编辑功能目前在 macOS 26 Tahoe 上损坏）。
...
- [IRC](/zh-CN/channels/irc) — 经典 IRC 服务器；通过配对/允许列表控制支持频道 + 私信。
...
- [语音通话](/zh-CN/plugins/voice-call) — 通过 Plivo 或 Twilio 实现电话通信（插件，需单独安装）。
- [WebChat](/zh-CN/web/webchat) — 通过 WebSocket 使用 Gateway 网关 WebChat UI。
...
- [Zalo 个人版](/zh-CN/channels/zalouser) — 通过二维码登录使用 Zalo 个人账号（内置插件）。
...
- 为了安全，会强制执行私信配对和允许列表；参见 [安全](/zh-CN/gateway/security)。
