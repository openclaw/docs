---
read_when:
    - 你想连接 Feishu/Lark 机器人
    - 你正在配置 Feishu 渠道
summary: Feishu 机器人概览、功能和配置
title: Feishu
x-i18n:
    generated_at: "2026-05-06T02:59:56Z"
    model: gpt-5.5
    provider: openai
...
Feishu/Lark 是一个一体化协作平台，团队可在其中聊天、共享文档、管理日历，并协同完成工作。

**Status：**已可用于生产环境中的机器人私信 + 群聊。WebSocket 是默认模式；webhook 模式可选。
...
<Note>
需要 OpenClaw 2026.4.25 或更高版本。运行 `openclaw --version` 检查版本。使用 `openclaw update` 升级。
</Note>
...
<Steps>
  ...
  ```
  使用你的 Feishu/Lark 移动应用扫描二维码，以自动创建 Feishu/Lark 机器人。
  </Step>
...
- `"open"` - 仅当 `allowFrom` 包含 `"*"` 时允许公开私信；如果是限制性条目，则只有匹配用户可以聊天
- `"disabled"` - 禁用所有私信
...
- 仅广播用的 `@all` 和 `@_all` 不会被视为机器人提及。既提及 `@all` 又直接提及机器人的消息，仍会计为机器人提及。
...
在 `allowlist` 模式下，你也可以通过添加显式的 `groups.<chat_id>` 条目来允许某个群组。显式条目不会覆盖 `groupPolicy: "disabled"`。`groups.*` 下的通配符默认值会配置匹配的群组，但它们本身不会准入群组。
...
在 Feishu/Lark 中打开群组，点击右上角菜单图标，然后前往**设置**。群组 ID（`chat_id`）会列在设置页面上。
...
<Note>
Feishu/Lark 不支持原生斜杠菜单命令，因此请将这些命令作为纯文本消息发送。
</Note>
...
`accounts.<id>.tts` 使用与 `messages.tts` 相同的结构，并深度合并到全局 TTS 配置之上，因此多机器人 Feishu 设置可以在全局保留共享的提供商凭证，同时按账号仅覆盖语音、模型、人设或自动模式。
...
- `typingIndicator`（默认 `true`）：设置为 `false` 可跳过输入状态反应调用
...
Feishu/Lark 支持用于私信和群组话题消息的 ACP。Feishu/Lark ACP 由文本命令驱动 - 没有原生斜杠菜单命令，因此请直接在对话中使用 `/acp ...` 消息。
...
| `channels.feishu.groups.<chat_id>.requireMention` | 每群组 @mention 覆盖；显式 ID 也会在允许列表模式下允许该群组                    | 继承             |
...
入站 Feishu/Lark 音频消息会规范化为媒体占位符，而不是原始 `file_key` JSON。配置 `tools.media.audio` 后，OpenClaw 会下载语音留言资源，并在智能体轮次之前运行共享音频转写，因此智能体会收到语音转写文本。如果 Feishu 在音频载荷中直接包含转写文本，则会使用该文本，而不会再调用一次 ASR。如果没有音频转写提供商，智能体仍会收到一个 `<media:audio>` 占位符以及保存的附件，而不是原始 Feishu 资源载荷。
...
原生 Feishu/Lark 音频气泡使用 Feishu `audio` 消息类型，并要求上传 Ogg/Opus 媒体（`file_type: "opus"`）。现有 `.opus` 和 `.ogg` 媒体会直接作为原生音频发送。只有在回复请求语音投递（`audioAsVoice` / 消息工具 `asVoice`，包括 TTS 语音留言回复）时，MP3/WAV/M4A 和其他可能的音频格式才会使用 `ffmpeg` 转码为 48kHz Ogg/Opus。普通 MP3 附件会保留为常规文件。如果缺少 `ffmpeg` 或转换失败，OpenClaw 会回退为文件附件并记录原因。

### 话题串和回复

- ✅ 行内回复
- ✅ 话题串回复
- ✅ 回复话题串消息时，媒体回复保持感知话题串

对于 `groupSessionScope: "group_topic"` 和 `"group_topic_sender"`，原生 Feishu/Lark 话题群组使用事件 `thread_id`（`omt_*`）作为规范话题会话键。OpenClaw 转换为话题串的普通群组回复会继续使用回复根消息 ID（`om_*`），以便首轮和后续轮次保持在同一会话中。
