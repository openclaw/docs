...
title: Feishu
x-i18n:
    generated_at: "2026-05-03T18:58:32Z"
    model: gpt-5.5
    provider: openai
...
**Status：** 已可用于生产环境的机器人私信 + 群聊。WebSocket 是默认模式；webhook 模式可选。
...
- 仅广播用的 `@all` 和 `@_all` 不会被视为机器人提及。同时提及 `@all` 和直接提及机器人的消息仍会计为机器人提及。
...
在 `allowlist` 模式下，你也可以通过添加显式的 `groups.<chat_id>` 条目来准入某个群组。显式条目不会覆盖 `groupPolicy: "disabled"`。`groups.*` 下的通配默认值会配置匹配的群组，但它们本身不会准入群组。
...
`accounts.<id>.tts` 使用与 `messages.tts` 相同的结构，并会深度合并到全局 TTS 配置之上，因此多机器人 Feishu 设置可以在全局保留共享的提供商凭据，同时仅按账号覆盖语音、模型、persona 或自动模式。
...
参见[获取群组/用户 ID](#get-groupuser-ids) 获取查找提示。
...
| `channels.feishu.groups.<chat_id>.requireMention` | 单群组 @mention 覆盖；显式 ID 也会在允许列表模式中准入该群组 | 继承        |
...
入站 Feishu/Lark 音频消息会规范化为媒体占位符，而不是原始 `file_key` JSON。当配置了 `tools.media.audio` 时，OpenClaw 会下载语音备注资源，并在智能体回合前运行共享音频转写，因此智能体会收到语音转写文本。如果 Feishu 在音频载荷中直接包含转写文本，则会使用该文本，而不会再次进行 ASR 调用。没有音频转写提供商时，智能体仍会收到一个 `<media:audio>` 占位符以及保存的附件，而不是原始 Feishu 资源载荷。
...
原生 Feishu/Lark 音频气泡使用 Feishu `audio` 消息类型，并要求上传 Ogg/Opus 媒体（`file_type: "opus"`）。现有 `.opus` 和 `.ogg` 媒体会直接作为原生音频发送。MP3/WAV/M4A 以及其他可能的音频格式，仅在回复请求语音交付（`audioAsVoice` / 消息工具 `asVoice`，包括 TTS 语音备注回复）时，才会通过 `ffmpeg` 转码为 48kHz Ogg/Opus。普通 MP3 附件会保持为常规文件。如果缺少 `ffmpeg` 或转换失败，OpenClaw 会回退为文件附件并记录原因。
...
对于 `groupSessionScope: "group_topic"` 和 `"group_topic_sender"`，原生 Feishu/Lark 话题群会使用事件 `thread_id`（`omt_*`）作为规范话题会话键。OpenClaw 转成话题串的普通群组回复会继续使用回复根消息 ID（`om_*`），因此首个回合和后续回合会保留在同一个会话中。
