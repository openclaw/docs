...
title: 音频和语音留言
x-i18n:
    generated_at: "2026-05-05T23:38:16Z"
    model: gpt-5.5
    provider: openai
...
- **媒体理解（音频）**：如果启用了音频理解（或自动检测到），OpenClaw 会：
  ...
  5. 成功后，会将 `Body` 替换为 `[Audio]` 块，并设置 `{{Transcript}}`。
...
OpenClaw 会按以下顺序自动检测，并在遇到第一个可用选项时停止：
...
注意：二进制检测在 macOS/Linux/Windows 上是尽力而为；请确保 CLI 位于 `PATH` 上（我们会展开 `~`），或使用完整命令路径设置显式 CLI 模型。
...
- 小于 1024 字节的微小/空音频文件会在提供商/CLI 转录前跳过。
- 音频的默认 `maxChars` **未设置**（完整转录）。设置 `tools.media.audio.maxChars` 或每个条目的 `maxChars` 可裁剪输出。
- OpenAI 自动默认值为 `gpt-4o-mini-transcribe`；若需要更高准确率，请设置 `model: "gpt-4o-transcribe"`。
- 使用 `tools.media.audio.attachments` 处理多个语音笔记（`mode: "all"` + `maxAttachments`）。
- 转录文本可在模板中作为 `{{Transcript}}` 使用。
- `tools.media.audio.echoTranscript` 默认关闭；启用后，会在智能体处理前向来源聊天发送转录确认。
- `tools.media.audio.echoFormat` 可自定义回显文本（占位符：`{transcript}`）。
- CLI stdout 有上限（5MB）；请保持 CLI 输出简洁。
- CLI `args` 应使用 `{{MediaPath}}` 表示本地音频文件路径。运行 `openclaw doctor --fix` 可从旧版 `audio.transcription.command` 配置中迁移已弃用的 `{input}` 占位符。
...
当群组聊天设置了 `requireMention: true` 时，OpenClaw 现在会在检查提及**之前**转录音频。这使得语音笔记即使包含提及也可以被处理。

**工作方式：**

1. 如果语音消息没有文本正文，且群组要求提及，OpenClaw 会执行“预检”转录。
2. 会检查转录文本中的提及模式（例如 `@BotName`、emoji 触发器）。
3. 如果发现提及，消息会进入完整回复流水线。
4. 转录文本用于提及检测，因此语音笔记可以通过提及门控。

**回退行为：**

- 如果预检期间转录失败（超时、API 错误等），消息会基于仅文本提及检测进行处理。
- 这可确保混合消息（文本 + 音频）绝不会被错误丢弃。
...
**示例：**用户在启用 `requireMention: true` 的 Telegram 群组中发送一条语音笔记，说“Hey @Claude, what's the weather?”。语音笔记会被转录，提及会被检测到，然后智能体会回复。

## 注意点

- 范围规则采用首个匹配胜出。`chatType` 会规范化为 `direct`、`group` 或 `room`。
