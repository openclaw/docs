...
read_when:
    ...
    - 将节点 canvas/camera 用于智能体上下文
    - 添加新的节点命令或 CLI 辅助工具
summary: 节点：用于画布/摄像头/屏幕/设备/通知/系统的配对、能力、权限和 CLI 辅助工具
title: 节点
x-i18n:
    generated_at: "2026-05-06T01:52:04Z"
    model: gpt-5.5
    provider: openai
...
**节点**是一台配套设备（macOS/iOS/Android/无头设备），它使用 `role: "node"` 连接到 Gateway 网关的 **WebSocket**（与操作者使用同一端口），并通过 `node.invoke` 暴露命令界面（例如 `canvas.*`、`camera.*`、`device.*`、`notifications.*`、`system.*`）。协议详情：[Gateway 网关协议](/zh-CN/gateway/protocol)。
...
如果节点使用已变更的认证详情（角色/范围/公钥）重试，之前的
...
- 设备配对记录是持久的已批准角色契约。令牌
  轮换保留在该契约内；它不能把已配对节点升级成配对批准从未授予的
  其他角色。
- `node.pair.*`（CLI：`openclaw nodes pending/approve/reject/remove/rename`）是独立的 Gateway 网关所有
  节点配对存储；它**不会**阻止 WS `connect` 握手。
- `openclaw nodes remove --node <id|name|ip>` 会从该
  独立的 Gateway 网关所有节点配对存储中删除过期条目。
- 批准范围遵循待处理请求声明的命令：
...
- 如果 OpenClaw 无法为解释器/运行时命令精确识别一个具体的本地文件，
  则会拒绝基于批准的执行，而不是假装拥有完整运行时覆盖。对于更广泛的解释器语义，请使用沙箱隔离、
...
- 如果已配置但未解析活动的本地 `gateway.auth.*` SecretRefs，节点主机认证会以关闭方式失败。
...
- `openclaw node run` / `openclaw node install` 上的 `--display-name`（会持久保存在节点上的 `~/.openclaw/node.json` 中）。
- `openclaw nodes rename --node <id|name|ip> --name "Build Node"`（Gateway 网关覆盖）。

### 将命令加入 allowlist

Exec 批准是**按节点主机**配置的。通过 Gateway 网关添加 allowlist 条目：
...
批准记录保存在节点主机上的 `~/.openclaw/exec-approvals.json`。
...
`host=auto` 不会自行隐式选择节点，但允许来自 `auto` 的显式按调用 `host=node` 请求。如果你希望节点 exec 成为会话默认值，请显式设置 `tools.exec.host=node` 或 `/exec host=node ...`。
...
2. Gateway 网关的平台策略必须允许已声明的命令。

Windows 和 macOS 配套节点默认允许安全的已声明命令，例如
`canvas.*`、`camera.list`、`location.get` 和 `screen.snapshot`。
通告 `talk` 能力或声明 `talk.*` 命令的可信节点
也默认允许已声明的 push-to-talk 命令（`talk.ptt.start`、`talk.ptt.stop`、
`talk.ptt.cancel`、`talk.ptt.once`），不受平台标签影响。
危险或隐私敏感的命令，例如 `camera.snap`、`camera.clip` 和
`screen.record`，仍然需要通过
`gateway.nodes.allowCommands` 显式选择加入。`gateway.nodes.denyCommands` 始终优先于
默认值和额外的 allowlist 条目。

插件所有的节点命令可以添加 Gateway 网关节点调用策略。该策略
在 allowlist 检查之后、转发到节点之前运行，因此原始
`node.invoke`、CLI 辅助命令和专用 agent 工具共享同一插件
权限边界。危险的插件节点命令仍然需要显式
`gateway.nodes.allowCommands` 选择加入。

节点更改其已声明命令列表后，请拒绝旧的设备配对，
...
- 节点必须**处于前台**才能使用 `canvas.*` 和 `camera.*`（后台调用会返回 `NODE_BACKGROUND_UNAVAILABLE`）。
...
当设置中启用 Location 时，节点会暴露 `location.get`。
...
- Motion 命令会受可用传感器的能力门控。

## 系统命令（节点主机 / Mac 节点）

macOS 节点提供 `system.run`、`system.notify` 和 `system.execApprovals.get/set`。
...
- `system.run` 会在载荷中返回 stdout/stderr/退出代码。
- Shell 执行现在会通过带有 `host=node` 的 `exec` 工具；`nodes` 仍然是显式节点命令的直接 RPC 接口面。
- `nodes invoke` 不暴露 `system.run` 或 `system.run.prepare`；它们仅保留在 exec 路径上。
- exec 路径会在审批前准备规范的 `systemRunPlan`。一旦审批获准，Gateway 网关会转发已存储的计划，而不是任何后续由调用方编辑的 command/cwd/session 字段。
- `system.notify` 会遵循 macOS 应用上的通知权限状态。
- 未识别的节点 `platform` / `deviceFamily` 元数据会使用保守的默认允许列表，该列表排除 `system.run` 和 `system.which`。如果你有意需要为未知平台启用这些命令，请通过 `gateway.nodes.allowCommands` 显式添加它们。
- `system.run` 支持 `--cwd`、`--env KEY=VAL`、`--command-timeout` 和 `--needs-screen-recording`。
- 对于 shell 包装器（`bash|sh|zsh ... -c/-lc`），请求范围内的 `--env` 值会缩减为显式允许列表（`TERM`、`LANG`、`LC_*`、`COLORTERM`、`NO_COLOR`、`FORCE_COLOR`）。
- 在允许列表模式下，对于始终允许决策，已知的分发包装器（`env`、`nice`、`nohup`、`stdbuf`、`timeout`）会持久化内部可执行文件路径，而不是包装器路径。如果无法安全解包，则不会自动持久化任何允许列表条目。
...
- 在 macOS 节点模式下，`system.run` 会受 macOS 应用中的 exec 审批门控（Settings → Exec approvals）。
  Ask/allowlist/full 的行为与无头节点主机相同；被拒绝的提示会返回 `SYSTEM_RUN_DENIED`。
- 在无头节点主机上，`system.run` 会受 exec 审批（`~/.openclaw/exec-approvals.json`）门控。
...
节点可能会在 `node.list` / `node.describe` 中包含 `permissions` 映射，该映射以权限名称为键（例如 `screenRecording`、`accessibility`），值为布尔值（`true` = 已授予）。
