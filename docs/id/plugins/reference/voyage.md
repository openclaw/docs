---
read_when:
    - Anda sedang memasang, mengonfigurasi, atau mengaudit Plugin voyage
summary: Menambahkan dukungan penyedia embedding memori.
title: Plugin Voyage
x-i18n:
    generated_at: "2026-05-02T20:58:15Z"
    model: gpt-5.5
    provider: openai
    source_hash: ba1dcf8d6f7c45c01f855797333e9d510bc822b2f7442c5a0727ac2a208ffe36
    source_path: plugins/reference/voyage.md
    workflow: 16
    postprocess_version: locale-links-v1
---

# Plugin Voyage

Menambahkan dukungan penyedia embedding memori.

## Distribusi

- Paket: `@openclaw/voyage-provider`
- Rute instalasi: disertakan dalam OpenClaw

## Permukaan

contracts: memoryEmbeddingProviders
