...
x-i18n:
    generated_at: "2026-07-16T18:20:21Z"
    model: gpt-5.6
...
Format bundel yang kompatibel menggunakan file manifesnya sendiri:
...
Setiap plugin OpenClaw native **harus** menyertakan `openclaw.plugin.json` di **root plugin**. OpenClaw membacanya untuk memvalidasi konfigurasi **tanpa mengeksekusi kode plugin**. Manifes yang tidak ada atau tidak valid akan memblokir validasi konfigurasi dan dianggap sebagai kesalahan plugin.

Lihat [Plugin](/id/tools/plugin) untuk panduan lengkap sistem plugin, dan [Model kapabilitas](/id/plugins/architecture#public-capability-model) untuk model kapabilitas native serta panduan kompatibilitas eksternal terkini.
...
- kepemilikan singkatan keluarga model
- snapshot statis kepemilikan kapabilitas (`contracts`)
- metadata pelaksana QA yang dapat diperiksa oleh host `openclaw qa` bersama
- metadata konfigurasi khusus saluran yang digabungkan ke permukaan katalog dan validasi

**Jangan gunakan untuk:** mendaftarkan perilaku runtime, mendeklarasikan titik masuk kode, atau metadata instalasi npm. Hal tersebut berada di dalam kode plugin dan `package.json` Anda.
...
| `requiresPlugins`                    | Tidak    | `string[]`                   | ID Plugin yang juga harus diinstal agar Plugin ini dapat berfungsi. Penemuan membuat Plugin tetap dapat dimuat, tetapi memperingatkan ketika ada Plugin wajib yang tidak tersedia.                                                                                           |
| `enabledByDefault`                   | Tidak    | `true`                       | Menandai Plugin bawaan sebagai diaktifkan secara default. Hilangkan, atau tetapkan nilai apa pun selain `true`, agar Plugin tetap dinonaktifkan secara default.                                                                                                            |
| `enabledByDefaultOnPlatforms`        | Tidak    | `string[]`                   | Menandai Plugin bawaan sebagai diaktifkan secara default hanya pada platform Node.js yang tercantum, misalnya `["darwin"]`. Konfigurasi eksplisit tetap diprioritaskan.                                                                                                     |
...
| `kind`                               | Tidak    | `PluginKind \| PluginKind[]` | Mendeklarasikan satu atau beberapa jenis Plugin eksklusif (`"memory"`, `"context-engine"`) yang digunakan oleh `plugins.slots.*`. Plugin yang memiliki kedua slot mendeklarasikan kedua jenis dalam satu larik.                                                          |
...
| `providerCatalogEntry`               | Tidak    | `string`                     | Jalur modul katalog penyedia ringan, relatif terhadap akar Plugin, untuk metadata katalog penyedia yang dicakup manifes dan dapat dimuat tanpa mengaktifkan runtime Plugin sepenuhnya.                                                                                       |
...
| `modelCatalog`                       | Tidak    | `object`                     | Metadata katalog model deklaratif untuk penyedia yang dimiliki oleh Plugin ini. Ini adalah kontrak bidang kendali untuk pencantuman hanya-baca, onboarding, pemilih model, alias, dan penyembunyian di masa mendatang tanpa memuat runtime Plugin.                             |
| `modelPricing`                       | Tidak    | `object`                     | Kebijakan pencarian harga eksternal milik penyedia. Gunakan untuk mengecualikan penyedia lokal/yang dihosting sendiri dari katalog harga jarak jauh atau memetakan referensi penyedia ke ID katalog OpenRouter/LiteLLM tanpa mengodekan ID penyedia secara permanen di inti. |
...
| `providerEndpoints`                  | Tidak    | `object[]`                   | Metadata host/baseUrl endpoint milik manifes untuk rute penyedia yang harus diklasifikasikan oleh inti sebelum runtime penyedia dimuat.                                                                                                                                    |
...
| `secretProviderIntegrations`         | Tidak    | `Record<string, object>`     | Preset penyedia exec SecretRef deklaratif yang dapat ditawarkan oleh antarmuka penyiapan atau penginstalan tanpa mengodekan integrasi khusus penyedia secara permanen di inti.                                                                                               |
| `cliBackends`                        | Tidak    | `string[]`                   | ID backend inferensi CLI yang dimiliki oleh Plugin ini. Digunakan untuk aktivasi otomatis saat dimulai berdasarkan referensi konfigurasi eksplisit.                                                                                                                        |
...
| `nonSecretAuthMarkers`               | Tidak    | `string[]`                   | Nilai placeholder kunci API milik Plugin bawaan yang mewakili status kredensial lokal, OAuth, atau ambien yang bukan rahasia.                                                                                                                                               |
| `commandAliases`                     | Tidak    | `object[]`                   | Nama perintah yang dimiliki oleh Plugin ini dan harus menghasilkan diagnostik konfigurasi dan CLI yang sadar-Plugin sebelum runtime dimuat.                                                                                                                                |
| `providerAuthEnvVars`                | Tidak    | `Record<string, string[]>`   | Metadata env kompatibilitas yang tidak digunakan lagi untuk pencarian autentikasi/status penyedia. Utamakan `setup.providers[].envVars` untuk Plugin baru; OpenClaw masih membacanya selama periode penghentian bertahap.                                                    |
...
| `channelEnvVars`                     | Tidak    | `Record<string, string[]>`   | Metadata env saluran ringan yang dapat diperiksa OpenClaw tanpa memuat kode Plugin. Gunakan ini untuk antarmuka penyiapan atau autentikasi saluran berbasis env yang perlu dilihat oleh pembantu startup/konfigurasi generik.                                                |
| `providerAuthChoices`                | Tidak    | `object[]`                   | Metadata pilihan autentikasi ringan untuk pemilih onboarding, resolusi penyedia pilihan, dan pengawatan flag CLI sederhana.                                                                                                                                                |
| `activation`                         | Tidak    | `object`                     | Metadata perencana aktivasi ringan untuk pemuatan yang dipicu oleh startup, penyedia, perintah, saluran, rute, dan kemampuan. Hanya metadata; runtime Plugin tetap memiliki perilaku sebenarnya.                                                                             |
| `setup`                              | Tidak    | `object`                     | Deskriptor penyiapan/onboarding ringan yang dapat diperiksa oleh antarmuka penemuan dan penyiapan tanpa memuat runtime Plugin.                                                                                                                                              |
...
| `contracts`                          | Tidak    | `object`                     | Snapshot statis kepemilikan kemampuan untuk kait autentikasi eksternal, embedding, ucapan, transkripsi waktu nyata, suara waktu nyata, pemahaman media, pembuatan gambar/video/musik, pengambilan web, pencarian web, penyedia pekerja, ekstraksi konten dokumen/web, dan kepemilikan alat. |
...
| `toolMetadata`                       | Tidak       | `Record<string, object>`     | Metadata ketersediaan yang ringan untuk alat milik plugin yang dideklarasikan dalam `contracts.tools`. Gunakan metadata ini ketika alat tidak boleh memuat runtime kecuali terdapat bukti konfigurasi, lingkungan, atau autentikasi.                                                                                                  |
| `channelConfigs`                     | Tidak       | `Record<string, object>`     | Metadata konfigurasi kanal milik manifes yang digabungkan ke dalam permukaan penemuan dan validasi sebelum runtime dimuat.                                                                                                                                                                 |
...
| `uiHints`                            | Tidak       | `Record<string, object>`     | Label UI, teks contoh isian, dan petunjuk sensitivitas untuk bidang konfigurasi.                                                                                                                                                                                                          |

## referensi katalog

`catalog` menyediakan petunjuk tampilan opsional untuk penjelajah plugin. Host dapat mengabaikan petunjuk ini. Petunjuk ini tidak pernah memasang atau mengaktifkan plugin, serta tidak mengubah perilaku runtime atau tingkat kepercayaannya.
...
| `order`    | `number`  | Petunjuk tampilan menaik di antara plugin pilihan; nilai lebih rendah muncul lebih awal. |
...
Gunakan bidang ini hanya untuk fakta deklaratif yang murah. Transportasi, transformasi permintaan, penyegaran token, validasi kredensial, dan perilaku pembuatan yang sebenarnya tetap berada dalam runtime plugin.
...
| `referenceAudioInputs` | Tidak    | `boolean`  | Khusus pembuatan video. Tetapkan ke `true` ketika penyedia menerima aset audio referensi; jika tidak, `video_generate` menyembunyikan parameter referensi audio. |

Setiap entri `configSignals` mendukung:

| Bidang           | Wajib    | Jenis      | Artinya                                                                                                                                                                                   |
| ---------------- | -------- | ---------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `rootPath`       | Ya       | `string`   | Jalur bertitik menuju objek konfigurasi milik plugin yang akan diperiksa, misalnya `plugins.entries.example.config`.                                                                                    |
| `overlayPath`    | Tidak    | `string`   | Jalur bertitik di dalam konfigurasi akar yang objeknya harus melapisi objek akar sebelum sinyal dievaluasi. Gunakan ini untuk konfigurasi khusus kapabilitas seperti `image`, `video`, atau `music`. |
| `overlayMapPath` | Tidak    | `string`   | Jalur bertitik di dalam konfigurasi akar yang setiap nilai objeknya harus melapisi objek akar. Gunakan ini untuk peta akun bernama seperti `accounts`, saat akun apa pun yang dikonfigurasi harus memenuhi syarat. |
| `required`       | Tidak    | `string[]` | Jalur bertitik di dalam konfigurasi efektif yang harus memiliki nilai terkonfigurasi. String tidak boleh kosong; objek dan larik tidak boleh kosong.                                     |
...
| `providerBaseUrl` | Tidak    | `object` | Pembatas opsional yang membuat sinyal dihitung hanya ketika penyedia terkonfigurasi yang dirujuk menggunakan URL dasar yang diizinkan. Gunakan ini ketika alias autentikasi hanya valid untuk API tertentu. |
...
Entri `toolMetadata` juga menerima `optional` (menandai alat sebagai tidak wajib untuk aktivasi plugin) dan `replaySafe` (menandai eksekusi alat sebagai aman untuk diulangi setelah giliran model yang tidak lengkap), selain bidang bersama `configSignals`/`authSignals` di atas.

Jika alat tidak memiliki `toolMetadata`, OpenClaw mempertahankan perilaku yang ada dan memuat plugin pemilik ketika kontrak alat cocok dengan kebijakan. Untuk alat jalur panas yang pabriknya bergantung pada autentikasi/konfigurasi, pembuat plugin harus mendeklarasikan `toolMetadata` alih-alih membuat inti mengimpor runtime untuk menanyakannya.

## Referensi providerAuthChoices

Setiap entri `providerAuthChoices` menjelaskan satu pilihan orientasi awal atau autentikasi. OpenClaw membaca ini sebelum runtime penyedia dimuat. Daftar penyiapan penyedia menggunakan pilihan manifes ini, pilihan penyiapan yang diturunkan dari deskriptor, dan metadata katalog pemasangan tanpa memuat runtime penyedia.
...
| `choiceLabel`         | Tidak       | `string`                                                              | Label yang ditampilkan kepada pengguna. Jika dihilangkan, OpenClaw menggunakan `choiceId`.                                         |
| `choiceHint`          | Tidak       | `string`                                                              | Teks bantuan singkat untuk pemilih.                                                                         |
| `assistantPriority`   | Tidak       | `number`                                                              | Nilai yang lebih rendah diurutkan lebih awal dalam pemilih interaktif yang dikendalikan asisten.                                        |
...
| `cliOption`           | Tidak       | `string`                                                              | Bentuk lengkap opsi CLI, seperti `--openrouter-api-key <key>`.                                              |
| `cliDescription`      | Tidak       | `string`                                                              | Deskripsi yang digunakan dalam bantuan CLI.                                                                             |
| `appGuidedSecret`     | Tidak       | `boolean`                                                             | Satu rahasia yang ditempelkan beserta nilai default penyedia sudah cukup untuk penyiapan yang dipandu aplikasi.                              |
| `appGuidedDiscovery`  | Tidak       | `boolean`                                                             | Metode autentikasi runtime yang cocok memiliki penemuan lokal hanya-baca melalui `appGuidedSetup`.                 |
...
| `onboardingScopes`    | Tidak       | `Array<"text-inference" \| "image-generation" \| "music-generation">` | Menentukan pada permukaan orientasi awal mana pilihan ini harus muncul. Jika dihilangkan, nilai default-nya adalah `["text-inference"]`.  |

Ketika `appGuidedDiscovery` bernilai benar, metode autentikasi penyedia yang cocok harus mengekspos
`appGuidedSetup.detect` dan `appGuidedSetup.prepare`. Deteksi harus bersifat
hanya-baca: tanpa login, penarikan model, pengunduhan, atau penulisan konfigurasi. Persiapan memeriksa ulang
model yang dipilih secara tepat dan mengembalikan usulan konfigurasi; OpenClaw menguji langsung
...
Gunakan `activation` ketika plugin dapat menyatakan secara murah peristiwa bidang kontrol mana yang harus menyertakannya dalam rencana aktivasi/pemuatan.

Blok ini adalah metadata perencana, bukan API siklus hidup. Blok ini tidak mendaftarkan perilaku runtime, tidak menggantikan `register(...)`, dan tidak menjanjikan bahwa kode plugin sudah dijalankan. Perencana aktivasi menggunakan bidang-bidang ini untuk mempersempit kandidat plugin sebelum kembali menggunakan metadata kepemilikan manifes yang ada, seperti `providers`, `channels`, `commandAliases`, `setup.providers`, `contracts.tools`, dan hook.

Utamakan metadata paling sempit yang sudah menjelaskan kepemilikan. Gunakan `providers`, `channels`, `commandAliases`, deskriptor penyiapan, atau `contracts` ketika bidang-bidang tersebut menyatakan hubungan itu. Gunakan `activation` untuk petunjuk perencana tambahan yang tidak dapat direpresentasikan oleh bidang kepemilikan tersebut. Gunakan `cliBackends` tingkat atas untuk alias runtime CLI seperti `claude-cli`, `my-cli`, atau `google-gemini-cli`; `activation.onAgentHarnesses` hanya untuk ID harness agen tertanam yang belum memiliki bidang kepemilikan.

Setiap plugin harus menetapkan `activation.onStartup` secara sengaja. Tetapkan ke `true` hanya ketika plugin harus berjalan saat Gateway dimulai. Tetapkan ke `false` ketika plugin tidak aktif saat mulai dan hanya boleh dimuat dari pemicu yang lebih sempit. Menghilangkan `onStartup` tidak lagi memuat plugin secara implisit saat mulai; gunakan metadata aktivasi eksplisit untuk pemicu aktivasi saat mulai, saluran, konfigurasi, harness agen, memori, atau pemicu aktivasi lain yang lebih sempit.
...
| Bidang              | Wajib | Tipe                                                 | Artinya                                                                                                                                                                               |
| ------------------ | -------- | ---------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `onStartup`        | Tidak       | `boolean`                                            | Aktivasi eksplisit saat Gateway dimulai. Setiap plugin harus menetapkan ini. `true` mengimpor plugin saat mulai; `false` mempertahankannya agar dimuat secara tunda saat mulai, kecuali pemicu lain yang cocok mengharuskan pemuatan. |
...
| `onConfigPaths`    | Tidak       | `string[]`                                           | Jalur konfigurasi relatif terhadap root yang harus menyertakan plugin ini dalam rencana saat mulai/pemuatan ketika jalur tersedia dan tidak dinonaktifkan secara eksplisit.                                                      |
...
- Perencanaan startup runtime agen menggunakan `activation.onAgentHarnesses` untuk harness tertanam dan `cliBackends[]` tingkat teratas untuk alias runtime CLI.
- Perencanaan penyiapan/channel yang dipicu channel beralih ke kepemilikan `channels[]` lama sebagai fallback saat metadata aktivasi channel eksplisit tidak ada.
- Perencanaan Plugin startup menggunakan `activation.onConfigPaths` untuk permukaan konfigurasi root non-channel seperti blok `browser` milik Plugin browser terpaket.
- Perencanaan penyiapan/runtime yang dipicu penyedia beralih ke kepemilikan `providers[]` lama dan `cliBackends[]` tingkat teratas sebagai fallback saat metadata aktivasi penyedia eksplisit tidak ada.
...
Gunakan `qaRunners` saat sebuah Plugin menyediakan satu atau beberapa runner transportasi di bawah
root `openclaw qa` bersama. Jaga agar metadata ini ringan dan statis; runtime
Plugin tetap memiliki pendaftaran CLI yang sebenarnya melalui permukaan
`runtime-api.ts` ringan yang mengekspor `qaRunnerCliRegistrations` yang sesuai. `adapterFactory`
opsional mengekspos transportasi tersebut ke skenario QA bersama tanpa
...
```json
{
  "qaRunners": [
    {
      "commandName": "matrix",
      "description": "Jalankan jalur QA langsung Matrix yang didukung Docker terhadap homeserver sekali pakai"
    }
  ]
}
```
...
Gunakan `setup` saat permukaan penyiapan dan orientasi awal memerlukan metadata ringan milik Plugin sebelum runtime dimuat.
...
`cliBackends` tingkat teratas tetap valid dan terus mendeskripsikan backend inferensi CLI. `setup.cliBackends` adalah permukaan deskriptor khusus penyiapan untuk alur bidang kontrol/penyiapan yang harus tetap hanya berupa metadata.

Jika tersedia, `setup.providers` dan `setup.cliBackends` merupakan permukaan pencarian berbasis deskriptor yang diutamakan untuk penemuan penyiapan. Jika deskriptor hanya mempersempit kandidat Plugin dan penyiapan masih memerlukan hook runtime waktu penyiapan yang lebih lengkap, tetapkan `requiresRuntime: true` dan pertahankan `setup-api` sebagai jalur eksekusi fallback.

OpenClaw juga menyertakan `setup.providers[].envVars` dalam pencarian autentikasi penyedia dan variabel lingkungan generik. `providerAuthEnvVars` tetap didukung melalui adaptor kompatibilitas selama periode penghentian, tetapi Plugin yang tidak terpaket dan masih menggunakannya akan menerima diagnostik manifes. Plugin baru harus menempatkan metadata lingkungan penyiapan/status pada `setup.providers[].envVars`.

Gunakan `providerUsageAuthEnvVars` saat kredensial tingkat penagihan atau organisasi harus mengaktifkan `resolveUsageAuth` tanpa menjadi kredensial inferensi. Nama-nama ini disertakan dalam pemblokiran dotenv ruang kerja, penghapusan dari proses turunan ACP, pemfilteran rahasia sandbox, dan pembersihan rahasia secara luas. Runtime penyedia tetap membaca dan mengklasifikasikan nilai tersebut di dalam `resolveUsageAuth`.

OpenClaw juga dapat memperoleh pilihan penyiapan sederhana dari `setup.providers[].authMethods` saat entri penyiapan tidak tersedia, atau saat `setup.requiresRuntime: false` menyatakan runtime penyiapan tidak diperlukan. Entri `providerAuthChoices` eksplisit tetap diutamakan untuk label khusus, flag CLI, cakupan orientasi awal, dan metadata asisten.

Tetapkan `requiresRuntime: false` hanya jika deskriptor tersebut memadai untuk permukaan penyiapan. OpenClaw memperlakukan `false` eksplisit sebagai kontrak khusus deskriptor dan tidak akan mengeksekusi `setup-api` atau `openclaw.setupEntry` untuk pencarian penyiapan. Jika Plugin khusus deskriptor masih menyertakan salah satu entri runtime penyiapan tersebut, OpenClaw melaporkan diagnostik tambahan dan tetap mengabaikannya. Jika `requiresRuntime` tidak dicantumkan, perilaku fallback lama dipertahankan agar Plugin yang sudah ada dan menambahkan deskriptor tanpa flag tersebut tidak rusak.

Karena pencarian penyiapan dapat mengeksekusi kode `setup-api` milik Plugin, nilai `setup.providers[].id` dan `setup.cliBackends[]` yang telah dinormalisasi harus tetap unik di seluruh Plugin yang ditemukan. Kepemilikan ambigu akan gagal secara tertutup alih-alih memilih pemenang berdasarkan urutan penemuan.

Saat runtime penyiapan benar-benar dieksekusi, diagnostik registri penyiapan melaporkan penyimpangan deskriptor jika `setup-api` mendaftarkan penyedia atau backend CLI yang tidak dideklarasikan oleh deskriptor manifes, atau jika deskriptor tidak memiliki pendaftaran runtime yang sesuai. Diagnostik ini bersifat tambahan dan tidak menolak Plugin lama.

### Referensi setup.providers

| Bidang          | Wajib | Tipe       | Artinya                                                                                    |
| -------------- | -------- | ---------- | ------------------------------------------------------------------------------------------------ |
| `id`           | Ya      | `string`   | ID penyedia yang diekspos selama penyiapan atau orientasi awal. Pastikan ID yang dinormalisasi unik secara global.             |
...
| `authEvidence` | Tidak       | `object[]` | Pemeriksaan bukti autentikasi lokal ringan bagi penyedia yang dapat mengautentikasi melalui penanda nonrahasia. |
...
| `credentialMarker` | Ya      | `string`   | Penanda nonrahasia yang dikembalikan saat bukti tersedia.                                                       |
...
| `configMigrations` | Tidak       | `string[]` | ID migrasi konfigurasi yang dimiliki permukaan penyiapan Plugin ini.                                          |
| `requiresRuntime`  | Tidak       | `boolean`  | Apakah penyiapan masih memerlukan eksekusi `setup-api` setelah pencarian deskriptor.                            |

## Referensi uiHints

`uiHints` adalah pemetaan dari nama bidang konfigurasi ke petunjuk rendering kecil. Kunci dapat menggunakan titik untuk bidang konfigurasi bertingkat, tetapi tidak ada segmen path yang boleh berupa `__proto__`, `constructor`, atau `prototype`; penyiapan menolak nama-nama tersebut.
...
| `agentToolResultMiddleware`      | `string[]` | ID runtime yang dapat digunakan Plugin ini untuk mendaftarkan middleware hasil alat.                                                                     |
...
| `documentExtractors`             | `string[]` | ID penyedia pengekstrak dokumen (misalnya PDF) yang dimiliki Plugin ini.                                                                  |
...
`contracts.embeddedExtensionFactories` dipertahankan untuk factory ekstensi khusus app-server Codex bawaan. Transformasi hasil alat bawaan sebaiknya mendeklarasikan `contracts.agentToolResultMiddleware` dan mendaftar dengan `api.registerAgentToolResultMiddleware(...)` sebagai gantinya. Plugin terinstal hanya dapat menggunakan seam middleware yang sama jika diaktifkan secara eksplisit dan hanya untuk runtime yang dideklarasikan dalam `contracts.agentToolResultMiddleware`.

Plugin terinstal yang memerlukan tingkat kebijakan pra-alat tepercaya host harus mendeklarasikan setiap ID lokal yang didaftarkan dalam `contracts.trustedToolPolicies` dan diaktifkan secara eksplisit. Plugin bawaan mempertahankan jalur kebijakan tepercaya yang ada, tetapi Plugin terinstal dengan ID kebijakan yang tidak dideklarasikan akan ditolak sebelum pendaftaran. ID kebijakan memiliki cakupan pada Plugin yang mendaftarkannya, sehingga dua Plugin dapat sama-sama mendeklarasikan dan mendaftarkan `workflow-budget`; satu Plugin tidak boleh mendaftarkan ID lokal yang sama dua kali.

Pendaftaran `api.registerTool(...)` runtime harus cocok dengan `contracts.tools`. Penemuan alat menggunakan daftar ini agar hanya memuat runtime Plugin yang dapat memiliki alat yang diminta.
...
Plugin penyedia yang mengimplementasikan `resolveUsageAuth` dan `fetchUsageSnapshot` harus mendeklarasikan setiap ID penyedia yang ditemukan otomatis dalam `contracts.usageProviders`. Penemuan penggunaan membaca kontrak ini sebelum memuat kode runtime, lalu memverifikasi kedua hook setelah hanya memuat pemilik yang dideklarasikan.

Penyedia embedding umum sebaiknya mendeklarasikan `contracts.embeddingProviders` untuk setiap adaptor yang didaftarkan dengan `api.registerEmbeddingProvider(...)`. Gunakan kontrak umum untuk pembuatan vektor yang dapat digunakan kembali, termasuk penyedia yang digunakan oleh pencarian memori. `contracts.memoryEmbeddingProviders` adalah kompatibilitas khusus memori yang tidak digunakan lagi dan tetap tersedia hanya selama penyedia yang ada bermigrasi ke seam penyedia embedding generik.

Penyedia pekerja harus mendeklarasikan setiap ID `api.registerWorkerProvider(...)` dalam `contracts.workerProviders`. Inti mempertahankan intent secara persisten sebelum memanggil `provision`; penyedia memvalidasi pengaturannya sebelum alokasi eksternal, dan panggilan berulang dengan ID operasi yang sama harus mengadopsi sewa yang sama. Inti juga mempertahankan snapshot pengaturan yang telah divalidasi tersebut dan meneruskannya bersama `leaseId` ke `inspect({ leaseId, profile })` dan `destroy({ leaseId, profile })`, termasuk setelah profil bernama diubah atau dihapus. Penghancuran bersifat idempoten, inspeksi mengembalikan union status tertutup `active` / `destroyed` / `unknown`, dan materi kunci privat SSH hanya direferensikan melalui `SecretRef`. Endpoint SSH yang disediakan juga harus menyertakan `hostKey` publik dari keluaran penyediaan tepercaya sebagai persis `algorithm base64`, tanpa nama host atau komentar, sehingga inti dapat menyematkan host sebelum terhubung. Penyedia yang membuat referensi identitas dinamis dapat mengimplementasikan `resolveSshIdentity({ leaseId, profile, keyRef })` otoritatif; penyedia tanpanya menggunakan resolver rahasia generik milik inti. `unknown` otoritatif membuat rekaman lokal aktif menjadi yatim; setelah permintaan penghancuran dipertahankan, rekaman tersebut mengonfirmasi pembongkaran.

`contracts.gatewayMethodDispatch` saat ini menerima `"authenticated-request"`. Ini adalah gerbang kebersihan API untuk rute HTTP Plugin native yang secara sengaja mengirimkan metode bidang kontrol Gateway dalam proses, bukan sandbox terhadap Plugin native berbahaya. Gunakan hanya untuk permukaan bawaan/operator yang ditinjau ketat dan telah mewajibkan autentikasi HTTP Gateway. Rute yang memiliki hak tetap dapat dijangkau saat penerimaan pekerjaan root Gateway ditutup hanya jika rute tersebut juga mendeklarasikan `auth: "gateway"` dan `gatewayRuntimeScopeSurface: "trusted-operator"` khusus rute; rute saudara biasa dari Plugin yang sama tetap berada di balik batas penerimaan. Hal ini menjaga status penangguhan dan tindakan melanjutkan tetap dapat dijangkau tanpa memberikan bypass penerimaan kepada seluruh Plugin. Jaga penguraian dan pembentukan respons tetap terbatas di luar pengiriman; pekerjaan substantif atau yang mengubah harus melalui pengiriman metode Gateway, yang memiliki penerapan penerimaan dan cakupan.

## Referensi configContracts

Gunakan `configContracts` untuk perilaku konfigurasi milik manifes yang diperlukan helper inti generik tanpa mengimpor runtime Plugin: deteksi flag berbahaya, target migrasi SecretRef, dan penyempitan jalur konfigurasi lama.
...
| Bidang                         | Wajib | Jenis       | Artinya                                                                                                                                                                                                                          |
| ----------------------------- | -------- | ---------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `compatibilityMigrationPaths` | Tidak       | `string[]` | Jalur konfigurasi relatif root yang menunjukkan bahwa migrasi kompatibilitas waktu penyiapan Plugin ini mungkin berlaku. Memungkinkan pembacaan konfigurasi runtime generik melewati setiap permukaan penyiapan Plugin jika konfigurasi tidak pernah mereferensikan Plugin tersebut.                 |
| `compatibilityRuntimePaths`   | Tidak       | `string[]` | Jalur kompatibilitas relatif root yang dapat dilayani Plugin ini selama runtime sebelum kode Plugin aktif sepenuhnya. Gunakan ini untuk permukaan lama yang sebaiknya mempersempit kumpulan kandidat bawaan tanpa mengimpor setiap runtime Plugin yang kompatibel. |
| `dangerousFlags`              | Tidak       | `object[]` | Literal konfigurasi yang harus ditandai `openclaw doctor` sebagai tidak aman atau berbahaya saat diaktifkan. Lihat di bawah.                                                                                                                                   |
| `secretInputs`                | Tidak       | `object`   | Jalur konfigurasi di bawah `plugins.entries.<id>.config` yang harus diperlakukan oleh registri target migrasi/audit SecretRef sebagai string berbentuk rahasia. Lihat di bawah.                                                                                  |
...
| Bidang                   | Wajib | Tipe       | Artinya                                                                                                                                                                                                   |
| ----------------------- | -------- | ---------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `bundledDefaultEnabled` | Tidak       | `boolean`  | Ganti pengaktifan default plugin bawaan saat menentukan apakah permukaan SecretRef ini aktif. Gunakan ini ketika plugin disertakan sebagai bawaan, tetapi permukaan harus tetap tidak aktif hingga diaktifkan secara eksplisit dalam konfigurasi. |
...
Gunakan `channelConfigs` ketika plugin saluran memerlukan metadata konfigurasi yang ringan sebelum runtime dimuat. Penemuan penyiapan/status saluran hanya-baca dapat menggunakan metadata ini secara langsung untuk saluran eksternal yang dikonfigurasi ketika tidak tersedia entri penyiapan, atau ketika `setup.requiresRuntime: false` menyatakan bahwa runtime penyiapan tidak diperlukan.

`channelConfigs` adalah metadata manifes plugin, bukan bagian konfigurasi pengguna tingkat atas yang baru. Pengguna tetap mengonfigurasi instans saluran di bawah `channels.<channel-id>`. OpenClaw membaca metadata manifes untuk menentukan plugin mana yang memiliki saluran terkonfigurasi tersebut sebelum kode runtime plugin dijalankan.
...
| `commands`    | `object`                 | Default otomatis statis untuk perintah native dan Skills native bagi pemeriksaan konfigurasi pra-runtime.       |
...
Gunakan `preferOver` ketika plugin Anda adalah pemilik yang diprioritaskan untuk ID saluran yang juga dapat disediakan oleh plugin lain. Kasus umum meliputi ID plugin yang diubah namanya, plugin mandiri yang menggantikan plugin bawaan, atau fork yang dipelihara dan mempertahankan ID saluran yang sama demi kompatibilitas konfigurasi.
...
Ketika `channels.chat` dikonfigurasi, OpenClaw mempertimbangkan ID saluran dan ID plugin yang diprioritaskan. Jika plugin berprioritas lebih rendah dipilih hanya karena disertakan sebagai bawaan atau diaktifkan secara default, OpenClaw menonaktifkannya dalam konfigurasi runtime efektif agar satu plugin memiliki saluran tersebut beserta alatnya. Pemilihan eksplisit pengguna tetap diutamakan: jika pengguna secara eksplisit mengaktifkan kedua plugin (melalui `plugins.allow` atau konfigurasi `plugins.entries` yang substantif), OpenClaw mempertahankan pilihan tersebut dan melaporkan diagnostik duplikasi saluran/alat alih-alih secara diam-diam mengubah kumpulan plugin yang diminta.

Batasi cakupan `preferOver` hanya pada ID plugin yang benar-benar dapat menyediakan saluran yang sama. Ini bukan bidang prioritas umum dan tidak mengganti nama kunci konfigurasi pengguna.
...
- referensi `provider/model` eksplisit menggunakan metadata manifes `providers` pemiliknya
- `modelPatterns` mengalahkan `modelPrefixes`
- jika satu plugin nonbawaan dan satu plugin bawaan sama-sama cocok, plugin nonbawaan yang menang
...
Entri `modelPatterns` dikompilasi melalui `compileSafeRegex`, yang menolak pola berisi pengulangan bertingkat (misalnya `(a+)+$`). Pola yang gagal dalam pemeriksaan keamanan dilewati secara diam-diam, sama seperti regex yang secara sintaksis tidak valid. Buat pola tetap sederhana dan hindari kuantifier bertingkat.
...
| Bidang            | Tipe                                                     | Artinya                                                                                               |
| ---------------- | -------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------- |
| `providers`      | `Record<string, object>`                                 | Baris katalog untuk id penyedia yang dimiliki oleh plugin ini. Kunci juga harus muncul dalam `providers` tingkat teratas.       |
...
| `discovery`      | `Record<string, "static" \| "refreshable" \| "runtime">` | Apakah katalog penyedia dapat dibaca dari metadata manifes, disegarkan ke dalam cache, atau memerlukan runtime. |
| `runtimeAugment` | `boolean`                                                | Tetapkan ke `true` hanya ketika runtime penyedia harus menambahkan baris katalog setelah perencanaan manifes/konfigurasi.       |
...
| `defaultUtilityModel` | `string`                 | Id model kecil opsional yang direkomendasikan penyedia untuk tugas utilitas internal singkat (judul, narasi kemajuan). Digunakan ketika `agents.defaults.utilityModel` tidak ditetapkan dan penyedia ini melayani model utama agen. |
...
| `input`            | `Array<"text" \| "image" \| "document">`                       | Modalitas yang diterima model. Nilai lain dibuang secara diam-diam.            |
...
| `thinkingLevelMap` | `Record<string, string \| null>`                               | Penggantian id model atau parameter per tingkat pemikiran opsional.                    |
| `cost`             | `object`                                                       | Harga USD opsional per satu juta token, termasuk `tieredPricing` opsional. |
...
| `status`           | `"available"` \| `"preview"` \| `"deprecated"` \| `"disabled"` | Status daftar. Lakukan supresi hanya ketika baris sama sekali tidak boleh muncul.          |
...
| `when.baseUrlHosts`        | `string[]` | Daftar opsional host URL dasar penyedia efektif yang diperlukan sebelum supresi diterapkan.               |
| `when.providerConfigApiIn` | `string[]` | Daftar opsional nilai `api` konfigurasi penyedia yang persis sama dan diperlukan sebelum supresi diterapkan.              |

Jangan masukkan data khusus runtime ke dalam `modelCatalog`. Gunakan `static` hanya ketika baris manifes cukup lengkap agar daftar yang difilter berdasarkan penyedia dan permukaan pemilih dapat melewati penemuan registri/runtime. Gunakan `refreshable` ketika baris manifes merupakan benih atau pelengkap yang berguna dan dapat didaftarkan, tetapi penyegaran/cache dapat menambahkan lebih banyak baris nanti; baris yang dapat disegarkan tidak bersifat otoritatif dengan sendirinya. Gunakan `runtime` ketika OpenClaw harus memuat runtime penyedia untuk mengetahui daftarnya.

## Referensi modelIdNormalization

Gunakan `modelIdNormalization` untuk pembersihan id model milik penyedia yang ringan dan harus dilakukan sebelum runtime penyedia dimuat. Ini menjaga alias seperti nama model pendek, id lama lokal penyedia, dan aturan prefiks proksi tetap berada dalam manifes plugin pemilik, bukan dalam tabel pemilihan model inti.
...
Gunakan `providerRequest` untuk metadata kompatibilitas permintaan berbiaya rendah yang diperlukan oleh kebijakan permintaan generik tanpa memuat runtime penyedia. Pertahankan penulisan ulang muatan khusus perilaku dalam hook runtime penyedia atau helper bersama keluarga penyedia.
...
| `compatibilityFamily` | `"moonshot"` | Kelompok kompatibilitas keluarga penyedia opsional untuk helper permintaan bersama.              |
...
Gunakan `secretProviderIntegrations` saat sebuah plugin dapat menerbitkan preset penyedia exec SecretRef yang dapat digunakan kembali. OpenClaw membaca metadata ini sebelum runtime plugin dimuat, menyimpan kepemilikan plugin dalam `secrets.providers.<alias>.pluginIntegration`, dan menyerahkan resolusi rahasia aktual kepada runtime SecretRef. Preset hanya diekspos untuk plugin bawaan dan plugin terinstal yang ditemukan dari root instalasi plugin terkelola, seperti instalasi git dan ClawHub.
...
Saat dimulai/dimuat ulang, OpenClaw menyelesaikan penyedia tersebut dengan memuat metadata manifes plugin saat ini, memeriksa bahwa plugin pemilik telah terinstal dan aktif, serta mewujudkan perintah exec dari manifes. Menonaktifkan atau menghapus plugin akan mencabut penyedia untuk SecretRef aktif. Operator yang menginginkan konfigurasi exec mandiri tetap dapat menulis penyedia manual `command`/`args` secara langsung.

Saat ini hanya preset `source: "exec"` yang didukung. `command` harus berupa `${node}`, dan `args[0]` harus berupa skrip resolver `./` yang relatif terhadap root plugin. OpenClaw mewujudkannya saat dimulai/dimuat ulang menjadi executable Node saat ini dan jalur skrip absolut di dalam plugin. Opsi Node seperti `--require`, `--import`, `--loader`, `--env-file`, `--eval`, dan `--print` bukan bagian dari kontrak preset manifes. Operator yang memerlukan perintah non-Node dapat mengonfigurasi penyedia exec manual mandiri secara langsung.

OpenClaw memperoleh `trustedDirs` untuk preset manifes dari root plugin dan, untuk preset `${node}`, direktori executable Node saat ini. `trustedDirs` yang ditulis dalam manifes akan diabaikan. Opsi penyedia exec lainnya seperti `timeoutMs`, `noOutputTimeoutMs`, `maxOutputBytes`, `jsonOnly`, `env`, `passEnv`, dan `allowInsecurePath` diteruskan ke konfigurasi penyedia exec SecretRef normal.

## Referensi modelPricing

Gunakan `modelPricing` saat penyedia memerlukan perilaku harga bidang kontrol sebelum runtime dimuat. Cache harga Gateway membaca metadata ini tanpa mengimpor kode runtime penyedia.
...
| Bidang        | Tipe              | Artinya                                                                                      |
| ------------ | ----------------- | -------------------------------------------------------------------------------------------------- |
| `external`   | `boolean`         | Atur `false` untuk penyedia lokal/dihosting sendiri yang tidak boleh mengambil harga OpenRouter atau LiteLLM. |
...
Indeks Penyedia OpenClaw adalah metadata pratinjau milik OpenClaw untuk penyedia yang pluginnya mungkin belum terinstal. Ini bukan bagian dari manifes plugin. Manifes plugin tetap menjadi otoritas plugin terinstal. Indeks Penyedia adalah kontrak fallback internal yang akan digunakan oleh antarmuka penyedia yang dapat diinstal dan pemilih model pra-instalasi mendatang saat plugin penyedia belum terinstal.
...
Indeks Penyedia tidak boleh memuat rahasia, status aktif, hook runtime, atau data model langsung khusus akun. Katalog pratinjaunya menggunakan bentuk baris penyedia `modelCatalog` yang sama dengan manifes plugin, tetapi harus tetap terbatas pada metadata tampilan yang stabil kecuali bidang adaptor runtime seperti `api`, `baseUrl`, harga, atau flag kompatibilitas sengaja diselaraskan dengan manifes plugin terinstal. Penyedia dengan penemuan `/models` langsung harus menulis baris yang disegarkan melalui jalur cache katalog model eksplisit, alih-alih membuat pencantuman normal atau orientasi awal memanggil API penyedia.

Entri Indeks Penyedia juga dapat membawa metadata plugin yang dapat diinstal untuk penyedia yang pluginnya telah dipindahkan dari inti atau belum terinstal. Metadata ini mencerminkan pola katalog saluran: nama paket, spesifikasi instalasi npm, integritas yang diharapkan, dan label pilihan autentikasi berbiaya rendah sudah cukup untuk menampilkan opsi penyiapan yang dapat diinstal. Setelah plugin terinstal, manifesnya diutamakan dan entri Indeks Penyedia diabaikan untuk penyedia tersebut.

`openclaw doctor --fix` memigrasikan sekumpulan kecil tertutup kunci kapabilitas manifes tingkat atas lama ke dalam `contracts.*`: `speechProviders`, `mediaUnderstandingProviders`, `imageGenerationProviders`, dan `tools`. Tidak satu pun dari kunci tersebut (atau daftar kapabilitas lainnya) dibaca lagi sebagai bidang manifes tingkat atas; pemuatan manifes normal hanya mengenalinya di bawah `contracts`.

## Manifes versus package.json

Kedua berkas tersebut menjalankan fungsi yang berbeda:
...
Jika tidak yakin di mana suatu metadata seharusnya berada, gunakan aturan ini:
...
| `openclaw.setupEntry`                                                                      | Titik masuk ringan khusus penyiapan yang digunakan selama orientasi awal, penundaan startup saluran, serta status saluran hanya-baca/penemuan SecretRef. Harus tetap berada di dalam direktori paket Plugin. |
...
| `openclaw.channel`                                                                         | Metadata katalog saluran ringan seperti label, jalur dokumentasi, alias, dan teks pilihan.                                                                                           |
| `openclaw.channel.commands`                                                                | Metadata default otomatis statis untuk perintah native dan skill native yang digunakan oleh permukaan konfigurasi, audit, dan daftar perintah sebelum runtime saluran dimuat.        |
| `openclaw.channel.configuredState`                                                         | Metadata pemeriksa status terkonfigurasi ringan yang dapat menjawab "apakah penyiapan khusus lingkungan sudah ada?" tanpa memuat runtime saluran lengkap.                             |
| `openclaw.channel.persistedAuthState`                                                      | Metadata pemeriksa autentikasi tersimpan ringan yang dapat menjawab "apakah sudah ada akun yang masuk?" tanpa memuat runtime saluran lengkap.                                         |
...
| `openclaw.install.expectedIntegrity`                                                       | String integritas dist npm yang diharapkan seperti `sha512-...`; alur instalasi dan pembaruan memverifikasi artefak yang diambil terhadapnya.                                  |
...
| `openclaw.install.requiredPlatformPackages`                                                | Alias paket npm yang harus diwujudkan ketika batasan platform lockfile-nya cocok dengan host saat ini.                                                                               |
| `openclaw.startup.deferConfiguredChannelFullLoadUntilAfterListen`                          | Memungkinkan permukaan saluran runtime penyiapan dimuat sebelum mulai mendengarkan, lalu menunda Plugin saluran terkonfigurasi lengkap hingga aktivasi setelah mulai mendengarkan.    |

Metadata manifes menentukan pilihan penyedia/saluran/penyiapan yang muncul dalam orientasi awal sebelum runtime dimuat. `package.json#openclaw.install` memberi tahu orientasi awal cara mengambil atau mengaktifkan Plugin tersebut ketika pengguna memilih salah satu pilihan itu. Jangan pindahkan petunjuk instalasi ke `openclaw.plugin.json`.

`openclaw.install.minHostVersion` diberlakukan selama instalasi dan pemuatan registri manifes untuk sumber Plugin nonbawaan. Nilai yang tidak valid ditolak; nilai yang lebih baru tetapi valid menyebabkan Plugin eksternal dilewati pada host yang lebih lama. Plugin sumber bawaan diasumsikan memiliki versi yang selaras dengan checkout host.

`openclaw.install.requiredPlatformPackages` ditujukan bagi paket npm yang mengekspos biner native wajib melalui alias opsional khusus platform. Cantumkan nama paket npm polos untuk setiap alias platform yang didukung. Selama instalasi npm, OpenClaw hanya memverifikasi alias yang dideklarasikan dan batasan lockfile-nya cocok dengan host saat ini. Jika npm melaporkan keberhasilan tetapi menghilangkan alias tersebut, OpenClaw mencoba lagi satu kali dengan cache baru dan membatalkan instalasi jika alias tersebut masih tidak ada.

`openclaw.compat.pluginApi` diberlakukan selama instalasi paket untuk sumber Plugin nonbawaan. Gunakan ini untuk batas bawah API runtime/SDK Plugin OpenClaw yang menjadi dasar pembuatan paket. Nilainya dapat lebih ketat daripada `minHostVersion` ketika paket Plugin memerlukan API yang lebih baru tetapi tetap mempertahankan petunjuk instalasi yang lebih rendah untuk alur lain. Sinkronisasi rilis resmi OpenClaw secara default menaikkan batas bawah API Plugin resmi yang ada ke versi rilis OpenClaw, tetapi rilis khusus Plugin dapat mempertahankan batas bawah yang lebih rendah ketika paket sengaja mendukung host yang lebih lama. Jangan gunakan versi paket saja sebagai kontrak kompatibilitas. `peerDependencies.openclaw` tetap merupakan metadata paket npm; OpenClaw menggunakan kontrak `openclaw.compat.pluginApi` untuk keputusan kompatibilitas instalasi.
...
Penyematan versi npm yang persis sudah berada di `npmSpec`, misalnya `"npmSpec": "@wecom/wecom-openclaw-plugin@1.2.3"`. Entri katalog eksternal resmi harus memasangkan spesifikasi persis dengan `expectedIntegrity` agar alur pembaruan gagal secara tertutup jika artefak npm yang diambil tidak lagi cocok dengan rilis yang disematkan. Orientasi awal interaktif tetap menawarkan spesifikasi npm registri tepercaya, termasuk nama paket polos dan dist-tag, demi kompatibilitas. Diagnostik katalog dapat membedakan sumber pilihan default yang persis, mengambang, disematkan integritasnya, tidak memiliki integritas, nama paketnya tidak cocok, dan tidak valid. Diagnostik juga memperingatkan ketika `expectedIntegrity` tersedia tetapi tidak ada sumber npm valid yang dapat disematkannya. Ketika `expectedIntegrity` tersedia, alur instalasi/pembaruan memberlakukannya; ketika dihilangkan, resolusi registri dicatat tanpa penyematan integritas.

Plugin saluran harus menyediakan `openclaw.setupEntry` ketika pemindaian status, daftar saluran, atau SecretRef perlu mengidentifikasi akun yang dikonfigurasi tanpa memuat runtime lengkap. Entri penyiapan harus mengekspos metadata saluran beserta adaptor konfigurasi, status, dan rahasia yang aman untuk penyiapan; pertahankan klien jaringan, listener Gateway, dan runtime transportasi di titik masuk ekstensi utama.
...
`openclaw.install.allowInvalidConfigRecovery` sengaja dibuat terbatas. Ini tidak membuat konfigurasi rusak apa pun dapat diinstal. Saat ini, ini hanya memungkinkan alur instalasi pulih dari kegagalan peningkatan Plugin bawaan kedaluwarsa tertentu, seperti jalur Plugin bawaan yang hilang atau entri `channels.<id>` kedaluwarsa untuk Plugin bawaan yang sama. Kesalahan konfigurasi yang tidak terkait tetap memblokir instalasi dan mengarahkan operator ke `openclaw doctor --fix`.
...
Gunakan ini ketika alur penyiapan, doctor, status, atau pemeriksaan keberadaan hanya-baca memerlukan pemeriksaan autentikasi ya/tidak yang ringan sebelum Plugin saluran lengkap dimuat. Status autentikasi tersimpan bukanlah status saluran terkonfigurasi: jangan gunakan metadata ini untuk mengaktifkan Plugin secara otomatis, memperbaiki dependensi runtime, atau memutuskan apakah runtime saluran harus dimuat. Ekspor target harus berupa fungsi kecil yang hanya membaca status tersimpan; jangan merutekannya melalui barrel runtime saluran lengkap.
...
Jika dua hasil penemuan memiliki `id` yang sama, hanya manifes dengan **prioritas tertinggi** yang dipertahankan; duplikat dengan prioritas lebih rendah dibuang alih-alih dimuat bersamanya. Prioritas, dari tertinggi hingga terendah:
...
- Salinan fork atau kedaluwarsa dari Plugin bawaan yang berada tanpa dilacak di ruang kerja atau root global tidak akan membayangi build bawaan.
- Untuk menimpa Plugin bawaan, jalankan `openclaw plugin install` untuk ID tersebut agar instalasi global terlacak mengungguli salinan bawaan, atau sematkan jalur tertentu melalui `plugins.entries.<id>` agar menang berdasarkan prioritas dipilih konfigurasi.
- Pembuangan duplikat dicatat agar Doctor dan diagnostik startup dapat menunjukkan salinan yang dibuang.
- Penimpaan duplikat yang dipilih konfigurasi disebut sebagai penimpaan eksplisit dalam diagnostik, tetapi tetap menghasilkan peringatan agar fork kedaluwarsa dan pembayangan yang tidak disengaja tetap terlihat.
...
- Kunci `channels.*` yang tidak dikenal merupakan **kesalahan**, kecuali jika id saluran dideklarasikan oleh manifes plugin. Jika id yang sama juga muncul di `plugins.allow`, `plugins.entries`, atau `plugins.installs` (plugin yang dirujuk tetapi saat ini tidak dapat ditemukan), OpenClaw menurunkannya menjadi **peringatan**.
- `plugins.entries.<id>`, `plugins.allow`, dan `plugins.deny` yang merujuk pada id plugin yang tidak dikenal merupakan **peringatan** ("entri konfigurasi usang diabaikan"), bukan kesalahan, sehingga peningkatan versi serta plugin yang dihapus/diubah namanya tidak menghalangi Gateway untuk dimulai.
- `plugins.slots.memory` yang merujuk pada id plugin yang tidak dikenal merupakan **kesalahan**, kecuali untuk plugin eksternal resmi `memory-lancedb` yang dikenal, yang menghasilkan peringatan.
- Jika plugin terinstal tetapi manifes atau skemanya rusak atau tidak ada, validasi gagal dan Doctor melaporkan kesalahan plugin.
- Jika konfigurasi plugin tersedia tetapi plugin **dinonaktifkan**, konfigurasi tetap disimpan dan **peringatan** ditampilkan di Doctor + log.

Lihat [Referensi konfigurasi](/id/gateway/configuration) untuk skema `plugins.*` lengkap.

## Catatan

- Manifes **diperlukan untuk plugin OpenClaw native**, termasuk pemuatan dari sistem berkas lokal. Runtime tetap memuat modul plugin secara terpisah; manifes hanya digunakan untuk penemuan + validasi.
...
- Hanya bidang manifes yang terdokumentasi yang dibaca oleh pemuat manifes. Hindari kunci tingkat teratas khusus.
- `channels`, `providers`, `cliBackends`, dan `skills` semuanya dapat dihilangkan jika plugin tidak memerlukannya.
- `providerCatalogEntry` harus tetap ringan dan tidak boleh mengimpor kode runtime yang luas; gunakan untuk metadata katalog penyedia statis atau deskriptor penemuan yang terbatas, bukan eksekusi pada saat permintaan.
...
- Deklarasikan jenis plugin eksklusif dalam manifes ini. `OpenClawPluginDefinition.kind` entri runtime sudah tidak digunakan lagi dan tetap tersedia hanya sebagai fallback kompatibilitas untuk plugin lama.
- Metadata variabel lingkungan (`setup.providers[].envVars`, `providerAuthEnvVars` yang sudah tidak digunakan lagi, dan `channelEnvVars`) hanya bersifat deklaratif. Status, audit, validasi pengiriman Cron, dan permukaan hanya-baca lainnya tetap menerapkan kebijakan kepercayaan plugin dan aktivasi efektif sebelum menganggap variabel lingkungan telah dikonfigurasi.
...
- Jika plugin Anda bergantung pada modul native, dokumentasikan langkah-langkah build dan persyaratan daftar izin pengelola paket apa pun (misalnya, pnpm `allow-build-scripts` + `pnpm rebuild <package>`).

## Terkait

<CardGroup cols={3}>
  <Card title="Membuat plugin" href="/id/plugins/building-plugins" icon="rocket">
