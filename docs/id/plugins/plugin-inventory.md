...
read_when:
    ...
    - Anda memerlukan daftar kanonis plugin internal vs eksternal
summary: Inventaris Plugin OpenClaw yang dihasilkan, disertakan dalam inti, dipublikasikan secara eksternal, atau hanya tersedia sebagai kode sumber
title: Inventaris Plugin
x-i18n:
    generated_at: "2026-07-22T01:26:06Z"
    model: gpt-5.6
...
dan pengecualian paket npm utama `files`. Buat ulang dengan:
...
- **Paket npm inti:** disertakan dalam paket npm `openclaw` dan tersedia tanpa instalasi Plugin terpisah.
- **Paket eksternal resmi:** Plugin yang dipelihara OpenClaw, tidak disertakan dalam paket npm inti, tercantum dalam inventaris resmi ini, dan diinstal sesuai kebutuhan melalui ClawHub dan/atau npm.
- **Khusus checkout sumber:** Plugin lokal repo yang tidak disertakan dalam artefak npm yang dipublikasikan dan tidak ditawarkan sebagai paket yang dapat diinstal.

Checkout sumber berbeda dari instalasi npm: setelah `pnpm install`, Plugin
bawaan dimuat dari `extensions/<id>` sehingga pengeditan lokal dan dependensi
workspace lokal paket tersedia.

## Menginstal Plugin

Gunakan jalur instalasi pada setiap entri untuk menentukan apakah instalasi diperlukan. Plugin
...
Sebagai contoh, Discord adalah paket eksternal resmi:
...
Selama peralihan peluncuran, spesifikasi paket biasa tanpa kualifikasi tetap diinstal dari npm.
Gunakan `clawhub:@openclaw/discord` atau `npm:@openclaw/discord` jika Anda memerlukan
...
- **[azure-speech](/id/plugins/reference/azure-speech)** (`@openclaw/azure-speech`) - disertakan dalam OpenClaw. Teks-ke-ucapan Azure AI Speech (MP3, catatan suara Ogg/Opus native, telefoni PCM).
...
- **[crabbox](/id/plugins/reference/crabbox)** (`@openclaw/crabbox-provider`) - disertakan dalam OpenClaw. Penyedia pekerja cloud yang didukung oleh CLI Crabbox.

- **[cua-computer](/plugins/reference/cua-computer)** (`@openclaw/cua-computer`) - disertakan dalam OpenClaw. Kontrol komputer cua-driver eksperimental untuk host node Windows dan Linux.
...
- **[linux-canvas](/id/plugins/reference/linux-canvas)** (`@openclaw/linux-canvas`) - disertakan dalam OpenClaw. Jembatan rendering Canvas untuk aplikasi desktop OpenClaw Linux.
...
- **[logbook](/id/plugins/reference/logbook)** (`@openclaw/logbook`) - disertakan dalam OpenClaw. Jurnal kerja otomatis: mengambil tangkapan layar berkala dari node yang dipasangkan dan mengubahnya menjadi linimasa aktivitas harian Anda yang dapat ditinjau.
...
- **[memory-wiki](/id/plugins/reference/memory-wiki)** (`@openclaw/memory-wiki`) - disertakan dalam OpenClaw. Kompilator wiki persisten dan brankas pengetahuan yang kompatibel dengan Obsidian untuk OpenClaw.
...
- **[migrate-claude](/id/plugins/reference/migrate-claude)** (`@openclaw/migrate-claude`) - disertakan dalam OpenClaw. Mengimpor instruksi Claude Code dan Claude Desktop, server MCP, keterampilan, serta konfigurasi aman ke OpenClaw.

- **[migrate-hermes](/id/plugins/reference/migrate-hermes)** (`@openclaw/migrate-hermes`) - disertakan dalam OpenClaw. Mengimpor konfigurasi, memori, keterampilan, dan kredensial yang didukung dari Hermes ke OpenClaw.
...
- **[oc-path](/id/plugins/reference/oc-path)** (`@openclaw/oc-path`) - disertakan dalam OpenClaw. Menambahkan CLI jalur openclaw untuk pengalamatan file workspace melalui oc://.
...
- **[open-prose](/id/plugins/reference/open-prose)** (`@openclaw/open-prose`) - disertakan dalam OpenClaw. Paket keterampilan VM OpenProse dengan perintah garis miring /prose.
...
- **[policy](/id/plugins/reference/policy)** (`@openclaw/policy`) - disertakan dalam OpenClaw. Menambahkan pemeriksaan doctor berbasis kebijakan untuk kepatuhan workspace.
...
- **[workboard](/id/plugins/reference/workboard)** (`@openclaw/workboard`) - disertakan dalam OpenClaw. Papan kerja dasbor untuk isu dan sesi yang dimiliki agen.
...
- **[zoom-meetings](/id/plugins/reference/zoom-meetings)** (`@openclaw/zoom-meetings`) - disertakan dalam OpenClaw. Bergabung ke rapat Zoom sebagai tamu melalui peramban Chrome.
...
- **[baseten](/id/plugins/reference/baseten)** (`@openclaw/baseten-provider`) - npm; ClawHub: `clawhub:@openclaw/baseten-provider`. Plugin penyedia Baseten OpenClaw.
...
- **[clickclack](/id/plugins/reference/clickclack)** (`@openclaw/clickclack`) - npm; ClawHub: `clawhub:@openclaw/clickclack`. Menambahkan antarmuka kanal Clickclack untuk mengirim dan menerima pesan OpenClaw.
...
- **[diagnostics-otel](/id/plugins/reference/diagnostics-otel)** (`@openclaw/diagnostics-otel`) - npm; ClawHub: `clawhub:@openclaw/diagnostics-otel`. Pengekspor diagnostik OpenTelemetry OpenClaw untuk metrik, jejak, dan log.

- **[diagnostics-prometheus](/id/plugins/reference/diagnostics-prometheus)** (`@openclaw/diagnostics-prometheus`) - npm; ClawHub: `clawhub:@openclaw/diagnostics-prometheus`. Pengekspor diagnostik Prometheus OpenClaw untuk metrik runtime.

- **[diffs](/id/plugins/reference/diffs)** (`@openclaw/diffs`) - npm; ClawHub. Plugin penampil diff hanya-baca dan perender berkas OpenClaw untuk agen.
...
- **[discord](/id/plugins/reference/discord)** (`@openclaw/discord`) - npm; ClawHub. Plugin kanal Discord OpenClaw untuk kanal, pesan langsung, perintah, dan peristiwa aplikasi.
...
- **[featherless](/id/plugins/reference/featherless)** (`@openclaw/featherless-provider`) - npm; ClawHub: `clawhub:@openclaw/featherless-provider`. Plugin penyedia Featherless AI OpenClaw.
...
- **[google-meet](/id/plugins/reference/google-meet)** (`@openclaw/google-meet`) - npm; ClawHub. Plugin peserta Google Meet OpenClaw untuk bergabung ke panggilan melalui transportasi Chrome atau Twilio.
...
- **[gradium](/id/plugins/reference/gradium)** (`@openclaw/gradium-speech`) - npm; ClawHub: `clawhub:@openclaw/gradium-speech`. Menambahkan dukungan penyedia text-to-speech.
...
- **[inworld](/id/plugins/reference/inworld)** (`@openclaw/inworld-speech`) - npm; ClawHub: `clawhub:@openclaw/inworld-speech`. Text-to-speech streaming Inworld (MP3, OGG_OPUS, PCM telepon).

- **[irc](/id/plugins/reference/irc)** (`@openclaw/irc`) - npm; ClawHub: `clawhub:@openclaw/irc`. Menambahkan antarmuka kanal IRC untuk mengirim dan menerima pesan OpenClaw.
...
- **[longcat](/id/plugins/reference/longcat)** (`@openclaw/longcat-provider`) - npm; ClawHub: `clawhub:@openclaw/longcat-provider`. Plugin penyedia LongCat OpenClaw.

- **[matrix](/id/plugins/reference/matrix)** (`@openclaw/matrix`) - ClawHub: `clawhub:@openclaw/matrix`; npm. Plugin kanal Matrix OpenClaw untuk ruang dan pesan langsung.

- **[mattermost](/id/plugins/reference/mattermost)** (`@openclaw/mattermost`) - npm; ClawHub: `clawhub:@openclaw/mattermost`. Menambahkan antarmuka kanal Mattermost untuk mengirim dan menerima pesan OpenClaw.

- **[memory-lancedb](/id/plugins/reference/memory-lancedb)** (`@openclaw/memory-lancedb`) - npm; ClawHub. Plugin memori jangka panjang OpenClaw yang didukung LanceDB dengan pemanggilan kembali otomatis, pengambilan otomatis, dan pencarian vektor.
...
- **[mxc](/id/plugins/reference/mxc)** (`@openclaw/mxc-sandbox`) - npm; ClawHub. Eksekusi alat dalam sandbox tingkat OS melalui MXC: menjalankan perintah dalam Windows ProcessContainer dengan berkas kebijakan MXC yang dikonfigurasi.
...
- **[raft](/id/plugins/reference/raft)** (`@openclaw/raft`) - npm; ClawHub. Plugin kanal Raft OpenClaw untuk jembatan pengaktifan CLI yang aman.
...
- **[signal](/id/plugins/reference/signal)** (`@openclaw/signal`) - npm; ClawHub: `clawhub:@openclaw/signal`. Menambahkan antarmuka kanal Signal untuk mengirim dan menerima pesan OpenClaw.

- **[slack](/id/plugins/reference/slack)** (`@openclaw/slack`) - npm; ClawHub. Plugin kanal Slack OpenClaw untuk kanal, pesan langsung, perintah, dan peristiwa aplikasi.
...
- **[qa-lab](/id/plugins/reference/qa-lab)** (`@openclaw/qa-lab`) - hanya checkout sumber. Plugin lab QA OpenClaw dengan UI pengawakutu privat dan pelaksana skenario.
