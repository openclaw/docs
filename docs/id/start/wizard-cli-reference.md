...
read_when:
    ...
    - Anda sedang men-debug hasil orientasi awal atau mengintegrasikan klien orientasi awal
sidebarTitle: CLI reference
summary: 'Perilaku langkah demi langkah untuk openclaw onboard: fungsi setiap langkah, konfigurasi yang ditulisnya, dan mekanisme internalnya'
title: Referensi penyiapan CLI
x-i18n:
    generated_at: "2026-07-19T05:12:46Z"
    model: gpt-5.6
...
(setiap `--flag`, contoh non-interaktif, perintah khusus penyedia),
...
- Penyiapan model dan autentikasi (Anthropic, OAuth langganan OpenAI Code, xAI, OpenCode, endpoint kustom, dan alur autentikasi lain milik penyedia)
...
<Steps>
  <Step title="Deteksi konfigurasi yang ada">
    ...
    - CLI `--reset` secara default menggunakan `config+creds+sessions`; gunakan `--reset-scope full` untuk turut menghapus ruang kerja.
    ...
    - Atur ulang memindahkan status ke Sampah (tidak pernah langsung menghapus) dan menawarkan cakupan:
      - Hanya konfigurasi
  ...
  <Step title="Kanal">
    ...
    - [iMessage](/id/channels/imessage): jalur CLI `imsg` + akses DB Messages; gunakan wrapper SSH saat Gateway berjalan di luar Mac
    - Keamanan DM: defaultnya adalah pemasangan. DM pertama mengirim kode; setujui melalui
  ...
  <Step title="Instalasi daemon">
    - macOS: LaunchAgent
      - Memerlukan sesi pengguna yang sedang login; untuk penggunaan headless, gunakan LaunchDaemon kustom (tidak disertakan).
    - Linux dan Windows melalui WSL2: unit pengguna systemd
      ...
      - Dapat meminta sudo (menulis `/var/lib/systemd/linger`); terlebih dahulu mencoba tanpa sudo.
  ...
  <Step title="Pemeriksaan kesehatan">
    ...
    - `openclaw status --deep` menambahkan probe kesehatan gateway aktif ke keluaran status, termasuk probe kanal jika didukung.

  </Step>
  <Step title="Skills">
    - Membaca skill yang tersedia dan memeriksa persyaratan.
    - Memungkinkan Anda memilih pengelola node: npm, pnpm, atau bun.
    - Menginstal dependensi opsional untuk skill bawaan tepercaya ketika penginstal yang diperlukan
      tersedia.
...
<Note>
Jika GUI tidak terdeteksi, wizard mencetak petunjuk penerusan port SSH untuk UI Kontrol alih-alih membuka browser.
Jika aset UI Kontrol tidak tersedia, wizard mencoba membangunnya; fallback-nya adalah `pnpm ui:build` (menginstal otomatis dependensi UI).
</Note>
...
<Steps>
  ...
  <Step title="Metode koneksi">
    ...
    - **Langsung**: terhubung melalui `wss://` dan meminta Anda memercayai
      sidik jari TLS yang ditemukan (penyematan berdasarkan kepercayaan saat pertama digunakan; hanya disematkan jika Anda menyetujuinya).
  ...
  <Step title="Autentikasi">
    ...
    sebagai SecretRef, bukan sebagai teks biasa.
  </Step>
</Steps>

<Note>
Jika gateway hanya menggunakan loopback dan tidak dapat ditemukan, gunakan tunneling SSH atau tailnet secara manual.
`ws://` teks biasa diterima untuk loopback, literal IP privat, `.local`, dan URL `*.ts.net` Tailnet; nama DNS privat lainnya memerlukan `OPENCLAW_ALLOW_INSECURE_PRIVATE_WS=1`.
</Note>

## Opsi autentikasi dan model

Jika langkah penyiapan penyedia gagal dalam onboarding interaktif (misalnya opsi penggunaan ulang CLI
tanpa login lokal), wizard menampilkan kesalahan dan kembali ke pemilih penyedia
alih-alih keluar. Proses `--auth-choice` eksplisit tetap gagal seketika untuk otomatisasi.
...
<AccordionGroup>
  ...
  <Accordion title="CLI Anthropic Claude">
    Jalur lokal yang lebih disukai dalam onboarding/konfigurasi interaktif; menggunakan kembali login CLI Claude yang ada jika tersedia.
  </Accordion>
  <Accordion title="Langganan OpenAI Code (OAuth)">
    Alur browser; tempelkan `code#state`.
  ...
  <Accordion title="Kunci API OpenAI">
    ...
    Pada penyiapan baru tanpa model utama, menetapkan `agents.defaults.model` ke
    `openai/gpt-5.6`; ID model API langsung tanpa prefiks diresolusikan ke tingkat Sol.

    Menambahkan atau mengautentikasi ulang OpenAI mempertahankan model utama eksplisit yang ada,
  ...
  <Accordion title="Kode perangkat xAI (Grok)">
    Login melalui browser yang cocok untuk akses jarak jauh dengan kode singkat sebagai pengganti
    panggilan balik localhost. Gunakan ini dari host SSH, Docker, atau VPS.
  </Accordion>
  <Accordion title="Kunci API xAI (Grok)">
    Meminta `XAI_API_KEY` dan mengonfigurasi xAI sebagai penyedia model. Gunakan ini
    jika Anda menginginkan kunci API xAI Console sebagai pengganti OAuth langganan.
  </Accordion>
  ...
  <Accordion title="MiniMax">
    Konfigurasi ditulis secara otomatis. Default yang di-host adalah `MiniMax-M3`; penyiapan kunci API menggunakan
  ...
  <Accordion title="Penyedia khusus">
    ...
    - **Gunakan referensi rahasia** (referensi env atau referensi penyedia yang dikonfigurasi, dengan validasi prapemeriksaan)

    Onboarding menyimpulkan dukungan gambar untuk ID model visi umum (GPT-4o/4.1/5.x, Claude 3/4, Gemini, Qwen-VL, LLaVA, Pixtral, dan yang serupa) dan hanya bertanya jika nama model tidak dikenal.

    Flag noninteraktif:
    - `--auth-choice custom-api-key`
    - `--custom-base-url`
    - `--custom-model-id`
    - `--custom-api-key` (opsional; menggunakan `CUSTOM_API_KEY` sebagai fallback)
...
- Saat onboarding dimulai dari pilihan autentikasi penyedia, pemilih model secara otomatis memprioritaskan
  ...
  `byteplus-plan/*`).
- Jika filter penyedia pilihan tersebut akan menghasilkan daftar kosong, pemilih menggunakan
  katalog lengkap sebagai fallback alih-alih tidak menampilkan model apa pun.
...
- Perilaku onboarding default menyimpan kunci API sebagai nilai teks biasa dalam profil autentikasi.
...
- Mode referensi interaktif menjalankan validasi prapemeriksaan cepat sebelum menyimpan.
  - Referensi env: memvalidasi nama variabel + nilai yang tidak kosong dalam lingkungan onboarding saat ini.
  - Referensi penyedia: memvalidasi konfigurasi penyedia dan menyelesaikan id yang diminta.
  - Jika prapemeriksaan gagal, onboarding menampilkan kesalahan dan memungkinkan Anda mencoba lagi.
- Dalam mode noninteraktif, `--secret-input-mode ref` hanya didukung oleh env.
  - Tetapkan variabel env penyedia dalam lingkungan proses onboarding.
  - Flag kunci sebaris (misalnya `--openai-api-key`) mengharuskan variabel env tersebut ditetapkan; jika tidak, onboarding langsung gagal.
  ...
  - Dalam kasus penyedia khusus tersebut, `--custom-api-key` mengharuskan `CUSTOM_API_KEY` ditetapkan; jika tidak, onboarding langsung gagal.
...
<Note>
Kiat untuk sistem tanpa antarmuka grafis dan server: selesaikan OAuth pada mesin dengan browser, lalu salin
...
- `tools.profile` (onboarding lokal menggunakan default `"coding"` jika belum ditetapkan; nilai eksplisit yang sudah ada dipertahankan)
- `gateway.*` (mode, pengikatan, autentikasi, tailscale)
- `session.dmScope` (onboarding mempertahankan nilai eksplisit dan jika tidak ada membiarkannya tidak ditetapkan, sehingga default `main` mempertahankan semua pesan langsung lintas saluran dalam sesi utama bergulir milik agen—default agen pribadi. Untuk kotak masuk bersama atau multi-pengguna, gunakan `per-channel-peer`; `openclaw security audit` merekomendasikan isolasi ketika mendeteksi lalu lintas pesan langsung multi-pengguna)
- `channels.telegram.botToken`, `channels.discord.token`, `channels.matrix.*`, `channels.signal.*`, `channels.imessage.*`
- Daftar izin saluran (Discord, iMessage, Signal, Slack, Telegram, WhatsApp) saat Anda memilih ikut serta dalam prompt; Discord dan Slack juga menyelesaikan nama yang dimasukkan menjadi ID
...
meminta untuk menginstal plugin (npm atau jalur lokal) sebelum konfigurasi saluran.
</Note>

### Rekomendasi aplikasi yang terinstal

Setelah pemeriksaan akses model berhasil, onboarding interaktif klasik di macOS memindai nama aplikasi dan ID bundel tanpa meminta izin privasi macOS. Onboarding mencari katalog plugin resmi dan ClawHub, lalu meminta model yang dikonfigurasi untuk menolak kecocokan nama palsu dan merekomendasikan plugin atau skill yang relevan. Kecocokan yang direkomendasikan dipilih secara default; kecocokan opsional memerlukan pemilihan eksplisit.

Layar hasil mencantumkan aplikasi yang terdeteksi dan menampilkan: "Nama aplikasi dicocokkan menggunakan model yang Anda konfigurasi dan pencarian ClawHub." Tetapkan `wizard.appRecommendations` ke `false` untuk menonaktifkan langkah onboarding ini dan akses Gateway ke inventaris aplikasi node. Pemindaian tidak digunakan dalam quickstart atau onboarding selain macOS.

## Penyiapan noninteraktif

`--non-interactive` memerlukan `--accept-risk` (mengakui bahwa agen
bersifat kuat dan akses penuh ke sistem berisiko):
...
Klien (aplikasi macOS dan UI Kontrol) dapat merender langkah-langkah tanpa menerapkan ulang logika onboarding.
...
- Pada platform lain (macOS, Linux non-x64), menginstal melalui Homebrew sebagai gantinya
- Menyimpan instalasi aset rilis di bawah `~/.openclaw/tools/signal-cli/<version>/`
- Menulis `channels.signal.cliPath` dalam konfigurasi
- Windows native belum didukung; jalankan onboarding di dalam WSL2 untuk mendapatkan jalur instalasi Linux
