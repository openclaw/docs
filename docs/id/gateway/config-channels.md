...
summary: 'Konfigurasi kanal: kontrol akses, penyandingan, kunci per kanal di Slack, Discord, Telegram, WhatsApp, Matrix, iMessage, dan lainnya'
title: Konfigurasi — kanal
x-i18n:
    generated_at: "2026-05-04T07:04:58Z"
    model: gpt-5.5
    provider: openai
...
penyiapan multi-akun, gating penyebutan, dan kunci per-channel untuk Slack, Discord,
...
| `allowlist`         | Hanya pengirim di `allowFrom` (atau penyimpanan izin yang telah dipairing) |
...
<Note>
`channels.defaults.groupPolicy` menetapkan default saat `groupPolicy` provider belum diatur.
...
### Penggantian model channel

Gunakan `channels.modelByChannel` untuk mengunci ID channel tertentu ke sebuah model. Nilai menerima `provider/model` atau alias model yang dikonfigurasi. Pemetaan channel berlaku saat sebuah sesi belum memiliki penggantian model (misalnya, diatur melalui `/model`).
...
- `channels.defaults.heartbeat.useIndicator`: render output heartbeat gaya indikator yang ringkas.
...
```json5
{
  ...
  channels: {
    whatsapp: {
      ...
      mediaMaxMb: 50,
      sendReadReceipts: true, // centang biru (false dalam mode chat mandiri)
      groups: {
        "*": { requireMention: true },
      },
...
```json5
{
  channels: {
    telegram: {
      ...
      linkPreview: true,
      streaming: "partial", // off | partial | block | progress (default: off; ikut serta secara eksplisit untuk menghindari batas laju edit pratinjau)
...
- Slug guild menggunakan huruf kecil dengan spasi diganti `-`; kunci saluran menggunakan nama yang telah di-slug (tanpa `#`). Utamakan ID guild.
...
- `channels.discord.guilds.<id>.ignoreOtherMentions` (dan override saluran) membuang pesan yang menyebut pengguna atau peran lain tetapi bukan bot (tidak termasuk @everyone/@here).
...
- Entri `bindings[]` tingkat atas dengan `type: "acp"` mengonfigurasi pengikatan ACP persisten untuk saluran dan thread (gunakan id saluran/thread di `match.peer.id`). Semantik field dibagikan di [Agen ACP](/id/tools/acp-agents#persistent-channel-bindings).
- `channels.discord.ui.components.accentColor` menetapkan warna aksen untuk kontainer Discord components v2.
- `channels.discord.voice` mengaktifkan percakapan saluran suara Discord serta override auto-join + LLM + TTS opsional. Konfigurasi Discord khusus teks membiarkan suara nonaktif secara default; tetapkan `channels.discord.voice.enabled=true` untuk ikut serta.
- `channels.discord.voice.model` secara opsional mengesampingkan model LLM yang digunakan untuk respons saluran suara Discord.
...
- `channels.discord.voice.reconnectGraceMs` mengontrol berapa lama sesi suara yang terputus boleh memasuki pensinyalan koneksi ulang sebelum OpenClaw menghancurkannya (`15000` secara default).
...
- `channels.discord.execApprovals`: pengiriman persetujuan eksekusi native Discord dan otorisasi pemberi persetujuan.
  ...
  - `target`: tempat mengirim prompt persetujuan. `"dm"` (default) mengirim ke DM pemberi persetujuan, `"channel"` mengirim ke saluran asal, `"both"` mengirim ke keduanya. Ketika target mencakup `"channel"`, tombol hanya dapat digunakan oleh pemberi persetujuan yang terselesaikan.
...
- `socketMode` meneruskan penyesuaian transport Slack SDK Socket Mode ke API receiver Bolt publik. Gunakan hanya saat menyelidiki timeout ping/pong atau perilaku websocket basi.
...
- Snapshot akun Slack mengekspos field sumber/status per kredensial seperti
  ...
  dikonfigurasi melalui SecretRef tetapi path command/runtime saat ini tidak dapat
...
- Streaming native Slack plus status thread bergaya asisten Slack "is typing..." memerlukan target thread balasan. DM tingkat atas tetap di luar thread secara default, sehingga masih dapat stream melalui pratinjau posting-draf-dan-edit Slack alih-alih menampilkan pratinjau stream/status native bergaya thread.
...
- `channels.slack.execApprovals`: pengiriman persetujuan eksekusi native Slack dan otorisasi pemberi persetujuan. Skema yang sama seperti Discord: `enabled` (`true`/`false`/`"auto"`), `approvers` (ID pengguna Slack), `agentFilter`, `sessionFilter`, dan `target` (`"dm"`, `"channel"`, atau `"both"`).
...
Ketika command native Mattermost diaktifkan:
...
- Entri `bindings[]` tingkat atas dengan `type: "acp"` dapat mengikat percakapan BlueBubbles ke sesi ACP persisten. Gunakan handle BlueBubbles atau string target (`chat_id:*`, `chat_guid:*`, `chat_identifier:*`) di `match.peer.id`. Semantik kolom bersama: [Agen ACP](/id/tools/acp-agents#persistent-channel-bindings).
...
- Entri `bindings[]` tingkat atas dengan `type: "acp"` dapat mengikat percakapan iMessage ke sesi ACP persisten. Gunakan handle yang dinormalisasi atau target chat eksplisit (`chat_id:*`, `chat_guid:*`, `chat_identifier:*`) di `match.peer.id`. Semantik kolom bersama: [Agen ACP](/id/tools/acp-agents#persistent-channel-bindings).
...
Balasan yang terlihat dikontrol secara terpisah. Room grup/channel default ke `messages.groupChat.visibleReplies: "message_tool"`: OpenClaw tetap memproses giliran tersebut, tetapi balasan final normal tetap privat dan output room yang terlihat memerlukan `message(action=send)`. Atur `"automatic"` hanya saat Anda menginginkan perilaku lama ketika balasan normal diposting kembali ke room. Untuk menerapkan perilaku balasan terlihat khusus-tool yang sama ke chat langsung juga, atur `messages.visibleReplies: "message_tool"`; harness Codex juga menggunakan perilaku khusus-tool tersebut sebagai default chat langsung yang belum diatur.

Balasan terlihat khusus-tool memerlukan model/runtime yang andal memanggil tool. Jika
log sesi menampilkan teks asisten dengan `didSendViaMessagingTool: false`, model
menghasilkan jawaban final privat alih-alih memanggil tool pesan.
Beralih ke model pemanggil tool yang lebih kuat untuk channel tersebut, atau atur
`messages.groupChat.visibleReplies: "automatic"` untuk memulihkan balasan final
terlihat lama.

Jika tool pesan tidak tersedia di bawah kebijakan tool aktif, OpenClaw fallback ke balasan terlihat otomatis alih-alih menyembunyikan respons secara diam-diam. `openclaw doctor` memperingatkan tentang ketidakcocokan ini.

Gateway hot-reload konfigurasi `messages` setelah file disimpan. Mulai ulang hanya saat file watching atau pemuatan ulang konfigurasi dinonaktifkan dalam deployment.

**Jenis mention:**

- **Penyebutan metadata**: @-mention platform asli. Diabaikan dalam mode chat-diri WhatsApp.
- **Pola teks**: Pola regex aman di `agents.list[].groupChat.mentionPatterns`. Pola yang tidak valid dan repetisi bersarang yang tidak aman diabaikan.
- Gerbang penyebutan hanya diberlakukan ketika deteksi memungkinkan (penyebutan asli atau setidaknya satu pola).
...
`messages.visibleReplies` adalah default global untuk giliran sumber; `messages.groupChat.visibleReplies` menimpanya untuk giliran sumber grup/channel. Ketika `messages.visibleReplies` tidak diatur, harness dapat menyediakan default langsung/sumbernya sendiri; harness Codex default ke `message_tool`. Daftar izin channel dan gerbang penyebutan tetap menentukan apakah suatu giliran diproses.
...
Sertakan nomor Anda sendiri di `allowFrom` untuk mengaktifkan mode chat-diri (mengabaikan @-mention asli, hanya merespons pola teks):
...
- Halaman ini adalah **referensi kunci konfigurasi**, bukan katalog perintah lengkap. Perintah milik channel/plugin seperti QQ Bot `/bot-ping` `/bot-help` `/bot-logs`, LINE `/card`, pemasangan perangkat `/pair`, memori `/dreaming`, kontrol ponsel `/phone`, dan Talk `/voice` didokumentasikan di halaman channel/plugin masing-masing plus [Perintah Slash](/id/tools/slash-commands).
- Perintah teks harus berupa pesan **mandiri** dengan awalan `/`.
- `native: "auto"` mengaktifkan perintah asli untuk Discord/Telegram, dan membiarkan Slack nonaktif.
- `nativeSkills: "auto"` mengaktifkan perintah Skills asli untuk Discord/Telegram, dan membiarkan Slack nonaktif.
- Timpa per channel: `channels.discord.commands.native` (bool atau `"auto"`). Untuk Discord, `false` melewati pendaftaran perintah asli dan pembersihan selama startup.
...
- `config: true` mengaktifkan `/config` (membaca/menulis `openclaw.json`). Untuk klien Gateway `chat.send`, penulisan persisten `/config set|unset` juga memerlukan `operator.admin`; `/config show` baca-saja tetap tersedia untuk klien operator berscope tulis biasa.
...
- `channels.<provider>.configWrites` mengatur gerbang mutasi konfigurasi per channel (default: true).
- Untuk channel multi-akun, `channels.<provider>.accounts.<id>.configWrites` juga mengatur gerbang penulisan yang menargetkan akun tersebut (misalnya `/allowlist --config --account <id>` atau `/config set channels.<provider>.accounts.<id>...`).
...
- `allowFrom` bersifat per penyedia. Ketika diatur, ini menjadi **satu-satunya** sumber otorisasi (daftar izin/pemasangan channel dan `useAccessGroups` diabaikan).
