---
read_when:
    - Menyetel default agen (model, pemikiran, ruang kerja, Heartbeat, media, Skills)
...
Root repositori opsional yang ditampilkan di baris Runtime prompt sistem. Jika tidak disetel, OpenClaw mendeteksinya otomatis dengan menelusuri ke atas dari ruang kerja.
...
- Setel `agents.list[].skills: []` agar tidak ada Skills.
...
- `"continuation-skip"`: giliran kelanjutan yang aman (setelah respons asisten selesai) melewati penyuntikan ulang bootstrap ruang kerja, sehingga mengurangi ukuran prompt. Eksekusi Heartbeat dan percobaan ulang pasca-Compaction tetap membangun ulang konteks.
...
Karakter maksimum per file bootstrap ruang kerja sebelum pemotongan. Bawaan: `12000`.
...
Total karakter maksimum yang disuntikkan di semua file bootstrap ruang kerja. Bawaan: `60000`.
...
- `"always"`: suntikkan pemberitahuan ringkas pada setiap eksekusi saat ada pemotongan.

Hitungan mentah/tersuntik yang terperinci dan bidang penyetelan konfigurasi tetap berada di diagnostik seperti
laporan dan log konteks/status; konteks pengguna/runtime WebChat rutin hanya
mendapatkan pemberitahuan pemulihan yang ringkas.
...
sengaja dipisahkan menurut subsistem, alih-alih semuanya mengalir melalui satu
kenop generik.
...
- `agents.defaults.startupContext.*`:
  prelude satu kali untuk eksekusi model reset/startup, termasuk file
...
Mengontrol prelude startup giliran pertama yang disuntikkan pada eksekusi model reset/startup.
...
- `memoryGetDefaultLines`: jendela baris `memory_get` default ketika `lines`
  dihilangkan.
- `toolResultMaxChars`: batas hasil alat langsung yang digunakan untuk hasil yang dipertahankan dan
  pemulihan overflow.
...
Nilai yang lebih rendah biasanya mengurangi penggunaan token visi dan ukuran payload permintaan untuk eksekusi yang banyak berisi screenshot.
...
Zona waktu untuk konteks prompt sistem (bukan stempel waktu pesan). Beralih ke zona waktu host jika tidak ada.
...
- `imageModel`: menerima string (`"provider/model"`) atau objek (`{ primary, fallbacks }`).
  ...
  - Juga digunakan sebagai rute fallback saat model yang dipilih/default tidak dapat menerima input gambar.
...
- `imageGenerationModel`: menerima string (`"provider/model"`) atau objek (`{ primary, fallbacks }`).
  - Digunakan oleh kapabilitas pembuatan gambar bersama dan permukaan alat/plugin apa pun di masa depan yang menghasilkan gambar.
...
- `toolProgressDetail`: mode detail untuk ringkasan alat `/verbose` dan baris alat draf progres. Nilai: `"explain"` (default, label manusia ringkas) atau `"raw"` (tambahkan perintah/detail mentah bila tersedia). `agents.list[].toolProgressDetail` per agen mengesampingkan default ini.
- `reasoningDefault`: visibilitas reasoning default untuk agen. Nilai: `"off"`, `"on"`, `"stream"`. `agents.list[].reasoningDefault` per agen mengesampingkan default ini. Default reasoning yang dikonfigurasi hanya diterapkan untuk pemilik, pengirim terotorisasi, atau konteks operator-admin Gateway saat tidak ada override reasoning per pesan atau sesi yang ditetapkan.
- `elevatedDefault`: tingkat output tinggi default untuk agen. Nilai: `"off"`, `"on"`, `"ask"`, `"full"`. Default: `"on"`.
- `model.primary`: format `provider/model` (mis. `openai/gpt-5.5` untuk akses kunci API atau `openai-codex/gpt-5.5` untuk Codex OAuth). Jika Anda menghilangkan penyedia, OpenClaw mencoba alias terlebih dahulu, lalu kecocokan penyedia terkonfigurasi yang unik untuk id model persis tersebut, dan baru setelah itu fallback ke penyedia default yang dikonfigurasi (perilaku kompatibilitas usang, jadi utamakan `provider/model` eksplisit). Jika penyedia itu tidak lagi mengekspos model default yang dikonfigurasi, OpenClaw fallback ke penyedia/model terkonfigurasi pertama alih-alih menampilkan default penyedia yang sudah dihapus dan basi.
- `models`: katalog model yang dikonfigurasi dan allowlist untuk `/model`. Setiap entri dapat menyertakan `alias` (pintasan) dan `params` (spesifik penyedia, misalnya `temperature`, `maxTokens`, `cacheRetention`, `context1m`, `responsesServerCompaction`, `responsesCompactThreshold`, `chat_template_kwargs`, `extra_body`/`extraBody`).
  ...
  - Alur konfigurasi/onboarding dengan cakupan penyedia menggabungkan model penyedia yang dipilih ke dalam peta ini dan mempertahankan penyedia lain yang sudah dikonfigurasi.
  - Untuk model OpenAI Responses langsung, Compaction sisi server diaktifkan secara otomatis. Gunakan `params.responsesServerCompaction: false` untuk berhenti menyuntikkan `context_management`, atau `params.responsesCompactThreshold` untuk mengesampingkan ambangnya. Lihat [Compaction sisi server OpenAI](/id/providers/openai#server-side-compaction-responses-api).
...
- Presedensi penggabungan `params` (konfigurasi): `agents.defaults.params` (basis global) digantikan oleh `agents.defaults.models["provider/model"].params` (per model), lalu `agents.list[].params` (id agen yang cocok) mengesampingkan berdasarkan kunci. Lihat [Penyimpanan Cache Prompt](/id/reference/prompt-caching) untuk detail.
- `params.extra_body`/`params.extraBody`: JSON pass-through lanjutan yang digabungkan ke dalam body permintaan `api: "openai-completions"` untuk proxy yang kompatibel dengan OpenAI. Jika bertabrakan dengan kunci permintaan yang dihasilkan, body tambahan menang; rute completions non-native tetap menghapus `store` khusus OpenAI setelahnya.
- `params.chat_template_kwargs`: argumen chat-template yang kompatibel dengan vLLM/OpenAI yang digabungkan ke dalam body permintaan tingkat atas `api: "openai-completions"`. Untuk `vllm/nemotron-3-*` dengan thinking nonaktif, plugin vLLM yang dibundel otomatis mengirim `enable_thinking: false` dan `force_nonempty_content: true`; `chat_template_kwargs` eksplisit mengesampingkan default yang dihasilkan, dan `extra_body.chat_template_kwargs` tetap memiliki presedensi akhir. Untuk kontrol thinking Qwen vLLM, tetapkan `params.qwenThinkingFormat` ke `"chat-template"` atau `"top-level"` pada entri model tersebut.
- `compat.supportedReasoningEfforts`: daftar effort reasoning per model yang kompatibel dengan OpenAI. Sertakan `"xhigh"` untuk endpoint kustom yang benar-benar menerimanya; OpenClaw lalu mengekspos `/think xhigh` di menu perintah, baris sesi Gateway, validasi patch sesi, validasi CLI agen, dan validasi `llm-task` untuk penyedia/model terkonfigurasi tersebut. Gunakan `compat.reasoningEffortMap` saat backend menginginkan nilai spesifik penyedia untuk level kanonis.
...
`agentRuntime` mengontrol executor tingkat rendah mana yang menjalankan giliran agen. Sebagian besar
deployment sebaiknya mempertahankan runtime OpenClaw Pi default. Gunakan saat plugin tepercaya
...
- `id: "auto"` memungkinkan harness plugin terdaftar mengklaim giliran yang didukung dan menggunakan PI saat tidak ada harness yang cocok. Runtime plugin eksplisit seperti `id: "codex"` mewajibkan harness tersebut dan gagal tertutup jika tidak tersedia atau gagal.
- Override lingkungan: `OPENCLAW_AGENT_RUNTIME=<id|auto|pi>` mengesampingkan `id` untuk proses tersebut.
- Untuk deployment khusus Codex, tetapkan `model: "openai/gpt-5.5"` dan `agentRuntime.id: "codex"`.
- Untuk deployment Claude CLI, utamakan `model: "anthropic/claude-opus-4-7"` plus `agentRuntime.id: "claude-cli"`. Referensi model lama `claude-cli/claude-opus-4-7` masih berfungsi untuk kompatibilitas, tetapi konfigurasi baru sebaiknya menjaga pemilihan penyedia/model tetap kanonis dan menempatkan backend eksekusi di `agentRuntime.id`.
- Kunci kebijakan runtime lama ditulis ulang ke `agentRuntime` oleh `openclaw doctor --fix`.
- Pilihan harness disematkan per id sesi setelah run embedded pertama. Perubahan konfigurasi/env memengaruhi sesi baru atau sesi yang direset, bukan transkrip yang sudah ada. Sesi lama dengan riwayat transkrip tetapi tanpa pin yang terekam diperlakukan sebagai disematkan ke PI. `/status` melaporkan runtime efektif, misalnya `Runtime: OpenClaw Pi Default` atau `Runtime: OpenAI Codex`.
...
Model Z.AI mengaktifkan `tool_stream` secara default untuk pengaliran pemanggilan alat. Tetapkan `agents.defaults.models["zai/<model>"].params.tool_stream` ke `false` untuk menonaktifkannya.
Model Anthropic Claude 4.6 secara default menggunakan pemikiran `adaptive` ketika tidak ada tingkat pemikiran eksplisit yang ditetapkan.

### `agents.defaults.cliBackends`

Backend CLI opsional untuk proses fallback khusus teks (tanpa pemanggilan alat). Berguna sebagai cadangan saat penyedia API gagal.
...
- Penerusan gambar didukung ketika `imageArg` menerima path berkas.

### `agents.defaults.systemPromptOverride`

Ganti seluruh prompt sistem yang disusun OpenClaw dengan string tetap. Tetapkan pada tingkat default (`agents.defaults.systemPromptOverride`) atau per agen (`agents.list[].systemPromptOverride`). Nilai per agen memiliki prioritas; nilai kosong atau hanya spasi diabaikan. Berguna untuk eksperimen prompt terkontrol.
...
- `suppressToolErrorWarnings`: ketika true, menekan payload peringatan galat alat selama proses Heartbeat.
...
- `lightContext`: ketika true, proses Heartbeat menggunakan konteks bootstrap ringan dan hanya mempertahankan `HEARTBEAT.md` dari berkas bootstrap ruang kerja.
...
- `skipWhenBusy`: ketika true, proses Heartbeat ditunda pada jalur sibuk tambahan: pekerjaan subagen atau perintah bertingkat. Jalur Cron selalu menunda Heartbeat, bahkan tanpa flag ini.
...
- Heartbeat menjalankan giliran agen penuh — interval yang lebih pendek menggunakan lebih banyak token.
...
- `provider`: ID Plugin penyedia Compaction terdaftar. Ketika ditetapkan, `summarize()` milik penyedia dipanggil sebagai pengganti peringkasan LLM bawaan. Kembali ke bawaan jika gagal. Menetapkan penyedia memaksa `mode: "safeguard"`. Lihat [Compaction](/id/concepts/compaction).
- `timeoutSeconds`: detik maksimum yang diizinkan untuk satu operasi Compaction sebelum OpenClaw membatalkannya. Default: `900`.
- `keepRecentTokens`: anggaran titik potong Pi untuk mempertahankan bagian akhir transkrip terbaru secara verbatim. `/compact` manual menghormati ini ketika ditetapkan secara eksplisit; jika tidak, Compaction manual adalah checkpoint tegas.
- `identifierPolicy`: `strict` (default), `off`, atau `custom`. `strict` menambahkan panduan bawaan untuk retensi pengidentifikasi buram di awal selama peringkasan Compaction.
- `identifierInstructions`: teks pelestarian pengidentifikasi kustom opsional yang digunakan ketika `identifierPolicy=custom`.
- `qualityGuard`: pemeriksaan coba ulang saat keluaran salah bentuk untuk ringkasan safeguard. Diaktifkan secara default dalam mode safeguard; tetapkan `enabled: false` untuk melewati audit.
- `midTurnPrecheck`: pemeriksaan tekanan loop alat Pi opsional. Ketika `enabled: true`, OpenClaw memeriksa tekanan konteks setelah hasil alat ditambahkan dan sebelum pemanggilan model berikutnya. Jika konteks tidak lagi muat, OpenClaw membatalkan upaya saat ini sebelum mengirimkan prompt dan menggunakan kembali jalur pemulihan prapemeriksaan yang ada untuk memotong hasil alat atau melakukan Compaction lalu mencoba lagi. Berfungsi dengan mode Compaction `default` dan `safeguard`. Default: dinonaktifkan.
...
- `model`: override `provider/model-id` opsional khusus untuk peringkasan Compaction. Gunakan ini ketika sesi utama harus mempertahankan satu model tetapi ringkasan Compaction harus berjalan pada model lain; ketika tidak ditetapkan, Compaction menggunakan model utama sesi.
- `maxActiveTranscriptBytes`: ambang byte opsional (`number` atau string seperti `"20mb"`) yang memicu Compaction lokal normal sebelum proses berjalan ketika JSONL aktif melewati ambang. Memerlukan `truncateAfterCompaction` agar Compaction yang berhasil dapat berotasi ke transkrip penerus yang lebih kecil. Dinonaktifkan ketika tidak ditetapkan atau `0`.
- `notifyUser`: ketika `true`, mengirim pemberitahuan singkat kepada pengguna saat Compaction dimulai dan saat selesai (misalnya, "Memadatkan konteks..." dan "Compaction selesai"). Dinonaktifkan secara default agar Compaction tetap senyap.
- `memoryFlush`: giliran agentik senyap sebelum Compaction otomatis untuk menyimpan memori tahan lama. Tetapkan `model` ke penyedia/model persis seperti `ollama/qwen3:8b` ketika giliran pemeliharaan ini harus tetap pada model lokal; override tidak mewarisi rantai fallback sesi aktif. Dilewati ketika ruang kerja hanya-baca.
...
- Pemangkasan terlebih dahulu memotong lunak hasil alat yang terlalu besar, lalu membersihkan keras hasil alat yang lebih lama jika diperlukan.

**Pemotongan lunak** mempertahankan awal + akhir dan menyisipkan `...` di tengah.

**Pembersihan keras** mengganti seluruh hasil alat dengan placeholder.
...
- Override kanal: `channels.<channel>.blockStreamingCoalesce` (dan varian per akun). Signal/Slack/Discord/Google Chat default ke `minChars: 1500`.
- `humanDelay`: jeda acak di antara balasan blok. `natural` = 800–2500ms. Override per agen: `agents.list[].humanDelay`.

Lihat [Pengaliran](/id/concepts/streaming) untuk detail perilaku + pemotongan bagian.
...
- Nilai `*Data` yang didukung SecretRef diselesaikan dari snapshot runtime secret aktif sebelum sesi sandbox dimulai

**Perilaku backend SSH:**

- melakukan seed workspace jarak jauh sekali setelah dibuat atau dibuat ulang
...
- `ro`: workspace sandbox di `/workspace`, workspace agent di-mount hanya-baca di `/agent`
...
Dalam mode `remote`, pengeditan lokal host yang dibuat di luar OpenClaw tidak disinkronkan ke dalam sandbox secara otomatis setelah langkah seed.
...
**`setupCommand`** berjalan sekali setelah kontainer dibuat (melalui `sh -lc`). Membutuhkan egress jaringan, root yang dapat ditulis, pengguna root.

**Kontainer menggunakan `network: "none"` secara default** — atur ke `"bridge"` (atau jaringan bridge kustom) jika agent membutuhkan akses keluar.
...
**Browser sandbox** (`sandbox.browser.enabled`): Chromium + CDP dalam kontainer. URL noVNC disuntikkan ke prompt sistem. Tidak memerlukan `browser.enabled` dalam `openclaw.json`.
...
- `sandbox.browser.binds` me-mount direktori host tambahan hanya ke kontainer browser sandbox. Saat ditetapkan (termasuk `[]`), ini menggantikan `docker.binds` untuk kontainer browser.
...
Untuk instalasi npm tanpa checkout sumber, lihat [Sandboxing § Image dan setup](/id/gateway/sandboxing#images-and-setup) untuk perintah `docker build` inline.

### `agents.list` (override per agent)

Gunakan `agents.list[].tts` untuk memberi agent provider TTS, suara, model,
...
agent hanya meng-override bidang suara atau provider yang mereka butuhkan. Override agent aktif
berlaku untuk balasan lisan otomatis, `/tts audio`, `/tts status`, dan
alat agent `tts`. Lihat [Text-to-speech](/id/tools/tts#per-agent-voice-overrides)
...
- `model`: bentuk string menetapkan primer per agen yang ketat tanpa model cadangan; bentuk objek `{ primary }` juga ketat kecuali Anda menambahkan `fallbacks`. Gunakan `{ primary, fallbacks: [...] }` untuk mengikutkan agen tersebut ke cadangan, atau `{ primary, fallbacks: [] }` untuk membuat perilaku ketat eksplisit. Pekerjaan Cron yang hanya menimpa `primary` tetap mewarisi cadangan default kecuali Anda mengatur `fallbacks: []`.
- `params`: parameter stream per agen yang digabungkan di atas entri model terpilih di `agents.defaults.models`. Gunakan ini untuk penimpaan khusus agen seperti `cacheRetention`, `temperature`, atau `maxTokens` tanpa menduplikasi seluruh katalog model.
- `tts`: penimpaan teks-ke-ucapan opsional per agen. Blok ini melakukan penggabungan mendalam di atas `messages.tts`, jadi simpan kredensial penyedia bersama dan kebijakan cadangan di `messages.tts` serta tetapkan hanya nilai khusus persona seperti penyedia, suara, model, gaya, atau mode otomatis di sini.
- `skills`: daftar Skills yang diizinkan opsional per agen. Jika dihilangkan, agen mewarisi `agents.defaults.skills` ketika diatur; daftar eksplisit menggantikan default alih-alih menggabungkannya, dan `[]` berarti tanpa Skills.
- `thinkingDefault`: level berpikir default opsional per agen (`off | minimal | low | medium | high | xhigh | adaptive | max`). Menimpa `agents.defaults.thinkingDefault` untuk agen ini ketika tidak ada penimpaan per pesan atau sesi yang diatur. Profil penyedia/model yang dipilih mengontrol nilai mana yang valid; untuk Google Gemini, `adaptive` mempertahankan berpikir dinamis milik penyedia (`thinkingLevel` dihilangkan pada Gemini 3/3.1, `thinkingBudget: -1` pada Gemini 2.5).
- `reasoningDefault`: visibilitas penalaran default opsional per agen (`on | off | stream`). Menimpa `agents.defaults.reasoningDefault` untuk agen ini ketika tidak ada penimpaan penalaran per pesan atau sesi yang diatur.
- `fastModeDefault`: default opsional per agen untuk mode cepat (`true | false`). Berlaku ketika tidak ada penimpaan mode cepat per pesan atau sesi yang diatur.
- `agentRuntime`: penimpaan kebijakan runtime tingkat rendah opsional per agen. Gunakan `{ id: "codex" }` untuk membuat satu agen khusus Codex sementara agen lain mempertahankan cadangan PI default dalam mode `auto`.
...
- `identity.avatar`: jalur relatif ruang kerja, URL `http(s)`, atau URI `data:`.
- `identity` menurunkan default: `ackReaction` dari `emoji`, `mentionPatterns` dari `name`/`emoji`.
- `subagents.allowAgents`: daftar id agen yang diizinkan untuk target eksplisit `sessions_spawn.agentId` (`["*"]` = apa pun; default: hanya agen yang sama). Sertakan id peminta ketika panggilan `agentId` yang menargetkan diri sendiri harus diizinkan.
...
- `type` (opsional): `route` untuk perutean normal (jenis yang hilang default ke route), `acp` untuk pengikatan percakapan ACP persisten.
- `match.channel` (wajib)
...
Untuk entri `type: "acp"`, OpenClaw menyelesaikan berdasarkan identitas percakapan persis (`match.channel` + akun + `match.peer.id`) dan tidak menggunakan urutan tingkat pengikatan rute di atas.
...
<Accordion title="Tidak ada akses sistem file (hanya perpesanan)">
...
- **`scope`**: strategi pengelompokan sesi dasar untuk konteks obrolan grup.
  - `per-sender` (default): setiap pengirim mendapatkan sesi terisolasi di dalam konteks kanal.
...
- **`identityLinks`**: memetakan id kanonis ke peer berprefiks penyedia untuk berbagi sesi lintas kanal. Perintah dock seperti `/dock_discord` menggunakan peta yang sama untuk mengalihkan rute balasan sesi aktif ke peer kanal tertaut lain; lihat [Penambatan kanal](/id/concepts/channel-docking).
...
- **`maintenance`**: kontrol pembersihan penyimpanan sesi + retensi.
...
- **`threadBindings`**: default global untuk fitur sesi terikat thread.
  - `enabled`: sakelar default utama (penyedia dapat mengganti; Discord menggunakan `channels.discord.threadBindings.enabled`)
  - `idleHours`: auto-unfocus default karena tidak aktif dalam jam (`0` menonaktifkan; penyedia dapat mengganti)
  - `maxAgeHours`: usia maksimum keras default dalam jam (`0` menonaktifkan; penyedia dapat mengganti)
  - `spawnSessions`: gerbang default untuk membuat sesi kerja terikat thread dari `sessions_spawn` dan spawn thread ACP. Default ke `true` saat pengikatan thread diaktifkan; penyedia/akun dapat mengganti.
...
- `messages.statusReactions.enabled`: mengaktifkan reaksi status siklus hidup di Slack, Discord, dan Telegram.
  Di Slack dan Discord, nilai yang tidak disetel membuat reaksi status tetap aktif saat reaksi tanda terima aktif.
  Di Telegram, setel secara eksplisit ke `true` untuk mengaktifkan reaksi status siklus hidup.

### Debounce masuk

Mengelompokkan pesan cepat yang hanya berisi teks dari pengirim yang sama menjadi satu giliran agen. Media/lampiran langsung memicu flush. Perintah kontrol melewati debounce.
...
- Penyedia ucapan bawaan dimiliki oleh plugin. Jika `plugins.allow` disetel, sertakan setiap plugin penyedia TTS yang ingin Anda gunakan, misalnya `microsoft` untuk Edge TTS. Id penyedia lama `edge` diterima sebagai alias untuk `microsoft`.
...
- Kunci Bicara datar lama (`talk.voiceId`, `talk.voiceAliases`, `talk.modelId`, `talk.outputFormat`, `talk.apiKey`) hanya untuk kompatibilitas dan otomatis dimigrasikan ke `talk.providers.<provider>`.
...
- `speechLocale` menetapkan id locale BCP 47 yang digunakan oleh pengenalan ucapan Bicara iOS/macOS. Biarkan tidak disetel untuk menggunakan default perangkat.
- `silenceTimeoutMs` mengontrol berapa lama mode Bicara menunggu setelah pengguna diam sebelum mengirim transkrip. Jika tidak disetel, jendela jeda default platform dipertahankan (`700 ms di macOS dan Android, 900 ms di iOS`).
