...
- **Peristiwa diagnostik** adalah rekaman dalam proses yang terstruktur dan dipancarkan oleh
  Gateway serta Plugin bawaan untuk proses model, alur pesan, sesi, antrean,
  dan exec.
...
| ----------- | ------------------------------------------------------------------------------------------------------------------------------------------ |
| **Metrik** | Counter dan histogram untuk penggunaan token, biaya, durasi proses, alur pesan, lane antrean, status sesi, exec, dan tekanan memori.          |
...
Aktifkan atau nonaktifkan `traces`, `metrics`, dan `logs` secara terpisah. Ketiganya aktif secara default
...
| `OPENCLAW_OTEL_PRELOADED`                                                                                         | Atur ke `1` ketika preload atau proses host lain sudah mendaftarkan SDK OpenTelemetry global. Plugin kemudian melewati siklus hidup NodeSDK miliknya sendiri tetapi tetap menghubungkan listener diagnostik dan menghormati `traces`/`metrics`/`logs`.                |

## Privasi dan penangkapan konten

Konten mentah model/alat **tidak** diekspor secara default. Span membawa
pengidentifikasi berbatas (kanal, penyedia, model, kategori error, id permintaan hanya hash)
...
- **Korelasi permintaan:** Permintaan HTTP Gateway dan frame WebSocket membuat
  ...
  mewarisi trace permintaan secara default, sementara span proses agent dan panggilan model
...
- `openclaw.session.stuck` (counter, attrs: `openclaw.state`; dipancarkan hanya untuk pembukuan sesi basi tanpa pekerjaan aktif)
- `openclaw.session.stuck_age_ms` (histogram, attrs: `openclaw.state`; dipancarkan hanya untuk pembukuan sesi basi tanpa pekerjaan aktif)
...
- `session.stalled`: pekerjaan aktif ada, tetapi eksekusi aktif belum melaporkan
  progres terbaru. Eksekusi tertanam yang macet tetap hanya-dipantau pada awalnya, lalu
  batalkan-kuras setelah `diagnostics.stuckSessionAbortMs` tanpa progres agar giliran
  antrean di belakang jalur dapat dilanjutkan. Jika tidak diatur, ambang pembatalan default ke
  jendela tambahan yang lebih aman, yaitu setidaknya 10 menit dan 5x
  `diagnostics.stuckSessionWarnMs`.
- `session.stuck`: pembukuan sesi kedaluwarsa tanpa pekerjaan aktif. Ini melepaskan
  jalur sesi yang terdampak secara langsung.
...
hanya setelah hasil pemulihan yang mengubah status (`aborted` atau `released`) dan hanya jika
generasi pemrosesan yang sama masih berjalan saat ini.
...
tidak berubah, jadi dasbor sebaiknya memberi peringatan pada kenaikan berkelanjutan, bukan setiap
tik heartbeat. Untuk knob konfigurasi dan nilai default, lihat
