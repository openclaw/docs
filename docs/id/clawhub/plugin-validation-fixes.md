...
x-i18n:
    generated_at: "2026-07-03T17:42:47Z"
    model: gpt-5.5
...
| `channel-env-vars`                      | [Cerminkan env vars saluran di metadata saat ini](/id/clawhub/plugin-validation-fixes#channel-env-vars)                                |
...
Paket mendeklarasikan titik masuk OpenClaw, tetapi file yang direferensikan hilang
...
- Gunakan versi API Plugin OpenClaw atau batas bawah semver yang Anda gunakan untuk
...
- Jika `openclaw.plugin.json` juga memiliki `version`, perbarui agar cocok atau hapus
  metadata versi manifes usang saat metadata paket menjadi sumber kebenaran.
...
titik masuk yang dideklarasikan di `package.json#openclaw`.
...
- Gunakan `openclaw/plugin-sdk/channel-core` untuk helper entry channel.
...
- Gunakan `patchSessionEntry(...)` saat memperbarui field pada entri sesi yang ada.
...
- Gunakan helper identitas transkrip atau target saat kode menyiapkan
  operasi transkrip.
...
Manifest menggunakan metadata env-var channel lama atau lebih tua tanpa metadata
...
- Cerminkan setup channel berbasis env ke setup saat ini, config channel, atau
