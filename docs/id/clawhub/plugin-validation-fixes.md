...
x-i18n:
    generated_at: "2026-07-01T20:35:23Z"
    model: gpt-5.5
...
- Bangun paket jika entrypoint dihasilkan ke `dist`.
...
- Jika `openclaw.plugin.json` juga memiliki `version`, perbarui agar cocok atau hapus
  metadata versi manifes yang usang ketika metadata paket bersifat otoritatif.
...
- Gunakan `patchSessionEntry(...)` saat memperbarui bidang pada entri sesi
  yang sudah ada.
...
- Tetap dokumentasikan perilaku sensitif keamanan di dokumen paket publik Anda atau
