...
read_when:
    ...
    - ClawHub menolak atau memberi peringatan saat penerbitan paket Plugin
    - Anda sedang memperbarui metadata paket Plugin sebelum rilis
summary: Perbaiki temuan validasi paket plugin ClawHub sebelum menerbitkan
...
Ini tidak mencakup temuan cakupan Plugin Inspector internal. Jika laporan lengkap
...
| `manifest-unknown-fields`               | [Hapus kolom manifes yang tidak didukung](/id/clawhub/plugin-validation-fixes#manifest-unknown-fields)                                  |
...
- Lihat [kolom package.json yang memengaruhi penemuan](/id/plugins/manifest#packagejson-fields-that-affect-discovery).
- Jalankan ulang `clawhub package validate <path-to-plugin>`.
...
- Lihat [Titik entri Plugin](/id/plugins/sdk-entrypoints) dan
  [kolom package.json yang memengaruhi penemuan](/id/plugins/manifest#packagejson-fields-that-affect-discovery).
- Jalankan ulang `clawhub package validate <path-to-plugin>`.
...
- Build paket jika entrypoint dihasilkan ke dalam `dist`.
- Perbarui metadata jika entrypoint dipindahkan.
- Lihat [Titik entri Plugin](/id/plugins/sdk-entrypoints).
- Jalankan ulang `clawhub package validate <path-to-plugin>`.
...
- Lebih pilih `package.json#version` sebagai versi rilis paket.
- Jika `openclaw.plugin.json` juga memiliki `version`, perbarui agar cocok atau hapus
  metadata versi manifes usang saat metadata paket menjadi otoritas.
...
Blok `package.json#openclaw` berisi kolom yang tidak didukung sebagai
...
- Perbaiki metadata paket yang tidak valid, skrip lifecycle yang rusak, atau entri file yang
...
Paket dapat dipaketkan, tetapi artefak yang dipaketkan tidak menyertakan file
entrypoint yang dideklarasikan di `package.json#openclaw`.

- Jalankan `npm pack --dry-run` dan periksa file yang akan disertakan.
- Build entrypoint yang dihasilkan sebelum pemaketan.
- Perbarui `files`, `.npmignore`, atau output build agar entrypoint yang
  dideklarasikan disertakan.
- Lihat [Titik entri Plugin](/id/plugins/sdk-entrypoints).
- Jalankan ulang `clawhub package validate <path-to-plugin>`.

### package-npm-pack-metadata-missing

Artefak yang dipaketkan kehilangan metadata OpenClaw yang ada dalam paket
sumber Anda.
...
- Ganti impor barrel root dengan impor subjalur publik yang terfokus.
...
Plugin masih menggunakan helper seluruh penyimpanan sesi yang sudah tidak digunakan
`loadSessionStore`.
...
Plugin masih menggunakan helper tulis seluruh penyimpanan sesi yang sudah tidak digunakan seperti
...
- Hindari memuat, memutasi, dan menyimpan seluruh objek penyimpanan sesi.
- Pertahankan helper tulis seluruh penyimpanan hanya selama rentang kompatibilitas yang Anda deklarasikan
  masih mendukung versi OpenClaw lama yang membutuhkannya.
- Lihat [API runtime](/id/plugins/sdk-runtime#agent-session-state) dan
  [subjalur SDK Plugin](/id/plugins/sdk-subpaths).
- Jalankan ulang `clawhub package validate <path-to-plugin>`.
...
Plugin masih menggunakan helper jalur file sesi yang sudah tidak digunakan seperti
...
- Pindahkan pekerjaan penimpaan model atau penyedia ke `before_model_resolve`.
...
Manifes menggunakan metadata variabel lingkungan kanal lama atau lebih tua tanpa metadata
setup atau konfigurasi saat ini yang diharapkan ClawHub.

- Pertahankan metadata variabel lingkungan kanal secara deklaratif agar OpenClaw dapat memeriksa status setup
  tanpa memuat runtime kanal.
- Cerminkan setup kanal berbasis variabel lingkungan ke metadata setup, konfigurasi kanal, atau
  metadata kanal paket saat ini yang digunakan oleh bentuk Plugin Anda.
...
- Pertahankan perilaku yang sensitif terhadap keamanan terdokumentasi dalam dokumen paket publik atau
