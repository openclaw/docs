...
x-i18n:
    generated_at: "2026-06-30T14:24:30Z"
    model: gpt-5.5
...
temuan yang dapat diperbaiki oleh penulis plugin di metadata paket, manifes, impor SDK,
...
| `channel-env-vars`                      | [Cerminkan env var saluran di metadata saat ini](/id/clawhub/plugin-validation-fixes#channel-env-vars)                         |
...
yang digunakan untuk build paket.
...
- Build entrypoint yang dihasilkan sebelum memaketkan.
...
Plugin masih menggunakan helper seluruh-session-store yang sudah usang
`loadSessionStore`.
...
- Hindari memuat, mengubah, dan menyimpan seluruh objek session store.
...
Plugin masih menggunakan helper penulisan seluruh-session-store yang sudah usang seperti
...
- Hindari memuat, mengubah, dan menyimpan seluruh objek session store.
- Pertahankan helper penulisan seluruh-store hanya selama rentang kompatibilitas yang Anda deklarasikan
  masih mendukung versi OpenClaw lama yang memerlukannya.
- Lihat [API Runtime](/id/plugins/sdk-runtime#agent-session-state) dan
  [Subpath Plugin SDK](/id/plugins/sdk-subpaths).
- Jalankan ulang `clawhub package validate <path-to-plugin>`.
...
- Gunakan identitas transcript atau helper target saat kode menyiapkan operasi transcript.
- Jangan menyimpan atau bergantung pada path file transcript lama.
- Lihat [API Runtime](/id/plugins/sdk-runtime#agent-session-state) dan
  [Subpath Plugin SDK](/id/plugins/sdk-subpaths).
- Jalankan ulang `clawhub package validate <path-to-plugin>`.
...
- Gunakan `resolveSessionTranscriptTarget(...)` saat kode memerlukan target operasi transcript
  terstruktur.
- Hindari membaca atau membuat target file transcript lama secara langsung.
...
- Gunakan `publishSessionTranscriptUpdateByIdentity(...)` untuk notifikasi pembaruan transcript.
...
Manifest menggunakan metadata env-var channel lama atau versi lama tanpa metadata setup
atau config saat ini yang diharapkan ClawHub.

- Jaga agar metadata env-var channel tetap deklaratif sehingga OpenClaw dapat memeriksa status setup
  tanpa memuat runtime channel.
- Cerminkan setup channel berbasis env ke metadata setup saat ini, config channel, atau
...
- Pertahankan `channelEnvVars` hanya sebagai metadata kompatibilitas selama versi OpenClaw
  lama yang didukung masih memerlukannya.
...
- Tetap dokumentasikan perilaku sensitif keamanan dalam dokumentasi paket publik atau
  README Anda sampai kontrak manifest tersedia.
- Jalankan ulang `clawhub package validate <path-to-plugin>`.
...
- [Membuat Plugin](/id/plugins/building-plugins)
