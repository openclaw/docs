...
summary: Perbaiki temuan validasi paket Plugin ClawHub sebelum penerbitan
title: Perbaikan validasi Plugin
x-i18n:
    generated_at: "2026-07-02T17:46:55Z"
    model: gpt-5.5
...
| `channel-env-vars`                      | [Cerminkan variabel env saluran dalam metadata saat ini](/id/clawhub/plugin-validation-fixes#channel-env-vars)                                |
...
- Pastikan `openclaw.plugin.json` disertakan saat paket adalah plugin native
  OpenClaw.
...
Manifes plugin memiliki field tingkat atas yang tidak didukung OpenClaw.

- Bandingkan setiap field tingkat atas dengan
  [referensi field manifes](/id/plugins/manifest#top-level-field-reference).
...
- Pindahkan perilaku runtime ke kode registrasi plugin, dan batasi `contracts`
  pada metadata kepemilikan kapabilitas statis.
- Jalankan ulang `clawhub package validate <path-to-plugin>`.
...
Plugin masih menggunakan helper whole-session-store yang sudah deprecated
`loadSessionStore`.
...
Plugin masih menggunakan helper penulisan whole-session-store yang sudah deprecated seperti
...
- Gunakan `getSessionEntry(...)` untuk membaca metadata sesi berdasarkan agent dan identitas sesi.
- Gunakan `patchSessionEntry(...)` atau `upsertSessionEntry(...)` untuk mempertahankan metadata sesi.
- Gunakan identitas transkrip atau helper target saat kode sedang menyiapkan operasi transkrip.
...
- Gunakan `resolveSessionTranscriptTarget(...)` saat kode memerlukan target operasi transkrip terstruktur.
- Hindari membaca atau membuat target file transkrip legacy secara langsung.
...
- Gunakan `appendSessionTranscriptMessageByIdentity(...)` untuk append transkrip.
...
Manifes masih menggunakan metadata autentikasi provider legacy `providerAuthEnvVars`.
...
Manifes menggunakan metadata env-var channel legacy atau lama tanpa metadata setup
...
- Tetap dokumentasikan perilaku yang sensitif terhadap keamanan di dokumentasi paket publik atau
  README Anda hingga contract manifes tersedia.
- Jalankan ulang `clawhub package validate <path-to-plugin>`.
