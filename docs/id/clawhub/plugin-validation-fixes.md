...
x-i18n:
    generated_at: "2026-06-28T22:32:41Z"
    model: gpt-5.5
...
ditujukan bagi pengelola OpenClaw, bukan penulis plugin.
...
- Jika `openclaw.plugin.json` juga memiliki `version`, perbarui agar cocok atau hapus
  metadata versi manifes lama saat metadata paket menjadi otoritatif.
...
- Simpan entrypoint paket, metadata kompatibilitas, instalasi, penyiapan, dan katalog
...
Paket dapat dikemas, tetapi artefak yang dikemas tidak menyertakan berkas
...
- Bangun entrypoint yang dihasilkan sebelum pengemasan.
...
Manifes menggunakan metadata variabel lingkungan kanal lama atau versi lama tanpa metadata
setup atau konfigurasi saat ini yang diharapkan ClawHub.

- Jaga agar metadata variabel lingkungan kanal tetap deklaratif sehingga OpenClaw dapat memeriksa status setup
...
Paket mengirimkan berkas manifes keamanan yang tidak didukung.

- Hapus `openclaw.security.json` sampai OpenClaw mendokumentasikan skema manifes keamanan
  berversi dan perilaku ClawHub.
- Tetap dokumentasikan perilaku sensitif keamanan di dokumentasi paket publik atau
  README Anda sampai kontrak manifes tersedia.
- Jalankan ulang `clawhub package validate <path-to-plugin>`.

## Terkait

- [ClawHub CLI](/id/clawhub/cli)
