...
read_when:
    ...
    - ClawHub menolak atau memberi peringatan pada publikasi paket plugin
...
x-i18n:
    generated_at: "2026-07-05T08:34:55Z"
    model: gpt-5.5
...
| `manifest-unknown-fields`               | [Hapus field manifes yang tidak didukung](/id/clawhub/plugin-validation-fixes#manifest-unknown-fields)                        |
...
| `channel-env-vars`                      | [Cerminkan env var saluran dalam metadata saat ini](/id/clawhub/plugin-validation-fixes#channel-env-vars)                     |
...
- Lihat [field package.json yang memengaruhi penemuan](/id/plugins/manifest#packagejson-fields-that-affect-discovery).
- Jalankan ulang `clawhub package validate <path-to-plugin>`.
...
- Tambahkan `openclaw.runtimeExtensions` saat paket yang dipublikasikan harus memuat
  JavaScript yang sudah dibangun.
- Simpan semua jalur entrypoint di dalam direktori paket.
- Lihat [Titik masuk Plugin](/id/plugins/sdk-entrypoints) dan
  [field package.json yang memengaruhi penemuan](/id/plugins/manifest#packagejson-fields-that-affect-discovery).
- Jalankan ulang `clawhub package validate <path-to-plugin>`.
...
- Jika `openclaw.plugin.json` juga memiliki `version`, perbarui agar cocok atau hapus
  metadata versi manifes usang saat metadata paket menjadi otoritatif.
...
Blok `package.json#openclaw` berisi field yang bukan metadata paket
OpenClaw yang didukung.

- Hapus field yang tidak didukung seperti `openclaw.bundle`.
...
- Simpan entrypoint paket, kompatibilitas, instalasi, penyiapan, dan metadata katalog
  dalam field `package.json#openclaw` yang didukung.
- Lihat [field package.json yang memengaruhi penemuan](/id/plugins/manifest#packagejson-fields-that-affect-discovery).
- Jalankan ulang `clawhub package validate <path-to-plugin>`.
...
- Pastikan `openclaw.plugin.json` disertakan saat paket adalah Plugin
  OpenClaw native.
...
- Tambahkan field `name` yang tidak kosong ke `openclaw.plugin.json`.
...
Manifest plugin memiliki field tingkat atas yang tidak didukung OpenClaw.

- Bandingkan setiap field tingkat atas dengan
  [referensi field manifest](/id/plugins/manifest#top-level-field-reference).
- Hapus field kustom dari `openclaw.plugin.json`.
- Pindahkan metadata paket atau instalasi ke field `package.json#openclaw` yang didukung
...
Plugin mengimpor dari barrel SDK root yang sudah deprecated:
`openclaw/plugin-sdk`.
...
Plugin masih menggunakan helper seluruh penyimpanan sesi yang sudah deprecated
`loadSessionStore`.
...
Plugin masih menggunakan helper penulisan seluruh penyimpanan sesi yang sudah deprecated seperti
`saveSessionStore` atau `updateSessionStore`.

- Gunakan `patchSessionEntry(...)` saat memperbarui field pada entri sesi yang sudah ada.
...
Plugin masih menggunakan helper path file sesi yang sudah deprecated seperti
...
Plugin masih menggunakan helper target file transkrip yang sudah deprecated
`resolveSessionTranscriptLegacyFileTarget`.

- Gunakan `resolveSessionTranscriptIdentity(...)` saat kode hanya memerlukan identitas sesi publik.
- Gunakan `resolveSessionTranscriptTarget(...)` saat kode memerlukan target operasi transkrip
  terstruktur.
- Hindari membaca atau membangun target file transkrip lama secara langsung.
...
Plugin masih menggunakan helper transkrip low-level yang sudah deprecated seperti
...
- Utamakan surface runtime transkrip terstruktur agar OpenClaw dapat menerapkan
  batas transaksi dan penanganan identitas yang benar.
- Pertahankan helper transkrip low-level hanya selama rentang kompatibilitas yang Anda deklarasikan
  masih mendukung versi OpenClaw lama yang memerlukannya.
- Lihat [API Runtime](/id/plugins/sdk-runtime#agent-session-state) dan
  [subpath SDK Plugin](/id/plugins/sdk-subpaths).
- Jalankan ulang `clawhub package validate <path-to-plugin>`.
...
Manifest masih menggunakan metadata auth provider lama `providerAuthEnvVars`.
...
Manifest menggunakan metadata env-var channel lama atau versi lebih lama tanpa metadata
...
- Hapus `openclaw.security.json` sampai OpenClaw mendokumentasikan skema manifest keamanan
  berversi dan perilaku ClawHub.
- Pertahankan perilaku sensitif keamanan terdokumentasi di dokumen paket publik atau
  README Anda sampai contract manifest tersedia.
- Jalankan ulang `clawhub package validate <path-to-plugin>`.
