...
x-i18n:
    generated_at: "2026-07-02T08:48:50Z"
    model: gpt-5.5
...
pemindaian paket otomatis. Halaman ini mencakup temuan yang ditujukan untuk pembuat, yaitu
temuan yang dapat diperbaiki pembuat plugin dalam metadata paket, manifes, impor SDK,
atau artefak yang dipublikasikan.

Ini tidak mencakup temuan cakupan Plugin Inspector internal. Jika laporan lengkap
berisi kode pemeliharaan pemindai tanpa panduan remediasi untuk pembuat, kode tersebut
ditujukan untuk pengelola OpenClaw, bukan pembuat plugin.
...
## Temuan yang ditujukan untuk pembuat
...
- Tetapkan `openclaw.install.defaultChoice` saat tersedia lebih dari satu sumber
  instalasi.
...
- Pisahkan ini dari versi paket. Versi paket mendeskripsikan rilis
  plugin; `openclaw.compat.pluginApi` mendeskripsikan kontrak API host.
- Lihat [bidang package.json yang memengaruhi penemuan](/id/plugins/manifest#packagejson-fields-that-affect-discovery).
- Jalankan ulang `clawhub package validate <path-to-plugin>`.
...
- Jika `openclaw.plugin.json` juga memiliki `version`, perbarui agar cocok atau hapus
  metadata versi manifes usang saat metadata paket bersifat otoritatif.
...
Plugin masih menggunakan helper transkrip level rendah yang sudah tidak digunakan seperti
...
- Utamakan permukaan runtime transkrip terstruktur agar OpenClaw dapat menerapkan batas transaksi
  dan penanganan identitas yang benar.
- Pertahankan helper transkrip level rendah hanya selama rentang kompatibilitas yang Anda deklarasikan
  masih mendukung versi OpenClaw lama yang membutuhkannya.
- Lihat [API Runtime](/id/plugins/sdk-runtime#agent-session-state) dan
  [subpath SDK Plugin](/id/plugins/sdk-subpaths).
- Jalankan ulang `clawhub package validate <path-to-plugin>`.
...
Manifes menggunakan metadata env-var kanal lama atau lebih tua tanpa metadata setup
atau konfigurasi saat ini yang diharapkan ClawHub.
...
- Pertahankan perilaku sensitif keamanan yang terdokumentasi di dokumen paket publik atau
  README Anda sampai kontrak manifes tersedia.
- Jalankan ulang `clawhub package validate <path-to-plugin>`.
