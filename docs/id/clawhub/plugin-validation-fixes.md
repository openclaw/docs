...
x-i18n:
    generated_at: "2026-07-03T09:59:51Z"
    model: gpt-5.5
...
- Jangan pertahankan atau bergantung pada path file transkrip legacy.
- Lihat [API Runtime](/id/plugins/sdk-runtime#agent-session-state) dan
  [subpath SDK Plugin](/id/plugins/sdk-subpaths).
- Jalankan ulang `clawhub package validate <path-to-plugin>`.
...
- Pertahankan `providerAuthEnvVars` hanya sebagai metadata kompatibilitas selama rentang OpenClaw yang didukung
  masih memerlukannya.
...
Manifest menggunakan metadata env-var channel legacy atau lama tanpa metadata setup atau config saat ini yang diharapkan ClawHub.
