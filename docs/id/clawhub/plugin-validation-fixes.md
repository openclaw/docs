...
x-i18n:
    generated_at: "2026-06-27T17:15:43Z"
    model: gpt-5.5
...
pemindaian paket otomatis. Halaman ini mencakup temuan yang ditujukan untuk penulis, yang berarti
...
| `unrecognized-security-manifest`        | [Hapus berkas manifest keamanan yang tidak didukung](/id/clawhub/plugin-validation-fixes#unrecognized-security-manifest)                   |
...
Paket mendeklarasikan entrypoint OpenClaw, tetapi berkas yang dirujuk tidak ada
...
berkas entrypoint yang dideklarasikan di `package.json#openclaw`.

- Jalankan `npm pack --dry-run` dan periksa berkas yang akan disertakan.
- Bangun entrypoint yang dihasilkan sebelum mengemas.
...
- Buat `name` mudah dibaca manusia dan jadikan `id` sebagai id mesin yang stabil.
- Lihat [Manifest Plugin](/id/plugins/manifest).
- Jalankan ulang `clawhub package validate <path-to-plugin>`.
...
- Pindahkan perilaku runtime ke kode registrasi plugin, dan biarkan `contracts`
  terbatas pada metadata kepemilikan kapabilitas statis.
- Jalankan ulang `clawhub package validate <path-to-plugin>`.
...
- Jaga metadata env-var channel tetap deklaratif agar OpenClaw dapat memeriksa status setup
...
- Tetap dokumentasikan perilaku sensitif keamanan dalam dokumen paket publik atau
