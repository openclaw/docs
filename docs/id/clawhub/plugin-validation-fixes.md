...
read_when:
    ...
    - ClawHub menolak atau memberikan peringatan saat penerbitan paket Plugin
...
x-i18n:
    generated_at: "2026-07-04T18:19:52Z"
    model: gpt-5.5
...
pemindaian paket otomatis. Halaman ini mencakup temuan untuk penulis, yang berarti
...
- Simpan metadata entrypoint paket, kompatibilitas, instalasi, penyiapan, dan katalog
  di kolom `package.json#openclaw` yang didukung.
- Lihat [kolom package.json yang memengaruhi penemuan](/id/plugins/manifest#packagejson-fields-that-affect-discovery).
- Jalankan ulang `clawhub package validate <path-to-plugin>`.
...
- Ganti impor barrel root dengan impor subpath publik yang terfokus.
...
Plugin masih menggunakan helper penulisan seluruh penyimpanan sesi yang sudah tidak digunakan seperti
...
- Pertahankan perilaku yang sensitif terhadap keamanan terdokumentasi di dokumen paket publik Anda atau
  README sampai kontrak manifes tersedia.
- Jalankan ulang `clawhub package validate <path-to-plugin>`.
