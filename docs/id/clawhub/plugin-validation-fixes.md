...
x-i18n:
    generated_at: "2026-07-05T05:35:36Z"
    model: gpt-5.5
...
| `channel-env-vars`                      | [Cerminkan env var kanal dalam metadata saat ini](/id/clawhub/plugin-validation-fixes#channel-env-vars)                                |
...
- Periksa setiap path dalam `openclaw.extensions`, `openclaw.runtimeExtensions`,
...
- Gunakan versi API Plugin OpenClaw atau batas bawah semver yang Anda jadikan dasar build dan pengujian.
...
- Jika `openclaw.plugin.json` juga memiliki `version`, perbarui agar cocok atau hapus
  metadata versi manifes yang usang saat metadata paket menjadi sumber otoritatif.
...
Plugin masih menggunakan helper penulisan seluruh penyimpanan sesi yang sudah usang seperti
...
- Gunakan `getSessionEntry(...)` untuk membaca metadata sesi berdasarkan agen dan identitas
  sesi.
...
- Cerminkan metadata variabel env penyedia ke `setup.providers[].envVars`.
...
Manifes menggunakan metadata variabel env kanal lama atau versi lebih lama tanpa metadata
setup atau konfigurasi saat ini yang diharapkan ClawHub.

- Pertahankan metadata variabel env kanal secara deklaratif agar OpenClaw dapat memeriksa status setup
...
- Dokumentasikan perilaku sensitif keamanan di dokumentasi paket publik atau
