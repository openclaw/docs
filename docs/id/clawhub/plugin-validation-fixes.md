...
x-i18n:
    generated_at: "2026-07-03T02:56:28Z"
    model: gpt-5.5
...
Artefak yang dikemas tidak memiliki metadata OpenClaw yang ada dalam sumber
paket Anda.
...
- Pastikan `openclaw.plugin.json` disertakan saat paket adalah Plugin OpenClaw native.
...
- Pindahkan metadata paket atau pemasangan ke bidang `package.json#openclaw` yang didukung,
  bukan ke manifest.
- Jalankan ulang `clawhub package validate <path-to-plugin>`.
...
Plugin masih menggunakan helper tulis whole-session-store yang sudah tidak digunakan seperti
...
Manifest masih menggunakan metadata auth penyedia legacy `providerAuthEnvVars`.
...
- Cerminkan setup channel berbasis env ke metadata setup, config channel, atau
  package channel saat ini yang digunakan oleh bentuk plugin Anda.
