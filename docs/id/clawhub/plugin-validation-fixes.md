...
read_when:
    ...
    - ClawHub menolak atau memberi peringatan saat publikasi paket plugin
...
x-i18n:
    generated_at: "2026-07-04T06:52:04Z"
    model: gpt-5.5
...
- Gunakan versi API plugin OpenClaw atau batas bawah semver yang Anda gunakan saat membangun dan menguji.
...
yang menjadi basis build paket tersebut.
...
- Gunakan `openclaw/plugin-sdk/channel-core` untuk pembantu entri channel.
...
Plugin masih menggunakan pembantu whole-session-store yang sudah tidak digunakan
`loadSessionStore`.
...
Plugin masih menggunakan pembantu penulisan whole-session-store yang sudah tidak digunakan seperti
...
- Hindari memuat, mengubah, dan menyimpan seluruh objek penyimpanan sesi.
- Pertahankan pembantu penulisan whole-store hanya selama rentang kompatibilitas yang Anda deklarasikan
  masih mendukung versi OpenClaw lama yang memerlukannya.
- Lihat [API Runtime](/id/plugins/sdk-runtime#agent-session-state) dan
  [subpath SDK Plugin](/id/plugins/sdk-subpaths).
- Jalankan ulang `clawhub package validate <path-to-plugin>`.
...
Plugin masih menggunakan pembantu path file sesi yang sudah tidak digunakan seperti
...
- Gunakan identitas transkrip atau pembantu target saat kode sedang menyiapkan operasi
  transkrip.
...
- Utamakan permukaan runtime transkrip terstruktur agar OpenClaw dapat menerapkan batas
  transaksi dan penanganan identitas yang benar.
...
Manifest masih menggunakan metadata auth penyedia lama `providerAuthEnvVars`.
...
Manifest menggunakan metadata env-var channel lama atau lebih lama tanpa metadata
