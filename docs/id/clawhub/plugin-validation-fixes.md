...
x-i18n:
    generated_at: "2026-07-02T22:49:05Z"
    model: gpt-5.5
...
| `package-install-metadata-incomplete`   | [Lengkapi metadata pemasangan](/id/clawhub/plugin-validation-fixes#package-install-metadata-incomplete)                               |
...
- Tambahkan metadata kompatibilitas dan pemasangan saat paket akan dipublikasikan atau
  dipasang melalui ClawHub.
- Lihat [bidang package.json yang memengaruhi penemuan](/id/plugins/manifest#packagejson-fields-that-affect-discovery).
- Jalankan ulang `clawhub package validate <path-to-plugin>`.

### package-openclaw-entry-missing

Metadata paket ada, tetapi tidak mendeklarasikan titik masuk runtime OpenClaw.
...
ClawHub tidak dapat menentukan bagaimana paket harus dipasang atau diperbarui.

- Isi `openclaw.install` dengan sumber pemasangan yang didukung, seperti
  `clawhubSpec`, `npmSpec`, atau `localPath`.
- Atur `openclaw.install.defaultChoice` saat lebih dari satu sumber pemasangan
  tersedia.
...
- Jika `openclaw.plugin.json` juga memiliki `version`, perbarui agar cocok atau hapus
  metadata versi manifes yang usang saat metadata paket menjadi otoritatif.
...
Blok `package.json#openclaw` berisi bidang yang bukan metadata paket OpenClaw
yang didukung.

- Hapus bidang yang tidak didukung seperti `openclaw.bundle`.
- Simpan metadata Plugin native di `openclaw.plugin.json`.
- Simpan titik masuk paket, kompatibilitas, pemasangan, penyiapan, dan metadata katalog
...
- Build titik masuk yang dibuat sebelum memaketkan.
...
- Pindahkan perilaku runtime ke kode registrasi Plugin, dan batasi `contracts`
  hanya pada metadata kepemilikan kemampuan statis.
- Jalankan ulang `clawhub package validate <path-to-plugin>`.
...
Manifes masih menggunakan metadata auth penyedia lama `providerAuthEnvVars`.
...
Manifes menggunakan metadata env-var kanal lama atau versi lama tanpa metadata
setup atau konfigurasi saat ini yang diharapkan ClawHub.

- Pertahankan metadata env-var kanal secara deklaratif agar OpenClaw dapat memeriksa status setup
  tanpa memuat runtime kanal.
- Cerminkan setup kanal berbasis env ke setup saat ini, konfigurasi kanal, atau
  metadata kanal paket yang digunakan oleh bentuk Plugin Anda.
...
- Pertahankan perilaku yang sensitif terhadap keamanan agar terdokumentasi di dokumentasi paket publik atau
