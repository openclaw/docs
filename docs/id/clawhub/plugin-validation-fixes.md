...
- Tambahkan `openclaw.runtimeExtensions` ketika paket yang dipublikasikan harus memuat JavaScript
  hasil build.
...
Artefak yang dipaketkan tidak memiliki metadata OpenClaw yang ada di paket
sumber Anda.
...
- Pertahankan `name` agar mudah dibaca manusia dan pertahankan `id` sebagai id mesin yang stabil.
- Lihat [Manifes Plugin](/id/plugins/manifest).
- Jalankan ulang `clawhub package validate <path-to-plugin>`.
...
- Gunakan `openclaw/plugin-sdk/channel-core` untuk helper entri saluran.
- Gunakan [Konvensi impor](/id/plugins/building-plugins#import-conventions) dan
  [Subjalur Plugin SDK](/id/plugins/sdk-subpaths) untuk menemukan impor yang sempit.
- Jalankan ulang `clawhub package validate <path-to-plugin>`.
...
- Gunakan [Subjalur Plugin SDK](/id/plugins/sdk-subpaths) dan
...
- Lihat [API runtime](/id/plugins/sdk-runtime#agent-session-state) dan
  [Subjalur Plugin SDK](/id/plugins/sdk-subpaths).
- Jalankan ulang `clawhub package validate <path-to-plugin>`.
...
Manifes menggunakan metadata env-var saluran lama atau versi lama tanpa metadata
setup atau konfigurasi saat ini yang diharapkan ClawHub.

- Pertahankan metadata env-var saluran tetap deklaratif agar OpenClaw dapat memeriksa status setup
  tanpa memuat runtime saluran.
- Cerminkan setup saluran berbasis env ke setup, konfigurasi saluran, atau
  metadata saluran paket saat ini yang digunakan oleh bentuk Plugin Anda.
...
- Pertahankan perilaku sensitif keamanan tetap terdokumentasi di dokumentasi paket publik atau
