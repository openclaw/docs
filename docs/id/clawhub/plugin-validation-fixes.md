...
pemindaian paket otomatis. Halaman ini membahas temuan yang ditujukan kepada penulis,
yang berarti temuan yang dapat diperbaiki penulis Plugin di metadata paket, manifest, impor SDK,
...
- Periksa metadata build OpenClaw apa pun di dalam paket, seperti versi OpenClaw
...
- Jika `openclaw.plugin.json` juga memiliki `version`, perbarui agar cocok atau hapus
  metadata versi manifest usang saat metadata paket bersifat otoritatif.
...
- Simpan metadata entrypoint paket, kompatibilitas, instalasi, penyiapan, dan katalog
  di field `package.json#openclaw` yang didukung.
- Lihat [field package.json yang memengaruhi discovery](/id/plugins/manifest#packagejson-fields-that-affect-discovery).
- Jalankan ulang `clawhub package validate <path-to-plugin>`.
...
Artefak yang dipaketkan kehilangan metadata OpenClaw yang ada di paket
sumber Anda.

- Jalankan `npm pack --dry-run` dan periksa file metadata yang disertakan.
- Pastikan `package.json` menyertakan blok `openclaw` di artefak yang dipaketkan.
...
- Jika perilakunya tidak memiliki SDK publik, pertahankan helper di dalam paket Anda atau
...
Manifest masih menggunakan metadata autentikasi penyedia lama `providerAuthEnvVars`.

- Cerminkan metadata env-var penyedia ke `setup.providers[].envVars`.
- Pertahankan `providerAuthEnvVars` hanya sebagai metadata kompatibilitas selama rentang OpenClaw yang
  Anda dukung masih memerlukannya.
...
Manifest menggunakan metadata env-var kanal lama atau versi lama tanpa metadata setup atau konfigurasi saat ini yang diharapkan ClawHub.
...
- Cerminkan setup kanal berbasis env ke metadata setup, konfigurasi kanal, atau
  paket kanal saat ini yang digunakan oleh bentuk plugin Anda.
...
- Pertahankan perilaku sensitif keamanan yang terdokumentasi di dokumen paket publik atau
