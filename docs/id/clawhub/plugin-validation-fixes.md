...
x-i18n:
    generated_at: "2026-06-30T22:35:09Z"
    model: gpt-5.5
...
pemindaian paket otomatis. Halaman ini membahas temuan yang ditujukan untuk author, yaitu
temuan yang dapat diperbaiki author Plugin di metadata paket, manifest, impor SDK,
atau artefak yang dipublikasikan.

Halaman ini tidak membahas temuan cakupan Plugin Inspector internal. Jika laporan lengkap
berisi kode pemeliharaan pemindai tanpa panduan remediasi untuk author, kode tersebut
ditujukan untuk maintainer OpenClaw, bukan author Plugin.
...
| `manifest-unknown-fields`               | [Hapus field manifest yang tidak didukung](/id/clawhub/plugin-validation-fixes#manifest-unknown-fields)                                  |
...
| `sdk-session-transcript-low-level`      | [Ganti helper transkrip level rendah](/id/clawhub/plugin-validation-fixes#sdk-session-transcript-low-level)                       |
| `legacy-before-agent-start`             | [Ganti before_agent_start](/id/clawhub/plugin-validation-fixes#legacy-before-agent-start)                                        |
| `provider-auth-env-vars`                | [Pindahkan env var provider ke metadata penyiapan](/id/clawhub/plugin-validation-fixes#provider-auth-env-vars)                             |
...
- Lihat [field package.json yang memengaruhi discovery](/id/plugins/manifest#packagejson-fields-that-affect-discovery).
- Jalankan ulang `clawhub package validate <path-to-plugin>`.
...
- Lihat [Titik masuk Plugin](/id/plugins/sdk-entrypoints) dan
  [field package.json yang memengaruhi discovery](/id/plugins/manifest#packagejson-fields-that-affect-discovery).
- Jalankan ulang `clawhub package validate <path-to-plugin>`.
...
- Gunakan versi API Plugin OpenClaw atau batas bawah semver yang Anda build dan uji
  terhadapnya.
...
Versi paket dan versi manifest Plugin tidak sama.
...
- Jika `openclaw.plugin.json` juga memiliki `version`, perbarui agar cocok atau hapus
  metadata versi manifest usang saat metadata paket menjadi sumber otoritatif.
...
- Perbaiki metadata paket yang tidak valid, lifecycle script yang rusak, atau entri files yang
...
Manifest plugin memiliki kolom tingkat atas yang tidak didukung OpenClaw.
...
- Gunakan `patchSessionEntry(...)` saat memperbarui kolom pada entri sesi yang ada.
...
- Gunakan identitas transkrip atau helper target saat kode sedang menyiapkan operasi
  transkrip.
- Jangan mempertahankan atau bergantung pada path file transkrip legacy.
- Lihat [API Runtime](/id/plugins/sdk-runtime#agent-session-state) dan
  [Subpath SDK Plugin](/id/plugins/sdk-subpaths).
- Jalankan ulang `clawhub package validate <path-to-plugin>`.
...
- Gunakan `resolveSessionTranscriptIdentity(...)` saat kode hanya membutuhkan identitas sesi publik.
- Gunakan `resolveSessionTranscriptTarget(...)` saat kode membutuhkan target operasi transkrip
  terstruktur.
- Hindari membaca atau menyusun target file transkrip legacy secara langsung.
...
Manifest masih menggunakan metadata autentikasi provider legacy `providerAuthEnvVars`.
...
Manifest menggunakan metadata env-var channel legacy atau lama tanpa metadata setup atau
...
- Hapus URL skema jika hanya bersifat saran.
...
- Hapus `openclaw.security.json` hingga OpenClaw mendokumentasikan skema manifest keamanan
  berversi dan perilaku ClawHub.
- Simpan perilaku sensitif keamanan yang terdokumentasi di dokumen paket publik atau
  README Anda hingga kontrak manifest tersedia.
- Jalankan ulang `clawhub package validate <path-to-plugin>`.
