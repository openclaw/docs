...
x-i18n:
    generated_at: "2026-07-02T01:15:08Z"
    model: gpt-5.5
...
temuan yang dapat diperbaiki penulis plugin dalam metadata paket, manifest, impor SDK,
...
- Pertahankan semua jalur entrypoint di dalam direktori paket.
...
- Gunakan versi API plugin OpenClaw atau batas bawah semver yang Anda bangun dan uji
  terhadapnya.
- Jaga ini tetap terpisah dari versi paket. Versi paket menjelaskan
...
Blok `package.json#openclaw` berisi kolom yang bukan metadata paket OpenClaw
yang didukung.
...
Plugin masih menggunakan helper seluruh penyimpanan sesi yang sudah usang
`loadSessionStore`.
...
Plugin masih menggunakan helper tulis seluruh penyimpanan sesi yang sudah usang seperti
...
- Gunakan `getSessionEntry(...)` untuk membaca metadata sesi berdasarkan identitas
  agen dan sesi.
- Gunakan `patchSessionEntry(...)` atau `upsertSessionEntry(...)` untuk mempertahankan
  metadata sesi.
...
- Gunakan `resolveSessionTranscriptIdentity(...)` saat kode hanya membutuhkan identitas
  sesi publik.
- Gunakan `resolveSessionTranscriptTarget(...)` saat kode membutuhkan target operasi
...
Manifest masih menggunakan metadata autentikasi penyedia legacy `providerAuthEnvVars`.
...
Manifest menggunakan metadata env-var channel legacy atau lama tanpa metadata
