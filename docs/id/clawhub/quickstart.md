...
x-i18n:
    generated_at: "2026-07-02T17:47:17Z"
    model: gpt-5.5
...
ClawHub adalah registri untuk keterampilan dan plugin OpenClaw.
...
## Temukan dan pasang keterampilan
...
Perbarui keterampilan yang terpasang:

```bash
openclaw skills update --all
```

OpenClaw mencatat asal keterampilan tersebut sehingga pembaruan berikutnya dapat
...
Keterampilan adalah folder dengan file `SKILL.md` wajib dan file pendukung
opsional.
...
Perintah ini melewati konten yang tidak berubah. Keterampilan baru dimulai dari
`1.0.0`; perubahan berikutnya secara otomatis menerbitkan versi patch berikutnya.
...
dibutuhkan keterampilan tersebut sebelum mereka memasangnya. Lihat [Format keterampilan](/id/clawhub/skill-format).

Untuk repositori yang berisi beberapa keterampilan, alur kerja GitHub yang dapat
digunakan ulang memanggil `skill publish` untuk setiap folder keterampilan
langsung di bawah `skills/`:
...
Gunakan `--dry-run` terlebih dahulu untuk mempratinjau metadata paket yang
...
pemasangan hingga diselesaikan.
