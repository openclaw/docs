...
x-i18n:
    generated_at: "2026-07-02T01:16:03Z"
    model: gpt-5.5
...
Pasang plugin yang di-host di ClawHub dengan sumber ClawHub eksplisit:
...
Skill adalah folder dengan berkas wajib `SKILL.md` dan berkas pendukung
opsional.
...
Perintah ini melewati konten yang tidak berubah. Skill baru dimulai dari
`1.0.0`; perubahan berikutnya secara otomatis menerbitkan versi patch
berikutnya. Gunakan `--dry-run` untuk meninjau pratinjau atau `--version` untuk
memilih versi eksplisit.
...
dibutuhkan skill sebelum memasangnya. Lihat [Format skill](/id/clawhub/skill-format).

Untuk repositori yang berisi beberapa skill, alur kerja GitHub yang dapat
digunakan kembali memanggil `skill publish` untuk setiap folder skill langsung
di bawah `skills/`:
...
Gunakan `--dry-run` terlebih dahulu untuk meninjau pratinjau metadata paket yang
di-resolve, kolom kompatibilitas, atribusi sumber, dan rencana unggah tanpa
menerbitkan.
