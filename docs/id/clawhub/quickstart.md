...
x-i18n:
    generated_at: "2026-07-02T08:49:55Z"
    model: gpt-5.5
...
ClawHub adalah registri untuk OpenClaw skills dan plugins.
...
Pasang plugin yang di-host ClawHub dengan sumber ClawHub eksplisit:

```bash
openclaw plugins install clawhub:<package>
```

Perbarui plugins yang terpasang:
...
Perintah ini melewati konten yang tidak berubah. Skills baru dimulai dari `1.0.0`; perubahan berikutnya
...
variabel lingkungan, tools, dan permissions yang diperlukan agar pengguna dapat memahami apa yang
dibutuhkan skill sebelum mereka memasangnya. Lihat [Format skill](/id/clawhub/skill-format).

Untuk repositori yang berisi beberapa skills, workflow GitHub yang dapat digunakan ulang memanggil
...
Gunakan `--dry-run` terlebih dahulu untuk mempratinjau metadata package yang di-resolve, bidang
...
moderasi mungkin disembunyikan dari permukaan pencarian dan pemasangan hingga terselesaikan.
