...
| `always`           | `boolean`  | Jika `true`, skill selalu aktif (tidak perlu pemasangan eksplisit).                                                                                |
...
Jika skill Anda membutuhkan dependensi yang dipasang, deklarasikan di array `install`:
...
Analisis keamanan ClawHub memeriksa bahwa apa yang dideklarasikan skill Anda cocok dengan apa yang sebenarnya dilakukannya. Jika kode Anda merujuk ke `TODOIST_API_KEY` tetapi frontmatter Anda tidak mendeklarasikannya di bawah `requires.env`, `primaryEnv`, atau `envVars`, analisis akan menandai ketidaksesuaian metadata. Menjaga deklarasi tetap akurat membantu skill Anda lulus peninjauan dan membantu pengguna memahami apa yang mereka pasang.
...
- Tipe konten yang dimulai dengan `text/` diperlakukan sebagai teks; ditambah daftar izin kecil (JSON/YAML/TOML/JS/TS/Markdown/SVG).
...
- Semua skills yang diterbitkan di ClawHub dilisensikan di bawah `MIT-0`.
