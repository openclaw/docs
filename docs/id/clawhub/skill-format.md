...
- `.clawhubignore` (pola pengabaian untuk publikasi, legacy `.clawdhubignore`)
- `.gitignore` (juga dihormati)

## Impor GitHub

Pengimpor GitHub web lebih ketat daripada publikasi/sinkronisasi lokal. Ia hanya menemukan file
`SKILL.md` atau `skills.md` legacy di repositori publik, non-fork, yang dimiliki oleh
akun GitHub yang masuk. Ia tidak mengimpor repo privat, fork,
...
Metadata skill dideklarasikan dalam frontmatter YAML di bagian atas `SKILL.md` Anda. Ini memberi tahu registry (dan analisis keamanan) apa yang diperlukan skill Anda untuk berjalan.
...
| `requires.config`  | `string[]` | Jalur file config yang dibaca skill Anda.                                                                                                          |
...
| `homepage`         | `string`   | URL ke beranda atau dokumen skill.                                                                                                         |
...
Jika skill Anda memerlukan dependensi yang terinstal, deklarasikan dalam array `install`:
...
Analisis keamanan ClawHub memeriksa bahwa apa yang dideklarasikan skill Anda cocok dengan apa yang benar-benar dilakukannya. Jika kode Anda merujuk `TODOIST_API_KEY` tetapi frontmatter Anda tidak mendeklarasikannya di bawah `requires.env`, `primaryEnv`, atau `envVars`, analisis akan menandai ketidakcocokan metadata. Menjaga deklarasi tetap akurat membantu skill Anda lolos peninjauan dan membantu pengguna memahami apa yang mereka instal.
...
- ClawHub tidak mendukung skill berbayar, penetapan harga per skill, paywall, atau bagi hasil pendapatan.
