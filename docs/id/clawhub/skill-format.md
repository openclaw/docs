...
x-i18n:
    generated_at: "2026-05-13T04:18:32Z"
    model: gpt-5.5
    provider: openai
...
- `.clawhubignore` (pola abaikan untuk publikasi/sinkronisasi, warisan `.clawdhubignore`)
...
| `os`               | `string[]` | Batasan OS (misalnya `["macos"]`, `["linux"]`).                                                                                                   |
...
- Jika skill Anda berintegrasi dengan layanan pihak ketiga berbayar, dokumentasikan biaya eksternal dan akun yang dibutuhkan secara jelas dalam instruksi skill dan deklarasi env (`requires.env` untuk variabel wajib, atau `envVars` dengan `required: false` untuk variabel opsional).
