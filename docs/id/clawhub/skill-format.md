...
x-i18n:
    generated_at: "2026-07-05T08:29:08Z"
    model: gpt-5.5
...
Metadata keterampilan dideklarasikan dalam frontmatter YAML di bagian atas `SKILL.md` Anda. Ini memberi tahu registri (dan analisis keamanan) apa yang dibutuhkan keterampilan Anda untuk berjalan.
...
| `skillKey`         | `string`   | Menimpa kunci pemanggilan keterampilan.                                                                                                   |
...
Jika keterampilan Anda membutuhkan dependensi yang dipasang, deklarasikan dependensi tersebut dalam array `install`:
...
Deklarasikan variabel lingkungan opsional di bawah `metadata.openclaw.envVars` dan tetapkan `required: false`. Jangan tambahkan entri opsional ke `requires.env`, karena `requires.env` berarti keterampilan tidak dapat berjalan tanpa variabel tersebut.
...
- Siapa pun dapat menggunakan, memodifikasi, dan mendistribusikan ulang keterampilan yang dipublikasikan, termasuk secara komersial.
- Atribusi tidak diperlukan.
- Jangan tambahkan ketentuan lisensi yang bertentangan di `SKILL.md`; ClawHub tidak mendukung override lisensi per keterampilan.
