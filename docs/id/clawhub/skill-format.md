...
x-i18n:
    generated_at: "2026-07-03T01:05:23Z"
    model: gpt-5.5
...
- `.clawhubignore` (pola abaikan untuk penerbitan, `.clawdhubignore` lama)
...
| `requires.config`  | `string[]` | Path file config yang dibaca oleh skill Anda.                                                                                             |
...
Analisis keamanan ClawHub memeriksa bahwa apa yang dideklarasikan skill Anda sesuai dengan apa yang benar-benar dilakukannya. Jika kode Anda merujuk ke `TODOIST_API_KEY` tetapi frontmatter Anda tidak mendeklarasikannya di bawah `requires.env`, `primaryEnv`, atau `envVars`, analisis akan menandai ketidaksesuaian metadata. Menjaga deklarasi tetap akurat membantu skill Anda lolos review dan membantu pengguna memahami apa yang mereka instal.
