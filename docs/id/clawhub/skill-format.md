...
x-i18n:
    generated_at: "2026-07-04T18:20:42Z"
    model: gpt-5.5
...
Gunakan `requires.env` untuk variabel lingkungan yang harus tersedia sebelum skill dapat berjalan. Gunakan `envVars` ketika Anda memerlukan metadata per variabel, termasuk variabel opsional dengan `required: false`.
...
Jika skill Anda memerlukan dependensi yang diinstal, deklarasikan di array `install`:
...
- File script tetap dipindai setelah upload; file PowerShell `.ps1`, `.psm1`, dan `.psd1` diterima sebagai teks.
