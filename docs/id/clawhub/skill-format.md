...
summary: Format folder Skill, berkas yang wajib ada, jenis berkas yang diizinkan, batasan.
x-i18n:
    generated_at: "2026-07-01T15:31:39Z"
    model: gpt-5.5
...
Gunakan `requires.env` untuk variabel lingkungan yang harus tersedia sebelum skill dapat berjalan. Gunakan `envVars` ketika Anda membutuhkan metadata per variabel, termasuk variabel opsional dengan `required: false`.
...
Analisis keamanan ClawHub memeriksa bahwa apa yang dideklarasikan skill Anda sesuai dengan apa yang sebenarnya dilakukannya. Jika kode Anda mereferensikan `TODOIST_API_KEY` tetapi frontmatter Anda tidak mendeklarasikannya di bawah `requires.env`, `primaryEnv`, atau `envVars`, analisis akan menandai ketidakcocokan metadata. Menjaga deklarasi tetap akurat membantu skill Anda lolos peninjauan dan membantu pengguna memahami apa yang mereka instal.
...
- ClawHub tidak mendukung skill berbayar, harga per skill, paywall, atau bagi hasil pendapatan.
