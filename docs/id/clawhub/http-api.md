...
Direktori pihak ketiga boleh menggunakan endpoint baca publik untuk mencantumkan atau mencari skill ClawHub. Harap cache hasil, patuhi `429`/`Retry-After`, tautkan pengguna kembali ke listing ClawHub kanonis (`https://clawhub.ai/<owner>/skills/<slug>`), dan hindari memberi kesan bahwa ClawHub mendukung situs pihak ketiga tersebut. Jangan mencoba mencerminkan konten tersembunyi, privat, atau diblokir moderasi di luar permukaan API publik.

Pintasan slug web di-resolve lintas keluarga registri, tetapi klien API sebaiknya menggunakan
...
- Endpoint tulis terautentikasi tidak boleh mengembalikan `Unauthorized` polos saat
...
- Jika `Retry-After` ada, tunggu selama itu dalam detik sebelum mencoba ulang.
...
- Popularitas diskalakan log dan dibatasi. Skill dengan engagement tinggi dapat berperingkat lebih rendah saat teks kueri merupakan kecocokan yang lebih lemah.
...
- Masukkan istilah yang secara literal akan dicari pengguna dalam nama tampilan, ringkasan, dan tag. Gunakan token slug yang berdiri sendiri hanya saat itu juga merupakan identitas stabil yang ingin Anda pertahankan.
...
- Alias penggantian nama mempertahankan resolusi untuk URL lama dan instalasi yang di-resolve melalui registri, tetapi peringkat pencarian didasarkan pada metadata skill kanonis setelah penggantian nama diindeks. Statistik yang ada tetap bersama skill.
...
- `recommended` menggunakan sinyal engagement dan resensi.
...
- Mengembalikan `queue.queuedAhead` dan `queue.position` saat masih antre agar klien dapat menampilkan berapa banyak pemindaian manual prioritas yang berada di depan permintaan. Antrean yang sangat besar dibatasi dan dilaporkan dengan `queuedAheadIsEstimate: true`.
...
- Identitas skill, identitas penerbit, dan metadata versi yang dipilih adalah field amplop tingkat atas (`slug`, `displayName`, `publisherHandle`, `version`, `resolvedFrom`, `tag`, `createdAt`) agar otomasi shell dapat membacanya tanpa membongkar wrapper bersarang.
...
- `security.signals.dependencyRegistry` dipertahankan untuk kompatibilitas respons v1, tetapi pemindai keberadaan registry dependensi telah dihentikan dan kunci ini selalu `null`.
- Ketiadaan Kartu Skill tidak memengaruhi `ok`, `decision`, atau `reasons` endpoint ini; klien sebaiknya membaca `skill-card.md` terinstal secara lokal ketika mereka membutuhkan konten kartu.
...
- `category` (opsional): filter kategori Plugin. Didukung hanya saat
...
- Pemanggil yang terautentikasi dapat melihat paket privat untuk penerbit yang mereka ikuti dalam hasil daftar/pencarian.
...
- Pemanggil anonim hanya melihat kanal paket publik.
- Pemanggil yang terautentikasi dapat mencari paket privat untuk penerbit yang mereka ikuti.
- `channel=private` hanya mengembalikan paket yang dapat dibaca oleh pemanggil yang terautentikasi.
...
- Endpoint ini tepat-versi. Klien sebaiknya memanggilnya setelah me-resolve
...
- Endpoint ini sengaja lebih sempit daripada endpoint moderasi pemilik/moderator.
  ...
  identitas pelapor, isi laporan, bukti privat, atau lini waktu peninjauan internal.
...
ditautkan ke versi. Laporan ini masuk ke antrean moderasi tetapi tidak otomatis menyembunyikan atau
...
- Handoff skill yang didukung GitHub tidak mem-proxy atau mencerminkan byte. Respons JSON
...
Khusus admin. Memulihkan penerbit personal untuk prinsipal GitHub OAuth pengganti yang telah diverifikasi
tanpa mengedit baris akun Convex Auth. Permintaan harus menyebutkan kedua id akun
penyedia GitHub yang immutable; handle yang dapat berubah hanya digunakan sebagai penjaga yang terlihat oleh operator.
...
prinsipal GitHub. Pemulihan gagal dengan mode tertutup ketika penerbit personal pengguna tujuan saat ini
...
jalur pemilik langsung selaras dengan otoritas penerbit baru. Reservasi protected-handle aktif
untuk handle yang dipulihkan juga dialihkan ke pengguna pengganti sehingga sinkronisasi profil nanti
