...
x-i18n:
    generated_at: "2026-06-28T10:01:39Z"
    model: gpt-5.5
...
Direktori pihak ketiga dapat menggunakan endpoint baca publik untuk mencantumkan atau mencari Skills ClawHub. Harap cache hasil, patuhi `429`/`Retry-After`, arahkan pengguna kembali ke daftar ClawHub kanonis (`https://clawhub.ai/<owner>/skills/<slug>`), dan hindari menyiratkan dukungan ClawHub terhadap situs pihak ketiga. Jangan mencoba mencerminkan konten tersembunyi, privat, atau diblokir moderasi di luar permukaan API publik.

Pintasan slug web diselesaikan lintas keluarga registri, tetapi klien API sebaiknya menggunakan
...
- Endpoint tulis terautentikasi tidak boleh mengembalikan `Unauthorized` polos ketika
  ...
  akun yang dihapus/dibanned/dinonaktifkan masing-masing harus mendapatkan teks yang dapat ditindaklanjuti agar klien
...
- Jika tidak ada IP klien tepercaya yang tersedia, permintaan anonim menggunakan bucket fallback
  yang hanya dicakup berdasarkan jenis batas laju. Bucket fallback ini tidak menyertakan
  path, slug, nama package, versi, string kueri, atau parameter
  artefak lain yang disediakan pemanggil.

## Respons error

Respons error v1 publik berupa teks polos dengan `content-type: text/plain; charset=utf-8`.
Ini mencakup kegagalan validasi (`400`), resource publik yang hilang (`404`), kegagalan auth dan
izin (`401`/`403`), batas laju (`429`), dan unduhan yang diblokir. Klien
sebaiknya membaca body respons sebagai string yang dapat dibaca manusia. Parameter kueri tidak dikenal
...
- Teks ASCII ditokenisasi pada batas kata dan tanda baca. Misalnya, `personal-map` berisi token `map` yang berdiri sendiri, sementara `amap-jsapi-skill` berisi `amap`, `jsapi`, dan `skill`; karena itu pencarian `map` memberi `personal-map` kecocokan leksikal yang lebih kuat daripada `amap-jsapi-skill`.
- Popularitas diskalakan log dan dibatasi. Skills dengan engagement tinggi dapat memiliki peringkat lebih rendah ketika teks kueri adalah kecocokan yang lebih lemah.
...
- Letakkan istilah yang benar-benar akan dicari pengguna di nama tampilan, ringkasan, dan tag. Gunakan token slug yang berdiri sendiri hanya ketika itu juga identitas stabil yang ingin Anda pertahankan.
...
- Alias penggantian nama mempertahankan resolusi untuk URL lama dan instalasi yang diselesaikan melalui registri, tetapi peringkat pencarian didasarkan pada metadata Skill kanonis setelah penggantian nama terindeks. Statistik yang ada tetap bersama Skill.
- Jika suatu Skill tiba-tiba tidak terlihat, periksa status moderasi terlebih dahulu dengan `clawhub inspect @owner/slug` saat login sebelum mengubah metadata terkait peringkat.
...
- Ketika `nonSuspiciousOnly=true`, sort berbasis kursor dapat mengembalikan kurang dari `limit` item pada sebuah halaman karena Skills mencurigakan difilter setelah pengambilan halaman.
...
- `metadata.os`: pembatasan OS yang dideklarasikan di frontmatter Skill (mis. `["macos"]`, `["linux"]`). `null` jika tidak dideklarasikan.
...
mengatur `status` kembali ke `open`. Berikan `finalAction: "hide"` dengan laporan yang telah ditriage
...
- `security.hasScanResult` bernilai `true` hanya ketika scanner menghasilkan putusan definitif (`clean`, `suspicious`, atau `malicious`).
...
Endpoint kirim terautentikasi untuk tugas ClawScan baru.
...
- Tugas pemindaian bersifat asinkron. Permintaan pemindaian manual diprioritaskan di depan pekerjaan publikasi/backfill normal, tetapi penyelesaian tetap bergantung pada ketersediaan worker.
...
- Mengembalikan `queue.queuedAhead` dan `queue.position` saat masuk antrean sehingga klien dapat menampilkan berapa banyak pemindaian manual terprioritas yang ada di depan permintaan. Antrean yang sangat besar dibatasi dan dilaporkan dengan `queuedAheadIsEstimate: true`.
...
- `security.signals` berisi bukti scanner pendukung seperti `staticScan`, `virusTotal`, dan `skillSpector`.
- `security.signals.dependencyRegistry` dipertahankan untuk kompatibilitas respons v1, tetapi scanner keberadaan registry dependensi sudah dihentikan dan kunci ini selalu `null`.
...
- Hasil berlaku per item; satu skill atau versi yang hilang tidak menggagalkan seluruh respons.
- Respons ini hanya keamanan. Respons tidak menyertakan data Skill Card, status kartu yang dihasilkan, daftar file artefak, atau payload scanner terperinci.
- `security.signals` hanya berisi bukti pendukung tingkat status; gunakan `/scan` atau halaman audit keamanan ClawHub untuk detail scanner lengkap.
- `security.signals.dependencyRegistry` dipertahankan untuk kompatibilitas respons v1, tetapi scanner keberadaan registry dependensi sudah dihentikan dan kunci ini selalu `null`.
- Ketiadaan Skill Card tidak memengaruhi `ok`, `decision`, atau `reasons` endpoint ini; klien harus membaca `skill-card.md` terinstal secara lokal ketika memerlukan konten kartu.
- Gunakan `/verify` ketika Anda memerlukan envelope verifikasi Skill Card untuk satu skill, `/card` ketika Anda memerlukan markdown kartu yang dihasilkan, dan `/scan` ketika Anda memerlukan data scanner terperinci.
...
`trending` adalah papan peringkat instal/unduhan tujuh hari dan tidak menggunakan total sepanjang masa.
...
- Skill yang dihosting menyertakan file versi tersimpan terbaru dan tercantum di
...
- `_errors.json` disertakan saat skill atau file individual tidak dapat
  diekspor.
...
- Skills juga dapat di-resolve melalui rute ini di katalog terpadu.
...
instal. Ini adalah permukaan konsumsi publik OpenClaw untuk menentukan apakah
...
- `trust.moderationState` dapat null. Nilainya `null` ketika tidak ada moderasi rilis
  manual.
- `trust.blockedFromDownload` adalah sinyal blok instal. OpenClaw dan klien
  instal lain sebaiknya memblokir instalasi ketika nilai ini `true`, alih-alih
...
- Endpoint ini persis versi. Klien sebaiknya memanggilnya setelah me-resolve
...
- Endpoint ini sengaja lebih sempit daripada endpoint moderasi pemilik/moderator.
  Ini mengekspos keputusan instal dan penjelasan publik, bukan identitas
...
- Versi ClawPack mengalirkan byte `.tgz` npm-pack persis yang diunggah.
...
- digest artefak
- asal-usul repo sumber dan commit
...
Melaporkan paket untuk tinjauan moderator. Laporan berada pada level paket, secara opsional
ditautkan ke sebuah versi. Laporan memberi masukan ke antrean moderasi tetapi tidak otomatis menyembunyikan atau
...
Endpoint owner/moderator untuk visibilitas moderasi paket.
...
- Packument paket scoped mendukung path permintaan `/api/npm/@scope/name` dan
...
- Header unduhan menyertakan SHA-256 ClawHub ditambah metadata integrity/shasum npm.
...
- Field payload opsional: `ownerHandle`. Saat ada, API me-resolve penerbit tersebut
...
- Field payload opsional: `migrateOwner`. Saat `true` dengan `ownerHandle`, sebuah
  skill yang sudah ada dapat dipindahkan ke pemilik tersebut jika aktor adalah admin/owner pada kedua
...
- Field formulir yang diizinkan adalah `payload`, blob `files` berulang, atau satu referensi tarball
  ...
  alur upload-url. Publikasi storage-id yang distaging juga harus menyertakan
...
Penghapusan lunak yang dimulai oleh owner mencadangkan slug selama 30 hari, lalu slug dapat diklaim oleh
...
pengguna bersama lama/penerbit personal, endpoint memigrasikannya menjadi penerbit org terlebih dahulu.
...
pemanggil sebagai owner. Endpoint ini tidak memigrasikan handle pengguna/personal yang sudah ada dan
...
dapat menerbitkan rilis code-plugin atau bundle-plugin sebenarnya ke nama tersebut nanti.
...
Khusus admin. Memulihkan penerbit personal untuk prinsipal GitHub OAuth pengganti yang sudah diverifikasi
tanpa mengedit baris akun Convex Auth. Permintaan harus menyebutkan kedua id akun provider GitHub
yang immutable; handle yang mutable hanya digunakan sebagai guard yang terlihat oleh operator.

Endpoint default-nya adalah dry-run. Menerapkan pemulihan memerlukan `dryRun: false` dan
`confirmIdentityVerified: true` setelah staf memverifikasi secara independen kesinambungan antara kedua
principal GitHub. Pemulihan gagal tertutup saat penerbit personal pengguna tujuan saat ini
memiliki skill, paket, atau sumber skill GitHub.
Pemulihan juga memigrasikan field `ownerUserId` lama untuk skill milik penerbit yang dipulihkan,
...
tidak dapat memulihkan otoritas pengguna lama yang bersaing. Setiap tabel utama dibatasi hingga
...
Sumber skill GitHub bersifat tercakup penerbit dan dilaporkan sebagai diperiksa, bukan ditulis ulang.
...
Cabut pemblokiran pengguna dan pulihkan skill yang memenuhi syarat (hanya admin).
...
Ubah alasan tersimpan untuk pemblokiran yang sudah ada tanpa mencabut pemblokiran atau memulihkan
