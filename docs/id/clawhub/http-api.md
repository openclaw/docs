...
read_when:
    ...
    - Melakukan debug permintaan CLI ↔ registri
summary: Referensi HTTP API (endpoint publik + CLI + autentikasi).
x-i18n:
    generated_at: "2026-07-02T08:50:58Z"
    model: gpt-5.5
...
- Hasil dikembalikan dalam urutan relevansi (kemiripan embedding + peningkatan token slug/nama tepat + prior popularitas kecil).
...
- Teks ASCII ditokenisasi pada batas kata dan tanda baca. Misalnya, `personal-map` berisi token `map` mandiri, sedangkan `amap-jsapi-skill` berisi `amap`, `jsapi`, dan `skill`; karena itu, mencari `map` memberi `personal-map` kecocokan leksikal yang lebih kuat daripada `amap-jsapi-skill`.
...
- Jika skill tiba-tiba tidak terlihat, periksa status moderasi terlebih dahulu dengan `clawhub inspect @owner/slug` saat masuk sebelum mengubah metadata terkait peringkat.
...
- Ketika `nonSuspiciousOnly=true`, sort berbasis kursor dapat mengembalikan item lebih sedikit daripada `limit` pada sebuah halaman karena skill mencurigakan difilter setelah pengambilan halaman.
...
Laporkan skill untuk peninjauan moderator. Laporan berada di tingkat skill, secara opsional ditautkan
...
yang sudah ditriase untuk menyembunyikan skill dalam alur kerja yang sama dan dapat diaudit.
...
- `tag` (opsional): menyelesaikan versi yang diberi tag (misalnya `latest`).
...
Endpoint pengiriman terautentikasi untuk job ClawScan baru.
...
- Job pemindaian bersifat asinkron. Permintaan pemindaian manual diprioritaskan di atas pekerjaan publikasi/backfill normal, tetapi penyelesaian tetap bergantung pada ketersediaan worker.
...
- Mengembalikan `queue.queuedAhead` dan `queue.position` saat mengantre sehingga klien dapat menampilkan berapa banyak pemindaian manual yang diprioritaskan berada di depan permintaan. Antrean yang sangat besar dibatasi dan dilaporkan dengan `queuedAheadIsEstimate: true`.
- Jika tersedia, `report` berisi bagian `clawscan`, `skillspector`, `staticAnalysis`, dan `virustotal`.
- Job pemindaian yang gagal mengembalikan `status: "failed"` dengan `lastError`.
...
- Identitas skill, identitas penerbit, dan metadata versi yang dipilih adalah field envelope tingkat atas (`slug`, `displayName`, `publisherHandle`, `version`, `resolvedFrom`, `tag`, `createdAt`) sehingga automasi shell dapat membacanya tanpa membongkar wrapper bersarang.
...
- `security.signals.dependencyRegistry` dipertahankan untuk kompatibilitas respons v1, tetapi pemindai keberadaan registry dependensi sudah dipensiunkan dan kunci ini selalu `null`.
- `provenance` adalah `server-resolved-github-import` hanya ketika ClawHub menyelesaikan dan menyimpan repo/ref/commit/path GitHub selama publikasi atau impor; selain itu nilainya `unavailable`.
...
terinstal yang perlu mereka tampilkan, seperti OpenClaw Control UI.
...
- Respons hanya berisi keamanan. Respons ini tidak menyertakan data Skill Card, status kartu yang dihasilkan, daftar file artefak, atau payload pemindai yang mendetail.
...
- Gunakan `/verify` ketika Anda memerlukan envelope verifikasi Skill Card untuk satu skill, `/card` ketika Anda memerlukan markdown kartu yang dihasilkan, dan `/scan` ketika Anda memerlukan data pemindai mendetail.
...
- Skills saat ini yang didukung GitHub dengan pemindaian `clean` atau `suspicious` menyertakan
  ...
  hash konten, dan URL arsip. Mereka tidak menyertakan berkas sumber yang dihosting ClawHub.
...
- `startDate` (wajib): batas bawah dalam milidetik Unix untuk `updatedAt` Plugin.
- `endDate` (wajib): batas atas dalam milidetik Unix untuk `updatedAt` Plugin.
- `limit` (opsional): bilangan bulat (1-250), bawaan `250`.
- `cursor` (opsional): kursor paginasi dari respons sebelumnya.
...
ditautkan ke versi. Laporan mengalir ke antrean moderasi tetapi tidak secara otomatis menyembunyikan atau
...
Melakukan stream byte tarball ClawPack yang diunggah secara persis untuk klien mirror npm.
...
- Serah terima skill yang didukung GitHub tidak mem-proxy atau me-mirror byte. Respons JSON
...
- Field payload opsional: `ownerHandle`. Saat ada, hanya admin yang dapat memublikasikan atas nama pemilik tersebut.
...
- Hanya penerbit org `openclaw` dan penerbit pribadi milik anggota org `openclaw`
  saat ini yang dapat memublikasikan ke channel `official`.
...
penerbit pengguna/pribadi bersama lama, endpoint memigrasikannya ke penerbit org terlebih dahulu.
...
Khusus admin. Mencadangkan slug root dan nama paket untuk pemilik yang sah tanpa memublikasikan
rilis. Nama paket menjadi paket placeholder privat tanpa baris rilis, sehingga pemilik yang sama
dapat memublikasikan rilis code-plugin atau bundle-plugin asli ke nama tersebut nanti.
...
Khusus admin. Memulihkan penerbit pribadi untuk principal OAuth GitHub pengganti yang terverifikasi
tanpa mengedit baris akun Convex Auth. Permintaan harus menyebutkan kedua id akun provider GitHub
yang immutable; handle yang mutable hanya digunakan sebagai guard yang berhadapan dengan operator.

Endpoint defaultnya adalah dry-run. Menerapkan pemulihan memerlukan `dryRun: false` dan
...
memiliki keterampilan, paket, atau sumber keterampilan GitHub.
Pemulihan juga memigrasikan bidang `ownerUserId` legacy untuk keterampilan penerbit yang dipulihkan,
alias slug keterampilan, paket, peringatan pemeriksa paket, dan baris digest pencarian turunan agar
...
Sumber keterampilan GitHub berskala penerbit dan dilaporkan sebagai diperiksa, bukan ditulis ulang.
...
- Kedua endpoint memerlukan autentikasi token API dan hanya bekerja untuk pemilik keterampilan.
...
Ban pengguna dan hapus permanen keterampilan yang dimiliki (hanya moderator/admin).
...
Cabut ban pengguna dan pulihkan keterampilan yang memenuhi syarat (hanya admin).
