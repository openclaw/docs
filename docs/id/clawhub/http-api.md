...
x-i18n:
    generated_at: "2026-06-28T00:10:38Z"
    model: gpt-5.5
...
- `X-RateLimit-Remaining` / `RateLimit-Remaining`: anggaran tersisa yang persis saat ada.
...
- Menggunakan header IP klien tepercaya, termasuk `cf-connecting-ip`, hanya saat
...
- Jika tidak ada IP klien tepercaya yang tersedia, permintaan anonim menggunakan bucket fallback
  yang hanya dicakup oleh jenis batas laju. Bucket fallback ini tidak menyertakan
  path, slug, nama paket, versi, string kueri, atau parameter
  artefak lain yang disediakan pemanggil.

## Respons kesalahan

Respons kesalahan v1 publik adalah teks biasa dengan `content-type: text/plain; charset=utf-8`.
...
sebaiknya membaca badan respons sebagai string yang dapat dibaca manusia. Parameter kueri yang tidak dikenal
...
- Hasil dikembalikan dalam urutan relevansi (kemiripan embedding + peningkat token slug/nama persis + prior popularitas kecil).
- Relevansi lebih kuat daripada popularitas. Kecocokan token slug atau nama tampilan yang presisi dapat mengungguli kecocokan yang lebih longgar dengan keterlibatan yang jauh lebih kuat.
...
- Popularitas diskalakan log dan dibatasi. Skills dengan keterlibatan tinggi dapat berperingkat lebih rendah saat teks kueri merupakan kecocokan yang lebih lemah.
...
- Letakkan istilah yang secara harfiah akan dicari pengguna di nama tampilan, ringkasan, dan tag. Gunakan token slug mandiri hanya saat itu juga merupakan identitas stabil yang ingin Anda pertahankan.
- Jangan mengganti nama slug hanya untuk mengejar satu kueri kecuali slug baru adalah nama kanonis jangka panjang yang lebih baik. Slug lama menjadi alias pengalihan, tetapi URL kanonis, slug yang ditampilkan, dan intisari pencarian mendatang menggunakan slug baru.
- Alias penggantian nama mempertahankan resolusi untuk URL lama dan instalasi yang diselesaikan melalui registri, tetapi peringkat pencarian didasarkan pada metadata Skill kanonis setelah penggantian nama diindeks. Statistik yang ada tetap bersama Skill.
...
- `createdAt` stabil untuk crawl Skill baru; `updated` berubah saat Skills yang ada dipublikasikan ulang.
- Saat `nonSuspiciousOnly=true`, sort berbasis kursor dapat mengembalikan item lebih sedikit dari `limit` pada satu halaman karena Skills mencurigakan difilter setelah pengambilan halaman.
...
ke versi, dan mengalir ke antrean laporan Skill.
...
mengatur `status` kembali ke `open`. Teruskan `finalAction: "hide"` dengan laporan
yang sudah ditriage untuk menyembunyikan Skill dalam alur kerja yang sama dan dapat diaudit.
...
- `moderation` adalah cuplikan moderasi tingkat skill saat ini yang diturunkan dari versi terbaru.
...
- Mengembalikan `queue.queuedAhead` dan `queue.position` saat dalam antrean sehingga klien dapat menampilkan berapa banyak pemindaian manual yang diprioritaskan berada di depan permintaan tersebut. Antrean yang sangat besar dibatasi dan dilaporkan dengan `queuedAheadIsEstimate: true`.
...
Rute pemindaian ulang batch kanonis khusus admin. Ini menerima bentuk payload yang sama seperti `POST /api/v1/skills/-/rescan-batch` lama.

### `POST /api/v1/skills/-/scan/batch/status`

Rute status batch kanonis khusus admin. Ini menerima `{ "jobIds": ["..."] }` dan mengembalikan penghitung agregat yang sama seperti `POST /api/v1/skills/-/rescan-batch/status` lama.
...
- `tag` (opsional): me-resolve versi bertag (misalnya `latest`).

Catatan:

- `ok` bernilai `true` hanya ketika versi yang dipilih memiliki Skill Card yang dibuat, tidak diblokir sebagai malware oleh moderasi, dan verifikasi ClawScan bersih.
- Identitas skill, identitas penerbit, dan metadata versi yang dipilih adalah field envelope tingkat atas (`slug`, `displayName`, `publisherHandle`, `version`, `resolvedFrom`, `tag`, `createdAt`) sehingga otomatisasi shell dapat membacanya tanpa membongkar wrapper bersarang.
- `security` adalah putusan ClawScan/keamanan tingkat atas. Otomatisasi sebaiknya mengacu pada `ok`, `decision`, `reasons`, dan `security.status`.
...
- `provenance` adalah `server-resolved-github-import` hanya ketika ClawHub me-resolve dan menyimpan repo/ref/commit/path GitHub selama publikasi atau impor; jika tidak, nilainya `unavailable`.
...
ClawHub terpasang mana yang perlu mereka tampilkan, seperti OpenClaw Control UI.
...
- Respons hanya berisi keamanan. Ini tidak menyertakan data Skill Card, status kartu yang dibuat, daftar file artefak, atau payload pemindai terperinci.
...
- `security.signals.dependencyRegistry` dipertahankan untuk kompatibilitas respons v1, tetapi pemindai keberadaan registri dependensi telah dihentikan dan kunci ini selalu `null`.
- Ketiadaan Skill Card tidak memengaruhi `ok`, `decision`, atau `reasons` endpoint ini; klien sebaiknya membaca `skill-card.md` terpasang secara lokal ketika memerlukan konten kartu.
- Gunakan `/verify` ketika Anda memerlukan envelope verifikasi Skill Card untuk satu skill, `/card` ketika Anda memerlukan markdown kartu yang dibuat, dan `/scan` ketika Anda memerlukan data pemindai terperinci.
...
- Pemanggil terautentikasi dapat melihat paket privat untuk penerbit tempat mereka tergabung dalam hasil daftar/pencarian.
...
- Pemanggil anonim hanya melihat saluran paket publik.
- Pemanggil terautentikasi dapat mencari paket privat untuk penerbit tempat mereka tergabung.
- `channel=private` hanya mengembalikan paket yang dapat dibaca oleh pemanggil terautentikasi.
...
- Skills saat ini yang didukung GitHub dengan pemindaian `clean` atau `suspicious` menyertakan
  ...
  hash konten, dan URL arsip. Skills tersebut tidak menyertakan file sumber yang dihosting ClawHub.
...
- `trust.moderationState` dapat bernilai null. Nilainya `null` ketika tidak ada moderasi
  rilis manual.
...
- `trust.stale` berarti ringkasan kepercayaan dihitung dari input yang sudah usang dan
  sebaiknya diperlakukan sebagai memerlukan penyegaran sebelum keputusan izinkan dengan keyakinan tinggi.

Catatan:

- Endpoint ini tepat berdasarkan versi. Klien sebaiknya memanggilnya setelah menyelesaikan
...
- Endpoint ini sengaja lebih sempit daripada endpoint moderasi pemilik/moderator.
  ...
  identitas pelapor, isi laporan, bukti privat, atau lini waktu tinjauan internal.
...
Melaporkan paket untuk tinjauan moderator. Laporan berada pada tingkat paket, secara opsional
ditautkan ke versi. Laporan tersebut masuk ke antrean moderasi tetapi tidak otomatis menyembunyikan atau
...
- `revoked`: diblokir setelah sebuah rilis sebelumnya dipercaya.
...
- Packument paket berscope mendukung path permintaan `/api/npm/@scope/name` dan
  `/api/npm/@scope%2Fname` yang dienkode npm.
...
- Handoff skill berbasis GitHub tidak melakukan proxy atau mirror byte. Respons JSON
...
- Statistik unduhan dihitung sebagai identitas unik per hari UTC (`userId` saat token API valid, selain itu IP).
...
- Direkomendasikan: `multipart/form-data` dengan JSON `payload` + blob `files[]`.
- Body JSON dengan `files` (berbasis storageId) juga diterima.
- Field payload opsional: `ownerHandle`. Jika ada, API me-resolve
  penerbit tersebut di sisi server dan mengharuskan aktor memiliki akses penerbit.
- Field payload opsional: `migrateOwner`. Jika `true` bersama `ownerHandle`, sebuah
...
- Field formulir yang diizinkan adalah `payload`, blob `files` berulang, atau satu referensi tarball
  ...
  alur upload-url. Penerbitan storage-id bertahap juga harus menyertakan
  `clawpackUploadTicket` yang dikembalikan bersama URL unggah tersebut.
- Gunakan salah satu dari `files` atau `clawpack`, jangan pernah keduanya dalam permintaan yang sama.
...
- Field payload opsional: `ownerHandle`. Jika ada, hanya admin yang boleh menerbitkan atas nama pemilik tersebut.
...
- Hanya penerbit org `openclaw` dan penerbit pribadi milik anggota org `openclaw`
...
pengguna bersama lama/penerbit pribadi, endpoint memigrasikannya terlebih dahulu menjadi penerbit org.
...
nanti dapat menerbitkan rilis code-plugin atau bundle-plugin asli ke nama tersebut.
...
Khusus admin. Memulihkan penerbit pribadi untuk principal GitHub OAuth pengganti yang terverifikasi
tanpa mengedit baris akun Convex Auth. Permintaan harus menyebutkan kedua id akun provider GitHub
yang tidak dapat diubah; handle yang dapat diubah hanya digunakan sebagai guard yang terlihat operator.
...
principal GitHub. Pemulihan gagal secara tertutup ketika penerbit personal pengguna tujuan saat ini
memiliki skills, paket, atau sumber skill GitHub.
Pemulihan juga memigrasikan field `ownerUserId` legacy untuk skills milik penerbit yang dipulihkan,
alias slug skill, paket, peringatan pemeriksa paket, dan baris digest pencarian turunan sehingga
jalur pemilik langsung selaras dengan otoritas penerbit baru. Reservasi handle terlindungi yang aktif
...
Batalkan blokir pengguna dan pulihkan skills yang memenuhi syarat (hanya admin).
...
Ubah alasan tersimpan untuk blokir yang sudah ada tanpa membatalkan blokir atau memulihkan
konten (hanya admin). Default ke mode uji coba kecuali `dryRun` bernilai `false`.
...
yang melakukan staging tarball ClawPack harus mengirim id penyimpanan yang dihasilkan sebagai
...
Jika Anda melakukan hosting sendiri, sajikan file ini (atau tetapkan `CLAWHUB_REGISTRY` secara eksplisit; legacy `CLAWDHUB_REGISTRY`).
