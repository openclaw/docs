...
x-i18n:
    generated_at: "2026-07-03T23:41:39Z"
    model: gpt-5.5
...
- Jika tidak ada IP klien tepercaya yang tersedia, permintaan anonim menggunakan bucket fallback
  ...
  path, slug, nama paket, versi, string kueri, atau parameter artefak lain yang dipasok
  pemanggil.
...
- Teks ASCII ditokenisasi pada batas kata dan tanda baca. Misalnya, `personal-map` berisi token `map` mandiri, sementara `amap-jsapi-skill` berisi `amap`, `jsapi`, dan `skill`; karena itu, pencarian `map` memberikan `personal-map` kecocokan leksikal yang lebih kuat daripada `amap-jsapi-skill`.
- Popularitas diskalakan log dan dibatasi. Skills dengan engagement tinggi dapat berperingkat lebih rendah ketika teks kueri merupakan kecocokan yang lebih lemah.
...
- Alias penggantian nama mempertahankan penyelesaian untuk URL lama dan instalasi yang diselesaikan melalui registry, tetapi peringkat pencarian didasarkan pada metadata Skill kanonis setelah penggantian nama telah diindeks. Statistik yang ada tetap bersama Skill.
...
- `metadata.os`: batasan OS yang dideklarasikan di frontmatter Skill (misalnya `["macos"]`, `["linux"]`). `null` jika tidak dideklarasikan.
...
Endpoint moderator/admin untuk pemasukan laporan Skill.
...
untuk menyembunyikan Skill dalam workflow yang dapat diaudit yang sama.
...
- Mengembalikan `queue.queuedAhead` dan `queue.position` saat mengantre agar klien dapat menampilkan berapa banyak pemindaian manual berprioritas yang berada di depan permintaan. Antrean yang sangat besar dibatasi dan dilaporkan dengan `queuedAheadIsEstimate: true`.
...
- `ok` bernilai `true` hanya ketika versi yang dipilih memiliki Skill Card yang dibuat, tidak diblokir malware oleh moderasi, dan verifikasi ClawScan bersih.
- Identitas skill, identitas penerbit, dan metadata versi yang dipilih adalah bidang amplop tingkat atas (`slug`, `displayName`, `publisherHandle`, `version`, `resolvedFrom`, `tag`, `createdAt`) sehingga otomasi shell dapat membacanya tanpa membongkar wrapper bersarang.
- `security` adalah putusan ClawScan/keamanan tingkat atas. Otomasi sebaiknya bertumpu pada `ok`, `decision`, `reasons`, dan `security.status`.
...
- Respons hanya keamanan. Respons ini tidak menyertakan data Skill Card, status kartu yang dibuat, daftar file artefak, atau payload pemindai terperinci.
...
- `security.signals.dependencyRegistry` dipertahankan untuk kompatibilitas respons v1, tetapi pemindai keberadaan registri dependensi sudah dihentikan dan kunci ini selalu `null`.
- Tidak adanya Skill Card tidak memengaruhi `ok`, `decision`, atau `reasons` endpoint ini; klien sebaiknya membaca `skill-card.md` terinstal secara lokal ketika membutuhkan konten kartu.
- Gunakan `/verify` ketika Anda membutuhkan amplop verifikasi Skill Card skill tunggal, `/card` ketika Anda membutuhkan markdown kartu yang dibuat, dan `/scan` ketika Anda membutuhkan data pemindai terperinci.
...
Jelajah katalog khusus plugin di seluruh paket code-plugin dan bundle-plugin.
...
`trending` adalah papan peringkat pemasangan/unduhan tujuh hari dan tidak menggunakan total sepanjang waktu.
...
- Skills yang di-hosting menyertakan file versi tersimpan terbaru dan dicantumkan di
...
- Skills saat ini yang didukung GitHub dengan pemindaian `clean` atau `suspicious` menyertakan
  ...
  hash konten, dan URL arsip. Mereka tidak menyertakan file sumber yang di-hosting ClawHub.
...
- `trust.blockedFromDownload` adalah sinyal blokir instalasi. OpenClaw dan klien
  instalasi lain harus memblokir instalasi ketika nilai ini `true`, bukan
...
- Endpoint ini persis versi. Klien harus memanggilnya setelah me-resolve
...
- Versi ClawPack melakukan streaming byte `.tgz` `npm-pack` yang persis diunggah.
...
- `dist.tarball`, `dist.integrity`, dan `dist.shasum` menggunakan field yang kompatibel dengan npm
  agar pengguna dapat mengarahkan npm ke mirror jika mereka memilihnya.
...
- Lebih disukai: `multipart/form-data` dengan JSON `payload` + blob `files[]`.
...
- Hanya penerbit org `openclaw` dan penerbit personal milik anggota org `openclaw` saat ini
  yang boleh menerbitkan ke channel `official`.
...
penerbit lain. Respons hapus menyertakan `slugReservedUntil` saat masa berlaku ini diterapkan.
...
Khusus admin. Memastikan penerbit org ada untuk suatu handle. Jika handle masih menunjuk ke
pengguna bersama lama/penerbit personal, endpoint terlebih dahulu memigrasikannya menjadi penerbit org.
...
dapat menerbitkan rilis code-plugin atau bundle-plugin nyata ke nama tersebut nanti.
...
tanpa mengedit baris akun Convex Auth. Permintaan harus menyebutkan kedua id akun
penyedia GitHub yang tidak dapat diubah; handle yang dapat berubah hanya digunakan sebagai guard yang terlihat operator.
...
Pemulihan juga memigrasikan kolom legacy `ownerUserId` untuk skill milik penerbit yang dipulihkan,
alias slug skill, package, peringatan pemeriksa package, dan baris digest pencarian turunan agar
jalur pemilik langsung selaras dengan otoritas penerbit yang baru. Reservasi handle terlindungi
yang aktif untuk handle yang dipulihkan juga dialihkan ke pengguna pengganti sehingga sinkronisasi
...
Sumber skill GitHub berskala penerbit dan dilaporkan sebagai sudah diperiksa, bukan ditulis ulang.
...
Ubah alasan yang tersimpan untuk blokir yang sudah ada tanpa membuka blokir atau memulihkan
