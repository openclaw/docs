...
- Jika token hilang/tidak valid, perilaku fallback ke penerapan berdasarkan IP.
- Endpoint tulis terautentikasi sebaiknya tidak mengembalikan `Unauthorized` polos ketika
  ...
  akun yang dihapus/dilarang/dinonaktifkan masing-masing harus mendapatkan teks yang dapat ditindaklanjuti agar klien CLI
...
- Terstandar: `RateLimit-Limit`, `RateLimit-Remaining`, `RateLimit-Reset`
...
- Jika tidak ada IP klien tepercaya yang tersedia, permintaan unduhan anonim menggunakan bucket fallback yang dicakup endpoint alih-alih satu bucket global `ip:unknown`. Permintaan baca/tulis anonim tetap menggunakan bucket unknown bersama agar routing IP yang hilang tetap terlihat dan konservatif.
...
- Unduhan digunakan sebagai prior kecil berskala log dan pemecah seri, bukan sebagai sinyal peringkat utama. Skills dengan unduhan tinggi dapat berperingkat lebih rendah ketika teks kueri merupakan kecocokan yang lebih lemah.
...
- Masukkan istilah yang secara harfiah akan dicari pengguna ke nama tampilan, ringkasan, dan tag. Gunakan token slug mandiri hanya ketika token itu juga merupakan identitas stabil yang ingin Anda pertahankan.
...
- Alias penggantian nama mempertahankan resolusi untuk URL lama dan instalasi yang di-resolve melalui registri, tetapi peringkat pencarian didasarkan pada metadata Skill kanonis setelah penggantian nama terindeks. Statistik yang ada tetap bersama Skill tersebut.
- Jika sebuah Skill tidak terlihat secara tidak terduga, periksa status moderasi terlebih dahulu dengan `clawhub inspect <slug>` saat sudah masuk sebelum mengubah metadata terkait peringkat.
...
- Ketika `nonSuspiciousOnly=true`, pengurutan berbasis kursor dapat mengembalikan item lebih sedikit dari `limit` pada sebuah halaman karena Skills mencurigakan difilter setelah pengambilan halaman.
...
Laporkan Skill untuk tinjauan moderator. Laporan berada pada level Skill, opsional ditautkan
...
- Menyertakan status verifikasi yang dinormalisasi beserta detail spesifik pemindai.
...
- `security.hasScanResult` bernilai `true` hanya ketika pemindai menghasilkan verdict definitif (`clean`, `suspicious`, atau `malicious`).
...
- Rilis ClawPack mencakup field kompatibel npm `npmIntegrity`, `npmShasum`, dan
  `npmTarballName`.
...
Endpoint moderator untuk menampilkan baris migrasi plugin OpenClaw resmi.
...
- Metadata khusus registri tidak disuntikkan ke dalam arsip yang diunduh.
...
dapat kemudian menerbitkan rilis code-plugin atau bundle-plugin yang sebenarnya ke nama tersebut.
...
Memblokir pengguna dan menghapus permanen skill yang dimiliki (hanya moderator/admin).
...
Membuka blokir pengguna dan memulihkan skill yang memenuhi syarat (hanya admin).
...
CLI dapat menemukan pengaturan registri/auth dari situs:
