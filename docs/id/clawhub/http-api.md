...
x-i18n:
    generated_at: "2026-07-03T09:57:49Z"
    model: gpt-5.5
...
- Jika `Retry-After` ada, tunggu selama detik tersebut sebelum mencoba ulang.
...
Respons kesalahan v1 publik adalah teks polos dengan `content-type: text/plain; charset=utf-8`.
...
- Letakkan istilah yang akan benar-benar dicari pengguna di nama tampilan, ringkasan, dan tag. Gunakan token slug yang berdiri sendiri hanya ketika itu juga merupakan identitas stabil yang ingin Anda pertahankan.
...
- `cursor` (opsional): cursor paginasi untuk pengurutan non-`trending` apa pun
...
- Ketika `nonSuspiciousOnly=true`, pengurutan berbasis cursor dapat mengembalikan item lebih sedikit dari `limit` pada satu halaman karena Skills mencurigakan difilter setelah pengambilan halaman.
...
Laporkan skill untuk ditinjau moderator. Laporan berada pada tingkat skill, dapat ditautkan secara opsional
...
- `security.signals.dependencyRegistry` dipertahankan untuk kompatibilitas respons v1, tetapi pemindai keberadaan registry dependensi sudah dihentikan dan kunci ini selalu `null`.
- Ketiadaan Skill Card tidak memengaruhi `ok`, `decision`, atau `reasons` endpoint ini; klien sebaiknya membaca `skill-card.md` yang terpasang secara lokal saat memerlukan konten kartu.
- Gunakan `/verify` ketika Anda memerlukan envelope verifikasi Skill Card untuk satu skill, `/card` ketika Anda memerlukan markdown kartu yang dihasilkan, dan `/scan` ketika Anda memerlukan data pemindai terperinci.
...
- `mcp-tooling`, `data`, dan `automation` diresolusikan ke `tools`.
- `observability` dan `deployment` diresolusikan ke `gateway`.
- `dev-tools` diresolusikan ke `runtime`.
...
- Skill yang dihosting mencakup file versi tersimpan terbaru dan dicantumkan di
  `_manifest.json` dengan `sourceRef: "public-clawhub"`.
- Skill saat ini yang didukung GitHub dengan pemindaian `clean` atau `suspicious` mencakup
...
- Setiap skill mencakup `_export_skill_meta.json`.
...
Mengembalikan ringkasan keamanan dan kepercayaan rilis paket persis untuk klien
...
- `package.name`, `package.displayName`, dan `package.family` mengidentifikasi
  paket registry yang diresolusi.
...
- `trust.stale` berarti ringkasan kepercayaan dihitung dari masukan yang sudah kedaluwarsa dan
  harus diperlakukan sebagai membutuhkan penyegaran sebelum keputusan izinkan dengan keyakinan tinggi.

Catatan:

- Endpoint ini spesifik versi. Klien sebaiknya memanggilnya setelah meresolusi
...
- `manual`: setiap rilis dengan override moderasi manual.
- `all`: setiap rilis dengan override manual, status pemindaian tidak bersih, atau laporan paket.
...
Melaporkan paket untuk ditinjau moderator. Laporan berada di tingkat paket, dan secara opsional
ditautkan ke versi. Laporan mengisi antrean moderasi tetapi tidak secara otomatis menyembunyikan atau
...
skill saat ini yang didukung GitHub dengan pemindaian `clean` atau `suspicious` dan tanpa versi yang dihosting.
...
- Handoff skill yang didukung GitHub tidak memproksi atau melakukan mirror byte. Respons JSON
...
yang tidak dapat diubah; handle yang dapat berubah hanya digunakan sebagai guard yang terlihat oleh operator.

Titik akhir secara default menggunakan uji coba. Menerapkan pemulihan memerlukan `dryRun: false` dan
...
memiliki Skills, paket, atau sumber Skills GitHub.
...
jalur pemilik langsung sesuai dengan otoritas penerbit yang baru. Reservasi handle terlindungi aktif
...
Sumber Skills GitHub memiliki cakupan penerbit dan dilaporkan sebagai diperiksa, bukan ditulis ulang.
...
konten (hanya admin). Default-nya uji coba kecuali `dryRun` adalah `false`.
...
Jika Anda menghosting sendiri, sajikan file ini (atau tetapkan `CLAWHUB_REGISTRY` secara eksplisit; `CLAWDHUB_REGISTRY` lama).
