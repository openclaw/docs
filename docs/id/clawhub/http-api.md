...
- Endpoint tulis terautentikasi tidak boleh mengembalikan `Unauthorized` polos ketika
  ...
  akun yang dihapus/diblokir/dinonaktifkan masing-masing harus mendapat teks yang dapat ditindaklanjuti agar klien
...
- Gunakan backoff berjitter untuk menghindari percobaan ulang yang tersinkron.
...
- Jika tidak ada IP klien tepercaya yang tersedia, permintaan anonim menggunakan bucket fallback
  yang dicakup hanya berdasarkan jenis rate-limit. Bucket fallback ini tidak menyertakan
  path, slug, nama paket, versi, string kueri, atau parameter artefak lain
  yang dipasok pemanggil.
...
- Teks ASCII ditokenisasi pada batas kata dan tanda baca. Misalnya, `personal-map` berisi token `map` mandiri, sementara `amap-jsapi-skill` berisi `amap`, `jsapi`, dan `skill`; pencarian `map` karena itu memberi `personal-map` kecocokan leksikal yang lebih kuat daripada `amap-jsapi-skill`.
...
- Ketika `nonSuspiciousOnly=true`, sort berbasis kursor dapat mengembalikan item lebih sedikit dari `limit` pada sebuah halaman karena Skills mencurigakan difilter setelah pengambilan halaman.
...
- Bukti direduksi untuk pemanggil publik dan hanya menyertakan snippet mentah untuk pemilik/moderator.
...
- Menyertakan status verifikasi yang dinormalkan plus detail khusus pemindai.
...
- Pemindaian yang dipublikasikan menulis kembali hanya ketika `update: true` dan pemindaian berhasil selesai.
...
- Mengembalikan `queue.queuedAhead` dan `queue.position` saat mengantre sehingga klien dapat menampilkan berapa banyak pemindaian manual prioritas yang berada di depan permintaan. Antrean yang sangat besar dibatasi dan dilaporkan dengan `queuedAheadIsEstimate: true`.
...
- `ok` bernilai `true` hanya ketika versi yang dipilih memiliki Kartu Skill yang dihasilkan, tidak diblokir malware oleh moderasi, dan verifikasi ClawScan bersih.
...
- `provenance` bernilai `server-resolved-github-import` hanya ketika ClawHub me-resolve dan menyimpan repo/ref/commit/path GitHub selama publikasi atau impor; jika tidak, nilainya `unavailable`.
...
- Respons hanya keamanan. Respons tidak menyertakan data Kartu Skill, status kartu yang dihasilkan, daftar file artefak, atau payload pemindai terperinci.
...
- `security.signals.dependencyRegistry` dipertahankan untuk kompatibilitas respons v1, tetapi pemindai keberadaan registry dependensi telah dihentikan dan kunci ini selalu `null`.
- Ketiadaan Kartu Skill tidak memengaruhi `ok`, `decision`, atau `reasons` endpoint ini; klien sebaiknya membaca `skill-card.md` terpasang secara lokal ketika membutuhkan konten kartu.
- Gunakan `/verify` ketika Anda membutuhkan amplop verifikasi Kartu Skill untuk satu skill, `/card` ketika Anda membutuhkan markdown kartu yang dihasilkan, dan `/scan` ketika Anda membutuhkan data pemindai terperinci.
...
- `isOfficial` (opsional): `true` atau `false`
- `category` (opsional): filter kategori plugin. Didukung hanya ketika
  permintaan dicakup ke paket plugin. Kategori terkontrol dan alias filter
...
- Skill terkini yang didukung GitHub dengan pemindaian `clean` atau `suspicious` menyertakan
...
Melaporkan paket untuk peninjauan moderator. Laporan berada di level paket, secara opsional
...
`note` wajib untuk `confirmed` dan `dismissed`; boleh dihilangkan saat
...
- Metadata khusus registry tidak disuntikkan ke dalam arsip yang diunduh.
...
- `dist.tarball`, `dist.integrity`, dan `dist.shasum` menggunakan field kompatibel npm
  agar pengguna dapat mengarahkan npm ke mirror jika mereka memilih.
- Packument paket bercakupan mendukung path permintaan `/api/npm/@scope/name` dan
  encoded `/api/npm/@scope%2Fname` milik npm.
...
- Handoff skill yang didukung GitHub tidak mem-proxy atau me-mirror byte. Respons JSON
  ...
  dan `archiveUrl`; status pemindaian/saat ini adalah gate dan tidak disertakan sebagai metadata payload
  keberhasilan.
- Statistik unduhan dihitung sebagai identitas unik per hari UTC (`userId` saat token API valid, jika tidak IP).

## Titik Akhir Auth (token Bearer)
...
pengguna bersama legacy/penerbit personal, titik akhir memigrasikannya ke penerbit org terlebih dahulu.
...
yang tidak dapat diubah; handle yang dapat berubah hanya digunakan sebagai penjaga yang dihadapkan ke operator.
...
berikutnya tidak dapat memulihkan otoritas bersaing milik pengguna sebelumnya. Setiap tabel utama dibatasi hingga
...
Tambahkan/hapus star (sorotan). Kedua endpoint bersifat idempotent.
