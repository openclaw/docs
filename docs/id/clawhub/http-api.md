...
x-i18n:
    generated_at: "2026-07-04T04:05:54Z"
    model: gpt-5.5
...
- Jika tidak ada IP klien tepercaya yang tersedia, permintaan anonim menggunakan bucket fallback
  ...
  jalur, slug, nama paket, versi, string kueri, atau parameter artefak lain yang disediakan pemanggil.
...
- Letakkan istilah yang benar-benar akan dicari pengguna di nama tampilan, ringkasan, dan tag. Gunakan token slug mandiri hanya ketika token itu juga merupakan identitas stabil yang ingin Anda pertahankan.
- Jangan mengganti nama slug hanya untuk mengejar satu kueri kecuali slug baru merupakan nama kanonis jangka panjang yang lebih baik. Slug lama menjadi alias pengalihan, tetapi URL kanonis, slug yang ditampilkan, dan digest pencarian mendatang menggunakan slug baru.
...
- `cursor` (opsional): cursor paginasi untuk sort non-`trending` apa pun
...
- Ketika `nonSuspiciousOnly=true`, sort berbasis cursor dapat mengembalikan item lebih sedikit daripada `limit` pada sebuah halaman karena Skills mencurigakan difilter setelah pengambilan halaman.
...
Laporkan skill untuk ditinjau moderator. Laporan berada pada tingkat skill, opsional ditautkan
...
- `limit` (opsional): integer (1-200)
- `cursor` (opsional): cursor paginasi
...
mengatur `status` kembali ke `open`. Berikan `finalAction: "hide"` dengan laporan yang sudah ditriase
untuk menyembunyikan skill dalam alur kerja yang dapat diaudit yang sama.
...
- Mencakup status verifikasi yang dinormalisasi plus detail khusus pemindai.
- `security.hasScanResult` adalah `true` hanya saat pemindai menghasilkan vonis definitif (`clean`, `suspicious`, atau `malicious`).
- `moderation` adalah cuplikan moderasi tingkat Skills saat ini yang diturunkan dari versi terbaru.
...
- Pemindaian yang dipublikasikan menulis balik hanya saat `update: true` dan pemindaian selesai dengan sukses.
...
- Mengembalikan `queue.queuedAhead` dan `queue.position` saat masih dalam antrean sehingga klien dapat menampilkan berapa banyak pemindaian manual yang diprioritaskan yang berada di depan permintaan. Antrean yang sangat besar dibatasi dan dilaporkan dengan `queuedAheadIsEstimate: true`.
...
- Memerlukan pemindaian yang sukses; pemindaian non-terminal mengembalikan `409`.
...
- `ok` adalah `true` hanya saat versi yang dipilih memiliki Skill Card yang dibuat, tidak diblokir sebagai malware oleh moderasi, dan verifikasi ClawScan bersih.
- Identitas skill, identitas penerbit, dan metadata versi yang dipilih adalah field amplop tingkat atas (`slug`, `displayName`, `publisherHandle`, `version`, `resolvedFrom`, `tag`, `createdAt`) sehingga automasi shell dapat membacanya tanpa membongkar wrapper bersarang.
- `security` adalah vonis ClawScan/keamanan tingkat atas. Automasi sebaiknya berpatokan pada `ok`, `decision`, `reasons`, dan `security.status`.
...
- `provenance` adalah `server-resolved-github-import` hanya saat ClawHub me-resolve dan menyimpan repo/ref/commit/path GitHub selama publikasi atau impor; jika tidak, nilainya `unavailable`.
...
- Respons hanya berisi keamanan. Respons tidak menyertakan data Skill Card, status kartu yang dibuat, daftar file artefak, atau payload pemindai terperinci.
...
- `security.signals.dependencyRegistry` dipertahankan untuk kompatibilitas respons v1, tetapi pemindai keberadaan registry dependensi sudah dihentikan dan kunci ini selalu `null`.
- Ketiadaan Skill Card tidak memengaruhi `ok`, `decision`, atau `reasons` endpoint ini; klien sebaiknya membaca `skill-card.md` yang terinstal secara lokal saat memerlukan konten kartu.
- Gunakan `/verify` saat Anda memerlukan amplop verifikasi Skill Card satu skill, `/card` saat Anda memerlukan markdown kartu yang dibuat, dan `/scan` saat Anda memerlukan data pemindai terperinci.
...
- Pemanggil terautentikasi dapat melihat paket privat untuk publisher tempat mereka menjadi anggota dalam hasil daftar/pencarian.
...
- Pemanggil anonim hanya melihat saluran paket publik.
- Pemanggil terautentikasi dapat mencari paket privat untuk publisher tempat mereka menjadi anggota.
- `channel=private` hanya mengembalikan paket yang dapat dibaca oleh pemanggil terautentikasi.
...
- `cursor` (opsional): cursor paginasi dari respons sebelumnya.
...
- `trust.stale` berarti ringkasan kepercayaan dihitung dari input usang dan
  harus diperlakukan sebagai memerlukan penyegaran sebelum keputusan izinkan berkeyakinan tinggi.
...
ditautkan ke versi. Laporan mengalir ke antrean moderasi tetapi tidak otomatis menyembunyikan atau
...
- `revoked`: diblokir setelah rilis sebelumnya pernah dipercaya.
...
- `dist.tarball`, `dist.integrity`, dan `dist.shasum` menggunakan field yang kompatibel
  dengan npm sehingga pengguna dapat mengarahkan npm ke mirror jika mereka memilihnya.
...
pengguna/penerbit personal bersama legacy, endpoint memigrasikannya terlebih dahulu menjadi penerbit org.
...
Pembuatan penerbit org secara mandiri yang terautentikasi. Membuat penerbit org baru dan menambahkan
...
nanti dapat menerbitkan rilis code-plugin atau bundle-plugin nyata ke nama tersebut.
...
yang immutable; handle yang mutable hanya digunakan sebagai pengaman yang menghadap operator.

Endpoint secara default menggunakan mode simulasi. Menerapkan pemulihan memerlukan `dryRun: false` dan
`confirmIdentityVerified: true` setelah staf memverifikasi kesinambungan antara kedua
prinsipal GitHub secara independen. Pemulihan gagal tertutup saat penerbit pribadi pengguna tujuan saat ini
...
profil berikutnya tidak dapat memulihkan otoritas pengguna lama yang bersaing. Setiap tabel utama dibatasi hingga
...
Sumber skill GitHub bersifat berbasis penerbit dan dilaporkan sebagai diperiksa, bukan ditulis ulang.
...
konten (hanya admin). Secara default menggunakan mode simulasi kecuali `dryRun` bernilai `false`.
...
Jika Anda menghosting sendiri, sajikan file ini (atau atur `CLAWHUB_REGISTRY` secara eksplisit; `CLAWDHUB_REGISTRY` lama).
