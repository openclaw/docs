...
x-i18n:
    generated_at: "2026-07-01T08:28:44Z"
    model: gpt-5.5
...
- Distandarkan: `RateLimit-Limit`, `RateLimit-Reset`
...
- Alias rename mempertahankan penyelesaian untuk URL lama dan instalasi yang diselesaikan melalui registri, tetapi peringkat pencarian didasarkan pada metadata skill kanonis setelah rename terindeks. Statistik yang ada tetap bersama skill tersebut.
...
- `createdAt` stabil untuk crawl skill baru; `updated` berubah ketika skill yang sudah ada dipublikasikan ulang.
...
- `moderation` disertakan hanya ketika skill ditandai atau owner melihatnya.
...
- Pemanggil publik hanya mendapat `200` untuk skill terlihat yang sudah ditandai.
- Bukti disunting untuk pemanggil publik dan hanya menyertakan snippet mentah untuk owner/moderator.

### `POST /api/v1/skills/{slug}/report`

Laporkan skill untuk peninjauan moderator. Laporan berada di level skill, secara opsional ditautkan
...
mengatur `status` kembali ke `open`. Berikan `finalAction: "hide"` bersama laporan
...
- Pemindaian yang dipublikasikan menulis balik hanya ketika `update: true` dan pemindaian berhasil diselesaikan.
...
- Pekerjaan pemindaian bersifat asinkron. Permintaan pemindaian manual diprioritaskan sebelum pekerjaan publikasi/pengisian balik normal, tetapi penyelesaian tetap bergantung pada ketersediaan worker.
...
- Identitas skill, identitas penerbit, dan metadata versi yang dipilih adalah kolom amplop tingkat atas (`slug`, `displayName`, `publisherHandle`, `version`, `resolvedFrom`, `tag`, `createdAt`) sehingga otomatisasi shell dapat membacanya tanpa membongkar wrapper bersarang.
...
- `security.signals.dependencyRegistry` dipertahankan untuk kompatibilitas respons v1, tetapi pemindai keberadaan registry dependensi telah dihentikan dan kunci ini selalu `null`.
- Ketiadaan Skill Card tidak memengaruhi `ok`, `decision`, atau `reasons` endpoint ini; klien sebaiknya membaca `skill-card.md` terinstal secara lokal ketika membutuhkan konten kartu.
- Gunakan `/verify` saat Anda membutuhkan amplop verifikasi Skill Card skill tunggal, `/card` saat Anda membutuhkan markdown kartu yang dihasilkan, dan `/scan` saat Anda membutuhkan data pemindai terperinci.
...
- Pemanggil terautentikasi dapat melihat paket privat untuk penerbit tempat mereka tergabung di hasil daftar/pencarian.
...
- `trust.stale` berarti ringkasan kepercayaan dihitung dari input yang sudah usang dan
  sebaiknya diperlakukan sebagai memerlukan penyegaran sebelum keputusan izin dengan keyakinan tinggi.
...
- Endpoint ini sengaja lebih sempit daripada endpoint moderasi pemilik/moderator.
  ...
  pelapor, isi laporan, bukti privat, atau linimasa tinjauan internal.
...
- digest artefak
- repo sumber dan provenans commit
...
Laporkan paket untuk tinjauan moderator. Laporan berada pada tingkat paket, secara opsional
...
- Packument paket berscope mendukung jalur permintaan `/api/npm/@scope/name` dan jalur permintaan
...
skill saat ini yang berbasis GitHub dengan pemindaian `clean` atau `suspicious` dan tanpa
...
- Handoff skill berbasis GitHub tidak memproksi atau me-mirror byte. Respons JSON
...
- Field payload opsional: `migrateOwner`. Saat `true` dengan `ownerHandle`,
  skill yang ada dapat berpindah ke pemilik tersebut jika aktor adalah admin/pemilik pada penerbit
...
- Body JSON dan metadata `payload.files` / `payload.artifact` yang disediakan pemanggil
  ditolak.
- Permintaan publikasi multipart langsung dibatasi 18MB. Tarball ClawPack dapat
...
- Publikasi atas nama pihak lain tetap memvalidasi kelayakan kanal official terhadap akun pemilik target.
...
untuk handle yang dipulihkan juga dialihkan ke pengguna pengganti agar sinkronisasi
profil berikutnya tidak dapat memulihkan otoritas pesaing pengguna sebelumnya. Setiap tabel utama dibatasi hingga
...
Buka blokir pengguna dan pulihkan skill yang memenuhi syarat (khusus admin).
...
Ubah alasan yang disimpan untuk blokir yang sudah ada tanpa membuka blokir atau memulihkan
konten (khusus admin). Default-nya adalah dry-run kecuali `dryRun` adalah `false`.
...
Daftarkan atau cari pengguna (khusus admin).
...
- `/.well-known/clawhub.json` (JSON, disukai)
