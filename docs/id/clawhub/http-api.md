...
x-i18n:
    generated_at: "2026-07-03T01:03:58Z"
    model: gpt-5.5
...
Pintasan slug web diselesaikan lintas keluarga registri, tetapi klien API harus menggunakan
...
- Jika `Retry-After` ada, tunggu sebanyak detik tersebut sebelum mencoba lagi.
...
- Jika tidak ada IP klien tepercaya yang tersedia, permintaan anonim menggunakan bucket fallback
  ...
  path, slug, nama paket, versi, string kueri, atau parameter artefak lain
  yang disediakan pemanggil.
...
harus membaca body respons sebagai string yang dapat dibaca manusia. Parameter kueri tidak dikenal
...
- Popularitas diskalakan log dan dibatasi. Skills dengan keterlibatan tinggi dapat berperingkat lebih rendah ketika teks kueri memiliki kecocokan yang lebih lemah.
...
Panduan ketercarian penerbit:

- Masukkan istilah yang secara harfiah akan dicari pengguna di nama tampilan, ringkasan, dan tag. Gunakan token slug mandiri hanya ketika itu juga identitas stabil yang ingin Anda pertahankan.
...
- Alias penggantian nama mempertahankan penyelesaian untuk URL lama dan instalasi yang diselesaikan melalui registri, tetapi peringkat pencarian didasarkan pada metadata Skill kanonis setelah penggantian nama diindeks. Statistik yang ada tetap bersama Skill.
...
- Bukti diredaaksi untuk pemanggil publik dan hanya menyertakan snippet mentah untuk owner/moderator.
...
Pemindaian terbitan menggunakan JSON:
...
- Pemindaian terbitan memerlukan akses manajemen pemilik/penerbit, atau wewenang moderator/admin platform.
- Pemindaian terbitan menulis balik hanya ketika `update: true` dan pemindaian selesai dengan sukses.
...
- Pekerjaan pemindaian bersifat asinkron. Permintaan pemindaian manual diprioritaskan di atas pekerjaan publikasi/backfill normal, tetapi penyelesaian tetap bergantung pada ketersediaan worker.
...
- Mengembalikan `queue.queuedAhead` dan `queue.position` saat berada dalam antrean sehingga klien dapat menampilkan berapa banyak pemindaian manual prioritas yang berada di depan permintaan. Antrean yang sangat besar dibatasi dan dilaporkan dengan `queuedAheadIsEstimate: true`.
...
- Identitas skill, identitas penerbit, dan metadata versi yang dipilih adalah kolom amplop tingkat atas (`slug`, `displayName`, `publisherHandle`, `version`, `resolvedFrom`, `tag`, `createdAt`) sehingga otomasi shell dapat membacanya tanpa membongkar wrapper bertingkat.
...
koleksi ini ditujukan untuk klien yang sudah tahu versi skill ClawHub terinstal
...
- Respons hanya keamanan. Ini tidak menyertakan data Skill Card, status kartu yang dihasilkan, daftar file artefak, atau payload pemindai terperinci.
...
- `security.signals.dependencyRegistry` dipertahankan untuk kompatibilitas respons v1, tetapi pemindai keberadaan registri dependensi sudah dihentikan dan kunci ini selalu `null`.
- Ketiadaan Skill Card tidak memengaruhi `ok`, `decision`, atau `reasons` endpoint ini; klien harus membaca `skill-card.md` terinstal secara lokal saat memerlukan konten kartu.
- Gunakan `/verify` saat Anda memerlukan amplop verifikasi Skill Card untuk satu skill, `/card` saat Anda memerlukan markdown kartu yang dihasilkan, dan `/scan` saat Anda memerlukan data pemindai terperinci.
...
- `category` (opsional): filter kategori plugin. Didukung hanya ketika
  ...
  alias filter legacy v1 didokumentasikan di bawah `GET /api/v1/plugins`.
...
- Entri skill tetap didukung oleh registry skill dan masih hanya dapat dipublikasikan melalui `POST /api/v1/skills`.
...
- `category` (opsional): filter kategori plugin. Didukung hanya ketika
  ...
  legacy v1 didokumentasikan di bawah `GET /api/v1/plugins`.
...
Alias filter legacy v1 tetap diterima pada endpoint baca:
...
Alias legacy tidak diterima sebagai nilai kategori yang disimpan atau dinyatakan oleh penulis.
...
- `package.name`, `package.displayName`, dan `package.family` mengidentifikasi
  paket registri yang di-resolve.
...
- `trust.blockedFromDownload` adalah sinyal pemblokiran instalasi. OpenClaw dan klien
  instalasi lainnya harus memblokir instalasi ketika nilai ini `true`, bukan
...
- Endpoint ini persis versi. Klien harus memanggilnya setelah me-resolve versi
  paket yang ingin mereka instal, bukan hanya setelah membaca metadata paket
  terbaru.
- Paket privat mengembalikan `404` kecuali pemanggil dapat membaca penerbit pemiliknya.
...
Laporkan paket untuk peninjauan moderator. Laporan berada di tingkat paket, secara opsional
...
`finalAction: "revoke"` dengan laporan yang telah dikonfirmasi untuk menerapkan moderasi rilis dalam
...
- Packument paket scoped mendukung path permintaan `/api/npm/@scope/name` dan
  `/api/npm/@scope%2Fname` yang dikodekan milik npm.
...
- Field payload opsional: `migrateOwner`. Jika `true` dengan `ownerHandle`,
  ...
  saat ini dan penerbit target. Tanpa opt-in ini, perubahan pemilik
  ditolak.
...
- Field form yang diizinkan adalah `payload`, blob `files` berulang, atau satu referensi tarball
...
- Body JSON dan metadata `payload.files` / `payload.artifact` yang disediakan pemanggil
  ditolak.
- Permintaan publish multipart langsung dibatasi hingga 18MB. Tarball ClawPack dapat
...
- Hanya penerbit org `openclaw` dan penerbit personal milik anggota org `openclaw` saat ini yang
  dapat menerbitkan ke channel `official`.
...
penerbit pengguna/personal bersama lama, endpoint memigrasikannya menjadi penerbit org terlebih dahulu.
...
pemanggil sebagai pemilik. Endpoint ini tidak memigrasikan handle pengguna/personal yang ada dan tidak
...
Pemulihan juga memigrasikan bidang `ownerUserId` lama untuk skills milik penerbit yang dipulihkan,
...
profil berikutnya tidak dapat memulihkan otoritas pesaing milik pengguna sebelumnya. Setiap tabel primer dibatasi hingga
100 baris per transaksi apply; pemulihan yang lebih besar harus lebih dulu menggunakan migrasi pemilik yang dapat dilanjutkan.
Sumber skill GitHub bersifat tercakup penerbit dan dilaporkan sebagai sudah diperiksa, bukan ditulis ulang.
...
Tambah/hapus star (sorotan). Kedua titik akhir bersifat idempoten.
