...
x-i18n:
    generated_at: "2026-07-04T06:50:30Z"
    model: gpt-5.5
...
- Teks ASCII ditokenisasi pada batas kata dan tanda baca. Misalnya, `personal-map` berisi token mandiri `map`, sementara `amap-jsapi-skill` berisi `amap`, `jsapi`, dan `skill`; karena itu, pencarian `map` memberi `personal-map` kecocokan leksikal yang lebih kuat daripada `amap-jsapi-skill`.
...
- `createdAt` stabil untuk crawl Skills baru; `updated` berubah ketika Skills yang ada dipublikasikan ulang.
...
- `metadata.os`: pembatasan OS yang dideklarasikan dalam frontmatter Skills (mis. `["macos"]`, `["linux"]`). `null` jika tidak dideklarasikan.
...
- `moderation` disertakan hanya ketika Skills ditandai atau owner sedang melihatnya.
...
ke versi, dan masuk ke antrean laporan Skills.
...
Endpoint moderator/admin untuk menyelesaikan atau membuka ulang laporan Skills.
...
untuk menyembunyikan Skills dalam workflow yang sama dan dapat diaudit.
...
Endpoint pengajuan terautentikasi untuk pekerjaan ClawScan baru.
...
- Pemindaian terpublikasi memerlukan akses manajemen pemilik/penerbit, atau wewenang moderator/admin platform.
- Pemindaian terpublikasi menulis kembali hanya ketika `update: true` dan pemindaian selesai dengan sukses.
...
Endpoint polling terautentikasi untuk pemindaian yang diajukan.

- Mengembalikan status queued/running/succeeded/failed.
- Mengembalikan `queue.queuedAhead` dan `queue.position` saat queued agar klien dapat menampilkan berapa banyak pemindaian manual berprioritas yang ada di depan permintaan. Antrean yang sangat besar dibatasi dan dilaporkan dengan `queuedAheadIsEstimate: true`.
...
Endpoint arsip laporan tersimpan terautentikasi untuk versi yang diajukan.
...
- `security` adalah putusan ClawScan/keamanan tingkat atas. Otomatisasi sebaiknya bertumpu pada `ok`, `decision`, `reasons`, dan `security.status`.
- `security.signals` berisi bukti pemindai pendukung seperti `staticScan`, `virusTotal`, dan `skillSpector`.
- `security.signals.dependencyRegistry` dipertahankan untuk kompatibilitas respons v1, tetapi pemindai keberadaan dependency registry sudah dipensiunkan dan kunci ini selalu `null`.
...
- Respons hanya keamanan. Respons ini tidak menyertakan data Skill Card, status kartu yang dihasilkan, daftar file artefak, atau payload pemindai terperinci.
- `security.signals` hanya berisi bukti pendukung tingkat status; gunakan `/scan` atau halaman security-audit ClawHub untuk detail pemindai lengkap.
- `security.signals.dependencyRegistry` dipertahankan untuk kompatibilitas respons v1, tetapi pemindai keberadaan dependency registry sudah dipensiunkan dan kunci ini selalu `null`.
...
- Gunakan `/verify` ketika Anda memerlukan amplop verifikasi Skill Card satu skill, `/card` ketika Anda memerlukan markdown kartu yang dihasilkan, dan `/scan` ketika Anda memerlukan data pemindai terperinci.
...
- `startDate` (wajib): batas bawah milidetik Unix untuk Plugin `updatedAt`.
- `endDate` (wajib): batas atas milidetik Unix untuk Plugin `updatedAt`.
...
- `version.sha256hash` adalah metadata kompatibilitas yang tidak lagi direkomendasikan untuk klien lama. Metadata ini
...
instalasi. Ini adalah permukaan konsumsi OpenClaw publik untuk memutuskan apakah sebuah
...
- `trust.blockedFromDownload` adalah sinyal pemblokiran instalasi. OpenClaw dan klien
  instalasi lainnya sebaiknya memblokir instalasi ketika nilai ini `true`, bukan
...
- `packageName` dinormalisasi sebagai nama npm; paket boleh tidak ada untuk migrasi yang direncanakan.
...
Rilis yang dikarantina dan dicabut mengembalikan `403` dari route unduhan artefak.
...
- Arsip Plugin/paket adalah file zip dengan root `package/` agar klien OpenClaw lama
  tetap berfungsi.
- Route ini tetap hanya ZIP. Ini tidak melakukan stream file ClawPack `.tgz`.
...
- Metadata khusus registry tidak disisipkan ke arsip yang diunduh.
...
Melakukan stream byte tarball ClawPack yang tepat seperti diunggah untuk klien mirror npm.
...
- `slug` (wajib)
- `hash` (wajib): sha256 hex 64 karakter dari sidik jari bundle
...
- Lebih disarankan: `multipart/form-data` dengan JSON `payload` + blob `files[]`.
...
- Field payload opsional: `migrateOwner`. Saat `true` dengan `ownerHandle`, skill
  yang sudah ada dapat dipindahkan ke pemilik tersebut jika aktor adalah admin/pemilik di kedua
...
- Field form yang diizinkan adalah `payload`, blob `files` berulang, atau satu referensi tarball
  ...
  alur upload-url. Publikasi storage-id yang dipentaskan juga harus menyertakan
...
- Body JSON dan metadata `payload.files` / `payload.artifact` yang disediakan pemanggil
  ditolak.
- Permintaan publikasi multipart langsung dibatasi pada 18MB. Tarball ClawPack dapat
...
- Publikasi atas nama tetap memvalidasi kelayakan channel official terhadap akun pemilik target.
...
pengguna bersama legacy/penerbit personal, endpoint memigrasikannya ke penerbit org terlebih dahulu.
...
Pembuatan penerbit org mandiri terautentikasi. Membuat penerbit org baru dan menambahkan
...
Sumber skill GitHub bersifat tercakup ke publisher dan dilaporkan sebagai telah diperiksa, bukan ditulis ulang.
...
### Titik akhir pengalihan kepemilikan
...
konten (hanya admin). Secara bawaan menggunakan dry-run kecuali `dryRun` bernilai `false`.
...
- `limit` (opsional): hasil maksimum (bawaan 20, maksimum 200)
...
Jika Anda meng-host sendiri, sajikan file ini (atau tetapkan `CLAWHUB_REGISTRY` secara eksplisit; `CLAWDHUB_REGISTRY` lama).
