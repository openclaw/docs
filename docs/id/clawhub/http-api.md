...
read_when:
    ...
    - Mendiagnosis permintaan CLI ↔ registri
...
`/api/...` dan `/api/cli/...` lama tetap ada untuk kompatibilitas (lihat `DEPRECATIONS.md`).
...
- Kompatibilitas lama: `X-RateLimit-Limit`, `X-RateLimit-Remaining`, `X-RateLimit-Reset`
...
- `Retry-After`: detik yang harus ditunggu sebelum mencoba ulang (penundaan) pada `429`
...
- Jika tidak ada IP klien tepercaya yang tersedia, permintaan unduh anonim menggunakan bucket fallback bercakupan endpoint alih-alih satu bucket global `ip:unknown`. Permintaan baca/tulis anonim tetap menggunakan bucket unknown bersama agar perutean IP hilang tetap terlihat dan konservatif.
...
- Unduhan digunakan sebagai prior kecil berskala log dan pemutus seri, bukan sebagai sinyal peringkat utama. Skills dengan unduhan tinggi dapat berperingkat lebih rendah ketika teks kueri merupakan kecocokan yang lebih lemah.
...
- Jangan mengganti nama slug hanya untuk mengejar satu kueri kecuali slug baru adalah nama kanonis jangka panjang yang lebih baik. Slug lama menjadi alias pengalih, tetapi URL kanonis, slug yang ditampilkan, dan digest pencarian mendatang menggunakan slug baru.
- Alias penggantian nama mempertahankan penyelesaian untuk URL lama dan instalasi yang diselesaikan melalui registri, tetapi peringkat pencarian didasarkan pada metadata Skill kanonis setelah penggantian nama terindeks. Statistik yang ada tetap bersama Skill tersebut.
- Jika suatu Skill secara tidak terduga tidak terlihat, periksa status moderasi terlebih dahulu dengan `clawhub inspect <slug>` saat sudah masuk sebelum mengubah metadata terkait peringkat.
...
- Gunakan `nextCursor` untuk melanjutkan paginasi ketika ada. Halaman pendek tidak dengan sendirinya berarti hasil telah berakhir.
...
- `security.hasScanResult` adalah `true` hanya ketika pemindai menghasilkan verdict definitif (`clean`, `suspicious`, atau `malicious`).
...
- `channel=private` hanya mengembalikan paket yang dapat dibaca pemanggil terautentikasi.
...
Menghapus paket dan semua rilisnya secara lunak.

Catatan:

- Memerlukan token API untuk pemilik paket, owner/admin penerbit org,
...
- Menggunakan bucket rate unduhan.
...
ditautkan ke versi. Laporan ini mengisi antrean moderasi tetapi tidak otomatis menyembunyikan atau
...
- Defaultnya adalah rilis terbaru.
...
Mengunduh arsip ZIP deterministik warisan untuk sebuah rilis paket.
...
- Metadata yang hanya ada di registry tidak disuntikkan ke arsip yang diunduh.
...
- Versi warisan yang hanya ZIP sengaja dihilangkan.
...
penerbit pengguna/pribadi bersama warisan, endpoint memigrasikannya ke penerbit org terlebih dahulu.
...
- Kedua endpoint memerlukan auth token API dan hanya berfungsi untuk pemilik skill.
...
Mencabut larangan pengguna dan memulihkan skill yang memenuhi syarat (khusus admin).
...
## Endpoint CLI warisan (tidak digunakan lagi)
...
Jika Anda melakukan self-host, sajikan file ini (atau atur `CLAWHUB_REGISTRY` secara eksplisit; `CLAWDHUB_REGISTRY` warisan).
