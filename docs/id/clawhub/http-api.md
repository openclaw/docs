...
x-i18n:
    generated_at: "2026-07-02T17:46:06Z"
    model: gpt-5.5
...
Direktori pihak ketiga dapat menggunakan endpoint baca publik untuk mencantumkan atau mencari Skills ClawHub. Harap cache hasil, patuhi `429`/`Retry-After`, tautkan pengguna kembali ke listing ClawHub kanonis (`https://clawhub.ai/<owner>/skills/<slug>`), dan hindari mengesankan dukungan ClawHub terhadap situs pihak ketiga. Jangan mencoba mencerminkan konten tersembunyi, privat, atau diblokir moderasi di luar permukaan API publik.
...
- Endpoint tulis terautentikasi tidak seharusnya mengembalikan `Unauthorized` mentah ketika
...
- `X-RateLimit-Remaining` / `RateLimit-Remaining`: anggaran tersisa persis saat ada.
  Permintaan sharded yang berhasil menghilangkan header ini alih-alih mengembalikan nilai global perkiraan.
...
- Teks ASCII ditokenisasi pada batas kata dan tanda baca. Misalnya, `personal-map` berisi token `map` yang berdiri sendiri, sedangkan `amap-jsapi-skill` berisi `amap`, `jsapi`, dan `skill`; mencari `map` karena itu memberi `personal-map` kecocokan leksikal yang lebih kuat daripada `amap-jsapi-skill`.
...
- Jika sebuah Skill tiba-tiba tidak terlihat, periksa status moderasi terlebih dahulu dengan `clawhub inspect @owner/slug` saat sudah masuk sebelum mengubah metadata terkait peringkat.
...
- `cursor` (opsional): kursor pagination untuk sort non-`trending` apa pun
...
- Gunakan `nextCursor` untuk melanjutkan pagination saat ada. Halaman pendek saja tidak berarti akhir hasil.
...
- `limit` (opsional): integer (1-200)
- `cursor` (opsional): kursor pagination
...
yang sudah ditriage untuk menyembunyikan Skill dalam workflow auditabel yang sama.
...
- Identitas skill, identitas penerbit, dan metadata versi yang dipilih adalah field envelope tingkat atas (`slug`, `displayName`, `publisherHandle`, `version`, `resolvedFrom`, `tag`, `createdAt`) sehingga automasi shell dapat membacanya tanpa membuka wrapper bertingkat.
- `security` adalah putusan ClawScan/keamanan tingkat atas. Automasi sebaiknya menggunakan `ok`, `decision`, `reasons`, dan `security.status` sebagai kunci.
...
- `security.signals.dependencyRegistry` dipertahankan untuk kompatibilitas respons v1, tetapi pemindai keberadaan registri dependensi sudah dihentikan dan kunci ini selalu `null`.
- Ketiadaan Skill Card tidak memengaruhi `ok`, `decision`, atau `reasons` endpoint ini; klien sebaiknya membaca `skill-card.md` terpasang secara lokal ketika membutuhkan konten kartu.
- Gunakan `/verify` ketika Anda membutuhkan envelope verifikasi Skill Card untuk satu skill, `/card` ketika Anda membutuhkan markdown kartu yang dihasilkan, dan `/scan` ketika Anda membutuhkan data pemindai terperinci.
...
- Entri Skills tetap didukung oleh registry Skills dan masih hanya dapat dipublikasikan melalui `POST /api/v1/skills`.
...
- Skills terkini yang didukung GitHub dengan pemindaian `clean` atau `suspicious` menyertakan
  ...
  hash konten, dan URL arsip. Skills tersebut tidak menyertakan file sumber yang di-host ClawHub.
...
- `version.sha256hash` adalah metadata kompatibilitas yang tidak digunakan lagi untuk klien lama. Ini
  membuat hash byte ZIP persis yang dikembalikan oleh `/api/v1/packages/{name}/download`.
...
Mengembalikan ringkasan keamanan dan kepercayaan rilis paket yang persis untuk klien instalasi. Ini adalah permukaan konsumsi publik OpenClaw untuk memutuskan apakah
...
ditautkan ke sebuah versi. Laporan memberi masukan ke antrean moderasi tetapi tidak menyembunyikan otomatis atau
...
- Field payload opsional: `migrateOwner`. Jika `true` dengan `ownerHandle`,
  skill yang sudah ada dapat dipindahkan ke pemilik tersebut jika aktor adalah admin/pemilik pada kedua
...
Jika ada, `reason` disimpan sebagai catatan moderasi skill dan disalin ke dalam log audit.
Penghapusan sementara yang dimulai pemilik mencadangkan slug selama 30 hari, lalu slug dapat diklaim oleh
penerbit lain. Respons penghapusan menyertakan `slugReservedUntil` saat masa kedaluwarsa ini berlaku.
...
tanpa mengedit baris akun Convex Auth. Permintaan harus menamai kedua id akun provider GitHub
...
Pemulihan juga memigrasikan field `ownerUserId` legacy untuk skill milik penerbit yang dipulihkan,
...
Ubah alasan tersimpan untuk blokir yang sudah ada tanpa mencabut blokir atau memulihkan
...
Jika Anda melakukan self-host, sajikan file ini (atau atur `CLAWHUB_REGISTRY` secara eksplisit; legacy `CLAWDHUB_REGISTRY`).
