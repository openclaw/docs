...
Direktori pihak ketiga dapat menggunakan endpoint baca publik untuk mencantumkan atau mencari skill ClawHub. Harap cache hasil, hormati `429`/`Retry-After`, tautkan pengguna kembali ke listing kanonis ClawHub (`https://clawhub.ai/<owner>/<slug>`), dan hindari menyiratkan dukungan ClawHub terhadap situs pihak ketiga tersebut. Jangan mencoba mencerminkan konten tersembunyi, privat, atau diblokir moderasi di luar permukaan API publik.
...
- Endpoint tulis terautentikasi sebaiknya tidak mengembalikan `Unauthorized` polos ketika
  server mengetahui alasannya. Token yang tidak ada, token tidak valid/dicabut, dan
...
- Terstandarisasi: `RateLimit-Limit`, `RateLimit-Remaining`, `RateLimit-Reset`
- Pada `429`: `Retry-After`

Semantik header:

- `X-RateLimit-Reset`: detik Unix epoch absolut
...
- Jika `Retry-After` ada, tunggu selama itu dalam detik sebelum mencoba lagi.
...
- Jika tidak ada IP klien tepercaya yang tersedia, permintaan unduh anonim menggunakan bucket fallback berskala endpoint alih-alih satu bucket global `ip:unknown`. Permintaan baca/tulis anonim tetap menggunakan bucket unknown bersama agar routing IP yang hilang tetap terlihat dan konservatif.
...
- Unduhan digunakan sebagai prior kecil berskala log dan penentu seri, bukan sebagai sinyal peringkat utama. Skill dengan unduhan tinggi dapat memiliki peringkat lebih rendah ketika teks kueri adalah kecocokan yang lebih lemah.
...
- Letakkan istilah yang akan dicari pengguna secara harfiah di nama tampilan, ringkasan, dan tag. Gunakan token slug mandiri hanya ketika itu juga merupakan identitas stabil yang ingin Anda pertahankan.
...
- Alias penggantian nama mempertahankan resolusi untuk URL lama dan instalasi yang di-resolve melalui registry, tetapi peringkat pencarian didasarkan pada metadata skill kanonis setelah penggantian nama telah diindeks. Statistik yang ada tetap bersama skill.
- Jika suatu skill tiba-tiba tidak terlihat, periksa status moderasi terlebih dahulu dengan `clawhub inspect <slug>` saat sudah login sebelum mengubah metadata terkait peringkat.
...
Laporkan skill untuk peninjauan moderator. Laporan berada pada level skill, dapat ditautkan secara opsional
...
- `version.security` mencakup status verifikasi scan yang dinormalisasi dan detail scanner
  (VirusTotal + LLM), jika tersedia.

### `GET /api/v1/skills/{slug}/scan`

Mengembalikan detail verifikasi scan keamanan untuk versi skill.
...
- Mencakup status verifikasi yang dinormalisasi plus detail khusus scanner.
- `security.capabilityTags` mencakup label kemampuan/risiko deterministik seperti
...
- `target` / `hostTarget` (opsional): bentuk singkat untuk `host:<target>`
- `os`, `arch`, `libc` (opsional): bentuk singkat untuk filter kapabilitas host
- `requiresBrowser`, `requiresDesktop`, `requiresNativeDeps`,
  `requiresExternalService`, `requiresBinary`, `requiresOsPermission`
  (opsional): bentuk singkat `true`/`1` untuk tag persyaratan lingkungan
- `externalService`, `binary`, `osPermission` (opsional): bentuk singkat untuk tag
...
- `target` / `hostTarget`, `os`, `arch`, `libc`, `requiresBrowser`,
  ...
  `osPermission` diterima sebagai bentuk singkat untuk tag kapabilitas umum
- `artifactKind` (opsional): `legacy-zip` atau `npm-pack`
...
Menghapus paket dan semua rilis secara soft-delete.
...
Melaporkan paket untuk tinjauan moderator. Laporan berada pada tingkat paket, dan secara opsional
...
`finalAction: "revoke"` dengan laporan yang sudah dikonfirmasi untuk menerapkan moderasi rilis dalam
...
- Default-nya uji coba tanpa perubahan.
- Rilis tanpa penyimpanan ClawPack diberi label `legacy-zip`.
- Baris yang sudah didukung ClawPack tetapi kehilangan `artifactKind` diperbaiki sebagai
  `npm-pack`.
...
- Field payload opsional: `migrateOwner`. Saat `true` dengan `ownerHandle`,
  skill yang sudah ada dapat dipindahkan ke pemilik itu jika aktor adalah admin/pemilik pada
...
- Disarankan: `multipart/form-data` dengan JSON `payload` + blob `files[]`.
- Isi JSON dengan `files` (berbasis storageId) juga diterima.
- Field payload opsional: `ownerHandle`. Saat ada, hanya admin yang boleh menerbitkan atas nama pemilik itu.
