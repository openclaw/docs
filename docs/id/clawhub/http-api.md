...
x-i18n:
    generated_at: "2026-07-04T18:21:44Z"
    model: gpt-5.5
...
Respons galat v1 publik berupa teks biasa dengan `content-type: text/plain; charset=utf-8`.
...
harus membaca isi respons sebagai string yang dapat dibaca manusia. Parameter kueri tidak dikenal
...
- Popularitas diskalakan log dan dibatasi. Skills dengan keterlibatan tinggi dapat berperingkat lebih rendah ketika teks kueri adalah kecocokan yang lebih lemah.
...
- Letakkan istilah yang secara harfiah akan dicari pengguna dalam nama tampilan, ringkasan, dan tag. Gunakan token slug mandiri hanya ketika itu juga merupakan identitas stabil yang ingin Anda pertahankan.
...
- Jika sebuah Skill tidak terlihat secara tak terduga, periksa status moderasi terlebih dahulu dengan `clawhub inspect @owner/slug` saat login sebelum mengubah metadata terkait peringkat.
...
- `moderation` disertakan hanya ketika Skill ditandai atau pemilik melihatnya.
...
- Bukti disunting untuk pemanggil publik dan hanya menyertakan snippet mentah untuk pemilik/moderator.
...
yang telah ditriase untuk menyembunyikan Skill dalam alur kerja yang sama dan dapat diaudit.
...
- Mengembalikan `queue.queuedAhead` dan `queue.position` saat masih queued agar klien dapat menampilkan berapa banyak pemindaian manual yang diprioritaskan berada di depan permintaan. Antrean yang sangat besar dibatasi dan dilaporkan dengan `queuedAheadIsEstimate: true`.
...
- Identitas skill, identitas penerbit, dan metadata versi yang dipilih adalah field amplop tingkat atas (`slug`, `displayName`, `publisherHandle`, `version`, `resolvedFrom`, `tag`, `createdAt`) agar otomatisasi shell dapat membacanya tanpa membongkar wrapper bersarang.
...
ClawHub terinstal yang perlu mereka tampilkan, seperti OpenClaw Control UI.
...
- `isOfficial` (opsional): `true` atau `false`
- `category` (opsional): filter kategori Plugin. Hanya didukung saat
  permintaan dibatasi ke paket Plugin. Kategori terkendali dan alias filter
...
- Kontrol urutan UI browser untuk pencarian Plugin mengurutkan ulang hasil relevansi yang dimuat,
...
- `trust.blockedFromDownload` adalah sinyal blok instalasi. OpenClaw dan klien
  instalasi lain harus memblokir instalasi ketika nilai ini `true` alih-alih
...
Mengembalikan metadata resolver artefak eksplisit untuk satu versi paket.
...
Melaporkan paket untuk peninjauan moderator. Laporan berada pada level paket, secara opsional
ditautkan ke sebuah versi. Laporan mengisi antrean moderasi tetapi tidak otomatis menyembunyikan atau
...
Pembuatan penerbit org layanan mandiri yang terautentikasi. Membuat penerbit org baru dan menambahkan
...
Hanya admin. Memulihkan penerbit personal untuk principal GitHub OAuth pengganti yang terverifikasi
...
principal GitHub. Pemulihan gagal secara tertutup ketika publisher personal saat ini milik pengguna tujuan
memiliki skills, packages, atau GitHub skill sources.
Pemulihan juga memigrasikan field `ownerUserId` lama untuk skills milik publisher yang dipulihkan,
alias slug skill, packages, peringatan inspector package, dan baris digest pencarian turunan agar
jalur pemilik langsung selaras dengan otoritas publisher baru. Reservasi protected-handle yang aktif
...
GitHub skill sources bersifat tercakup publisher dan dilaporkan sebagai diperiksa, bukan ditulis ulang.
...
- `GET /api/v1/transfers/outgoing`
  - Bentuk response: `{ "transfers": [{ "_id": "...", "skill": { "slug": "demo", "displayName": "Demo" }, "fromUser"|"toUser": { "handle": "..." }, "message": "...", "requestedAt": 0, "expiresAt": 0 }] }`

### `POST /api/v1/users/ban`

Blokir pengguna dan hard-delete skill yang dimiliki (hanya moderator/admin).
...
Tambahkan/hapus bintang (highlight). Kedua titik akhir bersifat idempoten.
...
yang melakukan staging tarball ClawPack harus mengirim storage id yang dihasilkan sebagai
