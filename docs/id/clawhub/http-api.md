...
x-i18n:
    generated_at: "2026-07-02T01:14:20Z"
    model: gpt-5.5
...
- Masukkan istilah yang secara harfiah akan dicari pengguna di nama tampilan, ringkasan, dan tag. Gunakan token slug mandiri hanya ketika itu juga merupakan identitas stabil yang ingin Anda pertahankan.
...
- Alias penggantian nama mempertahankan resolusi untuk URL lama dan instalasi yang diselesaikan melalui registri, tetapi peringkat pencarian didasarkan pada metadata Skills kanonis setelah penggantian nama diindeks. Statistik yang ada tetap bersama Skills tersebut.
- Jika sebuah Skills tiba-tiba tidak terlihat, periksa status moderasi terlebih dahulu dengan `clawhub inspect @owner/slug` saat login sebelum mengubah metadata terkait peringkat.
...
- Ketika `nonSuspiciousOnly=true`, sort berbasis kursor dapat mengembalikan item lebih sedikit dari `limit` pada suatu halaman karena Skills mencurigakan difilter setelah pengambilan halaman.
...
Laporkan Skills untuk ditinjau moderator. Laporan berada di tingkat Skills, opsional ditautkan
ke sebuah versi, dan masuk ke antrean laporan Skills.
...
yang sudah ditriage untuk menyembunyikan Skills dalam workflow yang sama dan dapat diaudit.
...
- `security.signals.dependencyRegistry` dipertahankan untuk kompatibilitas respons v1, tetapi pemindai keberadaan registry dependensi telah dipensiunkan dan kunci ini selalu `null`.
...
- Alias filter v1 lama yang didokumentasikan di bawah `GET /api/v1/plugins` juga
  diterima.
- Pemfilteran kategori adalah filter API nyata yang didukung oleh baris intisari
  kategori Plugin, bukan penulisan ulang kueri pencarian.
- Hasil dikembalikan berdasarkan urutan relevansi dan saat ini tidak dipaginasi.
...
Menandai paket dan semua rilis sebagai dihapus secara lunak.
...
- `trust.reasons` adalah daftar penjelasan untuk pengguna dan audit. Kode alasan
  adalah string ringkas yang stabil seperti `manual:quarantined`, `scan:malicious`,
...
- Endpoint ini spesifik versi. Klien sebaiknya memanggilnya setelah me-resolve
  versi paket yang hendak mereka instal, bukan hanya setelah membaca metadata
  paket terbaru.
- Paket privat mengembalikan `404` kecuali pemanggil dapat membaca penerbit pemiliknya.
...
- Versi ClawPack melakukan streaming byte `.tgz` `npm-pack` yang diunggah persis.
...
- Menggunakan bucket batas baca, bukan bucket unduhan.
...
- Packument paket scoped mendukung jalur permintaan `/api/npm/@scope/name` dan jalur
  terenode npm `/api/npm/@scope%2Fname`.

### `GET /api/npm/{package}/-/{tarball}.tgz`

Melakukan stream byte tarball ClawPack yang persis diunggah untuk klien mirror npm.

Catatan:

- Menggunakan bucket batas unduhan.
...
Khusus admin. Memulihkan penerbit personal untuk principal pengganti GitHub OAuth yang telah diverifikasi
tanpa mengedit baris akun Convex Auth. Permintaan harus menyebutkan kedua id akun
provider GitHub yang immutable; handle yang mutable hanya digunakan sebagai guard yang menghadap operator.

Endpoint secara default menggunakan uji coba. Menerapkan pemulihan memerlukan `dryRun: false` dan
...
alias slug skill, paket, peringatan inspektur paket, dan baris digest pencarian turunan agar
...
Sumber skill GitHub memiliki cakupan penerbit dan dilaporkan sebagai diperiksa, bukan ditulis ulang.
...
Ubah alasan tersimpan untuk pemblokiran yang ada tanpa membatalkan pemblokiran atau memulihkan
konten (khusus admin). Secara default menggunakan uji coba kecuali `dryRun` adalah `false`.
