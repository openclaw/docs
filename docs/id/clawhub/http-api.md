...
x-i18n:
    generated_at: "2026-05-11T20:23:56Z"
    model: gpt-5.5
    provider: openai
...
- Endpoint tulis terautentikasi seharusnya tidak mengembalikan `Unauthorized` polos ketika
...
- Jika tidak ada IP klien tepercaya yang tersedia, permintaan unduhan anonim menggunakan bucket fallback yang dibatasi per endpoint, bukan satu bucket global `ip:unknown`. Permintaan baca/tulis anonim tetap menggunakan bucket unknown bersama agar routing tanpa IP tetap terlihat dan konservatif.
...
- Relevansi lebih kuat daripada popularitas. Kecocokan token slug atau nama tampilan yang presisi dapat mengungguli kecocokan yang lebih longgar dengan jauh lebih banyak unduhan.
- Teks ASCII ditokenisasi pada batas kata dan tanda baca. Misalnya, `personal-map` berisi token `map` yang berdiri sendiri, sementara `amap-jsapi-skill` berisi `amap`, `jsapi`, dan `skill`; mencari `map` karena itu memberi `personal-map` kecocokan leksikal yang lebih kuat daripada `amap-jsapi-skill`.
- Unduhan digunakan sebagai prior kecil berskala log dan pemecah seri, bukan sebagai sinyal peringkat utama. Skill dengan unduhan tinggi dapat berperingkat lebih rendah ketika teks kueri adalah kecocokan yang lebih lemah.
...
- Letakkan istilah yang akan benar-benar dicari pengguna di nama tampilan, ringkasan, dan tag. Gunakan token slug yang berdiri sendiri hanya ketika token tersebut juga merupakan identitas stabil yang ingin Anda pertahankan.
...
- Jika skill tiba-tiba tidak terlihat, periksa status moderasi terlebih dahulu dengan `clawhub inspect <slug>` saat login sebelum mengubah metadata terkait peringkat.
...
- `createdAt` stabil untuk crawl skill baru; `updated` berubah ketika skill yang ada diterbitkan ulang.
- Ketika `nonSuspiciousOnly=true`, sort berbasis kursor dapat mengembalikan lebih sedikit dari `limit` item pada satu halaman karena skill mencurigakan difilter setelah pengambilan halaman.
...
- Pemilik dan moderator dapat mengakses detail moderasi untuk skill tersembunyi.
...
Endpoint pemilik/penerbit skill untuk mengajukan banding atas moderasi pada skill.

Auth:

- Memerlukan token API untuk pemilik skill atau anggota penerbit.
...
Meminta pemindaian ulang keamanan untuk versi skill terbaru yang diterbitkan.

Auth:

- Memerlukan token API untuk pemilik skill, admin penerbit, moderator platform,
  atau admin platform.
- Pemilik dan admin penerbit tunduk pada batas pemulihan pemilik per versi.
...
`note` wajib untuk `accepted` dan `rejected`; ini dapat dihilangkan ketika mengatur
...
- `security.hasScanResult` bernilai `true` hanya saat pemindai menghasilkan putusan definitif (`clean`, `suspicious`, atau `malicious`).
...
Menghapus secara lunak paket dan semua rilis.
...
- Rilis ClawPack menyertakan field yang kompatibel dengan npm, yaitu `npmIntegrity`, `npmShasum`, dan
  `npmTarballName`.
...
ditautkan ke versi. Laporan masuk ke antrean moderasi tetapi tidak menyembunyikan otomatis atau
...
Endpoint pemilik/penerbit paket untuk mengajukan banding atas moderasi pada rilis.
...
mencurigakan, atau berbahaya. ClawHub mempertahankan satu banding terbuka per rilis.
...
Meminta pemindaian ulang keamanan untuk rilis paket terbaru yang dipublikasikan.
...
`note` wajib untuk `accepted` dan `rejected`; ini dapat dihilangkan saat
mengatur `status` kembali ke `open`. Teruskan `finalAction: "approve"` dengan
banding yang diterima untuk menyetujui rilis yang terpengaruh dalam alur kerja
yang sama dan dapat diaudit.
...
`finalAction: "revoke"` dengan laporan yang dikonfirmasi untuk menerapkan
...
- Baris yang didukung ClawPack yang sudah ada tetapi tidak memiliki `artifactKind`
...
- Rute ini tetap hanya ZIP. Rute ini tidak melakukan streaming file `.tgz` ClawPack.
...
- Packument paket scoped mendukung jalur permintaan `/api/npm/@scope/name` dan
  jalur permintaan terenkripsi npm `/api/npm/@scope%2Fname`.
...
- Bidang payload opsional: `ownerHandle`. Jika ada, API menyelesaikan penerbit
...
- Disarankan: `multipart/form-data` dengan JSON `payload` + blob `files[]`.
- Isi JSON dengan `files` (berbasis storageId) juga diterima.
- Bidang payload opsional: `ownerHandle`. Jika ada, hanya admin yang dapat mempublikasikan atas nama pemilik tersebut.
...
- Hanya penerbit tepercaya yang dapat memublikasikan ke channel `official`.
...
penerbit pengguna/personal bersama legacy, endpoint terlebih dahulu memigrasikannya menjadi penerbit org.
...
Hanya admin. Mencadangkan slug akar dan nama paket untuk pemilik yang sah tanpa menerbitkan
rilis. Nama paket menjadi paket placeholder privat tanpa baris rilis, sehingga pemilik yang sama
nantinya dapat menerbitkan rilis Plugin kode atau Plugin bundel yang sebenarnya ke nama tersebut.
...
Blokir pengguna dan hapus permanen skill miliknya (hanya moderator/admin).
