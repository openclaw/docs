...
Direktori pihak ketiga dapat menggunakan endpoint baca publik untuk mencantumkan atau mencari Skills ClawHub. Harap cache hasil, patuhi `429`/`Retry-After`, tautkan pengguna kembali ke listing ClawHub kanonis (`https://clawhub.ai/<owner>/<slug>`), dan hindari menyiratkan dukungan ClawHub terhadap situs pihak ketiga. Jangan mencoba menduplikasi konten tersembunyi, privat, atau diblokir moderasi di luar permukaan API publik.
...
- Endpoint tulis terautentikasi sebaiknya tidak mengembalikan `Unauthorized` polos saat
  ...
  akun dihapus/diblokir/dinonaktifkan masing-masing harus mendapatkan teks yang dapat ditindaklanjuti agar klien CLI
  dapat memberi tahu pengguna apa yang memblokir mereka.
...
- Distandardisasi: `RateLimit-Limit`, `RateLimit-Remaining`, `RateLimit-Reset`
...
- Jika `Retry-After` ada, tunggu selama sekian detik sebelum mencoba ulang.
...
- Jika tidak ada IP klien tepercaya yang tersedia, permintaan unduhan anonim menggunakan bucket fallback berskala endpoint alih-alih satu bucket global `ip:unknown`. Permintaan baca/tulis anonim tetap menggunakan bucket tidak dikenal bersama sehingga routing IP hilang tetap terlihat dan konservatif.
...
- Hasil dikembalikan dalam urutan relevansi (kesamaan embedding + peningkatan token slug/nama persis + prior popularitas dari unduhan).
...
- Teks ASCII ditokenisasi pada batas kata dan tanda baca. Misalnya, `personal-map` berisi token mandiri `map`, sedangkan `amap-jsapi-skill` berisi `amap`, `jsapi`, dan `skill`; karena itu, pencarian `map` memberikan `personal-map` kecocokan leksikal yang lebih kuat daripada `amap-jsapi-skill`.
...
- Masukkan istilah yang secara harfiah akan dicari pengguna ke dalam nama tampilan, ringkasan, dan tag. Gunakan token slug mandiri hanya jika itu juga identitas stabil yang ingin Anda pertahankan.
...
- Alias penggantian nama mempertahankan resolusi untuk URL lama dan instalasi yang di-resolve melalui registry, tetapi peringkat pencarian didasarkan pada metadata Skill kanonis setelah penggantian nama terindeks. Statistik yang ada tetap bersama Skill.
...
- `createdAt` stabil untuk crawl Skill baru; `updated` berubah saat Skills yang ada diterbitkan ulang.
- Saat `nonSuspiciousOnly=true`, sort berbasis kursor dapat mengembalikan item lebih sedikit dari `limit` pada suatu halaman karena Skills mencurigakan difilter setelah pengambilan halaman.
...
Laporkan Skill untuk peninjauan moderator. Laporan berskala Skill, opsional ditautkan
...
yang sudah ditriage untuk menyembunyikan Skill dalam alur kerja teraudit yang sama.
...
Menghapus paket dan semua rilisnya secara soft-delete.

Catatan:

- Memerlukan token API untuk pemilik paket, owner/admin publisher org,
...
- Rilis ClawPack menyertakan field yang kompatibel dengan npm `npmIntegrity`, `npmShasum`, dan
  `npmTarballName`.
...
- Versi paket legacy mengembalikan artefak `legacy-zip` dan ZIP legacy
  `downloadUrl`.
...
- Versi ClawPack melakukan stream byte `.tgz` npm-pack persis seperti yang diunggah.
- Versi ZIP legacy mengalihkan ke `/api/v1/packages/{name}/download?version=`.
- Menggunakan bucket rate unduhan.

### `GET /api/v1/packages/{name}/readiness`

Mengembalikan readiness yang dihitung untuk konsumsi OpenClaw di masa mendatang.

Pemeriksaan readiness mencakup:
...
- `packageName` dinormalisasi sebagai nama npm; paket dapat tidak ada untuk migrasi
  yang direncanakan.
- Ini hanya melacak readiness migrasi. Ini tidak memutasi OpenClaw atau menghasilkan
  ClawPacks.
...
Melaporkan paket untuk review moderator. Laporan berada pada tingkat paket, secara opsional
ditautkan ke versi. Laporan tersebut masuk ke antrean moderasi tetapi tidak menyembunyikan otomatis atau
...
- Memerlukan token API untuk pemilik paket, anggota publisher, moderator, atau
  pengguna admin.
...
- Packument paket berscope mendukung path permintaan `/api/npm/@scope/name` dan
  path permintaan terenkripsi npm `/api/npm/@scope%2Fname`.
...
- Header unduhan menyertakan SHA-256 ClawHub beserta metadata integrity/shasum npm.
...
- Paket Plugin memerlukan `openclaw.plugin.json`. Unggahan `.tgz` ClawPack harus
  berisi file tersebut di `package/openclaw.plugin.json`.
- Plugin kode memerlukan `package.json`, metadata repo sumber, metadata commit
...
Menambah/menghapus bintang (sorotan). Kedua endpoint bersifat idempoten.
