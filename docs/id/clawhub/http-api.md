...
x-i18n:
    generated_at: "2026-06-27T17:14:52Z"
    model: gpt-5.5
...
`/api/...` dan `/api/cli/...` legacy tetap ada untuk kompatibilitas (lihat `DEPRECATIONS.md`).
OpenAPI: `/api/v1/openapi.json`.

## Penggunaan ulang katalog publik

Direktori pihak ketiga dapat menggunakan endpoint baca publik untuk mencantumkan atau mencari keterampilan ClawHub. Harap cache hasil, patuhi `429`/`Retry-After`, tautkan pengguna kembali ke daftar ClawHub kanonis (`https://clawhub.ai/<owner>/skills/<slug>`), dan hindari memberi kesan bahwa ClawHub mendukung situs pihak ketiga tersebut. Jangan mencoba mencerminkan konten tersembunyi, privat, atau diblokir moderasi di luar permukaan API publik.
...
- Distandardisasi: `RateLimit-Limit`, `RateLimit-Reset`
...
- `X-RateLimit-Remaining` / `RateLimit-Remaining`: anggaran tersisa persis ketika ada.
...
- Jika `Retry-After` ada, tunggu selama jumlah detik tersebut sebelum mencoba lagi.
- Gunakan backoff berjitter untuk menghindari percobaan ulang yang tersinkronisasi.
...
- Jika tidak ada IP klien tepercaya yang tersedia, permintaan anonim menggunakan bucket fallback
  yang dicakup hanya berdasarkan jenis batas laju. Bucket fallback ini tidak mencakup
...
Ini mencakup kegagalan validasi (`400`), sumber daya publik yang hilang (`404`), kegagalan auth dan
...
- `highlightedOnly` (opsional): `true` untuk memfilter ke keterampilan yang disorot
- `nonSuspiciousOnly` (opsional): `true` untuk menyembunyikan keterampilan mencurigakan (`flagged.suspicious`)
...
- Relevansi lebih kuat daripada popularitas. Kecocokan token slug atau nama tampilan yang presisi dapat mengungguli kecocokan yang lebih longgar dengan engagement jauh lebih kuat.
- Teks ASCII ditokenisasi pada batas kata dan tanda baca. Misalnya, `personal-map` berisi token `map` mandiri, sementara `amap-jsapi-skill` berisi `amap`, `jsapi`, dan `skill`; karena itu pencarian `map` memberi `personal-map` kecocokan leksikal yang lebih kuat daripada `amap-jsapi-skill`.
- Popularitas diskalakan log dan dibatasi. Keterampilan dengan engagement tinggi dapat berperingkat lebih rendah ketika teks kueri memiliki kecocokan yang lebih lemah.
- Status moderasi mencurigakan atau tersembunyi dapat menghapus keterampilan dari pencarian publik bergantung pada filter pemanggil dan status moderasi saat ini.

Panduan keterlihatan penerbit:

- Letakkan istilah yang secara harfiah akan dicari pengguna di nama tampilan, ringkasan, dan tag. Gunakan token slug mandiri hanya ketika itu juga merupakan identitas stabil yang ingin Anda pertahankan.
...
- Alias penggantian nama mempertahankan resolusi untuk URL lama dan instalasi yang diselesaikan melalui registry, tetapi peringkat pencarian didasarkan pada metadata keterampilan kanonis setelah penggantian nama terindeks. Statistik yang ada tetap bersama keterampilan tersebut.
- Jika suatu keterampilan tiba-tiba tidak terlihat, periksa status moderasi terlebih dahulu dengan `clawhub inspect @owner/slug` saat masuk sebelum mengubah metadata terkait peringkat.
...
- `sort` (opsional): `updated` (default), `recommended` (alias: `default`), `createdAt` (alias: `newest`), `downloads`, `stars` (alias: `rating`), alias instalasi legacy `installsCurrent`/`installs`/`installsAllTime` dipetakan ke `downloads`, `trending`
- `nonSuspiciousOnly` (opsional): `true` untuk menyembunyikan keterampilan mencurigakan (`flagged.suspicious`)
- `nonSuspicious` (opsional): alias legacy untuk `nonSuspiciousOnly`
...
- `trending` memeringkat berdasarkan instalasi dalam 7 hari terakhir (berbasis telemetri).
- `createdAt` stabil untuk crawl keterampilan baru; `updated` berubah ketika keterampilan yang ada dipublikasikan ulang.
- Ketika `nonSuspiciousOnly=true`, sort berbasis kursor dapat mengembalikan lebih sedikit dari `limit` item pada satu halaman karena keterampilan mencurigakan difilter setelah pengambilan halaman.
...
- Slug lama yang dibuat oleh alur penggantian nama/penggabungan pemilik diselesaikan ke keterampilan kanonis.
- `metadata.os`: pembatasan OS yang dideklarasikan di frontmatter keterampilan (misalnya `["macos"]`, `["linux"]`). `null` jika tidak dideklarasikan.
- `metadata.systems`: target sistem Nix (misalnya `["aarch64-darwin", "x86_64-linux"]`). `null` jika tidak dideklarasikan.
- `metadata` adalah `null` jika keterampilan tidak memiliki metadata platform.
- `moderation` disertakan hanya ketika keterampilan ditandai atau pemilik melihatnya.
...
- Pemilik dan moderator dapat mengakses detail moderasi untuk keterampilan tersembunyi.
- Pemanggil publik hanya mendapatkan `200` untuk keterampilan terlihat yang sudah ditandai.
- Bukti disunting untuk pemanggil publik dan hanya menyertakan cuplikan mentah untuk pemilik/moderator.

### `POST /api/v1/skills/{slug}/report`

Laporkan keterampilan untuk ditinjau moderator. Laporan berada pada level keterampilan, secara opsional ditautkan
ke versi, dan masuk ke antrean laporan keterampilan.
...
Endpoint moderator/admin untuk penerimaan laporan keterampilan.
...
Endpoint moderator/admin untuk menyelesaikan atau membuka ulang laporan keterampilan.
...
mengatur `status` kembali ke `open`. Teruskan `finalAction: "hide"` dengan laporan yang telah ditriage
untuk menyembunyikan keterampilan dalam alur kerja yang sama dan dapat diaudit.
...
Mengembalikan detail verifikasi pemindaian keamanan untuk versi keterampilan.
...
- Jika `version` maupun `tag` tidak diberikan, gunakan versi terbaru.
...
- Saat membuat kueri versi historis, periksa `moderation.matchesRequestedVersion` dan `moderation.sourceVersion` sebelum memperlakukan `moderation` dan `security` sebagai konteks versi yang sama.
...
- Pemindaian yang dipublikasikan memerlukan akses manajemen pemilik/penerbit, atau wewenang moderator/admin platform.
- Pemindaian yang dipublikasikan hanya menulis balik ketika `update: true` dan pemindaian selesai dengan berhasil.
...
- Memerlukan akses manajemen pemilik/penerbit ke skill atau plugin, atau wewenang moderator/admin platform.
...
- Identitas skill, identitas penerbit, dan metadata versi yang dipilih adalah field amplop tingkat atas (`slug`, `displayName`, `publisherHandle`, `version`, `resolvedFrom`, `tag`, `createdAt`) sehingga otomatisasi shell dapat membacanya tanpa membongkar wrapper bersarang.
...
Mengembalikan putusan keamanan ringkas saat ini untuk versi skill yang tepat. Endpoint
...
- Respons ini hanya keamanan. Respons ini tidak menyertakan data Skill Card, status kartu yang dihasilkan, daftar file artefak, atau payload pemindai terperinci.
...
- `security.signals.dependencyRegistry` dipertahankan untuk kompatibilitas respons v1, tetapi pemindai keberadaan registry dependensi sudah dihentikan dan kunci ini selalu `null`.
- Ketiadaan Skill Card tidak memengaruhi `ok`, `decision`, atau `reasons` endpoint ini; klien sebaiknya membaca `skill-card.md` yang terinstal secara lokal ketika membutuhkan konten kartu.
...
- `isOfficial` (opsional): `true` atau `false`
- `category` (opsional): filter kategori plugin. Hanya didukung saat
  permintaan dibatasi ke paket plugin. Kategori terkontrol dan alias
  filter v1 lama didokumentasikan di bawah `GET /api/v1/plugins`.
...
Penjelajahan katalog khusus plugin di seluruh paket code-plugin dan bundle-plugin.
...
Alias lama tidak diterima sebagai nilai kategori yang disimpan atau dinyatakan oleh penulis.
...
- Skills yang dihosting menyertakan file versi tersimpan terbaru dan dicantumkan dalam
...
- `_errors.json` disertakan saat Skills atau file individual tidak dapat
  diekspor.
...
- Kontrol pengurutan UI browser untuk pencarian Plugin menyusun ulang hasil relevansi yang dimuat,
...
Menandai paket dan semua rilis sebagai dihapus.
...
- `version.sha256hash` adalah metadata kompatibilitas yang tidak disarankan lagi untuk klien lama. Ini
  melakukan hash terhadap byte ZIP persis yang dikembalikan oleh `/api/v1/packages/{name}/download`.
  Klien modern sebaiknya menggunakan `version.artifact.sha256`, yang mengidentifikasi
  artefak rilis kanonis.
- `version.vtAnalysis`, `version.llmAnalysis`, dan `version.staticScan` disertakan
  ketika data pemindaian ada.
- Paket privat mengembalikan `404` kecuali pemanggil dapat membaca publisher pemilik.
...
- `trust.stale` berarti ringkasan kepercayaan dihitung dari input yang kedaluwarsa dan
  sebaiknya diperlakukan sebagai memerlukan penyegaran sebelum keputusan mengizinkan dengan keyakinan tinggi.

Catatan:

- Endpoint ini spesifik versi. Klien sebaiknya memanggilnya setelah menyelesaikan
  versi paket yang ingin diinstal, bukan hanya setelah membaca metadata paket
  terbaru.
- Paket privat mengembalikan `404` kecuali pemanggil dapat membaca publisher pemilik.
- Endpoint ini sengaja lebih sempit daripada endpoint moderasi pemilik/moderator.
  Ini mengekspos keputusan instalasi dan penjelasan publik, bukan
...
- Versi ClawPack men-stream byte `.tgz` `npm-pack` unggahan yang persis.
...
- `packageName` dinormalisasi sebagai nama npm; paket dapat tidak ada untuk migrasi yang
  direncanakan.
...
- `manual`: rilis apa pun dengan pengesampingan moderasi manual.
- `all`: rilis apa pun dengan pengesampingan manual, status pemindaian tidak bersih, atau laporan paket.
...
ditautkan ke versi. Laporan mengisi antrean moderasi tetapi tidak menyembunyikan otomatis atau
...
- Batas ukuran file: 200KB.
- Pemindaian VirusTotal yang tertunda tidak memblokir pembacaan; rilis berbahaya tetap dapat ditahan di tempat lain.
...
- `dist.tarball`, `dist.integrity`, dan `dist.shasum` menggunakan field yang
...
- Header unduhan menyertakan SHA-256 ClawHub ditambah metadata integritas/shasum npm.
...
Mengunduh ZIP versi skill yang di-hosting, atau mengembalikan handoff sumber GitHub untuk
skill saat ini yang berbasis GitHub dengan pemindaian `clean` atau `suspicious` dan tanpa versi
yang di-hosting.
...
- Jika baik `version` maupun `tag` tidak disediakan, versi terbaru digunakan.
...
- Handoff skill berbasis GitHub tidak mem-proxy atau me-mirror byte. Respons JSON
  ...
  dan `archiveUrl`; state pemindaian/saat ini adalah gate dan tidak disertakan sebagai metadata
  payload sukses.
...
- Field payload opsional: `migrateOwner`. Saat `true` dengan `ownerHandle`,
  skill yang sudah ada dapat dipindahkan ke pemilik tersebut jika aktor adalah admin/pemilik pada penerbit
...
- Field formulir yang diizinkan adalah `payload`, blob `files` berulang, atau satu referensi tarball
  `clawpack`. `clawpack` dapat berupa blob `.tgz` atau id storage yang dikembalikan oleh
  alur upload-url. Publikasi storage-id yang di-stage juga harus menyertakan
  `clawpackUploadTicket` yang dikembalikan bersama URL unggahan tersebut.
- Gunakan salah satu dari `files` atau `clawpack`, jangan keduanya dalam permintaan yang sama.
...
- Hanya penerbit org `openclaw` dan penerbit personal milik anggota org `openclaw`
  saat ini yang dapat menerbitkan ke channel `official`.
...
pengguna bersama lama/penerbit personal lama, endpoint memigrasikannya menjadi penerbit org terlebih dahulu.
...
pemanggil sebagai pemilik. Endpoint ini tidak memigrasikan handle pengguna/personal yang sudah ada dan
...
nantinya dapat menerbitkan rilis code-plugin atau bundle-plugin yang sebenarnya ke nama tersebut.
...
Khusus admin. Memulihkan penerbit personal untuk prinsipal GitHub OAuth pengganti yang terverifikasi
tanpa mengedit baris akun Convex Auth. Permintaan harus menyebutkan kedua id akun provider GitHub
yang immutable; handle yang mutable hanya digunakan sebagai guard yang menghadap operator.

Endpoint secara default berjalan dalam mode dry-run. Menerapkan pemulihan memerlukan `dryRun: false` dan
`confirmIdentityVerified: true` setelah staf memverifikasi secara independen kesinambungan antara kedua
principal GitHub. Pemulihan gagal tertutup ketika publisher personal saat ini milik pengguna tujuan
memiliki skills, packages, atau sumber skill GitHub.
Pemulihan juga memigrasikan field `ownerUserId` lama untuk skills milik publisher yang dipulihkan,
alias slug skill, packages, peringatan inspektur package, dan baris digest pencarian turunan agar
jalur pemilik langsung selaras dengan otoritas publisher baru. Reservasi protected-handle aktif
...
Sumber skill GitHub bersifat tercakup pada publisher dan dilaporkan sebagai diperiksa, bukan ditulis ulang.
...
Blokir pengguna dan hapus permanen skills yang dimilikinya (hanya moderator/admin).
...
Ubah alasan tersimpan untuk blokir yang sudah ada tanpa membuka blokir atau memulihkan
konten (hanya admin). Secara default berjalan dalam mode dry-run kecuali `dryRun` bernilai `false`.
...
Tambahkan/hapus star (sorotan). Kedua endpoint bersifat idempoten.
...
yang menyiapkan tarball ClawPack harus mengirim id storage yang dihasilkan sebagai
