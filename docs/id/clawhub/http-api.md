...
summary: Referensi API HTTP (publik + endpoint CLI + auth).
x-i18n:
    generated_at: "2026-07-04T11:01:57Z"
    model: gpt-5.5
...
Direktori pihak ketiga dapat menggunakan endpoint baca publik untuk mencantumkan atau mencari Skills ClawHub. Harap cache hasil, patuhi `429`/`Retry-After`, arahkan pengguna kembali ke listing ClawHub kanonis (`https://clawhub.ai/<owner>/skills/<slug>`), dan hindari kesan bahwa ClawHub mendukung situs pihak ketiga tersebut. Jangan mencoba mencerminkan konten tersembunyi, privat, atau diblokir moderasi di luar permukaan API publik.

Pintasan slug web diselesaikan lintas keluarga registry, tetapi klien API harus menggunakan
URL kanonis yang dikembalikan oleh endpoint baca, bukan menyusun ulang prioritas
rute.
...
- `X-RateLimit-Remaining` / `RateLimit-Remaining`: anggaran tersisa yang tepat jika ada.
...
Respons error publik v1 berupa teks biasa dengan `content-type: text/plain; charset=utf-8`.
...
harus membaca body respons sebagai string yang dapat dibaca manusia. Parameter kueri yang tidak dikenal
...
- Popularitas diskalakan log dan dibatasi. Skills dengan engagement tinggi dapat berperingkat lebih rendah ketika teks kueri memiliki kecocokan yang lebih lemah.
...
- Masukkan istilah yang benar-benar akan dicari pengguna dalam nama tampilan, ringkasan, dan tag. Gunakan token slug mandiri hanya ketika itu juga merupakan identitas stabil yang ingin Anda pertahankan.
...
- Alias penggantian nama mempertahankan penyelesaian untuk URL lama dan instalasi yang diselesaikan melalui registry, tetapi peringkat pencarian didasarkan pada metadata Skill kanonis setelah penggantian nama diindeks. Statistik yang ada tetap bersama Skill.
- Jika sebuah Skill tiba-tiba tidak terlihat, periksa status moderasi terlebih dahulu dengan `clawhub inspect @owner/slug` saat sudah login sebelum mengubah metadata terkait peringkat.
...
- Gunakan `nextCursor` untuk melanjutkan paginasi jika ada. Halaman pendek tidak dengan sendirinya berarti akhir hasil.
...
Laporkan Skill untuk ditinjau moderator. Laporan berada pada level Skill, secara opsional ditautkan
...
Endpoint kirim terautentikasi untuk pekerjaan ClawScan baru.
...
- Payload permintaan pemindaian dan laporan yang dapat diunduh kedaluwarsa dari penyimpanan scan-request setelah jendela retensi.
- Pemindaian yang dipublikasikan memerlukan akses manajemen pemilik/penerbit, atau otoritas moderator/admin platform.
...
Endpoint polling terautentikasi untuk pemindaian yang dikirim.

- Mengembalikan status mengantre/berjalan/berhasil/gagal.
- Mengembalikan `queue.queuedAhead` dan `queue.position` saat mengantre agar klien dapat menampilkan berapa banyak pemindaian manual yang diprioritaskan berada di depan permintaan. Antrean yang sangat besar dibatasi dan dilaporkan dengan `queuedAheadIsEstimate: true`.
...
Endpoint arsip laporan tersimpan terautentikasi untuk versi yang dikirim.

- Memerlukan akses manajemen pemilik/penerbit ke skill atau plugin, atau otoritas moderator/admin platform.
- Mengembalikan hasil pemindaian tersimpan untuk versi persis yang dikirim, termasuk versi yang diblokir atau disembunyikan.
- `kind` default ke `skill`; gunakan `kind=plugin` untuk pemindaian plugin/paket.
- Mengembalikan bentuk ZIP yang sama seperti unduhan scan-request.
...
- Identitas skill, identitas penerbit, dan metadata versi yang dipilih adalah field amplop tingkat atas (`slug`, `displayName`, `publisherHandle`, `version`, `resolvedFrom`, `tag`, `createdAt`) sehingga otomasi shell dapat membacanya tanpa membongkar wrapper bersarang.
- `security` adalah vonis ClawScan/keamanan tingkat atas. Otomasi harus berpatokan pada `ok`, `decision`, `reasons`, dan `security.status`.
- `security.signals` berisi bukti pemindai pendukung seperti `staticScan`, `virusTotal`, dan `skillSpector`.
- `security.signals.dependencyRegistry` dipertahankan untuk kompatibilitas respons v1, tetapi pemindai keberadaan registry dependensi telah dihentikan dan kunci ini selalu `null`.
- `provenance` bernilai `server-resolved-github-import` hanya ketika ClawHub menyelesaikan dan menyimpan repo/ref/commit/path GitHub selama publikasi atau impor; jika tidak, nilainya `unavailable`.

### `POST /api/v1/skills/-/security-verdicts`

Mengembalikan vonis keamanan ringkas saat ini untuk versi skill persis. Endpoint
koleksi ini ditujukan bagi klien yang sudah mengetahui versi skill ClawHub
...
- Respons hanya keamanan. Respons tidak menyertakan data Skill Card, status kartu yang dihasilkan, daftar file artefak, atau payload pemindai terperinci.
...
- `security.signals.dependencyRegistry` dipertahankan untuk kompatibilitas respons v1, tetapi pemindai keberadaan registry dependensi telah dihentikan dan kunci ini selalu `null`.
- Ketiadaan Skill Card tidak memengaruhi `ok`, `decision`, atau `reasons` endpoint ini; klien harus membaca `skill-card.md` terinstal secara lokal ketika mereka membutuhkan konten kartu.
- Gunakan `/verify` ketika Anda membutuhkan amplop verifikasi Skill Card satu skill, `/card` ketika Anda membutuhkan markdown kartu yang dihasilkan, dan `/scan` ketika Anda membutuhkan data pemindai terperinci.
...
Alias lama tidak diterima sebagai nilai kategori yang disimpan atau dinyatakan oleh pembuat.

### `GET /api/v1/skills/export`

Ekspor massal Skill publik terbaru untuk analisis offline.
...
- Skill yang dihosting menyertakan file versi tersimpan terbaru dan dicantumkan di
...
- Skill saat ini yang didukung GitHub dengan pemindaian `clean` atau `suspicious` menyertakan
  ...
  hash konten, dan URL arsip. Skill tersebut tidak menyertakan file sumber yang dihosting ClawHub.
...
- `version.sha256hash` adalah metadata kompatibilitas usang untuk klien lama. Ini
  melakukan hash byte ZIP persis yang dikembalikan oleh `/api/v1/packages/{name}/download`.
...
- `trust.stale` berarti ringkasan kepercayaan dihitung dari masukan usang dan
  harus diperlakukan sebagai memerlukan penyegaran sebelum keputusan izinkan dengan keyakinan tinggi.
...
- Versi ZIP lama mengarahkan ke `/api/v1/packages/{name}/download?version=`.
...
Mengunduh arsip ZIP deterministik legacy untuk rilis paket.
...
- Arsip Plugin/paket adalah file zip dengan root `package/` sehingga klien OpenClaw lama
  tetap berfungsi.
...
- Versi legacy yang hanya ZIP sengaja dihilangkan.
...
- Packument paket scoped mendukung jalur permintaan `/api/npm/@scope/name` dan jalur
  terenkripsi npm `/api/npm/@scope%2Fname`.

### `GET /api/npm/{package}/-/{tarball}.tgz`

Melakukan streaming byte tarball ClawPack yang diunggah secara persis untuk klien mirror npm.
...
- Field payload opsional: `ownerHandle`. Jika ada, API menyelesaikan penerbit tersebut
  di sisi server dan mengharuskan aktor memiliki akses penerbit.
- Field payload opsional: `migrateOwner`. Jika `true` dengan `ownerHandle`, skill
  yang ada dapat dipindahkan ke pemilik tersebut jika aktor adalah admin/pemilik pada
...
- Hanya penerbit org `openclaw` dan penerbit pribadi anggota org `openclaw` saat ini
...
Penghapusan sementara yang diinisiasi pemilik mencadangkan slug selama 30 hari, lalu slug dapat diklaim oleh
...
penerbit pengguna/pribadi bersama legacy, endpoint memigrasikannya ke penerbit org terlebih dahulu.
...
Pembuatan penerbit org swalayan yang terautentikasi. Membuat penerbit org baru dan menambahkan
pemanggil sebagai pemilik. Endpoint ini tidak memigrasikan handle pengguna/pribadi yang sudah ada dan
tidak menandai penerbit sebagai tepercaya/official.
...
yang immutable; handle yang mutable hanya digunakan sebagai guard yang terlihat bagi operator.

Titik akhir secara default menggunakan dry-run. Menerapkan pemulihan memerlukan `dryRun: false` dan
`confirmIdentityVerified: true` setelah staf memverifikasi secara independen kesinambungan antara kedua
prinsipal GitHub. Pemulihan gagal tertutup ketika penerbit pribadi pengguna tujuan saat ini
memiliki skills, package, atau sumber skill GitHub.
Pemulihan juga memigrasikan bidang `ownerUserId` lama untuk skill penerbit yang dipulihkan,
alias slug skill, package, peringatan pemeriksa package, dan baris digest pencarian turunan sehingga
jalur pemilik langsung sesuai dengan otoritas penerbit baru. Reservasi handle terlindungi aktif
untuk handle yang dipulihkan juga dialihkan ke pengguna pengganti sehingga sinkronisasi
profil berikutnya tidak dapat memulihkan otoritas pesaing milik pengguna sebelumnya. Setiap tabel utama dibatasi hingga
...
Sumber skill GitHub tercakup pada penerbit dan dilaporkan sebagai telah diperiksa, bukan ditulis ulang.

- Body: `{ "handle": "gingiris", "nextUserHandle": "gingiris-1031", "previousGitHubProviderAccountId": "123", "nextGitHubProviderAccountId": "456", "reason": "Kontinuitas akun terverifikasi untuk issue #2555", "confirmIdentityVerified": true, "dryRun": false }`
- Respons: `{ "ok": true, "dryRun": false, "recovered": true, "publisherId": "...", "handle": "gingiris", "previousUser": { "userId": "...", "handle": "gingiris", "nextHandle": "gingiris-recovered", "githubProviderAccountId": "123", "authAccountCount": 1 }, "nextUser": { "userId": "...", "handle": "gingiris-1031", "nextHandle": "gingiris", "githubProviderAccountId": "456", "authAccountCount": 1 }, "retiredPersonalPublisher": null, "resourceOwnerMigration": { "limitPerTable": 100, "skills": 1, "skillSlugAliases": 1, "packages": 0, "packageInspectorWarnings": 0, "githubSourcesChecked": 1, "handleReservations": 1 }, "identityVerified": true, "reason": "Kontinuitas akun terverifikasi untuk issue #2555" }`
...
- `POST /api/v1/skills/{slug}/transfer`
  - Body: `{ "toUserHandle": "target_handle", "message": "opsional" }`
...
Blokir pengguna dan hapus permanen skill yang dimiliki (khusus moderator/admin).

Body:

```json
{ "handle": "user_handle", "reason": "alasan blokir opsional" }
```

atau

```json
{ "userId": "users_...", "reason": "alasan blokir opsional" }
```
...
Cabut blokir pengguna dan pulihkan skill yang memenuhi syarat (khusus admin).

Body:

```json
{ "handle": "user_handle", "reason": "alasan cabut blokir opsional" }
```

atau

```json
{ "userId": "users_...", "reason": "alasan cabut blokir opsional" }
```
...
Ubah alasan yang tersimpan untuk blokir yang sudah ada tanpa mencabut blokir atau memulihkan
konten (khusus admin). Secara default menggunakan dry-run kecuali `dryRun` adalah `false`.
...
Cantumkan atau cari pengguna (khusus admin).
