...
summary: Referensi HTTP API (publik + endpoint CLI + auth).
x-i18n:
    generated_at: "2026-07-01T15:31:01Z"
    model: gpt-5.5
...
- `X-RateLimit-Remaining` / `RateLimit-Remaining`: sisa anggaran tepat saat ada.
...
- Jika tidak ada IP klien tepercaya yang tersedia, permintaan anonim menggunakan bucket fallback
  ...
  path, slug, nama paket, versi, string kueri, atau parameter artefak lain yang disediakan pemanggil.

## Respons galat

Respons galat v1 publik berupa teks polos dengan `content-type: text/plain; charset=utf-8`.
...
- Popularitas diskalakan secara log dan dibatasi. Skill dengan keterlibatan tinggi dapat berperingkat lebih rendah ketika teks kueri adalah kecocokan yang lebih lemah.
...
- Jangan mengganti nama slug hanya untuk mengejar satu kueri kecuali slug baru tersebut adalah nama kanonis jangka panjang yang lebih baik. Slug lama menjadi alias pengalihan, tetapi URL kanonis, slug yang ditampilkan, dan intisari pencarian mendatang menggunakan slug baru.
- Alias penggantian nama mempertahankan resolusi untuk URL lama dan instalasi yang diselesaikan melalui registri, tetapi peringkat pencarian didasarkan pada metadata skill kanonis setelah penggantian nama diindeks. Statistik yang ada tetap bersama skill tersebut.
...
- `createdAt` stabil untuk crawl skill baru; `updated` berubah saat skill yang ada diterbitkan ulang.
- Ketika `nonSuspiciousOnly=true`, sort berbasis kursor dapat mengembalikan item lebih sedikit dari `limit` pada sebuah halaman karena skill mencurigakan difilter setelah pengambilan halaman.
...
Laporkan skill untuk tinjauan moderator. Laporan berada pada level skill, secara opsional ditautkan
...
yang telah ditriage untuk menyembunyikan skill dalam alur kerja yang sama dan dapat diaudit.
...
- Pekerjaan pemindaian bersifat asinkron. Permintaan pemindaian manual diprioritaskan di depan pekerjaan publikasi/backfill normal, tetapi penyelesaiannya tetap bergantung pada ketersediaan worker.
...
- Identitas skill, identitas penerbit, dan metadata versi yang dipilih adalah field envelope tingkat atas (`slug`, `displayName`, `publisherHandle`, `version`, `resolvedFrom`, `tag`, `createdAt`) agar otomasi shell dapat membacanya tanpa membongkar wrapper bertingkat.
...
- Hasil bersifat per item; satu skill atau versi yang tidak ada tidak menggagalkan seluruh respons.
- Respons hanya berisi keamanan. Respons ini tidak menyertakan data Kartu Skill, status kartu yang dihasilkan, daftar file artefak, atau payload pemindai terperinci.
...
- `security.signals.dependencyRegistry` dipertahankan untuk kompatibilitas respons v1, tetapi pemindai keberadaan registri dependensi sudah dihentikan dan kunci ini selalu `null`.
- Ketiadaan Kartu Skill tidak memengaruhi `ok`, `decision`, atau `reasons` endpoint ini; klien sebaiknya membaca `skill-card.md` terpasang secara lokal ketika memerlukan konten kartu.
- Gunakan `/verify` ketika Anda memerlukan envelope verifikasi Kartu Skill untuk satu skill, `/card` ketika Anda memerlukan markdown kartu yang dihasilkan, dan `/scan` ketika Anda memerlukan data pemindai terperinci.
...
- Skill yang di-host menyertakan file versi tersimpan terbaru dan tercantum dalam
...
- Alias filter v1 legacy yang didokumentasikan di bawah `GET /api/v1/plugins` juga
  diterima.
...
- `version.sha256hash` adalah metadata kompatibilitas yang tidak digunakan lagi untuk klien lama. Field ini
...
- `version.vtAnalysis`, `version.llmAnalysis`, dan `version.staticScan`
  disertakan saat data pemindaian tersedia.
- Paket privat mengembalikan `404` kecuali pemanggil dapat membaca publisher pemiliknya.
...
- `trust.blockedFromDownload` adalah sinyal pemblokiran instalasi. OpenClaw dan klien
  instalasi lainnya harus memblokir instalasi saat nilai ini `true`, alih-alih
...
- `trust.pending` berarti satu atau beberapa input kepercayaan masih menunggu selesai.
...
- Endpoint ini tepat versi. Klien harus memanggilnya setelah menyelesaikan
...
- Versi ClawPack melakukan streaming byte `.tgz` npm-pack persis yang diunggah.
...
Melaporkan sebuah paket untuk peninjauan moderator. Laporan berada di tingkat paket, secara opsional
ditautkan ke sebuah versi. Laporan memasok antrean moderasi tetapi tidak otomatis menyembunyikan atau
memblokir unduhan sendiri; moderator harus menggunakan moderasi rilis untuk
...
Skills berbasis GitHub saat ini dengan pemindaian `clean` atau `suspicious` dan tanpa versi
yang di-host.
...
- Field payload opsional: `migrateOwner`. Saat `true` dengan `ownerHandle`, skill
  yang sudah ada dapat dipindahkan ke pemilik tersebut jika aktor adalah admin/pemilik pada kedua
...
- Body JSON dan metadata `payload.files` / `payload.artifact` yang disuplai pemanggil
  ditolak.
...
- Field payload opsional: `ownerHandle`. Saat ada, hanya admin yang boleh mempublikasikan atas nama pemilik tersebut.
...
- Hanya penerbit org `openclaw` dan penerbit personal milik anggota org `openclaw`
  saat ini yang dapat mempublikasikan ke channel `official`.
...
Khusus admin. Mencadangkan slug root dan nama paket untuk pemilik yang sah tanpa mempublikasikan
rilis. Nama paket menjadi paket placeholder privat tanpa baris rilis, sehingga pemilik yang sama
nantinya dapat mempublikasikan rilis code-plugin atau bundle-plugin yang sebenarnya ke nama tersebut.
...
Pemulihan juga memigrasikan bidang legacy `ownerUserId` untuk skill penerbit yang dipulihkan,
alias slug skill, package, peringatan pemeriksa package, dan baris digest pencarian turunan agar
path pemilik langsung selaras dengan otoritas penerbit baru. Reservasi protected-handle aktif
untuk handle yang dipulihkan juga dialihkan ke pengguna pengganti sehingga sinkronisasi profil berikutnya
tidak dapat memulihkan otoritas bersaing milik pengguna sebelumnya. Setiap tabel primer dibatasi hingga
...
Sumber skill GitHub bersifat tercakup pada penerbit dan dilaporkan sebagai diperiksa, bukan ditulis ulang.
...
Ubah alasan tersimpan untuk blokir yang ada tanpa membuka blokir atau memulihkan
konten (hanya admin). Secara default menggunakan dry-run kecuali `dryRun` bernilai `false`.
...
- `limit` (opsional): hasil maksimal (default 20, maksimal 200)
...
## Endpoint CLI legacy (deprecated)
...
`POST /api/cli/upload-url` mengembalikan `uploadUrl` dan `uploadTicket`. Penerbitan package
