...
- Jika tidak ada IP klien tepercaya yang tersedia, permintaan unduhan anonim menggunakan bucket fallback yang dibatasi per endpoint alih-alih satu bucket global `ip:unknown`. Permintaan baca/tulis anonim tetap menggunakan bucket unknown bersama agar perutean IP yang hilang tetap terlihat dan konservatif.
...
- Unduhan digunakan sebagai prior berskala log kecil dan pemutus seri, bukan sebagai sinyal peringkat utama. Skill dengan unduhan tinggi dapat mendapat peringkat lebih rendah ketika teks kueri merupakan kecocokan yang lebih lemah.
...
- Letakkan istilah yang akan benar-benar dicari pengguna dalam nama tampilan, ringkasan, dan tag. Gunakan token slug mandiri hanya ketika token itu juga merupakan identitas stabil yang ingin Anda pertahankan.
...
- Alias penggantian nama mempertahankan penyelesaian untuk URL lama dan instalasi yang diselesaikan melalui registri, tetapi peringkat pencarian didasarkan pada metadata skill kanonis setelah penggantian nama diindeks. Statistik yang sudah ada tetap bersama skill tersebut.
- Jika sebuah skill secara tidak terduga tidak terlihat, periksa status moderasi terlebih dahulu dengan `clawhub inspect <slug>` saat sudah masuk sebelum mengubah metadata terkait peringkat.
...
- Ketika `nonSuspiciousOnly=true`, sort berbasis kursor dapat mengembalikan item lebih sedikit daripada `limit` pada satu halaman karena skill mencurigakan difilter setelah pengambilan halaman.
...
Melaporkan skill untuk ditinjau moderator. Laporan berada di tingkat skill, secara opsional ditautkan
...
Mengembalikan metadata penyelesai artefak eksplisit untuk sebuah versi paket.
...
- Ini adalah permukaan penyelesai OpenClaw; ini menghindari tebakan format arsip dari
  URL bersama.

### `GET /api/v1/packages/{name}/versions/{version}/artifact/download`

Mengunduh artefak versi melalui jalur penyelesai eksplisit.
...
Melaporkan paket untuk ditinjau moderator. Laporan berada pada tingkat paket,
secara opsional ditautkan ke sebuah versi. Laporan ini masuk ke antrean moderasi tetapi tidak secara otomatis menyembunyikan atau
...
`note` diperlukan untuk `confirmed` dan `dismissed`; dapat dihilangkan saat
...
- Packument paket berscope mendukung path permintaan `/api/npm/@scope/name` dan path permintaan npm
  yang dienkode `/api/npm/@scope%2Fname`.
...
- Field payload opsional: `ownerHandle`. Jika ada, API menyelesaikan penerbit itu
...
nantinya dapat memublikasikan rilis code-plugin atau bundle-plugin yang sebenarnya ke nama tersebut.
...
Melarang pengguna dan menghapus permanen skill yang dimiliki (khusus moderator/admin).
