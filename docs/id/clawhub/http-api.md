...
x-i18n:
    generated_at: "2026-07-03T02:55:31Z"
    model: gpt-5.5
...
Jalur lama `/api/...` dan `/api/cli/...` tetap ada untuk kompatibilitas (lihat `DEPRECATIONS.md`).
OpenAPI: `/api/v1/openapi.json`.

## Penggunaan ulang katalog publik

Direktori pihak ketiga boleh menggunakan endpoint baca publik untuk mencantumkan atau mencari Skills ClawHub. Harap cache hasil, patuhi `429`/`Retry-After`, tautkan pengguna kembali ke daftar ClawHub kanonis (`https://clawhub.ai/<owner>/skills/<slug>`), dan hindari menyiratkan dukungan ClawHub terhadap situs pihak ketiga tersebut. Jangan mencoba mencerminkan konten tersembunyi, privat, atau diblokir moderasi di luar permukaan API publik.
...
- Endpoint tulis terautentikasi sebaiknya tidak mengembalikan `Unauthorized` kosong ketika
  ...
  akun yang dihapus/diblokir/dinonaktifkan masing-masing sebaiknya mendapatkan teks yang dapat ditindaklanjuti agar klien
...
- Terstandar: `RateLimit-Limit`, `RateLimit-Reset`
...
- Gunakan backoff berjitter untuk menghindari percobaan ulang tersinkronisasi.
- Jika `Retry-After` hilang, gunakan fallback ke `RateLimit-Reset` (atau hitung dari `X-RateLimit-Reset`).
...
- Hasil dikembalikan dalam urutan relevansi (kemiripan embedding + peningkatan token slug/nama yang tepat + prior popularitas kecil).
- Relevansi lebih kuat daripada popularitas. Kecocokan token slug atau nama tampilan yang tepat dapat mengungguli kecocokan yang lebih longgar dengan keterlibatan yang jauh lebih kuat.
- Teks ASCII ditokenisasi pada batas kata dan tanda baca. Misalnya, `personal-map` berisi token `map` mandiri, sementara `amap-jsapi-skill` berisi `amap`, `jsapi`, dan `skill`; karena itu pencarian untuk `map` memberi `personal-map` kecocokan leksikal yang lebih kuat daripada `amap-jsapi-skill`.
- Popularitas diskalakan secara log dan dibatasi. Skills dengan keterlibatan tinggi dapat berperingkat lebih rendah ketika teks kueri memiliki kecocokan yang lebih lemah.
...
- Masukkan istilah yang akan benar-benar dicari pengguna dalam nama tampilan, ringkasan, dan tag. Gunakan token slug mandiri hanya ketika itu juga merupakan identitas stabil yang ingin Anda pertahankan.
...
- Jika sebuah Skill tiba-tiba tidak terlihat, periksa status moderasi terlebih dahulu dengan `clawhub inspect @owner/slug` saat masuk sebelum mengubah metadata terkait peringkat.
...
- `createdAt` stabil untuk crawl Skill baru; `updated` berubah ketika Skills yang ada diterbitkan ulang.
- Ketika `nonSuspiciousOnly=true`, sort berbasis kursor dapat mengembalikan item lebih sedikit dari `limit` pada satu halaman karena Skills mencurigakan difilter setelah pengambilan halaman.
...
- Slug lama yang dibuat oleh alur penggantian nama/penggabungan pemilik diselesaikan ke Skill kanonis.
...
Laporkan Skill untuk peninjauan moderator. Laporan berada pada tingkat Skill, secara opsional ditautkan
...
`note` wajib untuk `confirmed` dan `dismissed`; ini boleh dihilangkan ketika
...
- `version.security` menyertakan status verifikasi pemindaian ternormalisasi dan detail pemindai
...
- Pemindaian yang dipublikasikan memerlukan akses pengelolaan pemilik/penerbit, atau otoritas moderator/admin platform.
...
- Tugas pemindaian bersifat asinkron. Permintaan pemindaian manual diprioritaskan sebelum pekerjaan publikasi/backfill normal, tetapi penyelesaian tetap bergantung pada ketersediaan worker.
...
- Mengembalikan `queue.queuedAhead` dan `queue.position` saat dalam antrean sehingga klien dapat menunjukkan berapa banyak pemindaian manual yang diprioritaskan berada di depan permintaan. Antrean yang sangat besar dibatasi dan dilaporkan dengan `queuedAheadIsEstimate: true`.
...
- Memerlukan akses pengelolaan pemilik/penerbit ke skill atau plugin, atau otoritas moderator/admin platform.
...
- `kind` default ke `skill`; gunakan `kind=plugin` untuk pemindaian plugin/package.
...
Mengembalikan envelope verifikasi Skill Card yang digunakan oleh `clawhub skill verify`.
...
- Identitas skill, identitas penerbit, dan metadata versi yang dipilih adalah field envelope tingkat atas (`slug`, `displayName`, `publisherHandle`, `version`, `resolvedFrom`, `tag`, `createdAt`) sehingga otomasi shell dapat membacanya tanpa membongkar wrapper bertingkat.
- `security` adalah putusan ClawScan/keamanan tingkat atas. Otomasi harus berpatokan pada `ok`, `decision`, `reasons`, dan `security.status`.
- `security.signals` berisi bukti pemindai pendukung seperti `staticScan`, `virusTotal`, dan `skillSpector`.
- `security.signals.dependencyRegistry` dipertahankan untuk kompatibilitas respons v1, tetapi pemindai keberadaan registri dependensi sudah dipensiunkan dan kunci ini selalu `null`.
- `provenance` bernilai `server-resolved-github-import` hanya ketika ClawHub menyelesaikan dan menyimpan repo/ref/commit/path GitHub selama publikasi atau impor; selain itu nilainya `unavailable`.

### `POST /api/v1/skills/-/security-verdicts`

Mengembalikan putusan keamanan ringkas saat ini untuk versi skill yang tepat. Endpoint
koleksi ini ditujukan untuk klien yang sudah tahu versi skill ClawHub terpasang
mana yang perlu mereka tampilkan, seperti OpenClaw Control UI.
...
- Respons hanya berisi keamanan. Respons tidak menyertakan data Skill Card, status kartu yang dihasilkan, daftar file artefak, atau payload pemindai terperinci.
...
- `security.signals.dependencyRegistry` dipertahankan untuk kompatibilitas respons v1, tetapi pemindai keberadaan registri dependensi sudah dipensiunkan dan kunci ini selalu `null`.
- Ketiadaan Skill Card tidak memengaruhi `ok`, `decision`, atau `reasons` endpoint ini; klien harus membaca `skill-card.md` terpasang secara lokal saat mereka membutuhkan konten kartu.
- Gunakan `/verify` saat Anda memerlukan envelope verifikasi Skill Card untuk satu skill, `/card` saat Anda memerlukan markdown kartu yang dihasilkan, dan `/scan` saat Anda memerlukan data pemindai terperinci.
...
- `category` (opsional): filter kategori Plugin. Didukung hanya ketika
  permintaan dicakup ke paket Plugin (`/api/v1/plugins`,
...
- `isOfficial` (opsional): `true` atau `false`
- `category` (opsional): filter kategori Plugin. Didukung hanya ketika
  permintaan dicakup ke paket Plugin. Kategori terkontrol dan alias filter v1
...
`trending` adalah papan peringkat instal/unduhan tujuh hari dan tidak menggunakan total sepanjang waktu.
Pada endpoint terpadu `/api/v1/packages`, ini khusus Plugin; gunakan
...
- Skill yang di-host menyertakan file versi tersimpan terbaru dan tercantum di
  `_manifest.json` dengan `sourceRef: "public-clawhub"`.
- Skill saat ini yang didukung GitHub dengan pemindaian `clean` atau `suspicious` menyertakan
  `_source_handoff.json` dengan `sourceRef: "public-github"`, repo, commit, path,
  hash konten, dan URL arsip. Skill tersebut tidak menyertakan file sumber yang di-host ClawHub.
...
- `_manifest.json` selalu disertakan di root ZIP.
- `_errors.json` disertakan saat plugin atau file individual tidak dapat
  diekspor.
...
- Alias filter v1 lama yang didokumentasikan di bawah `GET /api/v1/plugins` juga
  diterima.
- Pemfilteran kategori adalah filter API nyata yang didukung oleh baris ringkasan
...
- `version.sha256hash` adalah metadata kompatibilitas yang tidak digunakan lagi untuk klien lama. Kolom ini
...
- `version.vtAnalysis`, `version.llmAnalysis`, dan `version.staticScan`
  disertakan saat data pemindaian ada.
- Paket privat mengembalikan `404` kecuali pemanggil dapat membaca penerbit pemiliknya.
...
- `trust.moderationState` dapat bernilai null. Nilainya `null` saat tidak ada moderasi rilis
  manual.
...
- `trust.stale` berarti ringkasan kepercayaan dihitung dari input yang sudah kedaluwarsa dan
  sebaiknya diperlakukan sebagai perlu disegarkan sebelum keputusan izinkan dengan keyakinan tinggi.

Catatan:

- Endpoint ini spesifik versi. Klien sebaiknya memanggilnya setelah meresolusikan
...
- Ini adalah permukaan resolver OpenClaw; permukaan ini menghindari penebakan format arsip dari
  URL bersama.
...
- Versi ClawPack melakukan stream byte `.tgz` `npm-pack` yang diunggah persis.
...
- ringkasan artefak
- repositori sumber dan asal commit
...
`note` wajib untuk `confirmed` dan `dismissed`; nilai ini dapat dihilangkan saat
...
- `dist.tarball`, `dist.integrity`, dan `dist.shasum` menggunakan field yang kompatibel dengan npm
  agar pengguna dapat mengarahkan npm ke mirror jika mereka memilih.
- Packument paket berscope mendukung path permintaan `/api/npm/@scope/name` dan
  path permintaan npm yang dikodekan `/api/npm/@scope%2Fname`.

### `GET /api/npm/{package}/-/{tarball}.tgz`

Mengalirkan byte tarball ClawPack persis yang diunggah untuk klien mirror npm.
...
- `slug` (wajib)
- `hash` (wajib): sha256 heksadesimal 64 karakter dari sidik jari bundle
...
Mengunduh ZIP versi Skills yang dihosting, atau mengembalikan serah terima sumber GitHub untuk
Skills saat ini yang didukung GitHub dengan pemindaian `clean` atau `suspicious` dan tanpa
versi yang dihosting.
...
- Serah terima Skills yang didukung GitHub tidak mem-proxy atau me-mirror byte. Respons JSON
...
- Field payload opsional: `migrateOwner`. Saat `true` dengan `ownerHandle`,
  Skills yang sudah ada dapat dipindahkan ke pemilik tersebut jika aktor adalah admin/pemilik pada
...
- Field formulir yang diizinkan adalah `payload`, blob `files` berulang, atau satu referensi tarball
  ...
  `clawpackUploadTicket` yang dikembalikan bersama URL unggah tersebut.
...
- Hanya penerbit org `openclaw` dan penerbit personal milik anggota org `openclaw` saat ini
  yang dapat menerbitkan ke kanal `official`.
- Publikasi atas nama pihak lain tetap memvalidasi kelayakan kanal resmi terhadap akun pemilik target.
...
Khusus admin. Memastikan penerbit org ada untuk suatu handle. Jika handle masih mengarah ke
penerbit pengguna/personal bersama lama, endpoint terlebih dahulu memigrasikannya menjadi penerbit org.
...
Khusus admin. Memulihkan penerbit personal untuk prinsipal pengganti GitHub OAuth yang terverifikasi
tanpa mengedit baris akun Convex Auth. Permintaan harus menyebutkan kedua id akun provider GitHub
yang immutable; handle yang mutable hanya digunakan sebagai pengaman yang terlihat operator.

Endpoint secara default berjalan sebagai dry-run. Menerapkan pemulihan memerlukan `dryRun: false` dan
`confirmIdentityVerified: true` setelah staf memverifikasi secara independen kesinambungan antara kedua
prinsipal GitHub. Pemulihan gagal tertutup saat penerbit personal pengguna tujuan saat ini
memiliki skill, paket, atau sumber skill GitHub.
Pemulihan juga memigrasikan field legacy `ownerUserId` untuk skill milik penerbit yang dipulihkan,
...
Sumber skill GitHub memiliki cakupan penerbit dan dilaporkan sebagai sudah diperiksa, bukan ditulis ulang.
...
Banned pengguna dan hapus permanen skill yang dimiliki (hanya moderator/admin).
...
Cabut ban pengguna dan pulihkan skill yang memenuhi syarat (hanya admin).
...
konten (hanya admin). Secara default berjalan sebagai dry-run kecuali `dryRun` adalah `false`.
...
`clawpack` dan ticket yang dikembalikan sebagai `clawpackUploadTicket`.
