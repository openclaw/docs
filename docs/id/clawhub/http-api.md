...
- Endpoint tulis terautentikasi sebaiknya tidak mengembalikan `Unauthorized` polos ketika
  ...
  akun yang dihapus/diblokir/dinonaktifkan masing-masing sebaiknya mendapatkan teks yang dapat ditindaklanjuti agar klien CLI
...
- Jika tidak ada IP klien tepercaya yang tersedia, permintaan unduhan anonim menggunakan bucket fallback berskala endpoint, bukan satu bucket global `ip:unknown`. Permintaan baca/tulis anonim tetap menggunakan bucket unknown bersama agar perutean IP yang hilang tetap terlihat dan konservatif.
...
- Unduhan digunakan sebagai prior berskala log kecil dan pemecah seri, bukan sebagai sinyal peringkat utama. Skill dengan unduhan tinggi dapat berperingkat lebih rendah ketika teks kueri merupakan kecocokan yang lebih lemah.
...
- Masukkan istilah yang secara literal akan dicari pengguna dalam nama tampilan, ringkasan, dan tag. Gunakan token slug mandiri hanya ketika token itu juga merupakan identitas stabil yang ingin Anda pertahankan.
...
- Jika sebuah skill tiba-tiba tidak terlihat, periksa status moderasi terlebih dahulu dengan `clawhub inspect <slug>` saat masuk sebelum mengubah metadata terkait peringkat.
...
- Menyertakan status verifikasi yang dinormalisasi plus detail spesifik pemindai.
...
- Ketika mengueri versi historis, periksa `moderation.matchesRequestedVersion` dan `moderation.sourceVersion` sebelum memperlakukan `moderation` dan `security` sebagai konteks versi yang sama.
...
- `npmMirror` (opsional): `true`/`1` untuk menampilkan versi paket yang didukung ClawPack
  yang tersedia melalui cermin npm
...
Endpoint pemeliharaan khusus admin untuk melabeli rilis paket lama dengan
...
- Default ke dry-run.
- Rilis tanpa penyimpanan ClawPack dilabeli `legacy-zip`.
- Baris yang didukung ClawPack yang sudah ada tetapi tidak memiliki `artifactKind` diperbaiki sebagai
  `npm-pack`.
...
- Hanya penerbit tepercaya yang boleh menerbitkan ke channel `official`.
...
dapat kemudian menerbitkan rilis code-plugin atau bundle-plugin asli ke nama tersebut.
...
Jika Anda self-host, sajikan file ini (atau atur `CLAWHUB_REGISTRY` secara eksplisit; `CLAWDHUB_REGISTRY` lama).
