...
Legacy `/api/...` dan `/api/cli/...` tetap tersedia untuk kompatibilitas (lihat `DEPRECATIONS.md`).
...
- Popularitas diskalakan secara log dan dibatasi. Skills dengan engagement tinggi dapat berperingkat lebih rendah ketika teks kueri merupakan kecocokan yang lebih lemah.
...
- Alias penggantian nama mempertahankan resolusi untuk URL lama dan instalasi yang diselesaikan melalui registry, tetapi peringkat pencarian didasarkan pada metadata skill kanonis setelah penggantian nama terindeks. Statistik yang ada tetap bersama skill tersebut.
...
- Ketika `nonSuspiciousOnly=true`, sort berbasis kursor dapat mengembalikan lebih sedikit dari `limit` item pada suatu halaman karena skills mencurigakan difilter setelah pengambilan halaman.
...
- `metadata.os`: pembatasan OS yang dideklarasikan di frontmatter skill (misalnya `["macos"]`, `["linux"]`). `null` jika tidak dideklarasikan.
...
mengatur `status` kembali ke `open`. Teruskan `finalAction: "hide"` dengan laporan yang sudah di-triage
...
- `security.hasScanResult` bernilai `true` hanya saat pemindai menghasilkan verdict definitif (`clean`, `suspicious`, atau `malicious`).
...
Endpoint submit terautentikasi untuk pekerjaan ClawScan baru.
...
- Pemindaian yang dipublikasikan hanya menulis balik saat `update: true` dan pemindaian berhasil selesai.
...
- Pekerjaan pemindaian bersifat asinkron. Permintaan pemindaian manual diprioritaskan sebelum pekerjaan publikasi/backfill normal, tetapi penyelesaiannya tetap bergantung pada ketersediaan worker.
...
- Mengembalikan `queue.queuedAhead` dan `queue.position` saat berada dalam antrean agar klien dapat menampilkan berapa banyak pemindaian manual prioritas yang berada di depan permintaan tersebut. Antrean yang sangat besar dibatasi dan dilaporkan dengan `queuedAheadIsEstimate: true`.
...
- `ok` bernilai `true` hanya saat versi yang dipilih memiliki Skill Card yang dihasilkan, tidak diblokir sebagai malware oleh moderasi, dan verifikasi ClawScan bersih.
- Identitas skill, identitas penerbit, dan metadata versi terpilih adalah field envelope tingkat atas (`slug`, `displayName`, `publisherHandle`, `version`, `resolvedFrom`, `tag`, `createdAt`) sehingga otomatisasi shell dapat membacanya tanpa membongkar wrapper bertingkat.
- `security` adalah verdict ClawScan/keamanan tingkat atas. Otomatisasi sebaiknya menggunakan `ok`, `decision`, `reasons`, dan `security.status` sebagai acuan.
...
- `provenance` bernilai `server-resolved-github-import` hanya saat ClawHub menyelesaikan dan menyimpan repo/ref/commit/path GitHub selama publikasi atau impor; jika tidak, nilainya `unavailable`.

### `POST /api/v1/skills/-/security-verdicts`

Mengembalikan verdict keamanan ringkas saat ini untuk versi skill yang persis. Endpoint
koleksi ini ditujukan untuk klien yang sudah mengetahui versi skill ClawHub terpasang
...
- `security.signals.dependencyRegistry` dipertahankan untuk kompatibilitas respons v1, tetapi pemindai keberadaan registry dependensi sudah dihentikan dan kunci ini selalu `null`.
- Ketiadaan Skill Card tidak memengaruhi `ok`, `decision`, atau `reasons` endpoint ini; klien sebaiknya membaca `skill-card.md` terpasang secara lokal saat memerlukan konten kartu.
...
- Skill yang dihosting menyertakan file versi tersimpan terbaru dan dicantumkan dalam
...
- Rilis ClawPack menyertakan kolom yang kompatibel dengan npm `npmIntegrity`, `npmShasum`, dan
  `npmTarballName`.
...
- `trust.blockedFromDownload` adalah sinyal pemblokiran instalasi. OpenClaw dan klien
  instalasi lain sebaiknya memblokir instalasi ketika nilai ini adalah `true`, bukan
...
- Endpoint ini sengaja lebih sempit daripada endpoint moderasi pemilik/moderator.
  Ini mengekspos keputusan instalasi dan penjelasan publik, bukan identitas pelapor,
  isi laporan, bukti privat, atau lini waktu tinjauan internal.
...
skill saat ini yang didukung GitHub dengan pemindaian `clean` atau `suspicious` dan tanpa
...
- Kolom payload opsional: `ownerHandle`. Jika ada, API menyelesaikan penerbit tersebut
  di sisi server dan mewajibkan aktor memiliki akses penerbit.
- Kolom payload opsional: `migrateOwner`. Saat `true` dengan `ownerHandle`, skill
...
- Kolom formulir yang diizinkan adalah `payload`, blob `files` berulang, atau satu referensi tarball
...
pengguna bersama lama/penerbit personal, endpoint memigrasikannya ke penerbit org terlebih dahulu.
...
Endpoint ini defaultnya dry-run. Menerapkan pemulihan memerlukan `dryRun: false` dan
`confirmIdentityVerified: true` setelah staf memverifikasi secara independen kesinambungan antara kedua
principal GitHub. Pemulihan gagal secara tertutup ketika publisher pribadi pengguna tujuan saat ini
memiliki skills, packages, atau sumber skill GitHub.
Pemulihan juga memigrasikan bidang `ownerUserId` lama untuk skills milik publisher yang dipulihkan,
...
konten (hanya admin). Defaultnya dry-run kecuali `dryRun` adalah `false`.
