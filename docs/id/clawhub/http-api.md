...
Pintasan slug web diselesaikan di seluruh keluarga registry, tetapi klien API harus menggunakan
...
- Endpoint tulis terautentikasi tidak boleh mengembalikan `Unauthorized` begitu saja ketika
  server mengetahui alasannya. Token yang hilang, token tidak valid/dicabut, dan
  akun yang dihapus/diblokir/dinonaktifkan masing-masing harus mendapat teks yang dapat ditindaklanjuti sehingga klien
...
- Letakkan istilah yang benar-benar akan dicari pengguna dalam nama tampilan, ringkasan, dan tag. Gunakan token slug mandiri hanya ketika token itu juga merupakan identitas stabil yang ingin Anda pertahankan.
- Jangan mengganti nama slug hanya untuk mengejar satu kueri kecuali slug baru adalah nama kanonis jangka panjang yang lebih baik. Slug lama menjadi alias pengalihan, tetapi URL kanonis, slug yang ditampilkan, dan ringkasan pencarian masa depan menggunakan slug baru.
- Alias penggantian nama mempertahankan resolusi untuk URL lama dan instalasi yang diselesaikan melalui registry, tetapi peringkat pencarian didasarkan pada metadata Skill kanonis setelah penggantian nama diindeks. Statistik yang ada tetap bersama Skill tersebut.
- Jika suatu Skill tiba-tiba tidak terlihat, periksa status moderasi terlebih dahulu dengan `clawhub inspect @owner/slug` saat sudah masuk sebelum mengubah metadata terkait peringkat.
...
- `createdAt` stabil untuk crawl Skill baru; `updated` berubah ketika Skills yang sudah ada dipublikasikan ulang.
- Ketika `nonSuspiciousOnly=true`, pengurutan berbasis kursor dapat mengembalikan item lebih sedikit dari `limit` pada sebuah halaman karena Skills yang mencurigakan difilter setelah pengambilan halaman.
...
- Pemanggil publik hanya mendapat `200` untuk Skills terlihat yang sudah ditandai.
...
mengatur `status` kembali ke `open`. Berikan `finalAction: "hide"` dengan laporan yang telah ditriase
...
- `security.hasScanResult` adalah `true` hanya ketika pemindai menghasilkan vonis definitif (`clean`, `suspicious`, atau `malicious`).
...
- Pemindaian terpublikasi memerlukan akses pengelolaan pemilik/penerbit, atau otoritas moderator/admin platform.
...
- Pekerjaan pemindaian bersifat asinkron. Permintaan pemindaian manual diprioritaskan sebelum pekerjaan publikasi/backfill normal, tetapi penyelesaian tetap bergantung pada ketersediaan worker.
...
- Mengembalikan `queue.queuedAhead` dan `queue.position` saat dalam antrean agar klien dapat menampilkan berapa banyak pemindaian manual prioritas yang berada di depan permintaan. Antrean yang sangat besar dibatasi dan dilaporkan dengan `queuedAheadIsEstimate: true`.
...
- `ok` adalah `true` hanya ketika versi yang dipilih memiliki Skill Card yang dihasilkan, tidak diblokir sebagai malware oleh moderasi, dan verifikasi ClawScan bersih.
- Identitas skill, identitas penerbit, dan metadata versi yang dipilih adalah field envelope tingkat atas (`slug`, `displayName`, `publisherHandle`, `version`, `resolvedFrom`, `tag`, `createdAt`) agar otomatisasi shell dapat membacanya tanpa membongkar wrapper bersarang.
- `security` adalah vonis ClawScan/keamanan tingkat atas. Otomatisasi sebaiknya berpatokan pada `ok`, `decision`, `reasons`, dan `security.status`.
...
Mengembalikan vonis keamanan ringkas saat ini untuk versi skill yang persis. Endpoint
...
- Gunakan `/verify` saat Anda membutuhkan envelope verifikasi Skill Card untuk satu skill, `/card` saat Anda membutuhkan markdown kartu yang dihasilkan, dan `/scan` saat Anda membutuhkan data pemindai terperinci.
...
- `sort` (opsional): `updated` (default), `recommended`, `trending`, `downloads`, alias legacy `installs`
- `category` (opsional): filter kategori plugin. Didukung hanya ketika
  ...
  alias filter v1 legacy didokumentasikan di bawah `GET /api/v1/plugins`.
...
- `category` (opsional): filter kategori plugin. Didukung hanya ketika
  ...
  legacy didokumentasikan di bawah `GET /api/v1/plugins`.
...
- `cursor` (opsional): kursor paginasi
- `isOfficial` (opsional): `true` atau `false`
- `sort` (opsional): `recommended` (default), `trending`, `downloads`, `updated`, alias legacy `installs`
- `category` (opsional): filter kategori plugin. Nilai saat ini:
  `channels`, `models`, `memory`, `context`, `voice`, `media`, `web`,
  `tools`, `runtime`, `gateway`, `security`, `other`.

Alias filter v1 legacy tetap diterima pada endpoint baca:

- `mcp-tooling`, `data`, dan `automation` diselesaikan ke `tools`.
- `observability` dan `deployment` diselesaikan ke `gateway`.
- `dev-tools` diselesaikan ke `runtime`.
...
Alias legacy tidak diterima sebagai nilai kategori yang disimpan atau dideklarasikan penulis.
...
- Skills yang di-host menyertakan file versi tersimpan terbaru dan tercantum di
...
- `_manifest.json` selalu disertakan di root ZIP.
- `_errors.json` disertakan ketika masing-masing Plugin atau file tidak dapat
  diekspor.
...
- Hasil dikembalikan dalam urutan relevansi dan saat ini tidak memakai paginasi.
- Kontrol urut UI peramban untuk pencarian Plugin mengurutkan ulang hasil relevansi yang dimuat,
...
instalasi. Ini adalah permukaan konsumsi publik OpenClaw untuk memutuskan apakah sebuah
...
- `trust.stale` berarti ringkasan kepercayaan dihitung dari masukan yang kedaluwarsa dan
  sebaiknya diperlakukan sebagai perlu disegarkan sebelum keputusan izinkan dengan keyakinan tinggi.

Catatan:

- Endpoint ini presisi versi. Klien sebaiknya memanggilnya setelah me-resolve
...
- Versi ClawPack melakukan stream byte `.tgz` `npm-pack` persis yang diunggah.
...
Melaporkan paket untuk tinjauan moderator. Laporan berada di tingkat paket, secara opsional
ditautkan ke versi. Laporan masuk ke antrean moderasi tetapi tidak otomatis menyembunyikan atau
...
- `dist.tarball`, `dist.integrity`, dan `dist.shasum` menggunakan field yang kompatibel dengan npm
  sehingga pengguna dapat mengarahkan npm ke mirror jika mereka memilihnya.
- Packument paket berscope mendukung path permintaan `/api/npm/@scope/name` dan
  path permintaan npm yang dienkode `/api/npm/@scope%2Fname`.

### `GET /api/npm/{package}/-/{tarball}.tgz`

Melakukan streaming byte tarball ClawPack persis seperti yang diunggah untuk klien mirror npm.
...
skill saat ini yang didukung GitHub dengan pemindaian `clean` atau `suspicious` dan tanpa versi
yang dihosting.
...
- Handoff skill yang didukung GitHub tidak mem-proxy atau me-mirror byte. Respons JSON
...
## Endpoint autentikasi (Bearer token)
...
- Field payload opsional: `migrateOwner`. Jika `true` dengan `ownerHandle`,
  skill yang ada dapat dipindahkan ke pemilik itu jika aktor adalah admin/pemilik pada
...
- Body JSON dan metadata `payload.files` / `payload.artifact` yang diberikan pemanggil
  ditolak.
...
- Hanya penerbit org `openclaw` dan penerbit personal milik anggota org `openclaw`
  saat ini yang boleh menerbitkan ke kanal `official`.
- Publikasi atas nama tetap memvalidasi kelayakan kanal resmi terhadap akun pemilik target.
...
penerbit lain. Respons hapus menyertakan `slugReservedUntil` saat masa kedaluwarsa ini berlaku.
...
penerbit pengguna/personal bersama lama, endpoint memigrasikannya ke penerbit org terlebih dahulu.
...
Khusus admin. Memulihkan penerbit personal untuk principal OAuth GitHub pengganti yang terverifikasi
tanpa mengedit baris akun Convex Auth. Permintaan harus menyebutkan kedua id akun provider GitHub
yang tidak dapat diubah; handle yang dapat berubah hanya digunakan sebagai penjaga yang menghadap operator.

Endpoint secara default menggunakan dry-run. Menerapkan pemulihan memerlukan `dryRun: false` dan
`confirmIdentityVerified: true` setelah staf memverifikasi secara independen kesinambungan antara kedua
prinsipal GitHub. Pemulihan gagal tertutup ketika publisher pribadi saat ini milik pengguna tujuan
memiliki skills, paket, atau sumber skill GitHub.
Pemulihan juga memigrasikan bidang `ownerUserId` legacy untuk skills milik publisher yang dipulihkan,
alias slug skill, paket, peringatan pemeriksa paket, dan baris digest pencarian turunan sehingga
jalur pemilik langsung selaras dengan otoritas publisher baru. Reservasi handle terlindungi yang aktif
untuk handle yang dipulihkan juga dialihkan ke pengguna pengganti sehingga sinkronisasi profil berikutnya
tidak dapat memulihkan otoritas pesaing milik pengguna lama. Setiap tabel utama dibatasi hingga
...
Sumber skill GitHub bersifat tercakup ke publisher dan dilaporkan sebagai diperiksa, bukan ditulis ulang.
...
Ubah alasan tersimpan untuk pemblokiran yang ada tanpa mencabut blokir atau memulihkan
konten (hanya admin). Secara default menggunakan dry-run kecuali `dryRun` adalah `false`.
...
Tambah/hapus bintang (sorotan). Kedua endpoint bersifat idempoten.
