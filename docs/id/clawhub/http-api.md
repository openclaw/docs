...
x-i18n:
    generated_at: "2026-07-03T17:41:59Z"
    model: gpt-5.5
...
- Popularitas diskalakan secara log dan dibatasi. Skills dengan engagement tinggi dapat berperingkat lebih rendah ketika teks kueri adalah kecocokan yang lebih lemah.
...
- Masukkan istilah yang benar-benar akan dicari pengguna ke dalam nama tampilan, ringkasan, dan tag. Gunakan token slug mandiri hanya ketika itu juga merupakan identitas stabil yang ingin Anda pertahankan.
- Jangan mengganti nama slug hanya untuk mengejar satu kueri kecuali slug baru tersebut adalah nama kanonis jangka panjang yang lebih baik. Slug lama menjadi alias pengalihan, tetapi URL kanonis, slug yang ditampilkan, dan ringkasan pencarian mendatang menggunakan slug baru.
...
- Jika Skill tiba-tiba tidak terlihat, periksa status moderasi terlebih dahulu dengan `clawhub inspect @owner/slug` saat sudah login sebelum mengubah metadata terkait peringkat.
...
- Mengembalikan `queue.queuedAhead` dan `queue.position` saat dalam antrean agar klien dapat menampilkan berapa banyak pemindaian manual berprioritas yang berada di depan permintaan. Antrean yang sangat besar dibatasi dan dilaporkan dengan `queuedAheadIsEstimate: true`.
...
- Identitas skill, identitas penerbit, dan metadata versi yang dipilih adalah bidang amplop tingkat atas (`slug`, `displayName`, `publisherHandle`, `version`, `resolvedFrom`, `tag`, `createdAt`) sehingga otomatisasi shell dapat membacanya tanpa membongkar wrapper bersarang.
...
- Respons hanya untuk keamanan. Respons tidak menyertakan data Skill Card, status kartu yang dihasilkan, daftar file artefak, atau payload pemindai terperinci.
...
- `security.signals.dependencyRegistry` dipertahankan untuk kompatibilitas respons v1, tetapi pemindai keberadaan registri dependensi sudah dihentikan dan kunci ini selalu `null`.
- Tidak adanya Skill Card tidak memengaruhi `ok`, `decision`, atau `reasons` endpoint ini; klien sebaiknya membaca `skill-card.md` terpasang secara lokal saat membutuhkan konten kartu.
...
- Skills saat ini yang didukung GitHub dengan pemindaian `clean` atau `suspicious` menyertakan
  ...
  hash konten, dan URL arsip. File tersebut tidak menyertakan file sumber yang dihosting ClawHub.
...
- `version.sha256hash` adalah metadata kompatibilitas yang sudah usang untuk klien lama. Ini
...
- `release.artifactKind`, `release.artifactSha256`, `release.npmIntegrity`,
  ...
  artefak rilis.
- `trust.scanStatus` adalah status kepercayaan efektif yang berasal dari input pemindai
...
- `trust.stale` berarti ringkasan kepercayaan dihitung dari input yang sudah usang dan
  sebaiknya diperlakukan sebagai memerlukan penyegaran sebelum keputusan izinkan dengan kepercayaan tinggi.

Catatan:

- Endpoint ini presisi terhadap versi. Klien sebaiknya memanggilnya setelah me-resolve
...
- digest artefak
- repo sumber dan provenance commit
...
- Versi lama khusus ZIP sengaja dihilangkan.
...
- Packument paket scoped mendukung jalur permintaan `/api/npm/@scope/name` dan
  jalur permintaan terenkode npm `/api/npm/@scope%2Fname`.
...
- Serah-terima skill berbasis GitHub tidak mem-proxy atau mencerminkan byte. Respons JSON
...
pengguna bersama/penerbit personal lama, endpoint memigrasikannya menjadi penerbit org terlebih dahulu.
...
yang tidak dapat diubah; handle yang dapat berubah hanya digunakan sebagai penjaga yang terlihat oleh operator.
...
Pemulihan juga memigrasikan kolom lama `ownerUserId` untuk keterampilan milik penerbit yang dipulihkan,
alias slug keterampilan, paket, peringatan pemeriksa paket, dan baris digest pencarian turunan sehingga
...
Sumber keterampilan GitHub bersifat tercakup penerbit dan dilaporkan sebagai diperiksa, bukan ditulis ulang.
...
- Kedua titik akhir memerlukan autentikasi token API dan hanya berfungsi untuk pemilik keterampilan.
...
Blokir pengguna dan hapus permanen keterampilan yang dimiliki (hanya moderator/admin).
...
Buka blokir pengguna dan pulihkan keterampilan yang memenuhi syarat (hanya admin).
