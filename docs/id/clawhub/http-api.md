...
x-i18n:
    generated_at: "2026-07-04T15:34:50Z"
    model: gpt-5.5
...
Pintasan slug web diselesaikan di seluruh keluarga registri, tetapi klien API harus menggunakan
...
- Endpoint tulis terautentikasi tidak boleh mengembalikan `Unauthorized` polos ketika
  ...
  akun yang dihapus/dilarang/dinonaktifkan masing-masing harus mendapat teks yang dapat ditindaklanjuti agar klien
...
- `X-RateLimit-Remaining` / `RateLimit-Remaining`: anggaran tersisa persis saat ada.
  Permintaan sukses yang disharding menghilangkan header ini alih-alih mengembalikan nilai global perkiraan.
...
- Hasil dikembalikan dalam urutan relevansi (kemiripan embedding + peningkatan token slug/nama persis + prioritas popularitas kecil).
...
- Teks ASCII ditokenisasi pada batas kata dan tanda baca. Misalnya, `personal-map` berisi token `map` mandiri, sedangkan `amap-jsapi-skill` berisi `amap`, `jsapi`, dan `skill`; mencari `map` karena itu memberi `personal-map` kecocokan leksikal yang lebih kuat daripada `amap-jsapi-skill`.
...
- Alias penggantian nama mempertahankan resolusi untuk URL lama dan instalasi yang diselesaikan melalui registri, tetapi peringkat pencarian didasarkan pada metadata skill kanonis setelah penggantian nama telah diindeks. Statistik yang ada tetap bersama skill.
- Jika skill secara tak terduga tidak terlihat, periksa status moderasi terlebih dahulu dengan `clawhub inspect @owner/slug` saat login sebelum mengubah metadata terkait peringkat.
...
- Ketika `nonSuspiciousOnly=true`, sort berbasis kursor dapat mengembalikan item lebih sedikit daripada `limit` pada satu halaman karena skills mencurigakan difilter setelah pengambilan halaman.
...
- Mengembalikan `queue.queuedAhead` dan `queue.position` saat diantrekan sehingga klien dapat menunjukkan berapa banyak pemindaian manual yang diprioritaskan berada di depan permintaan. Antrean yang sangat besar dibatasi dan dilaporkan dengan `queuedAheadIsEstimate: true`.
...
- Identitas skill, identitas penerbit, dan metadata versi yang dipilih adalah field envelope tingkat atas (`slug`, `displayName`, `publisherHandle`, `version`, `resolvedFrom`, `tag`, `createdAt`) sehingga otomatisasi shell dapat membacanya tanpa membongkar wrapper bertingkat.
- `security` adalah vonis ClawScan/keamanan tingkat atas. Otomatisasi sebaiknya mengacu pada `ok`, `decision`, `reasons`, dan `security.status`.
...
- `security.signals.dependencyRegistry` dipertahankan untuk kompatibilitas respons v1, tetapi pemindai keberadaan registri dependensi sudah dipensiunkan dan kunci ini selalu `null`.
- Ketiadaan Skill Card tidak memengaruhi `ok`, `decision`, atau `reasons` endpoint ini; klien sebaiknya membaca `skill-card.md` yang terinstal secara lokal saat mereka membutuhkan konten kartu.
...
- Skill yang di-host menyertakan file versi tersimpan terbaru dan dicantumkan dalam
...
- `_errors.json` disertakan ketika masing-masing skill atau file tidak dapat
  diekspor.
...
- `_manifest.json` selalu disertakan di root ZIP.
- `_errors.json` disertakan ketika masing-masing Plugin atau berkas tidak dapat
  diekspor.
...
- `release.artifactKind`, `release.artifactSha256`, `release.npmIntegrity`,
  `release.npmShasum`, dan `release.npmTarballName` tersedia saat diketahui untuk
  artefak rilis.
...
- `trust.stale` berarti ringkasan kepercayaan dihitung dari input yang sudah usang dan
  sebaiknya diperlakukan sebagai perlu penyegaran sebelum keputusan izin berkeyakinan tinggi.

Catatan:

- Endpoint ini persis per versi. Klien sebaiknya memanggilnya setelah me-resolve
  versi paket yang ingin mereka instal, bukan hanya setelah membaca metadata paket
  terbaru.
- Paket privat mengembalikan `404` kecuali pemanggil dapat membaca penerbit pemilik.
...
- Versi paket lama mengembalikan artefak `legacy-zip` dan `downloadUrl` ZIP
  lama.
- Versi ClawPack mengembalikan artefak `npm-pack`, bidang integritas npm, sebuah
...
- `packageName` dinormalisasi sebagai nama npm; paket boleh belum ada untuk migrasi
  yang direncanakan.
...
ditautkan ke sebuah versi. Laporan mengalir ke antrean moderasi tetapi tidak otomatis menyembunyikan atau
...
- Default-nya adalah rilis terbaru.
...
Mengunduh ZIP versi Skills yang di-host, atau mengembalikan handoff sumber GitHub untuk
Skills saat ini yang berbasis GitHub dengan pemindaian `clean` atau `suspicious` dan tanpa versi
yang di-host.
...
- Handoff Skills berbasis GitHub tidak mem-proxy atau me-mirror byte. Respons JSON
...
- Field payload opsional: `ownerHandle`. Saat ada, API menyelesaikan penerbit tersebut
...
Menghapus secara lunak / memulihkan Skills (pemilik, moderator, atau admin).
...
pengguna bersama lama/penerbit personal, titik akhir memigrasikannya terlebih dahulu ke penerbit org.
...
Pembuatan penerbit org secara mandiri dengan autentikasi. Membuat penerbit org baru dan menambahkan
pemanggil sebagai pemilik. Titik akhir ini tidak memigrasikan handle pengguna/personal yang sudah ada dan
...
dapat menerbitkan rilis code-plugin atau bundle-plugin yang sebenarnya ke nama tersebut di kemudian hari.
...
Khusus admin. Memulihkan penerbit personal untuk prinsipal OAuth GitHub pengganti yang terverifikasi
...
jalur pemilik langsung selaras dengan otoritas penerbit baru. Reservasi protected-handle yang aktif
