...
- Endpoint tulis terautentikasi sebaiknya tidak mengembalikan `Unauthorized` polos ketika
  ...
  akun yang dihapus/diblokir/dinonaktifkan masing-masing sebaiknya mendapat teks yang dapat ditindaklanjuti agar klien
...
sebaiknya membaca isi respons sebagai string yang dapat dibaca manusia. Parameter kueri yang tidak diketahui
...
Panduan kemudahan ditemukan untuk penerbit:
...
- Jangan mengganti nama slug hanya untuk mengejar satu kueri kecuali slug baru tersebut adalah nama kanonis jangka panjang yang lebih baik. Slug lama menjadi alias pengalihan, tetapi URL kanonis, slug yang ditampilkan, dan digest pencarian mendatang menggunakan slug baru.
- Alias rename mempertahankan resolusi untuk URL lama dan pemasangan yang diselesaikan melalui registry, tetapi peringkat pencarian didasarkan pada metadata Skill kanonis setelah rename terindeks. Statistik yang ada tetap bersama Skill tersebut.
- Jika sebuah Skill secara tak terduga tidak terlihat, periksa status moderasi terlebih dahulu dengan `clawhub inspect @owner/slug` saat sudah masuk sebelum mengubah metadata terkait peringkat.
...
- `sort` (opsional): `updated` (default), `recommended` (alias: `default`), `createdAt` (alias: `newest`), `downloads`, `stars` (alias: `rating`), alias install legacy `installsCurrent`/`installs`/`installsAllTime` dipetakan ke `downloads`, `trending`
- `nonSuspiciousOnly` (opsional): `true` untuk menyembunyikan Skills mencurigakan (`flagged.suspicious`)
- `nonSuspicious` (opsional): alias legacy untuk `nonSuspiciousOnly`
...
- `metadata.os`: batasan OS yang dideklarasikan di frontmatter Skill (mis. `["macos"]`, `["linux"]`). `null` jika tidak dideklarasikan.
...
Laporkan sebuah Skill untuk ditinjau moderator. Laporan berada di tingkat Skill, secara opsional ditautkan
...
mengatur `status` kembali ke `open`. Kirim `finalAction: "hide"` dengan laporan
yang sudah di-triage untuk menyembunyikan Skill dalam workflow yang sama dan dapat diaudit.
...
Titik akhir pengiriman terautentikasi untuk pekerjaan ClawScan baru.
...
- Pemindaian terpublikasi memerlukan akses pengelolaan pemilik/penerbit, atau kewenangan moderator/admin platform.
- Pemindaian terpublikasi menulis balik hanya ketika `update: true` dan pemindaian selesai dengan sukses.
...
- Pekerjaan pemindaian bersifat asinkron. Permintaan pemindaian manual diprioritaskan di depan pekerjaan publikasi/pengisian balik normal, tetapi penyelesaiannya tetap bergantung pada ketersediaan pekerja.

### `GET /api/v1/skills/-/scan/{scanId}`

Titik akhir polling terautentikasi untuk pemindaian yang dikirimkan.

- Mengembalikan status antre/berjalan/berhasil/gagal.
- Mengembalikan `queue.queuedAhead` dan `queue.position` saat antre sehingga klien dapat menampilkan berapa banyak pemindaian manual yang diprioritaskan berada di depan permintaan. Antrean yang sangat besar dibatasi dan dilaporkan dengan `queuedAheadIsEstimate: true`.
...
Titik akhir arsip laporan tersimpan terautentikasi untuk versi yang dikirimkan.
...
- Mengembalikan bentuk ZIP yang sama dengan unduhan permintaan pemindaian.

### `POST /api/v1/skills/-/scan/batch`

Rute pemindaian ulang kumpulan kanonis khusus admin. Rute ini menerima bentuk payload yang sama seperti `POST /api/v1/skills/-/rescan-batch` lama.

### `POST /api/v1/skills/-/scan/batch/status`

Rute status kumpulan kanonis khusus admin. Rute ini menerima `{ "jobIds": ["..."] }` dan mengembalikan penghitung agregat yang sama seperti `POST /api/v1/skills/-/rescan-batch/status` lama.
...
- Identitas skill, identitas penerbit, dan metadata versi yang dipilih adalah kolom amplop tingkat atas (`slug`, `displayName`, `publisherHandle`, `version`, `resolvedFrom`, `tag`, `createdAt`) sehingga otomasi shell dapat membacanya tanpa membongkar pembungkus bersarang.
...
Mengembalikan putusan keamanan ringkas saat ini untuk versi skill yang persis. Titik akhir
...
- `security.signals.dependencyRegistry` dipertahankan untuk kompatibilitas respons v1, tetapi pemindai keberadaan registri dependensi sudah dihentikan dan kunci ini selalu `null`.
- Ketiadaan Skill Card tidak memengaruhi `ok`, `decision`, atau `reasons` titik akhir ini; klien harus membaca `skill-card.md` terpasang secara lokal saat membutuhkan isi kartu.
...
Alias lama tidak diterima sebagai nilai kategori tersimpan atau yang dideklarasikan penulis.
...
instal. Ini adalah permukaan konsumsi publik OpenClaw untuk memutuskan apakah
...
- `trust.moderationState` dapat bernilai null. Nilainya `null` ketika tidak ada moderasi rilis
  manual.
- `trust.blockedFromDownload` adalah sinyal blokir instal. OpenClaw dan klien
...
- Endpoint ini persis versi. Klien sebaiknya memanggilnya setelah me-resolve versi
  paket yang ingin diinstal, bukan hanya setelah membaca metadata paket terbaru.
- Paket privat mengembalikan `404` kecuali pemanggil dapat membaca penerbit pemilik.
- Endpoint ini sengaja lebih sempit daripada endpoint moderasi pemilik/moderator.
  Endpoint ini mengekspos keputusan instal dan penjelasan publik, bukan
...
- `bundledPluginId` dinormalisasi ke huruf kecil dan merupakan kunci upsert yang stabil.
...
Laporkan paket untuk tinjauan moderator. Laporan berada pada level paket, secara opsional
...
- Field payload opsional: `ownerHandle`. Jika ada, hanya admin yang dapat memublikasikan atas nama pemilik tersebut.
...
- Hanya penerbit org `openclaw` dan penerbit pribadi milik anggota org `openclaw` saat ini
  yang dapat memublikasikan ke channel `official`.
...
pemanggil sebagai pemilik. Endpoint ini tidak memigrasikan handle pengguna/pribadi yang ada dan
...
Khusus admin. Mencadangkan slug root dan nama paket untuk pemilik yang berhak tanpa memublikasikan
rilis. Nama paket menjadi paket placeholder privat tanpa baris rilis, sehingga pemilik yang sama
nanti dapat memublikasikan rilis code-plugin atau bundle-plugin nyata ke nama tersebut.
...
Khusus admin. Memulihkan penerbit pribadi untuk prinsipal GitHub OAuth pengganti yang terverifikasi
tanpa mengedit baris akun Convex Auth. Permintaan harus menyebutkan kedua id akun provider GitHub
yang tidak dapat diubah; handle yang dapat berubah hanya digunakan sebagai pengaman yang terlihat operator.

Endpoint secara default menjalankan simulasi. Menerapkan pemulihan memerlukan `dryRun: false` dan
`confirmIdentityVerified: true` setelah staf memverifikasi secara independen kesinambungan antara kedua
prinsipal GitHub. Pemulihan gagal secara aman ketika publisher personal pengguna tujuan saat ini
...
alias slug skill, packages, peringatan pemeriksa package, dan baris digest pencarian turunan sehingga
jalur pemilik langsung selaras dengan otoritas publisher yang baru. Reservasi protected-handle yang aktif
untuk handle yang dipulihkan juga dialihkan ke pengguna pengganti sehingga sinkronisasi profil berikutnya
tidak dapat memulihkan otoritas pengguna sebelumnya yang bersaing. Setiap tabel primer dibatasi hingga
...
Sumber skill GitHub bersifat scoped ke publisher dan dilaporkan sebagai diperiksa, bukan ditulis ulang.
...
Batalkan ban pengguna dan pulihkan skill yang memenuhi syarat (hanya admin).
...
Ubah alasan tersimpan untuk ban yang sudah ada tanpa membatalkan ban atau memulihkan
konten (hanya admin). Secara default menjalankan simulasi kecuali `dryRun` adalah `false`.
...
Jika Anda self-host, sajikan file ini (atau tetapkan `CLAWHUB_REGISTRY` secara eksplisit; `CLAWDHUB_REGISTRY` lama).
