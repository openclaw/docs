...
x-i18n:
    generated_at: "2026-07-05T08:28:19Z"
    model: gpt-5.5
...
- Popularitas diskalakan log dan dibatasi. Skills dengan engagement tinggi dapat mendapat peringkat lebih rendah ketika teks kueri memiliki kecocokan yang lebih lemah.
...
- Letakkan istilah yang akan dicari pengguna secara harfiah di nama tampilan, ringkasan, dan tag. Gunakan token slug mandiri hanya ketika token tersebut juga merupakan identitas stabil yang ingin Anda pertahankan.
...
- Alias penggantian nama mempertahankan resolusi untuk URL lama dan instalasi yang di-resolve melalui registry, tetapi peringkat pencarian didasarkan pada metadata skill kanonis setelah penggantian nama diindeks. Statistik yang ada tetap bersama skill tersebut.
...
- `sort` (opsional): `updated` (bawaan), `recommended` (alias: `default`), `createdAt` (alias: `newest`), `downloads`, `stars` (alias: `rating`), alias install legacy `installsCurrent`/`installs`/`installsAllTime` dipetakan ke `downloads`, `trending`
- `nonSuspiciousOnly` (opsional): `true` untuk menyembunyikan Skills mencurigakan (`flagged.suspicious`)
- `nonSuspicious` (opsional): alias legacy untuk `nonSuspiciousOnly`
...
- Ketika `nonSuspiciousOnly=true`, pengurutan berbasis kursor dapat mengembalikan item lebih sedikit daripada `limit` pada satu halaman karena Skills mencurigakan difilter setelah pengambilan halaman.
- Gunakan `nextCursor` untuk melanjutkan paginasi jika ada. Halaman pendek saja tidak berarti hasil sudah habis.
...
- Menyertakan status verifikasi yang dinormalkan beserta detail khusus pemindai.
- `security.hasScanResult` bernilai `true` hanya ketika sebuah pemindai menghasilkan vonis definitif (`clean`, `suspicious`, atau `malicious`).
...
Pemindaian terbit menggunakan JSON:
...
- Pemindaian terbit memerlukan akses pengelolaan pemilik/penerbit, atau otoritas moderator/admin platform.
- Pemindaian terbit menulis balik hanya ketika `update: true` dan pemindaian selesai dengan sukses.
...
- Pekerjaan pemindaian bersifat asinkron. Permintaan pemindaian manual diprioritaskan sebelum pekerjaan penerbitan/pengisian balik normal, tetapi penyelesaian tetap bergantung pada ketersediaan worker.
...
- Mengembalikan `queue.queuedAhead` dan `queue.position` saat dalam antrean agar klien dapat menampilkan berapa banyak pemindaian manual yang diprioritaskan berada di depan permintaan. Antrean yang sangat besar dibatasi dan dilaporkan dengan `queuedAheadIsEstimate: true`.
...
- Identitas skill, identitas penerbit, dan metadata versi terpilih adalah field envelope tingkat atas (`slug`, `displayName`, `publisherHandle`, `version`, `resolvedFrom`, `tag`, `createdAt`) sehingga otomasi shell dapat membacanya tanpa membongkar wrapper bersarang.
- `security` adalah vonis ClawScan/keamanan tingkat atas. Otomasi sebaiknya berpatokan pada `ok`, `decision`, `reasons`, dan `security.status`.
...
- `provenance` bernilai `server-resolved-github-import` hanya ketika ClawHub menyelesaikan dan menyimpan repo/ref/commit/path GitHub selama penerbitan atau impor; selain itu nilainya `unavailable`.
...
- `security.signals.dependencyRegistry` dipertahankan untuk kompatibilitas respons v1, tetapi pemindai keberadaan registri dependensi sudah dipensiunkan dan kunci ini selalu `null`.
- Ketiadaan Skill Card tidak memengaruhi `ok`, `decision`, atau `reasons` endpoint ini; klien sebaiknya membaca `skill-card.md` terpasang secara lokal saat membutuhkan konten kartu.
...
- `category` (opsional): filter kategori plugin. Didukung hanya jika
...
- Entri Skill tetap didukung oleh registri skill dan masih dapat dipublikasikan hanya melalui `POST /api/v1/skills`.
...
Melakukan soft-delete pada paket dan semua rilisnya.
...
- `version.sha256hash` adalah metadata kompatibilitas yang usang untuk klien lama. Ini
...
instalasi. Ini adalah permukaan konsumsi OpenClaw publik untuk menentukan apakah
...
- Endpoint ini spesifik versi. Klien sebaiknya memanggilnya setelah me-resolve
...
- Endpoint ini sengaja lebih sempit daripada endpoint moderasi pemilik/moderator.
  ...
  identitas pelapor, isi laporan, bukti privat, atau timeline peninjauan internal.
...
- `all`: rilis apa pun dengan override manual, status pemindaian yang tidak bersih, atau laporan paket.
...
ditautkan ke versi. Laporan tersebut memberi masukan ke antrean moderasi tetapi tidak otomatis menyembunyikan atau
...
- Arsip Plugin/paket adalah berkas zip dengan root `package/` agar klien OpenClaw lama
  tetap berfungsi.
- Rute ini tetap hanya ZIP. Rute ini tidak melakukan stream berkas ClawPack `.tgz`.
...
skill saat ini berbasis GitHub dengan pemindaian `clean` atau `suspicious` dan tanpa versi
yang dihosting.
...
- Bidang payload opsional: `ownerHandle`. Saat ada, API menyelesaikan
...
- Bidang payload opsional: `migrateOwner`. Saat `true` dengan `ownerHandle`,
  skill yang sudah ada dapat dipindahkan ke pemilik tersebut jika aktor adalah admin/pemilik pada
  penerbit saat ini dan penerbit target. Tanpa opt-in ini, perubahan pemilik
  ditolak.
...
penerbit lain. Respons hapus menyertakan `slugReservedUntil` saat masa berlaku ini berlaku.
...
penerbit pengguna/personal bersama legacy, endpoint memigrasikannya ke penerbit org terlebih dahulu.
...
principal GitHub. Pemulihan gagal tertutup ketika penerbit personal pengguna tujuan saat ini
...
path pemilik langsung selaras dengan otoritas penerbit yang baru. Reservasi protected-handle aktif
...
- `rename` mempertahankan slug sebelumnya sebagai alias redirect.
...
Ban pengguna dan hapus permanen skill yang dimiliki (hanya moderator/admin).
...
Ubah alasan yang tersimpan untuk ban yang sudah ada tanpa mencabut ban atau memulihkan
konten (hanya admin). Secara default berjalan sebagai dry-run kecuali `dryRun` bernilai `false`.
...
Tambah/hapus star (sorotan). Kedua endpoint bersifat idempoten.
