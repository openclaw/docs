...
- Endpoint tulis terautentikasi sebaiknya tidak mengembalikan `Unauthorized` saja ketika
...
- Jika tidak ada IP klien tepercaya yang tersedia, permintaan anonim menggunakan bucket fallback
  ...
  jalur yang disuplai pemanggil, slug, nama paket, versi, string kueri, atau parameter
  artefak lainnya.

## Respons error

Respons error v1 publik berupa teks biasa dengan `content-type: text/plain; charset=utf-8`.
...
- `highlightedOnly` (opsional): `true` untuk memfilter hanya ke Skills yang disorot
...
- Letakkan istilah yang benar-benar akan dicari pengguna di nama tampilan, ringkasan, dan tag. Gunakan token slug mandiri hanya ketika itu juga identitas stabil yang ingin Anda pertahankan.
- Jangan mengganti nama slug hanya untuk mengejar satu kueri kecuali slug baru adalah nama kanonis jangka panjang yang lebih baik. Slug lama menjadi alias redirect, tetapi URL kanonis, slug yang ditampilkan, dan ringkasan pencarian mendatang menggunakan slug baru.
- Alias rename mempertahankan resolusi untuk URL lama dan instalasi yang di-resolve melalui registry, tetapi peringkat pencarian didasarkan pada metadata Skill kanonis setelah rename diindeks. Statistik yang ada tetap bersama Skill.
...
- `security.hasScanResult` bernilai `true` hanya ketika scanner menghasilkan vonis definitif (`clean`, `suspicious`, atau `malicious`).
...
- Mengembalikan `queue.queuedAhead` dan `queue.position` saat berada dalam antrean agar klien dapat menampilkan berapa banyak pemindaian manual yang diprioritaskan yang berada di depan permintaan. Antrean yang sangat besar dibatasi dan dilaporkan dengan `queuedAheadIsEstimate: true`.
...
- `security` adalah vonis ClawScan/keamanan tingkat atas. Otomasi sebaiknya mengacu pada `ok`, `decision`, `reasons`, dan `security.status`.
- `security.signals` berisi bukti pendukung scanner seperti `staticScan`, `virusTotal`, dan `skillSpector`.
...
- Respons hanya berisi keamanan. Respons ini tidak menyertakan data Skill Card, status kartu yang dihasilkan, daftar file artefak, atau payload scanner mendetail.
...
- Gunakan `/verify` ketika Anda memerlukan envelope verifikasi Skill Card satu skill, `/card` ketika Anda memerlukan markdown kartu yang dihasilkan, dan `/scan` ketika Anda memerlukan data scanner mendetail.
...
- Skills terhosting mencakup file versi tersimpan terbaru dan dicantumkan dalam
...
- Skills saat ini yang didukung GitHub dengan pemindaian `clean` atau `suspicious` menyertakan
  ...
  hash konten, dan URL arsip. File sumber yang dihosting ClawHub tidak disertakan.
...
- `trust.stale` berarti ringkasan kepercayaan dihitung dari input kedaluwarsa dan
...
- Endpoint ini tepat versi. Klien sebaiknya memanggilnya setelah menyelesaikan
  versi paket yang akan mereka instal, bukan hanya setelah membaca metadata
  paket terbaru.
- Paket privat mengembalikan `404` kecuali pemanggil dapat membaca penerbit pemiliknya.
...
- Versi ClawPack men-stream byte `.tgz` npm-pack persis yang diunggah.
- Versi ZIP lama mengarahkan ulang ke `/api/v1/packages/{name}/download?version=`.
...
ditautkan ke sebuah versi. Laporan ini memberi masukan ke antrean moderasi tetapi tidak secara otomatis menyembunyikan atau
...
- Packument paket scoped mendukung path permintaan `/api/npm/@scope/name` dan
  path permintaan terenkode npm `/api/npm/@scope%2Fname`.
...
- Field formulir yang diizinkan adalah `payload`, blob `files` berulang, atau satu referensi tarball
  ...
  alur upload-url. Publikasi storage-id staged juga harus menyertakan
...
- Publikasi atas nama tetap memvalidasi kelayakan channel resmi terhadap akun pemilik target.
...
Khusus admin. Memulihkan penerbit pribadi untuk prinsipal OAuth GitHub pengganti yang terverifikasi
...
Endpoint secara default berjalan sebagai uji coba. Menerapkan pemulihan memerlukan `dryRun: false` dan
`confirmIdentityVerified: true` setelah staf secara independen memverifikasi kesinambungan antara kedua
prinsipal GitHub. Pemulihan gagal secara tertutup ketika penerbit personal pengguna tujuan saat ini
...
100 baris per transaksi penerapan; pemulihan yang lebih besar harus lebih dulu menggunakan migrasi pemilik yang dapat dilanjutkan.
...
Ubah alasan yang disimpan untuk pemblokiran yang sudah ada tanpa membuka blokir atau memulihkan
konten (hanya admin). Secara default berjalan sebagai uji coba kecuali `dryRun` adalah `false`.
