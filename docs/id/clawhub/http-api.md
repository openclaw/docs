...
x-i18n:
    generated_at: "2026-06-30T14:25:58Z"
    model: gpt-5.5
...
- Jika token tidak ada/tidak valid, perilaku kembali ke penerapan IP.
- Endpoint tulis terautentikasi tidak boleh mengembalikan `Unauthorized` polos ketika
  ...
  akun yang dihapus/dilarang/dinonaktifkan masing-masing harus mendapatkan teks yang dapat ditindaklanjuti agar klien
...
- `X-RateLimit-Remaining` / `RateLimit-Remaining`: anggaran tersisa persis saat ada.
...
- Menggunakan header IP klien tepercaya, termasuk `cf-connecting-ip`, hanya ketika
  deployment secara eksplisit mengaktifkan header forward tepercaya.
...
- Jika tidak ada IP klien tepercaya yang tersedia, permintaan anonim menggunakan bucket fallback
  ...
  artefak lain yang disuplai pemanggil.
...
- Hasil dikembalikan dalam urutan relevansi (kesamaan embedding + peningkatan token slug/nama persis + prior popularitas kecil).
...
- Teks ASCII ditokenisasi pada batas kata dan tanda baca. Misalnya, `personal-map` berisi token mandiri `map`, sedangkan `amap-jsapi-skill` berisi `amap`, `jsapi`, dan `skill`; karena itu, mencari `map` memberi `personal-map` kecocokan leksikal yang lebih kuat daripada `amap-jsapi-skill`.
...
- Letakkan istilah yang akan benar-benar dicari pengguna di nama tampilan, ringkasan, dan tag. Gunakan token slug mandiri hanya ketika token itu juga merupakan identitas stabil yang ingin Anda pertahankan.
...
- Jika suatu Skill tiba-tiba tidak terlihat, periksa status moderasi terlebih dahulu dengan `clawhub inspect @owner/slug` saat sudah login sebelum mengubah metadata terkait peringkat.
...
- `trending` memberi peringkat berdasarkan instal dalam 7 hari terakhir (berbasis telemetri).
- `createdAt` stabil untuk crawl Skill baru; `updated` berubah ketika Skills yang ada dipublikasikan ulang.
- Ketika `nonSuspiciousOnly=true`, sort berbasis kursor dapat mengembalikan lebih sedikit item daripada `limit` pada suatu halaman karena Skills mencurigakan difilter setelah pengambilan halaman.
...
Laporkan Skill untuk ditinjau moderator. Laporan berada pada level Skill, opsional ditautkan
...
Endpoint moderator/admin untuk asupan laporan Skill.
...
yang sudah ditriage untuk menyembunyikan Skill dalam alur kerja yang dapat diaudit yang sama.
...
- Pekerjaan pemindaian bersifat asinkron. Permintaan pemindaian manual diprioritaskan di atas pekerjaan publikasi/pengisian balik normal, tetapi penyelesaian tetap bergantung pada ketersediaan worker.
...
- Mengembalikan hasil pemindaian tersimpan untuk versi tepat yang dikirimkan, termasuk versi yang diblokir atau disembunyikan.
...
- Identitas skill, identitas penerbit, dan metadata versi yang dipilih adalah kolom amplop tingkat atas (`slug`, `displayName`, `publisherHandle`, `version`, `resolvedFrom`, `tag`, `createdAt`) sehingga otomasi shell dapat membacanya tanpa membongkar wrapper bersarang.
- `security` adalah putusan ClawScan/keamanan tingkat atas. Otomasi harus mengacu pada `ok`, `decision`, `reasons`, dan `security.status`.
...
koleksi ini ditujukan untuk klien yang sudah mengetahui versi skill ClawHub terinstal
...
- `security.signals.dependencyRegistry` dipertahankan untuk kompatibilitas respons v1, tetapi pemindai keberadaan registri dependensi telah dihentikan dan kunci ini selalu `null`.
- Ketiadaan Skill Card tidak memengaruhi `ok`, `decision`, atau `reasons` endpoint ini; klien harus membaca `skill-card.md` terinstal secara lokal ketika membutuhkan konten kartu.
- Gunakan `/verify` ketika Anda membutuhkan amplop verifikasi Skill Card untuk satu skill, `/card` ketika Anda membutuhkan markdown kartu yang dihasilkan, dan `/scan` ketika Anda membutuhkan data pemindai terperinci.
...
- `GET /api/v1/code-plugins` dan `GET /api/v1/bundle-plugins` tetap menjadi alias fixed-family.
...
`trending` adalah papan peringkat instalasi/unduhan tujuh hari dan tidak menggunakan total sepanjang masa.
...
- Rilis ClawPack menyertakan bidang yang kompatibel dengan npm, yaitu `npmIntegrity`, `npmShasum`, dan
  `npmTarballName`.
...
- `trust.stale` berarti ringkasan kepercayaan dihitung dari masukan yang usang dan
  harus diperlakukan sebagai membutuhkan penyegaran sebelum keputusan mengizinkan dengan keyakinan tinggi.

Catatan:

- Endpoint ini spesifik terhadap versi. Klien sebaiknya memanggilnya setelah me-resolve
...
- Endpoint ini sengaja lebih sempit daripada endpoint moderasi pemilik/moderator.
  ...
  identitas pelapor, isi laporan, bukti privat, atau lini masa peninjauan internal.
...
Titik akhir moderator/admin untuk penerimaan laporan paket.
...
Titik akhir pemilik/moderator untuk visibilitas moderasi paket.
...
Titik akhir moderator/admin untuk menyelesaikan atau membuka kembali laporan paket.
...
Titik akhir moderator/admin untuk peninjauan rilis paket.
...
Mengembalikan packument kompatibel npm untuk versi paket yang didukung ClawPack.

Catatan:

- Hanya versi dengan tarball npm-pack ClawPack yang diunggah yang dicantumkan.
- Versi lama yang hanya-ZIP sengaja dihilangkan.
- `dist.tarball`, `dist.integrity`, dan `dist.shasum` menggunakan bidang kompatibel npm
...
skill saat ini yang didukung GitHub dengan pemindaian `clean` atau `suspicious` dan tanpa versi yang
dihosting.
...
- Serah-terima skill yang didukung GitHub tidak mem-proxy atau mencerminkan byte. Respons JSON
...
## Titik akhir autentikasi (token Bearer)

Semua titik akhir memerlukan:
...
- Bidang payload opsional: `migrateOwner`. Saat `true` dengan `ownerHandle`, sebuah
  skill yang ada dapat dipindahkan ke pemilik tersebut jika aktor adalah admin/pemilik pada kedua
...
- Gunakan `files` atau `clawpack`, jangan keduanya dalam permintaan yang sama.
...
- Bidang payload opsional: `ownerHandle`. Jika ada, hanya admin yang boleh menerbitkan atas nama pemilik tersebut.
...
pengguna bersama lama/penerbit personal lama, titik akhir memigrasikannya ke penerbit org terlebih dahulu.
...
pemanggil sebagai pemilik. Titik akhir ini tidak memigrasikan handle pengguna/personal yang ada dan tidak
...
dapat kemudian menerbitkan rilis code-plugin atau bundle-plugin nyata ke nama tersebut.
...
Khusus admin. Memulihkan penerbit personal untuk principal GitHub OAuth pengganti yang terverifikasi
tanpa mengedit baris akun Convex Auth. Permintaan harus menyebutkan kedua id akun provider GitHub
yang tidak dapat diubah; handle yang dapat berubah hanya digunakan sebagai guard yang menghadap operator.
...
prinsipal GitHub. Pemulihan gagal tertutup ketika penerbit personal pengguna tujuan saat ini
memiliki skills, packages, atau sumber skill GitHub.
Pemulihan juga memigrasikan field lama `ownerUserId` untuk skills milik penerbit yang dipulihkan,
alias slug skill, packages, peringatan package inspector, dan baris digest pencarian turunan agar
jalur pemilik langsung sesuai dengan otoritas penerbit baru. Reservasi protected-handle yang aktif
untuk handle yang dipulihkan juga dialihkan ke pengguna pengganti sehingga sinkronisasi profil berikutnya
tidak dapat memulihkan otoritas bersaing milik pengguna sebelumnya. Setiap tabel utama dibatasi hingga
...
Sumber skill GitHub berada dalam cakupan penerbit dan dilaporkan sebagai telah diperiksa, bukan ditulis ulang.
...
Larang pengguna dan hapus permanen skills yang dimiliki (hanya moderator/admin).
...
Cabut larangan pengguna dan pulihkan skills yang memenuhi syarat (hanya admin).
...
Ubah alasan yang tersimpan untuk larangan yang sudah ada tanpa mencabut larangan atau memulihkan
...
Tambahkan/hapus star (highlight). Kedua endpoint bersifat idempoten.
