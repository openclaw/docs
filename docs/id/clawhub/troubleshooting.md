---
read_when:
    - Perintah registri ClawHub CLI atau OpenClaw gagal
    - Paket tidak dapat diinstal, dipublikasikan, atau diperbarui
summary: Pemecahan masalah masuk, penginstalan, publikasi, pembaruan, dan API ClawHub.
x-i18n:
    generated_at: "2026-07-16T17:53:00Z"
    model: gpt-5.6
...
CLI memulai server callback lokal berumur pendek selama proses masuk melalui browser.
...
- Di lingkungan tanpa antarmuka grafis, buat token API di UI web ClawHub lalu jalankan:
...
- `Retry-After`: jumlah detik yang harus ditunggu sebelum mencoba kembali.
- `RateLimit-Limit`: batas yang diterapkan pada permintaan ini.
- `RateLimit-Remaining`: sisa kuota Anda secara persis jika header tersedia. Pada `429`, nilainya adalah `0`.
...
orang hanya mengirim beberapa permintaan. Masuk jika memungkinkan dan coba kembali setelah
...
## Publikasi gagal karena metadata wajib tidak tersedia

Untuk skill, periksa frontmatter `SKILL.md`. Variabel lingkungan dan
alat yang diperlukan harus dideklarasikan agar pengguna dan pemindai dapat memahami paket tersebut.
...
Jika publikasi gagal karena handle pemilik, namespace organisasi, cakupan paket, slug skill,
atau nama paket telah diklaim atau dicadangkan, pertama-tama pastikan bahwa Anda
...
[Klaim Organisasi dan Namespace](/clawhub/namespace-claims) untuk panduan bukti dan hal-hal yang
tidak boleh dicantumkan dalam masalah publik.

## `sync` menyatakan tidak ada skill yang ditemukan

`sync` mencari folder yang berisi `SKILL.md` atau `skill.md`.

Arahkan ke akar direktori yang ingin Anda pindai:
...
- Arahkan pengguna kembali ke cantuman ClawHub kanonis.
