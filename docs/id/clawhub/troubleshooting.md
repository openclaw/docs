...
x-i18n:
    generated_at: "2026-07-02T22:49:21Z"
    model: gpt-5.5
...
Baca informasi retry di respons:

- `Retry-After`: detik yang perlu ditunggu sebelum mencoba lagi.
...
Untuk plugins, periksa metadata kompatibilitas `package.json`. Publish code-plugin
...
Jika publish gagal karena handle pemilik, namespace organisasi, scope paket, slug skill,
atau nama paket sudah diklaim atau dicadangkan, pertama pastikan bahwa Anda
memublikasikan dengan pemilik yang cocok dengan namespace. Untuk paket plugin,
nama berscope seperti `@example-org/example-plugin` harus dipublikasikan sebagai
...
- Hormati header retry `429` dan cache respons daftar/pencarian publik.
