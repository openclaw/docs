...
summary: Pemecahan masalah masuk, pemasangan, penerbitan, pembaruan, dan masalah API ClawHub.
x-i18n:
    generated_at: "2026-07-01T20:35:38Z"
    model: gpt-5.5
...
- Periksa aturan firewall lokal, VPN, dan proxy jika callback tidak pernah tiba.
...
- Jika Anda menggunakan path konfigurasi khusus, pastikan `CLAWHUB_CONFIG_PATH` mengarah ke
  file yang berisi token Anda saat ini.
- Jika Anda menggunakan token API, pastikan token tersebut tidak dicabut di UI web.

## Pencarian atau pemasangan mengembalikan `Rate limit exceeded` (429)
...
- `RateLimit-Remaining`: sisa kuota persis Anda saat header tersedia. Pada `429`, nilainya `0`.
...
jeda yang dilaporkan.

## Pencarian atau pemasangan gagal di balik proxy

CLI menghormati variabel proxy standar:
...
- Jika Anda memiliki skill tersebut, masuk dan inspeksi:
...
memerlukan field kompatibilitas OpenClaw seperti `openclaw.compat.pluginApi` dan
...
## Publikasi gagal dengan error pemilik GitHub atau sumber
...
Jika publikasi gagal karena handle pemilik, namespace org, scope paket, slug skill,
atau nama paket telah diklaim atau dicadangkan, pertama pastikan bahwa Anda
...
pemilik `example-org` yang sesuai.
...
[Klaim Org dan Namespace](/clawhub/namespace-claims) untuk panduan bukti dan apa yang
harus dikecualikan dari isu publik.
...
## Pemasangan Plugin gagal di OpenClaw
...
- Pastikan versi OpenClaw Anda memenuhi rentang kompatibilitas yang diiklankan
  oleh paket.
- Jika paket disembunyikan, ditahan, atau diblokir, paket mungkin tidak dapat dipasang sampai
...
- Jangan mencerminkan konten tersembunyi, privat, tertahan, atau diblokir moderasi di luar
