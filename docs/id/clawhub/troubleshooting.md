...
x-i18n:
    generated_at: "2026-07-12T14:02:46Z"
    model: gpt-5.6
...
CLI memulai server callback lokal berumur singkat selama proses masuk melalui browser.

- Pastikan browser Anda dapat mengakses `http://127.0.0.1:<port>/callback`.
...
- `RateLimit-Remaining`: sisa kuota Anda secara pasti ketika header tersedia. Pada `429`, nilainya adalah `0`.
- `RateLimit-Reset` atau `X-RateLimit-Reset`: waktu penyetelan ulang.
...
## Sebuah Skills tidak muncul dalam pencarian
...
Jika publikasi gagal karena nama pengguna pemilik, namespace organisasi, cakupan paket, slug Skills,
...
nama bercakupan seperti `@example-org/example-plugin` harus diterbitkan sebagai
...
## `update` menolak karena adanya perubahan lokal
...
- Gunakan sumber ClawHub yang eksplisit:
...
- Arahkan pengguna kembali ke daftar resmi ClawHub.
