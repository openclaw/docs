...
summary: Pemecahan masalah masuk, penginstalan, penerbitan, pembaruan, dan masalah API ClawHub.
x-i18n:
...
- Jika Anda menggunakan token API, pastikan token itu belum dicabut di UI web.
...
- Jika Anda pemilik skill tersebut, masuk dan inspeksi:
...
## Penerbitan gagal karena metadata wajib tidak ada

Untuk skill, periksa frontmatter `SKILL.md`. Variabel lingkungan dan
alat wajib harus dideklarasikan agar pengguna dan pemindai dapat memahami paket tersebut.

Untuk Plugin, periksa metadata kompatibilitas `package.json`. Penerbitan code-plugin
memerlukan bidang kompatibilitas OpenClaw seperti `openclaw.compat.pluginApi` dan
`openclaw.build.openclawVersion`.

Pratinjau payload penerbitan terlebih dahulu:

```bash
clawhub package publish <source> --family code-plugin --dry-run
```

## Penerbitan gagal dengan galat pemilik GitHub atau sumber
...
## Penerbitan gagal karena namespace sudah diklaim atau dicadangkan

Jika penerbitan gagal karena handle pemilik, namespace org, cakupan paket, slug skill,
...
Pratinjau terlebih dahulu jika Anda tidak yakin apa yang akan diterbitkan:
...
- Terbitkan salinan yang telah Anda edit sebagai slug atau fork baru.
...
- Pastikan versi OpenClaw Anda memenuhi rentang kompatibilitas yang diiklankan
  oleh paket.
- Jika paket disembunyikan, ditahan, atau diblokir, paket itu mungkin tidak dapat diinstal sampai
...
- Tautkan pengguna kembali ke listing ClawHub kanonis.
