...
- Jika Anda menggunakan token API, pastikan token itu tidak dicabut di UI web.
...
Jika publikasi gagal karena handle pemilik, namespace org, cakupan paket, slug Skills,
atau nama paket sudah diklaim atau dicadangkan, pertama pastikan bahwa Anda
mempublikasikan dengan pemilik yang cocok dengan namespace tersebut. Untuk paket Plugin,
nama bercakupan seperti `@example-org/example-plugin` harus dipublikasikan sebagai
pemilik `example-org` yang cocok.

Jika Anda yakin org, proyek, atau merek Anda adalah pemilik sah namespace tersebut tetapi
...
File lokal tidak cocok dengan versi mana pun yang diketahui ClawHub. Pilih satu:
