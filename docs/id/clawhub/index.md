...
- Gunakan perintah `openclaw` native untuk mencari, menginstal, dan memperbarui Skills serta untuk menginstal Plugin dari ClawHub.
- Gunakan CLI `clawhub` terpisah untuk alur kerja autentikasi registri, publikasi, serta hapus/batalkan hapus.
...
Instal CLI ClawHub saat Anda membutuhkan alur kerja yang diautentikasi registri seperti
...
Spesifikasi Plugin tanpa namespace yang aman untuk npm dapat diselesaikan melalui npm selama peralihan peluncuran, dan
...
yang diiklankan sebelum instalasi arsip berjalan. Ketika versi paket menerbitkan artefak
ClawPack, OpenClaw lebih memilih `.tgz` npm-pack unggahan persisnya, memverifikasi
...
yang cukup lama untuk melewati gerbang unggahan. Halaman detail publik merangkum status pemindaian
...
Saat Anda menjalankan `clawhub install` dalam keadaan masuk, CLI dapat mengirim event instalasi
