...
title: ClawHub
x-i18n:
    generated_at: "2026-07-19T04:50:04Z"
    model: gpt-5.6
...
Instal CLI ClawHub saat Anda memerlukan alur kerja yang diautentikasi oleh registri, seperti
penerbitan atau penghapusan/pemulihan:
...
| -------------- | ------------------------------------------------------------ | -------------------------------------------- |
| Skills         | Bundel teks berversi dengan `SKILL.md` beserta berkas pendukung | `openclaw skills install @openclaw/demo`     |
...
ClawHub melacak versi semver, tag seperti `latest`, log perubahan, berkas,
...
Spesifikasi Plugin sederhana yang aman untuk npm dapat diselesaikan melalui npm selama peralihan peluncuran, dan
`npm:<package>` tetap khusus npm ketika sumber harus dinyatakan secara eksplisit.

Instalasi Plugin memvalidasi kompatibilitas `pluginApi` dan `minGatewayVersion`
yang diumumkan sebelum instalasi arsip dijalankan. Ketika suatu versi paket menerbitkan
artefak ClawPack, OpenClaw memprioritaskan npm-pack `.tgz` yang diunggah secara persis, memverifikasi
...
- `--changelog <text>`: teks log perubahan.
...
ClawHub bersifat terbuka secara bawaan: siapa pun dapat mengunggah, tetapi penerbitan memerlukan akun
GitHub yang cukup lama untuk lolos dari gerbang pengunggahan. Halaman detail publik merangkum
status pemindaian terbaru sebelum instalasi atau pengunduhan.

ClawHub menjalankan pemeriksaan otomatis terhadap Skills dan rilis Plugin yang diterbitkan. Rilis yang
ditahan oleh pemindaian atau diblokir dapat menghilang dari katalog publik dan permukaan instalasi, sementara
...
menyembunyikan atau memulihkan konten, serta memblokir akun yang menyalahgunakan layanan. Lihat
...
Ketika Anda menjalankan `clawhub install` saat telah masuk, CLI dapat mengirim peristiwa
...
Penggantian lingkungan yang berguna:

| Variabel                      | Efek                                              |
| ----------------------------- | ------------------------------------------------- |
| `CLAWHUB_SITE`                | Mengganti URL situs yang digunakan untuk masuk melalui peramban. |
| `CLAWHUB_REGISTRY`            | Mengganti URL API registri.                       |
| `CLAWHUB_CONFIG_PATH`         | Mengganti lokasi CLI menyimpan status token/konfigurasi. |
| `CLAWHUB_WORKDIR`             | Mengganti direktori kerja bawaan.                 |
...
[Pemecahan Masalah](/clawhub/troubleshooting) untuk materi referensi yang lebih mendalam.
