...
- `firstSeenAt`, `lastSeenAt`, opsional `expiredAt`.

Untuk setiap Skills yang ditemukan di bawah sebuah root, kami menyimpan:

- `skillId` (diresolusikan berdasarkan slug; hanya Skills yang ada di registry yang dilacak).
- `firstSeenAt`, `lastSeenAt`.
- `lastVersion` (berdasarkan upaya terbaik; saat ini versi yang cocok dengan registry jika diketahui).
- opsional `removedAt` ketika pemasangan yang sebelumnya dilaporkan menghilang dari sebuah root.
...
- Tidak ada pelacakan untuk Skills yang tidak diunggah ke registry (slug yang tidak dikenal diabaikan).
...
- `installsAllTime`: pengguna unik yang pernah melaporkan Skills terpasang.
...
- Jika Skills dihapus dari semua root Anda, Skills tersebut tidak lagi dihitung dalam `installsCurrent`.
