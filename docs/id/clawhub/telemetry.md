...
- Anda **masuk** di CLI (kami memang sudah mewajibkan autentikasi untuk alur sinkronisasi/publikasi).
...
- `label`: label yang dapat dibaca manusia, diturunkan dari dua segmen jalur terakhir (jalur home ditampilkan dengan `~`).
- `firstSeenAt`, `lastSeenAt`, `expiredAt` opsional.

Untuk setiap skill yang ditemukan di bawah root, kami menyimpan:

- `skillId` (diselesaikan berdasarkan slug; hanya skill yang ada di registry yang dilacak).
...
Kami memelihara dua penghitung per skill:
...
Jika Anda menyinkronkan dari beberapa folder, kami memperlakukan setiap root pemindaian secara independen. Sebuah skill dianggap “sedang terpasang” jika ada di **root aktif mana pun**.

### Deteksi pencopotan

Karena `sync` melaporkan kumpulan lengkap per root:
...
- Menampilkan root persis + skill terpasang yang kami simpan.
