...
Untuk setiap skill yang ditemukan di bawah suatu root, kami menyimpan:
...
- `removedAt` opsional ketika pemasangan yang sebelumnya dilaporkan menghilang dari suatu root.
...
Jika Anda melakukan sinkronisasi dari beberapa folder, kami memperlakukan setiap root pemindaian secara independen. Sebuah skill dianggap “sedang terpasang” jika ada di **root aktif mana pun**.

### Deteksi pencopotan pemasangan

Karena `sync` melaporkan set lengkap per root:

- Jika sebuah skill menghilang dari suatu root pada sinkronisasi berikutnya, kami menandainya sebagai dihapus untuk root tersebut.
