...
read_when:
    ...
    - Men-debug penginstalan, pembaruan, penerbitan, atau sinkronisasi
...
(misalnya kontainer Docker, VPS Hetzner dengan internet hanya melalui proxy, firewall
korporat).
...
- Pencarian memprioritaskan kecocokan token slug/nama yang persis sebelum popularitas unduhan. Token slug mandiri seperti `map` lebih kuat mencocokkan `personal-map` dibanding substring di dalam `amap`.
- Unduhan adalah prior popularitas kecil, bukan jaminan penempatan teratas.
- Jika suatu skill seharusnya muncul tetapi tidak, jalankan `clawhub inspect <slug>` saat login untuk memeriksa diagnostik moderasi yang terlihat oleh owner sebelum mengganti nama metadata.
...
- `--limit <n>`: versi maksimum yang dicantumkan (1-200).
...
- Menampilkan `pinned` di samping keterampilan yang dibekukan dengan `clawhub pin`, termasuk alasan opsionalnya.

### `pin <slug>`

- Menandai keterampilan yang terinstal sebagai disematkan dalam lockfile.
- `--reason <text>` mencatat mengapa keterampilan tersebut dibekukan.
- Keterampilan yang disematkan dilewati oleh `update --all` dan ditolak oleh `update <slug>` langsung.
- Keterampilan yang disematkan juga menolak `install --force` sehingga byte lokal tidak dapat terganti secara tidak sengaja.

### `unpin <slug>`

- Menghapus sematan lockfile dari keterampilan yang terinstal sehingga pembaruan di masa mendatang dapat mengubahnya.
...
- Keterampilan yang disematkan tidak pernah diperbarui oleh `--force`.
...
- `--migrate-owner` memindahkan keterampilan yang sudah ada ke `--owner` sambil mempublikasikan
...
- Mempublikasikan keterampilan berarti keterampilan tersebut dirilis di bawah `MIT-0` di ClawHub.
- Keterampilan yang dipublikasikan bebas digunakan, dimodifikasi, dan didistribusikan ulang tanpa atribusi.
...
- Transfer ke handle org/penerbit langsung berlaku hanya ketika aktor memiliki
...
- `--tag <tag>`: memeriksa versi bertag (mis. `latest`).
...
- Laporan berada pada tingkat paket, secara opsional dikaitkan dengan versi,
...
- Memeriksa apakah sebuah paket siap untuk digunakan OpenClaw di masa mendatang.
...
- Menampilkan status migrasi berorientasi operator untuk paket yang mungkin
  menggantikan plugin OpenClaw bawaan.
...
- `--dry-run` menampilkan pratinjau payload penerbitan terselesaikan tanpa mengunggah.
...
atribusi sumber sebelum membuat rilis langsung:
...
- `openclaw.hostTargets` dan `openclaw.environment` adalah metadata opsional.
  ClawHub dapat menampilkannya jika ada, tetapi keduanya tidak wajib untuk penerbitan.
...
- Sematkan workflow reusable ke tag stabil atau SHA commit penuh. Jangan menjalankan penerbitan rilis dari `@main`.
...
- Menambahkan root skill Clawdbot secara otomatis ketika `~/.clawdbot/clawdbot.json` ada:
...
- Mematuhi `CLAWDBOT_CONFIG_PATH` / `CLAWDBOT_STATE_DIR` dan `OPENCLAW_CONFIG_PATH` / `OPENCLAW_STATE_DIR`.
- Flag:
...
- Dikirim selama `sync` ketika sudah masuk, kecuali `CLAWHUB_DISABLE_TELEMETRY=1` (legacy `CLAWDHUB_DISABLE_TELEMETRY=1`).
