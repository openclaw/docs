...
read_when:
    ...
    - Memecahkan masalah pemasangan, pembaruan, publikasi, atau sinkronisasi
...
- pengganti: `CLAWHUB_CONFIG_PATH` (legacy `CLAWDHUB_CONFIG_PATH`)
...
- Unduhan adalah prior kecil untuk popularitas, bukan jaminan posisi teratas.
...
- Menandai skill yang terpasang sebagai dipin di lockfile.
- `--reason <text>` mencatat alasan skill tersebut dibekukan.
...
- Menghapus pin lockfile dari skill yang terpasang sehingga pembaruan mendatang dapat memodifikasinya.
...
- `--migrate-owner` memindahkan skill yang ada ke `--owner` sambil menerbitkan versi
  baru. Memerlukan akses admin/pemilik pada kedua penerbit.
- Perilaku pemilik dan ulasan dijelaskan dalam `docs/publishing.md`.
...
- Gunakan ini untuk plugin dan entri keluarga paket lain; `search` tingkat atas tetap menjadi permukaan pencarian skill.
- Flag:
...
- Flag:
  ...
  - `--force`: menimpa file keluaran yang ada.
  - `--json`: keluaran yang dapat dibaca mesin.
...
- Nama paket berskup harus ditransfer ke pemilik scope yang sesuai.
...
- Laporan berlaku di tingkat paket, dapat dikaitkan secara opsional dengan versi, dan menjadi terlihat
...
- Untuk sumber GitHub, atribusi sumber diisi otomatis dari repo, commit yang diselesaikan, ref, dan subpath.
...
- `--dry-run` meninjau pratinjau payload penerbitan yang diselesaikan tanpa mengunggah.
...
- Nama paket berskup harus cocok dengan pemilik yang dipilih. Lihat `docs/publishing.md`.
...
Gunakan `--dry-run` terlebih dahulu agar Anda dapat mengonfirmasi metadata paket yang diselesaikan dan
