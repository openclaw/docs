...
read_when:
    ...
    - Men-debug install, pembaruan, atau publikasi
summary: 'Referensi CLI: perintah, flag, konfigurasi, dan perilaku lockfile.'
x-i18n:
    generated_at: "2026-06-27T17:14:35Z"
    model: gpt-5.5
...
(misalnya kontainer Docker, VPS Hetzner dengan internet khusus proksi, firewall
perusahaan).
...
Saat tidak ada variabel proksi yang diatur, perilakunya tidak berubah (koneksi langsung).
...
- Pencarian mengutamakan kecocokan token slug/nama yang persis sebelum popularitas unduhan. Token slug mandiri seperti `map` cocok dengan `personal-map` lebih kuat daripada substring di dalam `amap`.
- Popularitas adalah prior peringkat kecil, bukan jaminan posisi teratas.
- Jika skill seharusnya muncul tetapi tidak, jalankan `clawhub inspect @owner/slug` saat login untuk memeriksa diagnostik moderasi yang terlihat oleh owner sebelum mengganti nama metadata.
...
- Flag:
  ...
  - `--sort newest|updated|rating|downloads|trending` (default: newest). Alias sort instalasi legacy masih berfungsi untuk kompatibilitas.
...
- Skill yang dipin juga menolak `install --force` sehingga byte lokal tidak dapat terganti tanpa sengaja.
...
- Membandingkan fingerprint bundle lokal dengan ClawHub dan keluar dengan sukses saat
...
- Perilaku owner dan ulasan dijelaskan di `docs/publishing.md`.
...
Atur `dry_run: true` untuk pratinjau tanpa token. Publikasi nyata memerlukan secret
`clawhub_token`.
...
- Pemindaian bersifat asinkron dan dapat memerlukan waktu untuk selesai. Saat antre, spinner terminal menampilkan posisi pemindaian prioritas saat ini dan berapa banyak pemindaian di depan.
...
- Memerlukan `clawhub login`.
- Mengunduh ZIP laporan pemindaian tersimpan untuk versi skill atau Plugin yang dikirimkan, termasuk versi yang diblokir atau disembunyikan oleh pemeriksaan keamanan ClawHub.
...
- Publikasi skill V1 menggunakan `clawhub_token`; publikasi tepercaya OIDC GitHub untuk saat ini hanya untuk paket.

### `delete <skill>`

- Tanpa `--version`, hapus lunak sebuah skill (pemilik, moderator, atau admin).
- Memanggil `DELETE /api/v1/skills/{slug}`.
- Hapus lunak yang dimulai oleh pemilik mencadangkan slug selama 30 hari; perintah mencetak waktu kedaluwarsanya.
- `--version <version>` menghapus permanen satu versi milik sendiri yang bukan terbaru melalui rute khusus versi
  yang tertutup saat gagal.
...
- `--reason <text>` mencatat catatan moderasi pada hapus lunak seluruh skill dan log audit.
...
- Ganti nama skill milik sendiri dan pertahankan slug sebelumnya sebagai alias pengalihan.
- Memanggil `POST /api/v1/skills/{slug}/rename`.
- `--yes` melewati konfirmasi.

### `skill merge <source> <target>`

- Gabungkan satu skill milik sendiri ke skill milik sendiri lainnya.
...
- Default ke validasi offline/statis, tanpa menemukan atau mengimpor checkout
  OpenClaw lokal.
- Kesalahan kompatibilitas keras keluar dengan nilai non-nol. Temuan yang hanya berupa peringatan dicetak tetapi
...
- Tanpa `--version`, menghapus-lunak paket dan semua rilisnya.
- `--version <version>` menghapus permanen satu rilis yang dimiliki dan bukan terbaru melalui rute gagal-tertutup
  yang spesifik versi.
  Versi yang dihapus tidak dapat dipulihkan atau dipublikasikan ulang. Publikasikan pengganti sebelum menghapus
  versi terbaru saat ini. Alur khusus versi ini memerlukan pemilik paket atau admin penerbit org; staf platform tidak melewati kepemilikan paket.
- Penghapusan lunak seluruh paket memerlukan pemilik paket, owner/admin penerbit org, moderator platform, atau admin platform.
- Flag:
...
- Memerlukan pemilik paket, owner/admin penerbit org, moderator platform,
  atau admin platform.
...
- Laporan berada pada tingkat paket, secara opsional dikaitkan dengan versi, dan menjadi terlihat
  oleh moderator untuk ditinjau.
- Laporan tidak otomatis menyembunyikan paket atau memblokir unduhan.
- Flag:
...
- Sumber `.tgz` diperlakukan sebagai ClawPack. CLI mengunggah byte npm-pack
  persisnya dan memakai konten `package/` yang diekstrak hanya untuk validasi dan
  pengisian awal metadata.
- Folder Plugin kode dikemas ke tarball npm ClawPack sebelum diunggah agar
  instalasi OpenClaw dapat memverifikasi artefak persisnya. Folder Plugin bundle tetap
  memakai jalur penerbitan file yang diekstrak.
...
- `--dry-run` mempratinjau payload penerbitan yang di-resolve tanpa mengunggah.
...
- Nama package berscope harus cocok dengan owner yang dipilih. Lihat `docs/publishing.md`.
...
Gunakan `--dry-run` terlebih dahulu agar Anda dapat mengonfirmasi metadata package yang di-resolve dan
...
- `package.json.version` adalah versi rilis package Anda, tetapi tidak digunakan sebagai
...
- `openclaw.hostTargets` dan `openclaw.environment` adalah metadata opsional.
  ClawHub dapat menampilkannya saat tersedia, tetapi tidak wajib untuk penerbitan.
...
- Jika Anda menggunakan rilis CLI `clawhub` yang lebih lama, upgrade sebelum menerbitkan agar
...
- Workflow yang dapat digunakan kembali menetapkan default `source` ke repo pemanggil.
- Untuk monorepo, teruskan `source_path` agar workflow menerbitkan folder package Plugin,
  misalnya `source_path: extensions/codex`.
- Pin workflow yang dapat digunakan kembali ke tag stabil atau SHA commit penuh. Jangan menjalankan penerbitan rilis dari `@main`.
- `pull_request` harus memakai `dry_run: true` agar CI tetap tidak mencemari.
...
- Tetap sediakan `clawhub_token` untuk penerbitan pertama, package tidak tepercaya, atau penerbitan darurat.
- Workflow mengunggah hasil JSON sebagai artefak dan mengeksposnya sebagai output workflow.

### `package trusted-publisher get <name>`

- Menampilkan konfigurasi penerbit tepercaya GitHub Actions untuk sebuah package.
...
- Melampirkan atau mengganti konfigurasi penerbit tepercaya GitHub Actions untuk package
  yang sudah ada.
- Package harus dibuat terlebih dahulu melalui `clawhub package publish`
  manual normal atau yang diautentikasi token.
- Setelah konfigurasi diatur, penerbitan GitHub Actions yang didukung di masa mendatang dapat memakai
...
- Menghapus konfigurasi penerbit tepercaya dari sebuah package.
- Gunakan ini sebagai rollback jika workflow, repositori, atau pin environment perlu
  dinonaktifkan atau dibuat ulang.
- Penerbitan nyata di masa mendatang harus memakai penerbitan terautentikasi normal sampai konfigurasi
  diatur lagi.
- Flag:
  - `--json`: output yang dapat dibaca mesin.
...
- Dikirim setelah `clawhub install <slug>` saat login, kecuali
  `CLAWHUB_DISABLE_TELEMETRY=1` disetel.
