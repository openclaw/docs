...
read_when:
    ...
    - Men-debug pemasangan, pembaruan, publikasi, atau sinkronisasi
summary: 'Referensi CLI: perintah, opsi, konfigurasi, berkas kunci, perilaku sinkronisasi.'
x-i18n:
...
- Fallback lama: jika `clawhub/config.json` belum ada tetapi `clawdhub/config.json` ada, CLI menggunakan ulang path lama
- override: `CLAWHUB_CONFIG_PATH` (lama `CLAWDHUB_CONFIG_PATH`)
...
- Pencarian memprioritaskan kecocokan token slug/nama persis sebelum popularitas unduhan. Token slug mandiri seperti `map` cocok dengan `personal-map` lebih kuat daripada substring di dalam `amap`.
...
- Skill yang pinned juga menolak `install --force` agar byte lokal tidak terganti secara tidak sengaja.

### `unpin <slug>`

- Menghapus pin lockfile dari skill yang terinstal agar pembaruan mendatang dapat mengubahnya.
...
- `update --all` melewati slug yang pinned dan mencetak ringkasan tentang apa saja yang tetap dibekukan.
...
- `--migrate-owner` memindahkan skill yang ada ke `--owner` saat menerbitkan
...
- Menerbitkan skill berarti skill dirilis di bawah `MIT-0` di ClawHub.
...
- Gunakan ini untuk plugins dan entri keluarga paket lainnya; `search` tingkat atas tetap menjadi permukaan pencarian skill.
- Flag:
...
- Laporan berada di tingkat paket, dapat dikaitkan secara opsional ke versi,
...
- Menampilkan status pemindaian paket saat ini, jumlah laporan terbuka, status
  moderasi manual rilis terbaru, status pemblokiran unduhan, dan alasan moderasi.
- Opsi:
  - `--json`: keluaran yang dapat dibaca mesin.
...
- Memanggil endpoint kesiapan terkomputasi yang sama seperti `package readiness`, tetapi mencetak
...
- Sumber `.tgz` diperlakukan sebagai ClawPack. CLI mengunggah byte npm-pack yang tepat
...
- Folder Plugin kode dikemas menjadi tarball npm ClawPack sebelum diunggah agar
  instalasi OpenClaw dapat memverifikasi artefak yang tepat. Folder Plugin bundel tetap
...
- Untuk folder lokal, atribusi sumber terdeteksi otomatis dari git lokal ketika remote origin mengarah ke GitHub.
...
- Nama paket berlingkup harus cocok dengan pemilik yang dipilih. Lihat `docs/publishing.md`.
- Opsi yang sudah ada (`--family`, `--name`, `--version`, `--source-repo`, `--source-commit`, `--source-ref`, `--source-path`) tetap berfungsi sebagai override.
...
ClawHub juga menyediakan workflow resmi yang dapat digunakan kembali di
...
- Workflow yang dapat digunakan kembali menetapkan `source` secara default ke repo pemanggil.
- Untuk monorepo, berikan `source_path` agar workflow menerbitkan folder paket
  Plugin, misalnya `source_path: extensions/codex`.
- Pin workflow yang dapat digunakan kembali ke tag stabil atau SHA commit lengkap. Jangan menjalankan penerbitan rilis dari `@main`.
