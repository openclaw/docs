...
- Menampilkan `pinned` di samping skill yang dibekukan dengan `clawhub pin`, termasuk alasan opsionalnya.

### `pin <slug>`

- Menandai skill terpasang sebagai dipin di lockfile.
...
- Menghapus pin lockfile dari skill terpasang agar pembaruan mendatang dapat mengubahnya.
...
- Perilaku pemilik dan peninjauan dijelaskan dalam `docs/publishing.md`.
...
- Transfer ke handle pengguna membuat permintaan tertunda yang diterima penerima.
...
- Gunakan ini untuk metadata plugin, kompatibilitas, verifikasi, sumber, serta inspeksi versi/file.
- `--version <version>`: memeriksa versi tertentu (default: terbaru).
...
- Flag:
  ...
  - `--force`: menimpa file output yang ada.
  - `--json`: output yang dapat dibaca mesin.
...
- Laporan berlaku di tingkat paket, opsional ditautkan ke versi, dan menjadi terlihat
...
- Untuk sumber GitHub, atribusi sumber diisi otomatis dari repo, commit yang diresolve, ref, dan subpath.
...
- `--dry-run` mempratinjau payload publikasi yang diresolve tanpa mengunggah.
...
Gunakan `--dry-run` terlebih dahulu agar Anda dapat mengonfirmasi metadata paket yang diresolve dan
...
Plugin kode eksternal membutuhkan sedikit metadata OpenClaw di
...
- `openclaw.hostTargets` dan `openclaw.environment` adalah metadata opsional.
  ClawHub dapat menampilkannya ketika ada, tetapi tidak wajib untuk publikasi.
...
ClawHub juga mengirimkan workflow resmi yang dapat digunakan kembali di
...
- Tetap sediakan `clawhub_token` untuk publikasi pertama, paket tidak tepercaya, atau publikasi darurat.
