...
- Pencarian memprioritaskan kecocokan token slug/nama yang tepat sebelum popularitas unduhan. Token slug mandiri seperti `map` lebih kuat mencocokkan `personal-map` daripada substring di dalam `amap`.
...
- Me-resolve versi terbaru melalui `/api/v1/skills/<slug>`.
...
- Menandai skill yang terpasang sebagai pinned di lockfile.
- `--reason <text>` mencatat alasan skill dibekukan.
- Skill pinned dilewati oleh `update --all` dan ditolak oleh `update <slug>` langsung.
- Skill pinned juga menolak `install --force` sehingga byte lokal tidak dapat tergantikan secara tidak sengaja.

### `unpin <slug>`

- Menghapus pin lockfile dari skill yang terpasang sehingga pembaruan berikutnya dapat mengubahnya.
...
- `update <slug>` langsung gagal untuk slug pinned dan memberi tahu Anda untuk menjalankan `clawhub unpin <slug>` terlebih dahulu.
- `update --all` melewati slug pinned dan mencetak ringkasan tentang apa saja yang tetap dibekukan.
...
- `--version <version>`: periksa versi tertentu (bawaan: terbaru).
...
- `--files`: tampilkan daftar file untuk versi yang dipilih.
- `--file <path>`: ambil konten file mentah (hanya file teks; batas 200KB).
...
- Flag:
  ...
  - `--tag <tag>`: unduh versi bertag (bawaan: `latest`).
...
- Menampilkan status pemindaian paket saat ini, jumlah laporan terbuka, status moderasi
  manual rilis terbaru, status blokir unduhan, dan alasan moderasi.
- Flag:
  - `--json`: keluaran yang dapat dibaca mesin.
...
- `--dry-run` mempratinjau payload penerbitan terselesaikan tanpa mengunggah.
...
Gunakan `--dry-run` terlebih dahulu agar Anda dapat mengonfirmasi metadata paket terselesaikan dan
...
- `openclaw.hostTargets` dan `openclaw.environment` adalah metadata opsional.
  ClawHub dapat menampilkannya ketika ada, tetapi tidak wajib untuk penerbitan.
...
- Pertahankan `clawhub_token` tersedia untuk penerbitan pertama, paket tidak tepercaya, atau penerbitan darurat.
...
- Otomatis menambahkan root skill Clawdbot saat `~/.clawdbot/clawdbot.json` ada:
