...
read_when:
    ...
    - Menelusuri kesalahan pada pemasangan, pembaruan, penerbitan, atau sinkronisasi
...
- `--dir <dir>`: dir instalasi di bawah workdir (default: `skills`)
...
(misalnya kontainer Docker, VPS Hetzner dengan internet khusus proxy, firewall
korporat).
...
- Mengambil metadata skill dan berkas versi tanpa menginstal.
...
- `--files`: mencantumkan berkas untuk versi yang dipilih.
- `--file <path>`: mengambil konten berkas mentah (hanya berkas teks; batas 200KB).
- `--json`: output yang dapat dibaca mesin.

### `install <slug>`

- Menentukan versi terbaru melalui `/api/v1/skills/<slug>`.
...
- Menampilkan `pinned` di sebelah skill yang dibekukan dengan `clawhub pin`, termasuk alasan opsionalnya.

### `pin <slug>`

- Menandai skill yang terpasang sebagai disematkan di lockfile.
- `--reason <text>` mencatat mengapa skill dibekukan.
- Skill yang disematkan dilewati oleh `update --all` dan ditolak oleh `update <slug>` langsung.
- Skill yang disematkan juga menolak `install --force` agar byte lokal tidak terganti secara tidak sengaja.

### `unpin <slug>`

- Menghapus sematan lockfile dari skill yang terpasang agar pembaruan berikutnya dapat mengubahnya.
...
- `update <slug>` gagal cepat untuk slug yang disematkan dan memberi tahu Anda untuk menjalankan `clawhub unpin <slug>` terlebih dahulu.
...
- `--migrate-owner` memindahkan skill yang ada ke `--owner` sambil menerbitkan
...
- Menghapus sementara sebuah skill (pemilik, moderator, atau admin).
...
- Transfer ke handle org/penerbit diterapkan segera hanya ketika aktor memiliki
...
- Menghitung SHA-256 ClawHub, integritas `sha512` npm, dan shasum npm untuk
  artefak lokal.
...
- Memulihkan paket dan rilis yang dihapus secara soft-delete.
- Memerlukan pemilik paket, pemilik/admin penerbit organisasi, moderator platform,
  atau admin platform.
...
- Pemilik dan admin penerbit dapat memindai ulang paket mereka sendiri hingga batas
  pemulihan per rilis.
- Moderator dan admin platform dapat memindai ulang paket apa pun dan tidak diblokir oleh
...
- Perintah pemilik/penerbit untuk mengajukan banding atas moderasi rilis.
...
- Flag:
  - `--version <version>`: versi paket yang wajib diisi.
  - `--message <text>`: pesan banding yang wajib diisi.
  - `--json`: keluaran yang dapat dibaca mesin.
...
- Sumber `.tgz` diperlakukan sebagai ClawPack. CLI mengunggah byte npm-pack persis
...
- `--dry-run` meninjau payload publikasi yang terselesaikan tanpa mengunggah.
- `--json` menghasilkan keluaran yang dapat dibaca mesin untuk CI.
- `--owner <handle>` mempublikasikan di bawah handle penerbit pengguna atau organisasi ketika aktor memiliki akses penerbit.
...
- `openclaw.compat.minGatewayVersion` dan
  `openclaw.build.pluginSdkVersion` adalah tambahan opsional jika Anda ingin mempublikasikan
...
- Workflow reusable default menetapkan `source` ke repo caller.
- Untuk monorepo, berikan `source_path` agar workflow mempublikasikan folder paket
  Plugin, misalnya `source_path: extensions/codex`.
- Pin workflow reusable ke tag stabil atau SHA commit lengkap. Jangan menjalankan publikasi rilis dari `@main`.
...
- Pertahankan `clawhub_token` tersedia untuk publikasi pertama, paket tidak tepercaya, atau publikasi darurat.
...
- Otomatis menambahkan root skill Clawdbot ketika `~/.clawdbot/clawdbot.json` ada:
