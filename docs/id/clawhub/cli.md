...
Jika tidak ada variabel proxy yang disetel, perilaku tidak berubah (koneksi langsung).
...
- Interaktif jarak jauh/headless: `clawhub login --device` mencetak kode dan menunggu sementara Anda mengotorisasinya di `<site>/cli/device`.
...
- Pencarian mengutamakan kecocokan token slug/nama yang tepat sebelum popularitas unduhan. Token slug mandiri seperti `map` cocok dengan `personal-map` lebih kuat daripada substring di dalam `amap`.
- Unduhan adalah prior popularitas kecil, bukan jaminan posisi teratas.
- Jika sebuah skill seharusnya muncul tetapi tidak, jalankan `clawhub inspect <slug>` saat login untuk memeriksa diagnostik moderasi yang terlihat oleh pemilik sebelum mengganti nama metadata.
...
- Skill yang disematkan juga menolak `install --force` sehingga byte lokal tidak dapat diganti secara tidak sengaja.

### `unpin <slug>`

- Menghapus pin lockfile dari skill terinstal sehingga pembaruan berikutnya dapat mengubahnya.
...
- `update --all` melewati slug yang disematkan dan mencetak ringkasan tentang apa saja yang tetap dibekukan.
...
- Perilaku pemilik dan tinjauan dijelaskan di `docs/publishing.md`.
...
- `--clawscan-note <text>` menambahkan catatan ClawScan. Catatan ini memberi ClawScan
  konteks untuk perilaku yang mungkin tampak tidak biasa, seperti akses jaringan,
...
- Slug sumber berhenti tampil secara publik dan menjadi alias pengalihan ke target.
...
- Flag:
  ...
  - `--force`: timpa file output yang ada.
  - `--json`: output yang dapat dibaca mesin.
...
- Menghapus lunak sebuah paket beserta semua rilisnya.
...
- Laporan berada pada tingkat paket, secara opsional terkait dengan suatu versi, dan menjadi terlihat
...
- Memanggil endpoint kesiapan terkomputasi yang sama dengan `package readiness`, tetapi mencetak
...
- Sumber `.tgz` diperlakukan sebagai ClawPack. CLI mengunggah byte npm-pack persis
  dan menggunakan isi `package/` yang diekstrak hanya untuk validasi dan
...
- Untuk sumber GitHub, atribusi sumber diisi otomatis dari repo, commit yang diresolusikan, ref, dan subpath.
...
- Plugin kode eksternal harus mendeklarasikan `openclaw.compat.pluginApi` dan
  ...
  `package.json.version` tingkat teratas tidak digunakan sebagai fallback untuk validasi publikasi.
...
- `openclaw.hostTargets` dan `openclaw.environment` adalah metadata opsional.
  ClawHub dapat menampilkannya saat tersedia, tetapi tidak diwajibkan untuk publikasi.
...
- Workflow yang dapat digunakan ulang menetapkan default `source` ke repo caller.
...
- Menambahkan otomatis root skill Clawdbot saat `~/.clawdbot/clawdbot.json` tersedia:
  - `agent.workspace/skills` (agen utama)
  - `routing.agents.*.workspace/skills` (per agen)
  - `~/.clawdbot/skills` (bersama)
  - `skills.load.extraDirs` (pak bersama)
...
- Dikirim selama `sync` saat sudah login, kecuali `CLAWHUB_DISABLE_TELEMETRY=1` (legacy `CLAWDHUB_DISABLE_TELEMETRY=1`).
