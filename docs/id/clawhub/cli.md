...
read_when:
    ...
    - Pemecahan masalah instalasi, pembaruan, penerbitan, atau sinkronisasi
summary: 'Referensi CLI: perintah, flag, konfigurasi, berkas kunci, perilaku sinkronisasi.'
x-i18n:
...
- `--workdir <dir>`: direktori kerja (default: cwd; kembali ke ruang kerja Clawdbot jika dikonfigurasi)
...
- `--registry <url>`: URL dasar API (default: ditemukan, jika tidak `https://clawhub.ai`)
...
- Tanpa tampilan: `clawhub login --token clh_...`
- Interaktif jarak jauh/tanpa tampilan: `clawhub login --device` mencetak kode dan menunggu saat Anda mengotorisasinya di `<site>/cli/device`.
...
- Jika suatu skill seharusnya muncul tetapi tidak, jalankan `clawhub inspect <slug>` saat sudah login untuk memeriksa diagnostik moderasi yang terlihat oleh pemilik sebelum mengganti nama metadata.
...
- Menampilkan `pinned` di samping skills yang dibekukan dengan `clawhub pin`, termasuk alasan opsional.

### `pin <slug>`

- Menandai skill yang terinstal sebagai pinned di lockfile.
- `--reason <text>` mencatat mengapa skill dibekukan.
- Skill pinned dilewati oleh `update --all` dan ditolak oleh `update <slug>` langsung.
- Skill pinned juga menolak `install --force` agar byte lokal tidak dapat diganti secara tidak sengaja.
...
- Skill pinned tidak pernah diperbarui oleh `--force`.
- `update <slug>` gagal cepat untuk slug pinned dan memberi tahu Anda untuk menjalankan `clawhub unpin <slug>` terlebih dahulu.
- `update --all` melewati slug pinned dan mencetak ringkasan tentang apa yang tetap dibekukan.
...
- `--migrate-owner` memindahkan skill yang sudah ada ke `--owner` sambil memublikasikan
...
- `--clawscan-note <text>` menambahkan catatan ClawScan. Catatan ini memberi ClawScan
  ...
  akses host native, atau kredensial khusus provider. Catatan disimpan pada
...
- Flag:
  ...
  - `--force`: menimpa file output yang sudah ada.
  - `--json`: output yang dapat dibaca mesin.
...
- Sumber `.tgz` diperlakukan sebagai ClawPack. CLI mengunggah byte npm-pack yang persis
  dan hanya menggunakan isi `package/` yang diekstrak untuk validasi dan
...
- `openclaw.hostTargets` dan `openclaw.environment` adalah metadata opsional.
  ClawHub dapat menampilkannya ketika ada, tetapi tidak diwajibkan untuk penerbitan.
...
- Pastikan `clawhub_token` tersedia untuk penerbitan pertama, paket tidak tepercaya, atau penerbitan break-glass.
...
- Flag:
  - `--root <dir...>` root pemindaian ekstra
...
- Dikirim selama `sync` ketika sudah login, kecuali `CLAWHUB_DISABLE_TELEMETRY=1` (legacy `CLAWDHUB_DISABLE_TELEMETRY=1`).
