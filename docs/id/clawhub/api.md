...
x-i18n:
    generated_at: "2026-07-04T20:43:23Z"
    model: gpt-5.5
...
Anda dapat membangun katalog, direktori, atau permukaan pencarian pihak ketiga di atas API baca publik ClawHub. Metadata keterampilan publik dan file keterampilan diterbitkan berdasarkan aturan lisensi keterampilan ClawHub, sedangkan API itu sendiri dibatasi lajunya dan harus digunakan secara bertanggung jawab.
...
- Tautkan kembali ke URL keterampilan ClawHub kanonis saat menampilkan daftar agar pengguna dapat memeriksa rekaman registry sumber.
...
- Jangan menyalin konten tersembunyi, privat, atau yang diblokir moderasi dengan melewati filter API publik atau batas autentikasi.
...
Penegakan yang sadar autentikasi:

- Permintaan anonim: per IP.
- Permintaan terautentikasi (token Bearer valid): per bucket pengguna.
- Token yang hilang/tidak valid kembali ke penegakan berbasis IP.
...
- `GET /api/v1/skills?limit=&cursor=&sort=`
  - `sort`: `updated` (default), `recommended` (`default`), `createdAt` (`newest`), `downloads`, `stars` (`rating`), alias pemasangan legacy `installsCurrent`/`installs`/`installsAllTime` dipetakan ke `downloads`, `trending`
...
- `GET /api/v1/download?slug=&version=&tag=`
  - Keterampilan yang di-host mengembalikan byte ZIP deterministik.
  - Keterampilan saat ini yang berbasis GitHub dengan pemindaian `clean` atau `suspicious` mengembalikan
    deskriptor serah-terima JSON `public-github`, bukan byte ClawHub.
- `GET /api/v1/skills/export?startDate=&endDate=&limit=&cursor=`
  - Keterampilan yang di-host diekspor sebagai file tersimpan.
  - Keterampilan saat ini yang berbasis GitHub dengan pemindaian `clean` atau `suspicious` diekspor
