...
Anda dapat membangun katalog, direktori, atau permukaan pencarian pihak ketiga di atas API baca publik ClawHub. Metadata skill publik dan file skill diterbitkan berdasarkan aturan lisensi skill ClawHub, sementara API itu sendiri dibatasi laju dan harus digunakan secara bertanggung jawab.

Panduan:

- Gunakan endpoint baca publik seperti `GET /api/v1/skills`, `GET /api/v1/search`, dan `GET /api/v1/skills/{slug}` untuk daftar katalog.
- Cache respons dan hormati header `429`, `Retry-After`, dan rate-limit alih-alih melakukan polling secara agresif.
- Tautkan kembali ke URL skill ClawHub kanonis saat menampilkan daftar agar pengguna dapat memeriksa catatan registri sumber.
- Gunakan URL halaman kanonis dalam bentuk `https://clawhub.ai/<owner>/skills/<slug>`.
- Jangan menyiratkan bahwa ClawHub mendukung, memverifikasi, atau mengoperasikan situs pihak ketiga.
- Jangan mencerminkan konten tersembunyi, privat, atau diblokir moderasi dengan melewati filter API publik atau batas auth.

## Auth

- Baca publik: tidak perlu token.
- Tulis + akun: `Authorization: Bearer clh_...`.

## Batas laju

Penegakan sadar auth:
...
- Unduh: 1200/menit per IP, 6000/menit per key
...
- Kesalahan v1 berupa teks polos (`text/plain; charset=utf-8`), termasuk `400`,
  `401`, `403`, `404`, `429`, dan respons unduhan yang diblokir.
- Parameter kueri yang tidak dikenal diabaikan untuk kompatibilitas.
...
- `GET /api/v1/skills?limit=&cursor=&sort=`
  ...
  - Alias lama: `nonSuspicious=true`
  - Dengan `nonSuspiciousOnly=true`, halaman berbasis cursor dapat berisi item lebih sedikit daripada `limit`; gunakan `nextCursor` untuk melanjutkan.
  - `recommended` menggunakan sinyal keterlibatan dan kekinian.
...
- `GET /api/v1/download?slug=&version=&tag=`
  ...
  - Skills saat ini yang didukung GitHub dengan pemindaian `clean` atau `suspicious` mengembalikan deskriptor handoff JSON `public-github` alih-alih byte ClawHub.
...
- `POST /api/v1/skills` (terbitkan, multipart lebih disarankan)
