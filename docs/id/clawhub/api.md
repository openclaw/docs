...
x-i18n:
    generated_at: "2026-07-02T17:46:01Z"
    model: gpt-5.5
...
Anda dapat membangun katalog, direktori, atau permukaan pencarian pihak ketiga di atas API baca publik ClawHub. Metadata skill publik dan file skill diterbitkan berdasarkan aturan lisensi skill ClawHub, sementara API itu sendiri dibatasi lajunya dan harus digunakan secara bertanggung jawab.
...
- `X-RateLimit-Remaining` / `RateLimit-Remaining`: anggaran tersisa yang tepat saat
  ada; permintaan sukses yang di-shard menghilangkannya alih-alih mengembalikan nilai
  global perkiraan
...
- Kesalahan v1 berupa teks biasa (`text/plain; charset=utf-8`), termasuk respons `400`,
  `401`, `403`, `404`, `429`, dan unduhan yang diblokir.
...
- `GET /api/v1/skills?limit=&cursor=&sort=`
  - `sort`: `updated` (default), `recommended` (`default`), `createdAt` (`newest`), `downloads`, `stars` (`rating`), alias instalasi legacy `installsCurrent`/`installs`/`installsAllTime` dipetakan ke `downloads`, `trending`
  ...
  - Alias legacy: `nonSuspicious=true`
  - Dengan `nonSuspiciousOnly=true`, halaman berbasis kursor dapat berisi item lebih sedikit daripada `limit`; gunakan `nextCursor` untuk melanjutkan.
...
- `GET /api/v1/download?slug=&version=&tag=`
  - Skill yang dihosting mengembalikan byte ZIP deterministik.
  - Skill saat ini yang didukung GitHub dengan pemindaian `clean` atau `suspicious` mengembalikan
    deskriptor handoff JSON `public-github` alih-alih byte ClawHub.
- `GET /api/v1/skills/export?startDate=&endDate=&limit=&cursor=`
  - Skill yang dihosting diekspor sebagai file tersimpan.
  - Skill saat ini yang didukung GitHub dengan pemindaian `clean` atau `suspicious` diekspor
...
- `POST /api/v1/skills` (publikasi, multipart disarankan)
...
- `POST /api/v1/users/reserve` mencadangkan root slug dan placeholder paket privat tanpa rilis untuk handle pemilik.
