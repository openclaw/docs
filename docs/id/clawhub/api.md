...
x-i18n:
    generated_at: "2026-07-03T02:54:40Z"
    model: gpt-5.5
...
Anda dapat membangun katalog, direktori, atau permukaan pencarian pihak ketiga di atas API baca publik ClawHub. Metadata skill publik dan berkas skill diterbitkan menurut aturan lisensi skill ClawHub, sementara API itu sendiri dibatasi laju dan harus digunakan secara bertanggung jawab.

Panduan:

- Gunakan endpoint baca publik seperti `GET /api/v1/skills`, `GET /api/v1/search`, dan `GET /api/v1/skills/{slug}` untuk daftar katalog.
- Cache respons dan hormati header `429`, `Retry-After`, dan pembatasan laju, bukan melakukan polling secara agresif.
