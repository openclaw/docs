...
Anda dapat membangun katalog pihak ketiga, direktori, atau permukaan pencarian di atas API baca publik ClawHub. Metadata skill publik dan file skill dipublikasikan berdasarkan aturan lisensi skill ClawHub, sedangkan API itu sendiri dibatasi lajunya dan harus digunakan secara bertanggung jawab.

Panduan:

- Gunakan endpoint baca publik seperti `GET /api/v1/skills`, `GET /api/v1/search`, dan `GET /api/v1/skills/{slug}` untuk listing katalog.
- Cache respons dan patuhi header `429`, `Retry-After`, dan batas laju alih-alih melakukan polling secara agresif.
- Tautkan kembali ke URL skill ClawHub kanonis saat menampilkan listing agar pengguna dapat memeriksa catatan registry sumber.
...
- `GET /api/v1/skills?limit=&cursor=&sort=`
  ...
  - `cursor` berlaku untuk urutan non-`trending`
  - Filter opsional: `nonSuspiciousOnly=true`
  - Alias lama: `nonSuspicious=true`
  - Dengan `nonSuspiciousOnly=true`, halaman berbasis kursor mungkin berisi lebih sedikit dari `limit` item; gunakan `nextCursor` untuk melanjutkan.
...
- `GET /api/v1/download?slug=&version=&tag=`
  - Skill yang di-host mengembalikan byte ZIP deterministik.
...
- `GET /api/v1/skills/export?startDate=&endDate=&limit=&cursor=`
  - Skill yang di-host diekspor sebagai file yang disimpan.
