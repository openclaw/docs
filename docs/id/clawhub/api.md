...
x-i18n:
    generated_at: "2026-06-30T22:34:50Z"
    model: gpt-5.5
...
Anda dapat membangun katalog, direktori, atau permukaan pencarian pihak ketiga di atas API baca publik ClawHub. Metadata skill publik dan berkas skill diterbitkan di bawah aturan lisensi skill ClawHub, sementara API itu sendiri dibatasi lajunya dan harus digunakan secara bertanggung jawab.
...
- `GET /api/v1/skills?limit=&cursor=&sort=`
  - `sort`: `updated` (default), `recommended` (`default`), `createdAt` (`newest`), `downloads`, `stars` (`rating`), alias install lama `installsCurrent`/`installs`/`installsAllTime` dipetakan ke `downloads`, `trending`
...
- `GET /api/v1/skills/export?startDate=&endDate=&limit=&cursor=`
  - Skill yang di-host diekspor sebagai berkas tersimpan.
