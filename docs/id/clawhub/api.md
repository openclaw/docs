...
x-i18n:
    generated_at: "2026-07-04T15:36:09Z"
    model: gpt-5.5
...
Anda dapat membangun katalog, direktori, atau permukaan pencarian pihak ketiga di atas API baca publik ClawHub. Metadata skill publik dan file skill dipublikasikan berdasarkan aturan lisensi skill ClawHub, sementara API itu sendiri dibatasi lajunya dan harus digunakan secara bertanggung jawab.
...
- Tautkan kembali ke URL skill ClawHub kanonis saat menampilkan daftar agar pengguna dapat memeriksa catatan registry sumber.
...
- `X-RateLimit-Remaining` / `RateLimit-Remaining`: anggaran tersisa yang persis saat
  ada; permintaan berhasil yang dishard menghilangkannya alih-alih mengembalikan nilai
  global perkiraan
...
- `GET /api/v1/download?slug=&version=&tag=`
  ...
  - Skill saat ini yang didukung GitHub dengan pemindaian `clean` atau `suspicious` mengembalikan
    deskriptor handoff JSON `public-github`, bukan byte ClawHub.
...
- `POST /api/v1/users/reserve` mencadangkan slug root dan placeholder paket privat tanpa rilis untuk handle owner.
