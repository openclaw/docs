...
Anda dapat membangun katalog, direktori, atau permukaan pencarian pihak ketiga di atas API baca publik ClawHub. Metadata keterampilan publik dan file keterampilan diterbitkan berdasarkan aturan lisensi keterampilan ClawHub, sementara API itu sendiri dibatasi lajunya dan harus digunakan secara bertanggung jawab.

Pedoman:

- Gunakan endpoint baca publik seperti `GET /api/v1/skills`, `GET /api/v1/search`, dan `GET /api/v1/skills/{slug}` untuk daftar katalog.
- Simpan respons dalam cache dan hormati `429`, `Retry-After`, serta header batas laju alih-alih melakukan polling secara agresif.
- Tautkan kembali ke URL keterampilan ClawHub kanonis saat menampilkan daftar agar pengguna dapat memeriksa catatan registri sumber.
...
- Utamakan `Retry-After` saat tersedia.
...
- `GET /api/v1/skills?limit=&cursor=&sort=`
  - `sort`: `updated` (bawaan), `createdAt` (`newest`), `downloads`, `stars` (`rating`), `installsCurrent` (`installs`), `installsAllTime`, `trending`
