...
Anda dapat membangun katalog, direktori, atau permukaan pencarian pihak ketiga di atas API baca publik ClawHub. Metadata skill publik dan berkas skill dipublikasikan berdasarkan aturan lisensi skill ClawHub, sementara API itu sendiri dibatasi lajunya dan harus digunakan secara bertanggung jawab.
...
Pemberlakuan sadar autentikasi:

- Permintaan anonim: per IP.
- Permintaan terautentikasi (token Bearer valid): per bucket pengguna.
- Token yang hilang/tidak valid kembali ke pemberlakuan berbasis IP.
...
- `POST /api/v1/users/reserve` mencadangkan slug root dan placeholder paket tanpa rilis privat untuk handle pemilik.
