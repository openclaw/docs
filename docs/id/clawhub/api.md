...
x-i18n:
    generated_at: "2026-07-01T15:31:07Z"
    model: gpt-5.5
...
- `GET /api/v1/search?q=...`
  ...
  - Alias warisan: `nonSuspicious=true`
- `GET /api/v1/skills?limit=&cursor=&sort=`
  - `sort`: `updated` (default), `recommended` (`default`), `createdAt` (`newest`), `downloads`, `stars` (`rating`), alias pemasangan warisan `installsCurrent`/`installs`/`installsAllTime` dipetakan ke `downloads`, `trending`
...
- `GET /api/v1/packages?limit=&cursor=&sort=`
  - `sort`: `updated` (default), `recommended`, `downloads`, alias warisan `installs`
  - Nilai `sort` yang tidak valid mengembalikan `400`
- `GET /api/v1/plugins?limit=&cursor=&sort=`
  - `sort`: `recommended` (default), `downloads`, `updated`, alias warisan `installs`
...
`/api/*` dan `/api/cli/*` warisan masih tersedia. Lihat `DEPRECATIONS.md`.
