...
title: Penyiapan
x-i18n:
    generated_at: "2026-07-21T12:19:03Z"
    model: gpt-5.6
...
akan melanjutkan ke orientasi terpandu. Gunakan `-m`/`--message` untuk satu permintaan atau
`--baseline` untuk menginisialisasi folder konfigurasi/ruang kerja tanpa wizard.

Urutan perutean:

1. Setiap opsi orientasi (`--wizard`, `--baseline`, ruang kerja, pengaturan ulang,
...
3. Tanpa opsi perutean, sistem interaktif yang telah dikonfigurasi akan membuka OpenClaw. Sistem
   baru akan menjalankan orientasi. Pada sistem yang telah dikonfigurasi, `--json` mencetak
   ikhtisar sistem bahkan tanpa TTY; opsi orientasi tetap menggunakan ringkasan JSON
   orientasi.
...
pada instalasi baru. Jika daftar agen yang ada akan dipetakan ulang,
wizard klasik memerlukan konfirmasi eksplisit; penyiapan noninteraktif mempertahankan
ruang kerja armada saat ini dan mencetak peringatan.
...
model yang dikonfigurasi, proses masuk CLI yang didukung, variabel lingkungan kunci API, serta model
Ollama atau LM Studio yang sudah terinstal. Model lokal tidak pernah diunduh oleh
proses otomatis ini. Runtime lokal yang terdeteksi diuji secara otomatis setelah kandidat CLI dan
kunci API; jika beberapa model lokal tersedia, OpenClaw memilih
keluarga instruct terkuat yang mendukung pemanggilan alat. Kandidat yang dipilih harus memberikan
respons completion nyata sebelum konfigurasi penyedia dan modelnya disimpan.
CLI Gemini, Antigravity, Pi, dan OpenCode yang terinstal juga dilaporkan jika
...
`--skip-health`, `--skip-ui`, `--skip-hooks`). Teruskan `--tui` untuk menggunakan
jalur keluar terminal yang sama dengan `openclaw onboard --tui`. Lihat [Orientasi](/id/cli/onboard) dan
...
<Note>
`openclaw setup` ditujukan untuk instalasi dengan konfigurasi yang dapat diubah. Dalam mode Nix (`OPENCLAW_NIX_MODE=1`), OpenClaw menolak penulisan penyiapan karena file konfigurasi dikelola oleh Nix. Gunakan [Mulai Cepat nix-openclaw pihak pertama](https://github.com/openclaw/nix-openclaw#quick-start) atau konfigurasi sumber yang setara untuk paket Nix lainnya.
</Note>
...
| `--workspace <dir>`        | Usulan ruang kerja; armada yang ada memerlukan konfirmasi klasik dan dipertahankan secara noninteraktif. |
| `--baseline`               | Buat folder konfigurasi/ruang kerja/sesi dasar tanpa orientasi.                                      |
| `--wizard`                 | Paksa orientasi interaktif.                                                                          |
| `--tui`                    | Gunakan jalur keluar terminal alih-alih penyerahan ke browser.                                       |
...
| `--import-source <path>`   | Direktori asal agen sumber untuk `--import-from`.                                                  |
...
| `--json`                   | Sistem terkonfigurasi: ikhtisar OpenClaw. Rute orientasi: ringkasan orientasi.                       |

`--classic` dan `--non-interactive` saling eksklusif: mode klasik membuka
wizard dengan perintah interaktif, sedangkan penyiapan noninteraktif menggunakan jalur otomatisasi.
Dalam orientasi interaktif, `--remote-url` dan `--remote-token` mengisi sebelumnya
langkah Gateway jarak jauh dan lebih diprioritaskan daripada nilai jarak jauh tersimpan untuk proses tersebut.
Mengubah URL tidak menggunakan kembali kredensial tersimpan kecuali Anda juga meneruskan token.
...
menolak opsi orientasi, Gateway, autentikasi, pengaturan ulang, atau daemon yang eksplisit alih-alih
mengabaikannya secara diam-diam. Jika konfigurasi yang ada tidak valid, penyiapan dasar mempertahankannya
dan meminta Anda menjalankan `openclaw doctor` sebelum mencoba lagi.
...
- Jika status Hermes terdeteksi, orientasi interaktif dapat menawarkan migrasi secara otomatis. Orientasi impor memerlukan penyiapan baru; gunakan [Migrasi](/id/cli/migrate) untuk rencana uji coba tanpa perubahan, cadangan, dan mode penimpaan di luar orientasi.
