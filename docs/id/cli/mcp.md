---
read_when:
    - Menghubungkan Codex, Claude Code, atau klien MCP lainnya ke saluran yang didukung OpenClaw
...
title: MCP
x-i18n:
    generated_at: "2026-07-20T14:08:28Z"
    model: gpt-5.6
...
`serve` adalah OpenClaw yang bertindak sebagai server MCP. Subperintah lainnya adalah OpenClaw yang bertindak sebagai registri sisi klien MCP untuk server yang dapat digunakan oleh runtime-nya sendiri nanti.

<Note>
  `list`, `show`, `set`, dan `unset` hanya membaca dan menulis entri `mcp.servers` yang dikelola OpenClaw dalam konfigurasi OpenClaw. Perintah-perintah tersebut tidak menyertakan server mcporter dari `config/mcporter.json`; gunakan `mcporter list` untuk registri tersebut.
</Note>
...
| Mengedit konfigurasi MCP dari peramban                                      | Control UI `/settings/mcp` (alias `/mcp`)                            | Halaman ini menampilkan inventaris, pengaktifan, ringkasan OAuth/filter, petunjuk perintah, dan editor `mcp` yang tercakup.         |
| Memberikan server MCP native tercakup kepada Codex app-server                    | `mcp.servers.<name>.codex`                                           | Blok `codex` hanya memengaruhi proyeksi utas Codex app-server dan dihapus sebelum penyerahan konfigurasi native. |
...
- Codex, Claude Code, atau klien MCP lain harus berkomunikasi langsung dengan percakapan kanal yang didukung OpenClaw
- Anda sudah memiliki Gateway OpenClaw lokal atau jarak jauh dengan sesi yang dirutekan
- Anda menginginkan satu server MCP yang berfungsi di seluruh backend kanal OpenClaw, alih-alih menjalankan jembatan terpisah untuk setiap kanal

Gunakan [`openclaw acp`](/id/cli/acp) sebagai gantinya ketika OpenClaw harus menghosting runtime pengodean itu sendiri dan mempertahankan sesi agen di dalam OpenClaw.

### Cara kerjanya

`openclaw mcp serve` memulai server MCP stdio. Klien MCP memiliki proses tersebut. Selama klien mempertahankan sesi stdio tetap terbuka, jembatan terhubung ke Gateway OpenClaw lokal atau jarak jauh melalui WebSocket dan mengekspos percakapan kanal yang dirutekan melalui MCP.
...
<Steps>
  ...
  <Step title="Antrean peristiwa langsung">
    Peristiwa langsung diantrekan dalam memori selama jembatan terhubung.
  </Step>
...
<AccordionGroup>
  <Accordion title="Perilaku penting">
    ...
    - titik masuk agen sekali jalan seperti `openclaw agent` dan `openclaw infer model run` menghentikan runtime MCP terpaket yang dibukanya ketika balasan selesai, sehingga eksekusi skrip berulang tidak mengakumulasi proses turunan MCP stdio
    - server MCP stdio yang dijalankan oleh OpenClaw (terpaket atau dikonfigurasi pengguna) dihentikan sebagai pohon proses saat penonaktifan, sehingga subproses turunan yang dimulai oleh server tidak tetap berjalan setelah klien stdio induk berhenti
...
<Tabs>
  ...
  <Tab title="Claude Code">
    Alat MCP standar ditambah adaptor kanal khusus Claude. Aktifkan `--claude-channel-mode on` atau biarkan nilai default `auto`.
  </Tab>
</Tabs>
...
Jembatan menggunakan metadata rute sesi Gateway yang ada untuk mengekspos percakapan yang didukung kanal. Percakapan muncul ketika OpenClaw sudah memiliki status sesi dengan rute yang diketahui, seperti:
...
<AccordionGroup>
  ...
  <Accordion title="messages_read">
    Membaca pesan transkrip terbaru untuk satu percakapan berbasis sesi. `limit` memiliki nilai default 20, maks. 200.
  </Accordion>
  ...
  <Accordion title="events_poll">
    Membaca peristiwa langsung yang diantrekan sejak kursor numerik. `limit` maks. 200.
  </Accordion>
  <Accordion title="events_wait">
    Melakukan long-poll hingga peristiwa antrean berikutnya yang cocok tiba atau waktu tunggu habis (default 30 detik, maks. 300 detik).

    Gunakan ini ketika klien MCP generik memerlukan pengiriman mendekati waktu nyata tanpa protokol push khusus Claude.
  ...
  <Accordion title="messages_send">
    ...
    - menggunakan kanal, penerima, id akun, dan id utas sesi
    - hanya mengirim teks

  </Accordion>
  <Accordion title="permissions_list_open">
    Mencantumkan permintaan persetujuan eksekusi/plugin tertunda yang diamati jembatan sejak terhubung ke Gateway.
  </Accordion>
...
<Warning>
- antrean hanya untuk peristiwa langsung; antrean dimulai ketika jembatan MCP dimulai
...
Jembatan juga dapat mengekspos notifikasi kanal khusus Claude. Ini adalah padanan OpenClaw untuk adaptor kanal Claude Code: alat MCP standar tetap tersedia, tetapi pesan masuk langsung juga dapat diterima sebagai notifikasi MCP khusus Claude.
...
Ketika mode kanal Claude diaktifkan, server mengumumkan kapabilitas eksperimental Claude dan dapat memancarkan:
...
Ini sengaja dibuat khusus untuk klien. Klien MCP generik harus mengandalkan alat polling standar.
...
<ParamField path="--url" type="string">
  URL WebSocket Gateway. Nilai defaultnya adalah `gateway.remote.url` jika dikonfigurasi.
</ParamField>
...
Jembatan tidak menciptakan perutean. Jembatan hanya mengekspos percakapan yang sudah diketahui cara peruteannya oleh Gateway.

Artinya:

- daftar izin pengirim, pemasangan, dan kepercayaan tingkat kanal tetap berada dalam konfigurasi kanal OpenClaw yang mendasarinya
- `messages_send` hanya dapat membalas melalui rute tersimpan yang sudah ada
- status persetujuan hanya aktif/dalam memori untuk sesi jembatan saat ini
- autentikasi jembatan harus menggunakan kontrol token atau kata sandi Gateway yang sama dengan yang Anda percayai untuk klien Gateway jarak jauh lainnya

Jika suatu percakapan tidak ada dalam `conversations_list`, penyebab umumnya bukan konfigurasi MCP. Penyebabnya adalah metadata rute yang hilang atau tidak lengkap dalam sesi Gateway yang mendasarinya.

### Pengujian

OpenClaw menyediakan smoke Docker deterministik untuk jembatan ini:

```bash
pnpm test:docker:mcp-channels
```

Smoke tersebut menjalankan satu kontainer: mengisi status percakapan awal, memulai Gateway, lalu menjalankan `openclaw mcp serve` sebagai proses anak stdio dan mengendalikannya sebagai klien MCP. Pengujian ini memverifikasi penemuan percakapan, pembacaan transkrip, pembacaan metadata lampiran, perilaku antrean peristiwa langsung, serta notifikasi kanal dan izin bergaya Claude melalui jembatan MCP stdio yang sebenarnya. Perutean pengiriman keluar (`messages_send` yang menggunakan kembali rute percakapan tersimpan) dicakup secara terpisah oleh pengujian unit dalam `src/mcp/channel-server.test.ts`.

Ini adalah cara tercepat untuk membuktikan bahwa jembatan berfungsi tanpa menghubungkan akun Telegram, Discord, atau iMessage yang sebenarnya ke proses pengujian.
...
<AccordionGroup>
  <Accordion title="Tidak ada percakapan yang dikembalikan">
    Biasanya berarti sesi Gateway belum dapat dirutekan. Pastikan sesi yang mendasarinya memiliki metadata kanal/penyedia, penerima, serta rute akun/utas opsional yang tersimpan.
  </Accordion>
  <Accordion title="events_poll atau events_wait melewatkan pesan lama">
    Sesuai perkiraan. Antrean langsung dimulai ketika jembatan terhubung. Baca riwayat transkrip lama dengan `messages_read`.
  </Accordion>
  <Accordion title="Notifikasi Claude tidak muncul">
    ...
    - pesan masuk terjadi setelah jembatan terhubung

  </Accordion>
  <Accordion title="Persetujuan tidak ada">
    `permissions_list_open` hanya menampilkan permintaan persetujuan yang diamati saat jembatan terhubung. Ini bukan API riwayat persetujuan yang persisten.
  </Accordion>
</AccordionGroup>
...
Definisi tersimpan tersebut ditujukan untuk runtime yang nantinya dijalankan atau dikonfigurasi oleh OpenClaw, seperti OpenClaw tertanam dan adaptor runtime lainnya. OpenClaw menyimpan definisi secara terpusat agar runtime tersebut tidak perlu menyimpan daftar server MCP duplikatnya sendiri.
...
<AccordionGroup>
  <Accordion title="Perilaku penting">
    ...
    - `status --verbose` mencetak transportasi, autentikasi, batas waktu, filter, dan petunjuk pemanggilan alat paralel yang telah diselesaikan tanpa terhubung
    - `doctor` memeriksa definisi tersimpan untuk masalah penyiapan lokal seperti perintah stdio yang hilang, direktori kerja yang tidak valid, berkas TLS yang hilang, server yang dinonaktifkan, nilai header/env sensitif literal, dan otorisasi OAuth yang belum lengkap
    ...
    - `add` membuat definisi dari flag dan melakukan probe sebelum menyimpan, kecuali `--no-probe` ditetapkan atau otorisasi OAuth harus dilakukan terlebih dahulu
    - adaptor runtime menentukan bentuk transportasi yang benar-benar didukung saat eksekusi
    ...
    - server yang mengiklankan sumber daya atau prompt juga mengekspos alat utilitas untuk mencantumkan/membaca sumber daya dan mencantumkan/mengambil prompt; nama utilitas yang dihasilkan tersebut (`resources_list`, `resources_read`, `prompts_list`, `prompts_get`) menggunakan filter sertakan/kecualikan yang sama
    - perubahan dinamis pada daftar alat MCP membatalkan katalog yang di-cache untuk sesi tersebut; penemuan/penggunaan berikutnya menyegarkan data dari server
    - kegagalan permintaan/protokol alat MCP yang berulang akan menjeda server tersebut sebentar agar satu server yang rusak tidak menghabiskan seluruh giliran
    - runtime MCP bawaan yang tercakup dalam sesi dihentikan setelah tidak aktif selama 10 menit dan proses tertanam sekali jalan membersihkannya pada akhir proses

  </Accordion>
</AccordionGroup>

Adaptor runtime dapat menormalisasi registri bersama ini ke dalam bentuk yang diharapkan klien hilirnya. Misalnya, OpenClaw tertanam menggunakan nilai `transport` OpenClaw secara langsung, sedangkan Claude Code dan Gemini menerima nilai `type` bawaan CLI seperti `http`, `sse`, atau `stdio`.

Codex app-server juga mematuhi blok opsional `codex` pada setiap server. Ini adalah
metadata proyeksi OpenClaw hanya untuk utas Codex app-server; metadata ini tidak
...
untuk menghasilkan `default_tools_approval_mode` bawaan Codex bagi server tepercaya.
...
- `status` mengklasifikasikan transportasi yang dikonfigurasi tanpa terhubung. `--verbose` mencakup detail peluncuran, batas waktu, OAuth, filter, dan panggilan paralel yang telah diselesaikan, termasuk ketika token OAuth tersimpan memerlukan otorisasi tambahan. Argumen stdio yang memuat kredensial disamarkan dalam keluaran teks dan JSON.
- `doctor` menjalankan pemeriksaan statis tanpa terhubung. Tambahkan `--probe` jika perintah juga harus memverifikasi bahwa server yang diaktifkan dapat terhubung.
...
- `configure` memperbarui pengaktifan, filter alat, batas waktu, OAuth, TLS, dan petunjuk pemanggilan alat paralel tanpa mengganti seluruh definisi server. Tambahkan `--probe` untuk memverifikasi server yang diperbarui sebelum menyimpan.
...
- `logout` menghapus kredensial OAuth yang tersimpan untuk server bernama tanpa menghapus definisi server yang tersimpan.
...
- Gunakan `transport: "streamable-http"` untuk server MCP HTTP yang Dapat Dialirkan. `openclaw mcp set` juga menormalisasi `type: "http"` bawaan CLI ke bentuk konfigurasi kanonis yang sama untuk kompatibilitas.
...
<Tabs>
  <Tab title="Sistem file">
    ...
    Batasi server sistem file ke pohon direktori terkecil yang perlu dibaca atau diedit oleh agen.
  ...
  <Tab title="Memori">
    ...
    Gunakan filter alat jika server menyediakan alat tulis yang tidak boleh tersedia bagi agen biasa.
  ...
  <Tab title="Skrip lokal">
    ...
    `doctor` memeriksa bahwa `cwd` tersedia dan bahwa perintah dapat ditemukan dari lingkungan yang dikonfigurasi.
  ...
  <Tab title="HTTP jarak jauh">
    ...
    Gunakan OAuth jika server jarak jauh mendukungnya. Jika server memerlukan header statis, jangan melakukan commit token bearer literal.
  ...
  <Tab title="Desktop/CUA">
    ...
    Server pengendali desktop langsung mewarisi izin proses yang diluncurkannya. Gunakan filter alat yang ketat dan permintaan izin pada tingkat OS.
...
<AccordionGroup>
  ...
  <Accordion title="doctor --json">
    ...
    `doctor --json` keluar dengan kode bukan nol jika server aktif yang diperiksa memiliki masalah tingkat `error`. Masalah `warning` dan `info` dilaporkan, tetapi tidak menyebabkan perintah gagal dengan sendirinya.
...
| Bidang                      | Deskripsi                              |
| -------------------------- | --------------------------------- |
| `command`                  | Berkas executable yang akan dijalankan (wajib)    |
...
OpenClaw menolak kunci lingkungan untuk permulaan interpreter, pembajakan pemuat, dan inisialisasi shell sebelum menjalankan server MCP stdio, meskipun kunci tersebut muncul dalam blok `env` server. Hal ini menggunakan kebijakan keamanan lingkungan host yang sama dengan proses lain yang dijalankan OpenClaw: kebijakan ini memblokir kait permulaan interpreter yang diketahui (misalnya `NODE_OPTIONS`, `PYTHONSTARTUP`, `PERL5OPT`, `RUBYOPT`, `BASHOPTS`, `KSH_ENV`), prefiks injeksi pustaka bersama dan fungsi (`DYLD_*`, `LD_*`, `BASH_FUNC_*`), serta variabel kontrol runtime serupa. Saat permulaan, variabel ini dihapus secara diam-diam dan peringatan dicatat agar variabel tersebut tidak dapat menyuntikkan pendahuluan implisit, mengganti interpreter, mengaktifkan debugger, atau membajak penaut dinamis terhadap proses stdio. Daftar izin eksplisit menjaga agar variabel lingkungan kredensial MCP biasa tetap dapat digunakan (`GITHUB_TOKEN`, `GH_TOKEN`, `GITLAB_TOKEN`, `NPM_TOKEN`, `NODE_AUTH_TOKEN`, `DATABASE_URL`, `MONGODB_URI`, `REDIS_URL`, `AMQP_URL`, `AWS_ACCESS_KEY_ID`, `AWS_SECRET_ACCESS_KEY`, `AWS_SESSION_TOKEN`, `AZURE_CLIENT_ID`, `AZURE_CLIENT_SECRET`), bersama dengan variabel lingkungan proksi biasa dan variabel khusus server (`HTTP_PROXY`, `*_API_KEY` khusus, dan sebagainya). Kunci `AWS_*` lain seperti `AWS_CONFIG_FILE` dan `AWS_SHARED_CREDENTIALS_FILE` tetap diblokir karena menunjuk ke berkas kredensial, bukan membawa nilai kredensial secara langsung.
...
| `sslVerify`                 | Tetapkan ke false hanya untuk titik akhir HTTPS privat yang dipercaya secara eksplisit    |
...
Nilai sensitif dalam `url` (info pengguna) dan `headers` disamarkan dalam log dan keluaran status. `openclaw mcp doctor` memperingatkan ketika entri `headers` atau `env` yang tampak sensitif berisi nilai literal, sehingga operator dapat memindahkan nilai tersebut dari konfigurasi yang di-commit.

### Alur kerja OAuth

OAuth ditujukan untuk server MCP HTTP yang mengiklankan alur OAuth MCP. Header `Authorization` statis diabaikan untuk server selama `auth: "oauth"` diaktifkan. Kredensial yang disimpan oleh `openclaw mcp login` berfungsi dengan MCP tersemat, runner CLI, dan server aplikasi Codex lokal.

Sesi OAuth MCP native berada dalam basis data SQLite bersama yang hanya dapat diakses pemilik di `<state-dir>/state/openclaw.sqlite` (`mcp_oauth_stores`). Baris tersebut dapat berisi token akses dan penyegaran, rahasia pendaftaran klien dinamis, metadata penemuan, serta verifier PKCE sementara. Penyegaran, login, dan logout menggunakan lease SQLite yang sama, sehingga proses OpenClaw paralel tidak dapat menggunakan satu token penyegaran atau menghidupkan kembali sesi yang telah logout.

Peningkatan dari penyimpanan `<state-dir>/mcp-oauth/*.json` yang sudah dihentikan hanya ditangani oleh `openclaw doctor --fix`. Kode runtime tidak pernah membaca, menulis, atau melakukan fallback ke berkas tersebut.
...
Jika server menolak token dengan `insufficient_scope`, OpenClaw mempertahankan cakupan yang diminta dan meminta `openclaw mcp login <name>`, alih-alih mengulangi penyegaran yang tidak dapat memberikan cakupan baru. Login tersebut memulai permintaan otorisasi baru sembari mempertahankan token sebelumnya hingga kredensial pengganti disimpan.

Jika layanan MCP jarak jauh sudah didukung oleh profil autentikasi OpenClaw terpisah yang mampu melakukan penyegaran, Anda dapat menetapkan `oauth.authProfileId` secara opsional. OpenClaw menyegarkan salah satu sumber kredensial sebelum proyeksi runtime dan hanya meneruskan token akses terkini ke klien MCP hilir.
...
<Steps>
  ...
  <Step title="Mulai login">
    ...
    OpenClaw mencetak URL otorisasi dan menyimpan status verifier OAuth sementara dalam SQLite bersama.
  ...
  <Step title="Periksa otorisasi">
    Gunakan status atau doctor untuk memastikan token tersedia dan tidak memerlukan otorisasi tambahan. Jika status melaporkan `authorization-required` atau doctor meminta otorisasi tambahan, jalankan kembali `openclaw mcp login <name>`.
...
<Note>
Perintah registri tidak memulai bridge kanal. Hanya `probe` dan `doctor --probe` yang membuka sesi klien MCP aktif untuk membuktikan bahwa server tujuan dapat dijangkau.
</Note>
...
4. Alihkan status pengaktifan saat Anda ingin mempertahankan definisi tetapi mengecualikannya dari penemuan runtime.
...
7. Jalankan `openclaw mcp doctor --probe` saat Anda memerlukan bukti aktif bahwa server yang diedit berhasil dimulai dan mencantumkan alat.

Catatan:

- cuplikan perintah mengapit nama server dengan tanda kutip agar nama yang tidak lazim tetap dapat disalin ke shell
- nilai mirip URL yang ditampilkan disunting sebelum dirender jika berisi kredensial tersemat
- halaman tersebut tidak memulai transport MCP dengan sendirinya
- runtime aktif mungkin memerlukan `openclaw mcp reload`, publikasi konfigurasi Gateway, atau pemulaian ulang proses, bergantung pada proses yang memiliki klien MCP
...
Koneksi Gateway langsung memerlukan akses ke kedua port. Jika proksi balik atau terminator TLS mengekspos UI Kontrol, berikan origin publik khusus kepada Aplikasi dan proksikan hanya origin tersebut ke listener sandbox:
...
- Hanya sumber daya `ui://` dengan tipe MIME `text/html;profile=mcp-app` yang persis sama yang dirender.
- Sumber daya UI dibatasi hingga 2 MiB, ditempatkan di balik proksi iframe ganda pada origin luar khusus, dimuat ke dalam origin Aplikasi dalam yang opak, dan dibatasi oleh CSP yang diturunkan dari metadata sumber daya.
- Alat khusus Aplikasi (`_meta.ui.visibility: ["app"]`) tidak dimasukkan ke daftar alat model. Aplikasi hanya dapat memanggil alat yang terlihat oleh aplikasi pada server pemiliknya dan yang juga lolos kebijakan alat OpenClaw efektif untuk proses yang membuat tampilan tersebut.
- Izin Aplikasi yang terikat origin seperti kamera, mikrofon, dan geolokasi tidak diberikan selama dokumen Aplikasi dalam menggunakan origin opak untuk isolasi antar-Aplikasi.
- HTML Aplikasi, argumen alat lengkap, dan hasil mentah berada dalam lease tampilan dalam memori selama sepuluh menit yang dibatasi serta tidak ditulis ke disk atau disalin ke metadata pratinjau transkrip. Transkrip hanya menyimpan deskriptor server/alat/sumber daya terbatas yang terikat pada ID panggilan alat asli. Setelah Gateway dimulai ulang, UI Kontrol dapat memverifikasi deskriptor tersebut terhadap transkrip sesi terautentikasi dan mengambil ulang sumber daya `ui://`; tampilan yang direkonstruksi bersifat hanya-baca hingga proses baru menetapkan izin alat saat ini.
- Dalam percakapan kanal, tampilan Aplikasi terakhir yang berhasil dalam satu giliran menambahkan satu tindakan bergaya **Buka Aplikasi** ke balasan akhir asisten. DM Telegram menggunakan tombol Mini App bawaan; Slack dan Discord merender tindakan portabel yang sama sebagai tautan. Kanal lain mempertahankan teks balasan asli dan menambahkan tautan HTTPS yang mudah dipahami.
- Tautan peluncuran kanal hanya tersedia saat eksposur Tailscale Gateway telah menyiapkan origin HTTPS yang dipublikasikan. `gateway.tailscale.mode: "serve"` hanya dapat dijangkau dari tailnet; `"funnel"` dapat dijangkau dari internet publik. Funnel yang dikelola secara eksternal dan dipertahankan oleh `gateway.tailscale.preserveFunnel` juga dianggap dapat dijangkau dari internet. Lihat [Tailscale](/id/gateway/tailscale).
- Tiket peluncuran bersifat opak, dibuat hanya saat mewujudkan balasan akhir kanal, dan kedaluwarsa setelah paling lama dua menit atau saat lease tampilan yang mendasarinya kedaluwarsa, mana saja yang lebih dahulu. URL tidak berisi kredensial bearer Gateway, kunci sesi, metadata tampilan, HTML Aplikasi, input alat, atau hasil alat.
- Jika tidak tersedia origin yang dipublikasikan atau kapasitas tiket, tampilan atau tiket telah kedaluwarsa, atau transport tidak dapat merender kontrol bawaan, teks asisten asli tetap tersedia. UI Kontrol mempertahankan kanvas Aplikasi inline yang ada dan tidak menerima tindakan peluncuran duplikat.
- `openclaw security audit` memberikan peringatan selama bridge diaktifkan. Nonaktifkan dengan `openclaw config set mcp.apps.enabled false --strict-json` saat tidak diperlukan.
...
- transport HTTP/SSE/streamable-http terhubung ke satu server jarak jauh; belum ada upstream yang dimultipleks
