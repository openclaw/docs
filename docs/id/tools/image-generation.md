...
penyedia yang telah Anda konfigurasikan. Gambar yang dihasilkan dikirim secara otomatis sebagai lampiran media
dalam balasan agen.
...
<Steps>
  ...
  <Step title="Pick a default model (optional)">
    ...
    `openai-codex` dikonfigurasi, OpenClaw merutekan permintaan
    gambar melalui profil OAuth tersebut, bukan terlebih dahulu mencoba
    `OPENAI_API_KEY`. Konfigurasi eksplisit `models.providers.openai` (kunci API,
    URL dasar kustom/Azure) mengaktifkan kembali rute langsung OpenAI Images API.

  </Step>
  <Step title="Ask the agent">
    _"Buat gambar maskot robot yang ramah."_

    Agen memanggil `image_generate` secara otomatis. Tidak diperlukan
    daftar izin alat - alat ini diaktifkan secara default ketika penyedia tersedia.
...
| PNG/WebP OpenAI berlatar belakang transparan          | `openai/gpt-image-1.5`                             | `OPENAI_API_KEY` atau OpenAI Codex OAuth |
...
khusus OpenAI; penyedia lain mungkin masih mempertahankan alfa PNG jika
backend mereka menghasilkannya.

## Penyedia yang didukung

| Penyedia   | Model default                         | Dukungan edit                    | Auth                                                  |
| ---------- | ------------------------------------- | -------------------------------- | ----------------------------------------------------- |
...
opsi geometri yang dekat alih-alih yang persis diminta, OpenClaw memetakan ulang ke
...
dukungan dan dilaporkan dalam hasil alat. Hasil alat melaporkan pengaturan yang diterapkan;
`details.normalization` menangkap terjemahan apa pun dari yang diminta ke yang diterapkan.
</Note>
...
4. **Deteksi otomatis** - hanya default penyedia yang didukung auth:
   - penyedia default saat ini terlebih dahulu;
   - penyedia pembuatan gambar terdaftar yang tersisa dalam urutan id penyedia.

Jika penyedia gagal (kesalahan auth, batas laju, dll.), kandidat terkonfigurasi
berikutnya dicoba secara otomatis. Jika semua gagal, kesalahan menyertakan detail
dari setiap upaya.

<AccordionGroup>
  <Accordion title="Per-call model overrides are exact">
    Penggantian `model` per panggilan hanya mencoba penyedia/model tersebut dan
    tidak melanjutkan ke penyedia utama/fallback yang dikonfigurasi atau yang terdeteksi otomatis.
  </Accordion>
  <Accordion title="Auto-detection is auth-aware">
    Default penyedia hanya masuk ke daftar kandidat ketika OpenClaw benar-benar dapat
    mengautentikasi penyedia tersebut. Tetapkan
  ...
  <Accordion title="Timeouts">
    ...
    Panggilan alat dinamis Codex mematuhi anggaran timeout yang sama, dibatasi
    oleh maksimum bridge alat dinamis OpenClaw sebesar 600000 md.
  </Accordion>
  <Accordion title="Inspect at runtime">
    Gunakan `action: "list"` untuk memeriksa penyedia yang saat ini terdaftar,
    model default mereka, dan petunjuk variabel lingkungan auth.
  </Accordion>
</AccordionGroup>
...
parameter `images`. fal, MiniMax, dan ComfyUI mendukung 1.

## Ulasan mendalam penyedia
...
<AccordionGroup>
  <Accordion title="OpenAI gpt-image-2 (and gpt-image-1.5)">
    ...
    `openai-codex` dikonfigurasi, OpenClaw menggunakan kembali profil
    ...
    seperti `https://chatgpt.com/backend-api` dikanoniskan menjadi
    ...
    **tidak** diam-diam beralih ke `OPENAI_API_KEY` untuk permintaan tersebut -
    ...
    `gpt-image-1.5` untuk keluaran PNG/WebP berlatar belakang transparan; API
    ...
    nilai tersebut ke `size` yang didukung, jika tidak alat melaporkannya sebagai
    ...
    keluaran transparan memerlukan `outputFormat` `png` atau `webp` dan
    model gambar OpenAI yang mendukung transparansi. OpenClaw merutekan permintaan
    latar belakang transparan `gpt-image-2` default ke `gpt-image-1.5`.
    ...
    ke field permintaan `background` OpenAI yang sama saat penyedia OpenAI
  ...
  <Accordion title="OpenRouter image models">
    ...
    petunjuk `aspectRatio` / `resolution` yang kompatibel dengan Gemini ke OpenRouter.
...
kontrol latar belakang eksplisit, jadi `background: "transparent"` dilaporkan
sebagai diabaikan untuk mereka.

## Terkait

- [Ikhtisar alat](/id/tools) - semua alat agen yang tersedia
- [ComfyUI](/id/providers/comfy) - setup alur kerja ComfyUI lokal dan Comfy Cloud
...
- [xAI](/id/providers/xai) - setup gambar Grok, video, pencarian, eksekusi kode, dan TTS
