---
read_when:
    - Mengaktifkan teks-ke-suara untuk balasan
...
x-i18n:
    generated_at: "2026-05-02T22:23:41Z"
    model: gpt-5.5
    provider: openai
...
lampiran audio di tempat lain, serta stream PCM/Ulaw untuk telefoni dan Talk.
...
Tool agen `tts` bawaan hanya untuk intent eksplisit: obrolan biasa tetap berupa
...
| ----------------- | ---------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------- |
| **Azure Speech**  | `AZURE_SPEECH_KEY` + `AZURE_SPEECH_REGION` (juga `AZURE_SPEECH_API_KEY`, `SPEECH_KEY`, `SPEECH_REGION`)          | Output pesan suara Ogg/Opus native dan telefoni.                        |
...
| **Gradium**       | `GRADIUM_API_KEY`                                                                                                | Output pesan suara dan telefoni.                                        |
| **Inworld**       | `INWORLD_API_KEY`                                                                                                | API TTS streaming. Pesan suara Opus native dan telefoni PCM.            |
...
| **xAI**           | `XAI_API_KEY`                                                                                                    | TTS batch xAI. Pesan suara Opus native **tidak** didukung.             |
...
Untuk menyematkan persona per agen, atur `agents.list[].tts.persona` bersama konfigurasi
...
3. override kanal, ketika kanal mendukung `channels.<channel>.tts`
4. override akun, ketika kanal meneruskan `channels.<channel>.accounts.<id>.tts`
5. preferensi `/tts` lokal untuk host ini
6. direktif inline `[[tts:...]]` ketika [override model](#model-driven-directives) diaktifkan

Override kanal dan akun menggunakan bentuk yang sama seperti `messages.tts` dan
melakukan deep-merge di atas lapisan sebelumnya, sehingga kredensial penyedia
bersama dapat tetap berada di `messages.tts` sementara kanal atau akun bot hanya
...
di berbagai penyedia. Persona dapat memprioritaskan satu penyedia, menentukan maksud
prompt yang netral penyedia, dan membawa binding khusus penyedia untuk suara, model,
template prompt, seed, dan pengaturan suara.
...
Pemilihan penyedia berjalan dengan yang eksplisit lebih dahulu:
...
4. Override direktif TTS yang dikeluarkan model dan diizinkan

### Cara penyedia menggunakan prompt persona

Kolom prompt persona (`profile`, `scene`, `sampleContext`, `style`, `accent`,
`pacing`, `constraints`) bersifat **netral penyedia**. Setiap penyedia menentukan
cara menggunakannya:
...
| `provider-defaults` | Persona dihilangkan dari persiapan prompt untuk percobaan itu; penyedia menggunakan default netralnya sementara fallback ke penyedia lain berlanjut. |
...
Secara default, asisten **dapat** mengeluarkan direktif `[[tts:...]]` untuk
meng-override suara, model, atau kecepatan untuk satu balasan, plus blok opsional
...
kanal melihatnya, bahkan ketika terpisah di beberapa blok yang bersebelahan.

`provider=...` diabaikan kecuali `modelOverrides.allowProvider: true`. Ketika
balasan mendeklarasikan `provider=...`, kunci lain dalam direktif tersebut diparse
...
**Izinkan penggantian penyedia sambil tetap mempertahankan knob lain yang dapat dikonfigurasi:**
...
- `/tts chat on|off|default` menulis override auto-TTS cakupan sesi untuk chat saat ini.
...
- `/tts audio` membuat balasan audio sekali jalan (tidak **mengaktifkan** TTS).
...
- `/status` menampilkan mode TTS aktif plus penyedia, model, suara, dan metadata endpoint kustom tersanitasi yang dikonfigurasi ketika TTS diaktifkan.
...
Ini meng-override konfigurasi efektif dari `messages.tts` plus blok
...
apakah TTS bergaya suara harus meminta target native `voice-note` kepada penyedia atau
mempertahankan sintesis `audio-file` normal dan hanya menandai output kompatibel untuk
pengiriman suara.
...
- **Feishu / WhatsApp**: ketika balasan catatan suara dibuat sebagai MP3/WebM/WAV/M4A
  atau file lain yang kemungkinan audio, Plugin saluran mentranskodenya ke Ogg/Opus
...
- **MiniMax**: MP3 (model `speech-2.8-hd`, laju sampel 32kHz) untuk lampiran audio normal. Untuk target catatan suara yang dinyatakan oleh saluran, OpenClaw mentranskode MP3 MiniMax ke Opus 48kHz dengan `ffmpeg` sebelum pengiriman ketika saluran menyatakan dukungan transkode.
- **Xiaomi MiMo**: MP3 secara default, atau WAV ketika dikonfigurasi. Untuk target catatan suara yang dinyatakan oleh saluran, OpenClaw mentranskode output Xiaomi ke Opus 48kHz dengan `ffmpeg` sebelum pengiriman ketika saluran menyatakan dukungan transkode.
...
Format output OpenAI/ElevenLabs ditetapkan per saluran (lihat di atas).
...
- Dalam `mode: "final"`, tetap mengirim TTS hanya-audio untuk balasan final yang di-stream
...
Jika balasan melebihi `maxLength` dan ringkasan mati (atau tidak ada kunci API untuk
...
## Format output menurut saluran
  ...
  | Target                                | Format                                                                                                                                   |
  | ------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------- |
  | Feishu / Matrix / Telegram / WhatsApp | Balasan catatan suara lebih disarankan memakai **Opus** (`opus_48000_64` dari ElevenLabs, `opus` dari OpenAI). 48 kHz / 64 kbps menyeimbangkan kejernihan dan ukuran. |
  | Channel lain                          | **MP3** (`mp3_44100_128` dari ElevenLabs, `mp3` dari OpenAI). 44,1 kHz / 128 kbps default untuk ucapan.                                  |
  ...
  - **Transcoding Feishu / WhatsApp:** Saat balasan catatan suara masuk sebagai MP3/WebM/WAV/M4A, Plugin channel melakukan transcoding ke Ogg/Opus 48 kHz dengan `ffmpeg`. WhatsApp mengirim melalui Baileys dengan `ptt: true` dan `audio/ogg; codecs=opus`. Jika konversi gagal: Feishu beralih ke melampirkan file asli; pengiriman WhatsApp gagal alih-alih memposting payload PTT yang tidak kompatibel.
  - **MiniMax / Xiaomi MiMo:** MP3 default (32 kHz untuk MiniMax `speech-2.8-hd`); ditranscode ke Opus 48 kHz untuk target catatan suara melalui `ffmpeg`.
  ...
  - **Google Gemini:** Mengembalikan PCM mentah 24 kHz. OpenClaw membungkusnya sebagai WAV untuk lampiran, melakukan transcoding ke Opus 48 kHz untuk target catatan suara, dan mengembalikan PCM secara langsung untuk Talk/telefoni.
  ...
  - **Microsoft:** Menggunakan `microsoft.outputFormat` (default `audio-24khz-48kbitrate-mono-mp3`). `sendVoice` Telegram menerima OGG/MP3/M4A; gunakan OpenAI/ElevenLabs jika Anda membutuhkan pesan suara Opus yang terjamin. Jika format Microsoft yang dikonfigurasi gagal, OpenClaw mencoba lagi dengan MP3.

  Format keluaran OpenAI dan ElevenLabs ditetapkan per channel seperti tercantum di atas.
  ...
  <Accordion title="messages.tts.* tingkat atas">
    <ParamField path="auto" type='"off" | "always" | "inbound" | "tagged"'>
      Mode Auto-TTS. `inbound` hanya mengirim audio setelah pesan suara masuk; `tagged` hanya mengirim audio saat balasan menyertakan direktif `[[tts:...]]` atau blok `[[tts:text]]`.
    </ParamField>
    ...
    <ParamField path="provider" type="string">
      ID penyedia ucapan. Jika tidak disetel, OpenClaw menggunakan penyedia pertama yang dikonfigurasi dalam urutan pemilihan otomatis registry. `provider: "edge"` lama ditulis ulang menjadi `"microsoft"` oleh `openclaw doctor --fix`.
    </ParamField>
    ...
    <ParamField path="modelOverrides" type="object">
      Mengizinkan model mengeluarkan direktif TTS. `enabled` default ke `true`; `allowProvider` default ke `false`.
    </ParamField>
    <ParamField path="providers.<id>" type="object">
      Pengaturan milik penyedia yang di-key oleh ID penyedia ucapan. Blok langsung lama (`messages.tts.openai`, `.elevenlabs`, `.microsoft`, `.edge`) ditulis ulang oleh `openclaw doctor --fix`; commit hanya `messages.tts.providers.<id>`.
    </ParamField>
    ...
    <ParamField path="prefsPath" type="string">
      Mengganti path JSON prefs lokal (penyedia/batas/ringkasan). Default `~/.openclaw/settings/tts.json`.
    </ParamField>
  </Accordion>
  ...
  <Accordion title="Azure Speech">
    ...
    <ParamField path="voiceNoteOutputFormat" type="string">Azure `X-Microsoft-OutputFormat` untuk keluaran catatan suara. Default `ogg-24khz-16bit-mono-opus`.</ParamField>
  </Accordion>
  ...
  <Accordion title="Google Gemini">
    ...
    <ParamField path="personaPrompt" type="string">Teks prompt persona tambahan khusus Google yang ditambahkan ke Catatan Sutradara template.</ParamField>
  ...
  <Accordion title="Microsoft (tanpa API key)">
    ...
    <ParamField path="timeoutMs" type="number">Override timeout permintaan (md).</ParamField>
  ...
  <Accordion title="OpenAI">
    ...
    <ParamField path="instructions" type="string">Field `instructions` OpenAI eksplisit. Ketika diatur, field prompt persona **tidak** dipetakan otomatis.</ParamField>
    <ParamField path="extraBody / extra_body" type="Record<string, unknown>">Field JSON tambahan yang digabungkan ke body permintaan `/audio/speech` setelah field TTS OpenAI yang dihasilkan. Gunakan ini untuk endpoint yang kompatibel dengan OpenAI seperti Kokoro yang memerlukan key khusus penyedia seperti `lang`; key prototype yang tidak aman diabaikan.</ParamField>
    <ParamField path="baseUrl" type="string">
      Override endpoint TTS OpenAI. Urutan resolusi: konfigurasi → `OPENAI_TTS_BASE_URL` → `https://api.openai.com/v1`. Nilai non-default diperlakukan sebagai endpoint TTS yang kompatibel dengan OpenAI, sehingga nama model dan suara kustom diterima.
    </ParamField>
  </Accordion>
  ...
  <Accordion title="Volcengine (BytePlus Seed Speech)">
    ...
    <ParamField path="appId / token / cluster" type="string" deprecated>Field legacy Volcengine Speech Console. Env: `VOLCENGINE_TTS_APPID`, `VOLCENGINE_TTS_TOKEN`, `VOLCENGINE_TTS_CLUSTER` (default `volcano_tts`).</ParamField>
  </Accordion>
...
WhatsApp dapat mentranskode output TTS non-Opus pada jalur ini ketika `ffmpeg`
tersedia.
...
klien tidak secara konsisten merender keterangan pada catatan suara.

Alat ini menerima field `channel` dan `timeoutMs` opsional; `timeoutMs` adalah
timeout permintaan penyedia per panggilan dalam milidetik.
