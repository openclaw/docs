...
title: Alat eksekusi
x-i18n:
    generated_at: "2026-05-10T19:55:01Z"
    model: gpt-5.5
    provider: openai
...
Jalankan perintah shell di workspace. `exec` adalah permukaan shell yang bersifat memutasi: perintah dapat membuat, mengedit, atau menghapus file di mana pun host atau filesystem sandbox yang dipilih mengizinkan. Menonaktifkan alat filesystem OpenClaw seperti `write`, `edit`, atau `apply_patch` tidak membuat `exec` menjadi baca-saja.
...
Jadikan perintah otomatis background setelah penundaan ini (ms).
</ParamField>
...
Jalankan dalam pseudo-terminal jika tersedia. Gunakan untuk CLI khusus TTY, agen pengodean, dan UI terminal.
</ParamField>
...
- Tanpa konfigurasi tambahan, `host=auto` tetap "langsung berfungsi": tanpa sandbox berarti diselesaikan menjadi `gateway`; sandbox live berarti tetap berada di sandbox.
...
- `timeout` berlaku untuk eksekusi foreground, background, `yieldMs`, gateway, sandbox, dan Node `system.run`. Jika dihilangkan, OpenClaw menggunakan `tools.exec.timeoutSec`; `timeout: 0` eksplisit menonaktifkan timeout proses exec untuk panggilan tersebut.
- Pada host non-Windows, exec menggunakan `SHELL` ketika ditetapkan; jika `SHELL` adalah `fish`, ia lebih memilih `bash` (atau `sh`)
...
- OpenClaw menetapkan `OPENCLAW_SHELL=exec` dalam lingkungan perintah yang di-spawn (termasuk eksekusi PTY dan sandbox) sehingga aturan shell/profil dapat mendeteksi konteks alat exec.
- `openclaw channels login` diblokir dari `exec` karena ini adalah alur auth channel interaktif; jalankan di terminal pada host gateway, atau gunakan alat login native channel dari chat jika ada.
...
- Untuk pekerjaan berjalan lama yang dimulai sekarang, mulai sekali dan andalkan wake penyelesaian
  otomatis saat diaktifkan dan perintah memancarkan output atau gagal.
  Gunakan `process` untuk log, status, input, atau intervensi; jangan meniru
  penjadwalan dengan sleep loop, timeout loop, atau polling berulang.
...
- `tools.exec.approvalRunningNoticeMs` (default: 10000): pancarkan satu pemberitahuan "berjalan" ketika exec yang dibatasi persetujuan berjalan lebih lama dari ini (0 menonaktifkan).
...
- Dalam mode `security=full` plus `ask=off`, exec host mengikuti kebijakan yang dikonfigurasi secara langsung; tidak ada prefilter heuristik pengaburan perintah tambahan atau lapisan penolakan preflight skrip.
- `tools.exec.node` (default: tidak ditetapkan)
- `tools.exec.strictInlineEval` (default: false): ketika true, bentuk eval interpreter inline seperti `python -c`, `node -e`, `ruby -e`, `perl -e`, `php -r`, `lua -e`, dan `osascript -e` selalu memerlukan persetujuan eksplisit. `allow-always` tetap dapat mempertahankan pemanggilan interpreter/skrip yang aman, tetapi bentuk inline-eval tetap memunculkan prompt setiap kali.
- `tools.exec.pathPrepend`: daftar direktori yang akan ditambahkan di awal `PATH` untuk run exec (hanya gateway + sandbox).
...
- `host=node`: hanya override env yang tidak diblokir yang Anda berikan yang dikirim ke Node. Override `env.PATH`
  ...
  konfigurasi lingkungan layanan host Node (systemd/launchd) atau instal alat di lokasi standar.
...
UI kontrol: tab Nodes menyertakan panel kecil "Binding Node exec" untuk pengaturan yang sama.
...
Ini hanya memperbarui **state sesi** dan tidak menulis konfigurasi. Untuk menonaktifkan exec secara keras, tolak melalui
...
Agen yang di-sandbox dapat memerlukan persetujuan per permintaan sebelum `exec` berjalan pada host gateway atau Node.
...
berjalan setelah `tools.exec.approvalRunningNoticeMs`, satu pemberitahuan `Exec running` dipancarkan.
...
alat secara eksplisit menyatakan persetujuan chat tidak tersedia atau persetujuan manual adalah
satu-satunya jalur.

## Allowlist + safe bins

Penegakan allowlist manual mencocokkan glob path biner yang diselesaikan dan glob nama perintah
polos. Nama polos hanya cocok dengan perintah yang dipanggil melalui PATH, sehingga `rg` dapat mencocokkan
...
ada dalam allowlist atau merupakan safe bin. Chaining (`;`, `&&`, `||`) dan redirection
...
- `tools.exec.safeBinTrustedDirs`: direktori tepercaya tambahan eksplisit untuk path executable safe-bin.
- `tools.exec.safeBinProfiles`: kebijakan argv eksplisit untuk safe bins kustom.
...
Jika Anda secara eksplisit mengizinkan interpreter dalam allowlist, aktifkan `tools.exec.strictInlineEval` agar bentuk evaluasi kode inline tetap memerlukan persetujuan baru.

Untuk detail kebijakan lengkap dan contoh, lihat [Persetujuan exec](/id/tools/exec-approvals-advanced#safe-bins-stdin-only) dan [Bin aman versus allowlist](/id/tools/exec-approvals-advanced#safe-bins-versus-allowlist).
...
Polling ditujukan untuk status sesuai permintaan, bukan loop tunggu. Jika bangun otomatis saat selesai
diaktifkan, perintah dapat membangunkan sesi ketika mengeluarkan output atau gagal.
...
- `tools.exec.applyPatch.enabled` default-nya `true`; atur ke `false` untuk menonaktifkan alat bagi model OpenAI.
- `tools.exec.applyPatch.workspaceOnly` default-nya `true` (terbatas dalam workspace). Atur ke `false` hanya jika Anda sengaja ingin `apply_patch` menulis/menghapus di luar direktori workspace.
