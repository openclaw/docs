...
Alat `web_search` mencari di web menggunakan penyedia yang Anda konfigurasikan dan
...
`web_fetch` untuk pengambilan URL ringan. Dalam fase ini, `web_fetch` tetap
...
<Steps>
  <Step title="Pilih penyedia">
    Pilih penyedia dan selesaikan penyiapan apa pun yang diperlukan. Beberapa penyedia
  ...
  <Step title="Konfigurasi">
    ...
    (misalnya `BRAVE_API_KEY`) dan melewati langkah ini untuk penyedia yang
    didukung API.
  </Step>
  <Step title="Gunakan">
...
<CardGroup cols={2}>
  ...
  <Card title="SearXNG" icon="server" href="/id/tools/searxng-search">
    Meta-pencarian yang dihosting sendiri. Tidak memerlukan kunci API. Menggabungkan Google, Bing, DuckDuckGo, dan lainnya.
  </Card>
...
penyedia tersebut. Tujuan privat, loopback, link-local, dan metadata lainnya tetap diblokir.
...
11. **Ollama Web Search** -- fallback bebas kunci melalui host Ollama lokal yang Anda konfigurasi saat dapat dijangkau dan sudah masuk dengan `ollama signin`; dapat menggunakan kembali auth bearer penyedia Ollama saat host membutuhkannya, dan dapat memanggil pencarian langsung `https://ollama.com` saat dikonfigurasi dengan `OLLAMA_API_KEY` (urutan 110)
12. **SearXNG** -- `SEARXNG_BASE_URL` atau `plugins.entries.searxng.config.webSearch.baseUrl` (urutan 200)

Jika tidak ada penyedia yang terdeteksi, ini melakukan fallback ke Brave (Anda akan mendapatkan kesalahan
kunci hilang yang meminta Anda mengonfigurasikannya).

<Note>
  Semua kolom kunci penyedia mendukung objek SecretRef. SecretRef dengan cakupan Plugin
  di bawah `plugins.entries.<plugin>.config.webSearch.apiKey` diselesaikan untuk
  penyedia pencarian web bawaan yang didukung API, termasuk Brave, Exa, Firecrawl,
  ...
  menyimpan beberapa penyedia yang dikonfigurasi tanpa membayar biaya penyelesaian untuk
...
yang dideklarasikan oleh manifes Plugin bawaan dan terinstal, ditambah Plugin
penyedia yang diketahui dapat diinstal. Salah ketik seperti `"brvae"` akan membuat
validasi konfigurasi gagal, bukan diam-diam fallback ke deteksi otomatis. Jika penyedia
yang dikonfigurasi dikenal tetapi Plugin pemiliknya tidak tersedia, OpenClaw menjaga startup
tetap tangguh dan melaporkan peringatan agar Anda dapat menjalankan `openclaw doctor --fix`
untuk menginstal atau mengaktifkan Plugin tersebut. Perilaku peringatan yang sama berlaku untuk
bukti Plugin yang usang, seperti blok `plugins.entries.<plugin>` yang tersisa setelah
menghapus instalasi Plugin pihak ketiga.
...
- `web_fetch` non-sandbox dapat menggunakan penyedia Plugin terinstal yang mendeklarasikan
  `contracts.webFetchProviders`; fetch bersandbox tetap hanya bawaan
...
<Warning>
  ...
  bentuk jawaban yang grounded. Gemini mendukung `freshness`, `date_after`, dan
...
<Note>
  ...
  balasan, bookmark, atau tampilan, pilih lookup tertarget untuk URL postingan
  atau ID status yang tepat. Pencarian kata kunci luas mungkin menemukan postingan yang tepat tetapi mengembalikan
...
`plugins.entries.xai.config.xSearch.baseUrl` diatur. Jika bidang tersebut dihilangkan,
