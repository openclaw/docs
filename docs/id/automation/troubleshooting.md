---
summary: Alihkan ke /automation/cron-jobs
title: Pemecahan masalah otomatisasi
x-i18n:
    generated_at: "2026-04-24T08:57:18Z"
    model: gpt-5.4
    provider: openai
    source_hash: e7784858f6d2594a1d6019435b2a5a1647e12f5de6329198db9539e325f73737
    source_path: automation/troubleshooting.md
    workflow: 15
    postprocess_version: locale-links-v1
---

Halaman ini dipindahkan ke [Scheduled Tasks](/id/automation/cron-jobs#troubleshooting). Lihat [Scheduled Tasks](/id/automation/cron-jobs#troubleshooting) untuk dokumentasi pemecahan masalah.

## Terkait

- [Hooks](/id/automation/hooks)
- [Tugas latar belakang](/id/automation/tasks)
- [Pemecahan masalah Gateway](/id/gateway/troubleshooting)
