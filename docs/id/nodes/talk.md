...
summary: 'Mode bicara: percakapan suara berkelanjutan di seluruh STT/TTS lokal dan suara realtime'
title: Mode bicara
x-i18n:
    generated_at: "2026-07-02T22:49:38Z"
    model: gpt-5.5
...
- Bicara iOS menggunakan WebRTC milik klien untuk konfigurasi realtime OpenAI yang memilih `webrtc` atau menghilangkan transport. Konfigurasi realtime `gateway-relay`, `provider-websocket`, dan non-OpenAI yang eksplisit tetap berada pada relay milik Gateway; konfigurasi non-realtime menggunakan loop suara native.
...
- Bicara Android dapat ikut menggunakan sesi relay realtime milik Gateway dengan `talk.realtime.mode: "realtime"` dan `talk.realtime.transport: "gateway-relay"`. Jika tidak, ia tetap menggunakan pengenalan suara native, chat Gateway, dan `talk.speak`.
...
Bicara khusus transkripsi memancarkan envelope event Bicara umum yang sama seperti sesi realtime dan STT/TTS, tetapi menggunakan `mode: "transcription"` dan `brain: "none"`. Ini ditujukan untuk caption, dikte, dan penangkapan suara hanya-amati; catatan suara unggahan sekali pakai tetap menggunakan jalur media/audio.

## Perilaku (macOS)

- **Overlay selalu aktif** selama mode Bicara diaktifkan.
...
- `interruptOnSpeech`: true
- `silenceTimeoutMs`: saat tidak disetel, Bicara mempertahankan jendela jeda default platform sebelum mengirim transkrip (`700 ms on macOS and Android, 900 ms on iOS`)
...
- `realtime.providers.<provider>` menyimpan konfigurasi realtime milik penyedia. Browser hanya menerima kredensial sesi yang sementara atau dibatasi, tidak pernah kunci API standar.
...
- `realtime.brain`: `agent-consult` merutekan panggilan tool realtime melalui kebijakan Gateway; `direct-tools` adalah perilaku kompatibilitas direct-tool lama; `none` ditujukan untuk transkripsi atau orkestrasi eksternal.
...
- `realtime.instructions`: menambahkan instruksi sistem yang menghadap penyedia ke prompt realtime bawaan OpenClaw. Gunakan untuk gaya dan nada suara; OpenClaw mempertahankan panduan default `openclaw_agent_consult`.
- `talk.catalog` mengekspos mode valid, transport, strategi brain, format audio realtime, dan flag kapabilitas setiap penyedia agar klien Bicara pihak pertama dapat menghindari kombinasi yang tidak didukung.
- Penyedia transkripsi streaming ditemukan melalui `talk.catalog.transcription`. Relay Gateway saat ini menggunakan konfigurasi penyedia streaming Panggilan Suara hingga permukaan konfigurasi transkripsi Bicara khusus ditambahkan.
- `speechLocale`: id locale BCP 47 opsional untuk pengenalan suara Bicara di perangkat pada iOS/macOS. Biarkan tidak disetel untuk menggunakan default perangkat.
...
- Overlay:
  - **Mendengarkan**: cloud berdenyut sesuai level mikrofon
...
- Mikrofon manual berhenti saat aplikasi meninggalkan latar depan atau pengguna meninggalkan tab Suara.
- Mode Bicara terus berjalan hingga dimatikan atau node Android terputus, dan menggunakan tipe layanan latar depan mikrofon Android saat aktif.

## Catatan

- Memerlukan izin Ucapan + Mikrofon.
- Bicara native menggunakan sesi Gateway aktif dan hanya fallback ke polling riwayat saat event respons tidak tersedia.
...
- Bicara khusus transkripsi menggunakan `talk.session.create`, `talk.session.appendAudio`, `talk.session.cancelTurn`, dan `talk.session.close`; klien berlangganan ke `talk.event` untuk pembaruan transkrip parsial/final.
...
- Pemutaran MLX lokal macOS menggunakan helper `openclaw-mlx-tts` bawaan saat tersedia, atau executable pada `PATH`. Setel `OPENCLAW_MLX_TTS_BIN` agar menunjuk ke binary helper kustom selama pengembangan.
