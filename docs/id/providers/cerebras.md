...
title: Cerebras
x-i18n:
    generated_at: "2026-07-19T05:33:08Z"
    model: gpt-5.6
...
[Cerebras](https://www.cerebras.ai) menyediakan inferensi berkecepatan tinggi yang kompatibel dengan OpenAI pada perangkat keras inferensi khusus. Plugin ini menyertakan katalog statis berisi dua model (tanpa penemuan langsung).
...
```
  ...
  <Step title="Verifikasi ketersediaan model">
    ...
    Menampilkan kedua model statis. Jika `CEREBRAS_API_KEY` tidak dapat diselesaikan, `openclaw models status --json` melaporkan kredensial yang tidak tersedia di bawah `auth.unusableProfiles`.
...
Kedua model memiliki jendela konteks 128k dan maksimum 8.192 token keluaran.
...
Sebagian besar penyiapan hanya memerlukan kunci API. Gunakan konfigurasi `models.providers.cerebras` eksplisit untuk mengganti metadata model atau menjalankan dalam `mode: "merge"` terhadap katalog statis:
...
<Note>
Jika Gateway berjalan sebagai daemon (launchd, systemd, Docker), pastikan `CEREBRAS_API_KEY` tersedia bagi proses tersebut — misalnya di `~/.openclaw/.env` atau melalui `env.shellEnv`. Kunci yang hanya diekspor dalam shell interaktif tidak akan membantu layanan terkelola kecuali lingkungan diimpor secara terpisah.
</Note>
...
<CardGroup cols={2}>
  ...
  <Card title="Mode berpikir" href="/id/tools/thinking" icon="brain">
    Tingkat upaya penalaran untuk kedua model Cerebras yang mendukung penalaran.
  </Card>
