...
title: MiniMax
x-i18n:
    generated_at: "2026-07-12T14:37:17Z"
    model: gpt-5.6
...
---
  ...
  Plugin `minimax` bawaan mendaftarkan dua penyedia serta lima kapabilitas: percakapan, pembuatan gambar, pembuatan musik, pembuatan video, pemahaman gambar, ucapan (T2A v2), dan pencarian web.
  ...
  | ------------------------ | -------------------- | -------------------------------------------- |
  | `MiniMax-M3`             | Percakapan (penalaran) | Model penalaran terhosting default          |
  | `MiniMax-M2.7`           | Percakapan (penalaran) | Model penalaran terhosting sebelumnya       |
  | `MiniMax-M2.7-highspeed` | Percakapan (penalaran) | Tingkat penalaran M2.7 yang lebih cepat     |
  ...
  <Tab title="Kunci API">
    **Paling cocok untuk:** MiniMax terhosting dengan API yang kompatibel dengan Anthropic.
    ...
    <Warning>
    Endpoint streaming yang kompatibel dengan Anthropic milik MiniMax-M2.x memancarkan `reasoning_content` dalam potongan delta bergaya OpenAI, bukan blok pemikiran native Anthropic. Hal ini membocorkan penalaran internal ke keluaran yang terlihat jika pemikiran dibiarkan aktif secara implisit. OpenClaw menonaktifkan pemikiran M2.x secara default kecuali Anda secara eksplisit menetapkan `thinking` sendiri. MiniMax-M3 (dan M3.x yang kompatibel ke depan) dikecualikan: M3 memancarkan blok pemikiran Anthropic yang benar dan memerlukan pemikiran aktif untuk menghasilkan konten yang terlihat, sehingga OpenClaw mempertahankan M3 pada jalur pemikiran adaptif penyedia. Lihat bagian Default pemikiran di bawah Konfigurasi lanjutan berikut.
    </Warning>

    <Note>
    Penyiapan dengan kunci API menggunakan ID penyedia `minimax`. Referensi model mengikuti bentuk `minimax/MiniMax-M3`.
    </Note>
...
Plugin MiniMax mendaftarkan model `image-01` untuk alat `image_generate` pada `minimax` dan `minimax-portal`, dengan menggunakan kembali `MINIMAX_API_KEY` atau autentikasi OAuth yang sama seperti model teks.
...
- ID model bawaan: `speech-2.8-hd`, `speech-2.8-turbo`, `speech-2.6-hd`, `speech-2.6-turbo`, `speech-02-hd`, `speech-02-turbo`, `speech-01-hd`, `speech-01-turbo`, `speech-01-240228`
- Urutan resolusi autentikasi: `messages.tts.providers.minimax.apiKey`, lalu profil autentikasi OAuth/token `minimax-portal`, lalu kunci lingkungan Paket Token (`MINIMAX_OAUTH_TOKEN`, `MINIMAX_CODE_PLAN_KEY`, `MINIMAX_CODING_API_KEY`), kemudian `MINIMAX_API_KEY`
- Jika tidak ada host TTS yang dikonfigurasi, OpenClaw menggunakan kembali host OAuth `minimax-portal` yang dikonfigurasi dan menghapus akhiran jalur yang kompatibel dengan Anthropic seperti `/anthropic`
- Lampiran audio biasa tetap dalam format MP3. Target catatan suara (Feishu, Telegram, dan kanal lain yang meminta lampiran yang kompatibel dengan catatan suara) ditranskode dari MP3 MiniMax menjadi Opus 48 kHz dengan `ffmpeg`, karena misalnya API berkas Feishu/Lark hanya menerima `file_type: "opus"` untuk pesan audio native
- MiniMax T2A menerima nilai pecahan untuk `speed` dan `vol`, tetapi `pitch` dikirim sebagai bilangan bulat; OpenClaw memotong bagian pecahan nilai `pitch` sebelum permintaan API
...
| `messages.tts.providers.minimax.pitch`   |                        | `0`                           | Pergeseran nada berupa bilangan bulat, `-12..12`. |
...
- Kontrol perintah: `lyrics`, `instrumental`
- Format keluaran: `mp3`
...
- Mode: teks-ke-video dan alur referensi satu gambar
...
| ID penyedia      | Model gambar bawaan | Ekstraksi teks PDF |
| ---------------- | -------------------- | ------------------ |
| `minimax`        | `MiniMax-VL-01`      | `MiniMax-M2.7`     |
| `minimax-portal` | `MiniMax-VL-01`      | `MiniMax-M2.7`     |

Itulah sebabnya perutean media otomatis dapat menggunakan pemahaman gambar MiniMax meskipun katalog penyedia teks bawaan juga menyertakan referensi percakapan M3 berkemampuan gambar. Pemahaman PDF menggunakan `MiniMax-M2.7` hanya untuk ekstraksi teks; MiniMax tidak mendaftarkan jalur konversi PDF ke gambar.
...
- Alias variabel lingkungan yang diterima: `MINIMAX_CODING_API_KEY`, `MINIMAX_OAUTH_TOKEN`
...
<Note>
Lihat [Pencarian MiniMax](/id/tools/minimax-search) untuk konfigurasi dan penggunaan pencarian web selengkapnya.
</Note>
...
<AccordionGroup>
  <Accordion title="Opsi konfigurasi">
    ...
    | `agents.defaults.models` | Berikan alias pada model yang ingin Anda masukkan ke daftar yang diizinkan |
    | `models.mode` | Pertahankan `merge` jika Anda ingin menambahkan MiniMax bersama model bawaan |
  </Accordion>

  <Accordion title="Pengaturan bawaan penalaran">
    Pada `api: "anthropic-messages"`, OpenClaw menyisipkan `thinking: { type: "disabled" }` untuk model MiniMax M2.x kecuali pembungkus sebelumnya telah menetapkan bidang `thinking` dalam payload. Hal ini mencegah endpoint streaming M2.x menghasilkan `reasoning_content` dalam potongan delta bergaya OpenAI, yang dapat membocorkan penalaran internal ke keluaran yang terlihat.

    MiniMax-M3 (dan M3.x) dikecualikan: M3 mengembalikan larik `content` kosong dengan `stop_reason: "end_turn"` ketika penalaran dinonaktifkan, sehingga OpenClaw menghapus pengaturan bawaan nonaktif implisit untuk M3 dan, ketika tingkat penalaran ditetapkan, memaksakan `thinking: { type: "adaptive" }` sebagai gantinya.

    Tingkat penalaran yang tersedia untuk setiap keluarga model:

    | Keluarga model | Tingkat                                   | Bawaan     |
    | -------------- | ----------------------------------------- | ---------- |
  ...
  <Accordion title="Mode cepat">
    `/fast on` atau `params.fastMode: true` mengubah `MiniMax-M2.7` menjadi `MiniMax-M2.7-highspeed` pada jalur aliran yang kompatibel dengan Anthropic (`api: "anthropic-messages"`, penyedia `minimax` atau `minimax-portal`).
  </Accordion>

  <Accordion title="Contoh fallback">
    **Paling sesuai untuk:** mempertahankan model generasi terbaru dan terkuat Anda sebagai model utama, lalu beralih ke MiniMax M2.7 jika gagal. Contoh di bawah menggunakan Opus sebagai model utama konkret; ganti dengan model utama generasi terbaru pilihan Anda.
  ...
  <Accordion title="Detail penggunaan Coding Plan">
    ...
    - Polling penggunaan mengambil host dari `models.providers.minimax-portal.baseUrl` atau `models.providers.minimax.baseUrl` ketika dikonfigurasi, sehingga penyiapan global yang menggunakan `https://api.minimax.io/anthropic` melakukan polling ke `api.minimax.io`. URL dasar yang tidak ada atau salah format mempertahankan fallback CN demi kompatibilitas.
    - OpenClaw menormalisasi penggunaan paket pengodean MiniMax ke tampilan `% tersisa` yang sama seperti penyedia lain. Bidang mentah `usage_percent` / `usagePercent` milik MiniMax menunjukkan kuota tersisa, bukan kuota terpakai, sehingga OpenClaw membalikkannya. Bidang berbasis jumlah diutamakan jika tersedia.
    - Ketika API mengembalikan `model_remains`, OpenClaw mengutamakan entri model percakapan, memperoleh label jendela dari `start_time` / `end_time` jika diperlukan, dan menyertakan nama model terpilih dalam label paket agar jendela paket pengodean lebih mudah dibedakan.
    - Snapshot penggunaan memperlakukan `minimax`, `minimax-cn`, `minimax-portal`, dan `minimax-portal-cn` sebagai permukaan kuota MiniMax yang sama, serta mengutamakan OAuth MiniMax tersimpan sebelum beralih ke variabel lingkungan kunci Coding Plan.
...
- Model percakapan bawaan: `MiniMax-M3`. Model percakapan alternatif: `MiniMax-M2.7`, `MiniMax-M2.7-highspeed`
...
- Perbarui nilai harga dalam `models.json` jika Anda memerlukan pelacakan biaya yang tepat
...
<AccordionGroup>
  <Accordion title='"Model tidak dikenal: minimax/MiniMax-M3"'>
    Ini biasanya berarti **penyedia MiniMax belum dikonfigurasi** (tidak ada entri penyedia yang cocok serta tidak ditemukan profil autentikasi MiniMax atau kunci variabel lingkungan). Perbaiki dengan:
    ...
    Pastikan ID model **peka terhadap huruf besar-kecil**:
