---
read_when:
    - Bekerja pada fitur channel Discord
summary: Status dukungan, kapabilitas, dan konfigurasi bot Discord
title: Discord
x-i18n:
    generated_at: "2026-06-30T14:24:47Z"
    model: gpt-5.5
...
Anda perlu membuat aplikasi baru dengan bot, menambahkan bot ke server Anda, dan memasangkannya ke OpenClaw. Kami menyarankan agar Anda menambahkan bot ke server privat milik Anda sendiri. Jika belum memilikinya, [buat terlebih dahulu](https://support.discord.com/hc/en-us/articles/204849977-How-do-I-create-a-server) (pilih **Create My Own > For me and my friends**).
...
<Steps>
  ...
  <Step title="Aktifkan Developer Mode dan kumpulkan ID Anda">
    ...
    1. Klik **User Settings** (ikon roda gigi di samping avatar Anda) → Gulir ke **Developer** di bilah sisi → aktifkan **Developer Mode**
  ...
  <Step title="Konfigurasikan OpenClaw dan pasangkan">
    ...
    <Tabs>
      ...
      <Tab title="CLI / konfigurasi">
        Jika Anda lebih memilih konfigurasi berbasis file, atur:
...
```
        ...
        Untuk beberapa bot Discord, simpan setiap token bot dan ID aplikasi di bawah akunnya. `channels.discord.applicationId` tingkat atas diwarisi oleh akun, jadi atur di sana hanya ketika setiap akun harus menggunakan ID aplikasi yang sama.
...
Jika dua akun Discord yang diaktifkan diselesaikan ke token bot yang sama, OpenClaw hanya memulai satu monitor Gateway untuk token tersebut. Token yang bersumber dari config mengalahkan fallback env default; jika tidak, akun aktif pertama menang dan akun duplikat dilaporkan dinonaktifkan.
...
Setelah DM berfungsi, Anda dapat menyiapkan server Discord sebagai workspace penuh tempat setiap kanal mendapatkan sesi agennya sendiri dengan konteksnya sendiri. Ini disarankan untuk server privat yang hanya berisi Anda dan bot Anda.
...
<Steps>
  <Step title="Tambahkan server Anda ke allowlist guild">
    ...
    <Tabs>
      <Tab title="Tanya agen Anda">
        > "Tambahkan Discord Server ID saya `<server_id>` ke allowlist guild"
      </Tab>
      <Tab title="Config">
...
```
  ...
  <Step title="Izinkan respons tanpa @mention">
    ...
    Di kanal guild, balasan normal diposting otomatis secara default. Untuk ruang bersama yang selalu aktif, pilih `messages.groupChat.visibleReplies: "message_tool"` agar agen dapat memantau diam-diam dan hanya memposting saat memutuskan bahwa balasan kanal berguna. Ini bekerja paling baik dengan model generasi terbaru yang andal menggunakan alat, seperti GPT 5.5. Event ruang ambient tetap senyap kecuali alat mengirim. Lihat [Event ruang ambient](/id/channels/ambient-room-events) untuk konfigurasi mode pantau diam-diam lengkap.

    Jika Discord menampilkan indikator mengetik dan log menunjukkan penggunaan token tetapi tidak ada pesan yang diposting, periksa apakah turn dikonfigurasi sebagai event ruang ambient atau memilih balasan terlihat berbasis alat pesan.
...
```
        ...
        Untuk mewajibkan pengiriman alat pesan bagi balasan grup/kanal yang terlihat, atur `messages.groupChat.visibleReplies: "message_tool"`.
  ...
  <Step title="Rencanakan memori di kanal guild">
    ...
    <Tabs>
      <Tab title="Tanya agen Anda">
        > "Saat saya bertanya di kanal Discord, gunakan memory_search atau memory_get jika Anda membutuhkan konteks jangka panjang dari MEMORY.md."
      </Tab>
      <Tab title="Manual">
...
Sekarang buat beberapa kanal di server Discord Anda dan mulai chat. Agen Anda dapat melihat nama kanal, dan setiap kanal mendapatkan sesi terisolasinya sendiri — jadi Anda dapat menyiapkan `#coding`, `#home`, `#research`, atau apa pun yang sesuai dengan alur kerja Anda.
...
- Pengiriman pengumuman cron/heartbeat teks saja ke Discord menggunakan jawaban akhir yang terlihat oleh asisten satu kali. Payload media dan komponen terstruktur tetap berupa multi-pesan ketika agen memancarkan beberapa payload yang dapat dikirim.
...
Callback komponen kedaluwarsa setelah 30 menit secara default. Atur `channels.discord.agentComponents.ttlMs` untuk mengubah masa hidup registry callback tersebut untuk akun Discord default, atau `channels.discord.accounts.<accountId>.agentComponents.ttlMs` untuk menimpa satu akun dalam setup multi-akun. Nilainya dalam milidetik, harus berupa bilangan bulat positif, dan dibatasi pada `86400000` (24 jam). TTL yang lebih panjang berguna untuk alur kerja peninjauan atau persetujuan yang membutuhkan tombol tetap dapat digunakan, tetapi juga memperpanjang jendela saat pesan Discord lama masih dapat memicu tindakan. Pilih TTL tersingkat yang cocok dengan alur kerja, dan pertahankan default ketika callback yang kedaluwarsa akan mengejutkan.

Perintah slash `/model` dan `/models` membuka pemilih model interaktif dengan dropdown provider, model, dan runtime yang kompatibel ditambah langkah Submit. `/models add` sudah tidak digunakan lagi dan sekarang mengembalikan pesan penghentian penggunaan alih-alih mendaftarkan model dari chat. Balasan pemilih bersifat ephemeral dan hanya pengguna yang memanggilnya yang dapat menggunakannya. Menu pilih Discord dibatasi hingga 25 opsi, jadi tambahkan entri `provider/*` ke `agents.defaults.models` ketika Anda ingin pemilih menampilkan model yang ditemukan secara dinamis hanya untuk provider yang dipilih seperti `openai` atau `vllm`.
...
<Tabs>
  <Tab title="DM policy">
    ...
    ID numerik polos biasanya diselesaikan sebagai ID channel ketika default channel aktif, tetapi ID yang tercantum dalam DM efektif akun `allowFrom` diperlakukan sebagai target DM pengguna untuk kompatibilitas.

  </Tab>

  <Tab title="Access groups">
    DM Discord dan otorisasi perintah teks dapat menggunakan entri `accessGroup:<name>` dinamis di `channels.discord.allowFrom`.

    Nama grup akses dibagikan lintas channel pesan. Gunakan `type: "message.senders"` untuk grup statis yang anggotanya dinyatakan dalam sintaks `allowFrom` normal masing-masing channel, atau `type: "discord.channelAudience"` ketika audiens `ViewChannel` saat ini dari sebuah channel Discord harus menentukan keanggotaan secara dinamis. Perilaku grup akses bersama didokumentasikan di sini: [Grup akses](/id/channels/access-groups).
...
```
    ...
    Pencarian gagal tertutup. Jika Discord mengembalikan `Missing Access`, pencarian anggota gagal, atau channel milik guild lain, pengirim DM diperlakukan sebagai tidak berwenang.
  ...
  <Tab title="Guild policy">
    ...
    - jika sebuah guild memiliki `channels` yang dikonfigurasi, channel yang tidak terdaftar ditolak
    - jika sebuah guild tidak memiliki blok `channels`, semua channel di guild yang masuk allowlist tersebut diizinkan
...
- Perintah mungkin masih terlihat di UI Discord untuk pengguna yang tidak berwenang; eksekusi tetap menerapkan autentikasi OpenClaw dan mengembalikan "not authorized".
...
<AccordionGroup>
  <Accordion title="Tag balasan dan balasan native">
    ...
    peristiwa inbound adalah batch yang didebounce dari beberapa pesan. Ini berguna
  ...
  <Accordion title="Pratinjau tautan">
    Discord menghasilkan embed tautan kaya untuk URL secara default. OpenClaw menekan embed yang dihasilkan itu pada pesan Discord outbound secara default, sehingga URL yang dikirim agen tetap berupa tautan biasa kecuali Anda memilih ikut serta:
...
```
    ...
    - `block` memancarkan potongan seukuran draf (gunakan `draftChunk` untuk menyetel ukuran dan breakpoint, dibatasi ke `textChunkLimit`).
    ...
    - Baris alat/progres dirender sebagai emoji + judul + detail ringkas bila tersedia, misalnya `🛠️ Bash: run tests` atau `🔎 Web Search: for "query"`.
    - `streaming.progress.commentary` (default `false`) memilih untuk menyertakan teks komentar/preambule asisten dalam draf progres sementara. Komentar dibersihkan sebelum ditampilkan, tetap sementara, dan tidak mengubah pengiriman jawaban final.
    ...
    Streaming pratinjau hanya teks; balasan media kembali ke pengiriman normal. Saat streaming `block` diaktifkan secara eksplisit, OpenClaw melewati stream pratinjau untuk menghindari streaming ganda.
  ...
  <Accordion title="Riwayat, konteks, dan perilaku thread">
    ...
    - Thread Discord dirutekan sebagai sesi kanal dan mewarisi konfigurasi kanal induk kecuali di-override.
    - Sesi thread mewarisi pilihan `/model` level sesi milik kanal induk sebagai fallback khusus model; pilihan `/model` lokal thread tetap diutamakan dan riwayat transkrip induk tidak disalin kecuali pewarisan transkrip diaktifkan.
    ...
    Topik kanal disisipkan sebagai konteks **tidak tepercaya**. Allowlist membatasi siapa yang dapat memicu agen, bukan batas penyuntingan konteks tambahan penuh.
...
```
    ...
    Lihat [Sub-agents](/id/tools/subagents), [ACP Agents](/id/tools/acp-agents), dan [Referensi Konfigurasi](/id/gateway/configuration-reference).

  </Accordion>

  <Accordion title="Binding kanal ACP persisten">
    Untuk workspace ACP "always-on" yang stabil, konfigurasikan binding ACP bertipe level atas yang menargetkan percakapan Discord.
...
```
    ...
    - Dalam kanal atau thread yang terikat, `/new` dan `/reset` mereset sesi ACP yang sama di tempat. Binding thread sementara dapat meng-override resolusi target saat aktif.
  ...
  <Accordion title="Penulisan konfigurasi">
    Penulisan konfigurasi yang diinisiasi kanal diaktifkan secara default.
...
```
    ...
    Contoh activity (status kustom adalah tipe activity bawaan):
...
```
    ...
    - 2: Mendengarkan
    - 3: Menonton
    - 4: Kustom (menggunakan teks activity sebagai status state; emoji bersifat opsional)
    - 5: Berkompetisi
...
```
    ...
    Auto presence memetakan ketersediaan runtime ke status Discord: healthy => online, degraded atau unknown => idle, exhausted atau unavailable => dnd. Penggantian teks opsional:
  ...
  <Accordion title="Persetujuan di Discord">
    ...
    Discord mengaktifkan otomatis persetujuan exec native ketika `enabled` tidak ditetapkan atau `"auto"` dan setidaknya satu approver dapat di-resolve, baik dari `execApprovals.approvers` maupun dari `commands.ownerAllowFrom`. Discord tidak menyimpulkan approver exec dari channel `allowFrom`, `dm.allowFrom` legacy, atau direct-message `defaultTo`. Tetapkan `enabled: false` untuk menonaktifkan Discord sebagai klien persetujuan native secara eksplisit.
    ...
    Discord juga merender tombol persetujuan bersama yang digunakan oleh channel chat lain. Adapter Discord native terutama menambahkan routing DM approver dan fanout channel.
    ...
    prompt `/approve <id> <decision>` lokal yang deterministik tetap terlihat. Jika
    ...
    Auth Gateway dan resolusi persetujuan mengikuti kontrak klien Gateway bersama (ID `plugin:` di-resolve melalui `plugin.approval.resolve`; ID lain melalui `exec.approval.resolve`). Persetujuan kedaluwarsa setelah 30 menit secara bawaan.
...
OpenClaw menggunakan components v2 Discord untuk persetujuan exec dan penanda lintas konteks. Tindakan pesan Discord juga dapat menerima `components` untuk UI kustom (lanjutan; memerlukan penyusunan payload component melalui tool discord), sementara `embeds` legacy tetap tersedia tetapi tidak direkomendasikan.

- `channels.discord.ui.components.accentColor` menetapkan warna aksen yang digunakan oleh container component Discord (hex).
- Tetapkan per akun dengan `channels.discord.accounts.<id>.ui.components.accentColor`.
- `channels.discord.agentComponents.ttlMs` mengontrol berapa lama callback component Discord yang dikirim tetap terdaftar (bawaan `1800000`, maksimum `86400000`). Tetapkan per akun dengan `channels.discord.accounts.<id>.agentComponents.ttlMs`.
- `embeds` diabaikan ketika components v2 ada.
- Pratinjau URL biasa ditekan secara bawaan. Tetapkan `suppressEmbeds: false` pada tindakan pesan ketika satu tautan keluar perlu diperluas.
...
2. Aktifkan Server Members Intent ketika allowlist role/pengguna digunakan.
...
Gunakan `/vc join|leave|status` untuk mengontrol sesi. Perintah ini menggunakan agen bawaan akun dan mengikuti aturan allowlist dan kebijakan grup yang sama seperti perintah Discord lain.
...
- `voice.model` menggantikan otak agen OpenClaw untuk respons suara Discord dan konsultasi realtime. Biarkan tidak disetel untuk mewarisi model agen yang dirutekan. Ini terpisah dari `voice.realtime.model`.
- `voice.followUsers` memungkinkan bot bergabung, berpindah, dan keluar dari suara Discord dengan pengguna terpilih. Lihat [Ikuti pengguna dalam suara](#follow-users-in-voice) untuk aturan perilaku dan contoh.
- `agent-proxy` merutekan ucapan melalui `discord-voice`, yang mempertahankan otorisasi pemilik/tool normal untuk pembicara dan sesi target tetapi menyembunyikan tool `tts` agen karena suara Discord memiliki pemutaran. Secara default, `agent-proxy` memberi konsultasi akses tool penuh setara pemilik untuk pembicara pemilik (`voice.realtime.toolPolicy: "owner"`) dan sangat memprioritaskan konsultasi dengan agen OpenClaw sebelum jawaban substantif (`voice.realtime.consultPolicy: "always"`). Dalam mode default `always` tersebut, lapisan realtime tidak otomatis mengucapkan pengisi sebelum jawaban konsultasi; lapisan ini menangkap dan mentranskripsi ucapan, lalu mengucapkan jawaban OpenClaw yang dirutekan. Jika beberapa jawaban konsultasi paksa selesai saat Discord masih memutar jawaban pertama, jawaban ucapan persis berikutnya diantrekan sampai pemutaran idle alih-alih mengganti ucapan di tengah kalimat.
...
- Mode suara realtime menyertakan file profil kecil `IDENTITY.md`, `USER.md`, dan `SOUL.md` dalam instruksi provider realtime secara default sehingga giliran langsung yang cepat mempertahankan identitas, grounding pengguna, dan persona yang sama seperti agen OpenClaw yang dirutekan. Setel `voice.realtime.bootstrapContextFiles` ke subset untuk menyesuaikannya, atau `[]` untuk menonaktifkannya. File bootstrap realtime yang didukung terbatas pada file profil tersebut; `AGENTS.md` tetap berada dalam konteks agen normal. Konteks profil yang diinjeksi tidak menggantikan `openclaw_agent_consult` untuk pekerjaan workspace, fakta saat ini, pencarian memori, atau tindakan yang didukung tool.
- Dalam mode realtime OpenAI `agent-proxy`, setel `voice.realtime.requireWakeName: true` agar suara realtime Discord tetap diam sampai transkrip dimulai atau diakhiri dengan nama panggil. Nama panggil yang dikonfigurasi harus satu atau dua kata. Jika `voice.realtime.wakeNames` tidak disetel, OpenClaw menggunakan `name` agen yang dirutekan plus `OpenClaw`, dengan fallback ke id agen plus `OpenClaw`. Gate nama panggil menonaktifkan respons otomatis provider realtime, merutekan giliran yang diterima melalui jalur konsultasi agen OpenClaw, dan memberikan pengakuan lisan singkat ketika nama panggil di awal dikenali dari transkripsi parsial sebelum transkrip final tiba.
- Provider realtime OpenAI menerima nama event Realtime 2 saat ini dan alias lama yang kompatibel dengan Codex untuk event audio output dan transkrip, sehingga snapshot provider yang kompatibel dapat bergeser tanpa menjatuhkan audio asisten.
- `voice.realtime.bargeIn` mengontrol apakah event mulai-berbicara Discord menginterupsi pemutaran realtime aktif. Jika tidak disetel, ini mengikuti setelan interupsi audio input provider realtime.
- `voice.realtime.minBargeInAudioEndMs` mengontrol durasi minimum pemutaran asisten sebelum barge-in realtime OpenAI memotong audio. Default: `250`. Setel `0` untuk interupsi langsung di ruangan dengan echo rendah, atau naikkan untuk setup speaker dengan echo berat.
- Untuk suara OpenAI pada pemutaran Discord, setel `voice.tts.provider: "openai"` dan pilih suara Text-to-speech di bawah `voice.tts.providers.openai.speakerVoice`. `cedar` adalah pilihan bersuara maskulin yang baik pada model TTS OpenAI saat ini.
- Penggantian `systemPrompt` Discord per channel berlaku untuk giliran transkrip suara untuk channel suara tersebut.
- Giliran transkrip suara menurunkan status pemilik dari `allowFrom` Discord (atau `dm.allowFrom`) untuk perintah berpagar pemilik dan tindakan channel. Visibilitas tool agen mengikuti kebijakan tool yang dikonfigurasi untuk sesi yang dirutekan.
...
- `channels.discord.intents.voiceStates` dapat secara eksplisit menggantikan langganan intent voice-state. Biarkan tidak disetel agar intent mengikuti pengaktifan suara efektif.
...
- `voice.allowedChannels` adalah allowlist residensi opsional. Biarkan tidak disetel untuk mengizinkan `/vc join` ke channel suara Discord resmi mana pun. Saat disetel, `/vc join`, auto-join startup, dan perpindahan voice-state bot dibatasi ke entri `{ guildId, channelId }` yang tercantum. Setel ke array kosong untuk menolak semua join suara Discord. Jika Discord memindahkan bot ke luar allowlist, OpenClaw keluar dari channel tersebut dan bergabung kembali ke target auto-join yang dikonfigurasi ketika tersedia.
...
- Dalam mode `stt-tts`, pemutaran suara tidak berhenti hanya karena pengguna lain mulai berbicara. Untuk menghindari feedback loop, OpenClaw mengabaikan tangkapan suara baru saat TTS sedang diputar; berbicaralah setelah pemutaran selesai untuk giliran berikutnya. Mode realtime meneruskan awal bicara sebagai sinyal barge-in ke provider realtime.
...
- `voice.captureSilenceGraceMs` mengontrol berapa lama OpenClaw menunggu setelah Discord melaporkan bahwa pembicara telah berhenti sebelum memfinalisasi segmen audio tersebut untuk STT. Default: `2000`; naikkan ini jika Discord membagi jeda normal menjadi transkrip parsial yang patah-patah.
- Ketika ElevenLabs adalah provider TTS yang dipilih, pemutaran suara Discord menggunakan TTS streaming dan dimulai dari stream respons provider. Provider tanpa dukungan streaming melakukan fallback ke jalur temp-file yang disintesis.
- OpenClaw juga memantau kegagalan dekripsi penerimaan dan memulihkan otomatis dengan keluar/bergabung kembali ke channel suara setelah kegagalan berulang dalam jendela singkat.
...
- Event penerimaan `The operation was aborted` diharapkan ketika OpenClaw memfinalisasi segmen pembicara yang ditangkap; itu adalah diagnostik verbose, bukan peringatan.
- Log suara Discord verbose menyertakan pratinjau transkrip STT satu baris yang dibatasi untuk setiap segmen pembicara yang diterima, sehingga debugging menampilkan sisi pengguna dan sisi balasan agen tanpa membuang teks transkrip tak terbatas.
- Dalam mode `agent-proxy`, fallback konsultasi paksa melewati fragmen transkrip yang kemungkinan belum lengkap seperti teks yang berakhir dengan `...` atau konektor tertinggal seperti `and`, plus penutup yang jelas tidak dapat ditindaklanjuti seperti “be right back” atau “bye”. Log menampilkan `forced agent consult skipped reason=...` ketika ini mencegah jawaban antrean yang basi.

### Ikuti pengguna dalam suara

Gunakan `voice.followUsers` saat Anda ingin bot suara Discord tetap bersama satu atau beberapa pengguna Discord yang dikenal alih-alih bergabung ke channel tetap saat startup atau menunggu `/vc join`.
...
- `followUsers` menerima ID pengguna Discord mentah dan nilai `discord:<id>`. OpenClaw menormalisasi kedua bentuk sebelum mencocokkan event voice-state.
- `followUsersEnabled` default ke `true` ketika `followUsers` dikonfigurasi. Setel ke `false` untuk mempertahankan daftar tersimpan tetapi menghentikan pengikutan suara otomatis.
- Saat pengguna yang diikuti bergabung ke channel suara yang diizinkan, OpenClaw bergabung ke channel tersebut. Saat pengguna berpindah, OpenClaw berpindah bersama mereka. Saat pengguna yang diikuti aktif terputus, OpenClaw keluar.
- Jika beberapa pengguna yang diikuti berada dalam guild yang sama dan pengguna yang diikuti aktif keluar, OpenClaw berpindah ke channel pengguna diikuti terlacak lainnya sebelum keluar dari guild. Jika beberapa pengguna yang diikuti berpindah sekaligus, event voice-state terbaru yang diamati menang.
- `allowedChannels` tetap berlaku. Pengguna yang diikuti dalam channel yang tidak diizinkan diabaikan, dan sesi milik follow berpindah ke pengguna yang diikuti lainnya atau keluar.
- OpenClaw merekonsiliasi event voice-state yang terlewat saat startup dan pada interval yang dibatasi. Rekonsiliasi mengambil sampel guild yang dikonfigurasi dan membatasi lookup REST per run, sehingga daftar `followUsers` yang sangat besar mungkin memerlukan lebih dari satu interval untuk konvergen.
- Jika Discord atau admin memindahkan bot saat sedang mengikuti pengguna, OpenClaw membangun ulang sesi suara dan mempertahankan kepemilikan follow ketika tujuan diizinkan. Jika bot dipindahkan ke luar `allowedChannels`, OpenClaw keluar dan bergabung kembali ke target yang dikonfigurasi ketika ada.
- Pemulihan penerimaan DAVE dapat keluar dan bergabung kembali ke channel yang sama setelah kegagalan dekripsi berulang. Sesi milik follow mempertahankan kepemilikan follow melalui jalur pemulihan tersebut, sehingga pemutusan pengguna yang diikuti berikutnya tetap membuat bot keluar dari channel.

Pilih di antara mode join:

- Gunakan `followUsers` untuk setup pribadi atau operator ketika bot harus otomatis berada dalam suara saat Anda berada di sana.
- Gunakan `autoJoin` untuk bot ruang tetap yang harus hadir bahkan ketika tidak ada pengguna terlacak dalam suara.
- Gunakan `/vc join` untuk join sekali pakai atau ruangan tempat kehadiran suara otomatis akan terasa mengejutkan.

Codec suara Discord:

- Log penerimaan suara menampilkan `discord voice: opus decoder: libopus-wasm`.
- Pemutaran realtime mengenkode PCM stereo 48 kHz mentah ke Opus dengan paket bundel `libopus-wasm` yang sama sebelum menyerahkan paket ke `@discordjs/voice`.
- Pemutaran file dan stream provider melakukan transkode ke PCM stereo 48 kHz mentah dengan ffmpeg, lalu menggunakan `libopus-wasm` untuk stream paket Opus yang dikirim ke Discord.
...
- Transkrip dikirim melalui ingress dan routing Discord sementara LLM respons berjalan dengan kebijakan keluaran suara yang menyembunyikan alat `tts` agent dan meminta teks yang dikembalikan, karena suara Discord memiliki pemutaran TTS akhir.
- `voice.model`, jika disetel, hanya menimpa LLM respons untuk giliran kanal suara ini.
- `voice.tts` digabungkan di atas `messages.tts`; provider yang mendukung streaming mengumpan pemutar secara langsung, jika tidak file audio yang dihasilkan diputar di kanal yang telah diikuti.
...
Tanpa blok `voice.agentSession`, setiap kanal suara mendapatkan sesi OpenClaw terarahnya sendiri. Misalnya, `/vc join channel:234567890123456789` berbicara ke sesi untuk kanal suara Discord tersebut. Model realtime hanyalah antarmuka depan suara; permintaan substantif diserahkan ke agent OpenClaw yang dikonfigurasi. Jika model realtime menghasilkan transkrip akhir tanpa memanggil alat konsultasi, OpenClaw memaksa konsultasi sebagai fallback sehingga default tetap berperilaku seperti berbicara dengan agent.
...
Dalam mode `agent-proxy`, bot bergabung ke kanal suara yang dikonfigurasi, tetapi giliran agent OpenClaw menggunakan sesi dan agent terarah normal milik kanal target. Sesi suara realtime mengucapkan hasil yang dikembalikan kembali ke kanal suara. Agent supervisor masih dapat menggunakan alat pesan normal sesuai kebijakan alatnya, termasuk mengirim pesan Discord terpisah jika itu tindakan yang tepat.

Saat run OpenClaw yang didelegasikan sedang aktif, transkrip suara Discord baru diperlakukan sebagai kontrol run langsung sebelum memulai giliran agent lain. Frasa seperti "status", "cancel that", "use the smaller fix", atau "when you're done also check tests" diklasifikasikan sebagai status, pembatalan, pengarahan, atau masukan tindak lanjut untuk sesi aktif. Hasil status, pembatalan, pengarahan yang diterima, dan tindak lanjut diucapkan kembali ke kanal suara sehingga penelepon tahu apakah OpenClaw menangani permintaan tersebut.
...
Gunakan ini ketika model mendengar pemutaran Discord-nya sendiri melalui mikrofon terbuka, tetapi Anda tetap ingin menginterupsinya dengan berbicara. OpenClaw mencegah OpenAI melakukan interupsi otomatis pada audio input mentah, sementara `bargeIn: true` memungkinkan peristiwa mulai-berbicara speaker Discord dan audio speaker yang sudah aktif membatalkan respons realtime aktif sebelum giliran tangkapan berikutnya mencapai OpenAI. Sinyal barge-in yang sangat awal dengan `audioEndMs` di bawah `minBargeInAudioEndMs` diperlakukan sebagai kemungkinan echo/noise dan diabaikan sehingga model tidak terpotong pada frame pemutaran pertama.
...
- Saat realtime mulai: `discord voice: realtime bridge starting ... autoRespond=false interruptResponse=false bargeIn=false minBargeInAudioEndMs=...`
...
- Saat ujaran basi dilewati: `discord voice: realtime forced agent consult skipped reason=incomplete-transcript ...` atau `reason=non-actionable-closing ...`
...
- Saat jawaban agent: `discord voice: agent turn answer ...`
- Saat ujaran persis diantrekan: `discord voice: realtime exact speech queued ... queued=... outputAudioMs=... outputActive=...`, diikuti oleh `discord voice: realtime exact speech dequeued reason=player-idle ...`
...
3. `realtime input audio started` menandai frame audio aktual pertama yang diterima untuk giliran speaker tersebut. `outputActive=true` atau `outputAudioMs` bukan nol di sini berarti mikrofon mengirim input sementara pemutaran asisten masih aktif.
...
5. `barge-in requested reason=...` berarti OpenClaw meminta provider realtime untuk membatalkan atau memotong respons aktif. Ini menyertakan `outputAudioMs`, `outputActive`, dan `playbackChunks` sehingga Anda dapat melihat berapa banyak audio asisten yang benar-benar telah diputar sebelum interupsi.
...
- `capture ignored during playback (barge-in disabled)` berarti OpenClaw sengaja membuang input saat audio asisten aktif. Aktifkan `voice.realtime.bargeIn` jika Anda ingin ujaran menginterupsi pemutaran.
- `barge-in ignored ... outputActive=false` berarti Discord atau VAD provider melaporkan ujaran, tetapi OpenClaw tidak memiliki pemutaran aktif untuk diinterupsi. Ini seharusnya tidak memotong audio.
...
```
  ...
  <Accordion title="Long-running Discord turns or duplicate replies">
    ...
    - ini hanya mengontrol pekerjaan listener Gateway Discord, bukan masa hidup giliran agent

    Discord tidak menerapkan timeout milik channel pada giliran agent yang mengantre. Listener pesan langsung menyerahkan pekerjaan, dan proses Discord yang mengantre mempertahankan urutan per sesi sampai siklus hidup sesi/tool/runtime selesai atau membatalkan pekerjaan.
...
```
  ...
  <Accordion title="Gateway metadata lookup timeout warnings">
    OpenClaw mengambil metadata Discord `/gateway/bot` sebelum terhubung. Kegagalan sementara fallback ke URL Gateway default Discord dan dibatasi laju kemunculannya di log.
    ...
    - fallback env saat config belum disetel: `OPENCLAW_DISCORD_GATEWAY_INFO_TIMEOUT_MS`
    - default: `30000` (30 detik), maksimum: `120000`

  </Accordion>

  <Accordion title="Gateway READY timeout restarts">
    OpenClaw menunggu event Gateway `READY` Discord saat startup dan setelah runtime tersambung ulang. Setup multi-akun dengan penjadwalan startup bertahap dapat memerlukan jendela READY startup yang lebih panjang daripada default.
    ...
    - fallback env startup saat config belum disetel: `OPENCLAW_DISCORD_READY_TIMEOUT_MS`
    - default startup: `15000` (15 detik), maksimum: `120000`
    ...
    - fallback env runtime saat config belum disetel: `OPENCLAW_DISCORD_RUNTIME_READY_TIMEOUT_MS`
  ...
  <Accordion title="Bot to bot loops">
    ...
    OpenClaw juga menyertakan [perlindungan loop bot](/id/channels/bot-loop-protection) bersama. Setiap kali `allowBots` membiarkan pesan yang dibuat bot mencapai dispatch, Discord memetakan event masuk ke fakta `(account, channel, bot pair)` dan guard pasangan generik menekan pasangan tersebut setelah melampaui anggaran event yang dikonfigurasi. Guard mencegah loop dua bot tak terkendali yang sebelumnya harus dihentikan oleh batas laju Discord; ini tidak memengaruhi deployment bot tunggal atau balasan bot sekali jalan yang tetap berada di bawah anggaran.

    Pengaturan default (aktif saat `allowBots` disetel):

    - `maxEventsPerWindow: 20` -- pasangan bot dapat bertukar 20 pesan dalam jendela geser
    - `windowSeconds: 60` -- panjang jendela geser
    - `cooldownSeconds: 60` -- setelah anggaran terlampaui, setiap pesan bot-ke-bot tambahan di kedua arah dibuang selama satu menit

    Konfigurasikan default bersama sekali di bawah `channels.defaults.botLoopProtection`, lalu override Discord saat workflow yang sah memerlukan ruang lebih besar. Prioritasnya adalah:
...
- media/coba ulang: `mediaMaxMb` (membatasi unggahan keluar Discord, default `100MB`), `retry`
- tindakan: `actions.*`
