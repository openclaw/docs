...
Status: teks + lampiran DM didukung; pengiriman file channel/grup memerlukan `sharePointSiteId` + izin Graph (lihat [Mengirim file di obrolan grup](#sending-files-in-group-chats)). Polling dikirim melalui Adaptive Cards. Tindakan pesan mengekspos `upload-file` eksplisit untuk pengiriman yang mengutamakan file.
...
Gunakan paket polos untuk mengikuti tag rilis resmi saat ini. Sematkan versi persis
...
<Note>
`--allow-anonymous` diperlukan karena Teams tidak dapat mengautentikasi dengan devtunnels. Setiap permintaan bot masuk tetap divalidasi secara otomatis oleh Teams SDK.
</Note>
...
<Note>
Obrolan grup diblokir secara default (`channels.msteams.groupPolicy: "allowlist"`). Untuk mengizinkan balasan grup, tetapkan `channels.msteams.groupAllowFrom`, atau gunakan `groupPolicy: "open"` untuk mengizinkan anggota mana pun (dibatasi oleh mention).
</Note>

## Tujuan

- Berbicara dengan OpenClaw melalui DM Teams, obrolan grup, atau channel.
- Menjaga perutean deterministik: balasan selalu kembali ke channel tempat pesan diterima.
...
- `channels.msteams.allowFrom` harus menggunakan ID objek AAD yang stabil.
...
- Default: `channels.msteams.groupPolicy = "allowlist"` (diblokir kecuali Anda menambahkan `groupAllowFrom`). Gunakan `channels.defaults.groupPolicy` untuk mengganti default saat belum ditetapkan.
...
- Tetapkan `groupPolicy: "open"` untuk mengizinkan anggota mana pun (tetap dibatasi oleh mention secara default).
...
- Saat startup, OpenClaw menyelesaikan nama tim/channel dan allowlist pengguna menjadi ID (jika izin Graph mengizinkan)
  dan mencatat pemetaannya; nama tim/channel yang tidak terselesaikan tetap disimpan seperti yang diketik tetapi diabaikan untuk perutean secara default kecuali `channels.msteams.dangerouslyAllowNameMatching: true` diaktifkan.
...
5. Konfigurasikan `msteams` di `~/.openclaw/openclaw.json` (atau variabel env) dan mulai gateway.
...
### Langkah 5: Buat Manifest Aplikasi Teams
...
Gunakan Azure Managed Identity untuk autentikasi tanpa kata sandi. Ini ideal untuk deployment pada infrastruktur Azure (AKS, App Service, Azure VM) tempat identitas terkelola tersedia.
...
**Config (identitas terkelola yang ditetapkan pengguna):**
...
1. **Aktifkan identitas workload** pada cluster AKS Anda.
...
| Metode                | Config                                         | Kelebihan                         | Kekurangan                                  |
| --------------------- | ---------------------------------------------- | --------------------------------- | ------------------------------------------- |
...
| **Identitas Terkelola** | `authType: "federated"` + `useManagedIdentity` | Tanpa kata sandi, tidak ada rahasia untuk dikelola | Memerlukan infrastruktur Azure |
...
Teams tidak dapat menjangkau `localhost`. Gunakan tunnel dev yang persisten agar URL Anda tetap sama di seluruh sesi:
...
Semua kunci config juga dapat diatur melalui variabel lingkungan:
...
OpenClaw mengekspos tindakan `member-info` yang didukung Graph untuk Microsoft Teams sehingga agen dan automasi dapat menyelesaikan detail anggota channel (nama tampilan, email, peran) langsung dari Microsoft Graph.

Persyaratan:

- Izin RSC `Member.Read.Group` (sudah ada di manifes yang direkomendasikan)
...
- Riwayat thread yang diambil difilter berdasarkan daftar pengirim yang diizinkan (`allowFrom` / `groupAllowFrom`), sehingga penyemaian konteks thread hanya menyertakan pesan dari pengirim yang diizinkan.
- Konteks lampiran yang dikutip (`ReplyTo*` yang diturunkan dari HTML balasan Teams) saat ini diteruskan sebagaimana diterima.
- Dengan kata lain, daftar yang diizinkan mengontrol siapa yang dapat memicu agen; hanya jalur konteks tambahan tertentu yang difilter saat ini.
...
## Contoh manifes Teams (disamarkan)

Contoh minimal dan valid dengan bidang yang diperlukan. Ganti ID dan URL.
...
Setelah memperbarui, instal ulang aplikasi di setiap tim agar izin baru berlaku, dan **keluar sepenuhnya lalu jalankan ulang Teams** (bukan hanya menutup jendela) untuk menghapus metadata aplikasi yang tersimpan di cache.
...
**Intinya:** RSC ditujukan untuk mendengarkan secara real-time; Graph API ditujukan untuk akses historis. Untuk mengejar pesan yang terlewat saat offline, Anda memerlukan Graph API dengan `ChannelMessage.Read.All` (memerlukan persetujuan admin).
...
**Izin tambahan untuk mention pengguna:** @mention pengguna langsung berfungsi untuk pengguna dalam percakapan. Namun, jika Anda ingin mencari dan mention pengguna secara dinamis yang **tidak berada dalam percakapan saat ini**, tambahkan izin `User.Read.All` (Application) dan berikan persetujuan admin.

## Batasan yang diketahui

### Timeout Webhook

Teams mengirimkan pesan melalui webhook HTTP. Jika pemrosesan memakan waktu terlalu lama (misalnya, respons LLM yang lambat), Anda mungkin melihat:

- Timeout Gateway
- Teams mencoba ulang pesan (menyebabkan duplikat)
- Balasan yang terlewat

OpenClaw menangani ini dengan merespons cepat dan mengirim balasan secara proaktif, tetapi respons yang sangat lambat masih dapat menyebabkan masalah.
...
- `channels.msteams.mediaAuthAllowHosts`: allowlist untuk menambahkan header Authorization pada percobaan ulang media (default ke host Graph + Bot Framework).
...
- `channels.msteams.teams.<teamId>.tools`: penggantian kebijakan tool default per tim (`allow`/`deny`/`alsoAllow`) yang digunakan saat penggantian channel tidak ada.
- `channels.msteams.teams.<teamId>.toolsBySender`: penggantian kebijakan tool default per tim per pengirim (wildcard `"*"` didukung).
- `channels.msteams.teams.<teamId>.channels.<conversationId>.replyStyle`: penggantian per channel.
- `channels.msteams.teams.<teamId>.channels.<conversationId>.requireMention`: penggantian per channel.
- `channels.msteams.teams.<teamId>.channels.<conversationId>.tools`: penggantian kebijakan tool per channel (`allow`/`deny`/`alsoAllow`).
- `channels.msteams.teams.<teamId>.channels.<conversationId>.toolsBySender`: penggantian kebijakan tool per channel per pengirim (wildcard `"*"` didukung).
...
| ------------------------ | ---------------------------------------------------------- | ---------------------------------- |
| **Postingan** (klasik)   | Pesan muncul sebagai kartu dengan balasan berutas di bawah | `thread` (default)                 |
| **Utas** (mirip Slack)   | Pesan mengalir secara linear, lebih mirip Slack            | `top-level`                        |

**Masalahnya:** API Teams tidak mengekspos gaya UI yang digunakan suatu channel. Jika Anda menggunakan `replyStyle` yang salah:

- `thread` di channel bergaya Utas → balasan muncul bertumpuk dengan canggung
...
Saat bot mengirim balasan ke channel, `replyStyle` diselesaikan dari penggantian paling spesifik hingga default. Nilai non-`undefined` pertama yang menang:
...
Jika Anda menetapkan `requireMention: false` secara global tanpa `replyStyle` eksplisit, mention di channel bergaya Postingan akan muncul sebagai postingan tingkat atas meskipun inbound adalah balasan utas. Pin `replyStyle: "thread"` di tingkat global, tim, atau channel untuk menghindari kejutan.

### Pemeliharaan konteks utas

Saat `replyStyle: "thread"` berlaku dan bot di-@mention dari dalam utas channel, OpenClaw menempelkan kembali root utas asli ke referensi percakapan keluar (`19:…@thread.tacv2;messageid=<root>`) sehingga balasan mendarat di dalam utas yang sama. Ini berlaku untuk pengiriman live (in-turn) dan pengiriman proaktif yang dibuat setelah konteks turn Bot Framework kedaluwarsa (misalnya agen yang berjalan lama, balasan tool-call antrean melalui `mcp__openclaw__message`).

Root utas diambil dari `threadId` tersimpan pada referensi percakapan. Referensi tersimpan lama yang mendahului `threadId` fallback ke `activityId` (aktivitas inbound apa pun yang terakhir menginisialisasi percakapan), sehingga deployment yang ada tetap bekerja tanpa inisialisasi ulang.

Saat `replyStyle: "top-level"` berlaku, inbound utas channel sengaja dijawab sebagai postingan tingkat atas baru — tidak ada sufiks utas yang dilampirkan. Ini perilaku yang benar untuk channel bergaya Utas; jika Anda melihat postingan tingkat atas saat mengharapkan balasan berutas, `replyStyle` Anda salah untuk channel tersebut.

## Lampiran dan gambar

**Batasan saat ini:**

- **DM:** Gambar dan lampiran file bekerja melalui API file bot Teams.
...
- Untuk pengiriman file-first eksplisit, gunakan `action=upload-file` dengan `media` / `filePath` / `path`; `message` opsional menjadi teks/komentar pendamping, dan `filename` mengganti nama unggahan.

Tanpa izin Graph, pesan channel dengan gambar akan diterima sebagai teks saja (konten gambar tidak dapat diakses oleh bot).
Secara default, OpenClaw hanya mengunduh media dari nama host Microsoft/Teams. Ganti dengan `channels.msteams.mediaAllowHosts` (gunakan `["*"]` untuk mengizinkan host apa pun).
...
| ------------------------ | --------------------------------------------- | ----------------------------------------------- |
| **DM**                   | FileConsentCard → pengguna menerima → bot mengunggah | Berfungsi langsung tanpa konfigurasi            |
| **Obrolan grup/channel** | Unggah ke SharePoint → bagikan tautan         | Memerlukan `sharePointSiteId` + izin Graph      |
| **Gambar (konteks apa pun)** | Inline berkode Base64                     | Berfungsi langsung tanpa konfigurasi            |

### Mengapa obrolan grup memerlukan SharePoint

Bot tidak memiliki drive OneDrive pribadi (endpoint Graph API `/me/drive` tidak bekerja untuk identitas aplikasi). Untuk mengirim file di obrolan grup/channel, bot mengunggah ke **situs SharePoint** dan membuat tautan berbagi.
...
| Obrolan pribadi + file                           | Alur FileConsentCard (bekerja tanpa SharePoint)    |
| Konteks apa pun + gambar                         | Inline berkode Base64 (bekerja tanpa SharePoint)   |
...
- Jajak pendapat belum otomatis memposting ringkasan hasil (periksa file store jika diperlukan).
...
Target MSTeams menggunakan awalan untuk membedakan antara pengguna dan percakapan:
...
<Note>
Tanpa awalan `user:`, nama secara default diselesaikan sebagai grup atau tim. Selalu gunakan `user:` saat menargetkan orang berdasarkan nama tampilan.
</Note>

## Pesan proaktif

- Pesan proaktif hanya memungkinkan **setelah** pengguna berinteraksi, karena kami menyimpan referensi percakapan pada titik itu.
...
- Kunci tim = segmen jalur setelah `/team/` (di-URL-decode, misalnya, `19:Bk4j...@thread.tacv2`; tenant lama mungkin menampilkan `@thread.skype`, yang juga valid)
- Kunci channel = segmen jalur setelah `/channel/` (di-URL-decode)
...
- **Gambar tidak muncul di channel:** Izin Graph atau persetujuan admin hilang. Instal ulang aplikasi Teams dan keluar sepenuhnya/buka kembali Teams.
...
- **Ketidakcocokan versi (Teams masih menampilkan manifes lama):** hapus + tambahkan kembali aplikasi dan keluar sepenuhnya dari Teams untuk menyegarkan.
...
- **"Something went wrong" saat unggahan:** Unggah melalui [https://admin.teams.microsoft.com](https://admin.teams.microsoft.com) sebagai gantinya, buka DevTools browser (F12) → tab Network, dan periksa isi respons untuk kesalahan sebenarnya.
