...
summary: Dukungan iMessage lama melalui imsg (JSON-RPC melalui stdio). Pengaturan baru sebaiknya menggunakan BlueBubbles.
title: iMessage
x-i18n:
    generated_at: "2026-04-25T13:41:29Z"
    model: gpt-5.4
    provider: openai
...
Integrasi `imsg` bersifat lama dan dapat dihapus pada rilis mendatang.
</Warning>
...
<CardGroup cols={3}>
  <Card title="BlueBubbles (disarankan)" icon="message-circle" href="/id/channels/bluebubbles">
    Jalur iMessage yang dipilih untuk pengaturan baru.
  </Card>
...
```json5
{
  channels: {
    imessage: {
      ...
      // Opsional: ganti root lampiran yang diizinkan.
...
```
    ...
    Jika `remoteHost` tidak diatur, OpenClaw mencoba mendeteksinya secara otomatis dengan mem-parse skrip pembungkus SSH.
...
<Tip>
Izin diberikan per konteks proses. Jika gateway berjalan secara headless (LaunchAgent/SSH), jalankan perintah interaktif satu kali dalam konteks yang sama untuk memicu prompt:
...
<Tabs>
  ...
  <Tab title="Kebijakan grup + mention">
    ...
    Catatan runtime: jika `channels.imessage` tidak ada sama sekali, runtime fallback ke `groupPolicy="allowlist"` dan mencatat peringatan (bahkan jika `channels.defaults.groupPolicy` diatur).
    ...
    - tanpa pola yang dikonfigurasi, pembatasan mention tidak dapat ditegakkan
  ...
  <Tab title="Sesi dan balasan deterministik">
    ...
    Jika `chat_id` tersebut secara eksplisit dikonfigurasi di bawah `channels.imessage.groups`, OpenClaw memperlakukannya sebagai lalu lintas grup (pembatasan grup + isolasi sesi grup).
...
<AccordionGroup>
  <Accordion title="Pengguna bot macOS khusus (identitas iMessage terpisah)">
    Gunakan Apple ID dan pengguna macOS khusus agar lalu lintas bot terpisah dari profil Messages pribadi Anda.

    Alur umum:

    1. Buat/masuk dengan pengguna macOS khusus.
...
```
    ...
    Gunakan SSH key agar SSH maupun SCP bersifat non-interaktif.
...
iMessage mengizinkan penulisan konfigurasi yang diprakarsai saluran secara default (untuk `/config set|unset` saat `commands.config: true`).
...
```
    ...
    Konfirmasikan bahwa Full Disk Access + Automation diberikan untuk konteks proses yang menjalankan OpenClaw/`imsg`.
