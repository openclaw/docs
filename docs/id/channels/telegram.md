...
<Steps>
  <Step title="Create the bot token in BotFather">
    Buka Telegram dan mulai chat dengan **@BotFather** (pastikan handle persis `@BotFather`).
...
<AccordionGroup>
  <Accordion title="Privacy mode and group visibility">
    Bot Telegram default menggunakan **Privacy Mode**, yang membatasi pesan grup yang diterima.
...
<Tabs>
  <Tab title="DM policy">
    ...
    `dmPolicy: "open"` dengan `allowFrom: ["*"]` memungkinkan akun Telegram mana pun yang menemukan atau menebak nama pengguna bot untuk memberi perintah ke bot. Gunakan hanya untuk bot yang memang publik dengan alat yang dibatasi ketat; bot satu pemilik sebaiknya menggunakan `allowlist` dengan ID pengguna numerik.

    `channels.telegram.allowFrom` menerima ID pengguna Telegram numerik. Prefiks `telegram:` / `tg:` diterima dan dinormalisasi.
    Dalam config multi-akun, `channels.telegram.allowFrom` tingkat atas yang restriktif diperlakukan sebagai batas keamanan: entri `allowFrom: ["*"]` tingkat akun tidak membuat akun tersebut publik kecuali allowlist efektif akun masih berisi wildcard eksplisit setelah penggabungan.
    ...
    Jika Anda melakukan upgrade dan config berisi entri allowlist `@username`, jalankan `openclaw doctor --fix` untuk meresolusinya (upaya terbaik; memerlukan token bot Telegram).
    ...
    Untuk bot satu pemilik, pilih `dmPolicy: "allowlist"` dengan ID numerik `allowFrom` eksplisit agar kebijakan akses tahan lama di config (alih-alih bergantung pada persetujuan pemasangan sebelumnya).
...
```
  ...
  <Tab title="Group policy and allowlists">
    ...
    Contoh: izinkan anggota mana pun di satu grup spesifik:
...
```
    ...
    Contoh: izinkan hanya pengguna tertentu di dalam satu grup spesifik:
...
```
    ...
    <Warning>
      ...
      - Masukkan ID pengguna Telegram seperti `8734062810` di bawah `groupAllowFrom` saat Anda ingin membatasi siapa di dalam grup yang diizinkan dapat memicu bot.
...
- Telegram Bot API tidak memiliki dukungan tanda sudah dibaca (`sendReadReceipts` tidak berlaku).
...
<AccordionGroup>
  <Accordion title="Live stream preview (message edits)">
    ...
    - `progress` mempertahankan satu draf status yang dapat diedit dan memperbaruinya dengan progres alat sampai pengiriman final
    - `streaming.preview.toolProgress` mengontrol apakah pembaruan alat/progres menggunakan ulang pesan pratinjau yang sama yang diedit (default: `true` saat streaming pratinjau aktif)
    ...
    Untuk menjaga progres alat tetap terlihat tetapi menyembunyikan teks command/exec, atur:
    ...
    Gunakan `streaming.mode: "off"` hanya ketika Anda menginginkan pengiriman final saja: edit pratinjau Telegram dinonaktifkan dan obrolan alat/progres generik disembunyikan alih-alih dikirim sebagai pesan status mandiri. Prompt persetujuan, payload media, dan kesalahan tetap diarahkan melalui pengiriman final normal. Gunakan `streaming.preview.toolProgress: false` ketika Anda hanya ingin mempertahankan edit pratinjau jawaban sambil menyembunyikan baris status progres alat.
    ...
    - pratinjau yang diikuti keluaran non-pratinjau yang terlihat: OpenClaw mengirim balasan lengkap sebagai pesan final baru dan membersihkan pratinjau lama, sehingga jawaban final muncul setelah keluaran perantara
...
```
    ...
    - perintah kustom hanya merupakan entri menu; perintah tersebut tidak mengimplementasikan perilaku secara otomatis
    ...
    ### Perintah pemasangan perangkat (`device-pair` plugin)
    ...
    Kode penyiapan membawa token bootstrap berumur pendek. Serah terima bootstrap bawaan mempertahankan token node utama pada `scopes: []`; token operator apa pun yang diserahterimakan tetap dibatasi ke `operator.approvals`, `operator.read`, `operator.talk.secrets`, dan `operator.write`. Pemeriksaan cakupan bootstrap diberi prefiks peran, sehingga allowlist operator tersebut hanya memenuhi permintaan operator; peran non-operator masih membutuhkan cakupan di bawah prefiks perannya sendiri.

    Jika perangkat mencoba lagi dengan detail autentikasi yang berubah (misalnya peran/cakupan/kunci publik), permintaan tertunda sebelumnya digantikan dan permintaan baru menggunakan `requestId` yang berbeda. Jalankan ulang `/pair pending` sebelum menyetujui.
...
```
  ...
  <Accordion title="Telegram message actions for agents and automation">
    ...
    Pengiriman runtime menggunakan snapshot konfigurasi/secret aktif (startup/muat ulang), sehingga jalur tindakan tidak melakukan resolusi ulang SecretRef ad hoc per pengiriman.
  ...
  <Accordion title="Reply threading tags">
    ...
    Ketika threading balasan diaktifkan dan teks atau caption Telegram asli tersedia, OpenClaw menyertakan kutipan native Telegram secara otomatis. Telegram membatasi teks kutipan native pada 1024 unit kode UTF-16, sehingga pesan yang lebih panjang dikutip dari awal dan kembali ke balasan biasa jika Telegram menolak kutipan tersebut.
  ...
  <Accordion title="Forum topics and thread behavior">
    ...
    **Perutean agen per topik**: Setiap topik dapat diarahkan ke agen berbeda dengan mengatur `agentId` dalam konfigurasi topik. Ini memberi setiap topik workspace, memori, dan sesi terisolasi sendiri. Contoh:
    ...
    **Spawn ACP terikat thread dari chat**: `/acp spawn <agent> --thread here|auto` mengikat topik saat ini ke sesi ACP baru; tindak lanjut diarahkan langsung ke sana. OpenClaw mem-pin konfirmasi spawn dalam topik. Mengharuskan `channels.telegram.threadBindings.spawnSessions` tetap diaktifkan (default: `true`).
...
```
    ...
    Catatan video tidak mendukung caption; teks pesan yang disediakan dikirim secara terpisah.
...
```
  ...
  <Accordion title="Long polling vs Webhook">
    ...
    Listener lokal bind ke `127.0.0.1:8787`. Untuk ingress publik, tempatkan reverse proxy di depan port lokal atau tetapkan `webhookHost: "0.0.0.0"` secara sengaja.
  ...
  <Accordion title="Batas, retry, dan target CLI">
    ...
    - `channels.telegram.mediaGroupFlushMs` (default 500) mengontrol berapa lama album/grup media Telegram dibuffer sebelum OpenClaw mengirimnya sebagai satu pesan masuk. Tingkatkan jika bagian album datang terlambat; kurangi untuk mengurangi latensi balasan album.
    - `channels.telegram.timeoutSeconds` menimpa timeout klien API Telegram (jika tidak ditetapkan, default grammY berlaku). Klien bot membatasi nilai yang dikonfigurasi di bawah guard permintaan teks/typing keluar 60 detik agar grammY tidak membatalkan pengiriman balasan yang terlihat sebelum guard transport OpenClaw dan fallback dapat berjalan. Long polling tetap menggunakan guard permintaan `getUpdates` 45 detik agar polling idle tidak ditinggalkan tanpa batas.
...
```
    ...
    - `--force-document` untuk mengirim gambar keluar dan GIF sebagai dokumen, bukan unggahan foto terkompresi atau media animasi
  ...
  <Accordion title="Persetujuan exec di Telegram">
    ...
    `channels.telegram.allowFrom`, `groupAllowFrom`, dan `defaultTo` mengontrol siapa yang dapat berbicara dengan bot dan ke mana bot mengirim balasan normal. Semuanya tidak membuat seseorang menjadi pemberi persetujuan exec. Pairing DM pertama yang disetujui mem-bootstrap `commands.ownerAllowFrom` saat belum ada owner perintah, sehingga penyiapan satu owner tetap berfungsi tanpa menduplikasi ID di bawah `execApprovals.approvers`.

    Pengiriman channel menampilkan teks perintah di chat; aktifkan `channel` atau `both` hanya di grup/topik tepercaya. Saat prompt mendarat di topik forum, OpenClaw mempertahankan topik untuk prompt persetujuan dan tindak lanjut. Persetujuan exec kedaluwarsa setelah 30 menit secara default.

    Tombol persetujuan inline juga memerlukan `channels.telegram.capabilities.inlineButtons` untuk mengizinkan surface target (`dm`, `group`, atau `all`). ID persetujuan yang diawali `plugin:` di-resolve melalui persetujuan Plugin; lainnya di-resolve melalui persetujuan exec terlebih dahulu.
...
Saat agen mengalami error pengiriman atau provider, Telegram dapat membalas dengan teks error atau menyembunyikannya. Dua kunci konfigurasi mengontrol perilaku ini:

| Kunci                               | Nilai             | Default | Deskripsi                                                                                       |
| ----------------------------------- | ----------------- | ------- | ----------------------------------------------------------------------------------------------- |
| `channels.telegram.errorPolicy`     | `reply`, `silent` | `reply` | `reply` mengirim pesan error yang ramah ke chat. `silent` menyembunyikan balasan error sepenuhnya. |
| `channels.telegram.errorCooldownMs` | number (ms)       | `60000` | Waktu minimum antarbalasan error ke chat yang sama. Mencegah spam error saat gangguan.           |

Override per-akun, per-grup, dan per-topik didukung (pewarisan sama seperti kunci konfigurasi Telegram lainnya).
...
<AccordionGroup>
  <Accordion title="Bot tidak merespons pesan grup tanpa mention">
    ...
    - `openclaw channels status --probe` dapat memeriksa ID grup numerik eksplisit; wildcard `"*"` tidak dapat di-probe untuk keanggotaan.
  ...
  <Accordion title="Bot sama sekali tidak melihat pesan grup">
    ...
    - tinjau log: `openclaw logs --follow` untuk alasan lewati
  ...
  <Accordion title="Polling atau ketidakstabilan jaringan">
    ...
    - Jika `deleteWebhook` gagal dengan error jaringan sementara selama startup polling, OpenClaw melanjutkan ke long polling alih-alih membuat panggilan control-plane pra-polling lainnya. Webhook yang masih aktif muncul sebagai konflik `getUpdates`; OpenClaw kemudian membangun ulang transport Telegram dan mencoba ulang pembersihan webhook.
    - Jika soket Telegram didaur ulang pada kadensi tetap yang pendek, periksa apakah `channels.telegram.timeoutSeconds` rendah; klien bot membatasi nilai yang dikonfigurasi di bawah guard permintaan outbound dan `getUpdates`, tetapi rilis lama dapat membatalkan setiap poll atau balasan ketika ini disetel di bawah guard tersebut.
    ...
    - `openclaw channels status --probe` dan `openclaw doctor` memberi peringatan ketika akun polling yang berjalan belum menyelesaikan `getUpdates` setelah masa tenggang startup, ketika akun webhook yang berjalan belum menyelesaikan `setWebhook` setelah masa tenggang startup, atau ketika aktivitas transport polling terakhir yang berhasil sudah kedaluwarsa.
...
```
    ...
    - Jawaban rentang benchmark RFC 2544 (`198.18.0.0/15`) sudah diizinkan
      ...
      privat/internal/special-use lain selama unduhan media, Anda dapat opt in
...
```
    ...
    - Jika proxy Anda me-resolve host media Telegram menjadi `198.18.x.x`, biarkan
      flag berbahaya tetap mati terlebih dahulu. Media Telegram sudah mengizinkan
    ...
    <Warning>
      ...
      operator seperti perutean fake-IP Clash, Mihomo, atau Surge ketika mereka
      menyintesis jawaban privat atau special-use di luar rentang benchmark RFC 2544.
      Biarkan mati untuk akses Telegram internet publik normal.
    </Warning>
...
<Accordion title="Bidang Telegram sinyal tinggi">
