...
title: Telegram
x-i18n:
    generated_at: "2026-05-02T09:14:22Z"
    model: gpt-5.5
    provider: openai
...
<AccordionGroup>
  <Accordion title="Mode privasi dan visibilitas grup">
    Bot Telegram default ke **Privacy Mode**, yang membatasi pesan grup yang mereka terima.
  ...
  <Accordion title="Toggle BotFather yang membantu">
...
<Tabs>
  <Tab title="Kebijakan DM">
    ...
    `dmPolicy: "open"` dengan `allowFrom: ["*"]` memungkinkan akun Telegram mana pun yang menemukan atau menebak nama pengguna bot untuk memerintah bot. Gunakan hanya untuk bot publik yang disengaja dengan alat yang sangat dibatasi; bot dengan satu pemilik sebaiknya menggunakan `allowlist` dengan ID pengguna numerik.
    ...
    Untuk bot dengan satu pemilik, pilih `dmPolicy: "allowlist"` dengan ID numerik `allowFrom` eksplisit agar kebijakan akses tahan lama di config (alih-alih bergantung pada persetujuan penyandingan sebelumnya).

    Kebingungan umum: persetujuan penyandingan DM tidak berarti "pengirim ini diotorisasi di semua tempat".
    Penyandingan memberi akses DM. Jika belum ada pemilik perintah, penyandingan pertama yang disetujui juga menetapkan `commands.ownerAllowFrom` sehingga perintah khusus pemilik dan persetujuan exec memiliki akun operator eksplisit.
...
```
  ...
  <Tab title="Kebijakan grup dan allowlist">
    ...
    Pola praktis untuk bot dengan satu pemilik: setel ID pengguna Anda di `channels.telegram.allowFrom`, biarkan `groupAllowFrom` tidak disetel, dan izinkan grup target di bawah `channels.telegram.groups`.
    Catatan runtime: jika `channels.telegram` benar-benar tidak ada, runtime default ke fail-closed `groupPolicy="allowlist"` kecuali `channels.defaults.groupPolicy` disetel secara eksplisit.
...
```
    ...
    - atau inspeksi Bot API `getUpdates`
...
- Routing bersifat deterministik: inbound Telegram membalas kembali ke Telegram (model tidak memilih saluran).
- Pesan inbound dinormalisasi ke amplop saluran bersama dengan metadata balasan dan placeholder media.
...
- Long polling dijaga di dalam setiap proses gateway sehingga hanya satu poller aktif yang dapat menggunakan token bot pada satu waktu. Jika Anda masih melihat konflik `getUpdates` 409, gateway OpenClaw lain, skrip, atau poller eksternal kemungkinan menggunakan token yang sama.
...
<AccordionGroup>
  <Accordion title="Pratinjau streaming langsung (edit pesan)">
    ...
    - `streaming.preview.toolProgress` mengontrol apakah pembaruan alat/progress menggunakan ulang pesan pratinjau yang diedit yang sama (default: `true` saat streaming pratinjau aktif)
    ...
    Pembaruan pratinjau progress alat adalah baris pendek "Working..." yang ditampilkan saat alat berjalan, misalnya eksekusi perintah, pembacaan file, pembaruan perencanaan, atau ringkasan patch. Telegram menjaga ini aktif secara default agar cocok dengan perilaku OpenClaw yang dirilis dari `v2026.4.22` dan setelahnya. Untuk mempertahankan pratinjau yang diedit untuk teks jawaban tetapi menyembunyikan baris progress alat, setel:
    ...
    Gunakan `streaming.mode: "off"` hanya saat Anda menginginkan pengiriman final saja: edit pratinjau Telegram dinonaktifkan dan obrolan alat/progress generik ditekan alih-alih dikirim sebagai pesan "Working..." mandiri. Prompt persetujuan, payload media, dan error tetap dirutekan melalui pengiriman final normal. Gunakan `streaming.preview.toolProgress: false` saat Anda hanya ingin mempertahankan edit pratinjau jawaban sambil menyembunyikan baris status progress alat.

    Untuk balasan hanya teks:

    - pratinjau DM/grup/topik singkat: OpenClaw mempertahankan pesan pratinjau yang sama dan melakukan edit akhir di tempat
    - pratinjau yang lebih lama dari sekitar satu menit: OpenClaw mengirim balasan selesai sebagai pesan akhir baru lalu membersihkan pratinjau, sehingga stempel waktu yang terlihat di Telegram mencerminkan waktu penyelesaian, bukan waktu pembuatan pratinjau

    Untuk balasan kompleks (misalnya muatan media), OpenClaw kembali ke pengiriman akhir normal lalu membersihkan pesan pratinjau.
  ...
  <Accordion title="Pemformatan dan fallback HTML">
    ...
    - Jika Telegram menolak HTML yang diparse, OpenClaw mencoba lagi sebagai teks biasa.
...
```
    ...
    Jika perintah native dinonaktifkan, bawaan dihapus. Perintah kustom/plugin mungkin masih terdaftar jika dikonfigurasi.
    ...
    - `setMyCommands failed` dengan galat network/fetch biasanya berarti DNS/HTTPS keluar ke `api.telegram.org` diblokir.
    ...
    Kode penyiapan membawa token bootstrap berumur pendek. Serah terima bootstrap bawaan mempertahankan token node utama pada `scopes: []`; token operator apa pun yang diserahterimakan tetap dibatasi ke `operator.approvals`, `operator.read`, `operator.talk.secrets`, dan `operator.write`. Pemeriksaan cakupan bootstrap diberi prefiks peran, sehingga allowlist operator tersebut hanya memenuhi permintaan operator; peran non-operator tetap memerlukan cakupan di bawah prefiks perannya sendiri.

    Jika perangkat mencoba lagi dengan detail autentikasi yang berubah (misalnya peran/cakupan/kunci publik), permintaan tertunda sebelumnya digantikan dan permintaan baru menggunakan `requestId` berbeda. Jalankan ulang `/pair pending` sebelum menyetujui.
...
```
  ...
  <Accordion title="Aksi pesan Telegram untuk agent dan automasi">
  ...
  <Accordion title="Tag threading balasan">
    ...
    Saat threading balasan diaktifkan dan teks atau caption Telegram asli tersedia, OpenClaw menyertakan kutipan native Telegram secara otomatis. Telegram membatasi teks kutipan native pada 1024 unit kode UTF-16, sehingga pesan yang lebih panjang dikutip dari awal dan fallback ke balasan biasa jika Telegram menolak kutipan.
  ...
  <Accordion title="Topik forum dan perilaku thread">
    ...
    **Perutean agent per topik**: Setiap topik dapat dirutekan ke agent berbeda dengan menyetel `agentId` dalam konfigurasi topik. Ini memberi setiap topik workspace, memori, dan sesi terisolasi sendiri. Contoh:
    ...
    **Spawn ACP terikat thread dari chat**: `/acp spawn <agent> --thread here|auto` mengikat topik saat ini ke sesi ACP baru; tindak lanjut dirutekan langsung ke sana. OpenClaw menyematkan konfirmasi spawn di dalam topik. Memerlukan `channels.telegram.threadBindings.spawnSessions` tetap diaktifkan (default: `true`).
    ...
    Konteks templat mengekspos `MessageThreadId` dan `IsForum`. Chat DM dengan `message_thread_id` mempertahankan perutean DM tetapi menggunakan kunci sesi yang sadar thread.
  ...
  <Accordion title="Audio, video, dan stiker">
    ...
    - tag `[[audio_as_voice]]` dalam balasan agent untuk memaksa pengiriman catatan suara
    - transkrip catatan suara masuk dibingkai sebagai teks yang dihasilkan mesin dan tidak tepercaya dalam konteks agent; deteksi mention tetap menggunakan transkrip mentah sehingga pesan suara yang digating mention tetap berfungsi.
...
```
    ...
    Catatan video tidak mendukung caption; teks pesan yang diberikan dikirim terpisah.
...
```
  ...
  <Accordion title="Notifikasi reaksi">
    Reaksi Telegram tiba sebagai pembaruan `message_reaction` (terpisah dari muatan pesan).
    ...
    - Peristiwa reaksi tetap mematuhi kontrol akses Telegram (`dmPolicy`, `allowFrom`, `groupPolicy`, `groupAllowFrom`); pengirim tidak sah dibuang.
...
```
  ...
  <Accordion title="Long polling vs webhook">
    ...
    Listener lokal mengikat ke `127.0.0.1:8787`. Untuk ingress publik, letakkan proxy balik di depan port lokal atau atur `webhookHost: "0.0.0.0"` secara sengaja.

    Mode webhook memvalidasi guard permintaan, token rahasia Telegram, dan body JSON sebelum mengembalikan `200` ke Telegram.
    OpenClaw lalu memproses pembaruan secara asinkron melalui lane bot per-obrolan/per-topik yang sama seperti yang digunakan oleh long polling, sehingga giliran agen yang lambat tidak menahan ACK pengiriman Telegram.
  ...
  <Accordion title="Batas, percobaan ulang, dan target CLI">
    ...
    - `channels.telegram.timeoutSeconds` menimpa timeout klien API Telegram (jika tidak diatur, default grammY berlaku). Klien bot long-polling menjepit nilai terkonfigurasi yang berada di bawah guard permintaan `getUpdates` 45 detik agar polling idle tidak dibatalkan sebelum jendela polling 30 detik selesai.
    ...
    - allowlist Telegram terutama membatasi siapa yang dapat memicu agen, bukan batas redaksi konteks tambahan yang penuh.
    - Kontrol riwayat DM:
    ...
    Target pengiriman CLI dapat berupa ID obrolan numerik atau nama pengguna:
...
```
    ...
    - `--pin` atau `--delivery '{"pin":true}'` untuk meminta pengiriman yang disematkan ketika bot dapat menyematkan di obrolan tersebut
  ...
  <Accordion title="Persetujuan exec di Telegram">
    ...
    Tombol persetujuan inline juga memerlukan `channels.telegram.capabilities.inlineButtons` untuk mengizinkan permukaan target (`dm`, `group`, atau `all`). ID persetujuan yang diawali dengan `plugin:` diselesaikan melalui persetujuan plugin; yang lain diselesaikan melalui persetujuan exec terlebih dahulu.
...
Ketika agen mengalami error pengiriman atau penyedia, Telegram dapat membalas dengan teks error atau menyembunyikannya. Dua kunci konfigurasi mengontrol perilaku ini:

| Kunci                               | Nilai             | Default | Deskripsi                                                                                                   |
| ----------------------------------- | ----------------- | ------- | ----------------------------------------------------------------------------------------------------------- |
| `channels.telegram.errorPolicy`     | `reply`, `silent` | `reply` | `reply` mengirim pesan error yang ramah ke obrolan. `silent` menyembunyikan semua balasan error sepenuhnya. |
| `channels.telegram.errorCooldownMs` | number (ms)       | `60000` | Waktu minimum antara balasan error ke obrolan yang sama. Mencegah spam error selama gangguan.               |
...
<AccordionGroup>
  ...
  <Accordion title="Perintah berfungsi sebagian atau tidak sama sekali">
    ...
    - Panggilan startup `deleteMyCommands` / `setMyCommands` dibatasi dan dicoba ulang sekali melalui fallback transport Telegram saat timeout permintaan. Error jaringan/fetch persisten biasanya menunjukkan masalah keterjangkauan DNS/HTTPS ke `api.telegram.org`
  ...
  <Accordion title="Startup melaporkan token tidak sah">
    ...
    - `deleteWebhook 401 Unauthorized` saat startup juga merupakan kegagalan auth; memperlakukannya sebagai "tidak ada webhook" hanya akan menunda kegagalan bad-token yang sama ke panggilan API berikutnya.
    - Jika `deleteWebhook` gagal dengan error jaringan sementara saat startup polling, OpenClaw memeriksa `getWebhookInfo`; ketika Telegram melaporkan URL webhook kosong, polling berlanjut karena pembersihan sudah terpenuhi.

  </Accordion>

  <Accordion title="Ketidakstabilan polling atau jaringan">

    - Node 22+ + fetch/proxy khusus dapat memicu perilaku abort langsung jika jenis AbortSignal tidak cocok.
    - Beberapa host menyelesaikan `api.telegram.org` ke IPv6 terlebih dahulu; egress IPv6 yang rusak dapat menyebabkan kegagalan Telegram API secara berkala.
    - Jika log mencakup `TypeError: fetch failed` atau `Network request for 'getUpdates' failed!`, OpenClaw kini mencoba ulang ini sebagai kesalahan jaringan yang dapat dipulihkan.
    - Jika soket Telegram didaur ulang pada interval tetap yang singkat, periksa `channels.telegram.timeoutSeconds` yang rendah; klien bot polling panjang membatasi nilai yang dikonfigurasi di bawah pengaman permintaan `getUpdates`, tetapi rilis lama dapat membatalkan setiap polling ketika ini diatur di bawah batas waktu polling panjang.
    - Jika log mencakup `Polling stall detected`, OpenClaw memulai ulang polling dan membangun ulang transport Telegram setelah 120 detik tanpa kelangsungan hidup polling panjang yang selesai secara default.
    ...
    - Naikkan `channels.telegram.pollingStallThresholdMs` hanya ketika panggilan `getUpdates` yang berjalan lama sehat tetapi host Anda masih melaporkan restart macet polling yang salah. Kemacetan persisten biasanya mengarah ke masalah proxy, DNS, IPv6, atau egress TLS antara host dan `api.telegram.org`.
...
```
    ...
    - Jika host Anda adalah WSL2 atau secara eksplisit bekerja lebih baik dengan perilaku hanya IPv4, paksa pemilihan keluarga:
...
```
    ...
    - Jawaban rentang tolok ukur RFC 2544 (`198.18.0.0/15`) sudah diizinkan
...
```
    ...
    - Keikutsertaan yang sama tersedia per akun di
      `channels.telegram.accounts.<accountId>.network.dangerouslyAllowPrivateNetwork`.
    - Jika proxy Anda menyelesaikan host media Telegram menjadi `198.18.x.x`, biarkan
      flag berbahaya tetap nonaktif terlebih dahulu. Media Telegram sudah mengizinkan
      rentang tolok ukur RFC 2544 secara default.
    ...
    <Warning>
      ...
      menyintesis jawaban privat atau penggunaan-khusus di luar rentang tolok ukur
