...
Zapewniane przez aktywny Plugin pamięci (domyślnie: `memory-core`; ustaw `plugins.slots.memory = "none"`, aby wyłączyć).
...
- `--agent <id>`: ogranicz zakres do jednego agenta. Bez tej opcji te polecenia działają dla każdego skonfigurowanego agenta; jeśli lista agentów nie jest skonfigurowana, wracają do agenta domyślnego.
- `--verbose`: emituj szczegółowe dzienniki podczas sondowania i indeksowania.

`memory status`:

- `--deep`: sprawdź gotowość lokalnego magazynu wektorowego, gotowość dostawcy embeddingów i gotowość semantycznego wyszukiwania wektorowego. Zwykłe `memory status` pozostaje szybkie i nie uruchamia aktywnego generowania embeddingów ani wykrywania dostawcy; nieznany stan magazynu wektorowego lub semantycznych wektorów oznacza, że nie został sprawdzony w tym poleceniu. Leksykalne QMD `searchMode: "search"` pomija semantyczne sondowanie wektorów i utrzymanie embeddingów nawet z `--deep`.
...
- `--limit <n>` -- ogranicz liczbę wyświetlanych kandydatów.
- `--include-promoted` -- uwzględnij wpisy już promowane w poprzednich cyklach.

Pełne opcje:

- Szereguje kandydatów krótkoterminowych z `memory/YYYY-MM-DD.md` przy użyciu ważonych sygnałów promocji (`frequency`, `relevance`, `query diversity`, `recency`, `consolidation`, `conceptual richness`).
- Używa sygnałów krótkoterminowych zarówno z przywołań pamięci, jak i z przebiegów dziennego pozyskiwania, a także sygnałów wzmocnienia z faz light/REM.
- Gdy Dreaming jest włączone, `memory-core` automatycznie zarządza jednym zadaniem Cron, które uruchamia pełny przebieg (`light -> REM -> deep`) w tle (bez potrzeby ręcznego `openclaw cron add`).
- `--agent <id>`: ogranicz zakres do jednego agenta (domyślnie: agent domyślny).
...
- `--include-promoted`: uwzględnij już promowanych kandydatów w danych wyjściowych.
- `--json`: wypisz dane wyjściowe JSON.
...
- `--agent <id>`: ogranicz zakres do jednego agenta (domyślnie: agent domyślny).
- `--include-promoted`: uwzględnij już promowanych kandydatów.
- `--json`: wypisz dane wyjściowe JSON.
...
- Przełączaj z czatu za pomocą `/dreaming on|off` (albo sprawdzaj za pomocą `/dreaming status`).
...
- Ranking używa ważonych sygnałów: częstotliwości przywołań, trafności pobrań, różnorodności zapytań, świeżości czasowej, konsolidacji między dniami i wyprowadzonego bogactwa pojęciowego.
- Promocja ponownie odczytuje aktywną notatkę dzienną przed zapisem do `MEMORY.md`, więc edytowane lub usunięte wycinki krótkoterminowe nie zostają promowane z nieaktualnych migawek magazynu przywołań.
...
- Automatyczne uruchomienia rozchodzą się po skonfigurowanych obszarach roboczych pamięci.

Domyślne harmonogramowanie:

- **Kadencja przebiegu**: `dreaming.frequency = 0 3 * * *`
...
- `memory status` obejmuje wszystkie dodatkowe ścieżki skonfigurowane przez `memorySearch.extraPaths`.
- Jeśli efektywnie aktywne pola klucza zdalnego API pamięci są skonfigurowane jako SecretRefs, polecenie rozwiązuje te wartości z aktywnej migawki Gateway. Jeśli Gateway jest niedostępny, polecenie szybko kończy się błędem.
- Uwaga o rozjeździe wersji Gateway: ta ścieżka polecenia wymaga Gateway obsługującego `secrets.resolve`; starsze Gateway zwracają błąd nieznanej metody.
- Dostrój kadencję zaplanowanego przebiegu za pomocą `dreaming.frequency`. Polityka promocji deep jest poza tym wewnętrzna; użyj flag CLI w `memory promote`, gdy potrzebujesz jednorazowych ręcznych nadpisań.
- `memory rem-harness --path <file-or-dir> --grounded` wyświetla podgląd ugruntowanych sekcji `What Happened`, `Reflections` i `Possible Lasting Updates` z historycznych notatek dziennych bez zapisywania czegokolwiek.
- `memory rem-backfill --path <file-or-dir>` zapisuje odwracalne ugruntowane wpisy dziennika do `DREAMS.md` do przeglądu w UI.
- `memory rem-backfill --path <file-or-dir> --stage-short-term` dodatkowo zasila aktywny magazyn promocji krótkoterminowej ugruntowanymi trwałymi kandydatami, aby normalna faza deep mogła je uszeregować.
...
- Zobacz [Dreaming](/pl/concepts/dreaming), aby uzyskać pełne opisy faz i dokumentację konfiguracji.
