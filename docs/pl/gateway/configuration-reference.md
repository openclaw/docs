...
x-i18n:
    generated_at: "2026-05-10T19:35:29Z"
    model: gpt-5.5
    provider: openai
...
Dokumentacja referencyjna podstawowej konfiguracji dla `~/.openclaw/openclaw.json`. Przegląd zorientowany na zadania znajdziesz w sekcji [Konfiguracja](/pl/gateway/configuration).
...
- `openclaw config schema` wypisuje aktualny JSON Schema używany do walidacji i Control UI, z dołączonymi metadanymi pakietowymi/pluginów/kanałów, gdy są dostępne
...
Ścieżka wyszukiwania dla agenta: użyj akcji narzędzia `gateway` `config.schema.lookup`, aby
przed edycją uzyskać dokładną dokumentację i ograniczenia na poziomie pola. Użyj
[Konfiguracja](/pl/gateway/configuration) jako wskazówek zorientowanych na zadania, a tej strony
jako szerszej mapy pól, wartości domyślnych i odnośników do dokumentacji referencyjnych podsystemów.
...
- strony właścicielskie kanałów/pluginów dla powierzchni poleceń specyficznych dla kanałów
...
kanałów pakietowych (uwierzytelnianie, kontrola dostępu, wiele kont, bramkowanie wzmianek).

## Wartości domyślne agenta, multi-agent, sesje i wiadomości
...
- `talk.*` (tryb Talk)
  - `talk.consultThinkingLevel`: nadpisanie poziomu myślenia dla pełnego uruchomienia agenta OpenClaw za konsultacjami realtime Control UI Talk
  - `talk.consultFastMode`: jednorazowe nadpisanie trybu szybkiego dla konsultacji realtime Control UI Talk
  - `talk.speechLocale`: opcjonalny identyfikator ustawień regionalnych BCP 47 dla rozpoznawania mowy Talk na iOS/macOS
...
Polityka narzędzi, przełączniki eksperymentalne, konfiguracja narzędzi opartych na dostawcach oraz konfiguracja
dostawcy niestandardowego / bazowego URL zostały przeniesione na dedykowaną stronę - zobacz
[Konfiguracja - narzędzia i dostawcy niestandardowi](/pl/gateway/config-tools).

## Modele

Definicje dostawców, listy dozwolonych modeli oraz konfiguracja dostawców niestandardowych znajdują się w
[Konfiguracja - narzędzia i dostawcy niestandardowi](/pl/gateway/config-tools#custom-providers-and-base-urls).
...
- `models.providers`: mapa dostawców niestandardowych indeksowana według identyfikatora dostawcy.
- `models.providers.*.localService`: opcjonalny menedżer procesu uruchamianego na żądanie dla
  lokalnych serwerów modeli. OpenClaw sprawdza skonfigurowany endpoint health, uruchamia
...
- `models.pricing.enabled`: steruje bootstrapem cen w tle, który
  zaczyna się po tym, jak sidecary i kanały osiągną ścieżkę gotowości Gateway. Gdy `false`,
...
używane przez osadzone Pi oraz inne adaptery runtime. Polecenia `openclaw mcp list`,
...
- `mcp.sessionIdleTtlMs`: idle TTL dla runtime'ów MCP pakietowych ograniczonych do sesji.
  Jednorazowe uruchomienia osadzone żądają sprzątania po zakończeniu uruchomienia; ten TTL jest zabezpieczeniem dla
  długotrwałych sesji i przyszłych wywołań.
- Zmiany pod `mcp.*` stosują się na gorąco przez usunięcie z pamięci podręcznej runtime'ów MCP sesji.
  Następne wykrycie/użycie narzędzia odtwarza je z nowej konfiguracji, więc usunięte
  wpisy `mcp.servers` są zbierane natychmiast zamiast czekać na idle TTL.
...
- `allowBundled`: opcjonalna lista dozwolonych wyłącznie dla pakietowych Skills (nie wpływa na Skills zarządzane/obszaru roboczego).
- `load.extraDirs`: dodatkowe współdzielone katalogi główne Skills (najniższy priorytet).
- `load.allowSymlinkTargets`: zaufane rzeczywiste katalogi docelowe, do których symlinki Skills mogą
...
- `install.preferBrew`: gdy true, preferuje instalatory Homebrew, gdy `brew` jest
  dostępny, zanim przejdzie do innych rodzajów instalatorów.
...
- `install.allowUploadedArchives`: pozwala zaufanym klientom Gateway `operator.admin`
  ...
  (domyślnie: false). To włącza tylko ścieżkę przesłanych archiwów; zwykłe
  instalacje ClawHub jej nie wymagają.
- `entries.<skillKey>.enabled: false` wyłącza skill nawet wtedy, gdy jest pakietowy/zainstalowany.
- `entries.<skillKey>.apiKey`: udogodnienie dla Skills deklarujących podstawową zmienną env (jawny ciąg tekstowy lub obiekt SecretRef).
...
- Wykrywanie akceptuje natywne pluginy OpenClaw oraz zgodne pakiety Codex i pakiety Claude, w tym bezmanifestowe pakiety Claude z domyślnym układem.
...
- `bundledDiscovery`: domyślnie `"allowlist"` dla nowych konfiguracji, więc niepusta
  `plugins.allow` bramkuje także pakietowe pluginy dostawców, w tym dostawców runtime
  web-search. Doctor zapisuje `"compat"` dla zmigrowanych starszych konfiguracji listy dozwolonych,
  aby zachować istniejące zachowanie pakietowych dostawców, dopóki świadomie go nie zmienisz.
...
- `plugins.entries.<id>.hooks.allowConversationAccess`: gdy `true`, zaufane niepakietowe pluginy mogą czytać surową treść konwersacji z typowanych hooków, takich jak `llm_input`, `llm_output`, `before_model_resolve`, `before_agent_reply`, `before_agent_run`, `before_agent_finalize` i `agent_end`.
...
- `plugins.entries.<id>.llm.allowedModels`: opcjonalna lista dozwolonych kanonicznych celów `provider/model` dla zaufanych nadpisań uzupełniania LLM pluginu. Używaj `"*"` tylko wtedy, gdy celowo chcesz zezwolić na dowolny model.
...
- Ustawienia konta/runtime pluginu kanału znajdują się pod `channels.<id>` i powinny być opisywane przez metadane `channelConfigs` manifestu pluginu właścicielskiego, a nie przez centralny rejestr opcji OpenClaw.

### Konfiguracja pluginu harness Codex

Pakietowy plugin `codex` jest właścicielem natywnych ustawień harness serwera aplikacji Codex pod
`plugins.entries.codex.config`. Zobacz
[Dokumentacja referencyjna harness Codex](/pl/plugins/codex-harness-reference), aby poznać pełną powierzchnię konfiguracji,
oraz [Harness Codex](/pl/plugins/codex-harness), aby poznać model runtime.

`codexPlugins` dotyczy tylko sesji, które wybierają natywny harness Codex.
Nie włącza pluginów Codex dla Pi, zwykłych uruchomień dostawcy OpenAI, powiązań konwersacji ACP
ani żadnego harness innego niż Codex.
...
- `plugins.entries.codex.config.codexPlugins.enabled`: włącza natywną obsługę
  pluginów/aplikacji Codex dla uprzęży Codex. Domyślnie: `false`.
- `plugins.entries.codex.config.codexPlugins.allow_destructive_actions`:
  domyślna polityka akcji destrukcyjnych dla zmigrowanych wywołań aplikacji pluginów.
  Domyślnie: `false`.
- `plugins.entries.codex.config.codexPlugins.plugins.<key>.enabled`: włącza
  zmigrowany wpis pluginu, gdy globalne `codexPlugins.enabled` ma również wartość true.
...
- `plugins.entries.codex.config.codexPlugins.plugins.<key>.allow_destructive_actions`:
  nadpisanie polityki akcji destrukcyjnych dla pojedynczego pluginu. Gdy pominięte, używana jest globalna
  wartość `allow_destructive_actions`.

`codexPlugins.enabled` jest globalną dyrektywą włączenia. Jawne wpisy pluginów
zapisane przez migrację są trwałym zestawem kwalifikującym się do instalacji i naprawy.
...
asynchronicznie po utracie świeżości. Konfiguracja aplikacji wątku Codex jest obliczana podczas
ustanawiania sesji uprzęży Codex, nie przy każdej turze; użyj `/new`, `/reset` lub restartu Gateway
po zmianie natywnej konfiguracji pluginów.
...
- Włączone pluginy pakietu Claude mogą również dostarczać osadzone wartości domyślne Pi z `settings.json`; OpenClaw stosuje je jako oczyszczone ustawienia agenta, a nie jako surowe poprawki konfiguracji OpenClaw.
...
`commitments` kontroluje wywnioskowaną pamięć działań następczych: OpenClaw może wykrywać zgłoszenia kontrolne z tur rozmowy i dostarczać je przez uruchomienia Heartbeat.

- `commitments.enabled`: włącza ukryte wyodrębnianie przez LLM, przechowywanie i dostarczanie przez Heartbeat wywnioskowanych zobowiązań do działań następczych. Domyślnie: `false`.
- `commitments.maxPerDay`: maksymalna liczba wywnioskowanych zobowiązań do działań następczych dostarczanych na sesję agenta w przesuwającym się dniu. Domyślnie: `3`.
...
- W trybie restrykcyjnym zdalne punkty końcowe profili CDP (`profiles.*.cdpUrl`) podlegają temu samemu blokowaniu sieci prywatnych podczas sprawdzania osiągalności/wykrywania.
...
- `remoteCdpTimeoutMs` i `remoteCdpHandshakeTimeoutMs` dotyczą zdalnej osiągalności CDP oraz osiągalności CDP
  `attachOnly`, a także żądań otwierania kart. Zarządzane profile local loopback
...
- Jeśli zewnętrznie zarządzana usługa CDP jest osiągalna przez local loopback, ustaw dla tego
  profilu `attachOnly: true`; w przeciwnym razie OpenClaw traktuje port local loopback jako
...
- Profile `existing-session` zachowują obecne limity tras Chrome MCP:
  działania oparte na migawkach/referencjach zamiast wskazywania selektorami CSS, haki wysyłania
...
- `browser.executablePath` i `browser.profiles.<name>.executablePath` akceptują zarówno
  ...
  `userDataDir` na poziomie profilu w profilach `existing-session` również rozwija tyldę.
- Usługa sterowania: tylko local loopback (port wyprowadzony z `gateway.port`, domyślnie `18791`).
...
- `assistant`: nadpisanie tożsamości interfejsu sterowania. W razie braku używana jest tożsamość aktywnego agenta.
...
- **Starsze aliasy bindowania**: używaj wartości trybu bindowania w `gateway.bind` (`auto`, `loopback`, `lan`, `tailnet`, `custom`), a nie aliasów hosta (`0.0.0.0`, `127.0.0.1`, `localhost`, `::`, `::1`).
...
- **Uwierzytelnianie**: domyślnie wymagane. Bindowania inne niż loopback wymagają uwierzytelniania Gateway. W praktyce oznacza to współdzielony token/hasło albo reverse proxy świadome tożsamości z `gateway.auth.mode: "trusted-proxy"`. Kreator onboardingu domyślnie generuje token.
- Jeśli skonfigurowano jednocześnie `gateway.auth.token` i `gateway.auth.password` (w tym SecretRefs), ustaw jawnie `gateway.auth.mode` na `token` lub `password`. Przepływy uruchamiania oraz instalacji/naprawy usługi kończą się niepowodzeniem, gdy oba są skonfigurowane, a tryb nie jest ustawiony.
- `gateway.auth.mode: "none"`: jawny tryb bez uwierzytelniania. Używaj tylko dla zaufanych konfiguracji local loopback; celowo nie jest on oferowany w monitach onboardingu.
- `gateway.auth.mode: "trusted-proxy"`: deleguje uwierzytelnianie przeglądarki/użytkownika do reverse proxy świadomego tożsamości i ufa nagłówkom tożsamości z `gateway.trustedProxies` (zobacz [Uwierzytelnianie przez zaufane proxy](/pl/gateway/trusted-proxy-auth)). Ten tryb domyślnie oczekuje źródła proxy **innego niż loopback**; reverse proxy na tym samym hoście przez loopback wymagają jawnego `gateway.auth.trustedProxy.allowLoopback = true`. Wewnętrzni wywołujący z tego samego hosta mogą używać `gateway.auth.password` jako lokalnej bezpośredniej opcji awaryjnej; `gateway.auth.token` pozostaje wzajemnie wykluczające z trybem trusted-proxy.
...
- `gateway.auth.rateLimit`: opcjonalny limiter nieudanego uwierzytelniania. Działa według adresu IP klienta i zakresu uwierzytelniania (shared-secret i device-token są śledzone niezależnie). Zablokowane próby zwracają `429` + `Retry-After`.
  - Na asynchronicznej ścieżce interfejsu Control UI Tailscale Serve nieudane próby dla tego samego `{scope, clientIp}` są serializowane przed zapisem niepowodzenia. Współbieżne błędne próby od tego samego klienta mogą więc uruchomić limiter przy drugim żądaniu, zamiast przejść równolegle jako zwykłe niezgodności.
...
- Próby uwierzytelniania WS z originu przeglądarki są zawsze ograniczane z wyłączonym wyjątkiem dla loopback (dodatkowa warstwa ochrony przed przeglądarkowym atakiem brute force na localhost).
- Na loopback te blokady z originu przeglądarki są izolowane według znormalizowanej wartości `Origin`, więc powtarzające się niepowodzenia z jednego originu localhost nie blokują automatycznie innego originu.
- `tailscale.mode`: `serve` (tylko tailnet, bindowanie loopback) lub `funnel` (publiczne, wymaga uwierzytelniania).
...
- `controlUi.allowedOrigins`: jawna lista dozwolonych originów przeglądarki dla połączeń Gateway WebSocket. Wymagana, gdy oczekuje się klientów przeglądarkowych z originów innych niż loopback.
...
- `controlUi.dangerouslyAllowHostHeaderOriginFallback`: niebezpieczny tryb włączający fallback originu z nagłówka Host dla wdrożeń, które celowo polegają na polityce originu z nagłówka Host.
- `remote.transport`: `ssh` (domyślnie) lub `direct` (ws/wss). Dla `direct` wartość `remote.url` musi być `ws://` lub `wss://`.
- `OPENCLAW_ALLOW_INSECURE_PRIVATE_WS=1`: awaryjne nadpisanie po stronie klienta przez środowisko procesu, które pozwala na tekst jawny `ws://` do zaufanych adresów IP sieci prywatnej; domyślnie tekst jawny pozostaje ograniczony do loopback. Nie ma odpowiednika w `openclaw.json`, a konfiguracja prywatnej sieci przeglądarki, taka jak `browser.ssrfPolicy.dangerouslyAllowPrivateNetwork`, nie wpływa na klientów Gateway WebSocket.
- `gateway.remote.token` / `.password` to pola poświadczeń zdalnego klienta. Same nie konfigurują uwierzytelniania Gateway.
- `gateway.push.apns.relay.baseUrl`: bazowy URL HTTPS dla zewnętrznego przekaźnika APNs używanego przez oficjalne/TestFlight buildy iOS po opublikowaniu w Gateway rejestracji obsługiwanych przez przekaźnik. Ten URL musi odpowiadać URL-owi przekaźnika skompilowanemu w buildzie iOS.
...
- `gateway.handshakeTimeoutMs`: limit czasu przeduwierzytelnieniowego uzgadniania Gateway WebSocket w milisekundach. Domyślnie: `15000`. `OPENCLAW_HANDSHAKE_TIMEOUT_MS` ma pierwszeństwo, gdy jest ustawione. Zwiększ tę wartość na obciążonych lub słabszych hostach, gdzie lokalni klienci mogą się połączyć, gdy rozgrzewanie po uruchomieniu jeszcze się stabilizuje.
...
- `channels.<provider>.healthMonitor.enabled`: wyłączenie restartów monitora kondycji dla konkretnego kanału przy zachowaniu włączonego monitora globalnego.
- `channels.<provider>.accounts.<accountId>.healthMonitor.enabled`: nadpisanie dla konkretnego konta w kanałach wielokontowych. Gdy jest ustawione, ma pierwszeństwo przed nadpisaniem na poziomie kanału.
...
- `trustedProxies`: adresy IP reverse proxy, które terminują TLS lub wstrzykują nagłówki przekazanego klienta. Wymieniaj tylko proxy, które kontrolujesz. Wpisy loopback nadal są poprawne dla konfiguracji proxy/lokalnego wykrywania na tym samym hoście (na przykład Tailscale Serve lub lokalne reverse proxy), ale **nie** sprawiają, że żądania loopback kwalifikują się do `gateway.auth.mode: "trusted-proxy"`.
...
- `gateway.nodes.pairing.autoApproveCidrs`: opcjonalna lista dozwolonych CIDR/IP do automatycznego zatwierdzania pierwszego parowania urządzenia węzła bez żądanych zakresów. Jest wyłączona, gdy nie jest ustawiona. Nie zatwierdza automatycznie parowania operatora/przeglądarki/Control UI/WebChat ani nie zatwierdza automatycznie podniesień roli, zakresu, metadanych lub klucza publicznego.
- `gateway.nodes.allowCommands` / `gateway.nodes.denyCommands`: globalne kształtowanie allow/deny dla deklarowanych poleceń węzła po parowaniu i ocenie listy dozwolonych platformy. Użyj `allowCommands`, aby włączyć niebezpieczne polecenia węzła, takie jak `camera.snap`, `camera.clip` i `screen.record`; `denyCommands` usuwa polecenie nawet wtedy, gdy domyślna wartość platformy lub jawne zezwolenie inaczej by je obejmowały. Po zmianie deklarowanej listy poleceń przez węzeł odrzuć i ponownie zatwierdź parowanie tego urządzenia, aby Gateway zapisał zaktualizowaną migawkę poleceń.
...
- Wzmocnienia dla wejścia URL w Responses:
  - `gateway.http.endpoints.responses.maxUrlParts`
  - `gateway.http.endpoints.responses.files.urlAllowlist`
  - `gateway.http.endpoints.responses.images.urlAllowlist`
    Puste listy dozwolonych są traktowane jak nieustawione; użyj `gateway.http.endpoints.responses.files.allowUrl=false` i/lub `gateway.http.endpoints.responses.images.allowUrl=false`, aby wyłączyć pobieranie URL-i.
...
- `autoGenerate`: automatycznie generuje lokalną parę certyfikat/klucz z podpisem własnym, gdy jawne pliki nie są skonfigurowane; tylko do użytku lokalnego/deweloperskiego.
...
- `mode`: kontroluje sposób stosowania zmian konfiguracji w czasie działania.
  ...
  - `"hot"`: zastosuj zmiany w procesie bez restartu.
...
Tokeny hook w ciągu zapytania są odrzucane.
...
- Jeśli mapowanie lub preset używa szablonowanego `sessionKey`, ustaw `hooks.allowedSessionKeyPrefixes` oraz `hooks.allowRequestSessionKey=true`. Statyczne klucze mapowania nie wymagają takiej zgody.
...
- `transform` może wskazywać moduł JS/TS zwracający akcję hook.
  ...
  - Trzymaj `hooks.transformsDir` pod `~/.openclaw/hooks/transforms`; katalogi workspace Skills są odrzucane. Jeśli `openclaw doctor` zgłasza tę ścieżkę jako nieprawidłową, przenieś moduł transformacji do katalogu transformacji hook albo usuń `hooks.transformsDir`.
...
- `model` zastępuje LLM dla tego uruchomienia hook (musi być dozwolony, jeśli ustawiono katalog modeli).
...
- Jeśli zachowujesz to routowanie per wiadomość, ustaw `hooks.allowRequestSessionKey: true` i ogranicz `hooks.allowedSessionKeyPrefixes` tak, aby pasowało do przestrzeni nazw Gmail, na przykład `["hook:", "hook:gmail:"]`.
- Jeśli potrzebujesz `hooks.allowRequestSessionKey: false`, zastąp preset statycznym `sessionKey` zamiast szablonowanej wartości domyślnej.
...
- Node WebViews zwykle nie wysyłają nagłówków uwierzytelniania; po sparowaniu i połączeniu węzła Gateway ogłasza adresy URL capability o zakresie węzła do dostępu canvas/A2UI.
- Adresy URL capability są powiązane z aktywną sesją WS węzła i szybko wygasają. Rezerwowe rozwiązanie oparte na IP nie jest używane.
...
- `full`: dołącza `cliPath` + `sshPort`; rozgłaszanie multicast w LAN nadal wymaga włączenia dołączonego Pluginu `bonjour`.
- `off`: wyłącza rozgłaszanie multicast w LAN bez zmiany włączenia Pluginu.
- Dołączony Plugin `bonjour` uruchamia się automatycznie na hostach macOS, a na Linux, Windows i konteneryzowanych wdrożeniach Gateway wymaga włączenia.
- Nazwa hosta domyślnie jest nazwą hosta systemowego, gdy jest prawidłową etykietą DNS; w przeciwnym razie używa `openclaw`. Nadpisz za pomocą `OPENCLAW_MDNS_HOSTNAME`.
...
Odwołania do sekretów są addytywne: wartości zapisane jawnie nadal działają.
...
- Macierz kanoniczna: [Obszar poświadczeń SecretRef](/pl/reference/secretref-credential-surface)
- Cele `secrets apply` obsługują ścieżki poświadczeń w `openclaw.json`.
...
- Ścieżki dostawców file i exec kończą się błędem zamkniętym, gdy weryfikacja ACL Windows jest niedostępna. Ustaw `allowInsecurePath: true` tylko dla zaufanych ścieżek, których nie można zweryfikować.
- Dostawca `exec` wymaga bezwzględnej ścieżki `command` i używa ładunków protokołu na stdin/stdout.
- Domyślnie ścieżki poleceń będące dowiązaniami symbolicznymi są odrzucane. Ustaw `allowSymlinkCommand: true`, aby zezwolić na ścieżki dowiązań symbolicznych przy jednoczesnej walidacji rozpoznanej ścieżki docelowej.
- Jeśli skonfigurowano `trustedDirs`, sprawdzanie zaufanego katalogu dotyczy rozpoznanej ścieżki docelowej.
...
- Filtrowanie aktywnego obszaru jest stosowane podczas aktywacji: nierozwiązane odwołania na włączonych obszarach powodują niepowodzenie uruchomienia/przeładowania, a nieaktywne obszary są pomijane z diagnostyką.
...
- `billingBackoffHours`: bazowy backoff w godzinach, gdy profil zawodzi z powodu rzeczywistych
...
- `authPermanentBackoffMinutes`: bazowy backoff w minutach dla wysoce pewnych błędów `auth_permanent` (domyślnie: `10`).
...
- `overloadedProfileRotations`: maksymalna liczba rotacji profili uwierzytelniania tego samego dostawcy dla błędów przeciążenia przed przełączeniem na zapasowy model (domyślnie: `1`). Kształty zajętego dostawcy, takie jak `ModelNotReadyException`, trafiają tutaj.
- `overloadedBackoffMs`: stałe opóźnienie przed ponowieniem rotacji przeciążonego dostawcy/profilu (domyślnie: `0`).
- `rateLimitedProfileRotations`: maksymalna liczba rotacji profili uwierzytelniania tego samego dostawcy dla błędów limitu szybkości przed przełączeniem na zapasowy model (domyślnie: `1`). Ten koszyk limitu szybkości obejmuje tekst ukształtowany przez dostawcę, taki jak `Too many concurrent requests`, `ThrottlingException`, `concurrency limit reached`, `workers_ai ... quota limit exceeded` i `resource exhausted`.
...
- `redactSensitive` / `redactPatterns`: najlepsze możliwe maskowanie danych wyjściowych konsoli, dzienników plikowych, rekordów dziennika OTLP i utrwalonego tekstu transkrypcji sesji. `redactSensitive: "off"` wyłącza tylko tę ogólną politykę dziennika/transkrypcji; powierzchnie bezpieczeństwa UI/narzędzi/diagnostyki nadal redagują sekrety przed emisją.
...
- `cacheTrace.enabled`: zapisuje migawki śladu pamięci podręcznej dla uruchomień osadzonych (domyślnie: `false`).
...
- `cacheTrace.includeMessages` / `includePrompt` / `includeSystem`: sterują tym, co jest uwzględniane w danych wyjściowych śladu pamięci podręcznej (wszystkie domyślnie: `true`).
...
- `auto.betaCheckIntervalHours`: częstotliwość uruchamiania kontroli kanału beta w godzinach (domyślnie: `1`; maks.: `24`).
...
- `enabled`: globalna bramka funkcji ACP (domyślnie: `true`; ustaw `false`, aby ukryć wysyłanie ACP i ułatwienia tworzenia).
...
- `backend`: domyślny identyfikator backendu środowiska wykonawczego ACP (musi pasować do zarejestrowanego Plugin środowiska wykonawczego ACP).
  Najpierw zainstaluj Plugin backendu, a jeśli ustawiono `plugins.allow`, uwzględnij identyfikator Plugin backendu (na przykład `acpx`), inaczej backend ACP nie zostanie załadowany.
- `defaultAgent`: zapasowy identyfikator agenta docelowego ACP, gdy tworzone sesje nie określają jawnego celu.
- `allowedAgents`: lista dozwolonych identyfikatorów agentów dla sesji środowiska wykonawczego ACP; pusta oznacza brak dodatkowych ograniczeń.
...
- `stream.repeatSuppression`: tłumi powtarzane linie statusu/narzędzi w obrębie tury (domyślnie: `true`).
...
- `stream.maxOutputChars`: maksymalna liczba znaków danych wyjściowych asystenta projektowana na turę ACP.
...
Bieżące kompilacje nie zawierają już mostka TCP. Komponenty Node łączą się przez WebSocket Gateway. Klucze `bridge.*` nie są już częścią schematu konfiguracji (walidacja kończy się niepowodzeniem, dopóki nie zostaną usunięte; `openclaw doctor --fix` może usunąć nieznane klucze).
...
- `sessionRetention`: czas przechowywania ukończonych izolowanych sesji uruchomień cron przed usunięciem z `sessions.json`. Kontroluje także czyszczenie zarchiwizowanych transkrypcji usuniętych cron. Domyślnie: `24h`; ustaw `false`, aby wyłączyć.
- `runLog.maxBytes`: maksymalny rozmiar pliku dziennika pojedynczego uruchomienia (`cron/runs/<jobId>.jsonl`) przed przycięciem. Domyślnie: `2_000_000` bajtów.
- `runLog.keepLines`: najnowsze wiersze zachowywane po uruchomieniu przycinania dziennika uruchomień. Domyślnie: `2000`.
- `webhookToken`: token bearer używany do dostarczania POST cron Webhook (`delivery.mode = "webhook"`); jeśli pominięty, nagłówek auth nie jest wysyłany.
...
- `includeSkipped`: zlicza kolejne pominięte uruchomienia do progu alertu (domyślnie: `false`). Pominięte uruchomienia są śledzone osobno i nie wpływają na wycofywanie po błędach wykonania.
...
- `mode`: `"announce"` lub `"webhook"`; domyślnie `"announce"`, gdy dostępne są wystarczające dane celu.
...
- Gdy nie ustawiono ani globalnego, ani właściwego dla zadania miejsca docelowego niepowodzeń, zadania, które już dostarczają przez `announce`, przy niepowodzeniu wracają do tego głównego celu announce.
...
- Dołączenia główne, tablice dołączeń i dołączenia z nadpisaniami kluczy równorzędnych są tylko do odczytu dla zapisów należących do OpenClaw; takie zapisy kończą się zamkniętym niepowodzeniem zamiast spłaszczać konfigurację.
