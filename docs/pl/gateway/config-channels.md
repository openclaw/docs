...
read_when:
    ...
    - Audyt zasad dotyczących wiadomości bezpośrednich, grup lub bramkowania wzmianek
summary: 'Konfiguracja kanałów: kontrola dostępu, parowanie, klucze dla poszczególnych kanałów w Slack, Discord, Telegram, WhatsApp, Matrix, iMessage i innych kanałach'
...
| --------------------- | ------------------------------------------------------ |
| `allowlist` (domyślnie) | Tylko grupy zgodne ze skonfigurowaną listą zezwoleń          |
...
Jeśli blok dostawcy całkowicie nie istnieje (brak `channels.<provider>`), zasada grup w środowisku uruchomieniowym wraca do `allowlist` (domyślnie blokuje) z ostrzeżeniem podczas uruchamiania.
</Note>
...
Użyj `channels.defaults` dla współdzielonej zasady grup i zachowania Heartbeat u różnych dostawców:
...
- `channels.defaults.contextVisibility`: domyślny tryb widoczności kontekstu uzupełniającego dla wszystkich kanałów. Wartości: `all` (domyślnie, obejmuje cały cytowany/wątkowy/historyczny kontekst), `allowlist` (obejmuje tylko kontekst od nadawców z listy zezwoleń), `allowlist_quote` (tak samo jak allowlist, ale zachowuje jawny kontekst cytatu/odpowiedzi). Nadpisanie dla kanału: `channels.<channel>.contextVisibility`.
...
- `apiRoot` to wyłącznie katalog główny Telegram Bot API. Użyj `https://api.telegram.org` lub własnego hostowanego/proxy katalogu głównego, a nie `https://api.telegram.org/bot<TOKEN>`; `openclaw doctor --fix` usuwa przypadkowy końcowy sufiks `/bot<TOKEN>`.
...
- W konfiguracjach wielu kont (co najmniej 2 identyfikatory kont) ustaw jawną wartość domyślną (`channels.telegram.defaultAccount` lub `channels.telegram.accounts.default`), aby uniknąć zapasowego routingu; `openclaw doctor` ostrzega, gdy jej brakuje lub jest nieprawidłowa.
...
- Bezpośrednie wywołania wychodzące, które podają jawny `token` Discord, używają tego tokena dla danego wywołania; ustawienia ponawiania/polityki konta nadal pochodzą z wybranego konta w aktywnej migawce runtime.
...
- Slugi gildii są zapisywane małymi literami, a spacje są zastępowane przez `-`; klucze kanałów używają nazwy w postaci sluga (bez `#`). Preferuj identyfikatory gildii.
...
- `channels.discord.threadBindings` steruje routowaniem powiązanym z wątkami Discord:
  ...
  - `spawnSessions`: przełącznik dla `sessions_spawn({ thread: true })` oraz automatycznego tworzenia/powiązywania wątków przez ACP thread-spawn (domyślnie: `true`)
  - `defaultSpawnContext`: natywny kontekst subagenta dla uruchomień powiązanych z wątkiem (domyślnie `"fork"`)
- Wpisy najwyższego poziomu `bindings[]` z `type: "acp"` konfigurują trwałe powiązania ACP dla kanałów i wątków (użyj identyfikatora kanału/wątku w `match.peer.id`). Semantyka pól jest wspólna i opisana w [Agentach ACP](/pl/tools/acp-agents#persistent-channel-bindings).
...
- `channels.discord.voice.reconnectGraceMs` kontroluje, jak długo rozłączona sesja głosowa może wchodzić w sygnalizację ponownego połączenia, zanim OpenClaw ją zniszczy (domyślnie `15000`).
...
- `channels.discord.execApprovals`: natywne dla Discord dostarczanie zatwierdzeń exec i autoryzacja zatwierdzających.
  ...
  - `target`: miejsce wysyłania monitów zatwierdzenia. `"dm"` (domyślnie) wysyła do DM zatwierdzających, `"channel"` wysyła do kanału źródłowego, `"both"` wysyła do obu. Gdy cel zawiera `"channel"`, przyciski są dostępne tylko dla rozwiązanych zatwierdzających.
...
- JSON konta usługi: inline (`serviceAccount`) albo oparty na pliku (`serviceAccountFile`).
- Obsługiwany jest także SecretRef konta usługi (`serviceAccountRef`).
- Wartości awaryjne env: `GOOGLE_CHAT_SERVICE_ACCOUNT` albo `GOOGLE_CHAT_SERVICE_ACCOUNT_FILE`.
- Użyj `spaces/<spaceId>` albo `users/<userId>` jako celów dostarczania.
- `channels.googlechat.dangerouslyAllowNameMatching` ponownie włącza dopasowywanie po zmiennych pryncypałach e-mail (awaryjny tryb zgodności).
...
- **Tryb gniazda** wymaga zarówno `botToken`, jak i `appToken` (`SLACK_BOT_TOKEN` + `SLACK_APP_TOKEN` jako wartość awaryjna env dla konta domyślnego).
- **Tryb HTTP** wymaga `botToken` oraz `signingSecret` (na poziomie głównym albo dla konta).
- `socketMode` przekazuje dostrajanie transportu Socket Mode SDK Slack do publicznego API odbiornika Bolt. Używaj go tylko podczas badania limitów czasu ping/pong lub nieaktualnego zachowania websocketu.
...
- `channels.slack.streaming.mode` jest kanonicznym kluczem trybu strumienia Slack. `channels.slack.streaming.nativeTransport` kontroluje natywny transport strumieniowania Slack. Starsze wartości `streamMode`, logiczne wartości `streaming` i `nativeStreaming` pozostają aliasami runtime; uruchom `openclaw doctor --fix`, aby przepisać utrwaloną konfigurację.
- `unfurlLinks` i `unfurlMedia` przekazują wartości logiczne rozwijania linków i multimediów `chat.postMessage` Slack dla odpowiedzi bota. Pomiń je, aby zachować domyślne zachowanie Slack; ustaw je w `channels.slack.accounts.<accountId>`, aby nadpisać domyślną wartość najwyższego poziomu dla jednego konta.
...
**Izolacja sesji wątku:** `thread.historyScope` jest osobne dla wątku (domyślnie) albo współdzielone w kanale. `thread.inheritParent` kopiuje transkrypt kanału nadrzędnego do nowych wątków.

- Natywne strumieniowanie Slack oraz status wątku w stylu asystenta Slack „pisze...” wymagają celu wątku odpowiedzi. DM najwyższego poziomu domyślnie pozostają poza wątkiem, więc nadal mogą strumieniować przez szkice Slack publikowane i edytowane jako podglądy zamiast pokazywania natywnego podglądu strumienia/statusu w stylu wątku.
- `typingReaction` dodaje tymczasową reakcję do przychodzącej wiadomości Slack, gdy odpowiedź jest w toku, a następnie usuwa ją po zakończeniu. Użyj krótkiego kodu emoji Slack, takiego jak `"hourglass_flowing_sand"`.
...
niestandardowe kompilacje mogą zainstalować aktualny pakiet npm poleceniem
`openclaw plugins install @openclaw/mattermost`. Sprawdź
[npmjs.com/package/@openclaw/mattermost](https://www.npmjs.com/package/@openclaw/mattermost)
dla bieżących tagów dystrybucji przed przypięciem wersji.
...
- Opcjonalne `channels.mattermost.defaultAccount` nadpisuje domyślny wybór konta, gdy pasuje do identyfikatora skonfigurowanego konta.
...
- Opcjonalne `channels.signal.defaultAccount` nadpisuje domyślny wybór konta, gdy pasuje do identyfikatora skonfigurowanego konta.

### iMessage

OpenClaw uruchamia `imsg rpc` (JSON-RPC przez stdio). Nie jest wymagany daemon ani port. To preferowana ścieżka dla nowych konfiguracji iMessage w OpenClaw, gdy host może nadać uprawnienia do bazy danych Wiadomości i Automatyzacji.

Obsługa BlueBubbles została usunięta. Przenieś konfiguracje `channels.bluebubbles` do `channels.imessage`; OpenClaw obsługuje iMessage tylko przez `imsg`.

Jeśli Gateway nie działa na Macu zalogowanym do Wiadomości, pozostaw `channels.imessage.enabled=true` i ustaw `channels.imessage.cliPath` na wrapper SSH, który uruchamia `imsg "$@"` na tym Macu. Domyślna lokalna ścieżka `imsg` działa tylko w macOS.
...
- Opcjonalne `channels.imessage.defaultAccount` nadpisuje domyślny wybór konta, gdy pasuje do identyfikatora skonfigurowanego konta.
...
- `cliPath` może wskazywać wrapper SSH; ustaw `remoteHost` (`host` albo `user@host`) na potrzeby pobierania załączników przez SCP.
...
- Próby statusu Matrix i bieżące wyszukiwania katalogu używają tej samej polityki proxy co ruch w czasie działania.
- Pełna konfiguracja Matrix, reguły kierowania i przykłady konfiguracji są udokumentowane w [Matrix](/pl/channels/matrix).

### Microsoft Teams

Microsoft Teams jest oparte na Plugin i konfigurowane w `channels.msteams`.
...
- Omówione tutaj główne ścieżki kluczy: `channels.msteams`, `channels.msteams.configWrites`.
- Pełna konfiguracja Teams (poświadczenia, webhook, polityka DM/grup, nadpisania dla zespołów i kanałów) jest udokumentowana w [Microsoft Teams](/pl/channels/msteams).

### IRC

IRC jest oparte na Plugin i konfigurowane w `channels.irc`.
...
- Omówione tutaj główne ścieżki kluczy: `channels.irc`, `channels.irc.dmPolicy`, `channels.irc.configWrites`, `channels.irc.nickserv.*`.
...
- Bazowe ustawienia kanału mają zastosowanie do wszystkich kont, chyba że zostaną nadpisane dla konkretnego konta.
- Użyj `bindings[].match.accountId`, aby skierować każde konto do innego agenta.
- Jeśli dodasz konto inne niż domyślne za pomocą `openclaw channels add` (albo onboardingu kanału), pozostając nadal przy jednokontowej konfiguracji kanału najwyższego poziomu, OpenClaw najpierw przenosi jednokontowe wartości najwyższego poziomu objęte zakresem konta do mapy kont kanału, aby pierwotne konto nadal działało. Większość kanałów przenosi je do `channels.<channel>.accounts.default`; Matrix może zamiast tego zachować istniejący pasujący nazwany/domyślny cel.
...
- `openclaw doctor --fix` także naprawia mieszane kształty, przenosząc jednokontowe wartości najwyższego poziomu objęte zakresem konta do promowanego konta wybranego dla tego kanału. Większość kanałów używa `accounts.default`; Matrix może zamiast tego zachować istniejący pasujący nazwany/domyślny cel.
...
model wygenerował prywatną odpowiedź końcową zamiast wywołać narzędzie wiadomości.
...
- **Wzmianki metadanych**: Natywne platformowe wzmianki @. Ignorowane w trybie czatu z samym sobą w WhatsApp.
...
`messages.visibleReplies` to globalna wartość domyślna dla tur źródłowych; `messages.groupChat.visibleReplies` nadpisuje ją dla tur źródłowych grup/kanałów. Gdy `messages.visibleReplies` nie jest ustawione, harness może podać własną domyślną wartość dla tur bezpośrednich/źródłowych; harness Codex domyślnie używa `message_tool`. Listy dozwolonych kanałów i bramkowanie wzmianek nadal decydują, czy tura jest przetwarzana.
...
Kolejność rozstrzygania: nadpisanie dla konkretnego DM → domyślna wartość dostawcy → brak limitu (wszystko zachowane).
...
- Ta strona jest **odniesieniem do kluczy konfiguracji**, a nie pełnym katalogiem poleceń. Polecenia należące do kanałów/Pluginów, takie jak QQ Bot `/bot-ping` `/bot-help` `/bot-logs`, LINE `/card`, parowanie urządzeń `/pair`, pamięć `/dreaming`, sterowanie telefonem `/phone` i Talk `/voice`, są udokumentowane na stronach swoich kanałów/Pluginów oraz w [Poleceniach Slash](/pl/tools/slash-commands).
...
- `plugins: true` włącza `/plugins` dla wykrywania, instalowania oraz włączania/wyłączania Pluginów.
- `channels.<provider>.configWrites` bramkuje mutacje konfiguracji dla kanału (domyślnie: true).
- W przypadku kanałów wielokontowych `channels.<provider>.accounts.<id>.configWrites` także bramkuje zapisy kierowane do tego konta (na przykład `/allowlist --config --account <id>` lub `/config set channels.<provider>.accounts.<id>...`).
- `restart: false` wyłącza `/restart` i akcje narzędzia restartu Gateway. Domyślnie: `true`.
- `ownerAllowFrom` to jawna lista dozwolonych właścicieli dla poleceń/narzędzi dostępnych tylko właścicielowi. Jest oddzielna od `allowFrom`.
...
- `allowFrom` jest ustawiane dla dostawcy. Gdy jest ustawione, jest **jedynym** źródłem autoryzacji (listy dozwolonych kanałów/parowanie i `useAccessGroups` są ignorowane).
