---
read_when:
    - Potrzebujesz informacji referencyjnych o konfiguracji modeli dla poszczególnych dostawców
...
x-i18n:
    generated_at: "2026-05-06T09:08:31Z"
    model: gpt-5.5
    provider: openai
...
Dokumentacja dla **dostawców LLM/modeli** (nie kanałów czatu takich jak WhatsApp/Telegram). Reguły wyboru modeli znajdziesz w sekcji [Modele](/pl/concepts/models).
...
<AccordionGroup>
  ...
  <Accordion title="OpenAI provider/runtime split">
    ...
    Zobacz [OpenAI](/pl/providers/openai) i [Uprząż Codex](/pl/plugins/codex-harness). Jeśli podział dostawca/runtime jest niejasny, najpierw przeczytaj [Runtime agentów](/pl/concepts/agent-runtimes).
...
Pełna lista hooków SDK dostawcy oraz przykładów wbudowanych Plugin znajduje się w sekcji [Plugin dostawców](/pl/plugins/sdk-provider-plugins). Dostawca, który potrzebuje całkowicie niestandardowego wykonawcy żądań, jest osobną, głębszą powierzchnią rozszerzeń.

<Note>
Zachowanie runnera należące do dostawcy znajduje się w jawnych hookach dostawcy, takich jak polityka odtwarzania, normalizacja schematów narzędzi, opakowywanie strumienia oraz pomocnicze funkcje transportu/żądań. Starszy statyczny worek `ProviderPlugin.capabilities` służy tylko zgodności i nie jest już odczytywany przez współdzieloną logikę runnera.
</Note>
...
<CardGroup cols={3}>
  ...
  <Card title="MiniMax" href="/pl/providers/minimax">
    OAuth MiniMax Coding Plan albo dostęp z kluczem API.
  </Card>
...
<Warning>
OAuth Gemini CLI w OpenClaw jest nieoficjalną integracją. Niektórzy użytkownicy zgłaszali ograniczenia kont Google po użyciu klientów zewnętrznych. Przejrzyj warunki Google i użyj konta niekrytycznego, jeśli zdecydujesz się kontynuować.
</Warning>
...
- [Odwołanie konfiguracji](/pl/gateway/config-agents#agent-defaults) - klucze konfiguracji modeli
- [Przełączanie awaryjne modeli](/pl/concepts/model-failover) - łańcuchy zapasowe i zachowanie ponowień
