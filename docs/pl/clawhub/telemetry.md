---
read_when:
    - Prace nad ustawieniami telemetrii / prywatności
    - Pytania dotyczące tego, jakie dane są zbierane
summary: Telemetria instalacji zbierana przez `clawhub sync` + możliwość rezygnacji.
x-i18n:
...
ClawHub używa **minimalnej telemetrii** do obliczania **liczby instalacji** (tego, co jest faktycznie używane) oraz do lepszego sortowania/filtrowania.
...
Przy każdym `clawhub sync` CLI zgłasza **pełny zrzut** tego, co znalazło, pogrupowany według katalogu głównego skanowania („folder/root”).
...
- `label`: czytelna dla człowieka etykieta wyprowadzona z ostatnich dwóch segmentów ścieżki (ścieżki katalogu domowego są pokazywane z `~`).
- `firstSeenAt`, `lastSeenAt`, opcjonalnie `expiredAt`.

Dla każdej umiejętności znalezionej w katalogu głównym przechowujemy:

- `skillId` (rozwiązywane według sluga; śledzone są tylko umiejętności istniejące w rejestrze).
- `firstSeenAt`, `lastSeenAt`.
- `lastVersion` (na zasadzie best-effort; obecnie wersja dopasowana do rejestru, jeśli jest znana).
...
- Żadnego śledzenia umiejętności, które nie są przesłane do rejestru (nieznane slugi są ignorowane).

## Liczba instalacji

Utrzymujemy dwa liczniki dla każdej umiejętności:

- `installsCurrent`: unikatowi użytkownicy, którzy obecnie mają umiejętność zainstalowaną w co najmniej jednym aktywnym katalogu głównym.
- `installsAllTime`: unikatowi użytkownicy, którzy kiedykolwiek zgłosili zainstalowaną umiejętność.
...
- Jeśli umiejętność zostanie usunięta ze wszystkich Twoich katalogów głównych, przestaje być wliczana do `installsCurrent`.
...
ClawHub udostępnia prywatną kartę „Zainstalowane” na Twoim własnym profilu:
