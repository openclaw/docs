---
read_when:
    - Praca nad telemetrią / kontrolkami prywatności
    - Pytania dotyczące tego, jakie dane są zbierane
summary: Telemetria instalacji zbierana przez ClawHub CLI i sposób rezygnacji.
x-i18n:
    generated_at: "2026-07-04T06:53:40Z"
    model: gpt-5.5
...
- Uruchomisz `clawhub install <slug>`.
...
Przy każdym zgłoszonym `clawhub install` CLI wysyła jedno zdarzenie instalacji typu best-effort.
...
- Żadnych ścieżek folderów ani identyfikatorów wyprowadzonych z folderów.
- Żadnej zawartości plików.
- Żadnych logów poszczególnych uruchomień, promptów ani innych danych wyjściowych CLI.

## Liczniki instalacji

ClawHub utrzymuje zbiorcze liczniki dla każdego skill:
