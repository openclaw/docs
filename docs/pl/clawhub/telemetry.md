...
ClawHub używa **minimalnej telemetrii** do obliczania **liczby instalacji** (tego, co faktycznie jest używane) oraz do ulepszania sortowania/filtrowania.
...
Przy każdym `clawhub sync` CLI raportuje **pełną migawkę** tego, co znalazło, pogrupowaną według katalogu głównego skanowania („folder/katalog główny”).
...
- `label`: czytelna dla człowieka etykieta wyprowadzona z dwóch ostatnich segmentów ścieżki (ścieżki katalogu domowego są wyświetlane z `~`).
