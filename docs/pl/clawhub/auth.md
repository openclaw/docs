...
summary: Logowanie w ClawHub, tokeny API, logowanie przez CLI, przechowywanie tokenów i unieważnianie.
x-i18n:
...
4. Przeglądarka przekierowuje z powrotem do lokalnego serwera zwrotnego.
5. CLI zapisuje token w pliku konfiguracji ClawHub.

Jeśli przeglądarka nie może połączyć się z lokalnym serwerem zwrotnym z powodu zapory, VPN lub
...
CLI wyświetla jednorazowy kod i czeka, gdy autoryzujesz go na
...
Możesz odwołać tokeny API w internetowym interfejsie użytkownika ClawHub.

Odwołane, nieprawidłowe lub brakujące tokeny zwracają `401 Unauthorized`. Zaloguj się ponownie
