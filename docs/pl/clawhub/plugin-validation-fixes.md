...
read_when:
    ...
    - ClawHub odrzucił publikację pakietu Plugin lub zgłosił ostrzeżenie przy niej
...
x-i18n:
    generated_at: "2026-07-02T01:17:12Z"
    model: gpt-5.5
...
automatycznych skanów pakietów. Ta strona omawia ustalenia dla autorów, czyli
...
| `package-plugin-api-compat-missing`     | [Zadeklaruj zgodność z API Plugin](/pl/clawhub/plugin-validation-fixes#package-plugin-api-compat-missing)                   |
...
- Skorzystaj z [Tworzenia Plugin](/pl/plugins/building-plugins), aby zobaczyć minimalny przykład pakietu,
...
Pakiet można spakować, ale spakowany artefakt nie zawiera plików
punktów wejścia zadeklarowanych w `package.json#openclaw`.

- Uruchom `npm pack --dry-run` i sprawdź pliki, które zostałyby dołączone.
...
- Używaj udokumentowanego wersjonowanego schematu dopiero po opublikowaniu go przez OpenClaw.
- Uruchom ponownie `clawhub package validate <path-to-plugin>`.
...
- Zachowaj zachowania wrażliwe na bezpieczeństwo udokumentowane w publicznej dokumentacji pakietu lub
  README do czasu powstania kontraktu manifestu.
- Uruchom ponownie `clawhub package validate <path-to-plugin>`.
