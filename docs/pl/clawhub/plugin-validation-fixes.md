...
x-i18n:
    generated_at: "2026-06-30T14:26:34Z"
    model: gpt-5.5
...
automatycznych skanów pakietów. Ta strona obejmuje ustalenia skierowane do autorów,
czyli ustalenia, które autor Plugin może naprawić w metadanych pakietu, manifeście, importach SDK
...
- Skorzystaj z [Budowanie Plugin](/pl/plugins/building-plugins), aby zobaczyć minimalny przykład pakietu,
  oraz z [Manifest Plugin](/pl/plugins/manifest#manifest-versus-packagejson),
...
- Zobacz [pola package.json, które wpływają na wykrywanie](/pl/plugins/manifest#packagejson-fields-that-affect-discovery).
- Uruchom ponownie `clawhub package validate <path-to-plugin>`.
...
- Zobacz [Punkty wejścia Plugin](/pl/plugins/sdk-entrypoints) oraz
  [pola package.json, które wpływają na wykrywanie](/pl/plugins/manifest#packagejson-fields-that-affect-discovery).
- Uruchom ponownie `clawhub package validate <path-to-plugin>`.
...
- Porównaj każdy klucz w `contracts` z
  [dokumentacją contracts](/pl/plugins/manifest#contracts-reference).
...
Plugin importuje z przestarzałego głównego barrel SDK:
`openclaw/plugin-sdk`.

- Zastąp importy z głównego barrel ukierunkowanymi importami z publicznych podścieżek.
...
- Jeśli dane zachowanie nie ma publicznego SDK, zachowaj helper w swoim pakiecie albo
...
- Użyj `getSessionEntry(...)` lub `listSessionEntries(...)` podczas odczytywania
  stanu sesji.
- Użyj `patchSessionEntry(...)` lub `upsertSessionEntry(...)` podczas zapisywania
  stanu sesji.
...
- Unikaj ładowania, modyfikowania i zapisywania całego obiektu magazynu sesji.
- Zachowaj helpery zapisu całego magazynu tylko wtedy, gdy deklarowany zakres zgodności
...
- Użyj `getSessionEntry(...)`, aby odczytywać metadane sesji według tożsamości
  agenta i sesji.
- Użyj `patchSessionEntry(...)` lub `upsertSessionEntry(...)`, aby utrwalać
  metadane sesji.
- Użyj tożsamości transkryptu lub helperów celu, gdy kod przygotowuje operację
  transkryptu.
...
Plugin nadal używa przestarzałego helpera celu pliku transkryptu
...
- Zachowaj starszy helper tylko wtedy, gdy deklarowany zakres zgodności nadal
...
- Użyj `appendSessionTranscriptMessageByIdentity(...)` dla dopisywania do transkryptu.
- Użyj `publishSessionTranscriptUpdateByIdentity(...)` dla powiadomień o aktualizacji
  transkryptu.
...
- Zachowaj niskopoziomowe helpery transkryptu tylko wtedy, gdy deklarowany zakres zgodności
  nadal obsługuje starsze wersje OpenClaw, które ich wymagają.
- Zobacz [API runtime](/pl/plugins/sdk-runtime#agent-session-state) i
  [podścieżki Plugin SDK](/pl/plugins/sdk-subpaths).
- Uruchom ponownie `clawhub package validate <path-to-plugin>`.
...
- Zobacz [dokumentację setup](/pl/plugins/manifest#setup-reference) i
...
- Utrzymuj metadane zmiennych środowiskowych kanału w formie deklaratywnej, aby OpenClaw mógł sprawdzić status setup
...
- Utrzymuj zachowanie wrażliwe z punktu widzenia bezpieczeństwa udokumentowane w publicznej dokumentacji pakietu lub
