...
x-i18n:
    generated_at: "2026-07-03T02:57:18Z"
    model: gpt-5.5
...
automatycznych skanów pakietów. Ta strona obejmuje ustalenia widoczne dla
...
- Użyj [Budowanie Plugin](/pl/plugins/building-plugins) jako minimalnego przykładu
...
- Trzymaj wszystkie ścieżki punktów wejścia w katalogu pakietu.
...
Pakiet deklaruje punkt wejścia OpenClaw, ale wskazany plik nie istnieje w
...
- Wyrównaj minimalną wersję hosta z zakresem wersji hosta, który pakiet faktycznie
  obsługuje.
- Zobacz [pola package.json wpływające na wykrywanie](/pl/plugins/manifest#packagejson-fields-that-affect-discovery).
- Uruchom ponownie `clawhub package validate <path-to-plugin>`.
...
- Jeśli `openclaw.plugin.json` także ma `version`, zaktualizuj ją tak, aby
  pasowała, albo usuń nieaktualne metadane wersji manifestu, gdy metadane
  pakietu są autorytatywne.
...
- Napraw nieprawidłowe metadane pakietu, uszkodzone skrypty cyklu życia lub wpisy
  plików, które powodują niepowodzenie pakowania.
...
Plugin importuje ścieżkę SDK zarezerwowaną dla pluginów dołączonych lub wewnętrznej
zgodności.
...
- Utrzymuj zachowania wrażliwe pod względem bezpieczeństwa udokumentowane w publicznej dokumentacji pakietu lub
