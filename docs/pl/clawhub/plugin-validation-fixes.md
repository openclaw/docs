...
read_when:
    ...
    - ClawHub odrzucił publikację pakietu Plugin lub wyświetlił przy niej ostrzeżenie
...
x-i18n:
    generated_at: "2026-06-28T00:11:54Z"
    model: gpt-5.5
...
ClawHub sprawdza pakiety Plugin przed publikacją i może też pokazywać ustalenia z
automatycznych skanów pakietów. Ta strona opisuje ustalenia przeznaczone dla autorów,
czyli takie, które autor Plugin może naprawić w metadanych pakietu, manifeście, importach
...
| `legacy-root-sdk-import`                | [Zastąp główne importy SDK](/pl/clawhub/plugin-validation-fixes#legacy-root-sdk-import)                                         |
...
- Użyj [Budowanie Plugin](/pl/plugins/building-plugins) jako minimalnego przykładu pakietu
  oraz [Manifest Plugin](/pl/plugins/manifest#manifest-versus-packagejson)
  dla podziału między pakietem a manifestem.
...
- Użyj wersji API Plugin OpenClaw lub minimalnej wersji semver, względem której pakiet został
  zbudowany i przetestowany.
...
Pakietu nie da się spakować do artefaktu, który ClawHub sprawdzałby lub
publikował.
...
- Użyj `openclaw/plugin-sdk/channel-core` dla pomocników punktów wejścia kanałów.
...
- Jeśli zachowanie nie ma publicznego SDK, zachowaj pomocnik wewnątrz swojego pakietu albo
...
- Użyj `patchSessionEntry(...)` lub `upsertSessionEntry(...)` podczas zapisu stanu
  sesji.
- Unikaj wczytywania, mutowania i zapisywania całego obiektu magazynu sesji.
...
- Przenieś pracę nad nadpisywaniem modelu lub dostawcy do `before_model_resolve`.
...
- Zachowaj `before_agent_start` tylko wtedy, gdy deklarowany zakres zgodności nadal
  obsługuje starsze wersje OpenClaw, które tego wymagają.
...
- Zobacz [odniesienie konfiguracji](/pl/plugins/manifest#setup-reference) i
...
metadanych konfiguracji lub ustawień, których oczekuje ClawHub.

- Zachowaj deklaratywne metadane zmiennych środowiskowych kanału, aby OpenClaw mógł sprawdzać stan konfiguracji
  bez ładowania środowiska uruchomieniowego kanału.
- Odzwierciedl konfigurację kanału sterowaną zmiennymi środowiskowymi w bieżącej konfiguracji, konfiguracji kanału lub
...
- Usuń URL schematu, jeśli ma tylko charakter informacyjny.
...
- Usuń `openclaw.security.json`, dopóki OpenClaw nie udokumentuje wersjonowanego schematu manifestu
  bezpieczeństwa i zachowania ClawHub.
- Dokumentuj zachowania wrażliwe na bezpieczeństwo w publicznej dokumentacji pakietu lub
