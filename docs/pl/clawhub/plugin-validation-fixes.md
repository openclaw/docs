---
read_when:
    - Uruchomiłeś clawhub package validate i musisz naprawić ustalenia dotyczące Plugin
...
summary: Napraw problemy wykryte podczas walidacji pakietu Plugin ClawHub przed publikacją
...
ClawHub weryfikuje pakiety Plugin przed publikacją i może także pokazywać ustalenia z
automatycznych skanów pakietów. Ta strona opisuje ustalenia skierowane do autorów, czyli
ustalenia, które autor Plugin może naprawić w metadanych pakietu, manifeście, importach SDK
...
| `package-manifest-version-drift`        | [Dostosuj wersje pakietu i manifestu](/pl/clawhub/plugin-validation-fixes#package-manifest-version-drift)                      |
...
| `sdk-session-file-helper`               | [Zastąp helpery ścieżek plików sesji](/pl/clawhub/plugin-validation-fixes#sdk-session-file-helper)                             |
| `sdk-session-transcript-file-target`    | [Zastąp przestarzałe cele plików transkryptu](/pl/clawhub/plugin-validation-fixes#sdk-session-transcript-file-target)          |
| `sdk-session-transcript-low-level`      | [Zastąp niskopoziomowe helpery transkryptu](/pl/clawhub/plugin-validation-fixes#sdk-session-transcript-low-level)              |
...
| `security-manifest-schema-unavailable`  | [Usuń odwołania do niedostępnych schematów manifestu bezpieczeństwa](/pl/clawhub/plugin-validation-fixes#security-manifest-schema-unavailable) |
...
- Użyj [Budowanie Plugin](/pl/plugins/building-plugins), aby zobaczyć minimalny przykład pakietu,
  oraz [Manifest Plugin](/pl/plugins/manifest#manifest-versus-packagejson),
...
- Użyj wersji API Plugin OpenClaw lub minimalnego semver, względem którego budowano
  i testowano.
...
- Dostosuj minimalną wersję hosta do zakresu wersji hosta, który pakiet
...
- Jeśli `openclaw.plugin.json` również ma `version`, zaktualizuj ją, aby pasowała, albo usuń
  nieaktualne metadane wersji manifestu, gdy metadane pakietu są nadrzędne.
...
- Napraw nieprawidłowe metadane pakietu, uszkodzone skrypty cyklu życia lub wpisy plików,
...
- Zastąp importy z głównego barrela ukierunkowanymi importami z publicznych ścieżek podrzędnych.
- Użyj `openclaw/plugin-sdk/plugin-entry` dla `definePluginEntry`.
- Użyj `openclaw/plugin-sdk/channel-core` dla helperów punktu wejścia kanału.
- Użyj [Konwencji importu](/pl/plugins/building-plugins#import-conventions) i
  [ścieżek podrzędnych SDK Pluginu](/pl/plugins/sdk-subpaths), aby znaleźć wąski import.
- Uruchom ponownie `clawhub package validate <path-to-plugin>`.
...
- Zastąp zarezerwowane wewnętrzne importy SDK OpenClaw udokumentowanymi publicznymi
  ścieżkami podrzędnymi `openclaw/plugin-sdk/*`.
- Jeśli zachowanie nie ma publicznego SDK, trzymaj helper wewnątrz swojego pakietu albo
  poproś o publiczne API OpenClaw.
- Użyj [ścieżek podrzędnych SDK Pluginu](/pl/plugins/sdk-subpaths) i
...
- Używaj `getSessionEntry(...)` lub `listSessionEntries(...)` podczas odczytu stanu sesji.
- Używaj `patchSessionEntry(...)` lub `upsertSessionEntry(...)` podczas zapisu stanu sesji.
- Unikaj ładowania, modyfikowania i zapisywania całego obiektu magazynu sesji.
- Zachowaj `loadSessionStore(...)` tylko dopóki zadeklarowany zakres zgodności
...
- Używaj `patchSessionEntry(...)` podczas aktualizowania pól istniejącego wpisu sesji.
- Używaj `upsertSessionEntry(...)` podczas zastępowania lub tworzenia wpisu sesji.
- Unikaj ładowania, modyfikowania i zapisywania całego obiektu magazynu sesji.
- Zachowaj helpery zapisu całego magazynu tylko dopóki zadeklarowany zakres zgodności
...
- Użyj `getSessionEntry(...)`, aby odczytać metadane sesji według tożsamości agenta i sesji.
- Użyj `patchSessionEntry(...)` lub `upsertSessionEntry(...)`, aby utrwalić metadane sesji.
- Użyj tożsamości transkryptu lub helperów targetu, gdy kod przygotowuje operację
  transkryptu.
- Nie utrwalaj starszych ścieżek plików transkryptu ani nie polegaj na nich.
- Zobacz [API runtime](/pl/plugins/sdk-runtime#agent-session-state) i
  [ścieżki podrzędne SDK Pluginu](/pl/plugins/sdk-subpaths).
- Uruchom ponownie `clawhub package validate <path-to-plugin>`.
...
Plugin nadal używa przestarzałego helpera targetu pliku transkryptu
...
- Użyj `resolveSessionTranscriptTarget(...)`, gdy kod potrzebuje ustrukturyzowanego
  targetu operacji transkryptu.
- Unikaj bezpośredniego odczytywania lub konstruowania starszych targetów plików transkryptu.
- Zachowaj starszy helper tylko dopóki zadeklarowany zakres zgodności nadal
...
Plugin nadal używa przestarzałych niskopoziomowych helperów transkryptu, takich jak
`appendSessionTranscriptMessage` lub `emitSessionTranscriptUpdate`.

- Użyj `appendSessionTranscriptMessageByIdentity(...)` do dopisywania do transkryptu.
- Użyj `publishSessionTranscriptUpdateByIdentity(...)` do powiadomień o aktualizacji
  transkryptu.
- Preferuj ustrukturyzowaną powierzchnię runtime transkryptu, aby OpenClaw mógł stosować
  właściwe granice transakcji i obsługę tożsamości.
- Zachowaj niskopoziomowe helpery transkryptu tylko dopóki zadeklarowany zakres zgodności
  nadal obsługuje starsze wersje OpenClaw, które ich wymagają.
- Zobacz [API runtime](/pl/plugins/sdk-runtime#agent-session-state) i
  [ścieżki podrzędne SDK Pluginu](/pl/plugins/sdk-subpaths).
- Uruchom ponownie `clawhub package validate <path-to-plugin>`.
...
- Zachowaj `before_agent_start` tylko dopóki zadeklarowany zakres zgodności nadal
  obsługuje starsze wersje OpenClaw, które go wymagają.
...
Manifest nadal używa starszych metadanych autoryzacji providera `providerAuthEnvVars`.
...
Manifest używa starszych lub przestarzałych metadanych zmiennych środowiskowych kanału bez aktualnych
metadanych setupu lub konfiguracji, których oczekuje ClawHub.

- Utrzymuj metadane zmiennych środowiskowych kanału jako deklaratywne, aby OpenClaw mógł sprawdzić status setupu
  bez ładowania runtime kanału.
- Odzwierciedl setup kanału oparty na zmiennych środowiskowych w aktualnych metadanych setupu, konfiguracji kanału lub
  pakietu kanału używanych przez kształt Twojego pluginu.
...
- Usuń URL schematu, jeśli jest tylko doradczy.
