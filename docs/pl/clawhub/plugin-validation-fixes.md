...
x-i18n:
    generated_at: "2026-07-04T18:23:00Z"
    model: gpt-5.5
...
automatycznych skanów pakietów. Ta strona omawia ustalenia skierowane do autorów,
czyli takie, które autor Plugin może naprawić w metadanych pakietu, manifeście,
importach SDK lub opublikowanym artefakcie.

Nie obejmuje ona wewnętrznych ustaleń pokrycia Plugin Inspector. Jeśli pełny raport
...
Pakiet deklaruje punkt wejścia OpenClaw, ale wskazywanego pliku brakuje w
...
- Użyj wersji API Plugin OpenClaw lub dolnego zakresu semver, względem którego
...
- Jeśli `openclaw.plugin.json` także ma `version`, zaktualizuj ją, aby pasowała,
  albo usuń nieaktualne metadane wersji manifestu, gdy autorytatywne są
  metadane pakietu.
...
- Przechowuj metadane natywnego Plugin w `openclaw.plugin.json`.
...
- Jeśli dane zachowanie nie ma publicznego SDK, utrzymuj pomocnik wewnątrz swojego pakietu lub
...
- Użyj tożsamości transkrypcji lub pomocników celu, gdy kod przygotowuje operację
  transkrypcji.
- Nie utrwalaj przestarzałych ścieżek plików transkrypcji ani nie polegaj na nich.
- Zobacz [API środowiska uruchomieniowego](/pl/plugins/sdk-runtime#agent-session-state) i
  [ścieżki podrzędne SDK pluginu](/pl/plugins/sdk-subpaths).
- Uruchom ponownie `clawhub package validate <path-to-plugin>`.
...
- Zachowaj przestarzały pomocnik tylko wtedy, gdy zadeklarowany zakres zgodności nadal
...
Plugin nadal używa przestarzałego hooka `before_agent_start`.
...
Manifest używa przestarzałych lub starszych metadanych zmiennych środowiskowych kanału bez obecnych
...
- Odzwierciedl setup kanału oparty na zmiennych środowiskowych w obecnych metadanych setup, konfiguracji kanału lub
...
- Dokumentuj zachowanie istotne dla bezpieczeństwa w publicznej dokumentacji pakietu lub
  pliku README, dopóki kontrakt manifestu nie istnieje.
- Uruchom ponownie `clawhub package validate <path-to-plugin>`.
