...
x-i18n:
    generated_at: "2026-07-02T22:50:48Z"
    model: gpt-5.5
...
- Użyj wersji API Plugin OpenClaw lub dolnej granicy semver, względem której pakiet
...
Pakietu nie można spakować do artefaktu, który ClawHub by sprawdził lub
opublikował.
...
- Przenieś zachowanie środowiska wykonawczego do kodu rejestracji pluginu i ogranicz `contracts`
...
- Jeśli zachowanie nie ma publicznego SDK, zachowaj helper wewnątrz swojego pakietu lub
...
- Używaj `getSessionEntry(...)` lub `listSessionEntries(...)` podczas odczytywania
  stanu sesji.
- Używaj `patchSessionEntry(...)` lub `upsertSessionEntry(...)` podczas zapisywania
  stanu sesji.
...
- Użyj `patchSessionEntry(...)` lub `upsertSessionEntry(...)`, aby utrwalić metadane
  sesji.
- Użyj helperów tożsamości transkrypcji lub celu, gdy kod przygotowuje
  operację transkrypcji.
...
- Używaj `appendSessionTranscriptMessageByIdentity(...)` do dopisywania do transkrypcji.
- Używaj `publishSessionTranscriptUpdateByIdentity(...)` do powiadomień o aktualizacji
  transkrypcji.
- Preferuj ustrukturyzowaną powierzchnię środowiska wykonawczego transkrypcji, aby OpenClaw mógł zastosować
  właściwe granice transakcji i obsługę tożsamości.
- Zachowaj niskopoziomowe helpery transkrypcji tylko wtedy, gdy deklarowany zakres zgodności
  nadal obsługuje starsze wersje OpenClaw, które ich wymagają.
- Zobacz [API środowiska wykonawczego](/pl/plugins/sdk-runtime#agent-session-state) i
  [podścieżki SDK Pluginu](/pl/plugins/sdk-subpaths).
- Uruchom ponownie `clawhub package validate <path-to-plugin>`.
...
metadanych setupu lub konfiguracji oczekiwanych przez ClawHub.

- Utrzymuj metadane zmiennych środowiskowych kanału jako deklaratywne, aby OpenClaw mógł sprawdzać status setupu
  bez ładowania środowiska wykonawczego kanału.
- Odzwierciedl setup kanału sterowany zmiennymi środowiskowymi w bieżących metadanych setupu, konfiguracji kanału lub
