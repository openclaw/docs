...
x-i18n:
    generated_at: "2026-07-03T17:45:53Z"
    model: gpt-5.5
...
automatycznych skanów pakietów. Ta strona opisuje ustalenia przeznaczone dla autorów,
czyli takie, które autor Plugin może naprawić w metadanych pakietu, manifeście, importach SDK
lub opublikowanym artefakcie.

Nie obejmuje ona wewnętrznych ustaleń dotyczących pokrycia Plugin Inspector. Jeśli pełny raport
...
| `sdk-session-store-write`               | [Zastąp zapisy do całego magazynu sesji](/pl/clawhub/plugin-validation-fixes#sdk-session-store-write)                          |
...
| `security-manifest-schema-unavailable`  | [Usuń odwołania do niedostępnego schematu manifestu bezpieczeństwa](/pl/clawhub/plugin-validation-fixes#security-manifest-schema-unavailable) |
...
- Użyj [Budowanie plugins](/pl/plugins/building-plugins), aby zobaczyć minimalny przykład pakietu,
...
- Użyj wersji API Plugin OpenClaw lub dolnej granicy semver, względem której pakiet został zbudowany i przetestowany.
- Zachowaj to oddzielnie od wersji pakietu. Wersja pakietu opisuje wydanie
...
- Jeśli `openclaw.plugin.json` także ma `version`, zaktualizuj ją tak, aby była zgodna, albo usuń
...
- Zobacz [Budowanie plugins](/pl/plugins/building-plugins).
- Uruchom ponownie `clawhub package validate <path-to-plugin>`.
...
Plugin importuje z przestarzałej głównej beczki SDK:
`openclaw/plugin-sdk`.

- Zastąp importy z głównej beczki ukierunkowanymi importami z publicznych podścieżek.
...
- Jeśli zachowanie nie ma publicznego SDK, zachowaj pomocnik w swoim pakiecie albo
...
- Użyj `getSessionEntry(...)` lub `listSessionEntries(...)` podczas odczytywania stanu sesji.
- Użyj `patchSessionEntry(...)` lub `upsertSessionEntry(...)` podczas zapisywania stanu sesji.
...
- Użyj `patchSessionEntry(...)` podczas aktualizowania pól istniejącego wpisu sesji.
...
- Użyj tożsamości transkryptu lub pomocników celu, gdy kod przygotowuje operację
  transkryptu.
- Nie utrwalaj ani nie polegaj na starszych ścieżkach plików transkryptów.
- Zobacz [API środowiska uruchomieniowego](/pl/plugins/sdk-runtime#agent-session-state) i
  [podścieżki Plugin SDK](/pl/plugins/sdk-subpaths).
- Uruchom ponownie `clawhub package validate <path-to-plugin>`.
...
- Unikaj bezpośredniego odczytywania lub konstruowania starszych celów plików transkryptów.
...
- Użyj `appendSessionTranscriptMessageByIdentity(...)` dla dopisywania transkryptu.
- Użyj `publishSessionTranscriptUpdateByIdentity(...)` dla powiadomień o aktualizacji transkryptu.
- Preferuj ustrukturyzowaną powierzchnię środowiska uruchomieniowego transkryptu, aby OpenClaw mógł stosować
  właściwe granice transakcji i obsługę tożsamości.
- Zachowaj niskopoziomowe pomocniki transkryptu tylko wtedy, gdy deklarowany zakres zgodności
  nadal obsługuje starsze wersje OpenClaw, które ich wymagają.
- Zobacz [API środowiska uruchomieniowego](/pl/plugins/sdk-runtime#agent-session-state) i
  [podścieżki Plugin SDK](/pl/plugins/sdk-subpaths).
- Uruchom ponownie `clawhub package validate <path-to-plugin>`.
...
- Zachowaj `providerAuthEnvVars` tylko jako metadane zgodności, gdy obsługiwany
  zakres OpenClaw nadal tego potrzebuje.
...
Manifest używa starszych lub dawniejszych metadanych zmiennych środowiskowych kanału bez bieżących
metadanych setupu lub konfiguracji, których oczekuje ClawHub.

- Zachowaj deklaratywność metadanych zmiennych środowiskowych kanału, aby OpenClaw mógł sprawdzać status setupu
...
- Zachowaj `channelEnvVars` tylko jako metadane zgodności, gdy starsze obsługiwane
  wersje OpenClaw nadal tego wymagają.
...
- Zachowaj zachowanie wrażliwe pod względem bezpieczeństwa udokumentowane w publicznej dokumentacji pakietu lub
