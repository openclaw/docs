---
read_when:
    - Uruchomiono clawhub package validate i trzeba naprawić problemy wykryte w Plugin
...
x-i18n:
    generated_at: "2026-07-02T14:12:21Z"
    model: gpt-5.5
...
dla maintainerów OpenClaw, a nie dla autorów Plugin.
...
- Skorzystaj z [Tworzenie Plugin](/pl/plugins/building-plugins), aby zobaczyć minimalny przykład pakietu,
  oraz z [Manifest Plugin](/pl/plugins/manifest#manifest-versus-packagejson),
  aby zrozumieć rozdział między pakietem a manifestem.
- Uruchom ponownie `clawhub package validate <path-to-plugin>`.

### package-openclaw-metadata-missing

Pakiet ma `package.json`, ale nie deklaruje metadanych pakietu
OpenClaw.
...
Metadane pakietu istnieją, ale nie deklarują punktu wejścia runtime
OpenClaw.
...
- Zobacz [Punkty wejścia Plugin](/pl/plugins/sdk-entrypoints) i
...
Spakowany artefakt nie zawiera metadanych OpenClaw, które istnieją w pakiecie
źródłowym.
...
- Użyj tożsamości transkryptu lub helperów celu, gdy kod przygotowuje operację
  transkryptu.
- Nie utrwalaj przestarzałych ścieżek plików transkryptu ani od nich nie zależ.
- Zobacz [API środowiska uruchomieniowego](/pl/plugins/sdk-runtime#agent-session-state) i
  [podścieżki SDK Plugin](/pl/plugins/sdk-subpaths).
- Uruchom ponownie `clawhub package validate <path-to-plugin>`.
...
- Unikaj bezpośredniego odczytywania lub konstruowania przestarzałych celów pliku transkryptu.
- Zachowaj przestarzały helper tylko wtedy, gdy zadeklarowany zakres zgodności nadal
...
- Użyj `appendSessionTranscriptMessageByIdentity(...)` do dołączania transkryptu.
...
- Zachowaj niskopoziomowe helpery transkryptu tylko wtedy, gdy zadeklarowany zakres zgodności
  nadal obsługuje starsze wersje OpenClaw, które ich wymagają.
- Zobacz [API środowiska uruchomieniowego](/pl/plugins/sdk-runtime#agent-session-state) i
  [podścieżki SDK Plugin](/pl/plugins/sdk-subpaths).
- Uruchom ponownie `clawhub package validate <path-to-plugin>`.
...
- Odzwierciedl setup kanału oparty na zmiennych środowiskowych w bieżących metadanych setup, konfiguracji kanału lub
...
- Zachowaj dokumentację zachowań wrażliwych bezpieczeństwa w publicznej dokumentacji pakietu lub
