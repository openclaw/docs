...
| `package-openclaw-metadata-missing`     | [Dodaj blok package openclaw](/pl/clawhub/plugin-validation-fixes#package-openclaw-metadata-missing)                           |
...
- Użyj [Budowanie Plugin](/pl/plugins/building-plugins), aby zobaczyć minimalny przykład
...
- Użyj wersji API Plugin OpenClaw albo dolnej granicy semver, względem której
  budowano i testowano.
...
- Napraw nieprawidłowe metadane pakietu, uszkodzone skrypty cyklu życia albo wpisy plików,
  przez które pakowanie się nie udaje.
...
- Zaktualizuj `files`, `.npmignore` albo wynik kompilacji, aby zadeklarowane punkty wejścia
...
- Upewnij się, że `openclaw.plugin.json` jest uwzględniony, gdy pakiet jest natywnym
  Plugin OpenClaw.
- Zaktualizuj `files` albo `.npmignore`, aby metadane pakietu nie były wykluczone.
...
- Jeśli dane zachowanie nie ma publicznego SDK, trzymaj helper wewnątrz swojego pakietu albo
...
- Odwzoruj setup kanału oparty na zmiennych środowiskowych do bieżącego setup, konfiguracji kanału lub
