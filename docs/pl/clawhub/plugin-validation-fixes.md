...
x-i18n:
    generated_at: "2026-07-01T08:32:58Z"
    model: gpt-5.5
...
ClawHub sprawdza poprawność pakietów Plugin przed publikacją i może też pokazywać wyniki
zautomatyzowanych skanów pakietów. Ta strona opisuje wyniki przeznaczone dla autorów, czyli
wyniki, które autor Plugin może naprawić w metadanych pakietu, manifeście, importach SDK
lub opublikowanym artefakcie.

Nie obejmuje wewnętrznych wyników pokrycia Plugin Inspector. Jeśli pełny raport
...
## Wyniki przeznaczone dla autorów
...
- Użyj [Tworzenie Plugin](/pl/plugins/building-plugins), aby zobaczyć minimalny przykład
...
- Usuń `private: true`, jeśli ten pakiet jest przeznaczony do publicznego publikowania.
- Uruchom ponownie `clawhub package validate <path-to-plugin>`.
...
- `name` powinno być czytelne dla człowieka, a `id` zachowaj jako stabilny identyfikator maszynowy.
- Zobacz [Manifest Plugin](/pl/plugins/manifest).
- Uruchom ponownie `clawhub package validate <path-to-plugin>`.
...
- Jeśli to zachowanie nie ma publicznego SDK, zachowaj pomocnik wewnątrz swojego pakietu albo
...
- Używaj `patchSessionEntry(...)` lub `upsertSessionEntry(...)`, aby utrwalać metadane
  sesji.
- Używaj pomocników tożsamości transkrypcji lub celu, gdy kod przygotowuje operację
  transkrypcji.
- Nie utrwalaj starszych ścieżek plików transkrypcji ani od nich nie zależ.
- Zobacz [API środowiska uruchomieniowego](/pl/plugins/sdk-runtime#agent-session-state) i
  [podścieżki SDK Plugin](/pl/plugins/sdk-subpaths).
- Uruchom ponownie `clawhub package validate <path-to-plugin>`.
...
- Zachowaj `providerAuthEnvVars` tylko jako metadane zgodności, gdy obsługiwany
  zakres OpenClaw nadal ich wymaga.
...
- Zachowaj deklaratywność metadanych zmiennych środowiskowych kanału, aby OpenClaw mógł sprawdzać stan setup
