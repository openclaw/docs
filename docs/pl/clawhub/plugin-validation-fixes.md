...
x-i18n:
    generated_at: "2026-07-04T11:05:15Z"
    model: gpt-5.5
...
automatycznych skanów pakietów. Ta strona obejmuje ustalenia skierowane do autorów, czyli
...
dla opiekunów OpenClaw, a nie dla autorów Plugin.
...
- Skorzystaj z [Budowanie Plugin](/pl/plugins/building-plugins), aby zobaczyć minimalny przykład
  pakietu, oraz z [Manifest Plugin](/pl/plugins/manifest#manifest-versus-packagejson),
...
- Napraw nieprawidłowe metadane pakietu, zepsute skrypty cyklu życia lub wpisy plików,
...
- Zastąp importy z głównego barrela precyzyjnymi importami z publicznych podścieżek.
...
Plugin importuje ścieżkę SDK zarezerwowaną dla Plugin dołączonych w pakiecie lub wewnętrznej
zgodności.
...
- Użyj `publishSessionTranscriptUpdateByIdentity(...)` do powiadomień o aktualizacji
  transkrypcji.
- Preferuj ustrukturyzowaną powierzchnię runtime transkrypcji, aby OpenClaw mógł stosować
...
- Zobacz [referencję konfiguracji setup](/pl/plugins/manifest#setup-reference) i
...
metadanych setup lub config oczekiwanych przez ClawHub.

- Zachowaj deklaratywny charakter metadanych zmiennych środowiskowych kanału, aby OpenClaw mógł sprawdzać status setup
  bez ładowania runtime kanału.
- Odzwierciedl konfigurację kanału opartą na zmiennych środowiskowych w bieżących metadanych setup, config kanału lub
