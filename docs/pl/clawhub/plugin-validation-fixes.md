---
read_when:
    - Uruchomiono clawhub package validate i trzeba naprawić problemy dotyczące Plugin
    - ClawHub odrzucił lub ostrzegł przy publikowaniu pakietu Plugin
...
takie, które autor Plugin może naprawić w metadanych pakietu, manifeście, importach SDK
lub opublikowanym artefakcie.

Nie obejmuje wewnętrznych ustaleń dotyczących pokrycia Plugin Inspector. Jeśli pełny raport
zawiera kody konserwacji skanera bez wskazówek naprawczych dla autora, są one przeznaczone
...
Pakiet deklaruje punkt wejścia OpenClaw, ale wskazanego pliku brakuje w walidowanym
pakiecie.
...
- Porównaj każde pole najwyższego poziomu z
  [odwołaniem do pól manifestu](/pl/plugins/manifest#top-level-field-reference).
...
- Porównaj każdy klucz pod `contracts` z
  [odwołaniem do contracts](/pl/plugins/manifest#contracts-reference).
...
- Zobacz [odwołanie do setup](/pl/plugins/manifest#setup-reference) i
...
- Dokumentuj zachowania wrażliwe z punktu widzenia bezpieczeństwa w publicznej dokumentacji pakietu lub
