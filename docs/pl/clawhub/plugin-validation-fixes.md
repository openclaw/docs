...
read_when:
    ...
    - Aktualizowanie metadanych pakietu pluginu przed wydaniem
...
x-i18n:
    generated_at: "2026-07-16T18:05:32Z"
    model: gpt-5.6
...
ClawHub weryfikuje pakiety pluginów przed publikacją, a także może wyświetlać ustalenia z
automatycznych skanów pakietów. Ta strona opisuje ustalenia przeznaczone dla autorów, czyli
problemy, które autor pluginu może naprawić w metadanych pakietu, manifeście, importach
...
zawiera kody związane z utrzymaniem skanera bez instrukcji naprawczych dla autora, są one
przeznaczone dla opiekunów OpenClaw, a nie autorów pluginów.

Po zastosowaniu każdej poprawki ponownie uruchom:
...
| `package-min-host-version-drift`        | [Dostosuj minimalną wersję hosta](/pl/clawhub/plugin-validation-fixes#package-min-host-version-drift)                                   |
| `package-manifest-version-drift`        | [Uzgodnij wersje pakietu i manifestu](/pl/clawhub/plugin-validation-fixes#package-manifest-version-drift)                          |
...
| `package-npm-pack-entrypoint-missing`   | [Uwzględnij punkty wejścia w wyniku pakowania npm](/pl/clawhub/plugin-validation-fixes#package-npm-pack-entrypoint-missing)                  |
| `package-npm-pack-metadata-missing`     | [Uwzględnij metadane w wyniku pakowania npm](/pl/clawhub/plugin-validation-fixes#package-npm-pack-metadata-missing)                       |
...
| `channel-env-vars`                      | [Odzwierciedl zmienne środowiskowe kanału w aktualnych metadanych](/pl/clawhub/plugin-validation-fixes#channel-env-vars)                                |
...
Katalog główny pakietu nie zawiera `package.json`, dlatego ClawHub nie może zidentyfikować
pakietu npm, wersji, punktów wejścia ani metadanych OpenClaw.

- Dodaj `package.json` z `name`, `version` i `type`.
- Dodaj blok `openclaw`, gdy pakiet zawiera plugin OpenClaw.
- Minimalny przykład pakietu znajduje się w sekcji [Tworzenie pluginów](/pl/plugins/building-plugins), a
  podział na pakiet i manifest opisano w sekcji [Manifest pluginu](/pl/plugins/manifest#manifest-versus-packagejson).
- Ponownie uruchom `clawhub package validate <path-to-plugin>`.

### package-openclaw-metadata-missing

Pakiet zawiera `package.json`, ale nie deklaruje metadanych
pakietu OpenClaw.
...
- Dodaj metadane zgodności i instalacji, gdy pakiet będzie publikowany lub
  instalowany za pośrednictwem ClawHub.
- Zobacz [Pola package.json wpływające na wykrywanie](/pl/plugins/manifest#packagejson-fields-that-affect-discovery).
- Ponownie uruchom `clawhub package validate <path-to-plugin>`.

### package-openclaw-entry-missing

Metadane pakietu istnieją, ale nie deklarują punktu wejścia środowiska
uruchomieniowego OpenClaw.

- Dodaj `openclaw.extensions` dla natywnych punktów wejścia pluginu.
- Dodaj `openclaw.runtimeExtensions`, gdy opublikowany pakiet powinien ładować skompilowany
  kod JavaScript.
- Wszystkie ścieżki punktów wejścia muszą znajdować się w katalogu pakietu.
- Zobacz [Punkty wejścia pluginu](/pl/plugins/sdk-entrypoints) oraz
  [Pola package.json wpływające na wykrywanie](/pl/plugins/manifest#packagejson-fields-that-affect-discovery).
- Ponownie uruchom `clawhub package validate <path-to-plugin>`.

### package-entrypoint-missing

Pakiet deklaruje punkt wejścia OpenClaw, ale wskazany plik nie istnieje
...
- Zbuduj pakiet, jeśli punkt wejścia jest generowany w `dist`.
...
- Uzupełnij `openclaw.install` obsługiwanym źródłem instalacji, takim jak
...
- Użyj wersji API pluginów OpenClaw lub dolnej granicy semver, względem której pakiet został zbudowany i
  przetestowany.
- Nie należy łączyć tego z wersją pakietu. Wersja pakietu opisuje wydanie
  pluginu, natomiast `openclaw.compat.pluginApi` opisuje kontrakt API hosta.
- Zobacz [Pola package.json wpływające na wykrywanie](/pl/plugins/manifest#packagejson-fields-that-affect-discovery).
- Ponownie uruchom `clawhub package validate <path-to-plugin>`.
...
- Sprawdź wszystkie metadane kompilacji OpenClaw w pakiecie, takie jak wersja OpenClaw
  użyta podczas wydania.
- Dostosuj minimalną wersję hosta do zakresu wersji hosta faktycznie
  obsługiwanego przez pakiet.
- Zobacz [Pola package.json wpływające na wykrywanie](/pl/plugins/manifest#packagejson-fields-that-affect-discovery).
- Ponownie uruchom `clawhub package validate <path-to-plugin>`.
...
- Jeśli `openclaw.plugin.json` zawiera również `version`, zaktualizuj je, aby było zgodne, lub usuń
  nieaktualne metadane wersji manifestu, gdy metadane pakietu są źródłem nadrzędnym.
...
- Przechowuj metadane natywnego pluginu w `openclaw.plugin.json`.
- Przechowuj punkty wejścia pakietu oraz metadane zgodności, instalacji, konfiguracji i katalogu
...
Pakietu nie można spakować do artefaktu, który ClawHub mógłby sprawdzić lub
opublikować.

- Uruchom `npm pack --dry-run` z katalogu głównego pakietu.
- Napraw nieprawidłowe metadane pakietu, uszkodzone skrypty cyklu życia lub wpisy plików, które
  uniemożliwiają pakowanie.
- Usuń `private: true`, jeśli ten pakiet jest przeznaczony do publicznej publikacji.
- Ponownie uruchom `clawhub package validate <path-to-plugin>`.

### package-npm-pack-entrypoint-missing

Pakiet można spakować, ale spakowany artefakt nie zawiera
...
- Zaktualizuj `files`, `.npmignore` lub wynik kompilacji, aby uwzględnić
  zadeklarowane punkty wejścia.
- Zobacz [Punkty wejścia pluginu](/pl/plugins/sdk-entrypoints).
- Ponownie uruchom `clawhub package validate <path-to-plugin>`.
...
- Zadbaj, aby `name` był czytelny dla człowieka, a `id` pozostał stabilnym identyfikatorem maszynowym.
- Zobacz [Manifest pluginu](/pl/plugins/manifest).
- Ponownie uruchom `clawhub package validate <path-to-plugin>`.
...
- Przenieś zachowanie środowiska wykonawczego do kodu rejestracji pluginu, a `contracts`
...
Plugin importuje z przestarzałego głównego modułu zbiorczego SDK:
`openclaw/plugin-sdk`.

- Zastąp importy z głównego modułu zbiorczego precyzyjnymi importami z publicznych podścieżek.
...
Plugin importuje ścieżkę SDK zarezerwowaną dla pluginów dołączonych do dystrybucji lub wewnętrznej
zgodności.

- Zastąp zarezerwowane wewnętrzne importy SDK OpenClaw udokumentowanymi publicznymi
  podścieżkami `openclaw/plugin-sdk/*`.
- Jeśli dane zachowanie nie ma publicznego SDK, zachowaj funkcję pomocniczą wewnątrz pakietu lub
  poproś o publiczny interfejs API OpenClaw.
...
- Podczas odczytu stanu sesji użyj `getSessionEntry(...)` lub
  `listSessionEntries(...)`.
- Podczas zapisu stanu sesji użyj `patchSessionEntry(...)` lub
  `upsertSessionEntry(...)`.
...
- Zachowaj `loadSessionStore(...)` tylko dopóty, dopóki deklarowany zakres zgodności
  nadal obejmuje starsze wersje OpenClaw, które go wymagają.
- Zobacz [Interfejs API środowiska wykonawczego](/pl/plugins/sdk-runtime#agent-session-state) i
...
- Podczas aktualizowania pól istniejącego wpisu sesji użyj
  `patchSessionEntry(...)`.
- Podczas zastępowania lub tworzenia wpisu sesji użyj `upsertSessionEntry(...)`.
- Unikaj wczytywania, modyfikowania i zapisywania całego obiektu magazynu sesji.
- Zachowaj funkcje pomocnicze zapisu całego magazynu tylko dopóty, dopóki deklarowany zakres zgodności
  nadal obejmuje starsze wersje OpenClaw, które ich wymagają.
- Zobacz [Interfejs API środowiska wykonawczego](/pl/plugins/sdk-runtime#agent-session-state) i
  [Podścieżki SDK pluginów](/pl/plugins/sdk-subpaths).
- Ponownie uruchom `clawhub package validate <path-to-plugin>`.
...
- Użyj `getSessionEntry(...)`, aby odczytywać metadane sesji według tożsamości
  agenta i sesji.
- Użyj `patchSessionEntry(...)` lub `upsertSessionEntry(...)`, aby utrwalać metadane
  sesji.
- Gdy kod przygotowuje operację na transkrypcji, użyj funkcji pomocniczych tożsamości lub celu
  transkrypcji.
- Nie utrwalaj starszych ścieżek plików transkrypcji ani nie polegaj na nich.
- Zobacz [Interfejs API środowiska wykonawczego](/pl/plugins/sdk-runtime#agent-session-state) i
  [Podścieżki SDK pluginów](/pl/plugins/sdk-subpaths).
- Ponownie uruchom `clawhub package validate <path-to-plugin>`.
...
- Zachowaj starszą funkcję pomocniczą tylko dopóty, dopóki deklarowany zakres zgodności nadal
  obejmuje starsze wersje OpenClaw, które jej wymagają.
- Zobacz [Interfejs API środowiska wykonawczego](/pl/plugins/sdk-runtime#agent-session-state) i
  [Podścieżki SDK pluginów](/pl/plugins/sdk-subpaths).
- Ponownie uruchom `clawhub package validate <path-to-plugin>`.
...
- Użyj `appendSessionTranscriptMessageByIdentity(...)` do dołączania wpisów do transkrypcji.
- Użyj `publishSessionTranscriptUpdateByIdentity(...)` do powiadomień o aktualizacjach
  transkrypcji.
- Preferuj ustrukturyzowaną powierzchnię transkrypcji środowiska wykonawczego, aby OpenClaw mógł stosować
...
- Przenieś nadpisywanie modelu lub dostawcy do `before_model_resolve`.
- Przenieś modyfikowanie monitu lub kontekstu do `before_prompt_build`.
- Zachowaj `before_agent_start` tylko dopóty, dopóki deklarowany zakres zgodności nadal
  obejmuje starsze wersje OpenClaw, które go wymagają.
...
- Odwzoruj metadane zmiennych środowiskowych dostawcy również w `setup.providers[].envVars`.
- Zachowaj `providerAuthEnvVars` wyłącznie jako metadane zgodności, dopóki obsługiwany
...
metadanych konfiguracji lub ustawień oczekiwanych przez ClawHub.

- Zachowaj deklaratywny charakter metadanych zmiennych środowiskowych kanału, aby OpenClaw mógł sprawdzać stan konfiguracji
  bez wczytywania środowiska wykonawczego kanału.
- Odwzoruj konfigurację kanału opartą na zmiennych środowiskowych w bieżących metadanych konfiguracji, ustawień kanału lub
  kanału pakietu używanych przez daną postać pluginu.
- Zachowaj `channelEnvVars` wyłącznie jako metadane zgodności, dopóki starsze obsługiwane
...
Pakiet zawiera `openclaw.security.json` z odwołaniem do schematu, którego ClawHub
nie rozpoznaje jako dostępnego.

- Usuń adres URL schematu, jeśli ma on wyłącznie charakter informacyjny.
...
- Do czasu powstania kontraktu manifestu opisuj zachowania związane z bezpieczeństwem w publicznej dokumentacji pakietu lub
  pliku README.
- Ponownie uruchom `clawhub package validate <path-to-plugin>`.
