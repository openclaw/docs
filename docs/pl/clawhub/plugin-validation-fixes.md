...
x-i18n:
    generated_at: "2026-07-05T08:46:07Z"
    model: gpt-5.5
...
ClawHub waliduje pakiety Pluginów przed publikacją i może także pokazywać ustalenia z
...
zawiera kody utrzymaniowe skanera bez wskazówek naprawczych dla autora, są one przeznaczone
dla opiekunów OpenClaw, a nie autorów Pluginów.
...
- Użyj [Tworzenie Pluginów](/pl/plugins/building-plugins), aby zobaczyć minimalny przykład
  pakietu, oraz [Manifest Pluginu](/pl/plugins/manifest#manifest-versus-packagejson),
  aby zrozumieć podział na pakiet i manifest.
...
- Użyj wersji API Pluginu OpenClaw lub minimalnej wersji semver, względem której
...
- Użyj [konwencji importu](/pl/plugins/building-plugins#import-conventions) i
  [podścieżek SDK pluginów](/pl/plugins/sdk-subpaths), aby znaleźć wąski import.
- Uruchom ponownie `clawhub package validate <path-to-plugin>`.

### reserved-sdk-import

Plugin importuje ścieżkę SDK zarezerwowaną dla pluginów pakietowanych razem z OpenClaw lub wewnętrznej
zgodności.
...
- Użyj [podścieżek SDK pluginów](/pl/plugins/sdk-subpaths) i
...
- Zachowaj niskopoziomowe helpery transkrypcji tylko dopóki zadeklarowany zakres zgodności
  nadal obsługuje starsze wersje OpenClaw, które ich wymagają.
- Zobacz [API środowiska uruchomieniowego](/pl/plugins/sdk-runtime#agent-session-state) i
  [podścieżki SDK pluginów](/pl/plugins/sdk-subpaths).
- Uruchom ponownie `clawhub package validate <path-to-plugin>`.
...
- Zobacz [referencję konfiguracji](/pl/plugins/manifest#setup-reference) i
...
metadanych konfiguracji lub config, których oczekuje ClawHub.

- Zachowaj metadane zmiennych środowiskowych kanału jako deklaratywne, aby OpenClaw mógł sprawdzać status konfiguracji
  bez ładowania środowiska uruchomieniowego kanału.
- Odwzoruj konfigurację kanału opartą na zmiennych środowiskowych do bieżącej konfiguracji, config kanału lub
...
- Usuń URL schematu, jeśli ma on wyłącznie charakter doradczy.
...
- Dokumentuj zachowanie istotne dla bezpieczeństwa w publicznej dokumentacji pakietu lub
  README, dopóki nie istnieje kontrakt manifestu.
- Uruchom ponownie `clawhub package validate <path-to-plugin>`.
