...
x-i18n:
    generated_at: "2026-07-03T10:01:26Z"
    model: gpt-5.5
...
automatycznych skanów pakietów. Ta strona obejmuje ustalenia przeznaczone dla
autorów, czyli ustalenia, które autor Plugin może naprawić w metadanych pakietu,
manifeście, importach SDK lub opublikowanym artefakcie.
...
przeznaczone dla maintainerów OpenClaw, a nie dla autorów Plugin.
...
| `sdk-session-transcript-low-level`      | [Zastąp niskopoziomowe helpery transkrypcji](/pl/clawhub/plugin-validation-fixes#sdk-session-transcript-low-level)                       |
...
- Użyj [Budowanie pluginów](/pl/plugins/building-plugins), aby zobaczyć minimalny
  przykład pakietu, oraz [Manifest pluginu](/pl/plugins/manifest#manifest-versus-packagejson),
...
- Użyj wersji API pluginu OpenClaw albo dolnej granicy semver, względem której
...
Minimalna wersja hosta pakietu nie odpowiada metadanym wersji OpenClaw, względem
których pakiet został zbudowany.

- Sprawdź `openclaw.install.minHostVersion`.
- Sprawdź wszelkie metadane budowania OpenClaw w pakiecie, takie jak wersja
...
Wersja pakietu i wersja manifestu pluginu są niespójne.
...
- Jeśli `openclaw.plugin.json` także ma `version`, zaktualizuj ją, aby była
  zgodna, albo usuń przestarzałe metadane wersji manifestu, gdy metadane pakietu
  są autorytatywne.
...
- Trzymaj metadane natywnego pluginu w `openclaw.plugin.json`.
...
Pakietu nie da się spakować do artefaktu, który ClawHub sprawdziłby lub
opublikował.

- Uruchom `npm pack --dry-run` z katalogu głównego pakietu.
- Napraw nieprawidłowe metadane pakietu, uszkodzone skrypty cyklu życia albo
  wpisy files, które powodują niepowodzenie pakowania.
- Usuń `private: true`, jeśli ten pakiet jest przeznaczony do publicznej publikacji.
- Uruchom ponownie `clawhub package validate <path-to-plugin>`.

### package-npm-pack-entrypoint-missing

Pakiet da się spakować, ale spakowany artefakt nie zawiera plików punktów
wejścia zadeklarowanych w `package.json#openclaw`.

- Uruchom `npm pack --dry-run` i sprawdź pliki, które zostałyby uwzględnione.
- Zbuduj generowane punkty wejścia przed pakowaniem.
- Zaktualizuj `files`, `.npmignore` lub wynik budowania, aby zadeklarowane
...
- Przenieś zachowanie wykonawcze do kodu rejestracji pluginu, a `contracts`
...
- Zastąp importy z głównego modułu zbiorczego ukierunkowanymi importami z publicznych podścieżek.
...
- Jeśli dane zachowanie nie ma publicznego SDK, zachowaj pomocnik w swoim pakiecie albo
...
- Zobacz [API środowiska wykonawczego](/pl/plugins/sdk-runtime#agent-session-state) i
...
- Unikaj ładowania, mutowania i zapisywania całego obiektu magazynu sesji.
- Zachowaj pomocniki zapisu całego magazynu tylko dopóki zadeklarowany zakres zgodności
...
- Używaj tożsamości transkrypcji lub pomocników celu, gdy kod przygotowuje operację
  transkrypcji.
- Nie utrwalaj przestarzałych ścieżek plików transkrypcji ani od nich nie zależ.
- Zobacz [API środowiska wykonawczego](/pl/plugins/sdk-runtime#agent-session-state) i
  [podścieżki Plugin SDK](/pl/plugins/sdk-subpaths).
- Uruchom ponownie `clawhub package validate <path-to-plugin>`.
...
- Unikaj bezpośredniego odczytywania lub konstruowania przestarzałych celów plików transkrypcji.
- Zachowaj przestarzały pomocnik tylko dopóki zadeklarowany zakres zgodności nadal
...
- Używaj `publishSessionTranscriptUpdateByIdentity(...)` do powiadomień o aktualizacji
  transkrypcji.
- Preferuj ustrukturyzowaną powierzchnię wykonawczą transkrypcji, aby OpenClaw mógł stosować
  właściwe granice transakcji i obsługę tożsamości.
- Zachowaj niskopoziomowe pomocniki transkrypcji tylko dopóki zadeklarowany zakres zgodności
  nadal obsługuje starsze wersje OpenClaw, które ich wymagają.
- Zobacz [API środowiska wykonawczego](/pl/plugins/sdk-runtime#agent-session-state) i
  [podścieżki Plugin SDK](/pl/plugins/sdk-subpaths).
- Uruchom ponownie `clawhub package validate <path-to-plugin>`.
...
Plugin nadal używa przestarzałego haka `before_agent_start`.
...
Manifest nadal używa przestarzałych metadanych uwierzytelniania dostawcy `providerAuthEnvVars`.
...
Manifest używa przestarzałych lub starszych metadanych zmiennych środowiskowych kanału bez bieżących
...
- Usuń URL schematu, jeśli ma on tylko charakter informacyjny.
...
- Zachowaj zachowanie wrażliwe na bezpieczeństwo udokumentowane w publicznej dokumentacji pakietu lub
