...
read_when:
    ...
    - ClawHub odrzucił lub ostrzegł podczas publikacji pakietu Plugin
    - Aktualizujesz metadane pakietu Plugin przed wydaniem
summary: Napraw ustalenia dotyczące walidacji pakietu Plugin ClawHub przed publikacją
title: Poprawki walidacji Plugin
x-i18n:
    generated_at: "2026-07-02T08:53:09Z"
    model: gpt-5.5
...
czyli ustalenia, które autor Plugin może naprawić w metadanych pakietu, manifeście,
importach SDK lub opublikowanym artefakcie.

Nie obejmuje wewnętrznych ustaleń dotyczących pokrycia narzędzia Plugin Inspector.
Jeśli pełny raport zawiera kody utrzymaniowe skanera bez wskazówek naprawczych dla
autora, są one przeznaczone dla opiekunów OpenClaw, a nie dla autorów Plugin.
...
| `package-npm-pack-unavailable`          | [Uczyń artefakt npm możliwym do spakowania](/pl/clawhub/plugin-validation-fixes#package-npm-pack-unavailable)                      |
...
| `security-manifest-schema-unavailable`  | [Usuń odniesienia do niedostępnych schematów manifestu zabezpieczeń](/pl/clawhub/plugin-validation-fixes#security-manifest-schema-unavailable) |
...
- Przechowuj wszystkie ścieżki punktów wejścia w katalogu pakietu.
...
ClawHub nie może określić, jak pakiet powinien zostać zainstalowany lub
zaktualizowany.
...
Pakiet nie deklaruje obsługiwanego zakresu API Plugin OpenClaw.
...
której pakiet został zbudowany.
...
- Wyrównaj minimalną wersję hosta z zakresem wersji hosta faktycznie
  obsługiwanym przez pakiet.
- Zobacz [pola package.json wpływające na wykrywanie](/pl/plugins/manifest#packagejson-fields-that-affect-discovery).
- Uruchom ponownie `clawhub package validate <path-to-plugin>`.

### package-manifest-version-drift

Wersja pakietu i wersja manifestu Plugin są rozbieżne.

- Preferuj `package.json#version` jako wersję wydania pakietu.
- Jeśli `openclaw.plugin.json` również ma `version`, zaktualizuj ją, aby była
  zgodna, albo usuń nieaktualne metadane wersji manifestu, gdy metadane pakietu
  są autorytatywne.
...
- Przechowuj punkty wejścia pakietu, zgodność, instalację, konfigurację i
  metadane katalogu w obsługiwanych polach `package.json#openclaw`.
- Zobacz [pola package.json wpływające na wykrywanie](/pl/plugins/manifest#packagejson-fields-that-affect-discovery).
- Uruchom ponownie `clawhub package validate <path-to-plugin>`.
...
- Napraw nieprawidłowe metadane pakietu, uszkodzone skrypty cyklu życia lub
  wpisy plików, które powodują niepowodzenie pakowania.
...
Manifest Plugin zawiera pola najwyższego poziomu, których OpenClaw nie obsługuje.
...
- Porównaj każdy klucz w `contracts` z
  [referencją contracts](/pl/plugins/manifest#contracts-reference).
...
- Jeśli dane zachowanie nie ma publicznego SDK, trzymaj pomocnik wewnątrz swojego pakietu albo
...
- Użyj pomocników tożsamości transkrypcji lub celu, gdy kod przygotowuje operację
  transkrypcji.
...
- Użyj `publishSessionTranscriptUpdateByIdentity(...)` do powiadomień o aktualizacji transkrypcji.
- Preferuj ustrukturyzowaną powierzchnię runtime transkrypcji, aby OpenClaw mógł zastosować
  poprawne granice transakcji i obsługę tożsamości.
- Zachowaj niskopoziomowe pomocniki transkrypcji tylko wtedy, gdy zadeklarowany zakres zgodności
  nadal obsługuje starsze wersje OpenClaw, które ich wymagają.
- Zobacz [API środowiska uruchomieniowego](/pl/plugins/sdk-runtime#agent-session-state) i
  [podścieżki Plugin SDK](/pl/plugins/sdk-subpaths).
- Uruchom ponownie `clawhub package validate <path-to-plugin>`.
...
- Przenieś pracę związaną z nadpisywaniem modelu lub dostawcy do `before_model_resolve`.
...
Manifest używa starszych metadanych zmiennych środowiskowych kanału lub metadanych starszego typu bez bieżących
metadanych setup albo config, których oczekuje ClawHub.

- Utrzymuj metadane zmiennych środowiskowych kanału jako deklaratywne, aby OpenClaw mógł sprawdzać stan setup
  bez ładowania runtime kanału.
- Odzwierciedl sterowany zmiennymi środowiskowymi setup kanału w bieżących metadanych setup, config kanału lub
  pakietowych metadanych kanału używanych przez kształt Twojego Plugin.
...
- Dokumentuj zachowanie wrażliwe z perspektywy bezpieczeństwa w publicznej dokumentacji pakietu lub
  README do czasu istnienia kontraktu manifestu.
- Uruchom ponownie `clawhub package validate <path-to-plugin>`.
