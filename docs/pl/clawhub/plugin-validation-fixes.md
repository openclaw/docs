---
read_when:
    - Uruchomiono clawhub package validate i trzeba naprawić wykryte problemy dotyczące Plugin
...
x-i18n:
    generated_at: "2026-07-01T13:22:43Z"
    model: gpt-5.5
...
automatycznych skanów pakietów. Ta strona opisuje wyniki widoczne dla autora, czyli
wyniki, które autor Plugin może naprawić w metadanych pakietu, manifeście, importach
SDK lub opublikowanym artefakcie.

Nie obejmuje ona wewnętrznych wyników pokrycia Plugin Inspector. Jeśli pełny raport
zawiera kody konserwacyjne skanera bez wskazówek dotyczących działań naprawczych dla autora, są
one przeznaczone dla opiekunów OpenClaw, a nie dla autorów Plugin.
...
- Użyj [Budowanie Plugin](/pl/plugins/building-plugins), aby zobaczyć minimalny przykład pakietu,
  oraz [Manifest Plugin](/pl/plugins/manifest#manifest-versus-packagejson), aby poznać podział między pakietem a manifestem.
...
- Użyj wersji API Plugin OpenClaw lub dolnej granicy semver, z którą budowano i testowano
  pakiet.
- Trzymaj to oddzielnie od wersji pakietu. Wersja pakietu opisuje
  wydanie Plugin; `openclaw.compat.pluginApi` opisuje kontrakt API hosta.
- Zobacz [pola package.json wpływające na wykrywanie](/pl/plugins/manifest#packagejson-fields-that-affect-discovery).
- Uruchom ponownie `clawhub package validate <path-to-plugin>`.
...
- Usuń `private: true`, jeśli ten pakiet jest przeznaczony do publikacji publicznej.
- Uruchom ponownie `clawhub package validate <path-to-plugin>`.
...
W spakowanym artefakcie brakuje metadanych OpenClaw, które istnieją w Twoim źródłowym
pakiecie.
...
Natywny manifest wtyczki nie zawiera nazwy wyświetlanej.

- Dodaj niepuste pole `name` do `openclaw.plugin.json`.
- Niech `name` będzie czytelne dla człowieka, a `id` pozostanie stabilnym identyfikatorem maszynowym.
- Zobacz [Manifest Plugin](/pl/plugins/manifest).
- Uruchom ponownie `clawhub package validate <path-to-plugin>`.

### manifest-unknown-fields

Manifest wtyczki ma pola najwyższego poziomu, których OpenClaw nie obsługuje.
...
- Przenieś zachowanie środowiska uruchomieniowego do kodu rejestracji wtyczki i ogranicz `contracts`
...
Wtyczka importuje z przestarzałego głównego barrela SDK:
`openclaw/plugin-sdk`.
...
Wtyczka importuje ścieżkę SDK zarezerwowaną dla dołączonych wtyczek lub wewnętrznej
zgodności.

- Zastąp zarezerwowane wewnętrzne importy OpenClaw SDK udokumentowanymi publicznymi
  podścieżkami `openclaw/plugin-sdk/*`.
- Jeśli zachowanie nie ma publicznego SDK, trzymaj pomocnik wewnątrz swojego pakietu albo
...
Wtyczka nadal używa przestarzałego pomocnika całego magazynu sesji
`loadSessionStore`.
...
- Zobacz [API środowiska uruchomieniowego](/pl/plugins/sdk-runtime#agent-session-state) oraz
  [podścieżki Plugin SDK](/pl/plugins/sdk-subpaths).
- Uruchom ponownie `clawhub package validate <path-to-plugin>`.

### sdk-session-store-write

Wtyczka nadal używa przestarzałego pomocnika zapisu całego magazynu sesji, takiego jak
...
Wtyczka nadal używa przestarzałych pomocników ścieżek plików sesji, takich jak
...
Wtyczka nadal używa przestarzałego pomocnika celu pliku transkrypcji
...
Wtyczka nadal używa przestarzałych niskopoziomowych pomocników transkrypcji, takich jak
...
Wtyczka nadal używa starszego hooka `before_agent_start`.
...
Manifest używa starszych metadanych zmiennych środowiskowych kanału albo starszego formatu bez obecnych
metadanych setup lub konfiguracji, których oczekuje ClawHub.

- Utrzymuj metadane zmiennych środowiskowych kanału jako deklaratywne, aby OpenClaw mógł sprawdzić status setup
  bez ładowania środowiska uruchomieniowego kanału.
- Odzwierciedl konfigurację kanału sterowaną zmiennymi środowiskowymi w bieżącym setup, konfiguracji kanału lub
  metadanych kanału pakietu używanych przez kształt Twojej wtyczki.
...
- Zobacz [Manifest Plugin](/pl/plugins/manifest) oraz
  [wtyczki kanałów](/pl/plugins/sdk-channel-plugins).
- Uruchom ponownie `clawhub package validate <path-to-plugin>`.
...
- Usuń URL schematu, jeśli ma tylko charakter doradczy.
...
- Usuń `openclaw.security.json`, dopóki OpenClaw nie udokumentuje wersjonowanego schematu manifestu bezpieczeństwa
  oraz zachowania ClawHub.
- Dokumentuj zachowanie wrażliwe na bezpieczeństwo w publicznej dokumentacji pakietu lub
...
- [Tworzenie wtyczek](/pl/plugins/building-plugins)
