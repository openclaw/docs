...
x-i18n:
    generated_at: "2026-07-04T04:09:27Z"
    model: gpt-5.5
...
automatycznych skanów pakietów. Ta strona obejmuje ustalenia przeznaczone dla autorów, czyli
...
zawiera kody konserwacji skanera bez wskazówek naprawczych dla autora, są one
...
- Użyj [konwencji importów](/pl/plugins/building-plugins#import-conventions) i
  [podścieżek Plugin SDK](/pl/plugins/sdk-subpaths), aby znaleźć wąski import.
- Uruchom ponownie `clawhub package validate <path-to-plugin>`.

### reserved-sdk-import

Plugin importuje ścieżkę SDK zarezerwowaną dla pakietowych Plugin lub wewnętrznej
zgodności.
...
- Używaj `getSessionEntry(...)`, aby odczytać metadane sesji według tożsamości agenta i sesji.
- Używaj `patchSessionEntry(...)` lub `upsertSessionEntry(...)`, aby utrwalać metadane sesji.
- Używaj tożsamości transkryptu lub pomocników celu, gdy kod przygotowuje operację
  transkryptu.
- Nie utrwalaj ani nie opieraj się na starszych ścieżkach plików transkryptów.
- Zobacz [API środowiska uruchomieniowego](/pl/plugins/sdk-runtime#agent-session-state) i
  [podścieżki Plugin SDK](/pl/plugins/sdk-subpaths).
- Uruchom ponownie `clawhub package validate <path-to-plugin>`.
...
- Używaj `appendSessionTranscriptMessageByIdentity(...)` do dopisywania transkryptu.
...
Manifest używa starszych lub wcześniejszych metadanych zmiennych środowiskowych kanału bez aktualnych
...
- Zobacz [manifest Plugin](/pl/plugins/manifest) i
  [Plugin kanałowe](/pl/plugins/sdk-channel-plugins).
- Uruchom ponownie `clawhub package validate <path-to-plugin>`.
