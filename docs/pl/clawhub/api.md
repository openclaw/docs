...
read_when:
    ...
    - Dodawanie endpointów lub schematów
summary: 'Publiczny interfejs REST API (v1): omówienie i konwencje.'
x-i18n:
    generated_at: "2026-07-02T14:11:23Z"
    model: gpt-5.5
...
- Buforuj odpowiedzi i respektuj nagłówki `429`, `Retry-After` oraz limitów szybkości zamiast agresywnego odpytywania.
...
- Nie twórz kopii lustrzanych ukrytych, prywatnych ani zablokowanych moderacją treści przez obchodzenie publicznych filtrów API lub granic uwierzytelniania.
...
- Błędy v1 są zwykłym tekstem (`text/plain; charset=utf-8`), w tym odpowiedzi `400`,
  `401`, `403`, `404`, `429` oraz zablokowanego pobierania.
...
- `GET /api/v1/skills?limit=&cursor=&sort=`
  - `sort`: `updated` (domyślnie), `recommended` (`default`), `createdAt` (`newest`), `downloads`, `stars` (`rating`), aliasy legacy instalacji `installsCurrent`/`installs`/`installsAllTime` mapują się na `downloads`, `trending`
