...
x-i18n:
    generated_at: "2026-07-01T15:32:08Z"
    model: gpt-5.5
...
Możesz zbudować katalog, directory lub powierzchnię wyszukiwania innej firmy na publicznych interfejsach API odczytu ClawHub. Publiczne metadane Skills i pliki Skills są publikowane zgodnie z zasadami licencji Skills ClawHub, natomiast samo API ma limity szybkości i powinno być używane odpowiedzialnie.
...
- Podczas wyświetlania list odsyłaj do kanonicznego URL Skills w ClawHub, aby użytkownicy mogli sprawdzić źródłowy rekord registry.
...
- Nie mirroruj ukrytych, prywatnych ani zablokowanych przez moderację treści przez omijanie publicznych filtrów API lub granic uwierzytelniania.
...
- Brakujący/nieprawidłowy token powoduje powrót do egzekwowania na podstawie IP.
...
- `X-RateLimit-Remaining` / `RateLimit-Remaining`: dokładny pozostały budżet, gdy
  ...
  wartość globalną
- `Retry-After`: opóźnienie w sekundach, które należy odczekać przy `429`
...
- `GET /api/v1/download?slug=&version=&tag=`
  ...
  - Bieżące Skills obsługiwane przez GitHub ze skanem `clean` lub `suspicious` zwracają
...
- `GET /api/v1/skills/export?startDate=&endDate=&limit=&cursor=`
  ...
  - Bieżące Skills obsługiwane przez GitHub ze skanem `clean` lub `suspicious` są eksportowane
...
- `POST /api/v1/users/reserve` rezerwuje główne slugi i prywatne placeholdery pakietów bez wydań dla uchwytu właściciela.
