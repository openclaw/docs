...
summary: Przegląd i konwencje publicznego API REST (v1).
x-i18n:
    generated_at: "2026-07-01T08:32:06Z"
    model: gpt-5.5
...
Możesz zbudować katalog, spis lub interfejs wyszukiwania innej firmy na publicznych interfejsach API odczytu ClawHub. Publiczne metadane umiejętności i pliki umiejętności są publikowane zgodnie z zasadami licencji umiejętności ClawHub, natomiast samo API ma limity szybkości i powinno być używane odpowiedzialnie.
...
- `X-RateLimit-Remaining` / `RateLimit-Remaining`: dokładny pozostały budżet, gdy
  jest obecny; udane żądania w systemie shardowanym pomijają go zamiast zwracać przybliżoną
  wartość globalną
...
- Błędy v1 są zwykłym tekstem (`text/plain; charset=utf-8`), w tym odpowiedzi `400`,
  `401`, `403`, `404`, `429` oraz odpowiedzi z zablokowanym pobieraniem.
...
- `GET /api/v1/search?q=...`
  ...
  - Alias starszego typu: `nonSuspicious=true`
...
- `GET /api/v1/download?slug=&version=&tag=`
  ...
  - Obecne umiejętności oparte na GitHub z wynikiem skanowania `clean` lub `suspicious` zwracają
...
- `GET /api/v1/skills/export?startDate=&endDate=&limit=&cursor=`
  ...
  - Obecne umiejętności oparte na GitHub z wynikiem skanowania `clean` lub `suspicious` są eksportowane
...
- `POST /api/v1/users/reserve` rezerwuje główne slugi i prywatne placeholdery pakietów bez wydania dla uchwytu właściciela.

## Starsze wersje

Starsze `/api/*` oraz `/api/cli/*` są nadal dostępne. Zobacz `DEPRECATIONS.md`.
