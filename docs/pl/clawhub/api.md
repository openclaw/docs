...
Możesz zbudować katalog, spis lub powierzchnię wyszukiwania innej firmy na publicznych interfejsach API odczytu ClawHub. Publiczne metadane Skills i pliki Skills są publikowane zgodnie z regułami licencji Skills ClawHub, natomiast samo API ma limity szybkości i powinno być używane odpowiedzialnie.
...
- Nie odzwierciedlaj ukrytych, prywatnych ani zablokowanych przez moderację treści przez obchodzenie publicznych filtrów API lub granic uwierzytelniania.
...
- `POST /api/v1/users/reserve` rezerwuje główne slugi i prywatne symbole zastępcze pakietów bez wydań dla identyfikatora właściciela.
