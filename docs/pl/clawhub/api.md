...
x-i18n:
    generated_at: "2026-07-03T17:45:20Z"
    model: gpt-5.5
...
Możesz zbudować katalog, spis lub powierzchnię wyszukiwania strony trzeciej na publicznych interfejsach API odczytu ClawHub. Publiczne metadane Skills i pliki Skills są publikowane zgodnie z zasadami licencji Skills ClawHub, natomiast samo API ma limity szybkości i należy z niego korzystać odpowiedzialnie.

Wytyczne:

- Używaj publicznych endpointów odczytu, takich jak `GET /api/v1/skills`, `GET /api/v1/search` i `GET /api/v1/skills/{slug}` do list katalogu.
...
- Nie lustrz ukrytych, prywatnych ani zablokowanych moderacją treści przez obchodzenie publicznych filtrów API lub granic uwierzytelniania.
...
- `X-RateLimit-Remaining` / `RateLimit-Remaining`: dokładny pozostały budżet, gdy
  występuje; udane żądania dzielone na shardy pomijają go zamiast zwracać przybliżoną
  wartość globalną
...
- `POST /api/v1/users/reserve` rezerwuje główne slugi i prywatne zastępcze pakiety bez wydań dla identyfikatora właściciela.
