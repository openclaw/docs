...
Możesz zbudować katalog, directory lub powierzchnię wyszukiwania strony trzeciej na publicznych interfejsach API odczytu ClawHub. Publiczne metadane umiejętności i pliki umiejętności są publikowane zgodnie z regułami licencji umiejętności ClawHub, a samo API ma limity szybkości i powinno być używane odpowiedzialnie.
...
- Podczas wyświetlania list odsyłaj do kanonicznego adresu URL umiejętności ClawHub, aby użytkownicy mogli sprawdzić źródłowy rekord rejestru.
...
- Nie twórz kopii ukrytych, prywatnych ani zablokowanych przez moderację treści przez obchodzenie publicznych filtrów API lub granic uwierzytelniania.
...
- `GET /api/v1/download?slug=&version=&tag=`
  ...
  - Bieżące umiejętności wspierane przez GitHub ze skanem `clean` lub `suspicious` zwracają
    deskryptor przekazania JSON `public-github` zamiast bajtów ClawHub.
- `GET /api/v1/skills/export?startDate=&endDate=&limit=&cursor=`
  - Hostowane umiejętności są eksportowane jako przechowywane pliki.
  - Bieżące umiejętności wspierane przez GitHub ze skanem `clean` lub `suspicious` są eksportowane
