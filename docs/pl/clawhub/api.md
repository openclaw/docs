...
Możesz zbudować zewnętrzny katalog, spis lub powierzchnię wyszukiwania na publicznych interfejsach API ClawHub do odczytu. Publiczne metadane skill oraz pliki skill są publikowane zgodnie z zasadami licencji skill ClawHub, natomiast samo API ma limity szybkości i powinno być używane odpowiedzialnie.
...
- Podczas wyświetlania list odsyłaj do kanonicznego URL skill w ClawHub, aby użytkownicy mogli sprawdzić źródłowy rekord rejestru.
- Używaj kanonicznych URL stron w postaci `https://clawhub.ai/<owner>/skills/<slug>`.
- Nie sugeruj, że ClawHub popiera, weryfikuje lub obsługuje zewnętrzną witrynę.
- Nie twórz kopii lustrzanych ukrytych, prywatnych ani zablokowanych przez moderację treści przez obchodzenie filtrów publicznego API lub granic uwierzytelniania.
...
- `X-RateLimit-Remaining` / `RateLimit-Remaining`: dokładny pozostały budżet, gdy
  jest obecny; udane żądania obsługiwane przez shardy pomijają go zamiast zwracać przybliżoną
  wartość globalną
...
- `GET /api/v1/download?slug=&version=&tag=`
  ...
  - Aktualne skills oparte na GitHub z wynikiem skanowania `clean` lub `suspicious` zwracają
...
- `GET /api/v1/skills/export?startDate=&endDate=&limit=&cursor=`
  ...
  - Aktualne skills oparte na GitHub z wynikiem skanowania `clean` lub `suspicious` są eksportowane
