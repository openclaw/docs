...
summary: Omówienie publicznego API REST (v1) i konwencje.
x-i18n:
    generated_at: "2026-07-02T22:51:06Z"
    model: gpt-5.5
...
Możesz zbudować katalog, spis lub powierzchnię wyszukiwania firmy trzeciej na publicznych interfejsach API odczytu ClawHub. Publiczne metadane Skills i pliki Skills są publikowane zgodnie z regułami licencji Skills ClawHub, natomiast samo API ma limity szybkości i należy z niego korzystać odpowiedzialnie.
...
- Buforuj odpowiedzi i respektuj nagłówki `429`, `Retry-After` oraz nagłówki limitów szybkości zamiast agresywnego odpytywania.
- Przy wyświetlaniu list odsyłaj do kanonicznego adresu URL Skills w ClawHub, aby użytkownicy mogli sprawdzić źródłowy rekord rejestru.
- Używaj kanonicznych adresów URL stron w formie `https://clawhub.ai/<owner>/skills/<slug>`.
- Nie sugeruj, że ClawHub popiera, weryfikuje lub obsługuje witrynę firmy trzeciej.
- Nie mirroruj treści ukrytych, prywatnych lub zablokowanych przez moderację przez obchodzenie publicznych filtrów API albo granic uwierzytelniania.
...
Egzekwowanie świadome uwierzytelniania:
...
- `GET /api/v1/skills?limit=&cursor=&sort=`
  - `sort`: `updated` (domyślne), `recommended` (`default`), `createdAt` (`newest`), `downloads`, `stars` (`rating`), starsze aliasy instalacji `installsCurrent`/`installs`/`installsAllTime` mapują na `downloads`, `trending`
