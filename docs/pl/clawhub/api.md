...
summary: Przegląd i konwencje publicznego interfejsu API REST (v1).
x-i18n:
...
Adres bazowy: `https://clawhub.ai`

OpenAPI: `/api/v1/openapi.json`

## Ponowne użycie katalogu publicznego

Możesz zbudować zewnętrzny katalog, spis lub powierzchnię wyszukiwania na publicznych interfejsach API odczytu ClawHub. Publiczne metadane Skills i pliki Skills są publikowane zgodnie z regułami licencyjnymi ClawHub dla Skills, natomiast samo API ma limity żądań i powinno być używane odpowiedzialnie.
...
- Buforuj odpowiedzi i respektuj nagłówki `429`, `Retry-After` oraz nagłówki limitów żądań zamiast agresywnego odpytywania.
- Linkuj z powrotem do kanonicznego adresu URL Skills w ClawHub podczas wyświetlania list, aby użytkownicy mogli sprawdzić źródłowy rekord rejestru.
- Używaj kanonicznych adresów URL stron w postaci `https://clawhub.ai/<owner>/skills/<slug>`.
- Nie sugeruj, że ClawHub wspiera, weryfikuje lub obsługuje zewnętrzną witrynę.
- Nie mirroruj ukrytych, prywatnych ani zablokowanych moderacyjnie treści przez obchodzenie publicznych filtrów API lub granic uwierzytelniania.
...
- Brakujący/nieprawidłowy token wraca do egzekwowania według adresu IP.
...
- `X-RateLimit-Remaining` / `RateLimit-Remaining`: dokładny pozostały budżet, gdy
  jest obecny; shardingowane udane żądania pomijają go zamiast zwracać przybliżoną
  wartość globalną
- `Retry-After`: opóźnienie w sekundach do odczekania przy `429`
...
- Błędy v1 są zwykłym tekstem (`text/plain; charset=utf-8`), w tym odpowiedzi `400`,
...
- `GET /api/v1/skills?limit=&cursor=&sort=`
  ...
  - `cursor` stosuje się do sortowań innych niż `trending`
