...
x-i18n:
    generated_at: "2026-07-01T20:36:29Z"
    model: gpt-5.5
...
Możesz zbudować katalog, spis lub powierzchnię wyszukiwania innej firmy na podstawie publicznych interfejsów API odczytu ClawHub. Publiczne metadane Skills i pliki Skills są publikowane zgodnie z regułami licencji Skills ClawHub, natomiast sam interfejs API ma limity szybkości i powinien być używany odpowiedzialnie.
...
- Nie twórz kopii ukrytej, prywatnej ani zablokowanej przez moderację zawartości przez obchodzenie publicznych filtrów API lub granic uwierzytelniania.
...
Egzekwowanie zależne od uwierzytelniania:
...
- `X-RateLimit-Remaining` / `RateLimit-Remaining`: dokładny pozostały budżet, gdy jest
  obecny; podzielone na shard udane żądania pomijają go zamiast zwracać przybliżoną
  wartość globalną
- `Retry-After`: sekundy opóźnienia do odczekania przy `429`
