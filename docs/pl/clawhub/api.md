...
Możesz zbudować zewnętrzny katalog, katalog zasobów lub powierzchnię wyszukiwania na publicznych interfejsach API odczytu ClawHub. Publiczne metadane Skills i pliki Skills są publikowane zgodnie z regułami licencji Skills ClawHub, natomiast samo API ma limity szybkości i powinno być używane odpowiedzialnie.
...
- Używaj kanonicznych URL stron w formie `https://clawhub.ai/<owner>/skills/<slug>`.
- Nie sugeruj, że ClawHub popiera, weryfikuje lub obsługuje zewnętrzną witrynę.
- Nie twórz kopii lustrzanych treści ukrytych, prywatnych ani zablokowanych przez moderację przez obchodzenie publicznych filtrów API lub granic uwierzytelniania.
...
- Błędy v1 są zwykłym tekstem (`text/plain; charset=utf-8`), w tym `400`,
  `401`, `403`, `404`, `429` oraz odpowiedzi z zablokowanych pobrań.
- Nieznane parametry zapytania są ignorowane dla kompatybilności.
