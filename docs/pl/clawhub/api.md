...
x-i18n:
    generated_at: "2026-07-05T08:44:37Z"
    model: gpt-5.5
...
Podstawa: `https://clawhub.ai`

OpenAPI: `/api/v1/openapi.json`

## Ponowne wykorzystanie katalogu publicznego

Możesz zbudować zewnętrzny katalog, spis lub powierzchnię wyszukiwania na publicznych API odczytu ClawHub. Publiczne metadane Skills i pliki Skills są publikowane zgodnie z regułami licencji Skills w ClawHub, natomiast samo API ma limity żądań i powinno być używane odpowiedzialnie.
...
- Buforuj odpowiedzi i respektuj `429`, `Retry-After` oraz nagłówki limitów żądań zamiast agresywnego odpytywania.
- Podczas wyświetlania list odsyłaj do kanonicznego URL Skills w ClawHub, aby użytkownicy mogli sprawdzić źródłowy rekord rejestru.
- Używaj kanonicznych URL stron w formacie `https://clawhub.ai/<owner>/skills/<slug>`.
- Nie sugeruj, że ClawHub popiera, weryfikuje lub prowadzi zewnętrzną witrynę.
- Nie odzwierciedlaj ukrytych, prywatnych ani zablokowanych przez moderację treści przez obchodzenie filtrów publicznego API lub granic uwierzytelniania.
...
- W przeciwnym razie użyj `RateLimit-Reset` lub wyprowadź opóźnienie z `X-RateLimit-Reset`.
