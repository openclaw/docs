...
x-i18n:
    generated_at: "2026-07-03T01:04:41Z"
    model: gpt-5.5
...
Możesz zbudować katalog, spis lub powierzchnię wyszukiwania innej firmy na publicznych interfejsach API odczytu ClawHub. Publiczne metadane Skills i pliki Skills są publikowane zgodnie z regułami licencji Skills ClawHub, natomiast sam interfejs API ma limity żądań i powinien być używany odpowiedzialnie.
...
- Nie odzwierciedlaj ukrytych, prywatnych ani zablokowanych moderacyjnie treści przez obchodzenie filtrów publicznego API lub granic uwierzytelniania.
...
- `X-RateLimit-Remaining` / `RateLimit-Remaining`: dokładny pozostały budżet, gdy
  ...
  wartość globalną
- `Retry-After`: opóźnienie w sekundach przed ponowieniem po `429`
...
- W przeciwnym razie użyj `RateLimit-Reset` albo wyznacz opóźnienie z `X-RateLimit-Reset`.
