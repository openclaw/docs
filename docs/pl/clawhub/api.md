...
x-i18n:
    generated_at: "2026-07-04T18:22:39Z"
    model: gpt-5.5
...
Możesz zbudować zewnętrzny katalog, spis lub powierzchnię wyszukiwania na publicznych interfejsach API odczytu ClawHub. Publiczne metadane umiejętności i pliki umiejętności są publikowane zgodnie z regułami licencji umiejętności ClawHub, natomiast samo API ma limity żądań i powinno być używane odpowiedzialnie.
...
- Podczas wyświetlania list odsyłaj do kanonicznego adresu URL umiejętności w ClawHub, aby użytkownicy mogli sprawdzić źródłowy rekord rejestru.
...
- Nie twórz kopii ukrytych, prywatnych ani zablokowanych przez moderację treści przez omijanie publicznych filtrów API lub granic uwierzytelniania.
...
Egzekwowanie zależne od uwierzytelnienia:

- Żądania anonimowe: według adresu IP.
- Żądania uwierzytelnione (prawidłowy token Bearer): według koszyka użytkownika.
- Brakujący lub nieprawidłowy token powoduje powrót do egzekwowania według adresu IP.
...
- `X-RateLimit-Remaining` / `RateLimit-Remaining`: dokładny pozostały budżet, gdy jest
  obecny; podzielone na fragmenty udane żądania pomijają go, zamiast zwracać przybliżoną
  wartość globalną
- `Retry-After`: opóźnienie w sekundach przed ponowną próbą przy `429`
...
- Dodaj losowy jitter do ponownych prób.
...
- `GET /api/v1/download?slug=&version=&tag=`
  - Hostowane umiejętności zwracają deterministyczne bajty ZIP.
  - Bieżące umiejętności oparte na GitHub ze skanem `clean` lub `suspicious` zwracają
    deskryptor przekazania JSON `public-github` zamiast bajtów ClawHub.
- `GET /api/v1/skills/export?startDate=&endDate=&limit=&cursor=`
  - Hostowane umiejętności są eksportowane jako zapisane pliki.
  - Bieżące umiejętności oparte na GitHub ze skanem `clean` lub `suspicious` są eksportowane
