...
Możesz zbudować katalog, spis lub powierzchnię wyszukiwania innej firmy na publicznych interfejsach API do odczytu ClawHub. Publiczne metadane Skills i pliki Skills są publikowane zgodnie z zasadami licencji Skills ClawHub, a sam interfejs API jest objęty limitami szybkości i należy używać go odpowiedzialnie.
...
- Podczas wyświetlania list linkuj z powrotem do kanonicznego URL Skills ClawHub, aby użytkownicy mogli sprawdzić źródłowy rekord rejestru.
- Używaj kanonicznych URL-i stron w formie `https://clawhub.ai/<owner>/<slug>`.
- Nie sugeruj, że ClawHub popiera, weryfikuje lub obsługuje witrynę innej firmy.
- Nie odzwierciedlaj ukrytych, prywatnych ani zablokowanych przez moderację treści przez omijanie filtrów publicznego API lub granic autoryzacji.
...
- `POST /api/v1/users/reserve` rezerwuje główne slugi oraz prywatne symbole zastępcze pakietów bez wydań dla uchwytu właściciela.
