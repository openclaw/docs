...
x-i18n:
    generated_at: "2026-07-03T10:00:48Z"
    model: gpt-5.5
...
Możesz zbudować katalog, spis lub powierzchnię wyszukiwania innej firmy na podstawie publicznych API odczytu ClawHub. Publiczne metadane Skills i pliki Skills są publikowane zgodnie z regułami licencji Skills ClawHub, natomiast samo API ma limity żądań i powinno być używane odpowiedzialnie.
...
- Brakujący/nieprawidłowy token powoduje użycie egzekwowania według adresu IP.
...
- `X-RateLimit-Remaining` / `RateLimit-Remaining`: dokładny pozostały budżet, gdy
  jest obecny; podzielone na shard pomyślne żądania pomijają go zamiast zwracać przybliżoną
  wartość globalną
