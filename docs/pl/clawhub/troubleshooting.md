...
x-i18n:
    generated_at: "2026-07-02T22:51:04Z"
    model: gpt-5.5
...
CLI uruchamia krótkotrwały lokalny serwer callback podczas logowania przez przeglądarkę.

- Upewnij się, że przeglądarka może połączyć się z `http://127.0.0.1:<port>/callback`.
- Sprawdź lokalną zaporę, VPN i reguły proxy, jeśli callback nigdy nie dociera.
...
- Potwierdź, że wydanie jest publiczne i nie jest wstrzymane przez skanowanie ani moderację.
...
Najpierw podejrzyj payload publikowania:
...
## Publikowanie kończy się niepowodzeniem, ponieważ namespace jest zajęty lub zarezerwowany

Jeśli publikowanie kończy się niepowodzeniem, ponieważ uchwyt właściciela, namespace organizacji, zakres pakietu, slug skill
lub nazwa pakietu są już zajęte albo zarezerwowane, najpierw potwierdź, że publikujesz
jako właściciel zgodny z namespace. W przypadku pakietów pluginów
nazwy ze scopingiem, takie jak `@example-org/example-plugin`, muszą być publikowane jako
odpowiadający właściciel `example-org`.

Jeśli uważasz, że Twoja organizacja, projekt lub marka jest prawowitym właścicielem namespace, ale
nie możesz zarządzać bieżącym właścicielem ClawHub, otwórz
[zgłoszenie roszczenia organizacji / namespace](https://github.com/openclaw/clawhub/issues/new?template=org-namespace-claim.yml)
z publicznym, niewrażliwym dowodem. Zobacz
[Roszczenia organizacji i namespace](/clawhub/namespace-claims), aby uzyskać wskazówki dotyczące dowodów i informacji, których
...
- Respektuj nagłówki ponowienia `429` i cache'uj publiczne odpowiedzi listowania/wyszukiwania.
