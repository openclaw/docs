...
x-i18n:
    generated_at: "2026-07-04T04:09:35Z"
    model: gpt-5.5
...
CLI uruchamia krótkotrwały lokalny serwer callbacku podczas logowania przez przeglądarkę.
...
- Jeśli używasz tokenu API, potwierdź, że nie został cofnięty w interfejsie webowym.
...
## Publikowanie nie działa z powodu błędu właściciela lub źródła GitHub
...
Jeśli publikowanie nie działa, ponieważ uchwyt właściciela, przestrzeń nazw organizacji, zakres pakietu, slug skilla
...
nazwy ze zakresem, takie jak `@example-org/example-plugin`, muszą być publikowane jako
...
[Roszczenia organizacji i przestrzeni nazw](/clawhub/namespace-claims), aby poznać wytyczne dotyczące dowodów oraz informacje, czego
nie umieszczać w publicznych zgłoszeniach.

## `sync` mówi, że nie znaleziono żadnych skilli
...
- Potwierdź, że Twoja wersja OpenClaw spełnia deklarowany przez pakiet
  zakres zgodności.
- Jeśli pakiet jest ukryty, zatrzymany lub zablokowany, może nie dać się zainstalować, dopóki
  właściciel nie rozwiąże problemu.

## Publiczne żądania API kończą się niepowodzeniem

- Respektuj nagłówki ponawiania `429` i cache'uj odpowiedzi publicznych list/wyszukiwań.
- Kieruj użytkowników z powrotem do kanonicznej pozycji ClawHub.
