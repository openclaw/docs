...
summary: Rozwiązywanie problemów z logowaniem, instalacją, publikowaniem, aktualizacją i interfejsem API w ClawHub.
x-i18n:
    generated_at: "2026-06-30T14:27:22Z"
    model: gpt-5.5
...
- Upewnij się, że przeglądarka może osiągnąć `http://127.0.0.1:<port>/callback`.
- Sprawdź lokalną zaporę sieciową, VPN i reguły proxy, jeśli callback nigdy nie dociera.
...
- `Retry-After`: liczba sekund do odczekania przed ponowną próbą.
...
publikujesz jako właściciel pasujący do przestrzeni nazw. W przypadku pakietów plugins
...
- Nie kopiuj ukrytej, prywatnej, wstrzymanej ani zablokowanej przez moderację zawartości poza
