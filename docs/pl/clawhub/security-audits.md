...
x-i18n:
    generated_at: "2026-06-28T10:03:55Z"
    model: gpt-5.5
...
wolne od ryzyka. Zawsze kieruj się własną oceną przed przyznaniem dostępu do wrażliwych zasobów.

Zobacz też [Bezpieczeństwo](/pl/clawhub/security), [Dopuszczalne użycie](/pl/clawhub/acceptable-usage)
oraz [Moderowanie i bezpieczeństwo konta](/pl/clawhub/moderation).
...
| `Review`    | Przeczytaj ustalenia przed instalacją. Wydanie nadal może być zgodne z przeznaczeniem. |
...
Poziom ryzyka opisuje blast radius: jak dużą władzę wydanie wydaje się mieć, jeśli
używasz go zgodnie z przeznaczeniem.

| Poziom ryzyka | Znaczenie                                                                       |
| ---------- | ----------------------------------------------------------------------------- |
| `Low`      | Wykryto niewielkie uprawnienia do wrażliwych zasobów lub mały wpływ na użytkownika.                          |
...
- Poziom ryzyka pyta: „Jak duża władza jest tutaj dostępna?”
- Status audytu pyta: „Co mam zrobić z tym wynikiem?”

Na przykład skill do publikowania może pokazywać `Review` z ryzykiem `Medium`. To
...
- odpowiednią treść skilla lub Pluginu
- rekomendację
...
- instrukcje skilla lub metadane Pluginu
...
Zaawansowane zachowanie nie jest automatycznie złe. Wiele użytecznych narzędzi potrzebuje poświadczeń,
...
VirusTotal pozostaje telemetrią. Nie zastępuje własnej, świadomej artefaktu
analizy ryzyka ClawHub.

## Analiza ryzyka

Analiza ryzyka jest wewnętrznie obsługiwana przez ClawScan, własny system audytu
bezpieczeństwa ClawHub. Przegląda każde wydanie jako artefakt przeznaczony dla agenta: instrukcje,
