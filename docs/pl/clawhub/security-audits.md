...
x-i18n:
    generated_at: "2026-07-04T11:04:33Z"
    model: gpt-5.5
...
Audyty bezpieczeństwa ClawHub pomagają zdecydować, czy skill lub plugin jest wystarczająco
bezpieczny do instalacji. Pokazują, co robi wydanie, o jakie uprawnienia prosi i
...
wolne od ryzyka. Zawsze kieruj się własnym osądem przed przyznaniem dostępu do wrażliwych zasobów.
...
`Pass` jest uspokajający, ale nie zastępuje własnego osądu. Ma to największe
znaczenie w przypadku narzędzi, które mogą publikować treści, edytować dane, uruchamiać polecenia, czytać pliki lub
uzyskiwać dostęp do systemów produkcyjnych.

## Poziom ryzyka

Poziom ryzyka opisuje zakres oddziaływania: jak dużą władzę wydanie wydaje się mieć, jeśli
użyjesz go zgodnie z przeznaczeniem.

| Poziom ryzyka | Znaczenie                                                                      |
| ---------- | ----------------------------------------------------------------------------- |
| `Low`      | Wykryto niewielkie uprawnienia do wrażliwych zasobów lub niewielki wpływ na użytkownika. |
...
- Poziom ryzyka pyta: „Jak dużo władzy tu jest?”
- Status audytu pyta: „Co mam zrobić z tym wynikiem?”

Na przykład skill do publikowania może mieć status `Review` i ryzyko `Medium`. To
nie oznacza, że jest złośliwy. Oznacza, że skill wydaje się zgodny z celem, ale może
...
- odpowiednią zawartość skilla lub plugina
- rekomendację
...
pozostawała skoncentrowana na użytecznych dowodach.

## Co sprawdza ClawHub

ClawHub audytuje przesłane artefakty wydań, w tym:

- instrukcje skilla lub metadane plugina
...
poleceń lokalnych, interfejsów API dostawców lub instalacji pakietów. Audyt sprawdza, czy ta
...
sygnałów reputacji, które uzupełniają świadomy kontekstu agentów przegląd ClawHub. Gdy dostępne
są liczby silników dostawców, audyt podsumowuje je prostym językiem, na przykład:
...
Analiza ryzyka jest wewnętrznie zasilana przez ClawScan, własny system audytu
bezpieczeństwa ClawHub. Przegląda każde wydanie jako artefakt skierowany do agenta: instrukcje,
...
zadeklarowany przypadek użycia wydania.
