...
summary: Jak interpretować wyniki audytu bezpieczeństwa ClawHub przed zainstalowaniem Skills lub Plugin.
title: Audyty bezpieczeństwa
x-i18n:
    generated_at: "2026-07-01T20:38:03Z"
    model: gpt-5.5
...
wolne od ryzyka. Przed przyznaniem wrażliwego dostępu zawsze kieruj się własną oceną.
...
| `Review`    | Przed instalacją przeczytaj ustalenia. Wydanie nadal może być wiarygodne. |
...
Poziom ryzyka opisuje zasięg skutków: jak dużą moc wydaje się mieć wydanie, jeśli
używasz go zgodnie z przeznaczeniem.

| Poziom ryzyka | Znaczenie                                                                       |
| ---------- | ----------------------------------------------------------------------------- |
| `Low`      | Znaleziono niewiele wrażliwych uprawnień lub niewielki wpływ na użytkownika.                          |
...
- Status audytu pyta: „Co powinienem zrobić z tym wynikiem?”

Na przykład umiejętność publikowania może pokazywać `Review` z ryzykiem `Medium`. Nie oznacza to,
że jest złośliwa. Oznacza to, że umiejętność wydaje się zgodna z celem, ale może
...
Zaawansowane działanie nie jest automatycznie złe. Wiele użytecznych narzędzi potrzebuje poświadczeń,
...
zaufanym standardem branżowym reputacji plików i skanowania malware, a nasze
...
liczby dostawców silników, audyt podsumowuje je prostym językiem, na przykład:
...
Pyta, czy możliwość jest ujawniona, zgodna z celem i wsparta
