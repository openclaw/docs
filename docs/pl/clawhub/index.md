...
- Używaj osobnego CLI `clawhub` do uwierzytelniania w rejestrze, publikowania, usuwania/przywracania usunięć oraz przepływów synchronizacji.
...
publikowanie, synchronizacja lub usuwanie/przywracanie usunięć:
...
pobrania, gwiazdki i podsumowania skanowania bezpieczeństwa. Strony publiczne pokazują bieżący
stan rejestru, aby użytkownicy mogli sprawdzić Skill lub plugin przed instalacją.

## Natywne przepływy OpenClaw

Natywne polecenia OpenClaw instalują w aktywnej przestrzeni roboczej OpenClaw i zapisują
...
Surowe specyfikacje pluginów bezpieczne dla npm mogą być rozwiązywane przez npm podczas przełączeń startowych, a
...
CLI ma także polecenia instalowania/aktualizowania Skills dla bezpośrednich przepływów rejestru:
...
`openclaw.build.openclawVersion`. Zobacz [CLI](/pl/clawhub/cli), aby uzyskać pełną referencję poleceń,
oraz [Format Skill](/pl/clawhub/skill-format), aby uzyskać metadane Skill.
...
| `CLAWHUB_CONFIG_PATH`         | Nadpisuje miejsce przechowywania stanu tokenu/konfiguracji przez CLI. |
...
[Rozwiązywanie problemów](/pl/clawhub/troubleshooting), aby uzyskać bardziej szczegółowy materiał referencyjny.
