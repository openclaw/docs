...
title: ClawHub
x-i18n:
    generated_at: "2026-06-30T22:37:43Z"
    model: gpt-5.5
...
ClawHub jest publicznym rejestrem Skills i wtyczek OpenClaw.
...
pobrania, gwiazdki oraz podsumowania skanów bezpieczeństwa. Strony publiczne pokazują bieżący stan rejestru,
aby użytkownicy mogli sprawdzić Skill lub wtyczkę przed instalacją.
...
Użyj `clawhub:<package>`, gdy instalacja wtyczki ma zostać rozwiązana przez ClawHub.
Gołe specyfikacje wtyczek bezpieczne dla npm mogą być rozwiązywane przez npm podczas przejść uruchomieniowych, a
`npm:<package>` pozostaje wyłącznie npm, gdy źródło musi być jawne.

Instalacje wtyczek weryfikują deklarowaną zgodność `pluginApi` i `minGatewayVersion`
...
Publikuj wtyczki z lokalnego folderu, `owner/repo`, `owner/repo@ref` albo adresu URL GitHub:
...
poleceń, oraz [Format Skill](/clawhub/skill-format), aby uzyskać metadane Skills.
...
ClawHub uruchamia automatyczne kontrole opublikowanych Skills i wydań wtyczek. Wydania
...
Gdy uruchamiasz `clawhub install` będąc zalogowanym, CLI może wysłać zdarzenie instalacji
w trybie best-effort, aby ClawHub mógł obliczać zagregowane liczniki instalacji. Wyłącz to za pomocą:
