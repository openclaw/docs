...
publikowanie albo usuwanie i przywracanie:
...
pobrania, gwiazdki oraz podsumowania skanowania bezpieczeństwa. Strony publiczne pokazują bieżący
stan rejestru, aby użytkownicy mogli sprawdzić Skills lub plugin przed instalacją.
...
Same specyfikacje pluginów zgodne z npm mogą być rozwiązywane przez npm podczas przełączeń startowych, a
...
ClawPack, OpenClaw preferuje dokładnie przesłany pakiet npm `.tgz`, weryfikuje
...
CLI ma także polecenia instalowania i aktualizowania Skills dla bezpośrednich przepływów rejestrowych:
...
Publikuj Skills z folderu lokalnego zawierającego `SKILL.md`:
...
Publikuj pluginy z folderu lokalnego, `owner/repo`, `owner/repo@ref` albo adresu URL
GitHub:
...
poleceń, oraz [Format Skills](/pl/clawhub/skill-format), aby poznać metadane Skills.
...
wstrzymane przez skanowanie albo zablokowane mogą zniknąć z publicznego katalogu i powierzchni instalacji, pozostając
widoczne dla swojego właściciela w `/dashboard`.
...
Gdy uruchamiasz `clawhub install` po zalogowaniu, CLI może wysłać, w miarę możliwości,
