...
- Używaj osobnego CLI `clawhub` do uwierzytelniania w rejestrze, publikowania oraz przepływów usuwania/przywracania usuniętych elementów.
...
publikowanie lub usuwanie/przywracanie usuniętych elementów:
...
pobrania, gwiazdki i podsumowania skanów bezpieczeństwa. Publiczne strony pokazują aktualny stan rejestru,
...
Gdy uruchamiasz `clawhub install` będąc zalogowanym, CLI może wysłać zdarzenie instalacji w trybie najlepszej próby,
...
| `CLAWHUB_CONFIG_PATH`         | Nadpisuje miejsce, w którym CLI zapisuje stan tokena/konfiguracji. |
