...
title: ClawHub
x-i18n:
    generated_at: "2026-07-01T20:37:18Z"
    model: gpt-5.5
...
ClawHub to publiczny rejestr OpenClaw Skills i pluginów.
...
| --------------- | --------------------------------------------------------------- | -------------------------------------------- |
| Skills          | Wersjonowane pakiety tekstowe z `SKILL.md` oraz plikami wsparcia | `openclaw skills install @openclaw/demo`     |
...
metadane źródła, aby późniejsze polecenia aktualizacji mogły nadal korzystać z ClawHub.

Użyj `clawhub:<package>`, gdy instalacja pluginu ma być rozwiązana przez ClawHub.
Gołe, bezpieczne dla npm specyfikacje pluginów mogą być rozwiązywane przez npm podczas przejściowych wdrożeń,
a `npm:<package>` pozostaje wyłącznie npm, gdy źródło musi być jawne.

Instalacje pluginów sprawdzają reklamowaną zgodność `pluginApi` i `minGatewayVersion`
...
CLI ma też polecenia instalowania/aktualizacji Skills dla bezpośrednich przepływów rejestru:
...
dokumentację poleceń, oraz [Format Skill](/clawhub/skill-format), aby poznać metadane Skill.

## Bezpieczeństwo i moderacja

ClawHub jest domyślnie otwarty: każdy może przesyłać pakiety, ale publikowanie wymaga konta GitHub
...
lub zablokowane mogą zniknąć z publicznego katalogu i powierzchni instalacji, pozostając jednocześnie
...
w trybie best-effort, aby ClawHub mógł obliczać zbiorcze liczby instalacji. Wyłącz to za pomocą:
