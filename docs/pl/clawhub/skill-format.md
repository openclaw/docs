...
x-i18n:
    generated_at: "2026-07-03T17:44:27Z"
    model: gpt-5.5
...
Metadane umiejętności deklaruje się we frontmatter YAML na początku pliku `SKILL.md`. Informują one rejestr (oraz analizę bezpieczeństwa), czego umiejętność potrzebuje do działania.
...
| `requires.anyBins` | `string[]` | Pliki binarne CLI, z których musi istnieć co najmniej jeden.                                                                                                |
...
- Jeśli umiejętność integruje się z płatną usługą zewnętrzną, jasno udokumentuj koszt zewnętrzny i wymagane konto w instrukcjach umiejętności oraz deklaracjach env (`requires.env` dla wymaganych zmiennych albo `envVars` z `required: false` dla zmiennych opcjonalnych).
