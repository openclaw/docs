...
x-i18n:
    generated_at: "2026-07-03T23:44:26Z"
    model: gpt-5.5
...
- `.clawhubignore` (wzorce ignorowania dla publikowania, starsze `.clawdhubignore`)
...
Analiza bezpieczeństwa ClawHub sprawdza, czy to, co deklaruje umiejętność, zgadza się z tym, co faktycznie robi. Jeśli kod odwołuje się do `TODOIST_API_KEY`, ale frontmatter nie deklaruje jej w `requires.env`, `primaryEnv` ani `envVars`, analiza oznaczy niezgodność metadanych. Utrzymywanie dokładnych deklaracji pomaga umiejętności przejść weryfikację i pomaga użytkownikom zrozumieć, co instalują.
...
- Wszystkie umiejętności publikowane w ClawHub są objęte licencją `MIT-0`.
...
- Jeśli umiejętność integruje się z płatną usługą zewnętrzną, jasno udokumentuj zewnętrzny koszt i wymagane konto w instrukcjach umiejętności oraz deklaracjach env (`requires.env` dla wymaganych zmiennych lub `envVars` z `required: false` dla opcjonalnych zmiennych).
