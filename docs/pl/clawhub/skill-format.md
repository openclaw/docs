...
- `SKILL.md` (lub `skill.md`; starszy `skills.md` też jest akceptowany)
...
`SKILL.md` lub starsze pliki `skills.md` w publicznych repozytoriach, które nie są forkami i należą do
...
- Markdown z opcjonalnym nagłówkiem YAML.
- Serwer wyodrębnia metadane z nagłówka YAML podczas publikowania.
- `description` jest używane jako podsumowanie umiejętności w UI/wyszukiwaniu.

## Metadane w nagłówku YAML

Metadane umiejętności deklaruje się w nagłówku YAML na początku pliku `SKILL.md`. Informuje to rejestr (i analizę bezpieczeństwa), czego umiejętność potrzebuje do uruchomienia.

### Podstawowy nagłówek YAML
...
Zadeklaruj wymagania runtime swojej umiejętności w `metadata.openclaw` (aliasy: `metadata.clawdbot`, `metadata.clawdis`).
...
| `requires.bins`    | `string[]` | Binarne pliki CLI, które wszystkie muszą być zainstalowane.                                                                                |
| `requires.anyBins` | `string[]` | Binarne pliki CLI, z których musi istnieć co najmniej jeden.                                                                               |
...
- Tekst do osadzania obejmuje `SKILL.md` + do około 40 plików innych niż `.md` (limit typu best effort).
...
- Wszystkie umiejętności publikowane w ClawHub są licencjonowane na warunkach `MIT-0`.
- Każdy może używać, modyfikować i redystrybuować opublikowane umiejętności, także komercyjnie.
- Atrybucja nie jest wymagana.
- Nie dodawaj sprzecznych warunków licencyjnych w `SKILL.md`; ClawHub nie obsługuje nadpisywania licencji dla pojedynczych umiejętności.

## Płatne umiejętności

- ClawHub nie obsługuje płatnych umiejętności, cen dla pojedynczych umiejętności, paywalli ani dzielenia przychodów.
...
- Jeśli Twoja umiejętność integruje się z płatną usługą strony trzeciej, jasno udokumentuj zewnętrzny koszt i wymagane konto w instrukcjach umiejętności oraz deklaracjach środowiska (`requires.env` dla wymaganych zmiennych albo `envVars` z `required: false` dla opcjonalnych zmiennych).
