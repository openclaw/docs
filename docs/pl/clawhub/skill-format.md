...
x-i18n:
    generated_at: "2026-07-04T15:39:02Z"
    model: gpt-5.5
...
Metadane umiejętności są deklarowane we frontmatter YAML na początku pliku `SKILL.md`. Informuje to rejestr (i analizę bezpieczeństwa), czego umiejętność potrzebuje do działania.
...
- Typy zawartości zaczynające się od `text/` są traktowane jako tekst, wraz z małą listą dozwolonych typów (JSON/YAML/TOML/JS/TS/Markdown/SVG).

Limity (po stronie serwera):

- Całkowity rozmiar pakietu: 50 MB.
- Osadzany tekst obejmuje `SKILL.md` + do około 40 plików innych niż `.md` (limit typu best-effort).
