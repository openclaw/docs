...
x-i18n:
    generated_at: "2026-07-01T13:23:46Z"
    model: gpt-5.5
...
- `SKILL.md` (lub `skill.md`; akceptowane jest też starsze `skills.md`)
...
repozytoriów zarchiwizowanych/wyłączonych ani publicznych repozytoriów stron trzecich.
...
Metadane Skill są deklarowane we frontmatter YAML na początku pliku `SKILL.md`. Informuje to rejestr (oraz analizę bezpieczeństwa), czego Skill potrzebuje do uruchomienia.
...
---
name: my-skill
description: Krótkie podsumowanie tego, co robi ten Skill.
version: 1.0.0
---
```
...
| `requires.anyBins` | `string[]` | Binaria CLI, z których musi istnieć co najmniej jedno.                                                                               |
| `requires.config`  | `string[]` | Ścieżki plików konfiguracyjnych odczytywane przez Twoją umiejętność.                                                                 |
| `primaryEnv`       | `string`   | Główna zmienna środowiskowa z danymi uwierzytelniającymi dla Twojej umiejętności.                                                     |
...
Deklaruj opcjonalne zmienne środowiskowe w `metadata.openclaw.envVars` i ustaw `required: false`. Nie dodawaj opcjonalnych wpisów do `requires.env`, ponieważ `requires.env` oznacza, że umiejętność nie może bez nich działać.
...
- Tekst do osadzania obejmuje `SKILL.md` + do około 40 plików innych niż `.md` (limit best-effort).

## Slugi

- Domyślnie wyprowadzane z nazwy folderu.
- Zakresy pakietów muszą dokładnie odpowiadać uchwytowi wydawcy w ClawHub. Uchwyty wydawców mogą używać małych liter, cyfr, łączników, kropek i podkreśleń; muszą zaczynać się i kończyć małą literą lub cyfrą.
...
- Jeśli Twoja umiejętność integruje się z płatną usługą zewnętrzną, jasno udokumentuj zewnętrzny koszt i wymagane konto w instrukcjach umiejętności oraz deklaracjach środowiska (`requires.env` dla wymaganych zmiennych albo `envVars` z `required: false` dla opcjonalnych zmiennych).
