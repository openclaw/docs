...
x-i18n:
    generated_at: "2026-07-03T01:05:14Z"
    model: gpt-5.5
...
- `SKILL.md` (lub `skill.md`; starszy `skills.md` również jest akceptowany)
...
- `description` jest używany jako podsumowanie umiejętności w UI/wyszukiwaniu.
...
Zadeklaruj opcjonalne zmienne środowiskowe w `metadata.openclaw.envVars` i ustaw `required: false`. Nie dodawaj opcjonalnych wpisów do `requires.env`, ponieważ `requires.env` oznacza, że umiejętność nie może bez nich działać.
...
Analiza bezpieczeństwa ClawHub sprawdza, czy deklaracje umiejętności są zgodne z tym, co faktycznie robi. Jeśli kod odwołuje się do `TODOIST_API_KEY`, ale frontmatter nie deklaruje go w `requires.env`, `primaryEnv` ani `envVars`, analiza oznaczy niezgodność metadanych. Utrzymywanie dokładnych deklaracji pomaga umiejętności przejść przegląd i pomaga użytkownikom zrozumieć, co instalują.
...
- Typy treści zaczynające się od `text/` są traktowane jako tekst; dodatkowo obowiązuje krótka lista dozwolonych typów (JSON/YAML/TOML/JS/TS/Markdown/SVG).

Limity (po stronie serwera):

- Całkowity rozmiar pakietu: 50 MB.
- Osadzany tekst obejmuje `SKILL.md` + do około 40 plików innych niż `.md` (limit stosowany w miarę możliwości).

## Slugi

- Domyślnie wyprowadzane z nazwy folderu.
- Zakresy pakietów muszą dokładnie odpowiadać uchwytowi wydawcy ClawHub. Uchwyty wydawców mogą zawierać małe litery, cyfry, łączniki, kropki i podkreślenia; muszą zaczynać się i kończyć małą literą lub cyfrą.
...
- Wszystkie umiejętności opublikowane w ClawHub są licencjonowane na zasadach `MIT-0`.
- Każdy może używać, modyfikować i rozpowszechniać opublikowane umiejętności, również komercyjnie.
...
- Jeśli umiejętność integruje się z płatną usługą zewnętrzną, jasno opisz zewnętrzny koszt i wymagane konto w instrukcjach umiejętności oraz deklaracjach środowiskowych (`requires.env` dla wymaganych zmiennych albo `envVars` z `required: false` dla zmiennych opcjonalnych).
