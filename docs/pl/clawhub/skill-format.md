...
x-i18n:
    generated_at: "2026-07-02T17:48:14Z"
    model: gpt-5.5
...
Webowy importer GitHub jest bardziej restrykcyjny niż lokalne publikowanie/synchronizacja. Wykrywa tylko
pliki `SKILL.md` lub starsze `skills.md` w publicznych repozytoriach nieforkowanych należących do
zalogowanego konta GitHub. Nie importuje prywatnych repozytoriów, forków,
zarchiwizowanych/wyłączonych repozytoriów ani publicznych repozytoriów osób trzecich.
...
Metadane umiejętności deklaruje się we frontmatter YAML na początku pliku `SKILL.md`. Informują one rejestr (i analizę bezpieczeństwa), czego umiejętność potrzebuje do działania.
...
| `homepage`         | `string`   | URL do strony domowej lub dokumentacji umiejętności.                                                                                                           |
...
Analiza bezpieczeństwa ClawHub sprawdza, czy deklaracje umiejętności odpowiadają temu, co faktycznie robi. Jeśli kod odwołuje się do `TODOIST_API_KEY`, ale frontmatter nie deklaruje jej w `requires.env`, `primaryEnv` ani `envVars`, analiza oznaczy niezgodność metadanych. Utrzymywanie dokładnych deklaracji pomaga umiejętności przejść przegląd i pomaga użytkownikom zrozumieć, co instalują.
...
- Zakresy pakietów muszą dokładnie odpowiadać uchwytowi wydawcy ClawHub. Uchwyty wydawców mogą używać małych liter, cyfr, myślników, kropek i podkreśleń; muszą zaczynać się i kończyć małą literą lub cyfrą.
...
- Nie dodawaj sprzecznych warunków licencji w `SKILL.md`; ClawHub nie obsługuje nadpisywania licencji dla pojedynczych umiejętności.

## Płatne umiejętności

- ClawHub nie obsługuje płatnych umiejętności, cen dla pojedynczych umiejętności, paywalli ani podziału przychodów.
...
- Jeśli umiejętność integruje się z płatną usługą zewnętrzną, jasno udokumentuj koszt zewnętrzny i wymagane konto w instrukcjach umiejętności oraz deklaracjach env (`requires.env` dla wymaganych zmiennych lub `envVars` z `required: false` dla zmiennych opcjonalnych).
