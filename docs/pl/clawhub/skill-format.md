...
x-i18n:
    generated_at: "2026-07-01T15:32:43Z"
    model: gpt-5.5
...
- `description` jest używane jako podsumowanie umiejętności w interfejsie UI/wyszukiwaniu.
...
Zadeklaruj wymagania runtime umiejętności w `metadata.openclaw` (aliasy: `metadata.clawdbot`, `metadata.clawdis`).
...
- Jeśli umiejętność integruje się z płatną usługą zewnętrzną, jasno udokumentuj koszt zewnętrzny i wymagane konto w instrukcjach umiejętności oraz deklaracjach środowiska (`requires.env` dla wymaganych zmiennych lub `envVars` z `required: false` dla opcjonalnych zmiennych).
