...
Metadane umiejętności są deklarowane we frontmatter YAML na początku pliku `SKILL.md`. Informują rejestr (oraz analizę bezpieczeństwa), czego Twoja umiejętność potrzebuje do działania.
...
Użyj `requires.env` dla zmiennych środowiskowych, które muszą istnieć, zanim umiejętność będzie mogła działać. Użyj `envVars`, gdy potrzebujesz metadanych dla poszczególnych zmiennych, w tym zmiennych opcjonalnych z `required: false`.
...
| `emoji`            | `string`   | Wyświetlane emoji dla umiejętności.                                                                                                                    |
...
- Tagi to wskaźniki tekstowe do wersji; często używa się `latest`.
...
- ClawHub nie obsługuje płatnych umiejętności, cen dla poszczególnych umiejętności, płatnych ścian ani podziału przychodów.
...
- Jeśli Twoja umiejętność integruje się z płatną usługą zewnętrzną, jasno udokumentuj koszt zewnętrzny i wymagane konto w instrukcjach umiejętności oraz deklaracjach środowiska (`requires.env` dla wymaganych zmiennych albo `envVars` z `required: false` dla zmiennych opcjonalnych).
