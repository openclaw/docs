...
| `homepage`         | `string`   | URL do strony głównej lub dokumentacji umiejętności.                                                                                      |
...
Analiza bezpieczeństwa ClawHub sprawdza, czy deklaracje umiejętności odpowiadają temu, co faktycznie robi. Jeśli kod odwołuje się do `TODOIST_API_KEY`, ale frontmatter nie deklaruje go w `requires.env`, `primaryEnv` ani `envVars`, analiza zgłosi niezgodność metadanych. Dokładne deklaracje pomagają umiejętności przejść weryfikację i pomagają użytkownikom zrozumieć, co instalują.
...
- Typy treści zaczynające się od `text/` są traktowane jako tekst; plus mała lista dozwolonych typów (JSON/YAML/TOML/JS/TS/Markdown/SVG).
...
- Jeśli umiejętność integruje się z płatną usługą zewnętrzną, jasno udokumentuj koszt zewnętrzny i wymagane konto w instrukcjach umiejętności oraz deklaracjach środowiska (`requires.env` dla wymaganych zmiennych albo `envVars` z `required: false` dla zmiennych opcjonalnych).
