...
x-i18n:
    generated_at: "2026-07-02T08:53:33Z"
    model: gpt-5.5
...
Webowy importer GitHub jest bardziej restrykcyjny niż lokalne publikowanie/synchronizacja. Wykrywa tylko pliki
...
Stan instalacji workdir (zapisywany przez CLI):
...
- `description` jest używane jako podsumowanie Skill w interfejsie użytkownika/wyszukiwaniu.

## Metadane frontmatter

Metadane Skill są deklarowane we frontmatter YAML na początku pliku `SKILL.md`. Informuje to rejestr (oraz analizę bezpieczeństwa), czego Skill potrzebuje do działania.
...
Zadeklaruj wymagania środowiska uruchomieniowego swojego Skill w `metadata.openclaw` (aliasy: `metadata.clawdbot`, `metadata.clawdis`).
...
Użyj `requires.env` dla zmiennych środowiskowych, które muszą być obecne, zanim Skill będzie można uruchomić. Użyj `envVars`, gdy potrzebujesz metadanych dla poszczególnych zmiennych, w tym zmiennych opcjonalnych z `required: false`.
...
| `primaryEnv`       | `string`   | Główna zmienna środowiskowa z poświadczeniami dla Twojej umiejętności.                                                                             |
...
| `install`          | `array`    | Specyfikacje instalacji zależności (patrz niżej).                                                                                                  |
| `nix`              | `object`   | Specyfikacja pluginu Nix (patrz README).                                                                                                           |
| `config`           | `object`   | Specyfikacja konfiguracji Clawdbot (patrz README).                                                                                                 |

### Specyfikacje instalacji

Jeśli Twoja umiejętność wymaga zainstalowanych zależności, zadeklaruj je w tablicy `install`:
...
Deklaruj opcjonalne zmienne środowiskowe w `metadata.openclaw.envVars` i ustaw `required: false`. Nie dodawaj opcjonalnych wpisów do `requires.env`, ponieważ `requires.env` oznacza, że umiejętność nie może działać bez nich.
...
Analiza bezpieczeństwa ClawHub sprawdza, czy to, co deklaruje Twoja umiejętność, odpowiada temu, co faktycznie robi. Jeśli Twój kod odwołuje się do `TODOIST_API_KEY`, ale frontmatter nie deklaruje jej w `requires.env`, `primaryEnv` ani `envVars`, analiza zgłosi niezgodność metadanych. Dokładne deklaracje pomagają Twojej umiejętności przejść przegląd i pomagają użytkownikom zrozumieć, co instalują.
...
- Typy zawartości zaczynające się od `text/` są traktowane jako tekst; plus mała lista dozwolonych typów (JSON/YAML/TOML/JS/TS/Markdown/SVG).

Limity (po stronie serwera):

- Całkowity rozmiar pakietu: 50 MB.
- Tekst do osadzania obejmuje `SKILL.md` + maksymalnie około 40 plików innych niż `.md` (limit best-effort).
...
- Nie dodawaj sprzecznych warunków licencyjnych w `SKILL.md`; ClawHub nie obsługuje nadpisań licencji dla poszczególnych umiejętności.

## Płatne umiejętności

- ClawHub nie obsługuje płatnych umiejętności, cen dla poszczególnych umiejętności, paywalli ani dzielenia przychodów.
- Nie dodawaj metadanych cen do `SKILL.md`; nie są częścią formatu umiejętności i nie sprawią, że opublikowana umiejętność będzie płatna.
- Jeśli Twoja umiejętność integruje się z płatną usługą zewnętrzną, wyraźnie udokumentuj koszt zewnętrzny i wymagane konto w instrukcjach umiejętności oraz deklaracjach środowiskowych (`requires.env` dla wymaganych zmiennych albo `envVars` z `required: false` dla opcjonalnych zmiennych).
