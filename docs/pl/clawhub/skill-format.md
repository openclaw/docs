...
Analiza bezpieczeństwa ClawHub sprawdza, czy to, co deklaruje umiejętność, jest zgodne z tym, co faktycznie robi. Jeśli kod odwołuje się do `TODOIST_API_KEY`, ale frontmatter nie deklaruje jej w `requires.env`, `primaryEnv` ani `envVars`, analiza oznaczy niezgodność metadanych. Utrzymywanie dokładnych deklaracji pomaga umiejętności przejść przegląd i pomaga użytkownikom zrozumieć, co instalują.
...
- Tekst do osadzania obejmuje `SKILL.md` + maksymalnie około 40 plików innych niż `.md` (limit na zasadzie najlepszych starań).
