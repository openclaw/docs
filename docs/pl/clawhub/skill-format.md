...
pliki `SKILL.md` lub starsze `skills.md` w publicznych, nieforkowanych repozytoriach należących do
...
- Jeśli Twoja umiejętność integruje się z płatną usługą firmy trzeciej, jasno udokumentuj koszt zewnętrzny i wymagane konto w instrukcjach umiejętności oraz deklaracjach env (`requires.env` dla wymaganych zmiennych albo `envVars` z `required: false` dla zmiennych opcjonalnych).
