...
Metadane umiejętności są deklarowane we frontmatter YAML na początku pliku `SKILL.md`. Informuje to rejestr (oraz analizę bezpieczeństwa), czego umiejętność potrzebuje do uruchomienia.
...
Analiza bezpieczeństwa ClawHub sprawdza, czy to, co deklaruje Twoja umiejętność, odpowiada temu, co faktycznie robi. Jeśli Twój kod odwołuje się do `TODOIST_API_KEY`, ale frontmatter nie deklaruje jej w `requires.env`, `primaryEnv` ani `envVars`, analiza oznaczy niezgodność metadanych. Utrzymywanie dokładnych deklaracji pomaga Twojej umiejętności przejść przegląd i pomaga użytkownikom zrozumieć, co instalują.
...
Publikowanie akceptuje wyłącznie pliki „tekstowe”.

- Lista dozwolonych rozszerzeń znajduje się w `packages/schema/src/textFiles.ts` (`TEXT_FILE_EXTENSIONS`).
- Pliki skryptów nadal są skanowane po przesłaniu; pliki PowerShell `.ps1`, `.psm1` i `.psd1` są akceptowane jako tekst.
...
- Tagi są wskaźnikami tekstowymi do wersji; `latest` jest powszechnie używany.
...
- ClawHub nie obsługuje płatnych umiejętności, cen dla poszczególnych umiejętności, płatnych bramek ani podziału przychodów.
- Nie dodawaj metadanych cen do `SKILL.md`; nie są one częścią formatu umiejętności i nie sprawią, że opublikowana umiejętność stanie się płatna.
- Jeśli Twoja umiejętność integruje się z płatną usługą zewnętrzną, jasno udokumentuj zewnętrzny koszt i wymagane konto w instrukcjach umiejętności oraz deklaracjach env (`requires.env` dla wymaganych zmiennych albo `envVars` z `required: false` dla zmiennych opcjonalnych).
