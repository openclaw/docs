...
Metadane umiejętności są deklarowane we frontmatter YAML na początku pliku `SKILL.md`. Informują one rejestr (oraz analizę bezpieczeństwa), czego umiejętność potrzebuje do działania.
...
Analiza bezpieczeństwa ClawHub sprawdza, czy deklaracje umiejętności odpowiadają temu, co faktycznie robi. Jeśli kod odwołuje się do `TODOIST_API_KEY`, ale frontmatter nie deklaruje jej w `requires.env`, `primaryEnv` ani `envVars`, analiza oznaczy niezgodność metadanych. Dokładne deklaracje pomagają umiejętności przejść przegląd i pomagają użytkownikom zrozumieć, co instalują.
...
- Zakresy pakietów muszą dokładnie odpowiadać nazwie wydawcy ClawHub. Nazwy wydawców mogą używać małych liter, cyfr, łączników, kropek i podkreśleń; muszą zaczynać się i kończyć małą literą lub cyfrą.
...
- Jeśli umiejętność integruje się z płatną usługą zewnętrzną, jasno udokumentuj zewnętrzny koszt i wymagane konto w instrukcjach umiejętności oraz deklaracjach środowiska (`requires.env` dla wymaganych zmiennych lub `envVars` z `required: false` dla zmiennych opcjonalnych).
