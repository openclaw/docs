...
x-i18n:
    generated_at: "2026-06-30T22:36:31Z"
    model: gpt-5.5
...
Internetowy importer GitHub jest bardziej rygorystyczny niż lokalne publikowanie/synchronizacja. Wykrywa tylko
...
Analiza bezpieczeństwa ClawHub sprawdza, czy deklaracje umiejętności odpowiadają temu, co faktycznie robi. Jeśli kod odwołuje się do `TODOIST_API_KEY`, ale frontmatter nie deklaruje jej w `requires.env`, `primaryEnv` ani `envVars`, analiza oznaczy niezgodność metadanych. Utrzymywanie poprawnych deklaracji pomaga umiejętności przejść przegląd i pomaga użytkownikom zrozumieć, co instalują.
