...
zarchiwizowanych/wyłączonych repozytoriów ani publicznych repozytoriów zewnętrznych.
...
| `envVars`          | `array`    | Deklaracje zmiennych środowiskowych z `name`, opcjonalnym `required` i opcjonalnym `description`. Ustaw `required: false` dla opcjonalnych zmiennych env vars. |
...
- ClawHub nie obsługuje płatnych umiejętności, cenników dla poszczególnych umiejętności, paywalli ani udziału w przychodach.
...
- Jeśli umiejętność integruje się z płatną usługą zewnętrzną, jasno udokumentuj zewnętrzny koszt i wymagane konto w instrukcjach umiejętności oraz deklaracjach env (`requires.env` dla wymaganych zmiennych lub `envVars` z `required: false` dla zmiennych opcjonalnych).
