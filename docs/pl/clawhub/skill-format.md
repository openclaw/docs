...
x-i18n:
    generated_at: "2026-07-04T06:52:06Z"
    model: gpt-5.5
...
- `SKILL.md` (lub `skill.md`; starszy `skills.md` jest też akceptowany)
...
- `description` jest używane jako podsumowanie skilla w interfejsie użytkownika/wyszukiwaniu.

## Metadane frontmatter

Metadane skilla deklaruje się we frontmatter YAML na początku pliku `SKILL.md`. Informuje to rejestr (i analizę bezpieczeństwa), czego skill potrzebuje do działania.

### Podstawowe frontmatter
...
### Metadane runtime (`metadata.openclaw`)

Zadeklaruj wymagania runtime swojego skilla w `metadata.openclaw` (aliasy: `metadata.clawdbot`, `metadata.clawdis`).
...
| `requires.config`  | `string[]` | Ścieżki plików konfiguracji odczytywane przez skill.                                                                                                      |
| `primaryEnv`       | `string`   | Główna zmienna środowiskowa poświadczeń dla skilla.                                                                                                       |
| `envVars`          | `array`    | Deklaracje zmiennych środowiskowych z `name`, opcjonalnym `required` i opcjonalnym `description`. Ustaw `required: false` dla opcjonalnych zmiennych env. |
...
Jeśli skill wymaga zainstalowanych zależności, zadeklaruj je w tablicy `install`:
...
Zadeklaruj opcjonalne zmienne środowiskowe w `metadata.openclaw.envVars` i ustaw `required: false`. Nie dodawaj opcjonalnych wpisów do `requires.env`, ponieważ `requires.env` oznacza, że skill nie może działać bez tych zmiennych.
...
Analiza bezpieczeństwa ClawHub sprawdza, czy deklaracje skilla odpowiadają temu, co faktycznie robi. Jeśli kod odwołuje się do `TODOIST_API_KEY`, ale frontmatter nie deklaruje jej w `requires.env`, `primaryEnv` ani `envVars`, analiza oznaczy niezgodność metadanych. Utrzymywanie dokładnych deklaracji pomaga skillowi przejść przegląd i pomaga użytkownikom zrozumieć, co instalują.

### Przykład: kompletne frontmatter
...
- Tekst osadzania obejmuje `SKILL.md` + do około 40 plików innych niż `.md` (limit best-effort).
...
- Wszystkie skille opublikowane w ClawHub są licencjonowane na warunkach `MIT-0`.
- Każdy może używać, modyfikować i redystrybuować opublikowane skille, także komercyjnie.
- Atrybucja nie jest wymagana.
- Nie dodawaj sprzecznych warunków licencyjnych w `SKILL.md`; ClawHub nie obsługuje nadpisywania licencji dla pojedynczych skilli.

## Płatne skille

- ClawHub nie obsługuje płatnych skilli, cen dla pojedynczych skilli, paywalli ani udziału w przychodach.
- Nie dodawaj metadanych cenowych do `SKILL.md`; nie są częścią formatu skilla i nie sprawią, że opublikowany skill będzie płatny.
- Jeśli skill integruje się z płatną usługą zewnętrzną, jasno udokumentuj zewnętrzny koszt i wymagane konto w instrukcjach skilla oraz deklaracjach env (`requires.env` dla wymaganych zmiennych albo `envVars` z `required: false` dla zmiennych opcjonalnych).
