...
Metadane umiejętności są deklarowane we frontmatter YAML na początku pliku `SKILL.md`. Informują one rejestr (oraz analizę bezpieczeństwa), czego potrzebuje Twoja umiejętność do działania.
...
| `primaryEnv`       | `string`   | Główna zmienna środowiskowa poświadczenia dla Twojej umiejętności.                                                                     |
...
Jeśli Twoja umiejętność potrzebuje zainstalowanych zależności, zadeklaruj je w tablicy `install`:
...
- Tekst osadzania obejmuje `SKILL.md` + do około 40 plików innych niż `.md` (limit według najlepszych starań).

## Slugi

- Domyślnie tworzone z nazwy folderu.
- Muszą być pisane małymi literami i bezpieczne dla URL: `^[a-z0-9][a-z0-9-]*$`.

## Wersjonowanie + tagi

- Każde publikowanie tworzy nową wersję (semver).
...
- Jeśli Twoja umiejętność integruje się z płatną usługą zewnętrzną, jasno udokumentuj koszt zewnętrzny i wymagane konto w instrukcjach umiejętności oraz deklaracjach środowiskowych (`requires.env` dla wymaganych zmiennych albo `envVars` z `required: false` dla zmiennych opcjonalnych).
