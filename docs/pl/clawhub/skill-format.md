...
| `primaryEnv`       | `string`   | Główna zmienna środowiskowa poświadczenia dla umiejętności.                                                                                           |
...
Zadeklaruj opcjonalne zmienne środowiskowe w `metadata.openclaw.envVars` i ustaw `required: false`. Nie dodawaj wpisów opcjonalnych do `requires.env`, ponieważ `requires.env` oznacza, że umiejętność nie może działać bez tych zmiennych.
...
Podczas publikowania akceptowane są tylko pliki „tekstowe”.
...
- Typy zawartości zaczynające się od `text/` są traktowane jako tekst; dodatkowo obowiązuje krótka lista dozwolonych typów (JSON/YAML/TOML/JS/TS/Markdown/SVG).

Limity (po stronie serwera):

- Łączny rozmiar pakietu: 50 MB.
- Osadzany tekst obejmuje `SKILL.md` + maksymalnie około 40 plików innych niż `.md` (limit stosowany w miarę możliwości).
