...
x-i18n:
    generated_at: "2026-05-12T23:29:47Z"
    model: gpt-5.5
    provider: openai
...
- `.clawhubignore` (wzorce ignorowania przy publikowaniu/synchronizacji, starsze `.clawdhubignore`)
...
Metadane umiejętności deklaruje się we frontmatter YAML na początku pliku `SKILL.md`. Informują one rejestr (i analizę bezpieczeństwa), czego potrzebuje umiejętność do działania.
...
### Metadane środowiska wykonawczego (`metadata.openclaw`)

Zadeklaruj wymagania środowiska wykonawczego umiejętności w `metadata.openclaw` (aliasy: `metadata.clawdbot`, `metadata.clawdis`).
...
- Tekst do osadzania obejmuje `SKILL.md` + maksymalnie około 40 plików innych niż `.md` (limit stosowany w miarę możliwości).
...
- Jeśli umiejętność integruje się z płatną usługą zewnętrzną, jasno udokumentuj zewnętrzny koszt i wymagane konto w instrukcjach umiejętności oraz deklaracjach środowiska (`requires.env` dla wymaganych zmiennych albo `envVars` z `required: false` dla opcjonalnych zmiennych).
