...
- `SKILL.md` (lub `skill.md`; starsze `skills.md` też jest akceptowane)
...
pliki `SKILL.md` lub starsze pliki `skills.md` w publicznych repozytoriach niebędących forkami, należących do
...
| `homepage`         | `string`   | URL do strony głównej lub dokumentacji skilla.                                                                                                     |
...
Analiza bezpieczeństwa ClawHub sprawdza, czy to, co deklaruje skill, odpowiada temu, co faktycznie robi. Jeśli kod odwołuje się do `TODOIST_API_KEY`, ale frontmatter nie deklaruje jej w `requires.env`, `primaryEnv` ani `envVars`, analiza zgłosi niezgodność metadanych. Utrzymywanie dokładnych deklaracji pomaga skillowi przejść przegląd i pomaga użytkownikom zrozumieć, co instalują.
...
- Zakresy pakietów muszą dokładnie pasować do identyfikatora wydawcy ClawHub. Identyfikatory wydawców mogą używać małych liter, cyfr, łączników, kropek i podkreśleń; muszą zaczynać się i kończyć małą literą lub cyfrą.
...
- Wszystkie Skills opublikowane w ClawHub są licencjonowane na `MIT-0`.
...
- ClawHub nie obsługuje płatnych Skills, cen dla poszczególnych skilli, paywalli ani podziału przychodów.
...
- Jeśli skill integruje się z płatną usługą zewnętrzną, jasno udokumentuj koszt zewnętrzny i wymagane konto w instrukcjach skilla oraz deklaracjach zmiennych środowiskowych (`requires.env` dla wymaganych zmiennych albo `envVars` z `required: false` dla opcjonalnych zmiennych).
