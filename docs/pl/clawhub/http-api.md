...
summary: Dokumentacja referencyjna HTTP API (publiczne + endpointy CLI + uwierzytelnianie).
x-i18n:
...
Katalogi firm trzecich mogą używać publicznych endpointów odczytu do wyświetlania lub wyszukiwania Skills ClawHub. Buforuj wyniki, respektuj `429`/`Retry-After`, odsyłaj użytkowników do kanonicznej pozycji w ClawHub (`https://clawhub.ai/<owner>/skills/<slug>`) i unikaj sugerowania, że ClawHub popiera witrynę firmy trzeciej. Nie próbuj odzwierciedlać ukrytych, prywatnych ani zablokowanych przez moderację treści poza publiczną powierzchnią API.
...
- Endpointy zapisu z uwierzytelnieniem nie powinny zwracać samego `Unauthorized`, gdy
...
- Pobieranie: 1200/min per IP, 6000/min per klucz (endpointy pobierania)
...
- Trafność jest silniejsza niż popularność. Precyzyjne dopasowanie tokenu sluga lub wyświetlanej nazwy może wyprzedzić luźniejsze dopasowanie ze znacznie silniejszym zaangażowaniem.
...
- Popularność jest skalowana logarytmicznie i ograniczana. Skills z wysokim zaangażowaniem mogą mieć niższą pozycję, gdy tekst zapytania jest słabszym dopasowaniem.
...
- Umieść terminy, których użytkownicy będą dosłownie szukać, w wyświetlanej nazwie, podsumowaniu i tagach. Użyj samodzielnego tokenu sluga tylko wtedy, gdy jest on także stabilną tożsamością, którą chcesz zachować.
...
- `sort` (opcjonalne): `updated` (domyślnie), `recommended` (alias: `default`), `createdAt` (alias: `newest`), `downloads`, `stars` (alias: `rating`), starsze aliasy instalacji `installsCurrent`/`installs`/`installsAllTime` mapują do `downloads`, `trending`
- `nonSuspiciousOnly` (opcjonalne): `true`, aby ukryć podejrzane (`flagged.suspicious`) Skills
- `nonSuspicious` (opcjonalne): starszy alias dla `nonSuspiciousOnly`
...
- `metadata.os`: ograniczenia OS zadeklarowane w frontmatter Skill (np. `["macos"]`, `["linux"]`). `null`, jeśli nie zadeklarowano.
...
- `moderation` jest dołączane tylko wtedy, gdy Skill jest oflagowany lub gdy wyświetla go właściciel.
...
- Ładunki żądań skanowania i raporty do pobrania wygasają w magazynie żądań skanowania po okresie przechowywania.
- Opublikowane skanowania wymagają dostępu właściciela/wydawcy do zarządzania albo uprawnień moderatora/administratora platformy.
...
- Zadania skanowania są asynchroniczne. Ręczne żądania skanowania mają wyższy priorytet niż normalne prace publikowania/uzupełniania, ale ukończenie nadal zależy od dostępności workerów.
...
- Domyślną wartością `kind` jest `skill`; użyj `kind=plugin` dla skanowań pluginów/pakietów.
- Zwraca ten sam kształt ZIP co pobrania żądań skanowania.

### `POST /api/v1/skills/-/scan/batch`

Kanonizowana trasa wsadowego ponownego skanowania dostępna tylko dla administratorów. Akceptuje ten sam kształt ładunku co starsze `POST /api/v1/skills/-/rescan-batch`.

### `POST /api/v1/skills/-/scan/batch/status`

Kanonizowana trasa statusu wsadu dostępna tylko dla administratorów. Akceptuje `{ "jobIds": ["..."] }` i zwraca te same zagregowane liczniki co starsze `POST /api/v1/skills/-/rescan-batch/status`.
...
- Wyniki są zwracane osobno dla każdego elementu; jeden brakujący skill lub wersja nie powoduje niepowodzenia całej odpowiedzi.
...
- `security.signals.dependencyRegistry` jest zachowane dla zgodności odpowiedzi v1, ale skaner istnienia rejestru zależności został wycofany i ten klucz zawsze ma wartość `null`.
- Brak Skill Card nie wpływa na `ok`, `decision` ani `reasons` tego endpointu; klienci powinni lokalnie odczytywać zainstalowany `skill-card.md`, gdy potrzebują zawartości karty.
...
- `category` (opcjonalny): filtr kategorii Plugin. Obsługiwany tylko wtedy, gdy
  żądanie jest zawężone do pakietów Plugin (`/api/v1/plugins`,
...
- Wpisy Skills nadal opierają się na rejestrze Skills i nadal można je publikować tylko przez `POST /api/v1/skills`.
...
Ujednolicone wyszukiwanie katalogu wśród Skills i pakietów Plugin.
...
- `isOfficial` (opcjonalny): `true` albo `false`
- `category` (opcjonalny): filtr kategorii Plugin. Obsługiwany tylko wtedy, gdy
  żądanie jest zawężone do pakietów Plugin. Kontrolowane kategorie i starsze aliasy
...
Przeglądanie katalogu wyłącznie dla Plugin wśród pakietów code-plugin i bundle-plugin.
...
- `startDate` (wymagany): dolna granica Unix w milisekundach dla `updatedAt` Skills.
- `endDate` (wymagany): górna granica Unix w milisekundach dla `updatedAt` Skills.
...
- Obecne Skills oparte na GitHub ze skanem `clean` albo `suspicious` zawierają
...
- Kontrolki sortowania w interfejsie przeglądarki dla wyszukiwania pluginów porządkują załadowane wyniki według trafności,
...
- Wydania ClawPack zawierają pola zgodne z npm: `npmIntegrity`, `npmShasum` i
  `npmTarballName`.
- `version.sha256hash` to przestarzałe metadane zgodności dla starych klientów. Pole
  haszuje dokładne bajty ZIP zwracane przez `/api/v1/packages/{name}/download`.
...
Zwraca dokładne podsumowanie bezpieczeństwa i zaufania dla wydania pakietu dla klientów
instalacyjnych. Jest to publiczna powierzchnia użycia OpenClaw do decydowania, czy
rozwiązane wydanie może zostać zainstalowane.

Uwierzytelnianie:

- Publiczny punkt końcowy odczytu. Nie jest wymagany token właściciela, publikującego, moderatora ani administratora.
...
- Wersje ClawPack strumieniują dokładne przesłane bajty `.tgz` pakietu npm-pack.
...
- `open`: podejrzane, złośliwe, oczekujące, objęte kwarantanną, wycofane albo zgłoszone wydania.
- `blocked`: wydania objęte kwarantanną, wycofane albo złośliwe.
...
zatwierdzania, obejmowania kwarantanną albo wycofywania artefaktów.
...
- Packumenty pakietów z zakresem obsługują zarówno ścieżkę `/api/npm/@scope/name`, jak i zakodowaną
...
Pobiera hostowaną wersję umiejętności jako ZIP albo zwraca przekazanie do źródła GitHub dla
...
- Opcjonalne pole ładunku: `migrateOwner`. Gdy `true` z `ownerHandle`, istniejąca
  ...
  wydawców: bieżącego i docelowego. Bez tego jawnego wyboru zmiany właściciela są
  odrzucane.
...
- Dozwolone pola formularza to `payload`, powtarzane bloby `files` albo jedna referencja tarballa
  `clawpack`. `clawpack` może być blobem `.tgz` albo identyfikatorem storage zwróconym przez
  przepływ upload-url. Publikacje etapowane z identyfikatorem storage muszą też zawierać
...
- Code plugins wymagają `package.json`, metadanych repozytorium źródłowego, metadanych commita
  źródłowego, metadanych schematu konfiguracji, `openclaw.compat.pluginApi` i
...
- Publikacje w czyimś imieniu nadal walidują kwalifikowalność kanału oficjalnego względem docelowego konta właściciela.
...
Gdy obecne, `reason` jest przechowywane jako notatka moderacyjna umiejętności i kopiowane do dziennika audytu.
...
wydawca użytkownika docelowego ma skills, packages lub źródła GitHub skill.
Odzyskiwanie migruje również starsze pola `ownerUserId` dla skills odzyskanego wydawcy,
aliasów slugów skill, pakietów, ostrzeżeń inspektora pakietów oraz pochodnych wierszy digestu wyszukiwania, aby
ścieżki bezpośredniego właściciela były zgodne z nowym autorytetem wydawcy. Aktywna rezerwacja chronionego handle
dla odzyskanego handle jest również przypisywana użytkownikowi zastępczemu, aby późniejsza
...
100 wierszy na transakcję apply; większe odzyskiwania muszą najpierw użyć wznawialnej migracji właściciela.
Źródła GitHub skill mają zakres wydawcy i są zgłaszane jako sprawdzone, a nie przepisywane.
