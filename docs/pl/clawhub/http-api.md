...
x-i18n:
    generated_at: "2026-07-01T15:31:15Z"
    model: gpt-5.5
...
Katalogi innych firm mogą używać publicznych punktów końcowych odczytu do wyświetlania lub wyszukiwania umiejętności ClawHub. Buforuj wyniki, respektuj `429`/`Retry-After`, odsyłaj użytkowników do kanonicznego wpisu ClawHub (`https://clawhub.ai/<owner>/skills/<slug>`) i unikaj sugerowania, że ClawHub popiera witrynę innej firmy. Nie próbuj odzwierciedlać ukrytych, prywatnych ani zablokowanych przez moderację treści poza publiczną powierzchnią API.
...
- Jeśli nie jest dostępny żaden zaufany IP klienta, żądania anonimowe używają kubełków awaryjnych
...
powinni odczytywać treść odpowiedzi jako ciąg czytelny dla człowieka. Nieznane parametry zapytania są
...
- Trafność jest silniejsza niż popularność. Precyzyjne dopasowanie tokenu sluga lub nazwy wyświetlanej może wyprzedzić luźniejsze dopasowanie ze znacznie silniejszym zaangażowaniem.
...
- Popularność jest skalowana logarytmicznie i ograniczana. Umiejętności o wysokim zaangażowaniu mogą zajmować niższe miejsce, gdy tekst zapytania jest słabszym dopasowaniem.
...
- Umieszczaj terminy, których użytkownicy będą dosłownie szukać, w nazwie wyświetlanej, podsumowaniu i tagach. Używaj samodzielnego tokenu sluga tylko wtedy, gdy jest także stabilną tożsamością, którą chcesz zachować.
...
- `createdAt` jest stabilne dla crawlów nowych umiejętności; `updated` zmienia się, gdy istniejące umiejętności są ponownie publikowane.
...
- `moderation` jest dołączane tylko wtedy, gdy umiejętność jest oznaczona lub ogląda ją właściciel.
...
Zgłoś umiejętność do sprawdzenia przez moderatora. Zgłoszenia są na poziomie umiejętności, opcjonalnie powiązane
...
Skanowania lokalnych przesłanych plików nie są już obsługiwane. Żądania używające
...
- Ładunki żądań skanowania i raporty do pobrania wygasają z magazynu żądań skanowania po upływie okresu retencji.
...
- Zadania skanowania są asynchroniczne. Ręczne żądania skanowania mają wyższy priorytet niż zwykłe prace publikacji/wypełniania wstecznego, ale ukończenie nadal zależy od dostępności workerów.
...
- Wymaga dostępu właściciela/wydawcy do zarządzania Skill lub pluginem albo uprawnień moderatora/administratora platformy.
...
- Tożsamość Skill, tożsamość wydawcy i metadane wybranej wersji są polami najwyższego poziomu koperty (`slug`, `displayName`, `publisherHandle`, `version`, `resolvedFrom`, `tag`, `createdAt`), aby automatyzacja powłoki mogła je odczytać bez rozpakowywania zagnieżdżonych opakowań.
...
- Wyniki są zwracane dla każdego elementu; brakujący Skill lub wersja nie powoduje niepowodzenia całej odpowiedzi.
...
- Wpisy umiejętności nadal są oparte na rejestrze umiejętności i wciąż mogą być publikowane tylko przez `POST /api/v1/skills`.
...
Ujednolicone wyszukiwanie katalogu wśród Skills i pakietów pluginów.
...
Przeglądanie katalogu wyłącznie Pluginów wśród pakietów code-plugin i bundle-plugin.
...
`trending` to siedmiodniowa tabela liderów instalacji/pobrań i nie używa łącznych wartości z całego czasu.
...
- Aktualne Skills oparte na GitHub z wynikiem skanowania `clean` lub `suspicious` zawierają
...
- `startDate` (wymagany): dolna granica Unix w milisekundach dla `updatedAt` Pluginu.
- `endDate` (wymagany): górna granica Unix w milisekundach dla `updatedAt` Pluginu.
- `limit` (opcjonalny): liczba całkowita (1-250), domyślnie `250`.
...
- Kontrolki sortowania interfejsu przeglądarki dla wyszukiwania Pluginów zmieniają kolejność wczytanych wyników trafności,
  zgodnie z bieżącym działaniem przeglądania `/skills`.
...
- `version.artifact.kind` ma wartość `legacy-zip` dla starszych archiwów pakietów albo
...
- `trust.stale` oznacza, że podsumowanie zaufania obliczono z nieaktualnych danych wejściowych i
  należy je traktować jako wymagające odświeżenia przed decyzją o zezwoleniu z wysoką pewnością.
...
- Ten punkt końcowy jest celowo węższy niż punkty końcowe moderacji właściciela/moderatora.
  ...
  tożsamości zgłaszających, treści zgłoszeń, prywatne dowody ani wewnętrzne harmonogramy
  przeglądu.
...
- To jest powierzchnia resolvera OpenClaw; pozwala uniknąć zgadywania formatu archiwum ze
...
- Limit rozmiaru pliku: 200KB.
- Oczekujące skany VirusTotal nie blokują odczytów; złośliwe wydania mogą nadal być wstrzymywane w innych miejscach.
...
- `slug` (wymagany)
- `hash` (wymagany): 64-znakowy szesnastkowy sha256 odcisku palca bundla
...
- Dozwolone pola formularza to `payload`, powtarzane bloby `files` lub jedno odwołanie do tarballa `clawpack`.
  `clawpack` może być blobem `.tgz` lub identyfikatorem storage zwróconym przez
  przepływ upload-url. Publikacje etapowane z identyfikatorem storage muszą również zawierać
...
- Tylko wydawca organizacji `openclaw` oraz wydawcy osobiści bieżących członków organizacji `openclaw`
  mogą publikować do kanału `official`.
- Publikacje w czyimś imieniu nadal walidują kwalifikowalność kanału official względem docelowego konta właściciela.
...
wywołującego jako właściciela. Ten endpoint nie migruje istniejących uchwytów użytkownika/osobistych i nie
...
aliasów slugów umiejętności, pakietów, ostrzeżeń inspektora pakietów oraz wyprowadzonych wierszy skrótów wyszukiwania, tak aby
ścieżki bezpośredniego właściciela były zgodne z nowym autorytetem wydawcy. Aktywna rezerwacja chronionego uchwytu
dla odzyskanego uchwytu jest również przypisywana użytkownikowi zastępczemu, aby późniejsza
...
### Punkty końcowe przekazywania własności
