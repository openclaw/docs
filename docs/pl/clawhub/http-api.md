...
x-i18n:
    generated_at: "2026-07-04T18:22:46Z"
    model: gpt-5.5
...
Katalogi firm trzecich mogą używać publicznych endpointów odczytu do wyświetlania lub wyszukiwania umiejętności ClawHub. Buforuj wyniki, respektuj `429`/`Retry-After`, odsyłaj użytkowników do kanonicznej listy ClawHub (`https://clawhub.ai/<owner>/skills/<slug>`) i unikaj sugerowania, że ClawHub popiera witrynę firmy trzeciej. Nie próbuj odzwierciedlać ukrytych, prywatnych ani zablokowanych przez moderację treści poza publiczną powierzchnią API.
...
- Uwierzytelnione endpointy zapisu nie powinny zwracać samego `Unauthorized`, gdy
  ...
  usunięte/zbanowane/wyłączone konta powinny otrzymywać możliwy do działania tekst, aby
...
- Jeśli żaden zaufany IP klienta nie jest dostępny, żądania anonimowe używają koszyków zapasowych
  ograniczonych tylko przez rodzaj limitu szybkości. Te koszyki zapasowe nie obejmują
...
- Wyniki są zwracane w kolejności trafności (podobieństwo embeddingów + dopalacze dokładnego tokenu sluga/nazwy + niewielki priorytet popularności).
- Trafność jest ważniejsza niż popularność. Precyzyjne dopasowanie sluga lub tokenu nazwy wyświetlanej może wyprzedzić luźniejsze dopasowanie z dużo silniejszym zaangażowaniem.
...
- Podejrzany lub ukryty stan moderacji może usunąć umiejętność z publicznego wyszukiwania w zależności od filtrów wywołującego i bieżącego statusu moderacji.
...
- Nie zmieniaj nazwy sluga tylko po to, aby ścigać jedno zapytanie, chyba że nowy slug jest lepszą długoterminową nazwą kanoniczną. Stare slugi stają się aliasami przekierowań, ale kanoniczny URL, wyświetlany slug i przyszłe zestawienia wyszukiwania używają nowego sluga.
...
- `cursor` (opcjonalne): kursor paginacji dla każdego sortowania innego niż `trending`
...
- `createdAt` jest stabilne dla crawli nowych umiejętności; `updated` zmienia się, gdy istniejące umiejętności są publikowane ponownie.
...
- `moderation` jest dołączane tylko wtedy, gdy umiejętność jest oznaczona flagą albo widzi ją właściciel.
...
- Publiczni wywołujący otrzymują `200` tylko dla już oznaczonych flagą widocznych umiejętności.
...
aby ukryć umiejętność w tym samym audytowalnym przepływie pracy.
...
- `moderation` to bieżący snapshot moderacji na poziomie Skills, wyprowadzony z najnowszej wersji.
...
- Zwraca status w kolejce/w trakcie/ukończony powodzeniem/nieudany.
- Zwraca `queue.queuedAhead` i `queue.position`, gdy skan jest w kolejce, aby klienci mogli pokazać, ile priorytetowych ręcznych skanów znajduje się przed żądaniem. Bardzo duże kolejki są ograniczane i zgłaszane z `queuedAheadIsEstimate: true`.
...
- Wymaga dostępu właściciela/wydawcy do zarządzania Skills lub Plugin albo uprawnień moderatora/administratora platformy.
- Zwraca zapisane wyniki skanowania dla dokładnej przesłanej wersji, w tym wersji zablokowanych lub ukrytych.
- `kind` domyślnie ma wartość `skill`; użyj `kind=plugin` dla skanów plugin/pakietów.
...
- `POST /api/v1/packages` nadal służy tylko do wydań typu code-plugin i bundle-plugin.
...
- `category` (opcjonalny): filtr kategorii Plugina. Bieżące wartości:
...
- Treść: archiwum ZIP.
- Każdy wyeksportowany element Skills ma katalog główny w `{publisher}/{slug}/`.
...
- Każdy element Skills zawiera `_export_skill_meta.json`.
...
Eksport zbiorczy najnowszych publicznych wydań pluginów do analizy offline.
...
- Starsze aliasy filtrów v1 udokumentowane w `GET /api/v1/plugins` są również
  akceptowane.
- Filtrowanie kategorii jest rzeczywistym filtrem API opartym na wierszach skrótów kategorii pluginów,
...
instalacyjnych. Jest to publiczna powierzchnia konsumpcji OpenClaw służąca do decydowania, czy
...
- Ten punkt końcowy jest celowo węższy niż punkty końcowe moderacji
  właściciela/moderatora. Udostępnia decyzję instalacyjną i publiczne wyjaśnienie, a nie
...
- `packageName` jest normalizowany jako nazwa npm; pakiet może brakować dla planowanych
  migracji.
...
Zgłoś pakiet do przeglądu przez moderatora. Zgłoszenia są na poziomie pakietu, opcjonalnie
...
- Kontrole moderacji i dostępu do prywatnego pakietu nadal obowiązują.
...
- Przekazania Skills opartych na GitHub nie pośredniczą w bajtach ani ich nie mirrorują. Odpowiedź JSON
  ...
  i `archiveUrl`; stan skanu/bieżący stan jest bramką i nie jest dołączany jako metadane
  ładunku sukcesu.
...
- Opcjonalne pole ładunku: `migrateOwner`. Gdy ma wartość `true` z `ownerHandle`, istniejące
  Skills mogą zostać przeniesione do tego właściciela, jeśli aktor jest administratorem/właścicielem u obu
...
- Dozwolone pola formularza to `payload`, powtarzane bloby `files` albo jedno odwołanie
  tarball `clawpack`. `clawpack` może być blobem `.tgz` albo identyfikatorem magazynu zwróconym przez
  przepływ upload-url. Publikacje etapowe oparte na identyfikatorze magazynu muszą również zawierać
...
- Bezpośrednie żądania publikacji multipart są ograniczone do 18 MB. Archiwa tarball ClawPack mogą
  używać przepływu upload-url do limitu 120 MB dla tarball.
...
- Publikacje w czyimś imieniu nadal walidują kwalifikację do kanału oficjalnego względem konta właściciela docelowego.
...
innego wydawcę. Odpowiedź usunięcia zawiera `slugReservedUntil`, gdy ma zastosowanie ten termin wygaśnięcia.
...
aliasów slugów umiejętności, pakietów, ostrzeżeń inspektora pakietów oraz wyprowadzonych wierszy skrótu wyszukiwania, aby
...
dla odzyskiwanej nazwy użytkownika jest również ponownie przypisywana do użytkownika zastępczego, aby późniejsza
...
Zablokuj użytkownika i trwale usuń posiadane umiejętności (tylko moderator/admin).
...
Odblokuj użytkownika i przywróć kwalifikujące się umiejętności (tylko admin).
