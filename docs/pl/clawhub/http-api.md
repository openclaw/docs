...
x-i18n:
    generated_at: "2026-07-03T02:56:52Z"
    model: gpt-5.5
...
Katalogi firm trzecich mogą używać publicznych punktów końcowych odczytu do listowania lub wyszukiwania Skills ClawHub. Pamiętaj o buforowaniu wyników, respektowaniu `429`/`Retry-After`, odsyłaniu użytkowników do kanonicznej listy ClawHub (`https://clawhub.ai/<owner>/skills/<slug>`) i unikaniu sugerowania, że ClawHub popiera witrynę firmy trzeciej. Nie próbuj odzwierciedlać ukrytych, prywatnych ani zablokowanych przez moderację treści poza publiczną powierzchnią API.
...
- Jeśli żaden zaufany IP klienta nie jest dostępny, żądania anonimowe używają kubełków awaryjnych
  ograniczonych tylko przez rodzaj limitu szybkości. Te kubełki awaryjne nie obejmują
...
- Trafność jest silniejsza niż popularność. Dokładne dopasowanie tokenu slugu lub nazwy wyświetlanej może wyprzedzić luźniejsze dopasowanie ze znacznie silniejszym zaangażowaniem.
...
- Nie zmieniaj nazwy slugu tylko po to, aby gonić za jednym zapytaniem, chyba że nowy slug jest lepszą długoterminową nazwą kanoniczną. Stare slugi stają się aliasami przekierowania, ale kanoniczny URL, wyświetlany slug i przyszłe podsumowania wyszukiwania używają nowego slugu.
...
- `moderation` jest dołączane tylko wtedy, gdy Skill jest oflagowany lub właściciel go ogląda.
...
- Wywołujący publiczni otrzymują `200` tylko dla już oflagowanych widocznych Skills.
...
- `version.security` zawiera znormalizowany stan weryfikacji skanu i szczegóły skanera
...
- `tag` (opcjonalny): rozwiąż wersję oznaczoną tagiem (na przykład `latest`).
...
- Skanowania opublikowanych wersji zapisują wyniki z powrotem tylko wtedy, gdy `update: true` i skanowanie zakończy się pomyślnie.
...
- Zadania skanowania są asynchroniczne. Ręczne żądania skanowania mają priorytet przed normalnymi pracami publikowania/wypełniania zaległości, ale ukończenie nadal zależy od dostępności workerów.
...
- Zwraca `queue.queuedAhead` i `queue.position`, gdy żądanie jest w kolejce, aby klienci mogli pokazać, ile priorytetowych skanowań ręcznych znajduje się przed nim. Bardzo duże kolejki są ograniczane i raportowane z `queuedAheadIsEstimate: true`.
...
- `security.signals.dependencyRegistry` jest zachowane dla zgodności odpowiedzi v1, ale skaner istnienia rejestru zależności został wycofany i ten klucz zawsze ma wartość `null`.
- Brak Skill Card nie wpływa na `ok`, `decision` ani `reasons` tego endpointu; klienci powinni lokalnie odczytać zainstalowany `skill-card.md`, gdy potrzebują zawartości karty.
...
Przeglądanie katalogu tylko dla Pluginów, obejmujące pakiety `code-plugin` i `bundle-plugin`.
...
- `_errors.json` jest dołączany, gdy nie można było wyeksportować pojedynczych Pluginów
  lub plików.
...
- Filtrowanie kategorii to rzeczywisty filtr API oparty na wierszach skrótu
  kategorii Pluginu, a nie przepisanie zapytania wyszukiwania.
- Wyniki są zwracane w kolejności trafności i obecnie nie mają paginacji.
...
- `trust.moderationState` może być nullem. Ma wartość `null`, gdy nie istnieje żadna ręczna
  moderacja wydania.
...
- Ten punkt końcowy jest dokładny co do wersji. Klienci powinni wywoływać go po rozwiązaniu
...
Punkt końcowy moderatora do listowania oficjalnych wierszy migracji Pluginów OpenClaw.
...
Punkt końcowy administratora do tworzenia lub aktualizowania oficjalnego wiersza migracji Pluginu.
...
- `blocked`: poddane kwarantannie, unieważnione lub złośliwe wydania.
...
Zgłasza pakiet do przeglądu przez moderatora. Zgłoszenia dotyczą poziomu pakietu i opcjonalnie
...
Wydania objęte kwarantanną i wycofane zwracają `403` z tras pobierania artefaktów.
...
- Archiwa pluginów/pakietów są plikami zip z katalogiem głównym `package/`, aby starsi klienci OpenClaw
  nadal działały.
...
- Packumenty pakietów zakresowanych obsługują zarówno `/api/npm/@scope/name`, jak i
...
- Przekazania skill opartych na GitHub nie pośredniczą w przesyłaniu bajtów ani ich nie mirrorują. Odpowiedź JSON
...
- Dozwolone pola formularza to `payload`, powtarzane bloby `files` albo jedna referencja tarballa `clawpack`.
  ...
  `clawpackUploadTicket` zwrócony wraz z tym URL-em przesyłania.
- Użyj albo `files`, albo `clawpack`, nigdy obu w tym samym żądaniu.
- Treści JSON oraz dostarczone przez wywołującego metadane `payload.files` / `payload.artifact`
  są odrzucane.
...
- Pakiety pluginów wymagają `openclaw.plugin.json`. Przesłane pliki ClawPack `.tgz` muszą
...
- Publikacje w czyimś imieniu nadal weryfikują kwalifikowalność kanału official względem konta właściciela docelowego.
...
provider; zmienne uchwyty są używane wyłącznie jako zabezpieczenie widoczne dla operatora.
...
Odzyskiwanie migruje również starsze pola `ownerUserId` dla umiejętności odzyskanego wydawcy,
...
chronionego uchwytu dla odzyskanego uchwytu jest również przypisywana użytkownikowi zastępczemu, aby późniejsza
...
które etapują archiwum tarball ClawPack, muszą wysłać wynikowy identyfikator magazynu jako
