...
x-i18n:
    generated_at: "2026-07-04T06:51:55Z"
    model: gpt-5.5
...
Katalogi firm trzecich mogą używać publicznych punktów końcowych odczytu do wyświetlania listy lub wyszukiwania Skills ClawHub. Buforuj wyniki, honoruj `429`/`Retry-After`, odsyłaj użytkowników do kanonicznej pozycji ClawHub (`https://clawhub.ai/<owner>/skills/<slug>`) i unikaj sugerowania, że ClawHub wspiera witrynę firmy trzeciej. Nie próbuj odzwierciedlać ukrytych, prywatnych ani zablokowanych przez moderację treści poza publiczną powierzchnią API.
...
- Uwierzytelnione punkty końcowe zapisu nie powinny zwracać samego `Unauthorized`, gdy
  ...
  usunięte/zablokowane/wyłączone konta powinny otrzymać tekst możliwy do wykorzystania, aby klienci
...
- `X-RateLimit-Remaining` / `RateLimit-Remaining`: dokładny pozostały budżet, gdy występuje.
  Udane żądania z shardowaniem pomijają ten nagłówek zamiast zwracać przybliżoną wartość globalną.
...
- Jeśli istnieje `Retry-After`, zaczekaj tyle sekund przed ponowną próbą.
- Używaj wycofywania z jitterem, aby unikać zsynchronizowanych ponowień.
...
- Jeśli żadne zaufane IP klienta nie jest dostępne, żądania anonimowe używają koszyków zapasowych
  ograniczonych tylko do rodzaju limitu szybkości. Te koszyki zapasowe nie obejmują
...
- Wyniki są zwracane w kolejności trafności (podobieństwo embeddingu + wzmocnienia dokładnego tokenu sluga/nazwy + mały priorytet popularności).
...
- Tekst ASCII jest tokenizowany na granicach słów i interpunkcji. Na przykład `personal-map` zawiera samodzielny token `map`, a `amap-jsapi-skill` zawiera `amap`, `jsapi` i `skill`; dlatego wyszukiwanie `map` daje `personal-map` silniejsze dopasowanie leksykalne niż `amap-jsapi-skill`.
- Popularność jest skalowana logarytmicznie i ograniczona. Skills o wysokim zaangażowaniu mogą mieć niższą pozycję, gdy tekst zapytania jest słabszym dopasowaniem.
...
- Nie zmieniaj nazwy sluga tylko po to, aby celować w jedno zapytanie, chyba że nowy slug jest lepszą długoterminową nazwą kanoniczną. Stare slugi stają się aliasami przekierowania, ale kanoniczny URL, wyświetlany slug i przyszłe skróty wyszukiwania używają nowego sluga.
...
- Jeśli Skill jest niespodziewanie niewidoczny, najpierw sprawdź stan moderacji przez `clawhub inspect @owner/slug` po zalogowaniu, zanim zmienisz metadane związane z rankingiem.
...
- Gdy `nonSuspiciousOnly=true`, sortowania oparte na kursorach mogą zwrócić na stronie mniej elementów niż `limit`, ponieważ podejrzane Skills są filtrowane po pobraniu strony.
- Użyj `nextCursor`, aby kontynuować paginację, gdy występuje. Krótka strona sama w sobie nie oznacza końca wyników.
...
- `moderation` jest dołączane tylko wtedy, gdy Skill jest oznaczony lub wyświetla go właściciel.
...
Zgłoś Skill do przeglądu moderatora. Zgłoszenia są na poziomie Skill, opcjonalnie połączone
...
- Opublikowane skany zapisują wyniki zwrotnie tylko wtedy, gdy `update: true` i skan zakończy się pomyślnie.
...
- Zwraca `queue.queuedAhead` i `queue.position`, gdy żądanie jest w kolejce, aby klienci mogli pokazać, ile priorytetowych ręcznych skanów znajduje się przed żądaniem. Bardzo duże kolejki są ograniczane i raportowane z `queuedAheadIsEstimate: true`.
...
- Wymaga pomyślnie zakończonego skanu; skany nieterminalne zwracają `409`.
...
- Wymaga dostępu zarządzania właściciela/wydawcy do Skills lub Plugin albo uprawnień moderatora/administratora platformy.
...
Kanoniczna trasa statusu wsadu tylko dla administratorów. Akceptuje `{ "jobIds": ["..."] }` i zwraca te same zagregowane liczniki co starsze `POST /api/v1/skills/-/rescan-batch/status`.
...
- `ok` ma wartość `true` tylko wtedy, gdy wybrana wersja ma wygenerowaną Skill Card, nie jest zablokowana jako malware przez moderację, a weryfikacja ClawScan jest czysta.
...
- Wyniki są per element; brak jednego Skills lub jednej wersji nie powoduje niepowodzenia całej odpowiedzi.
...
- `security.signals.dependencyRegistry` jest zachowane dla zgodności odpowiedzi v1, ale skaner istnienia rejestru zależności został wycofany i ten klucz zawsze ma wartość `null`.
- Brak Skill Card nie wpływa na `ok`, `decision` ani `reasons` tego endpointu; klienci powinni czytać zainstalowany lokalnie `skill-card.md`, gdy potrzebują treści karty.
...
- `category` (opcjonalny): filtr kategorii Pluginu. Obsługiwany tylko wtedy, gdy
  ...
  `/api/v1/code-plugins`, `/api/v1/bundle-plugins` albo endpointów pakietów z
...
- Wpisy Skills nadal są oparte na rejestrze Skills i wciąż można je publikować tylko przez `POST /api/v1/skills`.
...
- `category` (opcjonalny): filtr kategorii Pluginu. Obecne wartości:
  `channels`, `models`, `memory`, `context`, `voice`, `media`, `web`,
  `tools`, `runtime`, `gateway`, `security`, `other`.

Starsze aliasy filtrów v1 nadal są akceptowane w endpointach odczytu:
...
- Treść: archiwum ZIP.
- Każda wyeksportowana pozycja Skills ma katalog główny w `{publisher}/{slug}/`.
...
- Bieżące Skills oparte na GitHub z wynikiem skanowania `clean` albo `suspicious` zawierają
  ...
  hashem zawartości i adresem URL archiwum. Nie zawierają plików źródłowych hostowanych w ClawHub.
- Każda pozycja Skills zawiera `_export_skill_meta.json`.
...
- `startDate` (wymagany): dolna granica w milisekundach uniksowych dla `updatedAt` wtyczki.
- `endDate` (wymagany): górna granica w milisekundach uniksowych dla `updatedAt` wtyczki.
- `limit` (opcjonalny): liczba całkowita (1-250), domyślnie `250`.
- `cursor` (opcjonalny): kursor paginacji z poprzedniej odpowiedzi.
...
- Treść: archiwum ZIP.
- Każda wyeksportowana wtyczka jest zakorzeniona w `{family}/{packageName}/`.
- Każda wyeksportowana wtyczka zawiera zapisane pliki najnowszego wydania.
- Metadane eksportu poszczególnych wtyczek są przechowywane w
  `__clawhub_export/{family}/{packageName}/plugin_meta.json`.
- `_manifest.json` jest zawsze dołączony w katalogu głównym ZIP.
- `_errors.json` jest dołączany, gdy nie udało się wyeksportować pojedynczych wtyczek lub plików.
...
- `category` (opcjonalny): filtr kategorii wtyczki. Bieżące wartości:
  `channels`, `models`, `memory`, `context`, `voice`, `media`, `web`,
  `tools`, `runtime`, `gateway`, `security`, `other`.
...
- Starsze aliasy filtrów v1 udokumentowane pod `GET /api/v1/plugins` są również
  akceptowane.
- Filtrowanie kategorii jest rzeczywistym filtrem API opartym na wierszach streszczenia kategorii wtyczek,
...
- `version.artifact.kind` ma wartość `legacy-zip` dla archiwów pakietów ze starego świata lub
  `npm-pack` dla wydań opartych na ClawPack.
- Wydania ClawPack zawierają pola zgodne z npm: `npmIntegrity`, `npmShasum` i
  `npmTarballName`.
- `version.sha256hash` to przestarzałe metadane zgodności dla starych klientów. Skrótuje
...
- `trust.moderationState` dopuszcza wartość null. Ma wartość `null`, gdy nie istnieje ręczna
  moderacja wydania.
...
- `trust.stale` oznacza, że podsumowanie zaufania zostało obliczone z przestarzałych danych wejściowych i
  powinno być traktowane jako wymagające odświeżenia przed decyzją o zezwoleniu z wysokim poziomem pewności.

Uwagi:

- Ten punkt końcowy jest dokładny względem wersji. Klienty powinny go wywołać po rozwiązaniu
...
- Wersje ClawPack strumieniują dokładne przesłane bajty npm-pack `.tgz`.
...
- `open`: podejrzane, złośliwe, oczekujące, objęte kwarantanną, wycofane lub zgłoszone wydania.
- `blocked`: wydania objęte kwarantanną, wycofane lub złośliwe.
...
połączone z wersją. Zasilają kolejkę moderacji, ale same nie ukrywają automatycznie ani nie
blokują pobrań; moderatorzy powinni używać moderacji wydań, aby
zatwierdzać, obejmować kwarantanną lub wycofywać artefakty.
...
- Używa koszyka limitu odczytu, a nie koszyka limitu pobierania.
...
- Prywatne pakiety zwracają `404`, chyba że wywołujący może czytać właściciela wydawcy.
...
- Archiwa Plugin/pakietu są plikami zip z katalogiem głównym `package/`, aby starsi klienci OpenClaw
  nadal działały.
...
- `dist.tarball`, `dist.integrity` i `dist.shasum` używają pól zgodnych z npm,
  aby użytkownicy mogli skierować npm na mirror, jeśli tak wybiorą.
...
- Przekazania skill opartej na GitHub nie pośredniczą ani nie mirrorują bajtów. Odpowiedź JSON
...
- Dozwolone pola formularza to `payload`, powtarzane bloby `files` albo jedno odwołanie do tarballa `clawpack`. `clawpack` może być blobem `.tgz` albo identyfikatorem storage zwróconym przez
  przepływ upload-url. Publikacje z przygotowanym storage-id muszą również zawierać
...
- Publikacje w imieniu innej osoby nadal walidują kwalifikowalność do kanału official względem konta właściciela docelowego.
...
Punkt końcowy domyślnie działa w trybie próbnym. Zastosowanie odzyskiwania wymaga `dryRun: false` i
...
aliasów slugów skills, pakietów, ostrzeżeń inspektora pakietów i pochodnych wierszy digestu wyszukiwania, aby
ścieżki bezpośredniego właściciela zgadzały się z nowym autorytetem wydawcy. Aktywna rezerwacja chronionej nazwy
dla odzyskiwanej nazwy jest też przypisywana do użytkownika zastępczego, aby późniejsza
...
Źródła skills z GitHub są ograniczone zakresem wydawcy i raportowane jako sprawdzone, a nie przepisywane.
...
które przygotowują archiwum tar ClawPack, muszą wysłać wynikowy identyfikator storage jako
