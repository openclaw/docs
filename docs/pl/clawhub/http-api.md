...
read_when:
    ...
    - Debugowanie żądań między CLI a rejestrem
...
Katalogi stron trzecich mogą używać publicznych punktów końcowych odczytu do listowania lub wyszukiwania Skills ClawHub. Buforuj wyniki, respektuj `429`/`Retry-After`, odsyłaj użytkowników do kanonicznej listy ClawHub (`https://clawhub.ai/<owner>/skills/<slug>`) i unikaj sugerowania, że ClawHub popiera stronę trzecią. Nie próbuj odzwierciedlać ukrytych, prywatnych ani zablokowanych przez moderację treści poza publiczną powierzchnią API.
...
- Żądania uwierzytelnione (prawidłowy token Bearer): egzekwowane na koszyk użytkownika.
- Jeśli tokenu brakuje albo jest nieprawidłowy, zachowanie wraca do egzekwowania na IP.
...
- Starsza zgodność: `X-RateLimit-Limit`, `X-RateLimit-Reset`
...
- Jeśli nie ma dostępnego zaufanego IP klienta, żądania anonimowe używają koszyków zastępczych
  ograniczonych tylko do rodzaju limitu szybkości. Te koszyki zastępcze nie obejmują
  ścieżek, slugów, nazw pakietów, wersji, ciągów zapytania ani innych
  parametrów artefaktu dostarczonych przez wywołującego.
...
powinni odczytywać ciało odpowiedzi jako tekst czytelny dla człowieka. Nieznane parametry zapytania są
...
- Trafność jest silniejsza niż popularność. Precyzyjne dopasowanie tokenu sluga lub nazwy wyświetlanej może wyprzedzić luźniejsze dopasowanie o znacznie większym zaangażowaniu.
...
- Popularność jest skalowana logarytmicznie i ograniczana. Skills o wysokim zaangażowaniu mogą mieć niższą pozycję, gdy tekst zapytania jest słabiej dopasowany.
...
- Nie zmieniaj nazwy sluga tylko po to, by ścigać jedno zapytanie, chyba że nowy slug jest lepszą długoterminową nazwą kanoniczną. Stare slugi stają się aliasami przekierowań, ale kanoniczny URL, wyświetlany slug i przyszłe skróty wyszukiwania używają nowego sluga.
...
- `moderation` jest dołączane tylko wtedy, gdy Skill jest oflagowany albo przegląda go właściciel.
...
- Ładunki żądań skanowania i raporty do pobrania wygasają w magazynie żądań skanowania po oknie retencji.
...
- Skany opublikowanych wersji zapisują wyniki z powrotem tylko wtedy, gdy `update: true` i skan zakończy się powodzeniem.
...
Uwierzytelniony endpoint zapisanego archiwum raportu dla przesłanych wersji.
...
- `kind` domyślnie przyjmuje `skill`; użyj `kind=plugin` dla skanów plugin/pakietu.
...
Kanonicalna trasa statusu wsadu tylko dla administratorów. Akceptuje `{ "jobIds": ["..."] }` i zwraca takie same zbiorcze liczniki jak starsze `POST /api/v1/skills/-/rescan-batch/status`.
...
- `provenance` ma wartość `server-resolved-github-import` tylko wtedy, gdy ClawHub rozwiązał i zapisał repozytorium GitHub/ref/commit/path podczas publikacji lub importu; w przeciwnym razie ma wartość `unavailable`.

### `POST /api/v1/skills/-/security-verdicts`

Zwraca bieżące zwięzłe werdykty bezpieczeństwa dla dokładnych wersji skill. Ten
...
- Wyniki są zwracane dla każdego elementu; brak jednego skill lub wersji nie powoduje niepowodzenia całej odpowiedzi.
...
- `security.signals.dependencyRegistry` jest zachowane dla zgodności odpowiedzi v1, ale skaner istnienia rejestru zależności został wycofany i ten klucz zawsze ma wartość `null`.
- Brak Skill Card nie wpływa na `ok`, `decision` ani `reasons` tego endpointu; klienci powinni odczytywać zainstalowany lokalnie `skill-card.md`, gdy potrzebują treści karty.
- Użyj `/verify`, gdy potrzebujesz koperty weryfikacji Skill Card dla pojedynczego skill, `/card`, gdy potrzebujesz wygenerowanego Markdown karty, oraz `/scan`, gdy potrzebujesz szczegółowych danych skanerów.
...
- Bieżące Skills oparte na GitHubie ze skanem `clean` lub `suspicious` zawierają
  ...
  hashem treści i adresem URL archiwum. Nie zawierają plików źródłowych hostowanych przez ClawHub.
- Każdy Skill zawiera `_export_skill_meta.json`.
- `_manifest.json` jest zawsze dołączany w katalogu głównym ZIP.
- `_errors.json` jest dołączany, gdy nie udało się wyeksportować pojedynczych Skills lub plików.
...
- `_manifest.json` jest zawsze dołączany w katalogu głównym ZIP.
- `_errors.json` jest dołączany, gdy nie udało się wyeksportować pojedynczych pluginów lub plików.
...
- Starsze aliasy filtrów v1 udokumentowane w `GET /api/v1/plugins` są również
  akceptowane.
- Filtrowanie kategorii to rzeczywisty filtr API oparty na wierszach skrótu kategorii pluginów,
...
- Wydania ClawPack zawierają pola zgodne z npm: `npmIntegrity`, `npmShasum` i
  `npmTarballName`.
- `version.sha256hash` to przestarzałe metadane zgodności dla starych klientów. Skrót
  obejmuje dokładne bajty ZIP zwracane przez `/api/v1/packages/{name}/download`.
...
- Starsze wersje pakietów zwracają artefakt `legacy-zip` oraz starszy
  `downloadUrl` ZIP.
...
- Wymaga tokena API właściciela pakietu, członka wydawcy, moderatora albo
  administratora.
...
- `revoked`: zablokowane po tym, jak wydanie było wcześniej uznane za zaufane.
...
- Używa limitu odczytów, nie limitu pobrań.
...
- Packumenty pakietów z zakresem obsługują zarówno ścieżkę żądania `/api/npm/@scope/name`, jak i zakodowaną przez npm
...
- Używa limitu pobrań.
- Nagłówki pobierania zawierają ClawHub SHA-256 oraz metadane integrity/shasum npm.
...
- Opcjonalne pole ładunku: `migrateOwner`. Gdy ma wartość `true` razem z `ownerHandle`, istniejąca
...
Publikuje wydanie code-plugin albo bundle-plugin.
...
Miękkie usunięcie / przywrócenie Skills (właściciel, moderator albo administrator).
...
- Zwraca `409`, gdy uchwyt jest już używany przez wydawcę, użytkownika albo osobistego wydawcę.
...
właściciel może później opublikować właściwe wydanie code-plugin albo bundle-plugin pod tą nazwą.
...
Endpoint domyślnie działa w trybie próbnym. Zastosowanie odzyskiwania wymaga `dryRun: false` i
`confirmIdentityVerified: true` po tym, jak personel niezależnie zweryfikuje ciągłość między obiema
tożsamościami GitHub. Odzyskiwanie kończy się bez zmian, gdy bieżący osobisty
...
ścieżki bezpośredniego właściciela zgadzały się z nowym autorytetem wydawcy. Aktywna rezerwacja chronionego identyfikatora
dla odzyskanego identyfikatora jest także przypisywana użytkownikowi zastępczemu, aby późniejsza
...
Źródła Skills GitHub są powiązane z wydawcą i zgłaszane jako sprawdzone, a nie przepisywane.
...
które przygotowują archiwum ClawPack tarball, muszą wysłać wynikowy identyfikator magazynu jako
