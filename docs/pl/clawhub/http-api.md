...
Katalogi zewnętrzne mogą używać publicznych punktów końcowych odczytu do listowania lub wyszukiwania Skills ClawHub. Buforuj wyniki, respektuj `429`/`Retry-After`, odsyłaj użytkowników do kanonicznej listy ClawHub (`https://clawhub.ai/<owner>/<slug>`) i unikaj sugerowania, że ClawHub popiera witrynę zewnętrzną. Nie próbuj odzwierciedlać ukrytych, prywatnych ani zablokowanych przez moderację treści poza publiczną powierzchnią API.
...
- Uwierzytelnione punkty końcowe zapisu nie powinny zwracać samego `Unauthorized`, gdy
  ...
  usunięte/zbanowane/wyłączone konta powinny otrzymać użyteczny tekst, aby klienci CLI
...
- Używaj backoffu z jitterem, aby unikać zsynchronizowanych ponownych prób.
...
- Jeśli zaufany IP klienta nie jest dostępny, anonimowe żądania pobierania używają awaryjnego kubełka ograniczonego do punktu końcowego zamiast jednego globalnego kubełka `ip:unknown`. Anonimowe żądania odczytu/zapisu nadal używają współdzielonego nieznanego kubełka, aby routing bez IP pozostał widoczny i konserwatywny.
...
- Wyniki są zwracane w kolejności trafności (podobieństwo embeddingów + wzmocnienia dla dokładnych tokenów sluga/nazwy + wcześniejsze prawdopodobieństwo popularności z pobrań).
...
- Pobrania są używane jako niewielki, logarytmicznie skalowany czynnik wcześniejszy i rozstrzygający remisy, a nie jako główny sygnał rankingu. Skills z dużą liczbą pobrań mogą zajmować niższą pozycję, gdy tekst zapytania jest słabiej dopasowany.
...
- Umieść terminy, których użytkownicy będą dosłownie szukać, w nazwie wyświetlanej, podsumowaniu i tagach. Używaj samodzielnego tokenu sluga tylko wtedy, gdy jest to też stabilna tożsamość, którą chcesz zachować.
- Nie zmieniaj sluga tylko po to, aby dopasować się do jednego zapytania, chyba że nowy slug jest lepszą długoterminową nazwą kanoniczną. Stare slugi stają się aliasami przekierowań, ale kanoniczny URL, wyświetlany slug i przyszłe skróty wyszukiwania używają nowego sluga.
...
- Jeśli Skill jest niespodziewanie niewidoczny, najpierw sprawdź stan moderacji poleceniem `clawhub inspect <slug>` po zalogowaniu, zanim zmienisz metadane związane z rankingiem.
...
Punkt końcowy moderatora/administratora do rozstrzygania lub ponownego otwierania zgłoszeń Skill.
...
- `moderation` jest bieżącą migawką moderacji na poziomie Skill wyprowadzoną z najnowszej wersji.
...
- Wpisy umiejętności nadal są wspierane przez rejestr umiejętności i wciąż można je publikować tylko przez `POST /api/v1/skills`.
...
Ujednolicone wyszukiwanie w katalogu obejmujące umiejętności i pakiety pluginów.
...
- Umiejętności także mogą być rozwiązywane przez tę trasę w ujednoliconym katalogu.
...
Zwraca jedną wersję pakietu, w tym metadane pliku, kompatybilność,
...
- Wersje ClawPack strumieniują dokładnie przesłane bajty `.tgz` npm-pack.
...
Zgłasza pakiet do przeglądu przez moderatora. Zgłoszenia są na poziomie pakietu i opcjonalnie
połączone z wersją. Trafiają do kolejki moderacji, ale same nie ukrywają automatycznie ani
...
Punkt końcowy moderatora/administratora dla przyjmowania zgłoszeń pakietów.
...
- Istniejące wiersze oparte na ClawPack bez `artifactKind` są naprawiane jako
  `npm-pack`.
- Nie generuje to ClawPacków ani nie modyfikuje bajtów artefaktu.
...
Waliduje token i zwraca identyfikator użytkownika.

### `POST /api/v1/skills`

Publikuje nową wersję.

- Preferowane: `multipart/form-data` z JSON-em `payload` + blobami `files[]`.
...
- Opcjonalne pole ładunku: `migrateOwner`. Gdy ma wartość `true` z `ownerHandle`, istniejący
  Skills może zostać przeniesiony do tego właściciela, jeśli aktor jest administratorem/właścicielem zarówno u obecnego,
  jak i docelowego wydawcy. Bez tej zgody zmiany właściciela są
  odrzucane.
...
Tylko dla administratorów. Zapewnia istnienie wydawcy organizacji dla identyfikatora. Jeśli identyfikator nadal wskazuje na
starszego współdzielonego wydawcę użytkownika/osobistego, endpoint najpierw migruje go do wydawcy organizacji.
...
- Oba endpointy wymagają uwierzytelnienia tokenem API i działają tylko dla właściciela Skills.
