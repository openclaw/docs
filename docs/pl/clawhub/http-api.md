...
x-i18n:
    generated_at: "2026-07-02T22:50:10Z"
    model: gpt-5.5
...
Katalogi firm trzecich mogą używać publicznych punktów końcowych odczytu do listowania lub wyszukiwania ClawHub skills. Buforuj wyniki, respektuj `429`/`Retry-After`, odsyłaj użytkowników do kanonicznego wpisu ClawHub (`https://clawhub.ai/<owner>/skills/<slug>`) i unikaj sugerowania, że ClawHub popiera witrynę firmy trzeciej. Nie próbuj powielać ukrytych, prywatnych ani zablokowanych przez moderację treści poza publiczną powierzchnią API.
...
- Żądania uwierzytelnione (prawidłowy token Bearer): egzekwowane dla kubełka użytkownika.
...
- Uwierzytelnione punkty końcowe zapisu nie powinny zwracać samego `Unauthorized`, gdy
  ...
  usunięte/zbanowane/wyłączone konta powinny otrzymać tekst możliwy do podjęcia działania, aby klienci
...
- `Retry-After`: liczba sekund do odczekania przed ponowną próbą (opóźnienie) przy `429`
...
- Jeśli zaufany adres IP klienta nie jest dostępny, żądania anonimowe używają kubełków zastępczych
  ograniczonych wyłącznie do rodzaju limitu szybkości. Te kubełki zastępcze nie obejmują
...
- Trafność jest silniejsza niż popularność. Dokładne dopasowanie tokenu sluga lub nazwy wyświetlanej może wyprzedzić luźniejsze dopasowanie ze znacznie silniejszym zaangażowaniem.
- Tekst ASCII jest tokenizowany na granicach słów i interpunkcji. Na przykład `personal-map` zawiera samodzielny token `map`, podczas gdy `amap-jsapi-skill` zawiera `amap`, `jsapi` i `skill`; wyszukiwanie `map` daje zatem `personal-map` silniejsze dopasowanie leksykalne niż `amap-jsapi-skill`.
...
- Nie zmieniaj nazwy sluga tylko po to, aby ścigać jedno zapytanie, chyba że nowy slug jest lepszą długoterminową nazwą kanoniczną. Stare slugi stają się aliasami przekierowania, ale kanoniczny URL, wyświetlany slug i przyszłe skróty wyszukiwania używają nowego sluga.
...
- `moderation` jest dołączane tylko wtedy, gdy skill jest oznaczony lub gdy ogląda go właściciel.
...
Zgłasza skill do weryfikacji przez moderatora. Zgłoszenia są na poziomie skill, opcjonalnie powiązane
...
ustawianiu `status` z powrotem na `open`. Przekaż `finalAction: "hide"` z poddanym triage zgłoszeniem,
...
Uwierzytelniony endpoint zgłaszania nowych zadań ClawScan.
...
- Skanowania opublikowanych wersji wymagają dostępu zarządczego właściciela/wydawcy albo uprawnień moderatora/administratora platformy.
- Skanowania opublikowanych wersji zapisują wyniki z powrotem tylko wtedy, gdy `update: true` i skanowanie zakończy się powodzeniem.
...
- Zwraca `queue.queuedAhead` i `queue.position`, gdy zadanie jest w kolejce, aby klienci mogli pokazać, ile priorytetowych ręcznych skanowań jest przed tym żądaniem. Bardzo duże kolejki są ograniczane i raportowane z `queuedAheadIsEstimate: true`.
...
- Wymaga skanowania zakończonego powodzeniem; skanowania nieterminalne zwracają `409`.
...
- `kind` domyślnie przyjmuje `skill`; użyj `kind=plugin` dla skanowań pluginu/pakietu.
...
Kanoniczna trasa statusu wsadu tylko dla administratorów. Akceptuje `{ "jobIds": ["..."] }` i zwraca te same zbiorcze liczniki co starsze `POST /api/v1/skills/-/rescan-batch/status`.
...
- `tag` (opcjonalnie): rozwiązuje wersję oznaczoną tagiem (na przykład `latest`).
...
- Tożsamość skill, tożsamość wydawcy i metadane wybranej wersji są polami koperty najwyższego poziomu (`slug`, `displayName`, `publisherHandle`, `version`, `resolvedFrom`, `tag`, `createdAt`), aby automatyzacja powłoki mogła je odczytać bez rozpakowywania zagnieżdżonych opakowań.
...
- Użyj `/verify`, gdy potrzebujesz koperty weryfikacji Skill Card dla pojedynczego skill, `/card`, gdy potrzebujesz wygenerowanego Markdown karty, oraz `/scan`, gdy potrzebujesz szczegółowych danych skanera.
...
- `category` (opcjonalny): filtr kategorii Plugin. Obsługiwany tylko wtedy, gdy
  ...
  `family=code-plugin`/`family=bundle-plugin`). Kategorie kontrolowane oraz
...
- Wpisy Skills nadal opierają się na rejestrze Skills i nadal mogą być publikowane tylko przez `POST /api/v1/skills`.
...
- `isOfficial` (opcjonalny): `true` lub `false`
- `category` (opcjonalny): filtr kategorii Plugin. Obsługiwany tylko wtedy, gdy
  żądanie jest ograniczone do pakietów Plugin. Kategorie kontrolowane i starsze
  aliasy filtrów v1 są udokumentowane w sekcji `GET /api/v1/plugins`.
...
Przeglądanie katalogu tylko Plugin w pakietach `code-plugin` i `bundle-plugin`.
...
W ujednoliconym endpoincie `/api/v1/packages` dotyczy tylko Plugin; użyj
...
- Bieżące Skills oparte na GitHub ze skanem `clean` lub `suspicious` zawierają
...
- `_errors.json` jest dołączany, gdy nie można było wyeksportować pojedynczych Skills lub plików.
...
- `startDate` (wymagany): dolna granica w milisekundach Unix dla `updatedAt` pluginu.
- `endDate` (wymagany): górna granica w milisekundach Unix dla `updatedAt` pluginu.
- `limit` (opcjonalny): liczba całkowita (1-250), domyślnie `250`.
- `cursor` (opcjonalny): kursor paginacji z poprzedniej odpowiedzi.
...
- `limit` (opcjonalny): liczba całkowita (1-100)
- `isOfficial` (opcjonalny): `true` lub `false`
- `category` (opcjonalny): filtr kategorii pluginu. Aktualne wartości:
  `channels`, `models`, `memory`, `context`, `voice`, `media`, `web`,
  `tools`, `runtime`, `gateway`, `security`, `other`.
...
- Starsze aliasy filtrów v1 udokumentowane w `GET /api/v1/plugins` są również
  akceptowane.
- Filtrowanie kategorii jest rzeczywistym filtrem API opartym na wierszach podsumowania
  kategorii pluginów, a nie przepisaniem zapytania wyszukiwania.
...
- Wydania ClawPack zawierają pola zgodne z npm: `npmIntegrity`, `npmShasum` i
  `npmTarballName`.
- `version.sha256hash` to przestarzałe metadane zgodności dla starych klientów. Obejmuje hashem
...
- Metadane wyłącznie z rejestru nie są wstrzykiwane do pobieranego archiwum.
...
- Packumenty pakietów z zakresem obsługują zarówno `/api/npm/@scope/name`, jak i zakodowaną
...
Pobiera hostowaną wersję ZIP skill albo zwraca przekazanie do źródła GitHub dla
bieżącego skilla opartego na GitHub ze skanem `clean` lub `suspicious` i bez hostowanej
wersji.
...
- Przekazania skilli opartych na GitHub nie pośredniczą ani nie mirrorują bajtów. Odpowiedź JSON
...
- Dozwolone pola formularza to `payload`, powtarzane bloby `files` lub jedno odwołanie do tarballa `clawpack`.
  `clawpack` może być blobem `.tgz` albo identyfikatorem storage id zwróconym przez
  przepływ upload-url. Publikacje ze stagingowym storage-id muszą również zawierać
...
- Treści JSON oraz metadane `payload.files` / `payload.artifact` dostarczone przez wywołującego
  są odrzucane.
...
- Code plugins wymagają `package.json`, metadanych repozytorium źródłowego, metadanych commitu źródłowego,
...
Miękkie usunięcie / przywrócenie skilla (właściciel, moderator lub administrator).
...
Gdy jest obecne, `reason` jest przechowywane jako notatka moderacyjna skilla i kopiowane do dziennika audytu.
Miękkie usunięcia zainicjowane przez właściciela rezerwują slug na 30 dni, po czym slug może zostać zajęty przez
...
- `403`: zabronione
- `404`: nie znaleziono skilla/użytkownika
...
u dostawcy; zmienne uchwyty są używane tylko jako zabezpieczenie widoczne dla operatora.
...
Odzyskiwanie migruje też starsze pola `ownerUserId` dla skills odzyskanego wydawcy,
aliasów slugów skill, pakietów, ostrzeżeń inspektora pakietów oraz pochodnych wierszy skrótu wyszukiwania, aby
ścieżki bezpośredniego właściciela zgadzały się z nowym autorytetem wydawcy. Aktywna rezerwacja chronionego handle
dla odzyskanego handle jest także przypisywana użytkownikowi zastępczemu, aby późniejsza
...
Źródła GitHub skills są zakresowane do wydawcy i raportowane jako sprawdzone, a nie przepisywane.
...
Zmień przechowywany powód istniejącej blokady bez odblokowywania ani przywracania
