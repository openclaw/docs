...
Katalogi zewnętrzne mogą używać publicznych punktów końcowych odczytu do listowania lub wyszukiwania umiejętności ClawHub. Buforuj wyniki, respektuj `429`/`Retry-After`, odsyłaj użytkowników do kanonicznej listy ClawHub (`https://clawhub.ai/<owner>/skills/<slug>`) i unikaj sugerowania, że ClawHub popiera zewnętrzną witrynę. Nie próbuj odzwierciedlać ukrytej, prywatnej ani zablokowanej przez moderację treści poza publiczną powierzchnią API.
...
- Uwierzytelnione punkty końcowe zapisu nie powinny zwracać samego `Unauthorized`, gdy
  ...
  usunięte/zbanowane/wyłączone konta powinny otrzymywać użyteczny tekst, aby klienci
...
- Jeśli nie jest dostępne żadne zaufane IP klienta, żądania anonimowe używają koszyków zastępczych
  ograniczonych tylko rodzajem limitu żądań. Te koszyki zastępcze nie obejmują
...
uprawnień (`401`/`403`), limity żądań (`429`) oraz zablokowane pobierania. Klienci
...
- Wyniki są zwracane w kolejności trafności (podobieństwo osadzeń + wzmocnienia dokładnych tokenów sluga/nazwy + niewielki priorytet popularności).
- Trafność jest silniejsza niż popularność. Precyzyjne dopasowanie tokena sluga lub nazwy wyświetlanej może wyprzedzić luźniejsze dopasowanie o znacznie większym zaangażowaniu.
- Tekst ASCII jest tokenizowany na granicach słów i znaków interpunkcyjnych. Na przykład `personal-map` zawiera samodzielny token `map`, podczas gdy `amap-jsapi-skill` zawiera `amap`, `jsapi` i `skill`; dlatego wyszukanie `map` daje `personal-map` silniejsze dopasowanie leksykalne niż `amap-jsapi-skill`.
- Popularność jest skalowana logarytmicznie i ograniczona. Umiejętności o wysokim zaangażowaniu mogą zajmować niższą pozycję, gdy tekst zapytania jest słabszym dopasowaniem.
...
- Umieść terminy, których użytkownicy będą dosłownie szukać, w nazwie wyświetlanej, podsumowaniu i tagach. Używaj samodzielnego tokena sluga tylko wtedy, gdy jest także stabilną tożsamością, którą chcesz zachować.
- Nie zmieniaj nazwy sluga tylko po to, by gonić za jednym zapytaniem, chyba że nowy slug jest lepszą długoterminową nazwą kanoniczną. Stare slugi stają się aliasami przekierowań, ale kanoniczny URL, wyświetlany slug i przyszłe skróty wyszukiwania używają nowego sluga.
...
- Gdy `nonSuspiciousOnly=true`, sortowania oparte na kursorze mogą zwracać na stronie mniej niż `limit` elementów, ponieważ podejrzane umiejętności są filtrowane po pobraniu strony.
...
- Stare slugi utworzone przez przepływy zmiany nazwy/scalenia właściciela rozwiązują się do kanonicznej umiejętności.
...
- `moderation` jest uwzględniane tylko wtedy, gdy umiejętność jest oflagowana albo ogląda ją właściciel.
...
- Materiał dowodowy jest redagowany dla publicznych wywołujących i obejmuje surowe fragmenty tylko dla właścicieli/moderatorów.
...
ustawianiu `status` z powrotem na `open`. Przekaż `finalAction: "hide"` ze striage'owanym
...
Uwierzytelniony punkt końcowy zgłoszeń dla nowych zadań ClawScan.
...
- Skanowania opublikowanych wersji zapisują wyniki z powrotem tylko wtedy, gdy `update: true`, a skanowanie zakończy się powodzeniem.
...
- Zadania skanowania są asynchroniczne. Ręczne żądania skanowania mają priorytet przed zwykłą pracą publikacji/uzupełniania, ale ukończenie nadal zależy od dostępności workerów.

### `GET /api/v1/skills/-/scan/{scanId}`

Uwierzytelniony punkt końcowy odpytywania dla zgłoszonego skanowania.

- Zwraca status w kolejce/w toku/zakończone powodzeniem/nieudane.
- Zwraca `queue.queuedAhead` i `queue.position`, gdy zadanie jest w kolejce, aby klienci mogli pokazać, ile priorytetowych ręcznych skanowań znajduje się przed żądaniem. Bardzo duże kolejki są ograniczane i raportowane z `queuedAheadIsEstimate: true`.
...
Uwierzytelniony punkt końcowy przechowywanego archiwum raportu dla zgłoszonych wersji.

- Wymaga dostępu właściciela/wydawcy do zarządzania Skills lub wtyczką albo uprawnień moderatora/administratora platformy.
- Zwraca przechowywane wyniki skanowania dla dokładnej zgłoszonej wersji, w tym wersji zablokowanych lub ukrytych.
- `kind` domyślnie ma wartość `skill`; użyj `kind=plugin` dla skanowań wtyczek/pakietów.
- Zwraca taki sam kształt ZIP jak pobrania żądań skanowania.

### `POST /api/v1/skills/-/scan/batch`

Kanononiczna trasa ponownego skanowania wsadowego tylko dla administratorów. Akceptuje ten sam kształt ładunku co starsze `POST /api/v1/skills/-/rescan-batch`.

### `POST /api/v1/skills/-/scan/batch/status`

Kanononiczna trasa statusu wsadu tylko dla administratorów. Akceptuje `{ "jobIds": ["..."] }` i zwraca te same zagregowane liczniki co starsze `POST /api/v1/skills/-/rescan-batch/status`.
...
- `security.signals` zawiera pomocnicze dowody skanera, takie jak `staticScan`, `virusTotal` i `skillSpector`.
...
Zwraca bieżące, zwarte werdykty bezpieczeństwa dla dokładnych wersji Skills. Ten
...
- Wyniki są zwracane dla każdego elementu; jeden brakujący Skills lub jedna brakująca wersja nie powoduje niepowodzenia całej odpowiedzi.
...
- `security.signals` zawiera tylko pomocnicze dowody na poziomie statusu; użyj `/scan` albo strony audytu bezpieczeństwa ClawHub, aby uzyskać pełne szczegóły skanera.
- `security.signals.dependencyRegistry` jest zachowane dla zgodności odpowiedzi v1, ale skaner istnienia rejestru zależności został wycofany i ten klucz zawsze ma wartość `null`.
- Brak Skill Card nie wpływa na `ok`, `decision` ani `reasons` tego punktu końcowego; klienci powinni odczytywać zainstalowany `skill-card.md` lokalnie, gdy potrzebują zawartości karty.
...
- `isOfficial` (opcjonalny): `true` lub `false`
- `category` (opcjonalny): filtr kategorii pluginów. Obsługiwany tylko wtedy, gdy
  żądanie jest ograniczone do pakietów pluginów. Kontrolowane kategorie i starsze aliasy filtrów v1
...
`trending` to siedmiodniowy ranking instalacji/pobrań i nie używa sum od początku istnienia.
...
- Bieżące Skills oparte na GitHub z wynikiem skanowania `clean` lub `suspicious` zawierają
  ...
  skrótem treści i adresem URL archiwum. Nie zawierają plików źródłowych hostowanych w ClawHub.
...
- `startDate` (wymagane): dolna granica w milisekundach uniksowych dla `updatedAt` Pluginu.
- `endDate` (wymagane): górna granica w milisekundach uniksowych dla `updatedAt` Pluginu.
...
- Starsze aliasy filtrów v1 udokumentowane pod `GET /api/v1/plugins` są również
  akceptowane.
- Filtrowanie kategorii jest rzeczywistym filtrem API opartym na wierszach skrótu kategorii Pluginu,
  a nie przekształceniem zapytania wyszukiwania.
- Wyniki są zwracane w kolejności trafności i obecnie nie są stronicowane.
...
- Wymaga tokenu API właściciela pakietu, właściciela/administratora organizacji publikującej,
...
- Wydania ClawPack zawierają zgodne z npm pola `npmIntegrity`, `npmShasum` i
  `npmTarballName`.
...
- `trust.reasons` to lista wyjaśnień dla użytkownika i audytu. Kody przyczyn
  ...
  i `package:malicious`.
- `trust.pending` oznacza, że co najmniej jedno wejście zaufania nadal czeka na ukończenie.
- `trust.stale` oznacza, że podsumowanie zaufania obliczono z nieaktualnych danych wejściowych i
  należy je traktować jako wymagające odświeżenia przed decyzją o zezwoleniu o wysokiej pewności.
...
- `packageName` jest normalizowany jako nazwa npm; pakietu może brakować dla planowanych
  migracji.
- Śledzi to tylko gotowość migracji. Nie mutuje OpenClaw ani nie generuje
  ClawPacków.
...
- Archiwa Plugin/pakietów to pliki zip z katalogiem głównym `package/`, dzięki czemu starzy klienci OpenClaw
  nadal działają.
...
- Pakumenty pakietów z zakresem obsługują zarówno `/api/npm/@scope/name`, jak i
...
- `slug` (wymagane)
- `hash` (wymagane): 64-znakowy szesnastkowy sha256 fingerprintu pakietu
...
- Przekazania Skills opartych na GitHub nie proxy'ują ani nie mirrorują bajtów. Odpowiedź JSON
  ...
  ładunku sukcesu.
- Statystyki pobrań są zliczane jako unikalne tożsamości na dzień UTC (`userId`, gdy token API jest ważny, w przeciwnym razie IP).
...
- Opcjonalne pole ładunku: `migrateOwner`. Gdy ma wartość `true` z `ownerHandle`, istniejąca
  Skills może zostać przeniesiona do tego właściciela, jeśli aktor jest administratorem/właścicielem zarówno u
  bieżącego, jak i docelowego wydawcy. Bez tej opcji zmiany właściciela są
  odrzucane.
...
- Dozwolone pola formularza to `payload`, powtarzane bloby `files` albo jedno odwołanie do tarballa `clawpack`.
  ...
  przepływ upload-url. Publikacje z przygotowanym identyfikatorem storage muszą również zawierać
...
- Bezpośrednie żądania publikacji multipart są ograniczone do 18 MB. Tarballe ClawPack mogą
  używać przepływu upload-url do limitu 120 MB tarballa.
...
bez edytowania wierszy kont Convex Auth. Żądanie musi wskazać oba niezmienne identyfikatory kont dostawcy GitHub;
zmienne uchwyty są używane tylko jako zabezpieczenie dla operatora.
...
Odzyskiwanie migruje też starsze pola `ownerUserId` dla umiejętności odzyskanego wydawcy,
aliasów slugów umiejętności, pakietów, ostrzeżeń inspektora pakietów i pochodnych wierszy skrótu wyszukiwania, aby
ścieżki bezpośredniego właściciela zgadzały się z nowym uprawnieniem wydawcy. Aktywna rezerwacja chronionego uchwytu
dla odzyskanego uchwytu jest też przypisywana użytkownikowi zastępczemu, aby późniejsza
synchronizacja profilu nie mogła przywrócić konkurencyjnego uprawnienia poprzedniego użytkownika. Każda tabela podstawowa jest ograniczona do
...
które etapują tarball ClawPack, muszą wysłać wynikowy identyfikator magazynu jako
`clawpack`, a zwrócony bilet jako `clawpackUploadTicket`.

## Odkrywanie rejestru (`/.well-known/clawhub.json`)

CLI może odkryć ustawienia rejestru/uwierzytelniania z witryny:
...
Jeśli hostujesz samodzielnie, serwuj ten plik (albo jawnie ustaw `CLAWHUB_REGISTRY`; starsze `CLAWDHUB_REGISTRY`).
