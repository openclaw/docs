...
summary: Dokumentacja HTTP API (punkty końcowe publiczne + CLI + uwierzytelnianie).
x-i18n:
    generated_at: "2026-07-01T20:36:44Z"
    model: gpt-5.5
...
Katalogi firm trzecich mogą używać publicznych punktów końcowych odczytu do listowania lub wyszukiwania Skills ClawHub. Buforuj wyniki, respektuj `429`/`Retry-After`, odsyłaj użytkowników do kanonicznego wpisu ClawHub (`https://clawhub.ai/<owner>/skills/<slug>`) i unikaj sugerowania, że ClawHub popiera witrynę firmy trzeciej. Nie próbuj odzwierciedlać ukrytych, prywatnych ani zablokowanych przez moderację treści poza publiczną powierzchnią API.

Skróty slugów WWW są rozpoznawane między rodzinami rejestru, ale klienci API powinni używać
...
- `X-RateLimit-Remaining` / `RateLimit-Remaining`: dokładny pozostały budżet, gdy występuje.
  Udane żądania w systemie shardingowym pomijają ten nagłówek zamiast zwracać przybliżoną wartość globalną.
...
- Jeśli zaufany adres IP klienta nie jest dostępny, żądania anonimowe używają koszyków awaryjnych
  ograniczonych wyłącznie do rodzaju limitu szybkości. Te koszyki awaryjne nie obejmują
...
Obejmuje to niepowodzenia walidacji (`400`), brakujące zasoby publiczne (`404`), błędy uwierzytelniania i
...
- Trafność jest silniejsza niż popularność. Precyzyjne dopasowanie sluga lub tokenu nazwy wyświetlanej może wyprzedzić luźniejsze dopasowanie ze znacznie większym zaangażowaniem.
...
- Nie zmieniaj nazwy sluga tylko po to, by gonić za jednym zapytaniem, chyba że nowy slug jest lepszą długoterminową nazwą kanoniczną. Stare slugi stają się aliasami przekierowań, ale kanoniczny adres URL, wyświetlany slug i przyszłe streszczenia wyszukiwania używają nowego sluga.
- Aliasy zmiany nazwy zachowują rozpoznawanie dla starych adresów URL i instalacji rozwiązywanych przez rejestr, ale ranking wyszukiwania opiera się na kanonicznych metadanych Skill po zaindeksowaniu zmiany nazwy. Istniejące statystyki pozostają przy Skill.
...
- `trending` porządkuje według instalacji z ostatnich 7 dni (na podstawie telemetrii).
- `createdAt` jest stabilne dla crawli nowych Skills; `updated` zmienia się, gdy istniejące Skills są publikowane ponownie.
...
- Stare slugi utworzone przez przepływy zmiany nazwy/scalenia właściciela rozpoznają się do kanonicznego Skill.
...
- `moderation` jest uwzględniane tylko wtedy, gdy Skill jest oflagowany albo ogląda go właściciel.
...
ustawianiu `status` z powrotem na `open`. Przekaż `finalAction: "hide"` ze ztriage'owanym
...
- `tag` (opcjonalny): rozpoznaj otagowaną wersję (na przykład `latest`).
...
- Ładunki żądań skanowania i raporty do pobrania wygasają w magazynie żądań skanowania po upływie okna przechowywania.
...
- Zwraca status w kolejce/uruchomiony/zakończony powodzeniem/nieudany.
...
- Zwraca przechowywane wyniki skanu dla dokładnie przesłanej wersji, w tym wersji zablokowanych lub ukrytych.
- `kind` domyślnie przyjmuje `skill`; użyj `kind=plugin` dla skanów pluginów/pakietów.
- Zwraca taki sam kształt ZIP jak pobieranie żądań skanowania.
...
- `tag` (opcjonalnie): rozwiąż wersję oznaczoną tagiem (na przykład `latest`).
...
- `security.signals` zawiera wyłącznie pomocnicze dowody na poziomie statusu; użyj `/scan` lub strony audytu bezpieczeństwa ClawHub, aby uzyskać pełne szczegóły skanera.
- `security.signals.dependencyRegistry` jest zachowane dla zgodności odpowiedzi v1, ale skaner istnienia rejestru zależności został wycofany i ten klucz zawsze ma wartość `null`.
...
- `isOfficial` (opcjonalny): `true` lub `false`
- `category` (opcjonalny): filtr kategorii Pluginów. Obsługiwany tylko wtedy, gdy
  żądanie jest zawężone do pakietów Pluginów. Kategorie kontrolowane i starsze aliasy
...
`trending` to siedmiodniowy ranking instalacji/pobrań i nie używa łącznych wartości z całego okresu.
...
- Hostowane Skills zawierają najnowsze pliki zapisanej wersji i są wymienione w
...
instalacyjnych. To publiczna powierzchnia użycia OpenClaw służąca do decyzji, czy
...
- `trust.moderationState` może mieć wartość null. Jest `null`, gdy nie istnieje ręczna
  moderacja wydania.
...
- `trust.reasons` to lista wyjaśnień dla użytkownika i audytu. Kody powodów
  ...
  i `package:malicious`.
- `trust.pending` oznacza, że co najmniej jedno źródło zaufania nadal oczekuje na zakończenie.
...
- Wymaga tokena API moderatora albo administratora.
...
- Wymaga tokena API administratora.
...
Zgłasza pakiet do przeglądu moderatora. Zgłoszenia dotyczą poziomu pakietu, opcjonalnie
są powiązane z wersją. Trafiają do kolejki moderacji, ale same nie ukrywają automatycznie ani
...
Pobiera ZIP hostowanej wersji Skills albo zwraca przekazanie do źródła GitHub dla
...
- Przekazania Skills opartych na GitHub nie pośredniczą w bajtach ani ich nie mirrorują. Odpowiedź JSON
  ...
  ładunku sukcesu.
- Statystyki pobierania są liczone jako unikalne tożsamości na dzień UTC (`userId`, gdy token API jest ważny, w przeciwnym razie IP).
...
- Opcjonalne pole ładunku: `migrateOwner`. Gdy ma wartość `true` z `ownerHandle`,
  istniejące Skills może zostać przeniesione do tego właściciela, jeśli aktor jest administratorem/właścicielem u obu
  wydawców: obecnego i docelowego. Bez tego opt-in zmiany właściciela są
  odrzucane.
...
- Dozwolone pola formularza to `payload`, powtarzane bloby `files` albo jedna referencja tarballa `clawpack`.
  ...
  przepływ upload-url. Publikacje z przygotowanego identyfikatora magazynu muszą też zawierać
...
Gdy `reason` jest obecne, jest przechowywane jako notatka moderacyjna Skills i kopiowane do dziennika audytu.
...
Tylko administrator. Odzyskuje osobistego wydawcę dla zweryfikowanego zastępczego głównego konta GitHub OAuth
...
podmiotami GitHub. Odzyskiwanie kończy się zamkniętą odmową, gdy bieżący osobisty
...
dla odzyskiwanego uchwytu jest także ponownie przypisywana do użytkownika zastępczego, aby późniejsza
...
Źródła skills GitHub są zakresowane do wydawcy i raportowane jako sprawdzone, zamiast być przepisywane.
