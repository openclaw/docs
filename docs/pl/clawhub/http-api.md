...
Katalogi stron trzecich mogą używać publicznych endpointów odczytu do listowania lub wyszukiwania umiejętności ClawHub. Buforuj wyniki, respektuj `429`/`Retry-After`, odsyłaj użytkowników do kanonicznej strony ClawHub (`https://clawhub.ai/<owner>/skills/<slug>`) i unikaj sugerowania, że ClawHub popiera witrynę strony trzeciej. Nie próbuj kopiować ukrytych, prywatnych ani zablokowanych moderacyjnie treści poza publiczną powierzchnię API.

Skróty slugów webowych rozwiązują się między rodzinami rejestru, ale klienci API powinni używać
kanonicznych URL-i zwracanych przez endpointy odczytu zamiast odtwarzać priorytet
tras.
...
- Żądania anonimowe: egzekwowane na IP.
- Żądania uwierzytelnione (prawidłowy token Bearer): egzekwowane na kubełek użytkownika.
- Jeśli tokenu brakuje lub jest nieprawidłowy, zachowanie wraca do egzekwowania na IP.
- Uwierzytelnione endpointy zapisu nie powinny zwracać samego `Unauthorized`, gdy
  ...
  usunięte/zbanowane/wyłączone konta powinny otrzymać tekst pozwalający podjąć działanie, aby klienci
...
- `X-RateLimit-Reset`: bezwzględne sekundy epoki Uniksa
...
- Jeśli nie ma dostępnego zaufanego IP klienta, żądania anonimowe używają kubełków awaryjnych
...
- Tekst ASCII jest tokenizowany na granicach słów i interpunkcji. Na przykład `personal-map` zawiera samodzielny token `map`, podczas gdy `amap-jsapi-skill` zawiera `amap`, `jsapi` i `skill`; dlatego wyszukanie `map` daje `personal-map` silniejsze dopasowanie leksykalne niż `amap-jsapi-skill`.
- Popularność jest skalowana logarytmicznie i ograniczana. Umiejętności o wysokim zaangażowaniu mogą zajmować niższą pozycję, gdy tekst zapytania jest słabszym dopasowaniem.
...
- Umieszczaj terminy, których użytkownicy będą dosłownie szukać, w nazwie wyświetlanej, podsumowaniu i tagach. Używaj samodzielnego tokenu sluga tylko wtedy, gdy jest on także stabilną tożsamością, którą chcesz zachować.
- Nie zmieniaj nazwy sluga tylko po to, aby ścigać jedno zapytanie, chyba że nowy slug jest lepszą długoterminową nazwą kanoniczną. Stare slugi stają się aliasami przekierowania, ale kanoniczny URL, wyświetlany slug i przyszłe digesty wyszukiwania używają nowego sluga.
- Aliasy zmiany nazwy zachowują rozwiązywanie dla starych URL-i i instalacji, które rozwiązują się przez rejestr, ale ranking wyszukiwania opiera się na kanonicznych metadanych umiejętności po zaindeksowaniu zmiany nazwy. Istniejące statystyki pozostają z umiejętnością.
- Jeśli umiejętność jest nieoczekiwanie niewidoczna, przed zmianą metadanych związanych z rankingiem najpierw sprawdź stan moderacji za pomocą `clawhub inspect @owner/slug` po zalogowaniu.
...
- `cursor` (opcjonalne): kursor stronicowania dla dowolnego sortowania innego niż `trending`
...
- `createdAt` jest stabilne dla przeszukiwania nowych umiejętności; `updated` zmienia się, gdy istniejące umiejętności są publikowane ponownie.
...
- `moderation` jest uwzględniane tylko wtedy, gdy umiejętność jest oznaczona albo ogląda ją właściciel.
...
ustawiania `status` z powrotem na `open`. Przekaż `finalAction: "hide"` ze sklasyfikowanym
...
- Skany opublikowanych wersji zapisują wyniki z powrotem tylko wtedy, gdy `update: true` i skan zakończy się pomyślnie.
...
- Zwraca `queue.queuedAhead` i `queue.position` podczas oczekiwania w kolejce, aby klienci mogli pokazać, ile priorytetowych skanów ręcznych znajduje się przed żądaniem. Bardzo duże kolejki są ograniczane i raportowane z `queuedAheadIsEstimate: true`.
...
Uwierzytelniony punkt końcowy archiwum przechowywanego raportu dla przesłanych wersji.
...
- Domyślną wartością `kind` jest `skill`; użyj `kind=plugin` dla skanów pluginów/pakietów.
- Zwraca ten sam kształt ZIP co pobrania żądań skanowania.

### `POST /api/v1/skills/-/scan/batch`

Kanoniczna trasa wsadowego ponownego skanowania tylko dla administratorów. Akceptuje ten sam kształt ładunku co starsze `POST /api/v1/skills/-/rescan-batch`.

### `POST /api/v1/skills/-/scan/batch/status`

Kanoniczna trasa statusu wsadu tylko dla administratorów. Akceptuje `{ "jobIds": ["..."] }` i zwraca te same liczniki zagregowane co starsze `POST /api/v1/skills/-/rescan-batch/status`.
...
- `version` (opcjonalnie): określony ciąg wersji.
...
- Wyniki są per element; jedna brakująca umiejętność lub wersja nie powoduje niepowodzenia całej odpowiedzi.
...
- `security.signals` zawiera tylko dowody wspierające na poziomie statusu; użyj `/scan` lub strony audytu bezpieczeństwa ClawHub, aby uzyskać pełne szczegóły skanera.
- `security.signals.dependencyRegistry` jest zachowane dla zgodności odpowiedzi v1, ale skaner istnienia rejestru zależności został wycofany i ten klucz zawsze ma wartość `null`.
- Brak Skill Card nie wpływa na `ok`, `decision` ani `reasons` tego punktu końcowego; klienci powinni czytać lokalnie zainstalowany `skill-card.md`, gdy potrzebują zawartości karty.
- Użyj `/verify`, gdy potrzebujesz koperty weryfikacji Skill Card dla pojedynczej umiejętności, `/card`, gdy potrzebujesz wygenerowanego markdownu karty, oraz `/scan`, gdy potrzebujesz szczegółowych danych skanera.
...
- Wpisy skills nadal bazują na rejestrze skills i wciąż mogą być publikowane tylko przez `POST /api/v1/skills`.
...
- `startDate` (wymagany): dolna granica w milisekundach Unix dla `updatedAt` skill.
- `endDate` (wymagany): górna granica w milisekundach Unix dla `updatedAt` skill.
...
- Skills hostowane obejmują najnowsze zapisane pliki wersji i są wymienione w
...
- Bieżące skills oparte na GitHub z wynikiem skanowania `clean` lub `suspicious` obejmują
  `_source_handoff.json` z `sourceRef: "public-github"`, repozytorium, commit, ścieżką,
  hashem zawartości i adresem URL archiwum. Nie obejmują plików źródłowych hostowanych przez ClawHub.
...
Eksport zbiorczy najnowszych publicznych wydań wtyczek do analizy offline.
...
- `startDate` (wymagane): dolna granica `updatedAt` wtyczki w milisekundach Unix.
- `endDate` (wymagane): górna granica `updatedAt` wtyczki w milisekundach Unix.
...
- Metadane eksportu dla każdej wtyczki są zapisane w
  `__clawhub_export/{family}/{packageName}/plugin_meta.json`.
- `_manifest.json` jest zawsze dołączany w katalogu głównym ZIP.
- `_errors.json` jest dołączany, gdy pojedynczych wtyczek lub plików nie udało się
  wyeksportować.
...
Wyszukiwanie tylko wtyczek w pakietach code-plugin i bundle-plugin.
...
- `category` (opcjonalne): filtr kategorii wtyczek. Obecne wartości:
  `channels`, `models`, `memory`, `context`, `voice`, `media`, `web`,
  `tools`, `runtime`, `gateway`, `security`, `other`.
...
- Starsze aliasy filtrów v1 udokumentowane w sekcji `GET /api/v1/plugins` są również
  akceptowane.
- Filtrowanie kategorii to rzeczywisty filtr API oparty na wierszach skrótu kategorii
  wtyczek, a nie przepisanie zapytania wyszukiwania.
- Wyniki są zwracane według trafności i obecnie nie są paginowane.
- Kontrolki sortowania interfejsu przeglądarki dla wyszukiwania wtyczek zmieniają kolejność wczytanych wyników trafności,
...
- `version.artifact.kind` ma wartość `legacy-zip` dla dawnych archiwów pakietów lub
...
- `release.artifactKind`, `release.artifactSha256`, `release.npmIntegrity`,
  ...
  artefaktu wydania.
- `trust.scanStatus` to efektywny status zaufania wyprowadzony z danych wejściowych skanerów
...
- `trust.reasons` to lista wyjaśnień dla użytkownika i audytu. Kody powodów
  ...
  i `package:malicious`.
- `trust.pending` oznacza, że jedno lub więcej wejść zaufania nadal oczekuje na zakończenie.
...
Zwraca wyliczoną gotowość do przyszłego użycia przez OpenClaw.
...
Punkt końcowy moderatora do wyświetlania wierszy migracji oficjalnych wtyczek OpenClaw.
...
- `open`: podejrzane, złośliwe, oczekujące, poddane kwarantannie, odwołane lub zgłoszone wydania.
- `blocked`: wydania poddane kwarantannie, odwołane lub złośliwe.
...
połączone z wersją. Zasilają kolejkę moderacji, ale same z siebie nie ukrywają automatycznie ani
nie blokują pobrań; moderatorzy powinni używać moderacji wydań do
zatwierdzania, poddawania kwarantannie lub odwoływania artefaktów.
...
Wydania w kwarantannie i cofnięte zwracają `403` z tras pobierania artefaktów.
...
- Pakiety prywatne zwracają `404`, chyba że wywołujący może odczytywać dane właściciela wydawcy.
...
- Pakumenty pakietów z zakresem obsługują zarówno `/api/npm/@scope/name`, jak i zakodowaną
...
Pobiera hostowany ZIP wersji skill albo zwraca przekazanie do źródła GitHub dla
...
- Przekazania skill opartych na GitHub nie pośredniczą w bajtach ani ich nie mirrorują. Odpowiedź JSON
  ...
  oraz `archiveUrl`; stan skanu/bieżący stan jest bramką i nie jest uwzględniany jako metadane
  ładunku powodzenia.
...
- Opcjonalne pole ładunku: `migrateOwner`. Gdy `true` z `ownerHandle`, istniejący
...
- Dozwolone pola formularza to `payload`, powtarzane bloby `files` albo jedno odwołanie do archiwum tarball `clawpack`. `clawpack` może być blobem `.tgz` albo identyfikatorem storage id zwróconym przez
  przepływ upload-url. Publikacje staged storage-id muszą także zawierać
  `clawpackUploadTicket` zwrócony z tym URL przesyłania.
- Użyj albo `files`, albo `clawpack`, nigdy obu w tym samym żądaniu.
- Treści JSON i metadane `payload.files` / `payload.artifact` dostarczone przez wywołującego
  są odrzucane.
...
- Pakiety Plugin wymagają `openclaw.plugin.json`. Przesłania `.tgz` ClawPack muszą
...
- Publikacje w czyimś imieniu nadal sprawdzają kwalifikację do kanału oficjalnego względem konta docelowego właściciela.
...
Gdy obecne, `reason` jest przechowywane jako notatka moderacji skill i kopiowane do dziennika audytu.
...
dostawcy GitHub; zmienne handle są używane tylko jako zabezpieczenie dla operatora.

Punkt końcowy domyślnie działa w trybie dry-run. Zastosowanie odzyskiwania wymaga `dryRun: false` oraz
...
aliasów slugów skills, pakietów, ostrzeżeń inspektora pakietów oraz pochodnych wierszy podsumowania wyszukiwania, tak aby
ścieżki bezpośredniego właściciela zgadzały się z nowym autorytetem wydawcy. Aktywna rezerwacja chronionego handle
dla odzyskanego handle jest również ponownie przypisywana do użytkownika zastępczego, aby późniejsza
