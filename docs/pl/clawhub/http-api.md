...
x-i18n:
    generated_at: "2026-07-02T08:54:24Z"
    model: gpt-5.5
...
Katalogi firm trzecich mogą używać publicznych punktów końcowych odczytu, aby wyświetlać lub wyszukiwać Skills ClawHub. Buforuj wyniki, respektuj `429`/`Retry-After`, kieruj użytkowników z powrotem do kanonicznego wpisu ClawHub (`https://clawhub.ai/<owner>/skills/<slug>`) i unikaj sugerowania, że ClawHub popiera witrynę firmy trzeciej. Nie próbuj odzwierciedlać ukrytych, prywatnych ani zablokowanych przez moderację treści poza publiczną powierzchnią API.
...
- Wyniki są zwracane w kolejności trafności (podobieństwo embeddingów + wzmocnienia dokładnych tokenów sluga/nazwy + niewielki wcześniejszy sygnał popularności).
- Trafność jest silniejsza niż popularność. Dokładne dopasowanie sluga lub tokenu nazwy wyświetlanej może wyprzedzić luźniejsze dopasowanie o znacznie silniejszym zaangażowaniu.
...
- Popularność jest skalowana logarytmicznie i ograniczana. Skills o wysokim zaangażowaniu mogą znaleźć się niżej, gdy tekst zapytania jest słabiej dopasowany.
...
- Nie zmieniaj nazwy sluga tylko po to, aby gonić jedno zapytanie, chyba że nowy slug jest lepszą długoterminową nazwą kanoniczną. Stare slugi stają się aliasami przekierowań, ale kanoniczny URL, wyświetlany slug i przyszłe digesty wyszukiwania używają nowego sluga.
...
- `moderation` jest dołączane tylko wtedy, gdy Skill jest oznaczony flagą albo ogląda go właściciel.
...
- `moderation` to bieżący zrzut moderacji na poziomie skilla, wyprowadzony z najnowszej wersji.
...
- Zadania skanowania są asynchroniczne. Ręczne żądania skanowania mają priorytet przed zwykłą pracą publikacji/uzupełniania, ale ich ukończenie nadal zależy od dostępności workerów.

### `GET /api/v1/skills/-/scan/{scanId}`

Uwierzytelniony endpoint odpytywania przesłanego skanu.

- Zwraca status oczekiwania/uruchomienia/powodzenia/niepowodzenia.
...
- Wymaga zakończonego pomyślnie skanu; skany niekońcowe zwracają `409`.
...
- Wymaga dostępu zarządzania właściciela/wydawcy do skilla lub pluginu albo uprawnień moderatora/administratora platformy.
...
Kanoniczna trasa ponownego skanowania wsadowego dostępna tylko dla administratorów. Akceptuje taki sam kształt ładunku jak starsze `POST /api/v1/skills/-/rescan-batch`.

### `POST /api/v1/skills/-/scan/batch/status`

Kanoniczna trasa statusu wsadu dostępna tylko dla administratorów. Akceptuje `{ "jobIds": ["..."] }` i zwraca te same zagregowane liczniki co starsze `POST /api/v1/skills/-/rescan-batch/status`.
...
- `version` (opcjonalne): konkretny ciąg wersji.
- `tag` (opcjonalne): rozwiązuje oznaczoną tagiem wersję (na przykład `latest`).
...
- Tożsamość skilla, tożsamość wydawcy i metadane wybranej wersji są polami najwyższego poziomu koperty (`slug`, `displayName`, `publisherHandle`, `version`, `resolvedFrom`, `tag`, `createdAt`), aby automatyzacja powłoki mogła je odczytać bez rozpakowywania zagnieżdżonych wrapperów.
...
- `security.signals` zawiera tylko pomocnicze dowody na poziomie statusu; użyj `/scan` albo strony audytu bezpieczeństwa ClawHub, aby uzyskać pełne szczegóły skanerów.
- `security.signals.dependencyRegistry` jest zachowane dla zgodności odpowiedzi v1, ale skaner istnienia rejestru zależności został wycofany i ten klucz zawsze ma wartość `null`.
...
- Użyj `/verify`, gdy potrzebujesz koperty weryfikacji Skill Card dla pojedynczego skilla, `/card`, gdy potrzebujesz wygenerowanego markdownu karty, oraz `/scan`, gdy potrzebujesz szczegółowych danych skanera.
...
- Uwierzytelnieni wywołujący mogą widzieć w wynikach listy/wyszukiwania prywatne pakiety wydawców, do których należą.
...
- `isOfficial` (opcjonalny): `true` albo `false`
- `category` (opcjonalny): filtr kategorii Pluginu. Obsługiwany tylko wtedy, gdy
  żądanie jest zawężone do pakietów Pluginów. Kontrolowane kategorie i starsze aliasy filtrów v1
...
`trending` to ranking instalacji/pobrań z siedmiu dni i nie używa łącznych wartości z całego okresu.
...
- Hostowane Skills obejmują najnowsze zapisane pliki wersji i są wymienione w
  `_manifest.json` z `sourceRef: "public-clawhub"`.
- Bieżące Skills oparte na GitHubie ze skanem `clean` albo `suspicious` obejmują
  `_source_handoff.json` z `sourceRef: "public-github"`, repozytorium, commit, ścieżkę,
  hash treści i URL archiwum. Nie obejmują plików źródłowych hostowanych w ClawHub.
- Każdy Skill obejmuje `_export_skill_meta.json`.
...
Eksport zbiorczy najnowszych publicznych wydań Plugin do analizy offline.
...
- `startDate` (wymagany): dolna granica czasu Unix w milisekundach dla `updatedAt` Plugin.
- `endDate` (wymagany): górna granica czasu Unix w milisekundach dla `updatedAt` Plugin.
- `limit` (opcjonalny): liczba całkowita (1-250), domyślnie `250`.
- `cursor` (opcjonalny): kursor paginacji z poprzedniej odpowiedzi.
...
- Akceptowane są także starsze aliasy filtrów v1 udokumentowane przy `GET /api/v1/plugins`.
...
- Kontrolki sortowania w interfejsie przeglądarki dla wyszukiwania Plugin zmieniają kolejność wczytanych wyników trafności,
...
- Wydania ClawPack zawierają pola zgodne z npm: `npmIntegrity`, `npmShasum` i
  `npmTarballName`.
- `version.sha256hash` to przestarzałe metadane zgodności dla starych klientów. Obejmuje
  haszem dokładne bajty ZIP zwracane przez `/api/v1/packages/{name}/download`.
...
Zwraca dokładne podsumowanie bezpieczeństwa i zaufania wydania pakietu dla klientów instalacyjnych. To publiczna powierzchnia konsumpcji OpenClaw służąca do decydowania, czy
...
- Ten punkt końcowy jest celowo węższy niż punkty końcowe moderacji właściciela/moderatora.
  ...
  tożsamości zgłaszających, treści zgłoszeń, prywatne dowody ani wewnętrzne osie czasu przeglądu.
...
- Wymaga tokenu API użytkownika będącego moderatorem albo administratorem.
...
Wydania w kwarantannie i unieważnione zwracają `403` z tras pobierania artefaktów.
...
- Używa koszyka limitu odczytu, nie koszyka limitu pobierania.
...
- Packumenty pakietów scoped obsługują zarówno `/api/npm/@scope/name`, jak i
...
Pobiera hostowaną wersję Skill w formacie ZIP albo zwraca przekazanie do źródła GitHub dla
...
- Przekazania Skill opartych na GitHub nie pośredniczą ani nie mirrorują bajtów. Odpowiedź JSON
  ...
  i `archiveUrl`; stan skanu/bieżący stan jest bramką i nie jest uwzględniany jako metadane
  ładunku sukcesu.
- Statystyki pobierania są liczone jako unikatowe tożsamości na dzień UTC (`userId`, gdy token API jest prawidłowy, w przeciwnym razie IP).
...
Sprawdza poprawność tokenu i zwraca handle użytkownika.
...
- Dozwolone pola formularza to `payload`, powtarzane bloby `files` albo jedno odwołanie do tarballa `clawpack`.
  ...
  przepływ upload-url. Publikacje etapowane na podstawie storage-id muszą również zawierać
...
- Bezpośrednie żądania publikacji multipart są ograniczone do 18 MB. Tarballe ClawPack mogą
  używać przepływu upload-url do limitu 120 MB dla tarballa.
...
- Publikacje w imieniu innej osoby nadal walidują uprawnienia do kanału official względem konta właściciela docelowego.
...
Gdy jest obecne, `reason` jest przechowywane jako nota moderacyjna Skill i kopiowane do dziennika audytu.
...
Tylko dla administratorów. Zapewnia istnienie wydawcy organizacji dla handle. Jeśli handle nadal wskazuje na
...
dostawcy GitHub; zmienne handle są używane tylko jako zabezpieczenie widoczne dla operatora.
...
Źródła umiejętności GitHub są ograniczone do wydawcy i raportowane jako sprawdzone, a nie przepisywane.
