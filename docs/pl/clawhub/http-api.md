...
x-i18n:
    generated_at: "2026-06-30T22:36:27Z"
    model: gpt-5.5
...
- Uwierzytelnione punkty końcowe zapisu nie powinny zwracać samego `Unauthorized`, gdy
  ...
  usunięte/zbanowane/wyłączone konta powinny dostawać tekst umożliwiający działanie, aby
...
- Używaj backoffu z losowym jitterem, aby uniknąć zsynchronizowanych ponowień.
...
- Jeśli żaden zaufany adres IP klienta nie jest dostępny, żądania anonimowe używają koszyków awaryjnych
...
- Trafność jest silniejsza niż popularność. Dokładne dopasowanie tokenu sluga lub nazwy wyświetlanej może wyprzedzić luźniejsze dopasowanie z dużo silniejszym zaangażowaniem.
...
- Popularność jest skalowana logarytmicznie i ograniczona. Skills z wysokim zaangażowaniem mogą plasować się niżej, gdy tekst zapytania jest słabszym dopasowaniem.
- Podejrzany albo ukryty stan moderacji może usunąć Skill z publicznego wyszukiwania zależnie od filtrów wywołującego i bieżącego statusu moderacji.

Wytyczne dotyczące wykrywalności dla publikujących:
...
- Nie zmieniaj sluga tylko po to, aby gonić za jednym zapytaniem, chyba że nowy slug jest lepszą długoterminową nazwą kanoniczną. Stare slugi stają się aliasami przekierowań, ale kanoniczny URL, wyświetlany slug i przyszłe skróty wyszukiwania używają nowego sluga.
...
- `metadata.os`: ograniczenia OS zadeklarowane we frontmatterze Skill (np. `["macos"]`, `["linux"]`). `null`, jeśli nie zadeklarowano.
...
Skanowania opublikowanych zasobów używają JSON:
...
- Skanowania opublikowanych zasobów wymagają dostępu właściciela/wydawcy do zarządzania albo uprawnień moderatora/administratora platformy.
- Skanowania opublikowanych zasobów zapisują wynik zwrotnie tylko wtedy, gdy `update: true` i skanowanie zakończy się powodzeniem.
- Odpowiedź ma kod `202` z `{ "ok": true, "scanId": "...", "jobId": "...", "status": "queued", "sourceKind": "published", "update": false, "queue": { "queuedAhead": 0, "queuedAheadIsEstimate": false, "position": 1, "running": 0, "runningIsEstimate": false, "note": "Scans are asynchronous and may take time to complete." } }`.
...
- `tag` (opcjonalnie): rozwiąż oznaczoną wersję (na przykład `latest`).
...
wersje ClawHub Skill muszą wyświetlić, takich jak OpenClaw Control UI.
...
- Użyj `/verify`, gdy potrzebujesz koperty weryfikacji Skill Card dla pojedynczego Skill, `/card`, gdy potrzebujesz wygenerowanego Markdown karty, i `/scan`, gdy potrzebujesz szczegółowych danych skanerów.
...
- Wpisy Skills nadal są oparte na rejestrze Skills i nadal mogą być publikowane wyłącznie przez `POST /api/v1/skills`.
...
`trending` to siedmiodniowa tabela rankingowa instalacji/pobrań i nie używa łącznych wartości z całego okresu.
...
- `startDate` (wymagany): dolna granica `updatedAt` pluginu w milisekundach Unix.
- `endDate` (wymagany): górna granica `updatedAt` pluginu w milisekundach Unix.
- `limit` (opcjonalny): liczba całkowita (1-250), domyślnie `250`.
...
- Wymaga tokenu API właściciela pakietu, właściciela lub administratora wydawcy organizacji,
...
- `trust.reasons` to lista wyjaśnień dla użytkownika i audytu. Kody powodów
  ...
  i `package:malicious`.
- `trust.pending` oznacza, że co najmniej jedno wejście zaufania nadal czeka na zakończenie.
- `trust.stale` oznacza, że podsumowanie zaufania zostało obliczone z nieaktualnych danych wejściowych i
  przed decyzją o zezwoleniu o wysokiej pewności powinno zostać potraktowane jako wymagające odświeżenia.
...
- Ten punkt końcowy jest dokładny względem wersji. Klienty powinny wywoływać go po rozwiązaniu
  ...
  metadanych pakietu.
- Prywatne pakiety zwracają `404`, chyba że wywołujący może odczytać właściciela wydawcy.
- Ten punkt końcowy jest celowo węższy niż punkty końcowe moderacji właściciela lub moderatora.
...
- Metadane tylko rejestrowe nie są wstrzykiwane do pobieranego archiwum.
...
- Packumenty pakietów z zakresem obsługują zarówno `/api/npm/@scope/name`, jak i zakodowaną ścieżkę żądania npm
...
- Przekazania Skills opartych na GitHub nie pośredniczą ani nie mirrorują bajtów. Odpowiedź JSON
...
- Pakiety Pluginów wymagają `openclaw.plugin.json`. Przesłania ClawPack `.tgz` muszą
...
- Publikacje w imieniu nadal walidują kwalifikowalność kanału oficjalnego względem konta właściciela docelowego.
...
tożsamościami GitHub. Odzyskiwanie kończy się zamknięciem bez zmian, gdy bieżący osobisty
publisher użytkownika docelowego ma skills, pakiety lub źródła GitHub skill.
Odzyskiwanie migruje także starsze pola `ownerUserId` dla skills odzyskanego publishera,
aliasów slugów skill, pakietów, ostrzeżeń inspektora pakietów oraz pochodnych wierszy digestu wyszukiwania, tak aby
ścieżki bezpośredniego właściciela były zgodne z nowym autorytetem publishera. Aktywna rezerwacja chronionego handle
dla odzyskanego handle jest także przypisywana do użytkownika zastępczego, aby późniejsza
...
Źródła GitHub skill są powiązane z publisherem i zgłaszane jako sprawdzone, a nie przepisywane.
...
które przygotowują tarball ClawPack, muszą wysłać wynikowy identyfikator storage jako
`clawpack` oraz zwrócony ticket jako `clawpackUploadTicket`.

## Wykrywanie registry (`/.well-known/clawhub.json`)

CLI może wykrywać ustawienia registry/uwierzytelniania z witryny:
