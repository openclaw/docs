...
Katalogi firm trzecich mogą używać publicznych punktów końcowych odczytu do wyświetlania lub wyszukiwania Skills ClawHub. Buforuj wyniki, respektuj `429`/`Retry-After`, odsyłaj użytkowników do kanonicznej listy ClawHub (`https://clawhub.ai/<owner>/<slug>`) i unikaj sugerowania, że ClawHub popiera witrynę firmy trzeciej. Nie próbuj odzwierciedlać ukrytych, prywatnych ani zablokowanych przez moderację treści poza publiczną powierzchnią API.

Skróty slugów webowych rozwiązywane są w rodzinach rejestru, ale klienci API powinni używać
...
- Domyślnie używa `cf-connecting-ip` (Cloudflare) jako adresu IP klienta.
- ClawHub używa zaufanych nagłówków przekazywania do identyfikowania adresów IP klientów na brzegu.
- Jeśli nie jest dostępny zaufany adres IP klienta, anonimowe żądania pobierania używają kubełka awaryjnego o zakresie punktu końcowego zamiast jednego globalnego kubełka `ip:unknown`. Anonimowe żądania odczytu/zapisu nadal używają współdzielonego kubełka nieznanego, aby trasowanie z brakującym IP pozostało widoczne i konserwatywne.
...
- Wyniki są zwracane według trafności (podobieństwo embeddingów + dokładne wzmocnienia tokenów sluga/nazwy + wcześniejsza popularność z pobrań).
- Trafność jest silniejsza niż popularność. Precyzyjny slug lub dopasowanie tokenu nazwy wyświetlanej może wyprzedzić luźniejsze dopasowanie z dużo większą liczbą pobrań.
...
- Pobrane pliki są używane jako niewielki wcześniejszy sygnał skalowany logarytmicznie i rozstrzygacz remisów, a nie jako główny sygnał rankingu. Skills z dużą liczbą pobrań mogą zajmować niższą pozycję, gdy tekst zapytania jest słabszym dopasowaniem.
...
- Umieść terminy, których użytkownicy będą dosłownie szukać, w nazwie wyświetlanej, podsumowaniu i tagach. Używaj samodzielnego tokenu sluga tylko wtedy, gdy jest on też stabilną tożsamością, którą chcesz zachować.
...
Punkt końcowy właściciela/wydawcy Skill do odwoływania się od moderacji Skill.

Uwierzytelnianie:

- Wymaga tokenu API właściciela Skill lub członka wydawcy.
...
Żąda ponownego skanowania bezpieczeństwa dla najnowszej opublikowanej wersji Skill.

Uwierzytelnianie:

- Wymaga tokenu API właściciela Skill, administratora wydawcy, moderatora
  platformy lub administratora platformy.
- Właściciele i administratorzy wydawcy podlegają limitowi odzyskiwania właściciela
  na wersję. Moderatorzy i administratorzy platformy nie podlegają mu, ale ClawHub nadal zezwala tylko na
...
ustawianiu `status` z powrotem na `open`. Przekaż `finalAction: "hide"` z przetworzonym
zgłoszeniem, aby ukryć Skill w tym samym audytowalnym przepływie pracy.

### `GET /api/v1/skills/-/appeals`

Punkt końcowy moderatora/administratora do przyjmowania odwołań Skill.

Parametry zapytania:

- `status` (opcjonalne): `open` (domyślnie), `accepted`, `rejected` lub `all`
- `limit` (opcjonalne): liczba całkowita (1-200)
- `cursor` (opcjonalne): kursor paginacji

### `POST /api/v1/skills/-/appeals/{appealId}/resolve`

Punkt końcowy moderatora/administratora do akceptowania, odrzucania lub ponownego otwierania odwołania Skill.
...
- `tag` (opcjonalne): rozwiązuje oznaczoną wersję (na przykład `latest`).
...
Ujednolicone wyszukiwanie w katalogu obejmujące umiejętności + pakiety pluginów.
...
Endpoint moderatora do wyświetlania wierszy migracji oficjalnych pluginów OpenClaw.
...
Endpoint właściciela pakietu/wydawcy do odwołania się od moderacji wydania.

Uwierzytelnianie:

- Wymaga tokena API właściciela pakietu lub członka wydawcy.
...
Odwołania są akceptowane tylko dla wydań objętych kwarantanną, unieważnionych,
podejrzanych lub złośliwych. ClawHub utrzymuje jedno otwarte odwołanie na wydanie.
...
- Wymaga tokenu API właściciela pakietu, administratora wydawcy, moderatora
  platformy lub administratora platformy.
- Właściciele i administratorzy wydawcy podlegają limitowi odzyskiwania
...
- `status` (opcjonalnie): `open` (domyślnie), `accepted`, `rejected` lub `all`
...
ustawianiu `status` z powrotem na `open`. Przekaż `finalAction: "approve"` wraz z zaakceptowanym
...
- Domyślnie działa jako przebieg próbny.
...
- `dist.tarball`, `dist.integrity` i `dist.shasum` używają pól zgodnych z npm,
  aby użytkownicy mogli wskazać npm na kopię lustrzaną, jeśli zechcą.
...
Strumieniuje dokładne bajty przesłanego archiwum tarball ClawPack dla klientów kopii lustrzanej npm.
...
Pobiera plik zip wersji umiejętności.
...
- Treść JSON z `files` (opartymi na storageId) jest również akceptowana.
...
- Pluginy kodu wymagają `package.json`, metadanych repozytorium źródłowego, metadanych commita
...
- Publikacje w czyimś imieniu nadal weryfikują uprawnienie do kanału oficjalnego względem docelowego konta właściciela.
