---
read_when:
    - Zgłaszanie praw do organizacji, marki, zakresu pakietów, uchwytu właściciela, sluga umiejętności lub przestrzeni nazw pakietów
...
summary: Jak poprosić o ocenę ClawHub w sporach o własność organizacji, marki, uchwytu właściciela, zakresu pakietu, slug Skills lub przestrzeni nazw.
...
w produkcie ani formularza odwołania konta do roszczeń dotyczących przestrzeni nazw.
...
lub zmieniona w inny sposób ze względu na rzeczywistą własność.

Przykłady obejmują:

- identyfikator organizacji pasujący do Twojej organizacji GitHub, projektu, firmy lub społeczności
...
- usunięty, nieaktywny lub nieosiągalny właściciel, który blokuje uprawnionego właściciela
  przestrzeni nazw
...
- tokenów API, kluczy podpisu ani danych uwierzytelniających
- tokenów wyzwań DNS
