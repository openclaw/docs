...
x-i18n:
    generated_at: "2026-07-02T01:15:45Z"
    model: gpt-5.5
...
(np. kontenery Docker, VPS Hetzner z internetem tylko przez proxy, zapory
firmowe).
...
- Starsza ścieżka awaryjna: jeśli `clawhub/config.json` jeszcze nie istnieje, ale istnieje `clawdhub/config.json`, CLI używa ponownie starszej ścieżki
...
- Bezinterfejsowo: `clawhub login --token clh_...`
- Zdalnie/bezinterfejsowo interaktywnie: `clawhub login --device` wypisuje kod i czeka, aż autoryzujesz go pod adresem `<site>/cli/device`.
...
- Flagi:
  ...
  - `--sort newest|updated|rating|downloads|trending` (domyślnie: newest). Starsze aliasy sortowania instalacji nadal działają ze względu na zgodność.
...
- `--files`: wypisuje pliki wybranej wersji.
...
- Przypięte umiejętności odrzucają również `install --force`, aby lokalne bajty nie mogły zostać przypadkowo zastąpione.
...
- Nowe umiejętności domyślnie otrzymują `1.0.0`; zmienione umiejętności domyślnie otrzymują następną wersję poprawki.
...
Ustaw `dry_run: true`, aby wykonać podgląd bez tokenu. Rzeczywiste publikacje wymagają
...
- Nowe umiejętności publikowane są jako `1.0.0`; zmienione umiejętności domyślnie publikowane są z następną wersją poprawki. Użyj `--bump minor|major` dla partii aktualizacji, które powinny przejść o
  większy krok semver.
...
- Wymaga `clawhub login`.
- Pobiera zapisany raport skanu ZIP dla zgłoszonej wersji umiejętności lub Plugin, w tym wersji zablokowanych albo ukrytych przez kontrole bezpieczeństwa ClawHub.
...
- `--version` jest wymagane, aby autorzy sprawdzili dokładną zgłoszoną wersję, którą ClawHub zablokował.
...
- `root` domyślnie ma wartość `skills` dla repozytoriów katalogów.
...
- Bez `--version` miękko usuwa umiejętność (właściciel, moderator lub administrator).
...
- `--version <version>` trwale usuwa jedną posiadaną wersję inną niż najnowsza przez zamkniętą w razie błędu,
  ...
  bieżącej najnowszej wersji. Pracownicy platformy nie obchodzą własności w tym przepływie tylko dla wersji.
...
- Użyj tego dla wtyczek i innych wpisów z rodziny pakietów; najwyższego poziomu `search` pozostaje powierzchnią wyszukiwania umiejętności.
- Flagi:
...
- Użyj tego do metadanych wtyczek, zgodności, weryfikacji, źródła oraz inspekcji wersji/plików.
...
- Dla artefaktów ClawPack npm-pack weryfikuje także integralność npm `sha512`,
  shasum npm oraz nazwę/wersję w `package.json` tarballa.
- Starsze wersje ZIP są pobierane przez starszą trasę ZIP.
- Flagi:
  - `--version <version>`: pobiera konkretną wersję.
  - `--tag <tag>`: pobiera oznaczoną wersję (domyślnie: `latest`).
...
- Uruchamia dołączony do CLI ClawHub Plugin Inspector względem lokalnego folderu pakietu wtyczki.
...
- Flagi:
  - `--out <dir>`: zapisuje raporty Plugin Inspector w tym katalogu.
  - `--openclaw <path>`: sprawdza względem jawnego lokalnego checkoutu OpenClaw.
  - `--runtime`: włącza przechwytywanie runtime; importuje kod wtyczki.
  - `--allow-execute`: zezwala na przechwytywanie runtime w izolowanym obszarze roboczym.
  - `--no-mock-sdk`: wyłącza zamockowany OpenClaw SDK podczas przechwytywania runtime.
  - `--json`: dane wyjściowe czytelne maszynowo.
...
- `--version <version>` trwale usuwa jedno posiadane wydanie inne niż najnowsze przez zamkniętą w razie błędu,
  trasę specyficzną dla wersji.
  Usuniętych wersji nie można przywrócić ani opublikować ponownie. Opublikuj zamiennik przed usunięciem
  bieżącej najnowszej wersji. Ten przepływ tylko dla wersji wymaga właściciela pakietu albo administratora wydawcy organizacji; pracownicy platformy nie obchodzą własności pakietu.
- Miękkie usunięcie całego pakietu wymaga właściciela pakietu, właściciela/administratora wydawcy organizacji, moderatora platformy albo administratora platformy.
- Flagi:
...
- Wymaga dostępu administratora zarówno do bieżącego właściciela pakietu, jak i wydawcy
...
- Sprawdza, czy pakiet jest gotowy do przyszłej konsumpcji przez OpenClaw.
...
- Zewnętrzne Pluginy kodu muszą jawnie deklarować `openclaw.compat.pluginApi` i
  ...
  Najwyższy poziom `package.json.version` nie jest używany jako fallback dla walidacji publikowania.
...
- Zakresowe nazwy pakietów muszą pasować do wybranego właściciela. Zobacz `docs/publishing.md`.
...
- `package.json.version` to wersja wydania Twojego pakietu, ale nie jest używana jako
  fallback dla walidacji zgodności/buildu OpenClaw.
...
- W przypadku monorepo przekaż `source_path`, aby przepływ publikował
  folder pakietu Pluginu, na przykład `source_path: extensions/codex`.
- Przypnij przepływ wielokrotnego użytku do stabilnego tagu lub pełnego SHA commita. Nie uruchamiaj publikowania wydań z `@main`.
...
- Przepływ przesyła wynik JSON jako artefakt i udostępnia go jako wyjścia przepływu.
...
- Użyj tego jako rollbacku, jeśli przepływ, repozytorium lub przypięcie środowiska trzeba
  wyłączyć albo utworzyć ponownie.
- Przyszłe rzeczywiste publikacje muszą używać zwykłego uwierzytelnionego publikowania, dopóki konfiguracja nie zostanie
  ustawiona ponownie.
- Flagi:
  - `--json`: wyjście czytelne maszynowo.
...
- Raportowanie działa na zasadzie best-effort. Polecenia instalacji nie kończą się niepowodzeniem, jeśli telemetria jest
  niedostępna.
