...
summary: Konfigurowanie, konfiguracja i używanie Plugin LINE Messaging API
title: LINE
x-i18n:
...
5. Ustaw URL webhook dla punktu końcowego Gateway (wymagany HTTPS):
...
- OpenClaw przetwarza zdarzenia webhook ze zweryfikowanych nieprzetworzonych bajtów żądania. Wartości `req.body` przekształcone przez wcześniejsze oprogramowanie pośredniczące są ignorowane, aby zachować integralność podpisu.
...
- Uwaga o runtime: jeśli `channels.line` jest całkowicie nieobecne, runtime wraca do `groupPolicy="allowlist"` dla kontroli grup (nawet jeśli ustawiono `channels.defaults.groupPolicy`).
...
- Formatowanie Markdown jest usuwane; bloki kodu i tabele są w miarę możliwości konwertowane na Flex
  cards.
...
- Przychodzące multimedia są zapisywane w `~/.openclaw/media/inbound/` przed przekazaniem
  agentowi, zgodnie ze wspólnym magazynem multimediów używanym przez inne wbudowane Plugin
  kanałów.

## Dane kanału (rozszerzone wiadomości)
...
Zobacz [agenci ACP](/pl/tools/acp-agents), aby uzyskać szczegóły.

## Wychodzące multimedia
...
Ogólne wysyłanie multimediów wraca do istniejącej trasy tylko dla obrazów, gdy ścieżka specyficzna dla LINE jest niedostępna.

## Rozwiązywanie problemów

- **Weryfikacja webhook nie przechodzi:** upewnij się, że URL webhook używa HTTPS i
  `channelSecret` pasuje do LINE console.
- **Brak zdarzeń przychodzących:** potwierdź, że ścieżka webhook pasuje do `channels.line.webhookPath`
