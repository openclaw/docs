...
summary: iMessage za pośrednictwem serwera BlueBubbles dla macOS (wysyłanie/odbieranie przez REST, pisanie, reakcje, parowanie, zaawansowane działania).
title: BlueBubbles
x-i18n:
...
<Note>
Obecne wydania OpenClaw zawierają BlueBubbles, więc zwykłe pakietowane kompilacje nie wymagają osobnego kroku `openclaw plugins install`.
</Note>

## Przegląd

- Działa na macOS przez aplikację pomocniczą BlueBubbles ([bluebubbles.app](https://bluebubbles.app)).
- Zalecane/testowane: macOS Sequoia (15). macOS Tahoe (26) działa; edycja jest obecnie zepsuta na Tahoe, a aktualizacje ikon grup mogą zgłaszać powodzenie, ale się nie synchronizować.
...
- Załączniki i naklejki są przyjmowane jako przychodzące multimedia (i udostępniane agentowi, gdy to możliwe).
- Odpowiedzi Auto-TTS syntetyzujące dźwięk MP3 lub CAF są dostarczane jako dymki notatek głosowych iMessage zamiast zwykłych załączników plików.
...
- Reakcje są udostępniane jako zdarzenia systemowe, tak jak w Slack/Telegram, więc agenci mogą je „wspomnieć” przed odpowiedzią.
- Funkcje zaawansowane: edycja, cofnięcie wysłania, wątki odpowiedzi, efekty wiadomości, zarządzanie grupami.
...
- Uwierzytelnianie hasłem jest sprawdzane przed odczytem/parsowaniem pełnych treści Webhooka.

</Warning>

## Utrzymywanie Messages.app przy życiu (VM / konfiguracje bez ekranu)

Niektóre konfiguracje VM macOS / zawsze włączone mogą doprowadzić do przejścia Messages.app w stan „bezczynności” (zdarzenia przychodzące zatrzymują się, dopóki aplikacja nie zostanie otwarta/przeniesiona na pierwszy plan). Prostym obejściem jest **pobudzanie Messages co 5 minut** za pomocą AppleScript + LaunchAgent.
...
<Tabs>
  ...
  <Tab title="Groups">
    ...
    - `channels.bluebubbles.groupAllowFrom` kontroluje, kto może wyzwalać odpowiedzi w grupach, gdy ustawione jest `allowlist`.
...
- Wyszukiwania uruchamiają się dopiero po tym, jak dostęp do grupy, autoryzacja poleceń i bramkowanie wzmianek przepuszczą wiadomość.
- Wzbogacani są tylko nienazwani uczestnicy telefoniczni.
- Surowe numery telefonów pozostają wartością zastępczą, gdy nie znaleziono lokalnego dopasowania.
...
- Autoryzowani nadawcy mogą uruchamiać polecenia sterujące nawet bez wspominania w grupach.

### Prompt systemowy dla grupy

Każdy wpis pod `channels.bluebubbles.groups.*` akceptuje opcjonalny ciąg `systemPrompt`. Wartość jest wstrzykiwana do promptu systemowego agenta przy każdej turze obsługującej wiadomość w tej grupie, więc możesz ustawić personę lub reguły zachowania dla grupy bez edytowania promptów agenta:
...
Klucz pasuje do tego, co BlueBubbles zgłasza jako `chatGuid` / `chatIdentifier` / numeryczne `chatId` dla grupy, a wpis z wieloznacznikiem `"*"` zapewnia domyślną wartość dla każdej grupy bez dokładnego dopasowania (ten sam wzorzec używany przez `requireMention` i zasady narzędzi dla grupy). Dokładne dopasowania zawsze wygrywają z wieloznacznikiem. DM-y ignorują to pole; zamiast tego użyj dostosowania promptu na poziomie agenta lub konta.
...
Czaty BlueBubbles można przekształcić w trwałe przestrzenie robocze ACP bez zmiany warstwy transportu.
...
- **Wskaźniki pisania**: OpenClaw wysyła zdarzenia rozpoczęcia pisania; BlueBubbles automatycznie czyści stan pisania przy wysłaniu lub przekroczeniu limitu czasu (ręczne zatrzymanie przez DELETE jest zawodne).
...
<AccordionGroup>
  <Accordion title="Dostępne działania">
    - **react**: Dodaj/usuń reakcje tapback (`messageId`, `emoji`, `remove`). Natywny zestaw tapbacków iMessage to `love`, `like`, `dislike`, `laugh`, `emphasize` i `question`. Gdy agent wybierze emoji spoza tego zestawu (na przykład `👀`), narzędzie reakcji wraca do `love`, aby tapback nadal się wyrenderował zamiast powodować niepowodzenie całego żądania. Skonfigurowane reakcje ack nadal są walidowane ściśle i zwracają błąd przy nieznanych wartościach.
    ...
    - **setGroupIcon**: Ustaw ikonę/zdjęcie czatu grupowego (`chatGuid`, `media`) - niestabilne w macOS 26 Tahoe (API może zwrócić sukces, ale ikona się nie synchronizuje).
...
### Identyfikatory wiadomości (krótkie a pełne)
...
Gdy użytkownik wpisze w iMessage polecenie razem z adresem URL - np. `Dump https://example.com/article` - Apple dzieli wysyłkę na **dwa osobne dostarczenia Webhook**:

1. Wiadomość tekstową (`"Dump"`).
2. Dymek podglądu URL (`"https://..."`) z obrazami podglądu OG jako załącznikami.

Na większości konfiguracji dwa webhooki docierają do OpenClaw w odstępie około 0,8-2,0 s. Bez scalania agent otrzymuje samo polecenie w turze 1, odpowiada (często „wyślij mi URL”) i widzi URL dopiero w turze 2 - w tym momencie kontekst polecenia jest już utracony.

`channels.bluebubbles.coalesceSameSenderDms` włącza dla DM scalanie kolejnych webhooków od tego samego nadawcy w jedną turę agenta. Czaty grupowe nadal są kluczowane per wiadomość, aby zachować strukturę tur wielu użytkowników.
...
<Tabs>
  <Tab title="Kiedy włączyć">
    ...
    - Twoi użytkownicy wklejają URL-e, obrazy lub długą treść razem z poleceniami.
  ...
  <Tab title="Włączanie">
    ...
    Przy włączonej fladze i bez jawnego `messages.inbound.byChannel.bluebubbles` okno debounce rozszerza się do **2500 ms** (wartość domyślna bez scalania to 500 ms). Szersze okno jest wymagane - kadencja dzielonej wysyłki Apple wynosząca 0,8-2,0 s nie mieści się w węższej wartości domyślnej.
  ...
  <Tab title="Kompromisy">
    - **Dodatkowe opóźnienie dla poleceń sterujących DM.** Przy włączonej fladze wiadomości z poleceniami sterującymi DM (takimi jak `Dump`, `Save` itd.) czekają teraz do końca okna debounce przed wysłaniem, na wypadek gdyby nadchodził webhook z payloadem. Polecenia na czatach grupowych zachowują natychmiastowe wysyłanie.
    - **Scalone wyjście jest ograniczone** - scalony tekst ma limit 4000 znaków z jawnym znacznikiem `…[truncated]`; załączniki mają limit 20; wpisy źródłowe mają limit 10 (powyżej tego zachowane są pierwszy i najnowszy). Każdy źródłowy `messageId` nadal trafia do deduplikacji przychodzącej, więc późniejsze odtworzenie dowolnego pojedynczego zdarzenia przez MessagePoller zostanie rozpoznane jako duplikat.
...
| Sam wklejony URL                                                   | 1 webhook                 | Natychmiastowe wysłanie                 | Natychmiastowe wysłanie (tylko jeden wpis w koszyku)                    |
...
### Rozwiązywanie problemów ze scalaniem dzielonych wysyłek

Jeśli flaga jest włączona, a dzielone wysyłki nadal przychodzą jako dwie tury, sprawdź każdą warstwę:
...
<AccordionGroup>
  ...
  <Accordion title="Okno debounce wystarczająco szerokie dla Twojej konfiguracji">
    Spójrz na log serwera BlueBubbles w `~/Library/Logs/bluebubbles-server/main.log`:

    ```
    grep -E "Dispatching event to webhook" main.log | tail -20
    ```

    Zmierz odstęp między wysłaniem tekstu w stylu `"Dump"` a następującym po nim wysłaniem `"https://..."; Attachments:`. Podnieś `messages.inbound.byChannel.bluebubbles`, aby komfortowo pokryć ten odstęp.

  </Accordion>
  <Accordion title="Znaczniki czasu JSONL sesji ≠ przyjście webhooka">
    Znaczniki czasu zdarzeń sesji (`~/.openclaw/agents/<id>/sessions/*.jsonl`) odzwierciedlają moment, w którym Gateway przekazuje wiadomość agentowi, **a nie** moment przyjścia webhooka. Druga wiadomość w kolejce oznaczona `[Queued messages while agent was busy]` oznacza, że pierwsza tura nadal działała, gdy przyszedł drugi webhook - koszyk scalania został już opróżniony. Dostrajaj okno względem logu serwera BB, a nie logu sesji.
  </Accordion>
  <Accordion title="Presja pamięci spowalniająca wysyłanie odpowiedzi">
    Na mniejszych maszynach (8 GB) tury agenta mogą trwać na tyle długo, że koszyk scalania opróżnia się przed zakończeniem odpowiedzi, a URL trafia jako druga tura w kolejce. Sprawdź `memory_pressure` i `ps -o rss -p $(pgrep openclaw-gateway)`; jeśli Gateway ma ponad ~500 MB RSS i kompresor jest aktywny, zamknij inne ciężkie procesy albo przejdź na większy host.
  </Accordion>
  <Accordion title="Wysyłki jako cytowana odpowiedź używają innej ścieżki">
    Jeśli użytkownik stuknął `Dump` jako **odpowiedź** na istniejący dymek URL (iMessage pokazuje plakietkę „1 Reply” na dymku Dump), URL znajduje się w `replyToBody`, a nie w drugim webhooku. Scalanie nie ma zastosowania - to kwestia skill/promptu, a nie debouncera.
  </Accordion>
</AccordionGroup>
...
<AccordionGroup>
  <Accordion title="Połączenie i webhook">
    ...
    - `channels.bluebubbles.webhookPath`: Ścieżka endpointu Webhook (domyślnie: `/bluebubbles-webhook`).
  ...
  <Accordion title="Polityka dostępu">
    ...
    - `channels.bluebubbles.enrichGroupParticipantsFromContacts`: W macOS opcjonalnie wzbogacaj nienazwanych uczestników grupy z lokalnych Kontaktów po przejściu bramek. Domyślnie: `false`.
  ...
  <Accordion title="Dostarczanie i dzielenie na fragmenty">
    ...
    - `channels.bluebubbles.textChunkLimit`: Rozmiar fragmentu wychodzącego tekstu w znakach (domyślnie: 4000).
    - `channels.bluebubbles.sendTimeoutMs`: Limit czasu na żądanie w ms dla wysyłania tekstu wychodzącego przez `/api/v1/message/text` (domyślnie: 30000). Zwiększ w konfiguracjach macOS 26, w których wysyłki iMessage przez Private API mogą zawieszać się na ponad 60 sekund wewnątrz frameworka iMessage; na przykład `45000` lub `60000`. Próby, wyszukiwania czatów, reakcje, edycje i kontrole kondycji obecnie zachowują krótszą domyślną wartość 10 s; rozszerzenie obsługi na reakcje i edycje jest planowane jako kolejny krok. Nadpisanie dla konta: `channels.bluebubbles.accounts.<accountId>.sendTimeoutMs`.
  ...
  <Accordion title="Multimedia i historia">
    ...
    - `channels.bluebubbles.mediaLocalRoots`: Jawna lista dozwolonych bezwzględnych katalogów lokalnych dopuszczonych dla lokalnych ścieżek multimediów wychodzących. Wysyłanie ścieżek lokalnych jest domyślnie blokowane, chyba że ta opcja jest skonfigurowana. Nadpisanie dla konta: `channels.bluebubbles.accounts.<accountId>.mediaLocalRoots`.
    - `channels.bluebubbles.coalesceSameSenderDms`: Scalaj kolejne webhooki DM od tego samego nadawcy w jedną turę agenta, aby rozdzielona wysyłka tekst+URL Apple docierała jako jedna wiadomość (domyślnie: `false`). Zobacz [Scalanie rozdzielonych wysyłek DM](#coalescing-split-send-dms-command--url-in-one-composition), aby poznać scenariusze, strojenie okna i kompromisy. Po włączeniu bez jawnego `messages.inbound.byChannel.bluebubbles` rozszerza domyślne okno debounce dla wiadomości przychodzących z 500 ms do 2500 ms.
    ...
    - `channels.bluebubbles.replyContextApiFallback`: Gdy przychodząca odpowiedź dociera bez `replyToBody`/`replyToSender`, a pamięci podręcznej kontekstu odpowiedzi w pamięci zabraknie trafienia, pobierz oryginalną wiadomość z BlueBubbles HTTP API jako awaryjną próbę best-effort (domyślnie: `false`). Przydatne we wdrożeniach wieloinstancyjnych współdzielących jedno konto BlueBubbles, po restartach procesu lub po eksmisji z długotrwałej pamięci podręcznej TTL/LRU. Pobieranie jest chronione przed SSRF przez tę samą politykę co każde inne żądanie klienta BlueBubbles, nigdy nie zgłasza wyjątku i zapełnia pamięć podręczną, aby kolejne odpowiedzi amortyzowały koszt. Nadpisanie dla konta: `channels.bluebubbles.accounts.<accountId>.replyContextApiFallback`. Ustawienie na poziomie kanału jest propagowane do kont, które pomijają tę flagę.
...
Gdy ten sam identyfikator ma na Macu zarówno czat iMessage, jak i SMS (na przykład numer telefonu zarejestrowany w iMessage, który otrzymywał też awaryjne wiadomości „zielonego dymka”), OpenClaw preferuje czat iMessage i nigdy po cichu nie obniża go do SMS. Aby wymusić czat SMS, użyj jawnego prefiksu celu `sms:` (na przykład `sms:+15555550123`). Identyfikatory bez pasującego czatu iMessage nadal wysyłają przez dowolny czat zgłaszany przez BlueBubbles.
...
- Nie ma obejścia localhost dla uwierzytelniania webhooków BlueBubbles. Jeśli pośredniczysz w ruchu webhooków przez proxy, zachowaj hasło BlueBubbles w żądaniu od początku do końca. `gateway.trustedProxies` nie zastępuje tutaj `channels.bluebubbles.password`. Zobacz [Bezpieczeństwo Gateway](/pl/gateway/security#reverse-proxy-configuration).
- Włącz HTTPS i reguły zapory na serwerze BlueBubbles, jeśli udostępniasz go poza siecią LAN.

## Rozwiązywanie problemów

- Jeśli zdarzenia pisania/odczytu przestają działać, sprawdź logi webhooków BlueBubbles i zweryfikuj, czy ścieżka Gateway pasuje do `channels.bluebubbles.webhookPath`.
...
- Edycja/cofnięcie wysłania wymagają macOS 13+ i zgodnej wersji serwera BlueBubbles. W macOS 26 (Tahoe) edycja jest obecnie zepsuta z powodu zmian w prywatnym API.
...
- `coalesceSameSenderDms` jest włączone, ale rozdzielone wysyłki (np. `Dump` + URL) nadal docierają jako dwie tury: zobacz checklistę [rozwiązywania problemów ze scalaniem rozdzielonych wysyłek](#split-send-coalescing-troubleshooting) - częste przyczyny to zbyt krótkie okno debounce, znaczniki czasu logów sesji błędnie odczytane jako przyjście webhooka lub wysłanie cytatu odpowiedzi (które używa `replyToBody`, a nie drugiego webhooka).
- Informacje o statusie/kondycji: `openclaw status --all` lub `openclaw status --deep`.

Ogólne informacje o przepływie pracy kanałów znajdziesz w [Kanałach](/pl/channels) oraz przewodniku [Plugins](/pl/tools/plugin).
