...
- **Nie**: pracujesz na własnej maszynie i chcesz po prostu najszybszej pętli deweloperskiej. Zamiast tego użyj zwykłego przepływu instalacji.
- **Uwaga dotycząca piaskownicy**: domyślny backend piaskownicy używa Docker, gdy piaskownica jest włączona, ale piaskownica jest domyślnie wyłączona i **nie** wymaga uruchamiania całego Gateway w Docker. Dostępne są także backendy piaskownicy SSH i OpenShell. Zobacz [Piaskownica](/pl/gateway/sandboxing).

## Wymagania wstępne

- Docker Desktop (lub Docker Engine) + Docker Compose v2
- Co najmniej 2 GB RAM na zbudowanie obrazu (`pnpm install` może zostać zakończony przez OOM na hostach z 1 GB, z kodem wyjścia 137)
- Wystarczająco dużo miejsca na dysku na obrazy i logi
- Jeśli uruchamiasz na VPS/hoście publicznym, sprawdź
...
<Steps>
  ...
  <Step title="Dokończ wdrożenie początkowe">
    Skrypt konfiguracji automatycznie uruchamia wdrożenie początkowe. Wykona on:

    - zapytanie o klucze API dostawcy
    - wygenerowanie tokenu Gateway i zapisanie go do `.env`
    - uruchomienie Gateway przez Docker Compose

    Podczas konfiguracji wdrożenie początkowe przed startem oraz zapisy konfiguracji są wykonywane bezpośrednio przez
...
narzędzie używane po starcie. Przed `docker compose up -d openclaw-gateway` uruchom wdrożenie początkowe
...
| `OPENCLAW_EXTENSIONS`                      | Dołącz wybrane pomocniki dołączonych plugin podczas budowania   |
...
| `OPENCLAW_SANDBOX`                         | Włącz bootstrap piaskownicy (`1`, `true`, `yes`, `on`)          |
| `OPENCLAW_SKIP_ONBOARDING`                 | Pomiń interaktywny krok wdrożenia początkowego (`1`, `true`, `yes`, `on`) |
...
| `OPENCLAW_DISABLE_BUNDLED_SOURCE_OVERLAYS` | Wyłącz nakładki bind-mount źródeł dołączonych plugin            |
| `OTEL_EXPORTER_OTLP_ENDPOINT`              | Wspólny endpoint kolektora OTLP/HTTP dla eksportu OpenTelemetry |
| `OTEL_EXPORTER_OTLP_*_ENDPOINT`            | Endpointy OTLP specyficzne dla sygnału: śladów, metryk lub logów |
...
| `OPENCLAW_OTEL_PRELOADED`                  | Pomiń uruchamianie drugiego SDK OpenTelemetry, gdy jeden jest już wstępnie załadowany |

Opiekunowie mogą testować źródło dołączonego plugin względem spakowanego obrazu przez zamontowanie
jednego katalogu źródłowego plugin na jego spakowanej ścieżce źródłowej, na przykład
`OPENCLAW_EXTRA_MOUNTS=/path/to/fork/extensions/synology-chat:/app/extensions/synology-chat:ro`.
Ten zamontowany katalog źródłowy zastępuje odpowiadającą mu skompilowaną paczkę
...
dołącz jego zależności uruchomieniowe:
...
dołączyć lokalne źródło plugin przez
`OPENCLAW_EXTENSIONS=diagnostics-otel`. Aby włączyć eksport, zezwól na
...
a nie maszynę hosta. Użyj `host.docker.internal` dla dostawców AI,
którzy działają na hoście:
...
Dołączona konfiguracja Docker używa tych adresów URL hosta jako domyślnych wartości wdrożenia początkowego
LM Studio i Ollama, a `docker-compose.yml` mapuje `host.docker.internal` na
host gateway Docker dla Linux Docker Engine. Docker Desktop już zapewnia
...
`OPENCLAW_DISABLE_BONJOUR=1`, aby Gateway nie wpadał w pętlę awarii ani nie restartował wielokrotnie
rozgłaszania, gdy most odrzuca ruch multicast.

Użyj opublikowanego URL Gateway, Tailscale albo szerokoobszarowego DNS-SD dla hostów Docker.
Ustaw `OPENCLAW_DISABLE_BONJOUR=0` tylko wtedy, gdy uruchamiasz z siecią hosta, macvlan
albo inną siecią, w której multicast mDNS na pewno działa.

Pułapki i rozwiązywanie problemów znajdziesz w [Wykrywanie Bonjour](/pl/gateway/bonjour).

### Przechowywanie i trwałość

Docker Compose montuje bindem `OPENCLAW_CONFIG_DIR` do `/home/node/.openclaw` oraz
...
`${HOME}/.openclaw/workspace` dla montowania obszaru roboczego), albo do `/tmp/.openclaw`,
...
[Docker VM Runtime - Co gdzie jest zachowywane](/pl/install/docker-vm-runtime#what-persists-where).

**Miejsca szybkiego przyrostu danych na dysku:** obserwuj `media/`, pliki JSONL sesji,
...
Jeśli ClawDock zainstalowano wcześniej ze starej surowej ścieżki `scripts/shell-helpers/clawdock-helpers.sh`, uruchom ponownie powyższe polecenie instalacji, aby lokalny plik pomocniczy śledził nową lokalizację.
...
Pełny przewodnik po skryptach pomocniczych znajdziesz w [ClawDock](/pl/install/clawdock).
...
<AccordionGroup>
  ...
  <Accordion title="Szybsze przebudowy">
    Uporządkuj Dockerfile tak, aby warstwy zależności były buforowane. Dzięki temu
    `pnpm install` nie będzie uruchamiane ponownie, chyba że zmienią się pliki blokady:
  ...
  <Accordion title="Metadane obrazu bazowego">
    ...
    `org.opencontainers.image.source` oraz inne. Skrót bazowego obrazu Node jest
    odświeżany przez PR-y Dependabot dla obrazu bazowego Docker; kompilacje wydań nie uruchamiają
...
[Docker VM Runtime](/pl/install/docker-vm-runtime), aby poznać kroki wdrażania współdzielonej maszyny VM,
w tym wbudowywanie binariów, trwałość danych i aktualizacje.
...
- [Piaskownica i narzędzia wielu agentów](/pl/tools/multi-agent-sandbox-tools) -- nadpisania per agent
...
W przypadku instalacji npm bez checkoutu źródeł zobacz [Piaskownica § Obrazy i konfiguracja](/pl/gateway/sandboxing#images-and-setup), aby znaleźć wbudowane polecenia `docker build`.

## Rozwiązywanie problemów

<AccordionGroup>
  <Accordion title="Brakuje obrazu albo kontener piaskownicy się nie uruchamia">
  ...
  <Accordion title="Niestandardowe narzędzia nie są znajdowane w piaskownicy">
    ...
    ścieżkami do niestandardowych narzędzi, albo dodaj skrypt w `/etc/profile.d/` w swoim Dockerfile.
  </Accordion>
