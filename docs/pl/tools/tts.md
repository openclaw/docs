...
title: Tekst na mowę
x-i18n:
    generated_at: "2026-05-02T10:05:53Z"
    model: gpt-5.5
    provider: openai
...
OpenClaw może konwertować odpowiedzi wychodzące na audio za pomocą **14 dostawców mowy**
...
| **Google Gemini** | `GEMINI_API_KEY` lub `GOOGLE_API_KEY`                                                                            | Gemini API TTS; uwzględnia personę przez `promptTemplate: "audio-profile-v1"`. |
...
| **Microsoft**     | brak                                                                                                             | Publiczny neuronowy TTS Edge przez `node-edge-tts`. Najlepszy wysiłek, bez SLA. |
| **MiniMax**       | `MINIMAX_API_KEY` (lub plan tokenowy: `MINIMAX_OAUTH_TOKEN`, `MINIMAX_CODE_PLAN_KEY`, `MINIMAX_CODING_API_KEY`)  | T2A v2 API. Domyślnie `speech-2.8-hd`.                                  |
| **OpenAI**        | `OPENAI_API_KEY`                                                                                                 | Używany także do automatycznego podsumowania; obsługuje personę `instructions`. |
| **OpenRouter**    | `OPENROUTER_API_KEY` (może używać ponownie `models.providers.openrouter.apiKey`)                                 | Domyślny model `hexgrad/kokoro-82m`.                                     |
...
SLA ani limitu — traktuj ją jako usługę typu best effort. Starszy identyfikator dostawcy `edge` jest
...
Kolejność pierwszeństwa dla automatycznych odpowiedzi, `/tts audio`, `/tts status` i
...
6. dyrektywy w treści `[[tts:...]]`, gdy [nadpisania modelu](#model-driven-directives) są włączone

Nadpisania kanału i konta używają tego samego kształtu co `messages.tts` i
głęboko scalają się z wcześniejszymi warstwami, więc współdzielone dane
uwierzytelniające dostawcy mogą pozostać w `messages.tts`, podczas gdy kanał lub
konto bota zmienia tylko głos, model, personę albo tryb automatyczny:
...
**Persona** to stabilna tożsamość mówiona, którą można stosować deterministycznie
u różnych dostawców. Może preferować jednego dostawcę, definiować neutralny
wobec dostawcy zamysł promptu i przenosić powiązania specyficzne dla dostawcy:
głosy, modele, szablony promptów, ziarna i ustawienia głosu.
...
Wybór dostawcy działa według zasady jawności w pierwszej kolejności:
...
`pacing`, `constraints`) są **neutralne wobec dostawcy**. Każdy dostawca decyduje,
jak ich użyć:
...
<AccordionGroup>
  <Accordion title="Google Gemini">
    ...
    dodawane na początku jako tekst promptu specyficzny dla Google. Tagi audio w treści,
    takie jak `[whispers]` lub `[laughs]` wewnątrz bloku `[[tts:text]]`, są zachowywane
...
| `fail`              | Pomiń tę próbę dostawcy z `reasonCode: "not_configured"` i `personaBinding: "missing"`. Dostawcy fallback są nadal próbowani.                     |
...
głos, model lub szybkość dla pojedynczej odpowiedzi, plus opcjonalny blok
`[[tts:text]]...[[/tts:text]]` dla ekspresyjnych wskazówek, które powinny pojawić
się tylko w audio:
...
tekstu, zanim kanał je zobaczy, nawet gdy są podzielone między sąsiadujące bloki.
...
**Zezwolenie na przełączanie dostawcy przy zachowaniu konfigurowalności innych ustawień:**
...
Pojedyncze polecenie `/tts`. W Discord OpenClaw rejestruje też `/voice`, ponieważ
...
<Note>
Polecenia wymagają autoryzowanego nadawcy (obowiązują reguły listy dozwolonych/owner)
oraz włączenia `commands.text` albo natywnej rejestracji poleceń.
</Note>
...
- `/tts latest` odczytuje najnowszą odpowiedź asystenta z transkryptu bieżącej sesji i wysyła ją raz jako audio. Przechowuje tylko hash tej odpowiedzi we wpisie sesji, aby powstrzymać duplikaty wysyłek głosowych.
...
Polecenia ukośnikowe zapisują lokalne nadpisania do `prefsPath`. Domyślnie jest to
`~/.openclaw/settings/tts.json`; można to nadpisać zmienną środowiskową
...
Dostarczanie głosu TTS jest sterowane możliwościami kanału. Pluginy kanałów
ogłaszają, czy TTS w stylu głosowym powinien prosić dostawców o natywny cel
`voice-note`, czy zachować zwykłą syntezę `audio-file` i tylko oznaczyć zgodne
...
- **Feishu / WhatsApp**: gdy odpowiedź w formie notatki głosowej zostanie wygenerowana jako MP3/WebM/WAV/M4A
  ...
  plik jako załącznik; wysyłka WhatsApp kończy się niepowodzeniem zamiast opublikowania niezgodnego
  ładunku PTT.
- **BlueBubbles**: utrzymuje syntezę dostawcy na zwykłej ścieżce pliku audio; wyjścia MP3
  i CAF są oznaczane do dostarczenia jako notatki głosowe iMessage.
- **Inne kanały**: MP3 (`mp3_44100_128` z ElevenLabs, `mp3` z OpenAI).
  - 44,1 kHz / 128 kbps to domyślny kompromis dla wyrazistości mowy.
- **MiniMax**: MP3 (model `speech-2.8-hd`, częstotliwość próbkowania 32 kHz) dla zwykłych załączników audio. Dla celów notatek głosowych ogłaszanych przez kanał OpenClaw transkoduje MP3 MiniMax do 48 kHz Opus za pomocą `ffmpeg` przed dostarczeniem, gdy kanał ogłasza transkodowanie.
- **Xiaomi MiMo**: domyślnie MP3 albo WAV po skonfigurowaniu. Dla celów notatek głosowych ogłaszanych przez kanał OpenClaw transkoduje wyjście Xiaomi do 48 kHz Opus za pomocą `ffmpeg` przed dostarczeniem, gdy kanał ogłasza transkodowanie.
- **Lokalny CLI**: używa skonfigurowanego `outputFormat`. Cele notatek głosowych są
  ...
  za pomocą `ffmpeg`.
- **Google Gemini**: API Gemini TTS zwraca surowe PCM 24 kHz. OpenClaw opakowuje je jako WAV dla załączników audio, transkoduje do 48 kHz Opus dla celów notatek głosowych i zwraca PCM bezpośrednio dla Talk/telefonii.
- **Gradium**: WAV dla załączników audio, Opus dla celów notatek głosowych oraz `ulaw_8000` przy 8 kHz dla telefonii.
- **Inworld**: MP3 dla zwykłych załączników audio, natywny `OGG_OPUS` dla celów notatek głosowych oraz surowy `PCM` przy 22050 Hz dla Talk/telefonii.
...
- W `mode: "final"` nadal wysyła TTS wyłącznie audio dla strumieniowanych odpowiedzi końcowych
...
Jeśli odpowiedź przekracza `maxLength`, a streszczenie jest wyłączone (lub brakuje klucza API dla
...
Odpowiedź -> TTS włączone?
  ...
  tak -> ma multimedia / MEDIA: / krótka?
          ...
          nie -> długość > limit?
                   ...
                   tak -> streszczenie włączone?
                            nie -> wyślij tekst
                            tak -> streść -> TTS -> dołącz audio
```
...
## Formaty wyjściowe według kanału
  ...
  | Cel                                   | Format                                                                                                                                |
  | ------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------- |
  | Feishu / Matrix / Telegram / WhatsApp | Odpowiedzi w notatkach głosowych preferują **Opus** (`opus_48000_64` z ElevenLabs, `opus` z OpenAI). 48 kHz / 64 kbps równoważy klarowność i rozmiar. |
  | Inne kanały                           | **MP3** (`mp3_44100_128` z ElevenLabs, `mp3` z OpenAI). 44,1 kHz / 128 kbps to wartość domyślna dla mowy.                             |
  | Rozmowa / telefonia                   | Natywne dla dostawcy **PCM** (Inworld 22050 Hz, Google 24 kHz) albo `ulaw_8000` z Gradium dla telefonii.                              |

  Uwagi według dostawców:

  - **Transkodowanie Feishu / WhatsApp:** Gdy odpowiedź w notatce głosowej trafia jako MP3/WebM/WAV/M4A, Plugin kanału transkoduje ją do Ogg/Opus 48 kHz za pomocą `ffmpeg`. WhatsApp wysyła przez Baileys z `ptt: true` i `audio/ogg; codecs=opus`. Jeśli konwersja się nie powiedzie: Feishu wraca do dołączenia oryginalnego pliku; wysyłanie przez WhatsApp kończy się niepowodzeniem zamiast opublikowania niezgodnego ładunku PTT.
  - **MiniMax / Xiaomi MiMo:** Domyślnie MP3 (32 kHz dla MiniMax `speech-2.8-hd`); transkodowane do Opus 48 kHz dla celów notatek głosowych za pomocą `ffmpeg`.
  ...
  - **Google Gemini:** Zwraca surowe PCM 24 kHz. OpenClaw opakowuje je jako WAV dla załączników, transkoduje do Opus 48 kHz dla celów notatek głosowych, zwraca PCM bezpośrednio dla Rozmowy/telefonii.
  - **Inworld:** Załączniki MP3, natywna notatka głosowa `OGG_OPUS`, surowe `PCM` 22050 Hz dla Rozmowy/telefonii.
  ...
  <Accordion title="Top-level messages.tts.*">
    ...
    <ParamField path="maxTextLength" type="number">
      Twardy limit znaków wejścia TTS. `/tts audio` kończy się niepowodzeniem po jego przekroczeniu.
    </ParamField>
  ...
  <Accordion title="Azure Speech">
    <ParamField path="apiKey" type="string">Zmienna środowiskowa: `AZURE_SPEECH_KEY`, `AZURE_SPEECH_API_KEY` albo `SPEECH_KEY`.</ParamField>
    <ParamField path="region" type="string">Region Azure Speech (np. `eastus`). Zmienna środowiskowa: `AZURE_SPEECH_REGION` albo `SPEECH_REGION`.</ParamField>
  ...
  <Accordion title="ElevenLabs">
    ...
    <ParamField path="voiceSettings" type="object">
      `stability`, `similarityBoost`, `style` (każde `0..1`), `useSpeakerBoost` (`true|false`), `speed` (`0.5..2.0`, `1.0` = normalna).
    </ParamField>
    ...
    <ParamField path="seed" type="number">Liczba całkowita `0..4294967295` dla najlepszej możliwej deterministyczności.</ParamField>
    <ParamField path="baseUrl" type="string">Nadpisuje bazowy URL API ElevenLabs.</ParamField>
  </Accordion>

  <Accordion title="Google Gemini">
    <ParamField path="apiKey" type="string">Wraca do `GEMINI_API_KEY` / `GOOGLE_API_KEY`. Jeśli pominięte, TTS może ponownie użyć `models.providers.google.apiKey` przed powrotem do zmiennych środowiskowych.</ParamField>
    ...
    <ParamField path="promptTemplate" type='"audio-profile-v1"'>Ustaw na `audio-profile-v1`, aby opakować pola promptu aktywnej persony w deterministyczną strukturę promptu Gemini TTS.</ParamField>
    <ParamField path="personaPrompt" type="string">Dodatkowy tekst promptu persony specyficzny dla Google, dołączany do notatek reżysera w szablonie.</ParamField>
  ...
  <Accordion title="Lokalny CLI (tts-local-cli)">
    <ParamField path="command" type="string">Lokalny plik wykonywalny lub ciąg polecenia dla TTS przez CLI.</ParamField>
  ...
  <Accordion title="OpenAI">
    ...
    <ParamField path="instructions" type="string">Jawne pole OpenAI `instructions`. Gdy jest ustawione, pola promptu persony **nie** są mapowane automatycznie.</ParamField>
    <ParamField path="extraBody / extra_body" type="Record<string, unknown>">Dodatkowe pola JSON scalane z treściami żądań `/audio/speech` po wygenerowanych polach TTS OpenAI. Użyj tego dla punktów końcowych zgodnych z OpenAI, takich jak Kokoro, które wymagają kluczy specyficznych dla dostawcy, takich jak `lang`; niebezpieczne klucze prototypu są ignorowane.</ParamField>
  ...
  <Accordion title="OpenRouter">
    ...
    <ParamField path="speed" type="number">Natywne nadpisanie szybkości dostawcy.</ParamField>
  </Accordion>

  <Accordion title="Volcengine (BytePlus Seed Speech)">
    <ParamField path="apiKey" type="string">Env: `VOLCENGINE_TTS_API_KEY` lub `BYTEPLUS_SEED_SPEECH_API_KEY`.</ParamField>
    <ParamField path="resourceId" type="string">Domyślnie `seed-tts-1.0`. Env: `VOLCENGINE_TTS_RESOURCE_ID`. Użyj `seed-tts-2.0`, gdy projekt ma uprawnienie TTS 2.0.</ParamField>
    ...
    <ParamField path="speedRatio" type="number">Natywny współczynnik szybkości dostawcy.</ParamField>
    <ParamField path="emotion" type="string">Natywny znacznik emocji dostawcy.</ParamField>
...
dostarczane jako wiadomość głosowa, a nie jako załącznik plikowy. Feishu i
