...
x-i18n:
    generated_at: "2026-05-06T09:32:54Z"
    model: gpt-5.5
    provider: openai
...
<Note>
Narzędzie pojawia się tylko wtedy, gdy dostępny jest co najmniej jeden dostawca generowania obrazów.
Jeśli nie widzisz `image_generate` w narzędziach agenta,
...
<Warning>
Dla zgodnych z OpenAI punktów końcowych w sieci LAN, takich jak LocalAI, zachowaj niestandardowy
`models.providers.openai.baseUrl` i jawnie włącz użycie za pomocą
...
podobną opcję geometrii zamiast dokładnie żądanej, OpenClaw mapuje ją ponownie na
najbliższy obsługiwany rozmiar, proporcje albo rozdzielczość przed wysłaniem.
Nieobsługiwane wskazówki wyjściowe są pomijane dla dostawców, którzy nie deklarują
obsługi, i zgłaszane w wyniku narzędzia. Wyniki narzędzia zgłaszają zastosowane
ustawienia; `details.normalization` przechwytuje wszelkie tłumaczenia
z żądanych wartości na zastosowane.
</Note>
...
kandydat jest próbowany automatycznie. Jeśli wszyscy zawiodą, błąd zawiera szczegóły
z każdej próby.
...
obrazów referencyjnych. Przekaż ścieżkę albo URL obrazu referencyjnego:
...
<AccordionGroup>
  ...
  <Accordion title="Podwójne uwierzytelnianie MiniMax">
    Generowanie obrazów MiniMax jest dostępne przez obie wbudowane ścieżki
...
- [Informacje o konfiguracji](/pl/gateway/config-agents#agent-defaults) - konfiguracja `imageGenerationModel`
