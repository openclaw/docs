...
seven-minute workflow: preflight, setup, queue time and other jobs still apply.
...
| `blacksmith-32vcpu-ubuntu-2404` | Eligible ordinary multi-group compact bins, numbered tooling bins, agent-support, the Docker seed job, `checks-ui-e2e` Control UI rows, `build-artifacts`, `check-lint`, `check-dependencies`, `check-test-types`, the five `check-test-types-core-*` stripes, `check-additional-extension-package-boundary`, `check-additional-runtime-topology-architecture`, and npm release preflight; these CPU-heavy lanes need the measured capacity described below                                                                                                                                                                                                                                                                                                                                                             |
...
Configurable heavy lanes are `build-artifacts`, `android`, and `macos-swift`. The iOS build and screenshot jobs always use `macos-26`. The focused `macos-node` lane uses the existing GitHub-hosted `macos-15` image in hybrid mode, with the same test inventory and two-worker limit. On every non-`github` backend, `openclaw/ci-gate` follows `preflight` routing to the existing 4-vCPU Blacksmith runner, including its contributor-trust checks and hosted dispatch/retry/fork fallbacks; `security-fast` uses Blacksmith only on eligible hybrid first attempts and stays hosted outside `hybrid`. This avoids a second hosted assignment wave on canonical Blacksmith runs. Security hooks use pinned installed packages and local hook definitions, so they no longer initialize remote Git repositories. Budget two control-job registrations per eligible Blacksmith run and three per eligible hybrid first attempt; the `github` override remains unchanged. Hybrid sends the compact Node matrix, up to 80 compact rows plus separately appended plugin fallback rows, thirteen-row `checks-ui-e2e` matrix for targets with the named-project contract, the `checks-ui-e2e-real-gateway` lane that shares its serial Chromium workload, six-row QA Smoke matrix, the two-part Windows matrix, `checks-ui`, `check-lint`, `check-test-types`, the five `check-test-types-core-*` stripes, `check-dependencies`, `check-additional-extension-package-boundary`, `check-additional-runtime-topology-architecture`, and `report-plugin-sdk-api-diff` to Blacksmith on attempt 1. Compact-small rows use `blacksmith-4vcpu-ubuntu-2404`, compact-large rows use `blacksmith-8vcpu-ubuntu-2404`, and the planner's measured small queue-tail promotions retain their 8-vCPU labels. Every other configurable `ci.yml` lane stays hosted in hybrid, including the five core-lint stripes, the remaining lint/check rows, docs, and Python skills. Separate Opengrep workflows remain GitHub-hosted.
...
The compact Node planner keeps separate Blacksmith and standard 4-core hosted timing ownership. The `github` and `hybrid` profiles admit 210 predicted seconds per job, including shared runtime preparation, without changing file partitions, process envelopes, workers or runner requirements. GitHub applies a 1.6x median scaling fallback only to unmeasured groups. Hybrid splits groups using the slower of the first-attempt Blacksmith estimate and the hosted retry estimate against the unchanged 150-second ceiling. This prevents a faster retry estimate from leaving a slow first attempt indivisible. Its attempt-1 packing applies the existing 0.87 scale to Blacksmith estimates, using committed measurements before the cold-start hints. These calibrated predictions remain separate from the recorded wrapper wall times. Refits can change the number and composition of compact jobs without changing runner policy. Direct sampled hints cover the doctor and cron-service outliers. In hybrid, the unmeasured `agentic-gateway-core-3` tail retains its 140-second fallback and can share the remaining seventy predicted seconds.
...
Ordinary non-Windows CI targets seven minutes; Windows must still pass but sits outside this latency objective. The 210-second expanded packing budget is an estimate, not a job timeout or a measured workflow result. Preflight, checkout/setup, queueing and actual test walls all count.
...
Plugin fallback envelopes from different configs can share a job in pairs under their own 150-second estimated budget. Each envelope retains its original child process, environment, native shard arguments and include scope, including process-bounded Codex, Matrix and Telegram work. Partitions of the same config and jobs with runtime prerequisites remain separate. Workers, timeouts and serial stop-on-failure behavior stay unchanged. The measured 2026-09-02 inventory packs 78 envelopes into 42 jobs; the prospective pair sums from run `33676780376` reach 222.31 seconds, below the existing 243.46-second memory singleton. These separate-run sums support a co-location trial, not a measured combined-job speedup. Native CI must verify the combined jobs and their cleanup.

Eligible Blacksmith compact bins with multiple ordinary groups request the existing 32-vCPU runner and two child-process slots. Their logical runner classes, check names, 200/276-second packing budgets and complete group inventories stay fixed. Exclusive groups, single groups, dist descriptors and jobs with runtime preparation remain serial. The shard executor uses the shared CI capacity predicate to admit at most two processes when the actual host has at least eight available CPUs and 24 GiB of memory; smaller or unknown capacity admits one. The workflow keeps each child at two Vitest workers, inner project parallelism remains one, and commands retain their serial file policy. GitHub and hybrid planner profiles keep their existing serial topology. This capacity trial adds no matrix rows; native elapsed-time, memory and cleanup evidence must establish its effect.
