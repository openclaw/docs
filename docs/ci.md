...
Plugin fallback groups from different configs share a job under a 150-second aggregate estimated budget. Every member must have a different config and compatible runner/dist requirements; total cost bounds packing rather than a pair limit. Each envelope retains its original child process, environment, native shard arguments and include scope, including process-bounded Codex, Matrix and Telegram work. Partitions of the same config and jobs with runtime prerequisites remain separate. Workers, timeouts and serial stop-on-failure behavior stay unchanged. On the same `abb0556a` inventory, the planner reduces 78 groups from 42 jobs to 38. The largest sum of matching historical child spans is 300.42 seconds; the memory singleton remains separate. These sums are prospective work estimates, not measured combined-job latency. Native CI must verify the new batches and their cleanup.

Eligible Blacksmith compact bins with multiple ordinary groups request the existing 32-vCPU runner and two child-process slots. They admit 300 predicted aggregate seconds across at most ten groups. Runtime consumers share preparation only with other consumers, keeping ordinary groups on their own capacity. Serial jobs retain their 200/276-second budgets, exclusive jobs retain 150 seconds, and groups above their existing serial cap stay alone. Exclusive groups, single groups, dist descriptors and jobs with runtime preparation remain serial. The shard executor uses the shared CI capacity predicate to admit at most two processes when the actual host has at least eight available CPUs and 24 GiB of memory; smaller or unknown capacity admits one. The workflow keeps each child at two Vitest workers, inner project parallelism remains one, and commands retain their serial file policy. GitHub and hybrid planner profiles keep their existing serial topology. The same `abb0556a` inventory produces 36 main Node jobs instead of 43, and 91 broad-PR Node jobs instead of 102 when combined with plugin packing. Native elapsed-time, memory and cleanup evidence must establish the effect of those new batches.
...
The browser-extension row prepares only its native-host runtime JavaScript and assets through the existing `qaRuntime` build profile rather than rebuilding declarations and the Control UI. Both current widths remain inside the conservative registration bound. A failed-job-only PR retry of a six-shard plan retains those six shards while live routing selects hosted Ubuntu, so it may take longer; its existing 25-minute timeout is unchanged. A retry that reruns preflight selects twelve rows, and canonical push retries retain their existing Blacksmith routing. The `max-parallel` ceiling stays 14 for historical targets without the named-project contract, which retain their previous width. Physical capacity must be checked separately from the registration bound.
