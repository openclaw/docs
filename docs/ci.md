...
Actual workflow replay at `0060198` emits 74 nondist compact rows and 77 plugin fallback rows for broad hybrid PRs, with up to 86 other potential job rows across backends. QA/provider partitioning adds five fallback rows, and the Mac split adds two other rows. Conservatively count all 86 other rows as Blacksmith admissions, including hosted and skipped rows. This gives a broad-PR bound of `96 + 77 + 86 = 259` against the compact cap; canonical main has no PR fallback descriptors, giving a bound of 182. Two active main slots plus both pending successors and the previously observed peak of fourteen non-skipped PR arrivals, including superseded heads, give `4 × 182 + 14 × 259 = 4,354` registrations in five minutes. The live bucket rechecked on 2026-09-01 was 10,000; the 6,000 operating target leaves 1,646 for adjacent repositories and burst overlap. This is an arrival-envelope estimate, not an organization-wide admission cap: re-evaluate emitted rows, adjacent repositories and queued-run overlap before increasing fanout. An unused registration bucket does not prove physical runner capacity.
