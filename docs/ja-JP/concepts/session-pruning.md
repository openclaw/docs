---
read_when:
    - ツール出力によるコンテキストの肥大化を抑えたい場合
    - Anthropicのプロンプトキャッシュ最適化を理解したい場合
summary: 古いツール結果をトリミングして、コンテキストを軽量に保ち、キャッシュ効率を高める
title: セッションのプルーニング
x-i18n:
    generated_at: "2026-04-26T11:28:10Z"
    model: gpt-5.4
    provider: openai
...
<Info>
プルーニングはメモリ内のみで行われ、ディスク上のセッショントランスクリプトは変更しません。
完全な履歴は常に保持されます。
</Info>
...
- [Gateway Configuration](/ja-JP/gateway/configuration) -- すべてのプルーニング設定項目
  (`contextPruning.*`)
