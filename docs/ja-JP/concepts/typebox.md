...
WebSocket プロトコル**（ハンドシェイク、リクエスト/レスポンス、サーバーイベント）の定義に使用しています。これらのスキーマは、macOS アプリ向けの **ランタイム検証**、**JSON Schema エクスポート**、**Swift コード生成**を駆動します。信頼できる情報源は 1 つで、それ以外はすべて生成されます。

高レベルのプロトコル文脈を知りたい場合は、
...
メソッド（例: `health`, `send`, `chat.send`）を呼び出したり、イベント（例:
`presence`, `tick`, `agent`）を購読したりできます。
...
- 公開機能/ディスカバリレジストリ: `src/gateway/server-methods-list.ts`
...
- `pnpm protocol:check`
  - 両方のジェネレータを実行し、出力がコミットされていることを検証します

## ランタイムでのスキーマの使用方法

- **サーバー側**: すべての受信フレームは AJV で検証されます。ハンドシェイクは、
  パラメータが `ConnectParams` に一致する `connect` リクエストのみを受け入れます。
- **クライアント側**: JS クライアントは、イベントフレームとレスポンスフレームを使用する前に検証します。
- **機能ディスカバリ**: Gateway は、`listGatewayMethods()` と
  `GATEWAY_EVENTS` から取得した保守的な `features.methods`
  と `features.events` のリストを `hello-ok` で送信します。
- そのディスカバリリストは、`coreGatewayHandlers` 内のすべての呼び出し可能なヘルパーを
...
そのメソッドがオペレータまたはノードクライアントから呼び出し可能な場合は、
スコープ強制と `hello-ok` の機能公開が一致するように、
...
- 副作用のあるメソッドは通常、params 内の `idempotencyKey` を必要とします
  （例: `send`, `poll`, `agent`, `chat.send`）。
- `agent` は、ランタイム生成のオーケストレーション文脈用に任意の `internalEvents` を受け入れます
  （たとえば subagent/cron タスク完了の引き継ぎ）。これは内部 API サーフェスとして扱ってください。

## ライブスキーマ JSON

生成された JSON Schema はリポジトリ内の `dist/protocol.schema.json` にあります。公開されている raw ファイルは、通常次で利用できます。
...
3. 新しい RPC にオペレータまたはノードのスコープ分類が必要な場合は、
