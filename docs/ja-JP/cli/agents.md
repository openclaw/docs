...
title: エージェント
x-i18n:
    generated_at: "2026-05-02T20:42:40Z"
    model: gpt-5.5
    provider: openai
...
- 対話モードでは、認証シードはポータブルな静的プロファイルのみをコピーします
  （デフォルトでは `api_key` と静的な `token`）。OAuth リフレッシュトークンプロファイルは、実際の `main` エージェントストアからの読み取り継承によってのみ
  利用できます。
  設定済みのデフォルトエージェントが `main` でない場合、新しいエージェントで OAuth
