...
read_when:
    ...
    - Pi 向けのエージェントセッションのライフサイクル、ツール機能、またはプロバイダー接続の変更
summary: OpenClaw の組み込み Pi エージェント統合とセッションライフサイクルのアーキテクチャ
title: Pi 統合アーキテクチャ
x-i18n:
    generated_at: "2026-05-11T20:33:25Z"
    model: gpt-5.5
    provider: openai
...
画像注入はプロンプト単位です。OpenClaw は現在のプロンプトから画像参照を読み込み、
そのターンのみ `images` 経由で渡します。過去の履歴ターンを再スキャンして
