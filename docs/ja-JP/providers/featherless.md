...
Featherless が Qwen 3 ファミリーのネイティブツール呼び出しを文書化しているため、Plugin はセットアップ時のデフォルトとして `Qwen/Qwen3-32B` を使用します。OpenClaw は、32,768 トークンのコンテキストウィンドウ、保守的な 4,096 トークンの出力上限、および Qwen チャットテンプレートの思考制御を設定します。

Featherless は複数の請求モードをサポートしており、OpenClaw はアカウント固有のプランやリクエスト料金を組み込まないため、カタログのコストフィールドはゼロです。
...
OpenClaw は、Featherless の公開モデルインデックス全体を意図的に選択画面へコピーしません。このインデックスは大規模であり、すべてのテキスト、ビジョン、埋め込み、推論モデルを安全に分類するために十分な構造化された機能メタデータを公開していません。そのため、不明な ID は保守的なテキスト専用・非推論のデフォルト設定、すなわち 4,096 トークンのコンテキストウィンドウと 1,024 トークンの出力上限で解決されます。
...
- `401` または `403`：`FEATHERLESS_API_KEY` が Gateway プロセスから参照できることを確認するか、オンボーディングを再実行してください。
- 不明なモデル：`featherless/` プレフィックスの後に、Featherless に記載された大文字と小文字を区別する正確な ID を指定してください。
- ツール呼び出しがテキストとして返される：Qwen 3 など、Featherless がネイティブ関数呼び出しに対応すると文書化しているモデルファミリーを選択してください。
