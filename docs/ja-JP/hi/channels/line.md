...
read_when:
    ...
    - LINE固有のメッセージオプションが必要です
summary: LINE Messaging API Plugin のセットアップ、設定、使用
title: LINE
x-i18n:
...
レシーバーとして動作し、認証には channel access token + channel secret を
使用します。
...
channel を設定する前に LINE をインストールします:
...
inbound events (POST) を受け入れます。agent
processing は非同期で続行されます。
custom path が必要な場合は、`channels.line.webhookPath` または
`channels.line.accounts.<id>.webhookPath` を設定し、URL をそれに合わせて更新します。

セキュリティノート:

- LINE signature verification は body-dependent です (raw body に対する HMAC)。そのため OpenClaw は verification の前に strict pre-auth body limits と timeout を適用します。
...
`tokenFile` と `secretFile` は regular files を指す必要があります。Symlinks は拒否されます。
...
ダイレクトメッセージは default で pairing になります。不明な senders には pairing code が送られ、その
messages は承認されるまで無視されます。
...
Allowlists と policies:

- `channels.line.dmPolicy`: `pairing | allowlist | open | disabled`
- `channels.line.allowFrom`: DMs 用の allowlisted LINE user IDs。`dmPolicy: "open"` には `["*"]` が必要です
- `channels.line.groupPolicy`: `allowlist | open | disabled`
- `channels.line.groupAllowFrom`: groups 用の allowlisted LINE user IDs
- Per-group overrides: `channels.line.groups.<groupId>.allowFrom`
- Static sender access groups は `allowFrom`、`groupAllowFrom`、per-group `allowFrom` から `accessGroup:<name>` で reference できます。
- Runtime note: `channels.line` が完全に missing の場合、runtime は group checks に `groupPolicy="allowlist"` へ fallback します (`channels.defaults.groupPolicy` が設定されていても同様です)。
...
LINE は ACP (Agent Communication Protocol) conversation bindings に対応しています:

- `/acp spawn <agent> --bind here` は child thread を作成せずに current LINE chat を ACP session に bind します。
- Configured ACP bindings と active conversation-bound ACP sessions は、LINE 上で他の conversation channels と同じように動作します。

詳細は [ACP agents](/ja-JP/tools/acp-agents) を参照してください。

## Outbound media

LINE Plugin は agent message tool を通じて images、videos、audio files の送信をサポートします。Media は appropriate preview と tracking handling を伴って LINE-specific delivery path から送信されます:

- **Images**: automatic preview generation 付きで LINE image messages として送信されます。
- **Videos**: explicit preview と content-type handling 付きで送信されます。
...
- **No inbound events:** Webhook path が `channels.line.webhookPath` と一致しており、
  Gateway が LINE から reachable であることを確認してください。
- **Media download errors:** media が default limit を超えている場合は、`channels.line.mediaMaxMb` を増やしてください。

## 関連

- [Channels の概要](/ja-JP/channels) — サポートされているすべての channels
