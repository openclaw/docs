...
title: Docs マップ
x-i18n:
    generated_at: "2026-07-04T03:35:30Z"
    model: gpt-5.5
...
- 見出し:
  - H2: クイック判断ガイド
  - H3: スケジュールされたタスク（Cron）と Heartbeat
  - H2: コアコンセプト
  - H3: スケジュールされたタスク（cron）
  - H3: タスク
  - H3: 推論されたコミットメント
  - H3: Task Flow
  - H3: 常時指示
...
- 見出し:
  ...
  - H3: すべてのグループを許可しつつ、@mention は必須にする
  - H3: 特定のグループのみ許可
  - H3: グループ内の送信者を制限する
  - H2: グループ/ユーザー ID の取得
...
- 見出し:
  ...
  - H2: プライベート API アクション
  - H2: 設定書き込み
  - H2: 分割送信 DM の結合（コマンド + URL を 1 つの作成に）
...
- 見出し:
  ...
  - H2: 音声メッセージ
  - H2: 承認メタデータ
  - H3: 完了済みプレビューを静かにするセルフホスト型プッシュルール
  - H2: ボット間ルーム
  - H2: 暗号化と検証
  - H3: 暗号化を有効化
  - H3: ステータスと信頼シグナル
...
- 見出し:
  ...
  - H3: ステップ 5: Teams App Manifest を構築
...
- 見出し:
  ...
  - H3: Dockerイメージを更新した後
  - H3: サンドボックス設定を変更した後
  - H3: SSHターゲットまたはSSH認証素材を変更した後
  - H3: OpenShellソース、ポリシー、またはモードを変更した後
  - H3: setupCommandを変更した後
  - H3: 特定のエージェントのみ
  - H2: これが必要な理由
  - H2: レジストリ移行
  - H2: 設定
...
- 見出し:
  ...
  - H3: 内部フック（Gatewayフック）
  - H3: Pluginフック（エージェント + gatewayライフサイクル）
  - H2: ストリーミング + 部分返信
  - H2: ツール実行 + メッセージングツール
  - H2: 返信整形 + 抑制
  - H2: Compaction + 再試行
...
- 見出し:
  ...
  - H2: セッション、Compaction、刈り込み（永続化されるもの）
...
- 見出し:
  ...
  - H3: 2. ID プロバイダーの委任を構成する
  - H4: Microsoft 365
  - H4: Google Workspace
  - H3: 3. デリゲートをチャネルに紐付ける
...
- 見出し:
  ...
  - H2: プロファイル ID
  - H2: ローテーション順序
  - H3: セッションの固定性（キャッシュに適合）
  - H3: OpenAI Codex サブスクリプションと API キーバックアップ
  - H2: クールダウン
  - H2: 課金による無効化
  - H2: モデルフォールバック
  - H3: 候補チェーンルール
  - H3: どのエラーでフォールバックを進めるか
  - H3: クールダウンスキップとプローブ動作の違い
...
- 見出し:
  ...
  - H2: 1つの WhatsApp 番号、複数の人（DM 分割）
  - H2: ルーティングルール（メッセージがエージェントを選ぶ仕組み）
  - H2: 複数アカウント / 電話番号
  - H2: 概念
  - H2: プラットフォーム例
  - H2: 一般的なパターン
  - H2: エージェントごとのサンドボックスとツール構成
  - H2: 関連
...
- ルート: /date-time
- 見出し:
  - H2: メッセージエンベロープ（デフォルトでローカル）
  - H3: 例
  - H2: システムプロンプト: 現在の日付と時刻
  - H2: システムイベント行（デフォルトでローカル）
  - H3: ユーザータイムゾーン + 形式の設定
  - H2: 時刻形式の検出（自動）
...
- 見出し:
  ...
  - H3: Gateway 設定（推奨）
  - H3: 1回限りの DNS サーバーセットアップ（Gateway ホスト）
  - H3: Tailscale DNS 設定
...
- 見出し:
  ...
  - H3: 推奨スターター
  - H2: 拡張例（主要オプション）
  - H3: シンボリックリンクされた兄弟スキルリポジトリ
  - H2: よくあるパターン
  - H3: 1 つのオーバーライドを持つ共有スキルベースライン
  - H3: マルチプラットフォーム設定
...
- 見出し:
  ...
  - H3: ハイブリッド設定: ホスト型を主系、ローカルをフォールバック
  - H3: ローカル優先、ホスト型の安全網付き
...
- 見出し:
  ...
  - H3: 個別の番号（WhatsApp、Signal、Telegram）
  ...
  - H2: インシデント対応
  - H3: 封じ込める
  - H3: ローテーションする（シークレットが漏えいした場合は侵害を想定）
  - H3: 監査する
  - H3: レポート用に収集する
  - H2: シークレットスキャン
  - H2: セキュリティ問題の報告
...
- 見出し:
  ...
  - H2: アップストリーム 403 によるブロックされたレスポンス
  - H2: ローカルの OpenAI 互換バックエンドは直接プローブに通るが、エージェント実行が失敗する
  - H2: 返信がない
...
- 見出し:
  ...
  - H3: Vision: 画像送信（添付 → マルチモーダルメッセージ）
...
- 見出し:
  ...
  - H2: 機能（任意）
  - H2: プロバイダーサポートマトリクス（OpenClaw 連携）
  - H2: モデル選択ガイダンス
  - H2: 添付ファイルポリシー
  - H2: 設定例
  - H2: ステータス出力
...
- 見出し:
  ...
  - H2: 非推奨ポリシー
  - H2: 現在の互換性領域
  - H3: WhatsApp インバウンドコールバックのフラットエイリアス
  - H3: WhatsApp インバウンド受け入れフィールド
  - H2: リリースノート
...
- 見出し:
  ...
  - H2: Gateway RPC
  - H2: トラブルシューティング
  - H3: セットアップがWebhook公開で失敗する
  - H3: プロバイダー認証情報が失敗する
  - H3: 通話は開始するがプロバイダーWebhookが届かない
  - H3: 署名検証が失敗する
  - H3: Google Meet Twilio参加が失敗する
  - H3: リアルタイム通話に発話がない
  - H2: 関連
...
- 見出し:
  ...
  - H2: 実行場所
  - H2: インストール
  - H3: オプションA: npmからインストールする
  - H3: オプションB: ローカルフォルダーからインストールする（開発）
  - H2: 設定
  - H2: CLI
  - H2: エージェントツール
  - H2: 関連
...
- 見出し:
  - H2: はじめに
  - H2: Thinkingのデフォルト（Claude Fable 5、4.8、4.6）
  - H2: プロンプトキャッシュ
  - H2: 高度な設定
  - H2: トラブルシューティング
  - H2: 関連
...
- 見出し:
  - H2: OpenClaw で Copilot を使う3つの方法
  - H2: 任意のフラグ
  - H2: 非対話型オンボーディング
  - H2: メモリ検索埋め込み
  - H3: 設定
...
- 見出し:
  ...
  - H3: 例: Heartbeat で 1h キャッシュを暖かい状態に保つ
...
- 見出し:
  ...
  - H2: グローバルルール: reasoning のみの未完了ターン
...
- 見出し:
  ...
  - H2: 重要な注意点
  - H2: 結果を再現する
  - H3: Gateway 公開と open gateway の誤設定
  ...
  - H3: ペアリングストアの並行性 / 冪等性
  - H3: 入口トレース相関 / 冪等性
  - H3: ルーティング dmScope precedence + identityLinks
  - H2: 関連
...
- 見出し:
  ...
  - H2: Bleeding edge ワークフロー（ターミナル内の Gateway）
...
- 見出し:
  ...
  - H2: プラグイン形式を理解する
  - H2: プラグインフック
  - H2: アクティブな Gateway の検証
  - H2: トラブルシューティング
  - H3: ブロックされたプラグインパスの所有権
  - H3: 低速なプラグインツールセットアップ
  - H2: 関連
