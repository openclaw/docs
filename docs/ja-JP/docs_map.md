...
title: ドキュメントマップ
x-i18n:
    generated_at: "2026-06-30T13:49:16Z"
    model: gpt-5.5
...
- 見出し:
  ...
  - H2: 対象形式
  - H2: プロアクティブメッセージング
  - H2: Team ID と Channel ID（よくある落とし穴）
  - H2: プライベートチャネル
  - H2: トラブルシューティング
  - H3: 一般的な問題
  - H3: マニフェストアップロードエラー
...
- 見出し:
  ...
  - H3: 内部 hooks (Gateway hooks)
  - H3: Plugin hooks (エージェント + gateway ライフサイクル)
  - H2: ストリーミング + 部分返信
  - H2: ツール実行 + メッセージングツール
  - H2: 返信整形 + 抑制
  - H2: Compaction + 再試行
...
- 見出し:
  ...
  - H2: Anthropic: Claude CLI とトークンの互換性
  - H2: Anthropic の注記
  - H2: モデル認証ステータスの確認
...
- 見出し:
  ...
  - H2: これが機能する理由
  - H2: `--profile rescue onboard` で変更されること
...
- 見出し:
  ...
  - H3: ステップ 3: Remote Gateway 認証を設定
...
- 見出し:
  ...
  - H3: 「ツール X がサンドボックスのツールポリシーでブロックされた」
  - H3: 「これは main だと思ったのに、なぜサンドボックス化されているのか」
  - H2: 関連
...
- 見出し:
  ...
  - H2: 設定強化の例
  - H3: ファイル権限
  - H3: ネットワーク露出（bind、port、firewall）
  ...
  - H3: 番号の分離（WhatsApp、Signal、Telegram）
...
- 見出し:
  ...
  - H2: ロールバック後のプロトコル不一致
  - H2: パスエスケープとして Skill symlink がスキップされた
  - H2: 長いコンテキストには Anthropic 429 の追加使用量が必要
  ...
  - H2: macOS gateway が無音で応答を停止し、ダッシュボードに触れると再開する
  - H2: 高メモリ使用中に Gateway が終了する
  - H2: Gateway が無効な設定を拒否した
  - H2: Gateway probe warnings
  - H2: チャンネルは接続されているが、メッセージが流れない
...
- 見出し:
  ...
  - H2: メディアと添付ファイル
  - H2: セキュリティとアクセス制御
  - H2: チャットコマンド、タスクの中止、「it will not stop」
  - H2: その他
  - H2: 関連
...
- 見出し:
  - H2: Live: ローカルスモークコマンド
  - H2: Live: Android ノード機能スイープ
  - H2: Live: モデルスモーク（プロファイルキー）
  - H3: レイヤー 1: 直接モデル補完（Gateway なし）
  - H3: レイヤー 2: Gateway + 開発エージェントスモーク（"@openclaw" が実際に行うこと）
  - H2: Live: CLI バックエンドスモーク（Claude、Gemini、またはその他のローカル CLI）
  - H2: Live: APNs HTTP/2 プロキシ到達性
  - H2: Live: ACP バインドスモーク（/acp spawn ... --bind here）
  - H2: Live: Codex app-server ハーネススモーク
  - H3: 推奨 Live レシピ
  - H2: Live: モデルマトリックス（カバー範囲）
  - H3: 最新スモークセット（ツール呼び出し + 画像）
  - H3: ベースライン: ツール呼び出し（Read + 任意の Exec）
  - H3: Vision: 画像送信（添付ファイル → マルチモーダルメッセージ）
  - H3: アグリゲーター / 代替 Gateway
  - H2: 認証情報（絶対にコミットしない）
  - H2: Deepgram Live（音声文字起こし）
  - H2: BytePlus コーディングプラン Live
  - H2: ComfyUI ワークフローメディア Live
  - H2: 画像生成 Live
  - H2: 音楽生成 Live
  - H2: 動画生成 Live
  - H2: メディア Live ハーネス
...
- 見出し:
  ...
  - H3: Live（実プロバイダー + 実モデル）
  - H2: どのスイートを実行すべきか
  - H2: Live（ネットワークに触れる）テスト
...
- 見出し:
  ...
  - H2: Plugin は存在するが疑わしい所有権でブロックされている
  - H2: デシジョンツリー
  - H2: 関連
...
- 見出し:
  ...
  - H3: コンテキストエンジンのアセンブリは Codex 入力に投影する必要がある
...
- 見出し:
  ...
  - H3: 1) Gateway を起動する
  - H3: 2) 検出を検証する（任意）
  - H4: Tailnet（Vienna ⇄ London）でのユニキャスト DNS-SD による検出
  - H3: 3) Android から接続する
  - H3: プレゼンス alive ビーコン
  - H3: 4) ペアリングを承認する（CLI）
...
- 見出し:
  - H1: Voice Overlay のライフサイクル (macOS)
  - H2: 現在の意図
  - H2: 実装済み (2025年12月9日)
  - H2: 次のステップ
  - H2: デバッグチェックリスト
...
- 見出し:
  ...
  - H3: discovery に影響する package.json フィールド
  - H2: discovery の優先順位（重複する Plugin ID）
  - H2: JSON Schema 要件
  - H2: 検証動作
  - H2: メモ
  - H2: 関連
...
- 見出し:
  ...
  - H2: 劣化ルール
  - H2: プロバイダーマッピング
  - H2: Presentation と InteractiveReply の比較
  - H2: 配信ピン
...
- 見出し:
  ...
  - H3: Pluginランタイムのモック
  - H3: インスタンス単位スタブを使ったテスト
  - H2: コントラクトテスト（リポジトリ内Plugin）
  - H3: スコープ付きテストの実行
  - H2: lint 強制（リポジトリ内Plugin）
  - H2: テスト設定
  - H2: 関連
...
- 見出し:
  ...
  - H2: 公開
  - H2: トラブルシューティング
  - H3: Pluginエントリが見つかりません: ./dist/index.js
  - H3: Pluginエントリが defineToolPlugin メタデータを公開していません
...
- 見出し:
  ...
  - H2: Gateway RPC
  - H2: トラブルシューティング
  - H3: セットアップが Webhook 公開に失敗します
  - H3: プロバイダー認証情報が失敗します
  - H3: 通話は開始するがプロバイダー Webhook が届きません
  - H3: 署名検証が失敗します
  - H3: Google Meet への Twilio 参加が失敗します
  - H3: リアルタイム通話に音声がありません
  - H2: 関連
...
- 見出し:
  ...
  - H2: 設定
  - H2: トラブルシューティング
  - H3: タブに Workboard は利用不可と表示されます
  - H3: カードが保存されません
  - H3: カードを開始しても期待したセッションが開きません
  - H3: ディスパッチがワーカーを開始しません
  - H2: 関連
...
- 見出し:
  ...
  - H2: 移行計画
  - H3: フェーズ 0: 境界を凍結する
  - H3: フェーズ 1: グローバル制御プレーンを完了する
  - H3: フェーズ 2: エージェント別データベースを導入する
...
- 見出し:
  ...
  - H3: 6) プロバイダー使用状況スナップショット（ステータス/健全性）
...
- 見出し:
  - H2: 主要なノブ
  - H3: cacheRetention（グローバルデフォルト、モデル、エージェント別）
