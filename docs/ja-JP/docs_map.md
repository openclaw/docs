...
このファイルは、エージェントがドキュメントツリーを移動しやすくするため、`docs/**/*.md` と `docs/**/*.mdx` の見出しから生成されています。
...
- 見出し:
  ...
  - H3: すべてのグループを許可、ただし @mention は引き続き必要
  - H3: 特定のグループのみ許可
  - H3: グループ内の送信者を制限
  - H2: グループ/ユーザー ID を取得
...
- 見出し:
  ...
  - H2: プライベート API アクション
  - H2: 設定書き込み
  - H2: 分割送信 DM の結合（コマンド + URL を 1 つの作成内容に）
  - H3: シナリオとエージェントが見る内容
...
- 見出し:
  ...
  - H2: デプロイパターン
  - H2: ランタイムモデル
  - H2: MeowCaller で現在のリクエスト元に発信する（実験的）
  - H2: 承認プロンプト
...
- 見出し:
  ...
  - H3: Claude CLI 固有事項
  - H2: セッション
  - H2: claude-cli セッションからのフォールバック前文
  - H2: 画像
  - H2: 入力と出力
  - H2: Plugin 所有のデフォルト
  - H2: テキスト変換オーバーレイ
...
- 見出し:
  ...
  - H3: macOS: LaunchAgent 経由の永続 SSH トンネル
...
- 見出し:
  ...
  - H2: カスタム bind mount
  - H2: イメージとセットアップ
  - H2: setupCommand (1 回限りのコンテナーセットアップ)
  - H2: ツールポリシーとエスケープハッチ
  - H2: マルチエージェントの上書き
  - H2: 最小限の有効化例
  - H2: 関連
...
- 見出し:
  ...
  - H2: メディアと添付ファイル
  - H2: セキュリティとアクセス制御
  - H2: チャットコマンド、タスク中断、「it will not stop」
  - H2: その他
  - H2: 関連
...
- 見出し:
  ...
  - H2: ライブ: ACP bind スモーク (/acp spawn ... --bind here)
...
- 見出し:
  ...
  - H2: どのスイートを実行すべきか
  - H2: ライブ (ネットワーク接触) テスト
  - H2: Docker ランナー (任意の「Linux で動く」確認)
  - H2: ドキュメントの健全性
...
- 見出し:
  ...
  - H2: ローカル OpenAI 互換バックエンドは直接動くが OpenClaw では失敗する
  - H2: Plugin インストールが missing openclaw extensions で失敗する
...
- 見出し:
  - H2: iOSノード
  - H3: iOSユーザー設定
  - H3: iOSコマンド（Gateway `node.invoke` 経由）
  - H3: iOSフォアグラウンド要件
  - H3: CLIヘルパー
  - H2: Androidノード
  - H3: Androidユーザー設定
  ...
  - H3: Androidコマンド（Gateway `node.invoke` 経由）
  - H2: macOSアプリ
  - H3: macOSユーザー設定
...
- 見出し:
  ...
  - H3: コンテキストエンジンのアセンブリはCodex入力へ投影する必要がある
  - H3: プロンプトキャッシュの安定性が重要
  - H3: ランタイム選択セマンティクスは変更しない
  - H2: 実装計画
  - H3: 1. 再利用可能なコンテキストエンジン試行ヘルパーをエクスポートまたは移動する
  - H3: 2. Codexコンテキスト投影ヘルパーを追加する
  - H3: 3. Codexスレッド起動前にブートストラップを配線する
  - H3: 4. `thread/start` / `thread/resume` と `turn/start` の前にアセンブルを配線する
  - H3: 5. プロンプトキャッシュの安定したフォーマットを維持する
  - H3: 6. トランスクリプトミラーリング後にターン後処理を配線する
  - H3: 7. 使用量とプロンプトキャッシュのランタイムコンテキストを正規化する
  - H3: 8. Compactionポリシー
  - H4: `/compact` と明示的なOpenClaw Compaction
  - H4: ターン内のCodexネイティブ`contextCompaction`イベント
...
- 見出し:
  ...
  - H2: 3. CLIとGatewayをインストールする
  - H2: トラブルシューティング
  - H3: ビルド失敗: ツールチェーンまたはSDKの不一致
  - H3: 権限付与時にアプリがクラッシュする
  - H3: Gatewayが「Starting...」のままになる
  - H2: 関連
...
- 見出し:
  ...
  - H3: screen.snapshot、camera、audio コマンドに失敗する
...
- 見出し:
  ...
  - H2: manifest と package.json の比較
...
- 見出し:
  ...
  - H3: 登録コントラクトのテスト
  - H3: ランタイム設定アクセスのテスト
  - H3: チャネルプラグインのユニットテスト
  - H3: プロバイダープラグインのユニットテスト
  - H3: プラグインランタイムのモック
  - H3: インスタンスごとのスタブを使ったテスト
  - H2: コントラクトテスト（リポジトリ内プラグイン）
  - H3: スコープ付きテストの実行
  - H2: Lint 適用（リポジトリ内プラグイン）
  - H2: テスト設定
  - H2: 関連
...
- 見出し:
  ...
  - H2: 公開
  - H2: トラブルシューティング
  - H3: プラグインエントリが見つかりません: ./dist/index.js
  - H3: プラグインエントリが defineToolPlugin メタデータを公開していません
...
- 見出し:
  ...
  - H2: 移行計画
  - H3: フェーズ1: IDとリースの追加
  - H3: フェーズ2: リース優先クリーンアップ
  - H3: フェーズ3: リース優先起動時刈り取り
  - H3: フェーズ4: セッション所有権行
  - H3: フェーズ5: レガシーヒューリスティックの削除
  - H2: テスト
  - H2: 互換性メモ
  - H2: 成功基準
...
- 見出し:
  ...
  - H2: 移行計画
  - H3: フェーズ0: 境界を凍結
  - H3: フェーズ1: グローバルコントロールプレーンを完成
  - H3: フェーズ2: エージェントごとのデータベースを導入
  - H3: フェーズ3: セッションストアAPIを置換
  - H3: フェーズ4: トランスクリプト、ACPストリーム、軌跡、VFSを移動
  - H3: フェーズ5: バックアップ、復元、Vacuum、検証
  - H3: フェーズ6: ワーカーランタイム
  - H3: フェーズ7: 旧世界を削除
  - H2: バックアップと復元
  - H2: ランタイムリファクタリング計画
...
- 見出し:
  ...
  - H3: Web検索ツール
  - H3: Web取得ツール（Firecrawl）
  - H3: プロバイダー使用状況スナップショット（ステータス/ヘルス）
  - H3: Compactionセーフガード要約
  - H3: モデルスキャン / プローブ
  - H3: Talk（音声）
  - H3: Skills（サードパーティAPI）
  - H2: 関連
...
- 見出し:
  ...
  - H2: 原則
  - H2: フェーズ1: ベースライン監査
  - H2: フェーズ2: プロダクトとUXの整理
  - H2: フェーズ3: フロントエンドアーキテクチャの強化
  - H2: フェーズ4: パフォーマンスと信頼性
  - H2: フェーズ5: 型、契約、テストの強化
  - H2: フェーズ6: ドキュメントとリリース準備
  - H2: 推奨される最初の切り出し
  - H2: フロントエンドskill更新
...
- 見出し:
  ...
  - H3: Heartbeatの保温
  - H2: プロバイダー動作
  - H3: Anthropic（直接APIとVertex AI）
  - H3: OpenAI（直接API）
  - H3: Amazon Bedrock
  - H3: OpenRouter
  - H3: Google Gemini（直接API）
  - H3: CLIハーネスプロバイダー（Claude Code、Gemini CLI）
  - H3: その他のプロバイダー
  - H2: システムプロンプトキャッシュ境界
  - H2: OpenClawのキャッシュ安定性ガード
  - H2: チューニングパターン
  - H3: 混合トラフィック（推奨デフォルト）
  - H3: コスト優先ベースライン
  - H2: ライブ回帰テスト
  - H3: Anthropicライブ期待値
  - H3: OpenAIライブ期待値
  - H2: diagnostics.cacheTrace設定
  - H3: Env切り替え（一回限りのデバッグ）
  - H3: 調査対象
  - H2: クイックトラブルシューティング
  - H2: 関連
...
- ルート: /reference/rpc
- 見出し:
  - H2: パターンA: HTTPデーモン（signal-cli）
  - H2: パターンB: stdio子プロセス（imsg）
  - H2: アダプターガイドライン
  - H2: 関連
...
- 見出し:
  - H2: サポートされる認証情報
  - H3: openclaw.jsonターゲット（secrets configure + secrets apply + secrets audit）
  - H3: auth-profiles.jsonターゲット（secrets configure + secrets apply + secrets audit）
  - H2: サポートされない認証情報
  - H2: 関連
...
- 見出し:
  ...
  - H2: セッションID（sessionId）
  - H2: セッションストアスキーマ（sessions.json）
  - H2: トランスクリプト構造（.jsonl）
  - H2: コンテキストウィンドウと追跡トークンの違い
  - H2: Compaction: それは何か
  - H3: チャンク境界とツールペアリング
  - H2: 自動Compactionが発生するタイミング
  - H2: Compaction設定
  - H2: プラグ可能なCompactionプロバイダー
  - H2: ユーザーに見えるサーフェス
  - H2: サイレントハウスキーピング（NOREPLY）
  - H2: Compaction前メモリフラッシュ
...
- 見出し:
  ...
  - H2: GatewayとE2E
  - H2: フルDockerスイート（pnpm test:docker:all）
  - H3: 注目すべきDockerレーン
  - H2: ローカルPRゲート
  - H2: テストパフォーマンスツール
  - H2: ベンチマーク
...
- 見出し:
  ...
  - H2: 現在のトークン使用量の確認方法
  - H2: コスト見積もり（表示される場合）
  - H2: キャッシュTTLとプルーニングの影響
  - H3: 例: Heartbeatで1時間キャッシュを保温する
  - H3: 例: エージェント単位のキャッシュ戦略を使う混合トラフィック
...
- 見出し:
  ...
  - H2: グローバルルール: 不完全なreasoning限定ターン
...
- 見出し:
  ...
  - H4: T-PERSIST-001: 悪意のある skill のインストール
...
- 見出し:
  ...
  - H3: 入口ゲーティング（メンションと制御コマンドのバイパス）
  - H3: ルーティングとセッションキー分離
  - H2: v1++ モデル: 並行性、リトライ、トレースの正確性
  - H3: ペアリングストアの並行性とべき等性
  - H3: 入口トレースの相関とべき等性
  - H3: ルーティング dmScope の優先順位と identityLinks
  - H2: 関連
...
- 見出し:
  - H2: 問題: ポート 18800 で Chrome CDP の起動に失敗した
  - H3: 根本原因
  - H3: 解決策 1: Google Chrome をインストールする（推奨）
  - H3: 解決策 2: snap Chromium をアタッチ専用モードで使用する
...
- 見出し:
  ...
  - H2: Brave または別の Chromium ベースのブラウザを使用する
  - H2: ローカル制御とリモート制御
...
- 見出し:
  ...
  - H3: Argv 検証と拒否されるフラグ
  - H3: 信頼済みバイナリディレクトリ
  - H3: シェル連結、ラッパー、マルチプレクサ
  - H3: セーフビンと許可リスト
  - H2: インタープリタ/ランタイムコマンド
  - H3: フォローアップ配信動作
  - H2: チャットチャンネルへの承認転送
  - H3: Plugin 承認転送
...
- 見出し:
  ...
  - H2: YOLO モード (承認なし)
  - H3: 永続的な gateway-host の「プロンプトを表示しない」セットアップ
  - H3: ローカルショートカット
  - H3: Node ホスト
  - H3: セッション限定ショートカット
  - H2: 許可リスト (エージェント単位)
...
- 見出し:
  ...
  - H2: プロバイダー機能マトリクス
  - H2: 非同期と同期
  - H2: Speech-to-text と Voice Call
  - H2: プロバイダーマッピング (ベンダーがサーフェスをどう分割するか)
  - H2: 関連
...
- 見出し:
  - H2: 読み込み順序
  - H2: Agent ごとの Skills と共有 Skills
  - H2: Agent 許可リスト
  - H2: Plugin と Skills
  - H2: Skill Workshop
...
- 見出し:
  ...
  - H3: Skill コマンド
  - H2: /tools: Agent が今使えるもの
  - H2: /model: モデル選択
  - H2: /config: ディスク上の config 書き込み
  - H2: /mcp: MCP サーバー config
  - H2: /debug: ランタイム専用の上書き
  - H2: /plugins: Plugin 管理
  - H2: /trace: Plugin トレース出力
  - H2: /btw: 補足の質問
  - H2: サーフェスの注記
  - H2: Provider の使用状況とステータス
  - H2: 関連
...
- 見出し:
  ...
  - H2: ペルソナ
  - H3: 最小ペルソナ
  - H3: 完全なペルソナ（Provider 非依存プロンプト）
  - H3: ペルソナ解決
  - H3: Provider がペルソナプロンプトを使う方法
  - H3: フォールバックポリシー
  - H2: モデル駆動ディレクティブ
  - H2: スラッシュコマンド
  - H2: ユーザーごとの設定
...
- 見出し:
  ...
  - H2: クイックスタート
  - H2: 仕組み
  - H3: Transcript と配信モデル
  - H2: Control UI Agent ツールパネル
  - H2: リモート利用
  - H2: 設定リファレンス (WebChat)
  - H2: 関連
