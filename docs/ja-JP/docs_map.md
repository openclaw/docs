...
title: ドキュメントマップ
x-i18n:
    generated_at: "2026-07-03T09:21:59Z"
    model: gpt-5.5
...
- 見出し:
  ...
  - H3: すべてのグループを許可し、引き続き @メンションを要求する
  - H3: 特定のグループのみ許可する
  - H3: グループ内の送信者を制限する
  ...
  - H3: Bot がメッセージを受信しない
  - H3: Feishu モバイルアプリで QR 設定が反応しない
...
- 見出し:
  ...
  - H2: プライベート API アクション
  - H2: 設定書き込み
  - H2: 分割送信 DM の結合（コマンド + URL を 1 つの作成にまとめる）
...
- 見出し:
  ...
  - H3: チャンネル内の全員に同じツールを使う
  - H3: 送信者ごとに異なるツールを使う（所有者はより強い権限を持つ）
  - H2: NickServ
  - H2: 環境変数
  - H2: トラブルシューティング
  - H2: 関連
...
- 見出し:
  ...
  - H3: アウトバウンドメッセージ配信を最適化する
  - H3: テキスト結合戦略を調整する
  - H2: よく使うコマンド
  - H2: トラブルシューティング
  - H3: botがグループチャットで応答しない
  - H3: botがメッセージを受信しない
  - H3: botが空またはフォールバックの返信を送信する
  - H3: App Secretが漏えいした
  - H2: 高度な設定
  - H3: 複数アカウント
  - H3: メッセージ制限
...
- 見出し:
  ...
  - H3: ツールプロファイル
  - H3: ツールグループ
  - H3: サンドボックスツールポリシー内の MCP とプラグインツール
  - H3: tools.codeMode
...
- 見出し:
  ...
  - H3: ロール
  - H3: スコープ（オペレーター）
  - H3: Caps/commands/permissions（ノード）
  - H2: Presence
...
- 見出し:
  ...
  - H3: ステップ 2: SSH Key をコピー
  - H3: ステップ 3: リモート Gateway Auth を設定
  - H3: ステップ 4: SSH Tunnel を開始
...
- 見出し:
  ...
  - H3: 「ツール X はサンドボックスのツールポリシーによってブロックされました」
...
- 見出し:
  ...
  - H2: 長いコンテキストには Anthropic 429 追加使用量が必要
  - H2: アップストリーム 403 ブロックレスポンス
  - H2: ローカル OpenAI 互換バックエンドは直接プローブに通るがエージェント実行が失敗する
  - H2: 返信がない
...
- 見出し:
  ...
  - H2: openclaw extensions が見つからず Plugin インストールに失敗する
...
- 見出し:
  ...
  - H2: 望ましい動作
  - H2: 設計制約
  - H3: Codex app-server はネイティブスレッド状態の正準のままにする
  ...
  - H3: 5. プロンプトキャッシュの安定したフォーマットを保持する
  - H3: 6. トランスクリプトミラーリング後に post-turn を接続する
...
- 見出し:
  ...
  - H3: 1) Gateway を起動
  - H3: 2) 探索を検証（任意）
  - H4: ユニキャスト DNS-SD による Tailnet（Vienna ⇄ London）探索
  - H3: 3) Android から接続
  - H3: プレゼンス alive ビーコン
  - H3: 4) ペアリングを承認（CLI）
  - H3: 5) ノードが接続されていることを検証
  - H3: 6) チャット + 履歴
  - H3: 7) キャンバス + カメラ
...
- 見出し:
  ...
  - H2: ライブテストチェックリスト
  - H2: トラブルシューティング
  - H3: エージェントが Google Meet ツールを確認できない
  - H3: Google Meet 対応 Node が接続されていない
...
- 見出し:
  ...
  - H3: ツールとコマンド
  - H3: インフラストラクチャ
  - H3: ワークフロー Plugin 向けのホストフック
  - H3: Gateway discovery 登録
  - H3: CLI 登録メタデータ
  - H3: CLI バックエンド登録
  - H3: 排他的スロット
  - H3: 非推奨のメモリ埋め込みアダプター
...
- 見出し:
  - H2: Plugin エントリ
  - H3: 非推奨の互換性とテストヘルパー
  - H3: 予約済み bundled Plugin ヘルパーサブパス
  - H2: 関連
...
- 見出し:
  ...
  - H3: 登録コントラクトのテスト
  - H3: ランタイム設定アクセスのテスト
  - H3: チャネル Plugin の単体テスト
  - H3: provider Plugin の単体テスト
  - H3: Plugin ランタイムのモック
...
- 見出し:
  ...
  - H2: カード実行とタスク
  - H2: エージェント連携
  - H3: dispatch worker の選択
  - H3: worker プロンプトとライフサイクル
  - H3: dispatch エントリポイント
  - H2: CLI とスラッシュコマンド
  - H2: セッションライフサイクル同期
  - H2: ダッシュボードワークフロー
  - H2: 権限
  - H2: 設定
  - H2: トラブルシューティング
  - H3: タブに Workboard は利用不可と表示される
  - H3: カードが保存されない
  - H3: カードを開始しても期待されるセッションが開かない
  - H3: dispatch が worker を開始しない
  - H2: 関連
...
- 見出し:
  ...
  - H3: フェーズ 4: トランスクリプト、ACP ストリーム、トラジェクトリ、VFS を移行
...
- 見出し:
  ...
  - H3: 例: heartbeat で 1 時間キャッシュを温めておく
...
- 見出し:
  ...
  - H4: T-EXFIL-002: 未承認メッセージ送信
  - H4: T-EXFIL-003: 認証情報の収集
  - H3: 3.8 影響（AML.TA0011）
  - H4: T-IMPACT-001: 未承認コマンド実行
...
- 見出し:
  ...
  - H3: イングレスゲーティング（メンション + 制御コマンドバイパス）
  - H3: ルーティング/セッションキー分離
  - H2: v1++: 追加の有界モデル（並行性、再試行、トレース正確性）
  - H3: ペアリングストアの並行性 / べき等性
  - H3: イングレストレース相関 / べき等性
...
- 見出し:
  ...
  - H2: YOLO モード（承認なし）
  - H3: 永続 Gateway ホスト「never prompt」設定
  - H3: ローカルショートカット
  - H3: Node ホスト
  - H3: セッション限定ショートカット
  - H2: 許可リスト（エージェントごと）
