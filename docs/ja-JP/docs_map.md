...
title: ドキュメントマップ
x-i18n:
    generated_at: "2026-07-04T06:21:53Z"
    model: gpt-5.5
...
- 見出し:
  ...
  - H3: すべてのグループを許可、ただし @メンションは引き続き必要
  - H3: 特定のグループのみ許可
  - H3: グループ内の送信者を制限
  - H2: グループ/ユーザー ID の取得
...
- 見出し:
  ...
  - H2: 音声メッセージ
  - H2: 承認メタデータ
  - H3: 静かな確定済みプレビュー向けのセルフホスト型プッシュルール
  - H2: ボット間ルーム
  - H2: 暗号化と検証
  - H3: 暗号化を有効化
  - H3: ステータスと信頼シグナル
...
- 見出し:
  ...
  - H2: hookポイント（介入できる場所）
  - H3: 内部hook（Gateway hook）
  - H3: Plugin hook（エージェント + Gatewayライフサイクル）
  - H2: ストリーミング + 部分返信
  - H2: ツール実行 + メッセージングツール
  - H2: 返信整形 + 抑制
  - H2: Compaction + リトライ
...
- 見出し:
  ...
  - H2: 主要なデフォルト
  - H2: Dreams UI
  - H2: Dreaming が実行されない: ステータスにブロック中と表示される
  - H2: 関連
...
- ルート: /gateway/bonjour
- 見出し:
  - H2: Tailscale 越しの広域 Bonjour（Unicast DNS-SD）
  - H3: Gateway 設定（推奨）
...
- 見出し:
  ...
  - H2: 一般的なパターン
  - H3: 1 つのオーバーライドを持つ共有 skill ベースライン
  - H3: マルチプラットフォーム設定
...
- 見出し:
  ...
  - H3: 「ツール X がサンドボックスツールポリシーでブロックされた」
...
- 見出し:
  ...
  - H2: 設定例
  - H3: Tailnet 限定（Serve）
  - H3: Tailnet 限定（Tailnet IP にバインド）
...
- 見出し:
  ...
  - H3: プロキシ環境サポート（Provider モデル）
  - H2: 機能（任意）
  - H2: Provider サポートマトリックス（OpenClaw 連携）
  - H2: モデル選択ガイダンス
  - H2: 添付ファイルポリシー
  - H2: 設定例
  - H2: ステータス出力
...
- 見出し:
  ...
  - H3: 3. Codex スレッド起動前に bootstrap を接続
  - H3: 4. thread/start / thread/resume および turn/start の前に assemble を接続
  - H3: 5. プロンプトキャッシュの安定したフォーマットを保持
...
- 見出し:
  ...
  - H3: 例: Heartbeat で 1時間キャッシュを温かく保つ
...
- 見出し:
  ...
  - H2: グローバルルール: 未完了の reasoning-only ターン
...
- 見出し:
  - H2: TL;DR
  - H2: 前提条件（ソースから）
  - H2: カスタマイズ戦略（更新で困らないために）
  - H2: このリポジトリから Gateway を実行
  - H2: 安定版ワークフロー（macOS アプリを先に）
...
- 見出し:
  ...
  - H2: 非同期と同期
  - H2: 音声テキスト変換と音声通話
  - H2: プロバイダーマッピング（ベンダーがサーフェスをどう分けるか）
  - H2: 関連
