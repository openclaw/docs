...
title: ドキュメントマップ
x-i18n:
    generated_at: "2026-07-04T10:26:54Z"
    model: gpt-5.5
...
- 見出し:
  - H1: BlueBubblesの削除とimsg iMessageパス
  - H2: 変更点
  - H2: すべきこと
  - H2: 移行メモ
  - H2: 関連項目
...
- 見出し:
  ...
  - H3: フックパック
  - H2: バンドル済みフック
  - H3: session-memoryの詳細
  - H3: bootstrap-extra-files設定
  - H3: command-loggerの詳細
  - H3: compaction-notifierの詳細
  - H3: boot-mdの詳細
  - H2: Pluginフック
  - H2: 設定
  - H2: CLIリファレンス
...
- 見出し:
  ...
  - H2: 常設指示とcronジョブ
  - H2: 例
  - H3: 例1: コンテンツとソーシャルメディア（週間サイクル）
  - H3: 例2: 財務オペレーション（イベントトリガー）
  - H3: 例3: 監視とアラート（継続）
  - H2: 実行・検証・報告パターン
  - H2: マルチプログラムアーキテクチャ
  - H2: ベストプラクティス
...
- 見出し:
  ...
  - H3: グループチャット
  - H2: グループ設定例
  - H3: すべてのグループを許可、@メンション不要
  - H3: すべてのグループを許可、引き続き@メンションが必要
  - H3: 特定のグループのみ許可
  - H3: グループ内の送信者を制限
  - H2: グループ/ユーザーIDを取得
  - H3: グループID（chatid、形式: ocxxx）
  - H3: ユーザーID（openid、形式: ouxxx）
  - H2: 一般的なコマンド
  - H2: トラブルシューティング
  - H3: ボットがグループチャットで応答しない
  - H3: ボットがメッセージを受信しない
  - H3: QR設定がFeishuモバイルアプリで反応しない
  - H3: App Secretが漏えいした
  - H2: 高度な設定
  - H3: 複数アカウント
  - H3: メッセージ制限
...
- 見出し:
  ...
  - H2: プライベートAPIアクション
  - H2: 設定書き込み
  - H2: 分割送信DMの結合（1つの構成内のコマンド + URL）
  - H3: シナリオとエージェントが見る内容
  - H2: ブリッジまたはGateway再起動後の受信復旧
  - H3: オペレーターに見えるシグナル
  - H3: 移行
  - H2: トラブルシューティング
  - H2: 設定リファレンスポインター
...
- 見出し:
  ...
  - H2: セキュリティデフォルト
  - H2: アクセス制御
  - H3: よくある落とし穴: allowFromはDM用であり、チャネル用ではない
  - H2: 返信トリガー（メンション）
  - H2: セキュリティメモ（公開チャネルで推奨）
  - H3: チャネル内の全員に同じツール
...
- 見出し:
  ...
  - H3: ステップ 2: 認証情報を取得
  - H3: ステップ 3: Messaging Endpoint を構成
...
- 見出し:
  ...
  - H3: 再利用可能な送信者グループ
  - H3: 状態の保存場所
  - H2: 2) Node デバイスペアリング（iOS/Android/macOS/headless ノード）
...
- 見出し:
  - H2: インストール（オンデマンド）
  - H2: クイックセットアップ
  - H2: MeowCallerで現在のリクエスト元に電話する（実験的）
  - H2: デプロイパターン
  - H2: ランタイムモデル
  - H2: 承認プロンプト
  - H2: Pluginフックとプライバシー
...
- 見出し:
  ...
  - H3: Docker イメージの更新後
  - H3: サンドボックス設定の変更後
  - H3: SSH ターゲットまたは SSH 認証素材の変更後
  - H3: OpenShell ソース、ポリシー、またはモードの変更後
  - H3: setupCommand の変更後
  - H3: 特定のエージェントのみ
  - H2: これが必要な理由
  - H2: レジストリ移行
  - H2: 設定
...
- 見出し:
  ...
  - H2: セッション、Compaction、刈り込み（永続するもの）
...
- 見出し:
  ...
  - H2: 主要なデフォルト
  - H2: Dreams UI
  - H2: Dreaming が実行されない: ステータスがブロック中と表示される
  - H2: 関連
...
- 見出し:
  ...
  - H3: フェーズ 4: Prepared Dispatcher ブリッジ
  - H3: フェーズ 5: 統合ライブライフサイクル
  - H3: フェーズ 6: 公開 SDK
  - H3: フェーズ 7: すべての送信者
  - H3: フェーズ 8: Turn 名称付き互換性の削除
  - H2: テスト計画
  - H2: 未解決の質問
  - H2: 受け入れ基準
  - H2: 関連
...
- 見出し:
  ...
  - H3: セキュア DM モード（共有 inbox / 複数ユーザー DM）
...
- 見出し:
  ...
  - H2: Upstream 403 ブロックされたレスポンス
  - H2: ローカル OpenAI 互換バックエンドは直接プローブに成功するが、エージェント実行が失敗する
  - H2: 返信がない
...
- 見出し:
  ...
  - H3: 3. Codex スレッド起動前にブートストラップを接続
  - H3: 4. thread/start / thread/resume と turn/start の前に assemble を接続
  - H3: 5. プロンプトキャッシュで安定したフォーマットを維持
  - H3: 6. トランスクリプトのミラーリング後に post-turn を接続
...
- 見出し:
  ...
  - H2: コスト見積もり（表示される場合）
  - H2: キャッシュ TTL と刈り込みの影響
  - H3: 例: Heartbeat で 1時間キャッシュを温め続ける
...
- 見出し:
  ...
  - H3: Node exec パイプライン（最も高リスクな機能）
  - H3: ペアリングストア（DM ゲーティング）
  - H3: 入口ゲーティング（メンション + 制御コマンドバイパス）
  - H3: ルーティング/セッションキー分離
  - H2: v1++: 追加の有界モデル（並行処理、再試行、トレース正確性）
  - H3: ペアリングストアの並行処理 / 冪等性
  - H3: 入口トレース相関 / 冪等性
...
- 見出し:
  ...
  - H2: このリポジトリから Gateway を実行
  - H2: 安定ワークフロー（macOS アプリを先に）
  - H2: 最先端ワークフロー（ターミナル内の Gateway）
  - H3: 0) （任意）macOS アプリもソースから実行
  - H3: 1) 開発用 Gateway を開始
  - H3: 2) 実行中の Gateway を macOS アプリに指定
  - H3: 3) 検証
  - H3: よくある落とし穴
  - H2: 認証情報ストレージマップ
  - H2: 更新（セットアップを壊さずに）
...
- 見出し:
  ...
  - H3: 根本原因
  - H3: 解決策 1: Google Chrome をインストール（推奨）
  - H3: 解決策 2: Snap Chromium を Attach-Only Mode で使用
  - H3: ブラウザーが動作することを確認
  - H3: 設定リファレンス
...
- 見出し:
  ...
  - H2: レイヤーごとに検証
  - H3: レイヤー 1: Chrome が Windows で CDP を提供していることを確認
  - H3: レイヤー 2: WSL2 からその Windows エンドポイントに到達できることを確認
  - H3: レイヤー 3: 正しいブラウザープロファイルを設定
  - H3: レイヤー 4: Control UI レイヤーを個別に確認
  - H3: レイヤー 5: エンドツーエンドのブラウザー制御を確認
  - H2: よくある誤解を招くエラー
  - H2: 高速トリアージチェックリスト
  - H2: 実践的な要点
  - H2: 関連
...
- 見出し:
  ...
  - H2: Brave または別の Chromium ベースブラウザーを使用
  - H2: ローカル制御とリモート制御
...
- 見出し:
  ...
  - H3: 安全な bin と許可リストの違い
  - H2: インタープリター/ランタイムコマンド
  - H3: フォローアップ配信動作
  - H2: チャットチャンネルへの承認転送
  - H3: Plugin 承認転送
  - H3: 任意チャンネルでの同一チャット承認
  - H3: ネイティブ承認配信
  - H3: macOS IPC フロー
  - H2: FAQ
...
- 見出し:
  ...
  - H2: Firecrawl 検索を設定
  - H2: Firecrawl webfetch フォールバックを設定
  - H3: セルフホスト Firecrawl
...
- 見出し:
  ...
  - H2: エージェントごとの適用
  - H2: 高速モード（/fast）
  - H2: 詳細指示（/verbose または /v）
  - H2: Plugin トレース指示（/trace）
  - H2: 推論の可視性（/reasoning）
  - H2: 関連
  - H2: Heartbeats
  - H2: Web チャット UI
