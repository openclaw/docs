...
x-i18n:
    generated_at: "2026-04-30T05:35:07Z"
    model: gpt-5.5
    provider: openai
...
**フレームワーク:** [MITRE ATLAS](https://atlas.mitre.org/) (AI システムの敵対的脅威ランドスケープ)

### フレームワークの帰属

この脅威モデルは、AI/ML システムへの敵対的脅威を文書化するための業界標準フレームワークである [MITRE ATLAS](https://atlas.mitre.org/) に基づいています。ATLAS は [MITRE](https://www.mitre.org/) が AI セキュリティコミュニティと協力して保守しています。
...
これは OpenClaw コミュニティが保守する継続的に更新されるドキュメントです。貢献に関するガイドラインについては [CONTRIBUTING-THREAT-MODEL.md](/ja-JP/security/CONTRIBUTING-THREAT-MODEL) を参照してください。
...
この脅威モデルは、AI/ML システム向けに特別に設計された MITRE ATLAS フレームワークを使用して、OpenClaw AI エージェントプラットフォームと ClawHub スキルマーケットプレイスに対する敵対的脅威を文書化します。
...
| **推奨事項**     | セキュアなデプロイを文書化し、検出エンドポイントにレート制限を追加する |
...
| ----------------------- | ------------------------------------------------------------------ |
| **ATLAS ID**            | AML.T0006 - アクティブスキャン                                     |
| **説明**                | 攻撃者がメッセージングチャネルを探査し、AI管理アカウントを特定する |
| **攻撃ベクトル**        | テストメッセージの送信、応答パターンの観察                         |
| **影響を受けるコンポーネント** | すべてのチャネル連携                                         |
| **現在の緩和策**        | 特になし                                                           |
| **残存リスク**          | 低 - 発見だけでは得られる価値は限定的                              |
...
| **説明**                | 攻撃者がペアリング猶予期間中にペアリングコードを傍受する（DMチャネルペアリングは1時間、ノードペアリングは5分） |
| **攻撃ベクトル**        | ショルダーサーフィン、ネットワークスニッフィング、ソーシャルエンジニアリング                                     |
| **影響を受けるコンポーネント** | デバイスペアリングシステム                                                                              |
| **現在の緩和策**        | 1時間の有効期限（DMペアリング）/ 5分の有効期限（ノードペアリング）、コードは既存チャネル経由で送信              |
| **残存リスク**          | 中 - 猶予期間が悪用される可能性がある                                                                           |
| **推奨事項**            | 猶予期間を短縮し、確認ステップを追加する                                                                         |

#### T-ACCESS-002: AllowFromスプーフィング

| 属性                    | 値                                                                                 |
| ----------------------- | ---------------------------------------------------------------------------------- |
| **ATLAS ID**            | AML.T0040 - AIモデル推論APIアクセス                                                |
| **説明**                | 攻撃者がチャネル内で許可済み送信者の識別情報を偽装する                             |
| **攻撃ベクトル**        | チャネルに依存 - 電話番号のスプーフィング、ユーザー名のなりすまし                  |
| **影響を受けるコンポーネント** | チャネルごとのAllowFrom検証                                                |
| **現在の緩和策**        | チャネル固有の識別情報検証                                                         |
| **残存リスク**          | 中 - 一部のチャネルはスプーフィングに脆弱                                          |
...
| **影響を受けるコンポーネント** | エージェントLLM、すべての入力サーフェス                                             |
...
| **現在の緩和策**        | 危険なコマンドに対するexec承認                               |
| **残存リスク**          | 高 - ユーザーの判断に依存                                    |
| **推奨事項**            | 引数検証、パラメータ化されたツール呼び出しを実装する         |

#### T-EXEC-004: Exec承認バイパス
...
| **説明**                | 攻撃者が悪意あるスキルをClawHubに公開する                                |
| **攻撃ベクトル**        | アカウントを作成し、隠された悪意あるコードを含むスキルを公開する         |
| **影響を受けるコンポーネント** | ClawHub、スキル読み込み、エージェント実行                         |
| **現在の緩和策**        | GitHubアカウント年齢の検証、パターンベースのモデレーションフラグ         |
| **残存リスク**          | 重大 - サンドボックス化なし、レビューは限定的                            |
| **推奨事項**            | VirusTotal連携（進行中）、スキルのサンドボックス化、コミュニティレビュー |

#### T-PERSIST-002: スキル更新ポイズニング

| 属性                    | 値                                                             |
| ----------------------- | -------------------------------------------------------------- |
| **ATLAS ID**            | AML.T0010.001 - サプライチェーン侵害: AIソフトウェア          |
| **説明**                | 攻撃者が人気スキルを侵害し、悪意ある更新をプッシュする        |
| **攻撃ベクトル**        | アカウント侵害、スキル所有者へのソーシャルエンジニアリング    |
| **影響を受けるコンポーネント** | ClawHubのバージョニング、自動更新フロー                 |
...
| **攻撃ベクトル**        | Unicode同形異字、エンコーディングのトリック、動的読み込み              |
| **影響を受けるコンポーネント** | ClawHub moderation.ts                                           |
| **現在の緩和策**        | パターンベースのFLAG_RULES                                             |
| **残存リスク**          | 高 - 単純な正規表現は容易にバイパスされる                              |
| **推奨事項**            | 振る舞い分析（VirusTotal Code Insight）、ASTベース検出を追加する       |
...
| **攻撃ベクトル**        | 「What tools do you have?」形式のクエリ               |
| **影響を受けるコンポーネント** | エージェントのツールレジストリ                       |
| **現在の緩和策**        | 特になし                                              |
| **残存リスク**          | 低 - ツールは通常文書化されている                     |
| **推奨事項**            | ツール可視性コントロールを検討する                   |
...
| **攻撃ベクトル**        | 「What did we discuss?」クエリ、コンテキスト探索      |
| **影響を受けるコンポーネント** | セッショントランスクリプト、コンテキストウィンドウ   |
| **現在の緩和策**        | 送信者ごとのセッション分離                            |
| **残存リスク**          | 中 - セッション内データにアクセス可能                 |
| **推奨事項**            | コンテキスト内の機密データのリダクションを実装する   |
...
| **攻撃ベクトル**        | エージェントに攻撃者サーバーへデータを POST させるプロンプトインジェクション |
| **影響を受けるコンポーネント** | web_fetch ツール                                                   |
| **現在の緩和策**        | 内部ネットワークに対する SSRF ブロック                              |
| **残存リスク**          | 高 - 外部 URL が許可されている                                      |
...
| ----------------------- | ------------------------------------------------------------- |
| **ATLAS ID**            | AML.T0009 - 収集                                              |
| **説明**                | 攻撃者が機密データを含むメッセージをエージェントに送信させる |
| **攻撃ベクトル**        | エージェントに攻撃者へメッセージを送信させるプロンプトインジェクション |
| **影響を受けるコンポーネント** | メッセージツール、チャネル連携                              |
| **現在の緩和策**        | アウトバウンドメッセージングのゲート                          |
| **残存リスク**          | 中 - ゲートがバイパスされる可能性がある                       |
...
| 属性                    | 値                                               |
| ----------------------- | ------------------------------------------------ |
| **ATLAS ID**            | AML.T0031 - AI モデル完全性の低下                |
| **説明**                | 攻撃者がユーザーシステム上で任意のコマンドを実行する |
| **攻撃ベクトル**        | プロンプトインジェクションと exec 承認バイパスの組み合わせ |
| **影響を受けるコンポーネント** | Bash ツール、コマンド実行                       |
| **現在の緩和策**        | Exec 承認、Docker サンドボックスオプション       |
| **残存リスク**          | 重大 - サンドボックスなしのホスト実行            |
| **推奨事項**            | サンドボックスをデフォルトにする、承認 UX を改善する |
...
| コントロール         | 実装                        | 有効性                                               |
| -------------------- | --------------------------- | ---------------------------------------------------- |
| GitHub アカウント年齢 | `requireGitHubAccountAge()` | 中 - 新しい攻撃者に対するハードルを上げる           |
| パスサニタイズ       | `sanitizePath()`            | 高 - パストラバーサルを防ぐ                         |
| ファイル種別検証     | `isTextFile()`              | 中 - テキストファイルのみだが、それでも悪意を持つ可能性がある |
| サイズ制限           | 合計バンドル 50MB           | 高 - リソース枯渇を防ぐ                             |
| 必須の SKILL.md      | 必須 readme                 | セキュリティ価値は低い - 情報提供のみ               |
| パターンモデレーション | moderation.ts の FLAG_RULES | 低 - 容易にバイパスされる                           |
...
- 実際の Skill コード内容を分析しない
- 単純な正規表現は難読化で容易にバイパスされる
- 振る舞い分析がない
...
| コミュニティ報告      | 部分的（`skillReports` テーブルが存在） | 中                                                                    |
| 監査ログ              | 部分的（`auditLogs` テーブルが存在）  | 中                                                                    |
...
| R-003 | 機密性の高いアクションの出力検証を追加する | T-EXEC-001, T-EXEC-002     |
...
| R-009 | config の完全性検証を実装する                 | T-PERSIST-003 |
...
| AML.T0031     | AI モデルの完全性低下          | T-IMPACT-001, T-IMPACT-002, T-IMPACT-003                         |
...
| `src/agents/sandbox/tool-policy.ts` | ツールポリシーの強制            | **重大**     |
...
| **プロンプトインジェクション** | 悪意ある指示が入力に埋め込まれる攻撃                    |
| **スキル**           | OpenClaw エージェント向けのダウンロード可能な拡張        |
| **SSRF**             | Server-Side Request Forgery                               |

---

_この脅威モデルは継続的に更新される文書です。セキュリティ問題は security@openclaw.ai に報告してください_

## 関連

- [形式的検証](/ja-JP/security/formal-verification)
