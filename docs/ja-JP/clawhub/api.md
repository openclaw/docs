...
- 積極的なポーリングではなく、レスポンスをキャッシュし、`429`、`Retry-After`、レート制限ヘッダーを尊重してください。
- 一覧を表示する際は、ユーザーがソースレジストリレコードを確認できるように、正規の ClawHub スキル URL へリンクしてください。
...
- `GET /api/v1/skills?limit=&cursor=&sort=`
  ...
  - レガシーエイリアス: `nonSuspicious=true`
  - `nonSuspiciousOnly=true` の場合、cursor ベースのページに含まれる項目数が `limit` より少ないことがあります。続行するには `nextCursor` を使用してください。
