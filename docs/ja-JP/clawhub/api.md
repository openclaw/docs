...
read_when:
    - API クライアントの構築
    - エンドポイントまたはスキーマの追加
summary: パブリック REST API (v1) の概要と規約。
x-i18n:
...
ClawHub の公開読み取り API の上に、サードパーティのカタログ、ディレクトリ、検索画面を構築できます。公開 skill メタデータと skill ファイルは ClawHub の skill ライセンス規則に基づいて公開されます。一方、API 自体にはレート制限があり、責任を持って利用する必要があります。
...
- 一覧を表示する際は、ユーザーがソースレジストリレコードを確認できるように、正規の ClawHub skill URL にリンクします。
- `https://clawhub.ai/<owner>/skills/<slug>` 形式の正規ページ URL を使用します。
- ClawHub がサードパーティサイトを承認、検証、または運営していると示唆しないでください。
...
- `X-RateLimit-Remaining` / `RateLimit-Remaining`: 存在する場合は正確な残り予算。シャード化された成功リクエストでは、概算のグローバル値を返すのではなく省略します
...
- `GET /api/v1/download?slug=&version=&tag=`
  - ホストされた skill は決定的な ZIP バイトを返します。
  - `clean` または `suspicious` スキャンを持つ現在の GitHub バックエンド skill は、ClawHub バイトの代わりに JSON `public-github` 引き継ぎ記述子を返します。
- `GET /api/v1/skills/export?startDate=&endDate=&limit=&cursor=`
  - ホストされた skill は保存済みファイルとしてエクスポートされます。
  - `clean` または `suspicious` スキャンを持つ現在の GitHub バックエンド skill は、`public-github` 引き継ぎ記述子としてエクスポートされます。
