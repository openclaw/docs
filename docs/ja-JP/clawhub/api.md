...
ClawHub の公開読み取り API の上に、サードパーティのカタログ、ディレクトリ、検索画面を構築できます。公開スキルメタデータとスキルファイルは ClawHub のスキルライセンス規則に従って公開されます。一方で API 自体にはレート制限があり、責任を持って利用する必要があります。
...
- 一覧を表示するときは、ユーザーがソースのレジストリレコードを確認できるように、正規の ClawHub スキル URL へリンクします。
...
- `GET /api/v1/skills?limit=&cursor=&sort=`
  ...
  - レガシーエイリアス: `nonSuspicious=true`
  - `nonSuspiciousOnly=true` の場合、カーソルベースのページには `limit` より少ない項目しか含まれないことがあります。続行するには `nextCursor` を使用してください。
...
- `GET /api/v1/download?slug=&version=&tag=`
  - ホスト型スキルは決定的な ZIP バイト列を返します。
  - 現在の GitHub ベースのスキルで、`clean` または `suspicious` のスキャン結果があるものは、ClawHub のバイト列ではなく JSON の `public-github` ハンドオフ記述子を返します。
- `GET /api/v1/skills/export?startDate=&endDate=&limit=&cursor=`
  - ホスト型スキルは保存済みファイルとしてエクスポートされます。
  - 現在の GitHub ベースのスキルで、`clean` または `suspicious` のスキャン結果があるものは、`public-github` ハンドオフ記述子としてエクスポートされます。
