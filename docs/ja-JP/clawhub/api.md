...
x-i18n:
    generated_at: "2026-05-11T20:22:40Z"
    model: gpt-5.5
    provider: openai
...
ClawHub の公開読み取り API の上に、サードパーティのカタログ、ディレクトリ、検索サーフェスを構築できます。公開スキルメタデータとスキルファイルは ClawHub のスキルライセンスルールに基づいて公開されますが、API 自体にはレート制限があり、責任を持って利用する必要があります。

ガイドライン:

- カタログ一覧には、`GET /api/v1/skills`、`GET /api/v1/search`、`GET /api/v1/skills/{slug}` などの公開読み取りエンドポイントを使用してください。
- 積極的にポーリングするのではなく、レスポンスをキャッシュし、`429`、`Retry-After`、レート制限ヘッダーを尊重してください。
- 一覧を表示するときは、ユーザーがソースレジストリレコードを確認できるように、正規の ClawHub スキル URL にリンクしてください。
- `https://clawhub.ai/<owner>/<slug>` 形式の正規ページ URL を使用してください。
...
- `GET /api/v1/skills?limit=&cursor=&sort=`
  ...
  - レガシーエイリアス: `nonSuspicious=true`
  - `nonSuspiciousOnly=true` の場合、カーソルベースのページには `limit` より少ないアイテムしか含まれないことがあります。続行するには `nextCursor` を使用してください。
...
- `POST /api/v1/users/reserve` は、オーナーハンドル用にルートスラッグと非公開のリリースなしパッケージプレースホルダーを予約します。
