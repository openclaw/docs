...
x-i18n:
    generated_at: "2026-07-01T07:50:30Z"
    model: gpt-5.5
...
ClawHub の公開読み取り API の上に、サードパーティのカタログ、ディレクトリ、検索画面を構築できます。公開スキルメタデータとスキルファイルは ClawHub のスキルライセンスルールに基づいて公開されます。一方、API 自体にはレート制限があり、責任を持って利用する必要があります。
...
- `GET /api/v1/skills?limit=&cursor=&sort=`
  ...
  - レガシーエイリアス: `nonSuspicious=true`
  - `nonSuspiciousOnly=true` の場合、カーソルベースのページに含まれるアイテム数が `limit` より少ないことがあります。続行するには `nextCursor` を使用します。
...
- `GET /api/v1/download?slug=&version=&tag=`
  ...
  - 現在の GitHub ベースのスキルで、`clean` または `suspicious` のスキャンがあるものは、ClawHub バイトの代わりに
    JSON の `public-github` ハンドオフ記述子を返します。
- `GET /api/v1/skills/export?startDate=&endDate=&limit=&cursor=`
  - ホストされたスキルは保存済みファイルとしてエクスポートされます。
  - 現在の GitHub ベースのスキルで、`clean` または `suspicious` のスキャンがあるものは、
...
- `POST /api/v1/users/reserve` は、所有者ハンドルのルート slug とプライベートな未リリースパッケージプレースホルダーを予約します。
