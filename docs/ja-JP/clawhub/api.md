...
x-i18n:
    generated_at: "2026-07-02T00:42:24Z"
    model: gpt-5.5
...
- ClawHub がサードパーティサイトを推奨、検証、または運営していることを示唆しないでください。
...
- `X-RateLimit-Remaining` / `RateLimit-Remaining`: 存在する場合は正確な残り予算。シャードされた成功リクエストでは、近似のグローバル値を返すのではなく省略されます
...
- `GET /api/v1/skills?limit=&cursor=&sort=`
  ...
  - レガシーエイリアス: `nonSuspicious=true`
  - `nonSuspiciousOnly=true` の場合、カーソルベースのページには `limit` 件より少ない項目しか含まれない場合があります。続行するには `nextCursor` を使用します。
...
- `GET /api/v1/download?slug=&version=&tag=`
  - ホスト型 skill は決定的な ZIP バイトを返します。
  - `clean` または `suspicious` のスキャン結果を持つ現在の GitHub バック skill は、ClawHub バイトの代わりに JSON `public-github` 引き継ぎ記述子を返します。
- `GET /api/v1/skills/export?startDate=&endDate=&limit=&cursor=`
  - ホスト型 skill は保存されたファイルとしてエクスポートされます。
  - `clean` または `suspicious` のスキャン結果を持つ現在の GitHub バック skill は、`public-github` 引き継ぎ記述子としてエクスポートされます。
