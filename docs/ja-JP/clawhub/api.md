...
ClawHub の公開読み取り API の上に、サードパーティのカタログ、ディレクトリ、検索サーフェスを構築できます。公開 skill メタデータと skill ファイルは ClawHub の skill ライセンス規則のもとで公開されます。一方、API 自体にはレート制限があり、責任を持って利用する必要があります。
...
- `POST /api/v1/users/reserve` は、オーナーハンドル用にルート slug とリリースなしの非公開パッケージプレースホルダーを予約します。
