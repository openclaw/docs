...
ClawHub の公開読み取り API の上に、サードパーティのカタログ、ディレクトリ、検索サーフェスを構築できます。公開スキルメタデータとスキルファイルは ClawHub のスキルライセンスルールに基づいて公開されます。一方で、API 自体にはレート制限があり、責任を持って利用する必要があります。
...
- ClawHub がサードパーティサイトを推奨、検証、運用していると示唆しないでください。
...
- `X-RateLimit-Remaining` / `RateLimit-Remaining`: 存在する場合は正確な残り予算。
  シャード化された成功リクエストでは、概算のグローバル値を返すのではなく、
  これを省略します
...
- v1 エラーは `400`、`401`、`403`、`404`、`429`、ブロックされたダウンロード応答を含め、
  プレーンテキスト（`text/plain; charset=utf-8`）です。
...
- `GET /api/v1/download?slug=&version=&tag=`
  ...
  - `clean` または `suspicious` スキャンを持つ現在の GitHub バックエンドのスキルは、
    ClawHub バイトの代わりに JSON の `public-github` 引き渡し記述子を返します。
