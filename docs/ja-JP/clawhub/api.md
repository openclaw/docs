...
ClawHub の公開読み取り API の上に、サードパーティのカタログ、ディレクトリ、検索サーフェスを構築できます。公開 skill メタデータと skill ファイルは ClawHub の skill ライセンス規則の下で公開されます。一方で API 自体にはレート制限があり、責任を持って利用する必要があります。
...
- 一覧を表示するときは、ユーザーがソースのレジストリレコードを確認できるように、正規の ClawHub skill URL にリンクします。
...
- `X-RateLimit-Remaining` / `RateLimit-Remaining`: 存在する場合は正確な残り予算。シャーディングされた成功リクエストでは、近似のグローバル値を返す代わりに省略されます
...
- `GET /api/v1/search?q=...`
  - オプションフィルター: `highlightedOnly=true`、`nonSuspiciousOnly=true`
...
- `GET /api/v1/skills?limit=&cursor=&sort=`
  ...
  - オプションフィルター: `nonSuspiciousOnly=true`
  - レガシーエイリアス: `nonSuspicious=true`
...
- `GET /api/v1/download?slug=&version=&tag=`
  ...
  - `clean` または `suspicious` スキャンを持つ現在の GitHub バックエンドの Skills は、ClawHub バイトの代わりに JSON `public-github` ハンドオフ記述子を返します。
- `GET /api/v1/skills/export?startDate=&endDate=&limit=&cursor=`
  - ホスト型 Skills は保存済みファイルとしてエクスポートされます。
  - `clean` または `suspicious` スキャンを持つ現在の GitHub バックエンドの Skills は、`public-github` ハンドオフ記述子としてエクスポートされます。
...
レガシーの `/api/*` と `/api/cli/*` はまだ利用できます。`DEPRECATIONS.md` を参照してください。
