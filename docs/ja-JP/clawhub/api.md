...
x-i18n:
    generated_at: "2026-07-01T10:56:25Z"
    model: gpt-5.5
...
ClawHub の公開読み取り API の上に、サードパーティのカタログ、ディレクトリ、検索サーフェスを構築できます。公開 skill メタデータと skill ファイルは ClawHub の skill ライセンス規則の下で公開されます。一方、API 自体にはレート制限があり、責任を持って利用する必要があります。
...
- 一覧を表示するときは、ユーザーがソースレジストリレコードを確認できるように、正規の ClawHub skill URL へリンクします。
...
- トークンがない、または無効な場合は IP ベースの強制適用にフォールバックします。
...
- `GET /api/v1/skills?limit=&cursor=&sort=`
  ...
  - レガシーエイリアス: `nonSuspicious=true`
  - `nonSuspiciousOnly=true` では、カーソルベースのページに `limit` 件未満の項目が含まれる場合があります。続行するには `nextCursor` を使用します。
...
- `GET /api/v1/download?slug=&version=&tag=`
  - ホストされた skill は決定論的な ZIP バイトを返します。
  - `clean` または `suspicious` のスキャンを持つ現在の GitHub バックエンドの skill は、ClawHub のバイト列の代わりに JSON の `public-github` 引き継ぎ記述子を返します。
- `GET /api/v1/skills/export?startDate=&endDate=&limit=&cursor=`
  - ホストされた skill は保存済みファイルとしてエクスポートされます。
  - `clean` または `suspicious` のスキャンを持つ現在の GitHub バックエンドの skill は、`public-github` 引き継ぎ記述子としてエクスポートされます。
...
- `POST /api/v1/users/reserve` は、所有者ハンドルのルート slug と非公開のリリースなしパッケージプレースホルダーを予約します。
