...
ClawHub の公開読み取り API の上に、サードパーティのカタログ、ディレクトリ、または検索サーフェスを構築できます。公開スキルメタデータとスキルファイルは ClawHub のスキルライセンスルールに基づいて公開されます。一方、API 自体にはレート制限があり、責任を持って利用する必要があります。
...
- `GET /api/v1/download?slug=&version=&tag=`
  ...
  - 現在の GitHub バックのスキルで、`clean` または `suspicious` のスキャン結果を持つものは、ClawHub バイトの代わりに JSON の `public-github` ハンドオフ記述子を返します。
- `GET /api/v1/skills/export?startDate=&endDate=&limit=&cursor=`
  - ホスト型スキルは保存済みファイルとしてエクスポートされます。
  - 現在の GitHub バックのスキルで、`clean` または `suspicious` のスキャン結果を持つものは、`public-github` ハンドオフ記述子としてエクスポートされます。
...
- `POST /api/v1/users/reserve` は、オーナーハンドル向けにルート slug と非公開のリリースなしパッケージプレースホルダーを予約します。
