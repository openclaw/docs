...
x-i18n:
    generated_at: "2026-07-05T11:08:43Z"
    model: gpt-5.5
...
- カタログ一覧には、`GET /api/v1/skills`、`GET /api/v1/search`、`GET /api/v1/skills/{slug}` などの公開読み取りエンドポイントを使用してください。
- 過度にポーリングするのではなく、レスポンスをキャッシュし、`429`、`Retry-After`、レート制限ヘッダーを尊重してください。
- 一覧を表示するときは、ユーザーがソースのレジストリレコードを確認できるように、正規の ClawHub スキル URL にリンクしてください。
- `https://clawhub.ai/<owner>/skills/<slug>` の形式の正規ページ URL を使用してください。
...
- `X-RateLimit-Remaining` / `RateLimit-Remaining`: 存在する場合は正確な残り予算。
  シャーディングされた成功リクエストでは、近似のグローバル値を返すのではなく省略されます
...
- 存在する場合は `Retry-After` を優先してください。
- それ以外の場合は `RateLimit-Reset` を使用するか、`X-RateLimit-Reset` から遅延を導出してください。
- 再試行にジッターを追加してください。
...
- `GET /api/v1/skills?limit=&cursor=&sort=`
  ...
  - レガシーエイリアス: `nonSuspicious=true`
  - `nonSuspiciousOnly=true` の場合、カーソルベースのページには `limit` より少ない項目しか含まれない場合があります。続行するには `nextCursor` を使用してください。
...
- `GET /api/v1/download?slug=&version=&tag=`
  ...
  - `clean` または `suspicious` スキャンを持つ現在の GitHub バックエンドのスキルは、ClawHub バイトの代わりに
    JSON `public-github` ハンドオフ記述子を返します。
- `GET /api/v1/skills/export?startDate=&endDate=&limit=&cursor=`
  - ホストされたスキルは保存済みファイルとしてエクスポートされます。
  - `clean` または `suspicious` スキャンを持つ現在の GitHub バックエンドのスキルは、
    `public-github` ハンドオフ記述子としてエクスポートされます。
...
- `POST /api/v1/users/reserve` は、所有者ハンドルのルートスラッグと非公開の未リリースパッケージプレースホルダーを予約します。
