...
ClawHub の公開読み取り API を基盤にして、サードパーティのカタログ、ディレクトリ、検索サーフェスを構築できます。公開スキルメタデータとスキルファイルは ClawHub のスキルライセンス規則に基づいて公開されます。一方で、API 自体にはレート制限があり、責任を持って利用する必要があります。
...
- 匿名リクエスト: IP 単位。
- 認証済みリクエスト（有効な Bearer トークン）: ユーザーバケット単位。
- トークンがない、または無効な場合は IP 単位の適用にフォールバックします。

- 読み取り: IP あたり 3000/分、キーあたり 12000/分
- 書き込み: IP あたり 300/分、キーあたり 3000/分
- ダウンロード: IP あたり 1200/分、キーあたり 6000/分
...
- `X-RateLimit-Remaining` / `RateLimit-Remaining`: 存在する場合は正確な残り予算。
  シャード化された成功リクエストでは、概算のグローバル値を返すのではなく省略されます
...
- `GET /api/v1/download?slug=&version=&tag=`
  - ホストされたスキルは決定的な ZIP バイト列を返します。
  - 現在の GitHub バックのスキルで `clean` または `suspicious` のスキャン結果があるものは、ClawHub のバイト列の代わりに JSON の `public-github` ハンドオフ記述子を返します。
- `GET /api/v1/skills/export?startDate=&endDate=&limit=&cursor=`
  - ホストされたスキルは保存済みファイルとしてエクスポートされます。
  - 現在の GitHub バックのスキルで `clean` または `suspicious` のスキャン結果があるものは、`public-github` ハンドオフ記述子としてエクスポートされます。
