...
- 過度なポーリングではなく、レスポンスをキャッシュし、`429`、`Retry-After`、レート制限ヘッダーを尊重します。
...
- `GET /api/v1/download?slug=&version=&tag=`
  ...
  - `clean` または `suspicious` のスキャンを持つ現在の GitHub バック skill は、ClawHub のバイトの代わりに JSON の `public-github` 引き継ぎ記述子を返します。
- `GET /api/v1/skills/export?startDate=&endDate=&limit=&cursor=`
  - ホストされた skill は保存済みファイルとしてエクスポートされます。
  - `clean` または `suspicious` のスキャンを持つ現在の GitHub バック skill は、`public-github` 引き継ぎ記述子としてエクスポートされます。
...
レガシー `/api/*` と `/api/cli/*` は引き続き利用できます。`DEPRECATIONS.md` を参照してください。
