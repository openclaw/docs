...
ClawHub の公開読み取り API の上に、サードパーティのカタログ、ディレクトリ、検索面を構築できます。公開 skill メタデータと skill ファイルは ClawHub の skill ライセンスルールの下で公開されます。一方、API 自体にはレート制限があり、責任を持って利用する必要があります。
...
- `POST /api/v1/users/reserve` は、所有者ハンドル向けにルート slug と非公開のリリースなし package プレースホルダーを予約します。
