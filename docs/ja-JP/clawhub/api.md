...
- 一覧を表示する際は、ユーザーがソースのレジストリレコードを確認できるように、正規の ClawHub スキル URL にリンクします。
...
- トークンがない、または無効な場合は IP による適用に戻ります。
...
- `GET /api/v1/download?slug=&version=&tag=`
  ...
  - `clean` または `suspicious` のスキャンを持つ現在の GitHub-backed スキルは、ClawHub バイトの代わりに JSON の `public-github` 引き渡し記述子を返します。
- `GET /api/v1/skills/export?startDate=&endDate=&limit=&cursor=`
  - ホスト型スキルは保存済みファイルとしてエクスポートされます。
  - `clean` または `suspicious` のスキャンを持つ現在の GitHub-backed スキルは、`public-github` 引き渡し記述子としてエクスポートされます。
...
- `POST /api/v1/users/reserve` は、所有者ハンドル用にルート slugs と非公開の未リリースパッケージプレースホルダーを予約します。
