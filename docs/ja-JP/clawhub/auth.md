...
サインイン後にログアウト状態に戻される場合、アカウントの状態が良好でない可能性があります。
...
ファイアウォール、VPN、またはプロキシのルールによってブラウザーがローカルコールバックに到達できない場合は、ヘッドレスのトークンフローを使用します。
...
CLI は 1 回限りのコードを表示し、`https://clawhub.ai/cli/device` で承認されるまで待機します。
