...
x-i18n:
    generated_at: "2026-07-05T05:04:20Z"
    model: gpt-5.5
...
ClawHub は Web サインインに GitHub を使用します。CLI は、サインイン済みアカウントを通じて作成された ClawHub API トークンを使用します。

## Web サインイン

[clawhub.ai](https://clawhub.ai) で GitHub を使用してサインインします。

削除、禁止、または無効化されたアカウントでは、通常の ClawHub サインインを完了できません。サインイン後にログアウト状態へ戻される場合、アカウントの状態が良好でない可能性があります。アカウントが禁止または無効化された場合で、これが誤りだと思う場合は、[ClawHub appeal form](https://appeals.openclaw.ai/) を使用してください。
...
CLI はワンタイムコードを表示し、`https://clawhub.ai/cli/device` で認可が行われるまで待機します。
...
削除、禁止、または無効化されたアカウントでは、既存の API トークンを使い続けることはできません。アカウントが禁止または無効化された場合で、これが誤りだと思う場合は、[ClawHub appeal form](https://appeals.openclaw.ai/) を使用してください。
