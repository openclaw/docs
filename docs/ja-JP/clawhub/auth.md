...
ClawHub はウェブサインインに GitHub を使用します。CLI は、サインイン済みアカウントを通じて作成された ClawHub API トークンを使用します。

## ウェブサインイン

[clawhub.ai](https://clawhub.ai) で GitHub を使用してサインインします。

削除、禁止、または無効化されたアカウントでは、通常の ClawHub サインインを完了できません。サインイン後にログアウト状態へ戻される場合、アカウントの状態が良好でない可能性があります。
...
1. CLI は `127.0.0.1` 上で一時的なコールバックサーバーを起動します。
...
ファイアウォール、VPN、またはプロキシのルールによりブラウザーがローカルコールバックに到達できない場合は、ヘッドレスのトークンフローを使用します。
...
CLI は一度限りのコードを表示し、`https://clawhub.ai/cli/device` で承認されるまで待機します。
