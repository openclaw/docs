...
read_when:
    ...
    - ClawHub CLI の使用方法
    - 401 エラーのデバッグ
summary: ClawHubへのサインイン、APIトークン、CLIログイン、トークンの保存、および失効。
x-i18n:
    generated_at: "2026-07-12T21:28:39Z"
    model: gpt-5.6
...
削除、利用禁止、または無効化されたアカウントでは、通常の ClawHub サインインを完了できません。サインイン後にログアウト状態へ戻る場合、アカウントが良好な状態ではない可能性があります。アカウントが利用禁止または無効化されており、それが誤りだと思われる場合は、[ClawHub 異議申し立てフォーム](https://appeals.openclaw.ai/)を使用してください。
...
CLI はワンタイムコードを表示し、`https://clawhub.ai/cli/device` で認証が行われるまで待機します。
...
CI のセットアップ用に、保存されたトークンを次のコマンドで出力します。
...
失効済み、無効、または存在しないトークンに対しては、`401 Unauthorized` が返されます。`clawhub login` で再度サインインするか、`clawhub login --token` で新しいトークンを指定してください。
