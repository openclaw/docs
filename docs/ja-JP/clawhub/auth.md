...
削除、BAN、または無効化されたアカウントでは、通常の ClawHub サインインを完了できません。サインイン後にログアウト状態へ戻される場合、アカウントの状態に問題がある可能性があります。アカウントが BAN または無効化された場合、誤りだと思われる場合は [ClawHub 異議申し立てフォーム](https://appeals.openclaw.ai/) を使用してください。
...
1. CLI が `127.0.0.1` 上で一時的なコールバックサーバーを起動します。
...
ファイアウォール、VPN、またはプロキシルールのためにブラウザーがローカルコールバックへ到達できない場合は、ヘッドレスのトークンフローを使用してください。
...
失効済み、無効、または欠落したトークンは `401 Unauthorized` を返します。`clawhub login` で再度サインインするか、`clawhub login --token` で新しいトークンを指定してください。

削除、BAN、または無効化されたアカウントは、既存の API トークンを使用し続けることはできません。アカウントが BAN または無効化された場合、誤りだと思われる場合は [ClawHub 異議申し立てフォーム](https://appeals.openclaw.ai/) を使用してください。
