...
x-i18n:
    generated_at: "2026-07-04T03:34:45Z"
    model: gpt-5.5
...
削除、BAN、または無効化されたアカウントでは、通常の ClawHub サインインを完了できません。
サインイン後にログアウト状態へ戻される場合、アカウントが良好な状態ではない可能性があります。アカウントが BAN または無効化された場合、誤りだと思うときは
...
ファイアウォール、VPN、またはプロキシのルールが原因でブラウザーがローカルコールバックに到達できない場合は、ヘッドレスのトークンフローを使用してください。
...
削除、BAN、または無効化されたアカウントでは、既存の API トークンを継続して使用できません。
アカウントが BAN または無効化された場合、誤りだと思うときは
[ClawHub 異議申し立てフォーム](https://appeals.openclaw.ai/) を使用してください。
