...
x-i18n:
    generated_at: "2026-07-05T20:18:14Z"
    model: gpt-5.5
...
スキルの場合は、`SKILL.md` のフロントマターを確認してください。必要な環境変数と
ツールを宣言し、ユーザーとスキャナーがパッケージを理解できるようにする必要があります。
...
スラッグ、またはパッケージ名がすでに取得済みまたは予約済みであるために公開が失敗した場合は、まず
その名前空間に一致するオーナーで公開していることを確認してください。Plugin パッケージでは、
`@example-org/example-plugin` のようなスコープ付き名は、一致する
`example-org` オーナーとして公開する必要があります。

自分の組織、プロジェクト、またはブランドが正当な名前空間オーナーだと考えているが、
現在の ClawHub オーナーを管理できない場合は、公開可能で機密性のない証拠を添えて
...
- パッケージが非表示、保留中、またはブロック済みの場合、オーナーが問題を解決するまで
  インストールできないことがあります。

## 公開 API リクエストが失敗する

- `429` の再試行ヘッダーに従い、公開一覧/検索レスポンスをキャッシュしてください。
- ユーザーを正規の ClawHub 一覧へ戻すリンクを案内してください。
- 非表示、プライベート、保留中、またはモデレーションでブロックされたコンテンツを
