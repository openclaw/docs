...
多数のユーザーが 1 つの送信元 IP を共有している場合、各ユーザーが少数のリクエストしか送信していなくても、匿名 IP 制限に達することがあります。可能な場合はサインインし、報告された遅延の後に再試行してください。
...
所有者に表示される診断情報に、スキャン、アップロードゲート、またはモデレーション状態の説明が含まれる場合があります。
...
- `429` の再試行ヘッダーを尊重し、公開リストや検索のレスポンスをキャッシュしてください。
- ユーザーを正規の ClawHub リストへリンクしてください。
- 非表示、非公開、保留、またはモデレーションでブロックされたコンテンツを、パブリック API サーフェスの外部にミラーしないでください。
