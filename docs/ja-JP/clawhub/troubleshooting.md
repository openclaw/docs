...
- 正確な slug または owner ページが分かっている場合は確認してください。
...
Skills の場合は、`SKILL.md` の frontmatter を確認してください。必須環境変数と
ツールは、ユーザーとスキャナーがパッケージを理解できるように宣言する必要があります。

plugins の場合は、`package.json` の互換性メタデータを確認してください。code-plugin の publish には、
...
## GitHub owner またはソースのエラーで publish が失敗する
...
slug、または package name がすでに取得済みまたは予約済みで publish が失敗する場合は、まず
namespace に一致する owner で公開していることを確認してください。Plugin パッケージの場合、
`@example-org/example-plugin` のような scoped name は、一致する
`example-org` owner として公開する必要があります。

自分の org、プロジェクト、またはブランドが正当な namespace owner だと考えているが、
...
を作成してください。証拠の指針と公開 issue に含めない内容については、
...
何が publish されるか不明な場合は、まずプレビューします。
...
- 編集済みコピーを新しい slug または fork として publish する。
...
- ユーザーを正規の ClawHub リストにリンクしてください。
- 非表示、private、保留、またはモデレーションでブロックされたコンテンツを、
