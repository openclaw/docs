...
read_when:
    ...
    - ClawHub への公開
summary: 'ClawHub の使用を開始します: Skills または plugins を検索、インストール、更新、公開します。'
x-i18n:
...
OpenClaw はその skill の取得元を記録するため、以降の更新でも ClawHub 経由で解決し続けられます。
...
このコマンドは変更されていない内容をスキップします。新しい skills は `1.0.0` から始まり、その後の変更では次のパッチバージョンが自動的に公開されます。プレビューするには `--dry-run` を、明示的なバージョンを選ぶには `--version` を使用します。

公開前に、`SKILL.md` のメタデータを確認してください。必要な環境変数、ツール、権限を宣言して、ユーザーがインストール前にその skill が何を必要とするかを理解できるようにします。[skill 形式](/ja-JP/clawhub/skill-format)を参照してください。
...
コード plugins では、`package.json` に OpenClaw 互換性メタデータを含める必要があります。これには `openclaw.compat.pluginApi` と `openclaw.build.openclawVersion` が含まれます。
