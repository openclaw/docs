...
x-i18n:
    generated_at: "2026-07-03T17:12:56Z"
    model: gpt-5.5
...
OpenClaw はスキルの取得元を記録するため、後の更新でも ClawHub 経由で解決し続けられます。
...
このコマンドは変更のないコンテンツをスキップします。新しいスキルは `1.0.0` から始まり、以降の変更では次のパッチバージョンが自動的に公開されます。プレビューするには `--dry-run` を、明示的なバージョンを選択するには `--version` を使用します。
