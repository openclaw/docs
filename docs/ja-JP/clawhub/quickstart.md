...
read_when:
    ...
    - ClawHubへの公開
summary: 'ClawHub の使用を開始する: スキルまたはプラグインを検索、インストール、更新、公開します。'
x-i18n:
    generated_at: "2026-07-03T13:15:20Z"
    model: gpt-5.5
...
このコマンドは変更されていない内容をスキップします。新しいスキルは `1.0.0` から始まり、以後の変更では次のパッチバージョンが自動的に公開されます。プレビューには `--dry-run` を使用し、明示的なバージョンを選ぶには `--version` を使用します。
