...
x-i18n:
    generated_at: "2026-07-03T00:51:40Z"
    model: gpt-5.5
...
OpenClaw はスキルの取得元を記録するため、以後の更新でも ClawHub 経由で解決を続けられます。
...
このコマンドは変更のない内容をスキップします。新しいスキルは `1.0.0` から始まり、その後の変更では次のパッチバージョンが自動的に公開されます。プレビューするには `--dry-run` を、明示的なバージョンを選ぶには `--version` を使用します。
...
公開リスティングには最新のスキャン状態が表示されます。モデレーションによって保留またはブロックされているリリースは、解決されるまで検索およびインストールの画面に表示されない場合があります。
