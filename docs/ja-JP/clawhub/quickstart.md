...
read_when:
    ...
    - ClawHub への公開
summary: 'ClawHub を使い始める: Skills やプラグインを検索、インストール、更新、公開します。'
x-i18n:
...
OpenClaw はその Skill の取得元を記録するため、後続の更新でも ClawHub 経由で解決を継続できます。
...
このコマンドは変更されていないコンテンツをスキップします。新しい Skills は `1.0.0` から始まり、その後の変更では次のパッチバージョンが自動的に公開されます。プレビューするには `--dry-run` を使用し、明示的なバージョンを選ぶには `--version` を使用します。

公開する前に、`SKILL.md` のメタデータを確認してください。ユーザーがインストール前に Skill が必要とするものを理解できるよう、必要な環境変数、ツール、権限を宣言します。[Skill 形式](/ja-JP/clawhub/skill-format)を参照してください。
