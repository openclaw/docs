...
read_when:
    ...
    - ClawHub への公開
summary: 'ClawHub の使用を開始: Skills または Plugin を検索、インストール、更新、公開します。'
x-i18n:
...
OpenClaw にものをインストールするときは OpenClaw を使用します。サインイン、公開、自分の掲載の管理、またはレジストリ固有のワークフローを使用するときは、`clawhub` CLI を使用します。
...
OpenClaw はスキルの取得元を記録するため、後続の更新でも ClawHub 経由での解決を継続できます。
...
公開掲載には最新のスキャン状態が表示されます。モデレーションによって保留またはブロックされているリリースは、解決されるまで検索およびインストール面から非表示になる場合があります。
