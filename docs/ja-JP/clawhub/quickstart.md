...
read_when:
    ...
    - ClawHub への公開
summary: ClawHub を使い始めましょう：Skills やプラグインを検索、インストール、更新、公開できます。
x-i18n:
    generated_at: "2026-07-12T21:28:59Z"
    model: gpt-5.6
...
OpenClaw は Skill の取得元を記録するため、以後の更新でも引き続き ClawHub 経由で解決できます。
...
OpenClaw で npm や別のソースではなく ClawHub 経由でパッケージを解決する場合は、`clawhub:` プレフィックスを使用します。
...
このコマンドは、変更されていないコンテンツをスキップします。新しい Skills は `1.0.0` から始まり、その後の変更では次のパッチバージョンが自動的に公開されます。プレビューするには `--dry-run`、明示的なバージョンを指定するには `--version` を使用します。

公開前に、`SKILL.md` のメタデータを確認してください。必要な環境変数、ツール、権限を宣言し、インストール前に Skill の要件をユーザーが把握できるようにします。[Skill の形式](/ja-JP/clawhub/skill-format)を参照してください。
...
最初に `--dry-run` を使用して、解決されたパッケージメタデータ、互換性フィールド、ソース帰属情報、アップロード計画を、公開せずにプレビューします。
...
公開されている掲載情報には、最新のスキャン状態が表示されます。モデレーションにより保留またはブロックされているリリースは、問題が解決するまで検索およびインストール画面に表示されない場合があります。
