...
read_when:
    ...
    - ClawHub への公開
summary: 'ClawHub の使用を開始する: スキルやプラグインを検索、インストール、更新、公開します。'
x-i18n:
    generated_at: "2026-07-05T04:49:11Z"
    model: gpt-5.5
...
ClawHub は OpenClaw Skills と plugins のレジストリです。
...
OpenClaw は skill の入手元を記録するため、以後の更新でも ClawHub 経由で解決し続けられます。
...
このコマンドは変更のない内容をスキップします。新しい skills は `1.0.0` から始まり、以後の変更では次のパッチバージョンが自動的に公開されます。プレビューするには `--dry-run` を、明示的なバージョンを選ぶには `--version` を使用します。

公開前に、`SKILL.md` のメタデータを確認します。必要な環境変数、ツール、権限を宣言して、ユーザーがインストール前に skill に必要なものを理解できるようにします。[skill 形式](/ja-JP/clawhub/skill-format) を参照してください。
...
コード plugins では、`package.json` に `openclaw.compat.pluginApi` と `openclaw.build.openclawVersion` を含む OpenClaw 互換性メタデータを含める必要があります。
...
公開リスティングには最新のスキャン状態が表示されます。モデレーションによって保留またはブロックされているリリースは、解決されるまで検索やインストール画面から非表示になる場合があります。
