...
ClawHub は OpenClaw Skills とプラグインのレジストリです。
...
OpenClaw は Skills の取得元を記録するため、以後の更新でも ClawHub 経由で解決を継続できます。
...
明示的な ClawHub ソースを使って、ClawHub でホストされている Plugin をインストールします。
...
公開する前に、`SKILL.md` のメタデータを確認します。ユーザーがインストール前に Skills に必要なものを理解できるように、必要な環境変数、ツール、権限を宣言します。[Skills 形式](/ja-JP/clawhub/skill-format)を参照してください。
...
公開せずに、解決されたパッケージメタデータ、互換性フィールド、ソース属性、アップロード計画をプレビューするには、まず `--dry-run` を使用します。
