...
read_when:
    ...
    - ClawHub への公開
summary: 'ClawHub の使用を開始します: Skills または Plugin を検索、インストール、更新、公開します。'
x-i18n:
...
OpenClaw はスキルの取得元を記録するため、後の更新でも ClawHub 経由で解決を続けられます。
...
npm や別のソースではなく ClawHub 経由でパッケージを解決したい場合は、`clawhub:` プレフィックスを使用します。
...
このコマンドは変更のないコンテンツをスキップします。新しいスキルは `1.0.0` から開始し、以降の変更では次のパッチバージョンが自動的に公開されます。プレビューするには `--dry-run` を、明示的なバージョンを選択するには `--version` を使用します。
...
公開せずに解決済みのパッケージメタデータ、互換性フィールド、ソース帰属、アップロード計画をプレビューするには、まず `--dry-run` を使用します。
...
公開リスティングには最新のスキャン状態が表示されます。モデレーションにより保留またはブロックされているリリースは、解決されるまで検索とインストールの画面で非表示になる場合があります。
