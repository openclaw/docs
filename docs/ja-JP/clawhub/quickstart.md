...
ClawHub がホストするプラグインを明示的な ClawHub ソースでインストールします。
...
このコマンドは変更されていないコンテンツをスキップします。新しいスキルは `1.0.0` から始まり、以降の変更では次のパッチバージョンが自動的に公開されます。プレビューするには `--dry-run` を、明示的なバージョンを選ぶには `--version` を使用します。
...
公開掲載情報には最新のスキャン状態が表示されます。モデレーションによって保留またはブロックされたリリースは、解決されるまで検索およびインストール画面から非表示になる場合があります。
