...
ClawHub は OpenClaw のスキルと Plugin のレジストリ層です。ユーザーには
...
メタデータは重要です。ユーザーがスキルをインストールするか判断しやすくなり、
...
OpenClaw が ClawHub から Plugin をインストールするとき、インストール前に公表された互換性
...
アップロード前に解決済みペイロードをプレビューするには、ドライランを使用します。公開ページには、その後
...
レジストリパッケージを解決して更新できます。ClawHub CLI は、完全な OpenClaw ワークスペース外で
...
自動チェック、ユーザーレポート、モデレーターの対応が適用されます。

利用可能な場合、公開ページにはスキャン概要が表示されます。保留、非表示、
またはブロックされたコンテンツは、診断や異議申し立てのために所有者には
表示されたまま、公開検索やインストールフローから消えることがあります。
...
ダウンロード向けの公開読み取り API を提供します。サードパーティのカタログは、正規の ClawHub リスティングへリンクし、
