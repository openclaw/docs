...
x-i18n:
    generated_at: "2026-07-04T10:26:18Z"
    model: gpt-5.5
...
サードパーティのディレクトリは、公開読み取りエンドポイントを使用して ClawHub skills を一覧表示または検索できます。結果をキャッシュし、`429`/`Retry-After` を尊重し、ユーザーを正規の ClawHub 掲載ページ（`https://clawhub.ai/<owner>/skills/<slug>`）へリンクし、ClawHub がそのサードパーティサイトを推奨していると示唆しないでください。公開 API サーフェスの外にある、非表示、非公開、またはモデレーションでブロックされたコンテンツをミラーしようとしないでください。
...
- 認証済みの書き込みエンドポイントは、サーバーが理由を把握している場合、単独の `Unauthorized` を返すべきではありません。トークンの欠落、無効または取り消されたトークン、削除済み、禁止済み、無効化済みのアカウントには、それぞれ CLI クライアントがユーザーに何がブロック要因だったかを伝えられるよう、対処可能なテキストを返すべきです。
...
- `X-RateLimit-Reset`: Unix epoch 秒の絶対値
...
これには、バリデーション失敗（`400`）、公開リソースの欠落（`404`）、認証および権限エラー（`401`/`403`）、レート制限（`429`）、ブロックされたダウンロードが含まれます。クライアントはレスポンス本文を人間が読める文字列として読み取るべきです。不明なクエリパラメーターは互換性のために無視されますが、認識されるクエリパラメーターに無効な値がある場合は `400` を返します。
...
- 結果は関連度順（埋め込み類似度 + 完全一致の slug/名前トークンのブースト + 小さな人気度事前値）で返されます。
- 関連度は人気度よりも強く扱われます。正確な slug または表示名トークンの一致は、エンゲージメントがはるかに強い緩い一致より上位になる場合があります。
...
- 人気度はログスケール化され、上限が設定されます。高エンゲージメントの skills でも、クエリテキストとの一致が弱い場合は順位が下がることがあります。
- 疑わしい、または非表示のモデレーション状態は、呼び出し元のフィルターと現在のモデレーション状態に応じて、公開検索から skill を除外することがあります。
...
- リネームエイリアスは、古い URL とレジストリ経由で解決されるインストールの解決を維持しますが、検索順位はリネームがインデックス化された後の正規 skill メタデータに基づきます。既存の統計は skill に残ります。
- skill が予期せず見えない場合は、順位関連メタデータを変更する前に、ログインした状態で `clawhub inspect @owner/slug` を使用して、まずモデレーション状態を確認してください。
...
- オーナーのリネームまたはマージフローで作成された古い slug は、正規 skill に解決されます。
...
skill 報告取り込み用のモデレーターまたは管理者エンドポイントです。
...
skill 報告の解決または再オープン用のモデレーターまたは管理者エンドポイントです。
...
- 正規化済みの検証ステータスに加えて、スキャナー固有の詳細を含みます。
...
- キュー内では `queue.queuedAhead` と `queue.position` を返すため、クライアントはリクエストより前にある優先手動スキャンの数を表示できます。非常に大きなキューは上限が適用され、`queuedAheadIsEstimate: true` として報告されます。
...
- 成功済みスキャンが必要です。非終端のスキャンは `409` を返します。
...
- `ok` は、選択されたバージョンに生成済み Skill Card があり、モデレーションによってマルウェアとしてブロックされておらず、ClawScan 検証が clean の場合にのみ `true` です。
- シェル自動化がネストされたラッパーを展開せずに読み取れるように、スキル ID、発行者 ID、選択されたバージョンメタデータはトップレベルのエンベロープフィールド（`slug`、`displayName`、`publisherHandle`、`version`、`resolvedFrom`、`tag`、`createdAt`）です。
...
- Skill エントリーは引き続き Skill レジストリをバックエンドとし、`POST /api/v1/skills` からのみ公開できます。
...
- `category` (任意): Plugin カテゴリーフィルター。リクエストが
  Plugin パッケージにスコープされている場合にのみサポートされます。管理対象カテゴリーとレガシー v1
...
コード Plugin とバンドル Plugin パッケージを横断する Plugin 専用カタログ閲覧です。
...
- `startDate` (必須): Skill の `updatedAt` の下限を表す Unix ミリ秒。
- `endDate` (必須): Skill の `updatedAt` の上限を表す Unix ミリ秒。
...
- ホストされた Skills には、保存済みの最新バージョンのファイルが含まれ、
  `_manifest.json` に `sourceRef: "public-clawhub"` として一覧表示されます。
- `clean` または `suspicious` のスキャンを持つ現在の GitHub バックエンドの Skills には、
  `_source_handoff.json` が含まれ、`sourceRef: "public-github"`、リポジトリ、コミット、パス、
  内容ハッシュ、アーカイブ URL が記載されます。ClawHub ホストのソースファイルは含まれません。
...
- `GET /api/v1/plugins` でドキュメント化されているレガシー v1 フィルターエイリアスも
  受け付けられます。
...
- Private パッケージは、呼び出し元が所有 Publisher を読み取れる場合を除き `404` を返します。
...
- パッケージ所有者、org Publisher の所有者/管理者、Platform Moderator、または Platform Admin の API トークンが必要です。
...
これは、解決済みリリースをインストールできるかどうかを判断するための、公開 OpenClaw 消費サーフェスです。

認証:

- Public read エンドポイントです。所有者、Publisher、Moderator、または Admin のトークンは
  必要ありません。
...
- `trust.stale` は、信頼要約が古い入力から計算されており、高信頼度の許可判断の前に
...
- Private パッケージは、呼び出し元が所有 Publisher を読み取れる場合を除き `404` を返します。
- このエンドポイントは、所有者/Moderator のモデレーションエンドポイントより意図的に狭いものです。
  インストール判断と公開説明を公開し、Reporter の身元、Report 本文、Private 証拠、内部レビュータイムラインは
  公開しません。
...
将来の OpenClaw 消費向けに計算された readiness を返します。
...
公式 OpenClaw Plugin 移行行を一覧表示するための Moderator エンドポイントです。

認証:

- Moderator または Admin ユーザーの API トークンが必要です。
...
公式 Plugin 移行行を作成または更新するための Admin エンドポイントです。
...
- `packageName` は npm 名として正規化されます。計画済みの移行では、パッケージが存在しない場合があります。
- これは移行 readiness のみを追跡します。OpenClaw を変更したり ClawPack を生成したりしません。

### `GET /api/v1/packages/moderation/queue`

パッケージリリースレビューキュー用の Moderator/Admin エンドポイントです。
...
- `all`: 手動上書き、clean ではないスキャン状態、またはパッケージ Report がある任意のリリース。
...
Moderator レビュー向けにパッケージを報告します。Report はパッケージレベルで、任意でバージョンに
リンクできます。これらはモデレーションキューに送られますが、それ自体ではダウンロードを自動的に非表示にしたり
ブロックしたりしません。Moderator は、アーティファクトを承認、隔離、または取り消すためにリリースモデレーションを
使用するべきです。
...
隔離または取り消し済みのリリースでは、成果物ダウンロードルートから `403` が返されます。
...
ClawPack で裏付けられたパッケージバージョンの npm 互換 packument を返します。
...
- スコープ付きパッケージ packument は、`/api/npm/@scope/name` と npm の
...
ホストされたスキルバージョンの ZIP をダウンロードします。または、`clean` または `suspicious` スキャンを持ち、ホストされた
バージョンを持たない現在の GitHub 裏付けスキルについて、GitHub ソース引き渡しを返します。
...
- GitHub 裏付けスキルの引き渡しでは、バイト列のプロキシやミラーは行いません。JSON レスポンスには
...
- 任意の payload フィールド: `ownerHandle`。存在する場合、API はサーバー側でその
  パブリッシャーを解決し、アクターにパブリッシャーアクセス権を要求します。
- 任意の payload フィールド: `migrateOwner`。`ownerHandle` とともに `true` の場合、
  アクターが現在および移行先の両方のパブリッシャーで管理者/所有者であれば、既存のスキルをその所有者へ移動できます。
...
- 許可されるフォームフィールドは、`payload`、繰り返しの `files` blob、または 1 つの `clawpack`
  tarball 参照です。`clawpack` は `.tgz` blob、または upload-url フローが返す storage id にできます。
...
- 任意の payload フィールド: `ownerHandle`。存在する場合、その所有者に代わって公開できるのは管理者のみです。
...
所有者による論理削除では slug が 30 日間予約され、その後は別のパブリッシャーが
...
新規作成された org には `memberHandle` を指定します。操作する管理者はメンバーとして追加されません。
...
- ハンドルがすでにパブリッシャー、ユーザー、または個人パブリッシャーで使用されている場合は `409` を返します。
...
個人パブリッシャーを復旧します。リクエストでは両方の不変な GitHub
プロバイダーアカウント ID を指定する必要があります。変更可能なハンドルは、オペレーター向けのガードとしてのみ使用されます。

エンドポイントの既定はドライランです。リカバリーを適用するには、スタッフが両方の
GitHub プリンシパル間の継続性を独立して確認した後で、`dryRun: false` と
`confirmIdentityVerified: true` が必要です。宛先ユーザーの現在の個人
パブリッシャーに Skills、パッケージ、または GitHub skill ソースがある場合、
リカバリーはフェイルクローズします。
リカバリーでは、復旧対象パブリッシャーの Skills、skill スラッグエイリアス、パッケージ、パッケージインスペクター警告、派生検索ダイジェスト行のレガシー `ownerUserId` フィールドも移行され、
直接所有者パスが新しいパブリッシャー権限と一致するようにします。復旧対象ハンドルのアクティブな保護ハンドル
予約も置換先ユーザーに再割り当てされるため、後続の
プロフィール同期で以前のユーザーの競合する権限が復元されることはありません。各プライマリテーブルは
適用トランザクションごとに 100 行に制限されます。より大きなリカバリーでは、先に再開可能な所有者移行を使用する必要があります。
GitHub skill ソースはパブリッシャースコープであり、書き換えではなく確認済みとして報告されます。
...
- `merge` はソースの一覧を非表示にし、ソーススラッグをターゲットの一覧へリダイレクトします。
...
（管理者のみ）。`dryRun` が `false` でない限り、既定ではドライランになります。
