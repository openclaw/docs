...
x-i18n:
    generated_at: "2026-07-01T12:47:26Z"
    model: gpt-5.5
...
サードパーティのディレクトリは、公開読み取りエンドポイントを使用して ClawHub Skills を一覧表示または検索できます。結果をキャッシュし、`429`/`Retry-After` を尊重し、ユーザーを正規の ClawHub 掲載ページ（`https://clawhub.ai/<owner>/skills/<slug>`）へリンクし、ClawHub がサードパーティサイトを推奨しているように示唆しないでください。公開 API surface の外で、非表示、非公開、またはモデレーションでブロックされたコンテンツをミラーしようとしないでください。

Web slug ショートカットは registry ファミリーをまたいで解決されますが、API クライアントは route の優先順位を再構築するのではなく、読み取りエンドポイントから返される正規 URL を使用する必要があります。
...
- 認証済みリクエスト（有効な Bearer token）: ユーザー bucket ごとに適用されます。
- token が欠落しているか無効な場合、動作は IP 適用にフォールバックします。
- 認証済みの書き込みエンドポイントは、サーバーが理由を把握している場合に、単なる `Unauthorized` を返すべきではありません。欠落した token、無効または取り消された token、削除済み、ban 済み、無効化済みのアカウントには、それぞれ CLI クライアントがユーザーに何がブロックしたかを伝えられるよう、対応可能なテキストを返す必要があります。
...
- `429` の場合: `X-RateLimit-Remaining: 0` and `RateLimit-Remaining: 0`
- `429` の場合: `Retry-After`

ヘッダーの意味:

- `X-RateLimit-Reset`: 絶対 Unix epoch 秒
- `RateLimit-Reset`: reset までの秒数（delay）
- `X-RateLimit-Remaining` / `RateLimit-Remaining`: 存在する場合は正確な残り budget。
  sharded な成功リクエストでは、近似の global 値を返す代わりにこのヘッダーを省略します。
- `Retry-After`: `429` で retry する前に待つ秒数（delay）
...
- `Retry-After` が存在する場合、retry する前にその秒数だけ待ちます。
- 同期された retry を避けるため、jitter 付き backoff を使用します。
- `Retry-After` がない場合は、`RateLimit-Reset` にフォールバックします（または `X-RateLimit-Reset` から計算します）。

IP source:

- deployment が trusted forwarded headers を明示的に有効にしている場合にのみ、`cf-connecting-ip` を含む trusted client IP headers を使用します。
- ClawHub は edge で client IP を識別するために trusted forwarding headers を使用します。
- trusted client IP が利用できない場合、匿名リクエストは rate-limit kind のみにスコープされた fallback buckets を使用します。これらの fallback buckets には、caller が指定した paths、slugs、package names、versions、query strings、またはその他の artifact parameters は含まれません。

## エラーレスポンス

公開 v1 エラーレスポンスは `content-type: text/plain; charset=utf-8` の plain text です。
これには、validation failures（`400`）、欠落している公開 resources（`404`）、auth and permission failures（`401`/`403`）、rate limits（`429`）、および blocked downloads が含まれます。クライアントはレスポンス body を人間が読める文字列として読み取る必要があります。不明な query parameters は互換性のために無視されますが、認識された query parameters に無効な値がある場合は `400` を返します。
...
- `q`（必須）: query string
- `limit`（任意）: integer
- `highlightedOnly`（任意）: highlighted skills に絞り込むには `true`
- `nonSuspiciousOnly`（任意）: suspicious（`flagged.suspicious`）skills を非表示にするには `true`
- `nonSuspicious`（任意）: `nonSuspiciousOnly` の legacy alias
...
- 結果は relevance 順（embedding similarity + exact slug/name token boosts + a small popularity prior）で返されます。
- relevance は popularity より強く作用します。正確な slug または display-name token の一致は、はるかに強い engagement を持つ緩い一致より上位になることがあります。
- ASCII text は word と punctuation boundaries で tokenized されます。たとえば、`personal-map` には standalone の `map` token が含まれますが、`amap-jsapi-skill` には `amap`、`jsapi`、`skill` が含まれます。そのため、`map` を検索すると `amap-jsapi-skill` より `personal-map` のほうが強い lexical match になります。
- popularity は log-scaled され、cap されます。query text の一致が弱い場合、high-engagement skills が低く rank されることがあります。
- suspicious または hidden moderation state により、caller filters と current moderation status に応じて、Skill が public search から除外されることがあります。

Publisher discoverability guidance:

- ユーザーが文字どおり検索する terms を display name、summary、tags に入れてください。standalone slug token は、それが保持したい stable identity でもある場合にのみ使用してください。
- 新しい slug が長期的によりよい正規名である場合を除き、1 つの query を追うためだけに slug を rename しないでください。古い slugs は redirect aliases になりますが、正規 URL、表示される slug、および将来の search digests では新しい slug が使用されます。
- Rename aliases は、古い URL と registry 経由で解決される installs の解決を保持しますが、search ranking は rename が indexed された後の正規 Skill metadata に基づきます。既存の stats は Skill に残ります。
- Skill が予期せず invisible になっている場合は、ranking 関連 metadata を変更する前に、ログインした状態で `clawhub inspect @owner/slug` を使用して moderation state を最初に確認してください。

### `GET /api/v1/skills`

Query params:

- `limit`（任意）: integer（1–200）
- `cursor`（任意）: `trending` 以外の sort 用 pagination cursor
- `sort`（任意）: `updated`（デフォルト）、`recommended`（alias: `default`）、`createdAt`（alias: `newest`）、`downloads`、`stars`（alias: `rating`）、legacy install aliases `installsCurrent`/`installs`/`installsAllTime` は `downloads` に map、`trending`
- `nonSuspiciousOnly`（任意）: suspicious（`flagged.suspicious`）skills を非表示にするには `true`
- `nonSuspicious`（任意）: `nonSuspiciousOnly` の legacy alias
...
- `recommended` は engagement と recency signals を使用します。
- `trending` は直近 7 日間の installs（telemetry-based）で rank します。
- `createdAt` は new-skill crawls に対して stable です。`updated` は既存 Skills が republished されると変わります。
- `nonSuspiciousOnly=true` の場合、cursor-based sorts は page retrieval 後に suspicious skills が filtered されるため、ページ上の items が `limit` より少なく返ることがあります。
- 存在する場合は `nextCursor` を使用して pagination を続行してください。short page だけでは end-of-results を意味しません。
...
- owner rename/merge flows によって作成された古い slugs は、正規 Skill に解決されます。
- `metadata.os`: Skill frontmatter で宣言された OS restrictions（例: `["macos"]`, `["linux"]`）。宣言されていない場合は `null`。
- `metadata.systems`: Nix system targets（例: `["aarch64-darwin", "x86_64-linux"]`）。宣言されていない場合は `null`。
- Skill に platform metadata がない場合、`metadata` は `null` です。
- `moderation` は、Skill が flagged されている場合、または owner が閲覧している場合にのみ含まれます。

### `GET /api/v1/skills/{slug}/moderation`

structured moderation state を返します。
...
- Owners と moderators は hidden skills の moderation details にアクセスできます。
- Public callers は、すでに flagged された visible skills に対してのみ `200` を受け取ります。
- Evidence は public callers 向けには redacted され、owners/moderators 向けにのみ raw snippets を含みます。

### `POST /api/v1/skills/{slug}/report`

moderator review のために Skill を report します。Reports は skill-level で、任意で version に linked され、Skill report queue に送られます。
...
Skill report intake 用の moderator/admin endpoint です。

Query params:

- `status`（任意）: `open`（デフォルト）、`confirmed`、`dismissed`、または `all`
- `limit`（任意）: integer（1-200）
- `cursor`（任意）: pagination cursor
...
Skill reports を解決または reopen するための moderator/admin endpoint です。
...
`note` は `confirmed` と `dismissed` では必須です。`status` を `open` に戻す場合は省略できます。triaged report と一緒に `finalAction: "hide"` を渡すと、同じ auditable workflow 内で Skill を hidden にします。
...
version metadata + files list を返します。

- `version.security` は、利用可能な場合、normalized scan verification status と scanner details（VirusTotal + LLM）を含みます。

### `GET /api/v1/skills/{slug}/scan`

Skill version の security scan verification details を返します。

Query params:

- `version`（任意）: specific version string。
- `tag`（任意）: tagged version を解決します（例: `latest`）。
...
新しい ClawScan ジョブの認証済み送信エンドポイント。
...
- スキャンジョブは非同期です。手動スキャンリクエストは通常の公開/バックフィル作業より優先されますが、完了はワーカーの可用性に依存します。

### `GET /api/v1/skills/-/scan/{scanId}`

送信済みスキャンの認証済みポーリングエンドポイント。

- キュー中/実行中/成功/失敗のステータスを返します。
- キュー中は `queue.queuedAhead` と `queue.position` を返すため、クライアントはリクエストの前にある優先手動スキャンの数を表示できます。非常に大きなキューは上限付きで、`queuedAheadIsEstimate: true` として報告されます。
...
送信済みバージョン向けの認証済み保存レポートアーカイブエンドポイント。

- Skill または plugin に対する所有者/公開者の管理アクセス、またはプラットフォームのモデレーター/管理者権限が必要です。
...
- Skill ID、公開者 ID、選択されたバージョンのメタデータはトップレベルのエンベロープフィールド（`slug`、`displayName`、`publisherHandle`、`version`、`resolvedFrom`、`tag`、`createdAt`）であるため、シェル自動化はネストされたラッパーを展開せずに読み取れます。
- `security` はトップレベルの ClawScan/security 判定です。自動化では `ok`、`decision`、`reasons`、および `security.status` を基準にしてください。
...
- `provenance` は、公開またはインポート中に ClawHub が GitHub repo/ref/commit/path を解決して保存した場合にのみ `server-resolved-github-import` です。それ以外の場合は `unavailable` です。

### `POST /api/v1/skills/-/security-verdicts`

正確な Skill バージョンに対する現在のコンパクトなセキュリティ判定を返します。この
コレクションエンドポイントは、OpenClaw Control UI など、表示する必要があるインストール済み
ClawHub Skill バージョンをすでに把握しているクライアントを対象としています。
...
- 結果はアイテムごとです。1 つの Skill またはバージョンが見つからなくても、レスポンス全体は失敗しません。
- レスポンスはセキュリティ専用です。Skill Card データ、生成済みカードステータス、アーティファクトファイルリスト、または詳細なスキャナーペイロードは含まれません。
- `security.signals` にはステータスレベルの補助的な証拠のみが含まれます。完全なスキャナー詳細には `/scan` または ClawHub security-audit ページを使用してください。
- `security.signals.dependencyRegistry` は v1 レスポンス互換性のために保持されていますが、依存関係レジストリ存在スキャナーは廃止されており、このキーは常に `null` です。
- Skill Card が存在しないことは、このエンドポイントの `ok`、`decision`、または `reasons` に影響しません。クライアントがカード内容を必要とする場合は、インストール済みの `skill-card.md` をローカルで読み取る必要があります。
...
- `category` (任意): plugin カテゴリフィルター。リクエストが plugin パッケージ (`/api/v1/plugins`、
  ...
  `family=code-plugin`/`family=bundle-plugin` を指定したパッケージエンドポイント) にスコープされている場合にのみ対応します。制御カテゴリと
...
- 認証済み呼び出し元は、所属する公開元のプライベートパッケージをリスト/検索結果で確認できます。
...
- `channel` (任意): `official`、`community`、または `private`
- `isOfficial` (任意): `true` または `false`
- `category` (任意): plugin カテゴリフィルター。リクエストが plugin パッケージにスコープされている場合にのみ対応します。制御カテゴリとレガシー v1
...
code-plugin と bundle-plugin パッケージ全体を対象にした、Plugin 専用カタログ閲覧。
...
- 本文: ZIP アーカイブ。
- エクスポートされた各 skill は `{publisher}/{slug}/` をルートとします。
- ホスト型 skills には、保存されている最新バージョンのファイルが含まれ、
  `_manifest.json` に `sourceRef: "public-clawhub"` として一覧されます。
- `clean` または `suspicious` スキャンのある現在の GitHub-backed skills には、
  `sourceRef: "public-github"`、repo、commit、path、
  content hash、archive URL を含む `_source_handoff.json` が含まれます。ClawHub ホスト型ソースファイルは含まれません。
...
- `version.sha256hash` は旧クライアント向けの非推奨の互換メタデータです。これは
...
- スキャンデータが存在する場合、`version.vtAnalysis`、`version.llmAnalysis`、および
...
- このエンドポイントはバージョン厳密です。クライアントは、最新のパッケージメタデータを読んだ後だけでなく、
  インストールしようとしているパッケージバージョンを解決した後に呼び出す必要があります。
- 非公開パッケージは、呼び出し元が所有パブリッシャーを読み取れる場合を除き `404` を返します。
- このエンドポイントは、所有者/モデレーター向けモデレーションエンドポイントよりも意図的に狭い範囲です。
  インストール判断と公開説明を公開し、報告者の身元、報告本文、非公開証拠、内部レビュータイムラインは公開しません。
...
パッケージリリースレビューキューのためのモデレーター/管理者エンドポイントです。
...
バージョンにリンクできます。これらはモデレーションキューに送られますが、それ自体でダウンロードを
自動的に非表示にしたりブロックしたりすることはありません。モデレーターはリリースモデレーションを使用して、
...
パッケージのモデレーション可視性のための、所有者/モデレーター用エンドポイント。
...
`confirmed` と `dismissed` では `note` が必須。`status` を `open` に戻す場合は省略できます。確認済みレポートに対して `finalAction: "quarantine"` または
...
- `approved`: 手動でレビュー済みで許可されています。
- `quarantined`: フォローアップ待ちでブロックされています。
- `revoked`: 以前に信頼済みだったリリースがブロックされています。
...
- バイナリファイルは `415` を返します。
- ファイルサイズ上限: 200KB。
- 保留中の VirusTotal スキャンは読み取りをブロックしません。悪意のあるリリースは、別の場所で引き続き withheld される場合があります。
...
ClawPack ベースのパッケージバージョンに対して npm 互換のパッキュメントを返します。
...
- `dist.tarball`、`dist.integrity`、および `dist.shasum` は npm 互換の
  フィールドを使用するため、ユーザーは望む場合に npm をミラーに向けることができます。
- スコープ付きパッケージのパッキュメントは、`/api/npm/@scope/name` と npm の
...
ホストされた skill バージョン ZIP をダウンロードするか、ホスト済みバージョンがなく、`clean` または `suspicious` スキャンの現在の GitHub ベース skill について GitHub ソース引き渡しを返します。
...
- GitHub ベースの skill 引き渡しでは、バイト列をプロキシまたはミラーしません。JSON レスポンスには
...
- 許可されるフォームフィールドは、`payload`、繰り返しの `files` blob、または 1 つの `clawpack`
  ...
  upload-url フローで返された storage id にできます。ステージング済み storage-id publish では、そのアップロード URL とともに返された
...
- JSON body と、呼び出し元が指定した `payload.files` / `payload.artifact`
  メタデータは拒否されます。
- 直接 multipart publish リクエストは 18MB に制限されます。ClawPack tarball は
...
- Code plugins には、`package.json`、ソースリポジトリメタデータ、ソースコミット
...
所有者が開始したソフト削除では slug が 30 日間予約され、その後、その slug は
別のパブリッシャーが取得できます。削除レスポンスには、この有効期限が適用される場合に `slugReservedUntil` が含まれます。
モデレーター/管理者による非表示化とセキュリティ削除は、この方法では期限切れになりません。
...
新しく作成される org には `memberHandle` を指定します。実行中の管理者はメンバーとして追加されません。
...
管理者専用。Convex Auth アカウント行を編集せずに、検証済みの置換 GitHub OAuth principal のために個人パブリッシャーを復旧します。
リクエストでは、不変の GitHub
provider account id を両方とも指定する必要があります。可変のハンドルは、オペレーター向けのガードとしてのみ使用されます。
...
パブリッシャーにスキル、パッケージ、または GitHub スキルソースがある場合、リカバリーは安全側に失敗します。
リカバリーでは、復元されたパブリッシャーのスキル、スキルスラッグエイリアス、パッケージ、パッケージ検査警告、派生検索ダイジェスト行のレガシー `ownerUserId` フィールドも移行されるため、直接所有者パスは新しいパブリッシャー権限と一致します。復元されたハンドルのアクティブな保護ハンドル予約も置換先ユーザーに再割り当てされるため、後続のプロファイル同期で以前のユーザーの競合する権限が復元されることはありません。各プライマリテーブルは
適用トランザクションあたり 100 行に制限されます。より大きなリカバリーでは、先に再開可能な所有者移行を使う必要があります。
...
ユーザーを禁止し、所有しているスキルを完全削除します（モデレーター/管理者のみ）。
