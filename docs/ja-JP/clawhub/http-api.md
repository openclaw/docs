...
x-i18n:
    generated_at: "2026-06-27T10:48:00Z"
    model: gpt-5.5
...
サードパーティのディレクトリは、ClawHub skills を一覧表示または検索するために、公開読み取りエンドポイントを使用できます。結果をキャッシュし、`429`/`Retry-After` を尊重し、ユーザーを正規の ClawHub リスティング (`https://clawhub.ai/<owner>/skills/<slug>`) に戻すリンクを設置し、ClawHub がサードパーティサイトを推奨しているかのように示唆しないでください。公開 API サーフェスの外側で、非表示、プライベート、またはモデレーションでブロックされたコンテンツをミラーしようとしないでください。

Web スラッグショートカットはレジストリファミリーをまたいで解決されますが、API クライアントはルートの優先順位を再構築するのではなく、読み取りエンドポイントから返された正規 URL を使用する必要があります。
...
- 認証済み書き込みエンドポイントは、サーバーが理由を把握している場合に、単なる `Unauthorized` を返すべきではありません。トークンの欠落、無効または取り消されたトークン、削除済み/禁止済み/無効化済みアカウントには、それぞれ実行可能な説明文を返し、CLI クライアントが何によりユーザーがブロックされたかを伝えられるようにする必要があります。
...
- `Retry-After`: `429` の場合、再試行前に待機する秒数 (遅延)
...
- 信頼済みクライアント IP が利用できない場合、匿名リクエストはレート制限種別のみをスコープとするフォールバックバケットを使用します。これらのフォールバックバケットには、呼び出し元が指定したパス、スラッグ、パッケージ名、バージョン、クエリ文字列、またはその他のアーティファクトパラメーターは含まれません。

## エラーレスポンス

公開 v1 エラーレスポンスは、`content-type: text/plain; charset=utf-8` のプレーンテキストです。
これには、バリデーション失敗 (`400`)、公開リソースの欠落 (`404`)、認証および権限の失敗 (`401`/`403`)、レート制限 (`429`)、ブロックされたダウンロードが含まれます。クライアントはレスポンス本文を人間が読める文字列として読む必要があります。未知のクエリパラメーターは互換性のため無視されますが、認識されるクエリパラメーターに無効な値がある場合は `400` を返します。
...
- 関連度は人気度より強く扱われます。正確なスラッグまたは表示名トークンの一致は、エンゲージメントが大幅に強いが一致が緩いものより上位になる場合があります。
- ASCII テキストは単語境界と句読点境界でトークン化されます。たとえば、`personal-map` には独立した `map` トークンが含まれますが、`amap-jsapi-skill` には `amap`、`jsapi`、`skill` が含まれます。そのため、`map` で検索すると、`amap-jsapi-skill` より `personal-map` の方が強い字句一致になります。
- 人気度は対数スケール化され、上限があります。エンゲージメントが高い skills でも、クエリテキストとの一致が弱い場合は順位が下がることがあります。
- 疑わしい、または非表示のモデレーション状態により、呼び出し元フィルターと現在のモデレーションステータスに応じて、skill が公開検索から除外されることがあります。

公開者向けの発見可能性ガイダンス:

- ユーザーが文字どおり検索する用語を、表示名、概要、タグに入れてください。独立したスラッグトークンは、それが保持したい安定した識別子でもある場合にのみ使用してください。
- 1 つのクエリを追いかけるためだけにスラッグを改名しないでください。ただし、新しいスラッグが長期的によりよい正規名である場合は除きます。古いスラッグはリダイレクトエイリアスになりますが、正規 URL、表示されるスラッグ、今後の検索ダイジェストでは新しいスラッグが使用されます。
- 改名エイリアスは、レジストリ経由で解決される古い URL とインストールの解決を維持しますが、検索ランキングは改名がインデックスされた後の正規 skill メタデータに基づきます。既存の統計はその skill に残ります。
- skill が予期せず見えない場合、ランキング関連メタデータを変更する前に、ログインした状態で `clawhub inspect @owner/slug` を使って、まずモデレーション状態を確認してください。
...
- `limit` (任意): 整数 (1–200)
- `cursor` (任意): `trending` 以外の任意のソート向けページネーションカーソル
...
- `createdAt` は新しい skill のクロールでは安定しています。`updated` は既存の skills が再公開されると変わります。
- `nonSuspiciousOnly=true` の場合、疑わしい skills がページ取得後にフィルターされるため、カーソルベースのソートでは、ページ内の項目数が `limit` より少なくなることがあります。
...
- オーナーの改名/マージフローによって作成された古いスラッグは、正規 skill に解決されます。
...
- 証拠は公開呼び出し元には編集済みで、オーナー/モデレーターにのみ生スニペットを含めます。

### `POST /api/v1/skills/{slug}/report`

モデレーターのレビューのために skill を報告します。報告は skill レベルで、任意でバージョンに紐付けられ、skill 報告キューに送られます。
...
`note` は `confirmed` と `dismissed` で必須です。`status` を `open` に戻す場合は省略できます。監査可能な同じワークフローで skill を非表示にするには、トリアージ済みの報告とともに `finalAction: "hide"` を渡してください。
...
送信されたスキャン用の認証済みポーリングエンドポイント。

- キュー投入済み/実行中/成功/失敗のステータスを返します。
- キュー投入中は `queue.queuedAhead` と `queue.position` を返すため、クライアントはリクエストの前にある優先手動スキャンの数を表示できます。非常に大きなキューは上限が設けられ、`queuedAheadIsEstimate: true` として報告されます。
...
- skill または plugin に対する所有者/公開者の管理アクセス権、またはプラットフォームモデレーター/管理者権限が必要です。
...
- Skill の識別情報、公開者の識別情報、選択されたバージョンのメタデータはトップレベルのエンベロープフィールド（`slug`、`displayName`、`publisherHandle`、`version`、`resolvedFrom`、`tag`、`createdAt`）であるため、シェル自動化はネストされたラッパーを展開せずに読み取れます。
...
ClawHub skill バージョンをすでに把握しているクライアント向けです。
...
- `security.signals.dependencyRegistry` は v1 レスポンス互換性のために保持されていますが、依存関係レジストリ存在スキャナーは廃止されており、このキーは常に `null` です。
- Skill Card が存在しないことは、このエンドポイントの `ok`、`decision`、`reasons` に影響しません。カード内容が必要な場合、クライアントはインストール済みの `skill-card.md` をローカルで読む必要があります。
...
- `category` (任意): Pluginカテゴリフィルター。リクエストがPluginパッケージ
  ...
  にスコープされている場合にのみサポートされます。制御カテゴリと
...
- 認証済みの呼び出し元は、所属するパブリッシャーのプライベートパッケージを一覧/検索結果で確認できます。
- `channel=private` は、認証済みの呼び出し元が読み取れるパッケージのみを返します。

### `GET /api/v1/packages/search`

スキル + Pluginパッケージ全体にわたる統合カタログ検索。
...
コードPluginとバンドルPluginパッケージを横断するPlugin専用カタログブラウズ。
...
- `version.sha256hash` は古いクライアント向けの非推奨の互換メタデータです。
...
- `release.artifactKind`、`release.artifactSha256`、`release.npmIntegrity`、
  ...
  既知の場合に存在します。
- `trust.scanStatus` は、スキャナー入力と手動リリースモデレーションから派生した
  有効な信頼ステータスです。
...
- このエンドポイントはバージョンに厳密です。クライアントは、最新のパッケージメタデータを
  読んだ直後ではなく、インストールしようとするパッケージバージョンを解決した後に
  呼び出すべきです。
- 非公開パッケージは、呼び出し元が所有パブリッシャーを読み取れる場合を除き `404` を返します。
- このエンドポイントは、所有者/モデレーター向けのモデレーションエンドポイントより意図的に
  狭くなっています。インストール判断と公開説明を公開し、報告者の身元、報告本文、
...
バージョンにリンクできます。報告はモデレーションキューに送られますが、それだけでダウンロードを
...
- ダウンロードレートバケットを使用します。
- ダウンロードヘッダーには ClawHub SHA-256 に加え、npm integrity/shasum メタデータが含まれます。
- モデレーションとプライベートパッケージのアクセスチェックは引き続き適用されます。

### `GET /api/v1/resolve`

ローカルフィンガープリントを既知のバージョンにマップするために CLI によって使用されます。
...
バージョンがない現在の GitHub 裏付けスキルについて GitHub ソース引き渡しを返します。
...
- GitHub 裏付けスキルの引き渡しは、バイトをプロキシまたはミラーしません。JSON レスポンスには
...
- 任意のペイロードフィールド: `ownerHandle`。存在する場合、API はその
  公開元をサーバー側で解決し、アクターに公開元アクセス権を要求します。
- 任意のペイロードフィールド: `migrateOwner`。`ownerHandle` とともに `true` の場合、
  既存のスキルは、アクターが現在と移行先の両方の公開元で管理者/オーナーであれば、そのオーナーへ移動できます。このオプトインがない場合、オーナー変更は
  拒否されます。
...
- 任意のペイロードフィールド: `ownerHandle`。存在する場合、そのオーナーの代理公開は管理者のみに許可されます。
...
- Code Plugin には `package.json`、ソースリポジトリメタデータ、ソースコミット
  メタデータ、構成スキーマメタデータ、`openclaw.compat.pluginApi`、および
  `openclaw.build.openclawVersion` が必要です。
- `openclaw.hostTargets` と `openclaw.environment` は任意のメタデータです。
- `official` チャネルに公開できるのは、`openclaw` org 公開元と現在の `openclaw` org メンバーの
  個人公開元のみです。
- 代理公開でも、対象オーナーアカウントに対して official-channel 適格性が検証されます。

### `DELETE /api/v1/skills/{slug}` / `POST /api/v1/skills/{slug}/undelete`

スキルを論理削除 / 復元します (オーナー、モデレーター、または管理者)。
...
オーナー主導の論理削除では slug が 30 日間予約され、その後 slug は
別の公開元が取得できるようになります。削除レスポンスには、この期限が適用される場合 `slugReservedUntil` が含まれます。
...
従来の共有ユーザー/個人公開元を指している場合、エンドポイントはまずそれを org 公開元に移行します。
...
オーナーが後でその名前に実際の code-plugin または bundle-plugin リリースを公開できます。
...
個人公開元を復旧します。リクエストには、両方の不変 GitHub
プロバイダーアカウント ID を指定する必要があります。変更可能なハンドルはオペレーター向けのガードとしてのみ使用されます。

エンドポイントはデフォルトでドライランになります。リカバリを適用するには、スタッフが両方の GitHub プリンシパル間の継続性を独立して確認した後に、`dryRun: false` と
`confirmIdentityVerified: true` が必要です。宛先ユーザーの現在の個人 publisher に skills、packages、または GitHub skill sources がある場合、リカバリは失敗して閉じます。
リカバリでは、復旧対象 publisher の skills、skill slug aliases、packages、package inspector warnings、および派生 search digest rows のレガシー `ownerUserId` フィールドも移行されるため、direct-owner paths は新しい publisher authority と一致します。復旧対象 handle のアクティブな protected-handle reservation も replacement user に再割り当てされるため、後続の profile synchronization が以前のユーザーの競合する authority を復元することはできません。各 primary table は apply transaction ごとに
100 行に制限されます。より大きなリカバリでは、先に resumable owner migration を使用する必要があります。
GitHub skill sources は publisher-scoped であり、書き換えられずに checked として報告されます。
...
既存の ban について、unban やコンテンツ復元を行わずに保存済みの理由を変更します
(admin のみ)。`dryRun` が `false` でない限り、デフォルトでドライランになります。
