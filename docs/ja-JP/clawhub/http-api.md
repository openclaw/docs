...
サードパーティのディレクトリは、公開読み取りエンドポイントを使用して ClawHub のスキルを一覧表示または検索できます。結果をキャッシュし、`429`/`Retry-After` を尊重し、ユーザーを正規の ClawHub リスト (`https://clawhub.ai/<owner>/skills/<slug>`) に戻すリンクを設置し、サードパーティサイトを ClawHub が推奨しているように示唆することは避けてください。公開 API サーフェスの外にある非表示、非公開、またはモデレーションでブロックされたコンテンツをミラーしようとしないでください。

Web スラッグのショートカットはレジストリファミリー横断で解決されますが、API クライアントはルートの優先順位を再構築するのではなく、読み取りエンドポイントから返される正規 URL を使用するべきです。
...
- 認証済みの書き込みエンドポイントは、サーバーが理由を把握している場合、単なる `Unauthorized` を返すべきではありません。トークンなし、無効または失効したトークン、削除済み、BAN 済み、または無効化されたアカウントは、それぞれ実行可能なテキストを返し、CLI クライアントが何によってブロックされたかをユーザーに伝えられるようにするべきです。
...
これには、検証失敗 (`400`)、存在しない公開リソース (`404`)、認証および権限失敗 (`401`/`403`)、レート制限 (`429`)、ブロックされたダウンロードが含まれます。クライアントはレスポンス本文を人間が読める文字列として読み取るべきです。互換性のため、不明なクエリパラメータは無視されますが、認識されるクエリパラメータに無効な値がある場合は `400` を返します。
...
- 人気度より関連度が重視されます。正確なスラッグまたは表示名トークンの一致は、エンゲージメントがはるかに強いが一致が緩いものより上位になることがあります。
...
- 人気度は対数スケール化され、上限があります。高エンゲージメントのスキルでも、クエリテキストとの一致が弱い場合は順位が低くなることがあります。
- 疑わしい、または非表示のモデレーション状態は、呼び出し元のフィルターと現在のモデレーション状態に応じて、公開検索からスキルを除外することがあります。

公開者向けの発見可能性ガイダンス:

- ユーザーが実際に検索する語を、表示名、概要、タグに入れてください。スタンドアロンのスラッグトークンは、それが維持したい安定した識別子でもある場合にのみ使用してください。
- 新しいスラッグがより良い長期的な正規名でない限り、1 つのクエリを狙うためだけにスラッグを変更しないでください。古いスラッグはリダイレクトエイリアスになりますが、正規 URL、表示されるスラッグ、今後の検索ダイジェストは新しいスラッグを使用します。
...
- `createdAt` は新規スキルのクロールに対して安定しています。`updated` は既存スキルが再公開されると変わります。
- `nonSuspiciousOnly=true` の場合、疑わしいスキルがページ取得後にフィルタリングされるため、カーソルベースのソートでは 1 ページの項目数が `limit` より少なくなることがあります。
...
- 公開呼び出し元は、すでにフラグ付けされている表示可能なスキルに対してのみ `200` を受け取ります。
- 証拠は公開呼び出し元向けには編集され、オーナー/モデレーター向けにのみ生のスニペットを含みます。

### `POST /api/v1/skills/{slug}/report`

モデレーター確認のためにスキルを報告します。報告はスキルレベルで、任意でバージョンに紐付けられ、スキル報告キューに送られます。
...
スキル報告取り込み用のモデレーター/admin エンドポイント。
...
`note` は `confirmed` と `dismissed` では必須です。`status` を `open` に戻す場合は省略できます。監査可能な同じワークフローでスキルを非表示にするには、トリアージ済みの報告とともに `finalAction: "hide"` を渡します。
...
- スキャンジョブは非同期です。手動スキャンリクエストは通常の公開/バックフィル作業より優先されますが、完了はワーカーの空き状況に依存します。
...
- スキルまたは plugin に対する所有者/公開者の管理アクセス、またはプラットフォームのモデレーター/管理者権限が必要です。
...
- `security.signals.dependencyRegistry` は v1 レスポンス互換性のために保持されていますが、依存関係レジストリ存在スキャナーは廃止されており、このキーは常に `null` です。
- Skill Card が存在しないことは、このエンドポイントの `ok`、`decision`、`reasons` に影響しません。クライアントはカード内容が必要な場合、インストール済みの `skill-card.md` をローカルで読み取る必要があります。
...
レガシー v1 フィルターエイリアスは、読み取りエンドポイントでは引き続き受け付けられます。
...
オフライン分析用に、最新の公開 Skills を一括エクスポートします。
...
- `clean` または `suspicious` のスキャンを持つ現在の GitHub-backed Skills には、
  ...
  content hash、archive URL を含む `_source_handoff.json` が含まれます。ClawHub ホスト型のソースファイルは含まれません。
...
code-plugin パッケージと bundle-plugin パッケージを横断する Plugin 専用検索。
...
- `version.artifact.kind` は、旧世界のパッケージアーカイブでは `legacy-zip`、
...
- `version.sha256hash` は、古いクライアント向けの非推奨の互換性メタデータです。
  ...
  モダンなクライアントは、正規リリースアーティファクトを識別する
...
インストールクライアント向けに、正確なパッケージリリースのセキュリティと信頼の概要を返します。これは、解決済みリリースをインストールできるかを判断するための公開 OpenClaw 消費サーフェスです。
...
- このエンドポイントはバージョン厳密です。クライアントは、単に最新のパッケージメタデータを読んだ後ではなく、
...
- これは移行の準備状態のみを追跡します。OpenClaw を変更したり ClawPacks を生成したりするものではありません。
...
- `blocked`: 隔離済み、取り消し済み、または悪意のあるリリース。
...
- レジストリ専用メタデータはダウンロードされたアーカイブに挿入されません。
...
ホストされたスキルバージョン ZIP をダウンロードするか、`clean` または `suspicious` スキャンがあり、ホスト済みバージョンがない
現在の GitHub ベースのスキルについて GitHub ソース引き継ぎを返します。
...
- GitHub ベースのスキル引き継ぎはバイト列をプロキシまたはミラーしません。JSON レスポンスには
...
- 任意のペイロードフィールド: `migrateOwner`。`ownerHandle` とともに `true` の場合、アクターが現在および移行先の両方の
  パブリッシャーで管理者/オーナーであれば、既存のスキルをそのオーナーへ移動できます。このオプトインがない場合、オーナー変更は
  拒否されます。
...
- 許可されるフォームフィールドは、`payload`、繰り返しの `files` blob、または 1 つの `clawpack`
  ...
  upload-url フローから返される storage id にできます。ステージ済み storage-id 公開では、そのアップロード URL とともに返された
...
新しく作成された org の場合は `memberHandle` を指定します。操作中の管理者はメンバーとして追加されません。
...
プロバイダーアカウント ID を指定する必要があります。可変のハンドルは、オペレーター向けガードとしてのみ使用されます。
...
リカバリでは、復元されたパブリッシャーのスキル、スキル slug エイリアス、パッケージ、パッケージインスペクター警告、および派生検索ダイジェスト行のレガシー `ownerUserId` フィールドも移行されるため、
直接所有者パスが新しいパブリッシャー権限と一致します。復元されたハンドルに対するアクティブな保護ハンドル予約も置換ユーザーに再割り当てされるため、その後の
プロファイル同期で以前のユーザーの競合する権限が復元されることはありません。各プライマリテーブルは、適用トランザクションごとに
100 行に制限されます。より大きなリカバリでは、まず再開可能な所有者移行を使用する必要があります。
GitHub スキルソースはパブリッシャースコープであり、書き換えられるのではなく確認済みとして報告されます。
...
ユーザーを ban し、所有スキルを物理削除します（モデレーター/admin のみ）。
...
`POST /api/cli/upload-url` は `uploadUrl` と `uploadTicket` を返します。ClawPack tarball をステージするパッケージ公開処理では、結果のストレージ ID を
`clawpack` として、返されたチケットを `clawpackUploadTicket` として送信する必要があります。

## Registry 検出（`/.well-known/clawhub.json`）

CLI はサイトから Registry/認証設定を検出できます:
