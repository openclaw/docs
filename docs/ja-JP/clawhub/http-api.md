...
サードパーティのディレクトリは、公開読み取りエンドポイントを使って ClawHub の Skills を一覧表示または検索できます。結果をキャッシュし、`429`/`Retry-After` を尊重し、ユーザーを正規の ClawHub リスト（`https://clawhub.ai/<owner>/skills/<slug>`）へリンクし、サードパーティサイトを ClawHub が推奨しているような表現は避けてください。非表示、非公開、またはモデレーションでブロックされたコンテンツを公開 API サーフェスの外でミラーしようとしないでください。
...
- トークンがないか無効な場合、動作は IP による適用へフォールバックします。
- 認証済みの書き込みエンドポイントは、サーバーが理由を把握している場合、単なる `Unauthorized` を返すべきではありません。トークンの欠落、無効または取り消し済みのトークン、削除済み、BAN 済み、無効化済みのアカウントには、それぞれ CLI クライアントがユーザーに何がブロックしたのかを伝えられるよう、対処可能なテキストを返す必要があります。
...
これには、検証失敗（`400`）、存在しない公開リソース（`404`）、認証と権限の失敗（`401`/`403`）、レート制限（`429`）、ブロックされたダウンロードが含まれます。クライアントはレスポンス本文を人間が読める文字列として読むべきです。未知のクエリパラメーターは互換性のため無視されますが、認識されるクエリパラメーターに無効な値がある場合は `400` を返します。
...
- 結果は関連度順で返されます（埋め込み類似度 + 正確なスラッグ/名前トークンのブースト + 小さな人気度事前値）。
- 関連度は人気度より強く扱われます。正確なスラッグまたは表示名トークンの一致は、エンゲージメントがはるかに強いが一致がより緩い結果を上回ることがあります。
- ASCII テキストは単語と句読点の境界でトークン化されます。たとえば、`personal-map` には独立した `map` トークンが含まれますが、`amap-jsapi-skill` には `amap`、`jsapi`、`skill` が含まれます。そのため、`map` を検索すると、`amap-jsapi-skill` よりも `personal-map` のほうが強い字句一致になります。
- 人気度は対数スケール化され、上限があります。エンゲージメントの高い Skills でも、クエリテキストとの一致が弱い場合は順位が下がることがあります。
- 疑わしい、または非表示のモデレーション状態にある Skills は、呼び出し元のフィルターと現在のモデレーション状態に応じて、公開検索から除外されることがあります。

公開者向けの発見可能性ガイダンス:

- ユーザーが実際に検索する語句を表示名、概要、タグに入れてください。独立したスラッグトークンは、それが保持したい安定した識別子でもある場合にのみ使用してください。
- 新しいスラッグが長期的により良い正規名でない限り、1 つのクエリを追いかけるためだけにスラッグを変更しないでください。古いスラッグはリダイレクトエイリアスになりますが、正規 URL、表示されるスラッグ、将来の検索ダイジェストでは新しいスラッグが使用されます。
- 名前変更エイリアスは、古い URL とレジストリ経由で解決されるインストールの解決を維持しますが、検索順位は名前変更がインデックス化された後の正規 Skill メタデータに基づきます。既存の統計は Skill に保持されます。
- Skill が予期せず見えない場合は、順位関連メタデータを変更する前に、ログインした状態で `clawhub inspect @owner/slug` を使って、まずモデレーション状態を確認してください。
...
- `createdAt` は新規 Skill のクロールでは安定しています。`updated` は既存の Skills が再公開されると変わります。
- `nonSuspiciousOnly=true` の場合、疑わしい Skills はページ取得後にフィルターされるため、カーソルベースのソートではページ内の項目数が `limit` より少なくなることがあります。
- 存在する場合は `nextCursor` を使ってページネーションを続行してください。短いページであること自体は結果の終端を意味しません。
...
- 公開の呼び出し元は、すでにフラグ付けされた表示中の Skills に対してのみ `200` を取得します。
- 証拠は公開の呼び出し元向けには編集され、未加工のスニペットはオーナー/モデレーターにのみ含まれます。
...
Skill 報告の受け付け用モデレーター/admin エンドポイント。
...
`note` は `confirmed` と `dismissed` では必須です。`status` を `open` に戻す場合は省略できます。監査可能な同じワークフロー内で Skill を非表示にするには、トリアージ済みの報告に `finalAction: "hide"` を渡します。
...
- スキャンジョブは非同期です。手動スキャンリクエストは通常の公開/バックフィル作業よりも優先されますが、完了は引き続きワーカーの可用性に依存します。
...
- キュー待ちの間は `queue.queuedAhead` と `queue.position` を返すため、クライアントはリクエストの前にある優先手動スキャンの数を表示できます。非常に大きなキューは上限が設けられ、`queuedAheadIsEstimate: true` として報告されます。
...
- `ok` は、選択されたバージョンに生成済みのスキルカードがあり、モデレーションによってマルウェアとしてブロックされておらず、ClawScan 検証がクリーンな場合にのみ `true` になります。
...
- レスポンスはセキュリティのみです。スキルカードデータ、生成済みカードステータス、アーティファクトファイル一覧、詳細なスキャナーペイロードは含まれません。
- `security.signals` には、ステータスレベルの補助的な証拠のみが含まれます。完全なスキャナー詳細には `/scan` または ClawHub の security-audit ページを使用してください。
- `security.signals.dependencyRegistry` は v1 レスポンス互換性のために保持されていますが、依存関係レジストリ存在スキャナーは廃止されており、このキーは常に `null` です。
- スキルカードが存在しないことは、このエンドポイントの `ok`、`decision`、`reasons` に影響しません。カード内容が必要な場合、クライアントはインストール済みの `skill-card.md` をローカルで読み取る必要があります。
...
- `category` (任意): プラグインカテゴリーフィルター。リクエストがプラグインパッケージ (`/api/v1/plugins`、`/api/v1/code-plugins`、`/api/v1/bundle-plugins`、または `family=code-plugin`/`family=bundle-plugin` を指定したパッケージエンドポイント) にスコープされている場合のみサポートされます。制御対象カテゴリーとレガシー v1 フィルターエイリアスは、`GET /api/v1/plugins` に記載されています。

注記:

- `family`、`channel`、`isOfficial`、`featured`、`highlightedOnly`、または `sort` の値が無効な場合は `400` を返します。不明なクエリパラメーターは無視されます。
- `GET /api/v1/code-plugins` と `GET /api/v1/bundle-plugins` は、固定 family のエイリアスとして残ります。
...
- `channel` (任意): `official`、`community`、または `private`
- `isOfficial` (任意): `true` または `false`
- `category` (任意): プラグインカテゴリーフィルター。リクエストがプラグインパッケージにスコープされている場合のみサポートされます。制御対象カテゴリーとレガシー v1 フィルターエイリアスは、`GET /api/v1/plugins` に記載されています。

注記:

- `family`、`channel`、`isOfficial`、`featured`、または `highlightedOnly` の値が無効な場合は `400` を返します。不明なクエリパラメーターは無視されます。
- 匿名呼び出し元には公開パッケージチャネルのみが表示されます。
...
コードプラグインとバンドルプラグインのパッケージを横断する、プラグイン専用カタログのブラウズです。
...
レガシーエイリアスは、保存済みまたは作者が宣言したカテゴリー値としては受け入れられません。
...
- ホスト型スキルには最新の保存済みバージョンファイルが含まれ、`_manifest.json` に `sourceRef: "public-clawhub"` として一覧表示されます。
- `clean` または `suspicious` のスキャンを持つ現在の GitHub backed スキルには、`sourceRef: "public-github"`、リポジトリ、コミット、パス、コンテンツハッシュ、アーカイブ URL を含む `_source_handoff.json` が含まれます。これらには ClawHub ホスト型のソースファイルは含まれません。
- 各スキルには `_export_skill_meta.json` が含まれます。
- `_manifest.json` は常に ZIP ルートに含まれます。
- 個別のスキルまたはファイルをエクスポートできなかった場合、`_errors.json` が含まれます。
...
- `version.sha256hash` は古いクライアント向けの非推奨の互換性メタデータです。これは
  `/api/v1/packages/{name}/download` が返す正確な ZIP バイトをハッシュします。
  現代的なクライアントは、正規リリースアーティファクトを識別する
...
- `trust.blockedFromDownload` はインストールブロックシグナルです。OpenClaw およびその他のインストールクライアントは、
  スキャナーまたはモデレーションフィールドからブロックルールを再導出するのではなく、この値が `true` の場合にインストールをブロックするべきです。
...
- 非公開パッケージは、呼び出し元が所有パブリッシャーを読み取れる場合を除き `404` を返します。
- このエンドポイントは、所有者/モデレーターのモデレーションエンドポイントより意図的に狭くなっています。
  インストール判断と公開説明を公開し、報告者の ID、報告本文、非公開証拠、内部レビュータイムラインは公開しません。
...
- `all`: 手動上書き、非 clean スキャン状態、またはパッケージ報告がある任意のリリース。
...
モデレーターレビューのためにパッケージを報告します。報告はパッケージレベルで、任意でバージョンにリンクできます。
モデレーションキューに反映されますが、それ自体ではダウンロードを自動的に非表示またはブロックしません。モデレーターはリリースモデレーションを使用して、
...
`confirmed` と `dismissed` には `note` が必要です。`status` を `open` に戻す場合は省略できます。確認済みレポートで `finalAction: "quarantine"` または
...
- `revoked`: 以前に信頼されたリリースの後にブロックされています。
...
- 非公開パッケージは、呼び出し元が所有発行者を読み取れる場合を除き `404` を返します。
...
ホストされた skill バージョン ZIP をダウンロードするか、`clean` または `suspicious` のスキャンを持ち、ホストされた
...
- GitHub ベース skill の引き渡しでは、バイトをプロキシまたはミラーしません。JSON レスポンスには
...
- 任意のペイロードフィールド: `ownerHandle`。存在する場合、API はその
  発行者をサーバー側で解決し、アクターに発行者アクセス権を要求します。
- 任意のペイロードフィールド: `migrateOwner`。`ownerHandle` とともに `true` の場合、
  アクターが現在と移行先の両方の発行者で管理者/所有者であれば、既存の skill をその所有者に移動できます。
...
- 許可されるフォームフィールドは `payload`、繰り返しの `files` blob、または 1 つの `clawpack`
  ...
  upload-url フローで返されるストレージ ID にできます。ステージング済み storage-id 公開では、そのアップロード URL とともに返された
  `clawpackUploadTicket` も含める必要があります。
- `files` または `clawpack` のどちらかを使用し、同じリクエストで両方を使用しないでください。
- JSON 本文および呼び出し元指定の `payload.files` / `payload.artifact`
  メタデータは拒否されます。
- 直接 multipart 公開リクエストは 18MB に制限されます。ClawPack tarball は
  upload-url フローを使用して 120MB tarball 上限まで扱えます。
- 任意のペイロードフィールド: `ownerHandle`。存在する場合、その所有者の代理公開は管理者のみが実行できます。
...
- `official` チャンネルに公開できるのは、`openclaw` 組織発行者と、現在の `openclaw` 組織メンバーの
  個人発行者のみです。
...
所有者による論理削除では slug が 30 日間予約され、その後は別の発行者がその slug を取得できます。
...
管理者専用。ハンドル用の組織発行者が存在することを保証します。ハンドルがまだ
従来の共有ユーザー/個人発行者を指している場合、エンドポイントはまずそれを組織発行者に移行します。
新しく作成される組織の場合は、`memberHandle` を指定します。実行中の管理者はメンバーとして追加されません。
...
管理者専用。Convex Auth アカウント行を編集せずに、検証済みの置き換え GitHub OAuth プリンシパル用に
...
パブリッシャーに Skills、パッケージ、または GitHub skill ソースがある場合、リカバリーはフェイルクローズします。
リカバリーでは、復旧されたパブリッシャーの Skills、
skill slug エイリアス、パッケージ、パッケージインスペクター警告、派生検索ダイジェスト行のレガシー `ownerUserId` フィールドも移行されるため、
直接所有者パスは新しいパブリッシャー権限と一致します。復旧されたハンドルのアクティブな protected-handle
予約も置換先ユーザーに再割り当てされるため、後続の
プロファイル同期で以前のユーザーの競合する権限が復元されることはありません。各プライマリテーブルは、
適用トランザクションごとに 100 行に制限されます。それより大きいリカバリーでは、まず再開可能な所有者移行を使用する必要があります。
...
ユーザーを ban し、所有している Skills を物理削除します（moderator/admin のみ）。
...
ban 解除やコンテンツの復元をせずに、既存の ban に保存されている理由を変更します
...
`POST /api/cli/upload-url` は `uploadUrl` と `uploadTicket` を返します。ClawPack tarball をステージングするパッケージ
