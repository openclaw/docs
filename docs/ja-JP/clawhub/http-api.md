...
サードパーティのディレクトリは、公開読み取りエンドポイントを使用して ClawHub スキルを一覧表示または検索できます。結果をキャッシュし、`429`/`Retry-After` を尊重し、ユーザーを正規の ClawHub リスティング (`https://clawhub.ai/<owner>/skills/<slug>`) にリンクし、サードパーティサイトを ClawHub が推奨しているように示唆することは避けてください。非表示、非公開、またはモデレーションでブロックされたコンテンツを公開 API サーフェスの外でミラーしようとしないでください。

Web スラッグショートカットはレジストリファミリー全体で解決されますが、API クライアントはルートの優先順位を再構築するのではなく、読み取りエンドポイントが返す正規 URL を使用してください。
...
- 認証済み書き込みエンドポイントは、サーバーが理由を把握している場合に素の `Unauthorized` を返すべきではありません。トークン不足、無効または取り消されたトークン、削除済み、禁止済み、無効化されたアカウントは、それぞれ CLI クライアントがユーザーに何がブロックしたかを伝えられるよう、実行可能なテキストを返すべきです。
...
これには検証失敗 (`400`)、見つからない公開リソース (`404`)、認証および権限失敗 (`401`/`403`)、レート制限 (`429`)、ブロックされたダウンロードが含まれます。クライアントはレスポンス本文を人間が読める文字列として読み取るべきです。不明なクエリパラメーターは互換性のため無視されますが、認識されたクエリパラメーターに無効な値がある場合は `400` を返します。
...
- 関連度は人気度より強く扱われます。正確なスラッグまたは表示名トークンの一致は、エンゲージメントがはるかに高い緩い一致より上位になることがあります。
...
- 人気度は対数スケールで上限があります。クエリテキストの一致が弱い場合、エンゲージメントの高いスキルが下位になることがあります。
- 疑わしい、または非表示のモデレーション状態は、呼び出し元のフィルターと現在のモデレーション状態に応じて、公開検索からスキルを除外することがあります。

公開者向けの見つけやすさのガイダンス:

- ユーザーが実際に検索する用語を表示名、概要、タグに入れてください。独立したスラッグトークンは、それが保持したい安定した識別子でもある場合にのみ使用してください。
- 新しいスラッグがより良い長期的な正規名でない限り、1 つのクエリを追うためだけにスラッグ名を変更しないでください。古いスラッグはリダイレクトエイリアスになりますが、正規 URL、表示されるスラッグ、将来の検索ダイジェストでは新しいスラッグが使われます。
- 名前変更エイリアスは、古い URL とレジストリ経由で解決されるインストールの解決を維持しますが、検索ランキングは名前変更がインデックス化された後の正規スキルメタデータに基づきます。既存の統計はスキルに残ります。
- スキルが想定外に見えない場合、ランキング関連メタデータを変更する前に、ログインした状態で `clawhub inspect @owner/slug` を使ってまずモデレーション状態を確認してください。
...
- `trending` は過去 7 日間のインストール数でランク付けします (テレメトリーベース)。
- `createdAt` は新規スキルのクロールで安定しています。`updated` は既存スキルが再公開されると変わります。
- `nonSuspiciousOnly=true` の場合、疑わしいスキルはページ取得後にフィルターされるため、カーソルベースのソートでは 1 ページに `limit` 件より少ない項目が返ることがあります。
...
- `moderation` は、スキルがフラグ付けされている場合、またはオーナーが表示している場合にのみ含まれます。
...
- 公開呼び出し元は、すでにフラグ付けされた可視スキルの場合にのみ `200` を受け取ります。
- 公開呼び出し元向けには証拠が編集され、生のスニペットはオーナー/モデレーターにのみ含まれます。
...
スキル報告受け入れ用のモデレーター/admin エンドポイント。
...
スキル報告を解決または再オープンするためのモデレーター/admin エンドポイント。
...
`note` は `confirmed` と `dismissed` で必須です。`status` を `open` に戻す場合は省略できます。監査可能な同じワークフローでスキルを非表示にするには、トリアージ済みの報告に `finalAction: "hide"` を渡してください。
...
- 履歴バージョンを問い合わせる場合は、`moderation` と `security` を同じバージョンコンテキストとして扱う前に、`moderation.matchesRequestedVersion` と `moderation.sourceVersion` を確認してください。
...
- キュー待ち中は `queue.queuedAhead` と `queue.position` を返すため、クライアントはそのリクエストより前にある優先手動スキャンの数を表示できます。非常に大きいキューは上限が設定され、`queuedAheadIsEstimate: true` として報告されます。
...
- スキルまたはPluginに対する所有者/公開者の管理アクセス権、またはプラットフォームのモデレーター/管理者権限が必要です。
...
- レスポンスはセキュリティ専用です。スキルカードデータ、生成済みカードステータス、アーティファクトファイルリスト、または詳細なスキャナーペイロードは含まれません。
...
- `category` (任意): plugin カテゴリフィルタ。リクエストが plugin パッケージ (`/api/v1/plugins`、
  ...
  にスコープされている場合のみサポートされます。管理対象カテゴリとレガシー v1
...
- 認証済みの呼び出し元は、自分が所属する発行者の private パッケージを一覧/検索結果で表示できます。
...
- `channel` (任意): `official`、`community`、または `private`
- `isOfficial` (任意): `true` または `false`
- `category` (任意): plugin カテゴリフィルタ。リクエストが plugin パッケージにスコープされている場合のみサポートされます。管理対象カテゴリとレガシー v1
  フィルタエイリアスは `GET /api/v1/plugins` に記載されています。
...
- 匿名呼び出し元には公開パッケージチャネルのみが表示されます。
- 認証済みの呼び出し元は、自分が所属する発行者の private パッケージを検索できます。
- `channel=private` は、認証済みの呼び出し元が読み取れるパッケージのみを返します。
...
レガシー v1 フィルタエイリアスは、読み取りエンドポイントで引き続き受け入れられます。
...
レガシーエイリアスは、保存されるカテゴリ値または作者が宣言するカテゴリ値としては受け入れられません。
...
- `clean` または `suspicious` のスキャンを持つ現在の GitHub backed skills には、
  ...
  コンテンツハッシュ、アーカイブ URL が含まれます。ClawHub ホストのソースファイルは含まれません。
...
- 本文: ZIP アーカイブ。
- エクスポートされた各プラグインは `{family}/{packageName}/` をルートとします。
...
code-plugin と bundle-plugin パッケージを対象にしたプラグイン専用検索。
...
- `GET /api/v1/plugins` で文書化されているレガシー v1 フィルターエイリアスも
  受け付けます。
- カテゴリーフィルタリングは、検索クエリの書き換えではなく、プラグインカテゴリーのダイジェスト
...
- パッケージ所有者、組織パブリッシャーの所有者/管理者、プラットフォームモデレーター、またはプラットフォーム管理者の
  API トークンが必要です。
...
- `version.sha256hash` は旧クライアント向けの非推奨互換メタデータです。これは
...
- `trust.blockedFromDownload` はインストールブロックのシグナルです。OpenClaw およびその他の
  インストールクライアントは、スキャナーやモデレーションフィールドからブロックルールを再導出する代わりに、
...
- このエンドポイントは、所有者/モデレーターのモデレーションエンドポイントより意図的に狭くなっています。
  インストール判断と公開説明を公開し、報告者の身元、報告本文、非公開の証拠、内部レビューの
  タイムラインは公開しません。
...
公式 OpenClaw プラグイン移行行を一覧表示するためのモデレーターエンドポイント。
...
公式プラグイン移行行を作成または更新するための管理者エンドポイント。
...
バージョンにリンクされます。報告はモデレーションキューに送られますが、それ自体でダウンロードを
...
パッケージのモデレーション可視性用のオーナー/モデレーターエンドポイント。
...
`note` は `confirmed` と `dismissed` では必須です。`status` を `open` に戻す場合は省略できます。確認済みレポートに対して `finalAction: "quarantine"` または
...
隔離済みおよび取り消し済みのリリースでは、アーティファクトダウンロードルートから `403` が返ります。
...
パッケージリリース用のレガシーな決定論的 ZIP アーカイブをダウンロードします。
...
- プライベートパッケージは、呼び出し元がオーナーでない限り `404` を返します。

### `GET /api/npm/{package}`

ClawPack に基づくパッケージバージョン向けに npm 互換の packument を返します。
...
ホストされたスキルバージョン ZIP をダウンロードするか、`clean` または `suspicious` スキャンがあり、ホストされたバージョンがない現在の GitHub backed スキルについて、GitHub ソースへの引き渡しを返します。
...
- GitHub backed スキルの引き渡しは、バイトをプロキシまたはミラーしません。JSON レスポンスには
  ...
  および `archiveUrl` が含まれます。スキャン/現在の状態はゲートであり、成功ペイロードメタデータとしては含まれません。
...
- 任意のペイロードフィールド: `migrateOwner`。`ownerHandle` とともに `true` の場合、アクターが現在および対象の両方のパブリッシャーで管理者/オーナーであれば、既存のスキルをそのオーナーに移動できます。この明示的な指定がない場合、オーナー変更は拒否されます。
...
- 同じリクエスト内で `files` と `clawpack` の両方を使用せず、どちらか一方を使用してください。
- JSON body と、呼び出し元が指定した `payload.files` / `payload.artifact`
  メタデータは拒否されます。
- 直接 multipart 公開リクエストの上限は 18MB です。ClawPack tarball は upload-url フローを使用して、120MB tarball 上限まで利用できます。
- 任意のペイロードフィールド: `ownerHandle`。存在する場合、そのオーナーの代理で公開できるのは管理者のみです。

検証の要点:

- `family` は `code-plugin` または `bundle-plugin` である必要があります。
- Plugin パッケージには `openclaw.plugin.json` が必要です。ClawPack `.tgz` アップロードでは、それを `package/openclaw.plugin.json` に含める必要があります。
...
- 代理公開でも、対象オーナーアカウントに対して official-channel の資格が検証されます。
...
オーナーが開始したソフト削除では slug が 30 日間予約され、その後は別のパブリッシャーが
...
管理者専用。Convex Auth アカウント行を編集せずに、検証済みの代替 GitHub OAuth principal の個人パブリッシャーを復旧します。
リクエストでは、不変の GitHub
provider account id を両方とも指定する必要があります。変更可能なハンドルは、オペレーター向けのガードとしてのみ使用されます。

エンドポイントはデフォルトでドライランになります。リカバリを適用するには、スタッフが両方の
GitHub プリンシパル間の継続性を独立して確認した後に、`dryRun: false` と
`confirmIdentityVerified: true` が必要です。移行先ユーザーの現在の個人 publisher に
skills、packages、または GitHub skill sources がある場合、リカバリは失敗して閉じます。
リカバリでは、復元された publisher の skills、skill slug aliases、packages、package inspector warnings、
および派生 search digest rows のレガシー `ownerUserId` フィールドも移行されるため、
直接所有者パスは新しい publisher 権限と一致します。復元された handle の有効な protected-handle
reservation も置換ユーザーに再割り当てされるため、後続のプロフィール同期で以前のユーザーの競合する権限が復元されることはありません。各プライマリテーブルは
適用トランザクションごとに 100 行に制限されます。より大きなリカバリでは、まず再開可能な owner migration を使用する必要があります。
GitHub skill sources は publisher スコープであり、書き換えではなくチェック済みとして報告されます。
...
ユーザーを ban し、所有する skills をハード削除します (moderator/admin のみ)。
...
CLI はサイトから registry/auth settings を検出できます。
