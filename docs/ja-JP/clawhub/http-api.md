...
x-i18n:
    generated_at: "2026-07-03T17:12:33Z"
    model: gpt-5.5
...
サードパーティのディレクトリは、公開読み取りエンドポイントを使って ClawHub skills を一覧表示または検索できます。結果をキャッシュし、`429`/`Retry-After` を尊重し、ユーザーを正規の ClawHub 掲載ページ（`https://clawhub.ai/<owner>/skills/<slug>`）へリンクし、サードパーティサイトを ClawHub が推奨しているように示唆しないでください。非表示、非公開、またはモデレーションでブロックされたコンテンツを公開 API サーフェスの外にミラーしようとしないでください。

Web slug ショートカットはレジストリファミリーを横断して解決されますが、API クライアントはルート優先順位を再構築するのではなく、読み取りエンドポイントから返される正規 URL を使用する必要があります。
...
- token がない、または無効な場合、挙動は IP による適用へフォールバックします。
- 認証済みの書き込みエンドポイントは、サーバーが理由を把握している場合、単なる `Unauthorized` を返すべきではありません。token の欠落、無効化または取り消された token、削除済み、禁止済み、または無効化されたアカウントには、それぞれ実行可能なテキストを返し、CLI クライアントが何によりユーザーがブロックされたかを伝えられるようにする必要があります。
...
これには、バリデーション失敗（`400`）、公開リソースの欠落（`404`）、認証および権限失敗（`401`/`403`）、レート制限（`429`）、ブロックされたダウンロードが含まれます。クライアントはレスポンス本文を人間が読める文字列として読み取る必要があります。未知のクエリパラメーターは互換性のため無視されますが、認識されるクエリパラメーターに無効な値がある場合は `400` を返します。
...
- 結果は関連度順（埋め込み類似度 + 完全な slug/name トークンのブースト + 小さな人気度事前分布）で返されます。
...
- 疑わしい、または非表示のモデレーション状態により、呼び出し元のフィルターと現在のモデレーションステータスに応じて、skill が公開検索から除外されることがあります。

公開者向けの発見可能性ガイダンス:

- ユーザーが文字どおり検索する用語を、表示名、summary、tags に入れてください。独立した slug トークンは、それも保持したい安定したアイデンティティである場合にのみ使用してください。
- 新しい slug がより良い長期的な正規名でない限り、1 つのクエリを追うためだけに slug を変更しないでください。古い slug はリダイレクトエイリアスになりますが、正規 URL、表示される slug、将来の検索ダイジェストは新しい slug を使用します。
- 名前変更エイリアスは、古い URL とレジストリ経由で解決されるインストールの解決を維持しますが、検索ランキングは名前変更がインデックス化された後の正規 skill メタデータに基づきます。既存の統計は skill に残ります。
...
- `sort`（任意）: `updated`（デフォルト）、`recommended`（エイリアス: `default`）、`createdAt`（エイリアス: `newest`）、`downloads`、`stars`（エイリアス: `rating`）、従来のインストールエイリアス `installsCurrent`/`installs`/`installsAllTime` は `downloads` にマッピング、`trending`
- `nonSuspiciousOnly`（任意）: 疑わしい（`flagged.suspicious`）skills を非表示にするには `true`
- `nonSuspicious`（任意）: `nonSuspiciousOnly` の従来のエイリアス
...
- `createdAt` は新しい skill のクロールに対して安定しています。`updated` は既存の skills が再公開されたときに変わります。
- `nonSuspiciousOnly=true` の場合、疑わしい skills はページ取得後にフィルタリングされるため、カーソルベースのソートではページ内の項目数が `limit` より少なくなることがあります。
- 存在する場合は `nextCursor` を使用してページネーションを続行します。短いページだけでは、それ自体で結果の終端を意味しません。
...
- owner の名前変更/マージフローによって作成された古い slug は、正規の skill に解決されます。
...
- `moderation` は skill が flagged されている場合、または owner が表示している場合にのみ含まれます。
...
- owners と moderators は非表示 skills のモデレーション詳細にアクセスできます。
- 公開呼び出し元は、すでに flagged された表示可能な skills に対してのみ `200` を取得します。
- 証拠は公開呼び出し元向けには編集され、owners/moderators 向けにのみ生のスニペットを含みます。

### `POST /api/v1/skills/{slug}/report`

モデレーターによるレビューのために skill を報告します。報告は skill レベルで、任意でバージョンにリンクされ、skill 報告キューへ送られます。
...
skill 報告受け付け用の moderator/admin エンドポイント。
...
skill 報告の解決または再オープン用の moderator/admin エンドポイント。
...
- 公開済みスキャンには、owner/publisher の管理アクセス、またはプラットフォームのモデレーター/管理者権限が必要です。
...
- キューに入っている間は `queue.queuedAhead` と `queue.position` を返すため、クライアントはリクエストより前にある優先手動スキャンの数を表示できます。非常に大きなキューは上限が設定され、`queuedAheadIsEstimate: true` として報告されます。
...
- スキルまたは Plugin への owner/publisher 管理アクセス、またはプラットフォームのモデレーター/管理者権限が必要です。
...
管理者専用の正規バッチ再スキャンルート。レガシー `POST /api/v1/skills/-/rescan-batch` と同じペイロード形状を受け付けます。

### `POST /api/v1/skills/-/scan/batch/status`

管理者専用の正規バッチステータスルート。`{ "jobIds": ["..."] }` を受け付け、レガシー `POST /api/v1/skills/-/rescan-batch/status` と同じ集計カウンターを返します。
...
- スキル ID、publisher ID、選択されたバージョンのメタデータはトップレベルのエンベロープフィールド（`slug`、`displayName`、`publisherHandle`、`version`、`resolvedFrom`、`tag`、`createdAt`）であるため、シェル自動化はネストされたラッパーを展開せずにそれらを読み取れます。
- `security` はトップレベルの ClawScan/security 判定です。自動化は `ok`、`decision`、`reasons`、`security.status` をキーにする必要があります。
- `security.signals` には、`staticScan`、`virusTotal`、`skillSpector` などのスキャナーの補助証拠が含まれます。
...
- レスポンスはセキュリティ専用です。Skill Card データ、生成済みカードのステータス、アーティファクトファイルリスト、詳細なスキャナーペイロードは含まれません。
- `security.signals` には、ステータスレベルの補助証拠のみが含まれます。完全なスキャナー詳細には `/scan` または ClawHub の security-audit ページを使用してください。
- `security.signals.dependencyRegistry` は v1 レスポンス互換性のために保持されていますが、依存関係レジストリ存在スキャナーは廃止されており、このキーは常に `null` です。
...
- `category` (任意): Pluginカテゴリフィルター。リクエストがPluginパッケージにスコープされている場合
  ...
  `family=code-plugin`/`family=bundle-plugin` を指定したパッケージエンドポイント) のみサポートされます。制御カテゴリと
...
- 認証済みの呼び出し元は、自分が属する公開者のプライベートパッケージを一覧/検索結果で表示できます。
...
- `channel` (任意): `official`、`community`、または `private`
- `isOfficial` (任意): `true` または `false`
- `category` (任意): Pluginカテゴリフィルター。リクエストがPluginパッケージにスコープされている場合のみサポートされます。制御カテゴリとレガシー v1
...
- 匿名の呼び出し元には公開パッケージチャネルのみが表示されます。
- 認証済みの呼び出し元は、自分が属する公開者のプライベートパッケージを検索できます。
- `channel=private` は、認証済みの呼び出し元が読み取れるパッケージのみを返します。

### `GET /api/v1/plugins`

コード Plugin とバンドル Plugin パッケージを横断するPlugin専用カタログ閲覧です。
...
オフライン分析用に、最新の公開Skillsを一括エクスポートします。
...
- `startDate` (必須): Skill の `updatedAt` に対する下限の Unix ミリ秒。
- `endDate` (必須): Skill の `updatedAt` に対する上限の Unix ミリ秒。
...
- 本文: ZIPアーカイブ。
- エクスポートされる各Skillは `{publisher}/{slug}/` をルートにします。
- ホストされているSkillsには、保存されている最新バージョンのファイルが含まれ、
  `_manifest.json` に `sourceRef: "public-clawhub"` として一覧表示されます。
- `clean` または `suspicious` のスキャンを持つ現在の GitHub ベースのSkillsには、
  `sourceRef: "public-github"`、リポジトリ、コミット、パス、
  コンテンツハッシュ、アーカイブURLを含む `_source_handoff.json` が含まれます。ClawHubでホストされるソースファイルは含まれません。
...
オフライン分析向けに、最新の公開プラグインリリースを一括エクスポートします。
...
- `startDate` (必須): プラグイン `updatedAt` の Unix ミリ秒の下限。
- `endDate` (必須): プラグイン `updatedAt` の Unix ミリ秒の上限。
- `limit` (任意): 整数 (1-250)、デフォルトは `250`。
- `cursor` (任意): 前回のレスポンスからのページネーションカーソル。
...
- 個別のプラグインまたはファイルをエクスポートできなかった場合、
  `_errors.json` が含まれます。
...
- `GET /api/v1/plugins` の下に文書化されている従来の v1 フィルターエイリアスも
  受け付けられます。
- カテゴリーフィルタリングは、検索クエリの書き換えではなく、プラグインカテゴリーダイジェスト
  行に裏付けられた実際の API フィルターです。
- 結果は関連度順で返され、現在はページネーションされません。
- プラグイン検索のブラウザー UI ソートコントロールは、読み込まれた関連度結果を並べ替え、
...
インストールクライアント向けに、正確なパッケージリリースのセキュリティと信頼の要約を返します。
...
- `package.name`、`package.displayName`、`package.family` は、解決済みレジストリパッケージを
  識別します。
...
- `trust.stale` は、信頼の要約が古い入力から計算されたことを意味し、
  高信頼度の許可判断の前に更新が必要なものとして扱うべきです。

注記:

- このエンドポイントはバージョン厳密です。クライアントは、最新のパッケージメタデータを
  読み取った直後ではなく、インストールする予定のパッケージバージョンを解決した後に
  呼び出すべきです。
- 非公開パッケージは、呼び出し元が所有パブリッシャーを読み取れる場合を除き `404` を返します。
- このエンドポイントは、所有者/モデレーター向けのモデレーションエンドポイントよりも意図的に狭くなっています。
  インストール判断と公開説明を公開し、報告者の身元、報告本文、非公開の証拠、
  内部レビューのタイムラインは公開しません。
...
- これは移行の準備状態のみを追跡します。OpenClaw を変更したり ClawPacks を生成したりはしません。
...
自動的に非表示またはブロックすることはありません。モデレーターは、アーティファクトを承認、
隔離、または取り消すためにリリースモデレーションを使用するべきです。
...
パッケージのモデレーション表示用の所有者/モデレーターエンドポイント。
...
- `revoked`: 以前信頼されていたリリースが後からブロックされています。

隔離済みおよび取り消し済みのリリースでは、アーティファクトダウンロードルートが `403` を返します。
...
- 呼び出し元が所有者である場合を除き、プライベートパッケージは `404` を返します。

### `GET /api/npm/{package}`

ClawPack が裏付けるパッケージバージョンの npm 互換 packument を返します。
...
ホストされたスキルバージョン ZIP をダウンロードします。または、`clean` か `suspicious` のスキャン結果を持ち、ホスト済み
...
- 任意のペイロードフィールド: `migrateOwner`。`ownerHandle` とともに `true` の場合、
  ...
  既存のスキルをその所有者に移動できます。この明示的な指定がない場合、所有者変更は
  拒否されます。
...
- 許可されるフォームフィールドは、`payload`、繰り返しの `files` blob、または 1 つの `clawpack`
  ...
  upload-url フローで返されたストレージ ID にできます。ステージング済み storage-id 公開では、そのアップロード URL とともに返された
...
- `official` チャネルに公開できるのは、`openclaw` org パブリッシャーと現在の `openclaw` org メンバーの
  個人パブリッシャーのみです。
- 代理公開でも、official チャネルの適格性は対象所有者アカウントに対して検証されます。
...
所有者主導の論理削除では slug が 30 日間予約され、その後、別のパブリッシャーが
その slug を取得できるようになります。この有効期限が適用される場合、削除レスポンスには `slugReservedUntil` が含まれます。
モデレーター/管理者による非表示化およびセキュリティ削除は、このようには期限切れになりません。
...
管理者専用です。ハンドルに対する org パブリッシャーが存在することを保証します。ハンドルがまだ
レガシーな共有ユーザー/個人パブリッシャーを指している場合、このエンドポイントはまずそれを org パブリッシャーに移行します。
新規作成される org では `memberHandle` を指定します。操作する管理者はメンバーとして追加されません。
...
認証済みセルフサービスの org パブリッシャー作成です。新しい org パブリッシャーを作成し、
...
- ハンドルがすでにパブリッシャー、ユーザー、または個人パブリッシャーに使用されている場合は `409` を返します。

### `POST /api/v1/users/reserve`

管理者専用です。リリースを公開せずに、正当な所有者のためにルート slug とパッケージ名を予約します。
...
管理者専用です。Convex Auth アカウント行を編集せずに、検証済みの代替 GitHub OAuth プリンシパル用に
個人パブリッシャーを復旧します。リクエストでは、不変の GitHub
プロバイダーアカウント ID の両方を指定する必要があります。変更可能なハンドルは、オペレーター向けのガードとしてのみ使用されます。
...
Skills、パッケージ、または GitHub skill sources がある場合、復旧はフェイルクローズします。
復旧では、復旧対象パブリッシャーの Skills、skill slug aliases、パッケージ、package inspector warnings、派生 search digest rows の従来の `ownerUserId` フィールドも移行されるため、
直接所有者パスは新しいパブリッシャー権限と一致します。復旧対象ハンドルのアクティブな protected-handle
reservation も置換ユーザーに再割り当てされるため、後続の
profile synchronization によって以前のユーザーの競合する権限が復元されることはありません。各 primary table は
適用トランザクションごとに 100 行に制限されます。より大きな復旧では、まず再開可能な owner migration を使用する必要があります。
GitHub skill sources はパブリッシャースコープであり、書き換えられるのではなくチェック済みとして報告されます。
...
- `merge` はソースの掲載を非表示にし、ソース slug をターゲットの掲載へリダイレクトします。
...
セルフホストする場合は、このファイルを配信してください (または `CLAWHUB_REGISTRY` を明示的に設定します。従来の `CLAWDHUB_REGISTRY`)。
