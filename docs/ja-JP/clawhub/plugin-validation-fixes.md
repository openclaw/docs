...
read_when:
    ...
    - ClawHub が Plugin パッケージの公開時に拒否または警告を出した
    - リリース前に Plugin パッケージメタデータを更新しています
summary: ClawHub Plugin パッケージの検証結果を公開前に修正する
title: Plugin 検証の修正
x-i18n:
    generated_at: "2026-07-04T20:24:08Z"
    model: gpt-5.5
...
ClawHub は公開前に Plugin パッケージを検証し、自動パッケージスキャンの検出事項も表示できます。このページでは、作者向けの検出事項を扱います。つまり、Plugin 作者が自分のパッケージメタデータ、マニフェスト、SDK インポート、または公開済みアーティファクトで修正できる検出事項です。

内部的な Plugin Inspector のカバレッジ検出事項は扱いません。完全なレポートに、作者向けの修正ガイダンスがないスキャナーメンテナンスコードが含まれる場合、それらは Plugin 作者ではなく OpenClaw メンテナー向けです。
...
| `provider-auth-env-vars`                | [プロバイダー env var をセットアップメタデータへ移動する](/ja-JP/clawhub/plugin-validation-fixes#provider-auth-env-vars)                             |
| `channel-env-vars`                      | [チャンネル env var を現在のメタデータにミラーする](/ja-JP/clawhub/plugin-validation-fixes#channel-env-vars)                                |
...
- 最小パッケージの例は [Plugin の構築](/ja-JP/plugins/building-plugins) を、パッケージとマニフェストの分割については [Plugin マニフェスト](/ja-JP/plugins/manifest#manifest-versus-packagejson) を使用します。
...
- 公開されたパッケージでビルド済み JavaScript をロードする必要がある場合は、`openclaw.runtimeExtensions` を追加します。
...
- `npm pack --dry-run` を実行し、含まれるファイルを確認します。
...
Plugin が、非推奨のセッションストア全体ヘルパー `loadSessionStore` をまだ使用しています。
...
Plugin が、非推奨のトランスクリプトファイルターゲットヘルパー `resolveSessionTranscriptLegacyFileTarget` をまだ使用しています。
...
- モデルまたはプロバイダーの上書き処理を `before_model_resolve` に移動します。
...
マニフェストが、ClawHub が期待する現在の setup または config メタデータのない、レガシーまたは古いチャンネル環境変数メタデータを使用しています。

- OpenClaw がチャンネルランタイムを読み込まずに setup 状態を検査できるように、チャンネル環境変数メタデータを宣言的に保ちます。
- 環境変数駆動のチャンネル setup を、現在の setup、チャンネル config、または Plugin 形状で使用されるパッケージチャンネルメタデータにミラーします。
- 古いサポート対象 OpenClaw バージョンがまだそれを必要とする間だけ、互換性メタデータとして `channelEnvVars` を維持します。
...
- マニフェスト契約が存在するまでは、セキュリティに関連する動作を公開パッケージドキュメントまたは README に記載しておきます。
- `clawhub package validate <path-to-plugin>` を再実行します。
