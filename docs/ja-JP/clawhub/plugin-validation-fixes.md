...
summary: ClawHub Plugin パッケージの検証で見つかった問題を公開前に修正する
title: Plugin 検証の修正
x-i18n:
    generated_at: "2026-07-06T10:47:21Z"
    model: gpt-5.5
...
内部的な Plugin Inspector のカバレッジ検出結果は扱いません。完全なレポートに、作者向けの修正ガイダンスがないスキャナー保守コードが含まれている場合、それらは Plugin 作者ではなく OpenClaw メンテナー向けです。
...
| `channel-env-vars`                      | [チャンネル env var を現在のメタデータに反映する](/ja-JP/clawhub/plugin-validation-fixes#channel-env-vars)                                |
...
- ビルドとテストの対象にした OpenClaw Plugin API バージョンまたは semver の下限を使用します。
- これはパッケージバージョンとは別に保ちます。パッケージバージョンは Plugin リリースを表し、`openclaw.compat.pluginApi` はホスト API コントラクトを表します。
- [検出に影響する package.json フィールド](/ja-JP/plugins/manifest#packagejson-fields-that-affect-discovery)を参照してください。
- `clawhub package validate <path-to-plugin>` を再実行します。

### package-min-host-version-drift

パッケージの最小ホストバージョンが、そのパッケージがビルドされた OpenClaw バージョンメタデータと一致していません。
...
- パッケージエントリーポイント、互換性、インストール、セットアップ、カタログのメタデータは、サポートされている `package.json#openclaw` フィールドに保ちます。
- [検出に影響する package.json フィールド](/ja-JP/plugins/manifest#packagejson-fields-that-affect-discovery)を参照してください。
- `clawhub package validate <path-to-plugin>` を再実行します。
...
- 宣言した互換性範囲が、これを必要とする古い OpenClaw バージョンをまだサポートしている間だけ、
...
Plugin はまだ、非推奨の transcript ファイルターゲットヘルパー
...
マニフェストは、ClawHub が期待する現在の setup または設定メタデータのない、レガシーまたは古いチャンネル環境変数メタデータを使用しています。

- OpenClaw がチャンネルランタイムを読み込まずに setup 状態を検査できるように、チャンネル環境変数メタデータは宣言的に保ちます。
- 環境変数駆動のチャンネル setup を、自分の Plugin 形状で使用される現在の setup、チャンネル設定、または
...
パッケージは、ClawHub が利用可能として認識しないスキーマ参照を持つ `openclaw.security.json` を同梱しています。
...
- マニフェスト contract が存在するまでは、セキュリティに影響する動作を公開パッケージドキュメントまたは
  README に記載します。
- `clawhub package validate <path-to-plugin>` を再実行します。
