...
title: Plugin検証の修正
x-i18n:
    generated_at: "2026-07-05T17:40:41Z"
    model: gpt-5.5
...
ClawHub は公開前に Plugin パッケージを検証し、自動パッケージスキャンの結果も表示できます。このページでは、作者向けの検出結果を扱います。つまり、Plugin 作者が自分のパッケージメタデータ、マニフェスト、SDK インポート、または公開済みアーティファクトで修正できる検出結果です。

内部の Plugin Inspector カバレッジ検出結果は扱いません。完全なレポートに、作者向けの修正ガイダンスがないスキャナーメンテナンスコードが含まれる場合、それらは Plugin 作者ではなく OpenClaw メンテナー向けです。
...
- 最小構成のパッケージ例には [Plugin の構築](/ja-JP/plugins/building-plugins) を、パッケージとマニフェストの分担には [Plugin マニフェスト](/ja-JP/plugins/manifest#manifest-versus-packagejson) を参照してください。
...
パッケージは OpenClaw エントリーポイントを宣言していますが、参照先ファイルが検証対象のパッケージにありません。
...
- パッケージエントリーポイント、互換性、インストール、セットアップ、カタログメタデータは、サポートされる `package.json#openclaw` フィールドに保持します。
- [検出に影響する package.json フィールド](/ja-JP/plugins/manifest#packagejson-fields-that-affect-discovery)を参照してください。
- `clawhub package validate <path-to-plugin>` を再実行します。
...
- パックに失敗する原因となる無効なパッケージメタデータ、壊れたライフサイクルスクリプト、または files エントリーを修正します。
...
- channel entry ヘルパーには `openclaw/plugin-sdk/channel-core` を使用します。
...
- サポート対象の OpenClaw 範囲がまだそれを必要としている間だけ、`providerAuthEnvVars` を互換性メタデータとして保持します。
...
マニフェストが、ClawHub が期待する現在の setup または config メタデータを伴わない、レガシーまたは古い channel env-var メタデータを使用しています。

- OpenClaw が channel ランタイムを読み込まずに setup 状態を検査できるように、channel env-var メタデータは宣言的に保ちます。
- env 駆動の channel setup を、自分の Plugin 形状で使用される現在の setup、channel config、またはパッケージ channel メタデータにミラーします。
- 古いサポート対象 OpenClaw バージョンがまだそれを必要としている間だけ、`channelEnvVars` を互換性メタデータとして保持します。
...
- それが助言目的だけである場合は、スキーマ URL を削除します。
...
- マニフェスト contract が存在するまでは、セキュリティに敏感な動作を公開パッケージドキュメントまたは README に記載しておきます。
- `clawhub package validate <path-to-plugin>` を再実行します。
