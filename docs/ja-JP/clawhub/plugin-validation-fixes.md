---
read_when:
    - clawhub package validate を実行して、Pluginの指摘事項を修正する必要があります
    - ClawHub が Plugin パッケージの公開を拒否または警告した
    - リリース前にPluginパッケージのメタデータを更新しています
summary: 公開前に ClawHub Plugin パッケージ検証の指摘を修正
title: Plugin 検証の修正
x-i18n:
...
ClawHub は公開前に Plugin パッケージを検証し、自動パッケージスキャンの検出事項も表示できます。このページでは、作者向けの検出事項、つまり Plugin 作者がパッケージメタデータ、マニフェスト、SDK インポート、または公開済み成果物で修正できる検出事項を扱います。

内部 Plugin Inspector のカバレッジ検出事項は扱いません。完全なレポートに、作者向けの修正ガイダンスがないスキャナー保守コードが含まれる場合、それらは Plugin 作者ではなく OpenClaw メンテナー向けです。
...
| `package-openclaw-unsupported-metadata` | [未対応の OpenClaw パッケージメタデータを削除する](/ja-JP/clawhub/plugin-validation-fixes#package-openclaw-unsupported-metadata)          |
| `package-npm-pack-unavailable`          | [npm 成果物をパック可能にする](/ja-JP/clawhub/plugin-validation-fixes#package-npm-pack-unavailable)                                 |
| `package-npm-pack-entrypoint-missing`   | [npm パック出力にエントリポイントを含める](/ja-JP/clawhub/plugin-validation-fixes#package-npm-pack-entrypoint-missing)                  |
| `package-npm-pack-metadata-missing`     | [npm パック出力にメタデータを含める](/ja-JP/clawhub/plugin-validation-fixes#package-npm-pack-metadata-missing)                       |
| `manifest-name-missing`                 | [マニフェスト表示名を追加する](/ja-JP/clawhub/plugin-validation-fixes#manifest-name-missing)                                           |
| `manifest-unknown-fields`               | [未対応のマニフェストフィールドを削除する](/ja-JP/clawhub/plugin-validation-fixes#manifest-unknown-fields)                                  |
| `manifest-unknown-contracts`            | [未対応のコントラクトキーを削除する](/ja-JP/clawhub/plugin-validation-fixes#manifest-unknown-contracts)                                 |
...
| `unrecognized-security-manifest`        | [未対応のセキュリティマニフェストファイルを削除する](/ja-JP/clawhub/plugin-validation-fixes#unrecognized-security-manifest)                   |
...
- 最小パッケージ例には [Plugin の構築](/ja-JP/plugins/building-plugins) を、パッケージとマニフェストの分離については [Plugin マニフェスト](/ja-JP/plugins/manifest#manifest-versus-packagejson) を使用します。
...
- パッケージを ClawHub で公開またはインストールする場合は、互換性メタデータとインストールメタデータを追加します。
...
パッケージは OpenClaw エントリポイントを宣言していますが、参照先ファイルが検証対象パッケージにありません。
...
- `openclaw.install` に、`clawhubSpec`、`npmSpec`、`localPath` など、対応しているインストールソースを入力します。
...
パッケージは、対応する OpenClaw Plugin API 範囲を宣言していません。
...
- 最小ホストバージョンを、パッケージが実際に対応するホストバージョン範囲に揃えます。
- [検出に影響する package.json フィールド](/ja-JP/plugins/manifest#packagejson-fields-that-affect-discovery)を参照してください。
- `clawhub package validate <path-to-plugin>` を再実行します。
...
- `openclaw.plugin.json` にも `version` がある場合は、それを一致するように更新するか、パッケージメタデータが信頼できる情報源である場合は古いマニフェストバージョンメタデータを削除します。
...
`package.json#openclaw` ブロックに、OpenClaw パッケージメタデータとして対応していないフィールドが含まれています。

- `openclaw.bundle` などの未対応フィールドを削除します。
- ネイティブ Plugin メタデータは `openclaw.plugin.json` に保ちます。
- パッケージエントリポイント、互換性、インストール、セットアップ、カタログメタデータは、対応している `package.json#openclaw` フィールドに保ちます。
- [検出に影響する package.json フィールド](/ja-JP/plugins/manifest#packagejson-fields-that-affect-discovery)を参照してください。
- `clawhub package validate <path-to-plugin>` を再実行します。
...
パッケージを、ClawHub が検査または公開する成果物としてパックできません。
...
- パッケージがネイティブ OpenClaw Plugin である場合は、`openclaw.plugin.json` が含まれていることを確認します。
...
- `name` は人間が読める形式にし、`id` は安定した機械用 ID として維持します。
- [Plugin マニフェスト](/ja-JP/plugins/manifest)を参照してください。
- `clawhub package validate <path-to-plugin>` を再実行します。
...
- 宣言済みの互換性範囲が、それを必要とする古い OpenClaw バージョンをまだサポートしている場合に限り、`loadSessionStore(...)` を保持します。
...
- セッションストアオブジェクト全体を読み込み、変更し、保存することは避けます。
- 宣言済みの互換性範囲が、それらを必要とする古い OpenClaw バージョンをまだサポートしている場合に限り、ストア全体書き込みヘルパーを保持します。
- [ランタイム API](/ja-JP/plugins/sdk-runtime#agent-session-state)と
  [Plugin SDK サブパス](/ja-JP/plugins/sdk-subpaths)を参照してください。
- `clawhub package validate <path-to-plugin>` を再実行します。
...
Plugin はまだ非推奨のトランスクリプトファイルターゲットヘルパー
...
- 宣言済みの互換性範囲が、それを必要とする古い OpenClaw バージョンをまだサポートしている場合に限り、レガシーヘルパーを保持します。
- [ランタイム API](/ja-JP/plugins/sdk-runtime#agent-session-state)と
  [Plugin SDK サブパス](/ja-JP/plugins/sdk-subpaths)を参照してください。
- `clawhub package validate <path-to-plugin>` を再実行します。
...
Plugin はまだ `appendSessionTranscriptMessage` や `emitSessionTranscriptUpdate` などの非推奨の低レベルトランスクリプトヘルパーを使用しています。
...
- 宣言済みの互換性範囲が、それらを必要とする古い OpenClaw バージョンをまだサポートしている場合に限り、低レベルトランスクリプトヘルパーを保持します。
- [ランタイム API](/ja-JP/plugins/sdk-runtime#agent-session-state)と
  [Plugin SDK サブパス](/ja-JP/plugins/sdk-subpaths)を参照してください。
- `clawhub package validate <path-to-plugin>` を再実行します。
...
- プロンプトまたはコンテキスト変更作業は `before_prompt_build` に移動します。
- 宣言済みの互換性範囲が、それを必要とする古い OpenClaw バージョンをまだサポートしている場合に限り、`before_agent_start` を保持します。
...
- サポート対象の OpenClaw 範囲がまだそれを必要としている間だけ、互換性メタデータとして `providerAuthEnvVars` を保持します。
...
マニフェストは、ClawHub が期待する現在の setup または config メタデータなしに、レガシーまたは古いチャンネル env-var メタデータを使用しています。

- OpenClaw がチャンネルランタイムを読み込まずにセットアップ状態を検査できるように、チャンネル env-var メタデータは宣言的に保ちます。
- env 駆動のチャンネルセットアップを、現在の setup、チャンネル config、または Plugin の形状で使われるパッケージチャンネルメタデータにミラーします。
- 古いサポート対象の OpenClaw バージョンがまだ必要としている間だけ、互換性メタデータとして `channelEnvVars` を保持します。
...
- マニフェスト契約が存在するまでは、セキュリティに関わる動作を公開パッケージドキュメントまたは README に文書化しておきます。
- `clawhub package validate <path-to-plugin>` を再実行します。
