...
title: Plugin 検証の修正
x-i18n:
    generated_at: "2026-07-05T04:57:29Z"
    model: gpt-5.5
...
ClawHub は公開前に Plugin パッケージを検証し、自動パッケージスキャンの検出結果も表示できます。このページでは、作者向けの検出結果を扱います。つまり、Plugin 作者が自分のパッケージメタデータ、マニフェスト、SDK インポート、または公開済みアーティファクトで修正できる検出結果です。
...
| `package-entrypoint-missing`            | [宣言したエントリポイントを公開](/ja-JP/clawhub/plugin-validation-fixes#package-entrypoint-missing)                                  |
...
| `manifest-name-missing`                 | [マニフェストの表示名を追加](/ja-JP/clawhub/plugin-validation-fixes#manifest-name-missing)                                           |
...
| `sdk-session-transcript-low-level`      | [低レベルトランスクリプトヘルパーを置き換え](/ja-JP/clawhub/plugin-validation-fixes#sdk-session-transcript-low-level)                       |
...
- 最小構成のパッケージ例には [Plugin の構築](/ja-JP/plugins/building-plugins) を、パッケージとマニフェストの分割には [Plugin マニフェスト](/ja-JP/plugins/manifest#manifest-versus-packagejson) を使用してください。
...
- パッケージを ClawHub を通じて公開またはインストールする場合は、互換性とインストールメタデータを追加します。
...
- 複数のインストール元が利用可能な場合は `openclaw.install.defaultChoice` を設定します。
...
- 構築およびテスト対象にした OpenClaw Plugin API バージョンまたは semver の下限を使用します。
...
`package.json#openclaw` ブロックには、OpenClaw パッケージメタデータとしてサポートされていないフィールドが含まれています。
...
- 無効なパッケージメタデータ、壊れたライフサイクルスクリプト、またはパックを失敗させる files エントリを修正します。
...
- ランタイム動作はプラグイン登録コードに移動し、`contracts` は静的なケイパビリティ所有メタデータに限定します。
- `clawhub package validate <path-to-plugin>` を再実行します。
...
プラグインが、同梱プラグインまたは内部互換性用に予約された SDK パスをインポートしています。
...
- レガシーなトランスクリプトファイルターゲットを直接読み取ったり構築したりすることを避けます。
...
プラグインがレガシーな `before_agent_start` フックをまだ使用しています。
...
- サポート対象の OpenClaw 範囲がまだ必要としている間だけ、互換性メタデータとして `providerAuthEnvVars` を保持します。
...
マニフェストが、ClawHub が想定する現在のセットアップまたは設定メタデータを持たない、レガシーまたは古いチャンネル env-var メタデータを使用しています。

- OpenClaw がチャンネルランタイムを読み込まずにセットアップ状態を検査できるよう、チャンネル env-var メタデータは宣言的に保ちます。
- env 駆動のチャンネルセットアップを、プラグイン形状で使用される現在のセットアップ、チャンネル設定、またはパッケージチャンネルメタデータにミラーします。
- 古いサポート対象 OpenClaw バージョンがまだ必要としている間だけ、互換性メタデータとして `channelEnvVars` を保持します。
...
パッケージが、ClawHub が利用可能と認識しないスキーマ参照を含む `openclaw.security.json` を同梱しています。

- それが助言目的のみであれば、スキーマ URL を削除します。
