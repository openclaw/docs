...
summary: 公開前に ClawHub プラグインパッケージ検証の指摘を修正する
title: Plugin 検証の修正
x-i18n:
...
ClawHub は公開前に Plugin パッケージを検証し、自動パッケージスキャンからの検出事項も表示できます。このページでは、作者向けの検出事項、つまり Plugin 作者がパッケージメタデータ、マニフェスト、SDK インポート、公開済みアーティファクトで修正できる検出事項を扱います。

内部の Plugin Inspector カバレッジ検出事項は扱いません。完全なレポートに、作者向けの修正ガイダンスがないスキャナーメンテナンスコードが含まれる場合、それらは Plugin 作者ではなく OpenClaw メンテナー向けです。
...
| `channel-env-vars`                      | [チャンネル環境変数を現在のメタデータに反映する](/ja-JP/clawhub/plugin-validation-fixes#channel-env-vars)                                |
...
- 最小限のパッケージ例については [Plugin の構築](/ja-JP/plugins/building-plugins)、パッケージとマニフェストの分割については [Plugin マニフェスト](/ja-JP/plugins/manifest#manifest-versus-packagejson) を使用します。
...
- `openclaw.install` に、`clawhubSpec`、`npmSpec`、`localPath` など、サポートされるインストールソースを入力します。
...
- ビルドおよびテストの対象にした OpenClaw Plugin API バージョン、または semver の下限を使用します。
...
- パッケージが実際にサポートするホストバージョン範囲に最小ホストバージョンをそろえます。
- [検出に影響する package.json フィールド](/ja-JP/plugins/manifest#packagejson-fields-that-affect-discovery)を参照してください。
- `clawhub package validate <path-to-plugin>` を再実行します。
...
- `openclaw.plugin.json` にも `version` がある場合、パッケージメタデータが正本であれば、それを一致するよう更新するか、古いマニフェストバージョンメタデータを削除します。
...
- パッケージがネイティブ OpenClaw Plugin の場合、`openclaw.plugin.json` が含まれていることを確認します。
...
- ランタイム動作は Plugin 登録コードに移動し、`contracts` は静的な capability 所有権メタデータに限定します。
- `clawhub package validate <path-to-plugin>` を再実行します。
...
- ルートバレルのインポートを、絞り込まれた公開サブパスインポートに置き換えます。
- `definePluginEntry` には `openclaw/plugin-sdk/plugin-entry` を使用します。
- チャネルエントリヘルパーには `openclaw/plugin-sdk/channel-core` を使用します。
- 絞り込まれたインポートを見つけるには、[インポート規約](/ja-JP/plugins/building-plugins#import-conventions)と
  [Plugin SDK サブパス](/ja-JP/plugins/sdk-subpaths)を使用します。
- `clawhub package validate <path-to-plugin>` を再実行します。

### reserved-sdk-import

Plugin が、同梱 Plugin または内部互換性のために予約されている SDK パスをインポートしています。
...
Plugin がまだ非推奨の transcript ファイルターゲットヘルパー
...
Plugin がまだ `appendSessionTranscriptMessage` や `emitSessionTranscriptUpdate` などの非推奨の低レベル transcript ヘルパーを使用しています。
...
- プロバイダー環境変数メタデータを `setup.providers[].envVars` に反映します。
...
マニフェストが、ClawHub が期待する現在の setup または config メタデータを伴わない、レガシーまたは古いチャネル環境変数メタデータを使用しています。

- OpenClaw がチャネルランタイムを読み込まずに setup 状態を検査できるよう、チャネル環境変数メタデータは宣言的に保ちます。
- 環境変数駆動のチャネル setup を、自分の Plugin 形状で使用される現在の setup、チャネル config、またはパッケージチャネルメタデータに反映します。
...
- アドバイザリー目的のみの場合は、スキーマ URL を削除します。
...
- マニフェスト契約が存在するまでは、セキュリティ上重要な動作を公開パッケージのドキュメントまたは README に記載しておきます。
- `clawhub package validate <path-to-plugin>` を再実行します。
