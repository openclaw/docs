...
ClawHub は公開前に Plugin パッケージを検証し、自動パッケージスキャンの検出事項も表示できます。このページでは、作成者向けの検出事項を扱います。つまり、Plugin 作成者が自分のパッケージメタデータ、マニフェスト、SDK インポート、または公開済み成果物で修正できる検出事項です。
...
- これはパッケージバージョンとは分けて管理します。パッケージバージョンは Plugin リリースを表し、`openclaw.compat.pluginApi` はホスト API 契約を表します。
- [検出に影響する package.json フィールド](/ja-JP/plugins/manifest#packagejson-fields-that-affect-discovery)を参照してください。
- `clawhub package validate <path-to-plugin>` を再実行します。
...
- `openclaw.plugin.json` にも `version` がある場合、パッケージメタデータが権威であるなら、それに一致するよう更新するか、古いマニフェストバージョンメタデータを削除します。
...
パッケージはパックできますが、パック済み成果物に `package.json#openclaw` で宣言されたエントリポイントファイルが含まれていません。
...
パック済み成果物に、ソースパッケージに存在する OpenClaw メタデータがありません。

- `npm pack --dry-run` を実行し、含まれるメタデータファイルを確認します。
- パック済み成果物内の `package.json` に `openclaw` ブロックが含まれていることを確認します。
...
Plugin が、バンドル済み Plugin または内部互換性向けに予約された SDK パスをインポートしています。
...
マニフェストが、ClawHub が期待する現在の setup または config メタデータを伴わない、レガシーまたは古いチャネル env-var メタデータを使用しています。

- OpenClaw がチャネルランタイムを読み込まずに setup 状態を検査できるよう、チャネル env-var メタデータは宣言的に保ちます。
- env 駆動のチャネル setup を、Plugin の形に応じて使用される現在の setup、チャネル config、またはパッケージチャネルメタデータにミラーします。
...
- マニフェスト契約が存在するまでは、セキュリティに関係する動作を公開パッケージドキュメントまたは README に記載しておきます。
- `clawhub package validate <path-to-plugin>` を再実行します。
