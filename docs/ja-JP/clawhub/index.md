...
read_when:
    ...
    - openclaw と clawhub の CLI フローを選択する
sidebarTitle: ClawHub
summary: 発見、インストール、公開、セキュリティ、および clawhub CLI 向けの公開 ClawHub 概要。
title: ClawHub
x-i18n:
    generated_at: "2026-07-06T10:47:19Z"
    model: gpt-5.5
...
ClawHub は semver バージョン、`latest` などのタグ、changelogs、ファイル、
...
ソースメタデータを保存するため、後続の更新コマンドも ClawHub を使い続けられます。

plugin のインストールを ClawHub 経由で解決する必要がある場合は、`clawhub:<package>` を使用します。
裸の npm-safe plugin spec はローンチ切り替え時に npm 経由で解決される場合があり、
...
ClawHub ダイジェストヘッダーとダウンロードされたバイト列を検証し、後続の更新用にアーティファクトメタデータを記録します。
...
アップロードせずに正確な公開計画を作成するには `--dry-run` を使用し、CI で扱いやすい出力には `--json`
を使用します。

コード plugins は、`package.json` に必須の OpenClaw 互換性メタデータを含める必要があります。
...
ClawHub は公開された skills と plugin リリースに対して自動チェックを実行します。スキャン保留またはブロックされた
リリースは、所有者には `/dashboard` で引き続き表示される一方で、公開カタログやインストールサーフェスから消える場合があります。
...
インストールイベントを送信する場合があります。これは次で無効にできます。
