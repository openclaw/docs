...
- `.clawhubignore`（公開/同期用の無視パターン、旧 `.clawdhubignore`）
- `.gitignore`（これも尊重されます）
...
- `description` は UI/検索でスキルの要約として使われます。
...
スキルの実行前に存在している必要がある環境変数には `requires.env` を使います。`required: false` の任意変数を含め、変数ごとのメタデータが必要な場合は `envVars` を使います。
...
任意の環境変数は `metadata.openclaw.envVars` の下で宣言し、`required: false` を設定します。任意の項目を `requires.env` に追加しないでください。`requires.env` は、その変数がないとスキルを実行できないことを意味します。
...
ClawHub のセキュリティ分析は、スキルが宣言している内容と、実際に行うことが一致しているかを確認します。コードが `TODOIST_API_KEY` を参照しているのに、フロントマターで `requires.env`、`primaryEnv`、または `envVars` の下に宣言していない場合、分析はメタデータの不一致としてフラグを立てます。宣言を正確に保つことで、スキルがレビューを通過しやすくなり、ユーザーもインストールする内容を理解しやすくなります。
...
- `SKILL.md` に価格メタデータを追加しないでください。これはスキル形式の一部ではなく、追加しても公開済みスキルが有料になることはありません。
- スキルが有料のサードパーティサービスと連携する場合は、外部コストと必要なアカウントを、スキル手順と環境変数宣言（必須変数には `requires.env`、任意変数には `required: false` を指定した `envVars`）に明確に記載してください。
