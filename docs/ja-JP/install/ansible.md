---
read_when:
    - セキュリティ強化を伴うサーバーの自動デプロイを行いたい
    - VPN アクセスを備えたファイアウォール隔離セットアップが必要です
    - リモートの Debian/Ubuntu サーバーにデプロイしている
summary: Ansible、Tailscale VPN、ファイアウォール分離による、自動化されハードニングされた OpenClaw インストール
title: Ansible
x-i18n:
...
**[openclaw-ansible](https://github.com/openclaw/openclaw-ansible)** で OpenClaw を本番サーバーにデプロイします -- セキュリティ最優先のアーキテクチャを備えた自動インストーラーです。
...
| **Ansible** | 2.14+（クイックスタートスクリプトで自動インストールされます） |
...
Ansible playbook は以下をインストールして設定します。
...
5. **OpenClaw** -- ホストベース、コンテナ化されません
6. **Systemd サービス** -- セキュリティハードニング付きで自動開始

<Note>
Gateway はホスト上で直接実行されます（Docker 内ではありません）。エージェントのサンドボックス化は任意です。この playbook は Docker をインストールしますが、これはデフォルトのサンドボックスバックエンドであるためです。詳細とその他のバックエンドについては、[サンドボックス化](/ja-JP/gateway/sandboxing)を参照してください。
</Note>
...
ポート 22（SSH）のみが開いているはずです。他のすべてのサービス（Gateway、Docker）はロックダウンされています。
