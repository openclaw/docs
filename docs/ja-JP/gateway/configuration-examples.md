...
read_when:
    ...
    - 設定例を探しています
    - OpenClaw の初回セットアップ
summary: 一般的なOpenClawセットアップ向けのスキーマに正確に準拠した設定例
title: 設定例
x-i18n:
...
ネットワーク経路を制御できる場合を除き、デバイスのペアリングは手動のままにしてください。専用の
...
未設定の場合、これは無効のままです。これは、要求されたスコープがない新規の `role: node` ペアリングにのみ適用されます。
オペレーター/ブラウザークライアント、およびロール、スコープ、メタデータ、公開鍵のアップグレードには、
引き続き手動承認が必要です。
...
そのリスクを明示的に受け入れる場合にのみ、各チャンネルの `dangerouslyAllowNameMatching: true` で、直接変更可能な名前/メール/ニックネーム照合を有効にしてください。
...
- プロバイダー ID は異なります（電話番号、ユーザー ID、チャンネル ID）。形式を確認するには、プロバイダーのドキュメントを参照してください。
