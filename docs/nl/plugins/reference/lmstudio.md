---
read_when:
    - Je installeert, configureert of controleert de lmstudio Plugin
summary: Voegt ondersteuning voor de LM Studio-modelprovider toe aan OpenClaw.
title: LM Studio Plugin
x-i18n:
    generated_at: "2026-05-02T20:52:14Z"
    model: gpt-5.5
    provider: openai
    source_hash: ec6c84ec422c0795d00f744239fcad96a5a01ef4fddd4823ba0b6ed93444a75b
    source_path: plugins/reference/lmstudio.md
    workflow: 16
---

# LM Studio Plugin

Voegt ondersteuning voor LM Studio-modelproviders toe aan OpenClaw.

## Distributie

- Pakket: `@openclaw/lmstudio-provider`
- Installatieroute: opgenomen in OpenClaw

## Oppervlak

providers: lmstudio; contracts: memoryEmbeddingProviders

## Gerelateerde documentatie

- [lmstudio](/nl/providers/lmstudio)
