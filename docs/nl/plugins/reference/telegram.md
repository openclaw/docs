---
read_when:
    - Je installeert, configureert of auditeert de Telegram Plugin
summary: Voegt de Telegram-kanaalinterface toe voor het verzenden en ontvangen van OpenClaw-berichten.
title: Telegram-plugin
x-i18n:
    generated_at: "2026-05-02T20:55:43Z"
    model: gpt-5.5
    provider: openai
    source_hash: cada3e82983630e71fa5cc32aa2ef0801eff1cab620f1e7f5c2dfdd3a565924e
    source_path: plugins/reference/telegram.md
    workflow: 16
    postprocess_version: locale-links-v1
---

# Telegram Plugin

Voegt het Telegram-kanaaloppervlak toe voor het verzenden en ontvangen van OpenClaw-berichten.

## Distributie

- Pakket: `@openclaw/telegram`
- Installatieroute: inbegrepen in OpenClaw

## Oppervlak

channels: telegram

## Gerelateerde documentatie

- [telegram](/nl/channels/telegram)
