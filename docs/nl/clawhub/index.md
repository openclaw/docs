...
- Gebruik de afzonderlijke `clawhub` CLI voor registry-auth, publiceren en delete/undelete-workflows.
...
| Bundel-Plugins | Verpakte Plugin-bundels voor OpenClaw-distributie            | `clawhub package publish <source>`           |
...
Kale npm-veilige Plugin-specificaties kunnen tijdens launch-cutovers via npm worden opgelost, en
`npm:<package>` blijft alleen npm wanneer een bron expliciet moet zijn.

Plugin-installaties valideren de geadverteerde compatibiliteit van `pluginApi` en `minGatewayVersion`
...
de ClawHub digest-header en gedownloade bytes, en registreert artefactmetadata voor
latere updates.
...
De CLI heeft ook Skill-installatie- en updatecommando's voor directe registry-workflows:
...
account dat oud genoeg is om de uploadgate te passeren. Openbare detailpagina's vatten de
...
of geblokkeerd zijn, kunnen verdwijnen uit de openbare catalogus en installatieoppervlakken, terwijl
...
Wanneer je `clawhub install` uitvoert terwijl je bent ingelogd, kan de CLI een best-effort
installatie-event verzenden zodat ClawHub geaggregeerde installatietellingen kan berekenen. Schakel dit uit met:
...
| `CLAWHUB_REGISTRY`            | Overschrijf de URL van de registry-API.           |
