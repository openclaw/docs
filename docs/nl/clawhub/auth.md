...
read_when:
    ...
    - 401's debuggen
summary: Aanmelden bij ClawHub, API-tokens, aanmelden via de CLI, tokenopslag en intrekking.
x-i18n:
...
ClawHub gebruikt GitHub voor aanmelding op het web. De CLI gebruikt ClawHub-API-tokens die via dat aangemelde account zijn aangemaakt.

## Aanmelding op het web

Gebruik GitHub om je aan te melden op [clawhub.ai](https://clawhub.ai).

Verwijderde, geblokkeerde of uitgeschakelde accounts kunnen de normale ClawHub-aanmelding niet voltooien. Als de aanmelding je terugbrengt naar een afgemelde status, heeft je account mogelijk geen goede status.

## CLI-aanmelding

De standaardaanmeldingsstroom van de CLI opent je browser:
...
Als je browser de lokale callback niet kan bereiken vanwege firewall-, VPN- of proxyregels, gebruik dan de tokenstroom zonder browser.
