...
read_when:
    ...
    - 401-fouten debuggen
summary: Aanmelding bij ClawHub, API-tokens, CLI-aanmelding, tokenopslag en intrekking.
x-i18n:
...
ClawHub gebruikt GitHub voor aanmelden via het web. De CLI gebruikt ClawHub API-tokens die worden aangemaakt
...
De standaard aanmeldingsflow van de CLI opent je browser:
...
met `clawhub login` of geef een vers token op met `clawhub login --token`.
