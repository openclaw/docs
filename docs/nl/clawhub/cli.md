...
- Voegt een vaardigheid toe aan of verwijdert deze uit je highlights.
...
- Zoeken geeft voorrang aan exacte slug-/naamtokenmatches boven downloadpopulariteit. Een op zichzelf staand slugtoken zoals `map` matcht sterker met `personal-map` dan met de substring in `amap`.
- Downloads zijn een kleine populariteitsvoorafweging, geen garantie op toppositie.
- Als een vaardigheid zou moeten verschijnen maar dat niet doet, voer dan `clawhub inspect <slug>` uit terwijl je bent ingelogd om eigenaar-zichtbare moderatiediagnostiek te controleren voordat je metadata hernoemt.

### `explore`

- Toont nieuwste vaardigheden via `/api/v1/skills?limit=...&sort=createdAt` (gesorteerd op `createdAt` aflopend).
- Vlaggen:
...
- `--versions`: lijst met versiegeschiedenis (eerste pagina).
- `--limit <n>`: maximaal aantal versies om te tonen (1-200).
- `--files`: bestanden voor de geselecteerde versie tonen.
- `--file <path>`: ruwe bestandsinhoud ophalen (alleen tekstbestanden; limiet 200 KB).
- `--json`: machineleesbare uitvoer.
...
- Weigert vastgepinde vaardigheden te overschrijven; voer eerst `clawhub unpin <slug>` uit.
- Schrijft:
...
- Verwijdert de lockfile-pin van een geïnstalleerde skill, zodat toekomstige updates deze kunnen aanpassen.
...
- `--owner <handle>` publiceert onder een uitgevershandle van een organisatie/gebruiker wanneer de
...
- `--migrate-owner` verplaatst een bestaande skill naar `--owner` tijdens het publiceren van een nieuwe
  versie. Vereist beheerder-/eigenaarstoegang voor beide uitgevers.
...
- Gepubliceerde skills mogen vrij worden gebruikt, aangepast en herverdeeld zonder naamsvermelding.
...
- `--clawscan-note <text>` voegt een ClawScan-notitie toe. Deze notitie geeft ClawScan
  ...
  native host-toegang of provider-specifieke inloggegevens. De notitie wordt opgeslagen bij
...
- Verberg een skill voorlopig (eigenaar, moderator of beheerder).
- Roept `DELETE /api/v1/skills/{slug}` aan.
- Door de eigenaar geïnitieerde voorlopige verwijderingen reserveren de slug 30 dagen; de opdracht drukt de vervaltijd af.
- `--reason <text>` legt een moderatienotitie vast bij de skill en in het auditlog.
...
- Hernoem een skill waarvan je eigenaar bent en behoud de vorige slug als redirect-alias.
...
- De bronslug wordt niet langer publiek vermeld en wordt een redirect-alias naar het doel.
...
- Met directe digest-flags wordt geverifieerd zonder netwerkopzoeking.
- Flags:
...
- Zet een pakket over naar een andere publisher.
...
- Namen van scoped pakketten moeten worden overgezet naar de overeenkomende scope-eigenaar.
...
- Geverifieerde opdracht om een pakket aan moderators te melden.
...
- Toont de huidige pakketscanstatus, het aantal open meldingen, de handmatige
...
- Rapporteert blokkades voor officiële status, beschikbaarheid van ClawPack, artifact-digest,
...
- Externe code-plugins moeten `openclaw.compat.pluginApi` en
  ...
  Top-level `package.json.version` wordt niet gebruikt als fallback voor publicatievalidatie.
...
- `--owner <handle>` publiceert onder een gebruikers- of org-publisherhandle wanneer de actor publishertoegang heeft.
- `--clawscan-note <text>` voegt een ClawScan-notitie toe. Deze notitie geeft ClawScan
...
- Bestaande flags (`--family`, `--name`, `--version`, `--source-repo`, `--source-commit`, `--source-ref`, `--source-path`) werken nog steeds als overschrijvingen.
...
Gebruik eerst `--dry-run` zodat u de opgeloste pakketmetadata en
bronvermelding kunt bevestigen voordat u een live release maakt:
...
Voor code-plugins bouwt en uploadt mappublicatie een ClawPack-artifact uit
de pakketmap:
...
- `package.json.version` is uw pakketreleaseversie, maar wordt niet gebruikt als
...
- `openclaw.hostTargets` en `openclaw.environment` zijn optionele metadata.
  ClawHub kan ze weergeven wanneer ze aanwezig zijn, maar ze zijn niet vereist voor publicatie.
- `openclaw.compat.minGatewayVersion` en
  `openclaw.build.pluginSdkVersion` zijn optionele extra's als u
  gedetailleerdere compatibiliteitsmetadata wilt publiceren.
- Als u een oudere `clawhub` CLI-release gebruikt, upgrade dan vóór het publiceren zodat
...
- Scant op lokale Skills-mappen en publiceert nieuwe/gewijzigde mappen.
- Roots kunnen elke map zijn: een Skills-map of een enkele Skills-map met `SKILL.md`.
- Voegt automatisch Clawdbot Skills-roots toe wanneer `~/.clawdbot/clawdbot.json` aanwezig is:
