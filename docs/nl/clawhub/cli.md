...
x-i18n:
    generated_at: "2026-05-11T20:23:33Z"
    model: gpt-5.5
    provider: openai
...
(bijv. Docker-containers, Hetzner VPS met internet alleen via proxy, bedrijfs-
firewalls).
...
- Zoeken geeft voorrang aan exacte slug-/naamtokenovereenkomsten vóór downloadpopulariteit. Een zelfstandig slugtoken zoals `map` komt sterker overeen met `personal-map` dan met de substring binnen `amap`.
...
- `--limit <n>`: maximaal aantal versies om weer te geven (1-200).
- `--files`: bestanden voor de geselecteerde versie weergeven.
- `--file <path>`: ruwe bestandsinhoud ophalen (alleen tekstbestanden; limiet van 200 KB).
- `--json`: machineleesbare uitvoer.
...
- Vastgezette skills worden overgeslagen door `update --all` en geweigerd door een directe `update <slug>`.
...
- Berekent een vingerafdruk uit lokale bestanden.
...
- `--owner <handle>` publiceert onder een uitgevers-handle van een org/gebruiker wanneer de
  actor uitgeverstoegang heeft.
- `--migrate-owner` verplaatst een bestaande skill naar `--owner` terwijl een nieuwe
  versie wordt gepubliceerd. Vereist admin-/eigenaarstoegang bij beide uitgevers.
- Eigenaars- en beoordelingsgedrag wordt uitgelegd in `docs/publishing.md`.
...
- Voeg één skill waarvan je eigenaar bent samen met een andere skill waarvan je eigenaar bent.
...
- Vraag een beveiligingsscan opnieuw aan voor de laatst gepubliceerde skillversie.
- Eigenaren en uitgeversadmins kunnen hun eigen skills opnieuw scannen tot aan de herstellimiet
  per versie.
- Platformmoderators en admins kunnen elke skill opnieuw scannen en worden niet geblokkeerd door de
  herstellimiet van de eigenaar, hoewel er per versie slechts één herscan tegelijk kan worden uitgevoerd.
...
- Overdrachten naar gebruikers-handles maken een wachtend verzoek aan dat de ontvanger accepteert.
- Overdrachten naar org-/uitgevers-handles worden alleen onmiddellijk toegepast wanneer de actor
  admin-toegang heeft tot zowel de huidige eigenaar als de doeluitgever.
- Subopdrachten:
...
- Gebruik dit voor pluginmetadata, compatibiliteit, verificatie, bron en inspectie van versies/bestanden.
- `--version <version>`: inspecteer een specifieke versie (standaard: nieuwste).
- `--tag <tag>`: inspecteer een getagde versie (bijv. `latest`).
- `--versions`: lijst versiegeschiedenis op (eerste pagina).
- `--limit <n>`: maximaal aantal versies om te vermelden (1-100).
...
- Voor ClawPack npm-pack-artefacten verifieert dit ook npm `sha512`-integriteit,
  npm shasum en de naam/versie van `package.json` van de tarball.
...
- Verwijdert een package en alle releases voorlopig.
- Vereist de package-eigenaar, een eigenaar/beheerder van een organisatiepublisher, platformmoderator,
...
- Herstelt een voorlopig verwijderde package en releases.
- Vereist de package-eigenaar, een eigenaar/beheerder van een organisatiepublisher, platformmoderator,
  of platformbeheerder.
...
- Draagt een package over aan een andere publisher.
- Vereist beheerderstoegang tot zowel de huidige package-eigenaar als de doelpublisher,
  tenzij dit wordt uitgevoerd door een platformbeheerder.
- Scoped package-namen moeten worden overgedragen aan de overeenkomende scope-eigenaar.
...
- Vraag een nieuwe beveiligingsscan aan voor de nieuwste gepubliceerde package-release.
- Eigenaren en publisherbeheerders kunnen hun eigen packages opnieuw scannen tot aan de herstel-limiet
  per release.
- Platformmoderators en beheerders kunnen elke package opnieuw scannen en worden niet geblokkeerd door
  de herstel-limiet voor eigenaren, hoewel er per release maar één nieuwe scan tegelijk kan lopen.
...
- Geauthenticeerde opdracht om een package aan moderators te rapporteren.
- Roept `POST /api/v1/packages/{name}/report` aan.
- Rapporten gelden op package-niveau, kunnen optioneel aan een versie zijn gekoppeld, en worden zichtbaar
  voor moderators ter beoordeling.
- Rapporten verbergen packages niet automatisch en blokkeren op zichzelf geen downloads.
- Vlaggen:
  - `--version <version>`: optionele package-versie om aan het rapport te koppelen.
...
- Opdracht voor eigenaren/publishers om bezwaar te maken tegen release-moderatie.
- Roept `POST /api/v1/packages/{name}/appeal` aan.
- Bezwaar wordt geaccepteerd voor in quarantaine geplaatste, ingetrokken, verdachte of schadelijke
  releases.
- Vlaggen:
  - `--version <version>`: vereiste package-versie.
...
- Eigenaarsopdracht om de moderatiezichtbaarheid van een package te controleren.
- Roept `GET /api/v1/packages/{name}/moderation` aan.
- Toont de huidige scanstatus van de package, het aantal open rapporten, de handmatige
  moderatiestatus van de nieuwste release, de status van downloadblokkering en moderatieredenen.
- Vlaggen:
  - `--json`: machineleesbare uitvoer.
...
- Controleert of een package klaar is voor toekomstig gebruik door OpenClaw.
...
- Toont operatorgerichte migratiestatus voor een package die een gebundelde
  OpenClaw-Plugin kan vervangen.
- Roept hetzelfde berekende readiness-endpoint aan als `package readiness`, maar drukt
  migratiegerichte status, nieuwste versie, officiële-packagestatus, controles en
...
- `.tgz`-bronnen worden behandeld als ClawPack. De CLI uploadt de exacte npm-pack
  ...
  vooraf ingevulde metadata.
- Code-Plugin-mappen worden vóór upload verpakt in een ClawPack npm-tarball zodat
  OpenClaw-installaties het exacte artifact kunnen verifiëren. Bundel-Plugin-mappen blijven
...
- `--owner <handle>` publiceert onder een gebruikers- of organisatiepublisherhandle wanneer de actor publishertoegang heeft.
...
- Private GitHub-repo's vereisen `GITHUB_TOKEN`.

#### Aanbevolen lokale workflow

Gebruik eerst `--dry-run` zodat je de opgeloste package-metadata en
...
`package.json`. Dit minimale manifest is genoeg voor succesvolle publicatie:
...
- `package.json.version` is je package-releaseversie, maar wordt niet gebruikt als
...
- Geef voor monorepo's `source_path` door zodat de workflow de Plugin-
  packagemap publiceert, bijvoorbeeld `source_path: extensions/codex`.
...
- Houd `clawhub_token` beschikbaar voor de eerste publicatie, niet-vertrouwde packages of noodpublicaties.
- De workflow uploadt het JSON-resultaat als artifact en stelt het beschikbaar als workflow-uitvoer.

### `sync`

- Scant naar lokale skill-mappen en publiceert nieuwe/gewijzigde mappen.
...
- Houdt rekening met `CLAWDBOT_CONFIG_PATH` / `CLAWDBOT_STATE_DIR` en `OPENCLAW_CONFIG_PATH` / `OPENCLAW_STATE_DIR`.
- Vlaggen:
...
- Verzonden tijdens `sync` wanneer je bent ingelogd, tenzij `CLAWHUB_DISABLE_TELEMETRY=1` (legacy `CLAWDHUB_DISABLE_TELEMETRY=1`).
