...
- Zoeken geeft voorrang aan exacte tokenmatches voor slug/naam boven downloadpopulariteit. Een zelfstandig slug-token zoals `map` matcht sterker met `personal-map` dan met de substring binnen `amap`.
- Downloads zijn een kleine populariteitsprior, geen garantie voor toppositie.
- Als een skill zou moeten verschijnen maar dat niet doet, voer dan `clawhub inspect <slug>` uit terwijl je bent ingelogd om voor eigenaren zichtbare moderatiediagnoses te controleren voordat je metadata hernoemt.
...
- `update --all` slaat vastgezette slugs over en drukt een samenvatting af van wat bevroren is gebleven.
...
- Gepubliceerde skills mogen gratis worden gebruikt, gewijzigd en opnieuw verspreid zonder naamsvermelding.
...
- `--clawscan-note <text>` voegt een ClawScan-notitie toe. Deze notitie geeft ClawScan
  ...
  toegang tot native hosts of providerspecifieke credentials. De notitie wordt opgeslagen bij
...
- Verwijder een skill voorlopig (owner, moderator of admin).
- Roept `DELETE /api/v1/skills/{slug}` aan.
- Door de owner geinitieerde voorlopige verwijderingen reserveren de slug 30 dagen; de opdracht drukt de vervaltijd af.
...
- Herstel een verborgen skill (owner, moderator of admin).
...
- Verberg een skill (owner, moderator of admin).
- Alias voor `delete`.

### `unhide <slug>`

- Maak een skill weer zichtbaar (owner, moderator of admin).
- Alias voor `undelete`.

### `skill rename <slug> <new-slug>`

- Hernoem een skill waarvan je owner bent en behoud de vorige slug als redirect-alias.
- Roept `POST /api/v1/skills/{slug}/rename` aan.
- `--yes` slaat bevestiging over.

### `skill merge <source-slug> <target-slug>`

- Voeg een skill waarvan je owner bent samen met een andere skill waarvan je owner bent.
- De bron-slug wordt niet meer publiek vermeld en wordt een redirect-alias naar het doel.
...
- Overdrachten naar org-/publisher-handles worden alleen direct toegepast wanneer de actor
  admintoegang heeft tot zowel de huidige owner als de doelpublisher.
- Subopdrachten:
...
- Voor ClawPack npm-pack-artifacts verifieert dit ook npm `sha512`-integriteit,
  npm shasum en de naam/versie in de tarball-`package.json`.
...
- Rapporten gelden op pakketniveau, kunnen optioneel aan een versie worden gekoppeld,
  en worden zichtbaar voor moderators ter beoordeling.
- Rapporten verbergen pakketten niet automatisch en blokkeren downloads niet op zichzelf.
- Vlaggen:
...
- Rapporteert blokkades voor officiële status, ClawPack-beschikbaarheid, artefactdigest,
  herkomst van broncode, OpenClaw-compatibiliteit, hostdoelen, omgevingsmetadata
  en scanstatus.
- Vlaggen:
  - `--json`: machineleesbare uitvoer.
...
- Code-pluginmappen worden vóór upload verpakt in een ClawPack npm-tarball zodat
  OpenClaw-installaties het exacte artefact kunnen verifiëren. Bundel-pluginmappen gebruiken nog steeds
...
- De herbruikbare workflow gebruikt standaard de aanroeperrepo als `source`.
