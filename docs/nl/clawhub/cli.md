...
summary: 'CLI-referentie: opdrachten, vlaggen, configuratie, lockfile, synchronisatiegedrag.'
x-i18n:
...
- Zoeken geeft voorkeur aan exacte tokenovereenkomsten voor slug/naam boven downloadpopulariteit. Een zelfstandig slug-token zoals `map` komt sterker overeen met `personal-map` dan met de substring binnen `amap`.
- Downloads zijn een kleine populariteitsprior, geen garantie op hoogste plaatsing.
- Als een skill zou moeten verschijnen maar niet verschijnt, voer dan `clawhub inspect <slug>` uit terwijl je bent ingelogd om voor de eigenaar zichtbare moderatiediagnostiek te controleren voordat je metadata hernoemt.
...
- `--migrate-owner` verplaatst een bestaande Skill naar `--owner` terwijl een nieuwe
  versie wordt gepubliceerd. Vereist beheerder-/eigenaarstoegang voor beide publishers.
- Eigenaar- en beoordelingsgedrag wordt uitgelegd in `docs/publishing.md`.
- Een Skill publiceren betekent dat deze onder `MIT-0` wordt uitgebracht op ClawHub.
- Gepubliceerde Skills zijn vrij te gebruiken, te wijzigen en opnieuw te verspreiden zonder naamsvermelding.
...
- `--clawscan-note <text>` voegt een ClawScan-notitie toe. Deze notitie geeft ClawScan
  ...
  native host-toegang of providerspecifieke credentials. De notitie wordt opgeslagen op
...
- Voeg één eigen Skill samen in een andere eigen Skill.
- De bron-slug wordt niet langer openbaar weergegeven en wordt een redirect-alias naar het doel.
...
- Overdrachten naar gebruikershandles maken een wachtend verzoek dat de ontvanger accepteert.
- Overdrachten naar org-/publisher-handles worden alleen direct toegepast wanneer de actor
  beheerderstoegang heeft tot zowel de huidige eigenaar als de bestemmingspublisher.
- Subopdrachten:
...
- Bladert door of doorzoekt de uniforme packagecatalogus via `GET /api/v1/packages` en `GET /api/v1/packages/search`.
- Gebruik dit voor Plugins en andere items uit packagefamilies; `search` op topniveau blijft het zoekoppervlak voor Skills.
- Flags:
...
- Verouderde ZIP-versies worden gedownload via de verouderde ZIP-route.
- Flags:
...
- Berekent ClawHub SHA-256, npm `sha512`-integriteit en npm shasum voor een lokaal
  artifact.
- Met `--package` lost het verwachte metadata op uit ClawHub en vergelijkt het
  lokale bestand met de gepubliceerde artifactmetadata.
- Met directe digest-flags verifieert het zonder netwerklookup.
- Flags:
...
- Vereist de pakketeigenaar, een eigenaar/beheerder van een org-uitgever, platformmoderator
...
- Herstelt een via soft-delete verwijderd pakket en releases.
- Vereist de pakketeigenaar, een eigenaar/beheerder van een org-uitgever, platformmoderator
  of platformbeheerder.
...
- Pakketnamen met scope moeten worden overgedragen aan de bijpassende scope-eigenaar.
...
- Rapporten gelden op pakketniveau, kunnen optioneel aan een versie zijn gekoppeld,
  en worden zichtbaar voor moderators ter beoordeling.
- Rapporten verbergen pakketten niet automatisch en blokkeren zelf geen downloads.
- Vlaggen:
...
- Publiceert een code-plugin of bundelplugin via `POST /api/v1/packages`.
- `<source>` accepteert:
...
- Code-pluginmappen worden vóór upload verpakt in een ClawPack npm-tarball zodat
...
- Voor GitHub-bronnen wordt bronattributie automatisch ingevuld op basis van de repo, opgeloste commit, ref en subpad.
...
- Als je een oudere `clawhub` CLI-release gebruikt, upgrade dan vóór het publiceren zodat
...
- Geef voor monorepo's `source_path` mee zodat de workflow de pluginpakketmap publiceert,
  bijvoorbeeld `source_path: extensions/codex`.
...
- Houd `clawhub_token` beschikbaar voor eerste publicatie, onvertrouwde pakketten of break-glass-publicaties.
