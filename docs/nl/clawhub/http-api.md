...
x-i18n:
    generated_at: "2026-07-03T23:35:56Z"
    model: gpt-5.5
...
Mappen van derden mogen de openbare leeseindpunten gebruiken om ClawHub-Skills te tonen of te doorzoeken. Cache resultaten, respecteer `429`/`Retry-After`, link gebruikers terug naar de canonieke ClawHub-vermelding (`https://clawhub.ai/<owner>/skills/<slug>`) en vermijd de indruk dat ClawHub de site van derden onderschrijft. Probeer verborgen, privé- of door moderatie geblokkeerde inhoud niet buiten het openbare API-oppervlak te spiegelen.

Web-slug-snelkoppelingen worden over registry-families heen opgelost, maar API-clients moeten de canonieke URL's gebruiken die door leeseindpunten worden teruggegeven in plaats van routeprioriteit opnieuw te reconstrueren.
...
- Geauthenticeerde schrijfeindpunten moeten geen kale `Unauthorized` teruggeven wanneer
...
- Downloaden: 1200/min per IP, 6000/min per sleutel (downloadeindpunten)
...
- `X-RateLimit-Remaining` / `RateLimit-Remaining`: exact resterend budget wanneer aanwezig.
  Geslaagde sharded verzoeken laten deze header weg in plaats van een benaderde globale waarde terug te geven.
...
permissiefouten (`401`/`403`), snelheidslimieten (`429`) en geblokkeerde downloads. Clients
...
- Resultaten worden teruggegeven in relevantievolgorde (embeddinggelijkenis + exacte slug-/naamtoken-boosts + een kleine populariteitsprior).
...
- Hernoemaliassen behouden resolutie voor oude URL's en installaties die via de registry worden opgelost, maar zoekranking is gebaseerd op de canonieke Skill-metadata nadat de hernoeming is geïndexeerd. Bestaande statistieken blijven bij de Skill.
...
- `trending` rankt op basis van installaties in de afgelopen 7 dagen (op basis van telemetrie).
...
- Oude slugs die zijn aangemaakt door hernoem-/mergeflows van owners worden opgelost naar de canonieke Skill.
...
Meld een Skill voor moderatorbeoordeling. Meldingen zijn op Skill-niveau, optioneel gekoppeld
...
Moderator-/admin-eindpunt voor inname van Skill-meldingen.
...
Moderator-/admin-eindpunt voor het oplossen of heropenen van Skill-meldingen.
...
`status` terug naar `open` wordt gezet. Geef `finalAction: "hide"` mee met een getriagede
...
Canonieke batch-route voor opnieuw scannen, alleen voor admins. Deze accepteert dezelfde payloadvorm als de legacy `POST /api/v1/skills/-/rescan-batch`.
...
- Skill-identiteit, uitgeveridentiteit en metadata van de geselecteerde versie zijn envelope-velden op topniveau (`slug`, `displayName`, `publisherHandle`, `version`, `resolvedFrom`, `tag`, `createdAt`), zodat shell-automatisering ze kan lezen zonder geneste wrappers uit te pakken.
- `security` is het ClawScan-/security-oordeel op topniveau. Automatisering moet sturen op `ok`, `decision`, `reasons` en `security.status`.
- `security.signals` bevat ondersteunend scannerbewijs zoals `staticScan`, `virusTotal` en `skillSpector`.
- `security.signals.dependencyRegistry` blijft behouden voor compatibiliteit met v1-antwoorden, maar de scanner voor het bestaan van dependencyregistries is buiten gebruik gesteld en deze sleutel is altijd `null`.
...
Retourneert actuele compacte security-oordelen voor exacte Skill-versies. Dit
...
- Het antwoord is alleen voor security. Het bevat geen Skill Card-gegevens, gegenereerde kaartstatus, bestandslijsten van artifacts of gedetailleerde scannerpayloads.
...
- Geverifieerde callers kunnen in lijst-/zoekresultaten privépakketten zien voor uitgevers waartoe ze behoren.
...
- `isOfficial` (optioneel): `true` of `false`
- `category` (optioneel): plugin-categoriefilter. Alleen ondersteund wanneer het
  verzoek is beperkt tot pluginpakketten. Beheerde categorieën en legacy v1-
...
- Anonieme callers zien alleen publieke pakketkanalen.
- Geverifieerde callers kunnen privépakketten zoeken voor uitgevers waartoe ze behoren.
- `channel=private` retourneert alleen pakketten die de geverifieerde caller kan lezen.

### `GET /api/v1/plugins`

Alleen-Plugin-catalogus om te bladeren door codeplugin- en bundelpluginpakketten.
...
`trending` is een ranglijst van installaties/downloads over zeven dagen en gebruikt geen totalen over de hele looptijd.
...
Bulkexport van nieuwste publieke skills voor offline analyse.
...
- Body: ZIP-archief.
- Elke geëxporteerde skill staat onder `{publisher}/{slug}/`.
...
- Body: ZIP-archief.
- Elke geexporteerde Plugin begint bij `{family}/{packageName}/`.
...
Zoeken alleen voor Plugins in pakketten van code-plugin en bundle-plugin.
...
- De verouderde v1-filteraliassen die zijn gedocumenteerd onder `GET /api/v1/plugins` worden ook
  geaccepteerd.
- Categoriefiltering is een echt API-filter ondersteund door digest-rijen voor Plugin-categorieen,
...
Retourneert de exacte beveiligings- en vertrouwenssamenvatting van de pakketrelease voor installatieclients. Dit is het openbare OpenClaw-verbruiksvlak om te beslissen of een
opgeloste release kan worden geinstalleerd.

Authenticatie:

- Openbaar leeseindpunt. Geen token van eigenaar, publisher, moderator of beheerder is
  vereist.
...
- `trust.stale` betekent dat de vertrouwenssamenvatting is berekend op basis van verouderde invoer en
  moet worden behandeld als vereistend om te vernieuwen voordat een toestemmingsbeslissing met hoge zekerheid wordt genomen.
...
- Dit eindpunt is opzettelijk smaller dan moderatie-eindpunten voor eigenaren/moderators.
  ...
  identiteiten van melders, rapportteksten, privebewijs of interne beoordelingstijdlijnen.
...
- Dit is het OpenClaw-resolvervlak; het voorkomt het raden van archiefformaat op basis van
  een gedeelde URL.
...
Moderator-eindpunt voor het tonen van migratierijen van officiele OpenClaw-Plugins.
...
Beheerders-eindpunt voor het maken of bijwerken van een migratierij voor een officiele Plugin.
...
Meld een pakket voor beoordeling door moderators. Rapporten zijn pakketniveau, optioneel
...
`note` is vereist voor `confirmed` en `dismissed`; deze mag worden weggelaten bij het
...
- Deze route blijft alleen ZIP. De route streamt geen ClawPack `.tgz`-bestanden.
...
- Overdrachten voor door GitHub ondersteunde skills proxyen of mirroren geen bytes. De JSON-respons
  ...
  en `archiveUrl`; scan-/huidige status is een gate en wordt niet opgenomen als metadata van de succespayload.
...
- Toegestane formuliervelden zijn `payload`, herhaalde `files`-blobs, of één `clawpack`
  tarballverwijzing. `clawpack` mag een `.tgz`-blob zijn of een opslag-id dat door
  de upload-url-flow wordt geretourneerd. Gepubliceerde staged storage-id's moeten ook het
  `clawpackUploadTicket` bevatten dat met die upload-URL is geretourneerd.
- Gebruik ofwel `files` of `clawpack`, nooit beide in dezelfde request.
- JSON-body's en door de aanroeper aangeleverde `payload.files`- / `payload.artifact`-
  metadata worden geweigerd.
- Directe multipart-publicatierequests zijn beperkt tot 18 MB. ClawPack-tarballs mogen
...
- Publicaties namens iemand anders valideren de geschiktheid voor het officiële kanaal nog steeds tegen het doeleigenaaraccount.
...
legacy gedeelde gebruiker/persoonlijke uitgever wijst, migreert het eindpunt deze eerst naar een org publisher.
...
Geauthenticeerde selfservice-aanmaak van org publisher. Maakt een nieuwe org publisher aan en voegt de
aanroeper toe als eigenaar. Dit eindpunt migreert geen bestaande gebruikers-/persoonlijke handles en markeert
de uitgever niet als trusted/official.
...
skill-slugaliassen, pakketten, package-inspectorwaarschuwingen en afgeleide search-digestrijen van de herstelde publisher, zodat
paden met directe eigenaar overeenkomen met de nieuwe publisher-autoriteit. Een actieve protected-handle
...
De CLI kan registry-/auth-instellingen ontdekken vanaf de site:
