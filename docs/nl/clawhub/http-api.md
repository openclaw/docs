...
x-i18n:
    generated_at: "2026-07-02T08:32:22Z"
    model: gpt-5.5
...
- Geauthenticeerde aanvragen (geldige Bearer-token): gehandhaafd per gebruikersbucket.
...
- `X-RateLimit-Remaining` / `RateLimit-Remaining`: exact resterend budget wanneer aanwezig.
  Gesplitste succesvolle aanvragen laten deze header weg in plaats van een geschatte globale waarde te retourneren.
...
- Als er geen vertrouwd client-IP beschikbaar is, gebruiken anonieme aanvragen fallback-buckets
  die alleen zijn gescoped op type ratelimiet. Deze fallback-buckets bevatten geen
...
machtigingsfouten (`401`/`403`), ratelimieten (`429`) en geblokkeerde downloads. Clients
...
- Populariteit is log-geschaald en begrensd. Skills met hoge betrokkenheid kunnen lager scoren wanneer de querytekst een zwakkere match is.
...
- Zet de termen waar gebruikers letterlijk naar zullen zoeken in de weergavenaam, samenvatting en tags. Gebruik alleen een zelfstandige slugtoken wanneer die ook een stabiele identiteit is die je wilt behouden.
...
- Hernoemingsaliassen behouden resolutie voor oude URL's en installaties die via het register worden opgelost, maar zoekrangschikking is gebaseerd op de canonieke skillmetadata nadat de hernoeming is geïndexeerd. Bestaande statistieken blijven bij de skill.
...
- `createdAt` is stabiel voor crawls van nieuwe skills; `updated` wijzigt wanneer bestaande skills opnieuw worden gepubliceerd.
...
- Oude slugs die zijn gemaakt door hernoemings-/samenvoegingsflows van eigenaren lossen op naar de canonieke skill.
...
Meld een skill voor beoordeling door moderators. Meldingen zijn op skillniveau, optioneel gekoppeld
aan een versie, en voeden de wachtrij voor skillmeldingen.
...
Moderator-/adminendpoint voor intake van skillmeldingen.
...
Moderator-/adminendpoint voor het oplossen of heropenen van skillmeldingen.
...
`status` terug naar `open` wordt gezet. Geef `finalAction: "hide"` mee met een getriageerde
...
Retourneert versiemetadata + bestandslijst.
...
Retourneert verificatiedetails van beveiligingsscans voor een skillversie.
...
- Vereist beheerstoegang tot de skill of Plugin als eigenaar/uitgever, of platformmoderator-/adminbevoegdheid.
...
Canonieke batchroute voor herscans, alleen voor admins. Deze accepteert dezelfde payloadvorm als de verouderde `POST /api/v1/skills/-/rescan-batch`.
...
- `security.signals.dependencyRegistry` wordt behouden voor compatibiliteit met v1-antwoorden, maar de scanner voor het bestaan van dependencyregistries is beëindigd en deze sleutel is altijd `null`.
...
- Het antwoord is alleen voor security. Het bevat geen Skill Card-gegevens, gegenereerde kaartstatus, lijsten met artefactbestanden of gedetailleerde scannerpayloads.
- `security.signals` bevat alleen ondersteunend bewijs op statusniveau; gebruik `/scan` of de ClawHub-pagina voor security-audits voor volledige scannerdetails.
- `security.signals.dependencyRegistry` wordt behouden voor compatibiliteit met v1-antwoorden, maar de scanner voor het bestaan van dependencyregistries is beëindigd en deze sleutel is altijd `null`.
- De afwezigheid van een Skill Card heeft geen invloed op `ok`, `decision` of `reasons` van dit endpoint; clients moeten geïnstalleerde `skill-card.md` lokaal lezen wanneer ze kaartinhoud nodig hebben.
...
- Skills-items blijven ondersteund door het skillsregister en kunnen nog steeds alleen via `POST /api/v1/skills` worden gepubliceerd.
...
Uniforme cataloguszoekactie voor Skills + pluginpakketten.
...
Catalogusbrowse alleen voor Plugins over code-plugin- en bundle-plugin-pakketten.
...
`trending` is een zeven-daagse ranglijst voor installaties/downloads en gebruikt geen totalen over de hele periode.
...
- `startDate` (vereist): ondergrens in Unix-milliseconden voor `updatedAt` van de skill.
- `endDate` (vereist): bovengrens in Unix-milliseconden voor `updatedAt` van de skill.
...
- Huidige door GitHub ondersteunde Skills met een `clean`- of `suspicious`-scan bevatten
  ...
  content-hash en archief-URL. Ze bevatten geen door ClawHub gehoste bronbestanden.
...
- Body: ZIP-archief.
- Elke geexporteerde Plugin heeft `{family}/{packageName}/` als hoofdmap.
...
- De verouderde v1-filteraliassen die zijn gedocumenteerd onder `GET /api/v1/plugins` worden ook
  geaccepteerd.
- Categoriefiltering is een echte API-filter die wordt ondersteund door digest-
  rijen voor Plugin-categorieen, geen herschrijving van de zoekquery.
- Resultaten worden geretourneerd op relevantievolgorde en hebben momenteel geen paginering.
- Sorteeropties in de browser-UI voor Plugin-zoeken herschikken de geladen relevantieresultaten,
...
Verwijdert een pakket en alle releases met een zachte verwijdering.
...
- `version.artifact.kind` is `legacy-zip` voor pakketarchieven uit de oude wereld of
  `npm-pack` voor releases met ClawPack-ondersteuning.
...
- `trust.stale` betekent dat de vertrouwenssamenvatting is berekend op basis van verouderde invoer en
  moet worden behandeld alsof vernieuwing vereist is voordat een toestemmingsbeslissing met hoge betrouwbaarheid wordt genomen.
...
- ClawPack-versies retourneren een `npm-pack`-artifact, npm-integriteitsvelden, een
  `tarballUrl` en de compatibiliteits-URL voor de verouderde ZIP.
...
Moderatorendpoint voor het weergeven van officiele OpenClaw Plugin-migratierijen.
...
Beheerdersendpoint voor het maken of bijwerken van een officiele Plugin-migratierij.
...
blokkeren downloads niet automatisch zelf; moderators moeten releasemoderatie gebruiken om
...
- Lopende VirusTotal-scans blokkeren leesacties niet; schadelijke releases kunnen elders nog worden tegengehouden.
...
- `slug` (vereist)
- `hash` (vereist): hex sha256 van 64 tekens van de bundel-fingerprint
...
- Door GitHub ondersteunde skill-overdrachten proxyen of mirroren geen bytes. De JSON-respons
  ...
  en `archiveUrl`; scan-/huidige status is een gate en wordt niet opgenomen als metadata van de success-payload.
...
- Toegestane formuliervelden zijn `payload`, herhaalde `files`-blobs of één `clawpack`-
  tarballreferentie. `clawpack` mag een `.tgz`-blob zijn of een storage-id dat door
  de upload-url-flow is geretourneerd. Publicaties met gestagede storage-id moeten ook het
...
- Publicaties namens iemand anders valideren nog steeds of het doel-eigenaaraccount in aanmerking komt voor het official-kanaal.
...
legacy gedeelde gebruiker/persoonlijke publisher wijst, migreert het eindpunt die eerst naar een org-publisher.
...
Het eindpunt staat standaard op dry-run. Recovery toepassen vereist `dryRun: false` en
`confirmIdentityVerified: true` nadat medewerkers onafhankelijk de continuiteit tussen beide
GitHub-principals hebben geverifieerd. Recovery faalt gesloten wanneer de huidige persoonlijke
publisher van de doelgebruiker skills, packages of GitHub-skillbronnen heeft.
Recovery migreert ook verouderde `ownerUserId`-velden voor de skills van de herstelde publisher,
...
100 rijen per apply-transactie; grotere recoveries moeten eerst een hervatbare ownermigratie gebruiken.
...
### Eindpunten voor beheer van owner-slugs
...
Voeg een star toe of verwijder een star (highlights). Beide eindpunten zijn idempotent.
...
De CLI kan registry-/auth-instellingen ontdekken via de site:
