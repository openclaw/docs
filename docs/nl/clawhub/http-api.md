...
x-i18n:
    generated_at: "2026-07-04T03:54:03Z"
    model: gpt-5.5
...
Externe directories mogen de openbare leesendpoints gebruiken om ClawHub Skills te tonen of te doorzoeken. Cache resultaten, respecteer `429`/`Retry-After`, link gebruikers terug naar de canonieke ClawHub-vermelding (`https://clawhub.ai/<owner>/skills/<slug>`) en wek niet de indruk dat ClawHub de externe site onderschrijft. Probeer verborgen, private of door moderatie geblokkeerde inhoud buiten het openbare API-oppervlak niet te spiegelen.
...
- Geauthenticeerde schrijfendpoints mogen geen kale `Unauthorized` teruggeven wanneer
  ...
  verwijderde/gebannde/uitgeschakelde accounts moeten elk bruikbare tekst krijgen zodat CLI-
...
- `X-RateLimit-Remaining` / `RateLimit-Remaining`: exact resterend budget indien aanwezig.
  Gespreide geslaagde requests laten deze header weg in plaats van een geschatte globale waarde terug te geven.
...
- Populariteit is loggeschaald en afgetopt. Skills met veel betrokkenheid kunnen lager ranken wanneer de querytekst een zwakkere match is.
...
- Hernoemaliassen behouden resolutie voor oude URL's en installaties die via de registry oplossen, maar zoekranking is gebaseerd op de canonieke Skill-metadata nadat de hernoeming is geïndexeerd. Bestaande statistieken blijven bij de Skill.
- Als een Skill onverwacht onzichtbaar is, controleer dan eerst de moderatiestatus met `clawhub inspect @owner/slug` terwijl je bent ingelogd, vóórdat je rankinggerelateerde metadata wijzigt.
...
- Oude slugs die zijn aangemaakt door eigenaar-hernoem-/merge-flows worden opgelost naar de canonieke Skill.
...
Geeft verificatiedetails van de beveiligingsscan terug voor een Skill-versie.
...
- Scanverzoekpayloads en downloadbare rapporten verlopen na de bewaartermijn uit de opslag voor scanverzoeken.
- Gepubliceerde scans vereisen beheerstoegang als eigenaar/uitgever, of moderator-/beheerdersrechten voor het platform.
...
- Vereist beheerstoegang als eigenaar/uitgever tot de skill of plugin, of moderator-/beheerdersrechten voor het platform.
...
Canonieke beheerdersroute voor batchgewijs opnieuw scannen. Deze accepteert dezelfde payloadstructuur als de verouderde `POST /api/v1/skills/-/rescan-batch`.

### `POST /api/v1/skills/-/scan/batch/status`

Canonieke beheerdersroute voor batchstatus. Deze accepteert `{ "jobIds": ["..."] }` en retourneert dezelfde geaggregeerde tellers als de verouderde `POST /api/v1/skills/-/rescan-batch/status`.
...
- Skill-identiteit, uitgeveridentiteit en metadata van de geselecteerde versie zijn velden op het hoogste niveau van de envelop (`slug`, `displayName`, `publisherHandle`, `version`, `resolvedFrom`, `tag`, `createdAt`), zodat shellautomatisering ze kan lezen zonder geneste wrappers uit te pakken.
...
- `provenance` is alleen `server-resolved-github-import` wanneer ClawHub tijdens publicatie of import een GitHub-repository/ref/commit/pad heeft opgelost en opgeslagen; anders is dit `unavailable`.
...
- `security.signals.dependencyRegistry` blijft behouden voor compatibiliteit met v1-antwoorden, maar de scanner voor het bestaan van dependencyregistries is uitgefaseerd en deze sleutel is altijd `null`.
- Het ontbreken van een Skill Card heeft geen invloed op de `ok`, `decision` of `reasons` van dit endpoint; clients moeten geïnstalleerde `skill-card.md` lokaal lezen wanneer ze kaartinhoud nodig hebben.
...
- Geauthenticeerde aanroepers kunnen privépakketten zien van uitgevers waartoe ze behoren in lijst-/zoekresultaten.
...
- Anonieme aanroepers zien alleen openbare pakketkanalen.
- Geauthenticeerde aanroepers kunnen zoeken in privépakketten van uitgevers waartoe ze behoren.
- `channel=private` retourneert alleen pakketten die de geauthenticeerde aanroeper kan lezen.
...
`trending` is een install/download-ranglijst over zeven dagen en gebruikt geen totalen over de hele looptijd.
...
- De verouderde v1-filteraliassen die zijn gedocumenteerd onder `GET /api/v1/plugins` worden ook
  geaccepteerd.
- Categoriefiltering is een echt API-filter dat wordt ondersteund door plugin category digest
  rows, geen herschrijving van zoekquery's.
- Resultaten worden teruggegeven op relevantievolgorde en worden momenteel niet gepagineerd.
- Sorteerknoppen in de browser-UI voor zoeken naar plugins herschikken de geladen relevantieresultaten,
...
Voert een soft delete uit van een pakket en alle releases.
...
Geeft de exacte beveiligings- en vertrouwenssamenvatting van de pakketrelease terug voor installatieclients. Dit is het openbare OpenClaw-verbruiksvlak om te bepalen of een
...
- `trust.stale` betekent dat de vertrouwenssamenvatting is berekend uit verouderde inputs en
  moet worden behandeld alsof verversen vereist is vóór een allow-beslissing met hoog vertrouwen.
...
Geeft de expliciete artefactresolvermetadata voor een pakketversie terug.

Opmerkingen:

- Verouderde pakketversies geven een `legacy-zip`-artefact en een verouderde ZIP-
  `downloadUrl` terug.
- ClawPack-versies geven een `npm-pack`-artefact, npm-integriteitsvelden, een
  `tarballUrl` en de verouderde ZIP-compatibiliteits-URL terug.
- Dit is het OpenClaw-resolvervlak; het voorkomt dat het archiefformaat moet worden geraden uit
  een gedeelde URL.
...
Moderatoreindpunt voor het tonen van officiële OpenClaw-pluginmigratierijen.
...
Moderator-/beheerdereindpunt voor reviewqueues van pakketreleases.
...
- GitHub-gebaseerde skill-overdrachten proxyen of mirroren geen bytes. Het JSON-antwoord
...
- Directe multipart-publicatieverzoeken zijn begrensd op 18 MB. ClawPack-tarballs kunnen
...
Ban een gebruiker en verwijder de eigen Skills definitief (alleen moderator/admin).
...
De CLI kan registry-/auth-instellingen via de site ontdekken:
