...
summary: HTTP-API-referentie (publieke + CLI-eindpunten + authenticatie).
x-i18n:
...
Web-slug-snelkoppelingen worden over registerfamilies heen opgelost, maar API-clients moeten de canonieke URL’s gebruiken die door leesendpoints worden teruggegeven in plaats van routeprioriteit te reconstrueren.
...
- `X-RateLimit-Remaining` / `RateLimit-Remaining`: exact resterend budget indien aanwezig.
  Geshalede geslaagde aanvragen laten deze header weg in plaats van een benaderde globale waarde terug te geven.
...
- Als er geen vertrouwd client-IP beschikbaar is, gebruiken anonieme aanvragen fallback-buckets die alleen per soort snelheidslimiet zijn gescoped. Deze fallback-buckets bevatten geen door de aanroeper aangeleverde paden, slugs, pakketnamen, versies, querystrings of andere artefactparameters.

## Foutresponsen

Openbare v1-foutresponsen zijn platte tekst met `content-type: text/plain; charset=utf-8`.
Dit omvat validatiefouten (`400`), ontbrekende openbare resources (`404`), authenticatie- en toestemmingsfouten (`401`/`403`), snelheidslimieten (`429`) en geblokkeerde downloads. Clients moeten de responsebody lezen als een voor mensen leesbare string. Onbekende queryparameters worden genegeerd voor compatibiliteit, maar herkende queryparameters met ongeldige waarden geven `400` terug.
...
- Relevantie weegt zwaarder dan populariteit. Een precieze slug- of weergavenaam-tokenmatch kan hoger scoren dan een lossere match met veel sterkere betrokkenheid.
- ASCII-tekst wordt getokenized op woord- en leestekengrenzen. Bijvoorbeeld: `personal-map` bevat een losstaand `map`-token, terwijl `amap-jsapi-skill` `amap`, `jsapi` en `skill` bevat; zoeken naar `map` geeft `personal-map` daarom een sterkere lexicale match dan `amap-jsapi-skill`.
...
- Hernoem een slug niet alleen om één query na te jagen, tenzij de nieuwe slug een betere canonieke naam voor de lange termijn is. Oude slugs worden omleidingsaliassen, maar de canonieke URL, getoonde slug en toekomstige zoekdigests gebruiken de nieuwe slug.
- Hernoemaliassen behouden oplossing voor oude URL’s en installs die via het register oplossen, maar zoekranking is gebaseerd op de canonieke Skill-metadata nadat de hernoeming is geïndexeerd. Bestaande statistieken blijven bij de Skill.
...
- Oude slugs die zijn gemaakt door eigenaar-hernoem-/mergeflows lossen op naar de canonieke Skill.
...
Rapporteer een Skill voor moderatorbeoordeling. Rapporten zijn op Skill-niveau, optioneel gekoppeld aan een versie, en voeden de wachtrij voor Skill-rapporten.
...
`note` is vereist voor `confirmed` en `dismissed`; deze mag worden weggelaten wanneer `status` terug op `open` wordt gezet. Geef `finalAction: "hide"` mee met een getriageerd rapport om de Skill in dezelfde controleerbare workflow te verbergen.
...
- Scanverzoek-payloads en downloadbare rapporten verlopen uit de scanverzoekopslag na de bewaarperiode.
- Gepubliceerde scans vereisen beheertoegang als eigenaar/uitgever, of moderator-/adminbevoegdheid op platformniveau.
...
- Retourneert `queue.queuedAhead` en `queue.position` zolang het verzoek in de wachtrij staat, zodat clients kunnen tonen hoeveel geprioriteerde handmatige scans vóór het verzoek staan. Zeer grote wachtrijen worden begrensd en gerapporteerd met `queuedAheadIsEstimate: true`.
...
- Vereist beheertoegang als eigenaar/uitgever tot de skill of Plugin, of moderator-/adminbevoegdheid op platformniveau.
...
Canonieke batchherscanroute, alleen voor admins. Deze accepteert dezelfde payloadvorm als de verouderde `POST /api/v1/skills/-/rescan-batch`.

### `POST /api/v1/skills/-/scan/batch/status`

Canonieke batchstatusroute, alleen voor admins. Deze accepteert `{ "jobIds": ["..."] }` en retourneert dezelfde geaggregeerde tellers als de verouderde `POST /api/v1/skills/-/rescan-batch/status`.
...
- Skillidentiteit, uitgeversidentiteit en metadata van de geselecteerde versie zijn velden op het hoogste envelopniveau (`slug`, `displayName`, `publisherHandle`, `version`, `resolvedFrom`, `tag`, `createdAt`), zodat shell-automatisering ze kan lezen zonder geneste wrappers uit te pakken.
...
- `security.signals.dependencyRegistry` blijft behouden voor compatibiliteit met v1-antwoorden, maar de scanner voor het bestaan van dependency-registers is buiten gebruik gesteld en deze sleutel is altijd `null`.
...
- Het antwoord bevat alleen beveiligingsgegevens. Het bevat geen Skill Card-gegevens, gegenereerde kaartstatus, lijsten met artefactbestanden of gedetailleerde scannerpayloads.
...
Uniform catalogus-eindpunt voor:
...
- `POST /api/v1/packages` is nog steeds alleen voor releases van codeplugins en bundelplugins.
- Anonieme aanroepers zien alleen openbare pakketkanalen.
- Geverifieerde aanroepers kunnen in lijst-/zoekresultaten privepakketten zien voor uitgevers waartoe ze behoren.
- `channel=private` retourneert alleen pakketten die de geverifieerde aanroeper kan lezen.

### `GET /api/v1/packages/search`

Uniforme cataloguszoekfunctie in skills + plugin-pakketten.
...
- `isOfficial` (optioneel): `true` of `false`
- `category` (optioneel): plugin-categoriefilter. Alleen ondersteund wanneer de
  aanvraag is beperkt tot plugin-pakketten. Beheerde categorieen en verouderde v1-
...
- Anonieme aanroepers zien alleen openbare pakketkanalen.
- Geverifieerde aanroepers kunnen privepakketten zoeken voor uitgevers waartoe ze behoren.
- `channel=private` retourneert alleen pakketten die de geverifieerde aanroeper kan lezen.

### `GET /api/v1/plugins`

Plugin-only catalogusweergave in codeplugin- en bundelplugin-pakketten.
...
`trending` is een zeven-daagse ranglijst voor installaties/downloads en gebruikt geen all-time totalen.
...
- Body: ZIP-archief.
- Elke geexporteerde skill begint bij `{publisher}/{slug}/`.
...
- Sorteerbesturingselementen in de browser-UI voor Plugin-zoekopdrachten herschikken de geladen relevanteresultaten,
...
- Vereist een API-token voor de pakketeigenaar, een eigenaar/beheerder van de organisatie-publisher,
...
- `trust.stale` betekent dat de vertrouwenssamenvatting is berekend uit verouderde invoer en
  moet worden behandeld als iets dat moet worden vernieuwd voordat een toestemmingsbeslissing met hoge zekerheid wordt genomen.
...
- Dit eindpunt is versie-exact. Clients moeten het aanroepen nadat ze de
  ...
  pakketmetadata.
- Privépakketten retourneren `404`, tenzij de aanroeper de eigenaar-publisher kan lezen.
- Dit eindpunt is bewust smaller dan moderatie-eindpunten voor eigenaars/moderators.
...
Retourneert de expliciete resolvermetadata voor artifacts voor een pakketversie.
...
- `packageName` wordt genormaliseerd als npm-naam; het pakket kan ontbreken voor geplande
  migraties.
- Dit volgt alleen migratiegereedheid. Het muteert OpenClaw niet en genereert
  geen ClawPacks.
...
- `all`: elke release met een handmatige override, niet-schone scanstatus of pakktrapport.
...
Moderator-/admin-eindpunt voor het innemen van pakketrapporten.
...
In quarantaine geplaatste en ingetrokken releases retourneren `403` vanaf routes voor artifact-downloads.
...
- Privépakketten retourneren `404`, tenzij de aanroeper de eigenaaruitgever kan lezen.

### `GET /api/v1/packages/{name}/download`

Downloadt het legacy deterministische ZIP-archief voor een pakketreleases.
...
- Overdrachten voor door GitHub ondersteunde skills proxyen of mirroren geen bytes. Het JSON-antwoord
  ...
  en `archiveUrl`; scan-/huidige status is een poortwachter en wordt niet opgenomen als metadata van een succesvolle payload.
...
- Toegestane formuliervelden zijn `payload`, herhaalde `files`-blobs of één `clawpack`-
  ...
  de upload-url-flow. Publicaties met gefaseerde storage-id's moeten ook het
...
- JSON-body's en door de aanroeper geleverde `payload.files`- / `payload.artifact`-
  metadata worden geweigerd.
- Directe multipart-publicatieverzoeken zijn beperkt tot 18 MB. ClawPack-tarballs kunnen
...
- Alleen de uitgever van de `openclaw`-org en de persoonlijke uitgevers van huidige leden van de `openclaw`-org
  mogen naar het `official`-kanaal publiceren.
- Publicaties namens iemand anders valideren nog steeds of de doel-eigenaarsaccount in aanmerking komt voor het officiële kanaal.
...
release te publiceren. Pakketnamen worden privéplaatsvervangende pakketten zonder releaserijen, zodat dezelfde
...
publisher van de doelgebruiker skills, pakketten of GitHub-vaardigheidsbronnen heeft.
Herstel migreert ook verouderde `ownerUserId`-velden voor de skills,
skill-slugaliassen, pakketten, pakketinspectiewaarschuwingen en afgeleide zoekdigest-rijen van de herstelde publisher, zodat
directe eigenaarspaden overeenkomen met de nieuwe publisher-bevoegdheid. Een actieve beschermde-handle-
reservering voor de herstelde handle wordt ook opnieuw toegewezen aan de vervangende gebruiker, zodat latere
profielsynchronisatie de concurrerende bevoegdheid van de voormalige gebruiker niet kan herstellen. Elke primaire tabel is begrensd op
100 rijen per toepastransactie; grotere herstelacties moeten eerst een hervatbare eigenaarsmigratie gebruiken.
GitHub-vaardigheidsbronnen zijn publisher-gebonden en worden gerapporteerd als gecontroleerd in plaats van herschreven.
...
Ban een gebruiker en verwijder vaardigheden in eigendom definitief (alleen moderator/admin).
...
Hef de ban van een gebruiker op en herstel in aanmerking komende vaardigheden (alleen admin).
...
Toon of zoek gebruikers (alleen admin).
...
Voeg een ster toe of verwijder er een (highlights). Beide eindpunten zijn idempotent.
