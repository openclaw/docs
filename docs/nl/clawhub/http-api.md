...
- `X-RateLimit-Remaining` / `RateLimit-Remaining`: exact resterend budget indien aanwezig.
  Geshaarde succesvolle requests laten deze header weg in plaats van een globale benaderde waarde terug te geven.
...
- Als `Retry-After` ontbreekt, val dan terug op `RateLimit-Reset` (of bereken dit vanuit `X-RateLimit-Reset`).
...
Dit omvat validatiefouten (`400`), ontbrekende publieke resources (`404`), auth- en
machtigingsfouten (`401`/`403`), snelheidslimieten (`429`) en geblokkeerde downloads. Clients
moeten de response-body lezen als een menselijk leesbare string. Onbekende queryparameters worden
...
- Resultaten worden teruggegeven in relevantievolgorde (embeddingsimilariteit + exacte slug-/naamtoken-boosts + een kleine populariteitsprior).
- Relevantie is sterker dan populariteit. Een precieze slug- of weergavenaamtokenmatch kan hoger scoren dan een lossere match met veel sterkere engagement.
...
- Populariteit is log-geschaald en afgetopt. Skills met hoge engagement kunnen lager ranken wanneer de querytekst een zwakkere match is.
...
- Hernoem een slug niet alleen om één query na te jagen, tenzij de nieuwe slug een betere canonieke naam voor de lange termijn is. Oude slugs worden redirect-aliassen, maar de canonieke URL, getoonde slug en toekomstige zoekdigests gebruiken de nieuwe slug.
- Hernoemingsaliassen behouden resolutie voor oude URL's en installs die via de registry oplossen, maar zoekranking is gebaseerd op de canonieke skillmetadata nadat de hernoeming is geïndexeerd. Bestaande statistieken blijven bij de skill.
...
- `sort` (optioneel): `updated` (standaard), `recommended` (alias: `default`), `createdAt` (alias: `newest`), `downloads`, `stars` (alias: `rating`), legacy-installaliassen `installsCurrent`/`installs`/`installsAllTime` wijzen naar `downloads`, `trending`
- `nonSuspiciousOnly` (optioneel): `true` om verdachte (`flagged.suspicious`) skills te verbergen
- `nonSuspicious` (optioneel): legacy-alias voor `nonSuspiciousOnly`
...
- Oude slugs die zijn aangemaakt door flows voor hernoemen/samenvoegen van owners lossen op naar de canonieke skill.
...
aan een versie, en voeden de skillrapportagewachtrij.
...
Geeft beveiligingsscanverificatiedetails voor een skillversie terug.
...
- Retourneert `queue.queuedAhead` en `queue.position` tijdens het wachten in de wachtrij, zodat clients kunnen tonen hoeveel geprioriteerde handmatige scans vóór het verzoek staan. Zeer grote wachtrijen worden begrensd en gerapporteerd met `queuedAheadIsEstimate: true`.
...
Canonieke batch-rescanroute alleen voor beheerders. Deze accepteert dezelfde payloadvorm als de verouderde `POST /api/v1/skills/-/rescan-batch`.

### `POST /api/v1/skills/-/scan/batch/status`

Canonieke batch-statusroute alleen voor beheerders. Deze accepteert `{ "jobIds": ["..."] }` en retourneert dezelfde geaggregeerde tellers als de verouderde `POST /api/v1/skills/-/rescan-batch/status`.
...
- Skill-identiteit, uitgeveridentiteit en metadata van de geselecteerde versie zijn velden op het hoogste envelopniveau (`slug`, `displayName`, `publisherHandle`, `version`, `resolvedFrom`, `tag`, `createdAt`), zodat shellautomatisering ze kan lezen zonder geneste wrappers uit te pakken.
...
- `security.signals.dependencyRegistry` blijft behouden voor compatibiliteit met v1-reacties, maar de scanner voor het bestaan van dependencyregisters is buiten gebruik gesteld en deze sleutel is altijd `null`.
...
- `security.signals` bevat alleen ondersteunend bewijs op statusniveau; gebruik `/scan` of de ClawHub-pagina voor beveiligingsaudits voor volledige scannergegevens.
- `security.signals.dependencyRegistry` blijft behouden voor compatibiliteit met v1-reacties, maar de scanner voor het bestaan van dependencyregisters is buiten gebruik gesteld en deze sleutel is altijd `null`.
...
- Exportspecifieke metadata per Plugin wordt opgeslagen in
...
- De legacy v1-filteraliassen die zijn gedocumenteerd onder `GET /api/v1/plugins` worden ook
  geaccepteerd.
- Categorie filtering is een echte API-filter, ondersteund door Plugin-categoriedigest-
...
clients. Dit is het openbare consumptieoppervlak van OpenClaw om te beslissen of een
...
- `trust.stale` betekent dat de vertrouwenssamenvatting is berekend uit verouderde invoer en
  moet worden behandeld als iets dat een vernieuwing vereist voordat een toestemmingsbeslissing met hoge zekerheid wordt genomen.
...
- Dit is het resolveroppervlak van OpenClaw; het voorkomt raden naar het archiefformaat op basis van
  een gedeelde URL.
...
Beheerderendpoint voor het maken of bijwerken van een officiele Plugin-migratierij.
...
Moderator-/beheerderendpoint voor reviewwachtrijen van pakketreleases.
...
Meld een pakket voor moderatorreview. Meldingen zijn op pakketniveau en optioneel
...
Downloadt een gehoste skillversie-ZIP, of retourneert een overdracht naar GitHub-broncode voor een
...
- Overdrachten voor door GitHub ondersteunde skills proxyen of mirroren geen bytes. Het JSON-antwoord
  ...
  en `archiveUrl`; scan-/huidige status is een gate en wordt niet opgenomen als metadata in een succesvol
  payload.
...
- Alleen de `openclaw`-org-publisher en persoonlijke publishers van huidige `openclaw`-orgleden
...
Geauthenticeerde self-service aanmaak van org-publishers. Maakt een nieuwe org-publisher aan en voegt de
aanroeper toe als eigenaar. Dit eindpunt migreert bestaande gebruikers-/persoonlijke handles niet en markeert
...
Het endpoint gebruikt standaard dry-run. Recovery toepassen vereist `dryRun: false` en
`confirmIdentityVerified: true` nadat medewerkers onafhankelijk de continuïteit tussen beide
GitHub-principals hebben geverifieerd. Recovery weigert standaard wanneer de huidige persoonlijke
publisher van de bestemmingsgebruiker Skills, pakketten of GitHub-Skill-bronnen heeft.
Recovery migreert ook legacy-velden `ownerUserId` voor de Skills van de herstelde publisher,
Skill-slugaliassen, pakketten, waarschuwingen van de pakketinspecteur en afgeleide zoekdigest-rijen, zodat
paden met directe eigenaar overeenkomen met de nieuwe publisher-autoriteit. Een actieve reservering met protected handle
...
100 rijen per apply-transactie; grotere recoveries moeten eerst een hervatbare eigenaarsmigratie gebruiken.
...
### Endpoints voor eigenaarsslugbeheer
...
- Beide endpoints vereisen authenticatie met een API-token en werken alleen voor de Skill-eigenaar.
...
Als je self-host, serveer dan dit bestand (of stel `CLAWHUB_REGISTRY` expliciet in; legacy `CLAWDHUB_REGISTRY`).
