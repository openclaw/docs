...
de canonieke URL's gebruiken die door leesendpoints worden geretourneerd in plaats van routeprioriteit
te reconstrueren.
...
- Geauthenticeerde schrijfendpoints moeten geen kale `Unauthorized` teruggeven wanneer
...
- `X-RateLimit-Remaining` / `RateLimit-Remaining`: exact resterend budget wanneer aanwezig.
  Geshaarde succesvolle verzoeken laten deze koptekst weg in plaats van een benaderde globale waarde terug te geven.
...
- Gebruikt vertrouwde client-IP-kopteksten, inclusief `cf-connecting-ip`, alleen wanneer de
  deployment vertrouwde doorgestuurde kopteksten expliciet inschakelt.
- ClawHub gebruikt vertrouwde forwarding-kopteksten om client-IP's aan de edge te identificeren.
- Als er geen vertrouwd client-IP beschikbaar is, gebruiken anonieme verzoeken fallback-buckets
  die alleen zijn afgebakend op snelheidslimiettype. Deze fallback-buckets bevatten geen
...
- Resultaten worden teruggegeven in relevantievolgorde (embedding-gelijkenis + exacte slug-/naamtoken-boosts + een kleine populariteitsprior).
...
- Populariteit is logaritmisch geschaald en afgetopt. Skills met veel betrokkenheid kunnen lager scoren wanneer de querytekst een zwakkere match is.
...
- Zet de termen waar gebruikers letterlijk naar zullen zoeken in de weergavenaam, samenvatting en tags. Gebruik alleen een zelfstandig slug-token wanneer het ook een stabiele identiteit is die je wilt behouden.
...
- Oude slugs die zijn gemaakt door eigenaar-hernoemings-/samenvoegingsflows worden opgelost naar de canonieke Skill.
...
Geeft details voor beveiligingsscanverificatie terug voor een Skill-versie.
...
- Scanverzoekpayloads en downloadbare rapporten verlopen uit de scan-requestopslag na de retentieperiode.
...
Canonieke batchroute voor opnieuw scannen, alleen voor admins. Deze accepteert dezelfde payloadvorm als legacy `POST /api/v1/skills/-/rescan-batch`.
...
- Skillidentiteit, uitgeveridentiteit en geselecteerde versiemetadata zijn envelopvelden op het hoogste niveau (`slug`, `displayName`, `publisherHandle`, `version`, `resolvedFrom`, `tag`, `createdAt`), zodat shellautomatisering ze kan lezen zonder geneste wrappers uit te pakken.
...
- `security.signals.dependencyRegistry` blijft behouden voor compatibiliteit met v1-antwoorden, maar de scanner voor het bestaan van dependency-registries is uitgefaseerd en deze sleutel is altijd `null`.
- `provenance` is alleen `server-resolved-github-import` wanneer ClawHub tijdens publicatie of import een GitHub-repo/ref/commit/path heeft herleid en opgeslagen; anders is deze `unavailable`.
...
- Het antwoord is alleen beveiligingsgericht. Het bevat geen Skill Card-gegevens, gegenereerde kaartstatus, lijsten met artifactbestanden of gedetailleerde scannerpayloads.
...
- Anonieme aanroepers zien alleen openbare pakketkanalen.
- Geverifieerde aanroepers kunnen zoeken in privé-pakketten voor uitgevers waartoe zij behoren.
- `channel=private` retourneert alleen pakketten die de geverifieerde aanroeper kan lezen.
...
`trending` is een zeven-daagse ranglijst voor installaties/downloads en gebruikt geen totalen over de volledige looptijd.
...
- Body: ZIP-archief.
- Elke geëxporteerde Plugin heeft als hoofdmap `{family}/{packageName}/`.
...
Alleen-Plugin-zoekactie in code-plugin- en bundle-plugin-pakketten.
...
- De verouderde v1-filteraliassen die zijn gedocumenteerd onder `GET /api/v1/plugins` worden ook
  geaccepteerd.
- Categoriefiltering is een echt API-filter, ondersteund door Plugin-categoriedigest-
...
- Sorteerknoppen in de browser-UI voor Plugin-zoekopdrachten herschikken de geladen relevantieresultaten,
  passend bij het huidige `/skills`-browsegedrag.
...
- Privépakketten retourneren `404`, tenzij de aanroeper de eigenaarspublisher mag lezen.

### `DELETE /api/v1/packages/{name}`

Verwijdert een pakket en alle releases op zachte wijze.

Opmerkingen:

- Vereist een API-token voor de pakketeigenaar, een eigenaar/beheerder van de organisatiepublisher,
...
- `trust.pending` betekent dat één of meer vertrouwensinvoerwaarden nog op voltooiing wachten.
- `trust.stale` betekent dat de vertrouwenssamenvatting is berekend uit verouderde invoer en
  moet worden behandeld als vereist vernieuwing voordat een toestemmingsbeslissing met hoge betrouwbaarheid wordt genomen.

Opmerkingen:

- Dit eindpunt is versie-exact. Clients moeten het aanroepen na het oplossen van de
...
- Dit eindpunt is bewust smaller dan moderatie-eindpunten voor eigenaars/moderators.
  ...
  reporteridentiteiten, rapportteksten, privébewijs of interne review-
  tijdlijnen.
...
- `all`: elke release met een handmatige override, niet-schone scanstatus of pakketrappport.
...
downloads en blokkeren ze ook niet zelfstandig; moderators moeten releasemoderatie gebruiken om
...
Moderator-/adminendpoint voor invoer van pakketrapporten.
...
- Plugin-/pakketarchieven zijn zipbestanden met een `package/`-root, zodat oude OpenClaw-clients
  blijven werken.
...
Streamt de exacte geüploade bytes van de ClawPack-tarball voor npm-mirrorclients.
...
- Bronoverdrachten voor door GitHub ondersteunde skills proxyen of spiegelen geen bytes. De JSON-respons
  bevat `sourceRef: "public-github"`, `repo`, `commit`, `path`, `contentHash`
  en `archiveUrl`; scan-/huidige status is een gate en wordt niet als metadata van een succesvolle payload
  opgenomen.
...
- Optioneel payloadveld: `ownerHandle`. Indien aanwezig lost de API die
  publisher server-side op en vereist deze dat de actor publishertoegang heeft.
...
Door de eigenaar geïnitieerde soft deletes reserveren de slug 30 dagen; daarna kan de slug door
een andere publisher worden geclaimd. De delete-respons bevat `slugReservedUntil` wanneer deze vervaldatum van toepassing is.
...
verouderde gedeelde gebruiker/persoonlijke publisher verwijst, migreert het endpoint deze eerst naar een org-publisher.
...
aanroeper toe als eigenaar. Dit endpoint migreert geen bestaande gebruikers-/persoonlijke handles en
markeert de publisher niet als vertrouwd/officieel.
...
Lijst met gebruikers weergeven of gebruikers zoeken (alleen admin).
