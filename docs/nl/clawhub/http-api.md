...
x-i18n:
    generated_at: "2026-07-01T20:26:33Z"
    model: gpt-5.5
...
- Geverifieerde schrijf-eindpunten mogen geen kale `Unauthorized` retourneren wanneer
...
- `X-RateLimit-Remaining` / `RateLimit-Remaining`: exact resterend budget indien aanwezig.
  Geslaagde sharded verzoeken laten deze header weg in plaats van een benaderde globale waarde te retourneren.
...
- Als er geen vertrouwd client-IP beschikbaar is, gebruiken anonieme verzoeken fallback-buckets
  die alleen zijn afgebakend op soort snelheidslimiet. Deze fallback-buckets bevatten geen
...
moeten de antwoordbody lezen als een voor mensen leesbare tekenreeks. Onbekende queryparameters worden
...
- Resultaten worden geretourneerd op volgorde van relevantie (embedding-gelijkenis + exacte slug-/naamtoken-boosts + een kleine populariteitsprior).
- Relevantie is sterker dan populariteit. Een precieze slug of overeenkomst met een display-name-token kan hoger scoren dan een lossere overeenkomst met veel sterkere betrokkenheid.
- ASCII-tekst wordt getokenized op woord- en interpunctiegrenzen. `personal-map` bevat bijvoorbeeld een zelfstandig `map`-token, terwijl `amap-jsapi-skill` `amap`, `jsapi` en `skill` bevat; zoeken naar `map` geeft `personal-map` daarom een sterkere lexicale overeenkomst dan `amap-jsapi-skill`.
- Populariteit wordt log-geschaald en begrensd. Skills met hoge betrokkenheid kunnen lager scoren wanneer de querytekst een zwakkere overeenkomst is.
...
- `sort` (optioneel): `updated` (standaard), `recommended` (alias: `default`), `createdAt` (alias: `newest`), `downloads`, `stars` (alias: `rating`), verouderde install-aliassen `installsCurrent`/`installs`/`installsAllTime` worden gekoppeld aan `downloads`, `trending`
- `nonSuspiciousOnly` (optioneel): `true` om verdachte (`flagged.suspicious`) Skills te verbergen
- `nonSuspicious` (optioneel): verouderde alias voor `nonSuspiciousOnly`
...
- Oude slugs die zijn gemaakt door owner-hernoemings-/mergeflows worden opgelost naar de canonieke Skill.
...
Retourneert verificatiedetails van de beveiligingsscan voor een Skill-versie.
...
- Scanverzoekpayloads en downloadbare rapporten verlopen uit de opslag voor scanverzoeken na de bewaarperiode.
...
- Skill-identiteit, uitgeveridentiteit en metadata van de geselecteerde versie zijn top-level envelopevelden (`slug`, `displayName`, `publisherHandle`, `version`, `resolvedFrom`, `tag`, `createdAt`), zodat shellautomatisering ze kan lezen zonder geneste wrappers uit te pakken.
...
- `security.signals.dependencyRegistry` blijft behouden voor compatibiliteit met v1-antwoorden, maar de scanner voor het bestaan van het afhankelijkheidsregister is buiten gebruik gesteld en deze sleutel is altijd `null`.
...
- Het antwoord bevat alleen beveiligingsgegevens. Het bevat geen Skill Card-gegevens, status van gegenereerde kaarten, artifact-bestandslijsten of gedetailleerde scannerpayloads.
...
- `category` (optioneel): filter voor Plugin-categorie. Alleen ondersteund wanneer het
...
- `POST /api/v1/packages` is nog steeds alleen bedoeld voor releases van code-Plugins en bundel-Plugins.
- Anonieme aanroepers zien alleen openbare pakketkanalen.
- Geverifieerde aanroepers kunnen privé-pakketten zien voor publishers waar ze bij horen in lijst-/zoekresultaten.
...
- Anonieme aanroepers zien alleen openbare pakketkanalen.
- Geverifieerde aanroepers kunnen privé-pakketten zoeken voor publishers waar ze bij horen.
- `channel=private` retourneert alleen pakketten die de geverifieerde aanroeper kan lezen.

### `GET /api/v1/plugins`

Catalogusweergave alleen voor Plugins over code-Plugin- en bundel-Plugin-pakketten.
...
Verouderde aliassen worden niet geaccepteerd als opgeslagen of door de auteur opgegeven categoriewaarden.
...
Zoekopdracht alleen voor Plugins in `code-plugin`- en `bundle-plugin`-pakketten.
...
- Resultaten worden geretourneerd op relevantievolgorde en zijn momenteel niet gepagineerd.
- Sorteerbesturingen in de browser-UI voor Plugin-zoeken herschikken de geladen relevantieresultaten,
...
clients. Dit is de openbare OpenClaw-gebruiksinterface om te bepalen of een
...
- `trust.moderationState` is nullable. Deze is `null` wanneer er geen handmatige release-
  moderatie bestaat.
- `trust.blockedFromDownload` is het installatiesignaal voor blokkeren. OpenClaw en andere
...
- `trust.stale` betekent dat de vertrouwenssamenvatting is berekend op basis van verouderde invoer en
  moet worden behandeld als iets dat een verversing vereist vóór een toestemmingsbeslissing met hoge zekerheid.
...
- Dit endpoint is bewust smaller dan moderatie-endpoints voor eigenaren/moderators.
  ...
  identiteiten van melders, rapportinhoud, privébewijs of interne review-
  tijdlijnen.
...
- Dit is de OpenClaw-resolverinterface; deze voorkomt dat het archiefformaat wordt gegist op basis van
  een gedeelde URL.
...
Moderator-/admin-eindpunt voor ontvangst van pakketmeldingen.
...
- Antwoorden bevatten de headers `ETag`, `Digest`, `X-ClawHub-Artifact-Type` en
  `X-ClawHub-Artifact-Sha256` voor integriteitscontroles door resolvers.
- Metadata die alleen voor het register is, wordt niet in het gedownloade archief geïnjecteerd.
...
- Overdrachten voor GitHub-ondersteunde skills proxieën of spiegelen geen bytes. Het JSON-antwoord
  bevat `sourceRef: "public-github"`, `repo`, `commit`, `path`, `contentHash`,
  en `archiveUrl`; scan-/huidige status is een gate en wordt niet opgenomen als metadata in de succes-
  payload.
...
- Optioneel payloadveld: `migrateOwner`. Wanneer `true` met `ownerHandle`, mag een
  ...
  de huidige als doeluitgevers. Zonder deze opt-in worden eigenaarswijzigingen
  geweigerd.
...
- JSON-bodies en door de aanroeper aangeleverde metadata `payload.files` / `payload.artifact`
  worden geweigerd.
...
- Publicaties namens iemand anders valideren de geschiktheid voor het officiële kanaal nog steeds tegen het doel-eigenaaraccount.
...
provideraccount-ID's benoemen; veranderlijke handles worden alleen gebruikt als operatorgerichte guard.
...
direct-owner-paden overeenkomen met de nieuwe publisher-bevoegdheid. Een actieve protected-handle-
...
Voeg een star toe of verwijder deze (highlights). Beide endpoints zijn idempotent.
