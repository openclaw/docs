...
- Geauthenticeerde schrijf-eindpunten mogen geen kale `Unauthorized` teruggeven wanneer de server de reden kent. Ontbrekende tokens, ongeldige/ingetrokken tokens en verwijderde/gebanneerde/uitgeschakelde accounts moeten elk bruikbare tekst krijgen, zodat CLI-clients gebruikers kunnen vertellen wat hen blokkeerde.
...
- `X-RateLimit-Remaining` / `RateLimit-Remaining`: exact resterend budget indien aanwezig.
  Geslaagde requests in shards laten deze header weg in plaats van een benaderde globale waarde terug te geven.
...
- Gebruikt vertrouwde client-IP-headers, waaronder `cf-connecting-ip`, alleen wanneer de deployment vertrouwde doorgestuurde headers expliciet inschakelt.
- ClawHub gebruikt vertrouwde forwarding-headers om client-IP's aan de edge te identificeren.
- Als er geen vertrouwd client-IP beschikbaar is, gebruiken anonieme requests fallback-buckets die alleen zijn gescopeerd per type snelheidslimiet. Deze fallback-buckets bevatten geen door de caller aangeleverde paden, slugs, pakketnamen, versies, querystrings of andere artifact-parameters.
...
- Resultaten worden teruggegeven op relevantievolgorde (embedding-gelijkenis + exacte slug-/naamtoken-boosts + een kleine populariteitsprior).
- Relevantie is sterker dan populariteit. Een precieze slug- of weergavenaamtoken-match kan hoger ranken dan een lossere match met veel sterkere betrokkenheid.
- ASCII-tekst wordt getokenized op woord- en leestekengrenzen. Bijvoorbeeld: `personal-map` bevat een losse `map`-token, terwijl `amap-jsapi-skill` `amap`, `jsapi` en `skill` bevat; zoeken naar `map` geeft `personal-map` daarom een sterkere lexicale match dan `amap-jsapi-skill`.
- Populariteit wordt log-geschaald en begrensd. Skills met veel betrokkenheid kunnen lager ranken wanneer de querytekst een zwakkere match is.
...
- `trending` rankt op installs in de afgelopen 7 dagen (gebaseerd op telemetry).
...
- Wanneer `nonSuspiciousOnly=true`, kunnen cursorgebaseerde sorteringen minder dan `limit` items op een pagina teruggeven, omdat verdachte Skills worden gefilterd na het ophalen van de pagina.
...
- Oude slugs die zijn aangemaakt door hernoemings-/mergeflows van owners worden opgelost naar de canonieke Skill.
...
`note` is vereist voor `confirmed` en `dismissed`; dit mag worden weggelaten wanneer `status` terug wordt gezet naar `open`. Geef `finalAction: "hide"` mee met een getriageerd rapport om de Skill in dezelfde controleerbare workflow te verbergen.
...
- `version.security` bevat genormaliseerde verificatiestatus van scans en scannerdetails (VirusTotal + LLM), indien beschikbaar.

### `GET /api/v1/skills/{slug}/scan`

Retourneert verificatiedetails van beveiligingsscans voor een Skill-versie.
...
- Als `version` noch `tag` is opgegeven, wordt de nieuwste versie gebruikt.
...
Canonieke batchroute voor herscans, alleen voor admins. Deze accepteert dezelfde payloadvorm als de legacy `POST /api/v1/skills/-/rescan-batch`.
...
- `security.signals.dependencyRegistry` blijft behouden voor compatibiliteit met v1-responsen, maar de scanner voor het bestaan van dependencyregistries is uitgefaseerd en deze sleutel is altijd `null`.
...
- De respons bevat alleen beveiligingsinformatie. Deze bevat geen Skill Card-gegevens, gegenereerde kaartstatus, lijsten met artifactbestanden of gedetailleerde scannerpayloads.
...
- Gebruik `/verify` wanneer je de Skill Card-verificatie-envelope voor één Skill nodig hebt, `/card` wanneer je gegenereerde kaart-Markdown nodig hebt, en `/scan` wanneer je gedetailleerde scannerdata nodig hebt.
...
- `category` (optioneel): filter voor plugincategorie. Alleen ondersteund wanneer het
...
Geünificeerd zoeken in de catalogus over skills + pluginpakketten.
...
`trending` is een zeven-daags installatie-/downloadklassement en gebruikt geen totalen over alle tijd.
...
- Gehoste skills bevatten de nieuwste opgeslagen versiebestanden en staan vermeld in
...
Bulkexport van nieuwste openbare pluginreleases voor offline analyse.
...
Alleen-plugin zoekfunctie in code-plugin- en bundle-plugin-pakketten.
...
- De verouderde v1-filteraliassen die zijn gedocumenteerd onder `GET /api/v1/plugins` worden ook
  geaccepteerd.
- Categoriefiltering is een echte API-filter, ondersteund door digest-rijen voor plugincategorieen,
  geen herschrijving van de zoekquery.
- Resultaten worden teruggegeven op relevantievolgorde en hebben momenteel geen paginering.
- Sorteerknoppen in de browser-UI voor pluginzoekopdrachten herschikken de geladen relevantieresultaten,
  overeenkomstig het huidige `/skills`-browsegedrag.

### `GET /api/v1/packages/{name}`

Geeft detailmetadata van een pakket terug.

Opmerkingen:

- Skills kunnen in de uniforme catalogus ook via deze route worden opgelost.
- Privepakketten geven `404` terug, tenzij de aanroeper de eigenaar-uitgever kan lezen.
...
Geeft een pakketversie terug, inclusief bestandsmetadata, compatibiliteit,
...
- `version.sha256hash` is verouderde compatibiliteitsmetadata voor oude clients. Het
  hasht exact de ZIP-bytes die worden teruggegeven door `/api/v1/packages/{name}/download`.
  Moderne clients moeten `version.artifact.sha256` gebruiken, waarmee het
  canonieke releaseartifact wordt geidentificeerd.
- `version.vtAnalysis`, `version.llmAnalysis` en `version.staticScan` worden
  opgenomen wanneer scandata bestaan.
- Privepakketten geven `404` terug, tenzij de aanroeper de eigenaar-uitgever kan lezen.

### `GET /api/v1/packages/{name}/versions/{version}/security`

Geeft de exacte beveiligings- en vertrouwenssamenvatting van de pakketrelease terug voor installatieclients. Dit is het openbare OpenClaw-consumptieoppervlak om te bepalen of een
...
- Dit endpoint is bewust smaller dan moderatie-endpoints voor eigenaars/moderators.
  ...
  reporteridentiteiten, rapportinhoud, privebewijs of interne reviewtijdlijnen.
...
- Verouderde pakketversies geven een `legacy-zip`-artifact en een verouderde ZIP-
  `downloadUrl` terug.
- ClawPack-versies geven een `npm-pack`-artifact, npm-integriteitsvelden, een
  `tarballUrl` en de verouderde ZIP-compatibiliteits-URL terug.
...
- ClawPack-versies streamen exact de geuploade npm-pack `.tgz`-bytes.
- Verouderde ZIP-versies redirecten naar `/api/v1/packages/{name}/download?version=`.
- Gebruikt de downloadratelimietbucket.

### `GET /api/v1/packages/{name}/readiness`

Geeft berekende gereedheid voor toekomstig OpenClaw-gebruik terug.
...
Moderator-/beheerdersendpoint voor pakketrelease-reviewwachtrijen.
...
Moderator-/admin-eindpunt voor inname van pakketterapporten.
...
Moderator-/admin-eindpunt voor het oplossen of heropenen van pakketterapporten.
...
- Bestandsgroottelimiet: 200 KB.
- Wachtende VirusTotal-scans blokkeren leesacties niet; schadelijke releases kunnen elders nog steeds worden achtergehouden.
...
Retourneert een npm-compatibele packument voor pakketversies met ClawPack-backend.
...
- Scoped package-packuments ondersteunen zowel `/api/npm/@scope/name` als het door npm
...
Downloadt een gehoste ZIP van een Skill-versie, of retourneert een GitHub-bronhandoff voor een
...
- GitHub-backed Skill-handoffs proxyen of mirroren geen bytes. De JSON-respons
  bevat `sourceRef: "public-github"`, `repo`, `commit`, `path`, `contentHash`
  en `archiveUrl`; scan-/huidige status is een gate en wordt niet opgenomen als metadata van een geslaagde payload.
...
- Toegestane formuliervelden zijn `payload`, herhaalde `files`-blobs, of één `clawpack`
  tarball-referentie. `clawpack` mag een `.tgz`-blob zijn of een storage-id die is geretourneerd door
  de upload-url-flow. Gefaseerde publicaties met storage-id moeten ook het
...
- Directe multipart-publicatieverzoeken zijn begrensd op 18 MB. ClawPack-tarballs mogen
  de upload-url-flow gebruiken tot aan de tarball-limiet van 120 MB.
...
- Publicaties namens iemand anders valideren de geschiktheid voor het official-kanaal nog steeds tegen het doel-eigenaarsaccount.
...
Wanneer aanwezig, wordt `reason` opgeslagen als de Skill-moderatienotitie en gekopieerd naar het auditlog.
...
aanroeper toe als eigenaar. Dit eindpunt migreert bestaande gebruikers-/persoonlijke handles niet en
...
directe eigenaarspaden overeenkomen met de nieuwe publisher-autoriteit. Een actieve reservering voor een beschermde handle
...
### Endpoints voor eigenaars-slugbeheer
...
Verban een gebruiker en verwijder in eigendom zijnde Skills definitief (alleen moderator/admin).
...
Gebruikers vermelden of zoeken (alleen admin).
...
Voeg een ster toe of verwijder er een (highlights). Beide endpoints zijn idempotent.
