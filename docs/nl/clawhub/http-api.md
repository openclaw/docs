...
x-i18n:
    generated_at: "2026-07-01T18:15:42Z"
    model: gpt-5.5
...
Directories van derden mogen de publieke lees-eindpunten gebruiken om ClawHub Skills te tonen of te doorzoeken. Cache resultaten, respecteer `429`/`Retry-After`, verwijs gebruikers terug naar de canonieke ClawHub-vermelding (`https://clawhub.ai/<owner>/skills/<slug>`) en wek niet de indruk dat ClawHub de site van derden ondersteunt. Probeer geen verborgen, privé- of door moderatie geblokkeerde inhoud buiten het publieke API-oppervlak te spiegelen.
...
prioriteit opnieuw te reconstrueren.
...
- Als er geen vertrouwd client-IP beschikbaar is, gebruiken anonieme verzoeken fallback-buckets
  die alleen zijn gescopet op soort snelheidslimiet. Deze fallback-buckets bevatten geen
...
toestemmingsfouten (`401`/`403`), snelheidslimieten (`429`) en geblokkeerde downloads. Clients
...
- Resultaten worden teruggegeven in volgorde van relevantie (embedding-overeenkomst + boosts voor exacte slug-/naamtoken + een kleine populariteitsprior).
...
- Verdachte of verborgen moderatiestatus kan een Skill uit publieke zoekresultaten verwijderen, afhankelijk van aanroepersfilters en huidige moderatiestatus.

Richtlijnen voor vindbaarheid voor publishers:

- Zet de termen waar gebruikers letterlijk naar zoeken in de weergavenaam, samenvatting en tags. Gebruik een zelfstandige slug-token alleen wanneer die ook een stabiele identiteit is die je wilt behouden.
...
- Hernoemaliassen behouden oplossing voor oude URL's en installaties die via de registry worden opgelost, maar zoekranking is gebaseerd op de canonieke Skill-metadata nadat de hernoeming is geïndexeerd. Bestaande statistieken blijven bij de Skill.
...
- Wanneer `nonSuspiciousOnly=true`, kunnen cursorgebaseerde sorteringen minder dan `limit` items op een pagina teruggeven doordat verdachte Skills na het ophalen van de pagina worden gefilterd.
...
- Oude slugs die door owner-hernoemings-/mergeflows zijn gemaakt, worden opgelost naar de canonieke Skill.
...
- Bewijs wordt geredigeerd voor publieke aanroepers en bevat alleen ruwe snippets voor owners/moderators.
...
Geeft beveiligingsscanverificatiedetails terug voor een Skill-versie.
...
- Gepubliceerde scans vereisen beheerrechten van eigenaar/uitgever, of platformmoderator-/adminbevoegdheid.
...
- Vereist beheerrechten van eigenaar/uitgever voor de skill of plugin, of platformmoderator-/adminbevoegdheid.
...
- Skill-identiteit, uitgeversidentiteit en metadata van de geselecteerde versie zijn envelope-velden op topniveau (`slug`, `displayName`, `publisherHandle`, `version`, `resolvedFrom`, `tag`, `createdAt`), zodat shellautomatisering ze kan lezen zonder geneste wrappers uit te pakken.
- `security` is het ClawScan-/security-oordeel op topniveau. Automatisering moet zich baseren op `ok`, `decision`, `reasons` en `security.status`.
- `security.signals` bevat ondersteunend scannerbewijs zoals `staticScan`, `virusTotal` en `skillSpector`.
- `security.signals.dependencyRegistry` wordt behouden voor v1-antwoordcompatibiliteit, maar de scanner voor het bestaan van het dependencyregister is buiten gebruik gesteld en deze sleutel is altijd `null`.
...
- Resultaten zijn per item; één ontbrekende skill of versie laat het hele antwoord niet mislukken.
- Het antwoord is alleen security. Het bevat geen Skill Card-gegevens, gegenereerde kaartstatus, lijsten met artifact-bestanden of gedetailleerde scannerpayloads.
- `security.signals` bevat alleen ondersteunend bewijs op statusniveau; gebruik `/scan` of de ClawHub-security-auditpagina voor volledige scannergegevens.
- `security.signals.dependencyRegistry` wordt behouden voor v1-antwoordcompatibiliteit, maar de scanner voor het bestaan van het dependencyregister is buiten gebruik gesteld en deze sleutel is altijd `null`.
- Het ontbreken van een Skill Card heeft geen invloed op de `ok`, `decision` of `reasons` van dit eindpunt; clients moeten de geïnstalleerde `skill-card.md` lokaal lezen wanneer ze kaartinhoud nodig hebben.
...
- `category` (optioneel): plugin-categoriefilter. Alleen ondersteund wanneer de
  request is beperkt tot pluginpakketten (`/api/v1/plugins`,
...
- Anonieme callers zien alleen publieke pakketkanalen.
- Geauthenticeerde callers kunnen private pakketten zien voor publishers waartoe ze behoren in lijst-/zoekresultaten.
- `channel=private` retourneert alleen pakketten die de geauthenticeerde caller kan lezen.

### `GET /api/v1/packages/search`

Uniform zoeken in catalogus over skills + pluginpakketten.
...
- `isOfficial` (optioneel): `true` of `false`
- `category` (optioneel): plugin-categoriefilter. Alleen ondersteund wanneer de
  request is beperkt tot pluginpakketten. Gecontroleerde categorieën en legacy v1-
...
- Anonieme callers zien alleen publieke pakketkanalen.
- Geauthenticeerde callers kunnen zoeken in private pakketten voor publishers waartoe ze behoren.
- `channel=private` retourneert alleen pakketten die de geauthenticeerde caller kan lezen.

### `GET /api/v1/plugins`

Catalogusweergave alleen voor Plugin over code-plugin- en bundle-plugin-pakketten.
...
`trending` is een zeven-daags installatie-/downloadklassement en gebruikt geen totalen over de volledige periode.
...
Bulkexport van nieuwste publieke skills voor offlineanalyse.
...
- De legacy v1-filteraliassen die zijn gedocumenteerd onder `GET /api/v1/plugins` worden ook
  geaccepteerd.
- Categoriefiltering is een echt API-filter, ondersteund door samenvattingsrijen voor Plugin-categorieën,
  geen herschrijving van zoekquery's.
- Resultaten worden geretourneerd op volgorde van relevantie en worden momenteel niet gepagineerd.
- Sorteeropties in de browser-UI voor Plugin-zoekopdrachten herschikken de geladen relevante resultaten,
...
Dit is het openbare OpenClaw-verbruiksoppervlak om te bepalen of een
opgeloste release kan worden geïnstalleerd.

Auth:

- Openbaar leesendpoint. Er is geen token voor eigenaar, publisher, moderator of beheerder
  vereist.
...
- `trust.stale` betekent dat de vertrouwenssamenvatting is berekend op basis van verouderde invoer en
  moet worden behandeld alsof vernieuwing vereist is vóór een allow-beslissing met hoge zekerheid.
...
- Dit endpoint is bewust smaller dan moderatie-endpoints voor eigenaren/moderators.
  ...
  identiteiten van melders, rapportteksten, privébewijs of interne beoordelingstijdlijnen.
...
Beheerendpoint voor het maken of bijwerken van een officiële Plugin-migratierij.
...
Moderator-/beheerendpoint voor beoordelingswachtrijen van pakketreleases.
...
Meld een pakket voor beoordeling door een moderator. Meldingen zijn pakketniveau, optioneel
gekoppeld aan een versie. Ze voeden de moderatiewachtrij, maar verbergen downloads niet automatisch
en blokkeren deze ook niet zelf; moderators moeten releasemoderatie gebruiken om
...
Moderator-/admin-eindpunt voor inname van pakketrappportages.
...
Moderator-/admin-eindpunt voor het afhandelen of heropenen van pakketrappportages.
...
`finalAction: "revoke"` mee met een bevestigde rapportage om releasemoderatie toe te passen in dezelfde
...
- Alleen-registermetadata wordt niet geïnjecteerd in het gedownloade archief.
...
- Door GitHub ondersteunde skilloverdrachten proxyen of spiegelen geen bytes. Het JSON-antwoord
...
- Toegestane formuliervelden zijn `payload`, herhaalde `files`-blobs, of één `clawpack`
  tarballreferentie. `clawpack` mag een `.tgz`-blob zijn of een storage-id dat is geretourneerd door
...
- Optioneel payloadveld: `ownerHandle`. Wanneer aanwezig, mogen alleen admins publiceren namens die eigenaar.
...
- Code-plugins vereisen `package.json`, metadata voor bronrepo, metadata voor broncommit,
  metadata voor configuratieschema, `openclaw.compat.pluginApi` en
...
- Publicaties namens iemand anders valideren geschiktheid voor het officiële kanaal nog steeds tegen het doeleigenaarsaccount.

### `DELETE /api/v1/skills/{slug}` / `POST /api/v1/skills/{slug}/undelete`

Een skill zacht verwijderen / herstellen (eigenaar, moderator of admin).
...
Door de eigenaar geïnitieerde zachte verwijderingen reserveren de slug 30 dagen, waarna de slug kan worden geclaimd door
...
legacy gedeelde gebruiker/persoonlijke publisher wijst, migreert het eindpunt deze eerst naar een org-publisher.
...
Geauthenticeerde selfservice-aanmaak van org-publisher. Maakt een nieuwe org-publisher en voegt de
...
release te publiceren. Pakketnamen worden privé-plaatshouderpakketten zonder releaserijen, zodat dezelfde
...
100 rijen per apply-transactie; grotere herstelacties moeten eerst een hervatbare ownermigratie gebruiken.
...
Ban een gebruiker en verwijder beheerde skills permanent (alleen moderator/admin).
