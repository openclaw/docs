---
read_when:
    - Endpointen toevoegen/wijzigen
    - CLI ↔ registeraanvragen debuggen
summary: HTTP API-referentie (publiek + CLI-eindpunten + authenticatie).
x-i18n:
    generated_at: "2026-07-03T09:44:25Z"
    model: gpt-5.5
...
- Geauthenticeerde schrijf-eindpunten zouden geen kale `Unauthorized` moeten teruggeven wanneer
...
- `X-RateLimit-Remaining` / `RateLimit-Remaining`: exact resterend budget indien aanwezig.
  Geshaarde succesvolle requests laten deze header weg in plaats van een globale waarde bij benadering terug te geven.
- `Retry-After`: seconden wachten voor opnieuw proberen (vertraging) bij `429`

Voorbeeld van `429`-response:
...
- Als `Retry-After` ontbreekt, val terug op `RateLimit-Reset` (of bereken vanuit `X-RateLimit-Reset`).
...
- Als er geen vertrouwd client-IP beschikbaar is, gebruiken anonieme requests fallback-buckets
  die alleen per rate-limit-soort zijn gescopt. Deze fallback-buckets bevatten geen
  door de aanroeper aangeleverde paden, slugs, pakketnamen, versies, querystrings of andere
  artifact-parameters.

## Foutresponses

Publieke v1-foutresponses zijn platte tekst met `content-type: text/plain; charset=utf-8`.
...
- Resultaten worden teruggegeven in volgorde van relevantie (embedding-gelijkenis + exacte slug-/naamtokenboosts + een kleine populariteitsprior).
...
- Verdachte of verborgen moderatiestatus kan een skill uit publieke zoekresultaten verwijderen, afhankelijk van aanroepfilters en huidige moderatiestatus.
...
- Hernoemaliassen behouden oplossing voor oude URL's en installs die via de registry oplossen, maar zoekranking is gebaseerd op de canonieke skillmetadata nadat de hernoeming is geïndexeerd. Bestaande statistieken blijven bij de skill.
...
- Oude slugs die door flows voor owner-hernoeming/-samenvoeging zijn aangemaakt, lossen op naar de canonieke skill.
...
Geeft versiemetadata + bestandslijst terug.
...
Geeft verificatiedetails van de beveiligingsscan voor een skillversie terug.
...
- Payloads voor scanverzoeken en downloadbare rapporten verlopen uit de scanverzoekopslag na de bewaartermijn.
...
Alleen voor admins toegankelijke canonieke route voor batch-rescans. Deze accepteert dezelfde payloadvorm als de legacy `POST /api/v1/skills/-/rescan-batch`.

### `POST /api/v1/skills/-/scan/batch/status`

Alleen voor admins toegankelijke canonieke route voor batchstatus. Deze accepteert `{ "jobIds": ["..."] }` en retourneert dezelfde geaggregeerde tellers als de legacy `POST /api/v1/skills/-/rescan-batch/status`.
...
- Skill-identiteit, uitgeversidentiteit en metadata van de geselecteerde versie zijn top-level envelope-velden (`slug`, `displayName`, `publisherHandle`, `version`, `resolvedFrom`, `tag`, `createdAt`) zodat shell-automatisering ze kan lezen zonder geneste wrappers uit te pakken.
- `security` is het top-level ClawScan-/security-oordeel. Automatisering moet zich baseren op `ok`, `decision`, `reasons` en `security.status`.
- `security.signals` bevat ondersteunend scannerbewijs zoals `staticScan`, `virusTotal` en `skillSpector`.
- `security.signals.dependencyRegistry` blijft behouden voor compatibiliteit met v1-antwoorden, maar de scanner voor het bestaan van dependency-registers is uitgefaseerd en deze sleutel is altijd `null`.
- `provenance` is alleen `server-resolved-github-import` wanneer ClawHub tijdens publiceren of importeren een GitHub-repo/ref/commit/path heeft opgelost en opgeslagen; anders is het `unavailable`.
...
- Het antwoord is alleen security. Het bevat geen Skill Card-gegevens, gegenereerde kaartstatus, lijsten met artifactbestanden of gedetailleerde scannerpayloads.
...
- Gebruik `/verify` wanneer je de verificatie-envelope voor de Skill Card van één skill nodig hebt, `/card` wanneer je gegenereerde kaart-Markdown nodig hebt, en `/scan` wanneer je gedetailleerde scannergegevens nodig hebt.
...
Geeft onbewerkte tekstinhoud terug.
...
- Geauthenticeerde callers kunnen privé-pakketten zien voor publishers waartoe ze behoren in lijst-/zoekresultaten.
- `channel=private` geeft alleen pakketten terug die de geauthenticeerde caller kan lezen.
...
- Anonieme callers zien alleen openbare pakketkanalen.
- Geauthenticeerde callers kunnen zoeken in privé-pakketten voor publishers waartoe ze behoren.
- `channel=private` geeft alleen pakketten terug die de geauthenticeerde caller kan lezen.
...
Verouderde v1-filteraliassen blijven geaccepteerd op leesendpoints:
...
`trending` is een zeven-daagse install/download-ranglijst en gebruikt geen totalen over alle tijd.
Op de uniforme `/api/v1/packages`-endpoint is dit alleen voor plugins; gebruik
...
- Sorteerbesturingselementen in de browser-UI voor Plugin-zoekopdrachten herschikken de geladen relevatieresultaten,
...
- Skills kunnen ook via deze route worden omgezet in de eengemaakte catalogus.
- Private pakketten retourneren `404`, tenzij de aanroeper de eigenaar-publisher mag lezen.
...
- `version.sha256hash` is verouderde compatibiliteitsmetadata voor oude clients. Het
  ...
  canonieke release-artifact wordt geidentificeerd.
...
Retourneert de exacte beveiligings- en vertrouwenssamenvatting van de pakketrelease voor installatieclients. Dit is de openbare OpenClaw-interface voor het bepalen of een
...
- `trust.moderationState` kan null zijn. Het is `null` wanneer er geen handmatige release-
  moderatie bestaat.
- `trust.blockedFromDownload` is het blokkadesignaal voor installatie. OpenClaw en andere
...
- `trust.stale` betekent dat de vertrouwenssamenvatting is berekend op basis van verouderde invoer en
  moet worden behandeld als iets dat moet worden vernieuwd voordat een toestemmingsbeslissing met hoge zekerheid kan worden genomen.
...
- Dit eindpunt is versie-exact. Clients moeten het aanroepen nadat ze de
  ...
  pakketmetadata.
- Private pakketten retourneren `404`, tenzij de aanroeper de eigenaar-publisher mag lezen.
- Dit eindpunt is opzettelijk beperkter dan moderatie-eindpunten voor eigenaar/moderator.
  Het stelt de installatiebeslissing en openbare verklaring beschikbaar, niet
  identiteiten van melders, rapportinhoud, privebewijs of interne reviewtijdlijnen.
...
- Dit is de OpenClaw-resolverinterface; deze vermijdt het raden van het archiefformaat op basis van
  een gedeelde URL.
...
- Bestandsgroottelimiet: 200KB.
- Lopende VirusTotal-scans blokkeren leesacties niet; kwaadaardige releases kunnen elders nog worden achtergehouden.
...
- Directe multipart-publicatieaanvragen zijn beperkt tot 18MB. ClawPack-tarballs mogen
...
een andere publisher worden geclaimd. De verwijderrespons bevat `slugReservedUntil` wanneer deze vervaldatum van toepassing is.
...
Alleen admin. Zorgt dat er een org-publisher bestaat voor een handle. Als de handle nog verwijst naar een
legacy gedeelde gebruikers-/persoonlijke publisher, migreert het eindpunt die eerst naar een org-publisher.
...
release te publiceren. Pakketnamen worden private tijdelijke aanduiding-pakketten zonder releaserijen, zodat dezelfde
...
Ban een gebruiker en verwijder skills in eigendom definitief (alleen moderator/beheerder).
...
content te herstellen (alleen beheerder). Staat standaard op dry-run tenzij `dryRun` `false` is.
...
Maak een lijst van gebruikers of zoek gebruikers (alleen beheerder).
