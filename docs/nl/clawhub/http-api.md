...
prioriteit opnieuw op te bouwen.
...
- Geauthenticeerde aanvragen (geldige Bearer-token): afgedwongen per gebruikersbucket.
...
- Geauthenticeerde schrijf-eindpunten mogen geen kale `Unauthorized` teruggeven wanneer
  ...
  verwijderde/geblokkeerde/uitgeschakelde accounts moeten elk bruikbare tekst krijgen zodat CLI-
...
- `X-RateLimit-Remaining` / `RateLimit-Remaining`: exact resterend budget wanneer aanwezig.
  Geslaagde gesharde aanvragen laten deze header weg in plaats van een globale benaderingswaarde terug te geven.
...
- Resultaten worden teruggegeven op volgorde van relevantie (embedding-gelijkenis + exacte slug-/naamtoken-boosts + een kleine populariteitsprior).
- Relevantie weegt zwaarder dan populariteit. Een precieze slug- of weergavenaamtoken-match kan hoger scoren dan een lossere match met veel sterkere engagement.
- ASCII-tekst wordt getokenized op woord- en interpunctiegrenzen. Bijvoorbeeld: `personal-map` bevat een op zichzelf staande `map`-token, terwijl `amap-jsapi-skill` `amap`, `jsapi` en `skill` bevat; zoeken naar `map` geeft `personal-map` daarom een sterkere lexicale match dan `amap-jsapi-skill`.
- Populariteit wordt log-geschaald en afgetopt. Skills met hoge engagement kunnen lager scoren wanneer de querytekst een zwakkere match is.
...
- Plaats de termen waar gebruikers letterlijk op zullen zoeken in de weergavenaam, samenvatting en tags. Gebruik alleen een op zichzelf staande slug-token wanneer die ook een stabiele identiteit is die je wilt behouden.
...
- `metadata.os`: OS-beperkingen die in Skill-frontmatter zijn verklaard (bijv. `["macos"]`, `["linux"]`). `null` indien niet verklaard.
- `metadata.systems`: Nix-systeemdoelen (bijv. `["aarch64-darwin", "x86_64-linux"]`). `null` indien niet verklaard.
...
- Payloads voor scanverzoeken en downloadbare rapporten verlopen uit de scan-request-store na de retentieperiode.
...
- `kind` is standaard `skill`; gebruik `kind=plugin` voor Plugin-/pakket-scans.
- Retourneert dezelfde ZIP-structuur als downloads van scanverzoeken.

### `POST /api/v1/skills/-/scan/batch`

Canonieke batchrescanroute, alleen voor admins. Deze accepteert dezelfde payloadstructuur als de legacy `POST /api/v1/skills/-/rescan-batch`.
...
- `security` is het top-level ClawScan-/beveiligingsoordeel. Automatisering moet sturen op `ok`, `decision`, `reasons` en `security.status`.
- `security.signals` bevat ondersteunend scannerbewijs, zoals `staticScan`, `virusTotal` en `skillSpector`.
- `security.signals.dependencyRegistry` blijft behouden voor compatibiliteit met v1-antwoorden, maar de scanner voor het bestaan van dependencyregistries is uitgefaseerd en deze sleutel is altijd `null`.
...
- `security.signals.dependencyRegistry` blijft behouden voor compatibiliteit met v1-antwoorden, maar de scanner voor het bestaan van dependencyregistries is uitgefaseerd en deze sleutel is altijd `null`.
- Afwezigheid van een Skill Card heeft geen invloed op `ok`, `decision` of `reasons` van dit endpoint; clients moeten de geïnstalleerde `skill-card.md` lokaal lezen wanneer ze kaartinhoud nodig hebben.
...
- `category` (optioneel): Plugin-categoriefilter. Alleen ondersteund wanneer het
  verzoek is afgebakend tot Plugin-pakketten (`/api/v1/plugins`,
...
Uniform cataloguszoeken in Skills + Plugin-pakketten.
...
- `isOfficial` (optioneel): `true` of `false`
- `category` (optioneel): Plugin-categoriefilter. Alleen ondersteund wanneer het
  verzoek is afgebakend tot Plugin-pakketten. Beheerde categorieën en legacy v1-
...
Alleen-Plugin catalogusbrowsing in code-Plugin- en bundle-Plugin-pakketten.
...
- Categoriefiltering is een echt API-filter dat wordt ondersteund door digest-rijen voor Plugin-categorieën,
  geen herschrijving van een zoekquery.
- Resultaten worden teruggegeven op relevantievolgorde en gebruiken momenteel geen paginering.
- Sorteerbesturingselementen in de browser-UI voor Plugin-zoekopdrachten herschikken de geladen relevantieresultaten,
...
- `version.sha256hash` is verouderde compatibiliteitsmetadata voor oude clients. Het
  hasht de exacte ZIP-bytes die door `/api/v1/packages/{name}/download` worden teruggegeven.
...
Geeft de exacte beveiligings- en vertrouwenssamenvatting van de pakketreleasede terug voor installatieclients. Dit is het openbare OpenClaw-consumptieoppervlak om te bepalen of een
opgeloste release kan worden geïnstalleerd.

Authenticatie:

- Openbaar leesendpoint. Geen token van eigenaar, uitgever, moderator of beheerder is
  vereist.
...
- `release.artifactKind`, `release.artifactSha256`, `release.npmIntegrity`,
  ...
  het releaseartifact.
- `trust.scanStatus` is de effectieve vertrouwensstatus die wordt afgeleid uit scannerinvoer
...
- `trust.stale` betekent dat de vertrouwenssamenvatting is berekend op basis van verouderde invoer en
  moet worden behandeld als iets dat moet worden vernieuwd vóór een allow-beslissing met hoge zekerheid.
...
Geeft de expliciete metadata van de artifactresolver terug voor een pakketversie.
...
Beheerderendpoint voor het aanmaken of bijwerken van een officiële Plugin-migratierij.
...
Moderator-/beheerderendpoint voor beoordelingswachtrijen van pakketreleases.
...
Meld een pakket voor moderatorbeoordeling. Meldingen zijn op pakketniveau, optioneel
...
In quarantaine geplaatste en ingetrokken releases geven `403` terug vanuit artifact-downloadroutes.
Elke wijziging schrijft een auditlogvermelding.

### `GET /api/v1/packages/{name}/file`

Geeft onbewerkte tekstinhoud terug voor een pakketbestand.
...
- Lopende VirusTotal-scans blokkeren leesacties niet; schadelijke releases kunnen nog steeds elders worden achtergehouden.
- Privépakketten geven `404` terug, tenzij de aanroeper de eigenaar-uitgever kan lezen.
...
Geeft een npm-compatibele packument terug voor pakketversies met ClawPack-backing.
...
- `slug` (vereist)
- `hash` (vereist): sha256 in hex van 64 tekens van de bundelvingerafdruk
...
Downloadt een gehoste ZIP met een skillversie, of geeft een GitHub-bronhandoff terug voor een
...
- Handoffs voor GitHub-backed skills proxyen of mirroren geen bytes. Het JSON-antwoord
...
- Toegestane formuliervelden zijn `payload`, herhaalde `files`-blobs of één `clawpack`-
  ...
  de upload-url-flow wordt teruggegeven. Gepubliceerde staged storage-id's moeten ook het
...
- JSON-body's en door de aanroeper aangeleverde metadata in `payload.files` / `payload.artifact`
  worden geweigerd.
...
verouderde gedeelde gebruiker/persoonlijke uitgever wijst, migreert het eindpunt die eerst naar een org-uitgever.
...
de uitgever niet als vertrouwd/official.
...
provideraccount-id's noemen; wijzigbare handles worden alleen gebruikt als operatorgerichte bescherming.
...
skill-slugaliassen, pakketten, package-inspectorwaarschuwingen en afgeleide zoekdigest-rijen van de herstelde publisher, zodat
paden voor directe eigenaars overeenkomen met de nieuwe publisher-autoriteit. Een actieve reservering met beschermd handvat
voor het herstelde handvat wordt ook opnieuw toegewezen aan de vervangende gebruiker, zodat latere
...
- `POST /api/v1/skills/{slug}/transfer/cancel`
  - Response (accepteren/weigeren/annuleren): `{ "ok": true, "skillSlug": "demo-skill?" }`
...
Verban een gebruiker en hard-delete eigen Skills (alleen moderator/admin).
...
Maak een gebruikersban ongedaan en herstel in aanmerking komende Skills (alleen admin).
...
Lijst of zoek gebruikers (alleen admin).
...
Zie `DEPRECATIONS.md` voor verwijderingsplan.
