...
x-i18n:
    generated_at: "2026-07-01T15:26:48Z"
    model: gpt-5.5
...
- `X-RateLimit-Remaining` / `RateLimit-Remaining`: exact resterend budget indien aanwezig.
  Gesherde succesvolle requests laten deze header weg in plaats van een globale waarde bij benadering te retourneren.
...
- Als er geen vertrouwd client-IP beschikbaar is, gebruiken anonieme requests fallback-buckets
  die alleen op snelheidslimietsoort zijn gescoped. Deze fallback-buckets bevatten geen
...
- Resultaten worden geretourneerd in volgorde van relevantie (embedding-overeenkomst + exacte slug-/naamtoken-boosts + een kleine populariteitsprior).
...
- `trending` rankt op installs in de laatste 7 dagen (op basis van telemetrie).
...
- Gebruik `nextCursor` om de paginering voort te zetten wanneer aanwezig. Een korte pagina betekent op zichzelf niet het einde van de resultaten.
...
- Oude slugs die zijn gemaakt door hernoemings-/mergeflows van eigenaren worden opgelost naar de canonieke Skill.
...
Rapporteer een Skill voor beoordeling door een moderator. Rapporten gelden op Skill-niveau, zijn optioneel gekoppeld
...
- `version.security` bevat genormaliseerde verificatiestatus van de scan en scannergegevens
...
- Payloads voor scanverzoeken en downloadbare rapporten verlopen uit de scan-request-opslag na de bewaarperiode.
- Gepubliceerde scans vereisen beheerstoegang als eigenaar/uitgever, of moderator-/beheerderbevoegdheid op platformniveau.
...
- Scantaken zijn asynchroon. Handmatige scanverzoeken krijgen prioriteit boven normaal publicatie-/backfill-werk, maar voltooiing blijft afhankelijk van workerbeschikbaarheid.

### `GET /api/v1/skills/-/scan/{scanId}`

Geauthenticeerd peileindpunt voor een ingediende scan.
...
- Vereist beheerstoegang tot de skill of plugin als eigenaar/uitgever, of moderator-/beheerderbevoegdheid op platformniveau.
...
Alleen voor beheerders beschikbare canonieke route voor batchgewijs opnieuw scannen. Deze accepteert dezelfde payloadvorm als de legacy `POST /api/v1/skills/-/rescan-batch`.

### `POST /api/v1/skills/-/scan/batch/status`

Alleen voor beheerders beschikbare canonieke route voor batchstatus. Deze accepteert `{ "jobIds": ["..."] }` en retourneert dezelfde geaggregeerde tellers als de legacy `POST /api/v1/skills/-/rescan-batch/status`.
...
- `security.signals.dependencyRegistry` blijft behouden voor compatibiliteit met v1-responses, maar de scanner voor het bestaan van dependency-registers is uitgefaseerd en deze sleutel is altijd `null`.
...
- De response is alleen voor beveiliging. Deze bevat geen Skill Card-gegevens, gegenereerde kaartstatus, bestandslijsten van artifacts of gedetailleerde scannerpayloads.
...
Geünificeerd cataloguszoeken in skills + plugin-pakketten.
...
- Anonieme aanroepers zien alleen openbare pakketkanalen.
- Geauthenticeerde aanroepers kunnen privé-pakketten doorzoeken voor uitgevers waartoe ze behoren.
- `channel=private` retourneert alleen pakketten die de geauthenticeerde aanroeper kan lezen.
...
`trending` is een zeven-daags install/download-klassement en gebruikt geen totalen over de volledige tijd.
...
- Body: ZIP-archief.
- Elke geëxporteerde skill heeft als root `{publisher}/{slug}/`.
...
- Sorteerknoppen in de browser-UI voor pluginzoeken herschikken de geladen relevantieresultaten,
...
Retourneert de exacte beveiligings- en vertrouwenssamenvatting van de pakketrelease voor installatieclients. Dit is het openbare consumptieoppervlak van OpenClaw om te bepalen of een
...
- Dit eindpunt is exact per versie. Clients moeten het aanroepen nadat ze de
...
- Dit eindpunt is bewust smaller dan moderatie-eindpunten voor eigenaars/moderators.
  ...
  reporteridentiteiten, rapportinhoud, privébewijs of interne reviewtijdlijnen.
...
Beheerder-eindpunt voor het maken of bijwerken van een officiële pluginmigratierij.
...
Moderator-/beheerder-eindpunt voor reviewwachtrijen van pakketreleases.
...
gekoppeld aan een versie. Ze voeden de moderatiewachtrij, maar verbergen downloads niet automatisch en
blokkeren deze ook niet uit zichzelf; moderators moeten releasemoderatie gebruiken om
...
Moderator-/admin-eindpunt voor intake van pakketterapporten.
...
Downloadt een gehoste ZIP voor een Skills-versie, of retourneert een GitHub-bronoverdracht voor een
...
- Door GitHub ondersteunde skill-overdrachten proxyen of mirroren geen bytes. De JSON-respons
  ...
  en `archiveUrl`; scan-/huidige status is een poortwachter en wordt niet opgenomen als metadata in de
  succesvolle payload.
...
- Optioneel payloadveld: `ownerHandle`. Wanneer aanwezig, lost de API die
  publisher aan serverzijde op en vereist deze dat de actor publishertoegang heeft.
...
- Toegestane formuliervelden zijn `payload`, herhaalde `files`-blobs of één `clawpack`-
  ...
  de upload-url-flow is geretourneerd. Publicaties met gefaseerde storage-id moeten ook het
...
- JSON-bodies en door de aanroeper geleverde `payload.files`- / `payload.artifact`-
...
- Alleen de publisher van de `openclaw`-org en persoonlijke publishers van huidige leden van de `openclaw`-org
...
Door de eigenaar gestarte zachte verwijderingen reserveren de slug 30 dagen, waarna de slug kan worden geclaimd door
een andere publisher. De verwijderrespons bevat `slugReservedUntil` wanneer deze vervaldatum van toepassing is.
...
Het eindpunt staat standaard op dry-run. Herstel toepassen vereist `dryRun: false` en
...
uitgever van de bestemmingsgebruiker skills, pakketten of GitHub-skillbronnen heeft.
Herstel migreert ook verouderde `ownerUserId`-velden voor de skills,
skill-slugaliassen, pakketten, pakketinspectiewaarschuwingen en afgeleide zoekdigest-rijen van de herstelde uitgever, zodat
directe-eigenaarspaden overeenkomen met de nieuwe uitgeversautoriteit. Een actieve beschermde-handle-reservering
...
GitHub-skillbronnen zijn uitgeversgebonden en worden gerapporteerd als gecontroleerd in plaats van herschreven.
...
- `rename` bewaart de vorige slug als een doorverwijzingsalias.
...
Voeg een ster toe of verwijder een ster (markeringen). Beide eindpunten zijn idempotent.
