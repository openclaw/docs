...
precedentie opnieuw te reconstrueren.
...
- `X-RateLimit-Remaining` / `RateLimit-Remaining`: exact resterend budget wanneer aanwezig.
  Geslaagde sharded verzoeken laten deze header weg in plaats van een geschatte globale waarde te retourneren.
...
- Als er geen vertrouwd client-IP beschikbaar is, gebruiken anonieme verzoeken fallback-buckets
  die alleen zijn gescoped op het type snelheidslimiet. Deze fallback-buckets bevatten geen
...
moeten de responsbody lezen als een menselijk leesbare string. Onbekende queryparameters worden
...
- Resultaten worden geretourneerd in relevantievolgorde (embedding-overeenkomst + exacte token-boosts voor slug/naam + een kleine populariteitsprior).
- Relevantie is sterker dan populariteit. Een precieze slug- of weergavenaam-tokenmatch kan hoger scoren dan een lossere match met veel sterkere engagement.
...
- Populariteit is log-geschaald en afgetopt. Skills met hoge engagement kunnen lager scoren wanneer de querytekst een zwakkere match is.
...
- Zet de termen waar gebruikers letterlijk naar zullen zoeken in de weergavenaam, samenvatting en tags. Gebruik alleen een zelfstandige slug-token wanneer die ook een stabiele identiteit is die je wilt behouden.
...
- Hernoemaliassen behouden resolutie voor oude URL's en installs die via het register oplossen, maar zoekranking is gebaseerd op de canonieke Skill-metadata nadat de hernoeming is geïndexeerd. Bestaande statistieken blijven bij de Skill.
...
- Wanneer `nonSuspiciousOnly=true`, kunnen cursor-gebaseerde sorteringen minder dan `limit` items op een pagina retourneren omdat verdachte Skills na het ophalen van de pagina worden gefilterd.
...
- Oude slugs die door eigenaar-hernoemings-/mergeflows zijn gemaakt, lossen op naar de canonieke Skill.
...
- `moderation` wordt alleen opgenomen wanneer de Skill is gevlagd of de eigenaar deze bekijkt.
...
- Publieke aanroepers krijgen alleen `200` voor al gevlagde zichtbare Skills.
...
Moderator-/admin-eindpunt voor de intake van Skill-rapporten.
...
Retourneert verificatiedetails van beveiligingsscan voor een Skill-versie.
...
Lokale uploadscans worden niet langer ondersteund. Verzoeken die
...
- Gepubliceerde scans vereisen eigenaar-/uitgeversbeheertoegang, of platformmoderator-/adminbevoegdheid.
...
- Scantaken zijn asynchroon. Handmatige scanverzoeken krijgen prioriteit boven normaal publicatie-/aanvulwerk, maar voltooiing hangt nog steeds af van beschikbaarheid van workers.
...
- Vereist eigenaar-/uitgeversbeheertoegang tot de skill of plugin, of platformmoderator-/adminbevoegdheid.
...
Alleen voor admins beschikbare canonieke route voor batchgewijs opnieuw scannen. Deze accepteert dezelfde payloadvorm als de verouderde `POST /api/v1/skills/-/rescan-batch`.

### `POST /api/v1/skills/-/scan/batch/status`

Alleen voor admins beschikbare canonieke route voor batchstatus. Deze accepteert `{ "jobIds": ["..."] }` en retourneert dezelfde geaggregeerde tellers als de verouderde `POST /api/v1/skills/-/rescan-batch/status`.
...
- `tag` (optioneel): een getagde versie oplossen (bijvoorbeeld `latest`).
...
- `security.signals.dependencyRegistry` wordt behouden voor compatibiliteit met v1-responsen, maar de scanner voor bestaan in het afhankelijkheidsregister is uitgefaseerd en deze sleutel is altijd `null`.
...
ClawHub-skillversies zij moeten weergeven, zoals OpenClaw Control UI.
...
- De respons is alleen security. Deze bevat geen Skill Card-gegevens, gegenereerde kaartstatus, lijsten met artifactbestanden of gedetailleerde scannerpayloads.
...
- Geauthenticeerde aanroepers kunnen privé-pakketten zien voor publishers waartoe ze behoren in lijst-/zoekresultaten.
...
- Anonieme aanroepers zien alleen openbare pakketkanalen.
- Geauthenticeerde aanroepers kunnen privé-pakketten zoeken voor publishers waartoe ze behoren.
- `channel=private` retourneert alleen pakketten die de geauthenticeerde aanroeper kan lezen.

### `GET /api/v1/plugins`

Alleen-Plugin catalogusoverzicht voor code-plugin- en bundle-plugin-pakketten.
...
- De legacy v1-filteraliassen die zijn gedocumenteerd onder `GET /api/v1/plugins` worden ook
  geaccepteerd.
- Categoriefiltering is een echt API-filter dat wordt ondersteund door digest-rijen voor plugincategorieën,
  geen herschrijving van de zoekquery.
- Resultaten worden geretourneerd op volgorde van relevantie en hebben momenteel geen paginering.
...
- `packageName` is npm-naamgenormaliseerd; het pakket kan ontbreken voor geplande
  migraties.
...
Meld een pakket voor moderatorbeoordeling. Rapporten zijn op pakketniveau, optioneel
...
Moderator-/admin-eindpunt voor inname van pakkagerapporten.
...
Moderator-/admin-eindpunt voor het oplossen of heropenen van pakkagerapporten.
...
- Lopende VirusTotal-scans blokkeren lezen niet; kwaadaardige releases kunnen elders nog steeds worden achtergehouden.
...
- `slug` (vereist)
- `hash` (vereist): sha256 in hex van 64 tekens van de bundel-fingerprint
...
huidige GitHub-gebaseerde skill met een `clean`- of `suspicious`-scan en zonder gehoste
versie.
...
- GitHub-gebaseerde skilloverdrachten proxyën of mirroren geen bytes. Het JSON-antwoord
...
- Optioneel payload-veld: `ownerHandle`. Indien aanwezig, lost de API die
...
- Optioneel payload-veld: `migrateOwner`. Wanneer `true` met `ownerHandle`, mag een
  ...
  de huidige als de doel-publisher. Zonder deze opt-in worden eigenaarswijzigingen
  geweigerd.
...
- Optioneel payload-veld: `ownerHandle`. Indien aanwezig, mogen alleen admins namens die eigenaar publiceren.
...
- Publicaties namens iemand anders valideren geschiktheid voor het officiële kanaal nog steeds tegen het doel-eigenaaraccount.
...
Indien aanwezig wordt `reason` opgeslagen als moderatienotitie voor de skill en naar het auditlog gekopieerd.
...
provideraccount-id's noemen; veranderlijke handles worden alleen gebruikt als operatorgerichte beveiligingscontrole.

Het endpoint staat standaard op dry-run. Herstel toepassen vereist `dryRun: false` en
...
directe eigenaarsroutes overeenkomen met de nieuwe publisher-autoriteit. Een actieve protected-handle-reservering
...
### Endpoints voor beheer van eigenaars-slugs
...
content te herstellen (alleen admin). Staat standaard op dry-run, tenzij `dryRun` `false` is.
