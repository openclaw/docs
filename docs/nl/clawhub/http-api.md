...
x-i18n:
    generated_at: "2026-07-05T05:18:28Z"
    model: gpt-5.5
...
- Geauthenticeerde verzoeken (geldige Bearer-token): afgedwongen per gebruikersbucket.
...
- `X-RateLimit-Remaining` / `RateLimit-Remaining`: exact resterend budget indien aanwezig.
  Gesegmenteerde succesvolle verzoeken laten deze header weg in plaats van een geschatte globale waarde te retourneren.
...
- Als er geen vertrouwd client-IP beschikbaar is, gebruiken anonieme verzoeken fallback-buckets
  die alleen per soort snelheidslimiet zijn gescoped. Deze fallback-buckets bevatten geen
...
- Resultaten worden geretourneerd in relevantievolgorde (embeddingsimilariteit + boosts voor exacte slug-/naam-tokenovereenkomsten + een kleine populariteitsprior).
- Relevantie is sterker dan populariteit. Een precieze slug- of weergavenaam-tokenovereenkomst kan hoger scoren dan een lossere overeenkomst met veel sterkere betrokkenheid.
- ASCII-tekst wordt getokeniseerd op woord- en interpunctiegrenzen. Bijvoorbeeld: `personal-map` bevat een zelfstandig `map`-token, terwijl `amap-jsapi-skill` `amap`, `jsapi` en `skill` bevat; zoeken naar `map` geeft `personal-map` daarom een sterkere lexicale overeenkomst dan `amap-jsapi-skill`.
- Populariteit is loggeschaald en afgetopt. Skills met hoge betrokkenheid kunnen lager ranken wanneer de querytekst een zwakkere overeenkomst is.
...
- Hernoem een slug niet alleen om één query na te jagen, tenzij de nieuwe slug een betere canonieke naam voor de lange termijn is. Oude slugs worden redirect-aliassen, maar de canonieke URL, weergegeven slug en toekomstige zoekoverzichten gebruiken de nieuwe slug.
- Hernoemingsaliassen behouden oplossing voor oude URL's en installaties die via de registry oplossen, maar zoekranking is gebaseerd op de canonieke skillmetadata nadat de hernoeming is geïndexeerd. Bestaande statistieken blijven bij de skill.
...
- Oude slugs die zijn aangemaakt door eigenaar-hernoemings-/mergeflows lossen op naar de canonieke skill.
...
Alleen voor admins beschikbare canonieke batchroute voor opnieuw scannen. Deze accepteert dezelfde payloadvorm als de verouderde `POST /api/v1/skills/-/rescan-batch`.

### `POST /api/v1/skills/-/scan/batch/status`

Alleen voor admins beschikbare canonieke batchstatusroute. Deze accepteert `{ "jobIds": ["..."] }` en retourneert dezelfde geaggregeerde tellers als de verouderde `POST /api/v1/skills/-/rescan-batch/status`.
...
- Skill-identiteit, uitgeversidentiteit en metadata van de geselecteerde versie zijn velden op het hoogste niveau van de envelop (`slug`, `displayName`, `publisherHandle`, `version`, `resolvedFrom`, `tag`, `createdAt`), zodat shellautomatisering ze kan lezen zonder geneste wrappers uit te pakken.
...
- `security.signals.dependencyRegistry` blijft behouden voor compatibiliteit met v1-responses, maar de scanner voor het bestaan van het afhankelijkheidsregister is beëindigd en deze sleutel is altijd `null`.
...
- Resultaten zijn per item; één ontbrekende skill of versie laat niet de hele response mislukken.
- De response bevat alleen beveiligingsgegevens. Deze bevat geen Skill Card-gegevens, gegenereerde kaartstatus, lijsten met artefactbestanden of gedetailleerde scannerpayloads.
...
- Gebruik `/verify` wanneer u de verificatie-envelop van de Skill Card voor één skill nodig hebt, `/card` wanneer u gegenereerde kaartmarkdown nodig hebt, en `/scan` wanneer u gedetailleerde scannergegevens nodig hebt.
...
- Skill-vermeldingen blijven gebaseerd op het Skill-register en kunnen nog steeds alleen worden gepubliceerd via `POST /api/v1/skills`.
...
- Anonieme aanroepers zien alleen openbare pakketkanalen.
- Geverifieerde aanroepers kunnen zoeken in privépakketten voor uitgevers waartoe ze behoren.
- `channel=private` retourneert alleen pakketten die de geverifieerde aanroeper kan lezen.

### `GET /api/v1/plugins`

Alleen-Plugin catalogus doorbladeren over code-Plugin- en bundel-Plugin-pakketten.
...
`trending` is een zeven-daags klassement voor installaties/downloads en gebruikt geen totalen over de gehele looptijd.
...
- `cursor` (optioneel): pagineringcursor uit het vorige antwoord.
...
- `limit` (optioneel): geheel getal (1-250), standaard `250`.
- `cursor` (optioneel): paginatiecursor uit het vorige antwoord.
...
- De verouderde v1-filteraliassen die zijn gedocumenteerd onder `GET /api/v1/plugins` worden ook
  geaccepteerd.
- Categoriefiltering is een echt API-filter dat wordt ondersteund door digest-rijen voor Plugin-categorieen,
  geen herschrijving van de zoekquery.
- Resultaten worden op relevantievolgorde geretourneerd en hebben momenteel geen paginatie.
...
- Privepakketten retourneren `404`, tenzij de aanroeper de eigenaarspublisher kan lezen.
...
- `trust.reasons` is de lijst met verklaringen voor gebruikers en audits. Redencodes
...
- `trust.stale` betekent dat de vertrouwenssamenvatting is berekend uit verouderde invoer en
  moet worden behandeld als iets dat moet worden vernieuwd voordat een beslissing met hoge zekerheid kan worden toegestaan.
...
- Dit eindpunt is bewust smaller dan moderatie-eindpunten voor eigenaren/moderators.
  ...
  identiteiten van melders, rapportteksten, privebewijs of interne review-
  tijdlijnen.
...
Moderator-eindpunt voor het weergeven van officiele OpenClaw Plugin-migratierijen.
...
Beheerderseindpunt voor het maken of bijwerken van een officiele Plugin-migratierij.
...
Meld een pakket voor moderatorreview. Rapporten zijn op pakketniveau en optioneel
...
huidige GitHub-backed skill met een `clean`- of `suspicious`-scan en zonder gehoste
versie.
...
- GitHub-backed skill-overdrachten proxyen of mirroren geen bytes. De JSON-respons
  ...
  en `archiveUrl`; scan-/huidige status is een poortwachter en wordt niet opgenomen als metadata
  van de succesvolle payload.
...
- Toegestane formuliervelden zijn `payload`, herhaalde `files`-blobs of één `clawpack`
  tarball-verwijzing. `clawpack` mag een `.tgz`-blob zijn of een opslag-id die is geretourneerd door
...
- Directe multipart-publicatieverzoeken zijn beperkt tot 18MB. ClawPack-tarballs mogen
  de upload-url-flow gebruiken tot de tarball-limiet van 120MB.
...
- Code-plugins vereisen `package.json`, bronrepo-metadata, broncommit-
  metadata, metadata voor config-schema, `openclaw.compat.pluginApi` en
...
- Alleen de `openclaw` org-uitgever en persoonlijke uitgevers van huidige `openclaw` org-leden
  mogen publiceren naar het `official`-kanaal.
- Publicaties namens iemand anders valideren nog steeds geschiktheid voor het officiële kanaal tegen het doeleigenaarsaccount.
...
Alleen admin. Zorgt dat er een org-uitgever bestaat voor een handle. Als de handle nog steeds verwijst naar een
legacy gedeelde gebruiker/persoonlijke uitgever, migreert het eindpunt deze eerst naar een org-uitgever.
...
Geauthenticeerde selfservice-aanmaak van org-uitgever. Maakt een nieuwe org-uitgever aan en voegt de
...
release te publiceren. Pakketnamen worden privé placeholder-pakketten zonder releaserijen, zodat dezelfde
...
GitHub-identiteiten hebben geverifieerd. Herstel weigert standaard wanneer de huidige persoonlijke
...
paden met directe eigenaar overeenkomen met de nieuwe publisher-bevoegdheid. Een actieve reservering voor een beschermde handle
voor de herstelde handle wordt ook opnieuw toegewezen aan de vervangende gebruiker, zodat latere
profielsynchronisatie de concurrerende bevoegdheid van de vorige gebruiker niet kan herstellen. Elke primaire tabel is begrensd op
100 rijen per toepastransactie; grotere herstelacties moeten eerst een hervatbare eigenaarsmigratie gebruiken.
GitHub-skillbronnen zijn publisher-gebonden en worden gerapporteerd als gecontroleerd in plaats van herschreven.
...
### Eindpunten voor eigenaarsslugbeheer
...
Ban een gebruiker en verwijder beheerde skills definitief (alleen moderator/admin).
...
die een ClawPack-tarball voorbereiden, moeten de resulterende opslag-id verzenden als
