...
summary: HTTP API-naslag (openbare + CLI-eindpunten + authenticatie).
x-i18n:
    generated_at: "2026-07-03T02:51:03Z"
    model: gpt-5.5
...
- Geverifieerde verzoeken (geldige Bearer-token): afgedwongen per gebruikersbucket.
...
- Geverifieerde schrijf-eindpunten mogen geen kale `Unauthorized` teruggeven wanneer
  ...
  verwijderde/gebande/gedeactiveerde accounts moeten elk bruikbare tekst krijgen zodat CLI
...
- `X-RateLimit-Remaining` / `RateLimit-Remaining`: exact resterend budget indien aanwezig.
  Geshaarde succesvolle verzoeken laten deze header weg in plaats van een geschatte globale waarde terug te geven.
...
- Als er geen vertrouwd client-IP beschikbaar is, gebruiken anonieme verzoeken fallback-buckets
  die alleen zijn gescoord op rate-limit-soort. Deze fallback-buckets bevatten geen
...
moeten de responsbody lezen als een door mensen leesbare string. Onbekende queryparameters worden
...
- Resultaten worden in relevantievolgorde teruggegeven (embedding-gelijkenis + exacte slug-/naamtoken-boosts + een kleine populariteitsprior).
- Relevantie is sterker dan populariteit. Een precieze slug- of weergavenaam-tokenmatch kan hoger ranken dan een lossere match met veel sterkere betrokkenheid.
- ASCII-tekst wordt getokenized op woord- en interpunctiegrenzen. Bijvoorbeeld: `personal-map` bevat een losstaand `map`-token, terwijl `amap-jsapi-skill` `amap`, `jsapi` en `skill` bevat; zoeken naar `map` geeft `personal-map` daarom een sterkere lexicale match dan `amap-jsapi-skill`.
...
- Zet de termen waarop gebruikers letterlijk zullen zoeken in de weergavenaam, samenvatting en tags. Gebruik alleen een losstaand slug-token wanneer het ook een stabiele identiteit is die je wilt behouden.
...
- Wanneer `nonSuspiciousOnly=true`, kunnen cursorgebaseerde sorteringen minder dan `limit` items op een pagina retourneren omdat verdachte skills na pagina-ophalen worden gefilterd.
...
- Oude slugs die zijn gemaakt door eigenaar-hernoemings-/samenvoegflows worden opgelost naar de canonieke skill.
...
`status` terug wordt gezet naar `open`. Geef `finalAction: "hide"` mee met een getriageerd
...
- Gepubliceerde scans vereisen beheerstoegang als eigenaar/uitgever, of platformmoderator-/beheerdersbevoegdheid.
- Gepubliceerde scans schrijven alleen terug wanneer `update: true` en de scan succesvol is voltooid.
- Respons is `202` met `{ "ok": true, "scanId": "...", "jobId": "...", "status": "queued", "sourceKind": "published", "update": false, "queue": { "queuedAhead": 0, "queuedAheadIsEstimate": false, "position": 1, "running": 0, "runningIsEstimate": false, "note": "Scans zijn asynchroon en kunnen tijd kosten om te voltooien." } }`.
...
Geauthenticeerd endpoint voor rapportarchief.
...
Geauthenticeerd endpoint voor opgeslagen rapportarchief voor ingediende versies.

- Vereist beheerstoegang als eigenaar/uitgever tot de skill of plugin, of platformmoderator-/beheerdersbevoegdheid.
...
- Skillidentiteit, uitgeveridentiteit en metadata van de geselecteerde versie zijn velden op het hoogste niveau van de envelop (`slug`, `displayName`, `publisherHandle`, `version`, `resolvedFrom`, `tag`, `createdAt`), zodat shellautomatisering ze kan lezen zonder geneste wrappers uit te pakken.
- `security` is het ClawScan-/security-oordeel op het hoogste niveau. Automatisering moet zich baseren op `ok`, `decision`, `reasons` en `security.status`.
- `security.signals` bevat ondersteunend scannerbewijs, zoals `staticScan`, `virusTotal` en `skillSpector`.
- `security.signals.dependencyRegistry` blijft behouden voor compatibiliteit met v1-responsen, maar de scanner voor het bestaan van afhankelijkhedenregisters is buiten gebruik gesteld en deze sleutel is altijd `null`.
...
Retourneert huidige compacte security-oordelen voor exacte skillversies. Dit
collectieendpoint is bedoeld voor clients die al weten welke geïnstalleerde
...
- De respons bevat alleen securitygegevens. Deze bevat geen Skill Card-gegevens, status van gegenereerde kaarten, lijsten met artefactbestanden of gedetailleerde scannerpayloads.
...
- `category` (optioneel): Plugin-categoriefilter. Alleen ondersteund wanneer het
...
- Anonieme aanroepers zien alleen openbare pakketkanalen.
- Geauthenticeerde aanroepers kunnen privépakketten doorzoeken voor uitgevers waartoe ze behoren.
- `channel=private` retourneert alleen pakketten die de geauthenticeerde aanroeper kan lezen.

### `GET /api/v1/plugins`

Catalogusverkenning alleen voor Plugins in code-plugin- en bundle-plugin-pakketten.
...
- `mcp-tooling`, `data` en `automation` worden herleid naar `tools`.
- `observability` en `deployment` worden herleid naar `gateway`.
- `dev-tools` wordt herleid naar `runtime`.

`trending` is een ranglijst voor installaties/downloads over zeven dagen en gebruikt geen totalen aller tijden.
...
Verouderde aliassen worden niet geaccepteerd als opgeslagen of door de auteur gedeclareerde categoriewaarden.
...
- De legacy v1-filteraliassen die zijn gedocumenteerd onder `GET /api/v1/plugins` worden ook
  geaccepteerd.
- Categoriefiltering is een echte API-filter die wordt ondersteund door plugin-category-digest
  rijen, geen herschrijving van de zoekquery.
- Resultaten worden geretourneerd op relevantievolgorde en worden momenteel niet gepagineerd.
- Sorteerbedieningselementen in de browser-UI voor pluginzoekopdrachten herschikken de geladen relevantieresultaten,
...
- Vereist een API-token voor de pakketeigenaar, een eigenaar/beheerder van een organisatiepublisher,
...
- Openbaar leeseindpunt. Er is geen owner-, publisher-, moderator- of admintoken
  vereist.
...
- Dit eindpunt is opzettelijk smaller dan moderatie-eindpunten voor eigenaars/moderators.
  ...
  reporter-identiteiten, report-bodies, privébewijs of interne reviewtijdlijnen.
...
- Dit is het OpenClaw-resolveroppervlak; het voorkomt raden naar het archiefformaat vanuit
  een gedeelde URL.
...
Beheerderseindpunt voor het maken of bijwerken van een officiële pluginmigratierij.
...
Moderator-/beheerders-eindpunt voor reviewqueues van pakketreleases.
...
- `open`: verdachte, kwaadaardige, pending, quarantined, ingetrokken of gerapporteerde releases.
- `blocked`: quarantined, ingetrokken of kwaadaardige releases.
...
Rapporteer een pakket voor moderatorreview. Rapporten zijn op pakketniveau en optioneel
...
Moderator-/admin-eindpunt voor inname van pakketrappporten.
...
Moderator-/admin-eindpunt voor het oplossen of heropenen van pakketrappporten.
...
- In behandeling zijnde VirusTotal-scans blokkeren leesacties niet; kwaadaardige releases kunnen elders nog steeds worden achtergehouden.
...
- Legacy-versies die alleen ZIP ondersteunen, worden bewust weggelaten.
...
- Gebruikt de bucket voor downloadsnelheid.
- Downloadheaders bevatten ClawHub SHA-256 plus npm-integrity-/shasum-metadata.
- Moderatie- en toegangscontroles voor private packages blijven van toepassing.
...
- `slug` (verplicht)
- `hash` (verplicht): 64-teken hex sha256 van de bundle-fingerprint
...
Downloadt een gehoste skill-versie-ZIP, of retourneert een GitHub-bronoverdracht voor een
...
- Optioneel payloadveld: `migrateOwner`. Wanneer `true` met `ownerHandle`, kan een
  bestaande skill naar die owner worden verplaatst als de actor admin/owner is bij zowel
  de huidige als de doel-publishers. Zonder deze opt-in worden ownerwijzigingen
  geweigerd.
...
- Toegestane formuliervelden zijn `payload`, herhaalde `files` blobs, of één `clawpack`
  tarball-referentie. `clawpack` kan een `.tgz` blob zijn of een opslag-id dat is geretourneerd door
  de upload-url-flow. Publicaties met staged opslag-id moeten ook het
...
- JSON-bodies en door de caller aangeleverde `payload.files` / `payload.artifact`
...
- Optioneel payloadveld: `ownerHandle`. Indien aanwezig, mogen alleen admins namens die owner publiceren.

Validatiehoogtepunten:

- `family` moet `code-plugin` of `bundle-plugin` zijn.
- Pluginpackages vereisen `openclaw.plugin.json`. ClawPack `.tgz`-uploads moeten
...
- Alleen de `openclaw` org-publisher en persoonlijke publishers van huidige leden van de `openclaw` org
  mogen naar het `official`-kanaal publiceren.
- Publicaties namens iemand anders valideren nog steeds of de doel-owneraccount in aanmerking komt voor het official-kanaal.

### `DELETE /api/v1/skills/{slug}` / `POST /api/v1/skills/{slug}/undelete`

Soft-delete / herstel een skill (owner, moderator of admin).
...
Indien aanwezig, wordt `reason` opgeslagen als moderatienotitie voor de skill en gekopieerd naar de auditlog.
Door de owner geïnitieerde soft deletes reserveren de slug 30 dagen; daarna kan de slug worden geclaimd door
een andere publisher. De delete-respons bevat `slugReservedUntil` wanneer deze vervaldatum van toepassing is.
Moderator-/admin-verbergacties en securityverwijderingen verlopen niet op deze manier.
...
Alleen voor admins. Zorgt dat er een org-publisher bestaat voor een handle. Als de handle nog naar een
...
caller toe als owner. Dit eindpunt migreert bestaande gebruikers-/persoonlijke handles niet en markeert
...
Alleen voor admins. Reserveert root-slugs en packagenamen voor een rechtmatige owner zonder een
release te publiceren. Packagenamen worden private placeholder-packages zonder release-rijen, zodat dezelfde
owner later de echte code-plugin- of bundle-plugin-release onder die naam kan publiceren.
...
Alleen voor admins. Herstelt een persoonlijke publisher voor een geverifieerde vervangende GitHub OAuth-principal
...
Skill-slugaliassen, pakketten, waarschuwingen van pakketinspectors en afgeleide zoekdigest-rijen, zodat
directe-eigenaarpaden overeenkomen met de nieuwe publisher-autoriteit. Een actieve reservering voor een beschermde handle
...
GitHub-Skill-bronnen zijn publisher-scoped en worden gerapporteerd als gecontroleerd in plaats van herschreven.
...
Ban een gebruiker en verwijder Skills in eigendom permanent (alleen moderator/admin).
...
Maak een ban van een gebruiker ongedaan en herstel in aanmerking komende Skills (alleen admin).
...
inhoud te herstellen (alleen admin). Standaard een dry-run tenzij `dryRun` `false` is.
...
- `limit` (optioneel): maximale resultaten (standaard 20, max. 200)
...
## Verouderde CLI-endpoints (afgekeurd)
...
publicaties die een ClawPack-tarball stagen, moeten de resulterende opslag-id verzenden als
