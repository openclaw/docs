...
Externe directory's mogen de openbare leesendpoints gebruiken om ClawHub-Skills weer te geven of te doorzoeken. Cache resultaten, respecteer `429`/`Retry-After`, link gebruikers terug naar de canonieke ClawHub-vermelding (`https://clawhub.ai/<owner>/skills/<slug>`) en vermijd de indruk te wekken dat ClawHub de externe site onderschrijft. Probeer geen verborgen, privé- of door moderatie geblokkeerde content buiten het openbare API-oppervlak te spiegelen.
...
- Geauthenticeerde schrijfendpoints mogen geen kale `Unauthorized` retourneren wanneer
  de server de reden kent. Ontbrekende tokens, ongeldige/ingetrokken tokens en
  verwijderde/verbannen/uitgeschakelde accounts moeten elk uitvoerbare tekst krijgen zodat CLI-
...
- `X-RateLimit-Remaining` / `RateLimit-Remaining`: exact resterend budget indien aanwezig.
  Geslaagde gesharde aanvragen laten deze header weg in plaats van een globale benaderingswaarde te retourneren.
...
- Gebruik jittered backoff om gesynchroniseerde herhalingen te voorkomen.
...
- Resultaten worden geretourneerd in relevantievolgorde (embedding-overeenkomst + exacte slug-/naam-tokenboosts + een kleine populariteitsprior).
- Relevantie is sterker dan populariteit. Een precieze slug- of weergavenaam-tokenmatch kan hoger scoren dan een lossere match met veel sterkere betrokkenheid.
...
- Populariteit wordt log-geschaald en begrensd. Skills met hoge betrokkenheid kunnen lager scoren wanneer de querytekst een zwakkere match is.
...
- Hernoem een slug niet alleen om één query na te jagen, tenzij de nieuwe slug een betere canonieke naam voor de lange termijn is. Oude slugs worden redirect-aliassen, maar de canonieke URL, weergegeven slug en toekomstige search digests gebruiken de nieuwe slug.
- Hernoemingsaliassen behouden oplossing voor oude URL's en installaties die via het register worden opgelost, maar zoekrangschikking is gebaseerd op de canonieke Skill-metadata nadat de hernoeming is geïndexeerd. Bestaande statistieken blijven bij de Skill.
...
- Oude slugs die door owner-hernoemings-/merge-flows zijn gemaakt, worden opgelost naar de canonieke Skill.
...
- Openbare callers krijgen alleen `200` voor reeds geflagde zichtbare Skills.
...
- Gepubliceerde scans vereisen beheerrechten als eigenaar/uitgever, of platformmoderator-/adminbevoegdheid.
...
- Vereist beheerrechten als eigenaar/uitgever voor de Skill of Plugin, of platformmoderator-/adminbevoegdheid.
...
Canonieke batchroute voor opnieuw scannen, alleen voor admins. Deze accepteert dezelfde payloadvorm als de verouderde `POST /api/v1/skills/-/rescan-batch`.
...
Retourneert de Skill Card-verificatie-envelope die door `clawhub skill verify` wordt gebruikt.
...
- Skill-identiteit, uitgeversidentiteit en metadata van de geselecteerde versie zijn toplevel envelope-velden (`slug`, `displayName`, `publisherHandle`, `version`, `resolvedFrom`, `tag`, `createdAt`), zodat shell-automatisering ze kan lezen zonder geneste wrappers uit te pakken.
- `security` is het toplevel ClawScan-/security-oordeel. Automatisering moet sturen op `ok`, `decision`, `reasons` en `security.status`.
- `security.signals` bevat ondersteunend scannerbewijs zoals `staticScan`, `virusTotal` en `skillSpector`.
- `security.signals.dependencyRegistry` blijft behouden voor compatibiliteit met v1-antwoorden, maar de scanner voor het bestaan van het dependency registry is buiten gebruik gesteld en deze sleutel is altijd `null`.
- `provenance` is alleen `server-resolved-github-import` wanneer ClawHub tijdens publicatie of import een GitHub-repo/ref/commit/path heeft opgelost en opgeslagen; anders is deze `unavailable`.

### `POST /api/v1/skills/-/security-verdicts`

Retourneert huidige compacte security-oordelen voor exacte Skill-versies. Dit
...
Catalogusoverzicht alleen voor plugins, over code-plugin- en bundle-plugin-pakketten.
...
- `mcp-tooling`, `data` en `automation` verwijzen naar `tools`.
- `observability` en `deployment` verwijzen naar `gateway`.
- `dev-tools` verwijst naar `runtime`.
...
- Sorteerknoppen in de browser-UI voor Plugin-zoekopdrachten herschikken de geladen relevanteresultaten,
...
- `trust.pending` betekent dat één of meer vertrouwensinvoeren nog op voltooiing wachten.
- `trust.stale` betekent dat de vertrouwenssamenvatting is berekend op basis van verouderde invoer en
  moet worden behandeld als iets dat moet worden vernieuwd vóór een toestemmingsbeslissing met hoge betrouwbaarheid.
...
- Dit is het OpenClaw-resolveroppervlak; het voorkomt dat het archiefformaat uit
  een gedeelde URL moet worden gegokt.
...
- ClawPack-versies streamen de exact geüploade npm-pack `.tgz`-bytes.
...
- `packageName` wordt npm-naamgenormaliseerd; het pakket kan ontbreken voor geplande
  migraties.
...
Moderator-/beheerderseindpunt voor de inname van pakketrapporten.
...
Moderator-/beheerderseindpunt voor het oplossen of heropenen van pakketrapporten.
...
Moderator-/beheerderseindpunt voor pakketreleasebeoordeling.
...
In quarantaine geplaatste en ingetrokken releases retourneren `403` vanuit downloadroutes voor artefacten.
...
- Gebruikt de leesratelimietbucket, niet de downloadbucket.
...
- Deze route blijft uitsluitend ZIP. Ze streamt geen ClawPack `.tgz`-bestanden.
...
Downloadt een gehoste ZIP-versie van een skill, of retourneert een GitHub-bronhandoff voor een
...
- Door GitHub ondersteunde skill-handoffs proxyen of mirroren geen bytes. De JSON-respons
  bevat `sourceRef: "public-github"`, `repo`, `commit`, `path`, `contentHash`
  en `archiveUrl`; de scan-/huidige status is een gate en wordt niet opgenomen als metadata van de succespayload.
...
- Toegestane formuliervelden zijn `payload`, herhaalde `files`-blobs of één `clawpack`
  ...
  de upload-url-flow. Gepubliceerde staged storage-id's moeten ook het
...
- Publicaties namens iemand anders valideren nog steeds de geschiktheid voor het officiële kanaal tegen het doel-eigenaaraccount.
...
Verbergacties door moderator/beheerder en beveiligingsverwijderingen verlopen niet op deze manier.
...
Alleen beheerder. Zorgt dat er een org-publisher bestaat voor een handle. Als de handle nog naar een
legacy gedeelde gebruiker/persoonlijke publisher wijst, migreert het endpoint deze eerst naar een org-publisher.
Geef voor een nieuw aangemaakte org `memberHandle` op; de handelende beheerder wordt niet als lid toegevoegd.
...
Alleen beheerder. Reserveert rootslugs en pakketnamen voor een rechtmatige eigenaar zonder een
...
Alleen beheerder. Herstelt een persoonlijke publisher voor een geverifieerde vervangende GitHub OAuth-principal
zonder Convex Auth-accountrecords te bewerken. Het verzoek moet beide onveranderlijke GitHub
provideraccount-id's noemen; wijzigbare handles worden alleen gebruikt als operatorgerichte beveiliging.

De endpoint gebruikt standaard een proefuitvoering. Herstel toepassen vereist `dryRun: false` en
...
paden met directe eigenaar overeenkomen met de nieuwe uitgeversautoriteit. Een actieve reservering
voor een beschermde handle voor de herstelde handle wordt ook opnieuw toegewezen aan de vervangende
gebruiker, zodat latere profielsynchronisatie de concurrerende autoriteit van de vorige gebruiker
niet kan herstellen. Elke primaire tabel is begrensd op 100 rijen per toepastransactie; grotere
herstelacties moeten eerst een hervatbare eigenaarsmigratie gebruiken. GitHub-Skill-bronnen zijn
uitgeversgebonden en worden gerapporteerd als gecontroleerd in plaats van herschreven.
...
- Beide endpoints vereisen API-tokenauthenticatie en werken alleen voor de Skill-eigenaar.
...
Ban een gebruiker en verwijder de Skills waarvan die eigenaar is definitief (alleen moderator/beheerder).
...
inhoud te herstellen (alleen beheerder). Gebruikt standaard een proefuitvoering, tenzij `dryRun` `false` is.
...
die een ClawPack-tarball klaarzetten, moeten de resulterende opslag-id als
`clawpack` verzenden en het geretourneerde ticket als `clawpackUploadTicket`.

## Registry-discovery (`/.well-known/clawhub.json`)

De CLI kan registry-/authenticatie-instellingen via de site ontdekken:
