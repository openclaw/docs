...
summary: Problemen oplossen met ClawHub-aanmelding, installatie, publiceren, synchronisatie, updates en API-problemen.
x-i18n:
...
De CLI start tijdens het inloggen via de browser een kortlevende lokale callbackserver.

- Zorg dat uw browser `http://127.0.0.1:<port>/callback` kan bereiken.
- Controleer lokale firewall-, VPN- en proxyregels als de callback nooit aankomt.
- Maak in headless omgevingen een API-token in de ClawHub-web-UI en voer uit:
...
- Als u een aangepast configuratiepad gebruikt, controleer dan of `CLAWHUB_CONFIG_PATH` verwijst naar het
  bestand dat uw huidige token bevat.
- Als u een API-token gebruikt, controleer dan of het niet is ingetrokken in de web-UI.

## Zoeken of installeren retourneert `Rate limit exceeded` (429)

Lees de retry-informatie in de respons:

- `Retry-After`: seconden om te wachten voordat u het opnieuw probeert.
- `RateLimit-Remaining` en `RateLimit-Limit`: uw huidige budget.
...
persoon slechts een paar verzoeken verstuurt. Meld u waar mogelijk aan en probeer het opnieuw na de
...
- Controleer of de release openbaar is en niet wordt vastgehouden door een scan of moderatie.
...
ClawHub gebruikt GitHub-identiteit en bronattributie om pakketten aan hun
uitgevers te koppelen.

- Zorg dat u bent aangemeld met het GitHub-account dat eigenaar is van het pakket of het kan publiceren.
...
Wijs het naar de roots die u wilt scannen:

```bash
clawhub sync --root /path/to/skills
```

Bekijk eerst een voorbeeld als u niet zeker weet wat wordt gepubliceerd:
...
- Publiceer uw bewerkte kopie als een nieuwe slug of fork.
...
- Controleer of uw OpenClaw-versie voldoet aan het geadverteerde
