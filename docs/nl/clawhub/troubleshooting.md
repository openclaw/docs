...
summary: Probleemoplossing voor aanmelding, installatie, publicatie, updates en API-problemen in ClawHub.
x-i18n:
...
De CLI start tijdens browser-login een kortlevende lokale callbackserver.
...
- `Retry-After`: seconden die je moet wachten voordat je het opnieuw probeert.
...
Als veel gebruikers één egress-IP delen, kunnen anonieme IP-limieten worden geraakt, zelfs wanneer elke
...
- Als jij de skill beheert, meld je dan aan en inspecteer deze:

```bash
clawhub inspect @openclaw/demo
```

Diagnostiek die zichtbaar is voor eigenaren kan de scan-, upload-gate- of moderatiestatus verklaren.
...
Bekijk eerst een preview van de publicatie-payload:
...
Als publicatie mislukt omdat de eigenaar-handle, org-namespace, pakketscope, skill-
...
Als je denkt dat jouw organisatie, project of merk de rechtmatige namespace-eigenaar is maar
je de huidige ClawHub-eigenaar niet kunt beheren, open dan een
[issue voor org-/namespaceclaim](https://github.com/openclaw/clawhub/issues/new?template=org-namespace-claim.yml)
met openbaar, niet-gevoelig bewijs. Zie
[Org- en namespaceclaims](/nl/clawhub/namespace-claims) voor richtlijnen over bewijs en wat je
...
- Respecteer retry-headers van `429` en cache openbare lijst-/zoekantwoorden.
