...
Gebruik eerst `--dry-run` om een voorbeeld te bekijken van de opgeloste pakketmetadata, compatibiliteitsvelden, brontoeschrijving en het uploadplan zonder te publiceren.

Codeplugins moeten OpenClaw-compatibiliteitsmetadata opnemen in `package.json`,
waaronder `openclaw.compat.pluginApi` en `openclaw.build.openclawVersion`.

## Skills synchroniseren die je onderhoudt

`sync` scant skillmappen en publiceert nieuwe of gewijzigde skills die nog niet
...
Gebruik voor installatie de ClawHub-webpagina of CLI-detailopdrachten om
...
kunnen verborgen zijn voor zoek- en installatieoppervlakken totdat dit is opgelost.
