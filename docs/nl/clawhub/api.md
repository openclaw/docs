...
x-i18n:
    generated_at: "2026-07-03T23:36:48Z"
    model: gpt-5.5
...
Je kunt een externe catalogus, directory of zoekoppervlak bouwen boven op de publieke lees-API's van ClawHub. Publieke Skills-metadata en Skills-bestanden worden gepubliceerd onder de Skills-licentieregels van ClawHub, terwijl de API zelf snelheidsbeperkt is en verantwoord moet worden gebruikt.

Richtlijnen:

- Gebruik publieke lees-eindpunten zoals `GET /api/v1/skills`, `GET /api/v1/search` en `GET /api/v1/skills/{slug}` voor catalogusvermeldingen.
- Cache antwoorden en respecteer `429`, `Retry-After` en rate-limit-headers in plaats van agressief te pollen.
- Link terug naar de canonieke ClawHub-Skills-URL wanneer je vermeldingen toont, zodat gebruikers de bronregistratierecord kunnen inspecteren.
...
- Spiegel geen verborgen, privé- of door moderatie geblokkeerde inhoud door publieke API-filters of authenticatiegrenzen te omzeilen.
...
- `X-RateLimit-Remaining` / `RateLimit-Remaining`: exact resterend budget wanneer
  aanwezig; op shards verdeelde succesvolle verzoeken laten dit weg in plaats van een benaderde
...
- v1-fouten zijn platte tekst (`text/plain; charset=utf-8`), inclusief `400`,
  `401`, `403`, `404`, `429` en geblokkeerde-downloadantwoorden.
...
- `GET /api/v1/skills?limit=&cursor=&sort=`
  - `sort`: `updated` (standaard), `recommended` (`default`), `createdAt` (`newest`), `downloads`, `stars` (`rating`), verouderde installatie-aliassen `installsCurrent`/`installs`/`installsAllTime` worden toegewezen aan `downloads`, `trending`
...
- `GET /api/v1/download?slug=&version=&tag=`
  ...
  - Huidige door GitHub ondersteunde Skills met een `clean`- of `suspicious`-scan geven een
    JSON-`public-github`-overdrachtsdescriptor terug in plaats van ClawHub-bytes.
...
- `POST /api/v1/users/reserve` reserveert root-slugs en privépakketplaatshouders zonder release voor een eigenaarshandle.
