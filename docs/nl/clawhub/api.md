...
x-i18n:
    generated_at: "2026-07-01T18:15:59Z"
    model: gpt-5.5
...
Je kunt een externe catalogus, directory of zoekinterface bouwen boven op de openbare lees-API's van ClawHub. Openbare Skills-metadata en Skills-bestanden worden gepubliceerd onder de licentieregels voor Skills van ClawHub, terwijl de API zelf frequentiebeperkt is en verantwoord moet worden gebruikt.
...
- Link terug naar de canonieke ClawHub Skills-URL wanneer je vermeldingen weergeeft, zodat gebruikers het bronrecord in de registry kunnen inspecteren.
...
- `POST /api/v1/users/reserve` reserveert root-slugs en privéplaatsaanduidingen voor pakketten zonder release voor een owner-handle.

## Legacy

Legacy `/api/*` en `/api/cli/*` zijn nog steeds beschikbaar. Zie `DEPRECATIONS.md`.
