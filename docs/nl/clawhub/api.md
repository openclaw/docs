...
x-i18n:
    generated_at: "2026-07-03T02:49:47Z"
    model: gpt-5.5
...
Je kunt een catalogus, directory of zoekoppervlak van derden bouwen boven op de openbare lees-API's van ClawHub. Openbare metagegevens van skills en skill-bestanden worden gepubliceerd onder de skill-licentieregels van ClawHub, terwijl de API zelf rate-limited is en verantwoord moet worden gebruikt.
...
- Link terug naar de canonieke ClawHub-skill-URL wanneer je vermeldingen weergeeft, zodat gebruikers het bronrecord in het register kunnen inspecteren.
...
`X-RateLimit-Remaining`, `RateLimit-Remaining` en `Retry-After` worden meegeleverd bij `429`.
...
- `X-RateLimit-Remaining` / `RateLimit-Remaining`: exact resterend budget wanneer
  aanwezig; gesharde succesvolle requests laten dit weg in plaats van een benaderde
