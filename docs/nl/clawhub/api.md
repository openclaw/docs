...
x-i18n:
    generated_at: "2026-07-02T01:02:05Z"
    model: gpt-5.5
...
Je kunt een catalogus, directory of zoekoppervlak van derden bouwen bovenop de openbare lees-API's van ClawHub. Openbare metadata van Skills en skillbestanden worden gepubliceerd onder ClawHub's licentieregels voor Skills, terwijl de API zelf rate-limited is en verantwoord moet worden gebruikt.
...
- Link terug naar de canonieke ClawHub-skill-URL bij het weergeven van vermeldingen, zodat gebruikers het bronrecord in het register kunnen inspecteren.
...
- `GET /api/v1/skills?limit=&cursor=&sort=`
  - `sort`: `updated` (standaard), `recommended` (`default`), `createdAt` (`newest`), `downloads`, `stars` (`rating`), legacy-installatiealiassen `installsCurrent`/`installs`/`installsAllTime` worden toegewezen aan `downloads`, `trending`
