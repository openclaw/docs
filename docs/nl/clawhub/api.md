...
summary: Overzicht en conventies van de publieke REST-API (v1).
x-i18n:
    generated_at: "2026-07-03T09:44:56Z"
    model: gpt-5.5
...
Je kunt een catalogus, directory of zoekoppervlak van derden bouwen boven op de openbare lees-API's van ClawHub. Openbare Skills-metadata en Skills-bestanden worden gepubliceerd onder de Skills-licentieregels van ClawHub, terwijl de API zelf een aanvraaglimiet heeft en verantwoord moet worden gebruikt.
...
- Link terug naar de canonieke ClawHub-Skills-URL bij het weergeven van vermeldingen, zodat gebruikers de bronrecord in het register kunnen bekijken.
...
- `GET /api/v1/skills?limit=&cursor=&sort=`
  - `sort`: `updated` (standaard), `recommended` (`default`), `createdAt` (`newest`), `downloads`, `stars` (`rating`), verouderde installatiealiassen `installsCurrent`/`installs`/`installsAllTime` wijzen naar `downloads`, `trending`
...
- `POST /api/v1/users/reserve` reserveert root-slugs en privépakketplaatsaanduidingen zonder release voor de handle van een eigenaar.

## Verouderd

Verouderde `/api/*` en `/api/cli/*` blijven beschikbaar. Zie `DEPRECATIONS.md`.
