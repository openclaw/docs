...
x-i18n:
    generated_at: "2026-07-03T17:28:12Z"
    model: gpt-5.5
...
Je kunt een catalogus, directory of zoekoppervlak van derden bouwen boven op de publieke lees-API's van ClawHub. Publieke skillmetadata en skillbestanden worden gepubliceerd onder de skilllicentieregels van ClawHub, terwijl de API zelf rate-limited is en verantwoord moet worden gebruikt.

Richtlijnen:

- Gebruik publieke leesendpoints zoals `GET /api/v1/skills`, `GET /api/v1/search` en `GET /api/v1/skills/{slug}` voor catalogusvermeldingen.
...
- Spiegel geen verborgen, private of door moderatie geblokkeerde content door publieke API-filters of auth-grenzen te omzeilen.
...
- Geverifieerde requests (geldig Bearer-token): per userbucket.
...
- `GET /api/v1/skills?limit=&cursor=&sort=`
  - `sort`: `updated` (standaard), `recommended` (`default`), `createdAt` (`newest`), `downloads`, `stars` (`rating`), legacy-installatiealiases `installsCurrent`/`installs`/`installsAllTime` worden gemapt naar `downloads`, `trending`
...
- `POST /api/v1/users/reserve` reserveert rootslugs en private pakketplaatsaanduidingen zonder release voor een ownerhandle.
