...
Je kunt een catalogus, directory of zoekoppervlak van derden bouwen boven op de openbare lees-API's van ClawHub. Openbare skillmetadata en skillbestanden worden gepubliceerd onder de licentieregels voor skills van ClawHub, terwijl de API zelf aan snelheidslimieten is gebonden en verantwoordelijk moet worden gebruikt.
...
- Link terug naar de canonieke ClawHub-skill-URL bij het tonen van vermeldingen, zodat gebruikers de bronregistratierecord kunnen inspecteren.
...
- Geauthenticeerde aanvragen (geldige Bearer-token): per gebruikersbucket.
- Ontbrekende/ongeldige token valt terug op afdwinging per IP.
...
- `GET /api/v1/skills?limit=&cursor=&sort=`
  - `sort`: `updated` (standaard), `recommended` (`default`), `createdAt` (`newest`), `downloads`, `stars` (`rating`), verouderde install-aliassen `installsCurrent`/`installs`/`installsAllTime` verwijzen naar `downloads`, `trending`
...
- `POST /api/v1/users/reserve` reserveert root-slugs en privé pakketplaatsaanduidingen zonder release voor een eigenaarshandle.
