...
- Cache reacties en respecteer `429`, `Retry-After` en headers voor limieten voor de verzoeksfrequentie in plaats van agressief te pollen.
- Link terug naar de canonieke ClawHub-skill-URL wanneer je vermeldingen weergeeft, zodat gebruikers het bronrecord in het register kunnen bekijken.
- Gebruik canonieke pagina-URL's in de vorm `https://clawhub.ai/<owner>/<slug>`.
- Wek niet de indruk dat ClawHub de site van derden aanbeveelt, verifieert of beheert.
...
## Limieten voor verzoeksfrequentie

Authenticatiebewuste handhaving:

- Anonieme verzoeken: per IP.
- Geauthenticeerde verzoeken (geldige Bearer-token): per gebruikersbucket.
...
- `X-RateLimit-Reset`: seconden sinds Unix-epoch (absolute resettijd)
...
- `POST /api/v1/users/reserve` reserveert root-slugs en privépakketplaatsaanduidingen zonder release voor een owner-handle.
