...
Je kunt een catalogus, directory of zoekoppervlak van derden bouwen boven op de publieke lees-API's van ClawHub. Publieke metadata van Skills en Skills-bestanden worden gepubliceerd onder de licentieregels voor Skills van ClawHub, terwijl de API zelf rate-limited is en verantwoord moet worden gebruikt.

Richtlijnen:

- Gebruik publieke lees-endpoints zoals `GET /api/v1/skills`, `GET /api/v1/search` en `GET /api/v1/skills/{slug}` voor catalogusvermeldingen.
- Cache reacties en respecteer `429`, `Retry-After` en rate-limit-headers in plaats van agressief te pollen.
- Link terug naar de canonieke ClawHub-Skill-URL bij het weergeven van vermeldingen, zodat gebruikers het bronregisterrecord kunnen inspecteren.
...
- Mirror geen verborgen, private of door moderatie geblokkeerde content door publieke API-filters of auth-grenzen te omzeilen.

## Auth

- Publieke leesactie: geen token vereist.
...
- `X-RateLimit-Remaining` / `RateLimit-Remaining`: exact resterend budget wanneer
  aanwezig; gesharde succesvolle aanvragen laten dit weg in plaats van een globale
  benaderingswaarde terug te geven
...
- `GET /api/v1/skills?limit=&cursor=&sort=`
  ...
  - `cursor` geldt voor niet-`trending`-sorteringen
...
- `GET /api/v1/download?slug=&version=&tag=`
  ...
  - Huidige GitHub-backed Skills met een `clean`- of `suspicious`-scan retourneren een
...
- `GET /api/v1/skills/export?startDate=&endDate=&limit=&cursor=`
  ...
  - Huidige GitHub-backed Skills met een `clean`- of `suspicious`-scan worden geëxporteerd
    als `public-github`-overdrachtsdescriptoren.
...
- `POST /api/v1/users/reserve` reserveert root-slugs en private no-release package-placeholders voor een eigenaarshandle.

## Legacy

Verouderde `/api/*` en `/api/cli/*` nog steeds beschikbaar. Zie `DEPRECATIONS.md`.
