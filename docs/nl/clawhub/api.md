...
Je kunt een catalogus, directory of zoekoppervlak van derden bouwen boven op ClawHub's openbare lees-API's. Openbare Skills-metadata en Skills-bestanden worden gepubliceerd onder de licentieregels voor Skills van ClawHub, terwijl de API zelf snelheidsbeperkt is en verantwoord moet worden gebruikt.
...
- Link terug naar de canonieke ClawHub-URL van de Skill wanneer je vermeldingen toont, zodat gebruikers de bronregistratierecord kunnen inspecteren.
...
- Openbare leesbewerkingen: geen token vereist.
...
Openbare leesbewerkingen:

- `GET /api/v1/search?q=...`
  - Optionele filters: `highlightedOnly=true`, `nonSuspiciousOnly=true`
  - Legacy-alias: `nonSuspicious=true`
- `GET /api/v1/skills?limit=&cursor=&sort=`
  - `sort`: `updated` (standaard), `recommended` (`default`), `createdAt` (`newest`), `downloads`, `stars` (`rating`), legacy installatie-aliassen `installsCurrent`/`installs`/`installsAllTime` worden gemapt naar `downloads`, `trending`
...
- `GET /api/v1/download?slug=&version=&tag=`
  ...
  - Huidige GitHub-ondersteunde Skills met een `clean`- of `suspicious`-scan retourneren een
...
- `GET /api/v1/skills/export?startDate=&endDate=&limit=&cursor=`
  ...
  - Huidige GitHub-ondersteunde Skills met een `clean`- of `suspicious`-scan worden geëxporteerd
