...
x-i18n:
    generated_at: "2026-06-30T22:23:22Z"
    model: gpt-5.5
...
Je kunt een externe catalogus, directory of zoekoppervlak bouwen boven op ClawHub's openbare lees-API's. Openbare Skills-metadata en Skills-bestanden worden gepubliceerd onder ClawHub's Skills-licentieregels, terwijl de API zelf snelheidsbeperkt is en verantwoord moet worden gebruikt.
...
- Link terug naar de canonieke ClawHub Skills-URL wanneer je vermeldingen weergeeft, zodat gebruikers het bronrecord in het register kunnen inspecteren.
...
- `GET /api/v1/skills?limit=&cursor=&sort=`
  - `sort`: `updated` (standaard), `recommended` (`default`), `createdAt` (`newest`), `downloads`, `stars` (`rating`), legacy install-aliassen `installsCurrent`/`installs`/`installsAllTime` mappen naar `downloads`, `trending`
...
- `GET /api/v1/download?slug=&version=&tag=`
  ...
  - Huidige GitHub-backed Skills met een `clean`- of `suspicious`-scan geven een
