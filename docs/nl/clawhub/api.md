...
x-i18n:
    generated_at: "2026-07-03T01:00:19Z"
    model: gpt-5.5
...
Je kunt een catalogus, directory of zoekoppervlak van derden bouwen bovenop de openbare lees-API's van ClawHub. Openbare metadata van Skills en skillbestanden worden gepubliceerd onder de skilllicentieregels van ClawHub, terwijl de API zelf rate-limited is en verantwoord moet worden gebruikt.
...
- Link terug naar de canonieke ClawHub-skill-URL wanneer je vermeldingen toont, zodat gebruikers het bronrecord in het register kunnen inspecteren.
...
- `GET /api/v1/skills?limit=&cursor=&sort=`
  - `sort`: `updated` (standaard), `recommended` (`default`), `createdAt` (`newest`), `downloads`, `stars` (`rating`), legacy install-aliassen `installsCurrent`/`installs`/`installsAllTime` worden gemapt naar `downloads`, `trending`
  - Ongeldige `sort`-waarden geven `400` terug
  - `cursor` is van toepassing op niet-`trending` sorts
  ...
  - `recommended` gebruikt signalen voor engagement en recentheid.
