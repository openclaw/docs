...
summary: 'Openbare REST-API (v1): overzicht en conventies.'
x-i18n:
...
Je kunt een externe catalogus, directory of zoekoppervlak bouwen bovenop de openbare read-API's van ClawHub. Openbare Skill-metadata en Skill-bestanden worden gepubliceerd onder de Skill-licentieregels van ClawHub, terwijl de API zelf rate-limited is en verantwoord moet worden gebruikt.

Richtlijnen:

- Gebruik openbare read-endpoints zoals `GET /api/v1/skills`, `GET /api/v1/search` en `GET /api/v1/skills/{slug}` voor catalogusvermeldingen.
- Cache responses en respecteer `429`, `Retry-After` en rate-limit-headers in plaats van agressief te pollen.
- Link terug naar de canonieke ClawHub-Skill-URL bij het weergeven van vermeldingen, zodat gebruikers de bronregistratierecord kunnen inspecteren.
