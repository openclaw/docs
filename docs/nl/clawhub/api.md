...
x-i18n:
    generated_at: "2026-07-01T20:26:11Z"
    model: gpt-5.5
...
Je kunt een catalogus, directory of zoekoppervlak van derden bouwen boven op de openbare lees-API's van ClawHub. Openbare skillmetadata en skillbestanden worden gepubliceerd onder de skilllicentieregels van ClawHub, terwijl de API zelf rate-limited is en verantwoordelijk moet worden gebruikt.
...
- Link terug naar de canonieke ClawHub-skill-URL wanneer je vermeldingen toont, zodat gebruikers het bronrecord in de registry kunnen inspecteren.
...
- `X-RateLimit-Reset`: Unix-epochseconden (absolute reset-tijd)
- `RateLimit-Reset`: vertraging in seconden tot reset
- `X-RateLimit-Remaining` / `RateLimit-Remaining`: exact resterend budget wanneer
  aanwezig; gesharde geslaagde requests laten dit weg in plaats van een geschatte
...
- v1-fouten zijn platte tekst (`text/plain; charset=utf-8`), inclusief `400`,
  `401`, `403`, `404`, `429` en geblokkeerde-download-responses.
