...
x-i18n:
    generated_at: "2026-07-04T18:07:33Z"
    model: gpt-5.5
...
Je kunt een catalogus, directory of zoekoppervlak van derden bouwen boven op ClawHub's openbare lees-API's. Openbare Skills-metadata en Skills-bestanden worden gepubliceerd onder ClawHub's licentieregels voor Skills, terwijl de API zelf aan snelheidslimieten is onderworpen en verantwoord moet worden gebruikt.

Richtlijnen:

- Gebruik openbare lees-eindpunten zoals `GET /api/v1/skills`, `GET /api/v1/search` en `GET /api/v1/skills/{slug}` voor catalogusvermeldingen.
- Cache reacties en respecteer `429`, `Retry-After` en rate-limit-headers in plaats van agressief te pollen.
- Link terug naar de canonieke ClawHub-Skills-URL wanneer je vermeldingen weergeeft, zodat gebruikers de bronvermelding in het register kunnen inspecteren.
- Gebruik canonieke pagina-URL's in de vorm `https://clawhub.ai/<owner>/skills/<slug>`.
- Wek niet de indruk dat ClawHub de site van derden onderschrijft, verifieert of beheert.
...
- `RateLimit-Reset`: wachttijd in seconden tot reset
- `X-RateLimit-Remaining` / `RateLimit-Remaining`: exact resterend budget wanneer
  aanwezig; gesharde succesvolle verzoeken laten dit weg in plaats van een benaderde
  globale waarde te retourneren
- `Retry-After`: wachttijd in seconden bij `429`
...
- Gebruik anders `RateLimit-Reset` of leid de wachttijd af uit `X-RateLimit-Reset`.
- Voeg jitter toe aan nieuwe pogingen.

## Fouten

- v1-fouten zijn platte tekst (`text/plain; charset=utf-8`), inclusief `400`,
  `401`, `403`, `404`, `429` en reacties op geblokkeerde downloads.
...
- `GET /api/v1/skills?limit=&cursor=&sort=`
  - `sort`: `updated` (standaard), `recommended` (`default`), `createdAt` (`newest`), `downloads`, `stars` (`rating`), verouderde installatie-aliassen `installsCurrent`/`installs`/`installsAllTime` mappen naar `downloads`, `trending`
  ...
  - Verouderde alias: `nonSuspicious=true`
  - Met `nonSuspiciousOnly=true` kunnen op cursor gebaseerde pagina's minder dan `limit` items bevatten; gebruik `nextCursor` om door te gaan.
...
- `GET /api/v1/download?slug=&version=&tag=`
  ...
  - Huidige op GitHub gebaseerde Skills met een `clean`- of `suspicious`-scan retourneren een
...
- `GET /api/v1/skills/export?startDate=&endDate=&limit=&cursor=`
  ...
  - Huidige op GitHub gebaseerde Skills met een `clean`- of `suspicious`-scan worden geëxporteerd
...
- `POST /api/v1/users/reserve` reserveert root-slugs en privé pakketplaatsaanduidingen zonder release voor een owner-handle.

## Verouderd

Verouderde `/api/*` en `/api/cli/*` zijn nog beschikbaar. Zie `DEPRECATIONS.md`.
