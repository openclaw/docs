...
Je kunt een catalogus, directory of zoekoppervlak van derden bouwen boven op de openbare lees-API's van ClawHub. Openbare Skills-metadata en Skills-bestanden worden gepubliceerd onder de Skills-licentieregels van ClawHub, terwijl de API zelf snelheidslimieten heeft en verantwoord moet worden gebruikt.
...
- Link terug naar de canonieke ClawHub-Skills-URL wanneer je vermeldingen toont, zodat gebruikers het bronrecord in het register kunnen inspecteren.
...
- `GET /api/v1/skills?limit=&cursor=&sort=`
  - `sort`: `updated` (standaard), `recommended` (`default`), `createdAt` (`newest`), `downloads`, `stars` (`rating`), verouderde installatie-aliassen `installsCurrent`/`installs`/`installsAllTime` verwijzen naar `downloads`, `trending`
...
- `POST /api/v1/users/reserve` reserveert root-slugs en privé pakketplaatsaanduidingen zonder release voor een eigenaars-handle.
