...
x-i18n:
    generated_at: "2026-07-02T22:38:58Z"
    model: gpt-5.5
...
Je kunt een catalogus, directory of zoekoppervlak van derden bouwen boven op de openbare lees-API's van ClawHub. Openbare skillmetadata en skillbestanden worden gepubliceerd onder de skilllicentieregels van ClawHub, terwijl de API zelf snelheidsbeperkt is en verantwoord moet worden gebruikt.
...
- Link terug naar de canonieke ClawHub-skill-URL wanneer je vermeldingen toont, zodat gebruikers de bronregistratie kunnen inspecteren.
...
- `GET /api/v1/skills?limit=&cursor=&sort=`
  - `sort`: `updated` (standaard), `recommended` (`default`), `createdAt` (`newest`), `downloads`, `stars` (`rating`), legacy installatie-aliassen `installsCurrent`/`installs`/`installsAllTime` mappen naar `downloads`, `trending`
...
- `GET /api/v1/skills/export?startDate=&endDate=&limit=&cursor=`
  - Gehoste skills worden als opgeslagen bestanden geëxporteerd.
