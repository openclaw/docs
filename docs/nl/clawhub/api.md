...
Je kunt een catalogus, directory of zoekoppervlak van derden bouwen boven op de openbare lees-API's van ClawHub. Openbare skillmetadata en skillbestanden worden gepubliceerd volgens de licentieregels voor Skills van ClawHub, terwijl de API zelf snelheidsbegrensd is en verantwoord moet worden gebruikt.
...
- Link terug naar de canonieke ClawHub-skill-URL wanneer je vermeldingen toont, zodat gebruikers het bronregistratierecord kunnen inspecteren.
...
- Geverifieerde aanvragen (geldige Bearer-token): per gebruikersbucket.
...
- `X-RateLimit-Remaining` / `RateLimit-Remaining`: exact resterend budget wanneer
  aanwezig; gesharde succesvolle aanvragen laten dit weg in plaats van een benaderende
