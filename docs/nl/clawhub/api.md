...
x-i18n:
    generated_at: "2026-07-04T20:37:05Z"
    model: gpt-5.5
...
Je kunt een catalogus, directory of zoekoppervlak van derden bouwen boven op de openbare lees-API's van ClawHub. Openbare Skills-metadata en Skills-bestanden worden gepubliceerd onder de licentieregels voor Skills van ClawHub, terwijl de API zelf snelheidsbeperkt is en verantwoord moet worden gebruikt.
...
- Cache responses en respecteer `429`, `Retry-After` en snelheidslimietheaders in plaats van agressief te pollen.
...
- `X-RateLimit-Remaining` / `RateLimit-Remaining`: exact resterend budget wanneer
  aanwezig; gesharde succesvolle requests laten dit weg in plaats van een globale
  benaderde waarde terug te geven
...
- v1-fouten zijn platte tekst (`text/plain; charset=utf-8`), inclusief `400`,
  `401`, `403`, `404`, `429` en responses voor geblokkeerde downloads.
...
- `GET /api/v1/skills?limit=&cursor=&sort=`
  - `sort`: `updated` (standaard), `recommended` (`default`), `createdAt` (`newest`), `downloads`, `stars` (`rating`), verouderde installatie-aliassen `installsCurrent`/`installs`/`installsAllTime` worden gekoppeld aan `downloads`, `trending`
...
- `POST /api/v1/users/reserve` reserveert root-slugs en privé-pakketplaatshouders zonder release voor een owner-handle.
