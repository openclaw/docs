...
x-i18n:
    generated_at: "2026-07-01T08:14:00Z"
    model: gpt-5.5
...
Je kunt een catalogus, directory of zoekoppervlak van derden bouwen boven op de openbare lees-API's van ClawHub. Openbare Skill-metadata en Skill-bestanden worden gepubliceerd onder de Skill-licentieregels van ClawHub, terwijl de API zelf snelheidsbeperkt is en verantwoord moet worden gebruikt.
...
- Link terug naar de canonieke ClawHub-Skill-URL bij het weergeven van vermeldingen, zodat gebruikers het bronrecord in het register kunnen bekijken.
...
- `X-RateLimit-Remaining` / `RateLimit-Remaining`: exact resterend budget wanneer
  aanwezig; gesharde succesvolle aanvragen laten dit weg in plaats van een benaderde
...
- `GET /api/v1/skills?limit=&cursor=&sort=`
  - `sort`: `updated` (standaard), `recommended` (`default`), `createdAt` (`newest`), `downloads`, `stars` (`rating`), verouderde installatie-aliassen `installsCurrent`/`installs`/`installsAllTime` wijzen naar `downloads`, `trending`
...
- `POST /api/v1/users/reserve` reserveert root-slugs en privé-placeholderpakketten zonder release voor een eigenaarshandle.
