...
- `GET /api/v1/skills?limit=&cursor=&sort=`
  - `sort`: `updated` (standaard), `recommended` (`default`), `createdAt` (`newest`), `downloads`, `stars` (`rating`), verouderde install-aliassen `installsCurrent`/`installs`/`installsAllTime` mappen naar `downloads`, `trending`
...
- `POST /api/v1/users/reserve` reserveert root-slugs en private tijdelijke no-release-pakketplaatsen voor een owner-handle.
