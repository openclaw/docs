...
x-i18n:
    generated_at: "2026-07-12T08:41:49Z"
    model: gpt-5.6
...
U kunt een catalogus, directory of zoekinterface van derden bouwen boven op de openbare lees-API's van ClawHub. Openbare metadata en bestanden van Skills worden gepubliceerd volgens de licentieregels voor Skills van ClawHub, terwijl de API zelf snelheidsbeperkingen heeft en verantwoord moet worden gebruikt.
...
- Cache antwoorden en respecteer `429`, `Retry-After` en headers voor snelheidsbeperkingen in plaats van agressief te pollen.
- Link bij het weergeven van vermeldingen terug naar de canonieke URL van de Skill op ClawHub, zodat gebruikers de bronrecord in het register kunnen bekijken.
- Gebruik canonieke pagina-URL's in de vorm `https://clawhub.ai/<owner>/skills/<slug>`.
- Wek niet de indruk dat ClawHub de externe site onderschrijft, verifieert of beheert.
...
- Anonieme verzoeken: per IP-adres.
- Geauthenticeerde verzoeken (geldig Bearer-token): per gebruikersbucket.
- Bij een ontbrekend of ongeldig token wordt teruggevallen op handhaving per IP-adres.

- Lezen: 3000/min per IP-adres, 12000/min per sleutel
- Schrijven: 300/min per IP-adres, 3000/min per sleutel
- Downloaden: 1200/min per IP-adres, 6000/min per sleutel
...
- `X-RateLimit-Reset`: Unix-epochtijd in seconden (absoluut resetmoment)
- `RateLimit-Reset`: aantal seconden vertraging tot de reset
- `X-RateLimit-Remaining` / `RateLimit-Remaining`: exact resterend quotum indien
  aanwezig; bij gesharde geslaagde verzoeken wordt dit weggelaten in plaats van een
  benaderde globale waarde te retourneren
...
- v1-fouten zijn platte tekst (`text/plain; charset=utf-8`), waaronder `400`,
  `401`, `403`, `404`, `429` en antwoorden voor geblokkeerde downloads.
...
- `GET /api/v1/skills?limit=&cursor=&sort=`
  - `sort`: `updated` (standaard), `recommended` (`default`), `createdAt` (`newest`), `downloads`, `stars` (`rating`), verouderde installatiealiassen `installsCurrent`/`installs`/`installsAllTime` worden toegewezen aan `downloads`, `trending`
  - Ongeldige waarden voor `sort` retourneren `400`
  - `cursor` is van toepassing op sorteervolgorden anders dan `trending`
...
- `GET /api/v1/download?slug=&version=&tag=`
  ...
  - Huidige door GitHub ondersteunde Skills met een scanresultaat `clean` of `suspicious` retourneren een
    JSON-overdrachtsdescriptor `public-github` in plaats van bytes van ClawHub.
- `GET /api/v1/skills/export?startDate=&endDate=&limit=&cursor=`
  - Gehoste Skills worden geëxporteerd als opgeslagen bestanden.
  - Huidige door GitHub ondersteunde Skills met een scanresultaat `clean` of `suspicious` worden geëxporteerd
    als overdrachtsdescriptors van het type `public-github`.
...
- `POST /api/v1/users/reserve` reserveert hoofdslugs en privéplaatshouders voor pakketten zonder release voor een eigenaarshandle.
