...
x-i18n:
    generated_at: "2026-07-01T13:08:49Z"
    model: gpt-5.5
...
- Link terug naar de canonieke ClawHub-Skill-URL wanneer je vermeldingen toont, zodat gebruikers het bronregisterrecord kunnen inspecteren.
- Gebruik canonieke pagina-URL's in de vorm `https://clawhub.ai/<owner>/skills/<slug>`.
- Wek niet de indruk dat ClawHub de site van derden goedkeurt, verifieert of beheert.
...
- `GET /api/v1/skills?limit=&cursor=&sort=`
  - `sort`: `updated` (standaard), `recommended` (`default`), `createdAt` (`newest`), `downloads`, `stars` (`rating`), legacy-installatiealiassen `installsCurrent`/`installs`/`installsAllTime` worden gemapt naar `downloads`, `trending`
...
- `POST /api/v1/users/reserve` reserveert rootslugs en privé-pakketplaatsaanduidingen zonder release voor een eigenaarshandle.
