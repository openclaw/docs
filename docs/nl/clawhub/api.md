...
x-i18n:
    generated_at: "2026-06-30T14:09:04Z"
    model: gpt-5.5
...
Je kunt boven op de openbare lees-API's van ClawHub een externe catalogus, directory of zoekoppervlak bouwen. Openbare Skill-metadata en Skill-bestanden worden gepubliceerd onder de Skill-licentieregels van ClawHub, terwijl de API zelf snelheidsbeperkt is en verantwoord moet worden gebruikt.
...
- Cache antwoorden en respecteer `429`, `Retry-After` en headers voor snelheidslimieten in plaats van agressief te pollen.
...
- `X-RateLimit-Remaining` / `RateLimit-Remaining`: exact resterend budget wanneer
  aanwezig; gesharde succesvolle aanvragen laten dit weg in plaats van een geschatte
