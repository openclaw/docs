...
x-i18n:
    generated_at: "2026-05-12T23:30:06Z"
    model: gpt-5.5
    provider: openai
...
ClawHub gebruikt **minimale telemetrie** om **installatieaantallen** te berekenen (wat daadwerkelijk in gebruik is) en om beter sorteren/filteren mogelijk te maken.
...
Bij elke `clawhub sync` rapporteert de CLI een **volledige momentopname** van wat is gevonden, gegroepeerd per scanroot (“map/root”).

Voor elke root slaan we op:

- `rootId`: een **SHA-256-hash** van het canonieke rootpad (de server ziet nooit het ruwe pad).
- `label`: een menselijk leesbaar label afgeleid van de laatste twee padsegmenten (homepaden worden weergegeven met `~`).
...
- `lastVersion` (best-effort; momenteel de register-gematchte versie als die bekend is).
...
- `installsCurrent`: unieke gebruikers die de skill momenteel in ten minste één actieve root hebben geïnstalleerd.
- `installsAllTime`: unieke gebruikers die ooit hebben gerapporteerd dat de skill is geïnstalleerd.

### Meerdere roots

Als je vanuit meerdere mappen synchroniseert, behandelen we elke scanroot afzonderlijk. Een skill is “momenteel geïnstalleerd” als die in **een** actieve root bestaat.
...
- Als de skill uit al je roots wordt verwijderd, telt die niet langer mee voor `installsCurrent`.
- `installsAllTime` daalt nooit, tenzij je telemetrie verwijdert (zie hieronder).
