...
Voor elke root slaan we het volgende op:
...
Voor elke skill die onder een root wordt gevonden, slaan we het volgende op:

- `skillId` (opgelost via slug; alleen skills die in het register bestaan worden gevolgd).
- `firstSeenAt`, `lastSeenAt`.
- `lastVersion` (best effort; momenteel de met het register overeenkomende versie, indien bekend).
...
- Als een skill bij de volgende synchronisatie uit een root verdwijnt, markeren we die als verwijderd voor die root.
