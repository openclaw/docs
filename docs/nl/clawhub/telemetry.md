---
read_when:
    - Bezig met telemetrie- en privacy-instellingen
    - Vragen over welke gegevens worden verzameld
summary: Installatietelemetrie verzameld door de ClawHub CLI en hoe u zich kunt afmelden.
x-i18n:
...
Bij elke gerapporteerde `clawhub install` verzendt de CLI één best-effort installatiegebeurtenis.

De gebeurtenis bevat:

- `slug`: de geïnstalleerde vaardigheidsslug.
...
- Geen bestandsinhoud.
- Geen logs, prompts of andere CLI-uitvoer per run.

## Installatietellingen

ClawHub houdt geaggregeerde tellers per vaardigheid bij:

- `installsAllTime`: unieke gebruikers die ten minste één CLI-installatie voor de vaardigheid hebben gerapporteerd.
...
Wanneer dit is ingesteld, verzendt de CLI geen installatietelemetrie.
