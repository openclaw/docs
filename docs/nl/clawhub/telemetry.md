...
- Telemetrie is **niet uitgeschakeld** (zie hieronder “Uitschakelen”).
...
- `rootId`: een **SHA-256-hash** van het canonieke rootpad (de server ziet het ruwe pad nooit).
- `label`: een voor mensen leesbaar label, afgeleid van de laatste twee padsegmenten (homepaden worden weergegeven met `~`).
...
- `lastVersion` (naar beste kunnen; momenteel de met het register overeenkomende versie als die bekend is).
