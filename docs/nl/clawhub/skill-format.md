...
Declareer optionele omgevingsvariabelen onder `metadata.openclaw.envVars` en stel `required: false` in. Voeg geen optionele items toe aan `requires.env`, omdat `requires.env` betekent dat de skill niet zonder deze items kan draaien.
...
- Embeddingtekst bevat `SKILL.md` + tot ongeveer 40 niet-`.md`-bestanden (best-effort-limiet).
...
- ClawHub ondersteunt geen betaalde skills, prijsstelling per skill, paywalls of omzetdeling.
