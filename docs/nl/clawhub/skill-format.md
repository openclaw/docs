...
`SKILL.md`- of legacy `skills.md`-bestanden in openbare, niet-fork-repository's die eigendom zijn van
...
Declareer optionele omgevingsvariabelen onder `metadata.openclaw.envVars` en stel `required: false` in. Voeg geen optionele vermeldingen toe aan `requires.env`, omdat `requires.env` betekent dat de skill niet zonder deze variabelen kan worden uitgevoerd.
...
Alleen “op tekst gebaseerde” bestanden worden door publish geaccepteerd.
...
- Package-scopes moeten exact overeenkomen met de publisher-handle van ClawHub. Publisher-handles mogen kleine letters, cijfers, koppeltekens, punten en underscores gebruiken; ze moeten beginnen en eindigen met een kleine letter of cijfer.
...
- ClawHub ondersteunt geen betaalde skills, prijzen per skill, paywalls of inkomstenverdeling.
