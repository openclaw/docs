...
x-i18n:
    generated_at: "2026-07-03T17:29:18Z"
    model: gpt-5.5
...
het aangemelde GitHub-account. Deze importeert geen private repos, forks,
gearchiveerde/uitgeschakelde repos of openbare repos van derden.
...
De beveiligingsanalyse van ClawHub controleert of wat je skill declareert overeenkomt met wat deze daadwerkelijk doet. Als je code verwijst naar `TODOIST_API_KEY`, maar je frontmatter deze niet declareert onder `requires.env`, `primaryEnv` of `envVars`, dan markeert de analyse dit als een metadata-mismatch. Nauwkeurige declaraties helpen je skill door de review te komen en helpen gebruikers begrijpen wat ze installeren.
...
- Embedding-tekst omvat `SKILL.md` + maximaal circa 40 niet-`.md`-bestanden (best-effort-limiet).

## Slugs

- Standaard afgeleid van de mapnaam.
- Package-scopes moeten exact overeenkomen met de ClawHub publisher-handle. Publisher-handles mogen kleine letters, cijfers, koppeltekens, punten en underscores gebruiken; ze moeten beginnen en eindigen met een kleine letter of cijfer.
...
- Iedereen mag gepubliceerde skills gebruiken, wijzigen en opnieuw distribueren, inclusief commercieel.
...
- Als je skill integreert met een betaalde dienst van een derde partij, documenteer dan duidelijk de externe kosten en het vereiste account in de skill-instructies en env-declaraties (`requires.env` voor vereiste variabelen, of `envVars` met `required: false` voor optionele variabelen).
