...
summary: Indeling van de Skill-map, vereiste bestanden, toegestane bestandstypen, limieten.
x-i18n:
...
| `envVars`          | `array`    | Declaraties van omgevingsvariabelen met `name`, optionele `required` en optionele `description`. Stel `required: false` in voor optionele env-vars.       |
...
- Embeddingtekst bevat `SKILL.md` + tot circa 40 niet-`.md`-bestanden (best-effort limiet).
