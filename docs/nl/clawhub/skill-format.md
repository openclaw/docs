...
- `.clawhubignore` (negeerpatronen voor publicatie, legacy `.clawdhubignore`)
...
- De server haalt metadata uit frontmatter tijdens publicatie.
...
Declareer optionele omgevingsvariabelen onder `metadata.openclaw.envVars` en stel `required: false` in. Voeg geen optionele items toe aan `requires.env`, omdat `requires.env` betekent dat de skill niet zonder deze kan draaien.
...
- Package-slugs moeten kleine letters gebruiken en npm-veilig zijn, bijvoorbeeld `@example.tools/demo-plugin` of `demo-plugin`.
