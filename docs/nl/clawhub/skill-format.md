...
read_when:
    - Skills publiceren
    - Fouten opsporen bij publicatie-/synchronisatiefouten
...
Metadata voor lokale installatie (geschreven door de CLI):
...
| `envVars`          | `array`    | Declaraties van omgevingsvariabelen met `name`, optioneel `required` en optioneel `description`. Zet `required: false` voor optionele omgevingsvariabelen.     |
...
- Ingesloten tekst omvat `SKILL.md` + maximaal ongeveer 40 niet-`.md`-bestanden (best-effort limiet).

## Slugs

- Standaard afgeleid van de mapnaam.
- Moet in kleine letters en URL-veilig zijn: `^[a-z0-9][a-z0-9-]*$`.
