...
x-i18n:
    generated_at: "2026-07-01T15:27:55Z"
    model: gpt-5.5
...
De GitHub-importer op het web is strenger dan lokaal publiceren/synchroniseren. Deze vindt alleen
`SKILL.md`- of legacy `skills.md`-bestanden in openbare, niet-geforkte repositories die eigendom zijn van
het ingelogde GitHub-account. Private repositories, forks,
...
- Scriptbestanden worden na het uploaden nog steeds gescand; PowerShell-bestanden `.ps1`, `.psm1` en `.psd1` worden geaccepteerd als tekst.
...
- Package-slugs moeten uit kleine letters bestaan en npm-safe zijn, bijvoorbeeld `@example.tools/demo-plugin` of `demo-plugin`.
