...
read_when:
    - Skills publiceren
    - Fouten bij publiceren opsporen
summary: Skillmapindeling, vereiste bestanden, toegestane bestandstypen, limieten.
x-i18n:
...
het ingelogde GitHub-account. Privé-repo's, forks,
...
Installatiestatus van de workdir (geschreven door de CLI):
...
Skill-metadata wordt gedeclareerd in de YAML-frontmatter bovenaan je `SKILL.md`. Dit vertelt de registry (en de beveiligingsanalyse) wat je skill nodig heeft om te draaien.
...
Declareer optionele omgevingsvariabelen onder `metadata.openclaw.envVars` en stel `required: false` in. Voeg geen optionele items toe aan `requires.env`, want `requires.env` betekent dat de skill zonder deze variabelen niet kan draaien.
...
- Package-scopes moeten exact overeenkomen met de ClawHub-publisherhandle. Publisherhandles mogen kleine letters, cijfers, koppeltekens, punten en underscores gebruiken; ze moeten beginnen en eindigen met een kleine letter of een cijfer.
...
- Voeg geen prijsmetadata toe aan `SKILL.md`; dit is geen onderdeel van het skill-formaat en maakt een gepubliceerde skill niet betaald.
