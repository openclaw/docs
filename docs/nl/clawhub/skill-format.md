...
`SKILL.md`- of legacy `skills.md`-bestanden in openbare, niet-fork-repositories die eigendom zijn van
het aangemelde GitHub-account. Deze importeert geen privérepo's, forks,
...
- `description` wordt gebruikt als skill-samenvatting in de UI/zoekfunctie.
...
| `always`           | `boolean`  | Indien `true`, is de skill altijd actief (geen expliciete installatie nodig).                                                                                  |
| `skillKey`         | `string`   | Overschrijft de invocation key van de skill.                                                                                                                  |
...
Als je skill afhankelijkheden geïnstalleerd moet hebben, declareer ze dan in de `install`-array:
...
Declareer optionele omgevingsvariabelen onder `metadata.openclaw.envVars` en stel `required: false` in. Voeg geen optionele vermeldingen toe aan `requires.env`, omdat `requires.env` betekent dat de skill zonder die variabelen niet kan draaien.
...
- Ingesloten tekst omvat `SKILL.md` + maximaal circa 40 niet-`.md`-bestanden (best-effortlimiet).

## Slugs

- Standaard afgeleid van mapnaam.
...
- Elke publish maakt een nieuwe versie aan (semver).
...
- Als je skill integreert met een betaalde externe dienst, documenteer de externe kosten en het vereiste account duidelijk in de skill-instructies en env-declaraties (`requires.env` voor vereiste variabelen, of `envVars` met `required: false` voor optionele variabelen).
