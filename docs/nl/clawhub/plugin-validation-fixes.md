...
x-i18n:
    generated_at: "2026-07-02T22:39:20Z"
    model: gpt-5.5
...
betekent dat de Plugin-auteur ze kan oplossen in de pakketmetadata, het manifest, SDK-
...
| `package-openclaw-metadata-missing`     | [Voeg het openclaw-blok voor het pakket toe](/nl/clawhub/plugin-validation-fixes#package-openclaw-metadata-missing)                            |
...
- Bouw het pakket als de entrypoint wordt gegenereerd naar `dist`.
...
- Gebruik `patchSessionEntry(...)` of `upsertSessionEntry(...)` bij het schrijven van sessie-
  state.
- Vermijd het laden, muteren en opslaan van het volledige sessiestoreobject.
...
Het manifest gebruikt nog steeds legacy `providerAuthEnvVars`-providerauthenticatiemetagegevens.
...
- Spiegel env-gestuurde channelsetup naar de huidige setup, channelconfiguratie of
  packagechannelmetagegevens die door je pluginvorm worden gebruikt.
...
- Verwijder `openclaw.security.json` totdat OpenClaw een versiegebonden schema voor beveiligingsmanifesten
  en ClawHub-gedrag documenteert.
- Houd beveiligingsgevoelig gedrag gedocumenteerd in je openbare packagedocs of
