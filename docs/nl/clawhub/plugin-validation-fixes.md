...
summary: OpenClaw ClawHub-pluginpakketvalidatiebevindingen oplossen vóór publicatie
title: Plugin-validatiefixes
x-i18n:
    generated_at: "2026-07-04T18:07:08Z"
    model: gpt-5.5
...
ClawHub valideert Pluginpakketten vóór publicatie en kan ook bevindingen tonen uit
geautomatiseerde pakketscans. Deze pagina behandelt bevindingen voor auteurs, wat
betekent: bevindingen die de Pluginauteur kan oplossen in pakketmetadata, manifest, SDK-
imports of gepubliceerd artifact.
...
bedoeld voor OpenClaw-maintainers in plaats van Pluginauteurs.
...
| `package-install-metadata-incomplete`   | [Voltooi installatiemetadata](/nl/clawhub/plugin-validation-fixes#package-install-metadata-incomplete)                               |
...
| `package-npm-pack-unavailable`          | [Maak het npm-artifact packbaar](/nl/clawhub/plugin-validation-fixes#package-npm-pack-unavailable)                                 |
| `package-npm-pack-entrypoint-missing`   | [Neem ingangspunten op in npm-packuitvoer](/nl/clawhub/plugin-validation-fixes#package-npm-pack-entrypoint-missing)                  |
...
npm-pakket, de versie, ingangspunten of OpenClaw-metadata niet kan identificeren.
...
- Houd alle paden naar ingangspunten binnen de pakketdirectory.
...
Het pakket declareert een OpenClaw-ingangspunt, maar het gerefereerde bestand ontbreekt
...
- Bouw het pakket als het ingangspunt naar `dist` wordt gegenereerd.
...
## Gepubliceerd artifact

### package-npm-pack-unavailable

Het pakket kan niet worden verpakt in het artifact dat ClawHub zou inspecteren of
publiceren.
...
- Los ongeldige pakketmetadata, defecte lifecycle-scripts of files-vermeldingen op die
  packen laten mislukken.
- Verwijder `private: true` als dit pakket bedoeld is voor openbare publicatie.
- Voer `clawhub package validate <path-to-plugin>` opnieuw uit.

### package-npm-pack-entrypoint-missing

Het pakket kan worden verpakt, maar het verpakte artifact bevat niet de
...
- Bouw gegenereerde ingangspunten voordat je verpakt.
- Werk `files`, `.npmignore` of builduitvoer bij zodat gedeclareerde ingangspunten worden
  opgenomen.
- Zie [Plugin-ingangspunten](/nl/plugins/sdk-entrypoints).
- Voer `clawhub package validate <path-to-plugin>` opnieuw uit.
...
Het verpakte artifact mist OpenClaw-metadata die in je bronpakket bestaat.

- Voer `npm pack --dry-run` uit en inspecteer de opgenomen metadatabestanden.
- Zorg dat `package.json` het `openclaw`-blok bevat in het verpakte artifact.
...
Het pluginmanifest bevat top-level velden die OpenClaw niet ondersteunt.
...
- Verplaats runtimegedrag naar pluginregistratiecode en houd `contracts`
  beperkt tot statische metadata over eigendom van mogelijkheden.
- Voer `clawhub package validate <path-to-plugin>` opnieuw uit.
...
- Vervang root-barrel-imports door gerichte imports uit publieke subpaden.
...
- Vervang gereserveerde interne SDK-imports van OpenClaw door gedocumenteerde publieke
...
- Gebruik `patchSessionEntry(...)` of `upsertSessionEntry(...)` om sessie-
  metadata persistent te maken.
- Gebruik transcriptidentiteit of targethelpers wanneer de code een
  transcriptbewerking voorbereidt.
- Maak legacy transcriptbestandspaden niet persistent en vertrouw er niet op.
- Zie [Runtime-API](/nl/plugins/sdk-runtime#agent-session-state) en
  [Plugin SDK-subpaden](/nl/plugins/sdk-subpaths).
- Voer `clawhub package validate <path-to-plugin>` opnieuw uit.
...
- Gebruik `appendSessionTranscriptMessageByIdentity(...)` voor het toevoegen aan transcripten.
- Gebruik `publishSessionTranscriptUpdateByIdentity(...)` voor meldingen over transcript-
  updates.
- Geef de voorkeur aan het gestructureerde transcriptruntimeoppervlak zodat OpenClaw de
...
- Spiegel env-gedreven channelsetup naar de huidige setup, channelconfiguratie of
  pakketchannelmetadata die door de vorm van je plugin wordt gebruikt.
...
- Gebruik een gedocumenteerd versioned schema pas nadat OpenClaw er een publiceert.
- Voer `clawhub package validate <path-to-plugin>` opnieuw uit.
...
- Verwijder `openclaw.security.json` totdat OpenClaw een versioned schema voor beveiligings-
