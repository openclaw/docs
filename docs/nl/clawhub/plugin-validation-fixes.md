...
read_when:
    ...
    - ClawHub heeft een publicatie van een Plugin-pakket geweigerd of erover gewaarschuwd
...
wat betekent bevindingen die de Plugin-auteur kan oplossen in de pakketmetadata,
het manifest, SDK-imports of het gepubliceerde artefact.

Dit behandelt geen interne dekkingsbevindingen van Plugin Inspector. Als een volledig rapport
scanner-onderhoudscodes bevat zonder herstelrichtlijnen voor auteurs, zijn die
...
De pakketmetadata bestaat, maar declareert geen OpenClaw-runtime-toegangspunt.
...
- Herstel ongeldige pakketmetadata, defecte lifecycle-scripts of files-vermeldingen die
...
- Verplaats runtimegedrag naar Plugin-registratiecode en houd `contracts`
  beperkt tot statische eigendomsmetagegevens voor capabilities.
- Voer `clawhub package validate <path-to-plugin>` opnieuw uit.
...
Het manifest gebruikt nog steeds legacy `providerAuthEnvVars`-provider-auth-metagegevens.
...
- Gebruik pas een gedocumenteerd versioned schema nadat OpenClaw er een publiceert.
- Voer `clawhub package validate <path-to-plugin>` opnieuw uit.
...
- Verwijder `openclaw.security.json` totdat OpenClaw een versioned beveiligingsmanifestschema
