...
summary: Los validatiebevindingen voor ClawHub-Plugin-pakketten op vóór publicatie
title: Oplossingen voor Plugin-validatie
x-i18n:
    generated_at: "2026-07-01T08:14:59Z"
    model: gpt-5.5
...
| `channel-env-vars`                      | [Spiegel channel-env-vars in huidige metadata](/nl/clawhub/plugin-validation-fixes#channel-env-vars)                                |
...
Het pakket declareert een OpenClaw-toegangspunt, maar het verwezen bestand ontbreekt
...
- Als `openclaw.plugin.json` ook `version` heeft, werk die dan bij zodat deze overeenkomt
  of verwijder verouderde manifestversiemetadata wanneer pakketmetadata gezaghebbend is.
...
Het pakket kan niet worden gepackt naar het artefact dat ClawHub zou inspecteren of
publiceren.

- Voer `npm pack --dry-run` uit vanuit de pakketroot.
- Herstel ongeldige pakketmetadata, kapotte lifecycle-scripts of files-vermeldingen die
...
- Werk `files`, `.npmignore` of build-uitvoer bij zodat gedeclareerde toegangspunten
  worden opgenomen.
- Zie [Plugin-toegangspunten](/nl/plugins/sdk-entrypoints).
- Voer `clawhub package validate <path-to-plugin>` opnieuw uit.
...
In het gepackte artefact ontbreekt OpenClaw-metadata die in je bronpakket bestaat.
...
- Houd `name` leesbaar voor mensen en houd `id` als het stabiele machine-id.
...
- Verplaats runtimegedrag naar pluginregistratiecode en beperk `contracts`
  tot statische metadata over eigenaarschap van mogelijkheden.
- Voer `clawhub package validate <path-to-plugin>` opnieuw uit.
...
De plugin importeert uit de verouderde hoofd-SDK-barrel:
`openclaw/plugin-sdk`.

- Vervang imports uit de hoofdbarrel door gerichte openbare subpadimports.
- Gebruik `openclaw/plugin-sdk/plugin-entry` voor `definePluginEntry`.
- Gebruik `openclaw/plugin-sdk/channel-core` voor helpers voor kanaalinvoerpunten.
...
De plugin gebruikt nog steeds de verouderde helper voor de hele sessiestore
`loadSessionStore`.
...
De plugin gebruikt nog steeds een verouderde schrijfhulp voor de hele sessiestore, zoals
...
- Vermijd het laden, muteren en opslaan van het volledige sessiestore-object.
- Houd schrijfhulpen voor de hele store alleen zolang je gedeclareerde compatibiliteitsbereik
...
- Leg geen verouderde transcriptbestandspaden vast en vertrouw er niet op.
- Zie [Runtime API](/nl/plugins/sdk-runtime#agent-session-state) en
  [Plugin SDK-subpaden](/nl/plugins/sdk-subpaths).
- Voer `clawhub package validate <path-to-plugin>` opnieuw uit.
...
- Gebruik `resolveSessionTranscriptTarget(...)` wanneer de code een gestructureerd
  doel voor transcriptbewerkingen nodig heeft.
- Vermijd het rechtstreeks lezen of construeren van verouderde transcriptbestandsdoelen.
...
- Spiegel env-gestuurde channelsetup naar de huidige setup, channelconfiguratie of
  channelmetadata van het pakket die door je pluginvorm wordt gebruikt.
