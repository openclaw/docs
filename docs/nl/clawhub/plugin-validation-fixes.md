...
read_when:
    ...
    - ClawHub heeft een publicatie van een Plugin-pakket geweigerd of hiervoor gewaarschuwd
    - Je werkt Plugin-pakketmetadata bij vóór de release
summary: Los ClawHub Plugin-pakketvalidatiebevindingen op vóór publicatie
title: Plugin-validatiefixes
x-i18n:
    generated_at: "2026-07-02T01:02:40Z"
    model: gpt-5.5
...
ClawHub valideert Plugin-pakketten voordat ze worden gepubliceerd en kan ook bevindingen tonen uit
...
scanneronderhoudscodes bevat zonder hersteladvies voor auteurs, zijn die
...
| `package-npm-pack-unavailable`          | [Maak het npm-artefact packable](/nl/clawhub/plugin-validation-fixes#package-npm-pack-unavailable)                                 |
...
| `sdk-load-session-store`                | [Vervang toegang tot de volledige session store](/nl/clawhub/plugin-validation-fixes#sdk-load-session-store)                                   |
| `sdk-session-store-write`               | [Vervang schrijfbewerkingen naar de volledige session store](/nl/clawhub/plugin-validation-fixes#sdk-session-store-write)                                  |
...
- Bouw het pakket als de entrypoint naar `dist` wordt gegenereerd.
...
- Los ongeldige pakketmetadata, kapotte lifecyclescripts of files-vermeldingen op die
...
- Gebruik `appendSessionTranscriptMessageByIdentity(...)` voor het toevoegen aan transcripts.
...
Het manifest gebruikt nog steeds legacy provider-authmetagegevens `providerAuthEnvVars`.

- Spiegel provider-env-varmetagegevens naar `setup.providers[].envVars`.
...
Het manifest gebruikt legacy of oudere channel-env-varmetagegevens zonder de huidige
setup- of configmetagegevens die ClawHub verwacht.

- Houd channel-env-varmetagegevens declaratief zodat OpenClaw de setupstatus kan inspecteren
  zonder de channel-runtime te laden.
- Spiegel env-gestuurde channelsetup naar de huidige setup-, channelconfig- of
  package-channelmetagegevens die door je pluginvorm worden gebruikt.
