...
read_when:
    ...
    - ClawHub heeft een Plugin-pakketpublicatie afgewezen of erover gewaarschuwd
...
x-i18n:
    generated_at: "2026-07-02T17:42:28Z"
    model: gpt-5.5
...
npm-pakket, de versie, toegangspunten of OpenClaw-metadata niet identificeren.
...
Het pakket declareert een OpenClaw-toegangspunt, maar het bestand waarnaar wordt verwezen ontbreekt
...
- Zorg ervoor dat `openclaw.plugin.json` is opgenomen wanneer het pakket een native
  OpenClaw-Plugin is.
...
Het Plugin-manifest heeft velden op topniveau die OpenClaw niet ondersteunt.
...
- Verplaats runtimegedrag naar de Plugin-registratiecode en beperk `contracts`
  tot statische eigendomsmetadata voor capabilities.
- Voer `clawhub package validate <path-to-plugin>` opnieuw uit.
...
- Gebruik [Importconventies](/nl/plugins/building-plugins#import-conventions) en
  [Plugin SDK-subpaths](/nl/plugins/sdk-subpaths) om de beperkte import te vinden.
- Voer `clawhub package validate <path-to-plugin>` opnieuw uit.
...
- Bewaar geen verouderde transcriptbestandspaden en wees er niet afhankelijk van.
- Zie [Runtime-API](/nl/plugins/sdk-runtime#agent-session-state) en
  [Plugin SDK-subpaths](/nl/plugins/sdk-subpaths).
- Voer `clawhub package validate <path-to-plugin>` opnieuw uit.
...
De Plugin gebruikt nog steeds de verouderde helper voor transcriptbestandstarget
...
- Vermijd het direct lezen of samenstellen van verouderde transcriptbestandstargets.
...
Het manifest gebruikt nog steeds verouderde `providerAuthEnvVars`-authmetadata voor providers.
