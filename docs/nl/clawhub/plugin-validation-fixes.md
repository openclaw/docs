...
read_when:
    ...
    - ClawHub weigerde of waarschuwde bij het publiceren van een Plugin-pakket
    - Je werkt Plugin-pakketmetadata bij vóór de release
summary: ClawHub Plugin-pakketvalidatiebevindingen oplossen vóór publicatie
title: Plugin-validatiefixes
x-i18n:
    generated_at: "2026-07-03T23:36:47Z"
    model: gpt-5.5
...
auteurs, wat betekent: bevindingen die de Plugin-auteur kan oplossen in
pakketmetadata, manifest, SDK-imports of gepubliceerd artefact.

Dit behandelt geen interne dekkingsbevindingen van Plugin Inspector. Als een
volledig rapport onderhoudscodes voor scanners bevat zonder herstelrichtlijnen
...
| `package-openclaw-entry-missing`        | [OpenClaw-pakkettoegangspunten declareren](/nl/clawhub/plugin-validation-fixes#package-openclaw-entry-missing)                         |
| `package-entrypoint-missing`            | [Het gedeclareerde toegangspunt publiceren](/nl/clawhub/plugin-validation-fixes#package-entrypoint-missing)                                  |
...
| `package-npm-pack-unavailable`          | [Het npm-artefact packbaar maken](/nl/clawhub/plugin-validation-fixes#package-npm-pack-unavailable)                                 |
| `package-npm-pack-entrypoint-missing`   | [Toegangspunten opnemen in npm-pack-uitvoer](/nl/clawhub/plugin-validation-fixes#package-npm-pack-entrypoint-missing)                  |
...
| `manifest-name-missing`                 | [Een manifestweergavenaam toevoegen](/nl/clawhub/plugin-validation-fixes#manifest-name-missing)                                           |
...
| `sdk-load-session-store`                | [Toegang tot de volledige sessiestore vervangen](/nl/clawhub/plugin-validation-fixes#sdk-load-session-store)                                   |
| `sdk-session-store-write`               | [Schrijfbewerkingen naar de volledige sessiestore vervangen](/nl/clawhub/plugin-validation-fixes#sdk-session-store-write)                                  |
...
| `sdk-session-transcript-file-target`    | [Verouderde transcriptiebestandsdoelen vervangen](/nl/clawhub/plugin-validation-fixes#sdk-session-transcript-file-target)                   |
| `sdk-session-transcript-low-level`      | [Low-level transcriptiehelpers vervangen](/nl/clawhub/plugin-validation-fixes#sdk-session-transcript-low-level)                       |
| `legacy-before-agent-start`             | [before_agent_start vervangen](/nl/clawhub/plugin-validation-fixes#legacy-before-agent-start)                                        |
| `provider-auth-env-vars`                | [Provider-env-vars verplaatsen naar setupmetadata](/nl/clawhub/plugin-validation-fixes#provider-auth-env-vars)                             |
...
De pakketmetadata bestaat, maar declareert geen OpenClaw-runtimetoegangspunt.
...
- Houd alle toegangspuntpaden binnen de pakketmap.
...
- Gebruik `openclaw.install.minHostVersion` voor de minimale
  OpenClaw-hostversie.
- Zie [package.json-velden die ontdekking beïnvloeden](/nl/plugins/manifest#packagejson-fields-that-affect-discovery).
- Voer `clawhub package validate <path-to-plugin>` opnieuw uit.
...
- Los ongeldige pakketmetadata, kapotte levenscyclusscripts of files-vermeldingen
...
- Voer `npm pack --dry-run` uit en inspecteer de bestanden die zouden worden
  opgenomen.
- Bouw gegenereerde toegangspunten vóór het packen.
- Werk `files`, `.npmignore` of de build-uitvoer bij zodat gedeclareerde
...
- Verplaats runtimegedrag naar Plugin-registratiecode en houd `contracts`
  beperkt tot statische metagegevens voor eigenaarschap van capabilities.
- Voer `clawhub package validate <path-to-plugin>` opnieuw uit.
...
- Gebruik `patchSessionEntry(...)` of `upsertSessionEntry(...)` om sessie-
  metagegevens te persisteren.
- Gebruik transcriptidentiteit of targethelpers wanneer de code een
  transcriptbewerking voorbereidt.
- Persisteer geen legacy transcriptbestandspaden en vertrouw er niet op.
- Zie [Runtime-API](/nl/plugins/sdk-runtime#agent-session-state) en
  [Plugin SDK-subpaths](/nl/plugins/sdk-subpaths).
- Voer `clawhub package validate <path-to-plugin>` opnieuw uit.
...
Het manifest gebruikt nog steeds legacy `providerAuthEnvVars`-providerauthmetagegevens.
...
- Spiegel env-gedreven channel-setup naar de huidige setup, channelconfiguratie of
  pakket-channelmetagegevens die door je Plugin-vorm worden gebruikt.
...
- Gebruik een gedocumenteerd geversioneerd schema pas nadat OpenClaw er een publiceert.
- Voer `clawhub package validate <path-to-plugin>` opnieuw uit.
