...
geautomatiseerde pakketscans. Deze pagina behandelt auteursgerichte bevindingen, wat
bevindingen betekent die de Plugin-auteur kan oplossen in pakketmetadata, manifest, SDK-
...
| `package-min-host-version-drift`        | [Stem minimale hostversie af](/nl/clawhub/plugin-validation-fixes#package-min-host-version-drift)                                   |
...
| `sdk-load-session-store`                | [Vervang toegang tot volledige session-store](/nl/clawhub/plugin-validation-fixes#sdk-load-session-store)                                   |
| `sdk-session-store-write`               | [Vervang schrijfacties naar volledige session-store](/nl/clawhub/plugin-validation-fixes#sdk-session-store-write)                                  |
| `sdk-session-file-helper`               | [Vervang helpers voor session-bestandspaden](/nl/clawhub/plugin-validation-fixes#sdk-session-file-helper)                                   |
| `sdk-session-transcript-file-target`    | [Vervang verouderde transcript-bestandsdoelen](/nl/clawhub/plugin-validation-fixes#sdk-session-transcript-file-target)                   |
...
De pakketroot bevat geen `package.json`, dus ClawHub kan het
npm-pakket, de versie, entrypoints of OpenClaw-metadata niet identificeren.
...
Het pakket declareert een OpenClaw-entrypoint, maar het genoemde bestand ontbreekt
...
- Corrigeer ongeldige pakketmetadata, kapotte lifecycle-scripts of files-vermeldingen die
  ervoor zorgen dat inpakken mislukt.
...
- Bouw gegenereerde entrypoints voordat je inpakt.
...
- Verplaats runtimegedrag naar de Plugin-registratiecode en houd `contracts`
  beperkt tot statische metagegevens over eigenaarschap van capabilities.
- Voer `clawhub package validate <path-to-plugin>` opnieuw uit.
...
Het manifest gebruikt nog steeds verouderde `providerAuthEnvVars`-metagegevens voor provider-authenticatie.

- Spiegel metagegevens van provider-env-vars naar `setup.providers[].envVars`.
...
Het manifest gebruikt verouderde of oudere channel-env-var-metagegevens zonder de huidige
...
- Spiegel env-gestuurde channel-setup naar de huidige setup, channel-config of
  package-channel-metagegevens die door je Plugin-vorm worden gebruikt.
...
- Gebruik alleen een gedocumenteerd versieschema nadat OpenClaw er een publiceert.
- Voer `clawhub package validate <path-to-plugin>` opnieuw uit.
...
- Houd beveiligingsgevoelig gedrag gedocumenteerd in je publieke package-docs of
