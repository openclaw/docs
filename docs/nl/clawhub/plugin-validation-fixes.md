...
geautomatiseerde pakketscans. Deze pagina behandelt auteurgerichte bevindingen, wat betekent:
...
voor OpenClaw-beheerders in plaats van Plugin-auteurs.
...
| `package-plugin-api-compat-missing`     | [Plugin-API-compatibiliteit declareren](/nl/clawhub/plugin-validation-fixes#package-plugin-api-compat-missing)                          |
...
| `sdk-load-session-store`                | [Toegang tot volledige sessiestore vervangen](/nl/clawhub/plugin-validation-fixes#sdk-load-session-store)                                   |
...
| `security-manifest-schema-unavailable`  | [Niet-beschikbare security-manifestschemaverwijzingen verwijderen](/nl/clawhub/plugin-validation-fixes#security-manifest-schema-unavailable) |
...
- Gebruik de OpenClaw Plugin-API-versie of semver-ondergrens waartegen u hebt gebouwd en getest.
...
- Verplaats runtimegedrag naar Plugin-registratiecode en houd `contracts`
  beperkt tot statische metagegevens voor eigenaarschap van mogelijkheden.
- Voer `clawhub package validate <path-to-plugin>` opnieuw uit.
...
Het manifest gebruikt nog steeds verouderde `providerAuthEnvVars`-metagegevens voor providerauthenticatie.
...
- Spiegel env-gestuurde channel-setup naar de huidige setup, channelconfiguratie of
  metagegevens voor pakket-channels die door de vorm van je plugin worden gebruikt.
