...
read_when:
    ...
    - Je werkt de pakketmetadata van de Plugin bij vóór release
...
x-i18n:
    generated_at: "2026-07-01T20:27:21Z"
    model: gpt-5.5
...
geautomatiseerde pakketscans tonen. Deze pagina behandelt auteursgerichte bevindingen, wat betekent
...
Het `package.json#openclaw`-blok bevat velden die niet worden ondersteund
als OpenClaw-pakketmetadata.
...
entrypointbestanden die zijn gedeclareerd in `package.json#openclaw`.
...
Het Plugin-manifest bevat top-level velden die OpenClaw niet ondersteunt.
...
- Verplaats runtimegedrag naar Plugin-registratiecode en beperk `contracts`
  tot statische metadata voor capability-eigenaarschap.
- Voer `clawhub package validate <path-to-plugin>` opnieuw uit.
...
- Sla geen verouderde transcriptbestandspaden op en vertrouw er niet op.
- Zie [Runtime API](/nl/plugins/sdk-runtime#agent-session-state) en
  [Plugin SDK-subpaden](/nl/plugins/sdk-subpaths).
- Voer `clawhub package validate <path-to-plugin>` opnieuw uit.
...
- Gebruik `resolveSessionTranscriptTarget(...)` wanneer de code een gestructureerd
  doel voor transcriptbewerking nodig heeft.
...
Het manifest gebruikt nog steeds legacy `providerAuthEnvVars`-provider-authmetadata.
