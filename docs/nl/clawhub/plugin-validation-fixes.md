...
read_when:
    ...
    - ClawHub heeft een publicatie van een Plugin-pakket geweigerd of ervoor gewaarschuwd
    - Je werkt de pakketmetadata van de Plugin bij vóór de release
summary: Herstel validatiebevindingen voor het ClawHub Plugin-pakket vóór publicatie
title: Plugin-validatiefixes
x-i18n:
    generated_at: "2026-07-03T02:52:10Z"
    model: gpt-5.5
...
ClawHub valideert pluginpakketten vóór publicatie en kan ook bevindingen uit
geautomatiseerde pakketscans tonen. Deze pagina behandelt bevindingen voor auteurs,
oftewel bevindingen die de pluginauteur kan oplossen in pakketmetadata, manifest,
SDK-imports of gepubliceerd artefact.

Dit behandelt geen interne dekkingsbevindingen van Plugin Inspector. Als een volledig
rapport scanneronderhoudscodes zonder hersteladvies voor auteurs bevat, zijn die
bedoeld voor OpenClaw-beheerders in plaats van pluginauteurs.
...
| `package-openclaw-entry-missing`        | [Declareer OpenClaw-pakketstartpunten](/nl/clawhub/plugin-validation-fixes#package-openclaw-entry-missing)                         |
| `package-entrypoint-missing`            | [Publiceer het gedeclareerde startpunt](/nl/clawhub/plugin-validation-fixes#package-entrypoint-missing)                                  |
...
| `package-npm-pack-unavailable`          | [Maak het npm-artefact pakbaar](/nl/clawhub/plugin-validation-fixes#package-npm-pack-unavailable)                                 |
| `package-npm-pack-entrypoint-missing`   | [Neem startpunten op in npm-pack-uitvoer](/nl/clawhub/plugin-validation-fixes#package-npm-pack-entrypoint-missing)                  |
...
| `legacy-root-sdk-import`                | [Vervang SDK-imports vanuit de root](/nl/clawhub/plugin-validation-fixes#legacy-root-sdk-import)                                             |
...
versie, startpunten of OpenClaw-metadata niet kan identificeren.
...
- Gebruik [Plugins bouwen](/nl/plugins/building-plugins) voor een minimaal
  pakketvoorbeeld en [Pluginmanifest](/nl/plugins/manifest#manifest-versus-packagejson)
...
- Neem startpuntmetadata op, zoals `openclaw.extensions` of
...
De pakketmetadata bestaat, maar declareert geen OpenClaw-runtime-startpunt.

- Voeg `openclaw.extensions` toe voor native pluginstartpunten.
- Voeg `openclaw.runtimeExtensions` toe wanneer het gepubliceerde pakket
  gebouwde JavaScript moet laden.
- Houd alle startpuntpaden binnen de pakketmap.
- Zie [Plugin-startpunten](/nl/plugins/sdk-entrypoints) en
  [package.json-velden die ontdekking beïnvloeden](/nl/plugins/manifest#packagejson-fields-that-affect-discovery).
- Voer `clawhub package validate <path-to-plugin>` opnieuw uit.

### package-entrypoint-missing

Het pakket declareert een OpenClaw-startpunt, maar het verwezen bestand ontbreekt
...
- Bouw het pakket als het startpunt naar `dist` wordt gegenereerd.
- Werk de metadata bij als het startpunt is verplaatst.
- Zie [Plugin-startpunten](/nl/plugins/sdk-entrypoints).
- Voer `clawhub package validate <path-to-plugin>` opnieuw uit.
...
De pakketversie en de pluginmanifestversie komen niet overeen.
...
- Houd pakketstartpunten, compatibiliteit, installatie, setup en catalogusmetadata
...
- Los ongeldige pakketmetadata, kapotte lifecycle-scripts of bestandsvermeldingen
  op die het verpakken laten mislukken.
...
startpuntbestanden die in `package.json#openclaw` zijn gedeclareerd.

- Voer `npm pack --dry-run` uit en inspecteer de bestanden die zouden worden
  opgenomen.
- Bouw gegenereerde startpunten vóór het verpakken.
- Werk `files`, `.npmignore` of builduitvoer bij zodat gedeclareerde startpunten
  worden opgenomen.
- Zie [Plugin-startpunten](/nl/plugins/sdk-entrypoints).
- Voer `clawhub package validate <path-to-plugin>` opnieuw uit.
...
- Zorg ervoor dat `openclaw.plugin.json` wordt opgenomen wanneer het pakket een
  native OpenClaw-plugin is.
...
- Verplaats runtimegedrag naar Pluginregistratiecode en houd `contracts`
  beperkt tot statische metadata over eigenaarschap van capabilities.
- Voer `clawhub package validate <path-to-plugin>` opnieuw uit.
...
- Gebruik `getSessionEntry(...)` om sessiemetadata te lezen op basis van agent- en sessie-
  identiteit.
...
De Plugin gebruikt nog steeds de verouderde `before_agent_start`-hook.
...
Het manifest gebruikt nog steeds verouderde `providerAuthEnvVars`-provider-authmetadata.
...
Het manifest gebruikt verouderde of oudere channel-env-var-metadata zonder de huidige
...
- Spiegel env-gedreven channel-setup naar de huidige setup, channelconfiguratie of
  pakket-channelmetadata die door je Pluginvorm wordt gebruikt.
...
- Gebruik alleen een gedocumenteerd versioned schema nadat OpenClaw er een publiceert.
- Voer `clawhub package validate <path-to-plugin>` opnieuw uit.
...
- Verwijder `openclaw.security.json` totdat OpenClaw een versioned beveiligings-
  manifestschema en ClawHub-gedrag documenteert.
- Houd beveiligingsgevoelig gedrag gedocumenteerd in je publieke pakketdocs of
