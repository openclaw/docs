...
read_when:
    ...
    - ClawHub heeft een Plugin-pakketpublicatie geweigerd of er een waarschuwing over gegeven
...
geautomatiseerde pakketscans. Deze pagina behandelt bevindingen voor auteurs, wat betekent
bevindingen die de Plugin-auteur kan oplossen in hun pakketmetadata, manifest, SDK-
imports of gepubliceerde artifact.
...
bedoeld voor OpenClaw-onderhouders in plaats van Plugin-auteurs.
...
| `package-npm-pack-entrypoint-missing`   | [Neem entrypoints op in de uitvoer van npm pack](/nl/clawhub/plugin-validation-fixes#package-npm-pack-entrypoint-missing)                  |
...
| `security-manifest-schema-unavailable`  | [Verwijder verwijzingen naar niet-beschikbare schema's voor beveiligingsmanifesten](/nl/clawhub/plugin-validation-fixes#security-manifest-schema-unavailable) |
...
- Los ongeldige pakketmetadata, kapotte lifecycle-scripts of files-items op die
  ervoor zorgen dat packen mislukt.
...
- Verplaats runtimegedrag naar Plugin-registratiecode en beperk `contracts`
  tot statische metagegevens voor eigenaarschap van capabilities.
- Voer `clawhub package validate <path-to-plugin>` opnieuw uit.
...
- Gebruik `getSessionEntry(...)` om sessiemetagegevens te lezen op basis van agent- en sessie-
  identiteit.
...
- Gebruik `publishSessionTranscriptUpdateByIdentity(...)` voor notificaties over transcriptupdates.
...
- Spiegel door env aangestuurde channel-setup naar de huidige setup, channelconfiguratie of
...
- Verwijder `openclaw.security.json` totdat OpenClaw een versiegebonden schema voor beveiligings-
  manifests en ClawHub-gedrag documenteert.
