...
x-i18n:
    generated_at: "2026-06-30T14:09:49Z"
    model: gpt-5.5
...
geautomatiseerde pakketscans. Deze pagina behandelt auteursgerichte bevindingen, dus
...
scanneronderhoudscodes bevat zonder herstelrichtlijnen voor auteurs, dan zijn die
...
| `sdk-load-session-store`                | [Vervang toegang tot volledige sessie-store](/nl/clawhub/plugin-validation-fixes#sdk-load-session-store)                                   |
| `sdk-session-store-write`               | [Vervang schrijfacties naar volledige sessie-store](/nl/clawhub/plugin-validation-fixes#sdk-session-store-write)                                  |
...
In het verpakte artefact ontbreekt OpenClaw-metadata die wel bestaat in je bronpakket.
...
- Verplaats runtimegedrag naar Plugin-registratiecode en houd `contracts`
  beperkt tot statische metadata voor eigendom van capabilities.
- Voer `clawhub package validate <path-to-plugin>` opnieuw uit.
...
- Verplaats werk voor model- of provider-override naar `before_model_resolve`.
...
- Spiegel metadata voor provider-env-vars naar `setup.providers[].envVars`.
...
- Spiegel door env aangestuurde kanaalsetup naar de huidige setup, kanaalconfig of
  pakketkanaalmetadata die door je Plugin-vorm wordt gebruikt.
