...
summary: Genereer video's via video_generate vanuit tekst-, afbeeldings- of videoverwijzingen voor 16 providerbackends
title: Videogeneratie
x-i18n:
    generated_at: "2026-05-05T06:20:12Z"
    model: gpt-5.5
    provider: openai
...
andere modelopties, invoermodi en functiesets. De agent kiest automatisch
de juiste provider op basis van je configuratie en beschikbare API-sleutels.

<Note>
De `video_generate`-tool verschijnt alleen wanneer ten minste één provider voor
videogeneratie beschikbaar is. Als je deze niet in je agenttools ziet, stel dan een
...
- `imageToVideo` — de aanvraag bevat één of meer referentieafbeeldingen.
- `videoToVideo` — de aanvraag bevat één of meer referentievideo’s.
...
2. De provider verwerkt de taak op de achtergrond (meestal 30 seconden tot enkele minuten, afhankelijk van de provider en resolutie; trage providers met wachtrijondersteuning kunnen doorlopen tot de geconfigureerde time-out).
...
4. De agent informeert de gebruiker en voegt de voltooide video toe. In groeps-/kanaalchats
   die zichtbare levering uitsluitend via de berichtentool gebruiken, geeft de agent het
   resultaat door via de berichtentool in plaats van dat OpenClaw het direct plaatst.
...
Gebruik `openclaw tasks list` of `openclaw tasks show <taskId>` om
voortgang vanuit de CLI te controleren.

Buiten agentuitvoeringen met sessieondersteuning (bijvoorbeeld directe toolaanroepen)
...
| xAI                   | `grok-imagine-video`           |   ✓   | 1 afbeelding als eerste frame of tot 7 `reference_image`s | 1 video                               | `XAI_API_KEY`                            |
...
| ComfyUI    |     ✓      |       ✓        |       —        | Niet in de gedeelde sweep; workflowspecifieke dekking bevindt zich bij Comfy-tests                                                       |
...
| Google     |     ✓      |       ✓        |       ✓        | `generate`, `imageToVideo`; gedeelde `videoToVideo` overgeslagen omdat de huidige buffergebaseerde Gemini/Veo-sweep die invoer niet accepteert |
| MiniMax    |     ✓      |       ✓        |       —        | `generate`, `imageToVideo`                                                                                                               |
| OpenAI     |     ✓      |       ✓        |       ✓        | `generate`, `imageToVideo`; gedeelde `videoToVideo` overgeslagen omdat dit org-/invoerpad momenteel provider-side toegang tot inpaint/remix nodig heeft |
...
bijbehorende referentielijst; fouten van één positie te veel of te weinig mislukken met een duidelijke fout.
...
<ParamField path="watermark" type="boolean">Schakel providerwatermerken in of uit wanneer dit wordt ondersteund.</ParamField>
...
<ParamField path="timeoutMs" type="number">Optionele time-out voor providerbewerking in milliseconden.</ParamField>
...
de dichtstbijzijnde door de provider ondersteunde waarde en vertaalt geometriehints
zoals grootte-naar-beeldverhouding opnieuw wanneer een terugvalprovider een ander
besturingsoppervlak biedt. Werkelijk niet-ondersteunde overrides worden naar beste vermogen
...
Sommige capaciteitscontroles worden toegepast op de terugvallaag in plaats van op de
toolgrens, zodat een aanvraag die de limieten van de primaire provider overschrijdt,
nog steeds op een geschikte terugval kan worden uitgevoerd:
...
- `maxDurationSeconds` van de actieve kandidaat ligt onder de aangevraagde `durationSeconds`
...
De eerste overslagreden in een aanvraag wordt gelogd op `warn`, zodat operators zien wanneer
hun primaire provider is gepasseerd; daaropvolgende overslagen worden gelogd op `debug` om
...
de expliciete items `model`, `primary` en `fallbacks` te gebruiken.
...
<AccordionGroup>
  ...
  <Accordion title="BytePlus (1.0)">
    ...
    dwingt 480p af), `camera_fixed` (booleaan).
  ...
  <Accordion title="BytePlus Seedance 1.5">
    ...
    `aspectRatio: "adaptive"` detecteert automatisch de verhouding op basis van de invoerafbeelding.
  ...
  <Accordion title="fal">
    Gebruikt een wachtrijgestuurde flow voor langlopende taken. OpenClaw wacht
    standaard tot 20 minuten voordat een lopende fal-wachtrijtaak als verlopen
    wordt behandeld. De meeste fal-videomodellen accepteren een enkele
    afbeeldingsreferentie. Seedance 2.0 reference-to-video-modellen accepteren
    maximaal 9 afbeeldingen, 3 video's en 3 audioreferenties, met maximaal
    12 referentiebestanden in totaal.
  </Accordion>
  <Accordion title="Google (Gemini / Veo)">
    Ondersteunt een afbeeldings- of videoreferentie. Verzoeken voor gegenereerde
    audio worden genegeerd met een waarschuwing op het Gemini API-pad, omdat die
    API de parameter `generateAudio` voor huidige Veo-videogeneratie weigert.
  </Accordion>
  <Accordion title="MiniMax">
    Alleen een enkele afbeeldingsreferentie. MiniMax accepteert resoluties
    `768P` en `1080P`; verzoeken zoals `720P` worden vóór verzending genormaliseerd
    naar de dichtstbijzijnde ondersteunde waarde.
  </Accordion>
  <Accordion title="OpenAI">
    Alleen de override `size` wordt doorgestuurd. Andere stijloverrides
  ...
  <Accordion title="OpenRouter">
    ...
    taak, pollt `polling_url` en downloadt `unsigned_urls` of het gedocumenteerde
    eindpunt voor taakinhoud. De meegeleverde standaard `google/veo-3.1-fast`
  ...
  <Accordion title="Runway">
    ...
    `runway/gen4_aleph`. Tekst-alleen-uitvoeringen bieden beeldverhoudingen
    `16:9` en `9:16`.
  </Accordion>
  ...
  <Accordion title="Vydra">
    ...
    die authenticatie laten vallen. `veo3` wordt alleen als tekst-naar-video
    meegeleverd; `kling` vereist een externe afbeeldings-URL.
  </Accordion>
  <Accordion title="xAI">
    Ondersteunt tekst-naar-video, afbeelding-naar-video met een enkele eerste
    frame-afbeelding, maximaal 7 `reference_image`-invoeren via xAI
    `reference_images`, en externe flows voor video bewerken/verlengen.
  </Accordion>
</AccordionGroup>

## Capaciteitsmodi voor providers
...
providerimplementaties moeten bij voorkeur expliciete modusblokken gebruiken:
...
standaard de voorkeur aan live/env-API-sleutels boven opgeslagen authenticatieprofielen,
en voert standaard een releaseveilige rooktest uit:
...
transformatiemodi uit te voeren die de gedeelde sweep veilig met lokale media
kan testen:
...
Tegenwoordig dekt de gedeelde live-lane voor `videoToVideo` alleen `runway`
wanneer je `runway/gen4_aleph` selecteert.
