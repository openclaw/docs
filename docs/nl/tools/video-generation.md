...
summary: Genereer video's via video_generate op basis van tekst-, afbeeldings- of videoreferenties met 16 backends van aanbieders
title: Videogeneratie
x-i18n:
    generated_at: "2026-05-06T09:38:24Z"
    model: gpt-5.5
    provider: openai
...
<Note>
De tool `video_generate` verschijnt alleen wanneer ten minste één videogeneratieprovider
...
- `imageToVideo` - verzoek bevat één of meer referentieafbeeldingen.
- `videoToVideo` - verzoek bevat één of meer referentievideo's.
...
2. De provider verwerkt de taak op de achtergrond (meestal 30 seconden tot enkele minuten, afhankelijk van de provider en resolutie; trage providers met wachtrijen kunnen tot de geconfigureerde time-out lopen).
...
4. De agent informeert de gebruiker en voegt de voltooide video toe. In groeps-/kanaalchats
   die zichtbare levering alleen via berichttools gebruiken, stuurt de agent het
   resultaat door via de berichttool in plaats van dat OpenClaw het rechtstreeks plaatst.
...
Buiten sessiegebonden agentruns (bijvoorbeeld directe toolaanroepen)
...
| `running`   | Provider is bezig met verwerken (meestal 30 seconden tot enkele minuten, afhankelijk van provider en resolutie). |
...
| xAI                   | `grok-imagine-video`            |  ✓   | 1 first-frame-afbeelding of tot 7 `reference_image`s | 1 video                                        | `XAI_API_KEY`                            |
...
Eén referentie-audio (pad of URL). Gebruikt voor achtergrondmuziek of
...
bijbehorende referentielijst; off-by-one-fouten mislukken met een duidelijke fout.
Gebruik een lege string om een slot niet in te stellen. Stel voor xAI elke afbeeldingsrol in op
...
<ParamField path="audio" type="boolean">
  Schakel gegenereerde audio in de uitvoer in wanneer dit wordt ondersteund. Onderscheiden van `audioRef*` (invoer).
</ParamField>
...
<ParamField path="model" type="string">Provider/model-override (bijv. `runway/gen4.5`).</ParamField>
...
zoals grootte-naar-beeldverhouding opnieuw toe wanneer een fallbackprovider een ander
bedieningsoppervlak biedt. Echt niet-ondersteunde overrides worden op best-effortbasis
...
- Referentie-audio-invoer **wijzigt niet** de opgeloste modus; deze wordt toegepast boven op
...
Sommige capaciteitscontroles worden toegepast in de fallbacklaag in plaats van aan de
toolgrens, zodat een verzoek dat de limieten van de primaire provider overschrijdt
nog steeds kan draaien op een geschikte fallback:
...
- `maxDurationSeconds` van actieve kandidaat ligt onder de aangevraagde `durationSeconds`
...
- Verzoek bevat `providerOptions` en de actieve kandidaat declareert expliciet
  een getypt `providerOptions`-schema → overgeslagen als aangeleverde sleutels
  ...
  passthrough). Een provider kan alle provideropties uitschakelen door
...
1. **`model`-toolparameter** - als de agent er een opgeeft in de aanroep.
...
<AccordionGroup>
  ...
  <Accordion title="fal">
    Gebruikt een wachtrij-ondersteunde flow voor langlopende taken. OpenClaw wacht standaard
    maximaal 20 minuten voordat een fal-wachtrijtaak die nog bezig is als verlopen wordt
    behandeld. De meeste fal-videomodellen
...
- `videoToVideo` wanneer `capabilities.videoToVideo.enabled` en de
  provider/het model buffer-ondersteunde lokale video-invoer in de gedeelde
  sweep accepteert.
