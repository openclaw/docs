...
- Lokale WebUI’s van gebruikers en lokale modelservers moeten waar nodig op de allowlist in het operatorproxybeleid worden gezet; OpenClaw stelt hiervoor geen algemene bypass voor het lokale netwerk beschikbaar.
- Gateway-control-plane-proxybypass is bewust beperkt tot `localhost` en letterlijke loopback-IP-URL’s. Gebruik `ws://127.0.0.1:18789`, `ws://[::1]:18789` of `ws://localhost:18789` voor lokale directe Gateway-control-plane-verbindingen; andere hostnamen worden gerouteerd zoals normaal hostnaamgebaseerd verkeer.
