...
Zie [Onboarding (CLI)](/nl/start/wizard) voor de korte gids.

## Wat de wizard doet

Lokale modus (standaard) begeleidt je door:
...
<Steps>
  <Step title="Bestaande configuratie detecteren">
    ...
    - Reset gebruikt `trash` en biedt bereiken:
      ...
      - Volledige reset (verwijdert ook werkruimte)
  ...
  <Step title="Gateway">
    ...
    - In wachtwoordmodus ondersteunt interactieve instelling ook opslag in platte tekst of SecretRef.
  ...
  <Step title="Daemon-installatie">
    ...
    - Linux en Windows via WSL2: systemd-gebruikerseenheid
      - Wizard probeert `loginctl enable-linger <user>` zodat de gateway actief blijft na uitloggen.
      - Kan om sudo vragen (schrijft `/var/lib/systemd/linger`); probeert het eerst zonder sudo.
    - Native Windows: eerst Scheduled Task
      - Als het maken van de taak wordt geweigerd, valt OpenClaw terug op een login-item per gebruiker in de Startup-map en start de gateway onmiddellijk.
      - Scheduled Tasks blijven de voorkeur hebben omdat ze betere supervisorstatus bieden.
  ...
  <Step title="Gezondheidscontrole">
    - Start gateway (indien nodig) en voert `openclaw health` uit.
    - `openclaw status --deep` voegt de live gateway-gezondheidsprobe toe aan statusuitvoer, inclusief kanaalprobes wanneer ondersteund.

  </Step>
  <Step title="Skills">
    - Leest beschikbare skills en controleert vereisten.
    - Laat je node manager kiezen: npm, pnpm of bun.
...
- Token als externe gateway-authenticatie vereist is (aanbevolen)

<Note>
- Als gateway alleen loopback is, gebruik dan SSH-tunneling of een tailnet.
- Discovery-hints:
...
<AccordionGroup>
  ...
  <Accordion title="OpenAI Code-abonnement (OAuth)">
    ...
    Stelt `agents.defaults.model` in op `openai/gpt-5.5` via de Codex-runtime wanneer model niet is ingesteld of al tot de OpenAI-familie behoort.
  ...
  <Accordion title="OpenAI API-sleutel">
    ...
    Stelt `agents.defaults.model` in op `openai/gpt-5.5` wanneer model niet is ingesteld, `openai/*` is, of verouderde Codex-modelrefs zijn.

  </Accordion>
  <Accordion title="xAI (Grok) OAuth">
    Browseraanmelding voor geschikte SuperGrok- of X Premium-accounts. Dit is het
    aanbevolen xAI-pad voor de meeste gebruikers. OpenClaw slaat het resulterende authenticatieprofiel op
    voor Grok-modellen, Grok `web_search`, `x_search` en `code_execution`.
  </Accordion>
  ...
  <Accordion title="OpenCode">
    ...
    Instellings-URL: [opencode.ai/auth](https://opencode.ai/auth).
  </Accordion>
  ...
  <Accordion title="MiniMax">
    Configuratie wordt automatisch geschreven. Gehoste standaard is `MiniMax-M3`; API-sleutelinstelling gebruikt
  ...
  <Accordion title="Moonshot en Kimi Coding">
    Moonshot-configuraties (Kimi K2) en Kimi Coding-configuraties worden automatisch geschreven.
...
- Onboarding voor aangepaste providers leidt beeldondersteuning af voor veelvoorkomende model-ID's en vraagt alleen wanneer de modelnaam onbekend is.
...
- Wizard voert een modelcontrole uit en waarschuwt als het geconfigureerde model onbekend is of authenticatie mist.
...
- `--secret-input-mode ref` schakelt referentiemodus in in plaats van opslag van sleutels in platte tekst.
  ...
  - omgevingsvariabelereferentie (bijvoorbeeld `keyRef: { source: "env", provider: "default", id: "OPENAI_API_KEY" }`)
  - geconfigureerde providerreferentie (`file` of `exec`) met provideralias + id
- Interactieve referentiemodus voert een snelle preflightvalidatie uit voordat er wordt opgeslagen.
  - Env-referenties: valideert variabelenaam + niet-lege waarde in de huidige onboardingomgeving.
  - Providerreferenties: valideert providerconfiguratie en lost het aangevraagde id op.
...
- In niet-interactieve modus wordt `--secret-input-mode ref` alleen door env ondersteund.
  - Stel de provideromgevingsvariabele in de procesomgeving voor onboarding in.
  - Inline sleutelvlaggen (bijvoorbeeld `--openai-api-key`) vereisen dat die omgevingsvariabele is ingesteld; anders mislukt onboarding snel.
  ...
  - In dat geval met een aangepaste provider vereist `--custom-api-key` dat `CUSTOM_API_KEY` is ingesteld; anders mislukt onboarding snel.
- Gateway-authreferenties ondersteunen keuzes voor platte tekst en SecretRef in interactieve configuratie:
...
- `gateway.*` (modus, binding, auth, tailscale)
...
- Kanaaltoelatingslijsten (Slack, Discord, Matrix, Microsoft Teams) wanneer je je tijdens prompts aanmeldt (namen worden waar mogelijk naar id's omgezet)
...
<Note>
Sommige kanalen worden als plugins geleverd. Wanneer ze tijdens de configuratie worden geselecteerd, vraagt de wizard
om de Plugin (npm of lokaal pad) te installeren voordat de kanaalconfiguratie begint.
</Note>
...
Gedrag van Signal-configuratie:
