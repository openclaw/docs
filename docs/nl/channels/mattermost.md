...
read_when:
    ...
    - Foutopsporing voor Mattermost-routering
...
1. Bijlagen gaan in `props.attachments`, niet in `attachments` op topniveau (wordt stilzwijgend genegeerd).
...
4. Actie-`id` mag **alleen alfanumeriek** zijn (`[a-zA-Z0-9]`). Koppeltekens en underscores verbreken de server-side actieroutering van Mattermost (retourneert 404). Verwijder ze vóór gebruik.
5. `context.action_id` moet overeenkomen met de `id` van de knop, zodat het bevestigingsbericht de knopnaam toont (bijv. "Approve") in plaats van een onbewerkte ID.
