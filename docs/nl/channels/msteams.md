...
Microsoft Teams wordt in huidige OpenClaw-releases meegeleverd als gebundelde Plugin, dus in de normale verpakte build is geen
aparte installatie vereist.

Als je een oudere build gebruikt of een aangepaste installatie die gebundelde Teams uitsluit,
installeer dan het npm-pakket direct:
...
De [`@microsoft/teams.cli`](https://www.npmjs.com/package/@microsoft/teams.cli) verzorgt botregistratie, manifestcreatie en het genereren van referenties in één opdracht.
...
De uitvoer toont `CLIENT_ID`, `CLIENT_SECRET`, `TENANT_ID` en een **Teams App ID** — noteer deze voor de volgende stappen. De opdracht biedt ook aan om de app direct in Teams te installeren.
...
Voor productie-implementaties kun je overwegen [gefedereerde authenticatie](/nl/channels/msteams#federated-authentication-certificate-plus-managed-identity) (certificaat of beheerde identiteit) te gebruiken in plaats van clientgeheimen.
...
- De wizard kan namen via Microsoft Graph naar ID's herleiden wanneer referenties dat toestaan.
...
- De configuratiewizard accepteert `Team/Channel`-vermeldingen en slaat die voor je op.
- Bij het opstarten herleidt OpenClaw team-/kanaal- en gebruikers-allowlistnamen naar ID's (wanneer Graph-machtigingen dit toestaan)
  en logt de mapping; onopgeloste team-/kanaalnamen blijven bewaard zoals ingevoerd, maar worden standaard genegeerd voor routering tenzij `channels.msteams.dangerouslyAllowNameMatching: true` is ingeschakeld.
...
Als je de Teams CLI niet kunt gebruiken, kun je de bot handmatig configureren via de Azure Portal.
...
2. Vul het tabblad **Basics** in:
   ...
   | **Resource group** | Maak een nieuwe aan of gebruik een bestaande                  |
...
Het Teams-kanaal start automatisch wanneer de Plugin beschikbaar is en er `msteams`-configuratie met referenties bestaat.
...
**Config (door gebruiker toegewezen beheerde identiteit):**
...
| Methode                 | Config                                         | Voordelen                                 | Nadelen                                           |
| ----------------------- | ---------------------------------------------- | ----------------------------------------- | ------------------------------------------------- |
| **Clientgeheim**        | `appPassword`                                  | Eenvoudige configuratie                   | Geheimrotatie vereist, minder veilig              |
| **Certificaat**         | `authType: "federated"` + `certificatePath`    | Geen gedeeld geheim via het netwerk       | Extra beheerlast voor certificaten                |
| **Beheerde identiteit** | `authType: "federated"` + `useManagedIdentity` | Zonder wachtwoord, geen geheimen beheren | Azure-infrastructuur vereist                      |
...
Controleert botregistratie, AAD-app, manifest en SSO-configuratie in één ronde.
...
Alle config-sleutels kunnen in plaats daarvan via omgevingsvariabelen worden ingesteld:
...
OpenClaw biedt een door Graph ondersteunde actie `member-info` voor Microsoft Teams, zodat agents en automatiseringen gegevens van kanaalleden (weergavenaam, e-mail, rol) rechtstreeks vanuit Microsoft Graph kunnen opzoeken.

Vereisten:

- RSC-machtiging `Member.Read.Group` (al opgenomen in het aanbevolen manifest)
- Voor lookups tussen teams: Graph Application-machtiging `User.Read.All` met beheerderstoestemming
...
# Versie wordt automatisch verhoogd als de inhoud is gewijzigd
```
...
- Bestandsbijlagen ontvangen in **persoonlijk (DM)**.
...
| **Complexiteit setup**   | Alleen appmanifest     | Vereist beheerderstoestemming + tokenstroom    |
...
## Media en geschiedenis met Graph ingeschakeld (vereist voor kanalen)
...
Teams levert berichten via een HTTP-Webhook. Als verwerking te lang duurt (bijvoorbeeld trage LLM-antwoorden), kun je het volgende zien:

- Gateway-time-outs
- Teams probeert het bericht opnieuw te verzenden (waardoor duplicaten ontstaan)
- Weggevallen antwoorden

OpenClaw handelt dit af door snel te retourneren en proactief antwoorden te sturen, maar zeer trage reacties kunnen nog steeds problemen veroorzaken.
...
- `channels.msteams.chunkMode`: `length` (standaard) of `newline` om te splitsen op lege regels (alineagrenzen) voordat op lengte wordt gechunkt.
- `channels.msteams.mediaAllowHosts`: allowlist voor hosts van inkomende bijlagen (standaard Microsoft-/Teams-domeinen).
- `channels.msteams.mediaAuthAllowHosts`: allowlist voor het toevoegen van Authorization-headers bij mediapogingen (standaard Graph- en Bot Framework-hosts).
...
- `thread` in een Threads-stijlkanaal → antwoorden verschijnen ongemakkelijk genest
- `top-level` in een Posts-stijlkanaal → antwoorden verschijnen als afzonderlijke top-level posts in plaats van in-thread
...
Authorization-headers worden alleen toegevoegd voor hosts in `channels.msteams.mediaAuthAllowHosts` (standaard Graph- en Bot Framework-hosts). Houd deze lijst strikt (vermijd multi-tenant achtervoegsels).
...
| ------------------------ | ------------------------------------------- | ---------------------------------------------- |
| **DM's**                 | FileConsentCard → gebruiker accepteert → bot uploadt | Werkt zonder extra configuratie                |
| **Groepschats/kanalen**  | Uploaden naar SharePoint → link delen       | Vereist `sharePointSiteId` + Graph-machtigingen |
| **Afbeeldingen (elke context)** | Base64-gecodeerd inline                    | Werkt zonder extra configuratie                |

### Waarom groepschats SharePoint nodig hebben

Bots hebben geen persoonlijke OneDrive-schijf (het `/me/drive` Graph API-eindpunt werkt niet voor applicatie-identiteiten). Om bestanden in groepschats/kanalen te verzenden, uploadt de bot naar een **SharePoint-site** en maakt een deellink aan.
...
- Peilingen plaatsen nog niet automatisch resultaatsamenvattingen (inspecteer zo nodig het opslagbestand).

## Presentatiekaarten

Verzend semantische presentatiepayloads naar Teams-gebruikers of -gesprekken met de tool `message` of de CLI. OpenClaw geeft ze weer als Teams Adaptive Cards op basis van het generieke presentatiecontract.
...
<Note>
Zonder het voorvoegsel `user:` worden namen standaard omgezet via groeps- of teamresolutie. Gebruik altijd `user:` wanneer je personen target op weergavenaam.
</Note>
...
- **Afbeeldingen worden niet weergegeven in kanalen:** Graph-machtigingen of beheerdersgoedkeuring ontbreken. Installeer de Teams-app opnieuw en sluit Teams volledig af en open het opnieuw.
- **Geen reacties in kanaal:** mentions zijn standaard vereist; stel `channels.msteams.requireMention=false` in of configureer dit per team/kanaal.
...
- **"Something went wrong" bij uploaden:** Upload in plaats daarvan via [https://admin.teams.microsoft.com](https://admin.teams.microsoft.com), open de browser-DevTools (F12) → tabblad Network en controleer de response body op de daadwerkelijke fout.
...
- [Naslag voor RSC-machtigingen](https://learn.microsoft.com/en-us/microsoftteams/platform/graph-api/rsc/resource-specific-consent)
