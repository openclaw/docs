...
Microsoft Teams wordt als gebundelde Plugin meegeleverd in huidige OpenClaw-releases, dus in de normale verpakte build is geen
...
Gebruik het kale pakket om de huidige officiele releasetag te volgen. Pin alleen een exacte
...
De [`@microsoft/teams.cli`](https://www.npmjs.com/package/@microsoft/teams.cli) verwerkt botregistratie, manifestaanmaak en credentialgeneratie in een enkele opdracht.
...
**4. Configureer OpenClaw** met de credentials uit de uitvoer:
...
Dit voert diagnostiek uit over botregistratie, AAD-appconfiguratie, manifestgeldigheid en SSO-setup.
...
<Note>
Groepschats worden standaard geblokkeerd (`channels.msteams.groupPolicy: "allowlist"`). Stel `channels.msteams.groupAllowFrom` in om groepsantwoorden toe te staan, of gebruik `groupPolicy: "open"` om elk lid toe te staan (met vermeldingspoort).
</Note>
...
- Vertrouw niet op UPN-/weergavenaam-matching voor allowlists - die kunnen veranderen. OpenClaw schakelt directe naammatching standaard uit; meld je expliciet aan met `channels.msteams.dangerouslyAllowNameMatching: true`.
- De wizard kan namen via Microsoft Graph naar ID's omzetten wanneer credentials dat toestaan.
...
- `channels.msteams.groupAllowFrom` bepaalt welke afzenders of statische afzendertoegangsgroepen in groepschats/kanalen kunnen activeren (valt terug op `channels.msteams.allowFrom`).
- Stel `groupPolicy: "open"` in om elk lid toe te staan (standaard nog steeds met vermeldingspoort).
...
- Wanneer `groupPolicy="allowlist"` en een teams-allowlist aanwezig is, worden alleen vermelde teams/kanalen geaccepteerd (met vermeldingspoort).
- De configuratiewizard accepteert `Team/Channel`-items en slaat ze voor je op.
- Bij het opstarten zet OpenClaw team-/kanaal- en gebruikers-allowlistnamen om naar ID's (wanneer Graph-machtigingen dat toestaan)
  en logt de mapping; niet-opgeloste team-/kanaalnamen worden bewaard zoals getypt, maar standaard genegeerd voor routering tenzij `channels.msteams.dangerouslyAllowNameMatching: true` is ingeschakeld.
...
2. Vul het tabblad **Basics** in:
   ...
   | **Resource group** | Maak nieuw aan of gebruik bestaand                       |
   | **Pricing tier**   | **Free** voor dev/testen                                 |
...
### Stap 3: Configureer het Messaging-eindpunt
...
### Stap 4: Schakel het Teams-kanaal in
...
| **Certificaat**      | `authType: "federated"` + `certificatePath`    | Geen gedeeld geheim via het netwerk | Beheeroverhead voor certificaten         |
| **Managed Identity** | `authType: "federated"` + `useManagedIdentity` | Zonder wachtwoord, geen te beheren geheimen | Azure-infrastructuur vereist      |
...
Alternatieven: `ngrok http 3978` of `tailscale funnel 3978` (URL's kunnen per sessie wijzigen).
...
- `MSTEAMS_USE_MANAGED_IDENTITY` (federatief + managed identity)
- `MSTEAMS_MANAGED_IDENTITY_CLIENT_ID` (alleen door gebruiker toegewezen MI)

## Actie voor lidgegevens

OpenClaw biedt een door Graph ondersteunde actie `member-info` voor Microsoft Teams, zodat agents en automatiseringen kanaalledengegevens (weergavenaam, e-mail, rol) rechtstreeks vanuit Microsoft Graph kunnen ophalen.

Vereisten:

- RSC-machtiging `Member.Read.Group` (staat al in het aanbevolen manifest)
- Voor zoekacties tussen teams: Graph-toepassingsmachtiging `User.Read.All` met beheerderstoestemming

De actie wordt afgeschermd door `channels.msteams.actions.memberInfo` (standaard: ingeschakeld wanneer Graph-referenties beschikbaar zijn).
...
- Opgehaalde threadgeschiedenis wordt gefilterd op afzender-toelatingslijsten (`allowFrom` / `groupAllowFrom`), zodat seedcontext voor threads alleen berichten van toegestane afzenders bevat.
- Geciteerde bijlagecontext (`ReplyTo*` afgeleid uit Teams-antwoord-HTML) wordt momenteel ongewijzigd doorgegeven.
- Met andere woorden: toelatingslijsten bepalen wie de agent kan activeren; vandaag worden alleen specifieke aanvullende contextpaden gefilterd.
...
Nadat je hebt bijgewerkt, installeer je de app opnieuw in elk team zodat nieuwe machtigingen van kracht worden, en **sluit Teams volledig af en start het opnieuw** (niet alleen het venster sluiten) om gecachte appmetadata te wissen.
...
| **Complexiteit van instellen** | Alleen appmanifest | Vereist beheerderstoestemming + tokenflow |
| **Werkt offline**        | Nee (moet actief zijn) | Ja (op elk moment opvragen)       |

**Kort gezegd:** RSC is voor realtime luisteren; Graph API is voor historische toegang. Om gemiste berichten in te halen terwijl je offline was, heb je Graph API nodig met `ChannelMessage.Read.All` (vereist beheerderstoestemming).
...
1. Voeg in Entra ID (Azure AD) **App Registration** Microsoft Graph-**toepassingsmachtigingen** toe:
...
OpenClaw handelt dit af door snel terug te keren en proactief antwoorden te verzenden, maar zeer trage reacties kunnen nog steeds problemen veroorzaken.

### Opmaak

Teams-markdown is beperkter dan Slack of Discord:

- Basisopmaak werkt: **bold**, _italic_, `code`, links
...
- `channels.msteams.dangerouslyAllowNameMatching`: break-glass-schakelaar om veranderlijke UPN-/weergavenaamkoppeling en directe routering op team-/kanaalnaam opnieuw in te schakelen.
- `channels.msteams.textChunkLimit`: segmentgrootte voor uitgaande tekst.
- `channels.msteams.chunkMode`: `length` (standaard) of `newline` om op lege regels (alinea-grenzen) te splitsen voordat op lengte wordt gesegmenteerd.
...
- `channels.msteams.teams.<teamId>.tools`: standaard overrides per team voor toolbeleid (`allow`/`deny`/`alsoAllow`) gebruikt wanneer een kanaaloverride ontbreekt.
...
Teams heeft onlangs twee kanaal-UI-stijlen geïntroduceerd bovenop hetzelfde onderliggende gegevensmodel:
...
- `top-level` in een kanaal met Posts-stijl → antwoorden verschijnen als afzonderlijke top-level posts in plaats van in-thread
...
Wanneer de bot een antwoord naar een kanaal verzendt, wordt `replyStyle` bepaald vanaf de meest specifieke override tot aan de standaard. De eerste niet-`undefined` waarde wint:
...
Als je `requireMention: false` globaal instelt zonder expliciete `replyStyle`, verschijnen vermeldingen in kanalen met Posts-stijl als top-level posts, zelfs wanneer de inkomende boodschap een threadantwoord was. Zet `replyStyle: "thread"` vast op globaal, team- of kanaalniveau om verrassingen te voorkomen.

### Behoud van threadcontext

Wanneer `replyStyle: "thread"` actief is en de bot vanuit een kanaalthread is @vermeld, koppelt OpenClaw de oorspronkelijke thread-root opnieuw aan de uitgaande gespreksreferentie (`19:…@thread.tacv2;messageid=<root>`), zodat het antwoord in dezelfde thread terechtkomt. Dit geldt voor zowel live (in-turn) sends als proactieve sends die worden gedaan nadat de Bot Framework-turncontext is verlopen (bijv. langlopende agents, in de wachtrij geplaatste tool-call-antwoorden via `mcp__openclaw__message`).

De thread-root wordt genomen uit de opgeslagen `threadId` op de gespreksreferentie. Oudere opgeslagen referenties van vóór `threadId` vallen terug op `activityId` (welke inkomende activiteit het gesprek als laatste heeft gezaaid), zodat bestaande implementaties blijven werken zonder opnieuw te seeden.

Wanneer `replyStyle: "top-level"` actief is, worden inkomende kanaalthreads bewust beantwoord als nieuwe top-level posts — er wordt geen threadsuffix toegevoegd. Dit is het juiste gedrag voor kanalen met Threads-stijl; als je top-level posts ziet waar je threaded antwoorden verwachtte, is je `replyStyle` voor dat kanaal verkeerd ingesteld.
...
Bots kunnen bestanden in DM's verzenden met de FileConsentCard-flow (ingebouwd). **Bestanden verzenden in groepschats/kanalen** vereist echter aanvullende installatie:
...
- Peilingen plaatsen nog niet automatisch samenvattingen van resultaten (controleer indien nodig het opslagbestand).

## Presentatiekaarten

Stuur semantische presentatiepayloads naar Teams-gebruikers of -gesprekken met de `message`-tool of CLI. OpenClaw rendert ze als Teams Adaptive Cards op basis van het generieke presentatiecontract.
...
<Note>
Zonder het voorvoegsel `user:` worden namen standaard verwerkt als groep- of teamresolutie. Gebruik altijd `user:` wanneer je personen op weergavenaam target.
</Note>
...
- **Negeer** de queryparameter `groupId` voor OpenClaw-routering. Dit is de Microsoft Entra-groeps-ID, niet de Bot Framework-gespreks-ID die wordt gebruikt in binnenkomende Teams-activiteiten.
...
2. Gebruik DM's - gebruikers kunnen de bot altijd rechtstreeks berichten
...
- **Afbeeldingen worden niet getoond in kanalen:** Graph-machtigingen of beheerdersgoedkeuring ontbreken. Installeer de Teams-app opnieuw en sluit Teams volledig af en open het opnieuw.
...
- **Versiemismatch (Teams toont nog steeds oud manifest):** verwijder de app en voeg deze opnieuw toe, en sluit Teams volledig af om te vernieuwen.
- **401 Unauthorized van Webhook:** verwacht bij handmatig testen zonder Azure JWT - dit betekent dat het endpoint bereikbaar is maar authenticatie is mislukt. Gebruik Azure Web Chat om correct te testen.
...
- **"webApplicationInfo.Id already in use":** De app is nog geïnstalleerd in een ander team/chat. Zoek deze eerst en verwijder de installatie, of wacht 5-10 minuten op propagatie.
- **"Something went wrong" bij uploaden:** Upload in plaats daarvan via [https://admin.teams.microsoft.com](https://admin.teams.microsoft.com), open browser-DevTools (F12) → tabblad Network, en controleer de responsebody op de daadwerkelijke fout.
