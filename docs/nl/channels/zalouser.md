...
read_when:
    ...
    - Fouten opsporen bij de aanmelding of berichtenstroom van Zalo Personal
...
5. DM-toegang gebruikt standaard koppeling; keur de koppelcode goed bij het eerste contact.
...
`channels.zalouser.allowFrom` moet stabiele Zalo-gebruikers-ID's gebruiken. Tijdens interactieve installatie kunnen ingevoerde namen worden omgezet naar ID's met de in-process contact lookup van de Plugin.

Als een ruwe naam in de configuratie blijft staan, wordt deze bij het opstarten alleen omgezet wanneer `channels.zalouser.dangerouslyAllowNameMatching: true` is ingeschakeld. Zonder die opt-in zijn afzendercontroles tijdens runtime alleen op ID gebaseerd en worden ruwe namen genegeerd voor autorisatie.
...
- Beperk tot een allowlist met:
  ...
  - `channels.zalouser.groups` (sleutels moeten stabiele groeps-ID's zijn; namen worden bij het opstarten alleen omgezet naar ID's wanneer `channels.zalouser.dangerouslyAllowNameMatching: true` is ingeschakeld)
...
- De configureerwizard kan vragen om groeps-allowlists.
...
- `channels.zalouser.dangerouslyAllowNameMatching: true` is een break-glass-compatibiliteitsmodus die veranderlijke naamomzetting bij het opstarten en runtime matching van groepsnamen opnieuw inschakelt.
- Als `groupAllowFrom` niet is ingesteld, valt runtime terug op `allowFrom` voor controles van groepsafzenders.
- Afzendercontroles gelden voor zowel normale groepsberichten als besturingsopdrachten (bijvoorbeeld `/new`, `/reset`).
...
- Volgorde van omzetting: exacte groeps-id/naam -> genormaliseerde groeps-slug -> `*` -> standaard (`true`).
...
- Geautoriseerde besturingsopdrachten (bijvoorbeeld `/new`) kunnen vermelding-gating omzeilen.
- Wanneer een groepsbericht wordt overgeslagen omdat een vermelding vereist is, slaat OpenClaw het op als wachtende groepsgeschiedenis en neemt het dit op in het volgende verwerkte groepsbericht.
- De limiet voor groepsgeschiedenis gebruikt standaard `messages.groupChat.historyLimit` (fallback `50`). Je kunt dit per account overschrijven met `channels.zalouser.historyLimit`.
...
- OpenClaw verzendt een typing event voordat een antwoord wordt verzonden (best-effort).
...
- Voor inkomende berichten die event metadata bevatten, verzendt OpenClaw afgeleverd- en gezien-bevestigingen (best-effort).
...
- Verwijder eventuele oude aannames over externe `zca`-processen.
