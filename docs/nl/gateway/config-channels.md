...
read_when:
    ...
    - DM-beleid, groepsbeleid of vermeldingsfilters auditen
...
set-ups met meerdere accounts, vermeldingsgatekeeping en sleutels per kanaal voor Slack, Discord,
...
| DM-beleid           | Gedrag                                                        |
| ------------------- | --------------------------------------------------------------- |
| `pairing` (default) | Onbekende afzenders krijgen een eenmalige koppelingscode; eigenaar moet goedkeuren |
...
| `open`                | Omzeil groeps-allowlists (vermeldingsgatekeeping blijft van toepassing) |
...
- `channels.defaults.groupPolicy`: terugval-groepsbeleid wanneer een `groupPolicy` op providerniveau niet is ingesteld.
- `channels.defaults.contextVisibility`: standaardmodus voor zichtbaarheid van aanvullende context voor alle kanalen. Waarden: `all` (standaard, neemt alle geciteerde/thread-/geschiedeniscontext op), `allowlist` (neemt alleen context op van afzenders op de allowlist), `allowlist_quote` (hetzelfde als allowlist maar behoudt expliciete citaat-/antwoordcontext). Overschrijving per kanaal: `channels.<channel>.contextVisibility`.
...
- Optionele `channels.whatsapp.defaultAccount` overschrijft die terugvalkeuze voor het standaardaccount wanneer deze overeenkomt met een geconfigureerde account-ID.
...
- `apiRoot` is alleen de root van de Telegram Bot API. Gebruik `https://api.telegram.org` of je zelfgehoste/proxy-root, niet `https://api.telegram.org/bot<TOKEN>`; `openclaw doctor --fix` verwijdert een per ongeluk achtergevoegd `/bot<TOKEN>`-suffix.
- Optionele `channels.telegram.defaultAccount` overschrijft de standaard accountkeuze wanneer deze overeenkomt met een geconfigureerde account-ID.
...
- Telegram-streamvoorvertoningen gebruiken `sendMessage` + `editMessageText` (werkt in directe chats en groepschats).
...
- Rechtstreekse uitgaande aanroepen die een expliciete Discord-`token` opgeven, gebruiken die token voor de aanroep; instellingen voor opnieuw proberen/beleid van het account komen nog steeds uit het geselecteerde account in de actieve runtime-snapshot.
...
- Guild-slugs zijn kleine letters waarbij spaties worden vervangen door `-`; kanaalsleutels gebruiken de geslugde naam (zonder `#`). Geef de voorkeur aan guild-ID's.
- Berichten die door bots zijn gemaakt, worden standaard genegeerd. `allowBots: true` schakelt ze in; gebruik `allowBots: "mentions"` om alleen botberichten te accepteren die de bot vermelden (eigen berichten blijven gefilterd).
...
- `maxLinesPerMessage` (standaard 17) splitst lange berichten, zelfs wanneer ze onder de 2000 tekens blijven.
- `channels.discord.threadBindings` beheert thread-gebonden routering voor Discord:
  - `enabled`: Discord-overschrijving voor thread-gebonden sessiefuncties (`/focus`, `/unfocus`, `/agents`, `/session idle`, `/session max-age`, en gebonden aflevering/routering)
  ...
  - `spawnSessions`: schakelaar voor `sessions_spawn({ thread: true })` en automatische thread-aanmaak/binding bij ACP-threadspawn (standaard: `true`)
...
- `channels.discord.voice` schakelt Discord-spraakkanaalgesprekken en optionele overschrijvingen voor automatisch deelnemen + LLM + TTS in. Discord-configuraties met alleen tekst laten spraak standaard uit; stel `channels.discord.voice.enabled=true` in om deel te nemen.
...
- `channels.discord.voice.reconnectGraceMs` beheert hoelang een verbroken spraaksessie de tijd mag nemen om reconnect-signalering binnen te gaan voordat OpenClaw deze vernietigt (standaard `15000`).
- OpenClaw probeert daarnaast spraakontvangst te herstellen door een spraaksessie te verlaten en opnieuw te betreden na herhaalde decryptiefouten.
- `channels.discord.streaming` is de canonieke streammodussleutel. Verouderde waarden `streamMode` en booleaanse `streaming` blijven runtime-aliassen; voer `openclaw doctor --fix` uit om persistente configuratie te herschrijven.
- `channels.discord.autoPresence` koppelt runtimebeschikbaarheid aan botaanwezigheid (gezond => online, gedegradeerd => idle, uitgeput => dnd) en staat optionele overschrijvingen van statustekst toe.
- `channels.discord.dangerouslyAllowNameMatching` schakelt veranderlijke naam-/tagmatching opnieuw in (compatibiliteitsmodus voor noodsituaties).
...
- `channels.googlechat.dangerouslyAllowNameMatching` schakelt veranderlijke matching van e-mailprincipals opnieuw in (compatibiliteitsmodus voor noodsituaties).
...
- **Socketmodus** vereist zowel `botToken` als `appToken` (`SLACK_BOT_TOKEN` + `SLACK_APP_TOKEN` als env-fallback voor het standaardaccount).
- **HTTP-modus** vereist `botToken` plus `signingSecret` (op rootniveau of per account).
- `socketMode` geeft transportafstemming voor Slack SDK Socket Mode door aan de publieke Bolt receiver-API. Gebruik dit alleen bij onderzoek naar ping/pong-timeouts of verouderd websocketgedrag.
- `botToken`, `appToken`, `signingSecret` en `userToken` accepteren plaintext
  strings of SecretRef-objecten.
- Slack-account-snapshots tonen bron-/statusvelden per credential, zoals
...
- `channels.slack.streaming.mode` is de canonieke Slack-streammodussleutel. `channels.slack.streaming.nativeTransport` beheert Slack's native streamingtransport. Verouderde waarden `streamMode`, booleaanse `streaming` en `nativeStreaming` blijven runtime-aliassen; voer `openclaw doctor --fix` uit om persistente configuratie te herschrijven.
...
- Slack native streaming plus de Slack-assistentstijl-threadstatus "is typing..." vereisen een antwoordthreaddoel. Top-level DM's blijven standaard buiten threads, zodat ze nog steeds kunnen streamen via Slack-conceptberichten met plaatsen-en-bewerken-previews in plaats van de threadstijl native stream-/statuspreview te tonen.
- `typingReaction` voegt een tijdelijke reactie toe aan het binnenkomende Slack-bericht terwijl een antwoord wordt uitgevoerd, en verwijdert die daarna bij voltooiing. Gebruik een Slack-emoji-shortcode zoals `"hourglass_flowing_sand"`.
...
Wanneer native Mattermost-commands zijn ingeschakeld:
...
- Native slash-callbacks worden geverifieerd met de tokens per opdracht die door Mattermost worden geretourneerd tijdens de registratie van slash-opdrachten. Als registratie mislukt of er geen opdrachten zijn geactiveerd, weigert OpenClaw callbacks met `Unauthorized: invalid command token.`
- Voor privé-/tailnet-/interne callbackhosts kan Mattermost vereisen dat `ServiceSettings.AllowedUntrustedInternalConnections` de callbackhost/het callbackdomein bevat. Gebruik host-/domeinwaarden, geen volledige URL's.
- `channels.mattermost.configWrites`: sta door Mattermost geïnitieerde configuratieschrijfbewerkingen toe of weiger ze.
- `channels.mattermost.requireMention`: vereis `@mention` voordat er in kanalen wordt gereageerd.
- `channels.mattermost.groups.<channelId>.requireMention`: override per kanaal voor mention-gating (`"*"` voor standaard).
- Optioneel `channels.mattermost.defaultAccount` overschrijft de standaard accountselectie wanneer dit overeenkomt met een geconfigureerde account-id.
...
- `channels.signal.configWrites`: sta door Signal geïnitieerde configuratieschrijfbewerkingen toe of weiger ze.
- Optioneel `channels.signal.defaultAccount` overschrijft de standaard accountselectie wanneer dit overeenkomt met een geconfigureerde account-id.
...
- Optioneel `channels.bluebubbles.defaultAccount` overschrijft de standaard accountselectie wanneer dit overeenkomt met een geconfigureerde account-id.
- Top-level `bindings[]`-items met `type: "acp"` kunnen BlueBubbles-gesprekken koppelen aan persistente ACP-sessies. Gebruik een BlueBubbles-handle of doeltekenreeks (`chat_id:*`, `chat_guid:*`, `chat_identifier:*`) in `match.peer.id`. Gedeelde veldsemantiek: [ACP-agenten](/nl/tools/acp-agents#persistent-channel-bindings).
...
- Optioneel `channels.imessage.defaultAccount` overschrijft de standaard accountselectie wanneer dit overeenkomt met een geconfigureerde account-id.
...
- SCP gebruikt strikte host-key-controle, dus zorg dat de host-key van de relayhost al bestaat in `~/.ssh/known_hosts`.
- `channels.imessage.configWrites`: sta door iMessage geïnitieerde configuratieschrijfbewerkingen toe of weiger ze.
- Top-level `bindings[]`-items met `type: "acp"` kunnen iMessage-gesprekken koppelen aan persistente ACP-sessies. Gebruik een genormaliseerde handle of expliciet chatdoel (`chat_id:*`, `chat_guid:*`, `chat_identifier:*`) in `match.peer.id`. Gedeelde veldsemantiek: [ACP-agenten](/nl/tools/acp-agents#persistent-channel-bindings).
...
- `channels.matrix.network.dangerouslyAllowPrivateNetwork` staat privé/interne homeservers toe. `proxy` en deze netwerkopt-in zijn onafhankelijke controles.
...
- `channels.matrix.execApprovals`: Matrix-native levering van exec-goedkeuringen en autorisatie van goedkeurders.
  ...
  - `sessionFilter`: optionele sessiesleutelpatronen (substring of regex).
  - `target`: waar goedkeuringsprompts naartoe moeten worden verzonden. `"dm"` (standaard), `"channel"` (oorspronkelijke room) of `"both"`.
...
- Optioneel `channels.irc.defaultAccount` overschrijft de standaard accountselectie wanneer dit overeenkomt met een geconfigureerde account-id.
...
- Bestaande kanaal-only bindingen (geen `accountId`) blijven overeenkomen met het standaardaccount; account-scoped bindingen blijven optioneel.
...
Groepsberichten vereisen standaard een **mention** (metadata-mention of veilige regexpatronen). Geldt voor WhatsApp, Telegram, Discord, Google Chat en iMessage-groepschats.

Zichtbare antwoorden worden afzonderlijk beheerd. Groeps-/kanaalrooms hebben standaard `messages.groupChat.visibleReplies: "message_tool"`: OpenClaw verwerkt de beurt nog steeds, maar normale eindantwoorden blijven privé en zichtbare roomuitvoer vereist `message(action=send)`. Stel `"automatic"` alleen in wanneer je het oude gedrag wilt waarbij normale antwoorden terug naar de room worden geplaatst. Om hetzelfde tool-only gedrag voor zichtbare antwoorden ook op directe chats toe te passen, stel je `messages.visibleReplies: "message_tool"` in; de Codex-harness gebruikt dat tool-only gedrag ook als de niet-ingestelde standaard voor directe chats.

Tool-only zichtbare antwoorden vereisen een model/runtime die betrouwbaar tools aanroept. Als het sessielogboek assistenttekst toont met `didSendViaMessagingTool: false`, heeft het model een privé-eindantwoord geproduceerd in plaats van de berichtentool aan te roepen. Schakel over naar een sterker tool-calling model voor dat kanaal, of stel `messages.groupChat.visibleReplies: "automatic"` in om oude zichtbare eindantwoorden te herstellen.
...
De Gateway herlaadt `messages`-configuratie live nadat het bestand is opgeslagen. Herstart alleen wanneer bestandsbewaking of configuratieherladen in de deployment is uitgeschakeld.

**Mention-typen:**

- **Metadata-vermeldingen**: Native platform-@-vermeldingen. Genegeerd in WhatsApp-zelfchatmodus.
...
{
  messages: {
    visibleReplies: "automatic", // globale standaard voor directe/bronchats; Codex-harnas zet directe chats standaard op message_tool wanneer niet ingesteld
    groupChat: {
      historyLimit: 50,
      visibleReplies: "message_tool", // standaard; gebruik "automatic" voor legacy-eindantwoorden
    },
  },
  agents: {
...
`messages.visibleReplies` is de globale standaard voor bronbeurten; `messages.groupChat.visibleReplies` overschrijft dit voor groeps-/kanaalbronbeurten. Wanneer `messages.visibleReplies` niet is ingesteld, kan een harnas zijn eigen standaard voor direct/bron leveren; het Codex-harnas gebruikt standaard `message_tool`. Kanaal-allowlists en vermeldingsgating bepalen nog steeds of een beurt wordt verwerkt.
...
{
  commands: {
    native: "auto", // registreer native opdrachten wanneer ondersteund
    nativeSkills: "auto", // registreer native skill-opdrachten wanneer ondersteund
    text: true, // parse /commands in chatberichten
    ...
    restart: true, // sta /restart + gateway-herstarttool toe
...
- `native: "auto"` schakelt native opdrachten in voor Discord/Telegram, laat Slack uit.
- `nativeSkills: "auto"` schakelt native skill-opdrachten in voor Discord/Telegram, laat Slack uit.
...
- `plugins: true` schakelt `/plugins` in voor Plugin-ontdekking, installatie en besturing om in/uit te schakelen.
...
- `useAccessGroups: false` staat opdrachten toe om toegangs-groepsbeleid te omzeilen wanneer `allowFrom` niet is ingesteld.
