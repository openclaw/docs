...
title: Modellen
x-i18n:
    generated_at: "2026-05-04T11:26:08Z"
    model: gpt-5.5
    provider: openai
...
Modeldetectie, scans en configuratie (standaardmodel, fallbacks, auth-profielen).
...
gebruikvensters van providers en quotasnapshots.
Huidige providers voor gebruikvensters: Anthropic, GitHub Copilot, Gemini CLI, OpenAI
...
overzicht, terwijl `auth.oauth` alleen de gezondheid van auth-storeprofielen is.
...
- `models list` is alleen-lezen: het leest configuratie, auth-profielen, bestaande catalogusstatus
  en providerbeheerde catalogusrijen, maar het herschrijft
  `models.json` niet.
...
- `models list --all --provider <id>` kan providerbeheerde statische catalogusrijen
  uit pluginmanifesten of gebundelde provider-catalogusmetadata bevatten, zelfs wanneer je
...
- `models list` houdt het control plane responsief terwijl provider-catalogusdetectie
  ...
  achtergrond afronden. Gebruik `--all` wanneer je de exacte volledige ontdekte catalogus nodig hebt en
  bereid bent te wachten op providerdetectie.
- Brede `models list --all` voegt manifestcatalogusrijen samen over registry-rijen
  zonder providerruntime-supplementhooks te laden. Providergefilterde manifest-
...
- Modelverwijzingen worden geparseerd door te splitsen op de **eerste** `/`. Als de model-ID `/` bevat (OpenRouter-stijl), neem dan het providerprefix op (voorbeeld: `openrouter/moonshotai/kimi-k2`).
- Als je de provider weglaat, lost OpenClaw de invoer eerst op als alias, daarna
  ...
  Als die provider het geconfigureerde standaardmodel niet meer beschikbaar stelt, valt OpenClaw
  terug op de eerste geconfigureerde provider/model in plaats van een
  verouderde verwijderde-providerstandaard te tonen.
- `models status` kan `marker(<value>)` in auth-uitvoer tonen voor niet-geheime placeholders (bijvoorbeeld `OPENAI_API_KEY`, `secretref-managed`, `minimax-oauth`, `oauth:chutes`, `ollama-local`) in plaats van ze als geheimen te maskeren.
...
zijn informatief en worden niet op de configuratie toegepast.
...
`--json` reserveert stdout voor de JSON-payload. Diagnostiek voor auth-profielen, providers
en opstarten wordt naar stderr geleid zodat scripts stdout direct kunnen doorgeven
aan tools zoals `jq`.
...
Te verwachten gevallen voor probe-detail/reason-code:
...
- `no_model`: provider-auth bestaat, maar OpenClaw kon geen probeable
...
`models auth login` voert de auth-flow van een providerplugin uit (OAuth/API-sleutel). Gebruik
...
- Anthropic-opmerking: Anthropic-medewerkers vertelden ons dat OpenClaw-stijl Claude CLI-gebruik weer is toegestaan, dus OpenClaw behandelt Claude CLI-hergebruik en `claude -p`-gebruik als gesanctioneerd voor deze integratie tenzij Anthropic een nieuw beleid publiceert.
