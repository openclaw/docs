---
read_when:
    - Op ACP gebaseerde IDE-integraties instellen
    - ACP-sessierouting naar de Gateway debuggen
summary: Start de ACP-brug voor IDE-integraties
title: ACP
x-i18n:
...
via WebSocket. Sessies van ACP blijven gekoppeld aan sessiesleutels van de Gateway.
...
- `loadSession` kan volledige ACP-eventledgergeschiedenis alleen opnieuw afspelen voor
  door de bridge aangemaakte sessies. Oudere sessies of sessies zonder ledger gebruiken nog steeds transcript-
  fallback en reconstrueren geen historische toolaanroepen of systeemmeldingen.
- Als meerdere ACP-clients dezelfde Gateway-sessiesleutel delen, zijn event- en annulerings-
  routing best-effort in plaats van strikt per client geïsoleerd. Geef de voorkeur aan de
  standaard geïsoleerde `acp:<uuid>`-sessies wanneer je schone editorlokale
  beurten nodig hebt.
...
- Initiële sessiebedieningselementen tonen momenteel een gerichte subset van Gateway-knoppen:
  denkniveau, tool-verbositeit, reasoning, gebruiksdetails en verhoogde
...
- `session_info_update` en `usage_update` worden afgeleid van Gateway-sessie-
  ...
  gegevens als vers markeert.
- Tool-meekijkgegevens zijn best-effort. De bridge kan bestandspaden tonen die
  voorkomen in bekende toolargumenten/resultaten, maar verzendt nog geen ACP-terminals of
  gestructureerde bestandsdiffs.
- Exec-goedkeuringsrelay is beperkt tot de actieve ACP-promptbeurt; goedkeuringen van
...
Gebruik de ingebouwde ACP-client om de bridge zonder IDE op gezondheid te controleren.
...
Voor debugging op protocolniveau start je een Gateway met geïsoleerde status en stuur je
...
Vermijd `openclaw gateway call sessions.list` als enig ACP-bewijs. Dat
...
Elke ACP-sessie wordt gekoppeld aan een enkele Gateway-sessiesleutel. Een agent kan veel
sessies hebben; ACP gebruikt standaard een geisoleerde `acp:<uuid>`-sessie, tenzij je
...
Als je wilt dat `acpx openclaw` elke keer op een specifieke Gateway en sessiesleutel is gericht,
...
contextuele informatie uit een OpenClaw-agent te laten halen zonder een terminal te scrapen.
...
Standaard krijgen ACP-sessies een geisoleerde Gateway-sessiesleutel met een `acp:`-voorvoegsel.
