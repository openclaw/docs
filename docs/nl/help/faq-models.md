...
read_when:
    ...
    - Foutopsporing voor overschakeling bij modeluitval / "Alle modellen zijn mislukt"
    - Authenticatieprofielen begrijpen en beheren
sidebarTitle: Models FAQ
summary: 'Veelgestelde vragen: modelstandaardwaarden, selectie, aliassen, overschakelen, failover en authenticatieprofielen'
title: 'Veelgestelde vragen: modellen en authenticatie'
x-i18n:
    generated_at: "2026-05-06T09:16:40Z"
    model: gpt-5.5
    provider: openai
...
---
  ...
  Model- en auth-profile-vragen en antwoorden. Zie voor installatie, sessies, gateway, kanalen en
  ...
  <Accordion title="Welk model raden jullie aan?">
    ...
    **Voor routinematige/chat met laag risico:** gebruik goedkopere fallback-modellen en routeer op basis van agentrol.

    MiniMax heeft eigen documentatie: [MiniMax](/nl/providers/minimax) en
    [Lokale modellen](/nl/gateway/local-models).

    Vuistregel: gebruik het **beste model dat je je kunt veroorloven** voor werk met hoog risico, en een goedkoper
  ...
  <Accordion title="Hoe wissel ik van model zonder mijn configuratie te wissen?">
    Gebruik **modelopdrachten** of bewerk alleen de **model**-velden. Vermijd volledige config-vervangingen.
    ...
    Inspecteer voor RPC-bewerkingen eerst met `config.schema.lookup` en geef de voorkeur aan `config.patch`. De lookup-payload geeft je het genormaliseerde pad, beknopte schemadocumentatie/beperkingen en samenvattingen van directe onderliggende items.
  ...
  <Accordion title="Welke modellen gebruiken OpenClaw, Flawd en Krill?">
    - Deze deployments kunnen verschillen en in de loop van de tijd veranderen; er is geen vaste provideraanbeveling.
  ...
  <Accordion title="Hoe wissel ik direct van model (zonder opnieuw op te starten)?">
    ...
    Je kunt ook een specifiek auth profile voor de provider afdwingen (per sessie):
    ...
    Tip: `/model status` toont welke agent actief is, welk `auth-profiles.json`-bestand wordt gebruikt en welk auth profile hierna wordt geprobeerd.
    ...
    **Hoe maak ik een profiel los dat ik met @profile heb vastgezet?**
    ...
    Gebruik `/model status` om te bevestigen welk auth profile actief is.
  ...
  <Accordion title="Kan ik GPT 5.5 gebruiken voor dagelijkse taken en Codex 5.5 voor programmeren?">
    ...
    - **Native Codex-codeeragent:** stel `agents.defaults.model.primary` in op `openai/gpt-5.5` en `agents.defaults.agentRuntime.id` op `"codex"`. Meld je aan met `openclaw models auth login --provider openai-codex` wanneer je ChatGPT/Codex-abonnementsauthenticatie wilt.
    ...
    - **Sub-agents:** routeer codeertaken naar een agent die alleen Codex gebruikt, met een eigen standaardmodel en eigen `agentRuntime`.
  ...
  <Accordion title="Hoe configureer ik de snelle modus voor GPT 5.5?">
    ...
    Voor OpenAI wordt de snelle modus gekoppeld aan `service_tier = "priority"` op ondersteunde native Responses-verzoeken. Sessie-`/fast`-overrides hebben voorrang op configuratiestandaarden.
  ...
  <Accordion title='Waarom zie ik "Unknown model: minimax/MiniMax-M2.7"?'>
    ...
    profile gevonden), waardoor het model niet kan worden opgelost.

    Controlelijst voor oplossing:

    1. Upgrade naar een actuele OpenClaw-release (of draai vanaf source `main`) en herstart daarna de gateway.
    2. Zorg dat MiniMax is geconfigureerd (wizard of JSON), of dat MiniMax-authenticatie
       bestaat in env/auth profiles zodat de bijpassende provider kan worden geïnjecteerd
       (`MINIMAX_API_KEY` voor `minimax`, `MINIMAX_OAUTH_TOKEN` of opgeslagen MiniMax
       OAuth voor `minimax-portal`).
    3. Gebruik het exacte model-id (hoofdlettergevoelig) voor je authenticatiepad:
  ...
  <Accordion title="Kan ik MiniMax als mijn standaard gebruiken en OpenAI voor complexe taken?">
    ...
    **Optie B: aparte agents**
    ...
    Documentatie: [Modellen](/nl/concepts/models), [Routering met meerdere agents](/nl/concepts/multi-agent), [MiniMax](/nl/providers/minimax), [OpenAI](/nl/providers/openai).

  </Accordion>

  <Accordion title="Zijn opus / sonnet / gpt ingebouwde snelkoppelingen?">
    Ja. OpenClaw levert een paar standaardverkortingen mee (alleen toegepast wanneer het model bestaat in `agents.defaults.models`):
  ...
  <Accordion title="Hoe definieer/overschrijf ik modelsnelkoppelingen (aliassen)?">
    ...
    Daarna wordt `/model sonnet` (of `/<alias>` wanneer ondersteund) omgezet naar dat model-ID.
...
<AccordionGroup>
  <Accordion title="Hoe werkt failover?">
    ...
    expliciete factureringstekst retourneert bij `401` of `403`, kan OpenClaw die nog steeds in
    ...
    exceeded` blijven op het pad voor Compaction/opnieuw proberen in plaats van modelterugval
    te activeren.

    Generieke serverfouttekst is bewust smaller dan "alles met
    onbekend/fout erin". OpenClaw behandelt wel providergebonden tijdelijke vormen
    zoals Anthropic kaal `An unknown error occurred`, OpenRouter kaal
    `Provider returned error`, stop-redenfouten zoals `Unhandled stop reason:
    ...
    overeenkomt.
    Generieke interne terugvaltekst zoals `LLM request failed with an unknown
  ...
  <Accordion title='Wat betekent "No credentials found for profile anthropic:default"?'>
    Het betekent dat het systeem probeerde het auth-profiel-ID `anthropic:default` te gebruiken, maar er geen credentials voor kon vinden in de verwachte auth-store.
    ...
    **Checklist voor oplossen:**
    ...
    - **Controleer of je env-var door de Gateway is geladen**
      - Als je `ANTHROPIC_API_KEY` in je shell instelt maar de Gateway via systemd/launchd uitvoert, neemt die deze mogelijk niet over. Zet deze in `~/.openclaw/.env` of schakel `env.shellEnv` in.
    - **Zorg dat je de juiste agent bewerkt**
      - Multi-agent-setups betekenen dat er meerdere `auth-profiles.json`-bestanden kunnen zijn.
    - **Controleer de model-/auth-status globaal**
      - Gebruik `openclaw models status` om geconfigureerde modellen te zien en of providers geauthenticeerd zijn.

    **Checklist voor oplossen van "No credentials found for profile anthropic"**
    ...
    - **Controleer of je opdrachten op de gatewayhost uitvoert**
  ...
  <Accordion title="Waarom probeerde het ook Google Gemini en mislukte dat?">
    Als je modelconfiguratie Google Gemini als terugval bevat (of je bent overgestapt op een Gemini-verkorting), probeert OpenClaw dit tijdens modelterugval. Als je geen Google-credentials hebt geconfigureerd, zie je `No API key found for provider "google"`.
    ...
    **LLM-verzoek geweigerd: thinking-signature vereist (Google Antigravity)**
    ...
    Oplossing: OpenClaw verwijdert nu unsigned thinking-blokken voor Google Antigravity Claude. Als dit nog steeds verschijnt, start je een **nieuwe sessie** of stel je `/thinking off` in voor die agent.
...
<AccordionGroup>
  <Accordion title="Wat is een auth-profiel?">
    ...
    Voer `openclaw models auth list` uit om opgeslagen profielen te inspecteren zonder secrets te dumpen (optioneel `--provider <id>` of `--json`). Zie [Modellen-CLI](/nl/cli/models#auth-profiles) voor details.
  ...
  <Accordion title="Kan ik bepalen welk auth-profiel als eerste wordt geprobeerd?">
    ...
    OpenClaw kan een profiel tijdelijk overslaan als het in een korte **cooldown** staat (rate limits/timeouts/auth-fouten) of in een langere **uitgeschakelde** status (facturering/onvoldoende credits). Voer `openclaw models status --json` uit en controleer `auth.unusableProfiles` om dit te inspecteren. Afstemming: `auth.cooldowns.billingBackoffHours*`.
...
- [FAQ — snel aan de slag en eerste installatie](/nl/help/faq-first-run)
