...
installatieprogramma heeft een eigen `--verbose`-flag, maar die flag is geen onderdeel van
`openclaw update`.
...
OpenClaw biedt nog geen LTS- of maandelijks supportupdatekanaal. We werken
toe naar SemVer-compatibele maandelijkse supportlijnen, maar vandaag zijn de ondersteunde
kanalen nog steeds `stable`, `beta` en `dev`.
...
en herstart die, tenzij je `--no-restart` doorgeeft.
...
Voeg `--no-onboard` toe om onboarding over te slaan. Om via het installatieprogramma
een specifiek installatietype af te dwingen, geef je `--install-method git --no-onboard` of
...
Geef voor beheerde installaties de voorkeur aan `openclaw update`, omdat het de
...
klaar is, zodat het oude proces niet blijft serveren vanuit vervangen
pakketbestanden.

Wanneer `openclaw update` een globale npm-installatie beheert, installeert het het doel eerst in
een tijdelijke npm-prefix, verifieert het de verpakte `dist`-inventaris en wisselt het vervolgens
...
probeert OpenClaw het eenmaal opnieuw met `--omit=optional`. Die nieuwe poging helpt hosts waar native
...
<AccordionGroup>
  <Accordion title="Read-only package tree">
    OpenClaw behandelt verpakte globale installaties tijdens runtime als alleen-lezen, zelfs wanneer de globale pakketmap schrijfbaar is voor de huidige gebruiker. Plugin-pakketinstallaties bevinden zich in npm/git-roots die eigendom zijn van OpenClaw onder de gebruikersconfiguratiemap, en het opstarten van de Gateway wijzigt de OpenClaw-pakketboom niet.
  ...
  <Accordion title="Hardened systemd units">
    Geef OpenClaw schrijftoegang tot zijn configuratie-/statusroots, zodat expliciete Plugin-installaties, Plugin-updates en doctor-opruiming hun wijzigingen kunnen behouden:
...
Updates via pakketbeheer die via de live Gateway-control-plane-handler worden aangevraagd,
forceren een niet-uitgestelde updateherstart zonder cooldown na de pakketwissel. Dat
...
blijft de voorkeursroute voor beheerde installaties omdat het de service rond de update kan stoppen en
herstarten.
...
Migreert configuratie, audit DM-beleid en controleert de Gateway-status. Details: [Doctor](/nl/gateway/doctor)
...
- Voor `openclaw update --channel dev` op broncheckouts bootstrapt de updater `pnpm` automatisch wanneer nodig. Als je een pnpm/corepack-bootstrapfout ziet, installeer `pnpm` dan handmatig (of schakel `corepack` opnieuw in) en voer de update opnieuw uit.
