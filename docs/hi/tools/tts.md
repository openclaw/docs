...
x-i18n:
    generated_at: "2026-07-16T17:57:07Z"
    model: gpt-5.6
...
Feishu, Matrix, Telegram और WhatsApp पर नेटिव वॉइस संदेश; अन्य सभी जगहों पर ऑडियो
अटैचमेंट; और टेलीफ़ोनी तथा Talk के लिए PCM/Ulaw स्ट्रीम।

TTS, Talk के `stt-tts` मोड का वाक्-आउटपुट भाग है (`talk.speak` कॉल भी इसी
संश्लेषण पथ का उपयोग करती हैं)। प्रदाता-नेटिव `realtime` Talk सत्र इसके बजाय
रीयलटाइम प्रदाता के भीतर वाक् संश्लेषित करते हैं; `transcription` सत्र कभी भी
...
<Steps>
  ...
  <Step title="API कुंजी सेट करें">
    अपने प्रदाता के लिए env var एक्सपोर्ट करें (उदाहरण के लिए `OPENAI_API_KEY`,
  ...
  <Step title="इसे चैट में आज़माएँ">
...
OpenClaw रजिस्ट्री के स्वतः-चयन क्रम में पहला कॉन्फ़िगर किया हुआ प्रदाता चुनता है।
अंतर्निहित `tts` एजेंट टूल केवल स्पष्ट अभिप्राय के लिए है: सामान्य चैट
टेक्स्ट ही रहती है, जब तक उपयोगकर्ता ऑडियो न माँगे, `/tts` का उपयोग न करे, या Auto-TTS/डायरेक्टिव
वाक् सक्षम न करे।
</Note>
...
| **Google Gemini** | `GEMINI_API_KEY` या `GOOGLE_API_KEY`                                                                             | Gemini API बैच TTS; `promptTemplate: "audio-profile-v1"` के माध्यम से व्यक्तित्व-सजग।               |
...
| **OpenAI**        | `OPENAI_API_KEY`                                                                                                 | स्वतः-सारांश के लिए भी उपयोग होता है; व्यक्तित्व `instructions` का समर्थन करता है।                                |
...
| **Volcengine**    | `VOLCENGINE_TTS_API_KEY` या `BYTEPLUS_SEED_SPEECH_API_KEY` (पुराना AppID/token: `VOLCENGINE_TTS_APPID`/`_TOKEN`) | BytePlus Seed Speech HTTP API।                                                              |
| **Vydra**         | `VYDRA_API_KEY`                                                                                                  | साझा छवि, वीडियो और वाक् प्रदाता।                                                   |
| **xAI**           | `XAI_API_KEY`                                                                                                    | xAI बैच TTS। नेटिव Opus वॉइस-नोट **समर्थित नहीं** है।                                 |
| **Xiaomi MiMo**   | `XIAOMI_API_KEY`                                                                                                 | Xiaomi चैट पूर्णताओं के माध्यम से MiMo TTS।                                                   |
...
तो उस प्रदाता को भी प्रमाणित किया जाना आवश्यक है।

<Warning>
बंडल किया हुआ **Microsoft** प्रदाता `node-edge-tts` के माध्यम से Microsoft Edge की ऑनलाइन न्यूरल TTS
सेवा का उपयोग करता है। यह बिना प्रकाशित SLA या कोटा वाली एक सार्वजनिक वेब सेवा
है—इसे सर्वोत्तम-प्रयास सेवा मानें। पुरानी प्रदाता आईडी `edge` को
`microsoft` में सामान्यीकृत किया जाता है और `openclaw doctor --fix` स्थायी
कॉन्फ़िगरेशन को पुनर्लिखता है; नए कॉन्फ़िगरेशन में हमेशा `microsoft` का उपयोग होना चाहिए।
</Warning>
...
`voiceName` फ़ील्ड नाम अब भी पुराने उपनामों के रूप में काम करते हैं।
    ...
    tts: {
      ...
      providers: {
        google: {
          ...
          // audioProfile: "शांत, पॉडकास्ट-होस्ट जैसे लहजे में बोलें।",
          // speakerName: "Alex",
        },
      },
    },
...
```
  ...
  <Tab title="Microsoft (कोई कुंजी नहीं)">
```json5
{
  messages: {
    tts: {
...
वॉइस-डिज़ाइन प्रॉम्प्ट पर सेट करें। OpenClaw उस प्रॉम्प्ट को TTS `user` संदेश के रूप में भेजता है
और voicedesign मॉडल के लिए `audio.voice` नहीं भेजता।

### प्रति-एजेंट वॉइस ओवरराइड

जब किसी एक एजेंट को किसी अलग प्रदाता, वॉइस, मॉडल, पर्सोना या ऑटो-TTS मोड के साथ बोलना हो, तब `agents.list[].tts` का उपयोग करें। एजेंट ब्लॉक को
`messages.tts` पर डीप-मर्ज किया जाता है, इसलिए प्रदाता क्रेडेंशियल वैश्विक प्रदाता कॉन्फ़िगरेशन में रह सकते हैं:
...
किसी प्रति-एजेंट पर्सोना को नियत करने के लिए, प्रदाता कॉन्फ़िगरेशन के साथ `agents.list[].tts.persona` सेट करें — यह केवल उस एजेंट के लिए वैश्विक `messages.tts.persona` को ओवरराइड करता है।

स्वचालित उत्तरों, `/tts audio`, `/tts status` और
`tts` एजेंट टूल के लिए वरीयता क्रम:
...
6. जब [मॉडल ओवरराइड](#model-driven-directives) सक्षम हों, तब इनलाइन `[[tts:...]]` निर्देश

चैनल और अकाउंट ओवरराइड का आकार `messages.tts` के समान होता है और वे
पहले की परतों पर डीप-मर्ज होते हैं, इसलिए साझा प्रदाता क्रेडेंशियल
`messages.tts` में रह सकते हैं, जबकि कोई चैनल या बॉट अकाउंट केवल स्पीकर वॉइस, मॉडल, पर्सोना
या ऑटो मोड बदलता है:
...
एक **पर्सोना** स्थिर वाचिक पहचान है, जिसे सभी प्रदाताओं पर निर्धारणात्मक रूप से
लागू किया जा सकता है। यह किसी एक प्रदाता को प्राथमिकता दे सकता है, प्रदाता-निरपेक्ष प्रॉम्प्ट
अभिप्राय परिभाषित कर सकता है और वॉइस, मॉडल, प्रॉम्प्ट टेम्पलेट,
सीड तथा वॉइस सेटिंग के लिए प्रदाता-विशिष्ट बाइंडिंग रख सकता है।
  ...
  messages: {
    tts: {
      ...
      personas: {
        alfred: {
          label: "अल्फ्रेड",
          description: "संयत हास्य वाला, आत्मीय ब्रिटिश बटलर वाचक।",
          provider: "google",
          fallbackPolicy: "preserve-persona",
          prompt: {
            profile: "एक प्रतिभाशाली ब्रिटिश बटलर। संयत हास्यपूर्ण, बुद्धिमान, आत्मीय, आकर्षक, भावनात्मक रूप से अभिव्यंजक, कभी भी सामान्य नहीं।",
            scene: "देर रात का एक शांत अध्ययन-कक्ष। किसी विश्वसनीय संचालक के लिए क्लोज़-माइक वर्णन।",
            sampleContext: "वक्ता संक्षिप्त आत्मविश्वास और संयत आत्मीयता के साथ एक निजी तकनीकी अनुरोध का उत्तर दे रहा है।",
            style: "परिष्कृत, संयत, हल्का मनोरंजित।",
            accent: "ब्रिटिश अंग्रेज़ी।",
            pacing: "नपी-तुली, छोटे नाटकीय विरामों के साथ।",
            constraints: ["कॉन्फ़िगरेशन मानों को ज़ोर से न पढ़ें।", "पर्सोना की व्याख्या न करें।"],
          },
          providers: {
            google: {
              model: "gemini-3.1-flash-tts-preview",
...
सक्रिय पर्सोना निर्धारणात्मक रूप से चुना जाता है:

1. `/tts persona <id>` स्थानीय प्राथमिकता, यदि सेट हो।
2. `messages.tts.persona`, यदि सेट हो।
3. कोई पर्सोना नहीं।

प्रदाता चयन में स्पष्ट विकल्प को पहले प्राथमिकता दी जाती है:
...
प्रदाता के प्रत्येक प्रयास के लिए, OpenClaw इस क्रम में कॉन्फ़िगरेशन मर्ज करता है:
...
पर्सोना प्रॉम्प्ट फ़ील्ड (`profile`, `scene`, `sampleContext`, `style`, `accent`,
`pacing`, `constraints`) **प्रदाता-निरपेक्ष** हैं। प्रत्येक प्रदाता तय करता है कि
उनका उपयोग कैसे करना है:

<AccordionGroup>
  <Accordion title="Google Gemini">
    पर्सोना प्रॉम्प्ट फ़ील्ड को Gemini TTS प्रॉम्प्ट संरचना में **केवल तभी**
    लपेटता है, जब प्रभावी Google प्रदाता कॉन्फ़िगरेशन `promptTemplate: "audio-profile-v1"`
    या `personaPrompt` सेट करता हो। पुराने `audioProfile` और `speakerName` फ़ील्ड
    अब भी Google-विशिष्ट प्रॉम्प्ट टेक्स्ट के रूप में पहले जोड़े जाते हैं। किसी `[[tts:text]]` ब्लॉक के भीतर
    `[whispers]` या `[laughs]` जैसे इनलाइन ऑडियो टैग
    Gemini ट्रांसक्रिप्ट में सुरक्षित रखे जाते हैं; OpenClaw ये टैग उत्पन्न नहीं करता।
  </Accordion>
  <Accordion title="OpenAI">
    पर्सोना प्रॉम्प्ट फ़ील्ड को अनुरोध के `instructions` फ़ील्ड से **केवल तभी**
    मैप करता है, जब कोई स्पष्ट OpenAI `instructions` कॉन्फ़िगर न हो। स्पष्ट `instructions`
    हमेशा प्रभावी होता है।
  </Accordion>
  <Accordion title="अन्य प्रदाता">
    केवल `personas.<id>.providers.<provider>` के अंतर्गत प्रदाता-विशिष्ट
    पर्सोना बाइंडिंग का उपयोग करते हैं। जब तक प्रदाता अपना पर्सोना-प्रॉम्प्ट मैपिंग
    लागू नहीं करता, पर्सोना प्रॉम्प्ट फ़ील्ड उपेक्षित रहते हैं।
  </Accordion>
</AccordionGroup>

### फ़ॉलबैक नीति

जब प्रयास किए जा रहे प्रदाता के लिए किसी पर्सोना की **कोई बाइंडिंग नहीं** होती,
तब `fallbackPolicy` व्यवहार नियंत्रित करता है:
...
| `provider-defaults` | उस प्रयास के लिए पर्सोना को प्रॉम्प्ट तैयार करने से बाहर रखा जाता है; अन्य प्रदाताओं पर फ़ॉलबैक जारी रहते हुए प्रदाता अपने निरपेक्ष डिफ़ॉल्ट का उपयोग करता है। |
| `fail`              | उस प्रदाता के प्रयास को `reasonCode: "not_configured"` और `personaBinding: "missing"` के साथ छोड़ दें। फ़ॉलबैक प्रदाताओं को फिर भी आज़माया जाता है।              |

पूरा TTS अनुरोध केवल तभी विफल होता है, जब प्रयास किए गए **हर** प्रदाता को छोड़ दिया जाए
या वह विफल हो जाए।

Talk सत्र का प्रदाता चयन सत्र-सीमित होता है। Talk क्लाइंट को
`talk.catalog` से प्रदाता आईडी, मॉडल आईडी, वॉइस आईडी और लोकेल चुनकर
उन्हें Talk सत्र या हैंडऑफ़ अनुरोध के माध्यम से पास करना चाहिए। वॉइस सत्र खोलने पर
`messages.tts` या वैश्विक Talk प्रदाता डिफ़ॉल्ट में बदलाव नहीं होना चाहिए।

## मॉडल-संचालित निर्देश

डिफ़ॉल्ट रूप से, सहायक किसी एक उत्तर के लिए वॉइस, मॉडल या गति को ओवरराइड करने हेतु
`[[tts:...]]` निर्देश, साथ ही केवल ऑडियो में दिखाई देने वाले
अभिव्यंजक संकेतों के लिए एक वैकल्पिक `[[tts:text]]...[[/tts:text]]` ब्लॉक उत्सर्जित **कर सकता है**:
...
जब `messages.tts.auto`, `"tagged"` होता है, तब ऑडियो ट्रिगर करने के लिए
**निर्देश आवश्यक होते हैं**। स्ट्रीमिंग ब्लॉक डिलीवरी निर्देशों को चैनल तक पहुँचने से पहले
दृश्य टेक्स्ट से हटा देती है, भले ही वे आसन्न ब्लॉकों में विभाजित हों।

जब तक `modelOverrides.allowProvider: true` न हो, `provider=...` को अनदेखा किया जाता है। जब कोई
उत्तर `provider=...` घोषित करता है, तो उस निर्देश की अन्य कुंजियों को
केवल वही प्रदाता पार्स करता है; असमर्थित कुंजियाँ हटा दी जाती हैं और TTS
निर्देश चेतावनियों के रूप में रिपोर्ट की जाती हैं।

**उपलब्ध निर्देश कुंजियाँ:**

- `provider` (पंजीकृत प्रदाता आईडी; `allowProvider: true` आवश्यक)
- `speakerVoice` / `speakerVoiceId` (लीगेसी उपनाम: `voice`, `voiceName`, `voice_name`, `google_voice`, `voiceId`)
- `model` / `google_model`
...
एकल कमांड `/tts`। Discord पर, OpenClaw `/voice` को भी पंजीकृत करता है, क्योंकि
`/tts` एक अंतर्निहित Discord कमांड है — टेक्स्ट `/tts ...` अब भी काम करता है।
...
`commands.text` या मूल कमांड पंजीकरण सक्षम होना चाहिए।
</Note>
...
- `/tts latest` वर्तमान सत्र ट्रांसक्रिप्ट से सहायक का नवीनतम उत्तर पढ़ता है और उसे एक बार ऑडियो के रूप में भेजता है। डुप्लिकेट वॉइस प्रेषण रोकने के लिए यह सत्र प्रविष्टि पर उस उत्तर का केवल एक हैश संग्रहीत करता है।
- `/tts audio` एक बार का ऑडियो उत्तर उत्पन्न करता है (TTS को चालू **नहीं** करता)।
- `/tts limit <chars>` **100–4096** स्वीकार करता है (4096 Telegram कैप्शन/संदेश की अधिकतम सीमा है); इस सीमा से बाहर के मान अस्वीकार किए जाते हैं।
- `limit` और `summary` मुख्य कॉन्फ़िगरेशन में नहीं, बल्कि **स्थानीय प्राथमिकताओं** में संग्रहीत होते हैं।
- `/tts status` में नवीनतम प्रयास के लिए फ़ॉलबैक निदान शामिल होते हैं — `Fallback: <primary> -> <used>`, `Attempts: ...` और प्रति-प्रयास विवरण (`provider:outcome(reasonCode) latency`)।
- TTS सक्षम होने पर `/status` सक्रिय TTS मोड के साथ कॉन्फ़िगर किया गया प्रदाता, मॉडल, वॉइस और स्वच्छ किया गया कस्टम एंडपॉइंट मेटाडेटा दिखाता है।

## प्रति-उपयोगकर्ता प्राथमिकताएँ

स्लैश कमांड स्थानीय ओवरराइड को `prefsPath` में लिखते हैं। डिफ़ॉल्ट
`~/.openclaw/settings/tts.json` है; इसे `OPENCLAW_TTS_PREFS` पर्यावरण चर
या `messages.tts.prefsPath` से ओवरराइड करें।

| संग्रहीत फ़ील्ड | प्रभाव                                                                           |
| ------------ | -------------------------------------------------------------------------------- |
| `auto`       | स्थानीय ऑटो-TTS ओवरराइड (`always`, `off`, …)                                     |
| `provider`   | स्थानीय प्राथमिक प्रदाता ओवरराइड                                                  |
| `persona`    | स्थानीय व्यक्तित्व ओवरराइड                                                           |
| `maxLength`  | सारांश/संक्षिप्तीकरण सीमा (डिफ़ॉल्ट `1500` वर्ण, `/tts limit` सीमा 100–4096) |
| `summarize`  | सारांश टॉगल (डिफ़ॉल्ट `true`)                                                  |

ये उस होस्ट के लिए `messages.tts` तथा सक्रिय
`agents.list[].tts` ब्लॉक से प्राप्त प्रभावी कॉन्फ़िगरेशन को ओवरराइड करते हैं।

## आउटपुट प्रारूप

TTS वॉइस डिलीवरी चैनल की क्षमता से संचालित होती है। चैनल Plugin बताते हैं
कि वॉइस-शैली TTS को प्रदाताओं से मूल `voice-note` लक्ष्य का अनुरोध करना चाहिए या
सामान्य `audio-file` संश्लेषण बनाए रखना चाहिए, और यह भी कि चैनल भेजने से पहले
गैर-मूल आउटपुट को ट्रांसकोड करता है या नहीं।

| लक्ष्य                                | प्रारूप                                                                                                                                |
| ------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------- |
| Feishu / Matrix / Telegram / WhatsApp | वॉइस-नोट उत्तरों में **Opus** को प्राथमिकता दी जाती है (ElevenLabs से `opus_48000_64`, OpenAI से `opus`)। 48 kHz / 64 kbps स्पष्टता और आकार में संतुलन रखता है। |
...
- **Xiaomi MiMo:** डिफ़ॉल्ट रूप से MP3, या कॉन्फ़िगर होने पर WAV; चैनल द्वारा घोषित वॉइस-नोट लक्ष्यों के लिए `ffmpeg` के साथ 48 kHz Opus में ट्रांसकोड किया जाता है।
...
- **Google Gemini:** रॉ 24 kHz PCM लौटाता है। OpenClaw इसे ऑडियो अटैचमेंट के लिए WAV में रैप करता है, वॉइस-नोट लक्ष्यों के लिए 48 kHz Opus में ट्रांसकोड करता है, और बातचीत/टेलीफ़ोनी के लिए सीधे PCM लौटाता है।
...
- **xAI:** डिफ़ॉल्ट रूप से MP3; ऑडियो-फ़ाइल संश्लेषण बफ़र किए गए और स्ट्रीमिंग, दोनों आउटपुट के लिए `mp3`, `wav`, `pcm`, `mulaw`, या `alaw` का उपयोग कर सकता है। वॉइस-नोट लक्ष्य स्ट्रीमिंग तथा बफ़र किए गए फ़ॉलबैक के लिए MP3 का उपयोग करते हैं, क्योंकि xAI के `pcm`, `mulaw`, और `alaw` आउटपुट हेडर-रहित रॉ ऑडियो हैं। बफ़र किया गया संश्लेषण xAI के बैच REST `/v1/tts` एंडपॉइंट का उपयोग करता है; `textToSpeechStream` मूल `wss://api.x.ai/v1/tts` का उपयोग करता है। यह रीयलटाइम वॉइस अनुबंध नहीं है। मूल Opus वॉइस-नोट प्रारूप समर्थित नहीं है।
- **Microsoft:** `microsoft.outputFormat` (डिफ़ॉल्ट `audio-24khz-48kbitrate-mono-mp3`) का उपयोग करता है।
  ...
  - जब कोई स्पष्ट वॉइस ओवरराइड सेट नहीं होता और डिफ़ॉल्ट अंग्रेज़ी वॉइस का उपयोग होता है, तो उत्तर का टेक्स्ट मुख्यतः CJK में होने पर OpenClaw स्वतः चीनी न्यूरल वॉइस (`zh-CN-XiaoxiaoNeural`, `zh-CN` लोकेल) पर स्विच हो जाता है।

OpenAI और ElevenLabs के आउटपुट प्रारूप ऊपर सूचीबद्ध अनुसार प्रत्येक चैनल के लिए निश्चित हैं।

## ऑटो-TTS व्यवहार

जब `messages.tts.auto` सक्षम होता है, तो OpenClaw:

- यदि उत्तर में पहले से संरचित मीडिया मौजूद है, तो TTS को छोड़ देता है।
- बहुत छोटे उत्तरों (10 वर्णों से कम) को छोड़ देता है।
- सारांश सक्षम होने पर लंबे उत्तरों को
  `summaryModel` (या `agents.defaults.model.primary`) का उपयोग करके संक्षिप्त करता है।
- जनरेट किए गए ऑडियो को उत्तर के साथ संलग्न करता है।
- `mode: "final"` में, टेक्स्ट स्ट्रीम पूरी होने के बाद भी स्ट्रीम किए गए अंतिम उत्तरों के लिए
  केवल-ऑडियो TTS भेजता है; जनरेट किया गया मीडिया सामान्य उत्तर अनुलग्नकों की तरह
  उसी चैनल मीडिया सामान्यीकरण से होकर गुजरता है।

यदि उत्तर `maxLength` से अधिक लंबा है, तो OpenClaw ऑडियो को कभी भी पूरी तरह नहीं छोड़ता:

- **सारांश चालू** (डिफ़ॉल्ट) और सारांश मॉडल उपलब्ध है: टेक्स्ट को लगभग
  `maxLength` वर्णों में संक्षिप्त करता है, फिर सारांश का वाक्-संश्लेषण करता है।
- **सारांश बंद**, संक्षिप्तीकरण विफल होता है, या सारांश मॉडल के लिए कोई API कुंजी
  उपलब्ध नहीं है: टेक्स्ट को `maxLength` वर्णों तक छोटा करता है और छोटे किए गए
  टेक्स्ट का वाक्-संश्लेषण करता है।
  ...
  हाँ  -> मीडिया है / छोटा है?
          ...
          नहीं -> लंबाई > सीमा?
                   ...
                   हाँ   -> सारांश सक्षम और उपलब्ध है?
                            नहीं -> छोटा करें -> TTS -> ऑडियो संलग्न करें
                            हाँ   -> सारांशित करें -> TTS -> ऑडियो संलग्न करें
```
...
## फ़ील्ड संदर्भ
  ...
  <Accordion title="शीर्ष-स्तरीय messages.tts.*">
    ...
    <ParamField path="provider" type="string">
      स्पीच प्रदाता आईडी। सेट न होने पर, OpenClaw रजिस्ट्री के स्वतः-चयन क्रम में पहले कॉन्फ़िगर किए गए प्रदाता का उपयोग करता है। पुराने `provider: "edge"` को `openclaw doctor --fix` द्वारा `"microsoft"` के रूप में पुनर्लिखित किया जाता है।
    </ParamField>
    <ParamField path="persona" type="string">
      `personas` से सक्रिय पर्सोना आईडी। लोअरकेस में सामान्यीकृत किया जाता है।
    </ParamField>
    <ParamField path="personas.<id>" type="object">
      स्थिर वाचिक पहचान। फ़ील्ड: `label`, `description`, `provider`, `fallbackPolicy`, `prompt`, `providers.<provider>`। [पर्सोना](#personas) देखें।
    </ParamField>
    <ParamField path="summaryModel" type="string">
      स्वतः-सारांश के लिए किफ़ायती मॉडल; डिफ़ॉल्ट `agents.defaults.model.primary` है। `provider/model` या कॉन्फ़िगर किया गया मॉडल उपनाम स्वीकार करता है।
    </ParamField>
    ...
    <ParamField path="providers.<id>" type="object">
      स्पीच प्रदाता आईडी द्वारा कुंजीबद्ध, प्रदाता-स्वामित्व वाली सेटिंग। पुराने प्रत्यक्ष ब्लॉक (`messages.tts.openai`, `.elevenlabs`, `.microsoft`, `.edge`) को `openclaw doctor --fix` द्वारा पुनर्लिखित किया जाता है; केवल `messages.tts.providers.<id>` को कमिट करें।
    </ParamField>
    <ParamField path="maxTextLength" type="number" default="4096">
      TTS इनपुट वर्णों की कठोर अधिकतम सीमा। सीमा पार होने पर `/tts audio`, `tts.convert`, और `tts.speak` विफल हो जाते हैं।
    </ParamField>
    <ParamField path="timeoutMs" type="number" default="30000">
      अनुरोध की समय-सीमा मिलीसेकंड में। सेट होने पर प्रति-कॉल `timeoutMs` (एजेंट टूल, Gateway) को प्राथमिकता मिलती है; अन्यथा स्पष्ट रूप से कॉन्फ़िगर किए गए `messages.tts.timeoutMs` को Plugin द्वारा निर्धारित किसी भी प्रदाता डिफ़ॉल्ट पर प्राथमिकता मिलती है।
    </ParamField>
  ...
  <Accordion title="Azure Speech">
    <ParamField path="apiKey" type="string">परिवेश चर: `AZURE_SPEECH_KEY`, `AZURE_SPEECH_API_KEY`, या `SPEECH_KEY`।</ParamField>
    <ParamField path="region" type="string">Azure Speech क्षेत्र (उदा. `eastus`)। परिवेश चर: `AZURE_SPEECH_REGION` या `SPEECH_REGION`।</ParamField>
  ...
  <Accordion title="ElevenLabs">
    <ParamField path="apiKey" type="string">उपलब्ध न होने पर `ELEVENLABS_API_KEY` या `XI_API_KEY` का उपयोग करता है।</ParamField>
    ...
    <ParamField path="seed" type="number">सर्वोत्तम-प्रयास निर्धारणवाद के लिए पूर्णांक `0..4294967295`।</ParamField>
    <ParamField path="baseUrl" type="string">ElevenLabs API के आधार URL को ओवरराइड करें।</ParamField>
  </Accordion>

  <Accordion title="Google Gemini">
    <ParamField path="apiKey" type="string">`GEMINI_API_KEY` / `GOOGLE_API_KEY` पर फ़ॉलबैक करता है। यदि छोड़ा गया हो, तो TTS env फ़ॉलबैक से पहले `models.providers.google.apiKey` का पुनः उपयोग कर सकता है।</ParamField>
    <ParamField path="model" type="string">Gemini TTS मॉडल। डिफ़ॉल्ट `gemini-3.1-flash-tts-preview`।</ParamField>
    <ParamField path="speakerVoice" type="string">Gemini की पहले से निर्मित वॉइस का नाम। डिफ़ॉल्ट `Kore`। पुराने उपनाम: `voiceName`, `voice`।</ParamField>
  ...
  <Accordion title="Inworld">
    ...
    <ParamField path="modelId" type="string">डिफ़ॉल्ट `inworld-tts-1.5-max`। अन्य: `inworld-tts-1.5-mini`, `inworld-tts-1-max`, `inworld-tts-1`।</ParamField>
  ...
  <Accordion title="स्थानीय CLI (tts-local-cli)">
    ...
    <ParamField path="outputFormat" type='"mp3" | "opus" | "wav"'>अपेक्षित CLI आउटपुट फ़ॉर्मेट। ऑडियो अटैचमेंट के लिए डिफ़ॉल्ट `mp3`।</ParamField>
    ...
    कमांड stdout तथा जनरेट या कन्वर्ट किया गया ऑडियो 50 MiB तक सीमित हैं। डायग्नोस्टिक stderr 1 MiB तक सीमित है। दोनों में से किसी भी सीमा के पार होने पर OpenClaw कमांड समाप्त कर देता है और संश्लेषण विफल हो जाता है।

  </Accordion>

  <Accordion title="Microsoft (कोई API कुंजी नहीं)">
    <ParamField path="enabled" type="boolean" default="true">Microsoft स्पीच के उपयोग की अनुमति दें।</ParamField>
    <ParamField path="speakerVoice" type="string">Microsoft न्यूरल वॉइस का नाम (जैसे `en-US-MichelleNeural`)। पुराना उपनाम: `voice`। यदि डिफ़ॉल्ट अंग्रेज़ी वॉइस प्रभावी है और उत्तर का टेक्स्ट मुख्यतः CJK है, तो OpenClaw स्वतः `zh-CN-XiaoxiaoNeural` पर स्विच करता है।</ParamField>
    <ParamField path="lang" type="string">भाषा कोड (जैसे `en-US`)।</ParamField>
    <ParamField path="outputFormat" type="string">Microsoft आउटपुट फ़ॉर्मेट। डिफ़ॉल्ट `audio-24khz-48kbitrate-mono-mp3`। बंडल किए गए Edge-समर्थित ट्रांसपोर्ट द्वारा सभी फ़ॉर्मेट समर्थित नहीं हैं।</ParamField>
    <ParamField path="rate / pitch / volume" type="string">प्रतिशत स्ट्रिंग (जैसे `+10%`, `-5%`)।</ParamField>
    ...
    <ParamField path="edge.*" type="object" deprecated>पुराना उपनाम। सहेजे गए कॉन्फ़िगरेशन को `providers.microsoft` में फिर से लिखने के लिए `openclaw doctor --fix` चलाएँ।</ParamField>
  </Accordion>
  ...
  <Accordion title="MiniMax">
    ...
    <ParamField path="speakerVoiceId" type="string">डिफ़ॉल्ट `English_expressive_narrator`। Env: `MINIMAX_TTS_VOICE_ID`। पुराना उपनाम: `voiceId`।</ParamField>
  ...
  <Accordion title="OpenAI">
    ...
    <ParamField path="speakerVoice" type="string">वॉइस का नाम (जैसे `alloy`, `cedar`)। डिफ़ॉल्ट `coral`। पुराना उपनाम: `voice`।</ParamField>
    <ParamField path="instructions" type="string">स्पष्ट OpenAI `instructions` फ़ील्ड। इसे सेट करने पर पर्सोना प्रॉम्प्ट फ़ील्ड स्वतः मैप **नहीं** होते।</ParamField>
    <ParamField path="extraBody / extra_body" type="Record<string, unknown>">जनरेट किए गए OpenAI TTS फ़ील्ड के बाद `/audio/speech` अनुरोध बॉडी में मर्ज किए गए अतिरिक्त JSON फ़ील्ड। इसका उपयोग Kokoro जैसे OpenAI-संगत एंडपॉइंट के लिए करें, जिन्हें `lang` जैसी प्रदाता-विशिष्ट कुंजियाँ चाहिए; असुरक्षित प्रोटोटाइप कुंजियों को अनदेखा किया जाता है।</ParamField>
    <ParamField path="baseUrl" type="string">
      OpenAI TTS एंडपॉइंट को ओवरराइड करें। समाधान क्रम: कॉन्फ़िगरेशन → `OPENAI_TTS_BASE_URL` → `https://api.openai.com/v1`। गैर-डिफ़ॉल्ट मानों को OpenAI-संगत TTS एंडपॉइंट माना जाता है, इसलिए कस्टम मॉडल और वॉइस नाम स्वीकार किए जाते हैं, और `speed` की `0.25..4.0` सीमा जाँच हट जाती है।
    </ParamField>
  </Accordion>
  ...
  <Accordion title="Volcengine (BytePlus Seed Speech)">
    ...
    <ParamField path="resourceId" type="string">डिफ़ॉल्ट `seed-tts-1.0`। Env: `VOLCENGINE_TTS_RESOURCE_ID`। जब आपके प्रोजेक्ट को TTS 2.0 की पात्रता प्राप्त हो, तब `seed-tts-2.0` का उपयोग करें।</ParamField>
    ...
    <ParamField path="speakerVoice" type="string">वॉइस प्रकार। डिफ़ॉल्ट `en_female_anna_mars_bigtts`। Env: `VOLCENGINE_TTS_VOICE`। पुराना उपनाम: `voice`।</ParamField>
    ...
    <ParamField path="appId / token / cluster" type="string" deprecated>पुराने Volcengine Speech Console फ़ील्ड। Env: `VOLCENGINE_TTS_APPID`, `VOLCENGINE_TTS_TOKEN`, `VOLCENGINE_TTS_CLUSTER` (डिफ़ॉल्ट `volcano_tts`)।</ParamField>
  </Accordion>

  <Accordion title="xAI">
    <ParamField path="apiKey" type="string">Env: `XAI_API_KEY`।</ParamField>
    <ParamField path="baseUrl" type="string">डिफ़ॉल्ट `https://api.x.ai/v1`। Env: `XAI_BASE_URL`।</ParamField>
    <ParamField path="speakerVoiceId" type="string">डिफ़ॉल्ट `eve`। प्रमाणीकरण के साथ, `openclaw infer tts voices --provider xai` वर्तमान अंतर्निर्मित कैटलॉग प्राप्त करता है; प्रमाणीकरण के बिना यह ऑफ़लाइन फ़ॉलबैक `ara`, `eve`, `leo`, `rex`, और `sal` सूचीबद्ध करता है। खाते की कस्टम वॉइस आईडी अंतर्निर्मित सूची में अनुपस्थित होने पर भी अग्रेषित की जाती हैं। पुराना उपनाम: `voiceId`।</ParamField>
  ...
  <Accordion title="Xiaomi MiMo">
    ...
    <ParamField path="model" type="string">डिफ़ॉल्ट `mimo-v2.5-tts`। Env: `XIAOMI_TTS_MODEL`। `mimo-v2-tts` और `mimo-v2.5-tts-voicedesign` भी समर्थित हैं।</ParamField>
    <ParamField path="speakerVoice" type="string">प्रीसेट-वॉइस मॉडल के लिए डिफ़ॉल्ट `mimo_default`। Env: `XIAOMI_TTS_VOICE`। पुराना उपनाम: `voice`। `mimo-v2.5-tts-voicedesign` के लिए नहीं भेजा जाता।</ParamField>
...
एक ऑडियो अटैचमेंट लौटाता है। Feishu, Matrix, Telegram, और WhatsApp पर ऑडियो
...
भेजता है, क्योंकि क्लाइंट वॉइस नोट पर कैप्शन निरंतर रूप से रेंडर नहीं करते।

टूल वैकल्पिक `channel` और `timeoutMs` फ़ील्ड स्वीकार करता है; `timeoutMs`
प्रति-कॉल प्रदाता अनुरोध टाइमआउट है, मिलीसेकंड में। प्रति-कॉल मान
`messages.tts.timeoutMs` को ओवरराइड करते हैं; कॉन्फ़िगर किए गए TTS टाइमआउट Plugin द्वारा लिखे गए
किसी भी प्रदाता डिफ़ॉल्ट को ओवरराइड करते हैं।
