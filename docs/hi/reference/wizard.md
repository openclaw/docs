...
x-i18n:
    generated_at: "2026-07-16T17:14:45Z"
    model: gpt-5.6
...
<Steps>
  <Step title="रीसेट (वैकल्पिक)">
    - `--reset` सेटअप चलने से पहले स्थिति रीसेट करता है; इसके बिना, ऑनबोर्डिंग दोबारा चलाने पर
      मौजूदा कॉन्फ़िगरेशन बना रहता है और उसे डिफ़ॉल्ट के रूप में फिर से उपयोग किया जाता है।
    - `--reset-scope` नियंत्रित करता है कि `--reset` क्या हटाता है: `config` (केवल कॉन्फ़िगरेशन फ़ाइल
      ), `config+creds+sessions` (डिफ़ॉल्ट), या `full` (वर्कस्पेस भी हटाता
      है)।
  ...
  <Step title="जोखिम की स्वीकृति">
    - पहली बार चलाने पर (या `wizard.securityAcknowledgedAt` सेट होने से पहले किसी भी बार)
      आपसे यह पुष्टि करने के लिए कहा जाता है कि आप समझते हैं कि एजेंट शक्तिशाली होते हैं और पूर्ण
      सिस्टम एक्सेस जोखिमपूर्ण है।
    - `--non-interactive` के लिए `--accept-risk` स्पष्ट रूप से आवश्यक है; इसके बिना,
      ऑनबोर्डिंग संकेत देने के बजाय त्रुटि के साथ बंद हो जाती है।
    - इंटरैक्टिव रन में फ़्लैग के बजाय पुष्टि का संकेत मिलता है; अस्वीकार करने पर
      सेटअप रद्द हो जाता है।

  </Step>
  <Step title="मॉडल/प्रमाणीकरण">
    - **Anthropic API कुंजी**: मौजूद होने पर `ANTHROPIC_API_KEY` का उपयोग करता है या कुंजी माँगता है, फिर उसे डेमन के उपयोग के लिए सहेजता है।
    - **Anthropic Claude CLI**: जब Claude CLI साइन-इन पहले से मौजूद हो, तो यह पसंदीदा स्थानीय पथ है; OpenClaw अब भी विकल्प के रूप में Anthropic सेटअप-टोकन प्रमाणीकरण का समर्थन करता है।
    ...
    - **OpenAI Code (Codex) सदस्यता (डिवाइस पेयरिंग)**: अल्पकालिक डिवाइस कोड वाला ब्राउज़र पेयरिंग प्रवाह।
      - बिना प्राथमिक मॉडल वाले नए सेटअप में, Codex रनटाइम के माध्यम से `agents.defaults.model` को `openai/gpt-5.6-sol` पर सेट करता है।
    - **OpenAI API कुंजी**: मौजूद होने पर `OPENAI_API_KEY` का उपयोग करता है या कुंजी माँगता है, फिर उसे प्रमाणीकरण प्रोफ़ाइल में संग्रहीत करता है।
      - बिना प्राथमिक मॉडल वाले नए सेटअप में, `agents.defaults.model` को `openai/gpt-5.6` पर सेट करता है; केवल प्रत्यक्ष-API मॉडल आईडी Sol स्तर पर रिज़ॉल्व होती है।
    - OpenAI जोड़ने या उसका पुनः प्रमाणीकरण करने पर `openai/gpt-5.5` सहित मौजूदा स्पष्ट प्राथमिक मॉडल सुरक्षित रहता है। यदि खाते में GPT-5.6 उपलब्ध नहीं है, तो `openai/gpt-5.5` स्पष्ट रूप से चुनें; OpenClaw मॉडल को चुपचाप निम्न स्तर पर नहीं ले जाता।
    - **xAI OAuth**: बिना किसी लोकलहोस्ट कॉलबैक की आवश्यकता वाला डिवाइस-कोड ब्राउज़र साइन-इन, इसलिए यह SSH/Docker/VPS पर भी काम करता है (`--auth-choice xai-oauth`)।
    - **xAI API कुंजी**: `XAI_API_KEY` (`--auth-choice xai-api-key`) माँगता है।
    ...
    - **Ollama**: पहले **क्लाउड + स्थानीय**, **केवल क्लाउड**, या **केवल स्थानीय** विकल्प देता है। `Cloud only`, `OLLAMA_API_KEY` माँगता है और `https://ollama.com` का उपयोग करता है; होस्ट-समर्थित मोड Ollama बेस URL (डिफ़ॉल्ट `http://127.0.0.1:11434`) माँगते हैं, उपलब्ध मॉडल खोजते हैं और आवश्यकता होने पर चुने गए स्थानीय मॉडल को स्वचालित रूप से पुल करते हैं; `Cloud + Local` यह भी जाँचता है कि वह Ollama होस्ट क्लाउड एक्सेस के लिए साइन-इन है या नहीं।
    ...
    - **Cloudflare AI Gateway**: खाता आईडी, Gateway आईडी और `CLOUDFLARE_AI_GATEWAY_API_KEY` माँगता है।
    - अधिक विवरण: [Cloudflare AI Gateway](/hi/providers/cloudflare-ai-gateway)
    - **MiniMax**: कॉन्फ़िगरेशन अपने-आप लिखा जाता है; होस्ट किया गया डिफ़ॉल्ट `MiniMax-M3` है।
    ...
    - **StepFun**: चीन या वैश्विक एंडपॉइंट पर StepFun मानक या Step Plan के लिए कॉन्फ़िगरेशन अपने-आप लिखा जाता है।
    ...
    - **Moonshot (Kimi K2)**: कॉन्फ़िगरेशन अपने-आप लिखा जाता है।
    - **Kimi Coding**: कॉन्फ़िगरेशन अपने-आप लिखा जाता है।
    - अधिक विवरण: [Moonshot AI (Kimi + Kimi Coding)](/hi/providers/moonshot)
    - **कस्टम प्रदाता**: OpenAI-संगत, OpenAI Responses-संगत या Anthropic-संगत एंडपॉइंट के साथ काम करता है। गैर-इंटरैक्टिव फ़्लैग: `--auth-choice custom-api-key`, `--custom-base-url`, `--custom-model-id`, `--custom-api-key` (वैकल्पिक; `CUSTOM_API_KEY` पर फ़ॉलबैक करता है), `--custom-provider-id` (वैकल्पिक; बेस URL से अपने-आप निकाला जाता है), `--custom-compatibility openai|openai-responses|anthropic` (डिफ़ॉल्ट `openai`), `--custom-image-input` / `--custom-text-input` (अनुमानित विज़न-मॉडल पहचान को ओवरराइड करता है)।
    - **छोड़ें**: अभी कोई प्रमाणीकरण कॉन्फ़िगर नहीं किया गया है।
    ...
    - API कुंजी संग्रहण मोड का डिफ़ॉल्ट सादे टेक्स्ट वाले प्रमाणीकरण-प्रोफ़ाइल मान हैं। इसके बजाय परिवेश-समर्थित संदर्भ संग्रहीत करने के लिए `--secret-input-mode ref` का उपयोग करें (उदाहरण के लिए `keyRef: { source: "env", provider: "default", id: "OPENAI_API_KEY" }`); संदर्भित परिवेश चर पहले से सेट होना चाहिए, अन्यथा ऑनबोर्डिंग तुरंत विफल हो जाती है।
    - प्रमाणीकरण प्रोफ़ाइल `~/.openclaw/agents/<agentId>/agent/auth-profiles.json` (API कुंजियाँ + OAuth) में रहती हैं। `~/.openclaw/credentials/oauth.json` केवल लीगेसी आयात के लिए है।
  ...
  <Step title="Gateway">
    ...
    - प्रमाणीकरण अनुशंसा: लूपबैक के लिए भी **टोकन** बनाए रखें, ताकि स्थानीय WS क्लाइंट को प्रमाणीकरण करना पड़े।
    - टोकन मोड में, इंटरैक्टिव सेटअप ये विकल्प देता है:
      ...
      - क्विकस्टार्ट ऑनबोर्डिंग प्रोब/डैशबोर्ड बूटस्ट्रैप के लिए `env`, `file` और `exec` प्रदाताओं में मौजूदा `gateway.auth.token` SecretRefs का दोबारा उपयोग करता है।
      - यदि वह SecretRef कॉन्फ़िगर है लेकिन रिज़ॉल्व नहीं हो सकता, तो ऑनबोर्डिंग रनटाइम प्रमाणीकरण को चुपचाप कमज़ोर करने के बजाय सुधार का स्पष्ट संदेश देकर जल्दी विफल हो जाती है।
    ...
    - प्रमाणीकरण केवल तभी अक्षम करें जब आपको हर स्थानीय प्रक्रिया पर पूरा भरोसा हो।
  ...
  <Step title="चैनल">
    ...
    - [Mattermost](/hi/channels/mattermost) (plugin): बॉट टोकन + बेस URL।
    ...
    - Discord, Feishu, Microsoft Teams, QQ Bot, Slack और अन्य चैनल
      plugins के रूप में उपलब्ध होते हैं, जिन्हें ऑनबोर्डिंग आपके लिए इंस्टॉल कर सकती है। पूरी सूची: [चैनल](/hi/channels)।
    - DM सुरक्षा: डिफ़ॉल्ट पेयरिंग है। पहला DM एक कोड भेजता है; `openclaw pairing approve <channel> <code>` के माध्यम से स्वीकृत करें या अनुमति-सूचियों का उपयोग करें।

  </Step>
  <Step title="वेब खोज">
    - Brave, Codex (Hosted Search), DuckDuckGo, Exa, Firecrawl, Gemini, Grok, Kimi, MiniMax Search, Ollama Web Search, Parallel, Perplexity, SearXNG या Tavily जैसा समर्थित प्रदाता चुनें (या छोड़ दें)।
    - API-समर्थित प्रदाता त्वरित सेटअप के लिए परिवेश चर या मौजूदा कॉन्फ़िगरेशन का उपयोग कर सकते हैं; कुंजी-रहित प्रदाता इसके बजाय अपने प्रदाता-विशिष्ट पूर्वापेक्षाओं का उपयोग करते हैं।
    - `--skip-search` के साथ छोड़ें।
    - बाद में कॉन्फ़िगर करें: `openclaw configure --section web`।

  </Step>
  <Step title="डेमन इंस्टॉल">
    - macOS: LaunchAgent
      - लॉग-इन उपयोगकर्ता सत्र आवश्यक है; हेडलेस के लिए, कस्टम LaunchDaemon का उपयोग करें (इसके साथ वितरित नहीं किया जाता)।
    - Linux (और WSL2 के माध्यम से Windows): systemd उपयोगकर्ता यूनिट
      - ऑनबोर्डिंग `loginctl enable-linger <user>` के माध्यम से लिंगरिंग सक्षम करने की कोशिश करती है, ताकि लॉगआउट के बाद भी Gateway चलता रहे।
      - sudo के लिए संकेत दे सकती है (`/var/lib/systemd/linger` लिखती है); पहले sudo के बिना प्रयास करती है।
    - नेटिव Windows: पहले Scheduled Task; यदि टास्क निर्माण अस्वीकृत हो जाता है, तो OpenClaw प्रति-उपयोगकर्ता Startup-फ़ोल्डर लॉगिन आइटम पर फ़ॉलबैक करता है और Gateway को तुरंत शुरू करता है।
    - **रनटाइम चयन:** Node आवश्यक है क्योंकि प्रामाणिक रनटाइम स्थिति संग्रह `node:sqlite` का उपयोग करता है। लीगेसी Bun सेवाएँ सुधार के दौरान Node पर माइग्रेट की जाती हैं।
    - यदि टोकन प्रमाणीकरण के लिए टोकन आवश्यक है और `gateway.auth.token` को SecretRef प्रबंधित करता है, तो डेमन इंस्टॉल उसे सत्यापित करता है, लेकिन रिज़ॉल्व किए गए सादे टेक्स्ट टोकन मानों को सुपरवाइज़र सेवा परिवेश मेटाडेटा में बनाए नहीं रखता।
    - यदि टोकन प्रमाणीकरण के लिए टोकन आवश्यक है और कॉन्फ़िगर किया गया टोकन SecretRef अनरिज़ॉल्व्ड है, तो डेमन इंस्टॉल कार्रवाई योग्य मार्गदर्शन के साथ अवरुद्ध हो जाता है।
    - यदि `gateway.auth.token` और `gateway.auth.password` दोनों कॉन्फ़िगर हैं और `gateway.auth.mode` सेट नहीं है, तो मोड स्पष्ट रूप से सेट किए जाने तक डेमन इंस्टॉल अवरुद्ध रहता है।

  </Step>
  <Step title="स्वास्थ्य जाँच">
    - Gateway शुरू करता है (यदि आवश्यक हो) और `openclaw health` चलाता है।
    - सुझाव: `openclaw status --deep` स्थिति आउटपुट में लाइव Gateway स्वास्थ्य प्रोब जोड़ता है, जिसमें समर्थित होने पर चैनल प्रोब भी शामिल होते हैं (एक पहुँच योग्य Gateway आवश्यक है)।
  ...
  <Step title="Skills (अनुशंसित)">
    ...
    - विश्वसनीय बंडल किए गए Skills के लिए वैकल्पिक निर्भरताएँ अपने-आप इंस्टॉल करता है (कुछ macOS पर Homebrew का उपयोग करते हैं)।
  ...
  <Step title="समाप्ति">
    - सारांश + अगले चरण, जिसमें Terminal, Browser या बाद के लिए **आप अपने एजेंट को किस तरह शुरू करना चाहते हैं?** संकेत शामिल है।
...
यदि Control UI एसेट अनुपलब्ध हैं, तो ऑनबोर्डिंग उन्हें बिल्ड करने का प्रयास करती है; फ़ॉलबैक `pnpm ui:build` है (UI निर्भरताएँ अपने-आप इंस्टॉल करता है)।
</Note>

## गैर-इंटरैक्टिव मोड

ऑनबोर्डिंग को स्वचालित करने या स्क्रिप्ट के माध्यम से चलाने के लिए `--non-interactive --accept-risk` का उपयोग करें (यह
...
- नेटिव Windows: अभी समर्थित नहीं है; Linux इंस्टॉल पथ पाने के लिए WSL2 के भीतर ऑनबोर्डिंग चलाएँ।
- दोनों स्थितियों में आपके कॉन्फ़िग में `channels.signal.cliPath` लिखता है।
...
- `session.dmScope` (सेट न होने पर स्थानीय ऑनबोर्डिंग इसका डिफ़ॉल्ट `"per-channel-peer"` निर्धारित करती है; मौजूदा स्पष्ट मान संरक्षित रहते हैं। विवरण: [CLI सेटअप संदर्भ](/hi/start/wizard-cli-reference#outputs-and-internals))
- `channels.telegram.botToken`, `channels.discord.token`, `channels.matrix.*`, `channels.signal.*`, `channels.imessage.*`
- चैनल प्रॉम्प्ट के दौरान सहमति देने पर चैनल DM अनुमति-सूचियाँ। जहाँ संभव हो, Discord, Matrix, Microsoft Teams और Slack नामों को आईडी में बदलते हैं; अन्य चैनल सीधे आईडी लेते हैं (उदाहरण के लिए संख्यात्मक Telegram प्रेषक आईडी या WhatsApp फ़ोन नंबर)।
- `skills.install.nodeManager`
  - `setup --node-manager`, `npm`, `pnpm`, या `bun` स्वीकार करता है।
  - मैन्युअल कॉन्फ़िग में `skills.install.nodeManager` को सीधे सेट करके अब भी `yarn` का उपयोग किया जा सकता है।
- `wizard.lastRunAt`
...
इनपुट और अभिलेख/सहायता आर्टिफ़ैक्ट के लिए किया जाता है।

कुछ चैनल plugins के रूप में उपलब्ध कराए जाते हैं। सेटअप के दौरान किसी को चुनने पर, ऑनबोर्डिंग
उसे कॉन्फ़िगर करने से पहले इंस्टॉल करने (npm या स्थानीय पथ से) के लिए कहेगी।
