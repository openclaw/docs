---
read_when:
    - आप OpenClaw मॉडल उपयोग, संदेश प्रवाह या सत्र मेट्रिक्स को किसी OpenTelemetry कलेक्टर को भेजना चाहते हैं
...
summary: diagnostics-otel Plugin के माध्यम से OpenClaw डायग्नोस्टिक्स को OpenTelemetry कलेक्टरों या stdout JSONL में निर्यात करें
title: OpenTelemetry निर्यात
x-i18n:
    generated_at: "2026-07-19T08:45:10Z"
    model: gpt-5.6
...
जिसमें **OTLP/HTTP (protobuf)** का उपयोग होता है। कंटेनर और सैंडबॉक्स लॉग
पाइपलाइनों के लिए लॉग stdout JSONL के रूप में भी लिखे जा सकते हैं। OTLP/HTTP
स्वीकार करने वाला कोई भी कलेक्टर या बैकएंड कोड में बदलाव किए बिना काम करता है।
स्थानीय फ़ाइल लॉग के लिए, [लॉगिंग](/hi/logging) देखें।

- **निदान इवेंट** संरचित, इन-प्रोसेस रिकॉर्ड हैं, जिन्हें मॉडल रन,
  संदेश प्रवाह, सत्रों, कतारों और exec के लिए Gateway तथा बंडल किए गए plugins
  उत्सर्जित करते हैं।
- **`diagnostics-otel`** उन इवेंट की सदस्यता लेता है और उन्हें
  OTLP/HTTP पर OpenTelemetry **मेट्रिक्स**, **ट्रेस** और **लॉग** के रूप में निर्यात
  करता है तथा लॉग रिकॉर्ड को stdout JSONL में प्रतिबिंबित कर सकता है।
- **प्रोवाइडर कॉल** को OpenClaw के विश्वसनीय मॉडल-कॉल स्पैन संदर्भ से
  W3C `traceparent` हेडर मिलता है, जब प्रोवाइडर ट्रांसपोर्ट कस्टम हेडर स्वीकार
  करता है। Plugin द्वारा उत्सर्जित ट्रेस संदर्भ प्रसारित नहीं किया जाता।
...
<Note>
`protocol` केवल `http/protobuf` का समर्थन करता है। चूँकि `traces` और `metrics` डिफ़ॉल्ट रूप से सक्षम होते हैं, कोई भी अन्य मान (`grpc` सहित) `unsupported protocol` चेतावनी के साथ पूरी diagnostics-otel सदस्यता को निरस्त कर देता है—इससे stdout लॉग निर्यात भी रुक जाता है। यदि आप गैर-OTLP प्रोटोकॉल मान के साथ केवल `logsExporter: "stdout"` चाहते हैं, तो `traces: false` और `metrics: false` को स्पष्ट रूप से सेट करें।
</Note>
...
| ----------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| **मेट्रिक्स** | टोकन उपयोग, लागत, रन अवधि, फ़ेलओवर, skill उपयोग, संदेश प्रवाह, Talk इवेंट, कतार लेन, सत्र स्थिति/पुनर्प्राप्ति, टूल निष्पादन, exec, मेमोरी, जीवंतता और निर्यातक स्वास्थ्य के काउंटर/हिस्टोग्राम। |
| **ट्रेस**  | मॉडल उपयोग, मॉडल कॉल, हार्नेस जीवनचक्र, skill उपयोग, टूल निष्पादन, exec, webhook/संदेश प्रसंस्करण, संदर्भ संयोजन और टूल लूप के स्पैन।                                                      |
...
डिफ़ॉल्ट रूप से OTLP का उपयोग करता है; stdout पर JSONL के लिए `diagnostics.otel.logsExporter` को
`stdout` पर या दोनों के लिए `both` पर सेट करें।
...
| `OTEL_EXPORTER_OTLP_TRACES_ENDPOINT` / `OTEL_EXPORTER_OTLP_METRICS_ENDPOINT` / `OTEL_EXPORTER_OTLP_LOGS_ENDPOINT` | मिलती हुई `diagnostics.otel.*Endpoint` कॉन्फ़िगरेशन कुंजी सेट न होने पर उपयोग किए जाने वाले सिग्नल-विशिष्ट एंडपॉइंट फ़ॉलबैक। सिग्नल-विशिष्ट कॉन्फ़िगरेशन को सिग्नल-विशिष्ट env पर प्राथमिकता मिलती है और सिग्नल-विशिष्ट env को साझा एंडपॉइंट पर।                                                                                                         |
...
| `OTEL_SEMCONV_STABILITY_OPT_IN`                                                                                   | नवीनतम GenAI इनफ़रेंस स्पैन स्वरूप उत्सर्जित करने के लिए `gen_ai_latest_experimental` पर सेट करें: `{gen_ai.operation.name} {gen_ai.request.model}` स्पैन नाम, `CLIENT` स्पैन प्रकार और पुराने `gen_ai.system` के बजाय `gen_ai.provider.name`। इसके बावजूद GenAI मेट्रिक्स हमेशा सीमित, कम-कार्डिनैलिटी विशेषताओं का उपयोग करते हैं। |
| `OPENCLAW_OTEL_PRELOADED`                                                                                         | जब कोई अन्य प्रीलोड या होस्ट प्रक्रिया पहले ही वैश्विक OpenTelemetry SDK पंजीकृत कर चुकी हो, तो `1` पर सेट करें। इसके बाद plugin अपना NodeSDK जीवनचक्र छोड़ देता है, लेकिन फिर भी निदान लिसनर जोड़ता है और `traces`/`metrics`/`logs` का पालन करता है।                                                                                    |

## गोपनीयता और सामग्री कैप्चर

कच्ची मॉडल/टूल सामग्री डिफ़ॉल्ट रूप से निर्यात **नहीं** की जाती। स्पैन में सीमित
पहचानकर्ता (चैनल, प्रोवाइडर, मॉडल, त्रुटि श्रेणी, केवल-हैश अनुरोध आईडी,
टूल स्रोत, टूल स्वामी, skill नाम/स्रोत) होते हैं और उनमें कभी भी प्रॉम्प्ट टेक्स्ट,
प्रतिक्रिया टेक्स्ट, टूल इनपुट, टूल आउटपुट, skill फ़ाइल पथ या सत्र कुंजियाँ शामिल
नहीं होतीं। स्कोप्ड एजेंट सत्र कुंजियों जैसे दिखने वाले मान (उदाहरण के लिए
...
स्वच्छ की गई विशेषताएँ बनाए रखते हैं; कच्ची लॉग संदेश बॉडी केवल
तभी निर्यात की जाती है, जब `diagnostics.otel.captureContent` बूलियन `true` हो। सूक्ष्म
`captureContent.*` उपकुंजियाँ कभी भी लॉग बॉडी सक्षम नहीं करतीं। Talk मेट्रिक्स केवल
सीमित इवेंट मेटाडेटा (मोड, ट्रांसपोर्ट, प्रोवाइडर, इवेंट प्रकार) निर्यात करते हैं—
कोई ट्रांसक्रिप्ट, ऑडियो पेलोड, सत्र आईडी, टर्न आईडी, कॉल आईडी, रूम आईडी या
हैंडऑफ़ टोकन नहीं।

आउटबाउंड मॉडल अनुरोधों में W3C `traceparent` हेडर शामिल हो सकता है, जो केवल
सक्रिय मॉडल कॉल के OpenClaw-स्वामित्व वाले निदान ट्रेस संदर्भ से जनरेट होता है।
कॉलर द्वारा दिए गए मौजूदा `traceparent` हेडर बदल दिए जाते हैं, इसलिए plugins
या कस्टम प्रोवाइडर विकल्प क्रॉस-सर्विस ट्रेस वंशावली की जालसाज़ी नहीं कर सकते।

`diagnostics.otel.captureContent.*` को `true` पर केवल तभी सेट करें, जब आपका कलेक्टर
और प्रतिधारण नीति प्रॉम्प्ट, प्रतिक्रिया, टूल या सिस्टम-प्रॉम्प्ट टेक्स्ट के लिए
स्वीकृत हों। प्रत्येक उपकुंजी स्वतंत्र है:
...
जब कोई उपकुंजी सक्षम होती है, तो मॉडल और टूल स्पैन को केवल उस वर्ग के लिए सीमित,
संशोधित `openclaw.content.*` विशेषताएँ मिलती हैं।
...
`toolInputs`/`toolOutputs` सामग्री बिल्ट-इन एजेंट
रनटाइम के टूल निष्पादनों के लिए कैप्चर की जाती है (पूर्ण/त्रुटि स्पैन पर
`openclaw.content.tool_input` और `gen_ai.tool.call.arguments`;
पूर्ण स्पैन पर `openclaw.content.tool_output` और
`gen_ai.tool.call.result`)। `openclaw.content.*` नाम स्थिर OpenClaw विशेषता
नाम बने रहते हैं; `gen_ai.tool.call.*` प्रतियाँ semconv-मूल व्यूअर के लिए उन्हें प्रतिबिंबित करती हैं।
बाहरी हार्नेस टूल कॉल (Codex, Claude CLI) सामग्री पेलोड के बिना
`tool.execution.*` स्पैन उत्सर्जित करते हैं। कैप्चर की गई सामग्री एक विश्वसनीय,
केवल-लिसनर चैनल पर जाती है और उसे कभी भी सार्वजनिक निदान इवेंट बस पर नहीं रखा
जाता।
...
- **लॉग:** OTLP लॉग `logging.level` (फ़ाइल लॉग स्तर) का पालन करते हैं और
  कंसोल फ़ॉर्मैटिंग के बजाय डायग्नोस्टिक लॉग-रिकॉर्ड रिडैक्शन पथ का उपयोग करते हैं। अधिक वॉल्यूम वाले
  इंस्टॉलेशन में स्थानीय सैंपलिंग के बजाय OTLP कलेक्टर सैंपलिंग/फ़िल्टरिंग को
  प्राथमिकता देनी चाहिए। जब आपका प्लेटफ़ॉर्म पहले से
  stdout/stderr को किसी लॉग प्रोसेसर को भेजता हो और आपके पास कोई OTLP लॉग
  ...
  `service.name`, गंभीरता, मुख्य सामग्री, रिडैक्ट किए गए एट्रिब्यूट और उपलब्ध होने पर विश्वसनीय ट्रेस
  फ़ील्ड होते हैं।
...
- **मॉडल-कॉल सहसंबंध:** `openclaw.model.call` स्पैन में डिफ़ॉल्ट रूप से सुरक्षित प्रॉम्प्ट
  घटकों के आकार और, प्रदाता परिणाम द्वारा उपयोग उपलब्ध कराने पर, प्रति-कॉल टोकन एट्रिब्यूट शामिल होते हैं। `openclaw.model.usage` समग्र लागत, संदर्भ और चैनल डैशबोर्ड के लिए रन-स्तरीय
  लेखांकन स्पैन बना रहता है और उत्सर्जन करने वाले रनटाइम में विश्वसनीय
  ट्रेस संदर्भ होने पर उसी डायग्नोस्टिक ट्रेस पर रहता है।

### मॉडल-कॉल अवलोकन इकाइयाँ

हर `openclaw.model.call` स्पैन `openclaw.model_call.observation_unit` के माध्यम से बताता है कि उसका जीवनचक्र क्या मापता है:

- `request` - एक अवलोकनीय मॉडल/प्रदाता अनुरोध। मूल एम्बेडेड मॉडल
  कॉल इस इकाई का उपयोग करते हैं और पुराने या बाहरी उत्सर्जकों के साथ संगतता के लिए
  निर्यातक अनुपस्थित मान को `request` मानते हैं।
- `turn` - एक अपारदर्शी एजेंट CLI टर्न, जिसमें छिपे हुए मॉडल अनुरोध,
  पुनः प्रयास, टूल कार्य या पृष्ठभूमि कार्य हो सकते हैं। Claude Code CLI और Codex app-server
  कॉल इस इकाई का उपयोग करते हैं।

दोनों इकाइयाँ मॉडल-कॉल स्पैन बनी रहती हैं, ताकि ट्रेस बैकएंड मॉडल इनपुट,
आउटपुट, उपयोग और पदानुक्रम प्रस्तुत कर सकें। अनुरोध स्पैन API से व्युत्पन्न GenAI ऑपरेशन
...
मेट्रिक्स में `openclaw.model_call.observation_unit` भी शामिल होता है; Prometheus
मॉडल-कॉल मेट्रिक्स समकक्ष `observation_unit` लेबल प्रदर्शित करते हैं।

### Claude Code CLI मॉडल-कॉल निष्ठा
...
कोई अवलोकनीयता Plugin या अन्य लिसनर सक्रिय न होने पर Claude CLI टर्न
...
प्रत्येक 128 KiB तक सीमित होते हैं; सहायक आउटपुट अधिकतम 200 एनवेलप में कुल 128 KiB तक
सीमित होता है, जिसमें अंतिम दृश्यमान
फ़ॉलबैक प्रतिक्रिया के लिए 16 KiB और एक आइटम आरक्षित रहता है। सीमा पूरी होने पर एक मार्कर ट्रंकेशन दर्ज करता है।
...
पदानुक्रम का उपयोग करते हैं; वास्तविक नया-सत्र पुनः प्रयास उसी OpenClaw रन के भीतर
एक और मॉडल-कॉल चाइल्ड बनाता है।

स्पैन तब शुरू होता है, जब OpenClaw तैयार CLI टर्न को स्वीकार करता है, और केवल
उस टर्न के सफल या विफल होने के बाद समाप्त होता है। प्रबंधित सत्रों के लिए, जब Claude परिणाम को रोके रखने वाले पृष्ठभूमि एजेंट या
वर्कफ़्लो की रिपोर्ट करता है, तब अंतरिम सफल परिणाम स्पैन समाप्त नहीं करता;
ड्रेन के बाद का अंतिम परिणाम इसे समाप्त करता है। निरस्तीकरण, समय-समाप्ति, प्रक्रिया विफलता,
आउटपुट/पार्स विफलता और अन्य टर्न विफलताएँ उसी स्पैन को त्रुटि के साथ समाप्त करती हैं।

Claude Code प्रति-सहायक-संदेश उपयोग की रिपोर्ट करता है और अपने टर्मिनल परिणाम पर संचयी
उपयोग की रिपोर्ट भी कर सकता है। OpenClaw उत्तर लेखांकन मौजूदा लागत अर्थविज्ञान को अपरिवर्तित रखने के लिए
अंतिम सहायक संदेश का उपयोग जारी रखता है;
टर्न-स्तरीय मॉडल-कॉल स्पैन उपलब्ध होने पर टर्मिनल संचयी उपयोग का उपयोग करता है,
...
- `openclaw.model_call.request_bytes` एक बार चलने वाले stdin/argv पर भेजे गए प्रॉम्प्ट मान
  या प्रबंधित stdio JSONL उपयोगकर्ता एनवेलप का UTF-8 आकार है। यह
  Claude Code के छिपे हुए मॉडल अनुरोध का आकार नहीं है।
- `openclaw.model_call.response_bytes` टर्न के दौरान देखे गए Claude CLI stdout का UTF-8 आकार है।
  यह Anthropic HTTP प्रतिक्रिया का आकार नहीं है।
...
मेल खाने वाले सूक्ष्म `captureContent` फ़ील्ड सक्षम होने पर, स्पैन
OpenClaw द्वारा Claude Code को भेजा गया प्रभावी प्रॉम्प्ट, OpenClaw का जोड़ा गया सिस्टम
प्रॉम्प्ट और `gen_ai.input.messages`, `gen_ai.output.messages` तथा
`gen_ai.system_instructions` के माध्यम से दृश्यमान सहायक टेक्स्ट/रीज़निंग/टूल-कॉल पहचान निर्यात करता है। टूल आर्ग्युमेंट, अपारदर्शी थिंकिंग सिग्नेचर और
टूल परिणाम Claude सहायक एनवेलप से हटा दिए जाते हैं। OpenClaw
Claude Code के निजी सिस्टम प्रॉम्प्ट, छिपे हुए पुनः आरंभ या
Compaction किए गए अनुरोध पेलोड, मूल आंतरिक टूल स्कीमा, अपरिष्कृत Anthropic HTTP
अनुरोध, आंतरिक पुनः प्रयास, अपस्ट्रीम अनुरोध आईडी या वास्तविक नेटवर्क TTFB तक पहुँच का
दावा नहीं करता। क्योंकि Claude Code अपनी प्रभावी मूल टूल परिभाषाएँ सटीक रूप से उपलब्ध नहीं कराता,
ये स्पैन `gen_ai.tool.definitions` को पॉप्युलेट नहीं करते।

बाहरी Claude हार्नेस टूल स्पैन, टूल सामग्री
कैप्चर सक्षम होने पर भी, केवल मेटाडेटा वाले बने रहते हैं। हर मॉडल स्पैन की तरह, कैप्चर की गई Claude CLI सामग्री
...
- `gen_ai.client.token.usage` (हिस्टोग्राम, GenAI अर्थगत-परंपरा मेट्रिक, एट्रिब्यूट: `gen_ai.token.type` = `input`/`output`, `gen_ai.provider.name`, `gen_ai.operation.name`, `gen_ai.request.model`)
- `gen_ai.client.operation.duration` (हिस्टोग्राम, सेकंड, मॉडल अनुरोधों और सिंथेटिक एजेंट टर्न के लिए GenAI अर्थगत-परंपरा मेट्रिक; एट्रिब्यूट: `gen_ai.provider.name`, `gen_ai.operation.name`, `gen_ai.request.model`, वैकल्पिक `error.type`; टर्न अवलोकन `gen_ai.operation.name = invoke_agent` का उपयोग करते हैं)
- `openclaw.model_call.duration_ms` (हिस्टोग्राम, एट्रिब्यूट: `openclaw.provider`, `openclaw.model`, `openclaw.api`, `openclaw.transport`, `openclaw.model_call.observation_unit`, और वर्गीकृत त्रुटियों पर `openclaw.errorCategory` तथा `openclaw.failureKind`)
- `openclaw.model_call.request_bytes` (हिस्टोग्राम, अंतिम मॉडल अनुरोध पेलोड का UTF-8 बाइट आकार; Claude Code CLI के लिए, ऊपर वर्णित अवलोकनीय प्रॉम्प्ट इनपुट/एनवेलप; कोई अपरिष्कृत पेलोड सामग्री नहीं)
- `openclaw.model_call.response_bytes` (हिस्टोग्राम, स्ट्रीम किए गए प्रतिक्रिया चंक पेलोड का UTF-8 बाइट आकार; उच्च-आवृत्ति टेक्स्ट, थिंकिंग और टूल-कॉल डेल्टा केवल वृद्धिशील `delta` बाइट गिनते हैं; Claude Code CLI के लिए, देखे गए stdout बाइट; कोई अपरिष्कृत प्रतिक्रिया सामग्री नहीं)
- `openclaw.model_call.time_to_first_byte_ms` (हिस्टोग्राम, पहले स्ट्रीम किए गए प्रतिक्रिया इवेंट से पूर्व बीता समय; Claude Code CLI के लिए, नेटवर्क TTFB के बजाय पहला अवलोकनीय CLI आउटपुट)
...
- `openclaw.talk.event.duration_ms` (हिस्टोग्राम, एट्रिब्यूट: `openclaw.talk.event` के समान; किसी Talk इवेंट द्वारा अवधि की रिपोर्ट किए जाने पर उत्सर्जित)
- `openclaw.talk.audio.bytes` (हिस्टोग्राम, एट्रिब्यूट: `openclaw.talk.event` के समान; बाइट लंबाई की रिपोर्ट करने वाले Talk ऑडियो फ़्रेम इवेंट के लिए उत्सर्जित)
...
- `openclaw.session.stuck` (काउंटर, विशेषताएँ: `openclaw.state`; पुनर्प्राप्ति योग्य पुराने सत्र की बहीखाता स्थिति के लिए उत्सर्जित)
- `openclaw.session.stuck_age_ms` (हिस्टोग्राम, विशेषताएँ: `openclaw.state`; पुनर्प्राप्ति योग्य पुराने सत्र की बहीखाता स्थिति के लिए उत्सर्जित)
...
- `openclaw.session.recovery.age_ms` (हिस्टोग्राम, विशेषताएँ: संबंधित पुनर्प्राप्ति काउंटर के समान)
- `openclaw.run.attempt` (काउंटर, विशेषताएँ: `openclaw.attempt`)

### सत्र सक्रियता टेलीमेट्री

`diagnostics.stuckSessionWarnMs` सत्र सक्रियता निदान के लिए प्रगति-रहित आयु सीमा है।
जब OpenClaw उत्तर, टूल, स्थिति, ब्लॉक या ACP रनटाइम की प्रगति देखता है, तब
`processing` सत्र इस सीमा की ओर पुराना नहीं होता। टाइपिंग कीपअलाइव को
प्रगति नहीं माना जाता, इसलिए मौन मॉडल या हार्नेस का फिर भी पता लगाया जा सकता है।

OpenClaw सत्रों को उस कार्य के आधार पर वर्गीकृत करता है जिसे वह अब भी देख सकता है:

- `session.long_running`: सक्रिय एम्बेडेड कार्य, मॉडल कॉल या टूल कॉल
  अब भी प्रगति कर रहे हैं। स्वामित्व वाले मॉडल कॉल जो
  `diagnostics.stuckSessionWarnMs` से अधिक समय तक मौन रहते हैं, वे
  `diagnostics.stuckSessionAbortMs` से पहले दीर्घकालिक के रूप में भी रिपोर्ट होते हैं,
  ताकि धीमे या गैर-स्ट्रीमिंग मॉडल प्रदाता, निरस्तीकरण देखे जा सकने तक,
  रुके हुए Gateway सत्र जैसे न दिखें।
- `session.stalled`: सक्रिय कार्य मौजूद है, लेकिन सक्रिय रन ने
  हाल की प्रगति की रिपोर्ट नहीं की है। स्वामित्व वाले मॉडल कॉल
  `diagnostics.stuckSessionAbortMs` पर या उसके बाद `session.long_running` से
  `session.stalled` पर स्विच होते हैं; स्वामी-रहित पुरानी मॉडल/टूल गतिविधि
  को निरापद दीर्घकालिक कार्य नहीं माना जाता।
  रुके हुए एम्बेडेड रन आरंभ में केवल अवलोकन की स्थिति में रहते हैं, फिर
  बिना प्रगति के `diagnostics.stuckSessionAbortMs` के बाद निरस्तीकरण-निकासी करते हैं, ताकि
  लेन के पीछे कतारबद्ध टर्न फिर से शुरू हो सकें। सेट न होने पर, निरस्तीकरण
  सीमा डिफ़ॉल्ट रूप से कम-से-कम 5 मिनट और
  `diagnostics.stuckSessionWarnMs` के 3 गुना की अधिक सुरक्षित विस्तारित अवधि होती है।
- `session.stuck`: बिना सक्रिय कार्य के पुराने सत्र की
  बहीखाता स्थिति, या पुरानी स्वामी-रहित मॉडल/टूल गतिविधि वाला निष्क्रिय
  कतारबद्ध सत्र। पुनर्प्राप्ति गेट पार होने के तुरंत बाद यह प्रभावित सत्र
  लेन को मुक्त कर देता है।

पुनर्प्राप्ति संरचित `session.recovery.requested` और
`session.recovery.completed` इवेंट उत्सर्जित करती है। निदान सत्र स्थिति को केवल
परिवर्तनकारी पुनर्प्राप्ति परिणाम (`aborted` या `released`) के बाद
और केवल तभी निष्क्रिय चिह्नित किया जाता है जब वही प्रोसेसिंग जनरेशन अभी भी वर्तमान हो।

केवल `session.stuck`, `openclaw.session.stuck` काउंटर,
`openclaw.session.stuck_age_ms` हिस्टोग्राम और `openclaw.session.stuck`
स्पैन उत्सर्जित करता है। सत्र के अपरिवर्तित रहने पर बार-बार होने वाले
`session.stuck` निदान का अंतराल बढ़ता जाता है, इसलिए डैशबोर्ड को प्रत्येक
Heartbeat टिक के बजाय निरंतर वृद्धि पर चेतावनी देनी चाहिए। कॉन्फ़िगरेशन विकल्प
और डिफ़ॉल्ट के लिए [कॉन्फ़िगरेशन संदर्भ](/hi/gateway/configuration-reference#diagnostics) देखें।
...
### टूल निष्पादन और लूप का पता लगाना

- `openclaw.tool.execution.duration_ms` (हिस्टोग्राम, विशेषताएँ: `gen_ai.tool.name`, `openclaw.toolName`, `openclaw.tool.source`, `openclaw.tool.owner`, `openclaw.tool.params.kind`, और त्रुटियों पर `openclaw.errorCategory`)
...
- `openclaw.tool.loop` (काउंटर, विशेषताएँ: `openclaw.toolName`, `openclaw.loop.level`, `openclaw.loop.action`, `openclaw.loop.detector`, `openclaw.loop.count`, वैकल्पिक `openclaw.loop.paired_tool`; दोहराव वाले टूल-कॉल लूप का पता चलने पर उत्सर्जित)
...
### निदान की आंतरिक कार्यप्रणाली (मेमोरी, पेलोड, एक्सपोर्टर स्वास्थ्य)
...
- `openclaw.memory.rss_bytes` / `openclaw.memory.heap_used_bytes` / `openclaw.memory.heap_total_bytes` / `openclaw.memory.external_bytes` / `openclaw.memory.array_buffers_bytes` (हिस्टोग्राम, कोई विशेषता नहीं; प्रोसेस मेमोरी नमूने)
- `openclaw.memory.pressure` (काउंटर, विशेषताएँ: `openclaw.memory.level`, `openclaw.memory.reason`)
- `openclaw.diagnostic.async_queue.dropped` (काउंटर, विशेषताएँ: `openclaw.diagnostic.async_queue.drop_class`; आंतरिक निदान-कतार बैकप्रेशर के कारण ड्रॉप)
...
- `openclaw.model.usage`
  ...
  - `openclaw.tokens.*` (इनपुट/आउटपुट/कैश_रीड/कैश_राइट/कुल)
  - डिफ़ॉल्ट रूप से `gen_ai.system`, या नवीनतम GenAI सिमैंटिक कन्वेंशन चुनने पर `gen_ai.provider.name`
...
- `openclaw.model.call`
  ...
  - जब परिणाम में उस अनुरोध या समेकित टर्न के लिए उपयोग शामिल हो, तब `openclaw.model_call.usage.*` और `gen_ai.usage.*`
  - जब अपस्ट्रीम प्रदाता का परिणाम अनुरोध आईडी उजागर करता है, तब विशेषता `openclaw.upstreamRequestIdHash` (सीमित, हैश-आधारित) के साथ स्पैन इवेंट `openclaw.provider.request`; अपरिष्कृत आईडी कभी निर्यात नहीं की जातीं
  - `OTEL_SEMCONV_STABILITY_OPT_IN=gen_ai_latest_experimental` के साथ, अनुरोध स्पैन नवीनतम GenAI इंफ़रेंस स्पैन नाम `{gen_ai.operation.name} {gen_ai.request.model}` का उपयोग करते हैं। टर्न स्पैन `invoke_agent` का उपयोग करते हैं, क्योंकि OpenClaw अपारदर्शी CLI सीमा से किसी मूल एजेंट नाम का दावा नहीं करता। दोनों `openclaw.model.call` के बजाय `CLIENT` स्पैन प्रकार का उपयोग करते हैं।
...
जब सामग्री कैप्चर स्पष्ट रूप से सक्षम किया जाता है, मॉडल और टूल स्पैन में
आपके द्वारा चुने गए विशिष्ट सामग्री वर्गों के लिए सीमित, संशोधित
`openclaw.content.*` विशेषताएँ भी शामिल हो सकती हैं।

## निदान इवेंट कैटलॉग

नीचे दिए गए इवेंट ऊपर के मेट्रिक्स और स्पैन का आधार हैं या सीधे
Plugin सदस्यता के लिए उपलब्ध हैं। `run.progress` और `run.execution_phase`
केवल-प्रत्यक्ष जीवनचक्र संकेत हैं; diagnostics-otel Plugin उन्हें स्वतंत्र
OTLP संकेतों के रूप में निर्यात नहीं करता। इवेंट प्रकार और `run.execution_phase.phase`
मान योगात्मक हैं। TypeScript उपभोक्ताओं को यह मानने के बजाय कि दोनों यूनियन
हमेशा पूर्ण रहेंगी, डिफ़ॉल्ट शाखाएँ बनाए रखनी चाहिए।

**मॉडल उपयोग**

- `model.usage` - टोकन, लागत, अवधि, संदर्भ, प्रदाता/मॉडल/चैनल,
  सत्र आईडी। `usage` लागत और टेलीमेट्री के लिए प्रदाता/टर्न लेखांकन है;
  `context.used` वर्तमान प्रॉम्प्ट/संदर्भ स्नैपशॉट है और कैश किए गए इनपुट या
  टूल-लूप कॉल शामिल होने पर प्रदाता के `usage.total` से कम हो सकता है।
...
- `harness.run.started` / `harness.run.completed` / `harness.run.error` -
  एजेंट हार्नेस के लिए प्रत्येक रन का जीवनचक्र। इसमें `harnessId`, वैकल्पिक
  `pluginId`, प्रदाता/मॉडल/चैनल और रन आईडी शामिल हैं। पूर्ण होने पर
  `durationMs`, `outcome`, वैकल्पिक `resultClassification`, `yieldDetected`,
  और `itemLifecycle` गणनाएँ जोड़ी जाती हैं। त्रुटियों में `phase`
...
- `exec.process.completed` - टर्मिनल परिणाम, अवधि, लक्ष्य, मोड, निकास
  ...
  शामिल नहीं हैं।
- `exec.approval.followup_suppressed` - सत्र के दोबारा जुड़ने के बाद पुराना अनुमोदन
  फ़ॉलो-अप हटा दिया गया। इसमें `approvalId`, `reason`
...
`diagnostics-otel` चलाए बिना निदान इवेंट को Plugin या कस्टम सिंक के लिए
उपलब्ध रखें:
...
या एकबारगी एनवायरनमेंट ओवरराइड के रूप में:

```bash
OPENCLAW_DIAGNOSTICS=telegram.http,telegram.payload openclaw gateway
```

फ़्लैग आउटपुट मानक लॉग फ़ाइल (`logging.file`) में जाता है और इसे अब भी
...
या `plugins.allow` में से `diagnostics-otel` को हटा दें, अथवा
...
- [Gateway लॉगिंग की आंतरिक कार्यप्रणाली](/hi/gateway/logging) - WS लॉग शैलियाँ, उपतंत्र उपसर्ग और कंसोल कैप्चर
- [निदान फ़्लैग](/hi/diagnostics/flags) - लक्षित डीबग-लॉग फ़्लैग
- [निदान निर्यात](/hi/gateway/diagnostics) - ऑपरेटर सहायता-बंडल टूल (OTEL निर्यात से अलग)
- [कॉन्फ़िगरेशन संदर्भ](/hi/gateway/configuration-reference#diagnostics) - `diagnostics.*` फ़ील्ड का पूरा संदर्भ
