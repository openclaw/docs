...
x-i18n:
    generated_at: "2026-07-01T18:13:02Z"
    model: gpt-5.5
...
वेब GitHub आयातक स्थानीय publish/sync से अधिक सख्त है। यह केवल साइन-इन किए गए GitHub खाते के स्वामित्व वाली सार्वजनिक, non-fork repositories में `SKILL.md` या लेगेसी `skills.md` फ़ाइलें खोजता है। यह private repos, forks, archived/disabled repos, या third-party public repos आयात नहीं करता।
...
- UI/search में skill summary के रूप में `description` का उपयोग किया जाता है।
...
`metadata.openclaw` के अंतर्गत अपने skill की runtime आवश्यकताएं घोषित करें (aliases: `metadata.clawdbot`, `metadata.clawdis`)।
...
कौशल चलने से पहले मौजूद होने वाले environment variables के लिए `requires.env` का उपयोग करें। जब आपको per-variable metadata चाहिए, जिसमें `required: false` वाले वैकल्पिक variables शामिल हैं, तब `envVars` का उपयोग करें।
...
`metadata.openclaw.envVars` के अंतर्गत वैकल्पिक environment variables घोषित करें और `required: false` सेट करें। `requires.env` में वैकल्पिक entries न जोड़ें, क्योंकि `requires.env` का अर्थ है कि skill उनके बिना चल नहीं सकता।
...
ClawHub का security analysis जांचता है कि आपका skill जो घोषित करता है वह उसके वास्तविक कार्य से मेल खाता है। यदि आपका code `TODOIST_API_KEY` संदर्भित करता है लेकिन आपका frontmatter इसे `requires.env`, `primaryEnv`, या `envVars` के अंतर्गत घोषित नहीं करता, तो analysis metadata mismatch को flag करेगा। घोषणाओं को सटीक रखना आपके skill को review पास करने में मदद करता है और users को यह समझने में मदद करता है कि वे क्या install कर रहे हैं।
...
- कोई भी प्रकाशित skills का उपयोग, modification, और redistribution कर सकता है, जिसमें commercial उपयोग भी शामिल है।
