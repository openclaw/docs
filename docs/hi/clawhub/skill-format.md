...
x-i18n:
    generated_at: "2026-06-28T22:45:51Z"
    model: gpt-5.5
...
वेब GitHub importer, स्थानीय publish/sync की तुलना में अधिक सख्त है। यह केवल
साइन-इन किए हुए GitHub खाते के स्वामित्व वाले सार्वजनिक, non-fork repositories में
`SKILL.md` या पुराने `skills.md` फ़ाइलों को खोजता है। यह private repos, forks,
archived/disabled repos, या third-party public repos आयात नहीं करता।
...
`requires.env` का उपयोग उन environment variables के लिए करें जो skill चलने से पहले मौजूद होने चाहिए। जब आपको प्रति-variable metadata चाहिए, जिसमें `required: false` वाले वैकल्पिक variables भी शामिल हों, तब `envVars` का उपयोग करें।
...
| `requires.bins`    | `string[]` | CLI binaries जिन्हें सभी install होना चाहिए।                                                                                                 |
...
| `always`           | `boolean`  | यदि `true`, skill हमेशा active रहता है (स्पष्ट install की आवश्यकता नहीं)।                                                                    |
...
ClawHub का security analysis जाँचता है कि आपका skill जो घोषित करता है, वह वास्तव में किए गए काम से मेल खाता है। यदि आपका code `TODOIST_API_KEY` को reference करता है लेकिन आपका frontmatter उसे `requires.env`, `primaryEnv`, या `envVars` के अंतर्गत घोषित नहीं करता, तो analysis metadata mismatch flag करेगा। declarations को सटीक रखने से आपका skill review पास करने में मदद करता है और users को यह समझने में मदद करता है कि वे क्या install कर रहे हैं।
...
- Tags किसी version के लिए string pointers हैं; `latest` सामान्यतः उपयोग किया जाता है।

## License

- ClawHub पर प्रकाशित सभी skills `MIT-0` के अंतर्गत licensed हैं।
- कोई भी प्रकाशित skills का उपयोग, modification, और redistribution कर सकता है, including commercially।
...
- यदि आपका skill किसी paid third-party service के साथ integrate करता है, तो skill instructions और env declarations में external cost और required account को स्पष्ट रूप से document करें (`requires.env` required variables के लिए, या optional variables के लिए `required: false` के साथ `envVars`)।
