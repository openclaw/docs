...
x-i18n:
    generated_at: "2026-07-03T17:21:49Z"
    model: gpt-5.5
...
- `.clawhubignore` (प्रकाशन के लिए ignore पैटर्न, लीगेसी `.clawdhubignore`)
- `.gitignore` (इसका भी सम्मान किया जाता है)

## GitHub आयात

वेब GitHub आयातक स्थानीय publish/sync से अधिक सख्त है। यह केवल साइन-इन किए गए GitHub खाते के स्वामित्व वाले सार्वजनिक, non-fork repositories में `SKILL.md` या लीगेसी `skills.md` फ़ाइलों को खोजता है। यह निजी repos, forks, archived/disabled repos, या third-party public repos आयात नहीं करता।
...
`metadata.openclaw` के अंतर्गत अपनी skill की runtime requirements घोषित करें (aliases: `metadata.clawdbot`, `metadata.clawdis`)।
...
Skill चलने से पहले मौजूद होने वाले environment variables के लिए `requires.env` का उपयोग करें। जब आपको प्रति-variable metadata चाहिए, जिसमें `required: false` वाले वैकल्पिक variables भी शामिल हैं, तब `envVars` का उपयोग करें।

### पूर्ण field reference

| Field              | Type       | Description                                                                                                                                  |
| ------------------ | ---------- | -------------------------------------------------------------------------------------------------------------------------------------------- |
| `requires.env`     | `string[]` | आपकी skill जिन आवश्यक environment variables की अपेक्षा करती है।                                                                                           |
...
| `always`           | `boolean`  | यदि `true`, skill हमेशा active रहती है (explicit install की आवश्यकता नहीं)।                                                                              |
...
वैकल्पिक environment variables को `metadata.openclaw.envVars` के अंतर्गत घोषित करें और `required: false` सेट करें। वैकल्पिक entries को `requires.env` में न जोड़ें, क्योंकि `requires.env` का अर्थ है कि skill उनके बिना चल नहीं सकती।
...
ClawHub का security analysis जाँचता है कि आपकी skill जो घोषित करती है वह उसके वास्तविक व्यवहार से मेल खाता है। यदि आपका code `TODOIST_API_KEY` को reference करता है लेकिन आपका frontmatter उसे `requires.env`, `primaryEnv`, या `envVars` के अंतर्गत घोषित नहीं करता, तो analysis metadata mismatch flag करेगा। घोषणाओं को सटीक रखने से आपकी skill review पास करने में मदद मिलती है और users समझ पाते हैं कि वे क्या install कर रहे हैं।

### उदाहरण: complete frontmatter
...
- Script files upload के बाद भी scanned होती हैं; PowerShell `.ps1`, `.psm1`, और `.psd1` फ़ाइलें text के रूप में स्वीकार की जाती हैं।
...
- `SKILL.md` में conflicting license terms न जोड़ें; ClawHub प्रति-skill license overrides support नहीं करता।

## Paid skills

- ClawHub paid skills, प्रति-skill pricing, paywalls, या revenue sharing support नहीं करता।
- `SKILL.md` में pricing metadata न जोड़ें; यह skill format का हिस्सा नहीं है और published skill को paid नहीं बनाएगा।
- यदि आपकी skill किसी paid third-party service के साथ integrate करती है, तो skill instructions और env declarations में external cost और required account को स्पष्ट रूप से document करें (required variables के लिए `requires.env`, या optional variables के लिए `required: false` के साथ `envVars`)।
