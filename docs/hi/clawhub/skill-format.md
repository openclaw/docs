...
x-i18n:
    generated_at: "2026-07-04T03:48:00Z"
    model: gpt-5.5
...
वेब GitHub आयातक स्थानीय publish/sync से अधिक सख्त है। यह केवल साइन-इन किए गए GitHub खाते के स्वामित्व वाले सार्वजनिक, non-fork रिपॉज़िटरी में `SKILL.md` या लेगेसी `skills.md` फ़ाइलें खोजता है। यह निजी repos, forks, archived/disabled repos, या तृतीय-पक्ष सार्वजनिक repos आयात नहीं करता।
...
अपने कौशल की runtime आवश्यकताएं `metadata.openclaw` के अंतर्गत घोषित करें (aliases: `metadata.clawdbot`, `metadata.clawdis`)।
...
कौशल चलने से पहले मौजूद होने वाले environment variables के लिए `requires.env` का उपयोग करें। जब आपको per-variable metadata चाहिए, जिसमें `required: false` वाले वैकल्पिक variables भी शामिल हों, तो `envVars` का उपयोग करें।
...
यदि आपके कौशल को dependencies install चाहिए, तो उन्हें `install` array में घोषित करें:
...
ClawHub का security analysis जांचता है कि आपका कौशल जो घोषित करता है वह उसके वास्तविक व्यवहार से मेल खाता है। यदि आपका code `TODOIST_API_KEY` को reference करता है लेकिन आपका frontmatter उसे `requires.env`, `primaryEnv`, या `envVars` के अंतर्गत घोषित नहीं करता, तो analysis metadata mismatch को flag करेगा। declarations को सही रखना आपके कौशल को review पास करने में मदद करता है और users को यह समझने में मदद करता है कि वे क्या install कर रहे हैं।
...
- Tags किसी version के string pointers होते हैं; `latest` आमतौर पर उपयोग किया जाता है।
...
- यदि आपका कौशल किसी paid third-party service के साथ integrate करता है, तो external cost और required account को कौशल instructions और env declarations में स्पष्ट रूप से document करें (required variables के लिए `requires.env`, या optional variables के लिए `required: false` के साथ `envVars`)।
