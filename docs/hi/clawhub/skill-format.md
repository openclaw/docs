...
वेब GitHub इम्पोर्टर local publish/sync से अधिक सख्त है। यह केवल
...
- `description` का उपयोग UI/search में कौशल सारांश के रूप में किया जाता है।
...
कौशल चलने से पहले मौजूद होने वाले environment variables के लिए `requires.env` का उपयोग करें। जब आपको प्रति-variable metadata चाहिए, जिसमें `required: false` वाले वैकल्पिक variables भी शामिल हैं, तब `envVars` का उपयोग करें।
...
| `always`           | `boolean`  | यदि `true`, तो कौशल हमेशा active रहता है (explicit install की जरूरत नहीं)।                                                                              |
...
यदि आपके कौशल को dependencies install करने की जरूरत है, तो उन्हें `install` array में घोषित करें:
...
ClawHub का security analysis जांचता है कि आपका कौशल जो घोषित करता है, वह उसके वास्तविक काम से मेल खाता है। यदि आपका code `TODOIST_API_KEY` reference करता है लेकिन आपका frontmatter उसे `requires.env`, `primaryEnv`, या `envVars` के अंतर्गत घोषित नहीं करता, तो analysis metadata mismatch flag करेगा। घोषणाओं को accurate रखने से आपका कौशल review pass कर पाता है और users को यह समझने में मदद मिलती है कि वे क्या install कर रहे हैं।
...
- Tags किसी version के string pointers होते हैं; `latest` आमतौर पर इस्तेमाल होता है।

## License

- ClawHub पर publish किए गए सभी Skills `MIT-0` के अंतर्गत licensed हैं।
...
- यदि आपका कौशल किसी paid third-party service के साथ integrate करता है, तो skill instructions और env declarations में external cost और required account को स्पष्ट रूप से document करें (required variables के लिए `requires.env`, या optional variables के लिए `required: false` के साथ `envVars`)।
