...
x-i18n:
    generated_at: "2026-07-01T15:24:13Z"
    model: gpt-5.5
...
एक Skill एक फ़ोल्डर है।

आवश्यक:

- `SKILL.md` (या `skill.md`; legacy `skills.md` भी स्वीकार की जाती है)
...
साइन-इन किए गए GitHub खाते के स्वामित्व वाली public, non-fork repositories में
`SKILL.md` या legacy `skills.md` फ़ाइलों को खोजता है। यह private repos, forks,
archived/disabled repos, या third-party public repos import नहीं करता।
...
- `description` को UI/खोज में Skill सारांश के रूप में उपयोग किया जाता है।
...
अपने Skill की runtime आवश्यकताएँ `metadata.openclaw` के अंतर्गत घोषित करें (aliases: `metadata.clawdbot`, `metadata.clawdis`)।
...
उन environment variables के लिए `requires.env` का उपयोग करें जो Skill चलने से पहले मौजूद होने चाहिए। जब आपको per-variable metadata चाहिए, जिसमें `required: false` वाले वैकल्पिक variables शामिल हैं, तो `envVars` का उपयोग करें।
...
| `requires.bins`    | `string[]` | CLI binaries जो सभी install होनी चाहिए।                                                                                                     |
| `requires.anyBins` | `string[]` | CLI binaries जिनमें से कम से कम एक मौजूद होनी चाहिए।                                                                                                  |
...
| `always`           | `boolean`  | यदि `true`, तो Skill हमेशा active रहता है (explicit install की आवश्यकता नहीं)।                                                                              |
...
ClawHub का security analysis जांचता है कि आपका Skill जो घोषित करता है, वह उसके वास्तविक काम से मेल खाता है। यदि आपका code `TODOIST_API_KEY` को reference करता है लेकिन आपका frontmatter उसे `requires.env`, `primaryEnv`, या `envVars` के अंतर्गत घोषित नहीं करता, तो analysis metadata mismatch flag करेगा। घोषणाओं को सटीक रखना आपके Skill को review पास करने में मदद करता है और users को यह समझने में मदद करता है कि वे क्या install कर रहे हैं।
...
- Package scopes का ClawHub publisher handle से बिल्कुल मेल होना चाहिए। Publisher handles lowercase letters, numbers, hyphens, dots, और underscores का उपयोग कर सकते हैं; उन्हें lowercase letter या number से शुरू और समाप्त होना चाहिए।
...
- Tags किसी version के लिए string pointers हैं; `latest` सामान्यतः उपयोग होता है।
...
- ClawHub सशुल्क Skills, per-skill pricing, paywalls, या revenue sharing का समर्थन नहीं करता।
- `SKILL.md` में pricing metadata न जोड़ें; यह Skill format का हिस्सा नहीं है और published Skill को paid नहीं बनाएगा।
- यदि आपका Skill किसी paid third-party service के साथ integrate करता है, तो Skill instructions और env declarations (`requires.env` आवश्यक variables के लिए, या वैकल्पिक variables के लिए `required: false` के साथ `envVars`) में external cost और required account को स्पष्ट रूप से document करें।
