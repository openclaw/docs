...
title: ClawHub
x-i18n:
    generated_at: "2026-07-01T12:58:27Z"
    model: gpt-5.5
...
- ClawHub से Skills खोजने, इंस्टॉल करने और अपडेट करने तथा plugins इंस्टॉल करने के लिए स्थानीय `openclaw` कमांड का उपयोग करें।
- रजिस्ट्री प्रमाणीकरण, प्रकाशन, और हटाने/वापस लाने वाले workflows के लिए अलग `clawhub` CLI का उपयोग करें।
...
जब आपको प्रकाशित करने या हटाने/वापस लाने जैसे रजिस्ट्री-प्रमाणीकृत workflows चाहिए हों, तब ClawHub CLI इंस्टॉल करें:
...
| ------------- | ------------------------------------------------------------- | -------------------------------------------- |
| Skills        | `SKILL.md` और सहायक फाइलों के साथ संस्करणित टेक्स्ट बंडल     | `openclaw skills install @openclaw/demo`     |
| कोड plugins   | संगतता मेटाडेटा के साथ OpenClaw plugin पैकेज                 | `openclaw plugins install clawhub:<package>` |
| बंडल plugins  | OpenClaw वितरण के लिए पैकेज किए गए plugin बंडल                | `clawhub package publish <source>`           |

ClawHub semver संस्करणों, `latest` जैसे टैग, changelogs, फाइलों,
डाउनलोड, स्टार, और सुरक्षा स्कैन सारांशों को ट्रैक करता है। सार्वजनिक पेज मौजूदा रजिस्ट्री
स्थिति दिखाते हैं ताकि उपयोगकर्ता किसी skill या plugin को इंस्टॉल करने से पहले उसकी जांच कर सकें।

## स्थानीय OpenClaw flows

स्थानीय OpenClaw कमांड सक्रिय OpenClaw workspace में इंस्टॉल करते हैं और
स्रोत मेटाडेटा को स्थायी रखते हैं ताकि बाद के अपडेट कमांड ClawHub पर बने रह सकें।

जब किसी plugin इंस्टॉल को ClawHub के माध्यम से resolve करना हो, तब `clawhub:<package>` का उपयोग करें।
launch cutovers के दौरान bare npm-safe plugin specs npm के माध्यम से resolve हो सकते हैं, और
जब स्रोत स्पष्ट होना जरूरी हो तब `npm:<package>` केवल npm तक सीमित रहता है।
...
और डाउनलोड किए गए bytes को verify करता है, और बाद के अपडेट के लिए artifact metadata रिकॉर्ड करता है।
...
वे कमांड मौजूदा working directory के अंतर्गत `./skills` में Skills इंस्टॉल करते हैं
...
- `--tags <tags>`: comma-separated tags, जिनका default `latest` है।
...
अपलोड किए बिना ठीक publish plan बनाने के लिए `--dry-run` का उपयोग करें, और CI-friendly output के लिए `--json`
का उपयोग करें।

Code plugins में `package.json` में जरूरी OpenClaw compatibility metadata शामिल होना चाहिए,
जिसमें `openclaw.compat.pluginApi` और
`openclaw.build.openclawVersion` शामिल हैं। पूरे command
...
ClawHub default रूप से खुला है: कोई भी upload कर सकता है, लेकिन publishing के लिए ऐसा GitHub
account चाहिए जो upload gate पास करने जितना पुराना हो। सार्वजनिक detail pages install या download से पहले
...
अपने owner को `/dashboard` में दिखते रहते हैं।

Signed-in users Skills और packages की report कर सकते हैं। Moderators reports की समीक्षा कर सकते हैं,
...
| `CLAWHUB_CONFIG_PATH`         | CLI token/config state कहां संग्रहीत करता है, उसे override करें। |
| `CLAWHUB_WORKDIR`             | Default working directory को override करें।      |
| `CLAWHUB_DISABLE_TELEMETRY=1` | Install telemetry disable करें।                  |

गहरे reference material के लिए [Telemetry](/clawhub/telemetry), [HTTP API](/clawhub/http-api), और
