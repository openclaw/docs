...
read_when:
    ...
    - openclaw और clawhub CLI प्रवाहों के बीच चुनाव करना
sidebarTitle: ClawHub
summary: सर्च, इंस्टॉल, पब्लिश, सुरक्षा, और clawhub CLI के लिए सार्वजनिक ClawHub ओवरव्यू।
title: ClawHub
x-i18n:
    generated_at: "2026-07-02T17:37:57Z"
    model: gpt-5.5
...
- Skills खोजने, install करने और update करने तथा ClawHub से plugins install करने के लिए native `openclaw` commands का उपयोग करें।
...
| कोड plugins    | Compatibility metadata के साथ OpenClaw plugin packages       | `openclaw plugins install clawhub:<package>` |
| बंडल plugins   | OpenClaw distribution के लिए packaged plugin bundles         | `clawhub package publish <source>`           |

ClawHub semver versions, `latest` जैसे tags, changelogs, files,
downloads, stars, और security scan summaries track करता है। Public pages वर्तमान registry
...
जब किसी plugin install को ClawHub के माध्यम से resolve होना चाहिए, तो `clawhub:<package>` का उपयोग करें।
Bare npm-safe plugin specs launch cutovers के दौरान npm के माध्यम से resolve हो सकते हैं, और
जब source explicit होना चाहिए तो `npm:<package>` npm-only रहता है।
...
Code plugins में `package.json` में required OpenClaw compatibility metadata शामिल होना चाहिए,
जिसमें `openclaw.compat.pluginApi` और
`openclaw.build.openclawVersion` शामिल हैं। Full command
...
अपने owner को `/dashboard` में visible बने रहते हैं।
...
जब आप logged in रहते हुए `clawhub install` run करते हैं, तो CLI best-effort
install event भेज सकता है, ताकि ClawHub aggregate install counts compute कर सके। इसे disable करें:
...
गहन reference material के लिए [Telemetry](/clawhub/telemetry), [HTTP API](/clawhub/http-api), और
