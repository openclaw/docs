...
title: ClawHub
x-i18n:
    generated_at: "2026-07-02T22:30:19Z"
    model: gpt-5.5
...
ClawHub OpenClaw Skills और Plugin के लिए सार्वजनिक registry है।

- Skills खोजने, install करने और update करने तथा ClawHub से Plugin install करने के लिए native `openclaw` commands का उपयोग करें।
...
| Code Plugin    | compatibility metadata वाले OpenClaw plugin packages         | `openclaw plugins install clawhub:<package>` |
| Bundle Plugin  | OpenClaw distribution के लिए packaged plugin bundles         | `clawhub package publish <source>`           |
...
जब किसी Plugin install को ClawHub के ज़रिए resolve करना हो, तो `clawhub:<package>` उपयोग करें।
Bare npm-safe plugin specs launch cutovers के दौरान npm के ज़रिए resolve हो सकते हैं, और
जब source explicit होना चाहिए तब `npm:<package>` केवल npm रहता है।
...
OpenClaw exact uploaded npm-pack `.tgz` को प्राथमिकता देता है, ClawHub digest header और
downloaded bytes verify करता है, और बाद के updates के लिए artifact metadata record करता है।
...
ये commands current working directory के नीचे `./skills` में Skills install करते हैं
...
Upload किए बिना exact publish plan build करने के लिए `--dry-run` उपयोग करें, और CI-friendly output
के लिए `--json`।

Code Plugin में `package.json` में आवश्यक OpenClaw compatibility metadata शामिल होना चाहिए,
...
ClawHub default रूप से open है: कोई भी upload कर सकता है, लेकिन publishing के लिए GitHub
account इतना पुराना होना चाहिए कि upload gate पास कर सके। Public detail pages install या download से पहले
...
उनके owner को `/dashboard` में visible रहते हैं।
