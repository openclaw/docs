...
x-i18n:
    generated_at: "2026-07-03T00:56:30Z"
    model: gpt-5.5
...
तृतीय-पक्ष निर्देशिकाएं ClawHub Skills को सूचीबद्ध करने या खोजने के लिए सार्वजनिक रीड एंडपॉइंट का उपयोग कर सकती हैं। कृपया परिणामों को कैश करें, `429`/`Retry-After` का सम्मान करें, उपयोगकर्ताओं को canonical ClawHub सूची (`https://clawhub.ai/<owner>/skills/<slug>`) पर वापस लिंक करें, और तृतीय-पक्ष साइट के ClawHub समर्थन का संकेत देने से बचें। सार्वजनिक API सतह के बाहर छिपी, निजी, या मॉडरेशन-द्वारा-अवरुद्ध सामग्री को मिरर करने का प्रयास न करें।

वेब slug शॉर्टकट registry families में resolve होते हैं, लेकिन API क्लाइंट को route
precedence को फिर से बनाने के बजाय रीड एंडपॉइंट द्वारा लौटाए गए canonical URL का उपयोग करना चाहिए।
...
- यदि token अनुपस्थित/अमान्य है, तो व्यवहार IP प्रवर्तन पर वापस जाता है।
- प्रमाणित write endpoints को तब bare `Unauthorized` नहीं लौटाना चाहिए जब
  server कारण जानता हो। अनुपस्थित tokens, अमान्य/revoked tokens, और
  हटाए गए/प्रतिबंधित/disabled accounts में से प्रत्येक को actionable text मिलना चाहिए ताकि CLI
...
- `X-RateLimit-Remaining` / `RateLimit-Remaining`: उपस्थित होने पर सटीक शेष budget।
...
- trusted client IP headers का उपयोग करता है, जिसमें `cf-connecting-ip` भी शामिल है, केवल तब जब
  deployment स्पष्ट रूप से trusted forwarded headers को सक्षम करता है।
...
- यदि कोई trusted client IP उपलब्ध नहीं है, तो anonymous requests fallback buckets का उपयोग करते हैं
  जो केवल rate-limit kind द्वारा scoped होते हैं। ये fallback buckets
  caller-supplied paths, slugs, package names, versions, query strings, या अन्य
  artifact parameters शामिल नहीं करते।

## त्रुटि प्रतिक्रियाएं

सार्वजनिक v1 त्रुटि responses `content-type: text/plain; charset=utf-8` के साथ plain text होती हैं।
...
संगतता के लिए ignore किया जाता है, लेकिन अमान्य values वाले recognized query parameters
`400` लौटाते हैं।
...
- Popularity log-scaled और capped है। High-engagement skills कम rank कर सकते हैं जब query text कमजोर match हो।
- Suspicious या hidden moderation state caller filters और वर्तमान moderation status के आधार पर public search से skill को हटा सकती है।

Publisher discoverability guidance:

- जिन शब्दों को users सचमुच खोजेंगे, उन्हें display name, summary, और tags में रखें। standalone slug token का उपयोग केवल तब करें जब वह भी एक स्थिर identity हो जिसे आप रखना चाहते हैं।
- केवल किसी एक query का पीछा करने के लिए slug का नाम न बदलें, जब तक नया slug बेहतर दीर्घकालिक canonical name न हो। पुराने slugs redirect aliases बन जाते हैं, लेकिन canonical URL, प्रदर्शित slug, और भविष्य के search digests नए slug का उपयोग करते हैं।
...
- यदि कोई skill अनपेक्षित रूप से invisible है, तो ranking-related metadata बदलने से पहले logged in रहते हुए `clawhub inspect @owner/slug` से moderation state पहले जांचें।
...
- `trending` पिछले 7 दिनों में installs द्वारा rank करता है (telemetry-based)।
- `createdAt` new-skill crawls के लिए stable है; `updated` existing skills के republish होने पर बदलता है।
...
- उपस्थित होने पर pagination जारी रखने के लिए `nextCursor` का उपयोग करें। छोटा page अपने-आप end-of-results नहीं दर्शाता।
...
version से linked हैं, और skill report queue को feed करती हैं।
...
- `moderation` नवीनतम संस्करण से निकला वर्तमान कौशल-स्तर moderation snapshot है।
...
Local upload scans अब समर्थित नहीं हैं। `multipart/form-data` या `{ "source": { "kind": "upload" } }` का उपयोग करने वाली requests `410` लौटाती हैं।
...
- Queued रहने के दौरान `queue.queuedAhead` और `queue.position` लौटाता है ताकि clients दिखा सकें कि request से आगे कितने prioritized manual scans हैं। बहुत बड़ी queues bounded होती हैं और `queuedAheadIsEstimate: true` के साथ report की जाती हैं।
...
- `security.signals.dependencyRegistry` v1 response compatibility के लिए रखा गया है, लेकिन dependency registry existence scanner retire हो चुका है और यह key हमेशा `null` होती है।
...
- `security.signals.dependencyRegistry` v1 response compatibility के लिए रखा गया है, लेकिन dependency registry existence scanner retire हो चुका है और यह key हमेशा `null` होती है।
- Skill Card absence इस endpoint के `ok`, `decision`, या `reasons` को affect नहीं करती; clients को card content की आवश्यकता होने पर installed `skill-card.md` को locally पढ़ना चाहिए।
- Single-skill Skill Card verification envelope चाहिए होने पर `/verify`, generated card markdown चाहिए होने पर `/card`, और detailed scanner data चाहिए होने पर `/scan` उपयोग करें।
...
- `category` (वैकल्पिक): Plugin श्रेणी फ़िल्टर। केवल तब समर्थित जब
  ...
  `family=code-plugin`/`family=bundle-plugin` वाले पैकेज endpoint)। नियंत्रित श्रेणियां और
...
- `GET /api/v1/code-plugins` और `GET /api/v1/bundle-plugins` fixed-family उपनाम बने रहते हैं।
- Skill प्रविष्टियां skill registry द्वारा समर्थित रहती हैं और अब भी केवल `POST /api/v1/skills` के माध्यम से प्रकाशित की जा सकती हैं।
...
- प्रमाणित कॉलर list/search परिणामों में उन प्रकाशकों के निजी पैकेज देख सकते हैं जिनसे वे संबंधित हैं।
- `channel=private` केवल वे पैकेज लौटाता है जिन्हें प्रमाणित कॉलर पढ़ सकता है।

### `GET /api/v1/packages/search`

Skills + Plugin पैकेज में एकीकृत कैटलॉग खोज।
...
- `isOfficial` (वैकल्पिक): `true` या `false`
- `category` (वैकल्पिक): Plugin श्रेणी फ़िल्टर। केवल तब समर्थित जब
  अनुरोध Plugin पैकेज तक सीमित हो। नियंत्रित श्रेणियां और लेगेसी v1
...
code-plugin और bundle-plugin पैकेज में केवल-Plugin कैटलॉग ब्राउज़।
...
लेगेसी v1 फ़िल्टर उपनाम read endpoints पर स्वीकार किए जाते रहते हैं:
...
`trending` सात-दिन का install/download लीडरबोर्ड है और all-time कुलों का उपयोग नहीं करता।
एकीकृत `/api/v1/packages` endpoint पर यह केवल-Plugin है; skill कैटलॉग के लिए
...
- Body: ZIP आर्काइव।
- प्रत्येक एक्सपोर्ट किया गया skill `{publisher}/{slug}/` पर रूटेड होता है।
...
- जब अलग-अलग Skills या फ़ाइलें एक्सपोर्ट नहीं की जा सकीं, तो
...
- `GET /api/v1/plugins` के अंतर्गत दस्तावेज़ीकृत legacy v1 फ़िल्टर उपनाम भी
...
सत्यापन, आर्टिफैक्ट मेटाडेटा, और स्कैन डेटा शामिल हैं।
...
- `version.sha256hash` पुराने क्लाइंट के लिए बहिष्कृत संगतता मेटाडेटा है। यह
  ...
  कैनोनिकल रिलीज़ आर्टिफैक्ट की पहचान करता है।
...
यह तय करने के लिए कि हल की गई रिलीज़ इंस्टॉल की जा सकती है या नहीं, यह सार्वजनिक
OpenClaw उपभोग सतह है।
...
- `trust.blockedFromDownload` इंस्टॉल ब्लॉक संकेत है। OpenClaw और अन्य
  इंस्टॉल क्लाइंट को स्कैनर या मॉडरेशन फ़ील्ड से ब्लॉकिंग नियमों को फिर से निकालने के बजाय
...
- यह एंडपॉइंट स्वामी/मॉडरेटर मॉडरेशन
  एंडपॉइंट की तुलना में जानबूझकर संकरा है। यह इंस्टॉल निर्णय और सार्वजनिक व्याख्या उजागर करता है,
  रिपोर्टर पहचान, रिपोर्ट बॉडी, निजी साक्ष्य, या आंतरिक समीक्षा
  टाइमलाइन नहीं।

### `GET /api/v1/packages/{name}/versions/{version}/artifact`

किसी पैकेज संस्करण के लिए स्पष्ट आर्टिफैक्ट resolver मेटाडेटा लौटाता है।

नोट्स:

- Legacy पैकेज संस्करण `legacy-zip` आर्टिफैक्ट और legacy ZIP
  `downloadUrl` लौटाते हैं।
- ClawPack संस्करण `npm-pack` आर्टिफैक्ट, npm इंटीग्रिटी फ़ील्ड,
  `tarballUrl`, और legacy ZIP संगतता URL लौटाते हैं।
- यह OpenClaw resolver सतह है; यह साझा URL से
  आर्काइव फ़ॉर्मेट का अनुमान लगाने से बचाता है।

### `GET /api/v1/packages/{name}/versions/{version}/artifact/download`

स्पष्ट resolver पथ के माध्यम से संस्करण आर्टिफैक्ट डाउनलोड करता है।

नोट्स:

- ClawPack संस्करण सटीक अपलोड किए गए npm-pack `.tgz` बाइट्स स्ट्रीम करते हैं।
- Legacy ZIP संस्करण `/api/v1/packages/{name}/download?version=` पर रीडायरेक्ट करते हैं।
...
- यह केवल माइग्रेशन readiness ट्रैक करता है। यह OpenClaw को mutate नहीं करता या
...
मॉडरेटर समीक्षा के लिए किसी पैकेज की रिपोर्ट करें। रिपोर्ट पैकेज-स्तर की होती हैं, वैकल्पिक रूप से
किसी संस्करण से लिंक होती हैं। वे मॉडरेशन कतार को फ़ीड करती हैं लेकिन स्वयं डाउनलोड को
ऑटो-हाइड या ब्लॉक नहीं करतीं; मॉडरेटर को आर्टिफैक्ट approve, quarantine, या revoke करने के लिए
...
पैकेज रिपोर्ट इनटेक के लिए मॉडरेटर/एडमिन endpoint।
...
- पैकेज owner, publisher member, moderator, या
...
पैकेज रिपोर्ट्स को resolve या reopen करने के लिए moderator/admin endpoint।
...
Quarantined और revoked releases artifact download routes से `403` लौटाती हैं।
...
- Pending VirusTotal scans reads को block नहीं करते; malicious releases फिर भी कहीं और withheld हो सकती हैं।
- Private packages `404` लौटाते हैं, जब तक caller owning publisher को read न कर सके।
...
- Plugin/package archives `package/` root वाली zip files होती हैं ताकि पुराने OpenClaw
...
- Download stats को प्रति UTC day unique identities के रूप में गिना जाता है (`userId` जब API token valid हो, अन्यथा IP)।
...
- Allowed form fields हैं `payload`, repeated `files` blobs, या एक `clawpack`
  ...
  upload-url flow द्वारा लौटाया गया storage id हो सकता है। Staged storage-id publishes में उस upload URL के साथ लौटाया गया
  `clawpackUploadTicket` भी शामिल होना चाहिए।
- या तो `files` या `clawpack` उपयोग करें, उसी request में दोनों कभी नहीं।
...
दूसरे publisher द्वारा claimed किया जा सकता है। यह expiry लागू होने पर delete response में `slugReservedUntil` शामिल होता है।
...
Newly-created org के लिए, `memberHandle` प्रदान करें; acting admin member के रूप में added नहीं होता।
...
owner बाद में वास्तविक code-plugin या bundle-plugin release उस name में publish कर सके।
...
रिकवरी, पुनर्प्राप्त publisher के skills, skill slug aliases, packages, package inspector warnings,
और derived search digest rows के लिए legacy `ownerUserId` फ़ील्ड भी माइग्रेट करती है, ताकि
direct-owner paths नए publisher authority से मेल खाएँ। पुनर्प्राप्त handle के लिए एक सक्रिय protected-handle
reservation भी replacement user को फिर से सौंपा जाता है, ताकि बाद की
profile synchronization पूर्व उपयोगकर्ता की competing authority को पुनर्स्थापित न कर सके। प्रत्येक प्राथमिक तालिका
प्रति apply transaction 100 rows तक सीमित होती है; बड़ी recoveries को पहले resumable owner migration का उपयोग करना होगा।
...
किसी मौजूदा ban के लिए stored reason को बदलें, बिना content को unban या restore किए
...
किसी उपयोगकर्ता role को बदलें (केवल admin)।
...
उपयोगकर्ताओं की सूची दिखाएँ या search करें (केवल admin)।
