...
x-i18n:
    generated_at: "2026-07-04T06:32:22Z"
    model: gpt-5.5
...
पुराने `/api/...` और `/api/cli/...` संगतता के लिए बने हुए हैं (`DEPRECATIONS.md` देखें)।
OpenAPI: `/api/v1/openapi.json`.

## सार्वजनिक कैटलॉग पुनः उपयोग

तृतीय-पक्ष डायरेक्टरियां ClawHub Skills को सूचीबद्ध या खोजने के लिए सार्वजनिक पठन endpoints का उपयोग कर सकती हैं। कृपया परिणाम cache करें, `429`/`Retry-After` का पालन करें, उपयोगकर्ताओं को canonical ClawHub listing (`https://clawhub.ai/<owner>/skills/<slug>`) पर वापस link करें, और तृतीय-पक्ष site के लिए ClawHub समर्थन का संकेत देने से बचें। सार्वजनिक API surface के बाहर छिपी हुई, निजी, या moderation-blocked सामग्री को mirror करने का प्रयास न करें।

Web slug shortcuts registry families में resolve होते हैं, लेकिन API clients को route
precedence को फिर से बनाने के बजाय read endpoints द्वारा लौटाए गए
...
- यदि token missing/invalid है, तो behavior IP enforcement पर वापस जाता है।
- Authenticated write endpoints को bare `Unauthorized` return नहीं करना चाहिए जब
  server reason जानता हो। Missing tokens, invalid/revoked tokens, और
...
- `X-RateLimit-Remaining` / `RateLimit-Remaining`: मौजूद होने पर exact remaining budget.
  Sharded successful requests approximate global value return करने के बजाय यह header omit करते हैं।
...
- यदि कोई trusted client IP उपलब्ध नहीं है, तो anonymous requests fallback buckets उपयोग करती हैं
  जो केवल rate-limit kind के scope में होते हैं। इन fallback buckets में
...
- Relevance popularity से अधिक मजबूत है। Precise slug या display-name token match, कहीं अधिक engagement वाले looser match से ऊपर rank कर सकता है।
- ASCII text को word और punctuation boundaries पर tokenize किया जाता है। उदाहरण के लिए, `personal-map` में standalone `map` token है, जबकि `amap-jsapi-skill` में `amap`, `jsapi`, और `skill` हैं; इसलिए `map` search करने पर `personal-map` को `amap-jsapi-skill` से stronger lexical match मिलता है।
- Popularity log-scaled और capped है। High-engagement skills नीचे rank कर सकते हैं जब query text weaker match हो।
...
- वे terms display name, summary, और tags में डालें जिन्हें users सचमुच search करेंगे। Standalone slug token का उपयोग केवल तब करें जब वह भी एक stable identity हो जिसे आप keep करना चाहते हैं।
- किसी एक query को chase करने के लिए slug rename न करें, जब तक नया slug बेहतर long-term canonical name न हो। पुराने slugs redirect aliases बन जाते हैं, लेकिन canonical URL, displayed slug, और future search digests नया slug उपयोग करते हैं।
- Rename aliases old URLs और registry के through resolve होने वाले installs के लिए resolution preserve करते हैं, लेकिन search ranking canonical skill metadata पर based होती है जब rename indexed हो चुका हो। Existing stats skill के साथ बने रहते हैं।
- यदि कोई skill अप्रत्याशित रूप से invisible है, तो ranking-related metadata बदलने से पहले logged in रहते हुए `clawhub inspect @owner/slug` के साथ moderation state पहले जांचें।
...
- `trending` पिछले 7 days में installs के आधार पर rank करता है (telemetry-based)।
...
- मौजूद होने पर pagination जारी रखने के लिए `nextCursor` का उपयोग करें। Short page अपने आप end-of-results का अर्थ नहीं है।
...
- `metadata.os`: skill frontmatter में declared OS restrictions (जैसे `["macos"]`, `["linux"]`)। यदि declared नहीं है तो `null`।
- `metadata.systems`: Nix system targets (जैसे `["aarch64-darwin", "x86_64-linux"]`)। यदि declared नहीं है तो `null`।
- यदि skill में कोई platform metadata नहीं है तो `metadata` `null` है।
- `moderation` केवल तब included है जब skill flagged हो या owner उसे view कर रहा हो।
...
- Public callers केवल already-flagged visible skills के लिए `200` पाते हैं।
...
- `moderation` नवीनतम संस्करण से व्युत्पन्न वर्तमान skill-स्तर का moderation स्नैपशॉट है।
...
नई ClawScan jobs के लिए प्रमाणित submit endpoint।
...
- Scan request payloads और डाउनलोड योग्य reports retention window के बाद scan-request store से समाप्त हो जाते हैं।
...
Submitted scan के लिए प्रमाणित poll endpoint।

- Queued/running/succeeded/failed status लौटाता है।
- Queued रहने के दौरान `queue.queuedAhead` और `queue.position` लौटाता है ताकि clients दिखा सकें कि request से आगे कितने prioritized manual scans हैं। बहुत बड़ी queues को bound किया जाता है और `queuedAheadIsEstimate: true` के साथ report किया जाता है।
...
Submitted versions के लिए प्रमाणित stored report archive endpoint।
...
- Scan-request downloads जैसी ही ZIP संरचना लौटाता है।
...
- `security.signals.dependencyRegistry` को v1 response compatibility के लिए रखा गया है, लेकिन dependency registry existence scanner retire हो चुका है और यह key हमेशा `null` रहती है।
...
collection endpoint उन clients के लिए है जिन्हें पहले से पता है कि उन्हें कौन से installed
...
- Results per item हैं; कोई एक missing skill या version पूरे response को fail नहीं करता।
...
- Single-skill Skill Card verification envelope की आवश्यकता होने पर `/verify` का उपयोग करें, generated card markdown की आवश्यकता होने पर `/card`, और detailed scanner data की आवश्यकता होने पर `/scan` का उपयोग करें।
...
- Skill प्रविष्टियां Skill रजिस्ट्री से समर्थित रहती हैं और अभी भी केवल `POST /api/v1/skills` के माध्यम से प्रकाशित की जा सकती हैं।
...
`trending` सात-दिन का इंस्टॉल/डाउनलोड लीडरबोर्ड है और all-time कुलों का उपयोग नहीं करता।
...
- `GET /api/v1/plugins` के अंतर्गत दस्तावेज़ित लीगेसी v1 फ़िल्टर उपनाम भी
  स्वीकार किए जाते हैं।
- श्रेणी फ़िल्टरिंग एक वास्तविक API फ़िल्टर है, जो Plugin श्रेणी डाइजेस्ट
  पंक्तियों से समर्थित है, खोज-क्वेरी पुनर्लेखन नहीं।
...
- `version.sha256hash` पुराने क्लाइंट के लिए डिप्रिकेटेड संगतता मेटाडेटा है। यह
...
- `trust.blockedFromDownload` इंस्टॉल ब्लॉक संकेत है। OpenClaw और अन्य
  इंस्टॉल क्लाइंट को यह मान `true` होने पर स्कैनर या मॉडरेशन फ़ील्ड से ब्लॉकिंग नियम
...
- `trust.stale` का अर्थ है कि भरोसा सारांश पुराने इनपुट से कंप्यूट किया गया था और
...
- यह एंडपॉइंट जानबूझकर स्वामी/मॉडरेटर मॉडरेशन
  एंडपॉइंट से संकरा है। यह इंस्टॉल निर्णय और सार्वजनिक स्पष्टीकरण दिखाता है,
...
- लीगेसी पैकेज संस्करण `legacy-zip` आर्टिफैक्ट और लीगेसी ZIP
  `downloadUrl` लौटाते हैं।
- ClawPack संस्करण `npm-pack` आर्टिफैक्ट, npm अखंडता फ़ील्ड,
...
भविष्य के OpenClaw उपभोग के लिए कंप्यूट की गई तैयारी लौटाता है।
...
- `open`: संदिग्ध, हानिकारक, pending, quarantined, revoked, या रिपोर्ट की गई रिलीज़।
...
मॉडरेटर समीक्षा के लिए पैकेज रिपोर्ट करें। रिपोर्टें पैकेज-स्तर की होती हैं, वैकल्पिक रूप से
किसी संस्करण से लिंक की जाती हैं। वे मॉडरेशन कतार को फ़ीड करती हैं लेकिन स्वयं डाउनलोड को
auto-hide या ब्लॉक नहीं करतीं; मॉडरेटर को आर्टिफैक्ट को
approve, quarantine, या revoke करने के लिए रिलीज़ मॉडरेशन का उपयोग करना चाहिए।
...
- `revoked`: किसी release को पहले trusted मानने के बाद blocked।
...
- `dist.tarball`, `dist.integrity`, और `dist.shasum` npm-compatible fields का उपयोग करते हैं
  ताकि users चाहें तो npm को mirror की ओर point कर सकें।
- Scoped package packuments `/api/npm/@scope/name` और npm के encoded
  `/api/npm/@scope%2Fname` request path, दोनों को support करते हैं।
...
Hosted skill version ZIP download करता है, या `clean` या `suspicious` scan वाले और बिना hosted
...
- यदि `version` और `tag` में से कोई भी प्रदान नहीं किया गया है, तो latest version उपयोग किया जाता है।
...
- Optional payload field: `ownerHandle`। मौजूद होने पर, API उस publisher को
  server-side resolve करता है और actor के पास publisher access होना आवश्यक करता है।
- Optional payload field: `migrateOwner`। `ownerHandle` के साथ `true` होने पर, कोई
  existing skill उस owner के पास move हो सकता है यदि actor current और target publishers,
  दोनों पर admin/owner है। इस opt-in के बिना, owner changes reject कर दिए जाते हैं।
...
- Allowed form fields हैं `payload`, repeated `files` blobs, या एक `clawpack`
  tarball reference। `clawpack` कोई `.tgz` blob या upload-url flow द्वारा लौटाया गया storage id
  हो सकता है। Staged storage-id publishes में उस upload URL के साथ लौटाया गया
...
- JSON bodies और caller-supplied `payload.files` / `payload.artifact`
  metadata reject कर दिए जाते हैं।
...
- Plugin packages को `openclaw.plugin.json` की आवश्यकता होती है। ClawPack `.tgz` uploads में
...
- Code plugins को `package.json`, source repo metadata, source commit
  ...
  `openclaw.build.openclawVersion` की आवश्यकता होती है।
...
दूसरा publisher claim कर सकता है। यह expiry लागू होने पर delete response में `slugReservedUntil` शामिल होता है।
...
Newly-created org के लिए, `memberHandle` प्रदान करें; acting admin को member के रूप में add नहीं किया जाता।
...
owner बाद में real code-plugin या bundle-plugin release उसी name में publish कर सके।
...
एंडपॉइंट डिफ़ॉल्ट रूप से ड्राई-रन होता है। रिकवरी लागू करने के लिए स्टाफ द्वारा दोनों GitHub प्रिंसिपलों के बीच निरंतरता स्वतंत्र रूप से सत्यापित करने के बाद `dryRun: false` और
`confirmIdentityVerified: true` आवश्यक हैं। जब गंतव्य उपयोगकर्ता के मौजूदा निजी
प्रकाशक के पास skills, packages या GitHub skill sources हों, तो रिकवरी बंद स्थिति में विफल होती है।
...
direct-owner paths नए publisher authority से मेल खाएं। पुनर्प्राप्त हैंडल के लिए active protected-handle
reservation को भी replacement user को फिर से असाइन किया जाता है ताकि बाद की
profile synchronization पूर्व उपयोगकर्ता की competing authority को फिर से बहाल न कर सके। प्रत्येक primary table प्रति apply transaction
100 rows तक सीमित है; बड़े recovery operations को पहले resumable owner migration का उपयोग करना होगा।
GitHub skill sources publisher-scoped होते हैं और rewritten होने के बजाय checked के रूप में रिपोर्ट किए जाते हैं।
...
- `GET /api/v1/transfers/outgoing`
  - रिस्पॉन्स shape: `{ "transfers": [{ "_id": "...", "skill": { "slug": "demo", "displayName": "Demo" }, "fromUser"|"toUser": { "handle": "..." }, "message": "...", "requestedAt": 0, "expiresAt": 0 }] }`
...
कंटेंट को unban या restore किए बिना मौजूदा ban के लिए stored reason बदलें
