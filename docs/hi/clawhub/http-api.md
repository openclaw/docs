...
x-i18n:
    generated_at: "2026-07-02T14:02:15Z"
    model: gpt-5.5
...
तृतीय-पक्ष निर्देशिकाएँ ClawHub skills को सूचीबद्ध या खोजने के लिए सार्वजनिक read endpoints का उपयोग कर सकती हैं। कृपया परिणाम cache करें, `429`/`Retry-After` का सम्मान करें, उपयोगकर्ताओं को canonical ClawHub listing (`https://clawhub.ai/<owner>/skills/<slug>`) पर वापस link करें, और तृतीय-पक्ष साइट के लिए ClawHub समर्थन का संकेत देने से बचें। public API surface के बाहर छिपी, निजी, या moderation-blocked सामग्री को mirror करने का प्रयास न करें।

Web slug shortcuts registry families में resolve होते हैं, लेकिन API clients को route
precedence को फिर से reconstruct करने के बजाय read endpoints द्वारा लौटाए गए
canonical URLs का उपयोग करना चाहिए।
...
- Authenticated write endpoints को bare `Unauthorized` वापस नहीं करना चाहिए जब
...
संगतता के लिए ignored हैं, लेकिन invalid values वाले recognized query parameters
`400` लौटाते हैं।
...
- ASCII text word और punctuation boundaries पर tokenized होता है। उदाहरण के लिए, `personal-map` में standalone `map` token होता है, जबकि `amap-jsapi-skill` में `amap`, `jsapi`, और `skill` होते हैं; इसलिए `map` खोजने पर `personal-map` को `amap-jsapi-skill` की तुलना में मजबूत lexical match मिलता है।
- Popularity log-scaled और capped है। High-engagement skills तब नीचे rank कर सकते हैं जब query text कमजोर match हो।
...
- वे terms display name, summary, और tags में रखें जिन्हें users सचमुच search करेंगे। Standalone slug token का उपयोग केवल तब करें जब वह ऐसी stable identity भी हो जिसे आप बनाए रखना चाहते हैं।
- किसी slug को केवल एक query के पीछे भागने के लिए rename न करें, जब तक कि नया slug बेहतर long-term canonical name न हो। Old slugs redirect aliases बन जाते हैं, लेकिन canonical URL, displayed slug, और future search digests नया slug उपयोग करते हैं।
- Rename aliases पुराने URLs और registry के माध्यम से resolve होने वाले installs के लिए resolution preserve करते हैं, लेकिन rename indexed हो जाने के बाद search ranking canonical skill metadata पर आधारित होती है। Existing stats skill के साथ रहते हैं।
- यदि कोई skill अप्रत्याशित रूप से invisible है, तो ranking-related metadata बदलने से पहले logged in रहते हुए `clawhub inspect @owner/slug` के साथ moderation state पहले check करें।
...
- `sort` (optional): `updated` (default), `recommended` (alias: `default`), `createdAt` (alias: `newest`), `downloads`, `stars` (alias: `rating`), legacy install aliases `installsCurrent`/`installs`/`installsAllTime` `downloads` पर map करते हैं, `trending`
- `nonSuspiciousOnly` (optional): suspicious (`flagged.suspicious`) skills छिपाने के लिए `true`
- `nonSuspicious` (optional): `nonSuspiciousOnly` के लिए legacy alias
...
- `createdAt` new-skill crawls के लिए stable है; `updated` तब बदलता है जब existing skills फिर से published होते हैं।
...
set करते समय इसे omit किया जा सकता है। Same auditable workflow में skill को hide करने के लिए triaged
...
- `security.hasScanResult` केवल तब `true` होता है जब किसी स्कैनर ने निश्चित verdict (`clean`, `suspicious`, या `malicious`) बनाया हो।
- `moderation` नवीनतम संस्करण से निकला मौजूदा skill-स्तरीय moderation स्नैपशॉट है।
...
नई ClawScan jobs के लिए authenticated submit endpoint.

Local upload scans अब समर्थित नहीं हैं। `multipart/form-data` या `{ "source": { "kind": "upload" } }` का उपयोग करने वाले अनुरोध `410` लौटाते हैं।
...
- Queued रहते समय `queue.queuedAhead` और `queue.position` लौटाता है, ताकि clients दिखा सकें कि अनुरोध से पहले कितने prioritized manual scans हैं। बहुत बड़ी queues bounded होती हैं और `queuedAheadIsEstimate: true` के साथ report की जाती हैं।
...
- `security.signals.dependencyRegistry` v1 response compatibility के लिए रखा गया है, लेकिन dependency registry existence scanner retired है और यह key हमेशा `null` होती है।
- Skill Card की अनुपस्थिति इस endpoint के `ok`, `decision`, या `reasons` को प्रभावित नहीं करती; clients को card content की जरूरत होने पर installed `skill-card.md` locally पढ़ना चाहिए।
- Single-skill Skill Card verification envelope की जरूरत होने पर `/verify`, generated card markdown की जरूरत होने पर `/card`, और detailed scanner data की जरूरत होने पर `/scan` का उपयोग करें।
...
- `category` (वैकल्पिक): plugin श्रेणी फ़िल्टर। केवल तब समर्थित जब
  ...
  `family=code-plugin`/`family=bundle-plugin` वाले पैकेज endpoints) तक सीमित हो। नियंत्रित श्रेणियाँ और
  legacy v1 फ़िल्टर aliases `GET /api/v1/plugins` के अंतर्गत दस्तावेज़ित हैं।
...
- Skill प्रविष्टियाँ skill registry से समर्थित रहती हैं और अभी भी केवल `POST /api/v1/skills` के माध्यम से प्रकाशित की जा सकती हैं।
...
- प्रमाणित कॉलर सूची/खोज परिणामों में उन publishers के निजी पैकेज देख सकते हैं जिनसे वे संबंधित हैं।
...
- `isOfficial` (वैकल्पिक): `true` या `false`
- `category` (वैकल्पिक): plugin श्रेणी फ़िल्टर। केवल तब समर्थित जब
  अनुरोध plugin पैकेजों तक सीमित हो। नियंत्रित श्रेणियाँ और legacy v1
...
- अनाम कॉलर केवल सार्वजनिक पैकेज चैनल देखते हैं।
- प्रमाणित कॉलर उन publishers के निजी पैकेज खोज सकते हैं जिनसे वे संबंधित हैं।
- `channel=private` केवल वे पैकेज लौटाता है जिन्हें प्रमाणित कॉलर पढ़ सकता है।
...
Legacy aliases stored या author-declared श्रेणी मानों के रूप में स्वीकार नहीं किए जाते।
...
- `_errors.json` तब शामिल होता है जब व्यक्तिगत skills या files
...
- `startDate` (आवश्यक): Plugin `updatedAt` के लिए Unix मिलीसेकंड में निचली सीमा।
- `endDate` (आवश्यक): Plugin `updatedAt` के लिए Unix मिलीसेकंड में ऊपरी सीमा।
- `limit` (वैकल्पिक): पूर्णांक (1-250), डिफ़ॉल्ट `250`।
...
- Plugin खोज के लिए ब्राउज़र UI क्रमबद्धता नियंत्रण लोड किए गए प्रासंगिकता परिणामों को
...
- Skills यूनिफ़ाइड कैटलॉग में इस रूट के माध्यम से भी रिज़ॉल्व हो सकते हैं।
...
- पैकेज स्वामी, संगठन पब्लिशर स्वामी/एडमिन, प्लेटफ़ॉर्म मॉडरेटर, या प्लेटफ़ॉर्म एडमिन के लिए API टोकन आवश्यक है।
...
- `version.artifact.kind` पुराने-विश्व पैकेज आर्काइव के लिए `legacy-zip` है या
  ClawPack-आधारित रिलीज़ के लिए `npm-pack`।
...
इंस्टॉल क्लाइंट के लिए सटीक पैकेज रिलीज़ सुरक्षा और भरोसा सारांश लौटाता है। यह यह तय करने के लिए सार्वजनिक OpenClaw उपभोग सतह है कि रिज़ॉल्व की गई रिलीज़ इंस्टॉल की जा सकती है या नहीं।

प्रमाणीकरण:

- सार्वजनिक रीड एंडपॉइंट। कोई स्वामी, पब्लिशर, मॉडरेटर, या एडमिन टोकन
  आवश्यक नहीं है।
...
- `trust.scanStatus` स्कैनर इनपुट और मैन्युअल रिलीज़ मॉडरेशन से निकाला गया प्रभावी भरोसा स्टेटस है।
- `trust.moderationState` nullable है। जब कोई मैन्युअल रिलीज़ मॉडरेशन मौजूद नहीं होता, तो यह `null` होता है।
- `trust.blockedFromDownload` इंस्टॉल ब्लॉक संकेत है। OpenClaw और अन्य
  इंस्टॉल क्लाइंट को स्कैनर या मॉडरेशन फ़ील्ड से ब्लॉकिंग नियमों को फिर से निकालने के बजाय, जब यह मान `true` हो, इंस्टॉलेशन ब्लॉक करना चाहिए।
...
- यह एंडपॉइंट संस्करण-सटीक है। क्लाइंट को इसे उस पैकेज संस्करण को रिज़ॉल्व करने के बाद कॉल करना चाहिए जिसे वे इंस्टॉल करना चाहते हैं, न कि केवल नवीनतम
  पैकेज मेटाडेटा पढ़ने के बाद।
- निजी पैकेज `404` लौटाते हैं, जब तक कॉलर स्वामी पब्लिशर को पढ़ नहीं सकता।
- यह एंडपॉइंट स्वामी/मॉडरेटर मॉडरेशन एंडपॉइंट की तुलना में जानबूझकर संकीर्ण है। यह इंस्टॉल निर्णय और सार्वजनिक स्पष्टीकरण उजागर करता है, रिपोर्टर पहचान,
...
- ClawPack संस्करण `npm-pack` आर्टिफ़ैक्ट, npm इंटेग्रिटी फ़ील्ड,
...
तैयारी जांचें इनको कवर करती हैं:
...
- यह केवल माइग्रेशन तैयारी ट्रैक करता है। यह OpenClaw को म्यूटेट नहीं करता या
...
- `open`: संदिग्ध, दुर्भावनापूर्ण, लंबित, क्वारंटीन किए गए, रद्द किए गए, या रिपोर्ट किए गए रिलीज़।
- `blocked`: क्वारंटीन किए गए, रद्द किए गए, या दुर्भावनापूर्ण रिलीज़।
...
डाउनलोड को ऑटो-हाइड या ब्लॉक नहीं करतीं; मॉडरेटर को आर्टिफ़ैक्ट स्वीकृत, क्वारंटीन, या रद्द करने के लिए रिलीज़ मॉडरेशन का उपयोग करना चाहिए।
...
पुष्ट रिपोर्ट के साथ `finalAction: "quarantine"` या
...
हर बदलाव ऑडिट लॉग प्रविष्टि लिखता है।
...
- फ़ाइल आकार सीमा: 200KB।
- लंबित VirusTotal स्कैन पढ़ने को ब्लॉक नहीं करते; दुर्भावनापूर्ण रिलीज़ कहीं और रोकी जा सकती हैं।
...
- रिस्पॉन्स में रिज़ॉल्वर इंटीग्रिटी जांचों के लिए `ETag`, `Digest`, `X-ClawHub-Artifact-Type`, और
...
- डाउनलोड हेडर में ClawHub SHA-256 और npm इंटीग्रिटी/shasum मेटाडेटा शामिल होता है।
...
होस्टेड skill संस्करण ZIP डाउनलोड करता है, या `clean` या `suspicious` स्कैन और बिना होस्टेड
संस्करण वाली मौजूदा GitHub-समर्थित skill के लिए GitHub स्रोत हैंडऑफ़ लौटाता है।
...
- अगर `version` और `tag` में से कोई भी नहीं दिया गया है, तो नवीनतम संस्करण उपयोग किया जाता है।
...
- GitHub-समर्थित skill हैंडऑफ़ बाइट्स को प्रॉक्सी या मिरर नहीं करते। JSON प्रतिक्रिया
  ...
  और `archiveUrl` शामिल होते हैं; स्कैन/मौजूदा स्थिति एक गेट है और सफल
  पेलोड मेटाडेटा के रूप में शामिल नहीं होती।
- डाउनलोड आंकड़े प्रति UTC दिन विशिष्ट पहचानों के रूप में गिने जाते हैं (`userId` जब API टोकन वैध हो, अन्यथा IP)।
...
- पसंदीदा: `payload` JSON + `files[]` ब्लॉब्स के साथ `multipart/form-data`।
...
- वैकल्पिक पेलोड फ़ील्ड: `migrateOwner`। `ownerHandle` के साथ `true` होने पर,
  मौजूदा skill उस मालिक के पास जा सकती है, अगर अभिनेता वर्तमान और लक्ष्य
  दोनों प्रकाशकों पर एडमिन/मालिक हो। इस opt-in के बिना, मालिक बदलाव
...
- अनुमत फ़ॉर्म फ़ील्ड `payload`, दोहराए गए `files` ब्लॉब्स, या एक `clawpack`
  टारबॉल संदर्भ हैं। `clawpack` `.tgz` ब्लॉब या upload-url फ़्लो द्वारा लौटाई गई
  storage id हो सकता है। Staged storage-id प्रकाशनों में उस अपलोड URL के साथ लौटाया गया
...
- JSON बॉडी और कॉलर-द्वारा-प्रदत्त `payload.files` / `payload.artifact`
  मेटाडेटा अस्वीकार किए जाते हैं।
- सीधे multipart प्रकाशन अनुरोध 18MB पर सीमित हैं। ClawPack टारबॉल
...
- On-behalf प्रकाशन अब भी लक्ष्य मालिक खाते के विरुद्ध official-channel पात्रता सत्यापित करते हैं।
...
मौजूद होने पर, `reason` skill मॉडरेशन नोट के रूप में संग्रहीत किया जाता है और ऑडिट लॉग में कॉपी किया जाता है।
मालिक-आरंभित सॉफ़्ट डिलीट slug को 30 दिनों के लिए आरक्षित रखते हैं, फिर slug पर
कोई दूसरा प्रकाशक दावा कर सकता है। यह समाप्ति लागू होने पर delete प्रतिक्रिया में `slugReservedUntil` शामिल होता है।
मॉडरेटर/एडमिन हाइड और सुरक्षा हटाने इस तरह समाप्त नहीं होते।
...
केवल एडमिन। किसी हैंडल के लिए org प्रकाशक का मौजूद होना सुनिश्चित करता है। अगर हैंडल अभी भी
लेगेसी साझा उपयोगकर्ता/व्यक्तिगत प्रकाशक की ओर इशारा करता है, तो एंडपॉइंट पहले उसे org प्रकाशक में माइग्रेट करता है।
नए बनाए गए org के लिए `memberHandle` दें; कार्यरत एडमिन सदस्य के रूप में नहीं जोड़ा जाता।
...
कॉलर को मालिक के रूप में जोड़ता है। यह एंडपॉइंट मौजूदा उपयोगकर्ता/व्यक्तिगत हैंडल को माइग्रेट नहीं करता और
...
मालिक बाद में उस नाम में वास्तविक code-plugin या bundle-plugin रिलीज़ प्रकाशित कर सके।
...
केवल एडमिन। Convex Auth खाता पंक्तियों को संपादित किए बिना सत्यापित प्रतिस्थापन GitHub OAuth principal
के लिए व्यक्तिगत प्रकाशक पुनर्प्राप्त करता है। अनुरोध में दोनों अपरिवर्तनीय GitHub
provider account ids का नाम होना चाहिए; बदल सकने वाले हैंडल केवल ऑपरेटर-फेसिंग गार्ड के रूप में उपयोग किए जाते हैं।

endpoint डिफ़ॉल्ट रूप से ड्राई-रन पर होता है। रिकवरी लागू करने के लिए `dryRun: false` और
`confirmIdentityVerified: true` आवश्यक हैं, जब स्टाफ दोनों
GitHub principals के बीच निरंतरता को स्वतंत्र रूप से सत्यापित कर लें। जब गंतव्य उपयोगकर्ता के मौजूदा व्यक्तिगत
publisher के पास skills, packages, या GitHub skill sources हों, तो रिकवरी सुरक्षित रूप से विफल होती है।
रिकवरी पुनर्प्राप्त publisher की skills, skill slug aliases, packages, package inspector warnings, और derived search digest rows के लिए legacy `ownerUserId` fields भी माइग्रेट करती है, ताकि
direct-owner paths नए publisher authority से मेल खाएँ। पुनर्प्राप्त handle के लिए सक्रिय protected-handle
reservation को भी replacement user को फिर से असाइन किया जाता है, ताकि बाद का
...
unban किए बिना या content restore किए बिना किसी मौजूदा ban का stored reason बदलें
