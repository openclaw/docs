...
x-i18n:
    generated_at: "2026-07-02T08:16:43Z"
    model: gpt-5.5
...
वेब slug शॉर्टकट registry families में resolve होते हैं, लेकिन API क्लाइंट्स को route precedence फिर से बनाने के बजाय रीड एंडपॉइंट्स द्वारा लौटाए गए कैनॉनिकल URLs का उपयोग करना चाहिए।
...
- प्रमाणित write एंडपॉइंट्स को bare `Unauthorized` नहीं लौटाना चाहिए जब सर्वर कारण जानता हो। अनुपस्थित tokens, अमान्य/रद्द tokens, और हटाए गए/प्रतिबंधित/अक्षम खातों में से प्रत्येक को actionable text मिलना चाहिए ताकि CLI क्लाइंट्स उपयोगकर्ताओं को बता सकें कि उन्हें किसने रोका।
...
- `X-RateLimit-Remaining` / `RateLimit-Remaining`: मौजूद होने पर सटीक शेष budget।
  Sharded सफल अनुरोध approximate global value लौटाने के बजाय इस header को omit करते हैं।
...
- trusted client IP headers, जिनमें `cf-connecting-ip` भी शामिल है, केवल तब उपयोग करता है जब deployment trusted forwarded headers को स्पष्ट रूप से enable करता है।
- ClawHub edge पर client IPs की पहचान करने के लिए trusted forwarding headers का उपयोग करता है।
- यदि कोई trusted client IP उपलब्ध नहीं है, तो anonymous requests fallback buckets का उपयोग करते हैं जो केवल rate-limit kind के scope में होते हैं। इन fallback buckets में caller-supplied paths, slugs, package names, versions, query strings, या अन्य artifact parameters शामिल नहीं होते।

## Error responses

सार्वजनिक v1 error responses `content-type: text/plain; charset=utf-8` के साथ plain text होते हैं।
इसमें validation failures (`400`), अनुपस्थित public resources (`404`), auth और permission failures (`401`/`403`), rate limits (`429`), और blocked downloads शामिल हैं। Clients को response body को human-readable string के रूप में पढ़ना चाहिए। Unknown query parameters संगतता के लिए ignore किए जाते हैं, लेकिन अमान्य values वाले recognized query parameters `400` लौटाते हैं।
...
- Relevance, popularity से अधिक मजबूत है। सटीक slug या display-name token match, बहुत अधिक engagement वाले ढीले match से ऊपर rank कर सकता है।
- ASCII text को word और punctuation boundaries पर tokenize किया जाता है। उदाहरण के लिए, `personal-map` में standalone `map` token होता है, जबकि `amap-jsapi-skill` में `amap`, `jsapi`, और `skill` होते हैं; इसलिए `map` की खोज `personal-map` को `amap-jsapi-skill` की तुलना में stronger lexical match देती है।
- Popularity log-scaled और capped है। High-engagement skills query text के कमजोर match होने पर नीचे rank कर सकते हैं।
...
- जिन terms को users सचमुच search करेंगे, उन्हें display name, summary, और tags में रखें। standalone slug token का उपयोग केवल तब करें जब वह एक stable identity भी हो जिसे आप बनाए रखना चाहते हैं।
- केवल किसी एक query को chase करने के लिए slug का rename न करें, जब तक नया slug बेहतर long-term canonical name न हो। पुराने slugs redirect aliases बन जाते हैं, लेकिन canonical URL, displayed slug, और future search digests नया slug उपयोग करते हैं।
- Rename aliases पुराने URLs और registry के माध्यम से resolve होने वाले installs के लिए resolution preserve करते हैं, लेकिन rename indexed हो जाने के बाद search ranking canonical skill metadata पर आधारित होती है। Existing stats skill के साथ बने रहते हैं।
- यदि कोई skill अप्रत्याशित रूप से invisible है, तो ranking-related metadata बदलने से पहले logged in रहते हुए `clawhub inspect @owner/slug` से पहले moderation state जाँचें।
...
अमान्य `sort` values `400` लौटाती हैं।
...
- `createdAt` new-skill crawls के लिए stable है; `updated` तब बदलता है जब existing skills republish किए जाते हैं।
- जब `nonSuspiciousOnly=true` होता है, cursor-based sorts किसी page पर `limit` items से कम लौटा सकते हैं क्योंकि suspicious skills page retrieval के बाद filter किए जाते हैं।
- मौजूद होने पर pagination जारी रखने के लिए `nextCursor` का उपयोग करें। छोटा page अपने आप end-of-results नहीं दर्शाता।
...
- Public callers के लिए evidence redacted होता है और raw snippets केवल owners/moderators के लिए शामिल होते हैं।

### `POST /api/v1/skills/{slug}/report`

Moderator review के लिए skill report करें। Reports skill-level हैं, वैकल्पिक रूप से किसी version से linked, और skill report queue को feed करती हैं।
...
`confirmed` और `dismissed` के लिए `note` आवश्यक है; `status` को वापस `open` पर set करते समय इसे omit किया जा सकता है। उसी auditable workflow में skill छिपाने के लिए triaged report के साथ `finalAction: "hide"` pass करें।
...
- `security.hasScanResult` केवल तब `true` होता है जब किसी scanner ने निर्णायक verdict (`clean`, `suspicious`, या `malicious`) दिया हो।
- `moderation` नवीनतम संस्करण से प्राप्त मौजूदा skill-स्तरीय moderation snapshot है।
...
नए ClawScan jobs के लिए प्रमाणित submit endpoint।
...
- Exact submitted version के stored scan results लौटाता है, जिसमें blocked या hidden versions भी शामिल हैं।
- `kind` का default `skill` है; plugin/package scans के लिए `kind=plugin` उपयोग करें।
...
- `security.signals.dependencyRegistry` v1 response compatibility के लिए retained है, लेकिन dependency registry existence scanner retired है और यह key हमेशा `null` होती है।
- Skill Card की अनुपस्थिति इस endpoint के `ok`, `decision`, या `reasons` को affect नहीं करती; card content की आवश्यकता होने पर clients को installed `skill-card.md` locally पढ़ना चाहिए।
- Single-skill Skill Card verification envelope की आवश्यकता होने पर `/verify`, generated card markdown की आवश्यकता होने पर `/card`, और detailed scanner data की आवश्यकता होने पर `/scan` उपयोग करें।
...
लेगेसी उपनामों को संग्रहीत या लेखक-घोषित श्रेणी मानों के रूप में स्वीकार नहीं किया जाता।
...
- `startDate` (आवश्यक): Skill `updatedAt` के लिए Unix मिलीसेकंड निम्न सीमा।
- `endDate` (आवश्यक): Skill `updatedAt` के लिए Unix मिलीसेकंड उच्च सीमा।
...
- `clean` या `suspicious` स्कैन वाले वर्तमान GitHub-समर्थित Skills में
  ...
  कंटेंट हैश, और आर्काइव URL होता है। इनमें ClawHub-होस्टेड स्रोत फ़ाइलें शामिल नहीं होतीं।
...
- `limit` (वैकल्पिक): पूर्णांक (1-250), डिफ़ॉल्ट `250`।
- `cursor` (वैकल्पिक): पिछले प्रतिक्रिया से पेजिनेशन कर्सर।
...
- पैकेज स्वामी, संगठन प्रकाशक स्वामी/एडमिन, प्लेटफ़ॉर्म मॉडरेटर, या प्लेटफ़ॉर्म एडमिन के लिए API टोकन आवश्यक।
...
यह यह तय करने के लिए सार्वजनिक OpenClaw उपभोग सतह है कि कोई रिज़ॉल्व की गई रिलीज़
...
- `release.releaseId`, `release.version`, और `release.createdAt` उस सटीक
  रिलीज़ की पहचान करते हैं जिसका मूल्यांकन किया गया।
...
- `trust.scanStatus` स्कैनर इनपुट और मैनुअल रिलीज़ मॉडरेशन से निकाली गई प्रभावी विश्वास स्थिति है।
- `trust.moderationState` nullable है। जब कोई मैनुअल रिलीज़ मॉडरेशन मौजूद नहीं होता तो यह `null` होता है।
- `trust.blockedFromDownload` इंस्टॉल ब्लॉक संकेत है। OpenClaw और अन्य
  ...
  ब्लॉकिंग नियम फिर से निकालने के बजाय इंस्टॉलेशन रोकना चाहिए।
...
- यह एंडपॉइंट स्वामी/मॉडरेटर मॉडरेशन एंडपॉइंट की तुलना में जानबूझकर संकरा है।
  यह इंस्टॉल निर्णय और सार्वजनिक स्पष्टीकरण उजागर करता है, रिपोर्टर पहचान,
...
- मॉडरेटर या एडमिन उपयोगकर्ता के लिए API टोकन आवश्यक।
...
- एडमिन उपयोगकर्ता के लिए API टोकन आवश्यक।
...
- यह केवल माइग्रेशन तत्परता ट्रैक करता है। यह OpenClaw को म्यूटेट नहीं करता या
...
- `open`: संदिग्ध, malicious, pending, quarantined, revoked, या रिपोर्ट की गई रिलीज़।
...
किसी संस्करण से लिंक की जाती हैं। वे मॉडरेशन कतार को फ़ीड करती हैं लेकिन स्वयं
डाउनलोड को ऑटो-हाइड या ब्लॉक नहीं करतीं; मॉडरेटर को आर्टिफ़ैक्ट स्वीकृत,
quarantine, या revoke करने के लिए रिलीज़ मॉडरेशन का उपयोग करना चाहिए।
...
पैकेज रिपोर्ट इनटेक के लिए मॉडरेटर/admin endpoint.

Auth:

- मॉडरेटर या admin उपयोगकर्ता के लिए API token आवश्यक है.
...
- पैकेज owner, publisher member, मॉडरेटर, या
  admin उपयोगकर्ता के लिए API token आवश्यक है.
...
सेट करते समय इसे छोड़ा जा सकता है. उसी audit योग्य workflow में release moderation लागू करने के लिए
...
पैकेज release समीक्षा के लिए मॉडरेटर/admin endpoint.
...
- `revoked`: पहले trusted release होने के बाद blocked.
...
- डिफ़ॉल्ट रूप से latest release का उपयोग करता है.
- download bucket नहीं, read rate bucket का उपयोग करता है.
- Binary files `415` लौटाती हैं.
- File size limit: 200KB.
- Pending VirusTotal scans reads को block नहीं करते; malicious releases अभी भी कहीं और रोकी जा सकती हैं.
...
- Scoped package packuments `/api/npm/@scope/name` और npm के
  encoded `/api/npm/@scope%2Fname` request path, दोनों support करते हैं.
...
- Download headers में ClawHub SHA-256 plus npm integrity/shasum metadata शामिल होता है.
...
Hosted skill version ZIP download करता है, या किसी current GitHub-backed skill के लिए GitHub source handoff लौटाता है, जब scan `clean` या `suspicious` हो और कोई hosted
version न हो.
...
- Optional payload field: `migrateOwner`. `ownerHandle` के साथ `true` होने पर,
  existing skill उस owner में move हो सकती है यदि actor current और target publishers,
  दोनों पर admin/owner है. इस opt-in के बिना, owner changes
  rejected होते हैं.
...
- Allowed form fields हैं `payload`, repeated `files` blobs, या एक `clawpack`
  tarball reference. `clawpack` `.tgz` blob हो सकता है या
  upload-url flow द्वारा लौटाया गया storage id. Staged storage-id publishes में उस upload URL के साथ लौटाया गया
  `clawpackUploadTicket` भी शामिल होना चाहिए.
- उसी request में या तो `files` या `clawpack` का उपयोग करें, दोनों कभी नहीं.
...
Admin-only. किसी handle के लिए org publisher मौजूद है यह सुनिश्चित करता है. यदि handle अभी भी
...
एंडपॉइंट डिफ़ॉल्ट रूप से dry-run पर रहता है। रिकवरी लागू करने के लिए स्टाफ़ द्वारा दोनों
...
रिकवरी, पुनर्प्राप्त publisher की skills, skill slug aliases, packages, package inspector warnings, और derived search digest rows के लिए legacy `ownerUserId` फ़ील्ड भी माइग्रेट करती है ताकि
direct-owner paths नए publisher authority से मेल खाएँ। पुनर्प्राप्त handle के लिए active protected-handle
...
GitHub skill sources publisher-scoped हैं और rewritten होने के बजाय checked के रूप में report किए जाते हैं।
...
Unban या content restore किए बिना मौजूदा ban का stored reason बदलें
(केवल admin)। जब तक `dryRun` `false` न हो, डिफ़ॉल्ट रूप से dry-run रहता है।
