...
x-i18n:
    generated_at: "2026-07-19T08:17:19Z"
    model: gpt-5.6
...
तृतीय-पक्ष डायरेक्टरियाँ ClawHub Skills को सूचीबद्ध करने या खोजने के लिए सार्वजनिक रीड एंडपॉइंट का उपयोग कर सकती हैं। कृपया परिणामों को कैश करें, `429`/`Retry-After` का पालन करें, उपयोगकर्ताओं को प्रामाणिक ClawHub सूची (`https://clawhub.ai/<owner>/skills/<slug>`) पर वापस भेजें और ऐसा संकेत देने से बचें कि ClawHub तृतीय-पक्ष साइट का समर्थन करता है। सार्वजनिक API सतह के बाहर छिपी, निजी या मॉडरेशन द्वारा अवरुद्ध सामग्री को मिरर करने का प्रयास न करें।
...
- जब सर्वर को कारण ज्ञात हो, तो प्रमाणित राइट एंडपॉइंट को केवल `Unauthorized` नहीं लौटाना चाहिए।
  ...
  प्रत्येक के लिए कार्रवाई योग्य टेक्स्ट मिलना चाहिए, ताकि CLI
  क्लाइंट उपयोगकर्ताओं को बता सकें कि उन्हें किस कारण अवरुद्ध किया गया।
...
सार्वजनिक v1 त्रुटि प्रतिक्रियाएँ `content-type: text/plain; charset=utf-8` सहित प्लेन टेक्स्ट होती हैं।
...
- `highlightedOnly` (वैकल्पिक): हाइलाइट की गई Skills तक फ़िल्टर करने के लिए `true`
...
- परिणाम प्रासंगिकता क्रम में लौटाए जाते हैं (एम्बेडिंग समानता + सटीक स्लग/नाम टोकन बूस्ट + लोकप्रियता की एक छोटी पूर्व-प्राथमिकता)।
- प्रासंगिकता लोकप्रियता से अधिक प्रभावी है। स्लग या डिस्प्ले-नाम टोकन का सटीक मिलान, कहीं अधिक सहभागिता वाले ढीले मिलान से ऊपर स्थान पा सकता है।
- ASCII टेक्स्ट को शब्द और विराम-चिह्न सीमाओं पर टोकन में बाँटा जाता है। उदाहरण के लिए, `personal-map` में एक स्वतंत्र `map` टोकन है, जबकि `amap-jsapi-skill` में `amap`, `jsapi` और `skill` हैं; इसलिए `map` की खोज में `personal-map` को `amap-jsapi-skill` की तुलना में अधिक मजबूत शाब्दिक मिलान मिलता है।
- लोकप्रियता लॉग-स्केल की जाती है और उसकी ऊपरी सीमा निर्धारित है। क्वेरी टेक्स्ट का मिलान कमजोर होने पर अधिक सहभागिता वाली Skills की रैंक नीचे हो सकती है।
...
- जिन शब्दों को उपयोगकर्ता वास्तव में खोजेंगे, उन्हें डिस्प्ले नाम, सारांश और टैग में रखें। स्वतंत्र स्लग टोकन का उपयोग केवल तभी करें, जब वह एक स्थायी पहचान भी हो जिसे आप बनाए रखना चाहते हैं।
- केवल एक क्वेरी के पीछे जाने के लिए स्लग का नाम न बदलें, जब तक नया स्लग दीर्घकाल के लिए बेहतर प्रामाणिक नाम न हो। पुराने स्लग रीडायरेक्ट उपनाम बन जाते हैं, लेकिन प्रामाणिक URL, प्रदर्शित स्लग और भावी खोज डाइजेस्ट नए स्लग का उपयोग करते हैं।
- नाम-परिवर्तन उपनाम पुराने URL और रजिस्ट्री के माध्यम से रिज़ॉल्व होने वाले इंस्टॉल के लिए रिज़ॉल्यूशन बनाए रखते हैं, लेकिन नाम-परिवर्तन के इंडेक्स हो जाने के बाद खोज रैंकिंग प्रामाणिक Skill मेटाडेटा पर आधारित होती है। मौजूदा आँकड़े Skill के साथ बने रहते हैं।
...
- `cursor` (वैकल्पिक): किसी भी गैर-`trending` सॉर्ट के लिए पृष्ठांकन कर्सर
- `sort` (वैकल्पिक): `updated` (डिफ़ॉल्ट), `recommended` (उपनाम: `default`), `createdAt` (उपनाम: `newest`), `downloads`, `stars` (उपनाम: `rating`), पुराने इंस्टॉल उपनाम `installsCurrent`/`installs`/`installsAllTime` को `downloads`, `trending` से मैप किया जाता है
- `nonSuspiciousOnly` (वैकल्पिक): संदिग्ध (`flagged.suspicious`) Skills छिपाने के लिए `true`
- `nonSuspicious` (वैकल्पिक): `nonSuspiciousOnly` का पुराना उपनाम
...
- `createdAt` नई Skill क्रॉल के लिए स्थिर है; मौजूदा Skills के पुनः प्रकाशित होने पर `updated` बदलता है।
- `nonSuspiciousOnly=true` होने पर, कर्सर-आधारित सॉर्ट एक पृष्ठ पर `limit` से कम आइटम लौटा सकते हैं, क्योंकि संदिग्ध Skills को पृष्ठ प्राप्ति के बाद फ़िल्टर किया जाता है।
- मौजूद होने पर पृष्ठांकन जारी रखने के लिए `nextCursor` का उपयोग करें। केवल छोटा पृष्ठ मिलने का अर्थ परिणामों का अंत नहीं है।
...
- स्वामी के नाम-परिवर्तन/विलय प्रवाह द्वारा बनाए गए पुराने स्लग प्रामाणिक Skill पर रिज़ॉल्व होते हैं।
...
- स्वामी और मॉडरेटर छिपी Skills के मॉडरेशन विवरण देख सकते हैं।
- सार्वजनिक कॉलर को पहले से फ़्लैग की गई दृश्यमान Skills के लिए केवल `200` मिलता है।
- सार्वजनिक कॉलर के लिए साक्ष्य संशोधित किया जाता है और केवल स्वामियों/मॉडरेटरों के लिए कच्चे स्निपेट शामिल करता है।
...
Skill रिपोर्ट स्वीकार करने के लिए मॉडरेटर/एडमिन एंडपॉइंट।
...
स्किल रिपोर्टों को हल करने या फिर से खोलने के लिए मॉडरेटर/एडमिन एंडपॉइंट।
...
- `tag` (वैकल्पिक): टैग किए गए संस्करण को हल करें (उदाहरण के लिए `latest`)।
...
- स्कैन जॉब अतुल्यकालिक होते हैं। मैन्युअल स्कैन अनुरोधों को सामान्य प्रकाशन/बैकफ़िल कार्य से पहले प्राथमिकता दी जाती है, लेकिन उनका पूरा होना फिर भी वर्कर की उपलब्धता पर निर्भर करता है।

### `GET /api/v1/skills/-/scan/{scanId}`

सबमिट किए गए स्कैन के लिए प्रमाणीकृत पोलिंग एंडपॉइंट।
...
- इसके लिए सफल स्कैन आवश्यक है; गैर-अंतिम स्कैन `409` लौटाते हैं।
...
- इसके लिए स्किल या plugin पर स्वामी/प्रकाशक प्रबंधन पहुँच या प्लेटफ़ॉर्म मॉडरेटर/एडमिन अधिकार आवश्यक हैं।
...
केवल एडमिन के लिए कैनोनिकल बैच रीस्कैन रूट। यह पुराने `POST /api/v1/skills/-/rescan-batch` के समान पेलोड संरचना स्वीकार करता है।

### `POST /api/v1/skills/-/scan/batch/status`

केवल एडमिन के लिए कैनोनिकल बैच स्थिति रूट। यह `{ "jobIds": ["..."] }` स्वीकार करता है और पुराने `POST /api/v1/skills/-/rescan-batch/status` के समान समेकित काउंटर लौटाता है।
...
- `ok` केवल तभी `true` होता है, जब चयनित संस्करण में जनरेट किया गया Skill Card हो, वह मॉडरेशन द्वारा मैलवेयर के कारण अवरुद्ध न हो, और ClawScan सत्यापन साफ़ हो।
...
- `security.signals` में `staticScan`, `virusTotal`, और `skillSpector` जैसे सहायक स्कैनर प्रमाण होते हैं।
- `security.signals.dependencyRegistry` को v1 प्रतिक्रिया संगतता के लिए बनाए रखा गया है, लेकिन निर्भरता रजिस्ट्री अस्तित्व स्कैनर सेवानिवृत्त हो चुका है और यह कुंजी हमेशा `null` रहती है।
- `provenance` केवल तभी `server-resolved-github-import` होता है, जब ClawHub ने प्रकाशन या आयात के दौरान GitHub रिपॉज़िटरी/रेफ़/कमिट/पथ को हल करके संग्रहीत किया हो; अन्यथा यह `unavailable` होता है।

### `POST /api/v1/skills/-/security-verdicts`

सटीक स्किल संस्करणों के लिए वर्तमान संक्षिप्त सुरक्षा निर्णय लौटाता है। यह संग्रह एंडपॉइंट उन क्लाइंट के लिए है, जिन्हें पहले से पता है कि उन्हें कौन-से इंस्टॉल किए गए ClawHub स्किल संस्करण प्रदर्शित करने हैं, जैसे OpenClaw Control UI।
...
- परिणाम प्रत्येक आइटम के लिए अलग होते हैं; एक अनुपलब्ध स्किल या संस्करण पूरी प्रतिक्रिया को विफल नहीं करता।
...
- `security.signals` में केवल स्थिति-स्तरीय सहायक प्रमाण होते हैं; स्कैनर के पूर्ण विवरण के लिए `/scan` या ClawHub सुरक्षा-ऑडिट पृष्ठ का उपयोग करें।
- `security.signals.dependencyRegistry` को v1 प्रतिक्रिया संगतता के लिए बनाए रखा गया है, लेकिन निर्भरता रजिस्ट्री अस्तित्व स्कैनर सेवानिवृत्त हो चुका है और यह कुंजी हमेशा `null` रहती है।
...
- एकल-स्किल Skill Card सत्यापन एनवेलप की आवश्यकता होने पर `/verify`, जनरेट किए गए कार्ड Markdown की आवश्यकता होने पर `/card`, और विस्तृत स्कैनर डेटा की आवश्यकता होने पर `/scan` का उपयोग करें।
...
- `category` (वैकल्पिक): plugin श्रेणी फ़िल्टर। केवल तभी समर्थित है, जब अनुरोध
  plugin पैकेजों तक सीमित हो (`/api/v1/plugins`,
...
- `category` (वैकल्पिक): Plugin श्रेणी फ़िल्टर। केवल तब समर्थित, जब
  अनुरोध का दायरा Plugin पैकेजों तक सीमित हो। नियंत्रित श्रेणियाँ और विरासती v1
...
- `cursor` (वैकल्पिक): पृष्ठांकन कर्सर
- `isOfficial` (वैकल्पिक): `true` या `false`
- `sort` (वैकल्पिक): `recommended` (डिफ़ॉल्ट), `trending`, `downloads`, `updated`, विरासती उपनाम `installs`
- `category` (वैकल्पिक): Plugin श्रेणी फ़िल्टर। वर्तमान मान:
  `channels`, `models`, `memory`, `context`, `voice`, `media`, `web`,
  `tools`, `runtime`, `gateway`, `security`, `other`।

विरासती v1 फ़िल्टर उपनाम पठन एंडपॉइंट पर अब भी स्वीकार किए जाते हैं:
...
विरासती उपनाम संग्रहीत या लेखक द्वारा घोषित श्रेणी मानों के रूप में स्वीकार नहीं किए जाते।
...
- `clean` या `suspicious` स्कैन वाले वर्तमान GitHub-समर्थित Skills में
  ...
  सामग्री हैश और संग्रह URL शामिल होते हैं। इनमें ClawHub द्वारा होस्ट की गई स्रोत फ़ाइलें शामिल नहीं होतीं।
...
- `_manifest.json` हमेशा ZIP के मूल में शामिल होता है।
- जब अलग-अलग Plugins या फ़ाइलें निर्यात नहीं की जा सकीं, तब
  `_errors.json` शामिल होता है।
...
- `GET /api/v1/plugins` के अंतर्गत प्रलेखित विरासती v1 फ़िल्टर उपनाम भी
...
- एकीकृत कैटलॉग में इस रूट के माध्यम से Skills का भी समाधान किया जा सकता है।
- यदि कॉलर स्वामी प्रकाशक को नहीं पढ़ सकता, तो निजी पैकेज `404` लौटाते हैं।

### `DELETE /api/v1/packages/{name}`

पैकेज और उसकी सभी रिलीज़ को सॉफ़्ट-डिलीट करता है।

टिप्पणियाँ:

- पैकेज स्वामी, संगठन प्रकाशक स्वामी/व्यवस्थापक,
  प्लेटफ़ॉर्म मॉडरेटर या प्लेटफ़ॉर्म व्यवस्थापक का API टोकन आवश्यक है।
...
- पुरानी व्यवस्था के पैकेज संग्रहों के लिए `version.artifact.kind` का मान `legacy-zip` या
...
- `version.sha256hash` पुराने क्लाइंट के लिए अप्रचलित संगतता मेटाडेटा है। यह
  ...
  प्रामाणिक रिलीज़ आर्टिफ़ैक्ट की पहचान करता है।
...
इंस्टॉल क्लाइंट के लिए सटीक पैकेज रिलीज़ सुरक्षा और विश्वसनीयता सारांश लौटाता है।
यह तय करने के लिए कि समाधान की गई रिलीज़ इंस्टॉल की जा सकती है या नहीं, यह सार्वजनिक OpenClaw
उपभोग सतह है।

प्रमाणीकरण:

- सार्वजनिक पठन एंडपॉइंट। स्वामी, प्रकाशक, मॉडरेटर या व्यवस्थापक टोकन
  आवश्यक नहीं है।
...
- `trust.scanStatus` स्कैनर इनपुट और मैन्युअल रिलीज़ मॉडरेशन से प्राप्त
  प्रभावी विश्वसनीयता स्थिति है।
- `trust.moderationState` का मान शून्य-योग्य है। मैन्युअल रिलीज़
  मॉडरेशन मौजूद न होने पर इसका मान `null` होता है।
- `trust.blockedFromDownload` इंस्टॉल अवरोध संकेत है। OpenClaw और अन्य
  इंस्टॉल क्लाइंट को स्कैनर या मॉडरेशन फ़ील्ड से अवरोध नियम पुनः प्राप्त करने के बजाय,
...
- `trust.pending` का अर्थ है कि एक या अधिक विश्वसनीयता इनपुट अभी भी पूरा होने की प्रतीक्षा में हैं।
- `trust.stale` का अर्थ है कि विश्वसनीयता सारांश की गणना पुराने इनपुट से की गई थी और
...
- यह एंडपॉइंट जानबूझकर स्वामी/मॉडरेटर मॉडरेशन
  एंडपॉइंट की तुलना में अधिक सीमित है। यह इंस्टॉल निर्णय और सार्वजनिक स्पष्टीकरण उजागर करता है,
  रिपोर्टकर्ता की पहचान, रिपोर्ट की सामग्री, निजी साक्ष्य या आंतरिक समीक्षा
  समयरेखा नहीं।
...
- विरासती पैकेज संस्करण `legacy-zip` आर्टिफ़ैक्ट और विरासती ZIP
  `downloadUrl` लौटाते हैं।
- ClawPack संस्करण `npm-pack` आर्टिफ़ैक्ट, npm अखंडता फ़ील्ड,
  `tarballUrl`, और विरासती ZIP संगतता URL लौटाते हैं।
- यह OpenClaw रिज़ॉल्वर सतह है; यह साझा URL से संग्रह प्रारूप का अनुमान लगाने से बचती है।
...
- ClawPack संस्करण अपलोड किए गए npm-pack की हूबहू `.tgz` बाइट्स स्ट्रीम करते हैं।
- पुराने ZIP संस्करण `/api/v1/packages/{name}/download?version=` पर रीडायरेक्ट करते हैं।
- डाउनलोड दर बकेट का उपयोग करता है।

### `GET /api/v1/packages/{name}/readiness`

भविष्य में OpenClaw द्वारा उपयोग के लिए परिकलित तत्परता लौटाता है।
...
- स्रोत रिपॉज़िटरी और कमिट का उद्गम
  ...
  "checks": [
    {
      ...
      "status": "fail",
      "message": "नवीनतम संस्करण केवल पुराने ZIP स्वरूप में है।"
    }
  ],
  "blockers": ["clawpack"]
}
...
- मॉडरेटर या एडमिन उपयोगकर्ता के API टोकन की आवश्यकता होती है।
...
- एडमिन उपयोगकर्ता के API टोकन की आवश्यकता होती है।
...
- `packageName` को npm नाम के अनुसार सामान्यीकृत किया जाता है; नियोजित माइग्रेशन के लिए पैकेज
  अनुपलब्ध हो सकता है।
- यह केवल माइग्रेशन की तत्परता ट्रैक करता है। यह OpenClaw में परिवर्तन नहीं करता या
...
किसी संस्करण से लिंक की जा सकती हैं। वे मॉडरेशन कतार में जाती हैं, लेकिन स्वयं स्वचालित रूप से
डाउनलोड छिपाती या अवरुद्ध नहीं करतीं; आर्टिफ़ैक्ट को स्वीकृत, क्वारंटीन या निरस्त करने के लिए
मॉडरेटर को रिलीज़ मॉडरेशन का उपयोग करना चाहिए।

प्रमाणीकरण:

- API टोकन की आवश्यकता होती है।
...
पैकेज रिपोर्ट का समाधान करने या उन्हें दोबारा खोलने के लिए मॉडरेटर/एडमिन एंडपॉइंट।
...
{
  "status": "confirmed",
  "note": "समीक्षा करके प्रभावित रिलीज़ को क्वारंटीन किया गया।",
  "finalAction": "quarantine"
}
```

`note`, `confirmed` और `dismissed` के लिए आवश्यक है; `status` को वापस
`open` पर सेट करते समय इसे छोड़ा जा सकता है। उसी ऑडिट-योग्य कार्यप्रवाह में रिलीज़ मॉडरेशन
...
- `revoked`: किसी रिलीज़ पर पहले विश्वास किए जाने के बाद अवरुद्ध।
...
पैकेज फ़ाइल की मूल टेक्स्ट सामग्री लौटाता है।
...
- फ़ाइल आकार सीमा: 200KB।
- लंबित VirusTotal स्कैन पढ़ने को अवरुद्ध नहीं करते; दुर्भावनापूर्ण रिलीज़ को अन्यत्र फिर भी रोका जा सकता है।
...
- Plugin/पैकेज संग्रह `package/` रूट वाली zip फ़ाइलें हैं, ताकि पुराने OpenClaw
...
- केवल पुराने ZIP स्वरूप वाले संस्करण जानबूझकर छोड़े जाते हैं।
- `dist.tarball`, `dist.integrity`, और `dist.shasum` npm-संगत
  फ़ील्ड का उपयोग करते हैं, ताकि उपयोगकर्ता चाहें तो npm को मिरर की ओर निर्देशित कर सकें।
- स्कोप किए गए पैकेज packument, `/api/npm/@scope/name` और npm के
  एन्कोड किए गए `/api/npm/@scope%2Fname` अनुरोध पथ, दोनों का समर्थन करते हैं।

### `GET /api/npm/{package}/-/{tarball}.tgz`

npm मिरर क्लाइंट के लिए अपलोड किए गए ClawPack tarball की हूबहू बाइट्स स्ट्रीम करता है।
...
स्थानीय फ़िंगरप्रिंट को किसी ज्ञात संस्करण से मैप करने के लिए CLI द्वारा उपयोग किया जाता है।

क्वेरी पैरामीटर:

- `slug` (आवश्यक)
- `hash` (आवश्यक): बंडल फ़िंगरप्रिंट का 64-वर्णीय hex sha256
...
होस्ट किए गए skill संस्करण का ZIP डाउनलोड करता है, या ऐसे वर्तमान GitHub-समर्थित skill के लिए
GitHub स्रोत हैंडऑफ़ लौटाता है, जिसका `clean` या `suspicious` स्कैन हो और कोई होस्ट किया गया
संस्करण न हो।
...
- GitHub-समर्थित skill हस्तांतरण बाइट्स को प्रॉक्सी या मिरर नहीं करते। JSON प्रतिक्रिया में
...
- डाउनलोड आँकड़ों में प्रति UTC दिन विशिष्ट पहचानों की गणना होती है (API टोकन मान्य होने पर `userId`, अन्यथा IP)।
...
- वरीयता: `payload` JSON + `files[]` ब्लॉब के साथ `multipart/form-data`।
...
- वैकल्पिक पेलोड फ़ील्ड: `migrateOwner`। `ownerHandle` के साथ `true` होने पर,
  यदि कर्ता वर्तमान और लक्षित दोनों प्रकाशकों पर व्यवस्थापक/स्वामी है, तो
  मौजूदा skill को उस स्वामी के पास स्थानांतरित किया जा सकता है। इस स्पष्ट सहमति के बिना स्वामी परिवर्तन
...
- अनुमत फ़ॉर्म फ़ील्ड `payload`, दोहराए गए `files` ब्लॉब, या एक `clawpack`
  टारबॉल संदर्भ हैं। `clawpack`, `.tgz` ब्लॉब या
  upload-url प्रवाह द्वारा लौटाई गई स्टोरेज आईडी हो सकता है। चरणबद्ध storage-id प्रकाशनों में उस अपलोड URL के साथ लौटाया गया
...
- JSON बॉडी और कॉलर द्वारा प्रदान किया गया `payload.files` / `payload.artifact`
  मेटाडेटा अस्वीकार किया जाता है।
...
- किसी की ओर से किए गए प्रकाशन में भी आधिकारिक-चैनल पात्रता को लक्षित स्वामी खाते के विरुद्ध सत्यापित किया जाता है।

### `DELETE /api/v1/skills/{slug}` / `POST /api/v1/skills/{slug}/undelete`

किसी skill को सॉफ़्ट-डिलीट / पुनर्स्थापित करें (स्वामी, मॉडरेटर या व्यवस्थापक)।
...
मौजूद होने पर `reason` को skill मॉडरेशन टिप्पणी के रूप में संग्रहीत किया जाता है और ऑडिट लॉग में कॉपी किया जाता है।
स्वामी द्वारा शुरू किए गए सॉफ़्ट डिलीट स्लग को 30 दिनों के लिए आरक्षित रखते हैं, जिसके बाद
दूसरा प्रकाशक स्लग पर दावा कर सकता है। यह समाप्ति लागू होने पर डिलीट प्रतिक्रिया में `slugReservedUntil` शामिल होता है।
मॉडरेटर/व्यवस्थापक द्वारा छिपाना और सुरक्षा निष्कासन इस प्रकार समाप्त नहीं होते।
...
केवल व्यवस्थापक। सुनिश्चित करता है कि किसी हैंडल के लिए संगठन प्रकाशक मौजूद हो। यदि हैंडल अब भी किसी
पुराने साझा उपयोगकर्ता/व्यक्तिगत प्रकाशक की ओर संकेत करता है, तो एंडपॉइंट पहले उसे संगठन प्रकाशक में माइग्रेट करता है।
नए बनाए गए संगठन के लिए `memberHandle` दें; कार्रवाई करने वाले व्यवस्थापक को सदस्य के रूप में नहीं जोड़ा जाता।
...
- जब हैंडल पहले से किसी प्रकाशक, उपयोगकर्ता या व्यक्तिगत प्रकाशक द्वारा उपयोग किया जाता है, तो `409` लौटाता है।

### `POST /api/v1/users/reserve`

केवल व्यवस्थापक। रिलीज़ प्रकाशित किए बिना सही स्वामी के लिए रूट स्लग और पैकेज नाम आरक्षित करता है।
...
केवल व्यवस्थापक। Convex Auth खाता पंक्तियों को संपादित किए बिना सत्यापित प्रतिस्थापन GitHub OAuth प्रिंसिपल के लिए
व्यक्तिगत प्रकाशक को पुनर्प्राप्त करता है। अनुरोध में दोनों अपरिवर्तनीय GitHub
प्रदाता खाता आईडी का उल्लेख होना चाहिए; परिवर्तनीय हैंडल केवल ऑपरेटर-सामना करने वाले सुरक्षा नियंत्रण के रूप में उपयोग होते हैं।

एंडपॉइंट का डिफ़ॉल्ट dry-run है। कर्मचारियों द्वारा दोनों
GitHub प्रिंसिपल के बीच निरंतरता का स्वतंत्र सत्यापन करने के बाद पुनर्प्राप्ति लागू करने के लिए `dryRun: false` और
`confirmIdentityVerified: true` आवश्यक हैं। गंतव्य उपयोगकर्ता के वर्तमान व्यक्तिगत
प्रकाशक के पास skills, पैकेज या GitHub skill स्रोत होने पर पुनर्प्राप्ति सुरक्षित रूप से विफल हो जाती है।
पुनर्प्राप्ति, पुनर्प्राप्त प्रकाशक के skills, skill स्लग उपनामों, पैकेजों, पैकेज निरीक्षक चेतावनियों और
व्युत्पन्न खोज डाइजेस्ट पंक्तियों के लिए पुराने `ownerUserId` फ़ील्ड भी माइग्रेट करती है, ताकि
प्रत्यक्ष-स्वामी पथ नए प्रकाशक प्राधिकार से मेल खाएँ। पुनर्प्राप्त हैंडल के लिए एक सक्रिय सुरक्षित-हैंडल
आरक्षण भी प्रतिस्थापन उपयोगकर्ता को पुनः सौंप दिया जाता है, ताकि बाद में
प्रोफ़ाइल समन्वयन पूर्व उपयोगकर्ता के प्रतिस्पर्धी प्राधिकार को पुनर्स्थापित न कर सके। प्रत्येक प्राथमिक तालिका
प्रति लागू लेन-देन 100 पंक्तियों तक सीमित है; बड़ी पुनर्प्राप्तियों में पहले पुनः प्रारंभ की जा सकने वाली स्वामी माइग्रेशन का उपयोग करना होगा।
GitHub skill स्रोत प्रकाशक-स्कोप वाले हैं और पुनर्लिखित होने के बजाय जाँचे गए के रूप में रिपोर्ट किए जाते हैं।
...
किसी उपयोगकर्ता को प्रतिबंधित करें और स्वामित्व वाले skills को स्थायी रूप से हटाएँ (केवल मॉडरेटर/व्यवस्थापक)।
...
किसी उपयोगकर्ता का प्रतिबंध हटाएँ और पात्र skills को पुनर्स्थापित करें (केवल व्यवस्थापक)।

बॉडी:

```json
{ "handle": "user_handle", "reason": "वैकल्पिक प्रतिबंध हटाने का कारण" }
```

या

```json
{ "userId": "users_...", "reason": "वैकल्पिक प्रतिबंध हटाने का कारण" }
```
...
प्रतिबंध हटाए या सामग्री पुनर्स्थापित किए बिना मौजूदा प्रतिबंध का संग्रहीत कारण बदलें
(केवल व्यवस्थापक)। जब तक `dryRun`, `false` न हो, डिफ़ॉल्ट dry-run है।
...
उपयोगकर्ता की भूमिका बदलें (केवल व्यवस्थापक)।
...
उपयोगकर्ताओं को सूचीबद्ध करें या खोजें (केवल व्यवस्थापक)।
...
स्टार जोड़ें/हटाएँ (हाइलाइट)। दोनों एंडपॉइंट इडेम्पोटेंट हैं।
...
`clawpackUploadTicket` के रूप में भेजना होगा।
