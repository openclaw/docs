...
read_when:
    ...
    - CLI ↔ registry अनुरोधों की डिबगिंग
summary: HTTP API संदर्भ (सार्वजनिक + CLI एंडपॉइंट + प्रमाणीकरण).
x-i18n:
    generated_at: "2026-06-30T22:15:09Z"
    model: gpt-5.5
...
तृतीय-पक्ष डायरेक्टरियाँ ClawHub skills को सूचीबद्ध या खोजने के लिए सार्वजनिक रीड endpoints का उपयोग कर सकती हैं। कृपया परिणाम cache करें, `429`/`Retry-After` का सम्मान करें, उपयोगकर्ताओं को canonical ClawHub listing (`https://clawhub.ai/<owner>/skills/<slug>`) पर वापस link करें, और तृतीय-पक्ष साइट के लिए ClawHub endorsement का संकेत देने से बचें। सार्वजनिक API surface के बाहर hidden, private, या moderation-blocked content को mirror करने का प्रयास न करें।

Web slug shortcuts registry families में resolve होते हैं, लेकिन API clients को route precedence को फिर से बनाने के बजाय read endpoints द्वारा लौटाए गए canonical URLs का उपयोग करना चाहिए।
...
- यदि token missing/invalid है, तो व्यवहार IP enforcement पर fall back करता है।
...
- trusted client IP headers, including `cf-connecting-ip`, केवल तब use करता है जब
  deployment trusted forwarded headers को explicitly enable करता है।
- ClawHub edge पर client IPs identify करने के लिए trusted forwarding headers का उपयोग करता है।
- यदि कोई trusted client IP available नहीं है, तो anonymous requests fallback buckets use करते हैं
...
Public v1 error responses plain text होते हैं जिनका `content-type: text/plain; charset=utf-8` होता है।
...
- `nonSuspiciousOnly` (optional): suspicious (`flagged.suspicious`) skills hide करने के लिए `true`
...
- ASCII text word और punctuation boundaries पर tokenized होता है। उदाहरण के लिए, `personal-map` में standalone `map` token होता है, जबकि `amap-jsapi-skill` में `amap`, `jsapi`, और `skill` होते हैं; इसलिए `map` search करने पर `personal-map` को `amap-jsapi-skill` की तुलना में stronger lexical match मिलता है।
...
- जिन terms को users सचमुच search करेंगे, उन्हें display name, summary, और tags में रखें। standalone slug token केवल तब use करें जब वह stable identity भी हो जिसे आप रखना चाहते हैं।
- किसी एक query का पीछा करने के लिए slug rename न करें, जब तक new slug बेहतर long-term canonical name न हो। Old slugs redirect aliases बन जाते हैं, लेकिन canonical URL, displayed slug, और future search digests new slug use करते हैं।
- Rename aliases old URLs और registry के माध्यम से resolve होने वाले installs के लिए resolution preserve करते हैं, लेकिन search ranking rename indexed होने के बाद canonical skill metadata पर based होती है। Existing stats skill के साथ रहते हैं।
- यदि कोई skill unexpectedly invisible है, तो ranking-related metadata बदलने से पहले logged in रहते हुए `clawhub inspect @owner/slug` से पहले moderation state check करें।
...
- जब `nonSuspiciousOnly=true` हो, cursor-based sorts किसी page पर `limit` items से कम return कर सकते हैं क्योंकि suspicious skills page retrieval के बाद filter किए जाते हैं।
- मौजूद होने पर pagination जारी रखने के लिए `nextCursor` use करें। short page अपने आप end-of-results नहीं दर्शाता।
...
version से linked, और skill report queue को feed करते हैं।
...
set करते समय इसे omit किया जा सकता है। उसी auditable workflow में skill hide करने के लिए triaged
...
- `security.hasScanResult` केवल तभी `true` होता है जब किसी scanner ने निश्चित verdict (`clean`, `suspicious`, या `malicious`) दिया हो।
- `moderation` नवीनतम संस्करण से निकाला गया वर्तमान skill-स्तरीय moderation snapshot है।
...
- Scan request payloads और डाउनलोड योग्य reports retention window के बाद scan-request store से expire हो जाते हैं।
- Published scans के लिए owner/publisher management access, या platform moderator/admin authority आवश्यक है।
- Published scans केवल तब write back करते हैं जब `update: true` हो और scan सफलतापूर्वक पूरा हो।
...
- Succeeded scan की आवश्यकता है; non-terminal scans `409` लौटाते हैं।
...
- `ok` केवल तभी `true` होता है जब selected version में generated Skill Card हो, moderation द्वारा malware-blocked न हो, और ClawScan verification clean हो।
- Skill identity, publisher identity, और selected version metadata top-level envelope fields (`slug`, `displayName`, `publisherHandle`, `version`, `resolvedFrom`, `tag`, `createdAt`) हैं, ताकि shell automation nested wrappers unpack किए बिना उन्हें read कर सके।
...
- `security.signals.dependencyRegistry` v1 response compatibility के लिए रखा गया है, लेकिन dependency registry existence scanner retired है और यह key हमेशा `null` होती है।
- Skill Card की अनुपस्थिति इस endpoint के `ok`, `decision`, या `reasons` को प्रभावित नहीं करती; जब clients को card content चाहिए, तो उन्हें installed `skill-card.md` locally read करना चाहिए।
- Single-skill Skill Card verification envelope चाहिए हो तो `/verify`, generated card markdown चाहिए हो तो `/card`, और detailed scanner data चाहिए हो तो `/scan` का उपयोग करें।
...
- डिफ़ॉल्ट नवीनतम संस्करण होता है।
...
- `category` (वैकल्पिक): plugin श्रेणी फ़िल्टर। केवल तब समर्थित जब
  ...
  नियंत्रित श्रेणियाँ और legacy v1 फ़िल्टर aliases `GET /api/v1/plugins` के अंतर्गत
  प्रलेखित हैं।
...
- कौशल प्रविष्टियाँ skill registry द्वारा समर्थित रहती हैं और अब भी केवल `POST /api/v1/skills` के ज़रिए प्रकाशित की जा सकती हैं।
...
- प्रमाणित callers सूची/खोज परिणामों में उन publishers के निजी packages देख सकते हैं जिनसे वे संबंधित हैं।
- `channel=private` केवल वे packages लौटाता है जिन्हें प्रमाणित caller पढ़ सकता है।

### `GET /api/v1/packages/search`

कौशल + plugin packages में एकीकृत कैटलॉग खोज।
...
- `isOfficial` (वैकल्पिक): `true` या `false`
- `category` (वैकल्पिक): plugin श्रेणी फ़िल्टर। केवल तब समर्थित जब
  अनुरोध plugin packages तक सीमित हो। नियंत्रित श्रेणियाँ और legacy v1
...
- अनाम callers केवल सार्वजनिक package channels देखते हैं।
- प्रमाणित callers उन publishers के निजी packages खोज सकते हैं जिनसे वे संबंधित हैं।
- `channel=private` केवल वे packages लौटाता है जिन्हें प्रमाणित caller पढ़ सकता है।
...
एकीकृत `/api/v1/packages` endpoint पर यह केवल plugin-only है; skill catalog के लिए
...
offline analysis के लिए नवीनतम सार्वजनिक कौशलों का bulk export।
...
- Hosted skills में नवीनतम stored version files शामिल होती हैं और वे
...
- Plugin खोज के लिए ब्राउज़र UI सॉर्ट नियंत्रण लोड किए गए प्रासंगिकता परिणामों को
  फिर से क्रमबद्ध करते हैं, जो वर्तमान `/skills` ब्राउज़ व्यवहार से मेल खाता है।
...
- निजी पैकेज `404` लौटाते हैं, जब तक कि कॉलर स्वामित्व वाले प्रकाशक को पढ़ न सके।
...
यह यह तय करने के लिए सार्वजनिक OpenClaw उपभोग सतह है कि कोई रिज़ॉल्व की गई
रिलीज़ इंस्टॉल की जा सकती है या नहीं।

प्रमाणीकरण:

- सार्वजनिक पढ़ने का एंडपॉइंट। किसी स्वामी, प्रकाशक, मॉडरेटर, या एडमिन टोकन की
  आवश्यकता नहीं है।
...
- `trust.pending` का अर्थ है कि एक या अधिक भरोसा इनपुट अभी भी पूरा होने की प्रतीक्षा कर रहे हैं।
- `trust.stale` का अर्थ है कि भरोसा सारांश पुराने इनपुट से गणना किया गया था और
  उच्च-विश्वास अनुमति निर्णय से पहले इसे रिफ़्रेश की आवश्यकता के रूप में माना जाना चाहिए।

नोट्स:

- यह एंडपॉइंट संस्करण-सटीक है। क्लाइंट को इसे उस पैकेज संस्करण को रिज़ॉल्व करने के बाद
  कॉल करना चाहिए जिसे वे इंस्टॉल करना चाहते हैं, केवल नवीनतम पैकेज मेटाडेटा पढ़ने के बाद नहीं।
- निजी पैकेज `404` लौटाते हैं, जब तक कि कॉलर स्वामित्व वाले प्रकाशक को पढ़ न सके।
- यह एंडपॉइंट स्वामी/मॉडरेटर मॉडरेशन एंडपॉइंट की तुलना में जानबूझकर संकरा है।
...
Readiness जाँचें कवर करती हैं:

- आधिकारिक channel स्थिति
...
आधिकारिक OpenClaw Plugin migration पंक्तियों की सूची के लिए मॉडरेटर एंडपॉइंट।
...
आधिकारिक Plugin migration पंक्ति बनाने या अपडेट करने के लिए एडमिन एंडपॉइंट।
...
- `packageName` npm-name सामान्यीकृत है; नियोजित migrations के लिए पैकेज अनुपस्थित हो सकता है।
...
- `open`: संदिग्ध, malicious, लंबित, quarantined, revoked, या रिपोर्ट की गई रिलीज़।
...
किसी संस्करण से लिंक होती हैं। वे मॉडरेशन कतार को फ़ीड करती हैं लेकिन अपने आप
...
- package owner, publisher member, moderator, या admin user के लिए API token आवश्यक है।
...
`confirmed` और `dismissed` के लिए `note` आवश्यक है; `status` को वापस `open` पर सेट करते समय इसे छोड़ा जा सकता है। उसी auditable workflow में release moderation लागू करने के लिए confirmed report के साथ `finalAction: "quarantine"` या `finalAction: "revoke"` पास करें।
...
- latest release पर डिफ़ॉल्ट करता है।
...
- latest release पर डिफ़ॉल्ट करता है।
- Skills `GET /api/v1/download` पर redirect होते हैं।
- Plugin/package archives `package/` root वाली zip files हैं ताकि पुराने OpenClaw clients काम करते रहें।
- यह route ZIP-only रहता है। यह ClawPack `.tgz` files stream नहीं करता।
- Responses resolver integrity checks के लिए `ETag`, `Digest`, `X-ClawHub-Artifact-Type`, और `X-ClawHub-Artifact-Sha256` headers शामिल करते हैं।
...
- केवल uploaded ClawPack npm-pack tarballs वाले versions सूचीबद्ध किए जाते हैं।
...
hosted skill version ZIP download करता है, या `clean` या `suspicious` scan और बिना hosted version वाले current GitHub-backed skill के लिए GitHub source handoff लौटाता है।
...
- यदि न `version` और न `tag` दिया गया है, latest version उपयोग किया जाता है।
- Soft-deleted versions `410` लौटाते हैं।
- GitHub-backed skill handoffs bytes को proxy या mirror नहीं करते। JSON response में `sourceRef: "public-github"`, `repo`, `commit`, `path`, `contentHash`, और `archiveUrl` शामिल होते हैं; scan/current state एक gate है और success payload metadata के रूप में शामिल नहीं होता।
...
- Optional payload field: `ownerHandle`। मौजूद होने पर, API उस publisher को server-side resolve करता है और actor के पास publisher access होना आवश्यक है।
- Optional payload field: `migrateOwner`। `ownerHandle` के साथ `true` होने पर, existing skill उस owner पर move हो सकता है यदि actor current और target publishers दोनों पर admin/owner है। इस opt-in के बिना, owner changes rejected होते हैं।
...
- Allowed form fields हैं `payload`, repeated `files` blobs, या एक `clawpack` tarball reference। `clawpack` `.tgz` blob या upload-url flow द्वारा लौटाई गई storage id हो सकता है। Staged storage-id publishes में उस upload URL के साथ लौटाया गया `clawpackUploadTicket` भी शामिल होना चाहिए।
- एक ही request में `files` या `clawpack` में से किसी एक का उपयोग करें, दोनों का नहीं।
- JSON bodies और caller-supplied `payload.files` / `payload.artifact` metadata rejected होते हैं।
- Direct multipart publish requests 18MB पर capped हैं। ClawPack tarballs upload-url flow का उपयोग 120MB tarball cap तक कर सकते हैं।
...
- केवल `openclaw` org publisher और current `openclaw` org members के personal publishers `official` channel पर publish कर सकते हैं।
...
Owner-initiated soft deletes slug को 30 दिनों के लिए reserve करते हैं, फिर slug किसी अन्य publisher द्वारा claim किया जा सकता है। जब यह expiry लागू होती है, delete response में `slugReservedUntil` शामिल होता है।
...
Admin-only। किसी handle के लिए org publisher मौजूद होना सुनिश्चित करता है। यदि handle अब भी किसी legacy shared user/personal publisher की ओर point करता है, तो endpoint पहले उसे org publisher में migrate करता है।
नए बनाए गए org के लिए, `memberHandle` दें; acting admin member के रूप में add नहीं किया जाता।
...
Authenticated self-serve org publisher creation। नया org publisher बनाता है और caller को owner के रूप में add करता है। यह endpoint existing user/personal handles को migrate नहीं करता और publisher को trusted/official mark नहीं करता।
...
Admin-only। release publish किए बिना rightful owner के लिए root slugs और package names reserve करता है। Package names बिना release rows वाले private placeholder packages बन जाते हैं, ताकि वही owner बाद में उस name में real code-plugin या bundle-plugin release publish कर सके।
...
Admin-only। Convex Auth account rows edit किए बिना verified replacement GitHub OAuth principal के लिए personal publisher recover करता है। Request में दोनों immutable GitHub provider account ids का नाम होना चाहिए; mutable handles केवल operator-facing guard के रूप में उपयोग किए जाते हैं।
...
`confirmIdentityVerified: true` आवश्यक है। जब गंतव्य उपयोगकर्ता के मौजूदा व्यक्तिगत
publisher में skills, packages, या GitHub skill sources हों, तो रिकवरी fail closed होती है।
रिकवरी, रिकवर किए गए publisher के skills, skill slug aliases, packages, package inspector warnings, और derived search digest rows के लिए legacy `ownerUserId` फ़ील्ड भी माइग्रेट करती है ताकि
direct-owner paths नए publisher authority से मेल खाएँ। रिकवर किए गए handle के लिए एक सक्रिय protected-handle
reservation भी replacement user को फिर से असाइन किया जाता है ताकि बाद की
profile synchronization पुराने उपयोगकर्ता की competing authority को पुनर्स्थापित न कर सके। हर primary table प्रति apply transaction
...
unban या content restore किए बिना मौजूदा ban के लिए stored reason बदलें
