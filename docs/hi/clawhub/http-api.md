...
x-i18n:
    generated_at: "2026-07-01T05:39:36Z"
    model: gpt-5.5
...
तृतीय-पक्ष डायरेक्टरियां ClawHub Skills को सूचीबद्ध या खोजने के लिए सार्वजनिक रीड एंडपॉइंट का उपयोग कर सकती हैं। कृपया परिणामों को कैश करें, `429`/`Retry-After` का पालन करें, उपयोगकर्ताओं को विहित ClawHub सूची (`https://clawhub.ai/<owner>/skills/<slug>`) पर वापस लिंक करें, और तृतीय-पक्ष साइट के लिए ClawHub समर्थन का संकेत देने से बचें। सार्वजनिक API सतह के बाहर छिपी, निजी, या मॉडरेशन-द्वारा-अवरुद्ध सामग्री को मिरर करने का प्रयास न करें।

वेब स्लग शॉर्टकट रजिस्ट्री परिवारों में रिज़ॉल्व होते हैं, लेकिन API क्लाइंट को रूट
प्राथमिकता को फिर से बनाने के बजाय रीड एंडपॉइंट द्वारा लौटाए गए विहित URL का उपयोग
करना चाहिए।
...
- यदि टोकन गायब/अमान्य है, तो व्यवहार IP प्रवर्तन पर वापस जाता है।
- प्रमाणीकृत राइट एंडपॉइंट को तब केवल `Unauthorized` नहीं लौटाना चाहिए जब
  सर्वर कारण जानता हो। गायब टोकन, अमान्य/रद्द टोकन, और
  हटाए/प्रतिबंधित/अक्षम खाते प्रत्येक को कार्रवाई योग्य पाठ मिलना चाहिए ताकि CLI
...
- डाउनलोड: 1200/मिनट प्रति IP, 6000/मिनट प्रति कुंजी (डाउनलोड एंडपॉइंट)
...
- समकालिक पुनः प्रयासों से बचने के लिए जिटर वाले बैकऑफ़ का उपयोग करें।
- यदि `Retry-After` गायब है, तो `RateLimit-Reset` पर फ़ॉलबैक करें (या `X-RateLimit-Reset` से गणना करें)।

IP स्रोत:

- विश्वसनीय क्लाइंट IP हेडर, जिनमें `cf-connecting-ip` शामिल है, का उपयोग केवल तब
  करता है जब डिप्लॉयमेंट स्पष्ट रूप से विश्वसनीय फ़ॉरवर्डेड हेडर सक्षम करता है।
- ClawHub एज पर क्लाइंट IP की पहचान करने के लिए विश्वसनीय फ़ॉरवर्डिंग हेडर का उपयोग करता है।
- यदि कोई विश्वसनीय क्लाइंट IP उपलब्ध नहीं है, तो अनाम अनुरोध केवल दर-सीमा प्रकार द्वारा
  स्कोप किए गए फ़ॉलबैक बकेट का उपयोग करते हैं। इन फ़ॉलबैक बकेट में
  कॉलर-प्रदत्त पाथ, स्लग, पैकेज नाम, संस्करण, क्वेरी स्ट्रिंग, या अन्य
  आर्टिफैक्ट पैरामीटर शामिल नहीं होते।

## त्रुटि प्रतिक्रियाएं

सार्वजनिक v1 त्रुटि प्रतिक्रियाएं `content-type: text/plain; charset=utf-8` के साथ सादा पाठ होती हैं।
इसमें सत्यापन विफलताएं (`400`), गायब सार्वजनिक संसाधन (`404`), प्रमाणीकरण और
...
- प्रासंगिकता लोकप्रियता से अधिक मजबूत होती है। सटीक स्लग या डिस्प्ले-नाम टोकन मिलान बहुत अधिक एंगेजमेंट वाले ढीले मिलान से ऊपर रैंक कर सकता है।
- ASCII पाठ को शब्द और विराम-चिह्न सीमाओं पर टोकनाइज़ किया जाता है। उदाहरण के लिए, `personal-map` में स्वतंत्र `map` टोकन होता है, जबकि `amap-jsapi-skill` में `amap`, `jsapi`, और `skill` होते हैं; इसलिए `map` की खोज `amap-jsapi-skill` की तुलना में `personal-map` को अधिक मजबूत शब्दगत मिलान देती है।
- लोकप्रियता लॉग-स्केल की जाती है और कैप की जाती है। जब क्वेरी पाठ कमजोर मिलान हो, तो उच्च-एंगेजमेंट Skills नीचे रैंक कर सकते हैं।
...
- उपयोगकर्ता जिन शब्दों को सचमुच खोजेंगे उन्हें डिस्प्ले नाम, सारांश, और टैग में रखें। स्वतंत्र स्लग टोकन केवल तब उपयोग करें जब वह एक स्थिर पहचान भी हो जिसे आप रखना चाहते हैं।
- केवल किसी एक क्वेरी का पीछा करने के लिए स्लग का नाम न बदलें, जब तक नया स्लग बेहतर दीर्घकालिक विहित नाम न हो। पुराने स्लग रीडायरेक्ट उपनाम बन जाते हैं, लेकिन विहित URL, प्रदर्शित स्लग, और भविष्य के खोज डाइजेस्ट नए स्लग का उपयोग करते हैं।
- नाम-बदलाव उपनाम पुराने URL और रजिस्ट्री से रिज़ॉल्व होने वाले इंस्टॉल के लिए रिज़ॉल्यूशन बनाए रखते हैं, लेकिन खोज रैंकिंग नाम-बदलाव इंडेक्स होने के बाद विहित Skill मेटाडेटा पर आधारित होती है। मौजूदा आँकड़े Skill के साथ रहते हैं।
...
- `createdAt` नए-Skill क्रॉल के लिए स्थिर है; मौजूदा Skills फिर से प्रकाशित होने पर `updated` बदलता है।
...
- मौजूद होने पर पेजिनेशन जारी रखने के लिए `nextCursor` का उपयोग करें। छोटा पेज अपने-आप परिणामों की समाप्ति का अर्थ नहीं है।
...
- मालिक के नाम-बदलाव/मर्ज फ़्लो द्वारा बनाए गए पुराने स्लग विहित Skill पर रिज़ॉल्व होते हैं।
...
- मालिक और मॉडरेटर छिपे हुए Skills के मॉडरेशन विवरण तक पहुंच सकते हैं।
- सार्वजनिक कॉलर केवल पहले से फ़्लैग किए गए दृश्य Skills के लिए `200` पाते हैं।
- सार्वजनिक कॉलर के लिए साक्ष्य संपादित किया जाता है और केवल मालिकों/मॉडरेटरों के लिए कच्चे स्निपेट शामिल करता है।

### `POST /api/v1/skills/{slug}/report`

मॉडरेटर समीक्षा के लिए किसी Skill की रिपोर्ट करें। रिपोर्ट Skill-स्तर की होती हैं, वैकल्पिक रूप से
किसी संस्करण से लिंक होती हैं, और Skill रिपोर्ट कतार को फ़ीड करती हैं।
...
Skill रिपोर्ट इनटेक के लिए मॉडरेटर/एडमिन एंडपॉइंट।
...
सेट करते समय इसे छोड़ा जा सकता है। उसी ऑडिट योग्य वर्कफ़्लो में Skill को छिपाने के लिए ट्रायेज की गई
...
- `moderation` नवीनतम संस्करण से प्राप्त मौजूदा skill-स्तर का मॉडरेशन स्नैपशॉट है।
...
- प्रकाशित स्कैन के लिए स्वामी/प्रकाशक प्रबंधन पहुंच, या प्लेटफॉर्म मॉडरेटर/प्रशासक अधिकार चाहिए।
...
- स्कैन जॉब असिंक्रोनस होते हैं। मैन्युअल स्कैन अनुरोधों को सामान्य प्रकाशन/बैकफिल कार्य से पहले प्राथमिकता दी जाती है, लेकिन पूर्णता फिर भी वर्कर उपलब्धता पर निर्भर करती है।
...
- कतारबद्ध रहते समय `queue.queuedAhead` और `queue.position` लौटाता है ताकि क्लाइंट दिखा सकें कि अनुरोध से आगे कितने प्राथमिकता-प्राप्त मैन्युअल स्कैन हैं। बहुत बड़ी कतारें सीमित की जाती हैं और `queuedAheadIsEstimate: true` के साथ रिपोर्ट की जाती हैं।
...
- सफल स्कैन की आवश्यकता होती है; गैर-टर्मिनल स्कैन `409` लौटाते हैं।
...
- skill या Plugin के लिए स्वामी/प्रकाशक प्रबंधन पहुंच, या प्लेटफॉर्म मॉडरेटर/प्रशासक अधिकार चाहिए।
- सटीक सबमिट किए गए संस्करण के लिए संग्रहीत स्कैन परिणाम लौटाता है, जिनमें अवरुद्ध या छिपे हुए संस्करण भी शामिल हैं।
...
प्रशासक-मात्र कैननिकल बैच रीस्कैन रूट। यह लेगेसी `POST /api/v1/skills/-/rescan-batch` जैसी ही पेलोड संरचना स्वीकार करता है।

### `POST /api/v1/skills/-/scan/batch/status`

प्रशासक-मात्र कैननिकल बैच स्थिति रूट। यह `{ "jobIds": ["..."] }` स्वीकार करता है और लेगेसी `POST /api/v1/skills/-/rescan-batch/status` जैसे ही समेकित काउंटर लौटाता है।
...
- Skill पहचान, प्रकाशक पहचान, और चयनित संस्करण मेटाडेटा शीर्ष-स्तरीय एनवेलप फ़ील्ड (`slug`, `displayName`, `publisherHandle`, `version`, `resolvedFrom`, `tag`, `createdAt`) हैं ताकि शेल ऑटोमेशन उन्हें नेस्टेड रैपर खोलें बिना पढ़ सके।
- `security` शीर्ष-स्तरीय ClawScan/security निर्णय है। ऑटोमेशन को `ok`, `decision`, `reasons`, और `security.status` के आधार पर काम करना चाहिए।
- `security.signals` में सहायक स्कैनर साक्ष्य होते हैं, जैसे `staticScan`, `virusTotal`, और `skillSpector`।
- `security.signals.dependencyRegistry` v1 प्रतिक्रिया संगतता के लिए रखा गया है, लेकिन निर्भरता रजिस्ट्री अस्तित्व स्कैनर सेवानिवृत्त हो चुका है और यह कुंजी हमेशा `null` होती है।
- `provenance` केवल तब `server-resolved-github-import` होता है जब ClawHub ने प्रकाशन या आयात के दौरान GitHub repo/ref/commit/path को हल करके संग्रहीत किया हो; अन्यथा यह `unavailable` होता है।

### `POST /api/v1/skills/-/security-verdicts`

सटीक skill संस्करणों के लिए मौजूदा संक्षिप्त सुरक्षा निर्णय लौटाता है। यह
संग्रह एंडपॉइंट उन क्लाइंट के लिए है जो पहले से जानते हैं कि उन्हें कौन से स्थापित
ClawHub skill संस्करण दिखाने हैं, जैसे OpenClaw Control UI।
...
- परिणाम प्रति आइटम होते हैं; एक अनुपस्थित skill या संस्करण पूरी प्रतिक्रिया को विफल नहीं करता।
- प्रतिक्रिया केवल सुरक्षा-संबंधी है। इसमें Skill Card डेटा, जनरेटेड कार्ड स्थिति, आर्टिफैक्ट फ़ाइल सूचियां, या विस्तृत स्कैनर पेलोड शामिल नहीं हैं।
- `security.signals` में केवल स्थिति-स्तर का सहायक साक्ष्य होता है; पूर्ण स्कैनर विवरणों के लिए `/scan` या ClawHub security-audit पेज का उपयोग करें।
- `security.signals.dependencyRegistry` v1 प्रतिक्रिया संगतता के लिए रखा गया है, लेकिन निर्भरता रजिस्ट्री अस्तित्व स्कैनर सेवानिवृत्त हो चुका है और यह कुंजी हमेशा `null` होती है।
- Skill Card की अनुपस्थिति इस एंडपॉइंट के `ok`, `decision`, या `reasons` को प्रभावित नहीं करती; कार्ड सामग्री की आवश्यकता होने पर क्लाइंट को स्थापित `skill-card.md` स्थानीय रूप से पढ़ना चाहिए।
- एकल-skill Skill Card सत्यापन एनवेलप चाहिए तो `/verify` का उपयोग करें, जनरेटेड कार्ड Markdown चाहिए तो `/card`, और विस्तृत स्कैनर डेटा चाहिए तो `/scan`।
...
- `category` (वैकल्पिक): plugin श्रेणी फ़िल्टर। केवल तब समर्थित जब
  ...
  `family=code-plugin`/`family=bundle-plugin` वाले package endpoints) तक scoped हो।
  नियंत्रित श्रेणियां और legacy v1 फ़िल्टर aliases `GET /api/v1/plugins` के अंतर्गत दस्तावेज़ित हैं।
...
- प्रमाणित callers list/search results में उन publishers के private packages देख सकते हैं जिनसे वे संबंधित हैं।
...
- `isOfficial` (वैकल्पिक): `true` या `false`
- `category` (वैकल्पिक): plugin श्रेणी फ़िल्टर। केवल तब समर्थित जब
  अनुरोध plugin packages तक scoped हो। नियंत्रित श्रेणियां और legacy v1
...
code-plugin और bundle-plugin packages में केवल Plugin वाला कैटलॉग browse।
...
एकीकृत `/api/v1/packages` endpoint पर यह केवल plugin के लिए है; skill catalog के लिए
...
- `clean` या `suspicious` scan वाले मौजूदा GitHub-backed skills में
...
- `_errors.json` तब शामिल होता है जब अलग-अलग skills या files
...
- `_errors.json` तब शामिल होता है जब अलग-अलग Plugins या फ़ाइलें निर्यात नहीं की जा सकीं।
...
- पैकेज स्वामी, org प्रकाशक स्वामी/admin, प्लेटफ़ॉर्म मॉडरेटर, या प्लेटफ़ॉर्म admin के लिए API टोकन आवश्यक है।
...
- `version.sha256hash` पुराने क्लाइंट्स के लिए अप्रचलित संगतता मेटाडेटा है। यह
  `/api/v1/packages/{name}/download` द्वारा लौटाए गए सटीक ZIP बाइट्स को हैश करता है।
  आधुनिक क्लाइंट्स को `version.artifact.sha256` का उपयोग करना चाहिए, जो
...
इंस्टॉल क्लाइंट्स के लिए सटीक पैकेज रिलीज़ सुरक्षा और ट्रस्ट सारांश लौटाता है। यह
रिज़ॉल्व की गई रिलीज़ इंस्टॉल की जा सकती है या नहीं, यह तय करने के लिए सार्वजनिक OpenClaw उपभोग सतह है।

प्रमाणीकरण:

- सार्वजनिक रीड एंडपॉइंट। कोई स्वामी, प्रकाशक, मॉडरेटर, या admin टोकन
  आवश्यक नहीं है।
...
- `trust.scanStatus` स्कैनर इनपुट और मैन्युअल रिलीज़ मॉडरेशन से निकाली गई प्रभावी ट्रस्ट स्थिति है।
- `trust.moderationState` nullable है। जब कोई मैन्युअल रिलीज़ मॉडरेशन मौजूद नहीं होता,
  यह `null` होता है।
- `trust.blockedFromDownload` इंस्टॉल ब्लॉक संकेत है। OpenClaw और अन्य
  इंस्टॉल क्लाइंट्स को यह मान `true` होने पर स्कैनर या मॉडरेशन फ़ील्ड से ब्लॉकिंग नियमों को
  फिर से निकालने के बजाय इंस्टॉलेशन ब्लॉक करना चाहिए।
...
- `trust.pending` का अर्थ है कि एक या अधिक ट्रस्ट इनपुट अभी भी पूर्णता की प्रतीक्षा कर रहे हैं।
- `trust.stale` का अर्थ है कि ट्रस्ट सारांश पुराने इनपुट से गणना किया गया था और
  उच्च-विश्वास अनुमति निर्णय से पहले इसे रीफ़्रेश की आवश्यकता वाला माना जाना चाहिए।

नोट्स:

- यह एंडपॉइंट संस्करण-सटीक है। क्लाइंट्स को इसे उस पैकेज संस्करण को रिज़ॉल्व करने के बाद कॉल करना चाहिए
  जिसे वे इंस्टॉल करना चाहते हैं, केवल नवीनतम पैकेज मेटाडेटा पढ़ने के बाद नहीं।
- निजी पैकेज `404` लौटाते हैं जब तक कि कॉलर स्वामी प्रकाशक को पढ़ न सके।
- यह एंडपॉइंट स्वामी/मॉडरेटर मॉडरेशन एंडपॉइंट्स से जानबूझकर अधिक संकीर्ण है।
  यह इंस्टॉल निर्णय और सार्वजनिक व्याख्या उजागर करता है, रिपोर्टर पहचान,
  रिपोर्ट बॉडी, निजी प्रमाण, या आंतरिक समीक्षा टाइमलाइन नहीं।
...
- ClawPack संस्करण `npm-pack` आर्टिफ़ैक्ट, npm integrity फ़ील्ड,
  `tarballUrl`, और पुराना ZIP संगतता URL लौटाते हैं।
- यह OpenClaw रिज़ॉल्वर सतह है; यह साझा URL से आर्काइव फ़ॉर्मैट का अनुमान लगाने से बचाता है।
...
- `packageName` npm-name सामान्यीकृत है; planned माइग्रेशन के लिए पैकेज अनुपस्थित हो सकता है।
- यह केवल माइग्रेशन तत्परता ट्रैक करता है। यह OpenClaw को mutate नहीं करता या
...
- `manual`: मैन्युअल मॉडरेशन override वाली कोई भी रिलीज़।
- `all`: मैन्युअल override, non-clean स्कैन स्थिति, या पैकेज रिपोर्ट वाली कोई भी रिलीज़।
...
किसी संस्करण से लिंक की जाती हैं। वे मॉडरेशन कतार को feed करती हैं लेकिन स्वयं डाउनलोड को auto-hide या
ब्लॉक नहीं करतीं; मॉडरेटर को आर्टिफ़ैक्ट approve, quarantine, या revoke करने के लिए रिलीज़ मॉडरेशन का उपयोग करना चाहिए।
...
पैकेज रिपोर्ट इनटेक के लिए मॉडरेटर/एडमिन अंतिम बिंदु।
...
पैकेज मॉडरेशन दृश्यता के लिए मालिक/मॉडरेटर अंतिम बिंदु।
...
पैकेज रिपोर्ट हल करने या फिर से खोलने के लिए मॉडरेटर/एडमिन अंतिम बिंदु।
...
पैकेज रिलीज़ समीक्षा के लिए मॉडरेटर/एडमिन अंतिम बिंदु।
...
- `quarantined`: फ़ॉलो-अप लंबित रहते हुए अवरुद्ध।
- `revoked`: किसी रिलीज़ को पहले विश्वसनीय माने जाने के बाद अवरुद्ध।

Quarantined और revoked रिलीज़ artifact डाउनलोड रूट से `403` लौटाती हैं।
...
- फ़ाइल आकार सीमा: 200KB।
- लंबित VirusTotal स्कैन रीड को अवरुद्ध नहीं करते; दुर्भावनापूर्ण रिलीज़ कहीं और रोकी जा सकती हैं।
- निजी पैकेज `404` लौटाते हैं, जब तक कि कॉलर मालिक प्रकाशक को पढ़ न सके।
...
- प्रतिक्रियाओं में रिज़ॉल्वर इंटीग्रिटी जांचों के लिए `ETag`, `Digest`,
...
- Scoped पैकेज packuments `/api/npm/@scope/name` और npm के encoded
...
होस्ट किए गए skill संस्करण ZIP को डाउनलोड करता है, या ऐसे वर्तमान GitHub-समर्थित
skill के लिए GitHub स्रोत हैंडऑफ़ लौटाता है जिसके पास `clean` या `suspicious`
स्कैन है और कोई होस्टेड संस्करण नहीं है।
...
- डाउनलोड आंकड़े प्रति UTC दिन अद्वितीय पहचानों के रूप में गिने जाते हैं (API टोकन मान्य होने पर `userId`, अन्यथा IP)।

## प्रमाणीकरण अंतिम बिंदु (Bearer token)

सभी अंतिम बिंदुओं के लिए आवश्यक है:
...
टोकन सत्यापित करता है और उपयोगकर्ता handle लौटाता है।
...
- वैकल्पिक payload फ़ील्ड: `ownerHandle`। मौजूद होने पर, API उस
  publisher को सर्वर-साइड रिज़ॉल्व करता है और actor के पास publisher access होना आवश्यक करता है।
- वैकल्पिक payload फ़ील्ड: `migrateOwner`। `ownerHandle` के साथ `true` होने पर,
  कोई मौजूदा skill उस owner पर जा सकता है यदि actor current और target publishers, दोनों
  पर admin/owner है। इस opt-in के बिना, owner बदलाव
...
- अनुमत form fields हैं `payload`, दोहराए गए `files` blobs, या एक `clawpack`
  tarball reference। `clawpack` एक `.tgz` blob या upload-url flow द्वारा लौटाई गई
  storage id हो सकती है। Staged storage-id publishes में उस upload URL के साथ लौटाया गया
...
- Direct multipart publish requests की सीमा 18MB है। ClawPack tarballs
  120MB tarball cap तक upload-url flow का उपयोग कर सकते हैं।
- वैकल्पिक payload फ़ील्ड: `ownerHandle`। मौजूद होने पर, केवल admins उस owner की ओर से publish कर सकते हैं।
...
- On-behalf publishes अब भी target owner account के विरुद्ध official-channel eligibility सत्यापित करते हैं।
...
मौजूद होने पर, `reason` skill moderation note के रूप में संग्रहीत किया जाता है और audit log में कॉपी किया जाता है।
...
केवल admin। सुनिश्चित करता है कि किसी handle के लिए org publisher मौजूद है। यदि handle अब भी
...
owner बाद में असली code-plugin या bundle-plugin release को उस name में publish कर सके।
...
एंडपॉइंट डिफ़ॉल्ट रूप से dry-run पर रहता है। रिकवरी लागू करने के लिए `dryRun: false` और
`confirmIdentityVerified: true` आवश्यक हैं, जब स्टाफ दोनों GitHub principals के बीच निरंतरता को स्वतंत्र रूप से सत्यापित कर ले।
जब गंतव्य उपयोगकर्ता के वर्तमान personal publisher के पास skills, packages, या GitHub skill sources हों, तो रिकवरी fail closed होती है।
रिकवरी recovered publisher के skills, skill slug aliases, packages, package inspector warnings, और derived search digest rows के लिए legacy `ownerUserId` fields को भी माइग्रेट करती है, ताकि
direct-owner paths नए publisher authority से सहमत रहें। recovered handle के लिए एक सक्रिय protected-handle
reservation को भी replacement user को फिर से सौंपा जाता है, ताकि बाद की
...
किसी मौजूदा ban को unban किए या content restore किए बिना उसके stored reason को बदलें
...
उपयोगकर्ताओं को list करें या search करें (केवल admin)।
...
यदि आप self-host करते हैं, तो इस file को serve करें (या `CLAWHUB_REGISTRY` स्पष्ट रूप से set करें; legacy `CLAWDHUB_REGISTRY`)।
