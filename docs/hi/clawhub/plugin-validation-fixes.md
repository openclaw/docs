...
x-i18n:
    generated_at: "2026-07-01T18:12:34Z"
    model: gpt-5.5
...
स्वचालित पैकेज स्कैन से मिली खोजें भी दिखा सकता है। यह पेज लेखक-सामने आने वाली खोजों को कवर करता है, यानी
वे खोजें जिन्हें Plugin लेखक अपने पैकेज मेटाडेटा, मैनिफेस्ट, SDK
इम्पोर्ट, या प्रकाशित आर्टिफैक्ट में ठीक कर सकता है।

यह आंतरिक Plugin Inspector कवरेज खोजों को कवर नहीं करता। यदि किसी पूर्ण रिपोर्ट में
...
| `sdk-load-session-store`                | [पूरे-सेशन-स्टोर ऐक्सेस को बदलें](/hi/clawhub/plugin-validation-fixes#sdk-load-session-store)                                   |
...
- OpenClaw Plugin API संस्करण या semver floor का उपयोग करें जिसके विरुद्ध आपने बिल्ड और परीक्षण किया है।
...
जिसके विरुद्ध पैकेज बिल्ड किया गया था।
...
- न्यूनतम होस्ट संस्करण को उस होस्ट संस्करण रेंज के साथ संरेखित करें जिसका पैकेज
...
- यदि `openclaw.plugin.json` में भी `version` है, तो उसे मिलाने के लिए अपडेट करें या
  जब पैकेज मेटाडेटा प्रामाणिक हो, तब पुराने मैनिफेस्ट संस्करण मेटाडेटा को हटाएँ।
...
- अमान्य पैकेज मेटाडेटा, टूटे हुए लाइफसाइकल स्क्रिप्ट, या ऐसी files entries ठीक करें
...
- `openclaw.plugin.json` में एक non-empty `name` field जोड़ें।
- `name` को मानव-पठनीय रखें और `id` को stable machine id के रूप में रखें।
- [Plugin manifest](/hi/plugins/manifest) देखें।
- `clawhub package validate <path-to-plugin>` फिर से चलाएँ।

### manifest-unknown-fields

plugin manifest में top-level fields हैं जिन्हें OpenClaw support नहीं करता।

- हर top-level field की तुलना
  [manifest field reference](/hi/plugins/manifest#top-level-field-reference) से करें।
- `openclaw.plugin.json` से custom fields हटाएँ।
- package या install metadata को manifest के बजाय supported `package.json#openclaw` fields
  में ले जाएँ।
- `clawhub package validate <path-to-plugin>` फिर से चलाएँ।
...
- `contracts` के अंतर्गत हर key की तुलना
...
plugin bundled plugins या internal compatibility के लिए reserved SDK path import करता है।
...
plugin अब भी deprecated whole-session-store helper
...
- पूरे session store object को load, mutate, और save करने से बचें।
- `loadSessionStore(...)` को केवल तब तक रखें जब तक आपकी घोषित compatibility range
  अब भी पुराने OpenClaw versions को support करती है जिन्हें इसकी आवश्यकता है।
...
plugin अब भी deprecated whole-session-store write helper जैसे
...
- session entry को replace या create करते समय `upsertSessionEntry(...)` का उपयोग करें।
- पूरे session store object को load, mutate, और save करने से बचें।
- whole-store write helpers को केवल तब तक रखें जब तक आपकी घोषित compatibility range
  अब भी पुराने OpenClaw versions को support करती है जिन्हें उनकी आवश्यकता है।
- [Runtime API](/hi/plugins/sdk-runtime#agent-session-state) और
  [Plugin SDK subpaths](/hi/plugins/sdk-subpaths) देखें।
- `clawhub package validate <path-to-plugin>` फिर से चलाएँ।
...
plugin अब भी deprecated session file-path helpers जैसे
...
- जब code transcript operation तैयार कर रहा हो, तब transcript identity या target helpers का उपयोग करें।
...
plugin अब भी deprecated transcript file target helper
`resolveSessionTranscriptLegacyFileTarget` का उपयोग करता है।

- जब code को केवल public session identity की आवश्यकता हो, तो `resolveSessionTranscriptIdentity(...)` का उपयोग करें।
- जब code को structured transcript operation target की आवश्यकता हो, तो `resolveSessionTranscriptTarget(...)` का उपयोग करें।
- legacy transcript file targets को सीधे पढ़ने या construct करने से बचें।
- legacy helper को केवल तब तक रखें जब तक आपकी घोषित compatibility range अब भी
  पुराने OpenClaw versions को support करती है जिन्हें इसकी आवश्यकता है।
- [Runtime API](/hi/plugins/sdk-runtime#agent-session-state) और
  [Plugin SDK subpaths](/hi/plugins/sdk-subpaths) देखें।
- `clawhub package validate <path-to-plugin>` फिर से चलाएँ।
...
plugin अब भी deprecated low-level transcript helpers जैसे
...
plugin अब भी legacy `before_agent_start` hook का उपयोग करता है।
...
- `before_agent_start` को केवल तब तक रखें जब तक आपकी घोषित compatibility range अब भी
  पुराने OpenClaw versions को support करती है जिन्हें इसकी आवश्यकता है।
...
manifest अब भी legacy `providerAuthEnvVars` provider auth metadata का उपयोग करता है।
...
- `providerAuthEnvVars` को केवल compatibility metadata के रूप में रखें जब तक आपकी supported
  OpenClaw range को अब भी इसकी आवश्यकता है।
...
manifest legacy या पुराने channel env-var metadata का उपयोग करता है, बिना उस current
...
- env-driven channel setup को current setup, channel config, या
  आपके plugin shape द्वारा उपयोग किए जाने वाले package channel metadata में mirror करें।
- `channelEnvVars` को केवल compatibility metadata के रूप में रखें जब तक पुराने supported
  OpenClaw versions को अब भी इसकी आवश्यकता है।
...
package `openclaw.security.json` को ऐसे schema reference के साथ ship करता है जिसे ClawHub
available के रूप में recognize नहीं करता।

- यदि schema URL केवल advisory है, तो उसे हटाएँ।
- documented versioned schema का उपयोग केवल OpenClaw द्वारा publish किए जाने के बाद करें।
- `clawhub package validate <path-to-plugin>` फिर से चलाएँ।

### unrecognized-security-manifest

package एक unsupported security manifest file ship करता है।

- `openclaw.security.json` को तब तक हटाएँ जब तक OpenClaw versioned security
  manifest schema और ClawHub behavior document नहीं करता।
- security-sensitive behavior को अपने public package docs या
  README में documented रखें, जब तक manifest contract मौजूद नहीं है।
- `clawhub package validate <path-to-plugin>` फिर से चलाएँ।
