...
x-i18n:
    generated_at: "2026-07-01T20:20:32Z"
    model: gpt-5.5
...
ClawHub प्रकाशन से पहले Plugin पैकेजों को सत्यापित करता है और स्वचालित पैकेज स्कैन से मिले निष्कर्ष भी दिखा सकता है। यह पृष्ठ लेखक-सामने वाले निष्कर्षों को कवर करता है, यानी वे निष्कर्ष जिन्हें Plugin लेखक अपने पैकेज मेटाडेटा, मैनिफेस्ट, SDK imports, या प्रकाशित artifact में ठीक कर सकता है।

यह आंतरिक Plugin Inspector कवरेज निष्कर्षों को कवर नहीं करता। यदि किसी पूर्ण रिपोर्ट में लेखक के लिए सुधार मार्गदर्शन के बिना scanner रखरखाव कोड हों, तो वे Plugin लेखकों के बजाय OpenClaw maintainers के लिए हैं।
...
| `package-manifest-version-drift`        | [पैकेज और मैनिफेस्ट version संरेखित करें](/hi/clawhub/plugin-validation-fixes#package-manifest-version-drift)                          |
...
| `package-npm-pack-entrypoint-missing`   | [npm pack output में एंट्रीपॉइंट शामिल करें](/hi/clawhub/plugin-validation-fixes#package-npm-pack-entrypoint-missing)                  |
...
| `channel-env-vars`                      | [channel env vars को वर्तमान मेटाडेटा में mirror करें](/hi/clawhub/plugin-validation-fixes#channel-env-vars)                                |
| `security-manifest-schema-unavailable`  | [अनुपलब्ध security मैनिफेस्ट schema references हटाएं](/hi/clawhub/plugin-validation-fixes#security-manifest-schema-unavailable) |
| `unrecognized-security-manifest`        | [असमर्थित security मैनिफेस्ट फ़ाइलें हटाएं](/hi/clawhub/plugin-validation-fixes#unrecognized-security-manifest)                   |

## पैकेज मेटाडेटा

### package-json-missing

पैकेज root में `package.json` शामिल नहीं है, इसलिए ClawHub npm पैकेज, version, एंट्रीपॉइंट, या OpenClaw मेटाडेटा की पहचान नहीं कर सकता।

- `name`, `version`, और `type` के साथ `package.json` जोड़ें।
- जब पैकेज OpenClaw Plugin ship करता है, तो `openclaw` ब्लॉक जोड़ें।
...
- जब पैकेज ClawHub के माध्यम से प्रकाशित या install किया जाएगा, तो compatibility और install मेटाडेटा जोड़ें।
- [discovery को प्रभावित करने वाले package.json फ़ील्ड](/hi/plugins/manifest#packagejson-fields-that-affect-discovery) देखें।
- `clawhub package validate <path-to-plugin>` फिर से चलाएं।

### package-openclaw-entry-missing

पैकेज मेटाडेटा मौजूद है, लेकिन यह OpenClaw runtime एंट्रीपॉइंट घोषित नहीं करता।
...
- सभी एंट्रीपॉइंट paths को पैकेज directory के अंदर रखें।
- [Plugin entry points](/hi/plugins/sdk-entrypoints) और [discovery को प्रभावित करने वाले package.json फ़ील्ड](/hi/plugins/manifest#packagejson-fields-that-affect-discovery) देखें।
- `clawhub package validate <path-to-plugin>` फिर से चलाएं।

### package-entrypoint-missing

पैकेज OpenClaw एंट्रीपॉइंट घोषित करता है, लेकिन referenced फ़ाइल सत्यापित किए जा रहे पैकेज से गायब है।

- `openclaw.extensions`, `openclaw.runtimeExtensions`, `openclaw.setupEntry`, और `openclaw.runtimeSetupEntry` में हर path जांचें।
- यदि एंट्रीपॉइंट `dist` में generate होता है, तो पैकेज build करें।
- यदि एंट्रीपॉइंट move हो गया है, तो मेटाडेटा update करें।
...
पैकेज अपने द्वारा समर्थित OpenClaw Plugin API range घोषित नहीं करता।
...
पैकेज minimum host version उस OpenClaw version मेटाडेटा से मेल नहीं खाता जिसके विरुद्ध पैकेज build किया गया था।

- `openclaw.install.minHostVersion` जांचें।
- पैकेज में कोई भी OpenClaw build मेटाडेटा जांचें, जैसे release के दौरान उपयोग किया गया OpenClaw version।
...
पैकेज version और Plugin मैनिफेस्ट version में असहमति है।

- पैकेज release version के रूप में `package.json#version` को प्राथमिकता दें।
- यदि `openclaw.plugin.json` में भी `version` है, तो इसे match करने के लिए update करें या जब पैकेज मेटाडेटा authoritative हो तो stale मैनिफेस्ट version मेटाडेटा हटाएं।
...
- native Plugin मेटाडेटा को `openclaw.plugin.json` में रखें।
- पैकेज एंट्रीपॉइंट, compatibility, install, setup, और catalog मेटाडेटा को समर्थित `package.json#openclaw` फ़ील्ड में रखें।
- [discovery को प्रभावित करने वाले package.json फ़ील्ड](/hi/plugins/manifest#packagejson-fields-that-affect-discovery) देखें।
- `clawhub package validate <path-to-plugin>` फिर से चलाएं।
...
- अमान्य पैकेज मेटाडेटा, टूटे lifecycle scripts, या packing को fail कराने वाली files entries ठीक करें।
- यदि यह पैकेज public publishing के लिए intended है, तो `private: true` हटाएं।
- `clawhub package validate <path-to-plugin>` फिर से चलाएं।

### package-npm-pack-entrypoint-missing

पैकेज pack किया जा सकता है, लेकिन packed artifact में `package.json#openclaw` में घोषित एंट्रीपॉइंट फ़ाइलें शामिल नहीं हैं।

- `npm pack --dry-run` चलाएं और शामिल होने वाली फ़ाइलों का निरीक्षण करें।
- packing से पहले generated एंट्रीपॉइंट build करें।
- `files`, `.npmignore`, या build output update करें ताकि घोषित एंट्रीपॉइंट शामिल हों।
- [Plugin entry points](/hi/plugins/sdk-entrypoints) देखें।
- `clawhub package validate <path-to-plugin>` फिर से चलाएं।

### package-npm-pack-metadata-missing

packed artifact में वह OpenClaw मेटाडेटा गायब है जो आपके source पैकेज में मौजूद है।

- `npm pack --dry-run` चलाएं और शामिल मेटाडेटा फ़ाइलों का निरीक्षण करें।
- सुनिश्चित करें कि `package.json` में packed artifact में `openclaw` ब्लॉक शामिल है।
...
Plugin मैनिफेस्ट में शीर्ष-स्तरीय फ़ील्ड हैं जिन्हें OpenClaw समर्थन नहीं देता।
...
Plugin ऐसा SDK path आयात करता है जो bundled plugins या आंतरिक
...
Plugin अब भी अप्रचलित whole-session-store helper
...
- `loadSessionStore(...)` को केवल तब तक रखें जब तक आपकी घोषित संगतता सीमा
  अब भी ऐसे पुराने OpenClaw संस्करणों का समर्थन करती है जिन्हें इसकी आवश्यकता है।
...
Plugin अब भी अप्रचलित whole-session-store write helper, जैसे
...
- whole-store write helpers को केवल तब तक रखें जब तक आपकी घोषित संगतता सीमा
  अब भी ऐसे पुराने OpenClaw संस्करणों का समर्थन करती है जिन्हें उनकी आवश्यकता है।
- [रनटाइम API](/hi/plugins/sdk-runtime#agent-session-state) और
  [Plugin SDK subpaths](/hi/plugins/sdk-subpaths) देखें।
- `clawhub package validate <path-to-plugin>` फिर से चलाएँ।
...
Plugin अब भी अप्रचलित session file-path helpers, जैसे
`resolveSessionFilePath` या `resolveAndPersistSessionFile`, का उपयोग करता है।

- agent और session identity के आधार पर session metadata पढ़ने के लिए
  `getSessionEntry(...)` का उपयोग करें।
- session metadata को स्थायी करने के लिए `patchSessionEntry(...)` या
  `upsertSessionEntry(...)` का उपयोग करें।
- जब कोड transcript operation तैयार कर रहा हो, तो transcript identity या target helpers
  का उपयोग करें।
- legacy transcript file paths को स्थायी न करें या उन पर निर्भर न रहें।
- [रनटाइम API](/hi/plugins/sdk-runtime#agent-session-state) और
  [Plugin SDK subpaths](/hi/plugins/sdk-subpaths) देखें।
- `clawhub package validate <path-to-plugin>` फिर से चलाएँ।
...
Plugin अब भी अप्रचलित transcript file target helper
`resolveSessionTranscriptLegacyFileTarget` का उपयोग करता है।

- जब कोड को केवल सार्वजनिक session identity की आवश्यकता हो, तो
  `resolveSessionTranscriptIdentity(...)` का उपयोग करें।
- जब कोड को संरचित transcript operation target की आवश्यकता हो, तो
...
- legacy helper को केवल तब तक रखें जब तक आपकी घोषित संगतता सीमा अब भी
  ऐसे पुराने OpenClaw संस्करणों का समर्थन करती है जिन्हें इसकी आवश्यकता है।
- [रनटाइम API](/hi/plugins/sdk-runtime#agent-session-state) और
  [Plugin SDK subpaths](/hi/plugins/sdk-subpaths) देखें।
- `clawhub package validate <path-to-plugin>` फिर से चलाएँ।
...
Plugin अब भी अप्रचलित low-level transcript helpers, जैसे
...
मैनिफेस्ट अब भी legacy `providerAuthEnvVars` provider auth metadata का उपयोग करता है।
...
मैनिफेस्ट current setup या config metadata के बिना legacy या पुराने channel env-var metadata का उपयोग करता है
...
- OpenClaw द्वारा प्रकाशित किए जाने के बाद ही दस्तावेज़ित versioned schema का उपयोग करें।
- `clawhub package validate <path-to-plugin>` फिर से चलाएँ।

### unrecognized-security-manifest

पैकेज असमर्थित security manifest file शिप करता है।

- `openclaw.security.json` को तब तक हटाएँ जब तक OpenClaw versioned security
  manifest schema और ClawHub behavior दस्तावेज़ित नहीं करता।
- manifest contract मौजूद होने तक security-sensitive behavior को अपने सार्वजनिक पैकेज docs या
