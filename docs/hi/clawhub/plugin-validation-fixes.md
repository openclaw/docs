...
x-i18n:
    generated_at: "2026-06-30T14:00:09Z"
    model: gpt-5.5
...
यह आंतरिक Plugin Inspector coverage निष्कर्षों को कवर नहीं करता। यदि किसी पूर्ण report में लेखक remediation guidance के बिना scanner maintenance codes हों, तो वे Plugin लेखकों के बजाय OpenClaw maintainers के लिए हैं।
...
| `package-openclaw-unsupported-metadata` | [असमर्थित OpenClaw पैकेज metadata हटाएँ](/hi/clawhub/plugin-validation-fixes#package-openclaw-unsupported-metadata)          |
| `package-npm-pack-unavailable`          | [npm artifact को pack करने योग्य बनाएँ](/hi/clawhub/plugin-validation-fixes#package-npm-pack-unavailable)                                 |
...
- जब पैकेज OpenClaw Plugin भेजता हो, तो एक `openclaw` block जोड़ें।
...
पैकेज में `package.json` है, लेकिन वह OpenClaw पैकेज metadata घोषित नहीं करता।

- `package.json#openclaw` जोड़ें।
- `openclaw.extensions` या `openclaw.runtimeExtensions` जैसे entrypoint metadata शामिल करें।
- जब पैकेज ClawHub के माध्यम से प्रकाशित या स्थापित किया जाएगा, तब compatibility और install metadata जोड़ें।
- [package.json fields that affect discovery](/hi/plugins/manifest#packagejson-fields-that-affect-discovery) देखें।
- `clawhub package validate <path-to-plugin>` फिर से चलाएँ।

### package-openclaw-entry-missing

पैकेज metadata मौजूद है, लेकिन वह OpenClaw runtime entrypoint घोषित नहीं करता।
...
- सभी entrypoint paths को पैकेज directory के भीतर रखें।
...
पैकेज एक OpenClaw entrypoint घोषित करता है, लेकिन संदर्भित file सत्यापित किए जा रहे पैकेज से गायब है।
...
- `openclaw.install` को supported install source से भरें, जैसे `clawhubSpec`, `npmSpec`, या `localPath`।
...
- वह OpenClaw Plugin API version या semver floor उपयोग करें जिसके against आपने build और test किया है।
...
पैकेज का न्यूनतम host version उस OpenClaw version metadata से मेल नहीं खाता जिसके against पैकेज build किया गया था।
...
- न्यूनतम host version को उस host version range से संरेखित करें जिसे पैकेज वास्तव में support करता है।
- [package.json fields that affect discovery](/hi/plugins/manifest#packagejson-fields-that-affect-discovery) देखें।
- `clawhub package validate <path-to-plugin>` फिर से चलाएँ।
...
- प्रकाशित metadata बदलने के बाद नया package version publish करें।
- [Plugin manifest](/hi/plugins/manifest) देखें।
- `clawhub package validate <path-to-plugin>` फिर से चलाएँ।

### package-openclaw-unsupported-metadata

`package.json#openclaw` block में ऐसे fields हैं जो OpenClaw package metadata के रूप में supported नहीं हैं।
...
पैकेज को उस artifact में pack नहीं किया जा सकता जिसकी ClawHub जाँच करेगा या publish करेगा।

- पैकेज root से `npm pack --dry-run` चलाएँ।
- invalid package metadata, broken lifecycle scripts, या packing fail कराने वाली files entries ठीक करें।
- यदि यह पैकेज public publishing के लिए intended है, तो `private: true` हटाएँ।
- `clawhub package validate <path-to-plugin>` फिर से चलाएँ।
...
- `files`, `.npmignore`, या build output update करें ताकि declared entrypoints शामिल हों।
- [Plugin entry points](/hi/plugins/sdk-entrypoints) देखें।
- `clawhub package validate <path-to-plugin>` फिर से चलाएँ।
...
- जब पैकेज native OpenClaw Plugin हो, तो सुनिश्चित करें कि `openclaw.plugin.json` शामिल हो।
...
- पैकेज या इंस्टॉल मेटाडेटा को मैनिफेस्ट के बजाय समर्थित `package.json#openclaw` फ़ील्ड में ले जाएँ।
- `clawhub package validate <path-to-plugin>` फिर से चलाएँ।
...
- रनटाइम व्यवहार को Plugin पंजीकरण कोड में ले जाएँ, और `contracts` को स्थिर क्षमता स्वामित्व मेटाडेटा तक सीमित रखें।
- `clawhub package validate <path-to-plugin>` फिर से चलाएँ।
...
Plugin ऐसा SDK पथ आयात करता है जो बंडल किए गए plugins या आंतरिक
...
- सेशन मेटाडेटा कायम रखने के लिए `patchSessionEntry(...)` या `upsertSessionEntry(...)` का उपयोग करें।
- जब कोड transcript ऑपरेशन तैयार कर रहा हो, तब transcript पहचान या लक्ष्य हेल्पर का उपयोग करें।
- पुराने transcript फ़ाइल पथों को कायम न रखें और उन पर निर्भर न रहें।
- [Runtime API](/hi/plugins/sdk-runtime#agent-session-state) और
  [Plugin SDK सबपाथ](/hi/plugins/sdk-subpaths) देखें।
- `clawhub package validate <path-to-plugin>` फिर से चलाएँ।
...
Plugin अभी भी अप्रचलित transcript फ़ाइल लक्ष्य हेल्पर
...
- जब कोड को संरचित transcript ऑपरेशन लक्ष्य चाहिए, तब `resolveSessionTranscriptTarget(...)` का उपयोग करें।
...
अप्रचलित निम्न-स्तरीय transcript हेल्पर का उपयोग करता है।
...
Plugin अभी भी पुराने `before_agent_start` hook का उपयोग करता है।

- मॉडल या प्रदाता override कार्य को `before_model_resolve` में ले जाएँ।
...
- प्रदाता env-var मेटाडेटा को `setup.providers[].envVars` में mirror करें।
...
मैनिफेस्ट पुराने या पूर्व चैनल env-var मेटाडेटा का उपयोग करता है, जिसमें वर्तमान
...
- env-चालित चैनल setup को आपके Plugin आकार द्वारा उपयोग किए जाने वाले वर्तमान setup, चैनल config, या
  पैकेज चैनल मेटाडेटा में mirror करें।
...
पैकेज `openclaw.security.json` को ऐसी schema संदर्भ के साथ भेजता है जिसे ClawHub
उपलब्ध के रूप में नहीं पहचानता।

- यदि schema URL केवल सलाहकारी है, तो उसे हटाएँ।
- OpenClaw द्वारा प्रकाशित किए जाने के बाद ही प्रलेखित versioned schema का उपयोग करें।
- `clawhub package validate <path-to-plugin>` फिर से चलाएँ।
...
- जब तक OpenClaw किसी versioned सुरक्षा मैनिफेस्ट schema और ClawHub व्यवहार का दस्तावेज़ीकरण नहीं करता,
  `openclaw.security.json` हटाएँ।
- जब तक मैनिफेस्ट contract मौजूद नहीं है, सुरक्षा-संवेदनशील व्यवहार को अपने सार्वजनिक पैकेज docs या
