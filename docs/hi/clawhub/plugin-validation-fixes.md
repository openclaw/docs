---
read_when:
    - आपने clawhub package validate चलाया और Plugin से जुड़ी पाई गई समस्याओं को ठीक करना है
...
x-i18n:
    generated_at: "2026-07-01T08:01:54Z"
    model: gpt-5.5
...
ClawHub प्रकाशित करने से पहले Plugin पैकेजों को सत्यापित करता है और स्वचालित पैकेज स्कैन से मिली निष्कर्ष भी दिखा सकता है। यह पृष्ठ लेखक-संबंधी निष्कर्षों को कवर करता है, यानी वे निष्कर्ष जिन्हें Plugin लेखक अपने पैकेज मेटाडेटा, मैनिफेस्ट, SDK इंपोर्ट, या प्रकाशित आर्टिफैक्ट में ठीक कर सकता है।

यह आंतरिक Plugin Inspector कवरेज निष्कर्षों को कवर नहीं करता। यदि किसी पूर्ण रिपोर्ट में लेखक सुधार मार्गदर्शन के बिना स्कैनर रखरखाव कोड शामिल हैं, तो वे Plugin लेखकों के बजाय OpenClaw मेंटेनरों के लिए हैं।
...
| `package-min-host-version-drift`        | [न्यूनतम होस्ट संस्करण मिलाएँ](/hi/clawhub/plugin-validation-fixes#package-min-host-version-drift)                                   |
| `package-manifest-version-drift`        | [पैकेज और मैनिफेस्ट संस्करण मिलाएँ](/hi/clawhub/plugin-validation-fixes#package-manifest-version-drift)                          |
...
| `sdk-session-transcript-file-target`    | [लीगेसी ट्रांसक्रिप्ट फ़ाइल लक्ष्य बदलें](/hi/clawhub/plugin-validation-fixes#sdk-session-transcript-file-target)                   |
...
- सभी एंट्रीपॉइंट पथ पैकेज डायरेक्टरी के अंदर रखें।
...
ClawHub यह नहीं समझ सकता कि पैकेज कैसे इंस्टॉल या अपडेट किया जाना चाहिए।
...
- वह OpenClaw Plugin API संस्करण या semver न्यूनतम सीमा उपयोग करें जिसके विरुद्ध आपने बिल्ड और परीक्षण किया है।
...
- न्यूनतम होस्ट संस्करण को उस होस्ट संस्करण रेंज से मिलाएँ जिसका पैकेज वास्तव में समर्थन करता है।
- [डिस्कवरी को प्रभावित करने वाले package.json फ़ील्ड](/hi/plugins/manifest#packagejson-fields-that-affect-discovery) देखें।
- `clawhub package validate <path-to-plugin>` फिर से चलाएँ।

### package-manifest-version-drift

पैकेज संस्करण और Plugin मैनिफेस्ट संस्करण अलग-अलग हैं।

- पैकेज रिलीज़ संस्करण के रूप में `package.json#version` को प्राथमिकता दें।
- यदि `openclaw.plugin.json` में भी `version` है, तो उसे मिलाने के लिए अपडेट करें या जब पैकेज मेटाडेटा आधिकारिक हो, तो पुराना मैनिफेस्ट संस्करण मेटाडेटा हटाएँ।
...
- अमान्य पैकेज मेटाडेटा, टूटे lifecycle स्क्रिप्ट, या ऐसी files प्रविष्टियाँ ठीक करें जो पैकिंग को विफल बनाती हैं।
...
Plugin बहिष्कृत रूट SDK बैरल से आयात करता है:
`openclaw/plugin-sdk`.
...
Plugin ऐसा SDK पथ आयात करता है जो bundled plugins या आंतरिक
...
Plugin अभी भी बहिष्कृत पूर्ण-सेशन-स्टोर हेल्पर
...
बहिष्कृत पूर्ण-सेशन-स्टोर लेखन हेल्पर का उपयोग करता है।
...
बहिष्कृत सेशन फ़ाइल-पथ हेल्पर का उपयोग करता है।
...
- जब कोड किसी ट्रांसक्रिप्ट ऑपरेशन की तैयारी कर रहा हो, तब ट्रांसक्रिप्ट पहचान या लक्ष्य हेल्पर
  का उपयोग करें।
...
Plugin अभी भी बहिष्कृत ट्रांसक्रिप्ट फ़ाइल लक्ष्य हेल्पर
...
- लेगेसी हेल्पर केवल तब तक रखें जब तक आपकी घोषित संगतता सीमा अभी भी
...
बहिष्कृत निम्न-स्तरीय ट्रांसक्रिप्ट हेल्पर का उपयोग करता है।
...
- प्रॉम्प्ट या संदर्भ mutation कार्य को `before_prompt_build` में ले जाएँ।
...
मैनिफेस्ट लेगेसी या पुराने चैनल env-var मेटाडेटा का उपयोग करता है, जिसमें वह वर्तमान
...
- env-चालित चैनल setup को आपके Plugin आकार में उपयोग किए गए वर्तमान setup, चैनल config, या
...
- OpenClaw द्वारा प्रकाशित किए जाने के बाद ही दस्तावेज़ित संस्करणित schema का उपयोग करें।
- `clawhub package validate <path-to-plugin>` फिर से चलाएँ।
...
- जब तक OpenClaw संस्करणित सुरक्षा
  मैनिफेस्ट schema और ClawHub व्यवहार का दस्तावेज़ीकरण नहीं करता, तब तक `openclaw.security.json` हटाएँ।
