...
read_when:
    ...
    - ClawHub ने Plugin पैकेज के प्रकाशन को अस्वीकार किया या उसके बारे में चेतावनी दी
...
x-i18n:
    generated_at: "2026-07-02T00:55:15Z"
    model: gpt-5.5
...
स्वचालित पैकेज स्कैन से मिले निष्कर्ष भी दिखा सकता है। यह पृष्ठ लेखक-सामने वाले निष्कर्षों को कवर करता है, यानी
वे निष्कर्ष जिन्हें Plugin लेखक अपने पैकेज मेटाडेटा, मैनिफेस्ट, SDK
इंपोर्ट, या प्रकाशित आर्टिफैक्ट में ठीक कर सकता है।
...
कोई भी सुधार लागू करने के बाद, दोबारा चलाएं:

```bash
clawhub package validate <path-to-plugin>
```

## लेखक-सामने वाले निष्कर्ष
...
| `reserved-sdk-import`                   | [आरक्षित SDK इंपोर्ट हटाएं](/hi/clawhub/plugin-validation-fixes#reserved-sdk-import)                                             |
| `sdk-load-session-store`                | [पूरे सेशन-स्टोर की पहुंच बदलें](/hi/clawhub/plugin-validation-fixes#sdk-load-session-store)                                   |
| `sdk-session-store-write`               | [पूरे सेशन-स्टोर लेखन बदलें](/hi/clawhub/plugin-validation-fixes#sdk-session-store-write)                                  |
| `sdk-session-file-helper`               | [सेशन फ़ाइल-पाथ हेल्पर बदलें](/hi/clawhub/plugin-validation-fixes#sdk-session-file-helper)                                   |
| `sdk-session-transcript-file-target`    | [लेगेसी ट्रांस्क्रिप्ट फ़ाइल लक्ष्य बदलें](/hi/clawhub/plugin-validation-fixes#sdk-session-transcript-file-target)                   |
| `sdk-session-transcript-low-level`      | [निम्न-स्तरीय ट्रांस्क्रिप्ट हेल्पर बदलें](/hi/clawhub/plugin-validation-fixes#sdk-session-transcript-low-level)                       |
...
- न्यूनतम पैकेज उदाहरण के लिए [Plugin बनाना](/hi/plugins/building-plugins)
  ...
  का उपयोग करें।
- `clawhub package validate <path-to-plugin>` दोबारा चलाएं।
...
- जब पैकेज ClawHub के माध्यम से प्रकाशित या
  इंस्टॉल किया जाएगा, तो संगतता और इंस्टॉल मेटाडेटा जोड़ें।
...
- `openclaw.extensions`, `openclaw.runtimeExtensions`,
  `openclaw.setupEntry`, और `openclaw.runtimeSetupEntry` में प्रत्येक पाथ जांचें।
...
- वह OpenClaw Plugin API संस्करण या semver फ्लोर उपयोग करें जिसके विरुद्ध आपने बनाया और परीक्षण किया है।
...
- पैकेज में कोई भी OpenClaw बिल्ड मेटाडेटा जांचें, जैसे रिलीज़ के दौरान उपयोग किया गया
  OpenClaw संस्करण।
...
- यदि `openclaw.plugin.json` में भी `version` है, तो उसे मिलाने के लिए अपडेट करें या
  जब पैकेज मेटाडेटा आधिकारिक हो, तो बासी मैनिफेस्ट संस्करण मेटाडेटा हटाएं।
...
- अमान्य पैकेज मेटाडेटा, टूटे हुए lifecycle scripts, या ऐसी files entries ठीक करें जो
  पैकिंग विफल करती हैं।
...
पैक किए गए आर्टिफैक्ट में OpenClaw मेटाडेटा गायब है जो आपके स्रोत
पैकेज में मौजूद है।
...
- जब पैकेज नेटिव OpenClaw Plugin हो, तो सुनिश्चित करें कि `openclaw.plugin.json` शामिल हो।
...
Plugin मैनिफेस्ट में शीर्ष-स्तरीय फ़ील्ड हैं जिन्हें OpenClaw सपोर्ट नहीं करता।
...
- रनटाइम व्यवहार को Plugin पंजीकरण कोड में ले जाएँ, और `contracts` को
  स्थिर capability ownership मेटाडेटा तक सीमित रखें।
- `clawhub package validate <path-to-plugin>` फिर से चलाएँ।
...
- root-barrel imports को केंद्रित सार्वजनिक subpath imports से बदलें।
...
- संकीर्ण import खोजने के लिए [Import conventions](/hi/plugins/building-plugins#import-conventions) और
  [Plugin SDK subpaths](/hi/plugins/sdk-subpaths) का उपयोग करें।
- `clawhub package validate <path-to-plugin>` फिर से चलाएँ।

### reserved-sdk-import

Plugin bundled plugins या आंतरिक संगतता के लिए आरक्षित SDK path import करता है।

- आरक्षित OpenClaw आंतरिक SDK imports को प्रलेखित सार्वजनिक
...
- समर्थित import चुनने के लिए [Plugin SDK subpaths](/hi/plugins/sdk-subpaths) और
...
- पूरे session store object को लोड करने, mutate करने और save करने से बचें।
- `loadSessionStore(...)` को केवल तब तक रखें जब तक आपकी घोषित compatibility range
  पुराने OpenClaw versions को सपोर्ट करती है जिन्हें इसकी आवश्यकता है।
...
- whole-store write helpers को केवल तब तक रखें जब तक आपकी घोषित compatibility range
  पुराने OpenClaw versions को सपोर्ट करती है जिन्हें उनकी आवश्यकता है।
- [Runtime API](/hi/plugins/sdk-runtime#agent-session-state) और
  [Plugin SDK subpaths](/hi/plugins/sdk-subpaths) देखें।
- `clawhub package validate <path-to-plugin>` फिर से चलाएँ।
...
- legacy helper को केवल तब तक रखें जब तक आपकी घोषित compatibility range पुराने OpenClaw versions को सपोर्ट करती है जिन्हें इसकी आवश्यकता है।
- [Runtime API](/hi/plugins/sdk-runtime#agent-session-state) और
  [Plugin SDK subpaths](/hi/plugins/sdk-subpaths) देखें।
- `clawhub package validate <path-to-plugin>` फिर से चलाएँ।
...
- low-level transcript helpers को केवल तब तक रखें जब तक आपकी घोषित compatibility range पुराने OpenClaw versions को सपोर्ट करती है जिन्हें उनकी आवश्यकता है।
- [Runtime API](/hi/plugins/sdk-runtime#agent-session-state) और
  [Plugin SDK subpaths](/hi/plugins/sdk-subpaths) देखें।
- `clawhub package validate <path-to-plugin>` फिर से चलाएँ।
...
- `before_agent_start` को केवल तब तक रखें जब तक आपकी घोषित compatibility range पुराने OpenClaw versions को सपोर्ट करती है जिन्हें इसकी आवश्यकता है।
...
मैनिफेस्ट legacy या पुराने channel env-var metadata का उपयोग करता है, जिसमें मौजूदा
setup या config metadata नहीं है जिसकी ClawHub अपेक्षा करता है।

- channel env-var metadata को declarative रखें ताकि OpenClaw channel runtime लोड किए बिना setup status inspect कर सके।
- env-driven channel setup को आपकी Plugin shape द्वारा उपयोग किए जाने वाले मौजूदा setup, channel config, या
...
पैकेज `openclaw.security.json` को schema reference के साथ ship करता है जिसे ClawHub
उपलब्ध के रूप में नहीं पहचानता।

- यदि schema URL केवल सलाहात्मक है, तो उसे हटाएँ।
- OpenClaw द्वारा प्रकाशित किए जाने के बाद ही documented versioned schema का उपयोग करें।
- `clawhub package validate <path-to-plugin>` फिर से चलाएँ।

### unrecognized-security-manifest

पैकेज unsupported security manifest file ship करता है।

- जब तक OpenClaw versioned security manifest schema और ClawHub behavior को document नहीं करता, तब तक `openclaw.security.json` हटाएँ।
