...
x-i18n:
    generated_at: "2026-07-03T09:36:36Z"
    model: gpt-5.5
...
ClawHub प्रकाशित करने से पहले Plugin पैकेजों को सत्यापित करता है और स्वचालित पैकेज स्कैन से मिले निष्कर्ष भी दिखा सकता है। यह पृष्ठ लेखक-संबंधी निष्कर्षों को कवर करता है, यानी वे निष्कर्ष जिन्हें Plugin लेखक अपने पैकेज मेटाडेटा, मैनिफ़ेस्ट, SDK इम्पोर्ट, या प्रकाशित आर्टिफ़ैक्ट में ठीक कर सकता है।
...
| `manifest-name-missing`                 | [मैनिफ़ेस्ट डिस्प्ले नाम जोड़ें](/hi/clawhub/plugin-validation-fixes#manifest-name-missing)                                           |
...
| `sdk-session-transcript-file-target`    | [लेगेसी ट्रांसक्रिप्ट फ़ाइल लक्ष्यों को बदलें](/hi/clawhub/plugin-validation-fixes#sdk-session-transcript-file-target)                   |
...
- `openclaw.extensions`, `openclaw.runtimeExtensions`, `openclaw.setupEntry`, और `openclaw.runtimeSetupEntry` में प्रत्येक पथ जाँचें।
...
पैकेज संस्करण और Plugin मैनिफ़ेस्ट संस्करण में असहमति है।

- पैकेज रिलीज़ संस्करण के रूप में `package.json#version` को प्राथमिकता दें।
- यदि `openclaw.plugin.json` में भी `version` है, तो उसे मिलाने के लिए अपडेट करें या जब पैकेज मेटाडेटा अधिकृत हो तो पुराना मैनिफ़ेस्ट संस्करण मेटाडेटा हटा दें।
...
पैकेज को उस आर्टिफ़ैक्ट में पैक नहीं किया जा सकता जिसे ClawHub जाँचेगा या प्रकाशित करेगा।

- पैकेज रूट से `npm pack --dry-run` चलाएँ।
- अमान्य पैकेज मेटाडेटा, टूटे हुए लाइफ़सायकल स्क्रिप्ट, या ऐसे files एंट्री ठीक करें जिनसे पैकिंग विफल होती है।
...
- `npm pack --dry-run` चलाएँ और शामिल की जाने वाली फ़ाइलों की जाँच करें।
- पैकिंग से पहले जनरेटेड एंट्रीपॉइंट बनाएँ।
...
पैक किए गए आर्टिफ़ैक्ट में वह OpenClaw मेटाडेटा नहीं है जो आपके स्रोत पैकेज में मौजूद है।

- `npm pack --dry-run` चलाएँ और शामिल मेटाडेटा फ़ाइलों की जाँच करें।
- सुनिश्चित करें कि पैक किए गए आर्टिफ़ैक्ट में `package.json` में `openclaw` ब्लॉक शामिल है।
...
नेटिव Plugin मैनिफ़ेस्ट में कोई प्रदर्शन नाम शामिल नहीं है।
...
Plugin मैनिफ़ेस्ट में शीर्ष-स्तरीय फ़ील्ड हैं जिन्हें OpenClaw समर्थित नहीं करता।
...
Plugin पदावनत रूट SDK बैरल से आयात करता है:
`openclaw/plugin-sdk`.
...
- चैनल एंट्री हेल्परों के लिए `openclaw/plugin-sdk/channel-core` का उपयोग करें।
...
Plugin अभी भी पदावनत पूर्ण-सेशन-स्टोर हेल्पर
...
- `loadSessionStore(...)` को केवल तब तक रखें जब तक आपकी घोषित संगतता सीमा
  अभी भी ऐसे पुराने OpenClaw संस्करणों का समर्थन करती है जिन्हें इसकी आवश्यकता है।
...
Plugin अभी भी `saveSessionStore` या `updateSessionStore` जैसे पदावनत
पूर्ण-सेशन-स्टोर लेखन हेल्पर का उपयोग करता है।
...
- पूरे सेशन स्टोर ऑब्जेक्ट को लोड करने, बदलने और सहेजने से बचें।
- पूर्ण-स्टोर लेखन हेल्परों को केवल तब तक रखें जब तक आपकी घोषित संगतता सीमा
  अभी भी ऐसे पुराने OpenClaw संस्करणों का समर्थन करती है जिन्हें उनकी आवश्यकता है।
- [रनटाइम API](/hi/plugins/sdk-runtime#agent-session-state) और
  [Plugin SDK सबपाथ](/hi/plugins/sdk-subpaths) देखें।
- `clawhub package validate <path-to-plugin>` फिर से चलाएँ।
...
पदावनत सेशन फ़ाइल-पाथ हेल्परों का उपयोग करता है।

- एजेंट और सेशन पहचान के आधार पर सेशन मेटाडेटा पढ़ने के लिए `getSessionEntry(...)` का उपयोग करें।
- सेशन मेटाडेटा बनाए रखने के लिए `patchSessionEntry(...)` या `upsertSessionEntry(...)` का उपयोग करें।
- जब कोड ट्रांसक्रिप्ट ऑपरेशन तैयार कर रहा हो, तब ट्रांसक्रिप्ट पहचान या लक्ष्य हेल्परों का उपयोग करें।
- पुराने ट्रांसक्रिप्ट फ़ाइल पाथ पर निर्भर न रहें या उन्हें बनाए न रखें।
- [रनटाइम API](/hi/plugins/sdk-runtime#agent-session-state) और
  [Plugin SDK सबपाथ](/hi/plugins/sdk-subpaths) देखें।
- `clawhub package validate <path-to-plugin>` फिर से चलाएँ।
...
Plugin अभी भी पदावनत ट्रांसक्रिप्ट फ़ाइल लक्ष्य हेल्पर
...
- जब कोड को संरचित ट्रांसक्रिप्ट ऑपरेशन लक्ष्य चाहिए, तब `resolveSessionTranscriptTarget(...)` का उपयोग करें।
...
पदावनत लो-लेवल ट्रांसक्रिप्ट हेल्परों का उपयोग करता है।
...
- लो-लेवल ट्रांसक्रिप्ट हेल्परों को केवल तब तक रखें जब तक आपकी घोषित संगतता सीमा
  अभी भी ऐसे पुराने OpenClaw संस्करणों का समर्थन करती है जिन्हें उनकी आवश्यकता है।
- [रनटाइम API](/hi/plugins/sdk-runtime#agent-session-state) और
  [Plugin SDK सबपाथ](/hi/plugins/sdk-subpaths) देखें।
- `clawhub package validate <path-to-plugin>` फिर से चलाएँ।
...
मैनिफ़ेस्ट अभी भी पुराने `providerAuthEnvVars` प्रदाता auth मेटाडेटा का उपयोग करता है।
...
मैनिफ़ेस्ट पुराने या पूर्ववर्ती चैनल env-var मेटाडेटा का उपयोग करता है, जिसमें वह वर्तमान
...
- env-संचालित चैनल setup को अपने Plugin आकार द्वारा उपयोग किए जाने वाले वर्तमान setup, चैनल config, या
...
- दस्तावेजीकृत versioned स्कीमा का उपयोग केवल OpenClaw द्वारा उसे प्रकाशित करने के बाद करें।
- `clawhub package validate <path-to-plugin>` फिर से चलाएँ।
...
- जब तक OpenClaw versioned सुरक्षा मैनिफ़ेस्ट स्कीमा और ClawHub व्यवहार का दस्तावेज़ीकरण नहीं करता,
  `openclaw.security.json` हटाएँ।
- जब तक मैनिफ़ेस्ट कॉन्ट्रैक्ट मौजूद नहीं होता, सुरक्षा-संवेदनशील व्यवहार को अपने सार्वजनिक पैकेज दस्तावेज़ों या
