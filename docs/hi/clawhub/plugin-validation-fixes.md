...
x-i18n:
    generated_at: "2026-07-02T22:30:46Z"
    model: gpt-5.5
...
ClawHub प्रकाशित करने से पहले Plugin पैकेजों को सत्यापित करता है और स्वचालित पैकेज स्कैन से निष्कर्ष भी दिखा सकता है। यह पृष्ठ लेखक-सामने निष्कर्षों को कवर करता है, यानी वे निष्कर्ष जिन्हें Plugin लेखक अपने पैकेज मेटाडेटा, manifest, SDK imports, या प्रकाशित artifact में ठीक कर सकता है।

यह आंतरिक Plugin Inspector कवरेज निष्कर्षों को कवर नहीं करता। यदि किसी पूर्ण रिपोर्ट में लेखक remediation मार्गदर्शन के बिना scanner maintenance codes हों, तो वे Plugin लेखकों के बजाय OpenClaw maintainers के लिए हैं।
...
| `package-min-host-version-drift`        | [न्यूनतम host version को संरेखित करें](/hi/clawhub/plugin-validation-fixes#package-min-host-version-drift)                                   |
| `package-manifest-version-drift`        | [पैकेज और manifest versions को संरेखित करें](/hi/clawhub/plugin-validation-fixes#package-manifest-version-drift)                          |
...
| `sdk-session-file-helper`               | [session file-path helpers को बदलें](/hi/clawhub/plugin-validation-fixes#sdk-session-file-helper)                                   |
| `sdk-session-transcript-file-target`    | [legacy transcript file targets को बदलें](/hi/clawhub/plugin-validation-fixes#sdk-session-transcript-file-target)                   |
| `sdk-session-transcript-low-level`      | [low-level transcript helpers को बदलें](/hi/clawhub/plugin-validation-fixes#sdk-session-transcript-low-level)                       |
...
- जब पैकेज OpenClaw Plugin ship करता है, तो `openclaw` block जोड़ें।
- न्यूनतम पैकेज उदाहरण के लिए [Plugin बनाना](/hi/plugins/building-plugins) और पैकेज बनाम manifest विभाजन के लिए [Plugin manifest](/hi/plugins/manifest#manifest-versus-packagejson) का उपयोग करें।
...
- `openclaw.extensions` या `openclaw.runtimeExtensions` जैसे entrypoint मेटाडेटा शामिल करें।
- जब पैकेज ClawHub के माध्यम से प्रकाशित या installed होगा, तो compatibility और install metadata जोड़ें।
...
- यदि entrypoint `dist` में generated है, तो पैकेज build करें।
- यदि entrypoint स्थानांतरित हो गया है, तो मेटाडेटा update करें।
...
पैकेज न्यूनतम host version उस OpenClaw version metadata से मेल नहीं खाता जिसके विरुद्ध पैकेज build किया गया था।

- `openclaw.install.minHostVersion` जाँचें।
- पैकेज में किसी भी OpenClaw build metadata को जाँचें, जैसे release के दौरान उपयोग किया गया OpenClaw version।
- न्यूनतम host version को उस host version range के साथ संरेखित करें जिसे पैकेज वास्तव में support करता है।
- [discovery को प्रभावित करने वाले package.json fields](/hi/plugins/manifest#packagejson-fields-that-affect-discovery) देखें।
- `clawhub package validate <path-to-plugin>` फिर से चलाएँ।
...
- यदि `openclaw.plugin.json` में भी `version` है, तो उसे match करने के लिए update करें या जब पैकेज metadata authoritative हो, तो stale manifest version metadata हटाएँ।
...
`package.json#openclaw` block में ऐसे fields हैं जो समर्थित OpenClaw पैकेज मेटाडेटा नहीं हैं।
...
- invalid package metadata, broken lifecycle scripts, या files entries ठीक करें जो packing को fail कराते हैं।
...
- `npm pack --dry-run` चलाएँ और शामिल metadata files inspect करें।
...
- पैकेज या इंस्टॉल मेटाडेटा को मैनिफेस्ट के बजाय समर्थित
  `package.json#openclaw` फ़ील्ड में ले जाएं।
- `clawhub package validate <path-to-plugin>` फिर से चलाएं।

### manifest-unknown-contracts

मैनिफेस्ट `contracts` के अंदर असमर्थित कुंजियां घोषित करता है।

- `contracts` के अंतर्गत प्रत्येक कुंजी की तुलना
  [contracts संदर्भ](/hi/plugins/manifest#contracts-reference) से करें।
- असमर्थित कॉन्ट्रैक्ट कुंजियां हटाएं।
...
- आरक्षित OpenClaw आंतरिक SDK इंपोर्ट को दस्तावेजीकृत सार्वजनिक
...
Plugin अभी भी अप्रचलित पूरे-session-store हेल्पर
...
- पूरे सेशन स्टोर ऑब्जेक्ट को लोड करने, बदलने और सेव करने से बचें।
...
Plugin अभी भी अप्रचलित पूरे-session-store राइट हेल्पर जैसे
...
- पूरे सेशन स्टोर ऑब्जेक्ट को लोड करने, बदलने और सेव करने से बचें।
- पूरे-स्टोर राइट हेल्पर को केवल तब तक रखें जब तक आपकी घोषित संगतता रेंज
...
- जब कोड ट्रांसक्रिप्ट ऑपरेशन तैयार कर रहा हो, तो ट्रांसक्रिप्ट पहचान या
  लक्ष्य हेल्पर का उपयोग करें।
- लेगेसी ट्रांसक्रिप्ट फ़ाइल पाथ को स्थायी न करें या उन पर निर्भर न रहें।
- [रनटाइम API](/hi/plugins/sdk-runtime#agent-session-state) और
  [Plugin SDK सबपाथ](/hi/plugins/sdk-subpaths) देखें।
- `clawhub package validate <path-to-plugin>` फिर से चलाएं।
...
- लेगेसी ट्रांसक्रिप्ट फ़ाइल लक्ष्य को सीधे पढ़ने या बनाने से बचें।
- लेगेसी हेल्पर को केवल तब तक रखें जब तक आपकी घोषित संगतता रेंज अभी भी
...
- संरचित ट्रांसक्रिप्ट रनटाइम सतह को प्राथमिकता दें ताकि OpenClaw सही
  ट्रांजैक्शन सीमाएं और पहचान प्रबंधन लागू कर सके।
...
मैनिफेस्ट अभी भी लेगेसी `providerAuthEnvVars` प्रदाता auth मेटाडेटा का उपयोग करता है।
...
मैनिफेस्ट लेगेसी या पुराने चैनल env-var मेटाडेटा का उपयोग करता है, जिसमें वर्तमान
...
- चैनल env-var मेटाडेटा को घोषणात्मक रखें ताकि OpenClaw चैनल रनटाइम लोड किए बिना
  setup स्थिति निरीक्षण कर सके।
- env-driven चैनल setup को वर्तमान setup, चैनल config, या
  आपके Plugin आकार द्वारा उपयोग किए गए पैकेज चैनल मेटाडेटा में मिरर करें।
...
- यदि स्कीमा URL केवल सलाहकारी है, तो उसे हटाएं।
- दस्तावेजीकृत संस्करणित स्कीमा का उपयोग केवल तब करें जब OpenClaw एक प्रकाशित करे।
- `clawhub package validate <path-to-plugin>` फिर से चलाएं।
...
- `openclaw.security.json` को तब तक हटाएं जब तक OpenClaw संस्करणित सुरक्षा
