...
x-i18n:
    generated_at: "2026-07-04T03:46:50Z"
    model: gpt-5.5
...
स्वचालित पैकेज स्कैन से मिली findings भी दिखा सकता है। यह पेज author-facing findings को कवर करता है, यानी
वे findings जिन्हें Plugin लेखक अपने पैकेज metadata, manifest, SDK
imports, या प्रकाशित artifact में ठीक कर सकता है।

यह आंतरिक Plugin Inspector coverage findings को कवर नहीं करता। यदि पूरी रिपोर्ट में
लेखक remediation guidance के बिना scanner maintenance codes हों, तो वे
...
- न्यूनतम पैकेज example के लिए [Building plugins](/hi/plugins/building-plugins)
  और पैकेज बनाम manifest split के लिए [Plugin manifest](/hi/plugins/manifest#manifest-versus-packagejson)
  का उपयोग करें।
...
- `openclaw.extensions` या
  `openclaw.runtimeExtensions` जैसा entrypoint metadata शामिल करें।
- जब पैकेज ClawHub के माध्यम से प्रकाशित या
  install किया जाएगा, तो compatibility और install metadata जोड़ें।
...
- `openclaw.extensions`, `openclaw.runtimeExtensions`,
  `openclaw.setupEntry`, और `openclaw.runtimeSetupEntry` में प्रत्येक path जाँचें।
- यदि entrypoint `dist` में generate होता है, तो पैकेज build करें।
- यदि entrypoint moved हो गया है, तो metadata update करें।
- [Plugin entry points](/hi/plugins/sdk-entrypoints) देखें।
- `clawhub package validate <path-to-plugin>` फिर से चलाएँ।

### package-install-metadata-incomplete

ClawHub यह नहीं जान सकता कि पैकेज कैसे install या update किया जाना चाहिए।

- `openclaw.install` को supported install source से भरें, जैसे
...
पैकेज version और Plugin manifest version मेल नहीं खाते।

- पैकेज release version के रूप में `package.json#version` को प्राथमिकता दें।
- यदि `openclaw.plugin.json` में भी `version` है, तो उसे match करने के लिए update करें या
  जब package metadata authoritative हो, तो stale manifest version metadata हटाएँ।
- published metadata बदलने के बाद नया पैकेज version publish करें।
- [Plugin manifest](/hi/plugins/manifest) देखें।
- `clawhub package validate <path-to-plugin>` फिर से चलाएँ।

### package-openclaw-unsupported-metadata

`package.json#openclaw` block में ऐसे fields हैं जो supported
...
- package entrypoints, compatibility, install, setup, और catalog metadata को
  supported `package.json#openclaw` fields में रखें।
- [package.json fields that affect discovery](/hi/plugins/manifest#packagejson-fields-that-affect-discovery) देखें।
- `clawhub package validate <path-to-plugin>` फिर से चलाएँ।
...
पैकेज को उस artifact में pack नहीं किया जा सकता जिसकी ClawHub जाँच करेगा या
publish करेगा।

- पैकेज root से `npm pack --dry-run` चलाएँ।
- invalid package metadata, broken lifecycle scripts, या files entries ठीक करें जो
  packing fail कराते हैं।
...
- `files`, `.npmignore`, या build output update करें ताकि घोषित entrypoints
  शामिल हों।
- [Plugin entry points](/hi/plugins/sdk-entrypoints) देखें।
- `clawhub package validate <path-to-plugin>` फिर से चलाएँ।
...
packed artifact में OpenClaw metadata गायब है जो आपके source
...
- `openclaw.plugin.json` में एक non-empty `name` फ़ील्ड जोड़ें।
...
मैनिफेस्ट `contracts` के अंदर असमर्थित keys घोषित करता है।

- `contracts` के अंतर्गत प्रत्येक key की तुलना
  [contracts संदर्भ](/hi/plugins/manifest#contracts-reference) से करें।
- असमर्थित contract keys हटाएँ।
- runtime व्यवहार को Plugin registration code में ले जाएँ, और `contracts`
...
- root-barrel imports को केंद्रित public subpath imports से बदलें।
...
Plugin ऐसा SDK path import करता है जो bundled plugins या internal
compatibility के लिए आरक्षित है।

- आरक्षित OpenClaw internal SDK imports को documented public
  `openclaw/plugin-sdk/*` subpaths से बदलें।
- यदि व्यवहार के लिए कोई public SDK नहीं है, तो helper को अपने पैकेज के अंदर रखें या
...
- `loadSessionStore(...)` को केवल तब तक रखें जब तक आपकी घोषित compatibility range
  अभी भी पुराने OpenClaw versions का समर्थन करती है जिनके लिए इसकी आवश्यकता है।
...
- किसी मौजूदा session entry पर फ़ील्ड update करते समय `patchSessionEntry(...)` का उपयोग करें।
...
- whole-store write helpers को केवल तब तक रखें जब तक आपकी घोषित compatibility range
  अभी भी पुराने OpenClaw versions का समर्थन करती है जिनके लिए उनकी आवश्यकता है।
- [Runtime API](/hi/plugins/sdk-runtime#agent-session-state) और
  [Plugin SDK subpaths](/hi/plugins/sdk-subpaths) देखें।
- `clawhub package validate <path-to-plugin>` फिर से चलाएँ।
...
- legacy helper को केवल तब तक रखें जब तक आपकी घोषित compatibility range अभी भी
  पुराने OpenClaw versions का समर्थन करती है जिनके लिए इसकी आवश्यकता है।
- [Runtime API](/hi/plugins/sdk-runtime#agent-session-state) और
  [Plugin SDK subpaths](/hi/plugins/sdk-subpaths) देखें।
- `clawhub package validate <path-to-plugin>` फिर से चलाएँ।
...
मैनिफेस्ट legacy या पुराने channel env-var metadata का उपयोग करता है, बिना उस current
...
- documented versioned schema का उपयोग केवल OpenClaw द्वारा प्रकाशित किए जाने के बाद करें।
- `clawhub package validate <path-to-plugin>` फिर से चलाएँ।
