---
read_when:
    - आपने clawhub package validate चलाया है और आपको Plugin निष्कर्षों को ठीक करना है
...
x-i18n:
    generated_at: "2026-07-03T17:19:28Z"
    model: gpt-5.5
...
स्वचालित पैकेज स्कैन से मिले निष्कर्ष भी दिखा सकता है। यह पृष्ठ लेखक-संबंधी निष्कर्षों को कवर करता है, जिसका अर्थ है
वे निष्कर्ष जिन्हें Plugin लेखक अपने पैकेज मेटाडेटा, मैनिफेस्ट, SDK
इम्पोर्ट, या प्रकाशित आर्टिफैक्ट में ठीक कर सकता है।

यह आंतरिक Plugin Inspector कवरेज निष्कर्षों को कवर नहीं करता। यदि किसी पूर्ण रिपोर्ट में
लेखक के लिए सुधार मार्गदर्शन के बिना स्कैनर रखरखाव कोड शामिल हैं, तो वे
Plugin लेखकों के बजाय OpenClaw अनुरक्षकों के लिए हैं।
...
| `channel-env-vars`                      | [चैनल env vars को वर्तमान मेटाडेटा में मिरर करें](/hi/clawhub/plugin-validation-fixes#channel-env-vars)                                |
...
पैकेज रूट में `package.json` शामिल नहीं है, इसलिए ClawHub
npm पैकेज, संस्करण, एंट्रीपॉइंट, या OpenClaw मेटाडेटा की पहचान नहीं कर सकता।
...
- जब पैकेज ClawHub के माध्यम से प्रकाशित या इंस्टॉल किया जाएगा, तो
  संगतता और इंस्टॉल मेटाडेटा जोड़ें।
...
पैकेज मेटाडेटा मौजूद है, लेकिन यह OpenClaw रनटाइम
एंट्रीपॉइंट घोषित नहीं करता।

- नेटिव Plugin एंट्रीपॉइंट के लिए `openclaw.extensions` जोड़ें।
- जब प्रकाशित पैकेज को बिल्ट
  JavaScript लोड करना चाहिए, तब `openclaw.runtimeExtensions` जोड़ें।
...
पैकेज OpenClaw एंट्रीपॉइंट घोषित करता है, लेकिन संदर्भित फ़ाइल
सत्यापित किए जा रहे पैकेज से गायब है।

- `openclaw.extensions`, `openclaw.runtimeExtensions`,
  `openclaw.setupEntry`, और `openclaw.runtimeSetupEntry` में प्रत्येक पाथ जाँचें।
...
- वह OpenClaw Plugin API संस्करण या semver फ़्लोर उपयोग करें जिसके विरुद्ध आपने बनाया और परीक्षण किया है।
- इसे पैकेज संस्करण से अलग रखें। पैकेज संस्करण
  Plugin रिलीज़ का वर्णन करता है; `openclaw.compat.pluginApi` होस्ट API कॉन्ट्रैक्ट का वर्णन करता है।
- [डिस्कवरी को प्रभावित करने वाले package.json फ़ील्ड](/hi/plugins/manifest#packagejson-fields-that-affect-discovery) देखें।
- `clawhub package validate <path-to-plugin>` फिर से चलाएँ।
...
- यदि `openclaw.plugin.json` में भी `version` है, तो उसे मिलान के लिए अपडेट करें या
  जब पैकेज मेटाडेटा आधिकारिक हो, तब पुराने मैनिफेस्ट संस्करण मेटाडेटा को हटाएँ।
...
- पैकेज एंट्रीपॉइंट, संगतता, इंस्टॉल, सेटअप, और कैटलॉग मेटाडेटा
  समर्थित `package.json#openclaw` फ़ील्ड में रखें।
- [डिस्कवरी को प्रभावित करने वाले package.json फ़ील्ड](/hi/plugins/manifest#packagejson-fields-that-affect-discovery) देखें।
- `clawhub package validate <path-to-plugin>` फिर से चलाएँ।
...
- अमान्य पैकेज मेटाडेटा, टूटे lifecycle scripts, या ऐसी files प्रविष्टियाँ ठीक करें
...
पैक किए गए आर्टिफैक्ट में वह OpenClaw मेटाडेटा गायब है जो आपके स्रोत
पैकेज में मौजूद है।

- `npm pack --dry-run` चलाएँ और शामिल मेटाडेटा फ़ाइलों का निरीक्षण करें।
- सुनिश्चित करें कि पैक किए गए आर्टिफैक्ट में `package.json` में `openclaw` ब्लॉक शामिल हो।
- जब पैकेज नेटिव
  OpenClaw Plugin हो, तो सुनिश्चित करें कि `openclaw.plugin.json` शामिल हो।
...
नेटिव Plugin मेनिफ़ेस्ट में डिस्प्ले नाम शामिल नहीं है।

- `openclaw.plugin.json` में एक गैर-खाली `name` फ़ील्ड जोड़ें।
- `name` को मनुष्यों के लिए पढ़ने योग्य रखें और `id` को स्थिर मशीन id के रूप में रखें।
- [Plugin मेनिफ़ेस्ट](/hi/plugins/manifest) देखें।
- `clawhub package validate <path-to-plugin>` फिर से चलाएँ।

### manifest-unknown-fields

Plugin मेनिफ़ेस्ट में शीर्ष-स्तरीय फ़ील्ड हैं जिनका OpenClaw समर्थन नहीं करता।

- प्रत्येक शीर्ष-स्तरीय फ़ील्ड की तुलना
  [मेनिफ़ेस्ट फ़ील्ड संदर्भ](/hi/plugins/manifest#top-level-field-reference) से करें।
- `openclaw.plugin.json` से कस्टम फ़ील्ड हटाएँ।
- पैकेज या इंस्टॉल मेटाडेटा को मेनिफ़ेस्ट के बजाय समर्थित `package.json#openclaw` फ़ील्ड में ले जाएँ।
- `clawhub package validate <path-to-plugin>` फिर से चलाएँ।

### manifest-unknown-contracts

मेनिफ़ेस्ट `contracts` के अंदर असमर्थित कुंजियाँ घोषित करता है।
...
- रनटाइम व्यवहार को Plugin पंजीकरण कोड में ले जाएँ, और `contracts` को स्थिर capability स्वामित्व मेटाडेटा तक सीमित रखें।
- `clawhub package validate <path-to-plugin>` फिर से चलाएँ।
...
Plugin पुराने रूट SDK बैरल से इम्पोर्ट करता है:
`openclaw/plugin-sdk`.
...
Plugin ऐसे SDK पथ को इम्पोर्ट करता है जो बंडल किए गए Plugins या आंतरिक
compatibility के लिए आरक्षित है।

- आरक्षित OpenClaw आंतरिक SDK इम्पोर्ट को दस्तावेजीकृत सार्वजनिक
  `openclaw/plugin-sdk/*` सबपाथ से बदलें।
- यदि उस व्यवहार के लिए कोई सार्वजनिक SDK नहीं है, तो हेल्पर को अपने पैकेज के अंदर रखें या
...
Plugin अभी भी पुराने पूरे-session-store हेल्पर
...
- `loadSessionStore(...)` को केवल तब तक रखें जब तक आपकी घोषित compatibility सीमा
...
Plugin अभी भी `saveSessionStore` या `updateSessionStore` जैसे पुराने पूरे-session-store write helper का उपयोग करता है।
...
- पूरे session store ऑब्जेक्ट को लोड करने, बदलने और सहेजने से बचें।
- पूरे-store write helpers को केवल तब तक रखें जब तक आपकी घोषित compatibility सीमा
...
Plugin अभी भी `resolveSessionFilePath` या `resolveAndPersistSessionFile` जैसे पुराने session file-path helpers का उपयोग करता है।
...
- जब कोड कोई transcript operation तैयार कर रहा हो, तो transcript identity या target helpers का उपयोग करें।
...
Plugin अभी भी पुराने transcript file target helper
...
- legacy helper को केवल तब तक रखें जब तक आपकी घोषित compatibility सीमा अभी भी
...
Plugin अभी भी `appendSessionTranscriptMessage` या `emitSessionTranscriptUpdate` जैसे पुराने low-level transcript helpers का उपयोग करता है।

- transcript append के लिए `appendSessionTranscriptMessageByIdentity(...)` का उपयोग करें।
...
- low-level transcript helpers को केवल तब तक रखें जब तक आपकी घोषित compatibility सीमा
  अभी भी पुराने OpenClaw संस्करणों का समर्थन करती है जिन्हें उनकी आवश्यकता है।
- [Runtime API](/hi/plugins/sdk-runtime#agent-session-state) और
  [Plugin SDK सबपाथ](/hi/plugins/sdk-subpaths) देखें।
- `clawhub package validate <path-to-plugin>` फिर से चलाएँ।
...
- `before_agent_start` को केवल तब तक रखें जब तक आपकी घोषित compatibility सीमा अभी भी
  पुराने OpenClaw संस्करणों का समर्थन करती है जिन्हें इसकी आवश्यकता है।
...
मेनिफ़ेस्ट अभी भी legacy `providerAuthEnvVars` provider auth metadata का उपयोग करता है।
...
मेनिफ़ेस्ट legacy या पुराने channel env-var metadata का उपयोग करता है, जिसमें वर्तमान
setup या config metadata नहीं है जिसकी ClawHub अपेक्षा करता है।

- channel env-var metadata को declarative रखें ताकि OpenClaw channel runtime लोड किए बिना setup status की जाँच कर सके।
- env-driven channel setup को आपके plugin shape द्वारा उपयोग किए गए वर्तमान setup, channel config, या
...
- [Plugin मेनिफ़ेस्ट](/hi/plugins/manifest) और
...
available के रूप में नहीं पहचानता।
...
पैकेज एक असमर्थित security manifest file शिप करता है।

- जब तक OpenClaw कोई versioned security
  manifest schema और ClawHub behavior दस्तावेजीकृत नहीं करता, तब तक `openclaw.security.json` हटाएँ।
- manifest contract मौजूद होने तक security-sensitive behavior को अपने सार्वजनिक package docs या
