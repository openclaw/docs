...
ClawHub प्रकाशन से पहले Plugin पैकेजों को वैलिडेट करता है और स्वचालित
पैकेज स्कैन से मिले निष्कर्ष भी दिखा सकता है। यह पेज लेखक-सामने आने वाले
निष्कर्षों को कवर करता है, यानी वे निष्कर्ष जिन्हें Plugin लेखक अपने पैकेज
मेटाडेटा, मैनिफेस्ट, SDK imports, या प्रकाशित आर्टिफैक्ट में ठीक कर सकता है।

यह आंतरिक Plugin Inspector कवरेज निष्कर्षों को कवर नहीं करता। अगर किसी पूरी
रिपोर्ट में लेखक के लिए सुधार मार्गदर्शन के बिना स्कैनर मेंटेनेंस कोड हों, तो
वे Plugin लेखकों के बजाय OpenClaw मेंटेनर के लिए हैं।
...
| `sdk-load-session-store`                | [पूरे-session-store एक्सेस को बदलें](/hi/clawhub/plugin-validation-fixes#sdk-load-session-store)                                   |
...
| `sdk-session-transcript-low-level`      | [लो-लेवल ट्रांसक्रिप्ट helpers बदलें](/hi/clawhub/plugin-validation-fixes#sdk-session-transcript-low-level)                       |
| `legacy-before-agent-start`             | [before_agent_start बदलें](/hi/clawhub/plugin-validation-fixes#legacy-before-agent-start)                                        |
| `provider-auth-env-vars`                | [प्रोवाइडर env vars को सेटअप मेटाडेटा में ले जाएं](/hi/clawhub/plugin-validation-fixes#provider-auth-env-vars)                             |
| `channel-env-vars`                      | [चैनल env vars को मौजूदा मेटाडेटा में मिरर करें](/hi/clawhub/plugin-validation-fixes#channel-env-vars)                                |
| `security-manifest-schema-unavailable`  | [अनुपलब्ध सिक्योरिटी मैनिफेस्ट स्कीमा संदर्भ हटाएं](/hi/clawhub/plugin-validation-fixes#security-manifest-schema-unavailable) |
| `unrecognized-security-manifest`        | [असमर्थित सिक्योरिटी मैनिफेस्ट फ़ाइलें हटाएं](/hi/clawhub/plugin-validation-fixes#unrecognized-security-manifest)                   |
...
- जब पैकेज कोई OpenClaw Plugin शिप करता है, तो `openclaw` ब्लॉक जोड़ें।
- न्यूनतम पैकेज उदाहरण के लिए [Plugin बनाना](/hi/plugins/building-plugins) और
  पैकेज बनाम मैनिफेस्ट विभाजन के लिए [Plugin मैनिफेस्ट](/hi/plugins/manifest#manifest-versus-packagejson)
  देखें।
...
- जब प्रकाशित पैकेज को बनाया गया JavaScript लोड करना चाहिए, तो
...
पैकेज एक OpenClaw एंट्रीपॉइंट घोषित करता है, लेकिन संदर्भित फ़ाइल वैलिडेट किए
जा रहे पैकेज में मौजूद नहीं है।

- `openclaw.extensions`, `openclaw.runtimeExtensions`,
  `openclaw.setupEntry`, और `openclaw.runtimeSetupEntry` में हर पाथ जांचें।
- अगर एंट्रीपॉइंट `dist` में जनरेट होता है, तो पैकेज बिल्ड करें।
- अगर एंट्रीपॉइंट स्थानांतरित हो गया है, तो मेटाडेटा अपडेट करें।
...
- इसे पैकेज संस्करण से अलग रखें। पैकेज संस्करण Plugin रिलीज का वर्णन करता है;
...
- पैकेज में कोई भी OpenClaw बिल्ड मेटाडेटा जांचें, जैसे रिलीज के दौरान उपयोग
...
- पैकेज रिलीज संस्करण के रूप में `package.json#version` को प्राथमिकता दें।
- अगर `openclaw.plugin.json` में भी `version` है, तो उसे मिलाने के लिए अपडेट
  करें या जब पैकेज मेटाडेटा प्रामाणिक हो तो पुराने मैनिफेस्ट संस्करण मेटाडेटा
  को हटा दें।
...
पैकेज को उस आर्टिफैक्ट में पैक नहीं किया जा सकता जिसे ClawHub जांचेगा या
प्रकाशित करेगा।

- पैकेज रूट से `npm pack --dry-run` चलाएं।
- अमान्य पैकेज मेटाडेटा, टूटे lifecycle scripts, या files entries ठीक करें जो
  पैकिंग को विफल बनाते हैं।
- अगर यह पैकेज सार्वजनिक प्रकाशन के लिए है, तो `private: true` हटाएं।
- `clawhub package validate <path-to-plugin>` फिर से चलाएं।
...
- `npm pack --dry-run` चलाएं और शामिल की जाने वाली फ़ाइलों की जांच करें।
...
पैक किए गए आर्टिफैक्ट में वह OpenClaw मेटाडेटा नहीं है जो आपके सोर्स पैकेज में
मौजूद है।

- `npm pack --dry-run` चलाएं और शामिल मेटाडेटा फ़ाइलों की जांच करें।
- सुनिश्चित करें कि `package.json` में पैक किए गए आर्टिफैक्ट में `openclaw`
  ब्लॉक शामिल है।
...
Plugin मेनिफेस्ट में शीर्ष-स्तर के फ़ील्ड हैं जिनका OpenClaw समर्थन नहीं करता.

- प्रत्येक शीर्ष-स्तर फ़ील्ड की तुलना
  [मेनिफेस्ट फ़ील्ड संदर्भ](/hi/plugins/manifest#top-level-field-reference) से करें.
- `openclaw.plugin.json` से कस्टम फ़ील्ड हटाएँ.
- पैकेज या इंस्टॉल मेटाडेटा को मेनिफेस्ट के बजाय समर्थित `package.json#openclaw` फ़ील्ड में ले जाएँ.
- `clawhub package validate <path-to-plugin>` फिर से चलाएँ.

### manifest-unknown-contracts

मेनिफेस्ट `contracts` के अंदर असमर्थित कुंजियाँ घोषित करता है.
...
Plugin अप्रचलित रूट SDK barrel से आयात करता है:
`openclaw/plugin-sdk`.

- रूट-barrel आयातों को केंद्रित सार्वजनिक subpath आयातों से बदलें.
...
- संकीर्ण आयात खोजने के लिए [आयात conventions](/hi/plugins/building-plugins#import-conventions) और
  [Plugin SDK subpaths](/hi/plugins/sdk-subpaths) का उपयोग करें.
- `clawhub package validate <path-to-plugin>` फिर से चलाएँ.

### reserved-sdk-import

Plugin bundled plugins या आंतरिक संगतता के लिए आरक्षित SDK path आयात करता है.
...
- `loadSessionStore(...)` को केवल तब तक रखें जब तक आपकी घोषित संगतता सीमा
  अभी भी उन पुराने OpenClaw versions का समर्थन करती है जिन्हें इसकी आवश्यकता है.
...
Plugin अभी भी `saveSessionStore` या `updateSessionStore` जैसे अप्रचलित whole-session-store write helper का उपयोग करता है.

- किसी मौजूदा session entry पर फ़ील्ड अपडेट करते समय `patchSessionEntry(...)` का उपयोग करें.
...
- whole-store write helpers को केवल तब तक रखें जब तक आपकी घोषित संगतता सीमा
  अभी भी उन पुराने OpenClaw versions का समर्थन करती है जिन्हें उनकी आवश्यकता है.
- [Runtime API](/hi/plugins/sdk-runtime#agent-session-state) और
  [Plugin SDK subpaths](/hi/plugins/sdk-subpaths) देखें.
- `clawhub package validate <path-to-plugin>` फिर से चलाएँ.
...
Plugin अभी भी `resolveSessionFilePath` या `resolveAndPersistSessionFile` जैसे अप्रचलित session file-path helpers का उपयोग करता है.
...
- जब कोड transcript operation तैयार कर रहा हो, तब transcript identity या target helpers का उपयोग करें.
...
- जब कोड को केवल सार्वजनिक session identity चाहिए, तब `resolveSessionTranscriptIdentity(...)` का उपयोग करें.
- जब कोड को structured transcript operation target चाहिए, तब `resolveSessionTranscriptTarget(...)` का उपयोग करें.
...
- legacy helper को केवल तब तक रखें जब तक आपकी घोषित संगतता सीमा अभी भी
  उन पुराने OpenClaw versions का समर्थन करती है जिन्हें इसकी आवश्यकता है.
- [Runtime API](/hi/plugins/sdk-runtime#agent-session-state) और
  [Plugin SDK subpaths](/hi/plugins/sdk-subpaths) देखें.
- `clawhub package validate <path-to-plugin>` फिर से चलाएँ.
...
Plugin अभी भी `appendSessionTranscriptMessage` या `emitSessionTranscriptUpdate` जैसे अप्रचलित low-level transcript helpers का उपयोग करता है.
...
- structured transcript runtime surface को प्राथमिकता दें ताकि OpenClaw सही transaction boundaries और identity handling लागू कर सके.
...
मेनिफेस्ट अभी भी legacy `providerAuthEnvVars` provider auth metadata का उपयोग करता है.
...
मेनिफेस्ट legacy या पुराने channel env-var metadata का उपयोग करता है, बिना उस मौजूदा
setup या config metadata के जिसकी ClawHub अपेक्षा करता है.

- channel env-var metadata को declarative रखें ताकि OpenClaw channel runtime लोड किए बिना
  setup status की जाँच कर सके.
- env-driven channel setup को आपके Plugin आकार द्वारा उपयोग किए जाने वाले मौजूदा setup, channel config, या
...
- जब तक OpenClaw versioned security manifest schema और ClawHub behavior को document नहीं करता,
  `openclaw.security.json` हटाएँ.
- जब तक manifest contract मौजूद नहीं है, security-sensitive behavior को अपने public package docs या
