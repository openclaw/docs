...
x-i18n:
    generated_at: "2026-07-05T06:16:33Z"
    model: gpt-5.5
...
ClawHub प्रकाशित करने से पहले Plugin पैकेजों को सत्यापित करता है और स्वचालित पैकेज स्कैन से मिली findings भी दिखा सकता है। यह पेज लेखक-सामने वाली findings को कवर करता है, जिसका अर्थ है वे findings जिन्हें Plugin लेखक अपने पैकेज metadata, manifest, SDK imports, या प्रकाशित artifact में ठीक कर सकता है।

यह आंतरिक Plugin Inspector coverage findings को कवर नहीं करता। अगर किसी पूर्ण रिपोर्ट में लेखक remediation guidance के बिना scanner maintenance codes शामिल हैं, तो वे Plugin लेखकों के बजाय OpenClaw maintainers के लिए हैं।
...
- जब प्रकाशित पैकेज को निर्मित JavaScript load करना चाहिए, तो `openclaw.runtimeExtensions` जोड़ें।
...
- अगर entrypoint `dist` में generated है, तो पैकेज build करें।
- अगर entrypoint स्थानांतरित हो गया है, तो metadata update करें।
...
पैकेज वह OpenClaw Plugin API range घोषित नहीं करता जिसका वह समर्थन करता है।

- `package.json` में `openclaw.compat.pluginApi` जोड़ें।
- वह OpenClaw Plugin API version या semver floor उपयोग करें जिसके विरुद्ध आपने build और test किया था।
...
- अगर `openclaw.plugin.json` में भी `version` है, तो उसे match करने के लिए update करें या जब package metadata authoritative हो तो stale manifest version metadata हटाएं।
...
`package.json#openclaw` block में ऐसे fields हैं जो OpenClaw package metadata के रूप में समर्थित नहीं हैं।
...
- invalid package metadata, टूटे lifecycle scripts, या files entries को ठीक करें जिनसे packing fail होती है।
- अगर यह package public publishing के लिए intended है, तो `private: true` हटाएं।
- `clawhub package validate <path-to-plugin>` फिर से चलाएं।
...
- generated entrypoints को packing से पहले build करें।
...
packed artifact में OpenClaw metadata नहीं है जो आपके source package में मौजूद है।
...
Plugin ऐसा SDK पाथ इम्पोर्ट करता है जो बंडल किए गए Plugin या आंतरिक
अनुकूलता के लिए आरक्षित है।
...
- `loadSessionStore(...)` को केवल तब तक रखें जब तक आपकी घोषित अनुकूलता सीमा
...
- पूरे सेशन स्टोर ऑब्जेक्ट को लोड करने, बदलने और सहेजने से बचें।
- पूर्ण-स्टोर लेखन हेल्पर को केवल तब तक रखें जब तक आपकी घोषित अनुकूलता सीमा
...
- जब कोड किसी ट्रांसक्रिप्ट ऑपरेशन की तैयारी कर रहा हो, तब ट्रांसक्रिप्ट पहचान या लक्ष्य हेल्पर का उपयोग करें।
- लेगेसी ट्रांसक्रिप्ट फ़ाइल पाथ को बनाए न रखें या उन पर निर्भर न रहें।
- [रनटाइम API](/hi/plugins/sdk-runtime#agent-session-state) और
  [Plugin SDK सबपाथ](/hi/plugins/sdk-subpaths) देखें।
- `clawhub package validate <path-to-plugin>` फिर से चलाएँ।
...
- लेगेसी हेल्पर को केवल तब तक रखें जब तक आपकी घोषित अनुकूलता सीमा अब भी
...
- निम्न-स्तरीय ट्रांसक्रिप्ट हेल्पर को केवल तब तक रखें जब तक आपकी घोषित अनुकूलता सीमा
  अब भी उन पुराने OpenClaw संस्करणों का समर्थन करती है जिन्हें उनकी आवश्यकता है।
- [रनटाइम API](/hi/plugins/sdk-runtime#agent-session-state) और
  [Plugin SDK सबपाथ](/hi/plugins/sdk-subpaths) देखें।
- `clawhub package validate <path-to-plugin>` फिर से चलाएँ।
...
- प्रॉम्प्ट या कॉन्टेक्स्ट परिवर्तन कार्य को `before_prompt_build` में ले जाएँ।
- `before_agent_start` को केवल तब तक रखें जब तक आपकी घोषित अनुकूलता सीमा अब भी
  उन पुराने OpenClaw संस्करणों का समर्थन करती है जिन्हें इसकी आवश्यकता है।
- [हुक](/hi/plugins/hooks) और
  [Plugin अनुकूलता](/hi/plugins/compatibility) देखें।
- `clawhub package validate <path-to-plugin>` फिर से चलाएँ।

### provider-auth-env-vars

मैनिफेस्ट अभी भी लेगेसी `providerAuthEnvVars` प्रोवाइडर auth मेटाडेटा का उपयोग करता है।

- प्रोवाइडर env-var मेटाडेटा को `setup.providers[].envVars` में मिरर करें।
- `providerAuthEnvVars` को केवल अनुकूलता मेटाडेटा के रूप में रखें, जब तक आपकी समर्थित
...
- `channelEnvVars` को केवल अनुकूलता मेटाडेटा के रूप में रखें, जब तक पुराने समर्थित
...
- दस्तावेजीकृत वर्ज़न वाले स्कीमा का उपयोग केवल OpenClaw द्वारा प्रकाशित किए जाने के बाद करें।
- `clawhub package validate <path-to-plugin>` फिर से चलाएँ।
...
- `openclaw.security.json` को तब तक हटाएँ जब तक OpenClaw वर्ज़न वाला सुरक्षा
...
- [Plugin अनुकूलता](/hi/plugins/compatibility)
