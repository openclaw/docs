...
summary: प्रकाशित करने से पहले ClawHub Plugin पैकेज सत्यापन निष्कर्षों को सुधारें
title: Plugin सत्यापन सुधार
x-i18n:
    generated_at: "2026-07-03T02:48:09Z"
    model: gpt-5.5
...
ClawHub प्रकाशन से पहले Plugin पैकेजों को सत्यापित करता है और स्वचालित पैकेज स्कैन से मिले निष्कर्ष भी दिखा सकता है। यह पेज लेखक-सामना निष्कर्षों को कवर करता है, यानी वे निष्कर्ष जिन्हें Plugin लेखक अपने पैकेज मेटाडेटा, मैनिफ़ेस्ट, SDK इम्पोर्ट, या प्रकाशित आर्टिफ़ैक्ट में ठीक कर सकता है।

यह आंतरिक Plugin Inspector कवरेज निष्कर्षों को कवर नहीं करता। यदि किसी पूर्ण रिपोर्ट में लेखक के लिए सुधार मार्गदर्शन के बिना स्कैनर रखरखाव कोड शामिल हैं, तो वे Plugin लेखकों के बजाय OpenClaw मेंटेनरों के लिए हैं।
...
| `channel-env-vars`                      | [चैनल env vars को मौजूदा मेटाडेटा में प्रतिबिंबित करें](/hi/clawhub/plugin-validation-fixes#channel-env-vars)                                |
...
- [Plugin एंट्री पॉइंट्स](/hi/plugins/sdk-entrypoints) और [डिस्कवरी को प्रभावित करने वाले package.json फ़ील्ड](/hi/plugins/manifest#packagejson-fields-that-affect-discovery) देखें।
- `clawhub package validate <path-to-plugin>` फिर से चलाएँ।
...
- `openclaw.install` को समर्थित इंस्टॉल स्रोत, जैसे `clawhubSpec`, `npmSpec`, या `localPath` से भरें।
...
- न्यूनतम होस्ट संस्करण को उस होस्ट संस्करण रेंज के साथ संरेखित करें जिसे पैकेज वास्तव में सपोर्ट करता है।
- [डिस्कवरी को प्रभावित करने वाले package.json फ़ील्ड](/hi/plugins/manifest#packagejson-fields-that-affect-discovery) देखें।
- `clawhub package validate <path-to-plugin>` फिर से चलाएँ।
...
- यदि `openclaw.plugin.json` में भी `version` है, तो उसे मिलाने के लिए अपडेट करें या जब पैकेज मेटाडेटा अधिकृत हो, तो पुराने मैनिफ़ेस्ट संस्करण मेटाडेटा को हटाएँ।
...
- अमान्य पैकेज मेटाडेटा, टूटे lifecycle scripts, या ऐसे files entries ठीक करें जिनसे पैकिंग विफल होती है।
...
Plugin मैनिफेस्ट में ऐसे शीर्ष-स्तरीय फ़ील्ड हैं जिन्हें OpenClaw समर्थन नहीं करता।
...
मैनिफेस्ट `contracts` के भीतर असमर्थित कुंजियाँ घोषित करता है।
...
- संकीर्ण इम्पोर्ट खोजने के लिए [इम्पोर्ट कन्वेंशन](/hi/plugins/building-plugins#import-conventions) और
...
- यदि व्यवहार के लिए कोई सार्वजनिक SDK नहीं है, तो हेल्पर को अपने पैकेज के भीतर रखें या
...
Plugin अब भी अप्रचलित पूरे-सत्र-स्टोर हेल्पर
...
- `loadSessionStore(...)` को केवल तब तक रखें जब तक आपकी घोषित संगतता सीमा
  अब भी पुराने OpenClaw संस्करणों का समर्थन करती है जिन्हें इसकी आवश्यकता है।
...
Plugin अब भी अप्रचलित पूरे-सत्र-स्टोर लेखन हेल्पर जैसे
`saveSessionStore` या `updateSessionStore` का उपयोग करता है।

- मौजूदा सत्र एंट्री पर फ़ील्ड अपडेट करते समय `patchSessionEntry(...)` का उपयोग करें।
- सत्र एंट्री को बदलते या बनाते समय `upsertSessionEntry(...)` का उपयोग करें।
- पूरे सत्र स्टोर ऑब्जेक्ट को लोड करने, बदलने और सहेजने से बचें।
- पूरे-स्टोर लेखन हेल्पर केवल तब तक रखें जब तक आपकी घोषित संगतता सीमा
  अब भी पुराने OpenClaw संस्करणों का समर्थन करती है जिन्हें उनकी आवश्यकता है।
- [रनटाइम API](/hi/plugins/sdk-runtime#agent-session-state) और
  [Plugin SDK सबपाथ](/hi/plugins/sdk-subpaths) देखें।
- `clawhub package validate <path-to-plugin>` फिर से चलाएँ।
...
Plugin अब भी अप्रचलित सत्र फ़ाइल-पथ हेल्पर जैसे
...
- सत्र मेटाडेटा को स्थायी करने के लिए `patchSessionEntry(...)` या `upsertSessionEntry(...)` का उपयोग करें।
- जब कोड transcript ऑपरेशन तैयार कर रहा हो, तो transcript पहचान या लक्ष्य हेल्पर का उपयोग करें।
- विरासत transcript फ़ाइल पथों को स्थायी न करें या उन पर निर्भर न रहें।
- [रनटाइम API](/hi/plugins/sdk-runtime#agent-session-state) और
  [Plugin SDK सबपाथ](/hi/plugins/sdk-subpaths) देखें।
- `clawhub package validate <path-to-plugin>` फिर से चलाएँ।
...
Plugin अब भी अप्रचलित transcript फ़ाइल लक्ष्य हेल्पर
...
- विरासत transcript फ़ाइल लक्ष्यों को सीधे पढ़ने या बनाने से बचें।
- विरासत हेल्पर को केवल तब तक रखें जब तक आपकी घोषित संगतता सीमा अब भी
...
Plugin अब भी अप्रचलित निम्न-स्तरीय transcript हेल्पर जैसे
...
- संरचित transcript रनटाइम सतह को प्राथमिकता दें ताकि OpenClaw सही
  लेनदेन सीमाएँ और पहचान हैंडलिंग लागू कर सके।
- निम्न-स्तरीय transcript हेल्पर केवल तब तक रखें जब तक आपकी घोषित संगतता सीमा
  अब भी पुराने OpenClaw संस्करणों का समर्थन करती है जिन्हें उनकी आवश्यकता है।
- [रनटाइम API](/hi/plugins/sdk-runtime#agent-session-state) और
  [Plugin SDK सबपाथ](/hi/plugins/sdk-subpaths) देखें।
- `clawhub package validate <path-to-plugin>` फिर से चलाएँ।
...
Plugin अब भी विरासत `before_agent_start` hook का उपयोग करता है।
...
मैनिफेस्ट अब भी विरासत `providerAuthEnvVars` provider auth मेटाडेटा का उपयोग करता है।

- provider env-var मेटाडेटा को `setup.providers[].envVars` में मिरर करें।
...
मैनिफेस्ट विरासत या पुराने चैनल env-var मेटाडेटा का उपयोग करता है जिसमें वर्तमान
...
- env-चालित चैनल setup को आपके Plugin आकार द्वारा उपयोग किए गए वर्तमान setup, चैनल config, या
...
- दस्तावेजीकृत versioned schema का उपयोग केवल OpenClaw द्वारा प्रकाशित किए जाने के बाद करें।
- `clawhub package validate <path-to-plugin>` फिर से चलाएँ।
...
- जब तक OpenClaw versioned सुरक्षा मैनिफेस्ट schema और ClawHub व्यवहार का दस्तावेजीकरण नहीं करता,
