...
x-i18n:
    generated_at: "2026-07-03T23:32:24Z"
    model: gpt-5.5
...
स्वचालित पैकेज स्कैन से मिले निष्कर्ष भी दिखा सकता है। यह पृष्ठ लेखक-उन्मुख निष्कर्षों को कवर करता है, यानी
वे निष्कर्ष जिन्हें Plugin लेखक अपने पैकेज मेटाडेटा, मैनिफेस्ट, SDK
आयातों, या प्रकाशित आर्टिफैक्ट में ठीक कर सकता है।

यह आंतरिक Plugin Inspector कवरेज निष्कर्षों को कवर नहीं करता। यदि पूरी रिपोर्ट में
लेखक के लिए सुधार मार्गदर्शन के बिना स्कैनर रखरखाव कोड हैं, तो वे
Plugin लेखकों के बजाय OpenClaw मेंटेनरों के लिए हैं।
...
| `legacy-root-sdk-import`                | [रूट SDK आयात बदलें](/hi/clawhub/plugin-validation-fixes#legacy-root-sdk-import)                                             |
| `reserved-sdk-import`                   | [आरक्षित SDK आयात हटाएँ](/hi/clawhub/plugin-validation-fixes#reserved-sdk-import)                                             |
| `sdk-load-session-store`                | [पूरे-session-store पहुँच को बदलें](/hi/clawhub/plugin-validation-fixes#sdk-load-session-store)                                   |
| `sdk-session-store-write`               | [पूरे-session-store लेखन को बदलें](/hi/clawhub/plugin-validation-fixes#sdk-session-store-write)                                  |
| `sdk-session-file-helper`               | [session फ़ाइल-पथ हेल्पर बदलें](/hi/clawhub/plugin-validation-fixes#sdk-session-file-helper)                                   |
| `sdk-session-transcript-file-target`    | [लेगेसी transcript फ़ाइल लक्ष्य बदलें](/hi/clawhub/plugin-validation-fixes#sdk-session-transcript-file-target)                   |
| `sdk-session-transcript-low-level`      | [लो-लेवल transcript हेल्पर बदलें](/hi/clawhub/plugin-validation-fixes#sdk-session-transcript-low-level)                       |
...
| `security-manifest-schema-unavailable`  | [अनुपलब्ध सुरक्षा मैनिफेस्ट schema संदर्भ हटाएँ](/hi/clawhub/plugin-validation-fixes#security-manifest-schema-unavailable) |
...
- जब पैकेज ClawHub के माध्यम से प्रकाशित या
  इंस्टॉल किया जाएगा, तब संगतता और इंस्टॉल मेटाडेटा जोड़ें।
...
- जब प्रकाशित पैकेज को बिल्ट JavaScript लोड करना चाहिए, तब
  `openclaw.runtimeExtensions` जोड़ें।
- सभी एंट्रीपॉइंट पथों को पैकेज डायरेक्टरी के अंदर रखें।
...
- `openclaw.extensions`, `openclaw.runtimeExtensions`,
  `openclaw.setupEntry`, और `openclaw.runtimeSetupEntry` में हर पथ जाँचें।
- यदि एंट्रीपॉइंट `dist` में जेनरेट होता है, तो पैकेज बिल्ड करें।
...
- जब एक से अधिक इंस्टॉल स्रोत उपलब्ध हों, तब
...
पैकेज उस OpenClaw Plugin API रेंज को घोषित नहीं करता जिसे यह समर्थन करता है।

- `package.json` में `openclaw.compat.pluginApi` जोड़ें।
- उस OpenClaw Plugin API संस्करण या semver न्यूनतम सीमा का उपयोग करें जिसके विरुद्ध आपने
  बिल्ड और परीक्षण किया है।
- इसे पैकेज संस्करण से अलग रखें। पैकेज संस्करण
  Plugin रिलीज़ का वर्णन करता है; `openclaw.compat.pluginApi` होस्ट API अनुबंध का वर्णन करता है।
- [डिस्कवरी को प्रभावित करने वाले package.json फ़ील्ड](/hi/plugins/manifest#packagejson-fields-that-affect-discovery) देखें।
- `clawhub package validate <path-to-plugin>` फिर से चलाएँ।
...
- पैकेज में कोई भी OpenClaw बिल्ड मेटाडेटा जाँचें, जैसे रिलीज़ के दौरान उपयोग किया गया
  OpenClaw संस्करण।
- न्यूनतम होस्ट संस्करण को उस होस्ट संस्करण रेंज से संरेखित करें जिसे पैकेज
...
- यदि `openclaw.plugin.json` में भी `version` है, तो उसे मेल खाने के लिए अपडेट करें या
...
- पैकेज एंट्रीपॉइंट, संगतता, इंस्टॉल, सेटअप, और कैटलॉग मेटाडेटा को
...
- अमान्य पैकेज मेटाडेटा, टूटे lifecycle scripts, या files entries को ठीक करें जिनसे
  पैकिंग विफल होती है।
...
- पैकिंग से पहले जेनरेट किए गए एंट्रीपॉइंट बिल्ड करें।
- `files`, `.npmignore`, या बिल्ड आउटपुट को अपडेट करें ताकि घोषित एंट्रीपॉइंट
  शामिल हों।
- [Plugin एंट्री पॉइंट](/hi/plugins/sdk-entrypoints) देखें।
- `clawhub package validate <path-to-plugin>` फिर से चलाएँ।
...
- सुनिश्चित करें कि `package.json` में पैक किए गए आर्टिफैक्ट में `openclaw` ब्लॉक शामिल है।
- जब पैकेज नेटिव
  OpenClaw Plugin हो, तब सुनिश्चित करें कि `openclaw.plugin.json` शामिल है।
- `files` या `.npmignore` को अपडेट करें ताकि पैकेज मेटाडेटा बाहर न हो।
...
- रूट-बैरल इम्पोर्ट्स को केंद्रित सार्वजनिक उपपथ इम्पोर्ट्स से बदलें.
...
- संकीर्ण इम्पोर्ट खोजने के लिए [इम्पोर्ट परंपराएँ](/hi/plugins/building-plugins#import-conventions) और
  [Plugin SDK उपपथ](/hi/plugins/sdk-subpaths) का उपयोग करें.
- `clawhub package validate <path-to-plugin>` फिर से चलाएँ.

### reserved-sdk-import

Plugin ऐसा SDK पथ इम्पोर्ट करता है जो बंडल किए गए plugins या आंतरिक
संगतता के लिए आरक्षित है.

- आरक्षित OpenClaw आंतरिक SDK इम्पोर्ट्स को प्रलेखित सार्वजनिक
  `openclaw/plugin-sdk/*` उपपथों से बदलें.
...
- समर्थित इम्पोर्ट चुनने के लिए [Plugin SDK उपपथ](/hi/plugins/sdk-subpaths) और
...
- [रनटाइम API](/hi/plugins/sdk-runtime#agent-session-state) और
  [Plugin SDK उपपथ](/hi/plugins/sdk-subpaths) देखें.
- `clawhub package validate <path-to-plugin>` फिर से चलाएँ.

### sdk-session-store-write

Plugin अभी भी `saveSessionStore` या `updateSessionStore` जैसे अप्रचलित
पूरे-सेशन-स्टोर लेखन हेल्पर का उपयोग करता है.

- मौजूदा सेशन प्रविष्टि पर फ़ील्ड अपडेट करते समय `patchSessionEntry(...)` का उपयोग करें.
- सेशन प्रविष्टि बदलते या बनाते समय `upsertSessionEntry(...)` का उपयोग करें.
- पूरे सेशन स्टोर ऑब्जेक्ट को लोड करने, बदलने और सहेजने से बचें.
- पूरे-स्टोर लेखन हेल्पर केवल तब तक रखें जब तक आपकी घोषित संगतता सीमा
...
अप्रचलित सेशन फ़ाइल-पथ हेल्पर का उपयोग करता है.
...
- पुराने ट्रांसक्रिप्ट फ़ाइल पथों को स्थायी न करें या उन पर निर्भर न रहें.
- [रनटाइम API](/hi/plugins/sdk-runtime#agent-session-state) और
  [Plugin SDK उपपथ](/hi/plugins/sdk-subpaths) देखें.
- `clawhub package validate <path-to-plugin>` फिर से चलाएँ.
...
मैनिफेस्ट अभी भी पुराने `providerAuthEnvVars` प्रदाता प्रमाणीकरण मेटाडेटा का उपयोग करता है.
...
मैनिफेस्ट पुराने या पुरानेतर चैनल env-var मेटाडेटा का उपयोग करता है, बिना उस वर्तमान
सेटअप या कॉन्फ़िग मेटाडेटा के जिसकी ClawHub अपेक्षा करता है.

- चैनल env-var मेटाडेटा को घोषणात्मक रखें ताकि OpenClaw चैनल रनटाइम लोड किए बिना
  सेटअप स्थिति की जाँच कर सके.
- env-चालित चैनल सेटअप को आपके Plugin आकार द्वारा उपयोग किए जाने वाले वर्तमान सेटअप, चैनल कॉन्फ़िग, या
...
पैकेज `openclaw.security.json` को ऐसे स्कीमा संदर्भ के साथ भेजता है जिसे ClawHub
उपलब्ध के रूप में नहीं पहचानता.

- यदि स्कीमा URL केवल परामर्शात्मक है, तो उसे हटाएँ.
- प्रलेखित संस्करणित स्कीमा का उपयोग केवल OpenClaw द्वारा प्रकाशित किए जाने के बाद करें.
- `clawhub package validate <path-to-plugin>` फिर से चलाएँ.

### unrecognized-security-manifest

पैकेज एक असमर्थित सुरक्षा मैनिफेस्ट फ़ाइल भेजता है.
...
- जब तक मैनिफेस्ट कॉन्ट्रैक्ट मौजूद नहीं है, सुरक्षा-संवेदनशील व्यवहार को अपने सार्वजनिक पैकेज दस्तावेज़ों या
