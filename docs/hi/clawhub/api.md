...
x-i18n:
    generated_at: "2026-07-04T15:17:26Z"
    model: gpt-5.5
...
आप ClawHub की सार्वजनिक रीड API के ऊपर तृतीय-पक्ष कैटलॉग, डायरेक्टरी या खोज सतह बना सकते हैं। सार्वजनिक Skills मेटाडेटा और Skills फाइलें ClawHub के Skills लाइसेंस नियमों के तहत प्रकाशित की जाती हैं, जबकि API स्वयं rate-limited है और इसे जिम्मेदारी से उपयोग किया जाना चाहिए।

दिशानिर्देश:

- कैटलॉग लिस्टिंग के लिए `GET /api/v1/skills`, `GET /api/v1/search`, और `GET /api/v1/skills/{slug}` जैसे सार्वजनिक रीड endpoints का उपयोग करें।
- प्रतिक्रियाओं को कैश करें और बहुत आक्रामक polling करने के बजाय `429`, `Retry-After`, और rate-limit headers का सम्मान करें।
- लिस्टिंग दिखाते समय canonical ClawHub Skills URL से लिंक करें ताकि उपयोगकर्ता स्रोत registry record की जांच कर सकें।
- `https://clawhub.ai/<owner>/skills/<slug>` के रूप में canonical page URLs का उपयोग करें।
...
- अनुपस्थित/अमान्य token IP प्रवर्तन पर fallback करता है।
...
- `X-RateLimit-Remaining` / `RateLimit-Remaining`: मौजूद होने पर exact remaining budget;
  sharded सफल requests approximate global value लौटाने के बजाय इसे छोड़ देते हैं
...
- Compatibility के लिए अज्ञात query parameters अनदेखे किए जाते हैं।
- अमान्य values वाले ज्ञात query parameters `400` लौटाते हैं।
...
- `GET /api/v1/download?slug=&version=&tag=`
  ...
  - `clean` या `suspicious` scan वाले वर्तमान GitHub-backed Skills ClawHub bytes के बजाय
    JSON `public-github` handoff descriptor लौटाते हैं।
- `GET /api/v1/skills/export?startDate=&endDate=&limit=&cursor=`
  - Hosted Skills stored files के रूप में export किए जाते हैं।
  - `clean` या `suspicious` scan वाले वर्तमान GitHub-backed Skills
