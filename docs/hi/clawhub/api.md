...
x-i18n:
    generated_at: "2026-07-03T13:29:16Z"
    model: gpt-5.5
...
आप ClawHub के सार्वजनिक read API के ऊपर तृतीय-पक्ष कैटलॉग, डायरेक्टरी, या खोज सतह बना सकते हैं। सार्वजनिक skill metadata और skill files ClawHub के skill license rules के तहत प्रकाशित किए जाते हैं, जबकि API स्वयं rate-limited है और इसका उपयोग जिम्मेदारी से किया जाना चाहिए।

दिशानिर्देश:

- कैटलॉग सूचियों के लिए `GET /api/v1/skills`, `GET /api/v1/search`, और `GET /api/v1/skills/{slug}` जैसे सार्वजनिक read endpoints का उपयोग करें।
- प्रतिक्रियाओं को cache करें और आक्रामक polling के बजाय `429`, `Retry-After`, और rate-limit headers का सम्मान करें।
- सूचियां दिखाते समय canonical ClawHub skill URL से वापस link करें ताकि उपयोगकर्ता source registry record देख सकें।
- `https://clawhub.ai/<owner>/skills/<slug>` रूप में canonical page URLs का उपयोग करें।
- यह संकेत न दें कि ClawHub तृतीय-पक्ष site का अनुमोदन, सत्यापन, या संचालन करता है।
- सार्वजनिक API filters या auth boundaries को bypass करके hidden, private, या moderation-blocked content को mirror न करें।
...
- Authenticated requests (मान्य Bearer token): प्रति user bucket।
- Missing/invalid token IP प्रवर्तन पर वापस चला जाता है।
...
- `X-RateLimit-Reset`: Unix epoch seconds (पूर्ण reset time)
- `RateLimit-Reset`: reset तक delay seconds
- `X-RateLimit-Remaining` / `RateLimit-Remaining`: मौजूद होने पर exact remaining budget; sharded successful requests approximate global value लौटाने के बजाय इसे omit करते हैं
- `Retry-After`: `429` पर wait करने के लिए delay seconds
...
- v1 errors plain text (`text/plain; charset=utf-8`) हैं, जिनमें `400`, `401`, `403`, `404`, `429`, और blocked-download responses शामिल हैं।
- compatibility के लिए unknown query parameters ignore किए जाते हैं।
