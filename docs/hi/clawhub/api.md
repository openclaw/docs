...
x-i18n:
    generated_at: "2026-07-16T13:43:55Z"
    model: gpt-5.6
...
आप ClawHub के सार्वजनिक रीड API के आधार पर तृतीय-पक्ष कैटलॉग, डायरेक्टरी या खोज इंटरफ़ेस बना सकते हैं। सार्वजनिक स्किल मेटाडेटा और स्किल फ़ाइलें ClawHub के स्किल लाइसेंस नियमों के अंतर्गत प्रकाशित की जाती हैं, जबकि स्वयं API पर दर सीमा लागू है और इसका उपयोग ज़िम्मेदारी से किया जाना चाहिए।
...
- `https://clawhub.ai/<owner>/skills/<slug>` के प्रारूप में प्रामाणिक पेज URL का उपयोग करें।
- ऐसा संकेत न दें कि ClawHub तृतीय-पक्ष साइट का समर्थन, सत्यापन या संचालन करता है।
- सार्वजनिक API फ़िल्टर या प्रमाणीकरण सीमाओं को दरकिनार करके छिपी हुई, निजी या मॉडरेशन द्वारा अवरुद्ध सामग्री की प्रतिलिपि न बनाएँ।
...
- टोकन अनुपस्थित या अमान्य होने पर IP-आधारित प्रवर्तन लागू होता है।
...
- `X-RateLimit-Reset`: Unix युग सेकंड (पूर्ण रीसेट समय)
- `RateLimit-Reset`: रीसेट होने तक विलंब के सेकंड
- `X-RateLimit-Remaining` / `RateLimit-Remaining`: उपलब्ध होने पर सटीक शेष सीमा; शार्ड किए गए सफल अनुरोध अनुमानित वैश्विक मान लौटाने के बजाय इसे छोड़ देते हैं
...
- v1 त्रुटियाँ सादा टेक्स्ट (`text/plain; charset=utf-8`) होती हैं, जिनमें `400`, `401`, `403`, `404`, `429`, और अवरुद्ध-डाउनलोड प्रतिक्रियाएँ शामिल हैं।
...
- `GET /api/v1/skills?limit=&cursor=&sort=`
  - `sort`: `updated` (डिफ़ॉल्ट), `recommended` (`default`), `createdAt` (`newest`), `downloads`, `stars` (`rating`), लेगेसी इंस्टॉल उपनाम `installsCurrent`/`installs`/`installsAllTime`, `downloads`, `trending` से मैप होते हैं
...
- `GET /api/v1/download?slug=&version=&tag=`
  - होस्ट किए गए स्किल नियतात्मक ZIP बाइट लौटाते हैं।
  - `clean` या `suspicious` स्कैन वाले मौजूदा GitHub-समर्थित स्किल ClawHub बाइट के बजाय JSON `public-github` हैंडऑफ़ विवरणक लौटाते हैं।
- `GET /api/v1/skills/export?startDate=&endDate=&limit=&cursor=`
  - होस्ट किए गए स्किल संग्रहीत फ़ाइलों के रूप में निर्यात किए जाते हैं।
  - `clean` या `suspicious` स्कैन वाले मौजूदा GitHub-समर्थित स्किल `public-github` हैंडऑफ़ विवरणकों के रूप में निर्यात किए जाते हैं।
...
- `POST /api/v1/skills` (प्रकाशन, multipart को प्राथमिकता)
...
- `POST /api/v1/users/reserve` किसी स्वामी हैंडल के लिए रूट स्लग और निजी, बिना-रिलीज़ वाले पैकेज प्लेसहोल्डर आरक्षित करता है।
