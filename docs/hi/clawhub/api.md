...
summary: सार्वजनिक REST API (v1) अवलोकन और परंपराएँ।
x-i18n:
    generated_at: "2026-07-04T17:58:28Z"
    model: gpt-5.5
...
आप ClawHub के सार्वजनिक रीड API के ऊपर तृतीय-पक्ष कैटलॉग, डायरेक्टरी, या खोज सतह बना सकते हैं। सार्वजनिक skill metadata और skill files ClawHub के skill license rules के अंतर्गत प्रकाशित किए जाते हैं, जबकि API स्वयं rate-limited है और इसका उपयोग जिम्मेदारी से किया जाना चाहिए।

दिशानिर्देश:

- कैटलॉग सूचियों के लिए `GET /api/v1/skills`, `GET /api/v1/search`, और `GET /api/v1/skills/{slug}` जैसे सार्वजनिक रीड endpoints का उपयोग करें।
- प्रतिक्रियाओं को cache करें और आक्रामक polling के बजाय `429`, `Retry-After`, और rate-limit headers का सम्मान करें।
- listings दिखाते समय canonical ClawHub skill URL पर वापस लिंक करें ताकि उपयोगकर्ता source registry record देख सकें।
- canonical page URLs को `https://clawhub.ai/<owner>/skills/<slug>` रूप में उपयोग करें।
...
- Missing/invalid token IP प्रवर्तन पर वापस जाता है।
...
- `X-RateLimit-Remaining` / `RateLimit-Remaining`: मौजूद होने पर exact remaining budget; sharded successful requests अनुमानित
  global value लौटाने के बजाय इसे छोड़ देते हैं
...
- compatibility के लिए unknown query parameters को अनदेखा किया जाता है।
...
- `GET /api/v1/download?slug=&version=&tag=`
  ...
  - `clean` या `suspicious` scan वाले current GitHub-backed skills ClawHub bytes के बजाय
    JSON `public-github` handoff descriptor लौटाते हैं।
- `GET /api/v1/skills/export?startDate=&endDate=&limit=&cursor=`
  - Hosted skills stored files के रूप में export किए जाते हैं।
  - `clean` या `suspicious` scan वाले current GitHub-backed skills
    `public-github` handoff descriptors के रूप में export किए जाते हैं।
...
- `POST /api/v1/users/reserve` owner handle के लिए root slugs और private no-release package placeholders reserve करता है।
