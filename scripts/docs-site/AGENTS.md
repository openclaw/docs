...
- The sidebar presents the full source section tree using native disclosures. Preserve all source groups/links and open state through page navigation. The sticky offset includes the network header and compact TOC row; home and articles share the wide TOC rail. Keep the full-width frame, centered reading column and edge-aligned side rails consistent; mount the home hero after the main pane is replaced during navigation.
