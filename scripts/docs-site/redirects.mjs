...
export function resolveRedirects({ redirects, pages, localeCodes, prefixes, publicPath }) {
  const rules = new Map();
  for (const { source, destination } of redirects) {
    ...
    if (rules.has(route) && rules.get(route).value !== dest.value) {
      throw new Error(`Conflicting redirect rules for ${route}: ${rules.get(route).value} and ${dest.value}`);
    }
    rules.set(route, dest);
  }
  for (const prefix of prefixes) redirectSource(prefix);
  const knownLocaleCodes = new Set([...Object.keys(localeLabels), ...localeCodes]);
  const pageByRoute = new Map();
  for (const page of pages) {
    pageByRoute.set(page.route, page);
    if (page.route === "/") pageByRoute.set("/index", page);
    else pageByRoute.set(`${page.route}/index`, page);
  }
  ...
  const localized = (route, locale) => locale === "en" ? route : `/${locale}${route === "/" ? "" : route}`;
  ...
  function resolve(source, preferredLocale) {
    ...
    while (true) {
      ...
      if (explicit.locale !== undefined) locale = explicit.locale;
      ...
      if (!dest) throw new Error(`Redirect has no terminal Markdown page: ${[...visited].join(" -> ")}`);
      if (dest.external) return { destination: dest.value };
      if (dest.query !== undefined) query = dest.query;
      if (dest.fragment !== undefined) fragment = dest.fragment;
      ...
      if (path.posix.extname(next.route)
        && !pageByRoute.has(localized(next.route, nextLocale)) && !pageByRoute.has(next.route)
  ...
  for (const source of [...rules.keys()].sort()) {
    ...
    for (const locale of variants) {
      ...
      if (alias !== source && rules.has(alias)) continue;
      const resolved = resolve(alias, locale);
      for (const prefix of ["", ...prefixes]) {
        const route = redirectSource(`${prefix}${alias}`);
        const record = { source: route, ...resolved };
        const previous = records.get(route);
        if (previous && (previous.destination !== record.destination || previous.markdownTarget !== record.markdownTarget)) {
          throw new Error(`Conflicting generated redirects for ${route}: ${previous.destination} and ${record.destination}`);
        }
...
function redirectDestination(value) {
  ...
  if (/^(https?:)?\/\//u.test(value)) return { value, external: true };
  if (!value.startsWith("/")) throw new Error(`Unsupported redirect destination: ${value}`);
...
function validatePath(value) {
  if (/[\u0000-\u0020<>\\*:]/u.test(value)) throw new Error(`Unsafe redirect path: ${value}`);
  for (const segment of value.split("/")) {
    let decoded;
    try { decoded = decodeURIComponent(segment); } catch { throw new Error(`Unsafe redirect path: ${value}`); }
