...
- `scripts/docs-site/site-layout.css` adapts the community site's compact network header, ruled frame, connected panels, and footer to documentation. The full source section tree is available in the left sidebar, with native expandable groups. Desktop pages use the full browser width, with navigation on the left, a centered reading column, and the TOC on the right. The landing hero spans the area beside the sidebar; the article and TOC start together below it. Narrower layouts collapse the TOC and navigation. `hero-art.mjs` owns the shared ASCII artwork with reduced-motion and route cleanup. Shared semantic tokens and component foundations remain in `site-css.mjs`; structural panels are square, while controls retain rounded corners.
