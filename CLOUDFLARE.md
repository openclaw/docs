...
The Worker reads alias metadata directly on every Markdown object miss, even when canonical Markdown is cached. Canonical bodies use the existing asset cache under their canonical path; the Worker never stores the served body under the missing alias Markdown key. HTML bypasses the Worker cache and reads fresh R2 objects, including when an older HTML cache entry still exists. Updated alias metadata selects the current canonical target without invalidating unrelated cached documents.
